...
contract FeeManagerV1 is IFeeManager, IxBaseV1 {
    using EnumerableSet for EnumerableSet.AddressSet;
    using SafeERC20 for IERC20;

    EnumerableSet.AddressSet private _allowedRecharger;
    ...
    uint256 public rechargeAmount;

    constructor(address admin, address forwarder, address wrappedEther, address payer) IxBaseV1(admin, forwarder) {
        _setConfig(wrappedEther, payer, 0.1 ether, 0.3 ether);
    }
    ...
    function setConfig(
        ...
        uint256 amount
    ) external nonEIP2771 onlyOwner {
        require(wrappedEther != address(0), "wrapped ether address can not be zero");
        require(payer != address(0), "fee payer address can not be zero");

        _setConfig(wrappedEther, payer, balancethreshold, amount);
    }

    function _setConfig(address wrappedEther, address payer, uint256 balancethreshold, uint256 amount) internal {
        emit ConfigChanged(
            WETH,
            wrappedEther,
            feePayer,
    ...
    function registerRecharger(address recharger) external nonEIP2771 onlyOwner {
        require(recharger != address(0), "recharger can not be zero address");
        require(_allowedRecharger.add(recharger), "recharger already registered");

        emit RechargerRegistered(recharger, _msgSender());
    }

    function unregisterRecharger(address recharger) external nonEIP2771 onlyOwner {
        require(_allowedRecharger.remove(recharger), "recharger is not registered");

        emit RechargerUnregistered(recharger, _msgSender());
    }

    function getRegisteredRechargerCount() external view returns (uint256) {
        return _allowedRecharger.length();
    }

    function getRegisteredRechargerAt(uint256 index) external view returns (address) {
        return _allowedRecharger.at(index);
    }

    function getRegisteredRechargers() external view returns (address[] memory) {
        return _allowedRecharger.values();
    }

    function isRegisteredRecharger(address recharger) public view returns (bool) {
        return _allowedRecharger.contains(recharger);
    }

    function rechargeGasFee(address target) external nonReentrant nonEIP2771 {
        require(isRegisteredRecharger(_msgSender()), "only allowed recharger can recharge gas fee");
        ...
        try this.innerRechargeGasFee(target) {
            // success
            _log(target, RechargeStatus.OK, "");
        } catch (bytes memory reason) {
            // any other errors
            _log(target, RechargeStatus.ERROR, reason);
        }
    }

    function _log(address target, RechargeStatus status, bytes memory reason) internal {
        emit GasFeeRecharged(target, rechargeAmount, status, reason);
    }

    function innerRechargeGasFee(address target) external {
        // this external function can only be called by Exchange itself,
        // creating an internal transaction.
        require(msg.sender == address(this), "only can be called by FeeManager itself");
        require(target.balance <= threshold, "target balance is above threshold");
        require(IERC20(WETH).balanceOf(feePayer) >= rechargeAmount, "not enough WETH balance of fee payer");
        require(
            IERC20(WETH).allowance(feePayer, address(this)) >= rechargeAmount,
            "not enough WETH allowance of fee payer"
        );

        // transfer from feePayer to address(this)
        IERC20(WETH).safeTransferFrom(feePayer, address(this), rechargeAmount);

        // withdraw WETH
        IWETH(WETH).withdraw(rechargeAmount);

        // transfer ETH to target
        // solhint-disable-next-line avoid-low-level-calls
        (bool success, ) = target.call{value: rechargeAmount}("");
        require(success, "transfer ETH failed");

        require(address(this).balance <= 0, "invalid remaining ETH");

        require(IERC20(WETH).balanceOf(address(this)) <= 0, "invalid remaining WETH");
    }
     ...
     *
     * send Ether
     *     --> msg.data is not empty --> fallback()
     *     --> msg.data is empty --> receive() not exists --> fallback()
     *                           --> receive() exists --> receive()
     */
    receive() external payable {
        require(msg.sender == WETH, "only WETH can send ether");
    }
}
