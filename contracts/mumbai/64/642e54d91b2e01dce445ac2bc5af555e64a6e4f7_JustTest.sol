    ...
    RoyaltiesV1Luxy
{
    ...
    mapping(address => uint8) private bought;
    ...
    uint256 public constant MAX_BATCH_MINT = 4;
    uint256 public constant MAX_MINT_PER_WALLET = 6;
    uint256 public constant MAX_SUPPLY = 45;
    uint256 public constant DROP_START_TIME = 1654717464;
    uint256 public constant PRICE_PER_TOKEN = 0.01 ether;
    uint256 public constant LAUNCHPAD_FEE = 25;

    /**
     * @dev Initializes the contract by setting a `name` and a `symbol` to the token collection.
     */
    function __JustTest_init(
        ...
        __ERC721_init_unchained("JustTest", "JT");
        __ERC721Enumerable_init_unchained();
        __Ownable_init_unchained();
        __JustTest_init_unchained(
            baseURI_,
            luxy_,
            teamAddress_,
            artistAddress_
    ...
    function __JustTest_init_unchained(
        string memory baseURI_,
        IERC20Upgradeable luxy_,
        address teamAddress_,
        address artistAddress_
    ...
    function mint(uint8 num) external payable {
        ...
        require(PRICE_PER_TOKEN * num <= msg.value, "Invalid amount");
        require(
            bought[_msgSender()] + num <= MAX_MINT_PER_WALLET,
            "Exceeds max mint per account"
        );
        if (block.timestamp < DROP_START_TIME + 3 days) {
            require(
                luxy.balanceOf(_msgSender()) > 1000 ether,
        ...
        bought[_msgSender()] += num;

        uint256 fee = (msg.value * LAUNCHPAD_FEE) / 100;
        (bool teamTx, ) = payable(teamAddress).call{value: fee}("");
        (bool artistTx, ) = payable(artistAddress).call{value: msg.value - fee}(
            ""
        );
        require(teamTx && artistTx, "Fee Transfer failed.");
    }
