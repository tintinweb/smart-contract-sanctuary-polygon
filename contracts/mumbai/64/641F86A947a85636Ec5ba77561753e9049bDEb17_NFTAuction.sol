...
contract NFTAuction is ReentrancyGuard, Ownable {
    ...
    function claimNFT() external nonReentrant {
        ...
        require(userBids[msg.sender] > 0, "User has not placed any bid");
        require(!hasClaimed[msg.sender], "Already claimed NFT");
        require(isWinner(msg.sender), "User is not a winner");

        address[] memory winners = calculateWinners();
        for (uint256 i = 0; i < winners.length; i++) {
            if (winners[i] == msg.sender) {
                hasClaimed[msg.sender] = true;
                nftFactory.mint(msg.sender, i);
                break;
            }
        }
    ...
    function claimRefund() external nonReentrant {
        ...
        require(!isWinner(msg.sender), "User is a winner");
    ...
    function calculateWinners() public view returns (address[] memory) {
        uint256 currentLength = userBidsIndex.length > MAX_WINNERS ? MAX_WINNERS : userBidsIndex.length;
        address[] memory winners = new address[](currentLength);
        uint256[] memory bids = new uint256[](currentLength);
        for (uint256 i = 0; i < userBidsIndex.length; i++) {
            address user = userBidsIndex[i];
            uint256 _bid = userBids[user];
            for (uint256 j = 0; j < currentLength; j++) {
                if (_bid > bids[j]) {
                    for (uint256 k = currentLength - 1; k > j; k--) {
                        bids[k] = bids[k - 1];
    ...
    function getBidAmount(address _user) public view returns (uint256) {
        return userBids[_user];
    }

    // withdraw raised amount
    function withdrawAdminFunds() external nonReentrant onlyOwner {
        require(!isAdminWithdraw, "Already withdrawn");
        require(block.timestamp > auctionEndTime, "Auction is still ongoing");
        address[] memory winners = calculateWinners();

        uint256 totalAmount = 0;
        for (uint256 i = 0; i < winners.length; i++) {
            totalAmount += userBids[winners[i]];
        }
        isAdminWithdraw = true;
        payable(ADMIN_WALLET).transfer(totalAmount);
        
        emit adminWithdrawal(ADMIN_WALLET, totalAmount);
    }

    // solo para probar despues borrar
...
contract NFTFactory is ERC721, AccessControl, ERC721Enumerable {
    ...
    function mintBulkWithIds(address to, uint256[] memory ids) public onlyRole(MINTER_ROLE) {
    ...
    // Method use for bulk minting
    function mintBulkWithQty(address to, uint256 qty) public onlyRole(MINTER_ROLE) {
