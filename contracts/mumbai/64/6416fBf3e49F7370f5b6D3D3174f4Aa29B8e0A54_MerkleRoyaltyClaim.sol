...
contract MerkleRoyaltyClaim is Ownable, AccessControl{
    ...
    constructor(bytes32 merkleroot, address[] memory claimers)
    {
        root = merkleroot;
        // Grant the minter role to a specified account
        if(claimers.length > 0){
            for (uint256 i = 0; i < claimers.length; ++i) {
                _setupRole(CLAIMER_ROLE, claimers[i]);
            }    
        }
    }
    ...
    function validateRoyalty(address account, uint256 amount, bytes32[] calldata _merkleProof) external
    view returns (bool)
    {
    ...
    function claimRoyaltyByAdmin(address account, uint256 amount, bytes32[] calldata _merkleProof) external onlyOwner {
       
       bytes32 leaf = _leaf(account, amount);
       require(MerkleProof.verify(_merkleProof, root, leaf), "Invalid proof.");

        require(!claimed[leaf], "Address has already been claimed");

        claimed[leaf] = true;
        emit Claimed(account, amount);
    }

    function claimRoyaltyByClaimer(uint256 amount, bytes32[] calldata _merkleProof) external  {
    ...
    function isClaimed(address account, uint256 amount)
