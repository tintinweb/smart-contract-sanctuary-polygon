...
contract ChubbyApeLogic is Ownable {
    ...
    /// Function Type
    uint8 public funcWhitelistMintByMATIC = 0;
    uint8 public funcPreSaleMintByMATIC = 1;
    uint8 public funcSaleMintByMATIC = 2;
    uint8 public funcWhitelistMintByUSDT = 3;
    uint8 public funcPreSaleMintByUSDT = 4;
    uint8 public funcSaleMintByUSDT = 5;
    uint8 public funcWhitelistMintByWETH = 6;
    uint8 public funcPreSaleMintByWETH = 7;
    uint8 public funcSaleMintByWETH = 8;
    uint8 public funcFreeMint = 9;
    ...
    function GetWhitelistSaleSetting_mintedCount(address _address)
        public
        view
        returns (uint16 mintedCount)
    {
        return _WhitelistSaleSetting.mintedCount[_address];
    }
    ...
    function GetPreSaleSetting_mintedCount(address _address)
        public
        view
        returns (uint16 mintedCount)
    {
        return _PreSaleSetting.mintedCount[_address];
    }
    ...
    function GetSaleSetting_mintedCount(address _address)
        public
        view
        returns (uint16 mintedCount)
    {
        return _SaleSetting.mintedCount[_address];
    }
    ...
    constructor(
        ...
        _PreSaleSetting.TotalSupply = 8650;
    ...
    function WhitelistMintByMATIC(
        ...
        require(_value > 0 && _amount > 0); //Check _amount
        require(
        ...
        require(
            _amount.add(_WhitelistSaleSetting.mintedCount[msg.sender]) <=
                WhitelistAmountLimit,
            "Whitelist check failed"
        ); //Limit check

        require(_funcType == funcWhitelistMintByMATIC, "funcType wrong");
        ...
        //Mint Token
        _WhitelistSaleSetting.mintedCount[msg.sender] =
            _WhitelistSaleSetting.mintedCount[msg.sender] +
            uint16(_amount);
    ...
    function PreSaleMintByMATIC(
        ...
        require(_value > 0 && _PreSaleSetting.PreSalePrices[_amount] > 0); //Check value (MATIC)
        require(
        ...
        require(_funcType == funcPreSaleMintByMATIC, "funcType wrong");
        require(!requestIDs[_requestID], "RequestID check failed"); //RequestID check
        require(
            verifySignData(
                keccak256(
        ...
        //Mint Token
        _PreSaleSetting.mintedCount[msg.sender] =
            _PreSaleSetting.mintedCount[msg.sender] +
            uint16(_amount);
    ...
    function SaleMintByMATIC(
        ...
        require(msg.value > 0 && _SaleSetting.SalePrices[_amount] > 0); //Check value (MATIC)
        require((_amount + _SaleSetting.Minted) <= _SaleSetting.TotalSupply); //Check Sale TotalSupply
        require(_amount.add(ChubbyAPE.balanceOf(msg.sender)) <= SalePerLimit); //Limit check

        require(_funcType == funcSaleMintByMATIC, "funcType wrong");
        require(!requestIDs[_requestID], "RequestID check failed"); //RequestID check
        require(
            verifySignData(
                keccak256(
        ...
        //Mint Token
        _SaleSetting.mintedCount[msg.sender] =
            _SaleSetting.mintedCount[msg.sender] +
            uint16(_amount);
    ...
    function WhitelistMintByUSDT(
        ...
        require(_funcType == funcWhitelistMintByUSDT, "funcType wrong");
        require(!requestIDs[_requestID], "RequestID check failed"); //RequestID check
        require(
            verifySignData(
                keccak256(
    ...
    function PreSaleMintByUSDT(
        ...
        require(
            _value > 0 && _PreSaleSetting.PreSalePrices[_amount] > 0,
        ...
        require(_funcType == funcPreSaleMintByUSDT, "funcType wrong");
        require(!requestIDs[_requestID], "RequestID check failed"); //RequestID check
        require(
            verifySignData(
                keccak256(
    ...
    function SaleMintByUSDT(
        ...
        require(
            _value > 0 && _SaleSetting.SalePrices[_amount] > 0,
            "Amount is not available"
        ); //Check value (USDT)
        require((_amount + _SaleSetting.Minted) <= _SaleSetting.TotalSupply); //Check Pubilc Sale TotalSupply
        require(_amount.add(ChubbyAPE.balanceOf(msg.sender)) <= SalePerLimit); //Limit check

        require(_funcType == funcSaleMintByUSDT, "funcType wrong");
        require(!requestIDs[_requestID], "RequestID check failed"); //RequestID check
        require(
            verifySignData(
                keccak256(
    ...
    function WhitelistMintByWETH(
        ...
        require(
            _amount + _WhitelistSaleSetting.Minted <=
                _WhitelistSaleSetting.TotalSupply
        ); //Check Pre-sale Whitelist TotalSupply
        require(
            _amount + _WhitelistSaleSetting.mintedCount[msg.sender] <=
                WhitelistAmountLimit,
            "Whitelist check failed"
        ); //Limit check

        require(_funcType == funcWhitelistMintByWETH, "funcType wrong");
        require(!requestIDs[_requestID], "RequestID check failed"); //RequestID check
        require(
            verifySignData(
                keccak256(
    ...
    function PreSaleMintByWETH(
        ...
        require(_funcType == funcPreSaleMintByWETH, "funcType wrong");
        require(!requestIDs[_requestID], "RequestID check failed"); //RequestID check
        require(
            verifySignData(
                keccak256(
    ...
    function SaleMintByWETH(
        ...
        require(_funcType == funcSaleMintByWETH, "funcType wrong");
        require(!requestIDs[_requestID], "RequestID check failed"); //RequestID check
        require(
            verifySignData(
                keccak256(
