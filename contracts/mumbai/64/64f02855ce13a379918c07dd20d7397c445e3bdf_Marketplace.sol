...
contract Marketplace is ReentrancyGuard, IERC721Receiver {
    ...
    */
    ...
    /**
        Admin Fee for listing NFT in marketplace (3% of NFT Price)
    */

    function AdminFeeCalc(uint value) public pure returns (uint) {
        uint fee = (value * 3e16) / 1e18;
        return fee;
    }

    /**
        Creator Royalty Fee for the NFT when sale is made in marketplace (7% of NFT Price)
    */

    function CreatorFeeCalc(uint value) public pure returns (uint) {
        uint fee = (value * 7e16) / 1e18;
        return fee;
    }
    ...
    */
        ...
        nonReentrant
    {
        ...
        if (checked[msg.sender]) {
            uint fee1 = CreatorFeeCalc(msg.value);
            uint adminFee1 = AdminFeeCalc(msg.value);

            uint remain1 = msg.value - (fee1 + adminFee1);
            marketItemIdToMarketItem[marketItemId].creator.transfer(fee1);
            marketItemIdToMarketItem[marketItemId].seller.transfer(remain1);
            payable(owner).transfer(adminFee1);
        } else {
            uint fee = CreatorFeeCalc(msg.value);
            uint remain = msg.value - fee;
            marketItemIdToMarketItem[marketItemId].creator.transfer(fee);
            marketItemIdToMarketItem[marketItemId].seller.transfer(remain);
        }
