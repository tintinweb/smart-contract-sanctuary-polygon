...
contract veFXSYieldDistributorV4 is Owned, ReentrancyGuard {
    ...
    function earned(address account) public view returns (uint256) {
        ...
        (
            uint256 eligible_current_vefxs,
            uint256 ending_timestamp
        ) = eligibleCurrentVeFXS(account);
        ...
        uint256 vefxs_balance_to_use;
        {
            ...
            if (eligible_current_vefxs > old_vefxs_balance) {
                ...
                vefxs_balance_to_use = (
                    (eligible_current_vefxs).add(old_vefxs_balance)
                ).div(2);
            }
        }
    ...
    function _checkpointUser(address account) internal {
        ...
        IveFXS.LockedBalance memory curr_locked_bal_pack = veFXS.locked(
            account
        );
    ...
    // Get yield function for gasless relayer transactions
    function getYield(address _address)
        external
        nonReentrant
        notYieldCollectionPaused
        checkpointUser(_address)
        returns (uint256 yield0)
    {
        require(greylist[_address] == false, "Address has been greylisted");

        yield0 = yields[_address];
        if (yield0 > 0) {
            yields[_address] = 0;
            TransferHelper.safeTransfer(
                emitted_token_address,
            ...
            emit YieldCollected(_address, yield0, emitted_token_address);
        }

        lastRewardClaimTime[_address] = block.timestamp;
    }
