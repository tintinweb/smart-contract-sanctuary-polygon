...
contract TestContract {
    ...
    uint256[15] private level4And5Percents = [300,100,200,200,200,200,200,100,100,100];
    ...
    function updateLevel(address _user) public {
        ...
        user.aon = _isRunning(_user, 0, false) ? true : false;

        if(user.level >= 5) {
            user.levelAchieved[1] = getRubyPlayersCount(_user);
        }
    }
    ...
    function _distributeDeposit(uint256 _amount) private {
        usdc.transfer(BackupAccount, _amount.mul(emergencyPercents).div(baseDivider));
    ...
    function _unfreezeFundAndUpdateReward(address _user, uint256 _amount) private {
        ...
        for (uint256 i = 0; i < orderInfos[_user].length; i++) {
            ...
            if (block.timestamp > order.unfreeze && order.isUnfreezed == false && _amount >= order.amount) {
                ...
                if(balRecoveredFirstTime && user.level > 3 && user.levelAchieved[0] < lastFreezed && !order.isBooster) {
                    rewardPercents = 1000;
                }

                uint256 staticReward = order.amount.mul(rewardPercents).div(baseDivider);

                if (isStopLoss1 && !order.isBooster) {
        ...
        if (level5Count > 0 && !isStopLoss2) {
            ...
            for (uint256 i = 0; i < diamondPlayers.length; i++) {
                ...
                if (userInfo[_user].level == 5) {
                    ...
                    if(rubyCount > 1) {
                        curReward = 10e6 + (5e6*(rubyCount - 1));
                    } else {
                        curReward = 10e6;
                    }
                    ...
                    uint256 calReward = curReward.mul(reduceRate.mul(rewardInfo[_user].diamondPoolCount)).div(baseDivider);
                    uint256 givenReward = curReward.sub(calReward);
    ...
    function depositByGrowthAccount(uint256 _amount) external {
    ...
    function transferByGrowthAccount(address _receiver, uint256 _amount) external {
        require(_amount >= minDeposit && _amount.mod(minDeposit) == 0, "amount err");
        uint256 growthLeft = getCurGrowth(msg.sender);
    ...
    function getRubyPlayersCount(address _user) public view returns(uint256) {
        uint256 count;
