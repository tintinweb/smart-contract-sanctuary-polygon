...
contract Ticketing is ERC721, Ownable{
    ...
    function createEvent(EventInformation memory _event) public onlyAdmin{
        uint incrementId = 1;
        uint autoId = autoGenerateId + incrementId;

        linkIdToEvent[autoId].eventTitle = _event.eventTitle;
        linkIdToEvent[autoId].venue = _event.venue;
        linkIdToEvent[autoId].startDate = _event.startDate;
        linkIdToEvent[autoId].endDate = _event.endDate;
        linkIdToEvent[autoId].ticketsQuantity = _event.ticketsQuantity;
        linkIdToEvent[autoId].ticketPrice = _event.ticketPrice;
        linkIdToEvent[autoId].maxTicketPerPerson = _event.maxTicketPerPerson;
        linkIdToEvent[autoId].publishTime = _event.publishTime;

        eventPubTime[autoId] = _event.publishTime;
        eventStartTime[autoId] = _event.startDate;
        eventEndTime[autoId] = _event.endDate;
        eventMapName[autoId] = _event.eventTitle;

        linkEventIdToTicketQuantity[autoId][_event.ticketsQuantity]=true;
        autoIds.push(autoId);
    ...
    function buyTickets( uint _eventId, uint _ticketCount) public {
            ...
            uint eventPublishTime = linkIdToEvent[_eventId].publishTime;    // Retrieve the publish time
            uint eventsStartTime =  linkIdToEvent[_eventId].startDate;           // Retrieve the end date
            require(block.timestamp >= eventPublishTime && block.timestamp <= eventsStartTime, "The Event is not yet published or Event already started");
    ...
    /**
     * if timestamp is greater than publishtime and lesser than start time -> event yet to start
     * if timestamp is greater than start time and lesser than end time -> event is live
     * if timestamp is greater than endtime -> event is ended
     */

    function YetToStartEvents() external view returns(EventYetToStart[] memory){
        uint count = 0;
        for(uint i = 100; i < autoIds.length; i++){
            if(block.timestamp > eventPubTime[i] && block.timestamp < eventStartTime[i]){
                count++;
            }
        }
        EventYetToStart[] memory events = new EventYetToStart[](count);
        for(uint i = 100; i < autoIds.length; i++){
            EventYetToStart memory ev;
            ev.eventName = eventMapName[i];
            ev.status = "Event yet to start";
        }
        return events;
    }

    function LiveEvents() external view returns(EventStarted[] memory){
        uint count = 0;
        for(uint i = 100; i < autoIds.length; i++){
            if(block.timestamp >  eventStartTime[i] && block.timestamp <  eventEndTime[i]){
                count++;
            }
        }
        EventStarted[] memory events = new EventStarted[](count);
        for(uint i = 100; i < autoIds.length; i++){
            EventStarted memory ev;
            ev.eventName = eventMapName[i];
            ev.status = "Event is live";
        }
        return events;
    }

    function ExpiredEvents() external view returns(EventEnded[] memory){
        uint count = 0;
        for(uint i = 100; i < autoIds.length; i++){
            if(block.timestamp > eventEndTime[i]){
                count++;
            }
        }
        EventEnded[] memory events = new EventEnded[](count);
        for(uint i = 100; i < autoIds.length; i++){
            EventEnded memory ev;
            ev.eventName = eventMapName[i];
            ev.status = "Event is expired";
        }
        return events;
    }
}
...
contract Ticmet is ERC20, Ownable {
    
    constructor(uint _amount) ERC20("Ticmet", "TIC") {
