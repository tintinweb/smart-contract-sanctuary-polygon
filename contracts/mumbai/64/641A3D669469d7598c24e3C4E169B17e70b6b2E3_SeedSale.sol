...
contract SeedSale is Ownable, Pausable {
    ...
    uint256 public firstUnlockPercent = 10;
    uint256 public vestingPeriodMonth = 20;

    bool private isBurned;

    mapping(address => Allocation) public allocations;

    event SetSale(uint256 start, uint256 end, uint256 amountTokens, uint256 tgeTs);
    event BuyTokens(address user, uint256 usdtAmount, uint256 tokenAmount);
    event ClaimTokens(address user, uint256 tokenAmount);
    event SetMerkleTree(bytes32 merkleTree);
    event SetTokenPrice (uint256 tokenPrice, uint256 tokenPriceDecimals);
    event SetVesting(uint256 maxPerUser, uint256 unlockPeriodTime, uint256 firstUnlockPercent, uint256 vestingPeriodMonth, uint256 tgeTs_);
    event SetTokens(IERC20 token, IERC20 USDT);
    event SetPaymentVault(address vaultAddress);
    event Sweep(IERC20 token, address recepient);
    event BurnRest(uint256 amount);
    ...
    function setSale(uint256 start, uint256 end, uint256 amount, uint256 tge) external onlyOwner {
        ...
        emit SetSale(start, end, amountToken, tgeTs);
    }
    ...
    function buyTokens(uint256 amount, bytes32[] memory merkleProof) external isNotBlackListed {
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(merkleProof, merkleRoot, leaf), "You're are not whitelisted");
    ...
    function getCurrentEraId() public view returns (uint256) {
        if (tgeTs >= block.timestamp) {
            return 0;
        }
        return (block.timestamp - tgeTs) / unlockPeriodTime > vestingPeriodMonth ? 
            vestingPeriodMonth : 
            (block.timestamp - tgeTs) / unlockPeriodTime;
    }
    ...
    function claim() external isNotBlackListed {
        ...
        if (claimablePeriods > 0) {
            ...
            if (allocation.lastClaimId == vestingPeriodMonth) {
                amountToClaim += allocation.boughtTokens - amountToClaim - allocation.claimedTokens;
            }
    ...
    function setMerkleTree(bytes32 merkleRoot_) external onlyOwner {
        merkleRoot = merkleRoot_;

        emit SetMerkleTree(merkleRoot_);
    }

    function setVestingSettings(uint256 maxPerUser_, uint256 unlockPeriodTime_, uint256 firstUnlockPercent_, uint256 vestingPeriodMonth_, uint256 tgeTs_) external onlyOwner {
        maxPerUser = maxPerUser_;
        ...
        emit SetVesting(maxPerUser_, unlockPeriodTime_, firstUnlockPercent_, vestingPeriodMonth_, tgeTs_);
    }
    ...
    function setPaymentVault (address paymentVault_) external onlyOwner {
        paymentVault = paymentVault_;

        emit SetPaymentVault(paymentVault_);
    }
    ...
    function burnRest() external onlyOwner {
        require(getStage() == SaleStage.Ended, "Sale is not ended");
        require(!isBurned, "You have already burned unsold tokens");
        
        uint256 tokensToBurn = amountToken - tokenSold;
        IERC20(token).burn(tokensToBurn);
        isBurned = true;

        emit BurnRest(tokensToBurn);
    }
}