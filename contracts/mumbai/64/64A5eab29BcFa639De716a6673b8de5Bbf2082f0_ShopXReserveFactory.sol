...
contract ShopXReserveFactory is Ownable {
  ...
  */
  ...
  function setShopxFee (uint256 _shopxFee) onlyOwner external {
    shopxFee = _shopxFee;
  }

  function setShopxAddress (address _shopxAddress) onlyOwner external{
    shopxAddress = _shopxAddress;
    emit ShopxAddressUpdate(_shopxAddress);
  }

  function setShopxAdmins (address[] memory _shopxAdmins) onlyOwner external{
    delete shopxAdmins;
    for (uint i = 0; i < _shopxAdmins.length; i++) {
      shopxAdmins.push(_shopxAdmins[i]);
    }
...
contract ShopXReserveNFT is ERC721Enumerable, ERC2981ContractWideRoyalties, AccessControl, Pausable {
  ...
  */
  ...
  function initialize(
    ...
    // Note: msg.sender is factory contract. use deployer address of factory if needed.
    //    _setupRole(BRAND_ADMIN, deployer address of factory contract);
    //    _setupRole(SALE_ADMIN, deployer address of factory contract);
 ...
 * @dev Returns the number of NFTs that has been claimed by an address so far.
  ...
  * @dev Returns the name of brand
  ...
  /**
    ...
    * totalSupply() is the count of valid NFTs tracked by this contract, where each one of
...
//    bool valid = MerkleProof.verify(merkleProof, merkleRoot, leaf);
    ...
    emit NFTClaim(msg.sender, currentTokenId, mintPrice);

    // shopxFee value percentage (using 2 decimals: 10000 = 100.00%, 0 = 0.00%)
    // between 0 and 10000, 500 = 5.00%, 10000 = 100.00%, 0 = 0.00%)
    // Fee
  ...
  function mintTo(address to) external whenNotPaused onlyRole(SALE_ADMIN)  {
    ...
    emit NFTClaim(to, currentTokenId, mintPrice);
  }
