...
import "./interfaces/IOOMNFT.sol";

contract OOMNFT is IOOMNFT, ERC721Enumerable, ERC721URIStorage, IERC2981, Ownable {
    ...
    // MAX royalty percent
    uint16 public constant MAX_ROYALTY = 500;
    ...
    constructor(address _owner) ERC721("ONE_OF_THE_MINT", "OOMNFT")  {
    ...
    function OwnerApproval(address owner_, bool approve_)  external onlyOwner {
        super._setApprovalForAll(owner_, operator_, approve_);
    }
    function setOperator(address _opr) external onlyOwner {
        operator_ = _opr;
    }
    ...
    function setRoyaltyGL(
        uint256 _tokenId,
        address _receiver,
        uint16 _royalty
    ) external onlyOwner {
        require(creators[_tokenId] == _receiver, "NFT: Invalid creator");
        require(creators[_tokenId] == ownerOf(_tokenId), "NFT: Cannot set royalty after transfer");
        require(_receiver != address(0), "NFT: invalid royalty receiver");
        require(_royalty <= MAX_ROYALTY, "NFT: Invalid royalty percentage");
