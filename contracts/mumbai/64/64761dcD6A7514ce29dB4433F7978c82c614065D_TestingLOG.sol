...
contract TestingLOG is ERC721A, Ownable, ReentrancyGuard {
    ...
    modifier checkPublicMintPhase(PhaseMint _phase) {
        if (_phase != PhaseMint.fcfs || _phase != PhaseMint.publicMint) {
    ...
    function claimReverse() external {
    ...
    //     feature[PhaseMint.reserve].startTime = feature[PhaseMint.freeMint]
    //         .endTime;
    //     feature[PhaseMint.reserve].endTime = block.timestamp + 14 hours;
    //     feature[PhaseMint.guaranteed].startTime = feature[PhaseMint.reserve]
    //         .endTime;
    ...
    //     feature[PhaseMint.fcfs].supply =
    //         MAX_SUPPLY -
    //         (alreadyMinted + tokenReserve);
    ...
    // need improvement to set up supply
    function openPublictMint(
        PhaseMint _phase,
        bool toggle,
        uint256 supplyProvide
    ) external onlyOwner checkPublicMintPhase(_phase) {
        feature[_phase].supply = supplyProvide;
        feature[_phase].isOpen = toggle;
    }
