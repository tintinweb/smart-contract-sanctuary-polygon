    ...
    ReentrancyGuard
{
    ...
    mapping(uint256 => uint256) private _lengthToPrice;

    ///@dev Self ERC20 Interface
    IERC20 public self;

    ///@dev Var to keep track of total self collected
    uint256 public totalSelfCollected;

    //=================Constructor===================
    constructor(address _self) ERC721("Self Identity NFT", "SIN") {
        require(_self != address(0), "SELF: Invalid address");
        _lengthToPrice[8] = 500;
        _lengthToPrice[7] = 1000;
        _lengthToPrice[6] = 2000;
        _lengthToPrice[5] = 4000;
        _lengthToPrice[4] = 8000;
        _lengthToPrice[3] = 16000;
    ...
    /**
     * @notice Checks that the input string (name) consists of valid characters and has a valid length.
     ...
     * @param name_ The input string (name) to be checked for valid characters and length.
     *
     ...
     */
    modifier isLegal(string memory name_) {
        ///@dev conversion to bytes
        bytes memory bName = bytes(name_);

        ///@dev validate length of a name
        require(bName.length >= 3, "SELF: Name is too short.");
        require(bName.length <= 30, "SELF: Name is too long.");

        uint8 _current;

        for (uint256 i; i < bName.length; i++) {
            _current = uint8(bName[i]);
             ...
             *
             * @notice This function is designed to only accept ASCII characters in the specified ranges. The likelihood of accepting non-latin or non-ASCII characters is extremely low due to these strict conditions.
             */
    ...
    * @param name_ The unique name to be registered as an NFT.
    * @return A boolean value indicating whether the name registration and NFT minting were successful.
 ...
 */
    function registerName(
        string memory name_
    ) external whenNotPaused nonReentrant isLegal(name_) returns (bool) {
        ///@dev hash the name and convert it into uint256
        uint256 _tokenId = uint256(keccak256(abi.encodePacked(name_)));
        ...
        uint _price = getPrice(name_);

        ///@dev checks if the name has a valid price
        require(_price > 0, "SELF: Price not set yet");
        ...
        totalSelfCollected += _price;

        ///@dev store the original name corresponding to the hashed name
        _tokenIdToName[_tokenId] = name_;

        ///@dev mint the nft
        _safeMint(msg.sender, _tokenId);

        ///@dev emit event
        emit NameRegistered(msg.sender, name_, _tokenId);

        return true;
    }

    /**
    * @notice Associates an IPFS URL as metadata with the given tokenId.
    * @param tokenId_ The unique identifier of the NFT whose metadata is to be updated.
    * @param metadata_ The IPFS URL containing metadata for the NFT.
    *
    * @dev This function is used to set or update the metadata associated with a specific tokenId. The metadata is expected to be an IPFS URL, which is a decentralized way to store and retrieve content.
    ...
    function setNameMetadata(
        ...
        require(_ownerOf(tokenId_) == msg.sender, "SELF: Not owner");

        ///@dev update uri
        _setTokenURI(tokenId_, metadata_);

        emit MetadataUpdated(tokenId_, metadata_);
    }

    /**
     * @notice Sets the price for registering a name of a specific length.
     * @param length_ The length of the name (in characters) for which the price is being set.
     * @param price_ The price (in tokens) for registering a name of the specified length.
     */
    function setPrice(uint256 length_, uint256 price_) external onlyOwner {
        ///@dev The length must be between 3 and 8 characters (inclusive).
        require(length_ >= 3 && length_ <= 8, "Self: Invalid length");
        require(price_ > 0, "SELF: Invalid price");

        _lengthToPrice[length_] = price_;

        emit PriceUpdated(length_, price_);
    }

    /// @notice Updates the address of the SELF token used for payments within the contract.
    /// @param self_ The address of the new SELF token.

    function setSelf(address self_) external onlyOwner {
        require(self_ != address(0), "SELF: Invalid address");
        self = IERC20(self_);

        emit SelfTokenUpdated(self_);
    }

    /// @notice Transfers the collected SELF tokens from the contract to the owner's address.
    function forwardCollectedSelf() external onlyOwner {
        uint _amountToTransfer = self.balanceOf(address(this));
        require(_amountToTransfer > 0, "SELF: No tokens to forward");
    ...
    /**
     * @notice Retrieves the registered name associated with a given tokenId.
     * @param tokenId_ The tokenId of the registered name to be fetched.
     * @return The registered name associated with the given tokenId.
     */
    function getNameFromTokenId(
        uint256 tokenId_
    ) external view returns (string memory) {
        return _tokenIdToName[tokenId_];
    }

    /**
     * @notice Retrieves all registered names owned by a specific address.
     * @param owner_ The address of the owner whose registered names will be fetched.
    ...
    function getNames(
        ...
        for (uint256 i = 0; i < _balance; i++) {
            names[i] = _tokenIdToName[tokenOfOwnerByIndex(owner_, i)];
        }
    }

    /**
     * @notice Checks if a given name is available for registration.
     * @param name_ The name to check for availability.
     * @return A boolean value indicating whether the name is available for registration.
     */
    function isNameAvailable(
        string memory name_
    ) external view isLegal(name_) returns (bool) {
        uint256 _tokenId = uint256(keccak256(abi.encodePacked(name_)));
        return _ownerOf(_tokenId) == address(0);
    }

    /**
     * @notice Retrieves the price associated with a specific name length.
     * @param length_ The length of the name.
     * @return The price for the given name length.
     */
    function getPriceOfLength(uint length_) external view returns (uint256) {
        return _lengthToPrice[length_];
    }

    /**
     * @notice Calculates the price for registering a given name.
     * @param name_ The name for which the price will be calculated.
     * @return The price for registering the given name.
     */
    function getPrice(string memory name_) public view returns (uint256) {
        uint256 nameLength = bytes(name_).length;
        uint256 effectiveNameLength = nameLength > 8 ? 8 : nameLength;
        return _lengthToPrice[effectiveNameLength] * 1 ether;
    }
    ...
    // Override tokenURI from both ERC721 and ERC721Metadata
    function tokenURI(
        uint256 tokenId
    ...
    // Override _burn from both ERC721 and ERC721Metadata
    function _burn(
        uint256 tokenId
