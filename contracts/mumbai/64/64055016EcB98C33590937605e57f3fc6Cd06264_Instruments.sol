...
contract Instruments is ERC721Base {
    ...
    function addCertToken(uint256 tokenId, address contractAddress, uint256 certTokenId) public {
        require(_exists(tokenId), "Instruments: token does not exist");

        require(IERC721(contractAddress).ownerOf(certTokenId) == msg.sender, "Instruments: caller is not owner of cert token");
        require(IERC721(address(this)).ownerOf(tokenId) == msg.sender, "Instruments: caller is not owner of instrument token");

        _certTokens[tokenId].push(Token(contractAddress, msg.sender, certTokenId));
    ...
    function removeCertToken(uint256 tokenId, uint256 certTokenIndex) public {
        require(_exists(tokenId), "Instruments: token does not exist");
        require(certTokenIndex < _certTokens[tokenId].length, "Instruments: cert token index out of bounds");
        Token memory certToken = _certTokens[tokenId][certTokenIndex];

        require(certToken.owner == msg.sender, "Instruments: cert token not assigned to this sender");
        require(IERC721(certToken.contractAddress).ownerOf(certToken.tokenId) == address(this), "Instruments: cert token not owned by the contract");
