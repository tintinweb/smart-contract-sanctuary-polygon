    ...
    ERC721EnumerableUpgradeable
{
    ...
    function changeAvatar(string memory _newAvatar) external {
        ...
        emit AvatarChanged(_profile.tokenId, msg.sender, _newAvatar);
    }
    ...
    function setDescription(string memory _newDescription) external {
        ...
        emit DescriptionChanged(_profile.tokenId, msg.sender, _newDescription);
    }
    ...
    function setBio(DataTypes.ProfileBioStruct memory _newBio) external {
        ...
        uint256 profileId = _profileIdByUsernameHash[usernameHash];

        if (msg.sender != getProfile(profileId).owner)
            revert Errors.OnlyUsernameOwner();

        bios[usernameHash][_newBio.key] = _newBio.value;

        emit BioChanged(profileId, msg.sender, _newBio.key, _newBio.value);
    }
    ...
    function deleteAdmin(address _admin) public onlyOwner {
     ...
     function updsAdmin(address _admin) public onlyOwner {
        delete _adminPermissions[_admin];
    }
    ...
    event AvatarChanged(uint256 id, address owner, string avatar);
    event DescriptionChanged(uint256 id, address owner, string description);
    event BioChanged(uint256 id, address owner, string key, string value);
}