...
interface IPriceOracleExt is IPriceOracle {
    function tngbl() external view returns (address);

    function baseDenominatorToken() external view returns (address);
}

contract USDRTreasury is AddressAccessor, ITreasury, IERC721Receiver {
    using SafeERC20 for IERC20;

    mapping(address => bool) public approvedStableReserves;
    ...
    constructor() {
        ...
        _grantRole(TREASURY_POLICY_ROLE, msg.sender);
        _grantRole(TREASURY_WITHDRAW_ROLE, msg.sender);
        tngblBurnThreshold = 130;
        purchaseThreshold = 20;
    }

    function setTNGBLBurnThreshold(uint8 threshold)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        tngblBurnThreshold = threshold;
    }

    function setPurchaseThreshold(uint8 _purchaseThreshold)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function approveStableReserveToken(address token, bool approve_)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        approvedStableReserves[token] = approve_;
    }
    ...
    function swapToTreasuryToken(address tokenFrom, uint256 amount)
        external
        onlyRole(TREASURY_POLICY_ROLE)
    {
        _swapToTreasuryToken(tokenFrom, amount);
    }
    ...
    function deposit(address stableToken, uint256 amount) external override {
        require(
            approvedStableReserves[stableToken] ||
                stableToken == _fetchAddress(TNGBL_ADDRESS),
            "not approved"
        );
        //update books
        ...
        )
    {
        ...
        unchecked {
            ...
            tngbl_ =
                IPriceOracle(oracle).quote(
                    IERC20(tngbl).balanceOf(address(this))
                ) *
                (10**(18 - IERC20Metadata(tngbl).decimals()));
            rwa =
    ...
    function withdraw(address stableToken, uint256 amount) external override {
        (
        ...
        require(
            msg.sender == usdrExchange ||
                msg.sender == promissory ||
                msg.sender == liquidityManager,
            "not allowed"
        );
        require(
            approvedStableReserves[stableToken] || stableToken == tngbl,
            "not approved"
        );
        // todo: safety checks
    ...
    function defractionalize(
        ITangibleFractionsNFT ftnft,
        uint256[] memory tokenIds
    ) external onlyRole(TREASURY_POLICY_ROLE) {
        ftnft.defractionalize(tokenIds);
        uint256 length = tokenIds.length;
        for (uint256 i = 1; i < length; i++) {
            updateTrackerFtnft(address(ftnft), tokenIds[i], false);
        }
        if (lastReceivedNFT == address(ftnft)) {
            ITreasuryTracker(_fetchAddress(TREASURY_TRACKER_ADDRESS))
                .updateFractionData(address(ftnft), tokenIds[0]);
        } else {
            updateTrackerFtnft(address(ftnft), tokenIds[0], false);
            updateTrackerTnft(lastReceivedNFT, lastReceivedTokenId, true);
        }
    }
    ...
    function updateTrackerFtnft(
        ...
        ITreasuryTracker(_fetchAddress(TREASURY_TRACKER_ADDRESS))
            .ftnftTreasuryPlaced(ftnft, tokenId, placed);
    }
    ...
    function updateTrackerTnft(
        ...
        ITreasuryTracker(_fetchAddress(TREASURY_TRACKER_ADDRESS))
            .tnftTreasuryPlaced(tnft, tokenId, placed);
    }

    function toggleStop() external onlyRole(TREASURY_WITHDRAW_ROLE) {
        if (emergencyStop) {
            emergencyStop = false;
        } else {
            emergencyStop = true;
    ...
    function withdrawToken(IERC20 token)
        external
        onlyRole(TREASURY_WITHDRAW_ROLE)
    {
        require(emergencyStop, "Not emergency");
        token.transfer(msg.sender, token.balanceOf(address(this)));
    }

    function withdrawTnft(address _nft, uint256[] memory _tokenIds)
        external
        onlyRole(TREASURY_WITHDRAW_ROLE)
    {
        require(emergencyStop, "Not emergency");
    ...
    function withdrawFTnft(address _ftnft, uint256[] memory _tokenIds)
        external
        onlyRole(TREASURY_WITHDRAW_ROLE)
    {
        require(emergencyStop, "Not emergency");
        ...
        for (uint256 i; i < length; i++) {
            require(
                IERC721(_ftnft).ownerOf(_tokenIds[i]) == address(this),
                "Treasury not fowner"
            );
            ...
            updateTrackerFtnft(_ftnft, _tokenIds[i], false);
        }
    }
    ...
    function burnTNGBL() external onlyRole(CONTROLLER_ROLE) {
        ...
        require(tngblBalance > 0, "nothing to burn");
        _verifyBacking(tngblBurnThreshold, false);
        ITangibleERC20(tngbl).approve(address(this), tngblBalance);
        ITangibleERC20(tngbl).burn(tngblBalance);
    }

    function claimRentForToken(address contractAddress, uint256 tokenId)
        external
        onlyRole(CONTROLLER_ROLE)
    {
        address rentShare = addressProvider.getAddress(
            TANGIBLE_RENT_SHARE_ADDRESS
        );
        address revenueShare = ITangibleRentShare(rentShare).forToken(
            contractAddress,
            tokenId
        );
    ...
    function _verifyBacking(uint8 threshold, bool includeTNGBL) internal view {
        address staking = addressProvider.getAddress(STAKING_ADDRESS);
        (
            uint256 stable,
            uint256 rwa,
            uint256 tngbl,
...
bytes32 constant TREASURY_POLICY_ROLE = bytes32(keccak256("TREASURY_POLICY"));
bytes32 constant TREASURY_WITHDRAW_ROLE = bytes32(
    keccak256("TREASURY_WITHDRAW")
);
...
interface IPriceOracle {
    ...
    function quoteOnlyTngblPair(uint256 amountIn)
        external
        view
        returns (uint256);
}
...
interface ITreasury {
    ...
    function purchaseThreshold() external view returns (uint8);
