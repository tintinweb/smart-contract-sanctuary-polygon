...
contract InterestEarner {
    ...
    // New
    uint256[] private StakingSchedulesIdsInterest;

    mapping(bytes32 => StakingSchedule) private stakingSchedules;
    
    //===========
    // New
    // uint256 public interestToPayOutpost; 
    ...
    // NEW
    event stakingSchedulesIdsInterest(uint256 value);
...
// new 
        function getArray() public view returns(uint256[] memory newArr){
            return StakingSchedulesIdsInterest;
        } 


//NEW
        function totalExpectedPayoutByIndex(uint256 Index, bytes32 stakeScheduleId)
        public
        // onlyOwner
        returns(uint256){
            //   require(holder == stakingSchedules[stakeScheduleId].beneficiary, "Sender is not staking holder wrong wallet address");
        ...
        // return ;
        StakingSchedulesIdsInterest.push(interestToPayOut);
            uint256 val=0;
            
            val = StakingSchedulesIdsInterest[Index];
                // emit stakingSchedulesIdsInterest(val);
        return val;  
        }
        ...
        noReentrant
    {
        ...
        // No Need - NEW
        // bytes32 stakingSchedulesIdsInterest = this.computeNextStakingScheduleInterestForIdHolder(msg.sender);
        ...
        // NEW
        // Insert interestEarnedForThisStake
