...
contract MultiChainToken is ERC20, ILayerZeroReceiver, Ownable {
    ...
    address private MultiChainTokenAddressPolygon;
    ...
    function setExternalMultiChainAddresses(
        ...
        address _MultiChainTokenAddressPolygon
    ) public onlyOwner {
        MultiChainTokenAddressEthereum = _MultiChainTokenAddressEthereum;
        MultiChainTokenAddressBSC = _MultiChainTokenAddressBSC;
        MultiChainTokenAddressPolygon = _MultiChainTokenAddressPolygon;
        renounceOwnership();
    }

    function sendTokensToEthereum(uint256 _qty) public payable{
        require(block.chainid != 1 && block.chainid != 4, "Already on Ethereum");
        bytes memory addrEth = abi.encode(MultiChainTokenAddressEthereum);
        _sendTokens(10001, addrEth, _qty * 10**18);
    }

    function sendTokensToBSC(uint256 _qty) public payable{
        require(block.chainid != 56 && block.chainid != 97, "Already on BSC");
        bytes memory addrBSC = abi.encode(MultiChainTokenAddressEthereum);
        _sendTokens(10002, addrBSC, _qty * 10**18);
    }

    function sendTokensToPolygon(uint256 _qty) public payable{
        require(block.chainid != 137 && block.chainid != 80001, "Already on Polygon");
        bytes memory addrPoly = abi.encode(MultiChainTokenAddressEthereum);
        _sendTokens(10009, addrPoly, _qty * 10**18);
    }
    ...
    function lzReceive(uint16 _srcChainId, bytes memory _fromAddress, uint64 _nonce, bytes memory _payload) override external{
        ...
        address from = abi.decode(_fromAddress, (address)); 
        require(from == MultiChainTokenAddressBSC || from == MultiChainTokenAddressPolygon || from == MultiChainTokenAddressEthereum);
