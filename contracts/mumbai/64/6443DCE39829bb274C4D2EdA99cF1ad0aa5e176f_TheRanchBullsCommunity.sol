...
contract TheRanchBullsCommunity is 
    Initializable,
    ...
    CountersUpgradeable.Counter public phaseWinnerSupply;
    ...
    mapping(address => TRBullOwner) public  TRBullOwners;
    ...
    function initialize() public initializer {
        ...
        mintCost[MintType.BRONZE] = 250;
        mintCost[MintType.SILVER] = 1000;
        mintCost[MintType.GOLD] =   5000;
        
    
        normalMintStoppingPoint = 250;
        mintStoppingPoint[MintType.NORMAL] = 4893;
        mintStoppingPoint[MintType.BRONZE] = 4960;
        mintStoppingPoint[MintType.SILVER] = 4990;
        mintStoppingPoint[MintType.GOLD] = 5000;
        ...
        whitelistBonusAmount[4] = 9;
        whitelistBonusAmount[5] = 2;
        whitelistBonusAmount[6] = 9;
        whitelistBonusAmount[7] = 3;
        whitelistBonusAmount[8] = 9;
        whitelistBonusAmount[9] = 4;



        phaseWinnerSupply.jumpTo(4894);
        bronzeTokenSupply.jumpTo(4900);
        silverTokenSupply.jumpTo(4960);
        goldTokenSupply.jumpTo(4990);


        allowedMintingQuantity[1] = true;
        allowedMintingQuantity[2] = true;
        allowedMintingQuantity[3] = true;
        allowedMintingQuantity[5] = true;
        allowedMintingQuantity[7] = true;
        allowedMintingQuantity[9] = true;
   ...
   // MINTING
    mapping(uint256 => bool) public allowedMintingQuantity;
    ...
    */
    function mint(uint256 _tokenQuantity) public payable {
        ...
        if (!allowedMintingQuantity[_tokenQuantity]) { revert Minting_IsZeroOrBiggerThanMax();}
        ...
        if (myBuddy[msg.sender] != address(0)){
            if ( TRBullOwners[msg.sender].buddyUpdated == false) {
                // set flag to true 
                TRBullOwners[msg.sender].buddyUpdated = true;
        ...
        if( TRBullOwners[msg.sender].earlySupporter == false){
            TRBullOwners[msg.sender].earlySupporter = true;
        }
    ...
    function paperMint(address _address, uint256 _tokenQuantity) public payable {
        ...
        if (_tokenQuantity ==  0 || _tokenQuantity > 9) { revert Minting_IsZeroOrBiggerThanMax();}
        if (normalTokenSupply.current() + _tokenQuantity  > normalMintStoppingPoint) {revert Minting_ExceedsTotalBulls();}
    ...
    function checkClaimEligibility(uint256 quantity) external view returns (string memory){
        ...
        } else if (totalSupply() + quantity > normalMintStoppingPoint) {
    ...
    function alphaBullsMint(string memory _mintTypeStr) public payable {
        MintType _mintType;
        if (keccak256(bytes(_mintTypeStr)) == keccak256(bytes("gold"))) {
            _mintType = MintType.GOLD;
        } else if (keccak256(bytes(_mintTypeStr)) == keccak256(bytes("silver"))) {
            _mintType = MintType.SILVER;
        } else if (keccak256(bytes(_mintTypeStr)) == keccak256(bytes("bronze"))) {
            _mintType = MintType.BRONZE;
        } else {
        ...
        uint256 index;
        uint256 costPerMint;
        uint256 mintStopPoint;

        if (_mintType == MintType.GOLD) {
            costPerMint = mintCost[_mintType];
            mintStopPoint = mintStoppingPoint[_mintType];
            index = goldTokenSupply.current();
        } else if (_mintType == MintType.SILVER) {
            costPerMint = mintCost[_mintType];
            mintStopPoint = mintStoppingPoint[_mintType];
            index = silverTokenSupply.current();
        } else if (_mintType == MintType.BRONZE) {
            costPerMint = mintCost[_mintType];
            mintStopPoint = mintStoppingPoint[_mintType];
            index = bronzeTokenSupply.current();
        }

        require((index + 1) <= mintStopPoint, "Sold Out");

        if(msg.sender != procurementWallet){
            IERC20Upgradeable usdcToken = IERC20Upgradeable(usdcTokenContract);
            uint256 mintingCost = costPerMint * 10 ** usdcTokenDecimals;
            uint256 totalTransactionCost = mintingCost;
            usdcToken.safeTransferFrom(msg.sender, marketingSafe, mintingCost);
        } 
        ...
        if(keccak256(bytes(_mintTypeStr)) == keccak256(bytes("bronze"))){bronzeTokenSupply.increment();}
        else if(keccak256(bytes(_mintTypeStr)) == keccak256(bytes("silver"))){silverTokenSupply.increment();}
        else if(keccak256(bytes(_mintTypeStr)) == keccak256(bytes("gold"))){goldTokenSupply.increment();}


        _safeMint(msg.sender, index + 1);
        emit alphaBullMinted(msg.sender, index);
    }
    ...
    */
    ...
    function freeMint() public nonReentrant{
        ...
        uint256 btcBullIndex = normalTokenSupply.current() + 1;
    ...
    function setProcurementWallet(address _address) public onlyOwner {
        if (address(_address ) == address(0)) { revert Address_CantBeAddressZero();}
        procurementWallet = _address;
    }
