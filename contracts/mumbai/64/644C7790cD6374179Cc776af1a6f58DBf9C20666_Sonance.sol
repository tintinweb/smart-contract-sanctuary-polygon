...
interface ISonance {
     ...
     */
    function getTrackOwner(uint256 _songId) external view returns (address);
     ...
     */
    function getTrackInspired(uint256 _songId) external view returns (address);
    ...
    function isTrackExplicit(uint256 _songId) external view returns (bool);

    /*
     * @dev: Check if a user liked a song
...
contract Sonance is ReentrancyGuard {
    using Counters for Counters.Counter;

    ISonance public sonanceInterface; // interface for Sonance.sol
    address public admin; // admin address

    constructor() {
        sonanceInterface = ISonance(address(this));
        msg.sender == admin;
    }
   ...
   */
    ...
    */
    ...
    function deleteAdminTrack(uint256 _songId) external onlyAdmin {
        if (_songId > tracks.length) {
            revert DoesntExist();
        }
        delete tracks[_songId];
