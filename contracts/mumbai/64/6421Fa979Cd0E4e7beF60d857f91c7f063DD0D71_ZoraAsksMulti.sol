...
import "@zoralabs/v3/dist/contracts/modules/Asks/V1.1/AsksV1_1.sol";
import "@zoralabs/v3/dist/contracts/ZoraModuleManager.sol";

contract ZoraAsksMulti{
  AsksV1_1 public immutable asks;
  ...
  constructor(address _asks_address, address _moduleManager) {
    asks = AsksV1_1(_asks_address);
    ZMM = ZoraModuleManager(_moduleManager);
  }

  function approveZora() public {
    ZMM.setApprovalForModule(address(asks), true);
  }

  function createAsks(AskData[] memory _askarr) public {
    for(uint i=0; i<_askarr.length; i++){
        asks.createAsk(
          _askarr[i].tokenContract,
          _askarr[i].tokenId,
          _askarr[i].askPrice,
          _askarr[i].askCurrency,
          _askarr[i].sellerFundsRecipient,
          _askarr[i].findersFeeBps
        );
    }
  }
}

// [["0x675B6BFd4e709a8Fd68913CEa7d7BCB91Cb81b2B", 1, 100000, "0x0000000000000000000000000000000000000000", "0x5813f3CD51920e74388c53112B6fD551200e9461", 100]]
