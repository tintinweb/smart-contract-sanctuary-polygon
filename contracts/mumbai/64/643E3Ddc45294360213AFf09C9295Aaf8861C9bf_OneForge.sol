...
contract OneForge {
    ...
    mapping(address => mapping(uint256 => StructuredLinkedList.List))
        public list;
    mapping(address => mapping(uint256 => uint256[])) private getTokenId;
    mapping(address => uint256) public idCounter;
    uint256 constant INITIAL_LINKED_LIST_ID = 140000;
    ...
    constructor(MintNFT _mintNft) {
        mintNft = _mintNft;
    }

    function merge(uint256[] memory tokenId) external {
        uint256 arrayLength = tokenId.length;
        uint256 finalId = INITIAL_LINKED_LIST_ID + idCounter[msg.sender];

        for (uint256 i = 0; i < arrayLength; i++) {
            list[msg.sender][finalId].pushBack(tokenId[i]);

            emit MergeNFT(tokenId[i], finalId, msg.sender);
            mintNft.burn(tokenId[i]);
        }
        idCounter[msg.sender] += 1;
    }

    // Merge NFT's with already merged NFT's
    function mergedNFT(uint256[] memory tokenId) external {
        uint256 _arrayLength = tokenId.length;
        uint256 finalId = INITIAL_LINKED_LIST_ID + idCounter[msg.sender];

        for (uint256 i = 0; i < _arrayLength; i++) {
            list[msg.sender][finalId].pushBack(tokenId[i]);
            emit MergedNFT(tokenId[i], finalId, msg.sender);
        }
        idCounter[msg.sender] += 1;
    }
