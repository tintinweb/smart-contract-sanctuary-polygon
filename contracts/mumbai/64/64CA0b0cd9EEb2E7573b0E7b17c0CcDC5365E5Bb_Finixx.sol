...
contract Finixx {
        ...
        for(uint256 i = 0; i < teamUsers[_user][0].length; i++){
            ...
            for(uint256 k=user.weeklyBusiness.length; k>0; k--) {
                if(block.timestamp.sub(user.weeklyBusiness[k-1].time) <= timestep) {
                    uint256 _curWeekly = user.weeklyBusiness[k-1].amount;
    ...
    function getRegisterationData() public view returns(uint256, uint256) {
        return (userInfo[msg.sender].registered[0], userInfo[msg.sender].registered[1]);
    }
