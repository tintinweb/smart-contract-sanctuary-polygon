...
error SameBTCPrice();
error InvalidBTCAmount();
error InvalidETHAmount();

contract BTCExchange is AccessControl, Pausable, ReentrancyGuard, Initializer {
    ...
    /// @dev Latest WBTC PRICE
    uint256 public latestPrice;

    /// @dev Latest WBTC PRICE timestamp
    ...
    /// @dev Set WrappedBTC contract address
    ...
    /// @dev Withdraw WBTC
    function withdrawWBTC(address _who, uint256 _amount) external onlyOwner {
        getBTC().transfer(_who, _amount);
    }
    ...
    function sell(uint256 _amount) external nonReentrant {
        ...
        uint256 amount = _amount;
        getBTC().transferFrom(_msgSender(), address(this), amount);
    ...
    /// @dev Get WrappedBTC contract address
    function getBTC() public view returns (IERC20) {
        return IERC20(WBTC_TOKEN);
    }
