...
//可暂停带黑名单机制的采集器铸造合约
contract HarvesterFactory is Ownable, ReentrancyGuard, AccessControlEnumerable, IFactory, Pausable, Blacklistable {
    ...
    event OnGeneMetaData(
        ...
        uint256 energy_efficiency, 
        uint256 uru_efficiency, 
        uint256 max_durability,
        string metadata
    );
    ...
    //构造函数
    constructor(uint256 _salt) ReentrancyGuard() {
   ...
   // 合约部署者增加操作员
        ...
        returns (string memory)
    {
        ...
        emit OnGeneMetaData(_to, _id, energy_efficiency, uru_efficiency, max_durability, output);
        ...
        returns (string memory) 
    {
        ...
        uint256 energy_efficiency = characters[_id].energy_efficiency + allowUpgrades[hash].energy_efficiency;
        uint256 uru_efficiency = characters[_id].uru_efficiency + allowUpgrades[hash].uru_efficiency;
        ...
        emit OnUpgradeHarvester(_to, _id, energy_efficiency, uru_efficiency, max_durability, output);
        ...
        returns (bool)
    {
        ...
        emit OnAllowUpgrade(_to, _id, energy_efficiency, uru_efficiency, max_durability);
        return true;
    }
    ...
    // 获取用户拥有的采集器列表， 可用于游戏
    function getHarvesterListByOwner(
        address owner
    ) 
        public 
        view 
