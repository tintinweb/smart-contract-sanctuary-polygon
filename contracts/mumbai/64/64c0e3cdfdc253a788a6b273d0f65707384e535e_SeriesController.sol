    ...
    SeriesControllerStorageV2
{
     ...
     * SC2: !deployer
     * SC3: Reentrant
     * SC4: Not a series !series
     * SC5: Invalid dateComponent
     * SC6: Invalid _addressesProvider
     * SC7: Invalid _vault
     * SC8: Invalid _erc1155Controller
     * SC9: Invalid _fees
     * SC10: !PAUSER
     * SC11: !strikePrices
     * SC12: !Array
    ...
    modifier onlyOwner() {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "SC1");
        ...
        returns (ISeriesController.Series memory)
    {
        ...
        // check series exists
        require(series.expirationDate > 0, "SC4");
        return series;
    }
    ...
    function __SeriesController_init(
        ...
        require(_addressesProvider != address(0x0), "SC6");
        require(_vault != address(0x0), "SC7");
        require(_erc1155Controller != address(0x0), "SC8");
    ...
    function pause() external virtual {
        require(hasRole(PAUSER_ROLE, msg.sender), "SC10");
        _pause();
    }
    ...
    function createSeries(
        ...
        require(_strikePrices.length != 0, "SC11");

        require(_strikePrices.length == _expirationDates.length, "SC12");

        // validate token data
        require(_tokens.underlyingToken != address(0x0), "SC13");
        require(_tokens.priceToken != address(0x0), "SC14");
        require(_tokens.collateralToken != address(0x0), "SC15");

        // validate that the token data makes sense given whether it's a Put or a Call
        if (_isPutOption) {
            require(_tokens.underlyingToken != _tokens.collateralToken, "SC16");
        } else {
            require(_tokens.underlyingToken == _tokens.collateralToken, "SC17");
        }
        ...
        require(_restrictedMinters.length != 0, "SC18");
    ...
    function createSeriesInternal(
        ...
        require(_strikePrice != 0, "SC19");
        require(_expirationDate > block.timestamp, "SC20");

        // Validate the expiration has been added to the list by the owner
        require(allowedExpirationsMap[_expirationDate] > 0, "SC21");
        ...
        require(!addedSeries[seriesHash], "SC4");
        ...
        nonReentrant
    {
        ...
        require(allSeries.length > _seriesId, "SC22");

        require(state(_seriesId) == SeriesState.OPEN, "SC23");

        // Is the caller one of the AMM pools, which are the only addresses with the MINTER_ROLE?
        require(hasRole(MINTER_ROLE, msg.sender), "SC24");
    ...
    function exerciseOption(
        ...
        require(state(_seriesId) == SeriesState.EXPIRED, "SC25");
    ...
    function transferOwnership(address _newAdmin) external onlyOwner {
        require(_newAdmin != msg.sender, "SC26");
    ...
    function updateAddressesProvider(address _addressesProvider)
        ...
        onlyOwner
    {
        require(_addressesProvider != address(0x0), "SC27");
        addressesProvider = IAddressesProvider(_addressesProvider);
    }
        ...
        onlySeriesDeployer
    {
        ...
        for (uint256 i = 0; i < timestamps.length; i++) {
            ...
            require((block.timestamp < timestamps[i]), "SC28");
