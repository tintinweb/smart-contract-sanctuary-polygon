...
contract Campaign {
    ...
    mapping(address => mapping(uint256 => uint)) approvals;
    string public name;
    ...
    function approveRequest(uint256 index, uint approvalNumber) public {
        Request storage request = requests[index];

        require(approvers[msg.sender] > 0);
        require(approvals[msg.sender][index] == 0);
        require(request.endDate > block.timestamp);

        approvals[msg.sender][index] = approvalNumber;
        if (approvalNumber == 1) request.approvalCount++;
        else if (approvalNumber == 2) request.disapprovalCount++;
    }
