...
contract SeretanERC721v2fix is ERC721Enumerable, OperatorFilterer, ERC2981, Ownable, PullPayment {
  ...
  uint256 private numberOfMinted;
  mapping(address => uint256) private numberOfMintedBy;
  ...
  constructor(
    ...
    OperatorFilterer(subscription_, true)
  {
  ...
  function _safeMintOne(
    address to
  )
    internal
  {
    uint256 tokenId = numberOfMinted;
  ...
  function safeMintOne(
    ...
    uint256 maxNumberOfMintedByDest
  )
    public
    payable
  {
    ...
    if (phaseList[currentPhaseNumber].allowlistRoot != 0) {
      bytes32 allowlistLeaf = keccak256(bytes.concat(keccak256(abi.encode(to, maxNumberOfMintedByDest))));
    ...
    require(phaseList[currentPhaseNumber].maxNumberOfMinted > numberOfMinted, "Unable to mint anymore");

    require(maxNumberOfMintedByDest > numberOfMintedBy[to], "Unable to mint anymore");

    require(msg.value > phaseList[currentPhaseNumber].price, "Not enough money");
