...
contract AbdelNFTMarketplace is ERC1155Holder, ReentrancyGuard, Ownable {
    ...
    function getTotalServiceFeeAmount()
        external
        view
        onlyOwner
        returns (uint256)
    {
        return _totalServiceFeeAmount;
    }
    ...
    function listItem(
        ...
        require(
            listData.listingTime >= block.timestamp,
    ...
    // ) public itemExists(_itemId) nonReentrant {
    //     Item memory item_ = marketItems[_itemId];
    //     require(
    //         _msgSender() != item_.seller,
    ...
    //     );
    //     uint256 _totalPrice;
    //     if (acceptedOffer[_itemId][_msgSender()] == 0) {
    //         _totalPrice = item_.basePrice * _nftAmount;
    //     } else {
    //         _totalPrice =
    //             acceptedOffer[_itemId][_msgSender()] *
    //             numberOfNFT[_itemId];
    //     }
    ...
    function acceptOffer(
        ...
        require(
            ...
            "acceptOffer: Not enough tokens on sale"
        );
        ...
        require(signer == item_.seller, "redeem: unauthorized signer");
    ...
    function testing(uint256 max, Item memory item) external {}
}
