   ...
   struct ForwardRequest {
        ...
        ) public returns(bool[] memory) 
    {
        ...
        for(uint16 i = 0; i < requests.length; i++){            
            ...
            // bool value = IRecipientERC20(tokenAddr).metaTransfer(from, to, amount);
            // success[i] = value;

            success[i] = IRecipientERC20(requests[i].tokenAddr)
                            .metaTransfer(requests[i].from,requests[i].to,requests[i].amount);
        }
