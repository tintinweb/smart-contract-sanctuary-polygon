...
/// @title Base NFT contract for dispatch express ecommerce stores
/// @author Dispatch.co
/// @dev 1 store contract per org
contract ExpressDispatchStore is ERC721, IERC4906, Ownable, Pausable, ReentrancyGuard {
  using Strings for uint256;
  ...
  // we use numerical tokenIdCounters which increment every mint and decrement every burn
  uint256 public tokenIdCounter;
  // disables transfers when true
  bool public soulboundNFTs = false;
  ...
  event ExpressSaleEvent(
    address _receiver, // who receives the product
    uint256 _amountInBatch, // quantity of the product received
    uint256 _tokenId, // tokenId
    string _productCollectionUri, // name of collection (e.g., sweater or hat)
    uint256 _productId, // ID of the product (variant) purchased
  ...
  /**
      @notice Verifies the token has an assigned URI
      @param _tokenId ID of the token
   */
  function _isValidToken(uint256 _tokenId) private view {
    // checks that the user is holding the NFT
    require(bytes(tokenIdtoUri[_tokenId]).length > 0, "This product does not exist.");
  }

  /**
      @notice Checks that NFT transfers are enabled (they are by default)
   */
  function _transferIsAllowed() private view _isWhitelistedAddress {
    // checks that the user is holding the NFT
    require(soulboundNFTs == false, "This NFT is soulbound and cannot be transferred.");
  }
  ...
  /**
      @notice Allows owner to restrict transfers
      @param _isSoulBound bool value which restricts transfers
   */
  function setSoulboundNFTs(bool _isSoulBound) external onlyOwner {
    soulboundNFTs = _isSoulBound;
  }

  /**
      @notice Allows owner to set the tokens URI.
      @param _tokenId ID of the token to set
      @param _productCollectionUri name of collection (e.g., sweater or hat)
      @param _productId numerical ID of the product/variant in the dispatch database
   */
  function setTokenUri(
    uint256 _tokenId,
    string memory _productCollectionUri,
    uint256 _productId
  ) public _isWhitelistedAddress {
    tokenIdtoUri[_tokenId] = string(abi.encodePacked(_productCollectionUri, "/", _productId.toString()));
  }
  ...
  /**
      @notice Purchase a product using a whitelisted address
      @param _amount product amount to purchase
      @param _productCollectionUri name of collection (e.g., sweater or hat)
      @param _productId ID of the product (variant) purchased
  ...
  function mintTo(
    ...
    string memory _data,
    address _receiver
  ) public payable _isWhitelistedAddress nonReentrant whenNotPaused {
    // mint the NFT and increment and emit events per item purchased
    for (uint256 i = 0; i < _amount; i++) {
      // add to the overall totalSupply
      tokenIdCounter++;
      // map the tokenID to the collectionUri and productID
      setTokenUri(tokenIdCounter, _productCollectionUri, _productId);
      // mint token to sender * amount
      _mint(_receiver, tokenIdCounter);
      emit ExpressSaleEvent(_receiver, _amount, tokenIdCounter, _productCollectionUri, _productId, _data);
    }
  }

  /**
      @notice Purchase a product using a whitelisted address in bulk
      @param _amount product amount to purchase
      @param _productCollectionUri name of collection (e.g., sweater or hat)
      @param _productId ID of the product (variant) purchased
      @param _data additional data passed to event for logging purposes
      @param _receiver array of addresses to receiveNFT
   */
  function bulkMintTo(
    uint256 _amount,
    string memory _productCollectionUri,
  ...
  ) external payable _isWhitelistedAddress whenNotPaused {
    // mint the NFT and increment and emit events per item purchased
    for (uint256 i = 0; i < _receiver.length; i++) {
      mintTo(_amount, _productCollectionUri, _productId, _data, _receiver[i]);
    }
  }
  ...
  function _beforeTokenTransfer(
    ...
    // transfers can be disabled by setting the soulboundNFTs prop to true
    // a whitelisted address can always transfer NFTs
    if (from == address(0) || to == address(0)) {
      // do nothing since its a mint or burn
    } else {
      // run transfer allowance
      _transferIsAllowed();
    }
