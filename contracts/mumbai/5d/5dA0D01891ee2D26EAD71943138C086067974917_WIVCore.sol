...
contract WIVCore is ERC1155M, AccessControl, Initializable, Pausable {
     ...
     */
    function initialize(address admin, string memory uri) initializer external {
        ...
        // Setting the tokenUri for the metatokend
        _setURI(uri);
    }
...
contract ERC1155M is ERC1155SupplyNE, ReentrancyGuard {
    ...
    function proxyMetatokenCall(
        ...
        MetatokenDetails memory details = _metatokenDetails[IMetatoken1155(metatoken)];
        if (!details.registered) {
            revert NoRegisteredMetatoken();
        }
        if (!details.enabled) {
        ...
        if (!_allowedMetatokenSelectors[(uint256(uint160(address(metatoken))) << TOKEN_ADDRESS_SHIFT) | uint32(selector)]) {
        ...
        for (uint256 i; i < ids.length; i++) {
            ...
            if (ids[i] & TOKEN_ADDRESS_MASK == 0) {
                ...
                if (metatokenDetails.hooks & metatokenHooksMask == metatokenHooksMask) {
                    ...
                    metatokenAddress.functionDelegateCall(
