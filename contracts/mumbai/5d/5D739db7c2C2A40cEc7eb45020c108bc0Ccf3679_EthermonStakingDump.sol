...
contract EthermonStakingDump is EthermonStakingBasic {
    ...
    function getDumpStakerByIndex(uint256 _lockId, uint256 _index)
        public
        view
        returns (TokenData memory)
    {
        return stakingDump[_lockId][_index];
    }
    ...
    //OR we can use map(tokenId) => (map(address) => TokenData) here so no need to loop;
