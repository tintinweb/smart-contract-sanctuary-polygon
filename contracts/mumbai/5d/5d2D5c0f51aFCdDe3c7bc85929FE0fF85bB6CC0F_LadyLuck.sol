...
contract LadyLuck is Ownable {
    ...
    // uint256 public buyFeeForMarket = 100;
    // uint256 public buyFeeForBurn = 0;
    // uint256 public buyFeeForAddPool = 400;
    // uint256 public buyFeeForTeam = 0;

    // uint256 public sellFeeForMarket = 100;
    // uint256 public sellFeeForBurn = 0;
    // uint256 public sellFeeForAddPool = 400;
    ...
    function setFees(uint256 _FeeForMarket, uint256 _FeeForBurn, uint256 _FeeForAddPool, uint256 _FeeForTeam) external onlyOwner {
    ...
    function getPairInfo(address _pair) public {
        ...
        if (i == 1) {
            ...
            require(vipList[tx.origin], "e026");
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (pairAddressList.contains(recipient)) {
            ...
            if (ldxAdd) {
                if ((!isContract(sender) && !havepush[sender] && !vipList[sender]) || (isContract(sender) && sender != address(this) && !havepush[tx.origin] && !vipList[tx.origin])) {
        ...
        if (pairAddressList.contains(recipient) && !vipList[sender]) {
            if (ldxAdd && !liquifySlippage) {
                ...
                takeSellFee(sender, recipient, amount, FeeForMarket, FeeForBurn, FeeForAddPool, FeeForTeam);
            }
