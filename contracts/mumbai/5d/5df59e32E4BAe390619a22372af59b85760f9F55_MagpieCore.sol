...
contract MagpieCore is ReentrancyGuard, Ownable, Pausable, IMagpieCore {
  ...
  function swapOut(
    ...
    if (payload.destGasTokenAmount > 0){
      ...
      sourceToNative.routes[0].amountIn= payload.destGasTokenAmount;
      sourceToNative.routes[0].hops[0]=args.swapArgs.routes[0].hops[0];
  ...
  function revertSwapOut(bytes memory encodedVmCore, SwapInArgs calldata args,bool tokenBridge) external payable returns(bool){
