...
// SPDX-License-Identifier: UNLICENSED    
    ...
    */
    interface IERC721 {
        ...
        */
        ...
        function mint(address wallet, uint256 tokenId) external;
    }
    ...
    */
    ...
    contract HFRental is Ownable {
        ...
        event OfferClosed(uint256 indexed tokenId, uint256 OfferId);
        event BidPlaced(uint256 tokenId, uint256 OfferId, uint256 amount, address wallet);
        ...
        function isLive(uint256 tokenId) public view returns(bool){
            ...
            uint256 created = checkLatest(tokenId).createdAt;
            if(endTime==0 &&  checkLatest(tokenId).minPrice > 0 && !accepted && (created + 7 days) > block.timestamp ){
                return true;
            }
            if(endTime==0 && !accepted){
                return true;
            } else {
                return false;
            }        
 ...
 */
        function listOffer(uint256 tokenId, uint256 price, uint256 duration) external {
            ...
            rentOffer[totalOfferred] = RentOffer(tokenId, totalOfferred,
                 IERC721(nftContract).ownerOf(tokenId), 
                 address(0), 
                price, duration, 0, false, false, 0,0);
            totalOffers[tokenId].push(totalOfferred);
            emit OfferCreated(tokenId, totalOfferred);
        }

        function listOfferatMinPrice(uint256 tokenId, uint256 minPrice, uint256 duration) external {
            require(msg.sender == IERC721(nftContract).ownerOf(tokenId), "You are not the owner");
            require(duration > 0,"End time should be of future"); 
            totalOfferred++;
            rentOffer[totalOfferred] = RentOffer(tokenId, totalOfferred,
                 IERC721(nftContract).ownerOf(tokenId), 
                 address(0), 
                0, duration, 0, false, false, minPrice, (block.timestamp));
            totalOffers[tokenId].push(totalOfferred);
            emit OfferCreated(tokenId, totalOfferred);
        }  

        function makeBid(uint256 tokenId, uint256 bidPrice) external payable  {
            require(isLive(tokenId), "Offer is not live");
            require(msg.value == bidPrice, "Send proper bid price");
            RentOffer memory offer = checkLatest(tokenId);
            require(bidPrice >= offer.minPrice);
            if(bids[offer.offerId].length>0){
                require(bidPrice>bids[offer.offerId][bids[offer.offerId].length].amount);
                BID memory oldBid = bids[offer.offerId][bids[offer.offerId].length];
                payable(oldBid.wallet).transfer(oldBid.amount);
            }

            bids[offer.offerId][bids[offer.offerId].length.add(1)] = BID(msg.sender, bidPrice);
           emit BidPlaced(tokenId,  offer.offerId, bidPrice,msg.sender);

        }

        function closeBids(uint256 tokenId) public {
            RentOffer memory offer = checkLatest(tokenId);
            require(isLive(tokenId) == false, "It is live");
            require(offer.minPrice > 0, "It is not auction");
            if(bids[offer.offerId].length > 0){
            
            BID memory bid = bids[offer.offerId][bids[offer.offerId].length];
            rentOffer[totalOffers[tokenId][totalOffers[tokenId].length - 1]].borrower = bid.wallet;
            rentOffer[totalOffers[tokenId][totalOffers[tokenId].length - 1]].endTime = block.timestamp + offer.duration;
            payable(offer.owner).transfer(bid.amount);
            IERC721(rentedNFT).mint(bid.wallet, offer.tokenId);
        ...
        function matchOffer(uint256 tokenId) external payable {
            ...
            require(offer.cancelled == false, "Offer is Cancelled");
            require(offer.endTime>block.timestamp, "Offer Expired");
            rentOffer[offer.offerId].confirmed = true;
            rentOffer[offer.offerId].endTime = offer.duration + block.timestamp;
        ...
        function cancelOffer(uint256 tokenId) external {
            require(msg.sender == checkLatest(tokenId).owner || 
            msg.sender == checkLatest(tokenId).borrower, "You are not auth");
            require(checkLatest(tokenId).confirmed == false, "Offer is confirmed");
            rentOffer[checkLatest(tokenId).offerId].cancelled = true;
        }
