    ...
    AuctionMarketPlaceStorage
{
    ...
    function setAuctionTiming(
        ...
        require(
            ...
            "AuctionMarkerPlace: Auction Extend Duration must be less than Auction Duration"
        );
        auctionDuration = _auctionDuration;
        auctionExtendDuration = _auctionExtendDuration;
    }

    function createAuction(uint256 _tokenId, uint256 _minBidPrice)
        external
        nonReentrant
        whenNotPaused
    {
    ...
    function changeMinBidPrice(uint256 _tokenId, uint256 _newMinBidPrice)
        external
        whenNotPaused
    {
        Auction storage auction = auctionMapping[_tokenId];
        ...
        require(
            auction.initiated,
            "AuctionMarkerPlace: Auction not initiated."
        );
        require(!auction.hasBid, "AuctionMarkerPlace: Auction started.");
        require(
            auction.seller == msg.sender,
            "AuctionMarkerPlace: Only seller can change minimum bid price of auction."
        );
        ...
        whenNotPaused
    {
        ...
        require(
            ...
            "AuctionMarketPlace: Auction times up."
        );
        require(
            msg.value >= auction.minBidPrice,
            "AuctionMarketPlace: Bid Should be more than minimum bid price."
        );
        require(
            msg.value >= (auction.lastBidAmount * 11) / 10,
            "AuctionMarketPlace: Bid should be 10% more than last bid."
        );
        require(
            msg.sender != auction.seller,
            "AuctionMarketPlace: Seller can not bid on its owned auction."
        );
        ...
        nonReentrant
    {
        ...
        require(
            auction.seller == msg.sender,
            "AuctionMarkerPlace: Only seller can cancel auction."
        );
    ...
    function settle(uint256 _tokenId) external whenNotPaused nonReentrant {
        ...
        require(
            block.timestamp > auction.startedAt + auctionDuration &&
                block.timestamp > auction.lastBidAt + auctionExtendDuration,
            "AuctionMarketPlace: auction not finished yet."
        );
        (
            uint256 marketPlaceShareAmount,
            uint256 royaltyAmount,
            address tokenCreator
        ) = getMarketShareAndRoyalty(_tokenId, auction.lastBidAmount);
    ...
    function withdraw(uint256 _tokenId) external nonReentrant {
        uint256 balance = pendingWidthrawMapping[_tokenId][msg.sender];
        pendingWidthrawMapping[_tokenId][msg.sender] = 0;
        (bool sent, ) = payable(msg.sender).call{value: balance}("");
        require(sent);
    }

    function getAuctionMapping(uint256 _tokenId)
        external
        view
    ...
    function getPendingWidthrawMapping(uint256 _tokenId, address _address)
        external
        view
        returns (uint256)
    {
        ...
        )
    {
        ...
        if (isFirstSale) {
            marketShareAmount =
                (price *
                    (marketPlaceShare +
                        NFTationContract.getRoyaltyPercentage(tokenId))) /
                100;
            return (marketShareAmount, 0, address(0));
        } else {
            marketShareAmount = ((marketPlaceShare * price) / 100);
            (creator, royaltyAmount) = NFTationContract.royaltyInfo(
                tokenId,
                (price)
            );
...
contract NFTationStorage {
    ...
    address internal fixedPriceMarketPlaceContract;
    address internal auctionMarketPlaceContract;
...

{
    ...
    function aprovalMarketPlacesForAccount() external {
        setApprovalForAll(fixedPriceMarketPlaceContract, true);
        setApprovalForAll(auctionMarketPlaceContract, true);
        //TODO emit event
    }
    ...
    function disableFirstSale(uint256 _tokenId) external {
        require(
            ...
            "NFTation: token must be approved"
        );
    ...
    function initMarketPlaces(
        ...
        auctionMarketPlaceContract = 
            _auctionMarketPlaceContract;
        fixedPriceMarketPlaceContract = 
            _fixedPriceMarketPlaceContract;
    }
    ...
    function isMArketPlacesActiveForAccount() public view returns (bool) {
        return (isApprovedForAll(
            msg.sender,
            address(fixedPriceMarketPlaceContract)
...
contract AuctionMarketPlaceStorage {
    ...
    mapping(uint256 => mapping(address => uint256))
        internal pendingWidthrawMapping;
