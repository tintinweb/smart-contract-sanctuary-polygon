...
import "../functions/PerpetualVolumeTierFunctions.sol";
import "../../libraries/Utils.sol";

//import "hardhat/console.sol";

/**
 * This contract implements the fee tiers for brokers and traders.
 * Traders can be charged a broker fee and a protocol fee. The protocol fee
 * depends on (1) the broker credentials if the trader is trading via broker and
 * (2) on the trader credentials if the trader is not trading via broker OR if
 * trader credentials lead to better fees than the ones implied by broker credentials
 */
contract PerpetualBrokerFeeLogic is
    IFunctionList,
    IPerpetualBrokerFeeLogic,
    PerpetualVolumeTierFunctions
{
    function determineExchangeFee(IPerpetualOrder.Order memory _order)
        ...
        returns (uint16)
    {
        return _determineExchangeFee(_order);
    }

    /**
     * Update the EMA used only by contract
     * @param _iPerpetualId  perpetual ID
     * @param _traderAddr    address of the trader
     * @param _brokerAddr    address of the broker, 0x0 if no broker
     * @param _tradeAmountBC trade amount in base currency (can be negative)
     */
    function updateVolumeEMAOnNewTrade(
        ...
        //console.log("Contract: updateVolumeEMAOnNewTrade");
        require(msg.sender == address(this), "onlythis");
        _updateVolumeEMAOnNewTrade(_iPerpetualId, _traderAddr, _brokerAddr, _tradeAmountBC);
    }

    /**
     * query exchange fee for trader and broker. Set broker to address 0x0 and poolId is irrelevant if no broker.
     * @param   _poolId         id of liquidity pool. Not relevant if no broker (set e.g. 0)
     * @param   _traderAddr     address of the trader for which we want to get the fee
     * @param   _brokerAddr     address of the broker, set address(0x0) if trader has no broker
     * @return  exchange fee for given trader, broker, pool. In tenth of a basis point. E.g., 20 = 2bps
     */
    function queryExchangeFee(
        ...
        return _determineExchangeFee(_poolId, _traderAddr, _brokerAddr);
    }

    function splitProtocolFee(uint16 _fee) external pure override returns (int128, int128) {
        return _splitProtocolFee(_fee);
    }

    function setOracleForPerpetual(uint24 _iPerpetualId, address _oracleAddr)
        external
        override
        onlyOwner
    {
        _setOracleForPerpetual(_iPerpetualId, _oracleAddr);
    }

    function setUtilityTokenAddr(address tokenAddr) external override onlyOwner {
        utilityTokenAddress = tokenAddr;
    }

    function setFeesForDesignation(uint16[] memory _feesTbps) external onlyOwner {
        _setFeesForDesignation(_feesTbps);
    }

    function setBrokerTiers(uint256[] memory _tiers, uint16[] memory _feesTbps)
        external
        onlyOwner
    {
        require(_tiers.length + 1 == _feesTbps.length, "fees must fit tier");
        _setTiers(_tiers, _feesTbps, brokerTiers, brokerFeesTbps);
    }

    function setTraderTiers(uint256[] memory _tiers, uint16[] memory _feesTbps)
        external
        onlyOwner
    {
        _setTiers(_tiers, _feesTbps, traderTiers, traderFeesTbps);
    }

    function setTraderVolumeTiers(uint256[] memory _tiers, uint16[] memory _feesTbps)
        external
        onlyOwner
    {
        _setTiers(_tiers, _feesTbps, traderVolumeTiers, traderVolumeFeesTbps);
    }

    function setBrokerVolumeTiers(uint256[] memory _tiers, uint16[] memory _feesTbps)
        external
        onlyOwner
    {
        _setTiers(_tiers, _feesTbps, brokerVolumeTiers, brokerVolumeFeesTbps);
    }
    ...
    function getBrokerInducedFee(uint8 _poolId, address _brokerAddr)
        ...
        returns (uint16)
    {
        return _getBrokerInducedFee(_poolId, _brokerAddr);
    }

    function getFeeForBrokerDesignation(uint16 _brokerDesignation) external view returns (uint16) {
        return _getFeeForBrokerDesignation(_brokerDesignation);
    }
    ...
    function getBrokerDesignation(uint8 _poolId, address _brokerAddr)
        ...
        returns (uint16)
    {
        return brokerMap[_poolId][_brokerAddr];
    }

    function getFeeForBrokerStake(address _brokerAddr) external view returns (uint16) {
        return _getFeeForStake(_brokerAddr, brokerTiers, brokerFeesTbps);
    }

    function getFeeForTraderStake(address traderAddr) external view returns (uint16) {
        return _getFeeForStake(traderAddr, traderTiers, traderFeesTbps);
    }
    ...
    function getFeeForTraderVolume(uint8 _poolId, address _traderAddr)
        ...
        returns (uint16)
    {
        return _getFeeForTraderVolume(_poolId, _traderAddr);
    }
    ...
    function getFeeForBrokerVolume(uint8 _poolId, address _brokerAddr)
        ...
        returns (uint16)
    {
        return _getFeeForBrokerVolume(_poolId, _brokerAddr);
    }

    function getFunctionList() external pure virtual override returns (bytes4[] memory, bytes32) {
        bytes32 moduleName = Utils.stringToBytes32("PerpetualBrokerFeeLogic");
        bytes4[] memory functionList = new bytes4[](19);
        functionList[0] = this.determineExchangeFee.selector;
        functionList[1] = this.setFeesForDesignation.selector;
        functionList[2] = this.setTraderTiers.selector;
        functionList[3] = this.setBrokerTiers.selector;
        functionList[4] = this.getBrokerInducedFee.selector;
        functionList[5] = this.getFeeForBrokerDesignation.selector;
        functionList[6] = this.getFeeForBrokerStake.selector;
        functionList[7] = this.getFeeForTraderStake.selector;
        functionList[8] = this.setUtilityTokenAddr.selector;
        functionList[9] = this.getFeesForDesignation.selector;
        functionList[10] = this.splitProtocolFee.selector;
        functionList[11] = this.queryExchangeFee.selector;
        functionList[12] = this.updateVolumeEMAOnNewTrade.selector;
        functionList[13] = this.setOracleForPerpetual.selector;
        functionList[14] = this.getLastPerpetualBaseToUSDConversion.selector;
        functionList[15] = this.setTraderVolumeTiers.selector;
        functionList[16] = this.setBrokerVolumeTiers.selector;
        functionList[17] = this.getFeeForBrokerVolume.selector;
        functionList[18] = this.getFeeForTraderVolume.selector;
...
// SPDX-License-Identifier: MIT
pragma solidity 0.8.16;
import "./PerpetualBrokerFeeFunctions.sol";

//import "hardhat/console.sol";

contract PerpetualVolumeTierFunctions is PerpetualBrokerFeeFunctions {
    using ABDKMath64x64 for int128;
    uint256[] public traderVolumeTiers;
    uint256[] public brokerVolumeTiers;
    uint16[] public traderVolumeFeesTbps;
    uint16[] public brokerVolumeFeesTbps;
    mapping(uint24 => address) perpBaseToUSDOracle;
    mapping(uint24 => int128) perpToLastBaseToUSD;
    uint64 lastBaseToUSDUpdateTs;
    uint64 constant BASE_TO_USD_UPDATE_TIME = 3600; //how frequent (seconds) do we update the price info
    struct VolumeEMA {
    ...
    int128 constant lambda = 18446736956910141440; //round((1 - 1/(30*24*60*60))*(2^64))

    // E( EMA ) = 1 / (1 - L) * E (trade size) ?=  N * E (trade size)
    // --> L = 1 - 1 / N

    function _setOracleForPerpetual(uint24 _iPerpetualId, address _oracleFactoryAddr) internal {
        perpBaseToUSDOracle[_iPerpetualId] = _oracleFactoryAddr;
    }

    /**
     * Determine exchange fee based on order
     * If broker signature invalid, the contract execution reverts
     * If there is no signature but a broker address, the broker address is ignored
     * @param _order    order struct
     * @return trading fee charged by protocol
     */
    function _determineExchangeFee(IPerpetualOrder.Order memory _order)
        internal
        view
        returns (uint16)
    {
        ...
        bytes32 digest = _getBrokerDigest(_order, address(this));
    ...
    function _determineExchangeFee(
        uint8 _poolId,
        address _traderAddr,
        address _brokerAddr
    ) internal view returns (uint16) {
        uint16 fFeeTbps = _getTraderInducedFee(_poolId, _traderAddr);
        if (_brokerAddr != address(0x0)) {
            uint16 fBrokerInducedFee = _getBrokerInducedFee(_poolId, _brokerAddr);
    ...
    /**
     * The USD conversion uses the perpetual S2 price, unless
     * there is a different oracle set via _setOracleForPerpetual.
     * @param _iPerpetualId id of the perpetual
     * @return current conversion rate from base to quote (e.g., BTC-price)
     */
    function _getPerpetualBaseToUSDConversion(uint24 _iPerpetualId) internal returns (int128) {
        int128 fPrice;
        if (uint64(block.timestamp) < BASE_TO_USD_UPDATE_TIME + lastBaseToUSDUpdateTs) {
            // no update required yet
            return perpToLastBaseToUSD[_iPerpetualId];
        }
        if (perpBaseToUSDOracle[_iPerpetualId] == address(0)) {
            // if no oracle is set then the perpetual instrument is quoted in USD
            // we can use S2 from the perpetual
            fPrice = _getSafeOraclePriceS2(_getPerpetual(_iPerpetualId));
        } else {
            bytes4 base = _getPerpetual(_iPerpetualId).S2BaseCCY;
            OracleFactory oracle = OracleFactory(perpBaseToUSDOracle[_iPerpetualId]);
            (fPrice, ) = oracle.getSpotPrice(base, bytes4(bytes("USD")));
        }
        // the price could be 0 if the market is closed,
        // in this case we use the last non-zero price
        if (fPrice == 0) {
            return perpToLastBaseToUSD[_iPerpetualId];
        } else {
            perpToLastBaseToUSD[_iPerpetualId] = fPrice;
            lastBaseToUSDUpdateTs = uint64(block.timestamp);
            return fPrice;
        }
    }

    function _getFeeForTraderVolume(uint8 _poolId, address _traderAddr)
        internal
        view
        returns (uint16)
    {
        int128 tradingVolumeUSD = traderVolumeEMA[_poolId][_traderAddr].fTradingVolumeEMAusd;
        return
            _getVolumeTierFee(
                uint256(ConverterDec18.toDec18(tradingVolumeUSD)),
                traderVolumeTiers,
                traderVolumeFeesTbps
            );
    }

    function _getFeeForBrokerVolume(uint8 _poolId, address _brokerAddr)
        internal
        view
        returns (uint16)
    {
        int128 tradingVolumeUSD = brokerVolumeEMA[_poolId][_brokerAddr].fTradingVolumeEMAusd;
        return
            _getVolumeTierFee(
                uint256(ConverterDec18.toDec18(tradingVolumeUSD)),
                brokerVolumeTiers,
    ...
    /**
     * Search for fee implied by trading volume for either broker or trader
     * @param   _tradingVolumeUSD  USD trading volume in DEC18 format
     * @param   _volumeTier        array of volume tiers
     * @param   _volumeFees        array of fees in Tenth of Bps
     * @return  trading fee in tenth of basis points
     */
    function _getVolumeTierFee(
        uint256 _tradingVolumeUSD,
        uint256[] storage _volumeTier,
        uint16[] storage _volumeFees
    ) internal view returns (uint16) {
        return _searchFee(_tradingVolumeUSD, _volumeTier, _volumeFees);
    }
     ...
     * @param _tradeAmountBC trade amount in base currency (can be negative)
     */
    function _updateVolumeEMAOnNewTrade(
        uint24 _iPerpetualId,
        address _traderAddr,
        address _brokerAddr,
        int128 _tradeAmountBC
    ) internal {
        uint8 _poolId = _getPoolIdFromPerpetual(_iPerpetualId);
        _tradeAmountBC = _tradeAmountBC.abs();
        // convert to USD
        _tradeAmountBC = _tradeAmountBC.mul(_getPerpetualBaseToUSDConversion(_iPerpetualId));
        if (_brokerAddr != address(0x0)) {
            // update broker EMA
            VolumeEMA storage brkrEMA = brokerVolumeEMA[_poolId][_brokerAddr];
            _ema(brkrEMA, _tradeAmountBC);
        }
        // update trader EMA
        VolumeEMA storage trdrEMA = traderVolumeEMA[_poolId][_traderAddr];
        _ema(trdrEMA, _tradeAmountBC);
    }

    function _ema(VolumeEMA storage vol, int128 _absTradeAmountUSD) internal {
        uint64 ts = uint64(block.timestamp);

        uint256 iDeltaTime = uint256(ts - vol.timestamp);
        int128 fLAdj;
        // if deltatime > 7 months, L^timediff would be less than 0.1%
        // so we don't exponentiate but set fLAdj to 0
        // equivalently, the volume ema resets if there is no activity in 7 months
        if (iDeltaTime > 18000000) {
            fLAdj = 0; // L^dT < 1% for dT=30days
        } else {
            fLAdj = lambda.pow(iDeltaTime);
        }
        vol.timestamp = ts;
        vol.fTradingVolumeEMAusd = vol.fTradingVolumeEMAusd.mul(fLAdj).add(_absTradeAmountUSD);
    }

    function _getBrokerInducedFee(uint8 _poolId, address _brokerAddr)
        internal
        view
        returns (uint16)
    {
        uint16 iBrokerDesignation = brokerMap[_poolId][_brokerAddr];
        if (iBrokerDesignation == 0) {
            // the broker did not invest a lot in this pool
            return 65535; //max uint16
        }
        uint16 fFee1 = _getFeeForBrokerDesignation(iBrokerDesignation);
        uint16 fFee2 = _getFeeForStake(_brokerAddr, brokerTiers, brokerFeesTbps);
        uint16 fFee3 = _getFeeForBrokerVolume(_poolId, _brokerAddr);
        return fFee1 > fFee2 ? (fFee1 > fFee3 ? fFee1 : fFee3) : (fFee2 > fFee3 ? fFee2 : fFee3);
    }

    function _getTraderInducedFee(uint8 _poolId, address _traderAddr)
        internal
        view
        returns (uint16)
    {
 ...
 * trader credentials lead to better fees than the ones implied by broker credentials
 */
contract PerpetualBrokerFeeFunctions is PerpetualBaseFunctions, PerpetualHashFunctions {
    using ABDKMath64x64 for int128;
    uint16[] public designationToFee;
    uint256[] public traderTiers;
    uint256[] public brokerTiers;
    uint16[] public traderFeesTbps;
    uint16[] public brokerFeesTbps;

    address public utilityTokenAddress;
    int128 public constant PNL_PART_FEE_RATIO = 0x5555555555555555; //ABDK(0x5555555555555555)=0.333

    function _getFeeForBrokerDesignation(uint16 _brokerDesignation)
        internal
        view
        returns (uint16)
    {
        require(designationToFee.length > 0, "no designation fee set");
        uint256 index = _brokerDesignation - 1;
        if (index > designationToFee.length - 1) {
            index = uint16(designationToFee.length) - 1;
        }
        return designationToFee[index];
    }

    function _splitProtocolFee(uint16 fee) internal pure returns (int128, int128) {
        int128 treasuryfee = (ConverterDec18.TBPSTOABDK * int128(uint128(fee)));
        int128 pnlPartFee = treasuryfee.mul(PNL_PART_FEE_RATIO);
        treasuryfee = treasuryfee.sub(pnlPartFee);
        return (treasuryfee, pnlPartFee);
    }

    function _setFeesForDesignation(uint16[] memory _feesTbps) internal {
        uint256 k;
        // delete old array (if it exists)
        while (designationToFee.length > 0) {
            designationToFee.pop();
        }
        assert(designationToFee.length == 0);
        // set fees
        for (k = 0; k < _feesTbps.length; k++) {
            require(_feesTbps[k] >= 0, "fee negative");
            designationToFee.push(_feesTbps[k]);
        }
    }

    function _setTiers(
        uint256[] memory _tiers,
        uint16[] memory _feesTbps,
        uint256[] storage _tiersTarget,
        uint16[] storage _feesTbpsTarget
    ) internal {
        require(_tiers.length + 1 == _feesTbps.length, "fees must fit tier");
        uint256 k;
        // delete old arrays (if it exists)
        if (_feesTbpsTarget.length > 0) {
            _feesTbpsTarget.pop();
            while (_tiersTarget.length > 0) {
                _feesTbpsTarget.pop();
                _tiersTarget.pop();
            }
            assert(_feesTbpsTarget.length == 0);
            assert(_tiersTarget.length == 0);
        }
        // set fees
        for (k = 0; k < _tiers.length; k++) {
            require(_feesTbps[k] > 0, "fee negative");
            _tiersTarget.push(_tiers[k]);
            _feesTbpsTarget.push(_feesTbps[k]);
        }
        _feesTbpsTarget.push(_feesTbps[k]);
    }

    /**
     *  Get token amount and search for fee
     * @param _theOneWithAStake address of broker or trader
     * @param _tiers            tier-array for broker or trader
     * @param _fees             fee-array for broker or trader
     */
    function _getFeeForStake(
        address _theOneWithAStake,
        uint256[] storage _tiers,
        uint16[] storage _fees
    ) internal view returns (uint16) {
        if (_fees.length == 1) {
            // no need for utility token if there is only 1 fee
            // OR if utility token not set, we reply with worst fee
            // which is in _fees[0]
            return _fees[0];
        }
        require(utilityTokenAddress != address(0x0), "utility tkn not set");
        IERC20Upgradeable utilityToken = IERC20Upgradeable(utilityTokenAddress);
        uint256 amount = utilityToken.balanceOf(_theOneWithAStake);
        return _searchFee(amount, _tiers, _fees);
    }

    /**
     * find the fee that corresponds to the tier implied by the amount
     * @param _amount   amount of utility tokens owned
     * @param _tiers    array with tier limits
     * @param _fees     array with fees in tenth of basis points
     */
    function _searchFee(
        uint256 _amount,
        uint256[] storage _tiers,
        ...
        uint256 k = 0;
        while (k < _tiers.length && _amount > _tiers[k]) {
            k = k + 1;
        }
        return _fees[k];
    }
