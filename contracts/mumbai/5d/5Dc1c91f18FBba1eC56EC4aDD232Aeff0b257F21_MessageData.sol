...
contract MessageData {
    ...
    */
    ...
    function updateContracts(address _kutils, address _hashtags, address _tagged, address _posts, address _messageFormat, address _followers, address _groupPosts, address _groups) public onlyAdmins {
    ...
    */
    ...
    function saveMsg(MsgData memory newMsg) public onlyAdmins {
        ...
        string memory thisBucketKey = append('posts-',KUtils.toString(buckets[0]),'','','');

        // if it's a comment, store the map to isCommentOf
        if (newMsg.isCommentOf > 0){

            // Get the bucket key to save the comment into
            string memory comBucketKey = getBucketKey(KUtils.toString(newMsg.isCommentOf), true);

            // Add to the comment bucket
            msgMap[comBucketKey].push(newMsg.msgID);

            // Add to the comment bucket flag
            msgMapMap[comBucketKey][newMsg.msgID] = true;
    ...
    function removeMsg(uint256 msgID, address requester) public onlyAdmins {
        ...
        if (thisMsgData.isCommentOf > 0){
            ...
            string memory comBucketKey = getBucketKeyByID(thisMsgData.isCommentOf);

            // Remove a comment stat
            addStat(2, thisMsgData.isCommentOf, -1, 0);

            // Remove the comment bucket flag
            msgMapMap[comBucketKey][thisMsgData.msgID] = false;

            delete comBucketKey;
        } else if (thisMsgData.isRepostOf > 0){
            // Find the bucket containing the message commented on
            string memory repBucketKey = getBucketKeyByID(thisMsgData.isRepostOf);

            // Remove a comment stat
            addStat(3, thisMsgData.isRepostOf, -1, 0);

            // Remove the comment bucket flag
            msgMapMap[repBucketKey][thisMsgData.msgID] = false;
        ...
        // If this is a repost, also remove the message from repost buckets
    ...
    function getMsgsByIDs(uint256[] memory msgIDs) public onlyAdmins view returns (string[][] memory){
        ...
        for (uint i=0; i < msgIDs.length; i++) {
            ...
            thisData[3] = thisMsgData.msgStats.tipsReceived;
            ...
            allData[i] = MessageFormat.buildMsg(thisData, thisMsgData.message, thisMsgData.postedBy, thisMsgData.hashtags, thisMsgData.taggedAccounts, thisMsgData.uri, thisMsgData.inGroups);

            delete thisData;
        }

//        console.log('inside', allData.length);
//        console.log('inside', allData[0]);
    ...
    function getBucketKey(string memory mapKey, bool toInsert) private view returns (string memory){
        ...
        while (mapID == 99999999999999999999){
            ...
            if (msgMap[bucketToCheck].length > 0){
                // exists
                b++;
                prevBucketLen = msgMap[bucketToCheck].length;
            } else if (b == 0) {
        ...
        delete b;
        delete prevBucketLen;
        return KUtils.append(mapKey,'-',KUtils.toString(mapID),'','');
    }
