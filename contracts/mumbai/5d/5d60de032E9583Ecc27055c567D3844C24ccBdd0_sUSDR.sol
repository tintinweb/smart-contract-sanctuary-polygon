...
contract sUSDR is AddressAccessor, ERC20Permit {
    using SafeERC20 for IERC20;

    event Rebase(uint256 indexed blockNumber, uint256 totalSupply);

    uint8 private constant DECIMALS = 9;

    uint256 private constant MAX_SUPPLY = 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFE;
    ...
    constructor() ERC20("Staked USDR", "sUSDR") ERC20Permit("Staked USDR") {
    ...
    {
        if (supplyDelta == 0) {
            emit Rebase(block.number, _totalSupply);
            return _totalSupply;
        }
        ITreasury treasury;
        (
            ...
            );
        {
            ...
            underlying.safeTransferFrom(msg.sender, address(this), supplyDelta);
            underlying.approve(address(treasury), supplyDelta);
            treasury.deposit(underlying_, supplyDelta);
            supplyDelta = IExchange(exchange).scaleFromUnderlying(supplyDelta);
        }
        uint256 totalSupply_ = _totalSupply + supplyDelta;
        if (totalSupply_ > MAX_SUPPLY) {
            totalSupply_ = MAX_SUPPLY;
        }
        {
            // check backing
            (uint256 stable, uint256 rwa, uint256 tngbl) = ITreasury(treasury)
                .getTreasuryValue();
            uint256 usdrMarketCap = IERC20(usdr).totalSupply() -
                IERC20(usdr).balanceOf(staking) +
                totalSupply_;
            require(
                usdrMarketCap * 1e9 > stable + rwa + tngbl,
                "insufficient backing"
            );
        }
        emit Rebase(block.number, totalSupply_);
        return _totalSupply = totalSupply_;
    }

    function balanceOf(address account) public view override returns (uint256) {
        return balanceForScaledBalance(_scaledBalances[account]);
    }

    function scaledBalanceForBalance(uint256 amount)
        public
        view
        returns (uint256 scaledAmount)
    {
        assembly {
            let total_supply := sload(_totalSupply.slot)
            if iszero(iszero(total_supply)) {
                scaledAmount := div(
                    mul(amount, sload(_totalSupplyScale.slot)),
                    total_supply
                )
            }
        }
    ...
    function balanceForScaledBalance(uint256 scaledAmount)
        public
        view
        returns (uint256 amount)
    {
        assembly {
            let scale := sload(_totalSupplyScale.slot)
            if iszero(iszero(scale)) {
                amount := and(
                    div(mul(scaledAmount, sload(_totalSupply.slot)), scale),
                    MAX_SUPPLY
                )
            }
        }
    ...
    function transfer(address to, uint256 amount)
        ...
        returns (bool)
    {
        uint256 scaledAmount = scaledBalanceForBalance(amount);
    ...
    function transferAll(address to) external returns (bool) {
        uint256 scaledAmount = _scaledBalances[msg.sender];
        delete _scaledBalances[msg.sender];
        _scaledBalances[to] += scaledAmount;
        emit Transfer(msg.sender, to, balanceForScaledBalance(scaledAmount));
        return true;
    }
    ...
    function transferAllFrom(address from, address to) public returns (bool) {
        uint256 scaledAmount = _scaledBalances[from];
        uint256 amount = balanceForScaledBalance(scaledAmount);
        _spendAllowance(from, msg.sender, amount);
        delete _scaledBalances[from];
        _scaledBalances[to] += scaledAmount;
        emit Transfer(from, to, amount);
        return true;
    }
        ...
        onlyRole(MINTER_ROLE)
    {
        ...
        require(amount <= MAX_SUPPLY - _totalSupply, "max supply exceeded");

        _beforeTokenTransfer(address(0), account, amount);

        if (_totalSupply == 0) {
            _totalSupplyScale = amount;
            _totalSupply = amount;
        } else {
            _totalSupplyScale =
                (_totalSupplyScale * (_totalSupply + amount)) /
                _totalSupply;
            _totalSupply += amount;
        }
        _scaledBalances[account] += scaledBalanceForBalance(amount);
    ...
    function burn(address account, uint256 amount) external {
        ...
        uint256 scaledAmount = scaledBalanceForBalance(amount);
        uint256 accountBalance = _scaledBalances[account];

        require(accountBalance >= scaledAmount, "burn amount exceeds balance");
...
contract AddressAccessor is AccessControl {
