...
contract StakeInTheTown is Staking721Base {
    ...
    function getUserMultiplier () external view returns (uint256 rewardMult) {
        ...
        if (multiplierToggle && block.timestamp < multiplierEndTime[msg.sender]) {
            if (rewardMult < 1) {
                rewardMult += 1;
            }
        }
    ...
    function getMultiplierRewardSettings() external view returns (uint256, bool, uint256, string memory, uint256) {
        return (
            rewardMultiplier,
            multiplierToggle,
    ...
    function getFixedRewardSettings() external view returns (uint256, bool, uint256, string memory, uint256) {
        return (
            fixedReward,
            fixedRewardToggle,
    ...
    function _claimRewards() internal override {
        ...
        if (multiplierToggle && block.timestamp < multiplierEndTime[msg.sender]) {
            ...
            rewards = rewards * rewardMultiplier;
        }
    ...
    function setSideCondition() external virtual {
        if (!_canSetStakeConditions()) {
            revert("Not authorized");
        }
        ...
        sideConditionId = _stakerConditionId;
    }

    // Secret word feature functions

    //function setSecretWord(string memory newSecretWord) external onlyOwner {
