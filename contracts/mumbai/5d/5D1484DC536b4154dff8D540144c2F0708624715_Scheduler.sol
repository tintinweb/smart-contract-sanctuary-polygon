...
// store no funds
// store no assets
// support to withdraw accidental deposit
    ...
    SchedulerStorage
{
    ...
    function initialize(
        ...
        setUp(abi.encode(_owners, _ops, _safeFactory, _singleton));
    }
        ...
        initializer
    {
        ...
        // create safe
        GnosisSafe safe = _createSafe(_owners, _safeFactory, _singleton);
    ...
    function executeSchmint(uint256 schmintId) external nonReentrant {
        // -> check if user is eligible for free schmint
        // -> transfer tokens in specified wallet
        // -> if user wants to receive in their wallet, transfer tokens to their wallet
        // -> transfer schminting fees

        // read schmint
        Schmint memory schmint = schmints[schmintId];

        // scoping checks into one block
        {
            // ensure the schmint is not executed
            // cannot execute schmint twice
        ...
        // write updated schmint struct to storage
        schmints[schmintId] = schmint;
...
contract SchedulerStorage {
    // number of schmints executed
    uint256 public schmintsExecuted;
    // counter to make count of schmints
    uint256 public schmintCounter;
    // mapping of schmint ID to schmint
    ...
    struct Schmint {
        ...
        uint256 value; // value to be sent to taret while schminting
        uint256 gasPriceLimit; // limit of gas price, if gas price above this value, schmint will not be executed
...
contract GnosisUtils is ISignatureValidator {
    ...
    // create safe
    function _createSafe(address[] memory _owners, address _safeFactory, address _singleton) internal returns(GnosisSafe safe) {
    ...
    function _enableModule(GnosisSafe safe) internal {
        ...
        // generate signature
        bytes memory temporarySignature = _generateSignature(
            execData,
            safe
        );
    ...
    function _removeOwner(GnosisSafe safe, address[] memory _owners) internal {
        ...
        // generate signature
            ...
            bytes memory temporarySignatureToRemoveOwner
         = _generateSignature(execDataToRemoveOwner, safe);
