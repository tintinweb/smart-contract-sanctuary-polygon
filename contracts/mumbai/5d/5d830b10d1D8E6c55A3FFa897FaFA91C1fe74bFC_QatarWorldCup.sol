...
contract QatarWorldCup is ERC721Enumerable, Ownable {
    ...
    uint256 public  mintPrice = 5000000000000000; //0.005 MATIC
    ...
    function mintBatch721Token(uint256 _numberOfTokens) public payable hasMintRunning {
        require(_numberOfTokens <= maxQWCPurchase, "QWC: Mint more than allowed at a time");
        require(totalSupply().add(_numberOfTokens) <= MAX_SUPPLY, "QWC: Purchase would exceed max supply of Apes");
        require(mintPrice.mul(_numberOfTokens) <= msg.value, "QWC: Ether value sent is not correct");
