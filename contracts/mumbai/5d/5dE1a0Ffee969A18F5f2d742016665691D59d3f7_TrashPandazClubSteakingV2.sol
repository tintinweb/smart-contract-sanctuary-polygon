...
// File: TrashPandazClubSteakingV2_flat.sol


pragma solidity 0.8.11;

/* TrashPandazClub Steaking V2 🥩 */
...
contract TrashPandazClubSteakingV2 is ERC721Holder, ReentrancyGuard, Ownable, Pausable {
    ...
    uint256 public rewardRateTimeframe = 86400; //seconds in a day
    uint256 public rewardRate = 10000000000000000000 / rewardRateTimeframe; //rewards per day. Default is 10 trash / day
        ...
        if (rewardDividedByPool == true){
            return
                rewardPerTokenStored +
                    ((rewardRate * (getMultiplier(_account) / 100)) * (block.timestamp - lastUpdateTime) * 1e18 / totalSupply) ;
        }
        return
            rewardPerTokenStored +
                    ((rewardRate * (getMultiplier(_account) / 100)) * (block.timestamp - lastUpdateTime) * 1e18) ;
    }

    function viewStakedIds(address _account) public view returns (uint256[] memory){ // may need to add memory after uint256[]
        uint256[] memory _ids;
        uint count = 0;
        for (uint256 i = 0; i < 4444; i += 1) {
            if( stakedAssets[i] == _account ){
                _ids[count] = i;
                count ++;
            }
        }
    ...
    function earned(address _account) public view returns (uint256) { //maybe change to msg.sender instead of _account?
        return
            ((balances[_account] *
                ((rewardPerToken(_account)) - userRewardPerTokenPaid[_account])) / 1e18) +
                    rewards[_account];
    }

    function contractRewardBalance() public view returns (uint){
        IERC20 x = IERC20(rewardsToken);
    ...
    function getReward() public nonReentrant updateReward(msg.sender) {
        ...
        if (reward > 0 && reward < contractRewardBalance()) {
            rewards[msg.sender] = 0;
            rewardsToken.safeTransfer(msg.sender, reward);
            emit RewardPaid(msg.sender, reward);
        } else if (reward > contractRewardBalance() && contractRewardBalance() > 0) {
            rewards[msg.sender] = 0;
            rewardsToken.safeTransfer(msg.sender, contractRewardBalance());
            emit RewardPaid(msg.sender, contractRewardBalance());
        }
    }
    ...
    function walletHoldsToken(address _account, address _contract, uint256 _id) internal view returns (uint256) {
        ...
        return token.balanceOf(_account, _id); 
    }

    function walletHoldsTrashLord() internal view returns (bool) {
        ...
        IERC1155 token = IERC1155(_c_addr);
        //use msg.sender instead of _account to get the user wallet address
        return token.balanceOf(msg.sender, _t_id) > 0; 
    }

    //call this function when staking or if user wants to update their multiplier
    function getMultiplier(address _account) public view returns (uint256){ 
        //declared within function so it isnt stored on blockchain.
    ...
    // set reward amount
    function setReward(uint256 _rewardRate, uint256 _rewardRateTimeframe) external onlyOwner {
        rewardRateTimeframe = _rewardRateTimeframe;
        rewardRate = _rewardRate / _rewardRateTimeframe;
        emit RewardUpdated(rewardRate, rewardRateTimeframe);
    }

    // remove rewards in case of bad actors
    function removeRewardTokens(uint256 _tokenAmount) external onlyOwner{
        rewardsToken.safeTransfer(msg.sender, _tokenAmount);
        emit RewardRemoved(msg.sender, _tokenAmount);
    }
    ...
    // adjust reward rate per token
    modifier updateReward(address _account){
        rewardPerTokenStored = rewardPerToken(_account);
    ...
    event RewardUpdated(uint256 rewardRate, uint256 rewardRateTimeframe);
    ...
    event RewardRemoved(address indexed user, uint256 tokenAmount);
}