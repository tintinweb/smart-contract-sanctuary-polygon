...
//    mapping (address => uint48)public TimeUpdateByUser;
	...
	function GroupMemberShowVal(uint8 id)public view returns(uint8[] memory out)
	{
	    address[] memory addrs = GroupMemberShow(id);
	    uint8 len = uint8(addrs.length);
	    for(uint8 i = 0;i < len;i++)
	    {
		out[i] = GroupMemberVal(id,addrs[i]);
	    }
	}
