...
contract TeamsStakingDeadlines is OwnableUpgradeable {
    ...
    /**
     * @notice Set cardImage's team id. If team id is 0, it means the card image is not assigned to any team. Only owner (back-end) can call this function.
     * @param _cardImageId card image id
     * @param _teamId team id
     */
    function setCardImageToTeam(uint256 _cardImageId, uint256 _teamId) external onlyOwner nonZeroTeamId(_teamId) {
    ...
    /**
     * @notice Set cardImage's team id. If team id is 0, it means the card image is not assigned to any team. Only owner (back-end) can call this function.
     * @param _cardImageIds card image ids
     * @param _teamId team id
     */
    function setCardImagesToTeam(uint256[] calldata _cardImageIds, uint256 _teamId) external onlyOwner nonZeroTeamId(_teamId) {
        for(uint256 i = 0; i < _cardImageIds.length; i++){
            require(nomoNFT.cardImageToExistence(_cardImageIds[i]), "CardImage doesn't exist");
            cardImageToTeam[_cardImageIds[i]] = _teamId;
            emit UpdatedCardImageToTeam(_cardImageIds[i], _teamId);
        }
    }
