...
contract Market is Context, ERC721Holder, Ownable {
        ...
        returns (int256)
    {
        if (amount < 1 ether) {
            ...
            int256 val = int256(amount) / 1e18;
            uint256 strLength = uintToString(uint256(val)).length;
            return int256(strLength) + minus;
        }
    }
    ...
    function _multiplyAndPow(
        ...
        int256 mulVal = int256(multiply) / 1 ether;
        if (powVal < 0) {
            return uint256((mulVal * 10) / 100);
        } else {
            if (powVal == 0) {
                return uint256(multiply * mulVal);
            } else {
                uint256 mul = (10 ** uint256(powVal) * 1 ether);
                return uint256(mulVal * int256(mul));
            }
        }
    }
