...
struct Ticket {
    ...
    uint winRewardRule;
    bool clamStatus;
    mapping(uint => uint32) numberPerIndexed;
}
...
contract QulotLottery is ReentrancyGuard, IQulotLottery, Ownable {
    ...
    function _processOrderTicket(uint32[] calldata _ticketNumbers, uint256 _roundId) internal returns (uint256) {
        ...
        for (uint i; i < numberLength; ) {
            ticket.numberPerIndexed[i] = _ticketNumbers[i];
            unchecked {
                ++i;
            }
        }
    ...
    function _checkMatchTicketNumber(
        ...
        uint32 ticketNumber = tickets[_ticketId].numberPerIndexed[numberIndex];
        return (winNumber != 0 && ticketNumber != 0 && winNumber == ticketNumber);
    }
