...
contract NFTMarketMinter is ERC1155, Ownable {
    ...
    modifier creatorOnly(uint256 _id) {
        require(creators[_id] == msg.sender, "NFTMarketMinter: only creator");
        _;
    }
     ...
     * NOTE: remove onlyOwner if you want third parties to create new tokens
     * on your contract (which may change your IDs)
    ...
    function setCreator(address _to, uint256[] memory _ids) public {
        require(_to != address(0), "NFTMarketMinter: to is address 0");
        for (uint256 i = 0; i < _ids.length; i++) {
            uint256 id = _ids[i];
            _setCreator(_to, id);
        }
    ...
    function idUri(uint256 _id) public view returns (string memory concat_) {
        require(_exists(_id), "NFTMarketMinter: token does not exist");
        concat_ = string.concat(baseMetadataURI, Strings.toString(_id));
    }
