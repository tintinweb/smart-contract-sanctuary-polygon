...
contract TheRanchBTCBullsChainLinkVRFRoyaltyRaffle is VRFConsumerBaseV2, Ownable {
    ...
    event winningEvent(
            address[]  _winners,
            uint256[]  _winningIndexes,
            uint indexed _winningAmount
    );

    event couldHaveBeenEvent(
            uint256[]  _winningIndexes,
            uint indexed _winningAmount
    );
    ...
    uint256[] _couldHaveWonIndexes;        // indexes that count have won the raffle but liquidity count failed them.
    uint256[] randomWords;


    function getRoyaltyWinnerS() public ADMIN_OR_DEFENDER {
    ...
    */
    function fulfillRandomWords(
        ...
        require(address(TheRanchBTCBullsCommunityAddress) != address(0), "ERROR: TheRanchBullsMintAndReward Address must be set first.");
   
        // Prep Transfer of USDC to the TRBC_Community Contract
        IERC20 tokenContract = IERC20(usdcTokenContract);
        uint256 _balance = tokenContract.balanceOf(address(this));

        require(_balance >= 100*10**6, "We must at least disperse out $100");

        tokenContract.approve(TheRanchBTCBullsCommunityAddress, _balance);
        
        TheRanchBTCBullsCommunity TRBC = TheRanchBTCBullsCommunity(TheRanchBTCBullsCommunityAddress);

        uint256 _btcBullCount = getNumberOfBTCBulls();
        ...
        for( uint256 i; i < randomWords.length ; i++ ) {
            ...
            uint256 _indexOfPotentialWinner = randomWords[i] % _btcBullCount;
            // get address of this winner from the proxy contract
            address _potentialWinnerAddress = TRBC.ownerOf(_indexOfPotentialWinner);
            // get the liquidation count for this address
            uint256 _liquidationCountOfPotentialWinningAddress  =  TRBC.getliquidationCountForAddress(_potentialWinnerAddress);

            if (_liquidationCountOfPotentialWinningAddress ==  0){
                _winningAddresses.push(_potentialWinnerAddress);
                _winningIndexes.push(_indexOfPotentialWinner);
                _winnersFoundCount += 1;
            } else {
                _couldHaveWonIndexes.push(_indexOfPotentialWinner);
            }

            if (_winnersFoundCount == 3){break;}
        }


        if (_winnersFoundCount > 0) {
            uint256 _winningAmount = _balance / _winnersFoundCount;

            TRBC.updateUsdcBonusFromAnotherContract(_winningAddresses, _winningAmount);
            emit winningEvent(_winningAddresses, _winningIndexes, _winningAmount);
        } else {

            emit couldHaveBeenEvent(_couldHaveWonIndexes, _balance);
        }

        // reset the data for next time
        _winningAddresses = new address[](0);
        _winningIndexes = new uint256[](0);
        _couldHaveWonIndexes = new uint256[](0);
    ...
    // function fakeRaffle() public onlyOwner {

    //     randomWords =  [
    //         12434343452345234,
    //         223452345234523452345,
    //         24524523452452,
    //         32442534534542352345,
    //         23452342345234523452354,
    //         14524324235234523455,
    //         25343576674657466,
    //         7456746524354235234457,
    //         2457534735462248,
    //         2456354673573735739,
    ...
    function fundRoyaltyRaffle(uint256 _amountToDeposit) public payable ADMIN_OR_DEFENDER{
        IERC20 usdcToken = IERC20(usdcTokenContract);
        usdcToken.safeTransferFrom(msg.sender, address(this), (_amountToDeposit));
    }


    function getNumberOfBTCBulls() internal returns (uint) {
        TheRanchBTCBullsCommunity TRBC = TheRanchBTCBullsCommunity(TheRanchBTCBullsCommunityAddress);
        return TRBC.totalSupply();
    }

    /**
     * @dev This needs to be the  TransparentUpgradeableProxy contract. 
    */
    function setTheRanchBTCBullsCommunityAddress(address _address) external onlyOwner {
    ...
    function setUsdcTokenAddress(address _address) public ADMIN_OR_DEFENDER {
        require(address(_address ) != address(0), "ERROR: The USDC contract address can't be address(0)");
        usdcTokenContract = _address;
    }

    function setUsdcTokenDecimals(uint _decimals) public ADMIN_OR_DEFENDER {
