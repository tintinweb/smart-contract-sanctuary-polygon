...
contract BabelToken is ERC20, ERC20Burnable, Ownable {
    event Claim(address indexed from, address indexed token, uint256 amount);

    string private _orderId;
    ...
    function orderId() public view virtual returns (string memory) {
        return _orderId;
    }
...
import "./BabelToken.sol";

import "@openzeppelin/contracts/access/Ownable.sol";

contract BabelTokenFactory is Ownable {
    event BabelTokenDeployed(address indexed tokenAddress);
    ...
    function deploy(
        ...
        BabelToken newToken = new BabelToken{salt: salt}(
            name,
            symbol,
            initialSupply,
            orderId,
        ...
        emit BabelTokenDeployed(address(newToken));
