...
contract FishingFacet is WithStorage {
    ...
    function cast(uint256 fishingPoleId, uint256 baitId) public {
        require(gs().gameItemLookup[fishingPoleId] == GameItemType.FISHING_POLE, "That is not a valid fishing pole id.");
        ...
        require((GameItemsFacet(address(this)).balanceOf(LibMeta.msgSender(), baitId) > 0), "You do not have enough of that bait type.");
    ...
    function reel() public returns (uint256) {
        ...
        (uint256 fishId, FishSpecies species) = SpawnPoolFacet(address(this)).checkForFish(x, y);
        ...
        if (fishId == gs().fishSpawnId) {
            // FIXME: This needs to be VRF based
        ...
        GameItemsFacet(address(this)).transferFish(metadata.id, address(this), msg.sender);
        emit FishCaught(LibMeta.msgSender(), metadata.id, metadata.species);
    ...
    function playerBaitBalances(address player) public view returns (uint256[15] memory) {
        // TODO: This should be dynamic instead of hard coded to current species list
        return [
            ...
            GameItemsFacet(address(this)).balanceOf(player, uint(FishSpecies.Octopus)),
