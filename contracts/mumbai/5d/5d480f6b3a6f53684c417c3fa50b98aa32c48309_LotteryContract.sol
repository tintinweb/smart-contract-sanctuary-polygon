...
contract LotteryContract is VRFConsumerBase, Ownable {
    ...
    function declareWinner(uint256 _lotteryId) public onlyOwner{
        ...
        uint totalParticipants = lottery.participants.length;
        for (uint256 k = 0; k < lottery.noOfWinner; k++) {
            uint256 winnerIndex = uint256(keccak256(abi.encode(r, k))) % totalParticipants;
            ...
            for (uint256 j = 0; j < totalParticipants; j++) {
                if (winnerAddr == lottery.participants[j]) {
                    lottery.participants[j] = lottery.participants[totalParticipants - 1]; 
                    totalParticipants--;
                }
            }
        }
