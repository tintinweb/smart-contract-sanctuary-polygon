...
contract ExtraProtocol is IExtraProtocol, MixinResolver, Ownable {
    ...
    /// @dev Hash of the deposit with position function
    bytes4 public constant DEPOSIT_POSITION =
        bytes4(keccak256(bytes("deposit(uint256,uint256,address)")));

    /// @dev Hash of the withdraw function
    bytes4 public constant WITHDRAW =
        bytes4(keccak256(bytes("withdraw(uint256,address,address)")));

    /// @dev Hash of the withdraw ith position function
    bytes4 public constant WITHDRAW_POSITION =
        bytes4(keccak256(bytes("withdraw(uint256,address)")));
    ...
    function withdraw(uint256 positionId) external {
        ...
        require(
            _deforex == msg.sender ||
                msg.sender == _factory.getAlp(position.token),
            "ExtraProtocol: Caller haven't access"
        );

        require(position.token != address(0));

        if (position.protocol != address(0)) {
            (bool success, ) = address(position.protocol).call(
                abi.encodeWithSelector(
                    WITHDRAW_POSITION,
                    positionId,
                    msg.sender
    ...
    function withdraw(
        ...
        require(
            _deforex == msg.sender || msg.sender == _factory.getAlp(token),
            "ExtraProtocol: Caller haven't access"
        );
    ...
    function balanceOf(uint256 positionId) external view returns (uint256) {
        ...
        return
            position.protocol == address(0)
                ? position.amount
                : IProtocol(position.protocol).balanceOf(positionId);
    }
