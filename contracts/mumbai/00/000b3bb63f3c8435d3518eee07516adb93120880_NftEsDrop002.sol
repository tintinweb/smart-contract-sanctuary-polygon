...
contract NftEsDrop002 is ERC721, Ownable, ERC721URIStorage {
using Address for address;
    //NFT category
    // NFT Description & URL
    ...
    function setDiscountPercentage(uint8 DiscountPercentage) external onlyOwner {
        require(DiscountPercentage!=_DiscountPercentage, "Discount Percentage Already Set");
    ...
    function setStatusMintFeeMaxMintsMaxNFTsAndTimer(bool mintStatus, uint8 maxMints, uint16 maxNFTs, uint32 startTime, uint32 endTime) external onlyOwner {
        //_mintFees = mintFee;
    ...
    function getStatusMintFeeMaxMintsAndTimer()
        external
        view
        onlyOwner
        returns (bool,uint256,uint256,uint32,uint32)
    {
        return (isPaused, _mintFees, _maxMints,_mintStartTime, _mintEndTime);
    }
        ...
        returns ( uint256[] memory)
    {
        ...
        string[] memory categoryNamesArr = new string[](_categoriesSet);
        for (uint16 i = 1; i <= (_categoriesSet)-1; i++) {
...
function RandomMint(address user_addr, uint8 noOfMints, uint8 categoryIDtoMint)
        ...
        returns (uint256[] memory, string[] memory)
    {
        ...
        for (uint i = 0; i <= noOfMints - 1; i++) {
            ...
            uint randIndex = categoryID % (numLeft-1);
            tokenId = (categoryIDtoMint*1000) + notMintedArray[categoryIDtoMint][randIndex];
            notMintedArray[randIndex] = notMintedArray[numLeft -1];
