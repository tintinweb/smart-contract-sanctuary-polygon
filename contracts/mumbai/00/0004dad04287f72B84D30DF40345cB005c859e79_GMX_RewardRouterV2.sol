...
contract GMX_RewardRouterV2 is GMX_ReentrancyGuard, GMX_Governable {
...
import "./GMX_IERC20.sol";
import "../math/GMX_SafeMath.sol";
import "../utils/GMX_Address.sol";
 ...
 */
library GMX_SafeERC20 {
    using GMX_SafeMath for uint256;
    using GMX_Address for address;

    function safeTransfer(GMX_IERC20 token, address to, uint256 value) internal {
        _callOptionalReturn(token, abi.encodeWithSelector(token.transfer.selector, to, value));
    }

    function safeTransferFrom(GMX_IERC20 token, address from, address to, uint256 value) internal {
     ...
     */
    function safeApprove(GMX_IERC20 token, address spender, uint256 value) internal {
    ...
    function safeIncreaseAllowance(GMX_IERC20 token, address spender, uint256 value) internal {
        uint256 newAllowance = token.allowance(address(this), spender).add(value);
        _callOptionalReturn(token, abi.encodeWithSelector(token.approve.selector, spender, newAllowance));
    }

    function safeDecreaseAllowance(GMX_IERC20 token, address spender, uint256 value) internal {
     ...
     */
    function _callOptionalReturn(GMX_IERC20 token, bytes memory data) private {
