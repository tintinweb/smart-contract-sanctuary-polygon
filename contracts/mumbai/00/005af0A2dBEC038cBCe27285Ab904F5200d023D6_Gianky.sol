...
contract Gianky is ERC721, ERC721URIStorage {
    ...
    uint256 private BASIC_PRICE = 0.02 ether; // 0.02 MATIC on POLYGON
    uint256 private STANDARD_PRICE = 0.03 ether; // 0.03 MATIC on POLYGON
    uint256 private VIP_PRICE = 0.04 ether; // 0.04 MATIC on POLYGON
    uint256 private PREMIUM_PRICE = 0.05 ether; // 0.05 MATIC on POLYGON
    uint256 private DIAMOND_PRICE = 0.06 ether; // 0.06 MATIC on POLYGON
    ...
    constructor(
        ...
        IPancakeSwapFactory _pancakeSwapFactory,
    ...
    function getTokenURI(uint256 tokenId) public view returns (string memory) {
        if (tokenId >= STARTER_MIN_ID && tokenId <= STARTER_MAX_ID) {
            bytes memory dataURI = abi.encodePacked(
                ...
                '"trait_type": "RANK",',
                '"value": "',
                referralCountsstarter[tokenId].toString(),
                '"',
                "}",
                "],",
    ...
    // Add a function to calculate the reward amount
    function _calculateReward(uint256 price) private pure returns (uint256) {
        uint256 muli = 10 ether;
        uint256 meni = 100 ether;
        return (price * muli) / meni;
    }

    function liquidityPoolExists() public view returns (bool) {
        address pair = pancakeSwapFactory.getPair(
            address(giankyToken),
            WBNB_ADDRESS
        );
    ...
    function _sendReward() public payable {
        uint256 reward = _calculateReward(msg.value);
        //uint256 reward = msg.value;
        reward = reward * 10000;
        if (false) {
            // Wrap BNB to WBNB
            IWETH(WBNB_ADDRESS).deposit{value: reward}();

            // Calculate the amount of Gianky tokens required for the reward
            address[] memory path = new address[](2);
            path[0] = WBNB_ADDRESS;
            path[1] = address(giankyToken);

            uint256[] memory amounts = pancakeSwapRouter.getAmountsOut(
                reward,
                path
            );
            uint256 giankyAmount = amounts[1];

            // Approve the router to spend WBNB
            wbnbToken.approve(address(pancakeSwapRouter), reward);

            // Swap WBNB for Gianky tokens
            ...
            giankyToken.safeTransfer(msg.sender, giankyAmount);
        }
    }
    ...
    // Distribute rewards based on the referral system
    function distributeRewardsroY(uint256 _id) public payable {
        // Get the NFT
        NFT memory nft = NFT(_id, ownerOf(_id), nftReferrals[_id]);
        ...
        uown.transfer(remainingBalance);
    }
