...
contract MusicCollaborationContract {
    ...
    constructor(
        ...
        require(
            _writerNames.length == _writerSplits.length && _writerNames.length == _writerEmails.length,
        ...
        for (uint256 i = 0; i < _writerNames.length; i++) {
            _compositionDetails.writers.push(
                Writer(_writerNames[i], _writerEmails[i], _writerSplits[i])
            );
        }
