...
import {ICommunityPortal} from "./CommunityPortal/ICommunityPortal.sol";
import {ICommunityPassport} from "./CommunityPassport/ICommunityPassport.sol";

contract PrizePoap is IPrizePoap, Ownable, ERC1155Supply, ReentrancyGuard {
    ICommunityPortal public communityPortal;
    string public baseURI;
    IPrizePoap.Prize[] private _prizeList;

    constructor(address _communityPortal, string memory _baseURI) ERC1155("") {
    ...
    function uri(uint256 tokenId) public view override returns (string memory) {
        ...
        return
            bytes(baseURI).length > 0
                ? string.concat(baseURI, _prizeList[tokenId].tokenURI)
                : "";
    }
    ...
    function setBaseURI(string memory _externalURL) external onlyOwner {
        baseURI = _externalURL;
    }
    ...
    function setTokenURI(
        ...
        _prizeList[tokenId].tokenURI = _tokenURI;
    }
    ...
    function setkBatchTokenURI(
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            _prizeList[tokenIds[i]].tokenURI = tokenURIs[i];
        }
    }

    function setRequiredExp(
        uint256 tokenId,
        uint32 _requiredExp
    ) external onlyOwner {
        _prizeList[tokenId].requiredExp = _requiredExp;
    }
    ...
    function setCommunityId(
        ...
        _prizeList[tokenId].communityId = _communityId;
    }

    function setClosed(uint256 tokenId, bool _closed) external onlyOwner {
        _prizeList[tokenId].closed = _closed;
    }
    ...
    function create(
        ...
        require(
            owner() == msg.sender || address(communityPortal) == msg.sender,
            "You cannot operate."
        );
        require(_requiredExp > 0, "Not enough exp");
        bool isClose;
        (, , , isClose) = communityPortal.getCommunity(_communityId);
        require(!isClose, "This community is not opend");
    ...
    function mint(uint256 tokenId) external nonReentrant {
        if (!checkObtainable(msg.sender, tokenId)) revert LackOfExp();
        if (!checkObtained(msg.sender, tokenId)) revert AlreadyObtained();
    ...
    function checkObtainable(
        ...
        uint32 communityId = _prizeList[tokenId].communityId;
        ...
        ICommunityPassport.Passport memory passport = passportContract
            .getPassport(target);
        return passport.exp < _prizeList[tokenId].requiredExp;
    }
