...
pragma solidity ^0.8.0;
 ...
 contract GamePlayToken is ERC20{
     ...
     function BuyStrategicSale() public payable{
         price = 23*10**15;
         uint tokenAmount = (msg.value*10**18/price);
         ...
         unlockedBalance[msg.sender] += (strategicSaleBalance[msg.sender]*4)/100;
     }
     function BuyPublicSale() public payable{
         ...
         unlockedBalance[msg.sender] += (publicSaleBalance[msg.sender]*5)/100;
     } 
     ...
     function WithdrawTokens() public{
         ...
         if(!publicFirstSell && publicSaleBalance[msg.sender]>0){
             publicSaleBalance[msg.sender] -= (publicSaleBalance[msg.sender]*5)/100;
             publicFirstSell = true;
             publicSaleRemainingAmount[msg.sender] = publicSaleBalance[msg.sender]/12;
         }
         if(strategicSaleBalance[msg.sender]>0 && !publicFirstSell){
            strategicSaleBalance[msg.sender]-=(strategicSaleBalance[msg.sender]*4)/100;
         }
         uint oldTokenBalance = tokenBalance[msg.sender];
         tokenBalance[msg.sender] = unlockedBalance[msg.sender]+oldTokenBalance;
         this.transfer(msg.sender,tokenBalance[msg.sender]);
         unlockedBalance[msg.sender] = 0;
     }
     function UnlockWallets() public OnlyOwner{
         for(uint i=0; i<strategicAdresses.length; i++){
             uint unlockedAmount = (strategicSaleBalance[strategicAdresses[i]]*4)/100;
             unlockedBalance[strategicAdresses[i]] += unlockedAmount;
             strategicSaleBalance[strategicAdresses[i]] -= unlockedAmount;
         }
         for(uint i=0; i<publicAddresses.length; i++){
            unlockedBalance[publicAddresses[i]] += publicSaleRemainingAmount[publicAddresses[i]];
            publicSaleBalance[publicAddresses[i]] -= publicSaleRemainingAmount[publicAddresses[i]];
         }
     }
