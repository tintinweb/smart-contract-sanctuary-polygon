...
contract Trade {
    ...
    function verifyOwnerSign(Sign memory sign, address caller, Sign memory ownerSign) internal view {
        bytes32 hash = keccak256(abi.encodePacked(sign.v, sign.r, sign.s, sign.nonce, caller, ownerSign.nonce));
        require(signer == getSigner(hash, ownerSign), "buyer sign verification failed");
    }
