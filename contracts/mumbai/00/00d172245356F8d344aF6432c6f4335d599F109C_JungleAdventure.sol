...
contract JungleAdventure is VRFConsumerBaseV2 {
	...
	event ManyAdventuresStarted(address indexed owner, uint256 indexed teamId, uint256 requestId);
	event ManyAdventuresCompleted(address indexed owner, uint256 indexed teamId, uint256 requestId);
	...
	function queryManyAdventuresVRF(uint256 _teamId, bool _energy, uint256 _amount) external returns(uint256) {
		...
		multiRunData[requestId] = MultiRunData({
			fulfilled: false,
			user: msg.sender,
			amount: _amount,
			seed: 0,
			teamId: _teamId,
			expGranted: _energy ? 0 : _amount
		});
		emit ManyAdventuresStarted(msg.sender, team, requestId);
		return requestId;
	}
	...
	function settleAdventureRuns(uint256 _requestId) external {
		...
		for (uint256 j = 0; j < amount; j++) {
			...
			emit AdventureDone(
				...
				returnData.gameData |
				(expFlag << 64) |
				(data.expGranted > 0 ? 1 << 69 : 0) |
				(_encodeTeam(getLevelsInTeam(team)) << 70) |
				(_getActiveCharm(msg.sender, data.teamId) << 95),
				block.timestamp);
		}
		_mintKongium(user, kongiumToMint);
		emit ManyAdventuresCompleted(user, team, _requestId);
	}
...
contract JungleLogic is IJungleLogic, Ownable {
	...
	function charmEnableEvent(uint256 _eventId) public onlyOwner {
		require(_eventId < charmEventCounter, "Outside of range");
		GameEvent storage _event = charmGameEvents[_eventId];
	...
	function rodEnableEvent(uint256 _eventId) public onlyOwner {
		require(_eventId < rodEventCounter, "Outside of range");
		GameEvent storage _event = rodGameEvents[_eventId];
