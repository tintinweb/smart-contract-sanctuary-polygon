...
contract Hivemind is HivemindStorage, HivemindI, AccessControlEnumerable, ReentrancyGuard {
  ...
  function getCollateralPriceInUnderlying(HTokenI _hToken) internal view returns (uint256) {
    PriceOracleI cachedOracle = _oracle;
 ...
 - Loan to value shortfall
    ...
    )
  {
    ...
    uint256 ltvValue = (markets[_hToken].maxLTVFactorMantissa * sumCollateral) / 1e18;
  ...
  function oracle() public view override returns (PriceOracleI) {
    return _oracle;
  }
  ...
  function _setPriceOracle(PriceOracleI _newOracle) external {
    ...
    emit PriceOracleUpdated(_oracle, _newOracle);
