...
contract Dao {
    ...
    constructor() {
        owner = msg.sender;
        nextProposal = 1;
        daoContract = IdaoContract(0x4B8a57eAf2Ce9Bd7a61cDcb81A6f70A3Df80A4Ef);
        validTokens = [1, 2, 3, 4];
    }
    ...
    function checkProposalEligibility(address _proposalist)
        ...
        returns (bool)
    {
        uint256 NFTCount = daoContract.getTotalNFTsCount();
        for (uint256 i = 0; i <= NFTCount; i++) {
            if (daoContract.ownerOf(validTokens[i]) == _proposalist) {
                return true;
            }
        }
        return false;
    ...
    function createProposal(string memory _description, address[] memory _canVote) public {
        ...
        newProposal.deadline = block.number + 1;
