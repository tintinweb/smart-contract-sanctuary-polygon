...
interface IMissionControl {
    ...
    struct HandleRaidCollectVars {
        ...
        TileRentalInfo _tileRentalInfo;
    ...
    function createRentTiles(
        address supertoken,
        address renter,
        PlaceOrder[] memory tiles, // changed to PlaceOrder so that it inclues tokenId and token Address
        int96 flowRate
    ) external;
...
contract MissionControl is IMissionControl, OwnableUpgradeable, EIP712Upgradeable {
    ...
    mapping(address => mapping(int256 => mapping(int256 => TileElement[2]))) tiles;
    ...
    mapping(address => uint256) private userAllTerminatedAt;
    ...
    mapping(address => mapping(int256 => mapping(int256 => TileRentalInfo))) tileRentalInfo;

    int96 private constant RENT_TILE_FLOW_RATE = 385802469135;
    ...
    modifier validCoordinates(
        ...
        require(rad <= allowedRadius && rad > 0 && sum == 0, "MISSION_CONTROL: Invalid coordinates");
        _;
    }

    /**
     * @notice Modifier to hinder calls with invalid coordinates, except for out of bounds
     * @param _x X-coordinate of tile
     * @param _y Y-coordinate of tile
     * @param _z Z-coordinate of tile
     */
    modifier validCoordinates_noMaxRad(
        int256 _x,
        int256 _y,
        int256 _z
    ) {
        int256 sum = _x + _y + _z;
        uint256 rad = (abs(_x) + abs(_y) + abs(_z)) / 2;
        require(rad > 0 && sum == 0, "MISSION_CONTROL: Invalid coordinates");
        _;
    }
    ...
    modifier onWhitelist(address _tokenAddress) {
        require(whiteList[_tokenAddress] || whiteList[tokenAliases[_tokenAddress]], "MISSION_CONTROL: Not whitelisted");
        _;
    }
    ...
    modifier onlyStreamer() {
        require(msg.sender == streamer, "MISSION_CONTROL: SENDER NOT STREAMER CONTRACT");
        _;
    }
    ...
    function placeNFT(
        ...
        uint256 rad = (abs(order.x) + abs(order.y) + abs(order.z)) / 2;
        if (rad > 1) {
            TileRentalInfo storage tile = tileRentalInfo[msg.sender][order.x][order.y];
            require(tile.isRented == true, "MISSION_CONTROL: TILE NOT RENTED");
            require(tile.pausedAt == 0, "MISSION_CONTROL: TILE PAUSED");
        }
        __placeNFT(order, _tokenId, _tokenAddress, msg.sender);
    }
    ...
    /// @inheritdoc IMissionControl
    function removeNFT(CollectOrder memory order) external override {
        __removeNFT(order, msg.sender);
    }

    /// @inheritdoc IMissionControl
    function removeNFTs(PlaceOrder[] memory _placeOrders) external override {
        for (uint256 i; i < _placeOrders.length; i++) {
            __removeNFT(_placeOrders[i].order, msg.sender);
        }
    }
    ...
    function createRentTiles(
        address _supertoken,
        address _renter,
        PlaceOrder[] memory _placeOrders,
        int96 _flowRate
    ) external onlyStreamer {
        require(_supertoken == aGold || _supertoken == aGoldLite, "MISSION_CONTROL: SUPERTOKEN NOT RECOGNIZED");
        int96 tilesRented;

        tilesRented += _handleTileRentals(_supertoken, _renter, _placeOrders);

        require(tilesRented * RENT_TILE_FLOW_RATE == _flowRate, "MISSION_CONTROL: INVALID FLOW");
    }
    ...
    function updateRentTiles(
        ...
        require(_supertoken == aGold || _supertoken == aGoldLite, "MISSION_CONTROL: SUPERTOKEN NOT RECOGNIZED");
        UpdateRentTileVars memory v;

        v.tilesRented += _handleTileRentals(_supertoken, _renter, _placeOrders);

        v.removeTileLength = _removeTiles.length;
        for (uint256 i; i < v.removeTileLength; i++) {
            v.removeTileItem = _removeTiles[i];
            TileRentalInfo storage tile = tileRentalInfo[_renter][v.removeTileItem.x][v.removeTileItem.y];

            require(tile.isRented == true, "MISSION_CONTROL: TILE IS NOT RENTED");
            ...
            v.index = userRentedTilesIndices[_renter][_supertoken][v.removeTileItem.x][v.removeTileItem.y];
            v.size = userRentedTiles[_renter][_supertoken].length;

            userRentedTiles[_renter][_supertoken][v.index] = userRentedTiles[_renter][_supertoken][v.size - 1];
            v.order = userRentedTiles[_renter][_supertoken][v.index];
            userRentedTilesIndices[_renter][_supertoken][v.order.x][v.order.y] = v.index;
            userRentedTiles[_renter][_supertoken].pop();
        }

        require(_flowRate == _oldFlowRate + v.tilesRented * RENT_TILE_FLOW_RATE, "MISSION_CONTROL: INVALID FLOW");
    }

    // user stop streaming to the game
    function deleteRentTiles(address _supertoken, address _renter) external onlyStreamer {
        ...
        for (i; i < length; i++) {
            ...
            TileRentalInfo storage tile = tileRentalInfo[_renter][rental.x][rental.y];
    ...
    function _handleTileRentals(
        ...
        address _renter,
        PlaceOrder[] memory _placeOrders
    ) internal returns (int96 _tilesRented) {
        uint256 i;
        uint256 length = _placeOrders.length;

        for (; i < length; i++) {
            PlaceOrder memory placeOrderItem = _placeOrders[i];
            TileRentalInfo storage _tileRentalInfo = tileRentalInfo[_renter][placeOrderItem.order.x][
                placeOrderItem.order.y
            ];
            ...
            if (_tileRentalInfo.pausedAt != 0) {
                ...
                userRentedTiles[_renter][_supertoken].push(placeOrderItem.order);
                userRentedTilesIndices[_renter][_supertoken][placeOrderItem.order.x][placeOrderItem.order.y] =
                    userRentedTiles[_renter][_supertoken].length -
                    1;

                TileElement[2] memory tileElements = tiles[_renter][placeOrderItem.order.x][placeOrderItem.order.y];
                TileElement memory tile = tileElements[1].nonce != 0 ? tileElements[1] : tileElements[0];
                if (tile.nonce != 0)
                    IMissionControlStakeable(placeOrderItem.tokenAddress).onResume(_renter, tile.nonce);

                ++_tilesRented;
            } else {
                require(!_tileRentalInfo.isRented, "MISSION_CONTROL: TILE ALREADY RENTED");
                _tileRentalInfo.isRented = true;
                _tileRentalInfo.rentalToken = _supertoken;
    ...
    function checkTileRentalInfo(
        address _user,
        int256 _x,
        int256 _y
    ) external view returns (TileRentalInfo memory _tileRentalInfo) {
        _tileRentalInfo = tileRentalInfo[_user][_x][_y];
    }

    /// @inheritdoc IMissionControl
    function checkTile(CheckTileInputs memory i)
        public
        view
        override
        validCoordinates(i.x, i.y, i.z)
        returns (uint256, uint256)
    {
        TileElement memory tile = tiles[i.user][i.x][i.y][__zeroOrOne(i.x, i.y, i.user)];
        return
            IMissionControlStakeable(tile.stakeable).checkTile(
                i.user,
                tile.nonce,
                tileRentalInfo[i.user][i.x][i.y].pausedAt,
                i.x,
                i.y
            );
    }
    ...
    function collectFromTiles(
        ...
        uint256 arrayLen = _orders.length;
        uint256[] memory ids = new uint256[](arrayLen);
        uint256[] memory amounts = new uint256[](arrayLen);
        uint256 cId;
        uint256 cAmount;
        // I hate this loop
        for (uint256 i; i < _orders.length; i++) {
            (cAmount, cId) = _handle_collect(HandleCollectInputs(_orders[i].x, _orders[i].y, _orders[i].z));
            ids[i] = cId;
            amounts[i] = cAmount;
        }
    ...
    function notifyRaided(
        ...
        address signer = ECDSA.recover(
            _hashTypedDataV4(keccak256(abi.encode(RAID_MESSAGE, _defender, _timestamp, _raidId))),
            _v,
            _r,
            _s
        );
        require(signers[signer], "MISSION_CONTROL: INVALID_SIGNATURE");
        require(int256(block.timestamp) - int256(_timestamp) < raidTimeout, "MISSION_CONTROL: Signature has timed out");
        require(!raidSpent[_raidId], "MISSION_CONTROL: Raid already registered");
        uint256 totalAmount;
        uint256 cAmount;
        for (uint256 i = 0; i < _orders.length; i++) {
            (cAmount, ) = _handle_raid_collect(HandleRaidCollectInputs(_defender, _orders[i].x, _orders[i].y));
            totalAmount += cAmount;
        }
        raidSpent[_raidId] = true;
        emit Raid(_defender, _raidId, totalAmount, _timestamp);
    }
    ...
    function checkStakedOnTile(
        ...
        int256 _y,
        int256 _z
    ) public view override validCoordinates(_x, _y, _z) returns (TileElement memory _base, TileElement memory _top) {
        _top = tiles[_user][_x][_y][1];
    ...
    function timeLeft(
        ...
        int256 _y,
        int256 _z
    ) public view override validCoordinates(_x, _y, _z) returns (int256 _timeLeft) {
        TileElement memory tile = tiles[_user][_x][_y][__zeroOrOne(_x, _y, _user)];
        _timeLeft = IMissionControlStakeable(tile.stakeable).timeLeft(_user, tile.nonce, _x, _y);
    }
    ...
    function _handleBasePlacement(
        ...
        require(tiles[_staker][_x][_y][0].nonce == 0, "MISSION_CONTROL: Already a base on tile");
        IMissionControlStakeable(_stakeable).onStaked(_staker, _tokenId, _nonce, address(0), 0, _x, _y);
    ...
    function _handleTopPlacement(
        ...
        require(
            tiles[_staker][_x][_y][0].nonce != 0 && tiles[_staker][_x][_y][1].nonce == 0,
            "MISSION_CONTROL: Invalid placement"
        );
        IMissionControlStakeable(_stakeable).onStaked(
            _staker,
            _tokenId,
            _nonce,
            tiles[_staker][_x][_y][0].stakeable,
            tiles[_staker][_x][_y][0].tokenId,
            _x,
            _y
        );
        TileElement storage tileElement = tiles[_staker][_x][_y][1];
        tileElement.nonce = _nonce;
        tileElement.tokenId = _tokenId;
        tileElement.stakeable = _stakeable;
    }
    ...
    function __placeNFT(
        ...
        if (IMissionControlStakeable(tokenAddress).isBase(_tokenId, order.x, order.y)) {
            ...
            _handleTopPlacement(order.x, order.y, _tokenId, tokenAddress, ++stakeNonces[_staker], _staker);
        }
     ...
     */
    function __removeNFT(CollectOrder memory order, address _renter)
        private
        validCoordinates_noMaxRad(order.x, order.y, order.z)
    {
        TileElement[2] storage tileElements = tiles[_renter][order.x][order.y];
        uint256 zeroOrOne = __zeroOrOne(order.x, order.y, _renter);
        _handleReturn(tileElements[zeroOrOne], _renter);
        if (zeroOrOne == 1) {
            IMissionControlStakeable(tileElements[0].stakeable).reset(_renter, tileElements[0].nonce);
        }
        emit NFTRemoved(
            _renter,
            tileElements[zeroOrOne].stakeable,
            zeroOrOne == 1 ? tileElements[0].stakeable : address(0),
            tileElements[zeroOrOne].tokenId,
            zeroOrOne == 1 ? tileElements[0].tokenId : 0,
            order.x,
            order.y
        );
    }
    ...
    /**
     * @notice Used to check whether there is a base, top or no token staked on a tile
     * @param _x x-coordinate
     * @param _y y-coordinate
     * @return zeroOrOne 1 if there is a top, 0 if there is a base, reverts if there is none
     */
    function __zeroOrOne(
        int256 _x,
        int256 _y,
        address _sender
    ) internal view returns (uint256 zeroOrOne) {
        if (tiles[_sender][_x][_y][1].nonce != 0) {
            zeroOrOne = 1;
        } else if (tiles[_sender][_x][_y][0].nonce != 0) {
            zeroOrOne = 0;
        } else {
            revert("MISSION_CONTROL: No token staked on coordinates");
        }
    }

    //TODO: Add an new property to tile elements, "pausedat" to keep track of when a tile was paused. when collecting from a paused tile, just
    // set seed to time paused at so that it stops accruing rewards. can possibly create a new function for unpausing or use the update hook to unpause.
    // when unpausing, set seed to the block timestamp and set "pausedat" to 0.
    function _handle_collect(HandleCollectInputs memory i)
        private
        validCoordinates(i.x, i.y, i.z)
        returns (uint256, uint256)
    {
        HandleCollectVars memory v;

        v.tileElement = tiles[msg.sender][i.x][i.y][__zeroOrOne(i.x, i.y, msg.sender)];
        v._tileRentalInfo = tileRentalInfo[msg.sender][i.x][i.y];
        return
            IMissionControlStakeable(v.tileElement.stakeable).onCollect(
                msg.sender,
                v.tileElement.nonce,
                v._tileRentalInfo.pausedAt,
                i.x,
                i.y
            );
    }

    function _handle_raid_collect(HandleRaidCollectInputs memory i)
        private
        validCoordinates(i.x, i.y, -(i.x + i.y))
        returns (uint256, uint256)
    {
        HandleRaidCollectVars memory v;

        v.tileElement = tiles[i.defender][i.x][i.y][__zeroOrOne(i.x, i.y, i.defender)];
        v._tileRentalInfo = tileRentalInfo[msg.sender][i.x][i.y];
        v.iStakeable = IMissionControlStakeable(v.tileElement.stakeable);

        if (!v.iStakeable.isRaidable(v.tileElement.tokenId, i.x, i.y)) {
            return (0, WASTE_ID);
        }

        return v.iStakeable.onCollect(msg.sender, v.tileElement.nonce, v._tileRentalInfo.pausedAt, i.x, i.y);
    }
    ...
    /**
     * @notice Sets an alias for a stakeable
     * @param _token Token address
     * @param _alias An alias for it
     */
    function setStakeableAlias(address _token, address _alias) external onlyOwner {
        tokenAliases[_alias] = _token;
    }
    ...
    function setRadius(uint256 _radius) external onlyOwner {
        require(_radius > 0, "Radius can't be zero");
        allowedRadius = _radius;
        emit RadiusSet(_radius);
    }

    function setRaidParams(int256 _timeout) external onlyOwner {
        require(_timeout > 0, "MISSION_CONTROL: Raid timeout must be >0");
        raidTimeout = _timeout;
    }
    ...
    function setStreamer(address _streamer) external onlyOwner {
        require(_streamer != address(0), "MISSION_CONTROL: ADDRESS ZERO");
        streamer = _streamer;
    }

    function setAGold(address _aGold) external onlyOwner {
        require(_aGold != address(0), "MISSION_CONTROL: ADDRESS ZERO");
        aGold = _aGold;
    }

    function setAGoldLite(address _aGoldLite) external onlyOwner {
        require(_aGoldLite != address(0), "MISSION_CONTROL: ADDRESS ZERO");
    ...
    function checkLastUpdated(
        ...
        uint256 zeroOrOne = __zeroOrOne(_x, _y, _user);
        _timestamp = IMissionControlStakeable(tiles[_user][_x][_y][zeroOrOne].stakeable).checkTimestamp(
            _user,
            tiles[_user][_x][_y][zeroOrOne].nonce
        );
    }
            ...
            for (int256 y = -radius; y <= radius; y++) {
                for (int256 z = -radius; z <= radius; z++) {
                    ...
                    if (rad <= allowedRadius && rad > 0 && sum == 0) {
                        (_base, _top) = checkStakedOnTile(_renter, x, y, z);
                        if (_base.nonce != 0) _timeLeft = timeLeft(_renter, x, y, z);
                        _totalTileInfo[_count].order = CollectOrder(x, y, z);
                        _totalTileInfo[_count].timeLeft = _timeLeft;
                        _totalTileInfo[_count].base = _base;
                        _totalTileInfo[_count].top = _top;
        ...
        for (int256 x = -v.radius; x <= v.radius; x++) {
            for (int256 y = -v.radius; y <= v.radius; y++) {
                for (int256 z = -v.radius; z <= v.radius; z++) {
                    ...
                    uint256 rad = (abs(x) + abs(y) + abs(z)) / 2;
                    if (rad <= allowedRadius && rad > 0 && sum == 0) {
                        TileElement[2] memory tileElements = tiles[_renter][x][y];
                        TileElement memory tile = tileElements[1].nonce != 0 ? tileElements[1] : tileElements[0];
                        _totalCheckTileInfo[v.count].order = CollectOrder(x, y, z);
                        if (tile.nonce != 0) {
                            (v.amount, v.tokenId) = checkTile(CheckTileInputs(_renter, x, y, z));
                            _totalCheckTileInfo[v.count].amount = v.amount;
                            _totalCheckTileInfo[v.count].tokenId = v.tokenId;
                        }
                        v.count++;
                    }
                }
