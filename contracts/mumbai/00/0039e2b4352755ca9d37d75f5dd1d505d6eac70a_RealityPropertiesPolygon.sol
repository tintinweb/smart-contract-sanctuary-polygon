...
* @dev The token amounts are internaly represented as it's full value, integer and fractional part. 
*/
contract RealityProperties is IERC2981, ERC1155SupplyUpgradeable, AccessControlUpgradeable, IRealityProperties {
    using AddressUpgradeable for address;
    ...
    function initialize(string memory uri_, string memory contractUri_, address royaltiesReceiver_) public virtual initializer {
     ...
     */
    function safeMint(uint256 tokenId, uint256 amount) public virtual onlyRole(MINTING_MANAGER) {
        require(!exists(tokenId), "RealityProperties: Already exists");
     ...
     */
    function setDefaultRoyalty(address receiver, uint96 feeNumerator) public virtual royaltyWithinLimit(feeNumerator) onlyRole(ROYALTIES_MANAGER) {
    ...
    function _createAdapter(uint256 tokenId) internal returns (address){
        address expectedAddress = computeAddress(tokenId, erc20AdapterImplementation);
        require(_adapterMapper[expectedAddress] == 0, "Adapter already deployed");

        address adapterAddress = deploy(tokenId, erc20AdapterImplementation);
        require(expectedAddress == adapterAddress, "Invalid contract address");
     ...
     */
    function computeAddress(uint256 tokenId, address implementation) internal view returns (address) {
        return
     ...
     * @dev The address is computed using create2 method based on token id and contract address
     */
    function deploy(uint256 tokenId, address implementation) internal returns(address) {
     ...
     */
    function getContractCreationCode(address implementation) internal pure returns (bytes memory) {
     ...
     */
    function getAdapterAddress(uint256 tokenId_) public view returns (address){
        return computeAddress(tokenId_, erc20AdapterImplementation);
    }
     ...
     * @dev See {IERC165-supportsInterface}.
     */
    function supportsInterface(bytes4 interfaceId) public view virtual override(IERC165, AccessControlUpgradeable, ERC1155Upgradeable, IERC165Upgradeable) returns (bool) {
...
// File contracts/RealityPropertiesPolygon.sol
...
*/
contract RealityPropertiesPolygon is RealityProperties {
    /**
     * @dev Polygon management contract
     */
    ...
    function initialize(string memory uri_, string memory contractUri_, address childChainManagerProxy_, address royaltiesReceiver_) public initializer {
        __RealityPropertiesPolygon_init(uri_, contractUri_, childChainManagerProxy_, royaltiesReceiver_);
    }

    // solhint-disable-next-line func-name-mixedcase
    function __RealityPropertiesPolygon_init(string memory uri_, string memory contractUri_, address childChainManagerProxy_, address royaltiesReceiver_) internal onlyInitializing {
        __RealityProperties_init(uri_, contractUri_, royaltiesReceiver_);
        __RealityPropertiesPolygon_init_unchained(childChainManagerProxy_);
    }

    // solhint-disable-next-line func-name-mixedcase
    function __RealityPropertiesPolygon_init_unchained(address childChainManagerProxy_) internal onlyInitializing {
    ...
    /**
     * @dev Minting is disabled on the child chain
     */
    function safeMint(uint256, uint256) public pure override {
        revert("RealityPropertiesPolygon: Minting not allowed");
    }
    ...
    function deposit(address user, bytes calldata depositData)
        external
    {
        require(msg.sender == childChainManagerProxy, "RealityPropertiesPolygon: Unauthorized Call");
        ...
        require(
            user != address(0),
            "RealityPropertiesPolygon: INVALID_DEPOSIT_USER"
        );
    ...
    /// @dev being proxified smart contract, most probably childChainManagerProxy contract's address
    /// is not going to change ever, but still, lets keep it 
    function updateChildChainManager(address newChildChainManagerProxy) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(newChildChainManagerProxy != address(0), "RealityPropertiesPolygon: Bad ChildChainManagerProxy address");
