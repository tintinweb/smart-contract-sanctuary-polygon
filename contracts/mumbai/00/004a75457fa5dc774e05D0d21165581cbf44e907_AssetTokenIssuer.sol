...
contract AssetTokenIssuer is AccessControlUpgradeable {
    ...
    function unauthorizeAsset(address _assetAddress) public requireAdmin requireNonEmptyAddress(_assetAddress) {
    ...
    function setCustodyAddress(address _custodyAddress) public requireAdmin requireNonEmptyAddress(_custodyAddress) {
        custodyAddress = _custodyAddress;
    }

    function setFeeBPS(uint256 _feeBPS) public requireAdmin {
        feeBPS = _feeBPS;
    }

    function calculateAmountToMint(address asset, uint256 amount) public view returns (uint256) {
        ...
        uint256 amountToMint = assetValue.mul(10**DECIMALS).div(uint256(assetTokenPrice));
    ...
    function mint(
    ...
    ) public requireNonEmptyAddress(asset) {
        ...
        uint256 allowance = IERC20(asset).allowance(_msgSender(), address(this));
        ...
        uint256 amountToMint = calculateAmountToMint(asset, amount);
        ...
        IAssetToken(assetTokenAddress).requestMint(amountToMint, _msgSender());
