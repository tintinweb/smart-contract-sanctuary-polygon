...
contract BetDex is Initializable, AccessControl, ReentrancyGuardUpgradeable, EIP712, Pausable, BetDexEvent {
	...
	uint8 public constant SIGNATURE_TIMESTAMP_THRESHOLD = 30;
...
//	mapping(address => uint256) public addressMinNonce;
	...
	mapping(address => mapping(address => uint256)) totalPendingWithdrawal; //contractAddress, userAddress, amount
	mapping(address => bool) withdrawalAddressAdded;
    ...
    modifier onlyOwners(bytes32 roomId) {
        require(
			rooms[roomId].owner == msg.sender || hasRole(ROLE_MANAGER, msg.sender),
            "PERMISSION_DENIED"
        );
        _;
    }
	...
	function approveWithdrawal(uint256 id) external whenNotPaused onlyRole(ROLE_AUDITOR) nonReentrant {
	...
	function batchApproveWithdrawal(uint256[] calldata ids) external whenNotPaused onlyRole(ROLE_AUDITOR) nonReentrant {
		...
		address[] memory roomAddress = new address[](idLength);
		uint roomAddressIndex;
		address[] memory userAddress = new address[](idLength);
		uint userAddressIndex;
		for (uint i; i < idLength; i = _uncheckedInc(i)) {
			...
			address roomContractAddress = rooms[pendingWithdrawal.roomId].contractAddress;
			if (!withdrawalAddressAdded[roomContractAddress]) {
				roomAddress[roomAddressIndex++] = roomContractAddress;
				withdrawalAddressAdded[roomContractAddress] = true;
			}
			if (!withdrawalAddressAdded[pendingWithdrawal.userAddress]) {
				userAddress[userAddressIndex++] = pendingWithdrawal.userAddress;
				withdrawalAddressAdded[pendingWithdrawal.userAddress] = true;
			}
			totalPendingWithdrawal[roomContractAddress][pendingWithdrawal.userAddress] += pendingWithdrawal.amount;
			pendingWithdrawal.executed = true;
		}
		for (uint i; i < roomAddressIndex; i = _uncheckedInc(i)) {
			for (uint j; j < userAddressIndex; j = _uncheckedInc(j)) {
				address roomAddr = roomAddress[i];
				address userAddr = userAddress[j];
				uint256 pendingAmount = totalPendingWithdrawal[roomAddr][userAddr];
				if (totalPendingWithdrawal[roomAddr][userAddr] != 0) {
					Token.withdrawal(roomAddr, userAddr, pendingAmount);
					totalPendingWithdrawal[roomAddr][userAddr] = 0;
					withdrawalAddressAdded[roomAddr] = false;
					withdrawalAddressAdded[userAddr] = false;
				}
			}
		}
	}
	...
	function _validateSignatures(Input calldata input, bytes32 hash)
		...
		returns (bool)
	{
		require((input.timestamp - SIGNATURE_TIMESTAMP_THRESHOLD) < block.timestamp, "INVALID_TIMESTAMP");
...
//		if (signatureVersion == SignatureVersion.Single) {
			/* Single authentication: Order signed by trader */
...
//			bytes32 computedRoot = MerkleVerifier._computeRoot(hash, merklePath);
...
//		require(input.nonce > addressMinNonce[input.bets[0]], "Nonce too low");
//		addressMinNonce[input.bets[0]] = input.nonce;
...
//		uint256 maxRoomLength,
		...
		uint256 roomLength = gameBetRoomsArr[matchId].length - processedRoomLength; // .min(maxRoomLength);
...
//				gameBets[roomId][matchId][gameType].length--;
