...
contract BTCLDynamicLottery is AutomationCompatible, AutomatedRandomness, ReentrancyGuard {
    ...
    */
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomness) internal override {
        ...
        rounds[round].status.endDate = block.timestamp;
    ...
    function claim(uint roundNr) external nonReentrant {
        ...
        if(totalAmount == 0) revert BTCLCore.UNAUTHORIZED_WINNER();

        distributionHelper(msg.sender, totalAmount);

        rounds[roundNr].winnerClaimed[msg.sender] == true;
        
        emit BTCLCore.WinnerClaimedPrizeSingle(msg.sender, totalAmount);
    }
    ...
    function multiClaim(uint[] memory roundNumbers) external nonReentrant {
        ...
        for (uint i = 0; i < roundNumbers.length; i++) {
            if(roundNumbers[i] >= round) revert BTCLCore.ROUND_NOT_FINISHED();
            if(rounds[roundNumbers[i]].winnerClaimed[msg.sender] == true) revert BTCLCore.PRIZE_ALREADY_CLAIMED();
        ...
        emit BTCLCore.WinnerClaimedPrizeMulti(msg.sender, roundNumbers);
    }
    ...
    function claimTreasury(uint roundNr, address treasuryAddress) external onlyManager {
        ...
        if(rounds[roundNr].claimedTreasury[msg.sender] == true) revert BTCLCore.PRIZE_ALREADY_CLAIMED();

        // Single Round Fees
        uint roundFees = rounds[roundNr].status.totalBets * ticketFee;
        
        // Mark the prize as claimed
        rounds[roundNr].claimedTreasury[msg.sender] = true;

        // Send accumulated fees to DAO Multisig
        distributionHelper(msg.sender, roundFees);

        // Emit the TreasuryClaimed event
        emit BTCLCore.TreasuryClaimedSingle(treasuryAddress, roundFees);
    }
     ...
     * @param roundNumbers Desired round numbers
     */
    function claimTreasury(uint[] memory roundNumbers) external nonReentrant {
        uint totalAmount;
        for (uint i = 0; i < roundNumbers.length; i++) {
            if(roundNumbers[i] >= round) revert BTCLCore.ROUND_NOT_FINISHED();
            if(rounds[roundNumbers[i]].claimedTreasury[msg.sender] == true) revert BTCLCore.PRIZE_ALREADY_CLAIMED();

            totalAmount += rounds[roundNumbers[i]].status.totalBets * ticketFee;

            rounds[roundNumbers[i]].claimedTreasury[msg.sender] = true;
        }
        
        // Send accumulated fees to DAO Multisig
        distributionHelper(msg.sender, totalAmount);

        emit BTCLCore.TreasuryClaimedMulti(msg.sender, roundNumbers);
    }
