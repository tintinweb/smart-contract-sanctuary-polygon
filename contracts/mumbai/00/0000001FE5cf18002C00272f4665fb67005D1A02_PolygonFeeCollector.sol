...
import {
    PolygonFeeCollectorInterface,
    WMATICInterface
} from "./interfaces/PolygonFeeCollectorInterface.sol";

/**
 * @title   PolygonFeeCollector
 * @author  OpenSea Protocol Team
 * @notice  PolygonFeeCollector is a contract that is used as an
 ...
 *          and transferring WMATIC.
 */
contract PolygonFeeCollector is BaseFeeCollector, PolygonFeeCollectorInterface {
    constructor() BaseFeeCollector() {}

    /**
     * @notice Unwraps and withdraws the given amount of WMATIC tokens from the
    ...
    function unwrapAndWithdraw(
        ...
        (bool result, bytes memory data) = wrappedTokenContract.call(
            abi.encodeWithSelector(WMATICInterface.withdraw.selector, amount)
        );
...
interface WMATICInterface {
    function withdraw(uint256 wad) external;
}

/**
 * @title   PolygonFeeCollectorInterface
 * @author  OpenSea Protocol Team
 * @notice  PolygonFeeCollectorInterface contains all external function
 *          interfaces for the fee collector implementation.
 */
interface PolygonFeeCollectorInterface {
    /**
     * @notice Unwraps and withdraws the given amount of WMATIC tokens from the
     *         provided contract address. Requires the caller to have the
     *         operator role, and the withdrawal wallet to be in the
     ...
     *
     * @param withdrawalWallet The wallet to be used for withdrawal.
     * @param tokenContract    The WMATIC token address to be unwrapped.
     * @param amount           The amount of tokens to be withdrawn.
     */
    function unwrapAndWithdraw(
        address withdrawalWallet,
