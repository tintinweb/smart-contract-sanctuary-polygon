...
import {ICollectioErrors} from "../ICollectioErrors.sol";
...
import {PaymentSplitter} from
    "@openzeppelin/contracts/finance/PaymentSplitter.sol";
import {Ownable} from "@openzeppelin/contracts/access/Ownable.sol";
import {VRFConsumerBase} from
    "@chainlink/contracts/src/v0.8/VRFConsumerBase.sol";
import {SaleConfigurationStandard} from "../SaleConfigurationStandard.sol";
import {WhitelistingStandard} from "./WhitelistingStandard.sol";
import {DefaultOperatorFilterer} from
    "../OperatorFilterRegistry/DefaultOperatorFilterer.sol";
import {HasRoyalties} from "../HasRoyalties.sol";
...
//                            ,&&&&&&&%&&/
//
/// @title CollectioW Contract
/// @dev This contract serves as a template for all ERC-721 generative collections.
/// It adds the delegated minting functionality, required by payment processors such as Wert.
/// @author Smart-Chain Team

contract CollectioW is
    ICollectioErrors,
    Ownable,
    ERC721,
    ERC721Burnable,
    ...
    HasRoyalties
{
    ...
    function setPreRevealURI(string calldata preRevealURI_)
        external
        onlyOwner
    {
    ...
    /// @param recipient - Address of the recipient of the minted asset.
    ...
    function mintPublicSale(uint256 amountToMint, address recipient)
        external
        payable
        notContract
        nonReentrant
    {
        ...
        if (
            addressToPublicSaleMintingAmount[recipient] + amountToMint
                > publicSaleHardcap
        ...
        for (uint256 i; i < amountToMint; i = unchecked_inc(i)) {
            unchecked {
                _mint(recipient, currentSupply + i);
            }
        }
    }
    ...
    function mintPrivateSale(
        ...
        require(
            verifyWhitelist(
                recipient, splitSignature.v, splitSignature.r, splitSignature.s
            )
        );
        ...
        if (
            addressToPrivateSaleMintingAmount[recipient] + amountToMint
                > privateSaleHardcap
        ) revert IndividualHardcapReached();
    ...
    function airdrop(address[] calldata recipient) external onlyAirdropper {
        ...
        if (
            recipientLength + reservedForAirdropMinted > totalReservedForAirdrop
        ) revert ExceedsReservedAllocation();

        if (maxTotalSupply < currentSupply + recipientLength) {
            revert MintableHardcapReached();
        }

        for (uint256 i; i < recipientLength; i = unchecked_inc(i)) {
            unchecked {
                _mint(recipient[i], currentSupply + i);
            }
        }
        unchecked {
            reservedForAirdropMinted += recipientLength;
        }
    }
...
abstract contract WhitelistingStandard is EIP712Context, Ownable {
    ...
    /// @param whitelistedAddress - Address to incorporate inside the hashed message.
    /// @param v - Signed message of the AL.
    /// @param r - Signed message of the AL.
    /// @param s - Signed message of the AL.
    ...
    function verifyWhitelist(
        ...
        bytes32 hash_ = hashWhitelist(whitelistedAddress);
        if (whitelistSigner != hash_.recover(v, r, s)) {
...
/// @title ICollectioErrors
/// @author Smart-Chain Team

interface ICollectioErrors {
...
/// @dev This contract sets up the sale configuration for the Collectio contract.
/// @author Smart-Chain Team

abstract contract SaleConfigurationStandard is Ownable {
    uint256 public maxPublicSaleMintPerTx = 20;
    uint256 public maxPrivateSaleMintPerTx = 10;
    uint256 public privateSaleHardcap = 10;
    ...
    uint256 public totalReservedForAirdrop = 200;
    ...
    /// @dev Allow the admin of the contract to update the private-sale status.
    ...
    /// @dev Allow the admin of the contract to update the public-sale status.
