...
import "./interfaces/IAIGC_NFT_UPGRADE.sol";
    ...
    Test
{
    ...
    // uint256 public baseFee; // @todo [ ] depracated.  1 USDC/USDT, number of decimals is 6

    uint public maxPurchaseOnce = 20;
    ...
    constructor(address _factory) {
        ...
        _disableInitializers(); // initialize once
    }

    /* -------Factory Operations------ */

    // @todo replace setInitialOwnership with initialize
 ...
 */
    function initialize(
        ...
        if (maxNftSupply_ != 0) {
            ...
            maxNftSupply = type(uint256).max;
        }
        saleStart = saleStart_;
        maxPurchaseOnce = maxPurchaseOnce_;
        ...
        DOMAIN_SEPARATOR = keccak256(
            abi.encode(
                ...
                keccak256(bytes(VERSION)), // @todo [ ] repalce "1" with VERSION
                chainId,
                address(this)
            )
        );
    ...
    function withdraw(address[] memory usdTokens_) public onlyOwner {
        for (uint256 i = 0; i < usdTokens_.length; i++) {
            if (usdTokens_[i] == 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE) {
                // withdraw all Matic
                uint256 balance = address(this).balance;
                payable(msg.sender).transfer(balance);
            } else {
                uint256 usdcBalance = IERC20(usdTokens_[i]).balanceOf(
                    address(this)
                );
                bool success = IERC20(usdTokens_[i]).transfer(
                    msg.sender,
                    usdcBalance
                );
    ...
    */
    ...
    function flipSaleState() public onlyOwner {
        ...
        emit SaleIsActive(saleIsActive);
    }
    ...
    function setMaxPurchaseOnce(uint256 maxPurchaseOnce_) public onlyOwner {
        maxPurchaseOnce = maxPurchaseOnce_;
    }
    ...
    function mint(
        ...
        uint256 baseFee = tokensSupported[usdToken].price.mul(
            10 ** uint256(tokensSupported[usdToken].decimals)
        );

        // Minter(msg.sender) pays fee to AIGC_NFT contract
        if (usdToken == 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE) {
            require(
                baseFee.mul(numberOfTokens) <= msg.value,
                "ETH value sent is not enough"
            );
            (bool sent, ) = payable(address(this)).call{
                value: baseFee.mul(numberOfTokens)
            }("");
            ...
            // Minter(msg.sender) pays fee to AIGC_NFT contract
            bool success = IERC20(usdToken).transferFrom(
                msg.sender,
                address(this),
                baseFee.mul(numberOfTokens)
            );
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
            ...
            if (totalSupply() < maxNftSupply) {
                ...
                console.log("tokenURI:", tokenURI(mintIndex));
    ...
    event SaleIsActive(bool indexed saleIsActive);
}
...
interface IAIGC_NFT_UPGRADE is IERC721Upgradeable {
...
abstract contract ERC721URIStorageUpgradeable is Initializable, IERC4906Upgradeable, ERC721Upgradeable {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        // if cannot found in here, look up it in the parent contract
        return super.tokenURI(tokenId);
    }
