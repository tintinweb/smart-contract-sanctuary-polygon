...
contract BetContract is ERC20
    ...
    */
    ...
    function flipCoin(string memory _HeadOrTail) public view returns(bool success) {
        ...
        if(random % 3 ==0 || random % 8 == 0 && keccak256(abi.encodePacked(_HeadOrTail)) == keccak256(abi.encodePacked("Head"))) {
            //Here code execute when Head come
            success = true;
            
        }
        else if(random % 3 != 0 || random % 8 != 0 && keccak256(abi.encodePacked(_HeadOrTail)) == keccak256(abi.encodePacked("Tail"))) {
            //Here code execute when Tail come
            success = true;
        }
    }
    function getRandomValue() public view returns(uint){
       return uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp)))%50;
    }
