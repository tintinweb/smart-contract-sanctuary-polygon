...
interface IMarketplace {
    ...
    enum ErcStandard { ERC721, ERC1155 }
...
interface IWithdraw {
    ...
    function storeNFT(address tokenOwner, address tokenContract, uint256 tokenId) external returns(bool);
...
contract Marketplace is ReentrancyGuard, IMarketplace, Ownable {
    ...
    */
    constructor(address _treasury, address _escrow, uint16 _tradeFeePercentage, uint16 _auctionFeePercentage) {
    ...
    function createSaleOrder(
        ...
        uint256 price,
        bool usdt
    ) external nonReentrant contractWhitelisted(tokenContract) returns (uint256 saleId, uint256 listId) {

        require(tokenContract != address(0), "Zero Address");
        require(tokenId >= 0, "Invalid Id");
        require(price > 0, "Invalid Price");
        require(
            IERC165(tokenContract).supportsInterface(INTERFACE_ID721) || IERC165(tokenContract).supportsInterface(INTERFACE_ID1155),
        ...
        sellOrderDetails[saleId] = Sell({
            ...
            erc: IERC165(tokenContract).supportsInterface(INTERFACE_ID721) ? ErcStandard.ERC721 : ErcStandard.ERC1155,
            usdt: usdt
        });
        ...
        IWithdraw(escrow).storeNFT(tokenOwner, tokenContract, tokenId);
    ...
    */
    function listNftToMarketplace(uint256 orderId, 
                      SaleType _type
    ...
    */ 
    function unlistNftFromMarketplace(uint256 listId) public returns(bool success)
    {
        require(IWithdraw(escrow).getAuthorised(msg.sender), "Invalid sender");
    ...
    function createAuction(
        ...
        if (IERC165(tokenContract).supportsInterface(INTERFACE_ID721)) {
            ...
            uint tokenBalance = IERC1155(tokenContract).balanceOf(msg.sender, tokenId);
            standard721 = false;
    ...
    function cancelAuction(uint256 auctionId) external auctionExist(auctionId) {
        ...
        require(bidId[auctionId].length == 0 ||
            block.timestamp > auctionOrderDetails[auctionId].endTime, "Auction is running");
        if (auc.bidAmount >= auc.reservePrice && auc.reservePrice != 0) {
            revert("Finish the bidding");
        }

        _deleteAuction(auctionId, auc.tokenOwner, auctionOrderDetails[auctionId].tokenContract, auctionOrderDetails[auctionId].tokenId);

        emit CancelAuction(auctionId);
    }

    /// @dev internal function to delete Auction from the marketplace
    function _deleteAuction(uint256 auctionId, address _to, address _contract, uint256 _tokenId) internal {
        ...
        IWithdraw(escrow).claimNFTback(_to, _contract, _tokenId);
    ...
    function doBiding(uint256 auctionId, uint256 amount, uint256 time) external auctionExist(auctionId) {
        ...
        require(amount <= IERC20(USDT).allowance(msg.sender, escrow), "Not Approved");
    ...
    */
    function auctionCounter(uint256 biddingId, uint256 amount, uint256 time) external auctionExist(bidIdDetails[biddingId].id) returns(uint256 counterId) {
    ...
    function finishAuction(uint256 auctionId, uint256 biddingId) external auctionExist(auctionId) {
        ...
        if (msg.sender == owner() || msg.sender == auc.tokenOwner) {
            ...
            require(IERC20(USDT).allowance(bid.whitelist, escrow) >= bid.amount, "Insufficient Approval");
            ...
            IWithdraw(escrow).transferCurrency(msg.sender, bid.amount, true, false);
        }
        ...
        IWithdraw(escrow).transferCurrency(auc.tokenOwner, buyersAmount, true, true);
    ...
    */
    ...
    function buyOrder(uint256 orderId) external payable tradeExists(orderId) nonReentrant {
        ...
        if (sell.usdt) {
            ...
            IWithdraw(escrow).transferCurrency(sell.tokenOwner, buyersAmount, true, true);
        } else {

            IWithdraw(escrow).transferCurrency{value: price}(msg.sender, price, sell.usdt, false);

            IWithdraw(escrow).transferCurrency(treasury, treasuryCut, false, true);
            uint256 buyersAmount = price - treasuryCut;
            IWithdraw(escrow).transferCurrency(sell.tokenOwner, buyersAmount, false, true);
        }
    ...
    function getAuctionBiddings(uint256 auctionId) external view returns(uint256[] memory) {
