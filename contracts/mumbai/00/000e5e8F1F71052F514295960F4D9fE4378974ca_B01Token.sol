...
contract B01Token is IB01Token, Ownable, ERC721URIStorage, ERC721Checkpointable {
    // The founders DAO address (creators org)
    address public foundersDAO;

    // An address who has permissions to mint B01
    address public minter;

    // An address who has permissions to update token URIs
    address public uriUpdater;

    // The B01 token URI descriptor
    IB01Descriptor public descriptor;

    // The B01 token seeder
    IB01Seeder public seeder;
    ...
    // The b01 seeds
    mapping(uint256 => IB01Seeder.Seed) public seeds;

    // The internal b01 ID tracker
    uint256 private _currentB01Id;
    ...
    /**
     * @notice Mint a B01 to the minter, along with a possible founders reward
     * B01. Founders reward B01 are minted every 10 B01, starting at 0,
     * until 183 b01der B01 have been minted (5 years w/ 24 hour auctions).
     * @dev Call _mintTo with the to address(es).
     */
    function mint() public override onlyMinter returns (uint256) {
        if (_currentB01Id <= 1820 && _currentB01Id % 10 == 0) {
            _mintTo(foundersDAO, _currentB01Id++);
        }
        return _mintTo(minter, _currentB01Id++);
    }

    /**
     * @notice Burn a B01.
     */
    function burn(uint256 b01Id) public override onlyMinter {
        _burn(b01Id);
        emit B01Burned(b01Id);
    }
    ...
    function tokenURI(uint256 tokenId) public view override(ERC721, ERC721URIStorage) returns (string memory) {
        require(_exists(tokenId), 'B01Token: URI query for nonexistent token');
     ...
     * @dev Only callable by the owner when not locked.
     */
    function setDescriptor(IB01Descriptor _descriptor) external override onlyOwner whenDescriptorNotLocked {
        descriptor = _descriptor;
     ...
     * @dev Only callable by the owner when not locked.
     */
    function setSeeder(IB01Seeder _seeder) external override onlyOwner whenSeederNotLocked {
        seeder = _seeder;
    ...
    function setTokenURI(uint256 tokenId, string memory uri) external onlyUriUpdater {
        require(_exists(tokenId), 'B01Token: URI set for nonexistent token');
    ...
    /**
     * @notice Mint a B01 with `b01Id` to the provided `to` address.
     */
    function _mintTo(address to, uint256 b01Id) internal returns (uint256) {
        IB01Seeder.Seed memory seed = seeds[b01Id] = seeder.generateSeed(b01Id, descriptor);

        _mint(owner(), to, b01Id);
        emit B01Created(b01Id, seed);
...
abstract contract ERC721URIStorage is ERC721 {
    ...
    // Mapping for unique B01 tokenURIs
    mapping(uint256 => string) private _tokenURIs;

    string private _tempB01Hash = 'QmZi1n79FqWt2tTLwCqiy6nLM6xLGRsEPQ5JmReJQKNNzX';
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        // Iff tokenId is empty in mapping yet exists in the contract, return the tempB01Hash
        string memory _tokenURI = _tokenURIs[tokenId];
        if (bytes(_tokenURI).length == 0) {
            return string(abi.encodePacked('ipfs://', _tempB01Hash));
        }
...
// With modifications by B01ders DAO.
//
//
// MODIFICATIONS:
