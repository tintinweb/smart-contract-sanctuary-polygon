...
interface IERC1155 is IERC165 {
    ...
    event setRoyaltyFee(uint256 fee);
...
contract ERC1155 is Context, ERC165, IERC1155, IERC1155MetadataURI {
    ...
    function getTotalBalance(uint256 tokenId) public view returns (uint256) {
        require(
            _exists(tokenId),
            "ERC1155Metadata: balance query for nonexistent token"
        );
    ...
    */
    ...
    function _mint(
        ...
        _balances[tokenId][_to] = _supply;
        _totalBalance[tokenId] = _supply;
        _setTokenURI(tokenId, _uri);

        emit TransferSingle(_to, address(0x0), _to, tokenId, _supply);
        emit URI(_uri, tokenId);
    }

    function _setRoyaltyFee(
        address creator,
        uint256 tokenId,
        uint256 fee
    ) internal virtual {
        require(creator != address(0), "ERC721: mint to the zero address");
        require(creator == creators[tokenId], "ERC721: not token creator");
        require(
            _balances[tokenId][creator] > 0,
            "ERC721: you have not even 1 token "
        );
    ...
    function _burnBatch(
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                _balances[tokenIds[i]][account] >= amounts[i],
                "ERC1155: insufficient balance for transfer"
            );
            _balances[tokenIds[i]][account] =
                _balances[tokenIds[i]][account] -
                amounts[i];
        }
...
interface IRDLANDWhitelistInterface {
    function isWhitelisted(address _address) external view returns (bool);
}

contract RDLAND1155 is ERC1155 {
    ...
    */
    ...
    function mint(
        ...
        _mint(_to, newTokenId, supply, uri, fee);
        uint256 tokenId = newTokenId;
        newTokenId = newTokenId + 1;

        if (approve == true) {
            setApprovalForAll(approve_address, true);
        }
        return tokenId;
    }
    ...
    function setCreatorRoyalty(uint256 tokenId, uint256 fee) external {
        require(_exists(tokenId), "ERC721: non existent token");
        require(
            balanceOf(msg.sender, tokenId) == getTotalBalance(tokenId),
            "ERC721: your token balance is not correct."
        );
        _setRoyaltyFee(msg.sender, tokenId, fee);
    }
}