...
contract MetaMaster {
    ...
    function getUserTotalReturns(address _userAddress) public view returns(uint256){
        ...
        for(uint8 i = 0; i < users[_userAddress].deposits.length; i++) {
            uint256 reward;
            uint256 numberOfDays = block.timestamp.sub(users[_userAddress].deposits[i].depositedTimeStamp).div(1 seconds);
            ...
            totalReward = totalReward + reward + users[_userAddress].bonus; 
        }
        return totalReward.sub(mintingAmountWithdrawn[_userAddress]+bonusAmountWithdrawn[_userAddress]);
    }

    function getUserPendingMintingWithdrawalAmount(address _userAddress) public view returns(uint256) {
        return getUserTotalReturns(_userAddress).sub(getUserPendingBonusWithdrawalAmount(_userAddress));
    }
    ...
    function withdraw(address _userAddress, uint256 _mintAmount, uint256 _bonusAmount) public {
        ...
        uint256 addAmount = mintingAmountWithdrawn[_userAddress] + bonusAmountWithdrawn[_userAddress];
        ...
        //getUserTotalReturns(_userAddress) = getUserTotalReturns(_userAddress)+(addAmount - creatorTax); 
