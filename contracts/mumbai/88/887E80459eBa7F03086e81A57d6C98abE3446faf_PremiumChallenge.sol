...
contract PremiumChallenge is ReentrancyGuard {
    ...
    function forecastChallengeState() private view returns (ChallengeState){
        ...
        if (block.timestamp >= metaData.startTime && block.timestamp < metaData.endTime - 1)
    ...
    function setWinners(address[] memory _winnersWallets, uint256 _qualifiedParticipants) public senderIsAddress onlyOwner challengeWinnersIsNotSet challengeStateIsValidForSettingWinners {
        // validate wallets
