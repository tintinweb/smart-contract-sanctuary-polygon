    ...
    */
    ...
    function burn(uint256 tokenId) public virtual {
        ...
        if(isRare(tokenId)) {
            ...
            bytes32 hashed = keccak256(abi.encodePacked(blockhash(block.number-1), block.timestamp, msg.sender));
            uint256 lotteryId = uint(hashed);
            string memory lotteryImage = neolastics.generateSVGofTokenById(lotteryId);
            string memory tokenImage = neolastics.generateSVGofTokenById(tokenId);
            if (keccak256(abi.encodePacked(lotteryImage)) == keccak256(abi.encodePacked(tokenImage))) {
                burnPrice = reserve;
                reserve = 0;
                gameEnded = true;
