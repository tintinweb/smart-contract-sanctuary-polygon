...
contract Celaris is  ERC721Royalty,  Ownable, ReentrancyGuard, UpdatableOperatorFilterer, EIP712 {
   ...
   function mints(address to,uint256[] memory tokenId) external {
    ...
    function verifyRedeemData(bytes32 hashdata, uint64 timestamp, address signer, bytes calldata signature) public view returns (bool) {
        bytes32 digest = _hashTypedDataV4(keccak256(abi.encode(CELLARIS_REDEEM_DATA_TYPEHASH, hashdata, timestamp)));
