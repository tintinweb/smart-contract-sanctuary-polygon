    ...
    SafeMathEthermon
{
    ...
    */
    ...
    function setSender() isActive external {
            sender = msg.sender;
        }



function debugMonInfo(uint64 _objId) public view returns (uint32){
  ...
  function debugMonExpLevel(uint64 _objId) public view returns (uint8){
        BasicObjInfo memory objInfo;
        (objInfo.classId, objInfo.owner, objInfo.exp) = getObjClassExp(_objId);
        objInfo.level = getLevel(objInfo.exp);
        return objInfo.level;
    ...
    function debugTransform(uint32 class_id) public view returns (uint32 transformClass, uint8 transformLevel) {
      
        (transformClass, transformLevel) = EtheremonTransformSettingInterface(
            transformSettingContract
        ).getTransformInfo(class_id);
        if (transformClass == 0 || transformLevel == 0) revert();

        return (transformClass, transformLevel);
    }



function debugLevelDet6(uint64 _objId) public view returns (uint32 classId) {
        ...
        return objInfo.classId;
     

    }
    
       function debugBasicInfo(uint64 _objId) public view returns (uint32 classId, uint8 level){
         BasicObjInfo memory objInfo;
        (objInfo.classId, objInfo.owner, objInfo.exp) = getObjClassExp(_objId);
        objInfo.level = getLevel(objInfo.exp);
        if (objInfo.classId == 0 || objInfo.owner != sender) revert();
        return (objInfo.classId, objInfo.level);

    }


function debugLevelDet5(uint64 _objId) public view returns (uint32 classId, address owner, address sender ) {

        BasicObjInfo memory objInfo;
        (objInfo.classId, objInfo.owner, objInfo.exp) = getObjClassExp(_objId);
        objInfo.level = getLevel(objInfo.exp);
        return (objInfo.classId, objInfo.owner, msg.sender);
     

    }


function debugLevelDet1(uint64 _objId) public view returns (uint32 transformClass) {
...
function debugLevelDet2(uint64 _objId) public view returns (uint8 transformLevel ) {
        ...
        //if (objInfo.level < transformLevel) revert(); //Reverting here.
      //  return true;

    }


function debugLevelDet3(uint64 _objId) public view returns ( uint8 level ) {
...
function debugLevelDet4(uint64 _objId) public view returns (uint32 transformClass, uint8 transformLevel, uint8 level ) {
        ...
        //if (transformClass == 0 || transformLevel == 0) revert();
        return (transformClass, transformLevel, objInfo.level);
    ...
    function debugLevel(uint64 _objId) public view returns (bool) {
        ...
        if (transformClass == 0 || transformLevel == 0) {
            revert();
        }
        if (objInfo.level < transformLevel) {
            revert(); //Reverting here.
        }
        return true;
    ...
    */
    function checkAncestors(
        ...
        if (_a1 > 0) {
            temp = getClassCheckOwner(_a1, _trainer); //Will return class by taking object id
 ...
 */
        // Will always return false as temp is ancestor id and requestAncestor contains class id which are not equal.
        //Logic to matic mon +1000
        uint32 ancestor1 = requestAncestors[0] >= 1000 ? requestAncestors[0] : (requestAncestors[0] + 1000);
        uint32 ancestor2 = requestAncestors[1] >= 1000 ? requestAncestors[1] : (requestAncestors[1] + 1000);
        uint32 ancestor3 = requestAncestors[2] >= 1000 ? requestAncestors[2] : (requestAncestors[2] + 1000);
        for (index = 0; index < ancestors.length; index++) {
            temp = ancestors[index];
          if (
                temp > 0 &&
                temp != requestAncestors[0] && (temp + 1000) != ancestor1 &&
                temp != requestAncestors[1] && (temp + 1000) != ancestor2 &&
                temp != requestAncestors[2] && (temp + 1000) != ancestor3
            ) return false;
        }
    ...
    function transform(
        ...
        emit EventTransform(msg.sender, _objId, newObjId);
    }
}