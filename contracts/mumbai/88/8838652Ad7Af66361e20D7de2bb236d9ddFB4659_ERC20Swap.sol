...
library Order {
     ...
     * @param FULL_ORDER_MESSAGE_TYPE message types using in eip712
    ...
    function verifyFullOrder(
        ...
        string memory FULL_ORDER_MESSAGE_TYPE
    ) internal pure returns (address) {
        ...
        bytes32 hash = keccak256(
            abi.encodePacked(
                ...
                keccak256(
                    abi.encode(
                        keccak256(abi.encodePacked(FULL_ORDER_MESSAGE_TYPE)),
                        nonce,
                        maker,
                        tokenToSell,
                        sellAmount,
     ...
     * @param sellAmount Amount of tokens the maker wants to sell
     * @param tokenToBuy Address of the token the maker wants to buy
     * @param taker address of the taker for this order
     * @param buyAmount Amount of tokens the maker wants to buy
     * @param signature Signature that the maker signed while making the order
     * @param DOMAIN_SEPARATOR domain seperator used for eip712
     * @param PRIVATE_ORDER_MESSAGE_TYPE message types using in eip712
     * @return Address that signed the given signature
     */
    function verifyPrivateOrder(
        ...
        bytes calldata signature,
        bytes32 DOMAIN_SEPARATOR,
        string memory PRIVATE_ORDER_MESSAGE_TYPE
    ) internal pure returns (address) {
        require(signature.length == 65, "wrong signature passed");
        ...
        bytes32 hash = keccak256(
            abi.encodePacked(
                ...
                keccak256(
                    abi.encode(
                        keccak256(abi.encodePacked(PRIVATE_ORDER_MESSAGE_TYPE)),
                        nonce,
                        maker,
                        tokenToSell,
                        sellAmount,
...
import "./ERC20Order.sol";

/**
 * @title A contract for non custodial OTC Swap between two parties
 */
contract ERC20Swap is ERC165, ReentrancyGuard, Pausable, AccessControl {
    ...
    function swapFullOrder(
        ...
        OrderType orderType = OrderType.FULL_ORDER;
        ...
        emit buyOrder(orderID, msg.sender, tokenToBuy, buyAmount, orderType);
    ...
    function swapPrivateOrder(
        ...
        platformFeesPerToken[tokenToSell] += makerFees;
        platformFeesPerToken[tokenToBuy] += takerFees;
        OrderType orderType = OrderType.PRIVATE_ORDER;
