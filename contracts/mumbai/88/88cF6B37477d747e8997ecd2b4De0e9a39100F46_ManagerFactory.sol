...
contract Proposal {
    ...
    function cancelProposal() external onlyAdmin {
        require(
            status == Status.Created || status == Status.Approved,
    ...
    function contributeFunds() external payable onlyStatus(Status.Approved) {
        ...
        if (collectedAmount >= amountToCollect) status = Status.Finished;
    }
    ...
    function withdrawFunds(uint amount) external {
        require(
            status == Status.Approved || status == Status.Canceled,
            'Not permitted in this status'
        );
    ...
    function redeemContributions() external onlyStatus(Status.Finished) {
