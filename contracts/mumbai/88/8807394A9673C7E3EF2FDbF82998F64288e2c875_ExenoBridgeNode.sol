    ...
    ERC1363Payable
{
    ...
    function deposit(
        ...
        external payable nonReentrant whenNotPaused
    {
        (Data memory data, uint256[2] memory fee, bytes32 operation, bytes memory signature) = _unpackSingleSignedData(encodedData);
        _verifySingleSignature(data, fee, operation, signature);
        _validateClass(data, operation, Class.DEPOSIT);
        _validateDeposit(data, fee);
        _processDeposit(data, false);
    ...
    function withdraw(
        ...
        external payable nonReentrant whenNotPaused
    {
        (Data memory data, uint256[2] memory fee, bytes32 operation, bytes[3] memory signature) = _unpackTrippleSignedData(encodedData);
        _verifyTrippleSignature(data, fee, operation, signature);
        _validateClass(data, operation, Class.WITHDRAW);
        _validateWithdraw(data, fee);
        _processWithdraw(data, false);
        ...
        external view returns(bool)
    {
        ...
        if (data.method[0] == METHOD_BURN) {
            require(_isMintable(data.token[0]),
                "ExenoBridgeNode: burn method cannot be done on a non-mintable token");
        } else if (data.method[0] == METHOD_FREEZE) {
            require(_isCore(data.token[0]),
                "ExenoBridgeNode: freeze method cannot be done on a non-core token");
        }
        ...
        external view returns(bool)
    {
        ...
        if (data.method[1] == METHOD_MINT) {
            require(_isMintable(data.token[1]),
                "ExenoBridgeNode: mint method cannot be done on a non-mintable token");
        } else if (data.method[1] == METHOD_UNFREEZE) {
            require(_isCore(data.token[1]),
                "ExenoBridgeNode: unfreeze method cannot be done on a non-core token");
        }
     ...
     * @param amount The amount of tokens transferred
     * @param wrappedData Encoded payload which specifies the intended deposit or withdrawal
     */
    function _transferReceived(
        address,
        address sender,
    ...
    /**
     * Unpack encoded signed data for deposits
     */
    function _unpackSingleSignedData(bytes memory encodedData)
        internal pure returns(Data memory, uint256[2] memory, bytes32, bytes memory)
    {
        (
            ...
            bytes32 operation,
            bytes memory signature
        ) = abi.decode(encodedData, (bytes, uint256[2], bytes32, bytes));
        return (_unpackPayload(payload), fee, operation, signature);
    }
    
    /**
     * Unpack encoded signed data for withdrawals
     */
    function _unpackTrippleSignedData(bytes memory encodedData)
        internal pure returns(Data memory, uint256[2] memory, bytes32, bytes[3] memory)
    {
        (
            bytes memory payload,
            uint256[2] memory fee,
            bytes32 operation,
            bytes[3] memory signature
        ) = abi.decode(encodedData, (bytes, uint256[2], bytes32, bytes[3]));
        return (_unpackPayload(payload), fee, operation, signature);
    }
