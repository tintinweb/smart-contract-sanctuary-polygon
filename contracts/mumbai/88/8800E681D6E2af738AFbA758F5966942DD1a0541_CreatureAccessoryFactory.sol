...
contract CreatureAccessoryFactory is FactoryERC1155, Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => bytes) private metadta;
    string
    ...
    uint256 public constant NUM_OPTIONS = 5000;
    ...
    function mint(
        ...
        return _mint(_optionId, _toAddress, _amount, _uri, _data);
    }
    ...
    function _createOrMint(
        ...
        if (!tradable.exists(_id)) {
            tokenIdList.push(_id);
            metadta[_id] = _data;
            tradable.create(_to, _id, _amount, _uri, _data);
        } else {
    ...
    function getMintedData() external view returns (bytes[] memory) {
        bytes[] memory mintedData = new bytes[](tokenIdList.length);
        for (uint i = 0; i < tokenIdList.length; i++){
            mintedData[i] = metadta[tokenIdList[i]];
        }
        return mintedData;
    }
}
