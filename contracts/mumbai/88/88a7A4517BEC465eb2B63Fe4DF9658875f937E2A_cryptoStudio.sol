...
//    @@@    ::   :::  @@@@@@@   :: ::::  ::: ::::  @@@@@@@   :   : :  ::         ::  @@@@@@@     \\
    ...
    string private constant SPACE_SCHEMA = "tokenID text, trait_type text, value text";

    string  private space_social_table;
    uint256 private space_social_tableID;
    string private constant SPACE_GROUP_TABLE_PREFIX = "space_group";
    string private constant SPACE_GROUP_SCHEMA = "spaceName text, space_owner text, groupID text";
    ...
    constructor() ERC721("CryptoStudio", "CS") 
    {
        ...
        space_social_tableID = tablelandContract.createTable(
            address(this),
            SQLHelpers.toCreateFromSchema(SPACE_GROUP_SCHEMA, SPACE_GROUP_TABLE_PREFIX)
        );

        space_social_table = SQLHelpers.toNameFromId(SPACE_GROUP_TABLE_PREFIX, space_social_tableID);

    }


    function mintSpace(string memory spaceName , string memory space_group) public {
        require(spaceMap[spaceName] == address(0) , "taken space");
        spaceMap[spaceName] = msg.sender;
        string memory insert_statement =
        SQLHelpers.toInsert(
                SPACE_GROUP_TABLE_PREFIX,
                space_social_tableID,
                "spaceName, space_owner, groupID",
                string.concat(
        ...
        runSQL(space_social_tableID,insert_statement);
    ...
    function updateAttribute(uint256 tokenid , string memory attributeName , string memory value, bool main_table) public {
        ...
        if(main_table){
            ...
            string memory Update_statement = SQLHelpers.toUpdate(MAIN_TABLE_PREFIX, spacesTableID, set, filter);
            runSQL(spacesTableID,Update_statement);
        }
    ...
    function space_groups_TableURI() 
    public view returns (string memory) {
        return string.concat(
            tableURI(), 
            "SELECT%20*%20FROM%20",
