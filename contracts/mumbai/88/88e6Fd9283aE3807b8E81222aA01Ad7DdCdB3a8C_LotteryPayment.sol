...
contract LotteryPayment is Ownable, ReentrancyGuard {
    event LotteryPriceUpdated(uint256 amount);
    event ChainLinkContractAddressUpdated(address contractAddress);
    uint256 public lotteryPrice;
    address public chainLinkContractAddress;
    
    constructor() {
        lotteryPrice = 10 * 1e8;
        chainLinkContractAddress = 0xd0D5e3DB44DE05E9F294BB0a3bEEaF030DE24Ada;
    }

    function setLotteryPrice(uint256 amount) external onlyOwner {
        lotteryPrice = amount;
        emit LotteryPriceUpdated(lotteryPrice);
    }

    function setChainLinkContractAddress(address contractAddress) external onlyOwner {
    ...
    function deposit() payable external nonReentrant {
        ...
        require(msg.value >= amount, "Invaid Payment Amount");
        (bool success, ) = owner().call{value: msg.value}("");
        require(success, "Failed to send the Payment Amount");
    }
    ...
    function recoverMatic(address walletAddress) external onlyOwner {
