...
contract Fundraiser is Pausable, Ownable {
    ...
    constructor(
        ...
        require(_fundraiseEndTime <= _distributionStartTime, "Fundraise end time must not be greater than distribution start time");
    ...
    function deposit(uint256 _amount) external whenNotPaused canDeposit(_amount) {
        require(block.timestamp >= timeConfig.openStartTime, "Fundraising has not started yet");
        require(depositedAmount[msg.sender] + _amount <= allocationConfig.baseAllocationPerWallet, "Max allocation per wallet reached");
    ...
    function depositWithNft(uint256 _amount, uint256[] memory nftIds) external whenNotPaused canDeposit(_amount) {
        ...
        useAllocationWithNfts(nftIds, _amount);
        if (
            !IERC20(tokenConfig.depositToken).transferFrom(msg.sender, address(this), _amount)
        ) {
            revert TransferFailed();
    ...
    function depositWithWhitelist(uint256 _amount, bytes32[] memory proof_) external whenNotPaused canDeposit(_amount) {
        require(block.timestamp >= timeConfig.nftStartTime, "Fundraising has not started yet");
    ...
    function depositWithWhitelistAndNft(uint256 _amount, uint256[] memory nftIds, bytes32[] memory proof_) external whenNotPaused canDeposit(_amount) {
        require(block.timestamp >= timeConfig.nftStartTime, "Fundraising has not started yet");
    ...
    function withdraw() external whenNotPaused onlyOwner {
        ...
        uint256 balance = IERC20(tokenConfig.depositToken).balanceOf(address(this));
        if (
            !IERC20(tokenConfig.depositToken).transfer(owner(), balance)
        ) {
            revert TransferFailed();
        }
    ...
    function calculateMaxAllocation(uint256 userDeposit, uint256 validNftCount, uint256 invalidNftCount) private view returns (uint256) {
        ...
        uint256 userBaseAllocation = allocationConfig.baseAllocationPerWallet;
        if (whitelistUser[msg.sender]) {
            userBaseAllocation = allocationConfig.baseAllocationPerWallet.add(whitelistedAllocation);
        }

        if (userDeposit <= userBaseAllocation) {
            maxAllocation = userBaseAllocation.add(validNftCount.mul(allocationConfig.nftTicketAllocation));
        } else {
            uint256 threshold1 = userBaseAllocation.add(allocationConfig.nftTicketAllocation);
            uint256 threshold2 = userBaseAllocation.add(allocationConfig.nftTicketAllocation.mul(2));
            ...
            maxAllocation = userBaseAllocation.add(allowedNfts.mul(allocationConfig.nftTicketAllocation));
        }
...
contract FundraiserFactory is Ownable {
    ...
    function getFundraiserStatus(address fundraiserAddress) external view returns (FundraiserStatus) {
        ...
        } else if (block.timestamp <= distributionStartTime) {
