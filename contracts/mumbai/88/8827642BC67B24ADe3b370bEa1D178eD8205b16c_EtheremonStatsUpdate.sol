...
// File: contracts/EtheremonStatsUpdate.sol
...
interface EtheremonTradeDataInterface {
    function isOnTrading(uint64 _objId) external view returns (bool);
}

contract EtheremonStatsUpdate is EthermonEnum, BasicAccessControl {
    ...
    mapping(uint32 => uint256) public customClassBurnPrice;
    ...
    constructor(address _dataContract, address _tradeDataContract) public {
        dataContract = _dataContract;
        tradeDataContract = _tradeDataContract;
    }

    function setContract(address _dataContract, address _tradeDataContract)
        external
        onlyModerators
    {
        dataContract = _dataContract;
    ...
    */
    function AddEthMonsterBaseStats(uint64 _objId, uint8[6] calldata stats)
        external
        onlyModerators
    {
        ...
        EtheremonTradeDataInterface tradeData = EtheremonTradeDataInterface(
            tradeDataContract
        );

        if (tradeData.isOnTrading(obj.monsterId)) revert();

        //Check if object-id passed isn't wrong
        require(obj.trainer != address(0));
        for (uint8 i = 0; i < stats.length; i += 1) {
    ...
    function BulkEthMonsterBaseStats(uint64[] calldata stats)
        external
        onlyModerators
    {
        EtheremonDataBase data = EtheremonDataBase(dataContract);
        ...
        //Check if object-id passed isn't wrong
        for (uint8 i = 0; i < stats.length - 1; i += 7) {
            MonsterObjAcc memory obj;
            (
                obj.monsterId,
                obj.classId,
                ...
                obj.lastClaimIndex,
                obj.createTime
            ) = data.getMonsterObj(stats[i]);

            if (obj.trainer == address(0)) revert();
            if (tradeData.isOnTrading(obj.monsterId)) revert();
            // 0 - stats and then i + 1 stats 1 2 3 4 5 6

            /** A bit complication here :D 
                i = 0 (first index),
                stats[i] first element is mon object id,
                stats[i + 1] is first stats
            */
            data.updateIndexOfArrayType(
                EthermonEnum.ArrayType.STAT_BASE,
                stats[i], //_objId,
