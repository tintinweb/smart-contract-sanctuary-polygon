...
/* @dev: Check if contract owner */
    ...
    function mint(address _to) public onlyAdmin {
        ...
        if (
            ...
            (NUM_SPECIAL_NFTS + totalNftsMinted) < NUM_SPECIAL_NFTS - specialNftsMinted) 
            {
              ...
              typeNFT = string(abi.encodePacked("Special Polykick NFT ", uint2str(tokenId)));
            ...
            typeNFT = string(abi.encodePacked("Normal Polykick NFT", uint2str(tokenId)));
            details[_to].typeNFT = typeNFT;
        }
    ...
    for (uint256 i = 0; i < totalNftsMinted; i++) {
        ...
        if (details[user].userWhitelistedIlos > 0) {
            ...
            details[user].userWhitelistedIlos -= 1;

            if (i == totalNftsMinted - 1 || gasleft() < 50000) {
                (bool success, ) = address(iloContract).call(abi.encodeWithSignature("addToWhiteListBulk(address[])", usersToAddToWhiteList));
                require(success, "Failed to add users to whitelist");
                numUsersToAddToWhiteList = 0;
            }
        }
    }
}
