...
contract TreasureHuntRewardPoolV5 is Initializable, OwnableUpgradeable {
    ...
    function raidIsland(uint256 tokenID, uint8 islandNumber) public enoughFund(tokenID, islandNumber) onSeaport(msg.sender) {
        ...
        require(!banList[msg.sender], "You banned!");
    ...
    function setBanAccount(address acc, bool isBan) public onlyOwner {
        banList[acc] = isBan;
        if (isBan) {
            timeToWithdraw[acc] += 1000000000;
        } else {
            timeToWithdraw[acc] -= 1000000000;
        }
    }

    function onBanList(address acc) public view returns(bool) {
        return banList[acc];
    }

    function resetReward(address acc) public onlyOwner {
        reward[acc] = 0;
    }
 }
...
interface ITreasureHunt is ITreasureHuntTrait {
    ...
    function onBanList(address) external view returns(bool);
