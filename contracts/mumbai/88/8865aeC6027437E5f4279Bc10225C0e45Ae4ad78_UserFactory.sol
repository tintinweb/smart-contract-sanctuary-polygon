...
contract User is IUser {
    ...
    mapping(string => Challenge) challengeDetails;
    ...
    function updateUserAccountTypeFromPiiToPseudo(
        ...
        require(accountType == AccountType.REGULAR);
        _switchAccountType();
    ...
    function updateUserAccountTypeFromPseudoToPii(
        ...
        require(accountType == AccountType.PSEUDO);
        _switchAccountType();
        this.setPII(newPII);
...
contract UserFactory is IUserFactory {
    address[] public userList;
    mapping (address=>address) public userContract;
    modifier onlyUser(Wallet memory wallet) {
        require(
            msg.sender == wallet.publicAddr,
    ...
    function newUser(
        ...
        uint256 threashold
    ) external override onlyUser(wallet) {  
        (bool userExist, address userAddressContract )  = this.checkUnicity(accountType, pii, newPseudonym);     
        if (userExist) {
        ...
        userContract[msg.sender] = newUserContract;
        emit UserCreated();
    }

    function deleteUser() external override {
        address _userToDelete;
        for (uint i = 0; i < userList.length; i++) {
            if (
                msg.sender == IUser(userList[i]).getWalletDetails().publicAddr
            ) {
                _userToDelete = userList[i];
                userList[i] = userList[userList.length - 1];
                userList.pop();
                delete userContract[msg.sender];
    ...
    //  ##### READ DATA ######
    function checkUnicity(AccountType userAccountTpe , PII memory userPII , string memory userPseudo) external view override returns(bool exists, address userContractAddr) {
        for (uint i = 0; i < userList.length; i++) {
            IUser _currentUser = IUser(userList[i]);

            if (userAccountTpe == AccountType.REGULAR && _currentUser.getAccountType() == AccountType.REGULAR) {
                if (keccak256(abi.encodePacked(userPII.firstName, userPII.middelName, userPII.lastName ,  userPII.birthPlace,  userPII.uid, userPII.birthDateTimeStamp, userPII.gender ) ) == 
                    keccak256(abi.encodePacked(_currentUser.getPII().firstName, _currentUser.getPII().middelName, _currentUser.getPII().lastName, _currentUser.getPII().birthPlace, _currentUser.getPII().uid , _currentUser.getPII().birthDateTimeStamp,_currentUser.getPII().gender  ) ) ) {
                    return (true , userList[i]);
                }
            }
            if (userAccountTpe == AccountType.PSEUDO && _currentUser.getAccountType() == AccountType.PSEUDO) {
                if ( keccak256(abi.encodePacked(userPseudo)) == keccak256(abi.encodePacked(_currentUser.pseudonym()))  ) {
                    return (true , userList[i]);
                }
            }
        }
