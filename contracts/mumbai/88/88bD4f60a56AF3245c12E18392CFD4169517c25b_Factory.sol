...
contract Factory is IFactory, IOwnable {
    ...
    function getFractionToTnftIdBatch(ITangibleFractionsNFT[] calldata ftnfts) external returns(TnftWithId[] memory batch){
        ...
        returns (ITangibleFractionsNFT)
    {
        ...
        string memory uri = _tnft.tokenURI(_tnftTokenId);
        ...
        if (address(rentShare_) != address(0)) {
            AccessControl(address(rentShare_)).grantRole(
                CLAIMER_ROLE,
