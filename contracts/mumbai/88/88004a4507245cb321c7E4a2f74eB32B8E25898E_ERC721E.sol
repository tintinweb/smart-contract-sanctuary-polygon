...
interface IERC721E {
    ...
    event NewEntry(uint256 indexed pos, string name, uint256 indexed price, uint256 maxSupply);
    ...
    function versionERC721E() external pure returns (string memory);
    ...
    function createEntry(string memory name, uint256 price, uint256 maxSupply, uint256 maxBuy) external;
    ...
    /// @param sellingPrice array of prices the sender will sell the token/ticket
    function buyTickets(uint256 entryPos, uint256 amount, uint256[] memory sellingPrice) external;
...
contract ERC721E is IERC721E, ERC721AMarketplace, Ownable, AccessControl {
    ...
    //in the form of user => (entryPos => amountOfTickets)
    mapping(address => mapping(uint256 => uint256)) private _userTicketsBought;
    ...
    function createEntry(
        ...
        _entryByPos[pos] = EntryType(name, price, maxSupply, maxBuy, 0);
        emit NewEntry(pos, name, price, maxSupply);
    }

    /// @inheritdoc IERC721E
    function editEntryName(uint256 entryPos, string memory name)
        external
        virtual
        override
    ...
    /// @inheritdoc IERC721E
    function editEntryPrice(uint256 entryPos, uint256 price)
        external
        virtual
        override
        onlyRole(CREATOR_ROLE)
    ...
    /// @inheritdoc IERC721E
    function editEntryMaxSupply(uint256 entryPos, uint256 maxSupply)
        external
        virtual
        override
        onlyRole(CREATOR_ROLE)
    ...
    /// @inheritdoc IERC721E
    function editEntryMaxBuy(uint256 entryPos, uint256 maxBuy)
        external
        virtual
        override
        onlyRole(CREATOR_ROLE)
    ...
    function buyTickets(
        ...
        if (_userTicketsBought[_msgSender()][entryPos] + amount > maxBuy)
            revert ERC721ETooManyTickets();
        ...
        _userTicketsBought[_msgSender()][entryPos] += amount;

        //Implement royalties
        uint256 totalPrice = amount * price;
        (
            address receiverRoyalty,
            uint256 amountRoyalty
        ) = _getDefaultRoyaltyBatch(totalPrice);
        IERC20(paymentToken).transferFrom(
            _msgSender(),
            venue,
            totalPrice - amountRoyalty
        );
        IERC20(paymentToken).transferFrom(
            _msgSender(),
            receiverRoyalty,
        ...
        emit MintTickets(_msgSender(), entryPos, amount);
    }
