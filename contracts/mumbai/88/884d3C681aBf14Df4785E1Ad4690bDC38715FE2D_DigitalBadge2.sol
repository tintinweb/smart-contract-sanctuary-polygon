    ...
    UUPSUpgradeable
{
    ...
    function own(uint256 tokenId) public view returns (address)  {
        address owner = ownerOf(tokenId);
        return owner;
    }
    function allowUSDC(uint256 tokenId) public view returns (uint256) {
        address owner = ownerOf(tokenId);
        return usdcToken.allowance(msg.sender, owner);
    }
    function balancUSDC()  public view returns (uint256) {
        return usdcToken.balanceOf(msg.sender);
    }


    function quemar(uint256 tokenId)  external {
