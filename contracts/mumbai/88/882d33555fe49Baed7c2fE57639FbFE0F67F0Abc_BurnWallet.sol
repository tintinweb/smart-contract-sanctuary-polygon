...
contract BurnWallet is Ownable {
    ...
    constructor(
        address dudeToken_,
        address owner_,
        address gelatoAddress_
    )
    {   
        _transferOwnership(owner_);
        token = IDudeToken(dudeToken_);
        gelatoAddress = gelatoAddress_;
    }

    modifier onlyBurnTaskOrOwner() {
        require(
            msg.sender == gelatoAddress || msg.sender == owner(),
            "account is not gelato or owner"
        );
        _;
    }

    function setGelatoAddress(address gelatoAddress_) 
    external 
    onlyOwner
    {
        gelatoAddress = gelatoAddress_;
    ...
    {   
        ...
        require(
            dayCapacity > 0, 
            "Day burn limit exceed"
        );

        // the burn amount
        uint256 toBurn = Math.min(token.balanceOf(address(this)), dayCapacity);


        // checking if a 900 billion is already burned
        require(
            totalBurn < burnHitPoint,
            "Total burn amount exceed"
        );
...
contract DuDeToken is ERC20Upgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    public initializer 
    {
        ...
        // declare USDC coin contract
        burnWallet = new BurnWallet(address(this), owner(), gelatoAddress_);
    ...
    modifier onlyBurnWallet() {
        require(msg.sender == address(burnWallet), "adress does not have burn accesss");
        _;
    }
    ...
    function airdrop(address[] calldata addrs, uint256[] calldata counts) external whenNotPaused onlyOwner {
        require(
            addrs.length == counts.length,
            "Invalid Data lengthes mismatch"
        );

        for (uint256 i = 0; i < addrs.length; ++ i) {
            require(
                addrs[i] != address(0),
                "Zero adress included"
            );
            _transfer(address(this), addrs[i], counts[i]);
        }
    }
