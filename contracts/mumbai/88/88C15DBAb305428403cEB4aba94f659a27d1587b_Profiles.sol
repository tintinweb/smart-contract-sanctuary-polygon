...
contract Profiles is Posts {
  ...
  function setProfile(address sender, CID memory profileMetadata) external {
  ...
  function checkProfile(
    address sender
  ) public view onlyKetlTokenOwners(sender) returns (uint) {
    return 1;
  }
}
...
contract Posts is KetlGuarded {
  ...
  ) external onlyAllowedFeedId(postRequest.feedId) {
  ...
  function pinOrUnpinPost(
    ...
    uint postId,
    bool pin
  ) public onlyAllowedFeedId(feedId) onlyElevatedPriveleges(feedId, sender) {
  ...
  ) external onlyAllowedFeedId(commentRequest.feedId) {
  ...
  ) external payable onlyAllowedFeedId(reactionRequest.feedId) {
  ...
  ) external onlyAllowedFeedId(reactionRequest.feedId) {
    uint feedId = reactionRequest.feedId;
    uint postId = reactionRequest.postId;
    uint commentId = reactionRequest.commentId;
