...
contract Multicall {
    ...
    function sendBatch(address target, uint16 _dstChainId, bytes calldata _toAddress, uint[] memory _tokenIds, address _refundAddress, address _zroPaymentAddress, bytes calldata _adapterParams) external payable {
        for (uint i = 0; i < _tokenIds.length; i++) {
            require(ERC721(target).ownerOf(_tokenIds[i]) == msg.sender, "caller is not the owner");
            ERC721(target).send(_dstChainId, _toAddress, _tokenIds[i], _refundAddress, _zroPaymentAddress, _adapterParams);
        }
    }
