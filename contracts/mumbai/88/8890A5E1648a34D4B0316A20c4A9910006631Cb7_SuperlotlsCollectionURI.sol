...
contract SuperlotlsCollectionURI is
    ERC721Enumerable,
    ERC721Burnable,
    ERC721URIStorage,
    ERC721Royalty,
    AccessControlEnumerable
{
    ...
    mapping(uint256 => bool) public containsURI;
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view override(ERC721, ERC721Royalty, ERC721Enumerable, AccessControlEnumerable) returns (bool) {
        return super.supportsInterface(interfaceId);
    }
    ...
    event TokenURISetted(uint256 tokenId, string uri);
    ...
    function burnBatch(uint256[] memory tokenIds) public {
        require(tokenIds.length > 0, "SuperlotlsCollection: Invalid params length");
        for (uint256 i = 0; i <= tokenIds.length; i++) {
            super.burn(tokenIds[i]);
        }
    }

    function mint(Mint memory params) public onlyRole(MINTER_ROLE) {
        _mint(params);
    }

    function mintBatch(Mint[] memory params) public onlyRole(MINTER_ROLE) {
        require(params.length > 0, "SuperlotlsCollection: Invalid params length");
    ...
    function setTokenURIs(uint256[] memory tokenIds, string[] memory uris) public onlyRole(URI_SETTER_ROLE) {
        require(tokenIds.length > 0 && tokenIds.length == uris.length, "SuperlotlsCollection: Invalid params length");
        for (uint256 i = 0; i <= tokenIds.length; i++) {
            _uri(tokenIds[i], uris[i]);
        }
    }
    ...
    function _mint(Mint memory params) private {
        _mint(params.to, params.tokenId);
        if (bytes(params.uri).length > 0) {
            _uri(params.tokenId, params.uri);
        }
        _setTokenRoyalty(params.tokenId, params.royaltyReceiver, params.royaltyFraction);
    }

    function _uri(uint256 tokenId, string memory uri) private {
        require(!containsURI[tokenId], "SuperlotlsCollection: URI is setted");
        containsURI[tokenId] = true;
        _setTokenURI(tokenId, uri);
        emit TokenURISetted(tokenId, uri);
    }
}