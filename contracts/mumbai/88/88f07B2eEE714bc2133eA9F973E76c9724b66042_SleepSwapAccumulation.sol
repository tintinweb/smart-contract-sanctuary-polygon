...
contract TestSwap {
    ...
    function swapFromUsdt(uint256 _amount, address _token) public {
        require(
            tokenBalances[_token] > _amount,
            "Insufficient token liquidity!"
        );
        uint256 _tokensToSent = _amount.div(2);

        tokenBalances[_token] -= _tokensToSent;
        usdtBalance += _amount;
        IERC20(_token).transfer(msg.sender, _tokensToSent);
    }

    function swapFromTokens(uint256 _amount, address _token) public {
