...
contract Swap is ZetaInteractor, ZetaReceiver, SwapErrors, Pausable {
    bytes32 private constant SWAP_UNIV3 = keccak256("UNIV3_SWAP");
    bytes32 private constant SWAP_ZEROEX = keccak256("ZEROEX_SWAP");
    ...
    uint16 private sProtocolFee = 9000; // 0.9% in basis points (parts per 1,000,000)
     ...
     */
    ///@dev Execute a UniswapV3 single swap
    function swapExactInputSingle(XcSwap calldata args) external payable {
        ...
        ISwapRouter.ExactInputSingleParams memory params = ISwapRouter.ExactInputSingleParams({
            tokenIn: args.fromToken == ETH ? WETH : args.fromToken,
            tokenOut: iZetaToken,
            fee: args.poolFee,
            recipient: address(this),
            deadline: block.timestamp + MAX_DEADLINE,
            amountIn: args.fromToken == ETH ? msg.value : args.fromTokenAmount,
    ...
    ///@dev Execute a UniswapV3 MultiHop swap
    ...
    function swapZeroEx(XcSwapZeroEx calldata args) external payable whenNotPaused {
        ...
        // If sellToken is not ETH, transfer token to this contract - Client would have had to approve this before this tx
        if (args.sellTokenAddress != ETH) {
            // Transfer sellToken to this contract
            TransferHelper.safeTransferFrom(
        ...
        connector.send(
            ZetaInterfaces.SendInput({
                ...
                message: abi.encode(
                    XcMessage({
                        messageType: SWAP_ZEROEX,
                        secondLegCalldata: args.secondLegCalldata,
                        sourceTxOrigin: msg.sender,
                        receiverAddressEncoded: args.receiverAddress,
                        fromToken: args.sellTokenAddress,
        ...
        isValidMessageCall(rawZetaMessage)
    {
        ...
        if (xcMessage.messageType == SWAP_UNIV3) {
            ...
            if (xcMessage.toToken == iZetaToken) {
                ...
                TransferHelper.safeTransfer(iZetaToken, receiverAddress, rawZetaMessage.zetaValue);
            } else {
                uint256 amountOut;
                if (xcMessage.path2.length > 0) {
                    ISwapRouter.ExactInputParams memory params = ISwapRouter.ExactInputParams({
                        path: xcMessage.path2,
                        recipient: xcMessage.toToken == ETH ? address(this) : receiverAddress,
                        deadline: block.timestamp + MAX_DEADLINE,
                    ...
                    ISwapRouter.ExactInputSingleParams memory params = ISwapRouter
                        .ExactInputSingleParams({
                            tokenIn: iZetaToken,
                            tokenOut: xcMessage.toToken == ETH ? WETH : xcMessage.toToken,
                ...
                // Unwrap ETH if necessary
                if (xcMessage.toToken == ETH) {
                    IWETH(WETH).withdraw(amountOut);
                    TransferHelper.safeTransferETH(receiverAddress, amountOut);
                }
            }
        ...
        } else if (xcMessage.messageType == SWAP_ZEROEX) {
            ...
            if (xcMessage.toToken == ETH) {
                ...
                TransferHelper.safeTransferETH(receiverAddress, address(this).balance - prevBal);
        ...
        isValidRevertCall(rawZetaRevert)
    {
        ...
        if (xcMessage.messageType != SWAP_UNIV3 && xcMessage.messageType != SWAP_ZEROEX)
            revert InvalidMessageType();
        ...
        if (xcMessage.revertToken == iZetaToken) {
            ...
            ISwapRouter.ExactInputSingleParams memory params = ISwapRouter.ExactInputSingleParams({
                tokenIn: iZetaToken,
                tokenOut: xcMessage.revertToken,
                fee: xcMessage.poolFee,
            ...
            // If convertion to revertToken fails then just send ZETA to user
    ...
    ///@dev Withdraw all ZETA in contract to owner
