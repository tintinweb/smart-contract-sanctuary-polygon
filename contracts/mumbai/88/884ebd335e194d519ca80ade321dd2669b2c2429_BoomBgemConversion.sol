...
contract BoomBgemConversion is Ownable {
  uint256 boomToBgemRate;
  uint256 dailyMaxAmount;
  ...
  mapping(address => uint48) lastTimeConverted;
  ...
  function changeDailyMaxAmount(uint256 newMaxAmount) external onlyOwner {
    dailyMaxAmount = newMaxAmount;
  }

  function convert(uint256 amount) external {
    if (lastUpdateAt - block.timestamp > 1 days) {
      lastUpdateAt = block.timestamp;
      currentAmount = 0;
    }
    require(currentAmount + amount < dailyMaxAmount, 
      "Maximum daily conversion amount reached");
   
    bgem.transferFrom(msg.sender, studio, amount);
    boom.transferFrom(studio, msg.sender, amount/boomToBgemRate);
  }
}
