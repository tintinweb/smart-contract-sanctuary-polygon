...
contract Lottery {
    uint256 public constant ticketPrice = 0.0005 ether;
    uint256 public constant maxTickets = 1800; // maximum tickets per lottery
    uint256 public constant airdrop = 0.0005 ether; // airdrop per ticket
    ...
    function DrawWinnerTicket() public isOperator {
        ...
        lastWinner = winner;
        winnings[winner] += (tickets.length * (ticketPrice - airdrop));
        lastWinnerAmount = winnings[winner];
        operatorTotalCommission += (tickets.length * airdrop);
        delete tickets;
        expiration = block.timestamp + duration;
    }
