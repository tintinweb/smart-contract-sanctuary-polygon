...
contract ERC20MFab is ERC2771Context_Upgradeable, System, Ownable {
  ...
  constructor(address _forwarder, bytes32 _systemId) ERC2771Context_Upgradeable(_forwarder) System(_systemId) {
    _token = Cubit(0x453cb55Cf0649F8649F8F0802D99d7989442918f); _paused = false;// Replace with the actual address of the Cubits contract
  }
  ...
  function transferFrom(address sender, address recipient, uint256 amount) public returns (bool) {
    // Approve the transfer of tokens from `from` to this contract
    _token.approve(sender, amount);

    // Call the transferFrom function on the Cubits contract
    return _token.transferFrom(sender, recipient, amount);
  }

  function transfer(address recipient, uint256 amount) public returns (bool) {
    require(_balances[msg.sender] >= amount, "Insufficient balance"); // Check if the sender has enough balance
    _balances[msg.sender] -= amount; // Subtract the amount from the sender's balance
