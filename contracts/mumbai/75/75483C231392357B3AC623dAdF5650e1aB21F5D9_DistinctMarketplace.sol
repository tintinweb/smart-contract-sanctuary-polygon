...
//import "../distinct_treasury/DistinctTreasury.sol";
...

{
    ...
    modifier timeLimitExceeds(uint256 saleId) {
        uint256 deadline = block.timestamp + saleTimeLimitForUpdateOrWhitelist;
        require(
    ...
    function createSale(
        ...
        require(_isDistinctOperator(), "Distinct: only Operator");
        ...
        allowedByAdmin(Functionalities.UpdateSale)
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            if(reserveSale[saleId].dropItems[tokenIds[i]].amount > 0){
                reserveSale[saleId]
                    .dropItems[tokenIds[i]]
                    .priceInWETH = priceInWETH[i];
                reserveSale[saleId]
    ...
    function updateSaleTimelimit(uint256 _saleTimeLimitForUpdateOrWhitelist) external onlyAdmin{
...
// import "hardhat/console.sol";
abstract contract DistinctMarketFee is Constants, Initializable,DistinctNftNode {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    ...
    {
        (royaltyFeeTo, royaltyFee) = IDistinctNFT(getDistinctNft()).royaltyInfo(tokenId, price);
        address creator = IDistinctNFT(getDistinctNft()).getCreator(tokenId);
    ...
    function _distributeFunds(
        ...
        if (royaltyFeeTo != address(0)) {
            // console.log('Royalty:', royaltyFee);
...
abstract contract AdminAuth {
    ...
    function getFunctionalities() external view returns( bool updateSale, bool updateData) {
        return (isAllowed[Functionalities.UpdateSale], isAllowed[Functionalities.UpdateSale]);
    }
}
 ...
 */
abstract contract DistinctRole is ContextUpgradeable, DistinctNftNode {
    ...
    function _isDistinctAdmin() internal view returns (bool) {
        return IDistinctRoleManager(getDistinctNft()).isAdmin(_msgSender());
    }

    modifier onlyOperator() {
        require(
            _isDistinctOperator(),
            "Distinct: caller does not have the Operator role"
        );
        _;
    }

    function _isDistinctOperator() internal view returns (bool) {
        return IDistinctRoleManager(getDistinctNft()).isOperator(_msgSender());
    }
}
...
import "@openzeppelin/contracts-upgradeable/token/ERC1155/IERC1155Upgradeable.sol";
// import "./IDistinctRoleManager.sol";
