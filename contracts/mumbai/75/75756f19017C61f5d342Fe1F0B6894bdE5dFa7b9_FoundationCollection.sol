...
contract FoundationCollection is ICollection, AccessControl, Ownable {
  ...
  *
  */
  uint256 public buyBackValue = 10000000000000000000000000; // 1.000.000 EFT 
...
/**
  ...
  *
*/
uint256 public mintBasePrice = 97088534040000000000;
...
/**
  ...
  *
  */
uint16 public totalMintedCountForJoker = 0;
...
/**
 ...
 function depositForFreeMint(uint8 _mintCount, uint256 _value) public  {
    ...
    require(_value  >=  cost, "not enough funds are sent");
...
function commit(bytes32 dataHash, uint8 _amount, Coupon memory _coupon, uint8 _availableAmount, bool _freeMint, uint256 _value) public {
    ...
    if(_value > cost) {
      eft.transfer(msg.sender, _value - cost);
    }
...
uint32[53] memory cardNLQts = [
    ...
    for(uint32 i; i < _amount; i++) {
      ...
      uint32 randomNumber = uint32(uint256(keccak256(abi.encodePacked(blockHash, revealHash, msg.sender, i))) % max);
...
/**
  ...
  function upgradeCards(uint160[] calldata _tokenIds, uint8 _targetCardType, uint8 _collectionId,  Coupon memory _coupon, uint _discount, uint256 _value) public  validCardType(_targetCardType) {
    ...
    require(_value >= upgradecost.payValue, "not enough funds sent");
    ...
    require(upgradecost.highestCardValue <= cardValue[_targetCardType], "cant upgrade to card with less cardValue");
    
    {
      if(upgradecost.upgradeFeeCost > 0){
        // Send upgrade fee to DAO
        gametreasury.upgradeCommission(daoAddress, upgradecost.upgradeFeeCost);
      }
      if(upgradecost.partnerFeeCost > 0){
        // Send upgrade fee to  partner DAO
        sendUpgradeFeeToPartner(upgradecost.upgradeFeeCost, upgradecost.partnerFeeCost, _value);
      }

      if(_value > 0){
       bool success = eft.transferFrom(msg.sender, gameTreasury, _value);
       require(success, "Transfer failed.");
      }
      
      // If user sent more funds than required, refund excess
      if(_value > upgradecost.payValue) {
        bool success = eft.transfer(msg.sender, _value - upgradecost.payValue);
        require(success, "Transfer failed.");
      }
...
function forceRemoveCommit(address _address) external onlyRole(SUPPORT) {
 ...
 function _isVerifiedCoupon(bytes32 _digest, Coupon memory _coupon) internal view returns (bool) {
  ...
  require(signer == couponSigner, 'invalid signer');
