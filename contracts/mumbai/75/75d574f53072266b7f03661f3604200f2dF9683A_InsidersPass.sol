...
contract InsidersPass is InsidersPassAuction {
    ...
    mapping(string => mapping(uint256 => bool))  promoIsValid; 
    
    mapping(string => mapping(uint256 => bool))  promoIsValidForTicket; 
    ...
    mapping(string => PromoCodes) promoCodeDetails;
    ...
    function mintTicketsWithFixPrice(
        ...
        require(ticketTypeDetails[ticketID].isMintable, "Not Mintable");
        require(
            eventDetails[ticketTypeDetails[ticketID].eventIDforTicket]
        ...
        if (_msgSender() != owner()) {
            if (
             ...
             if(promoCodeDetails[_promoCode].isFree){
                require(promoIsValidForTicket[_promoCode][ticketID],"PromoCodeIsNotValidForThisTicket");
                       uint256 newDiscountedAmount = 0;
                        if (
                            promoCodeDetails[_promoCode].maxDiscountAmount >
                            newDiscountedAmount
                        ) {
                            checkPrice(((newDiscountedAmount * noOfCopies)));
                            promoCodeDetails[_promoCode].claimedUsers++;
                        } else {
                            checkPrice(
                                (
                                    (promoCodeDetails[_promoCode]
                                        .maxDiscountAmount * noOfCopies)
                                )
                            );
                            promoCodeDetails[_promoCode].claimedUsers++;
             ...
             if (promoCodeDetails[_promoCode].discountIsPercentage) {
                        uint256 newDiscountedAmount = ticketTypeDetails[ticketID]
                            .ticketFixPrice -
                            ((promoCodeDetails[_promoCode].discountPercentage /
                                100) *
                                (ticketTypeDetails[ticketID].ticketFixPrice));
                        if (
                            promoCodeDetails[_promoCode].maxDiscountAmount >
                            newDiscountedAmount
                        ) {
                        ...
                        uint256 newDiscountedAmount = ticketTypeDetails[ticketID]
                            .ticketFixPrice -
                            (promoCodeDetails[_promoCode].fixedDiscount);
                        if (
                            promoCodeDetails[_promoCode].maxDiscountAmount >
                            newDiscountedAmount
                        ) {
    ...
    function addPromoCodes(
        ...
        for (uint256 i = 0; i < _eventIDs.length; i++) {
         ...
         { 
          ...
          {    
               promoIsValidForTicket[promoCode[0]][_promoCodes[0].tiketIDs] = true; //here is error not storing into mapping
       }
       }
