...
import "../FanNFT721.sol";
import "@openzeppelin/contracts/proxy/Clones.sol";
import "@openzeppelin/contracts/access/Ownable.sol";


contract NaganoFanNFT721Factory is Ownable {
    ...
    constructor() {
        nftImplementation = address(new NaganoFanNFT721());
    }
    ...
    function createCollection(
        ...
        NaganoFanNFT721(clone).initialize(_owner, _name, _symbol);
        emit NFT721Created(clone, _owner, _name, _symbol);
        return clone;
    }
}
...
contract NaganoFanNFT721 is Initializable, ERC721Upgradeable, OwnableUpgradeable {
    ...
    event Minted(uint256 tokenId, address owner, uint256 nonce);

    error InvalidSignatureError();
    error NonceAlreadyUsedError();
    ...
    function safeMint(
        ...
        if (verify(_to, _nonce, _message, _signature) == false) {
        ...
        if (usedNonces[_nonce]) {
            revert NonceAlreadyUsedError();
        }

        emit Minted(tokenId, _to, _nonce);
    ...
    function verify(
        ...
        return recover(ethSignedMessageHash, _sig) == owner();
    }
