...
function getBalanceNew(address ContractAddress) public view returns(uint){
    ...
    function purchaseMembersipInDollar() public payable{
        ...
        int256 oneDollarPrice = (1 * 1e18) / (oneMaticPrice * 1e10);

        emit check(amountCut * uint256(oneDollarPrice), "emitCheck", msg.value);

        require(msg.value == amountCut * uint256(oneDollarPrice), "200$ Required");
