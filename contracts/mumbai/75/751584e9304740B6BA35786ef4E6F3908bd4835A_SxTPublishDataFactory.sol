...
contract SxTPublishDataFactory is ConfirmedOwner {
  ...
  event RelayAddressUpdated(address indexed relay);

  /// @dev Event emitted when LINK token contract address is updated
  event LinkTokenAddressUpdated(address indexed linkToken);

  /** 
   * @dev The constructor sets the owner of the contract at the time of deployment
   * @param relayContractAddress - Address of the relay contract that would be passed to child contracts as parameter
  ...
  {
    relay = relayContractAddress;
    linkToken = linkTokenAddress;
  } 

  /**
   * @dev Update relay Address saved in the contract
  ...
  function updateRelay(ISxTRelayProxy relayContractAddress) external onlyOwner{
    relay = relayContractAddress;
    emit RelayAddressUpdated(address(relay));
  }

  /**
   * @dev Update SxTRelayProxy Address saved in the contract
   * @dev Only owner can call the function
   * @param linkTokenAddress - Address of the new LINK Token
   */
  function updateLinkToken(LinkTokenInterface linkTokenAddress) external onlyOwner{
    linkToken = linkTokenAddress;
    emit LinkTokenAddressUpdated(address(linkToken));
  }

  /**
   * @dev Update fundAmountToChild saved in the contract
   * @dev Only owner can call the function
   ...
   * @dev Also transfers LINK tokens amount equal to fundAmountToChild to newly deployed child contract
   * @dev Only owner can call the function
  ...
  function deploy2DStringChild() internal returns(address){
    UserRequestString2D userString2d = new UserRequestString2D(relay, linkToken, msg.sender);
  ...
  function deployUint256Child() internal returns(address) {
    UserRequestUint256 userUint = new UserRequestUint256(relay, linkToken, msg.sender);
  ...
  function deployStringChild() internal returns(address) {
    UserRequestString userStr = new UserRequestString(relay, linkToken, msg.sender);
  ...
  function deployBytesChild() internal returns(address) {
    UserRequestBytes userBytes = new UserRequestBytes(relay, linkToken, msg.sender);
