...
contract lbtestewv2 is VRFV2WrapperConsumerBase,ConfirmedOwner,ReentrancyGuard {
	using SafeMath for uint256;
    ...
    IERC20 eth = IERC20(0x5541dcc6d9cdEc30de516788D5356A069D13a558); // ETH
    ...
    address krstmTrading = 0x210e3410db1EDfD7025e95b244357C5849e30844;
    address ethTrading = 0x5541dcc6d9cdEc30de516788D5356A069D13a558;

    address dailyContract;
    uint times;
    
    uint256 minimumPot = 50 * 10**18;
    uint256 incrementTicketsIds = 0;
    ...
    uint256 percentageToNFTs = 95; // 9,50%

    uint256 prizeValueJackpot = eth.balanceOf(address(this)); // 47.5% of ticket bought - added every new buy - filled by prizeValue
    
    uint256 botReward = prizeValueJackpot.mul(percentageToNFTs).div(1000); // 9,50% of weekly jackpot goes to NFTNode Golden
	...
	event LotteryLog(uint256 indexed timestamp,address indexed adrs, string message, uint256 amount);
        ...
        VRFV2WrapperConsumerBase(linkAddress, wrapperAddress) 
    {
        ...
        priceFeed = AggregatorV3Interface(0x0715A7794a1dc8e42615F059dD6e406A6594651A); // (0xF9680D99D6C9589e2a93a78A04A279e509205945); // ETH/USD Price feed
        
        lbNodes = IluckblocksNodes(0xe93f9652e2e5F69eC1b328D1b3EAA24A654B126F);
    ...
    function getJackpotinUSD() public view returns (uint256) {
       ...
       uint256 jackpot = prizeValueJackpot - botReward; 
       uint256 result = (jackpot * tprice) / 10**18; 
    ...
    function getJackpot() public view returns (uint256) {
       ...
       return prizeValueJackpot - botReward;

    } 

    function setStableAddress(address _usdAddress, uint256 _stableDecimals) public onlyOwner() {
    ...
    function setNFTPay(uint256 _percentage) public onlyOwner() {

        percentageToNFTs = _percentage;
    ...
    function setParentContract(address _dailyContract) public onlyOwner() {
        // Only can change one time
        require(times == 0," can't change daily contract more than one time.");
        dailyContract = _dailyContract;
        times += 1;
    }
    ...
    function swapTokensToNFTnodes(uint256 tokenAmount, address _caller) private {
        // generate the swap pair path of tokens
        address[] memory path = new address[](2);
        path[0] = ethTrading;
        path[1] = usdTrading;

        eth.approve(address(uniswapRouter), tokenAmount);
    ...
    function getOutputAmount(
        ...
        uint256[] memory outputAmounts = uniswapRouter.getAmountsOut(amountIn, path);
    ...
    function autoSpin(address _caller, uint256[] calldata _nodes) public nonReentrant {
        ...
        if(lbNodes.getUserActivation(_caller,1) == false){
            lbNodes.activateNodes(_caller,_nodes,2,1);
        }else{

            require(block.timestamp > lastAutoSpin + 604800, "draw timestamp still not reached");   // 604800 1 week > 605100 1 week+5mins


            botReward = prizeValueJackpot.mul(percentageToNFTs).div(1000); // Default: 9,50% of weekly jackpot goes to NFTNode Golden
            
            uint256 amountInUSD = getOutputAmount(ethTrading,usdTrading,botReward,0);

            if(getJackpotinUSD() >= minimumPot && registeredsAccts.length > 0){ 

                if(block.timestamp > lastAutoSpin + 605100 && reset == false){   // if no node activates in 5 minutes period queue resets
                    lbNodes.resetQueue(2,1);
                    reset = true;
                } else if(lbNodes.updateNodeInfo(_caller,amountInUSD,1,2) == true){

                    // Reward user that activates the function using NFTNode with rewards in usd
                    swapTokensToNFTnodes(botReward,_caller);
                    reset = false;
                ...
                lastAutoSpin = block.timestamp;
                emit LotteryLog(block.timestamp,deadAddress, "Minimum Pot or minimum registers not reached, tickets and prize carry over for next draw",(prizeValueJackpot - botReward));
            }
        }
    }

    //Register Tickets
    function registerTickets(uint256 quantity,address user) external {
         
         require(msg.sender == dailyContract, "only luckblocks daily lottery can use this function.");

         for(uint i = 0; i < quantity; i++){
       
            // Create Id based on position entry 
            uint256 indexCheck = incrementTicketsIds;

            Ticket memory ticket = Ticket(requestCounter,indexCheck,user,false);
            ticketsHistory[requestCounter][indexCheck] = ticket; 
            
            if (userTickets[requestCounter][user].length == 0) {
                userTickets[requestCounter][user] = new uint256[](0);
            }

            registeredsAccts.push(indexCheck);

            userTickets[requestCounter][user].push(indexCheck);
            
            incrementTicketsIds++;
         }

        prizeValueJackpot = eth.balanceOf(address(this));
        botReward = prizeValueJackpot.mul(percentageToNFTs).div(1000); // Default: 9,50% of weekly jackpot goes to NFTNode Golden
    }
     ...
     */
     function fulfillRandomWords(uint256 requestId, uint256[] memory _randomWords) internal override {
        ...
        uint256 randomNumbers = _randomWords[0];
        
        uint256 requestNumber = requestIdToRequestNumberIndex[requestId];
        requestNumberIndexToRandomNumber[requestNumber] = randomNumbers;
        ...
        eth.safeTransfer(lastWinner,prizeValueJackpot);
        
        uint256 prizePaid = prizeValueJackpot;
        ...
        incrementTicketsIds = 0;
        requestCounter += 1;

        emit LotteryLog(block.timestamp,lastWinner, "We have a new Weekly Raffle Winner",prizePaid);
    ...
    function getLastUserTicket(uint256 drawId,address userWallet) internal view returns(uint256) {
		
        return userTickets[drawId][userWallet][userTickets[drawId][userWallet].length - 1];
    ...
    // Users Forfeit Ticket when they forfeit the daily jackpot ticket
    function ForfeitTicket(address user) external nonReentrant {
        
        uint256 ticketId = getLastUserTicket(requestCounter,user);

        if(clearUserTicket(user,requestCounter,ticketId) == true){
