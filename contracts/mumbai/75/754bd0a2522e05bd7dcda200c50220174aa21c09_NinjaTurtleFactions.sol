...
//           |        |\    
//           |_________\\   
//NFT Contract - Ninja Turtle Factions
⠀⠀⠀⠀⠀⠀⠀⠀ ⠀⠀⢀⣠⣤⣤⣤⣤⣄⡀⠀⠀⠀ ⠀⢠⣤⣄⠀⣀⠀⠀
⠀⠀⠀⠀⠀⠀⠀ ⣠⣴⠟⠛⠉⠁⠀⠀⠈⠉⠛⠻⣦⣄⠀⢸⡟⠙⣿⡟⣷⡀
⠀⠀⠀⠀ ⠀⢠⣾⠏⠁⣀⠀⠀⠀⠀⠀⠀⠀⢀⣀⡈⠻⣷⣼⣧⠀⢹⡇⣹⡇
⠀⠀⠀⠀⣰⡿⠟⠛⢛⣛⣛⡿⢶⣶⣶⡶⢿⣛⣛⡛⠛⠿⢿⣿⣷⣿⣣⡿⠁
...
contract NinjaTurtleFactions is ERC721Enumerable, Ownable, ReentrancyGuard {

    
    using Strings for uint256;
    uint256 public constant maxSupply = 3000;
    uint256 public constant maxMintAmount = 10;
    uint256 public constant whitelistCost = .01 ether;
    uint256 public constant publicCost = .02 ether;
    ...
    constructor() ERC721("Ninja Turtle Factions", "NTF") {}
    ...
    function _getNewId(uint256 _totalMinted) internal returns(uint256 value) {
        ...
        uint256 rand = uint256(keccak256(abi.encodePacked(msg.sender, block.prevrandao, block.timestamp, remaining))) % remaining;
        value = 0;
    ...
    function mint(uint256 numTokens) public payable nonReentrant {
        ...
        require(numTokens <= maxMintAmount, "Exceeds maximum allowed amount");
        require(msg.value >= getMintCost(numTokens), "Invalid amount sent");
        uint256 supply = totalSupply();

        if (_whitelist[msg.sender]) {
            require(msg.value >= whitelistCost * numTokens, "Invalid amount sent");
        } else {
        ...
        function getMintCost(uint256 numTokens) public view returns (uint256) {
        if (_whitelist[msg.sender]) {
            return whitelistCost * numTokens;
        } else {
            return publicCost * numTokens;
        }
    }
