...
contract Market is Context, ERC721Holder, Ownable {
    ...
    function setSallerFee(uint16 _fee) public onlyOwner {
        sellerFee = _fee;
    }
    ...
    function _isListingOnGoing(uint256 _listing) internal view returns (bool) {
        ...
        return block.timestamp < listing.endTime;
    }
    ...
    function makeListing(
        ...
        return _makeListing(_tokenId, endTime, reservePrice);
    }
    ...
    function getListingByTokenId(uint256 _tokenId)
        public
        view
        returns (Listing memory)
    {
        return listings[nftIdToListingId[_tokenId]];
    }
    ...
    function getOffersByNftId(uint256 _tokenID)
        public
        view
        returns (Offer[] memory)
    {
        uint256 offLength = offerListKeys[_tokenID].length();
        Offer[] memory _offers = new Offer[](offLength);
        for (uint256 i = 0; i < offLength; i++) {
            _offers[i] = _offers[offerListKeys[_tokenID].at(i)];
        }
