...
contract Land is Initializable, ERC721EnumerableUpgradeable, OwnableUpgradeable  {
    ...
    uint256 public LAND_PRICE_BBN;
    uint256 public WHITELIST_LAND_PRICE_BBN;
    uint256 public BUSD_BBN_PAIR; //1 busd value by bbn
    ...
    function initialize() public initializer {
        __ERC721_init("Polyversity Lands", "LAND");
        __Ownable_init();
        bbn = IERC20Upgradeable(0x09303a7EDD92c74CCd0f920c4CcC27ab70d68b90); //main
        // busd = IERC20Upgradeable(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); //main
        // bbn = IERC20Upgradeable(0xc39A5f634CC86a84147f29a68253FE3a34CDEc57);
        // busd = IERC20Upgradeable(0xeD24FC36d5Ee211Ea25A80239Fb8C4Cfd80f12Ee);
        setBaseURI("https://ipfs");
        ID_NOT_FOUND = 9999999999999999999;
        //block transaction or  set new land price if argument = ID_SKIP_PRICE_VALUE
        ID_SKIP_PRICE_VALUE = 9999999999999999;
        LAND_PRICE_BBN = 95;
        WHITELIST_LAND_PRICE_BBN = 85;
        BUSD_BBN_PAIR = 1 * decimals(); //1 busd value by bbn
    ...
    function setLandPriceWithBBN(uint256 _price, uint256 _whiteListPrice) public onlyOwner {   
        if (_price != ID_SKIP_PRICE_VALUE) {
            LAND_PRICE_BBN = _price;
        }

        if ( _whiteListPrice != ID_SKIP_PRICE_VALUE) {
            WHITELIST_LAND_PRICE_BBN = _whiteListPrice;
        }
    }
    
    //need to check
    function setBusdBBNPair(uint256 _price) public {
        require(msg.sender == PRICE_UPDATER || msg.sender == owner(), "price updater is not valid.");
        BUSD_BBN_PAIR = _price;
    }
    ...
    function withdrawBBN(address payable addr, uint256 _amount) external onlyOwner {
        SafeERC20Upgradeable.safeTransfer(bbn, addr, _amount);
    }
  ...
  function myCollection(address _owner) public view returns (uint256[] memory)
    ...
    function mintWithBBN(uint256[] memory _tids) public {
        ...
        uint256 totalPrice = calculateTotalPrice(filteredLands, ASSET.BBN);
        require(alreadyMinted + _tids.length < MAX_LAND_COUNT_PER_ACCOUNT && bbn.balanceOf(msg.sender) > totalPrice,  "User has not enough balance.");

        SafeERC20Upgradeable.safeTransferFrom(bbn, msg.sender, address(this), totalPrice);
    ...
    function calculateTotalPrice(uint256[] memory _tids, ASSET _asset) internal view returns(uint256) {
        ...
        if (whiteListAddresses[msg.sender] && whiteListSaleStatus) {
            if (_asset == ASSET.BBN) {
                _price = WHITELIST_LAND_PRICE_BBN * BUSD_BBN_PAIR;
            } 
        } else {
            require(publicSaleStatus, "public sale not opened.");
            if (_asset == ASSET.BBN) {
                _price = LAND_PRICE_BBN * BUSD_BBN_PAIR;
            } 
