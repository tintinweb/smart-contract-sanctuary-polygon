...
contract EthermonStakingData is EthermonStakingBasic {
    ...
    function setContracts(address _emon) public onlyModerators {
        emon = IERC20(_emon);
    }
    ...
    function depositeEmons(
        address _owner,
        uint256 _amount,
        uint256 minDepositeValue,
        uint256 maxDepositeValue
    ) public onlyModerators {
        uint256 balance = emon.balanceOf(_owner);

        require(balance >= minDepositeValue && balance <= maxDepositeValue);

        emon.safeTransferFrom(_owner, address(this), _amount);
    }

    function withdrawEmons(address _owner, uint256 _amount)
        public
        onlyModerators
    {
        require(_amount > 0);
        emon.safeTransfer(_owner, _amount);
    }
