...
contract Presale is Ownable {
	...
	mapping(address => uint256) public investAmounts;
	...
	function setminAmountPerWallet(uint256 _minAmountPerWallet)
		external
		onlyOwner
	{
		minAmountPerWallet = _minAmountPerWallet;
	...
	function investForUSDC(uint256 _amount) external {
		assert(_isInvestable(_amount) == true);
	...
	function investForETH() external payable {
		...
		assert(_isInvestable(usdcAmount));
		_invest(usdcAmount);
	}

	function _invest(uint256 _amount) internal {
		investAmounts[msg.sender] += _amount;
		totalInvestedAmount += _amount;
		emit Invested(msg.sender, _amount);
	}

	function _isInvestable(uint256 _amount) internal view returns (bool) {
		...
		require(
			investAmounts[msg.sender] + _amount <= maxAmountPerWallet &&
				investAmounts[msg.sender] + _amount >= minAmountPerWallet,
