...
contract Institutions is Initializable, PausableUpgradeable, AccessControlUpgradeable {
    ...
    // Courses management
    function registerCourse(string memory _baseURI) public onlyRole(ADMIN_ROLE) {
    ...
    function unregisterCourse(uint256 courseId) public onlyRole(ADMIN_ROLE) onlyActiveCourse(courseId) {
    ...
    function unregisterStudent(uint256 courseId, address student) public onlyRole(ADMIN_ROLE) onlyCertifiedStudent(courseId, student) {
    ...
    // Helper functions
    function setStudentsERC721(address _studentsERC721) onlyRole(ADMIN_ROLE) public {
        require(
    ...
    function getCertificateURI(uint courseId, uint256 certificateURI) internal view onlyRole(ADMIN_ROLE) returns (string memory) {
...
//         _grantRole(DEFAULT_ADMIN_ROLE, 0xaD733B7055eCAebFb3B38626f0148c5d12158F03);
//         _grantRole(PAUSER_ROLE, 0xaD733B7055eCAebFb3B38626f0148c5d12158F03);
//         _grantRole(MINTER_ROLE, 0xaD733B7055eCAebFb3B38626f0148c5d12158F03);
//     }
    
//     // Courses management
//     function registerCourse(string memory _baseURI) public onlyRole(MINTER_ROLE) {
//         courses[courseCounter.current()].activeStudentsBalance = 0;
//         courses[courseCounter.current()].isActive = true;
//         courses[courseCounter.current()].baseURI = _baseURI;
//         courseCounter.increment();
...
//         require(
//             IERC165(_studentsERC721).supportsInterface(INTERFACE_ERC721) || interfaceId == INSTITUTIONS_INTERFACE_ID,
//             "Contract must derive from ERC721"
//         );
//         studentsERC721 = _studentsERC721;
//     }
...
contract StudentsERC721 is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;
    address institution;

    constructor(address _institution) ERC721("StudentsERC721", "STU") {
    ...
    function pause() public onlyOwner onlyInstitution(msg.sender) {
        _pause();
    }

    function unpause() public onlyOwner onlyInstitution(msg.sender) {
        _unpause();
    }

    function safeMint(address to, string memory uri) public onlyInstitution(msg.sender) returns (uint256) {
    ...
    function _burn(uint256 tokenId) internal onlyInstitution(msg.sender) override(ERC721, ERC721URIStorage) {
        super._burn(tokenId);
    }
    ...
    modifier onlyInstitution(address _institution) {
        require(_institution == _institution, "Not institution"); 
        _;
    }
}
