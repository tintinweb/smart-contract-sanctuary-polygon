...
contract MintERC1155 is ERC1155, Ownable, ERC1155Burnable {
    ...
    mapping(uint256 => uint256) allDropTokensAmount;
    ...
    function freeClaimPass(uint256 amount) external {
        require(
            idToDrop[totalDrop].minted +
                idToMintPassDrop[totalMintPassDrop].minted +
                amount <=
                allDropTokensAmount[totalDrop],
            "amount of drop is exceeded"
        );
    ...
    function mintPass(uint256 amount) external {
        ...
        require(
            block.timestamp > idToMintPassDrop[totalMintPassDrop].startTime &&
                block.timestamp <
                idToMintPassDrop[totalMintPassDrop].startTime +
                    idToMintPassDrop[totalMintPassDrop].duration,
            "Not time to mint"
        );
        require(
            idToMintPassDrop[totalMintPassDrop].minted + amount <=
                idToMintPassDrop[totalMintPassDrop].totalSupply,
            "Supply is exceeded"
        );
    ...
    function makeMintPassDrop(
        ...
        require(
            idToMintPassDrop[totalMintPassDrop].startTime +
                idToMintPassDrop[totalMintPassDrop].duration <
                block.timestamp ||
                idToMintPassDrop[totalMintPassDrop].startTime == 0,
            "previous drop didn't finish"
        );
        totalMintPassDrop++;

        idToMintPassDrop[totalMintPassDrop] = drop(
            amount,
            0,
            startTime,
            duration,
    ...
    function makeDrop(
        ...
        uint256 duration,
        uint256 cost_,
        uint256 allDropTokensAmount_
    ) external onlyOwner {
        require(
            idToDrop[totalMintPassDrop].startTime +
                idToDrop[totalMintPassDrop].duration <
                block.timestamp ||
                idToDrop[totalMintPassDrop].startTime == 0,
            "previous drop didn't finish"
        );
        totalDrop++;
        idToDrop[totalDrop] = drop(amount, 0, startTime, duration, cost_);
        cost = cost_;
        allDropTokensAmount[totalDrop] = allDropTokensAmount_;
    }
    ...
    {
        return (nftAmountPerUser - publicMinted[msg.sender], cost);
    }
    ...
    function withdraw() public onlyOwner returns (uint256) {
        ...
        require(
            token.transfer(getOwner(), token.balanceOf(address(this))),
