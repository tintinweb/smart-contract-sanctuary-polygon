...
import "./interfaces/IBetLiquidityPool.sol";
import "./interfaces/IBookieMain.sol";
import "./interfaces/IBetHistory.sol";
import "./interfaces/IMarketHistory.sol";
...
//import "@openzeppelin/contracts/utils/math/SafeMath.sol"; // Also imported in BetData
// For debugging only
...
contract BookieMain is IBookieMain, AccessHandler {
    ...
    IBetLiquidityPool private lp;
    ILockBox private betLockBox; // Just used as an address
    IBetHistory private betHistory;
    IMarketHistory private marketHistory;
    ITokenTransferProxy private tokenTransferProxy;

    /**
     * @notice Event fires when bets for 0 tokens is attempted
     * @param better is the better address
 ...
 */
     ...
     * @param _lp The Liquidity Pool contract address, for matching bets.
     * @param _betLockBox The bet LockBox contract address.
     * @param _betHistory The bet history contract address for storing bets.
     * @param _marketHistory The market history contract for storing markets.
     * @param _tokenTransferProxy The TokenTransferProxy contract  address.
     */
    function init(
        IBetLiquidityPool _lp,
        ILockBox _betLockBox,
        IBetHistory _betHistory,
        IMarketHistory _marketHistory,
    ...
    {
        lp = _lp;
        betLockBox = _betLockBox;
        betHistory = _betHistory;
    ...
    function createBet(BetData.Bet memory bet) private {
        if (bet.amount == 0) {
            emit BetZero(bet.owner, bet.token);
            return;
        }
        ...
        (
            ...
            uint256 available
        ) = TokenAmountValidator.checkAllowanceAndBalance(
            bet.amount,
            bet.token,
            bet.owner,
        ...
        uint256 matchedAmount = bet.amount.mul(bet.decimalOdds).div(BetData.ODDS_PRECISION).sub(bet.amount);
        if (matchedAmount == 0)
            revert InvalidBet({reason: "0_MATCHED", bet: bet});
        ...
        // Transfer the betters tokens first
        ...
        //Now match, persist, and token-lock the bet
     ...
     */
    function settleBet(bytes32 betHash) external override {
        ...
        console.log("Pay to better:", res.paidToBetter.div(1 ether));
        console.log("Return to LP:", res.paidToLP.div(1 ether));
        console.log("Paid fee:", res.paidToFee.div(1 ether));
...
interface IBetLiquidityPool is ILiquidityPool, IBetHelper {
}
...
interface ILiquidityBox {
    ...
    function getUserDistTokenBalanceFromToken(address, address)
        external
        view
        returns (uint256);
...
library BetData {
        ...
        returns (string memory)
    {
        ...
        if (bet.decimalOdds == 0 || bet.decimalOdds > ODDS_PRECISION.mul(MAX_ODDS)) {
    ...
    function logBet(Bet storage bet) internal view {
        ...
        console.log("Market hash:", Strings.toHexString(uint256(bet.marketHash), 32));
        console.log("Token", bet.token);
        console.log("Amount:", bet.amount.div(1 ether));
        console.log("Odds:", bet.decimalOdds.div(ODDS_PRECISION));
        console.log("Expiry:", bet.expiry);
        console.log("Owner:", bet.owner);
    }
}
 ...
 */
abstract contract AccessHandler is IAccessHandler, BaseInitializer, AccessControl {
