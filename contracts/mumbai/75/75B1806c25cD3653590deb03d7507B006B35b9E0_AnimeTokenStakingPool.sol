 ...
 *             _
 *            |  0 (i=0)
 *      ΔT := |  ith period between Stake/Unstake Event (i = 1 .. n-1)
 *            |_ seconds from the last event (i = n)
 *      R - rewardAmount
 *      D - rewardDuratoin
 ...
 *        x(Ti) changes at t = Tk
 *        Ri, Di as fixed as R,D
 *   Then,
 ...
 *                  + x(Tk1) * (RPT(k2) - RPT(k1))
 *                  ... + x(Tk_{m}) * (RPT(n) - RPT(k_{m}))
 *
 * PROOF:
 *  Apply x(t) func to be constant and move it outside of SUM().
 ...
 *      rewards[user] is reset when user claimed
 *      RPT_k is stored as rewardPerTokenPayed[user]
 *
 * DEFINITION: Average Total Supply
 *   ATS(t) := SUM((S(Ti)*ΔTi) | i = 1..n) / (t - initialTime)
 *    S(Ti) - total supply at ΔTi after initial time
 *
 *   ATS(a,t) / SUM(ATS(a,t)| a = i ... A)
 *   ATS(a, t) := SUM((S(a,Ti)*ΔTi) | i = 1..n) / (t - initialTime)
 *
 * Comment:
 *   We watch it to notify RewardAmount and rewardDuration according our token schedule
 */
...
contract AnimeTokenStakingPool is Ownable {
    ...
    modifier updateReward(address account) {
        ...
        if (account != address(0)) { // called by user
    ...
    //  https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v4.6.0/contracts/finance/VestingWallet.sol
    function claimStudioReward() external {
      require (msg.sender == animeStudio);
      require(initialize == true, "AnimeBankStakingPool: not initialized");
      uint256 releasable = _vestingSchedule() - studioRewardPayed;
      studioRewardPayed += releasable;
      IERC20(stakingToken).transfer(animeStudio, releasable);
    }
    ...
    function buyNft() external updateReward(msg.sender) {
        // count up type to avoid to buy same items
        uint256 productType = userProductType[msg.sender] + 1;
        userProductType[msg.sender] = productType;
        require(
            productType < 1e4 &&
        ...
        // reward should rearch to min reward
        ...
        require(
            price <= reward, "AnimeBankStakingPool: price is higher than reward"
        );
    ...
    function getAverageTotalSupply() external view returns (uint256) {
        return
            getLatestCumulativeTotalSupply() / (block.timestamp - initialTime);
    }
...
/*
 *  This source code is inspired from GensoMetaStakingPool (0x1b2430aeedececafb52a3ff8cc8321e9426fc82c),
 *  redesigned for Animebank.
 *  Major difference is are below:
 *     - solc version
 *     - Anime Token is used only here
 *     - NFT is claimable using reward amount
 *     - Unstake and Claim methods are private
 *     - Anime studio is also claimable
 *     - Owner cannot recover anime token from
 *     - Average total supply defined below can be watched
 */
