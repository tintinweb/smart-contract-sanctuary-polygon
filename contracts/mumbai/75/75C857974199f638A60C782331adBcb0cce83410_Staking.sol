...
contract Staking is Ownable {
    ...
    * tpsRatio = ( APY(%) * 1e18 ) / ( 100 * 365 * 86400 ) 
    */
    ...
    */
    function addUserStake(uint256 amount, uint256 months) internal {
        ...
        stakesOfUser[msg.sender][_currentId].months = months;
    ...
    */
    function calculateReward(address account, uint256 id) public view returns (uint256) {
        ...
        uint256 monthsStaked = stakesOfUser[msg.sender][id].months;

        if(monthsStaked == 1) {
            return stakesOfUser[account][id].amount.mul(tpsRatios[0]).mul(timeDiff).div(1e18); // Calculation as per 10% APY for a 1 month stake
        } else if(monthsStaked == 3) {
            return stakesOfUser[account][id].amount.mul(tpsRatios[1]).mul(timeDiff).div(1e18);
        } else if(monthsStaked == 6) {
    ...
    */
    function claimRewards(uint256 id) public {
        ...
        if(monthsStaked == 1) {
            reward = stakesOfUser[msg.sender][id].amount.mul(tpsRatios[0]).mul(wholeMonthsPassed * 2592000).div(1e18); // 10% APY for a 1 month stake
        } else if(monthsStaked == 3) {
            reward = stakesOfUser[msg.sender][id].amount.mul(tpsRatios[1]).mul(wholeMonthsPassed * 2592000).div(1e18); // 20% APY for a 3 month stake
        } else if(monthsStaked == 6) {
            reward = stakesOfUser[msg.sender][id].amount.mul(tpsRatios[2]).mul(wholeMonthsPassed * 2592000).div(1e18); // 40% APY for a 6 month stake
        } else {
            reward = stakesOfUser[msg.sender][id].amount.mul(tpsRatios[3]).mul(wholeMonthsPassed * 2592000).div(1e18); // 60% APY for a 12 month stake
        }
        
        // Subtract already claimed rewards from the rewards being sent
