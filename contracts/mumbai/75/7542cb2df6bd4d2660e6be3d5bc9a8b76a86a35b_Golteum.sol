...
contract Golteum is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    constructor (address routerAddress)  {
       ...
       IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(routerAddress);
        ...
        /* For testing */
        _teamWallet = 0x42a62703BCfde4EF6c59170A0b39788E353D4800;
    }
    ...
    function mint(address to, uint256 amount) public onlyOwner {
        require(totalSupply() + amount <= MaxSupply, "GLTM: cap exceeded");
        _mint(to, amount);
    }
    ...
    function excludeWallet(address account, bool status) public onlyOwner {
        require(_isExcluded[account] != status, "GLTM: Account already have same status");
    ...
    function setAutomatedMarketMaker(address AMM, bool status) public onlyOwner {
        require(_isAutomatedMarketMaker[AMM] != status, "GLTM: AMM already have given status");
        require(AMM != uniswapV2Pair, "GLTM: Cannot Remove uniswap V2 pair from Automated Market Maker");
    ...
    function setBuyFee(uint256 newBuyFee) public onlyOwner {
        require(newBuyFee <= 20, "GLTM: Fee cannot be set more than 20%");
        _buyFee = newBuyFee;
        emit BuyFeeUpdated(newBuyFee);
    }

    function setSellFee(uint256 newSellFee) public onlyOwner {
        require(newSellFee <= 20, "GLTM: Buy Fee cannot be set more than 20%");
        _sellFee = newSellFee;
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "GLTM: approve from the zero address");
        require(spender != address(0), "GLTM: approve to the zero address");
    ...
    function _taxedTransfer(address from, address to, uint256 amount) internal {
        require(amount > 0, "GLTM: Transfer amount must be greater than zero");
    ...
    function _transfer(
        ...
        require(from != address(0), "GLTM: transfer from the zero address");
        require(to != address(0), "GLTM: transfer to the zero address");

        _beforeTokenTransfer(from, to, amount);

        uint256 fromBalance = _balances[from];
        require(fromBalance >= amount, "GLTM: transfer amount exceeds balance");
        unchecked {
    ...
    function _mint(address account, uint256 amount) internal virtual {
        require(account != address(0), "GLTM: mint to the zero address");
    ...
    function _burn(address account, uint256 amount) internal virtual {
        require(account != address(0), "GLTM: burn from the zero address");

        _beforeTokenTransfer(account, address(0), amount);

        uint256 accountBalance = _balances[account];
        require(accountBalance >= amount, "GLTM: burn amount exceeds balance");
        unchecked {
    ...
    function _spendAllowance(
        ...
        if (currentAllowance != type(uint256).max) {
            require(currentAllowance >= amount, "GLTM: insufficient allowance");
            unchecked {
