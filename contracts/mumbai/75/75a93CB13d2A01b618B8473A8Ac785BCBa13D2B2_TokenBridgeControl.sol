...
contract TokenBridgeControl is Ownable{
    ...
    function getBridgeFee(address polyToken, uint256 amount) public view returns (uint256,address) {
        ...
        if (ethUnit.mul(25).div(100).mul(10**IERC20(polyToken).decimals()).div(price) < fee_amount){//>0.25e
            return (ethUnit.mul(25).div(100).mul(10**IERC20(polyToken).decimals()).div(price), bridgeFeeVault);
        } else if (ethUnit.div(100).mul(10**IERC20(polyToken).decimals()).div(price) > fee_amount){//<0.01e
            return (ethUnit.div(100).mul(10**IERC20(polyToken).decimals()).div(price), bridgeFeeVault);
        } else {
