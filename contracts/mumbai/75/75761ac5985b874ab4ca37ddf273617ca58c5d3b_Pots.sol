...
contract DividendDistributor is IDividendDistributor {
    ...
    address public immutable PEPE =
        address(0x6972EEDb2f553d8054adD8e0258F450eAb8529B3); //UNI
...
contract Pots is ERC721, ERC721Enumerable, ERC721URIStorage, ERC2981, Ownable {
    ...
    // price per token is 0.1 ETH - 0.01 TESTNET
    uint256 public ETH_PRICE = 10000000000000000;

    // price per token is 5.000000e+8 PEPE - 5.000000e+4 TESTNET
    uint256 public PEPE_PRICE = 5000000000000000000000000;

    // max number of tokens to mint
    uint256 public constant MAX_TOKENS = 100;

    // max number of tokens to mint per transaction
    uint256 public constant MAX_TOKENS_PER_TX = 5;

    // max number of tokens to mint per wallet
    uint256 public constant MAX_TOKENS_PER_WALLET = 20;
    ...
    address public immutable PEPE_TOKEN_ADDRESS =
        address(0x6972EEDb2f553d8054adD8e0258F450eAb8529B3);

    // due to the fact that not all marketplaces support royalty
    // we only support approving a few marketplaces
    ...
    constructor(
        ...
        string memory tokenPrefix_,
        string memory tokenExtension_
    ) ERC721(name_, symbol_) {
        base = baseUri_;
        tokenPrefix = tokenPrefix_;
        tokenExtension = tokenExtension_;
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        require(
            _approvedMarketplaces[to],
            "Invalid Marketplace or we don't support this marketplace"
        );
    ...
    function setApprovedMarketplace(address market, bool approved)
        public
        onlyOwner
    {
    ...
    function airdrop(address[] memory wallets) public onlyOwner {
    ...
    function setShare(address sender) internal {
    ...
    function _mintNFTsWithETH(uint256 numberOfTokens) public payable {
        require(
        ...
        // use 30% of the ETH to buyback PEPE
        uint256 ethToSwap = msg.value.mul(30).div(100);
        swapBack(ethToSwap);

        // send the remaining ETH to the owner
        uint256 ethToOwner = msg.value.sub(ethToSwap);
        payable(owner()).transfer(ethToOwner);

        setShare(sender);
    }

    function _mintNFTsWithPEPE(uint256 numberOfTokens) public payable {
        address sender = msg.sender;
        uint256 pepeAmount = PEPE_PRICE.mul(numberOfTokens);
        require(
            pepeContract.balanceOf(sender) >= pepeAmount,
        ...
        // send to contract
        pepeContract.transferFrom(sender, address(this), pepeAmount.div(2));

        // send to owner
        pepeContract.transferFrom(sender, owner(), pepeAmount.div(2));

        setShare(sender);
    }

    function mintNFTs(uint256 numberOfTokens, bool buyWithPepe) public payable {
        ...
        if (buyWithPepe) {
            _mintNFTsWithPEPE(numberOfTokens);
        } else {
            _mintNFTsWithETH(numberOfTokens);
        }
    }
    ...
    function swapETHForPEPE(uint256 ethAmount) private {
        address[] memory path = new address[](2);
        path[1] = WETH;
        path[2] = PEPE_TOKEN_ADDRESS;
