...
contract EthermonMonsterHelper is Ownable {
    ...
    uint256 public currentTournament = 0;
    ...
    function enrollPlayer(address addr, uint256 tourneyNumber, uint256 amount)
        public
        payable
        {
            IERC20 tc = IERC20(tokenContract); 
            require(timers[currentTournament] > 0, "No Tourney Found");
            require(timers[currentTournament] < block.timestamp, "Tournament Already Started");
            require(amount == buyIn, 'Not enough funds');
            require(!enrolledPlayers[tourneyNumber][addr], "Already Enrolled");
            tokenPool[tourneyNumber] += buyIn-(buyIn/10);
            ownerBalance += buyIn/10;
            enrolledPlayers[tourneyNumber][addr] = true;
            tc.transferFrom(msg.sender, address(this), amount);
        }
