...
contract TestContract {
    ...
    function withdraw() external {
        ...
        IDiscount discount_ = IDiscount(address(0x10CDCBc103EB40B467f4C47deEcD420a63164772));

        // uint256 _fees;
        // address _feeRecipient;
        // (_feeRecipient, _fees) = _decir.primarySaleFeeRecipient();
        if(discount_.isDiscountApplied(msg.sender, 0)) {
            payable(msg.sender).transfer(address(this).balance);
        } else { 
            uint256 commission = calculateFee(balance, 500); 

            payable(0xEab2B6b5a76d5878a7B5D97d7F6812Da09A30953).transfer(commission);
            payable(msg.sender).transfer((balance - commission));
        }
    }
