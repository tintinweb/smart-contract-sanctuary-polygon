...
contract MultipleNFT is ERC1155, Pausable, ReentrancyGuard {
    address owner;
    ...
    modifier isPriceMatch(
        uint256 value,
        uint256 onePrice,
        uint256 _length
    ) {
        require(value >= onePrice * _length, "value not enought");
        _;
    }
    ...
    event BuyMultipleNFT(address to, uint256[] ids, uint256 price);
    event BuyNFT(address to, uint256 ids, uint256 price);
    ...
    function buyMultipleNFT(
        ...
        bytes memory data,
        uint256 price
    ) external payable isPriceMatch(msg.value, price, amounts.length) {
        _mintBatch(to, ids, amounts, data);

        emit MintBatchNFT(to, ids, amounts, data);
        emit BuyMultipleNFT(to, ids, price);
    }
    ...
    function buyNFT(
        ...
        emit MintNFT(to, ids, amounts, data);
        emit BuyNFT(to, ids, price);
    }
