...
contract unknownpro {
	...
	uint constant public			MAX_WITHDRAW_AMOUNT				= 100000000 ether;			// claim 10000 MATIC max
	...
	uint constant public			GUARD_SAFETYBALANCE_PERCENT	= 250;					// protection kicks in at 25% or lower
	uint constant public			REINVEST_PERCENT			= 100;					// auto reinvest 10% of claim
    uint constant public			SAFETYBALANCE_REINVEST_PERCENT	= 300;					// auto reinvest 30% of claim when low balance
	...
	uint public GUARD_SAFETYBALANCE;
	...
	function withdraw() public {
		...
		//reinvest
        uint reinvestAmount;
        if (_isSafetyBalanceGuarded()) { // protect balance empty
		    reinvestAmount = claimAmount * SAFETYBALANCE_REINVEST_PERCENT / PERCENTS_DIVIDER;
        } else {
	...
	function _isSafetyBalanceGuarded() internal returns(bool) {
		...
		GUARD_SAFETYBALANCE = rangeHigh*GUARD_SAFETYBALANCE_PERCENT/PERCENTS_DIVIDER;
		return balance < GUARD_SAFETYBALANCE;
	}
	...
	function getProjectInfo() public view returns(uint o_totDeposits, uint o_totInvested, uint o_totRefDividends, uint o_totClaimed, uint o_balance, uint o_guardSafetyBalance, uint o_totUsers, uint o_timestamp) {
		return( TOTAL_DEPOSITS, TOTAL_INVESTED, TOTAL_REFDIVIDENDS, TOTAL_CLAIMED, address(this).balance, GUARD_SAFETYBALANCE, TOTAL_USERS, block.timestamp );
	}
