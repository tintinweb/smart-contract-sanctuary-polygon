...
contract WeatherStaticNFT is FunctionsClient, ConfirmedOwner {
  ...
  CBOR.CBORBuffer private buffer;
  ...
  ) public pure returns (CBOR.CBORBuffer memory) {
  ...
  function setRequest(
    uint64 _subscriptionId,
    uint32 _fulfillGasLimit,
    CBOR.CBORBuffer memory newBuffer
  ) external onlyOwner {
    subscriptionId = _subscriptionId;
    fulfillGasLimit = _fulfillGasLimit;
    buffer = newBuffer;
    requestCBOR = newBuffer.buf.buf;
  }

  function meowintNFT(string memory zipcode, string memory countryCode) public {
    string[2] memory args = [zipcode, countryCode]; 
    if (args.length > 0) {
      CBOR.writeString(buffer, "args");
      CBOR.startArray(buffer);
      for (uint256 i = 0; i < args.length; i++) {
        CBOR.writeString(buffer, args[i]);
      }
      CBOR.endSequence(buffer);
    }
    bytes32 requestId = s_oracle.sendRequest(subscriptionId, buffer.buf.buf, fulfillGasLimit);
...
library Functions {
  ...
  function encodeCBOR2(Request memory self) internal pure returns (CBOR.CBORBuffer memory) {
    CBOR.CBORBuffer memory buffer;
    Buffer.init(buffer.buf, DEFAULT_BUFFER_SIZE);
