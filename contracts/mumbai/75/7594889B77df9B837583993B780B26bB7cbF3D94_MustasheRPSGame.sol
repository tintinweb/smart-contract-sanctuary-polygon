...
contract MustasheRPSGame {
    ...
    function endGame(uint256 _gameId) private {
        ...
        token.transfer(winner, reward / 1e18);
        token.transfer(address(this), fee / 1e18);
    }
    ...
    function forceEndGame(uint256 _gameId) external {
        ...
        token.transfer(game.creator, refundAmount / 1e18);
        token.transfer(game.opponent, refundAmount / 1e18);
    }
