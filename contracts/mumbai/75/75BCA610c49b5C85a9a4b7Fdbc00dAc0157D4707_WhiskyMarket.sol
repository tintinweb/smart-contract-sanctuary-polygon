    ...
    NFTMarketAuction
{
    ...
    function getUserTokensOnSale(address userAddress, address nftContract) public view returns(uint256[] memory) {
        uint256[] memory tokenList = new uint256[](
            userTokensOnSale[userAddress][nftContract].length
        );

        for (uint i = 0; i < userTokensOnSale[userAddress][nftContract].length; i++) {
            if(IERC721Upgradeable(nftContract).ownerOf(userTokensOnSale[userAddress][nftContract][i]) == address(this))
