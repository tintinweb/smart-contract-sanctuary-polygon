...
contract Trade {
    ...
    constructor (uint8 _buyerFee, uint8 _sellerFee, TransferProxy _transferProxy, address _wmatic) {
    ...
    function tradeAsset(Order calldata order, Fee memory fee, address buyer, address seller) internal virtual {
        ...
        if(order.erc20Address == wmatic) { 
            payOut(order.buyer, order.amount);
        }
        if(fee.platformFee > 0) {
            order.erc20Address != wmatic ? transferProxy.erc20safeTransferFrom(IERC20(order.erc20Address), buyer, owner, fee.platformFee)
            : payable(owner).transfer(fee.platformFee);
        }
        if(fee.royaltyFee > 0) {
           order.erc20Address != wmatic ? transferProxy.erc20safeTransferFrom(IERC20(order.erc20Address), buyer, fee.tokenCreator, fee.royaltyFee)
            : payable(fee.tokenCreator).transfer(fee.royaltyFee);
        }
        order.erc20Address != wmatic ? transferProxy.erc20safeTransferFrom(IERC20(order.erc20Address), buyer, seller, fee.assetFee)
        : payable(seller).transfer(fee.assetFee);
    }

    function payOut(address buyer, uint256 amount) internal {
        transferProxy.erc20safeTransferFrom(IERC20(wmatic), buyer, address(this), amount);
