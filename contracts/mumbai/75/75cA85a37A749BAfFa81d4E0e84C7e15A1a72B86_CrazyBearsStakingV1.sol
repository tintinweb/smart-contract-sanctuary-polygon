...
contract CrazyBearsStakingV1 is ERC1155Holder, IERC721Receiver, Ownable, ReentrancyGuard {
    ...
    function _sendReward(uint256 _tokenID) nonReentrant internal onlyOwner {
        uint fieldId = tokenById[_tokenID].fieldId;
        address account = tokenById[_tokenID].user;
        StakingField memory field = fields[fieldId];
        uint rewardId = field.rewardId;
        FieldReward memory reward = rewards[rewardId];
        uint rewardTokenId = reward.tokenId;
        ...
        rewardContract.safeTransferFrom(address(this), account, rewardId, earned, '');
        tokenById[_tokenID].since = block.timestamp;
    }

    function ownerUnstakeAllTokens(uint _fieldId) public onlyOwner {      
        for (uint index = 0; index < 10000; index ++){
            uint tokenId = index + 1;
            if (isStaked(tokenId)) {
                StakedToken storage staking = tokenById[tokenId];
                if (staking.fieldId == _fieldId) {
                    _sendReward(tokenId);
                    address account = staking.user;
                    stakeableContract.safeTransferFrom(address(this), account, tokenId); 
                    staking.user = address(0);
                }
            }
        }
