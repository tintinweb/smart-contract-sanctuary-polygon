...
contract SlapSamGame is ERC721, Pausable, Ownable{
...
event SetMinterEvent(address indexed account, address minter, bool status);
    constructor(address payable _ownerAddress,address _rewardWallet) ERC721("SlapNft", "SNM")
    {
    ...
    function getCurrentNftPrice() public view returns (uint256 _amount)
    {
        ...
        for(uint256 i = 0; i<=9; i++)
        {
            ...
            {
                return MintingLevels[i].amount;

            }
        }
    }

    function adminBuyNft(uint256 noOfBuyNft) public payable {

        require(msg.sender == ownerAddress,"Only admin can buy this nft's");

        require(adminTotalBuyCount < maxMintAdminNFT,"Admin can buy upto 1000 nft's");

        require(nftId < totalNFTQuantity,"Already sold all nft");

        uint256 count;
        if(noOfBuyNft <= maxMintAdminNFT-adminTotalBuyCount)
        {
            count = noOfBuyNft;
        }
        else
        {
            count = maxMintAdminNFT-adminTotalBuyCount;
        }
    ...
    function buyNft(uint256 noOfBuyNft) public payable {
        ...
        uint256 count;
        if(noOfBuyNft <= maxMintNFTPerWallet-totalNFTQuantity)
        {
            count = noOfBuyNft;
        }
        else
        {
            count = maxMintNFTPerWallet-totalNFTQuantity;
        }

         for(uint256 index=1;index<=count;index++)
        {

        uint256 _amount = getCurrentNftPrice();
...
function _baseURI() internal pure override returns (string memory) {
        return "https://tronwing.io/moonsea/api/api/Metadata/";
    }
...
function mintReward(address to,uint256 nftPrice) internal {
       // require(isMinter[_msgSender()] == true,"Transfer caller is not owner nor approved");
        ...
        nftDetails[nftId] = _NftStruct;

        emit NftMintEvent(nftId,block.timestamp,nftPrice,to);
    }
