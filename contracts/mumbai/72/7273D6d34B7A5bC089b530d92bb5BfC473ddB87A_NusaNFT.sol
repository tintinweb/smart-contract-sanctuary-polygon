 ...
 *
 */
contract NusaNFT is ERC1155, ContractMetadata, Ownable, Multicall, BatchMintMetadata {
    ...
    uint256 internal nextTokenId_ = 1;
    ...
    /**
    * @dev Require msg.sender to be owner
    */
    modifier ownerOnly() {
        require(owner()== msg.sender, "NusaNFT#creatorOnly: ONLY_CREATOR_ALLOWED");
        _;
    }
    ...
    */
    ...
    function create(address _to, string memory _tokenURI) public ownerOnly  {
        uint256 currentTokenId;
        uint256 nextNextTokenId = nextTokenIdToMint();
        currentTokenId = nextNextTokenId;
        nextTokenId_ += 1;
        _creators[nextNextTokenId] = _to;
        _setTokenURI(nextNextTokenId, _tokenURI);
    }
    ...
    /// @notice The tokenId assigned to the next new NFT.
    ...
    function claimVoucher(string calldata voucher, uint256 _tokenId, uint exp, address _to, bytes calldata sig, bytes32[] calldata merkleProof) public onlyOwner{
        _verifyVoucher(voucher, _tokenId, exp, merkleProof);
        mintTo(
            _validateClaimAddress(voucher, _tokenId, _to, sig), 
            _tokenId, "", 1);
    }

    function registerVoucher(uint256 _tokenId, bytes32 rootHash ) public {
        require(_creators[_tokenId] == msg.sender || owner() == msg.sender, "Only creator and ");
        _rootHashVoucher[_tokenId] = rootHash;
    }

    function isValidVoucher(string memory voucher, uint256 _tokenId, uint exp, bytes32[] calldata proof) public view returns(bool){
        return _verifyVoucher(voucher, _tokenId, exp,  proof);
    }
    ...
    function _verifyVoucher(string memory voucher, uint256 _tokenId, uint exp, bytes32[] calldata proof) internal view returns(bool){
        require(exp >= block.timestamp, "Voucher expired");
        require(_creators[_tokenId] != address(0x0), "Token not valid" );
        bytes32 hashVoucher = keccak256(abi.encodePacked(voucher, _tokenId, exp));
        require(!_isUsed[hashVoucher], "Voucher already use");
        bytes32 computedHash = hashVoucher;
    ...
    function _validateClaimAddress(
        ...
        uint256 _tokenId, 
        address _to, 
        bytes memory sig)internal view returns(address)
    {
       
        // require( getSignatureAddress(voucher, _tokenId, _to, sig) == owner(), "Address not valid");
        require( getSignatureAddress(voucher, _tokenId, _to, sig) == _to, "Address not valid");
        return _to;
    }
    ...
    {
        bytes32 hash = keccak256(abi.encodePacked(voucher, _tokenId, _to));
