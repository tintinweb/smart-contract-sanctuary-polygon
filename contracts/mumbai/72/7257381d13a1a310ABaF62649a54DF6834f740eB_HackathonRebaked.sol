...
contract HackathonRebaked is IHackathon, Ownable, ReentrancyGuard {
    ...
    function sendPrizes(
        ...
        for (uint16 index = 0; index < _arrId.length; index++) {
            ...
            if(localPrize.token == address(0)){
                TransferHelper.safeTransfer(fundReceiver, adminFee);
                TransferHelper.safeTransfer(winnerAddress, localPrize.value - adminFee);
            }
            else{
                IERC20(localPrize.token).safeTransfer(fundReceiver, adminFee);
    ...
    function close(string memory _hackathonId) external override onlyAdmin existHackathon(_hackathonId) {
        Hackathon memory tmpHackathon = getHackathonInfo(_hackathonId);
    ...
    function setFundReceiver(address newReceiver) external override onlyOwner {
        require(newReceiver != address(0), "Invalid receiver");
        fundReceiver = newReceiver;
        emit SetFundReceiver(newReceiver);
    }
    ...
    // Set an account to be contract admin
    ...
    function setFee(uint256 _newFee) external override onlyAdmin {
    ...
    function setTime(uint256 _newTime) external override onlyAdmin {
...
interface IHackathon {
    ...
    event SetFundReceiver(address newReceiver);
    ...
    function setFundReceiver(address newReceiver) external;
