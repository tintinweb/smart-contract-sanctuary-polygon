...
contract sUSDR is AddressAccessor, ERC20Permit {
    ...
    function burn(address account, uint256 amount) external {
        ...
        if (msg.sender != account) {
            _allowedFragments[account][msg.sender] -= amount;
        }

        uint256 scaledAmount = scaledBalanceForBalance(amount);
        uint256 scaledBalance = _gonBalances[account];

        require(scaledBalance >= scaledAmount, "burn amount exceeds balance");

        unchecked {
            _gonBalances[account] = scaledBalance - scaledAmount;
        }
    ...
    _gonBalances[account] += scaledBalanceForBalance(amount);
        ...
        returns (uint256)
    {
        ...
        _gonsPerFragment = TOTAL_GONS / totalSupply_;
    ...
    function transferAll(address to) external returns (bool) {
        uint256 scaledAmount = _gonBalances[msg.sender];
        delete _gonBalances[msg.sender];
        _gonBalances[to] += scaledAmount;
    ...
    {
    return scaledAmount / _gonsPerFragment;
    }
    ...
    function transferAllFrom(address from, address to) public returns (bool) {
        ...
        uint256 amount = gonValue / _gonsPerFragment;
        if (from != msg.sender) {
