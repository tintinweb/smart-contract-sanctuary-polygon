...
//        missionEventsHandler = MissionEventsHandler(_missionEventsHandlerAddr);
    ...
    function contractURI() public view returns (string memory) {
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, '/storefront')) : "";
    }
...
contract RefundHandler {
    ...
    function prepareRefunds(uint256 balance, address _sender ) external {
        ...
        // Tokens are 1 based not 0 based
        for (uint256 tokenId = 1; tokenId <= supplyUsed; tokenId++) {
            address minter = mainContract.ownerOf(tokenId);
