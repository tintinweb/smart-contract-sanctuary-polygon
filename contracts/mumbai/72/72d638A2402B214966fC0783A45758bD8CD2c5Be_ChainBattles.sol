...
contract ChainBattles is ERC721URIStorage {
    ...
    mapping(uint256 => Status) public tokenIdToLevels;
    ...
    function getLevels(uint256 tokenId) public view returns (string memory) {
		Status memory status = tokenIdToLevels[tokenId];
		return status.level.toString();
    }
    ...
    function getStrength(uint256 tokenId) public view returns (string memory) {
		Status memory status = tokenIdToLevels[tokenId];
		return status.strength.toString();
    }

    function getLife(uint256 tokenId) public view returns (string memory) {
		Status memory status = tokenIdToLevels[tokenId];
		return status.life.toString();
    }
	...
	function mint() public {
		...
		tokenIdToLevels[newItemId] = Status(0, 10, 10, 100);
	...
	function train(uint256 tokenId) public {
		...
		status.strength += 10;
		status.life += 50;
		tokenIdToLevels[tokenId] = status;
