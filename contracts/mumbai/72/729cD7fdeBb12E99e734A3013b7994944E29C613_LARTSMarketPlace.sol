...
//** Author ishanshahzad :  Marketplace) Contract 2023.2 */
...
contract LARTSMarketPlace is LBase {
    constructor(
        address _owner,
        ILConfig _lConfig
    ) LBase(_owner,  _lConfig) {}
    ...
    function _processFee(uint256 price) internal override {
        uint256 fee = (price * feeRate) / FEE_RATE_BASE;
        payable(lConfig.getRevenueAddress()).transfer(fee);
        revenueAmount += fee;
    }
...
//** Author ishanshahzad :  Marketplace Base Contract 2023.2 */
...
import "./interfaces/ILConfig.sol";

/// The contract is abstract so it cannnot be deployed.
abstract contract LBase is Ownable, ReentrancyGuard, IERC721Receiver {
    enum SellMode {
        FixedPrice,
        Auction
    }
    ...
    constructor(address _owner, ILConfig _lConfig) {
    ...
    function _addListing(
        ...
        if (_sellMode != SellMode.FixedPrice) {
            ...
            require(
                _duration >= lConfig.getMinDuration() && _duration <= lConfig.getMaxDuration(),
                "Invalid duration"
            );
        }
    ...
    function _deduceRoyalties(
        ...
        if (royaltiesAmount > 0) {
            uint256 royaltyFee = (royaltiesAmount * lConfig.getRoyalityFeeRate()) / FEE_RATE_BASE;
            if (royaltyFee > 0) {
                _transferToken(msg.sender, lConfig.getRevenueAddress(), royaltyFee);
