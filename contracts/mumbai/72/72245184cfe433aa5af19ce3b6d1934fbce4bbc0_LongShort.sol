...
contract MarketTieredLeverage is AccessControlledAndUpgradeable {
  ...
  */
  ...
  struct Pool {
    address token;
    uint256 value;
    uint256 leverage; // TODO: make this base 10e6 mabye rather.
  ...
  function initializePools(
    ...
    uint256 amountToLockInYieldManager = initialMarketSeedForEachPool * initPools.length;
    ...
    for (uint256 i = 0; i < initPools.length; i++) {
      require(
        initPools[i].token != address(0) && initPools[i].leverage >= 1,
      ...
      pools[initPools[i].poolType][numberOfPoolsOfType[uint256(initPools[i].poolType)]] = Pool({
        ...
        value: initialMarketSeedForEachPool,
...
contract OracleManagerFixedEpoch {
    ...
    )
  {
    ...
    console2.log("UpdatePriceWithIndex: ", uint256(oracleUpdateIndex));
  ...
  function priceAtIndex(uint80 index) public view returns (int256 price) {
    ...
    console2.log("Price at Index: ", uint256(index));
    ...
    )
  {
    ...
    if (timstampPriceUpdated < currentEpochStartTime + MINIMUM_EXECUTION_WAIT_THRESHOLD) {
      ...
      while (true) {
        ...
        console2.log("UpdatePriceNoIndex: ", uint256(oracleUpdateIndex));
  ...
  function getMissedEpochPriceUpdates(
    ...
    console2.log("GetMissedEpochPriceUpdates: ", uint256(_currentOracleIndex - 1));
    ...
    while (_isSearchingForMissedEpochExecutionInfo && latestRoundId >= _currentOracleIndex) {
      ...
      console2.log("GetMissedEpochPriceUpdates (in while): ", uint256(_currentOracleIndex));
      if (
