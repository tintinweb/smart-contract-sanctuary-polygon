...
contract test is ERC721A, Ownable, ReentrancyGuard {
    ...
    {
        require(isMintingStart , "TEST isMintingStart Not Open Yet !");
        require(qty <= maxPerTransaction, "TEST Max Per Max Per Transaction !");
        require(totalSupply() + qty <= mintSupply,"TEST Soldout !");
        require(msg.value >= qty * pricePublic,"TEST Insufficient Funds !");
        walletPublic[msg.sender] += qty;
        _safeMint(msg.sender, qty);
    }

    function testlistMint(uint256 qty, bytes32[] calldata _merkleProof) external payable
    { 
        require(isMintingStart, "TEST isMintingStart Not Open Yet !");
        if(wallettestlist[msg.sender] < freetestlist) 
        {
           uint256 claimFree = qty - freetestlist;
           require(msg.value >= claimFree * pricetestlist,"TEST Insufficient Eth");
        }
        else
        {
           require(msg.value >= qty * pricetestlist,"TEST Insufficient Eth");
        }
        require(qty <= maxPerTransaction, "TEST Max Per Max Per Transaction !");
        require(totalSupply() + qty <= maxSupply,"TEST Soldout !");
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), "TEST Not testlist");
        wallettestlist[msg.sender] += qty;
        _safeMint(msg.sender, qty);
    }
    ...
    function setPricetestlist(uint256 pricetestlist_) external onlyOwner {
        pricetestlist = pricetestlist_;
    }
    ...
    function setfreetestlist(uint256 freetestlist_) external onlyOwner {
        freetestlist = freetestlist_;
    }

    function setWalletMint(address addr_) external onlyOwner {
        walletPublic[addr_] = 0;
        wallettestlist[addr_] = 0;
    }
