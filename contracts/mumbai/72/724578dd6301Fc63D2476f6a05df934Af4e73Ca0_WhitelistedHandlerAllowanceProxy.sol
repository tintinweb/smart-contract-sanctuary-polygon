...
abstract contract BaseAllowanceHandler is IAllowance, PausableImpl {
     ...
     * @param amount Amount of allowance to be used
     */
    function useAllowance(address address_, bytes32 bucketName, uint256 amount) public
    returns (uint256 allowance, Bucket memory bucket) {
     ...
     */
    function _addOrUseAllowance(address address_, bytes32 bucketName, uint256 amount, bool add) internal virtual
    returns (uint256 allowance, Bucket memory bucket);
...
contract AllowanceHandler is BaseAllowanceHandler {
     ...
     * @param add Should the amount of allowance added or user for defined user on a given bucket
     */
    function _addOrUseAllowance(address address_, bytes32 bucketName, uint256 amount, bool add) internal virtual override whenNotPaused()
    returns (uint256 allowance, Bucket memory bucket) {
...
abstract contract BaseWhitelistedAllowance is BaseWhitelistHandler, BaseAllowanceHandler {
    ...
    function _addOrUseAllowance(address address_, bytes32 bucketName, uint256 amount, bool add) internal virtual override
    returns (uint256 allowance, Bucket memory bucket) {
        checkWhitelisted(address_, bucketName);
        return _addOrUseAllowanceSuper(address_, bucketName, amount, add);
    }
    function _addOrUseAllowanceSuper(address address_, bytes32 bucketName, uint256 amount, bool add) internal virtual
    returns (uint256 allowance, Bucket memory bucket);


    function checkProcess(bytes32 name, address address_, uint256 amount) public virtual view override(BaseWhitelistHandler, BaseAllowanceHandler) {
        BaseWhitelistHandler.checkProcess(name, address_, amount);
        BaseAllowanceHandler.checkProcess(name, address_, amount);
    }
    function doProcess(bytes32 name, address address_, uint256 amount) public virtual override(BaseWhitelistHandler, BaseAllowanceHandler) {
        // BaseWhitelistHandler process checking is done by _addOrUseAllowance overriding
        BaseAllowanceHandler.doProcess(name, address_, amount);
    }
...
contract WhitelistedAllowanceProxy is BaseWhitelistedAllowance, WhitelistProxy, AllowanceProxy {
    ...
    function _addOrUseAllowance(address address_, bytes32 bucketName, uint256 amount, bool add) internal override(BaseWhitelistedAllowance, AllowanceProxy)
    returns (uint256 allowance, Bucket memory bucket) {
        return BaseWhitelistedAllowance._addOrUseAllowance(address_, bucketName, amount, add);
    }
    function _addOrUseAllowanceSuper(address address_, bytes32 bucketName, uint256 amount, bool add) internal override
    returns (uint256 allowance, Bucket memory bucket) {
        return AllowanceProxy._addOrUseAllowance(address_, bucketName, amount, add);
    }


    function checkProcess(bytes32 name, address address_, uint256 amount) public virtual view override(BaseWhitelistedAllowance, BaseWhitelistHandler, BaseAllowanceHandler) {
        BaseWhitelistedAllowance.checkProcess(name, address_, amount);
    }
    function doProcess(bytes32 name, address address_, uint256 amount) public virtual override(BaseWhitelistedAllowance, BaseWhitelistHandler, BaseAllowanceHandler) {
        BaseWhitelistedAllowance.doProcess(name, address_, amount);
    }
...
contract WhitelistedAllowanceHandler is BaseWhitelistedAllowance, WhitelistHandler, AllowanceHandler {
     ...
     */
    constructor() {}
    function _addOrUseAllowance(address address_, bytes32 bucketName, uint256 amount, bool add) internal override(BaseWhitelistedAllowance, AllowanceHandler)
    returns (uint256 allowance, Bucket memory bucket) {
        return BaseWhitelistedAllowance._addOrUseAllowance(address_, bucketName, amount, add);
    }
    function _addOrUseAllowanceSuper(address address_, bytes32 bucketName, uint256 amount, bool add) internal override
...
interface IWhitelist is ISecuredProcess {
     ...
     */
    function getWhitelist(bytes32 name) external view returns (Whitelist memory);
    /**
     ...
     */
    function createWhitelist(bytes32 name, bytes32 whitelisterRole) external;
     ...
     */
    function isWhitelisted(address address_, bytes32 whitelistName) external view returns (bool whitelisted, Whitelist memory whitelistDef);
    /**
     ...
     */
    function checkWhitelisted(address address_, bytes32 whitelistName) external view returns (Whitelist memory whitelistDef);
    /**
     ...
     */
    function whitelistAll(address[] memory addresses, bytes32[] memory whitelistNames) external returns (bool[] memory whitelistedStatuses, Whitelist[] memory whitelistDefs);
    /**
     ...
     */
    function whitelist(address address_, bytes32 whitelistName) external returns (bool whitelisted, Whitelist memory whitelistDef);
    /**
     ...
     */
    function unwhitelistAll(address[] memory addresses, bytes32[] memory whitelistNames) external returns (bool[] memory whitelistedStatuses, Whitelist[] memory whitelistDefs);
    /**
     ...
     */
    function unwhitelist(address address_, bytes32 whitelistName) external returns (bool whitelisted, Whitelist memory whitelistDef);
    /**
     ...
     */
    function getWhitelistedCount(bytes32 whitelistName) external view returns (uint256);
}

error WhitelistHandler_NotWhitelisted();
error WhitelistHandler_WrongParams();
error WhitelistHandler_ForbiddenRole(bytes32 role);
error WhitelistHandler_WhitelistNotDefined(bytes32 name);
error WhitelistHandler_WhitelistAlreadyDefined(bytes32 name);
...
abstract contract BaseWhitelistHandler is IWhitelist, PausableImpl {
     ...
     */
    function createWhitelist(bytes32 name, bytes32 whitelisterRole) external onlyRole(WL_ADMIN_ROLE) {
     ...
     * @param whitelisterRole Role that user should be granted in order to allow/revoke user to/from the whitelist
     */
    function _createWhitelist(bytes32 name, bytes32 whitelisterRole) internal virtual;
    /**
     ...
     * @param whitelistName Name of the whitelist for which user's whitelisting status should be retrieved
     */
    function isWhitelisted(address address_, bytes32 whitelistName) public virtual view returns (bool whitelisted, Whitelist memory whitelistDef);
    /**
     ...
     * @param whitelistName Name of the whitelist for which user's whitelisting status should be checked
     */
    function checkWhitelisted(address address_, bytes32 whitelistName) public view returns (Whitelist memory whitelistDef) {
        // Get current whitelisting status
        (bool whitelisted, Whitelist memory whitelist_) = isWhitelisted(address_, whitelistName);
     ...
     * WL_ADMIN_ROLE
     * @param address_ Address to be checked for
     * @param whitelist_ Whitelist defininition for which to check for whitelister role
     */
     ...
     * @param whitelistNames Names of the whitelists to which given addresses should be allowed
     */
    function whitelistAll(address[] memory addresses, bytes32[] memory whitelistNames) external
    returns (bool[] memory whitelistedStatuses, Whitelist[] memory whitelistDefs) {
        return _setWhitelisted(addresses, whitelistNames, true);
    }
    /**
     ...
     */
    function whitelist(address address_, bytes32 whitelistName) external returns (bool whitelisted, Whitelist memory whitelistDef) {
        return _setWhitelisted(address_, whitelistName, true);
    }
    /**
     ...
     * @param whitelistNames Names of the whitelists from which given addresses should be revoked
     */
    function unwhitelistAll(address[] memory addresses, bytes32[] memory whitelistNames) external
    returns (bool[] memory whitelistedStatuses, Whitelist[] memory whitelistDefs) {
        return _setWhitelisted(addresses, whitelistNames, false);
    }
    /**
     ...
     */
    function unwhitelist(address address_, bytes32 whitelistName) external returns (bool whitelisted, Whitelist memory whitelistDef) {
        return _setWhitelisted(address_, whitelistName, false);
    }
    /**
     ...
     */
    function _setWhitelisted(address[] memory addresses, bytes32[] memory whitelistNames, bool doWhitelist) internal virtual
     ...
     * @param doWhitelist Should new whitelisting status be on or off
     */
    function _setWhitelisted(address address_, bytes32 whitelistName, bool doWhitelist) internal
    returns (bool whitelisted, Whitelist memory whitelistDef) {
        address[] memory addresses = new address[](1);
        bytes32[] memory whitelistNames = new bytes32[](1);
        addresses[0] = address_;
        whitelistNames[0] = whitelistName;
        (bool[] memory whitelisted_, Whitelist[] memory whitelistDef_) = _setWhitelisted(addresses, whitelistNames, doWhitelist);
     ...
     */
    function checkProcess(bytes32 whitelistName, address address_, uint256) public virtual view {
        checkWhitelisted(address_, whitelistName);
    }
    /**
     * Execute process for a whitelisting secured mechanism also consists of checking whitelisting status
     */
    function doProcess(bytes32 whitelistName, address address_, uint256) public virtual {
        checkWhitelisted(address_, whitelistName);
    }
...
contract WhitelistHandler is BaseWhitelistHandler {
    ...
    mapping(address => mapping(bytes32 => bool)) private _whitelistings;

    /** @dev Whitelists defined on this contract */
    mapping(bytes32 => Whitelist) private _whitelists;
    ...
    using EnumerableSet for EnumerableSet.AddressSet;
    mapping(bytes32 => EnumerableSet.AddressSet) private whitelistedAddresses;
     ...
     */
    event Whitelisted(address indexed admin, address indexed whitelisted, bytes32 indexed whitelist);
    /**
     ...
     */
    event Unwhitelisted(address indexed admin, address indexed unwhitelisted, bytes32 indexed whitelist);
     ...
     */
    function getWhitelistAt(uint256 index) external view returns (Whitelist memory) {
        return _whitelists[_whitelistNames.at(index)];
    }
    /**
     ...
     */
    function getWhitelist(bytes32 name) public view returns (Whitelist memory) {
        if(!_whitelistNames.contains(name)) revert WhitelistHandler_WhitelistNotDefined(name);
     ...
     * @param whitelisterRole Role that user should be granted in order to allow/revoke user to/from the whitelist
     */
    function _createWhitelist(bytes32 name, bytes32 whitelisterRole) internal override {
        // Check whitelist name existence
        if(_whitelistNames.contains(name)) revert WhitelistHandler_WhitelistAlreadyDefined(name);
        // Check for forbidden roles
     ...
     * @param whitelistName Name of the whitelist for which user's whitelisting status should be retrieved
     */
    function isWhitelisted(address address_, bytes32 whitelistName) public override view returns (bool whitelisted, Whitelist memory whitelistDef) {
     ...
     * @param doWhitelist Should new whitelisting status be on or off
     */
    function _setWhitelisted(address[] memory addresses, bytes32[] memory whitelistNames, bool doWhitelist) internal override whenNotPaused()
    returns (bool[] memory whitelisted, Whitelist[] memory whitelistDef) {
        if(addresses.length != whitelistNames.length) revert WhitelistHandler_WrongParams();
        whitelisted = new bool[](addresses.length);
        whitelistDef = new Whitelist[](addresses.length);
        for(uint256 i = 0 ; i < addresses.length ; i++) {
            // Get current whitelisting status
            (bool isWhitelisted_, Whitelist memory whitelist_) = isWhitelisted(addresses[i], whitelistNames[i]);
            ...
            if(doWhitelist) {
                ...
                emit Whitelisted(msg.sender, addresses[i], whitelistNames[i]);
            }
            else {
                whitelistedAddresses[whitelistNames[i]].remove(addresses[i]);
                emit Unwhitelisted(msg.sender, addresses[i], whitelistNames[i]);
            }
        }
     ...
     * for more information.
     */
    function getWhitelistedAt(bytes32 whitelistName, uint256 index) external view onlyRole(WL_ADMIN_ROLE) returns (address) {
...
abstract contract WhitelistProxy is ProxyDiamond, BaseWhitelistHandler {
    ...
    function isWhitelisted(address address_, bytes32 whitelistName) public override view returns (bool whitelisted, Whitelist memory whitelistDef) {
        return getWhitelistProxy().isWhitelisted(address_, whitelistName);
    }
    function _setWhitelisted(address[] memory addresses, bytes32[] memory whitelistNames, bool doWhitelist) internal override whenNotPaused()
    returns (bool[] memory whitelisted, Whitelist[] memory whitelistDef) {
        (whitelisted, whitelistDef) = doWhitelist ? getWhitelistProxy().whitelistAll(addresses, whitelistNames) :
                                                     getWhitelistProxy().unwhitelistAll(addresses, whitelistNames);
        for(uint256 i = 0 ; i < whitelisted.length ; i++) {
            // Address wasn't already whitelisted, role has to be checked against caller's ones
    ...
    function getWhitelistProxy() internal view returns (IWhitelist) {
        return IWhitelist(getProxy(IWhitelistInterfaceId));
    }
