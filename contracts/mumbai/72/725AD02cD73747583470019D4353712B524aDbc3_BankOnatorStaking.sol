...
contract BankOnatorStaking is Ownable, ReentrancyGuard, Pausable {
    ...
    function calculateReward(
        ...
        if (currentTimeStamp <= lastRewardTime) {
            timeElapsed = currentTimeStamp - lastStakedTime;
        } else {
            timeElapsed = lastRewardTime - lastStakedTime;
        }

        uint256 numerator = tokens * APY * timeElapsed;
        uint256 denominator = DIVIDER * APYBASE;
