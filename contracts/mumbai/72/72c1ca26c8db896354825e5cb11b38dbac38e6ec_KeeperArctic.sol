...
import "./market/MarketTieredLeverage.sol";
...
contract LongShort is ILongShort, AccessControlledAndUpgradeable {
  ...
  // TODO: these have to be the same variable.
  ...
  function initialize(
    ...
    emit LongShortV1(_admin, _tokenFactory, address(0));
  }
  ...
  function createNewSyntheticMarketExternalSyntheticTokens(
    ...
    require(
      marketIndex < SEPARATE_MARKET_CONTRACT_INDEX_START(),
      "using separate market contracts"
    );
  ...
  function registerPoolMarketContract(
    ...
    MarketTieredLeverage.Pool[] memory _longPools,
    MarketTieredLeverage.Pool[] memory _shortPools
  ) external adminOnly {
    uint32 marketIndex = ++latestMarket;

    //TODO update this event
    // emit SeparateMarketCreated(
    //   syntheticName,
    //   syntheticSymbol,
  ...
  function initializeMarket(
    ...
    // NOTE: kPeriod and balanceIncentiveCurve_equilibriumOffset are unused!
