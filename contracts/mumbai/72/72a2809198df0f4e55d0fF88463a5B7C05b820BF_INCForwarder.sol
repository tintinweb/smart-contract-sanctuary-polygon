...
import "./abstractions/EIP712.sol";

/**
 * @dev Simple minimal forwarder to be used together with an ERC2771 compatible contract. See {Forwardable}.
 */
contract INCForwarder is IForwarder, Manageable, Pausable, EIP712 {
    ...
    public payable override onlyManager whenNotPaused
...
library Helper {
    ...
    /**
     * @dev Compare the length first to make it more gas efficient. And if the length match you can make the actual comparison.
     */
    ...
    /**
     * @dev Check to see if the subject address is a contract on the Ethereum network
     */
...
interface IForwarder {
    ...
    /**
     * @dev Returns the sender of this contract, it is the one that receives ethers to finance the meta transactions.
     */
    function relayer()
    external view
    returns(address);
...
abstract contract Ownable is Context {
     ...
     */
    function renounceOwnership(string memory confirmation) public virtual onlyOwner {
        bool confirmed = Helper.hashCompareWithLengthCheck(confirmation, "Renounce contract ownership");
        require(confirmed, "Ownable: failed confirmation");
        _setOwner(address(0));
    }
     ...
     */
    function transferOwnership(address newOwner, string memory confirmation) public virtual onlyOwner {
        require(newOwner != address(0), "Ownable: new owner is the zero address");
        bool confirmed = Helper.hashCompareWithLengthCheck(confirmation, "Transfer contract ownership");
        require(confirmed, "Ownable: failed confirmation");
        _setOwner(newOwner);
    }
