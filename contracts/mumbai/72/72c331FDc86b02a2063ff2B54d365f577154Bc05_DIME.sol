...
//                    require(Groups.getOwnerOfGroupByID(inGroups[g]) != address(0), "That group doesn't exist");
    ...
    function addLike(uint256 msgID) public whenNotPaused nonReentrant {
        // require the approval of the sending of the token to the contract
    ...
    function removeLike(uint256 msgID) public whenNotPaused nonReentrant {
        // Unlike a post
    ...
    function getMsgIDsByAddress(address usrAddress, uint256 startFrom, bool getUserComments) public view whenNotPaused returns (uint256[] memory) {

        // Initialize the array as 256 to sent to Posts
        uint256[] memory whatToGet = new uint256[](2);
        ...
        return Posts.getMsgIDs(usrAddress, startFrom, whatToGet);
    }

    function getMsgIDsByPost(uint256 postID, uint256 startFrom) public view whenNotPaused returns (uint256[] memory) {
        // Initialize the array as 256 to sent to Posts
        uint256[] memory whatToGet = new uint256[](2);
        ...
        return Posts.getMsgIDs(address(0), startFrom, whatToGet);
    }
...
interface IPosts {
    ...
    function getMsgIDs(address usrAddress, uint256 startFrom, uint256[] calldata fromComments) external view returns(uint256[] memory);

    // Remove a post from a user mapping
    function addPost(uint256 msgID, address addressPoster, uint256 isCommentOf) external;

    // Add a post to a users mapping
    function removePost(uint256 msgID, address addressPoster, uint256 isCommentOf) external;
}
...
interface ITips {
    ...
    // Get a list of a users that tipped a post
    function getTipsFromMsgID(uint256 msgID, uint256 startFrom) external view returns(address[2] memory);
