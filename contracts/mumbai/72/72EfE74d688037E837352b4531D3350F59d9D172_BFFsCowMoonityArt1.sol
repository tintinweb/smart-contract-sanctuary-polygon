...
//   ___    ___  ___ _        ___                                      _ _             _        _       _  _   _ 
//  / __\  / __\/ __( )__    / __\_____      __/\/\   ___   ___  _ __ (_) |_ _   _    /_\  _ __| |_   _| || |_/ |
// /__\// / _\ / _\ |/ __|  / /  / _ \ \ /\ / /    \ / _ \ / _ \| '_ \| | __| | | |  //_\\| '__| __| |_  ..  _| |
/// \/  \/ /  / /    \__ \ / /__| (_) \ V  V / /\/\ \ (_) | (_) | | | | | |_| |_| | /  _  \ |  | |_  |_      _| |
//\_____/\/   \/     |___/ \____/\___/ \_/\_/\/    \/\___/ \___/|_| |_|_|\__|\__, | \_/ \_/_|   \__|   |_||_| |_|
//                                                                           |___/                      
...
//The first CowMoonity Art Contest Collection! We loved these submissions so much- we wanted to offer a mint to our BFF community 

//for all artists who chose to be involved!
...
contract BFFsCowMoonityArt1 is Ownable, ERC721A, ReentrancyGuard {

    using Address for address;
    using Strings for uint256;

    //allows for 100 tokens to be minted for giveaways etc before pre and public sale
    uint256 public MAX_TEAM_RESERVES_PRESALE = 100;
    //sets immutable value for total collection of 10k
    uint256 public immutable MAX_TOKENS;
    //limits the number of tokens during presale per address
    uint256 public MAX_PRESALE_TOKENS_PER_ADDRESS = 1;
        //limits the number of tokens during public sale per address
    uint256 public MAX_PUBLICSALE_TOKENS_PER_ADDRESS = 5;
    //presale token price
    ...
    //tracks number a user has minted during publicsale
    mapping (address => uint) public publicsaleNumMinted;

    constructor(
    uint256 maxBatchSize_,
    uint256 collectionSize_,
    uint256 maxTOKENS_
  ) ERC721A("BFFsCowMoonityArt1","BFFsArt1", maxBatchSize_,collectionSize_) {
  ...
  modifier callerIsUser() {
    ...
    function _checkMintRequirements(uint256 quantity) internal {
        ...
        require(quantity > 0 && quantity <= MAX_PUBLICSALE_TOKENS_PER_ADDRESS, "invalid quantity: zero or greater than mint allowance");
        require(msg.value == TOKEN_PRICE * quantity, "wrong amount of ether sent");
        publicsaleNumMinted[msg.sender] = publicsaleNumMinted[msg.sender] + quantity;
        require(publicsaleNumMinted[msg.sender] <= MAX_PUBLICSALE_TOKENS_PER_ADDRESS, "Cannot mint more than 1 per address in this phase");
    }
    ...
    function setMAX_PUBLICSALE_TOKENS_PER_ADDRESS(uint256 _MAX_PUBLICSALE_TOKENS_PER_ADDRESS) public onlyOwner() {
    MAX_PUBLICSALE_TOKENS_PER_ADDRESS = _MAX_PUBLICSALE_TOKENS_PER_ADDRESS;
    }
