...
import "./NimbusPalsNFT.sol";

contract NimbusPalsNFTProxy {
    NimbusPalsNFT public nimbusPalsNFT;
    uint256 private immutable CHAIN_ID;
    
    constructor(address _nimbusPalsNFT, uint256 _chainId) {
        nimbusPalsNFT = NimbusPalsNFT(_nimbusPalsNFT);
        CHAIN_ID = _chainId;
    }

    function mintNFT(address receiver) public payable {
        require(_getChainId() == CHAIN_ID, "NimbusPalsNFTProxy: wrong chain");
        nimbusPalsNFT.mint{value: msg.value}(receiver);
    }
...
library ExcessivelySafeCall {
    ...
    /// @return success and returndata, as `.call()`. Returndata is capped to
    /// `_maxCopy` bytes.
    function excessivelySafeStaticCall(address _target, uint _gas, uint16 _maxCopy, bytes memory _calldata) internal view returns (bool, bytes memory) {
        // set up for assembly call
        uint _toCopy;
        bool _success;
        bytes memory _returnData = new bytes(_maxCopy);
...
abstract contract ONFT721Core is NonblockingLzApp, ERC165, IONFT721Core {
    ...
    function _creditTill(uint16 _srcChainId, address _toAddress, uint _startIndex, uint[] memory _tokenIds) internal returns (uint) {
        uint i = _startIndex;
...
contract ONFT721 is ONFT721Core, ERC721, IONFT721 {
    constructor(string memory _name, string memory _symbol, uint _minGasToTransfer, address _lzEndpoint) ERC721(_name, _symbol) ONFT721Core(_minGasToTransfer, _lzEndpoint) {}
...
contract NimbusPalsNFT is DefaultOperatorFilterer, ONFT721, ERC2981, AccessControl {
    ...
    error NimbusPalsNFT__CallerNotOwner();

    /// @param _layerZeroEndpoint Handles message transmission across chains
    /// @param __baseURI URI endpoint to query metadata
    /// @param _startMintId First mint ID
    /// @param _endMintId Last mint ID
    /// @param _minGas Min amount of gas required to transfer, and also store the payload
    constructor(address _layerZeroEndpoint, string memory __baseURI, uint _startMintId, uint _endMintId, uint256 _minGas) ONFT721("NimbusPals", "Pals", _minGas, _layerZeroEndpoint) {
        baseURI = __baseURI;
        nextMintId = _startMintId;
        maxMintId = _endMintId;
        _setDefaultRoyalty(_msgSender(), ROYALTY_FEE);
    ...
    function mint(address receiver) public payable {
        require(hasRole(MINTER_ROLE, msg.sender), "NimbusPals: must have minter role to mint");
        require(nextMintId <= maxMintId, "NimbusPals: max mint reached");
        require(msg.value >= mintPrice, "NimbusPals: insufficient payment");
        // Immediately transfer the received Ether to the contract owner
        payable(owner()).transfer(mintPrice);
        
        _safeMint(receiver, nextMintId);
        unchecked {
            nextMintId++;
        }
    }
    ...
    function _requireOwner() internal view {
        if (_msgSender() != owner()) {
            revert NimbusPalsNFT__CallerNotOwner();
        }
    }
    /**
     * @dev See {IERC165-supportsInterface}.
     */
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC2981, ONFT721, AccessControl) returns (bool) {
