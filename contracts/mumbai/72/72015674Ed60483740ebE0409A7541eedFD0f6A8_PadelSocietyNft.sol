...
// Dependency file: contracts/interfaces/IERC4907.sol
...
// Dependency file: contracts/ERC4907.sol


// pragma solidity ^0.8.1;

// import "@openzeppelin/contracts/token/ERC721/extensions/ERC721Enumerable.sol";
// import "contracts/interfaces/IERC4907.sol";
...
// Root file: contracts/PadelSocietyNft.sol


pragma solidity ^0.8.1;

// import "@openzeppelin/contracts/access/Ownable.sol";
// import "contracts/ERC4907.sol";

contract PadelSocietyNft is ERC4907, Ownable {
    using Strings for uint256;

    string constant INITIAL_METADATA_EXTENSION = ".json";
    uint256 constant INITIAL_PRICE = 0.001 ether;
    uint256 constant INITIAL_MAX_SUPPLY = 500;
    uint256 constant INITIAL_MAX_MINT_AMOUNT = 5;
    ...
    constructor(
        ...
        setBaseURI(baseURI_);
        setHiddenBaseURI(hiddenBaseURI_);
        setPrice(INITIAL_PRICE);
        setMaxMintAmount(INITIAL_MAX_MINT_AMOUNT);
        setMaxSupply(INITIAL_MAX_SUPPLY);
        setMetadataExtension(INITIAL_METADATA_EXTENSION);
    }
    ...
    function _mintNft(uint256 _mintAmount, address _receiver) internal {
        uint256 supply = totalSupply();

        require(paused == false, "PADEL_SOCIETY: MINT_PAUSED");
        require(_mintAmount > 0, "PADEL_SOCIETY: MINT_AMOUNT_INVALID");
        require(
            _mintAmount <= maxMintAmount,
            "PADEL_SOCIETY: MINT_AMOUNT_INVALID"
        );
        require(supply + _mintAmount <= maxSupply, "PADEL_SOCIETY: MAX_SUPPLY");

        if (_receiver != owner()) {
            require(
                msg.value >= price * _mintAmount,
                "PADEL_SOCIETY: INSUFFICIENT_FUNDS"
            );
        }

        for (uint256 index = 1; index <= _mintAmount; index++) {
            _safeMint(_receiver, supply + index);
        }
    }

    function mint(uint256 _mintAmount) public payable {
        _mintNft(_mintAmount, msg.sender);
    }

    function mint(uint256 _mintAmount, address _receiver) public payable {
        _mintNft(_mintAmount, _receiver);
    }
        ...
        returns (string memory)
    {
        require(
            _exists(tokenId),
            "PADEL_SOCIETY: ERC721_METADATA_NON_EXISTENT_TOKEN"
        );
    ...
    function setHiddenBaseURI(string memory hiddenBaseURI_) public onlyOwner {
    ...
    function pause(bool paused_) public onlyOwner {
        paused = paused_;
    }
    ...
    function withdraw() public payable onlyOwner {
        ...
        require(success, "PADEL_SOCIETY: WITHDRAW_FAILED");
    }
    ...
    function userExpires(uint256 tokenId)
        ...
        returns (uint256)
    {
        return super.userExpires(tokenId);
    }
}