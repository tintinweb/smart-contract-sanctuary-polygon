    ...
    ERC20Upgradeable
{
    ...
    function initialize(
        ...
        _mint(_premintAddr, 1000000000000e18);
    }
        ...
        returns (uint256 circSupply)
    {
        ...
        circSupply = circSupply.sub(circulatingSupply);
    }

    function getPoolPrice() public view returns (uint256 price) {
        bool isToken0 = pair.token0() == address(this);
        ERC20 pToken = isToken0 ? ERC20(pair.token1()) : ERC20(pair.token0());
    ...
    function _setIsPauseTrade(bool isPause) internal {
        isPauseTrade = isPause;
    }
    ...
    function _transfer(
        ...
        if (to == address(pair)) {
            circulatingSupply = circulatingSupply + (amount - feeAmount);
        }
        ...
        if (!isWhitelist && isTrade) {
            require(isPauseTrade == false, "pozToken: The trade is paused");
            ...
            _setIsPauseTrade(true);
        }
    }

    function setIsPauseTrade(bool isPause) public onlyRift {
        isPauseTrade = isPause;
    }

    function dynamicAdjustment(bool tradeType) public onlyRift {
        uint256 pool_price = getPoolPrice();
        bool isToken0 = pair.token0() == address(this);
        ERC20 pToken = isToken0 ? ERC20(pair.token1()) : ERC20(pair.token0());
        uint256 poolBalance = pToken.balanceOf(address(pair));
        ...
        bool isIncrease = pozAmountOut > balanceOf(address(pair));
        if (isIncrease) {
            _balanceAdjustment(true, pozAmountOut - balanceOf(address(pair)));
        } else {
            _balanceAdjustment(false, balanceOf(address(pair)) - pozAmountOut);
        }
        uint256 next_pool_price = getPoolPrice();
        if (tradeType) {
            require(next_pool_price > pool_price, "price error");
        }
        _setIsPauseTrade(false);
    }
