...
// File: @openzeppelin\contracts\utils\introspection\ERC165.so
...
// File: contracts\CrunchToken.sol
...
contract CrunchToken is ERC20, AccessControl {
    ...
    constructor(address owner1, address owner2) ERC20("Crunch Token", "CT") {
        ...
        uint256 totalSupply = 500 * 1e6 * 1e18;

        maxTransactionAmount = (totalSupply * 5) / 1000; // 0.5% maxTransactionAmountTxn
        maxWallet = (totalSupply * 12) / 1000; // 2% maxWallet
        ...
        marketingWallet = 0x26F19037CB78efdEaDbf07E0D5f0c3161C0e0CD6; // set as marketing wallet
        devWallet = 0x91ce82a94658D0eFa9638861d1d79be42Ce7a370; // set as dev wallet

        // exclude from paying fees or having max transaction amount
        excludeFromFees(owner1, true);
        excludeFromFees(owner2, true);
        excludeFromFees(address(this), true);
        excludeFromFees(address(0xdead), true);
        excludeFromFees(buyBackWallet, true);

        excludeFromMaxTransaction(owner1, true);
        excludeFromMaxTransaction(owner2, true);
    ...
    function updateMaxAmount(uint256 newNum)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(
    ...
    function forceSwapBack() external onlyRole(DEFAULT_ADMIN_ROLE) {
        uint256 contractBalance = balanceOf(address(this));
        require(
    ...
    function buyBackTokens(uint256 ethAmountInWei)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
