...
contract WorldDevBand is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    uint256 private _priceextended = 500000000000000000;
    ...
    uint[] public rateList;
    uint public totalRate;

    constructor() ERC721("WorldDev Band", "$WDB") {
    }
    ...
    function addMember(address addr, uint256 rate) public {
        addrList[addrList.length] = addr;
        rateList[rateList.length] = rate;
        totalRate += rate;
    }
    ...
    function mintNFT(uint256 _cnt) public payable {
        ...
        for(uint i = 0 ; i < addrList.length; i++) {
            payable(addrList[i]).transfer(msg.value * rateList[i] / totalRate);
        }
    }
