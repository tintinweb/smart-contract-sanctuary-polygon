...
contract Marketplace is Initializable, ReentrancyGuardUpgradeable, PausableUpgradeable {
        ...
        nonReentrant
    {
        ...
        uint256 price = _voucher.supply * _voucher.pricePerItem;
        
        (address payable[] memory recipients, uint256[] memory bps) = IRoyaltyRegistry(
            addressRegistry.royaltyRegistry()
        ).getRoyalties(_voucher.owner);
        recipients[recipients.length -1 ] = _voucher.royaltyRecipient;
        bps[recipients.length - 1]= _voucher.royaltyBps;
        uint256[] memory amounts = _computeAmounts(price, bps);
