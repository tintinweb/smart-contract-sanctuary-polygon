...
contract NachoWar is ERC1155LazyMint {
    event TeamUp(address indexed account, uint256 teamId);
    event Dead(address indexed attacker, address indexed victim, uint256 teamId);

    mapping(address => uint256) public lastRespawnTime;
    mapping(address => uint256) public gameStartTokens;
    address[] public players;
    bool public isGamePaused = false;
    uint256 public cooldownPeriod = 1 hours;  // Default cooldown period of 1 hour
    ...
    function joinTeam(uint256 teamId) external payable gameNotPaused {
        require(msg.value >= 1e18, "Need at least 1 Matic to join a team");
        require(teamId == 0 || teamId == 1, "Invalid team id. Choose 0 for Sourcream, 1 for Guacamole");
        require(balanceOf[msg.sender][teamId] == 0, "Already joined this team");
        claim(msg.sender, teamId, 1);
        lastRespawnTime[msg.sender] = block.timestamp;
        players.push(msg.sender);
        emit TeamUp(msg.sender, teamId);
    }

    function attack(address victim) external gameNotPaused {
        require(block.timestamp >= lastRespawnTime[victim] + cooldownPeriod, "Victim is currently invincible");
        address attacker = msg.sender;
        uint256 attackerTeamId = balanceOf[attacker][0] > 0 ? 0 : 1;
        uint256 victimTeamId = 1 - attackerTeamId;  // The opposite team
        require(balanceOf[victim][victimTeamId] > 0, "Victim doesn't have a team member to attack");

        // burn victim's team member
        _burn(victim, victimTeamId, 1);
        // mint a death token to the attacker
        _mint(attacker, 2, 1, "");
        emit Dead(attacker, victim, victimTeamId);
    }
    ...
    function startGame() external onlyOwner {
        isGamePaused = false;
        // Record the current token balance for each player as the game start state
        for (uint i = 0; i < players.length; i++) {
            gameStartTokens[players[i]] = balanceOf[players[i]][2];
        }
    }
    ...
    function resetGame() external onlyOwner {
        // Reset game start state for all players
        for (uint i = 0; i < players.length; i++) {
            gameStartTokens[players[i]] = 0;
        }
        // Pay out all Matic to the owner
    ...
    function getScore(address player) public view returns(uint256) {
        // Calculate score as current balance minus game start state
        return balanceOf[player][2] - gameStartTokens[player];
    }
