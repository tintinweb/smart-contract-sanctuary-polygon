    ...
    FloatingPoint
{
    ...
    function setSoulBoundTokens(address _sbtContract)
        public
        onlyRole(ADMIN_ROLE)
    {
        sbts = _sbtContract;
    ...
    function buy() external payable nonReentrant {
        ...
        if (enableProtocolFee) {
            ...
            LABZ(labzToken).mintProtocolFee(feeWallet, _fee);
            emit ProtocolFeeMinted(feeWallet, _fee);
        }

        require(numMinted > 0, "error buying labz");

        (bool success, bytes memory amt) = multiWallet.call{
            value: msg.value
        }("");
    ...
    function TokenUSDRate() public view returns (uint256 r) {
        r = AKXPriceOracle(priceOracle).AkxUSD() / 1e18;
    }
    ...
    AccessControlEnumerable
{
    ...
    function mintProtocolFee(address to, uint256 amount) public onlyRole(MINTER_ROLE) returns(uint256) {
    ...
    function setNewMinter(address newMinter) public onlyRole(DEFAULT_ADMIN_ROLE) {
