...
contract ActivityFactory is Ownable {
    ...
    function setActivityImpl(address activityImpl_) external onlyOwner {
        activityImpl = activityImpl_;
    }


    function createActivity(string calldata contractName, string calldata symbol, string calldata activityName) external {
        address activity;
        bytes memory bytecode = type(Activity).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(msg.sender, nonce[msg.sender]++));
        assembly {
            activity := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
