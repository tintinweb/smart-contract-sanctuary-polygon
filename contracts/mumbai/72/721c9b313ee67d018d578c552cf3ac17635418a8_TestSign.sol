...
contract TestSign is Ownable, ERC721 {
    ...
    uint256 public constant MAX_WL_TOKENS = 3;
    uint256 public constant COST_PER_TOKEN = 0.001 ether;

    address private _signerAddress;

    mapping(address => uint256) public tokensMinted;

    constructor(address signerAddress_) ERC721("Test Mint", "TEST") {
        _signerAddress = signerAddress_;
    }

    function whitelistMint(uint256 numTokens, bytes calldata signature)
        public
        payable
    {
        require(
            MAX_SUPPLY >= _tokenIdCounter.current() + numTokens,
            "Minted tokens would exceed supply."
        );
        require(
            MAX_WL_TOKENS >= tokensMinted[msg.sender] + numTokens,
            "Claim limit exceeded."
        );
        require(
            msg.value == numTokens * COST_PER_TOKEN,
    ...
    function testSignerRecovery(bytes calldata signature)
        external
        view
        returns (address)
    {
