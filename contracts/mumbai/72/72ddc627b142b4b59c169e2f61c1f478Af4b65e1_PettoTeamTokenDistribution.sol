...
/**
* @title Petto Team Token Distribution
*/
contract PettoTeamTokenDistribution is AccessControl, Pausable, Ownable, ReentrancyGuard {
    ...
    */
    ...
    /**
    * @notice Emitted when a new distribution is run
    */
    ...
    address[] private teamMembers; //Store team members

    uint256 private constant _DECIMALS = 18;

    uint256 private constant _DISTRIBUTION_PERIOD = 30 days;
    
    uint256 private constant _DISTRIBUTION_DURATION = 36;

    uint256 private constant _MAX_TOKEN_SUPPLY = 100000000 * (10**_DECIMALS);
    
    uint256 distributionTimestamp; //store latest distribution timestamp
    
    bytes32 public constant ADMIN_ROLE = keccak256("ADMIN_ROLE");
    

    /**
    * @notice The constructor for the Petto Team Token Distribution.
    ...
    */
    constructor(address _tokenAddress, address _admin){
        pot = 0;
        amount_supplied = _MAX_TOKEN_SUPPLY; //The total amount of token released
        _setupRole(ADMIN_ROLE, _admin);
        distributionTimestamp = 0;
        teamMembers = [
            0x0517462bA52036EE0eb8DdC375a987F2d0FEc921, //Felysia
            0xcF4E6E7285511AdcF472770e288940416bcc2c62, //Nam
            0x6dAb7b69364c3F34631B789aDa9dB4506DBC5032, //Nataliia
            0x57f4977216f7A39F2Cf88C77bf64469C48f5b148, //Cung
            0xc0E7F76A9C0419837F11c3F0fD086b6D9FBd57f4, //Takis
            0xeE285d02e8515f363b69c2b13Ca7Cf75036f54b7, //Vladimir
            0x19AFE287Cd7f9091Fec5738C477f84Ec18d49A07, //Leo
            0x6988bF5B857a4E8f66858D200Be5C8c9cC22762d //Viktor
        ];
    ...
    */
    function depositPot(uint256 _amount) external whenNotPaused nonReentrant returns (bool){
    ...
    */
    function returnPot(uint256 _amount) external whenNotPaused nonReentrant returns (bool){
        require(hasRole(ADMIN_ROLE, msg.sender), "Unauthorized");
    ...
    */
    ...
    /**
    * @notice Add team memmber
    * @return bool
    */
    function addTeamMember(address _member) external whenNotPaused nonReentrant returns (bool){
        require(hasRole(ADMIN_ROLE, msg.sender), "Unauthorized");
        ...
        if(!alreadyAdded){
            teamMembers.push(_member);
        }
        return true;
    }
    
    /**
    * @notice Remove team memmber
    * @return bool
    */
    function removeTeamMember(address _member) external whenNotPaused nonReentrant returns (bool){
        require(hasRole(ADMIN_ROLE, msg.sender), "Unauthorized");
        for(uint256 i = 0; i < teamMembers.length; i++){
            if(teamMembers[i] == _member){
    ...
    /**
    * @notice distribute token to team members
    * @return bool return true if distribute successfully
    */
    function distributeToken() external whenNotPaused onlyOwner nonReentrant returns (bool){
        require(!isSubscriptionEnded(), "Subscription ended");
        uint256 current_time = block.timestamp;
        uint256 total_period = current_time.sub(distributionTimestamp);
        uint256 period = total_period.div(_DISTRIBUTION_PERIOD);
        if(period == 0){
            revert("Distribution should run every month");
        }
        uint256 numberOfMember = teamMembers.length;
        uint256 tokenToSend = _MAX_TOKEN_SUPPLY/_DISTRIBUTION_DURATION;
        if(tokenToSend > pot){
            revert("Pot exhausted");
        }
        uint256 transferAmount = tokenToSend/numberOfMember;
        for (uint256 i = 0; i<numberOfMember; i++){
            if(!pettoToken.transfer(teamMembers[i], transferAmount)){
                revert("Unable to transfer funds");
            }
        }
        distributionTimestamp = block.timestamp;
        updateSuppliedToken(tokenToSend);
