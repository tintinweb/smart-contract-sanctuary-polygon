...
contract BulkCrossChain is ERC20, IDapp {
  ...
  function bulkTransferCrossChain(
    TransferDetail[] calldata transferDetails,
    bytes calldata requestMetadata
  ) external payable {
    for (uint256 i = 0; i < transferDetails.length; i++) {
      TransferDetail calldata transferDetail = transferDetails[i];
      transferCrossChain(
        transferDetail.amount,
        transferDetail.destChainId,
