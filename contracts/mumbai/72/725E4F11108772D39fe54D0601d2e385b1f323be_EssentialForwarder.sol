...
contract EssentialForwarder is EIP712, AccessControl, SignedOwnershipProof {
        ...
        returns (bool, bytes memory)
    {
        ...
        _tokenNonces[req.nftContract][req.tokenId] += 1;

        (bool success, bytes memory returndata) = req.to.call{gas: req.gas, value: 0}(
            abi.encodePacked(req.data, req.nftContract, req.tokenId, req.authorizer)
...
contract SignedOwnershipProof {
        ...
        returns (bool)
    {
        ...
        uint256 tokenNonce = _tokenNonces[req.nftContract][req.tokenId];

        bytes32 message = createMessage(req.authorizer, req.nonce, req.nftContract, req.tokenId, tokenNonce)
