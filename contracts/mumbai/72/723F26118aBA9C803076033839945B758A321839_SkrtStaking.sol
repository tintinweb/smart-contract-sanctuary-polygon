...
contract SkrtStaking is Ownable {
    ...
    uint256 public stakingPeriodSeconds;
    ...
    constructor(address _tokenAddress, address _skrtVerificationAddress, address _rewardsAddress, uint256 _annualMintPercentage, uint256 _stakingPeriodSeconds, uint256 _minimumStakingAmount, uint256 _maximumStakingAmount, 
    uint256 _stakingPoolSize, uint256 _unStakeStartSeconds, uint256 _threeMonthsUnStakePenalty, uint256 _sixMonthsUnStakePenalty) {
        ...
        stakingPeriodSeconds = _stakingPeriodSeconds;
    ...
    **/
    ...
    returns (bool){
        ...
        uint256 totalStakeSeconds = (block.timestamp - lastStakeClaimed[msg.sender]);
        require(totalStakeSeconds >= unStakeStartSeconds, "[Withdraw] Cannot withdraw tokens too early");
    ...
    returns (bool) {
        ...
        require((totalStakedTokens  + _stakeReward) <= stakingPoolSize, "Staking Pool Limit reached, failed to restake");
    ...
    /**
     * @dev reset wallet address balance
     **/ 
    ...
    /**
     * @dev annual mint percentage (x 100)
     **/
    function setAnnualMintPercentage(uint256 _percent) 
    whenPaused
    external onlyOwner() 
    returns (bool) {
    ...
    /**
     * @dev set the minimum staking amount
     **/ 
    function setMinimumStakingAmount(uint256 _amount) 
    whenPaused
    external onlyOwner() 
    returns (bool) {
    ...
    /**
     * @dev set the maximum staking amount
     **/ 
    function setMaximumStakingAmount(uint256 _amount) 
    whenPaused
    external onlyOwner() 
    returns (bool) {
     ...
     **/ 
    function setStakingPoolSize(uint256  _stakingPoolSize) whenPaused
    external onlyOwner() 
    returns (bool) {
    ...
    /**
     * @dev set the 3 months early unstake penalty (percentage * 100)
     **/ 
    function setThreeMonthsUnStakePenalty(uint256 _percent) 
    whenPaused
    external onlyOwner() 
    returns (bool) {
    ...
    /**
     * @dev set the 6 months early unstake penalty (percentage * 100)
     **/ 
    function setSixMonthsUnStakePenalty(uint256 _percent) 
    whenPaused
    external onlyOwner() 
    returns (bool) {
    ...
    /**
     * @dev set staking period in seconds
     **/ 
    function setStakingPeriodSeconds(uint256 _seconds) 
    whenPaused
    external onlyOwner() 
    returns (bool) {
        stakingPeriodSeconds = _seconds;
        return true;
    }
    ...
    modifier _correctStakingAmount(uint256 amount) {
        require(amount >= minimumStakingAmount && amount <= maximumStakingAmount, "Staking amount is invalid");
        _;
    }
    ...
    function _checkForUnStakePenalties(uint256 _totalStakeSeconds, uint256 _stakeReward) internal view returns (uint256) {
        ...
        } else if (_totalStakeSeconds > 15780000 && _totalStakeSeconds < stakingPeriodSeconds) {
            //between 6 - 12 months
