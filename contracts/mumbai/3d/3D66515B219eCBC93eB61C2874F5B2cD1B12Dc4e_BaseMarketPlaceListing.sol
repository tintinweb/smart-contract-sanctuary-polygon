...
contract BaseMarketPlaceListing is Ownable {
    ...
    function getListing(string memory _listingId) public view virtual returns (Listing memory) {
     ...
     */
     function createListing(
        ...
        require(keccak256(abi.encodePacked(_listings[listingId].listingId)) !=
                keccak256(abi.encodePacked(listingId)), "Listing already existed for current listing Id");
        if (!isErc721) {
            require(
            ...
            require(
                ...
                "ERC721 token owner is not the seller"
            );
        }
     ...
     */
     //need to change msg.sender to buyer argument 
    function buyAssetFromListing(string memory listingId) external {
        Listing memory listing = _listings[listingId];
        require(listing.state == State.INITIATED, "Listing is not in initiated state");
        if(listing.isErc721) {
            require(
                IERC721(listing.nftAddress).ownerOf(listing.tokenId) == listing.seller,
                "ERC721 token owner is not the seller"
            );
            IERC721(listing.nftAddress).safeTransferFrom(
                ...
                abi.encodePacked(
                    "SafeTransferFrom",
                    "'''###'''",
                    _uint2str(listing.tokenId)
                )
            );
        } else {
            require(
                IERC1155(listing.nftAddress).balanceOf(listing.seller, listing.tokenId) >= listing.amount,
                "ERC1155 token balance is not sufficient for the buyer"
            );
        ...
        //we dont really want to do this, too much gas
        //_toRemove(listingId);
    }
