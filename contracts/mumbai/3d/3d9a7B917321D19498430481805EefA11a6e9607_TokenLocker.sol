...
contract TokenLocker is ReentrancyGuard {
    ...
    function releaseTokens(uint256 _lockerIndex) external {
        ...
        uint256 totalReleaseCount = locker.releasePercentage.div(100);
        uint256 totalTokensToRelease = locker.totalAmount.mul(totalReleaseCount);

        require(locker.claimedAmount < locker.totalAmount, "All tokens have been claimed");
        ...
        uint256 remainingAmount = locker.totalAmount.sub(tokensToRelease);
        locker.totalAmount = remainingAmount;
    ...
    function getClaimableTokens(uint256 _lockerIndex) external view returns (uint256) {
        ...
        if (locker.claimedAmount >= totalTokensToRelease) {
            return 0;
        }
