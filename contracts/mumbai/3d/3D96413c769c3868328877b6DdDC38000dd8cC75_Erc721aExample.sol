...
contract Erc721aExample is ERC721A, ReentrancyGuard, Ownable {
    ...
    function mint(uint256 quantity_) external payable {
        ...
        refundOverpayment(quantity_ * price);
    }

    function refundOverpayment(uint256 totalCost_) private {
        require(msg.value >= totalCost_, "need to send more ETH");
        if (msg.value > totalCost_) {
            payable(msg.sender).transfer(msg.value - totalCost_);
        }
    }
