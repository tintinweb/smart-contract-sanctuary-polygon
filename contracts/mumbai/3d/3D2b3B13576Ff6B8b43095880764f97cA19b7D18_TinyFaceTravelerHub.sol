...
contract TinyFaceTravelerHub is IONFT, NonblockingLzApp, ERC721 {
    using Counters for Counters.Counter;
    using Strings for uint256;

    Counters.Counter travelerCount;

    uint128 constant MAX_TRAVELERS = 10000;
    bool immutable isMainHub;
    string baseURI;
    ...
    constructor(
        ...
        isMainHub = lzEndpoint.getChainId() == _mainHubLZID;
        baseURI = _base;
    }

    function spawn() external {
        require(isMainHub, "TFTHub: Cannot spawn outside the Main Hub");
        require(
            travelerCount.current() + 1 < MAX_TRAVELERS,
            "TFTHub: No more Travelers allowed"
        );

        _safeMint(_msgSender(), travelerCount.current());
        travelerCount.increment();
    }

    function traverseChains(
        uint16 _dstChainId,
        bytes calldata _dstHub,
        uint256 _tokenId,
    ...
    function _send(
        ...
        require(
            ...
            "TFTHub: Not allowed to traverese this Traveler - Not owner nor approved"
        );

        require(
            msg.value >= estimateTravelGas(_dstChainId, _adapterParam),
            "TFTHub: Not enough gas for travel provided"
        );
    ...
    {
        require(_exists(_tokenId), "ONFT: URI query for nonexistent token");
    ...
    function estimateTravelGas(uint16 _dstChainId, bytes calldata _adapterParam)
        public
        view
        returns (uint256)
    {
        bytes memory mockPayload = abi.encode(address(0x1), 1);

        (uint256 msgFee, ) = lzEndpoint.estimateFees(
            _dstChainId,
            address(this),
            mockPayload,
            false,
    ...
    ILayerZeroUserApplicationConfig
{
    ...
    mapping(uint16 => mapping(bytes => bool)) internal trustedHubLookup;
    mapping(uint16 => bytes[]) internal trustedHubs;

    event AddTrustedHub(uint16 _srcChainId, bytes _srcAddress);
    ...
    function lzReceive(
        ...
        require(
            ...
            "TFTHub: Only reachable by corresponding endpoint"
        );
        // if will still block the message pathway from (srcChainId, srcAddress). should not receive message from untrusted remote.
        require(
            trustedHubLookup[_srcChainId][_srcAddress],
            "TFTHub: Untrusted origin hub"
        );
        // require(
        //     _srcAddress.length == trustedHubLookup[_srcChainId].length &&
        //         keccak256(_srcAddress) ==
        //         keccak256(trustedHubLookup[_srcChainId]),
    ...
    function _lzSend(
        ...
        require(
            trustedHubLookup[_dstChainId][_dstHub],
            "TFTHub: destination hub is not trusted"
        );
    ...
    function addTrustedHub(uint16 _srcChainId, bytes calldata _srcAddress)
        external
        onlyOwner
    {
        require(
            trustedHubLookup[_srcChainId][_srcAddress] == false,
            "TFTHub: Hub already added"
        );

        trustedHubLookup[_srcChainId][_srcAddress] = true;
        trustedHubs[_srcChainId].push(_srcAddress);
        emit AddTrustedHub(_srcChainId, _srcAddress);
    }
    ...
    function isTrustedRemote(uint16 _srcChainId, bytes calldata _srcAddress)
        ...
        returns (bool)
    {
        // bytes memory trustedSource = trustedHubLookup[_srcChainId];
        // return keccak256(trustedSource) == keccak256(_srcAddress);
        return trustedHubLookup[_srcChainId][_srcAddress];
    }

    //--------------------------- VIEW FUNCTION ----------------------------------------
    // interacting with the LayerZero Endpoint and remote contracts

    function getTrustedRemotes(uint16 _chainId)
        external
        view
        returns (bytes[] memory)
    {
        return trustedHubs[_chainId];
    }
