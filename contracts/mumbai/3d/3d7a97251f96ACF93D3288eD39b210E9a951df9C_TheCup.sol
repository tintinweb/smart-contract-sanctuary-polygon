...
//contract TheCup is ERC1155Supply, ERC2981, Ownable, ReentrancyGuard, QRNG {
contract TheCup is ERC1155Supply, ERC2981, Ownable, ReentrancyGuard {
    // Price of one Cube
    ...
    string public CupNFT;

    // Need a mapping for all NFTs minted overall
    // mapping(uint256 => uint256) public tokensMinted;

    //events
    event NFTMinted(address minter, uint256 _amount);
    event AdminMinted(address reciever, uint256 _amount, uint256 _id);

    //commenting out the airnode for now
    /*vconstructor(
        address _airnodeRrp,
        address _royalty,
        string memory _cupNFT
    ) ERC1155(_cupNFT) QRNG(_airnodeRrp) { */

    constructor(address _royalty, string memory _cupNFT) ERC1155(_cupNFT) {
    ...
    function mint(uint256 amount) external payable {
        ...
        require(
            (nftsMinted + amount) <= maxTotalSupply,
        ...
        for (uint256 i = 0; i < amount; i++) {
            ...
            //  uint256 sum = (seed + localRandom) % 58820;  // adjusting for 32 nfts remove the mod 115

            uint256 _tokenNumber = getTokenId(localRandom);
        
            _mint(msg.sender, _tokenNumber, 1, "");
    ...
    function airDrop(address[] memory _airdrops) external onlyOwner {
        require(!paused, "Contract Paused");
        require(
            (nftsMinted + _airdrops.length) <= maxTotalSupply,
            "Max supply exceeded"
        );
        ...
        for (uint256 i = 0; i < _airdrops.length; i++) {
            //randomize
            uint256 localRandom = randomNumber();
            //  uint256 sum = (realRandom + localRandom) % 32;  // adjusting for 32 nfts
            uint256 sum = localRandom % 32; // adjusting for 32 nfts - to be removed
            uint256 randomNFT = sum + 1; //offset 0

            _mint(_airdrops[i], randomNFT, 1, "");
            //        tokensMinted[randomNFT] += 1; // mapping for token ID
            nftsMinted += 1;
        }
    }
    ...
    function randomNumber() internal returns (uint256) {
        ...
        uint256 localRand = (block.difficulty + block.timestamp + shifter) % 58820;
        shifter = localRand;
        return localRand;
    }

    function getTokenId(uint256 chance) public pure  returns (uint256) {
       // uint256 chance = random % 58220;
        ...
        if (chance > 6220) {
            ...
            if (chance > 22220) {
                ...
                if (chance > 33220) {
                    ...
                    if (chance > 43220) {
                        // 31 or 32
                        if (chance > 50720) return 32;
                        else return 31;
                    } else {
                        // 29 or 30
                        if (chance > 38220) return 30;
                        else return 29;
                    }
                } else {
                    // 25 to 28
                    if (chance > 27220) {
                        // 27 or 28
                        if (chance > 29720) return 28;
                        else return 27;
                    } else {
                        // 25 or 26
                        if (chance > 24720) return 26;
                        else return 25;
                    }
                }
            } else {
                ...
                if (chance > 13220) {
                    ...
                    if (chance > 17220) {
                        // 23 or 24
                        if (chance > 19720) return 24;
                        else return 23;
                    } else {
                        // 21 or 22
                        if (chance > 15220) return 22;
                        else return 21;
                    }
                } else {
                    // 17 to 20
                    if (chance > 9220) {
                        // 19 or 20
                        if (chance > 11220) return 20;
                        else return 19;
                    } else {
                        // 17 or 18
                        if (chance > 7720) return 18;
                        else return 17;
                    }
                }
            }
            ...
            if (chance > 680) {
                ...
                if (chance > 2120) {
                    ...
                    if (chance > 3720) {
                        // 15 or 16
                        if (chance > 4720) return 16;
                        else return 15;
                    } else {
                        // 13 or 14
                        if (chance > 2920) return 14;
                        else return 13;
                    }
                } else {
                    // 9 to 12
                    if (chance > 1120) {
                        // 11 or 12
                        if (chance > 1620) return 12;
                        else return 11;
                    } else {
                        // 9 or 10
                        if (chance > 840) return 10;
                        else return 9;
                    }
                }
            } else {
                ...
                if (chance > 240) {
                    ...
                    if (chance > 420) {
                        // 7 or 8
                        if (chance > 540) return 8;
                        else return 7;
                    } else {
                        // 5 or 6
                        if (chance > 320) return 6;
                        else return 5;
                    }
                } else {
                    // Lower 4
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked(CupNFT, Strings.toString(_tokenId), ".json")
            );
    }
    ...
    function setNewMax(uint256 _newMax) external onlyOwner {
        maxTotalSupply = _newMax; //Incase Tokens go offbalance
    }
