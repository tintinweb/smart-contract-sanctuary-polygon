...
contract SphereToken is ERC20Detailed, Ownable {
    ...
    //return total tax of user with the bracket system
    function userTotalTax(address who) public view returns (uint256) {
        return totalSellFee.add(calculateAdditionalTax(who));
    }
    ...
    //Bracket Tax: get liquidity ratio in LP to find out how much % the whale holds of the pool and how much they technically
    //could dump on users and cause a price drop
    //motivation: directly contact the team and sell off market (OTC) to sustain price without huge damages to the protocol's LP
    function getIDEXPairLiquidityForSphere() internal view returns (uint256) {
        uint112 reserve0;
        uint112 reserve1;
        uint32 blockTimestampLast;
        uint256 tokenValue = getOwnAddressInIDexPair();
        uint256 totalLiquidity;

        (reserve0, reserve1, blockTimestampLast) = iDexPair.getReserves();
        //check what is the totalLiquidity of the main LP to see how much the user has in comparison
        if (tokenValue == 0) {
    ...
    //only calculates additional tax if necessary
    function calculateAdditionalTax(address userAddress) public view returns (uint256) {
       //make sure if the tax is not enabled, then return 0
        if(!isTaxBracket())
            return 0;
        //get the total liquidity off the pool
        uint256 totalLiq = getIDEXPairLiquidityForSphere();
        //gets the total balance of the user
        uint256 userTotal = balanceOf(userAddress);

        //calculate the percentage
        uint256 totalCap = userTotal.mul(100).div(totalLiq);
        ...
        return _bracket.mul(taxBracketMultiplier);
    }

    function getOwnAddressInIDexPair() internal view returns (uint256) {
        address token0 = iDexPair.token0();
        address token1 = iDexPair.token1();
        if (token0 == address(this)) {
            return 0; //first one
        } else if (token1 == address(this)) {
            return 1; //second one
        }
        require(1 == 0, "Pair is incorrect! Can't implement bracket tax");
        return 2;
    }
    ...
    function updateLaunchPeriodFee() private {
        //thanks to integer, if rebaseEpoch is > 47, sellLaunchFeeSubtracted goes to 1 (48 rebases everyday)
        //the calculation should always round down to the lowest fee deduction every day
        uint256 _sellLaunchFeeSubtracted = rebaseEpoch.div(48);
    ...
    function setwhaleFeeMultiplier(uint256 _whaleFeeMultiplier) external onlyOwner {
        require(
            _whaleFeeMultiplier <= MAX_WHALE_FEE_RATE, "max whale fee exceeded");
        taxBracketMultiplier = _whaleFeeMultiplier;
    }

    function setIDEXPair(address _iDexPair) external onlyOwner {
        iDexPair = IDexPair(_iDexPair);
    }
