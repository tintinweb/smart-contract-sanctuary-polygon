...
import "../../interfaces/IUIHelper.sol";
...
//TODO !!!!! Line 254 needs to be removed when releasing on mainnet
contract SiloFactory is Ownable, ERC721Enumerable{
    ...
    function enterSiloStrategy(uint[4] memory amount, uint siloID) external isOwner(siloID){
        ...
        (uint currentBalance, uint minimumBalance, uint riskAdjustedBalance,) = IUIHelper(uiHelper).getManagerStats(msg.sender);
        if(silo.highRiskAction()){//check that balance is above the riskadjusted minimum
            require(currentBalance > riskAdjustedBalance, "Manager Balance too low to deposit into High Risk Strategy");
        }
        else{
            require(currentBalance > minimumBalance, "Manager Balance too low to deposit into strategy");
        }
    ...
    function depositAllowed(uint _siloId) external view returns(bool){
        address user = ownerOf(_siloId);
        (uint currentBalance, uint minimumBalance, uint riskAdjustedBalance,) = IUIHelper(uiHelper).getManagerStats(user);
        ISilo silo = ISilo(siloMap[_siloId]);
        if(silo.highRiskAction()){//check that balance is above the riskadjusted minimum
    ...
    function getDefaultFeeList() public view returns(uint[4] memory){
        return defaultFeeList;
    }
...
interface IUIHelper{
    function getManagerStats(address _user) external view returns(uint currentBalance, uint minimumBalance, uint riskAdjustedBalance, uint warningBalance);
}
