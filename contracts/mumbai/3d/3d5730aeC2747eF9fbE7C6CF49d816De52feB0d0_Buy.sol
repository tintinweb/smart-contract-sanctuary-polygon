...
contract Buy is IExecutePlugin, BasePlugin {

    constructor(address _registry) {
        PLUGIN_VERSION = 1;
        PLUGIN_NAME = PluginsList.SUBSCRIPTION_BUY;
    ...
    function execute(
        ...
        (address _communityId, address _user, uint256 _amount, uint256 _paymentType, uint256 _startTime) =
        abi.decode(_data,(address, address, uint256, uint256, uint256));

        require(_communityId != address(0), "Buy: wrong community");
        checkPlugin(_version, _communityId);

        uint256 price = ICommunityBlank(_communityId).getPrice(DataTypes.PaymentType(_paymentType));
        require(price > 0, "Buy: wrong price for subscription");

        uint256 daysCount = _amount / price;
        require(IToken(registry.token()).transferFrom(_sender, _communityId, daysCount * price), "Buy: wrong transfer");
        uint256 endTime = _startTime + 1 days * daysCount;

        DataTypes.SimpleVars memory vars;
        vars.executedId = _executedId;
        vars.pluginName = PLUGIN_NAME;
        vars.version = PLUGIN_VERSION;
        vars.data = abi.encode(_communityId, _user, _paymentType, _startTime, endTime);
