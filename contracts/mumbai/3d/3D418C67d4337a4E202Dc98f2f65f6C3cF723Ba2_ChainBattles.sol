...
contract ChainBattles is ERC721URIStorage  {
    ...
    function train(uint256 tokenId, uint256 level, uint256 speed, uint256 strength, uint256 life) public {
        require(_exists(tokenId), "Please use an existing token");
        require(ownerOf(tokenId) == msg.sender, "You must own this token to train it");
        tokenIdToLevels[tokenId] = level;
        tokenIdToSpeeds[tokenId] = speed;
        tokenIdToStrengths[tokenId] = strength;
        tokenIdToLifes[tokenId] = life;
        _setTokenURI(tokenId, getTokenURI(tokenId));
    }

    function trainLevel(uint256 tokenId) public {
        require(_exists(tokenId), "Please use an existing token");
        require(ownerOf(tokenId) == msg.sender, "You must own this token to train it");
    ...
    function trainSpeed(uint256 tokenId) public {
        require(_exists(tokenId), "Please use an existing token");
        require(ownerOf(tokenId) == msg.sender, "You must own this token to train it");
        uint256 currentSpeed = tokenIdToSpeeds[tokenId];
        tokenIdToSpeeds[tokenId] = currentSpeed + 1;
        _setTokenURI(tokenId, getTokenURI(tokenId));
    }

    function trainStrength(uint256 tokenId) public {
        require(_exists(tokenId), "Please use an existing token");
        require(ownerOf(tokenId) == msg.sender, "You must own this token to train it");
        uint256 currentStrength = tokenIdToStrengths[tokenId];
        tokenIdToStrengths[tokenId] = currentStrength + 1;
        _setTokenURI(tokenId, getTokenURI(tokenId));
    }

    function trainLife(uint256 tokenId) public {
        require(_exists(tokenId), "Please use an existing token");
        require(ownerOf(tokenId) == msg.sender, "You must own this token to train it");
        uint256 currentLife = tokenIdToLifes[tokenId];
        tokenIdToLifes[tokenId] = currentLife + 1;
        _setTokenURI(tokenId, getTokenURI(tokenId));
    }
}
