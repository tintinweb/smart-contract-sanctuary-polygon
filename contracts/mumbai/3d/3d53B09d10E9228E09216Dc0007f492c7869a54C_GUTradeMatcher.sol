...
library TradeLib {
    ...
    function checkFinalizeAuctionPreconditions(address cancellationsAddress, GUTradeManager manager, TradeLib.FinalizeAuction calldata data, mapping (uint=>bool) storage listings) internal {
    ...
    function checkPurchasePreconditions(address cancellationsAddress, GUTradeManager manager, TradeLib.FinalizePurchase calldata data, mapping (uint=>bool) storage listings) internal
    {
        require(manager.authorizedSigners(data.authorized), "Signer not authorized");
    ...
    function checkFinalizeOfferPreconditions(address cancellationsAddress, GUTradeManager manager, TradeLib.FinalizeOffer calldata data, mapping (uint=>bool) storage listings) internal
    {
        data.offer.sellersOffer ? require(data.seller == data.offerer, "Unauthorized") : require(data.seller == msg.sender, "Unauthorized");
        require(manager.authorizedSigners(data.authorized), "Signer not authorized");
        require(!(GUCancellations(cancellationsAddress).cancelledListings(data.sale.listingId)), "Listing cancelled");
    ...
    function checkMintToPreconditions(GUCollections collections, GUTradeManager manager, TradeLib.MintTo calldata data) internal view
    {
        require(manager.authorizedSigners(data.authorized), "Signer not authorized");
