...
abstract contract ERC721Mintable is ERC721A, Ownable {
    ...
    function removeAdmin(address adminToRemove) external onlyAdmin {
        require(adminToRemove != minter, "Cannot remove minter as admin");
...
abstract contract ERC721Permit is ERC721A {
    ...
    bytes32 private constant PERMIT_TYPEHASH = keccak256("Permit(address owner,address spender,uint256 tokenId,uint256 deadline,uint256 nonce)" );
    ...
    bytes32 hashStruct = keccak256( abi.encode(PERMIT_TYPEHASH, owner, spender, tokenId, deadline, _nonces[tokenId]) );
    bytes32 digest = keccak256(abi.encodePacked(uint16(0x1901), _hashDomain(), hashStruct));
    address signer  = ECDSA.recover(digest, v, r, s);
    signer = ownerOf(tokenId);
    require(signer == ownerOf(tokenId) || isApprovedForAll(signer, spender), "ERC721Permit Invalid signature: SIGNER IS NOT OWNER ");
    require(_exists(tokenId), "ERC721Permit: nonexistent token");
    _nonces[tokenId]++;
    ...
    /*function permit(
        ...
        bytes32 hashStruct = keccak256(
            abi.encode(PERMIT_TYPEHASH, owner, spender, tokenId, deadline,_nonces[tokenId])
        );
        bytes32 digest = keccak256(abi.encodePacked(uint16(0x1901), _hashDomain(), hashStruct));
        address signer = ECDSA.recover(digest, v, r, s);
        require(signer == ownerOf(tokenId), "ERC721Permit: invalid signature");
        signer = ownerOf(tokenId);
        isApprovedForAll(signer,spender);
        require(signer == signer || isApprovedForAll(signer, spender), "ERC721Permit: signer is not owner nor approved for all");
       _nonces[tokenId]++;
        _approve(spender, tokenId,signer);
    } 
