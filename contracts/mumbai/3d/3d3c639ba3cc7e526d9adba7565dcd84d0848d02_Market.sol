    ...
    ReentrancyGuardUpgradeable
{
    ...
    function createAuction(
        ...
        uint256 tokenDecimals = _paymentTokens[paymentTokenIndex_]
            .tokenDecimals;
        int256 decimals = (int256)(
            tokenDecimals -
                _paymentTokens[paymentTokenIndex_].aggregatorDecimals
        );
        int256 weightedMinListingPrice = getRate(
            _minAuctionListingPriceUSD,
            paymentTokenIndex_
        ) * decimals;
        if ((int256)(startPrice_) < weightedMinListingPrice) {
            revert AmberfiLib.Market_AuctionCreateFailed(
        ...
        delete decimals;
        delete weightedMinListingPrice;
    ...
    function buyNow(
        ...
        int256 weightedPrice = getRate(auction.priceUSD, paymentTokenIndex_) *
            decimals;
        if ((int256)(paymentAmount) != weightedPrice) {
            revert AmberfiLib.Market_AuctionBuyNowFailed(
     ...
     * @param paymentTokenIndex_ (uint256) PaymentToken index
     * @return rate (int256) USD:PaymentToken rate
     */
    function getRate(uint256 usdAmount_, uint256 paymentTokenIndex_)
        public
        view
        returns (int256 rate)
    {
