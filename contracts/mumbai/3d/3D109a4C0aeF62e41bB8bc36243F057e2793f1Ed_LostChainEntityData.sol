...
contract LostChainEntityData is CustomERC2771ContextUpgradeable, UUPSUpgradeable {
    ...
    struct Weapon {
        ...
        uint256 minDamage;
        uint256 maxDamage;
        uint256 minAttackSpeed;
        uint256 maxAttackSpeed;
        uint256 minCriticalChance;
        uint256 maxCriticalChance;
    }
    ...
    struct Hero {
        ...
        string metadata;
        uint256 entityDataId;
        uint256 minBaseHealth;
        uint256 maxBaseHealth;
        uint256 minBaseDefense;
        uint256 maxBaseDefense;
        uint256 minBaseDamage;
        uint256 maxBaseDamage;
        uint256 minBaseAttackSpeed;
        uint256 maxBaseAttackSpeed;
        uint256 minBaseCriticalChance;
        uint256 maxBaseCriticalChance;
    }
    ...
    function addWeapon(
        ...
        require(minLevel >= 0, 'LCED: Min level cannot be empty');
        require(minDamage > 0, 'LCED: Min damage cannot be empty');
        require(maxDamage > 0, 'LCED: Max damage cannot be empty');
        require(minAttackSpeed > 0, 'LCED: Min attack speed cannot be empty');
        require(maxAttackSpeed > 0, 'LCED: Max attack speed cannot be empty');
        require(minCriticalChance > 0, 'LCED: Min critical chance cannot be empty');
        require(maxCriticalChance > 0, 'LCED: Max critical chance cannot be empty');
    ...
    function addArmor(
        ...
        require(minLevel >= 0, 'LCED: Min level cannot be empty');
        require(minDefense > 0, 'LCED: Min defense cannot be empty');
        require(maxDefense > 0, 'LCED: Max defense cannot be empty');
        require(minHealth > 0, 'LCED: Min health speed cannot be empty');
        require(maxHealth > 0, 'LCED: Max health speed cannot be empty');
        require(bytes(metadata).length > 0, 'LCED: Metadata link cannot be empty');
    ...
    function addHero(
        ...
        require(minBaseHealth > 0, 'LCED: Min base health cannot be empty');
        require(maxBaseHealth > 0, 'LCED: Max base health cannot be empty');
        require(minBaseDefense > 0, 'LCED: Min damage cannot be empty');
        require(maxBaseDefense > 0, 'LCED: Max damage cannot be empty');
        require(minBaseDamage > 0, 'LCED: Min attack speed cannot be empty');
        require(maxBaseDamage > 0, 'LCED: Max attack speed cannot be empty');
        require(minBaseAttackSpeed > 0, 'LCED: Min critical chance cannot be empty');
        require(maxBaseAttackSpeed > 0, 'LCED: Max critical chance cannot be empty');
        require(minBaseCriticalChance > 0, 'LCED: Min critical chance cannot be empty');
        require(maxBaseCriticalChance > 0, 'LCED: Max critical chance cannot be empty');
        require(bytes(metadata).length > 0, 'LCED: Metadata link cannot be empty');
