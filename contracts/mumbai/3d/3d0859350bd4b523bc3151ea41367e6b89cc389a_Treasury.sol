...
contract Treasury is Ownable {
   ...
   receive() external payable {}
    ...
    modifier shouldBeAuthenticated(address _addr) {
        require(isAuthenticated[_addr], "Not allowed to access withdrawal");
        _;
    }

    function withdraw(address account, uint _amount, uint8 txType) public shouldBeAuthenticated(msg.sender) {
        require(_amount > 0, 'Withdraw amount should be greated than 0');

        if(txType == 1) {
            payable(account).transfer(_amount);
        } else {
            hiveToken.transfer(account, _amount);
        }
    }

    function authenticate(address _addr) public onlyOwner {
        isAuthenticated[_addr] = true;
    }
}