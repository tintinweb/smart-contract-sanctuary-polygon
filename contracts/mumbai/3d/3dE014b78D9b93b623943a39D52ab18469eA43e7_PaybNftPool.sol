...
import "./interfaces/IPaybNftPool.sol";
import "./utils/ReentrancyGuard.sol";

contract PaybNftPool is IPaybNftPool, Ownable, ReentrancyGuard{
    address PaybMKT;
    using Address for address;
    using SafeMath for uint256;

    constructor(address _paybNftMarketplace) {
        PaybMKT = _paybNftMarketplace;
        transferOwnership(_paybNftMarketplace);
    }

    function subOwnerFeeBalance() onlyOwner() public payable override{
    }
    ...
    function withdraw(address _tokenAddress) external override nonReentrant {
        uint256 subOwnerFee = IPaybNftMarketplace(PaybMKT).getAmountEarn(msg.sender, _tokenAddress);
        if(_tokenAddress == address(0)){
            ...
            IPaybNftMarketplace(PaybMKT).setDefaultAmountEarn(msg.sender, _tokenAddress);
        }else{
            require(subOwnerFee > 0, "Earn = 0");
