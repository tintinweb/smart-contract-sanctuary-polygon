...
contract Rover is ERC721EnumerableUpgradeable, OwnableUpgradeable, ERC1155HolderUpgradeable, VRFConsumerBaseV2Upgradable {
    ...
    uint256 ixtRepairDamagedPrice;

    uint256 acRepairWornPrice;
    uint256 acRepairDamagedPrice;
    ...
    function initialize(
        ...
        ixtRepairDamagedPrice = 2.5 ether;

        acRepairWornPrice = 30 ether;
        acRepairDamagedPrice = 80 ether;
    ...
    function _vrfRequest(uint32 amount) internal returns (uint256 requestId) {
        ...
        requestId = COORDINATOR.requestRandomWords(keyHash, subscriptionId, requestConfirmations, callbackGasLimit, amount);
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        string memory image;
        if (rover.status == Status.Wrecked) {
            image = string.concat(uris[0], "/3.png");
        } else {
            if (rover.skin == 0) {
                image = string.concat(uris[0], "/", Strings.toString(uint256(rover.status)), "-", Strings.toString(uint256(rover.color)), ".png");
            } else {
        ...
        string memory color;
        if (rover.color == Color.Piercer) {
            color = "Piercer";
        } else if (rover.color == Color.Genesis) {
            color = "Genesis";
        } else {
            color = "Night";
        }
    ...
    function repairDamagedIXT(uint256 tokenId) public {
        IXT.transferFrom(msg.sender, address(this), ixtRepairDamagedPrice);
        _repair(tokenId, Status.Damaged, Status.Pristine);
    }
    ...
    function repairDamagedAC(uint256 tokenId) public {
        astroCredits.safeTransferFrom(msg.sender, address(this), acTokenId, acRepairDamagedPrice, "");
        _repair(tokenId, Status.Damaged, Status.Pristine);
    }
