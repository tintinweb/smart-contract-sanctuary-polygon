...
contract NftFactory is Ownable {
  event NftCollectionCreated(address indexed nftCollectionAddress);
  ...
  function deployCollection(
    ...
    emit NftCollectionCreated(address(collection));
  ...
  AccessControl
{
  ...
  function safeMint(address toAddress, uint256 numberOfTokens)
    public
    payable
    ...
    whenNotExceedMaxSupply(numberOfTokens)
  {
    for (uint256 i = 0; i < numberOfTokens; i++) {
      if (totalSupply() < maxSupply) {
        _safeMint(toAddress, _tokenIdCounter);
