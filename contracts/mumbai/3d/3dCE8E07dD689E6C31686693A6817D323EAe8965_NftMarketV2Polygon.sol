...
contract NftMarketV2Polygon is Ownable, ReentrancyGuard {
    ...
    // Named feeInBnb for consistency in the UI handlers, despite this returning the CKB value
    function feeInBnb() public view returns (uint) {
        if (address(priceConsumer) == address(0)) return listingFee;
        IBandPriceConsumer.ReferenceData memory data = priceConsumer.getReferenceData("CKB", "USD");
        return data.rate * listingFee;
    }
