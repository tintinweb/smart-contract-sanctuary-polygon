...
import "./transferAgentState.sol";

contract FiboTransferAgent is
    UUPSUpgradeable,
    AccessControlUpgradeable,
    PausableUpgradeable,
    FiboTransferAgentState,
    IFiboTransferAgent
{
    using SafeMathUpgradeable for uint256;

    event AssetTransferred(address indexed asset, address from, address to, uint256 quantity);
    event AssetRegistered(address indexed asset, AssetConfig assetConfig);
    event AssetUnregistered(address indexed asset);
    event GuardHashRegistered(bytes32 indexed guardHash);

    bytes32 public constant _TRANSFER_GUARD_HASH =
        keccak256("checkTransfer(address,address,address,address,uint,bytes)");

    modifier onlyAdminOrAsset(address asset) {
        require(assets[asset].registered, "not registered");
        require(
            (_msgSender() == asset) || hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
            "unauthorized"
        );
        _;
    }
    ...
    function registerAsset(address asset, bytes32[] calldata configGuardHashes)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        AssetStatus storage status = assets[asset];
        AssetGuardConfig[] storage assetGuards = status.config.guards;

        assembly {
            sstore(assetGuards.slot, configGuardHashes.length)
        }

        for (uint256 i = 0; i < configGuardHashes.length; i++) {
            assetGuards[i] = guardHashes[configGuardHashes[i]];
            require(status.config.guards[i].guardContract != address(0), "unknown guard hash");
        }

        status.registered = true;
        emit AssetRegistered(asset, status.config);
    }

    function unregisterAsset(address asset) external onlyRole(DEFAULT_ADMIN_ROLE) {
        emit AssetUnregistered(asset);
        assets[asset].registered = false;
    }

    function validateGuard(address guardAddress) internal {
        ITransferGuard guard = ITransferGuard(guardAddress);
        (bool success, bytes memory returnData) = guardAddress.call(
            abi.encode(guard.isTransferGuard.selector)
        );

        require(
            success &&
                returnData.length == 32 &&
                abi.decode(returnData, (bytes32)) == _TRANSFER_GUARD_HASH,
            string(
                abi.encodePacked(
                    StringsUpgradeable.toHexString(uint160(guardAddress), 20),
    ...
    function validateTransfer(
        ...
        // admins should be able to force any transfers, ignoring transfer guards
        if (hasRole(DEFAULT_ADMIN_ROLE, initiator)) {
            return;
        }
        ...
        for (uint256 i = 0; i < guards.length; ++i) {
            ...
            ITransferGuard(guards[i].guardContract).checkTransfer(
                asset,
                initiator,
                sender,
                receiver,
    ...
    function transfer(
        ...
        address to,
        uint256 quantity
    ) external onlyAdminOrAsset(asset) whenNotPaused {
    ...
    function getBalance(address asset, address user) external view returns (uint256 balance) {
        return assets[asset].balances[user];
    }

    function totalSupply(address asset) external view returns (uint256 supply) {
    ...
    function _computeGuardHash(AssetGuardConfig calldata guard) internal pure returns (bytes32) {
        return keccak256(abi.encode(guard));
    }

    function getGuardHash(AssetGuardConfig calldata guard) external pure returns (bytes32) {
        return _computeGuardHash(guard);
    }

    function isDefined(AssetGuardConfig calldata guard) external view returns (bool) {
        return guardHashes[_computeGuardHash(guard)].guardContract != address(0);
    }

    function defineAssetGuard(AssetGuardConfig calldata guard) external {
        validateGuard(guard.guardContract);
        bytes32 guardHash = _computeGuardHash(guard);
        guardHashes[guardHash] = guard;
        emit GuardHashRegistered(guardHash);
    }
}
...
import "./transferAgentTypes.sol";

interface IFiboTransferAgent {
    function registerAsset(address asset, bytes32[] calldata guardHashes) external;
...
import "../interfaces/transferAgentTypes.sol";

abstract contract FiboTransferAgentState {
    struct AssetStatus {
    ...
    mapping(bytes32 => AssetGuardConfig) public guardHashes;
}
