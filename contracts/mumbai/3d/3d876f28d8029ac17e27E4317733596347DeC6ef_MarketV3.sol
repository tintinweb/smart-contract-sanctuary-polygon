...
contract MarketV3 is Manager, ERC1155Holder, ERC721Holder, ReentrancyGuard {
	...
	function buy(
		...
		uint256 loyaltyFee = _getLoyaltyFee(order.tokenAddress, order.tokenId);
		uint256 exactPaymentAmount = (orderAmount * (ZOOM_FEE + xUserFeeMarket + loyaltyFee)) / ZOOM_FEE;
	...
	function acceptBid(
		...
		for (uint256 i = 0; i < _orderIds.length; i++) {
			...
			_match(
				...
				_orderIds[i],
				matchQuantity,
				(orderAmount * ZOOM_FEE) / (ZOOM_FEE + xUserFeeMarket + loyaltyFee),
				getRefData(msg.sender),
		...
		for (uint256 i = startId; i <= endId; i++) {
			...
			if (quantity > 0) {
				...
				if (tokenAddress == oldERC1155Address) {
					newOrder.tokenAddress = newERC1155Address;
				} else if (tokenAddress == oldERC721Address) {
					newOrder.tokenAddress = newERC721Address;
				} else {
		...
		for (uint256 j = startId; j <= endId; j++) {
			...
			if (quantity > 0) {
				...
				if (tokenAddress == oldERC1155Address) {
					...
					newBid.tokenAddress = newERC721Address;
				} else {
