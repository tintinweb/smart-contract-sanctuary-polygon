...
contract AkiEventRewardDispenser is Ownable {
    struct BillEnvelope {
        IERC20 token;
        string activityId;
        uint256 share;
        ...
        for (uint32 i = 0; i < activityId.length; i++) {
            for (uint32 x = 0; x < accountBook[msg.sender].length; x++) {
                ...
                if (
                    ...
                    require(
                        activity[activityId[i]].amount != 0,
                        "Invalid activity!"
                    );
                    require(
                    ...
                    require(akiUserBill.redemption == false, "Repeat rewards!");
                    require(akiUserBill.amount > 0, "Not open!");
                    akiUserBill.redemption = true;
                    ActivityEnvelope storage activityInfo = activity[
                        activityId[i]
                    ];
                    activityInfo.distribute += akiUserBill.amount;
                    activityInfo.recipient += 1;
                    activity[activityId[i]].token.transfer(
                        msg.sender,
                        akiUserBill.amount
                    );
                    emit receiveAward(
                        activity[activityId[i]].token,
                        akiUserBill.activityId,
                        akiUserBill.amount
                    );
                }
            }
    ...
    function receiveAllEventrewards(IERC20 token) public {
        BillEnvelope[] storage akiUserBillList = accountBook[msg.sender];
        require(akiUserBillList.length != 0, "No reward!");
        uint256 amount = 0;
        uint256 count = 0;

        for (uint32 i = 0; i < akiUserBillList.length; i++) {
            BillEnvelope storage akiUserBill = akiUserBillList[i];
            if (akiUserBill.redemption == false && akiUserBill.token == token) {
                require(
                    activity[akiUserBill.activityId].amount != 0,
                    "Invalid activity!"
                );
                require(
                    activity[akiUserBill.activityId].state ==
                        ActiveStatus.AWARDING,
                    "The event is not open for prize collection!"
                );
                require(akiUserBill.redemption == false, "Repeat rewards!");
                require(akiUserBill.amount > 0, "Not open!");
                require(
                    activity[akiUserBill.activityId].token == token,
                    "Currency Exception!"
                );

                akiUserBill.redemption = true;
                amount += akiUserBill.amount;
    ...
    function setTokenInfo(
        ...
        require(
            ...
            "Need to have same payment length!"
        );
        require(activity[activityId].amount != 0, "Invalid activity!");
        require(
            activity[activityId].state == ActiveStatus.WAITINGFORAWARD,
            "event is open!"
        );
        ActivityEnvelope storage activityInfo = activity[activityId];
        for (uint32 i = 0; i < payees.length; i++) {
            BillEnvelope[] storage billInfo = accountBook[payees[i]];
            BillEnvelope memory newBill;
            bool exist = false;
            for (uint256 i = 0; i < billInfo.length; i++) {
                if (hashCompareInternal(billInfo[i].activityId, activityId)) {
                    exist = true;
                    billInfo[i].token = activityInfo.token;
                    billInfo[i].activityId = activityInfo.activityId;
                    billInfo[i].share = shares[i];
                    billInfo[i].totalShare = totalShare;
                    billInfo[i].amount =
                        (activityInfo.amount / totalShare) *
                        shares[i];
                    billInfo[i].redemption = false;
                }
            }
            if (!exist) {
                newBill.token = activityInfo.token;
                newBill.activityId = activityInfo.activityId;
                newBill.share = shares[i];
                newBill.totalShare = totalShare;
                newBill.amount = (activityInfo.amount / totalShare) * shares[i];
                newBill.redemption = false;
                ActivityEnvelope storage activityInfo = activityInfo;
                activityInfo.winner += 1;
                billInfo.push(newBill);
            }
        }

        if (activityInfo.totalShare == 0) {
