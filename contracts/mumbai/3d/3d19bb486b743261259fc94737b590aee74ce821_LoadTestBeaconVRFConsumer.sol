...
contract LoadTestBeaconVRFConsumer is BeaconVRFConsumer {
    ...
    VRFBeaconTypes.RequestID[] public s_requestIDs;
        ...
        returns (VRFBeaconTypes.RequestID[] memory)
    {
        VRFBeaconTypes.RequestID[]
            memory pendingRequests = new VRFBeaconTypes.RequestID[](
                s_requestIDs.length
            );
        uint256 numPendingRequests;
        for (uint256 i = 0; i < s_requestIDs.length; i++) {
            VRFBeaconTypes.RequestID reqID = s_requestIDs[i];
            if (s_fulfillmentDurationInBlocks[s_resetCounter][reqID] == 0) {
                pendingRequests[numPendingRequests] = reqID;
                numPendingRequests++;
            }
        }
        assembly {
            mstore(pendingRequests, numPendingRequests)
        }
