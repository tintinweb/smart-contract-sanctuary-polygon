...
interface IUserReferrals {
    ...
    // Returns an array(3) of addresses of who referred an address [Level0, Level1, Level2]
...
interface ITokens {
    ...
    function getTotalTokensToMint() external returns(uint256);
    ...
    // Returns total remaining token amount available to mint
    function mint(uint256 seed, address sendTo) external returns(uint256);
}
    ...
    function pickWinner(uint newCostPerPlayer,uint newTotalPlayerLimit, uint256[3] calldata purses, uint newSideBetCost, uint seed) public onlyOwner nonReentrant {
        ...
        for (uint i = 0; i <= gamePurses.length - 1; i++) {
            if (gamePurses[i] > 0){
                ...
                (winningAmount, totalPaidAfterRef) = payReferrals(winningAmount, winner);

                // Update houseCut to subtract total paid after referrals
                houseCut -= totalPaidAfterRef;
                ...
                // Send the event
                emit PlayerWin(winnerAddresses[i], winningAmount);
            }
        }

        // Pay the house it's cut for future game investment
        Address.sendValue(payable(admin), ((gamePrizePool * houseCut) / 100));
        ...
        if (sideBetsCheck[triWinners] > 0){
            ...
            // Update side bet winners balance
            ...
            // Pay the referrals if there are any and update the winning amount as adjusted
            (winSideBetPoolBal, totalPaidAfterRef) = payReferrals(winSideBetPoolBal, sideBetWinner);
            ...
            // Transfer the remainder to house after deducting all payouts and reserve for new prize pool
            uint256 amountToCutHouse = sideBetPrizePool - newSideBetPrizePool - totalPaidAfterRef;
            Address.sendValue(payable(admin), amountToCutHouse);
        ...
        // Send the NFT if available las as to not get DoS'd
        if (tokens.tokensRemaining() > 0){
            tokens.mint(seed, firstPlace);
        }
    }


    function payReferrals(uint256 winningAmount, address winner) internal returns(uint256, uint256){
        ...
        if (referrers.length > 0) {
            ...
            uint256 refCut = (winningAmount / 9);
            ...
            userProfiles.updateTokensWon(referrers[0], gamePaymentToken, winningAmount);

            // Level 2
            if (referrers.length > 1){
                winningAmount = (refCut * 10) / 100;

                totalPaidOut += winningAmount;

                // Pay the referrer their 1%
                balanceOf[referrers[1]] += winningAmount;

                // Update the user's global profile with their referral winnings
                userProfiles.updateTokensWon(referrers[1], gamePaymentToken, winningAmount);
            }

            // Level 3
            if (referrers.length > 2){
                winningAmount = (refCut * 5) / 100;

                totalPaidOut += winningAmount;

                // Pay the referrer their 0.5%
                balanceOf[referrers[2]] += winningAmount;

                // Update the user's global profile with their winnings
                userProfiles.updateTokensWon(referrers[2], gamePaymentToken, winningAmount);
            }

            // Give player extra 2.5% back for having a referrer
            winningAmount = (refCut * 25) / 100;

            totalPaidOut += winningAmount;
        }

        return(winningAmount, totalPaidOut);
    }
