...
contract CRPool is ICRPool, PCToken, OwnableUpgradeable, BalancerReentrancyGuard {
    ...
    function joinPool(
        ...
        address sender
    ) external logs nonReentrant needsBPool lockUnderlyingPool {
        require(!rights.canWhitelistLPs || _liquidityProviderWhitelist[sender], "ERR_NOT_ON_WHITELIST");

        // Delegate to library to save space

        // Library computes actualAmountsIn, and does many validations
        // Cannot call the push/pull/min from an external library for
        // any of these pool functions. Since sender can be anybody,
    ...
    function joinswapExternAmountIn(
        ...
        uint256 minPoolAmountOut,
        address sender
    ) external logs nonReentrant needsBPool returns (uint256 poolAmountOut) {
        require(!rights.canWhitelistLPs || _liquidityProviderWhitelist[sender], "ERR_NOT_ON_WHITELIST");
    ...
    function joinswapPoolAmountOut(
        ...
        uint256 maxAmountIn,
        address sender
    ) external logs nonReentrant needsBPool returns (uint256 tokenAmountIn) {
        require(!rights.canWhitelistLPs || _liquidityProviderWhitelist[sender], "ERR_NOT_ON_WHITELIST");
