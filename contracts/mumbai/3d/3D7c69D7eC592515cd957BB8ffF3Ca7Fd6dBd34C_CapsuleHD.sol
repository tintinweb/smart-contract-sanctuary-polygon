...
contract CapsuleHD is ERC721A, Ownable, Pausable, PaymentSplitter {
    ...
    function mint(
        ...
        require(checkValidity(_merkleProof, params.merkleRootForWhitelist), "HD: address not whitelisted");
        ...
        _checkPayment(_amountOnyx, _amountGold, _amountDiamond);
        _safeMint(_to, _amountOnyx + _amountGold + _amountDiamond);
        emit Mint(_to, _amountOnyx, _amountGold, _amountDiamond, _totalSupply + 1);
    }

    function freeMintDiamond(
        address _to,
        bytes32[] calldata _merkleProof
        ...
        uint256 _totalSupply = totalSupply();
        require(
            _totalSupply + 1 <= maxSupply,
            "HD: total supply limit"
        );
        require(
            amountOfCapsulesMintedPerCategory.Diamond + 1 <= 
                maxSupplyPerCategory.Diamond,
            "HD: Diamond count per mint limit"
        );
        require(
            capsulesMintedPerAddress[_to].Diamond + 1 <=
                params.maxCapsulesPerAddress.Diamond,
            "HD: max count per address Diamond limit"
        );
        require(
            freeMintClaimed[_to] == false,
            "HD: free mint already claimed"
        );
        require(checkValidity(_merkleProof, params.merkleRootForFreeMint), "HD: address not whitelisted");
        ...
        _safeMint(_to, 1);
        emit FreeMintDiamond(_to, _totalSupply + 1);
    }
    ...
    function _checkPayment(uint256 _amountOnyx, uint256 _amountGold, uint256 _amountDiamond) private view {
        uint256 _amount = _amountOnyx * params.pricePerCategory.Onyx + _amountGold 
            * params.pricePerCategory.Gold + _amountDiamond * params.pricePerCategory.Diamond;
        require(msg.value == _amount, "HD: inexact funds sent");
    }
