...
interface ConsiderationInterface {
    ...
    function fulfillOrder(
        ...
        PIXMerkleParam calldata pixMerkleParam
    )
        external
        payable
        returns (bool fulfilled);
...
contract ConsiderationBase is ConsiderationEventsAndErrors {
    ...
    /// @dev This modifier only allows mods to access a function
    modifier onlyOwner() {
        require(msg.sender == owner, "NON_OWNER");
        _;
    }
    /**
     * @dev Derive and set hashes, reference chainId, and associated domain
    ...
    function setMerkleMinter(address _pixMerkleMinter) external onlyOwner {
...
uint256 constant EIP712_OfferItem_size = 0xe0;
...
enum ItemType {
    ...
    // 6: ERC721 items that are going to be minted by merkle minter
    ERC721MM
}
...
struct PIXMerkleParam {
    IPIX.PIXInfo merklePixInfo;
    bytes32 merkleRoot;
    bytes32[] merkleProof;
}
...
contract CriteriaResolution is CriteriaResolutionErrors {
        ...
        returns (bool withCriteria)
    {
        ...
        assembly {
            withCriteria := and(gt(itemType, 3), not(eq(itemType, 6)))
        }
    }
    ...
    AmountDeriver
{
    ...
    function _validateAndFulfillAdvancedOrder(
        ...
        if(recipient == msg.sender) {
            ...
            itemType = orderParameters.offer[0].itemType;
        }
        else {
            offerer = msg.sender;
            itemType = orderParameters.consideration[0].itemType;
        }

        if (itemType == ItemType.ERC721MM) {
            bytes32 pixHash = orderParameters.offer[0].pixHash;
            PIXMerkleParam memory pixMerkleParam = pixMerkleParam;

            uint256 tokenId = _verifyMerkleHashAndMintPix(
                offerer,
                recipient,
                pixHash,
                pixMerkleParam.merklePixInfo,
                pixMerkleParam.merkleRoot,
                pixMerkleParam.merkleProof
            );

            orderParameters.offer[0].identifierOrCriteria = tokenId;
            if(recipient == msg.sender) _transferERC20(recipient, orderParameters.consideration);
            else {
                ConsiderationItem[] memory considerationItems = new ConsiderationItem[](1);
                orderParameters.consideration[0].itemType = orderParameters.offer[0].itemType;
                orderParameters.consideration[0].token = orderParameters.offer[0].token;
                orderParameters.consideration[0].identifierOrCriteria = orderParameters.offer[0].identifierOrCriteria;
                orderParameters.consideration[0].startAmount = orderParameters.offer[0].startAmount;
                orderParameters.consideration[0].endAmount = orderParameters.offer[0].endAmount;
                orderParameters.consideration[0].recipient = payable(msg.sender);
                _transferERC20(recipient, orderParameters.consideration);
            }
    ...
    function _verifyMerkleHashAndMintPix(
        address seller,
        address buyer,
        bytes32 merkleHash,
        IPIX.PIXInfo memory merklePixInfo,
        bytes32 merkleRoot,
        ...
        IPIX.PIXInfo memory info = merklePixInfo;
        require(
            merkleHash ==
                keccak256(abi.encodePacked(_seller, info.pixId, info.category, info.size)),
            "INVALID_PIX_INFO"
        );
    ...
    function _transferERC20(
        ...
        uint256 len = considerationItems.length;

        for (uint256 i = 0; i < len; ++i) {
            address token = considerationItems[i].token;
            uint256 startAmount = considerationItems[i].startAmount;
            address recipient = considerationItems[i].recipient;

            IERC20Upgradeable(token).safeTransferFrom(buyer, recipient, startAmount);
        }
    }
 ...
 * @notice PIXMarketplace is a generalized ETH/ERC20/ERC721/ERC1155 marketplace. It
 ...
 */
contract PIXMarketplace is Consideration {
    /**
     * @notice Derive and set hashes, reference chainId, and associated domain
     *         separator during deployment.
     *
    ...
    function _name() internal pure override returns (string memory) {
        ...
        assembly {
            mstore(0x20, 0x20)
            mstore(0x47, 0x0d5049584d61726b6574706c616365)
            return(0x20, 0x60)
        }
    }
