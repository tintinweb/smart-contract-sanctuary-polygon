...
contract NFTV3 is NFTV2 {
    event TokenUriUpdated(address owner, uint256 tokenID, string tokenURI);
    function updateTokenUriV3(uint tokenId, string memory tokenURI) public {
...
contract NFTV2 is NFT {
    address public marketplaceAddressV2;
    function approveAndListOnsSaleAll(uint[] memory itemId, uint price, uint[] memory tokenId) public payable {
        require(msg.value >= nFTMarket.getListingPrice() * itemId.length, "Amount must be bigger than listing fees (.0025eth * numbers)");
        for (uint i=0;i<itemId.length;i++){  
            //approve(marketplaceAddress, tokenId);        
            require(nFTMarket.getIdToMarketItemOwner(itemId[i]) == msg.sender && nFTMarket.getIdToMarketItemTokenID(itemId[i]) == tokenId[i], 'only owner can put this item on sale');
            ...
            safeTransferFrom(msg.sender, marketplaceAddressV2, tokenId[i]);
            payable(nFTMarket.owner()).transfer(msg.value);
            nFTMarket.calcEarnedFeeAmounts(msg.value);
            nFTMarket.listItemOnSaleFromNFTContract(itemId[i], price, msg.sender);
            emit MarketItemChanged(msg.sender, marketplaceAddressV2, itemId[i], tokenId[i]);

        }
        
    }
    function setMarketAddressV2(address _marketplaceAddressV2) external onlyOwner {
        marketplaceAddressV2 = _marketplaceAddressV2;
    }
...
contract NFT is Initializable, ERC721URIStorageUpgradeable, UUPSUpgradeable, OwnableUpgradeable{
    ...
    function initialize(address deployedMarketplaceAddress, NFTMarket _nFTMarket)  public initializer {
