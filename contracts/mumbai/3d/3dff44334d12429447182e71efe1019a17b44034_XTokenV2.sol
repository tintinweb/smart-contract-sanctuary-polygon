    ...
    ReentrancyGuardUpgradeable
{
    ...
    uint256 private _maxBuyAmountPerAccount;
    mapping(address => uint256) private _buyAmountPerAccount;
    ...
    function setMaxBuyAmountPerAccount(uint256 maxAmount) external onlyOwner {
        _maxBuyAmountPerAccount = maxAmount;
    }
    ...
    function mintlistSaleMint(
        ...
        if (!mintlisted(msg.sender, proof)) revert NotMintlisted();

        uint256 price = _mintlistSaleTokenPrice * amount;
        if (msg.value < price) revert InsufficientValue();
        if (_buyAmountPerAccount[msg.sender] + amount > _maxBuyAmountPerAccount) revert BuyAmountCapped();
        ...
        _buyAmountPerAccount[msg.sender] += amount;
    ...
    function _checkSignature(bytes32 messageHash, bytes calldata signature) private view returns (bool) {
        return ECDSA.recover(messageHash, signature) == _signer;
    }
    ...
    function buyAmountPerAccount(address buyer) external view returns (uint256) {
        return _buyAmountPerAccount[buyer];
    }

    function maxBuyAmountPerAccount() external view returns (uint256) {
        return _maxBuyAmountPerAccount;
    }
    ...
    function mintlisted(address wallet, bytes32[] calldata proof) public view returns (bool) {
        return MerkleProofUpgradeable.verify(proof, _root, keccak256(abi.encodePacked(wallet)));
    }
