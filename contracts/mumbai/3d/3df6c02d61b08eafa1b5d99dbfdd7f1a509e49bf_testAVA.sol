...
contract testAVA is Context, IBEP20, Ownable 
{
    using SafeMath for uint256;
    using Address for address;

    address public marketingWalletAddress = 0x5C95256Af7afA178689d6A052f21bE6dE4f77fA0; // Marketing wallet Address.
    address public deadWalletAddress = 0x000000000000000000000000000000000000dEaD; // Dead wallet address. 
    ...
    uint256 private _tTotal = 22222222 * 10**7;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "testAVA"; 
    string private _symbol = "testAVA";
    ...
    uint256 public _totalFee = _taxFee.add(_liquidityFee).add(_marketingFee).add(_burnFee);

    uint256 public _maxTxAmount = _tTotal.div(100).mul(1); 
    uint256 private minimumTokensBeforeSwap = 40000 * 10**7;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private 
    {
        ...
        amount = amount.sub(marketingTokens).sub(burnTokens);
        ...
        {
            _transferStandard(sender, marketingWalletAddress, marketingTokens);
        }
        if(burnTokens > 0)
        {
            _transferStandard(sender, deadWalletAddress, burnTokens);
        }       
        restoreAllFee();
    ...
    {
        _amount = _amount.mul(100).div(100-_marketingFee-_burnFee);
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() 
    {
        ...
        _totalFee = _taxFee.add(_liquidityFee).add(_marketingFee).add(_burnFee);
        require(_totalFee<=15, "Too High Fee");
    ...
    {
        uint256 tokens=(((msg.value*(10**7))/rate));
