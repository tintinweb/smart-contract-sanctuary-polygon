...
contract MintNft is ERC721, ICrossTalkApplication {
   address public admin;
    ...
    function transferCrossChain(
        ...
        bytes memory payload = abi.encode(TransferParams(_nftId, CrossTalkUtils.toBytes(_recepient)));
        uint64 expiryTimestamp = uint64(block.timestamp) + expiryDurationInSeconds;

        Utils.DestinationChainParams memory destChainParams = Utils.DestinationChainParams(destGasLimit, destGasPrice, chainType, chainId, "0x");
        Utils.RequestArgs memory requestArgs = Utils.RequestArgs(expiryTimestamp, false);
    ...
    function handleRequestFromSource(
        ...
        require(keccak256(srcContractAddress) == keccak256(ourContractOnChains[srcChainType][srcChainId]), "ERR:CONTRACT_NOT_FOUND");

        TransferParams memory transferParams = abi.decode(payload, (TransferParams));
        _mint(CrossTalkUtils.toAddress(transferParams.recipient), transferParams.nftId);
