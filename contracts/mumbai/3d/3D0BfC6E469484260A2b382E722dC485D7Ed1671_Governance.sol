...
contract Governance is Governor, GovernorCompatibilityBravo, GovernorVotes, GovernorVotesQuorumFraction, GovernorTimelockControl {
    ...
    constructor(IVotes _token, IFarming _farm, TimelockController _timelock, ITokenTimeLockFactory _tokenTimeLockFactory)
        Governor("Governance")
        GovernorVotes(_token)
        GovernorVotesQuorumFraction(4)
        GovernorTimelockControl(_timelock)
    {
        regulation.token = _token;
    ...
    function setToken(IVotes _token) public onlyGovernance {
        require(address(_token) != address(0), "setToken: new token address is zero");
        regulation.token = _token;
    }

    function setFarm(IFarming _farm) public onlyGovernance {
        require(address(_farm) != address(0), "setFarm: new farm address is zero");
        farm = _farm;
    }
