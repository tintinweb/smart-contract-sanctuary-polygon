...
contract protoKol is iProtoKol {
   ...
   */
    function investInCampaign(
        ...
        _investment = (_investment * 10**IERC20(tokenAddress).decimals()) / 10**IERC20(tokenAddress).decimals();
        ...
        if (campaigns[_campaignId].campaignData.preSaleToken != _USDT) {
            ...
            if (tokenAddress == kolToken) {
                ...
                amountInUsdt = amountInUsdt - 50 * 10**IERC20(path[1]).decimals(); //hardcoded
                ...
                // console.log("378 Investment", _investment);
   ...
   */
    function depositPreSaleTokens(uint256 _campaignId, uint256 _amount) external {
        ...
        // campaigns[_campaignId].campaignData.preSaleBalance += _amount;
    ...
    function claimPreSaleTokens(
        ...
        if (campaigns[_campaignId].vestingData.isVestingInEnabled) {
            ...
            uint256 totalReward = (investmentDetails.eligibleReward * 10**IERC20(_USDT).decimals() * progress) /
                (10000 * 10**IERC20(_USDT).decimals());
            ...
            // campaigns[_campaignId].campaignData.preSaleBalance -= _reward;
            ...
            // campaigns[_campaignId].campaignData.preSaleBalance -= eligibleRwd;
   ...
   */
    function claimBackInvestmentByKOL(
        ...
        // uint256 usdtPricePerKOL = IOracle(oracle).fetch(kolToken);
        // usdtPricePerKOL = usdtPricePerKOL * _invest;
        // usdtPricePerKOL = usdtPricePerKOL / 1e18;
        // console.log("usdtPricePerKOL", usdtPricePerKOL);
        ...
        _invest = campaigns[_campaignId].campaignData.isKolInvestment == true
            ? ((_invest / 10**18) * 10**IERC20(_USDT).decimals())
            : _invest;

        _swapRouter(_invest, 1, path, msg.sender);
   ...
   */
    function _blackListedKOLFundsHandling(
        ...
        uint256 balance = campaigns[_campaignId].campaignData.enteredInvestmentAgainstMarketingBudget; //IPST(preSale).balanceOf(address(this)); //change here1
        if (balance < _reward) {
   ...
   */
    ...
    function updateAdmin(address _newAddress, bool isUpdateAdmin) external onlyAdmin {
        if (isUpdateAdmin) {
            admin = _newAddress;
        } else {
