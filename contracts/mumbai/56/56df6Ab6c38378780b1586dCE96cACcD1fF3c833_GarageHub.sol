...
contract GarageHub is ERC721, Ownable {
    using Counters for Counters.Counter;

    Counters.Counter private _nextTokenId;

    uint256 public maxSupply = 5000;
    uint256 public carPrice = 0.0002 ether;
    ...
    mapping(uint256 => Car) private tokenIdCar;

    Model[] private models;
    Color[] private cardColors;

    uint16[][2] private traitWeights;

    function setModels(Model[3] memory types) private {
        for (uint8 i = 0; i < types.length; i++) {
            models.push(types[i]);
        }
    }

    function setCardColors(Color[8] memory colors) private {
        for (uint8 i = 0; i < colors.length; i++) {
            cardColors.push(colors[i]);
        }
    }

    constructor() ERC721('Garage Hub', 'GH') {
        // Start at token 1
    ...
    function createTokenIdCar(uint256 tokenId) public view returns (Car memory) {
        uint256 pseudoRandomBase = uint256(keccak256(abi.encodePacked(blockhash(block.number - 1), tokenId)));

        return
            Car({
                cardColor: uint16(uint16(pseudoRandomBase) % 8),
                model: weightedRarityGenerator(uint16(uint16(pseudoRandomBase >> 10) % maxSupply), 5)
            });
    }


    function getTokenIdCarMetadata(Car memory car) public view returns (string memory metadata) {
        metadata = string(
            abi.encodePacked(
                metadata,
                '{"trait_type":"Model", "value":"',
                models[car.model].name,
                '"},',
                '{"trait_type":"Year", "value":"',
                models[car.model].year,
                '"},',
                '{"trait_type":"Color", "value":"',
                models[car.model].color,
                '"},'
            )
        );
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        Car memory car = tokenIdCar[tokenId];
                    ...
                    Base64.encode(
                        bytes(
                            string(
                                abi.encodePacked(
                                    '{"name": "Garage Hub NFT #',
                                    GarageHubLibrary.toString(tokenId),
                                    '", "description": "Garage Hub collection.", "image": "https://ipfs.io/ipfs/QmTDkAXeQnFGhFrQbNvtE4DhcqrqBcVor11hSYU3HTc3Qe/',
                                    models[car.model].carId,
                                    '.png,',
                                    '","attributes":',
                                    getTokenIdCarMetadata(car),
                                    '}'
                                )
                            )
                        )
    ...
    function internalMint(uint256 numberOfTokens) private {
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
            ...
            tokenIdCar[tokenId] = createTokenIdCar(tokenId);
    ...
    function mint(uint256 numberOfTokens) external payable {
        ...
        require(msg.value >= numberOfTokens * carPrice, 'Wrong ETH value sent.');

        internalMint(numberOfTokens);
    }

    function setCarPrice(uint256 newCarPrice) external onlyOwner {
