...
import "../includes/interfaces/IBandPriceConsumer.sol";
import "../includes/utils/ReentrancyGuard.sol";

contract NftMarketCollectionManagerPolygon is Ownable, ICollection, ReentrancyGuard {
    ...
    IBandPriceConsumer                      public priceConsumer;       // The price consumer
    ...
    constructor (address _treasury, address _priceConsumer, uint _createFee) {
        ...
        priceConsumer = IBandPriceConsumer(_priceConsumer);
    ...
    event MarketOwnerRevoked(address nft, bool lockedOut);
    event UpdateCreateFee(uint fee);
    event UpdateFeeWhitelist(address wallet, bool whitelisted);
    event UpdateCollectionMetadata(uint id);
    ...
    function feeInBnb() public view returns (uint) {
        if (address(priceConsumer) == address(0)) return createFee;
        IBandPriceConsumer.ReferenceData memory data = priceConsumer.getReferenceData("CKB", "USD");
        return data.rate * createFee;
    }
    ...
    function setPriceConsumer(address _priceConsumer) public onlyOwner() { priceConsumer = IBandPriceConsumer(_priceConsumer); }

    // Function to set the create fee
    function setCreateFee(uint _fee) public onlyOwner() { 
        createFee = _fee; 
        emit UpdateCreateFee(_fee);
    }
    ...
    function setMarketOwnerRevoked(address _nft, bool _lockedOut) public {
        ...
        emit MarketOwnerRevoked(_nft, _lockedOut);
    }
    ...
    function setMetadata(uint _id, string[] memory _keys, string[] memory _values) public {
        ...
        emit UpdateCollectionMetadata(_id);
    }
