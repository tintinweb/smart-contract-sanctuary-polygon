...
// TODO: 1. extract bitfield logic into library
//       2. extract random logic into contract with randomRequest modifier which will increment internal seed variable
//       3. Collection trait
    ...
    AccessControlUpgradeable
{
    ...
    // TODO: benchmark this approach and supplying changes, e.g. [1,1,3,2,1]
    /*
     * Traits generation uses weighted random, which means specifying the probability for each trait value.
     * This probability is represented as an array of N increasing numbers, which divide the segment (bounded by 0 and the last weight) into N smaller segments.
    ...
    function initialize(address duckiesAddress) public initializer {
        ...
        // Class
        traitWeights.push([74, 94, 99, 100]);
        // Head
        traitWeights.push([0, 9, 18, 25, 31, 37, 42, 47, 52, 57, 62, 67, 72, 77, 82, 86, 90, 94, 97, 100]);
        // Body
        traitWeights.push([0, 17, 33, 47, 61, 73, 83, 92, 100]);
        // Background
        traitWeights.push([1, 2, 3, 4]);
        // Element
        traitWeights.push([1, 2, 3, 4, 5]);
        // Eyes
        traitWeights.push([8, 15, 21, 27, 33, 38, 43, 48, 53, 58, 63, 67, 71, 75, 79, 84, 89, 92, 95, 98, 100, 102]);
        // Beak
        traitWeights.push([15, 29, 43, 54, 64, 73, 81, 88, 95, 101]);
        // Wings
        traitWeights.push([20, 39, 54, 67, 79, 90, 100]);
        // UniquenessIndex is presented as a Counter, and makes sense only for SuperLegendary NFTs

        meldWeights = [6, 11, 15, 18, 20, 21];
        meldingZombeakWeights = [[10, 100], [5, 100], [2, 100]];
    ...
    function mintPack(uint8 amount) external {
        ...
        for (uint256 i = 0; i < amount; i++) {
            (uint256 tokenId, uint256 gene) = _mintFromSeed(_randomSaltedTransactionSeed(i));
            emit Minted(tokenId, gene, msg.sender, _getChainId());
        }
    }

    function meld(uint256[5] calldata meldingTokenIds) external {
        ...
        uint256 seed = _randomTransactionSeed();

        uint256 meldedGene = _meldGenesConsideringSeed(genesToMeld, seed);
    ...
    function _mintFromSeed(uint256 seed) internal returns (uint256 tokenId, uint256 gene) {
        gene = _generateGeneFromSeed(seed);
    ...
    function _generateGeneFromSeed(uint256 seed) internal view returns (uint256) {
        uint256 gene = 0;

        // generate class
        uint8 class = _generateTraitFromSeed(Gene.Traits.Class, seed.getTrait(Gene.Traits(0)));
        ...
        for (; traitIdx < traitWeights.length; traitIdx++) {
            uint8 trait = _generateTraitFromSeed(Gene.Traits(traitIdx), seed.getTrait(Gene.Traits(traitIdx)));
    ...
    function _generateTraitFromSeed(Gene.Traits trait, uint256 seed) internal view returns (uint8) {
        return _randomWeightedNumber(traitWeights[uint8(trait)], seed);
    }

    // internal melding

    function _meldGenesConsideringSeed(uint256[5] memory genes, uint256 seed) internal view returns (uint256) {
        ...
        // TODO: check other requirements related to specs
        if (meldingClass == Gene.Classes.Legendary) {
            ...
            // Common, Rare, Epic
            // TODO: review randomness logic
            if (_checkZombeakWithSeed(meldingClass, seed)) {
        ...
        for (uint256 traitIdx = 1; traitIdx < traitWeights.length; traitIdx++) {
            uint8 meldedTrait = _meldTraitsConsideringSeed(
                genes,
                Gene.Traits(traitIdx),
                seed.getTrait(Gene.Traits(traitIdx))
            );
    ...
    function _meldTraitsConsideringSeed(
        uint256[5] memory genes,
        Gene.Traits trait,
        uint256 seed
    ) internal view returns (uint8) {
        uint8 meldedTraitIdx = _randomWeightedNumber(meldWeights, seed);

        // mutation, return upgraded best trait value
        if (meldedTraitIdx == 5) {
            uint8 bestTraitValue = _maxTrait(genes, trait);
            return bestTraitValue == traitWeights[uint8(trait)].length ? bestTraitValue : bestTraitValue + 1;
        }
    ...
    function _checkZombeakWithSeed(Gene.Classes class, uint256 seed) internal view returns (bool) {
        if (uint8(class) <= uint8(Gene.Classes.Epic)) {
            return _randomWeightedNumber(meldingZombeakWeights[uint8(class)], seed) == 0;
        } else {
            return false;
        }
    }
    ...
    function _randomTransactionSeed() internal view returns (uint256) {
        return uint256(keccak256(abi.encode(salt, block.timestamp)));
    }

    function _randomSaltedTransactionSeed(uint256 salt_) internal view returns (uint256) {
        return uint256(keccak256(abi.encode(salt_, _randomTransactionSeed())));
    }

    function _randomMaxNumber(uint256 seed, uint8 max) internal view returns (uint8) {
        return uint8(uint256(keccak256(abi.encode(seed, _randomTransactionSeed()))) % max);
    }

    function _randomWeightedNumber(uint8[] memory weights, uint256 seed) internal view returns (uint8) {
        // generated number should be strictly less than right segment boundary  \/
        uint8 randomNumber = _randomMaxNumber(seed, weights[weights.length - 1] - 1);
