...
import {DigiNFT} from "./DigiNFTnbu.sol";
 ...
 contract DigiNFTnbuMultiMint {
    ...
    function bulkMint(
        ...
        uint[] memory mintedTokenIds = new uint[](qtyToMint.length);
        uint mintTokenId = 0;
        for (uint i = 0; i < qtyToMint.length; i++) {
            for (uint j = 0; j < qtyToMint[i]; j++) {
                mintedTokenIds[mintTokenId] = DigiNFT(digiNFTContractAddress).mint(wallet, cardName[i], string(abi.encodePacked(baseUri, cardImage[i])), cardPhysical[i], string(abi.encodePacked(baseUri, tokenUriMetadata[i])));
                mintTokenId = mintTokenId + 1;
            }
        }
