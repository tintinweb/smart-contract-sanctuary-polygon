...
contract UTDigitalRFactory is Ownable, BlackHolePrevention {
    ...
    uint8       public constant     traitType            = 6;            // Digital redeemables
    ...
    struct inputTraitStruct {
        ...
        address collectionToRedeemFrom;             // Token contract address (ERC721Enumerable)
    }
    ...
    function addTrait(
        ...
        require(
            ...
            "UTDigitalRFactory: Only community admin may add traits"
        );
        ...
        if(controllerAddr == address(0)) {
            ...
            // Deploy _controller_
            controllerCtr = new UTDigitalRedeemController(ERC721, address(traitRegistry), 0, 0);
        ...
        // Deploy _storage_
        UTDigitalRedeemStorage storageCtr = new UTDigitalRedeemStorage(address(traitRegistry), traitId, _inputTrait.maxTokensToRedeem, _inputTrait.collectionToRedeemFrom);
...
contract UTDigitalRedeemStorage is UTGenericDroppableStorage {
    ...
    uint8     public immutable MAX_TOKENS_TO_REDEEM;    // Number of NFTs the user may ask for with this trait - how many tokens can be claimed (in any chunks)
    address   public immutable collectionToRedeemFrom;  // Token contract address (ERC721Enumerable)
    ...
    constructor(
        ...
        address _collectionToRedeemFrom             // Token contract address (ERC721Enumerable)

    ) UTGenericStorage(_registry, _traitId) {
        MAX_TOKENS_TO_REDEEM = _maxTokensToRedeem;
...
//     function MAX_TOKENS_TO_REDEEM() external view returns (uint8);
// }
...
contract UTDigitalRedeemController is UTGenericTimedController, UTDigitalRedeemVault {
    ...
    function setValue(uint16[] memory tokenId, uint8 value, address traitStorage) public {
        ...
        for(uint8 i = 0; i < tokenId.length; i++) {
            ...
            if(currentValue >= TRAIT_OPEN_VALUE && value > currentValue && value <= (TRAIT_OPEN_VALUE + storageCtr.MAX_TOKENS_TO_REDEEM()) ) {
                // From state 1 - "Open to redeem" to any state till (1 + storage.MAX_TOKENS_TO_REDEEM) - Only owner of the NFT can change it
                require(
                ...
                _requestRandomDropFromVault(storageCtr.collectionToRedeemFrom(), currentTokenId, traitStorage, tokensToDrop );
...
abstract contract UTDigitalRedeemVault is ERC721Holder {
    string                         public constant  REGISTRY_KEY_RANDOM_CONTRACT = "RANDOMV2_SSP_TRAIT_DROPPER";
    uint8                                 constant  TRAIT_OPEN_VALUE = 1;
    uint32                         public immutable community_id;
    ...
    event vaultDropFinalized(uint32 index);

    constructor(uint32 _community_id) {
        community_id = _community_id;
    }
     ...
     */
    function _requestRandomDropFromVault(address _erc721, uint16 _tokenId, address _traitStorage, uint8 _tokensToDrop) internal {

        // console.log("_requestRandomDropFromVault: ", _erc721, _tokenId, _traitStorage);
    ...
    function process(uint256 _random, uint256 _requestId) external {
        ...
        // console.log("process: ", _random, _requestId);
        // console.log("rnd contract: ", GalaxisRegistry.getRegistryAddress(REGISTRY_KEY_RANDOM_CONTRACT));

        require(msg.sender == GalaxisRegistry.getRegistryAddress(REGISTRY_KEY_RANDOM_CONTRACT), "UTDigitalRedeemVault: process() Unauthorised caller");
    ...
    /**
     * @dev Called in the VRF callback - transfers the randomly picked token from the vault to the user
     */
    function transferRandom(uint32 _dropId) internal {

        dropStruct memory drop = drops[_dropId];

        // console.log("from", msg.sender, address(this));
        IERC721Enumerable  token = IERC721Enumerable(drop.erc721);
        require(token.supportsInterface(type(IERC721Enumerable).interfaceId), "UTDigitalRedeemVault: not an ERC721Enumerable");
        uint256 balance = token.balanceOf(address(this));
        // console.log("balance:", balance);
        require(balance > 0, "UTDigitalRedeemVault: No available tokens in vault");
        uint256 tokenPos = drop.randomNumber % balance;
        uint256 tokenId = token.tokenOfOwnerByIndex(address(this), tokenPos);
        token.safeTransferFrom(address(this), drop.recipient, tokenId);
        ...
        require( newTraitValue <= TRAIT_OPEN_VALUE + traitStorage.MAX_TOKENS_TO_REDEEM(), "UTDigitalRedeemVault: claim over limit");
    ...
    /**
     * @dev Return all open drops
     */
    function getActiveDrops() public view returns (dropStructReturn[] memory)
    {
        uint16 retCount;

        // Count return length
        for(uint16 i = 1; i <= currentDropCount; i++) {       
            if (!drops[i].randomProcessed) {
                retCount++;
            }
        }
        ...
        uint16 pos;
        dropStruct memory drop;

        for(uint16 i = 1; i <= currentDropCount; i++) {       
            if (!drops[i].randomProcessed) {
                drop = drops[i];
                retval[pos] = dropStructReturn(i , drop.requestId, drop.erc721, drop.traitStorage, drop.tokenId, drop.recipient,
                                                   drop.tokensToDrop, drop.randomNumber, drop.randomReceived, drop.randomProcessed);
                pos++;
            }
        }
        return retval;
    ...
    function onERC721Received(
        ...
        // console.log("onERC721Received: ", _operator, _from, _tokenId);
        require(IERC721Enumerable(msg.sender).supportsInterface(type(IERC721Enumerable).interfaceId), "UTDigitalRedeemVault: not an ERC721Enumerable");

        tokenContracts.add(msg.sender);
    ...
    function notifyVault(address tokenContractAddress) external {
        require(IERC721Enumerable(tokenContractAddress).supportsInterface(type(IERC721Enumerable).interfaceId), "UTDigitalRedeemVault: not an ERC721Enumerable");
    ...
    function getAllVaults() public view returns (address[] memory)
    {
        ...
        // address[] memory retval = new address[](vaultLength);

        // for(uint16 i = 0; i < vaultLength; i++) {       
        //     retval[i] = getVaultAt(i);
        // }
        // return retval;
    }
