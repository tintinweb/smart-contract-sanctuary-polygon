...
contract TokenStaking is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //   1. The pool's `accSBCPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. SBCs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that SBCs distribution occurs.
        uint256 accSBCPerShare;   // Accumulated SBCs per share, times 1e18. See below.
        uint256 totalDeposits;    // Total tokens deposited in the pool.
    }

    // The SBC TOKEN!
    SBCToken public sbc;
    // SBC tokens created per block.
    uint256 public sbcPerBlock;
    ...
    // The block number when SBC mining starts.
    ...
    constructor(
        SBCToken _sbc,
        uint256 _startBlock,
        uint256 _sbcPerBlock
    ) {
        sbc = _sbc;
        startBlock = _startBlock;
        sbcPerBlock = _sbcPerBlock;
    }
    ...
    // Update the given pool's SBC allocation point and deposit fee. Can only be called by the owner.
    ...
    function pendingSBC(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 sbcReward = multiplier.mul(sbcPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accSBCPerShare = accSBCPerShare.add(sbcReward.mul(1e18).div(lpSupply));
        }
        uint256 pending = user.amount.mul(accSBCPerShare).div(1e18).sub(user.rewardDebt);
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accSBCPerShare = pool.accSBCPerShare.add(sbcReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        uint256 pending = user.amount.mul(pool.accSBCPerShare).div(1e18).sub(user.rewardDebt);
        ...
        user.rewardDebt = user.amount.mul(pool.accSBCPerShare).div(1e18);
    ...
    // Has to add hidden dummy pools in order to alter the emission, here we make it simple and transparent to all.
    function updateEmissionRate(uint256 _sbcPerBlock) public onlyOwner {
        massUpdatePools();
        emit EmissionRateUpdated(msg.sender, sbcPerBlock, _sbcPerBlock);
        sbcPerBlock = _sbcPerBlock;
    }
