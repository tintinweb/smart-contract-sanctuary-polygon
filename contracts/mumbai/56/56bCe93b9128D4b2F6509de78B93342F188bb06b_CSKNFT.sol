    ...
    ReentrancyGuard
{
    ...
    ) ERC721("Code Sekai", "CSK") EIP712(SIGNING_DOMAIN, SIGNATURE_VERSION) {
    ...
    function mint(
        ...
        if (_mintType == MintType.Mint) {
            ...
            require(block.timestamp <= mintDates.MINT_END_DATE, "Public ends.");
        } else if (_mintType == MintType.Whitelist) {
            require(
                block.timestamp >= mintDates.START_WHITELIST,
                "Wl not started."
            );
            require(block.timestamp <= mintDates.END_WHITELIST, "Whitelist ends.");
