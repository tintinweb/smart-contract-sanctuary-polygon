...
contract MarginTrade is Ownable {
    ...
    function tradeForClosePosition(uint256 tradeAmount, address[] memory path)
        internal
        returns (uint256, uint256)
    {
        uint256[] memory result = new uint256[](2);
    ...
    function getCurrentPrice(uint _positionID)
        public
        view
        returns (uint256)
    {
        ...
        if(IERC20(path[1]).decimals() == 18) {
            ...
            return ((1*10**18*10**(18-IERC20(path[1]).decimals()))/price[1]);
        }
    }
    ...
    function liquidateOpenPosition() external onlyOwner {
        ...
        for (uint256 i = 1; i < positionID; i++) {
            ...
            if (
                ...
                // Since position is closed asset goes to available margin
                // asset amount wont be the same -- check for errors
                if(positionInfo.marginUsed > depositInfo.usedMargin)
                    depositInfo.usedMargin = positionInfo.marginUsed.sub(
                        depositInfo.usedMargin
                    );
                else
                    depositInfo.usedMargin = depositInfo.usedMargin.sub(positionInfo.marginUsed);
                ...
                // Add loan back to available liquidity for that user -- not to global
                // Reduce from here
                if(depositInfo.usedLoan > positionInfo.loanUsed)
                    depositInfo.usedLoan = depositInfo.usedLoan.sub(
                        positionInfo.loanUsed
                    );
                else 
                    depositInfo.usedLoan = positionInfo.loanUsed.sub(
                        depositInfo.usedLoan
                    );    
                // ****** Below has to be in withdraw  *******//
                // // Add here
                // availableLiquidity[positionInfo.asset] = availableLiquidity[positionInfo.asset].add(allocatedLoan);
