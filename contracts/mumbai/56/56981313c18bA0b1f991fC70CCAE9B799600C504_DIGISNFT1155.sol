...
contract DIGISNFT721 is ERC721URIStorage {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;
    address public _owner;
    address public _contractAddress;

    constructor(address marketplaceAddress) ERC721("DIGISNFT", "DIGIS721") {
      _owner = msg.sender;
...
contract DIGISNFT1155 is ERC1155URIStorage {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;
    address public _owner;
    address public _contractAddress;

    constructor(address marketplaceAddress) ERC1155("DIGIS1155") {
      _contractAddress = marketplaceAddress;
    }
...
contract NFTMarket is ReentrancyGuard {
  ...
  using Counters for Counters.Counter;
  enum BuyingAssetType {ERC721, ERC1155} // 0 for type 721 and 1 for type 1155
  ...
  /* Updates the amount allowance ERC20 of the contract */
  ...
  /* Returns the Current amount allowance ERC20 of the contract */
  ...
  function approveAllowance(address erc20Address, address spender) internal {
    IERC20(erc20Address).approve(spender, _amountAllowance);
  }
  ...
  function listingNFTtoMarketplace(
    ...
    uint256 balanceSeller = IERC20(erc20Address).balanceOf(seller);
    ...
    IERC20(erc20Address).transferFrom(seller, address(this), fee);
    ...
    if(nftType == BuyingAssetType.ERC1155) {
      IERC1155(nftContract).safeTransferFrom(seller, address(this), tokenId, qty, "0x00");
    }
  ...
  function buyItemfromMarketplace(
    ...
    if(nftType == BuyingAssetType.ERC1155) {
      IERC1155(nftContract).safeTransferFrom(address(this), buyer, tokenId, quantity, "0x00");
    }
    // transfer fee to marketplace
    IERC20(erc20Address).transferFrom(buyer, address(this), fee);
  ...
  function stake(BuyingAssetType nftType, address nftContract, uint256 _tokenId, uint256 _amount) public {
    ...
    if(nftType == BuyingAssetType.ERC721) {
      ...
      IERC721(nftContract).transferFrom(addressStaker, address(this), _tokenId);
    ...
    if(nftType == BuyingAssetType.ERC1155) {
      ...
      IERC1155(nftContract).safeTransferFrom(addressStaker, address(this), _tokenId, _amount, "0x00");
  ...
  function unstake(uint256 itemIdStake) public returns (RewardStake memory) {
    ...
    if(nftType == BuyingAssetType.ERC721) {
      IERC721(nftContract).transferFrom(address(this), addressStaker, _tokenId);
    }

    if(nftType == BuyingAssetType.ERC1155) {
      IERC1155(nftContract).safeTransferFrom(address(this), addressStaker, _tokenId, amount, "0x00");
    }
  ...
  function WitdrawalFee(address erc20Address, address wdAddress,uint256 amount) public isOnlyOwner returns(bool) {
    uint256 balance = IERC20(erc20Address).balanceOf(address(this));
    require(balance >= amount, "Insufficient Funds");
    IERC20(erc20Address).approve(wdAddress, amount);
    emit WithdrawalFeeTransferred(wdAddress, amount);
    IERC20(erc20Address).transfer(wdAddress, amount);
    IERC20(erc20Address).approve(wdAddress, 0);
    return true;
  }
}