...
abstract contract BasePay is Initializable, OwnableUpgradeable{
    ...
    uint256 public paymentFixedRate = 1000000;

    uint256 public withdrawRate = 30;
    uint256 public withdrawFixedRate = 300000;
    ...
    function getPaymentFee(uint256 _orderAmount, uint256 _paidAmount, bool isFixedRate) internal view returns (uint256 usdcFee,uint256 tokenFee) {

        if(isFixedRate) {
            tokenFee = SafeMath.div(SafeMath.mul(_paidAmount, paymentFixedRate), _orderAmount);
            usdcFee = paymentFixedRate;
        } else {
            tokenFee = SafeMath.div((SafeMath.mul(_paidAmount ,paymentRate)), 10000);
            usdcFee = SafeMath.div((SafeMath.mul(_orderAmount ,paymentRate)), 10000);
        }

        return (usdcFee, tokenFee);

    }

    function getWithdrawFee(uint256 _orderAmount, uint256 _paidAmount, bool isFixedRate) internal view returns(uint256 usdcWithdrawFee,uint256 tokenWithdrawFee) {

        if(isFixedRate) {
            tokenWithdrawFee = SafeMath.div(SafeMath.mul(_paidAmount, withdrawFixedRate), _orderAmount);
            usdcWithdrawFee = withdrawFixedRate;
        } else {
            tokenWithdrawFee = SafeMath.div((SafeMath.mul(_paidAmount ,withdrawRate)), 10000);
            usdcWithdrawFee = SafeMath.div((SafeMath.mul(_orderAmount ,withdrawRate)), 10000);
        }

        return (usdcWithdrawFee, tokenWithdrawFee);
    }
    ...
    function setting(
        ...
        uint256 _paymentFixedRate,
        uint256 _withdrawRate,
        uint256 _withdrawFixedRate,
        uint24 _poolFee
    ) external onlyOwner {
        paymentRate = _paymentRate;
        paymentFixedRate = _paymentFixedRate;
        withdrawRate = _withdrawRate;
        withdrawFixedRate = _withdrawFixedRate;
        poolFee = _poolFee;
    }
...
contract Pay is BasePay {
    ...
    string public name = "BlockPurse Payment Protocol";
    ...
    function pay(
        ...
        if (address(0) != settleToken) {
            ...
            emit Order(_orderId, _paiAmount, _currency, _orderAmount, settleToken, usdcFee, _merchant, msg.sender, isFixedRate);
            ...
            if (iMerchant.getAutoSettle(_merchant)) {
                ...
                _autoWithdraw(_merchant, iMerchant.getSettleAccount(_merchant), _currency, _paiAmount - tokenFee, tokenWithdrawFee);
            } else {
                merchantFunds[_merchant][_currency] += (_paiAmount - tokenFee);
            }

            tradeFeeOf[_currency] += tokenFee;

            emit Order(_orderId, _paiAmount, _currency, _orderAmount, _currency, tokenFee, _merchant, msg.sender, isFixedRate);
