...
contract ERC2410 is Context, IERC2410, IERC2410Metadata {
    ...
    /**
    * Add withdraw of `owner` with reward.
    * Requirements:
    *
    * - `owner` cannot be the zero address.
    */
...
contract DMLToken is ERC2410, ReentrancyGuard, ERC1155Holder {
    using SafeMath for uint;
    address public token;
    address public nft;
    address public router;
    address public feeToProtocol;
    ...
    constructor(
        ...
        address _nft,
        uint256 _id,
        address _feeToProtocol,
        uint _fee
    ) ERC2410(name, symbol, _tokenReward){
        token = _token;
        nft = _nft;
        id = _id;
        feeToProtocol = _feeToProtocol;
        router = _router;
        feeOfExchange =_fee;
        Pool pool= new Pool(address(this), _tokenReward);
        _pool = address(pool);
    }
    ...
    function updateReward(uint256 amount) public virtual override onlyRouter() returns(uint256) {
        address owner = _msgSender();
        if(totalSupply() != 0){
            rewardPerShare = rewardPerShare + amount * 1e18/ totalSupply();
        }      
    ...
    function getReward() public virtual override returns(uint256){
        address owner = _msgSender();
        UserInformation memory user = informationOf(owner);
        uint256 reward = balanceOf(owner) * rewardPerShare + user.inReward - user.withdraw - user.outReward;
        _withdraw(owner, reward);
        if(reward / 1e18 > 0){
            Pool(_pool).transferToken(owner, reward / 1e18 );
        }  
        emit GetReward(owner, reward);
        return reward;
    }
    ...
    function getBalanceNFT() internal view returns(uint){
        return IERC1155(nft).balanceOf(address(this), id);
    }
    ...
    function burn(address to) external nonReentrant returns (uint amounterc, uint amountnft) {
        ...
        if(liquidity == _totalSupply){
            ...
            TransferHelper.safeTransferFromNFT(nft, address(this), to, id, balancenft, bytes(''));
        }else {
            ...
            if(amountnft > 0 ){
                TransferHelper.safeTransferFromNFT(nft, address(this), to, id, amountnft, bytes(''));
            }
        }
    ...
    function swap(uint amounttokenOut, uint amountnftOut, address to, uint amountFee, address from) external nonReentrant {
         ...
         
        {
            ...
            if (amounttokenOut > 0) {
                ...
                TransferHelper.safeTransfer(token, IReferral(referral).getReceiver(from) , amountFeeReferral);
                TransferHelper.safeTransfer(token, getPool() , amountFee.sub(amountFeeReferral));
            }
            if(amountnftOut > 0) TransferHelper.safeTransferFromNFT(nft, address(this), to, id, amountnftOut, bytes(''));
            balancetoken = getBalanceToken();
            balancenft = getBalanceNFT();
...
contract Factory is Ownable{
    ...
    event DmlTokenCreated(address erc, address nft, uint256 id, address dmlToken, uint256 length);
    uint public fee;
    ...
    function createDMLToken(address token, address nft, uint256 id) external returns (address) {
        require(token != nft, 'DEMASK: IDENTICAL_ADDRESSED');
        require(token != address(0) && nft != address(0), 'DEMASK: ZERO_ADDRESS');
        require(dmlToken[token][nft][id] == address(0), 'DEMASK: DML_EXISTS');
        DMLToken _dmltoken = new DMLToken("DeMask Liquidity Token", "DML", token, msg.sender, token, nft, id, feeTo, fee);
        dmlToken[token][nft][id] = address(_dmltoken);
        allDmlTokens.push(address(_dmltoken));
        emit DmlTokenCreated(token, nft, id, address(_dmltoken), allDmlTokens.length);
        return address(_dmltoken);
    }

    function getDmlToken(address token, address nft, uint256 id) external view returns(address){
        return dmlToken[token][nft][id];
    }
...
contract Pool is AccessControl {
    ...
    constructor(address _dml, address _tokenReward){
    ...
    function transferToken(address _to, uint256 _amount) external onlyRole(POOL_ROLE) {
        uint256 reward = (_amount > IERC20(tokenReward).balanceOf(address(this))) ? IERC20(tokenReward).balanceOf(address(this)) : _amount;
        if(reward > 0){
            IERC20(tokenReward).transfer(_to, reward); 
