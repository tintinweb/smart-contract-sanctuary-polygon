...
contract TestRic is ERC721, ERC721Enumerable, Ownable, Pausable {
  ...
  * we surpass MAX_MINTABLE Rick the mintTokens is
  ...
  */
  modifier mintCountMeetsSupply(uint256 numberOfTokens) {
    require(
      totalRick + numberOfTokens <= MAX_MINTABLE,
  ...
  modifier meetsRecyclingSupply() {
    require(
      recycleSupply > 0,
      "There are no Rick available via recycling"
    );
    _;
  }
  ...
  modifier meetsRecyclingCost(uint256[] memory tokenIds) {
    require(
      ...
      "Incorrect number of Rick passed for recycling"
    );
    _;
  }
  ...
  modifier meetsOwnership(uint256[] memory tokenIds) {
    for (uint256 i = 0; i < tokenIds.length; i++) {
      require(this.ownerOf(tokenIds[i]) == msg.sender, "You may only recycle Rick you own");
    }
    _;
  }
