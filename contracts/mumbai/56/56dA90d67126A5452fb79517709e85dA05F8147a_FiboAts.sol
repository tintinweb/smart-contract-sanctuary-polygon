...
contract FiboAts is FiboEIP712, AccessControlUpgradeable {
    ...
    function haltTrading() external onlyRole(DEFAULT_ADMIN_ROLE) {
        // XXX
    }
    ...
    /// only the contract executor or the order's `trader` can call this function
    /// @param order details of the order to cancel
    function cancelOrder(FiboLimitOrders.LimitOrder calldata order) external {
        require(
            msg.sender == order.trader || hasRole(EXECUTOR_ROLE, msg.sender),
            "only trader or executor can cancel"
        );
