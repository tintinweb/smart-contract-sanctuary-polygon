...
contract WasteTrader is OwnableUpgradeable, ReentrancyGuardUpgradeable, IWasteToCash {
    ...
    function _removeOrder(uint256 _index) internal {
        if (_index > s_wasteOrders[msg.sender].length - 1)
            revert WasteTrader__IndexOutOfBounds(_index);
        for (uint256 i = _index; i < s_wasteOrders[msg.sender].length - 1; ++i) {
            s_wasteOrders[msg.sender][i] = s_wasteOrders[msg.sender][i + 1];
        }
        s_wasteOrders[msg.sender].pop();
    }

    function claimAstroCredits(uint256 _orderId) external nonReentrant {
        uint256 gwsTime = s_gwsTime;
        WasteOrder[] storage wasteOrders = s_wasteOrders[msg.sender];
        if (wasteOrders.length == 0) revert WasteTrader__NoOrdersPlaced(msg.sender);
        if (_orderId > wasteOrders.length - 1) revert WasteTrader__InvalidId(_orderId);
        if (
            ((wasteOrders[_orderId].createdAt > 0) &&
                (block.timestamp - wasteOrders[_orderId].createdAt > gwsTime)) ||
            (wasteOrders[_orderId].completed == true)
        ) {
            uint256 orderAmount = wasteOrders[_orderId].orderAmount;
            if (orderAmount > 0) {
        ...
        returns (uint256 _waitPeriod)
    {
        ...
        if (_id > wasteOrders.length - 1) revert WasteTrader__InvalidId(_id);

        uint256 gwsTime = s_gwsTime;
        uint256 timeRemaining = wasteOrders[_id].timeRemaining < gwsTime
            ? wasteOrders[_id].timeRemaining
            : gwsTime;
        uint256 effectiveCreatedAt = wasteOrders[_id].createdAt - timeRemaining;
        _waitPeriod = block.timestamp - effectiveCreatedAt;
    }
    ...
    function getWasteOrders(address _player) external view returns (WasteOrder[] memory) {
...
interface IWasteToCash {
     ...
     * @param _orderId The order Id to claim against
     */
    function claimAstroCredits(uint256 _orderId) external;
     ...
     */
    function speedUpExchange(uint256 _numSpeedups) external;

    /**
     * @notice Checks the remaining time for an order to complete
     * @param _id Order Id
     * @param _player EOA address of the player
     * @return _waitPeriod Order remaining time
     */
    function getOrderCompletionTime(uint256 _id, address _player)
        external
        view
    ...
    /** @notice Retreives the player's orders
     *  @param _player EOA address of the player
     */
    function getWasteOrders(address _player) external view returns (WasteOrder[] memory);
}
