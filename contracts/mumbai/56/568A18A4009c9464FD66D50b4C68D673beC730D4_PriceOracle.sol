...
contract PriceOracle {
  ...
  function getMainPriceViaNativePair(address token) external view returns (uint256) {
    (uint256 mnPriceAdjusted, address mnToken0, address mnToken1) = _getTokenPrice(token, wrappedNative);
    ...
    )
  {
    ...
    return (uint256(res1).mul(10**18).mul(10**t0Dec).div(uint256(res0)).div(10**t1Dec), t0, t1);
  }
}
