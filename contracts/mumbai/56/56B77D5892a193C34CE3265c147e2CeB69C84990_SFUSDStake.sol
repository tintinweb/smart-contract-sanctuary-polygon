...
contract SFUSDStake is Ownable {
    ...
    function deposit(uint256 _amount) external {
        ...
        depositState[_id].depositAmount = _amount.sub(depositFee);
    ...
    function getClaimableReward(
        ...
        uint256 profit;
        if (investors[msg.sender].nextClaimDate <= block.timestamp) {
    ...
    function migrateInvestorToV2(
        ...
        if (investors[_investor].totalRewardAmount >= _amount) {
            ...
            capitalLockedPool = capitalLockedPool.add(
                _amount.sub(investors[_investor].totalRewardAmount)
            );
