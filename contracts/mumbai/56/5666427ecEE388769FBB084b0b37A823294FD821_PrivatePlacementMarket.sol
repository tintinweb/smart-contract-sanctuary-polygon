...
contract PrivatePlacementMarket is OwnablePausable, ReentrancyGuardUpgradeable{
    ...
    function buy(uint256 index) external whenNotPaused {
        ...
        if (_bags[index].stockValue > 0) {
            ...
            _stockNftFactory.mint(index, trader);
    ...
    function addBag(uint256 index, uint256[] calldata bag_) external onlyOwnerOrOperator {
        ...
        _stockNftFactory.createERC721(index, bag_[2]);
    }

    function initStockNftFactory(address stockNftFactoryAddr) external onlyDeployer {
        require(address(_stockNftFactory) == address(0), "not zero");
        _stockNftFactory = StockNftFactory(stockNftFactoryAddr) ;

        for (uint256 index; index < bagSize; index++) {
            _stockNftFactory.createERC721(index, _bags[index].stockValue);
        }
    }
    ...
    // ) {
    //     ids = new uint256[](bagSize);
    //     values = new uint256[](bagSize);
...
contract StockERC721 is ERC721Enumerable {
    ...
    constructor(address factoryAddress, uint256 index_, uint256 usdtValue_) ERC721("NFT-Stock-ERC721", "NFT-Stock-ERC721"){
...
contract StockNftFactory is OwnablePausable {
    ...
    function createERC721(uint256 index, uint256 value) external onlyPPM {
    ...
    function mint(uint256 index, address to) external onlyPPM {
