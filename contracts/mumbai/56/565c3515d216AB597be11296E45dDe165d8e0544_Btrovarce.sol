...
interface IERC721 is IERC165 {
    ...
    event Bought( uint256 tokenId , uint256 value , address seller , address sender); 
...
contract ERC721 is IERC721 , Context ,ERC165 , IERC721Metadata ,IERC721Enumerable {
...
contract Btrovarce is ERC721, Ownable {
    ...
    function ownerOfCollection(address _contract) public view returns(address){
        return CollectionsContract[_contract].ownerOfCollection;
    }

    function collectionProfit(address _contract) public view returns(uint){
        return CollectionsContract[_contract].percentOfCommission;
    }


    function collectionProfitP(address _contract) public payable returns(uint){
        return msg.value * CollectionsContract[_contract].percentOfCommission /100;
    }



    function addContractERC721 (address _contract, uint256 _commission) public {
    ...
    function buy(uint256 tokenId , address _constant) payable public {
        ...
        // Collections memory collection =  CollectionsContract[_constant];
        ...
        // require(collection.isEqual, "Not Collection Active");
        ...
        uint price = msg.value *  CollectionsContract[_constant].percentOfCommission / 100;
        
        uint FeeAmount = msg.value * percentAge / 100;
      
        // uint profitAmount = msg.value *  CollectionsContract[_constant].percentOfCommission / 100;

        
        uint sellerAmount = (msg.value  - FeeAmount);
        uint profit = (msg.value  - price);
    ...
    function _NoLongerForSale(address from, uint256 tokenId, address _contract) internal onlyOwnerItem(_msgSender(), tokenId,_contract){
        OfferedForSale[tokenId] = Offer(false, tokenId, from, 0, address(0));
    ...
    function enterBidForItem(uint256 tokenId) public payable {
        require(marketPaused == false, 'Market Paused');
        // require(ownerOf(tokenId) != _msgSender(), "Invalid");
        require(msg.value > 0, 'bid can not be zero');
        require(msg.value > ItemBids[tokenId].value, "Invalid");
        ...
        if (existing.value > 0) {
            ...
            // require(success , 'Refund the failing bid undone');
        }

        ItemBids[tokenId] = Bid(true, tokenId, _msgSender(), msg.value);
        emit BidEntered(tokenId, _msgSender(), msg.value);
    }


    function withdrawBidForItem(uint256 tokenId) public {
       
        // IERC721 collectionContract =  CollectionsContract[_contract];

        // require(collectionContract.ownerOf(tokenId) != _msgSender(), "Invalid");
        require(ItemBids[tokenId].bidder == _msgSender(), "Invalid");

        uint amountBid = ItemBids[tokenId].value;
        ItemBids[tokenId] = Bid(false, tokenId, address(0), 0);
        
        // Refund the bid money
        (bool success,) = _msgSender().call{value: amountBid}("");
        require(success, 'not send price to bidder ');
        emit BidWithdrawn(tokenId, ItemBids[tokenId].value, _msgSender());
    }
    ...
    function acceptBidForItem(uint256 tokenId, address _contract) public onlyOwnerItem(_msgSender(), tokenId,_contract){
        ...
        // Collections memory collection =  CollectionsContract[_contract];

        IERC721 buyOfContract =  CollectionsContract[_contract].contractAddressCollection; 

        require(ItemBids[tokenId].value > 0, 'there is not any bid');

        // require(collection.isEqual, "Not Collection Active");

        // uint minPrice = OfferedForSale[tokenId].minValue;

        // require(ItemBids[tokenId].value >= minPrice, "Invalid");
       ...
       // handle Calc
        ...
        uint price = bid.value *  CollectionsContract[_contract].percentOfCommission / 100;
        
        uint FeeAmount = bid.value * percentAge / 100;
        uint profitAmount = bid.value*  CollectionsContract[_contract].percentOfCommission / 100;

        
        uint sellerAmount = (bid.value  - FeeAmount);
        uint profit = (bid.value  - profitAmount);
