...
// File contracts/utils/Uint8a10.sol
...
interface ISettlement {
    function settlement(uint256 _random, address _player, uint256 _count, uint256 _requestId) external;
}

contract Lottery is VRFConsumerBaseV2, Roles {
    event PriceChange(uint value); // TODO set single draw price
    ...
    ISettlement public gamePool;

    mapping(uint256 => VRFRequest) public vrfReq;
    ...
    uint64 public subscriptionId = 3534;
    ...
    address public coordinator = 0x7a1BaC17Ccc5b313516C5E16fb24f7659aA5ebed;

    constructor(address _gamePool) VRFConsumerBaseV2(coordinator) Roles(_msgSender()) {
        COORDINATOR = VRFCoordinatorV2Interface(coordinator); // VRF
        gamePool = ISettlement(_gamePool);
    }

    uint256 public single = 2000000; // 2 dollars
    function startGame(
        address player,
        uint256 dollar,
        uint256 tokenId,
        ...
        // TODO require dollar % single == 0;
        return requestRandomWords(player, uint64(dollar / single));
    }
    ...
    function requestRandomWords(address player, uint64 gameCount) private returns (uint256) {
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        VRFRequest memory request = vrfReq[requestId];
        require(request.player != address(0), "request not found");

        uint256 random = randomWords[0]; // _lastRandomWord = random;
        address player = request.player;
        uint256 gameCount = request.count;
        gamePool.settlement(random, player, gameCount, requestId);
        delete vrfReq[requestId];
    }
