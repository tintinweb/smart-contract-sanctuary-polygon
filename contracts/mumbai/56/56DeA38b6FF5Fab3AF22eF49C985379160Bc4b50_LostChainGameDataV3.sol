...
contract LostChainGameDataV3 is CustomERC2771ContextUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event REWARDAMOUNT(uint8[]);
    event MAXREWARDAMOUNT(uint8[]);
    event ENEMYLIST(uint256[]);
    event REWARDLIST(uint256[]);
    event LANDPRICE(uint256);
    event LANDOWNER(address);
    ...
    function adventure(
        ...
        emit MAXREWARDAMOUNT(maxRewardAmount);
        emit ENEMYLIST(enemyList);
        emit REWARDLIST(rewardList);
        emit LANDPRICE(landPrice);
        emit LANDOWNER(landOwner);
