...
contract SwapFactory is Ownable, CallerPermission, ReEntrancyGuard, Ticker {
    ...
    function getReserve() public view returns (uint, uint) {
        ...
        if (isEtherBase) {
            // Convert input amount from wei to ether
            uint256 inputAmount = address(this).balance;
            inputAmount = inputAmount / (10 ** 18);
            // Convert input amount from ether to token
            inputAmount = inputAmount * (10 ** 6);
    ...
    /*    require(
        ...
        if (investers[_up].join_time > 0) {
            ...
            if (_up != address(0)) {
                paidFee += directAmount;
              payable(_up).transfer(directAmount);
            }
        }
