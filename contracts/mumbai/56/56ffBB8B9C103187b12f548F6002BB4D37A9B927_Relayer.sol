...
contract Relayer {
   ...
   */
  function _unwrapSignature(bytes memory signature) public pure returns (bytes32 r, bytes32 s, uint8 v) { // internal
    assembly {
   ...
   */
  function _recover(bytes32 hash, bytes memory signature) public pure returns (address) { //Internal
   ...
   */
  function _calculateHash(address[] memory from, address[] memory to, bytes[] memory data) public pure returns (bytes32) { //internal
   ...
   * @notice Function to calculate keccak256 hash
   */
  function _calculateHash(address from, address to, bytes memory data) public pure returns (bytes32) { //internal
    // Ethereum signed message prefix
    string memory prefix = "\x19Ethereum Signed Message:\n32";
