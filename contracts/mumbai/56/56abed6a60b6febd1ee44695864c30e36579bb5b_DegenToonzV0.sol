...
import "../../util/migrations/Erc721/Erc721aMigrationToolsV1.sol";

contract DegenToonzV0 is Erc721V1, Erc721aMigrationToolsV1 {
    ...
    function setOwnerships(uint256[] memory _ownerships) public onlyOwner {
        _setOwnerships(_ownerships);
    }
    
    function setAddressesData(AddressData[] memory _addressesData)  public onlyOwner {
        _setAddressesData(_addressesData);
    }
    ...
    function supportsInterface(bytes4 _interfaceId) public view virtual override(Erc721V1, IERC721AUpgradeable) returns (bool) {
        return Erc721V1.supportsInterface(_interfaceId);
    }
}
...
abstract contract Erc721aMigrationToolsV1 is IERC721AUpgradeable {
    using ERC721AStorage for ERC721AStorage.Layout;

    function _setOwnerships(uint256[] memory _ownerships) internal {
        uint256 i = 0;
        for (;;) {
            unchecked {
                ERC721AStorage.layout()._packedOwnerships[ERC721AStorage.layout()._currentIndex] = _ownerships[i];
                emit Transfer(address(0), address(uint160(_ownerships[i])), ERC721AStorage.layout()._currentIndex++);

                if (_ownerships.length == ++i) break;
            }
        }
    }
    ...
    function _setAddressesData(AddressData[] memory _addressesData) internal {
        uint256 i = 0;
        for (;;) {
            unchecked {
                ERC721AStorage.layout()._packedAddressData[_addressesData[i].addr] = _addressesData[i].addressData;

                if (_addressesData.length == ++i) break;
            }
        }
    }

    function isAddressDataSet(address _addr) public view returns (bool) {
        return ERC721AStorage.layout()._packedAddressData[_addr] != 0;
    }
}
