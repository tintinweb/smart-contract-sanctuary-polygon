...
contract MaterialVault is Ownable, AccessControl {
    ...
    function decreaseMaterialUserSingle(uint256 _materialId, uint256 _materialAmount) external whenMaterialVaultNotPaused {
        require(_materialAmount <= materialAmountbyUser[msg.sender][_materialId], "[MaterialVault.decreaseMaterialUserSingle] withdraw material should be less than User's material");
        require(materialAmountbyUser[msg.sender][_materialId] - _materialAmount <= materialAmountbyUser[msg.sender][_materialId], "[MaterialVault.decreaseMaterialUserSingle] User has materials underflow");
        materialAmountbyUser[msg.sender][_materialId] -= _materialAmount;

        emit DecreaseMaterialUserSingle(msg.sender, _materialId, _materialAmount);
    }

    function decreaseMaterialUser(uint256[] memory _materialIds, uint256[] memory _materialAmounts) external whenMaterialVaultNotPaused {
        uint256 arrayLength = _materialIds.length;
        
        for (uint i = 0; i < arrayLength; i++) {
            require(_materialAmounts[i] <= materialAmountbyUser[msg.sender][_materialIds[i]], "[MaterialVault.decreaseMaterialUser] withdraw material should be less than User's material");
            require(materialAmountbyUser[msg.sender][_materialIds[i]] - _materialAmounts[i] <= materialAmountbyUser[msg.sender][_materialIds[i]], "[MaterialVault.decreaseMaterialUser] User has materials underflow");
            materialAmountbyUser[msg.sender][_materialIds[i]] -= _materialAmounts[i];
        }

        emit DecreaseMaterialUser(msg.sender, _materialIds, _materialAmounts);
    }
