    ...
    UUPSUpgradeable
{
    ...
    function safeMintOne(address to, bytes32 tokenURI_) public onlyRole(PRIMARY_MINTER_ROLE) {
    ...
    function safeMint(
        ...
        require(quantity == tokenURIs.length, "VoiceA: Length of tokenURIs must equal the quantity");
        uint256 tokenId = _nextTokenId();
        ...
        returns (string memory)
    {
        ...
        if (_tokenURI.length > 0) {
            return string(abi.encodePacked(base, "/", encode(bytes.concat((_tokenURIs[tokenId])))));
...
interface IVoiceA is IERC721AUpgradeable {
  function safeMintOne(address to, bytes32 tokenURI) external;
  function safeMint(address to, uint256 quantity, bytes32[] calldata tokenURIs) external;
}