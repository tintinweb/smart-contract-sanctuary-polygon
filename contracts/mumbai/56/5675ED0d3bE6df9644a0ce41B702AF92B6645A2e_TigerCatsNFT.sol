...
contract TigerCatsNFT is ERC721URIStorage, Ownable, ReentrancyGuard {
    string private _collectionURI;
    string public baseURI;

    /**
     * whitelist are a max supply 2000
     * public mint max supply 5120
     **/

    uint256 public immutable maxWhitelistMint = 2000;
    uint256 public constant WHITELIST_SALE_PRICE = 0.05 ether;

    uint256 public immutable maxPublicMint = 5120;
    uint256 public constant PUBLIC_SALE_PRICE = 0.06 ether;

    // used to validate whitelists
    bytes32 public whitelistMerkleRoot;

    // puase minting (only, does not pause transfers)

    bool public openPublicMint = false;
    bool public openWlMint = false;
    ...
    modifier canMint(uint256 numberOfTokens) {
        require(
            tokenCount + numberOfTokens <= maxPublicMint,
    ...
    modifier whitelistLive() {
        require(openWlMint, "WL Minting not started.");
        _;
    }

    modifier publicMintLive() {
        require(openPublicMint, "Public Minting not started.");
        _;
    }
    ...
    /**
     * @dev mints specified # of token per whitelisted address
     * Max supply: 2000
     * Max Mint = 2
     * charges a fee
     */
    ...
    {
        require(
            tokenCount + numberOfTokens <= maxWhitelistMint,
            "Minted the maximum # of whitelist tokens"
        );
        require(numberOfTokens <= 2, "Maximum WL Mint is 2 TigerCats");
        require(!claimed[msg.sender], "WL wallet has already claimed.");
