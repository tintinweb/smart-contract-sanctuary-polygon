...
contract CatAttackNFT is ERC1155LazyMint {
    ...
    event RewardClaimed(address indexed player, uint256 rewardCount);
    ...
    function safeTransferFrom(
        ...
        if (from != to && id == 0) {
            ...
            // Increase badge count for both sender and receiver
            badgeCount[to] += 1;
            badgeCount[from] += 1;

            if (badgeCount[to] % BADGE_THRESHOLD == 0) {
                _rewardRandomPowerUpOrDefense(to);
            }

            if (badgeCount[from] % BADGE_THRESHOLD == 0) {
                _rewardRandomPowerUpOrDefense(from);
            }
        }
    }
    ...
    function burn(
        ...
        if (id == 1) {
            ...
            badgeCount[account] += 1;

            if (badgeCount[account] % BADGE_THRESHOLD == 0) {
                _rewardRandomPowerUpOrDefense(account);
            }
        }
    }
    ...
    function useAttackPowerUp(address victim1, address victim2) external gameNotPaused {
        require(balanceOf[msg.sender][POWER_UP_ID] > 0, "You need an Attack power-up to perform this action");
        ...
        attack(msg.sender, victim1);

        // Attack the second victim
        attack(msg.sender, victim2);
    }
    ...
    function useDefensePowerUp(address attacker1, address attacker2) external gameNotPaused {
        require(balanceOf[msg.sender][DEFENSE_ID] > 0, "You need a Defense power-up to perform this action");
        ...
        defend(msg.sender, attacker1);

        // Defend against the second attack
        defend(msg.sender, attacker2);
    }
     ...
     */
    function attack(address attacker, address victim) internal {
     ...
     * @param attacker The attacker's address
     */
    function defend(address defender, address attacker) internal {
    ...
    /**
     * @notice Pause or resume the game
     * @param pauseStatus True to pause the game, false to resume
     */
    function pauseGame(bool pauseStatus) external onlyOwner {
        isGamePaused = pauseStatus;
    }

    /**
     * @notice Internal function to reward the player with a random power-up or defense NFT
