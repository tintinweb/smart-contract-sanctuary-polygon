...
contract FixedSale is IERC721Receiver, Ownable {
  ...
  /**
  // * Modifiers
  @dev Check whether the sender is the seller of the token or nft
 */
  modifier sellerOnly(address _nft, uint256 _tokenId) {
    TokenDetails storage token = tokenToSale[_nft][_tokenId];
    require(token.seller == msg.sender, 'Only the seller allowed');
    _;
  }
  ...
  function putItemForSale(
    ...
    require(_nft != address(0), 'Address same as the smart contract address is not allowed');
    require(_price > 0 ether, 'Price must be greater than zero.');
    uint256 listingDate = block.timestamp;
    TokenDetails memory itemListing = TokenDetails({nftContract: _nft, nftId: _tokenId, seller: msg.sender, price: _price, isActive: true, date: listingDate});
  ...
  ) external sellerOnly(_nft, _tokenId) {
    TokenDetails storage token = tokenToSale[_nft][_tokenId];
  ...
  function cancelSale(address _nft, uint256 _tokenId) external sellerOnly(_nft, _tokenId) {
    TokenDetails storage token = tokenToSale[_nft][_tokenId];
   ...
   */
  function buyItem(address _nft, uint256 _tokenId) external payable {
    TokenDetails storage token = tokenToSale[_nft][_tokenId];
    require(token.seller != msg.sender, 'You cannot buy the item your selling');
    require(token.isActive, 'Sale has already ended');
    require(msg.value >= token.price, 'Amount should be greater than or equal to fixed price');
    token.isActive = false;
    uint256 cutFee = (msg.value * Fee) / BPS;
    (bool success, ) = token.seller.call{value: msg.value}('');
    require(success, 'Transfer failed');
    (bool transferFee, ) = owner().call{value: cutFee}('');
    require(transferFee, 'Transfer failed');
