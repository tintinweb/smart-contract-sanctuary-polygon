...
contract T222 is Ownable, TimeForDay, SortList {
    ...
    struct ShareItem {
        ...
        uint256 _totalAD1A; //累积Dao1指数
        uint256 _totalAD2A; //累积Dao2指数
        uint256 _totalAD3A; //累积Dao3指数
        uint256 _totalAD4A; //累积Dao4指数
    }
    ...
    function Invest(uint256 _amount) external {
        ...
        */
        ...
        //     currentAD1A = 0;
        // } else {
        //     currentAD1A =  (_amount * 2 / 100)  / (Dao1Set.length());
        // }
        // totalShareAllocationList[tatalInvestNumber]._totalAD1A += currentAD1A;
    ...
    function claimStaticReward() external {
        ...
        IERC20(daoToken).transfer(msg.sender, _penddingReward);
        userInfoList[_user]._totalClaiRewardAmount += _penddingReward;
        userInfoList[_user]._updatedReward = 0;
        require((userInfoList[_user]._depositAmount * 2) >= userInfoList[_user]._totalClaiRewardAmount, "e001");
        //用户领取收益之后,要更新剩余的权益额度
    ...
    //    uint256 public accumulatedDividend;
    //    uint256 public nextIndex;
    //    function distributeDao5(uint256 _dividendAmount, uint256 _batchSize) private {
    //        uint256 totalAddresses = Dao5Set.length();
    //        if (totalAddresses == 0) {
    //            accumulatedDividend += _dividendAmount;
    //            return;
    //        }
    //        _dividendAmount += accumulatedDividend;
    //        accumulatedDividend = 0;
    //        uint256 _dividendAmountPerUser = _dividendAmount.div(totalAddresses);
    //
    //        uint256 endIndex = nextIndex + _batchSize;
    //        if (endIndex > totalAddresses) {
    //            endIndex = totalAddresses;
    //        }
    //
    //        for (uint256 i = nextIndex; i < endIndex; i++) {
    //            address _user = Dao5Set.at(i);
    //            userInfoList[_user]._daoRewardAmount += _dividendAmountPerUser;
    //        }
    //        nextIndex = endIndex;
    //        if (nextIndex >= totalAddresses) {
    //            nextIndex = 0;
    //        }
    //
    //        uint256 remainingDividend = _dividendAmount - _dividendAmountPerUser * (endIndex - nextIndex);
    //        accumulatedDividend += remainingDividend;
    //    }
    ...
    function updateDao(address _user, uint256 _investId) private {
        ...
        if (bigAmount >= 500 * 10 ** 22 && totalSmallAmount >= 500 * 10 ** 22) {
            ...
            userInfoList[_user]._daoItem.beDao1 = _investId;
            Dao1Set.add(_user);
            ...
            if (!userInfoList[_user]._daoItem.isDao2) {
                ...
                userInfoList[_user]._daoItem.beDao2 = _investId;
                Dao2Set.add(_user);
                if (Dao3Set.contains(_user)) {
                    Dao3Set.remove(_user);
                }
            ...
            if (!userInfoList[_user]._daoItem.isDao3) {
                ...
                userInfoList[_user]._daoItem.beDao3 = _investId;
                Dao3Set.add(_user);
                if (Dao4Set.contains(_user)) {
                    Dao4Set.remove(_user);
                }
            ...
            if (!userInfoList[_user]._daoItem.isDao4) {
                ...
                userInfoList[_user]._daoItem.beDao4 = _investId;
                Dao4Set.add(_user);
            }
        }
    }
