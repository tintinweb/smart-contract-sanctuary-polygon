...
contract ERC721Sale is ReentrancyGuard, Ownable {
    ...
    //    uint(keccak256(abi.encodePacked("Polygon-Native-Currency")))
    ...
    string[] public supportCurrencyName = ["MATIC"];
    ...
    constructor(
        ...
        // supportCurrencies[uint8(supportCurrencyName.length)] = address(0xdAC17F958D2ee523a2206206994597C13D831ec7);
        // supportCurrencyName.push("USDT");

        // supportCurrencies[uint8(supportCurrencyName.length)] = address(_ikonicToken);
        // supportCurrencyName.push("IKONIC");
    }
    ...
    function sell(
        ...
        require(isCurrencyValid(_currencyId), "ERC721Sale.sell: Currency is not supported");
    ...
    function buy(
        ...
        require(_currencyId == saleInfos[address(_token)][_tokenId].currencyId, "ERC721Sale.buy: Buying currency is different from sale currency");
        ...
        distributePayment(_token, _tokenId, owner, _currencyId);
    ...
    function distributePayment(
        ...
        uint256 buyerServiceFee = tokenPrice.mul(serviceFeeProxy.getBuyServiceFeeBps()).div(10000);
     ...
     */
     function updateExpSaleDate(
        ...
        require(
            _token.ownerOf(_tokenId) == msg.sender,
            "ERC721Sale.updateExpSaleDate: Caller is not the owner of the token"
        );
    ...
    */
    ...
    function addSupportCurrency(string memory newCurrencyName, address newCurrencyAddress) external onlyOwner {
        uint8 i = 0;
        for (i = 1; i < supportCurrencyName.length; i++) {   
            require(supportCurrencies[i] != newCurrencyAddress, "ERC721Sale.addSupportCurrency: This currency already exists");
        }
    ...
    function updateCurrencyAddress(uint8 currencyIndex, address currencyAddress) external onlyOwner {
        require(isCurrencyValid(currencyIndex), "ERC721Sale.updateCurrencyAddress: Currency is not supported");
