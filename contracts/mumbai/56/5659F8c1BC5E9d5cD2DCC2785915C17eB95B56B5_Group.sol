...
contract Event is ERC721, Ownable {
  ...
  address payable private platformAddress = payable(0xE7F7BB8afFDc2e9b536fbD067806EC9C7435fCF8);
  ...
  function tokenURI(
    ...
    return ticketTypes[tokenTypeByTokenId[_tokenId]].metadataURI;
  }
  ...
  function mintEventTicket(uint256 _ticketTypeIndex) public payable {
    ...
    require((ticketType.fee > 0 && msg.value == ticketType.fee) || (ticketType.fee == 0 && msg.value == 0), 'Incorrect fee');
...
contract Group is ERC721, Ownable {
  ...
  receive() external payable {
      // Etherを受け取るためのコード。通常は何も書かない
  }

  fallback() external payable {
      // 一致する関数がない場合のフォールバックコード。通常は何も書かない
  }
}