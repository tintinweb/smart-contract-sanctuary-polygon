...
contract PositionFactory {
    ...
    event PositionCreated(address sender, address positionContract);

    mapping(address => address) public userToPosition;
    address public borrowToken; // e.g xUSDC
    address public marketplace;
    address public priceFeed;

    constructor(address _xUSDC, address _marketplace, address _priceFeed) {
        borrowToken = _xUSDC;
    ...
    function createUserPosition() external {
        require(
            userToPosition[msg.sender] == address(0),
            "User already has a position"
        );
        address userPosition = address(
            new UserPosition(borrowToken, marketplace, msg.sender, priceFeed)
        );
        userToPosition[msg.sender] = userPosition;

        emit PositionCreated(msg.sender, userPosition);
    }

    // TODO: add access control
    function mintXUSDC(uint amount) external {
        IMintableBurnableERC20(borrowToken).mint(msg.sender, amount);
    }
}
...
contract UserPosition {
    ...
    mapping(address => uint256) public borrowedAmount; // Amount borrowed by each user
    ...
    function borrow(
        ...
        require(
            borrowedAmount[msg.sender] + loanAmount <= maxLoanAmount,
        ...
        IPositionFactory(borrowToken).mintXUSDC(loanAmount);
    ...
    function repay() external {
        uint256 loanAmount = borrowedAmount[msg.sender];
    ...
    function calculateMaxLoanAmount(
        ...
        for (uint256 i = 0; i < userCollateral[user].length; i++) {
            ...
            console.log("this is isode the loop totalCollateralValue");
        }
    ...
    function calculateCollateralValue(
        ...
        address priceFeed = getTokenChainlinkPriceFeed(collateralToken);
        ...
        console.log(uint(price), "this is the price");
        console.log(collateralAmount, "input collateralAmount");
        // to covert every input amount to 18 decimals
        collateralAmount = _normalizeAmount(collateralAmount, collateralToken);
        console.log(collateralAmount, "normailized collateralAmount");
    ...
    function _normalizeAmount(
        ...
        uint256 fromDecimals = IERC20Metadata(_from).decimals();
    ...
    function getTokenChainlinkPriceFeed(
        ...
        return IPriceFeed(priceFeed).getCurrencyToFeed(_token);
    }
}
