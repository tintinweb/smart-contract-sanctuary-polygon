    ...
    Oracle
{
    ...
    function BuyWithCustomToken(
        ...
        require(
            !erc20List.isNative,
            "Buy With Custom Token: not custom token native sended"
        );

        /// @dev Validate user approve amount
        require(
            IERC20(erc20List.tokenAddress).allowance(
                _msgSender(),
                address(this)
            ) >= _amountTokens,
        ...
        require(
            IERC20(erc20List.tokenAddress).balanceOf(_msgSender()) >=
                _amountTokens,
    ...
    function BuyWithNativeToken(
        ...
        require(
            erc20List.active,
            "Buy With Native Token: Token is not available"
        );

        /// @dev check pair is native
        require(
            erc20List.isNative,
            "Buy With Native Token: The token sended is not native"
        );
