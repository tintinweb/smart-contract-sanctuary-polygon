...
contract InitFacet is AccessControlInternal, UsingDiamondOwner, WithStorage {

    function init(string memory name, string memory symbol, address trustedExchange, string memory uri) external onlyOwner {
        if (s().isInitialized) return;
        
        s()._name = name;
        s()._symbol = symbol;
        s()._trustedExchange = trustedExchange;
        s()._uri = uri;
        ...
        _grantRole(Roles.DEFAULT_ADMIN_ROLE, ds().contractOwner);
        _grantRole(Roles.MINTER_ROLE, ds().contractOwner);
...
abstract contract WithStorage {
    function s() internal pure returns (RVKStorage storage cs) {
        bytes32 position = keccak256("rvk.contracts.storage.RVKGenesisCollection");
        assembly {
           cs.slot := position
        }
    }
