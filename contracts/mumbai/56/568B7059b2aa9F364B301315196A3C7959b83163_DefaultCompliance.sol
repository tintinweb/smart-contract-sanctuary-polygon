...
contract DefaultCompliance is ICompliance, AgentRole, ComplianceStorage, Initializable {
    ...
    event CountryAuthorized(uint16 _country);

    event CountryRemoved(uint16 _country);
    ...
    function init(address _token, uint256 _holdReleaseTime, uint256 _tokenLimitPerUser) external initializer{
        ...
        emit HolderLimitSet(_tokenLimit);
    }
     ...
     */
    function transferOwnershipOnComplianceContract(address _pendingOwner) external override onlyAdmins {
        require(_pendingOwner != address(0), "potential owner can not be zero address");
        _potentialOwner = _pendingOwner;
        emit OwnerNominated(_pendingOwner);
    }

    function acceptOwnershipOnComplianceContract() external {
        require(msg.sender == _potentialOwner, "must be nominated as potential owner");
        transferOwnership(_potentialOwner);
