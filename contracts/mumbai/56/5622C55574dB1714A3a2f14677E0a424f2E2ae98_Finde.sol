...
contract Finde {
    ...
    function addURI(string memory _uri, string memory _country) public {
        ...
        if(addressToIndex[msg.sender] == 0){
            ...
            indexToUser[addressToIndex[msg.sender]].addr = msg.sender;
            indexToUser[addressToIndex[msg.sender]].uri = _uri;
            indexToUser[addressToIndex[msg.sender]].country = _country;
            indexToUser[_uriIndexCounter.current()] = indexToUser[addressToIndex[msg.sender]];
    ...
    function getPagenatedURIs(uint256 _from, uint256 _limit, uint rating , string calldata _country) external view ValidLimit(_from) returns(User[] memory){
        ...
        if ((_from + _limit) > _uriIndexCounter.current()){
             limit = _uriIndexCounter.current() - _from;
        }

        User[] memory data = new User[](limit);
        
        uint returnArrayLength = 0;

        for(uint i=0; i< _limit; i++){
            User memory usr = indexToUser[_from+i];
            if ((usr.votes.upVote - usr.votes.downVote) == rating && (keccak256(abi.encodePacked((usr.country))) == keccak256(abi.encodePacked((_country))))){
