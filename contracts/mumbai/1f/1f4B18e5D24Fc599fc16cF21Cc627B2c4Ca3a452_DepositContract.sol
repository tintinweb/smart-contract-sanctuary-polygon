...
contract DepositContract is Initializable, AccessControlUpgradeable, OwnableUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    
    event DepositedToken(address indexed tokenAddress, address indexed sender, uint256 amount, uint256 nonce);
    ...
    * Contract can not execute this function
    ...
    function depositToken(uint256 amount) external {
        ...
        IERC20Upgradeable(receiveToken).safeTransferFrom(msg.sender, address(this), amount);
        emit DepositedToken(receiveToken, msg.sender, amount, _nonce);
    }
    ...
    * only Admin can execute this function
    ...
    function withdrawToken(address receipient, uint256 amount) external onlyRole(ADMIN_ROLE) {
        IERC20Upgradeable(receiveToken).safeTransfer(receipient, amount);
        emit WithdrawedToken(receiveToken, receipient, amount);
    }
}
