...
contract MyntfloMarketplace is ERC2771Context {
    ...
    function addERC1155(uint256 tokenId, uint256 amount, uint256 price) public {
        require(erc1155Contract.balanceOf(msg.sender, tokenId) > 0, "Sender does not own the token");
        erc1155Prices[tokenId] = price;
        erc1155Inventory[tokenId] = amount;
    }

    // This function allows the owner of the marketplace to add a new ERC-721 token
    // to the marketplace and set its price in payment tokens
    function addERC721(uint256 tokenId, uint256 price) public {
        require(erc721Contract.ownerOf(tokenId) == msg.sender, "Sender does not own the token");
        erc721Prices[tokenId] = price;
        erc721Inventory[tokenId] = 1;
    }

    // This function allows a user to purchase an ERC-1155 token from the marketplace
    function buyERC1155(uint256 tokenId) public payable {
        require(erc1155Prices[tokenId] > 0, "Token is not for sale");
        require(erc1155Inventory[tokenId] >= 1, "Insufficient inventory");

        // Calculate the cost of the purchase
        uint256 cost = erc1155Prices[tokenId];
        ...
        // Transfer the ERC-1155 tokens from the contract to the buyer
        erc1155Contract.safeTransferFrom(address(this), msg.sender, tokenId, 1, '');

        // Update the inventory and emit the purchase event
        erc1155Inventory[tokenId] = erc1155Inventory[tokenId].sub(1);
        emit ERC1155Purchase(msg.sender, tokenId, cost);
    }
    ...
    function buyERC721(uint256 tokenId) public payable {
        ...
        emit ERC721Purchase(msg.sender, tokenId, cost);
    }
