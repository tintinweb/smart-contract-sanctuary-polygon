...
constructor() ERC721 ("Lease", "LEASE") {
  ...
  event AssetCreated(
    ...
    mapping(uint256 => mapping(address => bool)) public multiLeaseCheck;
...
function isLeaseAvailable(uint256 _tokenId, uint256 _start,
        ...
        require(_end > _start);
        require(_end.sub(_start) <= MAX_LEASE_DURATION);
        require(_end.sub(_start) >= MIN_LEASE_DURATION);
    ...
    function lease(uint256 _tokenId, uint256 _start, uint256 _end) external payable {
        require(_end > _start);
        require(_end.sub(_start) <= MAX_LEASE_DURATION, "Lease duration is too long");
        require(_end.sub(_start) >= MIN_LEASE_DURATION, "Lease duration is too short");
        require(isMulti[_tokenId] == false, "Asset is multi-lease, use multiLease function");
        require(_tokenId != 0);
        require(_start != 0);
        require(_end != 0);
      ...
      function isMultiLeaseActive(uint256 _tokenId, uint256 _now) public view returns (bool) {
            require(isMulti[_tokenId] == true, "Asset does not allow multiple leases at a time");
      ...
      function multiLease(uint256 _tokenId, uint256 _start, uint256 _end) external payable {
          require(_tokenId != 0);
          require(_start != 0);
          require(_end != 0);
          require(multiLeaseCheck[_tokenId][msg.sender] == false);
          ...
          multiLeaseCheck[_tokenId][msg.sender] = true;
        }

      function unlease(uint256 _tokenId, uint256 _date) external payable {
        require(isMulti[_tokenId] == false, "This assest is multi-lease, please use multiUnlease function");
        Term memory term = getLease(_tokenId, _date);
        require(term.lessee == msg.sender, 'Sender is not the owner of this lease');
        require(_date > block.timestamp);

        for (uint i=term.startTime; i<=term.endTime; i++) {
          daysTaken[_tokenId][i] = false;
        }
      }


function multiUnlease(uint256 _tokenId) external payable {
    require(isMulti[_tokenId] == true, "Asset is single lease use unlease function");
    require(multiLeaseCheck[_tokenId][msg.sender] == true);
    ...
    for (uint i=0; i<terms.length; i++) {
      if (terms[i].lessee == msg.sender) {
        multiLeaseCheck[_tokenId][msg.sender] == false;
...
function mintAsset(string memory _name, string memory _description, string memory _image, bool _isMulti) external payable {
    _tokenIds.increment();
    uint256 newItemId = _tokenIds.current();
    isMulti[newItemId] = _isMulti;
