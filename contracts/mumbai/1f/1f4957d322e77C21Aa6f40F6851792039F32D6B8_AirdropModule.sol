...
contract AirdropModule is ModuleBase, ReentrancyGuard {
    ...
    /**
     * @dev MANAGER ONLY. Adds new tokens to be added to positions when absorb is called.
     *
     * @param matrixToken    Address of MatrixToken
     * @param airdrop        Component to add to airdrop list
     */
    function addAirdrop(IMatrixToken matrixToken, IERC20 airdrop) external onlyManagerAndValidMatrix(matrixToken) {
        require(!isAirdropToken(matrixToken, airdrop), "AD0"); // "Token already added"

        _isAirdrop[matrixToken][airdrop] = true;
        _airdropSettings[matrixToken].airdrops.push(address(airdrop));

        emit AddAirdropComponent(matrixToken, airdrop);
    }

    /**
     * @dev MANAGER ONLY. Removes tokens from list to be absorbed.
     *
     * @param matrixToken    Address of MatrixToken
     * @param airdrop        Component to remove from airdrop list
     */
    function removeAirdrop(IMatrixToken matrixToken, IERC20 airdrop) external onlyManagerAndValidMatrix(matrixToken) {
        require(isAirdropToken(matrixToken, airdrop), "AD1"); // "Token not added"

        _isAirdrop[matrixToken][airdrop] = false;
        _airdropSettings[matrixToken].airdrops.quickRemoveItem(address(airdrop));

        emit RemoveAirdropComponent(matrixToken, airdrop);
    }

    /**
     * @dev MANAGER ONLY. Update whether manager allows other addresses to call absorb.
     *
     * @param matrixToken    Address of MatrixToken
     */
    ...
    /**
     * @dev MANAGER ONLY. Update address manager fees are sent to.
     *
     * @param matrixToken        Address of MatrixToken
    ...
    function updateFeeRecipient(IMatrixToken matrixToken, address newFeeRecipient) external onlyManagerAndValidMatrix(matrixToken) {
        require(newFeeRecipient != address(0), "AD2"); // "Passed address must be non-zero"
    ...
    /**
     * @dev MANAGER ONLY. Update airdrop fee percentage.
     *
     * @param matrixToken    Address of MatrixToken
    ...
    function updateAirdropFee(IMatrixToken matrixToken, uint256 newFee)
        ...
        onlyValidAndInitializedMatrix(matrixToken)
    {
        require(newFee <= PreciseUnitMath.preciseUnit(), "AD3"); // "Airdrop fee can't exceed 100%"
    ...
    /**
     * @dev MANAGER ONLY. Initialize module with MatrixToken and set initial airdrop tokens
     * as well as specify whether anyone can call absorb.
     *
     * @param matrixToken       Address of MatrixToken
    ...
    {
        require(airdropSetting.feeRecipient != address(0), "AD4a"); // "Zero fee address passed"
        require(airdropSetting.airdropFee <= PreciseUnitMath.preciseUnit(), "AD4b"); // "Fee must be <= 100%"

        if (airdropSetting.airdrops.length > 0) {
            require(!airdropSetting.airdrops.hasDuplicate(), "AD4c"); // "Duplicate airdrop token passed"
        }
    ...
    function _absorb(IMatrixToken matrixToken, IERC20 token) internal {
        require(isAirdropToken(matrixToken, token), "AD5"); // "Must be approved token"
...
interface IMatrixToken is IERC20 {
    ...
    /**
     * @param virtualUnit                Virtual value of a component's DEFAULT position.
