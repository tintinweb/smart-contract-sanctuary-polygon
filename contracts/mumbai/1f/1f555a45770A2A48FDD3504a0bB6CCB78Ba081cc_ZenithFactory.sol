...
contract ZenithFactory is Ownable {
        ...
        external onlyOwner
    {
        ...
        bytes memory bytecode = type(ZenithInitializable).creationCode;
        // pass constructor argument
        bytecode = abi.encodePacked(bytecode, abi.encode(_msgSender(), _admin, _stakedToken, _rewardToken, block.number));
        bytes32 salt = keccak256(abi.encodePacked(_stakedToken, _rewardToken, _startBlock));
        address zenithStakeAddress;
        assembly { zenithStakeAddress := create2(0, add(bytecode, 32), mload(bytecode), salt) }
        
        ZenithInitializable(zenithStakeAddress).initialize(
            _stakedToken,
            _rewardToken,
            _rewardPerBlock,
            _startBlock,
