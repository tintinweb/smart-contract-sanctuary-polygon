...
import "./utils/MultiOperator.sol";

contract ZenoGovernanceToken is ERC20Burnable, MultiOperator
{
    uint256 constant INITIAL_SUPPLY = 1000 ether;

    constructor() ERC20("ZenoGovernanceToken", "ZGT")
    {
...
abstract contract MultiOperator is Ownable
{
    ...
    modifier onlyOperator() {
        require(operators.contains(msg.sender), "Operator: caller is not an operator");
        _;
    }

    function isOperator(address _account) public view returns (bool) {
        return operators.contains(_account);
    }
    ...
    function _addOperator(address _account) internal {
        require(_account != address(0), "Operator: zero address given for new operator");
