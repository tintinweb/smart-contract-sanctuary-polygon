...
contract LimitOrderBook is IPerpetualOrder, IClientOrder, PerpetualHashFunctions {
    ...
    function executeOrder(
        ...
        _handleMarketOpening();
        _executeOrder(_digest, _executorAddr);
    }
    ...
    function executeOrders(
        ...
        uint256 shortestTimeElapsed = block.timestamp - timeLimit;
        for (uint256 i = 1; i < numOrders; ) {
            ...
            uint256 age = block.timestamp - timeLimit;
    ...
    function _executeOrder(bytes32 _digest, address _executorAddr) internal {
        ...
        if (block.timestamp <= order.iDeadline) {
            ...
            try perpManager.tradeViaOrderBook(order, approvedExecutor[msg.sender]) returns (
                bool orderSuccess
            ) {
...
interface IPerpetualLimitTradeManager is IPerpetualOrder {
    function tradeViaOrderBook(Order calldata _order, bool _isApprovedExecutor)
        external
        returns (bool);
