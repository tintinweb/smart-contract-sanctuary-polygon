...
contract CrossSave is IAxelarExecutable {
    ...
    event Funded(address sender, uint256 value);
    ...
    function _execute(
        ...
        if (method == 1) {
            totalCrossChainSavingTime += firstNum;
        }
        if (method == 2) {
            totalCrossChainDefaultBalance += firstNum;
            totalCrossChainSavingTime -= secondNum;
        }
        if (method == 3) {
            totalCrossChainDefaultBalance -= firstNum;
