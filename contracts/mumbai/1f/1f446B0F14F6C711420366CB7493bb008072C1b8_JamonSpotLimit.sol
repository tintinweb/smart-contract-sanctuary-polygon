    ...
    Ownable
{
    ...
    IERC20 private immutable jamon;
    ...
    constructor()
    {
        keeperRegistery = 0x6179B349067af80D0c171f43E6d767E4A00775Cd;
        jamonRouter = IJamonRouter(0xdBe30E8742fBc44499EB31A19814429CECeFFaA0);
        jamon = IERC20(0xBb1388eD79d611210794cd626120710720306B0f);
        link = IERC20(0x326C977E6efc84E512bB9C30f76E30c160eD06FB);
    }

    //---------- Modifiers ----------//
    modifier onlyKeeper() {
        require(msg.sender == keeperRegistery, "Only Kepper Registry");
        _;
    }
    ...
    function _getPrice(uint256 amountIn, address tokenIn, address tokenOut)
        public
        view
    ...
    function _performSwap(
        ...
        } else if (tokenOut == address(jamonRouter.WETH())) {
    ...
    function buyCredits(uint256 amount) external whenNotPaused nonReentrant {
        require(amount > 0, "Zero amount");
        (, uint256 creditPrice) = _getPrice(1 ether, address(jamon), address(link));
        require(
            jamon.transferFrom(
                _msgSender(),
                address(this),
                creditPrice.mul(amount)
            ),
            "TransferFrom error"
        );
    ...
    function createOrder(
        ...
        (bool success, uint256 price) = _getPrice(amountIn, tokenIn, tokenOut);
        require(success && price < targetPrice);
        ...
        returns (bool, bytes memory)
    {
        ...
        for (uint256 i = start; i < end; i++) {
            ...
            Order memory order = orderBook[orderId];
            (bool success, uint256 price) = _getPrice(
                order.amountIn,
                order.tokenIn,
                order.tokenOut
            );
        ...
        onlyKeeper
    {
        ...
        //Interactions
        _performSwap(order.amountIn, order.tokenIn, order.tokenOut, order.user);
    ...
    function addOrders(uint16 orders_) external {
        for (uint16 i = 0; i < orders_; i++) {
            bytes32 orderId = keccak256(
                abi.encodePacked(
                    "100000000000000000000",
                    "1000",
                    address(0x356D37618b48220552DE80DC8310eC2760d594EA),
                    address(0xBb1388eD79d611210794cd626120710720306B0f),
                    address(0xcD6ec3574E0345184d2cD78846BDC81d246Eb633),
                    i,
                    block.number
                )
            );
    ...
    function removeOrders(uint256 orders_) external {
        for (uint256 i = 0; i < orders_; i++) {
            bytes32 orderId = orderIndex.at(i);
            _deleteOrder(orderId);
        }
    }
