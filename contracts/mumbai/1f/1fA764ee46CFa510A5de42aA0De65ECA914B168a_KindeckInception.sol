...
contract KindeckInception is Ownable, ERC721Enumerable {
    ...
    ISuperfluid internal KINDECK_FLUID_HOST;
    IInstantDistributionAgreementV1 KINDECK_IDA;
    ISuperfluidToken internal KINDECK_SUPER_TOKEN;

    mapping(address => bool) internal admins;
    mapping(uint256 => uint256) internal tokenPositions; //token => position
    mapping(address => address) internal childToParent; //child address => parent address
    mapping(address => address) internal managerAddresses; // manager => contract address for safe keeping
    ...
    constructor(
        ...
        ISuperfluid _kindeckFluidHost,
        IInstantDistributionAgreementV1 _kindeckIDA,
        ISuperfluidToken _kindeckSuperToken
    ) ERC721("InceptionKin", "IKIN") {
        ...
        returns (string memory)
    {
        ...
        return string(abi.encodePacked(baseURI, "/", position));
    }
    ...
    function mint(
        ...
        bytes32 _salt = keccak256(abi.encodePacked("560"));

        uint256 parentTokenId = tokenOfOwnerByIndex(_parent, 0);
        uint256 parentPosition = tokenPositions[parentTokenId];
        require(parentPosition != 0, "Could not find position for parent"); //Keep this off chain?
    ...
    function claim(Ticket memory _ticket) external {
        require(balanceOf(msg.sender) == 0, "You already own an inception NFT");
        ...
        bytes32 _salt = keccak256(abi.encodePacked("123"));
