...
contract RiskManager_Covest is Initializable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event SignatureApproved(address indexed user, uint8 indexed riskCoefficient , bool indexed boolean);
    ...
    function initialize(address _nameRegistry,uint8 _maxRiskCoefficient) public initializer {
        require(_maxRiskCoefficient > 0,"maxRiskCoefficient should be more than 0.");
    ...
    function approveUser(address _user,uint8 _riskCoefficient) public nonReentrant isReady isUnderWriter {
        require(UW.isUnderWriter(_user) == false,"You are UnderWriter You can't approve your self.");
        require(_riskCoefficient > 0 && _riskCoefficient <= maxRiskCoefficient,"RiskManager : Risklevel Error.");
        require(_isApproved[_user] != true, "RiskManager: Already Approve.");

        S.update(msg.sender,IStaking.typeUser.UW);
    ...
    function reApproveUser(address _user,uint8 _riskCoefficient) public nonReentrant isReady isSuperManager {
        require(UW.isUnderWriter(_user) == false,"You are UnderWriter You can't approve your self.");
        require(_riskCoefficient > 0 && _riskCoefficient <= maxRiskCoefficient,"RiskManager : Risklevel Error.");
    ...
    function riskProfileUser(address _user) public view returns(bool approved, uint8 riskCoefficient) {
        return (_riskProfile[_user].approved, _riskProfile[_user].riskCoefficient);
    }
}
