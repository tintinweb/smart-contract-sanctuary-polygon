    ...
    ReentrancyGuard
{
    ...
    uint16 public constant MAX_SUPPLY = 1111;
    //the sale price per token
    uint256 public constant MINT_PRICE = 0.1 ether; // 0.1 MATIC
    ...
    ) ERC721("Simple Polygon Nouns", "SPOUNS") {
    ...
    function safeMint(address to, string memory) public onlyOwner {
        if (_currentId >= MAX_SUPPLY) revert InvalidCall();
        _mintTo(to, _currentId++);
    }
    ...
    function mint() external payable nonReentrant {
        ...
        if (recipient.code.length > 0
            || !mintOpened
            || _currentId >= MAX_SUPPLY
            || MINT_PRICE > msg.value
        ) revert  InvalidCall();
        _mintTo(recipient, _currentId++);
    }
    ...
    */
    function _mintTo(address _to, uint256 _tokenId) internal {
        ...
        seeds[_tokenId] = seeder.generateSeed(_tokenId, descriptor);
    }
     ...
     * @dev Starts the sale
     */
    function openMint(bool yes) external onlyOwner {
        mintOpened = yes;
    }
