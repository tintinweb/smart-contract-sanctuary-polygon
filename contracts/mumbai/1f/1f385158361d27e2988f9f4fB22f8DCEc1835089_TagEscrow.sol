    ...
    Ownable
{
    ...
    **/
    ...
    // -- VIEWS
    function getCallerEscrows() public view returns (uint256[] memory escr) {
        uint length = walletEscrows[msg.sender].length;
        escr = new uint256[](length);

        for(uint i = 0; i < length; i++) {
            escr[i] = walletEscrows[msg.sender][i].escrowId;
        }
    }
}