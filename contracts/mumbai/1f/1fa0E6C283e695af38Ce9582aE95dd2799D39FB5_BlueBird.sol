...
contract BlueBird is Initializable, ERC20Upgradeable, ERC20BurnableUpgradeable {
    ...
    function mintFromMatic() external payable checkBalance(address(msg.sender).balance,msg.value){
        ...
        minter4minted["ecosystem"] = minter4minted["ecosystem"]+uint(msg.value/1e18)*mintPrice;
        _mint(msg.sender, uint(msg.value/1e18)*mintPrice);
        emit mintFromMaticEvent(msg.sender, uint(msg.value/1e18)*mintPrice, mintPrice);
    }
