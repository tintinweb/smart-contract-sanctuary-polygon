...
import "./CedarERC721PremintV1Factory.sol";
import "./drop/CedarERC721DropFactory.sol";
import "./drop/CedarERC1155DropFactory.sol";
import "./paymentSplit/CedarPaymentSplitterV0Factory.sol";
import "./generated/deploy/BaseCedarDeployerV6.sol";

contract CedarDeployer is Initializable, UUPSUpgradeable, OwnableUpgradeable, BaseCedarDeployerV6 {
    CedarERC721PremintV1Factory premintV1Factory;
    CedarERC721DropFactory drop721Factory;
    CedarERC1155DropFactory drop1155Factory;
    CedarPaymentSplitterV0Factory paymentSplitterV0Factory;

    function initialize(
        CedarERC721PremintV1Factory _premintV1Factory,
        CedarERC721DropFactory _drop721Factory,
        CedarERC1155DropFactory _drop1155Factory,
        CedarPaymentSplitterV0Factory _paymentSplitterV0Factory
    ) public virtual initializer {
        premintV1Factory = _premintV1Factory;
        drop721Factory = _drop721Factory;
        drop1155Factory = _drop1155Factory;
        paymentSplitterV0Factory = _paymentSplitterV0Factory;
    ...
    function deployCedarERC721PremintV1(
    ...
    ) external override returns (ICedarERC721PremintV1) {
        CedarERC721PremintV1 newContract = premintV1Factory.deploy(
            adminAddress,
            _name,
            _symbol,
            _maxLimit,
        ...
        emit CedarInterfaceDeployed(address(newContract), major, minor, patch, "ICedarERC721PremintV1");
        return newContract;
    }
    ...
    function deployCedarERC721DropV3(
    ...
    ) external override returns (ICedarERC721DropV3 newContract) {
        newContract = drop721Factory.deploy(
            _defaultAdmin,
            _name,
            _symbol,
            _contractURI,
        ...
        (uint256 major, uint256 minor, uint256 patch) = newContract.implementationVersion();
        // TODO: shift the emission of this event to generated base class so it does not need manual updating
        emit CedarInterfaceDeployed(address(newContract), major, minor, patch, "ICedarERC721DropV3");
    }
    ...
    function deployCedarERC1155DropV1(
        ...
        uint128 _platformFeeBps,
        address _platformFeeRecipient
    ) external override returns (ICedarERC1155DropV1 newContract) {
        newContract = drop1155Factory.deploy(
            _defaultAdmin,
            _name,
            _symbol,
            _contractURI,
        ...
        (uint256 major, uint256 minor, uint256 patch) = newContract.implementationVersion();
        emit CedarInterfaceDeployed(address(newContract), major, minor, patch, "ICedarERC1155DropV1");
    }
    ...
    {
        return premintV1Factory.implementationVersion();
    }
    ...
    function cedarERC721PremintFeatures() public view override returns (string[] memory features) {
        return premintV1Factory.implementation().supportedFeatures();
    }
    ...
    external
    override
    returns (ICedarPaymentSplitterV0 newContract)
    {
        newContract = paymentSplitterV0Factory.deploy(payees, shares_);

        (uint256 major, uint256 minor, uint256 patch) = newContract.implementationVersion();
        emit CedarInterfaceDeployed(address(newContract), major, minor, patch, "ICedarPaymentSplitterV0");
    }
    ...
    {
        return paymentSplitterV0Factory.implementationVersion();
    }

    function cedarPaymentSplitterFeatures() external view override returns (string[] memory features) {
        return paymentSplitterV0Factory.implementation().supportedFeatures();
    }
}
...
// Filename: './BaseCedarDeployerV6.sol'
...
abstract contract BaseCedarDeployerV6 is ICedarDeployerV6 {
    ...
    function supportsInterface(bytes4 interfaceID) virtual override public view returns (bool) {
        return (interfaceID == type(IERC165Upgradeable).interfaceId) || ((interfaceID == type(ICedarDeployerV6).interfaceId) || ((interfaceID == type(ICedarDeployerEventsV5).interfaceId) || ((interfaceID == type(ICedarDeployerAddedV7).interfaceId) || ((interfaceID == type(ICedarDeployerAddedV3).interfaceId) || ((interfaceID == type(ICedarDeployerAddedV2).interfaceId) || (interfaceID == type(ICedarDeployerIntrospectionV0).interfaceId))))));
    }
}
...
import "./interfaces/IPlatformFee.sol";
import "./openzeppelin-presets/metatx/ERC2771ContextUpgradeable.sol";
import "../generated/impl/BaseCedarERC1155DropV1.sol";
    ...
    BaseCedarERC1155DropV1
{
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(ERC1155Upgradeable, AccessControlEnumerableUpgradeable, BaseCedarERC1155DropV1)
        returns (bool)
    {
     ...
     *       The URIs for each token is the provided `_baseURIForTokens` + `{tokenId}`.
     */
    function lazyMint(uint256 _amount, string calldata _baseURIForTokens) override external onlyRole(MINTER_ROLE) {
    ...
    function issue(
        ...
        if(!(maxTotalSupply[_tokenId] == 0 || totalSupply[_tokenId] + _quantity <= maxTotalSupply[_tokenId])) revert CrossedLimit();
        if(!(maxWalletClaimCount[_tokenId] == 0 || walletClaimCount[_tokenId][_msgSender()] + _quantity <= maxWalletClaimCount[_tokenId])) revert CrossedLimit();

        walletClaimCount[_tokenId][_msgSender()] += _quantity;
    ...
    function verifyClaim(
        ...
        if(!(_currency == currentClaimPhase.currency && _pricePerToken == currentClaimPhase.pricePerToken)) revert InvalidPrice();
        if(!( _quantity > 0 && (!verifyMaxQuantityPerTransaction || _quantity <= currentClaimPhase.quantityLimitPerTransaction))) revert CrossedLimit();
        if(!(currentClaimPhase.supplyClaimed + _quantity <= currentClaimPhase.maxClaimableSupply)) revert CrossedLimit();
        if(!(maxTotalSupply[_tokenId] == 0 || totalSupply[_tokenId] + _quantity <= maxTotalSupply[_tokenId])) revert CrossedLimit();
        if(!(maxWalletClaimCount[_tokenId] == 0 || walletClaimCount[_tokenId][_claimer] + _quantity <= maxWalletClaimCount[_tokenId])) revert CrossedLimit();
    ...
    function verifyClaimMerkleProof(
        ...
        if (currentClaimPhase.merkleRoot != bytes32(0)) {
            ...
            if(!validMerkleProof) revert InvalidProof();
            if(!(!claimCondition[_tokenId].limitMerkleProofClaim[_conditionId].get(merkleProofIndex))) revert InvalidProof();
            if(!(_proofMaxQuantityPerTransaction == 0 || _quantity <= _proofMaxQuantityPerTransaction)) revert InvalidQuantity();
        }
    }
    ...
    /// @dev Expose the current active claim condition including claim limits
    function getActiveClaimConditions(uint256 _tokenId) external view override returns (ClaimCondition memory condition, uint256 conditionId, uint256 walletMaxClaimCount, uint256 remainingSupply) {
        conditionId = getActiveClaimConditionId(_tokenId);
        condition = claimCondition[_tokenId].phases[conditionId];
        walletMaxClaimCount = maxWalletClaimCount[_tokenId];
        remainingSupply = 0; // @todo
    }

    /// @dev Expose the user specific limits related to the current active claim condition
    function getUserClaimConditions(uint256 _tokenId, address _claimer) external view override returns (uint256 conditionId, uint256 walletClaimedCount, uint256 lastClaimTimestamp, uint256 nextValidClaimTimestamp) {
        conditionId = getActiveClaimConditionId(_tokenId);
        (lastClaimTimestamp, nextValidClaimTimestamp) = getClaimTimestamp(_tokenId, conditionId, _claimer);
        walletClaimedCount = walletClaimCount[_tokenId][_claimer];
    }
    ...
    /// @dev Returns the default royalty recipient and bps.
    function getDefaultRoyaltyInfo() override external view returns (address, uint16) {
        return (royaltyRecipient, uint16(royaltyBps));
    }
    ...
    /// @dev Lets a contract admin set a new owner for the contract. The new owner must be a contract admin.
    function setOwner(address _newOwner) external onlyRole(DEFAULT_ADMIN_ROLE) override {
        if(!(hasRole(DEFAULT_ADMIN_ROLE, _newOwner))) revert InvalidPermission();
    ...
    /// @dev Lets a contract admin set the URI for contract-level metadata.
    function setContractURI(string calldata _uri) external onlyRole(DEFAULT_ADMIN_ROLE)  override {
        contractURI = _uri;
    }
