...
contract Boom is ERC20, ERC20Burnable, Ownable, Blacklist {
    ...
    function setContributionRate(uint256 rate) external onlyOwner() {
        require(((rate <= BASE_PRECISION) && (rate >= 0)) , "Rate should be be between 0 and 100");        
    ...
    function setContributionAddress(address _target) external onlyOwner() {        
    ...
    function excludeAccount(address account, bool isSender) external onlyOwner() {
        if (isSender) {
    ...
    function includeAccount(address account, bool isSender) external onlyOwner() {
        if (isSender) {
