...
contract SlaveCrossLedgerVault is CrossLedgerVault {
    ...
    function depositFundsToRootVault(
        ...
        uint256 totalAssetsBefore = rootVault.convertToAssets(rootVault.balanceOf(address(this)));
        shares = super.depositFundsToRootVault(token, value, transferId);
        uint256 totalAssetsAfter = rootVault.convertToAssets(rootVault.balanceOf(address(this)));
...
abstract contract CrossLedgerVault is LzApp {
	...
	enum LzMessage { UPDATE, REDEEM, REBALANCE, DEPOSITED_FROM_BRIDGE }
	...
	function _encodeRebalanceMessage(uint256 value, address asset) pure internal returns(bytes memory) {
		bytes memory params = abi.encode(value, asset);
...
interface IBridgeAdapter
{
	...
	function dstAssets(address srcAsset) external returns(address dstAsset);
}
