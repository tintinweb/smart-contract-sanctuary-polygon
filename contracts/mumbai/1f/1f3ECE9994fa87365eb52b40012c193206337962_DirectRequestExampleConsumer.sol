...
contract DirectRequestExampleConsumer is ChainlinkClient {
  using Chainlink for Chainlink.Request;
  ...
  function requestENSAddressInfo(string calldata _ensName) external returns (bytes32) {
    Chainlink.Request memory request = buildChainlinkRequest(s_jobID, address(this), this.fulfill.selector);
    request.add("ensName", _ensName);
    bytes32 reqID = sendChainlinkRequestTo(s_oracle, request, FEE);
  ...
  function setJobID(string calldata _jobID) external {
    s_jobID = keccak256(abi.encode(_jobID));
  }
