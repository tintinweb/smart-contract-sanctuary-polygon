...
contract IamTest is ERC721A, Ownable {
    ...
    uint private constant MAX_MINT_AMOUNT = 3;
    ...
    //     require(success, "Failed to verify Paper signature");
    //     _;
    // }
    ...
    modifier mintCompliant(
        ...
        require(block.timestamp >= launchDate, "Purchase available from XX/XX/XXXX Xpm CEST");
        _;
    }
    ...
    // Claiming function for native minter
    function claimTo(
        address _recipient,
        uint256 _quantity
    ...
    // Claiming function for Paper Minter
    // function claimToPaper(
    //     address _recipient,
    //     uint256 _quantity,
    //     bytes32 _nonce,
    //     bytes calldata _signature
    // ) external payable mintCompliant(_recipient, _quantity) onlyPaper(keccak256(abi.encode(_recipient, _quantity)), _nonce, _signature) {
    //     amountNFTsperWallet[_recipient] += _quantity;
    //     _safeMint(_recipient, _quantity);
    // }
    ...
    function getClaimIneligibilityReason(
        ...
        if (
            ...
            return "Invalid quantity, you can only buy 3 tokens";
        }
        else if (amountNFTsperWallet[_recipient] + _quantity > MAX_MINT_AMOUNT) {
            return "You are trying to buy more passes than you can, reduce the quantity!";
        }
        return "";
    }
    ...
    function withdrawCurrency() external onlyOwner {
        ...
        bool success1 = IERC20(currency).transfer(msg.sender, balance*50/100); // METABAY
        bool success2 = IERC20(currency).transfer(0x4588069c142A866C564541Dd3a29353cB08A4195, balance*50/100); // ADDRESS WALLET CREATOR
    ...
    // Set tranferable to boolean
    function setTransferable(bool _transferable) public onlyOwner {
        transferable = _transferable;
    }

    // Set tranferable to true when collection is sold out
    function soldOut() private {
        transferable = true;
    }
