...
contract mPCKT is Ownable, IERC20, IERC20Metadata, AccessControlEnumerable, Pausable {
   ...
   // address private constant PancakeRouter=0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
    ...
    address private immutable PancakeRouter;
    ...
    constructor(
        ...
        require(_vault != address(0), "ERC20: vault address not set");
        ...
        _systemContracts.add(address(_vault)), "error adding to lists");
...
//        isSwapping = false;
    ...
    function setBurnTax(
        ...
        require(_burnTax + partnerTax <= 100,'tax too high');
        burnTax = _burnTax;
    ...
    function setPartnerToken(address _partnerToken, address _partnerTokenFeeAddress, uint256 _partnerTax) external onlyTeam {
        require(_partnerTax + burnTax <= 100,'tax too high');
    ...
    function addWhitelistAddress(address _addr) external onlyTeam {
        require(_whitelist.add(_addr), 'list error');
    }

    function removeWhitelistAddress(address _addr) external onlyTeam {
        require(_whitelist.remove(_addr), 'list error');
    }

    function addBlacklistAddress(address _addr) external onlyTeam {
        require(_blacklist.add(_addr), 'list error');
    }

    function removeBlacklistAddress(address _addr) external onlyTeam {
        require(_blacklist.remove(_addr), 'list error');
    }
    ...
    function setVaultContract(address _vault) external onlyTeam {
        require(_vault != address(0), "ERC20: vault address not set");
