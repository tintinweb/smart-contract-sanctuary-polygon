    ...
    ERC2981
{
    ...
    function genUri(string memory uriHash, uint256 _tokenId)
        internal
        view
        returns (string memory)
    {
    ...
    function safeMint(
        ...
        string memory uri = genUri(uriHash, _tokenId);
        _safeMint(_to, _tokenId);
        _setTokenURI(_tokenId, uri);
        setRoyaltyForToken(_tokenId, msg.sender, _royaltyFee);
        tokenIdforUri[uriHash] = _tokenId;
        return _tokenId;
    }
    ...
    function airDrop(
        ...
        for (uint32 i = 0; i < _addresses.length; i++) {
            ...
            string memory uri = genUri(uriHashes[i], _tokenId);
            _setTokenURI(_tokenId, uri);
            setRoyaltyForToken(_tokenId, msg.sender, _royaltyFee);
            tokenIdforUri[uriHashes[i]] = _tokenId;
            tokenIds[i] = _tokenId;
        }
        return tokenIds;
    }
        ...
        returns (uint256)
    {
        ...
        tokenIdCounter.increment();
        uint256 _tokenId = tokenIdCounter.current();
        string memory fullUri = genUri(voucher.uri, _tokenId);

        _safeMint(msg.sender, _tokenId);
        _setTokenURI(_tokenId, fullUri);
