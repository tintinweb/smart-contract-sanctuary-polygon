...
/*
The MATIC forecast for 2022 mostly looks positive. If buyers 
...
Help Invester For Earn More Matic And More Profit
*/
...
contract MaticGlobalMini {
    ...
    uint256[6] public defaultPackages = [10 ether,20 ether,50 ether,100 ether,500 ether,1000 ether];
    uint256[6] public nonWorkingCapping = [20 ether,40 ether,100 ether,200 ether,1000 ether,2000 ether];
    ...
    constructor() public {
		primaryAdmin = 0xBf02B1f172844d941DfB593600CF721a32B1F723;
  ...
  function withdrawal() external{
    ...
    if(_userDetails.nonworkingIncomeCapping<(_user.referrerBonus+_user.singleDownlineBonus+_userUplineIncome(msg.sender))){
        uint256 ExtraPayable=(_user.referrerBonus+_user.singleDownlineBonus+_userUplineIncome(msg.sender)).sub(_userDetails.nonworkingIncomeCapping);
  ...
  function _refPayout(address _addr, uint256 _amount) internal {
        ...
        for(uint8 i = 0; i < ref_bonuses.length; i++) {
            if(up == address(0)) break;
            if(users[up].refs[0] >= requiredDirect[i] || users[up].amount>=500){ 
  ...
  function _usersDownlineIncomeDistribution(address _user, uint256 _Amount) internal {
      ...
      for (uint i = 0; i < maxupline; i++) {
            ...
            if(usersDetails[uplineparam].nonworkingIncomeCapping>=(payableAmount+users[_user].referrerBonus+users[_user].singleDownlineBonus+_userUplineIncome(_user))){
  ...
  function getEligibleWithdrawal(address _user) public view returns(uint8 reivest, uint8 withdrwal){  
   ...
   function _distributeNonWorkingFromPool(uint256 _Amount,uint _TotalNoofUser,uint _ToSingleLegLength) internal{
        ...
        for (uint i = 0; i < _TotalNoofUser; i++) {
            ...
            if (singleLeg[getRandom(_ToSingleLegLength)] != address(0)) {
               if(payableAmount <= availablepool_amount && (usersDetails[_user].nonworkingIncomeCapping>=(payableAmount+users[_user].referrerBonus+users[_user].singleDownlineBonus+_userUplineIncome(_user)))){
   ...
   function _distributeNonWorkingFromPoolToUser(uint256 _Amount,address _user) internal{
      if(_Amount <= availablepool_amount && (usersDetails[_user].nonworkingIncomeCapping>=(_Amount+users[_user].referrerBonus+users[_user].singleDownlineBonus+_userUplineIncome(_user)))){
   ...
   function distributeNonWorkingFromPool(uint256 _Amount,address _user) external {
	     require(primaryAdmin==msg.sender, 'Admin what?');
