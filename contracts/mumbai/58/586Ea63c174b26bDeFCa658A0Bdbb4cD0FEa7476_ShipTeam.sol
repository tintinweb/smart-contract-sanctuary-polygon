...
contract ShipTeam is Managable, Pausable {
    ...
    mapping(uint256 => uint256[]) public teamShips;
    mapping(uint256 => address) public teams;
    mapping(address => uint256[]) public addressTeams;

    event ChangedShipAddress(address _addr);
    event ShipTeamCreated(uint256 indexed _teamId, uint256[] _shipIds);
    event ShipTeamDisbanded(uint256 indexed _teamId);
    event ChangedDamageFixAddress(address _addr);
    event ShipTeamFullyFixed(uint256 indexed _teamId);
    ...
    function createTeam(uint256 _teamId, uint256[] calldata _shipIds) external whenNotPaused { 
        _createTeam(msg.sender, _teamId, _shipIds);
    }
    ...
    /// @notice function instantenously fixes and disbands the team using speedUp for all the ships
    ...
    function disbandTeam(LazyTeam.Team calldata _team, bytes calldata _signature) external whenNotPaused {
        ...
        for(uint i = 0; i < _team.botOrShipDamages.length; i++){
            require(_team.botOrShipDamages[i] == 0, "Ship Is Damaged");
        }
    ...
    function _fixTeamFully (LazyTeam.Team calldata _team, bytes calldata _signature, bool _speedUp) internal {
        ...
        uint256[] memory _ships = teamShips[_team.teamId];
        require(_ships.length == teamCapacity, "no such team");

        address _shipAddress = shipAddress;
        for(uint i = 0; i < _team.botOrShipDamages.length; i++){
            if(_team.botOrShipDamages[i] > 0){
                try IDamageFix(damageFixAddress).fixDurability(_owner, _team.botOrShipDamages[i], _shipAddress, _ships[i], _speedUp) {
        ...
        emit ShipTeamFullyFixed(_team.teamId);
    }    
    ...
    function _disbandTeam(uint256 _teamId) internal {
        uint256[] memory _ships = teamShips[_teamId];
        require(_ships.length == teamCapacity, "no such team");
        ...
        for(uint256 i = 0; i < _ships.length; i++) {
            (,uint _fixEnd,) = IDamageFix(damageFixAddress).getFixedItemDetails(shipAddress, _ships[i]);
            require(_fixEnd <= block.timestamp, "Ship is being Fixed");
        }

        for(uint256 i = 0; i < _ships.length; i++) {
            IERC721(shipAddress).transferFrom(address(this), _owner, _ships[i]);
        }

        delete(teamShips[_teamId]);
        ...
        emit ShipTeamDisbanded(_teamId);
    }

    function _createTeam(address _owner, uint256 _teamId, uint256[] calldata _shipIds) internal {
        require(teams[_teamId] == address(0), "this team already exists");
        require(_shipIds.length == teamCapacity, "bad team capacity");

        for(uint256 i = 0; i < _shipIds.length; i++) {
            IERC721(shipAddress).transferFrom(_owner, address(this), _shipIds[i]);
        }

        teamShips[_teamId] = _shipIds;
        teams[_teamId] = _owner;
        addressTeams[msg.sender].push(_teamId);

        emit ShipTeamCreated(_teamId, _shipIds);
    }
