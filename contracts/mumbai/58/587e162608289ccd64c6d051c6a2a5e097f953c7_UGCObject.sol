...
import { BaseUGC } from "../utils/BaseUGC.sol";
import { ReentrancyGuard } from "@openzeppelin/security/ReentrancyGuard.sol";
import { ERC1155, ERC1155Supply } from "@openzeppelin/token/ERC1155/extensions/ERC1155Supply.sol";

/// @title UGCObject
/// @dev The UGCObject contract inherits from UGCBase.
/// It represents unique digital assets created by users.
/// These UGCObjects are created through the UGCObjectFactory and are incorporated into
/// the Phi ecosystem as materials, products, and catalysts
contract UGCObject is BaseUGC, ReentrancyGuard, ERC1155Supply {
    /* -------------------------------------------------------------------------- */
    /*                                   EVENTS                                   */
    /* -------------------------------------------------------------------------- */
    ...
    event LogCraftObject(address indexed sender, uint256 tokenid);
    // Event to be emitted when the craftLogic contract is set.
    event SetCraftLogic(address indexed newCraftLogic);
    ...
    /*                               INITIALIZATION                               */
    /* -------------------------------------------------------------------------- */
    constructor(string memory _name, string memory _symbol, address _craftLogicAddress) ERC1155("") {
        if (_craftLogicAddress == address(0)) revert ZeroAddress();
        name = _name;
        symbol = _symbol;
        craftLogicAddress = _craftLogicAddress;
    }

    /* -------------------------------------------------------------------------- */
    /*                                  MODIFIERS                                 */
    /* -------------------------------------------------------------------------- */
    // Modifier to ensure only the craftLogic contract can call a function.
    ...
    function setCraftLogic(address _craftLogicAddress) external override onlyOwner {
        if (_craftLogicAddress == address(0)) revert ZeroAddress();
        craftLogicAddress = _craftLogicAddress;
        emit SetCraftLogic(_craftLogicAddress);
    }

    // Function to create a new object
    function createObject(uint256 tokenId, string memory tokenUri) external override onlyOwner {
    ...
    function craftObject(address to, uint256 tokenid, uint256 amount) external override onlyCraftLogic nonReentrant {
        ...
        // Mint the token
        super._mint(to, tokenid, amount, "");
        emit LogCraftObject(_msgSender(), tokenid);
    }
...
abstract contract BaseUGC is MultiOwner {
    string public name;
    string public symbol;
    address public craftLogicAddress;
    ...
    function setCraftLogic(address _craftLogicAddress) external virtual;

    function createObject(uint256 tokenId, string memory tokenUri) external virtual;

    function getObject(address to, uint256 tokenid, uint256 amount) external virtual;

    function craftObject(address to, uint256 tokenid, uint256 amount) external virtual;

    function burnObject(address from, uint256 tokenid, uint256 amount) external virtual;

    function burnBatchObject(address from, uint256[] memory tokenids, uint256[] memory amounts) external virtual;
}
