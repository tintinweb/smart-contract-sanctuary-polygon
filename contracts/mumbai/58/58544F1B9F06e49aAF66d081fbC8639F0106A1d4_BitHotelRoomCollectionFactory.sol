...
contract BitHotelRoomCollectionFactory is AccessControl {
    ...
    function deployAuction(
        ...
        for (uint256 i = 1; i <= replicas_; i++) {
            ...
            if (prices[1] == 0 || i == 1) {
                auction.setupAuction(creator, address(newCollection), i, prices[0], duration, true);
            } else {
                auction.setupAuction(creator, address(newCollection), i, prices[1], duration, true);
            }
        }
    ...
    function deployAuction(
        ...
        if (prices[1] == 0 || tokenId == 1) {
            auction.setupAuction(creator, collectionAddress, tokenId, prices[0], duration, false);
        } else {
            auction.setupAuction(creator, collectionAddress, tokenId, prices[1], duration, false);
        }
...
contract EnglishAuction is AccessControl {
    ...
    function setupAuction(
        ...
        emit AddAuction(_auctionId, _nftAddress, _tokenId, _duration, _startingBid, _msgSender(), nativeWLA);
    }
