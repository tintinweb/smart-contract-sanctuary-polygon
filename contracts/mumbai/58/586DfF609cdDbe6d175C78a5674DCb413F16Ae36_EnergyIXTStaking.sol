...
interface IEnergyIXTStaking {
    ...
    /**
     * @notice Add rewards to Energy Staking Pools
    ...
    /**
     * @notice Unstake Energy and IXT in the ratio 1:1
     * @param _amount Amount of IXT to unstake
     * @param _donate Option to donate portion of earnings to Amelia Foundation
     * @notice emit {EnergyUnstaked} event
     */
    function unstake(uint256 _amount, bool _donate) external;
     ...
     */
    function claim(bool _donate) external;
     ...
     */
    function earned(address _player) external view returns (uint256);
...
contract EnergyIXTStaking is IEnergyIXTStaking, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    /// @notice IXT contract
    IERC20Upgradeable private pixt;
    /// @notice Reward Token
    ...
    modifier onlyMod() {
        require(moderators[msg.sender], "ENERGY STAKING: NON_MODERATOR");
        _;
    }
    ...
    function stake(uint256 _amount) external nonReentrant {
        ...
        energy.trustedBurn(msg.sender, 24, _amount);
        pixt.transferFrom(msg.sender, address(this), _amount * 1 ether);
    }

    /// @inheritdoc IEnergyIXTStaking
    function unstake(uint256 _amount, bool _donate) external nonReentrant {
        require(_amount > 0, "ENERGY STAKING: CANT UNSTAKE 0");
        require(userStakedAmount[msg.sender] >= _amount, "ENERGY STAKING: INSUFFICIENT AMOUNT STAKED");
        ...
        uint256 rewardToPay;
        if (_donate) {
            rewardToPay = (s_rewardPool.rewards[msg.sender] * s_ameliaFoundationShare) / s_ameliaFoundationMaxBps;
            uint256 ameliaPortion = s_rewardPool.rewards[msg.sender] - rewardToPay;
            rewardToken.transfer(s_ameliaFoundation, ameliaPortion);
        } else {
            rewardToPay = s_rewardPool.rewards[msg.sender];
        }

        if (rewardToPay > 0) {
            s_rewardPool.rewards[msg.sender] -= rewardToPay;
            rewardToken.transfer(msg.sender, rewardToPay);
        }
        pixt.transfer(msg.sender, _amount * 1 ether);
    }

    /// @inheritdoc IEnergyIXTStaking
    function claim(bool _donate) external nonReentrant {
        require(userStakedAmount[msg.sender] > 0, "ENERGY STAKING: NOTHING STAKED");
        _updateRewards(msg.sender);
    ...
    function _updateRewards(address _walletAddress) internal {
        s_rewardPool.rewardPerTokenStored += rewardPerTokenStored();
        s_rewardPool.lastUpdateTime = lastUpdateTimeApplicable(block.timestamp);

        if (_walletAddress != address(0)) {
            s_rewardPool.rewards[_walletAddress] = earned(_walletAddress);
    ...
    function earned(address _walletAddress) public view returns (uint256) {
        return (userStakedAmount[_walletAddress] * (s_rewardPool.rewardPerTokenStored - s_rewardPool.userRewardPerTokenPaid[msg.sender])) + s_rewardPool.rewards[msg.sender];
    }
    ...
    function rewardPerTokenStored() public view returns (uint256) {
        ...
        uint256 untilTime = lastUpdateTimeApplicable(block.timestamp);
        uint256 reward = s_rewardPool.rewardRate * (untilTime - startTime);

        return reward / s_rewardPool.tokensStaked;
    }

    /// @notice Finds the last update time that was applicable, does not allow it to go beyond the epoch finish time
    function lastUpdateTimeApplicable(uint256 _untilTimeStamp) public view returns (uint256) {
    ...
    function addCurrentEpochRewards(uint256 _poolTotalReward, uint256 _epochDuration) external onlyRewardDistributor {
        require(s_rewardPool.periodFinish < block.timestamp, "ENERGY STAKING: EPOCH NOT YET FINISHED");
        ...
        s_rewardPool.rewardRate = totalRewardForThisEpoch / _epochDuration;
        s_rewardPool.lastUpdateTime = block.timestamp;
        s_rewardPool.periodFinish = block.timestamp + _epochDuration;

        rewardToken.transferFrom(s_rewardDistributor, address(this), _poolTotalReward);
     ...
     */
    function _facilityCalculation(uint256 _totalRewards) public returns (uint256) {
        uint256 facilityTotalSupply = energy.totalSupply(19) + energy.totalSupply(20) + energy.totalSupply(21) + energy.totalSupply(22) + energy.totalSupply(23);

        uint256 exponentPart = ABDKMathQuad.toUInt(ABDKMathQuad.exp(ABDKMathQuad.fromUInt((facilityTotalSupply - 1) / (4138))));
        uint256 poolShare = 10000 - (9900 / exponentPart);
        return (_totalRewards * poolShare) / 10000;
    }
    ...
    function _pushLockedPoolRewardToNextEpochs(uint256 _lockedRewards) internal {
        ...
        for (uint256 i; i < len; i++) {
            s_rewardPool.pushedRewards[s_epochCount + i] += (_lockedRewards * s_pushRewardToNextEpochPercentages[i]) / s_rewardsPercentageDenominator;
        }
    }
     ...
     */
    function setPushRewardNextEpochsPercentages(uint256[] calldata _pushRewardNextEpochsPercentages) external onlyMod {
     ...
     */
    function setAmeliaFoundationWallet(address _walletAddress) external onlyMod {
     ...
     */
    function setAmeliaFoundationShare(uint256 _share, uint256 _totalBps) external onlyOwner {
        s_ameliaFoundationShare = _share;
        s_ameliaFoundationMaxBps = _totalBps;
    }

    /// @inheritdoc IEnergyIXTStaking
    function setRewardDistributor(address _distributor) external onlyMod {
     ...
     */
    function setRewardPercentageDenominator(uint256 _denominator) external onlyMod {
     ...
     */
    function setEnergy(IEnergy _energy) external onlyOwner {
        energy = _energy;
    }
    ...
    function getCurrentRewardRate() external view returns (uint256) {
        return s_rewardPool.rewardRate;
    }
    ...
    function getCurrentEpochEndTime() external view returns (uint256) {
        return s_rewardPool.periodFinish;
    }

    /// @notice Gets last update time
    function getLastUpdateTime() external view returns (uint256) {
        return s_rewardPool.lastUpdateTime;
    }

    /// @notice Gets user rewards
    function getUserRewards(address _walletAddress) external view returns (uint256) {
        return s_rewardPool.rewards[_walletAddress];
    }
    ...
    function getUserRewardPerTokenPaid(address _walletAddress) external view returns (uint256) {
        return s_rewardPool.userRewardPerTokenPaid[_walletAddress];
    }
