...
contract DropShopFactoryV4 {
    ...
    function createNFTContract(string memory name_, string memory symbol_, string memory uri_, string memory _contractURI, address minterRole, address owner_, uint256 maxTotalSupply_, uint256 maxTokenId_, address tokenERC20) external returns (address) {
       bytes memory selector = abi.encodeWithSelector(DropShopNFTERC1155(address(0)).initialize.selector, name_, symbol_, uri_, _contractURI, minterRole, owner_, maxTotalSupply_, maxTokenId_, tokenERC20 );
    ...
    ReentrancyGuardUpgradeable
{
    ...
    event NftSold(address tokens, uint256 nftId, uint256 nftAmount, uint256 nftPrice, address seller, address buyer);
 ...
 /**
    ...
    */
    ...
    function transferMarketSaleItem( address from, address to, uint256  nftId, uint256 nftAmount, uint256 nftPrice) nonReentrant onlyOwner external {
        
        uint256 amountTobuy = tokenERC20.balanceOf(to);
        require(amountTobuy > 0, "The wallet does not contain enough crypto");
        require(amountTobuy >= nftPrice, "The wallet does not contain enough crypto to buy nft");
        require(nftAmount > 0, "NFT amount is invalid");
        
        //transfer wETH amount from buyer to this contract
        tokenERC20.transferFrom(to, address(this), nftPrice);
        //transfer onwer nft item ERC1155 to buyer
        this.safeTransferFrom(from, to, nftId, nftAmount, '0x00');
        //transfer wETH amount from contract to Seller
        tokenERC20.transfer(from, nftPrice);
       
        emit NftSold(address(tokenERC20), nftId, nftAmount, nftPrice,  from, to);
    }
     ...
     function safeTransferFrom(
        ...
        require(
          this.isApprovedForAll(from, address(this)) ,
            "ERC1155: caller is not owner nor approved"
        );
        _safeTransferFrom(from, to, id, amount, data);
    }

    function approveSaleItem(address operator) external {
        this.setApprovalForAll(operator, true);
    }

    function allowance(address owner, address spender) external view returns (uint256)  {
        return tokenERC20.allowance(owner, spender);
    }

    function balanceOfERC20(address account) external view returns (uint256){
        return tokenERC20.balanceOf(account);
    }

     function setERC20(address _erc20) external onlyOwner{
        tokenERC20 = IERC20(_erc20); 
    }
...
import "../../proxy/interfaces/draft-IERC1822.sol";
