    ...
    IMarketplaceNonCustodial
{
    ...
    // nft addr => nft id => user addr => qty
    mapping(address => mapping(uint256 => mapping(address => uint256)))
        public listedTokenQty;
    ...
    function listItem(
        ...
        require(
            listedTokenQty[_nftAddress][_tokenId][msg.sender] + _quantity <=
                ownerHas,
            "MarketplaceNonCustodial: Can't list any more of these tokens!"
        );
        _createNewOrder(
            _nftAddress,
            _standard,
    ...
    function modifyListing(
        ...
        if (_newPricePerItem == 0) {
            if (_qtyToAdd != 0) {
                // condition to prevent double listing
                require(
                    listedTokenQty[_order.nftAddress][_order.tokenId][
                        msg.sender
                    ] +
                        _qtyToAdd <=
                        ownerHas,
    ...
    function cancelListing(uint256 _orderId) external override {
        ...
        _addBuyerToOrder(_orderId, msg.sender);
    ...
    function isOrderClosed(uint256 _orderId) public view returns (bool) {
        // return (listings[_orderId].buyer != address(0));
    ...
    function _createNewOrder(
        ...
        listings[orderCtr].buyers = _buyers;
        listedTokenQty[_nftAddress][_tokenId][address(_seller)] += _qty;
        emit ItemsListed(
            orderCtr,
            _nftAddress,
            _standard,
    ...
    function _increaseOrderQty(
        ...
        listedTokenQty[_nftAddress][_tokenId][_seller] += _qtyToAdd;
    }
    ...
    function _addBuyerToOrder(uint256 _orderId, address _buyer) internal {
        listings[_orderId].buyers.push(_buyer);
    }
