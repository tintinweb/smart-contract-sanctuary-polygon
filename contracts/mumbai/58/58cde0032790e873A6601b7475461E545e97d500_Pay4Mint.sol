...
/// @title Base NFT contract for dispatch mint demos
/// @author Dispatch.xyz
/// @dev for dispatch.xyz dev purposes
contract Pay4Mint is NFTMetadata {
  using SafeMath for uint256;

  // our erc20 contract
  ERC20Permit private ERC20WithPermit;
  // whitelisted contracts can burn + transfer
  ...
  //default uri
  // https://gateway.pinata.cloud/ipfs/QmVWdvtQEn54GLGaAvattLGcSxENXoQAxd2eRvb5NmJk9v
  constructor(
    address _Erc20TokenAddress,
    uint256 _totalSupply,
    string memory _tokenUri
  ) ERC1155("https://ipfs.io/ipfs/") {
    ERC20WithPermit = ERC20Permit(_Erc20TokenAddress);
    erc20TokenAddress = _Erc20TokenAddress;
    name = "Dispatch Mint Demo";
    symbol = "LFG";
  ...
  modifier _validAmount(uint256 _amount) {
    // checks that the user is holding the NFT
    require(
      _amount <= maxPerTransaction && totalSupply.sub(_amount) > 0,
      "Cannot buy more than the maxPerTransaction or exceed total supply."
    );
    _;
  }
  ...
  function setTokenUri(string memory _metadata) public _isWhitelistedAddress {
  ...
  function setEthPrice(uint256 _amount) public onlyOwner {
    ethPrice = _amount;
  }

  function setErc20Price(uint256 _amount) public onlyOwner {
    erc20Price = _amount;
  }

  function setErc20TokenAddress(address _address) public onlyOwner {
    ERC20WithPermit = ERC20Permit(_address);
    erc20TokenAddress = _address;
  }

  function mintWithEth(uint256 _amount) external payable _validAmount(_amount) {
    require(msg.value >= ethPrice.mul(_amount), "Must pay correct ETH price.");
  ...
  function mintWithErc20(uint256 _amount) public _validAmount(_amount) {
    require(
      ERC20WithPermit.allowance(msg.sender, address(this)) >= erc20Price.mul(_amount),
      "Must pay correct ERC20 price."
    );
    bool sent = ERC20WithPermit.transferFrom(msg.sender, owner(), erc20Price.mul(_amount));
    require(sent, "Failed to send ERC20 funds");
    totalSupply -= _amount;
  ...
  function mintWithErc20Permit(
    ...
    bytes32 _r,
    bytes32 _s
  ) external payable _validAmount(_amount) {
    ERC20WithPermit.permit(msg.sender, _spender, erc20Price.mul(_amount), _deadline, _v, _r, _s);
    mintWithErc20(_amount);
  }
