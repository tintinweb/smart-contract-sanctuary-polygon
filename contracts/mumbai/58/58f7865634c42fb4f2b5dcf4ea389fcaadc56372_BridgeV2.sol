...
contract BridgeV2 is Initializable, OwnableUpgradeable, ManageableUpgradeable {
    ...
    mapping(string => Req) public reqs;
    uint256 nBridge;
    ...
    function request(
        ...
        require(supportedChains[block.chainid], "BRIDGE: Invalid source chain");
        require(
        ...
        string memory id = string(
            abi.encodePacked(block.chainid, "-", nBridge)
        );
        reqs[id] = Req({
    ...
    function processBridge(
        ...
        require(reqs[id].srcChainID == 0, "PROC: Already processed.");
        ...
        uint256 nativeReceived = ROUTER.exactInputSingle(params);
        if (amountToSwap > 0) {
    ...
    function changeStatus(string memory id, STATUS status) public {
        reqs[id].status = status;
    }
