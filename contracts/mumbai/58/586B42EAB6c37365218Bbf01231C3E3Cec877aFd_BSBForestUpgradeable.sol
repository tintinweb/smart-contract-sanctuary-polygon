...
contract BSBForestUpgradeable is Initializable, BSBBaseGameModeUpgradeable, VRFConsumerBaseUpgradeable {
    ...
    function calculateBearDailyYield(address collection_, uint tokenId_) public view returns (uint) {
        bool applyBuff = BSBMintPassBuffCaller.getBuffBalance(_msgSender()) != uint(0);
        BearStats memory bearStats = BSBStorageCaller.getBearStats(collection_, tokenId_);
        uint collectionYield = collectionsYield[collection_];
        return _calculateDailyYieldWithBonus(collection_, collectionYield, tokenId_, applyBuff, bearStats.level.value);
    }
    ...
    function calculateDailyYieldForBears(address collection_, uint[] calldata tokenIds_) external view returns (uint[] memory) {
        ...
        for (uint i = 0; i < tokenIds_.length; i++) {
            dailyYields[i] = calculateBearDailyYield(collection_, tokenIds_[i]);
        }
        return dailyYields;
    }

    function calculateStaminaIncreaseRate() external view returns (uint) {
    ...
    function getHouseEnrolments(
        ...
        address tokenGeneratorsCollection = contracts["tokenGenerator"];

        uint[] memory gen0Enrolments = new uint[](
            EnumerableSetUpgradeable.length(houseEnrollments[tokenId_][gen0Collection]));
        uint[] memory gen1Enrolments = new uint[](
            EnumerableSetUpgradeable.length(houseEnrollments[tokenId_][gen1Collection]));
        uint[] memory tokenGeneratorsEnrolments = new uint[](
            EnumerableSetUpgradeable.length(houseEnrollments[tokenId_][tokenGeneratorsCollection]));

        for (uint i = 0; i < EnumerableSetUpgradeable.length(houseEnrollments[tokenId_][gen0Collection]); ++i){
            gen0Enrolments[i] = EnumerableSetUpgradeable.at(houseEnrollments[tokenId_][gen0Collection], i);
        }

        for (uint i = 0; i < EnumerableSetUpgradeable.length(houseEnrollments[tokenId_][gen1Collection]); ++i){
            gen1Enrolments[i] = EnumerableSetUpgradeable.at(houseEnrollments[tokenId_][gen1Collection], i);
        }

        for (uint i = 0; i < EnumerableSetUpgradeable.length(houseEnrollments[tokenId_][tokenGeneratorsCollection]); ++i){
            tokenGeneratorsEnrolments[i] = EnumerableSetUpgradeable.at(houseEnrollments[tokenId_][tokenGeneratorsCollection], i);
        }
    ...
    function getTreeHouseCapacities(uint[] calldata tokenIds_) external view returns (uint[] memory) {
        ...
        for (uint i = 0; i < tokenIds_.length; i++) {
            tokenGeneratorBonuses[i] = _getTreeHouseBonus(tokenIds_[i], applyBuff_);
        }
    ...
    function getTokenGeneratorCapacities(uint[] calldata tokenIds_) external view returns (uint[] memory) {
        ...
        for (uint i = 0; i < tokenIds_.length; i++) {
            tokenGeneratorBonuses[i] = _getTokenGeneratorCapacity(collection_, tokenIds_[i], applyBuff_);
        }
    ...
    function setLevelImpacts(uint[] memory milestones_, ImpactType[] calldata impacts_) external onlyOwner {
        require(milestones_.length == impacts_.length);
    ...
    function initSeedGeneration() public onlyOwner returns (bytes32 requestId) {
        require(LINK.balanceOf(address(this)) >= vrfFee);
    ...
    function _removeFromElitesAndUnstackIfNecessary(CollectionItems calldata collectionItem_) internal {
        ...
        for (uint i = 0; i < collectionItem_.assets.length; ++i) {
            ...
            if (collectionItem_.collection == contracts["treeHouse"]) {
                require(getTreeHouseOccupancy(collectionItem_.assets[i]) == 0);
    ...
    function _stackToken(address collection_, uint tokenId_, uint houseId_) internal {
        require(!tokenHouseAppartenance[collection_][tokenId_]);
    ...
    function _claimTokenGeneratorRewards(
        ...
        for (uint i = 0; i < collectionItems_.assets.length; ++i) {
            if (tokenHouseAppartenance[collectionItems_.collection][collectionItems_.assets[i]]) {
                ...
                if (yield > 0) {
                    emit BloodTokensClaimed(collectionItems_.collection, collectionItems_.assets[i], yield);
                }
    ...
    function _calculateStaminaIncreaseRate(bool applyBuff_) internal view returns(uint) {
    ...
    function _calculateBearAccumulatedStamina(
        ...
        uint accumulatedStamina_ =
            ...
            _calculateStaminaIncreaseRate(applyBuff_);
