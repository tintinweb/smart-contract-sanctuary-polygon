    ...
    ReentrancyGuard
{
    ...
    /**
     * @notice A modifier to check if a name is unreserved.
     * @dev This modifier checks if the proposed name does not start with a reserved word. It will fail if the first 8 characters of the name are a reserved word.
    ...
    modifier containsNoReservedWord(string calldata _name) {
        ...
        if (bytes(_name).length >= 4) {
            // Check if the first 4 characters of the name are not a reserved word
            if (_equal(reservedWord, _name[0:4]))
    ...
    constructor(address _self) ERC721("Self Identity NFT", "SIN") {
        ...
        lengthToPrice[8] = 500;
        lengthToPrice[7] = 1000;
        lengthToPrice[6] = 2000;
        lengthToPrice[5] = 4000;
        lengthToPrice[4] = 8000;
        lengthToPrice[3] = 16000;
        ...
        returns (bool)
    {
        ...
        uint _remaining = _price - _agentCommission;
     ...
     * @dev This function can only be called by the contract owner. It allows the registration of names with a length less than 3 or greater than 30. Additionally, it facilitates the registration of names that are blacklisted or those that start with a reserved word.
     * 
     * @param _to The address to which the name will be registered.
     * @param _name The name to register.
     * 
     * @return true if name registration is successful.
    ...
    * @dev This function is used to set or update the metadata associated with a specific tokenId. The metadata is expected to be an IPFS URL, which is a decentralized way to store and retrieve content.
    ...
    function setPrice(uint256 _length, uint256 _price) external onlyOwner {
        ...
        if (_length < 3 || _length > 8) revert NameLengthOutOfRangeError();
    ...
    function removeAgent(address _agent) external onlyOwner {
        if (_agent == address(0)) revert InvalidAddressError();
        if (!agents[_agent].isAgent) revert();
    ...
    function isNameAvailable(
        string memory _name
    ) external view isLegal(_name) returns (bool) {
    ...
    function getPrice(string memory _name) public view returns (uint256) {
        ...
        return lengthToPrice[effectiveNameLength] * 1 ether;
    }
...
contract SelfNftStorage {
    ...
    ///@dev A word that should not be allowed at the start of any name. Only the administrator can register names that start with the prohibited word.
    string constant reservedWord = "self";
