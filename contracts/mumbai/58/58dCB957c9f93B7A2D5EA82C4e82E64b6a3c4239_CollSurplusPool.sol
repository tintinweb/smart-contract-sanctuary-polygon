...
contract CollSurplusPool is Ownable, CheckContract, ICollSurplusPool {
    ...
    /* Returns the MATIC state variable at ActivePool address.
       Not necessarily equal to the raw ether balance - ether can be forcibly sent to contracts. */
    function getMATIC() external view override returns (uint) {
        return MATIC;
    }
    ...
    function claimColl(address _account) external override {
        ...
        MATIC -= claimableColl;
        emit MATICSent(_account, claimableColl);

        if (claimableColl>0){
            (bool success,) = payable(_account).call{ value: claimableColl }("");
            require ( success, "CollSurplusPool: sending MATIC failed");
        }
    }
    ...
    receive() external payable {
        MATIC += msg.value;
        emit CollSurplusMATICBalanceUpdated(MATIC);
    }
}