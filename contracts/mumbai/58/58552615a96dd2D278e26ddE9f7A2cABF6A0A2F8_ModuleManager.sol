...
import "../base/IModuleListener.sol";

import "./IModuleManager.sol";


contract ModuleManager is ModuleUpgradeable, IModuleManager {
    using AddressUpgradeable for address;
    using StringsUpgradeable for uint256;
    bytes32 public constant RETIRING_MANAGER_ROLE =
        keccak256("RETIRING_MANAGER_ROLE");
    ...
    modifier isRetiringManager() {
        require(
            hasRole(RETIRING_MANAGER_ROLE, msg.sender),
            "IModuleManager: the caller must have RETIRING_MANAGER_ROLE"
        );
        _;
    }
    ...
    function getId() override public view returns (bytes32) {
        return CoreConsts.MANAGER_MODULE_ID;
    }

    function getName() override public view returns (string memory) {
        return CoreConsts.MANAGER_MODULE_NAME;
    }

    function getVersion() override external view virtual returns (bytes32) {
        return keccak256(abi.encodePacked("v1.0")); // First release
    }

    function onSwitchManager(address oldManager)
        public
        override
        isRetiringManager
    {
        for (
            uint i = 0;
            i < IModuleManager(oldManager).getModulesCount();
            i++
        ) {
            bytes32 hname = IModuleManager(oldManager).getModuleIdAt(i);
            hnames.push(hname);
            modules[hname] = IModuleManager(oldManager).getModule(hname);
            ids[modules[hname]] = hname;
        }
        emit ManagerUpdatedFrom(oldManager);
    }

    function addModule(address module_addr) public override onlyOwner {
        bytes32 id = IModuleListener(module_addr).getId();
        _addModule(id, module_addr);
    }

    function addModule(string calldata name, address module_addr)
        public
	override
        onlyOwner
    {
        bytes32 h_name = keccak256(abi.encodePacked(name));
        _addModule(h_name, module_addr);
    }

    function _addModule(bytes32 h_name, address module_addr) internal {
        bool is_listener = _isListener(module_addr);
        if (is_listener) {
            require(
                IModuleListener(module_addr).getId() == h_name,
                "Module's ID does not match the indicated ID"
            );
            require(
                IModuleListener(module_addr).getId() ==
                    keccak256(
                        abi.encodePacked(
                            IModuleListener(module_addr).getName()
                        )
                    ),
                "Module's ID does not match the indicated ID"
            );
        ...
        bool is_new = false;
        if (modules[h_name] != address(0)) {
            if (is_listener)
                IModuleListener(module_addr).onUpdate(
                    modules[h_name],
                    address(this)
                );
            if (_isListener(modules[h_name]))
                IModuleListener(modules[h_name]).onReplaced(
                    module_addr
                );
            if (address(authManager) != address(0))
                RoleManager(address(authManager)).transferRoles(
                    h_name,
                    modules[h_name],
                    module_addr
                );
            emit Man_ModuleReplaced(h_name, module_addr);
        } else {
            is_new = true;
            if (is_listener)
                IModuleListener(module_addr).onAdd(address(this));
            hnames.push(h_name);
            emit Man_ModuleAdded(h_name, module_addr);
        }
        for (uint i = 0; i < hnames.length; i++) {
            if (modules[hnames[i]] == address(0)) continue;
            if (_isListener(modules[hnames[i]]))
                IModuleListener(modules[hnames[i]]).onListenAdded(
                    h_name,
                    module_addr,
                    is_new
                );
            if (is_listener) {
                IModuleListener(module_addr).onListenAdded(
                    hnames[i],
                    modules[hnames[i]],
                    true
                );
                if (!is_new)
                    IModuleListener(modules[h_name])
                        .onListenRemoved(hnames[i]);
            }
        }
        if (h_name == CoreConsts.ROLE_MANAGER_MODULE_ID) {
            _linkRoleManager(module_addr);
            //	    console.log("AUTH MANAGER SET TO %s",uint256(uint160(module_addr)).toHexString());
        }

        modules[h_name] = module_addr;
        ids[module_addr] = h_name;
    }

    function removeModule(string calldata name) public override onlyOwner {
        bytes32 h_name = keccak256(abi.encodePacked(name));
        if (modules[h_name] == address(0)) return;
        bool is_listener = _isListener(modules[h_name]);
        if (_isListener(modules[h_name])) {
            IModuleListener(modules[h_name]).onRemoved();
        }
        for (uint i = 0; i < hnames.length; i++) {
            if (modules[hnames[i]] == address(0)) break;
            if (!_isListener(modules[hnames[i]])) break;
            IModuleListener(modules[hnames[i]]).onListenRemoved(
                h_name
            );
            IModuleListener(modules[h_name]).onListenRemoved(
                hnames[i]
            );
        }
        if (h_name == CoreConsts.ROLE_MANAGER_MODULE_ID) {
        ...
        delete ids[modules[h_name]];
        delete modules[h_name];
    }

    function isListener(address instance) public override view returns (bool) {
    ...
    function _isListener(address instance) internal view returns (bool) {
        if (instance.isContract()) {
            try
                IERC165(instance).supportsInterface(
                    type(IModuleListener).interfaceId
                )
            returns (bool responce) {
                return responce;
            } catch {
                return false;
            }
    ...
    function updateOwnership(address newOwner) override public virtual onlyOwner {
        for (uint i = 0; i < hnames.length; i++) {
            IModuleListener(modules[hnames[i]]).updateOwnership(
                newOwner
            );
        }
//        transferOwnership(newOwner);
	super._updateOwnership(newOwner);
    }

    function upgradeManager(address newManager) public override onlyOwner {
        for (uint i = 0; i < hnames.length; i++) {
            IModuleListener(modules[hnames[i]]).switchManager(
                newManager
            );
        }
        emit ManagerUpdatedTo(newManager);
        ModuleManager(newManager).onSwitchManager(address(this));
        ModuleManager(newManager).renounceRole(
            RETIRING_MANAGER_ROLE,
            address(this)
        );
    }

    function getModule(string calldata name) public override view returns (address) {
        bytes32 h_name = keccak256(abi.encodePacked(name));
        return modules[h_name];
    }

    function getModule(bytes32 _id) public override view returns (address) {
        return modules[_id];
    }

    function getModuleId(address addr) public override view returns (bytes32) {
        return ids[addr];
    }

    function getModuleIdAt(uint256 idx) public override view returns (bytes32) {
	return hnames[idx];
    }

    function getModulesCount() public override view returns (uint256) {
        return hnames.length;
    }

    function getManagerRoleCode() public override view returns (bytes32){
