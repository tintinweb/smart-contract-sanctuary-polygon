...
contract LockedStaking is
    ...
    uint256 public yearlyInterestRateBP;
        ...
        onlyOwner
    {
        ...
        yearlyInterestRateBP = _yearlyInterestRateBP;
    ...
    function openDeposit(
        ...
        LockedDeposit _deposit = new LockedDeposit(
            owner(),
            address(token),
            msg.sender,
            _amount,
            yearlyInterestRateBP,
            lockedDuration,
            harvestInterval
        );
    ...
    function canCloseDeposit(address _depositContractAddress) public view returns(bool _canClose) {
        ...
        LockedDeposit deposit = LockedDeposit(_depositContractAddress);
...
contract LockedDeposit is ReentrancyGuard {
    using SafeMath for uint256;
    ...
    ILockedStaking public stakingContract;
    ...
    constructor(
        ...
        stakingContract = ILockedStaking(msg.sender);
        ...
        addInterestAllocation((_amount.mul(_yearlyInterestRateBP).div(BASIS_POINTS_10000).div(4)), depositDate.add(_harvestInterval.mul(12)));
        addInterestAllocation((_amount.mul(_yearlyInterestRateBP).div(BASIS_POINTS_10000).div(4)), depositDate.add(_harvestInterval.mul(13)));
        addInterestAllocation((_amount.mul(_yearlyInterestRateBP).div(BASIS_POINTS_10000).div(4)), depositDate.add(_harvestInterval.mul(14)));
        addInterestAllocation((_amount.mul(_yearlyInterestRateBP).div(BASIS_POINTS_10000).div(4)), depositDate.add(_harvestInterval.mul(15)));
    }
