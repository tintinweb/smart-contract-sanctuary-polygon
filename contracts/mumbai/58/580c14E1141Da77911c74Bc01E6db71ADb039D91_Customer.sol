...
contract Customer is ERC721Holder {
    using Counters for Counters.Counter;
    Counters.Counter private _customerIds;
    Counters.Counter private _customerBankDetailsIds;
    address public owner;

    ITablelandTables private _tableland;

    string private _baseURIString;
    string private _customerTable;
    uint256 private _customerTableId;
    string private constant CUSTOMER_TABLE_PREFIX = "_customerTable";
    string private constant CUSTOMER_TABLE_SCHEMA =
        "custID text, address text, cust_name text, cust_dob text, cust_gender text, cust_address text, cust_contact text, cust_email text";

    string private _customerBankDetailsTable;
    uint256 private _customerBankDetailsTableId;
    string private constant CUSTOMER_BANK_DETAILS_TABLE_PREFIX = "_customerBankDetailsTable";
    string private constant CUSTOMER_BANK_DETAILS_TABLE_SCHEMA =
        "custBankDetailID text, address text, cust_holder_name text, cust_account_number text, cust_branch text, cust_bank text";
    ...
    constructor(address registry) {
        ...
        _customerTableId = _tableland.createTable(
            address(this),
            SQLHelpers.toCreateFromSchema(CUSTOMER_TABLE_SCHEMA, CUSTOMER_TABLE_PREFIX)
        );
        _customerTable = SQLHelpers.toNameFromId(CUSTOMER_TABLE_PREFIX, _customerTableId);

        _customerBankDetailsTableId = _tableland.createTable(
            address(this),
            SQLHelpers.toCreateFromSchema(
                CUSTOMER_BANK_DETAILS_TABLE_SCHEMA,
                CUSTOMER_BANK_DETAILS_TABLE_PREFIX
            )
        );
        _customerBankDetailsTable = SQLHelpers.toNameFromId(
            CUSTOMER_BANK_DETAILS_TABLE_PREFIX,
            _customerBankDetailsTableId
        );
    }
    ...
    function createCustomerPersonalDetails(
        string memory custName,
        string memory custDob,
        string memory custGender,
        string memory custAddress,
        string memory custContactNumber,
        string memory custEmail
    ) public {
        uint256 customerId = _customerIds.current();
        string memory insert_statement = SQLHelpers.toInsert(
            CUSTOMER_TABLE_PREFIX,
            _customerTableId,
            "custID, address, cust_name, cust_dob, cust_gender, cust_address, cust_contact, cust_email",
            string.concat(
                SQLHelpers.quote(Strings.toString(customerId)),
                ",",
                SQLHelpers.quote(Strings.toHexString(msg.sender)),
                ",",
                SQLHelpers.quote(custName),
                ",",
                SQLHelpers.quote(custDob),
                ",",
                SQLHelpers.quote(custGender),
                ",",
                SQLHelpers.quote(custAddress),
                ",",
                SQLHelpers.quote(custContactNumber),
                ",",
                SQLHelpers.quote(custEmail)
            )
        );
        _tableland.runSQL(address(this), _customerTableId, insert_statement);
        _customerIds.increment();
    }

    function createCustomerBankDetails(
        string memory custHolderName,
        string memory custAccountNumber,
        string memory custBranch,
        string memory custBank
    ) public {
        uint256 customerBankDetailId = _customerBankDetailsIds.current();
        string memory insert_statement = SQLHelpers.toInsert(
            CUSTOMER_BANK_DETAILS_TABLE_PREFIX,
            _customerBankDetailsTableId,
            "custBankDetailID, address, cust_holder_name, cust_account_number, cust_branch, cust_bank",
            string.concat(
                SQLHelpers.quote(Strings.toString(customerBankDetailId)),
                ",",
                SQLHelpers.quote(Strings.toHexString(msg.sender)),
                ",",
                SQLHelpers.quote(custHolderName),
                ",",
                SQLHelpers.quote(custAccountNumber),
                ",",
                SQLHelpers.quote(custBranch),
                ",",
                SQLHelpers.quote(custBank)
            )
        );
        _tableland.runSQL(address(this), _customerBankDetailsTableId, insert_statement);
        _customerBankDetailsIds.increment();
    }
}
