...
contract Agent {
    ...
    function createFile(uint fileNumber, address customer) public pure returns (File memory) {
        return File(fileNumber, customer);
    }

    function createCovenant(File memory file, uint expiration) public pure returns (Covenant memory) {
        return Covenant(file, expiration);
    }
    ...
    function isCovenantAdministrativelyEnded() internal view returns (bool) {
