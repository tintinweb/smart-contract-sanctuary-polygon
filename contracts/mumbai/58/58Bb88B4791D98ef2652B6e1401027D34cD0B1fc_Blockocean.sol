...
contract Blockocean {
    ...
    uint256[7] private slots = [50e6, 100e6, 250e6, 500e6, 1000e6, 2500e6, 5000e6];
    uint256[7] private AP4x2Layer1 = [5e6, 10e6, 25e6, 50e6, 100e6, 200e6, 400e6];
    uint256[7] private AP4x2Layer2 = [20e6, 40e6, 100e6, 200e6, 400e6, 800e6, 1600e6];
    uint256[7] private AP3x3Layer1 = [20e6, 40e6, 100e6, 200e6, 400e6, 800e6, 1600e6];
    uint256[7] private AP3x3Layer2 = [20e6, 40e6, 100e6, 200e6, 400e6, 800e6, 1600e6];
    uint256[7] private AP3x3Layer3 = [60e6, 120e6, 300e6, 600e6, 1200e6, 2400e6, 4800e6];
    ...
    function _buySlot(address _user, uint256 _slot) private {
        ...
        if(userInfo[user.referrer].directTeam == GPDirectRequired && _slot == 1 && !rewardInfo[user.referrer].APActive4x2[_slot - 1]) _updateAP4x2(user.referrer, _slot);
        ...
        if(user.mainUpline != address(0) && userInfo[user.mainUpline].slots >= user.slots) {
            ...
            } else if(rewardInfo[user.mainUpline].serial[_slot - 1] == 1 && user.mainUpline != defaultRefer) {
                curPercent = refPercent;
            } else if(rewardInfo[user.mainUpline].serial[_slot - 1] > 1 && _slot < 7 && rewardInfo[user.mainUpline].selfUpgrade < slots[_slot] && user.slots == userInfo[user.mainUpline].slots) {
                rewardInfo[user.mainUpline].selfUpgrade += (slots[_slot - 1] * otherPercent)/100;
                curPercent = refPercent;
                if(rewardInfo[user.mainUpline].selfUpgrade >= slots[_slot] && _slot < 7) {
    ...
    function _updateAP4x2(address _user, uint256 _slot) private {
        ...
        if(globalpool4x2[_slot].length > 1) {
            ...
            if(globalpool4x2[_slot].length > 5) {
                ...
                if(APIndex4x2 == end && userInfo[_curUpline].APTeam4x2[_slot - 1] != 4) {
                    APIndex4x2 = start;
                } else {
                    APIndex4x2 += 1;
                }
            ...
            Left += (slots[_slot - 1] * otherPercent)/100;
        }
    }
    ...
    function _updateAP3x3(address _user, uint256 _slot) private { 
        ...
        if(globalpool3x3[_slot].length > 1) {
            ...
            if(globalpool3x3[_slot].length > 4) {
                ...
                if(APIndex3x3 == end && userInfo[_curUpline].APTeam3x3[_slot - 1] != 3) {
                    APIndex3x3 = start;
                } else {
                    APIndex3x3 += 1;
                }
    ...
    function withdraw() external {
        ...
        uint256 total = reward.refIncome + reward.rankIncome;
        reward.incomeTaken[0] += reward.refIncome;
        reward.incomeTaken[1] += reward.rankIncome;
        reward.refIncome = 0;
        reward.rankIncome = 0;
    ...
    function getGPActive(address _user) external view returns(bool[7] memory, bool[7] memory) {
    ...
    function getIncomeTaken(address _user) external view returns(uint256[2] memory) {
