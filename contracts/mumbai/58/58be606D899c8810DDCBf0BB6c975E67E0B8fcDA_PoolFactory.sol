...
interface IPoolManager {
    ...
    function privatePoolForToken(string memory tokenName) external view returns (address);

    event TvlChanged(uint256 totalLocked, uint256 totalRaised);
    event ContributionUpdated(uint256 totalParticipations);
    event PoolForTokenRemoved(address indexed token, address pool);
    event PrivatePoolForTokenRemoved(string indexed tokenName, address pool);
...
contract PoolFactory is OwnableUpgradeable, Utility {
    ...
    event PrivatePoolCreated(address indexed pool_address, LibPresale.PrivateSale presale);
    event FairPoolCreated(address indexed pool_address, LibPresale.FairLaunch presale);
    event KYCUpdated(address indexed pool_address, bool status, string link);
    event AuditUpdated(address indexed pool_address, bool status, string link);
    event Cancelled(address indexed pool_address);
    event ContributionWithdrawn(address indexed pool_address);
    event LiquidityWithdrawn(address indexed pool_address);
    event EmergencyLiquidityWithdrawn(address indexed pool_address);
    event EmergencyWithdrawn(address indexed pool_address);
    event PoolUpdated(address indexed pool_address);
    event UserWhitelisted(address indexed pool_address);
    event UserDelisted(address indexed pool_address);
    event PoolFinalised(address indexed pool_address);
    ...
    event PresaleEnabled(bool _old, bool _new);
    ...
    function setIsEnabled(bool _enable) public onlyOwner {
        IsEnabled = _enable;
        emit PresaleEnabled(IsEnabled, _enable);
    }
    function setMasterAddress(address _address) public onlyOwner validAddress(_address) {
        master = _address;
    }

    function setAdminWallet(address payable _address) public onlyOwner validAddress(_address) {
    ...
    function setFeeIndex(uint8 _index) public onlyOwner {
        require(_index >= 0, "Invalid Fee Index");
        feeIndex = _index;
    }


    function setFees(Fee memory _fee, uint256 index) public onlyOwner {
        require(
            _fee.tokenFee >= 0 &&
            _fee.tokenFee <= 100 &&
            _fee.nativeFee >= 0 &&
    ...
    function getFee(uint256 index) public view returns (Fee memory) {
        return fees[index];
    }
    ...
    function createSale(
        ...
        require(paymentCurrencies[presale.paymentToken] == true || address(0) == presale.paymentToken, "Invalid payment token");
        //fees transfer to Admin wallet
        (bool success, ) = adminWallet.call{ value: msg.value }("");
        require(success, "Address: unable to send value, recipient may have reverted");

        bytes32 salt = keccak256(
            abi.encodePacked(presale.poolDetails, block.timestamp)
        );
        address pair = Clones.cloneDeterministic(master, salt);
        require(presale.feeIndex <= feeIndex, "Invalid Fee Index");
        Fee memory fee = fees[presale.feeIndex];
        //Clone Contract
        initializeClone(
            pair,
            [fee.nativeFee, fee.tokenFee],
            presale
        );
        checkFees();
        uint8 decimals = 18;
        if(presale.paymentToken != address(0)) {
            decimals = IERC20Info(presale.paymentToken).decimals();
        }

        IPoolManager(poolManager).addPoolFactory(pair);
        IPoolManager(poolManager).registerPool(
            pair, presale.token, presale.governance, version
        );
    ...
    function setPresalePoolPrice(uint256 _price) public onlyOwner validAmount(_price) {
        emit PoolFeesUpdated("master", _price, masterPrice);
        masterPrice = _price;
    }
