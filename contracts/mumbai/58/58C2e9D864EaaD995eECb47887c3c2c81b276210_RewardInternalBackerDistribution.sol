...
contract RewardInternalBackerDistribution is ERC1155Holder, ERC721Holder, ReentrancyGuard, AccessController {
    ...
    /** Event */
    event DepositReward(address indexed campaignOwner, string campaignId, uint256 timestamp);
    ...
    function depositReward(
        ...
        emit DepositReward(_msgSender(), _rewardOwner.campaignId, block.timestamp);
    }
    ...
    function claimReward(
        ...
        require(_validateSignatureClaim(_signature, _campaignId, _orderId, uint256(_reward), _amount, _msgSender(), _adminVerifier), "Invalid Input");
        rewardStorage.setUserClaim(_orderId, _campaignId, _msgSender(), _reward, _amount);
        if(existReward.rewardType == RewardStorage.RewardType.Token){
            TransferHelper.safeTransferFrom(existReward.contractAddress, address(this), _msgSender(), existReward.amountPerWinner * _amount);
        ...
        emit ClaimedReward(_msgSender(), _campaignId, uint256(_reward), _amount, uint256(existReward.rewardType), block.timestamp);
    }

    function withdrawRewardFT(string calldata _campaignId, RewardStorage.Reward _reward) external nonReentrant {
        (RewardStorage.RewardOwner memory existReward, uint256 _totalUserClaimInCampaign) = rewardStorage.getRewardInfo(_campaignId, _reward);
        ...
        rewardStorage.setTotalUserClaimInCampaign(_campaignId, _reward, existReward.totalWinner); // assure all reward is claimed
        TransferHelper.safeTransferFrom(existReward.contractAddress, address(this), _msgSender(), remainReward);
        emit OwnerWithdrawReward(_msgSender(), _campaignId, uint256(_reward), uint256(existReward.rewardType), remainReward, block.timestamp);
    }
    ...
    function _validateSignatureClaim(
        ...
        uint256 _amount,
        address _address,
        address _adminVerifier
    ) private pure returns (bool) {
        bytes32 hashValue = keccak256(
            abi.encodePacked(_orderId, _campaignId, _address, _reward, _amount)
        );
    ...
    function _validateSignatureVerifyReward(
        ...
        bytes32 verifyReward = keccak256(
            abi.encodePacked(_orderId, _rewardOwner.amountPerWinner, _rewardOwner.totalWinner, uint256(_rewardOwner.rewardType), _rewardOwner.claimExpiredTime, _rewardOwner.contractAddress)
        );
