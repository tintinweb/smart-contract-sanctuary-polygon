...
contract BSG {
    ...
    uint256 private constant minDeposit = 100 ether;
    ...
    function withdraw() external {
        ...
        withdrawable = withdrawable.add(user.levelIncome).add(userRewards.capitals);
    ...
    function getCurROI(address _user) public view returns(uint256){
        ...
        return rewardInfo[_user].capitals.add(staticReward);
    }

    function getCurOtherRewards(address _user) public view returns(uint256){
    ...
    function _calCurDynamicRewards(address _user) private view returns(uint256, uint256) {
        ...
        uint256 totalRewards = userInfo[upline].levelIncome.add(userRewards.level4Released).add(userRewards.level5Released);
    ...
    function _updateReferInfo(address _user, uint256 _amount) private {
        ...
        for(uint256 i = 0; i < referDepth; i++){
            if(upline != address(0)){
                    uint256 reward = _amount.mul(directPercents).div(baseDivider);
                    userInfo[upline].levelIncome = userInfo[upline].levelIncome.add(reward);
