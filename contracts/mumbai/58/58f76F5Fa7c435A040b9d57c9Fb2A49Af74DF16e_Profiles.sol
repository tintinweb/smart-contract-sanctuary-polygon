    ...
    ERC721EnumerableUpgradeable
{
    ...
    function setTwitterLink(string memory _newTwitterLink) external {
        DataTypes.ProfileStruct memory _profile = getProfileByAddress(
            msg.sender
        );

        if (bytes(_newTwitterLink).length > Constants.MAX_DESCRIPTION_LENGTH)
            revert Errors.MaxLength(Constants.MAX_DESCRIPTION_LENGTH);

        _profileById[_profile.tokenId].twitterLink = _newTwitterLink;
    }
