...
contract AllelePlantCore is PlantAuction {
        ...
        )
    {
        ...
        return (isGestating, isReady, cooldownIndex,nextActionAt , siringWithId, birthTime, parentId, generation);
    }
    ...
    function withdrawBalance() external onlyOwner {
        ...
        uint256 subtractFees = (pregnantPlants + 1) * autoBirthFee;
...
contract PlantBreeding is UIAMToken {
    ...
    uint256 public autoBirthFee = 200000000;

    uint256 public pregnantPlants;
    ...
    function _triggerCooldown(Plant storage _plant) internal {
        ...
        _plant.cooldownEndBlock = uint64(
            (cooldowns[_plant.cooldownIndex] / secondsPerBlock) + block.number
        );

        // Increment the breeding count, clamping it at 13, which is the length of the
        // cooldowns array. We could check the array size dynamically, but hard-coding
        // this as a constant saves gas. Yay, Solidity!
        if (_plant.cooldownIndex < 13) {
            _plant.cooldownIndex += 1;
        }
    }
    ...
    function giveBirth(
        ...
        uint256 plantId = _createPlant(sireId, _genes, owner, _tokenURI);
...
contract UIAMToken is AllelePlantBase {
    function mintUIAM(
        ...
        uint256 tokenId = _createPlant(0, _genes, msg.sender, _tokenURI);
        // add location
    ...
    function createGen2(
        ...
        require(
            ...
            "You are not the owner of the parent Plant"
        );
        require(total > 0, "Total number of children must be greater than 0");
        for (uint256 i = 0; i < total; i++) {
            _createPlant(_parentId, _genes, msg.sender, _tokenURI);
        }
    }
}
...
contract AllelePlantBase is ERC721URIStorage, Ownable, Pausable, Whitelist {
    ...
    constructor() ERC721("AllelePlant", "AP") {}
    ...
    function _createPlant(
        ...
        uint256 parentGeneration = getGeneration(_parentId);
        Plant({
            parentId: _parentId,
...
interface IGeoLocation {
    ...
    function getLocation(uint256 _tokenId) external view returns(Location memory);
    function setLocation(uint256 _tokenId, uint256 lat, uint256 long, string memory metadataUrl) external;
