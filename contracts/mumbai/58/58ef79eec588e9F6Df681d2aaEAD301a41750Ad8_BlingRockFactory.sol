...
import "./BlingRockCollection.sol";

contract BlingRockFactory {
    BlingRockCollection[] public collectionTokens;
    ...
    function deployCollection(
        ...
        BlingRockCollection t = new BlingRockCollection(
            _collectionName,
            _collectionUri
        );
        ...
        indexToOwner[index] = tx.origin;
        emit CollectionCreated(msg.sender, address(t), index);
        return address(t);
    }
    ...
    function getIdByName(uint256 _index, string memory _name)
        ...
        returns (uint256)
    {
        return collectionTokens[_index].nameToId(_name);
    }
    ...
    {
        return collectionTokens[_index].idToName(_id);
    }
    ...
    {
        BlingRockCollection token = collectionTokens[_index];
        return (
            address(token),
            token.owner(),
            token.uri(_id),
...
contract BlingRockCollection is ERC1155, Ownable {
    string[] public collectibleNames; //string array of names
    uint256[] public collectibleIds; //uint array of ids
    string public collectionMetadataURI; //the token metadata URI
    ...
    */
    constructor(string memory _collectionName, string memory _collectionUri)
        ERC1155(_collectionUri)
    {
        createMapping();
        setURI(_collectionUri);
        collectionMetadataURI = _collectionUri;
    ...
    */
    function createMapping() private {
        for (uint256 id = 0; id < collectibleIds.length; id++) {
            nameToId[collectibleNames[id]] = collectibleIds[id];
            idToName[collectibleIds[id]] = collectibleNames[id];
        }
    }
    ...
    */
    function mint(
        ...
        nameToId[name] = collectibleIds.length;
        idToName[collectibleIds.length] = name;
        collectionMetadataURI = _collectionUri;
        _mint(account, collectibleIds.length, amount, "");
