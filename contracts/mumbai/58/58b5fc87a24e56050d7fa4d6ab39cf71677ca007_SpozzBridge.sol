...
contract SpozzBridge is Ownable, Pausable, ReentrancyGuard {
    using SafeMath for uint;
    using SafeERC20 for IERC20;

    uint public constant MIN_AMOUNT = 1e9;
    uint public maxAmount = 5000000 * 1e9;
    uint public swapCommission;
    uint public maxSwapEndInPhase = 5000000 * 1e9;
    uint public currentPhase = 1;
    uint public totalSwapEnd;
    address public rootToken;
    address public commissionReceiver; // the address that will get new token.
    uint public feeMoveThreshold = 50 * 1e9;
    uint public feeAccAmount;
    ...
    mapping (address => bytes32) public userEventMap;
    ...
    function setCommission(uint _swapCommission, address _commissionReceiver) external onlyOwner{
    ...
    //BVG08 fixed // from address === to address
    function swapStart(
        uint toChainID,
        address to,
        uint amount
        ...
        _depositToken(amount); // transfer from.
        // uint inputAmount = amount;
        // uint commission;
        // if(swapCommission > 0 && msg.sender != commissionReceiver){
        //     commission = _commissionCalculate(amount);
        //     amount = amount.sub(commission);
        //     feeAccAmount = feeAccAmount.add(commission);
        //     if (feeAccAmount >= feeMoveThreshold) {
        //         _withdrawCommission(feeAccAmount);
        //         feeAccAmount = 0;
        //     }
        // }
        //BVG02 fixed
        ...
        require(eventStore[eventHash].depositCount == 0,
            "It's available just 1 swap with same: chainID, depositCount, from, to, inputAmount");
        eventStore[eventHash] = eventStr;
        userEventMap[msg.sender] = eventHash;
    ...
    function setFeeMoveThreshold(uint _threshold) external onlyOwner{
        require(_threshold > 0, "Threshold amount must be greater than 0");
        feeMoveThreshold = _threshold;
    }
    ...
    function _withdrawCommission(uint feeAmount) internal{
        if(feeAmount > 0 && commissionReceiver != address (0)){
            _transferToken(commissionReceiver, feeAmount);
        }
    }
