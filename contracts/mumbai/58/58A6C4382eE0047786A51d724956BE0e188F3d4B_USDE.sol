...
contract USDE is IERC20, IUSDE, IOWNER {
        ...
        returns (bool)
    {
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            if (_isWZero(trans.to)) {
                ...
                _balances[from_].sub(trans.value);
        ...
        require(_balanceOf(from_) >= 0, "Not balance");
        return true;
    }
    ...
    function transferFrom(
        ...
        uint256 fee_tx = _fee_tx(usdeWei);
        string memory note_ = "USDE: transfer amount exceeds allowance";
        if (fee_tx > 0) {
            _burn(from, fee_tx);
        }
        ...
        _approve(
            from,
            _sender(),
            _allowed[from][_sender()].sub(usdeWei, note_)
        );
        return true;
    }
    ...
    function _decStake(address staker_, uint256 usdeWei) internal virtual {
        _stakingOf[staker_].sub(usdeWei);
        _balances[staker_] += usdeWei;

        _totalStaking.sub(usdeWei);
        _totalExchange += usdeWei;
        _emitStake(staker_, usdeWei);
    }
    ...
    function _decLock(address locker_, uint256 usdeWei) internal virtual {
        _locksOf[locker_].sub(usdeWei);
        _mint(locker_, usdeWei);
        _totalLocking.sub(usdeWei);
