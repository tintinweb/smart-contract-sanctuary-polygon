...
contract AlchemistBank is Bank {
    constructor(address wethAddress_, address timelockController) Bank(wethAddress_, timelockController) {}
...
abstract contract Bank is AccessControl, ReentrancyGuard, OutgoingTransferSupport, IBank {
    ...
    constructor(address wethAddress_, address timelockController) OutgoingTransferSupport(wethAddress_) {
        _setRoleAdmin(BANK_ADMIN, BANK_ADMIN);
        require(timelockController != address(0), "Timelock controller address is invalid");
        _setRoleAdmin(SETTLEMENT_ROLE, BANK_ADMIN);
        _setupRole(BANK_ADMIN, timelockController);
    }
