...
contract Bridge is Trustable {
    ...
    function convertAmount(
        ...
        return (amount * (10**ERC20(token).decimals())) / (10**decimals);
    }
    ...
    function create(
        ...
        if (feeAmount > 0) {
            ERC20(tok.token).safeTransferFrom(msg.sender, tok.feeTarget, feeAmount);
        }

        amount = amount - feeAmount;
        ERC20(tok.token).safeTransferFrom(msg.sender, address(this), amount);
    ...
    function completeOrder(
        ...
        ERC20(tok.token).safeTransfer(to, convertAmount(tok.token, amount, decimals));
    ...
    function listTokensNames() external view returns (string[] memory) {
        ...
        for (uint16 i = 0; i < tokensLength; i++) {
            if (address(tokens[i].token) != address(0)) {
                list[i] = ERC20(tokens[i].token).symbol();
            }
        }
