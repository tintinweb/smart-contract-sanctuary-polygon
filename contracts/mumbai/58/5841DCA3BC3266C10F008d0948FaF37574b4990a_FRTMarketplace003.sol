...
contract FRTMarketplace003 is AccessControl{
    ...
    event ListNFT(uint256 indexed tokenId);
    event UnlistNFT(uint256 indexed tokenId);
    event NFTApproved(uint256 indexed tokenId);
    event NFTRejected(uint256 indexed tokenId);
    event BuyNFT(uint256 indexed tokenId, address indexed buyer);
    event VoteNFT(uint256 indexed tokenId, address indexed voter, string vote);
    event MintNFT(uint256 indexed tokenId, address indexed publisher, string indexed tokenHash);
    event DownloadNFTForVoting(string indexed tokenHash, address indexed tester);
