...
contract PerpetualPoolFactory is
    ...
    IPerpetualPoolFactory
{
     ...
     * @param   _iTargetPoolSizeUpdateTime   Time in seconds.
     * @return Pool id
     */
    function createLiquidityPool(
    ...
    ) external override onlyOwner returns (uint8) {
        _validateTreasuryAddress(_treasuryAddress);
        require(
            _marginTokenAddress != address(this) && _marginTokenAddress != address(0),
            "invalid marginTokenAddress"
        );
        _validateTargetPoolSizeUpdateTime(_iTargetPoolSizeUpdateTime);
        _validateFundTransferConvergenceHours(_iFundTransferConvergenceHours);
        ...
        pool.iFundTransferConvergenceHours = _iFundTransferConvergenceHours;
        pool.fMaxTransferPerConvergencePeriod = _fMaxTransferPerConvergencePeriod;
        pool.fBrokerCollateralLotSize = _fBrokerCollateralLotSize;
        ...
        emit LiquidityPoolCreated(
            ...
            _iFundTransferConvergenceHours,
            _fMaxTransferPerConvergencePeriod,
            _fBrokerCollateralLotSize
        );
     ...
     */
    function runLiquidityPool(uint8 _liqPoolID) external override onlyOwner {
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            if (perpetual.state == PerpetualState.INITIALIZING) {
                ...
                _getUpdateLogic().setNormalState(perpetual.id);
            }
        }
    ...
    function setTreasury(uint8 _poolId, address _treasury) external override onlyOwner {
    ...
    function setOracleFactory(address _oracleFactory) external override onlyOwner {
        oracleFactoryAddress = _oracleFactory;
    }
        ...
        onlyOwner
    {
        ...
        LiquidityPoolData storage pool = liquidityPools[_poolId];
        pool.iFundTransferConvergenceHours = _iFundTransferConvergenceHours;
        emit SetFundTransferConvergenceHours(_poolId, _iFundTransferConvergenceHours);
    }
    ...
    function setMaxTransferPerConvergencePeriod(
        ...
        require(_poolId <= iPoolCount, "pool index out of range");
        require(
            _fMaxTransferPerConvergencePeriod > 0,
            "invalid _fMaxTransferPerConvergencePeriod"
        );
        LiquidityPoolData storage pool = liquidityPools[_poolId];
        pool.fMaxTransferPerConvergencePeriod = _fMaxTransferPerConvergencePeriod;
        emit SetMaxTransferPerConvergencePeriod(_poolId, _fMaxTransferPerConvergencePeriod);
    }

    function setBrokerCollateralLotSize(uint8 _poolId, int128 _fBrokerCollateralLotSize)
        external
        override
        onlyOwner
    {
        require(_poolId <= iPoolCount, "pool index out of range");
        require(_fBrokerCollateralLotSize > 0, "invalid _fBrokerCollateralLotSize");
        LiquidityPoolData storage pool = liquidityPools[_poolId];
        pool.fBrokerCollateralLotSize = _fBrokerCollateralLotSize;
        emit SetBrokerCollateralLotSize(_poolId, _fBrokerCollateralLotSize);
    }

    function _validateTreasuryAddress(address _treasuryAddress) internal view {
        require(
            _treasuryAddress != address(this) && _treasuryAddress != address(0),
            "invalid treasuryAddress"
        );
    }

    function _validateTargetPoolSizeUpdateTime(uint16 _iTargetPoolSizeUpdateTime) internal pure {
        require(
            _iTargetPoolSizeUpdateTime >= 30 && _iTargetPoolSizeUpdateTime <= 65000,
            "invalid iTargetPoolSizeUpdateTime"
        );
    }

    function _validateFundTransferConvergenceHours(uint16 _iFundTransferConvergenceHours)
        internal
        pure
    {
        require(
            _iFundTransferConvergenceHours >= 24 && _iFundTransferConvergenceHours <= 1000,
            "invalid iFundTransferConvergenceHours"
        );
    }
    ...
    function getFunctionList() external pure virtual override returns (bytes4[] memory, bytes32) {
        ...
        functionList[6] = this.setOracleFactory.selector;
        functionList[7] = this.setFundTransferConvergenceHours.selector;
        functionList[8] = this.setMaxTransferPerConvergencePeriod.selector;
        functionList[9] = this.setBrokerCollateralLotSize.selector;
