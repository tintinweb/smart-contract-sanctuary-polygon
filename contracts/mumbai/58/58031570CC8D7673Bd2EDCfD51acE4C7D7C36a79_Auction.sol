...
contract Auction is Initializable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function createAuctionAndBidWithVoucher(
        ...
        uint256 _tokenId = INFT(_nftAddress).mintFromRequest(_requestID, _payToken, msg.sender);
    ...
    function resultAuction(
        ...
        require(
            IERC721Upgradeable(_nftAddress).ownerOf(_tokenId) == _owner && _owner == auction.owner,
        ...
        // Ensure this contract is approved to move the token
        require(IERC721Upgradeable(_nftAddress).isApprovedForAll(_owner, address(this)), "auction not approved");
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            total += amounts[i];
            IERC20Upgradeable(auction.payToken).safeTransferFrom(
                winner,
                recipients[i],
                amounts[i]
            );
    ...
    function _cancelAuction(address _nftAddress, uint256 _tokenId) private whenNotPaused {
        ...
        if (IERC165Upgradeable(_nftAddress).supportsInterface(INTERFACE_ID_ERC1155)) {
            // ERC1155 token
            if (IERC1155Upgradeable(_nftAddress).balanceOf(address(this), _tokenId) > 0) {
                // burn all tokens held by the contract
                IERC1155Upgradeable(_nftAddress).burn(address(this), _tokenId, IERC1155Upgradeable(_nftAddress).balanceOf(address(this), _tokenId));
            }
        } else {
            // ERC721
            if (IERC721Upgradeable(_nftAddress).balanceOf(address(this)) > 0) {
...
interface INFTFactory {
  function createNFTContract ( string memory _name, string memory _symbol, address _token, address _sender ) external returns ( address );
