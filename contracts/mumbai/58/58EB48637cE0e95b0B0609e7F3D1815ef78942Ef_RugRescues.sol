 ...
 *                                               LordLabz                                                        *
 *   Designed: JoeSoap8308 (Dev of the Wrapped project - https://opensea.io/collection/wrappedcompanion-1)    *
...
///////Start of contract/////
contract RugRescues is ERC721, IERC721Receiver, Pausable, Ownable {
    event Wrapped(uint256 indexed tokenId);
    event Unwrapped(uint256 indexed tokenId);
    address public originalProject;
    ...
    constructor(
        address _originalProject,
        string memory _defaultUri,
        string memory _name,
        string memory _symbol
    ) ERC721("Rug Rescues", "RUG") {
        originalProject = _originalProject;
        project = IERC721(_originalProject);
        ...
        returns (string memory)
    {
        ...
        if (dynamicart == true) {
            ...
            uint temp = wrapperdb(databaseContract).getArtStatus(tokenId);
    ...
    function returnUserFeeStatus(address _wallet) public view returns (uint) {
        ...
        uint _walletstatus;
        _walletstatus = wrapperdb(databaseContract).getFeesStatus(_wallet); //Get fee status 0/1/3
    ...
    function setOptions(
        ...
        if (_option == 4) {
            originalProject = _address;
        }
        if (_option == 5) {
            databaseContract = _address;
        }
        if (_option == 6) {
    ...
    function wrap(uint256[] calldata tokenIds_) external payable {
        ...
        uint _userstatus = wrapperdb(databaseContract).getFeesStatus(
            msg.sender
        ...
        ) ////This enables us to bypass fees globally if required.
        {
            if (templength == 1) {
                ...
                if (_userstatus == 0) //User has never wrapped
                {
                    ...
                    wrapperdb(databaseContract).setUserStatus(
                        msg.sender,
                        1,
                        true
                    );
        ...
        for (uint256 i = 0; i < templength; i++) {
            ...
            (_isblocked, s) = wrapperdb(databaseContract).isBlockedNFT(
                tokenIds_[i]
            );
            require(_isblocked != true, "Banned Token_ID!");
            project.safeTransferFrom(msg.sender, address(this), tokenIds_[i]);
        }
        //////Adjust the holder value to ensure rewards are split fairly!
        _isholder = wrapperdb(databaseContract).isHolder(msg.sender);
        if (_isholder == false) {
            wrapperdb(databaseContract).manageHolderAddresses(true, msg.sender); ///Sets user as a holder!
            wrapperdb(databaseContract).manageNumHolders(2); ///Bump the holder value
        }
        numwraps += 1;
    }
    ...
    function unwrap(uint256[] calldata tokenIds_) external {
        ...
        if (_balance == 0) {
            ...
            wrapperdb(databaseContract).manageHolderAddresses(
                false,
                msg.sender
            ); //Remove user as a holder
            wrapperdb(databaseContract).manageNumHolders(1); ///Dump the holder value
        }
    }
    ...
    function _flip(
        ...
        if (isWrapping_) {
            ...
            project.safeTransferFrom(address(this), who_, tokenId_);
    ...
    function onERC721Received(
        ...
        // Only supports callback from the original project contract and this contract
        require(
            msg.sender == address(project) || msg.sender == address(this),
            "must be Companion or WrappedCompanion"
        );

        bool isWrapping = msg.sender == address(project);
    ...
    function emergencyMintWrapped(uint256 tokenId_) external onlyOwner {
        if (project.ownerOf(tokenId_) == address(this)) {
    ...
    //Mint the Tokens that Founders of the project owned
    ...
    function mintFoundersBlockedNfts(
        ...
        for (uint256 i = 0; i < templength; i++) {
            //Add method in to verify banned ID's (Founders)
            (bool _isblocked, uint s) = wrapperdb(databaseContract)
