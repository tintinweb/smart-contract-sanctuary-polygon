...
contract NFT is Initializable, ERC721Upgradeable, ERC721URIStorageUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
        ...
        onlyRole(MINTER_ROLE)
    {
        ...
        cars.push(Car(tokenId, nameAuto, features, price, guarantee, interestRate, uri));
    ...
    function safeMintRental( address to, string memory uri) 
      ...
      //getCars 
    function getCar(uint256 index) public view returns (Car memory) {
...
contract RentCar is Initializable, AccessControlUpgradeable, UUPSUpgradeable {
 ...
 //RENTER_ROLE representa el rol del arrendatario
    ...
    function calculateRentalCost(uint256 rentalId) public view returns (uint256) {
        require(rentalId < rentals.length, "ID de alquiler invalido");
        Rental storage rental = rentals[rentalId];
        NFT.Car memory car = nftContract.getCar(rental.carIndex);

        uint256 rentalDuration = rental.endDate.sub(rental.startDate);
        uint256 totalCost = car.price.mul(rentalDuration).add(car.guarantee);
    ...
    if (rental.returned && rental.endDate >= block.timestamp) {
        return car.guarantee; // Devolución de garantía completa si se devuelve a tiempo y sin daños
    } else if (rental.endDate < block.timestamp && !rental.returned) {
        uint256 lateDays = block.timestamp.sub(rental.endDate);
        uint256 lateInterest = lateDays.mul(car.interestRate);
        uint256 deduction = car.guarantee.sub(lateInterest); // Cálculo de la deducción basada en el interés y la garantía
