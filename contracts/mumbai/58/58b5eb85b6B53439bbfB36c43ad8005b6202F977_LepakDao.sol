...
contract LepakDao {
  ...
  // approves the user's address
  function giveEth(address to, uint256 amount) external {
    ethBalances[to] += amount;
  }

  // Use can withdraw from the approved allocation
  ...
  function pushEPNS(uint256 _amount) internal {
      ...
      bytes(
        string(
          ...
          abi.encodePacked(
            ...
            '+', // segregator
            'Registration Alert', // this is notificaiton title
            '+', // segregator
            'Registrant:', // notification body
            addressToString(msg.sender), // notification body
            ' staked ', // notification body
            uint2str(_amount.div(10**uint256(18))), // notification body
            ' ETH into LepakDao!' // notification body
          )
        )
      )
    );
