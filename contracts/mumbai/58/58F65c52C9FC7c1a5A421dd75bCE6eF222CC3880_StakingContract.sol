...
contract StakingContract {
    ...
    function claimRewards(address user,uint256 poolIndex) public {
        ...
        for (uint256 i = 0; i < 15; i++) {
            ...
            users[ref].levelWiseIncome[i]+=(timePassed *
                    users[user].deposits[poolIndex].stakedAmount *
                    pools[poolIndex].levelRewards[i]) /
                10000 /
                pools[poolIndex].timePeriod;
        ...
        users[user].deposits[poolIndex].lastRewardTimestamp = block
            .timestamp;
    }
    ...
    function unstake(uint256 poolIndex) external  {
        ...
        require(
         ...
         pools[poolIndex].totalStake-=users[msg.sender].deposits[poolIndex].stakedAmount;
    ...
    function getDepositInfo(address user,uint256 poolIndex) external view returns (Deposit memory) {
