    ...
    EssentialERC2771Context
{
    ...
    mapping(bytes32 => MintPass) public openers;
    ...
    function openPack(uint256 tokenId) external {
        require(LINK_TOKEN.balanceOf(address(this)) >= LINK_FEE, "Not enough LINK");
        ...
        openers[requestId] = MintPass({minter: _msgSender(), count: tokenId});
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        MintPass memory pass = openers[requestId];
        randomizePack(pass.count, randomness, pass.minter);
    }
    ...
    function randomizePack(uint256 numShuffles, uint256 entropy, address minter) internal {
        ...
        for (uint256 i = shuffledCount; i < nextId; ) {
            ...
            // Update the value at the random index to the Token in current index
            tokens[randomIndex] = tokens[i];

            // Update the current value to the assignedToken
    ...
    function devMintMultiple(MintPass[] calldata addresses) external onlyOwner {
        uint256 count = addresses.length;

        for (uint256 index = 0; index < count; ) {
            MintPass calldata pass = addresses[index];
            require(mintedSupply + pass.count <= TOTAL_SUPPLY, "Mint pass exceeds token supply");
            _mint(pass.minter, pass.count, 1, "");
            unchecked {
                ++index;
                mintedSupply += pass.count;
            }
        }
    }
