...
library FCL_Elliptic_ZZ {
    ...
    function ecAff_SetZZ(uint256 x0, uint256 y0)
        internal
        pure
        ...
        returns (uint256 P0, uint256 P1, uint256 P2, uint256 P3)
    {
        unchecked {
            ...
            assembly {
                ...
                P1 :=
                    addmod(
                        mulmod(addmod(zz1, sub(p, P0), p), y2, p), mulmod(y1, P1, p), p
                    ) //R*(Q-X3)
            } //end assembly
        } //end unchecked
        return (P0, P1, P2, P3);
     ...
     */
    function ecZZ_IsZero(uint256 x0, uint256 y0, uint256 zz0, uint256 zzz0)
        internal
        pure
        returns (bool)
    {
     ...
     */
    function ecAff_IsZero(uint256 x, uint256 y)
        internal
        pure
        returns (bool flag)
    {
     ...
     */
    function ecAff_isOnCurve(uint256 x, uint256 y)
        internal
        pure
        returns (bool)
    {
        ...
        unchecked {
            ...
            uint256 RHS =
                addmod(mulmod(mulmod(x, x, p), x, p), mulmod(x, a, p), p); // x^3+ax
    ...
    function ecAff_add(uint256 x0, uint256 y0, uint256 x1, uint256 y1)
        internal
     ...
     */
            ...
            assembly {
                ...
                zz :=
                    add(
                        shl(1, and(shr(index, scalar_v), 1)), and(shr(index, scalar_u), 1)
                    )
                ...
                for {} gt(minus_1, index) { index := sub(index, 1) } {
                    ...
                    let T4 :=
                        mulmod(
                            3, mulmod(addmod(X, sub(p, zz), p), addmod(X, zz, p), p), p
                        ) //M=3*(X1-ZZ1)*(X1+ZZ1)
...

                    {
                        ...
                        if eq(y2, 0) {
                            if eq(T2, 0) {
                                ...
                                X :=
                                    addmod(mulmod(T4, T4, p), mulmod(minus_2, T3, p), p) //X3=M^2-2S
                                T2 := mulmod(T4, addmod(T3, sub(p, X), p), p) //M(S-X3)
                ...
                T[0] = 64
                    * (
                        128 * ((scalar_v >> zz) & 1) + 64 * ((scalar_v >> (zz - 64)) & 1)
                            + 32 * ((scalar_v >> (zz - 128)) & 1)
                            + 16 * ((scalar_v >> (zz - 192)) & 1) + 8 * ((scalar_u >> zz) & 1)
                            + 4 * ((scalar_u >> (zz - 64)) & 1) + 2 * ((scalar_u >> (zz - 128)) & 1)
                            + ((scalar_u >> (zz - 192)) & 1)
                    );
            }
            assembly {
                ...
                for { let index := 254 } gt(index, 191) {
                    index := add(index, 191)
                } {
                    {
                    ...
                    }
                    {
                        ...
                        let T1 :=
                            add(
                                T2,
                                add(shl(10, and(shr(index, scalar_v), 1)), shl(6, and(shr(index, scalar_u), 1)))
                            )
...

                    {
                        ...
                        let y2 :=
                            addmod(mulmod(mload(add(T, 32)), zzz, p), Y, p)
            ...
            assembly {
                ...
                for { let index := 254 } gt(index, 191) {
                    ...
                    T4 :=
                        add(
                            T4,
                            add(shl(12, and(shr(index, scalar_v), 1)), shl(8, and(shr(index, scalar_u), 1)))
                        )
                    index := sub(index, 64)
                    T4 :=
                        add(
                            T4,
                            add(shl(11, and(shr(index, scalar_v), 1)), shl(7, and(shr(index, scalar_u), 1)))
                        )
                    index := sub(index, 64)
                    T4 :=
                        add(
