...
contract InvoiceFactory is Invoice, VerifySignature, AdminOnly {
    ...
    event InvoicePaid(uint256 idInvoice, bool paid, uint256 date);
    ...
    function deleteMark(uint _id) public onlyInvoicer(_id) {
    ...
    function payInvoice(
        ...
        emit InvoicePaid(_id, true, block.timestamp);
    }
    ...
    function requestRenegotiation(
        ...
        emit NewRequest(_id, block.timestamp, _newFee, _newAmount, _newDate);
    }

    function declineRenegotiation(
        uint256 _id
    ) public validID(_id) onlyInvoicer(_id) {
        requests[_id] = Request(_id, 0, 0, 0, 0, "", "");
        emit Decline(
            _id,
            block.timestamp,
            invoices[_id].fee,
...
contract GoodNFT is Context, ERC165, IERC721, IERC721Metadata {
    ...
    InvoiceFactory private _otherNFT;
    ...
    constructor(address otherNFTAddress) {
        _otherNFT = InvoiceFactory(otherNFTAddress);
        factoryContract = otherNFTAddress;
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (batchSize > 1) {
            if (from != address(0)) {
                for (
                    uint256 i = _ownedTokens[from].length - batchSize;
                    i < _ownedTokens[from].length;
                    i++
                ) {
    ...
    function isApprovedForAll(
        address owner,
        address operator
    ) public view override onlyFactory returns (bool) {
        return _operatorApprovals[owner][operator];
    }
    ...
    function _afterTokenTransfer(
        ...
        if (batchSize > 1) {
            ...
            if (to != address(0)) {
                for (
                    uint256 i = _ownedTokens[to].length - batchSize;
                    i < _ownedTokens[to].length;
                    i++
                ) {
                    _ownedTokensIndex[_ownedTokens[to][i]] = i;
                }
    ...
    function soulbind(uint256 tokenId) public onlyFactory {
        require(
        ...
        require(
            !_nfts[tokenId].soulbound,
            "ERC721: token is already soulbound"
        );
...
contract Invoice is ERC721, ERC721Enumerable, ERC721URIStorage {
    ...
    constructor() ERC721("KryptoKreditInvoice", "KRED") {}
