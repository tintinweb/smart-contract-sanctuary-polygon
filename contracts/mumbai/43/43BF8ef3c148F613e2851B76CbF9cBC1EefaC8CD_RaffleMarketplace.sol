...
// access control to the owner who deploys the contract
...
error RaffleMarketplace__StageAlreadyExist(uint256 raffleId, Stage.StageType stageType);
...
contract RaffleMarketplace is Ownable, VRFV2SubscriptionManager,RaffleRegisterUpkeep {
    /*
        1. Create raffle
    ...
    event RaffleCreated(
        ...
        Raffle indexed raffle,
        Stage.RaffleStage[] stages,
        RafflePrize[] prizes
    );
    event RaffleVerified(uint256 indexed raffleTicker, address indexed deployedRaffle);
    event RaffleStageAdded(uint256 indexed raffleTicker, Stage.RaffleStage indexed stage);
    event RaffleWinnersPicked(uint256 indexed raffleTicker, address payable[] indexed winners);
    event RaffleStateUpdated(uint256 indexed raffleTicker, Stage.RaffleState indexed state);
    event RaffleStageTicketPriceUpdated(
        uint256 indexed raffleTicker,
        Stage.StageType indexed stageType,
    ...
    struct Raffle {
        ...
        PrizeCollectionCountry collectionCountry; // country from where the prize is to be collected / delivery where the prize can be delivered
        Stage.RaffleState raffleState; // state of the raffle, init as not_initialized
    }
    ...
    mapping(uint256 => Raffle) _raffles;
    ...
    mapping(uint256 => RafflePrize[]) _raffleToRafflePrizes;
    // mapping of raffle identifer to raffle stages
    mapping(uint256 => Stage.RaffleStage[]) _raffleToRaffleStages;
    // mapping of raffle identifer to ongoing stages
    mapping(uint256 => Stage.StageType) _raffleToOngoingStages;
    ...
    function createRaffle(
        ...
        RafflePrize[] memory prizes,
        CharityInformation memory charityInfo,
        Stage.RaffleStage[] memory stages,
        PrizeCollectionCountry _collectionCountry
    ) external {
        // adds raffle to the mapping
        address payable[] memory winners = new address payable[](prizes.length);
        Raffle memory raffleStruct = Raffle(
            raffleTicker,
            false,
            address(0),
            _category,
            ...
            winners,
            _collectionCountry,
            Stage.RaffleState.NOT_INITIALIZED
        );
    ...
    function verifyRaffle(uint256 id) external invalidTickerId(id) doesRaffleExists(id) onlyOwner {
        ...
        Raffle storage raffleStruct = _raffles[id];
        // verify the raffle
        _raffles[id].isVerifiedByMarketplace = true;
        raffleStruct.raffleState = Stage.RaffleState.OPEN;
        ...
        RaffleContract raffle = new RaffleContract(
            ...
            owner(),
            uint32(_raffleToRafflePrizes[id].length),
        ...
        topUpSubscription(1000000000000000000);
    ...
    */
        ...
        raffleStageNotExists(raffleId, stageType)
    {
        ...
        Stage.RaffleStage memory stage = Stage.RaffleStage(
            stageType,
            ticketsAvailable,
            ticketPrice,
            0
    ...
    {
        Stage.RaffleStage[] storage stages = _raffleToRaffleStages[raffleId];
    ...
    function _addPrizeInStorage(RafflePrize[] memory _prizes) internal {
        for (uint256 i = 0; i < _prizes.length; i++) {
            _raffleToRafflePrizes[raffleTicker].push(
                RafflePrize(i, _prizes[i].prizeTitle, _prizes[i].country, _prizes[i].prizeAmount)
            );
        }
    }
    ...
    function updateRaffleState(uint256 id, Stage.RaffleState state)
        external
        onlyRaffleContract(id)
    {
    ...
    function updateTicketsSold(
        ...
        for (uint256 i = 0; i < _raffleToRaffleStages[id].length; i++) {
            if (_raffleToRaffleStages[id][i].stageType == stageType) {
                _raffleToRaffleStages[id][i].ticketsSold = ticketsSold;
    ...
    function updateCurrentOngoingStage(uint256 id, Stage.StageType stageType)
        external
        onlyRaffleContract(id)
    {
    ...
    // returns prize information of the raffle
    function getRafflePrizeInformationById(uint256 id)
        public
        view
        invalidTickerId(id)
        returns (RafflePrize[] memory)
    {
        return _raffleToRafflePrizes[id];
    }
    ...
    function getRaffleStagesById(uint256 id)
        ...
        invalidTickerId(id)
        doesRaffleExists(id)
        returns (Stage.RaffleStage[] memory)
    {
        return _raffleToRaffleStages[id];
    }

    function getParticularRaffleStage(uint256 id, Stage.StageType stageType)
        external
        view
        invalidTickerId(id)
        ...
        returns (Stage.RaffleStage memory)
    {
        Stage.RaffleStage[] memory stage = getRaffleStagesById(id);
    ...
    function getOngoingRaffleStage(uint256 id) public view returns (Stage.StageType) {
    ...
    modifier raffleStageExists(uint256 raffleId, Stage.StageType stageType) {
    ...
    modifier raffleStageNotExists(uint256 raffleId, Stage.StageType stageType) {
        if (doesRaffleStageExists(raffleId, stageType)) {
    ...
    function doesRaffleStageExists(uint256 raffleId, Stage.StageType stageType)
        public
        view
        returns (bool)
    {
        Stage.RaffleStage[] memory stage = getRaffleStagesById(raffleId);
    ...
    //function to check if the raffle prize exists based on prize id
    function doesRafflePrizeExists(uint256 raffleId, uint256 prizeId)
        public
        view
        invalidTickerId(raffleId)
        doesRaffleExists(raffleId)
        returns (bool)
    {
        RafflePrize[] memory prizes = getRafflePrizeInformationById(raffleId);
    ...
    modifier rafflePrizeNotExists(uint256 raffleId, uint256 prizeId) {
        if (!doesRafflePrizeExists(raffleId, prizeId)) {
            revert RaffleMarketplace__PrizeDoesNotExist(raffleId, prizeId);
        }
        _;
    }
    modifier rafflePrizeExists(uint256 raffleId, uint256 prizeId) {
        if (doesRafflePrizeExists(raffleId, prizeId)) {
            revert RaffleMarketplace__PrizeAlreadyExist(raffleId, prizeId);
        }
        _;
    }
...
contract RaffleContract is VRFConsumerBaseV2, KeeperCompatibleInterface {
    ...
    */
    ...
    function _shuffle() internal returns(Players[] memory) {
    for (uint256 i = 0; i < totalTicketsSold(); i++) {
        uint256 n = uint256(keccak256(abi.encodePacked(block.timestamp))) % (totalTicketsSold() - i) + i;
        Players memory temp = s_players[n];
        s_players[n] = s_players[i];
        s_players[i] = temp;
    }
    return s_players;
}
    ...
    function getPlayers() public view returns (Players[] memory){
        return s_players;
    }
    ...
    function getCurrentState() external view returns(Stage.RaffleState ){
