...
contract SoulboundToken is ERC721Upgradeable, OwnableUpgradeable, CantBeEvilUpgradeable() {
  ...
  function updateWhiteList(address __account, uint256 __tokenId) external onlyOwner {
    whiteList[__account] = __tokenId;
  }

  function mintByOwner(address __account, uint256 __tokenId) external onlyOwner {
    mint(__account, __tokenId);
  }

  function mintByMember() external {
    require(whiteList[msg.sender] != 0, "not in whitelist");
    mint(msg.sender, whiteList[msg.sender]);
  }

  function mintBySignature(string memory __message, bytes memory __signature, uint256 __tokenId) external {
    ...
    mint(msg.sender, __tokenId);
  }

  function mint(address __to, uint256 __tokenId) internal {
    require(balanceOf(__to) == 0, "already minted");
    require(__tokenId <= maxSupply, "max supply reached");
    super._mint(__to, __tokenId);
    emit Mint(__to, __tokenId);
  }
