...
contract RUFFRAFFLE {
    ...
    function removeRaffle(uint256 raffleId) external onlyRaffleCreator(raffleId) {
        ...
        require(raffle.isActive || raffle.ticketsSold == 0, "Cannot remove an active raffle with sold tickets.");
    ...
    function endRaffle(uint256 raffleId) external onlyRaffleCreator(raffleId) {
        Raffle storage raffle = raffles[raffleId];
        require(raffleExists[raffleId], "Raffle does not exist.");
        require(raffle.isActive, "Raffle is not active.");
