...
contract RevenueShareForDonation is AccessControl, ReentrancyGuard{
  ...
  function batchWithdraw() external onlyRole(WITHDRAWER_ROLE) payable nonReentrant{
    require(withdrawnId < receiveId, "No Withdrawable Request");
  ...
  function setTokenToMaterial(uint8[25] memory materialList, uint256 page) public onlyRole(DEFAULT_ADMIN_ROLE){
  ...
  // nhiều token => 1 material (0 -> 6)=> 1 địa chỉ donation (0 -> 6)
