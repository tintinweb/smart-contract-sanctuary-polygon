    ...
    ERC721PausableUpgradeable
{
    ...
    uint256 public constant PRICE0 = 0.12 ether;
    uint256 public constant PRICE1 = 0.15 ether;
    uint256 public constant PRICE2 = 0.15 ether;
    ...
    function setPrice(uint256 price_, uint256 phase_) external onlyOwner {
        require(
            price_ != 0 && phase_ > phase && phase_ < prices.length,
            "!bad"
        );
    ...
    {
        require(
            amountMax_ != 0 && phase_ > phase && phase_ < prices.length,
            "!bad"
        );
    ...
    function setMerkleRoot(bytes32 merkleRoot_, uint256 phase_)
        ...
        onlyOwner
    {
        require(phase_ >= phase && phase_ < merkleRoots.length, "!bad");
    ...
    function isLastPhase() public view returns (bool) {
        return _isLastPhase(phase);
    }

    function _isLastPhase(uint256 ph) internal view returns (bool) {
        return ph == prices.length - 1;
    }
     ...
     */
    function available(address to) public view returns (uint256 quantity) {
        require(balances[phase] + quantity <= totalSupplies[phase], "!total");
    ...
    function batchMint(
        ...
        require(quantity <= available(_msgSender()), "!quantity");
        if (!isLastPhase()) {
            _merkleProofVerify(
                merkleRoots[phase],
                index,
