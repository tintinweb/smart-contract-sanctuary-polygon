 ...
 constructor(
    ...
    function numberOfProjInvestedIn(uint256 _tokenId) external {
        ...
        for (uint256 i = 0; i <= allPair; i++) {
            ...
            uint256 bal = IrigelProtocolLanchPadPool(pair).getUser(_msgSender());
            if (bal > 0) {
                tNumber = tNumber + 1;
            }
        }
    ...
    function getLaunchPadData(address _user) public view returns(uint256[] memory _userAmount, address[] memory pair) {  
        uint256 allPair = launchPadFactoryContract(launchPadFactory).allPairsLength();  
        for (uint256 i = 0; i <= allPair; i++) {
    ...
    function getNumbersOfProjInvestedIn(address _user) public view returns(uint256 _num) {        
        ...
        for (uint256 i = 0; i <= allPair; i++) {
            address pair = launchPadFactoryContract(launchPadFactory).allPairs(i);
            uint256 bal = IrigelProtocolLanchPadPool(pair).getUser(_user);
            if (bal > 0) {
                tNumber = tNumber + 1;
            }
        }
