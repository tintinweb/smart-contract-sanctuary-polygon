...
contract Lottery is VRFConsumerBaseV2 {
  ...
  function withdraw() external {
    ...
    LINKTOKEN.transfer(ownerAddress, linkBalance);
  }
	...
	/**
	 * @notice Update the subscription ID and add this contract as a consumer.
	 * @dev The process of creating a subscription, funding it and adding the new contract as a
	 * consumer is not automated yet due to time constraints.
	 * @param _subscriptionId the id of the subscription
	 */
	function setSubscription(uint64 _subscriptionId) public {
		if (msg.sender != ownerAddress) revert Unauthorized();
		subscriptionId = _subscriptionId;
		//CONNECTOR.addConsumer(subscriptionId, address(this))
	}
