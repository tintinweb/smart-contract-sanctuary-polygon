...
contract BratkiBeerChallenge is ERC721URIStorage {
    ...
    function generateCharacter(uint256 tokenId, string memory name) public returns(string memory){
        bytes memory svg = abi.encodePacked(
            ...
            '<text x="50%" y="50%" class="base" dominant-baseline="middle" text-anchor="middle">', "Beers: ",getBeerCount(tokenId),'</text>',
            '</svg>'
        );
        return string(
            abi.encodePacked(
    ...
    function getTokenURI(uint256 tokenId, string memory name) public returns (string memory) {
        bytes memory dataURI = abi.encodePacked( // dataURI aka metadata itself
        '{',
            '"name": "Bratki Beer Challenge Contestant #', tokenId.toString(), '",',
            '"description": "We wish good luck to ', name, '!",',
            '"image": "', generateCharacter(tokenId, name), '"',
    ...
    function mint(string memory _name) public {
        ...
        tokenIdToContestant[newItemId] = Contestant({ name: _name, beerCount: 0 });
        _setTokenURI(newItemId, getTokenURI(newItemId, _name)); // another erc721 function, setting metadata
    }
    ...
    function drinkBeer(uint256 tokenId) public {
        ...
        _setTokenURI(tokenId, getTokenURI(tokenId, tokenIdToContestant[tokenId].name));
    }
}
