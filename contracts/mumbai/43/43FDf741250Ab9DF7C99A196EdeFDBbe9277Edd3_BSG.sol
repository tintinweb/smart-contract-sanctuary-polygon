...
contract BSG is Ownable {
    ...
    uint256 public constant minDeposit = 1e14;    

    uint256 public  tradingSplit = 1875;

    uint256 private constant timeStep = 60;//1 days;

    uint256 private constant dayPerCycle = 420; // 7 days;

    uint256 private constant dayRewardPercents = 200;

    uint256 private constant maxAddFreeze = 2700;//45 days;
    ...
    constructor(address _defaultRefer, address[2] memory _feeReceivers)  {
    ...
    function withdraw() external {
        ...
        if(withdrawable>0)
        {
            ...
            if(widthfeePercents>0){
                ...
                payable(feeReceivers[0]).transfer(withdrawfee.div(2));

                payable(feeReceivers[1]).transfer(withdrawfee.div(2));

                withdrawable = withdrawable - withdrawfee;

            }            

            if(isSplitForWithdraw)
            {
                withdrawable = withdrawable.mul(withdrawSplitFactor).div(baseDivider);

                userInfo[_sender].retopUpWallet = userInfo[_sender].retopUpWallet.add(withdrawable);
    ...
    function _calLevelNow(address _user) private view returns(uint256) {
        ...
        if(total >= 5e16 && user.teamNum >= 5 && maxTeam >= 1e18 && otherTeam >= 1e18){

            levelNow = 5;

        //}else if(total >= 1000e18 && user.teamNum >= 40 && maxTeam >= 7000e18 && otherTeam >= 7000e18){
        }else if(total >= 1e15 && user.teamNum >= 2 && maxTeam >= 1e16 && otherTeam >= 1e16){
    ...
    function _distributeLevelPool() private {
        ...
        uint256 totalrewards;
          ...
          if(level4Pool > totalrewards)
          {
              ...
              payable(feeReceivers[0]).transfer(level4Pool);
          }
        ...
        totalrewards = 0;
           ...
           if(level5Pool > totalrewards)
          {
              ...
              payable(feeReceivers[0]).transfer(level5Pool);
          }
    ...
    function _distributeDeposit(uint256 _amount) private {
        ...
        payable(feeReceivers[0]).transfer(fee);

        uint256 tradingSplitamount = _amount.mul(tradingSplit).div(baseDivider);

        payable(feeReceivers[1]).transfer(tradingSplitamount);
    ...
    function updatedData(address[2] memory _feeReceivers, uint256 _depositFee, uint256 _tradingSplit, uint256 _level4royalty, uint256 _level4allow, uint256 _level5royalty, uint256 _level5allow) external onlyOwner returns(bool)
    {
    ...
    function withdrawFund(uint256 _amount) public onlyOwner returns(bool)
    {
