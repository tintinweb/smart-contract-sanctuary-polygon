...
contract TKNStakingT23 is Ownable {
    ...
    uint256 public constant ONE_DAY = 2 seconds;
    uint256 public constant MULTIPLIER = 1e14; // 4 decimals
    ...
    function getUnlockSpecs(uint256 _amount, uint256 _lockMode) public view returns(uint256 lockUntil, uint256 weight) {
        ...
        if(_lockMode == 0) {
            // 0 : 14-day lock
            return (now256() + 14 * ONE_DAY, _amount);
        }
        else if(_lockMode == 1) {
            // 1 : 30-day lock
