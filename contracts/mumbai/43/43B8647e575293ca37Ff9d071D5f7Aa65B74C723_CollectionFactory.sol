...
contract CollectionFactory is Initializable, OwnableUpgradeable, PausableUpgradeable {
    ...
    function createCollection(
        ...
        Proxy collection = new Proxy();
        bytes memory data =abi.encodePacked("initialize",
        abi.encodePacked(name, symbol, contractUri, baseUri, royalty, msg.sender));
        collection.initialize(implementationContract,data,msg.sender);
...
contract Proxy is Initializable, ERC1967UpgradeUpgradeable, OwnableUpgradeable {
    ...
    function initialize(
        ...
        _upgradeBeaconToAndCall(_implementation,_data,false);
        _changeAdmin(_admin);
    }
    
    function setUpgrade(address _implementation,
        bytes memory _data,
