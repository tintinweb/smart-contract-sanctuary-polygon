...
contract uTokenFactory is Ownable{
    ...
    function calculateRewardOfWinner() public view returns (uint256 ethForReward, RewardAgainstTokens[] memory rewardAgainstTokens) {
        
        rewardAgainstTokens = new RewardAgainstTokens[](all_AllowedTokensCount());

        address[] memory _allowedTokens = all_AllowedTokens();

        for(uint i; i < _allowedTokens.length; i++) {
            rewardAgainstTokens[i] = RewardAgainstTokens(_allowedTokens[i], IERC20(_allowedTokens[i]).balanceOf(address(this)));
        }

        return (ethForReward, rewardAgainstTokens);
    }
}