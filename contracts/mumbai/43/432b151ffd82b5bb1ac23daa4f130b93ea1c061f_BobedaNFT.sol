...
contract BobedaNFT is Ownable {
    ...
    modifier onlyAdminitrator() {
        require(
            administrator[msg.sender],
            "Error need to be administrator"
        );
        _;
    }
    ...
    function addAdministrator(address new_administrator) public onlyOwner {
        administrator[new_administrator] = true;
    }

    function removeAdministrator(address _administrator) public onlyOwner {
        administrator[_administrator] = false;
        delete administrator[_administrator];
    }
    ...
    function openSale(uint256 p_nftID, uint256 p_price) public securityFrontRunning(p_nftID) onlyAdminitrator payable {
    ...
    function cancelSale(uint256 p_nftID) public securityFrontRunning(p_nftID) payable{
        require(
            msg.sender == s_sales[p_nftID].owner,
            "Without permission"
        );
    ...
    function buy(uint256 p_nftID, uint256 p_price) public  securityFrontRunning(p_nftID) payable{
        require(s_sales[p_nftID].status == Status.open, "Is not Open");
