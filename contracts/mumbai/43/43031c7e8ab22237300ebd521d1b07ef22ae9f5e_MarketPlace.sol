...
contract MarketPlace is ERC165 {
    ...
    function placeOrder(
        ...
        if(!_isResale)require(ERC721Interface.isWhitelisted(msg.sender), "Not whitelisted");
        require(
    ...
    function buy(
        ...
        require(
            amount == (copies * _order.startPrice),
    ...
    function payment(
        ...
        uint256 mainAmount=amount;
        sendValue(msg.sender, Treasury, fees, _order.paymentToken, pnp);

        amount = amount - fees;
        (address user, uint256 royaltyFee) = ERC721Interface.royaltyInfo(
            _order.tokenId,
            (mainAmount)
        );
        ...
        (address receiver, uint256 xrRoyaltyFee) = ERC721Interface.xrRoyaltyInfo(
            _order.tokenId,
            (mainAmount)
        );
