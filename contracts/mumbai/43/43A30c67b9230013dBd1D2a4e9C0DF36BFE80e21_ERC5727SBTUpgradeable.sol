    ...
    ERC5727GovernanceUpgradeable
{
        ...
        override(ERC5727Upgradeable, ERC5727EnumerableUpgradeable)
    {
        if (from == address(0)) {
            if (
                _maxSupply[slot] != 0 &&
                _tokenCountInSlot(slot) + 1 > _maxSupply[slot]
            ) revert ExceedsMaxSupply(_maxSupply[slot]);

            (bool success, uint256 balance) = _ownerBalanceInSlot[slot].tryGet(to);
            if (balance > 0) {
                revert Forbidden();
            }
        }
    ...
    IERC3525SlotEnumerableUpgradeable
{
    ...
    mapping(uint256 => EnumerableSetUpgradeable.UintSet) internal _tokensInSlot;
