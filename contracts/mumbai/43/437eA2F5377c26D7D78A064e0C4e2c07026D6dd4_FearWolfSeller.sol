...
contract FearWolfSeller is MuonClient, AccessControl, Pausable, Withdrawable, ReentrancyGuard {
    using SafeMath for uint256;
    using Strings for uint256;

    /**
    * @dev Emitted when FEAR Wolf is bought with ETH.
    */
    event WolvesBoughtWithEth(address indexed account, uint256 price, uint256 count, uint256 totalAmount);

    /**
    * @dev Emitted when FEAR Wolf is bought with FEAR.
    */
    event WolvesBoughtWithFear(address indexed account, uint256 price, uint256 count, uint256 fearPrice, uint256 ethPrice, uint256 totalAmount);

    IERC20 public fearContract;
    IFearWolf public fearWolfContract;

    uint256 private percentageDivider = 10000;
    uint256 public fearWolfPrice = 0.2 ether;
    uint256 public fearPurchaseDiscount= 2000; // 20%
    ...
    function getFearAmount(uint256 wolvesNumber, uint256 fearPrice, uint256 ethPrice) public view returns(uint256) {
        uint256 singleWolfInFear = fearWolfPrice.mul(ethPrice).div(fearPrice);
        uint256 singleWolfInFearAfterDiscount = singleWolfInFear.mul(percentageDivider.sub(fearPurchaseDiscount)).div(percentageDivider);
        uint256 totalFear = singleWolfInFearAfterDiscount.mul(wolvesNumber);

        return totalFear;
    }

    function buyWolvesWithFear(
        uint256 wolvesCount,
        uint256 fearPrice,
        uint256 ethPrice,
        ...
        uint256 fearAmount = getFearAmount(wolvesCount, fearPrice, ethPrice);
        ...
        emit WolvesBoughtWithFear(caller, fearWolfPrice, wolvesCount, fearPrice, ethPrice, fearAmount);
    }


    function buyWolvesWithEth(uint256 wolvesCount) external payable whenNotPaused nonReentrant {
        address caller = _msgSender();

        require(tx.origin == caller, "Contracts are not allowed");
        require(wolvesCount > 0, "Cannot buy zero wolves");
        require(msg.value >= wolvesCount.mul(fearWolfPrice), "Insufficient ETH sent");

        fearWolfContract.setInitialOwner(caller, wolvesCount);
        emit WolvesBoughtWithEth(caller, fearWolfPrice, wolvesCount, msg.value);
    }
    ...
    function setFearPurchaseDiscount(uint256 _fearPurchaseDiscount) external onlyRole(MANAGER_ROLE) {
        fearPurchaseDiscount = _fearPurchaseDiscount;
    }
}
