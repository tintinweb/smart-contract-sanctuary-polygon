...
contract PledgeStakeV2 {
    ...
    function _updateRewards(address _user, uint256 _amount) private {
        ...
        for (uint256 i = lastProcessedStakeIndex[_user]; i < stakes[_user].length; i++) {
            ...
            if (block.timestamp >= userStake.startTime + (roundDuration * (i + 1))) {
                uint256 round = (block.timestamp - userStake.startTime) / roundDuration;
                if (round < totalRounds) {
                    uint256 roundReward = (_amount * roundRewardPercentages[round]) / 100;
