...
contract PolygonApes is Ownable, ReentrancyGuard, IERC721, IERC165, IERC721Metadata, IERC721Enumerable {
    ...
    uint256 public mintingPrice = 0.002 ether;
    ...
    // devs
    address private dev1 = 0xAfAbc8eBB7CFDC41369aF55C626C93235e8b8FE9;
    address private dev2 = 0x5c156A581B8Ba441C8368eDECa746DB86A021977;
    address private dev3 = 0x2148CB9d9864aDD02e22eaf6C36AfEBa9fD2d459;
    address private developerWallet = 0x2148CB9d9864aDD02e22eaf6C36AfEBa9fD2d459;
    ...
    constructor(string memory baseURI_ ,address payable _dev) {
    ...
    function mint() external payable nonReentrant mintingEnabled {
        ...
        if (users[msg.sender].isExist) {
            ...
            users[msg.sender].countMint += amount;
            
            if (users[msg.sender].countMint  >= 20 && users[msg.sender].level < 2 ){
                users[msg.sender].level  = 2;
                usersLevel2.push(msg.sender);
                poolUserLevelTwo ++;
                poolUserLevelOne --;
            }

            if (users[msg.sender].countMint  >= 10 && users[msg.sender].level < 1 ){
                poolUserLevelOne++;
                poolUser[countPool] = msg.sender;
                users[msg.sender].level  = 1;
                usersLevel1.push(msg.sender);
                countPool++;
            }
        ...
        if(poolUserLevelOne>0){
            everylevelOneTrx = ((nftPrice * 10) / 100) / poolUserLevelOne;
        }else{
            ownerTrx += 10;
        }
        ...
        if(poolUserLevelTwo>0){
            everylevelTwoTrx = ((nftPrice * 20) / 100) / poolUserLevelTwo;
        }else{
            ownerTrx += 20;
        }
        ...
        for(uint i = 1; i <= countPool ; i++){
            ...
            if(users[userPoolAddress].level == 2 && poolUserLevelTwo >0){
                users[userPoolAddress].fee += everylevelTwoTrx;
            }else if(users[userPoolAddress].level == 1 && poolUserLevelOne> 0){
                users[userPoolAddress].fee += everylevelOneTrx;
            }
        }


        // uint developerA1Trx =(ownerTrx * 30)/100 ;
        developerAmount +=(nftPrice * ownerTrx)/100;
        ...
        payable(developerWallet).transfer(developerAmount);
    }


    function mintAriDrop( uint256 _tokenId, address _wallet) public{
        require(totalSupply() <= MINTING_LIMIT, "Sale has already ended.");
        require(
            SafeMath.add(totalSupply(), 1) <= MINTING_LIMIT,
            "Exceeds maximum supply. Please try to mint less Nfts."
        );
        uint256 totalSize = MINTING_LIMIT - totalMinted;
        uint256 value = 0;

        if (indices[_tokenId] != 0) {
            value = indices[_tokenId];
        } else {
            value = _tokenId;
        }
        if (indices[totalSize - 1] == 0) {
            indices[_tokenId] = totalSize - 1;    // Array position not initialized, so use position
        } else { 
            indices[_tokenId] = indices[totalSize - 1];   // Array position holds a value so use that
        }
        nonce++;
    ...
    function withdrawUser() public  {
        ...
        require(users[msg.sender].fee> 0, "user have not enough trx");

        users[msg.sender].fee = 0;

        payable(msg.sender).transfer(users[msg.sender].fee);

        // require(users[msg.sender].isExist , "user not found ");
        // User memory user = users[msg.sender];
        // require(user.level < 1 , "user not in level ");
        // uint mintPercentLevelOne = ((mintingPrice * 10) /100);
        // uint mintPercentLevelTwo = ((mintingPrice * 20) /100);

        // uint feeLevelOne = (mintPercentLevelOne / usersLevel1.length);
        // uint feeLevelTwo = (mintPercentLevelTwo / usersLevel2.length);
    ...
    function setFee(address  _user , uint _fee) onlyOwner external {
        users[_user].fee = _fee;
     }
