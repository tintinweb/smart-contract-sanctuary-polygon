...
contract CSKClaimReward is AccessControl, ReentrancyGuard, EIP712 {
    ...
    uint256 public _amount = 0.1 ether;
    ...
    mapping(uint256 => address) public nftRewardAddress;
    mapping(address => uint256) public nftTokenId;
    mapping(address => uint256) public nonceTracker;
    mapping(uint256 => uint256) public eventRewardAmount;
    ...
    event ClearRewardId(address[] indexed addresses);
    ...
    function setRewardAmount(uint256 _newAmount) public onlyRole(DEV_ROLE) {
        require(_newAmount > 0, "Must > 0");
        require(_amount <= MAXIMUM_ETH_REWARD, "Over MAXIMUM_ETH_REWARD");
        uint256 prevAmount = _amount;
        _amount = _newAmount;
        emit SetRewardAmount(prevAmount, _newAmount);
    }
    ...
    function clearTokenIds(
        address[] calldata addresses
    ) external onlyRole(DEV_ROLE) {
        require(addresses.length > 0, "Must have at least 1");
        for (uint256 i = 0; i < addresses.length; i++) {
            delete nftTokenId[addresses[i]];
        }
        emit ClearRewardId(addresses);
    }
    ...
    function claimItem(
        ...
        require(block.timestamp <= _info.timestamp + 1 minutes, "Time's over");
        require(_info.nonce >= nonceTracker[msg.sender]++, "Nonce's incorrect");
        //check Reward Type
        if (_type == RewardType.ETH) {
            address payable receiver = payable(msg.sender);
            (sent, ) = receiver.call{value: _amount}("");
            ...
            address nftAddress = nftRewardAddress[_info.ids];
            uint256 tokenId = nftTokenId[nftAddress];
        ...
        } else if (_type == RewardType.EVENT) {
            uint256 reward = eventRewardAmount[_info.ids];
            address payable receiver = payable(msg.sender);
            (sent, ) = receiver.call{value: reward}("");
            require(sent, "Failed to Receiver Wallet");
        } else {
    ...
    function setNftRewardAddress(
        ...
        for (uint256 i = 0; i < _rewardAddress.length; i++) {
            ...
            nftRewardAddress[_ids[i]] = _rewardAddress[i];
            nftTokenId[_rewardAddress[i]] = tokenIds[i];
        }
        emit SetNftRewardAddress(_ids, _rewardAddress, tokenIds);
    }

    function setEventRewardAmount(
        uint256[] calldata _ids,
        uint256[] calldata amounts
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_ids.length > 0, "must have an address");
        require(
            _ids.length == amounts.length,
            "must be equal length"
        );

        for (uint256 i = 0; i < _ids.length; i++) {
            require(amounts[i] > 0, "Amount invalid");
            eventRewardAmount[_ids[i]] = amounts[i];
        }

        emit SetEventRewardAmount(_ids, amounts);
    }
