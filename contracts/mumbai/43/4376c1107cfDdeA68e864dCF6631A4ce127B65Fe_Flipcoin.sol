...
contract Flipcoin {
    ...
    function joinLobby(uint256 lobbyId) external gasLimit(200000) {
        ...
        require(_amberToken.balanceOf(msg.sender) >= _bets[lobbyId][_player1], "Insufficient AMBER token balance");

        uint256 tokenAmount = _bets[lobbyId][_player1] + _bets[lobbyId][_player2];
        uint256 feeAmount = (tokenAmount * 10) / 100; // 10% fee
        ...
        emit BetPlaced(msg.sender, _bets[lobbyId][_player1] / (10**_tokenDecimals), lobbyId);
    }

    function endGame(uint256 lobbyId) external {
        ...
        uint256 remainingAmount = _totalPot - winningAmount;
        if (remainingAmount > 0) {
            require(_amberToken.transfer(_owner, remainingAmount), "Failed to transfer remaining AMBER tokens to the contract");
        }

        emit FundsWithdrawn(winner, winningAmount / (10**_tokenDecimals));
        emit FundsWithdrawn(_owner, remainingAmount / (10**_tokenDecimals));
