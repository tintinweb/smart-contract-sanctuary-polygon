...
contract GamespadLandTEST is ERC721Psi, AccessProtected, IERC721Receiver {
    ...
    function setCoordsToTokenId(uint256 _x, uint256 _y) external onlyAdmin {
        require(_x > 0 && _y > 0, "Invalid coordinate for x and y");
            _nftIds.increment();
            uint256 newNftId = _nftIds.current();
            uint256 coords = formatCoordinates(_x,_y);
