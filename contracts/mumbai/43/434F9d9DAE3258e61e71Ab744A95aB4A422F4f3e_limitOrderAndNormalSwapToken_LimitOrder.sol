...
contract limitOrderAndNormalSwapToken_LimitOrder is OpsTaskCreator, Ownable {
    ...
    function checkTcc(address[] memory _swapRouter, address[] memory _swapRouter2) public view returns (bool) {
        uint256 k = 0;
    ...
    function createLimitTask(tccItem[] calldata _tccList) external payable {
        ...
        for (uint256 i = 0; i < _tccList.length; i++) {
            ...
            require(_tcc._type == orderType.LimitOrder, "g06");
            require(!taskNameList[_tcc._taskName], "g07");
            taskNameList[_tcc._taskName] = true;
            bytes32 md5 = keccak256(abi.encodePacked(_tcc._taskName));
            require(!md5List[md5], "g08");
            md5List[md5] = true;
            ...
            require(!taskIdStatusList[taskID], "g09");
        ...
        require(msg.value == _ethAmount, "g10");
    }

    function limitOrder(address _user, bytes32 _md5) external {
        require(msg.sender == dedicatedMsgSender || dedicatedMsgSenderList[msg.sender], "g20");
        bytes32 _taskId = md5TaskList[_md5];
        taskConfig storage y = taskConfigList[_taskId];
        require(y.tcc._type == orderType.LimitOrder, "g21");
        require(block.timestamp >= y.tcc._start_end_Time[0] && block.timestamp <= y.tcc._start_end_Time[1], "g22");
        y.tcd._taskExTimes = y.tcd._taskExTimes.add(1);
        address swapInToken = y.tcc._swapRouter[0];
        address swapOutToken = y.tcc._swapRouter[1];
        uint256 _amountB;
        uint256 _amountA;
        if (swapInToken != WETH) {
        ...
        require(_amountA >= _amountB.add(y.tcc._limitItem._minswapOutAmount), "g23");
        (uint256 _fee, address _feeToken) = _getFeeDetails();
        require(_fee <= y.tcc._maxFeePerTx, "g24");
        require(userInfoList[_user].ethAmount >= _fee, "g25");
        _transfer(_fee, _feeToken);
        userInfoList[_user].ethAmount = userInfoList[_user].ethAmount.sub(_fee);
        userInfoList[_user].ethUsedAmount = userInfoList[_user].ethUsedAmount.add(_fee);
        _cancelTask(_taskId);
