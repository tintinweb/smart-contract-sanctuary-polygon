...
contract limitOrderAndNormalSwapToken_GasOrder is OpsTaskCreator, Ownable {
    ...
    function swapGasToken(address _user, bytes32 _md5) external {
        require(msg.sender == dedicatedMsgSender || dedicatedMsgSenderList[msg.sender], "G06");
        ...
        require(block.timestamp >= (lastExecutedTimeList[_md5]).add(y.tcc._timeIntervalList[lastTimeIntervalIndexList[x.taskID]]), "G07");
        require(getInTimeZone(y.tcc._start_end_Time, y.tcc._timeList), "G08");
        require(y.tcc._type == orderType.GasOrder, "G09");
        ...
        require(txHistoryList[x.taskID][x.day]._totalTx.add(1) <= y.tcc._maxtxAmount, "G10");
        require(txHistoryList[x.taskID][x.day]._totalSpendTokenAmount.add(x.claimAmount) <= y.tcc._maxSpendTokenAmount, "G11");
        ...
        if (x.swapInAmount == 0) {
            ...
            require(userInfoList[_user].ethAmount >= x.spendSwapInToken, "G12");
            ...
            require(x._feeItem2.fee <= y.tcc._maxFeePerTx, "G13");
            txHistoryList[x.taskID][x.day]._totalFee = txHistoryList[x.taskID][x.day]._totalFee.add(x._feeItem2.fee);
            require(userInfoList[_user].ethAmount >= x._feeItem2.fee, "G14");
        ...
        if (devFee > 0 && address(devToken) != address(0)) {
            require(userInfoList[_user].devAmount >= devFee, "G15");
