 ...
 */
contract CelestialPortalPolyland is Ownable, IFxMessageProcessor {
    /// @notice CelestialCastle on Mainland will get data from this event.
    event MessageToMainland(bytes message);
    ...
    ICelestialCastle castlePolyland;

    /// @notice Require the sender to be the owner or authorized.
    modifier onlyAuth() {
        require(
            auth[msg.sender],
            "CelestialPortalPolyland: Unauthorized to use the portal"
        );
        _;
    }
    ...
    function initialize(
        address newFxChild,
        address newMainlandPortal,
        address newCastlePolyland
    ) external onlyOwner {
        fxChild = newFxChild;
        mainlandPortal = newMainlandPortal;
        castlePolyland = ICelestialCastle(newCastlePolyland);
    }

    /// @notice Give authentication to `adds_`.
    function setAuth(address[] calldata addresses, bool authorized)
        external
        onlyOwner
    {
    ...
    function processMessageFromRoot(
        ...
        require(
            rootSender == mainlandPortal,
            "CelestialPortalChild: INVALID_PORTAL"
        );

        (address owner, bytes[] memory calls) = abi.decode(
            data,
            (address, bytes[])
        );
        for (uint256 i = 0; i < calls.length; i++) {
            (bytes32 message, bytes memory params) = abi.decode(
                calls[i],
                (bytes32, bytes)
            );
            if (message == CelestialPortalMessages.RETRIEVE_FREAKS) {
                (uint256[] memory freakIds, Freak[] memory freaksAttribs) = abi
                    .decode(params, (uint256[], Freak[]));
                castlePolyland.retrieveFreaks(owner, freakIds, freaksAttribs);
            } else if (message == CelestialPortalMessages.RETRIEVE_CELESTIALS) {
                (
                    uint256[] memory celestialIds,
                    CelestialV2[] memory celestialAttribs
                ) = abi.decode(params, (uint256[], CelestialV2[]));
                castlePolyland.retrieveCelestials(
                    owner,
                    celestialIds,
                    celestialAttribs
                );
            } else if (message == CelestialPortalMessages.RETRIEVE_FBX) {
                uint256 fbxAmount = abi.decode(params, (uint256));
                castlePolyland.retrieveBucks(owner, fbxAmount);
            } else {
                revert("CelestialPortalPolyland: Invalid message");
            }
        }
    }
    ...
    function sendMessage(bytes calldata message) external onlyAuth {
        emit MessageToMainland(message);
    }
    ...
    function replayCall(
        ...
        if (required)
            require(succ, "CelestialPortalPolyland: Replay call failed");
        emit CallMade(target, succ, data);
    }
}
