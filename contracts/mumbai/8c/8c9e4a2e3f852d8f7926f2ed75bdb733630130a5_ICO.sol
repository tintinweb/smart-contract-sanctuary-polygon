...
contract ICO is Ownable{
    ...
    // isAdded
    mapping(address=>bool)  private isAddedd;
    ...
    function buyMeta(uint amount, bool isMatic) external payable{
        ...
        require(!isAddedd[msg.sender],"User exist in Private");
    ...
    function addInvestors( user[] memory _user) external onlyOwner{
        for(uint i=0;i<_user.length;i++){
            ...
            isAddedd[_user[i]._userAddress]=true;
              userDetails memory __user;
    ...
    function getLinearBalance(address _userAddress)public view returns(uint){
        ...
        if(block.timestamp>=linearStarts[getUserDetails[_userAddress].saleType]){
            ...
            if(block.timestamp>=linearStarts[getUserDetails[_userAddress].saleType]+70 minutes){
                if (isclaimed[_userAddress])
                 funds = getUserDetails[_userAddress].allocatedAmount- getUserDetails[_userAddress].claimedAmount;
                 else
                 funds=getUserDetails[_userAddress].allocatedAmount-getUserDetails[_userAddress].initialAmount;
            }
            else{
     ...
     function withdraw() public {
         ...
         require(getUserDetails[msg.sender].saleType>0,"Not a Investor");
