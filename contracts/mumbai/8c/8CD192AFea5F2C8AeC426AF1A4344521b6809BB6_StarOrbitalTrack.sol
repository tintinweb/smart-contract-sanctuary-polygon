...
import "./interfaces/IStarOrbitalTrackLogic.sol";
    ...
    IERC721ReceiverUpgradeable
{
    ...
    IStarOrbitalTrackLogic public starOrbitalTrackLogicContract;
    ...
    mapping(address => bool) public approvedTokenAddresses;
    ...
    uint256 internal generalInviteGroupID;
    ...
    event ContractSettingUpdated(
        ...
        address _starOrbitalTrackLogicAddress,
    ...
    // update Approved Token Addresses
    event ApprovedTokenAddressesUpdated(
    ...
    function updateContractSetting(
        ...
        require(
            _starAddress.isContract() &&
                ...
                _starOrbitalTrackLogicAddress.isContract() &&
        ...
        starOrbitalTrackLogicContract = IStarOrbitalTrackLogic(
            _starOrbitalTrackLogicAddress
        );
     ...
     */
    function setApprovedTokenAddresses(address _token, bool _isApproved)
        external
        onlyOwner
    {
        ...
        approvedTokenAddresses[_token] = _isApproved;
        emit ApprovedTokenAddressesUpdated(_token, _isApproved);
    }
        ...
        isNFTOwner(planetContract, _planetId)
    {
        ...
        uint256 slotIndex = starOrbitalTrackLogicContract
            .getStarOwnerAttachSlotIndex(_starId, _planetId);
     ...
     * @param _tokenAddress rental token address
    ...
    function updateRentalSetting(
        ...
        // if useing isNFTOwner, will be Stack too deep
        _checkNFTOwner(starContract, _starId);

        // require token address is approved
        // todo if only update track, _tokenAddress can't be change
        require(approvedTokenAddresses[_tokenAddress], "invalid token");
        ...
        if (
            ...
            generalInviteGroupID++;
            _inviteGroupId = generalInviteGroupID;
        ...
        // do not need to check _duration if count = 0
        if (count > 0) {
    ...
    function rentingOrbitalTrack(
        ...
        // IStarOrbitalTrackLogic _starOrbitalTrackLogicContract = starOrbitalTrackLogicContract;

        // (bool isInvited, ) = _starOrbitalTrackLogicContract
        //     .isInvitedStarOrbitalTrack(
        //         msg.sender,
        //         _starId,
        //         orbitalTrackData.inviteGroupId
        //     );

        // // check slot can be rent
        // _starOrbitalTrackLogicContract.checkSlotCanBeRent(
        //     msg.sender,
        ...
        // _starOrbitalTrackLogicContract.checkReachMaxCount(
        //     msg.sender,
        //     _starId,
        //     orbitalTrackData.inviteGroupId,
        //     _planetId
        ...
        // check slot can be rent and check planet owner is reach max count or not
        starOrbitalTrackLogicContract.checkRentingOrbitalTrack(
            msg.sender,
            _starId,
            _planetId,
            address(starContract),
    ...
    function renewRentingOrbitalTrack(
        ...
        IApeironStarOrbitalTrackData _starOrbitalTrackDataContract = starOrbitalTrackDataContract;

        StarOrbitalTrackData
            memory orbitalTrackData = _starOrbitalTrackDataContract
                .getStarOrbitalTrackDataWithIndex(_starId, _slotIndex);
        ...
        // check planet owner is reach max count or not
        starOrbitalTrackLogicContract.checkReachMaxCount(
            msg.sender,
            _starId,
            orbitalTrackData.inviteGroupId,
            _planetId
        ...
        // add StarOrbitalTrackRentedData
        _starOrbitalTrackDataContract
        ...
        // update orbitalTrackData
        _starOrbitalTrackDataContract.updateStarOrbitalTrackData(
            _starId,
            _slotIndex,
            orbitalTrackData
        );
    ...
    function starOwnerDetachPlanet(
        ...
        // check orbitalTrackData and _slotIndex data
        require(
            (orbitalTrackData.rentingState == OrbitalTrackState.Renting ||
                orbitalTrackData.rentingState ==
                OrbitalTrackState.OwnRenting) &&
                orbitalTrackData.attachedPlanetId == _planetId,
            "invalid argument"
        );
        ...
        if (orbitalTrackData.planetOwnerAddress != msg.sender) {
            ...
            ) = starOrbitalTrackLogicContract.getStarOwnerDetachPlanetBalance(
                    _starId,
                    _planetId,
                    _slotIndex,
                    earlyDetachFeePercent,
    ...
    function planetOwnerDetachPlanet(
        ...
        (
            uint256 starOwnerReceive,
            uint256 planetOwnerReceive
        ) = starOrbitalTrackLogicContract.getPlanetOwnerDetachPlanetBalance(
                _starId,
                _planetId,
                _slotIndex,
                earlyDetachFeePercent,
    ...
    function _attachingPlanet(
        ...
        StarOrbitalTrackData
            memory orbitalTrackData = starOrbitalTrackDataContract
                .getStarOrbitalTrackDataWithIndex(_starId, _slotIndex);
        if (_rentalEndTime == 0) {
            orbitalTrackData.rentingState = OrbitalTrackState.OwnRenting;
        } else {
            orbitalTrackData.rentingState = OrbitalTrackState.Renting;
        }
    ...
    function _detachingPlanet(
        ...
        uint256 _planetCooldown;
        uint256 _obCooldown;
        bool _isStarOwnerDetachOwnPlanet = _isStarOwnerDetach &&
            _planetOwner == msg.sender;
        // if planet owner detach and _haveDetachCooldown, or _isStarOwnerDetachOwnPlanet
        if (
            (_haveDetachCooldown && !_isStarOwnerDetach) ||
            _isStarOwnerDetachOwnPlanet
        ) {
            _planetCooldown = block.timestamp + detachCooldown;
        ...
        if (
            ...
            _obCooldown = block.timestamp + detachCooldown;
        }
    ...
    function _tokenTransfer(
        ...
        if (_amount > 0) {
            IERC20Upgradeable(_tokenContract).safeTransfer(_toAddress, _amount);
        }
    }
    ...
    function _tokenTransferFromTo(
        ...
        IERC20Upgradeable(_tokenContract).safeTransferFrom(
            _fromAddress,
            _toAddress,
            _amount
        );
...
interface IApeironStarOrbitalTrackData {
    ...
    function checkNAddOrUpdateStarOrbitalTracksRentedData(
        uint256 _starId,
        uint256 _slotIndex,
        StarMeta.StarOrbitalTrackData memory _orbitalTrackData
    ) external;
...
interface IStarOrbitalTrackLogic {
    function checkSlotCanBeRent(
    ...
    function isInvitedStarOrbitalTrack(
        address _invitee,
        uint256 _starId,
        uint256 _inviteGroupId
    ) external view returns (bool, uint256);

    function getStarOwnerAttachSlotIndex(uint256 _starId, uint256 _planetId)
        external
        view
        returns (uint256);

    function checkRentingOrbitalTrack(
        address _planetOwnerAddress,
        uint256 _starId,
        uint256 _planetId,
        address _starContractAddress,
        StarMeta.StarOrbitalTrackData memory _orbitalTrackData
    ...
    function getStarOwnerDetachPlanetBalance(
        ...
        uint256 _earlyDetachFeePercent,
        uint256 _percentMaxValue
    ) external view returns (uint256, uint256 planetOwnerReceive);
