...
contract Gallery is Ownable, IGallery, Multicall {
	...
	function manageAirDrop(address to, uint256 tokenId) public onlyOwner {
		require(listOfTokenIds.contains(tokenId), 'Tokenid is not listed in this gallery');
		if (tokeninfo[tokenId].onSell) cancelNftSell(tokenId);
	...
	// 	string memory message = string.concat('ArtSwap Token has been  air dropped.The token id is');
	// 	airdrop.verificationCode = getHash(message, tokenId);
	// }
	...
	// function getHash(string memory _message, uint256 tokenId) private pure returns (bytes32) {
	// 	return keccak256(abi.encodePacked(_message, tokenId));
	// }
}