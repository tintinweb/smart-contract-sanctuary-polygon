...
contract HedgeXv3 {
    ...
    function makeOrder(
        ...
        if(expiryType==2){
            require(expiryValue>block.timestamp, "Can't pick an expiration date that has already passed.");
        }
        else{
            require(expiryValue > 100, "Expiry must be at least 100 seconds from the current time");
            // Valid lock-up periods: (1 Hour, 1 Day, 1 Month)
            require(
                (expiryValue == 101 ||
                    expiryValue == 86400 ||
    ...
    function settleOrder(
        ...
        ) = hdgx_orderbook.orderbook_settle_order(settleInfo, priceUpdateData, maker_leg_feed_ids);
        ...
        positionSwapRouter.external_settle_order_cleanup(maker.order_ID, taker.order_ID);
    ...
    function swap_for_drvs(uint256 amountIn) public payable returns(uint256 amountOut) {
...
contract Orderbook {
        ...
        )
    {
        ...
        (
            ...
            uint256 expiryValue,
            int64 ratio
        ) = abi.decode(maker_info, (address, uint256,uint256, uint256, int64));
        ...
        )
    {
        ...
        uint256 expiry_timestamp = makerOrder.expiryType==1?block.timestamp+makerOrder.expiryValue:makerOrder.expiryValue;
        ...
        )
    {
        ...
        PythStructs.PriceFeed[] memory historical_prices = pyth.parsePriceFeedUpdates{value:pyth_fee}(priceUpdateData,maker_leg_feed_ids,uint64(taker.expiry)-20, uint64(taker.expiry)+20);
        // pyth.updatePriceFeeds{value: pyth_fee}(priceUpdateData);
