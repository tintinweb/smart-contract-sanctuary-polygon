...
contract OLNameRegistry is Ownable, Seriality {
    ...
    event logFoo2(bytes32 key, Registry data);

    function existsRegistry(bytes32 nameHash) public returns (bool) {
        bytes memory buffer = olNameStore.getBytes(nameHash);
        if (buffer.length > 0) {
            Registry memory data = deserializeRegistry(buffer);
            emit logFoo2(nameHash, data);
        } else {
            emit logFoo(nameHash);
        }
    ...
    event logFoo6(uint256 bufferSize, bytes buffer);
    ...
    function serializeRegistry(Registry memory props) internal returns (bytes memory) {
        ...
        emit logFoo6(bufferSize, buffer);
        return buffer;
    }
