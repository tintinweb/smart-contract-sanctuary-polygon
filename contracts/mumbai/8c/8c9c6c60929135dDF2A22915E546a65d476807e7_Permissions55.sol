    ...
    PermissionErrors
{
    ...
    modifier onlyDeployer() {
        require(balanceOf(_msgSender(), TOKEN_ROLE_ADMIN) > 0 || balanceOf(_msgSender(), TOKEN_ROLE_DEPLOYER) > 0,
        ...
        returns (bool success, uint256 requiredPermission)
    {
        ...
        if (
            ...
            return (balanceOf(account, TOKEN_ROLE_ADMIN) > 0 || balanceOf(account, TOKEN_ROLE_ADMIN + 1000 * permissionSetId) > 0, TOKEN_ROLE_ADMIN); 
            
        }

        if (roleId == TOKEN_ROLE_IS_WHITELISTED) {
            return (balanceOf(account, TOKEN_ROLE_WHITELIST_ADMIN) > 0 || balanceOf(account, TOKEN_ROLE_WHITELIST_ADMIN + 1000 * permissionSetId) > 0, TOKEN_ROLE_WHITELIST_ADMIN);
        }

        if (roleId == TOKEN_ROLE_IS_BLACKLISTED) {
            return (balanceOf(account, TOKEN_ROLE_BLACKLIST_ADMIN) > 0 || balanceOf(account, TOKEN_ROLE_BLACKLIST_ADMIN + 1000 * permissionSetId) > 0, TOKEN_ROLE_BLACKLIST_ADMIN);
        }
    ...
    function isAdmin(address account, uint256 permissionSetId) public view returns (bool) {
        return balanceOf(account, TOKEN_ROLE_ADMIN) > 0 || balanceOf(account, TOKEN_ROLE_ADMIN + permissionSetId * 1000) > 0;
    }
