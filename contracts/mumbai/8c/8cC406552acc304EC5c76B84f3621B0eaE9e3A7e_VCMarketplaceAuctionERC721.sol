...
interface IVCMarketplaceAuctionERC721 {
    error MktAlreadyListedError();
    error MktExistingBidError();
    error MktBidderNotAllowedError();
    error MktBidTooLateError();
    error MktBidTooLowError();
    error MktSettleTooEarlyError();
    ...
    event MktUnlistedAuction(address indexed seller, address indexed token, uint256 indexed tokenId);
    event MktBid(
     ...
     * @param _tokenId the token identifier
     */
    function unlistAuction(uint256 _tokenId) external;
...
import "./IVCMarketplaceAuctionERC721.sol";

contract VCMarketplaceAuctionERC721 is IVCMarketplaceAuctionERC721, VCMarketplaceBase, ERC721Holder {
    ...
    /**
     * @dev See {IVCMarketplaceAuctionERC721-listAuction}.
     */
    function listAuction(
        uint256 _tokenId,
        uint256 _initialPrice,
        ...
        IVCMarketManager.ListStatus status = IVCMarketManager(marketManager).getListStatusERC721(_tokenId);
        if (status == IVCMarketManager.ListStatus.FIXED_PRICE) {
            revert MktAlreadyListedOnFixedPriceError();
        } else if (status == IVCMarketManager.ListStatus.AUCTION) {
            revert MktAlreadyListedOnAuctionError();
        }
        ...
        _newList(ListingData(_tokenId, _initialPrice, _projects, _projectFeesBps), maturity);
    }

    /**
     * @dev See {IVCMarketplaceAuctionERC721-unlistAuction}.
     */
    function unlistAuction(uint256 _tokenId) public {
        ...
        if (listing.minted) {
            IVCArtNftERC721(artNft).transferFrom(address(this), listing.seller, _tokenId);
        }

        emit MktUnlistedAuction(msg.sender, artNft, _tokenId);
    }

    /**
     * @dev See {IVCMarketplaceAuctionERC721-bid}.
     */
     ...
     * @param _marketplaceFeeBps basis points fee for marketplace
        ...
        returns (uint256 marketFee, uint256 starterFee, uint256 poolFee, address royaltyReceiver, uint256 royaltyAmount)
    {
        ...
        (royaltyReceiver, royaltyAmount) = IERC2981(artNft).royaltyInfo(_tokenId, _listPrice);
    }
     ...
     * FIXME: Params
     */
    function _newList(ListingData memory data, uint256 _maturity) internal {
        bool minted = IVCArtNftERC721(artNft).exists(data.tokenId);
        if (!minted) {
            IVCArtNftERC721(artNft).requireCanRequestMint(msg.sender, data.tokenId);
        } else {
            IVCArtNftERC721(artNft).transferFrom(msg.sender, address(this), data.tokenId);
        }
        ...
        (
            ...
            address royaltyReceiver,
            uint256 royaltyAmount
        ) = _getFeesAndRoyalty(data.tokenId, msg.sender, data.price, marketplaceFeeBps);

        auctionListings[data.tokenId] = AuctionListing(
            minted,
            msg.sender,
            data.price,
            _maturity,
        ...
        IVCMarketManager(admin).setListStatusERC721(data.tokenId, true);

        emit MktListedAuction(
            artNft,
            data.tokenId,
            msg.sender,
            _maturity,
    ...
    function _minting(AuctionListing memory listing, uint256 _tokenId) internal {
        if (!listing.minted) {
            IVCArtNftERC721(artNft).mintTo(_tokenId, address(this));
            auctionListings[_tokenId].minted = true;
        }
        IVCArtNftERC721(artNft).transferFrom(address(this), listing.highestBidder, _tokenId);

        pocNft.mint(listing.highestBidder, listing.marketFee, true);
    ...
    /**
     * @dev validates auction listing before process a bid
     * @param listing listing information for the token
