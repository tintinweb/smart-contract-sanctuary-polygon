...
contract MarketPlace is ERC165 {
    ...
    function buy(
        ...
        require(
            payment(_order, msg.sender, amount, xrAmount, (_order.startPrice * copies)),
            "Payment failed"
        );
        ERC1155Interface.safeTransferFrom(
            address(this),
    ...
    function buyDutchAuction(
        ...
        require(
            (totalAmount + (((platformFees+xrDynamicRoyalty) * totalAmount) / 100)) <= tokenAmount,
            "Insufficient funds"
        );
        ...
        require(
            payment(
                ...
                (totalAmount + ((xrDynamicRoyalty * totalAmount) / 100)),
                totalAmount
            ),
            "Payment failed"
        );
    ...
    function payment(
        ...
        uint256 xrFees = amount - xrAmount;
        uint256 royaltyFees = amount - initialAmount;
        //uint256 buyerFees = amount - initialAmount;
        //uint256 sellerFees = amount - initialAmount;
        ...
        if (_buyerSNFTAmount > 0) {
            amount -= royaltyFees/2;
        }
        
        sendValue(msg.sender, Treasury, royaltyFees/2, _order.paymentToken);
        ...
        if (_sellerSNFTAmount > 0) {
            royaltyFees = royaltyFees / 2;
        }

        sendValue(msg.sender, Treasury, royaltyFees, _order.paymentToken);

        amount = amount - royaltyFees;
        ...
        if(xrDynamicRoyalty > 0) {
            amount -= xrFees;
            sendValue(msg.sender, xrDynamicAddress, xrFees, _order.paymentToken);
        }
