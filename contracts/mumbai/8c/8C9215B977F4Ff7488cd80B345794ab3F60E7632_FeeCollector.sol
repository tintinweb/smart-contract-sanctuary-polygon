...
contract FeeCollector is AccessControlEnumerable {
    ...
    constructor(
        ...
        distribution[0] = 80 * PERCENT_PRECISION; // 80%
        distribution[1] = 20 * PERCENT_PRECISION; // 20%
        stakingPool = 70 * PERCENT_PRECISION; // 70%
    ...
    function setStakingpool(uint256 ratio) external onlyAdmin {
        require(ratio > 0 , "zero value");
        stakingPool = ratio * PERCENT_PRECISION; 
    }
    function setValidator(address _validator) external onlyAdmin {
        validatorAddress = _validator;
    }
    ...
    function claim(address token) external {
        ...
        require(totalAmount > 0, "nothing to be claimed");
        uint256 amount = totalAmount.mul(stakingPool).div(
                PERCENT_PRECISION.mul(100)
        );
        IStaking(staking).deposit(validatorAddress, amount);
        totalAmount = totalAmount - amount;
        require(totalRatio() == 100e18, "invalid distribution");
        for (uint256 i; i < wallets.length; i++) {
            amount = totalAmount.mul(distribution[i]).div(
                PERCENT_PRECISION.mul(100)
            );
