...
contract ADM {
    ...
    function _calLevelNow(address _user) private view returns(uint256) {
        ...
        if(total >= 2000e6 && user.teamNum >= 200 && maxTeam >= 70000e6 && otherTeam >= 70000e6){
             ...
             levelNow = 4;
        }else if(user.teamNum >= 20 && maxTeam >= 10000e6 && otherTeam >= 10000e6){
             levelNow = 3;
        }
    ...
    function getRewardInfos(address _address, uint256 _type) public view returns (uint256, uint256, uint256, uint256, uint256, uint256){
     ...
     if(_type == 1){
         return (rewardInfo[_address].capitals, rewardInfo[_address].statics, rewardInfo[_address].directs, rewardInfo[_address].level4Freezed , rewardInfo[_address].level4Released, 0);
      }
      else{
