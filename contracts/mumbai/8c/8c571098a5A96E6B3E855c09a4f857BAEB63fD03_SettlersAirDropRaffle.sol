...
contract SettlersAirDropRaffle is ERC1155, ERC1155Supply, FoundingSettlersList {
    ...
    modifier isMint(uint _tokenID, uint _newSupply) {
    ...
    constructor() ERC1155("......INSERT IPFS JSON FILE URI HERE........") {
    ...
    *           obvious candidate seeds such as 0,1,10,22,etc. being used.
    ...
    */
    ...
    public onlyGovernor isMint(tokenID, addresses.length * amountPerRecipient) {
        for (uint _i = 1; _i <= addresses.length; _i++) {
            _mint(addresses.list[_i], tokenID, amountPerRecipient, "");
        }
    }

    function mintByRaffle(uint tokenID, uint amountPerWinner, uint numberOfWinners, uint randomSeed) 
    public onlyGovernor isMint(tokenID, amountPerWinner * numberOfWinners) { 
        ...
        for (uint _i = 0; _i < numberOfWinners; _i++) {
            _mint(addresses.list[_pseudoRandomNumbers[_i]], tokenID, amountPerWinner, "");
        }
        _resetRandomNumbers(numberOfWinners);
    }

    function mintToExtrasHolder(uint tokenID, uint amount) public onlyGovernor isMint(tokenID, amount) {
        _mint(extrasHolder, tokenID, amount, "");
    }
