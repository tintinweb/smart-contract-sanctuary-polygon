...
contract USDRExchange is AddressAccessor, IExchange, Pausable {
    ...
    function updateMintingStats(int128[7] calldata delta) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        mintingStats.usdrFromGains = uint256(int256(mintingStats.usdrFromGains) + delta[5]);
        mintingStats.usdrFromRebase = uint256(int256(mintingStats.usdrFromRebase) + delta[6]);
    }
        ...
        )
    {
        ...
        require(
            stable + rwa + liquidity + tngblLiquidity - debt <
