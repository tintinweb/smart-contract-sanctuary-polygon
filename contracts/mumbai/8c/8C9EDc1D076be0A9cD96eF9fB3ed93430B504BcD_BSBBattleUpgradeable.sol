...
contract BSBBattleUpgradeable is Initializable, BSBBaseGameModeUpgradeable, VRFConsumerBaseUpgradeable {
    ...
    function __BSBBattle_init(
        ...
        metaPassYieldMultiplierPercentage = 20;
        baseRisk = 50;
    ...
    function disEnroll(CollectionItems[] memory collectionItems_) external whenNotPaused {
        ...
        BSBBloodTokenCaller.mint(_msgSender(), accumulatedYield_);
    ...
    function claimRewards(CollectionItems[] memory collectionItems_) public override whenNotPaused {
        ...
        BSBBloodTokenCaller.mint(_msgSender(), accumulatedYield);
    }
    ...
    function calculateReward(address collection_, uint tokenId_) public view returns (uint, uint) {
        ...
        uint daysLifetime_ = _getMaxDaysLifetimeUntilBlock(collection_, tokenId_, bearStats.stamina, applyBuff_);

        return (
            _calculateLevelReward(daysLifetime_, bearStats.level),
            _calculateYieldReward(daysLifetime_, collectionsYield[collection_], bearStats.level.value, applyBuff_)
        );
    }
    ...
    function calculateStaminaDecreaseRate() external view returns (uint) {
        return _calculateActualDecreaseRate(
            BSBMintPassBuffCaller.getBuffBalance(_msgSender()) != uint(0)
        );
    }

    function calculateDailyYield(address collection_, uint tokenId_) public view returns (uint) {
        bool applyBuff = BSBMintPassBuffCaller.getBuffBalance(_msgSender()) != uint(0);
        BearStats memory bearStats = BSBStorageCaller.getBearStats(collection_, tokenId_);
    ...
    function _calculateYieldReward(
        ...
        return daysLifetime_ * _calculateDailyYieldWithBonus(yield_, level_, applyBuff_);
    }
    ...
    function _applyRisk(
        ...
        if (_pickPortionOutOfAmount(risk_, tokenId_, amount_)) {
            ...
            if (winner != address(0)) {
                ...
                BSBBloodTokenCaller.mint(winner, amount_);
    ...
    function _removeFromElitesAndUnstackIfNecessary(CollectionItems memory collectionItem_) internal {
        if (collectionItem_.collection == contracts["gen0"] || collectionItem_.collection == contracts["gen1"]) {
            ...
            for (uint i = 0; i < collectionItem_.assets.length; ++i) {
                ...
                if (bearsStats[i].isElite) {
                    _removeEliteFromPool(
                        collectionItem_.collection, collectionItem_.assets[i], bearsStats[i].faction
                    );
                }
            }
    ...
    function _claimRewards(CollectionItems memory collectionItems_, bool applyBuff_) internal returns (uint) {
        ...
        for (uint i = 0; i < collectionItems_.assets.length; ++i) {
            ...
            uint yieldRewardAfterRisk_ = _applyRisk(
                collectionItems_.collection,
                collectionItems_.assets[i],
...
//    }
    ...
    function encodeCollectionAsset(address collection_, uint256 tokenId_) public pure returns(bytes32) {
        return keccak256(abi.encodePacked(collection_, tokenId_));
    }
...
contract BSBBaseUpgradeable is Initializable, OwnableUpgradeable, PausableUpgradeable {
    ...
    uint256 constant public DAYS = 86400; // seconds
//    uint256 constant public DAYS = 1; // seconds
