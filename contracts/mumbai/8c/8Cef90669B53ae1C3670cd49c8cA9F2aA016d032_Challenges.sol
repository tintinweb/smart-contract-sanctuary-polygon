    ...
    SerializeBuyNowGet
{
    ...
    address private immutable _verifyChainId;
    ...
    function challVerifiableOnChain(
        ...
        require(VerifyChainId(_verifyChainId).hasCorrectChainIdx(assetId), "assetId belongs to a different chainIdx");
        bool isTXValid;
        require(
...
contract VerifyChainId is EncodingAssets, ChainId, IVerifyChainId {
    ...
    function hasCorrectChainIdx(uint256 assetId) public view returns (bool) {
        return decodeChainIdx(assetId) == getChainIdx();
    }
}
...
contract ChainId is IChainId {
    ...
    uint256 constant private POLYGON_MAINNET = 137;
    uint256 constant private POLYGON_MUMBAI = 80001;
    uint256 constant private HEDERA_MAINNET = 295;
    uint256 constant private HEDERA_DEVNET = 298;
    uint256 constant private TESTING = 1337;

    /// @inheritdoc IChainId
    function getChainIdx()
        public
        view
        returns (uint8 chainIdx)
    {
        ...
        // mainnet checks first, to save gas
        if (localChainId == POLYGON_MAINNET) return 0;
        if (localChainId == HEDERA_MAINNET) return 1;

        // revert unless supported testnet
        if (localChainId == POLYGON_MUMBAI || localChainId == TESTING) {
            return 0;
        } 
        if (localChainId > HEDERA_MAINNET && localChainId <= HEDERA_DEVNET) {
            return 1;
        }
