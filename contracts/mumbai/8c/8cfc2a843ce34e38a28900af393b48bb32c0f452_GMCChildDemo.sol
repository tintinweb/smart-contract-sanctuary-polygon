...
contract GMCChildDemo is UUPSUpgrade, OwnableUDS, FxERC721EnumerableChild, GMCMarket {
    ...
    function mint(uint256 gang, bytes calldata signature) external {
        ...
        if (!validSignature(signature, gang)) revert InvalidSignature();
    ...
    function validSignature(bytes calldata signature, uint256 gang) private view returns (bool) {
        bytes32 hash = keccak256(abi.encode(address(this), msg.sender, gang));
