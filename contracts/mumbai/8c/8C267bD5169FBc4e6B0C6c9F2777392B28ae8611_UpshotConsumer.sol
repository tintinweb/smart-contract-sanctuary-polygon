 ...
 */
contract UpshotConsumer is ChainlinkClient, Ownable {
    ...
    modifier onlyPriceFeed() {
        require(_msgSender() == priceFeed, "caller != priceFeed");
        _;
    }
     ...
     */
    function requestPrice(uint256 _assetId) public returns (bytes32 requestId) {
    ...
    function setSpecId(bytes32 _specId) external onlyOwner {
        specId = _specId;
    }
    ...
    function withdrawTokens(IERC20 _token, address _beneficiary) public onlyOwner {
        require(address(_token) != getLinkAddress(), "Cannot withdraw LINK");
