...
contract Swap is ISwap, Ownable {
    ...
    Listing public publicListing;
    SwapOffer public publicSwapOffer;
    CollectionOffer public publicCollectionOffer;

    Assets publicAssets ;
    SwapAssets publicSwapAssets ;
    ReserveInfo publicReserveInfo;
    Royalty publicRoyalty;

    bytes public publicSignature;

    bytes32[][] public publicProofs;
    ...
    constructor() {
        ...
        asset.tokens[0] = 0xfc0303DBA3D1B2ce6f72c3dF2346B1F9f0265941;
        asset.tokens[1] = 0x8981a87033Bc2344f2704Ce3F8f0c5d1Bf009DfB;
        asset.tokenIds[0] = 3;
        asset.tokenIds[1] = 4;
        asset.paymentTokens[0] = 0xde1bEA4Af8a3c12665350D8077a4f68c4D60f392;
        asset.paymentTokens[1] = 0xD93580c12564384B81b4309B247FBea9bDc45F91;
        asset.amounts[0] = 555;
        asset.amounts[1] = 666;

        publicAssets = asset;

        publicListing.listingAssets.tokens.push(0xc0BfAc0a4ee08e2D58F5C52064a5EE92f45E7b17);
        publicListing.listingAssets.tokens.push(0x6AE93A1Ad00B9Fb5eB42Fd8f53eB4FBE37b16C8d);
        publicListing.listingAssets.tokenIds.push(1);
        publicListing.listingAssets.tokenIds.push(2);
        publicListing.listingAssets.paymentTokens.push(0x83fD531ADe3e2e34A7C94E878E0AD3263Bf28Cdf);
        publicListing.listingAssets.paymentTokens.push(0x6eA4eC30Fd0bD5FE3b82f6461fC35878450C8049);
        publicListing.listingAssets.amounts.push(111);
        publicListing.listingAssets.amounts.push(222);

        SwapAssets memory ds  = SwapAssets({
            tokens: new address[](2),
            roots: new bytes32[](2),
            paymentTokens: new address[](2),
            amounts: new uint256[](2)
        ...
        ds.tokens[0] = 0xD2206Bb68aEf871085097914dC5E789B1886c62D;
        ds.tokens[1] = 0xD2206Bb68aEf871085097914dC5E789B1886c62D;
        ds.roots[0] = "0x123";
        ds.roots[1] = "0x234";
        ds.paymentTokens[0] = 0x23C8C8A9b57ACe73c7604805c1bBDC47aac83ed8;
        ds.paymentTokens[1] = 0xF0214FdF38aBf297eCdE402392BD93b44FB5F5EF;
        ds.amounts[0] = 333;
        ds.amounts[1] = 444;

        publicSwapAssets = ds;

        publicListing.directSwaps.push(ds);
        publicListing.directSwaps.push(ds);

        ReserveInfo memory ri = ReserveInfo({
            deposit: asset,
            remaining: asset,
            duration: 345
        });

        publicListing.reserves.push(ri);
        publicListing.reserves.push(ri);

        publicReserveInfo = ri;

        Royalty memory ry = Royalty({
            to: new address[](2),
            percentage: new uint256[](2)
        });

        ry.to[0] = 0x1C2B17108838788cEe976E3B60BB576c987FFAa9;
        ry.to[1] = 0x328a0190904202fC9D0871d53c2E62111A6846e1;
        ry.percentage[0] = 45;
        ry.percentage[1] = 56;

        publicRoyalty = ry;

        publicListing.royalty = ry;
        publicListing.tradeIntendedFor = 0x5fD8C5340E3b123b4AD12Bb137796BD038CFA527;
        publicListing.timePeriod = 87;
        publicListing.owner = 0x38763411f6699B36d911caA6e74a3C7aA2030C94;
        publicListing.nonce = 98;

        publicSignature = "0x98765434312";
        
        publicProofs.push(new bytes32[](2));
        publicProofs.push(new bytes32[](2));
        publicProofs[0][0] = "0x123";
        publicProofs[0][1] = "0x123";
        publicProofs[1][0] = "0x123";
        publicProofs[1][1] = "0x123";

        publicCollectionOffer.offeringItems = asset;
        publicCollectionOffer.considerationItems = ds;
        publicCollectionOffer.royalty = ry;
        publicCollectionOffer.timePeriod = 456;
        publicCollectionOffer.owner = 0x265fe3c0B1a8974E62680590E9B1174df0D2B08B;
        publicCollectionOffer.nonce = 76;

        publicSwapOffer.offeringItems = asset;
        publicSwapOffer.considerationRoot = "0x765";
        publicSwapOffer.royalty = ry;
        publicSwapOffer.timePeriod = 4561;
        publicSwapOffer.owner = 0x8DdF91D17230262227D85bc8658ff03c08D486D9;
        publicSwapOffer.nonce = 762;

    }

    function emitAllEvent() external {
        
        emit DirectSwapped(publicListing, publicAssets, msg.sender);
        emit UnlistedSwapOfferAccepted(publicSwapOffer, publicAssets, msg.sender);
        emit ListedSwapOfferAccepted(publicListing, publicSwapOffer, msg.sender);
        emit CollectionOfferAccepted(publicCollectionOffer, publicAssets, msg.sender);
        emit ListingCancelled(publicListing);
        emit SwapOfferCancelled(publicSwapOffer);
        emit CollectionOfferCancelled(publicCollectionOffer);
