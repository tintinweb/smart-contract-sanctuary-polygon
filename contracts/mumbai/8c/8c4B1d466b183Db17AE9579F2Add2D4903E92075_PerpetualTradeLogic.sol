...
contract PerpetualTradeLogic is PerpetualUpdateFunctions, IFunctionList, IPerpetualTradeLogic {
    ...
    function preTrade(IPerpetualOrder.Order memory _order)
        external
        virtual
        override
        returns (int128 fPrice, int128 fAmount)
    {
        require(msg.sender == address(this), "onlythis");
     ...
     * No referrer and broker is involved, only exchange fees.
    ...
    function _executeTrade(
        ...
        int128 fIndexS2 = _getOraclePrice([_perpetual.S2BaseCCY, _perpetual.S2QuoteCCY]);
        require(fIndexS2 > 0, "market is closed");
        ...
        // update fund allocation weight after AMM margin updates
        _updateFundAllocationWeight(_perpetual);
        if (!_isClose) {
    ...
    /**
     * The PnL participant cash is allocated to the different perpetuals via a weight
     * _perpetual.fFundAllocationWeightCC.div(_pool.fFundAllocationNormalizationCC)
     * This weight depends on `how much the perpetual is traded` measured by
     * open interest, and penalized by imbalances.
     * Updates _perpetual.fFundAllocationWeightCC and the corresponding pool value.
     * fFundAllocationWeightCC
     * @param _perpetual    reference to perpetual
     */
    function _updateFundAllocationWeight(PerpetualData storage _perpetual) internal {
        // P = S - L, OI = max(L, S) (K2 = -P)
        // P < 0 => L = OI, S = OI + P; P > 0 => S = OI & L = OI - P
        // => one is OI, the other OI - |P|

        // int128 fOpenInterestSmallerSide = _perpetual.fOpenInterest.sub(
        //     marginAccounts[_perpetual.id][address(this)].fPositionBC.abs()
        // );

        // // geometric mean:
        // // penalize imbalanced perps with less capital
        // int128 fWeight = (_perpetual
        //     .fOpenInterest
        //     .add(_perpetual.fMinimalTraderExposureEMA)
        //     .log_2()
        //     .add(fOpenInterestSmallerSide.add(_perpetual.fMinimalTraderExposureEMA).log_2()) >> 1)
        //     .exp_2();

        int128 fX = _perpetual.fOpenInterest + _perpetual.fMinimalTraderExposureEMA;
        int128 fY = fX - (marginAccounts[_perpetual.id][address(this)].fPositionBC).abs();

        // geometric mean:
        // penalize imbalanced perps with lower weights
        int128 fWeight = ((fX.log_2() + fY.log_2()) / 2).exp_2();

        // convert to CC
        fWeight = fWeight.mul(_getBaseToCollateralConversionMultiplier(_perpetual, false));
    ...
    function _preTrade(
        ...
        if (fNewPosSizeInLots < MIN_NUM_LOTS_PER_POSITION) {
            if (closePos) {
        ...
        bool isPriceSatisfied = _validatePrice(_fAmount >= 0, fPrice, _fLimitPrice);
        if (!isPriceSatisfied) {
     ...
     */
    function _getMaxSignedPositionSize(PerpetualData storage _perpetual, bool isLong)
        internal
        view
        returns (int128)
    {
        int128 fPosSize = _perpetual.fCurrentTraderExposureEMA;
        require(fPosSize > 0, "fCurrentTraderExposureEMA>0");
        int128 scale = int128(_perpetual.fMaximalTradeSizeBumpUp) << 35;
        ...
        if ((isLong && fkStar < 0) || (!isLong && fkStar > 0)) {
            ...
            // int128 fundingRatio = liqPool.fDefaultFundCashCC.add(liqPool.fAMMFundCashCC).div(
            //     liqPool.fTargetDFSize.add(liqPool.fTargetAMMFundSize)
            // );
        ...
        // maxAbs = emwaTraderK*bumpUp or reduced bumpUp
        fPosSize = isLong ? fPosSize.mul(scale) : fPosSize.mul(scale).neg();
        return fPosSize;
    }
     ...
     * @return maxSignedTradeAmount signed maximal trade size (negative if resulting position is short, positive otherwise)
     */
    function getMaxSignedOpenTradeSizeForPos(
        ...
        int128 fMaxPos = _getMaxSignedPositionSize(perpetual, _isBuy);
        ...
        if ((_isBuy && maxSignedTradeAmount < 0) || (!_isBuy && maxSignedTradeAmount > 0)) {
            ...
            int128 fkStar = perpetual.fkStar;
     ...
     */
    function _updateAverageTradeExposures(PerpetualData storage _perpetual, int128 _fTraderPos)
        internal
    {
        int128 fCurrentObs;
    ...
    function _transferBrokerFee(
        ...
        _transferFromVaultToUser(_liqPool.marginTokenAddress, _brokerAddr, _fBrokerFeeCC);
        emit TransferFeeToBroker(_perpetual.id, _brokerAddr, _fBrokerFeeCC);
    }

    /**
     * Transfer the specified fee amounts to the stakeholders
     * Referrer gets margin token (no choice of other tokens)
    ...
    function _transferProtocolFee(
        ...
        _liqPool.fAMMFundCashCC = _liqPool.fAMMFundCashCC.add(_fTreasuryFee);
        _perpetual.fAMMFundCashCC = _perpetual.fAMMFundCashCC.add(_fTreasuryFee);
     ...
     * @return  _fPrice the price for the queried amount
     */
    function _queryPriceFromAMM(PerpetualData storage _perpetual, int128 _fTradeAmount)
        internal
        view
        returns (int128 _fPrice)
    {
        AMMPerpLogic.AMMVariables memory ammState;
        AMMPerpLogic.MarketVariables memory marketState;
    ...
    function _queryPriceGivenAMMAndMarketData(
        ...
        if (_fTradeAmount != 0) {
            fCurrSpread = _calculateVolatilitySpread(_perpetual);
     ...
     */
    function _shrinkToMaxPositionToClose(int128 _fPosition, int128 _fAmount)
        internal
        pure
        returns (int128)
    {
     ...
     * @param   _traderAddr         The address of trader.
     * @param   _referrerAddr       The address of referrer who will get rebate from the deal.
     * @param   _brokerAddr         The address of referrer who will get rebate from the deal.
    ...
    function _distributeFees(
        ...
        // send fee to broker
        _transferBrokerFee(liqPool, _perpetual, _brokerAddr, fBrokerFee);
     ...
     * @param   _hasBroker          true if broker address specified
        ...
        )
    {
        ...
        if (!_hasOpened) {
            ...
            if (fAvailableMargin <= 0) {
                ...
                fPnLparticipantFee.add(fTreasuryFee).add(fReferralRebate).add(fBrokerFee) >
                fAvailableMargin
            ) {
                // make sure the sum of fees = available margin
                int128 fRate = fAvailableMargin.div(
                    fPnLparticipantFee.add(fTreasuryFee).add(fReferralRebate).add(fBrokerFee)
                );
                fTreasuryFee = fTreasuryFee.mul(fRate);
                fReferralRebate = fReferralRebate.mul(fRate);
                fBrokerFee = fBrokerFee.mul(fRate);
                fPnLparticipantFee = fAvailableMargin.sub(fTreasuryFee).sub(fReferralRebate).sub(
                    fBrokerFee
                );
            }
        } else {
     ...
     * @param _hasBroker true if the trade was submitted via broker
     * @param _hasOpened opening position?
     * @return fTreasuryFee fee that goes to protocol
     * @return fPnLparticipantFee fee that goes to pnl participants
     * @return fReferralRebate fixed fee for referrer
    ...
    {
        MarginAccount storage mgn = marginAccounts[_perpId][_trader];

        fBrokerFee = !_hasBroker
            ? int128(0)
            : ConverterDec18.tbpsToABDK(mgn.brokerFeeTbps).mul(_fDeltaPosCC);
        uint16 penaltyTbps = 0;
        if (!_hasOpened) {
            // closing position: if holding period not satisfied a penalty is added
            penaltyTbps = _getAMMPerpLogic().holdingPeriodPenalty(
                block.timestamp - mgn.iLastOpenTimestamp,
                LAMBDA_SPREAD_CONVERGENCE
            );
        }
        (fTreasuryFee, fPnLparticipantFee) = _getBrokerFeeLogic().splitProtocolFee(
            mgn.feeTbps + penaltyTbps
        );
        (fTreasuryFee, fPnLparticipantFee, fReferralRebate) = _getAMMPerpLogic()
            .relativeFeeToCCAmount(
                _fDeltaPosCC,
                fTreasuryFee,
                fPnLparticipantFee,
                _fReferralRebate,
