...
contract Box is Ownable {
    ...
    function getAllFile() public view allowedUser returns (File[] memory) {
    ...
    function getUsedStorage() public view onlyOwner returns (uint256) {
        return usedStorage;
    }

    // Function to get total available storage of box
    function getTotalStorage() public view onlyOwner returns (uint256) {
        return totalStorage;
    }
    ...
    function requestAccept(address user) public onlyOwner {
    ...
    function requestAutoAccept(address user) public callbyFactoryContract {
        allowedUsers[user] = true;
        isRequested[user].isRequested = false;
    }

    // Function to Reject Request
    function requestReject(address user) public callbyFactoryContract {
        require(isRequested[user].isRequested, "user not Request for Access");
        isRequested[user].isRequested = false;
    }
