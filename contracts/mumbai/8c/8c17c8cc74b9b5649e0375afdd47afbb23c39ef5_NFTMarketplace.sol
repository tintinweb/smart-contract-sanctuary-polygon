...
contract NFTMarketplace {
    ...
    function createListing(address nftContract, uint256 tokenId, uint256 price, ListingType listingType) external {
        ...
        listings[nftContract][tokenId] = Listing(tokenId, msg.sender, price, true, address(0), 0, listingType);
    }
    ...
    function buyDirect(address nftContract, uint256 tokenId) external payable onlyActiveListing(nftContract, tokenId) {
        Listing storage listing = listings[nftContract][tokenId];
        require(msg.sender != listing.seller, "Seller cannot buy their own NFT");
        require(msg.value >= listing.price, "Insufficient funds to buy the NFT");
        require(listing.listingType == ListingType.DirectSale, "This listing is not a direct sale");
