...
contract MITTechnologyReview is DefaultOperatorFilterer, ERC721A, Ownable {
    ...
    uint256 public priceDiscount = 0.1 ether;
    ...
    function mintWaitlist(uint256 _mintAmount, address payable _endUser, string memory _cid, bytes32[] memory _proof) public payable {
        require(!paused, "Contract paused");
        require(_mintAmount > 0, "Need to mint at least 1 NFT");
        require(phase() == 0, "mintWaitlist closed");
        require(count + _mintAmount <= maxSupply, "NFT limit amount exceeded");
        require(msg.value >= priceDiscount * _mintAmount, "Minting value different from the value defined in the contract");
        require(!whitelistClaimed[_endUser], "Address already claimed");
        require(isValid(_proof, keccak256(abi.encodePacked(_endUser))), "Not part of the waitlist");
