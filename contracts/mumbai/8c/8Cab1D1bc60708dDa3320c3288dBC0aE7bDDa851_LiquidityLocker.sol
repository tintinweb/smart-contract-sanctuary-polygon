...
// https://etherscan.io/address/0x663a5c229c09b049e36dcc11a9b0d4a8eb9db214#code

contract LiquidityLocker is ILiquidityLocker, Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    struct UserInfo {
        EnumerableSetUpgradeable.AddressSet lockedTokens; // records all unique tokens the user has locked
        mapping(address => EnumerableSetUpgradeable.UintSet) locksForToken; // map erc20 address to lock id list for that user / token.
    }
    ...
    EnumerableSetUpgradeable.AddressSet private lockedTokens; // a list of all unique locked liquidity tokens
    ...
    address payable devaddr; // recipient fee
    ...
    function initialize(IUniFactory _uniswapFactory, ICountryList _countryList) public initializer {
    ...
    function setDev(address payable _devaddr) public onlyAdmin {
        devaddr = _devaddr;
    }
     ...
     */
    function setMigrator(IMigrator _migrator) public onlyAdmin {
        migrator = _migrator;
    }

    function setSecondaryFeeToken(address _secondaryFeeToken) public onlyAdmin {
     ...
     */
    function setReferralTokenAndHold(IERCBurn _referralToken, uint256 _hold) public onlyAdmin {
     ...
     */
    function whitelistFeeAccount(address _user, bool _add) public onlyAdmin {
        if (_add) {
    ...
    function lockLPToken(
        ...
        if (!feeWhitelist.contains(msg.sender)) {
            if (_fee_in_eth) {
                ...
                IERC20Upgradeable(address(gFees.secondaryFeeToken)).safeTransferFrom(
                    address(msg.sender),
                    address(this),
                    burnFee
                );
                if (gFees.referralPercent != 0 && _referral != address(0)) {
                    ...
                    IERC20Upgradeable(address(gFees.secondaryFeeToken)).safeTransfer(_referral, referralFee);
                    burnFee -= referralFee;
                }
        ...
        IERC20Upgradeable(_lpToken).safeTransfer(devaddr, liquidityFee);
        ...
        EnumerableSetUpgradeable.UintSet storage user_locks = user.locksForToken[_lpToken];
    ...
    function relock(uint256 _lockID, uint256 _unlock_date) external nonReentrant {
        ...
        IERC20Upgradeable(userLock.lpToken).safeTransfer(devaddr, liquidityFee);
        emit onRelock(
            userLock.lockID,
            userLock.lpToken,
            msg.sender,
    ...
    function withdraw(uint256 _lockID, uint256 _amount) external nonReentrant {
        ...
        if (userLock.amount == 0) {
            EnumerableSetUpgradeable.UintSet storage userLocks = USERS[msg.sender].locksForToken[userLock.lpToken];
        ...
        IERC20Upgradeable(userLock.lpToken).safeTransfer(msg.sender, _amount);
    ...
    function incrementLock(uint256 _lockID, uint256 _amount) external nonReentrant {
        ...
        IERC20Upgradeable(userLock.lpToken).safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    function splitLock(uint256 _lockID, uint256 _amount) external payable nonReentrant {
        ...
        EnumerableSetUpgradeable.UintSet storage user_locks = user.locksForToken[userLock.lpToken];
    ...
    function transferLockOwnership(uint256 _lockID, address payable _newOwner) external {
        ...
        EnumerableSetUpgradeable.UintSet storage user_locks = user.locksForToken[transferredLock.lpToken];
        user_locks.add(transferredLock.lockID);

        // remove the lock from the old owner
        EnumerableSetUpgradeable.UintSet storage userLocks = USERS[msg.sender].locksForToken[transferredLock.lpToken];
    ...
    function migrate(uint256 _lockID, uint256 _amount, uint256 _migration_option) external nonReentrant {
        ...
        IERC20Upgradeable(userLock.lpToken).safeApprove(address(migrator), _amount);
        migrator.migrate(
            userLock.lpToken,
            _amount,
            userLock.unlockDate,
            msg.sender,
            userLock.countryCode,
            _migration_option
        );
    ...
    function getUserInfo(
        address _user
    ) external view returns (address[] memory _lockedTokens, uint256[][] memory _locksForToken) {
        _lockedTokens = USERS[_user].lockedTokens.values();
        for (uint256 i = 0; i < _lockedTokens.length; i++) {
            address token = _lockedTokens[i];
            _locksForToken[i] = USERS[_user].locksForToken[token].values();
        }
    }
}