...
contract CollectionList is ICollectionList, AccessControlEnumerableUpgradeable, PausableUpgradeable {
  ...
  Collection[] private _list;
  mapping(uint=>uint) private _collectionIdIndex;
  ...
  modifier withValidCollectionId(uint collectionId) {
    require(_collectionIdIndex[collectionId] > 0, "CollectionList: invalid collection id");
    _;
  }

  function _getEmptyCollection() internal pure returns(Collection memory) {
    return Collection({
      id: 0,
      name: "",
      chainId: 0,
      token: address(0),
      accessThresholdInCollectionTokens: 0,
      activityThresholdInSNK: 0,
      lockPeriodInSeconds: 0,
      revshareInPercents: 0,
      revshareRecipient: payable(address(0)),
      depositedSNK: 0,
      active: false,
      lockedAt: 0,
  ...
  function initialize(address adminAccount, address skill) public initializer {
    ...
    _list.push(_getEmptyCollection());
  }
  ...
  function addCollection(
    ...
    uint id = _collectionIds.current();
    _list.push(
      Collection({
        id: id,
        name: name,
    ...
    _collectionIdIndex[id] = _list.length - 1;
  ...
  function toggleDepositAccepting(uint collectionId) 
    ...
    external override 
  {
    uint index = _collectionIdIndex[collectionId];
    _list[index].depositsEnabled = !_list[index].depositsEnabled;
  }
  ...
  {
    uint index = _collectionIdIndex[collectionId];
    require(_list[index].depositsEnabled == false, 'CollectionList: depositing should be disabled');
    ...
    external override  
  {
    ...
    uint index = _collectionIdIndex[collectionId];
    Collection memory popped = _list[_list.length-1];
    Collection memory removed = _list[index];
    _list[index] = popped;
    _collectionIdIndex[popped.id] = index;
    _collectionIdIndex[removed.id] = 0;
    _list.pop();
  ...
  {
    uint index = _collectionIdIndex[collectionId];
    _list[index].lockedAt = block.timestamp;
  }
  ...
  {
    uint index = _collectionIdIndex[collectionId];
    _list[index].accessThresholdInCollectionTokens = accessThresholdInCollectionTokens;
  }
  ...
  {
    uint index = _collectionIdIndex[collectionId];
    _list[index].activityThresholdInSNK = activityThresholdInSNK;
  }
  ...
  {
    uint index = _collectionIdIndex[collectionId];
    _list[index].revshareRecipient = revshareRecipient;
  }
  ...
  {
    uint index = _collectionIdIndex[collectionId];
    _list[index].revshareInPercents = revshareInPercents;
  }
  ...
  {
    uint index = _collectionIdIndex[collectionId];
    _list[index].lockPeriodInSeconds = lockPeriodInSeconds;
  }
  ...
  {
    uint index = _collectionIdIndex[collectionId];
    require(_list[index].depositsEnabled == true, "CollectionList: deposits disabled");
    require(_skill.transferFrom(msg.sender, address(this), amount), "CollectionList: not enough funds");
    
    Collection storage collection = _list[index];
  ...
  {
    uint index = _collectionIdIndex[collectionId];
    return _list[index].revshareRecipient;
  }
  ...
  {
    uint index = _collectionIdIndex[collectionId];
    return _list[index].revshareInPercents;
  }
  ...
  function getCollections(uint startIdx, uint endIdx) external override view returns(Collection[] memory) {
    require(
      startIdx >= 1 &&
      endIdx < _list.length && 
      startIdx <= endIdx,
      'CollectionList: invalid indexes'  
    );
    Collection[] memory slice = new Collection[](endIdx-startIdx+1);
    for (uint i=startIdx; i<=endIdx; i++) {
      slice[i-startIdx] = _list[i];
    } 
    return slice;
  }
