...
contract IndexCommunity is Index, IIndexCommunity {
    ...
    function initialize(
        ...
        _setupRole(DAO_COMMUNITY_ROLE, admin);
        _initialize(
            adminDAO,
            admin,
            USD,
    ...
    function _rebalance(Asset[] memory newAssets) internal override {
        _clearActiveAssets();
        _clearNewAssets();

        uint256 len = newAssets.length;
        for (uint256 i; i < len; i++) {
            _pushAssetInActive(newAssets[i]);
            _addAssetInNewAssets(newAssets[i].asset);
...
abstract contract Index is AccessControl, IIndex {
    ...
    /**
     * @dev Stops the work of the contract.
    ...
    function getStatusPause() external view returns (bool) {
        return _pause;
    }
    ...
    function _initialize(
        ...
        _actualAcceptToken = USD;
        _adapter = adapter;
        _startPriceIndex = startPrice;
        _maxShare = 20 * PRECISION_E6; // TODO:  поставить 20
        ...
        emit Initialize(adminDAO, admin, USD, _indexLP);
    }

    function init(Asset[] memory newAssets) external onlyRole(ADMIN_ROLE) {
        _rebalance(newAssets);

        uint256 firstPriceIndex = _calcCost(1e18);

        emit Init(newAssets, firstPriceIndex);
    }

    // function rebalance(Asset[] memory newAssets, address[] memory path)
    //     external
    //     onlyRole(ADMIN_ROLE)
    // {
    //     require(
    ...
    //     uint256 usdAmount = _exchangeToUSD(); // меняет все на доллары

    //     if (_actualAcceptToken != _nextAcceptToken) {
    //         usdAmount = _changeActualToken(usdAmount, path);
    //     }

    //     _rebalance(newAssets); // обновляет структуры Asset
    //     _buyNewAssets(newAssets, usdAmount);

    //     emit Rebalance(newAssets);
    // }

    function rebalance(Asset[] memory newAssets) external onlyRole(ADMIN_ROLE) {
        require(
            block.timestamp > _lastRebalance + _rebalancePeriod,
            "It is not possible to rebalance"
        );

        uint256 oldCost = _calcCost(1e18);

        uint256 usdAmount = _exchangeToUSD(); // меняет все на доллары

        _rebalance(newAssets); // обновляет структуры Asset
        _buyNewAssets(_activeAssets, usdAmount);

        uint256 newCost = _calcCost(1e18);

        if (newCost < oldCost && newCost > oldCost - (oldCost / 100)) {
            test = "good";
        }
        test = "negood";
        // require(
        //     newCost < oldCost && newCost > oldCost - (oldCost / 100),
        //     "invalidcost"
        // );
    ...
    function stake(
        ...
        _stake(amountLPWithoutTax);

        uint256 amount = _ipartnerProgram.distributeTheReward(
            msg.sender,
            amountLPWithoutTax,
            _indexLP
        );
        IndexLP(_indexLP).mint(msg.sender, amount);
        IndexLP(_indexLP).mint(_tresuare, tax);

        emit Stake(msg.sender, cost, amount);
    }
    ...
    function unstake(uint256 amountLP) external isZeroAmount(amountLP) {
        ...
        IndexLP(_indexLP).burn(msg.sender, amountLP);
        IndexLP(_indexLP).mint(_tresuare, tax);
    ...
    function getAcceptToken() external view returns (address) {
        return _actualAcceptToken;
    }
        ...
        returns (uint256 amountActualAcceptToken)
    {
        ...
        _actualAcceptToken = _nextAcceptToken;
    }
    ...
    function _exchangeToUSD() internal returns (uint256 sum) {
        ...
        for (uint256 i; i < len; i++) {
            ...
            uint256 amount = _swapDEX(_activeAssets[i].totalAmount, reversPath);
    ...
    function _pushAssetInActive(Asset memory asset) internal {
    ...
    //     uint256 len = _activeAssets.length;
    //     for (uint256 i; i < len; i++) {
    //         uint256 amountAsset = (amountLP * _activeAssets[i].fixedAmount) /
    //             PRECISION;
    //         IERC20(_activeAssets[i].asset).safeTransfer(
    //             msg.sender,
    //             amountAsset
    //         );

    //         _activeAssets[i].totalAmount -= amountAsset;
    //     }
    // }
    ...
    function _stake(uint256 amountLP) internal {
        uint256 len = _activeAssets.length;
        for (uint256 i; i < len; i++) {
            uint256 amountForSwap = (amountLP * _activeAssets[i].fixedAmount) /
                PRECISION;

            uint256[] memory amountsIn = IUniswapV2Router02(_adapter)
                .getAmountsIn(amountForSwap, _activeAssets[i].path);
            _activeAssets[i].totalAmount += _swapDEX(
                amountsIn[0],
    ...
    function _rebalance(Asset[] memory newAssets) internal virtual {
        _clearActiveAssets();
