...
/**
 * @title Marketplace
 * @dev This contract allows buying and selling of ERC1155 tokens and charges a fee on each sale.
 */
    ...
    AccessControl
{
    ...
    modifier onlyDropMembers() {
        // TODO: Requirements for selling on the marketplace
        _;
    }

    constructor(
        address payable _modaTreasury,
        uint256 _treasuryFee,
        address _usdAddress
    ) WithTreasury(_modaTreasury, _treasuryFee) {
        require(_treasuryFee > 0, "Treasury fee cannot be 0");
        usdcInstance = IERC20(_usdAddress);
    }

    /**
     * @dev Create a sale for a given NFT
     */
    function createSale(
        ...
        uint256 _startAt,
        uint256 _endAt,
        uint256 _maxCountPerWallet
    ) external onlyDropMembers {
        ...
        require(_startAt >= block.timestamp, "Must start in the future");
        require(_maxCountPerWallet > 0, "Max wallet count must be gt 0");
        ...
        sales[msg.sender].push(
            Sale({
                ...
                startAt: _startAt,
                endAt: _endAt,
                maxCountPerWallet: _maxCountPerWallet
            })
        );

        emit SaleCreated(msg.sender, sales[msg.sender].length - 1);
    }

    /**
     * @dev Purchase a given NFT
     */
    function purchase(
        address _seller,
        uint256 _saleId,
        ...
        require(_tokenAmount > 0, "Cannot purchase 0 tokens");
        Sale storage sale = sales[_seller][_saleId];
        require(_tokenAmount <= sale.tokenAmount, "Insufficient supply");

        // Allow for seller to withdraw their NFTs at no cost
        if (_seller == _msgSender()) {
            sale.tokenAmount -= _tokenAmount;
        ...
        require(
            (IERC1155(sale.tokenAddress).balanceOf(msg.sender, sale.tokenId) +
                _tokenAmount) <= sale.maxCountPerWallet,
            "Cannot exceed max count per wallet"
        );

        uint256 totalPrice = sale.pricePerToken * _tokenAmount;
        uint256 fee = (treasuryFee * totalPrice) / 10_000;
        // Transfer USDC
        usdcInstance.safeTransferFrom(msg.sender, modaTreasury, fee);
        usdcInstance.safeTransferFrom(
            msg.sender,
            sale.beneficiary,
            totalPrice - fee
        );
    ...
    function saleAt(
        address _seller,
        uint256 _index
    ) external view override returns (Sale memory) {
        return sales[_seller][_index];
    }

    function saleCount(
        address _seller
    ) external view override returns (uint256) {
        return sales[_seller].length;
    }

    function setTreasuryFee(
        uint256 _newFee
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _setTreasuryFee(_newFee);
    }
