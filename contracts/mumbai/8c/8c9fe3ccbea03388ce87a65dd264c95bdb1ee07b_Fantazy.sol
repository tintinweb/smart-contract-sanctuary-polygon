 ...
 */
 ...
 contract Fantazy is Ownable, ReentrancyGuard{
    ...
    function Joinevent(
        ...
        uint256 limit = pools[pool].limit;
    ...
    function returnentryfee(uint256 poolid, uint256 team, uint256 eventID) internal {
    ...
    function withdrawPrizes2(address[] calldata users, uint256[] calldata amounts, uint256 eventID, uint256 poolid) external onlyOwner{
        ...
        for(uint256 i = 0; i < users.length; i++){
            ...
            (bool success,) = payable(address(uint160(users[i]))).call{value: amounts[i],gas:4500}('');
