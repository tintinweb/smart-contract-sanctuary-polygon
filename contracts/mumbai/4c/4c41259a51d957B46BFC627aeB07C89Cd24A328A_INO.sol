...
contract INO is ERC721Holder, ERC1155Holder {
    ...
    function changeTicketsCounter(ITicketsCounter tc_) external onlyOwner {
        ticketsCounter = tc_;
    }
    ...
    function draw(uint256 inoId_) external onlyAdmin onlyApprovedIno(inoId_) {
        ...
        if (ino.inoDetails.protocol == Protocol.ERC1155) {
            loadedNftAmount = ino.nftAmountsSum;
        } else {
        ...
        ino.drawn = true;
        ino.drawTime = uint32(block.timestamp);
    ...
    function loadNft(
        ...
        INOInfo storage ino = _INOs[inoId_];
        if (ino.inoDetails.protocol == Protocol.ERC1155) {
            require(tokenIds_.length == amounts_.length, 'wrong length of tokenIds/amounts');
            uint256 loadedAmountsSum;
            for (uint256 i = 0; i < tokenIds_.length; i++) {
                IERC1155(ino.inoDetails.nft).safeTransferFrom(msg.sender, address(this), tokenIds_[i], amounts_[i], '');
                ino.nftIds.push(tokenIds_[i]);
                ino.nftAmounts.push(amounts_[i]);
                loadedAmountsSum += amounts_[i];
            }
            require(ino.nftAmountsSum + loadedAmountsSum <= ino.inoDetails.totalNftAmount, 'limit');
            ino.nftAmountsSum += loadedAmountsSum;
        } else {
            require(ino.nftIds.length + tokenIds_.length <= ino.inoDetails.totalNftAmount, 'limit');
            for (uint256 i = 0; i < tokenIds_.length; i++) {
                IERC721(ino.inoDetails.nft).safeTransferFrom(msg.sender, address(this), tokenIds_[i]);
                ino.nftIds.push(tokenIds_[i]);
            }
        }
    }
    ...
    function claim(
        ...
        require(!_userInfos[index_][msg.sender].claimed, 'already claimed');
        if (!ino.cancelled) {
            ...
            if (tokenIds_.length == 0) {
                ...
                require(block.timestamp >= ino.drawTime + ino.inoDetails.vestingTime, 'vesting period yet');
        ...
        _userInfos[index_][msg.sender].claimed = true;
        emit Claimed(index_, msg.sender);
    }
    ...
    function _retrieveNFTs(
        ...
        if (protocol == Protocol.ERC1155) {
            ...
            for (uint256 i = 0; i < _tokenIds_.length; i++) {
                IERC721(_INOs[index_].inoDetails.nft).safeTransferFrom(address(this), user_, _tokenIds_[i]);
            }
        }
    }
