...
contract PoolFactory is Ownable {
    ...
    */
    ...
    //  * @return Created Pool Address
    // */
    function getCreatedTicketsByToken(address _pool)
        public
        view
        returns (address)
    {
    ...
    function registerPool(
    ...
    ) external onlyOwner returns (address createIDO, address createTicket) {
        ...
        IDOPool(payable(createIDO)).initialize(
            _token,
            _offeredCurrency,
            _wallet,
            createTicket,
        ...
        emit PoolCreated(msg.sender, _token, createIDO, allPools.length - 1);
    }
}
...
contract Ticket1155 is ERC1155, Ownable {
    ...
    event CreateTierBatch(uint256[]  ids, uint256[]  amounts, string[] _uri, bytes  _data);
    event CreateTier(uint256 _id, uint256 _amounts, string _uri, bytes _data);
    event MintBatch(address _to, uint256[] _ids, uint256[] _quantity, bytes _data);
    event Mint(address _to, uint256 _id, uint256 _quantity, bytes _data);
    event BurnBatch(address _from, uint256[] _ids, uint256[] _quantity);
    ...
    function initialize(string memory _name, string memory _symbol, address _minter) external {
        ...
        emit Initialize(_name, _symbol, _minter);
    }
    ...
    function getURI(uint256 _id)  external view returns (string memory) {
        require(_exists(_id), "ID not exist");

        string memory baseURI = tokenURI[_id];
    ...
    function createTierBatch(
        ...
        require(msg.sender == minter, "Only onwer can create new nft ticket");
                
        
        for (uint256 i = 0; i < ids.length; i++) {
            require(_exists(ids[i]) == false, "ID exist");
        }

        _mintBatch(msg.sender, ids, amounts, _data);
        ...
        emit CreateTierBatch(ids, amounts, _uri, _data);
    ...
    function createTier(
        ...
        require(msg.sender == minter, "Only onwer can create new nft ticket");
        require(_exists(_id) == false, "ID exist");

        creators[_id] = msg.sender;
        tokenURI[_id] = _uri;
        tokenSupply[_id] = _amounts;

        _mint(msg.sender, _id, _amounts, _data);

        emit CreateTier(_id, _amounts, _uri, _data);
    ...
    function mintBatch(
        ...
        for (uint256 i = 0; i < _ids.length; i++) {
            require(creators[_ids[i]] == msg.sender, "Only Creator can mint");
            require(_exists(_ids[i]), "ID not exist");
        }

        _mintBatch(_to, _ids, _quantity, _data);

        for (uint256 i = 0; i < _ids.length; i++) {
            tokenSupply[_ids[i]] = tokenSupply[_ids[i]] + _quantity[i];
        }

        emit MintBatch(_to, _ids, _quantity, _data);

        return (_ids, _quantity);
    }
    ...
    function mint(
        ...
        require(creators[_id] == msg.sender, "Only Creator can mint");
        require(_exists(_id), "ID not exist");
    ...
    function burnBatch(
        ...
        _burnBatch(_from, _ids, _quantity);

        for (uint256 i = 0; i < _ids.length; i++) {
            tokenSupply[_ids[i]] = tokenSupply[_ids[i]] - _quantity[i];
        }

        emit BurnBatch(_from, _ids, _quantity);
    }
    ...
    function burn(
        ...
        require(creators[_id] == msg.sender, "Only Creator can burn");
        require(_exists(_id), "ID not exist");
    ...
    function setTokenID(uint256 _id) external onlyOwner returns (uint256) {
        require(_exists(_id), "ID not exist");
