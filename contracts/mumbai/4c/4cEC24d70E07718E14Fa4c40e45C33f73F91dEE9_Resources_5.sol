...
contract Resources_5 is Initializable, IResourceLogic {
    using Items for Items.ID;
    using Resources for Resources.ID;
    IBaseLogic public baseLogic;
    modifier onlyGameContract() {
        require(msg.sender == baseLogic.gameSupport(), "p");
        _;
    }
    ...
    {
        if (resourceId == uint16(Resources.ID.N1)) {
            n1(id, position);
        } else if (resourceId == uint16(Resources.ID.O1)) {
            o1(id, position);
        } else if (resourceId == uint16(Resources.ID.O4)) {
            o4(id, position);
        } else if (resourceId == uint16(Resources.ID.O5)) {
            o5(id, position, randomNumber);
        } else if (resourceId == uint16(Resources.ID.P4)) {
            destroyPos = p4(id, position);
        } else if (resourceId == uint16(Resources.ID.P6)) {
            destroyPos = p6(id, position);
        } else if (resourceId == uint16(Resources.ID.R1)) {
            r1(id, position);
        } else if (resourceId == uint16(Resources.ID.R2)) {
            r2(id, position);
        } else if (resourceId == uint16(Resources.ID.R3)) {
            r3(id, position, randomNumber);
        } else if (resourceId == uint16(Resources.ID.W2)) {
            destroyPos = w2(id, position);
        } else if (resourceId == uint16(Resources.ID.R6)) {
            r6(id, position);
        } else if (resourceId == uint16(Resources.ID.R7)) {
            r7(id, position);
        } else if (resourceId == uint16(Resources.ID.S4)) {
            s4(id, position, randomNumber);
        } else if (resourceId == uint16(Resources.ID.S5)) {
            s5(id);
        } else if (resourceId == uint16(Resources.ID.S6)) {
            destroyPos = s6(id, position, randomNumber);
        } else if (resourceId == uint16(Resources.ID.S8)) {
            s8(id, position);
        } else if (resourceId == uint16(Resources.ID.S9)) {
            s9(id, position);
        }
    }

    function n1(uint256 id, uint16 position) internal {
        uint256 countNinja = IGameController(baseLogic.gameController())
            .countResource(id, uint256(Resources.ID.N1));
        if (countNinja > 1) {
            IGameController(baseLogic.gameController()).updatePayout(
                id,
                position,
            ...
            if (baseLogic.isItemExist(id, uint256(Items.ID.IC15), false)) {
                IGameController(baseLogic.gameController()).updatePayout(
                    id,
                    position,
                    0,
        ...
        if (baseLogic.isItemExist(id, uint256(Items.ID.IN1), false)) {
            ...
            for (uint256 i = 0; i < adjacentPosition.length; ++i) {
                ...
                if (adjResource == Resources.ID.M17) {
                    IGameController(baseLogic.gameController()).updatePayout(
                        id,
                        position,
                        5,
    ...
    function o1(uint256 id, uint16 position) internal {
        uint16[] memory adjacentPosition = Helper.getAdjacentPositions(
            position,
            baseLogic.checkAllAdjacent(id, position)
        );
    ...
    function o4(uint256 id, uint16 position) internal {
        uint256 countSpin = IGameController(baseLogic.gameController()).getProp(
            id,
            position
        );
    ...
    function o5(uint256 id, uint16 position, uint256 random) internal {
    ...
    function r1(uint256 id, uint16 position) internal {
        uint256 countSpin = IGameController(baseLogic.gameController()).getProp(
            id,
            position
        );
        ...
        if (countSpin == 0) {
            baseLogic.boostPayout(id, uint16(Resources.ID.R1), position, 2);
        }
        if (
            baseLogic.isItemExist(id, uint256(Items.ID.IL1), false) &&
        ...
        if (baseLogic.isItemExist(id, uint256(Items.ID.IL7), false)) {
            IGameController(baseLogic.gameController()).updatePayout(
                id,
                position,
                3,
        ...
        if (baseLogic.isItemExist(id, uint256(Items.ID.IG1), false)) {
            IGameController(baseLogic.gameController()).updatePayout(
                id,
                position,
                5,
    ...
    function r2(uint256 id, uint16 position) internal {
        // TODO raise chance get better symbols
        if (
            baseLogic.isItemExist(id, uint256(Items.ID.IL1), false) &&
            (position == 0 || position == 1 || position == 2 || position == 3)
        ) {
    ...
    function r3(uint256 id, uint16 position, uint256 random) internal {
        uint16[] memory adjacentPosition = Helper.getAdjacentPositions(
            position,
            baseLogic.checkAllAdjacent(id, position)
        );
    ...
    function r6(
        ...
        for (uint256 i = 0; i < adjacentPosition.length; ++i) {
            ...
            if (
                ...
                adjResource == Resources.ID.S6 ||
                adjResource == Resources.ID.B14 ||
                adjResource == Resources.ID.G4
            ) {
                coin += 3;
            } else if (
                adjResource == Resources.ID.A3 ||
                adjResource == Resources.ID.T1 ||
    ...
    function r7(uint256 id, uint16 position) internal {
        uint256 countRubies = IGameController(baseLogic.gameController())
            .countResource(id, uint16(Resources.ID.R7));
        if (countRubies > 1) {
            IGameController(baseLogic.gameController()).updatePayout(
                id,
                position,
            ...
            ).findPosition(id, uint16(Resources.ID.R7));
            IEventContract(baseLogic.eventContract()).emitSuperEffect(
                id,
                position,
                listPos
    ...
    function s4(uint256 id, uint16 position, uint256 random) internal {
        uint256 percent = ((random / 10 ** position) % 100000) % 100;
        if (
            (baseLogic.isItemExist(id, uint256(Items.ID.IF1), false) &&
    ...
    function s5(uint256 id) internal {
        // TODO updateChanceBetter for the roll
    }
    ...
    function s8(uint256 id, uint16 position) internal {
        uint256 countSpin = IGameController(baseLogic.gameController()).getProp(
            id,
            position
        );
    ...
    function s9(uint256 id, uint16 position) internal {
