...
contract PledgeStakeV2 {
    ...
    // uint256 public constant GUESS_REWARD = 1 * 10**18; // 1 token
    // uint256 public constant MAX_WINNERS = 5;
    // uint256 private constant GUESS_DURATION = 1 days;
    // uint256 public lastGuessTime;
    // uint256 public dailyNumber;
    ...
    // event Guess(address indexed user, uint256 guessedNumber);
    // event Winner(address indexed user, uint256 reward);
    ...
    function _updateRewards(address _user) private {
        ...
        for (uint256 i = 0; i < stakes[_user].length; i++) {
            ...
            if (!stake.unstaked && lastProcessedStakeIndex[_user] <= i) {
                ...
                for (uint256 j = 0; j < roundNumber; j++) {
                    ...
                    // Update level rewards for upline
                    address upline = user.referrer;
                    for (uint256 k = 0; k < LEVEL_COUNT; k++) {
                        if (upline != address(0)) {
                            ...
                            // Skip stake check for default referrer
                            if (upline != defaultReferrer || userInfo[upline].totalStake > 0) {
                ...
                // Update lastProcessedStakeIndex
                lastProcessedStakeIndex[_user] = i.add(1);
            }
        }
    }
    ...
    function unstake(uint256 unstakeIndex) external {
        ...
        uint256 roundDurationCompleted = (block.timestamp - stake.startTime) / ROUND_DURATION;
    ...
    // function guessNumber(uint256 _number) external {
    //     require(userGuesses[msg.sender] == 0, "User already guessed");
    //     userGuesses[msg.sender] = _number;
    //     emit Guess(msg.sender, _number);
    // }

    // function checkWinners() external {
    //     require(block.timestamp >= lastGuessTime + GUESS_DURATION, "Guess duration not completed");
    //     require(winnerCount < MAX_WINNERS, "All rewards have been claimed");

    //     if (userGuesses[msg.sender] == dailyNumber) {
    //         Pledge.transfer(msg.sender, GUESS_REWARD);
    //         winnerCount = winnerCount.add(1);
    //         emit Winner(msg.sender, GUESS_REWARD);
    //     }

    //     // Reset user's guess
    //     userGuesses[msg.sender] = 0;

    //     // Update daily number and reset winner count if all winners claimed reward
    //     if (winnerCount == MAX_WINNERS) {
    //         dailyNumber = uint256(keccak256(abi.encodePacked(block.timestamp, dailyNumber))) % 100;
