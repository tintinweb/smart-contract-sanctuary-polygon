...
contract StakingAlgo3 is ReentrancyGuard {
    ...
    function deposit(uint256 _amount, address _refer) external {
        ...
        // ex: _refer != address(0)
        if (!userInfo[msg.sender].isAddRefer && userInfo[_refer].depositedAmount > 0) {
    ...
    function apyNotVip() external view returns (uint256) {
        /**
         * totalSupply - total deposited amount
        ...
        uint256 apy = 0;
        if (totalSupply > 0) {
            apy = latestReward * APYDENOMINATOR * 365 * rewardPercent / totalSupply / rewardCycle / 100;
        }
    ...
    function apyOfVip1() external view returns (uint256) {
        ...
        uint256 apy = 0;
        if (totalSupply > 0) {
            apy = latestReward * APYDENOMINATOR * 365 * vip1RewardPercent / totalSupply / rewardCycle / 100;
        }
    ...
    function apyOfVip2() external view returns (uint256) {
        ...
        uint256 apy = 0;
        if (totalSupply > 0) {
            apy = latestReward * APYDENOMINATOR * 365 * vip2RewardPercent / totalSupply / rewardCycle / 100;
        }
