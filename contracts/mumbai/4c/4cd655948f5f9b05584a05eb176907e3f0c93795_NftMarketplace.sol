 ...
 */
	...
	modifier onlyValidNFTAddress(uint8 _tokenType, address _nftAddress) {
		if (_tokenType == uint8(TokenType.ERC721)) {
			...
			require(
				IERC1155Upgradeable(_nftAddress).supportsInterface(bytes4(0xd9b67a26)),
 ...
 */
		...
		nonReentrant
	{
		...
		require(
			IBEP20(listingBase.currency).transferFrom(
				msg.sender,
				address(this),
		...
		_transferTokens(listingBase.seller, listingBase.currency, _sale.saleData.sellingPrice);
		...
		nonReentrant
	{
		...
		require(
			IBEP20(listingBase.currency).transferFrom(_bid.bidderAddress, address(this), _bid.bidAmount),
			'MarketWithRoyalty: TRANSFER_FROM_FAILED'
		);

		_transferTokens(listingBase.seller, listingBase.currency, _bid.bidAmount);
		...
		returns (uint256 offerId)
	{
		...
		if (_tokenType == uint8(TokenType.ERC721)) {
			...
			require(
				IERC1155Upgradeable(_nftAddress).balanceOf(_nftHolder, _nftId) > 0,
	...
	function acceptOffer(uint256 _offerId) external onlyValidOfferId(_offerId) {
		...
		_transferTokens(offer.nftHolder, offer.currency, offer.price);
 ...
 */
	...
	function _transferTokens(
		...
		// require(IBEP20(_currency).transfer(feeReceiver, _serviceCharge), 'Market: TRANSFER_FAILED');

		// require(
		// 	IBEP20(_currency).transfer(_to, _paymentAmount - _serviceCharge),
	...
	function _transferNfts(
		...
		if (_tokenType == uint8(TokenType.ERC721)) {
			...
			IERC1155Upgradeable(_nftAddress).safeTransferFrom(_seller, _buyer, _tokenId, _copies, '');
		}
	}
