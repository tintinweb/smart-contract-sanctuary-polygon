...
contract ETHHODLStrategy is StrategyStandard{
    ...
    constructor(address _swaps, address _treasury, address _oracle, address _dex) StrategyStandard(bytes32("Asset Hold ETH"), _treasury, _oracle){
        swaps = ISwaps(_swaps);
    ...
    function minCollateral() external view override returns(uint){
        return 10;
    }


    function fund(uint256 _amountInvestment) public override onlyOwner{
        IERC20(stablecoin).safeTransferFrom(treasury, address(this), _amountInvestment);
...
abstract contract StrategyStandard is Ownable{
    ...
    event NewSwap(uint amountUnderlying);
    event FundsAdded(uint amount);
    event FundsRemoved(uint amount);

    constructor(bytes32 _name, address _treasury, address _oracle) Ownable(){
        treasury = _treasury;
    ...
    function minCollateral() external view virtual returns(uint){
        return 0;
    }
    ...
    * override to provide with the actual logic of the investment strategy
    ...
    function fund(uint256 _amountInvestment) public virtual onlyOwner{
        ...
        IERC20(stablecoin).safeTransferFrom(treasury, address(this), _amountInvestment);
        emit FundsAdded(_amountInvestment);
    }   
    ...
    * @dev function for owner (treasury) to remove funds 
    ...
    function removeFunds(uint256 _amountToRemove, address _receiver) public virtual onlyOwner{
        ...
        emit FundsRemoved(_amountToRemove);
    }
    ...
    * @dev function to buy swap on the strategy. Can only be done if it's free
    ...
    function buySwap(uint256 _amountUnderlying) public virtual{
        ...
        emit NewSwap(_amountUnderlying);
    }
