...
contract CrowdFundingCampaign is ICrowdFundingCampaign {
    ...
    event CampaignCreated(uint256 campaignId, string name, address creator, address token, uint256 goalAmount);
    event CampaignCancelled(uint256 campaignId, string name, address creator, address token, uint256 goalAmount);
    event CampaignClaimable(uint256 campaignId, string name, address creator, address token, uint256 goalAmount);
    event CampaignClaimed(uint256 campaignId, string name, address creator, address token, uint256 goalAmount);
    event CampaignPledged(uint256 campaignId, address pledger, address token, uint256 amount);
    event CampaignUnpledged(uint256 campaignId, address pledger, address token, uint256 amount);
    ...
    constructor(
        ...
        // cancelled = false;
        // claimable = false;
        emit CampaignCreated(_campaignId, _name, _creator, _tokenAddress, _goalAmount);
    }
    ...
    function cancel() external {
        ...
        emit CampaignCancelled(campaign.campaignId, campaign.name, campaign.creator, address(campaign.token), campaign.goalAmount);
    }
    ...
    function pledge(uint _amount) external {
        ...
        emit CampaignPledged(campaign.campaignId, msg.sender, address(campaign.token), _amount);
    }
    ...
    function refund() external {
        ...
        campaign.token.transferFrom(address(this), msg.sender, amount);
        emit CampaignUnpledged(campaign.campaignId, msg.sender, address(campaign.token), amount);
    }
    ...
    function setClaimable() external {
        ...
        emit CampaignClaimable(campaign.campaignId, campaign.name, campaign.creator, address(campaign.token), campaign.goalAmount);
    }
    ...
    function claim() external {
        ...
        emit CampaignClaimed(campaign.campaignId, campaign.name, campaign.creator, address(campaign.token), campaign.goalAmount);
    }
}
...
contract CrowdSourcingFactory {
    ...
    event NewCampaign(address campaignAddress, uint campaignId);
    ...
    function createCrowdFundingCampaign(string memory _name, address _tokenAddress, uint256 _goalAmount) external {
        ...
        CrowdFundingCampaign newCampaign = new CrowdFundingCampaign(campaignId, _name, _tokenAddress, _goalAmount, msg.sender);

        campaigns[campaignCount] = address(newCampaign);
        campaignCount++;

        emit NewCampaign(address(newCampaign), campaignId);
    }
}
