...
contract ApeironApostleSeasonMinting is
    Initializable,
    UUPSUpgradeable,
    OwnableUpgradeable,
    ChainlinkRngUpgradeable
{
    ...
    enum SEASON_MINT_TYPE {
        TEST_1,
        TEST_2
    }

    struct RewardMintStruct {
        address userAddress;
        uint256 gene;
        uint256 price;
    }
    ...
    // parameters
    bool public isEnabled; // control the minting period
    uint256[] public mintPriceArray; // minting price
    mapping(address => uint256) public addressMintedMapping; // Maximum 3 per address.
    mapping(address => uint256) public addressFreeMintedMapping; // Free mint Count
    mapping(uint256 => RewardMintStruct) public rewardMintStructMap; // uint256: Chainlink requestId

    // event
    event RequestPurchase(uint256 requestId);
    event PurchaseSuccess(uint256 indexed _tokenId);

    modifier onlyDuringSale() {
        _requireArgument(isEnabled, "Sale is not enabled");
        _;
    }

    // 2. Smart contract need to take a server side signed data, with apostle gene data and do the minting.
    // 3. Can set the period of minting.
    // 4. Can set the class which can mint. Season 2 can only mint priest, warrior and hunter.
    // 5. Can set minting price for each mint.
    // 7. Free mint item may allow
    ...
    function updateContractSetting(
        ...
        _requireArgument(_tokenAddress.isContract(), "invalid address");
     ...
     * @dev - callable only by owner
     *
     * @param _mintPriceArray - mint price array
     */
    function setMintPriceArray(
        uint256[] memory _mintPriceArray
    ) external onlyOwner {
        mintPriceArray = _mintPriceArray;
    }

    /**
     * @notice - get mint price
     */
    function getMintPriceArray() external view returns (uint256[] memory) {
        return mintPriceArray;
    }
     ...
     *
     * @param gene - gene of the NFT
     * @param time - time of the signature
     * @param signature - signature of the system
     */
    function requestPurchase(
        ...
        _requestPurchase(
            mintPriceArray[addressMintedMapping[msg.sender]],
            gene,
            time,
            signature
        );
     ...
     * @param signature - signature of the system
     */
    function requestFreeMint(
        uint256 gene,
        uint256 time,
        bytes memory signature
    ) external onlyDuringSale {
        _requestPurchase(0, gene, time, signature);
    }

    function _requestPurchase(
        uint256 price,
        uint256 gene,
        uint256 time,
        bytes memory signature
        ...
        // check signature
        bytes32 hash = keccak256(abi.encodePacked(gene, time, msg.sender));
        _requireArgument(
            hash.toEthSignedMessageHash().recover(signature) == systemAddress &&
        ...
        // check address minted count
        _requireArgument(
            addressMintedMapping[msg.sender] < 3,
            "Exceed maximum mint count"
        );
        ...
        rewardMintStructMap[requestId] = RewardMintStruct({
        ...
        // emit event
        emit RequestPurchase(requestId);
    }
     ...
     *
     * @param requestId - requestId of the Chainlink VRF
     */
    function _rngCallBack(uint256 requestId) internal virtual override {
        // check request info
        _requireArgument(
            rewardMintStructMap[requestId].userAddress != address(0),
            "Invalid requestId"
        );

        // mint NFT
        _purchase(
            rewardMintStructMap[requestId].userAddress,
            rewardMintStructMap[requestId].gene,
            rewardMintStructMap[requestId].price
        );
    }
    ...
    function _purchase(
        ...
        // check address minted count
        _requireArgument(
            addressMintedMapping[userAddress] < 3,
            "Exceed maximum mint count"
        );

        // transfer token
        if (price > 0) {
            token.transferFrom(userAddress, address(this), price);
        }

        // mint NFT
        uint256 tokenId = apostleContract.safeMint(gene, userAddress);

        // update address minted count
        addressMintedMapping[userAddress] += 1;

        // emit event
        emit PurchaseSuccess(tokenId);
    }
     ...
     */
    function withdrawFunds(uint256 amount, address wallet) external onlyOwner {
    ...
    UUPSUpgradeable
{
    ...
    function updateApostleData(
        uint256 _tokenId,
        uint256 _gene
    ) external hasTokenId(_tokenId) whenNotPaused onlyAdmin {
