...
abstract contract NonblockingReceiver is ILayerZeroReceiver, Ownable {
        ...
        onlyOwner
    {
        ...
        trustedRemoteLookup[_chainId] = remoteAndLocalAddresses;

        emit TrustedRemoteSet(_chainId, remoteAndLocalAddresses);
    }
...
contract PolygonSeaNFT is ERC721Enumerable, NonblockingReceiver {
    ...
    constructor(
        ...
        address _devwallet,
        address _endpoint, //address of layer zero endpoint in target chain
        uint256 _startMintId, //first tokenid available to mint
        uint256 _endMintId //last tokenid available to mint
    ...
    function changeauctioncollection(uint256 _collectionid, uint256 tokenid)
        internal
    {
        delete collectionstored[_collectionid][
            (idnumber[_collectionid][tokenid])
    ...
    function nftauctionend(uint256 tokenid)
        external
        view
        returns (bool auctionnftbool)
    {
        require(timeforauction[tokenid].time <= block.timestamp, "auction end");
    ...
    function collectionnft(uint256 collectionid)
        external
        view
    ...
    function totalcollectiondetails() external view returns (uint256[] memory) {
        return userinfo[msg.sender];
    }

    function auctiondetail(uint256 tokenid)
        external
        view
    ...
    function collectiondetails(uint256 id)
        external
        view
        returns (
            uint256,
    ...
    function listofsalenft(uint256 tokenid)
        external
        view
        returns (
            uint256[] memory,
    ...
    function nftinformation(uint256 id)
        external
        view
        returns (
            uint256,
