  ...
  ReentrancyGuard
{
  ...
  function updateMarketplaceAddress(address contractAddress) public onlyOwner {
  ...
  function mintCopiesBatch(
    ...
    for (uint256 i = 0; i < ids.length; i++) {
      require(((exists(ids[i])) || (ids[i] == 0)), "Token Id doesn't exists!");
    }
    require(
      MintCopiesAccess[msg.sender],
...
contract Marketplace is Ownable, ReentrancyGuard, ERC1155Holder {
  ...
  constructor(
    ...
    priceContract = FeedVIS(_priceContractAddress);
  ...
  function updatePriceContract(
    ...
    priceContract = FeedVIS(_newAddress);
    return true;
  }
  ...
  function markNFTForSale(
    uint256 _tokenId,
    uint256 _itemPrice, //Price is stored in USDC | Use decimal 2 i.e for 1 USDC use 100
    uint8 _saleType
  ) public returns (bool) {
    require(
    ...
    require(
      (tokenSaleStatus[_tokenId][_saleType] == 0) ||
        (tokenSaleStatus[_tokenId][_saleType] == 2),
    ...
    tokenSaleStatus[_tokenId][_saleType] = 1;
    return true;
  }
  ...
  function holdNFTSale(uint256 _saleId) public {
    ...
    if (allSales[_saleId].saleType == 1) {
      nftContract.safeTransferFrom(
        address(this),
        msg.sender,
        allSales[_saleId].tokenId,
        1,
        ""
      );
    }
  ...
  function updateNFTPrice(uint256 newPrice, uint256 _saleId) public {
    require(
      msg.sender == allSales[_saleId].sellerAddress || msg.sender == owner(),
      "Can only be called by NFT owners"
    );
    require(
      allSales[_saleId].status == 1,
    ...
    )
  {
    ...
    exclusiveAmounts = new uint256[](_saleIds.length);
    ...
    for (uint256 i = 0; i < _saleIds.length; i++) {
      ...
      require(
        tokenSaleStatus[allSales[_saleIds[i]].tokenId][_saleTypes[i]] == 1,
    ...
    )
  {
    ...
    return (saleConsiderationPriceVIS, itemTotalPriceVIS, contractFeeVIS);
  }
  ...
  function buyNFT(
    ...
    (
      ...
      uint256[] memory licenseAmounts,
      NFTSale[] memory currentSale
    ) = separateTypes(_saleIds, _saleTypes, buyerAddress);
    ...
    for (uint256 i = 0; i < _saleIds.length; i++) {
      sellerBalance[currentSale[i].sellerAddress] += currentSale[i].itemPrice;
    ...
    emit NFTSold(_saleIds, saleConsiderationPriceVIS, msg.sender);
  }

  function withdrawSellerBalance() public nonReentrant {
    require(sellerBalance[msg.sender] > 0, "Zero balance for the caller");
    uint256 sellerBalanceVIS = sellerBalance[msg.sender] *
      priceContract.estimateAmountsOut(USDCContract, 10000, 1);
    tokenContract.safeTransfer(msg.sender, sellerBalanceVIS);
...
contract User is Ownable {
  ...
  function updateKycAdminStatus(
    ...
    require(_address != owner(), "Cannot remove/add owner from junior admins");
  ...
  function getUserBlackListStatus(
    address _userAddress
  ) public view needAccess returns (bool) {
