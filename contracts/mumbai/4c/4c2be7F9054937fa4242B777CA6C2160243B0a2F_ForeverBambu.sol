    ...
    AccessControl
{
    ...
    struct SiteData {
        string siteName;
        uint256 area;
        uint256 carbonProducedAnnually;
        uint256 carbonProducedTotal;
    ...
    mapping(uint256 => SiteData) siteDatas;

    event CreateBambuSlot(uint256 indexed tokenId, string uri);
    event TransferBambuSlot(uint256 indexed tokenId, address indexed to);

    constructor()
        ERC721("ForeverBambuNFT", "FBT")
        PablockMetaTxReceiver("ForeverBambuNFT", "0.0.1")
    {
    ...
    modifier onlyMinter() {
        require(
            msgSender() == owner() || hasRole(MINTER_ROLE, msgSender()),
            "Caller not allowed"
        );
        _;
    }
    ...
    /// @notice Allow owner and deeper in grantRole there's a check about admin to grant minter role to address
    /// @notice It's possible to revoke role by calling revokeRole function of AccessControl
    /// @param _minter Address of minter
    function setMinter(address _minter) public onlyContractOwner {
    ...
    /// @notice Mint token, allow contract owner to mint token and specifiy on chain data about site
    /// @param to Receiver
    /// @param uri IPFS uri of site data
    /// @param _data data that need to be written onchain
    function mintToken(
        address to,
        string memory uri,
        SiteData memory _data
    ) public onlyMinter {
        // Mint the new token
        counter.increment();
        siteDatas[counter.current()] = _data;
        _safeMint(to, counter.current());
        _setTokenURI(counter.current(), uri);

        emit CreateBambuSlot(counter.current(), uri);
    }
    ...
    function transferFrom(
        ...
        emit TransferBambuSlot(tokenId, to);
    }

    function getSiteData(uint256 tokenId)
        public
        view
        returns (SiteData memory)
    {
        return siteDatas[tokenId];
    }
