...
contract SharedProperty is IMerchantProperty, Ownable {
    ...
    uint16 private _cashBackPercent;   // cash back percentage, denominator 10000
    ...
    function viewMerchantReceiveWallet() external pure override returns (address) {
        return address(0);
    }

    function viewMerchantReceiveContract() external pure override returns (address) {
        return address(0);
    }
    function isReceiveOnContract() external pure override returns (bool) {
        return false;
    }

    /**
     * @dev Merchant receive address in merchant-specific contract
     * Thats why it returns null address here
     */
    function viewReceiveAddress() external pure override returns (address, address, bool, uint256) {
    ...
    /**
     * @dev Update cash back percentage
     * Only callable by owner
     */
    function updateCashBackPercent(uint16 cashBack_) external override onlyOwner {
        require(cashBack_ < 10000 && cashBack_ >= 0, "Invalid percentage");
        _cashBackPercent = cashBack_;
    }
