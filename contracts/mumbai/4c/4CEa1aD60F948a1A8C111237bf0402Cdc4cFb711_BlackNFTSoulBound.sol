...
contract BlackNFTSoulBound is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, AccessControl, ERC721Burnable, ContextMixin, ReentrancyGuard {
    ...
    constructor(address[] memory minterAddresses) ERC721("BlackNFT", "BNFT") {
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(PAUSER_ROLE, msg.sender);

        for (uint256 i = 0; i < minterAddresses.length; i++) {
            _grantRole(MINTER_ROLE, minterAddresses[i]);
        }
    ...
    function _baseURI() internal view override returns (string memory) {
        return string(abi.encodePacked("https://api.fifteenshades.com/api/", Strings.toHexString(uint160(address(this)), 20), "/"));
    }
    ...
    function mint(address to, bytes32 _nonce, bytes calldata _signature) external payable nonReentrant onlyRole(MINTER_ROLE) {
        require(balanceOf(to) == 0, "Address already owns an NFT");
    ...
    function getAllNfts(address owner) public view returns (uint256[] memory) {
        uint256 ownedTokenCount = balanceOf(owner);
    ...
    {
        if (from != address(0)) {
            require(from == to, "NFT is non-transferable");
        }
        super._beforeTokenTransfer(from, to, tokenId, batchSize);
    }

    function _burn(uint256 tokenId) internal override(ERC721, ERC721URIStorage) onlyRole(MINTER_ROLE) {
        super._burn(tokenId);
    }
    ...
    function checkClaimEligibility(address recipientAddress)
    external
    view
    whenNotPaused
    returns (string memory)
    {
        if (balanceOf(recipientAddress) > 0) {
            return "Address already owns an NFT";
        }
