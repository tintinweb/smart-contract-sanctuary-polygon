...
contract UniverseX is ERC20, Ownable {
    ...
    uint256 constant public START_EMIT=10000000000 ether;
    uint256 constant public FEE=7;

    mapping(address=>bool) private _whitelist;
    constructor(address owner) ERC20("Universe X","UNIX"){
        _mint(owner, START_EMIT);
    ...
    function _transferWithFee(address from, address to, uint256 amount) internal{
        if (_whitelist[from] || _whitelist[to]){
            ...
            _transfer(from, to, amount * (100 - FEE) / 100);
            _burn(from, amount * FEE / 100);
        }
