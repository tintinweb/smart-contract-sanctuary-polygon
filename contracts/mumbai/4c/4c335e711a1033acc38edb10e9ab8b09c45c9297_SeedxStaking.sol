...
contract SeedxStaking is States, Ownable {
    ...
    function stake(uint256 _amount, uint256 _lockPeriod, uint256 _tier) public {
        require(isStakeActive, "staking is pause");
        _stakeTokens(_amount, _lockPeriod, _tier);
    }

    function _stakeTokens(uint256 _amount, uint256 _lockPeriod, uint256 _tier)  internal {
        ...
        require(_lockPeriod==2||_lockPeriod==3||_lockPeriod==4||_lockPeriod==10, "select correct tier");

        User memory user = deposit[_msgSender()];
        require(user.stakedAmount == 0, "Already Staked");
        
        deposit[_msgSender()] = User(block.timestamp.add(_lockPeriod), _amount, _amount, block.timestamp, block.timestamp, 0, _tier);

        TransferHelper.safeTransferFrom(address(stakingToken),msg.sender,address(this), _amount);

        totalStaked+=_amount;

        emit Stake(_msgSender(), _amount, _lockPeriod);
    }
    ...
    function _unstakeTokens(address _address) internal {
        User memory user = deposit[_address];
        ...
        deposit[_address] = User(0 , 0, 0, 0, 0, 0, 0);
    ...
    function checkReward(address _address) public view returns(uint256) {
        User memory info = deposit[_address];
        uint256 reward = 0;
    ...
    function changeLockPeriods(uint256 _lockPeriod1,uint256 _lockPeriod2,uint256 _lockPeriod3,uint256 _lockPeriod4) public onlyOwner {
      lockPeriod1 = _lockPeriod1;
      lockPeriod2 = _lockPeriod2;
      lockPeriod3 = _lockPeriod3;
      lockPeriod4 = _lockPeriod4;
    }
}