...
contract iNFTspaceSale is Ownable {
    ...
    struct SaleToken {
        ...
        uint8   standard;  // sale token standard: 0-721 1-1155
    ...
    event RegSaleToken(uint256 saleTokenId, address saleToken, uint8 standard);
    ...
    function unList(uint256 _saleTokenId, uint256 _assetId) public {
        ...
        if(saleToken.standard == SALE_TOKEN_ERC721) { // ERC721
            IERC721 iSaleToken = IERC721(saleToken.saleToken);
            iSaleToken.safeTransferFrom(address(this), s.saler, _sellingNFTs[_saleTokenId][_assetId].tokenId);
        } else {
            IERC1155 iSaleToken = IERC1155(saleToken.saleToken);
            iSaleToken.safeTransferFrom(address(this), s.saler, _sellingNFTs[_saleTokenId][_assetId].tokenId, 1, "");
        }
    ...
    function regSaleToken(address _saleToken, uint8 _standard) public onlyOperator{
    ...
    function regPayCurrency(address _currency) public onlyOperator{
        require(_payCurrencyExistsByAddress(_currency) == false, "iNFTSpaceSale:currency already add yet");
    ...
    function _payCurrencyExistsByAddress(address _currency) internal view returns (bool) {
        for(uint i=0; i<_supportCurrencies.length; i++){
            if(_supportCurrencies[i].currency == IERC20(_currency)){
                return true;
            }
