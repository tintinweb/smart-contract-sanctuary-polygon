...
contract SmartContractWallet is ERC721Holder, ERC1155Holder {
    ...
    event SwapOwner(address indexed previousOwner, address indexed currentOwner, uint256 swapTime);
    event NativeTokenReceived(address sender, uint256 value);
    event TokenReceived(address indexed previousOwner, address indexed currentOwner, uint256 swapTime);

    modifier isActive {
        require(active == true, "SCW: Contract Inactive");
        _;
    }

    modifier onlyOwner {
        require(owner == msg.sender, "SCW: Unauthorised Access");
        _;
    }
    ...
    function swapOwner(address _owner) external onlyOwner isActive {
        owner = _owner;
        emit SwapOwner(msg.sender, owner, block.timestamp);
    }

    function switchState() external isActive {
    ...
    function claimNativeToken() external onlyOwner {
        ...
        require(success, "SCW: Claim Error");
    }
