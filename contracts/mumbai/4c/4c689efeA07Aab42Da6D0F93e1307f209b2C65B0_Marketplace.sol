...
contract Marketplace is IMarketplace, Ownable, ReentrancyGuard {
    ...
    function getMarketItem(uint256 itemId)
        external
        view
        returns (MarketItemDetails memory)
    {
    ...
    function fetchItemsByOwner(
        ...
        if (address(collection) != address(0)) {
            ...
            Collection.Item memory current = first;
            for (uint256 i = 0; i < numItems; i++) {
                items[i] = _getMarketItemDetails(
                    _idToMarketItem[current.itemId]
                );
    ...
    function fetchItems(uint256[] calldata itemIds)
        public
        view
        returns (MarketItemDetails[] memory items)
    {
    ...
    function _getMarketItemDetails(MarketItem storage item)
        private
        view
        returns (MarketItemDetails memory details)
    {
        ...
        if (details.listed = item.listed) {
            ...
            //details.owner = IERC721(details.nftContract).ownerOf(
            //    details.tokenId
            //);
        }
    }
...
contract PassiveIncomeNFT is ERC721Enumerable, AccessControl {
    ...
    function claimableIncome(uint256[] calldata tokenIds)
        external
        view
