...
import "../Lib/IERC721Send.sol";
import "../Lib/IERC1155Send.sol";
 ...
 */
interface IERC721Send {
   
    function sendFrom(address _from, uint16 _dstChainId, bytes memory _toAddress, uint _tokenId, address payable _refundAddress, address _zroPaymentAddress, bytes memory _adapterParams) payable external;
 ...
 * @dev Required interface of an ERC721 compliant contract.
 */
interface IERC1155Send {
   
    function sendFrom(address _from, uint16 _dstChainId, bytes memory _toAddress, uint _tokenId, uint _amount, address payable _refundAddress, address _zroPaymentAddress, bytes memory _adapterParams) external payable ; 
    function estimateSendFee(uint16 _dstChainId, bytes memory _toAddress, uint _tokenId, uint _amount, bool _useZro, bytes memory _adapterParams) external  view  returns (uint nativeFee, uint zroFee);
...
contract CloneFactory is Initializable{
    ...
    function _CloneERC721(string memory name_, string memory symbol_,address owner,address _lzEndpoint) internal {
        IERC721Mint newERC721 = IERC721Mint(_createClone(address(Base721)));
       

        newERC721.initialize(name_, symbol_,owner,_lzEndpoint);

        ERC721Tokens.push(address(newERC721));
        emit TokenClone(address(newERC721));
    }

    function _CloneERC1155(string memory name_, string memory symbol_,address owner,address _lzEndpoint) internal {
        IERC1155Mint newERC1155 = IERC1155Mint(_createClone(address(Base1155)));

        newERC1155.initialize(name_, symbol_,owner,_lzEndpoint);
