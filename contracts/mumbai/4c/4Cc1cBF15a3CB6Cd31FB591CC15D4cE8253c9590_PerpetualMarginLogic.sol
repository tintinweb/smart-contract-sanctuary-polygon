...
contract PerpetualMarginLogic is
    ...
    IPerpetualMarginLogic
{
     ...
     * @param   _traderTknAddr       address of the trader token
     */
     ...
     */
    function withdrawDepositFromMarginAccount(uint24 _iPerpetualId, address _traderAddr)
        external
        override
    {
        require(msg.sender == address(this), "onlythis");
    ...
    function _depositMarginForOpeningTrade(
        ...
        if (
            !_order.flags.isMarketOrder() &&
            !_isDepositAllowed(marginTokenAddr, _order.traderAddr, _fDepositRequired)
        ) {
    ...
    function _reduceMarginCollateral(
        ...
        PerpStorage.LiquidityPoolData storage liquidityPool = liquidityPools[_perpetual.poolId];
        address marginTokenAddr = liquidityPool.marginTokenAddress;
    ...
    function _withdrawDepositFromMarginAccount(
        PerpetualData memory _perpetual,
        address _traderAddr
    ) internal {
        MarginAccount storage account = marginAccounts[_perpetual.id][_traderAddr];
...
interface IPerpetualDepositManager {
    ...
    function brokerDepositToDefaultFund(uint8 _poolId, uint16 _iLots) external;
