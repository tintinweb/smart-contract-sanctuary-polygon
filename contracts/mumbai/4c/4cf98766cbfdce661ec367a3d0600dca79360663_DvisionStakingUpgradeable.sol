    ...
    ReentrancyGuardUpgradeable
{
    ...
    /// @notice This event is fired whenever add/remove staking token addresses.
    /// @param  erc721Tokens - The list of ERC721 contract addresses.
    /// @param  erc1155Tokens - The list of ERC1155 contract addresses.
    /// @param  isValid - True/False if these token will added/removed.
    event UpdateTokens(
    ...
    /// @notice Manager Role bytes code.
    bytes32 public constant MANAGER_ROLE = keccak256("MANAGER_ROLE");

    /// @notice Contract Reward.
    IDvisionMysteryBox public REWARD_BOX;

    /// @notice Contract Proxy.
    address PROXY;

    /// @notice Mapping for ERC721 staking contracts.
    mapping(address => bool) public erc721Tokens;

    /// @notice Mapping for ERC1155 staking contracts.
    mapping(address => bool) public erc1155Tokens;
     ...
     * @param _erc721Tokens - The list of ERC721 NFT Land addresses
     * @param _erc1155Tokens - The list of ERC1155 NFT Land addresses
    ...
    function initialize(
        ...
        uint256 len = _erc721Tokens.length;
        for (uint i; i < len; i++) {
            erc721Tokens[_erc721Tokens[i]] = true;
        }
        len = _erc1155Tokens.length;
        for (uint i; i < len; i++) {
            erc1155Tokens[_erc1155Tokens[i]] = true;
        }
    ...
    /**
     * @dev   This function is called by the user when thay want to deposit their ERC721
     *        NFT Lands into this contract to earn rewards.
     * @param campaignId - An unique identifier for this particular campaign. There are 3
     *        campaign represent for 3 kinds of duration: 30, 60 and 90 days.
     * @param params - ERC721 address and list of tokenIds.
     */
    function depositERC721(uint8 campaignId, ParamsERC721 calldata params)
        external
        nonReentrant
        notProcessing(campaignId)
    {
        address token = params.token;
        require(erc721Tokens[token], "invalid token");
        uint256 len = params.tokenIds.length;
        ...
        _erc721Transfer(token, msg.sender, address(this), params.tokenIds);
    }

    /**
     * @dev   This function is called by the user when thay want to deposit their ERC1155
     *        NFT Lands into this contract to earn rewards.
     * @param campaignId - An unique identifier for this particular campaign. There are 3
     *        campaign represent for 3 kinds of duration: 30, 60 and 90 days.
     * @param params - ERC1155 address and list of tokenIds.
     */
    function depositERC1155(uint8 campaignId, ParamsERC1155 calldata params)
        external
        nonReentrant
        notProcessing(campaignId)
    {
        address token = params.token;
        require(erc1155Tokens[token], "invalid token");
        uint256 len = params.tokenIds.length;
        require(len > 0, "length equals 0");
        require(
            len == params.amounts.length,
            "length not match"
        );
        for (uint256 i; i < len; i++) {
            _deposit(
     ...
     *        ERC721 NFT Lands from this contract.
     * @param campaignId - An unique identifier for this particular campaign. There are 3
     *        campaign represent for 3 kinds of duration: 30, 60 and 90 days.
     * @param params - ERC721 address and list of tokenIds.
     */
    function withdrawERC721(uint8 campaignId, ParamsERC721 calldata params)
        external
        nonReentrant
        allowWithdraw(campaignId)
    {
        ...
        _erc721Transfer(token, address(this), msg.sender, params.tokenIds);
    }

    /**
     * @dev   This function is called by the user when thay want to withdraw their staked
     *        ERC1155 NFT Lands from this contract.
     * @param campaignId - An unique identifier for this particular campaign. There are 3
     *        campaign represent for 3 kinds of duration: 30, 60 and 90 days.
     * @param params - ERC1155 address and list of tokenIds.
     */
    function withdrawERC1155(uint8 campaignId, ParamsERC1155 calldata params)
        external
        nonReentrant
        allowWithdraw(campaignId)
    {
     ...
     * @param boxTypes - Rewawd box types Ids.
     * @param boxAmounts - Reward box amount each type Id.
     */
    ...
    /**
     * @dev   This function is called by the Admin when they want to set new Proxy contract
     *        address.
     * @param newProxy - The new Proxy contract address.
     */
    ...
    /**
     * @dev   This function is called by the Admin when they want to set new/remove a NFT
     *        contract address from list of staking contract.
     * @param _erc721Tokens - The new Proxy contract address.
     * @param _erc1155Tokens - The new Proxy contract address.
     * @param _isValid - The new Proxy contract address.
     */
    function updateTokens(
        ...
        uint256 len = _erc721Tokens.length;
        for (uint i; i < len; i++) {
            erc721Tokens[_erc721Tokens[i]] = _isValid;
        }
        len = _erc1155Tokens.length;
        for (uint i; i < len; i++) {
            erc1155Tokens[_erc1155Tokens[i]] = _isValid;
        }

        emit UpdateTokens(_erc721Tokens, _erc1155Tokens, _isValid);
    }
