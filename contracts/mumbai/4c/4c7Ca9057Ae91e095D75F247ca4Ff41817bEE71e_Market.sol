...
contract Market is  Ownable {
    ...
    function changeCommission (address _contract, uint256 _commission) public onlyownerOfCollection(_contract) {

        require(_commission <= MAX_COMMISSION , "The commission is more than the commission ceiling !!!");
        require(_commission <= 5 ,  "commission do not exceed 5%");
    ...
    function addCollectionOnlyOwner(uint256 _commission ,address _contract , address _owner ) public onlyOwner {
        require(_commission <= MAX_COMMISSION , "The commission is more than the commission ceiling !!!");

        Collections memory collection = Collections({
            isExist : true,
            contractAddressCollection : IERC721(_contract),
            ownerOfCollection : _owner,
            percentOfCommission : _commission
        });

        CollectionsContract[_contract] = collection;
        
        emit AddCollection(_contract ,_owner,_commission);      
    }

    function changeCollectionOnlyOwner(address _contract,uint256 _commission, address _owner) public onlyOwner {
        require(_commission <= MAX_COMMISSION , "The commission is more than the commission ceiling !!!");


        CollectionsContract[_contract].ownerOfCollection = _owner;
        
        CollectionsContract[_contract].percentOfCommission = _commission; 
    }

   function max_commissionChange(uint256 max_commission) public onlyOwner {
       MAX_COMMISSION = max_commission;
       require((MAX_COMMISSION+percentage) <= 99 ,'max_commission upper 99');
    }
    ...
    modifier onlyownerOfCollection(address _contract) {
       ...
       require(owner == msg.sender || ownerMarket == msg.sender, "You are not the owner of the collection !!");
        ...
        // if (owner != msg.sender){
        //      require(ownerMarket == msg.sender, "You are not the owner of the collection !!");
        // }
       _;
    }
