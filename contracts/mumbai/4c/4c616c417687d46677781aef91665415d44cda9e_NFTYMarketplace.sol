...
// File: delorean/AuctionContract.sol
...
contract NFTYMarketplace is AccessControlEnumerable, ReentrancyGuard {
    ...
    */
    ...
    function _payOut(
        ...
        if (_royaltyPercentage > 0) {
            royaltyProfit = _price.mul(_royaltyPercentage).div(10000);
        }
        sellerProfit = _price.sub(platformProfit.add(royaltyProfit));

        if (_currency == address(NATIVE_TOKEN_ADDRESS)) {
            if (platformFee.receiver != address(0) && platformProfit > 0) {
                ...
                require(
                    ...
                    "Transfer to platform fee receiver failed."
                );
            }
            if (_royaltyReciver != address(0) && royaltyProfit > 0) {
                (bool isRoyaltyFeeTransferSuccess, ) = payable(
                    _royaltyReciver
                ).call{value: royaltyProfit}("");
                require(
                    isRoyaltyFeeTransferSuccess,
                    "Transfer to royalty fee receiver failed."
                );
            }
    ...
    function _mintNFT(
        ...
        if (IERC721(_nftAddress).supportsInterface(ERC721InterfaceId)) {
            ERC721.Royalties memory royalties = ERC721.Royalties(
                payable(NftInfoStored.royaltyAddress),
                NftInfoStored.royaltyAmount
            );
            ERC721(_nftAddress).mint(
                _receiver,
                _tokenId,
                NftInfoStored.metaHash,
                royalties
            );
        } else if (
    ...
    function _settleTransfers(
        ...
        if (!isApproved && _isAuction) {
            ...
            delete mapAuction[_nftAddress][_tokenId][_seller];
            emit RefundAuction(_receiver, _price);
        } else if (!isApproved && !_isAuction) {
            delete mapSale[_nftAddress][_tokenId][_seller];
        } else if (isApproved && _isAuction) {
            if (IERC721(_nftAddress).supportsInterface(ERC721InterfaceId)) {
                _payOut(_price, _currency, _seller, true, NftInfoStored.royaltyAddress, NftInfoStored.royaltyAmount);
            ...
            Auction storage nftOnAuctionStorage = mapAuction[_nftAddress][
                _tokenId
            ][_seller];
            ...
            // settle buy nft swap token and swap currency
            if (IERC721(_nftAddress).supportsInterface(ERC721InterfaceId)) {
                _payOut(_price, _currency, _seller, true, NftInfoStored.royaltyAddress, NftInfoStored.royaltyAmount);
                IERC721(_nftAddress).safeTransferFrom(
                    _seller,
    ...
    function _NftSale(
        ...
        if (_isResell) {
            ...
            _mintNFT(_buyer, _nftAddress, _tokenId, _quantity);
    ...
    function _NftAuction(
        ...
        if (NftOnAuction.isResell) {
            ...
            _mintNFT(_buyer, _nftAddress, _tokenId, NftOnAuction.quantity);
            _payOut(
                // _nftAddress,
                // NftOnAuction.tokenId,
                NftOnAuction.salePrice,
