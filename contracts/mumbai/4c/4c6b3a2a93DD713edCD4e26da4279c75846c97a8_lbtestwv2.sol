...
contract lbtestwv2 is VRFV2WrapperConsumerBase,ConfirmedOwner,ReentrancyGuard {
  ...
  function autoSpin(address _caller, uint8[] calldata _nodes) public nonReentrant {
        ...
        if(lbNodes.getUserActivation(_caller,1) == false){
            ...
            require(block.timestamp > lastAutoSpin + 1800, "draw timestamp still not reached");   // 604800 1 week > 605100 1 week+5mins
            ...
            if(getJackpotinUSD() >= minimumPot && registeredsAccts.length > 0){ 
                ...
                if(block.timestamp > lastAutoSpin + 2100 && reset == false){   // if no node activates in 5 minutes period queue resets
