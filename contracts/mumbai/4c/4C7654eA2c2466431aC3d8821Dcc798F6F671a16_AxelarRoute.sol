    ...
    IRoute
{
    ...
    ) internal onlyAX() onlySrc(_translate(cid), sourceAddress.toAddress()) virtual override {
        emit Receive(
            "Axelar",
            cid,
            _translate(cid),
    ...
    function msend(
        ...
        if (shouldPayGas) { // sat fuji -> master moonbase
...
abstract contract AxelarModifiers is AxelarStorage, AxelarExecutable, CommonErrors {
    modifier onlyAX() {
        if (msg.sender != executor) revert OnlyGateway();
        _;
    }

    // slither-disable-next-line assembly
    modifier onlySrc(uint256 srcChain, address srcAddr) {
...
abstract contract AxelarAdmin is AxelarModifiers, AxelarEvents {
    function addSrc(uint256 srcChain, address newSrcAddress) external onlyAdmin() {
