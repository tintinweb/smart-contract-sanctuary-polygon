...
contract BCEvents is Ownable {
    ...
    struct BCEffect {
        bool permanent; // TODO: May be able to infer from having trait changes
        bool fullHealth;
        bool instantDeath;
        bool placeHazard;
        bool grantEgg;

        uint8 healAmt; // fullHealth + healAmt = bonus health
        uint8 healArmorAmt;
        uint8 hazardDamage;
        ...
        uint8 grantData;
        uint8 grantNumItems;
        uint8 takeNumItems; // 255 to take all?

        MovePlayer moveType;
        uint8 trapPlayerEscapeRoll;  // >0 to trap player
        uint8 grantAbility;  // TODO

        uint8 lockDoorStrength; // Locks ALL doors if > 0

        BCTypes.Traits traitModifiers; // TODO:  Should this split from one off effects to avoid waste
    }
    ...
    struct BCEvent {
        ...
        BCEffect defaultEffect;  // TODO: Eval doing it this way vs. using index and storing in array
        BCEffect lowEffect;
        BCEffect highEffect;
    }
    ...
    function extGetEffects(bool _roomEvent, uint _id) public view returns (BCEffect[] memory) {
        // TODO: Hardcoded length??
        BCEffect[] memory currentEffects = new BCEffect[](3);
    ...
    function _initializeDefaultCardEvents() internal {
        BCTypes.Traits memory noTraitsChanged;
    ...
    function _initializeDefaultRoomEvents() internal {
        ...
        bcRoomEvents.push(BCEvent(
            ...
            BCEffect(
                ...
                0,
                0,
                255, // TODO: Confirm this as method to take/destroy all
...
interface EventsInterface {
    ...
    function extGetEffects(bool _roomEvent, uint _id) external view returns (BCEvents.BCEffect[] memory);
...
contract BCPlayers is Ownable {
    ...
    event EventResolvedEvent(uint gameId, uint playerId, BCEvents.BCEvent bcEvent, BCEvents.BCEffect bcEffect);
    ...
    function extApplyEffect(uint _playerId, uint _gameId, uint _mapId, BCTypes.Position memory _position, uint _eventId, uint _effectNum) public {
        ...
        BCEvents.BCEffect memory effect = bcEvents.extGetEffects(true, _eventId)[_effectNum];
        _applyEffect(_playerId, _gameId, _mapId, _position, effect);
    }

    function _applyEffect(uint _playerId, uint _gameId, uint _mapId, BCTypes.Position memory _position, BCEvents.BCEffect memory _effect) internal {
        // TODO: Add a mechanism to just skip empty effects
        BCTypes.Player storage currentPlayer = players[_playerId];
        if(_effect.permanent) {
            // TODO
        }

        if(_effect.fullHealth) {
            currentPlayer.healthDmgTaken = 0;
        }

        if(_effect.instantDeath) {
            _respawnPlayer(_playerId);
        }

        if(_effect.placeHazard) {
        ...
        if(_effect.healAmt > 0) {
            if(_effect.fullHealth) {
                currentPlayer.healthDmgTaken = 0 - int8(_effect.healAmt);
            } else {
                currentPlayer.healthDmgTaken -= int8(_effect.healAmt);
        ...
        if(_effect.healArmorAmt > 0) {
            // TODO
        }

        if(_effect.hazardDamage > 0) {
            _hurtPlayer(_playerId, _effect.hazardDamage, true);
        }

        if(_effect.physicalDamage > 0) {
            // TODO
        }

        if(_effect.enemyType != BCEvents.EnemyType.NONE) {
            // TODO
        }

        if(_effect.grantData > 0) {
            grantDataToPlayer(_playerId, _effect.grantData);
        }

        if(_effect.grantNumItems > 0) {
        ...
        if(_effect.moveType == BCEvents.MovePlayer.REACTOR) {
        ...
        if(_effect.trapPlayerEscapeRoll > 0) {
            // TODO
        }

        if(_effect.grantAbility > 0) {
            // TODO
        }

        if(_effect.lockDoorStrength > 0) {
            ...
            for(uint i = 0; i < 4; i++) {
                BCTypes.Door memory currentDoor = maps.getDoor(_mapId, _position.row, _position.col, i);
                if (currentDoor.status == BCTypes.DoorStatus.CLOSED ||
                    ...
                    currentDoor.status == BCTypes.DoorStatus.BREACHED) {
                        maps.setDoorFromPos(_mapId, _position.row, _position.col, i, BCTypes.DoorStatus.CLOSED, _effect.lockDoorStrength);
                }
            }
        }
    ...
    function extChooseEffect(uint _choice, uint _eventPlayerId, uint _gameId, uint _mapId, BCTypes.Position memory _position, uint _eventNumber, bool _eventIsTile) public {
        require(msg.sender == address(games), "Not games contract");
        BCEvents.BCEvent memory currentEvent;
        BCEvents.BCEffect memory currentEffect;
        if(_eventIsTile) {
        ...
        BCEvents.BCEffect[] memory currentEffects = bcEvents.extGetEffects(_eventIsTile, _eventNumber);
