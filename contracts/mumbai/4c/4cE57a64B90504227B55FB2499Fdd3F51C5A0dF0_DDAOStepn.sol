...
*/
	...
	if(Txs[sale][addr][1].enable == false)
	{
	...
	Txs[sale][addr][k].enable = true;
	Txs[sale][addr][k].blk = block.number;
	Txs[sale][addr][k].sale = sale;
	Txs[sale][addr][k].amount = amount;
	Txs[sale][addr][k].addr = addr;
	...
	emit TxLog(sale,addr,amount,Sale[sale].vault,block.number,block.timestamp,TxsCount[sale][addr],TxsCountGlob[sale]);
        }
