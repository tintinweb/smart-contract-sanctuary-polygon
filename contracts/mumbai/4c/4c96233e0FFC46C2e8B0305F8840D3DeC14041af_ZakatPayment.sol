...
contract ZakatRequest is BasicAccessControl, ZakatBasic {
    ...
    {
        require(
            _requestId > 0 && _requestId < requestIds.length - 1,
            "Invalid request id"
        );
    ...
    function sendRequest(bool _doner, uint256 _amount) external {
        require(
            requestIds.length - 1 < MAX_REQUESTS,
        ...
        zktApprove.requestId = requestIds.length - 1;
...
contract ZakatPayment is BasicAccessControl, ZakatBasic {
    ...
    function setApprovedEligiblity(
        ...
        address approvalAddress = zktRequest.getApprovalAddress(_approvedId);
        ZakatApproval memory approval = zktRequest.getApprovalData(
            approvalAddress
        );

        zktRequest.setApprove(approval.requestId, _approve);
