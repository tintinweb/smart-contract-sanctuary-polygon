...
interface SegMintNFTVaultInterface {
    ...
    // batch unlock and transfer
    function batchUnlockAndTransferNFTs(BatchNFTsStruct[] memory lockData_, address wallet) external;
    ...
    SegMintNFTVaultInterface
{
    ...
    function batchWithdrawNFTs(BatchNFTsStruct[] memory withdrawData_)
        ...
        onlyOwner
    {
        _batchTransferNFTs(withdrawData_, msg.sender);
    }

    function _batchTransferNFTs(BatchNFTsStruct[] memory withdrawData_, address wallet) private {
    ...
    function batchUnlockAndTransferNFTs(BatchNFTsStruct[] memory lockData_, address wallet) public {
        for (uint256 i = 0; i < lockData_.length; i++) {
            // contract address
            address contractAddress = lockData_[i].contractAddress;
            ...
            _batchTransferNFTs(lockData_, wallet);
            // emit event
            emit BatchUnlockEvent(msg.sender, lockData_, block.timestamp);
        }
    }
