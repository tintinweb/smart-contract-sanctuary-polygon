...
contract MetaBazar {
    ...
    function _RoiWithPrincipleWithdrawal() external updateReward(msg.sender) {
        ...
        uint256 principle = user.selfCurrentPackagePurchase;
        // Set Reward 0 & Current Package Purchase
        usersDetails[msg.sender].roiUnSettled = 0;
        user.totalAdminChargeCollected+=rewardAdminCharge;
        usersDetails[msg.sender].roiBonus +=reward;
    ...
    function _Withdrawal(uint256 _amount) external updateReward(msg.sender) {
        User storage user = users[msg.sender];
