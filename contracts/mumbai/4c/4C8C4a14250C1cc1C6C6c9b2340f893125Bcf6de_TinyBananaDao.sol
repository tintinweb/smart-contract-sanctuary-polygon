...
import "./Dai.sol";
import "./BananaDao.sol";

contract TinyBananaDao {
  ...
  BananaDao private mainBananaDaoAddress;

  struct Account {
    bool exists;
    uint type75NodeCount;
    uint type50NodeCount;
    uint type25NodeCount;
    uint interestAccumulated;
  }

  mapping(address => Account) public accounts;

  uint [] public nodeRewards = [
    1 * 10 ** 17, // 25 DAI - 25 BANANA -- Reward: 0.1 BANANA
    3 * 10 ** 17, // 50 DAI - 50 BANANA -- Reward: 0.3 BANANA
    6 * 10 ** 17  // 75 DAI - 75 BANANA -- Reward: 0.6 BANANA
  ];

  constructor(BananaToken _bananaAddress, Dai _daiAddress, BananaDao _mainBananaDaoAddress) {
    owner = msg.sender;
    bananaAddress = _bananaAddress;
    daiAddress = _daiAddress;
    mainBananaDaoAddress = _mainBananaDaoAddress;
  }
  ...
  function mintNode(address _address, uint _bananaAmount, uint _daiAmount, uint _nodeType) public {
    ...
    require(_nodeType >= 0 && _nodeType <= 3, 'Invalid node type');
    ...
    if(_nodeType == 0){
      require(_bananaAmount >= 25 * 10 ** 18, 'You must provide at least 25 BANANA for the LP token');
      require(_daiAmount >= 25 * 10 ** 18, 'You must provide at least 25 DAI for the LP token');
      account.type25NodeCount++;
      totalNodes++;
    }
    else if(_nodeType == 1){
      require(_bananaAmount >= 50 * 10 ** 18, 'You must provide at least 50 BANANA for the LP token');
      require(_daiAmount >= 50 * 10 ** 18, 'You must provide at least 50 DAI for the LP token');
      account.type50NodeCount++;
      totalNodes++;
    }
    else if(_nodeType == 2){
      require(_bananaAmount >= 75 * 10 ** 18, 'You must provide at least 75 BANANA for the LP token');
      require(_daiAmount >= 75 * 10 ** 18, 'You must provide at least 75 DAI for the LP token');
      account.type75NodeCount++;
      totalNodes++;
    }

    accounts[_address] = account;

    bananaAddress.transferFrom(_address, address(mainBananaDaoAddress), _bananaAmount);
    daiAddress.transferFrom(_address, address(mainBananaDaoAddress), _daiAmount);
  }
  ...
  function widthrawInterest(address _to) public {
    ...
    // This contract will need tokens fed weekly to pay interest
  ...
  function payInterest() public {
    ...
    uint i;
    for(i=0; i<bananaNodesAddresses.length; i++){
      address a = bananaNodesAddresses[i];
      Account memory acc = accounts[a];
      uint interestAccumulated;
      interestAccumulated = acc.type25NodeCount * nodeRewards[0];
      interestAccumulated += acc.type50NodeCount * nodeRewards[1];
      interestAccumulated += acc.type75NodeCount * nodeRewards[2];
  ...
  function withdrawBanana(address _address, uint amount) public {
    require(msg.sender == owner, 'You must be the owner to run this.');
    bananaAddress.transfer(_address, amount);
  }

  function withdrawDai(address _address, uint amount) public {
    require(msg.sender == owner, 'You must be the owner to run this.');
