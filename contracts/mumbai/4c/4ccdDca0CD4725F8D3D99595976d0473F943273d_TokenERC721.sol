    ...
    ITokenERC721
{
    ...
    /// @dev token id => series number
    mapping(uint256 => uint8) public series;

    /// @dev token id => rarity
    mapping(uint256 => RarityType) public rarity;

    /// @dev wallet => burn tickets count
    mapping(address => uint256) public burnTickets;

    /// @dev count of burn ticket for each rarity
    uint8[] public burnTicketCountOfRarity = [1, 5, 10];
    ...
    function mintTo(address _to, string calldata _uri, uint8 _series, RarityType _rarity) external onlyRole(MINTER_ROLE) returns (uint256) {
        // `_mintTo` is re-used. `mintTo` just adds a minter role check.
        return _mintTo(_to, _uri, _series, _rarity);
    }
        ...
        returns (uint256 tokenIdMinted)
    {
        ...
        tokenIdMinted = _mintTo(receiver, _req.uri, _req.series, _req.rarity);
    ...
    function _mintTo(address _to, string calldata _uri, uint8 _series, RarityType _rarity) internal returns (uint256 tokenIdToMint) {
        ...
        series[tokenIdToMint] = _series;
        rarity[tokenIdToMint] = _rarity;
    ...
    function burn(uint256 tokenId) public virtual {
        ...
        _burn(tokenId);
        if (series[tokenId] == 2) {
            burnTickets[msg.sender] += burnTicketCountOfRarity[uint8(rarity[tokenId])];
        }
    }
...
interface ITokenERC721 is IERC721Upgradeable {
     ...
     */
    function mintTo(address to, string calldata uri, uint8 series, RarityType rarity) external returns (uint256);
