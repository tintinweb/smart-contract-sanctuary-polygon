...
interface IManager {
    ...
    function setLocker(address _staking) external;
...
interface IVoteOlive {
    ...
    /// @notice Set the veolive contract
    /// @param _veolive Address of the contract
    function setVeOlive(address _veolive) external;
...
contract Manager is IManager, Initializable, AccessControl, ReentrancyGuard {
    ...
    modifier onlyBalanceUpdaters() {
        require(hasRole(ADMIN_ROLE, _msgSender()) || _msgSender() == voting || _msgSender() == locker || pools.contains(_msgSender()), "NOT_ALLOWED");
        _;
    }

    modifier onlyPool() {
        require(pools.contains(_msgSender()), "NOT_POOL");
        _;
    }

    function initialize(uint256 _cycleDuration) public initializer {
        require(_cycleDuration > 0, "Cycle duration cannot be 0");
        __Context_init_unchained();
        __AccessControl_init_unchained();
    ...
    function registerController(bytes32 id, address controller) external override onlyAdmin {
        require(controllerIds.add(id), "CONTROLLER_EXISTS");
    ...
    function unRegisterController(bytes32 id) external override onlyAdmin {
        require(controllerIds.remove(id), "INVALID_CONTROLLER");
    ...
    function registerPool(address pool) external override onlyAdmin {
        require(pools.add(pool), "POOL_EXISTS");
        emit PoolRegistered(pool);
    }

    function unRegisterPool(address pool) external override onlyAdmin {
        require(pools.remove(pool), "INVALID_POOL");
        emit PoolUnregistered(pool);
    }

    function setVoting(address _voting) external override onlyAdmin {
        require(voting == address(0), "Already Initialized");
        require(_voting != address(0), "Cannot be zero address");
        voting = _voting;
        emit VotingSet(voting);
    }

    function setLocker(address _locker) external override onlyAdmin {
        // require(locker == address(0), "Already Initialized");
        require(_locker != address(0), "Cannot be zero address");
        locker = _locker;
    }

    function setCycleDuration(uint256 duration) external override onlyAdmin {
        require(duration > 0, "Cannot be 0");
    ...
    function completeRollover(string calldata rewardsIpfsHash) external override onlyRollover {
        require(block.number >= (currentCycle + cycleDuration), "PREMATURE_EXECUTION");
    ...
    function startCycleRollover() external override onlyRollover {
        require(!rolloverStarted, "Rollover in progress");
    ...
    function _completeRollover(string calldata rewardsIpfsHash) private {
        require(rolloverStarted, "Rollover not started");
        ...
        if (voting != address(0)) {
            IVoteOlive(voting).onCycleRollover(currentCycleIndex);
        }
    ...
    function setBalance(SetTokenBalance[] calldata balances) external override onlyAdmin {
        for (uint256 i = 0; i < balances.length; i++) {
            ...
            updateBalance({
                ...
                exchangeAmount: balance.exchangeAmount,
    ...
    function updateBalance(
        ...
        uint256 exchangeAmount,
        bool isPositive
    ) public override onlyBalanceUpdaters {
        ...
        uint256 updatedTotalBalance = currentTotalBalance - currentUserBalance.amount + amount;
        accountTokenBalances[account][token] = TokenBalance({token: token, amount: amount});
        totalTokenBalances[token] = updatedTotalBalance;
        emit BalanceUpdate(account, token, amount, exchangeAmount, isPositive);
    }

    function requestWithdrawalEvent(
        address account,
        address token,
        uint256 amount,
        uint256 cycle
        ...
        emit WithdrawalRequested(account, token, amount, cycle);
    }
}
