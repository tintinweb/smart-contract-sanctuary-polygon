...
contract Bazooka is Context, IBEP20, Ownable {
  ...
  constructor()  {
    ...
    priceFeedMAtic = AggregatorV3Interface(0xd0D5e3DB44DE05E9F294BB0a3bEEaF030DE24Ada);//test net
        tokenPerUsd = 3;
        minimumDeposite = 0.0001 ether;
        maximumDeposite = 3 ether;
   ...
   * @dev Start the sale.
   */ 
  function startICO(uint256 _endDate) public onlyOwner returns(bool){
   ...
   */
   ...
   // to change Price of the token
    ...
    function maticToToken(uint256 _amount) public view returns(uint256){
        ...
        uint256 maticToUsd = precision.mul(_amount).mul(getLatestPriceMatic()).div(1e18);
        uint256 numberOfTokens = maticToUsd.mul(tokenPerUsd);
