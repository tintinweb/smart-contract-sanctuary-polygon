...
contract Paymaster is WhitelistPaymaster {
    function initialize(address relayHub ,address trustedForwarder, address[] memory whiteTargets) initializer public {
        __Ownable_init();
        setTrustedForwarder(trustedForwarder);
        setRelayHub(IRelayHub(relayHub));
        for(uint i=0; i<whiteTargets.length; i++)
        {
            whitelistTarget(whiteTargets[i]);
        }
    }
}
