...
contract Lending {
 ...
 function ReturnToken(uint256 _TokenAmount)
 public
 payable
 {
 require(
 ...
 uint256 Adminfees = (_TokenAmount * ADMIN_FEES) / 100;
 uint256 interest = (_TokenAmount * ROI30) / 100 + Adminfees;
 uint256 Amountredeem = _TokenAmount * BorrwerPool[msg.sender].ETHPRICE - interest;
 ...
 uint256 Adminfees = (_TokenAmount * ADMIN_FEES) / 100;
 uint256 interest = (_TokenAmount * ROI60) / 100 + Adminfees;
 uint256 Amountredeem = _TokenAmount * BorrwerPool[msg.sender].ETHPRICE - interest;
 GCT_TOKEN.transferFrom(msg.sender, ADMIN, _TokenAmount);
 payable(msg.sender).transfer(Amountredeem);
 delete BorrwerPool[msg.sender];
 ...
 uint256 Adminfees = (_TokenAmount * ADMIN_FEES) / 100;
 uint256 interest = (_TokenAmount * ROI90) / 100 + Adminfees;
 uint256 Amountredeem = _TokenAmount * BorrwerPool[msg.sender].ETHPRICE - interest;
 GCT_TOKEN.transferFrom(msg.sender, ADMIN, _TokenAmount);
 payable(msg.sender).transfer(Amountredeem);
 delete BorrwerPool[msg.sender];
