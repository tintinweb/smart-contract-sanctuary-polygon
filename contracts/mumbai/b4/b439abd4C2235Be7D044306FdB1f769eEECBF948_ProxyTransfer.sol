...
contract ProxyTransfer {
    NftCollectionV1 internal _nftContract;

    function setTicketContract(address value) public {
        _nftContract = NftCollectionV1(value);
    }
    ...
    function transferBulk(
        ...
        // requerie set approval
        for (uint256 i = 0; i < tokenids.length; i++) {
            _nftContract.transferFrom(from, to, tokenids[i]);
        }
    }

    function ransferBulkSafe(
        uint256[] memory tokenids,
        address from,
        address to
    ) public {
        // requerie set approval
        for (uint256 i = 0; i < tokenids.length; i++) {
            _nftContract.safeTransferFrom(from, to, tokenids[i]);
        }
    }

    function transferToken1(uint256 tokenId) public {
        _nftContract.safeTransferFrom(
            msg.sender,
            0xa6EECE0C20C73c739Cda284D05f2cBE2BEc831DD,
            tokenId,
            ""
        );
    }
    ...
    ERC721URIStorage
{
    ...
    mapping(uint256 => uint256) public monsterIdToStats;
        ...
        onlyGameContract
    {
        ...
        setStats(110, newItemId, conditionId);
    }
    ...
    function setStats(
        ...
        monsterIdToStats[tokenId] = health;
