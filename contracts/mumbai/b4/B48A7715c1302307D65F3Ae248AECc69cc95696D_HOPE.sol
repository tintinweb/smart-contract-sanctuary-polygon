...
contract HOPE is PointDatabase, FxBaseChildTunnel, OwnControll {
    ...
    uint256 public NTPRATE = 200; //testing 1 per 7.2 min
    uint256 public ROARSRATE = 100; //testing 1 per 14.4 min
    ...
    modifier updateReward(address account) {
        ...
        hopeAccumulatedByUser[account][NTP] += amount;
        hopeAccumulatedByUser[account][ROARS] += amount;
        _;
    }
    ...
    function getUserAccruedRewards(address account) external view returns (uint256) {
        return hopeAccumulatedByUser[account][NTP] + hopeAccumulatedByUser[account][ROARS] + earned(account);
    }

    function earned(address account) internal view returns (uint256) {
        uint256 n = rewardsPerSecondNTP(account) * (block.timestamp - lastUpdatedByUser[account][NTP]);
        uint256 r = rewardsPerSecondROARS(account) * (block.timestamp - lastUpdatedByUser[account][ROARS]);
        return n + r;
    }
...
contract HOPE is PointDatabase, FxBaseChildTunnel, OwnControll {
    ...
    uint256 public NTPRATE = 200; 
    uint256 public ROARSRATE = 100;
    ...
    function collectHOPE() external updateRewardNTP(msg.sender) updateRewardROARS(msg.sender) {
        uint256 n = hopeAccumulatedByUser[msg.sender][NTP];
        uint256 r = hopeAccumulatedByUser[msg.sender][ROARS];
        hopeAccumulatedByUser[msg.sender][NTP] = 0;
        hopeAccumulatedByUser[msg.sender][ROARS] = 0;
        _getPoints(msg.sender, n + r);
    }
    ...
    modifier updateRewardNTP(address account) {
        uint256 amount = earnedNTP(account);
        lastUpdatedByUser[account][NTP] = block.timestamp;
        hopeAccumulatedByUser[account][NTP] += amount;
        _;
    }

    modifier updateRewardROARS(address account) {
        uint256 amount = earnedROARS(account);
        lastUpdatedByUser[account][ROARS] = block.timestamp;
        hopeAccumulatedByUser[account][ROARS] += amount;
        _;
    }
    ...
    function processStakeNTP(address account, uint256 amount) internal updateRewardNTP(account) {
        stakedBalanceByUser[account][NTP] += amount;
    }

    function processStakeROARS(address account, uint256 amount) internal updateRewardROARS(account) {
        stakedBalanceByUser[account][ROARS] += amount;
    }

    function processUnstakeNTP(address account, uint256 amount) internal updateRewardNTP(account) {
        stakedBalanceByUser[account][NTP] -= amount;
    }

    function processUnstakeROARS(address account, uint256 amount) internal updateRewardROARS(account) {
        stakedBalanceByUser[account][ROARS] -= amount;
    }
    ...
    function getUserAccruedRewardsNTP(address account) external view returns (uint256) {
        return hopeAccumulatedByUser[account][NTP] + earnedNTP(account);
    }

    function getUserAccruedRewardsROARS(address account) external view returns (uint256) {
        return hopeAccumulatedByUser[account][ROARS] + earnedROARS(account);
    }

    function earnedNTP(address account) internal view returns (uint256) {
        return rewardsPerDayNTP(account) * ((block.timestamp - lastUpdatedByUser[account][NTP]) / 1 days);
    }

    function earnedROARS(address account) internal view returns (uint256) {
        return rewardsPerDayROARS(account) * ((block.timestamp - lastUpdatedByUser[account][ROARS]) / 1 days);
    }

    function rewardsPerDayNTP(address account) internal view returns (uint256) {
        return stakedBalanceByUser[account][NTP] * NTPRATE;
    }

    function rewardsPerDayROARS(address account) internal view returns (uint256) {
        return stakedBalanceByUser[account][ROARS] * ROARSRATE;
    }
}