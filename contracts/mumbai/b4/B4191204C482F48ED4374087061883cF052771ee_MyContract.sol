...
contract MyContract is VRFV2WrapperConsumerBase {
    ...
    event BetResult(BetSelection indexed result, uint indexed randomWords);
    ...
    uint32 constant callbackGasLimit = 10_000_000;
    ...
    function fulfillRandomWords(
        ...
        if (scaledNumber < 4586) {
            ...
            if (statuses[requestId].choice == result) {
                ...
                uint256 transferAmount = (statuses[requestId].betAmount * 195) /
                    100; // Calculate the transfer amount
                require(
            ...
            if (statuses[requestId].choice == result) {
                statuses[requestId].didWin = true;
                uint256 transferAmount = (statuses[requestId].betAmount * 200) /
                    100; // Assuming no extra gain on winning for player
                require(
                    address(this).balance >= transferAmount,
                    "Not enough balance in the contract"
                ); // Ensure the contract has enough balance
            ...
            if (statuses[requestId].choice == result) {
                statuses[requestId].didWin = true;
                uint256 transferAmount = (statuses[requestId].betAmount * 900) /
                    100; // Assuming 8x gain on winning for tie
                require(
                    address(this).balance >= transferAmount,
                    "Not enough balance in the contract"
                ); // Ensure the contract has enough balance
            ...
            emit BetResult(result, randomWords[0]);
        }
    }
