...
contract GameMarketplace is VRFConsumerBaseV2, ConfirmedOwner {
    ...
    event PoolCreated(uint256 indexed poolId, uint256 categoryId);
    event ItemAddedToPool(uint256 itemId, uint256 poolId);
    event ItemCreated(uint256 itemId, uint256 categoryId, address owner);
    ...
    function createItem(uint256 _itemId, uint256 _categoryId) public {
        ...
        emit ItemCreated(_itemId, _categoryId, msg.sender);
    }
    ...
    function addItemToPool(uint256 _itemId, uint256 _categoryId) public virtual {
        ...
        erc721.safeTransferFrom(msg.sender, address(this), _itemId);
    ...
    function returnItemToOwner(uint256 _itemId) public {
        Item memory item = getItem(_itemId);
        require(item.owner == msg.sender, "Only the contract owner can return the item");
        IERC721 erc721 = IERC721(nftContractAddress);
        erc721.safeTransferFrom(address(this), item.owner, _itemId);
    }
