...
contract NFTicket is ERC721, ERC721Enumerable, Pausable, Ownable {
    ...
    uint256 public presale_price = 20 ether;
    uint256 public salePrice = 30 ether;
    uint256 public price;

    constructor() ERC721("NFTicket", "TKT") {
        price = presale_price;
    }
    ...
    function mintPresale(uint256 _amount) public payable {
        ...
        require( _amount > 0 && _amount <= MAX_MINT_PER_TX, "Can only mint between 1 and 2 tokens at once" );
        require( supply + _amount <= MAX_PRESALE_SUPPLY,    "Can't mint more than max supply" );
        require( msg.value == price * _amount,              "Wrong amount of ETH sent" );
        for(uint256 i; i < _amount; i++){
            _safeMint( msg.sender, supply + i );
        }
        if (supply + _amount == MAX_PRESALE_SUPPLY) {
            presaleActive = false;
            saleActive = true;
        }
    }
