...
contract PledgeStakeV2 {
    ...
    function calculateLevelRewards(address _user, uint256 _roundRewards) public view returns (uint256) {
        uint256 levelRewards;
        address upline = userInfo[_user].referrer;
        for (uint256 k = 0; k < LEVEL_COUNT; k++) {
            if (upline != address(0)) {
                uint256 levelRewardPercentage = levelPercentage[k];
                uint256 levelReward = _roundRewards.mul(levelRewardPercentage).div(100);
    ...
    function getTotalRewards(address _user) external view returns (uint256) {
        ...
        uint256 levelRewards = calculateLevelRewards(_user, roundRewards);
