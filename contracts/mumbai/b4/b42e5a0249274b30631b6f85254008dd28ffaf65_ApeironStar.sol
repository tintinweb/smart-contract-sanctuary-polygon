    ...
    UUPSUpgradeable
{
    ...
    // Mapping from token ID to StarData
    mapping(uint256 => StarData) private starDataMap;

    // event
    event UpdateStar(uint256 indexed _tokenId, uint256 _genId);
    ...
    function safeMint(
        ...
        starDataMap[_tokenId].geneId = _gene;
    }
    ...
    function updateStarData(uint256 _tokenId, uint256 _genId)
        external
        hasTokenId(_tokenId)
        whenNotPaused
        onlyAdmin
    {
        starDataMap[_tokenId].geneId = _genId;
        emit UpdateStar(_tokenId, _genId);
    }

    function getStarData(uint256 _tokenId)
        external
        view
        returns (
            StarData memory, //starData
            bool //isAlive
        )
    {
        return (starDataMap[_tokenId], _exists(_tokenId));
    }
