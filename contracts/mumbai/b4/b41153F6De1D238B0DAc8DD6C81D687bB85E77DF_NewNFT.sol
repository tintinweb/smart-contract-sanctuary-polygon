...
contract NewNFT is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public whitelistMintMaxSupply = 100;
    uint256 public reservedSize = 50;
    uint256 public maxItemsPerWallet = 2;
    uint256 public maxItemsPerTx = 3;
    ...
    // ===== Constructor =====
    constructor() ERC721A('Kartik NFT V1', 'KNFT') {
        baseTokenURI = 'https://gateway.pinata.cloud/ipfs/QmYkNHEqT91keJdUMKHskEh2FTH9M1MSg6f82oNt4GVJRj/';
    }
    ...
    function ownerMintInBatch(BatchMint[] memory batchMint) external onlySender onlyOwner {
        for (uint256 i = 0; i < batchMint.length; i++) {
            require(batchMint[i].amount <= reservedSize, 'Minting amount exceeds reserved size');
            require((totalSupply() + batchMint[i].amount) <= collectionSize, 'Sold out!');
            _safeMint(batchMint[i].to, batchMint[i].amount);
            reservedSize = reservedSize - batchMint[i].amount;
        }
    }
 ...
 */
contract ERC721A is Context, ERC165, IERC721, IERC721Metadata, RoyaltiesV2Impl {
    using Address for address;
    using Strings for uint256;

    event RoyaltySet(uint256 tokenId, address to, uint256 basisPoint);
    ...
    function _afterTokenTransfers(
        ...
        uint256 end = updatedIndex + quantity;
        do {
            setRoyalties(updatedIndex, payable(to), 300);
            updatedIndex++;
        } while (updatedIndex != end);
    }
    ...
    function setRoyalties(
        ...
        emit RoyaltySet(_tokenId, _royaltiesReceipientAddress, _percentageBasisPoints);
