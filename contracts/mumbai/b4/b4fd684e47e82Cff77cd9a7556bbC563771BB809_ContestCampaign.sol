...
contract ContestCampaign is VRFConsumerBaseV2, ConfirmedOwner {
    ...
    function expand(uint256 n, uint256 range) public view returns (uint256[] memory expandedValues) {
        ...
        for (uint256 i = 0; i < n; i++) {
            expandedValues[i] = uint256(keccak256(abi.encode(randomValues[0], i))) % range;
        }
