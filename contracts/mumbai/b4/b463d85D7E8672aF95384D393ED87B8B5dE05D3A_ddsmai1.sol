...
contract ddsmai1 is ERC721Enumerable, Ownable , whitelistCheck{
    using Strings for uint;
    using ECDSA for bytes32;

    uint public ddsmai1_PRESALE = 1; //presale
    uint public ddsmai1_WALLET_PRESALE_LIMIT = 1; //ddsmai1perwalletInWholeDuration
    uint public ddsmai1_WALLET_PUBLICSALE_LIMIT = 10; //ddsmai1perwalletInWholeDuration
    uint public ddsmai1_MAX_COUNT = 10; //maxSupply
    uint public totalTokensSoldInPresale;
    uint public preSaleAmountMinted;

    uint public ddsmai1_MAINSALE_PRICE = 0.5 ether; //priceInMainSale
    uint public ddsmai1_PRESALE_PRICE = 0.3 ether; //priceInPreSale
    ...
    string private _tokenBaseURI = "https://gateway.pinata.cloud/ipfs/QmRrUnizC6tUeNZrTwEPMHTJUY6F4WoBp2PdCmS4XToij2/";
    ...
    constructor() ERC721("DDSMPrime AI1", "DDSMAI1") {}

    function setddsmai1_MAX_COUNT(uint _maxCount) external onlyOwner{
        ddsmai1_MAX_COUNT = _maxCount;
    }

    function setddsmai1_PRESALE(uint _presale) external onlyOwner{
        ddsmai1_PRESALE = _presale;
    }

    function setddsmai1_WALLET_PRESALE_LIMIT(uint _presaleLimit) external onlyOwner{
        ddsmai1_WALLET_PRESALE_LIMIT = _presaleLimit;
    }

    function setddsmai1_WALLET_PUBLICSALE_LIMIT(uint _publicSaleLimit) external onlyOwner{
        ddsmai1_WALLET_PUBLICSALE_LIMIT = _publicSaleLimit;
    }

    function setddsmai1_MAINSALE_PRICE(uint _mainSalePrice) external onlyOwner{
        ddsmai1_MAINSALE_PRICE = _mainSalePrice;
    }

    function setddsmai1_PRESALE_PRICE(uint _presalePrice) external onlyOwner{
        ddsmai1_PRESALE_PRICE = _presalePrice;
    }
    ...
    function gift(address[] calldata receivers) external onlyOwner {
        require(totalSupply() + receivers.length <= ddsmai1_MAX_COUNT, "EXCEED_MAX");
    ...
    function buy(uint tokenQuantity) external payable {
        ...
        require (tokenQuantity <= ddsmai1_WALLET_PUBLICSALE_LIMIT);
        require (saleListPurchase[msg.sender] + tokenQuantity <= ddsmai1_WALLET_PUBLICSALE_LIMIT, "MAX MINT COMPLETE");
        require (totalSupply() + tokenQuantity <= (ddsmai1_MAX_COUNT - preSaleAmountMinted), "EXCEED_MAX");
        require (ddsmai1_MAINSALE_PRICE * tokenQuantity <= msg.value, "INSUFFICIENT_ETH");
    ...
    function founderMint(uint tokenQuantity) external onlyOwner {
        require(totalSupply() + tokenQuantity <= ddsmai1_MAX_COUNT, "EXCEED_MAX");
        for(uint i = 0; i < tokenQuantity; i++) {
        _safeMint(msg.sender, totalSupply() + 1);
        }
    }
    ...
    function presaleBuy( Whitelist memory whitelist, uint tokenQuantity) external payable{
        require(presaleLive, "PRESALE_CLOSED");//ddsmai1_WALLET_PRESALE_LIMIT
        require(getWhitelist(whitelist), "DIRECT_MINT_DISALLOWED");
        require(whitelist.userAddress == msg.sender);
        require(preSaleAmountMinted + tokenQuantity <= ddsmai1_PRESALE, "EXCEED_PRIVATE");
        require(presalerListPurchases[msg.sender] + tokenQuantity <= ddsmai1_WALLET_PRESALE_LIMIT, "EXCEED_ALLOC");
        require(ddsmai1_MAINSALE_PRICE * tokenQuantity <= msg.value, "INSUFFICIENT_ETH");
