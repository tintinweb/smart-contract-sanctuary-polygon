...
import "./muon/IMuonV02.sol";

/**
 * @dev FIXME.
 */
contract FearNftMuon721 is ERC721Enumerable, AccessControl, ReentrancyGuard, OpenSeaCompatible, IMRC721Metadata {
    ...
    */
    ...
    struct WolfParams {
        //bool clanLeader; // ? encapsulate in rank?
    ...
    uint256 public MUON_RANOM_NFT_APP_ID = 13;
    ...
    function _setTokenParams(uint256 id, uint8 rank, uint8 clan, uint8 skin,
                             Traits memory traits,
                             UpgradeRates memory upgradeRate) private {
        wolfParams[id] = WolfParams(rank, clan, skin, traits, upgradeRate);
    }
    ...
    function fulfillRandomness(uint256 id, uint256 randomness) internal {
        require(id > 0, "Unknown token randomness");

        uint256[] memory randomValues = new uint256[](10);
        ...
        params.rank = uint8(randomValues[0] % 3);
        params.clan = uint8(randomValues[1] % 5);
        params.skin = uint8(randomValues[2] % 10);

        params.upgradeRates.strengthRate = uint8(randomValues[3] % 10);
        params.upgradeRates.speedRate = uint8(randomValues[4] % 10);
        params.upgradeRates.enduranceRate = uint8(randomValues[5] % 10);
        params.upgradeRates.intelligenceRate = uint8(randomValues[6] % 10);
        params.upgradeRates.howlRate = uint8(randomValues[7] % 10);
        params.upgradeRates.spiritRate = uint8(randomValues[8] % 10);
        params.upgradeRates.loyaltyRate = uint8(randomValues[9] % 10);
    }

    function initialMint(address initialOwner, uint256 randomNumber, bytes calldata _reqId, IMuonV02.SchnorrSign[] calldata _sigs) external nonReentrant onlyRole(MINTER_ROLE) {
        ...
        fulfillRandomness(nextTokenId, randomNumber);
        nextTokenId++;
    }
    ...
    function mint(address to, uint256 id, bytes calldata data) external onlyRole(BRIDGER_ROLE) {
        ...
        bytes memory upgradeRatesData) = abi.decode(data, (uint8, uint8, uint8, bytes, bytes));

        _setTokenParams(id, rank, clan, skin, _decodeTraits(traitsData), _decodeUpgradeRates(upgradeRatesData));
    }
