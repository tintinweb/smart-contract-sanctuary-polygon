    ...
    ExpeditionMeta
{
    ...
    /// @param _treasureIds Array of Treasure index
    function claimTreasure(uint256[] memory _treasureIds)
        external
        nonReentrant
    {
        for (uint256 i = 0; i < _treasureIds.length; i++) {
            require(
                treasurePerId[_treasureIds[i]].user == msg.sender &&
                    treasurePerId[_treasureIds[i]].prizes.length > 0,
        ...
        for (uint256 j = 0; j < _treasureIds.length; j++) {
            Prize[] memory prizes = treasurePerId[_treasureIds[j]].prizes;
            for (uint256 i = 0; i < prizes.length; i++) {
                if (prizes[i].prizeType == PRIZE_TYPE.TRANSFER_ERC20) {
                    ...
                    prizes[i].prizeType ==
                    PRIZE_TYPE.MINTABLE_GODIVERSE_COLLECTION
                ) {
            ...
            delete treasurePerId[_treasureIds[j]];
        }
    }
    ...
    StakeAssetMeta
{
    ...
    function setupPeerContracts(
        address _stakingAndKeys,
    ...
    function stakeNFT(
        ...
        require(
            _assetAddress.isContract() && _tokenIds.length > 0,
            "invalid params"
        );
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            require(
                !blacklist.blacklistedNFT(
                    msg.sender,
                    _assetAddress,
                    _tokenIds[i]
                ),
                "tokenIds is blacklisted"
            );
            ...
            //check any duplicated tokenIds with staked assets
            for (uint256 j = 0; j < assets.length; j++) {
                require(
                    assets[j].tokenId != _tokenIds[i],
                    "tokenIds must be different"
                );
            }
        }
        ...
        if (
            ...
            for (uint256 i = 0; i < _tokenIds.length; i++) {
                ...
                require(
                    _tokenIds[i] <= 4585 || planetData.bornTime > 0,
                    "planet is not born"
                );
            }
    ...
    function unstakeNFT(
        ...
        require(
            ...
            "One of expedition is not started or already claimable"
        );
    ...
    StakeAssetMeta
{
    ...
    IERC20Upgradeable public exchangeToken;
    ...
    function stakeAssets(
        ...
        if (assetTypes[_assetAddress] == ASSET_TYPE.ERC20) {
            IERC20Upgradeable erc20 = IERC20Upgradeable(_assetAddress);
        ...
        /*if (assetTypes[_assetAddress] == ASSET_TYPE.ERC1155)*/
        else {
    ...
    function unstakeAssets(
        ...
        if (assetTypes[_assetAddress] == ASSET_TYPE.ERC20) {
            ...
            for (uint i = 0; i < _tokenIds.length; i++) {
                erc20.safeTransfer(_stakeholder, _amounts[i]);
            }
        } else if (assetTypes[_assetAddress] == ASSET_TYPE.ERC721) {
            IERC721 erc721 = IERC721(_assetAddress);
    ...
    function setupExchangeConfig(
        ...
        exchangeToken = IERC20Upgradeable(_exchangeTokenAddress);
    ...
    function exchangeKeys(
        ...
        if (tokenToKey) {
            ...
            exchangeToken.safeTransfer(_stakeholder, tokenAmount);
        }
    }
        ...
        onlyOwner
    {
        ...
        exchangeToken.safeTransfer(_wallet, _amount);
    }
}
