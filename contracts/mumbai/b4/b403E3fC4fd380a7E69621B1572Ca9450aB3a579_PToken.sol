...
contract PToken is VersionedInitializable, IPToken {
  ...
  address private _borrowingAsset;
  uint8 private _supplyingAssetDecimals;
  uint8 private _borrowingAssetDecimals;
  ...
  struct UserState {
    ...
    uint256 prxyBalance;
    uint40 lastUpdateTimestamp;
    uint256 supplyInterestRate; // in Ray
    uint256 borrowInterestRate; // in Ray
    uint256 prxyInterestRate; // in Ray
  ...
  uint256 private _borrowTotalBalance;
  uint256 private _prxyTotalBalance;
  uint256 private _supplyTotalInterestAmount;
  uint256 private _borrowTotalInterestAmount;
  ...
  function initialize(
    ...
    _borrowingAsset = borrowingAsset;
    _supplyingAssetDecimals = IERC20Detailed(supplyingAsset).decimals();
    _borrowingAssetDecimals = IERC20Detailed(borrowingAsset).decimals();
  ...
  function balanceOf(address user) external view returns (uint256) {
    ...
    if(_userState[user].referralCode == 0) {
      uint256 supplyInterestAmount = _userState[user].supplyBalance.rayMul(_userState[user].supplyInterestRate) * dt;
      curSupplyBalance += supplyInterestAmount;
    }

    if(_userState[user].borrowBalance > 0) {
      uint256 borrowInterestAmount = _userState[user].borrowBalance.rayMul(_userState[user].borrowInterestRate) * dt;
      if(curSupplyBalance < borrowInterestAmount) return 0;
      curSupplyBalance -= borrowInterestAmount;
    }
  ...
  function borrowBalanceOf(address user) external view returns (uint256) {
    return _userState[user].borrowBalance / WadRayMath.RAY;
  }
  ...
  function prxyBalanceOf(address user) external view returns (uint256) {
    ...
    if(_userState[user].referralCode == 1 && _userState[user].supplyBalance > 0) {
      uint256 prxyInterestAmount = _userState[user].supplyBalance.rayMul(_userState[user].prxyInterestRate) * dt;
      curPrxyBalance += prxyInterestAmount;
    }

    return curPrxyBalance / WadRayMath.RAY;
  }
  ...
  function totalSupply() external view returns (uint256) {
    ...
    uint256 curSupplyTotal = _supplyTotalBalance + _supplyTotalInterestAmount * dt;

     if(_borrowTotalBalance > 0) {
  ...
  function totalBorrow() external view returns (uint256) {
    return _borrowTotalBalance / WadRayMath.RAY;
  }
  ...
  function totalPrxy() external view returns (uint256) {
    ...
    return curPrxyTotal / WadRayMath.RAY;
  }
  ...
  function supply(address user, uint256 amount, uint16 referralCode) external virtual override onlyPool {
    ...
    if (_userState[user].supplyBalance == 0) {
      if(referralCode == 0) {
        _userState[user].supplyInterestRate = POOL.getSupplyInterestRate(_underlyingAsset);
        _userState[user].prxyInterestRate = 0;
      } else {
        _userState[user].supplyInterestRate = 0;
        _userState[user].prxyInterestRate = POOL.getPrxyInterestRate(_underlyingAsset);
      }
      _userState[user].supplyBalance = 0;
      _userState[user].borrowBalance = 0;
    ...
    uint256 scaledAmount = amount * WadRayMath.RAY;
    _userState[user].supplyBalance += scaledAmount;
    _supplyTotalBalance += scaledAmount;

    if(_userState[user].referralCode == 0)
      _supplyTotalInterestAmount += scaledAmount.rayMul(_userState[user].supplyInterestRate);
    else
      _prxyTotalInterestAmount += scaledAmount.rayMul(_userState[user].prxyInterestRate);
    _userState[user].lastUpdateTimestamp = _lastUpdateTimestamp = uint40(block.timestamp);
  }

  /// @inheritdoc IPToken
  function borrow(address user, uint256 amount) external virtual override onlyPool {
    _updateState(user);

    if (_userState[user].borrowBalance == 0) {
      _userState[user].borrowInterestRate = POOL.getBorrowInterestRate(_underlyingAsset);
    }

    uint256 scaledAmount = amount * WadRayMath.RAY;
    _userState[user].borrowBalance += scaledAmount;
    _borrowTotalBalance += scaledAmount;

    _borrowTotalInterestAmount += scaledAmount.rayMul(_userState[user].borrowInterestRate);
    _userState[user].lastUpdateTimestamp = _lastUpdateTimestamp = uint40(block.timestamp);
  }
  ...
  function withdraw(address user, uint256 amount) external virtual override onlyPool {
    ...
    uint256 scaledAmount = amount * WadRayMath.RAY;
    require (_userState[user].supplyBalance >= scaledAmount, Errors.INSUFFICIENT_SUPPLY_BALANCE);
    ...
    if(_userState[user].supplyBalance < WadRayMath.RAY) {
      _userState[user].supplyBalance = 0;
      _userState[user].borrowBalance = 0;
    } 
  }
  ...
  /// @inheritdoc IPToken
  function repay(address user, uint256 amount) external virtual override onlyPool {
    _updateState(user);

    uint256 scaledAmount = amount * WadRayMath.RAY;
    require (_userState[user].borrowBalance >= scaledAmount, Errors.INSUFFICIENT_BORROW_BALANCE);
  ...
  function liquidate(address user) external virtual override onlyPool {
    _updateState(user);
    uint256 supplyAmount = _userState[user].supplyBalance;
    _supplyTotalBalance -= supplyAmount;
    _supplyTotalInterestAmount -= supplyAmount.rayMul(_userState[user].supplyInterestRate);
    uint256 borrowAmount = _userState[user].borrowBalance;
    _borrowTotalBalance -= borrowAmount;
    _borrowTotalInterestAmount -= borrowAmount.rayMul(_userState[user].borrowInterestRate);

    IERC20(_underlyingAsset).safeTransfer(_addressesProvider.getTradingWallet(), supplyAmount / WadRayMath.RAY);
  ...
  function claimPrxy(address user, uint256 amount) external virtual override onlyPool {
    _updateState(user);
    uint256 scaledAmount = amount * WadRayMath.RAY;
    require(_userState[user].prxyBalance >= scaledAmount, Errors.INSUFFICIENT_PRXY_BALANCE);
    _userState[user].prxyBalance -= scaledAmount;
    _userState[user].lastUpdateTimestamp = _lastUpdateTimestamp = uint40(block.timestamp);
  }

  /// @inheritdoc IPToken
  function reduxInterestToTreasury() external virtual override onlyPool returns (uint256) {
    _updateReduxInterest();
    uint256 amount = reduxInterest.reduxAmount / WadRayMath.RAY;
    if(amount > 0)  IERC20(_underlyingAsset).safeTransfer(_treasury, amount);
    reduxInterest.reduxAmount = 0;

    reduxInterest.snapshotTimestamp = uint40(block.timestamp);
    return amount;
  }

  function _updateReduxInterest() internal {
    if(_borrowTotalInterestAmount > 0) 
      reduxInterest.reduxAmount += _borrowTotalInterestAmount * (block.timestamp - reduxInterest.lastUpdateTimestamp);
    reduxInterest.lastUpdateTimestamp = uint40(block.timestamp);
  }

  function _updateState(address user) internal {
    // Update the redux interest
    _updateReduxInterest();

    uint256 dT = block.timestamp - uint256(_lastUpdateTimestamp);
    // Update the supply total balance
    _supplyTotalBalance += _supplyTotalInterestAmount * dT;
    if(_borrowTotalBalance > 0) {
      uint256 borrowTotalInterest = _borrowTotalInterestAmount * dT;
    ...
    // Update the prxy total balance
    _prxyTotalBalance += _prxyTotalInterestAmount * dT;

    if(_userState[user].supplyBalance > 0) {
      uint256 dt = block.timestamp - uint256(_userState[user].lastUpdateTimestamp);
      // Update the user supply balance for Native Interest
      uint256 prevSupplyBalance = _userState[user].supplyBalance;
      uint256 nextSupplyBalance = _userState[user].supplyBalance;

      if(_userState[user].referralCode == 0)
        nextSupplyBalance += _userState[user].supplyBalance.rayMul(_userState[user].supplyInterestRate) * dt;

      if(_userState[user].borrowBalance > 0) {
        uint256 borrowInterestAmount = _userState[user].borrowBalance.rayMul(_userState[user].borrowInterestRate) * dt;
        require(nextSupplyBalance >= borrowInterestAmount, Errors.INVALID_BALANCE);
        nextSupplyBalance -= borrowInterestAmount;
      }

      // Update the user prxy balance for PRXY Interest
      if(_userState[user].referralCode == 1) {
        _userState[user].prxyBalance += _userState[user].supplyBalance.rayMul(_userState[user].prxyInterestRate) * dt;
      }
      _userState[user].supplyBalance = nextSupplyBalance;

      // Update the total interest amount
      uint256 dSupply;
      if(prevSupplyBalance < nextSupplyBalance) {
        dSupply = nextSupplyBalance - prevSupplyBalance;
        if(_userState[user].referralCode == 0)
          _supplyTotalInterestAmount += dSupply.rayMul(_userState[user].supplyInterestRate);
        else 
          _prxyTotalInterestAmount += dSupply.rayMul(_userState[user].prxyInterestRate);
      } else {
        dSupply = prevSupplyBalance - nextSupplyBalance;
        if(_userState[user].referralCode == 0)
          _supplyTotalInterestAmount -= dSupply.rayMul(_userState[user].supplyInterestRate);
        else 
          _prxyTotalInterestAmount -= dSupply.rayMul(_userState[user].prxyInterestRate);
  ...
  /// @inheritdoc IPToken
  function borrowDecimals() external view override returns (uint8) {
    return _borrowingAssetDecimals;
  }
...
interface IPToken is IERC20Detailed, IInitializablePToken {
   ...
   * @return The transfered amount of redux interest
   **/
  function reduxInterestToTreasury() external returns(uint256);
...
interface IPool {
  ...
  /**
   * @notice Function to transfer the redux interest to treasury
   **/
  function reduxInterestToTreasuryCall() external;
  ...
  /**
   * @notice Returns the supply interest rate
   * @param supplyingAsset The address of the supplying asset of the reserve
...
library Errors {
  ...
  string public constant INVALID_REFERRAL_CODE = '95'; // Invalid referral code
  string public constant INCORRECT_REFERRAL_CODE = '96'; // Referral code must be equal to the referral code of first supply 
  string public constant INSUFFICIENT_PRXY_BALANCE = '97'; // Insufficient PRXY balance
}
...
library DataTypes {
  struct ReserveData {
    ...
    uint256 borrowRate;
    // the prxy rate. Expressed in wad
    uint256 prxyRate;
  ...
  struct ExecuteReduxInterestToTreasuryCallParams {
    uint16 reservesCount;
  }
