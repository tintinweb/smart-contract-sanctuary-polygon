...
contract Staking is Ownable {
    ...
    */
    function calculateReward(uint256 id) public view returns (uint256) {
        ...
        if(timeDiff <= 2592000) {
            return stakesOfUser[msg.sender][id].amount.div(1e18).mul(tpsRatios[0]).mul(timeDiff);
        } else if(timeDiff <= 7776000) {
            return stakesOfUser[msg.sender][id].amount.div(1e18).mul(tpsRatios[1]).mul(timeDiff);
        } else if(timeDiff <= 15552000) {
            return stakesOfUser[msg.sender][id].amount.div(1e18).mul(tpsRatios[2]).mul(timeDiff);
        } else {
            return stakesOfUser[msg.sender][id].amount.div(1e18).mul(tpsRatios[3]).mul(timeDiff);
        }
    }
    ...
    */
    function claimRewards(uint256 id) public {
        ...
        require(wholeMonthsPassed != 0, "You must wait at least a month before you can claim your rewards!");
