...
contract MarketPlace is ReentrancyGuard {
    ...
    function calculateFee(uint256 _price) internal view returns (uint256) {
        uint256 fee = _price.div(1000).mul(feePercentage.div(1 ether));
...
contract NovelContract is ERC721 {
    ...
    /// @dev
    /// @param title string, novel title
    /// @param content string, first content of the novel
    function createNovel(string calldata title, string calldata content)
        external
        isNotEmpty(title)
        isNotEmpty(content)
    {
        ...
        bytes32 newNovelId = keccak256(
            abi.encodePacked(msg.sender, title, tokenId)
        );
        ...
        emit NovelCreated(newNovelId, title);
    }
    ...
    function addContent(
    ...
    ) external novelExists(_novelId) isNotEmpty(_content) {
        ...
        uint256 novelIndex = getNovelIndex(_novelId);
    ...
    function getNovelIndex(bytes32 _novelId) public view returns (uint256) {
        Novel[] memory _novels = novels;

        for (uint256 i = 0; i < _novels.length; i++) {
            Novel memory target = _novels[i];
            if (target.id == _novelId) return i;
        }
