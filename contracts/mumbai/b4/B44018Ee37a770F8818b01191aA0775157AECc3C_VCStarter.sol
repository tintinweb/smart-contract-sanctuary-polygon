...
contract VCStarter {
    ...
    // function closeCampaign(address _project) external {
    //     _onlyLabOwner(_project);
    //     uint256 campaignId = VCProject(_project).closeCampaign();
    //     emit SttrCampaignClosed(msg.sender, _project, campaignId);
    // }
    ...
    function backerWithdrawDefeated(address _project, IERC20 _currency) external {
        ...
        emit SttrBackerWithdrawal(_projectToLab[_project], _project, campaignId, _currency, backerAmount);
        if (campaignDefeated) {
            emit SttrCampaignDefeated(_projectToLab[_project], _project, campaignId);
        }
    }
    ...
    function labWithdraw(address _project, IERC20 _currency) external {
        _onlyLabOwner(_project);
    ...
    function transferUnclaimedFunds(address _project, IERC20 _currency) external {
        ...
        emit SttrUnclaimedFundsTransferredToPool(lab, _project, campaignId, _currency, amountToPool);
        if (statusDefeated) {
            emit SttrCampaignDefeated(lab, _project, campaignId);
        }
    }
