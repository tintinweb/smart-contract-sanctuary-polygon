...
contract UtilityToken is ERC20, Ownable {
    ...
    function increaseAllowanceToCustody(address spender, uint256 addedValue) public onlyOwner {
        _approve(address(this), spender, allowance(address(this), spender) + addedValue);
    }

    function decreaseAllowanceToCustody(address spender, uint256 subtractedValue) public onlyOwner {
        uint256 currentAllowance = allowance(address(this), spender);
        require(currentAllowance >= subtractedValue, "GovernanceToken: decreased allowance below zero");
        unchecked {
