...
contract StakingProxyMock is AccessControlEnumerable, ReentrancyGuardUpgradeable {
    ...
    function stake(uint256 _poolId, uint256 _amount) external nonReentrant {
        ...
        stakeInfo = StakeInfo(_poolId, blockTimestamp, valueDate, _amount, reward, 0, 0);
        ...
        )
    {
        ...
        if (stakeInfo.amount == 0 || stakeInfo.unStakeTime != 0 || stakeInfo.releasedRounds == pool.releaseTotalRounds)
            return (0, 0, 0);
        if (stakeInfo.valueDate + pool.cliff > blockTimestamp) return (0, 0, 0);

        uint256 lockedDays = ((blockTimestamp - stakeInfo.valueDate) / 1 days) + 1;
    ...
    function unStake(uint256 _poolId) external nonReentrant {
        ...
        _stakeHistories[_msgSender()][_stakeHistoryIndex[_poolId][_msgSender()]].unStakeTime = blockTimestamp;
