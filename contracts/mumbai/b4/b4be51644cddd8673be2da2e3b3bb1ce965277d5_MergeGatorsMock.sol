...
contract MergeGators is Ownable, VRFConsumerBaseV2, IMergeGators, ReentrancyGuard {
    ...
    function merge3NFTs(uint256 _1st, uint256 _2nd, uint256 _3rd) public payable nonReentrant() {
        ...
        (bool success, ) = address(alligators).call{gas: 10_000_000}(mergeParams);
        require(success, "LowCallFailed");
        // IAlligators instance = IAlligators(alligators);
        // instance.merge3alligators(_1st, _2nd, _3rd, msg.sender);
    ...
    function merge3Wjoker(uint256 _1st, uint256 _2nd, uint256 _3rd) public payable nonReentrant() {
        ...
        payable(address(this)).transfer(_calculatePortionToRewardPool());
        taxTreasury.transfer(_calculatePortionToRewardPool());
        bytes memory mergeParams = abi.encodeWithSignature("mergeWjoker(uint256, uint256, uint256, address)",_1st, _2nd, _3rd, msg.sender);        (bool success, ) = address(alligators).call{gas: 10_000_000}(mergeParams);
        require(success, "LowCallFailed");
        // IAlligators instance = IAlligators(alligators);
        // instance.mergeWjoker(_1st, _2nd, _3rd, msg.sender);
        _mergePrize(msg.sender);
    }
...
contract MergeGatorsMock is MergeGators {
    ...
    address payable collection_ = payable(0xA31193619CEbbb2c5b41d6cfa6589F3dC6E8Eb9F);
