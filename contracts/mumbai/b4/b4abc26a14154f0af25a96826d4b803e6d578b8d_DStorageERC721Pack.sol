...
contract DStorageERC721Pack is DStorageERC721Fixed {
    ...
    event PackWithdraw(address indexed user, uint256 tokenId, uint256 content);
    ...
    /**
     * @param name_ pack name
     * @param symbol_ pack symbol
     * @param uri_ base metadata uri
     * @param price_ mint price for one pack
     * @param batch_ max number of packs in one batch mint
     * @param size_ number of tokens in each pack
     * @param token_ random token
     */
    constructor(
        string memory name_,
        ...
        uint256 size_,
        address token_
    ) DStorageERC721Fixed(name_, symbol_, uri_, 0, price_, batch_) {
        require(batch_ > 0, "DStorageERC721Pack: batch size is zero");
        require(size_ > 0, "DStorageERC721Pack: pack size is zero");
        require(
            size_ <= IDStorageERC721Random(token_).batch(),
    ...
    function _mintPack(uint256 amount) private {
        ...
        require(
            r.total() + amount * size <= r.max(),
            "DStorageERC721Pack: exceeds max token supply"
        );
        for (uint256 i = 0; i < amount; i++) {
            total += 1;
            contents[total] = r.total() + 1; // keep ref to first token in pack
     ...
     * @param tokenId pack id to withdraw from
     */
    function withdraw(uint256 tokenId) external {
        ...
        // emit
        emit PackWithdraw(msg.sender, tokenId, c);
