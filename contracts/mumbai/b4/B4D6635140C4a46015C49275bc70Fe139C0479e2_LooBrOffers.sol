...
contract LooBrOffers is Ownable, ERC721BidStorage {
    ...
    ) public checkERC20Whitelist(_erc20Address) {
        _placeBid(_tokenAddress, _tokenId, _price, _duration, _erc20Address);
    }
    ...
    function onERC721Received(
        ...
        require(
            ERC20Interface(_erc20Address).transferFrom(bidder, _from, fee),
            "ERC721Bid#onERC721Received:: TRANSFER_AMOUNT_TO_MARKETPLACE_FAILED"
        );
        // Transfer token to bidder
        ERC721Interface(msg.sender).transferFrom(
            address(this),
            bidder,
            _tokenId
        );
        emit BidAccepted(bidId, msg.sender, _tokenId, bidder, _from, price, 0);
