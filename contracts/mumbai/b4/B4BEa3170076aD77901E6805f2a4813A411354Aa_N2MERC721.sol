...
/// @notice Read our terms of service at https://nfts2me.com/terms-of-service/
...
interface IN2MTestToVerify {
    ...
    /// @notice interface Test notice from interface
    ...
    /// @notice interface Test setTestVar notice from interface
    ...
    /// @notice onlyiface Test setTestVar notice onlyiface
...
interface IN2MFactory {
    error SignatureMismatch();
    error SignatureExpired();
    error InputLengthMismatch();

    event CollectionDeployed(address indexed collectionOwner, bytes16 indexed collectionId, bytes16 indexed deployId, address deployAddress, address erc20PaymentAddress);
    ...
    function createCollection(
        ...
        bytes32 baseURICIDHash,
        bytes32 placeholderImageCIDHash,
        IN2M_ERCStorage.RevenueAddress[] calldata revenueAddresses,
        address iErc20PaymentAddress,
        uint32 totalSupply,
        uint16 royaltyFee,
        bool soulboundCollection,
        IN2M_ERCStorage.MintingType iMintingType,
        bytes32 collectionAndDeployId,
        bytes32 implementationType
    ) payable external;

    function createAndMintCollection(
        string calldata tokenName,
        string calldata tokenSymbol,
    ...
    function predictDeterministicAddress(bytes32 implementationType, bytes32 collectionAndDeployId) external view returns (address);

    function getOwners(address collection, uint256 startId, uint256 endId) external view returns (address[] memory addresses, uint blockNumber, uint blockTimestamp);

    function n2mVersion() external pure returns (uint);
...
import "../interfaces/IN2MCrossFactory.sol";
import "../interfaces/IN2M_ERCCommon.sol";
import "../interfaces/IN2M_ERCLibrary.sol";
import "../interfaces/IN2M_ERCBase.sol";
...
address constant N2M_WITHDRAW_SIGNER = 0x0066600737B44C702056e213BE99a924b52fAF28;

/// @title NFTs2Me.com Smart Contracts Library
/// @author The NFTs2Me Team
/// @notice Read our terms of service
/// @custom:security-contact [email protected]
/// @custom:terms-of-service https://nfts2me.com/terms-of-service/
/// @custom:website https://nfts2me.com/
contract N2MLibrary is N2MCommonStorage, IN2M_ERCLibrary {

    /// @custom:oz-upgrades-unsafe-allow constructor
    constructor(address payable factoryAddress) N2MCommonStorage(factoryAddress) { }
    ...
    function contractURI() public view returns (string memory) {
        ...
        return
            string(
                abi.encodePacked(
                    "https://nfts2me.com/api/contractURI/",
    ...
    function setContractURI(bytes32 newContractURIMetadataCIDHash) external override onlyOwner
    {
        _contractURIMetadataCIDHash = newContractURIMetadataCIDHash;
    }

    function disableOperatorFilterRegistry() external onlyOwner {
        if (_operatorFilterStatus == OperatorFilterStatus.ENABLED_EXISTS) {
            _operatorFilterStatus = OperatorFilterStatus.DISABLED_EXISTS;
        } else {
            _operatorFilterStatus = OperatorFilterStatus.DISABLED_NOT_INITIALIZED;
        }
    }

    function enableOperatorFilterRegistry() external onlyOwner {
        if (_operatorFilterStatus == OperatorFilterStatus.DISABLED_EXISTS) {
            ...
            if (address(operatorFilterRegistry).code.length > 0) {
                ...
                _operatorFilterStatus = OperatorFilterStatus.ENABLED_NOT_INITIALIZED;
            }
        }
    }

    function isOperatorFilterRegistryEnabled() external view returns (bool) {
        return (_operatorFilterStatus == OperatorFilterStatus.ENABLED_EXISTS || _operatorFilterStatus == OperatorFilterStatus.ENABLED_NOT_INITIALIZED);
    }

    function changeMintPrice(uint256 newMintPrice) external override onlyOwner {
        _mintPrice = newMintPrice;
    }

    function setAndRevealBaseURI(bytes32 baseURICIDHash) external onlyOwner {
        if (isMetadataFixed()) revert MetadataAlreadyFixed();
        if (baseURICIDHash == 0) revert NewBaseURICantBeEmpty();
        _baseURICIDHash = baseURICIDHash;
        delete(_placeholderImageCIDHash);
    }

    function changePlaceholderImageCID(bytes32 newPlaceholderImageCIDHash) external override onlyOwner {
        if (isMetadataFixed()) revert MetadataAlreadyFixed();
        if (newPlaceholderImageCIDHash == 0) revert NewPlaceholderCantBeEmpty();

        _placeholderImageCIDHash = newPlaceholderImageCIDHash;
    }    

    function setPhase(SalePhase newPhase) external override onlyOwner {
        if (newPhase >= SalePhase.DROP_DATE) revert InvalidPhaseWithoutDate();
        if (_dropDateTimestamp != 0) delete(_dropDateTimestamp);
        if (_endDateTimestamp != 0) delete(_endDateTimestamp);
        _currentPhase = newPhase;
    }

    function setDropDate(uint256 dropDateTimestamp) external override onlyOwner {
        if (dropDateTimestamp < block.timestamp) revert InvalidDropDate();
        if (_endDateTimestamp != 0) delete(_endDateTimestamp);
        _currentPhase = SalePhase.DROP_DATE;
        _dropDateTimestamp = dropDateTimestamp;
    }

    function setDropAndEndDate(uint256 dropDateTimestamp, uint256 endDateTimestamp) external override onlyOwner {
        if (dropDateTimestamp < block.timestamp || endDateTimestamp <= dropDateTimestamp) revert InvalidDropDate();
    ...
    function setMaxPerAddress(uint16 newMaxPerAddress) external override onlyOwner {
    ...
    function isOpen() external view returns (bool) {
        ...
        ((_currentPhase == SalePhase.DROP_AND_END_DATE) && (block.timestamp >= _dropDateTimestamp) && (block.timestamp < _endDateTimestamp)) ||
        ((_currentPhase == SalePhase.DROP_DATE) && (block.timestamp >= _dropDateTimestamp)))
        {
            return true;
        }
        return false;
    ...
    function withdrawERC20(address erc20Address)
        ...
        onlyOwnerOrN2M
    {
        _withdrawERC20(N2M_FEE, erc20Address);
    }

    function withdrawERC20Pro(
        uint256 signatureExpireDate,
        uint256 n2mFee,
        address erc20Address,
        bytes calldata signature   
    ) external override onlyOwnerOrN2M {
        if (signatureExpireDate < block.timestamp) revert SignatureExpired();
        ...
        address signer = ECDSAUpgradeable.recover(
            ECDSAUpgradeable.toEthSignedMessageHash(
                keccak256(
                    abi.encodePacked(
                        signatureExpireDate,           
                        this.withdrawERC20Pro.selector,  
                        address(this),                   
                        block.chainid,                   
                        erc20Address,
                        n2mFee
        ...
        if (signer != N2M_WITHDRAW_SIGNER) revert SignatureMismatch();
        _withdrawERC20(n2mFee, erc20Address);
    }    

    function _withdrawERC20(uint256 n2mFeePercentage, address erc20Address) private nonReentrant {
        uint256 availableBalance = IERC20(erc20Address).balanceOf(address(this));
        ...
        for (uint256 i; i < revenueAddressesLength; ) {
        ...
        uint256 ownerPercentage = 100_00 - n2mFeePercentage - revenuePercentageTotal;
        if (ownerPercentage != 0) {
            amountToSend = ((availableBalance * ownerPercentage) / 100_00);
            try IERC20(erc20Address).transfer(owner(), amountToSend) {
                withdrawn += amountToSend;
            } catch {
            }
        }

        try IERC20(erc20Address).transfer(_getN2MFeeAddress(), (availableBalance - withdrawn)) {
            _withdrawnERC20Amount[erc20Address] += availableBalance;
        } catch {
            _withdrawnERC20Amount[erc20Address] += withdrawn;
        }
    ...
    function withdraw() external override onlyOwnerOrN2M {
        _withdraw(N2M_FEE);
    }

    function _withdraw(uint256 n2mFeePercentage) private nonReentrant {
        ...
        for (uint256 i; i < revenueAddressesLength; ) {
            ...
            if (_revenueInfo[i].to != address(0)) {
                (success, ) = _revenueInfo[i].to.call{value: amountToSend}("");
                if (success) {
                    withdrawn += amountToSend;
                }
            }
        ...
        (success, ) = _getN2MFeeAddress().call{value: (availableBalance - withdrawn)}("");
        if (success) {
    ...
    function affiliateWithdraw(address affiliate) external override nonReentrant {

        if (affiliate != msg.sender && _getN2MFeeAddress() != msg.sender) revert OnlyAffiliateOrN2M();

        uint256 pending = _pendingAffiliateBalance[affiliate];

        delete(_pendingAffiliateBalance[affiliate]);
        if (pending > _pendingTotalAffiliatesBalance) pending = _pendingTotalAffiliatesBalance;
        unchecked {
            _pendingTotalAffiliatesBalance -= pending;
        }

        uint256 affiliateAmount = ((pending * (100_00 - N2M_FEE)) / 100_00);
        pending -= affiliateAmount;

        if (_erc20PaymentAddress == address(0)) {

            (bool success, ) = affiliate.call{value: affiliateAmount}("");
            (success, ) = _getN2MFeeAddress().call{value: pending}("");
        } else {

            try IERC20(_erc20PaymentAddress).transfer(affiliate, affiliateAmount) { } catch { }
            try IERC20(_erc20PaymentAddress).transfer(_getN2MFeeAddress(), pending) { } catch { } 
        }
    }
    ...
    function setAffiliatesPercentageAndDiscount(
        ...
        if ((userDiscount > MAX_AFFILIATE_DISCOUNT) || (affiliatePercentage > MAX_AFFILIATE_PERCENTAGE)) {
            revert InvalidPercentageOrDiscountValues();
        }
        if ((userDiscount < currentAffiliateInfo.userDiscount) || (affiliatePercentage < currentAffiliateInfo.affiliatePercentage)) {
            revert CantLowerCurrentPercentages();
        }
    ...
    function setReverseENSName(
        address reverseRegistrar,
        string calldata collectionENSName
    ) external onlyOwner {

        IReverseRegistrar(reverseRegistrar).setName(collectionENSName);
    }

    function initializeAndSetReverseENSName(
        address reverseRegistrar,
        string calldata collectionENSName
    ...
    function collectionSize() external view override returns (uint256) {
        if (_mintingType == MintingType.CUSTOM_URI) {
            return _soldTokens;
        }
    ...
    function randomTickets(address minter) external view returns (uint, uint) {
        return (_randomTickets[minter].amount, _randomTickets[minter].blockNumberToReveal);
    }
    ...
    function withdrawnERC20Amount(address erc20) external view returns (uint256) {
        return _withdrawnERC20Amount[erc20];
    }
    ...
    function ownerMaxRevenue() external view returns (uint256) {
        unchecked {
            uint sum;
            for (uint i; i<_revenueInfo.length; i++) {
                sum += _revenueInfo[i].percentage;
            }
            return (100_00 - sum);
        }
    }
...
/// @custom:terms-of-service https://nfts2me.com/terms-of-service/
/// @custom:website https://nfts2me.com/
contract N2M_Paymaster is Initializable, Ownable2StepUpgradeable, UUPSUpgradeable {
    ...
    function initialize(address[] memory swapRouters) public initializer {
        __Ownable_init();
    ...
    function mintNativeUsingERC20(
        address collection,
        IN2M_ERCCommon.MintingType mintingType,
        uint256 amount,
        uint256[] memory tokenIds,
        address affiliate,
        address swapRouterAddress,
        bytes calldata swapPayload,
        uint256 erc20RequiredAmount,
        address erc20AddressUsed
    ) external {
        ...
        bool success = IERC20Upgradeable(erc20AddressUsed).transferFrom(
            msg.sender,
            address(this),
            erc20RequiredAmount
        );

        IERC20Upgradeable(erc20AddressUsed).approve(
            swapRouterAddress,
            erc20RequiredAmount
        );

        uint256 balance = address(this).balance;
        (success, returnData) = swapRouterAddress.call(swapPayload);
        if (success == false) {
            assembly {
        ...
        uint256 nativeRequiredAmount = IN2M_ERCCommon(collection).mintPrice() *
            amount;

        balance = address(this).balance - balance;
        require(balance >= nativeRequiredAmount, "Insufficient funds");

        if (mintingType == IN2M_ERCStorage.MintingType.SEQUENTIAL) {
            if (affiliate == address(0)) {
                IN2M_ERCCommon(collection).mintTo(msg.sender, amount);
            } else {
                IN2M_ERCCommon(collection).mintTo(
                    msg.sender,
                    amount,
                    affiliate
                );
            }
        } else if (mintingType == IN2M_ERCStorage.MintingType.RANDOM) {
            if (affiliate == address(0)) {
                IN2M_ERCCommon(collection).mintRandomTo(msg.sender, amount);
            } else {
                IN2M_ERCCommon(collection).mintRandomTo(
                    msg.sender,
                    amount,
                    affiliate
                );
            }
        } else if (mintingType == IN2M_ERCStorage.MintingType.SPECIFY) {
            if (affiliate == address(0)) {
                IN2M_ERCCommon(collection).mintSpecifyTo(msg.sender, tokenIds);
            } else {
                IN2M_ERCCommon(collection).mintSpecifyTo(
                    msg.sender,
                    tokenIds,
                    affiliate
                );
    ...
    function mintERC20UsingERC20(
        address collection,
        bytes calldata payload,
        uint256 erc20RequiredAmount,
        address erc20AddressRequired,
        address erc20AddressUsed
    ) external {
        ...
        onlyOwner
    {
        ...
        IERC20Upgradeable(_tokenContract).transfer(owner(), _amount);
    }

    function getEstimate(address collection, uint256 amount)
        public
        view
        returns (uint256)
    {}

    function getERC20Estimate(
        address collection,
        address erc20,
        uint256 amount
...
import "./interfaces/IN2M_ERCStorage.sol";
import "./libraries/NFTOwnableUpgradeable.sol";
import "./interfaces/IN2MCrossFactory.sol";
...
import "./interfaces/IN2M_ERCCommon.sol";
import "./interfaces/IN2MFactory.sol";
import "./interfaces/IN2MCrossFactory.sol";
import "./interfaces/IN2MBeaconFactory.sol";

address constant N2M_ENS_SIGNER = 0xc0FFE39d857A13FFE6f2070D62ad03942f742DBC;
address constant N2M_TREASURY = 0x955aF4de9Ca03f84c9462457D075aCABf1A8AfC8;
bytes32 constant erc721LatestType = "721";
bytes32 constant erc1155LatestType = "1155";
bytes32 constant erc721LatestUpgradeableType = "u721";
bytes32 constant erc1155LatestUpgradeableType = "u1155";
...
contract N2MFactoryV1000 is
    ...
    IN2MCrossFactory
{
    uint256[150] private __gappre;

    mapping(bytes32 => address) private _implementationAddresses;
    uint256 tempOwnerSelfForCreation;
    address currentERC721;
    address currentERC1155;
    ...
    function initialize(
        ...
        address ERC1155UpgradeableAddress
    ) public payable initializer {
        __ERC721_init("NFTs2Me Owners", "N2MOwners");
        __Ownable_init();
        __UUPSUpgradeable_init();
        _implementationAddresses[erc721LatestType] = ERC721Address;
        currentERC721 = ERC721Address;
        _implementationAddresses[erc1155LatestType] = ERC1155Address;
        currentERC1155 = ERC1155Address;
        _implementationAddresses[erc721LatestUpgradeableType] = ERC721UpgradeableAddress;
        _implementationAddresses[erc1155LatestUpgradeableType] = ERC1155UpgradeableAddress;
    }
    ...
    function createCollection(
        ...
        address clone = ClonesUpgradeable.cloneDeterministic(
            ...
            collectionAndDeployId
        );

        IN2M_ERCCommon(clone).initialize(
            tokenName,
            tokenSymbol,
            mintPrice,
            baseURICIDHash,
        ...
        _mint(msg.sender, uint256(uint160((clone))));

        bytes16[2] memory collectionAndDeployIdArray = [bytes16(0), 0];
        assembly {
            mstore(collectionAndDeployIdArray, collectionAndDeployId)
            mstore(add(collectionAndDeployIdArray, 16), collectionAndDeployId)
        }

        emit CollectionDeployed(
            msg.sender,
            collectionAndDeployIdArray[0],
            collectionAndDeployIdArray[1],
            clone,
            iErc20PaymentAddress
        );
    }
    ...
    function createAndMintCollection(
        ...
        IN2M_ERCCommon(clone).initialize(
            ...
            revenueAddresses,
            iErc20PaymentAddress,
            uint32(toAddresses.length),
            royaltyFee,
            soulboundCollection,
            IN2M_ERCStorage.MintingType.SEQUENTIAL
        );

        _mint(msg.sender, uint256(uint160((clone))));
        tempOwnerSelfForCreation = 1;
        IN2M_ERCCommon(clone).airdropSequential(toAddresses, false);
        delete(tempOwnerSelfForCreation);
    ...
    function setReverseENSNameForCollectionOnMainnet(
        uint256 signatureExpireDate,
        address reverseRegistrar,
        string calldata collectionENSName,
        address underlyingImplementation,
        bytes32 collectionAndDeployId,
        bytes calldata signature
    ) external {
        if (signatureExpireDate < block.timestamp) revert SignatureExpired();
        ...
        address signer = ECDSAUpgradeable.recover(
            ECDSAUpgradeable.toEthSignedMessageHash(
                keccak256(
                    abi.encodePacked(
                        signatureExpireDate,
                        this.setReverseENSNameForCollectionOnMainnet.selector,
                        address(this),
                        block.chainid,
                        reverseRegistrar,
                        collectionENSName,
        ...
        if (signer != N2M_ENS_SIGNER) revert SignatureMismatch();
        address clone = ClonesUpgradeable.cloneDeterministic(
            underlyingImplementation,
            collectionAndDeployId
        );
        IN2M_ERCCommon(clone).initializeAndSetReverseENSName(
            reverseRegistrar,
            collectionENSName
        );
    }
    ...
    function newContractImplementations(
        bytes32[] calldata implementationTypes,
        address[] calldata implementationAddresses
    ) public payable onlyOwner {

        uint256 length = implementationTypes.length;
        if (length!=implementationAddresses.length) revert InputLengthMismatch();
        for (uint256 i; i<length; ) {
            _implementationAddresses[implementationTypes[i]] = implementationAddresses[i];
            if (implementationTypes[i] == erc721LatestType) {
                currentERC721 = implementationAddresses[i];
            } else if (implementationTypes[i] == erc1155LatestType) {
                currentERC1155 = implementationAddresses[i];
            }
            unchecked {
                ++i;
            }
        ...
        returns (string memory)
    {
        ...
        try IN2M_ERCLibrary(collection).ownerMaxRevenue() returns (uint ownerMaxRevenue) {
            ownerRevenue = ownerMaxRevenue;
        } catch {
        }

        return string(abi.encodePacked("https://nfts2me.com/api/n2mowners/", StringsUpgradeable.toString(block.chainid), "/", StringsUpgradeable.toString(tokenId), "/", StringsUpgradeable.toString(uint(uint160(ownerOf(tokenId)))), "/", StringsUpgradeable.toString(ownerRevenue)));
    }

    function ownerOf(uint256 tokenId)
        public
        view
        override(ERC721Upgradeable, IN2MCrossFactory)
        returns (address)
    {
        return super.ownerOf(tokenId);
    }

    function strictOwnerOf(uint256 tokenId)
        public
        view
        override(IN2MCrossFactory)
        returns (address)
    {
        if (tempOwnerSelfForCreation != 0) return address(this);
        return super.ownerOf(tokenId);
    }

    function getN2MTreasuryAddress() public pure returns (address) {
        return address(N2M_TREASURY);
    }
        ...
        )
    {
        ...
        uint256 total;
        if (endId >= startId) {
            total = 1 + endId - startId;
            unchecked {
                addresses = new address[](total);
                for (uint256 i; i < total; ) {
                    try IERC721(collection).ownerOf(startId + i) returns (address a) {
                        addresses[i] = a;
                    } catch {}
                    ++i;
                }
    ...
    function n2mVersion() external virtual pure returns (uint256) {
        return 1000;
    }
    ...
    function getERC721Implementation()
        external
        view
        override
        returns (address)
    {
        return currentERC721;
    }

    function getERC1155Implementation()
        external
        view
        override
        returns (address)
    {
        return currentERC1155;
    }
}
...
import "../N2MERC721.sol";

contract ERC721v2Safe is N2MERC721 {
    uint256 immutable number;

    constructor(address libraryAddress, address payable factoryAddress) N2MERC721(libraryAddress, factoryAddress) {
        number = 9999911199997;
    }
    function n2mVersion() external pure override(IN2M_ERCStorage, N2MCommonStorage) returns (uint256) {
        return 9999911199999;
    }
...
import "../N2MFactoryV1000.sol";

contract Factoryv2Safe is N2MFactoryV1000 {
    uint256 newVar;
    address newAdd;

    function n2mVersion() external pure override returns (uint256) {
        return 9999922299999;
    }
