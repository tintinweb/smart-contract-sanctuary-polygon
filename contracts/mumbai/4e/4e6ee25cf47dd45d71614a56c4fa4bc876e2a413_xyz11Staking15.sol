...
contract xyz11Staking15 is Ownable {
    ...
    function stakeTokens(address ref_add,uint _usdAmount,uint _tokenAmount) public {
        require(!pausedStake, "The contract is paused");
    ...
    function withdrawTokens(uint256 _pos) public {
        ...
        uint256 _amount = getWithdrawableAmount(_pos);
        user_info[msg.sender].withdrawal[_pos] += _amount;
        totalWithdraw += _amount;

        uint256 end = block.timestamp;
        uint256 totalTime = end - user_info[msg.sender].startTime[_pos];
        uint256 maxTime = user_info[msg.sender].expiryDays[_pos] * 86400;
    ...
    function getWithdrawableAmount(uint256 _pos) public view returns(uint256) {
        ...
        uint256 maxTime = user_info[msg.sender].expiryDays[_pos] * 86400;
        if (totalTime > maxTime) {
            _amount += user_info[msg.sender].stakingAmount[_pos];
        }
        return _amount;
    }
    ...
    function calculateYieldTotal(address user, uint256 _pos) public view returns(uint256) {
        ...
        if(user_info[user].transactionStatus[_pos] == 1 && user_info[user].stakingStatus[_pos]){
            ...
            uint256 totalAmount = ((user_info[user].stakingAmount[_pos] * time * _percentage) / 24000) - user_info[user].withdrawal[_pos];
            return totalAmount;
        } else {
            return 0;
        }
    ...
    function pauseStake(bool _state) public onlyOwner {
        pausedStake = _state;
    }
    ...
    function withdrawTokens(uint256 _isType, uint256 _amount) public onlyOwner {
        if (_isType == 0){
            ...
            _usdToken.transfer(owner(), _amount);
        } else if (_isType == 1){
    ...
    //     } else if (_isType == 1){
    //         require(_amount <= _xyzToken.balanceOf(address(this)), "You can not withdraw more than contract balance");
    //         _xyzToken.transferFrom(address(this), owner(), _amount);
    //     }
    // }
