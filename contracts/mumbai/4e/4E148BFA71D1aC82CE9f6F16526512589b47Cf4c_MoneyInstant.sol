...
// File: payrollContract/Types.sol
...
// File: payrollContract/IMoneyInstant.sol
...
/**
 * @title IMoneyInstant provides an interface to receive tokens and disimburse tokens to multiple recipients at once.
 */
...
interface IMoneyInstant {
    ...
    /**
     * @notice Emits when tokens are transferred to a new address.
     */
    event TokensRefund(
     ...
     * @dev The items in the deposits should match the respective addresses in the recipients.
     ...
     * @param payments The recipient address and their deposit
     */
     ...
     * @param tokenAddress The addres of the token to check balance of.
    ...
    /**
     * @notice Creates a new native tokon instant transfer funded by `msg.sender` and paid towards `recipients`.
     * The length of the deposits is capped by the block gas limit.
     * Emits an {nativeInstantTransfer} event
     *  Throws if the payments size is zero
     *  Throws if the recipient is the zero address, the contract itself or the caller.
     *  Throws if the contract did not receive native token as sum of deposit tokens.
     *  Throws if there is a token transfer failure.
     * @param payments The recipient address and their deposit
     */
...
// File: payrollContract/MoneyInstant.sol
...
contract MoneyInstant is IMoneyInstant, ReentrancyGuard, Ownable {
    ...
    function createInstantTransfer(
        ...
        for (uint256 i = 0; i < payments.length; i++) {
            ...
            uint256 currentBalance = userToTokens[recipient][tokenAddress];
            userToTokens[recipient][tokenAddress] = currentBalance + deposit;
        }
        ...
        emit InstantTransfer(id, msg.sender, totalDeposit, tokenAddress);
    }
        ...
        nonReentrant
    {
        ...
        userToTokens[msg.sender][tokenAddress] = remainingBalance;
        IERC20(tokenAddress).safeTransfer(msg.sender, amount);
        emit InstantWithdraw(msg.sender, amount, tokenAddress);
    }
    ...
    /**
     * @dev Transfer the tokens owned by @param oldAddress to the @param newAddress.
     * Can only be called by the Owner of the contract.
     * Throws if the amount is zero.
     * @param newAddress Address to refund the tokens to.
     * @param oldAddress Address which is the owner of the tokens.
     * @param tokenAddress The ERC20 token to be used.
    ...
    function refundToken(
        ...
        require(amount > 0, 'amount is zero');
        require(newAddress != address(0x00), 'cannot send to the zero address');
        uint256 balance = userToTokens[oldAddress][tokenAddress];
        require(balance >= amount, 'amount exceeds the available balance');
        uint256 remainingBalance = balance - amount;
        userToTokens[oldAddress][tokenAddress] = remainingBalance;
        IERC20(tokenAddress).safeTransfer(newAddress, amount);
        emit TokensRefund(newAddress, oldAddress, amount, tokenAddress);
    }
    ...
    function createNativeTokenTransfer(
        ...
        for (uint256 i = 0; i < payments.length; i++) {
            ...
            totalDeposit += deposit;
            uint256 currentBalance = userToNative[recipient];
            userToNative[recipient] = currentBalance + deposit;
        }
    ...
    function withdrawNativeInstant(uint256 amount) external nonReentrant {
        ...
        require(balance >= amount, 'amount exceeds the available balance');
        uint256 remainingBalance = balance - amount;
        userToNative[msg.sender] = remainingBalance;
