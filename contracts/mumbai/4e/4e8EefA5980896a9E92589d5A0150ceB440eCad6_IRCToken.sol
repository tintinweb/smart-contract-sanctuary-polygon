...
contract IRCToken is IERC20, Ownable {
    using SafeMath for uint256;

    string public name = "IRANCOIN";
    ...
    uint256 transferPercentFee = 15; // 0.15%
    ...
    function transfer(
        ...
        uint256 transferFee = getSendFee(msg.sender, numTokens);
        require(
            numTokens <=
    ...
    function transferFrom(
        ...
        require(
            numTokens <= balances[owner] + getSendFee(msg.sender, numTokens)
        );

        require(numTokens <= allowed[owner][msg.sender]);
        balances[owner] = balances[owner].sub(numTokens.add(getSendFee(msg.sender, numTokens)));
    ...
    function mint(uint256 amount, address beneficiary)public onlyOwner returns(uint256){
        totalSupply_ += amount;
