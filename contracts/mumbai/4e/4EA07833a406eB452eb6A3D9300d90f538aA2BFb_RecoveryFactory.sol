...
interface IRecovery is IYousovStructs{
    ...
    event AgentAssignedAnswerForRecovery(address agent, address recovery);
...
contract Recovery is IRecovery, YousovRoles {
    ...
    function startTheRecovery() external override {
        ...
        require(recoveryStatus == RecoveryStatus.IN_PROGRESS && tx.origin == user  && (agentList.length ==
            IUser(
                IUserFactory(IRecoveryFactory(recoveryFactory).userFactory())
                    .userContract(user)
            ).userChallenges().length), "YOUSOV: Operation not authorized");
        ...
        for(uint256 i = 0; i < agentList.length; i++) {
                ...
                answerAgentsDetails[agentList[i]] = AnswerAgentsDetails(affectedChallenge.answer,"",affectedChallenge.id,false, AnswerStatus.NOT_ANSWERED);
                emit AgentAssignedAnswerForRecovery(agentList[i], address(this));
            
        }
    }


    function sendRecoveryAnswerToAnswerAgent(Challenge[] memory _challenges) external override {
        ...
        for (uint i = 0; i < agentList.length; i++) {
            ...
            for (uint j = 0; j < _challenges.length; j++) {
                ...
                if ( keccak256(abi.encodePacked(answerAgentsDetails[answerAgent].challengeID)) == keccak256(abi.encodePacked(_challenges[j].id)) ) {
                   AnswerAgentsDetails memory lastStat =  answerAgentsDetails[answerAgent] ;
                    answerAgentsDetails[answerAgent] = AnswerAgentsDetails(lastStat.initialAnswer,_challenges[j].answer,lastStat.challengeID,false,AnswerStatus.NOT_ANSWERED);
                    emit SendAnswersToAnswerAgents(answerAgent, address(this), _challenges[j].answer); 
                    break;
                }
            }
    ...
    function _isAllAnswersAgentsHaveAnswered() view private returns(RecoveryStats memory) {
        ...
        for (uint i = 0; i < agentList.length; i++) {
            if (answerAgentsDetails[agentList[i]].answerStatus == AnswerStatus.NOT_ANSWERED) {
    ...
    function agentCheckUserAnswer(address answerAgent,bool userAnswer) external override {
        require(msg.sender == answerAgent && recoveryStatus == RecoveryStatus.IN_PROGRESS && answerAgentsDetails[answerAgent].answerStatus == AnswerStatus.NOT_ANSWERED , "YOUSOV : Not authorised operation");
        answerAgentsDetails[answerAgent] = AnswerAgentsDetails(answerAgentsDetails[answerAgent].initialAnswer,answerAgentsDetails[answerAgent].actualAnswer, answerAgentsDetails[answerAgent].challengeID,userAnswer,AnswerStatus.ANSWERED);   
...
contract RecoveryFactory is IRecoveryFactory, YousovRoles {
    ...
    constructor(
        ...
        //  set this address in user factory to send unlock and user factory send it to a user
        yousovAccessControl = _yousovAccessControl;
    }
