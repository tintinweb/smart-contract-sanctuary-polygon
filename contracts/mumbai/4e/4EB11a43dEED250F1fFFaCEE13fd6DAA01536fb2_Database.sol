...
// import "contracts/interface/IUSDT.SOL";
...
contract Database is Ownable, IDB {
    ...
    function setRecepient(address _recepient) external zeroAddress(_recepient) onlyOwner {
        recepient = _recepient;
    }

    function setAdminWallet(address _newAdminWallet) external zeroAddress(_newAdminWallet) onlyOwner {
    ...
    function setPaymentFeeToken (address _newPaymentToken) external  zeroAddress(_newPaymentToken) onlyOwner {
        paymentFeeToken = _newPaymentToken;
        emit FeeTokenUpdated(_msgSender(),paymentFeeToken);
    }

    function payBridgeFee(address _tokenAddress) external {
        require(!isFeePaid[_tokenAddress], "FEES_ALREADY_PAID");
        IERC20(paymentFeeToken).transferFrom(_msgSender(), adminWallet, bridgeFee);
