...
// File: ViciERC721_FailUnexpectedly.sol

/**
 * This collection simulates an intermittent blockchain failure after
 * a certain number of nfts have been minted.
 */
contract ViciERC721_FailUnexpectedly is ViciNamedERC721 {
    uint256 failIndex;

    constructor()
        ViciNamedERC721(
            "ViciERC721_FailUnexpectedly",
            "V4T",
            0x1000000000
        )
    {
    ...
    function _post_mint_hook(address, uint256)
        internal
        override
    {
        if (totalSupply() >= failIndex) {
            revert("weird intermittent blockchain error");
        }
    }

    /**
     * @dev Sets the number that can be successfully minted before this 
     * contract will start reverting on mint.
     * @dev To simulate partial successes, set this value larger than the
     * batch size.
     * @dev batch size is min(25, number_we_can_afford_to_buy_at_once)
     * @param successes the number that can be minted.
     */
    function failAfter(uint256 successes) public onlyOwnerOrRole(CREATOR_ROLE_NAME) {
        failIndex = totalSupply() + successes;
    }
}