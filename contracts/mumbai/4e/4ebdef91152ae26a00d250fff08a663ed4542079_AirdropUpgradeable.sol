    ...
    ERC721HolderUpgradeable
{
     ...
     * @dev Initializer
     */
    function initialize(IERC20Upgradeable _busdToken) public initializer {
    ...
    function pendingClaims(address addressToClaim)
        external
        view
        returns (
            uint256 _busdToClaim,
            uint256 _erc1155ToClaim,
            uint256 _erc721ToClaim
        )
    {
        _busdToClaim = busdToClaim[addressToClaim];

        // add the amounts of each erc1155 nft token
        for (
            uint256 i = 0;
            i < addressToERC1155Airdrop[addressToClaim].length;
            i++
        ) {
            _erc1155ToClaim += addressToERC1155Airdrop[addressToClaim][i]
                .amount;
        }

        _erc721ToClaim = addressToERC721Airdrop[addressToClaim].length;
    }

    function calculateDistribution(
        uint256 _amount,
        uint256 _squiresLength,
        uint256 _knightsLength,
        uint256 _wizardsLength
    )
        internal
        pure
        returns (
    ...
    function airdropERC721NFT(
        ...
        for (uint256 i = 0; i < _nftContract.length; i++) {
            IERC721Upgradeable(_nftContract[i]).safeTransferFrom(
                msg.sender,
                address(this),
                _tokenId[i]
            );

            ERC721Airdrop memory airdrop = ERC721Airdrop(
                _nftContract[i],
                _tokenId[i]
            );
    ...
    function airdropERC1155NFT(
        ...
        for (uint256 i = 0; i < _nftContract.length; i++) {
            IERC1155Upgradeable(_nftContract[i]).safeTransferFrom(
                msg.sender,
                address(this),
                _tokenId[i],
                _amount[i],
            ...
            ERC1155Airdrop memory airdrop = ERC1155Airdrop(
                _nftContract[i],
                _tokenId[i],
                _amount[i]
            );
    ...
    function airdropBUSD(
        ...
        // calculate distribution
        (
            uint256 _squireValue,
            uint256 _knightValue,
            uint256 _wizardValue
        ...
        for (uint256 i = 0; i < _squires.length; i++) {
    ...
    function claimERC721(address _addressToClaim) public {
        ...
        ERC721Airdrop[] memory airdrops = addressToERC721Airdrop[
            _addressToClaim
        ];
        delete addressToERC721Airdrop[_addressToClaim];

        for (uint256 i = 0; i < length; i++) {
            IERC721Upgradeable(airdrops[i].nftAddress).safeTransferFrom(
                address(this),
                _addressToClaim,
                airdrops[i].tokenId
            );
    ...
    function claimERC1155(address _addressToClaim) public {
        ...
        ERC1155Airdrop[] memory airdrops = addressToERC1155Airdrop[
            _addressToClaim
        ];
