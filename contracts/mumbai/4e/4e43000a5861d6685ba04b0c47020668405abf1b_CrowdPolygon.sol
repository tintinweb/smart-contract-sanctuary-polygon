...
contract CrowdPolygon is DefaultOperatorFilterer, ERC721A, Ownable {
  using Address for address;
  using Strings for uint256;
  ...
  uint256 private _royaltyFund = 6;  
  uint256 private _royaltyArtist = 50;  

  address private wMobiup = 0x2919771f951fd264445d4D4c682EE5D8326B93ec;
  address private wCrowd = 0xB2178260FAEdFd86dB1BC8Cd0d4E91466Ca82eE9;
  address private wFund = 0x9fD17B94d6AF41FBAd13cd04fF1309433A63249e;
  address private wArtist = 0x9fD17B94d6AF41FBAd13cd04fF1309433A63249e;
  uint256 royaltiesSpender;

  uint256 private _royaltyFundMarket = 1;  
  uint256 private _royaltyArtistMarket = 4;
  uint256 private _royaltyCreatorMarket = 3;
  ...
  uint256 private initSaleWaitList = 1680811862;
  uint256 private endSaleWaitList = 1680811862;
  uint256 private startSalePublic = 1680811862;

  event _transferSend(address _from, address _to, uint _amount);

  constructor(    
    bytes32 _root    
  ) ERC721A("CrowdPolygon", "CROWDP") {    
    root = _root;
    _contractUri = "";
    ...
    function split(uint256 _mintAmount, address payable _recommendedBy, uint256 _indicationType ) public payable{
      ...
      uint256 amountMobiup = msg.value * _royaltyMobiup / 100;        
      payable(wMobiup).transfer(amountMobiup);
      

      uint256 amountCrowd = msg.value * _royaltyCrowd / 100;        
      payable(wCrowd).transfer(amountCrowd);
      

      uint256 amountFund = msg.value * _royaltyFund / 100;        
      payable(wFund).transfer(amountFund);
      
      uint256 amountArtist = msg.value * _royaltyArtist / 100;        
    ...
    receive() external payable {
        ...
        uint256 amountRoyaltiesTwo = msg.value * _royaltyArtistMarket / 100;
