    ...
    VRFConsumerBaseV2
{
    ...
    function safeMint(address to) public onlyOwner {
        ...
        uint256 idx = _randomWords[tokenId] % _arts_count;
        _setTokenURI(tokenId, _generateTokenUri(idx));
    }
    ...
    function updateAllTokensUris() internal {
        for (uint256 id = 0; id < _tokenIdCounter.current(); id++) {
            uint256 idx = _randomWords[id] % _arts_count;
            _setTokenURI(id, _generateTokenUri(idx));
        }
    }
    ...
    function _generateTokenUri(uint256 index)
        internal
        view
