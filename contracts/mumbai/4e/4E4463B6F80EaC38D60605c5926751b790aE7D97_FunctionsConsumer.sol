...
contract FunctionsConsumer is FunctionsClient, ConfirmedOwner {
  ...
  //     require(dataLength % 32 == 0, "Data length must be a multiple of 32");

  //     uint256 resultLength = dataLength / 32;
  //     uint[] memory result = new uint[](resultLength);

  //     assembly {
  //         let dataIndex := add(data, 32)
  //         let resultIndex := result

  //         for { let i := 0 } lt(i, resultLength) { i := add(i, 1) dataIndex := add(dataIndex, 32) resultIndex := add(resultIndex, 32) } {
  //             mstore(resultIndex, mload(dataIndex))
  //         }
  //     }
  //     return result;
  // }
