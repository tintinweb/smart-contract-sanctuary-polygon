...
contract VToken is IVToken,Context,IERC20{
    ...
    function initialize(address token,string memory tokenName,string memory tokenSymbol,uint8 tokenDecimals,address bridgeControl) external {
    ...
    function mint(address spender, uint256 amount) external override{
        require(msg.sender == BridgeControl, 'token mint:FORBIDDEN');
        _mint(spender, amount);
        emit Mint(spender, amount);
    }
    function burn(address spender, uint256 amount) external override{
        require(msg.sender == BridgeControl, 'token burn: FORBIDDEN');
