    ...
    ReentrancyGuardUpgradeable
{
    ...
    function setDappMetadata(string memory feePayerAddress) external payable nonReentrant returns (uint64) {
        require(msg.value >= requestToRouterDefaultFee, "C03");
        eventNonce++;
        emit SetDappMetadataEvent(eventNonce, toBytes(msg.sender), chainId, chainType, feePayerAddress);
        return eventNonce;
    }
    ...
    function requestToRouter(
        ...
        emit RequestToRouterEvent(
            Utils.InboundSourceInfo(routeAmount, routeRecipient, eventNonce, chainType, chainId),
            toBytes(msg.sender),
    ...
    function createCrossTalkData(
        Utils.RequestArgs memory requestArgs,
        Utils.DestinationChainParams memory destChainParams
    ) internal returns (Utils.SourceChainParams memory srcChainParams, address txOrigin) {
        ...
        srcChainParams = Utils.SourceChainParams(
            destChainNonces[destChainParams.destChainId][destChainParams.destChainType],
    ...
    function requestToDestEmitter(
        Utils.RequestArgs memory requestArgs,
        Utils.AckGasParams memory ackGasParam,
        Utils.DestinationChainParams memory destChainParams,
        Utils.ContractCalls memory contractCalls,
        Utils.AckType ackType,
        bool isReadCall
    ) internal {
        (Utils.SourceChainParams memory srcChainParams, address txOrigin) = createCrossTalkData(
            requestArgs,
            destChainParams
        );
    ...
    function requestToDest(
        ...
        requestToDestEmitter(requestArgs, ackGasParams, destChainParams, contractCalls, ackType, false);
        return eventNonce;
    }
    ...
    function requestFromRouter(
        ...
        if (isContract(shieldModuleAddress)) {
            IAdditionalSecurityModule asm = IAdditionalSecurityModule(shieldModuleAddress);
    ...
    function requestFromSource(
        ...
        if (block.timestamp > crossTalkPayload.expTimestamp || !isValidRequest) {
            bytes memory emptyData = "";
