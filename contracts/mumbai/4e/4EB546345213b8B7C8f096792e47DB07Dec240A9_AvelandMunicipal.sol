...
import "./interfaces/IAvelandContract.sol";
...
contract AvelandMunicipal is Multicall, AccessControlEnumerable, IContractFactory {
    /// @dev Only FACTORY_ROLE holders can approve/unapprove implementations for proxies to point to.
    bytes32 public constant FACTORY_ROLE = keccak256("FACTORY_ROLE");

    /// @dev Only FACTORY_ROLE holders can add implementation implementations for proxies to point to.
    ...
    /// @dev Emitted when a new implementation is registered or removed
    ...
    function add(address _deployer, address _deployment) public {
    ...
    function count(address _deployer) public view returns (uint256) {
    ...
    function deployProxy(bytes32 _type, bytes memory _data) external returns (address) {
        bytes32 salt = bytes32(count(_msgSender()));
    ...
    function addImplementation(address _implementation) external {
        ...
        IAvelandContract module = IAvelandContract(_implementation);
...
interface IContractFactory {
     ...
     *
     *  @param data                     Additional data to pass to the proxy constructor or any other data useful during deployment.
