...
contract FoundersStakingTR is ReentrancyGuard, Permissions, ContractMetadata {
    ...
    uint public startDate = 1665018000; //  2022-10-05 22:00:00 GMT-03:00
    uint public endDate = 1668560400; // 2022-11-10 22:00:00 GMT-03:00


    // Rewards per hour per token deposited in wei.
    uint256 public rewardsPerHour = 3339120370370370000;
    uint public legendaryRewardsPerHour = 3339120370370370000;
    uint public epicRewardsPerHour = 752314814814815000;
    uint public rareRewardsPerHour = 501157407407407000;
    ...
    function setRewardsPerHour(uint256 _rewardsPerHour) public {
       ...
       function setLegendaryRewardsPerHour(uint256 _legendaryRewardsPerHour) public {
        legendaryRewardsPerHour = _legendaryRewardsPerHour;
    }
    function setEpicRewardsPerHour(uint256 _epicRewardsPerHour) public {
        epicRewardsPerHour = _epicRewardsPerHour;
    }
    function setRareRewardsPerHour(uint256 _rareRewardsPerHour) public {
        rareRewardsPerHour = _rareRewardsPerHour;
    }
    ...
    function withdrawNFT(address _tokenContract, uint256 _tokenId) external {
    ...
    function stake(uint256 _tokenId) external nonReentrant {
        ...
        string memory nRarity = _tokenId <= 14 ? 'Legendary' : _tokenId <= 1126 ? 'Epic' : 'Rare';

        // Create StakedToken
        StakedToken memory stakedToken = StakedToken(msg.sender, _tokenId, nRarity);
        ...
        returns (uint256 _rewards)
    {
        uint256 endD = 0;
        uint256 iniD = 0;
        uint256 _rewardsCalc = 0;
        uint256 _rPHour = 0;

        if (stakers[_staker].amountStaked > 0) {
            for (uint256 j = 0; j < stakers[_staker].stakedTokens.length; j++) {
                endD = block.timestamp; // <= endDate ? endDate : block.timestamp; 
                iniD = stakers[_staker].timeOfLastUpdate; // >= startDate ? stakers[_staker].timeOfLastUpdate : startDate; 
               
                _rPHour = stringsEquals(stakers[_staker].stakedTokens[j].rarity , 'Legendary') ? legendaryRewardsPerHour : stringsEquals(stakers[_staker].stakedTokens[j].rarity , 'Epic') ? epicRewardsPerHour: rareRewardsPerHour;
                _rewardsCalc += ((endD - iniD) * _rPHour/3600);
            }
        }
