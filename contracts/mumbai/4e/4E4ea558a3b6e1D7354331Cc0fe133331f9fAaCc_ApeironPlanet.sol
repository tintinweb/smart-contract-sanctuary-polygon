    ...
    UUPSUpgradeable
{
    ...
    function safeMint(
        ...
        require(
            ...
            (parentA == 0 && parentB == 0) ||
                ...
                (_exists(parentA) &&
                    _exists(parentB) &&
                    parentA != parentB &&
                    planetDataMap[parentA].breedCount <
                    planetDataMap[parentA].breedCountMax &&
                    planetDataMap[parentB].breedCount <
                    planetDataMap[parentB].breedCountMax),
        ...
        planetDataMap[tokenId].parentA = parentA;
        planetDataMap[tokenId].parentB = parentB;
    ...
    function updatePlanetData(
        ...
        bool setBornTime
    ) external hasTokenId(tokenId) whenNotPaused onlyOwner {
        if (setBornTime) {
            require(
        ...
        )
    {
        require(
            ...
            "PlanetData is not existent"
        );

        return (planetDataMap[tokenId], _exists(tokenId));
    }

    function setBaseURI(string memory baseURI_) public whenNotPaused onlyOwner {
        baseURI = baseURI_;
    }
