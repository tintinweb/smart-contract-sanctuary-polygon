...
contract DistributorERC721 is AccessControl, ERC721Holder, AcceptedTokenList {
    ...
    //Address of token for distribution
    ...
    function claim(
        ...
        require(!hasStatus(_erc721Token, TokenState.unaccepted), "Token is unaccepted");
        require(
    ...
    {
        uint256 feeAmount;
        IERC20(_erc20TokenFrom).transferFrom(_buyer, _seller, _amountFrom);
        ...
        feeAmount += _payRoyalty(token, _tokenId, _erc20TokenFrom, _amountFrom);

        IERC20(_erc20TokenFrom).transfer(_seller, _amountFrom - feeAmount);
    }
        IERC721(token).safeTransferFrom(_seller, _buyer, _tokenId);
    ...
    function claimAndSell(
        ...
        bytes32 message = keccak256(
            abi.encodePacked(msg.sender, _buyer, _price, _erc20Token, _tokenId, block.chainid, address(this), _salt)
        );
        ...
        feeAmount += _payRoyalty(token, _tokenId, _erc20Token, _price);

        IERC20(_erc20Token).transfer(msg.sender, _price - feeAmount);

        IERC721(token).safeTransferFrom(address(this), _buyer, _tokenId);
