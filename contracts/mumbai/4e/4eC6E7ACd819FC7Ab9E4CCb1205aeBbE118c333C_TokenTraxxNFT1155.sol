    ...
    IERC1155Custom
{
     ...
     */
    function addQuantity(uint256 id, address _to, uint256 amount) external {
    ...
    function batchMintTraxx(
        ...
        require(
            validateParameters(amounts, uris, maxPurchaseLimit),
            throwError(INVALID_FUNCTION_PARAMS)
        );
        uint[] memory ids = new uint[](amounts.length);
        uint256 arrayLen = amounts.length;
       ...
       /**
     * @dev Validates parameters, internally called by batchMint().
     * @param amounts uri string for that token
     * @param uris Max number of purchase limit of that token
     * @param maxPurchaseLimit uri string for that token
     */
    ...
    /**
     * @dev Returns array of TokenBalance ( tokenIds owned by the owner and quantities associated to the tokenIds )
     * @param _owner address of the owner 
     */
    function tokenIdAndQuantityByOwner(
        address _owner
