...
contract Orderbook is Ownable {
    ...
    constructor (address TCOEaddress, address DAIaddress, address BiotokenAddress) {
    ...
    function updateDaiPerTCOE(uint256 _newValue)
        public
        onlyOwner
    {
    ...
    function updateBiotokenFee(uint256 _newFee)
        public
        payable
        onlyOwner
    {
    ...
    function createTokenSale(uint256 _amount) public {
        ...
        ordersCounter = ordersCounter.add(1);
    }

    /* Returns an specific order */
    function getOrderSale (uint256 orderId) public view returns (OrderSale memory) {
    ...
    function acceptSaleOrder(uint256 orderId) public {
        require(idToOrderSale[orderId].deleted == false, "Order must be active");

        address seller = idToOrderSale[orderId].seller;

        uint256 DAIToTransfer = (idToOrderSale[orderId].amount * daiPerTCOE) / 1 ether;
        ...
        itemsSold = itemsSold.add(1);

        // Transfiero los DAI al vendedor
        DAI.transferFrom(msg.sender, seller, DAIToTransfer);

        // Transfiero fee a Biotoken address - 0.25 % TCOE
        uint256 fee = (TCOEToTransfer * 25) / 10000;
    ...
    function cancelSaleOrder(uint256 orderId) public {
        require(idToOrderSale[orderId].deleted == false, "Order must be active");
        require(idToOrderSale[orderId].seller == msg.sender, "Only Seller can cancel");
