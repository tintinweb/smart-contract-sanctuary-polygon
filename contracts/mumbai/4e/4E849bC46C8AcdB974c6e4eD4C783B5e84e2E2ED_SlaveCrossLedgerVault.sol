...
contract SlaveCrossLedgerVault is CrossLedgerVault {
    constructor(
        ...
        address _zap,
        address _lzEndpoint
    ) CrossLedgerVault(_rootVault, _zap, _lzEndpoint) {}

    function transferCompleted(bytes32 transferId)
        public
        onlyFromBridgeAdapter
    {
        uint256 fromChainId = bridgeAdapterToChainId[msg.sender];
        uint16 lzChainId = chainIdToLzChainId[fromChainId];
        _lzSend(
            lzChainId,
            _encodeTransferCompletedMessage(transferId),
    ...
    function _blockingLzReceive(
        ...
        uint256 srcChainId = LzChainIdToChainId[_srcLzChainId];
        (LzMessage messageType, bytes memory data) = abi.decode(
            _payload,
            (LzMessage, bytes)
        );
        if (messageType == LzMessage.REDEEM) {
            (
                uint256 opId,
                uint256 shares,
                uint256 totalSupply,
            ...
            uint256 value = rootVaultZap.redeem(
                asset,
                (shares * rootVault.balanceOf(address(this))) / totalSupply
            );
            IBridgeAdapter adapter = bridgeAdapters[asset][srcChainId];
            bytes32 transferId = adapter.sendAssetsToAddress(
                value,
                asset,
                receiver
            );
            _lzSend(
                _srcLzChainId,
                _encodeUpdateMessage(transferId, opId),
                payable(address(this)),
                address(0),
                "",
                address(this).balance
    ...
    function depositFundsToRootVault(
        ...
        uint256 totalAssetsBefore = rootVault.totalAssets();
        shares = super.depositFundsToRootVault(token, value, transferId);
        uint256 totalAssetsAfter = rootVault.totalAssets();
        uint256 fromChainId = bridgeAdapterToChainId[msg.sender];
        uint16 lzChainId = chainIdToLzChainId[fromChainId];
        _lzSend(lzChainId, _encodeDepositedFromBridgeMessage(transferId, totalAssetsBefore, totalAssetsAfter), payable(address(this)), address(0), "", address(this).balance);
    }
}
...
interface IRootVault {
    ...
    function convertToLedger(uint256 assets) external view returns (uint256);
...
abstract contract CrossLedgerVault is LzApp {
	...
	enum LzMessage { UPDATE, REDEEM, REBALANCE, TRANSFER_COMPLETED, DEPOSITED_FROM_BRIDGE }
	...
	function _encodeRebalanceMessage(uint256 assets, address asset) pure internal returns(bytes memory) {
		bytes memory params = abi.encode(assets, asset);
		return abi.encode(LzMessage.REBALANCE, params);
	}

	function _encodeTransferCompletedMessage(bytes32 transferId) pure internal returns(bytes memory) {
		bytes memory params = abi.encode(transferId);
		return abi.encode(LzMessage.TRANSFER_COMPLETED, params);
	}
    ...
    function updateBridgeAdapter(
        ...
        require(tokens.contains(asset), "That token has not been added yet");
        require(
    ...
    function totalAssets() public view returns (uint256 total) {
        ...
        for (uint256 i = 0; i < tokens.length(); i++) {
            ...
            for (
                uint256 j = 0;
                j < chains.length();
                j++ // add balances of adapters
            ) {
                address adapter = address(
                    bridgeAdapters[address(token)][chains.at(j)]
                );
                if (adapter != address(0)) {
                    total +=
                        (token.balanceOf(adapter) * 10**18) /
                        10**token.decimals(); // todo: can be optimized
                }
            }
        }
    ...
    function depositFundsToRootVault(IERC20 token, uint256 value, bytes32)
        public
		virtual
        returns (uint256 shares)
    {
        token.safeTransferFrom(msg.sender, address(this), value);
        return rootVaultZap.deposit(address(token), value);
    }
...
{
	function sendAssetsToRootVault(uint256 value, address asset) external returns(bytes32 transferId);

	function sendAssetsToAddress(uint256 value, address asset, address receiver) external returns(bytes32 transferId);
}
