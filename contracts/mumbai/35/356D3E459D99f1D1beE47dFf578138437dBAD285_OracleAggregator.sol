...
contract OracleAggregator is Ownable{
    ...
    function getTokenPrice(address token) external view returns (uint256 tokenPrice){
        // token / eth 
    ...
    function getTokenValueOfOneEth(address token) external view virtual returns (uint256 exchangeRate) {
       ...
       exchangeRate = (10 ** _tokenOracleDecimals) * (10 ** IERC20Metadata(token).decimals()) / tokenPriceUnadjusted; 
    }

    function _getTokenPrice(address token) internal view returns (uint256 tokenPriceUnadjusted){
