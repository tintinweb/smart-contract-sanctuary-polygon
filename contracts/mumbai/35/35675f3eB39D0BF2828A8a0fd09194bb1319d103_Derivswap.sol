...
contract Derivswap {
    ...
    function settleOrder(
        ...
        (, , address maker_order_collateral, , , , , ) = derivswap_orderbook
            .makerOrdersByID(makerOrderID);
        if(maker_order_collateral!=address(0)){
            require(msg.value==pyth_update_fee, "Msg.value must == pyth_update_fee if alternative collateral is set on the maker order.");
        }
        ...
        uint256 protocol_earnings_deducted;
        uint256 protocol_profit;
        if (maker.collateral == address(0)) {
        //     // Update protocol earnings
        //     protocol_earnings_deducted = (payout.protocol_profit -
        //         (pyth_update_fee * maker.num_legs));
        //     protocol_profit = protocol_earnings_deducted;
            protocol_profit = payout.protocol_profit - (pyth_update_fee);
        } else {
        //     protocol_earnings_deducted = 0-(pyth_update_fee * maker.num_legs);
        //     protocol_earnings_by_asset[maker.collateral] += payout
        //         .protocol_profit;
        //     protocol_profit = payout.protocol_profit;
            protocol_profit = payout.protocol_profit;
        }

        // protocol_earnings_by_asset[address(0)] += protocol_earnings_deducted;
        ...
        if (maker.collateral == address(0)) {
            ...
            if (initialLiquidityProvided == true) {
                if (drvs.balanceOf(pool_address) > ((5 * (10 ** 18)))) {
                    ...
                    swapExactInputSingle(protocol_earnings_deducted / 4);
