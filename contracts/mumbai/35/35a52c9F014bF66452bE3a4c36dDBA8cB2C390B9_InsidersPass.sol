...
//public to check correct mapping
    ...
    event PromoCodeUsed(string value); 
    ...
    function checkPrice(uint256 _price) internal view {
    ...
    function mintTicketsWithFixPrice(
        ...
        require(ticketTypeDetails[ticketID].isMintable, "Not Mintable"); // write a = to valid a date of current timestamp
        require( eventDetails[ticketTypeDetails[ticketID].eventIDforTicket].eventEndDate >= block.timestamp,"Event Passed"  );
        require(
        ...
        if (_msgSender() != owner()) {
            if (
                keccak256(abi.encodePacked(_promoCode)) !=
                keccak256(abi.encodePacked(""))
            ) {
                require(
                    promoCodeDetails[_promoCode].claimedUsers <
                        promoCodeDetails[_promoCode].maxUsers,
                    "UsersLimitReached"
                );
                require(
                    promoCodeDetails[_promoCode].promoStartDate <
                        block.timestamp &&
                        promoCodeDetails[_promoCode].promoEndDate >
                        block.timestamp,
                    "NotValid"
                );
                require(
                    promoIsValid[_promoCode][
                        ticketTypeDetails[ticketID].eventIDforTicket
                    ],
                    "InvalidPromo"
                );

             if(promoCodeDetails[_promoCode].isFree){
                require(promoIsValidForTicket[_promoCode][ticketID],"PromoCodeIsNotValidForThisTicket");
                       uint256 newDiscountedAmount = 0;
                            checkPrice(
                                (
                                    (newDiscountedAmount * noOfCopies)
                                )
                            );
                            promoCodeDetails[_promoCode].claimedUsers++;
             }else
             if (promoCodeDetails[_promoCode].discountIsPercentage) { // check again formula of caluclating on percentage
                        uint256 newDiscountedAmount =  (((ticketTypeDetails[ticketID].ticketFixPrice * noOfCopies) * (promoCodeDetails[_promoCode].discountPercentage ))/ 100) ;

                        if (promoCodeDetails[_promoCode].maxDiscountAmount >= newDiscountedAmount ) {
                                                        checkPrice((( (ticketTypeDetails[ticketID].ticketFixPrice * noOfCopies) - newDiscountedAmount )));
                                                  promoCodeDetails[_promoCode].claimedUsers++;
                        }
                        else{
                            newDiscountedAmount = (ticketTypeDetails[ticketID].ticketFixPrice * noOfCopies) - promoCodeDetails[_promoCode].maxDiscountAmount;
                            checkPrice(((newDiscountedAmount)));
                            promoCodeDetails[_promoCode].claimedUsers++;
                        }
             }
                    ...
                    else if (!promoCodeDetails[_promoCode].discountIsPercentage && !promoCodeDetails[_promoCode].isFree) {
                        uint256 newDiscountedAmount = (ticketTypeDetails[ticketID].ticketFixPrice * noOfCopies ) - (promoCodeDetails[_promoCode].fixedDiscount);
                        require(newDiscountedAmount >= 0,"InvalidDiscountPriceOnFixedDiscount");
                               checkPrice(((newDiscountedAmount)));
                            promoCodeDetails[_promoCode].claimedUsers++;
    ...
    function registerEvent(
        ...
        addTicketType(newEventID, _TicketTypes);
    ...
    function scanTickets(uint256 ticketID, address _user)
        public
        view
        returns (bool)
    {
        ...
        require(
            balanceOf(_user, ticketID) > 0 ||
                Ticket[_user][ticketID].Exists == true,
            "YouDontHaveThisTicket"
        );
        bool response;
        for (
            uint256 i = 0;
        ...
        onlyOwner
    {
        ...
        for (uint256 i = 0; i < _ticketTypes.length; i++) {
            ...
            for (uint256 j = 0; j < _ticketTypes[i].validDates.length; j++) {
                require(
                    (eventDetails[eventID].eventStartDate <= _ticketTypes[i].validDates[j]) &&
                        (eventDetails[eventID].eventEndDate >=
                            _ticketTypes[i].validDates[j]),
                    "ValidityDatesMustBeWithInEventDates"
                );
            }
    ...
    function updateTicket(
        ...
        require(
            _ticket.eventIDforTicket ==
                ticketTypeDetails[_ticketID].eventIDforTicket,
            "InvalidEventID!"
        );
        ticketTypeDetails[_ticketID] = _ticket;
        setURI(_ticketID, _ticket.ticketURI);
        emit TicketCreated(ticketTypeDetails[_ticketID], _ticketID);
    }

    function forceMintTickets(
        address[] memory addresses,
        uint256[] memory ticketIDs,
        uint256[] memory noOfCopies
    ) external onlyOwner {
    ...
    function placeTicketForFixedPrice(uint256 ticketID, uint256 _ticketFixPrice)
        internal
        onlyOwner
    {
...
// added here modifier to withDraw amount
    function withdrawAmount(uint256 amount) external payable onlyOwner{
        require(
            user_Balance[_msgSender()] >= amount,
            "Balance>=WithdrawAmount"
        );
        //added for withdraw balance of Smart contract
        user_Balance[_msgSender()] -= amount;
...
abstract contract InsidersPassAuction is
    ...
    ERC1155,
    Pausable,
    InsidersPassRoyalties 
{
    event availableForAuction(uint256, string);
    event availableForFixPrice(uint256, string);
    event availableForRent(uint256, string);
    event removeFormSale(uint256, string);
    event packageCreated(uint256 packageID, ticketPackage);
    event bidAccepted(uint256 bidAmount, address receiver);
    event packageSold(uint256 packageID, string message);
    enum saleTypeChoice {
        onAuction,
        onRent,
        OnfixedPrice,
    ...
    struct ticketPackage {
        ...
        uint256 bidIncrementPercentage;
        uint256 index;
    ...
    mapping(uint256 => bool) public eventIsPaused;

    mapping(address => uint256) BidsAmount;

    function NftExist(address _owner, uint256 ticketID) internal view {
        require(
            Ticket[_owner][ticketID].Exists == true,"NFTDoes'tExist");
    }

    function eventIsNotPaused(uint256 _eventID) internal view {
        require(eventIsPaused[_eventID] == false, "EventIsPaused");
    }

    function eventIsRegistered(uint256 _eventID) internal view {
        require(eventExists[_eventID] == true, "EventNotExist");
    }

    function TicketExists(uint256 ticketID) internal view {
        require(ticketExists[ticketID] == true, "TicketNotExist");
    }

    function PackageExists(uint256 packageID) internal view {
        require(
            ticketsPackage[packageID].Exists == true,
            "Package Does't Exist "
        );
    }
     function isPackageOwner(uint256 packageID) internal view {
        require(
                ticketsPackage[packageID].packageOwner == _msgSender(),"PkgOwnerHasAccess");
    }


    function onFixedPrice(uint256 packageID) internal view {
        require(
    ...
    function isOnRent(uint256 packageID) internal view {
        require(
            ticketsPackage[packageID].salestatus == saleTypeChoice.onRent &&
                ticketsPackage[packageID].Exists,
            "NotOnRent"
        );
    }

    function onAuction(uint256 packageID) internal view {
        require(
    ...
    function deletePackage(uint256 packageID) external {
        PackageExists(packageID);
        isPackageOwner(packageID);
        for (
            uint256 k = 0;
            k < ticketsPackage[packageID].ticketIDs.length;
            k++
        ) {
            CopiesOnBidOrSale[ticketsPackage[packageID].packageOwner][
                ticketsPackage[packageID].ticketIDs[k]
            ] -= ticketsPackage[packageID].numOfCopies[k];
        }
    ...
    function buyPackageForFixedPrice(
        ...
        PackageExists(packageID);
        require(
            eventDetails[_eventID].eventEndDate > block.timestamp,
            "Event Passed"
        );
        require(
        ...
        emit packageSold(packageID, "SoldForFixedPrice");
    }
    ...
    function placePackageForTimedAuction(
        ...
        checkPackageConditions(_eventID, _ticketIDs, ticketCopiesCount);
        packagesCount++;
        ...
        setApprovalForAll(address(this), true);
        emit availableForAuction(newPackageID, "AcceptingBids");
    }
    ...
    function placePackageForFixedPrice(
        ...
        setApprovalForAll(address(this), true);
        emit availableForFixPrice(newPackageID, "AvailableForFixedPrice");
    }

    function checkPackageConditions(
        uint256 _eventID,
        uint16[] memory _ticketIDs,
        uint32[] memory ticketCopiesCount
    ) internal {
        ...
        for (uint256 i = 0; i < _ticketIDs.length; i++) {
             require(ticketTypeDetails[_ticketIDs[i]].isMintable,"Not Mintable");
            require(
                (super.balanceOf(msg.sender, _ticketIDs[i]) -
                    CopiesOnBidOrSale[msg.sender][_ticketIDs[i]]) >=
                    ticketCopiesCount[i] &&
                    (_eventID ==
                        ticketTypeDetails[_ticketIDs[i]].eventIDforTicket),
                "CopiesNotAvailable"
            );
    ...
    function claimPackage(uint256 packageID) external {
        PackageExists(packageID);
        require(
            block.timestamp >= ticketsPackage[packageID].endTime,"RentTimeNotOver"
        );
        ticketsPackage[packageID].Exists = false;
        for (
            uint256 i = 0;
            ...
            Ticket[ticketsPackage[packageID].packageOwner][
                ticketsPackage[packageID].ticketIDs[i]
            ].Exists = true;
            this.safeTransferFrom(
                address(this),
            ...
            emit packageSold(packageID, "SoldClaimedByOriginalOwner");
        }
    }

    function placePackageForSale(
        uint256 packageID,
        uint16[] memory _ticketIDs,
        uint32[] memory ticketCopiesCount,
        uint32 _startTime,
        ...
        ticketsPackage[packageID].startTime = _startTime;
        ticketsPackage[packageID].numOfCopies = ticketCopiesCount;
        ticketsPackage[packageID].endTime = _endTime;
    ...
    {
        PackageExists(packageID);
        uint256 highestBidAmount = ticketsPackage[packageID].bidAmount[
        ...
        for (
            ...
            if (
                ...
                (_eventID ==
                    ticketTypeDetails[ticketsPackage[packageID].ticketIDs[i]]
                        .eventIDforTicket)
            ) {
                result = false;
            }
        ...
        if (result == false) {
            for (
                uint256 j = 0;
                j < ticketsPackage[packageID].bidderAddress.length;
                j++
            ) {
                payable(ticketsPackage[packageID].bidderAddress[j]).transfer(
                    ticketsPackage[packageID].bidAmount[j]
                );
            }
            for (
                uint256 k = 0;
            ...
            for (
                uint256 m = 0;
                m < ticketsPackage[packageID].ticketIDs.length;
                m++
            ) {
                CopiesOnBidOrSale[ticketsPackage[packageID].packageOwner][
                    ticketsPackage[packageID].ticketIDs[m]
                ] -= ticketsPackage[packageID].numOfCopies[m];
                Ticket[
                    ticketsPackage[packageID].bidderAddress[
                        ticketsPackage[packageID].bidderAddress.length - 1
                    ]
                ][ticketsPackage[packageID].ticketIDs[m]].Exists = true;
            ...
            for (
                uint256 l = 0;
                l < ticketsPackage[packageID].bidderAddress.length - 1;
                l++
            ) {
                payable(ticketsPackage[packageID].bidderAddress[l]).transfer(
                    ticketsPackage[packageID].bidAmount[l]
                );
            }
            emit bidAccepted(
                highestBidAmount,
            ...
            emit packageSold(packageID, "SoldForTimedAuction");
    ...
    function addAuctionBid(uint256 packageID, uint256 _bidAmount)
        external
        payable
    {
        PackageExists(packageID);
        ...
        require(
            eventDetails[
                ticketTypeDetails[ticketsPackage[packageID].ticketIDs[0]]
                    .eventIDforTicket
            ].eventEndDate > block.timestamp,
            "Event Passed"
        );
        require(
            block.timestamp <= ticketsPackage[packageID].endTime,
            "AuctionTimeOver"
        );
