...
// File contracts/Rskullz.sol


pragma solidity ^0.8.7;
contract Rskullz is ERC721Enumerable, Ownable {
    using SafeMath for uint256;
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    uint256 public constant MAX_SUPPLY = 617;
    ...
    // Constructor
    constructor(string memory baseURI) ERC721("RSKullz", "RSKZ") {
        setBaseURI(baseURI);
    }
    ...
    function _mintToSingleNFT(address _to) private {
        uint256 newTokenID = _tokenIds.current();
        _safeMint(_to, newTokenID);
        _tokenIds.increment();
    }
    // Function to reserver NFT by the owner for some address
    function reserveNFT(address _to, uint256 _reserve) public payable onlyOwner {
        ...
        for (uint256 i = 0; i < _reserve; i++) {
            _mintToSingleNFT(_to);
        }
    }

   function airdropNFT_singleAddress(address _to, uint _quantity) public onlyOwner {
        uint256 newTokenID = _tokenIds.current();
...
// For airdropping bulk, check erc1155 safe
    // function airdropNFTsBulk(address[] memory _to, uint[] memory _quantity) public onlyOwner {
    //     require(_to.length == _quantity.length,"Addresses are not assigned NFTs properly");
    //     uint256 newTokenID = _tokenIds.current();
    //     for(uint i=0; i< _to.length; i++){
    //         for(uint j=0; j<[i][_quantity]; j++){
    //             _safeMint(_to, newTokenID);
    //             _tokenIds.increment();        
    //         }
    //     }
    ...
    function _mintNFTs(uint256 _count) public payable {
        ...
        require(
            _count > 0 && _count <= 5,
            "Minted NFTs shud be greater then 0 and less or equal to then 5"
        );
