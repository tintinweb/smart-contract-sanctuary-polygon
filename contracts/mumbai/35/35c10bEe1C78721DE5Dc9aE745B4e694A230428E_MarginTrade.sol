...
contract MarginTrade is Ownable {
    ...
    function createPositionandTrade(
        ...
        uint depositWithoutReserve = (depositInfo.assetAmount - (depositInfo.assetAmount.mul(reserveMargin).div(100))); // (80 - 80*0.2)
        uint marginForPosition  = (amount * depositWithoutReserve / (depositWithoutReserve.add(depositInfo.loanAmount)));
        uint loanForPosition = (amount.sub(depositInfo.assetAmount));
        ...
        uint _liquidationPrice = (amount - ((depositInfo.assetAmount * 80) / 100)) / (amount / tradePrice);
