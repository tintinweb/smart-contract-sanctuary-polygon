...
contract TTMVester is Owned, ReentrancyGuard {
    struct VestingSchedule {
        ...
        uint32 startTime; // Changed to uint32
        // cliff time of the vesting start in seconds since the UNIX epoch
        uint32 cliffTime; // Changed to uint32
        // duration of the vesting period in hours
        uint32 durationHours; // Changed to uint32
        // duration of a slice period for the vesting in hours
        uint32 slicePeriodHours; // Changed to uint32 and renamed
     ...
     * @param _slicePeriodHours duration of a slice period for the vesting in seconds
     * @param _revocable whether the vesting is revocable or not
     * @param _amount total amount of tokens to be released at the end of the vesting
     */
    function createVestingSchedule(
        address _beneficiary,
        uint32 _startTime, // Changed to uint32
        uint32 _cliff, // Changed to uint32
        uint32 _duration, // Changed to uint32
        uint32 _slicePeriodHours, // Changed to uint32 and renamed
        bool _revocable,
        uint256 _amount
    ) external onlyOwner {
        require(
        ...
        uint32 cliffTime = _startTime + _cliff * 3600; // convert _cliff to second and add to _stratTime
    ...
    function _computeReleasableAmount(
        ...
        // Otherwise, some tokens are releasable.
            ...
            uint32 hoursFromStart = (currentTime - vestingSchedule.startTime) /
                3600;
            uint32 vestedSlicePeriods = hoursFromStart /
                vestingSchedule.slicePeriodHours;
            uint32 vestedHours = vestedSlicePeriods *
                vestingSchedule.slicePeriodHours;
            // Compute the amount of tokens that are vested.
            uint256 vestedAmount = (vestingSchedule.amountTotal * vestedHours) /
                vestingSchedule.durationHours;
