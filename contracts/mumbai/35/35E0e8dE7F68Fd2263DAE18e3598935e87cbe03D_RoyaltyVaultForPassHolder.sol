...
contract RoyaltyVaultForPassHolder is AccessControl, ReentrancyGuard{
  ...
  uint256 public batchWithdrawnId;
  address public WETH;
  ...
  function batchWithdraw(uint256 counter) external onlyRole(WITHDRAWER_ROLE) payable nonReentrant{
    if(batchWithdrawnId + counter > requestId) revert NoWithdrawableRequest();
    // Transfer accounts of requestIdToRequest[].members
    for(uint i=batchWithdrawnId; i<= batchWithdrawnId + counter; i++) {
      address[] memory members = requestIdToRequest[i].members;
      for (uint j=0; j<members.length; j++) {
        address account = members[j];
        uint256 claimableETH = claimablePerAccount[account].amountETH;
        uint256 claimableWETH = claimablePerAccount[account].amountWETH;
        if(_isETHClaimable(claimableETH)){
          withdrawnPerAccount[account].amountETH += claimableETH;
          claimablePerAccount[account].amountETH = 0;
          _transfer(account, claimableETH);
        }
        if(_isWETHClaimable(claimableWETH)){
          withdrawnPerAccount[account].amountWETH += claimableWETH;
          claimablePerAccount[account].amountWETH = 0;
          IERC20(WETH).transfer(account, claimableWETH);
        }
        emit Withdrawed(msg.sender, claimableETH, claimableWETH);
      }
    }
    batchWithdrawnId += counter;
  }
  ...
  error TooMuchWithdrawETH(uint256 actual, uint256 claimed);
  function _isETHClaimable(uint256 claimed) internal view returns (bool){
    uint256 ETHbal = address(this).balance;
    if(claimed > ETHbal) revert TooMuchWithdrawETH(ETHbal, claimed);
    return true;
  }

  error NoWETHAddress();
  error TooMuchWithdrawWETH(uint256 actual, uint256 claimed);
  function _isWETHClaimable(uint256 claimed) internal view returns (bool){
    if(WETH == address(0)) revert NoWETHAddress();
    uint256 WETHbal = IERC20(WETH).balanceOf(address(this));
    if(claimed > WETHbal) revert TooMuchWithdrawETH(WETHbal, claimed);
    return true;
  }
}