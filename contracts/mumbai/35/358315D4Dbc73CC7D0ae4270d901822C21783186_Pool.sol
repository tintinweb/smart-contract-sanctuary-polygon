...
contract Pool is VersionedInitializable, PoolStorage, IPool {
    ...
    returns(uint256)
  {
    ...
    address prxyAsset = _reserves[supplyingAsset].prxyAsset;
  ...
  function getCollateralRate(address supplyingAsset, address borrowingAsset) external view override returns (uint256) {
    uint256 maxLtv = 10000 * WadRayMath.RAY / _reserves[supplyingAsset].configuration.getLiquidationThreshold();
    ...
    maxLtv = maxLtv * (supplyingUnit * borrowingAssetPrice) / (borrowingUnit * supplyingAssetPrice);
    return maxLtv;
  }
...
library BorrowLogic {
  ...
  function executeBorrow(
    ...
    ValidationLogic.validateBorrow(reserve, reservesList, params.reservesCount, params.borrowingAsset, params.amount, params.oracle);
...
library ValidationLogic {
   ...
   */
  function validateBorrow(DataTypes.ReserveData storage reserve, mapping(uint256 => address) storage reservesList, uint256 reservesCount, address asset, uint256 amount, address oracle)
    internal
    view
  {
    require(amount != 0, Errors.INVALID_AMOUNT);
