...
/**
  The Cron contract is a ankr automate-powered cron job runner for smart contracts.
  ...
  To use this contract, a user first deploys this contract and registers it on the ankr
...
/**
 * @title The CronTask contract
 * @notice A automate contract that runs various tasks on cron schedules.
 ...
 */
contract CronTask is IAutomateCompatible, AutomateBase, ConfirmedOwner, Pausable, Proxy {
  ...
  address internal immutable s_delegate;
   ...
   * @param delegate the contract to delegate checkTask calls to
  ...
  constructor(
    ...
    if (firstJob.length > 0) {
      (address target, bytes memory handler, Spec memory spec) = abi.decode(
        firstJob,
        (address, bytes, Spec)
      );
   ...
   */
  function performTask(bytes calldata performData) external override whenNotPaused {
  ...
  /**
   * @notice Pauses the contract, which prevents executing performTask
   */
   ...
   * @return taskNeeded signals if task is needed, performData is an abi encoding
   * of the id and "next tick" of the elligible cron job
   */
  function checkTask(bytes calldata)
    external
    override
    whenNotPaused
    notExecute
  ...
  /**
   * @notice validates the input to performTask
...
/**
 * @title The CronTaskDelegate contract
 * @notice This contract serves as a delegate for all instances of CronTask. Those contracts
 * delegate their checkTask calls onto this contract. Utilizing this pattern reduces the size
 * of the CronTask contracts.
 */
contract CronTaskDelegate {
  using EnumerableSet for EnumerableSet.UintSet;
  using Cron for Spec;
   ...
   */
  function checkTask(bytes calldata) external view returns (bool, bytes memory) {
