...
// File: contracts/SafeMathWithRequire.sol
...
// File: contracts/NftContributionCalculator.sol
...
contract NftContributionCalculator is IContributionCalculator, Ownable {
    ...
    function computeContribution(address account, uint256 amountStaked) external view override returns (uint256) {
        uint256 numNfts = multiplierNFToken.balanceOf(account);
        return _contribution(amountStaked, numNfts);
    }

    function contribution(uint256 amountStaked, uint256 numNfts) external pure returns (uint256) {
        return _contribution(amountStaked, numNfts);
    }
    ...
    function _contribution(uint256 amountStaked, uint256 numNfts) internal pure returns (uint256) {
        if (numNfts == 0) {
            return amountStaked;
        }
        uint256 nftContrib =
            NFT_FACTOR_6 * (NFT_CONSTANT_3 + SafeMathWithRequire.cbrt3((((numNfts - 1) * ROOT3_FACTOR) + 1)));
