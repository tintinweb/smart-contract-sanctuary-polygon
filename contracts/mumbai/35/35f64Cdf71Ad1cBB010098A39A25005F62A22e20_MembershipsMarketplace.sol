 ...
 * @dev See https://github.com/Fanszoid/contracts/blob/main/src/Memberships.sol
 */
interface IMembership is IERC1155 {
    ...
    // @dev for transferin membership ownership
 ...
 * @dev This Fanszoid's Marketplace is a smart contract that allows you manage memberships.
...
contract MembershipsMarketplace is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event MembershipDeleted(uint256 indexed membershipId, address owner, uint256[] ids, uint256[] amounts);
    ...
    /// @dev Verifies that the sender is either the marketplace's owner nor the given membership's creator.
        ...
        returns (uint256[] memory membershipIds)
    {
        ...
        for (uint256 i = 0; i < quantity; i++) {
            emit MembershipPublished(
                msg.sender,
                newMembershipIds[i],
                amounts[i],
                memberships[i].price,
                memberships[i].royalty,
                memberships[i].amountToSell,
                memberships[i].isResellable,
                uris[i]
            );
        }
    ...
    function deleteMembership(
        ...
        uint256[] memory ids,
        uint256[] memory amounts
    ) external whenNotPaused onlyMembershipCreatorOrContractOwner(membershipId) {
        ...
        emit MembershipDeleted(membershipId, msg.sender, ids, amounts);
    }
    ...
    function claimFreeMembership(
        ...
        require(IMembership(membershipAddress).balanceOf(seller, membershipId) >= 1, 'Seller hasnt enough memberships.');
        require(IMembership(membershipAddress).balanceOf(claimer, membershipId) == 0, 'Claimer has a membership.');
        require(offers[seller][membershipId].price == 0, 'Membership is not free.');
        require(offers[seller][membershipId].amount >= 1, 'Not enough memberships for claim');

        IMembership(membershipAddress).safeTransferFrom(address(seller), claimer, membershipId, 1, '');
        offers[seller][membershipId].amount -= 1;

        emit MembershipBought(membershipId, seller, claimer, 0, 1);
    }
    ...
    function setAsk(
        ...
        require(IMembership(membershipAddress).balanceOf(msg.sender, membershipId) > 0, 'Sender does not have membership.');
