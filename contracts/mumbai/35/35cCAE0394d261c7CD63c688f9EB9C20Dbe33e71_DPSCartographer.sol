...
contract DPSCartographer is Ownable, ReentrancyGuard {
    ...
    function buyVoyages(VOYAGE_TYPE _voyageType, uint256 _amount) external nonReentrant {
        ...
        require(tmap.balanceOf(msg.sender) >= amountOfTmap * _amount, "Not enought TMAP");
        for (uint256 i = 0; i < _amount; i++) {
            ...
            voyage.mint(msg.sender, voyages, voyageConfig);
            emit VoyageCreated(msg.sender, voyages, _voyageType);
        }
    }
     ...
     */
    function reinitializeVoyages(uint256 _length) external onlyOwner {
        voyages = _length;
    }
