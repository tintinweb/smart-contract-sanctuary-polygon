...
contract ApeliensLarvae is BaseERC721 {
    error InvalidAddress();
    error InvalidArrayLength();
    error NotOwner();

    IERC721Enumerable public bapeliens;

    constructor(
        address _bapeliens
    ) BaseERC721("Apeliens Larvae", "LARVAE", "", "", 3333, 0xb80C2654ac73730D508028dD4e84F13913B2DDb1, 750) {
        bapeliens = IERC721Enumerable(_bapeliens);
    }

    function claim(uint256[] calldata _tokenIds) external whenNotPaused {
        uint256 tokenIdsLength = _tokenIds.length;
        if (tokenIdsLength == 0) revert InvalidArrayLength();
        if (address(bapeliens) == address(0)) revert InvalidAddress();

        for (uint256 i = 0; i < tokenIdsLength; ) {
            if (bapeliens.ownerOf(_tokenIds[i]) != msg.sender) revert NotOwner();
    ...
    function checkClaimed(uint256 _tokenId) public view returns (bool) {
        return _exists(_tokenId);
    }

    function unclaimedTokenIds(address _to) public view returns (uint256, uint256[] memory) {
        if (_to == address(0)) revert InvalidAddress();
        if (address(bapeliens) == address(0)) revert InvalidAddress();

        uint256 balance = bapeliens.balanceOf(_to);
        uint256[] memory unclaimed = new uint256[](balance);
        uint256 idx = 0;
        for (uint256 i = 0; i < balance; ) {
            uint256 tokenId = bapeliens.tokenOfOwnerByIndex(_to, i);
            if (!_exists(tokenId)) {
                unclaimed[idx] = tokenId;
