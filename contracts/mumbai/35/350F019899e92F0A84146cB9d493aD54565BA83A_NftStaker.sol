...
contract NftStaker is Ownable{
    ...
    function _restake(uint256 depositId) internal {
        ...
        uint256 rewardMultiplier = ((block.timestamp - nftDeposits[depositId].depositTime) / stakingTime) * (stakeWeight[nftDeposits[depositId].tokenId] / stakedTotal);
    ...
    function withdrawRewards(uint256 depositId) public {
        require(depositId <= nftDeposits.length);
        ...
        for (uint256 i = 0; i < nftDeposits.length; i++) {
            if (nftDeposits[i].depositOwner==msg.sender){
                if ((block.timestamp - nftDeposits[i].depositTime) >= nftDeposits[i].timeLockInSeconds+stakingTime) {
                    uint256 rewardMultiplier = ((block.timestamp - nftDeposits[i].depositTime) / stakingTime) * (stakeWeight[nftDeposits[i].tokenId] / stakedTotal);
