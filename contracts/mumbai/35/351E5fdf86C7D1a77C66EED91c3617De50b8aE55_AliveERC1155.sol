...
import "./required_imports/ERC1155Supply.sol";
    ...
    ERC1155Supply
{
    ...
    mapping(uint => bool) public isValidTokenId; // to check if a token id exists in a erc1155 collection
    mapping(uint => uint) public maxSupplyOfTokenId; // maxSupply
    mapping(uint => uint) public priceOfTokenId;
    ...
    // manager is fetched from registry contract, a manger has permission for functions that deploy some sort of contract or reset some data
    ...
    constructor(
        ...
        uint initialeTokenId,
        uint initialPrice,
        uint initialMaxSupply, 
        string memory baseURI, 
        string memory contractURi, 
        address _aliveRegistry
    )
        ERC1155(baseURI
            // "https://alive-static-assets.s3.ap-south-1.amazonaws.com/artist/huyana/"
        )
    {
        _pause();

        allTokenIds.push(initialeTokenId);
        isValidTokenId[initialeTokenId] = true;
        maxSupplyOfTokenId[initialeTokenId] = initialMaxSupply;
        priceOfTokenId[initialeTokenId] = initialPrice;
    ...
    function createNewToken(
        uint initialeTokenId,
        uint initialPrice,
        uint initialMaxSupply) public onlyAdmin {
...
function changeAliveRegistry(address _aliveRegistry) public onlyAdmin {
aliveRegistry = _aliveRegistry;
}
 ...
 function contractURI() public view returns (string memory) {
    ...
    function unpause() public virtual onlyGovernor {
        require(
            ...
            "ERC1155PresetMinterPauserAutoId: must have pauser role to unpause"
        );
        _unpause();
    }
    ...
    function mint(
        ...
        require(isValidTokenId[tokenId] == true, "token doesn't exist");
        uint price = priceOfTokenId[tokenId] * amount;
        require(
            msg.value >= priceOfTokenId[tokenId] * amount,
            "Insufficient funds!"
        );
        require(
            totalSupply(tokenId) + amount <= maxSupplyOfTokenId[tokenId],
    ...
    function _beforeTokenTransfer(
        ...
        bytes memory data
    ) internal override(ERC1155, ERC1155Pausable, ERC1155Supply) whenNotPaused {
