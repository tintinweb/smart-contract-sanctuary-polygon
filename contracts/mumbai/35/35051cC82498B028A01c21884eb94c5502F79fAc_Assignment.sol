...
contract Assignment is ERC1155, ERC2771Context, Ownable  {
    ...
    //Mint 1000 nft to owner's address
    ...
    constructor(string memory _name, string memory _symbol, address _trustedForwarder ) ERC1155("") ERC2771Context(_trustedForwarder) {
    ...
    //Tracking whether the NFTs are burned or not
    ...
    //Blocking transfers from non-owner wallets
    ...
    function claimNFT(address to) public {
        require(!isBurned[to], "NFT already claimed!");
        require(balanceOf(to, 1) == 0, "Only one mint per address");
        this.safeTransferFrom(owner(), to, 1, 1, "");
    }
