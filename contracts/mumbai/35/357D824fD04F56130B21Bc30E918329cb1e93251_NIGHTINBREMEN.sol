...
// File: contracts/penta_hotels.sol
...
contract NIGHTINBREMEN is Ownable, ERC721A {
    ...
    // Sales configs
    uint256 [] public maxSupply = [3,3,3,3];
    uint256 [] public currentSupply = [0,0,0,0];
    uint256 public maxPerAddress = 5;
    // Timeframes
    uint256 public windowOpens = 1655918414;
    // Metadata uris
    string public baseURI = "https://55unity.mypinata.cloud/ipfs/QmPiR9tDcjt2mjtXTRkQP95HsCm4Rix2U2DRe43MtVqqGv/";
    // Amount minted at phases
    mapping(uint256 => uint256) public tokenIdsUris;
    
    constructor() ERC721A("One Night in Bremen", "One Night in Bremen") {}
    ...
    function mint(address to, uint256 quantity, uint256 id, bytes calldata signature) external payable {
        require(block.timestamp >= windowOpens, "Purchase window closed.");
        require(verifySignature(signature, to), "Account not eligible for claim.");
        require(currentSupply[id] + quantity <= maxSupply[id], "This amount of tokens would surpass the max supply.");
    ...
    function giveaway(address to, uint256 id, uint256 quantity) external onlySender {
        require(currentSupply[id] + quantity <= maxSupply[id], "This amount of tokens would surpass the max supply for this token.");
        require(quantity + _numberMinted(to) <= maxPerAddress, "User already received max per address.");
