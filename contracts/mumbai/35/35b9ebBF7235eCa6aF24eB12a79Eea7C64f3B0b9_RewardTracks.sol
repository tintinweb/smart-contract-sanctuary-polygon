...
contract RewardTracks is OwnableUpgradeable{
    ...
    function _addTrackToSpecificCard(
      ...
      returns (uint256){
        ...
        // set new track id
        if(isAdvertisement){
            require(IERC721Upgradeable(_advContractAddress).ownerOf(_advCardId) == msg.sender,'adv crd isnt urs');
        }
    ...
    function _addTrackToContract(
      ...
      returns(uint256) {
        ...
        if(isAdvertisement){
            IERC721Upgradeable(_advContractAddress).ownerOf(_advCardId);
        }
        // Create a new struct of type "TrackMeta" 
