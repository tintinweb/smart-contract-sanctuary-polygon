...
contract HTokenHelper is HTokenHelperI, Ownable {
  ...
  function getMarketOraclePrices(HTokenI _hToken) external view override returns (uint256, uint256) {
    (uint128 floorPriceInETH, uint128 lastUpdated) = priceOracle.getUnderlyingFloorNFTPrice(
   ...
   */
  function getFloorPriceInUSD(HTokenI _hToken) external view override returns (uint256) {
    (uint128 floorPriceInETH, uint128 lastUpdated) = priceOracle.getUnderlyingFloorNFTPrice(
      address(_hToken.collateralToken()),
      _hToken.decimals()
    );
    ...
    return (floorPriceInETH * ethPrice) / 10**_hToken.decimals();
  }
   ...
   * @return underlying price in usd
   */
  function getUnderlyingPriceInUSD(HTokenI _hToken) external view override returns (uint256) {
