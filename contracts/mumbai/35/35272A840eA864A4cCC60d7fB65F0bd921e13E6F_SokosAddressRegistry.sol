...
import {AddressRegistryLib} from "./library/AddressRegistryLib.sol";
    ...
    UUPSUpgradeable
{
    ...
    AddressRegistryLib.Collection[] public collections;
    ...
    function createCollection(
        ...
        require(
            ...
            string(
                abi.encodePacked(
                    "AR:",
                    AddressRegistryLib.toAsciiString(_msgSender())
                )
            )
        );
        for (uint256 i; i < collections.length; i++) {
            AddressRegistryLib.Collection memory collection = collections[i];
            require(
    ...
    {
        for (uint256 i; i < collections.length; i++) {
            AddressRegistryLib.Collection memory coll = collections[i];
            if (
                ...
                collections.pop();
                emit AddressRegistryLib.collectionRemoved(_collection);
                return true;
            }
        }
        return false;
