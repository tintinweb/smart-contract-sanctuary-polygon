...
interface IERC165 {
    /**
     * @dev Returns true if this contract implements the contract defined by
     * `contractId`. See the corresponding
     * https://eips.ethereum.org/EIPS/eip-165#how-contracts-are-identified[EIP section]
...
/**
 * @dev Implementation of the {IERC165} contract.
 *
 * Contracts that want to implement ERC165 should inherit from this contract and override {supportscontract} to check
 * for the additional contract id that will be supported. For example:
 *
 * ```solidity
 * function supportscontract(bytes4 contractId) public view virtual override returns (bool) {
 *     return contractId == type(Mycontract).contractId || super.supportscontract(contractId);
 * }
 * ```
 *
...
/**
 * @dev Required contract of an ERC1155 compliant contract, as defined in the
...
/**
 * @dev contract of the optional ERC1155MetadataExtension contract, as defined
 ...
 * _Available since v3.1._
 */
abstract contract ERC1155W is Context, ERC165, IERC1155, IERC1155MetadataURI {
       using nftAddress for address;
 ...
 */
 abstract contract  TokenIdentifiers {
    ...
    
{
    ...
    uint256 private UpdateId;

    event PermanentURI(string _value, uint256 indexed _id);

    uint256 constant TOKEN_SUPPLY_CAP = 100000000;
    ...
    constructor(
        ...
        name = "TESTINGERC1155";
        symbol =  "DNFT";
        proxyRegistryAddress = 0xd87A3BfD0CABF7B1eBdAc4485688800dC0f4DF44 ;
        _initializeEIP712(name);
         //_setOwner(_msgSender());

         
            setTemplateURI("ipfs.io/");
    ...
    /**
     * Override isApprovedForAll to whitelist user's dynamicnft proxy accounts to enable gas-free listings.
     */
    ...
    {
        // Whitelist dynamicnft proxy contracts for easy trading.
    ...
    function burn(
        ...
        uint256 _id,
        uint256 _quantity
    ) public virtual onlyApproved(_from) onlyTokenAmountOwned(_from, _id, _quantity) {
    ...
    /**
     * This is used instead of msg.sender as transactions won't be sent by the original token owner, but by dynamicnft.
     */
    ...
    function setTemplateURI(string memory _uri) internal onlyOwnerOrProxy {
        templateURI = _uri;
    }
