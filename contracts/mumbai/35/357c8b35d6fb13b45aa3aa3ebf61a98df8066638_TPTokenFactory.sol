...
import {TPToken} from "src/TPToken.sol";

contract TPTokenFactory is Initializable, Ownable2StepUpgradeable, UUPSUpgradeable {
    address public implementation;
    address public manager;

    event CreateToken(
        address indexed token, address indexed vault, string indexed symbol, string name, uint256 maxSupply
    );
    ...
    function setManager(address _manager) external {
        require(manager == address(0), "TPE007");
        manager = _manager;
    }
    ...
    function createToken(address owner, address vault, uint256 maxSupply, string calldata name, string calldata symbol)
        ...
        returns (address)
    {
        require(msg.sender == manager, "TPE008");
        ...
        returns (address token)
    {
        ...
        token = _createProxy(implementation, initializeCalldata);
        emit CreateToken(token, vault, symbol, name, maxSupply);
    }
...
import {
    ...
    ERC20Upgradeable
} from "openzeppelin-contracts-upgradeable/contracts/token/ERC20/extensions/ERC20SnapshotUpgradeable.sol";
    ...
    UUPSUpgradeable
{
    ...
    modifier onlyManagerOwner() {
        require(msg.sender == manager.owner(), "TPE011");
        _;
    }
    ...
    {
        require(_checkTransferAddress(from, to), "TPE012");
        super._beforeTokenTransfer(from, to, amount);
    }

    function _checkTransferAddress(address from, address to) internal view returns (bool) {
        return manager.checkTransferAddresses(address(this), from, to);
    }
}
