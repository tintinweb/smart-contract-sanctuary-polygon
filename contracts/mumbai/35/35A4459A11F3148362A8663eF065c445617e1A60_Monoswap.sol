 ...
 */
contract Monoswap is OwnableUpgradeable {
  ...
  // token pool status is to track if the pool has already been created for the token
  ...
  function setVCash (address _vcash) onlyOwner external {
    vCash = IvCash(_vcash);
  }
  ...
  function listNewToken (address _token, uint _price, 
    ...
    uint256 tokenAmount,
    address from, 
    address to) external onlyRouter returns(uint _pid, uint256 liquidity) {
    _pid = _createPool(_token, _price, PoolStatus.LISTED);
    liquidity = _addLiquidityPair(_token, vcashAmount, tokenAmount, from, to);
  }
  ...
  function _addLiquidityPair (address _token, 
    ...
    if(vcashAmount>0){
      vCash.safeTransferFrom(from, address(monoXPoolLocal), vcashAmount);
    ...
    // can't add more vcash than token
    requireTokenValueGreaterThanVCashValue(_token);
  ...
  function _syncPoolInfo (address _token, uint256 vcashIn, uint256 vcashOut) internal {
    ...
    require(tokenReserve <= type(uint112).max);
  ...
  function _updateVcashBalance (address _token, 
    ...
    if(_vcashOut>0){
      ...
      require(_vcashCredit <= type(uint112).max && _vcashDebt <= type(uint112).max);
  ...
  function _updateTokenInfo (address _token, uint256 _price,
    ...
    require(_balance <= type(uint112).max);
  ...
  function getAmountOut(address tokenIn, address tokenOut, 
    ...
    if(tokenOut==vcashAddress){
      ...
      // assuming p1*p2 = k, equivalent to uniswap's x * y = k
      uint directSwapTokenOutPrice = allowDirectSwap?tokenInPoolPrice.mul(tokenOutPoolPrice).div(tokenInPrice):type(uint).max;
  ...
  // need to do this at the end of the swap
  function requireTokenValueGreaterThanVCashValue(address _token) internal {
    // token value in the pool should be greater than vCash value
    uint256 tokenBalanceVcashValue = pools[_token].price.mul(pools[_token].tokenBalance)/1e18;
    require(tokenBalanceVcashValue >= pools[_token].vcashCredit, "MonoX:TOO_MUCH_vCash");
  }
  ...
  function swapIn (address tokenIn, address tokenOut, address from, address to,
    ...
    requireTokenValueGreaterThanVCashValue(tokenIn);
    requireTokenValueGreaterThanVCashValue(tokenOut);
