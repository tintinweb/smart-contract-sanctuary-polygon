...
contract Game is IGame, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    modifier eventValidateById(uint256 eventId_) {
        require(eventId_ < freeEventInfos.length, "E");
        _;
    }

    modifier whitelistValidateById(uint256 eventFreeId_) {
        require(eventFreeId_ < countEventFree, "E");
        _;
    }
    ...
    function arrangeMissile(uint256[12] memory positions) external override nonZero(positions) nonReentrant whenNotPaused {
        ...
        if (blacklistIds[msg.sender] != 0) {
            if (uint256(blacklistInfos[blacklistIds[msg.sender]].lastestUpdateSpend) < CoreUtility._calDay(block.timestamp, LAUNCH_TIME)){
                blacklistInfos[blacklistIds[msg.sender]].spent = 0;
                blacklistInfos[blacklistIds[msg.sender]].lastestUpdateSpend = uint96(CoreUtility._calDay(block.timestamp, LAUNCH_TIME));
            }
    ...
    function shoot() external override nonReentrant whenNotPaused {
        if (blacklistIds[msg.sender] != 0) {
            if (uint256(blacklistInfos[blacklistIds[msg.sender]].lastestUpdateNumber) < CoreUtility._calDay(block.timestamp, LAUNCH_TIME)){
                blacklistInfos[blacklistIds[msg.sender]].numberUsed = 0;
                blacklistInfos[blacklistIds[msg.sender]].lastestUpdateNumber = uint96(CoreUtility._calDay(block.timestamp, LAUNCH_TIME));
            }
    ...
    function removeFromBlacklist(address account) external onlyOwner {
        ...
        require(id != 0 && account != address(0), "bl");
