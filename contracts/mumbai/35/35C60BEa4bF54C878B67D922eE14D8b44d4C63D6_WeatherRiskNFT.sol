...
contract WeatherRiskNFT is 
        ...
        onlyRole(MINTER_ROLE) 
    {
        ...
        contractState memory state = contractState(to, viewerKey, nodeKey, dappKey, startDate, endDate, programName, 0, stage);
        _contractStates[tokenId] = state;
    }

    // Token state methods

    /**
     * @param tokenId uint256 token ID of contract state to retrieve
     * @return contractState full contract state
     */
    function tokenState(uint256 tokenId)
        public
        view
        returns (contractState memory)
    {
        return _contractStates[tokenId];
    }
        ...
        recordChainlinkFulfillment(_requestId)
    {
        ...
        contractState memory state = _contractStates[_tokenId];
        state.viewerAddress = _viewerAddress;
        state.viewerKey = _viewerKey;
        _contractStates[_tokenId] = state;
    ...
    {
        contractState memory state = _contractStates[tokenId];
        require(state.endDate + EVALUATION_BUFFER < block.timestamp, "unable to call until coverage period has ended");

        Chainlink.Request memory req = buildChainlinkRequest(EVALUALTION_JOB_ID, address(this), this.fulfillEvaluation.selector);
        req.addBytes("nodeKey", state.nodeKey);      // encrypted decryption key
        req.add("uri", super.tokenURI(tokenId));        // encrypted contract terms
        req.addUint("startDate", state.startDate);   // contract coverage period state date
        req.addUint("endDate", state.endDate);       // contract coverage period end date
        req.add("programName", state.programName);   // contract program name
        ...
        recordChainlinkFulfillment(_requestId)
    {
        ...
        _contractStates[_tokenId].computedPayout = _payout;
