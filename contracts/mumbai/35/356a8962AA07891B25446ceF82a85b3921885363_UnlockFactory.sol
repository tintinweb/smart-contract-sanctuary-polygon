...
import { IAirdropOnPurchaseHook } from "../hooks/airdrop/IAirdropOnPurchaseHook.sol";
...
contract UnlockFactory is Initializable, OwnableUpgradeable {
    ...
    /// @param oldOnKeyPurchaseHook old onKeyPurchaseHook address address
    /// @param newOnKeyPurchaseHook new onKeyPurchaseHook address address
    ...
    /// @notice hook to be set as `onKeyPurchaseHook` on locks
    ...
    /// @param _onKeyPurchaseHook hook to be set as `onKeyPurchaseHook` on locks
    function initialize(
    ...
    ///      If lock has airdrop, creates an airdrop on AirdropOnPurchaseHook and transfers all tokens.
    ///      Also sets `onKeyPurchaseHook` on lock.
    ///      Emits `LockDeployed`
    ...
    function deployLock(
        ...
        if (_lock._airdropToken != address(0) && _lock._airdropAmount != 0) {
            if (onKeyPurchaseHook == address(0)) revert InvalidAddress();

            IERC20Upgradeable(_lock._airdropToken).safeTransferFrom(
                msg.sender,
                address(this),
                _lock._airdropAmount * lock.maxNumberOfKeys()
            );
            IERC20Upgradeable(_lock._airdropToken).safeApprove(
                onKeyPurchaseHook,
                _lock._airdropAmount * lock.maxNumberOfKeys()
            );
            IAirdropOnPurchaseHook(onKeyPurchaseHook).setAirdrop(
                address(lock),
                _lock._airdropToken,
    ...
    /// @param _onKeyPurchaseHook onKeyPurchaseHook address to set in deployed locks
...
interface IAirdropOnPurchaseHook is ILockKeyPurchaseHook {
