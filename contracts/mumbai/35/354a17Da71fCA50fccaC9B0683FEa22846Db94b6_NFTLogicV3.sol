...
contract NFTLogicV3 is EternalStorage {
  ...
  /**
   * @notice Mint multiple token
   * @param targets Destination addresses for new token
   * @param tokenIds Token IDs for mint
   * @param uris Json Metadata Uri strings for new token
   */
  function safeMultiMint(
    ...
    require(targets.length == tokenIds.length, "Count mismatch");
    require(targets.length == uris.length, "Count mismatch");
  ...
  /**
   * @notice Transfer nft from owner to the receiver.
   * @param from Address
   * @param to Address
   * @param tokenId NFT tokenId
   */
  ...
  /**
   * @notice Approve token to the approver
   * @param to Address
   * @param tokenId NFT tokenId
   */
