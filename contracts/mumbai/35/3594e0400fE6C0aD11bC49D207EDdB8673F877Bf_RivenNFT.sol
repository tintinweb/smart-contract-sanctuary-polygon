...
import {IRivenNFT} from "./interfaces/IRivenNFT.sol";
import {ITransferManagerNFT} from "./interfaces/ITransferManagerNFT.sol";
import {ITransferSelectorNFT} from "./interfaces/ITransferSelectorNFT.sol";
import {IWETHW} from "./interfaces/IWETHW.sol";

// Nuwton libraries
import {OrderTypes} from "./libraries/OrderTypes.sol";
import {SignatureChecker} from "./libraries/SignatureChecker.sol";

contract RivenNFT is IRivenNFT, ReentrancyGuard, Ownable {
     ...
     * @param _WETHW wrapped ether address (for other chains, use wrapped native asset)
    ...
    constructor(
        ...
        DOMAIN_SEPARATOR = keccak256(
            abi.encode(
                ...
                0x29503d7257177fbf2a87dff0d628db4f2552987532cbf15e601ba43e882d433f, // keccak256("RivenNFT");
     ...
     */
    function matchAskWithTakerBidUsingETHAndWETHW(
        OrderTypes.TakerOrder calldata takerBid,
        OrderTypes.MakerOrder calldata makerAsk
    ) external payable override nonReentrant {
        require((makerAsk.isOrderAsk) && (!takerBid.isOrderAsk), "Order: Wrong sides");
        require(makerAsk.currency == WETHW, "Order: Currency must be WETHW");
        require(msg.sender == takerBid.taker, "Order: Taker must be the sender");

        // If not enough ETH to cover the price, use WETHW
        if (takerBid.price > msg.value) {
            IERC20(WETHW).safeTransferFrom(msg.sender, address(this), (takerBid.price - msg.value));
        } else {
            require(takerBid.price == msg.value, "Order: Msg.value too high");
        }

        // Wrap ETH sent to this contract
        IWETHW(WETHW).deposit{value: msg.value}();
        ...
        // Execution part 1/2
        _transferFeesAndFundsWithWETHW(
            makerAsk.strategy,
            makerAsk.collection,
            tokenId,
            makerAsk.signer,
     ...
     * @param currency currency being used for the purchase (e.g., WETHW/USDC)
     ...
     * @param minPercentageToAsk minimum percentage of the gross amount that goes to ask
     */
    function _transferFeesAndFundsWithWETHW(
        address strategy,
        address collection,
        uint256 tokenId,
        address to,
        ...
        // 1. Protocol fee
        {
            ...
            // Check if the protocol fee is different than 0 for this strategy
            if ((protocolFeeRecipient != address(0)) && (protocolFeeAmount != 0)) {
                IERC20(WETHW).safeTransfer(protocolFeeRecipient, protocolFeeAmount);
                finalSellerAmount -= protocolFeeAmount;
            }
        }
        ...
        // 2. Royalty fee
        {
            ...
            // Check if there is a royalty fee and that it is different to 0
            if ((royaltyFeeRecipient != address(0)) && (royaltyFeeAmount != 0)) {
                IERC20(WETHW).safeTransfer(royaltyFeeRecipient, royaltyFeeAmount);
                finalSellerAmount -= royaltyFeeAmount;

                emit RoyaltyPayment(collection, tokenId, royaltyFeeRecipient, address(WETHW), royaltyFeeAmount);
            }
        }
        ...
        // 3. Transfer final amount (post-fees) to seller
        {
            IERC20(WETHW).safeTransfer(to, finalSellerAmount);
        }
    }
