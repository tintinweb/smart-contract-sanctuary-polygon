...
contract SX1155NFT is Pausable, SX1155NFTBase, RoleManager, AuthorizeManager, AccountControl {
    ...
    function supportsInterface(bytes4 _interfaceId) public view virtual override(SX1155NFTBase, RoleManager) returns (bool) {
        return super.supportsInterface(_interfaceId);
    }
    ...
    /// are allowed.
    function requestTransfer(address to, uint256 tokenId, uint256 quantity) external returns(uint256 reqId) {
        require(isGreyListed(_msgSender()) || isGreyListed(to), "SX1155: from or to not in grey list" );
        ...
        //emit GreylistTransferRequestCreated(from, to, id, reqId);
        greylistNextReqId++;
    ...
    function approveTransferRequest(uint256 reqId) external onlyIssuer {
       ...
       // emit GreylistTransferApproved(reqId);        
    }

    function canacelTransferRequest(uint256 reqId) external {
        GreylistTransferRequest memory req =  greylistTransferRequests[reqId];
      
        require(req.valid, "SX1155:Invalid request id");

        require(req.from == _msgSender(), "SX1155: Not owner of the transfer request");

        delete greylistTransferRequests[reqId];

        // emit GreylistTransferCancelled(req.from, req.to, req.tokenId, reqId);
    }

    function transferRequestInfo(uint256 reqId) external view returns(address from,
    address to, uint256 tokenId, uint256 quantity) {
        return (greylistTransferRequests[reqId].from,
        greylistTransferRequests[reqId].to, greylistTransferRequests[reqId].tokenId,
        greylistTransferRequests[reqId].quantity);        
    }

    function forceTransfer(address from, address to, uint256 tokenId, uint256 quantity) external onlyIssuer {
        _safeTransferFrom(from, to, tokenId, quantity, "");
    }

    function forceBurn(address from, uint256 tokenId, uint256 quantity) external onlyIssuer {
    ...
    function _beforeTokenTransfer(
        ...
        if (hasIssuerRole(_msgSender()))
            return;

	    // No transfers from users when paused
	    require(!isTokenPaused(), "SX1155: Token paused");

        if (isAccountFrozen(from) || isAccountFrozen(to))
            revert("SX1155: from or to account frozen");
        ...
        if (isGreyListed(from) || isGreyListed(to))
            revert("SX1155: from or to in grey list");
