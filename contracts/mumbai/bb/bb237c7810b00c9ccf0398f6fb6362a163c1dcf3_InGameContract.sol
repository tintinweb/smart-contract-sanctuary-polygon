...
// File: contracts/ThesisErc20.sol
...
// File: contracts/InGameContract.sol
...
contract InGameContract {
   ...
   function isInGame(uint256 _gameIndex, address _user)  view public returns (bool){
       for (uint256 i = 0; i< gameData[_gameIndex].joinUsers.length; i++){
           if (_user == gameData[_gameIndex].joinUsers[i]) return true;
       }
       return false;
   }
   ...
   function joinGame() public {
       ...
       require(gameStructCurrent.gameStatus == GameStatus.Pending, "Not correct data game status");
        mainToken.transferFrom(msg.sender, address(this), inGamePrice);
        require(!isInGame(indexGame, msg.sender));
        gameStructCurrent.joinUsers.push(msg.sender);
        emit JoinGame(indexGame, msg.sender);
   ...
   function finalGame(bool[] calldata _winningIndex, uint256 gameID) public isFromWhitelist {
    ...
    //    gameStructCurrent._winningIndex
        uint256 sumPenty;
        ...
        for (uint256 i = 0 ; i<_winningIndex.length;i++){
            mainToken.transfer(gameStructCurrent.joinUsers[i], inGamePrice* amountJoinGame/sumPenty * (10**18 - rateBackWinner)/10**18);
        }
