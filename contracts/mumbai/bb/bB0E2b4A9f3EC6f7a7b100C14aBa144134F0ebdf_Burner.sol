...
contract Burner is Ownable {
  ...
  constructor(uint[] memory _dates, uint _burnStep, IERC20 _token) {
    dates = _dates;
    burnStep = _burnStep;
    totalBurn = _burnStep * 12;
    token = _token;
  }

  /* @dev: Allows to update the timestamps the burn occurs on */
  function updateBurnTimestamps(uint[] memory _dates) external onlyOwner {
    dates = _dates;
    emit BurnTimestampsUpdated();
  }
  ...
  function getTotalBurned() public view returns (uint) {
    ...
    unchecked {
      while (block.timestamp >= dates[i] && i <= 11) {
        i++;
      }
    }
