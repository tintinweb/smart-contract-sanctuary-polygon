...
contract Swap is ZetaInteractor, ZetaReceiver, SwapErrors, Pausable {
    ...
    function swapExactInputSingle(XcSwapArgs calldata args) external payable {
        ...
        // Get wNATIVE
        {
            (bool wethSuccess, ) = iWETH.call{value: msg.value}("");
            if (!wethSuccess) revert ErrorTransferringTokens(iWETH);
        }

        // Transfer the specified amount of weth to this contract.
        TransferHelper.safeTransferFrom(iWETH, msg.sender, address(this), msg.value);

        // Approve the router to spend weth.
        TransferHelper.safeApprove(iWETH, address(swapRouter), msg.value);
