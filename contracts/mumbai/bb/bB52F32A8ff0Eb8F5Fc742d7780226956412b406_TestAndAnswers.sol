...
contract TestAndAnswers is ERC1155 {
    ...
    constructor() ERC1155 ("Hope Accelerated") {
        Teacher = msg.sender;
        questionsCount = 0;
    ...
    modifier onlyTeacher {
        require(msg.sender == Teacher, "For Teacher only.");
        _;
    }
    ...
    function addStudent(address _studentAddress) public onlyTeacher {
        students[_studentAddress] = Student(true);
    }

    function postQuestion(bytes32 _questionId, string memory _questionStr, string memory _publicKey) public onlyTeacher {
        Question storage question = questions[questionsCount];
            question.questionId = _questionId;
            question.questionStr = _questionStr;
            question.answersCount = 0;
            question.publicKey = _publicKey;
            question.revealed = false;
            question.privateKey = "";
            question.trueAnswer = "";
            question.pointsDistributed = false;
    ...
    function revealAnswer(bytes32 _questionId, uint _questionIndex, string memory _privateKey, string memory _trueAnswer) public onlyTeacher {
        Question storage question = questions[_questionIndex];
        require(question.questionId == _questionId, "Different question.");
        require(!question.revealed, "Answer already revealed.");
    ...
    /**
        decryptedAnswers: Teacher should decrypt answers from students offline, and submit them here
        (Because it's too expensive to be computed on-chain)
        @param answerJudgements bool[]: each student answered correctly or not
     */
    function distributePoints(bytes32 _questionId, uint _questionIndex, bool[] memory answerJudgements) public onlyTeacher {
        Question storage question = questions[_questionIndex];
