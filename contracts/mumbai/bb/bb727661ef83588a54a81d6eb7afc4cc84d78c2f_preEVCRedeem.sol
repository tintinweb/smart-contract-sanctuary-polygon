...
contract preEVCRedeem is Ownable, ReentrancyGuard {
    ...
    constructor(uint256 _startBlock, address _preevcAddress, address _evcAddress, address _redeemVested) {
    ...
    function swappreEVCForEVC() external nonReentrant {
        ...
        EVCToken.safeTransfer(RedeemVested, swapAmount);
        emit preEVCToEVC(RedeemVested, swapAmount);
    }
    ...
    function setVestingAddress(address _redeemVested) public onlyOwner {
        RedeemVested = _redeemVested;
    }
