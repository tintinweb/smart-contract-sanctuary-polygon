...
contract NFTicket is INFTicket, ERC721, AccessControl {
    using SafeERC20 for IERC20;

    mapping(uint64 => EventData) public events;

    mapping(uint256 => TicketPriceData) public ticketLevelsPrice;

    mapping(address => TokenData) public tokens;

    mapping(address => mapping(address => uint256)) public organisersBalance;

    mapping(uint256 => bool) public soldTickets;

    mapping(address => uint32) public organiserEventIdCounter;

    address public immutable organiserAccessControl;

    bytes32 public constant SET_FLAGS_ROLE = keccak256("SET_FLAGS_ROLE");
    ...
    // Check that the sender of the transaction (msg.sender) has SET_FLAGS_ROLE role
    modifier onlyAdminOrSetFlagsRole() {
        if (!this.isAdminRole(msg.sender) && !this.isSetFlagsRole(msg.sender)) {
            revert OnlyAdminOrSetFlagsAllowed();
        }
        _;
    }

    // Check if the organiser is approved
    modifier checkOrganiserApproved() {
        (IOrganiserAccessControl.Status status,) =
            IOrganiserAccessControl(organiserAccessControl).organisers(msg.sender);
        if (status != IOrganiserAccessControl.Status.APPROVED) {
            revert OrganiserNotInApprovedStatus();
        }
        _;
    }

    // Check if the event with parameter eventId doesn't exist
    modifier checkIfEventNotExist(uint64 eventId) {
        if (events[eventId].organiser != address(0)) {
            revert EventAlreadyExist();
        }
        _;
    }

    // Check if the event with parameter eventId doesn't exist
    modifier checkIfEventExist(uint64 eventId) {
        if (events[eventId].organiser == address(0)) {
            revert EventIsNotExist();
        }
        _;
    }

    modifier checkIfTicketExist(uint256 ticketId, bytes32[] calldata proof) {
        if (!this.isTicketExist(ticketId, proof)) {
            revert TicketDoesNotExist();
        }
        _;
    }

    modifier checkIfMsgSenderIsOrganiser(uint64 eventId) {
        if (events[eventId].organiser != msg.sender) {
            revert MsgSenderIsNotEventOrganiser();
        }
        _;
    }

    modifier checkIfTicketSellAlreadyStarted(uint64 eventId) {
        if (events[eventId].ticketSellStartTime <= block.timestamp) {
            revert TicketSellAlreadyStarted();
        }
        _;
    }

    constructor(address root, address _organiserAccessControl) ERC721("NFTicket", "NFTCKT") {
        organiserAccessControl = _organiserAccessControl;

        _setupRole(DEFAULT_ADMIN_ROLE, root);

        // SETUP role Hierarchy:
        // DEFAULT_ADMIN_ROLE > SET_FLAGS_ROLE
        _setRoleAdmin(SET_FLAGS_ROLE, DEFAULT_ADMIN_ROLE);
    }

    function addToken(address tokenAddress) external override onlyAdmin {
        if (tokens[tokenAddress].exist) {
            revert TokenAlreadyExist();
        }
        tokens[tokenAddress].exist = true;
        tokens[tokenAddress].isActive = true;
    }

    function changeTokenActiveStatus(address tokenAddress, bool isActive) external override onlyAdmin {
        if (!tokens[tokenAddress].exist) {
    ...
    ) external override checkOrganiserApproved checkIfEventNotExist(eventId) {
        if (ticketSellStartTime <= block.timestamp) {
            revert TicketSellStartMustBeInFuture();
        }
        if (ticketSellStartTime >= ticketSellEndTime) {
            revert TicketSellStartIsAfterTicketSellEnd();
        }
        if (tokens[tokenAddress].isActive == false) {
            revert TokenIsNotActive();
        }
        uint32 organiserIdFromEventId = this.getOrganiserIdFromTicketId(eventId);
        (, uint32 organiserId) = IOrganiserAccessControl(organiserAccessControl).organisers(msg.sender);
        if (organiserIdFromEventId != organiserId) {
            revert OrganiserIdIsNotCorrect();
        }
        uint32 organiserEventId = this.getOrganiserEventIdFromTicketId(eventId);
        if (organiserEventId != this.getNextOrganiserEventId(msg.sender)) {
            revert OrganiserEventIdIsNotCorrect();
        }

        events[eventId].merkleTreeRoot = merkleTreeRoot;
        events[eventId].organiser = msg.sender;
        events[eventId].ticketSellStartTime = ticketSellStartTime;
        events[eventId].ticketSellEndTime = ticketSellEndTime;
        events[eventId].tokenAddress = tokenAddress;
        events[eventId].allowToSetTicketSoldFlag = allowToSetTicketSoldFlag;
    }

    function updateMerkleTreeRoot(uint64 eventId, bytes32 merkleTreeRoot)
        external
        override
        checkIfEventExist(eventId)
        checkIfMsgSenderIsOrganiser(eventId)
        checkIfTicketSellAlreadyStarted(eventId)
    {
        events[eventId].merkleTreeRoot = merkleTreeRoot;
    }

    function updateTicketSellTimes(uint64 eventId, uint256 ticketSellStartTime, uint256 ticketSellEndTime)
        external
        override
        checkIfEventExist(eventId)
        checkIfMsgSenderIsOrganiser(eventId)
    ...
    function updateTokenAddress(uint64 eventId, address tokenAddress)
        external
        override
        checkIfEventExist(eventId)
        checkIfMsgSenderIsOrganiser(eventId)
    ...
    function setBlockTicketSellOnEvent(uint64 eventId, bool blockTicketSell)
        external
        override
        onlyAdminOrSetFlagsRole
        checkIfEventExist(eventId)
    {
        if (events[eventId].ticketSellEndTime < block.timestamp) {
            revert TicketSellIsFinished();
        }
        events[eventId].blockTicketSell = blockTicketSell;
    }

    function setTicketSoldFlag(uint256 ticketId, bytes32[] calldata proof, bool ticketSoldFlag)
        external
        override
        onlyAdminOrSetFlagsRole
        checkIfTicketExist(ticketId, proof)
    {
        if (_exists(ticketId)) {
            revert TicketAlreadyMinted();
        }
        EventData memory eventData = events[this.getEventIdFromTicketId(ticketId)];
        if (!eventData.allowToSetTicketSoldFlag) {
            revert SetTicketSoldFlagNotAllowed();
        }
        if (eventData.ticketSellEndTime < block.timestamp) {
            revert TicketSellIsFinished();
        }

        soldTickets[ticketId] = ticketSoldFlag;
    }

    function setupTicketPrice(uint256 ticketLevel, uint256 price) external override {
        uint64 eventId = this.getEventIdFromTicketId(ticketLevel);
        if (events[eventId].organiser != msg.sender) {
            if (events[eventId].organiser == address(0)) {
                revert EventIsNotExist();
            }
            revert OnlyEventOrganiserAllowed();
        }
        ...
        ticketLevelsPrice[ticketLevel].price = price;
        ticketLevelsPrice[ticketLevel].isFree = isFree;
    }

    function mint(uint256 ticketId, bytes32[] calldata proof) external override checkIfTicketExist(ticketId, proof) {
        if (soldTickets[ticketId]) {
            revert TicketIsSetAsSold();
        }
        EventData memory eventData = events[this.getEventIdFromTicketId(ticketId)];
        if (eventData.ticketSellStartTime > block.timestamp) {
            revert TicketSellIsNotStart();
        }
        if (eventData.ticketSellEndTime < block.timestamp) {
            revert TicketSellIsFinished();
        }
        if (eventData.blockTicketSell) {
            revert TicketSellIsBlocked();
        }
        TicketPriceData memory ticketPriceData = this.getTicketPrice(ticketId);
        if (ticketPriceData.price == 0 && ticketPriceData.isFree == false) {
            revert TicketPriceIsNotSet();
        }

        if (ticketPriceData.price > 0) {
            organisersBalance[eventData.organiser][eventData.tokenAddress] =
                organisersBalance[eventData.organiser][eventData.tokenAddress] + ticketPriceData.price;
            IERC20(eventData.tokenAddress).safeTransferFrom(msg.sender, address(this), ticketPriceData.price);
        }
        _mint(msg.sender, ticketId);
    }

    function withdraw(address tokenAddress, uint256 amount) external override checkOrganiserApproved {
        if (!tokens[tokenAddress].exist) {
            revert TokenDoesNotExist();
        }
        if (amount > organisersBalance[msg.sender][tokenAddress]) {
            revert NoEnoughRequestedAmountForThisToken();
        }
        organisersBalance[msg.sender][tokenAddress] = organisersBalance[msg.sender][tokenAddress] - amount;
        IERC20(tokenAddress).safeTransfer(msg.sender, amount);
    }

    function getNextOrganiserEventId(address account) external view override returns (uint32) {
        return organiserEventIdCounter[account] + 1;
    }
    ...
    function isSetFlagsRole(address account) external view override returns (bool) {
        return hasRole(SET_FLAGS_ROLE, account);
    }

    function isTicketExist(uint256 ticketId, bytes32[] calldata proof) external view override returns (bool) {
        uint64 eventId = this.getEventIdFromTicketId(ticketId);
        bytes32 root = events[eventId].merkleTreeRoot;
        string memory ticketIdString = Strings.toString(ticketId);
        bytes32 leaf = keccak256(abi.encodePacked(ticketIdString));
        return MerkleProof.verify(proof, root, leaf);
    }

    function getTicketPrice(uint256 ticketId) external view override returns (TicketPriceData memory) {
        uint256 mask = type(uint256).max;
        uint8 counter = 0;
        while (counter < 7) {
            uint256 currentTicketLevel = ticketId & mask;
            mask = mask >> 32;
            if (
                ticketLevelsPrice[currentTicketLevel].price > 0
                    || (ticketLevelsPrice[currentTicketLevel].price == 0 && ticketLevelsPrice[currentTicketLevel].isFree)
            ) {
                return ticketLevelsPrice[currentTicketLevel];
            }
            counter++;
        }
        return TicketPriceData(0, false);
    }
    ...
    function encodeTicketID(uint32[] calldata levels) external pure override returns (uint256) {
        if (levels.length > 8) {
            revert MoreThanEightLevels();
        }

        uint256 tickedId = 0;
        for (uint8 counter = 0; counter < levels.length; counter++) {
            uint256 currentLevel = levels[counter];
            uint256 mask = currentLevel << 32 * counter;
            tickedId = tickedId | mask;
        }

        return tickedId;
    }

    function decodeTicketID(uint256 ticketId) external pure override returns (uint32[] memory) {
        uint32[] memory levels = new uint32[](8);
        uint256 mask = 0xffffffff;
        uint8 counter = 0;
        while (counter < 8) {
            uint256 currentLevel = ticketId & mask;
            currentLevel = currentLevel >> 32 * counter;
            levels[counter] = uint32(currentLevel);
    ...
    function getOrganiserIdFromTicketId(uint256 ticketId) external pure override returns (uint32) {
        uint256 mask = 0xffffffff;
        return uint32(ticketId & mask);
    }

    function getOrganiserEventIdFromTicketId(uint256 ticketId) external pure override returns (uint32) {
        uint64 mask = 0xffffffff << 32;
        return uint32((ticketId & mask) >> 32);
    }

    function getEventIdFromTicketId(uint256 ticketId) external pure override returns (uint64) {
        uint256 mask = 0xffffffffffffffff;
        return uint64(ticketId & mask);
    }
}
