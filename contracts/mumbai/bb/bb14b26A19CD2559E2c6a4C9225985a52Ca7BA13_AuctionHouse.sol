...
contract AuctionHouse is ERC1155Receiver, ERC2771Context, AccessControlUpgradeable, Ownable {
  ...
  constructor(IERC1155 auctionableNft, IERC20 currency_, IERC20 stableCurrency_, address trustedForwarder) ERC2771Context(trustedForwarder) {
    ...
    _grantRole(AUCTION_CREATOR_ROLE, _msgSender());
 
  }
   function _msgSender() internal view override(ERC2771Context, Context, ContextUpgradeable)
      returns (address sender) {
      sender = ERC2771Context._msgSender();
  }

  function _msgData() internal view override(ERC2771Context, Context, ContextUpgradeable)
  ...
  function createAuction(
    ...
    uint40 timeIncrement,
    uint256 stableExchangeRate
  ) external onlyRole(AUCTION_CREATOR_ROLE)
  returns (uint256)  {
  ...
  function supportsInterface(bytes4 interfaceId) public view virtual override(AccessControlUpgradeable, ERC1155Receiver) returns (bool) {
