	...
	Withdrawable
{
	...
	uint256 public constant PUBLIC_SALES_START_PRICE = 1.96 ether;
	...
	uint256 public constant PRICE_DROP_FLOOR = 0.01 ether;

	// whitelist purchase limit
	// uint256 public constant PRESALE_MAX_PURCHASE_ALLOWED = 20;
	uint256 public constant PRESALE_MAX_PURCHASE_ALLOWED = 2;
	...
	function preSaleMint(bytes32[] calldata _proof, uint256 numberOfTokens)
		public
		payable
		isPreSalesActive
	{
		...
		require(msg.value >= getPrice() * numberOfTokens, "MATIC value sent is not correct.");
		require(balanceOf(msg.sender) + numberOfTokens <= PRESALE_MAX_PURCHASE_ALLOWED, "Exceed maximum allowance.");
	...
	function publicSaleMint(uint256 numberOfTokens)
		...
		isPublicSalesActive
	{
		require(numberOfTokens <= getAvailableBirds(), "Purchase would exceed max supply.");
		require(numberOfTokens <= MAX_PURCHASE, "Can only mint 20 tokens at a time");
		require(msg.value >= getPrice() * numberOfTokens, "MATIC value sent is not correct");
