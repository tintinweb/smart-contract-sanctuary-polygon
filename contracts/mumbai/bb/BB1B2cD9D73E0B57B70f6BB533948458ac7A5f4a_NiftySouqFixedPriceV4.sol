...
contract NiftySouqFixedPriceV4 is Initializable {
    ...
    //Purchase
    function buyNft(uint256 offerId_, uint256 quantity_, address buyer_) public payable {
        NiftySouqIMarketplace.Offer memory offer = NiftySouqIMarketplace(
            _marketplace
        ).getOfferStatus(offerId_);
        require(
        ...
        Payout memory payoutData = _buyNft(
            BuyNFT(offerId_, buyer_, quantity_, msg.value)
        );
        ...
        if (payoutData.soldout) {
            ...
            emit ePurchase(offerId_, buyer_, address(0), quantity_, true);
        } else {
            emit ePurchase(offerId_, buyer_, address(0), quantity_, false);
        }
    }
