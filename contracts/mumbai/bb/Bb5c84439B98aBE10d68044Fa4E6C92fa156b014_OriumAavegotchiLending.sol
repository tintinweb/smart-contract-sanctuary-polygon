...
import { ILendingGetterAndSetterFacet, GotchiLending } from "./interfaces/ILendingGetterAndSetterFacet.sol";
...
contract OriumAavegotchiLending is OwnableUpgradeable {
    ...
    function isLendingClaimable(uint32 tokenId) private view returns (bool) {
        GotchiLending memory lending = ILendingGetterAndSetterFacet(_aavegotchiDiamondAddress).getGotchiLendingFromToken(tokenId);
    ...
    function getLendingAction(address owner, uint32 tokenId) private view returns (NftLendingAction memory) {
        ...
        if (lendingGetterAndSetterFacet.isLendingOperator(owner, address(this), tokenId) == false) {
            return NftLendingAction(tokenId, LendingAction.REMOVE);
        }
        bool isListed = lendingGetterAndSetterFacet.isAavegotchiListed(tokenId);
        if (isListed == false) {
            return NftLendingAction(tokenId, LendingAction.LIST);
        } else {
            if (lendingGetterAndSetterFacet.isAavegotchiLent(tokenId) == true && isLendingClaimable(tokenId) == true) {
    ...
    // @param limit If true, limits the amount of actions to _maxActions
    // @return List of pending actions
    function getPendingActions(bool limit) external view returns (uint32[] memory listNfts, uint32[] memory claimAndListNfts, uint32[] memory removeNfts) {
        uint256 listCounter;
        uint256 claimAndListCounter;
        uint256 removeCounter;
        uint256[] memory tokenIds = EnumerableSet.values(_tokenIds);
        uint256 actions_limit = limit == true ? _maxActions : tokenIds.length;
        ...
        for (uint256 i; i < tokenIds.length && (listCounter + claimAndListCounter + removeCounter) < actions_limit; i++) {
            ...
            AddGotchiListing memory listing = _listingsParameters[tokenId];
    ...
    function batchSchedule(
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            uint32 tokenId = tokenIds[i];
            require(IERC721(_aavegotchiDiamondAddress).ownerOf(tokenId) == msg.sender, "Sender must be the owner of the NFT");
