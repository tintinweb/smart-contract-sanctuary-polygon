...
contract Raids is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function getBossFights(uint256 bossId) external view returns (Fight[] memory) {
        uint256[] memory fighters = bossUniqueBeasts[bossId];
        Fight[] memory bossFights = new Fight[](fighters.length);
        for (uint256 index = 0; index < fighters.length; index++) {
            uint256 fightId = beastBestFights[fighters[index]][bossId];
            bossFights[index] = fights[fightId];
        }
        return bossFights;
    }

    function getAllBossFights(uint256 bossId) external view returns (Fight[] memory) {
        Boss memory boss = bosses[bossId];
        if (boss.firstFightId == 0) return new Fight[](0);
        uint256 totalFights = boss.lastFightId - boss.firstFightId + 1;
        Fight[] memory bossFights = new Fight[](totalFights);
        for (uint256 index = 0; index < totalFights; index++) {
            bossFights[index] = fights[boss.firstFightId + index];
        }
        return bossFights;
    }

    function getWalletFights(address wallet, uint256 bossId) public view returns (Fight[] memory) {
        uint256[] memory fightIds = _walletFightsByBoss[wallet][bossId];
        Fight[] memory walletFights = new Fight[](fightIds.length);
        for (uint256 index = 0; index < fightIds.length; index++) {
            walletFights[index] = fights[fightIds[index]];
        }
        return walletFights;
    }
    ...
    function _getFightRounds(uint256 fightId) internal view returns (Round[] memory, uint8) {
        ...
        (
            ...
            uint256 beastDodge,
            uint256 beastCrit
        ) = _getStats(_fight.beastRepresentation, _fight.bossId);

        Round[] memory tempRounds = new Round[](50);
        uint8 roundIndex;
        tempRounds[roundIndex++] = Round(uint32(beastHp), uint32(bossHp));

        while (beastHp > 0 && bossHp > 0 && roundIndex < 50) {
            beastHp = _doDamage(bossDamage, beastHp, beastDodge, 0, noncedRandom++);
            bossHp = _doDamage(beastDamage, bossHp, 0, beastCrit, noncedRandom++);
            tempRounds[roundIndex++] = Round(uint32(beastHp), uint32(bossHp));
        }
        ...
        )
    {
        ...
        bossHp = boss.hp;
        beastHp = 100 + (beast.stats.con * 10) + beast.level;
        ...
        beastDamage = (baseDamageModifier) * 5 + beast.level / 2;
        beastDamage = (beastDamage * bossDefense) / 100;
        beastDodge = beast.stats.dex + (beast.level / 9);
        beastCrit = beast.stats.dex + beast.stats.wis;
        ...
        bossDamage = bossPDamage > bossMDamage ? bossPDamage : bossMDamage;
    }
