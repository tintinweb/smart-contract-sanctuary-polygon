...
contract USDE is IERC20, IUSDE, IOWNER {
    ...
    function connect(address token) public virtual override returns (bool) {
        bool notThis_ = token != _contract && token.isContract();
        if (notThis_ && _isWZero(contractOf()) && _isAdmin()) {
            BIT = IBIT(token);
            emit Connect(token);
            return true;
        }
    ...
    function burnBy(uint256 usdeWei) external virtual override returns (bool) {
        if (_isBlack(_wallet()) || !_isConnect()) {
            return false;
        }
    ...
    function stakeBy(
        ...
        uint256 stakeMonth_ = _stakeMonth(_wallet());
        bool isPool = _wallet() != pool && _wallet() != _poolOf(_wallet());
        bool isMonth_ = month >= stakeMonth_ && _checkMonth(_wallet(), month);
        // can stake on connect and this contract;
        bool canStake_ = _isConnect() && isMonth_ && isPool;
        bool isWeiStake_ = canStake_ && _balanceOf(_wallet()) >= usdeWei;
        if (isWeiStake_) {
            return false;
        }
        return _stake(pool, _wallet(), usdeWei, month);
    }

    function unStakeBy() public virtual override returns (bool) {
        uint256 usdeStaking = _staking(_wallet());
        bool isTime = block.timestamp > _unStakeAt(_wallet());

        if (usdeStaking > 0 && isTime && _isConnect()) {
            return _unStake(_wallet());
        }
        return false;
    }
    ...
    function donate(
        ...
        if (_stakingOf[to] == 0 && _poolOf(to) == address(0)) {
            _transfer(_sender(), to, usdeWei);
            _stake(_sender(), to, usdeWei, month);
            return true;
        }
        ...
        returns (bool)
    {
        ...
        return _swapFrom(token, tokenWei);
    }
        ...
        returns (bool)
    {
        ...
        uint256 _totalOut_;
        uint256 fee_tx = _fee_tx(_totalOut_);
        ...
        returns (bool)
    {
        ...
        uint256 fee_tx = _fee_tx(usdeWei);
        if (_hasWei(_sender(), usdeWei + fee_tx)) {
            _transfer(_sender(), to, usdeWei);
            return fee_tx > 0 ? _burn(_sender(), fee_tx) : true;
        }
        return false;
    }
    ...
    function ofStake(address wallet)
        external
        view
        virtual
        override
    ...
    function isBlack(address wallet)
        external
        view
        virtual
        override
    ...
    function poolOf(address wallet)
        external
        view
        virtual
        override
    ...
    function isWallet(address wallet)
        external
        pure
        virtual
        override
    ...
    function _incTotalExchange(uint256 usdeWei) internal virtual {
        _totalExchange += usdeWei;
    }

    function _incTotalSupply(uint256 usdeWei) internal virtual {
        _totalBalance += usdeWei;
    }

    function _decTotalSupply(uint256 usdeWei) internal virtual {
        _totalBalance -= usdeWei;
    }

    function _incStake(address staker_, uint256 usdeWei) internal virtual {
        _stakingOf[staker_] += usdeWei;
        _balances[staker_] -= usdeWei;

        _totalStaking += usdeWei;
        _totalExchange += usdeWei;

        _emitStake(staker_, usdeWei);
    }

    function _decStake(address staker_, uint256 usdeWei) internal virtual {
        _stakingOf[staker_] -= usdeWei;
        _balances[staker_] += usdeWei;
    ...
    function _emitStake(address staker_, uint256 usdeWei) internal virtual {
        emit Staking(staker_, usdeWei, _unStakeAt(staker_));
    }

    function _incLock(address locker_, uint256 usdeWei) internal virtual {
        _locksOf[locker_] += usdeWei;
        _totalLocking += usdeWei;
        _burn(locker_, usdeWei);
        _emitLock(locker_, usdeWei);
    }

    function _decLock(address locker_, uint256 usdeWei) internal virtual {
        _locksOf[locker_] -= usdeWei;
        _totalLocking -= usdeWei;
        _mint(locker_, usdeWei);
    ...
    function _emitLock(address from, uint256 usdeWei) internal virtual {
        emit Locking(from, usdeWei, _unLockAt(_sender()));
    }
    ...
    function _incBalance(address wallet, uint256 usdeWei) internal virtual {
        _balances[wallet] += usdeWei;
    }

    function _decBalance(address wallet, uint256 usdeWei) internal virtual {
        _balances[wallet] -= usdeWei;
    }
    ...
    function _burn(address from, uint256 usdeWei)
        internal
        virtual
        returns (bool)
    {
    ...
    function _mint(address to, uint256 usdeWei)
        internal
        virtual
        returns (bool)
    {
        ...
        _emitTransfer(address(0), to, usdeWei);
        return true;
    }
    ...
    function _approve(
        ...
        require(owner_ != address(0), "USDE: approve from the zero address");
        require(spender_ != address(0), "USDE: approve to the zero address");
    ...
    function _swapFrom(
        ...
        // Before if
        // _token.approve(_contract, tokenWei); // if approve before transaction;

        uint256 usdeWei = _toUsde(_SYMBOL(token), tokenWei, _decimalOf(token));
        bool isTokenWei = _isTokenList(token) && usdeWei > 0 && tokenWei > 0;

        if (isTokenWei) {
            require(_token.transferFrom(fr_, ctr_, tokenWei) == true, "Error");
            return _mint(fr_, usdeWei);
        }

        return false;
    }

    function _swap(address toToken, uint256 usdeWei)
        internal
        virtual
        returns (bool)
    {
        ...
        if (canTrans) {
            bool sended_ = _token.transfer(_sender(), tokenWei);
            require(sended_ == true, "SellToken");
            return _burn(_sender(), usdeWei_tx);
        }
        return false;
    }
    ...
    function _fee_tx(uint256 usdeWei) internal view returns (uint256) {
        uint256 mulOf_ = usdeWei * BIT.oracle(_FEE_TX, "USD");
    ...
    function _stake(
        ...
        if (month_ >= _stakeMonth(staker_)) {
            _stakingMonth[staker_] = month_;
            uint256 unAt_ = _stakeMonth(staker_) * 30 * 24 * 3600;
            _unStakingAt[staker_] = block.timestamp + unAt_;
        }
        _stakingAt[staker_] = block.timestamp;
        if (_isWZero(_poolOf(staker_)) && usdeWei > 0) {
            _stakingUp[staker_] = pool_;
        }
        if (_staking(staker_) == 0) {
            _countStaking += 1;
        }

        _incStake(staker_, usdeWei);
        return true;
    }

    function _unStake(address staker_) internal virtual returns (bool) {
        _decStake(staker_, _staking(staker_));
        _countStaking -= 1;
        return true;
    }
    ...
    function _toUsde(
        ...
        return SafeMath.div(mulOf_, _10Pow(dcm_));
        // => weis of Bit in decimal;
    }

    function _totalSupply() internal view virtual returns (uint256) {
        return _totalBalance + _totalStaking + _totalLocking;
    }
    ...
    function _isConnect() internal view virtual returns (bool) {
        return _sender() == address(BIT) && _contract == BIT.contractOf();
    }
    ...
    function _isWallet(address wallet) internal pure returns (bool) {
        return wallet == address(wallet) && wallet != address(0);
    }
    ...
    function _forStart() internal virtual {
        _mint(_sender(), 10000 * _10Pow(_decimal));
        _mint(_contract, 10000 * _10Pow(_decimal));
    }
