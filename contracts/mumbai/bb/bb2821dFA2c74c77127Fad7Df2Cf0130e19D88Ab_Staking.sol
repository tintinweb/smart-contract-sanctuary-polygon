...
contract Staking is Initializable , Ownable{
    ...
    function intialize(address _owner, address _token) public initializer {
        token = IERC20(_token);
     ...
     function unstake(uint amount) public {
        require(amount > 0, 'Staking: amount should not be less than or equal to 0');
        require(amount <= stake[msg.sender], 'Staking: Greater than staked amount');
        previousReward[msg.sender] = previousReward[msg.sender] + (stake[msg.sender] * (S - s1[msg.sender]))/ 10 ** 18;
        stake[msg.sender] -= amount;
        totalstakeToken -= amount;
        token.transfer(msg.sender,amount);
     ...
     function rewardIsuue(uint256 Ramount) public onlyOwner returns(uint256){
        token.transferFrom(msg.sender, address(this),Ramount);
