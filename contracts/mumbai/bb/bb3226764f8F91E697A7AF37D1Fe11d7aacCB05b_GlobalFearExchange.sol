...
/**
 * @dev Contract implementing Global Fear Exchange.
 */
contract GlobalFearExchange is AccessControl, Pausable, Withdrawable, ContextMixin, NativeMetaTransaction {

    ITradeContract[] public tradeContracts;
    mapping (string => ITradeContract) public tradeContractsByName;
    ...
    function trade(string memory contractName, string memory positionName, uint256 amount) external whenNotPaused {
        ITradeContract tradeContract = tradeContractsByName[contractName];
        require(address(tradeContract) != address(0), "Trade contract doesn't exist");
        tradeContract.trade(_msgSender(), positionName, amount);
    }

    function addTradeContract(address _address) external onlyRole(MANAGER_ROLE) {
        ITradeContract newContract = ITradeContract(_address);
        tradeContracts.push(newContract);
        tradeContractsByName[newContract.name()] = newContract;
    }
    
    function removeTradeContract(uint256 index) public onlyRole(MANAGER_ROLE) {
        string memory name = tradeContracts[index].name();
        tradeContracts[index] = tradeContracts[tradeContracts.length - 1];
        tradeContracts.pop();

        delete tradeContractsByName[name];
    }

    function removeTradeContractByName(string memory name) external onlyRole(MANAGER_ROLE) {
        bytes32 hashedName = keccak256(bytes(name));

        for (uint i = 0; i < tradeContracts.length; i++) {
            if (keccak256(bytes(tradeContracts[i].name())) == hashedName) {
                removeTradeContract(i);
                break;
            }
        }
    }
...
interface ITradeContract {
    ...
    function trade(address buyer, string memory positionName, uint256 amount) external;
}
