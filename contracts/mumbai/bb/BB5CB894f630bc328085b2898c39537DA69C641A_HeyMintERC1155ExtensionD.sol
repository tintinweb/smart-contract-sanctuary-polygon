...
* @notice Returns the public price in wei. Public price is stored with 5 decimals (1 = 0.00001 ETH), so total 5 + 13 == 18 decimals
    ...
    */
    ...
    function creditCardMint(
        ...
        HeyMintStorage.State storage state = HeyMintStorage.state();
        address[5] memory defaultAddresses = getDefaultCreditCardMintAddresses();
        ...
        require(
            state.data.totalSupply[_tokenId] + _numTokens <= state.tokens[_tokenId].maxSupply,
            "MAX_SUPPLY_EXCEEDED"
        );
        ...
        if (state.data.totalSupply[_tokenId] >= state.tokens[_tokenId].maxSupply) {
