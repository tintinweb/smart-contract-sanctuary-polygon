...
// File contracts/PoFInternalValidator.sol


pragma solidity ^0.8.12;
/**
@title PoF Regional regulator interface 
*/
interface IPoFRegionalRegulator {
    function reward(
        address payable,
        address payable,
        address payable,
...
/**
@title PoF Internal Validator 
@author Javier Ortin
@notice Proof of Feedback validator registering the users' feedback then
        randomly drawing the jurors assigned for evaluating the feedback 
        and sending the final decision to the regulator node
*/
contract PoFInternalValidator is Ownable {
    ...
    @dev evaluated how many jurors have already evaluated
    */
    struct Feedback {
        address customer;
        address juror1;
    ...
    /// @dev map for storing feedbacks' information
    mapping(address => Feedback) public feedbacks;

    /**
    @dev minimum amount of jurors that the internal validator must have
        to have a fair random selection of jurors and start operating
     */
    uint256 public minJurors = 7;

    /// @notice PoF Regional Regulator to which this validator connects
    ...
    /**
     * @dev Reverts if called by any account that is not the PoF regulator.
     */
    ...
    modifier onlyJuror() {
        require(
            isJuror[_msgSender()],
            "Juror: caller is not a PoF Internal validator juror"
        );
        _;
    }
    /// Functions
    ...
    */
    ...
    /**
    @notice Function to registerthe feedback in the blockchain
    ...
    */
    ...
    /**
    @notice Function that only jurors can call to evaluate the feedback they have been assigned
    @param feedbackID id of the given feedback in the form of a valid address. 
    @param originality evaluation points for this criteria
    @param usefulness evaluation points for this criteria
    @param execution evaluation points for this criteria
    */
    ...
    /**
    @notice Calculation of the final evaluation. Arithmetic mean.
    @param feedbackID id of the given feedback in the form of a valid address. 
    ...
    */
    function finalEvaluation(address feedbackID) private {
        uint256 evaluation = (
            ...
            3;
        feedbacks[feedbackID].finalEvaluation = evaluation;
        IPoFRegionalRegulator(pofRegulator).reward(
            payable(feedbacks[feedbackID].customer),
            payable(feedbacks[feedbackID].juror1),
            payable(feedbacks[feedbackID].juror2),
            payable(feedbacks[feedbackID].juror3),
            feedbackID,
            evaluation
        );
    }
    ...
    /**
    @notice Algorithm for randomly selecting the jurors.
    @param feedbackID id of the given feedback used as seed.
    @return address 3 addresses of the jurors selected.
