    ...
    function claimtoken(address _customerAddress, uint256 planno) onlybelievers () public returns(bool){
		 ...
		 uint256 finish = deposits[_customerAddress][planno].start.add(deposits[_customerAddress][planno].time).mul(1 days);
		  if (block.timestamp > finish) {
            uint256 tamount = deposits[_customerAddress][planno].tokenamount;
			tokenBalanceLedger_[_customerAddress] = SafeMath.add(tokenBalanceLedger_[_customerAddress], tamount );
			stakeBalanceLedger_[_customerAddress] = SafeMath.sub(stakeBalanceLedger_[_customerAddress], tamount );
