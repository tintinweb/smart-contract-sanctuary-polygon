...
contract Polyverse {
    ...
    function addCreator(string memory _name, string memory _imagepath, string memory _category, string memory _bio, uint _subscriptionFee) public {
        _creatorIds.increment();
        uint256 creatorId = _creatorIds.current();
        Creator storage newCreator = _creators[creatorId];
        address[] memory emptyAddressArray;
        newCreator.name = _name;
        newCreator.imageIpfs = _imagepath;
        newCreator.category = _category;
        newCreator.bio = _bio;
        newCreator.subscriptionFee = _subscriptionFee;
        newCreator.subscribers = emptyAddressArray;
        newCreator.balance = 0;
        newCreator.owner = msg.sender;
    ...
    function listEvent(
        ...
        address nftContract = address(new PolyverseEventNFT(_name));
        _eventIds.increment();

        Event storage newEvent = _events[_eventIds.current()];
        newEvent.eventNFT = nftContract;
        newEvent.id = _eventIds.current();
        newEvent.cost = _cost;
        newEvent.date = _date;
        newEvent.name = _name;
        newEvent.time = _time;
        newEvent.location = _location;
        newEvent.maxTickets = _maxTickets;
        newEvent.tickets = _maxTickets;
        newEvent.owner = msg.sender;
        allEvents.push(newEvent);
        emit EventCreated(_eventIds.current(), msg.sender, _maxTickets, _cost);
    }

    function purchaseTicket(uint256 eventId) public payable {
        Event storage eventDetails = _events[eventId];
        require(msg.value >= eventDetails.cost, "Not enough amount");
        require(eventDetails.participants.length < eventDetails.maxTickets, "Full event");
        //require(block.timestamp  eventDetails.date, "Event over");
        IERC20(tokenAddress).approve(msg.sender, eventDetails.cost);
