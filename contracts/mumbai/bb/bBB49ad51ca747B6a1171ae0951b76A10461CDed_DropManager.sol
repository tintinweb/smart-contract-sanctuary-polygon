...
contract DropManager is  IDropManager, AccessControl {

    bytes32 public constant OPERATOR_ROLE = keccak256("OPERATOR_ROLE");

    address public NFTContract;
    uint256 public reservationTime = 900; //15 min
    ...
    function nftPaySell(uint256 dropId, uint256 nftId) external onlyRole(OPERATOR_ROLE) returns(bool){
        ...
        if (_unreserveNft(dropId, nftId)){
            Reservation memory reserv = _getReservationsById(dropId, nftId);
            Struct.NFTDetails memory nftDetails = INFT(NFTContract).getNftDataById(nftId);
            ITransferManager.TransferDto memory transferDto = ITransferManager.TransferDto(nftDetails.winery, reserv.holder, nftId, address(0));
            transfer_res = transferManager.transferERC721From(transferDto);
            detach_res = _detachNftFromDropEventById(dropId, nftId);
            drop.nftOwnerCap[reserv.holder] += 1;
        }
    ...
    function getAvailableNft(uint256 dropId) external view returns(uint availableNft){
        return Drops[dropId].nftIdList.length;
    }

    function getNftIds(uint256 dropId) external view dropExists(dropId) returns(uint256[] memory nftIds){
        uint256[] memory nftIds = Drops[dropId].nftIdList;
        return nftIds;
    }

    function getReservations(uint256 dropId) external view dropExists(dropId) returns(Reservation[] memory _reservedNfts){
        Drop storage drop = Drops[dropId];
        Reservation[] memory _reservedNfts = drop.reservedNftId;
        return _reservedNfts;
    }
    ...
    function _checkEndedReservations(uint256 dropId) internal returns(bool){
        Drop storage drop = Drops[dropId];
        if (drop.reservedNftId.length == 0){
            return false;
        }
        for(uint256 i = 0; i < drop.reservedNftId.length; i++){
            if (block.timestamp >= drop.reservedNftId[i].reservationDeadline){
                _unreserveNftByIndex(dropId, i);
            }
        }
        return true;
    ...
    function _getReservationsById(uint256 dropId, uint256 nftId) internal returns(Reservation memory reservation){
        Drop storage drop = Drops[dropId];
...
interface IDropManager {
    ...
    function getAvailableNft(uint dropId) external view returns(uint availableNft);

    function getDropData(uint dropId) external view returns(uint256 _releaseStartDate,
                                                            uint256 _releaseEndDate,
                                                            uint256[] memory _nftIdList,
                                                            Reservation[] memory _reservedNfts,
                                                            uint256 _nftAmountAllowedToBuy);
