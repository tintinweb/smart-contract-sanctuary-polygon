...
contract ExitFacet is IOperations, ReentrancyGuard {
    /* ------ EVENTS ------ */
    /// @notice
    event ExitedExpiredDeposit(uint256 requestId);

    /// @notice
    event ExitAsset(address defaultReceiver, uint8 listingId, uint88[] _tokenIds, uint128[] _amounts);

    /* ------ EXTERNAL FUNTIONS ------ */

    /**
     * @notice Exit expired pending deposit record. Returns assets to depositor.
     * @param _requestId Request ID of the expired deposit record.
     * @param _listingId Listing ID of the expired deposit record.
     * @param _tokenIds Token IDs of the expired deposit record.
     * @param _amounts Amounts of the expired deposit record.
     */
    function exitExpiredDeposit(
        uint256 _requestId,
        uint8 _listingId,
        uint88[] calldata _tokenIds,
        uint88[] calldata _amounts
    ) external nonReentrant {
        LibStatusControl._requireExitEnabled();
        require(_tokenIds.length == _amounts.length, "ExitFacet: tokenIds and amounts length mismatch");
        require(
            ...
            "ExitFacet: Pending deposit record not expired"
        );

        uint256 dataLength = _tokenIds.length;
        if (dataLength == 1) {
            uint88 tokenId_ = _tokenIds[0];
            uint88 amount_ = _amounts[0];
            ...
            require(
                s.pendingDepositRecord[_requestId].hashedRequestData ==
                    keccak256(abi.encode(s.domainSeparator, LibOperations._hashDepositRequest(deposit))),
                "ExitFacet: Invalid hash data"
            );
        } else if (dataLength > 1 && dataLength < 11) {
            DepositBatch memory depositBatch = DepositBatch({
                from: msg.sender,
                listingId: _listingId,
                arrayLength: LibSafeCast.toUint8(dataLength),
                tokenIds: _tokenIds,
                amounts: _amounts
            });
            require(
                s.pendingDepositRecord[_requestId].hashedRequestData ==
                    keccak256(abi.encode(s.domainSeparator, LibOperations._hashDepositBatchRequest(depositBatch))),
                "ExitFacet: Invalid hash data"
            );
        } else {
            revert("ExitFacet: Invalid data length");
        }

        // delete pending deposit record
        delete s.pendingDepositRecord[_requestId];
        uint88[] memory tokenIds = new uint88[](dataLength);
        uint88[] memory amounts = new uint88[](dataLength);

        // transfer assets to depositor
        for (uint256 i = 0; i < dataLength; i++) {
            tokenIds[i] = _tokenIds[i];
            amounts[i] = _amounts[i];
            LibWithdrawBalance._releaseAsset(msg.sender, _listingId, _tokenIds[i], _amounts[i], false);
        }
        emit ExitedExpiredDeposit(_requestId);
    }
    ...
    function exitAsset(
        ...
        LibStatusControl._requireExitEnabled();
        require(_tokenIds.length == _amounts.length, "ExitFacet: tokenIds and amounts length mismatch");
        require(_tokenIds.length > 0 && _tokenIds.length < 21, "ExitFacet: Invalid data length");
        require(s.tokenTypes[_listingId] != 0, "ExitFacet: Invalid token type");

        require(
            LibOperations._verifyRoot(
                s.latestCommittedBatch.stateRoot,
                _defaultReceiver,
                _listingId,
                _tokenIds,
                _amounts,
        ...
        require(!s.isExited[exitAssetKey], "Exit:exited");

        s.isExited[exitAssetKey] = true;
        uint88[] memory tokenIds = new uint88[](_tokenIds.length);
        uint128[] memory amounts = new uint128[](_tokenIds.length);
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            tokenIds[i] = _tokenIds[i];
            amounts[i] = _amounts[i];
            LibWithdrawBalance._releaseAsset(_defaultReceiver, _listingId, tokenIds[i], amounts[i], false);
        }
        emit ExitAsset(_defaultReceiver, _listingId, _tokenIds, _amounts);
    }
}
...
library LibOperations {
    ...
    {
        // request length must be 44 bytes
    ...
    {
        require(_data.length > 66 && (_data.length - 23) % 11 == 0, "Operations: Invalid data length");
        uint256 offset = _offset; //offset start from 1
        ...
        require(depositBatch.arrayLength > 1 && depositBatch.arrayLength < 11, "Operations: Invalid batch length");
        ...
        returns (IOperations.Withdraw1155C memory withdraw1155C)
    {
        ...
        require(withdraw1155C.arrayLength > 0 && withdraw1155C.arrayLength < 11, "Operations: Invalid batch length");
    ...
    function _addPendingRequest(bytes32 _hashedRequestData) internal returns (uint256) {
        AppStorage storage s = LibAppStorage.diamondStorage();

        uint256 expirationTimestamp = block.timestamp + REQUEST_EXPIRATION_PERIOD;
        uint256 newRequestId = s.currentPendingRequestId + s.numberOfPendingRequest;
...
library LibStatusControl {
    ...
    function _requireWithdrawEnabled() internal view {
        ...
        require(
            s.status == 1 ||
                (s.status == 0 && (block.timestamp < s.migrationExpirationTime - s.defaultMigrationTime / 2)),
            "Status Control: Withdrawal is not enabled"
        );
    }
...
library LibWithdrawBalance {
    event PendingWithdraw(address recipient, uint8 listingId, uint88 tokenId, uint128 amount);
    ...
    function _increasePendingBalance(
        ...
        s.pendingWithdrawBalance[withdrawKey] += _amount;
        emit PendingWithdraw(_to, _listingId, _tokenId, _amount);
    }
    ...
    function _transferERC20(
        ...
        require(balanceAfter >= _minLiquidity, "LWB: Minimum liquidity limit is reached");
    ...
    function _transferNativeToken(
        ...
        require(address(this).balance >= _amount, "LWB: Insufficient balance");
        require(address(this).balance - _amount >= _minLiquidity, "LWB: Minimum liquidity limit is reached");
        address payable toPayable = payable(_to);
    ...
    function _releaseAsset(
        ...
        uint256 limit;
        if (_enableLimit) {
            limit = s.minLiquidity[_listingId];
        }

        if (_listingId == 0) {
            releasedAmount = _transferNativeToken(_recipient, _amount, limit);
        } else {
            address tokenAddress = LibTokenManagement._validateListingId(_listingId);
            uint8 tokenType = s.tokenTypes[_listingId];
            if (tokenType == 1) {
