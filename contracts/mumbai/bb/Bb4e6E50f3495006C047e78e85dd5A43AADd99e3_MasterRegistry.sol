...
contract MasterRegistry is Access {
   ...
   /**
   * @dev To store project details.
   * @param factoryId ID of a token factory the token with `tokenAddress` is created from.
   * @param tokenAddress Address of token.
   * @param name Name of project, can differ from `name` attribute of the ERC721 token.
   * @param bannerUrl
   * @param visible Set to false by default. See {createProject}. Can be altered later.
   */
    struct Project {
        uint256 factoryId;
    ...
    /// @dev To store factory details. 
    struct Factory {
        string name;
        address factoryAddress;
    }

    /// @dev Maps a factory ID to a Factory object.
    mapping (uint256 => Factory) public factories;

    /// @dev Maps a project ID to a Project object.
    mapping (uint256 => Project) public projects;

    /// @dev Number of factories.
    Counters.Counter public factoryCount;

    /// @dev Number of projects.
    Counters.Counter public projectCount;

    /**
    * @dev Emitted when a Factory is added to registry.
    * @param id ID of factory - autoincremented.
    * @param name Name of factory.
    * @param factoryAddress Address of factory.
    */  
    event FactoryAdded(uint256 id, string name, address factoryAddress);
  
    /**
    * @dev Emitted when a new project is created.
    * @param id ID of a project - autoincremented.
    * @param factoryId ID of factory the token belongs to.
    * @param tokenAddress Address of the deployed token.
    * @param name Name of the project (can differ from `name` attribute of the ERC721 token)
    * @param bannerUrl Project banner URL.
    * @param initialOwner Address of initial owner of project.
    */
    ...
    /**
    * @dev Emitted when the name and/or the banner url of a project changes.
    ...
    */
    ...
    /**
    * @dev Modifier to check if project with `_projectId` ID exists.
    * @dev Reverts with `MasterRegistry: project does not exist`.
    */ 
    modifier projectExists(uint256 _projectId) {
        Project memory project = projects[_projectId];
        require (project.tokenAddress != address(0),
                "MasterRegistry: project does not exist");
        _;
    }

    /**
    * @dev Modifier to check if factory with `_factoryId` ID exists.
    * @dev Reverts with `MasterRegistry: factory does not exist`.
    */ 
    modifier factoryExists(uint256 _factoryId) {
        Factory memory factory = factories[_factoryId];
        require (factory.factoryAddress != address(0),
                "MasterRegistry: factory does not exist");
        _;
    }
  
    /**
    * @notice Adds a factory to registry.
    * @dev Emits a {FactoryAdded} event.
    * @dev FactoryID is an autoincremented value.
    * @param _factoryName Name of the factory (arbitrary).
    * @param _factoryAddress Address of a deployed factory.
    */
    function addFactory(string memory _factoryName, address _factoryAddress)
        external
        onlyAdmin
    {
        uint256 factoryId = factoryCount.current();
        factories[factoryId] = Factory(_factoryName, _factoryAddress);
        factoryCount.increment();
        emit FactoryAdded(factoryId, _factoryName, _factoryAddress);
    }
   
    /**
    * @notice Creates a project in the registry, deploys a token.
    * @dev See {Config} for input parameters.
    * @dev ProjectID is an autoincremented value.
    * @dev Only with ADMIN_ROLE.
    * @dev Requires factory to exist in registry.
    * @dev Default project visibility is false.
    * @dev Emits a {ProjectCreated} event.
    */
    function createProject(
        ProjectConfiguration memory projectConfig,
        TokenConfiguration memory tokenConfig,
        address _initialOwner
    )
        external
        onlyAdmin
        factoryExists(projectConfig.factoryId)
    {
        uint256 factoryId = projectConfig.factoryId;
        IFactory projectFactory = IFactory(factories[factoryId].factoryAddress);
        address tokenAddress = projectFactory.deployToken(tokenConfig, _initialOwner);
        uint256 nextProjectId = projectCount.current();
        projects[nextProjectId] = Project(
            factoryId,
            tokenAddress,
            projectConfig.name, 
            projectConfig.bannerUrl, 
            projectConfig.visible
        );
    ...
    /**
    * @dev Function for user to update the name or the banner URL of a project that owns.
    * @dev Can only be called by the owner of the project (owner of the token contract associated with the project).
    * @dev Reverts with `MasterRegistry: caller is not the project owner`.
    * @dev Emits a {ProjectInformationChanged} event.
    ...
    */
    ...
    {
        address owner = projectOwner(_projectId);
        require(msg.sender == owner, "MasterRegistry: caller is not the project owner");
        Project storage projectToUpdate = projects[_projectId];
        projectToUpdate.name= _newProjectName;
        projectToUpdate.bannerUrl = _newProjectBannerUrl;
    ...
    * @dev The visibility field of projects can be used to filter inappropiate projects for off-chain applications.
    * @dev Can only be called by users with admin priviledges.
    * @dev Emits a {VisibilityChanged} event.
    ...
    */
    ...
    /**
    * @notice Function to get owner address of a project (token).
    * @param _projectId ID of a project
    * @return Address of project owner.
    */
    function projectOwner(uint256 _projectId) public view returns(address) {
        IToken token = IToken(projects[_projectId].tokenAddress);
        return token.owner();
    }
}
