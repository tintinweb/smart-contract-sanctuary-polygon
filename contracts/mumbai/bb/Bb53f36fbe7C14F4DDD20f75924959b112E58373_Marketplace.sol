    ...
    BasicMetaTransaction
{
    ...
    event TokensAdded(uint256 tokenId, uint256 listingId, uint256 amountAdded);

    event PriceUpdated(uint256 tokenId, uint256 listingId, uint256 newPrice);
    ...
    /**
     * @notice update the price of the listed property
     * @param proof MerkleProof required to verify _msgSender() whitelisted
     * @param listingId Unique identity to every listing inside marketplace
     * @param newPrice per wei token updated price for the listed property
     */
    function updatePrice(
        ...
        require(
            isWhitelisted(proof, keccak256(abi.encodePacked(_msgSender()))),
            "update: Address not whitelisted"
        );
        require(
            idToProperty[listingId].completed == false,
            "update: Property Not Available!"
        );
        require(
            idToProperty[listingId].seller == _msgSender(),
            "update: Caller not Owner"
        );

        idToProperty[listingId].price = newPrice;

        emit PriceUpdated(idToProperty[listingId].tokenId, listingId, newPrice);
    }

    /**
     * @notice add more property tokens on marketplace for the listed property
     * @param proof MerkleProof required to verify _msgSender() whitelisted
     * @param listingId Unique identity to every listing inside marketplace
     * @param amountToAdd no. of tokens to be added for the current listing
     */
    function addTokens(
        bytes32[] memory proof,
        uint256 listingId,
        ...
        require(
            isWhitelisted(proof, keccak256(abi.encodePacked(_msgSender()))),
            "add: Address not whitelisted"
        );
        require(
            idToProperty[listingId].completed == false,
            "add: Property Not Available!"
        );
        require(
            idToProperty[listingId].seller == _msgSender(),
            "add: Caller not Owner"
        );
        require(amountToAdd > 0, "add : Amount must be greater than 0!");
        require(
            mogul.balanceOf(_msgSender(), idToProperty[listingId].tokenId) >=
                amountToAdd,
            "add: Caller must own given token!"
        );
        ...
        idToProperty[listingId].tokensAvailable += amountToAdd;
        idToProperty[listingId].amount += amountToAdd;
    }
