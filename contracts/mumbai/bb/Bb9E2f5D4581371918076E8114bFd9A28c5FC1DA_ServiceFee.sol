...
contract ServiceFee is IServiceFee, AccessControl {
    ...
    function setServiceFee(
        ...
        // skip sender
        bytes32 id = keccak256(abi.encode(_target, _nftAsset));
        setFlags[id] = true;
        adminFees[id] = _fee;
    ...
    function getServiceFee(
        ...
        // skip sender
        // target only
        bytes32 id = keccak256(abi.encode(_target, address(0)));
        ids[0] = id;

        // collection only
        id = keccak256(abi.encode(address(0), _nftAsset));
        ids[1] = id;

        // target + collection
        id = keccak256(abi.encode(_target, _nftAsset));
        ids[2] = id;
