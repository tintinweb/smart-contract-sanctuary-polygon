...
contract Market is IMarket, Context {
  ...
  */
   ...
   */
  function eligibilityMethod(uint offerId_) onlyActiveOffers(offerId_) onlyApprovedOffers(offerId_) public view returns (string memory) {
  ...
  function _paperValidateBuyParams(uint256 offerId_, bool directBuy_, address buyer_) internal view {
    ...
    if (offer.paymentToken == address(0)) {
      ...
      require(IERC20(offer.paymentToken).allowance(_msgSender(), address(this) ) >= getValue(offerId_),'M115');
    }
  }
  ...
  */
    ...
    onlyPaper(keccak256(abi.encode(offerId_, buyer_, directBuy_, hiddenId_)), nonce_, signature_)
  {
    ...
    emit OfferCompleted( offerId_, hiddenId_, _msgSender());
  }
  ...
  */
  ...
  /**
   * @notice function to check if msg.sender is relayer module
   * @param _msgSender address to check
  */
  function _validCaller(address _msgSender) internal view returns (bool){
    return (_msgSender == moduleManager.getModule(4)) ? true : false;
  }
  ...
  function createOffer(
    ...
    bool notApproved = _validateOwnership(tokenIds_, collectionAddresses_) || _validCaller(msg.sender);
  ...
  */
  ...
  function _transactAllTokens(uint256 offerId_) internal  {
    ...
    for (uint256 i = 0; i < offer.collectionAddresses.length; i++) {
      require(IERC721(offer.collectionAddresses[i]).ownerOf(auxTokenIds[i]) == offer.seller , 'E413');
      require(IERC721(offer.collectionAddresses[i]).isApprovedForAll(offer.seller, address(this)), 'M118');
      IERC721(offer.collectionAddresses[i]).safeTransferFrom(offer.seller, _msgSender(), auxTokenIds[i], 'M135');
    }
  }
...
//! --------------------------------------------------------------------------- GETTERS ---------------------------------------------------------------------------
  ...
  /**
   * @notice Function to check if the user is allowed to {create/buy}
   */
  function isUser(address toAddress) public view returns (string memory) {
    return rolesContract.isUser(toAddress) ? '' : 'not allowed';
  }
