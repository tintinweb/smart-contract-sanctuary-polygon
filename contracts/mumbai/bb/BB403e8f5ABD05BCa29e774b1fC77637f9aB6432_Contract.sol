...
// Import the pre-defined Smart Contract of the ERC721 Base
import "@thirdweb-dev/contracts/base/ERC721Base.sol";
import "@thirdweb-dev/contracts/extension/PermissionsEnumerable.sol";
import "@thirdweb-dev/contracts/extension/interface/IDelayedReveal.sol";

// Define our contract.
// Here the contract exists out of multiple sub-contracts provided by Thridweb.
contract Contract is ERC721Base, PermissionsEnumerable {

      // We want to set the power level of the NFTs.
      // Thus we need a mapping from Token ID to the power level of that NFT.
      // uint256 is the format for the token ID.
      // Making something 'public' you can easily reuse this in your code.
      ...
      constructor(
       ...
       // Call the actual function, with the pre-set variables from above.
        ERC721Base(
            _name,
            _symbol,
            _royaltyRecipient,
    ...
    {
        // Set up the DEFUALT ADMIN ROLE.
        // And provide that role to the wallet deployer
        // Meaning the Wallet that deployed the smart contract automatically becomes the Admin.
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
    }

    // This function updates the power level of an NFT.
    // And is only callable by the admin members.
    ...
    // Here we change the original mintTo function to do some extra work.
    // What we want it to do, is to update the power level of each NFT as it gets minted to be equal to the token ID
    // that we are minting. E.g. when NFT 1 is about to be minted, we set its power level to 1.
    //
    // For this we 'override' the pre-defined mintTo function in the imported contract.
    // Set the power level of the token ot be the power level of the token ID
    function mintTo(address _to, string memory _tokenURI) public virtual override {

        // Different to the original function, we first set the next tokenID.
        // Grab the next available token ID
        uint256 tokenID = nextTokenIdToMint();

        // Now cantinue to actually mint the NFT with the original mintTo function.
        // Altough we 'override', we can still call the original function using 'super'.
        super.mintTo(_to, _tokenURI);

        // Next re-set the power level of the minted NFT
