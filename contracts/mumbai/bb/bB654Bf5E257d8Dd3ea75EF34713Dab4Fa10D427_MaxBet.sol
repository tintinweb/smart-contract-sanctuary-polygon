...
contract MaxBet is Pool {
    ...
    uint256 public MINIMUM_BET_AMOUNT = 0.5 ether;
    ...
    mapping(bytes32 => bool) public allCommitments;
    ...
    */
    ...
    function getLuckyNumber(uint64 betIndex, uint256 secret)
        private
        view
        returns (uint64)
    {
    ...
    function placeBet(
        ...
        require(!allCommitments[commitment], "!allCommitments");
        require(validTill > block.timestamp, "!valid time");
        ...
        uint64 index = uint64(bets.length);
        ...
        allCommitments[commitment] = true;
        ...
        bets.push(
            Bet({
                ...
                isFinished: false,
                luckyNumber: 0,
                commitment: uint256(commitment),
                bh: uint256(blockhash(block.number - 1)),
                blockHeight: block.number
            })
        );
        emit NewBet(msg.sender, commitment, index, number, isOver, msg.value);
    }
    ...
    function settleBet(
        ...
        uint64 luckyNum = getLuckyNumber(bet.index, secret);
    ...
    function setRefferalReward(uint256 value) external onlyOwner {
