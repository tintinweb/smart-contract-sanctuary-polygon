...
import "IHybridBulkMinter.sol";
import "ERC721ABurnableUpgradeable.sol";

contract HybridBulkMinter is
IHybridBulkMinter,
...
UUPSUpgradeable {
    ...
    function bulkMint(bytes32[] calldata metadata_, bytes[] calldata signatures) external {
        uint256 quantity = metadata_.length;
        for (uint i = 0; i < quantity; i++) {
            require(msg.sender == ECDSAUpgradeable.recover(ECDSAUpgradeable.toEthSignedMessageHash(metadata_[i]), signatures[i]), "Invalid signature");
            metadata[latestMintedTokenId + i] = metadata_[i];
        }
    ...
    function tokenURI(uint256 tokenId) public view virtual override(IERC721AUpgradeable, ERC721AUpgradeable) returns (string memory) {
        ...
        return string(abi.encodePacked("ar://", encode(bytes.concat((metadata[tokenId])))));
    }
    ...
    returns (bool)
    {
        return AccessControlUpgradeable.supportsInterface(interfaceId) || ERC721AUpgradeable.supportsInterface(interfaceId);
    }
...
interface IHybridBulkMinter {
    function bulkMint(bytes32[] calldata metadata, bytes[] calldata signatures) external;
}
