...
contract MultiSigFactory is Factory, Ownable {
    ...
    function createMultiSig(
        ...
        require(
           ...
           if(_subscription == 1){
               require(_owners.length <= _sub1Owners, "Only three owners for subscription 1");
           }
           else if (_subscription == 2){
                require(_owners.length <= _sub2Owners, "Only five owners for subscription 2");
           }
           else{
                require(_owners.length <= _sub3Owners, "Only ten owners for subscription 3");
           }
        
        address[] memory temp = totalAddresses[0];
        bool tempCheck;
        for (uint256 j = 0; j < temp.length; j++) {
            if (msg.sender == temp[j]) {
                tempCheck = true;
            }
        }
        if (tempCheck == false) {
        ...
        for (uint256 k = 0; k < _owners.length; k++) {
            if (isOneOfOwner[_owners[k]] == false)
                isOneOfOwner[_owners[k]] = true;
        }
        uint256 price;
        if (_subscription == 1) {
