...
abstract contract Consumer is ConsumerInterface, FunctionsClient {
    using Functions for Functions.Request;

    StonkslyInterface public immutable i_stonksly;
    uint64 public immutable i_subscriptionId;
    string internal s_requestSource;

    mapping(bytes32 => uint256) internal s_requests;

    event RequestSent(bytes32 oracleRequestId, uint256 stonkslyRequestId);
    event ResponseReceived(bytes32 oracleRequestId, uint256 stonkslyRequestId, uint256 price);
    event RequestFailed(bytes32 oracleRequestId, bytes error);

    constructor(
        StonkslyInterface _stonksly,
     ...
     function init(
        ...
        emit RequestSent(requestId, _stonkslyRequestId);
    }
}
