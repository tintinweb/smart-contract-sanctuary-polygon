    ...
    function createMarketSale(address nftContractAddress, uint256 marketItemId) public payable nonReentrant {
        ...
        if (seller == creator) {
           ...
           payable(account).transfer(fee);
            ...
            uint256 sellerfee = msg.value * 30 / 100;
  
            marketItemIdToMarketItem[marketItemId].seller.transfer(msg.value - fee - sellerfee);
            IERC721(nftContractAddress).transferFrom(address(this), msg.sender, tokenId);
   
            _tokensSold.increment();
            payable(account).transfer(fee);
            payable(creator).transfer(sellerfee);
        }
    }
...
contract NFTCollection is ERC721, Ownable, ERC721Burnable, ERC721Enumerable, ERC2981Collection {
 ...
 mapping(uint256 => string) private _tokenURIs;
  ...
  constructor(address _marketplaceAddress) ERC721("For Test26", "MARK") {
