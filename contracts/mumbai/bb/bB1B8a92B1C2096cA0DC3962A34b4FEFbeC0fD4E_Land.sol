...
contract Land is Royalty, AccessControl, BaseRegistrarImplementation, Pausable, TokenURISetting {
     ...
     */
    function init(address micAddress_, address midAddress_, address payable micBank_, address payable midLandBank_, address payable alchemistBank_) external {
        // require(hasRole(LAND_ADMIN, msg.sender), "init only land admin");
        // require(_midAddress == address(0) && midAddress_ != address(0), "init already initialized");
        _midAddress = midAddress_;
        ROYALTY = 100*(10**18);
        init(micBank_, midLandBank_, alchemistBank_, micAddress_);
    }
    ...
    function priceOracle(uint256 count) internal pure returns(uint256) {
        uint256 eplch = count / 5; //100
        if (eplch == 0) {
            return 0;
        }
        return 500 ether + eplch * 10 ether - 10 ether;
    }
    ...
    function mint(string memory name_) public nonReentrant {
        require(name_.lenOfChars() > 0 && name_.lenOfChars() <= 8, "");
        ...
        require(_midAddress != address(0x0), "MetaLoot is not initialized");
    ...
    function updateMember(uint256 tokenIdOfLand, uint256 tokenIdOfMid) public nonReentrant {
        ...
        require(msg.sender == _midAddress, "only mid owner can update relationship");
...
abstract contract Royalty is ReentrancyGuard, Context, IncomingTransferSupport{
     ...
     * @dev Initializes the contract with the given parameters.
     */
    function init(address payable micBank_, address payable midLandBank_, address payable alchemistBank_, address micAddress_) internal {
        _micBank = micBank_;
