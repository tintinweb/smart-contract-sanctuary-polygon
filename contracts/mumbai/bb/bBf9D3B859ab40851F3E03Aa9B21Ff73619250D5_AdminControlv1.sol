...
contract AdminControlv1 is Initializable, OwnableUpgradeable {
    ...
    function addAdmin(address _address) public onlyOwner {
        require(adminPermissions[_address].role != Role.ADMIN, "Admin already exists");
    ...
    //INVESTOR MANAGEMENT
    function hasInvestorManagement(address _address) external view returns (bool) {
        return adminPermissions[_address].role == Role.SUPER_ADMIN ? true : adminPermissions[_address].investorManagement;
    }
    ...
    //CHAIN MANAGEMENT
    function hasChainManagement(address _address) external view returns (bool) {
        return adminPermissions[_address].role == Role.SUPER_ADMIN ? true : adminPermissions[_address].chainManagement;
    }
    ...
    //TEST
    function hasTest(address _address) external view returns (bool) {
        return adminPermissions[_address].role == Role.SUPER_ADMIN ? true : adminPermissions[_address].test;
    }

    function grantTest(address _address) public onlyOwner {
        require(adminPermissions[_address].role == Role.ADMIN, "Address is not a test add");
        adminPermissions[_address].test = true;
    }

    function revokeTest(address _address) public onlyOwner {
        require(adminPermissions[_address].role == Role.ADMIN, "Address is not a test add");
        adminPermissions[_address].test = true;
    }
}