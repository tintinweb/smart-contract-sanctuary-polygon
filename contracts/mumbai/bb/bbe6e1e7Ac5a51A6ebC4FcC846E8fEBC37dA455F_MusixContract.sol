...
contract MusixContract is ERC721Enumerable,Ownable {
    ...
    function tryOne() public view returns (uint256){
        return block.timestamp - startDate;

    }
    function tryTwo() public view returns (uint256){
        return block.timestamp;

    }
    function tryThree() public view returns (uint256){
        return startDate;

    }
    function tryFour() public view returns (uint256){
        return (block.timestamp - startDate) / (1* 1 days);

    }
    function tryFive() public view returns (uint256){
        uint256 daysAfterLaunch = 0;
        if(block.timestamp - startDate > 0){
            daysAfterLaunch = (block.timestamp - startDate) / (1* 1 days);
        }
        return MIN_PRICE+(daysAfterLaunch* 0.01168 ether);
