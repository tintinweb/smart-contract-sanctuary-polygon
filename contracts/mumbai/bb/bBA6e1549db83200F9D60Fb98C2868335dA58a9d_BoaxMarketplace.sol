...
import "./interfaces/IBoaxMarketplaceState.sol";
import "@openzeppelin/contracts/access/AccessControlEnumerable.sol";
import "@openzeppelin/contracts/utils/Counters.sol";
import "@openzeppelin/contracts/security/ReentrancyGuard.sol";

contract BoaxMarketplace is AccessControlEnumerable, ReentrancyGuard {
    ...
    // solhint-disable-next-line
    IBoaxMarketplaceState private BoaxMarketplaceState;
     ...
     */
    function mint(string memory _tokenURI) external onlyRole(MINTER_ROLE) {
        uint256 _tokenId = BoaxERC721.totalSupply();
        BoaxERC721.mint(_tokenURI, msg.sender);
        BoaxMarketplaceState.setTokenArtist(_tokenId, msg.sender);
    }
    ...
    function createOrder(uint256 _tokenId, uint256 _price) external {
        ...
        BoaxERC721.transferFrom(msg.sender, address(this), _tokenId); // transfer token to contract
    ...
    function removeOrder(uint256 _orderId) external isOrderOwner(_orderId) {
        ...
        BoaxERC721.transferFrom(address(this), _order.from, _order.tokenId); // transfer token to owner
    ...
    function _trade(TradeInnovice memory _tradeInnovice) private {
        ...
        ) = BoaxMarketplaceState.getStateInfo(_tradeInnovice.tokenId);
        if (!isSecondarySale) {
            BoaxMarketplaceState.setSecondarySale(_tradeInnovice.tokenId);
        } else {
            uint256 _secondaryFeeArtist = (_tradeInnovice.price * artistFee) /
                10000; // Fee paid by the user that fills the order, a.k.a. msg.sender.
            sellerAmount -= _secondaryFeeArtist;

            address artist = BoaxMarketplaceState.artists(
                _tradeInnovice.tokenId
            );
            // solhint-disable-next-line avoid-low-level-calls
            (success, ) = payable(artist).call{value: _secondaryFeeArtist}(""); // transfer secondary sale fees to fee artist
            // solhint-disable-next-line reason-string
            require(success);
        }

        marketplaceAmount = (_tradeInnovice.price * marketplaceFee) / 10000;
     ...
     */
    constructor(address boaxERC721, address boaxMarketplaceStateContract) {
        require(
            boaxERC721 != address(0) ||
                boaxMarketplaceStateContract != address(0),
            "Invalid address"
        );
        // default values
        ...
        BoaxMarketplaceState = IBoaxMarketplaceState(
            boaxMarketplaceStateContract
        );
    }
}
...
interface IBoaxMarketplaceState {
    function getStateInfo(uint256)
        external
        view
        returns (
            address,
     ...
     */
    function artists(uint256) external view returns (address);

    /**
     * @dev only either auction or marketplace contract can call it to set tokenId as secondary sale.
     */
    function setSecondarySale(uint256) external;

    /**
     * @dev only either auction or marketplace contract can call it to set tokenId to artist.
     */
    function setTokenArtist(uint256, address) external;
}
