    ...
    RoyaltiesV2Impl
{
    ...
    function mintWithRoyalty(
        ...
        uint96[] memory royaltyFeeNumerator
    ) public {
        require(
            royaltyReceivers.length == royaltyFeeNumerator.length,
            "ElevateNFT: Invalid Input"
        );
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        _safeMint(to, tokenId);
        ...
        LibPart.Part[] memory _royalties = new LibPart.Part[](
            royaltyReceivers.length
        );
        for (uint256 i = 0; i < royaltyReceivers.length; i++) {
            _royalties[i].value = royaltyFeeNumerator[i];
            _royalties[i].account = payable(royaltyReceivers[i]);
        ...
        returns (address receiver, uint256 royaltyAmount)
    {
        ...
        if (_royalties.length > 0) {
            ...
            //getRaribleV2Royalties(_tokenId);
        }
        return (address(0), 0);
    }
