...
/**
 * @title StStableImplementationV3
 * @dev this contract is a Pausable ERC20 token with Burn and Mint
 * controlled by a central SupplyController. By implementing StStableImplementation
 ...
 */
contract StStableImplementationV1 {
    ...
    string public constant name = "Stable"; // solium-disable-line
    string public constant symbol = "StStable"; // solium-disable-line uppercase
    ...
    */
    ...
    /**
     * @dev Reclaim all StStable at the contract address.
     * This sends the StStable tokens that this contract add holding to the owner.
     * Note: this is not affected by freeze constraints.
     */
    function reclaimStStable() external onlyOwner {
    ...
    */
    ...
    modifier onlySupplyController() {
        require(tx.origin == supplyController, "onlySupplyController");
        _;
    }
    ...
    function _betaDelegatedTransfer(
        ...
        bytes32 delegatedTransferHash = keccak256(abi.encodePacked(// solium-disable-line
                EIP712_DELEGATED_TRANSFER_SCHEMA_HASH, bytes32(uint256(uint160(address(to)))), value, fee, seq, deadline
            ));
        ...
        require(!frozen[to] && !frozen[_from] && !frozen[msg.sender], "address frozen");
         require(value + fee <= balances[_from], "insufficient fund");
    ...
    function betaDelegatedTransferBatch(
        bytes32[] memory r, bytes32[] memory s, uint8[] memory v, address[] memory to, uint256[] memory value, uint256[] memory fee, uint256[] memory seq, uint256[] memory deadline
    ) public returns (bool) {
