...
contract CookieToken is ERC20, ERC20Burnable, Ownable {
    ...
    uint256 private _minSellTime = 30 days;

    event SetFreeFee(address _to, bool _free);
    ...
    constructor(uint256 _initialSupply, IUniswapV2Router02 _router) ERC20("Cookie", "CKIE") {
        ...
        _setFreeFee(address(this), true);
        _setFreeFee(owner(), true);
    ...
    function removeToHasFee(address _addresWithFee) public onlyOwner {
        delete hasFee[_addresWithFee];
    }

    function addToHasFee(address _addresWithFee) public onlyOwner {
        hasFee[_addresWithFee] = true;
    }

    function setFreeFee(address _to, bool _free) external onlyOwner {
        _setFreeFee(_to, _free);
    }

    function _setFreeFee(address _to, bool _free) internal {
        freeFee[_to] = _free;
    ...
    function setSellTime(uint256 minSellTime_) external onlyOwner {
        _minSellTime = minSellTime_;
        emit Minselltime(minSellTime_);
    }

    function haveTax(address user) public view returns(bool) {
        return block.timestamp - _lastTransfer[user] < _minSellTime;
    }
    ...
    function _transfer(address _from, address _to, uint256 _amount) internal override {
        if (hasFee[_to] && !freeFee[_from] && treasury != address(0)) {
            // if (block.timestamp - _lastTransfer[_from] < _minSellTime) {
            if (haveTax(_from)) {
