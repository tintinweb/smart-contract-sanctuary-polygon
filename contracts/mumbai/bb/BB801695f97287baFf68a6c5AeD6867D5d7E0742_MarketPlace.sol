...
contract MarketPlace is Ownable, IMarketPlace, Multicall {
	...
	modifier onlyGalleryOrTokenOwner(uint256 _tokenId) {
		...
		if (!isGallery[msg.sender] && owner != msg.sender) {
			revert('Should have access to call this function');
		}
		_;
	}
	...
	modifier onlyAdmin() {
		require(isAdmin[msg.sender], 'Only Admin can execute this function');
		_;
	}
	...
	function buy(uint256 _tokenId, address _buyer) public payable override {
		...
		require(_buyer != owner, 'Owner: owner cannot buy');
		...
		require(msg.value == sellingPrice, 'Price didnot match selling price');
		...
		feeinfo.totalgalleryownerfee = addTotal(_galleryownerfee, feeinfo.totalgalleryownerfee);
		...
		transferfees(owner, ownerfee);

		nft.safeTransferFrom(owner, _buyer, _tokenId);
		TokenInfo.onSell = false;
		TokenInfo.totalSell += 1;
		TokenInfo.minPrice = 0;
		TokenInfo.artistfee = 0;
		TokenInfo.galleryownerfee = 0;
		TokenInfo.thirdpartyfee = 0;
		TokenInfo.USD = false;
		tokenIdOnSell.remove(_tokenId);
		emit nftbought(_tokenId, owner, _buyer, msg.value);
	}
		...
		)
	{
		...
		artistcommssion = cutPer10000(TokenInfo.artistfee, sellingPrice);
		platformCommission = cutPer10000(platformfee, sellingPrice);
		_galleryOwnercommission = cutPer10000(TokenInfo.galleryownerfee, sellingPrice);

		if (block.timestamp <= TokenInfo.feeExpiryTime) {
			thirdPartyCommission = cutPer10000(TokenInfo.thirdpartyfee, sellingPrice);
		} else thirdPartyCommission = 0;
		uint256 ownerfee = sellingPrice.sub(artistcommssion).sub(platformCommission).sub(_galleryOwnercommission).sub(
			thirdPartyCommission
		);

		return (_galleryOwnercommission, artistcommssion, platformCommission, thirdPartyCommission, ownerfee);
	}
	...
	function getLatestPrice() public view returns (int256) {
		(, int256 price, uint256 startedAt, uint256 timeStamp, uint80 answeredInRound) = priceFeed.latestRoundData();
		return price;
	}
