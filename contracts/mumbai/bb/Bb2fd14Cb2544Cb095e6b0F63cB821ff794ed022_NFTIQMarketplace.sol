    ...
    ReentrancyGuard
{
    ...
    */
    ...
    function listNft(
        ...
        require(_isNFT(_nftAddress), "Sell Nfts: Not confirming to an NFT contract");
        _setNftDetails(_nftAddress, _tokenId, _ipfsHash, _royaltyReciever, _royaltyPercentage);    
        _setSaleDetails(
            _tokenId,
            _price,
            _quantity,
    ...
    */
    ...
    function settleAuction(
        ...
        if (NftOnAuction.currentBidder != address(0)) {
            ...
            _mintNFT(NftOnAuction.currentBidder, _nftAddress, NftOnAuction.tokenId, NftOnAuction.quantity); 
        }
    ...
    function _payOut(
        ...
        if (_currency == address(0)) {
            ...
            if(_isDirectPurchase){
                if(platformFee.receiver != address(0) && platformProfit > 0){
                    IERC20(_currency).safeTransferFrom(msg.sender, platformFee.receiver, platformProfit);
                }
                IERC20(_currency).safeTransferFrom(msg.sender, _seller, sellerProfit);
            }
            else{
                if(platformFee.receiver != address(0) && platformProfit > 0){
                    IERC20(_currency).safeTransferFrom(msg.sender, platformFee.receiver, platformProfit);
    ...
    function _mintNFT(address _receiver, address _nftAddress, uint256 _tokenId, uint256 _quantity) internal {

        NftInfo storage NftInfoStored = mapNftInfo[_nftAddress][_tokenId];
        if (IERC721(_nftAddress).supportsInterface(ERC721InterfaceId)) {
            ERC721.Royalties memory royalties = ERC721.Royalties(payable(NftInfoStored.royaltyAddress), NftInfoStored.royaltyAmount);
            ERC721(_nftAddress).mint(_receiver, _tokenId, string(NftInfoStored.metaHash), royalties);
        } 
        else if (IERC1155(_nftAddress).supportsInterface(ERC1155InterfaceId)) {
            ERC1155.Royalties memory royalties = ERC1155.Royalties(payable(NftInfoStored.royaltyAddress), NftInfoStored.royaltyAmount);
            ERC1155(_nftAddress).mint(_receiver, _tokenId, _quantity, NftInfoStored.metaHash, royalties); 
        }
    }
