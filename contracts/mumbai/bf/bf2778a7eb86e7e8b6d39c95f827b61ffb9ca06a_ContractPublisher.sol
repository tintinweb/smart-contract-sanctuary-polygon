...
// File: interfaces/IContractPublisher.sol
...
interface IContractPublisher {
    ...
    struct CustomContractSet {
        EnumerableSetUpgradeable.Bytes32Set contractIds;
...
contract ContractPublisher is IContractPublisher, ERC2771ContextUpgradeable, AccessControlEnumerableUpgradeable, MulticallUpgradeable {
    ...
    function getAllPublishedContracts(address _publisher)
        ...
        returns (CustomContractInstance[] memory published)
    {
        uint256 total = EnumerableSetUpgradeable.length(contractsOfPublisher[_publisher].contractIds);

        published = new CustomContractInstance[](total);

        for (uint256 i = 0; i < total; i += 1) {
            bytes32 contractId = EnumerableSetUpgradeable.at(contractsOfPublisher[_publisher].contractIds, i);
    ...
    function publishContract(
        ...
        bytes32 contractIdInBytes = keccak256(bytes(_contractId));
        EnumerableSetUpgradeable.add(contractsOfPublisher[_publisher].contractIds, contractIdInBytes);
        ...
        onlyUnpausedOrAdmin
    {
        ...
        bool removed = EnumerableSetUpgradeable.remove(contractsOfPublisher[_publisher].contractIds, contractIdInBytes);
