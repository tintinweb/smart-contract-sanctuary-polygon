...
contract StargateBridgeAdapter is BridgeAdapterBase {
    ...
    constructor(
        ...
        _feeExchangeRate = feeExchangeRate;
        emit FeeExchangeRateChanged(feeExchangeRate);
    }
    ...
    function setFeeExchangeRate(uint256 feeExchangeRate) external onlyOwner {
        _feeExchangeRate = feeExchangeRate;
        emit FeeExchangeRateChanged(feeExchangeRate);
    }
    ...
    function bridge(
        ...
        (state.destChainId, state.srcPoolId, state.dstPoolId, state.refundAddress, state.maxFee) = abi.decode(
            params.data,
            (uint16, uint256, uint256, address, uint256)
        );
        ...
        state.feeTokenIn = (state.fee * _feeExchangeRate) / FEE_DENOMINATOR;
        if (state.feeTokenIn > 0) {
            if (state.tokenInDecimals < _nativeAssetDecimals) {
                state.feeTokenIn = state.feeTokenIn / (10 ** (_nativeAssetDecimals - state.tokenInDecimals));
            } else if (state.tokenInDecimals > _nativeAssetDecimals) {
                state.feeTokenIn = state.feeTokenIn * (10 ** (state.tokenInDecimals - _nativeAssetDecimals));
            }
            require(state.feeTokenIn <= state.maxFee, "StargateBridgeAdapter: insufficient fee");
            require(state.feeTokenIn < state.amountIn, "StargateBridgeAdapter: insufficient amount");
            IERC20(params.tokenIn).safeTransfer(_feeRecipient, state.feeTokenIn);
            state.amountIn -= state.feeTokenIn;
        }
