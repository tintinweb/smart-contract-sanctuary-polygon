...
// File: contracts/CreationUpdateRecord.sol
...
contract CreationUpdateRecord is ERC1155{
    ...
    constructor () ERC1155("HopeAccelerated")    {
        owner = msg.sender;
    ...
    function start_visit(uint _time) public only_owner returns (address){
        address unique_id = address(bytes20(sha256(abi.encodePacked(msg.sender, block.timestamp))));
        record_mapping[unique_id].is_uid_generated = true;
        record_mapping[unique_id].record_id = uint160(unique_id);
