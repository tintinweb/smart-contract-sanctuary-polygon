...
contract WonderGameAuction is Ownable, INFTMintInitiator {
    ...
    function _start(uint256 _tokenId) internal {
        ...
        auctionRegistry[auctionId].endTime = startTime + (bidtime * (auctionId + 1));

        auctionRegistry[auctionId].nftId = _tokenId;
        auctionRegistry[auctionId].highestBid = startingBid;
        auctionRegistry[auctionId].startTime = startTime + (bidtime * auctionId);
        emit AuctionStarted(auctionId, _tokenId, auctionRegistry[auctionId].startTime);
    }

    function bid(uint256 _bidAmount) external {
        require(
            _bidAmount > auctionRegistry[auctionId].highestBid,
    ...
    function increaseBid(uint256 _newBid) external {
        address user = msg.sender;
        require(user == auctionRegistry[auctionId].highestBidder, "Only highest bidder can increase the bid!");

        uint256 _previousHighestBid = auctionRegistry[auctionId].highestBid;

        shroomContract.transferFrom(user, address(this), _newBid - _previousHighestBid);
    ...
    function _bid(uint256 _bidAmount) internal {
        ...
        BidHistory memory tempBidHistory = BidHistory(user, auctionRegistry[auctionId].highestBid, currenttimestamp);

        auctionRegistry[auctionId].bidHistorybyId.push(tempBidHistory);
        auctionRegistry[auctionId].bidCounter += 1;

        emit Bid(auctionId, user, auctionRegistry[auctionId].highestBid, currenttimestamp);
    }
