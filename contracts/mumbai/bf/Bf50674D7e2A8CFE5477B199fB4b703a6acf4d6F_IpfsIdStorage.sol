...
contract IpfsIdStorage is Ownable {
    ...
    mapping(uint256 => mapping(uint256 => EnumerableSet.UintSet))private _freeIds;
    ...
    modifier onlyAuth() {
        require(
            msg.sender == gameAddresses.getZaiAddress(),
             "!Auth");
        _;
    }
    ...
    function setSeasonCID(uint256 _season, string memory _cid) external onlyOwner{
        _seasonCid[_season] = _cid; 
    }

    function getSeasonCID(uint256 _season) external view returns(string memory){
    ...
    function updateSeason(uint256 _season) external onlyOwner{
        _currentSeason = _season;
    }

    function setMultiplesIds(uint256 _ids, uint256 _season, uint256 _state) external onlyOwner {
    ...
    function _addID(uint256 _season, uint256 _state, uint256 _id) internal returns(bool) {
       return EnumerableSet.add(_freeIds[_season][_state], _id);
    }

    function _removeID(uint256 _season, uint256 _state, uint256 _id) internal returns(bool) {
       return EnumerableSet.remove(_freeIds[_season][_state], _id);
    }

    function getIdsLength(uint256 _season, uint256 _state) external view returns(uint256) {
        return _getIdsLength(_season,_state);
    }

    function _getIdsLength(uint256 _season, uint256 _state) internal view returns(uint256) {
        return EnumerableSet.length(_freeIds[_season][_state]);
    }
    ...
    function getTokenURI(uint256 _season, uint256 _state, uint256 _id) external view returns(string memory){
        ...
        return string(abi.encodePacked("https://ipfs.io/ipfs/",_seasonCid[_season],"/content/",_stateName,"/",Strings.toString(_id),".json"));
    }

    function getNextIpfsId(uint256 _state) external onlyAuth returns(uint256){
        uint256 _newIpfsId = _getRandomId(_state);
        _removeID(_currentSeason,_state,_newIpfsId);
    ...
    function _getRandomId(uint256 _state) internal returns(uint256){
        ...
        uint256 index = 
            ...
            .getRandom(keccak256(abi.encodePacked(_currentSeason,EnumerableSet.length(_freeIds[_currentSeason][_state]),block.timestamp)));

        index = index % (EnumerableSet.length(_freeIds[_currentSeason][_state]));
        return EnumerableSet.at(_freeIds[_currentSeason][_state], index);
    }
