    ...
    IFeeTierPlacementExtension
{
    ...
    bytes32 private constant TIER_CONTROLLER_ROLE =
        keccak256("TIER_CONTROLLER_ROLE");

    /// @dev Mapping from proxy contract or proxy deployer address => pricing tier.
    mapping(address => Tier) private tier;

    /// @dev Mapping from pricing tier id => Fee Type (libraries/FeeType.sol) => FeeInfo
    ...
    constructor(address _trustedForwarder, address _factory)
    ...
    function setFeeTierPlacementExtension(address _extension)
        public
        ...
        )
    {
        ...
        (
            upfrontSalesRecipient,
            upfrontSalesBps,
            royaltySalesRecipient,
            royaltySalesBps
        ) = (
            targetFeeInfo.upfrontSalesRecipient,
            targetFeeInfo.upfrontSalesBps,
            targetFeeInfo.royaltySalesRecipient,
            targetFeeInfo.royaltySalesBps
        );
    }
    ...
    function setFeeInfoForTier(
        uint256 _tierId,
        address _upfrontSalesRecipient,
        uint256 _upfrontSalesFeeBps,
        address _royaltySalesRecipient,
        uint256 _royaltySalesFeeBps,
        uint256 _feeType
    ) external onlyRole(TIER_FEE_ROLE) {
        require(_upfrontSalesFeeBps <= MAX_FEE_BPS, "fee too high.");
        require(_royaltySalesFeeBps <= MAX_FEE_BPS, "fee too high.");

        FeeInfo memory feeInfoToSet = FeeInfo({
            upfrontSalesBps: _upfrontSalesFeeBps,
            upfrontSalesRecipient: _upfrontSalesRecipient,
            royaltySalesBps: _royaltySalesFeeBps,
            royaltySalesRecipient: _royaltySalesRecipient
        });
        ...
        emit FeeTierUpdated(
            _tierId,
            _feeType,
            _upfrontSalesRecipient,
            _upfrontSalesFeeBps,
            _royaltySalesRecipient,
            _royaltySalesFeeBps
        );
    }
