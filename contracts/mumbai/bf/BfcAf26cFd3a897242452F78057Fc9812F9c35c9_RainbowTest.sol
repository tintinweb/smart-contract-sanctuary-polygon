...
contract RainbowTest is ERC721Enumerable, IRamperInterface721, Ownable, ERC2981 {
    uint256 public constant MAX_TOKENS = 734;
    uint256 public constant MAX_TOKENS_PER_TXN = 6;
    uint256 public constant MAX_TOKENS_PER_ADDRESS = 12;
    ...
    constructor(string memory _name, string memory _symbol, string memory _URI) ERC721(_name, _symbol) {
        ...
        _setDefaultRoyalty(deployer, 150);

        uint256 tokenStart = totalSupply();
        for (uint256 i = 0; i < 10; i++) {
            uint256 _tokenId = tokenStart + i;
            _safeMint(deployer, _tokenId);
        }
    }

    function availableTokens(address _userWallet) external view override returns (uint256 quantity) {
    ...
    function mint(address _userWallet, uint256 _quantity) external payable override {
        require(msg.value >= cost * _quantity, "Insufficient funds for requested tokens");
    ...
    function airdrop(address _rec, uint256 _quantity) public onlyOwner {
        require(this.availableTokens(_rec) >= _quantity, "Requested number of tokens is over limit for minting");
        uint256 tokenStart = totalSupply();

        for (uint256 i = 0; i < _quantity; i++) {
            uint256 _tokenId = tokenStart + i;
            _safeMint(_rec, _tokenId);
        }
    }
    ...
    function safeBulkTransferFrom(address _from, address _to, uint256[] memory _tokenIds) external override {
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            safeTransferFrom(_from, _to, _tokenIds[i]);
        }
    }

    function updatePrice(uint256 _cost) public onlyOwner {
        cost = _cost;
    }

    function tokensOwner(address owner) public view returns (uint256[] memory) {
    ...
    function contractURI() public pure returns (string memory) {
        return "https://www.jsonkeeper.com/b/WH8M";
    }
}
