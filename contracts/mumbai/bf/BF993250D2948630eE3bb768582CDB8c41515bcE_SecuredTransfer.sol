...
contract SecuredTransfer is Ownable {
    ...
    // user sended transactions
    mapping (address => EnumerableSet.Bytes32Set) sended;
    ...
    */
    modifier validateHash(address from, address to, uint256 amount, uint256 code) {
        ...
        sended[from].remove(messageHash);
        _;
    }
    ...
    */
     ...
     * @param to receiver address
    */
    function getUserClaimTransfers(address to) public onlyOwner view returns (transactionInfo[] memory)
    {
     ...
     * @param from sender address
    */
    function getUserSendedTransfers(address from) public onlyOwner view returns (transactionInfo[] memory) {
        EnumerableSet.Bytes32Set storage sendedMap = sended[from];
        transactionInfo[] memory sendedArray = new transactionInfo[](sendedMap.length());

        for (uint256 k = 0; k < sendedArray.length; ++ k) {
            bytes32 msgHash = sendedMap.at(k);
            sendedArray[k] = hashTransactionInfo[msgHash];
        }
    ...
    */
    function revertTransfer(address to, address from, uint256 amount, bytes32 msgHash) 
    public
    onlyOwner 
    {
        delete hashTransactionInfo[msgHash];
        received[to].remove(msgHash);
        sended[from].remove(msgHash);
