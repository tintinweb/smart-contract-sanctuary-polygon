...
error BCBCT__Stake__InsufficientBalance();
...
contract Staking is APR, ReentrancyGuard {
    ...
    event Withdrawn(address indexed owner, uint256 tokenAmount);
    //--------------enum---------------------//
    enum Status {
        UNSTAKED,
        STAKED
    ...
    mapping(address => uint256[]) internal stakeID;
    //--------------modifier---------------------//
    ...
    modifier isFreezeFree(uint256 amount) {
        (, uint256 unfreezed_ , uint256  gifted_) = BCBCT2.transferAble(msg.sender);
        uint canStakedAmount = gifted_ + unfreezed_;
        console.log("canStakedAmount",canStakedAmount);
        if (amount > canStakedAmount){
            revert BCBCT__Stake__InsufficientBalance();
        }else{}
        _;
    }
        ...
        returns (bool)
    {
        ...
        emit Staked(msg.sender, stakeAmount, _tenure, (len + 1),_apr);
        ...
        returns (bool)
    {
        ...
        if (getStakesIDOf(sender).length <= 1)
            stakeStatus[sender].isStaked = Status.UNSTAKED;
        (reward, penalty) = getReward(sender,_stakeID);
    ...
    function _unstake(
        ...
        emit UnStaked(sender, cliamable, reward, penalty,_stakeID);
    ...
    function getReward(address account ,uint256 _stakeID)
        public
        view
        returns (uint256 rewardClaimable_, uint256 penalty_)
    {
    ...
    function getApr(uint256 _amount) public view returns (uint8 apr_) {
    ...
    function getStakesInfo(address account, uint256 _stakeID)
        external
        view
        returns (Data memory)
    {
        return stakeStatus[account].stakeDetails[_stakeID];
    }
    ...
    function getStakesIDOf(address account)
        public
        view
        returns (uint256[] memory userStakesIDs)
    {
        uint256 activeStakeCount = 0;
        uint256 len = stakeID[account].length;
        for (uint256 i = 0; i < len; i++) {
            if (stakeID[account][i] != 0) {
                activeStakeCount++;
            }
        }
