...
contract ChainlinkOracle is PriceOracleI, ChainlinkClient, AccessControlEnumerable {
  ...
  function fulfillFloor(bytes32 _requestId, bytes32 _result) external recordChainlinkFulfillment(_requestId) {
    ...
    floorPrices[request.collection] = ResponseFloor(
      request.collection,
      request.pricingAsset,
      request.time,
      dateAndPrice.date,
      dateAndPrice.price * 1e4,
      _requestId
    );
   ...
   */
  function getUnderlyingFloorNFTPrice(address _collectionAddress, uint256 _decimals) external view override returns (uint256) {
    uint256 price = floorPrices[testingCollections[_collectionAddress]].estimate;
  ...
  function getUnderlyingIndividualNFTPrice(
    ...
    uint256 price = individualPrices[testingCollections[_collectionAddress]][_tokenId].estimate;
  ...
  function addPriceFeed(IERC20 _erc20, address _oracle) external {
  ...
  function setTestingMode(bool _testing) external {
    if (!hasRole(DEFAULT_ADMIN_ROLE, msg.sender) && !hasRole(FACTORY, msg.sender)) revert Unauthorized();
    testing = _testing;
  }
   ...
   */
  function addTestingCollection(address _testCollection, address _prodCollection) external {
    if (!hasRole(DEFAULT_ADMIN_ROLE, msg.sender) && !hasRole(FACTORY, msg.sender)) revert Unauthorized();
