...
import "./ScryptaTokenRegistry.sol";
import "./ScryptaERC1155.sol";
import "./ScryptaERC20.sol";

contract ScryptaContract {

//    mapping(address => bool) public externalAccounts;

    ScryptaTokenRegistry public tokenRegistry;

    constructor(ScryptaTokenRegistry _tokenRegisty) {
        tokenRegistry = _tokenRegisty;
    }

    function createToken(uint256 collectionId, address creator, uint256 amount) public {
        ScryptaERC1155 collection = tokenRegistry.collections(collectionId);
...
//
//        require(totalShares <= 10000000, "ScryptaERC1155: shares can not be over 100%");
...
//        safeTransferFrom(to, from, currencyTokenId, currencyTokenAmount - (currencyTokenAmount * totalShares) / 10000000, data);
//
//        // pay the total fee for the payees | to --> creator
//        safeTransferFrom(to, creator, currencyTokenId, (currencyTokenAmount * totalShares) / 10000000, data);
//
//        // pay the shares to the payees | creator --> payees
//        for (uint256 i = 0; i < payees.length; i++)
//            safeTransferFrom(creator, payees[i], currencyTokenId, (currencyTokenAmount * shares[i]) / 10000000, data);
//
...
contract ScryptaTokenRegistry {

    ScryptaERC1155[] public collections;
    ScryptaERC20[] public coins;

    function createCollection(string memory name, string memory symbol, string memory metadataUri) public returns (uint256) {
        ScryptaERC1155 collection = new ScryptaERC1155(name, symbol, metadataUri);
//        collection.transferOwnership(owner());
        collections.push(collection);
        return collections.length-1;
    }

    function createCoin(string memory name, string memory symbol) public returns (uint256) {
...
//
//        // transfer collections to newOwner
//        for (uint256 i=0; i<collections.length; i++)
//            collections[i].transferOwnership(newOwner);
//
//        // transfer coins to newOwner
//        for (uint256 i=0; i<coins.length; i++)
//            coins[i].transferOwnership(newOwner);
//    }
...
contract ScryptaERC20 is ERC20Burnable, ERC20Pausable {
...
contract ScryptaERC1155 is ERC1155Burnable, ERC1155Supply, ERC1155Pausable {
    ...
    constructor(string memory _name, string memory _symbol, string memory _metadataUri) ERC1155(_metadataUri) {
        name = _name;
        symbol = _symbol;
    }
