...
contract TokenVesting is Ownable, AccessControl, ReentrancyGuard, Initializable {
    ...
    */
    ...
    returns(uint256){
        ...
        uint256 vestingEndDate = vestingSchedule.start + vestingSchedule.duration;

        uint256 releasedAmount = vestingSchedule.released;

        uint256 releasableAmount;

        if(vestingSchedule.revoked == true || currentTime < vestingSchedule.cliff) {
            // vesting is revoked of less then cliff
            return 0;
        }
        ...
        if(currentTime > vestingSchedule.start && currentTime < vestingEndDate) {
            // vesting is active
