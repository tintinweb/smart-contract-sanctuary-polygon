...
contract StoryChain is ERC721, ERC721URIStorage, FunctionsClient, ConfirmedOwner {
  ...
  struct Story {
    ...
    bool lastPromptApproved;
    bool updating;
    bool executeRequestSent;
    uint256 storyAIid;
    uint256 imageAIid;
  }
  ...
  mapping(bytes32 => RequestData) public requestToData;
  ...
  uint256 public fixedCost = 200000000000000000;
  mapping(uint256 => uint256) public categoryCost;
  mapping(uint256 => uint256) public storyAICost;
  mapping(uint256 => uint256) public imageAICost;

  uint256 public constant CATEGORY_ALL = 0;
  uint256 public constant CATEGORY_SFW = 1;
  uint256 public constant CATEGORY_KIDS = 2;

  event PromptEntered(string prompt, uint256 nonce, uint256 page, address author);
  ...
  constructor(
    ...
    uint32 _gasLimit
  ) ERC721("StoryChain", "STRY") FunctionsClient(oracle) ConfirmedOwner(msg.sender) {
    source = _source;
    secrets = _secrets;
    gasLimit = _gasLimit;

    categoryCost[1] = 200000000000000000;
    categoryCost[2] = 200000000000000000;
    storyAICost[0] = 500000000000000000;
    storyAICost[1] = 1000000000000000000;
    imageAICost[0] = 200000000000000000;
    imageAICost[3] = 500000000000000000;
  }

  function calculateCost(uint256 category, uint256 storyAIid, uint256 imageAIid) public view returns (uint256 cost) {
    cost = fixedCost + categoryCost[category] + storyAICost[storyAIid] + imageAICost[imageAIid];
  }
  ...
  function createStory(
    ...
    require(msg.value >= calculateCost(category, storyAIid, imageAIid), "cant cover page cost");
    require(category == CATEGORY_ALL || category == CATEGORY_KIDS || category == CATEGORY_SFW, "wrong category");
    Story storage s = stories[nonce];
    s.nonce = nonce;
    s.title = title;
    s.pageLimit = pageLimit;
    s.category = category;
    s.storyAIid = storyAIid;
    s.imageAIid = imageAIid;
    s.creator = msg.sender;
    s.updating = true;

    if (category == CATEGORY_ALL) {
      s.lastPromptApproved = true;
      emit SafetyChecked("0", "autoapproved", "", nonce, 0, msg.sender, prompt);
    } else {
      emit PromptEntered(prompt, nonce, 0, msg.sender);
    }

    nonce++;
  }

  function continueStory(string calldata prompt, uint256 storyNonce) public payable {
    Story storage s = stories[storyNonce];
    require(s.pages > 0, "no pages yet");
    require(s.pageLimit == 0 || s.pages < s.pageLimit, "story is complete");
    require(!s.updating, "waiting for current page");
    require(msg.value >= calculateCost(s.category, s.storyAIid, s.imageAIid), "cant cover page cost");

    s.updating = true;
    if (s.category == CATEGORY_ALL) {
      s.lastPromptApproved = true;
      emit SafetyChecked("0", "autoapproved", "", storyNonce, s.pages, msg.sender, prompt);
    } else {
      s.lastPromptApproved = false;
      s.executeRequestSent = false;
      emit PromptEntered(prompt, storyNonce, s.pages, msg.sender);
    }
  }

  function checkPromptSafety(string calldata prompt, uint256 storyNonce, address author) external onlyOwner {
    Story storage s = stories[storyNonce];
    require(s.updating, "no prompt given");
    require(!s.lastPromptApproved, "already approved");
    require(!s.executeRequestSent, "already checked");
    s.executeRequestSent = true;
    bytes32 reqId = executeRequest(prompt, s.category);
    requestToData[reqId].nonce = storyNonce;
    requestToData[reqId].prompt = prompt;
    requestToData[reqId].author = author;
  }

  function executeRequest(string memory prompt, uint256 category) internal returns (bytes32) {
    Functions.Request memory req;
    ...
    args[0] = prompt;
    args[1] = Strings.toString(category);
    req.addArgs(args);
  ...
  function fulfillRequest(bytes32 requestId, bytes memory response, bytes memory err) internal override {
    ...
    uint256 storyNonce = requestToData[requestId].nonce;
    string memory prompt = requestToData[requestId].prompt;
    address author = requestToData[requestId].author;

    Story storage s = stories[storyNonce];
    bool rejected = keccak256(abi.encodePacked(response)) == keccak256(abi.encodePacked("rejected"));
    if (s.pages == 0 && rejected) {
      s.lastPromptApproved = true;
      s.creationRejected = true;
      s.updating = false;
    } else if (rejected) {
      s.lastPromptApproved = false;
      s.updating = false;
      ...
      s.lastPromptApproved = true;
      emit SafetyChecked(requestId, response, err, storyNonce, s.pages, author, prompt);
    }
  }

  function updateStoryIPFS(
    uint256 storyNonce,
    ...
    Story storage s = stories[storyNonce];
    require(storyPage == s.pages + 1, "Wrong page");
    s.ipfs = ipfsHash;
    s.pages++;
    s.updating = false;
  ...
  function updateSecrets(bytes memory _secrets) external onlyOwner {
    secrets = _secrets;
  }
  ...
  function setCategoryCost(uint256 _category, uint256 _cost) external onlyOwner {
    categoryCost[_category] = _cost;
  }

  function setStoryAICost(uint256 _id, uint256 _cost) external onlyOwner {
    storyAICost[_id] = _cost;
  }

  function setImageAICost(uint256 _id, uint256 _cost) external onlyOwner {
    imageAICost[_id] = _cost;
  }
