...
contract DigicapMarket is Manager, ERC721Holder, ReentrancyGuard {
    ...
    function callBuyerCommission(
        ...
        require(_buyerCommission.tax <= ZOOM_FEE, "Invalid tax");
        (
            uint256 _categoryId,
            uint256 _branchId,
            uint256 _collectionId
        ...
        if (
            ...
            if (_buyerCommission.isReduced) {
                _commission -= (_commission * _buyerCommission.tax) / ZOOM_FEE;
            } else {
                _commission += (_commission * _buyerCommission.tax) / ZOOM_FEE;
            }
        }
        uint256 _commissionFromBuyer = (_buyerCommission.amount * _commission) /
            ZOOM_FEE;
        uint256 amountToBuyer = _buyerCommission.amount + _commissionFromBuyer;
    ...
    function encodeData(
        ...
        return
            keccak256(
                abi.encode(id, _tokenAddress, _paymentToken, _tokenId, _price)
            );
    }
