...
contract RequestMFSContract is Initializable, Proxied, Governed, ReentrancyGuard, IRequestMFSContract {
    ...
    function realizeMFS(uint256 amountMFS) external override returns (uint256) {
        uint256 userId;
        uint256 levelQueue;
        uint256 requestId;
        uint256 amountUSD;
        ...
        while (amountMFS != 0 && requestId != 0) {
            ...
            amountUSDinMFS = core.calcMFSAmountForUSD(amountUSD);
            /*
            ...
            if (toWalletMFS <= amountMFS) {
                ...
                toAirdropPoolMFS = (amountMFS * 2).mul(airdropPoolCoeffs[userLevel - 1]);
                toWalletMFS = amountMFS;
                core.giveMFSFromPool(userId, toWalletMFS);
                emit TokensMFSIsBuyingInRequest(userId, toWalletMFS, requestId);
                uint256 amountMFSinUSD = core.calcUSDAmountForMFS(amountMFS + toAirdropPoolMFS);
...
interface IMetaForceContract {
    ...
    event OutOfGas(
        uint256 userId,
        uint256 lastCheckedRefId,
        bool activate,
        uint256 level,
        uint256 amountMfsLeft,
        bool marketing,
        uint256 counter
    );

    function setDistributionLoopsLimit(uint256 newLimit) external;
    ...
    function distributionLoopsLimit() external view returns (uint256);
}
