...
/// @dev Extiint & Apeiron
/// @title Main contract (Weapon NFT & state)
contract Main is RandomNumberGenerationMain, UUPSUpgradeable, ERC721EnumerableUpgradeable, OwnableUpgradeable {
  ...
  bool initialized;
  mapping(address => address) public players_referral;
  mapping(address => uint256) public players_chests_purchased;
  mapping(address => uint256) public players_referrals_count;
  mapping(address => uint256) public players_referrals_rewards;
  ...
  event CollectedReferralRewards(address indexed player, uint256 chest_rewards);
  ...
  /// @dev If the sender specifies a ref, set it once & count chest purchased by referred.
  function buyChests(address ref) external payable {
    Player storage player = players[_msgSender()];
    (uint256 cost_chest,) = getCosts();
    uint256 count = msg.value / cost_chest;

    require(ref != _msgSender(), "Can't ref yourself.");
    require(player.id != 0, "You need to own a character.");
    require(count >= 1, "One chest minimum.");

    player.chests += count;
    players_chests_purchased[_msgSender()] += count;

    if (ref != address(0x0) && players_referral[_msgSender()] == address(0x0)) { // ref is provided & sender has no ref
      players_referral[_msgSender()] = ref;
    }

    address referral = players_referral[_msgSender()];

    if (referral != address(0x0) && players_chests_purchased[referral] > 0) { // ref is not null & referrer has purchased at least one chest
      unchecked {
        players_referrals_count[referral] += count;
        players_referrals_rewards[referral] += count; // referrer gets rewarded
        players_referrals_rewards[_msgSender()] += count; // self gets rewarded
      }
    }
  ...
  /// @notice collects chest rewarded by the referral system. Referrals rewards both the referrer & the referree.
  /// @notice Rewards for chests are half chest counted from refs.
  /// @dev chest rewards are half of chest purchased by referred accounts.
  function collectReferralChests() external {
    Player storage player = players[_msgSender()];
    uint256 chest_rewards = players_referrals_rewards[_msgSender()] / 2;

    require(chest_rewards > 0, "No chest to collect from referrals.");

    player.chests += chest_rewards;

    players_referrals_rewards[_msgSender()] = 0;

    emit CollectedReferralRewards(_msgSender(), chest_rewards);
  }
  ...
  /// @dev O(n) algorithm, not recommended but clearly essential here, it is an optional function anyway.
  ...
  function getCosts() public pure returns (uint256, uint256) {
    ...
    uint256 cost_upgrade = 0.00004 ether;
