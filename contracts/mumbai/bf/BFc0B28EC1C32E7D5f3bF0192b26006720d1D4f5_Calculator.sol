...
contract Calculator {
    ...
    function calculateTokenReleasedPerFundraise(uint fundraiseId) public view returns (uint) {
        return fundraiseToAmountRaised[fundraiseId].mul(totalTokenReleased).div(tokenRaiseTarget);
    }
    ...
    function calculateTokensProportion(uint fundraiseId, uint tokenStaked) public view returns (uint, uint) {
        uint dailyTokens = calculateDailyTokens(fundraiseId);
        uint soldNFT = calculateNftRaise(fundraiseId);

        uint tokensReleasedStakers = tokenStaked.mul(dailyTokens).div(soldNFT);

        uint remainTokens = dailyTokens.sub(tokensReleasedStakers);
        uint tokensToTreasury = remainTokens.mul(stakingAdjustment).div(100);
