    ...
    UUPSUpgradeable
{
    ...
    // Full amount eg 0.01 usdc
    uint256 private mintFee;

    address private receiverWallet;

    mapping(uint256 => Whitelist) private whitelist;
    ...
    function initialize(uint256 _mintFee, address _receiverWallet)
        public
        initializer
    {
        __ERC721_init("Wisdom Land Premium User", "WIZ");
    ...
    function safeMint(uint256 tokenId) external payable {
        Whitelist memory _whitelist = whitelist[tokenId];
        require(_whitelist.user == msg.sender, "Sender not whitelisted");
        require(msg.value >= mintFee, "Mint fee not payed");

        _safeMint(msg.sender, tokenId);
        _setTokenURI(tokenId, _whitelist.uri);

        delete whitelist[tokenId];

        (bool sent, ) = receiverWallet.call{value: msg.value}("");
    ...
    function safeMintOwner(
        ...
        require(tokenId > 0, "ID cant be 0");
    ...
    function setReceiver(address _receiverWallet) external onlyOwner {
        receiverWallet = _receiverWallet;
    }
    ...
    function setWhitelist(
        ...
        require(whitelist[id].user == address(0), "Token already whitelisted");

        whitelist[id].user = user;
        whitelist[id].uri = uri;
    }
}
