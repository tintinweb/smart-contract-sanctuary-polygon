...
contract TreasuryTracker is AddressAccessor, ITreasuryTracker {
    ...
    function addValueAfterPurchase(
        ...
        if (notInEscrow) {
            ...
            value = _convertToCorrectDecimals(
                value,
                ptDecimals,
                IERC20Metadata(underlying).decimals()
            );
            currencyInfo[currency].valueInNativeCurrencyEscrow += value;
        }
    }
        ...
        )
    {
        ...
        if (usdValue != 0) {
            // oracleDecimals should be 3
            usdValue = convertPriceToUSDCustom(token, usdValue, oracleDecimal);
        }
    }
