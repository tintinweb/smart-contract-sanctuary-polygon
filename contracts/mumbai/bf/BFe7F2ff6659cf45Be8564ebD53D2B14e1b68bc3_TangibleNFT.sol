    ...
    IERC721Receiver
{
    ...
    function _produceTNFTtoStock(
        ...
        if (paysRent) {
            ...
            AccessControl(address(rentRevenueShare_)).grantRole(
                SHARE_MANAGER_ROLE,
                address(this)
            );
            AccessControl(address(rentRevenueShare_)).grantRole(
    ...
    function setRolesForFraction(address ftnft, uint256 tnftTokenId)
        ...
        onlyFactory
    {
        RevenueShare rentRevenueShare = IFactory(factory).rentShare().forToken(
            address(this),
            tnftTokenId
        );
        AccessControl(address(rentRevenueShare)).grantRole(
            SHARE_MANAGER_ROLE,
            ftnft
        );
        AccessControl(address(rentRevenueShare)).grantRole(CLAIMER_ROLE, ftnft);
    }
    ...
    function _destroyTNFT(uint256 tokenId, address burningFrom) internal {
        ...
        bool ownerIsFactAdmin = IFactory(factory).isFactoryAdmin(tokenOwner);
        bool burnerIsFactAdmin = IFactory(factory).isFactoryAdmin(burningFrom);
        bool ownerWantsToBurn = tokenOwner == burningFrom;
        bool ownerAllowedToBurn = ownerIsFactAdmin && ownerWantsToBurn;
        // bool ownerIsMinter = _isTokenMinter(tokenOwner, tokenId);
        bool storageTimePassed = (storageEndTime[tokenId] + 180 days) <
            block.timestamp;

        if (
            ownerAllowedToBurn ||
            (!storageRequired && burnerIsFactAdmin) ||
            (storageTimePassed && burnerIsFactAdmin)
        ) {
            _burn(tokenId);
            delete _originalTokenOwners[tokenId];
            // delete tokenBrand[tokenId];
            delete tokensFingerprint[tokenId];
            //pinft handling if we have it locked
            if (tnftToPassiveNft[tokenId] != 0) {
                PassiveIncomeNFT piNft = IFactory(factory).passiveNft();
