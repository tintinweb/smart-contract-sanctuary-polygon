...
interface ICraft{    
	...
	function nftPayment(address from, uint256 amount) external returns (bool);	
...
contract GrandMaster is ERC721Enumerable, Ownable {
    ...
    function mintNFTs() public payable {
		bool transferred = ICraft(token).nftPayment(
            _msgSender(),
            uint256(2000)
        );
		require(transferred, "NFT Mint: You have no 2000 Craft");
        _mintNFTs(1);
    }
