...
contract HelloBlockchain {
    ...
    function SendStruct(MyStruct[] memory data) public returns (uint256 numOfEntries) {
        numOfEntries = data.length;
        if (numOfEntries > 0)
        {
            MyStructData.uint8Value = data[0].uint8Value;
            uint8Value = data[0].uint8Value;
            MyStructData.uint256Value = data[0].uint256Value;
            uint256Value = data[0].uint256Value;
            MyStructData.stringValue = data[0].stringValue;
            stringValue = data[0].stringValue;
        }
    }
}