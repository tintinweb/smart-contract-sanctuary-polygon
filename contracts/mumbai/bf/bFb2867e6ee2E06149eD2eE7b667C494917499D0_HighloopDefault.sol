...
// 700 - supply
// 400000000000000000 - price in gwei
// 0x81D86fAf587daEA18497103666dCFD8F369d61c8 - wallet to withdrawn funds
// 10 - max tx wallet
// 3 - max per tx
// true - whitelist active
// Collection_Name - name of collection
// HIGHLOOP-COLLECTION_NAME - ethereum slug
// https://metadata.highloop.io/collection/ - metadata URI

contract HighloopDefault is IHighloopDefault, ERC721A, Ownable, ReentrancyGuard {


    using SafeMath for uint256;
    using Strings for uint256;

    uint256 public nftTotalSupply = 0;
    ...
    // Limit the number of NFTs that can be minted per wallet
    mapping(address => uint256) public purchaseTxs;
    uint8 public maxPerWallet;

    // Limit the number of NFTs that can be minted per transaction
    ...
    modifier onlyWithdrawalWallet() {
        require(
            address(withdrawalWallet) == _msgSender(),
            "Exclusible: caller is not the withdrawal wallet"
        );
        _;
    }

    function changeSupplyLimit(
        uint256 _supplyLimit
    ...
    function toggleSaleActive() external override onlyOwner {
    ...
    function setSmartContractAllowance(
        bool _smartContractAllowance
    ) external override onlyOwner {
    ...
    function setMaxPerWallet(uint8 _maxPerWallet) external override onlyOwner {
        maxPerWallet = _maxPerWallet;
    }

    function setMaxPerTx(uint8 _maxPerTx) external override onlyOwner {
        maxPerTx = _maxPerTx;
    }

    function toggleWhitelistActive() external override onlyOwner {
        whitelistActive = !whitelistActive;
        emit ToggleWhitelistState(whitelistActive);
    }
    ...
    function reserve(uint256 _amount) external onlyOwner {
        require(
            nftTotalSupply.add(_amount) <= supplyLimit,
            "Not enough NFT token left."
        );
     ...
     /**
     * @dev Mint NFT token
     * @param _amount  Amount of NFT token to mint
     */
    function buy(uint256 _amount, bytes32[] calldata merkleProof ) external payable nonReentrant{
        // Check if the sale is active
        require(saleActive, "Exclusible::buy: Sale is not active.");
      
        // Check if the user exceeds the max NFT token per transaction
        require(
            _amount <= maxPerTx,
            "Exclusible::buy: Exceeds max NFT token per transaction."
        );

        // Check if the user exceeds the max NFT token per wallet
        require(
            purchaseTxs[msg.sender].add(_amount) <= maxPerWallet,
            "Exclusible::buy: Exceeds max NFT token per wallet."
        );

        // Check if the user has enough ETH to buy the NFT token
        require(
            msg.value >= mintPrice.mul(_amount),
            "Exclusible::buy: Insufficient payment."
        );

        if (!smartContractAllowance) {
            require(
                tx.origin == msg.sender,
                "Exclusible::buy: Smart contracts are not allowed to buy."
            );
        }
        ...
        require(
            whitelistSaleActive,
            "Exclusible::buy: Whitelist sale is not active."
        );

            require(
                MerkleProof.verify(merkleProof, merkleRoot, node),
                "Exclusible::buy: Address is not whitelisted - Merkle Invalid proof."
            );
        }
    ...
    function _mint(uint256 _amount) internal {
        ...
        nftTotalSupply = nftTotalSupply.add(_amount);
    ...
    function editMintPrice(uint256 _price) external onlyOwner {
        mintPrice = _price;
    }

    /**
     * @dev Mint NFT token for crossmint
     * @param to  Address to mint NFT token
     * @param _count  Amount of NFT token to mint 
     */
    function mintTo(address to, uint _count) external payable {
        ...
        require(
            nftTotalSupply.add(_count) <= supplyLimit,
            "Not enough NFT token left."
        );

        nftTotalSupply = nftTotalSupply.add(_count);

        _safeMint(to, _count);
    }

    /**
     * @dev Mint NFT token only for owner (private mint)
     * @param to  Address to mint NFT token
     * @param _count  Amount of NFT token to mint 
     */
    function mintToPrivate(address to, uint _count) external onlyOwner {
        // Optional Restriction
        require(
            nftTotalSupply.add(_count) <= supplyLimit,
            "Not enough NFT token left."
...
interface IHighloopDefault {
    ...
    function setMaxPerWallet(uint8 _maxTxValue) external;

    function setMaxPerTx(uint8 _maxPerTx) external;

    function toggleWhitelistActive() external;
    ...
    function smartContractAllowance() external view returns (bool);

    function withdrawalWallet() external view returns (address payable);
    ...
    event ToggleWhitelistState(bool _whitelistActive);
...
// SPDX-License-Identifier: MIT
// Creator: Chiru Labs
// Further optimization: Nefture + Exclusible
...
contract ERC721A is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function safeTransferFrom(
        ...
        _transfer(from, to, tokenId);
        require(_checkOnERC721Received(from, to, tokenId, _data), "AP10");
    }
     ...
     */
    function _beforeTokenTransfers(address from, address to) internal virtual {}
}
