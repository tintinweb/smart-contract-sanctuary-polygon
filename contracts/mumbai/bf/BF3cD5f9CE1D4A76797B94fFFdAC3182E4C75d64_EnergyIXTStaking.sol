...
interface IEnergyIXTStaking {
     ...
     * @param totalEnergyStaked The amount of tokens staked
     * @param userRewardPerTokenPaid The rewards paid out
     * @param rewards The rewards accumulated to be paid out
     * @param ameliaRewards Rewards a user has earned for committing to amelia fund
     * @param pushedRewards The rewards pushed to next epoch
     */
    struct RewardPool {
     ...
     */
    function stakeEnergy(uint256 _amount, bool _ameliaCommitment) external;
     ...
     * @param _ameliaCommitment Option to donate portion of earnings to Amelia Foundation
     * @notice emit {RewardClaimed} event
     */
    function claim(bool _ameliaCommitment) external;
...
contract EnergyIXTStaking is IEnergyIXTStaking, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function stakeIXT(uint256 _amount) external nonReentrant {
        require(_amount > 0, "ENERGY STAKING: CANT STAKE 0");
        userStakes[msg.sender].push(Stake({amount: _amount, stakedAt: block.timestamp}));
    ...
    function stakeEnergy(uint256 _amount, bool _ameliaCommitment) external nonReentrant {
        ...
        s_rewardPool.totalEnergyStaked += _amount;
    ...
    function unstakeOldIXT(uint256 _amount) external nonReentrant {
        require(_amount > 0, "ENERGY STAKING: CANT UNSTAKE 0");
        require(userStakedAmount[msg.sender] >= _amount, "USER DOES NOT HAVE ENOUGH IXT STAKED");

        userStakedAmount[msg.sender] -= _amount;
        pixt.transfer(msg.sender, _amount * 1 ether);
    }

    function unstakeOldAmeliaIXT(uint256 _amount) external nonReentrant {
        require(_amount > 0, "ENERGY STAKING: CANT UNSTAKE 0");
        require(userAmeliaCommitment[msg.sender] >= _amount, "USER DOES NOT HAVE ENOUGH IXT STAKED");
    ...
    function claim(bool _ameliaCommitment) external nonReentrant {
        if (_ameliaCommitment) require(userEnergyStakedAmelia[msg.sender][s_epochCount] > 0, "ENERGY STAKING: INSUFFICIENT AMOUNT COMMITTED");
        else require(userEnergyStaked[msg.sender][s_epochCount] > 0, "ENERGY STAKING: INSUFFICIENT AMOUNT STAKED");

        _updateRewards(msg.sender);

        _proportionAndTransferRewards(_ameliaCommitment);
    }

    /*---------------- STAKING UTILS ----------------*/

    function _updateStake(address _user, uint256 _amount) internal {
        Stake[] memory userStaked = userStakes[_user];
        ...
        for (uint256 i; i < userStaked.length; i++) {
            if (block.timestamp > (userStaked[i].stakedAt + LOCKUP_PERIOD)) {
                if (remaining >= userStaked[i].amount) {
                    ...
                    _removeFromArray(0, userStakes[_user]);
                } else {
                    userStakes[_user][0].amount -= remaining;
                    remaining = 0;
                }
            }
        }
    ...
    function _proportionAndTransferRewards(bool _ameliaCommitment) internal {
        uint256 rewardToPay;
        if (_ameliaCommitment) {
    ...
    function _updateRewards(address _walletAddress) internal {
        ...
        if (_walletAddress != address(0)) {
            ...
            s_rewardPool.ameliaRewards[_walletAddress] = earnedAmelia(_walletAddress);
            s_rewardPool.userRewardPerTokenPaid[_walletAddress][s_epochCount] = s_rewardPool.rewardPerTokenStored;
        }
    }

    /// @inheritdoc IEnergyIXTStaking
    function earned(address _walletAddress) public view returns (uint256) {
        return (userEnergyStaked[_walletAddress][s_epochCount] * (rewardPerTokenStored() - s_rewardPool.userRewardPerTokenPaid[_walletAddress][s_epochCount])) + s_rewardPool.rewards[_walletAddress];
    }

    function earnedAmelia(address _walletAddress) public view returns (uint256) {
        return
            (userEnergyStakedAmelia[_walletAddress][s_epochCount] * (rewardPerTokenStored() - s_rewardPool.userRewardPerTokenPaid[_walletAddress][s_epochCount])) +
            s_rewardPool.ameliaRewards[_walletAddress];
    }
    ...
    function rewardPerTokenStored() public view returns (uint256) {
        if (s_rewardPool.totalEnergyStaked == 0) {
        ...
        return s_rewardPool.rewardPerTokenStored + (reward / s_rewardPool.totalEnergyStaked);
    }
    ...
    function addCurrentEpochRewards(uint256 _poolTotalReward, uint256 _epochDuration) external onlyRewardDistributor {
        ...
        s_rewardPool.totalEnergyStaked = 0;
        s_rewardPool.rewardPerTokenStored = 0;
    ...
    function getTotalStakedAmount() external view returns (uint256) {
        return s_rewardPool.totalEnergyStaked;
    }
    ...
    function getUserRewardPerTokenPaid(address _walletAddress) external view returns (uint256) {
        return s_rewardPool.userRewardPerTokenPaid[_walletAddress][s_epochCount];
    }
