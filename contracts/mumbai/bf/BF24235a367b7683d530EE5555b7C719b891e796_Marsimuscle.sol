...
contract Marsimuscle is RiskyGame {
  ...
  constructor(address _bootcampPlayer, address _reddies, address _oracle)
    ...
    {
        baseRate: 29491,
        riskyReward: 70 ether,
        safeReward: 25 ether
    }
    ));
  }

  function traitBoost(uint256 tokenId) public view override returns (uint32) {
      Metadata memory metadata = bootcampPlayer.getPlayerMetadata(tokenId);
      return metadata.skillLevel == 0 ? 13107 : 0;
  }
  ...
  function attributeBoost(uint256 tokenId) public view override returns (uint32) {
      ...
      return uint32(uint256(stats.strength) * 1311);
  }
}
...
//import "@openzeppelin/contracts/utils/structs/Queue.sol";
//import "@openzeppelin/contracts/utils/math/SafeCast.sol";
...
abstract contract RiskyGame is Game {
  ...
  function _play(uint256 tokenId) internal virtual {
  ...
  // uint256 gas = gasleft();
  // gas -= gasleft();
  // payable(msg.sender).transfer(block.basefee * gas);  // priority fee not refunded to prevent miner exploits
  ...
  function playRisky(uint16[] calldata tokenIds) public virtual duringGame originCheck whenNotPaused {
    ...
    _revealMany(max(20, tokenIds.length));
