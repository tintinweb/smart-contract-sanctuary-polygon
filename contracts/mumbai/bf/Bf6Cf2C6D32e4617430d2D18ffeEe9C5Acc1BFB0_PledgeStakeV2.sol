...
import "@openzeppelin/contracts/utils/math/SafeMath.sol";
 contract PledgeStakeV2 {
    ...
    // Calculate level rewards for the user's upline
    // Calculate level rewards for the user's upline
    function getLevelReward(address _user) public view returns (uint256 levelReward) {
        UserInfo storage user = userInfo[_user];
        address upline = user.referrer;
        levelReward = 0;
        if (upline != defaultReferrer){
            //here we will check the level of the upline and having a non zero stake
            for (uint256 i = 0; upline != address(0) && i < LEVEL_COUNT; i++) {
                uint256 roundRewrd = getTotalRoundRewards(_user);
                uint256 levelRewardPercentage = levelPercentage[i];
                if (userInfo[upline].level > i && userInfo[upline].totalStake > 0) {
                    levelReward = roundRewrd.mul(levelRewardPercentage).div(100);
