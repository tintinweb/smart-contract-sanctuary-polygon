...
contract RevvBadges is ERC1155, Ownable, Pausable, ERC1155Supply, Permissions {
    using Counters for Counters.Counter;

    event LogTokenCreation(uint256 tokenId);
    ...
    // NOTE :: Associate a token id with an token's details || token_id => Token
    mapping(uint256 => Token) private _tokenMapping;
    ...
    function updateName(string memory newName) public onlyAdmins {
        _name = newName;
    }
    ...
    function updateSymbol(string memory newSymbol) public onlyAdmins {
        _symbol = newSymbol;
    }
    ...
    function uri(uint256 id) public view virtual override returns (string memory) {
        ...
        Token memory token = _tokenMapping[id];
        return string(abi.encodePacked(super.uri(id), token.hash));
    }
    ...
    function createToken(string memory hash, uint256 expiry) public onlyAdmins {
        bytes32 _hashBytes = bytes32(bytes(hash));
        uint256 id = _tokenIdCounter.current();
        _tokenMapping[id] = Token(_hashBytes, expiry);
        emit LogTokenCreation(id);

        _tokenIdCounter.increment();
    }

    function updateToken(uint256 id, string memory hash, uint256 expiry) public onlyAdmins {
        bytes32 _hashBytes = bytes32(bytes(hash));
        _tokenMapping[id] = Token(_hashBytes, expiry);
    }

    function mint(address account, uint256 id) public onlyAdmins {
        _mint(account, id, 1, "");
    }
    ...
    function mintBatchToken(address[] memory tos, uint256 id) public onlyAdmins {
    ...
    function burnBatch(address account, uint256[] memory ids) public virtual {
        require(
            account == _msgSender() || isApprovedForAll(account, _msgSender()),
            "ERC1155: caller is not token owner or approved"
        );
    ...
    function _beforeTokenTransfer(
        ...
        if (from != address(0) && to != address(0)) {
            require(!!_isAdmin(msg.sender), "Tokens can only be transferred with the approval of the contract owner.");
            require(!!_allowedWallets[from], "Wallet owner hasn't enable tokens to be transferred.");
        }
    ...
    function getTokenDetails(uint256 id) public view returns (ReadableToken memory) {
        Token memory token = _tokenMapping[id];
        return ReadableToken(uri(id), token.expiry);
    }

    function isMintable(address account, uint256 id) public view returns (bool) {
        require(id < _tokenIdCounter.current(), "Token does not exist.");
        require(block.timestamp <= _tokenMapping[id].expiry, "Minting window has expired.");
        require(super.balanceOf(account, id) == 0, "Wallet already holds token.");
        return true;
    }
}