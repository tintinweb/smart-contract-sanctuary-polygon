...
contract Posts {
    ...
    */
    function getMsgIDs(address usrAddress, uint256 startFrom) public view returns(uint256[] memory) {
        ...
        if (startFrom == 0){
            startFrom = ((postsMap[KUtils.append(usrAddressStr,'-',KUtils.toString(bucketKeyID),'','')].length) + (bucketKeyID * maxItemsPerBucket));
            if (startFrom != 0){
                startFrom -= 1;
            } else {
                // It's empty, so end
        ...
        for (uint i=0; i <= bucketKeyID; i++) {
            ...
            if (startFrom >= (i * maxItemsPerBucket) && startFrom <= ((postsMap[_bucketKey].length) + (i * maxItemsPerBucket))) {
                bucketKeyID = i;
    ...
    function removePost(uint256 msgID, address addressPoster) public onlyAdmins {
        // Stringify the address
        string memory addressPosterStr = KUtils._toLower(KUtils.addressToString(addressPoster));
    ...
    function addPost(uint256 msgID, address addressPoster) public onlyAdmins {
        // Stringify the address
        string memory addressPosterStr = KUtils._toLower(KUtils.addressToString(addressPoster));
