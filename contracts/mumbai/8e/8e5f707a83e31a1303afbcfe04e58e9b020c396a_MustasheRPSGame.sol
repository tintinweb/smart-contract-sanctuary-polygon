...
contract MustasheRPSGame {
    ...
    function joinGame(uint256 _gameId) external {
        ...
        require(msg.sender != game.creator, "You are the creator of this game");
    ...
    function play(uint256 _gameId, string memory _choice) external {
        ...
        require((game.creator == msg.sender && game.opponent != address(0)) || (game.opponent == msg.sender && game.creator != address(0)), "You are not part of the game");
    ...
    for (uint256 i = 1; i <= gameId; i++) {
        ...
        if (game.opponent == address(0) || !game.gameEnded) {
            activeGamesCount++;
        }
    }

    uint256[] memory activeGameIds = new uint256[](activeGamesCount);
    address[] memory activeCreators = new address[](activeGamesCount);
    address[] memory activeOpponents = new address[](activeGamesCount);
    uint256[] memory activeBetAmounts = new uint256[](activeGamesCount);
    uint256[] memory activeWins1 = new uint256[](activeGamesCount);
    uint256[] memory activeWins2 = new uint256[](activeGamesCount);
    ...
    for (uint256 i = 1; i <= gameId; i++) {
        ...
        if (game.opponent == address(0) || !game.gameEnded) {
            ...
            activeCreators[index] = game.creator;
            activeOpponents[index] = game.opponent;
            activeBetAmounts[index] = game.betAmount / 10**18;
            activeWins1[index] = game.wins[game.creator];
            activeWins2[index] = game.wins[game.opponent];
            index++;
        }
    }

    return (activeGameIds, activeCreators, activeOpponents, activeBetAmounts, activeWins1, activeWins2);
}
