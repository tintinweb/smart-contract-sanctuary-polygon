...
contract PaymentReceiver is IAccessControlHolder, IPaymentReceiver {
    ...
    function _esnureHasGemsTraderRole(address _wallet) internal view {
        if (!acl.hasRole(GEMS_TRADER_ROLE, _wallet)) {
            revert NotAllowedToExchangeGems(_wallet);
        }
    }

    modifier deadlineIsNotMissed(uint256 _deadline) {
