...
contract LoadHealersHut is ReentrancyGuard, Ownable, EIP712 {
    using ECDSA for bytes32;
    using SafeERC20 for IERC20;

    event Heal(address indexed buyer, uint256 indexed tokenId, string indexed healId);
    event Resurrect(address indexed buyer, uint256 indexed tokenId, string indexed ressurectId);
    ...
    function heal(
        uint256 healAmount,
        uint256 tokenId,
        string calldata healId,
        bytes calldata signature,
        string calldata nonce
    ) external nonReentrant {
        require(
            matchAddresSigner(hashTransactionHeal(msg.sender, healAmount, tokenId, healId, nonce), signature),
            "DIRECT_HEAL_DISALLOWED"
        );
        ...
        uint256 costToHeal = priceHandler.totalCostHeal(healAmount);

        IERC20 dcauContract = IERC20(DCAU_TOKEN_CONTRACT);

        IRewardReceiver rewardReceiver = IRewardReceiver(REWARD_RECEIVER_CONTRACT);

        dcauContract.safeTransferFrom(msg.sender, REWARD_RECEIVER_CONTRACT, costToHeal);
        rewardReceiver.receiveTokens(DCAU_TOKEN_CONTRACT, costToHeal);

        emit Heal(msg.sender, tokenId, healId);
    }

    function resurrect(
        uint256 level,
        uint256 tokenId,
        string calldata resurrectId,
        bytes calldata signature,
        string calldata nonce
    ) external nonReentrant {
        require(
            matchAddresSigner(hashTransactionResurrect(msg.sender, level, tokenId, resurrectId, nonce), signature),
            "DIRECT_RES_DISALLOWED"
        );
        require(tokenId > 0, "Token ID greater than 0");
        require(!_usedNonces[nonce], "HASH_USED");
        ...
        uint256 costToRes = priceHandler.costToResurrect(level);
        ...
        emit Resurrect(msg.sender, tokenId, resurrectId);
    }

    function hashTransactionHeal(
        address sender,
        uint256 healAmount,
        uint256 tokenId,
        ...
        bytes32 hash = _hashTypedDataV4(
            keccak256(
                abi.encode(
                    keccak256("Heal(address sender,uint256 healAmount,uint256 tokenId,string actionId,string nonce)"),
                    sender,
                    healAmount,
                    tokenId,
    ...
    /**
     * @dev Creates hash for the resurrect function to match the signature.
     */
    function hashTransactionResurrect(
        address sender,
        uint256 level,
        uint256 tokenId,
        string calldata resurrectId,
        ...
        bytes32 hash = _hashTypedDataV4(
            keccak256(
                abi.encode(
                    keccak256("Resurrect(address sender,uint256 level,uint256 tokenId,string resurrectId,string nonce)"),
                    sender,
                    level,
                    tokenId,
                    keccak256(bytes(resurrectId)),
                    keccak256(bytes(nonce))
                )
            )
        );
