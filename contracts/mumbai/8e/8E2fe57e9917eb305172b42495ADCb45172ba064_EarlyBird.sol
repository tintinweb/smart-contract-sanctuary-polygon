...
contract EarlyBird is Ownable {
    ...
    function withdraw(address _receiver) external onlyOwner {
        require(_receiver != address(0), "Cannot transfer to zero address");

        uint256 amountInEnj = IERC20(enjToken).balanceOf(address(this));
        uint256 amountInEfi = IERC20(efiToken).balanceOf(address(this));

        IERC20(enjToken).transfer(_receiver, amountInEnj);
        IERC20(efiToken).transfer(_receiver, amountInEfi);

        emit TokensWithdrawn(msg.sender, _receiver, amountInEfi, amountInEnj);
    }
}