    ...
    AccessControlUpgradeable
{
    ...
    */
    function mintMechIndividually(
        ...
        require(
            _isWhitelisted(_proof, _leaf) == true,
            "mintMech: address not whitelisted"
        );
    ...
    */
    ...
    // set root
    function setMerkelRoot(bytes32 _root) external onlyRole(ADMIN_ROLE) {
        _merkelRoot = _root;
    }

    // is address whitelisted or not
    function _isWhitelisted(bytes32[] memory _proof, bytes32 _leaf)
        internal
        view
        returns (bool)
    {
        return MerkleProofUpgradeable.verify(_proof, _merkelRoot, _leaf);
    }
