...
contract AuctionWithAdmin is Ownable {
    ...
    function createAuction(
        ...
        emit AuctionCreated(curr_auction_id, msg.sender, _tokenId, _price, _startTime, _endTime);
    }
    ...
    function auctionClaim(uint256 _tokenId) public {
        require(
            auction[_tokenId].highestBidder != msg.sender || auction[_tokenId].endTime < block.timestamp,
    ...
    function sell(uint256 _tokenId, uint256 _price) public {
        ...
        curr_sale_id++;
        tokenIdToSaleId[_tokenId] = curr_sale_id;
        EnumerableMap.set(saleId, _tokenId, msg.sender);
        saleTokenIds[msg.sender].push(_tokenId);
        nft_token.transferFrom(msg.sender, address(this), _tokenId);
        emit Sell(curr_sale_id, msg.sender, _tokenId, _price, block.timestamp);
    }
