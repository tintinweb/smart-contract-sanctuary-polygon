...
contract Factory1155 {
	...
	function deploy(bytes32 _salt, string memory name, string memory symbol, string memory tokenURIPrefix) external returns(address addr) {
		addr = address(new HeftyverseUserToken1155{salt: _salt}(name, symbol, tokenURIPrefix));
		HeftyverseUserToken1155 token = HeftyverseUserToken1155(address(addr));
...
contract HeftyverseUserToken1155 is ERC1155 {
