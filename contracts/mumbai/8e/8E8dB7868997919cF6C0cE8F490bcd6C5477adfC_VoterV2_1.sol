...
contract VoterV2_1 is IVoter, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function claimBribes(address[] memory _bribes, address[][] memory _tokens, uint256 _tokenId) external {
        ...
        IVotingEscrow(_ve).onRewardsClaimed(_tokenId, false);
    }
    ...
    function _rolloverFees(IBribe _bribe, address _token, uint256 _lastTimestamp, uint256 _currentTimestamp) internal {
        uint256 _amount = _bribe.skim(_token, _lastTimestamp);
        if (_amount != 0) {
            IERC20Upgradeable(_token).approve(address(_bribe), _amount);
            _bribe.notifyRewardAmount(_token, _currentTimestamp);
        }
    }

    function _rolloverFees(address _gauge, uint256 _lastTimestamp, uint256 _currentTimestamp) internal {
        IBribe _bribe = IBribe(internal_bribes[_gauge]);
        IPair _pool = IPair(poolForGauge[_gauge]);
        (address _token0, address _token1) = _pool.tokens();
        _rolloverFees(_bribe, _token0, _lastTimestamp, _currentTimestamp);
        _rolloverFees(_bribe, _token1, _lastTimestamp, _currentTimestamp);
    }
    ...
    function distribute(address _gauge) public nonReentrant {
        ...
        if (lastTimestamp < currentTimestamp) {
            ...
            _rolloverFees(_gauge, lastTimestamp, currentTimestamp);
        }
    }
