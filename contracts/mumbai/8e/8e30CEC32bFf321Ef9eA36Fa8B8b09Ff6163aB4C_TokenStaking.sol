...
// will be transferred to a governance smart contract once SBC is sufficiently
...
contract TokenStaking is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of SBCs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accSBCPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accSBCPerShare` (and `lastRewardStamp`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardStamp;  // Last block timestamp that SBCs distribution occurs.
        uint256 accSBCPerShare;   // Accumulated SBCs per share, times 1e12. See below.
    ...
    // The SBC TOKEN!
    SBCToken public sbc;
    // Bonus muliplier for early sbc makers.
    ...
    // The block timestamp when SBC mining starts.
    ...
    // View function to see pending SBCs on frontend.
    function pendingSBC(uint256 _pid, address _user) external view returns (uint256) {
        require(rewardContract != address(0x00));

        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];

        uint256 accSBCPerShare = pool.accSBCPerShare;
        ...
        if (block.timestamp > pool.lastRewardStamp && lpSupply != 0) {
            ...
            uint256 rewardPoolBal = sbc.balanceOf(rewardContract);
            uint256 sbcPerSecond = rewardPoolBal.div(365).div(24).div(3600);
            uint256 sbcReward = multiplier.mul(sbcPerSecond);

            accSBCPerShare = accSBCPerShare.add(sbcReward.mul(1e12).div(lpSupply));
        }
        uint256 pending = user.amount.mul(accSBCPerShare).div(1e12).sub(user.rewardDebt);
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accSBCPerShare = pool.accSBCPerShare.add(sbcReward.mul(1e12).div(lpSupply));
        pool.lastRewardStamp = block.timestamp;
    }

    // Deposit LP tokens to TokenStaking for SBC allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        uint256 pending = user.amount.mul(pool.accSBCPerShare).div(1e12).sub(user.rewardDebt);
        if (pending > 0 || user.rewardLockedUp > 0) {
            ...
            // send rewards
            safeSBCTransferFrom(rewardContract, msg.sender, totalRewards);
        ...
        user.rewardDebt = user.amount.mul(pool.accSBCPerShare).div(1e12);
    ...
    // Safe sbc transfer function, just in case if rounding error causes pool to not have enough SBC.
    function safeSBCTransferFrom(address _from, address _to, uint256 _amount) internal {
        uint256 sbcBal = sbc.balanceOf(_from);
        if (_amount > sbcBal) {
            sbc.transferFrom(_from, _to, sbcBal);
        } else {
            sbc.transferFrom(_from, _to, _amount);
        }
    }
