...
contract MaykNFTFactory is Ownable {
    mapping (address=>address) private contracts;
    mapping (uint256=>address) private artists;
    using Counters for Counters.Counter;
    Counters.Counter private _artistId;

    function getArtistToken(address artistAddress_) public view onlyOwner returns (address) {
        return contracts[artistAddress_];
    }

    function setArtistToken(
        address artistAddress_,
        address contractAddress
    ) internal onlyOwner {
        require(contracts[artistAddress_] == address(0), "Must not be set");
        _artistId.increment();
        contracts[artistAddress_] = contractAddress;
        artists[_artistId.current()] = artistAddress_;
    }
    ...
    function createArtistToken(
        ...
        require(contracts[artistAddress_] == address(0), "Must not be set");
        MaykNFT artistToken = new MaykNFT(tokenName, contractURI_);
        setArtistToken(artistAddress_, address(artistToken));
    }
    ...
    function mint(
        ...
        address token = getArtistToken(artistAddress_);
        require(token != address(0), "Must already have an artist token");
        MaykNFT nft = MaykNFT(token);
        nft.mint(tokenURI_, artistAddress_, royaltyAddress);
    }
    ...
    function transferTokenOwnership(
        ...
        require(tokenAddress != address(0) && newOwner != address(0), "Must already have an artist token");
        MaykNFT nft = MaykNFT(tokenAddress);
        require(nft.owner() == address(this), "Must be the owner!");
        nft.transferOwnership(newOwner);
    }

    function artistAddress(uint256 artistId) public view returns (address) {
        require(artistId > 0 && artistId <= _artistId.current(), "Invalid artist ID");
        return artists[artistId];
    }

    function nextArtistId() public view returns (uint256) {
        return _artistId.current() + 1;
    }
}
...
contract MaykNFT is ERC721URIStorage, ERC721Royalty, ContextMixin, Ownable {
    using Counters for Counters.Counter;
    ...
    constructor(
        string memory tokenName,
        string memory contractURI_
    ) ERC721(tokenName, "MAYK") {
        _contractURI = contractURI_;
        _setDefaultRoyalty(msgSender(), 1000);
    }
    ...
    function mint(
        ...
        _tokenId.increment();
        _safeMint(artistAddress_, _tokenId.current());
        _setTokenURI(_tokenId.current(), tokenURI_);
        _setTokenRoyalty(_tokenId.current(), royaltyHolder, 1000);
    }
    ...
    function isApprovedForAll(
        ...
        return OpenSeaGasFreeListing.isApprovedForAll(_owner, _operator)
