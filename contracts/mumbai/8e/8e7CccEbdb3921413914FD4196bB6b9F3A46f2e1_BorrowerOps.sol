...
interface IActivePool is IPool {
    ...
    // --- Functions ---
    function sendROSE(IaMATICToken amatic_Token, address _account, uint _amount) external;
}
...
interface IBorrowerOps {
    ...
    function claimCollateral(IaMATICToken amaticToken) external;
...
contract BorrowerOps is OrumBase, Ownable, CheckContract, IBorrowerOps {
    ...
    function openVault(uint _maxFeePercentage, uint _OSDAmount, address _upperHint, address _lowerHint) external payable override {
        ContractsCache memory contractsCache = ContractsCache(vaultManager, activePool, osdToken, amatic_Token);
        ...
        // ICR is based on the composite debt, i.e. the requested OSD amount + OSD gas comp.
    ...
    */
    ...
    function closeVault() external override {
        ...
        activePoolCached.sendROSE(amatic_Token, msg.sender, coll);
    }

    /**
     * Claim remaining collateral from a redemption or from a liquidation with ICR > MCR in Recovery Mode
     */
    function claimCollateral(IaMATICToken amaticToken) external override {
    ...
    (
        ...
        uint _netDebtChange,
        uint _borrowFee,
        IaMATICToken _amatic_Token
    )
        internal
    {
        if (_isDebtIncrease) {
            _withdrawOSD(_activePool, _osdToken, _borrower, _OSDChange, _netDebtChange);
            _activePool.sendROSE(amatic_Token,orumFeeDistributionAddress, _borrowFee);
        ...
        if (_isCollIncrease) {
            ...
            _activePool.sendROSE(amatic_Token, _borrower, _collChange);
        }
    }
