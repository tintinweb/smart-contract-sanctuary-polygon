...
contract Faucet is Authorizable, ReentrancyGuard {
    ...
    event ClaimedByBettor(address indexed account, uint256 claimedTokens);
    ...
    string[] private roles = ["DexWinner", "InnerCircle", "Agent"];
    mapping(address => User) private addressToUser;
    mapping(address => bool) private bettorClaimed;

    uint256 private dexWinnerMaxTk = 10000;
    uint256 private innerCircleMaxTk = 20000;
    uint256 private agentMaxTk = 400000;

    uint256 private bettorClaimLimit = 1000;
    uint256 private dexWinnerClaimLimit = 1000;
    uint256 private innerCircleClaimLimit = 2000;
    uint256 private agentClaimLimit = 40000;
    uint256 private claimFrequnecy = 604800;

    //uint256 private claimFrequnecy = 120;
    ...
    function bettorClaim() public nonReentrant {
        if (bettorClaimed[msg.sender]) {
            revert BettorCanClaimOnlyOnce();
        }
        if (addressToUser[msg.sender].flag) {
            revert OnlyForBettor();
        }
        bettorClaimed[msg.sender] = true;
        i_dexwinToken.transfer(msg.sender, (bettorClaimLimit * (10**s_decimals)));
        emit ClaimedByBettor(msg.sender, bettorClaimLimit);
    }

    function assignUser(address account, uint256 roleValue) public onlyAuthorized {
    ...
    function promoteRole(address account, uint256 roleValue) public onlyAuthorized {
        require(addressToUser[account].flag, "No Role to promote");
        require(roleValue > 0, "Invalid Role value");
        string memory role = roles[roleValue];
        if (roleValue == 1) {
            //InnerCircle
            addressToUser[account] = User(
    ...
    function getBettorClaimedStatus(address account) public view returns (bool) {
        return bettorClaimed[account];
    }

    function getDexwinnerInfo() public view returns (uint256, uint256) {
        return (dexWinnerMaxTk, dexWinnerClaimLimit);
    }

    function getInnerCircleInfo() public view returns (uint256, uint256) {
        return (innerCircleMaxTk, innerCircleClaimLimit);
    }

    function getAgentInfo() public view returns (uint256, uint256) {
        return (agentMaxTk, agentClaimLimit);
    }

    function getClaimFrequency() public view returns (uint256) {
        return claimFrequnecy;
    }
}
