...
contract OyesMatic {
    ...
    event MissedMaticReceive(address indexed receiver,uint receiverId, address indexed from,uint indexed fromId, uint8 matrix, uint8 level);
    ...
    function registration(address userAddress, address referrerAddress) private {
        ...
        lastUserId++;
        totalearnedMatic = totalearnedMatic+20 ether;
        ...
        if (matrix == 1) {
            while (true) {
                if (users[receiver].O3Matrix[level].blocked) {
                    emit MissedMaticReceive(receiver,users[receiver].id, _from,users[_from].id, 1, level);
            ...
            while (true) {
                if (users[receiver].O6Matrix[level].blocked) {
                    emit MissedMaticReceive(receiver,users[receiver].id, _from,users[_from].id, 2, level);
                    isExtraDividends = true;
