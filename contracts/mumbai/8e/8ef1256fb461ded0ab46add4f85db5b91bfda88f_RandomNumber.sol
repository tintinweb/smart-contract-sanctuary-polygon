...
contract RandomNumber {
    ...
    function getCurrentPrice0() public view returns (uint256) {
        (uint256 _reserve0, uint256 _reserve1, ) = uniswapPair.getReserves();
        return _reserve1.div(_reserve0);
    }

    function getCurrentPrice00() public view returns (uint256) {
    ...
    ) public view returns (uint112, uint112, uint256) {
        (
            uint112 _reserve0,
            uint112 _reserve1,
            uint256 _blockTimestampLast
        ) = IUniswapV2Pair(pair_address).getReserves();

        return (_reserve0, _reserve1, _blockTimestampLast);
    }

    function getReserves2(
        address pair_address
