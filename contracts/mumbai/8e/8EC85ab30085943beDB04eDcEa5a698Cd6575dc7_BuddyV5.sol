...
abstract contract NFT721Metadata is NFT721Creator {
     ...
     */
    function _setTokenIPFSPath(address owner, uint256 tokenId, string memory _tokenIPFSPath)
        internal
    {
        ...
        require(
            !creatorToIPFSHashToMinted[owner][_tokenIPFSPath],
            "NFT721Metadata: NFT was already minted"
        );

        creatorToIPFSHashToMinted[owner][_tokenIPFSPath] = true;
    ...
    TreasuryNode
{
    ...
    event PaymentDetails(uint256 tokenId, address indexed paymentToken, uint256 indexed feeAmount);
    ...
    function mint(
        ...
        if (tokenFreePassStatus[paymentToken] == 0)
            _checkMintFees(owner, paymentToken, feeAmount, _type);
        else {
            require(
        ...
        _lock(owner, tokenIds, collectionAddress, tokenId);
        _mint(owner, tokenId);
        _updateTokenCreator(tokenId, owner);
        _setTokenIPFSPath(owner, tokenId, tokenIPFSPath);

        emit PaymentDetails(tokenId, paymentToken, feeAmount);
    ...
    function updateTokenURI(
        ...
        require(owner == ownerOf(tokenId), "Buddy: Not Authorized");

        if (tokenFreePassStatus[paymentToken] == 0)
            _checkUpdateFees(owner, paymentToken, feeAmount, _type);
        else {
            require(
                nftIdPassStatus[paymentToken][feeAmount] == false,
                "Buddy: Ticket is Used"
        ...
        _setTokenIPFSPath(owner, tokenId, tokenIPFSPath);
        _release(owner, releaseTokenIds, releaseColAddresses, tokenId);
        _lock(owner, tokenIds, collectionAddresses, tokenId);
