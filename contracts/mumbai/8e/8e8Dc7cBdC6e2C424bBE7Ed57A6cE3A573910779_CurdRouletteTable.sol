...
import "erc721a-upgradeable/contracts/extensions/ERC4907AUpgradeable.sol";
    ...
    UUPSUpgradeable
{
    ...
    function initialize(
        ...
        __ERC721A_init("Curd Roulette", "CRT");
        __ERC4907A_init();
        ...
        price = _price;
        rouletteOperator = _rouletteOperator;
        baseURI = _newBaseURI;
    }

    function mint(address _to, uint256 _quantity) external payable {
        require(
            currency.transferFrom(_to, address(this), _quantity * price),
            "Insufficient amount"
        );
        uint64 expires = 1924934400; // 2030 dec 31st
        setUser(_nextTokenId(), rouletteOperator, expires);
        _mint(_to, _quantity);
    }

    function adminMint(address _to, uint256 _quantity)
        external
        payable
    ...
    function setRouletteOperator(address _rouletteOperator)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        rouletteOperator = _rouletteOperator;
    ...
    function setCurrency(address _currency)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        currency = IERC20(_currency);
...
abstract contract ERC4907AUpgradeable is ERC721A__Initializable, ERC721AUpgradeable, IERC4907AUpgradeable {
    ...
    function setUser(
        ...
        address user,
        uint64 expires
    ) public virtual override(IERC4907AUpgradeable) {
     ...
     */
    function userOf(uint256 tokenId) public view virtual override(IERC4907AUpgradeable) returns (address) {
     ...
     */
    function userExpires(uint256 tokenId) public view virtual override(IERC4907AUpgradeable) returns (uint256) {
