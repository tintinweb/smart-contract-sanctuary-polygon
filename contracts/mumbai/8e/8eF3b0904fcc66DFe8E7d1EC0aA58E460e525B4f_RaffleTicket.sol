...
contract RaffleTicket is HSystemChecker, Multicall, ERC721A, ERC721AQueryable {
    ...
    function burn(address buyer, uint256 tokenId) external onlyRole(CONTRACT_ROLE) {
        _burn(tokenId, false);
    }

    /// @notice Burn multiple Raffle Ticket tokens
    /// @param buyer - Address of buyer
    /// @param tokenIds - tokenIds to be burn
    function batchBurn(address buyer, uint256[] calldata tokenIds) external onlyRole(CONTRACT_ROLE) {
