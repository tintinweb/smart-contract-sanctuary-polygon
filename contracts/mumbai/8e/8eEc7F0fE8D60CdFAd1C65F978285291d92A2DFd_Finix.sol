...
contract Finix {
    ...
    uint256 private constant poolTimeStep = 20 minutes;
    ...
    uint256[10] private communityPackages = [50e18, 1000e18, 1100e18, 1500e18, 1600e18, 2000e18, 210018, 2500e18, 2600e18, 3000e18];
    uint256[6] private communityReturns = [700, 800, 900, 1000, 1100, 1300];
    uint256[20] private bonusRewards = [1500, 800, 800, 800, 700, 700, 600, 600, 500, 500, 200, 200, 200, 200, 200, 200, 200, 200, 200, 200];
    uint256[8] private royaltyPoolPercents = [70, 70, 50, 50, 40, 40, 30, 30];
    uint256[8] private PoolBusinessRequired = [100e18, 150e18, 200e18, 250e18, 300e18, 350e18, 400e18];
    uint256[8] public royaltyPool;
    address public jackpotFund = 0x95D9E7945681D1E7D4F9B9CeeF6a4aEcAe5CD0Ac; 
    address public recoveryFund = 0xc538779A628a21D7CCA7b1a3E57E92f5226C3E27; 
    address public registerationFee = 0xc538779A628a21D7CCA7b1a3E57E92f5226C3E27; 
    ...
    uint256 private constant lotteryDuration = 20 minutes;
    ...
    uint256 private rewardingMultiple = 7000;
    uint256 private freezePercents = 5000;
    uint256[11] private SLWeeklyBusiness = [100e18, 150e18, 200e18, 250e18, 300e18, 350e18, 40018, 450e18, 500e18, 550e18, 600e18];
    ...
    function register(address _ref) public {  
        ...
        for(uint256 i=0; i<registrationRewards.length; i++) {
            if(upline != address(0)) {
                ...
                if(userInfo[upline].directTeam >= i && userInfo[upline].isActive && userInfo[upline].curRevenue < userInfo[upline].selfDeposit.mul(_rewarding).div(baseDivider)) {
                    ...
                    rewardInfo[upline].growth = rewardInfo[upline].growth.add(reward.mul(freezePercents).div(baseDivider));
    ...
    function _calLevel(address _user) private view returns(uint256) {
        ...
        if(user.directTeam >= 2 && user.totalTeam >= 3 && user.selfDeposit >= 2000e18) {
            ...
            if(forLevel >= 3000e18) return 11;
        } 
        if(user.directTeam >= 1 && user.totalTeam >= 3 && user.selfDeposit >= 1600e18) {
            (uint256 forLevel, , , ) = calBusiness(_user, 2500e18);
            if(forLevel >= 2500e18) return 10;
        } 
        if(user.directTeam >= 1 && user.totalTeam >= 2 && user.selfDeposit >= 1500e18) {
            (uint256 forLevel, , , ) = calBusiness(_user, 2400e18);
            if(forLevel >= 2400e18) return 9;
        } 
        if(user.directTeam >= 1 && user.totalTeam >= 2 && user.selfDeposit >= 1200e18) {
            (uint256 forLevel, , , ) = calBusiness(_user, 2100e18);
            if(forLevel >= 2100e18) return 8;
        } 
        if(user.directTeam >= 1 && user.totalTeam >= 2 && user.selfDeposit >= 1100e18) {
            (uint256 forLevel, , , ) = calBusiness(_user, 2000e18);
            if(forLevel >= 2000e18) return 7;
        }
        if(user.directTeam >= 1 && user.totalTeam >= 2 && user.selfDeposit >= 500e18) {
            (uint256 forLevel, , , ) = calBusiness(_user, 500e18);
            if(forLevel >= 500e18) return 6;
        }
        if(user.directTeam >= 1 && user.totalTeam >= 2 && user.selfDeposit >= 400e18) {
            (uint256 forLevel, , , ) = calBusiness(_user, 400e18);
            if(forLevel >= 400e18) return 5;
        }
        if(user.directTeam >= 1 && user.totalTeam >= 2 && user.selfDeposit >= 300e18) {
            (uint256 forLevel, , , ) = calBusiness(_user, 400e18);
            if(forLevel >= 400e18) return 4;
        }
        if(user.directTeam >= 1 && user.totalTeam >= 2 && user.selfDeposit >= 200e18) {
            (uint256 forLevel, , , ) = calBusiness(_user, 250e18);
            if(forLevel >= 250e18) return 3;
        }
        if(user.directTeam >= 1 && user.totalTeam >= 2 && user.selfDeposit >= 200e18) {
            (uint256 forLevel, , , ) = calBusiness(_user, 200e18);
            if(forLevel >= 200e18) return 2;
        }
        if(user.directTeam >= 1 && user.totalTeam >= 1 && user.selfDeposit >= 100e18) {
            (uint256 forLevel, , , ) = calBusiness(_user, 200e18);
            if(forLevel >= 200e18) return 1;
        }
    ...
    function calBusiness(address _user, uint256 _amount) public view returns(uint256, uint256, uint256, uint256){
        ...
        for(uint256 i = 0; i < teamUsers[_user][0].length; i++){
            ...
            uint256 toCheck = _amount.mul(10000).div(baseDivider);
    ...
    function _upgrade(address _user, uint256 amount, uint256 _type) private {
        ...
        if(_type == 0) {
            ...
            if(_prevAmount > 0) {
                require(block.timestamp.sub(reward.upgraded) >= 15 minutes, "cannot upgrade befor time");
            }
            ...
            if(_prevAmount > 0) {
                require(block.timestamp.sub(reward.upgraded) >= 15 minutes, "cannot retopup befor time");
            }
            ...
            if(user.curPackage == 1) {
                ...
                require(user.upgraded[1] < 1, "cannot increase more than 1 times");
                require(amount.mod(50e18) == 0, "amount should be in multiple of 50");
            }
        ...
        for(uint256 j=0; j<8; j++) {
            for(uint256 i=0; i<users.length; i++) {
                if(userInfo[users[i]].level == level && userInfo[users[i]].isActive) {
                    uint256 _weeklyBusiness = getWeeklyRevenue(users[i]);
    ...
    function upgradeByGrowth(uint256 amount) public {
        require(userInfo[msg.sender].referrer != address(0), "Can only be used after registeration");
    ...
    function transferByGrowth(address _to, uint256 _amount) public {
        ...
        require(_amount >= 10e18 && _amount.mod(10e6) == 0, "invalid error");
    ...
    function withdraw() public {
        ...
        uint256 totalRewards = reward.directs.add(reward.statics).add(reward.incomeReleased).add(reward.layerIncome).add(reward.royalty);

        require(totalRewards >= 10e18, "cannot be less than 10");

        uint256 _curGrowth = getCurGrowth(msg.sender);
        uint256 _curLottery = getCurGrowth(msg.sender);
        reward.directs = 0;
        reward.statics = 0;
        ...
        reward.lottery = reward.lottery.add(_curLottery);
        uint256 withdrawable = totalRewards.sub(8000).div(baseDivider);
    ...
    function getCurGrowth(address _user) public view returns(uint256) {
        ...
        uint256 reward = user.directs.add(user.statics).add(user.incomeReleased).add(user.layerIncome).add(user.royalty);
    ...
    function Lottery(uint256 _guess) public {
        ...
        if(lotteryBal > 5e18) {
            ...
            growthTaken = lotteryBal;
        }
        dai.transferFrom(msg.sender, address(this), lotteryBetFee.sub(growthTaken));
        uint256 dayNow = getCurDay();
        lotteryPool = lotteryPool.add(lotteryBetFee);
        userRewards.growthDebt = userRewards.growthDebt.add(growthTaken);
        ...
        for(uint256 i = 0; i < winners.length; i++){
            if(winners[i] != address(0)){
                ...
                if(curUser.isActive) {
                    ...
                    if(curUser.curRevenue.add(reward) > curUser.selfDeposit.mul(_rewarding).div(baseDivider)) {
                        reward = (curUser.selfDeposit.mul(_rewarding).div(baseDivider)).sub(curUser.curRevenue);
                    }

                    if(curUser.curRevenue.add(reward) > curUser.selfDeposit.mul(2000).div(baseDivider)) {
                        uint256 left = curUser.curRevenue.add(reward).sub(curUser.selfDeposit.mul(2000).div(baseDivider));
                        uint256 temp = reward;
                        if(temp > left) {
                            temp = temp.sub(left);
                        } else {
                        ...
                        rewardInfo[winners[i]].incomeFreezed = rewardInfo[winners[i]].incomeFreezed.add(left);
                        rewardInfo[winners[i]].LotteryIncome = rewardInfo[winners[i]].LotteryIncome.add(temp);
                    } else {
                        rewardInfo[winners[i]].LotteryIncome = rewardInfo[winners[i]].LotteryIncome.add(reward);
                    }

                    totalReward = totalReward.add(reward);
                    rewardInfo[winners[i]].LotteryIncome = rewardInfo[winners[i]].LotteryIncome.add(reward);
                    userInfo[winners[i]].totalRevenue = userInfo[winners[i]].totalRevenue.add(reward);
                    userInfo[winners[i]].curRevenue = userInfo[winners[i]].curRevenue.add(reward);
                }
            } else {
                break;
            }
    ...
    function _calCurRewardingMultiple(address _user) private view returns(uint256) {
        ...
        if(SL2 && userInfo[_user].level > 0) {
            uint256 _weeklyBusiness = getWeeklyRevenue(_user);
    ...
    function getWeeklyRevenue(address _user) public view returns(uint256) {
        uint256 totalWeekly;
        for(uint256 k=userInfo[_user].weeklyBusiness.length; k>0; k--) {
            if(block.timestamp.sub(userInfo[_user].weeklyBusiness[k-1].time) <= 10 minutes) {
                totalWeekly = totalWeekly.add(userInfo[_user].weeklyBusiness[k-1].amount);
            } else {
                break;
            }
        }
