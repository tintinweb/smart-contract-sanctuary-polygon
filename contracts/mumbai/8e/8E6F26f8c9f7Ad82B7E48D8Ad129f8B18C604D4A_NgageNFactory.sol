...
contract NgageNFactory is
  AccessControl,
  ERC2771Context,
  ReentrancyGuardUpgradeable
{
  bytes32 public immutable CREATOR;
  bytes32 public immutable CONTRACT_CREATOR;
  bytes32 public immutable HANDLER;
  ...
  mapping(string => bool) public isCollectionCreated;
  mapping(string => address) public collectionOwner;
    ...
    ERC2771Context(_minimalForwarder)
  {
    ...
    CONTRACT_CREATOR = keccak256('CONTRACT_CREATOR');
    HANDLER = keccak256('HANDLER');
    _grantRole(DEFAULT_ADMIN_ROLE, _msgSender());
    _grantRole(CONTRACT_CREATOR, _msgSender());
  ...
  function createCollection(
    ...
    require(!isCollectionCreated[_collectionId], 'NF:Factory exists');
    require(_owner != _msgSender(), 'NF:Contract owner cant be proxy admin');
    TransparentUpgradeableProxy _proxy = new TransparentUpgradeableProxy(
      ngageN,
      _msgSender(),
      abi.encodeCall(
        NgageN(address(0)).initialize,
        (_uri, _contractUri, _royaltyRecipient, _royaltyPercentageShare)
      )
    );
    Administrator _admn = new Administrator(
      INgageN(address(_proxy)),
      minimalForwarder
    );
    MintNFT _mintNFT = new MintNFT(INgageN(address(_proxy)), customForwarder);
    NgageN(address(_proxy)).grantRole(DEFAULT_ADMIN_ROLE, _owner);
    NgageN(address(_proxy)).grantRole(HANDLER, address(_admn));
    NgageN(address(_proxy)).grantRole(HANDLER, address(_mintNFT));
    ...
    collectionOwner[_collectionId] = _owner;
   ...
   */
  function setNewNgageN(address _ngageN) external onlyAdmin {
    require(_ngageN != address(0), 'NF: Invalid address');
    ngageN = _ngageN;
  }

  /**
   * @dev Public function to set new minimal forwarder contract address
   * Reverts if the caller is not an admin
   * @param _forwarder The address that will be set as new custom forwarder contract
   */
  function setNewMinimalForwarder(address _forwarder) external onlyAdmin {
   ...
   * @param _forwarder The address that will be set as new custom forwarder contract
   */
  function setNewCustomForwarder(address _forwarder) external onlyAdmin {
    require(_forwarder != address(0), 'NF: Invalid address');
  ...
  ReentrancyGuardUpgradeable
{
  ...
  mapping(address => EnumerableSet.UintSet) private tokenByOwner;
  ...
  function _setRoyalties(address recipient, uint256 value) internal {
    ...
    _royalties = RoyaltyInfo(uint24(value), recipient);
  }
  ...
  function burnNFT(
    ...
    require(
      _msgSender() == from || hasRole(HANDLER, _msgSender()),
      'NN: Only administrator or token holder can burn tokens'
    );
    _burn(from, id, amount);
    if (balanceOf(from, id) == 0) {
      tokenByOwner[from].remove(id);
    }
  }
  ...
  function setApprovalForToken(
    ...
    require(
      _msgSender() == account || hasRole(HANDLER, _msgSender()),
  ...
  /**
   * @dev Public function to fetch the list of tokens owner by an account.
   * @param account The address for whom we want to fetch the tokens.
   */
  function fetchTokensByOwner(address account)
    external
    view
    returns (uint256[] memory)
  {
    return tokenByOwner[account].values();
  }
...
contract MintNFT is AccessControl, ERC2771Context {
   ...
   * @param _forwarder Custom Forwarder instance address for ERC2771Context constructor
   */
  ...
  /**
     ...
     */
  modifier onlyAdmin() {
    require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), 'MintNFT: Only Admin');
    _;
  }
   ...
   */
  function setNgageNAddress(address _ngagenAddress) external onlyAdmin {
...
contract Administrator is AccessControl, ERC2771Context {
   ...
   * @param _ngagenAddress address of the ngageN contract
   * @param _forwarder Minimal Forwarder instance address for ERC2771Context constructor
   */
  constructor(INgageN _ngagenAddress, address _forwarder)
    ERC2771Context(_forwarder)
  {
   ...
   */
  function setTokenURI(uint256 id, string memory _tokenUri) external onlyAdmin {
  ...
  function burnNFT(
    ...
    require(balanceOf(from, id) >= amount, 'ADMN: Insufficient tokens to burn');
