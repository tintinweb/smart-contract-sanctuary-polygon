...
contract nftShare20230423 is Ownable {
    ...
    event ShareEvent(uint256 _shareId, uint256 _nftTotalNumber, uint256 _BronzeNumber, uint256 _timestamp, uint256 _shareAmount, uint256 _nftPerShare, uint256 _nftTotalPerShare);
    ...
    function setShare() public {
        ...
        if (_BronzeNumber > 0 && useShareMode && _shareAmount >= minShareAmount) {
            ...
            emit ShareEvent(shareId, _nftTotalNumber, _BronzeNumber, block.timestamp, _shareAmount, _nftPerShare, nftTotalPerShare);
            shareId += 1;
        }
    }
    ...
    function _pendingReward(address _user) private view returns (uint256) {
        ...
        uint256 _minShareId;
        uint256 _maxShareId;
        _minShareId = getMinIndex(_user, _tokenId);
        if (_isBronze) {
