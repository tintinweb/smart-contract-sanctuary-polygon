...
contract swapHelperDiyTask is OpsTaskCreator, Ownable {
    ...
    function setSwapInfo(IAocoRouter02 _routerAddress, address[] memory _swapRouter, address[] memory _swapRouter2) private {
        require(_swapRouter[0] == address(USDTPoolAddress) && _swapRouter2[_swapRouter2.length - 1] == address(USDTPoolAddress), "e001");
        require(_swapRouter2[0] != address(USDTPoolAddress) && _swapRouter[_swapRouter2.length - 1] != address(USDTPoolAddress), "e002");
    ...
    function createTask(IAocoRouter02 _routerAddress, address[] memory _swapRouter, address[] memory _swapRouter2, uint256 _swapAmount, uint256 _interval, uint256 _salt) external payable {
        ...
        md5TaskList[md5] = id;
        taskConfig memory k = taskConfig(_routerAddress, _swapRouter, _swapRouter2, _swapAmount, _interval, _salt, 0, taskAmount, md5);
        taskConfigList[id] = k;
    ...
    function swapToken(address _user, bytes32 _md5) external onlyDedicatedMsgSender {
        ...
        if (x._swapIn == 0) {
            ...
            if (address(USDTPoolAddress.USDT()) == x.swapInToken) {
                USDTPoolAddress.claimUSDT(y._swapAmount);
            }
            ...
            if (address(USDTPoolAddress.USDT()) == x.swapInToken) {
                uint256 poolFee = y._swapAmount.mul(1).div(100);
                uint256 allFee = x.spendSwapInToken.add(poolFee);
                require(userInfoList[_user].usdtAmount >= allFee);
