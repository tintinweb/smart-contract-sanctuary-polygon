...
contract SlaveCrossLedgerVault is CrossLedgerVault {
    ...
    function _redeemCompleted(uint256 received) internal override {
        ...
        bytes32 transferId = adapter.sendAssets(
            currentAction.amountOut,
            address(0),
            0 // won't be used
        );

        _sendUpdateMessage(chainIdToLzChainId[masterChainId], transferId);
    }
}
...
abstract contract CrossLedgerVault is LzApp, AccessControl {
    ...
    RootVaultAction public currentAction;
    ...
    constructor(IRootVault _rootVault, address _lzEndpoint) LzApp(_lzEndpoint) {
        ...
        IERC20Metadata(mainAsset).safeIncreaseAllowance(
            address(rootVault),
            type(uint256).max
        );
    ...
    modifier onlyFromBridgeAdapter() {
        require(
            bridgeAdapterToChainId[msg.sender] != 0,
            "only bridge adapter can call this method"
        );
        _;
    }
    ...
    function removeChain(uint256 chainId) external onlyRole(ADMIN_ROLE) {
        ...
        lzChainIdToChainId[lzChainId] = 0;
    ...
    function updateBridgeAdapter(
        ...
        if (oldBridgeAdapter != address(0)) {
            mainAsset.safeDecreaseAllowance(
                oldBridgeAdapter,
                mainAsset.allowance(address(this), oldBridgeAdapter)
            );
        }
    ...
    function transferCompleted(
        ...
        uint256 value,
        uint8 slippage
    ) external onlyFromBridgeAdapter {
        uint256 fromChainId = bridgeAdapterToChainId[_msgSender()];
    ...
    function _convertRootVaultAssetsToAssets(
        uint256 rootVaultAssets
    ) internal view returns (uint256) {
        return
            (rootVaultAssets * (10 ** _decimals)) /
            (10 ** rootVault.decimals());
    }

    function totalAssets() public view returns (uint256) {
        return
            _convertRootVaultAssetsToAssets(
                rootVault.convertToAssets(rootVault.balanceOf(address(this)))
            );
    }

    function _depositLocal(uint256 assets, uint8 slippage) internal {
        currentAction = RootVaultAction({
    ...
    function _redeemLocal(
        ...
        uint256 amountToRedeem = (shares * rootVault.balanceOf(address(this))) /
            totalSupply;
        currentAction = RootVaultAction({
    ...
    function _finalizeCurrentAction() internal virtual {
        if (currentAction.actionType == RootVaultActionType.REDEEM) {
            _redeemCompleted(currentAction.amountOut);
        } else if (currentAction.actionType == RootVaultActionType.DEPOSIT) {
            uint256 totalAssetsAfter = totalAssets();
            uint256 deposited = _convertRootVaultAssetsToAssets(
                rootVault.convertToAssets(currentAction.amountOut)
            );
            uint256 totalAssetsBefore = totalAssetsAfter - deposited;
    ...
    ) external returns (bool isExecutedCompletly) {
        require(isActionInProgress, "No action is in progress");
        require(
            currentAction.amountIn >= amountToProcess,
            "Trying to process more than available"
        );
        require(amountToProcess > 0, "Can't process zero amount");

        bool receivedEnoughAssets;
        if (currentAction.actionType == RootVaultActionType.DEPOSIT) {
            uint256 receivedShares = rootVault.deposit(
                amountToProcess,
                address(this)
            );

            uint256 receivedAssets = _convertRootVaultAssetsToAssets(
                rootVault.convertToAssets(receivedShares)
            );
            currentAction.amountOut += receivedShares;

            receivedEnoughAssets =
                receivedAssets >=
                ((1000 - currentAction.slippage) * amountToProcess) / 1000;
        } else {
            uint256 expectedAssets = rootVault.feeInclusivePricePerToken() * amountToProcess / (10 ** rootVault.decimals());
            uint256 receivedAssets = rootVault.redeem(
                amountToProcess,
                address(this)
            );
            currentAction.amountOut += receivedAssets;

            receivedEnoughAssets =
                receivedAssets >=
                ((1000 - currentAction.slippage) * expectedAssets) / 1000;
        }

        require(receivedEnoughAssets, "Slippage is too high");

        currentAction.amountIn -= amountToProcess;

        if (currentAction.amountIn == 0) {
    ...
    function _sendUpdateMessage(
        uint256 toChainId,
        bytes32 transferId
    ) internal {
        bytes memory payload = UpdateMessage({transferId: transferId})
            .encodeMessage();
    ...
    function _blockingLzReceive(
        ...
        (LzMessageType messageType, bytes memory data) = _payload
            .decodeTypeAndData();
...
abstract contract LzApp is ILayerZeroReceiver {
    ...
    function _setTrustedRemoteAddress(
        ...
        trustedRemotes[_remoteChainId] = abi.encodePacked(
            _remoteAddress,
            address(this)
        );
    }
...
library LzMessages {
    function encodeMessage(
        RedeemMessage memory message
    ) internal pure returns (bytes memory payload) {
        return abi.encode(LzMessageType.REDEEM, abi.encode(message));
    }

    function encodeMessage(
        UpdateMessage memory message
    ) internal pure returns (bytes memory payload) {
        return abi.encode(LzMessageType.UPDATE, abi.encode(message));
    }

    function encodeMessage(
        DepositedFromBridgeMessage memory message
    ) internal pure returns (bytes memory payload) {
        return
            abi.encode(
                LzMessageType.DEPOSITED_FROM_BRIDGE,
                abi.encode(message)
            );
    }

    function decodeTypeAndData(bytes memory payload) internal pure returns(LzMessageType messageType, bytes memory data) {
