...
contract WinMatic3 {
    ...
    // Event emitted when a commission is paid
    event CommissionPaid(address indexed user, uint256 amount);

    // Map user addresses to user data
    mapping(address => User) public users;

    // Map referral addresses to referrer addresses
    ...
    // Event emitted when commission is withdrawn
    ...
    // modifier to check if caller is the defi operator
    modifier isOperator() {
    ...
    // Calculate the number of tokens to issue
        uint256 numOfTokens = msg.value / minDeposit;

        // Check if the deposit amount is a multiple of minDeposit
        require(msg.value == numOfTokens * minDeposit, "Deposit amount must be a multiple of minDeposit");

        if (referrer != address(0)) {
           require(isContract(referrer), "Invalid referrer address");
        }
        ...
        uint256 commission = SafeMath.div(
            SafeMath.mul(msg.value, COMMISSION_RATE),
            100
        );
        uint256 depositAmount = SafeMath.sub(msg.value, commission);

        // Transfer commission to the contract operator
        if (commission > 0) {
            (bool commissionSuccess, ) = payable(defiOperator).call{
                value: commission
            }("");
            require(commissionSuccess, "Commission transfer failed");
            emit CommissionPaid(defiOperator, commission);
        }

        user.balance = SafeMath.add(user.balance, depositAmount);
        user.depositTime = block.timestamp;
        user.depositCount++;

        // Add deposit amount to depositAmounts mapping
        user.depositAmounts[user.depositCount] = depositAmount;

        // Add deposit timestamp to depositTimestamps mapping
        ...
        contractBalance = SafeMath.add(contractBalance, depositAmount);

        emit Deposit(msg.sender, depositAmount);
    }

    // Function to get user deposit amounts
    ...
    function stake(uint256 amount) external {
        ...
        // Add staked amount to totalStaked
    ...
    function withdraw() external {
        ...
        // Calculate withdrawal amount and commission
        ...
        // Transfer payable amount to user
        ...
        // Add withdrawal amount to withdrawalAmounts mapping
