...
contract CommunityJoiningRules is ICommunityJoiningRules, Context {
    ...
    function validate(address _communityId, address _user, uint256 _tokenId) external view override onlyPlugin returns(bool) {
        ...
        if (isActiveRule(_communityId, RulesList.WHEN_JOINING_PAYMENT)) {
            if (!IAccount(registry.account()).isCommunityUser(_communityId, _user)) {
                DataTypes.PaymentInfo memory paymentInfo = ISubscription(registry.subscription()).getPaymentInfo(_tokenId);
                require(
                    paymentInfo.paymentType == DataTypes.PaymentType.FOR_COMMUNITY_JOIN
                    && paymentInfo.owner == _user
                    && paymentInfo.communityId == _communityId,
                    "CommunityJoiningRules: you do not have subscription token"
                );
            }
        }
        if (isActiveRule(_communityId, RulesList.PERIODIC_PAYMENT)) {
            if (IAccount(registry.account()).isCommunityUser(_communityId, _user)) {
                DataTypes.PaymentInfo memory paymentInfo = ISubscription(registry.subscription()).getPaymentInfo(_tokenId);
                require(
                    paymentInfo.paymentType == DataTypes.PaymentType.FOR_PERIODIC_ACCESS
                    && ISubscription(registry.subscription()).ownerOf(_tokenId) == _user,
                    "CommunityJoiningRules: do not have subscription token"
                );
                uint256 currentTime = block.timestamp;
                require(
                    paymentInfo.startTime < currentTime &&  currentTime < paymentInfo.endTime,
                    "CommunityJoiningRules: wrong subscription time"
                );
            }
        }
...
interface ICommunityJoiningRules is IBaseRulesWithPostId {
