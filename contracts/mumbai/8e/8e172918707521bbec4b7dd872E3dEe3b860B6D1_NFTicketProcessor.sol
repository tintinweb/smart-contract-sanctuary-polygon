...
contract NFTicketProcessor is INFTicketProcessor {
    using NFTreasuryLib for NFTreasuryLib.Treasury;
    using NFTicketLib for NFTicketLib.TicketLib;
    ...
    {
        require(ticketLib.inWhitelist(sender), "Sender not whitelisted");
        _;
    }

    constructor(address _nfticket, address _master, address _blxft) {
        nfticket = _nfticket;
        master = _master;
        BLXFT_ERC20 = _blxft;
    }
    ...
    {
        if ( serviceDescriptor == 0) { // top up "cash" balance
            return 0; // TODO
        }
        else {
            NFTicket(nfticket).topUpTicket(tokenID, creditsAdded, erc20Token, numberERC20Tokens);
        }
     } 
    ...
    /**
     * The presentTicket() function is implemented twice, once with and once without the
     * ticketReceiver argument. Calling the function without that argument transfers the
     * ticket to the NFTicket contract owner. Calling it with the ticketReceiver argument
     * transfers the ticket to this address.
     *
     * The main functionality of presentTicket() handling checks and the ERC20 distribution
     * has been outsourced to the internal _handleChecksAndFees() function.
     */
    ...
    {
        Ticket memory ticket = _handleChecksAndFees(
            tokenID,
            _serviceProvider,
            credits,
            presenter
        ...
        // only keep ticket if this is a checkout service
        if (ticketLib.isCheckoutService(ticket.serviceDescriptor)) {
            NFTicket(nfticket).transferFrom(presenter, master, tokenID);
        }
        else {
            // user can keep the NFTicket
        }
        return (ticket);
    }

    function presentTicket( //TODO JAN: function is specified external but should be public. agree?
        uint256 tokenID,
        address presenter,
        address _serviceProvider,
        uint256 credits,
        ...
        returns (Ticket memory ticket) 
    {
        ...
        // only keep ticket if this is a checkout service
        if (ticketLib.isCheckoutService(ticket.serviceDescriptor)) {
            NFTicket(nfticket).transferFrom(presenter, ticketReceiver, tokenID);
        }
        else {
            // user can keep the NFTicket
        }
        return (ticket);
        //TODO JAN: add event: TicketPresented(uint256 tokenID, address from, address to, uint256 creditsPresented)
    }

    function _handleChecksAndFees(
        uint256 tokenID,
        address _serviceProvider,
        uint256 credits,
        address presenter
        ...
        /***
         * TODO
         * Ticket data should come from web2
         * SC stores 
            mapping(keccack(Ticket) => tokenID)
            and compares this to tokenID in call
            For this to work, TicketData always needs to emitted
            (and stored until OK(keccack) received)
            so web2 can pick it up
        ***/
        Ticket memory _t = NFTicket(nfticket).getTicketData(tokenID);

        /****  START Verify ownership of nfticket before processing */
        if (
            !(msg.sender == ERC721(nfticket).ownerOf(tokenID) ||
                msg.sender == ERC721(nfticket).getApproved(tokenID) ||
                NFTicket(nfticket).isServiceProvider(msg.sender)
            )
                &&
            ((_t.serviceDescriptor & CHECKOUT_TICKET) ==  CHECKOUT_TICKET)

        ) {
            _msg = "msg.sender ";
            id = Strings.toString(uint256(uint160(msg.sender)));
            _msg = string(abi.encodePacked(_msg, id, " not owner "));
            id = Strings.toString(uint256(uint160(ERC721(nfticket).ownerOf(tokenID))));
            _msg = string(abi.encodePacked(_msg, id, " nor apprvd "));
            id = Strings.toString(uint256(uint160(ERC721(nfticket).getApproved(tokenID))));
            _msg = string(abi.encodePacked(_msg, id, "."));
            require(
                (msg.sender == ERC721(nfticket).ownerOf(tokenID) ||
                    msg.sender == ERC721(nfticket).getApproved(tokenID)) ||
                        NFTicket(nfticket).isServiceProvider(msg.sender),
        ...
        /****  END Verify ownership of NFTicket before processing */

        if (ticketLib.isTicket(_t)) {
            uint256 availableCredits = _t.credits;

            if (!(availableCredits >= credits)) {
                _msg = "available credits ";
                id = Strings.toString(availableCredits);
                string memory sCredits = Strings.toString(credits);
                _msg = string(
                    abi.encodePacked(_msg, id, " insufficent for ", sCredits)
                );
                require(availableCredits >= credits, _msg);
            }
            bool _isServiceProvider = NFTicket(nfticket).isServiceProvider(_serviceProvider);
            if (!_isServiceProvider) {
                id = Strings.toString(uint256(uint160(_serviceProvider)));
                _msg = string(
                    abi.encodePacked("contract ", id, " not a ServiceProvider")
                );
                require(_isServiceProvider, _msg);
            }

            _t = NFTicket(nfticket).presentTicketFeeToPool(_t, credits, BLXFT_ERC20);
            emit CreditsToService(
                tokenID,
                credits,
                value, // TODO: This variable is not calculated before usage here!
                presenter,
                _serviceProvider
            );
        } else if (ticketLib.isCertificate(_t)) {
            value = _t.certValue;

            if (!(block.timestamp >= _t.certValidFrom)) {
                _msg = "earliest date for this NFTCert is ";
                _msg = string(
                    abi.encodePacked(
                        _msg,
                        StringLib.uintToString(_t.certValidFrom)
                    )
                );
                require(block.timestamp >= _t.certValidFrom, _msg);
            }

            if (_t.credits > 0) {
                NFTicket(nfticket).presentCertificateRepurchase(msg.sender, _t);
                _t.credits = 0;

                // TODO NFTService(serviceProvider).consumeCredits()
            }
        }
    ...
    // refund ticket fee to ticket owner
    function refundTicketFeeToTicketOwner(uint256 tokenId, uint256 ticketFee)
        external
        override
    {
        // TODO require(msg.sender == NRService);
        TransferHelper.safeTransfer(BLXFT_ERC20, ERC721(nfticket).ownerOf(tokenId), ticketFee);
        // TODO distributionVerification();
    }
...
contract NFTicket is Ownable, ERC721URIStorage, INFTicket {
    ...
    EnumerableSet.AddressSet _whitelistSet;
    ...
    modifier onlyWhiteList(address sender) {
        require(inWhitelist(sender), "Sender not whitelisted");
        _;
    }
            ...
            uint256 transactionFee)
    {
        ...
        newTicketID = _tokenIds.current();
        _ticket.tokenID = newTicketID;
        treasuryLib.setTicketData(newTicketID, _ticket);

        _mint(recipient, newTicketID);
        _setTokenURI(newTicketID, _ticket.tokenURI);
        (serviceFee, resellerFee, transactionFee) = treasuryLib.splitRevenue(_ticket);
        emit SplitRevenue(
            newTicketID,
            msg.value,
            serviceFee,
        ...
        return (newTicketID, serviceFee, resellerFee, transactionFee);
    }
        ...
        override
    {
        ...
        require(credits <= _t.credits, "insufficient credits on NFTicket");

        emit WithDrawCredits(_ticketID, erc20Contract, credits, msg.sender, sendTo);
    }
    ...
    {  
        ...
        emit WithDrawERC20(_ticketID, erc20Contract, amountERC20Tokens, msg.sender, sendTo);
    }

    function topUpTicket(uint256 _ticketID, uint256 creditsAdded, address erc20Token, uint256 numberERC20Tokens)
        external
        override
        returns(uint256 creditsAffordable, uint256 chargedERC20)
    {
        uint32 status;
        (status, creditsAffordable, chargedERC20) = treasuryLib.topUpTicket(_ticketID, creditsAdded, erc20Token, numberERC20Tokens);
    ...
    */
    ...
    function registerResellerServiceProvider(
        ...
        require(
            ...
            "registerResellerServiceProvider: serviceProvider not registered"
        );
    ...
    function getServiceDescriptor(address serviceProvider)
        external
        view
        returns (uint32 descriptor)
    {
        descriptor = treasuryLib.getServiceDescriptor(serviceProvider);
    }
    ...
    {
        contractAddress = treasuryLib.serviceProvidersReseller.get(
            uint256(uint160(serviceProvider))
        );
    ...
    function isServiceProvider(address sContract) public view returns (bool) {
        return (treasuryLib.getServiceDescriptor(sContract) != 0);
    }

    function getServiceProvider(uint32 serviceDescriptor)
        public
        view
        returns (address contractAddress, address walletAddress)
    {
        contractAddress = treasuryLib._providers[serviceDescriptor];
        require(contractAddress != address(0), "undefined serviceProvider");
        walletAddress = treasuryLib
...
library NFTicketLib {
    ...
    struct TicketLib {
        EnumerableSet.AddressSet _whitelistSet;
        mapping(string => DataSchema) _dataSchemata;
        mapping(bytes32 => DataSchema) _schemaHashes;
    }

    function addWhitelist(TicketLib storage _t, address _address) external {
        _t._whitelistSet.add(_address);
    }

    function removeWhitelist(TicketLib storage _t, address _address) public {
        _t._whitelistSet.remove(_address);
    }

    function inWhitelist(TicketLib storage _t, address _address)
        public
        view
        returns (bool)
    {
        return _t._whitelistSet.contains(_address);
    }
    ...
    function isCheckoutService(TicketLib storage , uint32 serviceDescriptor) 
        public
        pure
        returns(bool)
    {
        return( (serviceDescriptor & CHECKOUT_TICKET) == CHECKOUT_TICKET);
    }

    function isCertificate(TicketLib storage , Ticket memory _ticket) 
        public 
        pure
        returns (bool) 
    {
        return ((_ticket.serviceDescriptor & IS_CERTIFICATE) == IS_CERTIFICATE);
    }

    function isTicket(TicketLib storage, Ticket memory _ticket) 
        public 
        pure
        returns (bool) 
    {
        return ((_ticket.serviceDescriptor & IS_TICKET) == IS_TICKET);
    }

    function extractSchemaFromData(TicketLib storage _t, DataRecords calldata _calldata)
        public
        returns (bytes32)
    {
        DataSchema memory _schema;
        bytes32 schemaHash;

        _schema = _calldata._schema;

        _t._dataSchemata[_schema.name] = _schema;
        _t._schemaHashes[schemaHash] = _schema;

        // emit evtSchemaRegistered(_schema.name, _schema);
        return (schemaHash);
    }
...
library NFTreasuryLib {
    ...
    struct Treasury {
         ...
         */
        EnumerableMap.UintToAddressMap resellersServiceProvider;
        EnumerableMap.UintToAddressMap serviceProvidersReseller;
        mapping(uint32 => address) _providers;
        mapping(address => uint32) _serviceDescriptors;
        ...
        returns(Ticket memory)
    {
        ...
        if ( (_t.serviceDescriptor & CASH_VOUCHER) == CASH_VOUCHER ) {
            return(_t);
        } else {
        ...
        internal
    {
     ...
     function _splitRevenue(Treasury storage _treasury, Ticket memory _ticket, uint256 serviceFee, uint256 resellerFee, uint256 transactionFee) 
        ...
        uint32 serviceDescriptor = _ticket.serviceDescriptor;
        if ((serviceDescriptor & CASH_VOUCHER) == CASH_VOUCHER) {
            require( (_ticket.pricePerCredit == 0) &&  (_ticket.credits == 0), "CASH_VOUCHER MUST have ppC == credits == 0");
        }
        if (( serviceDescriptor & IS_CERTIFICATE) == IS_CERTIFICATE) {
            distributeCertificateRevenueToPool(_treasury, _ticket, serviceFee, resellerFee, transactionFee);
        } else if ((_ticket.serviceDescriptor & IS_TICKET) == IS_TICKET) {
            ...
            _msg = string(
                abi.encodePacked(
                    ...
                    "neither CERT nor TICKET"
                )
            );
            require ( ((serviceDescriptor & IS_CERTIFICATE) == IS_CERTIFICATE) || ((_ticket.serviceDescriptor & IS_TICKET) == IS_TICKET), _msg );
        }
    ...
    function splitRevenue(
        ...
        f.add(_ticket.transactionFee); // full share --> BASE
        uint256 v = uint256(_treasury.ratioBASE).mul(1 ether);
        string memory _msg;
        if ( f !=  v ) {
            _msg = "resellerFee= ";
            _msg = string(abi.encodePacked(_msg,Strings.toString(_ticket.resellerFee)," + serviceFee=", Strings.toString(_ticket.serviceFee)," != txBASE=",(_treasury.ratioBASE * (1 ether))));
            require(f == v, _msg);
        }
        ...
        *****/
        ...
        uint256 serviceTransaction = serviceShare.mul(_treasury.TRANSACTIONFEE); // 
        serviceTransaction = serviceTransaction.div(_treasury.ratioBASE); // e.g. 2.7 = 270 * 10/1000
        serviceShare -= serviceTransaction; // e.g. 267.3 = 270 - 2.7; i.e. NET serviceShare AFTER txFEE
        ...
        *****/
        ...
        uint256 resellerTransaction = resellerShare.mul(_treasury.TRANSACTIONFEE); // e.g. 30 * 10 = 300
        resellerTransaction = resellerTransaction.div(_treasury.ratioBASE); // e.g. 0.3 = 300 / 1.000 
        resellerShare -= resellerTransaction; // e.g. 29.7 = 30 - 0.3; i.e. NET serviceShare AFTER txFEE
         ...
         *
         ********************************************/
        verifyAllowances(_treasury, _ticket, serviceShare, resellerShare, transactionShare);
    ...
    function fundTicket(Treasury storage _treasury, Ticket memory _ticket, address, uint256 erc20Tokens)
        public
        returns(uint256 addedTokens)
    {
        uint256 serviceFee = erc20Tokens;
        uint256 resellerFee = 0;
        uint256 transactionFee = 0;


        if ( ! ((_ticket.serviceDescriptor & CASH_VOUCHER) == CASH_VOUCHER) ) {
            string memory _msg;

            _msg = "SP_DESCRIPTOR ";
            _msg = string(abi.encodePacked(_msg, Strings.toString(_ticket.serviceDescriptor), " is not a CASH_VOUCHER "));
            require((_ticket.serviceDescriptor & CASH_VOUCHER) == CASH_VOUCHER, _msg);
            require(_ticket.credits == 0 , "require credits == 0 for CASH_VOUCHER");
        }

        uint256 ticketBalanceBefore = _treasury.remainingTicketBalance[_ticket.tokenID];

        _splitRevenue(_treasury, _ticket, serviceFee, resellerFee, transactionFee);
        ...
        returns(uint32 status, uint256 creditsAffordable, uint256 chargedERC20)
    {
         ...
         if ( (_ticket.serviceDescriptor & CASH_VOUCHER) == CASH_VOUCHER) {
            ...
            uint256 addedTokens = fundTicket(_treasury, _ticket, erc20Contract, erc20Tokens); // credits == 0; ppC == 0
        ...
        require(ticketBalanceAfter.sub(ticketBalanceBefore) == chargedERC20);
        /*****
        *
        ...
        *******/
        ...
        verifyAllowances(_treasury, _ticket, serviceFee, resellerFee, transactionFee);
   ...
   function distributeTicketRevenueToPool(
        ...
        TransferHelper.safeApprove(
            _treasury.ERC20_Token, _treasury.owner, _treasury.transactionPool[_treasury.owner] +
                _treasury.serviceProviderAccount[_ticket.serviceProvider].serviceTicketPool
        );
        // reseller fee for reseller
        address resellerWallet = getResellerWallet(_treasury, _ticket.serviceProvider);
        TransferHelper.safeApprove(_treasury.ERC20_Token, resellerWallet, 
            _treasury.serviceProviderAccount[_ticket.serviceProvider].resellerPool);
    }
    ...
    {
        address resellerWallet = getResellerWallet(_treasury, _ticket.serviceProvider);
        address serviceProviderWallet = getServiceProviderWallet(_treasury, _ticket.serviceProvider);
        // serviceProvider share
        _treasury.serviceProviderAccount[_ticket.serviceProvider].serviceProviderPool 
            += serviceFee;
        TransferHelper.safeApprove(_treasury.ERC20_Token, serviceProviderWallet,
            _treasury.serviceProviderAccount[_ticket.serviceProvider].serviceProviderPool);

        // reseller share
        _treasury.serviceProviderAccount[_ticket.serviceProvider].resellerPool += resellerFee;
        TransferHelper.safeApprove(_treasury.ERC20_Token, resellerWallet, 
            _treasury.serviceProviderAccount[_ticket.serviceProvider].resellerPool);
    }
    ...
    function verifyAllowances(Treasury storage _treasury, Ticket memory _t, uint256 serviceFee, uint256 resellerFee, uint256 transactionFee) 
        internal  
        view
    {
        ...
        if ( !( nfticketAllowance + resellerAllowance + serviceProviderAllowance <= contractBalance )) {
            ...
            _msg = string(abi.encodePacked(_msg, abi.encodePacked(Strings.toString(resellerAllowance)),
                    ")"));
            require(nfticketAllowance + resellerAllowance + serviceProviderAllowance <= contractBalance,_msg);
        }
        // sum of absolute fees are equal to full ticket price
        require((serviceFee + resellerFee + transactionFee) == _t.price, "Absolute fees do not add up.");
        ...
        if ((_t.serviceFee + _t.resellerFee + _t.transactionFee).div(1 ether) != _treasury.ratioBASE) {
            ...
            require(v == _treasury.ratioBASE, _msg);
        }
    ...
    function isRegisteredServiceProvider(
        ...
        return (_treasury._serviceDescriptors[serviceProvider] != 0);
    }

    function registerServiceProvider(
        Treasury storage _treasury,
        address serviceProvider,
        uint32 providerDescriptor,
        address serviceProviderWallet
        ...
        if (isRegisteredServiceProvider(_treasury, serviceProvider)) {
            ...
            _treasury._providers[providerDescriptor] = serviceProvider;
            _treasury._serviceDescriptors[serviceProvider] = providerDescriptor;
            _treasury.serviceProvidersReseller.set(
                _serviceProvider,
                serviceProvider
            );
            _treasury.resellersServiceProvider.set(
                _serviceProvider,
                serviceProvider
            );
            _treasury
            ...
            _treasury
                .serviceProviderAccount[serviceProvider]
                .resellerWallet = serviceProviderWallet;
        }
        return (200);
    }
    ...
    function getServiceDescriptor(
        ...
        return _treasury._serviceDescriptors[serviceProvider];
    }
    ...
    function registerServiceReseller(
        ...
        uint256 serviceProvider = uint256(uint160(_serviceProvider));
        uint256 reseller = uint256(uint160(_reseller));

        _treasury.serviceProvidersReseller.set(serviceProvider, _reseller);
        _treasury.resellersServiceProvider.set(reseller, _serviceProvider);
        _treasury
            .serviceProviderAccount[_serviceProvider]
            .resellerWallet = _resellerWallet;
        _treasury.serviceProviderAccount[_serviceProvider].resellerPool = 0;
...
uint32 constant IS_CERTIFICATE = 0x80000000; // highest bit of CERTT-half-byte = 1
...
interface INFTicket {
    ...
    */
    ...
    function updateTicketData(uint256 tokenId, Ticket memory ticket)
        external;
