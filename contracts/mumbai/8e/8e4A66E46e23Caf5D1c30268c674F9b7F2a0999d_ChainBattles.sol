...
contract ChainBattles is ERC721URIStorage {
    ...
    function generateCharacter(uint256 tokenId) public view returns(string memory){
        ...
        bytes memory svg = abi.encodePacked(
            ...
            '<rect x="0" y="0" width="10%" height="13%" fill="black" />',
            classToSvg[getClass(class)],
            '<text x="2%" y="3%" class="baseClass" >',getClass(class),'</text>',
            '<text x="2%" y="5%" class="baseStat" >', "Level: ",getLevels(tokenId),'</text>',
            '<text x="2%" y="8%" class="baseStat" >', "Strength: ",getStrength(tokenId),'</text>',
            '<text x="2%" y="10%" class="baseStat" >', "Speed: ",getSpeed(tokenId),'</text>',
            '<text x="2%" y="12%" class="baseStat" >', "Life: ",getLife(tokenId),'</text>',
            '</svg>'
        );
        return string(
            abi.encodePacked(
    ...
    function mint() public {
        ...
        tokenIdToCharacter[newItemId] = (Character(1, random(3)+1, random(3)+1, random(3)+1)); // level 1 character with base stats of at least 1.
