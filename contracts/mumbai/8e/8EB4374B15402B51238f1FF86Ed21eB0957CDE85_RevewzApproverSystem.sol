    ...
    UUPSUpgradeable
{
 ...
 event addPayoutWallet(
    ...
    ////@dev this method approves reward to be withdrawn by approvers . This makes it possible for approvers to request their rewards and transfer it to their wallet
        ...
        onlyRole(APPROVER_ADMIN)
    {
        ...
        require(
            balance > maxRewardAmount,
            "Wallet address does not have enough rvz to payout reward"
        );
        // set payout wallet to approver_admin 
        payoutWallet = msg.sender;
        emit addPayoutWallet(
            payoutWallet,
            "successfull added payout wallet on blockchain"
        );
    }
    ...
    // ////@dev this method approves reward to be withdrawn by approvers . This makes it possible for approvers to request their rewards and transfer it to their wallet
    // function approveRewardForWithdrawal(string memory _reviewCID)
    //     public
    //     //internal changing to public , so it could be called externally
    //     onlyRole(APPROVER_ADMIN)
    // {
    ...
    //         "Reward already paid out approver"
    //     );
    //     // prevents users from requesting multiple payments for the same review
    //      require(
    //         approversRewards[_reviewCID].approvedPayout == false,
    //         "Reward already approved to be payedout "
    //     );
    //     // just to ensure we are not sending over 200RVZ as reward to approvers
    //     require(approversRewards[_reviewCID].rewardBalance < maxRewardAmount);
    //     RevewzToken token = RevewzToken(rvzTokenAddress);
    ...
    //     address receipient = approversRewards[_reviewCID].approver;
    //     approversRewards[_reviewCID].approvedPayout = true;
    //     token.approve(receipient, rewardAmt);
    //     emit RewardedApprover(
    //         receipient,
    //         _reviewCID,
    //         rewardAmt,
    //         "successfull approved payed out reward entry on blockchain for approver "
    //     );
     
    // }

    // ////@dev this method transfers rewards from system to approvers wallet. This makes it possible for approvers to request their rewards and transfer it to their wallet
    // function withdrawReward(string memory _reviewCID)
    //     public
    //     //internal changing to public , so it could be called externally
    ...
    //      require(
    //         approversRewards[_reviewCID].approvedPayout == true,
    //         "Reward already not approved for withdrawal"
    //     );
    ...
    //     address receipient = approversRewards[_reviewCID].approver;
    //     approversRewards[_reviewCID].paid = true;
    //     token.transferFrom(payoutWallet,receipient, rewardAmt);

    //     emit RewardedApprover(
    //         receipient,
    //         _reviewCID,
    //         rewardAmt,
    //         "successfull withdrew payed out reward on blockchain for approver "
    //     );
    // }
