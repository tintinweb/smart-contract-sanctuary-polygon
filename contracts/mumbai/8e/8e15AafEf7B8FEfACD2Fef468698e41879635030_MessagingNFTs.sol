...
contract MessagingNFTs is OwnableUpgradeable{
    ...
    function getReceivedMessages () public view returns(Message[] memory){
        ...
        for (uint256  i = 1; i < _messageId.current() + 1 ; i++){
            Message memory _message = messages[i];
            address _cardOwner = IERC721Upgradeable(_message.toContractAddress).ownerOf(_message.toCardId);
    ...
    function getSentMessages () public view returns(Message[] memory) {
        ...
        for (uint256  i = 1; i < _messageId.current() + 1 ; i++){
            Message memory _message = messages[i];
            address _cardOwner = IERC721Upgradeable(_message.fromContractAddress).ownerOf(_message.fromCardId);
