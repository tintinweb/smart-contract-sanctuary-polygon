...
interface ITypeMedical {
    ...
    function searchUser(address _address) external view returns (uint256);
    function checkIsExistType(bytes calldata _type) external view returns(bool);
}
contract TypeMedicalContract is Ownable {
    using Converter for string;

    // Array Type Medical
    bytes[] TypeMedical;
    mapping(bytes => bool) isTypeMedical;

    mapping(address => bytes) userMedical;
    ...
    function searchUser(address _address) public view returns (bytes memory) {
        return userMedical[_address];
    }
    function checkIsExistType(bytes calldata _type) public view returns(bool) {
        return isTypeMedical[_type];
    }
...
contract NFTMarket is ReentrancyGuard, Ownable {
  ...
  function listingNFTtoMarketplace(
    ...
    require(typeMedical.checkIsExistType(_typeMedical),"List type medical not registered");
  ...
  function fetchMarketItems(bytes calldata _typeMedical) public view returns (MarketItem[] memory) {
...
contract BookingSystem is ReentrancyGuard, Ownable {
  ...
  function makeBooking(uint256 _doctorId, uint256 _patientId) public returns(bool) {
    ...
    patientBooking[msg.sender] = bookingID;
