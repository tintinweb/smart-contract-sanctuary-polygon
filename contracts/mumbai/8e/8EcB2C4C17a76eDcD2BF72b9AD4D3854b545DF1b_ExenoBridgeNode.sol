    ...
    ERC1363Payable
{
     ...
     * @param fee - the required fee: first value is denominated in native currency, while the second value is denominated in core token
    ...
    {
        Data memory data = Data(id, network, beneficiary, token, amount, timeout, operation);
        _verifySignature(data, fulfilled, fee, signature);
        _validateDeposit(data, fulfilled, fee);
    ...
    function deposit(
        ...
        external payable nonReentrant whenNotPaused
    {
        (Data memory data, bool fulfilled, uint256[2] memory fee, bytes memory signature) = _unpackSignedData(encodedData);
        _verifySignature(data, fulfilled, fee, signature);
        _validateDeposit(data, fulfilled, fee);
        _processDeposit(data);
    }
        ...
        external payable nonReentrant whenNotPaused
    {
        Data memory data = Data(id, network, beneficiary, token, amount, timeout, operation);
        _verifySignature(data, fulfilled, fee, signature);
        _validateWithdraw(fulfilled, fee);
    ...
    function payout(
        ...
        external onlyRouter
    {
        _processWithdraw(Data(id, network, beneficiary, token, amount, timeout, operation));
    }
    ...
    function _verifySignature(
        ...
        internal view
    {
        bytes memory payload = abi.encodePacked(d.id, d.network, d.beneficiary, d.token, d.amount, d.timeout, d.operation, fulfilled, fee);
        ...
        internal
    {
        ...
        emit Deposit(d.id, d.network, d.beneficiary, d.token, d.amount, d.timeout, d.operation);
    }
        ...
        internal
    {
        ...
        emit Withdraw(d.id, d.network, d.beneficiary, d.token, d.amount, d.timeout, d.operation);
    }
        ...
        internal pure returns(Data memory)
    {
        ...
        ) = abi.decode(encodedData, (bytes32, bytes32[2], address[2], address[2],  uint256[2], uint256[2], bytes32[2]));
        return Data(id, network, beneficiary, token, amount, timeout, operation);
    }
    ...
    function _unpackSignedData(bytes memory encodedData)
        internal pure returns(Data memory, bool, uint256[2] memory, bytes memory)
    {
        (
            bytes32 id,
            bytes32[2] memory network,
            ...
            uint256[2] memory fee,
            bytes memory signature
        ) = abi.decode(encodedData, (bytes32, bytes32[2], address[2], address[2], uint256[2], uint256[2], bytes32[2], bool, uint256[2], bytes));
        return (Data(id, network, beneficiary, token, amount, timeout, operation), fulfilled, fee, signature);
    }
     ...
     */
    function freezeTokens(uint256 amount)
        external onlyOwner nonReentrant
    {
        require(IERC20(_coreToken()).allowance(msg.sender, address(this)) >= amount,
            "ExenoBridgeNode: before freezing owner must approve amount");
        require(IERC20(_coreToken()).transferFrom(msg.sender, address(this), amount),
            "ExenoBridgeNode: ERC20 transferFrom failed");
        frozenTokens += amount;
