...
import "./IDK/ERC20BasicApproveExtension.sol";
import "./IDK/WithAdmin.sol";
import "./IDK/WithSuperOperators.sol";
import "./CoM.sol";
import './CoMv2.sol';

pragma solidity ^0.8.0;

contract CoMv3 is CoMv2 {
    ...
    function init_tf(address tf) external reinitializer(3) {
        __ERC2771Handler_initialize(tf);
    }
}
...
abstract contract ERC20BasicApproveExtension is ERC20Internal, ContextUpgradeable {
...
contract CoM is Initializable, ERC20Upgradeable, OwnableUpgradeable {

    function initialize(address beneficiary,uint256 amount) external initializer {
        __ERC20_init("Citizen of Metaland", "CoM");
        __Ownable_init();
...
contract CoMv2 is CoM,ERC20BasicApproveExtension, WithAdmin, WithSuperOperators
{
    ...
    function transferFrom(
        ...
        if (_msgSender() != from && !_superOperators[_msgSender()] && _msgSender() != _admin) {
            uint256 currentAllowance = allowance(from,_msgSender());
            if (currentAllowance != ~uint256(0)) {
                // save gas when allowance is maximal by not reducing it (see https://github.com/ethereum/EIPs/issues/717)
                require(currentAllowance >= amount, "NOT_AUTHORIZED_ALLOWANCE");
                _approveFor(from,_msgSender(),currentAllowance - amount);
            }
        }
        _transfer(from, to, amount);
        return true;
    ...
    function approveFor(
        ...
        require(_msgSender() == owner || _superOperators[_msgSender()] || _msgSender() == _admin, "NOT_AUTHORIZED");
        _approveFor(owner, spender, amount);
        return true;
    }

    function init_admin(address admin) external reinitializer(2) {
        _admin = admin;
    }
}
