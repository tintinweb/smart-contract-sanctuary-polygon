...
contract ManifestoSeries is ERC721, Ownable {
    ...
    constructor(string memory baseURI, address projectAddress) ERC721("ManifestoSeries", "MFS") {
        setRoyalties(projectAddress, 500);
    ...
    function mintNFTs(address _to, uint256 _number) public payable {
    ...
    function _mintSingleNFT(address _to) internal {
      uint256 new_index = index + 1;
      uint newTokenID = uint256(keccak256(abi.encodePacked(new_index, msg.sender, block.timestamp, blockhash(block.number-1))));
      _safeMint(_to, _pickRandomUniqueId(newTokenID));
      emit NFTMinted(_to, _pickRandomUniqueId(newTokenID));
    }
    ...
    uint256 new_index = index + 1;
    uint256 len = ids.length - new_index;
