...
* updates referralRecordMap (Element referringAddress) (Entity depositedAmt) as (referralRecordMap with element referringAddress with element depositedAmt) + (5000000000000000000 * _amt / 1000000000000000000)
* updates referringAddress as referralRecordMap with element referringAddress with element referringAddress
* if referringAddress is equals to Address 0 then ()
* updates referralRecordMap (Element referringAddress) (Entity depositedAmt) as (referralRecordMap with element referringAddress with element depositedAmt) + (2500000000000000000 * _amt / 1000000000000000000)
* updates referringAddress as referralRecordMap with element referringAddress with element referringAddress
*/
	function addReferral(uint256 _amt, address _referringAddress) internal {
		...
		referralRecordMap[referringAddress].depositedAmt  = (referralRecordMap[referringAddress].depositedAmt + 5000000000000000000 * _amt / 1000000000000000000);
		referringAddress  = referralRecordMap[referringAddress].referringAddress;
		if ((referringAddress == address(0))){
			return;
		}
		referralRecordMap[referringAddress].depositedAmt  = (referralRecordMap[referringAddress].depositedAmt + 2500000000000000000 * _amt / 1000000000000000000);
		referringAddress  = referralRecordMap[referringAddress].referringAddress;
	}
...
*/
	function unstake0(uint256 _unstakeAmt) public {
		...
		require(((block.timestamp - (300 * 864)) >= thisRecord.stakeTime), "Insufficient stakeperiod");
