...
contract XillionQuiz is Ownable, ReentrancyGuard {
    ...
    */
     ...
     */
    mapping(RewardRole => uint256) rewardRoleToMultiplier;
     ...
     */
    function createQuiz(address quizTokenAddess, uint256 tokensRewardAmount, uint256 tokensAmountPerWallet) external onlyModerator {
     ...
     */
    function transferTokensToQuiz(uint256 quizId_, uint256 tokensAmount_) public {
        ...
        if(quiz_.currentTokensAmount >= quiz_.tokensRewardAmount){
            ...
            emit QuizStateChanged(quiz_.quizState);
        }
    ...
    function claim(uint256 quizId_, uint256 stakingStatus_, bytes calldata signature_) external nonReentrant{
        ...
        if(quiz_.currentTokensAmount == 0){
            ...
            emit QuizStateChanged(QuizState.Done);
        }
     ...
     * @param userAddress_ Id of voting
     * @param tokenAmount_ how much tokens user claimed
     */
    event Claimed(
        uint256 userAddress_,
