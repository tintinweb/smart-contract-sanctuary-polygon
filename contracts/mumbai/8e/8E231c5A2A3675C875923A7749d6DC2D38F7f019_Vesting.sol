...
contract Vesting {
    ...
    uint256 public constant TGE_UNLOCK = 10; // 10% : released percent at TGE stage
    uint256 public constant UNLOCK_UNIT = 30; // 30% of the total allocation will be unlocked
    uint256 public constant CLIFF_PERIOD = 10 minutes; // cliff period
    ...
    constructor(address _SBCToken, address _presaleContract, address payable _multiSigAdmin) {
        owner = msg.sender;
    ...
    function getTGETime() public view returns (uint256) {
        return presaleContract.startTime().add(presaleContract.PERIOD()).add(5 minutes);
    }
    ...
    function getVested(address beneficiary) public view virtual returns (uint256 _amountVested) {
        ...
        uint256 secondVestingPoint = firstVestingPoint.add(10 minutes);
        uint256 thirdVestingPoint = secondVestingPoint.add(10 minutes);
