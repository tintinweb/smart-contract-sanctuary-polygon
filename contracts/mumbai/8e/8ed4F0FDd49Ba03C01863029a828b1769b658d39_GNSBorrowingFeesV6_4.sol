...
contract GNSBorrowingFeesV6_4 is Initializable, GNSBorrowingFeesInterfaceV6_4 {
    ...
    function _setPairParams(uint pairIndex, PairParams calldata value) private {
        ...
        emit PairParamsUpdated(pairIndex, value.groupIndex, value.feePerBlock, value.feeExponent);
    }

    // Manage pair max OIs
    function setPairMaxOi(uint pairIndex, uint72 value) external onlyManager {
        _setPairMaxOi(pairIndex, value);
    }

    function setPairMaxOiArray(uint[] calldata indices, uint72[] calldata values) external onlyManager {
        uint len = indices.length;
        require(len == values.length, "WRONG_LENGTH");

        for (uint i; i < len; ) {
            _setPairMaxOi(indices[i], values[i]);
            unchecked {
                ++i;
            }
        }
    ...
    function _setPairMaxOi(uint pairIndex, uint72 value) private {
        _setPairPendingAccFees(pairIndex, ChainUtils.getBlockNumber());
        pairOis[pairIndex].max = value;
        emit PairMaxOiUpdated(pairIndex, value);
    }
