...
{
  bytes32 constant KEY_FRONTCOLOR = keccak256('frontColor');
  bytes32 constant KEY_BACKCOLOR = keccak256('backColor');
  bytes32 constant KEY_RADIUS = keccak256('radius');
  ...
  function mint(string memory frontColor, string memory backColor, string memory radius) public onlyOwner {
        ...
        _setValue(tokenId, KEY_FRONTCOLOR, abi.encodePacked(frontColor));
        _setValue(tokenId, KEY_BACKCOLOR, abi.encodePacked(backColor));
        _setValue(tokenId, KEY_RADIUS, abi.encodePacked(radius));
  }
 
  function tokenName(uint256 tokenId) internal view override returns (string memory){
      return string(abi.encodePacked(name(), ' #', tokenId));
  }
  ...
  function tokenProperties(uint256 tokenId) internal view override returns (string[] memory trait_type, string[] memory trait_value, string[] memory trait_display){   
      ...
      trait_type[0] = 'Front Color'; trait_value[0] = string(_getValue(tokenId, KEY_FRONTCOLOR));
      trait_type[1] = 'Background Color'; trait_value[1] = string(_getValue(tokenId, KEY_BACKCOLOR));
      trait_type[2] = 'Radius'; trait_value[2] = string(_getValue(tokenId, KEY_RADIUS));
  }
...
{     
    ...
    * @param trait_value array, the value of the traits.
    * @param trait_display array, the type of the traits.
    * @param background_color of the token.
    ...
    */
    function parametriceURL(string memory baseURL, string[] memory keys, string[] memory values) public pure returns(string memory){
        ...
        return string(abi.encodePacked(baseURL, baseURIbytes[baseURIbytes.length - 1] != bytes1('/') ? '/' : '', '?', parameters));
    }
...
/**
* @title ERC721 with On-Chain Metadata
* 
*/
abstract contract ERC721OnChainMetadata is ERC721, NFTOnChainTokenMetadata, NFTOnChainContractMetadata
{
  ...
  /*
  function tokenName(uint256 tokenId) internal view virtual override returns (string memory){
      return string(abi.encodePacked(name(), ' #', tokenId));
  }

  function tokenDescription(uint256 tokenId) internal view virtual override returns (string memory){
      tokenId;
      return "";
  }

  function tokenImageURI(uint256 tokenId) internal view virtual override returns (string memory){
      tokenId;
      return "";
  }
  ...
  function tokenProperties(uint256 tokenId) internal view virtual override returns (string[] memory trait_type, string[] memory trait_value, string[] memory trait_display){
      tokenId;
      trait_type = new string[](0);
      trait_value = new string[](0);
      trait_display = new string[](0);
  }
*/
...
/*
  function contractName() internal view virtual override returns (string memory){
      return name();
  }
  
  function contractDescription() internal pure virtual override returns (string memory){
      return "";
  }

  function contractImageURI() internal pure virtual override returns (string memory){
      return "";
  }

  function contractExternalURL() internal pure virtual override returns (string memory){
      return "";
  }

  function contractSellerFee() internal pure virtual override returns (uint256){
      return 0;
  }

  function contractFeeRecipient() internal pure virtual override returns (address){
      return address(0);
  }
*/
...
abstract contract ERC721HtmlBased is ERC721OnChainMetadata
{
  string internal _baseURL;
  mapping(uint256 => bytes32[]) internal _keys;
  mapping(uint256 => bytes[]) internal _values;

  constructor(string memory name, string memory symbol, string memory baseURL) ERC721OnChainMetadata(name, symbol){
    _baseURL = baseURL;
  }
  ...
  function tokenAnimationURL(uint256 tokenId) internal view virtual override returns (string memory){
    ...
    string[] memory values = new string[](keysBytes.length);
    for (uint256 i = 0; i < keysBytes.length; i++) {
      keys[i] = string(abi.encodePacked(keysBytes[i]));
      values[i] = string(abi.encodePacked(valuesBytes[i]));
    }
