...
contract YieldFarmLP {
    ...
    function harvest (uint128 epochId) external returns (uint){
        ...
        require (_getEpochId() > epochId, "YieldFarmLP: this epoch is in the future");
        require(epochId <= NR_OF_EPOCHS, "YieldFarmLP: maximum number of epochs is 1000");
        require (lastEpochIdHarvested[msg.sender].add(1) == epochId, "YieldFarmLP: harvest in order");
    ...
    function _initEpoch(uint128 epochId) internal {
        require(lastInitializedEpoch.add(1) == epochId, "YieldFarmLP: epoch can be init only in order");
    ...
    function _harvest (uint128 epochId) internal returns (uint) {
        ...
        // if it fails an AnalMusk account will need to init the epochs manually
        ...
        // exit if there are no stake in the epoch
        if (epochs[epochId] == 0) {
            return 0;
        }
    ...
    // compute epoch id from block timestamp and epoch start date
