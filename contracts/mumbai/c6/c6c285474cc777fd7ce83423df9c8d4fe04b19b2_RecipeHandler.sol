...
interface IMintable721 {
  ...
  function studioMint(address to, uint256 mintAmount) external;
}
...
bytes32 constant CHEST_REQUEST = keccak256("Chest");
bytes32 constant UPGRADE_REQUEST = keccak256("Upgrade");
bytes32 constant SUMMON_REQUEST = keccak256("Summon");
...
contract RecipeHandler is Ownable, StudioSignature {
  uint32 private immutable chainId;
  IBgem public immutable bgem;
  IBoom public immutable boom;
  IMintable721 public immutable hunters;
  IMintable1155 public immutable perks;
  IMintable1155 public immutable shards;
  IMintable1155 public immutable equipments;
  ...
  event UpgradeSuccess(address indexed account, uint256 hunterId);
  event SummonSuccess(address indexed account, uint256[] hunterIds);
  event ApplyArtifactSuccess(
    address indexed account, uint256 artifactId, uint256 hunterId, uint256 slot
  );
  ...
  constructor(
    ...
    IBgem initBgem,
    IBoom initBoom,
    IMintable721 initHunters,
    IMintable1155 initPerks,
    IMintable1155 initEquipments,
    IMintable1155 initShards
  ) {
    require(initStudio != address(0), "Invalid studio");
    studio = initStudio;
  ...
  ///      outIds           uint256[ ]  *
  ///      outAmts          uint256[ ]  *
  ///      nonce            uint32      To differentiate between signatures
  ...
  function openChest(RecipeSignature calldata packet) external {
    require(packet.chainId == chainId, "Invalid chain ID");
    require(packet.request == CHEST_REQUEST, "Invalid request");
    ...
    emit ChestOpened(user, hunterRarities, outHunters, outSizes[0], outSizes[1], outSizes[2]);

    // IN/OUT  - BGems
    if (inBgem > outBgem) {
      require(bgem.transferFrom(user, studio, inBgem - outBgem), "Bgem transfer failed");
    } else if (outBgem > inBgem) {
      bgem.mint(user, outBgem - inBgem);
    }

    // OUT - Hunters
    for (uint256 i = 0; i < outHunters.length; ++i) {
      hunters.studioMint(user, outHunters[i]);
    }
    ...
    for (uint256 i = 0; i < 3; ++i) {
      ...
      for (; sizeCtr < nextSize; sizeCtr++) {
        if (i == 0) {
          perks.mint(user, outIds[sizeCtr], outAmts[sizeCtr], "");
        } else if (i == 1) {
          shards.mint(user, outIds[sizeCtr], outAmts[sizeCtr], "");
        } else {
          equipments.mint(user, outIds[sizeCtr], outAmts[sizeCtr], "");
        }
      }
    }
  }
  ...
  function upgrade(RecipeSignature calldata packet) external {
    ...
    (
      ...
      abi.decode(packet.payload, (address, uint256, uint256, uint256[], uint256[], uint256, uint32));
    ...
    // IN - Booms
    require(boom.transferFrom(user, studio, inBoom), "Boom transfer failed");
  ...
  ///      (user, 100, [3], [5], [12])
  /// @dev Emits {SummonSuccess}
  function summon(RecipeSignature calldata packet) external {
    require(packet.chainId == chainId, "Invalid chain ID");
    require(packet.request == UPGRADE_REQUEST, "Invalid request");
    ...
    (
      ...
      packet.payload, (address, uint256, uint256[], uint256[], uint256[], uint256, uint32)
    );

    require((expiresAt == 0) || block.timestamp > expiresAt, "Signature expired");

    emit SummonSuccess(user, outHunterIds);
    ...
    for (uint256 i = 0; i < inEquipmentIds.length; ++i) {
      equipments.safeTransferFrom(user, studio, inEquipmentIds[i], inEquipmentAmts[i], "");
    }

    // OUT - Hunters
    for (uint256 i = 0; i < outHunterIds.length; ++i) {
      hunters.studioMint(user, outHunterIds[i]);
    }
  }
  ...
  function applyArtifact(RecipeSignature calldata packet) external {
    ...
    (address user, uint256 hunterId, uint256 artifactId, uint256 slot,) =
      abi.decode(packet.payload, (address, uint256, uint256, uint256, uint32));

    emit ApplyArtifactSuccess(user, artifactId, hunterId, slot);

    require(hunters.ownerOf(hunterId) == user, "Not the owner of hunter");
