...
interface IGen2PlayerToken is IERC721Upgradeable {
    ...
    function mint(uint256, uint256, address) external returns (uint256 _tokenId);
}
...
abstract contract Staker is StakeValidator {
    ...
    /**
     * @notice Allows user to change his team lineup in one transaction
     * @dev Uses unstakePlayer() for each id in _tokenIdsToUnstake and stakePlayer() for _tokenIdsToStake.
     * @dev Caller must be registered user.
     * @param _tokenIdsToUnstake An array of token IDs to unstake.
     * @param _tokenIdsToStake An array of token IDs to stake.
     */
    function changeLineup(uint256[] calldata _tokenIdsToUnstake, uint256[] calldata _tokenIdsToStake) external {
        // Unstake players
        for (uint256 i = 0; i < _tokenIdsToUnstake.length; ++i) {
            _unstakePlayer(_tokenIdsToUnstake[i], _msgSender());
        }

        // Stake players
        for (uint256 i = 0; i < _tokenIdsToStake.length; ++i) {
            _stakePlayer(_tokenIdsToStake[i], _msgSender());
        }
    }

    /**
     * @notice Allows relayer to change user's team lineup in one transaction
     * @dev Uses unstakePlayer() for each id in _tokenIdsToUnstake and stakePlayer() for _tokenIdsToStake.
     * @dev Caller must be registered Relayer.
     * @param _tokenIdsToUnstake An array of token IDs to unstake.
     * @param _tokenIdsToStake An array of token IDs to stake.
     * @param _user Address on whose behalf acting relayer
     */
    function changeLineupForUser(
        uint256[] calldata _tokenIdsToUnstake,
        uint256[] calldata _tokenIdsToStake,
        address _user
    ) external onlyRole(RELAYER_ROLE)  {
        // Unstake players
        for (uint256 i = 0; i < _tokenIdsToUnstake.length; ++i) {
            _unstakePlayer(_tokenIdsToUnstake[i], _user);
        }

        // Stake players
        for (uint256 i = 0; i < _tokenIdsToStake.length; ++i) {
            _stakePlayer(_tokenIdsToStake[i], _user);
        }
    }
