...
contract Points is Ownable {
    ...
    function seeHistory(bytes32 _id) external view returns (OperationsHistory[] memory) {
        ...
        for (uint256 i = 0; i <= lastOperation; i++){
            ...
            foundMatches++;
                // add to array / struct / any structure that can contain history
            }
        }

        return res;

        //return the array / struct / structure

    }

    function checkLastOperation() external view returns (bytes32){
        return bytes32(lastOperation);
        }
