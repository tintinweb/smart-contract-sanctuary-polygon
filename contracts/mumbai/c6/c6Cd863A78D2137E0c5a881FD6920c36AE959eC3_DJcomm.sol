...
contract DJcomm is ERC721Drop, PermissionsEnumerable {
    ...
    mapping(uint256 => MappingRecord[]) public dailyMapping;
    mapping(uint256 => uint256) public dailyTotalCount;
    ...
    function bulkUpdateMapping(
        uint256[] calldata keys,
        uint256[] calldata to,
        uint256[] calldata timestamps
    ) public {
        require(
            keys.length == to.length &&
                keys.length == timestamps.length,
            "Invalid input lengths"
        );

        uint256[] memory uniqueKeys = keys;
        uint256[] memory uniqueTo = to;
        uint256[] memory uniqueTimestamps = timestamps;
        ...
        for (uint256 i = 0; i < uniqueKeys.length; i++) {
            MappingRecord memory record = MappingRecord(uniqueTo[i], uniqueTimestamps[i]);
            dailyMapping[uniqueKeys[i]].push(record);
        }
    }

    function dailyClaimTotal(
        uint256 timestamps,
        uint256 total
    ) public {
        require(timestamps > 0, "Invalid timestamp");

        // Sort the timestamps array
        uint256[] memory sortedTimestamps = new uint256[](1);
        sortedTimestamps[0] = timestamps;
        //ArrayUtils.sort(sortedTimestamps);

        // Update the totals
        for (uint256 i = 0; i < sortedTimestamps.length; i++) {
            dailyTotalCount[sortedTimestamps[i]] += total;
        }
    }
