    ...
    UUPSUpgradeable
{
    ...
    modifier onlyRole(string memory name, address account) {
        require(LCRBALinker.checkAccountRole(name, account), 'LCRBA: Unauthorized');
        _;
    }
    ...
    function initialize(LostChainForwarder forwarder, address rbaLinkerAddress) public initializer {
        require(address(forwarder) != address(0x0), 'LCTM: Forwarder is zero address');
        require(rbaLinkerAddress != address(0x0), 'LCTM: RBA linker is zero address');
        LCRBALinker = ILostChainRBA(rbaLinkerAddress);
    ...
    function mint(address to, uint256 amount) public onlyRole('MINTER', _msgSender()) {
        _mint(to, amount);
    }
    ...
    function TESTING31() external pure returns (bool) {
        return true;
    }
}
