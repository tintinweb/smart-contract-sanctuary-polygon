...
contract MLDStaking is Ownable {
    ...
    function calculateMaturityAmount(
        ...
        return (effectiveAPY * _stakedMLD) / 1e12;
    }
    ...
    function stake(uint256 _mldAmount, uint256 _stakeDurationInSec) external {
        ...
        userDeposits[msg.sender].push(
            DepositInfo(
                ...
                _mldAmount,
                _requiredLord,
                calculateMaturityAmount(_mldAmount, _stakeDurationInSec)
            )
        );
    ...
    /**
     * @dev Withdraw staked tokens along with the rewards if matured just burning some lord as fees 
            or charge penalty fees in MLD & burn staked LORD if not matured
     * @param _index User' stake index
     */
    function withdraw(uint256 _index) external {
        address user = msg.sender;

        require(_index < userDeposits[user].length, "Invalid Index");
        DepositInfo storage userDepositInfo = userDeposits[user][_index];
        ...
        userTotalDeposits[user].totalMLDStaked -= userStakedMLD;
        userTotalDeposits[user].totalLordStaked -= _lordStaked;

        userDeposits[user][_index] = userDeposits[user][
            userDeposits[user].length - 1
        ];
        userDeposits[user].pop();

        if (block.timestamp >= userDepositInfo.maturity) {
            // Unstake if the investment is matured
            uint256 returnAmount = userDepositInfo.maturityAmount;
            uint256 lordFees = (_lordStaked * unstakeLordFeesPercentage) / 100;

            mldToken.transfer(user, returnAmount);
            lordToken.burn(lordFees);
            lordToken.transfer(user, _lordStaked - lordFees);

            emit Unstaked(user, returnAmount);
        } else if (block.timestamp < userDepositInfo.maturity) {
            // Pre matured withdrawal, charge penalty fees in MLD and burn staked LORD
            uint256 mldPenalty = (userStakedMLD * preMatureMLDPenalty) / 100;
            uint256 withdrawableAmount = userStakedMLD - mldPenalty;

            mldToken.transfer(user, withdrawableAmount);
    ...
    function userLatestId(address _user) public view returns (uint256) {
        return userDeposits[_user].length - 1;
    }
