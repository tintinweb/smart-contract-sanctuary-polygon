...
contract Finde {
    ...
    function getPaginatedURIs(
        ...
        _offset = _offset + 1; // increase offset by 1 to ignore the first index
    ...
    {
        emit UpVoted(msg.sender, _providerAddress, indexToProvider[addressToIndex[msg.sender]].lastVoteTime );
    ...
    function downVoteProvider(address _providerAddress)
        ...
        EligibleToVote(_providerAddress)
    {
        emit DownVoted(msg.sender, _providerAddress, indexToProvider[addressToIndex[msg.sender]].lastVoteTime );
    ...
    function getRemainingTimeForVote() external view returns (int256) {
        int256 timeLeft = int256(
            (indexToProvider[addressToIndex[msg.sender]].lastVoteTime +
                votingWaitTime) - block.timestamp
        );
        if (timeLeft >= 0) {
            return timeLeft;
        }
