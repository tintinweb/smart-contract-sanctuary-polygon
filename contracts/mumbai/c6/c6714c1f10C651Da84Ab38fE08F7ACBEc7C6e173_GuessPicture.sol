...
contract GuessPicture is PriceFeed, Ownable {
    ...
    function newGame() public payable {
        ...
        require(
            rePointGames.userRegistered(userAddr),
            "first you have to register in rePointGames"
        );
        ...
        round.userGames[msg.sender].push(Game(enterPriceUSD, 0, 0));
        round.userRound[msg.sender].entrance += enterPriceUSD;
    ...
    function score(address userAddr, uint256 points) public onlyOwner {
        ...
        require(
            len > 0,
            "user has no games in this round"
        );
    ...
    function info() public view returns(string memory temp) {
        Round storage round = _rounds[roundCounter];
        temp = string.concat(
            "GuessPicture : {",
                ...
                " ,roundEnteredUSD : ", round.totalEntered.toString(),
    ...
    function userInfo(string memory username) public view returns(string memory temp) {
        address userAddr = rePointGames.userAddr(username);
        Round storage round = _rounds[roundCounter];
        temp = string.concat(
            "GuessPicture : {",
...
interface IFactory {
    function userRegistered(address userAddr) external view returns(bool);
