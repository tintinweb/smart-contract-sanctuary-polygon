...
contract AdminStorage is Storage {
    ...
    event UpdateWriter(address indexed newWriter);
    event UpdateFeePercentage(uint256 newFee);

    constructor(address serviceWalletAddress, address supervisor, address betWriter)
        Storage(serviceWalletAddress, supervisor, betWriter)
    {}
    ...
    function setWriter(address newWriter) public onlyOwner {
        _betWriter = newWriter;
        emit UpdateWriter(newWriter);
    }
    ...
    function cancelBet(string calldata _fixtureStringId) external onlyOwner {
        uint256 fixtureId = uint256(keccak256(abi.encodePacked(_fixtureStringId)));
        Fixture storage fixture = idToFixture[fixtureId];
        fixture.isFinished = true;

        for (uint256 i = 0; i < fixture.betters[Outcome.HOME].length; i++) {
            address better = fixture.betters[Outcome.HOME][i];
            fixture.bets[Outcome.HOME][better].won = true;
            fixture.bets[Outcome.HOME][better].gains = fixture.bets[Outcome.HOME][better].amount;
        }

        for (uint256 i = 0; i < fixture.betters[Outcome.AWAY].length; i++) {
            address better = fixture.betters[Outcome.AWAY][i];
            fixture.bets[Outcome.AWAY][better].won = true;
            fixture.bets[Outcome.AWAY][better].gains = fixture.bets[Outcome.AWAY][better].amount;
        }

        for (uint256 i = 0; i < fixture.betters[Outcome.TIE].length; i++) {
            address better = fixture.betters[Outcome.TIE][i];
            fixture.bets[Outcome.TIE][better].won = true;
            fixture.bets[Outcome.TIE][better].gains = fixture.bets[Outcome.TIE][better].amount;
        }
    }
    ...
    function pushFixtureIds(uint256 id) external onlySupervisorOrWriter {
        _fixtureIds.push(id);
    }

    function setIdToFixtureStringId(uint256 id, string memory stringId)
        external
        onlySupervisorOrWriter
    {
    ...
    function setIdToFixtureWinningOutcome(uint256 id, Outcome winningOutcome)
        external
        onlySupervisor
    {
    ...
    function setIdToFixtureTotalPoolSize(uint256 id, uint256 totalPoolSize)
        external
        onlySupervisor
    {
    ...
    function pushIdToFixtureBetters(
        ...
        Outcome outcome,
        address address_
    ) external onlySupervisorOrWriter {
    ...
    function addIdToFixtureTotalPoolSize(uint256 id, uint256 value)
        external
        onlySupervisorOrWriter
    {
...
abstract contract Storage is IStorage, Ownable {
    ...
    modifier onlySupervisorOrWriter() {
        require(msg.sender == _supervisor || msg.sender == _betWriter, "Not supervisor or writer");
        _;
    }
    ...
    function getServiceWalletAddress() external view onlyOwner returns (address) {
    ...
    function getIdToFixtureBetters(uint256 id, Outcome outcome)
        external
        view
    ...
    function getIdToFixtureBettersLength(uint256 id, Outcome outcome)
        external
        view
        returns (uint256)
    {
    ...
    function getWriter() external view onlyOwner returns (address) {
        return _betWriter;
    }
}
