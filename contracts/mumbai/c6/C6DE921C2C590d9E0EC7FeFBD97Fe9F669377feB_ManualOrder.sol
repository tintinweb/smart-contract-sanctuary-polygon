...
contract ManualOrder is OrderBase {
    ...
    function createManualOrder(TccItem[] calldata _tccList, uint256 _ethAmount) external nonReentrant payable {
        address _user = msg.sender;
        //require(msg.value == _ethAmount, "U01");
        uint256 _wethAmount = 0;
        uint256 _num = _tccList.length;
    ...
    function doManualOrder(address _user, bytes32 _md5) external nonReentrant {
        ...
        address _to = y.tcc._swapRouter2.length > 0 ? address(this) : _user;
        if (y.tcc._swapRouter2.length > 0) {
            ...
            if (y.tcc._swapRouter[y.tcc._swapRouter.length - 1] == weth) {
            ...
            //成交在执行的价格区间
            require(x.swapInAmount.mul(10 ** 18) > y.tcc._limitItem._swapPriceZone[0].mul(_in) && x.swapInAmount.mul(10 ** 18) < y.tcc._limitItem._swapPriceZone[1].mul(_in), "P09");
        }
        //兼容单向兑换
        if (y.tcc._swapRouter2.length > 0) {
            //收取一部分代币B
