...
contract BLKSwap is Ownable {
    ...
    event SwapIntoBLK(address indexed account, address tokenIn, uint256 depositedAmount, uint256 price);
    ...
    function getPriceFromOracle(address _token) public view returns (uint256) {
        ...
        if(oracle != address(0)) {
            ...
            return latestPriceConverted.div(10**tokenDecimals);
        } else {
            return 0;
        }
    }
    ...
    function estimateTokenIn(address _token, uint256 _amount) public view returns (uint256) {
        return _amount.mul(getPriceFromOracle(_token));
    }

    function estimateEthIn(uint256 _amount) public view returns (uint256) {
        return _amount.mul(getPriceFromOracle(address(0)));
    }

    function swapTokenIn(address _token, uint256 _amount) public {
        require(_amount > 0, "incorrect amount sent");
        emit SwapIntoBLK(msg.sender, _token, _amount, getPriceFromOracle(_token));
    }

    function swapEthIn() public payable {
        require(msg.value > 0, "incorrect amount sent");
        emit SwapIntoBLK(msg.sender, address(0), msg.value, getPriceFromOracle(address(0)));
    }
    ...
    function withdrawToken(address _token) public onlyOwner() {
        ERC20(_token).transfer(msg.sender, ERC20(_token).balanceOf(msg.sender));
    }
