...
// File: contracts/dotWAGMItest.sol
...
contract dotWAGMI is ERC721, ERC721URIStorage, ERC721Burnable, Ownable {
    ...
    uint internal minUserLength = 3;
    uint internal UID = 10000; //This is used to set unique DId to each wallet group
    ...
    function setMinLength(uint newLength) public onlyOwner {
        minUserLength = newLength;
    }
    ...
    function addLinked(address[] memory linkedAdd, bytes memory sigHash, bytes32 mesHash) public {
        ...
        DIdToUserWallets[addressToDId[msg.sender]].linkedAddresses.push(linkedAdd[i]);
        }
    }
    ...
    function transferHandler(address f, address t, uint256 id) internal {
        if (addressToTokenId[f] == addressToTokenId[t]) {
            ...
            address[] memory prevLinked = DIdToUserWallets[addressToDId[f]].linkedAddresses;
            for (uint i=0; i < prevLinked.length; i++) {
            addressToTokenId[prevLinked[i]] = 0;
            }
            addressToTokenId[DIdToUserWallets[addressToDId[f]].defaultAddress] = 0;
    ...
    function setDId(address toSet, string memory usernameToSet, uint tokenIdInvolved) internal {
        if (addressToDId[toSet] == 0) {
                ...
                addressToTokenId[DIdToUserWallets[addressToDId[toSet]].defaultAddress] = tokenIdInvolved;
            }
            else {
