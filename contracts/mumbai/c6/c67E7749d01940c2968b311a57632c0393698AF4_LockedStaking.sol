    ...
    ERC1155HolderUpgradeable
{
    ...
    uint256 public StakePeriod = 730 days;
    uint256 public OmmniverseReward = 50;
    uint256 public FractionsReward = 40;
    ...
    modifier onlyAllowedCollections() {
        require(allowedCollections[msg.sender]);
        _;
    }
    ...
    /// Owner Functions ///
    function updateAllowedCollections(
        address _collection,
        bool _isAllowed
    ) external onlyOwner {
    ...
    function updateStakePeriod(uint256 _new) external onlyOwner {
        StakePeriod = _new;
    }

    function updateOmmniverseReward(uint256 _newReward) external onlyOwner {
        OmmniverseReward = _newReward;
    }

    function updateFractionReward(uint256 _newReward) external onlyOwner {
        FractionsReward = _newReward;
    }
    ...
    function nftStake(
        address _collection,
        uint256 _tokenId
    ) external nonReentrant onlyAllowedCollections {
        require(
    ...
    function unStakeNft(uint256 _id) external {
        ...
        require(
            nftStakers[_id].joined + StakePeriod <= block.timestamp,
    ...
    function unStakeFration(uint256 _id) external {
        ...
        require(
            fractionStakers[_id].joined + StakePeriod <= block.timestamp,
            "Fraction Staking not completed"
        );
