...
contract PartnersOW {
    ...
    for (uint256 i = 0; i < _totalSupply(); i++) {
        ...
        if (investment > 0) {
            ...
            uint256 initialInvestment = investment - profitPayout;
            uint256 totalPayout = initialInvestment + profitPayout;

            // Transfer funds from contract's balance to investor
            payable(investor).transfer(totalPayout);

            // Deduct the profit payout from the investor's investment
            _investments[investor] -= profitPayout;
        }
    }
}
