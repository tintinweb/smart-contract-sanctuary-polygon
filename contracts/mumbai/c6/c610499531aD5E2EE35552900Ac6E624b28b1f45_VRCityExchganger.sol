...
contract VRCityExchganger is Ownable, ReentrancyGuard {
        ...
        returns (uint256 amountOut)
    {
        ...
        IERC20(wmatic).safeApprove(swapRouter, amountMatic);
        uint256 amountUSDC = ISwapRouter(swapRouter).exactInputSingle(
            ISwapRouter.ExactInputSingleParams({
                tokenIn: wmatic,
                tokenOut: usdc,
                fee: feePool,
                recipient: address(this),
                amountIn: amountMatic,
                amountOutMinimum: 0,
                sqrtPriceLimitX96: 0
            })
        );
        ...
        IERC20(address(vrcity)).safeTransfer(msg.sender, amountOut);
    }
