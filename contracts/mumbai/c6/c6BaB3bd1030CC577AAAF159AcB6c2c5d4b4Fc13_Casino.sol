 ...
 */
contract Casino is AccessControlUpgradeable, EIP712Upgradeable {
    ...
    struct AggregatedBets {
        uint256 spent;
        uint256 gained;
        uint256 margin;
        uint256 miningRate;
        uint256 lfiExchangeRate;
        uint32 coinIndex;
        address user;
    }
     ...
     *
     * @param aggregatedBets Array of aggregated bets.
     ...
     */
    function syncBets(
        AggregatedBets[] calldata aggregatedBets,
        ...
        onlyRole(AGENT_ROLE)
    {
        ...
        _syncBets(aggregatedBets, batchId);
    }
    ...
    function _syncBets(
        ...
        internal
    {
        uint256 numAggregatedBets = aggregatedBets.length;

        for (uint256 i = 0; i < numAggregatedBets; i++) {
            require(allowUnsignedBetsFlag[aggregatedBets[i].user], "Missing casino permission");

            (, uint256 affiliateCut, uint256 lionGamingCut) = lionGaming.applyBalanceDelta(
                aggregatedBets[i].user,
                aggregatedBets[i].spent,
                aggregatedBets[i].gained,
                aggregatedBets[i].margin,
                aggregatedBets[i].coinIndex
            );

            uint256 miningReward = lionGaming.giveLFIRewards(
                aggregatedBets[i].user,
                aggregatedBets[i].spent * aggregatedBets[i].lfiExchangeRate,
                aggregatedBets[i].miningRate,
                aggregatedBets[i].margin
            );
        ...
        private
    {
        ...
        for (uint256 i = 0; i < numFlags; i++) {
            ...
            betFlagNonces[permissions[i].user] = permissions[i].nonce;
