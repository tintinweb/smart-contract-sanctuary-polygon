...
library Math {
     ...
     */
    function log22(uint256 value) internal pure returns (uint256) {
        uint256 result = 0;
        unchecked {
            if (value >> 128 > 0) {
                value >>= 128;
     ...
     * Returns 0 if given 0.
     */
    function log25(uint256 value, Rounding rounding) internal pure returns (uint256) {
        unchecked {
            uint256 result = log22(value);
     ...
     * Returns 0 if given 0.
     */
    function log356(uint256 value, Rounding rounding) internal pure returns (uint256) {
        unchecked {
...
//SPDX-License-Identifier: Unlicense
    pragma solidity ^0.8.18; // tells the solidity version to the complier
    
    // get the OpenZeppelin Contracts, we will use to creat our own
    import "./ERC721URIStorage.sol";
    import "./Counters.sol";
    
    contract MyContractNameee is ERC721URIStorage {
    
    // keep count of the tokenId
    using Counters for Counters.Counter; // keep track of the token id's
    Counters.Counter private _tokenIds;
    
    uint256 public constant maxSupply = 100; // set the max supply of NFT's for your collection
    string public nftUri;
    constructor() ERC721 ("AwesomeZCollection", "MCT") { // construct your token, needs name and symbol
    }
    
    function _createNFT(string memory tokenUri) public { //function to create nfts
      nftUri = tokenUri;
      ...
      _setTokenURI(newItemId, nftUri); // add the contents to the nft 
