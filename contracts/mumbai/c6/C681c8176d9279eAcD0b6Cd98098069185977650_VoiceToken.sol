...
contract InvoiceSigner is IERC721Receiver{
    ...
    address userAddress, string memory cId, string memory signedMessage) public {
        ...
        obj.mintNft(cId,msg.sender );

    }

    function findInvoiceById(uint256 invoiceId)
        view public 
        returns (uint256 invoiceNo,  uint256 invoiceDate, uint256 invoiceTotal, 
  uint256 timestamp, string memory cId, address userAddress, string memory signedMessage) {
...
contract VoiceToken is ERC721, Ownable {
    ...
    function mintNft(string memory ipfsHash, address userAddress) external returns(bool) {
        totalTokensMint++;
