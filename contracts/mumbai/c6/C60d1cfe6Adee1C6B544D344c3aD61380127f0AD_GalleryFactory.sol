...
contract GalleryFactory is IGalleryFactory, Multicall, Ownable {
	...
	///@param _name name of the gallery
	...
	function mintNftInNewGallery(galleryAndNFT memory gallerydata) external override {
		bytes32 galleryid = findHash(gallerydata._name);
		require(!allgalleriesId.contains(galleryid), 'Name already registered');
		galleryInfo storage gallery = galleries[galleryid];
		allgalleriesId.add(galleryid);
		Gallery galleryaddress = new Gallery(gallerydata._name, gallerydata._owner, address(NFT), address(marketPlace));
	...
	function changeNftAddress(address newnft) public override onlyOwner {
		NFT = INFT(newnft);
	}
	...
	function changeMarketAddress(address newMarket) public override onlyOwner {
...
contract Gallery is Ownable, IGallery, Multicall {
	///@notice map the given address with boolean
	///@dev checks whether the given address is added as admins or not
	mapping(address => bool) public admins;

	///@notice name of the gallery
	///@dev provides the name of this gallery
	string public name;
	...
	modifier onlyOwnerorTokenOwner(uint256 tokenid) {
		...
		if (tokenowner != msg.sender && owner() != msg.sender && !admins[msg.sender])
			revert('Need to be either token owner or gallery owner');
		_;
	}

	///@notice stores the token info in a struct
	struct TokenInfo {
		uint256 tokenId;
		uint256 minprice;
		uint256 artistfee;
	...
	///@param _feeExpiryTime time limit to pay third party commission fee
	...
	function burn(uint256 _tokenId) public override onlyOwnerorTokenOwner(_tokenId) {
		nft.burn(_tokenId);
	...
	function transferNft(
		...
		address to,
		uint256 tokenId
	) public override onlyOwnerorTokenOwner(tokenId) {
	...
	///@param galleryOwnerFee commission rate to be transferred to gallery owner while selling nft
	...
	///@param _name name of the nft be listed for sale
	///@param _uri token uri of the nft to be minted
	...
	function mintAndSellNft(
		...
		uint256 tokenId = mintNFT(_name, _uri, artist, thirdParty, feeExpiryTime);
		sellNft(tokenId, amount, artistFee, galleryOwnerFee, thirdPartyFee, physicalTwin, true);
		emit Nftmintedandsold(tokenId, address(this), amount);
		return tokenId;
	}

	///@notice cancel the nft listed for sell
	///@param tokenid id of the token to be removed from list
	///@dev only gallery owner or token owner can cancel the sell of nft
	function cancelNftSell(uint256 tokenid) public override onlyOwnerorTokenOwner(tokenid) {
		require(listOfTokenIds.contains(tokenid), 'Tokenid is not listed in this gallery');
		TokenInfo storage Token = tokeninfo[tokenid];
		Token.minprice = 0;
		Token.onSell = false;
	...
	function changeArtistCommission(uint256 _tokenId, uint256 _artistfee) public onlyOwnerorTokenOwner(_tokenId) {
	...
	///@param _tokenId id of the token
	///@param _galleryOwnerFee new gallery owner fee commission rate
	///@dev only gallery owner or token owner can change  the gallery owner commission rate for given  nft
	function changeGalleryCommission(uint256 _tokenId, uint256 _galleryOwnerFee) public onlyOwnerorTokenOwner(_tokenId) {
		require(listOfTokenIds.contains(_tokenId), 'Tokenid is not listed in this gallery');
		TokenInfo storage Token = tokeninfo[_tokenId];
	...
	///@dev only gallery owner or token owner can change  the artist commission rate for given  nft
	function reSaleNft(uint256 _tokenId, uint256 _minprice) public onlyOwnerorTokenOwner(_tokenId) {
		require(listOfTokenIds.contains(_tokenId), 'Tokenid is not listed in this gallery');
		TokenInfo storage Token = tokeninfo[_tokenId];
	...
	function retreiveBalance() public override onlyOwner {
		uint256 amount = address(this).balance;
		(bool success, ) = payable(msg.sender).call{ value: amount }(' ');
		require(success, 'Failed to retreive the amount');
	}
	...
	// function airDrop(uint256 tokenId) public onlyOwner {
	// 	AirDropInfo storage airdrop = airDropInfo[tokenId];
	// 	airdrop.tokenId = tokenId;
	// 	airdrop.isClaimed = false;
	// 	string memory message = ('ArtSwap Token has been  air dropped to you in timestamp');
	// 	airdrop.verificationCode = getHash(message, block.timestamp);
	// }
	...
	// 	require(airdrop.verificationCode == _code, 'Verification code mismatch');
	// 	require(listOfTokenIds.contains(_tokenId), 'Tokenid is not listed in this gallery');
	// 	if (tokeninfo[_tokenId].onSell) cancelNftSell(_tokenId);
	...
	///@param time expiry time in terms of year
	...
	function calculateExpiryTime(uint256 time) private view returns (uint256) {
		...
		return (block.timestamp + timeToAdd * 1 days);
	}

	// 	function getHash(string memory _message, uint256 _time) private pure returns (bytes32) {
	// 		return keccak256(abi.encodePacked(_message, _time));
	// 	}
}