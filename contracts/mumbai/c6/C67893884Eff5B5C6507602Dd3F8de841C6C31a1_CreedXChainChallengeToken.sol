 ...
 */
contract CreedXChainChallengeToken is IXReceiver, ERC20 {
  ...
  // The address of the deployer, for initialization purposes
  ...
  event Flag(address indexed challenger);

  constructor(IConnext _connext, string memory _name, string memory _symbol) ERC20(_name, _symbol, 18) {
    connext = _connext;
    deployer = msg.sender;
  }

  function initialize(address _target, uint32 _destinationDomain) public {
    require(initialized == false, "Already initialized");
    require(msg.sender == deployer, "Only the deployer can initialize");
  ...
  /** 
   * @notice Override transfer function to enforce the 100 tokens per account limit.
   */
  function transfer(address to, uint256 amount) public override returns (bool) {
    _accountMaxTokensCheck(to, amount);
    return super.transfer(to, amount);
  }

  /** 
   * @notice Override transferFrom function to enforce the 100 tokens per account limit.
   */
  function transferFrom(
        address from,
        address to,
  ...
  /** 
   * @notice Sends tokens across chains. For simplicity, you can only transfer to yourself (the same address) on the target chain.
   * @param relayerFee The fee offered to relayers on the xCall.
   * @param mintNumber The amount of bloating tokens to mint on the target chain.
   */
  function xChainTransfer(uint256 relayerFee, uint256 mintNumber) public payable {
    // Burn the tokens from the sender.
    _burn(msg.sender, mintNumber);

    // Include the relayerFee so the bridging agents pick it up (PLEASE RUN THE ESTIMATOR SCRIPT!)
    // Include the address of the account to receive the tokens on the other chain and the amount to send.
    bytes memory _callData = abi.encode(msg.sender, mintNumber);

    // Send the xCall to the Connext bridge contract.
   ...
   * @dev The "callback" function for this example. Will be triggered after the other chain's token xcalls back. There are unused params to comply with the XReceiver interface.
   */
  function xReceive(
    bytes32, //_transferId,
    uint256, //_amount,
    ...
    emit XCallReceived(_minter, _mintNumber);
  ...
  /** 
   * @notice Function to check that each account is allowed to have, at most, 100 tokens.
   */
  function _accountMaxTokensCheck(address to, uint256 amount) internal view {
    require(balanceOf[to] + amount <= 100*10**18, "Cannot have more than 100 tokens in one account.");
  }
}
