...
contract GameMarketplace is Ownable, VRFConsumerBaseV2 {
    ...
    mapping(uint256 => uint256) s_results;
    ...
    * @dev Add item to a pool.
    ...
    function vrfTest() public onlyOwner  {
   ...
   function fulfillRandomWords(
        ...
        require(requestId == lastRequestId, "Only the last request can be fulfilled");
        uint256 randomIndex = randomWords[0] % pools[s_results[lastRequestId]].availableItems.length;
       emit RequestFulfilled(randomIndex);
    }
...
//
//contract GameMarketplace is VRFConsumerBase, Ownable {
...
//    mapping(uint256 => uint256) s_results;
//
//    // Resto do contrato aqui
//
//    constructor(uint64 subscriptionId, address VRFCoordinator, address _VRFCoordinator, address _LinkToken, bytes32 _keyHash)
...
//    }
//
//    function rentItemFromPool(uint256 _categoryId) public payable {
//        Pool storage pool = pools[_categoryId];
//        require(pool.isActive, "Pool with the given category ID does not exist or is not active");
//        require(pool.availableItems.length > 0, "No items available to rent");
//
...
//            requestConfirmations,
//            callbackGasLimit,
//            1 // We only need one random number
//        );
//
//        lastRequestId = requestId;
//    }
...
//        require(requestId == lastRequestId, "Only the last request can be fulfilled");
//
//        uint256 randomIndex = randomWords[0] % poolsList[s_results[lastRequestId]].availableItems.length;
//
//        Item memory itemToRent = poolsList[s_results[lastRequestId]].availableItems[randomIndex];
//
//        // Logic to rent the item to the user goes here
//
//        // Remove the item from the pool of available items and add it to the rented items
//        uint256 lastAvailableItemIndex = poolsList[s_results[lastRequestId]].availableItems.length - 1;
//        poolsList[s_results[lastRequestId]].availableItems[randomIndex] = poolsList[s_results[lastRequestId]].availableItems[lastAvailableItemIndex];
//        poolsList[s_results[lastRequestId]].availableItems.pop();
//        poolsList[s_results[lastRequestId]].rentedItems.push(itemToRent);
//
//        // Update the item status
//        itemsList[itemToRent.id].rentedBy = msg.sender;
//
//        emit ItemRented(itemToRent.id, msg.sender);
//    }