...
contract RaffleMainProjectv2 {
    ...
    uint256 private prizePercentage;
    uint256 private adminsFee;
    ...
    event RaffleLocked();
    event RaffleCleared();
    event InTheMomentThePrizeIs(uint256 prize);
    ...
    RaffleVRFv2Consumer vrfv2consumer = RaffleVRFv2Consumer (0x922a41972E2db3A3000B72A99EC41c902663b550);
    ...
    */
    ...
    /**
    * @dev This function returns the raffle status.
    * It can be called by anyone to check the raffle's closing status.
    * First, it checks if the raffle is in the 'RaffleStage.Open' stage; otherwise, returns the raffle closing time.
    ...
    */
    function closingTime() public returns (uint256 status) {
        if(block.timestamp < raffleClosing){
            ...
            require(playerSelector.length > 1, "There no participants enough!");
            require(address(this).balance == SafeMath.mul(ticketCost, playerSelector.length), "Contract balance does not match ticket sales.");

            updateStage(RaffleStage.Calculating);
            
            emit RaffleAlreadyClosed();
            emit WaitingForDraw();
            return raffleClosing;
        }
    }
    ...
    function rafflePrize() public returns(uint256){
        require(stage == RaffleStage.Open, "Raffle must be Open to see the prize");

        prize = (address(this).balance.mul(prizePercentage).div(100));

        emit InTheMomentThePrizeIs(prize);
    ...
    * @param newPrizePercentage The updated prize for the raffle.
    * @param newAdminsFeePercentage The updated softcap for the raffle.
    * @param timeToClose The time duration for the raffle to close.
    */
    function updateAndOpenRaffle(uint256 feeToUpdate,
                          uint256 newPrizePercentage,
                          uint256 newAdminsFeePercentage,
        ...
        require(newPrizePercentage <= 100, "Invalid prize percentage");
        require(newAdminsFeePercentage <= 100, "Invalid admins fee percentage");

        lastTimeStamp = block.timestamp;
        ticketCost = feeToUpdate;
        adminsFee = newAdminsFeePercentage;
    ...
    */
    function requestRefund() public safetyLock{
        require(stage == RaffleStage.Open || stage == RaffleStage.Canceled, "Refund not allowed at this stage.");
...
contract RaffleVRFv2Consumer is VRFConsumerBaseV2 {
    ...
    require(newOwner != owner, "Ownable: new owner is the zero address");
        owner = newOwner ;
    }
