...
contract PremiumChallenge is ReentrancyGuard {
    ...
    function forecastChallengeState() private view returns (ChallengeState){
        ...
        if (block.timestamp >= metaData.startTime && block.timestamp < metaData.endTime - 5)
    ...
    function setWinners(address[] memory _winnersWallets, uint256 _qualifiedParticipants) public senderIsAddress onlyOwner challengeWinnersIsNotSet {
        // validate wallets
    ...
    function isParticipated(address _wallet) public view returns (bool) {
        require(_wallet != address(0), 'Invalid arg wallet address');
        return participants[_wallet] != address(0);
    }

    // get winner data
    function getWinnerData(address _wallet) public challengeWinnersIsSet view returns (Winner memory) {
        require(_wallet != address(0), 'Invalid arg wallet address');
    ...
    // check is winner
    function isWinner(address _wallet) public challengeWinnersIsSet view returns (bool) {
        require(_wallet != address(0), 'Invalid arg wallet address');
        return winners[_wallet].wallet != address(0);
    }

    // check is reward to claim
    function isRewardClaimed(address _wallet) public challengeWinnersIsSet view returns (bool) {
        require(_wallet != address(0), 'Invalid arg wallet address');
        require(winners[_wallet].wallet != address(0), 'Winner not found');
