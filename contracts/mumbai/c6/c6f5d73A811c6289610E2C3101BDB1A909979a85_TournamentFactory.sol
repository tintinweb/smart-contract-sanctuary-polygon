...
contract Tournament is Ownable {
    ...
    function initialize(address _token, uint256 _awardAmount) public onlyOwner {
        token = ERC20(_token);
        token.transferFrom(msg.sender, address(this), _awardAmount);
        awardAmount = _awardAmount;
    }

    function setWinner(address payable _winner) public onlyOwner {
        winner = _winner;
        token.approve(winner, awardAmount);
    }

    function withdraw() public {
        require(msg.sender == winner, "You aren't the winner");
        token.transfer(winner, awardAmount);
