 ...
 */
contract XplusERC20Bridge is OwnableUpgradeable {
    ...
    function __XplusErc20Bridge_init(address _relayer) public initializer {
        __Ownable_init();
    ...
    function addToken(
        ...
        require(tokenInfos[tokenKey].tokenAddress == address(0), "XplusERC20Bridge: token already exists");
        require(tokenAddress != address(0), "XplusERC20Bridge: zero address");
    ...
    function removeToken(bytes32 tokenKey) external onlyOwner {
        require(tokenInfos[tokenKey].tokenAddress != address(0), "XplusERC20Bridge: token does not exists");
    ...
    function addChainSupportForToken(bytes32 tokenKey, uint256 chainId) external onlyOwner {
        require(!tokenSupportedOnChain[tokenKey][chainId], "XplusERC20Bridge: already supported");
    ...
    function dropChainSupportForToken(bytes32 tokenKey, uint256 chainId) external onlyOwner {
        require(tokenSupportedOnChain[tokenKey][chainId], "XplusERC20Bridge: not supported");
    ...
    function deposit(
        ...
        require(tokenInfo.tokenAddress != address(0), "XplusERC20Bridge: token not found");

        require(amount > 0, "XplusERC20Bridge: amount must be positive");
        require(destChainId != currentChainId, "XplusERC20Bridge: dest must be different from src");
        require(isTokenSupportedOnChain(token, destChainId), "XplusERC20Bridge: token not supported on chain");
        require(recipient != 0, "XplusERC20Bridge: zero address");
    ...
    function withdraw(
        ...
        require(destChainId == currentChainId, "XplusERC20Bridge: wrong chain");
        require(!withdrawnDeposits[srcChainId][depositId], "XplusERC20Bridge: already withdrawn");
        require(recipient != 0, "XplusERC20Bridge: zero address");
        require(amount > 0, "XplusERC20Bridge: amount must be positive");
        ...
        require(recoveredAddress == relayer, "XplusERC20Bridge: invalid signature");
    ...
    function _setRelayer(address _relayer) private {
        require(_relayer != address(0), "XplusERC20Bridge: zero address");
        require(_relayer != relayer, "XplusERC20Bridge: relayer not changed");
