...
contract NgageN is AccessControlUpgradeable, ERC1155Upgradeable {
    ...
    modifier onlyOperator() {
        require(hasRole(NFT_OPERATOR_ROLE, _msgSender()), "NN: Only Operator");
        _;
    }
    ...
    function burnNFT(
        ...
        require(
            _msgSender() == from || hasRole(NFT_OPERATOR_ROLE, _msgSender()),
            "NN: Only operator or token holder can burn tokens"
        );
    ...
    function setApprovalForTokens(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            require(balanceOf(account, ids[i]) > 0, "NN: approver must own the token");
