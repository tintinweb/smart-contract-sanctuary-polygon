...
uint256 public walyCost; //LP WALY buy cost
   ...
   //check offer has not be purchased    
    ...
    function add1155ToLaunchPad(uint256 tokenId, address tokenAddress, uint256 units, uint256 askingPrice, bool useLTP) OnlyItemOwner(tokenAddress,tokenId) HasTransferApproval(tokenAddress) isLaunchPadItem(tokenAddress) onlyOwner external returns(uint256){
            require(activeItems[tokenAddress][tokenId] == false,"This offer has been listed already.");
    ...
    function buyERC1155Offer(uint256 id) payable external ItemExist(id) IsForSale(id) onlyWMTPholder() {
            ...
            if(itemsForSale[id].useLPT == true){
                ...
                IERC20(LaunchPadTokens).transfer(burn, itemsForSale[id].askingPrice *(10**18)); //Forever locks LPT 
                ...
                uint256 LPT = itemsForSale[id].askingPrice * 5 / 100; //LPT rewards amount

                //if sodGuard can cover rewards, pay out if not continue.
                if(guardBal >= LPT){
                    //reward rates
                    uint256 amount = itemsForSale[id].askingPrice;
                    uint256 rewardCalc = amount * 5 / 100; 

                    IERC20(LaunchPadTokens).transferFrom(sodGuard, msg.sender, rewardCalc *(10**18)); //transfer  LaunchPad Rewards
   
                    emit LaunchPadTokenReward(LaunchPadTokens, rewardCalc, sodGuard, msg.sender, block.timestamp);
                }
    ...
    function buyERC721Offer(uint256 id) payable external ItemExist(id) IsForSale(id) onlyWMTPholder()  {
            ...
            if(itemsForSale[id].useLPT == true){
                ...
                IERC20(LaunchPadTokens).transfer(burn, itemsForSale[id].askingPrice *(10**18)); //for ever lock up
                ...
                uint256 LPTamount = itemsForSale[id].askingPrice * 5 / 100; //reward calc

                 if(guardBal >= LPTamount){
                        uint256 amount = itemsForSale[id].askingPrice;
                        uint256 rewardCalc = amount * 5 / 100;

                        IERC20(LaunchPadTokens).transferFrom(sodGuard, msg.sender, rewardCalc *(10**18)); //reward LPT
