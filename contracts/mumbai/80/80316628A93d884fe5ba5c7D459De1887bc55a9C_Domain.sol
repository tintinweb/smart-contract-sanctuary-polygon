...
contract Domain is Ownable {
    ...
    mapping(string => DOMAIN[]) private subdomainsByDomain;
    ...
    function setDomainPrice(uint256 price) public onlyOwner {
        domainPrice = price;
    }

    function buyDomain(string memory dname) public payable {
        require(msg.value >= domainPrice, "You need to pay more!");
        require(isDomain(dname), "It is already on the list!");
        customer[dname] = msg.sender;
        domainsByOwner[msg.sender].push(dname);
    }
    ...
    function registerS(string memory subDname, string memory dname, string memory str) public {
        ...
        DOMAIN memory dom = DOMAIN(subDname, str);
        subdomainsByDomain[dname].push(dom);
    }

    function readDomains() public view returns(string[] memory) {
        return domainsByOwner[msg.sender];
    }   

    function reawdSubdomains(string memory dname) public view returns(DOMAIN[] memory) {
