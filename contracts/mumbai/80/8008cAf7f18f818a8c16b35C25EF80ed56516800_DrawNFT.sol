...
contract DrawNFT is ERC721, ERC721URIStorage, Ownable {
    ...
    function verifyMintParam(uint256 allowance, uint256 start, uint256 end, string memory baseUri, bytes memory signature) public view {
        string memory message = string(abi.encodePacked(Strings.toHexString(msg.sender), ' ', Strings.toString(allowance), ' ', Strings.toString(start), ' ', Strings.toString(end), ' ', baseUri));
    ...
    function mint(address to, uint256 allowance, uint256 start, uint256 end, string memory baseUri, bytes memory signature) public {
        verifyMintParam(allowance, start, end, baseUri, signature);
        uint256 tokenId = _tokenIdCounter.current();
