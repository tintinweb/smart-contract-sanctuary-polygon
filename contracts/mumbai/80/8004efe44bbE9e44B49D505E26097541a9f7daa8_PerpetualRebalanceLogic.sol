...
contract PerpetualRebalanceLogic is
    ...
    IPerpetualRebalanceLogic
{
     ...
     */
    function rebalance(uint24 _iPerpetualId) external override {
     ...
     */
    function decreasePoolCash(uint8 _iPoolIdx, int128 _fAmount) external override {
        require(msg.sender == address(this), "onlythis");
        LiquidityPoolData storage liquidityPool = liquidityPools[_iPoolIdx];
        _decreasePoolCash(liquidityPool, _fAmount);
    }
    ...
    function getFunctionList() external pure virtual override returns (bytes4[] memory, bytes32) {
        ...
        functionList[1] = this.increasePoolCash.selector;
        functionList[2] = this.decreasePoolCash.selector;
...
contract PerpetualRebalanceFunctions is PerpetualBaseFunctions {
    ...
    function _rebalanceToTarget(PerpetualData storage _perpetual) internal {
        ...
        if (uint64(block.timestamp) <= _perpetual.iLastDefaultFundTransfer) {
            ...
            fGap = fGap.sub(fGapFillDF).mul(fMaxTransfer);
        }
