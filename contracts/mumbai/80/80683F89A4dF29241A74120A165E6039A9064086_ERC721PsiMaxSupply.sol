...
contract ERC721PsiMaxSupply is ERC721Psi, Ownable {
    ...
    event TokensMinted(uint256 quantity, address recipient);
        ...
        public onlyOwner
    {
        ...
        require(quantity + totalSupply() <= maxSupply, "Quantity exceeds max supply");

        _safeMint(recipient, quantity);

        emit TokensMinted(quantity, recipient);
    }

    /// @notice Mints the max number of tokens available
    function mintMax(address recipient) 
        external onlyOwner 
    {
        require(totalSupply() < maxSupply, "Max supply minted");
