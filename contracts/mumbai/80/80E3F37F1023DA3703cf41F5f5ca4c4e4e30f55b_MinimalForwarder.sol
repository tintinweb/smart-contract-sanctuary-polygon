 ...
 */
contract MinimalForwarder is EIP712, Ownable, TokenWithdrawer {
    using ECDSA for bytes32;

    struct ForwardRequest {
        address from; /**Game minter. _msgSender() at destination contract */
        address to; /**Token contract */
        address relayer; /**Person who pays gas fee. _msgSender() at this point */
        uint256 value; /**Function name and parameters */
        uint256 gas; /**Gas */
        uint256 nonce; /**To invalidate older transaction of relayer */
        uint256 feeAmount; /**How much relayer is being charged in nitro */
        bytes data; /**Any additional info */
    }

    bytes32 private constant _TYPEHASH =
        keccak256(
            "ForwardRequest(address from, address to, address relayer, uint256 value, uint256 gas, uint256 nonce, uint256 feeAmount, bytes data)"
        );
    ...
    function withdrawERC20(address token, uint256 amount) external onlyOwner {
        _withdrawERC20(_treasury, token, amount);
    }
    ...
    function withdrawERC721(address token, uint256 tokenID) external onlyOwner {
        _withdrawERC721(_treasury, token, tokenID);
    }
    ...
    function withdrawERC1155(
        ...
        _withdrawERC1155(_treasury, token, tokenID, amount);
    }
}
