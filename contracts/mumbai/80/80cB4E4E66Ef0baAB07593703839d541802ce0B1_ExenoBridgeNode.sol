    ...
    ERC1363Payable
{
        ...
        external payable nonReentrant whenNotPaused
    {
        ...
        _verifySignature(data, fee, signature);
    ...
    function deposit(
        ...
        external payable nonReentrant whenNotPaused
    {
        (Data memory data, uint256[2] memory fee, bytes memory signature) = _unpackDepositData(encodedData);
        _verifySignature(data, fee, signature);
        _validateDeposit(data, fee);
        _processDeposit(data);
        if (_isImmediate(data)) {
    ...
    function withdraw(
        ...
        external payable nonReentrant whenNotPaused
    {
        (Data memory data, uint256[2] memory fee, bytes[3] memory signature) = _unpackWithdrawData(encodedData);
        _verifySignature(data, fee, signature);
        _validateWithdraw(data, fee);
        _processWithdraw(data);
    }
    ...
    function payout(
        ...
        external onlyOwner
    {
        _processWithdraw(_unpackPayload(payload));
    }
    ...
    {
        (bytes32 operation, bytes memory encodedData) = abi.decode(wrappedData, (bytes32, bytes));
        // In case of a deposit we treat tokens sent to this contract as a deposit and/or fee payment
        if (operation == bytes32("deposit")) {
            (Data memory data, uint256[2] memory fee, bytes memory signature) = _unpackDepositData(encodedData);
            _verifySignature(data, fee, signature);
        ...
        // In case of a withdrawal we treat tokens sent to this contract as a fee payment
        if (operation == bytes32("withdraw")) {
            (Data memory data, uint256[2] memory fee, bytes[3] memory signature) = _unpackWithdrawData(encodedData);
            _verifySignature(data, fee, signature);
        ...
        revert("ExenoBridgeNode: transfer received with an unexpected operation");
    }
    ...
    function _verifySignature(
        ...
        internal view
    {
        bytes memory payload = abi.encodePacked(d.id, d.platform, d.network, d.beneficiary, d.token, d.amount, d.timeout, d.operation, fee);
     ...
     */
    function _unpackDepositData(bytes memory encodedData)
     ...
     */
    function _unpackWithdrawData(bytes memory encodedData)
