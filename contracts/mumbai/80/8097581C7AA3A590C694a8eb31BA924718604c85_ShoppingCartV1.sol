...
import "./lib/CurrencyTransferLibUpgradeable.sol";
import "@openzeppelin/contracts/utils/cryptography/MerkleProof.sol";
import "./Phaseable.sol";

abstract contract DroppableUpgradeable is PrimarySale, Phaseable {
    ...
    function claim(
        ...
        if (totalPrice > 0) {
            require(totalPrice == msg.value, "Value is incorrect");
        }
        ...
        CurrencyTransferLibUpgradeable.transferCurrency(
            CurrencyTransferLibUpgradeable.NATIVE_TOKEN,
            msg.sender,
    ...
    function _checkOrigin() internal virtual {
...
library CurrencyTransferLibUpgradeable {
...
abstract contract Phaseable {
    ...
    event MintingPhasesUpdated(MintPhase[] phases);
    ...
    function setPrice(uint256 _mintPhaseId, uint256 _pricePerToken) external {
        require(_canSetMintingPhases(), "Unauthorized");
        require(_mintPhaseExists(_mintPhaseId), "Phase not found");

        MintPhase storage mintPhase = _mintPhases[_mintPhaseId];
        mintPhase.pricePerToken = _pricePerToken;
    }
    ...
    function getClaimsByWallet(
        ...
        require(_mintPhaseExists(_id), "Phase not found");
    ...
    function _setupMintingPhases(MintPhase[] calldata _phases) internal {
        ...
        for (uint256 i = 0; i < _phases.length; i++) {
            ...
            require(i == 0 || lastEndTime > 0, "End time invalid");
            require(
...
import "@nefty/base-contracts/contracts/DroppableUpgradeable.sol";
import "./ContextMixin.sol";

abstract contract BacanoGoCollection is
    Initializable,
    ERC721Upgradeable,
    ...
    DroppableUpgradeable,
    ContextMixin
{
    uint32 public maxSupply; // 0 means no max supply

    function __BacanoGoCollection_init(
        string memory _name,
        ...
        __BacanoGoCollection_init_unchained(
            _name,
            _symbol,
            _maxSupply,
            _royaltyRecipient,
    ...
    function __BacanoGoCollection_init_unchained(
        string memory _name,
        string memory _symbol,
        uint32 _maxSupply,
        address _royaltyRecipient,
    ...
    function _beforeClaim(
        ...
        require(_quantity <= 100, "Limit exceeded");
    ...
    // Operator filter implementation
...
import "./BacanoGoCollection.sol";
import "./MetadataBuilder.sol";

/* solhint-disable quotes */
contract BacanoGoGiveawayV1 is BacanoGoCollection {
    ...
    mapping(CardType => CardSpec) public cardSpecs;
    mapping(CardType => mapping(uint256 => uint256)) private mintCounters;
    ...
    function configureCharacters(
        CardType _cardType,
        uint32 _odds,
        CharacterSpec[] memory _characters,
        string[2][] memory _attributes,
        bool _randomDistribution
    ) external onlyOwner {
        cardSpecs[_cardType].odds = _odds;
        cardSpecs[_cardType].randomDistribution = _randomDistribution;
        cardSpecs[_cardType].attributes = _attributes;

        require(_characters.length > 0, "Must have at least one character");
        require(
            cardSpecs[_cardType].characters.length == 0 ||
                cardSpecs[_cardType].characters.length == _characters.length,
            "Can't change characters size"
        );

        // Reset the characters
        delete cardSpecs[_cardType].characters;

        // Add the new characters
        for (uint256 i = 0; i < _characters.length; i++) {
            cardSpecs[_cardType].characters.push(_characters[i]);
        }
    }
    ...
    // Droppable implementation
    function _checkOrigin() internal view override {
        bool isWhitelisted = contractWhitelist[msg.sender];
        if (!isWhitelisted) {
            require(msg.sender == tx.origin, "BOT");
        }
    }
    ...
    function transferTokensOnClaim(
        ...
        uint256 odds = cardSpecs[CardType.CARNAVAL].odds +
            cardSpecs[CardType.HOLOFOIL].odds +
            cardSpecs[CardType.GOLD].odds;
        require(odds > 0, "No config");
        ...
        for (uint256 i = 0; i < _quantity; i++) {
            ...
            uint256 target = randomVal % odds;

            // Get card type from generated number
            CardType cardType;
            if (target <= cardSpecs[CardType.CARNAVAL].odds) {
                cardType = CardType.CARNAVAL;
            } else if (
                target <=
                (cardSpecs[CardType.CARNAVAL].odds +
                    cardSpecs[CardType.HOLOFOIL].odds)
            ) {
                cardType = CardType.HOLOFOIL;
            } else {
                cardType = CardType.GOLD;
            }

            CardSpec storage cardSpec = cardSpecs[cardType];

            // Get the index of the character to mint
            uint256 characterIndex;
            if (cardSpec.randomDistribution) {
                uint256 randomCharacterVal = uint256(
                    keccak256(abi.encode(random, i + 1))
                );
                characterIndex =
                    randomCharacterVal %
                    cardSpec.characters.length;
            } else {
                characterIndex =
                    cardSpec.nextIndex %
                    cardSpec.characters.length;
            ...
            uint256 mintNumber = mintCounters[cardType][characterIndex] + 1;
            ...
            mintCounters[cardType][characterIndex]++;
            cardSpec.nextIndex = characterIndex + 1;
    ...
    function mint(address _recipient, uint32 _quantity) public {
        require(
            msg.sender == owner() || contractWhitelist[msg.sender],
            "Not authorized"
        );
        _beforeClaim(_recipient, _quantity, 0);
        transferTokensOnClaim(_recipient, _quantity);
    }
    ...
    function tokenURI(
        ...
        CardSpec memory cardSpec = cardSpecs[token.cardType];
        CharacterSpec memory character = cardSpec.characters[
            token.characterIndex
        ];

        // Leading zeros for mint numbers below 100
        string memory leadingZeros = "";
        if (token.mintNumber < 10) {
            leadingZeros = "00";
        } else if (token.mintNumber < 100) {
            leadingZeros = "0";
        }

        // Build the name including the mint number
        string memory name = string(
            abi.encodePacked(
                character.name,
                " #",
                leadingZeros,
                Strings.toString(token.mintNumber)
            )
        );

        string[2][] memory attributes = new string[2][](
            1 + cardSpec.attributes.length
        );
        attributes[0][0] = "Character";
        attributes[0][1] = character.name;
        for (uint256 i = 0; i < cardSpec.attributes.length; i++) {
            attributes[i + 1][0] = cardSpec.attributes[i][0];
            attributes[i + 1][1] = cardSpec.attributes[i][1];
        }
        return
...
import "@openzeppelin/contracts-upgradeable/access/OwnableUpgradeable.sol";
import "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
import "./BacanoGoGiveawayV1.sol";

contract ShoppingCartV1 is Initializable, OwnableUpgradeable {
    struct Purchase {
    ...
    struct Claim {
        DroppableUpgradeable droppable;
        uint32 amount;
        uint256 value;
    }
    ...
    function whitelist(address contractAddress) external onlyOwner {
        isWhitelisted[contractAddress] = true;
    }

    function setGiveawayContract(address contractAddress) external onlyOwner {
        giveawayContract = contractAddress;
    }

    // Admin functions
    function checkout(
        address recipient,
        Purchase[] memory cart
    ) external payable {
        uint256 totalPrice = 0;
        uint32 freeClaims = 0;
        bytes32[] memory proof = new bytes32[](0);

        Claim[] memory claims = new Claim[](cart.length);
        for (uint256 i = 0; i < cart.length; i++) {
            Purchase memory purchase = cart[i];
            require(
                isWhitelisted[purchase.contractAddress],
                "Contract is not whitelisted"
            );

            if (purchase.contractAddress != giveawayContract) {
                freeClaims += purchase.amount;
            }

            DroppableUpgradeable droppable = DroppableUpgradeable(
                purchase.contractAddress
            );
            (DroppableUpgradeable.MintPhase memory phase, ) = droppable
                .getActiveMintPhase();

            uint256 value = phase.pricePerToken * purchase.amount;
            claims[i] = Claim(droppable, purchase.amount, value);
            totalPrice += value;
        }
        ...
        for (uint256 i = 0; i < claims.length; i++) {
            ...
            claim.droppable.claim{value: claim.value}(
                recipient,
                claim.amount,
                proof
            );
        ...
        if (freeClaims > 0 && giveawayContract != address(0)) {
            BacanoGoGiveawayV1 giveaway = BacanoGoGiveawayV1(
                giveawayContract
            );
            giveaway.mint(recipient, freeClaims);
        }
    }
}
