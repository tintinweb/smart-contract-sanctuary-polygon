  ...
  UUPSUpgradeable 
{
  ...
  address private _PAGE_TREE_STORE;
  ...
  function createMarketItem(
    ...
    whenNotPaused
  {
    // Check if book is from Page Tree Store
    if (nftContract == _PAGE_TREE_STORE) {
      bytes memory fetchAuthor = AddressUpgradeable.functionCall(_PAGE_TREE_STORE, abi.encodeWithSignature("getBookAuthor(uint256)", tokenID)); 
  ...
  // Returns _MARKET_FEE_RECEIVER, _MARKET_FEE_IN_BIPS, _PAGE_TREE_STORE
  ...
  {
    return (_MARKET_FEE_RECEIVER, _MARKET_FEE_IN_BIPS, _PAGE_TREE_STORE);
  }
  ...
  function setPageTreeStoreAddr(address storeAddr)
    public
    onlyRole(OPERATOR_ROLE)
  {
    _PAGE_TREE_STORE = storeAddr;
  }
