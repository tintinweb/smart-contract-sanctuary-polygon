...
contract Trade {
    ...
    event TradingFeeUpdated(uint8 TradingFee);
    ...
    uint8 private tradingFeePermille;                       
    ...
    constructor (uint8 _BuyerFeePermille, uint8 _sellererFeePermille, uint8 _tradingFee, uint8 _swappingFee, TransferProxy _transferProxy) {
        buyerFeePermille = _BuyerFeePermille;
        sellerFeePermille = _sellererFeePermille;
        tradingFeePermille = _tradingFee;
    ...
    function buyerTradingFee() external view virtual returns (uint8) {
        return tradingFeePermille;
    }

    function sellerTradingFee() external view virtual returns (uint8) {
        return tradingFeePermille;
    }

    function setTradingFee(uint8 _tradingFee) external onlyOwner returns(bool) {
        tradingFeePermille = _tradingFee;
        emit TradingFeeUpdated(tradingFeePermille);
        return true;
    }
    ...
    function setSwappingFee(uint8 _swappingFee) external onlyOwner returns(bool) {
        swappingFeePermille = _swappingFee;
        emit SwappingFeeUpdated(swappingFeePermille);
        return true;
    }
    ...
    function getFees(Order memory order) internal view returns(Fee memory){
        ...
        uint assetFee;
        uint price = order.amount * 1000 / (1000 + tradingFeePermille);
        tradingFee = (order.amount - price);
        price = order.amount * 1000 / (1000 + buyerFeePermille);
        ...
        assetFee = price - (fee + sellerFee + tradingFee);
        return Fee( platformFee, tradingFee , assetFee, royaltyAddress, royaltyFee, price);
    }
    ...
    function tradeAsset(Order calldata order, Fee memory fee, address buyer, address seller) internal virtual {
        ...
        if(fee.platformFee > 0) {
            ...
            transferProxy.erc20safeTransferFrom(IERC20(order.erc20Address), seller, owner, fee.platformFee);
        }
        if(fee.tradingFee > 0) {
            transferProxy.erc20safeTransferFrom(IERC20(order.erc20Address), buyer, owner, (fee.tradingFee / 2));
            transferProxy.erc20safeTransferFrom(IERC20(order.erc20Address), seller, owner, (fee.tradingFee / 2));
        }
    }
