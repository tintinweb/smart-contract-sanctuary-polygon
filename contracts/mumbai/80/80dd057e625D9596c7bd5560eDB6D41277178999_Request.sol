...
contract Request is ChainlinkClient, Ownable {
    ...
    mapping(string => uint256) public volumes;
    ...
    mapping(bytes32 => string) public volumeRequests;
    ...
    function requestVolume(string memory symbol)
        public
        onlyRequester
        returns (bytes32 requestId)
    {
        Chainlink.Request memory request = buildChainlinkRequest(
            priceJobId,
            address(this),
            this.fulfillVolume.selector
        );

        request.add("string_1", symbol);
        // Sends the request
        bytes32 _requestId = sendChainlinkRequestTo(oracle, request, fee);
        volumeRequests[_requestId] = symbol;
        return _requestId;
    }
    ...
    function fulfillVolume(bytes32 _requestId, uint256 _returnedVolume)
        public
        recordChainlinkFulfillment(_requestId)
    {
        // volume = _volume;

        string storage _symbol = volumeRequests[_requestId];

        volumes[_symbol] = _returnedVolume;
        blocks[_symbol] = block.number;
    }
    ...
    function getVolumeandBlock(string memory _symbol)
        external
        view
        returns (uint256, uint256)
    {
        return (volumes[_symbol], blocks[_symbol]);
    }
