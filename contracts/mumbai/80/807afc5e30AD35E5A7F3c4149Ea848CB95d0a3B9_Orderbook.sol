...
contract Orderbook {
    ...
    function orderbook_make_order(
        ...
        for (uint256 x = 0; x < legs.length; x++) {
            //Require priceId is valid pyth id.
            require(pyth_feed_exists(legs[x].feedID) == true, "Invalid pyth feed ID for one or more of your position's legs.");
    ...
    function pyth_feed_exists(bytes32 feedID) public view returns(bool exists){
        exists = pyth.priceFeedExists(feedID);
    }
