    ...
    UUPSUpgradeable
{
    ...
    modifier onlyRole(string memory name, address account) {
        require(LCRBALinker.checkAccountRole(name, account), 'LCNFT: Unauthorized');
        _;
    }

    function setRBALinker(address rbaLinkerAddress) public onlyRole('DEFAULT_ADMIN_ROLE', _msgSender()) {
        require(rbaLinkerAddress != address(0x0), 'LCNFT: RBA linker is zero address');
    ...
    function initialize(LostChainForwarder forwarder, address rbaLinkerAddress) public initializer {
        require(address(forwarder) != address(0x0), 'LCNFT: forwarder is zero address');
        require(rbaLinkerAddress != address(0x0), 'LCNFT: RBA linker is zero address');
        LCRBALinker = ILostChainRBA(rbaLinkerAddress);
    ...
    mapping(uint256 => bool) private rewardable;
    CountersUpgradeable.Counter private _nftId;
    ...
    mapping(uint256 => Experience) heroExperience;

    function setLCGD(address gameDataAddress) public onlyRole('DEFAULT_ADMIN_ROLE', msg.sender) {
        require(gameDataAddress != address(0x0), 'LCNFT: Game data is zero address');
    ...
    function mint(
        ...
        require(id == getLatestId(), 'LCNFT: ID mistmach');
        _mintNFT(to, id, amount, tokenURI, isHero, isRewardable);
    }
    ...
    function _mintNFT(
        ...
        if (isHero) {
            heroExperience[id].exists = true;
            heroExperience[id].heroId = id;
            heroExperience[id].level = 1;
            heroExperience[id].xp = 0;
        }

        if (isRewardable) rewardable[id] = true;
    }

    function setRewardable(uint256 nftId, bool state) public onlyRole('DEFAULT_ADMIN_ROLE', _msgSender()) {
        uint256 maxNftId = _nftId.current();
        require(maxNftId > nftId, 'LCNFT: NFT id is out of range');
        rewardable[nftId] = state;
    }
    ...
    function getHeroExperience(uint256 heroId) public view returns (Experience memory) {
        require(heroExperience[heroId].exists, 'LCNFT: Hero experience does not exists');
        return heroExperience[heroId];
    }

    function getAllHeroExperiences(uint256[] memory ids) public view returns (Experience[] memory) {
        uint256 maxNftId = _nftId.current();
        require(maxNftId > 0, 'LCNFT: No NFT available');

        Experience[] memory tempRet = new Experience[](maxNftId);
        uint256 totalIndex = 0;
        for (uint256 i = 0; i < ids.length; i++) {
            if (heroExperience[ids[i]].exists) {
                tempRet[totalIndex] = heroExperience[ids[i]];
                totalIndex++;
            }
        }
        ...
        Experience[] memory ret = new Experience[](totalIndex);
    ...
    function userNFTBalances(address account, uint256[] memory ids) external view returns (uint256[] memory) {
        require(account != address(0x0), 'LCNFT: Account is zero address');
        require(ids.length != 0, 'LCNFT: Ids length is zero');
    ...
    function gameDataSendNFT(
        ...
        require(_msgSender() == LCGD, 'LCNFT: caller is not LCGD');
        require(isApprovedForAll(from, operator), 'LCNFT: Not approved');
    ...
    function gameDataMint(
        ...
        require(_msgSender() == LCGD, 'LCNFT: caller is not LCGD');
        _mintNFT(to, id, 1, tokenURI, false, false);
    }
