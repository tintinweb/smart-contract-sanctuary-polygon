...
function setFeeForToken(address token, uint256 fee) public {
  require(admin==msg.sender);
  FeeForToken[token]=fee;
  ...
  function recieve(
    ...
    uint256 fee= FeeForToken[token].mulDiv(amount,100);
    userRemainingBalance[to][token] += (amount-fee);

    totalReservesForTokens[token] +=fee;
    ...
    returns (uint8, bytes32, bytes32)
  {
    ...
    function addToken(address token1,address token2, uint256 fee) external{
        require(msg.sender==admin);
        require(allowedTokens[token1]==0);
        allowedTokens[token1]=1;
        TokenToToken[token1]=token2;
        setFeeForToken(token1, fee);
        PauseTokens(token1);
}
...
function getLiquidityFromSupplyOnSecondBlockchain(address token,uint256 supplyAmount,uint256 nonce,bytes memory signature)external {
  require(totalSupplyByOwner[msg.sender][token]>=supplyAmount);
        uint256 reserves=totalReservesForTokens[token];
        uint256 supply=totalTokensSupply[token];
        uint256 liquidity=supplyAmount.mulDiv(reserves,supply);
        if(totalActualReservesForTokens[token]<liquidity){
          liquidity=liquidity-totalActualReservesForTokens[token];
          supplyAmount=liquidity.mulDiv(supply,reserves);
        }
...
//cant be loose
function PauseTokens(address token)public{
  require(pausedTokens[token]==0);
  require(admin==msg.sender);
