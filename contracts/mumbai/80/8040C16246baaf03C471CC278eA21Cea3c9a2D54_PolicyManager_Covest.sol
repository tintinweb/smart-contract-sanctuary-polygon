	...
	ReentrancyGuardUpgradeable
{
	...
	struct BuyPolicyParams {
		...
		address asset;
		string policyId;
		uint256 buyValue; // decimals 18 //
		uint256 maxCoverage; // decimals 18 //
	 ...
	 *  @return The uint8 value of the decimals.
	 */
	 ...
	 */
	function buyPolicyInternal(BuyPolicyParams memory _buyPolicyParams)
		private
		returns (bool)
	{
		require(
			_buyPolicyParams.asset != address(0),
			"Asset address is not valid."
		);

		require(
			_buyPolicyParams.expiresAt > block.timestamp,
			"Time has expired"
		);
		bytes32 hashMessage = verifyBuyCover(
			verifyBuyCoverParams(
				msg.sender,
				_buyPolicyParams.asset,
				_buyPolicyParams.policyId,
				_buyPolicyParams.buyValue,
				_buyPolicyParams.maxCoverage,
				_buyPolicyParams.coveragePeriodHours,
				_buyPolicyParams.generatedAt,
				_buyPolicyParams.expiresAt
			)
		);
	 ...
	 */
	function redeemPolicyInternal(RedeemPolicyParams memory _redeemPolicyParams)
		private
		returns (bool)
	{
		require(
			_redeemPolicyParams.expiresAt > block.timestamp,
			"Time has expired"
		);

		bytes32 hashMessage = verifyRedeemCover(
			verifyRedeemCoverParams(
				msg.sender,
				_redeemPolicyParams.asset,
				_redeemPolicyParams.policyId,
				_redeemPolicyParams.redeemPercentage,
				_redeemPolicyParams.generatedAt,
				_redeemPolicyParams.expiresAt
			)
		);
		...
		address signer = ECDSAUpgradeable.recover(
			message,
			_redeemPolicyParams.v,
			_redeemPolicyParams.r,
			_redeemPolicyParams.s
		);
		if (signer == V) {
			_existsHashMessage[hashMessage] = true;
			return true;
	 ...
	 * @return The boolean value that tell the policy param for buyPolicy is valid or not.
	 */
	function buyPolicyWillIssue(BuyPolicyParams memory _buyPolicyParams)
		private
		returns (bool)
	{
		...
		try
			PDS.issuePolicy(
				IPolicyDetails.IssuePolicyParams(
					_buyPolicyParams.policyId,
					msg.sender,
					_buyPolicyParams.asset,
					_buyPolicyParams.buyValue,
	...
	/**
	 * @notice buyPolicy.
	 * @param _buyPolicyParams The buy policy parameters as a tuple.
	 *
	 * Requirements:
	 ...
	 */
	function buyPolicy(BuyPolicyParams memory _buyPolicyParams)
		public
		nonReentrant
		isReady
		isApprove
		checkAsset(_buyPolicyParams.asset)
	{
		IERC20 _ERC20 = IERC20(_buyPolicyParams.asset);
		...
		require(
			isBlacklistAssetsUser(msg.sender, _buyPolicyParams.asset) != true,
		...
		require(
			PDS.exist(_buyPolicyParams.policyId) != true,
			"This _policyId exist."
		);
		...
		require(buyPolicyInternal(_buyPolicyParams), "Data error.");
		uint8 _decimalsForDiv = decimals() - _ERC20.decimals();
		require(
			_ERC20.allowance(msg.sender, address(this)) >=
				(_buyPolicyParams.buyValue / 10**_decimalsForDiv),
			"Insufficient allowance"
		);
		require(
			_ERC20.balanceOf(msg.sender) >=
				(_buyPolicyParams.buyValue / 10**_decimalsForDiv),
...

		{
			...
			bool isSuccess = buyPolicyWillIssue(_buyPolicyParams);
	...
	/**
	 * @notice redeemPolicy.
	 * @param _redeemPolicyParams The redeem policy parameters as a tuple.
	 *
	 * Requirements:
	 *  - `nonReentrant` Can't call multiple when the first tx is pending of a user.
	 *  - `isReady` The pool must not paused.
		...
		isReady
	{
		(
			...
			bool isActive,
			bool isRedeem
		) = PDS.policyData(_redeemPolicyParams.policyId);
		...
		require(
			_redeemPolicyParams.asset != address(0),
			"Asset address is not valid."
		);

		require(
			_redeemPolicyParams.asset == PolicyIdUserData.asset,
			"Invalid asset"
		);

		require(
			CP.pendingClaimRequest(_redeemPolicyParams.policyId) == 0,
		...
		require(redeemPolicyInternal(_redeemPolicyParams), "Redeem Error.");

		require(
			_redeemPolicyParams.redeemPercentage <= 100 &&
				_redeemPolicyParams.redeemPercentage > 0,
			"Error Percentage."
		);
		...
		require(
			PDS.whoIsRequest(redeemId) == msg.sender,
			"ERROR THIS REDEEMID NOT YOUR OWN."
		);

		uint256 amountRequested = (PDS.amountRequest(redeemId) /
			10**(decimals() - IERC20(PolicyIdUserData.asset).decimals()));
		uint256[] memory weightRequested = PDS.weightRequest(redeemId);
	 ...
	 * @param _verifyBuyCoverParams The verify buy cover (policy) parameters as a tuple.
		...
		returns (bytes32)
	{
		return
			keccak256(
				abi.encodePacked(
					_verifyBuyCoverParams.user,
					_verifyBuyCoverParams.asset,
					_verifyBuyCoverParams.policyId,
					_verifyBuyCoverParams.buyValue,
					_verifyBuyCoverParams.maxCoverage,
					_verifyBuyCoverParams.coveragePeriodHours,
					RM.isApprove(_verifyBuyCoverParams.user),
					RM.riskCoefficientOfUser(_verifyBuyCoverParams.user),
					_verifyBuyCoverParams.generatedAt,
					_verifyBuyCoverParams.expiresAt,
					address(this)
				)
			);
	}
	 ...
	 * _generatedAt Timestamp in second.
	 * _expiresAt Timestamp in second.
	 * @param _verifyRedeemCoverParams The verify redeem cover(policy) parameters as a tuple.
	 * @return The bytes32 value.
	 */
	function verifyRedeemCover(
		...
		return
			keccak256(
				abi.encodePacked(
					_verifyRedeemCoverParams.user,
					_verifyRedeemCoverParams.asset,
					_verifyRedeemCoverParams.policyId,
					_verifyRedeemCoverParams.redeemPercentage,
					_verifyRedeemCoverParams.generatedAt,
					_verifyRedeemCoverParams.expiresAt,
					address(this)
				)
			);
	}
