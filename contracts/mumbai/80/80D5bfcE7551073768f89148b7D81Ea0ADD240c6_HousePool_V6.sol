...
//------------------------------------------------------------
// Retain above structure as is
// Variables above are for internal version V6. 
// Do not change types or order of variables above.
// Add any new requireded variables for future versions below.
//------------------------------------------------------------
    ...
    function setSportsBookContract(address _sportsAddress)
        external
    ...
    function unstakeMax() public {
        ...
        bool success = IStakedToken(stakedToken).transfer(
            msg.sender,
    ...
    function getMyLiquidity(address _user) external view returns (int256) {
        return
            ceil(
                int256((balanceOf(_user) * liquidity)),
                int256(10**(pool_decimals * 2))
            ) / int256(totalSupply());
    }
    ...
    function updateBets(uint256[] memory _Id, uint256[] memory _payout)
        external
        onlyRole(HOUSE_POOL_DATA_PROVIDER)
    {
    ...
    function stake(address onBehalfOf, uint256 amount) public {
        require(
            amount > 1 && amount <= stakedToken.balanceOf(msg.sender),
            "HOUSEPOOL: INVALID AMOUNT"
        );
    ...
    function getMaxWithdrawal(address _user) public view returns (int256) {
        ...
        if (voi.expectedValue < userEVTracker[_user]) {
            ...
            maxUserWithdrawalAmount =
                ceil(int256(balanceOf(_user) * lpTokenPrice), 10**18) /
                10**18;
        } else if (voi.expectedValue > userEVTracker[_user]) {
            int256 XtotalValueLocked = int256(liquidity) -
                pendingStakes +
                (userEVTracker[_user]);
            uint256 XlpTokenPrice = (uint256(XtotalValueLocked) *
                10**MAX_PRECISION) / totalSupply();
            maxUserWithdrawalAmount =
                ceil(int256(XlpTokenPrice * balanceOf(_user)), 10**18) /
                10**18;
        }
