    ...
    IDelegator
{
    ...
    function borrowBalanceStored(
        address account
    ) external override returns (uint256 totalBorrowBalance, uint256 principal) {
        ...
        emit BorrowBalanceStored(totalBorrowBalance, principal);
    }
    ...
    function getAccountAssets(
    ...
    ) external override returns (CollateralMarket[] memory collateralMarkets) {
        bytes memory data = _delegatecall(abi.encodeWithSelector(
        ...
        emit GetAccountAssets(collateralMarkets);
    }

    function exchangeRateStored() external override returns (uint256 rate) {
        bytes memory data = _delegatecall(abi.encodeWithSelector(
    ...
    function getAccountLiquidity(
        ...
        emit GetAccountLiquidity(liquidity, shortfall);
    }
    ...
    function addChain(uint16 chainId) external override {
        _delegatecall(abi.encodeWithSelector(
    ...
    function setDestination(
        ...
        _delegatecall(abi.encodeWithSelector(
            MasterAdmin.setDestination.selector,
            _dstChainId,
...
abstract contract MasterAdmin is IMaster, IMasterState, MasterModifiers {
    function addChain(uint16 chainId) external override onlyOwner {
        chains.push(chainId);
    }
...
abstract contract IMasterState is MasterStorage {
    ...
    function addChain(uint16 chainId) external virtual;
...
abstract contract MasterInternals is IMaster, MasterEvents {
    ...
    function _liquidateBorrow(
        ...
        uint256 protocolSeizeAmount = _exchangeRateStored() * protocolSeizeTokens / 1e8;
        markets[chainId][pTokenCollateral].totalReserves += protocolSeizeAmount;
        markets[chainId][pTokenCollateral].totalSupply -= protocolSeizeTokens;
