...
contract GenesisAirDrop is ReentrancyGuard, Ownable, Pausable {
    ...
    mapping(uint256 => uint256) _category_limit;

    /* mangoNFT category price
    key: category name
    value:mangoNFT price */
    mapping(uint256 => uint256) _category_price;

    /* mangoNFT saled count by category
    key: category name
    value:saled count */
    mapping(uint256 => uint256) _category_saled_count;
    ...
    constructor(
        ...
        _treasury = treasury;
        _category_price[category1] = price1;
        _category_limit[category1] = limit1;

        _category_price[category2] = price2;
        _category_limit[category2] = limit2;

        _category_price[category3] = price3;
        _category_limit[category3] = limit3;
    }
    ...
    function getLimit(uint256 category) public view returns (uint256) {
        return _category_limit[category];
    }

    function setLimit(uint256 category, uint256 limit) public onlyOwner {
        _category_limit[category] = limit;
    }

    function setPrice(uint256 category, uint256 price) public onlyOwner {
    ...
    function getSaledCount(uint256 category) public view returns (uint256) {
    ...
    /*claim a Genesis NFT*/
    function claim(uint256 category, uint256 quantity)
        public
        payable
        nonReentrant
        whenNotPaused
