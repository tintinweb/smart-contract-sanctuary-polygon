...
contract degree is ERC721URIStorage{
    ...
    constructor()ERC721("collegedegree","degree"){
   owner=msg.sender;
    }
...
mapping(address =>bool) public issuedegree;

function issue_degree(address _to) onlyowner external{
  issuedegree[_to]=true;

 }
 function claimdegree(string memory tokenuri) public returns(uint256){
  require(issuedegree[msg.sender],"Degree not issued");

  _tokensIds.increment();
  uint256 newid= _tokensIds.current();
  _mint(msg.sender,newid);
  _setTokenURI(newid,tokenuri);
  issuedegree[msg.sender]=false;
