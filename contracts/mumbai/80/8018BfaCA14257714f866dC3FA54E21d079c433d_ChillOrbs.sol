    ...
    DefaultOperatorFilterer
{
    ...
    function setTokenFactoryAddress(address _tokenFactory) external onlyOwner {
    ...
    // Helper functions to call the zipped contract
    function generateJSONURI(string memory _previewBaseURI, uint256 _tokenId, bool _isColored) public view returns (string memory) {
        __forwardToZipped(abi.encodeCall(this.generateJSONURI, (_previewBaseURI, _tokenId, _isColored)));
    }

    function __forwardToZipped(bytes memory callData) private view {
        function (bytes memory) fwd = __forwardToZippedNonView;
        function (bytes memory) view vfwd;
        assembly ("memory-safe") { vfwd := fwd }
        vfwd(callData);
    }

    function  __forwardToZippedNonView(bytes memory callData) private {
        address zipped = tokenFactoryAddress;
        assembly ("memory-safe") {
            let s := call(gas(), zipped, callvalue(), add(0x20, callData), mload(callData), 0x00, 0x00)
        ...
        returns (string memory)
    {
        ...
        return generateJSONURI(previewBaseURI, tokenId, isColored(tokenId));
    }
