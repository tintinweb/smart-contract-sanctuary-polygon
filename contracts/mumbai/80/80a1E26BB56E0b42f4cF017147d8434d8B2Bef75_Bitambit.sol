...
contract Bitambit {
    ...
    function invest(address referrer) public payable {
        ...
        _usersDownlineIncomeDistribution(msg.sender,msgValue);  
         ...
         //_users UplineIncome
        _usersUplineIncomeDistribution(msg.sender,msgValue);   
    ...
    function reinvest(address _user, uint256 _amount) private{
        ...
        _usersDownlineIncomeDistribution(_user,_amount);
         ...
         //_users UplineIncome
        _usersUplineIncomeDistribution(_user,_amount); 
  ...
  function withdrawal() external{
    ...
    _user.singleUplineBonusTaken = users[msg.sender].singleUplineBonus;
  ...
  function _usersUplineIncomeDistribution(address _user, uint256 _Amount) internal {
      uint256 DistributionPayment = _Amount.mul(maxdownline).div(100);
      address downlineparam = users[_user].singleDownline;
      for (uint i = 0; i < maxdownline; i++) {
            if (downlineparam != address(0)) {
            uint256 payableAmount = DistributionPayment.div(maxdownline);
            users[downlineparam].singleUplineBonus = users[downlineparam].singleUplineBonus.add(payableAmount); 
            downlineparam = users[downlineparam].singleDownline;
            }else break;
        }
  }
  ...
  function TotalBonus(address _user) public view returns(uint256){
     uint256 TotalEarn = users[_user].referrerBonus.add(users[_user].singleUplineBonus).add(users[_user].singleDownlineBonus).add(users[_user].stakeHolderBonus);
