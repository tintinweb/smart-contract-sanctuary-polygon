...
contract OmniERC721 is IOmniERC721, ERC721URIStorage {
    ...
    function setDates(uint256 _from, uint256 _to) onlyRepository external override {
        require(_from > 0 && (_to == 0 || _to > _from) && _from >= (block.timestamp - 1 days) && _from <= (block.timestamp + 92 days + 1 hours) && _to <= (_from + 8 days));
        dropFrom = _from;
        dropTo = _to;
    }
    ...
    function getOwnedBy(address addr) public view returns (uint256[] memory) {
        return ownedBy[addr];
    }
}
