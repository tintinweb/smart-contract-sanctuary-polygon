...
contract EternalStorage is MembershipsTypes, AccessControl {
	...
	mapping(address => mapping(uint256 => CampaignsAddress)) public campaignsByAddress;
	...
	function getSchedule(bytes32 record) external view returns (MintingSchedule memory) {
		return schedules[record];
	}

	function setSchedule(bytes32 record, MintingSchedule calldata value) external onlyRole(WRITER_ROLE) {
		schedules[record] = value;
	}

	function getReferral(bytes32 record) external view returns (ScheduleReferral memory) {
		return schedulesReferral[record];
	}

	function setReferral(bytes32 record, ScheduleReferral calldata value) external onlyRole(WRITER_ROLE) {
		schedulesReferral[record] = value;
	}

	function removeReferral(bytes32 record, address oldReferral) external onlyRole(WRITER_ROLE) {
		...
		for (uint256 i = 0; i < campaignsByOwner; i++) {
			if (campaignsByAddress[oldReferral][i].campaignIndex == scheduleToCampaign[record].campaignIndex) {
				campaignsByAddress[oldReferral][i] = campaignsByAddress[oldReferral][campaignsByOwner - 1];
	...
	function getBuyPerWallet(bytes32 scheduleID, address addr) external view returns (uint256) {
		return _buyPerWallet[scheduleID][addr];
	}

	function setTokensAllowed(address token, bool value) external onlyRole(WRITER_ROLE) {
		for (uint256 i = 0; i < tokensAllowedArr.length; i++) {
			if (tokensAllowedArr[i] == token) {
				if (value) {
					return;
				} else {
					tokensAllowedArr[i] = tokensAllowedArr[tokensAllowedArr.length - 1];
	...
	function getBuyWalletCount(bytes32 scheduleID) external view returns (uint256) {
		return _buyPerWalletCount[scheduleID];
	}

	function updateCampaignMetadata(bytes32 campaignId, string calldata metadata) external onlyRole(WRITER_ROLE) {
		campaigns[campaignToIndex[campaignId]].metadata = metadata;
	}

	function getCampaignMetadata(bytes32 campaignId) external view returns (string memory) {
	...
	function getClaimed(bytes32 scheduleID, UserType userType) external view returns (uint256) {
		return _claimed[scheduleID][userType];
	}

	function addCampaign(Campaign calldata value) external onlyRole(WRITER_ROLE) {
		bytes32 phase0 = value.phases[0];

		campaigns.push(value);

		for (uint256 i = 0; i < value.phases.length; i++) {
			scheduleToCampaign[value.phases[i]] = ScheduleCampaign(value.campaignId, campaigns.length - 1, i);
		}
		...
		campaignsByAddress[owner][campaignsByOwner].campaignIndex = campaigns.length - 1;
	...
	function updateReferralIndex(address referral, uint256 campaignIndex) external onlyRole(WRITER_ROLE) {
		uint256 referralCampaignsCount = campaignsByAddressLength[referral];
		campaignsByAddress[referral][referralCampaignsCount].campaignIndex = campaignIndex;
		campaignsByAddress[referral][referralCampaignsCount].userType = UserType.REFERRAL;
		campaignsByAddressLength[referral]++;
	}

	function getCampaign(uint256 record) external view returns (Campaign memory) {
		return campaigns[record];
	}

	function getCampaignCreatedByAddress(address addr) external view returns (uint256) {
		return campaignsCreatedByAddress[addr];
	}

	function getCampaignByAddressLength(address addr) external view returns (uint256) {
...
contract MembershipsImpl is MembershipsTypes, AccessControl, MembershipsErrors {
	...
	event EventAllowlistUpdated(bytes32 indexed scheduleId, bytes32 indexed newRoot);

	event EventScheduleOwnerTransferred(bytes32 indexed scheduleId, address indexed oldOwner, address indexed newOwner);

	event EventScheduleRevoked(bytes32 indexed scheduleId);

	event EventUnsoldTokensClaimed(address indexed memberships, bytes32 indexed scheduleId, uint256 amount);

	// @dev: we emit two kind of event, one per buy and one per token
	event EventBuyLot(address indexed from, bytes32 indexed scheduleId, uint256 lots);

	event EventBuyToken(address indexed from, bytes32 indexed scheduleId, address indexed token, uint256 tokens);

	event EventClaim(address indexed from, bytes32 indexed scheduleId, uint256 value);
	...
	function getBuyPerWallet(bytes32 scheduleId, address addr) internal view returns (uint256) {
	...
	function setTokensAllowed(address token, bool value) public onlyRole(MEMBERSHIP_ROLE) {
		_eternalStorage.setTokensAllowed(token, value);
	}

	function getClaimed(bytes32 scheduleId, UserType userType) public view returns (uint256) {
	...
	function addCampaign(Campaign memory value) external onlyRole(MEMBERSHIP_ROLE) {
		_eternalStorage.addCampaign(value);
	}

	function getSchedule(bytes32 record) public view returns (MintingSchedule memory) {
		return _eternalStorage.getSchedule(record);
	}

	function setSchedule(bytes32 record, MintingSchedule memory value) public onlyRole(MEMBERSHIP_ROLE) {
		_eternalStorage.setSchedule(record, value);
	}

	function getReferral(bytes32 record) public view returns (ScheduleReferral memory) {
		return _eternalStorage.getReferral(record);
	}

	function setReferral(bytes32 record, ScheduleReferral memory value) public onlyRole(MEMBERSHIP_ROLE) {
		_eternalStorage.setReferral(record, value);
	}

	function _removeReferral(bytes32 record, address oldReferral) internal onlyRole(MEMBERSHIP_ROLE) {
		_eternalStorage.removeReferral(record, oldReferral);
	}

	function updateCampaignMetadata(bytes32 campaignId, string calldata metadata) public onlyRole(MEMBERSHIP_ROLE) {
	...
	// VALIDATORS
	// ==================
	function createMintingScheduleValidation(CreateMintingScheduleParams calldata params) external view {
		...
		if (
			params.lotToken.length == 0 || params.lotSize.length == 0 || params.lotToken.length != params.lotSize.length
		) revert ErrorME18LotArrayLengthMismatch();
		if (
			(params.referral != address(0) || params.referralFee != 0) &&
			(params.referral == address(0) || params.referralFee == 0)
		) revert ErrorME20InvalidReferral();
		if (params.referralFee >= HelperLib.FEE_SCALE) revert ErrorME21InvalidReferralFee();
		if (params.amountTotal == 0) revert ErrorME28InvalidAmount();
		if (params.maxBuyPerWallet == 0) revert ErrorME29InvalidMaxBuyPerWallet();
	}
	...
	function buy(
		...
		if (amount + schedule.released > schedule.amountTotal) revert ErrorME27TotalAmountExceeded();

		if (amount + callerPreviousAmount > schedule.maxBuyPerWallet) revert ErrorME22MaxBuyPerWalletExceeded();

		if (schedule.paymentAsset.assetType == AssetType.ETH) {
			if (msgValue != schedule.pricePerLot * amount) revert ErrorME19NotEnoughEth();
		} else {
			IERC20 token = IERC20(schedule.paymentAsset.token);
			token.safeTransferFrom(caller, memberships, schedule.pricePerLot * amount);
		}
		...
		for (uint256 i = 0; i < schedule.lotToken.length; i++) {
			...
			token.safeTransferFrom(memberships, caller, schedule.lotSize[i] * amount);

			emit EventBuyToken(caller, scheduleId, schedule.lotToken[i], schedule.lotSize[i] * amount);
		}
	 ...
	 * @notice In original contract this method is called Withdraw
	 */
	function claim(address memberships, bytes32 scheduleId) external onlyRole(MEMBERSHIP_ROLE) {
		MintingSchedule memory schedule = getSchedule(scheduleId);

		uint256 earned = schedule.pricePerLot * schedule.released;

		uint256 totalClaimed = getClaimed(scheduleId, MembershipsTypes.UserType.OWNER);

		if (totalClaimed != 0 && totalClaimed >= earned) revert ErrorME23TotalClaimedError();
	...
	function claimRoll(
		...
		uint256 earned = HelperLib.getFeeFraction(schedule.pricePerLot * schedule.released, schedule.rollFee);

		uint256 totalClaimed = getClaimed(scheduleId, MembershipsTypes.UserType.ROLL);
	...
	function claimReferral(address memberships, bytes32 scheduleId) external onlyRole(MEMBERSHIP_ROLE) {
		MintingSchedule memory schedule = getSchedule(scheduleId);

		ScheduleReferral memory scheduleR = getReferral(scheduleId);

		uint256 earned = HelperLib.getFeeFraction(schedule.pricePerLot * schedule.released, scheduleR.referralFee);

		uint256 totalClaimed = getClaimed(scheduleId, MembershipsTypes.UserType.REFERRAL);
	...
	function claimUnsoldTokens(address memberships, bytes32 scheduleId) external onlyRole(MEMBERSHIP_ROLE) {
		MintingSchedule memory schedule = getSchedule(scheduleId);

		uint256 totalClaimed = getClaimed(scheduleId, MembershipsTypes.UserType.UNSOLD);

		if (schedule.amountTotal - schedule.released != 0 && totalClaimed == (schedule.amountTotal - schedule.released))
		...
		setClaimed(scheduleId, MembershipsTypes.UserType.UNSOLD, (schedule.amountTotal - schedule.released));

		emit EventUnsoldTokensClaimed(memberships, scheduleId, schedule.amountTotal - schedule.released);
	}
	...
	function verifyMerkle(
		...
		if (!MerkleProof.verify(proof, schedule.merkleRoot, leaf)) revert ErrorME24InvalidProof();
	}
	...
	// set a new merkle tree root
	function setAllowlist(bytes32 scheduleId, bytes32 root) external onlyRole(MEMBERSHIP_ROLE) {
	...
	// transfer the ownership
	function transferScheduleOwner(bytes32 scheduleId, address owner_) external onlyRole(MEMBERSHIP_ROLE) {
		MintingSchedule memory s = getSchedule(scheduleId);
		if (s.owner != owner_) {
	...
	// change referral
	function updateReferral(bytes32 scheduleId, address referral) external onlyRole(MEMBERSHIP_ROLE) {
		ScheduleReferral memory s = getReferral(scheduleId);
		address oldReferral = s.referral;
		s.referral = referral;

		(, uint256 campaignIndex, ) = _eternalStorage.scheduleToCampaign(scheduleId);
	...
	function revoke(bytes32 scheduleId) external onlyRole(MEMBERSHIP_ROLE) {
		MembershipsTypes.MintingSchedule memory schedule = getSchedule(scheduleId);
	 ...
	 */
	function computeUnsoldLots(bytes32 scheduleId) external view returns (uint256) {
		MembershipsTypes.MintingSchedule memory schedule = getSchedule(scheduleId);
