...
contract i1WPNFT is ERC721URIStorage, Ownable {
	...
	function createNFT (address receiver, string memory tokenURI) external onlyOwner returns (uint256) {
		_tokenIds.increment();
	...
	function createWPNFT (address receiver, string memory tokenURI, uint256 newItemId) external onlyOwner returns (uint256) {
		_mint(receiver, newItemId);
		_setTokenURI(newItemId, tokenURI);
