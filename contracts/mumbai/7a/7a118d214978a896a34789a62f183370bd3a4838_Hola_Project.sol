...
// File: upgradeable/OperatorFiltererUpgradeable.sol
...
// File: upgradeable/DefaultOperatorFiltererUpgradeable.sol
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize(
        uint256[] memory _fIds,
        uint256[] memory _fAmount,
        string memory _contractURI
    ) public initializer {
        __ERC1155_init(
            "ipfs://bafybeiakoqfhdgydvaxgxzm3irkj5nvmtb4qc2ug5ybk7b6aonokj2oxzi/"
        );
        __Ownable_init();
        __Pausable_init();
        ...
        mintBatch(msg.sender, _fIds, _fAmount);
    }
    ...
    function mint(uint256 id, uint256 amount) public payable nonReentrant {
        ...
        require(id > 0 && id <= 385, "Not an existing NFT");
        require(
            msg.value == mintPrice * amount,
            "Incorrect payment, please enter the right purchase amount!"
        );
        require(
            totalSupply(id) + amount <= maxSupply,
            "Sorry, max supply reached or will be exceded by this purchase."
        );
    ...
    function mintBatch(
        ...
        require(!paused(), "Minting is paused");
        for (uint256 i = 0; i < ids.length; i++) {
            require(ids[i] > 0 && ids[i] <= 385, "Not an existing NFT");
            require(
                totalSupply(ids[i]) + amounts[i] <= maxSupply,
                stringifyingID(ids[i])
            );
        }
        _mintBatch(to, ids, amounts, "");
    }

    function stringifyingID(uint256 _id)
        internal
        view
        virtual
        returns (string memory)
    {
        return
            string(
                abi.encodePacked(
                    "Sorry, max supply for token ID = ",
                    StringsUpgradeable.toString(_id),
                    " reached or will be exceded by this purchase."
                )
            );
    }
    ...
    function setProjectDetails(string memory _name, string memory _symbol)
        external
        onlyOwner
    {
        name = _name;
