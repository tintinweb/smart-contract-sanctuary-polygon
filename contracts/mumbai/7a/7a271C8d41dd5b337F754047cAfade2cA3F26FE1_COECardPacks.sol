...
contract COECardPacks is ERC1155, Ownable, ReentrancyGuard {
  ...
  constructor(address _aegAddress) ERC1155("COE Card Packs") {
    totalTokens = 0;
  ...
  function mintBatch(
    ...
    for (uint256 i = 0; i < _ids.length; i++) {
      ...
      require(tokens[_ids[i]].exists && _ids[i] != 0, "Token doesn't exist."); //0 is basic pack and can only be minted by fpMintBasic(from burnAndMintAddress)
  ...
  function withdrawERC20(address _tokenAddress) public onlyOwner {
    ...
    FunctionInterface(_tokenAddress).transfer(
      msg.sender,
      FunctionInterface(_tokenAddress).balanceOf(address(this))
    );
  }
}
