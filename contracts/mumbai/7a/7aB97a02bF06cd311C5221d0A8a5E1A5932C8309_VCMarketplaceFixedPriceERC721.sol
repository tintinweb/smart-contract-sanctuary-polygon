...
contract VCMarketplaceFixedPriceERC721 is IVCMarketplaceFixedPriceERC721, VCMarketplaceBase, ERC721Holder {
    ...
    function unlistFixedPrice(uint256 _tokenId) external {
        ...
        if (listing.minted) {
            IArtNftERC721(artNft).transferFrom(address(this), listing.seller, _tokenId);
        }
    ...
    function _newList(ListingData memory data) internal {
        bool minted = IArtNftERC721(artNft).exists(data.tokenId);
        if (!minted) {
            IArtNftERC721(artNft).requireCanRequestMint(msg.sender, data.tokenId);
        } else {
            IArtNftERC721(artNft).transferFrom(msg.sender, address(this), data.tokenId);
        }
    ...
    function _minting(FixedPriceListing memory listing, uint256 _tokenId) internal {
        if (!listing.minted) {
            IArtNftERC721(artNft).mintTo(_tokenId, address(this));
            fixedPriceListings[_tokenId].minted = true;
        }
        IArtNftERC721(artNft).transferFrom(address(this), msg.sender, _tokenId);
