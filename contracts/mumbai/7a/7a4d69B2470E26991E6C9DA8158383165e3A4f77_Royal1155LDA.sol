...
import { IRoyalties } from "../royalties/IRoyalties.sol";
    ...
    IRoyal1155LDA
{
    ...
    /**
     * @notice (tierID) => wallet => count
     *  Represents the number of LDAs owned by a user within a particular tier.
     * @dev We must maintain this count correctly whenever a transfer happens.
     *  Also included is an owner-only function setOwnedTokens to backfill this mapping.
     */

    mapping(uint128 => mapping(address => uint128[])) private userOwnedTokens;

    address public _EXTRAS_CONTRACT_;
    address public _ROYALTIES_CONTRACT;

    bool public isOwnerBackfillComplete;
    ...
    /**
     * @dev Called by the owner to backfill the mapping of owned token counts by user.
     */
    function setOwnedTokens(
        ...
        onlyOwner
    {
        require(!isOwnerBackfillComplete, "Backfill already run");
        require(owners.length == balances.length, "Params length mismatch");
        for (uint256 i = 0; i < owners.length; i++) {
            userOwnedTokens[tierId][owners[i]] = balances[i];
        }
    }
    ...
    {
        uint128[] memory tokens = userOwnedTokens[tierId][owner];
        return tokens;
    }
    ...
    function completeOwnerBackfill()
        external
        onlyOwner
    {
        isOwnerBackfillComplete = true;
    }
    ...
    {
        require(
            !this.tierExists(tierID) && _tierCurrentSupply[tierID] == 0,
            "Tier already exists"
        );
        ...
        onlyOwner
    {
        ...
        (uint128 _tierID, , ) = RoyalUtil.decomposeLDA_ID(ldaIDs[0]);
        ...
        for (uint256 a = 0; a < ldaIDs.length; a++) {
            require(_owners[ldaIDs[a]] == address(0), "LDA already minted");
            (uint128 curTierID, , ) = RoyalUtil.decomposeLDA_ID(ldaIDs[a]);
        ...
        whenNotPaused
    {
        ...
        (uint128 tierID, , uint128 tokenID) = RoyalUtil.decomposeLDA_ID(ldaID);
        ...
        override
    {
        ...
        for (uint256 a; a < ids.length; a++) {
            ...
            (uint128 tierID, , uint128 tokenID) = RoyalUtil.decomposeLDA_ID(ids[a]);

            // call the callback on the Royalties contract for this transfer
            IRoyalties(_ROYALTIES_CONTRACT).onLdaTokenTransfer(from, to, tierID);

            // update the user owned token counts
            if (to != address(0)) {
                userOwnedTokens[tierID][to].push(tokenID);
            }

            if (from != address(0) && (isOwnerBackfillComplete || userOwnedTokens[tierID][from].length != 0)) {
                uint128[] memory currTokens = userOwnedTokens[tierID][from];
                // TODO: optimized array delete
                delete userOwnedTokens[tierID][from];
                for (uint256 i = 0; i < currTokens.length; i++) {
                    if (currTokens[i] != tokenID) {
                        userOwnedTokens[tierID][from].push(currTokens[i]);
                    }
                }
            }
        ...
        override
    {
        ...
        require(tokenOwner != address(0), "token DNE");
        require(
        ...
        // Bump version number
        (uint128 tierID, uint256 version, uint128 tokenID) = RoyalUtil
            .decomposeLDA_ID(ldaID);
        uint256 newLDA_ID = RoyalUtil.composeLDA_ID(tierID, ++version, tokenID);
    ...
    /**
     * Setter for {_ROYALTIES_CONTRACT_} that defines the address for the extras contract
     */
    ...
    function setRoyaltiesContract(
        ...
        onlyOwner
    {
        _ROYALTIES_CONTRACT = newRoyaltiesContract;
    }
}
