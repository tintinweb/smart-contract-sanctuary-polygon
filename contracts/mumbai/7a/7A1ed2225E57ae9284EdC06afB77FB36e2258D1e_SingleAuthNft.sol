...
contract SingleAuthNft is ERC721, ERC721Enumerable, ERC721Burnable, Ownable {
    ...
    constructor() ERC721("Single Auth NFT", "SANFT") {
        self = address(this);
        signer = msg.sender;
     ...
     * @param _handleType The type of the handle.
    ...
    function mint(
        ...
        string memory _handleType,
        uint256 _profileId,
        Signature calldata _signature
    ) public returns (uint256) {
        require(_to != address(0), "Invalid address");
        bytes32 messagehash = keccak256(
            abi.encodePacked(_profileId, _handle, _handleType)
        );
        require(
        ...
        emit MintNft(_to, newTokenId, _handle, _handleType);
        _tokenIds.increment();
    ...
    /**
     * @dev Burns `tokenIds`. See {ERC721-_burn}.
     *
     * Requirements:
     *
     * - `tokenId` must exist.
    ...
    function _baseURI() internal pure override returns (string memory) {
        return "https://www.lens-profile.xyz/";
    }

    /**
     * @dev Override _transfer, not allow owner to transfer nft.
     */
    function _transfer(
        address from,
        address to,
