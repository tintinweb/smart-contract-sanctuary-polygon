...
contract ClaimDIVALinearVesting is Ownable, Pausable {
    ...
    function claimTokens(
        ...
        require(trigger, "ClaimDIVA: Contract should be triggered by admin before claiming.");
        ...
        require(claimedAmount[msg.sender].add(amountClaimable) <= _amount, "ClaimDIVA: Full amount already claimed");
    ...
    function availableDrawDownAmount(
        ...
        uint256 drawDownRate = _vestingAmount.div(_time);
        uint256 amountClaimable = timePassedSinceLastInvocation.mul(drawDownRate);
    ...
    function setClaimPeriodStartTime(uint256 _claimPeriodStarts) external isTriggered onlyOwner {
        require(_claimPeriodStarts > block.timestamp, "ClaimDIVA: Time must be greater than current time");
        claimPeriodStarts = _claimPeriodStarts;
        emit ClaimStartTimeChanged(_claimPeriodStarts);
    }

    function setMerkleRoot(bytes32 _merkleRoot) external isTriggered onlyOwner {
    ...
    function setTrigger() external onlyOwner isTriggered {
        trigger = true;
    }

    function withdrawUnclaimedDivaTokens() external onlyOwner {
        require(
            block.timestamp >= claimPeriodStarts.add(year.mul(3)),
            "ClaimDIVA: cant claim diva tokens until three years from claim start date"
        );
