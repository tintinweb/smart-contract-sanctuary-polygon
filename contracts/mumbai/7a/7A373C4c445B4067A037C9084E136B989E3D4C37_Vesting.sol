...
contract Vesting is Ownable {
    ...
    mapping (address => uint) public firstClaimOfType4;
    ...
    */
    ...
    function isValidInvestor (address _investor) view internal returns(bool) {
        if (Investors[_investor].investorType<=5 && Investors[_investor].investorType !=0)
        return true;
        else return false;
    }
    ...
    function calculateMoney(address _sender, uint percent) view internal returns (uint) {
        return ((Investors[_sender].vestingBalance)*percent)/100;
    }
    ...
    function claimIntermediate() external {
        ...
        uint _tokensToRelease = calculateMoney(_msgSender(), 5);
    ...
    function withDrawFirstTime4 () external {
        require (Investors[_msgSender()].isInitialAmountClaimed==false,'Initial_Amount Not Claimed');
        require (Investors[_msgSender()].investorType==4);
        Investors[_msgSender()].isInitialAmountClaimed = true;
        firstClaimOfType4[_msgSender()] = block.timestamp;
    ...
    function withdrawTokenInvestor4 () external {
        require (Investors[_msgSender()].isInitialAmountClaimed==true,'Initial_Amount Not Claimed');
        require (Investors[_msgSender()].claimTimes < 4);
        // @dev you can change 30 days to any time of your wish to test the code
        require (block.timestamp >= firstClaimOfType4[_msgSender()]+ (Investors[_msgSender()].claimTimes+1) * 30 days, 'Claim Time Not Reached');
        require (Investors[_msgSender()].investorType == 4 ,'Not valid investor');
        Investors[_msgSender()].claimTimes +=1;
        uint _tokensToRelease = calculateMoney(_msgSender(), 20);
        Investors[_msgSender()].vestingBalance -= _tokensToRelease;
        token.transfer(_msgSender() , _tokensToRelease);
        emit TokenWithdraw(_msgSender(), _tokensToRelease);
    }

    function withdrawTokenInvestor5 () external {
        require (Investors[_msgSender()].isInitialAmountClaimed==true,'Initial_Amount Not Claimed');
        require (Investors[_msgSender()].claimTimes < 4);
        // @dev you can change 30 days to any time of your wish to test the code
        require (block.timestamp >= seedStartDate + (Investors[_msgSender()].claimTimes+1) * 30 days, 'Claim Time Not Reached');
        require (Investors[_msgSender()].investorType == 5 ,'Not valid investor');
        Investors[_msgSender()].claimTimes +=1;
        uint _tokensToRelease = calculateMoney(_msgSender(), 25);
        Investors[_msgSender()].vestingBalance -= _tokensToRelease;
        token.transfer(_msgSender() , _tokensToRelease);
        emit TokenWithdraw(_msgSender(), _tokensToRelease);
    }
