...
contract Polytank is Ownable, ReentrancyGuard, IERC721, IERC165, IERC721Metadata, IERC721Enumerable {
    using SafeMath for uint256;
    using EnumerableSet for EnumerableSet.UintSet;
    // using EnumerableSetMarket for EnumerableSetMarket.MarketLotSet;

    string internal _name = "Polytank";
    string internal _symbol = "PTANK";
    string private _baseURI;

    uint256 internal totalMinted = 0;

    uint256 public MINTING_LIMIT = 3;
...
mapping (uint256 => address) public poolUser;
    ...
    uint256 public mintingPrice = 150000000000000000;
    uint256 public whitelistOff = 30000000000000000;
    ...
    // devs
    address private dev1 = 0xfF5ccfcdAc26A042CfCC078edAD2F7eA640eD3e2;
    ...
    uint256[]  internal indices = new uint256[](MINTING_LIMIT);
    ...
    function increseTotalSupply() external {
        MINTING_LIMIT = 6;
  ...
  function addToWhiteList(address _walletForWhitelist) onlyOwner external  {

            if(!users[_walletForWhitelist].isExist){

                UserStruct memory userStruct;
               
                userStruct = UserStruct({
                isExist : true,
                wallet : _walletForWhitelist
                });
                users[_walletForWhitelist] = userStruct;   
            }
        }
    ...
    function mint(uint256 amount,address psua) external payable nonReentrant mintingEnabled {

        uint256 nftPrice = getNFTPrice(amount);
        if(users[psua].isExist && !users[msg.sender].isExist)
        {
            
            uint256 finalNftPrice = nftPrice.sub(whitelistOff.mul(amount));
            if (msg.value > finalNftPrice) {
                payable(msg.sender).transfer(msg.value - finalNftPrice);
            }
            _promint(amount,finalNftPrice);
            
            payable(psua).transfer(whitelistOff.mul(amount));

        }
        
        else if(!users[msg.sender].isExist && !users[psua].isExist)
        {
            if (msg.value > nftPrice) {
                payable(msg.sender).transfer(msg.value - nftPrice);
            }
            _promint(amount,nftPrice);
          
        }
        else if(users[msg.sender].isExist )
        {

            uint256 finalNftPrice = nftPrice.sub(whitelistOff.mul(2).mul(amount));
            if (msg.value > finalNftPrice) {
                payable(msg.sender).transfer(msg.value - finalNftPrice);
            }
            _promint(amount,finalNftPrice);
    ...
    function _promint(uint256 amount,uint256 nftPrice)  internal {
         if(msg.value < nftPrice) {
            revert NOT_ENOUGH_MATIC();
            }
