...
contract mQuarkNFTImplementation is ImQuarkNFT, ERC721, Ownable, Initializable, ReentrancyGuard {
  event TokenMint(
    uint256 tokenId,
    address to,
    uint64 projectId,
  ...
  event WithdrawProtocol(address to, uint256 amunt, uint256 savedAmountOwner, uint256 totalWithdrawn);
  event Withdraw(address to, uint256 amunt, uint256 savedAmountProtocol, uint256 totalWithdrawn);
  ...
  uint32 public constant ROYALTY_DIVISOR = 1000000000;
  ...
  function initilasiable(
    ...
    //   _collection.mintType,
    //   _collection.mintPrice,
    //   _collection.mintPerAccountLimit,
    //   _collection.name,
    //   _collection.symbol,
    //   _collection.verifier,
    //   _collection.isWhitelisted,
    //   _collection.isFree
    // );
  }
  ...
  function mint(uint256 _variationId) external payable {
    ...
    if (m_tempData.collectionURIs.length <= _variationId) revert InvalidVariation("A721", _variationId);
    //paid
    if (m_tempData.mintType < 6) {
      if (msg.value == 0 || msg.value != m_tempData.mintPrice) revert InvalidPayment("E721");
    }
    //free
    else {
      if (msg.value != 0) revert NoPaymentRequired("F721");
    }
  ...
  function mintWithURI(
    ...
    Collection memory m_tempData = _validityChecks();
    if (m_tempData.mintType != 3 && m_tempData.mintType != 9) revert WrongMintType("D721", m_tempData.mintType);
    if (
      !_verifySignature(
        _signature,
        _signer,
  ...
  function mintWhitelist(bytes32[] memory _merkleProof, uint256 _variationId) external payable {
    ...
    if (!MerkleProof.verify(_merkleProof, s_merkleRoot, leaf)) revert NotWhitelisted("H721");
    if (m_tempData.collectionURIs.length <= _variationId) revert InvalidVariation("A721", _variationId);
    //paid
    if (m_tempData.mintType < 6) {
      if (msg.value == 0 || msg.value != m_tempData.mintPrice) revert InvalidPayment("E721");
   ...
   * Makes a call to mQuark contract to mint single NFT with given validated URI.
   *
   * @param _signer       Registered project address of the given collection
   * @param _signature    Signed data by project's wallet
   * @param _uri          The metadata URI that will represent the template.
   * Dynamic variation -  Whitelisted
   */
  function mintWithURIWhitelist(
    ...
    Collection memory m_tempData = _validityChecks();
    if (m_tempData.mintType != 2 && m_tempData.mintType != 8) revert WrongMintType("D721", m_tempData.mintType);
    bytes32 leaf = keccak256(bytes.concat(keccak256(abi.encode(msg.sender))));
    if (!MerkleProof.verify(_merkleProof, s_merkleRoot, leaf)) revert NotWhitelisted("H721");
    if (
      !_verifySignature(
   ...
   * @param _projectId              The ID of the slot's project.
   * @param _projectDefaultUri The project's default URI that will be set to the added slot.
   */
  function subscribeToProject(
    address _owner,
    uint256 _tokenId,
    ...
    s_tokenSubscriptions[_tokenId][_projectId] = TokenSubscriptionInfo(true, _projectDefaultUri);
  }
  ...
  function subscribeToProjects(
    ...
    for (uint256 i = 0; i < m_numberOfProjects; ) {
      s_tokenSubscriptions[_tokenId][_projectIds[i]] = TokenSubscriptionInfo(true, _projectDefaultUris[i]);
      unchecked {
        ++i;
      }
    }
  ...
  function updateURISlot(
    ...
    if (ownerOf(_tokenId) != _owner) revert NotOwner("I721", _tokenId);
    if (!s_tokenSubscriptions[_tokenId][_projectId].isSubscribed) revert Unsubscribed("J721", _tokenId, _projectId);
    s_tokenSubscriptions[_tokenId][_projectId].uri = _updatedUri;
  }
  ...
  function transferTokenProjectURI(
    ...
    if (ownerOf(_tokenId) != _owner) revert NotOwner("I721", _tokenId);
    if (!s_tokenSubscriptions[_tokenId][_projectId].isSubscribed) revert Unsubscribed("J721", _tokenId, _projectId);
    s_tokenSubscriptions[_tokenId][_projectId].uri = _transferredUri;
  }
  ...
  function resetSlotToDefault(
    ...
    if (ownerOf(_tokenId) != _owner) revert NotOwner("I721", _tokenId);
    if (!s_tokenSubscriptions[_tokenId][_projectId].isSubscribed) revert Unsubscribed("J721", _tokenId, _projectId);
    s_tokenSubscriptions[_tokenId][_projectId].uri = _projectDefaultUri;
  }
   ...
   * @notice Returns the project URI for the given token ID
   *
   * @param _tokenId        The ID of the token whose project URI is to be returned
   * @param _projectId      The ID of the project associated with the given token
   *
   * @return           The URI of the given token's project slot
   */
  ...
  function getProtocolRoyalty() external view returns (uint256) {
    return ROYALTY_DIVISOR / s_mintRoyaltyInfo.royalty;
  }

  function getRoyaltyInfo() external view returns (MintRoyalty memory) {
    return s_mintRoyaltyInfo;
  }
  ...
  function _verifySignature(
    ...
    bytes32 m_messageHash = keccak256(abi.encode(_verifier, _projectId, _templateId, _collectionId, _uri, _salt));
  ...
  function _withdraw(bool isProtocolWithdraw) internal {
    ...
    _send(targetAddress, withdrawable);

    if (isProtocolWithdraw) {
      emit WithdrawProtocol(msg.sender, withdrawable, info.savedAmountOwner, info.totalWithdrawn);
    } else {
      _send(payable(msg.sender), royalty);
      emit Withdraw(msg.sender, withdrawable, info.savedAmountProtocol, info.totalWithdrawn);
    }
  }
