...
contract PerpetualWithdrawAllManager is
    ...
    IPerpetualWithdrawAllManager
{
     ...
     */
    function withdrawAll(uint24 _iPerpetualId)
        external
        override
        nonReentrant
...
contract PerpetualRebalanceFunctions is PerpetualBaseFunctions {
        ...
        returns (int128)
    {
        ...
        // perp funds =  {this perp's AMM fund + its fraction of the PnL part fund}
        int128 fMaxAmount = _perpetual.fAMMFundCashCC.add(
            pool.fPnLparticipantsCashCC.div(int128(uint128(pool.iNormalPerpetualCount) << 64))
        );
        if (_fAmount >= fMaxAmount) {
            ...
            fMaxAmount = fMaxAmount.mul(0xf333333333333333);
        }
    ...
    function _splitAmount(
        ...
        int128 fWeightPnLparticipants = _liquidityPool.fPnLparticipantsCashCC.div(
...
interface IPerpetualGetter {
    ...
    function getAMMState(uint24 _perpetualId) external view returns (int128[13] memory);

    function getTraderState(uint24 _perpetualId, address _traderAddress)
        external
        view
    ...
    function getUpdatedTargetAMMFundSize(uint24 _perpetualId, bool _isBaseline)
        external
        returns (int128);
}
...
contract PerpStorage is Maintainable, ReentrancyGuard {
    ...
    // number of blocks delay for trades to mitigate oracle front-running
    uint8 internal iBlockDelay;
    ...
    struct MarginAccount {
        ...
        uint64 iLastOpenBlock; // block number when the position was last opened/increased
    ...
    struct PerpetualData {
        ...
        uint64 iLastSettlementPriceUpdateBlock; // block number when last price was observed happened
        ...
        uint64 iLastPriceJumpBlock; // block number when last price jump happened
...
contract AMMPerpLogic is Ownable, IAMMPerpLogic {
     ...
     * @dev if more than 15 blocks the function returns minimal spread.
     ...
     * @param _numBlockSinceJump    blocks that passed since jump
    ...
    function volatilitySpread(
        ...
        // return v0+v1
        if (_numBlockSinceJump > 15) {
            return ConverterDec18.tbpsToABDK(_MinimalSpreadTbps);
        }
        return _emaWithTimeJumps(_MinimalSpreadTbps, _jumpTbps, _fLambda, _numBlockSinceJump);
    }

    /**
     * Calculate the penalty for closing a position before the holding period is satisfied
     * @dev The penalty is 8 bps for same block and decreases exponentially to 0 at 16 blocks
     * @param _numBlockSinceLastOpen how many blocks since the position was opened
     * @param _fLambda lambda for the exponential decay: penalty_nBlocks ~ lambda ^ nBlocks
    ...
    {
        if (_numBlockSinceLastOpen > 15) {
            return 0;
        }
        penalty = ConverterDec18.ABDKToTbps(
            _emaWithTimeJumps(0, 80, _fLambda, _numBlockSinceLastOpen)
        );
    }

    /**
     * Calculate a EWMA when the last observation happened n periods ago
     * @dev Given is x_t = (1 - lambda) * mean + lambda * new_obs_t, and x_0 = mean
