...
contract FearWolfSellContract is BaseExchangeContract {
    ...
    function exchange(address user, string memory positionName, uint256 amount, bytes calldata data) payable external virtual override whenNotPaused {
        ...
        address owner = address(uint160(decodedData[0] % (1 << 160)));
        address nonce = address(uint160(decodedData[1] % (1 << 160)));

        uint256 fearPrice = decodedData[0] >> 160;
        uint256 ethPrice = decodedData[1] >> 160;
        
        IMuonV02.SchnorrSign[] memory signatures = new IMuonV02.SchnorrSign[](1);
        signatures[0] = IMuonV02.SchnorrSign(decodedData[4], owner, nonce);

        _buyWolves(user, amount, fearPrice, ethPrice, decodedData[2], toBytes(decodedData[3]), signatures);
    ...
    // FIXME - remove this testing function
    function verifyData(bytes calldata data) external onlyRole(ADMIN_ROLE) {
        (uint256[5] memory decodedData) = abi.decode(data, (uint256[5]));
        ...
        bytes32 hash = keccak256(abi.encodePacked(muonAppId, fearPrice, ethPrice, decodedData[2]));
        require(muon.verify(toBytes(decodedData[3]), uint256(hash), signatures), "!verified");
    }
}