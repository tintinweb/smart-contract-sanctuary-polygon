...
contract AuctionOrderBook is Ownable {
    ...
    function bid(uint256 _artworkId, uint256 _bidAmount) external payable {
        ...
        for (
            uint256 version = auctions.currentAuction;
            version < SUPPLY[_artworkId] &&
                block.timestamp > auctions.auctions[version].endAt;
            ++version
        ) {
        ...
        Auction memory auction = auctions.auctions[auctions.currentAuction];
        ...
        auctions.auctions[auctions.currentAuction] = auction;
    ...
    function withdrawProfit() external onlyOwner {
        Auctions[ARTWORK_COUNT] storage artworkToAuctions_ = artworkToAuctions;

        for (uint8 artworkId; artworkId < ARTWORK_COUNT; ++artworkId) {
            Auctions storage auctions = artworkToAuctions_[artworkId];
        ...
        profitAmount = 0;
        payable(msg.sender).transfer(availableProfit);
    }
    ...
    function currentAuctionOf(
        ...
        uint8 version = auctions.currentAuction;

        require(version < SUPPLY[_artworkId], "No auction for this artwork");

        Auction memory auction = auctions.auctions[version];
