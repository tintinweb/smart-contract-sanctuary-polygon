 ...
 *30307188
 */
contract BarrensNoms is ERC20, Ownable, Authorizable {
    ...
    // supply, and it can't be less than 0.05% of the supply.
   ...
   // event MaxTransferAmountRateUpdated(uint256 previousRate, uint256 newRate);
    ...
    //modifier antiWhale(address sender, address recipient, uint256 amount) {
        //if (maxTransferAmount() > 0) { //Lol remember this trick, if maxTransferAmount() == 0 you can skip antiWhale
         //   if (
    ...
    constructor (
      ...
      uint256 _lockToBlock
        ...
        lockFromBlock = _lockFromBlock; //29916260 last cur block when me lookn
        lockToBlock = _lockToBlock;//31125860 POLYTEST + one year
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "NomsToken::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "NomsToken::delegateBySig: invalid nonce"
        );
        require(block.timestamp <= expiry, "NomsToken::delegateBySig: signature expired");
        ...
        returns (uint256)
    {
        require(
            ...
            "NomsToken::getPriorVotes: not yet determined"
        );
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber =
            safe32(
                block.number,
                "NomsToken::_writeCheckpoint: block number exceeds 32 bits"
            );
...
import "@openzeppelin/contracts/access/Ownable.sol";//utils/Context.sol
