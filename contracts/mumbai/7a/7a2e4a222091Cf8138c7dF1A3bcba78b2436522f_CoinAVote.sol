...
//interval
import './CoinABase.sol';
import './utils/ICapsuleContract.sol';
import './utils/Imisc.sol';
...
contract CoinAVote is CoinADAO {
    ...
    function submitNormalProposal(
        ...
        NormalProposal memory _Proposal = NormalProposal({proposer: msg.sender, isAnonymous: isAnonymous, isPublic: isPublic, endAt: interval + block.timestamp, options: options});
    ...
    function gainNormalProposal(uint256 id)
        external
        view
        returns (
            address,
    ...
    function submitImportantProposal(
        address sc,
        uint256 op,
        bool isAnonymous,
        bool isPublic,
    ...
    function executeProposal(uint256 id) external {
        ...
        if (op == 0) {
            ...
            ICapsuleContract sc = ICapsuleContract(_Proposal.sc);
            sc.writePriceInfo(_Proposal.param2);
        } else if (op == 7) {
            ...
            Imisc sc = Imisc(_Proposal.sc);
            sc.cancelAuctionWhenPaused(_Proposal.param1, _Proposal.param2);
        } else if (op == 13) {
    ...
    function gainImportantProposal(uint256 id)
        external
        view
        returns (
            address,
    ...
    function removeExpiredProposal() internal {
        ...
        for (uint256 i = 0; i < l; ) {
            if (ImportantProposals[i].endAt <= block.timestamp && !goalAchieved(i)) {
                ImportantProposal memory _Proposal = ImportantProposals[i];
                emit importantProposalEvent(
                    _Proposal.id,
                    _Proposal.proposer,
                    _Proposal.sc,
                    _Proposal.op,
                    _Proposal.endAt,
                    _Proposal.isAnonymous,
                    _Proposal.isPublic,
                    _Proposal.param1,
                    _Proposal.param2,
                    false
...
contract CoinABase is ERC20Implementation, ISpawnCoin {
    ...
    function setStartTime(uint256 _uint) external onlyAdmin {
    ...
    function setSpawnContractAddress(address addr)
        external
        override
        onlySuperAdmin
    {
        require(addr != address(0), INVALID_ADDRESS);
        emit SpawnContractAddressChanged(
            SpawnContractAddress,
            addr,
            block.timestamp
        );
...
interface ICapsuleContract {
    ...
    function createCapsule(address caller, bool triple)
        external
...
interface Imisc {
    function changeCapsuleContract(address addr) external;
...
interface ISpawnCoin {
    event SpawnContractAddressChanged(
