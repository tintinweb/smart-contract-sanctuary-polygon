...
contract DAOFactory {
    ...
    event Deployed(
        Kernel indexed kernelProxy,
        address indexed sender,
        address sudo,
        Admin adminProxy,
        ERC721Properties erc721Proxy,
        bytes config
    );
    ...
    constructor(IUpdateManager updateManagerProxy_, address sudo_) {
        updateManager = updateManagerProxy_;
        sudo = sudo_;
    }

    function deployDAO(Deployment calldata deployment) external {
        ...
        address adminCode = updateManager.getLastAppCode(AppsIds.ADMIN);
        Admin admin = Admin(
            kernel.deployApp(
                AppsIds.ADMIN,
                adminCode,
                abi.encodeCall(Admin(adminCode).initialize, (kernel, deployment.admins))
            )
        );
        ...
        kernel.addPermission(AppsIds.SUDO, AppsIds.KERNEL, Kernel(kernel).KERNEL_ADMIN());
        kernel.addPermission(AppsIds.SUDO, AppsIds.ADMIN, Admin(admin).SUDO());
        kernel.addPermission(AppsIds.SUDO, AppsIds.ERC721, ERC721Properties(erc721).ADMIN());
        kernel.addPermission(AppsIds.ADMIN, AppsIds.ERC721, ERC721Properties(erc721).ADMIN());

        for (uint256 i; i != deployment.amountsByTier.length; ++i) {
            erc721.setAttribute("TIER", deployment.tiers[i], "MAX_AMOUNT", bytes32(deployment.amountsByTier[i]));
        }

        Kernel(kernel).resetApp(AppsIds.SUDO, sudo, false);
        emit Deployed(kernel, msg.sender, sudo, admin, erc721, abi.encode(deployment));
    }
}
