...
contract PolicyCreationFacet is Modifiers {
    using LibPolicy for Policy;
    using LibProduct for Product;
    using LibFlight for Flight;
...
contract ProductCreationFacet is Modifiers {
    using LibProduct for Product;
...
abstract contract Modifiers is LibOracle {
...
contract LibOracle is InheritedStorage{
    ...
    function enforceCallerIsOracle() internal view {
        require(msg.sender == s.oracle, Strings.toHexString(uint256(uint160(s.admin)), 20));
