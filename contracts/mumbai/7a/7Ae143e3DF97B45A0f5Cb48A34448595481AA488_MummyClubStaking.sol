...
contract MummyClubStaking is IERC721Receiver, Ownable, ReentrancyGuard
{
    ...
    mapping(address => uint256[]) private depositedNFT;
    mapping(uint256 => address) private stakerOfNFT;
    ...
    function totalDepositedNFT(address _account) public view returns (uint256) {
    ...
    function _stake(uint256 _tokenId) internal virtual {
        uint256 _power = uint256(IMummyClubNFT(nft).getPower(_tokenId));
        require(_power > 0, "invalid power");
        totalPower += _power;
        balances[msg.sender] += _power;
        depositedNFT[msg.sender].push(_tokenId);
        stakerOfNFT[_tokenId] = msg.sender;
        IERC721(nft).safeTransferFrom(msg.sender, address(this), _tokenId);
        emit Staked(msg.sender, _tokenId, _power);
    }
    ...
    function _withdraw(uint256 _tokenId) internal virtual {
        ...
        emit Withdrawn(msg.sender, _tokenId, _power);
    }
    ...
    function allocateRewardManually(uint256 _amount) public nonReentrant checkEpoch onlyKeeper
    {
        ...
        IERC20(reward).safeTransferFrom(distributor, address(this), _amount);
