 ...
 */
contract HorrorHubHost is AccessControl {
    ...
    mapping (string => GameInfo) public gamesById;
    mapping (address => mapping (string => bool)) userInstallations;
    ...
    function getUserGames(address user) external view returns(string[] memory) {
        uint256 userGamesCount;
        for (uint i = 0; i < games.length; i++) {
            if (userInstallations[user][games[i].id])
                userGamesCount++;
        }

        string[] memory userGames = new string[](userGamesCount);
        userGamesCount = 0;

        for (uint i = 0; i < games.length; i++) {
            if (userInstallations[user][games[i].id]) {
                userGames[userGamesCount] = games[i].id;
                userGamesCount++;
            }
        }
    ...
    function addGame(string memory id, string memory metadataUrl) external onlyRole(MANAGER_ROLE) {
        require(bytes(gamesById[id].id).length == 0, "Game with this name already exists");

        GameInfo storage newGame = games.push();
        newGame.id = id;
        newGame.metadataUrl = metadataUrl;
        gamesById[id] = newGame;
        
    }

    function editGame(string memory id, string memory metadataUrl) external onlyRole(MANAGER_ROLE) {
        require(bytes(gamesById[id].id).length > 0, "Game not found");

        GameInfo storage existingGame = gamesById[id];
        existingGame.metadataUrl = metadataUrl;
    }

    function removeGame(string memory id) public onlyRole(MANAGER_ROLE) {
        require(bytes(gamesById[id].id).length > 0, "Game not found");

        uint gameIndex;
        for (uint i = 0; i < games.length; i++) {
            if (keccak256(bytes(games[i].id)) == keccak256(bytes(id)))
                gameIndex = i;
        }

        for (uint i = gameIndex; i < games.length - 1; i++) {
            games[i] = games[i + 1];
        }
    ...
    function registerInstallation(address user, string memory gameName) external onlyRole(MANAGER_ROLE) {
        userInstallations[user][gameName] = true;
    }

    function registerUninstallation(address user, string memory gameName) external onlyRole(MANAGER_ROLE) {
        userInstallations[user][gameName] = false;
    }
}
