...
contract TrackedTokenFacet is Modifiers {
    ...
    */
    ...
    /**
    @notice get tracked tokens and real amounts per provided amount of portfolio token
            with adjustment for not yet minted fees
    @param _addr portfolio token address
    @param _amount amount of porfolio token
    @param _timestamp timestamp to be used for streaming fee calculation, block.timestamp will be used if 0 provided
    @return TrackedTokenList struct with tracked token data
     */
    function getAdjustedRealAmount(address _addr, uint256 _amount, uint256 _timestamp) public view returns (LibTrackedToken.TrackedTokensList memory) {
        return LibTrackedToken.getAdjustedRealAmount(_addr, _amount, _timestamp);
    }
