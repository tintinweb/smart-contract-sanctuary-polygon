...
contract HashUpAuction is HashUpBase {
    ...
    function cancelAuction(
        uint256 _auctionId
    ) external nonReentrant isRunningAuction(_auctionId) {
        Auction storage auction = auctions[_auctionId];
        ...
        isExistingBidder(_auctionId)
    {
        ...
        (bool success, ) = _msgSender().call{value: bidAmount}("");
        if (!success) revert TransferFailed(_msgSender(), bidAmount);
    }

    function claim(
        uint256 _auctionId
    ) external nonReentrant isExpiredAuction(_auctionId) {
        Auction storage auction = auctions[_auctionId];
        ...
        if (auction.creator == _msgSender()) {
            if (auction.bidders.length == 0) {
                ...
                if (!success) revert TransferFailed(_msgSender(), amount);
            }
            ...
            uint amount = auction.biddersToAmount[_msgSender()];
            (bool success, ) = _msgSender().call{value: amount}("");
            if (!success) revert TransferFailed(_msgSender(), amount);
        }
    }

    function getAuctionDetailsById(
        uint256 _auctionId
    )
        external
        view
