...
// File contracts/TradingCards.sol

// File: contracts/interfaces/IOMTradingCards.sol



pragma solidity ^0.8.11;

interface IOMTradingCards {
  /**
   *  @notice All info relevant to packs.
   *
   *  @param tokensPerPack           The amount of ERC721 tokens a user receives per pack.
   *  @param closed                  Whether the pack is closed or not.
   *  @param locked                  Whether the pack can be opened or not.
   *  @param uri                     The URI of the pack.
   *  @param minted                  The amount of packs minted.
   *  @param maxSupply               The maximum amount of packs that can be minted.
   *  @param maxPerWallet            The maximum amount of packs that can be minted per wallet.
   *  @param cost                    The cost to mint the pack.
   */
  struct Pack {
    uint256 tokensPerPack;
    bool closed;
    bool locked;
    string uri;
    uint256 minted;
  ...
  /// @notice Emitted when a pack is created.
  event PackCreated(uint256 indexed tokenId);

  /// @notice Emitted when a pack is updated.
  event PackUpdated(uint256 indexed tokenId);

  /// @notice Emitted when a pack is opened.
  event PackOpened(uint256 indexed tokenId, address indexed opener);

  /**
   *  @notice Creates a pack with the stated parameters.
   *
   *  @param _pack                 The pack to create.
   */
  function createPack(Pack calldata _pack) external;

  /**
   *  @notice Lets a pack owner open a pack and receive the pack contents.
   *
   *  @param _tokenId       The identifier of the pack to open.
   *  @param _count         The amount of the pack to open.
   */
  function openPack(uint256 _tokenId, uint256 _count) external;

  /**
   *  @notice Lets team members update a pack.
   *
   *  @param _tokenId       The identifier of the pack to open.
   *  @param _cost          The cost to mint the pack.
   *  @param _maxPerWallet  The maximum amount of packs that can be minted per wallet.
   *  @param _closed        Whether the pack is closed or not.
   *  @param _locked        Whether the pack can be opened or not.
   *  @param _uri           The URI of the pack.
   *
   */
  function updatePack(
    uint256 _tokenId,
  ...
  /**
   * @return The amount of tokens that have been created.
   */
  function getPackCount() external view returns (uint256);

  /**
   * @notice Lets team members send tokens to multiple recipients.
   *
   * @param _recipients The addresses of the recipients.
   * @param _tokenIds The identifiers of the tokens to send.
  ...
  /**
   * @notice Lets users mint tokens.
   *
   * @param _tokenIds The identifiers of the tokens to mint.
   * @param _amounts The amount of tokens to mint.
   *
   */
  function mint(uint256[] memory _tokenIds, uint256[] memory _amounts)
    external
    payable;
  ...
  /**
   * @notice Returns a list of balances for each token by address.
   *
  ...
  /**
   * @notice Returns the pack with the given identifier.
   *
   * @param _tokenId The identifier of the token.
   *
   * @return pack The pack.
   */
  function getPack(uint256 _tokenId) external view returns (Pack memory pack);

  /**
   * @notice Returns all packs.
   *
   * @return packs The packs.
   */
  function getPacks() external view returns (Pack[] memory packs);
}

// File: contracts/interfaces/INFTArtGen.sol
...
// File: contracts/OMTradingCards.sol
...
contract OMTradingCards is
  ...
  IOMTradingCards
{
  /// @dev mapping from address to tokenId to amount minted
  mapping(address => mapping(uint256 => uint256)) private _minted;
  /// @dev mapping from tokenId to Pack
  mapping(uint256 => Pack) private _packs;
  /// @dev next tokenId to be used
  uint256 internal _nextTokenIndex;
  /// @dev address of the ERC721 contract of pack contents
  address public immutable erc721Address;

  // replace __OMTradingCards_init with constructor
  constructor(address _erc721Address, uint256 _commission) ERC1155("") {
  ...
  function mint(uint256[] memory _tokenIds, uint256[] memory _amounts)
    external
    payable
    _beforeMint(_tokenIds, _amounts)
  {
    _mintBatch(msg.sender, _tokenIds, _amounts, "");
  }

  function createPack(Pack calldata _pack) external onlyTeamOrOwner {
    require(_pack.tokensPerPack > 0, "TPP");
    require(_pack.minted == 0, "M");
    require(_pack.maxSupply > 0, "MS");
    require(_pack.tokensPerPack > 0, "TPP");
    _packs[_nextTokenIndex] = _pack;
  ...
  function updatePack(
    ...
    _packs[_tokenId].cost = _cost;
    _packs[_tokenId].maxPerWallet = _maxPerWallet;
    _packs[_tokenId].closed = _closed;
    _packs[_tokenId].locked = _locked;
    _packs[_tokenId].uri = _uri;
    emit PackUpdated(_tokenId);
  }

  function openPack(uint256 _tokenId, uint256 _count) external {
    require(msg.sender == tx.origin, "EOA");
    require(_count > 0, "Count");
    Pack storage pack = _packs[_tokenId];
    require(!pack.locked, "Locked");

    _burn(msg.sender, _tokenId, _count);
    address[] memory _recipients = new address[](1);
    _recipients[0] = msg.sender;
    uint256[] memory _amounts = new uint256[](1);
    _amounts[0] = pack.tokensPerPack * _count;
    INFTArtGen erc721 = INFTArtGen(erc721Address);
    erc721.airdrop(_recipients, _amounts);
    emit PackOpened(_tokenId, msg.sender);
  }
  ...
  function uri(uint256 _tokenId)
    public
    view
    override(ERC1155, IOMTradingCards)
  ...
  function supportsInterface(bytes4 interfaceId)
    public
    view
    override(ERC1155Receiver, ERC1155, IERC165)
    returns (bool)
  {
    return
  ...
  function tokensOfOwner(address _address)
    ...
    returns (uint256[] memory)
  {
    require(_exists(0), "DNE");
    uint256[] memory balances = new uint256[](_nextTokenIndex);
  ...
  function getPack(uint256 _tokenId) external view returns (Pack memory pack) {
    require(_exists(_tokenId), "DNE");
    pack = _packs[_tokenId];
  }

  function getPacks() external view returns (Pack[] memory packs) {
    packs = new Pack[](_nextTokenIndex);
    for (uint256 i = 0; i < _nextTokenIndex; i++) {
      packs[i] = _packs[i];
    }
  }
  ...
  function _beforeTokenTransfer(
    ...
    for (uint256 i = 0; i < ids.length; i++) {
      if (from == address(0)) {
        ...
        require(_minted[to][ids[i]] <= _packs[ids[i]].maxPerWallet, "MPW");
        _packs[ids[i]].minted += amounts[i];
      }
    }
  ...
  modifier _beforeMint(uint256[] memory _tokenIds, uint256[] memory _amounts) {
    require(_tokenIds.length == _amounts.length, "L");
    uint256 totalCost = 0;
    for (uint256 i = 0; i < _tokenIds.length; i++) {
      require(_exists(_tokenIds[i]), "DNE");
      require(!_packs[_tokenIds[i]].closed, "Closed");
      require(
        _packs[_tokenIds[i]].minted + _amounts[i] <=
          _packs[_tokenIds[i]].maxSupply,
        "MS"
      );
      require(
        _minted[msg.sender][_tokenIds[i]] + _amounts[i] <=
          _packs[_tokenIds[i]].maxPerWallet,
        "MPW"
      );
      totalCost += _packs[_tokenIds[i]].cost * _amounts[i];
    }
...
contract TradingCards is OMTradingCards {
  constructor(
    address _erc721Address,
    uint256 _commission
    ) OMTradingCards(_erc721Address, _commission) {}
  }