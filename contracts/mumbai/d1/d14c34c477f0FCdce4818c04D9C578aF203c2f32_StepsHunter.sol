...
import '@openzeppelin/contracts/utils/Strings.sol';
    ...
    // test
    // 0x
    // 0xDcB19e1FcF76e475C27C7a2aF8BD0FA1054e70b0
    // 500000
    // 0x46E40347CE742Bc0ab8c515025b6C81752220C6F
...
struct Goal {
    ...
    uint startDateTimestamp; // remove
    uint timeZoneOffset;
...
contract StepsHunter is ChainlinkClient, ConfirmedOwner, AutomationCompatibleInterface {
    ...
    event GoalCreated(uint indexed goalId, uint indexed goalUpkeepID, GoalParams goalParams);
    ...
    function createGoal(
        ...
        goal.lostBetRecipientAddr = 0x46E40347CE742Bc0ab8c515025b6C81752220C6F; // stepsHunter admin account
        goal.userAddr = msg.sender;
        goal.upkeepID = upkeepID;

        emit GoalCreated(goal.id, goal.upkeepID, goalParams);
    }
    ...
    function addGoalDayRecord(
        ...
        if (goal.dayRecordsCount == goal.durationDaysCount && goal.failDaysCount <= goal.maxFailDaysCount) {
            ...
            // payable(goal.userAddr).transfer(goal.betAmount);
        ...
        if (goal.dayRecordsCount == goal.durationDaysCount && goal.failDaysCount > goal.maxFailDaysCount) {
            ...
            // payable(goal.lostBetRecipientAddr).transfer(goal.betAmount);

            (bool isSent, bytes memory data) = payable(goal.lostBetRecipientAddr).call{value: goal.betAmount}("");
            require(isSent, "Failed to send Ether");
    ...
    function checkUpkeep(
        ...
        return (upkeepNeeded1, checkData);
    }
