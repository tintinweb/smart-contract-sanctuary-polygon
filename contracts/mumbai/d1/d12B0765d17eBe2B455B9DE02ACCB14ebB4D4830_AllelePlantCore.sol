...
contract AllelePlantCore is PlantBreeding {
    ...
    function withdrawAgt(uint256 _amount) external onlyOwner {
...
contract PlantBreeding is UIAMToken {
    ...
    function breedWithAuto(
        uint256 _matronId, uint256 _sireId, string memory _tokenURI
    ...
    function _giveBirth(uint256 _matronId, uint256 _sireId, string memory _tokenURI)
        internal
        whenNotPaused
        returns (uint256)
    {
        ...
        uint collectionId = getCollectionId(_matronId);
        setCollectionWithoutOwner(collectionId, plantId);

        emit Born(plantId, collectionId);
...
contract UIAMToken is AlleleAnalysis {
    ...
    function mintUIAM(
        ...
        _mintUIAM( _tokenURI, msg.sender, _collectionId);
    }
    ...
    function _mintUIAM(string memory _tokenURI, address _signer, uint _collectionId) internal {
        uint256 tokenId = _createPlant(0, 0,  _signer, _tokenURI);
        setCollection(_collectionId, tokenId);
        emit Born(tokenId, _collectionId);
    }

    function _mintChildUIAM(uint256 _parentId, string memory _tokenURI) internal {
        uint256 tokenId = _createPlant(_parentId, 0,  ownerOf(_parentId), _tokenURI);
        uint parentCollection = getCollectionId(_parentId);
        setCollectionWithoutOwner(parentCollection, tokenId);
        emit Born(tokenId, parentCollection);
    }
...
contract AlleleAnalysis is AllelePlantOwnership {
    ...
    function getReports(uint256 _tokenId) public view returns(Analysis[] memory) {
        return tokenToReports[_tokenId];
    }

    function publishReport(uint256 _tokenId, string memory _reportURI) public {
        require(_owns(msg.sender, _tokenId) || _hasPhenoHuntingPermission(_tokenId,msg.sender), "You are not owner nor phenoHunter");
...
contract AllelePlantOwnership is AllelePlantBase {
    ...
    event GrowingPermission(uint256 indexed tokenId, address _addr);
    event PhenoHuntingPermission(uint256 indexed tokenId, address _addr);
    event MusicTracksPermission(uint256 indexed tokenId, address _addr);

    // Mappings
    mapping(uint256 => EnumerableSet.AddressSet) private breedingPermissionSet;
    mapping(uint256 => EnumerableSet.AddressSet) private growingRightsSet;
    mapping(uint256 => EnumerableSet.AddressSet) private phenoHuntingRights;
    mapping(uint256 => EnumerableSet.AddressSet) private musicTracksRights;
   ...
   //-------------------------------------------------------------//
    ...
    function _owns(address _user, uint256 _tokenId) internal view returns(bool) {
        return ownerOf(_tokenId) == _user;
    }

    function owns(address _user, uint256 _tokenId) external view returns(bool) {
   ...
   //-------------------------------------------------------------//
    ...
    function verifyBreedingPermission(uint256 _tokenId, address _user) external view returns(bool) {
        return _hasBreedingPermissions(_user, _tokenId);
    }

    function getBreedingRights(uint256 _tokenId) external view returns(address[] memory) {
   ...
   function _addGrowingPermission(uint256 _tokenId, address _user) internal {
        growingRightsSet[_tokenId].add(_user);
        emit GrowingPermission(_tokenId, _user);
   }

   function addGrowingPermission(uint256 _tokenId, address _user) external {
        require(
   ...
   function _hasGrowingPermission(uint256 _tokenId, address _user) internal view returns (bool){
        return growingRightsSet[_tokenId].contains(_user);
   }

   function getGrowingPermission(uint256 _tokenId) external view returns(address[] memory) {
        return growingRightsSet[_tokenId].values();
   } 

   function verifyGrowingPermission(uint256 _tokenId, address _user) external view returns(bool) {
   ...
   function _addPhenoHuntingPermission(uint256 _tokenId, address _user) internal {
        phenoHuntingRights[_tokenId].add(_user);
        emit PhenoHuntingPermission(_tokenId, _user);
   }

   function addPhenoHuntingPermission(uint256 _tokenId, address _user) external {
        require(
   ...
   function _hasPhenoHuntingPermission(uint256 _tokenId, address _user) internal view returns (bool){
        return phenoHuntingRights[_tokenId].contains(_user);
   }

   function getPhenoHuntingPermission(uint256 _tokenId) external view returns(address[] memory) {
        return phenoHuntingRights[_tokenId].values();
   }

   function verifyPhenoHuntingPermission(uint256 _tokenId, address _user) external view returns(bool) {
   ...
   function _addMusicTracksPermission(uint256 _tokenId, address _user) internal {
        musicTracksRights[_tokenId].add(_user);
        emit MusicTracksPermission(_tokenId, _user);
   }

   function addMusicTracksPermission(uint256 _tokenId, address _user) external {
        require(
   ...
   function _hasMusicTracksPermission(uint256 _tokenId, address _user) internal view returns (bool){
        return musicTracksRights[_tokenId].contains(_user);
   }

   function getMusicTracksPermission(uint256 _tokenId) external view returns(address[] memory) {
        return musicTracksRights[_tokenId].values();
   }

   function verifyMusicTracksPermission(uint256 _tokenId, address _user) external view returns(bool) {
   ...
   function transferFrom(
        ...
        addBreedingPermission(uint256(_tokenId), _from);
    ...
    AlleleCollections
{
    ...
    event Born(uint256 indexed plantId, uint256 indexed collectionId);
...
contract AlleleCollections {

    using EnumerableSet for EnumerableSet.AddressSet;

    event CollectionCreated(uint indexed collectionId, address indexed owner ,string metadataURI );

    event TokenAddedToCollection(uint indexed collectionId, uint indexed tokenId);

    mapping(uint => uint) private tokenToCollectionId;

    mapping(uint => string) private metadataToCollection;

    mapping(uint => address) private ownerToCollection;

    mapping(uint => EnumerableSet.AddressSet) private moderatorToCollectionSet;

    uint private totalCollections;

    function createCollection(string memory _contractURI) external returns(uint){
        totalCollections++;
        metadataToCollection[totalCollections] = _contractURI;
        ownerToCollection[totalCollections] = msg.sender;
        emit CollectionCreated(totalCollections, msg.sender, _contractURI);
        return totalCollections;
    }

    function setCollection(uint _collectionId, uint _tokenId) public {
        require(_collectionId <= totalCollections, "Collection ID not exists");
        require(ownerToCollection[_collectionId] == msg.sender || moderatorToCollectionSet[_collectionId].contains(msg.sender), "You are not the owner or moderator of this collection");
        tokenToCollectionId[_tokenId] = _collectionId;
        emit TokenAddedToCollection(_collectionId, _tokenId);
    }

    function setCollectionWithoutOwner(uint _collectionId, uint _tokenId) internal {
        require(_collectionId <= totalCollections, "Collection ID not exists");
        tokenToCollectionId[_tokenId] = _collectionId;
        emit TokenAddedToCollection(_collectionId, _tokenId);
    }

    function setModerator(uint _collectionId, address _moderator) internal {
        require(_collectionId <= totalCollections, "Collection ID not exists");
        moderatorToCollectionSet[_collectionId].add(_moderator);
    }

    function setModeratorByOwner(uint _collectionId, address _moderator) external {
        require(ownerToCollection[_collectionId] == msg.sender, "You are not the owner of this collection");
        setModerator(_collectionId, _moderator);
    }

    function getCollectionId(uint _tokenId) public view returns(uint) {
        return tokenToCollectionId[_tokenId];
    }

    function contractURI(uint _collectionId) public view returns(string memory) {
        return metadataToCollection[_collectionId];
    }

    function getModerators(uint _collectionId) public view returns(address[] memory) {
        return moderatorToCollectionSet[_collectionId].values();
    }
