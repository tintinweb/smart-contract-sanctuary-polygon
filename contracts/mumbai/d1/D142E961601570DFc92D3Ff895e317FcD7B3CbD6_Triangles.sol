...
// File: contracts\Triangles.sol


pragma solidity ^0.8.4;
contract Triangles is ERC721, ERC721URIStorage, Ownable {
    using Counters for Counters.Counter;

    Counters.Counter public _tokenIdCounter;

    constructor() ERC721("Triangles", "Triangles") {
        _tokenIdCounter.increment();
    }


    function safeMint20() public onlyOwner {
        for (uint i = 0; i < 20; i++) {
            uint256 tokenId = _tokenIdCounter.current();
            _tokenIdCounter.increment();
            _safeMint(msg.sender, tokenId);
            _setTokenURI(tokenId, string(abi.encodePacked("ipfs://Qme6vaq8DNcNZjUxmFsWoA7cLhB77Mzu8mDVshVpFnVx5H/", Strings.toString(tokenId), ".json")));
        }
    }
