...
// File: contracts/dotWAGMIprotocol.sol
...
contract dotWAGMI is ERC721, ERC721URIStorage, ERC721Burnable, Ownable {
    ...
    mapping (uint => userWallets) internal DIdToUserWallets;
    mapping (string => bool) internal userNameTaken;
    mapping (address => bool) public whitelist;
    mapping (bytes32 => bool) private usedNonces;

    function whitelistUsers(address[] memory toBeWl) public onlyOwner {
        for (uint i=0; i < toBeWl.length; i++) {
            whitelist[toBeWl[i]] = true;
        }
    }
    ...
    constructor() ERC721("dotWAGMI", "WAGMI") {}


    //Mint contyrollers
    function isValidUsernameCharacter(bytes1 char) internal  pure returns (bool) {
    ...
    function setServerKey(address newSerPub) public onlyOwner  {
    ...
    function setDId(address toSet, string memory usernameToSet, uint tokenIdInvolved) internal {
        if (addressToDId[toSet] == 0) {
                UID++;
                DIdToUserWallets[UID].defaultAddress = toSet;
                DIdToUserWallets[UID].linkedAddresses = [toSet];
                addressToDId[toSet] = UID;
                usernameToDId[usernameToSet] = UID;
            }
            else {
                usernameToDId[usernameToSet] = addressToDId[toSet];
                for (uint8 i=0; i< DIdToUserWallets[addressToDId[toSet]].linkedAddresses.length; i++) {
                    addressToTokenId[DIdToUserWallets[addressToDId[toSet]].linkedAddresses[i]] = tokenIdInvolved; 
                }
            }
    }
    function mint(string memory _username, string memory uri, bytes memory signature, bytes32 messageHash) public payable {
        require(isAuthorized(messageHash, signature), "Unauthorized mint request");
        // nonce++;
        require(addressToTokenId[msg.sender] == 0, "Already Minted");
        // require(addressToDId[msg.sender] == 0, "You have minted once and sold it");
        uint8 userLength =  uint8(bytes(_username).length);
        require(userLength > 2, "Too small...That's what she said too");
        require(isValidUsername(_username), "Username contains invalid characters");
        uint mintTokenFee;
        ...
        if (whitelist[msg.sender] == true) {
            ...
            mintTokenFee = (7 - userLength) *  mintFee;
        }
        require(msg.value >= mintTokenFee, "Insufficient payment for selected username");
        string memory fullUsername = string(abi.encodePacked(_username, ".wagmi"));
        require(!userNameTaken[fullUsername] , "Username already taken");
        tokenIdCount++;
        _safeMint(msg.sender, tokenIdCount);
        usedNonces[messageHash] = true;
        userNameTaken[fullUsername] = true;
        _setTokenURI(tokenIdCount, uri);
        tokenIdToUsername[tokenIdCount] = fullUsername;
        setDId(msg.sender, fullUsername, tokenIdCount);
        addressToTokenId[msg.sender] = tokenIdCount;
        addressToDId[msg.sender] = UID;
        if (msg.value > mintTokenFee) {
            payable(msg.sender).transfer(msg.value - mintTokenFee);
        }
    }
    ...
    function addLinked(address[] memory linkedAdd, bytes memory sigHash, bytes32 mesHash) public {
        ...
        DIdToUserWallets[addressToDId[msg.sender]].linkedAddresses = linkedAdd;
    }
    ...
    function setDefault(address toSetDefault) public {
        ...
        DIdToUserWallets[usernameToDId[ tokenIdToUsername[addressToTokenId[msg.sender]]]].defaultAddress = toSetDefault;
    }
    ...
    function checkDefault(string memory userToCheck) public view returns (userWallets memory) {
        return DIdToUserWallets[usernameToDId[userToCheck]];
    }
    ...
    // function nonceView() public view returns (uint32) {
    //     return nonce;
    // }
    ...
    function transferFrom(
        ...
        if (addressToTokenId[from] == addressToTokenId[to]) {
            super.transferFrom(from, to, tokenId);
            emit internalTransfer(from, to, tokenId);
        } else if (addressToTokenId[to] ==0) {
            super.transferFrom(from, to, tokenId);
            addressToTokenId[from] = 0;
            string memory username = tokenIdToUsername[tokenId];
            emit externalTransfer(from,to, tokenId, username);
            address[] memory prevLinked = DIdToUserWallets[addressToDId[msg.sender]].linkedAddresses;
            for (uint i=0; i < prevLinked.length; i++) {
            addressToTokenId[prevLinked[i]] = 0;
            }
            addressToTokenId[to] = tokenId;
            setDId(to, username, tokenId);
        }
        else {
            revert("Error");
        }
