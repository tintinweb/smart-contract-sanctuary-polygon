...
contract JungleRelayer is OwnableUpgradeable, ERC1155ReceiverUpgradeable, IERC721ReceiverUpgradeable, UUPSUpgradeable, NativeMetaTransaction {
    ...
    function initialize(address _initialContract, address _feeRecipient, address _defaultFeeController, uint16 _resaleFee, uint16 _cashback, bytes4 _functionSig) external initializer{
        __Ownable_init();
        _initializeEIP712();
     ...
     */
    function setVerified(address _contractAddress, bytes4 _functionSig, bool _status) external onlyFeeController {
        require(_contractAddress != address(0), "Contract Address cannot be a zero address");
        require(isVerified[_contractAddress][_functionSig] != _status, "Already has the same verification status");
