...
contract AssetRegistry is HubOwnable, UUPSUpgradeable {
    ...
    constructor(address _dremHub, address _priceAggregator) HubOwnable(_dremHub) {
        if (_priceAggregator == address(0)) revert Errors.ZeroAddress();
     ...
     */
    function addWhitelistedAssets(address[] calldata _assets) external onlyHubOwner {
    ...
    function isAssetDenominationAsset(address _asset) external view returns (bool) {
        return _isAssetWhitelisted(_asset) && PRICE_AGGREGATOR.isAssetSupported(_asset)
            && denominationAssets.contains(_asset);
    }
     ...
     */
    function _validateArray(address[] calldata _array) internal pure {
    ...
    /**
     * @dev overridden authorize upgrade... (required by OZ)
     */
