...
contract Marketplace is ReentrancyGuard, Ownable{
    ...
    mapping(address => MarketStat) private marketLog;
    mapping(address => UserOffer[]) private usrCoinOffer;
    mapping(address => UserOffer[]) private usrTokenOffer;
    ...
    function sellNFTItem(
        ...
        if (marketLog[nftContractAddress].nftId.length == 0) {
            uint256[] memory tokenIdArray = new uint256[](1);
            tokenIdArray[0] = tokenId;
            marketLog[nftContractAddress] = MarketStat(
                tokenIdArray, 
                1
            );
        }
            ...
            uint256[] storage nftIds = marketLog[nftContractAddress].nftId;
            ...
            if (!alreadyListed) {
                nftIds.push(tokenId); // This is a new NFT listing
                marketLog[nftContractAddress].totalListed += 1;
            }
        }
    ...
    function getMarketStats(address _nftAddress) public view returns (uint256[] memory, uint256) {
        MarketStat memory stats = marketLog[_nftAddress];
        return (stats.nftId, stats.totalListed);
    }
    ...
    function fetchSellItemByAddress(address _seller) public view returns (MarketItem[] memory) {
        ...
        uint256 currentIndex = 0;
        for (uint256 i = 1; i <= itemsCount; i++) { // start loop from 1 instead of 0
            MarketItem memory item = marketItemIdToMarketItem[i];
        ...
        // Resize the marketItems array to exclude any unused elements
        assembly { marketItems := add(marketItems, 32) }
        assembly { mstore(marketItems, currentIndex) }
        return marketItems;
    }

    function getAllTokenOffers(uint256 marketItemId) public view returns (Offer[] memory) {
        return _offersToken[marketItemId];
    }

    function getAllCoinOffers(uint256 marketItemId) public view returns (Offer[] memory) {
        return _offersCoin[marketItemId];
    }

    function getUserOffersList(address user) public view returns (uint256[] memory coinOffers, uint256[] memory tokenOffers) {
        uint256 coinOfferCount = usrCoinOffer[user].length;
        uint256 tokenOfferCount = usrTokenOffer[user].length;
        coinOffers = new uint256[](coinOfferCount);
        tokenOffers = new uint256[](tokenOfferCount);
        for (uint256 i = 0; i < coinOfferCount; i++) {
            coinOffers[i] = usrCoinOffer[user][i].marketId;
        }
        for (uint256 i = 0; i < tokenOfferCount; i++) {
            tokenOffers[i] = usrTokenOffer[user][i].marketId;
        }
    }
