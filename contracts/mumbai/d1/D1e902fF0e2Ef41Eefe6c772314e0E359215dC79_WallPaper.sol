...
contract WallPaper is ERC1155Supply, BaseObject {
    ...
    function initWallPaper(
        ...
        emit InitWallPaper(tokenId, tokenUri, size, creator, maxClaimed, price);
    }
     ...
     */
    function getFreeWallPaper(uint256 tokenId) external {
    ...
    function buyWallPaper(uint256 tokenId) public payable {
        ...
        // require(success1);
        payable(allObjects[tokenId].creator).transfer(royality);

        // (bool success2, ) = payable(treasuryAddress).call{ value: (msg.value - royality) }("");
        // require(success2);
        payable(treasuryAddress).transfer(msg.value - royality);
    ...
    function batchWallPaper(uint256[] memory tokenIds) public payable {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            // require(success1);
            payable(allObjects[tokenIds[i]].creator).transfer(royality);

            // (bool success2, ) = payable(treasuryAddress).call{ value: (allObjects[tokenIds[i]].price - royality) }("");
            // require(success2);
            payable(treasuryAddress).transfer(allObjects[tokenIds[i]].price - royality);
...
abstract contract BaseObject is MultiOwner, IERC2981, ReentrancyGuard {
    ...
    function getMaxClaimed(uint256 tokenId) external view virtual returns (uint256) {
        return allObjects[tokenId].maxClaimed;
    }

    function getExp(uint256 tokenId) external view virtual returns (uint256) {
    ...
    function setTreasuryAddress(address payable newTreasuryAddress) external onlyOwner {
        require(newTreasuryAddress != address(0));
...
abstract contract MultiOwner is Context {
     ...
     */
    function owner(address targetAddress) external virtual returns (bool) {
