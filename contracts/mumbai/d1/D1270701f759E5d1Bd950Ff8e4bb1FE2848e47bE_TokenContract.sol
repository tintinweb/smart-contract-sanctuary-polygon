...
contract TokenContract is Initializable, ERC20Upgradeable, OwnableUpgradeable {
    ...
    IFactory public factoryAddress;
    ...
    function _onlyAdmin() internal {
        require(factoryAddress.hasRole(ADMIN_ROLE, msg.sender), "OnlyAdmin");
    }
    ...
    /**
     * @dev Returns the total supply of the token, which is equal to the limit supply.
    ...
    function mint(
        address _to,
        uint256 _amount
    ) public notBlacklisted(msg.sender) notBlacklisted(_to) {
        _onlyAdmin();
        require(
    ...
    function burn(
        address _userAddress,
        uint256 _amount
    ) external notBlacklisted(msg.sender) notBlacklisted(_userAddress) {
        _onlyAdmin();
    ...
    @dev Add multiple addresses to the blacklist array by only admin.
    @param _blankList Array of addresses to be blacklisted.
   */
    ...
    @dev Remove multiple addresses from the blacklist array by only admin.
    @param _removeBlankList Array of addresses to be removed from the blacklist.
    */
