...
contract Lilly is IERC20 {
   ...
   constructor(uint256 _totalsupply, uint256 _increase, uint256 _lilly_mint, address payable [] memory _staff, address payable _burn_addr, address payable _lilly_bank, uint _cost_to_play) public {
    ...
    // function to authorize prize reward
/*
    function auth_reward (bool authorize) public view returns (bool new_auth){
        bool test = false;
        require(msg.sender == owner || check_staff(msg.sender));
        auth_reward = authorize;
        emit auth_reward(old_auth, auth_reward);
...
*/
    ...
    function change_cost_to_play(uint256 new_cost_) public returns (bool){
        require(msg.sender == owner || check_staff(msg.sender), "you are not the owner or staff");
        cost_to_play = new_cost_ ;
        return true;
