...
import "./IERC1155StoreGeneric_Paper.sol";
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function addBulkDiscount(
        ...
        if (s_sales[_saleId].tokenId == 0) revert SG__NonExistentSale(_saleId);
        if (s_sales[_saleId].unitsPerUser < _breakpoint) revert SG__ValueTooLarge(_breakpoint);
        if (_basisPoints == 0) revert SG__InvalidSaleParameters();
        ...
        _validateDiscounts(_saleId);

        emit BulkDiscountAdded(_saleId, _breakpoint, _basisPoints);
    }

    /// @inheritdoc IERC1155StoreGeneric
    function addOwnershipDiscount(uint256 _saleId, OwnershipDiscount calldata _info)
        external
        onlyGov
    {
        if (s_sales[_saleId].tokenId == 0) revert SG__NonExistentSale(_saleId);
        if (_info.tokenAddress == address(0)) revert SG__ZeroAddress();
        if (_info.basisPoints > MAXIMUM_BASIS_POINTS) revert SG__InvalidSaleParameters();
        if (_info.basisPoints == 0) revert SG__InvalidSaleParameters();
        ...
        if (uint256(_info.tokenType) == 0) {
            ...
            ) revert SG__InvalidSaleParameters();
        }
        ...
        for (uint256 i; i < discounts.length; ++i) {
            if (
                discounts[i].tokenAddress == _info.tokenAddress &&
                discounts[i].tokenId == _info.tokenId
            ) {
                foundRebate = true;
        ...
        emit OwnershipDiscountAdded(_saleId, _info);
    }
    ...
    function _buyTokens(
        ...
        if (s_sales[_saleId].tokenId == 0) revert SG__NonExistentSale(_saleId);
        if (s_sales[_saleId].paused == true) revert SG__SaleInactive();
        if (s_whitelistedCurrencies[_saleId][_currency] == false)
            revert SG__CurrencyNotWhitelisted(_currency);
        if (s_perPlayerSold[_saleId][_buyer] + _numPurchases > s_sales[_saleId].unitsPerUser)
            revert SG__PurchaseExceedsPlayerMax();
        if (s_sold[_saleId] + _numPurchases > s_sales[_saleId].totalUnitSupply)
            revert SG__PurchaseExceedsTotalMax();

        unchecked {
            s_perPlayerSold[_saleId][_buyer] += _numPurchases;
            s_sold[_saleId] += _numPurchases;
        }

        Sale memory sale = s_sales[_saleId];

        uint256 balance = _applyBulkDiscount(_saleId, _numPurchases);
        if (_tokenAddress != address(0))
            balance = _applyOwnershipDiscount(_saleId, balance, _tokenAddress, _tokenId, _buyer);

        if (_currency == address(0)) {
            _ethPayment(_buyer, sale.defaultCurrency, _currency, balance);
        } else {
            _erc20Payment(_buyer, sale.defaultCurrency, _currency, balance, sale.profitState);
        }

        if (_optInBonuses) {
            s_saleBonuses.claimBonusReward(
                _saleId,
                uint32(_numPurchases),
                _optInCategories,
                _buyer
            );
        }
    ...
    function _validateDiscounts(uint256 _saleId) internal view {
        ...
        OwnershipDiscount[] memory discounts = s_ownershipDiscounts[_saleId];
        uint256 best;
        OwnershipDiscount memory current;
        for (uint256 j; j < discounts.length; j++) {
            current = discounts[j];
            if (current.basisPoints > best) {
                best = current.basisPoints;
            }
        }
        if (sum >= MAXIMUM_BASIS_POINTS) revert SG__DiscountTooLarge(sum, MAXIMUM_BASIS_POINTS);
        if (best >= MAXIMUM_BASIS_POINTS) revert SG__DiscountTooLarge(best, MAXIMUM_BASIS_POINTS);
        /*
        ...
        */
        Sale memory info = s_sales[_saleId];
        uint256 postRebatePrice = (((info.unitPrice * (MAXIMUM_BASIS_POINTS - sum)) /
            MAXIMUM_BASIS_POINTS) * (MAXIMUM_BASIS_POINTS - best)) / MAXIMUM_BASIS_POINTS;
        if (postRebatePrice == 0)
            revert SG__CombinedDiscountTooLarge(_saleId, info.unitPrice, sum, best);
    }
...
interface IERC1155StoreGeneric {
     ...
     */
    function addOwnershipDiscount(uint256 _saleId, OwnershipDiscount calldata _info) external;
