...
contract PluckMarket is Ownable {
  ...
  event MyNewLog(address signer, string stuff);
  ...
  function insure(
    ...
    if (_insurancePrice <= 0) {
      emit InsurancePaymentFailure(
        _insuranceProviderId,
        _amountAvailable,
        _insurancePrice,
        _tokenId,
        "Insurance amount must be greater than 0"
      );
    }
    ...
    if (_insurancePrice > _amountAvailable) {
      emit InsurancePaymentFailure(
        ...
        _insurancePrice,
        _tokenId,
        "Not enough funds were sent in transaction"
      );
    }
    ...
    returns (bool isValid)
  {
    ...
    address _signer = ecrecover(
      _hash,
      _signatureComponents.signature.v,
      _signatureComponents.signature.r,
      _signatureComponents.signature.s
    );
  ...
  function checkInsuranceTicket(SignatureInsuranceComponents memory _signatureComponents)
    private
    returns (bool insuranceIsValid)
  {
    ...
    emit MyNewLog(_signer, "Is this the real world?");
