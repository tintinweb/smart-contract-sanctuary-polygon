...
contract Game {
    ...
    // function addRound(uint _round, bool _result) private {
    //     Round memory newRound = Round(_round, _result);
    ...
    function battle(uint _index) public {
        ...
        if (players[_index].health < 0 && spawns[_index].health < 0){
            Round memory newRound = Round(_index, false);
            rounds.push(newRound);
            // return false;
        } else if (players[_index].health > 0 && spawns[_index].health <= 0){
            Round memory newRound = Round(_index, true);
            rounds.push(newRound);
            score++;
            // return true;
        } else if (players[_index].health <= 0 && spawns[_index].health > 0){
