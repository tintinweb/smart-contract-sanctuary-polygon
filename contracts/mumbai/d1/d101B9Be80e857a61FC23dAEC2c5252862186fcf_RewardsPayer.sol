...
/// @author @Mike_Bello90
/// @notice You can use this contract to calculate the user rewards for Holders, Minters and Rents.
...
using PRBMathUD60x18Typed for PRBMath.UD60x18;
    ...
    /// @dev Struct to register and validate the data need it to calculate the user's rewards.
    struct UserRewards {
        uint256 rewardPerBuilder;
        uint256 lastTimePayBuilder;
        uint256 lastTimeCalcBuilder;
        uint256 rewardPerBuilderNFT; // Front statistics
        uint256 rewardPerHolder;
        uint256 lastTimePayHolder;
        uint256 lastTimeCalcHolder;
        uint256 rewardPerHolderNFT; // front statistics
    }
    ...
    address vaultLenders, address VaultRewardsContract, address PoolRewardsLenders) {
        ...
        _setupRole(DEFAULT_ADMIN_ROLE, 0x30268390218B20226FC101cD5651A51b12C07470); // multisign
 ...
 /// @dev this allow the user to withdraw his rewards if he is a Builder user
   ...
   ///@dev Function to check if the user has a Mortgage debt and pay the past due debt before he can withdraw the rents from a collection.
     ...
     * el Autotask debe validar si el monto a repartir es muy bajo y podria dar 0 para que en ese caso no haga el calculo ni el reseteo para que se sumen 
     * al siguiente dia y asi calcular solo cuando no dara 0.
     */
    ...
    function CalcBuilderRewardsDaily(address[] calldata _walletsUsers,uint256[] calldata _numNFTsMinted, address _token) public onlyDev {
        ...
        PRBMath.UD60x18 memory ITD = PRBMath.UD60x18({value: (ingresoTemporalDiario * 1e12)});
        //PRBMath.UD60x18 memory PercentageM = PRBMath.UD60x18({value: percentageMinters });
        PRBMath.UD60x18 memory multITDxPercentageM = ITD.mul(PRBMath.UD60x18({value: percentageMinters }));
        PRBMath.UD60x18 memory ingresoDiarioForBuilders = multITDxPercentageM.div(PRBMath.UD60x18({value: 10000}));
       // PRBMath.UD60x18 memory TotalNFTsMinteadosGeneral = PRBMath.UD60x18({value: (336 * 1e18)});  // solo para testing
        //PRBMath.UD60x18 memory TotalNFTsMinteadosGeneral = PRBMath.UD60x18({value: (controlContract.seeCounter() * 1e18)}); 
        PRBMath.UD60x18 memory PrePayPerNFT = ingresoDiarioForBuilders.div(PRBMath.UD60x18({value: (336 * 1e18)}));
 
        for (uint16 i = 0; i < wlength; ) {
            if (_numNFTsMinted[i] != 0) {
                userRewardsBH[_walletsUsers[i]].rewardPerBuilder += _numNFTsMinted[i] * (PrePayPerNFT.value/1e12);
                userRewardsBH[_walletsUsers[i]].rewardPerBuilderNFT = (PrePayPerNFT.value/1e12);
    ...
    function CalcHolderRewardsDaily(address[] calldata _walletsUsers,uint256[] calldata _numNFTsHolder, address _token) public onlyDev {
        ...
        PRBMath.UD60x18 memory ITD = PRBMath.UD60x18({value: ingresoTemporalDiario * 1e12});
        PRBMath.UD60x18 memory multITDxPercentageH = ITD.mul(PRBMath.UD60x18({value: percentageHolders}));
        PRBMath.UD60x18 memory ingresoDiarioForHolders = multITDxPercentageH.div(PRBMath.UD60x18({value: 10000}));
        // PRBMath.UD60x18 memory PrePayPerNFT = ingresoDiarioForHolders.div(PRBMath.UD60x18({value: (controlContract.seeCounter() * 1e18)}));
        // solo para testing
        PRBMath.UD60x18 memory PrePayPerNFT = ingresoDiarioForHolders.div(PRBMath.UD60x18({value: (336 * 1e18)}));  // solo para testing

        for (uint16 i = 0; i < wLength; ) {
            if (_numNFTsHolder[i] != 0) {
                userRewardsBH[_walletsUsers[i]].rewardPerHolder += _numNFTsHolder[i] * (PrePayPerNFT.value/1e12);
                userRewardsBH[_walletsUsers[i]].rewardPerHolderNFT = (PrePayPerNFT.value/1e12);
    ...
    function CalcRentRewardsForCollection(address _collection,address[] calldata _walletsUsers,uint256[] calldata _numNFTsHolder) public onlyDev {
        ...
        PRBMath.UD60x18 memory MRI = PRBMath.UD60x18({value: monthlyRentIncome * 1e12});
        PRBMath.UD60x18 memory dailyIncome = MRI.div(PRBMath.UD60x18({value: 30 * 1e18}));
        // solo para testing
        PRBMath.UD60x18 memory PayPerNFT = dailyIncome.div(PRBMath.UD60x18({value: 336 * 1e18})); // solo para testing
        //PRBMath.UD60x18 memory PayPerNFT = dailyIncome.div(PRBMath.UD60x18({value: CollectionContract.totalSupply() * 1e18})); // case for mainnet
        
        /* uint256 dailyIncome = monthlyRentIncome / 30;
        uint256 totalNFtsxCollection = CollectionContract.totalSupply();
        //uint256 totalNFtsxCollection = 336;
        uint256 PayPerNFT = dailyIncome / totalNFtsxCollection; */

        for (uint16 i = 0; i < wLength; ) {
            if (_numNFTsHolder[i] != 0) {
                collectionRewardsRentPerUser[_walletsUsers[i]][_collection].rewardPerRent += _numNFTsHolder[i] * (PayPerNFT.value / 1e12);
                collectionRewardsRentPerUser[_walletsUsers[i]][_collection].rewardPerRentNFT = (PayPerNFT.value / 1e12);
    ...
    ///@dev function to show the user's rent rewards in the frontend
    ...
    ///@dev admin functions to update contracts addresses 
...
contract HelperRewardsContract is ReentrancyGuard, AccessControl{
   ...
   address public walletPanoram = 0x30268390218B20226FC101cD5651A51b12C07470; 
