...
contract Draft is ERC721, ERC2981, Pausable, Ownable, ReentrancyGuard {
  ...
  constructor() ERC721("MyToken", "MTK") {
    ...
    setMintStatus("ALLOWLIST", 0.077 ether, 2);
    setMintStatus("PREMIUM_ALLOWLIST", 0.088 ether, 2);
    setMintStatus("PUBLIC", 0.1 ether, 10);

    setActiveMintStatus("ALLOWLIST");
  }

  // Mint: public
  function mint(uint256 _amount, bytes memory _signature) external payable whenNotPaused onlyEOA nonReentrant {
    require(addressCanMintAmount(msg.sender, _amount), "Not available");
    require(isSignatureValid(_signer, keccak256(abi.encodePacked(msg.sender, _amount)), _signature), "Not allowed");
