...
contract Orchestrator is IOrchestrator, OwnableUpgradeable, UUPSUpgradeable {
    ...
    */
        ...
        public
    {
        ...
        uint256 license = IKeeperRegistry(keeperRegistry).checkLicense(msg.sender);
    ...
    function actionApprovalStatus(bytes32 actionHash) public view returns (bool) {
        uint256 maxLicenseId = IKeeperRegistry(keeperRegistry).maxNumKeepers() + 1;
        uint256 yesVotes;
        ...
        uint256 numKeepers = IKeeperRegistry(keeperRegistry).currentNumKeepers();
        ...
        returns (ActionState)
    {
        ...
        if (actionApproved) {
            ...
            (success, ) = address(this).call{
                gas: IGasVault(gasVault).gasAvailableForTransaction(targetAddress)
            ...
            IGasVault(gasVault).reimburseGas(targetAddress, originalGas, newActionHash);
