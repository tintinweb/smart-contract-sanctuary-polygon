...
interface IPricingManagerInternal {
...
import {IPricingManagerInternal} from "../interfaces/IPricingManagerInternal.sol";
import {MerkleProofUpgradeable} from "@openzeppelin/contracts-upgradeable/utils/cryptography/MerkleProofUpgradeable.sol";

abstract contract PricingManagerInternalFacet is IPricingManagerInternal {
    using InsuranceBaseStorage for InsuranceBaseStorage.Layout;

    function _setPricingMerkleRoot(bytes32 _merkleRoot_) internal {
        InsuranceBaseStorage.layout().pricingMerkleRoot = _merkleRoot_;

        emit PricingMerkleRootChanged(_merkleRoot_);
    }

    function _getPricingMerkleRoot() internal view returns (bytes32) {
        return InsuranceBaseStorage.layout().pricingMerkleRoot;
    }

    function _verifyMerkleProof(
        bytes32[] memory _proof_,
        string memory _policyType_, // cargo => general_01 || ไปรษณีญ์ => จังหวัด_อำเภอ_ตำบล_รหัสปณ, value //
        uint256 _premiumRate_ // 10**6 = 100% //
    ) internal view returns (bool) {
        require(_premiumRate_ <= 1 * 10**6 && _premiumRate_ > 0, "IPR");
        return
            MerkleProofUpgradeable.verify(
                _proof_,
                _getPricingMerkleRoot(),
...
import {InsuranceBaseStorage} from "./base/InsuranceBaseStorage.sol";
import {PricingManagerInternalFacet} from "./internal/PricingManagerInternalFacet.sol";
import {AccessControlInternal} from "@solidstate/contracts/access/access_control/AccessControl.sol";

contract PricingManagerFacet is
    PricingManagerInternalFacet,
    AccessControlInternal
{
    function setPricingMerkleRoot(bytes32 _merkleRoot_)
        public
        onlyRole(InsuranceBaseStorage.SUPER_MANAGER_LEVEL)
    {
        _setPricingMerkleRoot(_merkleRoot_);
    }

    function getPricingMerkleRoot() public view returns (bytes32) {
        return _getPricingMerkleRoot();
    }

    function verifyMerkleProof(
        bytes32[] calldata _proof_,
        string memory _reference_,
        uint256 _pricing_
    ) public view returns (bool) {
        return _verifyMerkleProof(_proof_, _reference_, _pricing_);
    }
}