...
contract DegenMascotsNFTMint is Ownable, ERC721Enumerable {
    using Address for address;
     ...
     */
    constructor(string memory _baseURIParam) ERC721("Degen Mascots", "DGBM") {
    ...
    function MintDegenMascotsNft(uint256 _mintNum) external payable {
        ...
        for (uint256 inx = 0; inx < _mintNum; inx++) {
            ...
            for (uint256 i = id; i < mintableNFTNumber; i++) {
                if (!_exists(i)) {
        ...
        onlyOwner
    {
        ...
        for (uint256 inx = 0; inx < _qty; inx++) {
            for (uint256 i = id; i < mintableNFTNumber; i++) {
                if (!_exists(i)) {
                    _mint(_buyer, i);
                    id = i + 1;
