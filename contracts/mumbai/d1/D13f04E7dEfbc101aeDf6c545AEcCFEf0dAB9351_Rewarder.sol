...
contract Rewarder is IRewarder, SybelAccessControlUpgradeable, ContentBadges, ListenerBadges, PushPullReward {
    ...
    event TestContentPool(uint256 contentId, uint256 amount, address contentPoolAddr);
    event TestReward(uint256 totalReward, uint256 userReward, uint256 ownedReward, uint256 earningFactor);
    ...
    function computeRewardForContent(
        ...
        if (hasOnePaidFraktion) {
            ...
            emit TestContentPool(contentId, contentPoolReward, address(contentPool));
        ...
        emit TestReward(totalReward, userReward, ownerReward, earningFactor);
