...
contract ERC20 is IERC20, IERC20Metadata, Admin, ReEntrancyGuard {
    ...
    function areTokensAvailable(address _owner, uint _amountToWithdraw) public activeContract returns (bool isValidTransfer) {
        ...
        if (_amountToWithdraw <= unlockedAmount) {
            amountAvailable = _destroyLock(_owner, newIndex, _amountToWithdraw);
            if (_amountToWithdraw <= amountAvailable) return true;
        }
    ...
    function _destroyLock(address _owner,
        ...
        require((_unlockedLocks > 0) && (_amountToWithdraw != 0), "LOCK => You need at least one token/lock.");
        ...
        while ( (brokenLocks < _unlockedLocks) && (_amountAvailable < _amountToWithdraw) ) {
            // Search the lock to destroy (or not)
            _lock = addressToLock[_owner][addressToValidIndex[_owner] + brokenLocks];
            ...
            if (amountLeft < _lock.amount) {
                ...
                _addressToUnlockedAmount[_owner] += _amountToWithdraw;
                break;
        ...
        // We modify the index if we break any lock and remove the total available
        // plus emiting an event for the locks destroyed
        if (brokenLocks > 0) {
            addressToValidIndex[_owner] += brokenLocks;
            emit lockDestroyed(_owner, brokenLocks, _amountAvailable);
        }
    }
...
contract FanToken is ERC20 {
    ...
    function buyTokensWithMATIC(uint256 _amountOfTokens) public payable
        ...
        // This is taking advantage of the return flag to cut the user flow elegant 
        //      with a message.    
        require(_updateMaticPrice() == 2, "The MATIC price just change. Check again.");
    ...
    function buyTokensWithUSDT(uint256 _amountOfTokens) public
        ...
        _updateMaticPrice();
        uint256 priceInDollars = ((tokenPrice / 1e8) * _amountOfTokens) * 1e6;
     ...
     * @dev NOTE If the timestamp is different, update the price                    *
     * @dev NOTE This returns different if the price is updates. This is meant to   *
     *      avoid unexpected changes while calling buyWithMATICFunction             *
     */
    function _updateMaticPrice() internal returns (uint) {
