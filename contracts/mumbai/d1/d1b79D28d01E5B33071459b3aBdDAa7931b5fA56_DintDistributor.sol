...
contract DintDistributor is Ownable {
    ...
    /// @dev Maximum Duration allowed for earning rewards 
    uint64 public maxDuration = 315360000; // 10 year by default

    /// @dev Referral earning limit in usd amount multiplied by 10**6 for better precision 
    uint256 public earningLimit = 500_000E6; // 500k (multiplied by 10**6)
    ...
    function register(
      ...
      user[_user].isReferrer = true;
      user[_user].tipReceiverToReferrer = _referrer;
      user[_user].startedReferringAt = uint64(block.timestamp);
    }
    ...
    function blockUnblockReferrer(address _referrer, bool _blocked) external onlyOwner {
      ...
      if (_blocked) {
        ...
        user[_referrer].isReferrer = true;
      }
    }
    ...
    /**
	    * @dev Sets maximum allowed duration for earning rewards as a referrer
	    *
      * @notice Restricted access function, should be called by owner only
      * 
      * @param _durationInSeconds duration in seconds
      */
    function setMaxDuration(uint64 _durationInSeconds) external onlyOwner {
      require(_durationInSeconds != 0, "Invalid duration");
      maxDuration = _durationInSeconds;
    }

    /**
	    * @dev Sets referral earning limit in usd amount multiplied by 10**6 for better precision
	    *
      * @notice Restricted access function, should be called by owner only
      * 
      * @param _limit earning limit in usd amount multiplied by 10**6
      */
    function setEarningLimit(uint256 _limit) external onlyOwner {
    ...
    /**
      ...
      */
    function isValidReferrer(address _referrer) internal view returns(bool) {
      return user[_referrer].isReferrer &&
      (block.timestamp - user[_referrer].startedReferringAt) < maxDuration;
    }
    ...
    /**
      ...
      * @param _priceInUSD dint token price in terms of usd multiplied by 10**6 for better pricision
	    */
    function sendDint(address _sender, address _recipient, uint256 _amount, uint256 _priceInUSD) external onlyOwner {
      ...
      if(isValidReferrer(user[_recipient].tipReceiverToReferrer)) {
        referrer[0] = user[_recipient].tipReceiverToReferrer;
        forReferral[0] = 400;
        forDintApp -= 400;
        count++;
            
        if(isValidReferrer(user[referrer[0]].tipReceiverToReferrer)) {
          referrer[1] = user[referrer[0]].tipReceiverToReferrer;
          forReferral[1] = 200;
          forDintApp -= 200;
          count++;

          if(isValidReferrer(user[referrer[1]].tipReceiverToReferrer)) {
            referrer[2] = user[referrer[1]].tipReceiverToReferrer;
            forReferral[2] = 100;
            forDintApp -= 100;
            count++;
                
            if(isValidReferrer(user[referrer[2]].tipReceiverToReferrer)) {
      ...
      for(uint i; i < count; i++) {
        uint _amountInUSD =  ((_amount * forReferral[i] / 10000) * _priceInUSD) / 1E18;

        if(user[referrer[i]].earned + _amountInUSD <= earningLimit) {
          IERC20(dintToken).transferFrom(_sender, referrer[i], (_amount * forReferral[i] / 10000));
          user[referrer[i]].earned += _amountInUSD;
        } else {
          IERC20(dintToken).transferFrom(_sender, feeCollector, (_amount * forReferral[i] / 10000));
        }
