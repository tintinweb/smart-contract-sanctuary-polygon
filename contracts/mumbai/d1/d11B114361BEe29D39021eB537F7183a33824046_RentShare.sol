...
/**
 * @dev Extension of {StandardAccessControlEnumerableUpgradeable} that offer support for RentClaimerVault role.
 */
contract RentShareAccessControlEnumerable is
    StandardAccessControlEnumerableUpgradeable
{
    /// @notice Role for ERC4626Vault that can harvestRewardsForVault in RentShare.sol
    ...
    uint256[50] private __gap_rentShareAccessControl;

    /// @notice Modifier to ensure that caller has "RENT_CLAIMER_VAULT_ROLE" role
    modifier onlyRentClaimerVault() {
        _require(
            hasRole(RENT_CLAIMER_VAULT_ROLE, msg.sender),
            Errors.CALLER_NOT_VAULT
        );
        _;
    }
}
...
/**
 * @title RentShare
 * @notice To Facilitate users and Vault to havest rewards. Admin to update/pause/unpause Rewards of property
 */
    ...
    SelfPermitUpgradeable
{
    ...
    /// @notice To ensure that caller delay to harvest rewards is passed
    modifier delayedHarvestRewards() {
        require(
            storageParams.harvestDelay == 0 ||
                (storageParams.pendingHarvestRewardsTimestamps[msg.sender] !=
                    0 &&
                    block.timestamp >=
                    storageParams.pendingHarvestRewardsTimestamps[msg.sender]),
            "Harvest not requested yet or it is too early to harvest"
        );
        storageParams.pendingHarvestRewardsTimestamps[msg.sender] = 0;
        _;
    }

    /**
     * @notice Initializes the contract.
     * @param _rewardTokenAddress address of token to be used as rent rewards token
     * @param finder address of finder to fetch protocol's used addresses
     */
    function initialize(
        ...
        storageParams.harvestDelay = 0;
        storageParams.maxHarvestDelay = 1 weeks;
    }
    ...
    function isPropertyRewardsPaused(
        ...
        return
            storageParams.rewardsPaused[
                isPropertyTokenWhitelisted(tokenSymbol)
            ];
    }
    ...
    function getPoolStakerAmount(
        ...
        return
            storageParams
            .poolStakers[isPropertyTokenWhitelisted(tokenSymbol)][_staker]
                .amount;
    }
     ...
     * @param tokenSymbol wrapped token symbol
     * @param _staker address of the staker
     * @return rewardsToHarvest amount of reward tokens claimable by user
     */
    ...
    function pausePropertyRewards(
        ...
        uint256 poolId = isPropertyTokenWhitelisted(tokenSymbol);
        updatePoolRewards(poolId);
        storageParams.rewardsPaused[poolId] = true;

        emit RewardsPaused(poolId);
    }

    /**
     * @notice to unpause the rewards of specific pool
     * @param tokenSymbol of the wrapped property
     */

    function unPausePropertyRewards(
        string memory tokenSymbol
    ) external onlyMaintainer {
        uint256 poolId = isPropertyTokenWhitelisted(tokenSymbol);
        updatePoolRewards(poolId);
        storageParams.rewardsPaused[poolId] = false;

        emit RewardsUnpaused(poolId);
    }
     ...
     * @param maintainer WrappedLegal to perform specific task in this contact e.g withdraw, deposit.
     * @param symbol symbol of the wrappedLegal token
    ...
    function deposit(
        ...
        _require(_amount > 0, Errors.ZERO_AMOUNT);
    ...
    /// @notice Request to harvest rewards if harvestDelay is nonzero
    /// @dev This is required to prevent users from frontrunning resetMaliciousStakerAmount
    function requestHarvestReward() external {
        require(storageParams.harvestDelay > 0, "Harvest delay is 0");
        require(
            storageParams.pendingHarvestRewardsTimestamps[msg.sender] == 0,
            "Harvest Rewards request already pending"
        );

        storageParams.pendingHarvestRewardsTimestamps[msg.sender] =
            block.timestamp +
            storageParams.harvestDelay;
    }
    ...
    function harvestRewards(
        string memory symbol
    ) public delayedHarvestRewards returns (uint) {
        uint poolId = isPropertyTokenWhitelisted(symbol);
        IRentShare.PoolStaker storage staker = storageParams.poolStakers[
            poolId
        ][_msgSender()];
        ...
        _require(
            block.timestamp -
                ...
                7 days,
            Errors.CLAIMING_TWICE_A_WEEK
        );

        uint rewardsHarvested = _harvestRewards(poolId, _msgSender());
        storageParams.userToPropertyRentClaimTimestamp[_msgSender()][
            symbol
        ] = block.timestamp;
    ...
    /// @notice Sets the withdrawal delay time to prevent resetMaliciousStakerAmount from frontrunning and harvestRewards
    function setHarvestDelay(uint256 delay) external onlyMaintainer {
        require(
            delay <= storageParams.maxHarvestDelay,
            "StakingModule: Desired delay exceeds maximum"
        );
        storageParams.harvestDelay = delay;
    }

    /**
     * @notice only vault should be able to harvest rewards for given pool id
     * @dev made this function to avoid the restriction to claim rent only once a week
     * @param symbol of the wrapped Property
     */

    function harvestRewardsForVault(
        string memory symbol
    ) public onlyRentClaimerVault returns (uint) {
        uint poolId = isPropertyTokenWhitelisted(symbol);
        uint rewardsHarvested = _harvestRewards(poolId, _msgSender());
        storageParams.userToPropertyRentClaimTimestamp[_msgSender()][
            symbol
        ...
        emit VaultRewardsHarvested(poolId, _msgSender(), rewardsHarvested);
        return rewardsHarvested;
    }
    ...
    function _harvestRewards(
        ...
        uint amount = storageParams.harvestRewards(_poolId, sender);
        emit HarvestRewards(_poolId, sender, amount);
        return amount;
    }
     ...
     * @return when the last time user claimed rent
     */
    ...
    /// @notice returns the token used as rent reward
    function getRewardToken() external view returns (address) {
        return storageParams.rewardToken;
    }

    /// @notice poolId if token is whitelisted, else reverts
    function isPropertyTokenWhitelisted(
        string memory propertySymbol
    ) public view returns (uint) {
        return storageParams._isPropertyTokenWhitelisted(propertySymbol);
    }

    /**
     * @param poolId id of the pool
     * @return symbol against the pool id
     */
    function getPoolIdToSymbol(
        uint poolId
    ) external view returns (string memory) {
        return storageParams.poolIdToSymbol[poolId];
    }
    ...
    function getPoolRewardsPerSecond(
        ...
        uint256 poolId = isPropertyTokenWhitelisted(tokenSymbol);
        return storageParams.pools[poolId].rewardTokensPerSecond;
    }

    /// @notice returns harvest delay
    function getHarvestDelay() external view returns (uint) {
        return storageParams.harvestDelay;
    }

    /// @notice returns max delay
    function getMaxHarvestDelay() external view returns (uint) {
        return storageParams.maxHarvestDelay;
    }

    /**
     * @param _user user address
     * @return time after which user is eligible to harvestRewards
     */
    function getUserRequestToHarvestRewardTimestamp(
        address _user
    ) external view returns (uint) {
        return storageParams.pendingHarvestRewardsTimestamps[_user];
    }
}
