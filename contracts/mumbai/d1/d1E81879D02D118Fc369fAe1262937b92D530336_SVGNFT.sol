...
contract SVGNFT is ERC721, ERC721URIStorage, Ownable {
    ...
    function addPoints(uint256 tokenId, uint256 points) public onlyOwner {
        require(_exists(tokenId), "ERC721URIStorage: nonexistent token id");
        _points[tokenId].push(points);
    }
    ...
    function _totalPoints(uint256 tokenId) private view returns (uint256) {
        uint256 total = 0;
        for (uint256 i = 0; i < _points[tokenId].length; i++) {
            total += _points[tokenId][i];
        }
        if (total > 100) {
            return 100;
        }
    ...
    function getTotalPoints(uint256 tokenId) public view returns (uint256) {
        require(_exists(tokenId), "ERC721URIStorage: nonexistent token id");
        return _totalPoints(tokenId);
    }

    function getSvg(uint tokenId) private view returns (string memory) {

        uint256 index = _totalPoints(tokenId) / 5;
        if (index > 19) {
            index = 19;
        }

        uint8 pos = pointpos[index];
        
        return _replace(svg, "@", Strings.toString(pos));
    }
    ...
    function tokenURI(
        ...
        bytes memory name = abi.encodePacked("SVG NFT #", Strings.toString(tokenId));
