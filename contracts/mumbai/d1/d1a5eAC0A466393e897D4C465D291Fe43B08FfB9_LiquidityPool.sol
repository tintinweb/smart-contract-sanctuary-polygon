    ...
    LiquidStorage
{
    ...
    function calculateUserRewardShare(
        ...
        // A compounding period must exists, I've set it as 12, as in compounds 12 times a year
    ...
    /**
     * @notice Swaps tokens, calculates price accordingly
    ...
    LiquidCalculate
{
    ...
    function changeOperationsWallet(
        ...
        if (_operationsWallet == address(0))
            revert("Reward wallet can't be null address");
    ...
    function changeRewardWallet(
        ...
        LiquidTypes.TaxInfo storage taxInfo = LiquidStorage.taxWalletInfo[
            _poolID
        ];
    ...
    function setAllocationPoints(
        ...
        LiquidTypes.AllocationPointsPerPair memory alloc = LiquidStorage
            .allocPairs[_poolID];
     ...
     */
    function getRewardWalletBalance(
        uint256 _poolID
    ) external view returns (uint256) {
    ...
    function updateSlippagePercent(
        uint256 _poolID,
        uint256 _newSlippagePercent
    ) external {
        LiquidTypes.PoolPair storage pairs = LiquidStorage.poolPairs[_poolID];
    ...
    function returnLiquidityProviders()
        external
        view
    ...
    // function getUserAllowed(uint256 _poolID) external view returns (uint256) {
    //     LiquidTypes.UserInfo storage users = LiquidStorage.userInfo[_poolID][
    //         msg.sender
    //     ];
    //     return user.amount;
    // }
    function getUserAllowed(
        uint256 _poolID,
        address _user
    ) external view returns (uint256) {
        LiquidTypes.UserInfo storage user = LiquidStorage.userInfo[_poolID][
...
interface IERC20 {
    ...
    // function transferFunds(address sender, address recipient, uint256 amount) external returns (bool);
