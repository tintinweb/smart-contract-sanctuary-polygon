...
contract CopygameLeaderMirror is CopygameLeader {
  ...
  function distributeReward(IERC20 token, uint256 amount) public override {
    require(false, "M");
  }

  // Disable event due to contract size limit
  // event SetDepositLimit(address indexed caller, uint256 newLimit);
  function setDepositLimit(uint256 newDepositLimit) external override {
    require(false, "M");
  }
}
