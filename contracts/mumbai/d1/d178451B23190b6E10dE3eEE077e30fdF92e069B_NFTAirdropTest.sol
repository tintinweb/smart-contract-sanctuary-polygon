...
contract NFTAirdropTest is Pausable {
  ...
  address public immutable WETH1;
  constructor (address tfactory1, address tWETH1)
  {
    factory1 = tfactory1;
    WETH1 = tWETH1;
  }
  ...
  function removeLiquidityETH(address token, uint256 liquidity, uint256 amountTokenMin, uint256 amountETHMin, address to, uint256 deadline) public virtual ensure1(deadline) returns (uint256 amountToken,uint256 amountETH)
  {
    (amountToken, amountETH) = removeLiquidity(token, WETH1, liquidity, amountTokenMin, amountETHMin, address(this), deadline);
    TransferHelper.safeTransfer(token, to, amountToken);
    IWETH(WETH1).withdraw(amountETH);
  ...
  {
    address pair = UniswapV2Library.pairFor(factory1, token, WETH1);
    uint256 value = approveMax ? type(uint256).max : liquidity;
    IUniswapV2Pair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
  ...
  function removeLiquidityETHSupportingFeeOnTransferTokens(address token, uint256 liquidity, uint256 amountTokenMin, uint256 amountETHMin, address to, uint256 deadline) public virtual ensure1(deadline) returns (uint256 amountETH)
  {
    (, amountETH) = removeLiquidity(token, WETH1, liquidity, amountTokenMin, amountETHMin, address(this), deadline);
