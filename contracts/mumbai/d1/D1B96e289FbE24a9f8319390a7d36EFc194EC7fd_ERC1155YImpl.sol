...
// File upgradeable_contracts/ContextMixinUpgradeable.sol
...
// File upgradeable_contracts/PlatformUpgradeable.sol
...
contract PlatformUpgradeable is Initializable {
    ...
    function __Platform_init_unchained() internal onlyInitializing {
        ...
        minPlatformRoyalty = 5000;
        minTokenPrice = 30000000000000000000; // In Wei (MATIC)
        devToPlatform = true;
        devToCreator = false;
    }

    modifier onlyDeveloper() {
        require(devAddress == msg.sender, 'Dev Only: Caller is not a developer');
        _;
    }
    ...
    function setMintTokenPrice(uint256 _price) external onlyDeveloper {
    ...
    function partiesParams(
        ...
        require(platformRoyalty >= 5000, "Platform royalty must be equal or bigger than 5000");
...
// File upgradeable_contracts/ERC1155OpenseaUpgradeable.sol
...
// File upgradeable_contracts/ERC1155YUpgradeable.sol
...
contract ERC1155YUpgradable is ERC1155OpenseaUpgradeable {
    using StringsUpgradeable for uint256;
    ...
    function purchase(address to, uint256 tokenId, uint256 tokensNumber) public payable {
    ...
    function bulkPurchase(address to, uint256[] memory ids, uint256[] memory amounts) public payable {
    ...
    function setWalletBaseLimit(uint256 _walletLimit) public onlyOwner salesActive{
        perWalletLimit = _walletLimit;
    }

    function setTokenBaseSupply(uint256 _tokenSupply) public onlyOwner salesActive{
        maxSupply = _tokenSupply;
    }
    
    function setTokenBasePrice(uint256 _tokenPrice) public onlyOwner salesActive{
        require(_tokenPrice >= minTokenPrice, string(abi.encodePacked("Token price at least must be ", StringsUpgradeable.toString(minTokenPrice), " wei (MATIC)")));
        tokenPrice = _tokenPrice;
    }

    function setWalletLimitByToken(uint256 tokenId, uint256 _walletLimit) public onlyOwner salesActive{
        maxLimitTokens[tokenId] = _walletLimit;
    }

    function setSupplyByToken(uint256 tokenId, uint256 _tokenSupply) public onlyOwner salesActive{
        maxSupplyTokens[tokenId] = _tokenSupply;
    }
    
    function setPriceByToken(uint256 tokenId, uint256 _tokenPrice) public onlyOwner salesActive{
        require(_tokenPrice >= minTokenPrice, string(abi.encodePacked("Token price at least must be ", StringsUpgradeable.toString(minTokenPrice), " wei (MATIC)")));
...
// File upgradeable_contracts/ERC1155YImpl.sol

pragma solidity ^0.8.0;

contract ERC1155YImpl is ERC1155YUpgradable {
  function initialize(
    string memory _name,
    string memory _symbol,
    uint256 _maxSupplyPerToken,
    ...
    __ERC1155Y_init(_name, _symbol, _maxSupplyPerToken, _maxMintPerWallet, _tokenPrice, _platformAddress, _platformRoyalty, _creatorAddress, _creatorRoyalty);
  }
}