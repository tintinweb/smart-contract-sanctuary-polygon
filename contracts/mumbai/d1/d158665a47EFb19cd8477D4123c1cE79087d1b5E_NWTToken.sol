...
contract ERC20 is Context, IERC20, IERC20Metadata {
     ...
     * - when `from` is zero, `amount` tokens have been minte
     * d for `to`.
...
contract NWTToken is ERC20, Ownable {
    ...
    uint256 public maxOwnedAmount = 9000000 * (10**18);
    uint256 public initialSupply = 900000000 * (10**18); // Initial supply
    ...
    constructor() ERC20("Ninja Warrior Token", "NWT") {
        teamFeeOnBuy = 1;
        teamFeeOnSell = 1;
    ...
    function setTeamFee(uint256 _onBuy, uint256 _onSell) external onlyOwner {
        require(_onBuy <= 10 && _onSell <= 10, "Invalid Fee");
        teamFeeOnBuy = _onBuy;
    ...
    function setBuyBackFee(uint256 _onBuy, uint256 _onSell) external onlyOwner {
        require(_onBuy <= 10 && _onSell <= 10, "Invalid Research Fee");
    ...
    function blackList(address account, bool blacklisted) public onlyOwner {
        require(
            _isBlackList[account] != blacklisted,
            "Account is already the value of 'blacklisted'"
        );
        require(account != address(0), "Account is zero");
        _isExcludedFromFees[account] = blacklisted;
    }
    ...
    function updateChairtyWallet(address _charityWallet) internal onlyOwner {
        require(
    ...
    function updateBuyBackWallet(address _buyBackWallet) internal onlyOwner {
        require(
    ...
    function updateMaxOwnedAmount(uint256 _amount) public onlyOwner {
    ...
    function _transfer(
        ...
        require(_isBlackList[to] == false, "Receiver is blacklisted");
        require(_isBlackList[from] == false, "Sender is blacklisted");
    ...
    function _tokenTransfer(
        ...
        // uint256 _fromReflections = _reflections[from];

        // _reflections[from] = 0;
        // _balances[from] = _fromBalance+_fromReflections;
    ...
    function swapTokens(uint256 tokenAmount) private {
        ...
        uint256 nativeForCharity = swappedNative.mul(charityShare).div(
            totalShares
        );
        uint256 nativeForbuyBack = swappedNative.mul(buyBackShare).div(
            totalShares
        );
        uint256 nativeForTeam = swappedNative
            .sub(nativeForMarketing)
            .sub(nativeForLiquidity)
            .sub(nativeForCharity)
    ...
    function allowedContracts(
        ...
        for (uint256 i = 0; i < _addresses.length; i++) {
            _allowedContracts[_addresses[i]] = _allowed[i];
        }
    }

    function mintTokens(uint256 _amount) external {
        require(
            _allowedContracts[msg.sender] == true,
    ...
    function _getReflections(uint256 amount) public view returns (uint256) {
        require(amount <= initialSupply, "Must be less than initial supply");
        uint256 totalSupply = totalSupply();
        uint256 _totalReflections = 100 ether;
        // uint256 reflectionPerToken = totalReflections.div(totalSupply);
        uint256 reflectionPerToken = _totalReflections.div(totalSupply);
        return amount.mul(reflectionPerToken);
    }
    ...
    function calculateTaxFee(uint256 _amount) private view returns (uint256) {
        uint256 totalFeesToGive = totalFeesOnBuy.sub(totalFeesOnBuy);
        return _amount.mul(totalFeesToGive).div(10**2);
    }
    ...
    function calculateLiquidityFee(uint256 _amount)
        ...
        returns (uint256)
    {
        return _amount.mul(liquidityFeeOnBuy).div(10**2);
    }
