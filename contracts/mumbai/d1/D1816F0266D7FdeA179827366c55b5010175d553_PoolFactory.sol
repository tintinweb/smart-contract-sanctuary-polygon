...
contract Pool is IPool, Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, NZAGuard {
  ...
  /// @inheritdoc IPool
  function cancelCallBack() external override nonDefaulted onlyPrime returns (bool success) {
  ...
  function dueOf(
    ...
    (due, spreadFee) = dueInterestOf(lender);

    /// @dev Due calculation. due == principal + interest + penalty - spreadFee
    due = due + member.principal;
  ...
  function totalDueInterest() external view override returns (uint256 totalInterest) {
    ...
    for (uint256 i = 0; i < lendersLength; ++i) {
      ...
      (uint256 interest, uint256 spreadAmount) = _dueInterestAtTime(lender, timestamp);

      (uint256 additionalInterest, uint256 additionalSpread, uint256 penalty) = _getOverdueAmounts(
        lender
      );
      totalInterest += interest + spreadAmount + additionalInterest + additionalSpread + penalty;
    }
  }
  ...
  function dueInterestOf(
    ...
    if (isBulletLoan) {
      due = member.totalInterest;

      /// @dev Define the lender's active timestamp and origination fee if any
      uint256 principalPaymentTimestamp = block.timestamp;

      if (_poolCallbacks[lender].isCreated && principalPaymentTimestamp < maturityDate) {
        /// @dev Substract extra interest from the total interest in case of callback
        due -=
          (member.principal * _annualRate(rateMantissa, maturityDate - principalPaymentTimestamp)) /
          1e18;
      }

      (uint256 additionalInterest, , uint256 penalty) = _getOverdueAmounts(lender);

      due += additionalInterest;

      /// @dev SpreadAmount = spreadRate * (interest)
      spreadFee = (spreadRate * due) / 1e18;

      /// @dev Due interest calculation. due == principal + interest + penalty - spreadFee
      due = due + penalty - spreadFee;
    } else {
      uint256 timestamp = getNextPaymentTimestamp();
      ...
      (due, spreadFee) = _dueInterestAtTime(lender, timestamp);

      due += penalty + additionalInterest;
      spreadFee += additionalSpread;
    }
  }
  ...
  function penaltyOf(address lender) public view override returns (uint256 penalty) {
    ...
    (, , penalty) = _getOverdueAmounts(lender);
    return penalty;
  }
  ...
  function _repayTo(address lender) internal returns (bool success) {
    ...
    if (totalFees != 0) {
      require(IERC20Lite(asset).transferFrom(msg.sender, prime().treasury(), totalFees), 'TFL');
    }
    return IERC20Lite(asset).transferFrom(msg.sender, lender, memberDueAmount);
  }
  ...
  function _repayInterestTo(address lender) internal {
    ...
    (uint256 interest, uint256 spreadFee) = _dueInterestAtTime(lender, lastPaidTimestamp);
    ...
    interest += additionalInterest + penalty;
    spreadFee += additionalSpread;
    ...
    if (spreadFee != 0) {
      require(IERC20Lite(asset).transferFrom(msg.sender, prime().treasury(), spreadFee), 'TFL');
    }
    /// @dev Repay interest and penalty if any.
    /// @dev interest == 0 is not possible because of the check above for member.accrualTs
    require(IERC20Lite(asset).transferFrom(msg.sender, lender, interest), 'TFL');
  }
  ...
  /// @dev Calculates penalty fee, interest, spread fee for the penalty period for the lender
  /// @param lender - The address of the lender
  function _getOverdueAmounts(
    address lender
  ) internal view returns (uint256 additionalInterest, uint256 additionalSpread, uint256 penalty) {
    ...
    if (endingDate > startingDate) {
      additionalInterest =
        (member.principal * _annualRate(rateMantissa, endingDate - startingDate)) /
        1e18;

      additionalSpread = (additionalInterest * spreadRate) / 1e18;
      uint256 penaltyRateMantissa = prime().penaltyRate(endingDate - startingDate);

      /// @dev If penalty rate is zero, no penalty fee is charged
      if (penaltyRateMantissa == 0) {
        return (additionalInterest, additionalSpread, 0);
      }

      /// @dev Penalty fee == (penaltyRateForTime * principal)
      /// @dev function callable only if principal is not zero
      penalty = (penaltyRateMantissa * member.principal) / 1e18;
    } else {
  ...
  /// @return initialInterest - The interest amount during maturity
  /// @return spreadAmount - The spread amount
  function _dueInterestAtTime(
    address lender,
    uint256 timestamp
  ) internal view returns (uint256 initialInterest, uint256 spreadAmount) {
    /// @dev Link to member's data struct
    Member storage member = poolMembers[lender];
    ...
    if (timestamp < maturityDate) {
      ...
      initialInterest = member.totalInterest;
    }

    spreadAmount = (initialInterest * spreadRate) / 1e18;

    initialInterest -= spreadAmount;
  }
}
