...
/**
 * @title Splyt Product Factory
 * @dev Create smart contract that will create ERC721 contracts for a given product.
 */
contract ShopXReserveFactory is Ownable {

  address[] public allReserveX;

  event FactoryCreated(address indexed ShopXReserveFactory);
  event ContractCreated(string indexed name, string indexed symbol, address indexed ReserveX, uint index);
  ...
  function createReserveNFT(
    ...
    //bytes memory bytecode = type(ShopXReserveNFT).creationCode;
    ...
    emit ContractCreated(_name, _symbol, address(reserveX), allReserveX.length);
  ...
  _addressArgs[2]: _beneficiaryAddress
  */
...
// ERC2981ContractWideRoyalties.sol source:
// https://github.com/dievardump/EIP2981-implementation

contract ShopXReserveNFT is ERC721Enumerable, ERC2981ContractWideRoyalties, AccessControl, Pausable {
  ...
  uint256 mintPrice;     // Price in ETH required to mint NFTs (in wei)
  ...
  // Events
  event ETHReceive(address indexed _from, uint _amount);
  event NFTClaim(address indexed _claimant, uint indexed _tokenId);
  event PriceChange(uint indexed _price, uint indexed _tokenId);
  ...
  constructor (
    ...
    */
    ...
    // Brand Settings
    beneficiaryAddress = _addressArgs[2];
    ...
    // Note: msg.sender is factory contract.
//    _setupRole(BRAND_ADMIN, msg.sender);
//    _setupRole(SALE_ADMIN, msg.sender);
  ...
  modifier onlyAllow(address _address, bytes32[] calldata _merkleProof) {
    // _address: msg.sender
    ...
    require (valid, "ReserveNFT: Invalid Proof. Valid proof required.");
    _;
  }
 ...
 * @dev Sets the merkle root.
  ...
  function setMaxSupply(uint256 _maxSupply) onlyRole(SALE_ADMIN) external {
  ...
  * @dev function to set the base URI for all token IDs. It is
  ...
  function setTokenURI(uint256 tokenId, string memory _tokenURI) onlyRole(SALE_ADMIN) external {
 ...
 * @dev Returns the number of NFTs that has been claimed by an address so far.
  ...
  /// @inheritdoc	ERC165
  function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721Enumerable, ERC2981Base, AccessControl) returns (bool) {
...
* @dev Mints a token to the msg.sender. It consumes whitelisted supplies.
    */
  function mint(bytes32[] calldata merkleProof) payable public whenNotPaused {
    require(totalSupply() < maxSupply, "NotEnoughNFT: NFT supply not available");
    require(claimed[msg.sender] < mintLimitPerWallet, "ReserveNFT: NFTs already claimed.");
    require(msg.value == mintPrice, "WrongETHAmount: More or less than the required amount of ETH sent.");
...
//    bool valid = MerkleProof.verify(merkleProof, merkleRoot, leaf);
    ...
    emit NFTClaim(msg.sender, currentTokenId);

    // Fee
    payable(shopxAddress).transfer(this.getBalance()*shopxFee/1000);
    // Sweep
  ...
  /**
    ...
    */  
  function mintTo(address to) external whenNotPaused onlyRole(SALE_ADMIN)  {
    require(totalSupply() < maxSupply, "NotEnoughNFT: NFT supply not available");
    ...
    emit NFTClaim(to, currentTokenId);
  }

  function burn(uint256 tokenId) onlyRole(SALE_ADMIN) external {
    _burn(tokenId);
  }
