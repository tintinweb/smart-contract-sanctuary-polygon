...
contract BattleGame is ERC721URIStorage{
    ...
    function mint()public{
        ...
        tokenIdtoStatistics[t_Id]=statistics(0,0,0,0);
    ...
    function train(uint _tokenId)public{
        ...
        statistics storage warrior=tokenIdtoStatistics[_tokenId];
        warrior.level++;
        warrior.attack=uint(keccak256(abi.encodePacked(msg.sender)))%100;
        warrior.defense=uint(keccak256(abi.encodePacked(block.timestamp,_tokenId)))%100;
        warrior.speed=uint(keccak256(abi.encodePacked(block.timestamp,warrior.level)))%100;
