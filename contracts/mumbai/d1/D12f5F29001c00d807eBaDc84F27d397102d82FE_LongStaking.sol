...
contract LongStaking is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    using SafeERC20Upgradeable for IUniswapPair;
    ...
    function add(uint256 _rootPid, bool _withUpdate) public onlyOwner {
        ...
        IERC20Upgradeable tokenA = IERC20Upgradeable(_lpToken.token0());
        IERC20Upgradeable tokenB = IERC20Upgradeable(_lpToken.token1());
        tokenA.approve(address(swapRouter), type(uint256).max);
        tokenB.approve(address(swapRouter), type(uint256).max);
        _lpToken.approve(address(swapRouter), type(uint256).max);
    ...
    function updatePool(uint256 _pid) public returns (PoolInfo memory pool) {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            if (lpSupply > 0) {
                ...
                pool.accNSDXPerShare = pool.accNSDXPerShare + (nsdxReward * 1e12 / lpSupply);
            }
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accNSDXPerShare = accNSDXPerShare + (nsdxReward * 1e12 / lpSupply);
        }
        return user.amount * accNSDXPerShare / 1e12 - user.rewardDebt;
    }
    ...
    function deposit(
        ...
        PoolInfo storage pool = poolInfo[_pid];
        IERC20Upgradeable tokenA = IERC20Upgradeable(pool.lpToken.token0());
        IERC20Upgradeable tokenB = IERC20Upgradeable(pool.lpToken.token1());
    ...
    function deposit(
        ...
        user.rewardDebt = user.amount * pool.accNSDXPerShare / 1e12;
    ...
    function _getReward(
        ...
        uint256 pending = _user.amount * _pool.accNSDXPerShare / 1e12 - _user.rewardDebt;
        if (pending > 0) {
