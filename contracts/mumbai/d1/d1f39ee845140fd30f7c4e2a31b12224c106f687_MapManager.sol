...
contract MapManager is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function stakeBeast(uint16 beastId) external {
        IBeast(beastAddress).transferFrom(msg.sender, address(this), beastId);
        _beastStates[beastId] = BeastState({
            representation: IBeast(beastAddress).getBeastRepresentation(beastId),
            groupId: 0,
            beastId: beastId
        });
        _playerBeasts[msg.sender].add(beastId);
    }

    function unstakeBeast(uint16 beastId) external onlyBeastOwner(beastId) {
    ...
    function joinGroup(uint240 groupId, uint16 beastId) external onlyBeastOwner(beastId) {
    ...
    function leaveGroup(uint16 beastId) external onlyBeastOwner(beastId) {
        ...
        if (!_isAnyBeastFromPlayer(groupBeasts.values(), msg.sender)) {
            _playerGroups[msg.sender].remove(groupId);
        }
    }

    function offerTears(uint240 groupId, uint256 amount) external {
        IRagnarokERC20(tearAddress).mint(msg.sender, amount);
    ...
    function startGroup(uint240 groupId) external {
        ...
        IRagnarokERC20(skullAddress).burn(map.price * groupSize);
        uint256 timeReduction = ((_groupTears[groupId] / 1 ether) * 60);
        uint256 maxReduction = map.duration / 3;
        timeReduction = timeReduction > maxReduction ? maxReduction : timeReduction;
        group.endTime = block.timestamp + map.duration - timeReduction;
        group.state = GroupState.STARTED;
    }

    function claimReward(uint16 beastId) external onlyBeastOwner(beastId) {
        BeastState memory beastState = _beastStates[beastId];
        uint240 groupId = beastState.groupId;
        require(groupId != 0, "not in group");
        Group storage group = _groups[groupId];
        Map memory map = _maps[group.mapId];
        require(group.state != GroupState.IDLE, "not started");
        console.log("endTime", group.endTime);
        ...
        uint256[] memory playerBeasts = _playerBeasts[msg.sender].values();
        bool isRelevant = false;
        for (uint256 index = 0; index < playerBeasts.length; index++) {
            if (_beastStates[uint16(playerBeasts[index])].groupId == groupId) {
                isRelevant = true;
                break;
            }
        }
    ...
    function getGroups(address player) external view returns (PublicGroup[] memory) {
    ...
    modifier onlyBeastOwner(uint16 beastId) {
        require(_playerBeasts[msg.sender].contains(beastId), "E02: Only the owner can exacute this action");
        _;
    }
...
interface IBeast {
    ...
    function giveExperience(uint256 beastId, uint32 experience)
        external
        returns (uint256);
...
library SharedLib {
    ...
    function classToNumberRepresentation(uint8 faction, uint8 class) internal pure returns (uint16) {
        if (class == 0) {
            return 0;
        }
    ...
    function numberRepresentationToClass(uint16 numberRepresentation) internal pure returns (uint8) {
        return uint8(numberRepresentation % FACTIONS);
    }
}
