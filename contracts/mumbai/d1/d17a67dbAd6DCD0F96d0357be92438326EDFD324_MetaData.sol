...
error SecondChance_RaffleTokenCountMismatch(uint256 poolId);
...
interface ISecondChance {
    ...
    event SecondChance_UserRegisteredIntoPool(
...
contract MetaData is IMetaData, VRFConsumerBaseV2, Ownable {
    ...
    ISecondChance private s_secondChance;
    ...
    function fulfillRandomWords(
        ...
        if (
            ...
            (bool shouldOpenPool, uint256 poolId) = s_secondChance
                .needsToOpenPool(s_scratchedOffCount);

            if (shouldOpenPool) {
                s_secondChance.openPrizePool(poolId, block.timestamp + 4 hours);
        ...
        if (
            ...
            uint256 prizeIndex = randomWords[0] % s_prizes.length;
