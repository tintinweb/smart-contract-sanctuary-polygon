...
interface IProxyNFT {
     ...
     *
     */
    event Bundled(uint256 proxyTokenID, Delegate[] delegate, address ownerAddress);

    /**
     * @dev Emitted when one or more delegate NFTs are removed from a Proxy NFT 
     */
    event Unbundled(uint256 proxyTokenID, Delegate[] delegate);

    /**
     * @dev Accepts the tokenId of the Proxy NFT and returns an array of delegate token data
     */
    function delegateTokens(uint256 proxyTokenID) external view returns (Delegate[] memory);
     ...
     */
    function unBundle(Delegate[] memory delegate, uint256 proxyTokenID) external;
     ...
     */
    function bundle(Delegate[] memory delegate, uint256 proxyTokenID) external;
...
// File: contracts/MetalinqNFT.sol
    ...
    AccessControl
{
    ...
    /// @dev maps wearableId with details
    mapping(uint256 => Wearable) public wearablesData;
    /// @dev maps wearableId with delegates
    mapping(uint256 => Delegate[]) public wearableDelegates;
    /// @dev maps tokenId with  wearableId
    mapping(uint256 => uint256) private _proxyToWearableId;
    /// @dev maps proxyTokenId with its wearable price by Id
    ...
    mapping(address =>mapping(uint256 => uint256)) public wearableBalances;

    modifier onlyOwner() {
        _checkOwner();
        _;
    }
    event Mint(uint256 proxyTokenID, uint256 wearableId, address minter);
    ...
    function setDefaultRoyaltyPercentage(uint256 royaltyPercent)
        external
        onlyOwner
    {
    ...
    function setWearableNFTsData(
        ...
        for (
            ...
            uint256 wearableId = currentWerableId.add(1);
            ...
            wearableNFTsCount = wearableNFTsCount.add(1);
            currentWerableId = wearableId;
        }
    }
    ...
    function mint(uint256 wearableId) external payable nonReentrant {
    ...
    function _distributeFees() internal {
        ...
        uint256 mintCost = msg.value.sub(commissionCost);
    ...
    function _beforeTokenTransfer(
        ...
        super._beforeTokenTransfer(from, to, tokenId);
        uint256 wearableId = _proxyToWearableId[tokenId];
        if(from != address(0))
            wearableBalances[from][wearableId] = wearableBalances[from][wearableId].sub(1);
        wearableBalances[to][wearableId] = wearableBalances[to][wearableId].add(1);
    }
