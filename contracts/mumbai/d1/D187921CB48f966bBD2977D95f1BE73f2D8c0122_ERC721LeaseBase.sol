...
function isLeaseAvailable(uint256 _tokenId, uint256 _start,
    ...
    function lease(uint256 _tokenId, uint256 _start, uint256 _end) public {
        ...
        if (leaseGood) {
          ...
          if (startDate == _oldStart) {
              console.log(1);
            lease(_tokenId, _end, oldEnd);
            emit Leased(_tokenId, msg.sender, _end, oldEnd);
          } else if (endDate == _oldEnd) {
              console.log(2);
            lease(_tokenId, oldStart, _start);
            emit Leased(_tokenId, msg.sender, oldStart, _start);
          } else {
              console.log(3);
              lease(_tokenId, oldStart, _start);
              lease(_tokenId, _end, oldEnd);
