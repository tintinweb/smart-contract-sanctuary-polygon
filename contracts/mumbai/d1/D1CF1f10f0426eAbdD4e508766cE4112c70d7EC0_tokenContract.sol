    ...
    ReentrancyGuard
{
    ...
    uint256 private _totalSupplyLimit = 1000 * (10**6) * 10** decimals();  // 1b tokens for distribution
    ...
    error Total_Supply_Limit_Reached();
    ...
    /**
     * @dev To send money to Contract.
     * Requirements:
     *      Contract must be unpaused.
     */
    function receiveMoney() whenNotPaused public payable {
    }
    ...
    /**
     * @dev To withdraw All balance from smart Contract.
     * Requirements:
     *      Contract must be unpaused.
     *      Caller must be owner.
     */
    function withdrawMoney() whenNotPaused public onlyOwner {
        ...
        public 
    {
        ...
        uint256 bnbAmount = (_amount * (10 ** decimals()) / 1000); // 1 BNB = 1000 ABZ tokens, adjust as needed
        uint256 teamTaxFee = ((bnbAmount * teamTax()) / 100);
        uint256 marketingTaxFee = ((bnbAmount * marketingTax()) / 100);
        ...
        if ((_msgSender().balance) < (bnbAmount + teamTaxFee + marketingTaxFee))
        {
            revert Not_Enough_Balance_And_Tax();
        }

        if ((totalSupply() + _amount)  > _totalSupplyLimit)
        {
            revert Total_Supply_Limit_Reached();
        }
    ...
    function sellToken(
        ...
        payable
    {
        uint256 bnbAmount = (_amount / 1000) * (10 ** decimals()); // 1 ABZ token = 0.001 ETH, adjust as needed
        uint256 teamTaxFee = ((bnbAmount * _teamTax) / 100) ;
        uint256 marketingTaxFee = (bnbAmount * _marketingTax) ;
        ...
        if (_amount > getRemainingSellLimit(_msgSender()))
        {
            revert Sell_Limit_Exceeded();
        }

        if (lastSellTimestamp[_msgSender()] + lockTime() > block.timestamp)
        {
            revert Locked_For_Selling();
        }

        sellLimits[_msgSender()] -= _amount;
        lastSellTimestamp[_msgSender()] = block.timestamp;
