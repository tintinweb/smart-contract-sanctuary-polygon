...
contract nftDrop is ERC721Drop {
    ...
    // Define a mapping to keep track of claimed token IDs
    mapping(uint256 => bool) public claimedTokens;

    // uint256 public constant MAX_TOKENS = 50; // Maximum number of NFTs in your project.
    // uint256 private nonce = 0; // Nonce for pseudo-random number generation.
    ...
    function viewBalance(address recipient) public view returns (uint256) {
        require(
            ERC721A(externalContractAddress).balanceOf(recipient) > 0,
            "User does not own a NFT in the external contract"
        );

        return ERC721A(externalContractAddress).balanceOf(recipient);
    }

    function claimExternalContract(
        address _receiver,
        uint256 _quantity
    ) public payable {
        require(
            ERC721A(externalContractAddress).balanceOf(_receiver) > 0,
            "User does not own a NFT in the external contract"
        );
        require(msg.value >= 0.1 ether, "provided amount is not correct");
