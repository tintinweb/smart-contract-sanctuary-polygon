...
contract SupplyChain {
    ...
    function getOrderbyContainerID(uint256 _containerID) public view returns (string memory, string memory, string memory, string memory, uint256, uint256, uint[] memory, uint256[] memory){
        ...
        return (thisContainer.dateOfDept, thisContainer.placeOfDept, thisContainer.expectArrivalDate, thisContainer.expArrivalLocation, thisContainer.vessel, thisContainer.voyage, thisContainer.UPC_ShippedCartons, thisContainer.recievedCartoons);
    } 
    function containerUnload(uint256 _containerID, string memory _dateUnloaded, string memory _locationUnloaded, uint256[] memory _recievedCartoons ) public {
        OrderDetails storage thisContainer = containerDetail[_containerID];
        require(thisContainer.UPC_ShippedCartons.length == _recievedCartoons.length,"Please recived cartoons for each");
        thisContainer.dateUnloaded = _dateUnloaded;
        thisContainer.locationUnloaded = _locationUnloaded;
        thisContainer.recievedCartoons = _recievedCartoons;
    }
}
