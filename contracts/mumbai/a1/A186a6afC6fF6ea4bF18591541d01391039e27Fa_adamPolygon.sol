...
// File: contracts/lib/IERC721Metadata.sol
...
// File: contracts/tokens/adamPolygon.sol
...
/**
 * @title adamPolygon is adam NFT Polygon version
 */
contract adamPolygon is IFxERC721, ERC721 {
    ...
    function setupMetaData(string memory _name, string memory _symbol) private {
    ...
    function _setTokenURI(uint256 tokenId, string memory newValue) private {
    ...
    /*
    function setTokenURIByOwner(uint256 tokenId, string memory newValue) external onlyOwner {
        // tokenId's existence is checked in _setTokenURI(uint256,string)
        require(_exists(tokenId), "nonexistent token");
        _tokenURIs[tokenId] = newValue;
    }
