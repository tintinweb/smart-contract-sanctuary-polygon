...
contract IITGoaGraduationSBT is ERC721URIStorage {
    ...
    constructor() ERC721("GraduationAffiliation_IITGoaSbt", "SBT") {
        admin = _msgSender();
    }
    ...
    function mint(address _to, studentSbt memory _studentSbt, string memory _tokenURI)
        external
        onlyAdmin()
    {
        _mint(_to, SBT_ID);
    ...
    function transferOwnership(address _newAdmin) external onlyAdmin() {
        ...
        admin = _newAdmin;
        emit adminUpdate(oldAdmin,_newAdmin);
    }
    function updateSbtdata(uint256 _sbtId, studentSbt memory _studentSbt) external onlyAdmin(){
        studentSbtInfo[_sbtId] = _studentSbt;
    }

    function burn(uint256 _sbtId) external onlyAdmin() {
        _burn(_sbtId);
