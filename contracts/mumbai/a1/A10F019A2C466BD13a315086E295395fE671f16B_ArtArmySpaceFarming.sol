...
// File contracts/ArtArmySpace.sol
...

{
    ...
    */
     ...
     */
    function mint(address to, string memory URI, address _artistWallet, uint256 _royaltyInBasisPoints, bool metaverse) public virtual {
        require(hasRole(MINTER_ROLE, _msgSender()), "ERC721PresetMinterPauserAutoId: must have minter role to mint");
        require(metaverse, "ERC721: You need to confirm this is a metaverse");
        // We cannot just use balanceOf to create the new tokenId because tokens
        // can be burned (destroyed), so we need a separate counter.
        require(_tokenIdTracker.current() < 999, "We can never mint more than 999 spaces in the metaverse of art.army");
...
// File contracts/ArtArmySpaceFarming.sol
...
contract ArtArmySpaceFarming is Ownable  {
    ...
    ArtArmySpace public _nftContract;
    address _sellerAddress;
    event FarmingEvent(uint256 amount);

    string private _contractName;

    mapping (uint256 => uint256) private _rarity;

    uint256 _lastFarming = 0;
    uint256 _farmingPerDay;

    /**
     * @dev Constructor sets token that can be received
     */
    constructor (string memory contractName, IERC20 token, ArtArmySpace nftContract, uint256 farmingPerDay, address sellerAddress) {

        _contractName = contractName;
        _token = token;
        _nftContract = nftContract;
        _farmingPerDay = farmingPerDay;
    ...
    */
    ...
    /**
   * @dev Returns the last Time farming event.
   */
    function getLastFarming() public view returns (uint256) {
        return _lastFarming;
    }

    /**
    * @dev Returns the farming per day.
    */
    function getFarmingPerDay() public view returns (uint256) {
        return _farmingPerDay;
    }
    ...
    */
    ...
    /**
    * @dev Returns the NFT Token address.
    */
    function getNft() public view returns (ArtArmySpace) {
        return _nftContract;
    }
     ...
     */
    function getRarity(uint256 tokenId) public view virtual returns (uint256) {
    ...
    */
    ...
    function setFarmingPerDay(uint256 farmingPerDay) external returns (bool){
    ...
    function setRarity(uint256 tokenId, uint256 rarity) external returns (bool){
        require(address(_msgSender()) == address(owner()), "Only the owner can call this function");
    ...
    */
    ...
    function farming() external returns (bool){

        require(address(msg.sender) == address(owner()), "Only the owner can call this function");

        uint256 contractBalance = _token.balanceOf(address(this));

        require(contractBalance >= _farmingPerDay, "The contract needs to have the same or more amount of tokens");

        require( (24*60*60) - 120 + _lastFarming < block.timestamp, "Only is possible to do the farm in 24h");

        uint256 totalSupply = _nftContract.totalSupply();

        uint256 unitsToDistribute = 0;


        for(uint i=0; i < totalSupply; i++){
            uint256 tokenId = _nftContract.tokenByIndex(i);
            unitsToDistribute += _rarity[tokenId];
        }

        uint256 amountToFarming = _farmingPerDay / unitsToDistribute;

        for(uint i=0; i < totalSupply; i++){
            uint256 tokenId = _nftContract.tokenByIndex(i);
            address holder = _nftContract.ownerOf(tokenId);

            if(holder != _sellerAddress && holder != address(0) && _rarity[tokenId] > 0){
                uint256 amount = _rarity[tokenId] * amountToFarming;
                _token.transfer(address(holder), amount);
            }

        }

        _lastFarming = block.timestamp;
