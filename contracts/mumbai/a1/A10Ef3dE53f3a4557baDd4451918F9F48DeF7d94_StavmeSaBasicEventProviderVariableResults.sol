...
import "../lib/StavmeSaEventProviderVariableResults.sol";

contract StavmeSaBasicEventProviderVariableResults is
    StavmeSaEventProviderVariableResults
{
...
abstract contract StavmeSaEventProviderVariableResults is StavmeSaEventProviderBase {
    ...
    event EventResultAdded(uint256 eventId, EventResult result, bytes data);

    mapping (uint256 => EventResult[]) __resultsByEventId;

    mapping (uint256 => mapping (EventResult => bool)) __resultsExistenceByEventId;

    function _addResults(uint256 eventId, AddResultDTO[] memory resultsDto) internal {
        ...
        for (uint256 i = 0; i < numResults; i++) {
            ...
            if (__resultsExistenceByEventId[eventId][result]) continue;

            __resultsExistenceByEventId[eventId][result] = true;
            __resultsByEventId[eventId].push(result);

            emit EventResultAdded(eventId, result, resultDto.data);
        }
    }
    ...
    function _isPossibleEventResult(
        ...
        return __resultsExistenceByEventId[eventId][result];
    }

    function _getAllPossibleEventResults(
        uint256 eventId
    ) internal view virtual override returns (EventResult[] memory) {
        return __resultsByEventId[eventId];
    }
}
