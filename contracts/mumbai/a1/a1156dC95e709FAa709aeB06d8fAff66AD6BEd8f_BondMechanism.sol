    ...
    ERC1155HolderUpgradeable
{
    ...
    CountersUpgradeable.Counter private _bondIds;

    address public bondFeeReceiverAddress;

    uint256 public bondMintingLimit;

    mapping(address => uint256[]) public ownersTokenList;
    mapping(uint256 => string) private _tokenURIs;
    mapping(uint256 => uint256) public basePriceOfBonds;
    ...
    /**@notice intializer for proxy - replacement to constructor
        @param _feeReceiverAddress fee receiver address
    */
    function initialize(
        address _feeReceiverAddress
    ) external initializer {
        bondFee = 10;
        bondFeeReceiverAddress = _feeReceiverAddress;
        __ERC1155_init("0x00");
        __Ownable_init();
    ...
    */
    function mintBonds(
        ...
        require(
            _amount <= bondMintingLimit,
            "mintBonds: amount cannot exceed bond minting limit"
        );
        _beforeTokenMint(_receiver, _amount, _uri);

        _bondIds.increment();
        uint256 _bondId = _bondIds.current();
        _setURI(_bondId, _uri);
        _mint(address(this), _bondId, _amount, "");
        safeTransferFrom(
            address(this),
            bondFeeReceiverAddress,
            _bondId,
            bondFee,
            ""
        );
    ...
    function setBondFee(uint256 _updatedBondFee) external onlyOwner {
        bondFee = _updatedBondFee;
    }

    function setBondFeeReceiverAddress(
        address _newReceiverAddress
    ) external onlyOwner {
        bondFeeReceiverAddress = _newReceiverAddress;
    }

    function setBondMintingLimit(uint256 _newLimit) external {
        bondMintingLimit = _newLimit;
    }
