...
contract Pool is Initializable, AccessControlEnumerableUpgradeable, IPool {
    bytes32 private constant OPENER_ROLE = keccak256("OPENER_ROLE");
    uint256 public poolId;
    uint128 public openStartTimestamp;
    uint128 public amountDistributedPerOpen;
    IPoolFactory public factory;
    IContractOpenablePack public packContract;

    function initialize(IPoolFactory _factory, IContractOpenablePack _packContract, uint256 _poolId, uint128 _openStartTimestamp, uint128 _amountDistributedPerOpen) public initializer {
        factory = _factory;
    ...
    function _isPackCreated() internal view returns(bool) {
        return packContract.created(poolId);
    }
    ...
    function onERC721Received(
        ...
        from;
        data;
        ITokenBundle.Token[] memory token;
        uint256[] memory numOfRewardUnits;
        token[0] = ITokenBundle.Token(
            operator,
            ITokenBundle.TokenType.ERC721,
            tokenId,
            1
        );
        numOfRewardUnits[0] = 1;

        createOrAddToPack(token, numOfRewardUnits);
    ...
    function onERC1155Received(
        ...
        ITokenBundle.Token[] memory token = new ITokenBundle.Token[](1);
        uint256[] memory numOfRewardUnits = new uint256[](1);
        numOfRewardUnits[0] = hasData ? abi.decode(data, (uint256)) : value;

        require(numOfRewardUnits[0] != 0, "!unit");
        require(value % numOfRewardUnits[0] == 0, "!mod");
        token[0] = ITokenBundle.Token(
            msg.sender,
    ...
    function onERC1155BatchReceived(
        ...
        ITokenBundle.Token[] memory token = new ITokenBundle.Token[](ids.length);
        uint256[] memory numOfRewardUnits = hasData ? abi.decode(data, (uint256[])) : values;
        require(numOfRewardUnits.length == token.length, "!len");

        for (uint256 i = 0; i < ids.length; i++) {
            require(numOfRewardUnits[i] != 0, "!unit");
            require(values[i] % numOfRewardUnits[i] == 0, "!mod");

            token[i] = ITokenBundle.Token(
                msg.sender,
                ITokenBundle.TokenType.ERC1155,
                ids[i],
                values[i]
    ...
    function approveToken(ITokenBundle.Token memory t) internal {
        if (t.tokenType == ITokenBundle.TokenType.ERC1155) {
            IERC1155Upgradeable(t.assetContract).setApprovalForAll(address(packContract), true);
            return;
        }
        if (t.tokenType == ITokenBundle.TokenType.ERC721) {
            IERC721Upgradeable(t.assetContract).approve(address(packContract), t.tokenId);
            return;
        }
    }

    function createOrAddToPack(
        ITokenBundle.Token[] memory token,
        uint256[] memory numOfRewardUnits
    ) internal {
        for (uint256 i=0; i < token.length; i++) {
            approveToken(token[i]);
        }
        if (!_isPackCreated()) {
    ...
    function openPack(uint256 amount, address recipient) external onlyRole(OPENER_ROLE) {
        packContract.openPack(poolId, amount, recipient);
    }
...
contract PoolFactory is Initializable, AccessControlEnumerableUpgradeable, TimeboundEIP712Upgradeable, UUPSUpgradeable, IPoolFactory, IContractFactory {
    ...
    bytes32 internal constant TYPEHASH =
        keccak256(abi.encodePacked(
            "OpenPackRequest(BaseRequest baseRequest,uint256 poolId,uint256 amountToOpen,address recipient)",
            BASEREQUESTTYPE
        ));
    ...
    function createPool(uint128 _openStartTimestamp, uint128 _amountDistributedPerOpen) public onlyRole(DEFAULT_ADMIN_ROLE) returns(address _poolAddress) {
        uint256 _poolId = nextPoolId;
        require(_amountDistributedPerOpen > 0, "!amt");
        ...
        packContract.grantRole(MINTER_ROLE, _poolAddress);
    ...
    function deployProxyByImplementation(address _impl, bytes calldata _data, bytes32) external onlyRole(DEFAULT_ADMIN_ROLE) returns (address poolAddr) {
        ...
        (,,,,uint128 _openStartTimestamp, uint128 _amountDistributedPerOpen) = abi.decode(_data, (bytes4, address, address, uint256, uint128, uint128));
        poolAddr = createPool(_openStartTimestamp, _amountDistributedPerOpen);
        emit ProxyDeployed(_impl, poolAddr, _msgSender());
    }

    function removePool(uint256 _poolId, address _recipient) external onlyRole(DEFAULT_ADMIN_ROLE) {
        packContract.releasePack(_poolId, _recipient);
    }
    ...
    function onERC1155Received(
        ...
        require(_msgSender() == address(packContract), "!pack");
        return IERC1155ReceiverUpgradeable.onERC1155Received.selector;
    }
    ...
    function openPackWithSignature(OpenPackRequest calldata _req, bytes calldata _sig) external {
        ...
        packContract.openPack(_req.poolId, _req.amountToOpen, _req.recipient);
    }
...
import "@thirdweb-dev/contracts/extension/interface/ITokenBundle.sol";
...
import "@thirdweb-dev/contracts/extension/interface/IPermissionsEnumerable.sol";
import "@openzeppelin/contracts-upgradeable/token/ERC1155/IERC1155Upgradeable.sol";

interface IContractOpenablePack is ITokenBundle, IPermissionsEnumerable, IERC1155Upgradeable {
    /**
    ...
    function createPack(
        ...
        uint128 amountDistributedPerOpen,
        address recipient
    ) external payable returns (uint256 packTotalSupply);
    ...
    function addPackContents(
        ...
        uint256[] calldata numOfRewardUnits,
        address recipient
    ) external payable returns (uint256 packTotalSupply, uint256 newSupplyAdded);
     ...
     */
    function openPack(uint256 packId, uint256 amountToOpen, address recipient) external returns (Token[] memory);

    function created(uint256 packId) external view returns(bool);
    function releasePack(uint256 packId, address recipient) external;
...
interface IPool is IERC721ReceiverUpgradeable, IERC1155ReceiverUpgradeable {
    function poolId() external view returns(uint256);
    function factory() external view returns(IPoolFactory);
    // function transferPackTo(uint256 poolId, address receiver, uint256 amount) external;
}
...
interface IPoolFactory is ITimeboundEIP712, IERC1155ReceiverUpgradeable {
    event PoolCreated(uint256 indexed poolId, address addr);
    ...
    function createPool(uint128 _openStartTimestamp, uint128 _amountDistributedPerOpen) external returns(address);
    function removePool(uint256 _poolId, address _recipient) external;
}
...
interface ITimeboundEIP712 {
     ...
     *
     *  @param payload The payload / mint request.
     *  @param signature The signature produced by an account signing the mint request.
     *
     *  returns (success, signer) Result of verification and the recovered address.
     */
    function verify(bytes memory payload, bytes calldata signature)
        external
        view
        returns (bool success, address signer);
    ...
    //  *  @param signature The signature produced by an account signing the mint request.
    //  */
    // function mintWithSignature(MintRequestWithParams calldata req, bytes calldata signature)
    //     external
    //     payable
