...
contract CorporationAvatars is ERC1155SupplyUpgradeable, OwnableUpgradeable, IAssetManager, ITrustedMintable {
    ...
    function _safeMint(
        ...
        uint256 mintedSupply = totalSupply(_tokenId);
        uint256 maxSupply = s_maxSupplies[_tokenId];
        if (mintedSupply + _amount <= maxSupply) {
            _mint(_to, _tokenId, _amount, "");
            emit MintedTo(_to, _tokenId, _amount);
        }
        else {
            uint256 availableToMint = maxSupply - mintedSupply;
            uint256 leftToBeMinted = _amount - availableToMint;
            if (availableToMint > 0) {
                _mint(_to, _tokenId, availableToMint, "");
                emit MintedTo(_to, _tokenId, _amount);       
            }

            uint256 nextTokenId;
            uint256 nextAvailableToMint;
            uint256 nextMintedSupply;
            uint256 nextMaxSupply;
            bool exit;
            for(uint256 i=0; i<s_tokensByRarity.length; i++) {
                if (exit) break;
                if (s_tokensByRarity[i] == _tokenId) {
                    while (leftToBeMinted > 0) {
                        nextTokenId = s_tokensByRarity[i+1];
                        nextMintedSupply = totalSupply(nextTokenId);
                        nextMaxSupply = s_maxSupplies[nextTokenId];
                        nextAvailableToMint = nextMaxSupply - nextMintedSupply;
                        if (nextAvailableToMint > leftToBeMinted) {
                            _mint(_to, nextTokenId, leftToBeMinted, "");
                            emit MintedTo(_to, nextTokenId, leftToBeMinted);
                            exit = true;
                            break;     
                        }
                        else{
                            leftToBeMinted = leftToBeMinted - nextAvailableToMint;
                            _mint(_to, nextTokenId, nextAvailableToMint, "");
                            i++;
                            emit MintedTo(_to, nextTokenId, leftToBeMinted);       
                        }
                    }
