...
contract Keld {
    ...
    uint256 private constant initDayNewbies = 9;
    uint256 private constant incInterval = 1;
    ...
    uint256 private constant bonusPercents = 300;
    ...
    uint256[5] private levelTeam = [0, 0, 0, 30, 150];
    uint256[5] private levelInvite = [0, 0, 0, 10000e6, 15000e6];
    ...
    uint256[3] private REFERRAL_PERCENTS = [3, 1, 1]; 
    ...
    function refPayout(address _userAddr, uint256 _amount) internal {
            ...
            for (uint256 i = 0; i < REFERRAL_PERCENTS.length; i++) {
                ...
                uint256 amount = _amount.mul(REFERRAL_PERCENTS[i]).div(100);                     
                upRewards.referralBonus = upRewards.referralBonus.add(amount);
    ...
    function withdraw() external {
        ...
        uint256 rewardsStatic = userRewards.statics.add(userRewards.invited).add(userRewards.bonusReleased).add(userRewards.predictWin).add(userRewards.referralBonus);
        ...
        uint256 leftReward = rewardsStatic.add(userRewards.l5Released).sub(predictPoolFee);
        ...
        if(!isFreezing) _setFreezeReward();
        emit Withdraw(msg.sender, 0, predictPoolFee, split, withdrawable);
    }
