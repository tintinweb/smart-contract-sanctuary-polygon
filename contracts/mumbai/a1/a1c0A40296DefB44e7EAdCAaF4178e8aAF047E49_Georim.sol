...
contract Georim is ERC721, ReentrancyGuard {
    ...
    // Tile Structure
    mapping(string => mapping(string => mapping(string => Tile))) tileDetails;
    // mapping(string => mapping(string => mapping(string => bool))) isMinted;

    /**** Custom Error ****/
    error isAlreadyMinted(string x, string y, string zoom);
    ...
    function safeMint(
        ...
        address payable buyer = payable(msg.sender); // Buyer
        uint256 amount = msg.value; // Only 1 Matic
        require(amount == 0.5 ether, "1 Matic Only"); // Check for the 1 Matic
