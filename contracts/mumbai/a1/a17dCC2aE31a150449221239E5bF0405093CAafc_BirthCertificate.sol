...
// on chain NFT Certificate
  function generateCertificate(uint256 tokenId) public returns(string memory) {
    bytes memory svg = abi.encodePacked(
      ...
      '<text x="50%" y="50%" class="base" dominant-baseline="middle" text-anchor="middle">', "Date of Birth: ",getDateofBirth(tokenId),'</text>',
      '<text x="50%" y="50%" class="base" dominant-baseline="middle" text-anchor="middle">', "Location: ",getLocation(tokenId),'</text>',
      '<text x="50%" y="50%" class="base" dominant-baseline="middle" text-anchor="middle">', "Hospital Location: ",getLocationofBirth(tokenId),'</text>',
      '<text x="50%" y="50%" class="base" dominant-baseline="middle" text-anchor="middle">', "Voter Eligibity: ",getVoterEligibity(tokenId),'</text>',
      '</svg>'
    );
  ...
  // fetch data from certificate struct to be used at on chain art SVG
  ...
  function getName(uint256 tokenId) public view returns(string memory) {
    string memory name;
    (name,,,,) = getCertificate(tokenId);
    return name;
  }

  function getLocationofBirth(uint256 tokenId) public view returns(string memory) {
    string memory locationofBirth;
    (,locationofBirth,,,) = getCertificate(tokenId);
    return locationofBirth;
  }

  function getLocation(uint256 tokenId) public view returns(string memory) {
    string memory location;
    (,,location,,) = getCertificate(tokenId);
    return location;
  }  

  function getDateofBirth(uint256 tokenId) public view returns(string memory) {
    uint256 dateofBirth;
    (,,,dateofBirth,) = getCertificate(tokenId);
    return dateofBirth.toString();
  }

  function getVoterEligibity(uint256 tokenId) public view returns(string memory) {
    uint256 voterEligibity;
    (,,,,voterEligibity) = getCertificate(tokenId);
    return voterEligibity.toString();
  }
  ...
  function checkVoteEligibity(uint256 _dateofBirth) view public returns(uint256) {
    ...
    uint256 ageRequirement = 18;
    uint256 calculateAge = (_now - _dateofBirth) / 60 / 60 / 24 / 30 / 12; // for now disregard leap year
    if(calculateAge >= ageRequirement) {
      eligibity = 1;
    }
    return eligibity;
  }
  ...
  function mint(address _address, string memory _name, string memory _locationofBirth, string memory _location, uint256 _dateofBirth) public onlyOwner {
    ...
    _tokenIds.increment(); // start the counter at 1 instead of 0
    uint256 newTokenId = _tokenIds.current();
    _safeMint(_address, newTokenId);
    uint256 _voterEligibity = checkVoteEligibity(_dateofBirth);
    Certificate memory newCertificate = Certificate(_name, _locationofBirth, _location, _dateofBirth, _voterEligibity);
    tokenIdtoCertificate[newTokenId] = newCertificate;
