...
contract TicketNFT is ERC721Enumerable, Ownable, Pausable, Decode {
    ...
    function mintBySignature(bytes memory _signurate, bytes32 _mHash, uint256 _price, uint256[] memory _tokenIds, uint256 _timeout, string memory _uri) public {
        require(!paused(), "Pausable: token transfer while paused");
        require(_tokenIds.length > 0, "ERC721: token empty!!!");
        require(block.timestamp <= _timeout, "ERC721: time out!!!");
        ...
        require(VerifyNonce(Strings.toHexString(uint256(uint160(msg.sender)), 20), _price, tokenIds, _timeout, _uri, _mHash), "Hash validation error.");
    ...
    function VerifyNonce(string memory _sender, uint256 _price, string memory _tokenId, uint256 _timeout, string memory _nonce, bytes32 _msgHash) public pure returns (bool) {
        bytes memory prefix = abi.encodePacked("\x19Ethereum Signed Message:\n", Strings.toString(abi.encodePacked(_sender, Strings.toString(_price), _tokenId, _timeout, _nonce).length));
        bytes32 prefixedHashNonce = keccak256(abi.encodePacked(prefix, abi.encodePacked(_sender, Strings.toString(_price), _tokenId, _timeout, _nonce)));
