...
contract Ghost is IFlashLoanReceiver {
  ...
  function executeOperation(
    ...
    for(uint i=0; i<_DebtTokenBalance.length; i++) {
      ...
      if (_DebtTokenBalance[i].stableDebtTokenBalance != 0) {
        ...
        AaveV3Mumbai.POOL.repay(_DebtTokenBalance[i].tokenAddress, _DebtTokenBalance[i].stableDebtTokenBalance, uint256(1), _sender);
      }
      // Repay VariableDebt
  ...
  function merge(
    ...
    bytes memory params = abi.encode(_fromAccount, msg.sender, _borrowToken, _supplyToken);
