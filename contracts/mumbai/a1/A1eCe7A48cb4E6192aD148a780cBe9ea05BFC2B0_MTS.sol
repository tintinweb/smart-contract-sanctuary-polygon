...
contract MTS is ERC20, ERC20Pausable, ERC20Burnable, SecurityBase {
    ...
    function isBlackListed(address _user) 
        external 
        view 
        returns (bool)
    {
    ...
    function addBlackList (address[] calldata _userList) 
        external 
        onlyMinter
    {
    ...
    function removeBlackList (address[] calldata _userList) 
        external 
        onlyMinter
    {
        require(_userList.length > 0, "MTS: bad request");
    ...
    function pay(address to, uint256 amount, string memory action)
        external 
    {
