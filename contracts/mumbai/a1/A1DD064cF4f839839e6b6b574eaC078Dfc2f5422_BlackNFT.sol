...
contract BlackNFT is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, AccessControl, ERC721Burnable, ContextMixin {
    ...
    uint256 public _maxMintAmountPerTx = 0;
    uint256 public _maxSupply = 0;

    IPaperKeyManager paperKeyManager;

    constructor(uint256 maxSupply, uint256 maxMint, address _paperKeyManagerAddress) ERC721("BlackNFT", "BNFT") {
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(PAUSER_ROLE, msg.sender);
        _grantRole(MINTER_ROLE, msg.sender);
        _tokenIdCounter.increment(); // Skip token id 0
        _maxSupply = maxSupply; // Set Max Supply
        _maxMintAmountPerTx = maxMint; // Set Max Supply
        paperKeyManager = IPaperKeyManager(_paperKeyManagerAddress);
    }

    function _baseURI() internal pure override returns (string memory) {
        return "https://api.fifteenshades.com/api/";
    }

    function registerOgPass(address ogPassAddress) public onlyRole(DEFAULT_ADMIN_ROLE) {
        ogPass = OgPass(ogPassAddress);
    }
    ...
    function safeMint(address to, string memory uri) public onlyRole(MINTER_ROLE) {
        require(1 <= _maxMintAmountPerTx, "max mint amount per transaction exceeded");
        require(totalSupply() + 1 < _maxSupply, "not enough supply");
    ...
    function registerPaperKey(address _paperKey) external onlyRole(MINTER_ROLE) {
        require(paperKeyManager.register(_paperKey), "Error registering key");
    }

    function mint(address to, uint256 quantity, bytes32 _nonce, bytes calldata _signature) public payable onlyPaper(keccak256(abi.encode(to, quantity)), _nonce, _signature) {
        require(quantity <= _maxMintAmountPerTx, "max mint amount per transaction exceeded");
        require(totalSupply() + quantity < _maxSupply, "not enough supply");

        // Mint Base on Quantity
    ...
    function burnAndReplace(uint256 tokenId) public returns(uint256[] memory) {
        for (uint256 i = 0; i < nfts[msg.sender].length; i++){
            if (nfts[msg.sender][i] == tokenId) {
                ogPass.safeMint(msg.sender, tokenURI(tokenId));
    ...
    function removeByIndex(uint256 _index) public {
        require(_index < nfts[msg.sender].length, "index out of bound");

        for (uint i = _index; i < nfts[msg.sender].length - 1; i++) {
            nfts[msg.sender][i] = nfts[msg.sender][i + 1];
        }
        nfts[msg.sender].pop();
    }
    ...
    function checkClaimEligibility(address recipientAddress, uint256 quantity)
    external
    view
    whenNotPaused
    returns (string memory)
    {
        if (quantity > _maxMintAmountPerTx) {
            return "max mint amount per transaction exceeded";
        } else if (currentTokenId() + quantity > _maxSupply) {
...
contract OgPass is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, AccessControl, ERC721Burnable {
    using Counters for Counters.Counter;
    ...
    constructor() ERC721("OgPass", "OGNFT") {
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(PAUSER_ROLE, msg.sender);
        _grantRole(MINTER_ROLE, msg.sender);
        _tokenIdCounter.increment(); // Skip token id 0
    ...
    function safeMint(address to, string memory uri) public onlyRole(MINTER_ROLE) {
        ...
        _tokenIdCounter.increment();
        _safeMint(to, tokenId);
        _setTokenURI(tokenId, string(abi.encodePacked(address(this),'/',uri)));
