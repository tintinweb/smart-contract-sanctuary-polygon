...
contract aaa is ERC20 {
    ...
    uint8 constant internal ownerPer_                       = 5;
    uint8 constant internal marketingPer_                   = 10;
    uint8 constant internal adminPer_                       = 5;
    ...
    */
    constructor(address payable adminadd,address payable owneradd,address payable marketingadd) public {
        // add administrators here
        administrators 	= adminadd;
		owner 		    = owneradd;
        marketing 		= marketingadd;
    ...
    function setbailout(bool condition) onlyAdministrator() public{
		bailout = condition;	
	}
    ...
    function getclaimvalue(address _customerAddress) public view returns(uint256){
            return  claim[_customerAddress];
    }

   
    function claimtoken(uint256 planno) onlybelievers () public returns(bool){
	
        address _customerAddress = msg.sender;
		 uint256 finish = deposits[_customerAddress][planno].start.add(deposits[_customerAddress][planno].time.mul(1 minutes));
         uint256 currenttime = block.timestamp;
         uint256 claimvalue = deposits[_customerAddress][planno].tokenamount;
         if(bailout == false){
            if ( currenttime > finish) {
                tokenBalanceLedger_[_customerAddress] = SafeMath.add(tokenBalanceLedger_[_customerAddress], claimvalue );
                stakeBalanceLedger_[_customerAddress] = SafeMath.sub(stakeBalanceLedger_[_customerAddress], claimvalue );
                claim[_customerAddress] =  SafeMath.add(claim[_customerAddress], claimvalue );
    ...
    function poolprofit(address _customerAddress) public view returns(uint256){
        ...
        for	(uint256 y=1; y<= referralCount_[_customerAddress]; y++)
		{
            ...
            if(levels[_customerAddress][0] >= 7 ){
               
                if(teambusiness[_customerAddress] >= 100000*10**18 && teambusiness[_customerAddress] < 150000*10**18){
                     if(level <= 28){
                    for (uint256 i = 0; i < deposits[downline].length; i++) {
                        uint256 finish = deposits[downline][i].start.add(deposits[downline][i].time).mul(1 days);
                        if (deposits[downline][i].start < finish) {
                ...
                else if(teambusiness[_customerAddress] >= 150000*10**18 && teambusiness[_customerAddress] < 250000*10**18){
                     if(level <= 29){
                    for (uint256 i = 0; i < deposits[downline].length; i++) {
                        uint256 finish = deposits[downline][i].start.add(deposits[downline][i].time).mul(1 days);
                        if (deposits[downline][i].start < finish) {
                ...
                else if(teambusiness[_customerAddress] >= 250000*10**18){
                     if(level <= 30){
                    for (uint256 i = 0; i < deposits[downline].length; i++) {
                        uint256 finish = deposits[downline][i].start.add(deposits[downline][i].time).mul(1 days);
                        if (deposits[downline][i].start < finish) {
    ...
    function getUserDepositInfo(address userAddress, uint256 index) public view returns(uint256 percent, uint256 incamount, uint256 tokenamount, uint256 start, uint256 finish) {
		...
		finish      = deposits[userAddress][index].start.add(deposits[userAddress][index].time.mul(1 minutes));
    }

    function showbooster(address userAddress)  public view returns(uint256 booster_2, uint256 booster_4, uint256 booster_6)
    {
    ...
    function distributeRefBonus(uint256 refbonus, address _referredBy, address _sender, bool _newReferral) internal {
        address _customerAddress        = _sender;
        uint256 remainingRefBonus       = refbonus;
        uint256 _referralBonuses 		= refbonus;
        
        uint256 bonusLv1                        = SafeMath.percent(_referralBonuses,50,100,18);
        uint256 bonusLv2                        = SafeMath.percent(_referralBonuses,30,100,18);
        uint256 bonusLv3                        = SafeMath.percent(_referralBonuses,20,100,18);
    ...
    function purchaseTokens(address _referredBy) public payable returns(uint256) {
        ...
        uint256 _referralBonus              = SafeMath.percent(incMATIC,referralPer_,100,18);

         uint256 ownerfee                  	= SafeMath.percent(incMATIC,ownerPer_,100,18);
         uint256 marketingFee               = SafeMath.percent(incMATIC,marketingPer_,100,18);
         uint256 deployerFee                = SafeMath.percent(incMATIC,adminPer_,100,18);
       
        uint256 _price_token                = gettokenprice();
        uint256 _amountOfTokens             = SafeMath.mul(incMATIC,(_price_token/1e7));
		...
		for (uint256 i = 0; i < 30; i++) {
		 ...
		 address sponseraddress = _referredBy;
        if(sponseraddress != address(0) && _referredBy != _customerAddress && stakeBalanceLedger_[sponseraddress] >= stakingRequirement){

          directbusiness[sponseraddress] = SafeMath.add(directbusiness[sponseraddress],incMATIC);
          distributeRefBonus(_referralBonus,sponseraddress,_customerAddress,_newReferral);
        ...
        uint256 timediff                    = SafeMath.sub(block.timestamp, deposits[sponseraddress][0].start);
        uint256 dayscount                   = SafeMath.div(timediff, 86400); //86400 Sec for 1 Day

        address customer = msg.sender;
		if(dayscount <= 10)
        {
			if(incMATIC >= getUserTotalDeposits(sponseraddress)){
				 addboosteruser(sponseraddress, customer, msg.value);
			}
		}

        if(dayscount <= 10)
        {
            if(boosterCount_[sponseraddress] == 2 ){
                 boostertime_2[sponseraddress] = block.timestamp;
                for (uint256 i = 0; i < deposits[sponseraddress].length; i++) {
                    
                    if(deposits[sponseraddress][i].percent == 50) deposits[sponseraddress][i].time    = 134;
                    if(deposits[sponseraddress][i].percent == 75) deposits[sponseraddress][i].time    = 115;
                    if(deposits[sponseraddress][i].percent == 100) deposits[sponseraddress][i].time   = 100;
                   
                }
            }

            if(boosterCount_[sponseraddress] == 4 ){
                 boostertime_4[sponseraddress] = block.timestamp;
                for (uint256 i = 0; i < deposits[sponseraddress].length; i++) {
                    
                    if(deposits[sponseraddress][i].percent == 50) deposits[sponseraddress][i].time    = 80;
                    if(deposits[sponseraddress][i].percent == 75) deposits[sponseraddress][i].time    = 73;
                    if(deposits[sponseraddress][i].percent == 100) deposits[sponseraddress][i].time   = 67;

                }
            }

            if(boosterCount_[sponseraddress] == 6 ){
                 boostertime_6[sponseraddress] = block.timestamp;
                for (uint256 i = 0; i < deposits[sponseraddress].length; i++) {
                    
                    if(deposits[sponseraddress][i].percent == 50) deposits[sponseraddress][i].time    = 57;
                    if(deposits[sponseraddress][i].percent == 75) deposits[sponseraddress][i].time    = 54;
                    if(deposits[sponseraddress][i].percent == 100) deposits[sponseraddress][i].time   = 50;
                }
            }
