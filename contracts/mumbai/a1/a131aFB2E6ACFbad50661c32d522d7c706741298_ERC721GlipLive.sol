    ...
    IERC2981
{
        ...
        returns (IGlipERC721LazyData.DecodedMintData memory)
    {
        ...
        bytes32 royaltySplitterAddress;
        LibPart.Part memory royaltiesSplitter;

        if (signer == data.creator) {
            creators = data.creators;
            royaltiesSplitter = data.royalty;
        } else {
            (fee, creators, royaltySplitterAddress, royaltiesSplitter) = IMinterUpgradeable(minter).getDetailsForMinting(token, data.creator, signer);
            if (creators.length == 0) {
                creators[0] = LibPart.Part(data.creator, 10000);
            }
        }
        
        if ( creators.length == 1 && creators[0].account == address(0x0)){
            creators[0].account = data.creator;
    ...
    // /*
    // Converts encoded data to complete mint data format
    // */
    // function decodeAndFormatLazyMintData(bytes calldata encoded)
    //     external
    //     view
    //     virtual
    //     returns (LibERC721LazyMint.Mint721Data memory)
    // {
    //     (, LibERC721LazyMint.Mint721Data memory data) = abi.decode(
    //         encoded,
    //         (address, LibERC721LazyMint.Mint721Data)
    //     );
    //     // By convention auctioneer is the last creator and same as minter
    //     return data;
    // }

    // ----------------------- VERIFIER -------------------------------------//

    function verifyAssetAndSigner(LibERC721LazyMint.Mint721Data memory data)
        internal
        view
        returns (address)
    {
        // !! TODO Check if token id can be created on asset contract itself
    ...
    data structure
     */
    function mintAndTransferEncodedData(bytes memory encoded) external virtual {
        (address token, LibERC721LazyMint.Mint721Data memory data) = abi.decode(
            encoded,
            (address, LibERC721LazyMint.Mint721Data)
        );
        require(token == address(this), "Wrong contracts");
        mintAndTransfer(data, data.creator);
    }
    ...
    function mintAndTransfer(
        ...
        // Verify if message sender is approved address, otherwise anyone
        // with access to mint data can just mint it free of cost
        require(
            _isDefaultApproved(_msgSender()) ||
                _msgSender() == data.creator,
            "Not approved"
        );

        // Verify legality of (Mint data + Message signer + Message sender + Contract owner + Approved) combination
        address signer = verifyAssetAndSigner(data);

        // Verify if creator has allowed the minter
        bytes32 royaltySplitterAddress = IMinterUpgradeable(minter).getDetailsForRoyalty(address(this), data.creator, signer);
        if (signer == data.creator) {

        }

        _safeMint(to, data.tokenId);
        _saveRoyalties(data.tokenId, royaltySplitterAddress);
    ...
    // --------------- UPDATE DATA AFTER MINT (Update Royalty Address) -------------------- //
    ...
    function royaltyInfo(
        ...
        if (royalty.value != 0 && royalty.account == address(0x0)) {
            ...
            royalty = IMinterUpgradeable(minter).getSplitter(royaltySplitterAddresses[_tokenId]);
        }
...
abstract contract AbstractRoyalties {
    ...
    mapping (uint256 => bytes32) royaltySplitterAddresses;

    function _saveRoyalties(uint256 id, bytes32 royaltySplitterAddress ) internal {
        royaltySplitterAddresses[id] = royaltySplitterAddress;
    }

    function _saveCreatorSignedRoyalties(uint256 id, address creator, LibPart.Part memory royalty ) internal {
        if (creator == address(id >> 96)) {
