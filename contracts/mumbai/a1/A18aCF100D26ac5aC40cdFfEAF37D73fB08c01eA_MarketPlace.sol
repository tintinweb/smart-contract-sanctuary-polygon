...
contract MarketPlace is IMarketPlace {
	...
	function cancelSell(uint256 _tokenId) public override onlyTokenOwner(_tokenId) {
		tokenMarketInfo storage TokenInfo = TokenMarketInfo[_tokenId];
		tokenIdOnSell.remove(_tokenId);
		TokenInfo.totalSell -= 1;
		TokenInfo.onSell = false;
	...
	function resale(uint256 _tokenId, uint256 _minprice) public override onlyTokenOwner(_tokenId) {
		tokenMarketInfo storage TokenInfo = TokenMarketInfo[_tokenId];
		require(TokenInfo.onSell, 'Token Not on Sell');
		TokenInfo.minPrice = _minprice;
	}

	function changeArtistFee(uint256 _tokenId, uint256 _artistFee) public override onlyTokenOwner(_tokenId) {
		tokenMarketInfo storage TokenInfo = TokenMarketInfo[_tokenId];
		require(TokenInfo.onSell, 'Token Not on Sell');
		TokenInfo.tokenId = _tokenId;
		TokenInfo.artistfee = _artistFee;
	...
	function changeGalleryFee(uint256 _tokenId, uint256 _galleryFee) public override onlyTokenOwner(_tokenId) {
		tokenMarketInfo storage TokenInfo = TokenMarketInfo[_tokenId];
		require(TokenInfo.onSell, 'Token Not on Sell');
		TokenInfo.tokenId = _tokenId;
