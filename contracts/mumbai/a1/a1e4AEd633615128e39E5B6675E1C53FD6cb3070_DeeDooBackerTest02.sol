...
interface IContractTeacher {
  function ownerOf(uint256 tockenId) external view returns (address);
  function totalSupply() external view returns (uint256);
  function getCourseName(uint256 _tokenId) external view returns (string memory);
}

contract DeeDooBackerTest02 is ERC721A, Ownable, ReentrancyGuard {
  ...
  /**
   * with mapBackingAmount we store the amount of each backing operation on change and is inmutable 
   * with mapBackingCourseTokenId we store the relationship between every backing operation to the course NFT Collection
   */
  mapping(uint256 => uint256) public mapBackingAmount;
  mapping(uint256 => uint256) public mapBackingCourseTokenId;
  ...
  bool public backerMintEnabled = true;
  ...
  /**
   * with IContractTeacher we link this contract with DEEDOO TEACHER smart contract to validate 
   *the course on which the BACKER want to invest or support
   */

  function backerMint(uint256 _courseTokenId) public payable {
    require(backerMintEnabled, 'The backer mint is not enabled!');
    require(msg.value > 0, 'The value can not be cero!');
    require(IContractTeacher(contractWhiteListAddress).totalSupply()>=_courseTokenId, 'Course token id not found');
    /**
     * Storing the backing value
     */
    mapBackingAmount[totalSupply() + 1] = msg.value; 
    mapBackingCourseTokenId[totalSupply() + 1] = _courseTokenId;
    /**
     * Mint just one token because is just one backing operation
     */
  ...
  function validateNftTokenIdWhitelistedByOwner(address _owner, uint256 tokenId) public view returns (bool) {
    ...
    return (IContractTeacher(contractWhiteListAddress).ownerOf(tokenId) == _owner);
  ...
  function getTotalBackForCourse(uint256 _courseTokenId) public view returns (uint256) {
    uint256 auxTotalBackForCourse = 0;
    for (uint i=1; i<=totalSupply(); i++) {
      if (mapBackingCourseTokenId[i] == _courseTokenId) {
        auxTotalBackForCourse = auxTotalBackForCourse + mapBackingAmount[i];
      }
    }
    return auxTotalBackForCourse;
  }

  function getTotalBackForCourseByWalletAddress(uint256 _courseTokenId, address _backerAddress) public view returns (uint256) {
    uint256 auxTotalBackAmount = 0;
    for (uint i=1; i<=totalSupply(); i++) {
      if (mapBackingCourseTokenId[i] == _courseTokenId && _backerAddress == ownerOf(i)) {
        auxTotalBackAmount = auxTotalBackAmount + mapBackingAmount[i];
      }
    }
    return auxTotalBackAmount;
  }
  ...
  function setBackertMintEnabled(bool _state) public onlyOwner {
    backerMintEnabled = _state;
  }
