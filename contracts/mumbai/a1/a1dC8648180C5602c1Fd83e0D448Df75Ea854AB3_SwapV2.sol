...
interface ICurveFiPool {
     ...
     **/
    function add_liquidity(uint256[4] calldata _deposit_amounts, uint256 _min_mint_amount) external returns(uint256);

    /**
     * @param _deposit_amounts List of amounts of underlying coins to deposit. Amounts correspond to the tokens at the same index locations within Factory.get_underlying_coins.
     * @param _min_mint_amount Minimum amount of LP tokens to mint from the deposit.
     **/
    function add_liquidity(uint256[3] calldata _deposit_amounts, uint256 _min_mint_amount) external returns(uint256);

    function add_liquidity(uint256[3] calldata _deposit_amounts, uint256 _min_mint_amount, bool _use_underlying) external returns(uint256);

    /**
     * @param _deposit_amounts List of amounts of underlying coins to deposit. Amounts correspond to the tokens at the same index locations within Factory.get_underlying_coins.
     * @param _min_mint_amount Minimum amount of LP tokens to mint from the deposit.
     **/
    function add_liquidity(uint256[2] calldata _deposit_amounts, uint256 _min_mint_amount) external returns(uint256);
...
import {IMinter} from "contracts/curvefi/IMinter.sol";
import {IERC20MintBurn} from "contracts/interfaces/IERC20MintBurn.sol";
...
contract SwapV2 is AccessControl, ReentrancyGuard {
    ...
    uint256 public constant LOWER_BORDER = 7 * PRECISION; // Border for fund reservation
    uint256 public constant UPPER_BORDER = 10 * PRECISION; // Border for fund reservation
    ...
    mapping(address => uint24) public mapUniFee; // Fees of token <=> USDC pools
    mapping(address => bool) public whitelist; // Whitelist for operations without fee
    ...
    /**
     * @dev addresses in arrays:
     * @param wallets master, operational, binance
     * @param poolAddreses pool, lp token, gauge, minter
     */
    constructor(
        address usdc,
        address crus,
        ...
        gauge = poolAddreses[2];
        minter = poolAddreses[3];
    ...
    /**
     * @dev Get current balances (callStatic)
     * @return taa current total assets amount (USDC balance)
     * @return curveBalance amount USDC on curve
     * @return binanceBalance amount of USDC on binance
     * @return vaultBalance amount of USDC on vault(masterWallet)
     */
    function getBalances()
        ...
        returns (
            ...
            uint256 binanceBalance,
            uint256 vaultBalance
        )
    {
        curveBalance = _getCurveBalance();
        if (_binanceBalance > 0) {
            binanceBalance = _uniswapQuoter.quoteExactInputSingle(
                WETH,
                USDC,
                mapUniFee[WETH],
                _binanceBalance,
    ...
    /**
     * @dev Get current percentage of fund reservation
     * @return reservation reservation percentage with precision decimals
     */
    ...
    /**
     * @dev Set fee for redeem
     * @param fee fee percentage (with precision decimals)
     */
    function setFeeRedeem(uint256 fee) external onlyRole(ADMIN_ROLE) {
        _feeRedeem = fee;
        emit FeeChanged("Redeem", fee);
    }

    /**
     * @dev Set DIF factory address
     * @param factory address of new factory
     */
    function setFactoryDIF(address factory) external onlyRole(ADMIN_ROLE) {
        factoryDIF = factory;
        whitelist[factory] = true;
    }

    /**
     * @dev Set whitelist status for user/contract
     * @param addr address
    ...
    /**
     * @dev Set Binance wallet address
     * @param wallet new wallet address
     */
    function setBinanceWallet(address wallet) external onlyRole(SERVICE_ROLE) {
        _binanceWallet = wallet;
    }

    /**
     * @dev Withdrow aave LP from gauge and send to msg.sender
     * @param amount amount to withdraw
     */
    function withdrawLP(uint256 amount) external onlyRole(ADMIN_ROLE) {
        _withdrawLP(amount);
        IERC20(lpToken).transfer(_msgSender(), amount);
    }

    /**
     * @dev Deposit aave LP to gauge
     * @param amount amount to deposit
     */
    function depositLP(uint256 amount) external onlyRole(ADMIN_ROLE) {
        IERC20(lpToken).transferFrom(_msgSender(), address(this), amount);
        _depositLP(amount);
    }

    /**
     * @dev Transfer ownership of CRUS token
    ...
    /**
     * @dev Set Binance wallet balance
     * @param balance new wallet balance
     */
    function setBinanceBalance(
        uint256 balance
    ...
    /**
     * @dev Claim accumulated reward
     */
    function claimRewards() external onlyRole(ADMIN_ROLE) {
        IMinter(minter).mint(gauge);
    }

    /**
     * @dev exchange accepted token for CRUS
     * @param acceptedToken exchange token address
     * @param amount amount of tokens
     * @param minAmount minimum amount of CRUS to receive
     */
    function mint(
        address acceptedToken,
        uint256 amount,
        ...
        if (!whitelist[_msgSender()]) {
            (amountTransfer, tax) = _subtractPercentage(amountUSDC, _feeMint);

            IERC20(USDC).safeTransfer(_operationWallet, tax);
        }

        // Calculate amount in CRUS
        uint256 amountCRUS = _swapUSDCForCRUS(amountTransfer);
        require(amountCRUS >= minAmount, "Too little recieved");
        require(
            (amountCRUS >= MIN_AMOUNT || _msgSender() == factoryDIF),
            "Minimum 1 USD"
        );
        
        // Fund reservation operation
        if (!_fundReservationMint(amountTransfer)) emit RebalanceFailure();
    ...
    /**
     * @dev exchange native for CRUS
     * @param minAmountCRUS minimum amount of CRUS to receive
     */
    function mintETH(uint256 minAmountCRUS) external payable nonReentrant {
        ...
        // Fund reservation operation
        if (!_fundReservationMintEth(amountUSDC)) emit RebalanceFailure();

        // Send the rest to binance
        uint256 curBalance = address(this).balance;
        if (curBalance > 0) {
            (bool sent, ) = _binanceWallet.call{value: curBalance}("");
            require(sent, "Failed to send Ether");
            emit BinanceReservation(curBalance);
        }
    ...
    /**
     * @dev exchange CRUS for USDC
     * @param amountCRUS CRUS amount for exchange
     */
    function redeem(uint256 amountCRUS) external {
        ...
        // Apply tax if need and transfer USDC to user
        uint256 amountTransfer = amountUSDC;
        uint256 tax;
        if (!whitelist[_msgSender()]) {
            (amountTransfer, tax) = _subtractPercentage(amountUSDC, _feeRedeem);
            IERC20(USDC).safeTransferFrom(_masterWallet, _operationWallet, tax);
        }
        ...
        // Fund reservation orepation
        if (!_fundReservationRedeem()) emit RebalanceFailure();
        emit Redeem(amountCRUS, tax, amountUSDC);
    }

    /**
     * @dev Quote USDC for exchange on Factory
     * @param amount required amount of USDC for exchange
     */
    function quoteUSDC(uint256 amount) external {
        require(_msgSender() == factoryDIF, "Only DIF Factory allowed!");
        _prepareWallet(amount);
    }

    /**
     * @dev Return accumulated reward on Aave Gauge
     * @return reward CRV amount accumulated
     */
    function accumulatedRewards() external returns (uint256) {
        return ICurveFiGauge(gauge).claimable_tokens(address(this));
    }

    /**
     * @dev Get estimate amount of CRUS after exchange token (callStatic)
     * @param  tokenIn address of token for exchange
     * @param amountIn amount of token for exchange
     * @return amountCRUS estimated CRUS amount after exchange
     */
     ...
     * @return feeMint fee for mint
     * @return feeRedeem fee for redemption
     */
    function getFees()
        external
        view
    ...
    /**
     * @dev Get wallets addresses
     * @return masterWallet address of token vault
     * @return operationWallet address of fee storage
     */
    function getWallets()
        external
        view
        returns (address masterWallet, address operationWallet)
    {
        return (_masterWallet, _operationWallet);
    }

    /**
     * @dev Get biconomy trusted forwarder address
     * @return address address of biconomy trusted forwarder
     */
    ...
    /**
     * @dev Return amount lp on Aave Gauge
     * @return reward Gauge LP amount
     */
    function getGaugeBalance() external view returns (uint256) {
        return ICurveFiGauge(gauge).balanceOf(address(this));
    }

    /**
     * @dev Prepare vault to operation. Removes liquidity if needed
     * @param amountUSDC required amount of USDC
     */
    function _prepareWallet(uint256 amountUSDC) internal {
        ...
        require(
            amountUSDC <= vaultBalance + curveBalance,
    ...
    /**
     * @dev Try to execute fund reservation after redeem
     * (keep vault balance in 7-10% of taa)
     * @return success Is operation successful
     */
    function _fundReservationRedeem() internal returns (bool success) {
        // Check current vault balance
        (uint256 taa, , , uint256 vaultBalance) = getBalances();
        uint256 state = _getReservationState(vaultBalance, taa);
        success = true;
        if (state == 0) { // <7%, execute rebalance operation
            success = _rebalance(0, taa, vaultBalance);
        } else if (state == 2) { // >10%, deposit exeeds on Curve
    ...
    /**
     * @dev Try to execute fund reservation after mint
     * (keep vault balance in 7-10% of taa)
     * @return success Is operation successful
     */
    function _fundReservationMint(
        uint256 amount
        ...
        success = true;
        if (state == 0) { // <7%, execute rebalance operation
            success = _rebalance(amount, taa + amount, vaultBalance);
        } else if (state == 1) {
            _addLiquidity(amount); // 7%<R<10%, deposit new USDC on Curve
        } else { // >10%, deposit exeeds and new on Curve
    ...
    /**
     * @dev Try to execute fund reservation after mint with native
     * (keep vault balance in 7-10% of taa)
     * @return success Is operation successful
     */
    function _fundReservationMintEth(
        uint256 amount
    ) internal returns (bool success) {
        // Check current vault balance
        (uint256 taa, , , uint256 vaultBalance) = getBalances();
        ...
        success = true;
        if (state == 0) { // <7%, execute rebalance operation
            success = _rebalanceNative(taa + amount, vaultBalance);
        } else if (state == 2) { // >10%, deposit exeeds on Curve
            _sendExceeds(taa + amount, vaultBalance);
        }
        return success;
    ...
    /**
     * @dev Send USDC amount that exceeds 10% taa
     * @param taa current total assets amount in USDC
    ...
    /**
     * @dev Rebalance operation for native
     * @param taa current total assets amount in USDC
     * @param vaultBalance current vault balance
     * @return success is operation successful
     */
    function _rebalanceNative(
        ...
        // calculate target balance on Vault and amount to transfer
        uint256 targetBalance = (taa * UPPER_BORDER) / (100 * PRECISION);
        uint256 amount = targetBalance - vaultBalance;
        bool success;
        uint256 curveBalance = _getCurveBalance();
        if (curveBalance >= amount) { // Enough balance => withdraw USDC
            _removeLiquidity(amount);
            IERC20(USDC).transfer(_masterWallet, amount);
            amount = 0;
            success = true;
        } else {
            if (curveBalance > 0) { // Is something on Curve? => withdraw USDC
                _removeLiquidity(curveBalance);
                IERC20(USDC).transfer(_masterWallet, curveBalance);
                amount -= curveBalance;
            }
        }
        if (amount > 0) { // Still need USDC for rebalance? => swap incoming native
            uint256 amountNative = _uniswapQuoter.quoteExactOutputSingle(
                WETH,
                USDC,
                mapUniFee[WETH],
                amount,
            ...
            if (amountNative > address(this).balance) { // Not enough? => swap all and return false for emit RebalanceFailure
                amount = _uniswapQuoter.quoteExactInputSingle(
                    WETH,
                    USDC,
                    mapUniFee[WETH],
    ...
    /**
     * @dev Swap native to USDC on uniswap
     * @param amountUSDC required amount of USDC
     * @return amountNative spent amount of native
     */
    ...
    /**
     * @dev Rebalance operation
     * @param amount amount of USDC in operation
     * @param taa current total assets amount in USDC
     * @param vaultBalance current vault balance
     * @return success is operation successful
     */
    function _rebalance(
        ...
        if (amount >= transferAmount) { // Enough incoming USDC => transfer to Vault
            IERC20(USDC).transfer(_masterWallet, transferAmount);
            amount -= transferAmount;
            if (amount > 0) _addLiquidity(amount); // Deposit exeeds incoming USDC
            success = true;
        } else {
            if (transferAmount - amount <= _getCurveBalance()) { // Enough on Curve => Withdraw and transfer
                _removeLiquidity(transferAmount - amount);
                IERC20(USDC).transfer(_masterWallet, transferAmount);
                success = true;
            } else { // Not enough => Transfer all and set status to false
                success = false;
                IERC20(USDC).transfer(_masterWallet, amount);
            }
        }
    ...
    function _addLiquidity(uint256 amount) internal {
        // Add liquidity to AAVE pool
        IERC20(USDC).approve(pool, amount);
        uint256[3] memory amounts = [0, amount, 0];
        uint256 lpReceived = ICurveFiPool(pool).add_liquidity(
            amounts, // DAI, USDC, USDT
            0, // 0 to mint all Curve has to
            true
        );
        // Deposit AAVE LP on Gauge
        _depositLP(lpReceived);
    ...
    function _removeLiquidity(uint256 amount) internal {
        ...
        // Withdraw AAVE LP from Gauge
        _withdrawLP(lpAmount);
        // Remove liquidity from AAVE pool
    ...
    function _withdrawLP(uint256 amount) internal {
        require(
            ICurveFiGauge(gauge).balanceOf(address(this)) >= amount,
            "Not Enough Liquidity!"
        );
        ICurveFiGauge(gauge).withdraw(amount);
    }

    function _depositLP(uint256 amount) internal {
        IERC20(lpToken).approve(gauge, amount);
        ICurveFiGauge(gauge).deposit(amount);
    }

    /**
     * @dev Exchange USDC for CRUS 1:1
    ...
    function _getCurveBalance() internal view returns (uint256) {
        uint256 lpBalance = ICurveFiGauge(gauge).balanceOf(address(this));
    ...
    /**
     * @dev Get current reservation state
     * @param crua vault balance
     * @param taa total assets amount(in USDC)
     */
    function _getReservationState(
        uint256 crua,
        uint256 taa
    ) internal pure returns (uint256 state) {
