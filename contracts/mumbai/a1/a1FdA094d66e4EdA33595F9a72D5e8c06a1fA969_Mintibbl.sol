...
contract Mintibbl is ERC721URIStorage, Algoz {
    uint256 private s_tokenCounter;
    mapping(string => bool) private s_usedTokenURIs;

    event NewDrawing(address indexed sender, uint256 indexed tokenId);

    constructor(address tokenVerifier) ERC721("Mintibbl", "MNTBL") Algoz(tokenVerifier, true, 3) {
        s_tokenCounter = 0;
    }
...
contract Algoz {
    ...
    uint256 public proof_expiry_length;

    constructor(
        address _token_verifier,
        bool _verify_enabled,
        uint256 _proof_expiry_length
    ) {
