...
contract NX7NFT is ERC721Enumerable, Ownable {
    ...
    function amountToPay() view public returns (uint256) {
       int256 maticPrice = PriceFeed(priceFeed).latestAnswer();
       uint8 decimals = PriceFeed(priceFeed).decimals();
        return (_mintPrice * 10 ** decimals)  / uint256(maticPrice); 
    }
    ...
    function mint(address to, uint32 amount) external payable {
        ...
        require(
            received * amount  == amountToPay() * (amount),
