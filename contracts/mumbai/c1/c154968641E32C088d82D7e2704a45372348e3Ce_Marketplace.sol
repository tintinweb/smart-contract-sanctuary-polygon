...
error Marketplace__NoRights(address nftAddress, uint256 tokenId, address seller);
error Marketplace__PriceMustBeAboveZero();
error Marketplace__NotApprovedForMarketplace(address nftAddress, uint256 tokenId);
error Marketplace__NotListed();
error Marketplace__PriceNotMet(address nftAddress, uint256 tokenId, uint256 price);
...
contract Marketplace is ReentrancyGuard {
    ...
    modifier isOwner(
        ...
        if (nftContract.ownerOf(tokenId) != seller) {
            revert Marketplace__NoRights(nftAddress, tokenId, seller);
        }
        _;
    }
    ...
    function listItem(
        ...
        if (nftContract.getApproved(tokenId) != address(this)) {
            revert Marketplace__NotApprovedForMarketplace(nftAddress, tokenId);
        }
        ...
        nonReentrant
    {
        ...
        if (msg.value < listedItem.price) {
            revert Marketplace__PriceNotMet(nftAddress, tokenId, listedItem.price);
        }
     ...
     */
    function withdrawProceeds(address seller) external nonReentrant {
