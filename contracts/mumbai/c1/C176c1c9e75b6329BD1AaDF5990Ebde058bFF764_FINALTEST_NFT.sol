...
contract FINALTEST_NFT is ERC721Royalty, Ownable, ContextMixin, ReentrancyGuard, AccessControl {
    ...
    function VIPList(uint256 numberOfTokens) public saleFlip nonReentrant payable {
        ...
        require(VIP1Supply.add(numberOfTokens) <= MAX_VIP_1_List, "Max limit of NFTs");
        require(msg.value >= price.mul(numberOfTokens), "Ether value sent is not correct");
        require(isVIPlist[msg.sender].add(numberOfTokens) <= 5, "User exceed Max limit");
    ...
    function VIPList2(uint256 numberOfTokens) public saleFlip2 nonReentrant payable {
        ...
        require(msg.value >= price.mul(numberOfTokens), "Ether value sent is not correct");
        require(isVIPlist2[msg.sender].add(numberOfTokens) <= 5, "User exceed Max limit");
        for(uint i = 0; i < numberOfTokens; i++) {
            uint256 newItemId = _tokenIds.current();
            _safeMint(msg.sender, newItemId+1);
            _setTokenRoyalty(newItemId+1, msg.sender, royaltyPer);
