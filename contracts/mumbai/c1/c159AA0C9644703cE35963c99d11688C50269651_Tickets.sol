    ...
    ERC2981Upgradeable
{
    ...
    event TicketIds(uint256 ticket, uint256 token);
    ...
    function mintTicket(uint256 ticketId, address to, string memory uri)
        public
        whenNotPaused
        onlyRole(MINTER_ROLE)
        returns (uint256)
    {
        return _mintTicket(ticketId, to, uri);
    }

    function mintTicketsToManyUsers(uint256 ticketId, address[] memory to, string[] memory uri)
        public
        whenNotPaused
        onlyRole(DEFAULT_ADMIN_ROLE)
        returns (bool)
    {
        require(to.length == uri.length,"to length and uri length must be equals");
        for (uint256 i = 0; i < to.length; ++i) {
            _mintTicket(ticketId, to[i], uri[i]);
        }
        return true;
    }

    function mintManyTicketsToUser(uint256[] memory ticketIds, address to, string[] memory uri)
        public
        whenNotPaused
        onlyRole(DEFAULT_ADMIN_ROLE)
        returns (bool)
    {
        require(ticketIds.length == uri.length,"tickets length and uri length must be equals");
        for (uint256 i = 0; i < ticketIds.length; ++i) {
            _mintTicket(ticketIds[i], to, uri[i]);
        }
        return true;
    }

    function burn(uint256 token) public override onlyRole(DEFAULT_ADMIN_ROLE) {
        _burn(token);
    }
    ...
    function contractURI() public pure returns (string memory) {
        return
            "https://us-central1-welook-siwe.cloudfunctions.net/ipfs/v2/t/contract/0";
    }
    ...
    function _mintTicket(uint256 ticketId, address to, string memory uri)
        internal
        returns (uint256)
    {
        ...
        _mint(to, tokenId);
        _token[tokenId] = ticketId;
        _setTokenURI(tokenId, uri);
        emit TicketIds(ticketId, tokenId);
        return tokenId;
    }
