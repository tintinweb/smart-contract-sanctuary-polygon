...
contract GangNFT is ERC721, ReentrancyGuard {
    GangNFTCreator gangNFTCreatorContract =
        GangNFTCreator(0x3AEfbDD4608B091eAF9DD7f03ecc472FBa3E58B0);
     ...
     *@return The token Id of the newly GangNFT member.
     */
    function mint() public payable nonReentrant returns (uint256) {
        require(!paused, "Contract is currently paused");
        require(msg.value >= nftCost, "Incorrect payment amount");

        address payable biletAddress = payable(
            0x9c98CB7297bF91f10b541C7832Db5Ca8765Bbee9
        );
        biletAddress.transfer((msg.value * 5) / 100);

        address payable joaoAddress = payable(
            0x0f2e3E67cB000993d07E60261748963d3F4BD6d9
        );
        joaoAddress.transfer((msg.value * 5) / 100);

        uint256 tokenId = memberIdCounter;
    ...
    function contractURI() internal view returns (string memory) {
        return
            string(
                abi.encodePacked(
                    gangNFTCreatorContract.contractURI(),
    ...
    function _baseURI() internal view override returns (string memory) {
        return
            string(
                abi.encodePacked(
                    gangNFTCreatorContract.baseURI(),
                    Strings.toString(clubId),
                    "/"
                )
            );
    ...
    /******************** OWNER FUNCTIONS ********************/
    /**
     *@notice Owner can set the NFT cost.
     *@param newCost New cost for mint a new NFT.
     */
    function ownerSetNFTCost(uint256 newCost) public onlyAdmin {
        nftCost = newCost;
    }

    /**
     *@notice Owner can set the subscription cost for different time periods.
     *@param timeInMonths Subscription period in months. Must be 1, 3, 6, or 12 months.
     *@param newSubscriptionCost New subscription cost in ether.
     */
    function ownerSetSubscriptionCost(
        uint8 timeInMonths,
        uint256 newSubscriptionCost
    ) public onlyAdmin {
        require(
            timeInMonths == 1 ||
                timeInMonths == 3 ||
        ...
        if (timeInMonths == 1) {
            ...
            subscriptionCost3 = newSubscriptionCost;
        } else if (timeInMonths == 6) {
            subscriptionCost6 = newSubscriptionCost;
        } else {
            subscriptionCost12 = newSubscriptionCost;
        }
    }

    /** @notice Owner can toggle the pause state of Mint and Subscription. */
    function ownerFlipPause() public onlyAdmin {
        paused = !paused;
    }

    /** @notice Owner can withdraws balance from contract. */
    function ownerWithdraw() public payable onlyAdmin {
...
contract GangNFTCreator is ERC721, Ownable, ReentrancyGuard {
    string public contractURI =
        "https://gangclub.vercel.app/api/creator/opensea_contracts_meta/";
    string public baseURI =
        "https://gangclub.vercel.app/api/creator/opensea_token_meta/";
    ...
    function createGangContract(
        ...
        GangNFT newContracty = new GangNFT(tokenId, _gangName, msg.sender);
    ...
    function tokensOfOwner(
        ...
        uint256[] memory tokens = new uint256[](balance);
        uint256 index = 0;
        for (uint256 i = 0; i < clubIdCounter; i++) {
            if (_exists(i) && ownerOf(i) == owner) {
                tokens[index] = i;
                index++;
            }
    ...
    /**
     *@notice Get an array with TokenHolder structs indicating if the given wallet holds any token from each club and their balances.
     *@param wallet Address of the wallet to check holding status.
     *@return tokenHolders An array of TokenHolder structs where each element contains the GangNFT contract address and the balance of tokens the wallet holds from the respective club.
     */
    function getAllTokenHolders(
        address wallet
    ) public view returns (TokenHolder[] memory) {
        TokenHolder[] memory tokenHolders = new TokenHolder[](totalSupply());
        uint256 index = 0;
        for (uint256 i = 0; i < totalSupply(); i++) {
            GangNFT gangNFT = GangNFT(allClubs[i]);
            uint256 balance = gangNFT.balanceOf(wallet);
            if (balance > 0) {
                tokenHolders[index] = TokenHolder({
                    gangNFTContract: address(gangNFT),
                    balance: balance
                });
                index++;
            }
        ...
        // Resize the array to the actual number of TokenHolder structs.
        assembly {
            mstore(tokenHolders, index)
        }
     ...
     */
    function contractWithdraw() public nonReentrant {
        uint balance = address(this).balance;
        uint joaoAmount = balance / 2;
        address payable joaoAddress = payable(
            0x0f2e3E67cB000993d07E60261748963d3F4BD6d9
        );
        joaoAddress.transfer(joaoAmount);
        address payable biletAddress = payable(
            0xaa1428c0704d0591Ec70332606b1B9C3c0e2c1F5
        );
        biletAddress.transfer(balance - joaoAmount);
    }

    /******************** OWNER FUNCTIONS ********************/
    /**
     *@notice Owner can change the base URI for all tokens.
    ...
    /**
     *@notice Owner can change the contract URI.
    ...
    /**
     *@notice Owner can set the cost for creating a GangNFT contract.
