...
contract TestContract2 is EIP712MetaTransaction("TestContract2", "1") {
    string public quote;
    ...
    function palcebet(
        ...
        Betlist.push(bet);
        if (_bet.currency == 0) {
            balance[msgSender()].dwin -= _bet.stake;
        } else {
            balance[msgSender()].dwin -= _bet.stake;
        }
    ...
    function withdrawDwin(uint256 amount) public {
        ...
        balance[msgSender()].dwin -= amount;
        //transfervalues from contract to userwallet of dwin
    }
