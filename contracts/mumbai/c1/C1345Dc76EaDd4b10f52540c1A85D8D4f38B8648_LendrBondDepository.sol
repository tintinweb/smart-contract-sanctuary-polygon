...
contract LendrBondDepository is Ownable {
    ...
    IERC20 public immutable Lendr; // token given as payment for bond
    IERC20 public immutable principle; // token used to create bond
    ITreasury public immutable treasury; // mints Lendr when receives principle
    ...
    uint32 public lastDecay; // reference lendr for debt decay
    ...
    struct Adjust {
        ...
        uint32 lastTime; // lendr when last adjustment made
    }
    ...
    function initializeBondTerms( 
        ...
        require( _vestingTerm >= 350, "Vesting must be longer than 36 hours" ); //originial 129600
    ...
    function setBondTerms ( PARAMETER _parameter, uint _input ) external onlyOwner() {
        if ( _parameter == PARAMETER.VESTING ) { // 0
            require( _input >= 350, "Vesting must be longer than 36 hours" ); //originial 129600
    ...
    function deposit( 
        ...
        uint balanceBefore = Lendr.balanceOf(address(this));
        /**
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            Lendr.safeTransfer( DAO, fee ); 
        }
        require(balanceBefore.add(profit) == Lendr.balanceOf(address(this)), "Not enough Lendr to cover profit");
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            Lendr.transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                Lendr.approve( address(stakingHelper), _amount );
                stakingHelper.stake( _amount, _recipient );
            } else {
                Lendr.approve( address(staking), _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return Lendr.totalSupply().mul( terms.maxPayout ) / 100000 ;
    }
