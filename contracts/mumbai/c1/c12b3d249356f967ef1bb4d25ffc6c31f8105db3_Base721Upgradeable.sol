    ...
    ERC721WithPermitUpgradable
{
    ...
    function buy(uint256 typeNFT_, uint256 quantity_) external payable override nonReentrant {
...

        {
            ...
            if (typeInfo.paymentToken == address(0)) {
                refund = value - total; // will throw underflow error if value < total -> skip refund
                if (refund > 0.1 ether) {
                    SafeTransferLib.safeTransferETH(sender, refund);
                    SafeTransferLib.safeTransferETH(recipient, total);
                } else {
                    SafeTransferLib.safeTransferETH(recipient, value);
                }
            } else {
                SafeTransferLib.safeTransferFrom(typeInfo.paymentToken, sender, recipient, total);
            }
        }
        _setSold(typeNFT_, quantity_);
        uint256 tokenId = _idCounter;
        emit Registered(sender, typeNFT_, tokenId, quantity_, _sold[typeNFT_]);
        _batchProcess(sender, tokenId, quantity_, typeInfo.operation);
    }
    ...
    function _batchProcess(
        ...
        address tokenCollection = address(this);
        address tba;
        bytes memory callData = operation_.data;

        for (uint256 i; i < totalMint_; ) {
            tba = registry.account(implementation, chainId, tokenCollection, tokenId_, 0);
            assembly {
            ...
            _call(operation_);
            registry.createAccount(implementation, chainId, tokenCollection, tokenId_, 0, "");
            IAccount(tba).initialize();
...
abstract contract PoseTypeUpgradeable is IPoseTypeUpgradeable, BaseUpgradeable {
    ...
    function getTypeNFT(uint256 typeNFT) external view returns (address, uint256, uint256, Operation memory) {
        return (
