...
contract sUSDR is AddressAccessor, ERC20Permit {
    ...
    function burn(address account, uint256 amount) external {
        ...
        uint delta = (_supplyDelta * amount) / totalSupply();
        _totalSupply -= amount;
        _supplyDelta -= delta;
    ...
    function rebase(uint256 supplyDelta) external onlyRole(CONTROLLER_ROLE) {
        ...
        if (supplyDelta > 0) {
            ...
            uint256 maxDelta = MAX_SUPPLY - _totalSupply - _supplyDelta;
            if (supplyDelta > maxDelta) {
                _supplyDelta += maxDelta;
                emit Rebase(block.number, maxDelta);
            } else {
                _supplyDelta += supplyDelta;
                emit Rebase(block.number, supplyDelta);
            }
        } else {
    ...
    function balanceOf(address account) public view override returns (uint256) {
        ...
        return
            _scale(
                _scale(balance.value, balance.supply, balance.delta),
                _totalSupply + _supplyDelta,
                _totalSupply
            );
    }
    ...
    function totalSupply() public view override returns (uint256) {
        return _totalSupply + _supplyDelta;
    }
    ...
    function _decreaseBalance(address account, uint256 amount) private {
        ...
        Balance storage balance = _balances[account];
        balance.value = balance_ - amount;
        balance.supply = _totalSupply;
        balance.delta = _supplyDelta;
    }
