...
contract MSG {
    ...
    uint256 private constant timeStep = 3 seconds;
    uint256 private constant dayPerCycle = 30 seconds; 
    uint256 private constant dayRewardPercents = 150;
    uint256 private constant maxAddFreeze = 90 seconds;
    ...
    // 余额控制
    uint256[5] private balReached = [50000e6, 100e10, 200e10, 500e10, 1000e10];
    uint256[5] private balFreeze = [35000e6, 70e10, 100e10, 300e10, 500e10];
    uint256[5] private balUnfreeze = [500000e6, 150e10, 200e10, 500e10, 1000e10];
    ...
    uint256 private constant predictDuration = 15 seconds;
    ...
    mapping(uint256=>address[]) public dayNewbies;
    ...
    function _deposit(address _userAddr, uint256 _amount, uint256 _types) private {
        ...
        if(_types == 0){
            ...
            if(isFreezing){
                _setFreezeReward();
                if(!isUnfreezedReward[_userAddr][freezedTimes]) isUnfreezedReward[_userAddr][freezedTimes] = true;
            }
        }
    ...
    function withdraw() external {
        ...
        uint256 rewardsStatic = userRewards.statics.add(userRewards.invited).add(userRewards.predictWin);
        uint256 incomeFee = rewardsStatic.mul(incomeFeePercents).div(baseDivider);
        // 手续费
        usdt.transfer(feeReceiver, incomeFee);
        uint256 predictPoolFee = rewardsStatic.mul(predictPoolPercents).div(baseDivider);
        // 竞猜费
        ...
        uint256 split = leftReward.mul(splitPercents).div(baseDivider);
    ...
    function transferBySplit(address _receiver, uint256 _amount) external {
        uint256 minTransfer = minDepositable.mul(2);
        ...
        if(block.timestamp >= lastDistribute.add(timeStep)){
            ...
            for(uint256 i = 0; i < winners.length; i++){
                if(winners[i] != address(0)){
                    uint256 reward = predictPool.mul(predictWinnerPercents[i]).div(baseDivider);
    ...
    function getPredictWinners(uint256 _day) public view returns(address[] memory winners) {
        uint256 deposits = dayDeposits[_day];
        uint256 steps = deposits.div(searchStep);
        uint256 maxWinners = predictWinnerPercents.length;
        winners = new address[](maxWinners);
        uint256 counter;
        for(uint256 i = steps; i >= 0; i--){
            uint256 winAmount = i.mul(searchStep);
    ...
    function getUserInfos(address _userAddr) external view returns(UserInfo memory, RewardInfo memory, OrderInfo[] memory, bool) {
        return (userInfo[_userAddr], rewardInfo[_userAddr], orderInfos[_userAddr], isUnfreezedReward[_userAddr][freezedTimes]);
    }
    ...
    function getContractInfos() external view returns(address[3] memory, uint256[9] memory, bool) {
        address[3] memory infos0;
        infos0[0] = address(usdt);
        infos0[1] = feeReceiver;
        infos0[2] = defaultRefer;

        uint256[9] memory infos1;
        infos1[0] = startTime;
        ...
        return (infos0, infos1, isFreezing);
    }
}
