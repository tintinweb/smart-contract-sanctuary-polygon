...
contract Marimo is ERC721, Ownable, ERC721Enumerable {
  ...
  uint256 public constant NUMBER_OF_TOKEN = 100;
  ...
  function getElapsedTimeFromLastWaterChanged(uint256 tokenId) external view returns (uint256) {
    require(_exists(tokenId), "nonexistent token");
    return block.timestamp - _lastWaterChangedAt[tokenId];
  }

  function getLastSize(uint256 tokenId) external view returns (uint256) {
    require(_exists(tokenId), "nonexistent token");
    return _lastSize[tokenId];
  }
  ...
  function publicMint() external returns(uint256) {
    _tokenIds.increment();
    _generatedAt[_tokenIds.current()] = block.timestamp;
    _lastWaterChangedAt[_tokenIds.current()] = block.timestamp;
    _lastSize[_tokenIds.current()] = 250;
