...
contract PULSEROLL is VRFConsumerBase {
    ...
    function playerRollDice(uint256 rollUnder,uint _OddEvenStatus ,uint rangeLowerLimit,uint rangeUpperLimit) public payable gameIsActive  CheckOddRangeSelect(rangeLowerLimit,rangeUpperLimit,_OddEvenStatus,rollUnder) betIsValid(rollUnder,_OddEvenStatus,rangeLowerLimit,rangeUpperLimit) CheckZero 
    returns(bytes32)
    {
    ...
    function GetMultiplier(uint _RollUnder,uint _OddEvenStatus,bool _IsRangeTrue,uint RangeLow,uint RangeHigh)public view CheckOddRangeSelect(RangeLow,RangeHigh,_OddEvenStatus,_RollUnder) returns(uint)
    {
        ...
        if(_OddEvenStatus==1 || _OddEvenStatus ==2)
        {
            ...
            Multiplier =Multiplier + (oddEvenMultiplier*(1e18));//It is Fixed everytime ti will give 2x
        }
        uint range;
        uint totalChances;
   ...
   //Owner functions
    ...
    function ChangeMulandOddBase(uint _mulBaseAll,uint _mulBaseOnlyRange)public onlyOwner{
    ...
    function ChangeBaseDivisorRange(uint _baseDivisorRange)public onlyOwner
    {
