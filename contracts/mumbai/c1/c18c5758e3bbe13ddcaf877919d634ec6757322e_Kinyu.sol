...
contract Kinyu is IERC20, IERC20Metadata {
    ...
    uint64 internal constant BASE_TOKENS_PER_SHARE_SCALE = 1e18;

    address _baseToken;
    uint8 _baseTokenDecimals;
    address payable _weth;

    uint _supplierBaseTokensPerShare = 1 * BASE_TOKENS_PER_SHARE_SCALE;
    uint _borrowerBaseTokensPerShare = 1 * BASE_TOKENS_PER_SHARE_SCALE;
    ...
    function supply(address token, uint amount) external {
        _supply(address(this), msg.sender, token, amount);
    }
    ...
    function supplyTo(
        ...
        _supply(address(this), destination, token, amount);
    }

    function supply() external payable {
        IWETH9(_weth).deposit{value: msg.value}();
        _supply(address(this), msg.sender, _weth, msg.value);
    }

    function supplyTo(address destination) external payable {
        IWETH9(_weth).deposit{value: msg.value}();
        _supply(address(this), destination, _weth, msg.value);
    }
    ...
    function _transferTokenIn(address token, uint amount) private {
        if (amount > uint(int(type(int128).max))) {
            ...
            return
                int128(
                    (baseTokens * int64(BASE_TOKENS_PER_SHARE_SCALE)) /
                        int(_borrowerBaseTokensPerShare)
                );
        } else {
            return
                int128(
                    (baseTokens * int64(BASE_TOKENS_PER_SHARE_SCALE)) /
                        int(_supplierBaseTokensPerShare)
                );
        }
    }
        ...
        returns (int baseTokens)
    {
        if (shares < 0) {
            return
                (shares * int(_borrowerBaseTokensPerShare)) /
                int64(BASE_TOKENS_PER_SHARE_SCALE);
        } else {
            return
                (shares * int(_supplierBaseTokensPerShare)) /
                int64(BASE_TOKENS_PER_SHARE_SCALE);
        }
    }
