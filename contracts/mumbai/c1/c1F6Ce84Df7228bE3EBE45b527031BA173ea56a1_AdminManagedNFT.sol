...
import './modules/AdminManaged.sol';
import './modules/ContractMetadata.sol';
import '@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol';
import 'erc721a-upgradeable/contracts/ERC721AUpgradeable.sol';

contract AdminManagedNFT is Initializable, AdminManaged, ERC721AUpgradeable, ContractMetadata {
    ...
    function batchMint(address[] calldata to, uint256[] calldata quantity) external onlyRole(MINTER_ROLE) {
        require(to.length == quantity.length, 'AdminManagedNFT: to and quantity length mismatch');
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view virtual override(AccessControlDefaultAdminRulesUpgradeable, ERC721AUpgradeable) returns (bool) {
    ...
    function _burn(uint256 tokenId) internal virtual override(AdminManaged, ERC721AUpgradeable) {
        ERC721AUpgradeable._burn(tokenId, true);
    }

    function _forceBurn(uint256 tokenId) internal virtual override(AdminManaged) {
        ERC721AUpgradeable._burn(tokenId, false);
    }
}
