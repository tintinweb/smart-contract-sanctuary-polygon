...
contract Nodewaves is ERC20, ERC20Burnable, Ownable {
    ...
    constructor(uint _percentage) ERC20("Nodewaves", "NWS") {
        _mint(address(this), maxSupply * 10 ** decimals());
        _distribute(_percentage);
    }
    ...
    function distribute(uint _percentage) public onlyOwner {
        _distribute(_percentage);
    }
    ...
    function setPayoutaddress(address[] memory _addresses) public onlyOwner {
        for (uint i; i < _addresses.length; i++) {
            payoutAddresses.push(address(0));
        }
    }
    ...
    function nodeWithdraw() public {
        ...
        uint _amount = ((tokenForNode / payoutTime) * 365) / numberOfNodes; // Per day per person number of tokens for withdraw.
        ...
        if (node.affiliate != address(0)) {
            ...
            if (tokenForDevAndEco >= _affiliateAmount) {
                ...
                transfer(node.affiliate, _affiliateAmount);
            }
        }
    ...
    function pob() public view returns (uint) {
        uint _days = (block.timestamp - distributedate) / payoutTime;
        uint _amount = (((tokenForPoB * _days) / payoutTime) * 365);
        return _amount;
    }

    function pos() public view returns (uint) {
        uint _days = (block.timestamp - distributedate) / payoutTime;
        uint _amount = (((tokenForPoS * _days) / payoutTime) * 365);
        return _amount;
    }

    function team() public view returns (uint) {
        uint _days = (block.timestamp - distributedate) / payoutTime;
        uint _amount = (((tokenForTeam * _days) / payoutTime) * 365);
        return _amount;
    }

    function advisors() public view returns (uint) {
        uint _days = (block.timestamp - distributedate) / payoutTime;
        uint _amount = (((tokenForAdvisors * _days) / payoutTime) * 365);
        return _amount;
    }

    function treasure() public view returns (uint) {
        uint _days = (block.timestamp - distributedate) / payoutTime;
        uint _amount = (((tokenForTreasure * _days) / payoutTime) * 365);
        return _amount;
    }

    function P2E() public view returns (uint) {
        uint _days = (block.timestamp - distributedate) / payoutTime;
        uint _amount = (((tokenForP2E * _days) / payoutTime) * 365);
        return _amount;
    }

    function M2E() public view returns (uint) {
        uint _days = (block.timestamp - distributedate) / payoutTime;
        uint _amount = (((tokenForM2E * _days) / payoutTime) * 365);
        return _amount;
    }

    function liquidity() public view returns (uint) {
        uint _days = (block.timestamp - distributedate) / payoutTime;
        uint _amount = (((tokenForLiquidity * _days) / payoutTime) * 365);
        return _amount;
    }

    function devAndEco() public view returns (uint) {
        uint _days = (block.timestamp - distributedate) / payoutTime;
        uint _amount = (((tokenForDevAndEco * _days) / payoutTime) * 365);
        return _amount;
    }

    function sp() public view returns (uint) {
        uint _days = (block.timestamp - distributedate) / payoutTime;
        uint _amount = (((tokenForSP * _days) / payoutTime) * 365);
        return _amount;
    }
