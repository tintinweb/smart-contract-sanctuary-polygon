...
contract limitOrderAndNormalSwapToken_NormalOrder is OpsTaskCreator, Ownable {
    ...
    function swapToken(address _user, bytes32 _md5) external {
        ...
        if (x.swapInAmount == 0) {
            ...
            require(userTokenAmountList[msg.sender][x._TokenItem.swapInToken].leftAmount >= x.spendSwapInToken, "U14");
            userTokenAmountList[msg.sender][x._TokenItem.swapInToken].leftAmount = userTokenAmountList[msg.sender][x._TokenItem.swapInToken].leftAmount.sub(x.spendSwapInToken);
            userTokenAmountList[msg.sender][x._TokenItem.swapInToken].usedAmount = userTokenAmountList[msg.sender][x._TokenItem.swapInToken].usedAmount.add(x.spendSwapInToken);
        }
