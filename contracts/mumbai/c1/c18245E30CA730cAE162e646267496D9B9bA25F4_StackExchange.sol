...
contract StackExchange is Ownable {
  ...
  constructor(address _baseToken, address _rewardToken, address _feeAccount) {
  ...
  function deductExchangeFee(uint amount) private returns (uint) {
    ...
    if (exchangeFee > 0) {
      if (!IERC20(baseToken).transfer(feeAccount, exchangeFee)) {
        revert("EXCHANGE_FEE_FAILED");
      }
    }
  ...
  function deductSwapFee(Pool storage pool, uint amount) private returns (uint) {
    ...
    if (swapFee > 0) {
      ...
      uint reward = rewardReserve - (((rewardPool.baseReserve - swapFee) * rewardReserve) / rewardPool.baseReserve);

      if (!IERC20(rewardToken).transfer(address(rewardTreasury), reward)) {
        revert("SWAP_FEE_FAILED");
      }
