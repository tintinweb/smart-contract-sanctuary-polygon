...
contract Factory is IFactory, IOwnable {
    ...
    RevenueShare public override rentShare;
    ...
    mapping(ITangibleNFT => mapping(uint256 => ITangibleFractionsNFT))
        public
        override fractions;
    mapping(ITangibleNFT => mapping(uint256 => IFractionStorageManager))
        public
         storageManagers;
    ...
    function setPassiveNft(address _passiveNft) external onlyOwner {
        ...
        if (address(passiveNft) != _passiveNft) {
            //set proper roles
            uint256 lengthTnfts = _tnfts.length;
            for (uint256 i = 0; i < lengthTnfts; i++) {
                AccessControl(address(_passiveNft)).grantRole(
                    REVENUE_MANAGER_ROLE,
                    address(_tnfts[i])
                );
                AccessControl(address(passiveNft)).revokeRole(
                    REVENUE_MANAGER_ROLE,
                    address(_tnfts[i])
                );
            }
            lengthTnfts = _tnftsFractions.length;
            for (uint256 i = 0; i < lengthTnfts; i++) {
                AccessControl(address(_passiveNft)).grantRole(
                    REVENUE_MANAGER_ROLE,
                    address(_tnftsFractions[i])
                );
                AccessControl(address(passiveNft)).revokeRole(
                    REVENUE_MANAGER_ROLE,
                    address(_tnftsFractions[i])
    ...
    function setRevenueShare(address _revenueShare) external onlyOwner {
        ...
        if (address(revenueShare) != _revenueShare) {
            ...
            uint256 lengthTnfts = _tnfts.length;
            for (uint256 i = 0; i < lengthTnfts; i++) {
                AccessControl(address(_revenueShare)).grantRole(
                    SHARE_MANAGER_ROLE,
                    address(_tnfts[i])
                );
                AccessControl(address(revenueShare)).revokeRole(
                    SHARE_MANAGER_ROLE,
                    address(_tnfts[i])
                );
            }
    ...
    function setRentShare(address _rentShare) external onlyOwner {
        ...
        if (address(rentShare) != _rentShare) {
            //set proper roles
            uint256 lengthTnfts = _tnfts.length;
            for (uint256 i = 0; i < lengthTnfts; i++) {
                AccessControl(address(_rentShare)).grantRole(
                    SHARE_MANAGER_ROLE,
                    address(_tnfts[i])
                );
                AccessControl(address(rentShare)).revokeRole(
                    SHARE_MANAGER_ROLE,
                    address(_tnfts[i])
                );
            }
            ...
            emit RentShareSet(address(rentShare), _rentShare);
            rentShare = RevenueShare(_rentShare);
        }
    }
    ...
    function setInstantLiquidity(address _instantLiquidity) external onlyOwner {
        ...
        if (instantLiquidity != _instantLiquidity) {
            emit InstantLiquidityAddressSet(
                instantLiquidity,
                _instantLiquidity
            );
    ...
    function setFractDeployer(address _fractDeployer) external onlyOwner {
        ...
        emit FractionsDeployerAddressSet(
            address(fractionsDeployer),
            _fractDeployer
        );
    ...
    function payTnftStorageWithManager(
        ...
        (uint256 tokenPrice,, , , ) = priceManager
            .getPriceOracleForCategory(tnft)
            .usdcPrice(tnft, 0, tokenId);
         uint256 amount = tnft.adjustStorageAndGetAmount(tokenId, _years, tokenPrice);
    ...
    ) external override onlyMarketplace returns (ITangibleFractionsNFT) {
