...
contract ContractFactory {
    ...
    * @dev Deploys smart contract using given params.
    ...
    /**
    * @dev Grants native token role to a new whitelisted contract.
    * @param newContract - a contract which will have a bridge caller role.
    */
    function _grantNativeTokenRole(address newContract) internal {
    ...
    function _createContractInstance(bytes memory bytecode, bytes32 bytecodeHash, bytes memory constructorParams,
        bytes32 collectionId, string calldata name, address owner
    ) internal returns (address) {
        address newContract = _deploy(bytecode, constructorParams, collectionId);

        if (contractNative[contractName[bytecodeHash]]) {
            _grantNativeTokenRole(newContract);
        }
     ...
     */
    function createContractInstance(bytes memory bytecode, bytes memory constructorParams, bytes32 collectionId, string calldata name, address owner) external returns (address) {
        return _createContractInstance(bytecode, keccak256(bytecode), constructorParams, collectionId, name, owner);
    }
    ...
    * @dev Creates contract instance for whitelisted byteCode.
     ...
     * @param owner - owner of the collection.
     */
    function createContractInstanceByHash(bytes32 bytecodeHash, bytes memory constructorParams, bytes32 collectionId, string calldata name, address owner) external returns(address) {
        require (hashRegistered[bytecodeHash], "Contract not registered");

        return _createContractInstance(contractsBytecode[bytecodeHash], bytecodeHash, constructorParams, collectionId, name, owner);
    }
...
contract LayerZeroBridge is ILayerZeroReceiver, IBridge {
    ...
    function _handleCall(CallData memory callData, uint16 srcChainId, bytes memory fromAddress, uint64 nonce) internal returns (bytes memory) {
        address target = systemContext.getCollectionRegistry().ownerOf(callData.collectionId);
    ...
    function _deploy(DeployData memory deployData) internal returns(address) {
        return systemContext.getContractFactory().createContractInstanceByHash(
            deployData.bytecodeHash, deployData.ctorParams, deployData.collectionId, deployData.collectionName, address(this)
        );
    }

    function _handleDeploy(DeployData memory deployData, uint16 srcChainId, bytes memory fromAddress, uint64 nonce) internal returns (address) {
        address deployedContractAddr = _deploy(deployData);
        emit ContractDeployed(srcChainId, fromAddress, nonce, deployedContractAddr);
        return deployedContractAddr;
    }

    function _handleERC721Bridge(ERC721BridgeData memory callData, uint16 srcChainId, bytes memory fromAddress, uint64 nonce) internal returns (address) {
        bytes memory ctorParams = abi.encode(callData.name, callData.symbol, callData.collectionId, address(this), systemContext);
        (address newAddress) = systemContext.getContractFactory().createContractInstanceByHash(
            callData.bytecodeHash, ctorParams, callData.collectionId, callData.name, address(this)
        );
        if (_isContract(newAddress)) {
    ...
    function _handleAll(
        ...
        if (data.operation == Operation.CALL) {
            ...
            _handleERC721Bridge(callData, _srcChainId, _fromAddress, _nonce);
        }
        else {
    ...
    function deployExternalCollection(
        ...
        uint256 chainId = systemContext.chainId();

        // TODO check if `params.originalCollection` supports interface ERC721
        _deploy(
            DeployData({bytecodeHash: params.bytecodeHash,
                ctorParams: abi.encode(systemContext, params.originalCollection, collectionId_),
                collectionId: collectionId_,
        ...
        for (uint256 i = 0; i < deploymentParams_.length; i++) {
            ...
            require(deploymentParams_[i].chainId != chainId, "Cannot deploy locally");

            _sendMessageWithValue(
                deploymentParams_[i].chainId,
                deploymentParams_[i].bridgeAddress,
                deployContractEncode(params.bytecodeHash, collectionId_, deploymentParams_[i].params, params.collectionName),
    ...
    function deployNativeCollection(
        ...
        for (uint256 i = 0; i < deploymentParams_.length; i++) {
            ...
            if(deploymentParams_[i].chainId == systemContext.chainId()) {
                _deploy(DeployData({bytecodeHash: params.bytecodeHash,
    ...
    // TODO check if we can proof that call multiple is safe
...
contract CollectionRegistry is ICollectionRegistry {
    ...
    */
    ...
    function getRecord(bytes32 collectionId) public view returns(Record memory) {
...
contract AccessControlList is AccessControlEnumerable {
    ...
    constructor(address admin) {
        ...
        _setRoleAdmin(CONTRACT_FACTORY_ROLE, CONTROL_LIST_ADMIN_ROLE);
    ...
    function grantNativeTokenRole(address addr) external onlyRole(CONTRACT_FACTORY_ROLE) {
 ...
 */
contract ERC721Mintable is ERC721Enumerable, AccessControl, ITokenBridgeable {
    ...
    */
    ...
    function moveTo(uint16 _l0ChainId, bytes calldata _destinationBridge, uint256 _tokenId) external override payable {
...
interface ICollectionRegistry {
    ...
    function registerCollection(bytes32 collectionId_, string calldata name_, address owner_, address collectionAddress_,
        string calldata contractName_, uint16 contractVersion_) external;
