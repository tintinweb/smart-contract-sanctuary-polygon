...
contract ZokuNFT is ERC721Enumerable, Ownable {
  using Counters for Counters.Counter;
  using EnumerableSet for EnumerableSet.UintSet;

  event TokenMinted(address to, uint256 tokenId, string tokenURI);
  ...
  Counters.Counter private _tokenIdCount = Counters.Counter({
    _value: 0
  });
  ...
  function initialize(address _settings) external initializer {
    ...
    _initialize(settings.tokenName(), settings.tokenSymbol());
    _transferOwnership(_msgSender());
    
    uri = settings.tokenBaseURI();
    maxSupply = settings.tokenMaxSupply();
  }

  modifier onlyMinter() {
    require(_msgSender() == minter || _msgSender() == owner(), "NFT: Caller not minter");
    _;
  }
  ...
  function mint(
    ...
    require(_tokenIdCount.current() <= maxSupply, "Max supply was reached out");
    ...
    (address factory, address sales, address marketplace) = settings.protocols();
    
    _setApprovalForAll(_to, factory, true);
    _setApprovalForAll(_to, sales, true);
  ...
  {
    address royaltiesReceiver = settings.royaltyFeeReceiver();
    uint256 royaltiesPercentage = settings.royaltyFee();
