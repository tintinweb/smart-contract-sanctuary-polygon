...
contract NFTMarketplace is ReentrancyGuard, Ownable {
    ...
    function createMarketSale(address nftContract, uint256 itemId) public payable nonReentrant {
        ...
        uint256 feeAmount = feePercent * price / 100;
        // transfer the (item price - royalty amount - fee amount) to the seller
        if (royaltyRecipient != address(0)) item.seller.transfer(price - feeAmount);
        else item.seller.transfer(price - royaltyAmount - feeAmount);
