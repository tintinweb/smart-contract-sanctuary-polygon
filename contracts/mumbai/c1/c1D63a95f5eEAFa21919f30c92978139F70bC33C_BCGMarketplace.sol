...
import "../BCG_Items/IBCGItemsIsERC1155.sol";
import "../openzeppelin/utils/Counters.sol";
import "./IBCGMarketplace.sol";

contract BCGMarketplace is IBCGMarketplace, Ownable, ERC1155Holder, ReentrancyGuard {

    IBCGItemsIsERC1155 itemsContract;
    IBCGTokenIsERC20 tokenContract;
    ...
    event NewListing(uint256 _offerId, address indexed _seller, uint256 _tokenId, uint256 _amount, uint256 _deadline, bool _isDeadline, uint256 _price);
    event CanceledListing (address indexed _seller, uint256 _tokenId, uint256 _amountOfToken);
    event NewSale(address indexed _buyer, address indexed _seller, uint256 _tokenId, uint256 _amountOfToken, uint256 _price);
    ...
    event CanceledBid (address indexed _bidder, uint256 _tokenId, uint256 _amountOfToken);
    event BidAccepted(address indexed _bidder, address indexed _seller, uint256 _tokenId, uint256 _amountOfToken, uint256 _price);

    constructor(address _tokenContract, address _itemsContract)  {
        tokenContract = IBCGTokenIsERC20(_tokenContract);
        itemsContract = IBCGItemsIsERC1155(_itemsContract);
    }
    ...
    function purchaseListing(uint256 _listingId) external nonReentrant {
        ...
        require(listingById[_listingId].seller != msg.sender, "You can't buy your own item");
        uint256 salePrice = listingById[_listingId].price;
        require(tokenContract.balanceOf(msg.sender) >= salePrice, "You don't have enough XPlay tokens to purchase this item");
    ...
    function cancelList(uint256 _listingId) external nonReentrant {
        require(listingById[_listingId].seller == msg.sender, "Cancel: should be the owner of the sell.");
        require(listingById[_listingId].isCanceled != true, "Cancel: offer already canceled.");
        require(listingById[_listingId].isSold != true, "Cancel: already sold.");
    ...
    function updateListPrice(uint256 _listingId, uint256 _newPrice) external {
        require(listingById[_listingId].seller == msg.sender, "Update price: should be the owner of the sell.");
        require(listingById[_listingId].isCanceled != true, "Update price: offer already canceled.");
        require(listingById[_listingId].isSold != true, "Update price: already sold.");

        listingById[_listingId].price = _newPrice;
    }

    function increaseListDeadline(uint256 _listingId, uint256 _secondsToAdd) external {
        require(listingById[_listingId].seller == msg.sender, "Increase deadline: should be the owner of the list.");
        require(listingById[_listingId].isCanceled != true, "Increase deadline: offer already canceled.");
        require(listingById[_listingId].isSold != true, "Increase deadline: already sold.");
        require(listingById[_listingId].isDeadline == true, "Increase deadline: no deadline set.");
    ...
    function createBid(uint256 _tokenId, uint256 _amountOfToken, uint256 _deadLine, bool _isDeadLine, uint256 _price) external nonReentrant {
        require(_amountOfToken > 0, "Amount can't be 0");
        require(_price > 0, "Price can't be 0");
        require(tokenContract.balanceOf(msg.sender) >= _price, "You don't have enough XPlay tokens to bid");
    ...
    function cancelBid(uint256 _bidId) external nonReentrant {
        require(bidById[_bidId].bidder == msg.sender, "Cancel: should be the owner of the bid.");
        require(bidById[_bidId].isCanceled != true, "Cancel: bid already canceled.");
        require(bidById[_bidId].isAccepted != true, "Cancel: already accepted.");
    ...
    function updateBidPrice(uint256 _bidId, uint256 _newPrice) external {
        require(bidById[_bidId].bidder == msg.sender, "Update: should be the owner of the bid.");
        require(bidById[_bidId].isCanceled != true, "Update: bid already canceled.");
        require(bidById[_bidId].isAccepted != true, "Update: bid already accepted.");

        bidById[_bidId].price = _newPrice;
    }

    function increaseBidDeadline(uint256 _bidId, uint256 _secondsToAdd) external {
        require(bidById[_bidId].bidder == msg.sender, "Increase: should be the owner of the bid.");
        require(bidById[_bidId].isCanceled != true, "Increase: bid already canceled.");
        require(bidById[_bidId].isAccepted != true, "Increase: already accepted.");
        require(bidById[_bidId].isDeadline, "Increase: no deadline set.");
