...
contract MerkleDistributor is AccessControl {
     ...
     * batchClaim
     */
    function batchClaim(uint256 roundID, uint256 amount, ClaimData[] calldata claimdata) public payable {
        Project storage project = round[roundID];
     ...
     * freeBatchMint
     */
    function freeBatchMint(uint256 roundID, ClaimData[] calldata claimdata) public {
        Project storage project = round[roundID];
        // Verify time
        require(project.startTime <= block.timestamp, "The Launchpad activity not started");
        require(project.endTime >= block.timestamp, "The Launchpad activity has ended");
    ...
    function freeClaim(
        ...
        // Verify the merkle proof.
        bytes32 node = keccak256(abi.encodePacked(roundID, index, num, calldataABI, msg.sender));
        if (!MerkleProof.verify(merkleProof, free ? project.vipMerkleRoot : project.merkleRoot, node))
            revert InvalidProof();
    ...
    function getProject(
        uint256 roundID
    ) external view returns (address, address, bytes32, address, uint256, uint256, uint256) {
        Project storage project = round[roundID];
        return (
            project.target,
            project.receipt,
    ...
    function getProjectV1(
        uint256 roundID
    ) external view returns (address, address, bytes32, bytes32, address, uint256, uint256, uint256) {
        Project storage project = round[roundID];
        return (
            project.target,
            project.receipt,
