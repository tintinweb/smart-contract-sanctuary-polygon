...
contract Vesting is IVesting, Ownable {
  ...
  // /// @notice Initialize vesting, register tokens for vesting
  // /// @dev can be invoked only one time by the owner
  // function initializer() external onlyOwner {
  //   require(!_initialized, "Contract is already initialized");

  //   _initialized = true;
  //   totalTokensAllocation = token.balanceOf(address(this));
  // }
