...
interface IMetadata {
    ...
    function supportMetadata(uint256 tokenId) external view returns (bool);
}
...
interface IPOP {
    function getWork(uint256 tokenId) external view returns (Work memory);

    function getExtra(uint8 origType) external view returns (Extra memory);
}
...
import "./interface/IPOP.sol";
import "base64-sol/base64.sol";
import "@openzeppelin/contracts/utils/Strings.sol";
import "./libs/DateTimeLibrary.sol";

contract POPMetadata is IMetadata {
    ...
    function supportMetadata(
        ...
        Work memory work = pop.getWork(tokenId);
        bytes memory hashb = bytes(work.fileHash);
        require(hashb.length == 46, "mismatch hash length");
        return true;
    }
    ...
    function tokenURI(
        ...
        return generateTokenUri(svg, tokenId);
    }
    ...
    function shortenTitle(
        ...
        (uint slen, uint blen) = raw.strlen();
        if (slen <= 40 && blen <= 60) {
            // at most 20个中文字符, 40英文字符
            return raw;
        }

        uint blen10times = blen * 10;
        if (blen == slen) {
            //  en
            return raw.shorten(40);
        } else if (blen10times < slen * 15) {
            return raw.shorten(45);
        } else if (blen10times >= slen * 15 && blen10times <= slen * 25) {
            // 大概一半中文一半英文
            return raw.shorten(51);
        } else {
            return raw.shorten(60); // 大部分是中文
        }
    }

    function nowDateTime(
        uint ts
    ) internal view returns (string memory datatime) {
        // block.timestamp
        (
            uint year,
            uint month,
            ...
            uint minute,
            uint second
        ) = DateTimeLibrary.timestampToDateTime(ts);
        // (GMT+8)
        hour = (hour + 8) % 24;
    ...
    function generateSVG(
        ...
        Work memory work = pop.getWork(tokenId);
        string memory title = shortenTitle(work.title);
        string memory author = work.author;
        Extra memory extra = pop.getExtra(work.origType);
        ...
        string memory nowDate = nowDateTime(work.timestamp);
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    '<text class="cs5" transform="translate(24.19 90.9)">',
                    unicode"文章标题：",
                    "</text>",
                    '<text class="cs6" transform="translate(41.6 90.9)">',
                    title,
                    "</text>",
                    '<text class="cs5" transform="translate(24.19 98.25)">',
                    extra.identity,
                    "</text>",
                    '<text class="cs6" transform="translate(41.6 98.25)">',
                    author,
                    "(",
                    Strings.toString(work.uid),
                    ")",
                    "</text>",
                    ...
                    "</text>",
                    '<text class="cs5" transform="translate(24.19 112.93)">',
                    unicode"存证时间：",
                    "</text>",
                    '<text class="cs6" transform="translate(41.6 112.93)">',
                    nowDate,
                    "</text>",
                    '<text class="cs5" transform="translate(24.19 120.27)">',
                    unicode"存证哈希：",
                    "</text>",
                    '<text class="cs6" transform="translate(41.6 120.27)">',
                    hashpart1,
                    "</text>",
                    '<text class="cs6" transform="translate(41.6 126.93)">',
                    hashpart2,
                    "</text>",
                    "</svg>"
                )
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                ...
                                hashb,
                                '"},',
                                '"external_url": "https://learnblockchain.cn"'
                                "}"
                            )
                        )
                    )
