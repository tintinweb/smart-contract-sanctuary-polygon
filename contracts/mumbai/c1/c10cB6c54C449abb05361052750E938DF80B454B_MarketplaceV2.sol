...
contract MarketplaceV2 is Ownable, ReentrancyGuard {
    ...
    */
    ...
    /**
        ...
        @param _addrs            A list of required addresses
            + _addrs[0]: Address owner
            + _addrs[1]: Address of NFT contract
        @param _tokenIds tokenIds of the NFT to mint
        @param _signatures 2 signatures of the minter and the BE verifier
            + _signatures[0]: Signatures of minter
            + _signatures[1]: Signatures of verifier
     */
    function mintMultiple(
        address[2] calldata _addrs,
        uint256[] calldata _tokenIds,
        bytes[2] calldata _signatures
    ) external {
        bytes32 _msgHash = ECDSA.toEthSignedMessageHash(
            keccak256(abi.encodePacked(_addrs[1], _addrs[0], _tokenIds))
        );
        _validateSig(_addrs[0], _msgHash, _signatures[0]);

        bytes32 verifierMsg = ECDSA.toEthSignedMessageHash(
            keccak256(abi.encodePacked(_signatures[0]))
        );
        require(
            ECDSA.recover(verifierMsg, _signatures[1]) == gov.verifier(),
            "Invalid params or signature verifier"
        );

        IMetaFarm721 _nftContr = IMetaFarm721(_addrs[1]);
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            _nftContr.mint(_addrs[0], _tokenIds[i]);
        }
    }
}
