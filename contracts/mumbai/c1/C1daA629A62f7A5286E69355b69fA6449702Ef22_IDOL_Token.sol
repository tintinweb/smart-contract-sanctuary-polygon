...
contract IDOL_Token {
    string public _name = "HIGH-DOL";
    string public constant _symbol = "IDOL";
    uint256 public Total_supply = 500000000000000000000000;
    ...
    uint256 public forthstackingEnd =1668988799;
    ...
    event forthroundStackingEnd(uint256 stavkingEnd);
    ...
    mapping(uint256 => mapping(address => uint256)) private numberOfTokensFirst;
    mapping(uint256 => mapping(address => uint256)) private numberOfTokensSecond;
    mapping(uint256 => mapping(address => uint256)) private numberOfTokensThird;
    mapping(uint256 => mapping(address => uint256)) private numberOfTokensForth;
    ...
    mapping(uint256 => mapping(address => uint256)) private locktimeFirst;
     mapping(uint256 => mapping(address => uint256)) private locktimeSecond;
     mapping(uint256 => mapping(address => uint256)) private locktimeThird;
      mapping(uint256 => mapping(address => uint256)) private locktimeforth;
    modifier onlyOwner() {
        ...
        
    {
        ...
        if (
            ...
            numberOfTokensSecond[_Txid][_buyer] = numOfTokens;
            SecondRoundnumToken_initialToken += numOfTokens;
            uint256 confirm_time =block.timestamp + 6048000;
            // uint256 confirm_time = block.timestamp + 120;
            locktimeSecond[_Txid][_buyer] = confirm_time;
        ...
        //ROUND 3 : Private VCs (31 Oct.- 06 Nov) Tokens 62,500
            ...
            numberOfTokensThird[_Txid2][_buyer] = numOfTokens;
            ThirdRoundnumToken_initialToken += numOfTokens;
            uint256 confirm_time =block.timestamp + 4838400;
            // uint256 confirm_time = block.timestamp + 120;
            locktimeThird[_Txid2][_buyer] = confirm_time;
     ...
     function PublicBuy(address _buyer, uint256 numOfTokens, bool _NeedStack )
        public onlyOwner{
            ...
            emit EForthroundTranfer(txIndex, msg.sender, _buyer, numOfTokens,block.timestamp,0);
                ...
                uint256 ForthroundStackEnd =1668988799;
                require(block.timestamp <= ForthroundStackEnd,"now cannot stack");
            if (count == 0) {
                if (balances[Team] > 0) {
                    balances[Team] -= firstRoundBalance;
                    balances[MainWallet] += firstRoundBalance;
            ...
            balances[Public] -= numOfTokens;
            numberOfTokensForth[_Txid3][_buyer] = numOfTokens;

            ForthRliquitityPool += numOfTokens;
            uint256 confirm_time = block.timestamp + 3628800;
            locktimeforth[_Txid3][_buyer] = confirm_time;
           ...
           pubTransactions.push(
            ...
            emit forthroundStackingEnd(ForthroundStackEnd);
            _Txid3 += 1;
            }
        ...
        notConfirmed(_txIndex)
    {
        ...
        numberOfTokensFirst[_txIndex][user] = VALUE;
        ...
        // uint256 confirm_time = block.timestamp + 120;
        locktimeFirst[_txIndex][user] = confirm_time;
        emit FirstRoundTransaction(
            user,
            _txIndex,
            FirstRoundnumToken_initialToken,
            numberOfTokensFirst[_txIndex][user],
    ...
    function FirstcalcuationofTotalSold() public onlyOwner returns (bool){
        ...
        uint256 fortstack = (Third_Round_End_Date + 1668908770);
    ...
    {
        require(
            block.timestamp >= locktimeFirst[_txIndex][_user],
        ...
        require(FirstRoundalreadyTranferred[_txIndex][_user]==false,"already Tranferred");
            uint256 transferredIdol = (numberOfTokensFirst[_txIndex][_user]+((numberOfTokensFirst[_txIndex][_user]*FirstRBonusAmount)));
    ...
    {
        require(
            block.timestamp >= locktimeSecond[_txIndex][_user],
            "Stacking Periods Not Over"
        );
        require(calcuationofTotal >=1, "Transfer to liquidty Pool First" );
        require(SecondRoundalreadyTranferred[_txIndex][_user]==false,"already Tranferred");
            uint256 transferredIdol = (numberOfTokensSecond[_txIndex][_user]+((numberOfTokensSecond[_txIndex][_user]*SecondRBonusAmount)));
        balances[_user] += transferredIdol;
    ...
    {
        require(
            block.timestamp >= locktimeThird[_txIndex][_user],
            "Stacking Periods Not Over"
        );
        require(calcuationofTotal >=1, "Transfer to liquidty Pool First" );
        require(ThirdRoundalreadyTranferred[_txIndex][_user]==false,"already Tranferred");
            uint256 transferredIdol = (numberOfTokensThird[_txIndex][_user]+((numberOfTokensThird[_txIndex][_user]*ThirdRBonusAmount)));
        balances[_user] += transferredIdol;
    ...
    {
        require(
            block.timestamp >= locktimeforth[_txIndex][_user],
            "Stacking Periods Not Over"
        );
        require(calcuationofTotal >=1, "Transfer to liquidty Pool First" );
        require(ForthRoundalreadyTranferred[_txIndex][_user]==false,"already Tranferred");
            uint256 transferredIdol = (numberOfTokensForth[_txIndex][_user]+((numberOfTokensForth[_txIndex][_user]*ForthRBonusAmount)));
        balances[_user] += transferredIdol;
