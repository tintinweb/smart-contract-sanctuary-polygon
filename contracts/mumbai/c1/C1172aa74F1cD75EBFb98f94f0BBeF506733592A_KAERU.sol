...
contract ERC721DelayedReveal is ERC721LazyMint, DelayedReveal {
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        (uint256 tokenId, ) = _getTokenId(_tokenId);
        string memory tokenUri = _getBaseURI(_tokenId);

        if (isEncryptedToken(tokenId)) {
            return string(abi.encodePacked(tokenUri, "0"));
        } else {
            return string(abi.encodePacked(tokenUri, _tokenId.toString()));
        }
    }
    ...
    function reveal(uint256 _index, bytes calldata _key) external virtual override returns (string memory revealedURI) {
        ...
        uint256 tokenId = getTokenIdAtIndex(_index);
        revealedURI = getRevealURI(tokenId, _key);

        _setEncryptedData(tokenId, "");
        _setBaseURI(tokenId, revealedURI);
...
contract BatchMintMetadata {
     ...
     */
    function getTokenIdAtIndex(uint256 _index) public view returns (uint256) {
    ...
    function _getTokenId(uint256 _tokenId) internal view returns (uint256 tokenId, uint256 index) {
        uint256 numOfTokenBatches = getBaseURICount();
        uint256[] memory indices = tokenIds;
    ...
    function _setBaseURI(uint256 _tokenId, string memory _baseURI) internal {
    ...
    ) internal returns (uint256 nextTokenIdToMint, uint256 tokenId) {
        tokenId = _startId + _amountToMint;
        nextTokenIdToMint = tokenId;
...
abstract contract DelayedReveal is IDelayedReveal {
    ...
    function _setEncryptedData(uint256 _tokenId, bytes memory _encryptedData) internal {
        encryptedData[_tokenId] = _encryptedData;
    }

    /**
     *  @notice             Returns revealed URI for a batch of NFTs.
     *  @dev                Reveal encrypted base URI for `_tokenId` with caller/admin's `_key` used for encryption.
     *                      Reverts if there's no encrypted URI for `_tokenId`.
     *                      See {encryptDecrypt}.
     *
     *  @param _tokenId     ID of the batch for which URI is being revealed.
     *  @param _key         Secure key used by caller/admin for encryption of baseURI.
     *
     *  @return revealedURI Decrypted base URI.
     */

    function getRevealURI(uint256 _tokenId, bytes calldata _key) public view returns (string memory revealedURI) {
        bytes memory data = encryptedData[_tokenId];
        if (data.length == 0) {
    ...
    /**
     *  @notice         Returns whether the relvant token of NFTs is subject to a delayed reveal.
     *  @dev            Returns `true` if `_tokenId`'s base URI is encrypted.
     *  @param _tokenId ID of a token of NFTs.
     */
    function isEncryptedToken(uint256 _tokenId) public view returns (bool) {
        return encryptedData[_tokenId].length > 0;
    }
}
...
contract KAERU is ERC721DelayedReveal {
    constructor(
        string memory _name,
        string memory _symbol,
        address _royaltyRecipient,
