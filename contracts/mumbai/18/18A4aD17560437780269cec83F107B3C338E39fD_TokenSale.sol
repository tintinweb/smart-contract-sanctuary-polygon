...
contract TokenSale is Initializable, OwnableUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping(address => Purchase[]) public purchases; // account=>Purchase[]
    ...
    function buy(address _token, uint256 _amount, address referrer) public nonReentrant returns (uint256) {
        ...
        purchases[_msgSender()].push(Purchase(amount, block.timestamp));
    ...
    function sell(address _token, uint256 amount) public nonReentrant returns (uint256) {
        require(amount <= IERC20MetadataUpgradeable(token).balanceOf(_msgSender()) / 2, "Invalid amount");
    ...
    function getLastMonthPurchasedAmount(address account) public view returns (uint256) {
        uint256 lastMonth = block.timestamp - 30 days;
        uint256 totalAmount;
        for (uint256 i = purchases[account].length; i > 0; ) {
            if (purchases[account][i - 1].timestamp >= lastMonth) {
                totalAmount += purchases[account][i - 1].amount;
            } else {
                break;
            }
