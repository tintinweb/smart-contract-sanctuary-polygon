...
contract SuperfluidOps {
    ...
    mapping(address => EnumerableSet.Bytes32Set) internal _createdAction;
    ...
    function createAction(
        ...
        bytes memory callAgreementData = getCallAgreementData(
            _action,
            _token,
            msg.sender,
            _receiver,
        ...
        bytes32 id = getActionId(callAgreementData, _actionTime);

        _createdAction[msg.sender].add(id);
    }
    ...
    function doAction(
        ...
        bytes32 id = getActionId(_callAgreementData, _actionTime);

        require(
            _createdAction[_user].contains(id),
        ...
        bytes32 opsTaskId = _getOpsTaskId(_callAgreementData, _actionTime);
    ...
    function getActionIdsByUser(address _user)
        external
        view
        returns (bytes32[] memory)
    {
        return _createdAction[_user].values();
    }

    function getActionId(bytes memory _callAgreementData, uint256 _actionTime)
        public
        pure
        returns (bytes32 id)
    {
        id = keccak256(abi.encode(_callAgreementData, _actionTime));
    }

    function getCallAgreementData(
        OperatorAction _action,
        address _token,
        address _sender,
        address _receiver,
    ...
    function _getOpsTaskId(bytes memory _callAgreementData, uint256 _actionTime)
        private
        view
        returns (bytes32 opsTaskId)
    {
        bytes memory resolverData = _getResolverData(
            _callAgreementData,
            _actionTime
