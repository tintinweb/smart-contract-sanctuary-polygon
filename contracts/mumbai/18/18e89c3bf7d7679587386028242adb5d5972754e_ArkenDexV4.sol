...
// File contracts/interfaces/ICurveTricryptoV2.sol
...
// File contracts/lib/DMMLibrary.sol
...
// File contracts/swapV4/ArkenDexTrader.sol
...
abstract contract ArkenDexTrader {
    ...
    function _weth() internal virtual returns (address) {}
    ...
    function _tradeRoute(
        ...
        if (
            ...
            (route.toToken == desc.dstToken ||
                (_ETH_ == desc.dstToken && _weth() == route.toToken))
        ) {
            require(
                route.to == address(0),
        ...
        if (route.from == address(0)) {
            amountIn =
                (IERC20(route.fromToken == wethDfyn ? _weth() : route.fromToken)
    ...
    function _tradeVyper(TradeRoute calldata route, uint256 amountIn) internal {
        require(route.from == address(0), 'route.from should be zero address');
        _increaseAllowance(route.fromToken, route.routerAddress, amountIn);
    ...
    function _transferFromSender(
        ...
        if (srcToken != _ETH_) {
            ...
            _wrapEther(_weth(), amount);
            if (to != address(this)) {
                IERC20(_weth()).safeTransfer(to, amount);
            }
        }
    }
...
// File contracts/swapV4/ArkenDexV4.sol
    ...
    ArkenDexTrader
{
    ...
    */
    function tradeOutside(
        ...
        if(dexIndex == 0){//Swap at 0x
            //call 0x ExchangeProxy Contract
            assembly {
                let freeMemoryPointer := mload(0x40)
                calldatacopy(freeMemoryPointer, interactionDataOutside.offset, interactionDataOutside.length)
                if iszero(
                    call(
                        gas(),
                        targetOutside,
            ...
            //call other dex aggre's contract
        }
