...
contract ImplementationSpecificFeeManager is Ownable {
    ...
    ) external payable returns (uint256 fetchedFees) {
     ...
     * @notice Fetch fees from valid feeDistributors contract when called externally
     */
    function fetchExactFees(address _implemetation, uint256 _feeAmount)
        external
        payable
        returns (uint256 fetchedFees)
    {
        fetchedFees = _fetchFees(_implemetation, true, _feeAmount);
    }
    ...
    function getImplementationFeeInfo(
        ...
        address _factoryCaller,
        bytes memory _encodedData
    ) external view returns (uint256 _feeAmount, address _feeToken) {
        ImplementationFeeInfo memory feeInfo = implementationFeeInfos[
