...
import "./../ExternalSwap/IJarvisDex.sol";
...
contract MockSynthereumMultiLpLiquidityPool is StandardAccessControlEnumerable {
    ...
    // mint flow
    // get price of jtry/usdc
    // calculate how many usdc to safeTransferFrom user
    // ---------- calculation ----------
    // price is in 8 decimals from chainlink
    // price = 0.052
    // 50 * 1e18 * 0.052 * 1e8 / 1e20

    function mint(
        IJarvisDex.MintParams memory params
    ) external returns (uint, uint) {
        // Calculate the amount of synthetic tokens to mint
        uint256 syntheticAmount = _mintHelper(params.collateralAmount);

        // Transfer the collateral tokens from the user's balance to the contract
        ...
        // Mint the synthetic tokens to the user's account
        IERC20(syntheticToken).safeTransfer(params.recipient, syntheticAmount);

        return (syntheticAmount, 0);
    }

    function redeem(
        IJarvisDex.RedeemParams memory params
    ) external returns (uint, uint) {
        ...
        // // Calculate the amount of collateral tokens to redeem
        uint256 collateralAmount = _redeemHelper(params.numTokens);
    ...
    function getMintTradeInfo(
        ...
        synthTokensReceived = _mintHelper(_collateralAmount);
    }
    ...
    {
        collateralAmountReceived = _redeemHelper(_syntTokensAmount);
    }

    function _redeemHelper(
        uint amountOfSyntheticTokens
    ) internal view returns (uint) {
        (, int priceFromFeed, , , ) = priceFeed.latestRoundData();
        return (amountOfSyntheticTokens * uint(priceFromFeed)) / 1e20;
    }

    function _mintHelper(
        uint amountOfCollateralTokens
    ) internal view returns (uint) {
        (, int priceFromFeed, , , ) = priceFeed.latestRoundData();
        return (amountOfCollateralTokens * 1e20) / uint(priceFromFeed);
    }

    function updatePriceFeed(
        AggregatorV3Interface _newFeed
    ) external onlyMaintainer {
        priceFeed = _newFeed;
    }
}
