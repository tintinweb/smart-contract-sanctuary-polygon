...
contract MEGAMATIC {
    ...
    function withdraw() public {
        ...
        for (uint i = 0; i < user.deposits.length; i++) {
            ...
            if (uint(user.deposits[i].withdrawn) < uint(user.deposits[i].amount).mul(42).div(5)) {
                ...
                if (uint(user.deposits[i].withdrawn).add(dividends) > uint(user.deposits[i].amount).mul(42).div(5)) {
                    dividends = (uint(user.deposits[i].amount).mul(42).div(5)).sub(uint(user.deposits[i].withdrawn));
                }
    ...
    function isActive(address userAddress) public view returns (bool) {
        ...
        return (user.deposits.length > 0) && uint(user.deposits[user.deposits.length-1].withdrawn) < uint(user.deposits[user.deposits.length-1].amount).mul(42).div(5);
    }
