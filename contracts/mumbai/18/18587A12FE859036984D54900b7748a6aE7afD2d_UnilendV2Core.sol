...
import "../lib/lib/utils/Address.sol";
import "../lib/lib/utils/math/SafeMath.sol";

import "../lib/lib/token/ERC20/IERC20.sol";
import "../lib/lib/token/ERC20/extensions/IERC20Metadata.sol";
import "../lib/lib/token/ERC20/utils/SafeERC20.sol";
import "../lib/lib/security/ReentrancyGuard.sol";
...
contract UnilendV2Core is ReentrancyGuard {
    ...
    function balanceOfUserToken0(
        address _pool,
        address _address
    ) external view returns (uint _lendBalance0, uint _borrowBalance0) {
        (address _token0, ) = getPoolTokens(_pool);
        if (_token0 != address(0)) {
            uint _nftID = IUnilendV2Position(positionsAddress).getNftId(
                _pool,
                _address
            );
            if (_nftID > 0) {
    ...
    function getUserHealthFactor(
        address _pool,
        address _address
    ) external view returns (uint _healthFactor0, uint _healthFactor1) {
        (address _token0, ) = getPoolTokens(_pool);
        if (_token0 != address(0)) {
            uint _nftID = IUnilendV2Position(positionsAddress).getNftId(
                _pool,
    ...
    function getPoolAvailableLiquidity(
        address _pool
    ) external view returns (uint _token0Liquidity, uint _token1Liquidity) {
        (address _token0, ) = getPoolTokens(_pool);
        if (_token0 != address(0)) {
    ...
    function setPoolInterestRateAddress(
        address _pool,
        address _address
    ) external onlyGovernor {
    ...
    function setDefaultInterestRateAddress(
        address _address
    ) external onlyGovernor {
        require(_address != address(0), "UnilendV2: ZERO ADDRESS");
...
// eslint-disable-next-line no-use-before-define
    function iLend(
        ...
        int _amount,
        uint _nftID
    ) internal returns (int mintedTokens) {
    ...
    function redeem(
        ...
        int _token_amount,
        address _receiver
    ) external nonReentrant returns (int redeemTokens) {
        (address _token0, ) = getPoolTokens(_pool);
        require(_token0 != address(0), "UnilendV2: POOL NOT FOUND");
    ...
    function liquidate(
        ...
        address _receiver,
        bool uPosition
    ) external onlyAmountNotZero(_amount) nonReentrant returns (int payAmount) {
        (address _token0, address _token1) = getPoolTokens(_pool);
        require(_token0 != address(0), "UnilendV2: POOL NOT FOUND");
        ...
        if (uPosition) {
            uint _toNftID = IUnilendV2Position(positionsAddress).getNftId(
                _pool,
                _receiver
            );
            if (_toNftID == 0) {
    ...
    function liquidateMulti(
        address _pool,
        uint[] calldata _nftIDs,
        int[] calldata _amounts,
        address _receiver,
        bool uPosition
    ) external nonReentrant returns (int payAmount) {
        (address _token0, address _token1) = getPoolTokens(_pool);
        require(_token0 != address(0), "UnilendV2: POOL NOT FOUND");
        require(
