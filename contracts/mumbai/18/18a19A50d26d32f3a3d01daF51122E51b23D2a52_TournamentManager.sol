...
contract TournamentManager is ERC1155("sample_uri"), Ownable, AccessControl {
    ...
    function getTournamentRewards(uint256 tournamentId)
        public
        view
        returns (uint256[] memory rewards)
    {
        rewards = new uint256[](tournamentRewardAmounts[tournamentId].length);
        for (
            uint256 i = 0;
            i < tournamentRewardAmounts[tournamentId].length;
            i++
        ) {
            rewards[i] = tournamentRewardAmounts[tournamentId][i];
        }
    }

    function getTournamentUnderlyingAssets(uint256 tournamentId)
        public
        view
        returns (string[] memory assets)
    {
        assets = new string[](underlyingAssets.length);
        for (uint256 i = 0; i < underlyingAssets.length; i++) {
            if (
                ...
                assets[i] = underlyingAssets[i].symbol;
            }
        }
    }
