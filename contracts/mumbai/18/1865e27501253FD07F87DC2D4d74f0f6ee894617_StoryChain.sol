...
contract StoryChain is ERC721, ERC721URIStorage, FunctionsClient, ConfirmedOwner {
  ...
  uint256 public fixedCost = 20000000000000000;
  ...
  constructor(
    ...
    categoryCost[1] = 20000000000000000;
    categoryCost[2] = 20000000000000000;
    storyAICost[0] = 50000000000000000;
    storyAICost[1] = 100000000000000000;
    imageAICost[0] = 20000000000000000;
    imageAICost[3] = 50000000000000000;
  }
  ...
  function continueStory(string calldata prompt, uint256 storyNonce) public payable {
    ...
    require(!s.voteFirst, "this story requires voting");
  ...
  function proposePrompt(string memory prompt, uint256 storyNonce) public payable {
    Story storage s = stories[storyNonce];
    require(s.voteFirst, "no voting on this story");
    require(s.pages > 0, "no pages yet");
    require(s.pageLimit == 0 || s.pages < s.pageLimit, "story is complete");
    require(!s.updating, "waiting for current page");
    require(msg.value >= calculateCost(s.category, s.storyAIid, s.imageAIid), "cant cover page cost");
    ...
    if (s.category == CATEGORY_ALL) {
      addPromptForVoting(prompt, storyNonce, msg.sender);
    } else {
      emit PromptEntered(prompt, storyNonce, s.pages, msg.sender);
    }
  }

  function getUserVoteOfStory(address user, uint256 storyNonce) public view returns (uint256) {
  ...
  function voteForPrompt(uint256 index, uint256 storyNonce) external {
    uint256 userVotes = getUserVoteOfStory(msg.sender, storyNonce);
    require(userVotes > 0, "you dont own nft to vote");
    Story memory s = stories[storyNonce];
    require(!s.lastPromptApproved, "already approved");
    require(proposedPrompts[storyNonce].length > index, "no prompt at index");
    PromptProposal storage pp = proposedPrompts[storyNonce][index];
    pp.votes += userVotes;
    if (pp.votes > currentTopPromptVote[storyNonce]) {
      currentTopPromptVote[storyNonce] = pp.votes;
      currentTopPromptIndex[storyNonce] = index;
    }
  }

  function addPromptForVoting(string memory prompt, uint256 storyNonce, address author) internal {
    require(!stories[storyNonce].lastPromptApproved, "already approved");
    uint256 index = proposedPrompts[storyNonce].length;
    PromptProposal storage pp = proposedPrompts[storyNonce][index];
    //proposedPrompts[storyNonce].push(pp);
    pp.prompt = prompt;
    pp.author = author;
    pp.votes = 1;
    if (currentTopPromptVote[storyNonce] == 0) currentTopPromptVote[storyNonce] = 1;
  }

  function executePropose(uint256 storyNonce) public {
    require(block.timestamp > storyLastUpdate[storyNonce] + votePeriod, "voting not over");
    require(proposedPrompts[storyNonce].length > 0, "no proposed prompt");
    PromptProposal memory pp = proposedPrompts[storyNonce][currentTopPromptIndex[storyNonce]];
    Story storage s = stories[storyNonce];
    s.lastPromptApproved = true;
    emit SafetyChecked("", "", "", storyNonce, s.pages, pp.author, pp.prompt);
  }
  ...
  function fulfillRequest(bytes32 requestId, bytes memory response, bytes memory err) internal override {
    ...
    if (s.pages == 0 && rejected) {
      ...
      if (!s.voteFirst) {
        ...
        addPromptForVoting(prompt, storyNonce, author);
      }
    }
  }
