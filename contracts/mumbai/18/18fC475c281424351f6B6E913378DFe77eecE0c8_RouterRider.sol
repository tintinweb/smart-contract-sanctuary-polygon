    ...
    ICrossTalkApplication
{
     ...
     * @param eventIdentifier event id of this transfer on gateway contract
     * @param sender address of sender who is executing cross chain tranfer
     * @param receivers array of receivers to whom ownership od rider will be transferred on dst chain
     ...
     * @param srcChainType src chain type
     ...
     * @param eventIdentifier event id of crosschain transfer event
     */
    event CrossChainReceived(uint256 eventIdentifier);
    ...
    /**
     * @notice this shows wheather this chain is minting chain or not, since there will a single chain on which minting will be done
     */
    bool public isMintingChain;
    /**
     ...
     * @param _isMintingChain sets to isMintingChain, if this contract is minting or not
    ...
    function initialize(
        ...
        isMintingChain = _isMintingChain;
        chainId = _chainId;
    ...
    function redeemRider() public {
        require(isMintingChain, "Chain: not minting chain");
    ...
    function enrollRemoteContract(
        ...
        // require(
        //     ourContractOnChains[_chainType][_chainId].length == 0,
        //     "already enrolled"
        // );
    ...
    function mapChainContract(
        ...
        require(
            _chainTypes.length < numberOfChainSupported,
     ...
     * @param _riderIds An array of token IDs of rider NFTs that need to be transferred.
     * @param receivers An array of receivers in the same order as the _riderIds array.
     */
    function crossChainTransfer(
        ...
        uint64 destGasPrice,
        uint64 ackGasPrice,
        uint256[] memory _riderIds,
        address[] memory receivers
    ) public payable {
        require(msg.value > 0, "fee not sent");
        uint256 noOfRider = _riderIds.length;
        require(noOfRider == receivers.length, "mismatch length");

        bytes[] memory payloads = new bytes[](noOfRider);
        bytes[] memory destinationContractAddresses = new bytes[](noOfRider);
        for (uint256 i = 0; i < noOfRider; i++) {
            uint256 riderId = _riderIds[i];
            require(
                _ownerOf(riderId) == msg.sender,
                "RP: transfer from incorrect owner"
            );
            require(
                riderMetadatas[riderId].lastRideTime + coolDownTime <
                    block.timestamp,
                "try again after cooldown"
            );
            require(
                receivers[i] != address(0),
                "RP: transfer to the zero address"
            );
            payloads[i] = abi.encode(
                riderId,
                tokenURI(riderId),
                msg.sender,
                riderMetadatas[riderId],
                receivers[i]
            );
            destinationContractAddresses[i] = ourContractOnChains[
                _dstChainType
            ][_dstChainId];
        }

        // token will be burned on ack
        uint64 eventIdentifier = CrossTalkUtils
            .multipleRequestsWithAcknowledgement(
                gatewayContract,
                Utils.RequestArgs(expiryTimestamp, true, Utils.FeePayer.USER),
                ...
                destinationContractAddresses, // destination contract address
                payloads
            );
    ...
    function handleRequestFromSource(
        ...
        (
            ...
            address receiver
                ...
                payload,
                (uint256, string, address, RiderMetadata, address)
            );
        ...
        return abi.encode(srcChainId, srcChainType, sender, riderId);
    }
     ...
     * @param eventIdentifier event id of cross chain transfer event
    ...
    function handleCrossTalkAck(
        ...
        // ACK_ON_SUCCESS
        for (uint256 i = 0; i < execData.length; i++) {
            bytes memory _execData = abi.decode(execData[i], (bytes));
            (
                string memory _chainId /*uint64 _chainType*/,
                ,
                ,
                /*address sender*/
                uint256 riderId
            ) = abi.decode(_execData, (string, uint64, address, uint256));
            require(
        ...
        emit CrossChainReceived(eventIdentifier);
    }
