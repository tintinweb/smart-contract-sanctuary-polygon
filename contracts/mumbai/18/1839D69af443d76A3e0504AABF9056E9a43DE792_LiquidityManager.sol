...
contract LiquidityManager is AddressAccessor, Pausable {
    ...
    function missingLiquidity() external view returns (uint256) {
        ...
        uint256 poolBalance;
        {
            ...
            uint256 usdrBalance = IExchange(exchange).scaleToUnderlying(
                IERC20(usdr).balanceOf(uniswapV3Pool)
            );
            poolBalance = (underlyingBalance + usdrBalance) >> 1;
        }
        if (poolBalance < minSize) return (minSize - poolBalance);
        */
        uint256 underlyingBalance = IERC20(underlying).balanceOf(uniswapV3Pool);
        return underlyingBalance < minSize ? (minSize - underlyingBalance) : 0;
    }
    ...
    function _splitTokenAmounts(
        ...
        if (scaledUSDRBalance < underlyingBalance) {
            ...
            IExchange(exchange).swapFromUnderlying(swapAmount, address(this));
        }
    }
}
