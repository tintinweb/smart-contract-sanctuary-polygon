...
contract AnimatedClipartNFTs is ERC721A,Whitelist {
  using Strings for uint256;
  ...
  uint256 public generalSaleCost = 0.10 ether;
  uint256 public maxSupply = 1111;
  uint256 public nftPerAddressLimitOnPreSale = 5;
  ...
  uint256 public halfSelloutReward = 4.05 ether;
  uint256 public fullSelloutReward = 8.11 ether;
  bool public halfSelloutRewardTransferred;
  bool public fullSelloutRewardTransferred;
  ...
  function _baseURI() internal view virtual override returns (string memory) {
    ...
    function communityWalletReward() internal {
        if(maxSupply.mul(50).div(100) < totalSupply() && !halfSelloutRewardTransferred && communityWallet != address(0)){
            if(address(this).balance >= halfSelloutReward){
                payable(communityWallet).transfer(halfSelloutReward);
                halfSelloutRewardTransferred = true;
            }
        }
        if(maxSupply == totalSupply() && !fullSelloutRewardTransferred && communityWallet != address(0)){
            if(address(this).balance >= fullSelloutReward){
                payable(communityWallet).transfer(fullSelloutReward);
                fullSelloutRewardTransferred = true;
            }
        }
    }
    ...
    function mint(uint _mintAmount) public payable {
        ...
        if(block.timestamp < generalSaleTime) {
            ...
            _safeMint(msg.sender, _mintAmount);
            addressMintedPresaleBalance[msg.sender] = addressMintedPresaleBalance[msg.sender].add(_mintAmount);
        }
        else{
            ...
            addressMintedGeneralsaleeBalance[msg.sender] = addressMintedGeneralsaleeBalance[msg.sender].add(addressMintedGeneralsaleeBalance[msg.sender]);
            _safeMint(msg.sender, _mintAmount);
        }
        communityWalletReward();
    }
  ...
  function setHalfSelloutReward(uint256 _halfSelloutReward) public onlyOwner {
    halfSelloutReward = _halfSelloutReward;
  }


  function setFullSelloutReward(uint256 _fullSelloutReward) public onlyOwner {
    fullSelloutReward = _fullSelloutReward;
  }
