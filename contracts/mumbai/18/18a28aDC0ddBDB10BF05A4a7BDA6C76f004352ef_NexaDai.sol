...
//-------------------[PRICE CHART STORAGE]----------------
    ...
    uint constant ROYALTY_VALIDITY=300;
...
// Replace below address with main token token
   ...
   struct userInfo {
    ...
    // FINANCIAL EVENT
    event regUserEv(uint ID,address user, address referral);
    ...
    function regUser( address _referral ) public returns(bool) 
    {
        ...
        emit regUserEv(lastIDCount,msg.sender, _referral);
   ...
   function _updateTeamNum(address ref) private {
    ...
    function _defaultUser(address _user) internal {
        ...
        userInfos[_user]=user;
         ...
         emit regUserEv(lastIDCount,_user,address(0));
