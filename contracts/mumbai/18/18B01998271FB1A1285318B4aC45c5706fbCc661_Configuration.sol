...
interface IConfig {
    ...
    function setWalletAddress(address developer_, address burnAddress_,address poolAddress_,address treasury_)
        external
        returns (bool);

    function getWalletAddress() external view returns (address, address,address,address);
...
contract Configuration is IConfig {
    ...
    uint256 public Aave_APY_Bet_Participant_Distribution;
    uint256 public Aave_APY_Admin_Distribution;
    ...
    {
        return (developerAddress, burnAddress,poolAddress,treasury);
    }
    ...
    function setAaveFeeConfig(
        uint256 aave_apy_bet_participant_distrubution_,
        uint256 aave_apy_bet_admin_distrubution_
    ) public override isAdmin returns (bool) {
        Aave_APY_Bet_Participant_Distribution = aave_apy_bet_participant_distrubution_;
        Aave_APY_Admin_Distribution = aave_apy_bet_admin_distrubution_;

        emit AaveFeeConfigUpdated(
            aave_apy_bet_participant_distrubution_,
            aave_apy_bet_admin_distrubution_
        );
    ...
    function getAaveConfig() public view override returns (uint256, uint256) {
        return (
            Aave_APY_Bet_Participant_Distribution,
            Aave_APY_Admin_Distribution
        );
    }
