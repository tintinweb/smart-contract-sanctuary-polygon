...
abstract contract Entity {
    /**
        - from: The seller's address
        - to: The non-custodial wallet's address
        - token: The payment token
        - amount: The amount of token to be on sale
        - _id: The mongo id of sell request
    */
    struct Transfer {
        address from;
        address to;
    ...
    function hash(Transfer memory transfer) internal pure returns (bytes32) {
        ...
        return ECDSAUpgradeable.toEthSignedMessageHash(digest);
    }
    ...
    function hash(Withdraw memory wd) internal pure returns (bytes32) {
        ...
        require(wd.tokens.length == wd.signatures.length);
        ...
        for (uint256 i = 0; i < len; i++) {
            encodedTokens = abi.encodePacked(encodedTokens, wd.tokens[i]);
            encodedAmounts = abi.encodePacked(encodedAmounts, wd.amounts[i]);
...
abstract contract Setting is Upgradeable {
...
import "./common/Entity.sol";
import "./common/Event.sol";
import "./common/Setting.sol";

contract Swap2Cash is Setting, Event, Entity {
    ...
    /**
        ...
        @param orderSignatures Signature of each order whose token to be withdrawn
    ...
    function withdraw(
        ...
        for (uint256 i = 0; i < len; i++) {
            _invalidateSignature(orderSignatures[i]);
            _transfer(address(this), msg.sender, wd.tokens[i], wd.amounts[i]);
        }
    ...
    function cancelSellOrder(
        ...
        _transfer(address(this), msg.sender, token, amount);
    ...
    function emergencyWithdraw(address[] calldata tokens) external onlyOwner {
        ...
        for (uint i = 0; i < len; i++) {
            amounts[i] = _withdraw(tokens[i], msg.sender);
        }
