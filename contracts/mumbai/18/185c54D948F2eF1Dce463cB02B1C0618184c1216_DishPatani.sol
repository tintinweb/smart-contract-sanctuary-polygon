...
contract DishPatani is ERC721URIStorage,Ownable{
    struct Schema{
        string name;
        string image;
        string description;
    ...
    function updatePoint(uint _tokenId,uint _points) public onlyOwner{
    ...
    function revealCard(address _to,uint _tokenId,uint _templateId) external onlyOwner{
        require(_tokenId/BASE_TEMPLATE_ID == 1 ,'Error: Invalid card to reveal');
        require(ownerOf(_tokenId) == _to, 'Error: Invalid Token Owner');
        _burn(_tokenId);
        delete metaData[_tokenId];
        uint mintedToken = mint(_to,_templateId);
        emit randomMint(_to,_templateId,mintedToken);    
    }
 ...
 function setBaseTokenURI(string calldata _uri) external onlyOwner() {
    ...
    function burn(uint _tokenId) public {
        require(ownerOf(_tokenId) != address(0), 'Error: Invalid TokenId!!');
        require(ownerOf(_tokenId) == msg.sender, 'Error: Only Token Owner can Burn'); 
        _burn(_tokenId);
    }
