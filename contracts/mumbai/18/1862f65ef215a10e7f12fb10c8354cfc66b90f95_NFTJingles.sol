...
contract NFTJingles is ERC1155, Ownable {
    ...
    function selectOptionsMint(uint _id, uint _amount, string memory _contact, string memory _selection) public {
        userDetails.push(UserDetails(msg.sender, _contact, _amount, _selection));
        mint(msg.sender, _id, _amount);
    }

    function getUserDetails(uint _index) external view onlyAdmins returns (address userAddress, string memory contact, uint256 amountMinted,string memory selectionID) {
        UserDetails storage userDetail = userDetails[_index];
        return (userDetail.userAddress, userDetail.contact, userDetail.amountMinted, userDetail.selectionID);
    }
