...
/// @title: manifold.xyz 1155 Creator Contract
/// @author: manifold.xyz

import "./manifold/ERC1155Creator.sol";

// This contract should be the name of the user's NFT collection

contract NAME is ERC1155Creator {
    constructor(address _trustedForwarder) ERC1155Creator() {
        BaseRelayRecipient.setTrustedForwarder(_trustedForwarder);
    }

    function setTrustedForwarder(address _forwarder) override internal virtual onlyOwner {
 ...
 */
abstract contract ERC1155Creator is AdminControl, ERC1155, ERC1155CreatorCore {
    ...
    function mintExtensionNew(address[] calldata to, uint256[] calldata amounts, string[] calldata uris) public virtual override nonReentrant extensionRequired returns(uint256[] memory tokenIds) {
        return _mintNew(_msgSender(), to, amounts, uris);
    }
    ...
    function mintExtensionExisting(address[] calldata to, uint256[] calldata tokenIds, uint256[] calldata amounts) public virtual override nonReentrant extensionRequired {
        for (uint i = 0; i < tokenIds.length; i++) {
            require(_tokensExtension[tokenIds[i]] == address(_msgSender()), "A token was not created by this extension");
        }
        _mintExisting(_msgSender(), to, tokenIds, amounts);
    }
...
abstract contract ERC1155CreatorCore is CreatorCore, IERC1155CreatorCore {
    ...
    function setApproveTransferExtension(bool enabled) external override extensionRequired {
        require(!enabled || ERC165Checker.supportsInterface(_msgSender(), type(IERC1155CreatorExtensionApproveTransfer).interfaceId), "Extension must implement IERC1155CreatorExtensionApproveTransfer");
        if (_extensionApproveTransfers[_msgSender()] != enabled) {
            _extensionApproveTransfers[_msgSender()] = enabled;
            emit ExtensionApproveTransferUpdated(_msgSender(), enabled);
        }
    }
    ...
    function _setMintPermissions(address extension, address permissions) internal {
        ...
        if (_extensionPermissions[extension] != permissions) {
            ...
            emit MintPermissionsUpdated(extension, permissions, _msgSender());
        }
    }
    ...
    function _checkMintPermissions(address[] memory to, uint256[] memory tokenIds, uint256[] memory amounts) internal {
        if (_extensionPermissions[_msgSender()] != address(0x0)) {
            IERC1155CreatorMintPermissions(_extensionPermissions[_msgSender()]).approveMint(_msgSender(), to, tokenIds, amounts);
        }
    }
 ...
 */
abstract contract CreatorCore is ReentrancyGuard, ICreatorCore, ERC165, BaseRelayRecipient {
    ...
    modifier extensionRequired() {
        require(_extensions.contains(_msgSender()), "Must be registered extension");
        _;
    }
    ...
    function _registerExtension(address extension, string calldata baseURI, bool baseURIIdentical) internal {
        ...
        if (!_extensions.contains(extension)) {
            ...
            emit ExtensionRegistered(extension, _msgSender());
    ...
    function _unregisterExtension(address extension) internal {
       if (_extensions.contains(extension)) {
           emit ExtensionUnregistered(extension, _msgSender());
    ...
    function _blacklistExtension(address extension) internal {
       ...
       if (!_blacklistedExtensions.contains(extension)) {
           emit ExtensionBlacklisted(extension, _msgSender());
    ...
    function _setBaseTokenURIExtension(string calldata uri, bool identical) internal {
        _extensionBaseURI[_msgSender()] = uri;
        _extensionBaseURIIdentical[_msgSender()] = identical;
    }
    ...
    function _setTokenURIPrefixExtension(string calldata prefix) internal {
        _extensionURIPrefix[_msgSender()] = prefix;
    }
    ...
    function _setTokenURIExtension(uint256 tokenId, string calldata uri) internal {
        require(_tokensExtension[tokenId] == _msgSender(), "Invalid token");
...
abstract contract BaseRelayRecipient is IRelayRecipient {
    ...
    function setTrustedForwarder(address _forwarder) internal virtual {
