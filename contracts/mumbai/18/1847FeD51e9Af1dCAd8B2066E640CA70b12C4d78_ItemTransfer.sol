...
contract ItemTransfer is IItemTransfer, GameBase {
    constructor(address game)
    GameBase(game) {
    }

    // TODO: add access control modifier
    function transfer(TransferRecord calldata record) external virtual override {
        TransferValidationResult memory result = this.validateTransfer(record);
        require(result.validTokenId, "wrong tokenId");
        require(result.validTargetTokenId, "wrong targetTokenId");
        require(result.validLength, "length of itemDefinitionIds and amounts do not match");
        for (uint256 i; i < record.itemDefinitionIds.length; i++) {
            require(result.validTransferables[i], "is not transferable item");
            require(result.validAmounts[i], "wrong amount");
        }

        Character from = getCharacter(record.worldId, record.tokenId);
        Character to = getCharacter(record.worldId, record.targetTokenId);
        for (uint256 i; i < record.itemDefinitionIds.length; i++) {
            uint256 itemId = record.itemDefinitionIds[i];
            int64 amount = record.amounts[i];

            from.addItem(itemId, amount * -1);
            to.addItem(itemId, amount);
        }

        emit TransferItems(record.worldId, record.tokenId, record.targetTokenId, record.itemDefinitionIds, record.amounts);
    }

    function validateTransfer(TransferRecord calldata record) external view virtual override returns(TransferValidationResult memory) {
        World world = getWorld(record.worldId);
        Character from = getCharacter(record.worldId, record.tokenId);
        Character to = getCharacter(record.worldId, record.targetTokenId);
        uint256[] calldata itemDefinitionIds = record.itemDefinitionIds;

        return TransferValidationResult(
            address(world) != address(0),
            address(from) != address(0),
            address(to) != address(0),
            itemDefinitionIds.length <= 100 && itemDefinitionIds.length == record.amounts.length,
            _validateTransferables(record),
            _validateAmounts(record)
        );
    }

    function _validateTransferables(TransferRecord calldata record) private view returns(bool[] memory) {
        Character from = getCharacter(record.worldId, record.tokenId);
        IItemDefinition itemDefinition = getItemDefinition(record.worldId);
        uint256[] calldata itemDefinitionIds = record.itemDefinitionIds;

        bool[] memory validTransferables = new bool[](itemDefinitionIds.length);
        if (address(from) != address(0)) {
            for (uint256 i; i < itemDefinitionIds.length; i++) {
                (,,, bool transferable,) = itemDefinition.getDefinition(itemDefinitionIds[i]);
                validTransferables[i] = transferable;
            }
        } else {
            for (uint256 i; i < itemDefinitionIds.length; i++) {
                validTransferables[i] = false;
            }
        }

        return validTransferables;
    }

    function _validateAmounts(TransferRecord calldata record) private view returns(bool[] memory) {
        Character from = getCharacter(record.worldId, record.tokenId);
        uint256[] calldata itemDefinitionIds = record.itemDefinitionIds;
        int64[] calldata amounts = record.amounts;

        bool[] memory validAmounts = new bool[](itemDefinitionIds.length);
        if (address(from) != address(0)) {
            int64[] memory items = from.getItems(itemDefinitionIds);
            for (uint256 i; i < itemDefinitionIds.length; i++) {
                validAmounts[i] = items[i] >= amounts[i] && amounts[i] > 0;
            }
        } else {
            for (uint256 i; i < itemDefinitionIds.length; i++) {
                validAmounts[i] = false;
            }
        }
...
contract World is Ownable {
    IItemPackNFT[] public itemPackNFTs;
    ...
    function addItemPackNFT(address itemPackNFT) public {
        itemPackNFTs.push(IItemPackNFT(itemPackNFT));
    }
...
contract ItemPack {
    ...
    // TODO: add access control modifier
    function burnByNFT(address nftAddress, uint256 tokenId) public virtual {
        _burn(_itemPackIdsByNft[nftAddress][tokenId]);
    }
    ...
    function _mint(uint256 itemPackDefinitionId_, address playerWallet, address nftAddress, uint256 tokenId) internal virtual {
        ...
        if (nftAddress != address(0) && tokenId != 0) {
            _itemPackIdsByNft[nftAddress][tokenId] = _currentIndex;
        }

        _itemPackRecords[_currentIndex] = ItemPackRecord(
            _currentIndex,
            itemPackDefinitionId_,
            playerWallet,
            nftAddress,
    ...
    function _burn(uint256 itemPackId) internal virtual {
        address playerWallet = _itemPackRecords[itemPackId].playerWallet;
        if (playerWallet != address(0)) {
            _itemPackIdsByWallet[playerWallet].removeById(itemPackId);
        }

        address nftAddress = _itemPackRecords[itemPackId].nftAddress;
        uint256 tokenId = _itemPackRecords[itemPackId].tokenId;
        if (nftAddress != address(0) && tokenId != 0) {
            ItemPackNFT nft = ItemPackNFT(nftAddress);
            nft.burn(tokenId);
        }

        _itemPackRecords[itemPackId] = ItemPackRecord(itemPackId, 0, address(0), address(0), 0, true);
    }
...
contract EventCheckin is IEventCheckin, GameBase {
    ...
    function _checkExecutableTimes(address playerWallet_, uint256 worldId_, uint256 eventDefinitionId) private view returns(bool) {
        ...
        uint256 currentCount = checkinCountsPerEvent[eventDefinitionId];
    ...
    function _checkExecutableTimesPerPlayer(address playerWallet_, uint256 worldId_, uint256 eventDefinitionId) private view returns(bool) {
        ...
        uint256 count = checkinCountsPerPlayer[playerWallet_][eventDefinitionId];
    ...
    function _checkInterval(address playerWallet_, uint256 worldId_, uint256 eventDefinitionId) private view returns(bool) {
        ...
        uint256 lastCheckinTimestamp = checkinTimeStampPerPlayer[playerWallet_][eventDefinitionId];
...
contract ItemPackNFT is ERC721AQueryable, IItemPackNFT, Ownable {
    string private _baseTokenURI;
    bool private _enabled = false;

    // key: tokenId, value: itemPackDefinitionId
    mapping(uint256 => uint256) private _itemPackDefinitionIds;
    ItemPack public itemPack;
...
//            uint256 itemPackDefinitionId = _itemPackDefinitionIds[currentId];
//            itemPack.burnByNFT(address(this), currentId);
//            itemPack.mintByNFT(address(this), currentId, itemPackDefinitionId);
//        }
//    }

    // TODO: update access control modifier
    function setBaseTokenURI(string memory baseTokenURI) public virtual onlyOwner {
    ...
    // TODO: update access control modifier
    function setEnabled(bool enabled) external virtual onlyOwner {
        _enabled = enabled;
    }

    // TODO: update access control modifier
    function mint(address to, uint256 quantity, uint256 itemPackDefinitionId) external virtual onlyOwner {
        uint256 nextTokenId = _nextTokenId();
        for (uint256 i; i < quantity; i++) {
            _itemPackDefinitionIds[nextTokenId + i] = itemPackDefinitionId;
        }
    ...
    function burn(uint256 tokenId) external virtual {
        ...
        itemPack.mintToWallet(ownerOf(tokenId), _itemPackDefinitionIds[tokenId]);

        _burn(tokenId);
    }

    function getTokens(address owner) external view override returns(uint256[] memory) {
        return this.tokensOfOwner(owner);
    }

    function setItemPack(address itemPack_) public {
        itemPack = ItemPack(itemPack_);
    }
}
