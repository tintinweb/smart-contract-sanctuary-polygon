...
import "../interface/token/IToken.sol";
import "../error/Errors.sol";
import "hardhat/console.sol";

contract BlueOceanProgram is IToken, ERC20, Ownable, Base {
    address public minter;
    address public treasury;
    address public burner;

    uint256 public maxSupply = 35_000_000 * 10**decimals();

    constructor(address minter_, address treasury_)
        ERC20("Blue Ocean Program", "BOP")
        checkNonZeroAddress(minter_)
        checkNonZeroAddress(treasury_)
    {
        minter = minter_;
        treasury = treasury_;
    }
    ...
    function mint(uint256 amount)
        external
        checkExpectedCaller(msg.sender, minter)
    {
    ...
    function burn(uint256 amount)
        external
        checkExpectedCaller(msg.sender, burner)
    {
        _burn(burner, amount);
    }
    ...
    function decimals()
        ...
        view
        virtual
        override(ERC20, IToken)
        returns (uint8)
    {
        return 18;
    }

    function setMinter(address acc)
        external
        onlyOwner
        checkNonZeroAddress(acc)
    {
        address old = minter;
        minter = acc;

        emit MinterChanged(old, acc);
    }

    function setBurner(address acc)
        external
        onlyOwner
        checkNonZeroAddress(acc)
    {
        ...
        emit BurnerChanged(old, acc);
    }

    function setTreasury(address acc)
        external
        onlyOwner
        checkNonZeroAddress(acc)
    {
...
interface IToken is IERC20 {
    ...
    /// @notice Emitted when the tx would exceed the token's max supply
    error MaxSupplyExceeded(uint256 maxSupply, uint256 exceeded);

    /* --------------------------------- Events --------------------------------- */
    event MinterChanged(address from, address to);
    event BurnerChanged(address from, address to);
