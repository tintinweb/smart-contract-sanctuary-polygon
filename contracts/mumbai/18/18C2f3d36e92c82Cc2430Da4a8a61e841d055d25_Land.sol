...
contract Land {
    ...
    function registerUser(string memory _name, uint _age, string memory _city,string memory _Cnic_Number, string memory _NTN_Number, string memory _document, string memory _email
    ) public {
        ...
        UserMapping[msg.sender] = User(msg.sender, _name, _age, _city,_Cnic_Number,_NTN_Number, _document,_email,false);
    ...
    //-----------------------------------------------Land-----------------------------------------------
    function  addLand (uint _area, string memory _address, uint _landPrice,string memory _allLatiLongi, uint _propertyPID,string memory _surveyNum, string memory _document)  public {
        require(isUserVerified(msg.sender));
        landsCount++;
        lands[landsCount] = Landreg(landsCount, _area, _address, _landPrice,_allLatiLongi,_propertyPID, _surveyNum , _document,false,payable(msg.sender),false);
