  ...
  ProviderFees
{
  ...
  */
  ...
  function _safeMint(address to, uint256 timeUnits, bool adminMint) internal {
  ...
  function _safeMintLifetime(
    ...
    emit Transfer(address(0), to, startTokenId);
    require(
        _checkOnERC721Received(address(0), to, startTokenId, ""),
        "ERC721A: transfer to non ERC721Receiver implementer"
    );
  ...
  function _reclaimLifetime(address to) internal {
    uint256 tokenId = findExpiredTokenId();
    TokenOwnership memory prevOwnership = ownershipOfExpiry(tokenId);
  ...
  */
  ...
  // @dev Allow the owner to increase the collection size of the collection. If minted out should allow normal mints again
  // even if recycle cycles have already run.
  function increaseCollectionSize(uint256 _newCollectionSize) public onlyTeamOrOwner {
    if(_newCollectionSize <= collectionSize) revert InvalidCollectionResize();
    ...
    ReentrancyGuard
{
    ...
    */
    ...
    function mintToLifetimeAdmin(address _to) public onlyTeamOrOwner {
      if (currentTokenId() != collectionSize) {
        if(currentTokenId() + 1 > collectionSize) revert CapExceeded();
        _safeMintLifetime(_to, true);
        return;
      }
      _reclaimLifetime(_to);
    }
    ...
    */
    ...
    function mintToERC20AL(address _to, uint256 _timeUnits, bytes32[] calldata _merkleProof, address _erc20TokenContract) public payable {
      if(_timeUnits == 0) revert MintZeroQuantity();
      if(!inAllowlistMint()) revert AllowlistMintClosed();
      if(!isAllowlisted(_to, _merkleProof)) revert AddressNotAllowlisted();
      if(!canMintQty(_timeUnits)) revert ExcessiveOwnedMints();
