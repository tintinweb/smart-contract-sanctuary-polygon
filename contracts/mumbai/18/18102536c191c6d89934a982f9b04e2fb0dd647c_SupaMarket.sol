...
// File contracts/SupaMarket.sol

error InvalidOrderType();
error InvalidSignature();
error NotOwner(address user, address collectionAddress, uint256 itemId);
error NoBalance(address user, address coinAddress, uint256 price);

contract SupaMarket is
    ERC2771Context,
    Initializable,
    ReentrancyGuard,
    SignatureVerifier
{
    // Buy order => Someone making an offer on an item
    bytes32 public immutable BUY_ORDER = keccak256("BUY_ORDER");
    // Sell order => Someone listing their item for sale
    bytes32 public immutable SELL_ORDER = keccak256("SELL_ORDER");
    struct Used {
        bool used;
    }
    mapping(uint256 => Used) usedNonces;
    struct Order {
        // Item data
    ...
    modifier unusedNonce(uint256 nonce) {
        require(usedNonces[nonce].used == false, "Order already executed");
        _;
    }
    ...
    function getOrderHash(
        ...
        if (order.orderType != BUY_ORDER && order.orderType != SELL_ORDER)
    ...
    // Checks for ERC20 allowance and ERC721 approval need to be done manually previous to this
    // Preferrably before creating order on frontend
    function executeOrder(
        Order calldata order,
        bytes calldata signature
    ) public unusedNonce(order.nonce) nonReentrant {
        bytes32 orderHash = getOrderHash(order);
        // Validate signature
        if (!verify(orderHash, signature, order.makerAddress)) {
            // Signature invalid => Revert
            revert InvalidSignature();
        }
        IERC721 collection = IERC721(order.collectionAddress);
        IERC20 coin = IERC20(order.tokenAddress);
        // If order is BUY
        if (order.orderType == BUY_ORDER) {
            // Guards
            // User (_msgSender) needs to be owner
            if (_msgSender() != collection.ownerOf(order.itemId))
                revert NotOwner(
                    _msgSender(),
                    order.collectionAddress,
                    order.itemId
                );
            // Maker needs to have balance of coin
            if (coin.balanceOf(order.makerAddress) < order.tokenAmount)
                revert NoBalance(
                    order.makerAddress,
                    order.tokenAddress,
                    order.tokenAmount
            ...
            //  Able to accept
            //  Transfer coin FROM order.makerAddress TO msgSender
            coin.transferFrom(
                order.makerAddress,
                _msgSender(),
                order.tokenAmount
            );
            //  Transfer collection item FROM msgSender TO order.makerAddress
            collection.safeTransferFrom(
                _msgSender(),
                order.makerAddress,
                order.itemId
            );
            usedNonces[order.nonce].used = true;
            emit OrderExecuted(_msgSender(), order.nonce, order);
        }
        // If order is SELL
        if (order.orderType == SELL_ORDER) {
            // Guards
            // Maker needs to be owner of item
            if (
                order.makerAddress !=
                IERC721(order.collectionAddress).ownerOf(order.itemId)
            )
                revert NotOwner(
                    order.makerAddress,
                    order.collectionAddress,
            ...
            // User (_msgSender) needs to have enough balance
            if (coin.balanceOf(_msgSender()) < order.tokenAmount) {
                revert NoBalance(
                    _msgSender(),
                    order.tokenAddress,
                    order.tokenAmount
            ...
            //  Able to accept
            //  Transfer coin FROM msgSender TO order.makerAddress TO
            coin.transferFrom(
                _msgSender(),
                order.makerAddress,
                order.tokenAmount
