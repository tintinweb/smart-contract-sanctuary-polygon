...
contract Metaverse is Ownable {
    ...
    function removeMetaverseEvent(uint256 _id) external onlyOwner {
        require(
            metaverseEvents[_id].id != 0,
            "Metaverse: airdrop event is not exsited"
        );

        delete metaverseEvents[_id];
    }
    ...
    function claim721Event(uint256 _id, uint256 _tokenId) external {
        ...
        nftContract.mint(userAddress, _tokenId);
        metaverseEventClaims[_id][userAddress] = true;
        emit MetaverseEventClaimed(
            userAddress,
            _id,
