...
contract ChainlinkOracle is ChainlinkClient, AccessControlEnumerable {
  ...
  uint256 private constant ORACLE_PAYMENT = ((1 * LINK_DIVISIBILITY) / 100) * 10;
  ...
  function requestFloor(string calldata _collection, string calldata _pricingAsset) external onlyRole(REQUESTOR_ROLE) {
    ...
    requests[reqId] = Request(uint64(block.timestamp), _collection, _pricingAsset, reqId);
    emit RequestSubmited(reqId, block.timestamp, _collection, _pricingAsset);
  }
