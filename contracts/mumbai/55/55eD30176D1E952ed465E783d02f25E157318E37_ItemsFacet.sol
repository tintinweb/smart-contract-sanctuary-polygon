...
contract ItemsFacet is ERC1155Supply, IItems {
  ...
  modifier onlySBD() {
    require(_msgSender() == address(this),
      "ItemsFacet: function can only be called from inside diamond");
    _;
  }

  function mint(address to, uint256 id, uint amount) external onlySBD {
    ERC1155Supply._mint(to, id, amount, '');
  }

  function burn(address from, uint256 id, uint amount) external onlySBD {
