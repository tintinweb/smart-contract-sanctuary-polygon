...
contract MSNFT is ERC721, Ownable {
    using Address for address;
    ...
    modifier isMintable() {
        require(totalSupply_ > _tokenIDPointer, "mint completed!");
        _;
    }
    ...
    constructor() ERC721("MSNFT-TCOL", "TMS") {}

    function mint() external onlyOwner {
        _mint_(msg.sender);
    }

    function batchMint(uint256 _batch) external onlyOwner {
        for (uint256 i = 0; i < _batch; ++i) _mint_(msg.sender);
    }

    function _mint_(address _minter) private isMintable {
