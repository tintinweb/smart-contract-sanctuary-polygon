...
import "../../interfaces/collections/ILoot8POAPTieredCollection.sol";
import "@openzeppelin/contracts/token/ERC721/extensions/ERC721Enumerable.sol";


contract Loot8TieredPOAPCollection is ILoot8POAPTieredCollection, ERC721Enumerable, Loot8POAPCollection {
    ...
    // Mapping Tier => Tier Start TokenID, Tier End TokenID in the Normal Token ID Range
    // For Tier 0 Start Token ID = 0 and End Token ID = 0 always
    // Start Token ID = 0 and End Token ID = 0 when range not set for a tier
    // Start Token ID > 0 and End Token ID > 0 and Start Token ID <  End Token ID for a limited tier range
    // Start Token ID > 0 and End Token ID = 0 for open ended
    mapping(uint256 => uint256[2]) public tierRange;
    ...
    function setTier( string memory _name, uint256 _start, uint256 _end) external {
        require(_msgSender() == governor || _msgSender() == owner(), "UNAUTHORIZED");
        require(_start > 0, "INVALID START TOKEN ID");
        require(_end >= _start || _end == 0, "END SHOULD BE 0 OR GREATER THAN OR EQUAL TO START");
        require(!_rangeClash(_start, _end), "RANGE CLASHING WITH ANOTHER TIER");

        tiers++;

        tierName[tiers] = _name;
        tierRange[tiers] = [_start, _end];

        emit SetTier(_name, tiers, _start, _end);
    }
    ...
    function _refreshPatronTierData(address _patron) internal {
        ...
        while(j < patronBalance) {
            ...
            for(uint256 i = 0; i <= tiers; i++) {
                if(
                    (tokenId >= tierRange[i][0] && tokenId <= tierRange[i][1]) ||
                    (tierRange[i][0] > 0 && tierRange[i][1] == 0 && tokenId >= tierRange[i][0])
                ) {
        ...
        for(uint256 k = 0; k < fromTier.length; k++) {
            if(!fromTier[k]) {
