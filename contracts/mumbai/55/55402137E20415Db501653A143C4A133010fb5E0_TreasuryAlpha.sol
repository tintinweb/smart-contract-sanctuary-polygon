...
contract TreasuryAlpha is AccessControlledAndUpgradeable {
  ...
  function convertSynthsToPaymentTokenNextPriceLong(
    uint32 marketIndex,
    uint224 pool,
    uint256 tokens_redeem
  ) external {
    ILongShort(longShort).redeemLong(marketIndex, pool, tokens_redeem);
  }

  function convertSynthsToPaymentTokenNextPriceShort(
    uint32 marketIndex,
    uint224 pool,
    uint256 tokens_redeem
  ) external {
    ILongShort(longShort).redeemShort(marketIndex, pool, tokens_redeem);
  }
}
