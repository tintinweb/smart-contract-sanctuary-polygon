...
contract ExtremeCardsGame is Ownable, ChainlinkVRF_V1_Polygon_Mumbai, ReentrancyGuard, ContextMixin {
    ...
    // Error 4: Already Settled

    // Error 5: Already Disbursed

    // Error 6: Already Refunded

    // Error 7: Already Reverted

    // Error 8: Low Win Count

    // Error 9: Low Count

    // Error 10: Low Total Bet Amount

    // Error 11: Low Total Win Amount

    // Error 12: Sender is Not Card Owner

    // Error 13: Card Already in Another Game

    // Error 14: Game is Already Filled

    // Error 15: Wrong Entry Fee

    // Error 16: Card Not in Game

    // Error 17: House Profit Sending Failed

    // Error 18: Card Not Found

    // Error 19: Try After [refundAfterBlock] Amount of Blocks

    // Error 20: Contract on Hold

    // Error 21: Game Type not Active

    // Error 22: Not Filled Yet!

    // Error 23: Wrong Exit Fee

    // Error 24: Already Settled

    // Error 25: Not Settled!

    // Error 26: Ownable: Not an Operator

    // Error 27: Ownable: Not Owner or Operator
    ...
    modifier onlyController() {
        ...
        require(isControllerAddress[_msgSender()] == true); // , "E26"); // Error 26: Ownable: Not an Operator
    ...
    modifier onlyOwnerOrController() {
        ...
        require(isControllerAddress[_msgSender()] == true || owner() == _msgSender()); // , "E27"); // Error 27: Ownable: Not Owner or Operator
    ...
    */
    ...
    function onlyValidGames(uint _gameId, bool notLocked) internal view {

        require(!isPaused); // , "E20"); // Error 20: Contract on Hold

        require(gameData[_gameId].gameStatus > 0); // , "E1"); // Error 1: Invalid ID

        // boolField -> 0: Filled, 1: Locked, 2: Settled, 3: Disbursed, 4: Refunded, 5: Reverted

        if(notLocked) {

            require(gameData[_gameId].boolField[1]); // , "E2"); // Error 2: Not Locked

        } else {

            require(!gameData[_gameId].boolField[1]); // , "E3"); // Error 3: Locked

        }



        require(!gameData[_gameId].boolField[2]); // , "E4"); // Error 4: Already Settled

        require(!gameData[_gameId].boolField[3]); // , "E5"); // Error 5: Already Disbursed

        require(!gameData[_gameId].boolField[4]); // , "E6"); // Error 6: Already Refunded

        require(!gameData[_gameId].boolField[5]); // , "E7"); // Error 7: Already Reverted
    ...
    */
    ...
    function createNewGame(uint32 _gameTypeId) external onlyController returns(uint) {
        ...
        require(gameType[_gameTypeId].isActive); // , "E21"); // Error 21: Game Type not Active
        ...
        gameData.push(
            ...
            GameData(
                ...
                new uint32[](gameType[_gameTypeId].tokenSlot),

                new uint32[](gameType[_gameTypeId].winnerCount)
    ...
    // Method to Join Game [ Public ]

    function joinGame(uint _gameId, uint32 _tokenId) external payable nonReentrant returns(GameData memory) {



        onlyValidGames(_gameId, false);

        // onlyQualifiedTokens(_gameId, _tokenId);



        GameType memory __gameType = gameType[gameData[_gameId].gameType];

        IExtremeCards.CommonTrait memory __commonTrait = gameCards.getCommonTrait(_tokenId);
        ...
        for(uint i = 0; i < 5; i++) {
            ...
            if(__gameType.minCounts[i] > 0){

                if(i == 2) {

                    require((__commonTrait.u32[1] + __commonTrait.u32[2]) >= __gameType.minCounts[2]); // , "E8"); // Error 8: Low Win Count

                } else {

                    require(__commonTrait.u32[i] >= __gameType.minCounts[i]); // , "E9"); // Error 9: Low Count
        ...
        if(__gameType.minAmounts[0] > 0) {

            require(__commonTrait.u128[0] >= __gameType.minAmounts[0]); // , "E10"); // Error 10: Low Total Bet Amount

        }

        if(__gameType.minAmounts[0] > 0) {

            require(__commonTrait.u128[1] >= __gameType.minAmounts[1]); // , "E11"); // Error 11: Low Total Win Amount

        }



        require(gameCards.ownerOf(_tokenId) == _msgSender()); // , "E12"); // Error 12: Sender is Not Card Owner

        require(gameCards.getTokenLock(_tokenId) == false); // , "E13"); // Error 13: Card Already in Another Game

        require(!gameData[_gameId].boolField[0]); // , "E14"); // Error 14: Game is Already Filled

        require(msg.value == gameType[gameData[_gameId].gameType].gameFees[0]); // , "E15"); // Error 15: Wrong Entry Fee
    ...
    // Request Randomness for Game

    function requestRandomnessForGame(uint _gameId) external nonReentrant onlyController returns(GameData memory) {

        onlyValidGames(_gameId, false);



        require(gameData[_gameId].boolField[0]); // , "E22"); // Error 22: Not Filled Yet!
    ...
    function exitGame(uint _gameId, uint32 _tokenId) external payable nonReentrant returns(GameData memory) {
        ...
        require(gameCards.getTokenLock(_tokenId) == true); // , "E16"); // Error 16: Card Not in Game



        uint128 exitFee = gameType[gameData[_gameId].gameType].gameFees[1];
    ...
    */
    ...
    function requestRefundOrRevert(uint _gameId) external nonReentrant {
     ...
     function getTokenList(uint256 _gameId) external view returns(uint32[] memory) {
     ...
     function getWinnerList(uint256 _gameId) external view returns(uint32[] memory) {
    ...
    */
    ...
    function settleGame(uint256 _gameId) external onlyController nonReentrant {
        ...
        require(gameData[_gameId].boolField[2]); // , "E25"); // Error 25: Not Settled!
    ...
    */
    ...
    function fulfillRandomness(bytes32 _requestId, uint256 _randomness) internal override {
        ...
        require(!gameData[_gameId].boolField[2]); // , "E24"); // Error 24: Already Settled
