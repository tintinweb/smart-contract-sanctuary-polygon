...
abstract contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    mapping(uint256 => address) internal _nfts; // mapping from id to owner address
    mapping(address => uint256) internal _balances; // mapping balance of minted token for owner address
    mapping(address => uint256[]) internal _owners;
    // mapping(uint256 => bool) private _mappingTokenExist;
    ...
    function _exists(uint256 tokenId) internal view virtual returns (bool) {
        return _nfts[tokenId] != address(0);
    ...
    function _mint(address to, uint256 tokenId) internal virtual {
        ...
        _balances[to]++;
        // _mappingTokenExist[tokenId] = true;
        _supply++;
    ...
    function _burn(uint256 tokenId) internal virtual {
        ...
        _nfts[tokenId] = address(0);
        _balances[owner]--;
        _removeTokenFromOwner(owner, tokenId);
        _removeTokenFromArray(tokenId);
        // _mappingTokenExist[tokenId] = false;
        _supply--;

        emit Transfer(owner, address(0), tokenId);
    }

    function _removeTokenFromArray(uint256 tokenId) internal virtual {
        bool found = false;
    ...
    function _removeTokenFromOwner(address owner, uint256 tokenId) internal virtual {
        bool found = false;
        for(uint i = 0; i < _owners[owner].length; i++) {
            if ( _owners[owner][i] == tokenId ) {
                found = true;
            }
            if ( found == true ) {
                if ( i == _owners[owner].length - 1) {
                    _owners[owner][i] = 0;
                } else {
                    _owners[owner][i] = _owners[owner][i + 1];
                }
            }
        }
        _owners[owner].pop();
...
abstract contract ERC721Enumerable is ERC721, IERC721Enumerable {
    ...
    function tokenByIndex(uint256 index) public view virtual override returns (uint256) {
        require(index < _tokenIds.length, "ERC721Enumerable: global index out of bounds");
    ...
    function tokenOfOwnerByIndex(address owner, uint256 index) public view virtual override returns (uint256 tokenId) {
        ...
        // uint count;
        // for(uint i; i < _owners[owner].length; i++){
        //     if(owner == _owners[owner][i]){
        //         if(count == index) return i;
        //         else count++;
        //     }
        // }
        return tokenId = _owners[owner][index];
    }
}
