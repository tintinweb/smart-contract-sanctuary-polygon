...
contract SoNFT is ERC721Burnable, Ownable, Pausable, AccessControl {
    using Strings for string;

    string public _contractUri = "https://d27b24gorg1v04.cloudfront.net/test/static/ff447b0cf9001bc21bafda23b3719ab9/contract.json";
    string public _baseUri = "https://d27b24gorg1v04.cloudfront.net/test/static/ff447b0cf9001bc21bafda23b3719ab9/";

    uint256 private totalCount;
    mapping(uint64 => mapping(uint256 => uint8)) private totalSupply;

    mapping(address => bool) private transferWhitelist;

    constructor() ERC721("SoNFT", "SN")  {
        setTransferWhitelist(address(0), true);
        setAdminRole(_msgSender());
        pause();
    }

    function mint(uint64 activityID, uint256 tokenId, address to) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), 'Unauthorized');
        require(to != address(0), 'Invalid address');
        require(activityID > 0 && tokenId > 0, 'activityID or tokenId cannot be 0');
        require(totalSupply[activityID][tokenId] == 0, 'Existing');
        _mint(to, tokenId);
        totalSupply[activityID][tokenId] == 1;
    ...
    function _beforeTokenTransfer(
        ...
        require(!paused() || transferWhitelist[from], 'NFT transfer while paused');
    }
