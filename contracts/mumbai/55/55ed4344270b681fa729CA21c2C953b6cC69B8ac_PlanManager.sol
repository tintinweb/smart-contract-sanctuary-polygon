...
contract PlanManager is IPlanManager, Initializable, OwnableUpgradeable {
    address public manager;

    mapping(uint256 => mapping (uint256 => Plan)) private merchantTokenIdToPlans;
    mapping(uint256 => uint256) private merchantTokenIdNextPlanIndex;
        ...
        returns (uint256 planIndex)
    {
        ...
        planIndex = merchantTokenIdNextPlanIndex[merchantTokenId];
        merchantTokenIdToPlans[merchantTokenId][planIndex] = plan;
        merchantTokenIdNextPlanIndex[merchantTokenId]++;
    }

    function disablePlan(uint256 merchant, uint256 planIndex)
        external
        override
        onlyManager
    {
        Plan storage plan = merchantTokenIdToPlans[merchant][planIndex];
        plan.enabled = false;
    }

    function getPlan(uint256 merchantTokenId, uint256 planIndex)
        external
        view
        override
        returns (Plan memory plan)
    {
        plan = merchantTokenIdToPlans[merchantTokenId][planIndex];
    }

    function getPlans(uint256 merchantTokenId)
        external
        view
        override
        returns (Plan[] memory plans)
    {
        
        for (uint256 i = 0; i < merchantTokenIdNextPlanIndex[merchantTokenId]; i++) {
            plans[i] = merchantTokenIdToPlans[merchantTokenId][i];
        }
    }
}