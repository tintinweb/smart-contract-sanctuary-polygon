...
contract LifeSpan is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, PausableUpgradeable, AccessControlUpgradeable, ERC721BurnableUpgradeable, UUPSUpgradeable {
    ...
    mapping(uint256 => string) private genderNames;
    ...
    function safeMint(address to, string memory name, uint256 gender, uint256 birthdayDate) public onlyRole(MINTER_ROLE) {
        require(gender == 0 || gender == 1, "ERC721: incorrect gender value");
    ...
    function baseSection(uint256 tokenId) internal view returns (bytes memory) {
        bytes memory base = abi.encodePacked(
            '{',
            '"description":"Plush ecosystem avatar",',
            '"external_url": "https://home.plush.family/token/', tokenId.toString(), '",',
            '"name": "', metaData[tokenId].name, '",',
            '"image":"ipfs://QmVVsfk8n8KdNeo5wAFCweDsoWMCLhqfYtbgdVXt8y2JhA",'
        );

        return base;
    }

    function attributesSection(uint256 tokenId) internal view returns (bytes memory) {
        bytes memory atr = abi.encodePacked(
            '"attributes":',
            '[{',
            '"display_type":"date","trait_type":"birthday","value":"', metaData[tokenId].birthdayDate.toString(), '"',
            '},{',
            '"trait_type":"Gender","value":"', genderNames[metaData[tokenId].gender], '"',
            '}]',
            '}'
        );
    ...
    function updateGender(uint256 tokenId, uint256 gender) public {
        require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");
        require(ownerOf(tokenId) == msg.sender, "ERC721: you are not the owner of the token");
        require(gender == 0 || gender == 1, "ERC721: incorrect gender value");

        metaData[tokenId].gender = gender;
    }
