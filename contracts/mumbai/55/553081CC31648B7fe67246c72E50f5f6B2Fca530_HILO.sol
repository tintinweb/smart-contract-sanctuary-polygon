...
contract HILO is ERC1155Supply, Ownable, Pausable, ReentrancyGuard {
    ...
    function buy(uint256 tokenId, uint256 amount) public whenNotPaused {
        ...
        if (forceUpdate) {
            // since we forced update, reset for next price
            buyForceCounts[tokenId] = 0;
            buyCounts[tokenId] = 0;
        } else {
            // no update forced, just increase
            increaseBuyCounts(tokenId);
        }

        // if we've hit force update, kick off any sales in the queue
        if (forceUpdate) sellFromQueue(tokenId);
    ...
    function _sell(
        ...
        // check the lock is open or we are convergent or from queue
        bool isUnlocked = buyCounts[tokenId] == buyRequiredCount || fromQueue;
        require(
