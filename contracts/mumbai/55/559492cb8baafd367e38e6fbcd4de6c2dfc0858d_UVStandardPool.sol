...
contract UVStandardPool is ERC20, ERC20Burnable, Ownable, AccessControl {
    ...
    uint256 public maxRenewed = 293786000000000000000000; // check
    uint64 public poolOpenTime = 1683280200; // check
    ...
    uint256 public currentSizePool = 235028519160000000000000; // check
    uint256 public maxSizePool = 285000000000000000000000; // check
    uint8 public orderNumber = 2; // check
    uint256 public minimumDeposit = 500 ether; //check
    bool public isClose = true; // check
    ...
    // Set swap router
    function setSwapRouter(address _swapRouterAddress) public onlyRole(MANAGER_ROLE) {
        SWAP_ROUTER_ADDRESS = _swapRouterAddress;
    ...
    // update renewed list
    function updateRenewedList(
        address[] memory _user,
        uint256[] memory _amount
    ) public onlyOwner {
        require(hasRole(MANAGER_ROLE, msg.sender), "You are not manager");
        require(_user.length == _amount.length, "Length is not equal");
        for (uint256 i = 0; i < _user.length; i++) {
            renewedList[_user[i]] += _amount[i];
        }
    }

    // renewed token
    function renewedToken(address[] memory _listAddress) public onlyOwner {
        require(maxRenewed > 0, "Max renewed is 0");
        for (uint256 i = 0; i < _listAddress.length; i++) {
            uint256 amount = renewedList[_listAddress[i]];
            _mint(_listAddress[i], amount);
            renewedList[_listAddress[i]] = 0;
            maxRenewed -= amount;
            _balances[_listAddress[i]] = _balances[_listAddress[i]].add(amount);
        }
    }
}