...
contract LostWorldValidator is CustomERC2771ContextUpgradeable, UUPSUpgradeable {
    ...
    function validate(
        ...
        for (uint256 i = 0; i < rewardAmount.length; i++) {
            require(rewardAmount[i] == 1, 'LWV: Reward amount must be 1');
            require(maxRewardAmount[i] > 0, 'LWV: Max reward amount must be minimum of 1');
            require(rewardAmount[i] <= maxRewardAmount[i], 'LWV: Min reward cannot be over the max reward');
        }
    ...
    function validateRarity(
        ...
        uint8 highestEnemyIndex = 0;
        string memory highestRarity;
        for (uint256 i = 0; i < enemyList.length; i++) {
            (uint8 rarityIndex, string memory rarityString) = LWD.getEnemyRarity(enemyList[0]);
            if (rarityIndex >= highestEnemyIndex) {
                highestEnemyIndex = rarityIndex;
        ...
        require(highestEnemyIndex > 0, 'LWV: Enemy rarity is not valid');
        ...
        if (highestEnemyIndex >= highestRewardRarity) rewardRarityValidated = true;
