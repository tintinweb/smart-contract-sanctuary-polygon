...
library VaultLifecycle {
        ...
        returns (uint256)
    {
        ...
        if (currentOption == address(0)) {
            return getNextAlternateDay(block.timestamp);
        }
        ...
        return getNextAlternateDay(currentExpiry);
    }
     ...
     */
    function getNextAlternateDay(uint256 timestamp) internal pure returns (uint256) {
        // dayOfWeek = 0 (sunday) - 6 (saturday)
        uint256 dayOfWeek = ((timestamp / 1 days) + 4) % 2;
        uint256 nextAlternateDay = timestamp + ((2 + 1 - dayOfWeek) % 2) * 1 days;
        uint256 alternateDay8am = nextAlternateDay - (nextAlternateDay % (24 hours)) + (8 hours);

        // If the passed timestamp is day=AlternateDay hour>8am, we simply increment it by a week to next AlternateDay
        if (timestamp >= alternateDay8am) {
            alternateDay8am += 2 days;
        }
        return alternateDay8am;
    }
}
    ...
    ERC20Upgradeable
{
    ...
    /// @notice 2 day period between each options sale.
    uint256 public constant PERIOD = 2 days;
