...
contract ConsentRegistry is IConsentRegistry, Ownable {
    ...
    function addConsent(string memory name, string memory label, bytes32 consentHash) external onlyOwner override {
        if (consentHash == 0) {
            ...
            require(hashHistory[consentHash] != true);
            _consentsCount++;
    ...
    /**
     * @dev deactivates consent
     * @param id id of consentt
     */
...
contract Consents is IConsents, ConsentRegistry, Managers {
    ...
    function updatePersonConsents(bytes32 id, uint32[] memory agreedConsents) external override {
        ...
        if (newConsents.length > 0) {
            if(!_isOwner() || !_isManagerAllowedToChangeConsents(newConsents)){
    ...
    /**
     * @dev Retrieves all patients that given have given their agreement to a particular consent
...
contract Managers is IManagers, Ownable {
    ...
    function _isManagerAllowedToChangeConsents(uint32[] memory consentsIds) internal view returns (bool) {
        if (!isManager(_msgSender())) {
            revert Managers_Error("Managers: Caller is not manager of any Consent");
        } else {
