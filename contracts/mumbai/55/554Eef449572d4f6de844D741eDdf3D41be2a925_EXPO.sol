...
contract EXPO is
    ...
    OwnableUpgradeable
{
    struct LockingDetail {
        uint256 amount;
        uint256 deadline;
    }
    uint256 public maxSupply;
    mapping(address => mapping(uint256 => LockingDetail)) public lockedTokens;
    ...
    function initialize(uint256 initialSupply) public initializer {
        ...
        maxSupply = 2700000000000000000000000000000;
        if(initialSupply > 0){
            mint(msg.sender, initialSupply);
        }
        __ERC777_init("EXPO", "EXPO", defaultOperators);
    }
    ...
    function transfer(address to, uint256 amount)
        ...
        returns (bool)
    {
        if (lockCount[msg.sender] > 0) {
            uint256 notAllowedTransferAmount = 0;
            for (uint256 i = 1; i <= lockCount[msg.sender]; i++) {
                if (lockedTokens[msg.sender][i].deadline >= block.timestamp) {
                    notAllowedTransferAmount += lockedTokens[msg.sender][i]
                        .amount;
                }
            }
            uint256 allowedTransferAmount = balanceOf(msg.sender) -
                notAllowedTransferAmount;
            require(allowedTransferAmount >= amount, "Tokens Locked");
    ...
    function _transferWithLock(
        ...
        ++lockCount[to];
        lockedTokens[to][lockCount[to]] = LockingDetail(amount, deadline);
    ...
    function transferWithLock(
        ...
        if (lockCount[msg.sender] > 0) {
            ...
            uint256 allowedTransferAmount = balanceOf(msg.sender) -
                notAllowedTransferAmount;
            require(allowedTransferAmount > amount, "Tokens Locked");
            _transferWithLock(to, amount, deadline);
        } else {
            _transferWithLock(to, amount, deadline);
        }
    }
