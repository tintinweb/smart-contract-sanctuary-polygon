...
contract DispatchStore is ERC721, IERC4906, Ownable, Pausable, ReentrancyGuard {
  ...
  function setTreasuryAddresses(
    address payable _newDispatchAddress,
    address payable _newMerchantAddress
  ) public onlyOwner {
  ...
  /**
      @notice Purchase a product with the L1 token in bulk - callable only by whitelisted address, no msg.value passed
      @param _amount product amount to purchase
      @param _productId product numerical ID
      @param _data additional data passed to event for logging purposes
      @param _receiver address to receiveNFT
   */
  function freeBulkMintTo(
    uint256 _amount,
    uint256 _productId,
    string memory _data,
    address[] memory _receiver
  ) external _isValidAmount(_amount, _productId) whenNotPaused _isWhitelistedAddress {
    for (uint256 i = 0; i < _receiver.length; i++) {
      _handleMint(_amount, _productId, _receiver[i], _data);
    }
  }
  ...
  /**
      ...
      @dev private function
   */
  function _handleMint(uint256 _amount, uint256 _productId, address _receiver, string memory _data) private {
    uint256 l1Price = getL1PriceForProduct(_amount, _productId);
    // check that the user sent enough ETH - note that whitelisted addresses can purchase for free
    require(msg.value >= l1Price || whiteListedAddress[msg.sender] == true, "Invalid amount paid.");
