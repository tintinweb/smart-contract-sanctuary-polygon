...
/**
 * Simple implementation for quick testing of KYC
 * verification of Token & Account.  This is not the production
 * version and shouldn't be called directly by any application
 * building against the test SDK.
 * This smart contract is proprietary and not to be copied or
 * reproduced without the express permission of the Amberfi
 * Platform.
 */
contract KYCVerify is
    Initializable,
    ContextUpgradeable,
    AccessControlUpgradeable
{
    string private _kycProvider;

    mapping(address => bool) public allowedAccounts;
    mapping(address => mapping(uint256 => bool)) public revokedTokens;
    ...
    event AccountVerifyRequested(address indexed account);

    function initialize(string calldata kycProvider_) public initializer {
        _kycProvider = kycProvider_;
    }

    function requestAccountVerify() external {
        emit AccountVerifyRequested(_msgSender());
    }
    ...
    function setTokenStatus(
        ...
        uint256 tokenId_,
        bool status_
    ) external onlyRole(AmberfiLib.KYC_VERIFIER) {
        revokedTokens[account_][tokenId_] = status_;

        emit TokenStatusChanged(_msgSender(), account_, tokenId_, status_);
    }

    function setAccountStatus(address account_, bool status_)
        external
        onlyRole(AmberfiLib.KYC_VERIFIER)
    {
        allowedAccounts[account_] = status_;

        emit AccountStatusChanged(_msgSender(), account_, status_);
    }

    function isTokenVerified(address account_, uint256 tokenId_)
        public
        view
        returns (bool)
    {
        return
            allowedAccounts[account_]
                ? !revokedTokens[account_][tokenId_]
                : false;
    }

    function isAccountVerified(address account_) public view returns (bool) {
        return allowedAccounts[account_];
    }
}
...
library AmberfiLib {
    ...
    struct Auction {
        ...
        uint256 minIncrementAmount; // The minimum increase for the bid
        address creator; // Auction creator
        address nftContract; // The address of the NFT contract
        uint256 tokenId; // Token ID
        uint256 highestBidAmount; // Highest bid amount in PaymentToken
        address highestBidder; // Bidder address with highest amount
        uint256 paymentTokenIndex; // PaymentToken index to place bid at this auction
    ...
    event BoughtNFT(address indexed buyer, uint256 amount);// Event emited when an instant sale auction item sold
    event BidPlaced(address indexed bidder, uint256 bid); // Event emited when a bid placed
