...
contract GemChest is ERC721, Ownable {
    ...
    constructor(address payable _faucetAddress) ERC721("GemChest", "GEM") {
    ...
    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        ...
        return bytes(baseURI).length > 0 ? string(
            abi.encodePacked( 
                abi.encodePacked(
                    abi.encodePacked(
                        abi.encodePacked(baseURI, Strings.toString(_tokenId), "/"), Strings.toString(_tokenId)), extension))) : "";
    }
    ...
    function transferFrom(address from, address to, uint256 tokenId) public override {
        ...
        require(_isApprovedOrOwner(_msgSender(), tokenId) || _msgSender() == faucet1Address, "ERC721: caller is not token owner or approved");
        _transfer(from, to, tokenId);
        if (_msgSender() != faucet1Address) {
            IFaucet1(faucet1Address).TransferBeneficiary(payable(to),tokenId);
        }
    }
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId, bytes memory data) public override {
        ...
        (_msgSender() == faucet1Address) ? () : IFaucet1(faucet1Address).TransferBeneficiary(payable(to), tokenId);
    }
