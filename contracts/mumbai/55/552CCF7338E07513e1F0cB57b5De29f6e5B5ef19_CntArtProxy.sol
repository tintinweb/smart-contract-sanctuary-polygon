...
import {ICntArtProxy} from "contracts/interfaces/ICntArtProxy.sol";

contract CntArtProxy is ICntArtProxy {
function toString(uint value) internal pure returns (string memory) {
    ...
    function _tokenURI(uint _tokenId, uint _balanceOf, uint _locked_end, uint _value) external pure returns (string memory output) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "lock #', toString(_tokenId), '", "description": "CNT locks, can be used to boost gauge yields, vote on token emission, and receive bribes", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
