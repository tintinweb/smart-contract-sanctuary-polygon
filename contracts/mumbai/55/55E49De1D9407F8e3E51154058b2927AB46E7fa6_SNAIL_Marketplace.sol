...
import "./snail-nft.sol";

contract SNAIL_Marketplace is ReentrancyGuard {
    ...
    event CancelSell(uint256 indexed itemId, uint256 tokenId);
     ...
     * @dev Set NFT Factory
     */
    function updateFactory(address _nftFactory) public {
    ...
    function createSale(
        ...
        SNAIL_NFT(nftFactory).transferFrom(msg.sender, address(this), tokenId);
    ...
    function buyNFT(uint256 tokenId) public nonReentrant {
        ...
        require(
            SNT.balanceOf(msg.sender) >= price,
        ...
        SNAIL_NFT(nftFactory).transferFrom(address(this), msg.sender, tokenId);
    ...
    function updatePrice(
        ...
        require(_newPrice > 0, "Price must be at least 0");
        address seller = idToMarketItem[tokenId].seller;
        bool is_sold = idToMarketItem[tokenId].sold;
        uint256 itemId = idToMarketItem[tokenId].itemId;
        require(msg.sender == seller, "updatePrice : Is not Seller");
        require(is_sold == false, "updatePrice : Unavailable");
        idToMarketItem[tokenId].price = _newPrice;
        emit ItemUpdated(
            itemId,
            tokenId,
            _newPrice
    ...
    function cancelSell(uint256 tokenId) public nonReentrant {
        ...
        SNAIL_NFT(nftFactory).transferFrom(address(this), msg.sender, tokenId);
        delete idToMarketItem[tokenId];
        emit CancelSell(itemId, tokenId);
    }
    ...
    {
        SNT.transferFrom(address(this), to, amount);
    }
