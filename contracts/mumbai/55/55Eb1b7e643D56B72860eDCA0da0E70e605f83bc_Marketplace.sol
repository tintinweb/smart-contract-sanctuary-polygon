...
contract Marketplace is Ownable, ReentrancyGuard {
    ...
    function purchaseToken(
        ...
        require(
            msg.sender != idToListing[listingId].seller,
            "Can't buy your onw tokens!"
        );
        require(
            msg.value >= idToListing[listingId].price * amount,
            "Insufficient funds!"
        );
        require(
            token.balanceOf(
        ...
        payable(idToListing[listingId].seller).transfer(
            ((idToListing[listingId].price * amount) / 50) * 49
        ); //Transfering 98% to seller, fee 2%  ((msg.value/50)*49)
    }

    function deleteListing(uint _listingId) public {
        require(
            msg.sender == idToListing[_listingId].seller,
            "Not caller's listing!"
        );
        require(
            idToListing[_listingId].completed == false,
