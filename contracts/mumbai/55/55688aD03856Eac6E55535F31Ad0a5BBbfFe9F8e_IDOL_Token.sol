...
contract IDOL_Token {
    ...
    function firstroundTransfer(address _to, uint256 _numToken)
        ...
        onlyOwner
    {
        require(block.timestamp >= First_Round_Start_Date, "Round Start Soon");
        require(block.timestamp <= First_Round_End_Date, "Period is exceed");
        require(
            _numToken <= FirstRoundnumToken,
            "Number should be less than allocation "
        );
        require(
            FirstRoundnumToken_initialToken + _numToken <= FirstRoundnumToken,
