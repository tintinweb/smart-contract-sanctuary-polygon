...
contract OmnichainRouter is IOmnichainRouter, NonblockingLzApp, IAxelarExecutable {
    ...
    event MsgReceived(string srcChain);
    event Redirect(string originChain, string fromChain, string toChain);

    struct Route {
        string provider; // 'lz': LayerZero |
        uint16 chainId; // LayerZero .send() param
        string dstChain; // Axelar gateway.callContract() param
    }
    ...
    constructor(address _lzEndpoint, address _axGateway, address _axGasReceiver) NonblockingLzApp(_lzEndpoint) IAxelarExecutable(address(0))  {
        if (address(gateway) != address(0) || address(axGasReceiver) != address(0)) revert AlreadyInitialized();
        _owner = msg.sender;
        chainName = "Polygon"; // https://docs.axelar.dev/dev/chain-names
    ...
    function addRoute(string memory provider, uint16 chainId, string memory dstChain) external onlyOwner {
        ...
        chainNameToRoute[dstChain].push(route);
    }
    ...
    function isAxRemote(string memory _chainName, address _address) public view returns (bool) {
        return axRemotes[_chainName] == _address;
    }

    function testInfo(string memory dstChainName, uint gas, address origin) external payable {
        bytes memory fnData = abi.encode(dstChainName, gas, msg.sender, origin);
        bytes memory _payload = abi.encode("Info", fnData);
        call(dstChainName, _payload, gas, origin);
    }

    function send(string memory dstChainName, address dstUA, bytes memory _fnData, uint gas, address origin) external override payable {
        bytes memory _payload = abi.encode(dstChainName, dstUA, _fnData, gas, msg.sender, chainName, origin);
        call(dstChainName, _payload, gas, origin);
    }

    function call(string memory dstChainName, bytes memory _payload, uint gas, address origin) internal {
        Route[] storage routes = chainNameToRoute[dstChainName];
        Route storage route = routes[0];
        uint16 chId = route.chainId;

        if (keccak256(bytes(route.provider)) == keccak256(bytes('lz'))) {
            lzEndpoint.send{value : msg.value}(chId, this.getTrustedRemote(chId), _payload, payable(origin), address(0x0), _getAdapter(gas));

            return;
        }

        if (keccak256(bytes(route.provider)) == keccak256(bytes('ax'))) {
            string memory dstStringAddress = axRemotes[dstChainName].toString();
            axGasReceiver.payNativeGasForContractCall{value: msg.value}(address(this), route.dstChain, dstStringAddress, _payload, origin);
            gateway.callContract(route.dstChain, dstStringAddress, _payload);
        }
    }
    ...
    function _nonblockingLzReceive(uint16 _srcChainId, bytes memory _srcAddress, uint64, bytes memory _payload) internal override {
        emit MsgReceived("Avalanche");
        require(this.isTrustedRemote(_srcChainId, _srcAddress), "NOT_LZ_REMOTE");
//        handleCall(_payload);
    }
    ...
    function _execute(
        ...
        emit MsgReceived(srcChain);
        require(isAxRemote(srcChain, srcAddressString.toAddress()), 'NOT_AX_REMOTE');
        handleCall(_payload);
    }

    function handleCall(bytes memory _payload) internal {
        (string memory dstChainName, address dstUA, bytes memory fnData, uint gas, address srcUA, string memory srcChain, address origin) = abi.decode(_payload, (string, address, bytes, uint, address, string, address));

        if (keccak256(bytes(dstChainName)) != keccak256(bytes(chainName))) {
            emit Redirect(srcChain, chainName, dstChainName);
            call(dstChainName, _payload, gas, origin);
