...
contract DafSales is Ownable {
    ...
    */
    ...
    function checkSignature(uint256 nonce, uint256 maxAmount, bytes memory signature) internal {
        require(!usedNonces[nonce], "Daf: nonce already used");
        bytes32 message = keccak256(abi.encodePacked(msg.sender, maxAmount, nonce));
        bytes32 hash = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", message));
        address signatureAddress = hash.recover(signature);
        require(signatureAddress == messageSigner, "Daf: invalid signature");
     ...
     * @param _maxAmount - max coin amount to be purchased
     * @param _maxPayTokenAmount - max tokens which should be used
     * @param coinType - payed token type
     * @param nonce - nonce for signature
     * @param signature - allowed signature passed by backend
    */

    function buy(
        uint256[] memory _batcheIds,
        uint256 _amount,
        uint256 _maxAmount,
    ...
    payable {
        if (KYCEnabled)
            checkSignature(nonce, _maxAmount,signature);
        ...
        require(amount <= _maxAmount, "Daf: max purchase amount exceeds");
    ...
    */
    ...
    external payable {
        ...
        if (coinType == HedgeCoin.WBTC) {
            ...
            ERC20(WBTCAddress).transferFrom(msg.sender, dafWalletAddress, _maxPayTokenAmount - payedAmount);
            ...
            require(_maxPayTokenAmount <= msg.value, "Daf: insufficient eth balance");
            // fee should be transferred into Daf contract wallet

            payable(dafWalletAddress).transfer(_maxPayTokenAmount - payedAmount);
            value = payedAmount;
        }
    ...
    /**
     * @dev spending whole usd amount and getting the purchased daf coin count
     * @param _batcheIds - target sale batches
     * @param _payUSDAmount - amount of USD to spend
    */
