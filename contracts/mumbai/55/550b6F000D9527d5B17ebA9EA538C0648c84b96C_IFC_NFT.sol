...
contract IFC_NFT is ERC721A, Ownable, ReentrancyGuard {
    ...
    /*
     * Function to set max min per wallet by the owner
     */
    ...
    /*
     * Function to get max min per wallet by the owner
     */
    function getmaxMintAmountPerTx() public view returns (uint256) {
        return maxMintAmountPerTx;
    }

    function setRevealed(uint256 _tokenId, bool _state) public onlyOwner {
        revealed[_tokenId] = _state;
    }
    ...
    function setCurrentTokenURI(string memory _currentTokenURI) public onlyOwner {
        currentTokenURI = _currentTokenURI;
    }
        ...
        isOperator
    {
        ...
        // keep track of minted token id
        for (uint256 i = _next_token_id; i < _next_token_id + _quantity; i++) {
            revealed[i] = false;
        }
        _next_token_id = _next_token_id + _quantity;
    }
