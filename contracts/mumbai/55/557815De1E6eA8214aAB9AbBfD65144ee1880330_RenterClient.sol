...
interface IRentingContract {
    ...
    event RentStart(uint256 indexed landId, uint256 collectionId, address renter, address owner);
    event RentEnd(uint256 indexed landId, uint256 collectionId, address renter, address owner);
    event RentRenewed(uint256 indexed landId, uint256 collectionId);
    ...
    event BattleSetDisband(uint256 landId);
...
interface IRentingContractStorage is IRentingContract {
    ...
    function createRenting(BattleSet memory bs, RentingType rt, Coin coin, uint256 price, address owner, address renter,
        uint256 rentingEnd, uint256 collectionId, bool perpetual) external;
...
contract RenterClient is Context, IRentingContract, TokensManager {
    ...
    uint private constant DEFAULT_RENTING_DURATION = 10 minutes;

    uint private constant RENTING_RENEWAL_PERIOD_GAP = 2 minutes;
    ...
    function rentFromCollection(uint256 id, uint256 _landId, uint256[] memory botsIds) external returns (bool) {
        ...
        super.safeTransferFromSetForRent(collection.owner, _msgSender(), _landId, botsIds);

        storageContract.updateCollectionRentedAssets(id, Array256Lib.remove(collection.landIds, _landId),
            Array256Lib.removeAll(collection.botsIds, botsIds), Array256Lib.add(collection.rentedLandIds, _landId),
            Array256Lib.addAll(collection.rentedBotsIds, botsIds));


        BattleSet memory bs = BattleSet({landId : _landId, botsIds : botsIds});
        storageContract.createRenting(bs, collection.rentingType, collection.chargeCoin, collection.price, collection.owner, _msgSender(),
            block.timestamp + DEFAULT_RENTING_DURATION, collection.id, collection.perpetual);
