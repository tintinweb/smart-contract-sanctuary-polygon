...
import "./lib/FxERC20Child.sol";

contract MiceTokenChild is FxERC20Child, AccessControl {
    bytes32 public constant AUTHORITY = keccak256("AUTHORITY");

    constructor(address _fxChild)
        ERC20("MICE", "MICE", 18)
        FxBaseChildTunnel(_fxChild) // @note make immutable
    {
    ...
    function withdrawFrom(
        ...
        address to,
        uint256 amount
    ) external onlyRole(AUTHORITY) {
        _withdraw(from, to, amount);
    }

    function mint(address user, uint256 amount) external payable onlyRole(AUTHORITY) {
        _mint(user, amount);
    }
    ...
    function burnFrom(address user, uint256 amount) external payable {
        if (msg.sender != user || !hasRole(AUTHORITY, msg.sender)) {
...
abstract contract FxERC20Child is FxBaseChildTunnel, ERC20 {
