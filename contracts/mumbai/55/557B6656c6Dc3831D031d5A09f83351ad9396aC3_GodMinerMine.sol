...
interface IGodMiner is IERC721 {
    ...
    function minerPS(uint256 tokenId) external view returns (uint256);
    function tokenOfOwnerByIndex(address owner, uint256 index) external view returns (uint256);
    function physicalStrengthOfOwner(address owner) external view returns (uint256);
}
...
contract GodMinerMine is Ownable {
    ...
    uint256 public outputPerPS = 10**18;
    ...
    function setOutputPerPS(uint256 _outputPerPS) public onlyOwner() {
        outputPerPS = _outputPerPS;
    }
    ...
    function mineByTokenID(uint256 tokenId) public {
        ...
        uint256 clacPS = minerContract.minerPS(tokenId);

        uint256 mineCalcAmount = clacPS.mul(outputPerPS).mul(workTime).div(period);
