...
contract DesignatedShareNFT is ERC721 {
    ...
    function transferShare(address _from, uint256 _tokenId, address _to, uint256 _amount) external {
        ...
        if (getShareExit(_tokenId, _to)) {
            ...
            uint256 shareIndex = getShareArrayIndex(_to);
        ...
        uint256 shareOneIndex = getShareArrayIndex(_from);
        shares[shareOneIndex] = share;
    ...
    function getShareArrayIndex(address owner) internal view returns (uint256){
        uint256 index;
        for (uint i = 0; i < shares.length; i++) {
            if (shares[i].owner == owner) {
                index = i;
