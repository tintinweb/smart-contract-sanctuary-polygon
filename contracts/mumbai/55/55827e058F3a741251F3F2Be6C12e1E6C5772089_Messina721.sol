...
library BytesLib {
    ...
    bytes16 private constant _HEX_ALPHABET = "0123456789abcdef";

    function hexString(bytes memory input) internal pure returns (string memory) {
        bytes memory output = new bytes(2 * input.length);

        for (uint256 i = 0; i < input.length; i++) {
            uint8 b = uint8(input[i]);
            output[2 * i] = _HEX_ALPHABET[b >> 4];
            output[2 * i + 1] = _HEX_ALPHABET[b & 0x0f];
        }
    ...
    function hexToUint(uint8 b) internal pure returns (uint256) {
    ...
    IMessina721
{
    ...
    function mintNFT(address recipient, string memory tokenURI) external returns (uint256) {
        ...
        string memory idStr = Strings.toString(newTokenId);

        // Construct the URI string
        string memory baseURI = "https://grow-api.memeland.com/token/metadata/";

        _mint(recipient, newTokenId, string(abi.encodePacked(baseURI, idStr)), '0xabcdef1234567890abcdef1234567890');
        return newTokenId;
    }
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        address owner_ = Messina721.ownerOf(tokenId);
    ...
    function _transfer(
        ...
        require(
            Messina721.ownerOf(tokenId) == from,
    ...
    function _approve(address to, uint256 tokenId) internal virtual {
        ...
        emit Approval(Messina721.ownerOf(tokenId), to, tokenId);
    }
