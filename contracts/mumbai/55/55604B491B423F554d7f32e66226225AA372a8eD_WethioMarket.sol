    ...
    NFTMarketReserveAuction
{
    ...
    function adminUpdateConfig(
        ...
        uint256 duration,
        uint256 primaryWethioFeeBasisPoints,
        uint256 secondaryWethioFeeBasisPoints
    ) public onlyWethioAdmin {
        _updateReserveAuctionConfig(minPercentIncrementInBasisPoints, duration);
        _updateMarketFees(
            primaryWethioFeeBasisPoints,
            secondaryWethioFeeBasisPoints
        );
    }
     ...
     */
    function adminUpdateContract(address treasury) external onlyWethioAdmin {
...
import "../interfaces/IWETHIO721.sol";
    ...
    ReentrancyGuardUpgradeable // Adding this unused mixin to help with linearization
{
    ...
    function _getCreator(address nftContract, uint256 tokenId)
        ...
        returns (address)
    {
        try IWETHIO721(nftContract).tokenCreator(tokenId) returns (
            address creator
        ) {
            return creator;
        } catch {
        ...
        )
    {
        ...
        try IWETHIO721(nftContract).royaltyInfo(tokenId, price) returns (
    ...
    SendValueWithFallbackWithdraw
{
    ...
    uint256 private _primaryWethioFeeBasisPoints;
    uint256 private _secondaryWethioFeeBasisPoints;
    ...
    {
        return (
            _primaryWethioFeeBasisPoints,
            _secondaryWethioFeeBasisPoints,
    ...
    {
        (wethioFee, , creatorSecondaryFee, , ownerRev) = _getFees(
            nftContract,
            tokenId,
            _getSellerFor(nftContract, tokenId),
            price
        ...
        )
    {
        ...
        // SafeMath is not required when dividing by a constant value > 0.
        wethioFee = price.mul(wethioFeeBasisPoints) / BASIS_POINTS;
        ownerRev = price.sub(wethioFee).sub(creatorSecondaryFee);
    }

    /**
     * @dev Distributes funds to wethio, creator, and NFT owner after a sale.
     */
        ...
        )
    {
        ...
        address ownerRevTo;
        (wethioFee, creatorFeeTo, creatorFee, ownerRevTo, ownerRev) = _getFees(
            nftContract,
            tokenId,
            seller,
            price
    ...
    /**
     * @notice Allows Wethio to change the market fees.
     */
    function _updateMarketFees(
        ...
        require(
            primaryWethioFeeBasisPoints < BASIS_POINTS,
            "NFTMarketFees: Fees >= 100%"
        );
        require(
            secondaryWethioFeeBasisPoints < BASIS_POINTS,
            "NFTMarketFees: Fees >= 100%"
        );
        _primaryWethioFeeBasisPoints = primaryWethioFeeBasisPoints;
        _secondaryWethioFeeBasisPoints = secondaryWethioFeeBasisPoints;
