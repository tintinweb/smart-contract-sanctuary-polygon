...
contract ETHCleanMixer is CleanMixer {
    ...
    function _payReferral(
        ...
        uint8[3] memory rootOwnerPercentage = [
            twoLevelReferral.getRootOwnerPercentage(0),
            twoLevelReferral.getRootOwnerPercentage(1),
            twoLevelReferral.getRootOwnerPercentage(2)
        ];

        if (_referrerAddress != address(0)) {
            // check first level
            address firstLevelRef = twoLevelReferral.checkExistingFirstLevel(_depositor, _referrerAddress);
            uint256 firstLevelPayAmount = twoLevelReferral.calculateFirstLevelPay(_denomination);

            // check second level
            address secondLevelRef = twoLevelReferral.checkExistingSecondLevel(_depositor, _referrerAddress);
            ...
            if (secondLevelRef != address(0)) {
                ...
                (success, ) = firstLevelRef.call{value: firstLevelPayAmount}("");
                require(success, "Transfer Failed");
...
interface ITwoLevelReferral {
    ...
    function checkExistingFirstLevel(address _depositor, address _referrerAddress)
        external
        returns (address firstLevelReferral);

    function checkExistingSecondLevel(address _depositor, address _referrerAddress)
        external
        returns (address twoLevelReferral);
