...
contract GNSPairInfosV6_1 {
    ...
    mapping(address => mapping(uint256 => mapping(uint256 => TradeInitialAccFees))) public tradeInitialAccFees;
    ...
    function getPendingAccRolloverFees(uint256 pairIndex) public view returns (uint256) {
        ...
        return r.accPerCollateral
            + (block.number - r.lastUpdateBlock) * pairParams[pairIndex].rolloverFeePerBlockP * 1e18 / PRECISION / 100;
    }
    ...
    function getPendingAccFundingFees(uint256 pairIndex) public view returns (int256 valueLong, int256 valueShort) {
        ...
        int256 openInterestDaiLong = int256(storageT.openInterestDai(pairIndex, 0));
        int256 openInterestDaiShort = int256(storageT.openInterestDai(pairIndex, 1));

        int256 fundingFeesPaidByLongs = (openInterestDaiLong - openInterestDaiShort)
            * int256(block.number - f.lastUpdateBlock) * int256(pairParams[pairIndex].fundingFeePerBlockP)
        ...
        if (openInterestDaiShort > 0) {
            valueShort += fundingFeesPaidByLongs * 1e18 * (-1) / openInterestDaiShort;
        }
    }
        ...
        )
    {
        ...
        priceImpactP = (startOpenInterest + tradeOpenInterest / 2) * PRECISION / 1e18 / onePercentDepth;
    ...
    function getTradeLiquidationPricePure(
        ...
        int256 liqPriceDistance = int256(openPrice)
            * (int256(collateral * LIQ_THRESHOLD_P / 100) - int256(rolloverFee) - fundingFee) / int256(collateral)
            / int256(leverage);

        int256 liqPrice = long ? int256(openPrice) - liqPriceDistance : int256(openPrice) + liqPriceDistance;
    ...
    function getTradeValuePure(
        ...
        // 1e18 (DAI)
        int256 value = int256(collateral) + int256(collateral) * percentProfit / int256(PRECISION) / 100
