...
library LotteryLibrary {
    struct SweepstakeTicketInfo {
        uint256 number;
        uint256 ticketsAmount;
        uint256 ticketsValue;
        uint dateRaffle;
        uint dateStart;
        uint dateEnd;
        address addrWinner;
...
| |                 | |           |  \/  |(_)           | |               |  ___|       | |                /  | 
| |     _   _   ___ | | __ _   _  | .  . | _  _ __    __| | ___   ______  | |__   _ __  | |_  _ __  _   _  `| | 
| |    | | | | / __|| |/ /| | | | | |\/| || || '_ \  / _` |/ __| |______| |  __| | '_ \ | __|| '__|| | | |  | | 
| |____| |_| || (__ |   < | |_| | | |  | || || | | || (_| |\__ \          | |___ | | | || |_ | |   | |_| | _| |_
\_____/ \__,_| \___||_|\_\ \__, | \_|  |_/|_||_| |_| \__,_||___/          \____/ |_| |_| \__||_|    \__, | \___/
                            __/ |                                                                    __/ |      
                           |___/                                                                    |___/       
*/
...
import "./libraries/LotteryLibrary.sol";

contract TicketNFT is ERC721, ERC721Burnable, Ownable, ReentrancyGuard {
    ...
    LotteryLibrary.SweepstakeTicketInfo public sweepstakesTicket;
    ...
    uint256 public constant MAX_PUBLIC_SALE_MINTS = 70;
    ...
    // sweepstakesTicket.ticketsAmount
    ...
    function tokenURI(
        ...
        return sweepstakesTicket.urlItem;
    }
    ...
    function withdrawToken(
        ...
        SafeERC20.safeTransfer(IERC20(tokenAddress), dest, balance);

        emit WithdrawToken(tokenAddress, dest);
    }
    ...
    //TicketNFT
    function setSweepstakeTicketInfo(
        uint256 number,
        uint256 ticketsAmount,
        uint256 ticketsValue,
        uint dateRaffle,
        ...
        address addrWinner,
        uint256 ticketWinner,
        string memory urlItem
    ) public onlyOwner {
        //publicSaleActive

        sweepstakesTicket = LotteryLibrary.SweepstakeTicketInfo(
            number,
            ticketsAmount,
            ticketsValue,
            dateRaffle,
        ...
        PUBLIC_SALE_PRICE = ticketsValue;
        MAX_TOTAL_SUPPLY = ticketsAmount;
    }

    function getSweepstakeTicketInfo()
        public
        view
        returns (LotteryLibrary.SweepstakeTicketInfo memory)
    {
        return sweepstakesTicket;
    }

    function setUrlItem(string memory url) external onlyOwner {
        sweepstakesTicket.urlItem = url;
    }

    function raffle(uint256 rd) public view onlyOwner returns (uint256) {
        uint range = 10;
        uint256 qtd = sweepstakesTicket.ticketsAmount;
        uint fator = SafeMath.div(qtd, range);
        uint mod = qtd % 10;

        if (mod != 0) {
            fator = fator + 1;
        }
        uint decimalMod = SafeMath.div(fator, range);
        uint limit = fator - decimalMod + 1;
        uint[] memory raffleArray = new uint[](limit + 1);
        uint first = 0;
        uint last = 0;
        for (uint i = 1; i <= limit && last < qtd; i++) {
            first = last + 1;
            last = first + range;
            if (last >= qtd) {
                last = qtd;
            ...
            raffleArray[i] = random(last, first, i + rd);
        }

        uint idx = random(raffleArray.length - 1, 1, rd);
        uint winner = raffleArray[idx];
        return winner;
    }
    ...
    function random(
        ...
        require(maxNumber > minNumber, "require maxNumber  >  minNumber");
        amount =
            uint(
                keccak256(
                    abi.encodePacked(
    ...
    function raffleRegister(uint nonce) external onlyOwner {
        //require(block.timestamp > sweepstakesTicket.dateEnd, "date not end");
        require(
            tokenCounter.current() == sweepstakesTicket.ticketsAmount,
            "not sold out"
        );
        uint256 winner = raffle(nonce);
        sweepstakesTicket.ticketWinner = winner;
        sweepstakesTicket.addrWinner = ownerOf(winner);
        sweepstakesTicket.dateRaffle = block.timestamp;

        saleState = SaleState.Inactive;
        emit SetSaleState(SaleState.Inactive);
        emit WinnerTicket(
            sweepstakesTicket.addrWinner,
            sweepstakesTicket.ticketWinner
        );
    }
}