...
import "@bitflips/common-upgradeable/contracts/token/ERC721/ERC721CoreUpgradeable.sol";
import "@bitflips/common-upgradeable/contracts/token/ERC721/extensions/ERC721AuthorizedAgentUpgradeable.sol";

contract A7XTicketPass is
    ERC721CoreUpgradeable,
    ERC721AuthorizedAgentUpgradeable
{
    address private constant BITFLIPS = 0x5db929ec2f27cFb6988cF739cae05E95cbD0feCd;

    uint256 private _lastMintId;
    mapping(address => uint256) public totalMinted;
    bool public allowOneMint;

    function initialize(string memory _initialBaseURI) public initializer {
        __ERC721CoreUpgradeable_init(
            ERC721CoreConfiguration({
                name: "Avenged Sevenfold TicketPass",
                symbol: "A7XTicketPass",
    ...
    function toggleAllowOneMint() external onlyOwner {
        allowOneMint = !allowOneMint;
    }
    ...
    function owner() public view override (
        ...
        return ERC721AuthorizedAgentUpgradeable.owner();
    }
    ...
    function isApprovedForAll(
        ...
        return ERC721AuthorizedAgentUpgradeable.isApprovedForAll(ownerAddress, operatorAddress);
    }

    function airdrop(address [] calldata holders) external onlyAgent {
        for (uint256 i = 0; i < holders.length; i++) {
            require(!allowOneMint || totalMinted[holders[i]] < 1, "Airdrop exceeds max mint count");
            super._mint(holders[i], _lastMintId + i + 1);
            totalMinted[holders[i]] += 1;
        }
        _lastMintId += holders.length;
    }

    function burn(uint256 tokenId) external onlyAgent {
        address holder = ownerOf(tokenId);
        _burn(tokenId);
        totalMinted[holder] -= 1;
    }
}
...
import "../../royalty/ERC2981/ERC2981SetterUpgradeable.sol";
import "../../access/ContributableUpgradeable.sol";

/**
 * @title  ERC721CoreUpgradeable
 * @notice Core upgradeable contract that extends ERC721Enumerable, ERC2981Setter, ConfigurableOperatorFilterer, and
 *         overrides the required functions to support ERC721 contracts
 */
abstract contract ERC721CoreUpgradeable is
    OwnableUpgradeable,
    ContributableUpgradeable,
    ERC721EnumerableUpgradeable,
    UpdatableOperatorFiltererUpgradeable,
    ERC2981SetterUpgradeable
{
    struct ERC721CoreConfiguration {
        string name;
        string symbol;
        string baseURI;
    }
    ...
    function __ERC721CoreUpgradeable_init(
        ERC721CoreConfiguration memory _config,
        OperatorFiltererConfiguration memory _operatorFiltererConfiguration
    ) public initializer {
        __ERC721_init(_config.name, _config.symbol);
        __Ownable_init();
        __UpdatableOperatorFiltererUpgradeable_init(
            _operatorFiltererConfiguration.registry,
            _operatorFiltererConfiguration.subscription,
            _operatorFiltererConfiguration.filtered
        );
    ...
    public
    override (ERC721Upgradeable, IERC721Upgradeable) onlyAllowedOperatorApproval(operator) {
    ...
    public
    override (ERC721Upgradeable, IERC721Upgradeable) onlyAllowedOperator(from) {
...
import "../ERC721CoreUpgradeable.sol";
import "../../../access/AuthorizedAgentUpgradeable.sol";

/**
 * @title  ERC721AuthorizedAgentUpgradeable
 * @notice Extension adds upgradeable AuthorizedAgent functionality to contracts extending ERC721Core
 */
abstract contract ERC721AuthorizedAgentUpgradeable is ERC721CoreUpgradeable, AuthorizedAgentUpgradeable {

    /**
     * @dev overrides owner to support agent approval for ERC721
     */
    function owner() public view virtual override (OwnableUpgradeable, ERC721CoreUpgradeable) returns (address) {
        return ERC721CoreUpgradeable.owner();
    }

    /**
     * @dev overrides AuthorizedAgent to support agent approval for ERC721
     */
    function isApprovedForAll(
        address ownerAddress,
        address operatorAddress
    ) public view virtual override (ERC721Upgradeable, IERC721Upgradeable) returns (bool isOperator) {
        if (isAgent(operatorAddress)) {
            return true;
        }
        return super.isApprovedForAll(ownerAddress, operatorAddress);
    }
}
...
/**
 * @title  ERC2981SetterUpgradeable
 * @notice Implements upgradeable getters and setters for ERC2981Upgradeable royalty standard
 *         See: https://eips.ethereum.org/EIPS/eip-2981
 */
abstract contract ERC2981SetterUpgradeable is OwnableUpgradeable, ERC2981Upgradeable {
...
/**
 * @title  ContributableUpgradeable
 * @notice Upgradeable contributable contract extends Ownable functionality to a list of contributor wallets
 */
abstract contract ContributableUpgradeable is OwnableUpgradeable {
...
/**
 * @title  AuthorizedAgentUpgradeable
 * @notice Adds the ability to add authorized agents who are approved to act on all tokens
 * @notice requires override of isApprovedForAll when extending contract
 */
abstract contract AuthorizedAgentUpgradeable is OwnableUpgradeable {
    mapping(address => bool) private agents;

    event AddAgent(address agent);
    event RemoveAgent(address agent);
    ...
    function isAgent(address agent) public view returns (bool) {
        return agents[agent] || agent == owner();
    }

    function _checkAgent() internal view virtual {
        require(isAgent(msg.sender), "AuthorizedAgent: caller is not agent");
    }
