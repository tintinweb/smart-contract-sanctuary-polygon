...
contract RaffleTest is VRFConsumerBaseV2, Ownable{
   ...
   // IERC721 public token;
    ...
    mapping(uint => Raffle) public raffle;
   ...
   // mapping(uint => mapping(address => bool)) isParticipate;
    ...
    modifier checkRaffle(uint Raffle_id){
        ...
        require((raffle[Raffle_id].currentRound + 1) <= raffle[Raffle_id].totalRounds, "Raffle Round is not exist");
        _;
    }
    ...
    function startRaffle(uint Raffle_id, uint totalRounds, uint winnersPerRound, uint threshold ,bool regRequired) external onlyOwner {
        require(raffle[Raffle_id].status == RaffleStatus.NotInitialized, "Raffle is already Initialized");
        raffle[Raffle_id] = Raffle({creator: msg.sender, totalRounds: totalRounds, winnersPerRound: winnersPerRound, threshold: threshold, status: RaffleStatus.Initialized, currentRound: 0, regRequired: regRequired});
    }
    ...
    function pickWinners(uint Raffle_id, bytes32[] memory _prizes) external onlyOwner checkRaffle(Raffle_id) returns (uint256 requestId) {
        raffle[Raffle_id].currentRound += 1;
        if(raffle[Raffle_id].currentRound == 1){
        ...
        prizes  = _prizes;
        uint32 numWords = uint32(raffle[Raffle_id].winnersPerRound);
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        ...
        //address[] memory _winners = new address[](0);
