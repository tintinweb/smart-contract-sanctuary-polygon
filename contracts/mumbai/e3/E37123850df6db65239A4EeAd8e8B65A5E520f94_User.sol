...
contract User is ERC721, ERC721Burnable, ERC721Sendable, AccessControl {
    ...
    event centralitiesUpdated(
        uint256 userIdStarting,
        uint256 userIdCurrent,
        uint256 userIdFollower,
        ShortestPath SP
    );
    event fired(uint256 iter);
    event betweennessUpdate(
        uint256 iter,
        uint256 userId,
        uint256 newBetweenness
    );
    event printStartCurrent(
        uint256 startingUserId,
        uint256 currentUserId,
        uint256 followerUserId,
        uint256 depth,
        uint256 p
    );

    uint256 public numNodes;
    uint256 public numDigits;
    uint256 public usernameCount;
    mapping(uint256 => UserStruct) public userIdToUser;
    mapping(address => uint256) public numUsersMinted;
    uint256 public maxMintsPerWallet = 1;
    Rewards public rewards;

    // mapping to be used when graphing
    mapping(uint256 => mapping(uint256 => ShortestPath)) public shortestPath;
    mapping(uint256 => mapping(uint256 => bool)) public prevFollowingUsers;
    mapping(uint256 => mapping(uint256 => mapping(uint256 => mapping(uint256 => uint256))))
        public prevFollowerUsersDepth;
    uint256 public prevFollowingIter;
    uint256 public prevFollowerIter;
    ...
    function mintUser(string memory username, address to)
        public
    ...
    function getUser(uint256 userId) public view returns (UserStruct memory) {
    ...
    function calculateCentralityValues(
        ...
        uint256 p = prevFollowerUsersDepth[prevFollowerIter][startingUserId][
            currentUserId
        ][followerUserId];
        if (p != 0 && depth >= p) {
            return true;
        ...
        if (
            startingUserId == followerUserId || startingUserId == currentUserId
        ) {
            prevFollowerUsersDepth[prevFollowerIter][startingUserId][
                currentUserId
            ][followerUserId] = depth;
            return true;
        }

        prevFollowerUsersDepth[prevFollowerIter][startingUserId][currentUserId][
            followerUserId
        ] = depth;
        ...
        if (SP.size == 0) {
            ...
            shortestPath[startingUserId][followerUserId].prev.push(
                currentUserId
            );
            if (depth > 1) {
                userIdToUser[currentUserId]
                    .betweennessCentrality += (numDigits /
                    (SP.prev.length + 1));
                emit betweennessUpdate(
                    0,
                    currentUserId,
                    userIdToUser[currentUserId].betweennessCentrality
                );
            }
            ...
            if (depth > 1) {
                ...
                if (!stop) {
                    uint256 oldBetweenness = (numDigits / SP.prev.length);
                    uint256 newBetweenness = (numDigits / (SP.prev.length + 1));
                    userIdToUser[currentUserId]
                        .betweennessCentrality += newBetweenness;
                    emit betweennessUpdate(
                        1,
                        currentUserId,
                        userIdToUser[currentUserId].betweennessCentrality
                    ...
                    for (uint256 i; i < SP.prev.length; i++) {
                        userIdToUser[SP.prev[i]]
                            .betweennessCentrality -= oldBetweenness;
                        userIdToUser[SP.prev[i]]
                            .betweennessCentrality += newBetweenness;
                        emit betweennessUpdate(
                            1,
                            SP.prev[i],
                            userIdToUser[SP.prev[i]].betweennessCentrality
                        );
                    }
                    shortestPath[startingUserId][followerUserId].size = depth;
                    shortestPath[startingUserId][followerUserId].prev.push(
            ...
            userIdToUser[startingUserId].farnessCentrality -=
                SP.size *
                numDigits;
    ...
    function updateCentrality(
        ...
        if (user.followers.length > 0) {
            for (uint256 i; i < user.followers.length; i++) {
                ...
                // }
                bool done = calculateCentralityValues(
                    startingUserId,
                    currentUserId,
                    user.followers[i],
                    depth + 1
    ...
    function updateFromFollowHead(uint256 userId) public {
    ...
    function follow(
        uint256 userIdThatFollowed,
        uint256 userIdToFollow,
        address sender
    ) public onlyRole(MINTER_ROLE) {
        UserStruct memory user = userIdToUser[userIdThatFollowed];
        uint256 followingLength = userIdToUser[userIdThatFollowed]
            .following
            .length;
        for (uint256 i; i < followingLength; i++) {
            require(
                userIdToFollow != user.following[i],
    ...
    function unFollow(
        uint256 userIdToUnFollowed,
        uint256 userIdThatUnFollowed,
        address sender
    ) public onlyRole(MINTER_ROLE) {
        ...
        uint256 followersAfter = userIdToUser[userIdThatUnFollowed]
            .following
            .length;
        emit unFollowHappened(userIdToUser[userIdThatUnFollowed], sender);
    }

    function getsCentralitiesNormalized(uint256 userId)
        public
        view
        returns (uint256[3] memory)
    {
        uint256[3] memory result = [uint256(0), uint256(0), uint256(0)];
        if (numNodes == 0) {
            return result;
        }
        // These are numbers between 0 and 1 so multiply by numDigits
        UserStruct memory user = userIdToUser[userId];
        uint256 degree = user.degreeCentrality / (numNodes * numDigits);
        uint256 closeness = 0;
        if (user.farnessCentrality != 0) {
            closeness = ((numNodes - 1) * numDigits) / (user.farnessCentrality);
        }
        uint256 betweenness = user.farnessCentrality /
            ((numNodes - 1) * (numNodes - 2) * numDigits);
        result = [degree, closeness, betweenness];
        return result;
    }

    function getCentralityScore(uint256 userId) public view returns (uint256) {
        ...
        uint256[3] memory weights = [uint256(1), uint256(1), uint256(1)];
        if (centralities[0] < centralities[1]) {
            weights[0]++;
        } else {
            weights[1]++;
        }
        if (centralities[0] < centralities[2]) {
            weights[0]++;
        } else {
            weights[2]++;
        }
        if (centralities[1] < centralities[2]) {
            weights[1]++;
        } else {
            weights[2]++;
        }
    ...
    function addExp(
        ...
        */
        ...
        if (user.experience >= user.expToNextLvl) {
            user.level++;
            user.expToNextLvl +=
                (user.expToNextLvl * uint256(1104)) /
                uint256(1000);
        }
    ...
    function changeRewards(Rewards memory _rewards)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        rewards = _rewards;
    ...
    function getLevel(uint256 userId) public view returns (uint256) {
        return userIdToUser[userId].level;
    }

    function getScore(uint256 userId) public view returns (uint256) {
        uint256 centralityScore = getCentralityScore(userId);
        uint256 userLevel = userIdToUser[userId].level;
        uint256 userCaptureRate = userIdToUser[userId].captureRate;
