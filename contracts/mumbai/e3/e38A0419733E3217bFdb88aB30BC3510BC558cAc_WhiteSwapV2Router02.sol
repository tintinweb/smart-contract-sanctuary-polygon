...
contract WhiteSwapV2Router02 is IWhiteSwapV2Router02 {
    ...
    constructor(address _factory, address _WrappedNative) public {
        factory = _factory;
        WrappedNative = _WrappedNative;
    }

    receive() external payable {
        assert(msg.sender == WrappedNative); // only accept Native via fallback from the WrappedNative contract
    }
    ...
    function addLiquidityNative(
        ...
        address pair = WhiteSwapV2Library.pairFor(factory, token, WrappedNative);
    ...
    function removeLiquidityNativeSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransfer(token, to, INative(token).balanceOf(address(this)));
        IWrappedNative(WrappedNative).withdraw(amountNative);
        TransferHelper.safeTransferNative(to, amountNative);
    }
    ...
    {
        require(path[0] == WrappedNative, 'WhiteSwapV2Router: INVALID_PATH');
        amounts = WhiteSwapV2Library.getAmountsOut(factory, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'WhiteSwapV2Router: INSUFFICIENT_OUTPUT_AMOUNT');
        IWrappedNative(WrappedNative).deposit{value: amounts[0]}();
        assert(IWrappedNative(WrappedNative).transfer(WhiteSwapV2Library.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    ...
    returns (uint[] memory amounts)
    {
        require(path[path.length - 1] == WrappedNative, 'WhiteSwapV2Router: INVALID_PATH');
        amounts = WhiteSwapV2Library.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, 'WhiteSwapV2Router: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, WhiteSwapV2Library.pairFor(factory, path[0], path[1]), amounts[0]
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            { // scope to avoid stack too deep errors
                ...
                amountInput = INative(input).balanceOf(address(pair)).sub(reserveInput);
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        uint balanceBefore = INative(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            INative(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
    ...
    ensure(deadline)
    {
        ...
        assert(IWrappedNative(WrappedNative).transfer(WhiteSwapV2Library.pairFor(factory, path[0], path[1]), amountIn));
        uint balanceBefore = INative(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            INative(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
    ...
    ensure(deadline)
    {
        ...
        uint amountOut = INative(WrappedNative).balanceOf(address(this));
...
library WhiteSwapV2Library {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'fdea32460289a55373734abcedee6cb1669bec4da2c74d86eb62d2a088a7b438' // init code hash
            ))));
    }
