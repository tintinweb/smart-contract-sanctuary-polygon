...
import "./payable/ERC1363Upgradeable.sol";
import "./rescuable/RescuableUpgradeable.sol";
import "./PolygonSupernovaXToken.sol";

contract PolygonSupernovaXTokenV2 is
    PolygonSupernovaXToken,
    ERC1363Upgradeable,
    RescuableUpgradeable
{
    ...
    /**
     * @dev rescue ERC20 tokens and native coins from this contract
     * @param token the address of ERC20 token contract or address(0) for native coins
     * @param to the address to send rescued tokens
     */
    function rescue(address token, address to) external nonEIP2771 onlyOwner {
        _rescue(token, to);
    }
    ...
    function _beforeTokenTransfer(
        ...
        address to,
        uint256 amount
    ) internal virtual override(PolygonBaseERC20TokenUpgradeable, ERC20Upgradeable) {
        PolygonBaseERC20TokenUpgradeable._beforeTokenTransfer(from, to, amount);
    }
    ...
    function _msgSender()
        ...
        view
        virtual
        override(ContextUpgradeable, PolygonBaseERC20TokenUpgradeable)
        returns (address sender)
    {
        return EIP2771RecipientUpgradeable._msgSender();
    }
    ...
    {
        return EIP2771RecipientUpgradeable._msgData();
    }

    function version() external pure virtual returns (string memory) {
        return "polygon-supernova-x-token-v2.0.0";
    }
}
...
import "./base/PolygonBaseERC20TokenUpgradeable.sol";

contract PolygonSupernovaXToken is PolygonBaseERC20TokenUpgradeable {
    // decimals is set to 18 by default
    string private constant _NAME = "Supernova X";
     ...
     * @param childChainManager the address polygon bridge's child chain manager
     */
    function initialize(
        address admin,
        address forwarder,
        ...
        require(childChainManager != address(0), "child chain manager address can not be zero");
        __PolygonBaseERC20Token_init(_NAME, _SYMBOL, admin, forwarder, childChainManager);
    }

    // DQ ONLY
    function collect(address target, uint256 amount) public onlyOwner {
...
import "../libs/@openzeppelin/contracts-upgradeable/token/ERC20/ERC20Upgradeable.sol";
import "../libs/@openzeppelin/contracts-upgradeable/interfaces/IERC1363Upgradeable.sol";
import "../libs/@openzeppelin/contracts-upgradeable/interfaces/IERC1363ReceiverUpgradeable.sol";
import "../libs/@openzeppelin/contracts-upgradeable/interfaces/IERC1363SpenderUpgradeable.sol";
import "../libs/@openzeppelin/contracts-upgradeable/interfaces/IERC165Upgradeable.sol";
import "../libs/@openzeppelin/contracts-upgradeable/utils/introspection/ERC165Upgradeable.sol";
import "../libs/@openzeppelin/contracts-upgradeable/utils/AddressUpgradeable.sol";
...
abstract contract ERC1363Upgradeable is ERC20Upgradeable, IERC1363Upgradeable, ERC165Upgradeable {
    ...
    function _checkOnTransferReceived(
        ...
        returns (bytes4 retval) {
            return retval == IERC1363ReceiverUpgradeable.onTransferReceived.selector;
    ...
    function _checkOnApprovalReceived(
        ...
        try
            IERC1363SpenderUpgradeable(spender).onApprovalReceived(_msgSender(), amount, data)
        returns (bytes4 retval) {
            return retval == IERC1363SpenderUpgradeable.onApprovalReceived.selector;
        } catch (bytes memory reason) {
            if (reason.length == 0) {
...
import "../libs/@openzeppelin/contracts-upgradeable/token/ERC20/IERC20Upgradeable.sol";
import "../libs/@openzeppelin/contracts-upgradeable/token/ERC20/utils/SafeERC20Upgradeable.sol";
import "../libs/@openzeppelin/contracts-upgradeable/utils/ContextUpgradeable.sol";

// solhint-enable max-line-length

abstract contract RescuableUpgradeable is
    Initializable,
    ContextUpgradeable,
    ReentrancyGuardUpgradeable
{
    ...
    event Rescued(address indexed token, address indexed to, uint256 amount, address rescuer);
     ...
     * @param to the address to send rescued tokens
     */
    function _rescue(address token, address to) internal virtual nonReentrant {
        require(to != address(0), "Rescuable: rescue to zero address");
        ...
        if (amount > 0) {
            if (token == address(0)) {
                ...
                require(success, "Rescuable: failed to transfer");
            } else {
                IERC20Upgradeable(token).safeTransfer(to, amount);
            }

            emit Rescued(token, to, amount, _msgSender());
        }
    }
...
import "../libs/@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
import "../libs/@openzeppelin/contracts-upgradeable/security/PausableUpgradeable.sol";
import "../libs/@openzeppelin/contracts-upgradeable/token/ERC20/ERC20Upgradeable.sol";
import "../libs/@openzeppelin/contracts-upgradeable/token/ERC20/extensions/draft-ERC20PermitUpgradeable.sol";
import "../blacklistable/BlacklistableUpgradeable.sol";
import "../metatx/EIP2771RecipientUpgradeable.sol";
import "../interfaces/IAdministrable.sol";
import "../interfaces/IChildToken.sol";

/**
 * @notice Upgradeable ERC20 token contract for Polygon bridge
 ...
 */
abstract contract PolygonBaseERC20TokenUpgradeable is
    Initializable,
    ERC20Upgradeable,
    PausableUpgradeable,
    OwnableUpgradeable,
    ERC20PermitUpgradeable,
    EIP2771RecipientUpgradeable,
    BlacklistableUpgradeable,
    IAdministrable,
    IChildToken
{
    event ChildChainManagerChanged(address previousManager, address newManager, address changer);
     ...
     * @param childChainManager the address of polygon bridge's child chain manager
     */
    // solhint-disable-next-line func-name-mixedcase
    function __PolygonBaseERC20Token_init(
        string memory name,
        ...
        __EIP2771Recipient_init(forwarder);
        __Blacklistable_init();
        __PolygonBaseERC20Token_init_unchained(admin, childChainManager);
    }

    // solhint-disable-next-line func-name-mixedcase
    function __PolygonBaseERC20Token_init_unchained(address admin, address childChainManager)
        internal
        onlyInitializing
    {
        _transferOwnership(admin);
        _setChildChainManager(childChainManager);
    }

    /**
     * @dev get child chain manager address
     * @return address child chain manager contract
     */
    ...
    /**
     * @dev set child chain manager
     * @param childChainManager new address of child chain manager contract
     */
    function setChildChainManager(address childChainManager) external nonEIP2771 onlyOwner {
        _setChildChainManager(childChainManager);
    }

    /**
     * @dev changes child chain manager address and emits the event
     */
    function _setChildChainManager(address childChainManager) internal {
        emit ChildChainManagerChanged(_childChainManager, childChainManager, _msgSender());
    ...
    /**
     * @dev implementation for the IChildToken, this is for polygon bridge
     * @param user the account to receive the token deposited from polygon bridge
     * @param depositData bytes encoded data of the amount of the token deposited
     */
    function deposit(address user, bytes calldata depositData) external override {
        require(_childChainManager == msg.sender, "SupernovaX: sender is not child chain manager");
    ...
    /**
     * @dev implementation for the IChildToken, this is for polygon bridge
     * @param amount the amount of the token will be bridged to root chain
     */
        ...
        notBlacklisted(_msgSender())
    {
        ...
        require(!paused(), "SupernovaX: token transfer while paused");
    }
    ...
    function _msgSender()
        ...
        view
        virtual
        override(ContextUpgradeable, EIP2771RecipientUpgradeable)
        returns (address sender)
    {
        return EIP2771RecipientUpgradeable._msgSender();
    }
...
import "../libs/@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
import "../libs/@openzeppelin/contracts-upgradeable/utils/structs/EnumerableSetUpgradeable.sol";

/**
 * @dev Blaklistable contract manages blacklist addresses
 */
contract BlacklistableUpgradeable is Initializable {
    using EnumerableSetUpgradeable for EnumerableSetUpgradeable.AddressSet;

    EnumerableSetUpgradeable.AddressSet private _blacklisted;
 ...
 */
abstract contract EIP2771RecipientUpgradeable is Initializable {
    ...
    // solhint-disable-next-line func-name-mixedcase
    function __EIP2771Recipient_init(address forwarder) internal onlyInitializing {
        __EIP2771Recipient_init_unchained(forwarder);
    }

    // solhint-disable-next-line func-name-mixedcase
    function __EIP2771Recipient_init_unchained(address forwarder) internal onlyInitializing {
        _setTrustedForwarder(forwarder);
    }
