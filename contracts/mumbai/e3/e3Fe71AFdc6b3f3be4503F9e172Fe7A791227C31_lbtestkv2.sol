...
contract lbtestkv2 is VRFV2WrapperConsumerBase,ConfirmedOwner,ReentrancyGuard {
	using SafeMath for uint256;
    ...
    uint256 minimumPot = 200 * 10**18;

    uint256 minimumPrize;
    uint256 incrementTicketIds;
    ...
    uint256 internal TICKET_VAL = 20 * 10**stableDecimals; //minimum amount (in wei) for getting registered on list
        ...
        VRFV2WrapperConsumerBase(linkAddress, wrapperAddress) 
    {
        ...
        priceFeed = AggregatorV3Interface(0xd0D5e3DB44DE05E9F294BB0a3bEEaF030DE24Ada); // (0xAB594600376Ec9fD91F8e885dADF0CE036862dE0); // MATIC/USD Price feed
        
        lottoWeekly = IluckblocksWeekly(0xe672Adbf19CB2418069EDE541722f063B14ca618);
        weeklyContract = 0xe672Adbf19CB2418069EDE541722f063B14ca618;
    ...
    function getJackpotinUSD() public view returns (uint256) {
       uint256 tprice = getLatestPrice(krstmTrading,usdTrading,1 * 10**18);
       tprice = tprice * 10**12;
    ...
    function withdrawExcessBalance () public onlyOwner() {
        ...
        krstm.safeTransfer(msg.sender,contractBalance);
    ...
    function fillJackpot(uint256 amount) external {
        ...
        uint256 tprice = getLatestPrice(krstmTrading,usdTrading,1 * 10**18);
        tprice = tprice * 10**12;
        uint256 amountInToken = (40 * 10**18 * 10**18) / tprice;  // 40 usd in token

        minimumPrize = amountInToken;

        require(amount >= minimumPrize,"not enough amount - below 40 usd");
        require(krstm.balanceOf(msg.sender) >= amount,"not enough balance");
        
        krstm.safeTransferFrom(msg.sender,address(this),amount);
        ...
        if(registeredsAccts.length < 1){
            // Add 2 contract tickets
            for(uint index = 0; index < 2; index++){
            
                uint256 indexCheck = incrementTicketIds;
    ...
    function accumulateJackpot(uint256 amount) internal {
        ...
        if(amount > minimumPrize){
         ...
         krstm.safeTransfer(weeklyContract,amount.mul(15).div(100));
    ...
    function swapTokensToUSD(uint256 tokenAmount, address user, uint typeSwap) private {
        ...
        krstm.approve(address(uniswapRouter), (amountInToken+1000000000000000000000));

        //Get Previous Amounts
        uint256 prevBalance = krstm.balanceOf(address(this));

        // make the swap
        uniswapRouter.swapTokensForExactTokens(
            tokenAmount,
            amountInToken+1000000000000000000000, // to garantee the trade
            path,
            user, // receiving user
            block.timestamp + 300
        );

        // new balance and math
        uint256 nowBalance = krstm.balanceOf(address(this));

        uint256 deduct = prevBalance - nowBalance;

        if(typeSwap == 1){
         
         prizeValueJackpot -= deduct;

        } else if (typeSwap == 0){

         contractBalance -= deduct;
    ...
    function getRandomNumber() internal returns (uint256 requestId) {
        ...
        // minimum 20 usd to nftnodes if sufficient balance

        if(contractBalance >= minimumPrize*3){
                         
            swapTokensToUSD((20 * 10**stableDecimals),address(this),0);
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory _randomWords) internal override {
        ...
        if(userWallet == address(this) || userWallet == address(0) || idInPosition < 2){
            ...
            krstm.safeTransfer(lastWinner,prizeValueJackpot);
    ...
    function BuyTicket(address player,uint quantity) public nonReentrant {
        ...
        // update balances
         ...
         prizeValueJackpot = (krstm.balanceOf(address(this)) - contractBalance); // ((dailyFeed * 10**12) / tprice) / 10**10;
    ...
    function ForfeitTicket(uint256 index) public nonReentrant {
        ...
        require(getJackpotinUSD() >= 20 * 10**18, "Contract need to have atleast 20 USD in value.");

        uint256 ticketPosition;

        uint256 weeklyDraw = lottoWeekly.requestCounter();

        uint256[] memory getTicketsWeekly = lottoWeekly.getUserTicketsByDraw(weeklyDraw,owner);
