...
import {HeyMintStorage, TokenConfig, BaseConfig, AdvancedConfig, Data} from "../libraries/HeyMintStorage.sol";
...
contract HeyMintERC1155ExtensionC is HeyMintERC1155Upgradeable {
     ...
     */
    function getTokenSettings(uint16 tokenId)
    external
    view
    ...
    /**
     * @notice Update the full config (base config + all tokens) on the contract.
     */
    function updateFullConfig(
        BaseConfig memory _baseConfig, TokenConfig[] memory _tokenConfigs, AdvancedConfig memory _advancedConfig
    ) external onlyOwner {
        HeyMintStorage.State storage state = HeyMintStorage.state();
        uint256 oldTokensLength = state.data.tokenIds.length;
        this.updateBaseConfig(_baseConfig);
        delete state.data.tokenIds;
        for (uint256 i = 0; i < _tokenConfigs.length; i++) {
            this.upsertToken(_tokenConfigs[i]);
        }
        // Pop any extra tokenIds if the new array is shorter
        for (uint256 i = oldTokensLength; i > _tokenConfigs.length; i--) {
            state.data.tokenIds.pop();
        }
        state.advCfg = _advancedConfig;
        // TODO: also update burn tokens w/ this function or nah?
    }
 ...
 * @notice Sets a URI for a specific token id.
    ...
    function setTokenUri(
        ...
        HeyMintStorage.state().tokens[_tokenId].tokenUri = _newTokenURI;
    }
    ...
    function setGlobalUri(string calldata _newTokenURI) external onlyOwner {
        HeyMintStorage.state().cfg.uriBase = _newTokenURI;
    }
