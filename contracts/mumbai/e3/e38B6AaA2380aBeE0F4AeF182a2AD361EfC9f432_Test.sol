...
contract Test{
    ...
    function updateX6Referrer(address userAddress, address referrerAddress, uint8 level) private{
        ...
        if(users[referrerAddress].holdAmount[level]<mlevelPrice[level+1] && !(users[referrerAddress].activeX6Levels[level+1]) && level<9)
        {
          ...
          if(users[referrerAddress].holdAmount[level]>=mlevelPrice[level+1] && !(users[referrerAddress].activeX6Levels[level+1]))
          {
        ...
        else
        {
            if(level==9 && users[referrerAddress].x6Matrix[level].referrals.length==0)
            {
                ...
                address(uint160(referrerAddress)).transfer(mlevelPrice[level]-ded);
                address freeX6Referrer = findFreeX6Referrer(1);
            ...
            users[referrerAddress].x6Matrix[level].referrals.push(userAddress);
            emit NewUserPlace(userAddress, referrerAddress, 2, level, uint8(users[referrerAddress].x6Matrix[level].referrals.length));
            address(uint160(owner)).transfer((mlevelPrice[level]*5)/100); 
            return address(uint160(referrerAddress)).transfer((mlevelPrice[level]*95)/100);
            }
            users[referrerAddress].x6Matrix[level].referrals.push(userAddress);
            emit NewUserPlace(userAddress, referrerAddress, 2, level, uint8(users[referrerAddress].x6Matrix[level].referrals.length));
             if(level<6)
             {
                ...
                address(uint160(referrerAddress)).transfer((((mlevelPrice[level]*95)/100)-sponsorBonus[level]));
             }
