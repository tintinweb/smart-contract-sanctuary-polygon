...
import "./interfaces/IHonestTraits.sol";

enum MintToken {
	MATIC
}

contract HonestFarmerClubV1 is ERC1155, Ownable {
	/**
	 * State
	 */
...
interface IHonestTraits {
	function uri(uint256 tokenId) external view returns (string memory);

	function increaseXP(uint256 tokenId, uint256 xp) external;

	function levelUp(uint256 tokenId, uint8 levelId) external;
}
