...
contract EtheremonAdventureHandler is BasicAccessControl, EthermonEnum {
    uint8 public constant STAT_MAX_VALUE = 31;
    uint8 public constant LEVEL_MAX_VALUE = 254;
    ...
    function handleSingleItem(
        ...
        if (_classId == expItemClass) {
            ...
            //Made this changeable with mod access function either add exp to EthermonData or EthermonSeason Exp.

            uint8 currentLevel = getLevel(uint32(currentSeasonExp));
            currentLevel += uint8(_value);
            // if (
            //     levelExps[currentLevel - 1] < obj.exp ||
            //     currentLevel > LEVEL_MAX_VALUE
            // ) revert();
            require(
                levelExps[currentLevel - 1] > currentSeasonExp &&
                    currentLevel < LEVEL_MAX_VALUE,
                "Invalid level exp or current level"
            );
    ...
    function handleMultipleItems(
        ...
        uint8 updatedValue = currentValue + 3;
    ...
    //     objWithStats.exp = getMonSeasonExp(obj.monsterId);
    //     return objWithStats;
    // }
