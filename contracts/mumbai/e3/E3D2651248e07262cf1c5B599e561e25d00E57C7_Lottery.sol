...
contract Lottery {
    ...
    function pickWinnerForGrand() internal  {
        require(grandPrizeList.length>grandPrizeNum,"over");
        uint loopIndex=0;
    ...
    function pickWinnerForFist() internal  {
        require(grandPrizeList.length>firstPrizeNum,"over");
        uint loopIndex=0;
        for(uint256 i=0;i<totalParticipation;i++){
            loopIndex=i;
    ...
    function pickWinnerForSecond() internal  {
        require(grandPrizeList.length>secondPrizeNum,"over");
        uint loopIndex=0;
        for(uint256 i=0;i<totalParticipation;i++){
            loopIndex=i;
    ...
    function pickWinnerForThird() internal  {
        require(grandPrizeList.length>thirdPrizeNum,"over");
        uint loopIndex=0;
        for(uint256 i=0;i<totalParticipation;i++){
            loopIndex=i;
    ...
    function pickWinnerForFourth() internal  {
        require(grandPrizeList.length>fourthPrizeNum,"over");
        times++;
        uint loopIndex=0;
        if(lists.length>0){
            delete(lists);
