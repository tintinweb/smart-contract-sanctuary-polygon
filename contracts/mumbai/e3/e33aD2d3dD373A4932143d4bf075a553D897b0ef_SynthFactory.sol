...
import "./interfaces/ISynths.sol";
import "./interfaces/IAdmin.sol";
import "../Periphery/BasicMetaTransaction.sol";

contract SynthFactory is Initializable, BasicMetaTransaction {
    ...
    function initialize(address _masterToken, address _admin) external {
        admin = IAdmin(_admin);
    ...
    function createToken(
        ...
        require(admin.hasRole(OPERATOR, _msgSender()), "OnlyOperator");
        address instance = Clones.clone(masterToken);
        ISynths(instance).initialize(name_, symbol_,decimals_,owner_, totalTokenSupply_,tokenID);
        emit TokenCreated(instance);
    }
...
interface IAdmin is IAccessControl {
    ...
    function setSynthFactory(address _address) external;
