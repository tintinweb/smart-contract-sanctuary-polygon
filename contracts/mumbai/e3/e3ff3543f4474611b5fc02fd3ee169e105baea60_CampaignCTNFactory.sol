...
import "./dependencies/openzeppelin/interfaces/IAccessControl.sol";
...
import "./dependencies/openzeppelin/BeaconProxy.sol";
import "./dependencies/openzeppelin/UpgradeableBeacon.sol";
contract CampaignCTNFactory is OwnableUpgradeable, PausableUpgradeable{
  ...
  function initialize(address _campaignCTNBluePrint, address _campaignCTNNoAirdropBluePrint) external initializer{
  ...
  */
  function createCampaignCTN(
    ...
    if (params.tokenType == DataTypes.TOKEN_TYPE.ERC721 || params.tokenType == DataTypes.TOKEN_TYPE.ERC1155) revert UnSupportedError();
    ...
    if (params.tokenType == DataTypes.TOKEN_TYPE.NONE) {
      ...
      (campaign, params.token) = _createCampaignCTN(params, _signature);
    }
  ...
  * 0x0eb624be --- initialize(address,string,bytes)
  */
  function _createCampaignCTN(DataTypes.CampaignCTNInfo memory _params, bytes memory _signature) internal returns(address, address) {
...
library TransferHelper {
  ...
  function safeTransferFrom(
    ...
    if (token == DataTypes.ETH_ADDRESS) require(msg.value >= value, "STF");
  ...
  function safeTransfer(
    ...
    if (token == DataTypes.ETH_ADDRESS) safeTransferETH(to, value);
  ...
  function nft721SafeMint(
    ...
    (bool success, ) = token.call(abi.encodeWithSelector(0x6a627842, to));
    if (success == false) {
      (success, ) = token.call(abi.encodeWithSelector(0x755edd17, to));
      if (success == false) revert('nft721SafeMint');
    }
  }
