...
contract OneofMarketplace is Marketplace {
    constructor(address _nativeTokenWrapper) initializer Marketplace(_nativeTokenWrapper) {}

    function createListingWorkaround(address assetContract, uint256 tokenId, uint256 startTime,
        uint256 secondsUntilEndTime, uint256 quantityToList, address currencyToAccept,
        uint256 reservePricePerToken, uint256 buyoutPricePerToken, ListingType listingType
    ) external {
        ListingParameters memory _params = ListingParameters(
            assetContract, tokenId, startTime, secondsUntilEndTime,
            quantityToList, currencyToAccept, reservePricePerToken, buyoutPricePerToken, listingType);
        (bool success, ) = address(this).delegatecall(abi.encodeWithSignature(
                "createListing((address,uint256,uint256,uint256,uint256,address,uint256,uint256,uint8))", _params));

        require(success, "createListingWorkaround: delegatecall failed");
    }
}