...
contract TournamentManager is VRFConsumerBaseV2 {
  ...
  function submitRound(address[] memory looser) public onlyOwner {
  ...
  function createTeam(uint256[5] memory gotchis, uint256 gotchiLeader, uint256[5][2] memory gotchiFormation) public {
    ...
    registerTeam(wrapToTeam(gotchis, gotchiLeader, gotchiFormation));
  }
  ...
  /*Remove the team at teamIndex, the last item of the team array will replace the team at teamIndex */
  function deleteTeam(uint256 teamIndex) internal {
    require(teamIndex < participantToTeams[msg.sender].length, "Team index invalid");

    uint256 lastIndex = participantToTeams[msg.sender].length - 1;

    // move the element of the last place of the team array to the teamIndex
    participantToTeams[msg.sender][teamIndex] = participantToTeams[msg.sender][lastIndex];
    // Supress the last element (that will be present 2 times in the array)
    participantToTeams[msg.sender].pop();
}



  function wrapToTeam(uint256[5] memory gotchis, uint256 gotchiLeader, uint256[5][2] memory gotchiFormation) internal pure returns (Team memory) {
    Team memory newTeam;
  ...
  function modifyTeamBeforeBattle(uint256 teamIndex, uint256[5] memory gotchis, uint256 gotchiLeader, uint256[5][2] memory gotchiFormation) external {
    require(gotchis.length == 5, "Invalid number of Gotchi IDs provided.");
    require(tournament_state == TOURNAMENT_STATE.INSCRIPTION_OPEN, "Tournament not open");
    ...
    participantToTeams[(msg.sender)][teamIndex] = wrapToTeam(gotchis, gotchiLeader, gotchiFormation);
  }

  function modiftyTeamAfterBattle(uint256 teamIndex, uint256[5] memory gotchis, uint256 gotchiLeader, uint256[5][2] memory gotchiFormation) external {
    require(gotchis.length == 5, "Invalid number of Gotchi IDs provided.");
