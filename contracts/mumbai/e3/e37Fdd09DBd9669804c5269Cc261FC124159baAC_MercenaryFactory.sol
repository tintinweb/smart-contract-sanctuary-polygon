...
//可暂停雇佣兵铸造合约
contract MercenaryFactory is Ownable, ReentrancyGuard, AccessControlEnumerable, IFactory, Pausable {
    ...
    bytes32 hash_move = keccak256(abi.encodePacked("move"));
    bytes32 hash_power = keccak256(abi.encodePacked("power"));
    bytes32 hash_capacity = keccak256(abi.encodePacked("capacity"));
    bytes32 hash_potential = keccak256(abi.encodePacked("potential"));
    bytes32 hash_scan = keccak256(abi.encodePacked("scan"));

    struct Mercenary {
        uint256 item_config_id;       //物品id
        uint256 weight;               //权重
        uint256 prop_sum;             //属性总和(attack + defense + hp )
        uint256 count;                //实际数量
        bytes32 margin_name;           //余量属性名称, 总和减去其它
    }
    //index 与 Mercenary 的映射
    mapping(uint256 => Mercenary) mercenaries;

    struct Setting {
        uint256 base;             //基数,  如： 100
        uint256 section;          //区间,  如： 50
    }
    ...
    struct Character {
        uint256 attack;        //攻击, 总值
        uint256 defense;       //防御, 总值
        uint256 hp;            //生命, 总值
        uint256 move;          //移速
        uint256 power;         //源力
        uint256 capacity;      //背包最大容量
    ...
    UpgradeSection[] upgrade_sections;
       ...
       //将部署者设置为超级管理员
        ...
        DOMAIN_SEPARATOR = keccak256(
                abi.encode(
                    ...
                    keccak256(bytes("MercenaryFactory")),
                    keccak256(bytes("1")),
                    block.chainid,
                    address(this)
                )
        ...
        //初始化
        //学徒（攻击、生命） 
        uint _type_id = _addItem(500001,10000,310,"attack");
        _setPropValues(_type_id, "attack",100,150);
        _setPropValues(_type_id, "defense",10,50);
        _setPropValues(_type_id, "hp",80,100);
        _setPropValues(_type_id, "scan",0,0);
        _setPropValues(_type_id, "potential",1,4);
        _setPropValues(_type_id, "move",30,50);
        _setPropValues(_type_id, "power",50,100);
        //胖子（防御、生命） 
         _type_id = _addItem(500002,10000,309,"defense");
        _setPropValues(_type_id, "attack",50,100);
        _setPropValues(_type_id, "defense",50,100);
        _setPropValues(_type_id, "hp",80,100);
        _setPropValues(_type_id, "scan",50,100);
        _setPropValues(_type_id, "potential",1,4);
        _setPropValues(_type_id, "move",30,50);
        _setPropValues(_type_id, "power",50,100);
        //蒸汽（攻击、移速） 
         _type_id = _addItem(500003,15000,308,"attack");
        _setPropValues(_type_id, "attack",100,150);
        _setPropValues(_type_id, "defense",10,75);
        _setPropValues(_type_id, "hp",10,75);
        _setPropValues(_type_id, "scan",50,100);
        _setPropValues(_type_id, "potential",1,4);
        _setPropValues(_type_id, "move",50,70);
        _setPropValues(_type_id, "power",50,100);
        //兔子（移速、容量） 
         _type_id = _addItem(500004,20000,307,"hp");
        _setPropValues(_type_id, "attack",50,100);
        _setPropValues(_type_id, "defense",50,100);
        _setPropValues(_type_id, "hp",50,100);
        _setPropValues(_type_id, "scan",50,100);
        _setPropValues(_type_id, "potential",1,4);
        _setPropValues(_type_id, "move",70,100);
        _setPropValues(_type_id, "power",50,100);
        //瞭望（扫描、源力） 
         _type_id = _addItem(500005,20000,306,"hp");
        _setPropValues(_type_id, "attack",50,100);
        _setPropValues(_type_id, "defense",50,100);
        _setPropValues(_type_id, "hp",50,100);
        _setPropValues(_type_id, "scan",200,300);
        _setPropValues(_type_id, "potential",1,4);
        _setPropValues(_type_id, "move",30,50);
        _setPropValues(_type_id, "power",100,150);
        //教练） 
         _type_id = _addItem(500101,5000,360,"attack");
        _setPropValues(_type_id, "attack",120,170);
        _setPropValues(_type_id, "defense",20,60);
        _setPropValues(_type_id, "hp",100,120);
        _setPropValues(_type_id, "scan",50,100);
        _setPropValues(_type_id, "potential",1,5);
        _setPropValues(_type_id, "move",30,50);
        _setPropValues(_type_id, "power",50,100);
        //力士） 
         _type_id = _addItem(500102,5000,359,"defense");
        _setPropValues(_type_id, "attack",60,110);
        _setPropValues(_type_id, "defense",70,120);
        _setPropValues(_type_id, "hp",100,120);
        _setPropValues(_type_id, "scan",50,100);
        _setPropValues(_type_id, "potential",1,5);
        _setPropValues(_type_id, "move",30,50);
        _setPropValues(_type_id, "power",50,100);
        //奔腾） 
         _type_id = _addItem(500103,7500,358,"attack");
        _setPropValues(_type_id, "attack",120,170);
        _setPropValues(_type_id, "defense",20,85);
        _setPropValues(_type_id, "hp",30,95);
        _setPropValues(_type_id, "scan",50,100);
        _setPropValues(_type_id, "potential",1,5);
        _setPropValues(_type_id, "move",50,70);
        _setPropValues(_type_id, "power",50,100);
        //闪电） 
         _type_id = _addItem(500104,10000,357,"hp");
        _setPropValues(_type_id, "attack",60,110);
        _setPropValues(_type_id, "defense",70,120);
        _setPropValues(_type_id, "hp",70,120);
        _setPropValues(_type_id, "scan",50,100);
        _setPropValues(_type_id, "potential",1,5);
        _setPropValues(_type_id, "move",70,100);
        _setPropValues(_type_id, "power",50,100);
        //鹰眼） 
         _type_id = _addItem(500105,10000,356,"hp");
        _setPropValues(_type_id, "attack",60,110);
        _setPropValues(_type_id, "defense",70,120);
        _setPropValues(_type_id, "hp",70,120);
        _setPropValues(_type_id, "scan",200,300);
        ...
        //强化次数对应的最小最大值
        upgrade_sections.push(UpgradeSection(1,10));  //第一次强化
        upgrade_sections.push(UpgradeSection(10,20));  //第二次强化
        upgrade_sections.push(UpgradeSection(20,30));  //第三次强化
        upgrade_sections.push(UpgradeSection(40,70));  //第四次强化
        upgrade_sections.push(UpgradeSection(70,100));  //第五次强化          
    }

    function _addItem(
        uint256 _item_config_id,      //物品id
        uint256 _weight,              //权重
        uint256 _prop_sum,            //属性总和(attack + defense + hp )
        ...
        mercenaries[_type_id] = Mercenary (
            _item_config_id,      //物品id 
            _weight,              //权重
            _prop_sum,            //综合值
            0,                    //计数器
            _hash_margin_name     //余量字段的哈希
        );
        return _type_id;
    }
    ...
    function _setPropValues(uint _type_id, string memory _prop_name, uint _min, uint _max) internal {
        ...
        settings[_type_id][keccak256(abi.encodePacked(_prop_name))] = Setting (
            _min,
            _max - _min
        ); 
    }
    ...
    function setPropValues(
        ...
        props[keccak256(abi.encode(_prop_name))] = 0;
        return true;
    }
  ...
  // MANAGING Authorization Contract Address for Call 
   ...
   // 合约部署者增加操作员
    ...
    // 合约部署者设置对应id(1-5)的upgrade_base及upgrade_max
    function setUpgradeSection(uint id, uint256 _min, uint256 _max) external nonReentrant onlyOwner returns(bool){
        require(id<=5);
        upgrade_sections[id].upgrade_base = _min;
        upgrade_sections[id].upgrade_max = _max;
        return true;
    }

    // 合约操作员设置采集器类型权重
    function setWeight(uint256 _type_id, uint256 _weight) external nonReentrant onlyOperator returns(bool){
        mercenaries[_type_id].weight = _weight;
        return true;
    }
    ...
    function geneMetaData(
        ...
        external
        override 
        onlyAuthorizationContract // TODO 上线后需要打开 只允许主合约调用
        returns(uint[] memory)
    {
        //msg.sender是erc1155合约
        require(_id > 9999);

        props[hash_attack]   = 0; //攻击, 总值
        props[hash_defense]  = 0; //防御, 总值
        props[hash_hp]       = 0; //生命, 总值
        uint256 move;          //移速
        uint256 power;         //源力
        uint256 capacity;      //背包最大容量
        uint256 potential;     //潜力
        ...
        for (uint i = 0; i < _typeCount; i++) {
             if (mercenaries[i].count < mercenaries[i].prop_sum) {
                 _count ++;
             }
        }
        if (_count == 0) {
            for (uint k = 0; k < _typeCount; k++) {
                mercenaries[k].count =0;
            }
            _type_id =0;
        ...
        require(
            mercenaries[_type_id].margin_name == hash_attack ||  
            mercenaries[_type_id].margin_name == hash_defense ||  
            mercenaries[_type_id].margin_name == hash_hp, 
            "wrong margin_name"
        );

        Mercenary storage mercenary = mercenaries[_type_id];
        mercenary.count +=1;
        uint new_rand;
        ...
        //不做溢出检测
        unchecked {
            uint margin = mercenaries[_type_id].prop_sum - (props[hash_attack] + props[hash_defense] + props[hash_hp]);
            props[mercenaries[_type_id].margin_name] +=  margin;
        }

       
        if (settings[_type_id][hash_move].section > 0) {
            new_rand = randomResult.sub(randomResult%10000).div(10000);
            move = settings[_type_id][hash_move].base +new_rand % settings[_type_id][hash_move].section; 
        }

        
        if (settings[_type_id][hash_power].section > 0) {
            new_rand = randomResult.sub(randomResult%100000).div(100000);
            power = settings[_type_id][hash_power].base +new_rand % settings[_type_id][hash_power].section;
        }

        
        if (settings[_type_id][hash_capacity].section > 0) {
            new_rand = randomResult.sub(randomResult%1000000).div(1000000);
            capacity = settings[_type_id][hash_capacity].base +new_rand % settings[_type_id][hash_capacity].section;
        }

        
        if (settings[_type_id][hash_potential].section > 0) {
             new_rand = randomResult.sub(randomResult%10000000).div(10000000);
            potential = settings[_type_id][hash_potential].base +new_rand % settings[_type_id][hash_potential].section;
        }

        
        if (settings[_type_id][hash_scan].section > 0 ) {
            new_rand = randomResult.sub(randomResult%100000000).div(100000000);
            scan = settings[_type_id][hash_scan].base +new_rand % settings[_type_id][hash_scan].section;
        }
        ...
        returns(uint[] memory)
    {
        ...
        //取出刚刚合成的属性值
        Character memory ch = revertcharacters[_id];
       
        //更新强化之后的属性值
        Character storage ch_new = characters[_id];

        uint newPercent = upgrade_sections[_potential].upgrade_base + randomResult %  upgrade_sections[_potential].upgrade_max;

        if ( randomResult % 3 == 0) {
           ch_new.attack = ch.attack.add(ch.attack.mul(newPercent).div(100));
        } else if ( randomResult % 3 == 1) {
            ch_new.defense = ch.defense.add(ch.defense.mul(newPercent).div(100));
        } else {
            ch_new.hp =ch.hp.add(ch.hp.mul(newPercent).div(100));
        }
        uint[] memory charactor = new uint[](3);
        charactor[0] =  ch_new.attack;
        charactor[1] =  ch_new.defense;
        charactor[2] =  ch_new.hp;
        ...
        requiresRevert(_to, _id, _orderid, signature)
    {
        ...
        ch.power     = revertcharacters[_id].power;       //源力
        ch.move      = revertcharacters[_id].move;        //移速
        ch.capacity  = revertcharacters[_id].capacity;    //背包最大容量
        ch.potential = revertcharacters[_id].potential;   //潜力
        ch.scan      = revertcharacters[_id].scan;        //扫描
    }
    ...
    function getItemInfo(
        uint256 item_config_id
    )
        public
        view
        returns (Mercenary memory)
    {
        uint  _typeCount = _typeTracker.current();
        Mercenary memory mercenary;
        for (uint i = 0; i < _typeCount; i++) {
             if (mercenaries[i].item_config_id == item_config_id) {
