...
contract ContractOpenablePack is
    Initializable,
    ContractMetadata,
    Ownable,
    ...
    IContractOpenablePack
{
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(ERC1155Receiver, ERC1155Upgradeable, IERC165Upgradeable )
        returns (bool)
    {
        return
            super.supportsInterface(interfaceId) ||
    ...
    function createPack(
        ...
        packTotalSupply = createOrAddBundle(
            _contents,
            _numOfRewardUnits,
            packId,
    ...
    /// @notice Lets a pack owner open packs and generate the rewards to be given out.
    function openPack(uint256 _packId, uint256 _amountToOpen, address recipient) external returns (Token[] memory) {
        ...
        Token[] memory rewardUnits = getRewardUnits(_packId, _amountToOpen, pack.amountDistributedPerOpen, pack, recipient);

        _burn(opener, _packId, _amountToOpen);

        emit PackOpened(_packId, recipient, _amountToOpen, rewardUnits);

        return rewardUnits;
    }

    /// @dev Stores assets within the contract.
    function createOrAddBundle(
        Token[] calldata _contents,
        uint256[] calldata _numOfRewardUnits,
        uint256 packId,
        ...
        if (isUpdate) {
            ...
            _createBundle(_contents, packId);
        }
    }
...
interface IContractOpenablePack is ITokenBundle, IPermissionsEnumerable, IERC1155Upgradeable {
    ...
    function burnPack(uint256 packId) external;
