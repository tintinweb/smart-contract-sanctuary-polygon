    ...
    IPerpetualDepositManager
{
    ...
    function deposit(
        ...
        _transferFromUserToVault(liqPool, traderAddr, _fAmount);
        ...
        onlyMaintainer
    {
        ...
        _transferFromUserToVault(pool, msg.sender, _fAmount);
    ...
    /**
     * To become a broker, deposit to the broker fund.
     * The broker fund is part of protocol owned funds and is used to fill
     * AMM margin cash. The broker fund is "one way", meaning that profit is not
     * paid back to the broker fund but exits via default fund and participation fund.
     * No choice of collateral.
     * @param   _poolId   Reference to liquidity pool
     ...
     *                    (not ABDK)
     */
    function depositBrokerLots(uint8 _poolId, uint32 _iLots) external override whenNotPaused {
        require(_poolId <= iPoolCount, "pool index out of range");
        ...
        pool.fBrokerFundCashCC = pool.fBrokerFundCashCC.add(fAmount);
        _transferFromUserToVault(pool, msg.sender, fAmount);
        emit UpdateBrokerAddedCash(_poolId, _iLots, brokerLots + _iLots);
        emit UpdateBrokerFundCash(_poolId, fAmount, pool.fBrokerFundCashCC);
    }
        ...
        onlyMaintainer
    {
        ...
        _transferFromVaultToUser(pool, treasuryAddress, _fAmount);
        ...
        onlyMaintainer
    {
        ...
        int128 fEarningsDF;
        {
            int128 fTargetDFSize = liquidityPool.fTargetDFSize;
            // account for excess liquidity in PnL pool to possible reduce effective target
            int128 fPnLParticipantCash = _getCollateralTokenAmountForPricing(liquidityPool);
            if (fPnLParticipantCash > liquidityPool.fTargetAMMFundSize) {
                fTargetDFSize = fTargetDFSize.sub(
                    fPnLParticipantCash.sub(liquidityPool.fTargetAMMFundSize)
                );
                fTargetDFSize = fTargetDFSize > 0 ? fTargetDFSize : int128(0);
            }
            fEarningsDF = liquidityPool.fDefaultFundCashCC.sub(fTargetDFSize);
        }
        ...
        if (fEarningsDF > 0) {
            ...
            // transfer tokens
            _transferFromVaultToUser(liquidityPool, treasuryAddress, fEarningsDF);
        }
    ...
    function getFunctionList() external pure virtual override returns (bytes4[] memory, bytes32) {
        ...
        functionList[4] = this.depositBrokerLots.selector;
