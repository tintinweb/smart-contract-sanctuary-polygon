...
interface esketitNFT {
    function pricePerItem() external view returns (uint256);
}

contract EsketitMarketplace is Ownable, ReentrancyGuard {
    ...
    function buyFromEsketit(
        ...
        uint256 _pricePerItem = esketitNFT(_nftAddress).pricePerItem();
        IERC20(_payToken).safeTransferFrom(
            _msgSender(),
            esketitTreasury,
            _pricePerItem.mul(_tokenId.length)
        );
    ...
    function listItem(
        ...
        require(esketitLoanBasket[_msgSender()], "unknown nft address");
        require(payTokens[_payToken], "not verified payToken");
    ...
    function updateEsketitLoanBasket(address _esketitLoanBasket) external onlyOwner {
