  ...
  ERC2771Recipient
{
  ...
  // 0xcb8A8D1862d2C3c75Ba1151851afB2651c05b6be
  ...
  struct AlterKeyDetails {
    uint256 id;
    address owner;
  }

  mapping(uint256 => AlterKeyDetails) NFTLogs;
  ...
  modifier mintCompliance(address _address) {
    require(!pauseMinter, "The contract is paused!");
    require(claimedWallets[_address] == false, "User already claimed NFT");
    require(tokensInCirculation < maxSupply, "Max supply exceeded!");
    _;
  }
  ...
  // Mint Methods
  function mintPass() public virtual mintCompliance(_msgSender()) {
    _mint(_msgSender(), nextTokenId, 1, "");

    AlterKeyDetails memory newNft = AlterKeyDetails({
      id: nextTokenId,
      owner: _msgSender()
    });
    NFTLogs[nextTokenId] = newNft;
    claimedWallets[_msgSender()] = true;
    emit Mint(nextTokenId, _msgSender(), newNft, uri(nextTokenId));
    nextTokenId++;
    tokensInCirculation++;
  }

  function batchMint(uint256[] memory ids, uint256[] memory amounts)
    public
    virtual
  {
    require(
    ...
    require(!pauseMinter, "The contract is paused!");
    require(
      tokensInCirculation + ids.length <= maxSupply,
      "Max supply exceeded!"
    );
    ...
    for (uint256 i = 0; i < ids.length; i++) {
      ...
      claimedWallets[_msgSender()] = true;
      emit MintBatch(ids[i], _msgSender(), newNft, uri(nextTokenId));
      nextTokenId++;
      tokensInCirculation++;
    }
  }
  ...
  function uri(uint256 _id) public view override returns (string memory) {
    require(_id <= nextTokenId, "Non Existent Token");
    return (
      bytes(baseURI).length > 0
        ? string(abi.encodePacked(baseURI, _id.toString(), uriExtension))
        : ""
    );
  }
  ...
  function setUriExtension(string memory _extension) public virtual {
    require(
      hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
      "ERC1155PresetMinterPauser: must have minter role to mint"
    );
  ...
  function setMaxSupply(uint256 supply) public virtual {
    require(
      hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
      "AlterKey: Must have admin role to change"
    );
    maxSupply = supply;
  }

  function setCreator(address _creator) public virtual {
    creator = _creator;
  }

  function isMaxSupplyReached() public view returns (bool) {
    if (tokensInCirculation >= maxSupply) return true;
    return false;
  }
  ...
  // func to get tokens in circulation
  function checkWalletEligibility(address _address) public view returns (bool) {
    return !claimedWallets[_address];
  }

  function getNFTDetails(uint256 id)
    external
    view
    returns (AlterKeyDetails memory)
  {
    return NFTLogs[id];
  }
