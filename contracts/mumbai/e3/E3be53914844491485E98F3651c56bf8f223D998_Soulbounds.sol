...
contract Soulbounds is ERC1155, ERC1155Burnable, Ownable, ERC5633 {
    constructor() ERC1155("") ERC5633() {
        _setSoulbound(0, true);
        _setSoulbound(1, true);
        _setTokenUri(
            0,
            "https://gateway.pinata.cloud/ipfs/QmVjcNEpLHG5eEZTMPeb9hYb2FgjNBrPqAJnzpg4dovT89"
        );
        _setTokenUri(
            1,
            "https://gateway.pinata.cloud/ipfs/QmcDyWRTXm3m6Aed51ZRTjpvsecpJ4doVQL6Aj3dAKVVoQ"
        );
    }

    event BasicTokenMinted(address indexed to, uint id, uint amount);
    event TokensUpgraded(address indexed to, uint id, uint amount);

    mapping(uint256 => string) public _tokenURIs;

    uint public totalBasicMinted;
    uint public totalSuperMinted;

    function _setTokenUri(uint256 tokenId, string memory tokenURI) public onlyOwner {
        _tokenURIs[tokenId] = tokenURI;
    }

    function mintBasicToken(address to, uint amount) public onlyOwner {
        uint id = 0;
        bytes memory data = "";
        _mint(to, id, amount, data);
        totalBasicMinted++;
        emit BasicTokenMinted(to, id, amount);
        if (balanceOf(to, 0) >= 10) {
            burn(to, 0, 10);
            _mint(to, 1, 1, "");
            totalSuperMinted++;
            emit TokensUpgraded(to, 1, 1);
        }
    }

    function burnSuperToken() public {
        burn(msg.sender, 1, 1);
    }

    function totalBasic() public view returns (uint) {
        return totalBasicMinted;
    }

    function totalSuper() public view returns (uint) {
        return totalSuperMinted;
    }
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view override(ERC1155, ERC5633) returns (bool) {
    ...
    function _beforeTokenTransfer(
        ...
        uint256[] memory amounts,
        bytes memory data
    ) internal override(ERC1155, ERC5633) {
        super._beforeTokenTransfer(operator, from, to, ids, amounts, data);
    }

    function burn(address account, uint256 id, uint256 value) public override {
        require(
            account == _msgSender() || _msgSender() == owner(),
            "ERC1155: caller is not token owner or contract owner"
        );
