...
/**
   ...
   @dev This contract is being used as Governance of Fifa
...
contract Governance is AccessControlEnumerable {
    ...
    constructor(address _treasury, address[] memory _tokens) {
        ...
        for (uint256 i; i < _size; i++) paymentTokens[_tokens[i]] = true;
    }

    /**
       @notice Set `pausedTransfer = true`
       @dev  Caller must have DEFAULT_ADMIN_ROLE
    */
    function pause() external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(!pausedTransfer, "Paused");
        pausedTransfer = true;
    }

    /**
       @notice Set `pausedTransfer = false`
       @dev  Caller must have DEFAULT_ADMIN_ROLE
    */
    function unpause() external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(pausedTransfer, "Unpause");
