...
/// @title      Rebel Organization Contract

import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/token/ERC721/extensions/ERC721URIStorage.sol";
import "@openzeppelin/contracts/utils/Counters.sol";
import {RebelLibrary} from "./RebelLibrary.sol";

contract RebelCertificate is ERC721URIStorage, Ownable {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds; // Maximum number of token that can be issued is 65534.
    Counters.Counter private _acknowledgementIds; // Maximum number of acknowledgements is 254.

    /// @dev K: Id of the Acknowledgement formed using counter      V: Details of Acknowledgement stored using struct
    mapping(uint8 => RebelLibrary.Acknowledgement) private _acknowledgementById;

    /// @dev K: Volunteers Wallet address       V: Details of Volunteer stored using struct
    mapping(address => RebelLibrary.VolCert) private _certsByVolWallet;

    // Only runs when contract is deployed
    constructor() ERC721("Rebel Club Certificate", "RebelCert") {}

    /// @dev This function updates and inserts Acknowledgement details i.e., stored on-chain
    /// @param id Id of the acknowledgement to update or insert 
    /// @param title Title of acknowledgement
    /// @param uom Unit of measurement by which acknowledgement duration is measured
    function upsertAcknowledgement(uint8 id, string memory title, string memory uom) public onlyOwner{
        require(_acknowledgementIds.current() != 0, 'Specified AcknowledgementId does not exist!');
        require(!(id > _acknowledgementIds.current()), 'Specified AcknowledgementId does not exist!');
        RebelLibrary.Acknowledgement storage storedAcknowledgement = _acknowledgementById[id];
        storedAcknowledgement._title = title;
        storedAcknowledgement._uom = uom;
    }

    /// @dev Inserts new acknowledgement on-chain
    /// @param title Takes acknowledgement Title
    /// @param uom Takes acknowledgement Unit Of Measurement(UOM)
    function insertAcknowledgement(string memory title, string memory uom) public onlyOwner{
        require(_tokenIds.current() <= 254, 'Highest number of acknowledgements reached!');
        _acknowledgementIds.increment();
        upsertAcknowledgement(uint8(_acknowledgementIds.current()), title, uom);
    }

    /// @dev Deletes a particular acknowledgement
    /// @param id Takes Id of the acknowledgement to be deleted
    function deleteAcknowledgement(uint8 id) public onlyOwner{
        require(_acknowledgementIds.current() != 0, 'Specified AcknowledgementId does not exist!');
        require(!(id > _acknowledgementIds.current()), 'Specified AcknowledgementId does not exist!');
        for(uint8 i = id + 1; i <= _acknowledgementIds.current(); i++){
            _acknowledgementById[i - 1]._title = _acknowledgementById[i]._title;
            _acknowledgementById[i - 1]._uom = _acknowledgementById[i]._uom;
        }
        delete _acknowledgementById[uint8(_acknowledgementIds.current())];
        _acknowledgementIds.decrement();
    }

    /// @dev To view all the acknowledgements stored on-chain 
    function viewAcknowledgements() public view onlyOwner returns (string memory){
        bytes memory acknowledgementsToShow = 'Stored Acknowledgements: ';
        for (uint8 i = 1; i <= _acknowledgementIds.current(); i++) {
            acknowledgementsToShow = abi.encodePacked(acknowledgementsToShow,
                '   Id:                 ', i,
                '   Name:               ', _acknowledgementById[i]._title,
                '   Unit of Measurement:', _acknowledgementById[i]._uom
            );
        }
        return string(acknowledgementsToShow);
    }

    /// @dev Function to Mint NFT
    /// @param volWallet Volunteer Wallet address who gets the minted NFT 
    function mint(address volWallet) internal {
        require(_tokenIds.current() <= 65534, 'Highest number of tokens reached!');
        _tokenIds.increment();
        uint16 newItemId = uint16(_tokenIds.current());
        _safeMint(volWallet, uint256(newItemId));
        _setTokenURI(uint256(newItemId), RebelLibrary.getTokenURI(newItemId, _certsByVolWallet[volWallet], _acknowledgementById));
    }

    /**
     * @dev Payable function to mint the Volunteer's NFT and store volunteer details on chain
     * @param role role of the Volunteer
     * @param name name of the Volunteer
     * @param addressedBy how you wanted to be addressed eg, his/her
     * @param issuedOn when the certificate is issued
     * @param walletAddress wallet address of the certificate owner to be
     * @param lastUpdatedOn when the report is last updated
     * @param volAcknowledgementIds list of ids of strunts performed
     * @param servedDurations duration in uom volunteer served corresponding to acknowledgementIds
     */
    function mintNFTForVolunteer(
        string memory role,
        string memory name,
        ...
        RebelLibrary.emitAndCheckVolDetails(
            role,
            name,
            addressedBy,
            issuedOn,
        ...
        RebelLibrary.VolCert storage volDetail = _certsByVolWallet[walletAddress];
        require((volDetail._isPopulated != true), 'Volunteer already exists!');
        volDetail._name = name;
        volDetail._role = role;
        volDetail._addressedBy = addressedBy;
        volDetail._issuedOn = issuedOn;
        volDetail._walletAddress = walletAddress;
        volDetail._lastUpdatedOn = lastUpdatedOn;
        volDetail._acknowledgementIds = volAcknowledgementIds;
        volDetail._servedDurations = servedDurations;
        volDetail._isPopulated = true;
        volDetail._volTokenId = uint16(_tokenIds.current() + 1);
    ...
    function updateVolunteerAcknowledgements(
        address walletAddress,
        uint8[] memory volAcknowledgementIds,
        uint8[] memory servedDurations
    ) public onlyOwner{
        RebelLibrary.VolCert storage volDetail = _certsByVolWallet[walletAddress];
        require((volDetail._isPopulated == true), 'Volunteer not created!');
        volDetail._acknowledgementIds = volAcknowledgementIds;
        volDetail._servedDurations = servedDurations;

        _setTokenURI(uint256(_tokenIds.current() + 1), RebelLibrary.getTokenURI(volDetail._volTokenId, volDetail, _acknowledgementById));        
    }
...
library RebelLibrary {
    ...
    function emitAndCheckVolDetails(
        string memory role,
        string memory name,
        string memory addressedBy,
        string memory issuedOn,
        ...
        require(volAcknowledgementIds.length == servedDurations.length, 'Mismatching arrar lengths of Volunteer Acknowledgement Ids corresponding to served durations!');
        require(volAcknowledgementIds.length <= 3, 'At max 3 acknowledgements can be given to a particular Volunteer!');
        
        require((bytes(name).length != 0), 'Volunteer should have a name!');
        require((bytes(role).length != 0), 'Volunteer should have a role!');
        require((bytes(addressedBy).length != 0), 'Addressed By field cannot be blank!');
        require((bytes(issuedOn).length != 0), 'Volunteer should have a issuedOn date!');
        require((walletAddress != 0x0000000000000000000000000000000000000000), 'Volunteer should have a wallet address!');
        require((lastUpdatedOn != 0), 'Last Updated On field cannot be empty!');
        require(volAcknowledgementIds.length > 0, 'Volunteer should have at least one acknowledgement!');
        require(volAcknowledgementIds.length > 0, 'Volunteer should have corresponding durations');
