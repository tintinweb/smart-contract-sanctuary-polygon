...
contract Vault10x is Ownable {
    ...
    AggregatorV2V3Interface public priceOracle;
    ...
    constructor(
        ...
        treasury = _treasury;
        isa = IERC20(_isa);
        price[eTY] = _initialPrice;
        price[eITY] = _initialPrice;
        isaOracle = PRICEORACLE(_isaOracle);
        priceOracle = AggregatorV2V3Interface(_priceOracle);
    }
    ...
    function tokenBuy(address token, uint256 amount) public virtual isActive {
        ...
        uint256 tokensToMint = isaPerUSD.mul(amount);
        require(tokensToMint >= MIN_BUY, "NOT-ENOUGH-ISA");
        require(isatkn.balanceOf(msg.sender) >= amount, "NOT-ENOUGH-BALANCE");
        require(
            isatkn.allowance(msg.sender, address(this)) >= amount,
    ...
    function getLatestPrices() public view returns (uint256, uint256) {
        uint256 price0 = isaOracle.price();
        int256 price1 = priceOracle.latestAnswer();
        return (price0, uint256(price1));
    }
    ...
    function updatePrice() public {
        ...
        if (changeInPrice < 20) {
            if (isIncreased) {
                price[eTY] = lastPrice
                    .mul(leverage.mul(changeInPrice).add(denominator))
                    .div(denominator);
                price[eITY] = lastPrice
                    .mul(denominator.sub(leverage.mul(changeInPrice)))
                    .div(denominator);
            }
            if (!isIncreased) {
                price[eTY] = lastPrice
            ...
            price[eTY] = lastPrice.mul(denominator.add(80)).div(denominator);
            price[eITY] = lastPrice.mul(denominator.sub(20)).div(denominator);
        }
    }
