...
contract GigawattFactory is Ownable {
  address public tranchePoolImplementation;

  event NewTranchePool(uint256 RMWNftId, address poolAddress);

  mapping(uint256 => address) public poolAddress;

  constructor(address _tranchePoolImplementation) {
    // check if the implementation contract has an interface of initializer and if its a contract
    tranchePoolImplementation = _tranchePoolImplementation;
  }

  // make only owner
  function updateTranchePoolImplementation(address _tranchePoolImplementation) external onlyOwner {
    tranchePoolImplementation = _tranchePoolImplementation;
  }

  function createNewTranchePool(uint256 _rmwNFTId) external returns (address) {
    require(poolAddress[_rmwNFTId] == address(0), 'Pool already created');
    poolAddress[_rmwNFTId] = address(new ERC1967Proxy(tranchePoolImplementation, ''));

    emit NewTranchePool(_rmwNFTId, poolAddress[_rmwNFTId]);

    return poolAddress[_rmwNFTId];
  }

  function getPoolAddress(uint256 id) external view returns (address) {
    return poolAddress[id];
  }
}
