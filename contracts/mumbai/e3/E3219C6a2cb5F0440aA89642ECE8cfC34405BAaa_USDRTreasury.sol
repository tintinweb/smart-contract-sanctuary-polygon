...
contract USDRTreasury is AddressAccessor, ITreasury, IERC721Receiver {
    ...
    function burnTNGBL() external onlyRole(CONTROLLER_ROLE) {
        ...
        (uint256 stable, uint256 rwa, ) = _getTreasuryValue();
    ...
    function _verifyBacking() internal view {
        address staking = addressProvider.getAddress(STAKING_ADDRESS);

        (uint256 stable, uint256 rwa, uint256 tngbl) = _getTreasuryValue();
        uint256 scaledMarketCap = IStaking(staking).usdrMarketCap() * 1e9;
        require(
            scaledMarketCap <= stable + rwa + tngbl,
