...
contract MyContract is ERC1155LazyMint {
    ...
    uint256 public mintRate = 1 * 10 ** 18;

    bool public isGamePaused = false;

    uint256 constant BATTLE_FEE_PERCENTAGE = 10; // 10% fee for the contract owner
    ...
    struct Battle {
        ...
        uint256 startTime; // Start time of the battle
        uint256 endTime; // End time of the battle
        bool finished; // Indicates if the battle has finished
        uint256 winner; // The index of the winner player in the players array
    }
    ...
    struct Player {
        ...
        uint256 playerStamina; /// @param playerStamina player stamina
        uint256 lastStaminaRefillTime; /// @param lastStaminaRefillTime the timestamp of the last time the player's stamina was refilled
    ...
    function registerPlayer(string memory _name, uint256 amount) external {
        require(!isPlayer(msg.sender), "Player already registered"); // Require that player is not already registered

        uint256 fee = mintRate * amount;
        require(paymentToken.transferFrom(msg.sender, address(this), fee), "");

        uint256 _id = players.length;
        players.push(
            Player(msg.sender, _name, 10, 25, 5, block.timestamp, false)
    ...
    function claimHero(uint256 _tokenId, uint256 _quantity) external {
        require(!isPlayer(msg.sender), "Player already registered"); // Require that player is not already registered
        claim(msg.sender, _tokenId, _quantity);
    }
    ...
    function verifyClaim(
        ...
        uint256 payedBalance = userBalance - mintRate;

        require(!isPlayer(msg.sender), "Player already registered"); // Require that player is not already registered

        require(
            paymentToken.balanceOf(msg.sender) >= mintRate,
            "you have not enought token balance for Mint Hero!"
        );

        require(
            paymentToken.allowance(msg.sender, address(this)) >= mintRate,
            "Insufficient allowance for Mint Hero!"
        );

        require(
            paymentToken.balanceOf(msg.sender) >= payedBalance,
            "You haven't paid the required amount!"
        );
        ...
        require(
            ...
            "You have a Hero, you cant claim. If you want to more Heroes you must buy on NftMarket."
        );

        for (uint256 i = 0; i < 8; i++) {
            require(
                balanceOf[_claimer][i] == 0,
                "You have a Hero, you cant claim. If you want to more Heroes you must buy on NftMarket."
            );
        }
    }
    ...
    function getBattle() public view returns (address[] memory) {}

    function joinBattle(uint256 _betAmount) public {}

    function battleMove(uint8 move) public {}

    function isWinner(address winner) public view returns (bool) {}

    function endBattle() internal {}

    function claimBetAmount() public {}
