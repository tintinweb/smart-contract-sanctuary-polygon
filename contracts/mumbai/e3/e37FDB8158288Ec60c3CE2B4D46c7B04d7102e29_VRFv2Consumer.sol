...
contract VRFv2Consumer is VRFConsumerBaseV2 {
    ...
    function shuffle(uint size, uint entropy) private pure returns (uint[] memory) {
    ...
    function fulfillRandomWords(
        ...
        emit Shuffled(shuffle(airdropCampaigns[contestIndexFromRequestId].contestants_addresses.length - 1, randomWord));
        _updateWinners(contestIndexFromRequestId);
    }
    ...
    function drawContest(uint contestIndex) external onlyOwner {
        ...
        // FROM --> s_requestId = COORDINATOR.requestRandomWords(
        ...
        s_requestIdToContestIndex[requestId] = contestIndex;
    }

    function _updateWinners(uint contestIndex) private {
        // This is still really redundant storage in my opinion. How do winners get paid out? 
        // Now that the contestants are shuffled just take the first numberOfWinners from the contestants_addresses array offchain
        for (uint i = 0; i < airdropCampaigns[contestIndex].numberOfWinners; i++) {
            airdropCampaigns[contestIndex].winners.push(airdropCampaigns[contestIndex].contestants_addresses[i]);
        }
        //airdropCampaigns[contestIndex].winners = airdropCampaigns[contestIndex].contestants_addresses[:airdropCampaigns[contestIndex].numberOfWinners];
      }
    ...
    function getWinners(uint contestIndex) external view returns(address[] memory) {
        ...
        for (uint i = 0; i < airdropCampaigns[contestIndex].numberOfWinners; i++) {
            tempWinners[i] = airdropCampaigns[contestIndex].contestants_addresses[i];
        }
        return tempWinners;
    }
}