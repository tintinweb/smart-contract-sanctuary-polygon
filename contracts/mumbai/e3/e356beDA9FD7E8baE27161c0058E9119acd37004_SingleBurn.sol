...
contract SingleBurn is IExecutePlugin, BasePluginWithRules {

    constructor(address _registry) {
        PLUGIN_VERSION = 1;
        PLUGIN_NAME = PluginsList.SINGLE_BURN_COMMENT;
    ...
    function execute(
        ...
        ICommentData commentData = ICommentData(registry.commentData());

        DataTypes.MinSimpleVars memory readCommentVars;
        readCommentVars.pluginName = PLUGIN_NAME;
        readCommentVars.version = PLUGIN_VERSION;
        readCommentVars.data = _data;
        DataTypes.CommentInfo memory comment = commentData.readComment(readCommentVars);
        require(_sender == comment.owner, "Burn: wrong comment owner");
        ...
        require(commentData.burnComment(vars),"Burn: wrong burning");
