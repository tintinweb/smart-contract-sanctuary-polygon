...
import "@wraith-works/contracts/common/Errors.sol";
import "@wraith-works/contracts/tokens/ERC721/AirdropERC721.sol";

contract OwlphaCore is AirdropERC721 {
    using Strings for uint256;
    ...
    modifier onlyAuthorizedBurner {
        if (msg.sender != authorizedBurner) revert Errors.Unauthorized(msg.sender);
        _;
    }

    error TokenIdCoreTypeLocked();

    address public authorizedBurner;
    bool public tokenIdCoreTypeIsLocked = false;
    mapping(uint256 => CoreType) private _tokenIdToCoreType;
    ...
    constructor()
        BaseERC721(
            ...
            ".json",
            7777,
            0xac991D1484735c13ECC8c1F7A23A81441750Cfb4,
            650
        )
        AutoIncrementERC721(0)
    {}

    function bulkSetTokenIdsCoreType(uint256[] calldata _tokenIds, CoreType _coreType) external onlyOwner {
        if (tokenIdCoreTypeIsLocked) revert TokenIdCoreTypeLocked();

        uint256 length = _tokenIds.length;
        for (uint256 i = 0; i < length; ) {
            _tokenIdToCoreType[_tokenIds[i]] = _coreType;
    ...
    function lockTokenIdCoreTypeSetting() external onlyOwner {
        tokenIdCoreTypeIsLocked = true;
    }

    function setAuthorizedBurner(address _authorizedBurner) external onlyOwner {
        if (_authorizedBurner == address(0)) revert Errors.ZeroAddress();
        authorizedBurner = _authorizedBurner;
    }

    function authorizedBurn(uint256 _tokenId) external onlyAuthorizedBurner {
        if (!_exists(_tokenId)) revert DoesNotExist(_tokenId);
        _burn(_tokenId);
    }

    function getTokenIdCoreType(uint256 _tokenId) public view returns (CoreType) {
        return _tokenIdToCoreType[_tokenId];
    }

    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        if (!_exists(_tokenId)) revert DoesNotExist(_tokenId);
        uint256 coreType = uint256(_tokenIdToCoreType[_tokenId]);
        return
            bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, coreType.toString(), baseURIExtension)) : "";
    }
}