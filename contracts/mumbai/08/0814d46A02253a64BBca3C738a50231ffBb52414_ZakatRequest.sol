...
contract ZakatRequest is BasicAccessControl, ZakatBasic {
    ...
    mapping(address => ZakatApproval) requests;
    ...
    function setRequest(bool _doner) external {
        require(
        ...
        zktApprove.requestID = requestCount;
    ...
    function approve(
        ...
        requestCount++;
        ZakatApproval memory zktApprove = requests[_person];
        require(
            zktApprove.requestID > 0,
            "Request does not exists for this user."
        );
        delete requests[_person];
        emit Approve(_person, zktApprove.doner, _approve, zktApprove.requestID);
    }
}