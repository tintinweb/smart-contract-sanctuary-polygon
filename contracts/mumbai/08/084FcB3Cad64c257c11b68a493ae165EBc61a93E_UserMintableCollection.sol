    ...
    Whitelist
{
    ...
    constructor(
        ...
        require(trustedForwarder != address(0),"address must be valid");
        forwarder = trustedForwarder;
    }


    modifier onlyValidAddress(){
        require(msg.sender == forwarder || _msgSender() == owner() ,"only owner can call this function");
        _;
    }
     ...
     *
     */
    function mint(address _to, string memory _tokenURI,uint96 _feeNumerator)
        external
        onlyValidAddress()
    {
        require(_to != address(0), "Collection: _to address not valid");
        require(bytes(_tokenURI).length > 0,"Token URI is not valid");
        counter++;
