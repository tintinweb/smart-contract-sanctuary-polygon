...
contract LostChainIAP is CustomERC2771ContextUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    // Lost Chain RBA Linker
    ILostChainRBA private LCRBALinker;
    modifier onlyRole(string memory name, address account) {
        require(LCRBALinker.checkAccountRole(name, account), 'LCIAP: Unauthorized');
        _;
    }

    function setRBALinker(address rbaLinkerAddress) public onlyRole('DEFAULT_ADMIN_ROLE', _msgSender()) {
        require(rbaLinkerAddress != address(0x0), 'LCIAP: RBA linker is zero address');
    ...
    function initialize(
        ...
        require(address(forwarder) != address(0x0), 'LCIAP: Forwarder is zero address');
        require(rbaLinkerAddress != address(0x0), 'LCIAP: RBA linker is zero address');
        require(nftAddress != address(0x0), 'LCIAP: NFT is zero address');
        LCRBALinker = ILostChainRBA(rbaLinkerAddress);
    ...
    function setLC(address lostCrystalAddress) public onlyRole('DEFAULT_ADMIN_ROLE', msg.sender) {
        require(lostCrystalAddress != address(0x0), 'LCIAP: Lost crystal is zero address');
        LC = ILostCrystal(lostCrystalAddress);
    }
