...
contract FactoryNFT {
    ...
    event NFTTokenCreated(address owner, address tokenContract,uint tokenId); //emitted when ERC1155 token is deployed
    event NFTMinted(address owner, address tokenContract,uint tokenID); //emmited when ERC1155 token is minted

    function deployNFT(string memory _contractName, string memory _symbol ,string memory _uri) public returns (address) {
    ...
    function createNFTToken(uint _index,string memory _name, bool _limited, uint128 _limit,bool _allowed, bool _allowedList) public {
        tokens[_index].createToken(_name,_limited,_limit,_allowed,_allowedList);
        emit NFTTokenCreated(tokens[_index].owner(), address(tokens[_index]),_index);
    }


    function mintNFT(uint _index,address to) public {

        // mint function
        tokens[_index].mintTokenItem(to);
        emit NFTMinted(tokens[_index].owner(), address(tokens[_index]),_index);
    }
   /*
    ...
    */
    function getCountERC1155byIndex(uint256 _index) public view returns (uint amount) {
        return tokens[_index].balanceOf(tokens[_index].owner(), _index);
...
contract NFT is ERC1155, Ownable {
    ...
    function createToken(string memory _name, bool _limited, uint128 _limit,bool _allowed, bool _allowedList) external view onlyOwner {
        require(
       ...
       token.id = tokenId.current();
       token.name = _name;
       token.limited = _limited;
       token.tokenLimit = _limit;
       token.tokenMinted= 0;
       token.isAllowed = _allowed;
       token.isAllowedList = _allowedList;
    }

    // set token Allowed - owner
    function setTokenAllowed(bool _allowed) public  onlyOwner {
        tokenIdToToken[tokenId.current()].isAllowed = _allowed;
    }

    // Set token allowed list - owner
    function setTokenAllowedList(bool _allowedList) public  onlyOwner {
        tokenIdToToken[tokenId.current()].isAllowedList = _allowedList;
    }
    ...
    function batchMintTokenItem(address[] memory to) public {
...
/*
    function getAllowedList(uint256 _tokenId) public returns (mapping(uint=>mapping(address => bool))  memory ) {
        return allowedList;
    } */
}
