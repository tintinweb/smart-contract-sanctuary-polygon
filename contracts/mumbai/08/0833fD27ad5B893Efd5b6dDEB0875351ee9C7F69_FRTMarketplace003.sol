...
contract FRTMarketplace003 is AccessControl{
    ...
    function _approveNFT(uint256 _tokenId) private {
        ...
        NFTOwners[publisher].push(_tokenId);
    ...
    /**  
     * @dev Let user download an NFT with pending status from {NFTlist} for testing.
     *
     * Requirements:
     * 
     * - `_tokenId` must be in {NFTlist} and status should be pending
     ...
     *
     */
    function downloadNFTForTesting(uint256 _tokenId) external{
        ...
        if(testingAllowed[_tokenId][msg.sender] == 0){
    ...
    function giveReward(address _publisher, uint256 _tokenId) internal {
        ...
        uint256 _year = ((block.timestamp - contractCreatedAt) / 1 hours) < 30 ? ((block.timestamp - contractCreatedAt) / 1 hours)+1 : 30 ; // get ongoing year since contract deployed
    ...
    function getCurrentRewardYear() external view returns(uint256 _year) {
        _year = ((block.timestamp - contractCreatedAt) / 1 hours)+1; // get ongoing year since contract deployed
    }
    /******TO BE REMOVED CODE******/
    function getNftIdsByAddress(address _NFTOwner) external view returns( uint256[] memory _nfts){
        return NFTOwners[_NFTOwner];
    }
    function getNftsByAddress(address _NFTOwner, uint256 _page) external view returns( NFTDetail[] memory _nfts){
        uint perPage = 10;
