...
contract FootballBets is Ownable {
    ...
    uint16 public feePercent = 20; // 20/1000 * 100 = 2%
    ...
    constructor(VEGAERC20Token _token, address _addressMarketing) {
    ...
    function changeAddressMarketing(address _addressMarketing) external onlyOwner {
        addressMarketing = _addressMarketing;
    }

    function changeFeePercent(uint16 _fee) external onlyOwner{
        require(_fee >= 0, 'fee must > 0');
        feePercent = _fee;
    }
    ...
    function buyTicket(uint256 _matchId, uint8 _betTypeId, uint8 _betDoor, uint32 _betOdd, uint256 _betAmount) public checkBlackList  returns (uint256 _ticketIndex) {
        uint256 _amountFeeMarketing = _betAmount.mul(feePercent).div(10 ** 3);
        uint256 _actualBetAmount = _betAmount.sub(_amountFeeMarketing).sub((_betAmount - _amountFeeMarketing).mul(tokenAddress.getBuyTicketTax()).div(10 ** 3)); // _betAmount - _amountFeeMarketing - _communityHolder
        ...
        // transfer token
        tokenAddress.transferFrom(msg.sender, addressMarketing, _amountFeeMarketing);
        tokenAddress._buy(msg.sender, _betAmount - _amountFeeMarketing);
