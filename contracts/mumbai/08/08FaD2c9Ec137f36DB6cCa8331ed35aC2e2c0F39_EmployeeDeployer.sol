...
contract EmployeeDeployer is Initializable, AccessControl {
    ...
    uint8 public playToEarnFee = 89;
    uint8 public employeePoolFee = 1;
    ...
    function changeFees(
        ...
        require(
            (_employeeStaking + _playToEarn + _creator + _employeePool) == 100,
            INVALID_FEES
        );

        creatorFee = _creator;
        playToEarnFee = _playToEarn;
        employeePoolFee = _employeePool;
        nftStakeFee = _employeeStaking;
    }

    function setDeployer(BaseDeployer _deployer) public onlyRole(MAIN_OWNER) {
    ...
    function payMint(address customer) private returns (bool) {
        if (whiteList[customer] > 0) whiteList[customer]--;
            ...
            if (employeePoolFee > 0) {
                token.transferFrom(
                    customer,
                    employeePool,
                    (employeePrice * employeePoolFee) / 100
                );
            }
        }
