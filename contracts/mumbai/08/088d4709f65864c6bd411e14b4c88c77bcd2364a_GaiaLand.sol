...
contract GaiaLand is ERC721, Ownable {
  ...
  uint256 public gaiaLandReserve = 2000;
  ...
  function reserveGaiaLands(
    ...
    require(totalSupply() == (toSupply + _reserveAmount), 'Sorry, you must mint again later');
  }
  ...
  function getTokenPrice1(address pairAddress, uint amount) public view returns(uint) {
  ...
  function getTokenPrice0(address pairAddress, uint amount) public view returns(uint) {
    IUniswapV2Pair pair = IUniswapV2Pair(pairAddress);
  ...
  function mintGaiaLandToken(
    ...
    require(saleIsActive, 'Sale must be active to mint GaiaLand');
    require(totalSupply() == toSupply, 'Sorry, you must mint again later');
    require(_price > 0, 'Price is equal to zero');
    require(
      numberOfTokens > 0 && numberOfTokens <= MAX_GAIALANDS_PURCHASE,
      'Can only mint 200 tokens at a time'
    );
    ...
    require(
      _price.mul(tokenRate) >=
        (gaiaGoodLandPrice.mul(numberOfGoodLands) +
          gaiaRegularLandPrice.mul(numberOfRegularLands)).mul(1e24),
      'Gaia value sent is not correct'
    );
    ...
    require(
      IERC20(WMATICAddress).transferFrom(msg.sender, address(this), _price)
    );
  }
