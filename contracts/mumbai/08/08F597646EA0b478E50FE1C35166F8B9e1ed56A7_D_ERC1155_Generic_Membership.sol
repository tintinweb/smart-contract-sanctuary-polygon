...
/**
 * @title D_ERC1155_Generic_Membership contract.
 ...
 * @dev `D` in the `D_ERC1155_Generic_Membership` contract means that token Metadata can be updated dynamically.
 * `Membership` in the `D_ERC1155_Generic_Membership` contract means that tokens are not transferable.
 */
contract D_ERC1155_Generic_Membership is ERC1155, Ownable, AccessControl {
    struct Token {
        uint256 level;
        string[] uris;
    }
    ...
    constructor(
        ...
        require(bytes(name_).length != 0, "D_ERC1155_Generic_Membership: Incorrect name");
        require(bytes(symbol_).length != 0, "D_ERC1155_Generic_Membership: Incorrect symbol");
        require(bytes(contractUri_).length != 0, "D_ERC1155_Generic_Membership: Incorrect contract Uri");
     ...
     */
    function setToken(string memory backendTokenKey_, string[] memory uris_) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(uris_.length > 0 && bytes(uris_[0]).length > 0, "D_ERC1155_Generic_Membership: Uris cannot be empty");
        require(_backendTokenKeyToId[backendTokenKey_] == 0, "D_ERC1155_Generic_Membership: Mint need to be unique");
    ...
    function changeToken(string memory backendTokenKey_, uint256 level_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        uint256 tokenId = getTokenKeyId(backendTokenKey_);
        require(
            level_ < (_tokens[tokenId].uris).length,
            "D_ERC1155_Generic_Membership: Update not available for this token"
        );
    ...
    /**
     * @dev Function overrides `safeTransferFrom` from {ERC1155} parent contract and makes transfering
     * of tokens batches not available. See {IERC1155-safeTransferFrom} .
     */
    function safeTransferFrom(
        address from,
        address to,
        ...
        revert("D_ERC1155_Generic_Membership: Tokens not transferable");
    }

    /**
     * @dev Function overrides `safeBatchTransferFrom` from {ERC1155} parent contract and makes transfering
     * of tokens not available. See {IERC1155-safeBatchTransferFrom} .
     */
    function safeBatchTransferFrom(
        address from,
        address to,
    ...
    function uri(uint256 tokenId_) public view override returns (string memory) {
        require((_tokens[tokenId_].uris).length > 0, "D_ERC1155_Generic_Membership: URI doesnt exist");
    ...
    function getTokenKeyId(string memory backendTokenKey) public view returns (uint256) {
        require(
            _backendTokenKeyToId[backendTokenKey] > 0,
            "D_ERC1155_Generic_Membership: No token exist with this name"
        );
