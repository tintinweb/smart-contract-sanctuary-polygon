...
import {EIP712Base, EIP712MetaTransaction} from "./EIP712MetaTransaction.sol";
...
contract CreatorV3 is EIP712MetaTransaction, ERC721Upgradeable, IERC2981Upgradeable, OwnableUpgradeable {
    // ================================
    // TYPES
    // ================================
    ...
    struct Edition {
        ...
        // The price at which each token will be sold, in WETH, fee not included.
        uint256 price;
    ...
    // Flag to show revealed or unrevealed metadata
    mapping(uint256 => bool) public tokenURIRevealed;
    // Mapping to prevent from buyEdition re-use
    mapping(address => uint256) buyNonces;
    // The purchase typehash (used for checking signature validity)
    bytes32 public constant PURCHASE_TYPEHASH =
        keccak256('EditionInfo(address contractAddress,address buyerAddress,uint256 nonce,uint256 editionId)');
    ...
    event TokenURISet(uint256 indexed tokenId, string tokenURI, uint256 timestamp);
    event UnrevealedURISet(string newUnrevealedURI, uint256 timestamp);

    modifier onlyAdmin() {
        require(admin == msgSender(), "caller is not the admin");
        _;
    }
    ...
    function initialize(
        ...
        __Ownable_init();
        __ERC712_init(DOMAIN_NAME, DOMAIN_VERSION, block.chainid);

        // Set ownership to original sender of contract call
        transferOwnership(_owner);

        // Set address used for signature verification 
        admin = _admin;

        // Set matic weth address
    ...
    /// @notice Overrides _msgSender() function of contract Context inherited by ERC721Upgradeable
    /// Replaces msg.sender with msgSender() function of contract EIP712MetaTransaction
    function _msgSender() internal view override returns (address sender) {
        return msgSender();
    }    
    
    /// @notice Overrides isApprovedForAll to auto-approve Opensea's proxy contract   
    ...
    /// @param _signature A signed message for authorizing purchases
    function buyEdition(
        uint256 _editionId,
        ...
        require(startTime < block.timestamp, "Auction hasn't started");
        // Don't allow purchases after the end time
        require(endTime > block.timestamp, 'Auction has ended');
        // Check that the signature is valid.
        require(getSigner(_signature, _editionId) == admin, 'Invalid signer');        
        // Check weth allowance to this contract
        require(weth.allowance(msgSender(), address(this)) >= price, 'Insuffient approval for weth');

        // Update the deposited total for the edition
        depositedForEdition[_editionId] += price;

        // Increment the number of tokens sold for this edition.
        editions[_editionId].numSold++;

        // increase nonce for user (to avoid re-use)
        buyNonces[msgSender()] = buyNonces[msgSender()] + 1;

        // Transfer weth to fundingRecipient
        weth.safeTransferFrom(msgSender(), editions[_editionId].fundingRecipient, price);

        // Mint a new token for the sender, using the `tokenId`.
        _mint(msgSender(), atTokenId.current());

        // Store the mapping of token id to the edition being purchased.
        tokenToEdition[atTokenId.current()] = _editionId;       

        emit EditionPurchased(_editionId, atTokenId.current(), editions[_editionId].numSold, msgSender());
    ...
    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        ...
        bool revealed = tokenURIRevealed[_tokenId];        
        ...
        return bytes(unrevealedURI).length > 0 ? string(abi.encodePacked(base, unrevealedURI)) : "";
    }

    /// @notice Sets token URI, only admin can set
    function setTokenURI(uint256 _tokenId, string memory _tokenURI) external onlyAdmin {        
        ...
        tokenURIRevealed[_tokenId] = true;
        tokenURIs[_tokenId] = _tokenURI;
        emit TokenURISet(_tokenId, _tokenURI, block.timestamp);
    }

    /// @notice Sets new unrevealed URI, only admin can set
    function setUnrevealedURI(string memory _newUnrevealedURI) external onlyAdmin {        
        unrevealedURI = _newUnrevealedURI;
        emit UnrevealedURISet(_newUnrevealedURI, block.timestamp);
    }
    ...
    function getOwnersOfEdition(uint256 _editionId) public view returns (address[] memory) {
        ...
        for (uint256 id = 1; id < atTokenId.current(); id++) {
            if (tokenToEdition[id] == _editionId) {
                ownersOfEdition[index] = ownerOf(id);
                index++;
            }
        }
    ...
    function getBuyNonce(address user) public view returns (uint256) {
        return buyNonces[user];
    }
...
/// @dev Started as a fork of Sound XYZ's ArtistCreator.sol https://github.com/soundxyz/protocol/blob/master/contracts/ArtistCreator.sol
contract CreatorFactory is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    using CountersUpgradeable for CountersUpgradeable.Counter;

    // ============ Storage ============

    CountersUpgradeable.Counter public atCreatorId;
    address public admin;
    ...
    event CreatorDeployed(uint256 creatorId, string name, string symbol, address indexed creatorAddress);
    ...
    function initialize() public initializer {
        ...
        // set admin for creator deployment authorization
        admin = msg.sender;
        DOMAIN_SEPARATOR = keccak256(abi.encode(keccak256('EIP712Domain(uint256 chainId)'), block.chainid));

        // set up beacon with msg.sender as the owner
        UpgradeableBeacon _beacon = new UpgradeableBeacon(address(new Creator()));
        _beacon.transferOwnership(msg.sender);
        beaconAddress = address(_beacon);

        // Set creator id start to be 1 not 0
        atCreatorId.increment();
    }

    /// Creates a new creator contract as a factory with a deterministic address
    ...
    function deployCreator(
        ...
        // add to registry
        creatorContracts.push(address(proxy));

        emit CreatorDeployed(atCreatorId.current(), _name, _symbol, address(proxy));
    ...
    /// Sets the admin for authorizing creator deployment
...
contract EIP712MetaTransaction is EIP712Base {
  string internal constant DOMAIN_NAME = "moments.tv";
...
/// @title Creator
/// @author Moments - Sejong Shon
/// @notice This contract has a minimal code since setting up a beacon from CreatorFactory using CreatorV2
/// @notice exceeds minimal contract size for deployment. Thus removed functions except some storage state variables
/// @notice and beacon will be upgraded to CreatorV2 right after.
/// @dev Started as a fork of Mirror's Editions.sol https://github.com/mirror-xyz/editions-v1/blob/main/contracts/Editions.sol
/// @dev Started as a fork of Sound XYZ's Artist.sol https://github.com/soundxyz/protocol/blob/master/contracts/Artist.sol
contract Creator is ERC721Upgradeable, IERC2981Upgradeable, OwnableUpgradeable {
    using LibUintToString for uint256;
    using SafeERC20Upgradeable for IERC20Upgradeable;
    using CountersUpgradeable for CountersUpgradeable.Counter;
...
contract EIP712Base is Initializable {
  ...
  function __ERC712_init(string memory name, string memory version, uint256 chainId) internal onlyInitializing {
    __ERC712_init_unchained(name, version, chainId);
  }

  function __ERC712_init_unchained(string memory name, string memory version, uint256 chainId) internal onlyInitializing {
    _setDomainSeparator(name, version, chainId);
  }

  function _setDomainSeparator(string memory name, string memory version, uint256 chainId) internal {
