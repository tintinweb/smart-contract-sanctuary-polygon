    ...
    ReentrancyGuardUpgradeable
{
    ...
    function redeemSettled(
        ...
        uint256 collQtyToBeRedeemed = (_volatilityIndexTokenQty *
            settlementPrice) +
    ...
    {
        require(
            _volatilityTokenAmount != 0,
            "Volmex: amount should be non-zero"
        );
        ...
        (uint256 volatilityAmount, ) = v2Protocol.collateralize(
            collQtyToBeRedeemed
        );

        IERC20Modified _volatilityToken = v2Protocol.volatilityToken();
        IERC20Modified _inverseVolatilityToken = v2Protocol
            .inverseVolatilityToken();

        _volatilityToken.safeTransfer(msg.sender, volatilityAmount);
        _inverseVolatilityToken.safeTransfer(msg.sender, volatilityAmount);
    }
    ...
    function _redeem(
        ...
        collateral.safeTransfer(_receiver, _collateralQtyRedeemed);
...
interface IVolmexProtocol {
    ...
    function redeemSettled(uint256 _volatilityIndexTokenQty, uint256 _inverseVolatilityIndexTokenQty) external returns (uint256, uint256);
