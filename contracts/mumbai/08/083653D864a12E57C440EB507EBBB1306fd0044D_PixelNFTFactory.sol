...
contract PixelNFTMinter is Ownable {
    ...
    event PrivateMintEnabled(bool _enabled);
    event PublicMintEnabled(bool _enabled);
    ...
    function mint(address _to, uint _count) public payable isMintValid(_count) {
        require(publicMintEnabled, "!enabled public mint");
        _mint(_to, _count);
    ...
    ) public payable isMintValid(_count) isValidMerkleProof(_to, _proof) {
        require(publicMintEnabled, "!enabled public mint");
        _mint(_to, _count);
    }
    ...
    function selectWinner() private {
        ...
        privateMintEnabled = false;
        requestId = vrf.requestRandomWords();
    }

    function adminSelectWinner() public onlyOwner {
        return selectWinner();
    }
    ...
    function updatePublicMintEnabled(bool enable) public onlyOwner {
        publicMintEnabled = enable;
        emit PublicMintEnabled(enable);
    }

    function updatePrivateMintEnabled(bool enable) public onlyOwner {
        privateMintEnabled = enable;
        emit PrivateMintEnabled(enable);
    }
    ...
    function updateFeePercent(uint8 _fee) public onlyOwner {
        adminFeePercent = _fee;
        emit FeePercentUpdated(_fee);
    }

    function updateNftContrcat(IPixelNFT _newAddress) public onlyOwner {
        nftContract = IPixelNFT(_newAddress);
    }

    function updateVRF(IVRF2 _newAddress) public onlyOwner {
