    ...
    EIP712Upgradeable
{
    ...
    {
        require(_exists(id), "ERC721Tradable#uri: NONEXISTENT_TOKEN");
        uris[id] = newuri;
    }
    ...
    function uri(uint256 _id) public view override returns (string memory) {
        ...
        string memory uri_id = StringsUpgradeable.toString(_id);

        if (bytes(uris[_id]).length > 0) {
            uri_id = uris[_id];
        }

        return strConcat(super.uri(_id), uri_id, ".json");
    }
    ...
    function create(
        ...
        if (bytes(super.uri(_id)).length > 0) {
            emit URI(super.uri(_id), _id);
        }
    ...
    function mintBatch(
        ...
        uint256[] memory amounts,
        bytes memory data
    ) public override onlyRole(MINTER_ROLE) nonReentrant {
        for (uint256 i = 0; i < ids.length; i++) {
            // uint256 _id = ids[i];
    ...
    function exists(uint256 id) public view virtual override returns (bool) {
        return _exists(id) || ERC1155SupplyUpgradeable.totalSupply(id) > 0;
    }
    ...
    function operatorPermit(
        ...
        require(block.timestamp <= deadline, "ERC1155Permit: expired deadline");
        require(hasRole(OPERATOR_ROLE, operator), "ERC1155Permit: only operator");
        bytes32 _hashedMessage = keccak256(
            abi.encodePacked("approve operator")
        );
        bytes memory prefix = "\x19Ethereum Signed Message:\n32";
        bytes32 prefixedHashMessage = keccak256(
            abi.encodePacked(prefix, _hashedMessage)
        );
        address signer = ECDSAUpgradeable.recover(prefixedHashMessage, v, r, s);
        require(signer == owner, "ERC1155Permit: invalid signature");
        _setApprovalForAll(owner, operator, approved);
    }

    function operatorPermitAndTransfer(
        address owner,
        bool approved,
        uint256 deadline,
        uint8 v,
        ...
        operatorPermit(owner, msg.sender, approved, deadline, v, r, s);
        safeTransferFrom(owner, receiver, tokenId, amount, "");
    }
}