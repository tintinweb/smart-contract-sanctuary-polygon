...
contract PolicyBookFabric is IPolicyBookFabric, OwnableUpgradeable, AbstractDependant {
    ...
    function create(
        ...
        if (_contractType == ContractType.CUSTODIAN) {
            require(owner() == msg.sender, "PBF: Only owner can create custodian pool");
        }
...
contract PolicyBookFacade is IPolicyBookFacade, AbstractDependant, Initializable {
    ...
    function _reevaluateProvidedLeverageStable(uint256 newAmount) internal {
        ...
        _newAmountPercentage = newAmount.mul(PERCENTAGE_100).tryDiv(_totalLiq);
    ...
    function deployLeverageFundsAfterRebalance(
        ...
        for (uint256 i = 0; i < userLeveragePools.length(); i = uncheckedInc(i)) {
    ...
    function _updateShieldMining(
        ...
        if (isWithdraw) {
            userLiquidity[liquidityProvider] = userLiquidity[liquidityProvider].trySub(
                liquidityAmount
            );
        } else {
        ...
        returns (uint256)
    {
        ...
        return
            _coverTokens.uncheckedDiv(100).mul(DECIMALS18).div(
    ...
    function requestWithdrawal(uint256 _tokensToWithdraw) external override {
        ...
        require(
            !claimingRegistry.hasProcedureOngoing(address(policyBook), 0),
        ...
        returns (address[] memory _userLeveragePools)
    {
        ...
        _userLeveragePools = new address[](to.uncheckedSub(offset));
    ...
    function secondsToEndCurrentEpoch() public view override returns (uint256) {
        uint256 epochNumber =
            block
                .timestamp
                .trySub(policyBook.epochStartTime())
                .uncheckedDiv(EPOCH_DURATION)
                .tryAdd(1);

        return
            epochNumber.tryMul(EPOCH_DURATION).trySub(
                block.timestamp.trySub(policyBook.epochStartTime())
            );
    }
    ...
    function forceUpdateBMICoverStakingRewardMultiplier() external override {
        ...
        if (policyBook.whitelisted()) {
            ...
            if (coverTokens > 0 && liquidity > 0) {
                ...
                uint256 utilizationRatio = coverTokens.mul(PERCENTAGE_100).uncheckedDiv(liquidity);

                if (utilizationRatio < MODERATE_UTILIZATION_RATIO) {
                    rewardMultiplier = Math
                        .max(utilizationRatio, PRECISION)
                        .uncheckedSub(PRECISION)
                        .mul(BASE_REWARD.uncheckedSub(MINIMUM_REWARD))
                        .uncheckedDiv(MODERATE_UTILIZATION_RATIO)
                        .add(MINIMUM_REWARD);
                } else if (utilizationRatio > RISKY_UTILIZATION_RATIO) {
                    rewardMultiplier = MAXIMUM_REWARD
                        .uncheckedSub(BASE_REWARD)
                        .mul(utilizationRatio.uncheckedSub(RISKY_UTILIZATION_RATIO))
                        .uncheckedDiv(PERCENTAGE_100.uncheckedSub(RISKY_UTILIZATION_RATIO))
                        .add(BASE_REWARD);
                }
            }
        }

        IPolicyBookFabric.ContractType contractType = policyBook.contractType();

        rewardsGenerator.updatePolicyBookShare(
            rewardMultiplier.uncheckedDiv(10**22),
            address(policyBook),
            contractType
        ...
        )
    {
        ...
        totalSeconds = secondsToEndCurrentEpoch().add(
            (_epochsNumber.uncheckedSub(1)).tryMul(EPOCH_DURATION)
        );
    ...
    {
        return (
            IERC20Metadata(address(policyBook)).symbol(),
