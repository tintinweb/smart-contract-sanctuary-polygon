...
contract Staking is ReentrancyGuard, AccessControl, Pausable {
    ...
    uint256 private _minimumAmountToStake;
    ...
    function stake(uint256 amount, uint256 depositTypeIndex) external whenNotPaused {
        if (amount < _minimumAmountToStake) {
        ...
        uint256 depositsNumberPerOwnerLocal = _depositsNumberPerOwner[msg.sender];
        ...
        _depositsByOwner[msg.sender][depositsNumberPerOwnerLocal++] = deposit;
    ...
    function unstake(uint256 depositId) public whenNotPaused {
        ...
        /// @dev check if the deposits have been already unstaked
        ...
        if (currentDeposit.maturityTimestamp <= block.timestamp && currentDeposit.coolingTimestamp == 0) {
            ...
            currentDeposit.reward = 0;
            currentDeposit.coolingTimestamp = uint40(block.timestamp) + currentDeposit.depositType.coolingDuration;
            _depositsByOwner[msg.sender][depositId] = currentDeposit;
            emit UnstakePriorMaturity(msg.sender, depositId, currentDeposit.coolingTimestamp);
        }
    }

    /**
     * @notice Claim unstaked deposit prior maturity
     * @notice half of the fees are going back to the reward pool, the other half is burned
     * @param depositId id of the deposit
     */
    ...
    function setMinimumAmountToStake(uint256 newMinimumAmountToStake) external onlyRole(SETTER_ROLE) {
        if (newMinimumAmountToStake == 0) {
            revert InvalidAmount();
        }
        _minimumAmountToStake = newMinimumAmountToStake;
        emit SetMinimumAmountToStake(newMinimumAmountToStake);
    }
     ...
     * @param coolingDuration duration in seonds to wait if the amount is unstaked prior maturity
    ...
    function minimumAmountToStake() external view returns (uint256) {
        return _minimumAmountToStake;
    }
    ...
    event UnstakePriorMaturity(address indexed owner, uint256 depositId, uint40 coolingTimestamp);
    ...
    event ClaimCooldownDeposit(address indexed owner, uint256 amountOfDeposit, uint256 claimedAmount);
    ...
    event SetMinimumAmountToStake(uint256 minimumAmountToStake);
}
