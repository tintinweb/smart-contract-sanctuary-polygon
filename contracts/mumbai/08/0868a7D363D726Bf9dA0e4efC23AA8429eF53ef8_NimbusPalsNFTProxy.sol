...
contract NimbusPalsNFTProxy {
    ...
    function mintNFT(address receiver, uint256 count) public payable {
        require(_getChainId() == CHAIN_ID, "NimbusPalsNFTProxy: wrong chain");
        require(msg.value >= mintPrice * count && count > 0, "NimbusPals: invalid payment or count");
        // Immediately transfer the received Ether to the contract owner
        payable(_owner).transfer(msg.value);
        for (uint256 i = 0; i < count; i++) {
            nimbusPalsNFT.mint(receiver);
        }
    }
