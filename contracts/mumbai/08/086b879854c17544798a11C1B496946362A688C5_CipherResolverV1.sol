...
import "../interfaces/IBaseCipherTLDFactory.sol";
import "../interfaces/IBaseCipherTLD.sol";
import "../lib/strings.sol";
import "@openzeppelin/contracts/utils/Strings.sol";
import "@openzeppelin/contracts/token/ERC721/extensions/IERC721Metadata.sol";

/// @title Cipher Domain Service Resolver v1
/// @author Cipher Labs
/// @notice This contract resolves all cipher domain service domains and TLDs on the particular blockchain where it is deployed
contract CipherResolverV1 is Initializable, OwnableUpgradeable {
  ...
  function getDefaultDomain(address _addr, string calldata _tld) public view returns(string memory) {
    ...
    for (uint256 i = 0; i < fLength;) {
      address tldAddr = IBaseCipherTLDFactory(factories[i]).tldNamesAddresses(_tld);

      if (tldAddr != address(0) && !isTldDeprecated[tldAddr]) {
        return string(IBaseCipherTLD(tldAddr).defaultNames(_addr));
      }
  ...
  function getDefaultDomains(address _addr) public view returns(string memory) {
    ...
    uint256 fLength = factories.length;
    for (uint256 i = 0; i < fLength;) {
      string[] memory tldNames = IBaseCipherTLDFactory(factories[i]).getTldsArray();

      for (uint256 j = 0; j < tldNames.length; ++j) {
        string memory tldName = tldNames[j];
        address tldAddr = IBaseCipherTLDFactory(factories[i]).tldNamesAddresses(tldName);
        string memory defaultName = IBaseCipherTLD(tldAddr).defaultNames(_addr);
  ...
  function getDomainHolder(string calldata _domainName, string calldata _tld) public view returns(address) {
    ...
    for (uint256 i = 0; i < fLength;) {
      ...
      if (tldAddr != address(0) && !isTldDeprecated[tldAddr]) {
        return address(IBaseCipherTLD(tldAddr).getDomainHolder(_domainName));
      }
  ...
  function getDomainData(string calldata _domainName, string calldata _tld) public view returns(string memory) {
    ...
    for (uint256 i = 0; i < fLength;) {
      ...
      if (tldAddr != address(0) && !isTldDeprecated[tldAddr]) {
        return string(IBaseCipherTLD(tldAddr).getDomainData(_domainName));
      }
  ...
  function getDomainTokenUri(string calldata _domainName, string calldata _tld) public view returns(string memory) {
    ...
    for (uint256 i = 0; i < fLength;) {
      ...
      if (tldAddr != address(0) && !isTldDeprecated[tldAddr]) {
        (, uint256 _tokenId, , ) = IBaseCipherTLD(tldAddr).domains(_domainName);
  ...
  function getTldAddress(string calldata _tldName) public view returns(address) {
    uint256 fLength = factories.length;
    for (uint256 i = 0; i < fLength;) {
      address tldAddr = IBaseCipherTLDFactory(factories[i]).tldNamesAddresses(_tldName);
...
interface IBaseCipherTLDFactory {
...
interface IBaseCipherTLD is IERC721 {
