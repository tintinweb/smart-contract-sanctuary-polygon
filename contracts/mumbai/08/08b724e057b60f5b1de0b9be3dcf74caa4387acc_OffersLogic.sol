...
contract OffersLogic is IOffers, ReentrancyGuardLogic, ERC2771ContextConsumer {
    ...
    modifier onlyAssetRole(address _asset) {
        require(
            PermissionsLogic(address(this)).hasRoleWithSwitch(
                ASSET_ROLE,
                _asset
            ),
            "!ASSET_ROLE"
    ...
    modifier onlyExistingOffer(uint256 _offerId) {
        OffersStorage.Data storage data = OffersStorage.offersStorage();
        require(
            data.offers[_offerId].status == IOffers.Status.CREATED,
    ...
    function getAllOffers(
        ...
        OffersStorage.Data storage data = OffersStorage.offersStorage();
        require(
            _startId <= _endId && _endId < data.totalOffers,
            "invalid range"
        );
    ...
    function _payout(
        ...
        (address platformFeeRecipient, uint16 platformFeeBps) = IPlatformFee(
            address(this)
        ).getPlatformFeeInfo();
        ...
        try
            IERC2981(_offer.assetContract).royaltyInfo(
                _offer.tokenId,
                _totalPayoutAmount
            )
