    ...
    PausableUpgradeable
{
    ...
    function addSubscription(
        ...
        totalSubscription.push(subscriptionID);
    ...
    function autoClaim() external whenNotPaused onlyOperator {
        for (uint256 index = 0; index < totalSubscription.length; index++) {
            Subscription memory subscription = _subscriptions[index];
            if (block.timestamp > subscription.startTime)
                _claim(totalSubscription[index], block.timestamp);
        }
    }
    ...
    function _claim(uint256 subscriptionID, uint256 blockTime) internal {
        ...
        require(subscription.isActive == true);
        ...
        if (blockTime > subscription.endTimeVesting) {
            ...
            uint256 index = _findIndexArray(
                totalSubscription,
                totalSubscription.length,
                subscriptionID
            );
            _remove(totalSubscription, index);
