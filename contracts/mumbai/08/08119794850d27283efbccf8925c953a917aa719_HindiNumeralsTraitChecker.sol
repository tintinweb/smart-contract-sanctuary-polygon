...
contract HindiNumeralsTraitChecker is Ownable, ITraitChecker {
    string public constant TRAIT_FAMILY = "Hindi Numeral Club";
    ...
    function getTrait(string calldata _username) external pure returns (string memory, string memory) {
        ...
        for (uint i = 0; i < b.length;) {
            ...
            i += size;
            if(code < 0xe0a5a6 || code > 0xe0a5af) {
                return ("","");
            }
        }
        uint l = b.length / 3;
    
        if(l == 1) return (TRAIT_FAMILY,"Hindi 10 club");
        if(l == 2) return (TRAIT_FAMILY,"Hindi 100 club");
        if(l == 3) return (TRAIT_FAMILY,"Hindi 1000 club");
        if(l == 4) return (TRAIT_FAMILY,"Hindi 10k club");
        if(l == 5) return (TRAIT_FAMILY,"Hindi 100k club");
        if(l == 6) return (TRAIT_FAMILY,"Hindi 1m club");
