...
contract NFTRentMarketplace is VRFConsumerBaseV2, ConfirmedOwner, IERC721Receiver {
  ...
  function getRentQuote(
    ...
    rentQuoteMatic = calculateRentPrice(basePrice, _rentTime);
    (int answer, uint8 decimal) = getLatestPrice();
    rentQuoteDollar = (uint256(answer) * rentQuoteMatic) / (10 ** decimal);
    //atualizar para não ser matic, e sim ser blockchainCurrency
  ...
  function finishRent(uint256 _rentId, address _nftContractAddress) public onlyRenteeOrContractOwner(_rentId) {
  ...
  function calculateRentPrice(uint256 _basePrice, uint256 _rentTime) internal view returns (uint256) {
