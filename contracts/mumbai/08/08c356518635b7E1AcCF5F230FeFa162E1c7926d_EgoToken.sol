...
contract EgoToken is AxelarExecutable, IERC20CrossChain, Pausable, ERC20, AccessControl, ERC20Permit {
    using StringToAddress for string;
    using AddressToString for address;
    using ECDSA for bytes32;

    //Admin can withdraw tokens from contract, withdraw native currency and setTrustedRemote
    bytes32 public constant ADMIN_ROLE = keccak256("ADMIN_ROLE");

    //Can pause contract
    bytes32 public constant PAUSER_ROLE = keccak256("PAUSER_ROLE");

    //Validate transfer remote transactions
    bytes32 public constant VALIDATOR_ROLE = keccak256("VALIDATOR_ROLE");

    //Can receive tokens in transferRemote transaction
    bytes32 public constant BRIDGE_MASTER_ROLE = keccak256("BRIDGE_MASTER_ROLE");

    //Status of bridge. If true, cross chain transfers available.
    bool public bridgeStatus = false;

    //Axelar gas receiver contract
    IAxelarGasService public immutable gasReceiver;

    //Maximum amount of tokens which can exist in this network
    ...
    //nameOfNetworkToChainId[string nameOfNetwork] = chainId
    mapping( string => uint256 ) public nameOfNetworkToChainId; ///can make it struct
    ...
    //Emitted than try to to call `execute` from invalid contract 
    event FalseSender(string sourceChain, string sourceAddress);
    
    //Emitted than ADMIN `changeBridgeStatus`
    event BridgeStatusChanged(address sender,bool currentBridgeStatus);
    
    //Emitted than `setTrustedRemote`
    event RemoteStatusChanged(string chain, bool newStatus, uint256 chainId);

    //Emitted than user 'transferRemote'
    event MessageSended(address sender, bytes payload, string destinationChain);

    /** @notice Constructor of contract
        * @param _name Name of token
        * @param _symbol Symbol of token
        * @param gateway_ Address of Axelar gateway. Gateway can transfer token between chains
        * @param gasReceiver_ Address of Axelar gas receiver. Send payment for moving between chains to this address
        * @param validator Addres which receive initial supply of tokens
    */
    constructor(
        string memory _name, 
        ...
        //Mint initial supply to polygon
        if(block.chainid == 80001){
            _mint(validator, 1_000_000_000 * 10 ** 18);
        }
    ...
    /** @notice Function can enable or disable bridge functionality
    */
    ...
    /** @notice Withdraw tokens from contract address
        * @param _token Address of token
        * @param _receiver Address of receiver
        * @param _amount amount of tokens for withraw
    */
    ...
    */
    ...
    /** @notice Transfer tokens from current chain to choosen chain
        ...
        * @param destinationAddress Address of tokens receiver in destination chain
    ...
    */
    function transferRemote(
        ...
        _checkRole(BRIDGE_MASTER_ROLE, destinationAddress);
        {
            ...
            bytes32 message = keccak256(
                abi.encodePacked(block.chainid, msg.sender, destinationAddress, amount, address(this), _salt, nameOfNetworkToChainId[destinationChain]));
     ...
     * @param recipient address which receive native currency
    ...
    /** @notice Internal function for cross chain message execution
    */
    function _execute(
...
///add here lock bidge
        ...
        (
            ...
            bytes32 r,
            bytes32 s
        ) = abi.decode(payload, (address, address, uint256, uint256, uint8, bytes32, bytes32));
