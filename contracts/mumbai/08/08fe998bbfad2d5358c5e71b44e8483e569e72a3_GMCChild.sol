...
contract GMCChild is UUPSUpgrade, OwnableUDS, FxERC721EnumerableChild, GMCMarket {
    ...
    function _afterIdRegistered(
        ...
        if (from != address(0)) {
            ...
            // fail in rare cases (when resetting the gang vault and all its shares).
...
bytes32 constant DIAMOND_STORAGE_GANG_VAULT = keccak256("diamond.storage.gang.vault.persistent");
bytes32 constant DIAMOND_STORAGE_GANG_VAULT_FX = keccak256("diamond.storage.gang.vault.flexible");
