...
contract BeaconLightClient is BeaconLightClientUpdate,BLS12381 {
    ...
    function import_next_sync_committee(
        ...
        bytes32 domain = compute_domain(DOMAIN_SYNC_COMMITTEE, header_update.fork_version, GENESIS_VALIDATORS_ROOT);
        bytes32 signing_root = compute_signing_root(header_update.attested_header, domain);
        uint256[28] memory fieldElement =  hashToField(signing_root);
        for(uint256 i =0;i<fieldElement.length;i++){
            require(header_update.sync_aggregate.proof.input[i] == fieldElement[i],"!verifyProof");
        }
    ...
    function import_finalized_header(FinalizedHeaderUpdate calldata update) external {
        ...
        uint64 current_period = compute_sync_committee_period(update.finalized_header.slot);
        bytes32 singature_sync_committee_root = sync_committee_roots[current_period];
        require(singature_sync_committee_root != bytes32(0), "!missing");
        ...
        uint256[28] memory fieldElement =  hashToField(signing_root);
        for(uint256 i =0;i<fieldElement.length;i++){
            require(update.sync_aggregate.proof.input[i] == fieldElement[i],"!verifyProof");
        }
