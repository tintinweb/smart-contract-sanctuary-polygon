    ...
    OwnableUpgradeable
{
    ...
    function _payout(IMarketplace.Listing calldata _listing) internal {
        uint256 balance = _listing.price;
        // extract fee
        uint256 marketRevenue = balance.mul(marketPercent).div(10000);
        ...
        if (address(royaltyRegistry) != address(0)) {
            ...
            if (collectionPayee != address(0) && collectionRoyalty > 0) {
                uint256 collectionRevenue = _listing
                    .price
                    .mul(collectionRoyalty)
                    .div(10000);
