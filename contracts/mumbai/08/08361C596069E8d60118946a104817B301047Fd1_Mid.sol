...
contract Mid is Royalty, AccessControl, BaseRegistrarImplementation, Pausable, TokenURISetting, Initializable {
    ...
    mapping(uint256 => bytes32) private  _nameOfTokenId;
    mapping(bytes32 => uint256) private  _tokenIdOfName;
    ...
    constructor(
        ...
        address payable micTreasury_,
        address payable midLandTreasury_,
        address payable alchemistTreasury_,
        address payable fundTreasury_,
        address usdt_) BaseRegistrarImplementation("MetaID", "MID", nns_) Royalty(erc20TransferHelper_) {
        ...
        _usdt = usdt_;
        _fundTreasury = fundTreasury_;
        _init(micTreasury_, midLandTreasury_, alchemistTreasury_);
    }

    /**
     * @dev Initializes the contract with the given parameters.
     */
    function init(address micAddress_, address landAddress_) external initializer {
    ...
    function priceOracle(uint256 count) public pure returns(uint256) {
        ...
        return 200 ether * 105 ** epoch / 100 ** epoch;
    }
    ...
    function mintMid(string memory name) nonReentrant external {
        ...
        IERC20(_usdt).safeTransferFrom(msg.sender, _fundTreasury, currentPrice());
    ...
    function mintRoute(string memory name, address owner_) external {
        ...
        uint256 seed = Rand.getRand(Strings.toString(block.timestamp), tokenId);
        bytes3 _rarity = rarity(seed);
    ...
    function setRoyaltyOracle(address royaltyOracle_) public nonReentrant {
...
abstract contract Royalty is ReentrancyGuard, Context, IncomingTransferSupport{
    address payable _micTreasury;
    address payable _midLandTreasury;
    address payable _alchemistTreasury;
     ...
     * @dev Initializes the contract with the given parameters.
     */
    function _init(address payable micTreasury_, address payable midLandTreasury_, address payable alchemistTreasury_) internal {
        _micTreasury = micTreasury_;
        _midLandTreasury = midLandTreasury_;
     ...
     */
    function midLandTreasury() public view returns (address) {
     ...
     */
    function alchemistTreasury() public view returns (address) {
     ...
     */
    function royalteOracleAddress() public view returns (address) {
    ...
    function compute(
        ...
        if (address(_royaltyOracle) != address(0)) {
            if(from != address(0) && preOwner[tokenId] != address(0)){
                require(_micTreasury != address(0) && _midLandTreasury != address(0) && _alchemistTreasury != address(0), "Address cannot be 0");
                IERC20 token = _royaltyOracle.token();
