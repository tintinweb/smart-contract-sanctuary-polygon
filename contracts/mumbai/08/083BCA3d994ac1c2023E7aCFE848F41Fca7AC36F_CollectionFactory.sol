...
contract CollectionFactory is NonblockingLzApp {
    ...
    function create(string memory chSym, string memory collName, string memory collURI, string memory fileURI, uint256 price, uint256 from, uint256 to, string[] memory URIs, uint gas) public payable {
        require(bytes(collName).length >= 3 && chainSymbolToId[chSym] > 0);
        if (chainSymbolToId[chSym] == chainId) {
            IOmniERC721 collection = _add(collName, collURI, msg.sender, price);
            _setDetails(collection, fileURI, URIs, from, to);
            emit Created(collection.getAddress(), msg.sender);
            return;
        }
        bytes memory payload = _getPayload(collName, collURI, fileURI, price, from, to, msg.sender, URIs);
    ...
    function onMessageFailed(
        ...
        emit FailedCreate(srcId, src, nonce, _payload);
    }
