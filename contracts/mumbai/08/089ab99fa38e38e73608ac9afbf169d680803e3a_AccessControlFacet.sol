 ...
 * @notice Facet that implements AccessControl in the EIP-2535 standard.
 */
contract AccessControlFacet is IAccessControl, BaseContract {
    /**
     * @dev Returns `true` if `account` has been granted `role`.
     */
    function hasRole(bytes32 role, address account) public view returns (bool) {
        return LibDiamond.hasRole(role, account);
    }
     ...
     * @param role role for which the admin role is to be set
     * @param adminRole new admin role for the role
     */
    function setRoleAdmin(bytes32 role, bytes32 adminRole) public onlyRole(Constants.OWNER_ROLE) {
