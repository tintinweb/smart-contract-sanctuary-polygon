...
contract ProjectFacet is ProjectModifiers {
        ...
        categoryIsActive(categoryId_)
    {
        ...
        s.projects[s.projectsCount].categoryId = categoryId_;
    }
        ...
        categoryIsActive(categoryId_)
    {
        ...
        s.projects[id_].categoryId = categoryId_;
    }
    ...
    /// @dev Explain to a developer any extra details
    /// @param categoryId_ The category to gets the projects
    /// @return Array of ProjectStorage
    // TODO check that category exists
    function getAllProjectsPerCategory(uint256 categoryId_) external view returns (ProjectStorage[] memory) {
        ProjectStorage[] memory _projects = new ProjectStorage[](
            s.projectsCount
        );
        for (uint256 i = 0; i < s.projectsCount; i++) {
            if (categoryId_ ==  s.projects[i+1].categoryId)
                _projects[i] = s.projects[i+1];
        }
        return _projects;
    }
    ...
    /// @notice Return the projects withs in ids_
    /// @dev revert whe a id does not exits
    /// @param ids_ The category to gets the projects
    /// @return ProjectStorages of ids_
    function getProjectsByIds(uint256[] calldata ids_)
        public
        view
        returns (ProjectStorage[] memory)
    {
        ...
        for (uint256 i = 0; i < ids_.length; i++) {
            if (!_projectExists(ids_[i])) revert QueryNonExistentProject();
