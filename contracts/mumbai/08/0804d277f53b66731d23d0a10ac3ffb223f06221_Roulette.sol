    ...
    OwnableUpgradeable
{
    ...
    */
    ...
    event Pause(uint256 roundId);
    event Unpause(uint256 roundId);
    ...
    event TokenStakedUpdated(uint256 roundId, address token, uint8 decimals);
    ...
    function claim(uint256 epoch) external virtual notContract nonReentrant{
        ...
        emit Claim(rounds[msg.sender][epoch].roundId, msg.sender, epoch, reward);
    }
    ...
    function claimAll() external virtual notContract nonReentrant{
        ...
        uint256 currentCheckpoint = claimCheckpoint[msg.sender];

        _updateClaimCheckpoint(currentUserEpoch[msg.sender]);
        _safeTransferToken(address(msg.sender), reward);

        emit ClaimAll(
            rounds[msg.sender][currentCheckpoint.add(1)].roundId,
            rounds[msg.sender][currentUserEpoch[msg.sender]].roundId, 
            msg.sender, 
            currentUserEpoch[msg.sender], 
            reward
            );
