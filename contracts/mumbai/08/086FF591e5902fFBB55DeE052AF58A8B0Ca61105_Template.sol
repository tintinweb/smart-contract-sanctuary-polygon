...
contract Template is ERC721URIStorageUpgradeable, ERC2981Upgradeable, EIP712Upgradeable, BasicMetaTransaction{
    ...
    function initialize(string memory _name, 
                        ...
                        address _admin, 
                        address _token, 
                        address _marketplace) public initializer {
    ...
    function _safeMint(address to, uint256 tokenId, string memory tokenURI, address royaltyKeeper, uint96 royaltyFees) internal {
    ...
    function _hash(Voucher.NFTvoucher calldata voucher) internal view returns (bytes32) {
        return _hashTypedDataV4(keccak256(abi.encode(
            keccak256(
                "NFTvoucher(uint256 tokenId,uint256 price,string tokenUri,uint160 royaltyKeeper,uint96 royaltyFees)"
            ),
            voucher.tokenId,
            voucher.price,
    ...
    function redeem(Voucher.NFTvoucher calldata _voucher, address redeemer) public {
        ...
        // console.log("signerrrrrrr",signer);
        require(signer == admin || signer == creator,"invalid signer");
        _safeMint(signer, _voucher.tokenId, _voucher.tokenUri, address(_voucher.royaltyKeeper), _voucher.royaltyFees);
