...
contract NftStaker is Ownable, ReentrancyGuard{
    ...
    function _restake(uint256 depositId) internal {
        ...
        nftDeposits[depositId].rewardsEarned = rewardAmount;
        nftDeposits[depositId].rewardsReleased = rewardMultiplier;
    }
    ...
    function unstakeAll() public {
        for(uint256 i = 0; i < nftDeposits.length; i++) {
            if(nftDeposits[i].depositOwner == msg.sender) {
    ...
    function TempWithdrawRewards1(uint256 depositId) public nonReentrant{
        _restake(depositId);
    }

    function TempWithdrawRewards2(uint256 depositId) public nonReentrant{
        rewardsToken.rewardsMint(msg.sender, nftDeposits[depositId].rewardsEarned );
        
    }

    function TempWithdrawRewards3(uint256 depositId) public nonReentrant{
        nftDeposits[depositId].rewardsReleased += nftDeposits[depositId].rewardsEarned;
        nftDeposits[depositId].rewardsEarned = 0;
        emit RewardsWithdrawLog(
            depositId,
