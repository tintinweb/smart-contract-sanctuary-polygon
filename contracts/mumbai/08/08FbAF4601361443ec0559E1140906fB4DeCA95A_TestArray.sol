...
contract TestArray {
    ...
    function encodeDefault() public returns (bytes memory) {
         encodeData = abi.encode(testArray);
        return encodeData;
    }

    function decodeDefault() public returns (address[] memory) {
        decodeData = abi.decode(abi.encode(testArray), (address[]));
        return decodeData;
    }

    function decode(bytes memory encodedData) public view returns (address[] memory) {
