...
contract TreasureHuntSeaport is Initializable, ITreasureHuntTrait, ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable, OwnableUpgradeable {
    ...
    mapping(address => bool) public seaportOwner;
    ...
    function presaleMint() public {
        require(!seaportOwner[msg.sender], "You already own seaport!");
        require(presaleCnt < presaleMintMax, "Presale End!");
        presaleCnt++;
        ITreasureHunt(rewardPool).transferCost(mintPrice, true);
        mysteryChestAmt[msg.sender] = 1;
    }

    function opneBox() public {
    ...
    function mint() private {
        ...
        saleCnt += 1;
        seaportOwner[msg.sender] = true;
        initSeaport(tokenID);
        join2Seaport(tokenID);
    }
    ...
    function join2Seaport(uint256 tokenID) public {
        require(seaportInfo[tokenID].Current + 1 <= seaportInfo[tokenID].Capacity, "Already full!");
        playersBySeaport[tokenID].add(msg.sender);
        seaportInfo[tokenID].Current++;
        onSeaport[msg.sender] = ownerOf(tokenID);
    }

    function exitFromSeaport(uint256 tokenID) public {
    ...
    function initSeaport(uint256 tokenID) internal {
        seaportInfo[tokenID] = Seaport(tokenID, 1, seaportSizeByLevel[1], 0);
