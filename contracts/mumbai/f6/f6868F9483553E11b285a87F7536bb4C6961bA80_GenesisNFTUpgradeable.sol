    ...
    UUPSUpgradeable
{
    ...
    function mintingForSales(uint256 requestedCount)
        external
        payable
        blockNumberValidator
        salesValidator(requestedCount, balanceOf(_msgSender()))
    {
        require(msg.value == mintPriceByAccount(_msgSender()).mul(requestedCount), "Sales: not enough ETH");
        _mintingForSales(requestedCount);
        _setLastCallBlockNumber();
    }

    function setDiscountRate(uint256 discountRate_) public onlyOwner {
        discountRate = discountRate_;
    }

    function mintPriceByAccount(address account) public view returns (uint256) {
        return isWhiteList(account) ? salesInfo().mintPrice.mul(100 - discountRate).div(10) : salesInfo().mintPrice;
    }
...
abstract contract WhitelistUpgradeable is Initializable, ContextUpgradeable {
    ...
    /**
     * @dev Returns true if account is whitelist.
     */
    function isWhiteList(address account) public view virtual returns (bool) {
