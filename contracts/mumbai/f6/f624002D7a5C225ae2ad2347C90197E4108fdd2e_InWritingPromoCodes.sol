...
contract InWritingPromoCodes is Ownable {
    ...
    function get_affiliate_comission(string memory promo)
        public
        view
        returns (uint8)
    {
    ...
    function mint_and_send_promo_NFT(string memory str, string memory promo)
        public
        payable
        returns (uint256)
    {
        ...
        require(
            msg.value >=
                ((write.get_minting_cost() * (100 - promoDiscount[promo])) /
                    100),
            string(
                abi.encodePacked(
        ...
        promoCodes[promo] -= 1;
        uint256 tokenId = write.mint_NFT{
            value: msg.value +
                (write.get_minting_cost() * promoDiscount[promo]) /
                100
        }(str);
    ...
    function mint_and_send_affiliate_NFT(
        ...
        require(
            msg.value >=
                ((write.get_minting_cost() * (100 - affiliateDiscount[promo])) /
                    100),
            string(
                abi.encodePacked(
        ...
        uint256 tokenId = write.mint_NFT{
            value: msg.value +
                (write.get_minting_cost() * affiliateDiscount[promo]) /
                100
        }(str);
        write.transferFrom(address(this), addr, tokenId);
        payable(affiliateAddress[promo]).transfer(
            (write.get_minting_cost() * affiliateCommission[promo]) / 100
        );
        return tokenId;
    }
