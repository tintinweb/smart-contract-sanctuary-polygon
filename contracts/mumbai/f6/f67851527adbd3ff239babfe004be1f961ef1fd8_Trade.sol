...
contract tariNFT is EIP712Upgradeable{
    ...
    function __tarinft(string memory domain, string memory version) internal{
        SIGNING_DOMAIN = domain;
        SIGNATURE_VERSION = version;
        __EIP712_init(domain, version);
    }

    function getSigner(whitelisted memory tarinft) public view returns(address){
        return _verify(tarinft);
    }
  ...
  function _hash(whitelisted memory list) internal view returns (bytes32) {
        return _hashTypedDataV4(
            keccak256(
                abi.encode(
                    keccak256("whitelisted(address seller,address buyer,address nftAddress,bool inEth,uint256 amount,uint256 tokenId,uint256 qty,uint256 timestamp)"),
                    list.seller,
                    list.buyer,
                    list.nftAddress,
                    list.inEth,
    ...
    function _verify(whitelisted memory tarinft) internal view returns (address) {
        bytes32 digest = _hash(tarinft);
        return ECDSAUpgradeable.recover(digest, tarinft.signature);
    }
...
contract Trade is tariNFT{
    ...
    function initialize(uint _platformPercentage, uint _mintFee,string memory domain,string memory version ) external initializer{
        __tarinft(domain, version);
        owner = msg.sender;
        platformPercentage = _platformPercentage;
        mintFee=(_mintFee*1 ether)/10000;
    ...
    */
    ...
    function createNFT(address nftAddress, string memory uri, uint256 supply, uint96 fee, whitelisted memory sign) external payable{
        ...
        (uint256 tokenId, bool mint) = IERC1155Upgradeable(nftAddress).mint(msg.sender, uri, supply, fee);
    ...
    function getFee() internal returns(bool) {
        require(msg.value >= mintFee, "Mint: value must be greater tha 0.01 eth");
        if(msg.value >= mintFee){
            if((payable(owner).send(mintFee))) return true;
        }
        return false;
    }
