...
contract AssentBondLpToken is Ownable, ReentrancyGuard, Pausable, ErrorReporter {
    ...
    function deposit(uint256 amount, bool autoStake) whenNotPaused nonReentrant external returns (uint256) {
        // can't deposit more than maxPerUser
        if (amount > getMaxPrincipalDeposit(msg.sender)) amount = getMaxPrincipalDeposit(msg.sender);
    ...
    function getMaxPrincipalDeposit( address _user) public view returns ( uint256 maxDeposit) {
        UserInfo memory info = userInfo[_user];
        uint256 tokenPerPrincipal;
        (,,,,,tokenPerPrincipal) = payoutFor(0,_user);

        uint256 maxUserPayout;
        if (bondTerms.maxPayoutPerUser > info.remainingPayout) {
            maxUserPayout = bondTerms.maxPayoutPerUser-info.remainingPayout;
        }
        else {
            maxUserPayout = 0;
        }
        uint256 rawMaxDeposit = maxUserPayout*MULTIPLIER/tokenPerPrincipal;
        maxDeposit = (rawMaxDeposit * 10**(IERC20(principal).decimals()))/MULTIPLIER;
