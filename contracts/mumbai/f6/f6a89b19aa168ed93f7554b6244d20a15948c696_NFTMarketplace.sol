...
contract NFTMarketplace is ReentrancyGuard {
  ...
  function getFee(uint256 price) internal view returns(uint256) {
    return price.div(100).mul(listingPrice);
  }
  ...
  function listingNFTtoMarketplace(
    ...
    if(nftType == BuyingAssetType.BEP721) {
      IBEP721(nftContract).safeTransferFrom(seller, address(this), tokenId);
    }
    ...
    /**
     * @dev Return calculate price with ratio 
     * @return uint256 of price with ratio
     */
    function getPriceWithRatio(uint256 price, uint256 ratio, uint8 _decimals) public pure returns (uint256) {
        if(_decimals < 18) {
            uint8 decimalTotal = _decimals + 18;
            return price.mul(10**_decimals).div(10**18).mul(ratio.mul(10**_decimals)).div(10**decimalTotal);
        }
        if(_decimals > 18) {
            uint8 decimalTotal = _decimals - 18;
            return price.mul(10**decimalTotal).mul(ratio.mul(10**decimalTotal)).div(10**_decimals);
        }
        return price.mul(ratio).div(10**_decimals);
    }
  ...
  function buyItemfromMarketplace(
    ...
    {
        uint8 decimalsToken = IBEP20Metadata(BEP20Address).decimals();
        priceRatio = getPriceWithRatio(price, ratio, decimalsToken);
        fee = getFee(priceRatio);
        uint256 totalPrice = priceRatio + fee;
    ...
    BuyingAssetType nftType = idToMarketItem[itemId].nftType;

    {
        address sellerAdd = idToMarketItem[itemId].seller;
        // transfer token to seller
        IBEP20(BEP20Address).transferFrom(buyer, sellerAdd, priceRatio);
        // transfer fee to marketplace
        IBEP20(BEP20Address).transferFrom(buyer, address(this), fee);
    }

    if(nftType == BuyingAssetType.BEP721) {
      IBEP721(nftContract).safeTransferFrom(address(this), buyer, tokenId);
    }
  ...
  function WitdrawalFee(address BEP20Address, address wdAddress,uint256 amount) public isOnlyOwner nonReentrant returns(bool) {
