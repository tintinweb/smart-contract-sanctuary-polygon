...
contract SwapV3 {
    ...
    function singleSwap(
        ...
        if (amountOut == 0) {
            ...
            swapAmountIn = swapExactOutputSingle(trader, inputToken, outputToken, amountOut, amountIn, recipient);
        }
    }
    ...
    function swapExactInputSingle(
        ...
        swapAmountOut = swapRouter.exactInputSingle{value: tokenIn == WETH9 ? amountIn : 0}(params);

        if (outETH) {
            IWETH9(WETH9).withdraw(swapAmountOut);
            (bool success, ) = recipient.call{value: swapAmountOut}("");
    ...
    function swapExactOutputSingle(
        ...
        uint256 amountInMaximum,
        address recipient
    ) public returns (uint256 swapAmountIn) {
        ...
        swapAmountIn = swapRouter.exactOutputSingle{value: tokenIn == WETH9 ? amountInMaximum : 0}(params);
        if (swapAmountIn < amountInMaximum) {
            if (tokenIn == WETH9) {
                swapRouter.refundETH();
                IERC20(address(0)).uniTransfer(payable(trader), amountInMaximum - swapAmountIn);  
            } else {
                IERC20(tokenIn).uniTransfer(payable(trader), amountInMaximum - swapAmountIn);   
