...
contract Ranking is Ownable{
    ...
    function showRank(string memory Name) public view onlyOwner returns(Rank memory){
        require(_ranks.length > 0, "RANK: There are no ranks.");
        return _ranks[searchRank(Name)];
    }

    function showRank(uint256 Number) public view onlyOwner returns(Rank memory){
        require(_ranks.length > Number, "RANK: There are no ranks.");
        return _ranks[Number];
    }

    function giveRank(address user) public view returns(Rank memory){
        return _ranks[_rankTable[user]];
    }
