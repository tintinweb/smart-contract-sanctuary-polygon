...
contract mQuark is ImQuark, ERC721, IERC2981, DefaultOperatorFilterer, Ownable {
  ...
  /// @dev  apping from a 'external ERC721 collection address' to 'template uri'
  mapping(address => string) private _externalCollectionTemplateUris;
  ...
  function mint(
    ...
    if (_tempData.mintType > 1) revert WrongMintType();
    ...
    _mint(to, _tokenId);
    _tokenInfo[_tokenId] = TokenInfo(false, _tempData.royaltyReceiver, _tempData.collectionURIs[variationId]);
  ...
  function mintWithPreURI(
    ...
    _onlymQuarkControl();
    if (templateId == 0) revert InvalidTemplateId();
    if (!_verifyCollectionURIEditSignature(signature, signer, projectId, templateId, collectionId, uri, salt))
      revert VerificationFailed();
    Collection memory _tempData = _projectCollections[templateId][projectId][collectionId];
    if (_tempData.mintType != 2) revert WrongMintType();
    if (_tempData.minTokenId == 0) revert UnexsistingTokenMint();
    if (_tempData.mintCount >= _tempData.totalSupply) revert NotEnoughSupply();
    _inoperativeSignatures[signature] = true;
    uint256 _tokenId = _tempData.minTokenId + _tempData.mintCount;
    _mint(to, _tokenId);
    _tokenInfo[_tokenId] = TokenInfo(false, _tempData.royaltyReceiver, uri);
    ++_projectCollections[templateId][projectId][collectionId].mintCount;
    emit NFTMintedWithPreUri(projectId, templateId, collectionId, uri, _tokenId, to);
  }

  /**
   *  @notice Performs single free mint withot any slots.(Static and Limited Dynamic)
  ...
  function mintFree(
    ...
    if (templateId == 0) revert InvalidTemplateId();
    Collection memory _tempData = _projectCollections[templateId][projectId][collectionId];
    if (_tempData.mintType < 3 || _tempData.mintType > 4) revert WrongMintType();
    if (_tempData.minTokenId == 0) revert UnexsistingTokenMint();
    if (_tempData.mintCount >= _tempData.totalSupply) revert NotEnoughSupply();
    if (_tempData.collectionURIs.length <= variationId) revert InvalidVariation();
    uint256 _tokenId = _tempData.minTokenId + _tempData.mintCount;
    _tokenInfo[_tokenId] = TokenInfo(true, _tempData.royaltyReceiver, _tempData.collectionURIs[variationId]);
    ++_projectCollections[templateId][projectId][collectionId].mintCount;
  ...
  function mintFreeWithPreURI(
    ...
    if (templateId == 0) revert InvalidTemplateId();
    Collection memory _tempData = _projectCollections[templateId][projectId][collectionId];
    if (_tempData.mintType != 5) revert WrongMintType();
    if (_tempData.minTokenId == 0) revert UnexsistingTokenMint();
    if (_tempData.mintCount >= _tempData.totalSupply) revert NotEnoughSupply();
    if (!_verifyCollectionURIEditSignature(signature, signer, projectId, templateId, collectionId, uri, salt))
      revert VerificationFailed();
    uint256 _tokenId = _tempData.minTokenId + _tempData.mintCount;
    _tokenInfo[_tokenId] = TokenInfo(true, _tempData.royaltyReceiver, uri);
    ++_projectCollections[templateId][projectId][collectionId].mintCount;
    _inoperativeSignatures[signature] = true;
    _mint(msg.sender, _tokenId);
    emit NFTMintedFree(projectId, templateId, collectionId, (-1), _tokenId, uri, msg.sender);
  }
  ...
  function mintBatch(
    ...
    _onlymQuarkControl();
    if (templateIds.length != collectionIds.length) revert LengthMismatch();
    if (templateIds.length != amounts.length) revert LengthMismatch();
    Collection memory _tempData;
    uint256 _tokenId;
    uint256 _templateIdsLength = (templateIds.length);
    for (uint256 i = 0; i < _templateIdsLength; ) {
      if (amounts[i] > 10) revert ExceedsLimit();
      uint256 _mintAmount = amounts[i];
      if (templateIds[i] == 0 || _mintAmount == 0) revert InvalidIdAmount();
      _tempData = _projectCollections[templateIds[i]][projectId][collectionIds[i]];
      if (_tempData.mintType > 1) revert WrongMintType();
      if (_tempData.collectionURIs.length <= variationIds[i]) revert InvalidVariation();
      if (_tempData.minTokenId == 0) revert UnexsistingTokenMint();
      if ((_tempData.mintCount + _mintAmount) > (_tempData.totalSupply)) revert NotEnoughSupply();
      for (uint256 ii = 0; ii < _mintAmount; ) {
        _tokenId = _tempData.minTokenId + _tempData.mintCount;
        _mint(to, _tokenId);
        ++_tempData.mintCount;
        _tokenInfo[_tokenId] = TokenInfo(false, _tempData.royaltyReceiver, _tempData.collectionURIs[variationIds[i]]);
        emit NFTMinted(
          projectId,
          templateIds[i],
          collectionIds[i],
          variationIds[i],
          _tokenId,
          _tempData.collectionURIs[variationIds[i]],
          to
        );
        unchecked {
          ++ii;
      ...
      _projectCollections[templateIds[i]][projectId][collectionIds[i]].mintCount += _mintAmount;
      unchecked {
        ++i;
      }
    }
  ...
  function mintBatchWithURISlot(
    ...
    _onlymQuarkControl();
    if (templateIds.length != collectionIds.length) revert LengthMismatch();
    if (amounts.length != collectionIds.length) revert LengthMismatch();
    Collection memory _tempData;
    uint256 _tokenId;
    uint256 templateIdsLength = (templateIds.length);
    for (uint256 i = 0; i < templateIdsLength; ) {
      if (templateIds[i] == 0 || collectionIds[i] == 0) revert InvalidId(templateIds[i], collectionIds[i]);
      _tempData = _projectCollections[templateIds[i]][projectId][collectionIds[i]];
      if (_tempData.mintType > 1) revert WrongMintType();
      if (_tempData.collectionURIs.length <= variationIds[i]) revert InvalidVariation();
      if (_tempData.minTokenId == 0) revert UnexsistingTokenMint();
      if ((_tempData.mintCount) >= (_tempData.totalSupply)) revert NotEnoughSupply();
      _tokenId = _tempData.minTokenId + _tempData.mintCount;
      uint256 _mintAmounts = amounts[i];
      _tokenId = _tempData.minTokenId + _tempData.mintCount;
      for (uint256 ii = 0; ii < _mintAmounts; ) {
        _mint(to, _tokenId);
        _tokenProjectURIs[address(this)][_tokenId][projectId] = projectDefaultUri;
        _tokenInfo[_tokenId] = TokenInfo(false, _tempData.royaltyReceiver, _tempData.collectionURIs[variationIds[i]]);
        emit NFTMinted(
          projectId,
          templateIds[i],
        ...
        emit ProjectURISlotAdded(address(this), _tokenId, projectId, projectDefaultUri);
        ++_tokenId;
        ++_tempData.mintCount;
        unchecked {
          ++ii;
      ...
      _projectCollections[templateIds[i]][projectId][collectionIds[i]].mintCount += _mintAmounts;
      unchecked {
        ++i;
      }
    }
  ...
  function addURISlotToNFT(
    ...
    if (isMainContract && _tokenInfo[tokenId].isLocked) revert LockedNFT(tokenId);
    ...
    emit ProjectURISlotAdded(tokenContract, tokenId, projectId, projectSlotDefaultUri);
  }
  ...
  function updateURISlot(
    ...
    if (!_verifyUpdateTokenURISignature(signature, project, projectId, tokenContract, tokenId, updatedUri))
      revert VerificationFailed();
    _inoperativeSignatures[signature] = true;
    _tokenProjectURIs[tokenContract][tokenId][projectId] = updatedUri;
    emit ProjectURIUpdated(signature, projectId, tokenContract, tokenId, updatedUri);
  }
   ...
   * * collectionPrices  Mint prices for the collection, can't be lower than their templates prices
  ...
  function createCollections(
    ...
    bool isVerified;
    uint8 mintType;
    uint256 templateCount = createParams.templateIds.length;
    for (uint256 i = 0; i < templateCount; ) {
      _collectionId = ++_projectCollectionIds[projectId][createParams.templateIds[i]];
      if (_collectionId != createParams.collectionIds[i]) revert InvalidCollectionId();
      for (uint256 j = 0; j < signatures[i].length; ) {
        isVerified = _verifyCollectionURIEditSignature(
          signatures[i][j],
          signer,
          projectId,
          createParams.templateIds[i],
          _collectionId,
          uris[i][j],
          "0x01"
        );
      ...
      Collection memory _tempData = _projectCollections[createParams.templateIds[i]][projectId][_collectionId];
      if (isCollectionFree[i]) uris[i].length > 1 ? mintType = 4 : mintType = 3;
      else uris[i].length > 1 ? mintType = 1 : mintType = 0;
      _tempData = Collection(
        ...
        _collectionId,
        (_lastUsedTokenId + 1),
        _lastUsedTokenId + createParams.totalSupplies[i],
        0,
        uris[i],
        createParams.totalSupplies[i],
        mintType
      );
      _lastUsedTokenId += createParams.totalSupplies[i];
      _projectCollections[createParams.templateIds[i]][projectId][_collectionId] = _tempData;
      emit CollectionCreated(
        projectId,
        createParams.templateIds[i],
        _collectionId,
        createParams.collectionPrices[i],
        createParams.totalSupplies[i],
  ...
  function createCollectionsWithoutURIs(
    ...
    for (uint256 i = 0; i < templateCount; ) {
      ...
      if (_collectionId != createParams.collectionIds[i]) revert InvalidCollectionId();
      Collection memory _tempData = _projectCollections[createParams.templateIds[i]][projectId][_collectionId];
      if (isCollectionFree[i]) mintType = 5;
      else mintType = 2;
      _tempData = Collection(
        royaltyReciever,
        projectId,
  ...
  function registerExternalCollection(address tokenContract, string calldata templateUri) external {
    ...
    _externalCollectionTemplateUris[tokenContract] = templateUri;
    emit ExternalCollectionRegistered(tokenContract, templateUri);
  }
  ...
  function unlockFreeMintNFT(
    ...
    _tokenInfo[tokenId].isLocked = false;
  }
  ...
  function externalCollectionURI(address collectionAddress) external view returns (string memory) {
    ...
    return _externalCollectionTemplateUris[collectionAddress];
  }
  ...
  function isSlotAddedForProject(
    ...
    isAdded = !_compareStrings(_tokenProjectURIs[contractAddress][tokenId][projectId], "");
  }
  ...
  function getIsFreeMinted(uint256 tokenId) external view returns (bool) {
    return _tokenInfo[tokenId].isLocked;
  }
  ...
  {
    return (
      _projectCollections[templateId][projectId][collectionId].royaltyReceiver,
      _projectCollections[templateId][projectId][collectionId].projectId,
      _projectCollections[templateId][projectId][collectionId].templateId,
      _projectCollections[templateId][projectId][collectionId].collectionId,
      _projectCollections[templateId][projectId][collectionId].minTokenId,
      _projectCollections[templateId][projectId][collectionId].maxTokenId,
      _projectCollections[templateId][projectId][collectionId].mintCount,
      _projectCollections[templateId][projectId][collectionId].collectionURIs,
      _projectCollections[templateId][projectId][collectionId].totalSupply,
      _projectCollections[templateId][projectId][collectionId].mintType
    );
  }
...
interface ImQuark {
  ...
  // Event for when an External Collection is registered
  event ExternalCollectionRegistered(address tokenContract,string templateUri );
  ...
  event ProjectURISlotAdded(address contractAddress, uint256 tokenId, uint256 projectId, string uri);
  // Event for when a URI slot is reset for a project for a token
  event ProjectSlotURIReset(address contractAddress,uint256 tokenId, uint256 projectId);
  // Event for when a URI is updated for a project for a token
  event ProjectURIUpdated(bytes signature, uint256 projectId, address contractAddress, uint256 tokenId, string updatedUri);
...
interface ImQuarkControl {
   ...
   * @param projectId Project registered id
   * @param amount  Amount of funds that will be transferred in wei
   */
...
contract mQuarkControl is ImQuarkControl, ReentrancyGuard, PullPayment, AccessControl {
  ...
  function mintWithURI(
    ...
    if (msg.value == 0) revert SentAmountIsZero();
    if (msg.value != _collectionPrices[projectId][templateId][collectionId]) revert InvalidSentAmount();
    mQuark.mintWithPreURI(signer, msg.sender, projectId, templateId, collectionId, signature, uri, salt);
    _registeredProjects[projectId].balance += (msg.value * projectPercentage) / 100;
    adminBalance[adminWallet] += (msg.value * (adminPercentage)) / 100;
    emit FundsDeposit(msg.value, projectPercentage, projectId);
  }
  ...
  function createCollections(
    ...
    if (_templatesLength != createParams.collectionPrices.length) revert ArrayLengthMismatch();
    bool[] memory isCollectionFree = new bool[](createParams.collectionPrices.length);
    uint16 _maxSelectingLimit = MAX_SELECTING_LIMIT;
    for (uint256 i = 0; i < _templatesLength; ) {
      if (
        (createParams.collectionPrices[i] < _templateMintPrices[createParams.templateIds[i]]) &&
        (createParams.collectionPrices[i] != 0)
      ...
      _collectionPrices[projectId][createParams.templateIds[i]][createParams.collectionIds[i]] = createParams
        .collectionPrices[i];
      createParams.collectionPrices[i] == 0 ? isCollectionFree[i] = true : isCollectionFree[i] = false;
      unchecked {
        ++i;
      }
    }
  ...
  {
    return (
      _registeredProjects[projectId].wallet,
...
contract PaymentWrapper {
  ...
  event Withdrawn(address from, uint256 to);
  event PaymentMade(address from, uint256 amount, uint256 tokenId);
  ...
  function setReceiverAddress(address addr) external onlyAdmin {
    paymentReceiver = addr;
  }
  ...
  function voluntarilyPayment(
    ...
    receiverBalance[paymentReceiver] += msg.value;
    emit PaymentMade(msg.sender, msg.value, nextMintedToken);
  }

  function withdraw(uint256 amount) external {
    if (msg.sender != paymentReceiver) revert("not receiver");
    if (amount > receiverBalance[msg.sender]) revert("insufficient balance");
    receiverBalance[msg.sender] -= amount;
