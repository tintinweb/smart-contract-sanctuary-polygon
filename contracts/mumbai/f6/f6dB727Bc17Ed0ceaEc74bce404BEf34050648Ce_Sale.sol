...
library LibSign {
    ...
    function isValid(bytes32 messageHash, bytes memory signature, bytes memory pubKey) internal view returns (bool) {
        bytes32 ethSignedMessageHash = getEthSignedMessageHash(messageHash);
        console.log(address(uint160(uint256(keccak256(pubKey)))));
        // console.log(recoverSigner(ethSignedMessageHash, signature));
	...
	ReentrancyGuardUpgradeable
{
  ...
  // bytes32 public BUY_TOKEN_HASH = keccak256(bytes("buyToken(uint256 _tokenAmount,uint256 _phaseIndex)"));
  bytes32 public constant BUY_TOKEN_HASH = 0xa31cbbba7c2e487c67b0577044e22c273158ad81b194e61ef63d45d0ef1d29e2;
	...
	//array with number phases
	uint256[] public arrPhaseIndex;
	...
	event Purchase(address indexed account, uint256 amount, uint256 id, uint256 phaseIndex);
	...
	event PhaseOver(bool over, uint256 index);
	...
	modifier isPublicSale(uint256 _phaseIndex) {
		if (!phases[_phaseIndex].isPublic) {
			require(whitelist[msg.sender], "This phase is private");
		}
		_;
	}

	/// @notice require send ETH/BNB
	function buyToken(uint256 _tokenAmount,uint256 _phaseIndex, bytes memory _signature)
		external
		payable
		isPublicSale(_phaseIndex)
		nonReentrant{
		...
		bytes32 digest = keccak256(
				abi.encodePacked(
						'\x19\x01',
						DOMAIN_SEPARATOR,
						keccak256(abi.encode(BUY_TOKEN_HASH,_tokenAmount, _phaseIndex))
				)
		);

		(bytes32 r, bytes32 s, uint8 v) = LibSign.splitSignature(_signature);
		address recoveredAddress = ecrecover(digest, v, r, s);
		// console.log(recoveredAddress);

		require(recoveredAddress != address(0) && recoveredAddress == msg.sender, 'INVALID_SIGNATURE');
		require(
			block.timestamp < phases[_phaseIndex].endAt,
			"This phase is over, try next"
		);

		require(
			phases[_phaseIndex].supply >= _tokenAmount,
			"Not enough supply"
		);

		require(
			_tokenAmount >= phases[_phaseIndex].minimunEntry,
			"There are too few tokens"
		);

		uint256 finalPrice =
			(_tokenAmount * phases[_phaseIndex].price) / (getLatestPrice());
		...
		id++;
		if (phases[_phaseIndex].timelock > 0) {
			...
			getTimeToPresent(phases[_phaseIndex].timesToRelease);
			tokenLocksForSale[id].percentsToRelease = phases[_phaseIndex]
				.percentsToRelease;
		} else {
		...
		if (phases[_phaseIndex].supply == 0) {
			phases[_phaseIndex].over = true;
			emit PhaseOver(true,_phaseIndex);
		}

		tokensRemainForSale -= _tokenAmount;

		emit Purchase(msg.sender, _tokenAmount, id,_phaseIndex);
	}
	...
	function getPhase(uint256 _phaseIndex) external view returns (Phase memory) {
		return phases[_phaseIndex];
	}

	function getCurrentSupplyRemain(uint256 _phaseIndex) external view returns (uint256) {
		return phases[_phaseIndex].supply;
	}

	function getVestingTimePhase(uint256 _phaseIndex) external view returns (uint256[] memory) {
		return phases[_phaseIndex].timesToRelease;
	}

	function getVestingPercentPhase(uint256 _phaseIndex) external view returns (uint256[] memory) {
		return phases[_phaseIndex].percentsToRelease;
	}
	...
	function createPhase(
		...
		phaseIndex++;
		phases[phaseIndex] = p;
		arrPhaseIndex.push(phaseIndex);

		emit PhaseCreated(phaseIndex,_isPublic, _minimunEntry, _price, _endAt, _supply,phaseIndex);
	}
	///@notice add a list of wallet to vesting
	function addUsersToVesting(uint256 _phaseIndex,uint256 _tokenAmount,address _address) external onlyRole(DEFAULT_ADMIN_ROLE) {
			require(
			block.timestamp < phases[_phaseIndex].endAt,
		...
		require(
			...
			tokenLocksForSale[id].remainAmount = _tokenAmount;
			getTimeToPresent(phases[_phaseIndex].timesToRelease);
			tokenLocksForSale[id].percentsToRelease = phases[_phaseIndex].percentsToRelease;
			phases[_phaseIndex].supply -= _tokenAmount;
		...
		tokensRemainForSale -= _tokenAmount;
		emit AddUsersToVesting(_phaseIndex,_tokenAmount,_address);
	}
	...
	function cancelPhase(uint256 _phaseIndex) external onlyRole(DEFAULT_ADMIN_ROLE) {
		require(
			phases[_phaseIndex].over == false,
			"This phase is over,  try next"
		);
		if (phases[_phaseIndex].supply > 0) {
			ERC20(tokenAddress).transfer(
				dispatcher,
				phases[_phaseIndex].supply
			);
			phases[_phaseIndex].supply = 0;
		}
		phases[_phaseIndex].over = true;
		emit PhaseOver(true,_phaseIndex);
	}
	...
	function changeEndDate(uint256 _newEndDate,uint256 _phaseIndex)
		public
		onlyRole(DEFAULT_ADMIN_ROLE)
	{
		require(block.timestamp < _newEndDate);
		phases[_phaseIndex].endAt = _newEndDate;
	}
	...
	function withdrawBUSD(address _account, uint256 _amount) external onlyRole(DEFAULT_ADMIN_ROLE) nonReentrant {
		require(ERC20(BUSDaddress).transfer(_account,_amount),"Failed busd transfer");
