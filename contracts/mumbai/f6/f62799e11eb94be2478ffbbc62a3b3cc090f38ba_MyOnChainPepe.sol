...
contract MyOnChainPepe is ERC721A, ReentrancyGuard, Ownable, OnChainMetadata, IERC4906  {
    ...
    function beginPepeMetaMorphoseAsNewOwner(uint256 tokenId) external {
    ...
    function rebuildMyPepe(uint256 tokenId,uint256 sHue,uint256 sSat,uint256 sLum,uint256 cHue,uint256 cSat,uint256 cLum, uint256 _gum, uint256 _lazer ) external payable {
        require(ownerOf(tokenId) == msg.sender && msg.value >= (mintPrice) );
        require(sHue < 361);
        require(sSat < 101);
        require(sLum < 101);
        require(cHue < 361);
        ...
        require(cLum < 101);
        require( 
            (pepeRebuilder[tokenId] == msg.sender) && (block.number > (pepeMetaMorphBlock[tokenId] + 100 ) )
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                ...
                                image,
                                '", "description": "My On Chain Pepe Art is generated from from the smart contract with no external dependencies. Minters selected skin and clothes color via HSL at the time of mint ","attributes": [{"trait_type": "Metadata","value": "OnChain"},{"trait_type": "Gum","value":"',
