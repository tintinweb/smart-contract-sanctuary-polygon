...
contract StakeSO_TestV27 is Ownable {
 ...
 /*-------------  Staking, Unstaking & Claiming -------------*/
    ...
    */
    function unstakeAndClaim(uint256 _tokenId) public {
        ...
        // check correctness
        unstaker.idRates[tokenIdIndex] = lastIndexKeyFirstStaked;
...
/*------------- Calculation, Logics -------------*/
    ...
    */
    ...
    /*
    When called, this function unstakes all tokens that are staked longer than max duration (in months)
    */
    function checkSixMonths() public {
        // amount of addresses that have staked
        uint256 len = stakerAddresses.length;
        for(uint256 i = 0; i < len; i++) {
            // get stakers address
            address staker_add = stakerAddresses[i];
            // get stakers information (struct)
            Staker storage staker = stakers[staker_add];
            // get amount of tokens staked by staker
            uint256 tokens_staked = staker.tokenIds.length;
            for(uint256 j = 0; j < tokens_staked; j++) {
                // current time
                uint256 time_now = block.timestamp;
                //  max_months in seconds = 60*60*24*365/months
                uint256 max_months_ago = time_now - 31536000/max_months;
                // get timestamp when specific token was staked
                uint256 token_staked_at = staker.firstStakedAt[j];
                // if time staked at is smaller than max months ago, unstake it!
                if(token_staked_at < max_months_ago) {
                    unstakeAndClaim(staker.tokenIds[j]);
                }
            }
        }
    }
