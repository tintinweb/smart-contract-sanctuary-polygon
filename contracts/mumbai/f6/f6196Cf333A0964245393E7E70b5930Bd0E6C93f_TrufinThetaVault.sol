...
contract TrufinThetaVault is TrufinVault, TrufinThetaVaultStorage {
   ...
   /**
     * @notice Sets period between each options sale.
     * @param period_in_hours is the new duration in hours for options sale.
     */
    function setPeriod(uint256 period_in_hours ) external onlyKeeper {
        period = period_in_hours * 1 hours;                                       // change 2
    }
...
abstract contract TrufinThetaVaultStorage {
    ...
    address public liquidityGauge;
     ...
     /// @dev period between each options sale.
    uint256 public period = 1 hours;         //  change 1
...
library VaultLifecycle {
    ...
    struct CloseParams {
        ...
        /// @dev the duration between option sale
        uint256 period;
    }
    ...
    {
        uint256 expiry = getNextExpiry(closeParams.currentOption, closeParams.period);
        ...
        returns (uint256)
    {
        ...
        if (currentOption == address(0)) {
            return getNextFriday(block.timestamp, period);
        }
        ...
        if (block.timestamp > currentExpiry + period) {
            return getNextFriday(block.timestamp, period);
        }
        return getNextFriday(currentExpiry, period);
    }
     ...
     */
    function getNextFriday(uint256 timestamp, uint period) internal pure returns (uint256) {
