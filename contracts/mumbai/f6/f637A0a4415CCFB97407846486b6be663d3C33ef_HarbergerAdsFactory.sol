...
contract HarbergerAdsFactory {
  ...
  function create(uint256 _totalSupply, uint256 _taxRate, uint256 _cooldownPeriod, IERC20 _currency, address _collector, string calldata _name, string calldata _symbol, string calldata _tokenURI) public {
    HarbergerAds harbergerAdsContract = new HarbergerAds(_totalSupply, _taxRate, _cooldownPeriod, _currency, _collector, _name, _symbol, _tokenURI);
{    emit CollectionCreated(harbergerAdsContract, _totalSupply, _taxRate, _cooldownPeriod, _currency, _collector);
}  }
}
