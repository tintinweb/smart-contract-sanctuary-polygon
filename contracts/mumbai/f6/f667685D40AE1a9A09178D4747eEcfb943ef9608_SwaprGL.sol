...
interface ISwaprFee {
    ...
    function getFinalAuctionFee(
        uint subjectAmount,
        address token
    ) external view returns (uint finalFee);
...
contract SwaprGL is BaseGovernanceWithUserUpgradable, ListingHelper {
    ...
    function depositNFTs(bytes calldata data) external {
        require(_verify(_msgSender(), data), "INV_SIG");
        (bytes memory message, bytes memory sig) = _decodeBytesToBytesBytes(
            data
        );
        (uint depositType, address lock, uint nftId) = abi.decode(
            message,
            (uint, address, uint)
        );
        require(depositType == 3, "INV_DEP_TYPE");
    ...
    function createListing(
        ...
        require(_verify(theMarketplace, data), "INV_SIG");
        ...
        (
            ...
            ,
            bytes memory sig
        ) = _validateListing(sender, listingType, listingData);
        require(isValid);
        require(
            _swaprFee.getFeePaid(sender, paymentToken) >= fee,
            "LOW_FEE_PAID"
        );
    ...
    /// @param listingType 1 equals Auction, 2 equals Order
    /// @param data contains the encoded string
    function updateListing(uint listingType, bytes calldata data) external {
        require(_verify(theMarketplace, data), "INV_SIG");
        address sender = _msgSender();
        (
            bytes memory activeListingData,
            bytes memory proposedListingData,

        ) = abi.decode(data, (bytes, bytes, bytes));
        require(
            _verify(sender, activeListingData) &&
                _verify(sender, proposedListingData),
            "SNDR_ISNT_SLR"
        );
        uint createdOn;
        if (listingType == 1) {
        ...
        (
            ...
            ,
            bytes memory sig
        ) = _validateListing(sender, listingType, proposedListingData);
        require(isValid);
    ...
    function claim(bytes calldata data) external {
        ...
        ) = _decodeBytesToBytesBytes(message);

        address claimant = _msgSender();
        (bool success, string memory res) = _validateClaim(
            claimant,
            listingData,
            lastBid
        );
        ...
        if (_verify(claimant, listingData)) {
            //for seller
            require(block.timestamp > bid.listingEndTime, "AUC_NOT_END");
    ...
    function withdrawFunds(bytes calldata data) external {
        require(_verify(theMarketplace, data), "INV_SIG");
        (bytes memory info, ) = _decodeBytesToBytesBytes(data);
        (address token, uint withdrawable, address receiver) = abi.decode(
            info,
    ...
    function buyNowOrder(
        ...
        require(
            swaprWallet.isNFTLocked(order.lock, order.nftId),
            "ORD_NOT_EXIST"
        );
        ...
        require(
            split > 0 &&
                split <= EXP &&
                split <= order.remainingPart &&
                split <= order.maxTokenSell,
            "INC_SPLIT_AMT"
        );
        ...
        require(_payUpfront(payOps, order.depositType, msg.value));
        ...
        if (split < order.remainingPart) {
            ...
            addresses[1] = buyer;
            uint[] memory newIDs = swaprWallet.splitReleaseNFT(
                order.lock,
                order.nftId,
                splitParts,
                addresses
    ...
    /// @dev you can check the deposits by getBalance() or else get approval for funds
    /// @param data must be Auction & Bid type provided with marketplace signature
    function buyNowAuction(bytes memory data) public payable {
        require(_verify(theMarketplace, data), "INV_SIG");
        (bytes memory message, ) = _decodeBytesToBytesBytes(data);
        (bool isActiveBid, bytes memory listingData, bytes memory lastBid) = abi
            .decode(message, (bool, bytes, bytes));

        Auction memory auction = _extractAuctionInfo(listingData);
        require(
            swaprWallet.isNFTLocked(auction.lock, auction.nftId),
            "AUC_NOT_EXST"
        );
        ...
        require(_payUpfront(payOps, auction.depositType, msg.value));
    ...
    function payNow(
        PayNow memory payOps,
        uint256 depositType
    ) public payable returns (bool paid) {
    ...
    function _payFromWallet(
        ...
        if (_payOps.acceptedToken == address(0)) {
            swaprWallet.swapNative(
                _payOps.from,
                _payOps.receiver,
                _payOps.amount
            );
            swaprWallet.releaseNative(
                feeReceiver,
                _payOps.receiver,
                finalListingFee
            );
    ...
    /// @return true if payment succeeds
    function _payUpfront(
        PayNow memory _payOps,
        uint256 depositType,
        uint value
    ) private returns (bool) {
        address receiver = _payOps.toEOA
            ? _payOps.receiver
            : address(swaprWallet);
        address feeReceiver = _swaprFee.getFeeReceiver();
        uint256 finalListingFee = depositType == 1
        ...
        if (_payOps.acceptedToken == address(0)) {
            ...
            if (_payOps.toEOA) {
                ...
                swaprWallet.depositNativeSwapr{
                    value: _payOps.amount - finalListingFee
                }(_payOps.receiver);
            }
        } else {
            ...
            require(
                paymentToken.allowance(_payOps.from, address(this)) >=
                    _payOps.amount,
                "INSF_ALWNC"
            );
    ...
    function _withdraw(address claimant, address lock, uint nftId) private {
        require(swaprWallet.isNFTLocked(lock, nftId), "ASST_NOT_AVAIL");
...
abstract contract ListingHelper {
    ...
    function validateBid(
        bytes calldata data
    ) external view returns (bool isValid, Bid memory validatedBid) {
        require(_verify(theMarketplace, data), "INV_SIG");
        (bytes memory message, ) = _decodeBytesToBytesBytes(data);
        (
            bool isActiveBid,
        ...
        if (!_verify(proposedBid.bidder, proposedBidData)) {
            return (isValid, validatedBid);
        }
        if (!isActiveBid) {
            (isValid, proposedBid.listingEndTime) = _validateAsFirstBid(
                listingData,
                proposedBid
            );
        } else {
            Bid memory activeBid = _extractBid(activeBidData);
            (isValid, proposedBid.listingEndTime) = _validateAsLastBid(
                message,
                activeBid,
                proposedBid
            );
        }
        uint bidderTotalBalance = swaprWallet.getBalance(
            proposedBid.bidder,
            auction.acceptedToken
        );
        if (
    ...
    function getSigner(
        ...
        return
            ECDSAUpgradeable.recover(
                ECDSAUpgradeable.toEthSignedMessageHash(dataHash),
                signature
            );
    }
        ...
        )
    {
        if (!_verify(sender, listingData)) {
            return (isValid, lock, nftId, paymentToken, sig);
        }
        ...
        if (listingType == 1) {
            //Auction
            (isValid, lock, nftId, paymentToken) = _validateAuctionCapability(
                listingData
            );
        } else if (listingType == 2) {
            //Order
            (isValid, lock, nftId, paymentToken) = _validateOrderCapability(
                listingData
            );
        }
    }
    ...
    function _validateClaim(
        ...
        if (
            !_verify(auction.seller, listingData) ||
            !_verify(lastBid.bidder, bidData) ||
            (block.timestamp < lastBid.listingEndTime)
        ) {
            res = "INV_DATA";
            return (false, res);
        }
        if (_verify(claimant, listingData)) {
            ...
            if (
                bidderTotalBalance - lastBid.lockedBalance >= lastBid.offerPrice
            ) {
                isValid = true;
            }
    ...
    function _extractAuctionInfo(
        bytes memory auctionData
    ) internal pure returns (Auction memory auction) {
    ...
    function _extractBid(
        bytes memory bidData
    ) internal pure returns (Bid memory bid) {
    ...
    function _verify(
        ...
        (
            ...
            bytes memory signature
        ) = _decodeBytesToBytesBytes(_data);
        return
    ...
    function _decodeBytesToBytesBytes(
        bytes memory data
    ) internal pure returns (bytes memory, bytes memory) {
        (bytes memory first, bytes memory second) = abi.decode(
            data,
            (bytes, bytes)
        );
        return (first, second);
    ...
    ) private view returns (bool isValid, uint proposedEndTime) {
        if (
            (activeBid.listingEndTime < auction.endTime) ||
            (activeBid.listingEndTime < block.timestamp)
        ) {
            return (isValid, proposedEndTime);
        }
        if (
            (proposedBid.offerPrice < auction.startingPrice) ||
            (activeBid.offerPrice >= proposedBid.offerPrice)
        ) {
            return (isValid, proposedEndTime);
        }
    ...
    function _getProposedEndTime(
        uint listingEndTime
    ) private view returns (uint proposedEndTime) {
        ...
        )
    {
        ...
        if (swaprWallet.isNFTLocked(auction.lock, auction.nftId)) {
            require(
                auction.activeDepositType == 0 ||
                    auction.activeDepositType == 3,
                "ALRD_LISTD"
            );
            isValid = true;
        } else {
            require(auction.depositType == 1, "INV_DEP_TYPE");
            require(auction.seller != address(0), "INV_SELLER");
            require(
        ...
        )
    {
        ...
        if (swaprWallet.isNFTLocked(order.lock, order.nftId)) {
            require(
                order.activeDepositType == 0 || order.activeDepositType == 3,
                "ALR_LISTED"
            );
            isValid = true;
        } else {
            require(order.depositType == 2, "INV_DEP_TYPE");
            require(order.seller != address(0), "INV_SELLER");
            require(order.fixedPrice != 0, "INV_PRICE");
            isValid = true;
        }
        lockAddr = order.lock;
        tokenId = order.nftId;
    ...
    function _validateAsFirstBid(
        ...
        if (
            !_verify(
                auction.seller,
                abi.encode(listingInfo, swaprWallet.getNFT(bid.lock, bid.nftId))
    ...
    function _validateAsLastBid(
        ...
        (, bytes memory listingData, bytes memory activeBidData, ) = abi.decode(
            message,
        ...
        require(
            _verify(
                ...
                abi.encode(
                    listingInfo,
                    swaprWallet.getNFT(activeBid.lock, activeBid.nftId)
                )
            ) && _verify(activeBid.bidder, activeBidData),
            "INV_SIG"
        );
        (isValid, proposedEndTime) = _bidsCrossValidate(
            auction,
            activeBid,
            proposedBid
        );
