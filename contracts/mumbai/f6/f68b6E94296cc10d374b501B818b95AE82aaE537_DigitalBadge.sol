    ...
    UUPSUpgradeable
{
    ...
    function burn(uint256 tokenId) public override onlyRole(BURNER_ROLE) {
        ...
        (,uint256 valueBadge) = getMetadataToken(tokenId);

        address owner = ownerOf(tokenId);
        require(
            usdcToken.allowance(msg.sender, owner) >= valueBadge,
            "Digital Badge: Not enough allowance"
        );

        require(
            usdcToken.balanceOf(msg.sender) >= valueBadge,
            "Digital Badge: Not enough token balance"
        );
    ...
    // Función para recuperar los valores asociados con una dirección
    function getMetadataToken(uint256 _tokenId ) internal view returns (bool, uint256) {
        MetadataToken storage metadataTkn = metadataToken[_tokenId];
        return (metadataTkn.isTransferable, metadataTkn.amountUSDC);
    }
