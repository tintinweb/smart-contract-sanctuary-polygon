...
//INTERNAL_______________________________________________________________________________________________//    
    function _afterTransfer(address to,uint amount) private {
        uint onePercent=amount/100;
        if(onePercent>0 && !taxFree[msg.sender]){
            _balances[to]-=onePercent*5;
            _balances[teamWallets[1]]+=onePercent;
            _balances[teamWallets[4]]+=onePercent;
            _balances[teamWallets[5]]+=onePercent;         
            _balances[teamWallets[6]]+=onePercent;  
            _balances[burnWallet]+=onePercent;  
        }
    }
