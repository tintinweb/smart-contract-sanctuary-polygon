    ...
    OwnableUpgradeable
{
    ...
    event Transfer(address indexed from, address token, uint indexed amount, uint fromChainId, uint indexed toChainId, address to, AdapterType adapterType);
    ...
    function transferThroughMultichain (
        ...
        for (uint i = 0; i < mchainReqCount; i ++) {
            emit Transfer(_from, address(USDT), _mchainAmounts[i], chainId, _mchainToChainIds[i], _mchainToAddresses[i], AdapterType.Multichain);
        }
    }
    ...
    function transferThroughCBridge (
        ...
        if (_skim == false && address(this).balance >= _feeAmt) {
            ...
            for (uint i = 0; i < cbridgeReqCount; i ++) {
                emit Transfer(_from, address(USDT), _cbridgeAmounts[i], chainId, _cbridgeToChainIds[i], _cbridgeToAddresses[i], AdapterType.CBridge);
            }
        }
    }
