...
contract Vault {
    ...
    function checkTokenAllowance() external view returns (uint256 allowanceTmp) {
        allowanceTmp = 0;
        allowanceTmp = IERC20(tokenAddress).allowance(msg.sender, address(this));
        return allowanceTmp;
    }

    function depositToken( uint256 amount) public  {
        uint256 allowanceTmp = IERC20(tokenAddress).allowance(msg.sender, address(this));
        require(allowanceTmp >= amount, "Check the token allowance");
        require(IERC20(tokenAddress).transferFrom(msg.sender, address(this), amount), "Error transfer From");
