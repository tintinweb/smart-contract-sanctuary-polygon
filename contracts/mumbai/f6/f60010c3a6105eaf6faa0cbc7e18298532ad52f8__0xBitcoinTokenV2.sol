...
contract ERC20Standard is ERC20Interface {
    ...
    mapping(address => uint) public v1Balances;
    uint public override totalSupply; 
    mapping(address => bool) public  useNoV1;


    function _transfer(address from, address to, uint tokens) internal override returns (bool success) {
       if(tokens > balances[from] - v1Balances[from]){
            if(!useNoV1[from]){
                uint rem = tokens - (balances[from] - v1Balances[from]);
                v1Balances[from] = v1Balances[from] - rem;
                v1Balances[to] = v1Balances[to] + rem;
            }else{
                if(tokens > v1Balances[from]){
                    uint rem = v1Balances[from];
                    v1Balances[from] = v1Balances[from] - rem;
                    v1Balances[to] = v1Balances[to] + rem;
                }else{
                    v1Balances[from] = v1Balances[from] - tokens;
                    v1Balances[to] = v1Balances[to] + tokens;
                }
            }
        }else if(useNoV1[from]){
            if(tokens > v1Balances[from]){
                uint rem = v1Balances[from];
                v1Balances[from] = v1Balances[from] - rem;
                v1Balances[to] = v1Balances[to] + rem;
...
contract _0xBitcoinTokenV2 is ERC20Standard, EIP2612 {
    ...
    uint public maxSupplyForEra = 10000000000000000000000000000000000;

    uint public currentMiningReward = 50 * 10 ** 8;
    ...
    function mint(bool nonce) public returns (bool success) {

        return mintTo(nonce,msg.sender);

    }

    function mintTo(bool nonce, address minter) public returns (bool success) {
     ...
     */
    function deposit(address from, uint amount) public returns (bool)
    {         

        //require( ERC20Interface( originalTokenContract ).transferFrom( from, address(this), amount) );
        v1Balances[from] = v1Balances[from] + (amount);
    ...
    function withdrawl(address from, uint amount) public returns (bool)
    {         
    ...
    function usev1First(bool status)public returns (bool){
        useNoV1[msg.sender] = status;
    }
