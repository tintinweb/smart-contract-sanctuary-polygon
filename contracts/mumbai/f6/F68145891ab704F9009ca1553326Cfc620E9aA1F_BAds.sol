...
contract BAds is Ownable{
    ...
    function _getAddressAccount(uint256 userId) internal view returns(address){
    ...
    function claimRewards(uint256 userId) external {
        require(_getAddressAccount(userId) == msg.sender, "Not Owner of account");
        if(msg.sender == users[userId].userAddress){
    ...
    function initDataCollection(uint256 userId) public dataCollectionInitiated {
        if (_userExists(userId) == true){
            require(_getAddressAccount(userId) == msg.sender, "Not Owner of account");
            if (msg.sender == users[userId].userAddress){
                uint256 start = collectionStarted[userId];
                if (start == 0){
    ...
    function deleteData(uint256 userId) public {
        require(_getAddressAccount(userId) == msg.sender, "Not Owner of account");
        collectionStarted[userId] = 0;
