...
contract ZaiStats is Ownable{
    ...
    function updateCounterWinLoss(uint256 _zaiId,uint256 _challengerId, uint256[30] memory _fightProgress) external onlyGame returns(bool){
        ...
        if(p2.zaiTotalFights > totalFightKing.totalScore){
            emit newTotalKing(_challengerId, totalFightKing.actualKing, block.timestamp - totalFightKing.kingSince);
            totalFightKing.totalScore = p2.zaiTotalFights;
            totalFightKing.kingSince = block.timestamp;
            totalFightKing.actualKing = _challengerId;
        }
        
        p2.zaiTotalRandomSelected += 1;

        if(p2.zaiTotalRandomSelected > randomKing.totalScore){
            emit newRandomKing(_challengerId, randomKing.actualKing, block.timestamp - randomKing.kingSince);
            randomKing.totalScore = p2.zaiTotalRandomSelected;
            randomKing.kingSince = block.timestamp;
            randomKing.actualKing = _challengerId;
        }
