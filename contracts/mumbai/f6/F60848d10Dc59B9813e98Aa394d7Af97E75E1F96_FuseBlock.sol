...
interface IItem {
    function mint(address _address, uint256 _amount, string memory _tokenURI) external;
}
...
contract FuseBlock is ERC721, Ownable {
    ...
    function mintItem(uint256 _fuseBlockId, uint256 _amount, string memory _tokenURI) external{
        require(itemAddress != address(0), "item NFT not set");
        require(ownerOf(_fuseBlockId) == msg.sender, "not token owner");
        require(_amount >= 0, "invalid aura amount");
        require(auraAmounts[_fuseBlockId] >= _amount, "insufficient balance");
        auraAmounts[_fuseBlockId] = auraAmounts[_fuseBlockId] - _amount;

        totalReleasedAmount = totalReleasedAmount - _amount;
        IERC20(auraAddress).transfer(itemAddress, _amount);
        IItem(itemAddress).mint(msg.sender, _amount, _tokenURI);
    }
