    ...
    UUPSUpgradeable
{
    ...
    /**
     * @notice Mint a new or update an existing ASBT for a specific buyer
     ...
     * @param _isUpdated Boolean variable used to update the total RENW funded by the buyer after batch transfer of SBTs
     */
    ...
    UUPSUpgradeable
{
    ...
    /**
     * @notice Fund multiple projects on different years
     * @dev SBTs/ASBTs hashes and token ids are generated before calling this function
     * @dev _paymentAddress should be equal to USDC contract address if user funded with USDC, otherwise
     * @dev should be zero address if user funded with MATIC
     *
     * @dev The SBTs/ASBTs hashes and the tokens IDs are generated before calling this function
     * @dev _paymentAddress should be equal to USDC contract address if the user funded with USDC, otherwise
     * @dev should be the zero address if the user funded with MATIC
     *
     * @param _paymentAddress The USDC contract address if the payment type is usdc, otherwise address(0)
     * @param _projectIDs The array with the projects IDs to be funded
     * @param _RENWFunded The array with the amounts to fund each project with; 1 RENW token = 1 MWh
     * @param _projectsVintage vintage years of the projects to be funded
     * @param _ipfsHashSBTs The SBTs metadata hashes stored on IPFS
     * @param _ipfsHashASBTs The ASBTs metadata hashes stored on IPFS
     * @param _ipfsSBTIds The SBTs tokens IDs to be assigned to each new minted SBT
     * @param _ipfsASBTIds The ASBTs tokens IDs to be assigned to each ASBT
     */
