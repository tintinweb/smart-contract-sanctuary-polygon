...
//public to check correct mapping
    ...
    event PromoCodeUsed(string value); //event to emit value in a function
    ...
    function mintTicketsWithFixPrice(
        ...
        require( eventDetails[ticketTypeDetails[ticketID].eventIDforTicket].eventEndDate >= block.timestamp,"Event Passed"  ); // = for check current time
        require(
            ...
            if (
                ...
                require(
                    promoCodeDetails[_promoCode].promoStartDate <
                        block.timestamp &&
                        promoCodeDetails[_promoCode].promoEndDate >= // 24 hours up = 
                        block.timestamp,
                    "NotValid"
                );
                require(
...
//emit to check the value of discount
                     ...
                     // variable change for msg.value equal
                        ...
                        require(newDiscountedAmount >= 0,"InvalidDiscountPriceOnFixedDiscount");
                               checkPrice(((newDiscountedAmount))); // here we can add checkprice direct function to find float value that's why error come 
                            promoCodeDetails[_promoCode].claimedUsers++;
                    }
 ...
 // updatePromoCode
   function updatePromoCodee ( uint64 eventIDs, string memory promoCode, PromoCodes[] memory _promoCodes ) external onlyOwner whenNotPaused{
