...
contract CallerPlus is Ownable {
    ...
    function execWithMerkleProof(address _Proxy, address _user, bytes32 _md5, bytes32[] calldata proof) external {
        require(canDoManualOrder, "e003");
        if (useWhiteList) {
            require(MerkleProof.verify(proof, merkleRoot, keccak256(abi.encodePacked(msg.sender))), "e004");
        }
        uint256 b0 = address(this).balance;
        Proxy(_Proxy).doManualOrder(_user, _md5);
        uint256 b1 = address(this).balance;
