...
contract lbtestkv2 is VRFV2WrapperConsumerBase,ConfirmedOwner,ReentrancyGuard {
        ...
        VRFV2WrapperConsumerBase(linkAddress, wrapperAddress) 
    {
        ...
        lottoWeekly = IluckblocksWeekly(0x64Aaf51A4534c9778AE2F383E14b2f4B3Cb59E91);
        weeklyContract = 0x64Aaf51A4534c9778AE2F383E14b2f4B3Cb59E91;
    ...
    function fillJackpot(uint256 amount) external {
        ...
        uint256 tprice = getLatestPrice(krstmTrading,usdTrading,1 * 10**18);
        tprice = tprice * 10**12;
        uint256 amountInToken = (minimumPot * 10**18) / tprice;  // 40 usd in token
    ...
    function swapTokensToUSD(uint256 tokenAmount, address user, uint typeSwap) private {
        ...
        if(typeSwap == 1){
         ...
         prizeValueJackpot = (krstm.balanceOf(address(this)) - contractBalance);

        } else if (typeSwap == 0){

         contractBalance = (krstm.balanceOf(address(this)) - prizeValueJackpot);
    ...
    function autoSpin(address _caller, uint256[] calldata _nodes) public nonReentrant {
        if(lbNodes.getUserActivation(_caller,0) == false){
            ...
            if(getJackpotinUSD() >= 200 * 10**18 && registeredsAccts.length >= 10){ 
