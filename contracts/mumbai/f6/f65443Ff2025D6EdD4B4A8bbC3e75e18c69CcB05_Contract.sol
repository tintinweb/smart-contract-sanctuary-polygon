...
contract Contract is ReentrancyGuard, ERC1155Holder {
    DropERC1155 public immutable capeCollection;
    TokenERC20 public immutable pvpToken;

      constructor(DropERC1155 capeAddress, TokenERC20 pvpTokenAddress){
        capeCollection = capeAddress;
        pvpToken = pvpTokenAddress;
      }
      ...
      mapping(address => MapValue) public playerCape;
    ...
    function stake(uint256 _tokenId) external  {
        require(capeCollection.balanceOf(msg.sender, _tokenId) >= 1, "You must have an NFT to steak");

        if(playerCape[msg.sender].isData){
            capeCollection.safeTransferFrom(address(this), msg.sender, playerCape[msg.sender].value, 1, "Returning cape");
        }
        uint256 reward = calculateP2WRewards(msg.sender);
        pvpToken.transfer(msg.sender, reward);

        capeCollection.safeTransferFrom(msg.sender, address(this), _tokenId, 1, "Staking your cape");

        playerCape[msg.sender].value = _tokenId;
        playerCape[msg.sender].isData = true;
    ...
    function withdraw() external nonReentrant {
        require(playerCape[msg.sender].isData, "You have no capes to withdraw");
        uint256 reward = calculateP2WRewards(msg.sender);
        pvpToken.transfer(msg.sender, reward);
        capeCollection.safeTransferFrom(address(this), msg.sender, playerCape[msg.sender].value, 1, "Returning old cape");
  ...
  function claim() external nonReentrant {
    ...
    function calculateP2WRewards(address _player) public view returns(uint256 _rewards){
        if(!playerLastUpdate[_player].isData || !playerCape[_player].isData){
            return 0;
        }

        uint256 timeDiff = block.timestamp - playerLastUpdate[_player].value;
        uint256 rewards = timeDiff * 10_000_000_000_000 * (playerCape[_player].value + 1);
        return rewards;
    }
