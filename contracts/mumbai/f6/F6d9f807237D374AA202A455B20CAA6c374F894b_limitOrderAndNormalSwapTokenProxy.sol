...
contract limitOrderAndNormalSwapTokenProxy is OpsTaskCreator, Ownable {
    ...
    function depositDevToken(uint256 _devAmount) external {
        (bool success,bytes memory result) = OrderAdminAddress.delegatecall(abi.encodeWithSelector(limitOrderAndNormalSwapToken.depositDevToken.selector, _devAmount));
        _getRevertMsg(success, result);
    }

    function withdrawDevToken(uint256 _devAmount) external {
        (bool success,bytes memory result) = OrderAdminAddress.delegatecall(abi.encodeWithSelector(limitOrderAndNormalSwapToken.withdrawDevToken.selector, _devAmount));
        _getRevertMsg(success, result);
    }
