...
contract DispatchStore is ERC721, Ownable, Pausable, ReentrancyGuard {
  // the fee as a percentage
  // 3 percent is the default
  uint256 public DISPATCH_FEE = 3;
   ...
   */
  function setFees(uint256 _dispatchFee) external onlyOwner {
   ...
   */
  function mint(uint256 _amount, uint256 _productId)
    external
    payable
    _isValidAmount(_amount, _productId)
    nonReentrant
    whenNotPaused
  {
    // check that the user sent enough ETH
    require(msg.value >= product[_productId].productPrice.mul(_amount), "Invalid amount paid.");
    ...
    if (product[_productId].productTokenGateAddress != address(0)) {
      if (product[_productId].productTokenGateTokenId > 0) {
        ...
        require(
          IERC721(product[_productId].productTokenGateAddress).balanceOf(msg.sender) > 0,
    ...
    emit SaleEvent(
      _productId,
      _amount,
      product[_productId].productPrice,
      product[_productId].productTotalSupply,
      product[_productId].productMaxSupply,
      product[_productId].productCollectionUri,
      product[_productId].productTokenGateAddress,
      product[_productId].productTokenGateTokenId
    );
  }
  ...
  /**
      ...
      @param _id of the asset
   */
  function burn(uint256 _id) public _isWhitelistedAddress whenNotPaused {
    _burn(_id);
  }
