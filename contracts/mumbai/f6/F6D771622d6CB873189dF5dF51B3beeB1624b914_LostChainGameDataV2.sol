...
contract LostChainGameDataV2 is CustomERC2771ContextUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event TESTADVENTURE(uint8[], uint8[], uint256[], uint256[], uint256, address);
    ...
    function adventure(
        ...
        require(LCT.getUserBalance(player) > landPrice, 'LCGD: Insufficient fund from player');
        ...
        emit TESTADVENTURE(rewardAmount, maxRewardAmount, enemyList, rewardList, landPrice, landOwner);
        LCT.gameDataTokenTransfer(msg.sender, _msgSender(), landOwner, landPrice);
    ...
    function checkAdventure(address user) public view returns (Adventure memory) {
