...
contract Deposito is Ownable{
    ...
    event ValorDepositado(address depositante, uint256 valor);
    event ValorRetirado(address dono, uint256 valor);

    mapping(address => uint256) public depositantes;

    function receber() public payable{
        if( msg.value < (1*decimal) ) revert ValorAbaixoDoPedido();
        depositantes[ msg.sender ] += msg.value;

        emit ValorDepositado(msg.sender, msg.value);
    }
    ...
    function retirada() public onlyOwner returns(bool){
        if(saldo()==0) revert SaldoZerado();

        payable( msg.sender ).transfer( saldo() );

        emit ValorRetirado(msg.sender, saldo() );
        return true;
    }
}