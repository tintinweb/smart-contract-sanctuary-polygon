...
contract RelationshipRegistry is SismoConnect {
  ...
  function createRelationship(CreateRelationshipParams memory params) public returns (address) {
    _checkSignatures(params.signatures, params.dataHash, params.parents);
    _validatezkProofs(params.zkProofs, params.groupIds, params.parents);

    return
      _createRelationship(params.implementation, params.parents[0], params.parents[1], params.salt, params.initData);
  }
  ...
  function _validatezkProofs(
    bytes[2] memory zkProofs,
    bytes16[2] memory groupIds,
    address[2] memory parents
  ) internal {
    for (uint256 i = 0; i < 2; i++) {
      require(_validatezkProof(zkProofs[i], groupIds[i], parents[i]), "Invalid zkProof");
    }
  }
  ...
  function _checkSignatures(
    ...
    for (uint256 i = 0; i < 2; i++) {
      require(_checkSignature(signatures[i], dataHash, parents[i]), "Invalid signature");
    }
  }
