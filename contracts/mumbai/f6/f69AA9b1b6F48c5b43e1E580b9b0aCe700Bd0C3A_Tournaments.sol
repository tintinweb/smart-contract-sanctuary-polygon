...
contract Tournaments is Initializable {
    ...
    {
        return (games[i].name, games[i].size, currNPlayer[i], gameOn[i], userIn[i][msg.sender]);
    }
    ...
    function joinGame(uint256 i) external {
        ...
        players[i][currNPlayer[i]++] = msg.sender;
    ...
    function endGame(uint256 i) external {
        ...
        gameOn[i] = false;
        for (uint256 j = 0; j < players[i].length; ) {
            userIn[i][players[i][j]] = false;
        }
        delete players[i];
    }
