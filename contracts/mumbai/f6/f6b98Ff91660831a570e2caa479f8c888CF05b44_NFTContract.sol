...
contract NFTContract is ERC721, INft, ERC165Storage, AccessControl {
    
    uint256 internal _tokenIds = 0;
    ...
    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        ...
        return string(abi.encodePacked(baseURI, toString(_tokenId), _suffix));
    }
    ...
    function mint(address _to) public onlyRole(EXECUTOR_ROLE) {
    ...
    function safeTransferFrom(address _from, address _to, uint256 _tokenId) public override(ERC721, IERC721){
        ICoreContract Dynamic = ICoreContract(coreContractAddress);
        Dynamic.transferNft(_from, _to, _tokenId);
        safeTransferFrom(_from, _to, _tokenId, "");
    }

    /// @inheritdoc IERC165
    function supportsInterface(bytes4 _interfaceId) public view override(ERC165Storage, IERC165, ERC721, AccessControl) returns (bool) {
        return ERC165Storage.supportsInterface(_interfaceId);
    }
...
interface ICoreContract {
    ...
    function mintGenesis(address to) external payable;
