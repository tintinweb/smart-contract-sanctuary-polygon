...
import "../structs/AgentDetail.sol";

contract AgentExtension is Initializable, Authority, IAgentExtension {
    using EnumerableSet for EnumerableSet.AddressSet;

    event Bind(address indexed user_, address indexed agent_);
    event Pay(address indexed user_, address indexed token_, uint256 amount_);

    mapping(address => address) _binds;
    EnumerableSet.AddressSet private _agents;
    mapping(address => AgentDetail) _agentDetails;
    mapping(address => mapping(address => uint256)) _userFundFlows;
    ...
    // ===> PRIVATE METHODS
    function _bind(address user_, address agent_) internal {
        require(agent_ != user_, "the agent is must different from the user");
        _binds[user_] = agent_;
        _agents.add(agent_);
        _agentDetails[agent_].users.add(user_);
        emit Bind(user_, agent_);
    }
    ...
    function _pay(
        ...
        _userFundFlows[user_][token_] += amount_;
        emit Pay(user_, token_, amount_);
    }

    // <=== PRIVATE METHODS

    // ===> PUBLIC METHODS
    function bind(address user_, address agent_)
        external
        override
    ...
    function agentCount(address agent_)
        ...
        returns (uint256)
    {
        return _agentDetails[agent_].users.length();
    }
    ...
    function userFundFlow(address user_, address token_)
        ...
        returns (uint256)
    {
        return _userFundFlows[user_][token_];
    }
    ...
    {
        uint256 total;
        for (uint256 i; i < _agentDetails[agent_].users.length(); i++) {
            total += _userFundFlows[_agentDetails[agent_].users.at(i)][token_];
        }
        return total;
    }
    // <=== PUBLIC METHODS
