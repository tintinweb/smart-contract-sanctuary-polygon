...
contract FootEarnBamiINO is Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    function buyNft(uint256 _nftType) public isRun validBuyTime nonReentrant{
        ...
        userBoughtNft[msg.sender][_nftType]++;
    ...
    function claimNft() public isRun validClaimTime nonReentrant{
        require(inWhiteList[msg.sender] == true, "You are ot in whitelist");
        require(isClaimed[msg.sender] == false, "You have claimed already");

        for(uint256 index = 0; index < nftTypeList.length; index++){
            address nft = nftTypeMap[nftTypeList[index]].nftAddress;

            require(IERC721(nft).balanceOf(address(this)) >= userBoughtNft[msg.sender][nftTypeList[index]], "Not enough NFT");

            for(uint256 jndex = 0; jndex < userBoughtNft[msg.sender][nftTypeList[index]]; jndex++){
                uint256 tokenId = ERC721Enumerable(nft).tokenOfOwnerByIndex(address(this), 0);

                IERC721(nft).transferFrom(address(this), msg.sender, tokenId);

                emit TransferComplete(msg.sender, nftTypeList[index], tokenId);
            }
        }
    ...
    function withdrawPaymentToken() external onlyOwner {
        ...
        paymentToken.safeTransfer(devWallet, withdrawAmount);
    }

    function nftWithdraw() external onlyOwner {
        for(uint256 index = 0; index < nftTypeList.length; index++){
            address nft = nftTypeMap[nftTypeList[index]].nftAddress;

            for(uint256 jndex = 0; jndex < ERC721(nft).balanceOf(address(this)); jndex++){
                uint256 tokenId = ERC721Enumerable(nft).tokenOfOwnerByIndex(address(this), 0);

                IERC721(nft).safeTransferFrom(address(this), devWallet, tokenId);
            }
        }
    }
