...
contract PrivateSaleV2 is AbstractSale, Pausable {
    ...
    modifier isWhiteList(address sender, address account) {
        if(!_whiteList[sender] && !_whiteList[account]) {
...
abstract contract AbstractSale is ReferralSystem, ISaleRound, AccessControl {
    ...
    {
        uint256 month = (block.timestamp - _startTime) / 30 minutes;
...
interface ISaleRound {
    ...
    /**
     * @dev emits when new currency is available for purchase
     */
    event AvailableCurrency(address token);

    /**
     * @dev emits when new parameters of the referral system is set
     */
    event PercentRewards(uint256[] percentReward);
    ...
    */
     ...
     * @dev burn unsold MGBs
     */
    function burnUnsoldToken() external;
    ...
    /**
     * @dev Add new account to referrer's list
     * @param account Address of the new account
     */
    function addToReferrerList(address account) external;

    /** 
     * @dev Set new available stable coin
     * @param token Address of the new stable coin
     */
    function setAvailableCurrency(address token) external;
    ...
    /**
     * @dev stop the private sale
     */
    function stopSale() external;

    /**
     * @dev resume the private sale
     */
...
contract ReferralSystem {
     ...
     */
    function _setSystemParameters(uint256[] memory percentReward)
        internal
    {
    ...
    function getDataRefSystem()
        external
        view
        returns (uint256[] memory percentReward)
    {
        return _percentReward;
    }
