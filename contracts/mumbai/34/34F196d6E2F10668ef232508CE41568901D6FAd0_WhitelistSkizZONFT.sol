...
contract WhitelistSkizZONFT is IERC721, Ownable {
     ...
     * Public minting.
     */
    function mint(uint256 numberOfNfts) external reentrancyGuard {
  
        require(publicDrop, "Drop not started.");
        require(numberOfNfts > 0, "numberOfNfts cannot be 0");
        require(numberOfNfts <= 1, "You can not mint more than 1 NFT!");
        require(totalSupply().add(numberOfNfts) <= TOKEN_LIMIT, "Exceeds TOKEN_LIMIT");
        require(_getOwnerNFTCount(msg.sender) <= 0,"Max 1 NFT per Wallet!");
            _mint(msg.sender); 
    }
