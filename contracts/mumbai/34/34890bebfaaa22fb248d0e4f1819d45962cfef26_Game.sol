...
contract Game is IGame, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable{
    ...
    function canShoot(address account) external override view returns (bool) {
        return !requestIdUsed[userRequestIds[account]];
    }

    function getAvailableMissile() external override view returns (uint256[] memory) {
        uint256 count;
...
pragma solidity ^0.8.0; 
 ...
 interface IGame {
    ...
    function getAvailableMissile() external view returns (uint256[] memory);
