...
interface IAdminFunctions {
   ...
   function getLiquidationPenalty() external view returns(uint256);
    ...
    function getPositionV2Contract() external view returns(address);
    
    function getPositionV3Contract() external view returns(address);

    function getPositionZKContract() external view returns(address);

    function getPosition1inchContract() external view returns(address);
...
interface IMargin {
    ...
    function ltmRatio(address user, address assetAddress, uint256 source) external returns (uint256);

    function setPositionAmount(address user, address assetAddress, uint256 marginUsed, uint256 loanUsed, uint256 source) external;

    function closePositionAmount(address user, address assetAddress, uint256 marginUsed, uint256 loanUsed, uint256 marginAmount, uint256 temp) external;
...
import "./utils/PositionV2Storage.sol";
// SPDX-License-Identifier: UNLICENSED

pragma solidity ^0.8.0;

contract PositionUniswapV2 is ReentrancyGuard, Ownable, PositionV2Storage {
    ...
    //uniswap v2
    function createPositionAndTradeV2(address tokenAddress,address targetToken, uint256 amount, address userAddress) public 
    returns(uint256,uint256, uint256,uint256, address[] memory){
       ...
       (uint256 availableMargin,
        uint256 loanAmount,
        ,
        ) = IMargin(IAdminFunctions(adminContract).getMarginContract()).getDepositAmounts(userAddress, tokenAddress);
        uint256 ltmRatio = IMargin(IAdminFunctions(adminContract).getMarginContract()).ltmRatio(userAddress, tokenAddress, 2);
        uint256 marginUsed = (precision * amount)/(ltmRatio + precision);
        uint256 loanUsed = (ltmRatio * amount)/(ltmRatio + precision);
        if(availableMargin < marginUsed) {
            loanUsed += marginUsed - availableMargin;
        }
        if(loanAmount < loanUsed) {
            loanUsed = loanAmount;
            amount =  amount - (loanUsed - loanAmount);
        }
        // Path for quickswap 
        ...
        require(IUniswapV2Factory(uniswapV2Router.factory()).getPair(
            path[0],
            path[1]
        ) != address(0), "Position: Invalid Pair");
        
        // Store trade result and traded price
        (uint256 result, uint256 tradePriceAfterTrade) = trade(amount, path, 1);

        return (marginUsed, loanUsed, result, tradePriceAfterTrade, path);
    }
    ...
    function closePositionV2(address asset, address targetAsset, uint256 targetAmount) public 
    returns(uint256, uint256) {
        ...
        path[0] = targetAsset;
        if(path[1] == address(0)) path[1] = uniswapV2Router.WETH();
        (uint256 _result, ) = trade(targetAmount, path, 2);
        ...
        if(asset != address(0) && asset != uniswapV2Router.WETH()) {
            require(IERC20(asset).balanceOf(address(this)) >= _result, "Position: No balance");
            IERC20(asset).transfer(IAdminFunctions(adminContract).getMarginFundsContract(), _result);
        }
        else {
        ...
        return (_result, price[1]);
    }


    function liquidateV2(address asset, uint256 targetAmount, address[] memory path) public 
     returns(uint256, uint256) {
        (uint256 _result, ) = trade(targetAmount, path, 2);
        //0.02%
        ...
        returns (uint256)
    {
     ...
     function getCurrentPriceV2(address[] memory path) public view returns (uint256) {
        // Since price should be in same asset
        // Calculate path again and get price
        uint256 decimals = IERC20Metadata(path[0]).decimals();
        // Get Price
        uint256[] memory price = uniswapV2Router.getAmountsOut(1 * 10**decimals, path);
