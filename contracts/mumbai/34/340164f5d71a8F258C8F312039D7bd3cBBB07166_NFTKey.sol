...
contract NFTKey is ERC721, ERC721URIStorage, EIP712, Ownable, ReentrancyGuard {
    ...
    constructor(string memory tokenName, string memory symbol, address payable recipientAddress, address minterAddress) ERC721(tokenName, symbol) EIP712(SIGNING_DOMAIN, SIGNING_VERSION)
    {
    ...
    function mint(NFTVoucher calldata voucher) external payable nonReentrant returns (uint256)
    {
        ...
        require(_verifyVoucher(voucher) == minter, "Permission denied");
        require(_msgSender() == minter || msg.value >= voucher.minPrice, "Insufficient funds");
        ...
        if(msg.value > 0){
            Address.sendValue(recipient, voucher.minPrice);
        }
