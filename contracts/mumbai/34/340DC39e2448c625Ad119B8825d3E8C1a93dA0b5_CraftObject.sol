...
contract CraftObject is BaseObject, ERC1155Supply {
    ...
    /*                               INITIALIZATION                               */
    /* -------------------------------------------------------------------------- */
    constructor(address payable _treasuryAddress, address _phiMapAddress, address _craftLogicAddress) ERC1155("") {
    ...
    modifier onlyCraftLogic() {
        require(msg.sender == craftLogicAddress, "Unauthorized");
        _;
    }
    ...
    /* -------------------------------------------------------------------------- */
    /// @dev get CraftLogic
    function getCraftLogic() external view returns (address) {
        return craftLogicAddress;
    }

    /// @dev set CraftLogic
    function setCraftLogic(address newCraftLogic) external onlyOwner {
        require(newCraftLogic != address(0), "cant set address 0");
        craftLogicAddress = newCraftLogic;
        emit SetCraftLogic(newCraftLogic);
    }
    ...
    // Overridden functions from BaseObject, now with added check for whether the token ID is locked
    function isLocked(uint256 tokenId) public view returns (bool) {
        return _isLocked[tokenId];
    }

    function lockTokenId(uint256 tokenId) public onlyOwner {
        _isLocked[tokenId] = true;
    }

    function setTokenURI(uint256 tokenId, string memory _uri) public override onlyOwner {
        require(!_isLocked[tokenId], "Token ID is locked");
        super.setTokenURI(tokenId, _uri);
    }

    function setSize(uint256 tokenId, Size memory _size) public override onlyOwner {
        require(!_isLocked[tokenId], "Token ID is locked");
        super.setSize(tokenId, _size);
    }

    function setExp(uint256 tokenId, uint256 _exp) public override onlyOwner {
        require(!_isLocked[tokenId], "Token ID is locked");
        super.setExp(tokenId, _exp);
    }
    ...
    function getObject(address to, uint256 tokenid, uint256 amount) external onlyCraftLogic nonReentrant {
        // check if the function caller is not an zero account address
        require(to != address(0), "to(0) is invalid");
        // check token is open for sale
        require(allObjects[tokenid].forSale, "not Open");
