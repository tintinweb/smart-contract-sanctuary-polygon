...
contract BIOrbit is ERC721, ERC721URIStorage {
	...
	function getProjectsNotOwned() public view returns (Project[] memory) {
		uint256 projectCount = projectIdCounter.current();
		Project[] memory notOwnedProjects = new Project[](projectCount);
		uint256 notOwnedProjectsCount = 0;

		for (uint256 i = 1; i <= projectCount; i++) {
			Project storage project = Projects[i];
			if (project.owner != msg.sender) {
				notOwnedProjects[notOwnedProjectsCount] = project;
