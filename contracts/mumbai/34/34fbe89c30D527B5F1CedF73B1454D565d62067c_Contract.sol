...
contract Contract is ERC721LazyMint {
    ...
    function _startTokenId(uint256 _index) internal view virtual returns (uint256) {
        return _index;
    }

    function _transferTokensOnClaim(address _reciever, uint256 _quantity)
        internal
        virtual
        override
        returns (uint256 startTokenId)
    {
        startTokenId = _startTokenId(2);
        _safeMint(_reciever, _quantity);
    }

    function claimNFT(address _reciever, uint256 _quantity)
        public
        payable
        virtual
    {
        ...
        uint256 startTokenId = _transferTokensOnClaim(_reciever, _quantity); // Mints tokens. Apply any state updates by overriding this function.
        emit TokensClaimed(msg.sender, _reciever, startTokenId, _quantity);
    }
}