...
contract IncentiveVault is AddressAccessor {
    uint16 public apy; // APY * 100 (e.g. 100 = 1%)
    ...
    function setAPY(uint16 value) external onlyRole(DEFAULT_ADMIN_ROLE) {
        apy = value;
    }
    ...
    function withdraw(uint256 amount) external onlyRole(CONTROLLER_ROLE) {
        ...
        require(_availableAmount() >= amount, "amount exceeds availability");
        _lastWithdrawal = block.timestamp / 1 days;
        IERC20(underlying).transfer(msg.sender, amount);
    }

    function _availableAmount() private view returns (uint256) {
        (address underlying, address exchange, address sUSDR) = abi.decode(
