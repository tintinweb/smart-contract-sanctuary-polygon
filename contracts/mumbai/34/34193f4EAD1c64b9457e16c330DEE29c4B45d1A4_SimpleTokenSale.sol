...
contract SimpleTokenSale {
    ...
    address payable private maticFunds = payable(0xC5A362Ce165e3517f900823f8390Db43fcf767Ef);
    ...
    function SimpleTokenPriceInMATIC() public view returns(int) {
        int maticPrice = getMATICPrice();
        return tokenPriceUSD / maticPrice;
    }

    function buyToken(uint256 _amount) public payable {
        int SimpleTokenPriceMATIC = SimpleTokenPriceInMATIC();
        
        // Check that the buyer sends enough MATIC
        require(int(msg.value) >= SimpleTokenPriceMATIC * int(_amount));
        // Check that the sale contract provides enough MATIC to make this transaction.
        require(token.balanceOf(address(this)) >= _amount);
        // Make the transaction inside of the require
        // transfer returns a boolean value.
        require(token.transfer(msg.sender, _amount));
        // Transfer the MATIC of the buyer to us
        maticFunds.transfer(msg.value);
