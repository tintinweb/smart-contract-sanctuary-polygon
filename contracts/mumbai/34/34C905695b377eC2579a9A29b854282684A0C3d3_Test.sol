...
contract Test{
    ...
    function getUserBoard(uint8 level) public view returns(address, address) {
                address boardUser=globalIndex[level][currentGlobalCount[level]];
                address boardMember=users[boardUser].boardMatrix[level].firstReferrals[0];
                return(boardUser, boardMember);
       }
