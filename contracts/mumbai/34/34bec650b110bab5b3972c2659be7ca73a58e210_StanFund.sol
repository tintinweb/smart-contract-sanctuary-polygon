...
contract StanFund is Initializable {
    ...
    function initialize(address _collectionNFT, address _tokenStan)
        public
        initializer
    {
        owner = msg.sender;
    ...
    function distributeToken(
        ...
        uint256 creatorAmount = (params.amount *
            infoCollection.ratioCreator *
            100) / 100000;
        uint256 creatorStan = (params.amount * infoCollection.ratioStan * 100) /
            100000;
        ...
        onlyAuthorizer
    {
        ...
        uint256 fee = (params.amount * params.fee * 100) / 100000;

        if (
            params.method != AuctionStructure.Method.BUY &&
            stanFund[params.buyer] < (params.amount + fee)
        ) revert InvalidBalance();

        distributeToken(params, fee);
    }
    ...
    function deposit(
        ...
        stanFund[_owner] += _amount;
        tokenStan.transferFrom(_owner, _stanWalletAddress, _amount);
    }
    ...
    function withdraw(
        ...
        uint256 stanFundAmount = stanFund[_owner];
        if (stanFundAmount == 0 || _amount > stanFundAmount)
            revert InvalidBalance();
        stanFund[_owner] -= _amount;
        tokenStan.transferFrom(_stanWalletAddress, _owner, _amount);
    }
}
...
contract StanToken is ERC20Upgradeable, OwnableUpgradeable {
    function initialize() public initializer {
        ERC20Upgradeable.__ERC20_init("Stan", "STC");
