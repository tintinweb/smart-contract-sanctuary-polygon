 ...
 */
contract Auction is Initializable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    event UpdateAuctionEndTime(address indexed nftAddress, uint256 indexed tokenId, uint256 endTime, bool isMultiToken);
    ...
    /// @notice ERC721 Address -> Token ID -> AuctionData Parameters
    mapping(address => mapping(uint256 => AuctionData)) public auctions;
    ...
    /// @notice globally and across all auctions, the amount by which a bid has to increase (percent value in *10^3) Ex: Assign 2.5% as 0.025*10^3 = 25;
    ...
    /// @notice Init for upgradable contracts
    /// @param _bidWithdrawalLockTime enter uint in minutes (5 = 5 minutes)
    /// @param _minBidIncrement enter uint in multiples of 10^3 (1% == 0.01 => 0.01*10^3 = 10)
    function delegatedConstructor(
        address _addressRegistry,
        uint256 _bidWithdrawalLockTime,
        uint256 _minBidIncrement
    ) public initializer {
        require(_minBidIncrement > 1, "Auction: _minBidIncrement should be greater than 1");
        addressRegistry = IAddressRegistry(_addressRegistry);
        minBidIncrement = _minBidIncrement;
        bidWithdrawalLockTime = _bidWithdrawalLockTime * 60;
        __Pausable_init();
    ...
    /**
     ...
     @param _minBidReserve Boolean value to check if it ha a minimum bid value
    ...
    function createAuction(
        ...
        require(
            ...
            "Auction: Not a valid NFT standard"
        );
        if (IERC165(_nftAddress).supportsInterface(INTERFACE_ID_ERC1155)) {
            require(
                IMultiToken(_nftAddress).creators(_tokenId) == msg.sender &&
                    IMultiToken(_nftAddress).isApprovedForAll(msg.sender, address(this)),
                "Auction: Not owner and or contract not approved"
            );
        } else if (IERC165(_nftAddress).supportsInterface(INTERFACE_ID_ERC721)) {
            require(
                INFT(_nftAddress).ownerOf(_tokenId) == msg.sender &&
                    INFT(_nftAddress).isApprovedForAll(msg.sender, address(this)),
                "Auction: Not owner and or contract not approved"
            );
        }
        require(
            _payToken == address(0) ||
                (addressRegistry.treasuryManagement() != address(0) &&
                    ITreasuryManagement(addressRegistry.treasuryManagement()).dataFeedSourceAddress(_payToken) !=
                    address(0)),
            "Auction: Invalid pay token"
        );
    ...
    function createAuctionAndBidWithVoucher(
        ...
        require(_isContract(msg.sender) == false, "Auction: No contracts permitted");
        if (_minBidReserve) {
            require(_bidAmount >= _reservePrice, "Auction: bid cannot be lower than reserve price");
        }
        // Ensure this contract is approved to move the token
        ...
        if (_voucher.isMultiToken) {
            ...
            _nftAddress = INFTFactory(addressRegistry.factory()).createNFTContractFromRequest(_requestID, _payToken, msg.sender);
    ...
    /**
     ...
     @param _tokenId Token ID of the item being auctioned
     */
    function withdrawBid(address _nftAddress, uint256 _tokenId) external whenNotPaused nonReentrant {
     ...
     */
    function cancelAuction(address _nftAddress, uint256 _tokenId) external nonReentrant whenNotPaused {
        // Check valid and not resulted
        AuctionData memory auction = auctions[_nftAddress][_tokenId];

        require(
            INFT(_nftAddress).ownerOf(_tokenId) == msg.sender && msg.sender == auction.owner,
    ...
    /**
     ...
     @param _nftAddress ERC 721 Address
     @param _tokenId Token ID of the item being auctioned
     @param _owner NFT owner's address
     */
    function resultAuction(
        ...
        AuctionData storage auction = auctions[_nftAddress][_tokenId];

        require(INFT(_nftAddress).ownerOf(_tokenId) == _owner && _owner == auction.owner, "sender must be item owner");
        ...
        // Ensure this contract is approved to move the token
        require(INFT(_nftAddress).isApprovedForAll(_owner, address(this)), "auction not approved");
        ...
        ).getRoyalty(_nftAddress, _tokenId, winningBid);

        uint256 total = 0;
        for (uint256 i = 0; i < recipients.length; i++) {
            total += amounts[i];
            IERC20Upgradeable(auction.payToken).safeTransferFrom(winner, recipients[i], amounts[i]);
        }
        require(total <= winningBid, "Royalty splitup shouldn't be greater than 99.99% of actual price");
        IERC20Upgradeable(auction.payToken).safeTransferFrom(_msgSender(), _owner, winningBid - total);

        // Transfer the token to the winner
        

        bool isMultiToken = IERC165(_nftAddress).supportsInterface(INTERFACE_ID_ERC1155);
        if(isMultiToken){
        IMultiToken(_nftAddress).safeTransferFrom(IMultiToken(_nftAddress).creators(_tokenId), winner, _tokenId, 1, "0x00");
        }
        else{
        INFT(_nftAddress).safeTransferFrom(INFT(_nftAddress).ownerOf(_tokenId), winner, _tokenId);
        }
    ...
    /**
     ...
     @param _tokenId Token ID of the NFT being auctioned
     */
    function cancelAuctionAdmin(address _nftAddress, uint256 _tokenId) external nonReentrant onlyAdmin {
        // Check valid and not resulted
        AuctionData memory auction = auctions[_nftAddress][_tokenId];
        // Check auction is real
        require(auction.endTime > 0, "no auction exists");
     ...
     */
    function updateBidWithdrawalLockTime(uint256 _bidWithdrawalLockTime) external onlyAdmin {
    ...
    function _createAuction(
        ...
        require(_endTimestamp > _getNow(), "invalid start time");
        ...
        // Setup the auction
        auctions[_nftAddress][_tokenId] = AuctionData({
            owner: msg.sender,
            payToken: _payToken,
            minBid: minimumBid,
    ...
    function _cancelAuction(address _nftAddress, uint256 _tokenId) private whenNotPaused {
        ...
        // Check if item is lazy minted (owner is marketplace contract)
        if (IERC165(_nftAddress).supportsInterface(INTERFACE_ID_ERC1155)) {
            // ERC1155 token
            if (IMultiToken(_nftAddress).balanceOf(address(this), _tokenId) > 0) {
                // burn all tokens held by the contract
                IMultiToken(_nftAddress).burn(
                    address(this),
                    _tokenId,
                    IMultiToken(_nftAddress).balanceOf(address(this), _tokenId)
                );
            }
        } else {
            // ERC721
            if (INFT(_nftAddress).balanceOf(address(this)) > 0) {
                // burn the related token id held by the contract
                INFT(_nftAddress).burn(_tokenId);
            }
        }

        emit AuctionCancelled(
            _nftAddress,
            auctions[_nftAddress][_tokenId].owner,
            _tokenId,
            IERC165(_nftAddress).supportsInterface(INTERFACE_ID_ERC1155),
            highestBid.bid
        );
    ...
    function _placeBid(
        ...
        require(auction.payToken != address(0), "Auction: ERC20 method used for FTM auction");

        if (auction.minBid == auction.reservePrice) {
            require(_bidAmount >= auction.reservePrice, "Auction: bid cannot be lower than reserve price");
        }

        // Ensure bid adheres to outbid increment and threshold
        HighestBid storage highestBid = highestBids[_nftAddress][_tokenId];
        uint256 minBidRequired = highestBid.bid + highestBid.bid.mul(minBidIncrement).div(10**3);

        require(_bidAmount >= minBidRequired, "Auction: failed to outbid highest bidder");
        ...
        if (auction.payToken != address(0)) {
            ...
            require(
                ...
                "Auction: insufficient balance or not approved"
            );
        }
     ...
     */
    function reclaimERC20(address _tokenContract) external onlyAdmin {
...
interface IMultiToken {
  ...
  function mint ( address _to, uint256 _supply, string memory _uri, address _ERC20Token, address _sender, address royaltyRecipient, uint256 basisPoints ) external;
  function mintFromRequest ( uint256 _requestId, address _ERC20token, address _sender) external returns (uint256);
  function name (  ) external view returns ( string memory );
  function owner (  ) external view returns ( address );
  function renounceOwnership (  ) external;
