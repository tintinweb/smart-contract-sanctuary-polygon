...
contract MemberCard is IMemberCard, ERC721, ERC2981, AccessControlEnumerable {
    ...
    uint _maxSupply
        ...
        _grantRole(PLATFORM, _platformAddress); //address or owner only pausing matters for this
        _grantRole(VAULT, _vaultAddress);
        _grantRole(OWNER, msg.sender);
        _setRoleAdmin(PLATFORM, PLATFORM);
        maxSupply = _maxSupply; 
        setBaseURI(_initBaseURI);
        setApprovalForAll(_marketPlace, true);
    }
    
    function setContractRoyalty (uint96 _newFee) public onlyRole(OWNER) {
        s_royaltyFee = _newFee;
    }
    ...
    function setPayoutFee (uint96  _fee) public onlyRole(OWNER) {
        s_payoutFee = _fee;
    }

    function burnCard(uint256 _tokenId) external override(IMemberCard) onlyRole(VAULT)
    {
    ...
    function mint(address _to) external override(IMemberCard) onlyRole(VAULT) payable {
    ...
    //Weighted For The Birthtime Token 
    ...
    function getPayoutFee () external view returns (uint96) {
        return s_payoutFee;
    }
...
contract MemberCardFactory is IMemberCardFactory {
    
    uint256 memberCardsCounter = 0;

    mapping(uint256 => MemberCard) public memberCards; 

    event memberCardCollectionCreated(uint256 id, MemberCard card);
    ...
    ) external override(IMemberCardFactory) returns (MemberCard) {
        MemberCard cardsAddress = new MemberCard( _name,  _symbol, _initBaseURI, _vaultAddress, _platformAddress, _marketPlace, _maxSupply);

        memberCards[memberCardsCounter] = cardsAddress;
        memberCardsCounter++;

        emit memberCardCollectionCreated(memberCardsCounter, cardsAddress);
        return cardsAddress;
        
    }

    function getCardCollections(uint256 id) external override(IMemberCardFactory) view returns (MemberCard) {
