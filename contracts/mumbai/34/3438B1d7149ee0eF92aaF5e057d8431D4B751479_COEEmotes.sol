  ...
  DefaultOperatorFilterer
{
  ...
  mapping(address => mapping(uint256 => uint256)) public addressMinted;
  ...
  constructor() ERC721("Aether Games Card Backs", "AGCB") {
    _royaltyAmount = 500;
  ...
  function airdrop(
    ...
    require(
      types[_type].currentSupply + _addresses.length <= types[_type].maxSupply,
      "Not enough supply left."
    );
    for (uint256 i = 0; i < _addresses.length; i++) {
      _intMint(_type, _addresses[i], _sb);
    }
  }
  ...
  function adminMint(
    ...
    require(types[_type].exists, "Type does not exist.");
    require(
      types[_type].currentSupply + _amount <= types[_type].maxSupply,
      "Not enough supply left."
    );

    for (uint256 i = 0; i < _amount; i++) {
      _intMint(_type, _to, _sb);
    }
  }

  function _intMint(uint256 _type, address _to, bool _sb) internal {
  ...
  function setTypes(
    ...
    require(_typeIds.length == _uris.length, "Arrays must be the same length");
    for (uint256 i = 0; i < _typeIds.length; i++) {
      _setType(_typeIds[i], _uris[i]);
    }
  }

  function _setType(uint256 _typeId, string memory _uri) private {
  ...
  function setItemsSb(
    ...
    require(_tokenIds.length == _sbs.length, "Arrays must be the same length");
    for (uint256 i = 0; i < _tokenIds.length; i++) {
      items[_tokenIds[i]].sb = _sbs[i];
    }
  }
  ...
  function tokenURI(
    ...
    return string(abi.encodePacked(types[items[tokenId].typeId].uri));
  }

  function totalSupply() public view returns (uint256) {
    return _tokenIdCounter.current() - burnCount - 1;
  }
  ...
  modifier onlyRole(bytes32 role) {
    require(roles[role][msg.sender], "Not authorized to CB.");
    _;
  }
