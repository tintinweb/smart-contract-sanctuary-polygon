...
contract Admeal is ERC721Enumerable, Ownable {
    ...
    uint256 public constant DISCOUNT_SUUPLY = 1000;

    // Price
    uint256 public MINT_PRICE = 0.1 ether;
    uint256 public MINT_PRICE_DISCOUNT = 0.05 ether;

    // Noodle NFT Collection Address
    address public Noodle_Collection = 0x129F3153E143A32CFb3FC0ca023375109491f435;

    // Merkle roots for whitelisted and partner whitelisted addresses
    bytes32 public whitelistedMerkleRoot;

    // Counter for discount sale
    uint256 discount_sale = 0;

    // Mapping for whitelisted users
    mapping(address => bool) whitelisetdMinted;
    ...
    function whitelistMint(bytes32[] calldata merkleProof) external {
        ...
        require(!whitelisetdMinted[msg.sender], "Already minted");
        require(balanceOf(msg.sender) == 0, "Only 1 NFT can be free minted");

        whitelisetdMinted[msg.sender] = true;
        uint256 tokenId = totalSupply() + 1;
        _safeMint(msg.sender, tokenId);
    }
