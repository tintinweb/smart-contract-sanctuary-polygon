 ...
 * applications.
 */
contract NTERC20 is Context, IERC20, IERC20Metadata {
...
contract SafeEnergy is Initializable, IERC20Upgradeable, IERC20MetadataUpgradeable, OwnableUpgradeable {
    uint8 constant public CWATT = 1;
    uint8 constant public XWATT = 2;
    uint8 constant public LWATT = 3;
    uint8 constant public NWATT = 4;
    uint8 constant public PWATT = 5;
    uint8 constant public SWATT = 6;
    uint8 constant public VWATT = 7;

    uint16 constant public MIN_BPS = 1;
    uint16 constant public MAX_BPS = 10000;
    ...
    error InvalidMultiplier(uint16 multiplier, uint16 minMultiplier, uint16 maxMultiplier);
    error TransfersDisabled();
    ...
    function addTokens() external onlyOwner {
        tokenAddresses[CWATT] = address(new Token("CWATT", "CWATT", address(this)));
        tokenAddresses[XWATT] = address(new Token("XWATT", "XWATT", address(this)));
        tokenAddresses[LWATT] = address(new Token("LWATT", "LWATT", address(this)));
        tokenAddresses[NWATT] = address(new Token("NWATT", "NWATT", address(this)));
        tokenAddresses[PWATT] = address(new Token("PWATT", "PWATT", address(this)));
        tokenAddresses[SWATT] = address(new Token("SWATT", "SWATT", address(this)));
        tokenAddresses[VWATT] = address(new Token("VWATT", "VWATT", address(this)));

        tokenMultipliers[CWATT] = MAX_BPS;
        tokenMultipliers[XWATT] = MAX_BPS;
        tokenMultipliers[LWATT] = MAX_BPS;
        tokenMultipliers[NWATT] = MAX_BPS;
        tokenMultipliers[PWATT] = MAX_BPS;
        tokenMultipliers[SWATT] = MAX_BPS;
        tokenMultipliers[VWATT] = MAX_BPS;
    }

    function addTokenMinter(uint8 id, address minter) external onlyOwner validTokenId(id) {
        tokenMinters[id][minter] = true;
    }

    function revokeTokenMinter(uint8 id, address minter) external onlyOwner validTokenId(id) {
        tokenMinters[id][minter] = false;
    }

    function setTokenMultiplier(uint8 id, uint16 multiplier) external onlyOwner validTokenId(id) {
    ...
    function mint(address account, uint8 id, uint256 amount) public onlyMinter(id) validTokenId(id) {
        if (amount < 1e18) {
        ...
        // Calculates the increase in watts by using the formula:
        // wattsIncrease = log10((amount + balance) / balance)
        uint256 wattsIncrease;
        uint16 multiplier = tokenMultipliers[id];
        if (balance > 0) {
            wattsIncrease = log10(ud(amount + balance).div(ud(balance))).intoUint256();
        } else {
            wattsIncrease = log10(ud(amount)).intoUint256();
        }

        wattsIncrease = wattsIncrease * multiplier / MAX_BPS;
        if (wattsIncrease > 0) {
            ...
            unchecked {
                ...
                _balances[account] += wattsIncrease;
            }
...
contract Token is NTERC20 {
    ...
    constructor(string memory _name, string memory _symbol, address _minter) NTERC20(_name, _symbol) {
        minter = _minter;
    }

    modifier onlyMinter() {
        if (msg.sender != minter) revert InvalidMinter(msg.sender);
        _;
    }
