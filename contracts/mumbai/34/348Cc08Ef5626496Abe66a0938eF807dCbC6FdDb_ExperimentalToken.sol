...
import "./IExperimentalToken.sol";
import "@openzeppelin/contracts-upgradeable/interfaces/IERC721Upgradeable.sol";

contract ExperimentalToken is Erc721V1, IExperimentalToken {
    IERC721Upgradeable private constant oldContract = IERC721Upgradeable(0xde79166F2Ac5578DEd3E960f77e34d86ae79CAb5);
    ...
    function initialize(
        ...
        // All tokens are already minted at deployment but no Transfer events
        // have to be sent as stated in the EIP721.
        // See: https://eips.ethereum.org/EIPS/eip-721#specification
        ERC721AStorage.layout()._currentIndex = 8889;
    }

    // =============================================================
    // Temporary implementation
    // -------------------
    // The functions below are known not to be gas gas-efficient.
    //
    // The goal is to match these requirements, ordered by priority:
    // 1) Being compliant with the EIP721
    // 2) Ensuring the lower costs possible for loyal holders
    //    (ideally no cost at all)
    // 3) Optimizing gas costs for the team during this crazy times
    //    for the Ethereum Mainnet
    // 4) Ensuring people will still be able to trade the tokens
    //    with a clear and reasonable user experience
    //
    // This intermediate step allow us to carry on with the planned
    // migration while also dealing with the insane gas costs we are
    // all facing lately.
    // Once gas will be back to reasonable prices we will be able to
    // upgrade the implementation so that any operation will be much
    // more efficient than before the migration.
    //
    // Briefly, the following functions will behave in a standard
    // way for any tokens that have been already transferred at
    // least once, while they will fallback to the old contract for
    // any tokens that have never been transferred through the new
    // contract before. (This causes an extra-gas consuption)
    // Transferring a token for the first time initializes it and
    // this means the ownership data will be stored on this contract
    // instead of the old one.
    // We have plans to initialize any uninitialized tokens in
    // batches as soon as gas prices get back to reasonable values.
    // =============================================================
    ...
    function _initializeTokenData(address _owner, uint256 _tokenId) private {
        uint256 packedOwnershipData;

        assembly {
            // Mask `owner` to the lower 160 bits, in case the upper bits somehow aren't clean.
            _owner := and(_owner, _BITMASK_ADDRESS)
            // `owner | (block.timestamp << _BITPOS_START_TIMESTAMP)`.
            packedOwnershipData := or(_owner, shl(_BITPOS_START_TIMESTAMP, timestamp()))
        }

        ERC721AStorage.layout()._packedOwnerships[_tokenId] = packedOwnershipData;

        // We can directly increment and decrement the balances.
        ++ERC721AStorage.layout()._packedAddressData[_owner]; // Updates: `balance += 1`.

        emit Transfer(address(0), _owner, _tokenId);
    }

    function balanceOf(address _owner) public view override returns (uint256) {
        uint256 upperLimit = 8889;
        uint256 result = 0;
        uint256 newBalanceOf = super.balanceOf(_owner);
        uint256 oldBalanceOf = oldContract.balanceOf(_owner);

        uint256 i = 0;
        for (;;) {
            if ((newBalanceOf == 0 && oldBalanceOf == 0) || upperLimit == ++i) break;

            if (newBalanceOf > 0 && _ownershipAt(i).addr == _owner) {
                result++;
                newBalanceOf--;

                if (oldBalanceOf > 0 && oldContract.ownerOf(i) == _owner) {
                    oldBalanceOf--;
                }
    ...
    function getApproved(uint256 _tokenId) public view virtual override returns (address) {
        ...
        if (ERC721AStorage.layout()._packedOwnerships[_tokenId] != 0) {
            approvedOperator = ERC721AUpgradeable.getApproved(_tokenId);
        }

        approvedOperator = oldContract.getApproved(_tokenId);

        _checkFilterOperator(approvedOperator);

        return approvedOperator;
    }

    function isApprovedForAll(address _owner, address _operator) public view virtual override returns (bool) {
        // Until this temporary implementation will be deprecated, approvals for
        // all tokens will be read from this contract as well as from the old
        // one in order to avoid the need of double approvals for users during
        // the transition.

        _checkFilterOperator(_operator);

        return ERC721AUpgradeable.isApprovedForAll(_owner, _operator) || oldContract.isApprovedForAll(_owner, _operator);
    }
    ...
    function transferFrom(
        ...
        if (oldContract.ownerOf(_tokenId) != _from) {
        ...
        if (_from != _msgSenderERC721A()) {
            if (_from != oldContract.getApproved(_tokenId)) {
                if (!oldContract.isApprovedForAll(_from, _msgSenderERC721A())) {
        ...
        // Clean approved operator for the token... (if any)
        ERC721AStorage.layout()._tokenApprovals[_tokenId].value = address(0);

        // Initialize the token...
        _initializeTokenData(_to, _tokenId);
    }

    // =============================================================
    // Off-chain Indexing Tools
    // ------------------------
    // Because of the temporary implementation explained in the
    // section above we have to adapt the implementation of these
    // function too.
    // This impacts the optimization considerably and results might
    // be partially inaccurate, but it allows us to use the same
    // interface during the transition.
    //
    // We decided to disable some functions completely as their
    // results would be pretty much useless until the migration has
    // been completed.
    // =============================================================

    error FeatureNotImplementedYet();

    function getAddressData(address /*_address*/) public pure returns (AddressData memory) {
        revert FeatureNotImplementedYet();
    }

    function getAddressesData(address[] memory /*_addresses*/) public pure returns (AddressData[] memory) {
        revert FeatureNotImplementedYet();
    }
    ...
    function getTokenData(uint256 _tokenId) public view returns (TokenData memory) {
        ...
        if (ERC721AStorage.layout()._packedOwnerships[_tokenId] != 0) {
            ...
            tokenData = TokenData(_tokenId, oldContract.ownerOf(_tokenId), 0);
        }
    ...
    function getTokensDataOfOwner(
        ...
        for (;;) {
            ...
            if (_endId == currentTokenId++) break;
        }
    ...
    AccessControlUpgradeable
{
    ...
    function transferFrom(address _from, address _to, uint256 _tokenId) public payable override virtual onlyAllowedOperator(_from) {
