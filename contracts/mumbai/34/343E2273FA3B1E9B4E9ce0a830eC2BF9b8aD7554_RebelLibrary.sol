...
library RebelLibrary {
    ...
    /**
        ...
        "attributes": [
            ...
            "acknowledgement_type": "Aqua Power", 
            "value": 40
            }, 
            {
            "display_type": "boost_percentage", 
            "acknowledgement_type": "Stamina Increase", 
            "value": 10
            }, 
            {
            "display_type": "number", 
            "acknowledgement_type": "Generation", 
            "value": 2
            }
        ]
    */
    ...
    function generateNFTMetadataAttribs(VolCert memory volDetail, mapping(uint8 => Acknowledgement) storage acknowledgementById) private view returns (bytes memory) {
        ...
        attributesJson = abi.encodePacked(attributesJson,
            '{',
                '"display_type": "date",',
                '"acknowledgement_type": "Last Updated On",',
        ...
        for (uint8 i = 0; i < volDetail._acknowledgementIds.length; i++) {            
            ...
            /*
                ...
                {
                    "acknowledgement_type": "Background",
                    "value": "Green"
                },
             */
            attributesJson = abi.encodePacked(attributesJson, 
                '{',
                    //ex. "acknowledgement_type": "Tasks Performed",
                    '"acknowledgement_type": "' , acknowledgementValue._title, '",',
                    '"value": "' , acknowledgementValue._uom, '"',
    ...
    function generateAcknowledgements(VolCert memory volDetail, mapping(uint8 => Acknowledgement) storage acknowledgementById) internal view returns (string memory) {
        ...
        for (uint8 i = 0; i < volDetail._acknowledgementIds.length; i++) {
            attributesJson = abi.encodePacked(attributesJson,
                '<text fill="#ffffff" font-size="20px" font-family="Goudy Old Style" x="80" y="', uint32(i*35 + 355).toString() ,'">&#9825; ', acknowledgementById[volDetail._acknowledgementIds[i]]._title ,' for ', volDetail._servedDurations[i] ,' ', acknowledgementById[volDetail._acknowledgementIds[i]]._uom ,'</text>'
            );
        }
