...
import './SoloFullMath.sol';
import './SoloFixedPoint96.sol';

/// @title Liquidity amount functions
/// @notice Provides functions for computing liquidity amounts from token amounts and prices
library SoloLiquidityAmounts {
    ...
    function getLiquidityForAmount0(
        ...
        uint256 intermediate = SoloFullMath.mulDiv(sqrtRatioAX96, sqrtRatioBX96, SoloFixedPoint96.Q96);
        return toUint128(SoloFullMath.mulDiv(amount0, intermediate, sqrtRatioBX96 - sqrtRatioAX96));
    }
    ...
    function getLiquidityForAmount1(
        ...
        if (sqrtRatioAX96 > sqrtRatioBX96) (sqrtRatioAX96, sqrtRatioBX96) = (sqrtRatioBX96, sqrtRatioAX96);
        return toUint128(SoloFullMath.mulDiv(amount1, SoloFixedPoint96.Q96, sqrtRatioBX96 - sqrtRatioAX96));
    }
    ...
    function getAmount0ForLiquidity(
        ...
        return
            SoloFullMath.mulDiv(
                uint256(liquidity) << SoloFixedPoint96.RESOLUTION,
    ...
    function getAmount1ForLiquidity(
        ...
        return SoloFullMath.mulDiv(liquidity, sqrtRatioBX96 - sqrtRatioAX96, SoloFixedPoint96.Q96);
    }
...
import './SoloFullMath.sol';
import './SoloTickMath.sol';
import '../../interfaces/IUniswapV3Pool.sol';

/// @title Oracle library
/// @notice Provides functions to integrate with V3 pool oracle
library SoloOracleLibrary {
        ...
        returns (int24 arithmeticMeanTick, uint128 harmonicMeanLiquidity)
    {
        ...
        // TODO Review this refactoring for unexpected math errors

        arithmeticMeanTick = int24(int56(uint56(tickCumulativesDelta) / uint56(secondsAgo)));
        // Always round to negative infinity
        if (tickCumulativesDelta < 0 && (uint56(tickCumulativesDelta) % secondsAgo != 0)) arithmeticMeanTick--;
    ...
    function getQuoteAtTick(
        ...
        uint160 sqrtRatioX96 = SoloTickMath.getSqrtRatioAtTick(tick);

        // Calculate quoteAmount with better precision if it doesn't overflow when multiplied by itself
        if (sqrtRatioX96 <= type(uint128).max) {
            uint256 ratioX192 = uint256(sqrtRatioX96) * sqrtRatioX96;
            quoteAmount = baseToken < quoteToken
                ? SoloFullMath.mulDiv(ratioX192, baseAmount, 1 << 192)
                : SoloFullMath.mulDiv(1 << 192, baseAmount, ratioX192);
        } else {
            uint256 ratioX128 = SoloFullMath.mulDiv(sqrtRatioX96, sqrtRatioX96, 1 << 64);
            quoteAmount = baseToken < quoteToken
                ? SoloFullMath.mulDiv(ratioX128, baseAmount, 1 << 128)
                : SoloFullMath.mulDiv(1 << 128, baseAmount, ratioX128);
        }
    }
    ...
    function getBlockStartingTickAndLiquidity(address pool) internal view returns (int24, uint128) {
        ...
        // TODO: review this rewrite for unexpected math errors

        tick = int24(uint24(uint32(uint24(int24((tickCumulative - prevTickCumulative))) / delta)));
        uint128 liquidity =
            uint128(
