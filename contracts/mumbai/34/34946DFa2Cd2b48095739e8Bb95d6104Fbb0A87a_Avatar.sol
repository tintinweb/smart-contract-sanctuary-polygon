    ...
    UUPSUpgradeable
{
    ...
    mapping(uint256 => string) public token_id_reference_id;
    ...
    modifier mintingValidation(uint256 count) {
    ...
    /**
     * @notice Allows an admin to update token URI related config variables.
     */
    ...
    /**
     * @notice Allows an admin to withdraw all the funds in this smart-contract.
     */
    ...
    function _mintAnElement(
        ...
        string memory reference_id
    ) internal {
        _tokenIdTracker.increment();
        uint256 tokenId = _tokenIdTracker.current();
        _safeMint(_toAddress, tokenId);
        token_id_reference_id[tokenId] = reference_id;
        emit AvatarMinted(
            tokenId,
            _toAddress,
            _byAddress,
    ...
    function mint(
        address _toAddress,
        string[] memory reference_ids,
        string memory mintingType
    ) external mintingValidation(reference_ids.length) onlyRole(MINTER_ROLE) {
        uint256 _count = reference_ids.length;
        require(
            compareStrings(mintingType, "public") ||
                compareStrings(mintingType, "reserved"),
            "Minting type not valid"
        );
        if (compareStrings(mintingType, "reserved")) {
            require(
    ...
    function remainingNumReservedTokens() public view returns (uint256) {
        return _RESERVED_NUM_TOKENS.sub(_reservedTokenCounter.current());
    }

    function remainingNumPublicTokens() public view returns (uint256) {
        return
            _MAX_NUM_TOKENS.sub(_RESERVED_NUM_TOKENS).sub(
                _tokenIdTracker.current().sub(_reservedTokenCounter.current())
            );
    }
    ...
    function adminUpdateReferenceId(uint256 tokenId, string memory referenceId)
        external
        onlyRole(ADMIN_ROLE)
    {
        require(_exists(tokenId), "query for nonexistent token");
        require(
            bytes(token_id_reference_id[tokenId]).length == 0,
            "Reference Id already present"
        );
        token_id_reference_id[tokenId] = referenceId;
    }
    ...
    function getTokenURIsOfOwner(address owner)
        external
        view
        returns (string[] memory)
    {
