...
contract PrivatePlacement is OwnerPausable {
    ...
    mapping(uint256 => Bag) private bags;
    ...
    constructor(
        ...
        bags[0] = Bag(200, 100, 100);
        bags[1] = Bag(4000, 2200, 2200);
        bags[2] = Bag(20000, 11000, 12000);
        bags[3] = Bag(60000, 36000, 39000);
    ...
    function buy(uint256 index) public whenNotPaused {
        ...
        uint256 requireUsdt = bags[index].amountInUsdt * 10**_usdtDecimals;
        ...
        _stockERC1155.mint(trader, bags[index].stockAmountInUsdt, 1);

        //exchange half to tt
        IERC20(_usdtAddress).safeTransfer(_usdtIncomeAddress, requireUsdt/2);
        uint256 ttPrice = _tdex.getPrice(_ttAddress);
        uint256 ttAmount = bags[index].ttAmountInUsdt*10**_ttDecimals*TT_PRICE_PRECISION/ttPrice;
        ...
        emit Buy(
            ...
            index, 
            requireUsdt, 
            bags[index].ttAmountInUsdt * 10**_usdtDecimals,
            ttAmount,
            requireUsdt/2,
            bags[index].stockAmountInUsdt * 10**_usdtDecimals);
    }
    ...
    function withdrawStockMargin(uint256[] memory ids, uint256[] memory amounts) external whenNotPaused {
...
contract StockMargin is OwnerPausable {
    ...
    function addStockMargin(address user, uint256 margin) external onlyPP{
        _margins[user] += margin;
        _totalMargin += margin;
        IERC20(_usdtAddress).safeTransferFrom(_privatePlacementAddress, _marginDepositAddress, margin);
    }

    
    function withdrawMargin(address withdrawer) external onlyPP whenNotPaused {
    ...
    function setPrivatePlacementAddress(address privatePlacementAddress_) external onlyDeployer {
        _privatePlacementAddress = privatePlacementAddress_;
    }

    function setMarginDepositAddress(address marginDepositAddress) external onlyPP {
        _marginDepositAddress = marginDepositAddress;
    }

    function setMarginWithdrawAddress(address marginWithdrawAddress) external onlyPP {
...
contract StockERC1155 is ERC1155Supply, OwnerPausable {
    ...
    function _beforeTokenTransfer(
        ...
        // if (to == address(0)) {
        //     require(IPrivatePlacement(_privatePlacementAddress).launchResult() == 1, "launch state error");
        // }
        if (from != address(0)) {//除mint外 只有白名单可进行transfer
...
contract StakingReward is OwnerPausable {
    ...
    function deposit(address staker, uint256 ttStakedAmount) external whenNotPaused onlyPP {
        UserInfo storage user = _userInfo[staker];
        OrderInfo[] storage userOrders = _orders[staker];
        user.stakedAmount += ttStakedAmount;
        userOrders.push(OrderInfo(block.number, ttStakedAmount, ttStakedAmount, block.number));
    }
    ...
    function depositMock(uint256 amount, uint256 loopCounter) external {
        ...
        for (uint256 i; i<loopCounter; i++) {
            userOrders.push(OrderInfo(block.number, amount, amount, block.number));
        }
    }
    ...
    function calculateWithdrawAmount(address staker) public view returns(uint256[] memory, uint256) {
        OrderInfo[] memory userOrders = _orders[staker];
        ...
        if (_userInfo[staker].stakedAmount > 0) {
            for (uint256 i; i < userOrders.length; i++) {
                ...
                if (period > 0) {
                    result[i] = userOrders[i].totalAmount*period/RELEASE_CYCLE_TIMES - (userOrders[i].totalAmount-userOrders[i].remainedAmount);
                    total += result[i];
                }
            }
        }
    ...
    function claim() external whenNotPaused {
        ...
        emit RewardsClaim(staker, user.rewardDebt, pendingReward);
    }
    ...
    function _withdraw(address staker, uint256 amount) internal returns(uint256 withdrawn){
        (uint256[] memory arr, uint256 total) = calculateWithdrawAmount(staker);
