...
interface AirGangCreatorInterface {
    ...
    */
    ...
    /**
     *@notice Get the name of an Air Gang contract by its address
     *@param airGangAddress The address of the Air Gang contract
     *@return The name associated with the Air Gang contract address
     */
    function airGangAddressToName(
        address airGangAddress
    ) external view returns (string memory);
    ...
    /**
     *@notice Get the data of an Air Gang by its name
     *@param airGangName The name of the Air Gang
     *@return The data of the Air Gang
     */
    function airGangNameToData(
        string memory airGangName
    ) external view returns (AirGangData memory);
...
contract AirGangSwap is ERC1155, ReentrancyGuard, IERC721Receiver {
    // Air Gang Creator contract.
    AirGangCreatorInterface public airGangCreatorContract =
        AirGangCreatorInterface(
            payable(0x92De7Ffa06a1f803a77dC9C092fD957eBAc58255)
        );

    string public name = "Air Gang Swap";
    string public symbol = "AGS";

    string public contractURI = "https://token.bilets.club/api/contract";

    string public baseURI = "https://token.bilets.club/api/token/";
    ...
    constructor() ERC1155("https://token.bilets.club/api/token/{id}") {}
    ...
    //     emit NFTDeposited(operator, from, tokenId, tokenId, data);
    ...
    function onERC721Received(
        ...
        string memory airGangName = airGangCreatorContract.airGangAddressToName(
            operator
        );

        AirGangCreatorInterface.AirGangData
            memory airGangData = airGangCreatorContract.airGangNameToData(
                airGangName
            );

        uint256 airGangID = airGangData.airGangID;

        _mint(from, airGangID, 1, data);

        emit NFTDeposited(operator, from, tokenId, airGangID, data);
        return IERC721Receiver.onERC721Received.selector;
    }
}