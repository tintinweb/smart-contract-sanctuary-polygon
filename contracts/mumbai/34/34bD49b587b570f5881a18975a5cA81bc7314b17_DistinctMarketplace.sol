    ...
    Whitelist
{
    ...
    function buyDropItems(
        ...
        for (uint256 i = 0; i < numberOftokenIds; i++) {
            ...
            if( paymentTokens[i] == address(WETH)) {
                totalInWETH += dropItem.priceInWETH * amounts[i] ;
            }
            if( paymentTokens[i] == address(USDT)) {
                totalInUSDT += dropItem.priceInUSDT * amounts[i];
            }
        }
    ...
    function joinWhitelist(uint256 saleId) external {
        require(reserveSale[saleId].status == SaleStatus.open, 'Distinct: Sale not available');
        _joinWhitelist(_msgSender(),saleId);
    }


    function _feeHandler(uint256 tokenID,uint256 price) internal view returns(address, uint256, uint256){
        (address receiver, uint256 fee) = DistinctNFT.royaltyInfo(tokenID, price);
        return (receiver, fee , (price - fee));
    }
