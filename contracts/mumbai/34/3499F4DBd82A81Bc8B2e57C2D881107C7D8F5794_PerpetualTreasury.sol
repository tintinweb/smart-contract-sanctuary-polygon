...
contract PerpetualTreasury is PerpetualBaseFunctions, IFunctionList, IPerpetualTreasury {
     ...
     */
    function removeLiquidity(uint8 _poolId, uint256 _shareAmount) external override nonReentrant {
        require(msg.sender == tx.origin, "direct call only");
        //inlines the modifier updateFundingAndPrices: updateFundingAndPrices(uint24(0), _poolId)
        uint24 iPerpetualId;
        uint24 iOtherPerpId;
        (iPerpetualId, iOtherPerpId) = _selectPerpetualIds(uint24(0), _poolId);
        _getUpdateLogic().updateFundingAndPricesBefore(iPerpetualId, iOtherPerpId);
        // main part
        int128 _fShareAmount = int256(_shareAmount).fromDec18();
        ...
        if (_shareAmount > shareToken.balanceOf(user)) {
            ...
            _fShareAmount = int256(_shareAmount).fromDec18();
        }

        int128 fTokenAmountToReturn = _getTokenAmountToReturn(pool, _fShareAmount);
        IShareToken(pool.shareTokenAddress).burn(user, _shareAmount);
        ...
        emit LiquidityRemoved(_poolId, user, fTokenAmountToReturn.toUDec18(), _shareAmount);
