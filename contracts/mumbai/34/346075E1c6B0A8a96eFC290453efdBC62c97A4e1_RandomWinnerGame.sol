   ...
   */
    ...
    */
    ...
    function finishGame() public onlyOwner {
        // Confirm there is a game already running
        require(gameStarted, "Game already finished");
        delete players;
        gameStarted = false;

        (bool sent,) = owner().call{value: address(this).balance}("");
        require(sent, "Failed to send Ether back to owner");
        // Emit that the game has ended
        bytes32 value = bytes32(0);
        emit GameEnded(gameId, owner() , value);
    }
    ...
    function printFeeAndEther() public payable {
        emit PlayerJoined(entryFee, 0x0000000000000000000000000000000000000000);
        emit PlayerJoined(entryFee * 10**18, 0x0000000000000000000000000000000000000001);
        emit PlayerJoined(10**18, 0x0000000000000000000000000000000000000002);
        emit PlayerJoined(msg.value, 0x0000000000000000000000000000000000000003);
    }

    function joinGameWith18Gap() public payable {
        // Check if a game is already running
        require(gameStarted, "Game has not been started yet");
        // Check if the value sent by the user matches the entryFee
        require(msg.value * 10**18 == entryFee, "Value sent is not equal to entryFee");
        // Check if there is still some space left in the game to add another player
        require(players.length < maxPlayers, "Game is full");
        // add the sender to the players list
        players.push(msg.sender);
