...
library TransferHelper {
    ...
    function safeApproveForAllNFT1155(
        ...
        require(
            success && (data.length == 0 || abi.decode(data, (bool))),
            "TransferHelper: APPROVE_NFT_FAILED"
        );
    }
    ...
    function safeTransferNFT1155(
        ...
        require(
            success && (data.length == 0 || abi.decode(data, (bool))),
            "TransferHelper: TRANSFER_NFT_FAILED"
        );
    }
...
//nextTokenIdToMint
// File: Library/ChallengeHelper.sol
...
library ChallengeHelper{
    ...
    function getIndexToken(address _erc721Address) private view returns(uint256) {
        if(ExerciseSupplementNFT(_erc721Address).nextTokenIdToMint() == 0) {
            return ExerciseSupplementNFT(_erc721Address).nextTokenIdToMint();
        } else {
            return ExerciseSupplementNFT(_erc721Address).nextTokenIdToMint() - 1;
        }
    }
}
...
contract ChallengeDetail is IERC721Receiver, IChallenge {
    ...
    */
    ...
    function giveUp() external canGiveUp notSelectGiveUp onTime available onlyStakeHolders {
        ...
        ChallengeHelper.transferNFTWhenFailed(erc721Address[0]);
    ...
    function transferToListReceiverSuccess() private {      
        ...
        if(allowGiveUp[2]) {
            indexNft = ExerciseSupplementNFT(erc721Address[0]).nextTokenIdToMint().sub(1);
    ...
    function getIndexToken(address _erc721Address) private view returns(uint256) {
        return ExerciseSupplementNFT(_erc721Address).nextTokenIdToMint().sub(1);
    }
...
contract HistoryBalanceChallenges{
    ...
    function getData(address payable _contractChallengeAddress) public view returns(address[] memory) {
        return ChallengeDetail(_contractChallengeAddress).allContractERC20();
    }
