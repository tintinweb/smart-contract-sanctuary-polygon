...
contract BetsFuzzedUTBETS is UltibetsCore {
    ...
    function placeBet(
        ...
        UTBETSContract.transferFrom(msg.sender, address(this), _predictionAmount);
        totalBetNumber += 1;
        ...
        betDataByBettor[msg.sender][_eventID][_eventValue] = totalBetNumber;
        dailyBetsByBettor[msg.sender][eventList[_eventID].eDate].push(totalBetNumber);
        betAmountsPerSideByEvent[_eventID][_eventValue] += _predictionAmount;
        eventList[_eventID].bettingVolume += _predictionAmount;

        totalBetAmountByBettor[msg.sender] += _predictionAmount;
