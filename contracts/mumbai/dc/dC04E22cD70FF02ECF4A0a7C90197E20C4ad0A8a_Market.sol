...
contract Market is IMarket, Context {
   ...
   */
  mapping(uint => address) public auctionWinners;

  /**
   * @notice List of approved offers
   */
  mapping(uint => bool) public approvedOffers;
  ...
  modifier onlyApprovedOffers(uint256 offerId_) {
    require((approvedOffers[offerId_]) || (rolesContract.isVerifiedUser(offersList[offerId_].seller)), 'M101');
    _;
  }
   ...
   */
  modifier onlyAuctions(uint offerId_) {
   ...
   * @param endTime_ indicate the time when the offer will be end (just if is auction)
   ...
   * @param paymentToken_ This token has be a valid ERC20 address
   */
  ...
  /**
   * @notice function to validate the ownership of the tokens when creating an offer
   ...
   */
  function _validateTokensOwnership(uint256[] memory tokenIds_, address[] memory collectionAddresses_) internal view returns (bool flag) {
    for (uint i; i < collectionAddresses_.length; i++) {
      _validateOwnership(collectionAddresses_[i], tokenIds_[i], _msgSender());
      // Declare a flag if this address is 
      // NOT in the approved ERC721 mapping 
      if (!(approvedERC721[collectionAddresses_[i]] && flag)) 
        flag = true;
    }
  }

  /**
   * @notice Function that checks the ownership of a single ERC721 tokenñ
   * @param collectionAddress Contract address 
   * @param tokenId ...
   * @param owner Address of the owner
   */
  function _validateOwnership(address collectionAddress, uint tokenId, address owner) internal view {
    require(IERC721(collectionAddress).ownerOf(tokenId) == owner, 'E413');
    require(IERC721(collectionAddress).isApprovedForAll(owner, address(this)), 'E407');
  }
  ...
  function createOffer(
    ...
    bool notApproved = _validateTokensOwnership(tokenIds_, collectionAddresses_);
  ...
  */
  ...
  function _transactAllTokens(uint256 offerId_) internal  {
    Offer memory offer = offersList[offerId_];
    uint256[] memory tokenIds = getDecodedTokenIds(offerId_);
    
    // This loop will cicle less or equal to 5 times 
    for (uint i = 0; i < offer.collectionAddresses.length; i++) {
      _validateOwnership(offer.collectionAddresses[i], tokenIds[i], offer.seller);
      // Finally, transfer the token [i]
      IERC721(offer.collectionAddresses[i]).safeTransferFrom(offer.seller, _msgSender(), tokenIds[i], '');
    }
  }
  ...
  function _validateUserBalance(uint256 offerId_, uint256 value_) internal view {
    uint balance = _getActualBalance(_msgSender(), offersList[offerId_].paymentToken);
   ...
   * @param isAuction_ 0: direct-sale, 1: auction
   ...
   */
  function _setInactive(uint offerId_) internal { 
   ...
   */
  function _setMinBid(uint offerId_, uint min) internal {
   ...
   */
  function _getActualBalance(address who, address paymentToken) internal view returns (uint balance) {
  ...
  function setWinner(uint offerId_, address bidder_) public onlyModerator {
    ...
    // Finally, set {bidder_} as winner with {valueBidded_}
    ...
    (uint winnersBalance, uint bidderBalance) = 
      (
        _getActualBalance(auctionWinners[offerId_], offersList[offerId_].paymentToken),
        _getActualBalance(bidder_, offersList[offerId_].paymentToken)
      );
