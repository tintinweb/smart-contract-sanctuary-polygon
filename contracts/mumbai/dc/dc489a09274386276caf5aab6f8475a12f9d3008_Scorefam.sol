    ...
    UUPSUpgradeable
{
    ...
    struct Game {
        ...
        uint256 wpy; //Weekly Reward Category
     ...
     */
    function getHalvedReward(uint256 _wpy) internal view returns (uint256) {
        uint256 reward;
        ...
        returns (uint256)
    {
        ...
        uint256 wpy = _gaming[_msgSender()][game_].wpy;
        ...
        require(gameType == 3);
            ...
            if(isGameResultResolved(_gaming[_msgSender()][game_].fixtureID)){
                ...
                if ( 
                    ...
                    (outcomeType == 3 && (homeScore < awayScore)) ) && (wpy == 1) ) 
                {
                    result = (getHalvedReward(wpy).mul(amount).div(100)).add(amount);
                } 
                ...
                //Bronze
                    ...
                    ( (outcomeType == 1 && (homeScore > awayScore)) || 
                    (outcomeType == 2 && (homeScore == awayScore)) ||
                    (outcomeType == 3 && (homeScore < awayScore)) ) && (wpy == 2) ) 
                {
                    result = (getHalvedReward(wpy).mul(amount).div(100)).add(amount);
                }
                ...
                //Gold
                    ...
                    ( (outcomeType == 1 && (homeScore > awayScore)) || 
                    (outcomeType == 2 && (homeScore == awayScore)) ||
                    (outcomeType == 3 && (homeScore < awayScore)) ) && (wpy == 3) ) 
                {
                    result = (getHalvedReward(wpy).mul(amount).div(100)).add(amount);
                }
                ...
                //VIP
                    ...
                    ( (outcomeType == 1 && (homeScore > awayScore)) || 
                    (outcomeType == 2 && (homeScore == awayScore)) ||
                    (outcomeType == 3 && (homeScore < awayScore)) ) && (wpy == 4) ) 
                {
                    result = (getHalvedReward(wpy).mul(amount).div(100)).add(amount);
                }
    ...
    function playGame(
        ...
        emit PlayedGame(_msgSender(), amount_, wpy_, fixtureID_, outcomeType_);
    ...
    function playSlip(
        ...
        if(gameType_ == 1){
            ...
            for(uint256 i = 0; i < _slipCuts.length; i++){
                ...
                _gameTypeOptions[gameType_].gameTypeStakedPool += amount_;
                emit PlayedSlip(_msgSender(), amount_, gameType_, accOddPoint_);
        } 
    ...
    function updateGameResult(uint256 _leagueId, uint256 _date) internal {
        ...
        bytes32 reqId = sc.requestId(_leagueId,_date);
        for(uint8 i = 0; i < sc.getRequestIdGamesLength(reqId); i++){
            gameResult[sc.getGameResolve(reqId,i).gameId] = GameResult(bytes(sc.getGameResolve(reqId,i).status), sc.getGameResolve(reqId,i).homeScore, sc.getGameResolve(reqId,i).awayScore, true);
        }
    }
    ...
    function claimGameReward(uint256 game_, uint256 _leagueId, uint256 _date) external returns (bool){
        ...
        emit Claimed(_msgSender(), amount_, 3);
    ...
    //FOR SLIPS
    function claimSlipReward(uint256 game_, uint256[2][] memory _leagueDate) external returns (bool){
        require(_gamingSlip[_msgSender()][game_].gameActive);

        for(uint256 i = 0; i < _leagueDate.length; i++){
            if(!isGameResultResolved(_gamingSlip[_msgSender()][game_].slips[i].fixtureID)){
               updateGameResult(_leagueDate[i][0], _leagueDate[i][1]); 
            } else {
                continue;
            }
        ...
        emit Claimed(_msgSender(), amount_, _gamingSlip[_msgSender()][game_].gameType);
