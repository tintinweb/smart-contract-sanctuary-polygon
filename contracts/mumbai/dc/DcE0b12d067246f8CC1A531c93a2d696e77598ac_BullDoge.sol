...
contract ERC20Stakeable is ERC20, ERC20Burnable, ReentrancyGuard {
    ...
    struct Staker {
        ...
        // Parent address for referral purpose
        address referrer;
    }
    ...
    uint256 public referralRate = 5; // In Percent
    ...
    mapping(address => uint256) private referralEarningsTracking;
    ...
    function deposit(uint256 _amount, address referrer) external nonReentrant {
        ...
        if (stakers[msg.sender].referrer == address(0)) {
            stakers[msg.sender].referrer = referrer;
        }
    ...
    function claimRewards() external nonReentrant {
        ...
        if (referrer != address(0)) {
            uint256 refEarnings = calculateReferralEarnings(rewards);
            referralEarningsTracking[referrer] += refEarnings;
            _mint(stakers[msg.sender].referrer, refEarnings);
        }
    }
    ...
    function withdrawAll() external nonReentrant {
        ...
        address referrer = stakers[msg.sender].referrer;
        if (referrer != address(0)) {
            uint256 refEarnings = calculateReferralEarnings(_rewards);
            referralEarningsTracking[referrer] += refEarnings;
            _mint(stakers[msg.sender].referrer, refEarnings);
        }
    ...
    function calculateReferralEarnings(uint256 withdrawValue) internal view returns (uint256 referralEarnings) {
        return withdrawValue * (referralRate/100);
    }

    function getMyReferralEarnings() public view returns (uint256 earnings) {
        return referralEarningsTracking[msg.sender];
    }
}
// File: contracts/BullDoge.sol
...
contract BullDoge is ERC20Stakeable, Ownable {
    constructor()
        ERC20Stakeable("BullDoge",  "BULL")
    {
