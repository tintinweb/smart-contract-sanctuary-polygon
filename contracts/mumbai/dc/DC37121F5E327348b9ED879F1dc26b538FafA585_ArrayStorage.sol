...
import {PlonkVerifier} from "../ZK--Circom-Circuit-Generator/verifier.sol";
...
contract ArrayStorage {
    ...
    mapping(address => Proof) public proofMap;

    function saveProof(uint256[24] calldata proof1, uint256[1] calldata proof2, uint256 circuitid, address account) public {
        Proof storage newProof = proofMap[account];
        newProof.circuitid = circuitid;
        PlonkVerifier verifier = new PlonkVerifier();
        if (verifier.verifyProof(proof1, proof2) == true){
            newProof.active = true;
        }else{
            newProof.active = false;
        }
    }
}
