...
contract MutualFriendsNFT is ERC721Royalty, Ownable, DefaultOperatorFilterer {
    ...
    function safeMint(uint8 quantity) public payable {
        ...
        require(
            isFriendsSaleActive || isPublicSaleActive || isPreSaleActive,
            "Sale is not active wait for announcement"
        );

        require(msg.value == mintingPrice * quantity, "Not enough ETH sent; check price and quantity!");
        ...
        if (isFriendsSaleActive) {
            require(friendsSaleEligible, "User not eligible in Friends Sale or already minted allowed amount in pre sale");
        ...
        if (isPreSaleActive) {
            require(preSaleEligible, "User not eligible in Pre Sale or already minted allowed amount in pre sale");
    ...
    function giveawayNFT(address receiver, uint8 quantity) public onlyOwner {
        require(
    ...
    function getRefund() public {
        ...
        require(success, "Unabled to send refund");
    }

    // VIEWS

    function availableForPublicMint() public view returns (uint256) {
        return maxSupply - teamSupply + Counters.current(_teamMintedCount);
    }
