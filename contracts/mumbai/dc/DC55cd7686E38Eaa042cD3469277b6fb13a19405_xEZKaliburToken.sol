...
contract xEZKaliburToken is ERC20('X Mad Meerkat Finance', 'xMMF') {
    using EnumerableSet for EnumerableSet.AddressSet;
    EnumerableSet.AddressSet private _minters;
    xEZKaliburToken private MMF;
    using SafeMath for uint256;
    ...
    constructor(xEZKaliburToken _mmf) public {
        MMF = _mmf;
    }
    ...
    function lock(uint256 _amount) public {
        require(MMF.transferFrom(msg.sender, address(this), _amount));
    ...
    function finalizeRedeem(uint256 index) public {
       require(index < userLock[msg.sender].length,"index not valid");
        ...
        _redeem(0, info.rewardAmount);

        uint256 MMFresidual = info.amount.sub(info.rewardAmount);

        if(MMFresidual > 0) {
            MMF.approve(address(this), MMFresidual);
            MMF.burnFrom(address(this), MMFresidual);
        }
    ...
    function getRedeemRatio(uint256 _days) public view returns(uint256) {
        ...
        return minRedeemRatio.add((_days.sub(minRedeemDays)).mul(maxRedeemRatio.sub(minRedeemRatio)).div(maxRedeemDays.sub(minRedeemDays)));
    }
    ...
    function setMillisecInDay(uint256 millisec) public onlyMinter {
        secInDay = millisec;
    }

    function redeem(uint256 _amount) public onlyMinter {
        require(MMF.balanceOf(address(this)) >= _amount);
        _redeem(_amount, _amount);
    }

    function _redeem(uint256 _burnAmount, uint256 _rewardAmount) internal {
        MMF.transfer(msg.sender, _rewardAmount);
        if(_burnAmount > 0)
    ...
    function addMinter(address _addMinter) public onlyOwner returns (bool) {
        require(_addMinter != address(0), "xMMF: _addMinter is the zero address");
        return EnumerableSet.add(_minters, _addMinter);
    }

    function delMinter(address _delMinter) public onlyOwner returns (bool) {
        require(_delMinter != address(0), "xMMF: _delMinter is the zero address");
    ...
    function getMinter(uint256 _index) public view onlyOwner returns (address){
        require(_index <= getMinterLength() - 1, "xMMF: index out of bounds");
