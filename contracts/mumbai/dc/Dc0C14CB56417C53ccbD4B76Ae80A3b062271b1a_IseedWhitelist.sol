...
contract IseedWhitelist is Ownable, ERC1155 {
    ...
    constructor (
        ...
        for(uint256 i = 0; i < length; i++) {
            authorizedUsers[usersWithPreordainedItem[i]] = true;
        }
    ...
    function burnWhitelistForIseed (uint256 _amount, address _user) external returns(bool) {
    ...
    function withdraw (address _firstLiquidityPool, uint256 _percentageForFirstLiquidityPool, address _secondLiquidityPool) public onlyOwner {
        if(_percentageForFirstLiquidityPool > 0) {
            LiquidityPoolInterface firstLiquidityPoolInstance = LiquidityPoolInterface(_firstLiquidityPool);
            bool sentoToFirst = firstLiquidityPoolInstance.transferBalance{value: (address(this).balance * (_percentageForFirstLiquidityPool / 100))}();
            require(sentoToFirst);
        }

        (bool sentToSecond, ) = _secondLiquidityPool.call{value: address(this).balance}("");
        require(sentToSecond);
    }
}