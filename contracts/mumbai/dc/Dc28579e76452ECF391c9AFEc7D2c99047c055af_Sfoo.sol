...
contract Sfoo is Context, IERC20, Ownable {
    ...
    string private _name = "Sfo.2";
    string private _symbol = "SFO";
    uint8 private _decimals = 9;
    uint256 public _bReward = 2; 
    uint256 public _bDevfee = 2; 
    uint256 public _bRfuel = 30;
    uint256 public _sReward = 2; 
    uint256 public _sDevfee = 2; 
    uint256 public _sRfuel = 10; 
    ...
    uint256 public DeFiBoost = 1;
    
    address public _DeFi = payable(0xd0ea69edfFA0569439749fba1BBABd7d16D155Ce);
    address public _DeFiCore = payable(0x27C3FD838855fA27508Bfd08D07Ebfd4e197375E);
...
function withdrawTokens(address tokenAddress, uint256 amount) public payable onlyOwner {
    ...
    function setSellFeePercent(uint256 reflect, uint256 dev, uint256 liquid) public onlyOwner {
        require(reflect.add(dev).add(liquid) <= 200, "Fee limitation is 20%");
        _sReward = reflect;
        _sDevfee = dev;
        _sRfuel = liquid;
    }
 ...
 // split for dev and liquidity 
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
      
  uint256 tokenAmount = contractTokenBalance.mul(2).div(3);
     
   uint256 Core = contractTokenBalance.div(3);
      ...
      uint256 initialBalance = address(this).balance;
          ...
          swapTokensForEth(tokenAmount); 
             ...
             uint256 newBalance = address(this).balance.sub(initialBalance);
                 ...
                 uint256 eCore = newBalance.div(2);
     
                 addLiquidity(Core, eCore);
   
            uint256 eArth = address(this).balance.sub(initialBalance);
   
       payable(_DeFiCore).transfer(eArth);
     
   emit SwapAndLiquify(tokenAmount, newBalance, Core);
    }
