...
contract Unite_Auction_Token is Ownable {
  ...
  string                                            public  constant name = "Unite Auction Token"; 
  ...
  function mint(uint amount) external onlyOwner {
    require(mintTarget != address(0), "mint to Zero");
    _mint(mintTarget, amount);
  }

  function setMintTarget(address target) external onlyOwner {
    mintTarget = target;
  }
  ...
  function send(address recipient, uint amount, bytes calldata exData) external returns (bool) {
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public  returns (bool) {
        ...
        require(currentAllowance >= subtractedValue, "UAT: decreased allowance below zero");
    ...
    function transferFrom(address src, address dst, uint wad) public  returns (bool) {
        ...
        if (src != msg.sender && allowed != type(uint).max) {
            require(allowed >= wad, "UAT: Insufficient approval");
    ...
    function _transfer(address src, address dst, uint wad) internal returns (bool) {
        require(dst != address(0), "UAT:cannot transfer to the zero address");
        require(_balanceOf[src] >= wad, "UAT: Insufficient balance");
    ...
    function _mint(address dst, uint wad) internal {
        require(dst != address(0), "UAT: mint to the zero address");
        _balanceOf[dst] = _balanceOf[dst] + wad;
