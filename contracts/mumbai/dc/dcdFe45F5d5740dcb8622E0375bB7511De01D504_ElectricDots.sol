...
contract ElectricDots is ERC721, Ownable {
    // Structs
    struct Dot {
    ...
    uint constant maxSupply = 100000; // max number of tokens
    uint public totalSupply = 0; // number of tokens minted
    uint public mintPrice = 300000000000000;
    ...
    constructor() ERC721("Electric Dots", "e-DOT") {}
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    uint2str(dot.x + dot.width - 150),
                    " ",
                    uint2str(dot.y + dot.height - 50),
                    " A 100 100 0 0 0 ",
                    uint2str(dot.x + dot.width - 50),
                    " ",
                    uint2str(dot.y + dot.height - 150),
                    '" />'
                )
            );
    }
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                '{"name": "Electric Dots #',
                                uint2str(tokenId),
                                '", "description": "Electric Dots are an assortment of 100000 fully on-chain, randomly generated, happy art balls", "attributes": "", "image":"data:image/svg+xml;base64,',
    ...
    // Mint new Electric Dots
    function mintElectricDots(uint tokenId) public payable {
