...
contract ZokuFactory is OwnableUpgradeable {
  ...
  address public salesImplementation;
  address public settingsImplementation;
  ...
  EnumerableSetUpgradeable.AddressSet private _projectTokens;
  ...
  constructor(
    ...
    address _salesImplementation,
    address _settingsImplementation,
    address _paymentToken
  ) {
    tokenImplementation = _tokenImplementation;
    salesImplementation = _salesImplementation;
    settingsImplementation = _settingsImplementation;
  ...
  function getProjects() public view returns (Project[] memory) {
    Project[] memory projects = new Project[](
      _projectTokens.length()
    );

    for (uint256 i = 0; i < _projectTokens.length(); i++) {
      Project storage project = _projects[_projectTokens.at(i)];
      projects[i] = project;
    }
  ...
  function getProject(address _token) public view returns (Project memory) {
    Project storage project = _projects[_token];
    return project;
  }
  ...
  function create(
    ...
    uint256 adminFee = 150;
    address adminFeeReceiver = _msgSender();
    address royaltyFeeReceiver = tokenOwner;

    /// clone contracts
    address token = ClonesUpgradeable.clone(tokenImplementation);
    address sales = ClonesUpgradeable.clone(salesImplementation);
    address settings = ClonesUpgradeable.clone(settingsImplementation);

    /// initialize contracts with default values
    ISettings(settings).initialize(token, name, symbol, uri, maxSupply);
    IERC721Standard(token).initialize(settings);
    ISales(sales).initialize(settings);

    /// set sales contract as minter
    IERC721Standard(token).setMinter(sales);

    /// settings configuration
    ISettings(settings).setTokenOwner(tokenOwner);
    ISettings(settings).setPaymentToken(paymentToken);
    ISettings(settings).addAdmin(address(this));
    ISettings(settings).addAdmin(tokenOwner);
    ISettings(settings).addAdmin(_msgSender());
    ISettings(settings).setAdminFee(adminFee);
    ISettings(settings).setAdminFeeReceiver(payable(adminFeeReceiver));
    ISettings(settings).setRoyaltyFee(royaltyFee);
    ISettings(settings).setRoyaltyFeeReceiver(payable(royaltyFeeReceiver));
    ISettings(settings).setFundReceiver(payable(sales));
    ISettings(settings).setProtocols(address(this), sales, marketplace);

    _addProject(token, tokenOwner, sales, settings);
  }
   ...
   ****************************************************
   */
  function settingsSetTokenOwner(address _token, address _newOwner) external onlyAdmin {
    Project storage project = _projects[_token];
    ISettings(project.settings).setTokenOwner(_newOwner);
  }

  function settingsAddAdmin(address _token, address _admin) external onlyAdmin {
    Project storage project = _projects[_token];
    ISettings(project.settings).setTokenOwner(_admin);
  }

  function settingsRemoveAdmin(address _token, address _admin) external onlyAdmin {
    Project storage project = _projects[_token];
    ISettings(project.settings).removeAdmin(_admin);
  }

  function settingsSetAdminFee(address _token, uint256 _adminFee) external onlyAdmin {
    Project storage project = _projects[_token];
    ISettings(project.settings).setAdminFee(_adminFee);
  }

  function settingsSetAdminFeeReceiver(address _token, address payable _adminFeeReceiver) external onlyAdmin {
    Project storage project = _projects[_token];
    ISettings(project.settings).setAdminFeeReceiver(payable(_adminFeeReceiver));
  }

  function settingsSetRoyaltyFee(address _token, uint256 _royaltyFee) external onlyAdmin {
    Project storage project = _projects[_token];
    ISettings(project.settings).setRoyaltyFee(_royaltyFee);
  }

  function settingsSetRoyaltyFeeReceiver(address _token, address payable _royaltyFeeReceiver) external onlyAdmin {
    Project storage project = _projects[_token];
    ISettings(project.settings).setRoyaltyFeeReceiver(payable(_royaltyFeeReceiver));
  }

  function settingsSetPaymentToken(address _token, address _paymentToken) external onlyAdmin {
    Project storage project = _projects[_token];
    ISettings(project.settings).setPaymentToken(_paymentToken);
  }

  function settingsSetFundReceiver(address _token, address payable _fundReceiver) external onlyAdmin {
    Project storage project = _projects[_token];
    ISettings(project.settings).setFundReceiver(payable(_fundReceiver));
  }

  function settingsSetRefundable(address _token, bool _refundable, bool _refundableIncludeFee) external onlyAdmin {
    Project storage project = _projects[_token];
    ISettings(project.settings).setRefundable(_refundable, _refundableIncludeFee);
  }

  function settingsSetMarketplace(address _token, address _marketplace) external onlyAdmin {
    Project storage project = _projects[_token];
    ISettings(project.settings).setProtocols(address(this), project.sales, _marketplace);
  }

  function settingsTransferOwnership(address _token, address _newOwner) external onlyAdmin {
    Project storage project = _projects[_token];
    ISettings(project.settings).transferOwnership(_newOwner);
  }

  function settingsRenounceOwnership(address _token) external onlyAdmin {
    Project storage project = _projects[_token];
    ISettings(project.settings).renounceOwnership();
  }
   ...
   ****************************************************
   */
  function salesTransferOwnership(address _token, address _newOwner) external onlyAdmin {
    Project storage project = _projects[_token];
    ISales(project.sales).transferOwnership(_newOwner);
  }
}
