...
contract SwordNft is ERC1155MultiUri, Ownable {
    ...
    function uri(uint256 tokenId) override(ERC1155MultiUri) public view returns (string memory) {
        string memory json = Base64.encode(
            bytes(string(
                abi.encodePacked(
                    '{"name": "', attributes[tokenId].name, '",',
                    '"image": "', "https://api.mining.game/1.jpg", '",',
