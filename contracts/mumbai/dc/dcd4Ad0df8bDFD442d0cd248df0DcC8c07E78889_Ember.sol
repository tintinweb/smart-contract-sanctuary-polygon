...
contract Ember is factory, IEmber{
    ...
    mapping(uint256 => IEmber.LendingRenting) public lendingRenting;
    ...
    */
    ...
    function stopLending(
        ...
        if(IERC721(_nft).ownerOf(_tokenId)!= address(this)){
            ...
            emit LendingStopped(_lendingId, block.timestamp);
    ...
    /**
    * @notice To Update NFT rent charges. 
    * @dev only lender of that NFT can Update the charges  
    * @param _lendingId - lendingID for that NFT
    * @param _perDayRentCharges - daily rent charges 
    */

    function updateNFTRentCharges(uint256 _lendingId, uint256  _perDayRentCharges) external{

        require(lendingRenting[_lendingId].lending.lenderAddress == msg.sender,"Ember :: Invalid LendingID");
        require(_perDayRentCharges!=0, "Invalid rent charges");
        lendingRenting[_lendingId].lending.dailyRentPrice = _perDayRentCharges;
    
    }

    /**
    * @notice To Update NFT stake duration. 
    * @dev only lender of that NFT can Update the charges  
    * @param _lendingId - lendingID for that NFT
    * @param _maxRentDuration - NFT staked  
    */


    function updateNFTStakeDuration(uint256 _lendingId, uint256  _maxRentDuration) external{

        require(lendingRenting[_lendingId].lending.lenderAddress == msg.sender,"Ember :: Invalid LendingID");
        require(_maxRentDuration!=0, "Invalid duration");
    ...
    */
    ...
    function checkProxy(address _borrower) public override view returns(address){
        return BorrowerProxyAddress[_borrower].proxyAddress;
        //return lendingRenting[_lendingId].renting.proxyAddress;
    }
...
interface IEmber {
    ...
    event LendingStopped(uint256 lendingId, uint256 stoppedAt);
...
// this contract delegates call to suitable adapters which further calls to game contract
 ...
 contract Proxy {
    ...
    */
     ...
     * @param _lendingId - lendingId
    */
    function getNFT(address _nft, uint256 _tokenId, uint256 _lendingId) public {
