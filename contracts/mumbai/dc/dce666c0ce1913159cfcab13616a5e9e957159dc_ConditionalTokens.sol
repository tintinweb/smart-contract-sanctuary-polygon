...
contract ConditionalTokensStorage {
    /// Mapping key is an condition ID. Value represents numerators of the
    /// payout vector associated with the condition. This array is initialized
    /// with a length equal to the outcome slot count. E.g. Condition with 3
    /// outcomes [A, B, C] and two of those correct [0.5, 0.5, 0]. In Ethereum
    /// there are no decimal values, so here, 0.5 is represented by fractions
    /// like 1/2 == 0.5. That's why we need numerator and denominator values.
    /// Payout numerators are also used as a check of initialization. If the
    /// numerators array is empty (has length zero), the condition was not
    /// created/prepared. See getOutcomeSlotCount.
    mapping(ConditionID => uint256[]) public payoutNumerators;
    /// Denominator is also used for checking if the condition has been
    /// resolved. If the denominator is non-zero, then the condition has been
    /// resolved.
    ...
    ConditionalTokensStorage
{
    ...
    /// @dev Called by the oracle for reporting results of conditions. Will set
    /// the payout vector for the condition with the ID
    /// ``keccak256(abi.encodePacked(oracle, questionId, outcomeSlotCount))``,
    /// where oracle is the message sender, questionId is one of the parameters
    /// of this function, and outcomeSlotCount is the length of the payouts
    /// parameter, which contains the payoutNumerators for each outcome slot of
    /// the condition.
    ...
    function reportPayouts(QuestionID questionId, uint256[] calldata payouts) external {
        ...
        ConditionID conditionId = CTHelpers.getConditionId(msg.sender, questionId, outcomeSlotCount);
        ...
        for (uint256 i = 0; i < outcomeSlotCount; i++) {
            if (payoutNumerators[conditionId][i] != 0) revert PayoutAlreadyReported();
        ...
        if (den == 0) revert PayoutsAreAllZero();
    ...
    /// @dev This function splits a position. If splitting from the collateral,
    /// this contract will attempt to transfer `amount` collateral from the
    /// message sender to itself.  Regardless, if successful, `amount` stake will be
    /// minted in the split target positions. If any of the transfers, mints, or
    /// burns fail, the transaction will revert.  The transaction will also
    /// revert if the given partition is trivial, invalid, or refers to more
    /// slots than the condition is prepared with.
    ...
    function redeemPositions(
        ...
        if (den == 0) revert ResultNotReceivedYet();
        ...
        for (uint256 i = 0; i < indices.length; i++) {
            ...
            uint256 positionId = CTHelpers.getPositionId(
                collateralToken,
                CTHelpers.getCollectionId(conditionId, index)
            );
        ...
        emit PayoutRedemption(msg.sender, collateralToken, conditionId, indices, totalPayout);
    }
    ...
    // TODO: add batch redeem

    /// @dev Gets the outcome slot count of a condition.
    /// @param conditionId ID of the condition.
    /// @return Number of outcome slots associated with a condition, or zero if condition has not been prepared yet.
    function getOutcomeSlotCount(ConditionID conditionId) public view returns (uint256) {
    ...
    function getConditionId(
        ...
        QuestionID questionId,
        uint256 outcomeSlotCount
    ) external pure returns (ConditionID) {
    ...
    /// @dev Constructs list of positionIds for a condition.
...
interface IConditionalTokens is IERC1155Upgradeable {
    ...
    // ERC20 collateral
    error TransferFailed(); // No longer used
