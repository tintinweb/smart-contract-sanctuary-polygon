...
contract Matic20V1 {
    ...
    uint256 constant public secRate = 2314851; //DAILY 20%
    ...
    function transferPayout(address _receiver, uint256 _amount) private {
        if (_amount > 0 && _receiver != address(0)) {
            if (contractBalance() > 0) {
                ...
                uint256 insuranceFee = payout.mul(1).div(100); // 1%
    ...
    function distributeRef(uint256 _matic, address _affFrom) private{

        uint256 _allaff = (_matic.mul(20)).div(100);
        address affAddr = _affFrom;
        for(uint i = 0; i < affRate.length; i++){
          uint256 _affRewards = (_matic.mul(affRate[i])).div(100);
    ...
    function updateStarttime(uint256 _releaseTime) public returns(bool){
      require(msg.sender == owner, "unauthorized call");
      require(now < _releaseTime, "invalid time");
      releaseTime = _releaseTime;
      return true;
    }
}
