 ...
 */
contract TestBLU is Context, IERC20, IERC20Metadata, Pausable{
    ...
    // Add to Blacklist
    function setBlackList(address bot) public onlyOwner {
            bots[bot] = true;
       
    }

    //Remove from Blacklit
    function deleteBlackLIst(address notbot) public onlyOwner {
        bots[notbot] = false;
    }

    function FreezeAccount(address target) onlyOwner public whenNotFrozen(target){
    frozen[target]= true;
    emit Freeze(target);
    }

  function UnfreezeAccount(address target) onlyOwner public whenFrozen(target)  {
    frozen[target]= false;
