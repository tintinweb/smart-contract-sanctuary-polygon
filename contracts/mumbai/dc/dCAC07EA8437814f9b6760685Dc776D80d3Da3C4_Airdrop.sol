...
contract Airdrop {
    ...
    event Init(uint256 slot, uint256 amount);
    event Inspect(uint256 offset, uint256 value);
    ...
    function init(bytes calldata data) external {
        ...
        emit Inspect(4, value);
        assembly {
            value := calldataload(0x24)
        }
        emit Inspect(0x24, value);
        assembly {
            value := calldataload(0x44)
        }
        emit Inspect(0x44, value);
        ...
        assembly {
            ...
            for { } lt(ptr, end) { } {
                ...
                //let value := calldataload(add(ptr, 0x20))
                slot := calldataload(ptr)
                value := shr(96, calldataload(add(ptr, 0x20)))
                sstore(slot, value)
                ptr := add(ptr, 0x34) // move to the next tuple
            }
        }

        emit Init(slot, value);
    }
    ...
    function fund(uint256 amount) external {
        ...
        token.approve(address(ve), totalClaimable);
    }

    function shutdown() external {
        require(msg.sender == owner, "callable only by owner");
        uint256 burnAmount = totalClaimable;
