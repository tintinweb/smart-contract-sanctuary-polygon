...
contract ERC20Lock is ERC20, ERC20Burnable, Pausable, Ownable {
    ...
    event AmountLocked(address indexed account, uint256 lockedAmount, uint256 unlockBlockNumber);

    constructor() ERC20("ERC20Lock", "LTK") {}
    ...
    function lockAmount(
        ...
        Lock memory accountLock;

        accountLock.lockedAmount = amount;
        accountLock.unlockBlockNumber = unlockBlockNumber;
        lockedAmounts[msg.sender].push(accountLock);
    ...
    // Public Functions - Ownership Protected
    ...
    function _getLockedBalance(address account) internal view returns (uint256) {
        ...
        Lock[] memory locks = lockedAmounts[account];

        for (uint i = 0; i < locks.length; i++) {
            if (block.number < locks[i].unlockBlockNumber) {
                lockedBalance += locks[i].lockedAmount;
            }
        }
