...
contract MapManager is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function getStakedBeasts(address player) external view returns (uint256[] memory) {
        uint256[] memory playerBeasts = _playerBeasts[player].values();
        uint256 availableBeastsCount;
        for (uint256 index = 0; index < playerBeasts.length; index++) {
            BeastState memory beastState = _beastStates[uint16(playerBeasts[index])];
            if (beastState.groupId == 0) availableBeastsCount++;
        }
        uint256[] memory availableBeasts = new uint256[](availableBeastsCount);
        uint256 availableBeastsIndex;
        for (uint256 index = 0; index < playerBeasts.length; index++) {
            uint16 beastId = uint16(playerBeasts[index]);
            BeastState memory beastState = _beastStates[beastId];
            if (beastState.groupId == 0) {
                availableBeasts[availableBeastsIndex] = beastId;
            }
        }
