...
contract SoulboundToken is ERC721URIStorageUpgradeable, OwnableUpgradeable, CantBeEvilUpgradeable() {
  ...
  function supportsInterface(bytes4 interfaceId) public view virtual override(CantBeEvilUpgradeable, ERC721Upgradeable) returns (bool) {
  ...
  function updateWhiteList(address __account, string memory __tokenURI) external onlyOwner {
    whiteList[__account] = __tokenURI;
  }

  function mintByOwner(address __account, string memory __tokenURI) external onlyOwner {
    _mint(__account, __tokenURI);
  }

  function mintByMember() external {
    require(bytes(whiteList[msg.sender]).length > 0, "not in whitelist");
    _mint(msg.sender, whiteList[msg.sender]);
  }

  function mintBySignature(string memory __message, bytes memory __signature, string memory __tokenURI) external {
    ...
    _mint(msg.sender, __tokenURI);
  }

  function _mint(address __to, string memory __tokenURI) internal {
    tokenIdCounter.increment();
    uint256 _tokenId = tokenIdCounter.current();
    _safeMint(__to, _tokenId);
    _setTokenURI(_tokenId, __tokenURI);
    emit Mint(__to, _tokenId);
  }
