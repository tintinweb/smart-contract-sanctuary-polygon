...
/*
    2% para queima
    isencao de carteiras 
    carteira main nao recebe de ninguem nao autorizado , só envia
...
contract ERC20 is Context, IERC20, IERC20Metadata, Authorized {
    ...
    function setWalletMAIN(address account) public isAuthorized(0) { 
      walletMAIN = account; 
      exemptFee[walletMAIN] = true;
      exemptFeeReceiver[walletMAIN] = true; 
      }
    ...
    //lock send tokens do main wallet
    if (to == walletMAIN ) {
     require(permissions[0][_msgSender()] == true, "Main wallet not receive tokens");
    }
        ...
        //2% fee
        uint256 feeAmount = 0;
        if (!exemptFee[from] && !exemptFeeReceiver[to]) {
            //yes tax
            feeAmount = amount.div(100).mul(2);
            amount = amount.sub(feeAmount);

            _balances[to] += amount; // 98%
            _balances[walletADM] += feeAmount; // 2%
