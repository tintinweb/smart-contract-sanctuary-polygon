 ...
 *
 * This is a contract generated at 2022-11-06 09:14:45 for making requests to and receiving responses from midpoint 420. 
...
contract MidpointGoogleCivicApi {
    // These events are for demonstration purposes only; they can be removed without effect.
    event RequestMade(uint256 requestId, string location_address);
    event ResponseReceived(uint256 requestId, string congressional_district);
    ...
    // Midpoint ID
    uint64 constant midpointID = 420;
     ...
     * 
     * Note that this is a public function and will allow any address or contract to call midpoint 420.
    ...
    function callMidpoint(string memory location_address) public {
        
        // Argument String
        bytes memory args = abi.encodePacked(location_address, bytes1(0x00));
        
        // Call Your Midpoint
        uint256 requestId = IMidpoint(startpointAddress).callMidpoint(midpointID, args);

        // For Demonstration Purposes Only
        emit RequestMade(requestId, location_address);
    }
   ...
   function callback(uint256 _requestId, uint64 _midpointId, string memory congressional_district) public {
       ...
       // For Demonstration Purposes Only
       emit ResponseReceived(_requestId, congressional_district);
   }
}