...
contract MintNFT is Ownable, ERC721, AccessControl {
    ...
    function mint(
        ...
        require(x.length != 0, "At least one coordinate required");
        require(
            x.length == y.length,
        ...
        for (uint256 i = 0; i < len; ) {
            ...
            _mint(to, encodedTokenId);

            _updateMetadata(encodedTokenId, metadata);
            emit CreateNFT(to, metadata, x[i], y[i], z[i], encodedTokenId);
            unchecked {
                ++i;
            }
        }
        calculateAndTransfer(to, len);
    }

    function setOneForge(address oneForgeAddr) external onlyOwner {
        require(oneForgeAddr != address(0), "Address cannot be zero");
        oneForge = oneForgeAddr;
    }

    function burn(uint256 tokenId) external {
        require(msg.sender == oneForge, "Only called by oneForge");
    ...
    function _encodeTokenId(
        ...
        require(0 < x && x <= MAX_X, "(x,y,z) should be inside bounds");
        require(0 < y && y <= MAX_Y, "(x,y,z) should be inside bounds");
        require(0 < z && z <= MAX_Z, "(x,y,z) should be inside bounds");
    ...
    function calculateAndTransfer(address user, uint256 tokenIdArrayLength)
        internal
    {
        uint256 amount = NFT_PRICE * tokenIdArrayLength;
        ogtToken.safeTransferFrom(user, address(this), amount);
    }
}
