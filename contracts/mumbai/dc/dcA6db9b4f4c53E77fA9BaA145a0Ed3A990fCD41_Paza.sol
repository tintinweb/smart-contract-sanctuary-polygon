...
contract Paza is ERC20, ERC20Burnable, AccessControl, BasicMetaTransaction {
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    uint price = 1 ether;
    IERC20 public usdt;
    event pazabuy(address user, uint _amount);
    event pazasell(address user,uint _amount);
    constructor(address _usdt) ERC20("Paza", "PAZA") {
        usdt = IERC20(_usdt);
    ...
    //  function burn(address user, uint _amount)  public{
    //  _burn(user,_amount);
    // }

    function buy(uint _amount) public 
    {
        require(usdt.balanceOf(_msgSender())>=_amount*10**decimals());
        usdt.transferFrom(_msgSender(), address(this), _amount*10**decimals());
        _mint(_msgSender(),_amount*10**decimals());
        emit pazabuy(_msgSender(),_amount*10**decimals());
    }

    function sell(uint _amount) public{
     require(balanceOf(_msgSender())>=_amount*10**decimals());
     _burn(_msgSender(),_amount*10**decimals());
     usdt.transfer( _msgSender(),_amount*10**decimals());
     emit pazasell(_msgSender(),_amount*10**decimals());
    }
