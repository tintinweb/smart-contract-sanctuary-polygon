...
contract Coin_MCTE is ERC20 {
	...
	uint256 public mintPrice_Native = uint256(2000000000000000);
	event NewPrice_Native (uint256 newPrice);

	constructor() ERC20("MYCUESTEST", "MCTE") {
		owner = msg.sender;
		super._mint(msg.sender, uint256(10000000000000000000));
	}
 ...
 * checks that (amount of native currency sent to contract) is equals to ((1000000000000000000) * ((mintPrice_Native) / (10 raised to the power of 18)))
...
*/
	function mint() public payable {
		...
		require((msg.value == (uint256(1000000000000000000) * (mintPrice_Native / (uint256(10) ** uint256(18))))), "Wrong Fee");
		super._mint(msg.sender, uint256(1000000000000000000));
		emit Mint(msg.sender);
	}

/**
 * Function changeMintPrice_Native
 ...
 * checks that the function is called by the owner of the contract
 * updates mintPrice_Native as _newMintPrice
 * emits event NewPrice_Native with inputs _newMintPrice
*/
	function changeMintPrice_Native(uint256 _newMintPrice) public onlyOwner {
		mintPrice_Native  = _newMintPrice;
		emit NewPrice_Native(_newMintPrice);
	}
