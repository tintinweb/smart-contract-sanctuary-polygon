...
contract Flipcoin {
    ...
    function closeBet() external onlyOwner {
        ...
        uint256 winnerIndex = _generateRandomNumber() % 2;
        ...
        delete _bets[winner];
        delete _bets[loser];
        _removeAddress(winner);
        _removeAddress(loser);
    }
    ...
    function _removeAddress(address addr) private {
        for (uint256 i = 0; i < _betAddresses.length; i++) {
            if (_betAddresses[i] == addr) {
                if (i != _betAddresses.length - 1) {
                    _betAddresses[i] = _betAddresses[_betAddresses.length - 1];
                }
                _betAddresses.pop();
                break;
            }
    ...
    function _generateRandomNumber() private view returns (uint256) {
        return uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty, _totalPot))) % (2**256 - 1);
    }
}