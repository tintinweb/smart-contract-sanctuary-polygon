...
contract Purchase is BaseContract, ReentrancyGuardUpgradeable {
    ...
    function addDivToken(address _token) external onlyOwner {
    ...
    function setDivToken(address _token, uint256 _index) external onlyOwner {
        require(_token != address(0), "currency is zero");
        for (uint256 i = 0; i < payTokens.length; i++) {
            require(payTokens[i] != _token, "existed");
        }
        payTokens[_index] = _token;
        emit SetDivCurrency(_token, _index);
    }

    function getDivLength() external view returns (uint256) {
        return payTokens.length;
    }
    ...
    event AddDivCurrency(address indexed token);
    event DeleteDivCurrency(address indexed token);
    event SetDivCurrency(address indexed token, uint256 indexed index);
}
