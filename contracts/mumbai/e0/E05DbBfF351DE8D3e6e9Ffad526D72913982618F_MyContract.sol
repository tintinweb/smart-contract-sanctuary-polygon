...
contract MyContract is ERC1155LazyMint {
    ...
    mapping(uint256 => uint256) public nftBought;
    mapping(uint256 => uint256) public nftEvolved;
    mapping(uint256 => uint256) public nftSold;
    mapping(uint256 => uint256) public nftCurrentlyHolded;

    mapping(address => mapping(uint256 => bool)) public whiteList;

    mapping(address => mapping(uint256 => uint256)) public referralCount;
    mapping(address => mapping(uint256 => uint256)) public referralsPaid;
    mapping(uint256 => uint256) public mintPrices;
    mapping(uint256 => uint256) public values;
    mapping(uint256 => uint256) public bonusValues;
    uint256 public requiredRefsNumber;

    event TokenClaimed(address indexed recipient, uint256 indexed tokenId);
    event AddressWhitelisted(address indexed recipient, uint256 indexed tokenId);
    event Bought(address indexed user, uint256 indexed tokenId, address indexed referral);
    event Evolved(address indexed user, uint256 indexed tokenId);
    event Sold(address indexed user, uint256 indexed tokenId);
    ...
    function setConstantValues() internal 
    {
        requiredRefsNumber = 1;
        // 0.001 MATIC is 1e15 MWei
        // Eggs : 1,2,3    Small : 4,5,6      Big : 7,8,9

        mintPrices[4] = 1e13;   // 0.00 001 MATIC
        mintPrices[5] = 1e14;   // 0.00 01 MATIC
        mintPrices[6] = 1e15;   // 0.00 1 MATIC

        values[4] = 3e12;       // 0.00 0003 MATIC
        values[5] = 3e13;       // 0.00 003 MATIC
        values[6] = 3e14;       // 0.00 03 MATIC

        values[7] = 2e13;       // 0.00 002 MATIC
        values[8] = 2e14;       // 0.00 02 MATIC
        values[9] = 2e15;       // 0.00 2 MATIC

        bonusValues[7] = 8e12;  // 0.00 0008 MATIC
        bonusValues[8] = 8e13;  // 0.00 008 MATIC
        bonusValues[9] = 8e14;  // 0.00 08 MATIC
    }

    function buyNFT(uint256 _tokenId, address _ref) external payable nonReentrant {
        require(_tokenId >= 4 && _tokenId <= 6, "Token ID must be between 4 and 6");
        uint256 nftPrice = mintPrices[_tokenId];
        require(msg.value >= nftPrice, "Insufficient MATIC sent to mint NFT");
        require(_ref != msg.sender, "You cannot refer yourself");
        require(this.balanceOf(msg.sender, _tokenId) == 0, "You have already minted this NFT");

        if (_ref != address(0)) {
            referralCount[_ref][_tokenId]++;
        }
        // Mint the NFTs up to the _tokenId
        for(uint256 i = 4; i <= _tokenId; i++) {
            if(this.balanceOf(msg.sender, i) == 0)
            {
                _transferTokensOnClaim(msg.sender, i, 1);
                nftBought[i]++;
                nftCurrentlyHolded[_tokenId]++;
                emit Bought(msg.sender, this.totalSupply(i), _ref);
            }
        }
    }

    function evolve(uint256 _tokenId) external nonReentrant
    {
        require(this.balanceOf(msg.sender, _tokenId) == 1, "No NFT to evolve");
        ...
        {
            evolvedTokenId = _tokenId + 6;
            referalsId = _tokenId + 3;
        }
        else if (_tokenId >= 4 && _tokenId <= 6)
        {
            evolvedTokenId = _tokenId + 3;
            referalsId = _tokenId;
        }
        else
        {
        ...
        require(referralCount[msg.sender][referalsId] >= requiredRefsNumber, "Insufficient referral count to evolve");
        _burn(msg.sender, _tokenId, 1);
        _transferTokensOnClaim(msg.sender, evolvedTokenId, 1);
        payable(msg.sender).transfer(values[evolvedTokenId]);
        nftEvolved[_tokenId]++;
        nftEvolved[evolvedTokenId]++;
        nftCurrentlyHolded[_tokenId]--;
        emit Evolved(msg.sender, evolvedTokenId);
    }
    ...
    function sellNft(uint256 _tokenId) external nonReentrant
    {
        require(_tokenId >= 4 && _tokenId <= 6, "Token ID must be between 4 and 6");
        _burn(msg.sender, _tokenId, 1);
        payable(msg.sender).transfer(values[_tokenId]);
        nftSold[_tokenId]++;
        nftCurrentlyHolded[_tokenId]--;
        emit Evolved(msg.sender, _tokenId);
    }

    function getMyReferalCount(uint256 _tokenId) external view returns (uint256)
    {
        return referralCount[msg.sender][_tokenId];
    }
    
    function ClaimBonuses() external nonReentrant 
    {
        uint256 totalReward = 0;
        for(uint256 i = 7; i <= 9; i++) 
        { 
            uint256 referrals = referralCount[msg.sender][i] >= requiredRefsNumber ? referralCount[msg.sender][i]-requiredRefsNumber : 0;

            uint256 paid = referralsPaid[msg.sender][i];
            if(referrals <= paid + requiredRefsNumber - 1) {
                continue; // Skip to next iteration if there are no new referrals for this token
            }
            uint256 unpaidReferrals = referrals - paid;
            uint256 withdrawableCount = unpaidReferrals / requiredRefsNumber;
            if(withdrawableCount == 0) {
                continue; // Skip to next iteration if there are not enough referrals for withdrawal
            }
            uint256 reward = withdrawableCount * bonusValues[i];
            // Mark these referrals as paid
            referralsPaid[msg.sender][i] += withdrawableCount * requiredRefsNumber;
            // Update total reward
            totalReward += reward;
        }
        require(totalReward > 0, "No rewards available for withdrawal");
        // Transfer the reward to the user
        payable(msg.sender).transfer(totalReward);
    }

    function setDrop(address[] memory recipients, uint256[] memory _tokenIds) external onlyOwner {
        for(uint256 i = 0; i < _tokenIds.length; i++) {
            uint256 tokenId = _tokenIds[i];
            require(tokenId >= 1 && tokenId <= 3, "Token ID must be between 1 and 3");

            for(uint256 j = 0; j < recipients.length; j++) {
                whiteList[recipients[j]][tokenId] = true;
                //emit AddressWhitelisted(recipients[j], tokenId);
            }
        }
    }
    ...
    function claimDrop() external nonReentrant {
        for(uint256 i = 1; i <= 3; i++) {
            if(whiteList[msg.sender][i] == true) {
                _transferTokensOnClaim(msg.sender, i, 1);
                whiteList[msg.sender][i] = false; 
                nftCurrentlyHolded[i]++;
                emit TokenClaimed(msg.sender, i);
            }
        }
    }

    // withdraw available money
    function promotion(uint256 amount) external nonReentrant onlyOwner {
    ...
    function claim(
        ...
        uint256 _quantity
            ...
            revert("Claiming is disabled. Please use buyNFT.");
    }
...
// Override safeTransferFrom function to prevent transfers
    ...
    // Override safeBatchTransferFrom function to prevent transfers
    function safeBatchTransferFrom(
        address,
        address,
        uint256[] memory,
