...
contract TokenERC721 is ERC721Base {
    ...
    mapping(uint256 => uint8) public series; // token id => series number
    mapping(uint256 => RarityType) public rarity; // token id => rarity
    mapping(address => uint256) public burnTickets; // wallet => burn tickets count
    ...
    function mintTo(address _to, string memory _tokenURI, uint8 _series, RarityType _rarity) public {
        require(_canMint(), "Not authorized to mint.");
        uint256 mintTokenId = nextTokenIdToMint();
        _setTokenURI(mintTokenId, _tokenURI);
        _safeMint(_to, 1, "");
        series[mintTokenId] = _series;
        rarity[mintTokenId] = _rarity;
    }

    function burn(uint256 _tokenId) public override {
        _burn(_tokenId, true);
        if (series[_tokenId] == 2) {
            burnTickets[msg.sender] += burnTicketCountOfRarity[uint8(rarity[_tokenId])];
        }
    }
