    ...
    ResourceImplementationPointer
{
    ...
    modifier onlyNFTOwner() {
        ...
        require(
            sender != address(0),
            "NFTs: Cannot be from the zero address"
        );
        require(
            isOwnerOfNFTs(sender),
            "NFTs: No NFT owned by this account"
        );
        _;
    }
    ...
    modifier checkPermissions(uint256 _nftId) {
        ...
        require(
            isOwnerOfNFT(sender, _nftId),
            "NFTs: You do not control this NFT"
        );
        _;
    }
    ...
    modifier resourceSet() {
        require(
            address(resource) != address(0),
            "NFTs: Resource is not set"
        );
        _;
    }

    function initialize(address _treasury) external initializer {
        __ERC721_init("Zootopia Ecosystem", "ZOO");
        __Ownable_init();
        ...
        changeNodeMinPrice(2000 * (10**18)); // 2,000 Resources
        changeCompoundDelay(3600); // 1h
        changeTierSystem(
            [100000, 105000, 110000, 120000, 130000, 140000],
            [1000, 500, 100, 50, 10, 0]
        );
        changeTokenBaseURIs([
            "ipfs://QmRgR28xBXziSc5h55kTFnzGzp8AWCmwS4Yf5efzieXv7k",
            "ipfs://Qmb2onA133EZWTcydkYtDzYm7AR6dLq3Cnobn9eydyytfK",
            "ipfs://QmP5kRbMJ1giBNpyjWwdAVwMhmAz1MDe9H1ThDrWR3fEU7",
            "ipfs://QmQrFkDbd8qjQMo7HRF4nNcRHyiFoMrWS2oreAke4yijRM",
            "ipfs://QmcsYBwjpj6nhxh1rMkSK4DFAAFKw3QPzXXEToUwbsAqJE",
            "ipfs://QmPFxQKRPgcLGD5CgCuEEA3Mw6x4yyYpKMt4b8qLQpny7H",
            "ipfs://QmTecVkFuYNRohAAB6qdh9eUAquKRQ2iafS5YxYk7Vf3Ua"
        ]);
    ...
    function renameNFT(uint256 _nftId, string memory nftName)
        external
        nonReentrant
        onlyNFTOwner
        checkPermissions(_nftId)
        whenNotPaused
        resourceSet
    {
        require(
            bytes(nftName).length > 1 && bytes(nftName).length < 32,
            "NFTs: Incorrect name length, must be between 2 to 31"
        );
        NFTEntity storage nft = _nfts[_nftId];
        require(nft.nftValue > 0, "Error: NFT is empty");
        (uint256 newNFTValue, uint256 feeAmount) = getProcessingFee(
            nft.nftValue,
            5
        );
        logTier(nft.rewardMult, -int256(feeAmount));
        burnedFromRenaming += feeAmount;
        nft.nftValue = newNFTValue;
        nft.name = nftName;
    }

    function createNFTWithTokens(
        string memory nftName,
        uint256 nftValue
    ) external payable nonReentrant whenNotPaused resourceSet returns (uint256) {
        address sender = _msgSender();
        ...
        require(
            nftValue >= creationMinPrice || nftValue == 0,
            "NFTs: NFT value set below minimum"
        );
        require(
            isNameAvailable(sender, nftName),
            "NFTs: Name not available"
        );
        require(
            resource.balanceOf(sender) >= nftValue,
            "NFTs: Balance too low for creation"
        );
        require(
            mintPrice == msg.value,
            "NFTs: msg.value == mint_price"
        );

        if (nftValue > 0) {
            resource.accountBurn(sender, nftValue);
        }
        ...
        totalValueLocked += nftValue;
        logTier(tierLevel[0], int256(nftValue));
        ...
        _setTokenURI(newNFTId, tokenBaseURIs[getTokenBaseURI(nftValue)]);

        return newNFTId;
    }

    function stakeTokensOnNFT(
        uint256 _nftId,
        uint256 nftValue
    ) external nonReentrant onlyNFTOwner checkPermissions(_nftId) whenNotPaused resourceSet{
        address sender = _msgSender();
        require(
            nftValue >= creationMinPrice,
            "NFTs: NFT value set below minimum"
        );

        require(
            resource.balanceOf(sender) >= creationMinPrice,
            "NFTs: Balance too low for creation"
        );
        ...
        nft.nftValue += nftValue;
        nft.lastProcessingTimestamp = currentTime;
        nft.rewardMult = tierLevel[0];

        _setTokenURI(_nftId, tokenBaseURIs[getTokenBaseURI(nftValue)]);
    }

    function cashoutReward(uint256 _nftId)
        external
        nonReentrant
        onlyNFTOwner
        checkPermissions(_nftId)
        ...
        resourceSet
    {
        ...
        uint256[] memory nftsOwned = getNFTIdsOf(account);
        for (uint256 i = 0; i < nftsOwned.length; i++) {
            (
                uint256 amountToReward,
                uint256 feeAmount,
                uint256 feeBurnAmount
            ) = _getNFTCashoutRewards(nftsOwned[i]);
    ...
    function compoundReward(uint256 _nftId)
        external
        nonReentrant
        onlyNFTOwner
        checkPermissions(_nftId)
        ...
        resourceSet
    {
        (
            ...
            uint256 feeAmount
        ) = _getNFTCompoundRewards(_nftId);
        require(
            amountToCompound > 0,
            "NFTs: You must wait until you can compound again"
        );
        if (feeAmount > 0) {
        ...
        resourceSet
    {
        ...
        uint256[] memory nftsOwned = getNFTIdsOf(account);
        uint256[] memory nftsAffected = new uint256[](nftsOwned.length);

        for (uint256 i = 0; i < nftsOwned.length; i++) {
            (
                uint256 amountToCompound,
                uint256 feeAmount
            ) = _getNFTCompoundRewards(nftsOwned[i]);
            if (amountToCompound > 0) {
                nftsAffected[i] = nftsOwned[i];
                feesAmount += feeAmount;
                amountsToCompound += amountToCompound;
            } else {
                delete nftsAffected[i];
            }
        }
    ...
    function _getNFTCashoutRewards(uint256 _nftId)
        private
        returns (
            uint256,
            uint256,
        ...
        )
    {
        ...
        if (!isProcessable(nft)) {
            return (0, 0, 0);
        }

        uint256 reward = calculateReward(nft);
        nft.totalClaimed += reward;

        if (nft.rewardMult != tierLevel[0]) {
            logTier(nft.rewardMult, -int256(nft.nftValue));
            logTier(tierLevel[0], int256(nft.nftValue));
        }

        (
            uint256 takeAsFeePercentage,
            uint256 burnFromFeePercentage
        ) = getCashoutDynamicFee(nft.rewardMult);
    ...
    function _getNFTCompoundRewards(uint256 _nftId)
        private
        returns (uint256, uint256)
    {
        ...
        if (reward > 0) {
            ...
            nft.lastProcessingTimestamp = block.timestamp;
            nft.nftValue += amountToCompound;
            nft.rewardMult += increaseMultiplier(nft.rewardMult);

            logTier(nft.rewardMult, int256(nft.nftValue));

            _setTokenURI(_nftId, tokenBaseURIs[getTokenBaseURI(nft.nftValue)]);
    ...
    function _cashoutReward(
        ...
        require(
            amountToReward > 0,
            "NFTs: You don't have enough reward to cash out"
        );
        address to = _msgSender();
        resource.accountReward(to, amountToReward);
        resource.liquidityReward(feeAmount);
        if (feeBurnAmount > 0) {
            resource.liquidityBurn(feeBurnAmount);
        }
    }
    ...
    function logTier(uint256 mult, int256 amount) private {
        ...
        if (tierStorage.exists) {
            require(
                ...
                "NFTs: rewardMult does not match in TierStorage"
            );
            ...
            require(
                ...
                "NFTs: amountLockedInTier cannot underflow"
            );
            tierStorage.amountLockedInTier = amountLockedInTier;
        } else {
            require(
                amount > 0,
                "NFTs: Fatal error while creating new TierStorage. Amount cannot be below zero."
            );
    ...
    function getTokenBaseURI(uint256 amount)
        private
        pure
        returns (uint256)
    {
        ...
        if (amount == 0) {
            ...
            index = 3;
        } else if (amount == 15000 * (10**18)) {
            index = 4;
        } else if (amount == 20000 * (10**18)) {
            index = 5;
        } else if (amount == 30000 * (10**18)) {
            index = 6;
        } else {
            index = 7;
        }
    ...
    function isProcessable(NFTEntity memory nft)
        private
        view
        returns (bool)
    {
        return
            block.timestamp >= nft.lastProcessingTimestamp + compoundDelay;
    }

    function calculateReward(NFTEntity memory nft)
        private
        view
        returns (uint256)
    {
        return
            _calculateRewardsFromValue(
                nft.nftValue,
                nft.rewardMult,
                block.timestamp - nft.lastProcessingTimestamp,
                rewardPerDay
            );
    }

    function rewardPerDayFor(NFTEntity memory nft)
        private
        view
        returns (uint256)
    {
    ...
    function _calculateRewardsFromValue(
        ...
        return (rewardsMultiplicated * _nftValue) / 100000;
    }

    function nftExists(uint256 _nftId) private view returns (bool) {
    ...
    function isNameAvailable(address account, string memory nftName)
        public
        view
        returns (bool)
    {
        uint256[] memory nftsOwned = getNFTIdsOf(account);
        for (uint256 i = 0; i < nftsOwned.length; i++) {
            NFTEntity memory nft = _nfts[nftsOwned[i]];
            if (keccak256(bytes(nft.name)) == keccak256(bytes(nftName))) {
                return false;
            }
        }
        return true;
    ...
    function isOwnerOfNFTs(address account) public view returns (bool) {
        return balanceOf(account) > 0;
    }

    function isOwnerOfNFT(address account, uint256 _nftId)
        public
        view
        returns (bool)
    {
        uint256[] memory nftIdsOf = getNFTIdsOf(account);
        for (uint256 i = 0; i < nftIdsOf.length; i++) {
            if (nftIdsOf[i] == _nftId) {
                return true;
            }
        }
        return false;
    ...
    function getNFTIdsOf(address account)
        public
        view
    ...
    function getNFTsByIds(uint256[] memory _nftIds)
        external
        view
        returns (NFTInfoEntity[] memory)
    {
        NFTInfoEntity[] memory nftsInfo = new NFTInfoEntity[](
            _nftIds.length
        );
        ...
        for (uint256 i = 0; i < _nftIds.length; i++) {
            ...
            uint256 pendingRewardsGross = calculateReward(nft);
            uint256 rewardsPerDayGross = rewardPerDayFor(nft);
    ...
    function changeNodeMinPrice(uint256 _creationMinPrice) public onlyOwner {
        require(
            _creationMinPrice > 0,
            "NFTs: Minimum price to create a NFT must be above 0"
        );
    ...
    function changeCompoundDelay(uint256 _compoundDelay) public onlyOwner {
        require(
            _compoundDelay > 0,
            "NFTs: compoundDelay must be greater than 0"
        );
    ...
    function changeTierSystem(
        ...
        require(
            _tierLevel.length == 6,
            "NFTs: newTierLevels length has to be 6"
        );
        require(
            _tierSlope.length == 6,
            "NFTs: newTierSlopes length has to be 6"
        );
        tierLevel = _tierLevel;
        tierSlope = _tierSlope;
    }

    function changeTokenBaseURIs(
        string[7] memory _tokenBaseURIs
    ) public onlyOwner {
        require(
            _tokenBaseURIs.length == 7,
            "NFTs: URIs length has to be 7"
        );
        tokenBaseURIs = _tokenBaseURIs;
    }
