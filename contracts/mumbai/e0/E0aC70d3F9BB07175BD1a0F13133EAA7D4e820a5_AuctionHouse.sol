    ...
    OwnableUpgradeable
{
 ...
 uint256[] public availableTokens;
    ...
    function _createAuction() internal {
        require(availableTokens.length > 0, "No available tokens to auction");

        // Get a random index
        uint256 randomIndex = _getRandomIndex(availableTokens.length);
        ...
        _removeFromAvailableTokens(randomIndex);
    ...
    if (_auction.bidder == address(0)) {
        ...
        availableTokens.push(_auction.vrbId);  // If no bidder, add back the token into available list
    } else {
...
function _removeFromAvailableTokens(uint256 index) internal {
...
/// @title Interface for vrb Auction Houses
...
interface IAuctionHouse {
    struct Auction {
        // ID for the vrb (ERC721 token ID)
        uint256 vrbId;
    ...
    event AuctionCreated(uint256 indexed vrbId, uint256 startTime, uint256 endTime);

    event AuctionBid(uint256 indexed vrbId, address sender, uint256 value, bool extended);

    event AuctionExtended(uint256 indexed vrbId, uint256 endTime);

    event AuctionSettled(uint256 indexed vrbId, address winner, uint256 amount);
...
interface ISeeder {
    ...
    function generateSeed(uint256 vrbId, IDescriptorMinimal descriptor) external returns (Seed memory);
}
