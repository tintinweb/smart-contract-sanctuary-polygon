...
error METH_Only_FND_Market_Allowed();
...
contract METH {
  ...
  /// @notice The  market contract with permissions to manage lockups.
  address payable private  Market;
   ...
   * @return  ETH
   */
  string public constant name = "METH Token";
  /**
  ...
  // Custom events
  /**
   * @notice Emitted when METH tokens are locked up by the  market for 24-25 hours
  ...
  /**
   * @notice Emitted when METH tokens are unlocked by the  market.
   ...
   * @dev This may be triggered by the user, an approved operator, or the  market.
  ...
  /// @dev Allows the  market permission to manage lockups for a user.
  modifier onlyMarket() {
    if (msg.sender != Market) {
      revert METH_Only_FND_Market_Allowed();
    }
    _;
  }
  ...
  /**
   * @notice Called once to configure the contract after the deployment.
   * @param _Market The address of the  NFT marketplace.
   */
  function initialize(address payable _Market) external  {
    require((msg.sender == initializer) && (initialized == false), "Invalid");
    initialized = true;
    if (!_Market.isContract()) {
   ...
   * These tokens are not subject to any lockup period.
   * @dev This may be used by the  market to credit a user's account with METH tokens.
  ...
  ) external payable onlyMarket returns (uint256 expiration) {
   ...
   * This will revert if the tokens were previously unlocked.
   */
  function marketWithdrawFrom(address from, uint256 amount) external onlyMarket {
  ...
  /**
   * @notice Gets the  market address which has permissions to manage lockups.
   * @return market The  market contract address.
   */
  function getMarket() external view returns (address market) {
    market = Market;
  }
