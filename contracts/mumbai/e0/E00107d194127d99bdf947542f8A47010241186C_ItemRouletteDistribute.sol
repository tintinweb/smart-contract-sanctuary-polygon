...
abstract contract RouletteDistributeBaseContract is CooperatableBase {
    ...
    event OnIntervalAdded(uint256 id,string name,uint32 range_min,uint32 range_max,uint32 quota,bool is_ssr,bool include_in_ssr_gurantee);
    event OnIntervalRemoved(uint256 id);
    event OnIntervalUpdated(uint256 id,string name,uint32 range_min,uint32 range_max,uint32 quota,bool is_ssr,bool include_in_ssr_gurantee);
    ...
    function _add_interval(string memory name_,uint32 min_,uint32 max_,uint32 quota_,bool ssr_,bool include_ssr_guarantee_) internal returns (uint256){
        ...
        emit OnIntervalAdded(_next_id,name_,min_,max_,quota_,ssr_,include_ssr_guarantee_);
        return  _next_id;
    }
    function _remove_interval(uint256 _id) internal {
        ...
        emit OnIntervalRemoved(_id);
    }

    function update_interval(uint256 id_,string memory name_,uint32 min_,uint32 max_,uint32 quota_,bool ssr_,bool include_ssr_guarantee_) external onlyOwner{

        require(defined[id_],"NO_INTERVAL");
        require(max_ >= min_,"INVALID_INTERVAL_RANGE");
        intervals[id_].name = name_;
        intervals[id_].min_value = min_;
        intervals[id_].max_value = max_;
        intervals[id_].distribute_quota = quota_;
        if(quota_ < intervals[id_].remain_quota)
            intervals[id_].remain_quota = quota_;
        intervals[id_].is_ssr = ssr_; 
        intervals[id_].include_in_ssr_gurantee = include_ssr_guarantee_;
        emit OnIntervalUpdated(id_,name_,min_,max_,quota_,ssr_,include_ssr_guarantee_);
    }
    function update_interval_range(uint256 id_,uint32 min_,uint32 max_) external onlyOwner {
        require(defined[id_],"NO_INTERVAL");
        require(max_ >= min_,"INVALID_INTERVAL_RANGE");
        intervals[id_].min_value = min_;
        intervals[id_].max_value = max_;
        emit OnIntervalUpdated(id_,intervals[id_].name,min_,max_,intervals[id_].distribute_quota,intervals[id_].is_ssr,intervals[id_].include_in_ssr_gurantee);
    }
    function update_interval_quota(uint256 id_,uint32 quota,bool reset_remaining) external onlyOwner {
        require(defined[id_],"NO_INTERVAL");
        ...
        emit OnIntervalUpdated(id_,intervals[id_].name,intervals[id_].min_value,intervals[id_].max_value,quota,intervals[id_].is_ssr,intervals[id_].include_in_ssr_gurantee);
    }
    function update_interval_name(uint256 id_,string memory name) external onlyOwner {
        require(defined[id_],"NO_INTERVAL");
        intervals[id_].name = name;
        emit OnIntervalUpdated(id_,name,intervals[id_].min_value,intervals[id_].max_value,intervals[id_].distribute_quota,intervals[id_].is_ssr,intervals[id_].include_in_ssr_gurantee);
    }
    function update_ssr_metadata(uint256 id_,bool ssr_,bool include_ssr_guarantee_) external onlyOwner {
        require(defined[id_],"NO_INTERVAL");
        intervals[id_].is_ssr = ssr_; 
        intervals[id_].include_in_ssr_gurantee = include_ssr_guarantee_ && ssr_;
        emit OnIntervalUpdated(id_,intervals[id_].name,intervals[id_].min_value,intervals[id_].max_value,intervals[id_].distribute_quota,ssr_,include_ssr_guarantee_);
    }
    ...
    function _do_guarantee_ssr(address _addr) internal returns (uint256){
        ...
        for(uint256 tid = 1; tid <= _interval_ids.current(); tid++){
            //Still have quota and ssr guarantee
            if(is_ssr_guarantee(tid) && is_on_quota(tid)){
        ...
        if((ssr_gurantee_count > 0) && (roulette_nounces[_addr] > ssr_gurantee_nounce)){
            uint256 found_interval_id = ssr_gurantees[overlap_nonnce%ssr_gurantee_count];
            deduct_quota(found_interval_id);
            overlap_nonnce++;
            return found_interval_id;
        }
        return 0;
    }
    //Do roulette and return match interval_id. 
    function _roulette(address _addr,uint32 rand_value) internal returns (bool,uint256,bool,uint32){
        ...
        for(uint256 tid = 1; tid <= _interval_ids.current(); tid++){
            if(defined[tid] && is_on_quota(tid) && is_in_range(tid,chosen_value)){
        ...
        bool got_ssr = false;
        bool is_ssr_guarantee_hit = false;
        found_interval_id = _do_guarantee_ssr(_addr);
        if(found_interval_id > 0){
           got_ssr = true;
           is_ssr_guarantee_hit = true;
        }
        else{
            if(overlap_count >= 1){
                if(overlap_count > 1){
        ...
        return (overlap_count >= 1,found_interval_id,is_ssr_guarantee_hit,chosen_value);
    }
    ...
    function is_ssr_guarantee(uint256 interval_id) internal view returns (bool){
    ...
    function get_ssr_guarantee_nounce() public view returns (uint32){return ssr_gurantee_nounce;}
...
contract ItemRouletteDistribute is RouletteDistributeBaseContract,IItemRouletteDistributor,ERC165 {
    
    using ERC165Checker for address;
   
    event OnRoulette(address indexed _addr,uint32 value,RewardType reward_type,address indexed contract_address,uint256 indexed token_id,uint256 amount,bool is_ssr_guarantee_hit,bool is_default_reward);
    event OnRewardSet(uint256 id,RewardType reward_type,address indexed contract_address,uint256 indexed token_id,uint256 min_amount,uint256 max_amount);
    event OnRewardSetUnset(uint256 id);
    
    mapping(uint256 => Reward) private rewards;    
    Reward[] private default_rewards;
    address private randomness_address;

    constructor(uint32 total_range_,uint32 ssr_gurantee_nounce_,address randomness_address_)
    ...
    function _set_randomness_internal(address _addr) internal{
        require(_addr != address(0) && _addr.supportsInterface(type(IRandomnessConsumer).interfaceId),"NOT_RANDOMNESS");
    ...
    function remove_reward(uint256 interval_id) external onlyOwner {
        ...
        emit OnRewardSetUnset(interval_id);
    }
    function _do_roulette(address _addr,uint32 rand_value) internal returns (RewardType,address,uint256,uint256){
       
        (bool found,uint256 interval_id,bool is_ssr_guarantee_hit,uint32 chosen_value) = _roulette(_addr, rand_value);
        Reward memory _reward = (found && (interval_id > 0)) ? get_reward(interval_id) : sampling_default_reward(_addr);
        uint256 amount = sampling_amount(_reward,rand_value);

        emit OnRoulette(_addr,chosen_value,_reward.reward_type,_reward.contract_address,_reward.token_id,amount,is_ssr_guarantee_hit,!found || (interval_id == 0));
        return (_reward.reward_type,_reward.contract_address,_reward.token_id,amount);
    }
    function roulette(address _addr) external onlyOwnerOrOperator returns (RewardType,address,uint256,uint256){
       require(randomness_address != address(0),"NO_RANDOMNESS");
       IRandomnessConsumer randomness = IRandomnessConsumer(randomness_address);
       return _do_roulette(_addr,uint32(randomness.random(type(uint32).max)));
    }
    function test_roulette(address _addr,uint32 rand_value) external onlyOwner returns (RewardType,address,uint256,uint256){
       return _do_roulette(_addr,rand_value);
    }
    function sampling_amount(Reward memory _reward,uint32 rand_value) internal pure returns (uint256){
    ...
    function sampling_default_reward(address _addr) internal view returns (Reward memory) {
        require(default_rewards.length > 0,"NO_DEF_REWARD");
    ...
    function is_available(address contract_address,uint256 token_id) public view returns (bool){
        for(uint256 tid = 1; tid <= get_last_interval_id(); tid++){
            if(is_defined(tid)){
                if(rewards[tid].contract_address == contract_address && 
                   rewards[tid].token_id == token_id){
                    if(is_on_quota(tid)){
                        return true;
                    }
                }
    ...
    function _set_reward_as_token(uint256 interval_id,address token_contract,uint256 min_amount,uint256 max_amount) internal{
        ...
        emit OnRewardSet(interval_id,RewardType.TOKEN,token_contract,0,min_amount,max_amount);
    }
    function _set_reward_as_multitoken(uint256 interval_id,address token_contract,uint256 token_id,uint256 min_amount,uint256 max_amount) internal{
        ...
        emit OnRewardSet(interval_id,RewardType.ERC1155,token_contract,token_id,min_amount,max_amount);
    }
    function _set_reward_as_individual(uint256 interval_id,address token_contract,uint256 min_amount,uint256 max_amount) internal{
        ...
        emit OnRewardSet(interval_id,RewardType.ERC721,token_contract,0,min_amount,max_amount);
    }
    function _form_token_reward(address token_contract,uint256 min_amount,uint256 max_amount) internal view returns (Reward memory){
        require(token_contract != address(0) && token_contract.supportsInterface(type(IERC20TokenContract).interfaceId),"NOT_ERC20");
    ...
    function _form_erc1155_reward(address token_contract,uint256 token_id,uint256 min_amount,uint256 max_amount) internal view returns (Reward memory){
        require(token_contract != address(0) && token_contract.supportsInterface(type(IERC1155TokenContract).interfaceId),"NOT_ERC1155");
        require(min_amount <= max_amount,"INVALID_RANGE");
    ...
    function _form_erc721_reward(address token_contract,uint256 min_amount,uint256 max_amount)  internal view returns (Reward memory){
        require(token_contract != address(0) && token_contract.supportsInterface(type(IERC721TokenContract).interfaceId),"NOT_ERC721");
        require(min_amount <= max_amount,"INVALID_RANGE");
        return Reward(RewardType.ERC721,token_contract,0,min_amount,max_amount);
    }
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return
            interfaceId == type(IItemRouletteDistributor).interfaceId ||
