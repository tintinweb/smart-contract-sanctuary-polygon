...
interface IAuctionHouse {
    ...
    event MinBidIncrementAmountChanged(uint256 _oldIncreament,uint256 _newIncrement);
    event TreasuryAddressChanged(address _oldTreasuryAddress,address _newTreasuryAddress);
...
contract Whitelist is Ownable {
    ...
    function addCollectionAddress(address _address) external onlyOwner {
        require(
            ...
            "Collection WhiteList: address already whitelisted"
        );
        whitelistedMap[_address] = true;
    ...
    function removeCollectionAddress(address _address) external onlyOwner {
        require(
            ...
            "Collection WhiteList: address already removed"
        );
        whitelistedMap[_address] = false;
    ...
    ERC721Holder
{
    ...
    uint256 public plateformFeePercentage;
    ...
    address public LNQTokenAddress;
    ...
    // set all plateform fee transsfer to this address
        ...
        ERC2771Context(trustedForwarder)
    {
        ...
        plateformFeePercentage = 500; // for 5% fee
        timeBuffer = 0;
    ...
    modifier isWhitelisted(address tokenAddress) {
        require(whitelisted(tokenAddress), "is not whitelisted collection");
        _;
    }
    ...
    modifier orderExists(uint256 orderId) {
        require(_exists(orderId, false), "Order doesn't exist");
        _;
    }
    ...
    modifier onlyValidUser() {
        require(
            ...
            "Only Valid user can create auction"
        );
        _;
    }
    ...
    function createSaleOrder(
        ...
        address tokenContract,
        uint256 reservePrice
    ) external nonReentrant isWhitelisted(tokenContract) returns (uint256) {
        require(
        ...
        orderExists(orderId)
    {
        require(
            ...
            "price must be difference"
        );
        require(
            _msgSender() == saleOrder[orderId].tokenOwner,
            "Must be sale order owner can change"
        );
    ...
    function buyOrder(uint256 orderId, uint256 amount)
        ...
        nonReentrant
    {
        uint256 plateformFee = 0;
        uint256 royaltyFee = 0;
        Order storage order = saleOrder[orderId];
        uint256 tokenOwnerProfit = order.reservePrice;
        require(
            amount == tokenOwnerProfit,
            "amount must be equal to sale order price"
        );
        ...
        if (treasury != address(0)) {
            plateformFee =
                (tokenOwnerProfit * plateformFeePercentage) /
                (10000);
            _handleOutgoingBid(treasury, plateformFee, LNQTokenAddress);
        }
        if (checkRoyalties(order.tokenContract)) {
            (address reciver, uint256 royaltyAmount) = IERC2981(
                order.tokenContract
            ).royaltyInfo(order.tokenId, tokenOwnerProfit);
            if (reciver != address(0)) {
                _handleOutgoingBid(reciver, royaltyAmount, LNQTokenAddress);
                emit RoyaltyTransafer(order.tokenId,order.tokenContract,reciver,royaltyAmount);
                royaltyFee = royaltyAmount;
            }
        }
        tokenOwnerProfit = tokenOwnerProfit - (plateformFee + (royaltyFee));
        _handleOutgoingBid(
            order.tokenOwner,
            tokenOwnerProfit,
            LNQTokenAddress
        ...
        returns (uint256)
    {
        ...
        require(
            ...
            "start time must be greater than equal to current time "
        );
        if (startTime == 0) {
        ...
        nonReentrant
    {
        ...
        require(
            ...
            "Must send more than last bid by minBidIncrementAmount amount"
        );
        ...
        nonReentrant
    {
        ...
        if (checkRoyalties(auction.tokenContract)) {
            ...
            if (reciver != address(0)) {
                _handleOutgoingBid(reciver, royaltyAmount, LNQTokenAddress);
                emit RoyaltyTransafer(auction.tokenId,auction.tokenContract,reciver,royaltyAmount);
                royaltiyFee = royaltyAmount;
            }
        }
        tokenOwnerProfit = tokenOwnerProfit - (plateformFee + (royaltiyFee));
        _handleOutgoingBid(
            auction.tokenOwner,
            tokenOwnerProfit,
            LNQTokenAddress
    ...
    /**
     *@dev owner can plateform fee percentage
     */
    function changePlateFormFeePercentage(uint256 _plateformFeePercentage)
        external
        onlyOwner
    {
        require(
            plateformFeePercentage != _plateformFeePercentage,
            "plateformFeePercentage is already same"
        );
        emit platFormFeeChanged(plateformFeePercentage,_plateformFeePercentage);
        plateformFeePercentage = _plateformFeePercentage; 
    }
        ...
        onlyOwner
    {
        require(
            ...
            "state is already same"
        );
        emit AuctionCreatorStateChanged(isAuctionRestricted ,_isAuctionRestricted);
    ...
    function changeBufferTime(uint256 _timeBuffer) external onlyOwner {
        require(timeBuffer != _timeBuffer, "_timeBuffer is already same");
        ...
        onlyOwner
    {
        require(
            ...
            "minBidIncrementAmount is already same"
        );
        emit MinBidIncrementAmountChanged(minBidIncrementAmount,_minBidIncrementAmount);
        minBidIncrementAmount = _minBidIncrementAmount;
    }
    ...
    function _cancelOrder(uint256 orderId) internal {
        ...
        IERC721(saleOrder[orderId].tokenContract).safeTransferFrom(
            address(this),
            tokenOwner,
            saleOrder[orderId].tokenId
        );
