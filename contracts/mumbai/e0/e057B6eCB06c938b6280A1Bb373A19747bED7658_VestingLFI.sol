		...
		if (vestingSchedulesById[id].isEnded) {
			...
			if (block.timestamp < vestingSchedulesById[id].startTime) {
				...
				if (
					...
					_amount = vestingSchedulesById[id].allocatedAmount * (block.timestamp - vestingSchedulesById[id].startTime) / vestingSchedulesById[id].vestingPeriod - vestingSchedulesById[id].vestedAmount;
				} else {
					_amount = 0;
				}
			}
