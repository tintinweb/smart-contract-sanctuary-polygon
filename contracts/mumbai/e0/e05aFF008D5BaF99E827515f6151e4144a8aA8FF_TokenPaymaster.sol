...
contract TokenPaymaster is BasePaymaster {
    ...
    /// first assume we only collect single erc20 token as fee
    /// the exchange rate is omitted
    ...
    constructor(
        ...
        // IUniswapV3[] memory _uniswaps
    ) {
        // uniswaps = _uniswaps;
        _paymentToken = IERC20(tokenAddress_);
    ...
    returns (address payer, uint256 tokenPreCharge) {
        ...
        // we assume the token price is 1:1 with matic
        tokenPreCharge = ethMaxCharge;
    ...
    returns (bytes memory context, bool revertOnRecipientRevert) {
        ...
        (address payer, uint256 tokenPrecharge) = _calculatePreCharge(token, relayRequest, maxPossibleGas);
        // since the transfer amount may be not allowed at this point, we will do it after relay call
    ...
    function _postRelayedCallInternal(
        ...
        // override, assume price is 1:1
        uint256 tokenActualCharge = ethActualCharge;
        token.transferFrom(payer, address(this), tokenActualCharge);
        // uint256 tokenRefund = tokenPrecharge - tokenActualCharge;
        // _refundPayer(payer, token, tokenRefund);
        /// we don't have to automatically deposit to hub, since we can do it in batch
