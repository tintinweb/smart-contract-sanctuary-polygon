...
contract Boynds is ERC721, Ownable {
    ...
    constructor() ERC721("Boynds", "BOYND") {}
    ...
    function isAdmin(address _address) internal virtual returns (bool) {
        ...
        return IAdmin(AdminsContract).isAdmin(_address);
    ...
    function withdraw(uint256 amount) public {
        ...
        payable(BankAddress).transfer(amount);
    ...
    function mint(uint256 amount) public payable {
        ...
        require(Minted + amount <= MaxSupply, "Not enough supply");

        require(msg.value >= amount * Rate, "Insufficient amount sent");

        // Check Whitelist

        address minter = msg.sender;

        (bool whitelisted, string memory whitelist_error, uint max_mint) = IWhitelist(WhitelistContract).isWhitelisted(minter);

        require(whitelisted, whitelist_error);

        // Check max mint

        require(amount <= max_mint, string(abi.encodePacked("You can only mint ", Strings.toString(max_mint))));
        ...
        IWhitelist(WhitelistContract).addMinted(minter, amount);
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId, uint256 batchSize) internal override(ERC721) {
        ...
        if (batchSize > 1) {
            ...
            revert("ERC721: consecutive transfers not supported");
        ...
        if(balanceOf(to) == 0) {
            ...
            Holders[to].time = block.timestamp;
        ...
        if(from != address(0)) {
            ...
            for(uint256 i; i < Holders[from].tokens.length; i++) {

                if(Holders[from].tokens[i] != tokenId) {

                    Holders[from].tokens[j] = Holders[from].tokens[i];
        ...
        if(to != address(0)) {
            ...
            Holders[to].tokens.push(tokenId);
    ...
    function getHolderTime(address holder) public view returns (uint256) {

        if(balanceOf(holder) == 0) return 0;

        return Holders[holder].time;

    }

    function getHolderTokens(address holder) public view returns (uint256[] memory) {

        return Holders[holder].tokens;
