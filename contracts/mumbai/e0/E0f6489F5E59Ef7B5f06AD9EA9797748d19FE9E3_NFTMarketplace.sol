...
contract NFTMarketplace is ReentrancyGuard, ERC721URIStorage, ERC2981, Ownable{
    ...
    function mintNFT(string memory uri, uint256 nftRoyaltyPercent) public payable nonReentrant{
        ...
        require(nftRoyaltyPercent >= 0 && nftRoyaltyPercent <= 50, "NFT Royalty should be in-between 0 & 50");
        _tokenIds.increment();
        uint256 newTokenId = _tokenIds.current();
        _mint(msg.sender, newTokenId);
        _setTokenURI(newTokenId, uri);
        _tokenIDURI[newTokenId] = uri;
        MarketItemDatabase[newTokenId] = MarketItem(newTokenId, _scAddress, uri, payable(msg.sender), payable(address(0)), 0, nftRoyaltyPercent, false);
        emit MarketItemCreated(newTokenId, _scAddress, uri, msg.sender, address(0), 0, nftRoyaltyPercent, false);
    }
    function listNFT(uint256 tokenId, uint256 price) public {
        ...
        _setTokenRoyalty(MarketItemDatabase[tokenId].tokenId, msg.sender, uint96(MarketItemDatabase[tokenId].nftRoyaltyPercent));
    }
