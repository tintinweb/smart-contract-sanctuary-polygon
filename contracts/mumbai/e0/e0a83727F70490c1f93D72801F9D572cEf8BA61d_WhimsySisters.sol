...
contract WhimsySisters is ERC721A, Ownable {
    ...
    uint public price = 0.0007 ether;
    ...
    address public refundWallet = 0x316c72934e91f0671E3C7cd765f1c15E06382318;
    uint public maxTimestampForRefund = 1652486400;

    address public authorizedSigner = 0xf89C869460AA20b8A0982A5D90f2470E8f6EbC65;
    address[] public friendCollections = [0xe785E82358879F061BC3dcAC6f0444462D4b5330, 0x5180db8F5c931aaE63c74266b211F580155ecac8, 0x2eFa07CaC3395599db83035D196F2A0e7263F766];
    ...
    function setFriendCollections(address[] calldata collections) external onlyOwner {
        friendCollections = collections;
    }
    ...
    function configure(
        ...
        maxTimestampForRefund = _maxTimestampForRefund;
    ...
    function isAllowedToPresale(address account, bytes calldata signature) public view returns (bool) {
        if (signature.length > 0) {
            return recoverSignerAddress(account, signature) == authorizedSigner;
        } else {
            for (uint i = 0; i < friendCollections.length; i++) {
                if (IERC721(friendCollections[i]).balanceOf(account) > 0) {
                    return true;
                }
            }
        }
    ...
    function presale(uint amount, bytes calldata signature) public payable {
        ...
        require(isAllowedToPresale(_msgSender(), signature), "msg.sender is not allowed to presale");
    ...
    function isApprovedForAll(address _owner, address operator) public view override returns (bool) {
        // Approve contract address for refunds
    ...
    function refund(uint[] calldata tokenIds) external {
        require(block.timestamp <= maxTimestampForRefund, "Refund program is over");
        require(tokenIds.length > 0, "Empty tokenIds array");
        for (uint i = 0; i < tokenIds.length; i++) {
            safeTransferFrom(_msgSender(), refundWallet, tokenIds[i], "");
        }
        payable(_msgSender()).transfer(price * tokenIds.length);
    }
