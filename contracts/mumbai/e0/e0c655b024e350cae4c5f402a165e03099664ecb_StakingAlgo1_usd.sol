...
interface IERC20 {
    ...
    // only for HRTERC20 token
    function transferFeeAmount(address from, uint256 amount)
        external
        view
        returns (uint256);
}

contract StakingAlgo1_usd is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    struct UserInfo {
        ...
        uint256 rewardPayouts; // available reward amount at latest action
        // total reward amount at the time of the latest action like deposit, claim
        // ALA is the abbreviation for At the time of the Latest Action
        uint256 totalRewardALA;
    }
    ...
    uint256 public vip1MinAmount;
    uint256 public vip1LockingPeriod;
    uint256 public vip1RewardPercent;
    uint256 public vip2MinAmount;
    uint256 public vip2LockingPeriod;
    uint256 public vip2RewardPercent;
    uint256 public rewardPercent;
    uint256 public minDepositAmount;
    uint256 public depositFeePc;
    uint256 public depositFeePcForVip1;
    uint256 public rewardCycle;
    uint256 public APYDENOMINATOR;
    ...
    IERC20 public depositToken; // USD stable coin
    IERC20 public rewardToken; // HRT token
    ...
    modifier checkActive() {
        require(block.timestamp >= startDate && !paused, "can not do now");
        _;
    }

    // USD stable coin - HRT pool
    // usdc-hrt, usdt-hrt, dai-hrt, miMATIC-hrt, etc
    function initialize(
        uint256 _startDate,
        ...
        vip1MinAmount = 100000; // $100000
        vip1LockingPeriod = 60 * 60 * 24 * 365; // 1 year
        vip1RewardPercent = 90; // 90% of totalReward
        vip2MinAmount = 1000000; // $1000000
        vip2LockingPeriod = 60 * 60 * 24 * 365 * 3; // 3 year
        vip2RewardPercent = 100; // 100% of totalReward
        rewardPercent = 80; // 80% of totalReward
        minDepositAmount = 100; // $100
        depositFeePc = 50; // 5%
        depositFeePcForVip1 = 25; // 2.5%
        rewardCycle = 7; // 7 days
        APYDENOMINATOR = 100 * 10**6;
        DENOMINATOR = 1000;
        USD = 10**8; // usd decimals is 8
        paused = false;

        startDate = _startDate;
        require(_admin != address(0), "can't be zero address");
        require(_depositToken != address(0), "can't be zero address");
        require(_rewardToken != address(0), "can't be zero address");
        admin = _admin;
        depositToken = IERC20(_depositToken); // usd stable token
        rewardToken = IERC20(_rewardToken); // HRT token
    }

    /** change only owner functions */
    function changeAdmin(address _admin) external onlyOwner {
        require(_admin != address(0), "can't be zero address");
        admin = _admin;
    }

    function changeDepositToken(address _depositToken) external onlyOwner {
        require(_depositToken != address(0), "can't be zero address");
    ...
    function changeStartDate(uint256 _newDate) external onlyOwner {
        startDate = _newDate;
    }

    function changeDepositFeePc(uint256 _feePc, uint256 _vip1FeePc)
        external
        onlyOwner
    {
        depositFeePc = _feePc;
        depositFeePcForVip1 = _vip1FeePc;
    }
    ...
    function changeMinDepositAmount(uint256 _newAmount) external onlyOwner {
        minDepositAmount = _newAmount;
    }

    function changeRewardCycle(uint256 _days) external onlyOwner {
        require(_days > 0, "reward cycle can't be 0");
        rewardCycle = _days;
    }
    ...
    function allTokenWithdraw() external onlyOwner {
        paused = true;
        totalDeposit = 0;
        totalReward = 0;
    ...
    // call only admin function
    function addReward(uint256 _amount) external onlyAdmin {
        rewardToken.transferFrom(msg.sender, address(this), _amount);

        uint256 _burnAmount = rewardToken.transferFeeAmount(
            msg.sender,
            _amount
        );
    ...
    function deposit(uint256 _amount) external nonReentrant checkActive {
        ...
        require(
            _amount / (10**_decimals) >= minDepositAmount,
            "amount is less than min deposit amount"
        );
        depositToken.transferFrom(msg.sender, address(this), _amount);

        uint256 _vip1MinAmount = vip1MinAmount * (10**_decimals);
        uint256 _vip2MinAmount = vip2MinAmount * (10**_decimals);

        // deposit fee
        uint256 _fee = 0;
        uint256 _DENOMINATOR = DENOMINATOR;
        if (_amount < _vip1MinAmount) {
            _fee = (_amount * depositFeePc) / _DENOMINATOR;
        } else if (_amount < _vip2MinAmount) {
            _fee = (_amount * depositFeePcForVip1) / _DENOMINATOR;
        }

        if (_fee > 0) {
            // send fee to owner
            depositToken.transfer(owner(), _fee);
        }

        userInfo[msg.sender].rewardPayouts = getRewardAmount(msg.sender);

        uint256 _actualAmount = _amount - _fee;
        totalDeposit += _actualAmount;

        uint256 _newDepositedAmount = userInfo[msg.sender].depositedAmount +
            _actualAmount;
        userInfo[msg.sender].depositedAmount = _newDepositedAmount;
        userInfo[msg.sender].totalRewardALA = totalReward;

        if (_newDepositedAmount >= _vip2MinAmount) {
            userInfo[msg.sender].lockedTime =
                block.timestamp +
                vip2LockingPeriod;
        } else if (_newDepositedAmount >= _vip1MinAmount) {
            userInfo[msg.sender].lockedTime =
                block.timestamp +
                vip1LockingPeriod;
        }

        emit Deposit(msg.sender, _actualAmount, _fee, block.timestamp);
    }
    ...
    function withdraw() external nonReentrant checkActive {
        ...
        depositToken.transfer(msg.sender, _withdrawAmount);
        if (_reward > 0) {
            rewardToken.transfer(msg.sender, _reward);
        }

        emit Withdraw(msg.sender, _withdrawAmount, _reward, block.timestamp);
    }
    ...
    function claim() external nonReentrant checkActive {
        ...
        emit Claim(msg.sender, _reward, block.timestamp);
    }
    ...
    function getRewardAmount(address _user) public view returns (uint256) {
        require(_user != address(0), "can't be zero address");
    ...
    function apyNotVip() external view returns (uint256) {
        /**
         * rewardPercent - user(not vip)'s reward percent, default is 80% of totalReward
         */
        return calculateAPY(rewardPercent);
    }

    function apyOfVip1() external view returns (uint256) {
        /**
         * vip1RewardPercent - vip1 user's reward percent, default is 90% of totalReward
         */
        return calculateAPY(vip1RewardPercent);
    }
    ...
    function apyOfVip2() external view returns (uint256) {
         ...
         */
        return calculateAPY(vip2RewardPercent);
    }

    function calculateAPY(uint256 _rewardPercent) private view returns (uint256) {
        /**
         * totalDeposit - total deposited amount
         * APYDENOMINATOR - decimal places, default is 6: 100000000
         * 365 - days of 1 year
         * rewardPercent - reward percent
         * latestReward - latest reward amount set by the owner
        ...
        if (totalDeposit > 0) {
            apy =
                (latestReward * APYDENOMINATOR * 365 * _rewardPercent) /
                (totalDeposit * rewardCycle * 100);
        }
    ...
    function getUserStatus(address _user) public view returns (uint8) {
        ...
        uint256 _decimals = uint256(depositTokenDecimals());
        uint256 _usdAmountOfUser = userInfo[_user].depositedAmount /
            10**_decimals;
