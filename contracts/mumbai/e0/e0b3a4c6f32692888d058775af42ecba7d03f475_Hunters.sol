...
contract Hunters is ERC721, Ownable, Pausable {
  ...
  string constant ERR_LOCK_BALANCE = "Account doesn't own hunter";
  string constant ERR_LOCKED = "Hunter is locked";

  using Strings for uint256;
  using Counters for Counters.Counter;

  Counters.Counter private supply;
  mapping (address => uint) hunterCounts;
  ...
  function lockHunter(
      ...
      require(ownerOf(id) == account, ERR_LOCK_BALANCE);
  ...
  function isHunterLocked(
      uint256 id
  ) public view returns (bool) {
      return (lockedHunters[id] != address(0x0));
  }

  function getHunterLocker(
      uint256 id
  ...
  function publicSaleMintHunter(uint256 _mintAmount) external payable mintCompliance(_mintAmount) onlyOrigin {
    require(isPublicMintActive, "Public mint is not active!");
    require(msg.value >= cost * _mintAmount, "Insufficient funds!");
    require(hunterCounts[msg.sender] + _mintAmount <= maxMintAmountPerWallet, "Exceeds mint amount per wallet!");
  ...
  function preSaleMintHunter(uint256 _mintAmount, bytes32[] calldata proof) external payable mintCompliance(_mintAmount) onlyOrigin {
  ...
  function _beforeTokenTransfer(address from, address to, uint256 tokenId)
        ...
        override
    {
        require(!isHunterLocked(tokenId), ERR_LOCKED);
