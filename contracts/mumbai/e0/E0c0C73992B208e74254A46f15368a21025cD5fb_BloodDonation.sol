...
contract BloodDonation is ERC721, Ownable {
    ...
    event DonorRegistered(uint256 indexed donorId, string name, string bloodType, string userAddress,string mobile, int256 latitude,int256 longitude,address walletaddress);
    event BloodBankRegistered(uint256 indexed bloodBankId, string name, string userAddress, string mobile, int256 latitude, int256 longitude, address walletaddress);
    event BloodRequest(uint256 indexed requestId, uint256 indexed bloodBankId, string bloodType,uint quantity, uint isCompleted, uint createdDate, uint dueDate);
    ...
    event BloodRequestCompleted(uint indexed requestId, uint indexed bloodbankId);
    constructor() ERC721("BloodDonation", "BDN") {}
    function registerDonor(string memory name, string memory bloodType, string memory userAddress,string memory mobile, int256 latitude,int256 longitude,address walletaddress) public {
        uint256 donorId = _tokenIds.current();
        _tokenIds.increment();
        donors[donorId] = Donor(donorId, name, bloodType, userAddress, mobile, latitude, longitude, walletaddress);
        emit DonorRegistered(donorId, name, bloodType, userAddress, mobile, latitude, longitude, walletaddress);
    }
    function registerBloodBank(string memory name, string memory userAddress,string memory mobile, int256 latitude,int256 longitude,address walletaddress) public onlyOwner {
        uint256 bloodBankId = _tokenIds.current();
        _tokenIds.increment();
        bloodBanks[bloodBankId] = BloodBank(bloodBankId, name, userAddress, mobile, latitude, longitude, walletaddress);
        emit BloodBankRegistered(bloodBankId, name, userAddress, mobile, latitude, longitude, walletaddress);
    }
    // function requestBlood(uint256 bloodBankId, string memory bloodType, uint quantity, bool isCompleted ) public onlyOwner {
    ...
    //     emit BloodRequest(requestId, bloodBankId, bloodType,quantity,isCompleted );
    // }
    function createBloodRequest(string memory bloodType,uint quantity, uint dueDate, uint256 bloodBankId) public {
        _requestIds.increment();
        uint256 requestId = _requestIds.current();
        require(bloodBanks[bloodBankId].bloodBankId != 0, "Blood bank not registered.");
        bloodrequests[requestId] = BloodRequestDetails(requestId,bloodType, quantity, false, block.timestamp,dueDate,bloodBankId);
    }
    function acceptBloodRequest(uint256 requestId, uint256 donorId) public {
        require(donors[donorId].donorId != 0, "Donor not registered.");
    ...
    function getRequestDetails(uint256 tokenId) public view returns (uint256 requestId, uint256 bloodBankId, string memory bloodType) {
        ...
        require(bloodBank.bloodBankId != 0, "Blood bank not registered.");
        bytes32 bloodTypeHash = bytes32(requestId % 1000000);
        bloodType = _hashToBloodType(bloodTypeHash);
    }
    function completedBloodRequest(uint requestId) public {
        require(requestId != 0, "Request not found.");
        BloodRequestDetails memory bloodRequest = bloodrequests[requestId];
        bloodRequest.isCompleted = true;
        bloodrequests[requestId] = bloodRequest;
        emit BloodRequestCompleted(requestId, bloodRequest.bloodBankId);
    }
