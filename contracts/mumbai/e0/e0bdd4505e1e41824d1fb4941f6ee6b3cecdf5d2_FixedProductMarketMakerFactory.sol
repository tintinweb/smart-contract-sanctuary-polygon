...
contract FixedProductMarketMakerFactory {
    ...
    function createMarket(
        ...
        bytes32 salt = keccak256(abi.encode(conditionalTokens, collateralToken, oracle, questionId, outcomeSlotCount));
        ConditionID conditionId = conditionalTokens.prepareCondition(oracle, questionId, outcomeSlotCount);

        address clone = Clones.cloneDeterministic(fpmmTemplate, salt);

        FixedProductMarketMaker fpmm = FixedProductMarketMaker(clone);
        fpmm.initialize(conditionalTokens, collateralToken, conditionId, haltTime, fee);
...
interface IConditionalTokens is IERC1155Upgradeable {
    ...
    error InvalidIndex();
    error PositionsAlreadyRedeemOrNonexistent();
...
contract FixedProductMarketMaker is IMarketMaker, Initializable, ERC20Upgradeable, ERC1155ReceiverUpgradeable {
