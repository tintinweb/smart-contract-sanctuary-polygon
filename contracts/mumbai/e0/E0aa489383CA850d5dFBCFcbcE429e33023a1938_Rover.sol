...
error Rover__IncorrectStatus(uint256 from, uint256 to);
error Rover__InvalidSkin(uint256 skin);
error Rover__MaxSupplyReached();
error Rover__PaymentTooLow();
error Rover__NotMC();

contract Rover is ERC721EnumerableUpgradeable, OwnableUpgradeable, ERC1155HolderUpgradeable {
    IERC20Upgradeable IXT;
    IERC1155Upgradeable astroCredits;
    IERC20 USDT;
    IERC20 WMATIC;
    ...
    string[] public uris;
    mapping(uint256 => uint256) public ixtSkinPrices;
    mapping(uint256 => uint256) public acSkinPrices;
    mapping(uint256 => RoverInfo) public rovers;

    uint256 ixtNamePrice;
    uint256 acNamePrice;

    uint256 ixtRepairWornPrice;
    uint256 ixtRepairBrokenPrice;

    uint256 acRepairWornPrice;
    uint256 acRepairBrokenPrice;
    ...
    function initialize(
        ...
        if (_WMATIC == address(0)) revert Rover__ZeroAddress();
        if (_astroCredits == address(0)) revert Rover__ZeroAddress();

        setMissionControl(_missionControl);
        ...
        ixtRepairWornPrice = 1 ether;
        ixtRepairBrokenPrice = 2.5 ether;

        acRepairWornPrice = 30 ether;
        acRepairBrokenPrice = 80 ether;
    ...
    function purchaseWithUSDT(uint32 amount) public returns (uint256 requestId) {
        USDT.transferFrom(msg.sender, address(this), 10_000_000 * amount);
        requestId = _vrfRequest(amount);
    }
    ...
    function purchaseWithMatic(uint32 amount) public payable returns (uint256 requestId) {
        uint256 amountToPay = s_oracle.getAmountOut(address(USDT), address(WMATIC), 10 * amount);

        // convert to full decimals
        uint256 total = amountToPay * 1e18;
        if (msg.value < total) {
    ...
    function purchaseWithIXT(uint32 amount) public returns (uint256 requestId) {
        uint256 wmaticAmount = s_oracle.getAmountOut(address(USDT), address(WMATIC), 10 * amount);
        uint256 amountToPay = s_oracle.getAmountOut(address(WMATIC), address(IXT), wmaticAmount);

        // convert to full decimals
        uint256 total = amountToPay * 1e18;
        IXT.transferFrom(msg.sender, address(this), total);
        requestId = _vrfRequest(amount);
    }
    ...
    function _vrfRequest(uint32 amount) internal returns (uint256 requestId) {
        ...
        // requestId = s_randNumOracle.getRandomNumber(amount);
        // s_requests[requestId] = msg.sender;
        for (uint256 i = 0; i < amount; ) {
            uint256 tokenId = totalSupply();
            uint256 randomWord = uint256(keccak256(abi.encodePacked(block.timestamp))) % 100;
            Color color;
            if (randomWord < 89) {
                color = Color.White;
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        string memory image;
        if (rover.skin == 0) {
            uint256 index = 4 * uint256(rover.color) + uint256(rover.status);
            image = uris[index];
        } else {
            image = uris[rover.skin];
        }

        string memory color;
        if (rover.color == Color.White) {
            color = "White";
        } else if (rover.color == Color.Green) {
            color = "Green";
        } else {
            color = "Black";
        }
        string memory tokenIdHex;
        if (tokenId == 0) {
            tokenIdHex = "0";
        } else {
            tokenIdHex = string(abi.encodePacked(tokenId));
        }
        string memory attributes = string.concat('"attributes":[{"trait_type":"NFT","value":"Rover"},{"trait_type":"ID","value":"', tokenIdHex, '"},{"trait_type":"Color","value":"', color, '"}]');
        string memory json = string.concat('{"name": "', rover.name, '", "description": "Planet IX Rover", "image": "', image, '",', attributes, "}");
    ...
    function addSkin(
        ...
        uris.push(uri);
        ixtSkinPrices[uris.length - 1] = ixtPrice;
        acSkinPrices[uris.length - 1] = acPrice;
    }

    /** Repairs **/
    function repairWornIXT(uint256 tokenId) public {
        IXT.transferFrom(msg.sender, address(this), ixtRepairWornPrice);
        _repair(tokenId, Status.Worn, Status.Pristine);
    }

    function repairBrokenIXT(uint256 tokenId) public {
        IXT.transferFrom(msg.sender, address(this), ixtRepairBrokenPrice);
        _repair(tokenId, Status.Broken, Status.Pristine);
    }

    function repairWornAC(uint256 tokenId) public {
        astroCredits.safeTransferFrom(msg.sender, address(this), acTokenId, acRepairWornPrice, "");
        _repair(tokenId, Status.Worn, Status.Pristine);
    }

    function repairBrokenAC(uint256 tokenId) public {
        astroCredits.safeTransferFrom(msg.sender, address(this), acTokenId, acRepairBrokenPrice, "");
        _repair(tokenId, Status.Broken, Status.Pristine);
    }
    ...
    function _repair(
        ...
        if (rovers[tokenId].status != from) revert Rover__IncorrectStatus(uint256(from), uint256(to));
        rovers[tokenId].status = to;
    }

    function _breakdown(uint256 tokenId, Status to) external {
        if (msg.sender != missionControl) revert Rover__NotMC();
        rovers[tokenId].status = to;
    }

    /** Set Name **/
    function setNameWithIXT(uint256 tokenId, string memory name) public {
        IXT.transferFrom(msg.sender, address(this), ixtNamePrice);
        _setName(tokenId, name);
    }

    function setNameWithAC(uint256 tokenId, string memory name) public {
        astroCredits.safeTransferFrom(msg.sender, address(this), acTokenId, acNamePrice, "");
        _setName(tokenId, name);
    }
    ...
    function upgradeSkinIXT(uint256 tokenId, uint256 _skin) public {
        if (ixtSkinPrices[_skin] == 0) revert Rover__InvalidSkin(_skin);
        IXT.transferFrom(msg.sender, address(this), ixtSkinPrices[_skin]);
        _upgradeSkin(tokenId, _skin);
    }

    function upgradeSkinAC(uint256 tokenId, uint256 _skin) public {
        if (acSkinPrices[_skin] == 0) revert Rover__InvalidSkin(_skin);
        astroCredits.safeTransferFrom(msg.sender, address(this), acTokenId, acSkinPrices[_skin], "");
        _upgradeSkin(tokenId, _skin);
    }

    function _upgradeSkin(uint256 tokenId, uint256 _skin) internal {
        if (ownerOf(tokenId) != msg.sender) revert Rover__NotOwner();
    ...
    function withdraw() public onlyOwner {
        ...
        astroCredits.safeTransferFrom(address(this), msg.sender, acTokenId, astroCredits.balanceOf(address(this), acTokenId), "");
    ...
    //     requestID = COORDINATOR.requestRandomWords(keyHash, subscriptionId, requestConfirmations, callbackGasLimit, uint32(1));
    // }
    ...
    /** Interface **/
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC1155ReceiverUpgradeable, ERC721EnumerableUpgradeable) returns (bool) {
        return interfaceId == type(IERC721EnumerableUpgradeable).interfaceId || super.supportsInterface(interfaceId);
    }
}