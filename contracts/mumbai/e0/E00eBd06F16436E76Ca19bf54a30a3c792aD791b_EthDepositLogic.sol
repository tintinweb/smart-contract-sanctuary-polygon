...
contract EthDepositLogic {
    ...
    /**
     * @dev Initialize eth as forwardable
     */
    bool public isDisableEthAutoForwardable = false;
    ...
    /**
     * @dev This is only set in EthDepositLogic (this) contract's storage.
     * It has the ability to change the auto-forwardable status, minInput amount,
    ...
    function returnBackErc20(
        ...
        emit ReturnBackErc20(to, address(erc20Addr), msg.sender, amount);
    }

    /**
     * @notice External function for returning ETH to specific address when the Eth is not auto-forwarding
        ...
        onlyOperator
    {
        ...
        (bool result, ) = to.call{value: amount}(""); // trasfer eth
        require(result, "Could not return back ETH");
        emit ReturnBackEth(to, msg.sender, amount);
    }
    ...
    /**
     * @notice External function for changing the status of eth auto-forwarding switch
     * @param disableForwardable boolen
     */
    function forwardingSwitch(bool disableForwardable)
        external
        onlyEthDepositLogic
        onlyAlive
        onlyOperator
    {
        require(
            isDisableEthAutoForwardable != disableForwardable,
            "nothing to change with forwardable"
        );
        isDisableEthAutoForwardable = disableForwardable;
    }
    ...
    receive() external payable {
        ...
        require(isDisableEthAutoForwardable == false, "not forwardable");
