...
contract Boglins is ERC721 {
    uint256 public tokenCount;
    address public owner;

    // uint256 public cost = 0.069;

    constructor() public ERC721("Boglins", "pBog") {
        tokenCount = 0;
        owner = msg.sender;
    }

    function mintInit(string memory _tokenURI) public payable {
        require(msg.value >= 69000000 * 1000000000);
        mintTime(_tokenURI);
    }

    function mintTime(string memory _tokenURI) internal returns (uint256) {

        uint256 currId = tokenCount;
        _safeMint(msg.sender, currId);
        _setTokenURI(currId, _tokenURI);
        tokenCount += 1;

        return tokenCount;
    }

    function retrieveCurrent() public view returns (uint256) {
        return tokenCount;
    }

    function withdrawal() public payable {
        require(msg.sender == owner, "Transaction Failed, Not Owner");
    }
}