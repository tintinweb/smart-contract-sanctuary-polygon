...
contract ReservForwarder is
    ...
    ERC165
{
    address private _venueRegistar;

    uint256 public factoriesCount; //will start from 1
    ...
    {
        return "1.0.0-beta.0+fob.rsv.iReservForwarder";
    }

    /// @inheritdoc IReservForwarder
    /// @notice factories count starts from 1
    function addfactory(address factory) external virtual override onlyReserv {
        if(factory == address(0)) revert ForwarderFactoryAddressZero();
        if(_positions[factory] > 0) revert ForwarderFactoryAlreadyAdded();
        
        bytes4 factoryWideInterfaceId = type(IERC721FactoryWide).interfaceId;
        (bool success, bytes memory data) = factory.call(
            abi.encodeWithSignature(
                "supportsInterface(bytes4)",
                factoryWideInterfaceId
            )
        );
        if(!success) revert ForwarderUnableToRetrieveFactryInterface();

        bool isSupported = abi.decode(data, (bool));
        if(!isSupported) revert ForwarderNotSupportedInterface();

        factoriesCount += 1;
        _factories[factoriesCount] = factory;
        _positions[factory] = factoriesCount;

        emit NewFactory(factoriesCount, factory);
    }

    /// @inheritdoc IReservForwarder
    function deprecateFactory(address factory, bool deprecate)
        external
        virtual
        override
        onlyReserv
    {
        ...
        uint256 pos = _positions[factory];
        if(pos == 0 || pos > factoriesCount) revert ForwarderUnexistentFactory();

        if(deprecate && deprecated[factory]) revert ForwarderDeprecatedFactory();
        else if(!deprecate && !deprecated[factory]) revert ForwarderNotDeprecatedFactory();

        deprecated[factory] = deprecate;

        emit DeprecateFactory(factory, deprecate);
    }
    ...
    function deploy(
        ...
        address venue = _msgSender();
        IVenueRegistar venueRegistar = IVenueRegistar(_venueRegistar);
        if(!venueRegistar.isVenue(venue)) revert ForwarderUnknownVenue();

        address factory = _factories[typeOf];
        if(factory == address(0)) revert ForwarderUnexistentFactory();
        if(deprecated[factory]) revert ForwarderDeprecatedFactory();

        (bool success, bytes memory data) = factory.call(
            abi.encodeWithSignature(
                "deploy(string,string,address,bytes)",
                name,
                symbol,
                venue,
                mainData
        ...
        if(!success) revert ForwarderErrorDeploying();

        address deployed = abi.decode(data, (address));
        emit Deployment(venue, factory, deployed);

        return deployed;
    }

    /// @inheritdoc IReservForwarder
    function removeEvent(address _event, uint256 typeOf) external virtual override {
        address venue = _msgSender();
        IVenueRegistar venueRegistar = IVenueRegistar(_venueRegistar);
        if(!venueRegistar.isVenue(venue)) revert ForwarderUnknownVenue();
        ...
        (bool success, ) = factory.call(
            abi.encodeWithSignature("removeEvent(address,address)", venue, _event)
        );
        if(!success) revert ForwarderErrorRemoving();
        emit Removed(venue, factory, _event);
    }

    /// @inheritdoc IReservForwarder
    function getFactoryAt(uint256 position) external virtual override view returns(address) {
        return _factories[position];
    }

    /// @inheritdoc IReservForwarder
    function getFactoryPosition(address factory) external virtual override view returns(uint256) {
        return _positions[factory];
    }
    ...
    {
        return
            interfaceId == type(IReservForwarder).interfaceId ||
