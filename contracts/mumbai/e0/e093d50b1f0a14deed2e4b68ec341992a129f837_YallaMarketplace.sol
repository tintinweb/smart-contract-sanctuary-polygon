...
contract YallaMarketplace is ReentrancyGuard {
    ...
    */
    ...
    function set2ndSaleRoyality( uint256 _firstCommission, uint256 _secondCommission) public onlyOwner {
        firstCommission = _firstCommission;
        secongCommission = _secondCommission;
    }
    function set1stSaleROyality(uint256 _adminCommission) public onlyOwner {
        adminCommission = _adminCommission;
    }


    function secondCommitionDistribution(uint256 _value) internal view returns(uint256,uint256){
        uint256 first = _value/100*firstCommission;
        uint256 second= _value/100*secongCommission;
        return (first,second);
    }
    function firstCommitionDistribution(uint256 _value) internal view returns(uint256){
    ...
    function processBuy(address recipient, uint256 tokenId, uint256 noOfTokens, address tokenHolder, uint256 commission) private returns (uint256) {
        ...
        if(tokenHolder==tokenCreator){
            uint256 amount= firstCommitionDistribution(commission_Amount);
            payable(marketplaceWalletAddress).transfer(amount);
            payable(tokenCreator).transfer(commission_Amount.sub(amount));
        }else{
           (uint256 a,uint256 b) = secondCommitionDistribution(commission_Amount);
           uint256 commitionDistibuted=a.add(b);
            payable(marketplaceWalletAddress).transfer(a);
            payable(tokenCreator).transfer(b);
            payable(tokenHolder).transfer(commission_Amount.sub(commitionDistibuted));
