...
contract mayTheForceBeWithYou is NFTStandard {
    constructor(string memory _name, string memory _symbol, string memory _contractUri) NFTStandard(_name, _symbol, _contractUri) {}
}
...
abstract contract NFTStandard is 
    ...
    */
    function _baseURI(uint256 tokenId) private view returns (string memory) {
        ...
        for(uint256 idx = cachedBaseUris.length; idx > 0; idx--) {
            if (EnumerableSale._inRange(tokenId, cachedBaseUris[idx-1]._range)) {
    ...
    */
    ...
    function royaltyInfo(
        ...
        for(uint256 idx = cachedRoyaltyInfos.length; idx > 0; idx--) {
            if (EnumerableSale._inRange(_tokenId, cachedRoyaltyInfos[idx-1]._range)) {
    ...
    */
     ...
     * @dev If changed during a sale, only the ending of blockRange, whitelist, minterlist, baseURI 
     * and royaltyInfo are in effect.
    ...
    function setMintingCondition(
        ...
        sales._setMintingCondition(_mintingConditionId, _mintingCondition);
     ...
     * @return a tuple of (Range, Range, Range, address[], bytes32, uint256, uint256, string, address, uint96)
     */
    ...
    */
    ...
    /**
     * @notice The minted tokens belong to msg.sender if whitelist mode enabled 
     * and belongs to receivers if minters mint.
    ...
    function mint(
        ...
        if(
            ...
            if (sales._tokenTrackerCurrent(_mintingConditionId) == sales._totalToken(_mintingConditionId)) {
                sales._closeSale(_mintingConditionId, sale._tokenRange, false);
            }

            _refund(_msgSender(), msg.value - (sale._price * _amountPerAddress * _receivers.length));
        ...
        //validate whitelist program and mint to caller
            ...
            _refund(_msgSender(), msg.value - (sale._price * _amountPerAddress));
        }
    }
    ...
    */
    ...
    /**
     * @notice refund
     * @param _sender The sender who gets refunded
     * @param _fund The fund
    */
    function _refund(address _sender, uint256 _fund) private {
        if (_fund != 0) {
            (bool sent, ) = payable(_sender).call{value: _fund}("");
            require(sent, "NS28");
        }
    }
}
...
library EnumerableSale {
    ...
    event MintingConditionSet(uint256 _mintingConditionId, MintingConditionStruct _mintingCondition);
    ...
    */
    ...
    /**
     * @dev track whom minted how many tokens for _commonValidation()
     * @param key The sale/minting-condition Id
    */
    ...
    */
    function _getLatestSaleId(Sales storage sales) private view returns(uint256) {
    ...
    */
    function _totalToken(Sales storage sales, uint256 key) external view returns(uint256) {
        return (
     ...
     * @param key the sale/minting-condition id
     */
    function _remove(Sales storage sales, uint256 key) private returns (bool) {
        // assert key must exist
        assert(_contains(sales, key));
    ...
    function _at(
        Sales storage sales, 
        uint256 index
    ) internal view returns (uint256, MintingConditionStruct memory) {
     ...
     * @param _mintingCondition The minting condition to be set for each sale
     */
    function _setMintingCondition(
        Sales storage sales, 
        uint256 _mintingConditionId, 
        MintingConditionStruct calldata _mintingCondition
    ) public {
        require(_mintingCondition._blockRange._startId <= _mintingCondition._blockRange._endId, "NS15");
        uint256 saleId;
        MintingConditionStruct memory sale;
        // validating the input minting condition vacancy and resolve valid overlapped configuration
        for (uint256 i = 0; i < _length(sales); ++i) {
            (saleId, sale) = _at(sales, i);
            bool overlaping = _overlapped(_mintingCondition._tokenRange, sale._tokenRange);

            // sale period expired
            if (block.number > sale._blockRange._endId) {
                if (!_getClosedState(sales, saleId)) {
                    if(_tokenTrackerCurrent(sales, saleId) != 0) {
                        ...
                        _closeSale(
                            ...
                            Range(
                                ...
                                sale._tokenRange._startId + _tokenTrackerCurrent(sales, saleId) - 1
                                ), 
                            true
                        );
                    } else if (overlaping) {
                        // totally unsatisfactory sale, allow for overlapping reconfiguration
                        _remove(sales, saleId);
                    }
                }
            // in sale period
        ...
        require(_isRangeVacant(sales, _mintingCondition._tokenRange), "NS20");
        
        // creating new minting condition
        if (_mintingConditionId == 0) {
            _commonValidation(_mintingCondition);
            _newlySet(sales, _mintingConditionId, _mintingCondition);
            emit MintingConditionSet(_getLatestSaleId(sales), _mintingCondition);
        // updating existing minting condition
        } else {
            require(
                _contains(sales, _mintingConditionId), 
                string(abi.encodePacked("NS22, ", _mintingConditionId.toString()))
                );
            require(!_getClosedState(sales,_mintingConditionId), "NS31");
            // updating activated minting condition
            if (_inRange(block.number, _get(sales, _mintingConditionId)._blockRange)) {
                MintingConditionStruct memory newsaleInstance = _get(sales, _mintingConditionId);
                ...
                _set(sales, _mintingConditionId, newsaleInstance);
    ...
    */
    function _overlapped(Range memory range1, Range memory range2) private pure returns(bool) {
    ...
    */
    function _isRangeVacant(Sales storage sales, Range memory _tokenRange) private view returns(bool) {
        Range[] memory rangesMinted = sales.mintedRanges;
        for (uint i = 0; i < rangesMinted.length; ++i) {
            if (_overlapped(_tokenRange, rangesMinted[i]))
                return false;
        }
        return true;
    }
    ...
    */
    function _closeSale(
        Sales storage sales, 
        uint256 _mintingConditionId, 
        Range memory _tokenRange, 
        bool _unsatisfactory
    ) public {
        sales.mintedRanges.push(_tokenRange);
        _tokenTrackerReset(sales, _mintingConditionId);
        _setClosedState(sales, _mintingConditionId, true);
    ...
    function _newlySet(
        ...
        _set(sales, _mintingConditionId, _mintingCondition);
        //setBaseURI(_mintingCondition._tokenRange, _mintingCondition._baseURI);
     ...
     */
    function _commonValidation(MintingConditionStruct calldata _mintingCondition) private view {
        require(_mintingCondition._tokenRange._startId <= _mintingCondition._tokenRange._endId, "NS11");
        ...
        require(_mintingCondition._clientIdRange._startId <= _mintingCondition._clientIdRange._endId, "NS18");
    ...
    */
    function _inRange(uint256 _id, Range memory _range) internal pure returns(bool) {
