...
contract AavegotchiDiamondSimulator {
    ...
    event UnDelegatedScholarshipProgram(address owner, address vaultAddress, address indexed nftAddress, uint256 indexed tokenId);
    event Withdrew(address recipient, address tokenAddress, uint256 amount);

    constructor() {}

    function simulateGotchiLendingClaimedEvent(
        AddGotchiListing memory listing,
        uint32 _listingId,
        address _lender,
        address _borrower,
    ...
    function simulateUnDelegateScholarshipProgramEvent(
        address _owner,
        address _vaultAddress,
        address _nftAddress,
        uint256 _tokenId
    ) public {
        emit UnDelegatedScholarshipProgram(_owner, _vaultAddress, _nftAddress, _tokenId);
    }

    function simulateWithdrewEvent(
        address _recipient,
        address _tokenAddress,
        uint256 _amount
    ) public {
        emit Withdrew(_recipient, _tokenAddress, _amount);
    }
}