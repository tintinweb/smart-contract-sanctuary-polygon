...
error MetaStarNFT__ContractIsInactive();
...
contract MetaStarNFTV2 is ERC721URIStorage, Ownable {
    ...
    uint256 internal immutable i_threshold0;
    uint256 internal immutable i_threshold1;
    uint256 internal immutable i_threshold2;
    uint256 internal immutable i_threshold3;
    uint256 internal immutable i_mintFee_type0;
    uint256 internal immutable i_mintFee_type1;
    uint256 internal immutable i_mintFee_type2;
    uint256 internal immutable i_mintFee_type3;
    uint256 internal immutable i_mintFee_type4;
    uint private immutable i_MAX_STARS;
    string[4] internal s_metaStarTokenUris;
    ...
    /// @notice Modifiers
    modifier onlyAdmin {
        require(msg.sender == owner() || isAdmin[msg.sender], "User is not owner or admin.");
        _;
    }

    constructor(string[4] memory metaStarTokenUris, uint256 maxStarsSupply, uint256 threshold0, uint256 threshold1, uint256 threshold2, uint256 threshold3, uint256 mintFee_type0, uint256 mintFee_type1, uint256 mintFee_type2, uint256 mintFee_type3, uint256 mintFee_type4) ERC721("METASTAR NFT", "MST") {
        /**
        ...
        */
        ...
        i_MAX_STARS = maxStarsSupply;
        s_metaStarTokenUris = metaStarTokenUris;
    }
    ...
    /**
    * @notice Function used to reserve a star for admins
    * @dev Only callable by Admins
    */
    function reserveStar(uint256 tokenId) public onlyAdmin {
        if (tokenId >= s_metaStarTokenUris.length) {
    ...
    */
    function handleMintMetaStarNFT(uint256 tokenId) public payable {
        if (s_contractState == ContractState.INACTIVE) {
            revert MetaStarNFT__ContractIsInactive();
        }
        if (tokenId >= s_metaStarTokenUris.length) {
            revert MetaStarNFT__NFTIndexDoesNotExist();
        }
    ...
    * - Call mintFunction
    */
    function buyNormalPack(uint256 packType, uint256 numStars) public payable {
        uint256 appliedFee;
        ...
        if (msg.value < ((appliedFee/s_DISCOUNT_FACTOR)*(packType * numStars))){
            revert MetaStarNFT__NotEnoughMoneySent();
        } 
        if (packType == 1){ 
            ...
            appliedTokenId = s_smallTokenCounter = s_smallTokenCounter + numStars;
            s_smallTokenCounter++;
        }else if(packType == 3){
            ...
            appliedTokenId = s_mediumTokenCounter;
            ...
            appliedTokenId = s_bigTokenCounter;
            ...
            appliedTokenId = s_specialTokenCounter;
