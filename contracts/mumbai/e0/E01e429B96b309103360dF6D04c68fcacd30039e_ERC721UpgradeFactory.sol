...
contract ERC721Upgrade is Initializable, ContextUpgradeable, ERC721Upgradeable {
    using Strings for uint256;
    string private baseURI;

    uint64 public cost = 0 ether;
    uint64 public currentSupply = 0;
    uint64 public maxSupply = 1010;
    ...
    //Constructor
    //@dev the max supply will be one when the _maxSupply is 1, and it could be uppgraded from a function
    ...
    // constructor(uint64 _maxSupply) ERC721("TestToken", "TTKN") {
    //     maxSupply = _maxSupply;
    // }

    modifier onlyOwner() {
        require(_msgSender() == _owner, "only owner can call this methos");
        _;
    }
    ...
    function _getTokenURIs(uint256 tokenId)
        internal
        view
        returns (string memory)
    {
        ...
        string memory currentURI = bytes(currentBaseURI).length > 0
            ? string(
...
import "./ERC721Upgrade.sol";

contract ERC721UpgradeFactory {
    ...
    function createERC7211Upgrade(string memory _name, string memory _symbol)
        public
    {
        ERC721Upgrade testContract = new ERC721Upgrade();
        testContract.initialize(_name, _symbol);
