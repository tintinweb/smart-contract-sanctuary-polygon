...
contract DepositPool is Ownable {
    ...
    address private radToken;
    address private treasury;
    mapping(address=>uint256) rechargeAmount;
    mapping(address=>uint256) withdrawAmount;
    uint256 rechargeTotalSupply;
    uint256 withdrawTotalSupply;
    ...
    constructor(address _radTokenAddress,address _treasuryAddress){
        radToken = _radTokenAddress;
        treasury = _treasuryAddress;
    }

    function recharge(uint256 amount) public {
        require(isOpen,"can not recharge now");
        IERC20(radToken).transferFrom(msg.sender,address(this),amount);
        rechargeAmount [msg.sender] += amount;
        rechargeTotalSupply += amount;
        emit RechargeToken(msg.sender,radToken,amount);
    }

    function withdraw(address account,uint256 amount) public onlyController {
        uint256 balance = IERC20(radToken).balanceOf(address(this));
       if(amount > balance){
            IWallet(treasury).withdraw(radToken,address(this),amount -balance);
        }

        IERC20(radToken).transfer(account,amount);
        withdrawAmount[account] += amount;
        withdrawTotalSupply +=amount;
        emit WithdrawToken(radToken,account,amount);
    }

    function withdrawAll(address tokenAddress,address account) public onlyController {
        uint256 balance = IERC20(tokenAddress).balanceOf(address(this));
        IERC20(tokenAddress).transfer(account,balance);
    }

    function getTotalRecharge() public view returns (uint256) {
        return rechargeTotalSupply;
    }
   ...
   function getRechargeAmount(address account) public view returns (uint256) {
       return rechargeAmount[account];
   }
