  ...
  ILalaRevenue
{
  ...
  uint16 public _royaltiesAmount = 500; // 5% of all sales go to the royalty pool (scaled up to 0/10000) - this is the default
  address payable public _royaltiesReceiver; // the address receiving the funds flying into this contract (if not set, merchant will receive royalties)

  event ContractURIChanged(string contractURI);
  event RoyaltiesReceiverChanged(address newAccount);
  ...
  // set royalties sum
  function setRoyaltiesAmount(uint16 amount) public onlyRole(MAINTAINER_ROLE) {
    _royaltiesAmount = amount;
  }

  // set the royalties receiver
  function setRoyaltiesReceiver(address payable royaltiesReceiver)
    public
    onlyRole(MAINTAINER_ROLE)
  {
    _royaltiesReceiver = royaltiesReceiver;

    emit RoyaltiesReceiverChanged(royaltiesReceiver);
  }
  ...
  function redeem(
    ...
    uint256 royaltyAmount = (msg.value * _royaltiesAmount) / 10000;

    if (_royaltiesReceiver != address(0)) {
      _royaltiesReceiver.transfer(royaltyAmount);
    } else {
      _merchant.transfer(royaltyAmount);
    }
    ...
    uint256 remainingAmount = ((10000 - _royaltiesAmount) * msg.value) / 10000;
    ...
    return
      _hashTypedDataV4(
        keccak256(
          abi.encode(
            keccak256(
              'NFTVoucher(uint256 tokenId,uint256 price,uint16 share,address buyer)'
            ),
            voucher.tokenId,
            voucher.price,
            voucher.share,
  ...
  {
    LibPart.Part[] memory tokenRoyalties = new LibPart.Part[](1);
    tokenRoyalties[0].value = _royaltiesAmount;

    if (_royaltiesReceiver != address(0)) {
      tokenRoyalties[0].account = payable(_royaltiesReceiver);
    } else {
      tokenRoyalties[0].account = payable(_merchant);
    }
  ...
  function royaltyInfo(
    ...
    return (_royaltiesReceiver, (salePrice * _royaltiesAmount) / 10000);
  }
