...
contract Factory 
{
    ...
    */
    ...
    {
        uint count = totalToken[ownerAddress];
        uint addIndex = 0;
        address[] memory result = new address[](count);

        for(uint ix = 0; ix < tokens.length; ix++)
        {
            address tokenAddress = tokens[ix];
            if(tokenOwner[tokenAddress] == ownerAddress)
            {
                result[addIndex] = tokenAddress;
                addIndex++;

                if (addIndex >= result.length) 
                {
                    break;
                }
            }
