...
contract DEX {
    ...
    constructor( address _collection)  {
      ...
      // add NFTs and ETH 
    function init2(address _collection, uint256 _tokens) public payable{
        require(totalLiquidity == 0, "DEX: init - already has liquidity");
        totalLiquidity = address(this).balance;
        liquidity[msg.sender] = totalLiquidity;   
        for(uint256 i = 0; i < _tokens; i ++) {
           IMyNft(nftCollection).transferFrom(msg.sender, address(this),i);
        }
        k = totalLiquidity * totalNFTs;
    }

    // returns how many NFTs are in the contract
   // function getNFTBalance(address _collection) public view returns (uint256) {
  ...
  function getNFTSupply() public view returns(uint256) {
     ...
     * xInput is amount of tokens
     * xReserves is Balloons, yReserves is Eth
     */
 function price(uint256 xInput,uint256 xReserves,uint256 yReserves) public pure returns (uint256 yOutput) {
        uint256 xInputWithFee = xInput.mul(997);  // 4 ballons * 997
    ...
    function getETH(uint256 _tokens) public view returns (uint256) {
        uint256 newY =  k / (totalNFTs -  _tokens);
        uint256 eth_price  = newY - address(this).balance;
        return eth_price;
    }

    function getETHWithFee(uint256 _tokens) public view returns (uint256) {
        uint256 newY =  k / (totalNFTs -  _tokens);
        uint256 eth_price  = (newY - address(this).balance) * 997 / 1000 ; // less .003% fee
        return eth_price;
    }
    // user sells NFT to AMM and receives eth
    function sellNFT(uint256 _nftID) public returns (uint256 ) {
        require (IMyNft(nftCollection).ownerOf(_nftID)==msg.sender, "Not owner");
        totalNFTs = totalNFTs + 1;      
        uint256 salePrice =  price(1, totalNFTs, address(this).balance);
        IMyNft(nftCollection).transferFrom(msg.sender, address(this), _nftID); // transfer NFT     
        (bool sent, ) = msg.sender.call{ value: salePrice }("");             // send eth
        require(sent, "nftToEth: revert in transferring eth to you!");     
        return salePrice;
    }

     // users can buy NFTs using this function
    function buyNFT(uint256 amountToBuy) public payable  {
        require(msg.value > 0, "didn't send eth");
        uint256 newY =  k / (totalNFTs -  amountToBuy);
        uint256 eth_price  = newY + msg.value - address(this).balance ;// have to add msg.value
       if (msg.value != eth_price) {  // require the correct amount of eth to be sent
        ...
        for (uint i = 0 ; i < amountToBuy; i ++){
          totalNFTs = totalNFTs - 1;  
          IMyNft(nftCollection).transferFrom(address(this), msg.sender, getID(0, address(this)) ); // transfer the  NFT to purchaser  
        }
    
    }

    function getPrice(uint256 _nfts) public view returns(uint256){
        uint256 x = totalNFTs - _nfts;   // 49
        uint256 y = k  / x ;               // 50 / 49= 1.1
        return y / x;                    // 1.02 / 49
    }
    ...
    function getLiquidity(address lp) public view returns (uint256) {
      ...
      function getPriceWithFee(uint16 _nftsBeingSold) public view returns(uint256){
          uint256 newX = totalNFTs - _nftsBeingSold;
          uint256 newY = k * 10 ** 18 / newX * 10 ** 18;
          uint256 price = (newY - address(this).balance) / _nftsBeingSold;
          return price.mul(1003).div(1000);
    }
    /**
     ...
     */
    function deposit(uint256 _tokens) public payable returns (uint256 tokensDeposited) {
        require(msg.value > 0, "Must send value when depositing");
        require(IMyNft(nftCollection).balanceOf(msg.sender)>= _tokens, "Not enough NFTs" );
        uint256 ethReserve = address(this).balance.sub(msg.value);
        uint256 tokenReserve = getBalance();  // get the current number of NFTs in the contract
    ...
    */
    function withdraw(uint256 amount) public returns (uint256 eth_amount, uint256 token_amount) {
       ...
       for(uint256 i = 0; i < tokenAmount; i++) {
          IMyNft(nftCollection).transferFrom(address(this), msg.sender, getID(0, msg.sender) );
        }
