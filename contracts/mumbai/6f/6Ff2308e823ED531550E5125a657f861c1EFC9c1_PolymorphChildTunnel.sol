...
contract PolymorphChildTunnel is FxBaseChildTunnel, PolymorphTunnel {
    ...
    //         uint256 genomeChanges = polymorphContract.genomeChanges(
    //             _tokenIds[i]
    //         );
    ...
    function _moveThroughWormholeSingle(uint256 _tokenId) private {
        require(
            polymorphContract.ownerOf(_tokenId) == msg.sender,
            "Msg.sender should be the polymorph owner"
        );
        uint256 gene = polymorphContract.geneOf(_tokenId);
        bool isNotVirgin = polymorphContract.isNotVirgin(_tokenId);
        uint256 genomeChanges = polymorphContract.genomeChanges(_tokenId);
        polymorphContract.burn(_tokenId);
        ...
        override
    {
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            _moveThroughWormholeSingle(_tokenIds[i]);
        }
    }
