...
contract StakingContract is ReentrancyGuard {
    ...
    function stake(uint256 _tokenId) external nonReentrant {
        ...
        // Create StakedToken
        StakedToken memory stakedToken = StakedToken(msg.sender, _tokenId, 1*(10**18));
    ...
    function calculateAllRewards(address _staker) internal view returns (uint256){
        uint256 rewards = 0;
        for (uint256 k = 0; k < stakers[_staker].stakedTokens.length; k++){
            uint256 rewardsLimitCurrent = stakers[_staker].stakedTokens[k].rewardsLimitDB;
            uint256 amount = (block.timestamp - stakers[_staker].timeOfLastUpdate) * rewardsPerHour / 3600;
            if (rewardsLimitCurrent == 0) {
                // stakers[_staker].stakedTokens[k].unclaimedRewards = 0;
                rewards += 0;
            }
            if (rewardsLimitCurrent < amount && rewardsLimitCurrent != 0){
                // stakers[_staker].stakedTokens[k].unclaimedRewards = rewardsLimitCurrent;
                rewards += rewardsLimitCurrent;
            } 
            if (rewardsLimitCurrent >= amount && rewardsLimitCurrent != 0) {
                // stakers[_staker].stakedTokens[k].unclaimedRewards = amount;
                rewards += amount;
            }
        }
        return rewards;
