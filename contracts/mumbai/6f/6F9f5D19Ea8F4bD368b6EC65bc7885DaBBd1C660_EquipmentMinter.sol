...
contract EquipmentMinter is Ownable{
    ...
    function mintEquipments(address user, uint256[] memory randomNumberRequested) public onlyVRF{
        ...
        for(uint256 i=0; i < randomNumberRequested.length; i++){
    ...
    function getRarity(uint16 number) internal pure returns (uint64 rarity, uint32 stat_sum){
        ...
        if(roll_value > 994){rarity = 4; stat_sum = 100;} //.5% chance. If you ever get one, all of the luck you will ever have in your life would be consumed in this transaction.
    ...
    function getStats(uint16[8] memory random_stats, uint32 stat_sum) internal pure returns (equipment_stats memory _equipment_stats, uint64 dominant_stat, uint64 extremity){
        ...
        extremity = getExtremity(_stats[dominant_stat], total);
    }
    ...
    function getExtremity(uint32 dominant_stat_value, uint32 total) internal pure returns (uint64 extremity){
        uint256 percentage_allocation = (dominant_stat_value * 1000) / total;
