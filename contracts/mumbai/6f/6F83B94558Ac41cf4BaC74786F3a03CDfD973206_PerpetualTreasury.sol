...
contract PerpetualTreasury is PerpetualBaseFunctions, IFunctionList, IPerpetualTreasury {
    ...
    function _adjustAnchorOnLiquidityAdd(LiquidityPoolData storage _pool, uint128 _newShares)
        internal
    {
     ...
     * @param   _tokenAmount   The amount of tokens in collateral currency to add, in wei
     */
    function addLiquidity(uint8 _poolId, uint256 _tokenAmount)
        external
        override
        nonReentrant
        whenNotPaused
    {
        ...
        uint24 iPerpetualId;
        uint24 iOtherPerpId;
        (iPerpetualId, iOtherPerpId) = _selectPerpetualIds(uint24(0), _poolId);
        _getUpdateLogic().updateFundingAndPricesBefore(iPerpetualId, iOtherPerpId);

        // main
        int128 _fTokenAmount = int256(_tokenAmount).fromDec18();
        _validateLiquidityData(_poolId, _tokenAmount);
        LiquidityPoolData storage pool = liquidityPools[_poolId];
        require(_tokenAmount >= pool.fBrokerCollateralLotSize.toUDec18(), "amt too small");

        _checkPoolState(pool);

        _rebalance(pool);

        _transferFromUserToVault(pool.marginTokenAddress, msg.sender, _fTokenAmount);
        ...
        _getRebalanceLogic().increasePoolCash(pool.id, _fTokenAmount);
        ...
        _getUpdateLogic().updateFundingAndPricesAfter(iPerpetualId, iOtherPerpId);
    }
     ...
     */
    function withdrawLiquidity(uint8 _poolId, uint256 _shareAmount)
        external
        override
        nonReentrant
        whenNotPaused
     ...
     */
    function executeLiquidityWithdrawal(uint8 _poolId, address _lpAddr)
        external
        override
        nonReentrant
        whenNotPaused
    {
        ...
        if (
            ...
            _transferFromVaultToUser(pool.marginTokenAddress, msg.sender, fFee);
        }
        _transferFromVaultToUser(pool.marginTokenAddress, _lpAddr, fTokenAmountToReturn);
        activeWithdrawals[_poolId].remove(_lpAddr);
        delete lpWithdrawMap[_lpAddr][_poolId];
        emit LiquidityRemoved(_poolId, _lpAddr, fTokenAmountToReturn.toUDec18(), shareAmount);
     ...
     *  no choice of margin token
     * @param   _iPerpetualId   perpetual identifier
     * @param   _fTokenAmount   The amount of token to add. ABDK 64.64
     */
    function addAMMLiquidityToPerpetual(uint24 _iPerpetualId, int128 _fTokenAmount)
        external
        override
        onlyMaintainer
        nonReentrant
     ...
     */
    function _getShareAmountToMint(LiquidityPoolData storage _pool, int128 _fAmount)
        internal
        view
        returns (int128)
    {
     ...
     */
    function _getTokenAmountToReturn(LiquidityPoolData storage _pool, int128 _fShareAmount)
        internal
        view
        returns (int128)
    {
    ...
    function getFunctionList() external pure virtual override returns (bytes4[] memory, bytes32) {
        ...
        functionList[2] = this.addAMMLiquidityToPerpetual.selector;
        functionList[3] = this.executeLiquidityWithdrawal.selector;
        functionList[4] = this.getTokenAmountToReturn.selector;
        functionList[5] = this.getCollateralTokenAmountForPricing.selector;
        functionList[6] = this.pauseLiquidityProvision.selector;
        functionList[7] = this.getShareTokenPriceD18.selector;
        functionList[8] = this.getWithdrawRequests.selector;
