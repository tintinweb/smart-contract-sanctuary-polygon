...
import "./AirGangCreator.sol";
import "./AirGangToken.sol";

/** 
@title Air Gang
@author bilets.eth -> https://bilets.club/
@notice This contract represents a digital Air Gang where users can mint and subscribe to unlock access to premium content for a period of time.
*/

contract AirGang is ERC721, ReentrancyGuard, IERC2981 {
    using SafeMath for uint256;
    string public baseURI = "https://gangnft.vercel.app/api/opensea_meta/";

    AirGangToken public airGangTokenContract =
        AirGangToken(payable(0xAE9fD749957cF5AEA8D26598831A16b33df4826a));

    AirGangCreator public airGangCreatorContract;

    uint256 rewardRatio = 50;

    modifier onlyAdmin() {
        address admin = AirGangCreator(airGangCreatorContract).ownerOf(
            airGangID
        );
    ...
    constructor(
        ...
        address creator,
        address payable _airGangCreator
    )
        ERC721(
            string(abi.encodePacked("Air Gang @", _gangName)),
            string(abi.encodePacked("AG @", _gangName))
        )
    {
        airGangID = tokenId;
        _safeMint(creator, 1);
        airGangCreatorContract = AirGangCreator(payable(_airGangCreator));
    }
    ...
    function mint(uint256 quantity) public payable nonReentrant {
        ...
        airGangCreatorAddress.transfer((msg.value * 10) / 100);

        uint256 rewardAmount = msg.value * rewardRatio;

        AirGangToken(airGangTokenContract).airGangTokenMint(
            msg.sender,
            rewardAmount
        );
    ...
    function owner() public view returns (address) {
        return AirGangCreator(airGangCreatorContract).ownerOf(airGangID);
    }
    ...
    function _baseURI() internal view override returns (string memory) {
        return string(abi.encodePacked(baseURI, Strings.toString(airGangID)));
    }
     ...
     * @param member Address of owner of tokens.
    ...
    function royaltyInfo(
        ...
        return (
            AirGangCreator(airGangCreatorContract).ownerOf(airGangID),
            salePercentage
        );
    }
    ...
    function adminWithdraw() public payable nonReentrant onlyAdmin {
    ...
    function transferOwnership(address oldOwner, address newOwner) public {
        require(
            ...
            "Caller is not the Air Gang Creator contract"
        );
        emit OwnershipTransferred(oldOwner, newOwner);
    }
}
...
contract AirGangCreator is ERC721, Ownable, ReentrancyGuard, IERC2981 {
    ...
    mapping(uint256 => string) public airGangIDToName;
    mapping(string => bool) public airGangNameTaken;

    event BatchMetadataUpdate(uint256 _fromTokenId, uint256 _toTokenId);

    constructor() ERC721("Air Gang Creator", "AG") {}
    ...
    /**
     *@notice Creates a new AirGang.
     *@param _name Gang name for the AirGang.
     */
    function createAirGang(string memory _name) public payable nonReentrant {
        if (onlyAccessList) {
        ...
        require(!airGangNameTaken[_name], "Ait Gang name already exists");
        require(msg.value >= nftCost, "Incorrect payment amount");

        bytes memory gangNameBytes = bytes(_name);
        require(
            gangNameBytes.length >= 1 && gangNameBytes.length <= 20,
            "Gang name should be between 1 and 20 characters"
        );
        for (uint256 i = 0; i < gangNameBytes.length; i++) {
            require(
                ...
                "Invalid character in gang name"
            );
        }

        _safeMint(msg.sender, airGangID);
        airGangNameTaken[_name] = true;
        airGangIDToName[airGangID] = _name;

        AirGang newContracty = new AirGang(
            airGangID,
            _name,
            msg.sender,
        ...
        airGangTokenContract.gangCreatorFlipGang(address(newContracty));

        airGangNameToData[_name] = AirGangData({
            airGangName: _name,
            airGangID: airGangID,
        ...
        airGangID++;
        allAirGang.push(address(newContracty));
    ...
    function _beforeTokenTransfer(
        address from,
        address to,
        uint256 _airGangIDToName,
        uint256 batchSize
    ) internal virtual override {
        super._beforeTokenTransfer(from, to, _airGangIDToName, batchSize);
        if (from != address(0)) {
            string memory airGangName = airGangIDToName[_airGangIDToName];

            address airGangNFTAddress = airGangNameToData[airGangName]
                .contractAddress;

            AirGang airGangNFT = AirGang(airGangNFTAddress);
    ...
    function getAllTokenHolders(
        ...
        uint256[] memory tokenHolders = new uint256[](allAirGang.length);
        for (uint256 i = 0; i < allAirGang.length; i++) {
            AirGang airGangNFT = AirGang(allAirGang[i]);
            if (airGangNFT.balanceOf(wallet) > 0) {
                tokenHolders[i] = airGangNFT.airGangID();
            }
        }
        return tokenHolders;
    }
    ...
    /**
     *@notice Set the cost for creating a AirGang contract.
     *@param _newNftCost New cost for creating a AirGang contract.
     */
...
/** 
@title Air Gang Token
@author bilets.eth -> http://bilets.club/
@notice This contract is responsible for mint and burn Air Gang Token.
*/

contract AirGangToken is ERC20, Ownable, ReentrancyGuard {
    string public contractURI =
        "https://gangnft.vercel.app/api/opensea_meta/contractCreator/";
    string public baseURI =
    ...
    AirGangCreator public airGangCreatorContract =
        AirGangCreator(payable(0x6f96967751049f409fD54413f94C1854880f860B));

    modifier onlyAdmins() {
        require(gangs[msg.sender] == true, "Caller is not aa Admin");
        _;
    }
    modifier isPaused() {
    ...
    constructor() ERC20("Air Gang Token", "AG") {}
