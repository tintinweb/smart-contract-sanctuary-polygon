...
contract GenesisMint is ERC721A, ERC2981, AccessControl {
    enum Status {
        Phase1,
        Phase2,
        Phase3,
        ...
        ERC721A("Centaurify_Collection_AAA", "CENT_AAA")
    {
        ...
        _setupRole(CREATOR_ROLE, 0x30824cB687E2768d239c84B69b242D4da9808D32);
    ...
    function earlyReveal(uint tokenId) external earlyRevealIsOpen isTokenOwner(tokenId) {
    ...
    function removeStuckTokens(address _token, address _to) external onlyRole(CREATOR_ROLE) {
    ...
    /// @dev Restricted to onlyRole(ADMIN_ROLE).
    function setBaseTokenURI(string memory __baseTokenURI) public onlyRole(ADMIN_ROLE) {
    ...
    /// @dev Restricted to onlyRole(ADMIN_ROLE).
    function setContractURI(string memory __contractURI) public onlyRole(ADMIN_ROLE) {
        _contractURI = __contractURI;
    }

    /// @notice Get the current merkleRoot.
    /// @dev Restricted to onlyRole(ADMIN_ROLE).
    function getMerkleRoot() public view onlyRole(ADMIN_ROLE) returns (bytes32) {
        return merkleRoot;
    }
