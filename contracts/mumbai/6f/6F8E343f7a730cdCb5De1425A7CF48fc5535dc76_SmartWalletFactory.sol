...
contract SmartWalletFactory {
    ...
    function deploy(address owner) external payable returns (address) {
        return _deploy(owner);
    }

    function deployIfNeeded(address owner) external payable returns (address) {
        return _deployIfNeeded(owner);
    }

    function deployAndExecute(
        address owner,
        SmartWallet.Request calldata _data,
        bytes memory _signature
    ) external payable {
        address addr = _deployIfNeeded(owner);
        SmartWallet(payable(addr)).execute(_data, _signature);
    }

    function walletAddress(address owner) external view returns (address) {
        return _getDeploymentAddress(_getBytecode(owner), _getSalt(owner));
    }

    function _deploy(address owner) internal returns (address) {
        SmartWallet ctr = new SmartWallet{salt: _getSalt(owner)}(owner);
        address addr = address(ctr);
        emit Deploy(addr);
        return (addr);
    }

    function _deployIfNeeded(address owner) internal returns (address) {
        address src = _getDeploymentAddress(
            _getBytecode(owner),
            _getSalt(owner)
        );
        if (_isDeployed(src)) {
    ...
    function _getDeploymentAddress(bytes memory bytecode, bytes32 _salt)
        public
        view
        returns (address)
    {
    ...
    function _getBytecode(address _owner) public pure returns (bytes memory) {
        bytes memory bytecode = type(SmartWallet).creationCode;
        return abi.encodePacked(bytecode, abi.encode(_owner));
    }

    function _isDeployed(address account) internal view returns (bool) {
