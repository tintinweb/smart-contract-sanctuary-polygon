...
contract LensCampaignPublisher {
    ...
    struct Campaign {
        uint256 id;
        string contentCid;
        string pinataContentId; // TODO: Store on back-end servers
        uint128 blockNumber;
        Invitee[] invitees;
    ...
    struct Invitee {
        string handle; // TODO: query lens-api by using profileId
        uint256 profileId;
    }
    ...
    function createCampaign(
        ...
        string memory pinataContentId,
        uint128 blockNumber,
        Invitee[] memory invitees
    ) public {
        uint256 id = uint256(
        ...
        if (campaign.id != 0)
            revert("LensCampaignPublisher: campaign already exists");
        campaign.id = id;
        campaign.contentCid = contentCid;
        campaign.pinataContentId = pinataContentId;
        campaign.blockNumber = blockNumber;
        for (uint256 i = 0; i < invitees.length; i++) {
            campaign.invitees.push(invitees[i]);
        }
    }

    function addPublishment(
        uint256 campaignId,
        uint256 InviteeIndex,
        string memory contentCid,
        string memory pinataContentId
    ) public {
        Campaign storage campaign = campaigns[campaignId];
        if (campaign.id == 0)
            revert("LensCampaignPublisher: campaign does not exist");
        if (campaign.blockNumber <= block.number)
            revert("LensCampaignPublisher: campaign has already ended");

        Invitee memory invitee = campaign.invitees[InviteeIndex];
        if (invitee.profileId == 0)
            revert("LensCampaignPublisher: invitee does not exist");
        if (IERC721(HUB).ownerOf(invitee.profileId) != msg.sender)
            revert("LensCampaignPublisher: you are not invitee.");

        Publishment memory publishment = Publishment({
            handle: invitee.handle,
            profileId: invitee.profileId,
            pubId: 0,
            contentCid: contentCid,
            pinataContentId: pinataContentId
        });

        campaign.publishments[InviteeIndex] = publishment;
    }

    function postPublishments(uint256 campaignId) public {
        Campaign storage campaign = campaigns[campaignId];
        if (campaign.id == 0)
            revert("LensCampaignPublisher: campaign does not exist");
        if (campaign.blockNumber > block.number)
            revert("LensCampaignPublisher: campaign has not started");

        for (uint256 i = 0; i < campaign.publishments.length; i++) {
            Publishment storage publishment = campaign.publishments[i];
            if (publishment.profileId == 0) continue;
            if (publishment.pubId != 0) continue; // NOTE: already published
            if (
                ILensHub(HUB).getDispatcher(publishment.profileId) ==
                address(this)
            ) continue; // NOTE: dispatcher is not set

            uint256 pubId = ILensHub(HUB).post(
                LensDataTypes.PostData({
                    profileId: publishment.profileId,
                    contentURI: publishment.contentCid,
                    collectModule: address(0), // TODO: collectModule
                    collectModuleInitData: "", // TODO: collectModuleInitData
                    referenceModule: address(0),
                    referenceModuleInitData: ""
                })
            );

            publishment.pubId = pubId;
        }
    }

    function test(uint256 profileId, string memory contentURI) public {
        ILensHub(HUB).post(
            LensDataTypes.PostData({
                profileId: profileId,
