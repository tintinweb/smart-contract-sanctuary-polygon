...
contract AlphaBettyActivityAirdrop is ERC1155Supply, Ownable {
    ...
    constructor() ERC1155(collectionURI) {
        name_ = "AlphaBetty Activity Airdrop";
        symbol_ = "AAA";
        tokenQty = 3500;
        currentTokenId = 1;
        maxWalletQty = 1;
    ...
    function airdrop(address[] calldata _to)
        public
        onlyOwner
    {
        require(totalSupply(currentTokenId) < tokenQty, "All Airdropped");

        for (uint256 i = 0; i < _to.length; i++) {
            require(this.balanceOf(_to[i], currentTokenId) + maxWalletQty <= maxWalletQty, "You have hit the max airdrops for this wallet");
             _mint(_to[i], currentTokenId, maxWalletQty, "");
        }
    }
