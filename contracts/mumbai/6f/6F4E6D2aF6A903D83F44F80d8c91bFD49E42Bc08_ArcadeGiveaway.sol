...
contract ArcadeGiveaway is IArcadeGiveaway, OwnableUpgradeable {
  ...
  event Claimed(uint256 indexed withTokenId, uint256 indexed giveawayId, uint256 indexed claimingId);
  ...
  */
  function addClaimingsToGiveaway(
    uint256 giveawayId,
    Claiming[] calldata claimings
  ) public onlyMods {
  ...
  function _claim(
    ...
    for(uint256 i=0; i<claimingIds.length; i++) {
      ...
      for(uint256 j=0; j<withTokenIds.length; j++) {
        ...
        require(withTokenId >= giveaway.startTokenId && withTokenId <= giveaway.endTokenId, "Giveaway: WRONG_TOKEN_ID");
  ...
  */
  function claim(
    ...
    for(uint256 i=0; i<withTokenIds.length; i++) {
      require(IERC721(giveaway.rewardToken).ownerOf(withTokenIds[i]) == msg.sender, "Giveaway: NOT_OWNER");
    }
  ...
  */
  function claimWithProof(
    ...
    require(_verifySignature(abi.encode(msg.sender, withTokenIds, claimingIds, signatureTtl), signatureProof), "Giveaway: INVALID_SIGNATURE");
   ...
   */
  function getGiveaways() external override view returns(Giveaway[] memory) {
   ...
   */
  function getClaimings(uint256 giveawayId) external override view returns(Claiming[] memory) {
    Giveaway storage giveaway = _getGiveaway(giveawayId);
   ...
   */
  function getAlreadyClaimedForTokenId(uint256 giveawayId, uint256 withTokenId) external override view returns(bool[] memory) {
    Giveaway storage giveaway = _getGiveaway(giveawayId);
