...
contract W5006Lib {
    ...
    {
        IERC1155RentalMarket.Renting memory renting = IERC1155RentalMarket(
            market
        ).rentingOf(rentingId);
        if (renting.recordId == 0) {
            revert ("------renting.recordId ==0 ---");
            // return IERC5006.UserRecord(0, address(0), 0, address(0), 0);
        }
        IERC1155RentalMarket.Lending memory lending = IERC1155RentalMarket(
            market
        ).lendingOf(renting.lendingId);
