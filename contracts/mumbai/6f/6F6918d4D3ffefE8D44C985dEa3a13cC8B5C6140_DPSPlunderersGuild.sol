...
contract DPSPlunderersGuild is ERC1155Holder, Ownable, ReentrancyGuard {
    ...
    event ClaimedChest(address indexed _owner, VOYAGE_TYPE _type, uint256 _amount);
    ...
    event WonLockBock(address indexed _user, uint256 _amount);
    ...
    function lockToClaimRewards(uint256 _chestsType, uint256 _amount) external nonReentrant {
        ...
        if (lockedChests[msg.sender].length > 0) revert WrongState(1);

        if (chest.balanceOf(msg.sender, _chestsType) < _amount) revert WrongParams(1);

        lockedChests[msg.sender].push(
            LockedChest(VOYAGE_TYPE(_chestsType), block.number, block.timestamp, 0, _amount, msg.sender)
        );
    ...
    function lockToClaimLockBox(uint256 _amount) external nonReentrant {
        // require(gameSettings.isPaused(4) == 0, "Paused");
        if (gameSettings.isPaused(4) == 1) revert Paused();
        // require(lockedLockBoxes[msg.sender].amount == 0, "lockedLockBoxes");
        // require(_amount < 5, "amount > 5");
        if (lockedLockBoxes[msg.sender].amount > 0) revert WrongParams(1);
        if (_amount >= 5) revert WrongParams(2);
    ...
    function claimLockedRewards(CausalityParams calldata causalityParams) external nonReentrant {
        LockedChest[] memory userLockedChests = lockedChests[msg.sender];
        if (userLockedChests.length == 0) revert WrongParams(1);
        VOYAGE_TYPE voyageType = userLockedChests[0].chestType;
        uint256 doubloonsRewards;

        uint256 maxRoll = gameSettings.getMaxRollPerChest(voyageType);

        uint256 rewardedLockBox = 0;
        if (userLockedChests[0].lockedBlock + gameSettings.getBlockJumps() + 1 != causalityParams.blockNumber[0])
            revert WrongState(2);
        if (userLockedChests[0].lockedBlock + gameSettings.getBlockJumps() >= block.number) revert WrongState(3);

        for (uint256 i; i < userLockedChests.length; i++) {
            LockedChest memory lockedChest = userLockedChests[i];
            uint256 result = causality.getRandom(
                msg.sender,
                causalityParams.blockNumber[0],
                causalityParams.hash1[0],
                causalityParams.hash2[0],
                causalityParams.timestamp[0],
                causalityParams.signature[0],
            ...
            doubloonsRewards = gameSettings.getChestDoubloonRewards(voyageType) * lockedChest.amount;

            emit ClaimedChest(msg.sender, lockedChest.chestType, lockedChest.amount);
            lockedChest.claimedBlock = block.number;
            claimedChests[msg.sender][uint8(voyageType)].push(lockedChest);
            chest.burn(address(this), uint256(lockedChest.chestType), lockedChest.amount);
        }
        if (doubloonsRewards > 0) doubloons.mint(msg.sender, doubloonsRewards);
        delete lockedChests[msg.sender];
        if (rewardedLockBox > 0) {
            emit WonLockBock(msg.sender, rewardedLockBox);
            chest.mint(msg.sender, 4, rewardedLockBox);
        }
    }

    function claimLockedBox(uint256 _amount, CausalityParams calldata causalityParams) external nonReentrant {
        LockedLockBox memory lockedLockBox = lockedLockBoxes[msg.sender];
        if (lockedLockBox.amount != _amount) revert WrongParams(1);
        // require(lockedLockBox.amount == _amount, "!lockedLockBox");
        if ((lockedLockBox.lockedBlock + gameSettings.getBlockJumps()) + 1 != causalityParams.blockNumber[0])
            revert WrongState(2);
        if ((lockedLockBox.lockedBlock + gameSettings.getBlockJumps()) >= block.number) revert WrongState(3);
        // require((lockedLockBox.lockedBlock + gameSettings.getBlockJumps()) < block.number, "notYet");
        ...
        for (uint i; i < _amount; i++) {
            uint256 result = causality.getRandom(
                ...
                0,
                gameSettings.getMaxRollCapLockBoxes()
            );
            artifacts[i] = interpretResult(result);
            chest.burn(address(this), 4, 1);
        }
    ...
    function setContract(address _contract, uint8 _target) external onlyOwner {
        ...
        else if (_target == 4) chest = MintableBurnableIERC1155(_contract);
