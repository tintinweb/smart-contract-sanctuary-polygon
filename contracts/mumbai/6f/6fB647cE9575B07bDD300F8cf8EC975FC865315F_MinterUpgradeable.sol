...
contract MinterUpgradeable is ERC1271, OwnableUpgradeable, ERC165Upgradeable, IMinterUpgradeable, RolesValidator {
    ...
    LibPart.Part[] private placeholder;

    function __MinterUpgradable_init (address _minter, uint96 _fee, uint96 _minDefaultMinterRoyalty ) public initializer {
        __Ownable_init();
    ...
    function upsertDefaultCreatorPayoutsAndRoyalties(LibMinter.DefaultMinter memory data) public onlyOwner {
        ...
        defaultRoyaltySplittersMapping[data.token][data.creator] = royalties;
    }
    ...
    function upsertDefaultCreatorPayoutsAndRoyaltiesByCreator(LibMinter.DefaultMinter memory data) public {
        ...
        if (!setDefault){
            ...
            bytes32 royaltiesBytes = getOrCreateSplitterUsingSplit(royalties);
    ...
    function getOrCreateSplitterUsingSplit(LibPart.Part[] memory payees) internal virtual returns (bytes32){
        ...
        uint96 totalValue;
        bytes32 splitterHash = LibPart.hashParts(payees);
        if (payees.length == 1) {
            ...
            splitters[splitterHash] = LibPart.Part(payees[0].account, payees[0].value);

        } else if ( payees.length > 0) {
            splitter = splitters[splitterHash].account;
            totalValue = splitters[splitterHash].value;
            
            // Check if splitter is not already created
            if (splitter == address(0x0)){
                ...
                // Deploy new royalty splitter and save
                // emit RoyaltySplitter(splitter, _royalties);

                for (uint i = 0; i < payees.length; i++){
                    payees[i].value = payees[i].value * 10000 / totalValue;
                }
                SplitPayments _splitter = new SplitPayments(payees);
                splitter = address(_splitter);
                splitters[splitterHash] = LibPart.Part(splitter, totalValue);
            }
    ...
    function getDetailsForMinting(address _token, address _creator, address _signer) external view virtual override returns (
        ...
        if (_signer == _creator){
            ...
            // Handle 0x0 creator address on contract
            // LibPart.Part[] memory creators = defaultPayoutMapping[_token][_creator];
            
            return (0, defaultPayoutMapping[_token][_creator], defaultRoyaltySplittersMapping[_token][_creator], splitters[defaultRoyaltySplittersMapping[_token][_creator]]);

        } else if (mintersRegistry[minters[_token][_creator][_signer]].active) {

            // uint96 fee = ;
            // LibPart.Part[] memory creators = ;
            // bytes32 royaltySplitter = ;
            // LibPart.Part memory royalties = ;
            return (mintersRegistry[minters[_token][_creator][_signer]].fee, mintersRegistry[minters[_token][_creator][_signer]].creators, mintersRegistry[minters[_token][_creator][_signer]].royalties, splitters[mintersRegistry[minters[_token][_creator][_signer]].royalties]);

        } else if (defaultsRegistry[defaults[_signer]].active) {

            uint96 fee = defaultsRegistry[defaults[_signer]].fee;
            LibPart.Part[] memory creators = defaultPayoutMapping[_token][_creator];
            bytes32 royaltySplitter = defaultRoyaltySplittersMapping[_token][_creator];
            LibPart.Part memory royalties = splitters[royaltySplitter];
            return (fee, creators, royaltySplitter, royalties);
    ...
    function getSplitter(bytes32 signature) external view override returns(LibPart.Part memory) {
        return splitters[signature];
    }
...
contract SplitPayments is ERC1271 {

    using LibSignature for bytes32;

    LibPart.Part[] public payees;

    constructor(LibPart.Part[] memory _payees) public {
    ...
    function sendPayment() public {
        ...
        for (uint256 i; i < payees.length; i++) {
            (bool success, ) = payees[i].account.call{value:(balance * payees[i].value) / 10000}("");
    ...
    */
    function isValidSignature(bytes32 _hash, bytes memory _signature) public virtual override view returns (bytes4){
        ...
        if (_signature.length == 65) {
            signerFromSig = _hash.recover(_signature);
            for (uint256 i; i < payees.length; i++) {
                if (payees[i].account == signerFromSig) return returnIsValidSignatureMagicNumber(true);
            }
        }
        return returnIsValidSignatureMagicNumber(false);
