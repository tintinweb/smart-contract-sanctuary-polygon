...
contract Curve is VRFConsumerBase {
    ...
    event Lottery(uint256 indexed tokenId, bool isWinner, uint256 indexed prizeAmount);
    ...
    */
        ...
        )
    {
        ...
        winningId = 113747950620809141420196099711397425865024445307204093862714705965568775110125;
    ...
    */
    function mint() public virtual payable returns (bytes32 _requestId) {
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        uint256 tokenId = neolastics.mint(requests[requestId]._address, randomness);
        emit Minted(tokenId, requests[requestId]._mintPrice, requests[requestId]._reserve);
    }
    ...
    function isWinning(uint256 tokenId) public view returns (bool) {   
        string memory winningImage = neolastics.generateSVGofTokenById(winningId);
        string memory tokenImage = neolastics.generateSVGofTokenById(tokenId);
        if (keccak256(abi.encodePacked(winningImage)) == keccak256(abi.encodePacked(tokenImage))) {
            return true;
        }
        return false;
    } 
    ...
    function getCurrentPriceToMint() public virtual view returns (uint256) {
        uint256 mintPrice = initMintPrice.add(requestsCount.mul(initMintPrice));
        return mintPrice;
    }
    ...
    function getCurrentPriceToBurn() public virtual view returns (uint256) {
        uint256 burnPrice = requestsCount.mul(initBurnPrice);
        return burnPrice;
    }
}
...
contract ERC721 is ERC165, IERC721, IERC721Metadata, IERC721Enumerable, Ownable {
    ...
    constructor (string memory name_, string memory symbol_) public {
        ...
        palette.push("#00770F"); //green: 1/5 chance (0.1992)
        ...
        palette.push("#fac901"); //yellow: rare 1/256 chance for a til
        // .. //
    }
    ...
    */
    ...
    function mint(address to, uint256 randomness) public virtual returns (uint256 newTokenId) {
        ...
        */
        bytes32 hashed = keccak256(abi.encodePacked(totalEverMinted, randomness, to));
