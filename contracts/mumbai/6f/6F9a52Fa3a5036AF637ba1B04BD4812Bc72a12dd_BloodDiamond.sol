...
contract BloodDiamond is Context, IERC20, Ownable {
    ...
    address public marketingWallet = 0xe92d0a76B04D3d89fC199060a229D50e78355238;
    address public DeveloperWallet = 0xe92d0a76B04D3d89fC199060a229D50e78355238;
    ...
    //All Taxes implemented on Sell 3%
    uint _sellmarketingFee = 10;
    uint _SelldeveloperFee = 10;
    uint _sellliquidity = 10;

    uint denominator = 1000;

    uint public totalTaxes;

    uint256 private _totalSupply =  1100000 * 10 ** _decimals;
    ...
    constructor () {
        ...
        totalTaxes = _sellliquidity.add(_SelldeveloperFee).add(_sellmarketingFee);
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(isMarketPair[recipient]){
            feeAmount = amount.mul(totalTaxes).div(denominator);
        }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 tokensForLP = tAmount.mul(_sellliquidity).div(totalTaxes).div(2);
        uint256 tokensForSwap = tAmount.sub(tokensForLP);

        swapTokensForEth(tokensForSwap);
        uint256 amountReceived = address(this).balance;

        uint256 totalBNBFee = totalTaxes.sub(_sellliquidity.div(2));
        
        uint256 amountBNBLiquidity = amountReceived.mul(_sellliquidity).div(totalBNBFee).div(2);
        uint256 amountBNBTeam = amountReceived.mul(_SelldeveloperFee).div(totalBNBFee);
        ...
        if(amountBNBTeam > 0)
            transferToAddressETH(DeveloperWallet, amountBNBTeam);
