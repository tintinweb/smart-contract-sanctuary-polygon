...
contract AuctionWithAdmin is Ownable {
    ...
    mapping(address => uint256[]) private getSaleTokenIds;
    mapping(address => uint256[]) private getAuctionTokenIds;
    mapping(address => uint256[]) private getDealTokenIds;
    ...
    function cancelDeal(uint256 _tokenId) public {
        require(msg.sender == EnumerableMap.get(dealId, _tokenId), "You are not owner");
    ...
    function bid(uint256 _tokenId, uint256 _amount) public {
        ...
        require(
            pending_claim_auction[msg.sender][_tokenId] + _amount >= auction[_tokenId].price,
        ...
        auction[_tokenId].highestBid = pending_claim_auction[msg.sender][_tokenId];
    ...
    function sell(uint256 _tokenId, uint256 _price) public {
        ...
        getSaleTokenIds[msg.sender].push(_tokenId);
        nft_token.transferFrom(msg.sender, address(this), _tokenId);
    ...
    function cancelSell(uint256 _tokenId) public {
        ...
        curr_sale_id--;
        delete token_price[_tokenId];
        for(uint256 i = 0; i < getSaleTokenIds[msg.sender].length; i++){
            if(getSaleTokenIds[msg.sender][i] == _tokenId){
                getSaleTokenIds[msg.sender][i] = getSaleTokenIds[msg.sender][getSaleTokenIds[msg.sender].length-1];
                delete getSaleTokenIds[msg.sender][getSaleTokenIds[msg.sender].length-1];
                break;
            }
        }
        EnumerableMap.remove(saleId, _tokenId);      
    ...
    function cancelAuction(uint256 _tokenId) public {
        require(msg.sender ==  EnumerableMap.get(auctionId, _tokenId), "You are not owner");
        require(
