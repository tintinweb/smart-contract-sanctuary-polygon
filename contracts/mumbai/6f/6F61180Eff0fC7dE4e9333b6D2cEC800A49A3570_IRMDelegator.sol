    ...
    IDelegator
{
    ...
    function setPusdLowerTargetPrice(uint256 _lowerPrice) external override returns (uint256 lowerPrice) {
        bytes memory data = _delegatecall(abi.encodeWithSelector(
        ...
        emit SetPusdLowerTargetPrice(lowerPrice);
    }

    function setPusdUpperTargetPrice(uint256 _upperPrice) external override returns (uint256 upperPrice) {
        bytes memory data = _delegatecall(abi.encodeWithSelector(
        ...
        emit SetPusdUpperTargetPrice(upperPrice);
    }
