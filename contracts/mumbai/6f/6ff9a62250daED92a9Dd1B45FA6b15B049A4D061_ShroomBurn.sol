...
// File: contracts/ShroomBurn/ShroomBurnAccessControl.sol
...
abstract contract ShroomBurnAccessControl is AccessControl {
...
// File: contracts/ShroomBurn/ShroomBurn.sol
...
contract ShroomBurn is ShroomBurnAccessControl {
    IMushroom shroom;

    // price in 10**18,1 raffle -> 1e18 shroom
    uint256 public price;
    mapping(address => uint256) public burned;
    IRaffle raffle;

    event ShroomBurned(uint256 amount, address user);
    ...
    function setRaffleMultiplier(uint256 _rMul) external onlyRole(OWNER_ROLE) {
        price = _rMul;
    }

    function buyRaffle(uint256 _raffle) external whenNotPaused {
        address user = msg.sender;
        uint256 shroomToBurn = (price * _raffle);
        shroom.burnFrom(user, shroomToBurn);
        burned[user] += shroomToBurn;
        emit ShroomBurned(shroomToBurn, user);
        raffle.claimRaffle(user, _raffle, "ShroomBurner");
    }
