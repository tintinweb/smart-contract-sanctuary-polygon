    ...
    FeeSplitterUpgradeable
{
    ...
    function buyTicket(uint256 type_, address recipient_) external payable override nonReentrant {
        ...
        for (uint256 i = 0; i < length; ) {
            SafeTransferLib.safeTransferFrom(typeInfo.asset, treasury, recipient_, ids[i]);
            unchecked {
                ++i;
            }
        }

        emit BuyTicket(msg.sender, type_, typeInfo.asset, ids);
    }
}
...
interface IGachaUpgradeable {
    ...
    event BuyTicket(address user, uint256 gachaType, address collection, uint256[] tokenIds);
...
contract RandomSelectionUpgradeable is Initializable, BusinessAddressesUpgradeable, CollectionUpgradeable {
    ...
    function _selectRandomElements(address nft_, uint256 k_) internal returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < k_; ) {
            rdIdx = _getRandomNumber(seed, length);
    ...
    function _getRandomNumber(uint256 seed_, uint256 upper) private pure returns (uint256) {
        uint256 seed = uint256(keccak256(abi.encodePacked(seed_ + upper)));
        return seed % upper;
    }
