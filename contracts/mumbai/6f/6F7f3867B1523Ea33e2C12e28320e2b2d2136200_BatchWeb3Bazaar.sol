...
// File: webazaar/BatchWeb3Bazaar.sol
...
contract BatchWeb3Bazaar is Context, ERC1155Holder, ERC721Holder, MultiAccessControl
{
    ...
    struct TradeUser 
    {
         ...
         mapping(address => TradeCounterpart ) _counterpart;
    }
    ...
    //get trade users (creator and executer) and the status of the trade
    ...
    // supply tradeId and user involved on the trade then get list of contract address
    ...
    {
        Trade storage store = _transactions[tradeId];
        address[] memory tokensAddress = store._traders[userWallet].tokenAddress;

        address[]  memory _tradeTokenAddress = new address[](tokensAddress.length);
        uint256[]  memory _tradeTokenIds     = new uint256[](tokensAddress.length);
        uint256[]  memory _tradeTokenAmount  = new uint256[](tokensAddress.length);
        uint8[]    memory _tradeType         = new uint8[](tokensAddress.length);

        for(uint y = 0; y < tokensAddress.length; y++){
                TradeCounterpart memory tInfo = store._traders[userWallet]._counterpart[tokensAddress[y]];
    ...
    // supply tradeId, userWallet[creator, executer] and token address[counterpart]
    function getTrade(uint256 tradeId, address userWallet, address tokenAddress ) public view returns (uint256 tokenId, uint256 amount, uint traderType)
    {
        Trade storage store = _transactions[tradeId];
        TradeCounterpart storage tInfo = store._traders[userWallet]._counterpart[tokenAddress];
        return (tInfo.idAsset, tInfo.amount, uint8( tInfo.traderType));
    }

    // verify erc721 id ownership
    ...
    // swap erc721 token from a user to other
    ...
    // swap ERC1155 token from a user to other
    ...
    // swap ERC20 token from a user to other
    ...
    function executeTrade(uint256 tradeId )  public returns(uint256)
    {
        ...
        //check if both users still had the asset (executer and the creator then perform the trade)

        address tempTokenAddr;
        TradeCounterpart storage tempSwap;
        // creator 
        for (uint i = 0; i < store._traders[creator].tokenAddress.length; i++) 
        {
            // get token address for contract
            tempTokenAddr = store._traders[creator].tokenAddress[i];
            // use token address to index on trade swap info
            tempSwap =  store._traders[creator]._counterpart[tempTokenAddr];
            // swap methods call according with asset type
        ...
        // executer 
        for (uint i = 0; i < store._traders[executer].tokenAddress.length; i++) 
        {
            // get token address for contract
            tempTokenAddr = store._traders[executer].tokenAddress[i];
            // use token address to index on trade swap info
            tempSwap =  store._traders[executer]._counterpart[tempTokenAddr];
            // swap methods call according with asset type
             if(tempSwap.traderType == TradeType.ERC20){
        ...
        //remove trade for both creator and executer
    ...
    function verifyTradeIntegrity(address tokenAddress, uint256 tokenId,  uint256 amount, uint8 tokenType) internal pure returns(bool)
    {
                         ...
                         address  executerAddress , address[] memory executorTokenAddress, uint256[] memory executorTokenId, uint256[] memory executorAmount, uint8[] memory executorTokenType  )  public returns(uint256)
    {
        ...
        require(executorTokenAddress.length == executorTokenId.length && executorTokenAddress.length ==  executorAmount.length 
         ...
         _tradeId++;
        /*
            Validate creator is the owner of all assets he want to trade
            call verifier method for each type of contract erc20, erc1155, erc721
        */
        ...
        //creator
        //fill creator trade info for each token address
        for (uint i = 0; i < creatorTokenAddress.length; i++) 
        {
            // verify data integraty for the creator part!
            verifyTradeIntegrity(creatorTokenAddress[i], creatorTokenId[i], creatorAmount[i], creatorTokenType[i] );
            // verify trade for each contract type
            ...
            // fill the creator trade information after all checks
            _transactions[_tradeId]._traders[msg.sender].tokenAddress.push(creatorTokenAddress[i]);
            _transactions[_tradeId]._traders[msg.sender]._counterpart[creatorTokenAddress[i]].contractAddr = creatorTokenAddress[i];
            _transactions[_tradeId]._traders[msg.sender]._counterpart[creatorTokenAddress[i]].idAsset = creatorTokenId[i];
            _transactions[_tradeId]._traders[msg.sender]._counterpart[creatorTokenAddress[i]].amount = creatorAmount[i];
            _transactions[_tradeId]._traders[msg.sender]._counterpart[creatorTokenAddress[i]].traderType = TradeType(creatorTokenType[i]);
            _transactions[_tradeId]._traders[msg.sender]._counterpart[creatorTokenAddress[i]].traderStatus = UserStatus.OPEN;
        }

        //executer
        //fill executor trade info for each token address
        for (uint i = 0; i < executorTokenAddress.length; i++) 
        {
            ...
            // Verifiy if the executor owns the asset they want to swap
            ...
            // fill the executerAddress trade information after all checks
            _transactions[_tradeId]._traders[executerAddress].tokenAddress.push(executorTokenAddress[i]);
            _transactions[_tradeId]._traders[executerAddress]._counterpart[executorTokenAddress[i]].contractAddr = executorTokenAddress[i];
            _transactions[_tradeId]._traders[executerAddress]._counterpart[executorTokenAddress[i]].idAsset = executorTokenId[i];
            _transactions[_tradeId]._traders[executerAddress]._counterpart[executorTokenAddress[i]].amount = executorAmount[i];
            _transactions[_tradeId]._traders[executerAddress]._counterpart[executorTokenAddress[i]].traderType = TradeType(executorTokenType[i]);
            _transactions[_tradeId]._traders[executerAddress]._counterpart[executorTokenAddress[i]].traderStatus = UserStatus.OPEN;
        }
        _transactions[_tradeId].tradeStatus = TradeStatus.TRADE_CREATED;
        // add a trade info for both executer and creator
