...
contract StakingAlgo2 is ReentrancyGuard {
    ...
    function deposit(uint256 _amount) external nonReentrant checkActive {
        ...
        require(
            _amount * _usdPrice / (10**_decimals) >= minDepositAmount * USD,
        ...
        if (_amount * _usdPrice / (10**_decimals) < _vip1MinAmount * USD) {
        ...
        uint256 _actualAmount = _amount - _adminFee - _depositorFee;
        if (totalDeposit > 0) {
            totalDeposit += _actualAmount;
            if (_depositorFee > 0) {
                profitPerShare += _depositorFee * MAGNITUDE / totalDeposit;
                _dividends = _actualAmount * (_depositorFee * MAGNITUDE / totalDeposit);
            }
        } else {
            totalDeposit = _actualAmount;
        }

        int256 _updatedPayouts = (int256)(profitPerShare * _actualAmount - _dividends);
        ...
        uint256 _usdAmountOfUser = _totalDepositedAmount * _usdPrice / (10**_decimals);
        if (_usdAmountOfUser >= vip2MinAmount * USD) {
            userInfo[msg.sender].lockedTime = _timeNow + vip2LockingPeriod;
        } else if (_usdAmountOfUser >= _vip1MinAmount * USD) {
            userInfo[msg.sender].lockedTime = _timeNow + vip1LockingPeriod;
        } else {
            userInfo[msg.sender].lockedTime = _timeNow;
        }

        emit Deposit(msg.sender, _actualAmount, _adminFee, _depositorFee, _timeNow);
    }
    ...
    function withdraw() external nonReentrant checkActive {
        ...
        userInfo[msg.sender].depositorFeePayouts = 0;
    ...
    function getDepositorFeeAmount(address _user)
        public
        view
        returns (uint256)
    {
