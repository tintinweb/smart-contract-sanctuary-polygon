...
contract Buk is ERC1155, IERC1155Receiver, Ownable, ReentrancyGuard{
    ...
    struct Booking{
        ...
        uint256 base_price; //base price of Hotel Room on which taxes are caluculated to get total price
        uint256 discount;
    ...
    function bookRoom(Booking[] memory booking,address user, bytes memory signature, string memory nonce) external {                 
        ...
        for(uint256 i=0; i< totalNfts; i++){
           ...
           // require(allowance[msg.sender][address(this) >= TotalAmount]);
           // uint256 bukFee  = (booking[i].base_price * _bukCommission / 100);
            uint256 bukActualTotal = ((booking[i].base_price * platformFee / 100))- booking[i].discount;
            address Hotel_Treasury = idToHotel[booking[i].hotelId].hotelTreasury;
            uint256 HotelAmount = TotalAmount - bukActualTotal;
            IERC20(currency).transferFrom(msg.sender, treasury, bukActualTotal);
            IERC20(currency).transferFrom(msg.sender, Hotel_Treasury, HotelAmount);
    ...
    function cancelBookingUser(uint256 id, address user,string memory  nonce, bytes memory signature) external {
        ...
        uint256 bukComm = ((bookingDetails[id].base_price * platformFee / 100))- bookingDetails[id].discount; // bukFee and discounts deducted from Total paid amount 
        ...
        if( owner() == msg.sender || msg.sender == idToHotel[bookingDetails[id].hotelId].hotelManager){
            ...
            uint256 feeHotel =  ActualAmount*cancellationFeeHotel/10000;
            uint256 feeBuk = ActualAmount*cancellationFeeBuk/10000;
            ...
            IERC20(currency).transfer(user, ActualAmount - (feeBuk + feeHotel)); 
            }
        }
...
/*
    ...
    function checkOut(uint256 id, address user) external {
        ...
        "Access Denied" );  
        require(block.timestamp > bookingDetails[id].time + 79200+ checkOutTime,"CheckOut window closed");  
        uint256 itemId = BukMarket(marketplaceContract).getTokenToItem(id);
        if(itemId != 0){
            BukMarket(marketplaceContract).endSale(itemId);
        }
