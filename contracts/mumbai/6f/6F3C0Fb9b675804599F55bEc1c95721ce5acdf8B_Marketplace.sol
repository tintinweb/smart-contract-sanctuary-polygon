...
contract Marketplace is Exchange, IMarketplace, IERC721Receiver {
    ...
    function sellFractionInitial(
        ...
        tnft.safeTransferFrom(tokenOwner, address(this), tokenId, abi.encode(0, false));
        //fractionalize
        //check if ftnft already deployed
        ITangibleFractionsNFT ftnft = factory.fractions(tnft, tokenId);
        if(address(ftnft) == address(0)){
            ftnft = factory.newFractionTnft(tnft, 5, tokenId);
        }
    ...
    function buyFraction(
        ...
        require((existing.seller != address(0)) && (ftnft.fractionShares(fractTokenId) >= share), "NEFT");
        uint256 leftover = ftnft.fractionShares(fractTokenId) - share;
        require(
        ...
        factory.USDC().safeTransferFrom(buyer, existing.seller, amount);
        //take the fee
        if (sellFee > 0) {
            // if there is fee set,take the fee
            uint256 fee = ((amount * sellFee) / 10000);
            factory.USDC().safeTransferFrom(buyer, sellFeeAddress, fee);
        }

        if (share == ftnft.fractionShares(fractTokenId)) {
    ...
    function _sellFraction(
        ...
        require((ftnft.ownerOf(fractTokenId) == caller) || (existing.seller == caller), "NOW");
        ...
        if ((existing.tokenId == fractTokenId) && (existing.seller == caller)) {
            ...
            if (ftnft.fractionShares(fractTokenId) == shares[0]) {
                ...
                delete marketplaceFract[address(ftnft)][fractTokenId];
                //update second piece
                LotFract memory lotFract = marketplaceFract[address(ftnft)][splitedTokens[1]];
    ...
    function buy(
        ...
        if (factory.shouldLockTngbl(shouldLockTngbl)) {
            if (nft.tnftToPassiveNft(tokenId) == 0) {
                //convert locked amount to tngbl and send to nft contract
                //locktngbl can be called only from factory
                _lockTngbl(nft, tokenId, lockedAmount, _years, onlyLock);
            }
        } else {
        ...
        //buy the token
        _buy(nft, tokenId, !factory.isFactoryAdmin(_lot.seller));
    }
...
interface IFactory is IVoucher {
    ...
    event RentShareSet(address indexed oldAddress, address indexed newAddress);
    event DeployerAddressSet(
        address indexed oldAddress,
        address indexed newAddress
    );
    event FractionsDeployerAddressSet(
        address indexed oldAddress,
        address indexed newAddress
    );
    ...
    event InitialFract(address indexed ftnft, uint256 indexed tokenKeep, uint256 indexed tokenSell);
    ...
    function newFractionTnft(
        ITangibleNFT _tnft,
        uint256 _shareDecimals,
    ...
    /// @dev fetches RevenueShareContract
    function rentShare() external view returns (RevenueShare);
