...
contract ARC_Presale is Ownable, ReentrancyGuard{
    ...
    uint256 public USDpricePerMatic= 1000000000000000000; // 1 USD per Matic
    uint256 public arcreward=10000000000000000000; //10 arc
    uint256 public Tax=1000000000000000000; //1 %
    ...
    function BuyARCWithMATIC() public payable nonReentrant
    {
        ...
        uint256 Taxfee=((msg.value.mul(Tax)).div(100)).div(1e18); 
    ...
    {
        uint256 Taxfee=((value.mul(Tax)).div(100)).div(1e18);
        uint256 remainingAmount=value.sub(Taxfee);
        uint256 usdprice=(USDpricePerMatic.mul(remainingAmount)).div(1e18);
