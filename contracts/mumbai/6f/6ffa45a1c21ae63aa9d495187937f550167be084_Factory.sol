...
// File: contracts/ERC721Template.sol
...
contract Factory  {
     ...
     */
    event Deployed(address indexed account, address indexed token);

    /**
     * @dev Create FT Token
     */
    function createFT(string memory name, string memory symbol, uint8 decimals, uint256 cap, address owner) external returns (address) {
        // constructor(string memory name_, string memory symbol_, uint8 decimals_, uint256 cap_, address owner_) ERC20(name_, symbol_) {
        ERC20Template token = new ERC20Template(name, symbol, decimals, cap, owner);
        userTokens[msg.sender].push(address(token));        
        totalFT = totalFT + 1;

        emit Deployed(msg.sender, address(token));

        return address(token);
    }

    /**
     * @dev Create NFT Token
     */
    function createNFT(string memory name, string memory symbol, uint256 startTokenId, address owner) external returns (address) {
        // constructor(string memory name, string memory symbol, uint256 startTokenId, address owner) ERC721(name, symbol) {
        ERC721Template token = new ERC721Template(name, symbol, startTokenId, owner);
        userTokens[msg.sender].push(address(token));
        totalNFT = totalNFT + 1;
