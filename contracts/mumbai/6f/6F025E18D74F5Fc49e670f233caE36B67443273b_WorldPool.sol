...
contract WorldPool is ReentrancyGuard, Ownable {

    // AAVE

    address _aaveAddressesProviderAddress = 0x5343b5bA672Ae99d627A1C87866b8E53F47Db2E6;
    address ethAsset = address(0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE);
    IPoolAddressesProvider public immutable _aaveAddressProvider;
    IPool public immutable _aavePool;
    ...
    constructor() {
        _aaveAddressProvider = IPoolAddressesProvider(address(_aaveAddressesProviderAddress));
        _aavePool = IPool(_aaveAddressProvider.getPool());
        _nonce = 1;
    }

    // Staking

    function stakeContractFunds() public onlyOwner {
        uint256 amount = address(this).balance / 2;

        IERC20(ethAsset).approve(address(_aavePool), amount);
        _aavePool.deposit(ethAsset, amount, address(this), 0);
    }

    function unstakeContractFunds() public onlyOwner {
        _aavePool.withdraw(ethAsset, MAX_INT, address(this));
    }
