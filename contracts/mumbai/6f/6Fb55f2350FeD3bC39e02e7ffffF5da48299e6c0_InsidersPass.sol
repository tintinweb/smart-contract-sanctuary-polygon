    ...
    InsidersPassRoyalties //is SpaceERC20
{
    ...
    function onFixedPrice( uint256 packageID) internal view {
        require(
            ...
            "Package is Not Available for Fixed Price"
        );
    }
    ...
    function placePackageForTimedAuction(
       ...
       require(
        ...
        placePackageForSale(newPackageID, _ticketIDs,ticketCopiesCount, _startTime, _endTime,packageMinPrice,_incrementPercentage  );
    ...
    function placePackageForFixedPrice(
        ...
        uint256 newPackageID = packagesCount;
         CurrentStatus = saleTypeChoice(2);
        placePackageForSale(newPackageID, _ticketIDs,ticketCopiesCount, 0, 0,packageFixPrice,0  );
        emit availableForFixPrice(newPackageID, "Available for Fixed Price");
    }

    function checkPackageConditions(uint _eventID,  uint16[] memory _ticketIDs,uint32[] memory ticketCopiesCount) internal {
    ...
    function placePackageForRent(
        ...
        CurrentStatus = saleTypeChoice(1);
         placePackageForSale(newPackageID, _ticketIDs,ticketCopiesCount, _startTime, _endTime,packageFixPrice,0);
    ...
    function claimPackage(uint256 packageID) external {
        ...
        require(
         ...
         for (uint256 i = 0;i < ticketsPackage[packageID].ticketIDs.length;i++) {
             //ErrorIsHere
            CopiesOnBidOrSale[ticketsPackage[packageID].packageOwner][ticketsPackage[packageID].ticketIDs[i]] -= ticketsPackage[packageID].numOfCopies[i];
            Ticket[ticketsPackage[packageID].packageOwner][ticketsPackage[packageID].ticketIDs[i]].Exists = true;
            this.safeTransferFrom(
                address(this),
                ticketsPackage[packageID].packageOwner,
                ticketsPackage[packageID].ticketIDs[i],
    ...
    function placePackageForSale( uint256 packageID,
        uint16[] memory _ticketIDs,
        uint32[] memory ticketCopiesCount,
        uint32 _startTime,
        uint32 _endTime,
        uint256 packageMinPrice,
        uint8 _incrementPercentage) internal {
