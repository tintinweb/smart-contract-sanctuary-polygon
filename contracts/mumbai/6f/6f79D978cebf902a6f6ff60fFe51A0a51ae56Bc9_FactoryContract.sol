    ...
    AccessControlUpgradeable
{
    ...
    event StableTransfer(
        address indexed _stableTokenAddress,
        address indexed _userAddress,
    ...
    */
    ...
    // function stableTokenApproval(
    //     address _token,
    //     address _to,
    //     uint256 _value,
    //     string memory _uniqueId
    // ) public notZeroAddress(_token) {
    //     IToken token = IToken(_token);
    //     (bool success, bytes memory result) = address(token).delegatecall(
    //         abi.encodeWithSignature("approval(address,value)", _to, _value)
    //     );
    //     require(success, "delegatecall failed");
    //     emit StableTokenApproval(_token, _msgSender(), _to, success, _uniqueId);
    // }

    function stableTransfer(
        ...
        if (_poolAddress != address(0)) {
            ...
            if (!_direct) {
                IToken(_tokenAddress).mint(_toAddress, amount);
                emit StableTransfer(
                    _tokenAddress,
                    address(0),
                    _toAddress,
