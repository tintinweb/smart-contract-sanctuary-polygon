...
contract MyOnChainPepe is ERC721A, ReentrancyGuard, Ownable, OnChainMetadata {
    ...
    function setColor(uint256 tokenId,uint256 sHue,uint256 sSat,uint256 sLum,uint256 cHue,uint256 cSat,uint256 cLum ) external payable {
        require(ownerOf(tokenId) == msg.sender && msg.value >= (mintPrice/10) );
        require(sHue < 361);
        require(sSat < 101);
        require(sLum < 101);
        require(cHue < 361);
        require(cSat < 101);
        require(cLum < 101);
        tokenLevel[tokenId] = tokenLevel[tokenId] + 1;
    ...
    function mint(uint256 _count,address _to) external payable nonReentrant {

        require(
            totalSupply() + _count <= maxSupply
        );
        require(
            (mintPrice * _count <= msg.value) || (msg.sender == owner())
        );
    ...
    function _buildTokenURI(uint256 id) internal view returns (string memory) {
        ...
        bytes memory image = abi.encodePacked(
            ...
            Base64.encode(
                bytes(
                    abi.encodePacked(
                        '<?xml version="1.0" standalone="yes"?><svg xmlns="http://www.w3.org/2000/svg" width="420" height="303"   viewBox="0 0 840 606" preserveAspectRatio="xMidYMid meet" ><path style="fill:#383838; stroke:none;" d="M0 0L0 606L840 606L840 0L0 0z"/><path class = "skin"  style="fill:hsl(',
                        ...
                        '%); stroke:none;" d="M158 342L158 566L608 566L608 492L327 492L258 492C252.032 492 238.003 494.575 233.742 489.566C231.224 486.606 232 481.591 232 478L232 451C232 414.665 233 378.382 233 342L158 342z"/><path class = "skin2" style="fill:hsl(',
                        Strings.toString(skinHue[id]),
                        ", ",
                        Strings.toString(skinSat[id]),
                        "%, ",
                        Strings.toString(skinLum[id]),
                        '%); stroke:none;" d="M233 416L233 491L608 491L608 416L233 416z"/></svg>'
                    )
                )
            )
        );
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                ...
                                image,
                                '", "description": "My On Chain Pepe Art is generated from from the smart contract with no external dependencies. Minters selected skin and clothes color via HSL at the time of mint ","attributes": [{"trait_type": "Metadata","value": "OnChain"},{"trait_type": "Token Level","value":',
                                Strings.toString(tokenLevel[id]),
