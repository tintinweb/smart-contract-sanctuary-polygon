    ...
    ErrorCodes
{
    ...
    function validateMaxQuantityAllowed(
        ...
        try IERC1155Custom(assetContract).maxQuantityToPurchaseLimit() returns (uint256 maxQuantityAllowed) {
            if(maxQuantityAllowed > 0){
                require(
                    IERC1155(assetContract).balanceOf(buyer, tokenId) + quantityToBuy <= maxQuantityAllowed,
