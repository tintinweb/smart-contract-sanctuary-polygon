...
contract Ratings is Ownable, IRatings{
    ...
    // Gets a users rating for a site (if exists)
    function getRating(string memory site, address user) public view returns(Rating memory rating){
        // Get index
        Index memory index = _userSiteRatingsIndex[user][site];

        // Check index exists
        require(index.Exists, 'No user-site rating has been made!');

        // Get the rating
        return _allRatings[index.Position];
    }
