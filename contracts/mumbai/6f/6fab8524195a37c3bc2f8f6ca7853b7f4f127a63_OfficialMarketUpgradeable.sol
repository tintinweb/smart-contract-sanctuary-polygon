    ...
    PausableUpgradeable
{
    ...
    function initialize(
        ...
        minBidIncrementPercentage = 25; // 2.5%

        //TODO: set wallet addresses at initialize

        // cuts are in %age * 10
        treasuryCut = 100; // 10%
        playToEarnCut = 450; // 45%
        liquidityCut = 250; // 25%
        foundersCut = 200; // 20%
     ...
     * @param _tokenId ID of the token to put on sale
     * @param _amount Amount of tokens to put on sale (only relevant for ERC1155)
     * @param _fixedPrice Fixed price of the fixed price sale
     * @param _startingTime Starting time of the sale in UNIX timestamp
     * @param _cuts artist cuts
     * @param _cutReceivers artist addresses
     */
    function createSaleFixedPrice(
        ...
        whenNotPaused
    {
        require(_cuts.length == _cutReceivers.length, "array length mismatch");
        require(_cuts.length <= 3, "incorrect number of cuts");
        uint256 cutsSum;
        for (uint256 i = 0; i < _cuts.length; i++) {
            require(
                _cutReceivers[i] != address(0),
                "can't distrubute to 0 address"
            );
            require(_cuts[i] != 0, "can't distrubute 0% cut");

            cutsSum += uint256(_cuts[i]);
        }
        //ensuring artist cuts remain <= 25 also ensures market cut remains >= 75
        require(cutsSum <= 250, "cut amounts exceeding 25%");

        SaleReference memory saleReference;
        preSaleCreation(_nftAddress, _tokenId, _amount, saleReference);

        // call the internal contract function to create the fixed price sale
        createFixedSale(
    ...
    /**
     * @dev Creates the fixed price sale for a bulk of tokens without the artist fee by calling the public createSaleFixedPrice function. Can only be called by owner.
     * @param _nftAddress Address of NFT
     * @param _tokenId IDs of the tokens to put on sale
     * @param _amount Amount of each token (only relevant for ERC1155)
     * @param _fixedPrice Fixed prices of the sale
     * @param _startingTime Starting times of the sale in UNIX timestamp
     */
    function createBulkSaleFixedPriceNoArtist(
        address _nftAddress,
    ...
    function createBulkSaleFixedPrice(
        ...
        require(
            _cuts.length == _numberOfTokens,
            "cuts not equal to number of tokens"
        );
        require(
            _cutReceivers.length == _numberOfTokens,
            "cut receivers not equal to number of tokens"
        );
    ...
    /**
     * @dev Creates the auction sale for the token by calling the internal auction sale contract. Can only be called by owner.
     * @param _nftAddress Address of NFT
     * @param _tokenId ID of the token to put on auction
     * @param _amount Amount of tokens to put on sale (only relevant for ERC1155)
     * @param _startingPrice Fixed price of the auction
     ...
     * @param _duration duration times of the auction in UNIX timestamp
     */
    function createBulkSaleAuctionNoArtist(
        address _nftAddress,
        uint256[] memory _tokenId,
        uint256[] memory _amount,
        ...
        address[] memory empty_ = new address[](0);
    ...
    function withdrawArtistCut() public {
        uint256 cutValue = artistCuts[msg.sender];
        require(cutValue > 0, "no cuts to distribute");
        // reset artist cut amount
        artistCuts[msg.sender] = 0;
        Token.transfer(msg.sender, cutValue);
    }
    ...
    function _buy(uint256 _saleId) internal override {
        ...
        _payout(
            referenceFixedSale.cuts,
            referenceFixedSale.cutReceivers,
    ...
    function _finishAuction(uint256 _saleId) internal override {
        ...
        // if there was a successful bid, pay the seller and transfer the token to the buyer
            ...
            _payout(
                referenceAuction.cuts,
                referenceAuction.cutReceivers,
    ...
    function changeFees(
        ...
        require(
            _treasuryCut + _foundersCut + _playToEarnCut + _liquidityCut ==
                1000,
