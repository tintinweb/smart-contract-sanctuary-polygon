...
contract Lotery is Ownable, Pausable{
  ...
  function topReferrerVerification(address refererrToCompare) internal {
    (bool isRepited, uint pos) = topVerifications(refererrToCompare);
    if(topReferrers.length < 5 && !isRepited){
      ...
      address auxTopAddress;
        ...
        for(uint i; i < topReferrers.length; i++){          
          if (buysOfReferrerToCompare > referralsAmount[topReferrers[i]]){
            if(isRepited && pos == i){
              i = topReferrers.length; // breake the loop
            }
  ...
  function topVerifications(address refererrToCompare) internal view returns(bool verification, uint index){
