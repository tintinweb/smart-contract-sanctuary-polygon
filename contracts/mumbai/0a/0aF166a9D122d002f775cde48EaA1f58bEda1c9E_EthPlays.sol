...
/// @title An experiment in permissionless collaborative gaming
/// @author olias.eth
...
contract EthPlays is ERC20, Ownable {
  ...
  /// @notice The current reward (in POKE) for each submitted input
  ...
  uint256 private previousAlignmentDecayBlock;
  uint256 private alignmentDecayPerBlock = 900;

  /// @notice The active banner message
  string public bannerMessage;
  /// @notice The block timestamp marking the start of the active banner message
  uint256 public previousBannerMessageStart;
  ...
  uint256 public previousIndividualControlStart;
  ...
  event AlignmentVote(address from, bool alignment);
  ...
  modifier onlyActive() {
    if (!active) revert GameNotActive();
    _;
  }
  ...
  constructor() ERC20('Ethereum Plays', 'POKE', 18) {
    active = false;
    reward = 10 * (10**18);
    chatCost = 5 * (10**18);
    bannerMessageCost = 1000 * (10**18);
    individualControlCost = 3000 * (10**18);
    inputIndex = 0;
  }
  ...
  /// @param buttonIndex The index of the button input. Must be between 1 and 8.
  function submitButtonInput(uint256 buttonIndex) external onlyActive onlyRegistered {
    ...
    if (
      block.timestamp <= previousIndividualControlStart + 30 seconds &&
      msg.sender != individualControlAddress
    ) {
  ...
  /// @param _alignment The alignment vote. True corresponds to order, false to chaos.
  function submitAlignmentVote(bool _alignment) external onlyActive onlyRegistered {
    if (block.number - previousAlignmentDecayBlock < 100) {
      // If players are active, reduce alignment magnitude by 10% each block
      alignment *= int256(alignmentDecayPerBlock);
      alignment /= int256(1000);
    } else {
      // If no player has submitted a vote in 100 blocks, reset alignment to zero.
      alignment = 0;
    }

    // Apply sender alignment update.
    alignment += _alignment ? int256(1000) : -1000;

    previousAlignmentDecayBlock = block.number;

    emit AlignmentVote(msg.sender, _alignment);
  }
  ...
  function submitBannerMessage(string memory message) external onlyActive onlyRegistered {
    ...
    if (block.timestamp < previousBannerMessageStart + 1 minutes) {
      revert BannerMessageActive();
    }

    _burn(msg.sender, bannerMessageCost);

    previousBannerMessageStart = block.timestamp;
  ...
  function submitIndividualControl() external onlyActive onlyRegistered {
    ...
    if (block.timestamp < previousIndividualControlStart + 1 minutes) {
      revert IndividualControlActive();
    }

    _burn(msg.sender, individualControlCost);

    previousIndividualControlStart = block.timestamp;
  ...
  /// @notice Indicates if the banner message is currently active/reserved.
  /// @return result The result, where true means the banner message is active/reserved.
  function isBannerMessageActive() external view returns (bool) {
    return block.timestamp <= previousBannerMessageStart + 1 minutes;
  }

  /// @notice Indicates if individual control is currently active/reserved.
  /// @return result The result, where true means individual control is active/reserved.
  function isIndividualControlActive() external view returns (bool) {
    return block.timestamp <= previousIndividualControlStart + 30 seconds;
  }

  /// @notice Indicates if the game is currently in chaos mode.
  /// @return result The result, where true means the game is in chaos mode.
  function isChaos() external view returns (bool) {
    return alignment <= 0;
  }

  /// @notice Indicates if the game is currently in order mode.
  /// @return result The result, where true means the game is in order mode.
  function isOrder() external view returns (bool) {
    return alignment > 0;
  }
