...
contract StakingLP is
    ...
    */
     ...
     * @return the withdrawn user rewards
     */
    ...
    /**
     * @notice if _lpAmount is all lp tokens staked by the user , user will receive all rewards too
     ...
     * @return the withdrawn Lp Amount and user rewards when user exit from opportunity In order
     */
    function withdraw(
        uint256 _lpAmount,
        address _originAccount
    ) external nonReentrant whenNotPaused updateReward(_originAccount) returns (uint256,uint256)
    {
        require(opportunityContract != address(0),"LPStaking:Opportunity Contract address does not set yet.");
        require(msg.sender == opportunityContract, "LPStaking: Request is not from opportunity Contract ");
        ...
        //user wants to exit from opportunity and withdraws all lp staked tokens and rewards
        if(currentLPBalance == _lpAmount)
        {
            ...
            addresses.pop();
        } else //user wants to withdraw some LP tokens
        {
    ...
    /**
     * @return how much reward ,this account has earned so far
     */
