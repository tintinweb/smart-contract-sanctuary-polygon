...
contract Vehicles is Initializable, OpenSeaCollection, ERC721PausableUpgradeable, WheelcoinPausable {
    ...
    function tokenURI(uint256 _tokenId) public view override requireMinted(_tokenId) returns (string memory) {
    ...
    function supportsInterface(
        ...
        bytes4 interfaceId
    ) public view virtual override(AccessControlUpgradeable, OpenSeaCollection, ERC721Upgradeable) returns (bool) {
        return true;
    }
}
...
library ContractMetadataGenerator {
    ...
    function valid(ContractParams memory _params) internal pure {
        require(bytes(_params.name).length != 0, "ContractParams: name can't be empty");
        require(bytes(_params.description).length != 0, "ContractParams: description can't be empty");
        require(bytes(_params.imageURL).length != 0, "ContractParams: image can't be empty");
        require(_params.sellerFeeBasisPoints <= 2000, "ContractParams: seller fee can't be more than 2000");
        if (_params.sellerFeeBasisPoints > 0) {
            require(_params.feeRecipient != address(0), "ContractParams: no recipient, but fee>0");
        }
    }
