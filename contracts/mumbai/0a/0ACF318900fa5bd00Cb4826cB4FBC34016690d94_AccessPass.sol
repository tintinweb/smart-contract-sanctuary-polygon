...
contract AccessPass is Initializable, ERC721URIStorageUpgradeable {
    ...
    address[] public accessPassHolders;
    ...
    function buyAccessPass() external payable {
        require(msg.value >= price, "Please pay the right amount");

        _safeMint(msg.sender, _tokenIds.current());

        _tokenIds.increment();
        accessPassHolders.push(msg.sender);
    }

    function getAccessPassHolders() public view returns (address[] memory) {
        return accessPassHolders;
    }

    function getSummary() public view returns(uint256, address, uint256, uint256) {
        return (price, owner, accessPassHolders.length, address(this).balance);
    }
