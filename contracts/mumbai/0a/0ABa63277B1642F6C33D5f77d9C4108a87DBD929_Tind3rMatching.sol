    ...
    ITind3rMatching
{
    ...
    ITind3rMembership public membershipContract;
        ...
        initializer
    {
        ...
        membershipContract = initMembershipContract;
    }
     ...
     */
    function uri(uint256) public view virtual override returns (string memory) {
        return string.concat(membershipContract.prefixURI(), "{id}");
    }
    ...
    function burn(
        ...
        if (
            address(membershipContract) == _msgSender() ||
            account == _msgSender() ||
     ...
     */
    modifier onlyT3M() {
        if (msg.sender != address(membershipContract))
    ...
    function _afterTokenTransfer(
        ...
        uint256 fromId = membershipContract.getUserId(from);
        uint256 toId = membershipContract.getUserId(to);
        for (uint256 i = 0; i < size; ++i) {
            uint256 targetId = ids[i];
            address targetUser = membershipContract.ownerOf(targetId);
