...
interface IsoftwareNFT {
 ...
 contract softwarenftstake  {
    ...
    mapping(uint256=>address) stakersByAddress; // mapping of contract number to address of the 
...
mapping(address=>mapping(uint256=>uint256)) prevReward;
mapping(address=>uint256) public numOfDays; // mapping from user address to contract count
     uint256 public t1;
     uint256 public stakeTime;
     uint256 updateRewardCount;
    function updateRewardForStakers() public {
        ...
        for(uint256 i=0;i<contractCount;i++){
            for(uint256 j=0;j<stakingCount[stakersByAddress[i]];j++){
                 ...
                 numOfDays[stakersByAddress[i]]++;
                 stakingRewards[stakersByAddress[i]][j].currentReward =mintProduction(stakersByAddress[i],j)+reward;
...
mapping(address=>mapping(uint256=>uint256)) prevRewardPerMonth;
mapping(address=>uint256) numOfMonths; 

     function updateRewardForStakersPerMonth() public {
   
        require(owner==msg.sender,"Only Owner Can Update");
        require( block.timestamp - t1   >=  120 , " 2 min not completed");
        for(uint256 i=0;i<contractCount;i++){
            for(uint256 j=0;j<stakingCount[stakersByAddress[i]];j++){
                 ...
                 uint reward=  prevReward[stakersByAddress[i]][i];
                 numOfDays[stakersByAddress[i]]++;
                 stakingRewards[stakersByAddress[i]][j].currentReward =mintProduction(stakersByAddress[i],j)*30+reward;
...
return (mimValue_ , diff);
   ...
   function carryingCapacity() public view returns(uint256){
    return 15*(nftContract.totalSupply()/250000); 
...
function onERC1155Received(address, address, uint256, uint256) public virtual returns (bytes4) {
    ...
    function onERC721Received(address, address, uint256) public virtual returns (bytes4) {
