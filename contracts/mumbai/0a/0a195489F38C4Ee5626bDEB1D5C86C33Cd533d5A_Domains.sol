...
contract Domains is ERC721URIStorage, Ownable {
  ...
  function price(string calldata name) public pure returns (uint) {
    ...
    if (len <= 2) revert TooShort(name);
    if (len == 3) {
      return 5 * 10 ** 15; //0.005
    } else if (len >= 4) {
      return 3 * 10 ** 15; //0.003
    }
  }
  ...
  function register(string calldata name) external payable {
    ...
    if (domains[name] != address(0)) revert Unauthorized();
    if (!valid(name)) revert InvalidName(name);
    // Check if enough Matic was paid in the transaction
    if (msg.value < _price) revert LowFunds();
  ...
  function setRecord(
    ...
    if (domains[name] != msg.sender) revert NotDomainOwner();
    records[name] = record;
