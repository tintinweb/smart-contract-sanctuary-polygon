...
contract PirateNFTMock is PirateNFT {
    ...
    constructor(address gameRegistryAddress, address traitsProviderAddress)
        PirateNFT(gameRegistryAddress, traitsProviderAddress)
    {}
    ...
    function mintSoulbound(address to, uint256 tokenId) external {
        _safeMint(to, tokenId);
        _setSoulbound(tokenId);
    }

    function lockTokenForTests(uint256 tokenId) external {
        _lockToken(tokenId);
    }

    function unlockTokenForTests(uint256 tokenId) external {
...
library TraitsLibrary {
    ...
    uint32 public constant COMMAND_RANK_TRAIT_ID = 3;

    // Total gold capacity for a ship
    uint32 public constant SHIP_GOLD_CAPACITY_TRAIT_ID = 4;

    // How much gold per day the ship earns
    uint32 public constant SHIP_GOLD_PER_DAY_TRAIT_ID = 5;
