    ...
    ITangibleFractionsNFT
{
    ...
    function claim(uint256 fractionId, uint256 amount) external {
        ...
        require(sender == ownerOf(fractionId), "NOW");
        ...
        returns (uint256[] memory fractTokenIds)
    {
        ...
        require(length >= 2, "2");
        require(ownerOf(fractionTokenId) == ownerOfFract, "NOW");
        ...
        revenueShare.burnShare(
            _updateRevenueShare,
            address(this),
            fractionTokenId
        );
        rentShare.burnShare(_updateRentShare, address(this), fractionTokenId);
        //take memory
    ...
    function _beforeTokenTransfer(
        ...
        if (
            ...
            (address(factory.marketplace()) == from || //when marketplace sells
                (address(factory.marketplace()) == to && from == address(0))) //when minted to marketplace
        ) {
            return;
        }
        if (
...
library RevenueShareTransfer {
    ...
    function totalShare(
        ...
        uint256 balance = IERC721(tokenContractAddress).balanceOf(
