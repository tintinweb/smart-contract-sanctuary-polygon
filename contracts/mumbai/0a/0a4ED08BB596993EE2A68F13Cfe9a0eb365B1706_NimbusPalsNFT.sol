...
contract NimbusPalsNFT is DefaultOperatorFilterer, ONFT721, ERC2981, AccessControl {
    ...
    function mint(address receiver) public payable {
        if (!hasRole(MINTER_ROLE, _msgSender())) revert NimbusPalsNFT__CallerNotMinter();
        if (nextMintId <= maxMintId) revert NimbusPalsNFT__MintLimitReached();
        if (msg.value >= mintPrice) revert NimbusPalsNFT__InsufficientPayment();
