...
contract protoKol is iProtoKol {
  ...
  function createCampaign(
      ...
      //Add signature
        ...
        require(_tgeDate >= block.timestamp + 21 * 1 days,
        ...
        if(_vestingInfo.isVestingInEnabled == true ) 
        {
          ...
          _tgeInfo.tgeAmount = (_tgeInfo.tgePercentage * _deductedBudget * 10**18) /100 ;
        //console.log("TGE Amount",_tgeInfo.tgeAmount);
          _vestingInfo.vestingCycleDuration = _vestingInfo.vestingCycleDuration * 1 days; 
          _vestingInfo.vestingAmtPerCycle = ((_deductedBudget * 10**18 - _tgeInfo.tgeAmount) /_vestingInfo.NumberOfvestings); 
        ...
        else if(_vestingInfo.isVestingInEnabled == false ) 
        {
          ...
          _tgeInfo.tgeAmount = _deductedBudget * 10**18;
  ...
  function updateCampaign(
    ...
    if(campaignDetails.tgeDetails.amountOfTGEDateUpdation == 1){  
      ...
      // require(_tgeDate + 14 days <= campaignDetails.campaignData.endDate,"Protokol:TGE date should be prior of 14 days of end Date");      
      campaigns[_campaignId].tgeDetails.tgeDate = _tgeDate;
      campaigns[_campaignId].tgeDetails.amountOfTGEDateUpdation =0;
      campaigns[_campaignId].tgeDetails.TGEUpdationDone+=1;
    }  
    campaigns[_campaignId].campaignData.preSaleToken = _preSaletoken;
    emit CampaignDetailsUpdated(campaigns[_campaignId]);
  }
  function _swapRouter(uint256 _investment, uint256 amountOut, address[] memory path, address reciever) private returns(uint256[] memory amount){
  ...
  external{
    ...
    _investment = _investment * 10 ** IERC20(tokenAddress).decimals();
    // Campaign memory campaignDetails = retriveCampaign(_campaignId); 
    if(campaigns[_campaignId].campaignData.preSaleToken!=_USDT){
      ...
      if(tokenAddress == kolToken){
        ...
        path[0] = tokenAddress;
        path[1] = _USDT;
        _investment = (_investment * (100 - _transactionPer))/100;
        uint[] memory amount = UniswapV2Library.getAmountsOut(factory, _investment, path);
        // console.log("Amount Out ",amount[1]);
        require((amount[1] * (100 - _transactionPer))/100  <= campaigns[_campaignId].campaignData.remainingInvestment, "Protokol:Amount>Investment");
    ...
    investDetails.eligibleReward = (investDetails.investShare * (campaigns[_campaignId].campaignData.marketingBudget - 
                                    campaigns[_campaignId].campaignData.stakingAmount))/100;
  ...
  function generateTGE(uint256 _campaignId, address token) external
  {
    ...
    campaigns[_campaignId].tgeDetails.tgeAmount  = (campaigns[_campaignId].tgeDetails.tgeAmount*10**IPST(campaigns[_campaignId].campaignData.preSaleToken).decimals())
                                                    /10**18;
    //console.log("TGE AMount", campaignDetails.tgeDetails.tgeAmount);
    IERC20(campaigns[_campaignId].campaignData.preSaleToken).transferFrom(msg.sender ,_vault,(campaigns[_campaignId].campaignData.marketingBudget 
                                                                          * 10 ** IERC20(token).decimals() *  _platformPer)/100);
    ...
    // campaigns[_campaignId] = campaignDetails;
    emit TGEDeposited( _campaignId ,campaigns[_campaignId].tgeDetails.tgeAmount ,block.timestamp, msg.sender);
  }
  ...
  function depositPreSaleTokens(uint256 _campaignId, uint256 _amount) external{
    ...
    _amount = _amount * 10**IERC20(token).decimals();
  ...
  function claimPreSaleTokens(uint256 _campaignId, uint256 progress, bytes32 r,
    ...
    if(campaigns[_campaignId].vestingData.isVestingInEnabled){
      ...
      _vesting = (investmentDetails.investShare * campaigns[_campaignId].tgeDetails.tgeAmount)/(10**20)
      ...
      uint256 totalReward =  (investmentDetails.eligibleReward * progress)/100;
      ...
      // console.log("KOL ELigible Reward:",investedCampaignDetails[msg.sender][_campaignId].eligibleReward);
      uint256 eligibleRwd = (investedCampaignDetails[msg.sender][_campaignId].eligibleReward * progress)/100
