...
contract StanNFTV2 is ERC721A {
    ...
    modifier onlyAuthorizerAndOwner(uint256 _tokenId) {
        if (!whiteList[msg.sender] && ownerOf(_tokenId) != msg.sender)
            revert InvalidOwner();
        _;
    }
    ...
    function moveBatchNFTToCollection(
        ...
        if (!_checkOwnerNFTs(_tokenIds, maker)) revert InvalidOwnerNFT();
    ...
    function createNFTByStan(
        ...
        if (_nftIds.length != _quantity) revert InvalidParameter();
    ...
    function approveForAll(address[] calldata _operators, bool _approved)
        external
    {
