...
contract Marketplace is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function listItem(
        ...
        if (IERC165(_nftAddress).supportsInterface(INTERFACE_ID_ERC721)) {
            ...
            require(
                IMultiTokenFactory(addressRegistry.MultiTokenFactory()).exists(
                    _nftAddress
                ),
                "NFT Wasn't minted on Veritone"
            );
    ...
    function _isNFT(address _nftAddress) internal view returns (bool) {
        return
            addressRegistry.NFT() == _nftAddress ||
            INFTFactory(addressRegistry.factory()).exists(_nftAddress);
    }
    ...
    function getPrice(address _payToken) public returns (int256) {
        ITreasuryManagement priceFeed = ITreasuryManagement(
            addressRegistry.treasuryManagement()
        );
        require(
            priceFeed.whitelistedToken(_payToken),
            "Not a whitelited token"
        );
        require(_payToken != address(0), "Only ERC20 tokens");
        return
            priceFeed.getPriceOfTokenByAddress(_payToken) * (int256(10)**(8));
    }
    ...
    function _validPayToken(address _payToken) internal view {
        require(
            (addressRegistry.treasuryManagement() != address(0) &&
                ITreasuryManagement(addressRegistry.treasuryManagement())
                    .whitelistedToken(_payToken)),
            "invalid pay token"
        );
    }
...
interface IAddressRegistry {
  ...
  function updateMintRequest ( address _mintRequest ) external;
  function updateMultiTokenFactory ( address _MultiTokenFactory ) external;
  function updateNFT ( address _NFT ) external;
  function updateNFTFactory ( address _factory ) external;
  function updateTreasuryManagement ( address _treasuryManagement ) external;
  function updateWhitelistContract ( address _whitelist ) external;
...
interface INFTFactory {
  ...
  function updateTollplazaAddress ( address _tollplazaAddress ) external;
  function updateWhitelistContract ( address _whitelistAddress ) external;
...
interface IMultiTokenFactory {
  ...
  function treasuryManagement (  ) external view returns ( address );
  function unpause (  ) external;
  function updateTollPlazaContract ( address _treasuryManagement ) external;
  function updateWhitelistContract ( address _whitelistAddress ) external;
  function whitelistAddress (  ) external view returns ( address );
}
