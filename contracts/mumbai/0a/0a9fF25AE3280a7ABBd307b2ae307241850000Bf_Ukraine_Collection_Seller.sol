...
// File: contracts/NFT_Initial_Ukraine_Seller.sol
...
// MUST be created by the same owner as Ukraine_Collection
contract Ukraine_Collection_Seller is Ownable { // to customize

    address _owner = msg.sender;

    address public _receiver = 0x5C61b722a3f946930eA4c60AA989F61aE2163646;  // МОЗ допомога медикам
    
    struct Listing {
        uint256 price; // 75000000000000000 Wei = 0.075 ETH (https://converter.murkin.me/)
        address seller; // must be msg.sender
    }
    ...
    function addListing(address contractAddr, uint256 tokenId) public onlyOwner {
        ...
        require(listings[contractAddr][tokenId].seller != msg.sender, "Listing already exists!");

        listings[contractAddr][tokenId] = Listing(75000000000000000, msg.sender);
    }
    ...
    function buy(address contractAddr, uint256 tokenId, uint256 amount) public payable {
        ...
        require(token.balanceOf(_owner, tokenId) >= amount, "All NFTs of this tokenId are bought (you cannot buy more than 40)!");
