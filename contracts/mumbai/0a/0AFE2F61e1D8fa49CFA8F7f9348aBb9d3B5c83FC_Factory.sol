    ...
    AccessControlUpgradeable
{
    ...
    event SharePercentSet(
        uint256 borborrowerSharePercent,
    ...
    function loanTransferOfBlacklistedBank(
        ...
        if (Paza.balanceOf(pools[_blacklistedBank]) > 0) {
            IPool(pools[_blacklistedBank]).removeStakedTokens(_auctionWinner);
        }
...
interface IPool {
    ...
    function removeStakedTokens(address _auctionWinner) external;
    function getUserDetails(address borrower, uint loanId) external view returns(LoanLib.Loan memory _details);
    function transferRewards(address _borrower, uint _loanId) external;
}
