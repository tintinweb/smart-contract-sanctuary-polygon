...
contract swapHelperDiyTask is OpsTaskCreator, Ownable {
    ...
    event swapToenTaskEvent(uint256 _index, bytes32 _md5, bytes32 _taskID);

    constructor(uint256 _approveAmount, address payable _ops, address _fundsOwner) OpsTaskCreator(_ops, _fundsOwner){
    ...
    function swapToken(address _user, bytes32 _md5) external onlyDedicatedMsgSender {
        ...
        if (x._swapIn == 0) {
            ...
            emit swapToenTaskEvent(y._index, y._md5, md5TaskList[_md5]);
