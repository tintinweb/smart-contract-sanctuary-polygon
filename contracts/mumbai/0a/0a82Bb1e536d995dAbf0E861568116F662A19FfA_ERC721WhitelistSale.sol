...
contract ERC721BaseSale is App {
    bytes32 constant WALLET_EXTERNAL_APP = keccak256("WALLET");
    ...
    function _transferToWallet() internal {
        address wallet = _kernel.getAppAddress(WALLET_EXTERNAL_APP);
...
contract ERC721WhitelistSale is ERC721BaseSale {
    bytes public merkleProofIpfsHash;
    ...
    function setMerkleTree(
        bytes32 merkleRoot,
        bytes calldata merkleProofIpfsHash_
    )
        external
        requirePermission(ADMIN) 
    {
    ...
    function buy(bytes32[] calldata proof,  bytes32 tierValue) external payable {
        ...
        require(msg.value >= cost && cost != 0, Exceptions.VALIDATION_ERROR);
        ERC721Properties(_kernel.getAppAddress(ERC721_APP)).mint(msg.sender, tierValue);
