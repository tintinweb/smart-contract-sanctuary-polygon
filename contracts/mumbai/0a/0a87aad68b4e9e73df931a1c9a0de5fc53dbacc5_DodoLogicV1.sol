...
struct Prop {
 ...
 contract DodoLogicV1 is Initializable {
  ...
  // 获取上次投注结果的key
  function getLastBetResultKey() private pure returns (string memory) {
    return "lastBetResult";
  }

  // 获取上次投注额的key
  function getLastBetAmountKey() private pure returns (string memory) {
  ...
  function getIncomeInfo(address player) public view returns (uint256, uint256) {
    PlayerData memory playerData = DodoStorageInterface(dataContract).getPlayerData(player);
    return (getIncomeBase(),playerData.incomeLevel * getIncomeLevelRate());
  }
  ...
  function checkWin(uint256 result) private pure returns (uint256) {
    ...
    // 如果 result 中出现 个十百位上的数都是7 odds = 25； 2个7 odds = 8； 其他3个相同数字 odds = 8； 2个相同数字 odds = 3；
  ...
  function casinoBet(uint256 amount) public {
    ...
    require(amount <= getBetMaxAmount(), "DodoLogic: amount must <= betMaxAmount");
    // 判断余额是否足够
    uint256 balance = IDodoCoin(tokenContract).balanceOf(player);
    ...
    // 更新玩家投额外数据
    DodoStorageInterface(dataContract).setPlayerExtraData(player, getLastBetResultKey(), result);
    DodoStorageInterface(dataContract).setPlayerExtraData(player, getLastBetAmountKey(), amount);
    // 触发投注事件
    emit betEvent(player, amount, 0, 0);
  }

  // 获取上次投注结果
  function getLastBetInfo(address player) public view returns (uint256 result, uint256 amount, uint256 odds) {
    result = DodoStorageInterface(dataContract).getPlayerExtraData(player, getLastBetResultKey());
    amount = DodoStorageInterface(dataContract).getPlayerExtraData(player, getLastBetAmountKey());
