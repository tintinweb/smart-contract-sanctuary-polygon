...
import "./access/OnlyFromAddress.sol";
...
contract World is IWorld, OnlyFromAddress {
    ...
    function expandWorldRadius(uint256 amount) external onlyFromAddress(hub) {
        worldRadius += amount;
    }
...
abstract contract OnlyFromAddress is Context {
    modifier onlyFromAddress(address from) {
        require(from == _msgSender(), "OnlyAddress: call not from address");
        _;
    }
}
