...
contract CvaFactory is AccessControlEnumerable, Ownable, Pausable, ICvaFactory {
  ...
  address public cvaAddress; // chain VA
  ...
  event CvaCreated(bytes userDID, address newCvaAddress, uint256 createdAtTime, uint256 createAtBlock);

  event NativeReceived(address sender, address cvaAddress, uint256 amount);
  ...
  function getCva(bytes calldata _userDID, bytes calldata _salt) external view returns (address) {
    return cvaAddress.predictDeterministicAddress(getFinalSalt(_userDID, _salt));
  }

  function isCvaDeployed(address _cvaAddress) public view returns (bool) {
    return cvaAddress.isClone(_cvaAddress);
  }

  function createCva(bytes calldata _userDID, bytes calldata _salt) external whenNotPaused {
    require(_msgSender() == deployerAddress, '!deployer');
    require(isValid(), '!valid');

    address payable clone = payable(cvaAddress.cloneDeterministic(getFinalSalt(_userDID, _salt)));

    // Initialize cva
    Cva(clone).init(_userDID, wethAddress);

    emit CvaCreated(_userDID, clone, block.timestamp, block.number);
  }

  function emitNativeReceived(address _sender, address _cvaAddress, uint256 _amount) external {
    require(_msgSender() == _cvaAddress && isCvaDeployed(_cvaAddress), '!cva');

    emit NativeReceived(_sender, _cvaAddress, _amount);
  }

  function setCvaAddress(address _cvaAddress) external onlyOwner whenPaused {
    cvaAddress = _cvaAddress;
  }
