...
contract Trade {
    ...
    constructor(address _erc20Address, address _transferProxy) {
    ...
    function verifyBuyerSign(
        ...
        bytes32 hash = keccak256(
            abi.encodePacked(assetAddress, price, tokenURI)
        );
        require(
    ...
    function buyAsset(
        ...
        require(
            (order.amount >= order.unitPrice * order.qty),
            "Paid invalid amount"
        );
        verifyBuyerSign(
            order.buyer,
        ...
        tradeAsset(order, order.buyer, order.seller);
