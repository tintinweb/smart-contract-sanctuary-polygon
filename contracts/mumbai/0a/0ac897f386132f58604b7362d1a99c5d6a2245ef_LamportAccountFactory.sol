...
// File contracts/AccountAbstraction/core/Helpers.sol
...
// File contracts/AccountAbstraction/interfaces/UserOperation.sol
...
// File contracts/AccountAbstraction/interfaces/IAccount.sol
...
// File contracts/AccountAbstraction/interfaces/IAggregator.sol
...
// File contracts/AccountAbstraction/interfaces/IStakeManager.sol
...
// File contracts/AccountAbstraction/interfaces/IEntryPoint.sol
...
// File contracts/AccountAbstraction/core/BaseAccount.sol
...
// File contracts/AccountAbstraction/Lamport/LamportBaseA.sol

pragma solidity ^0.8.12;



abstract contract LamportBaseA is BaseAccount, Pausable  {
	using Address for address payable;
	...
	function verify_u256(uint256 bits, bytes[256] memory sig, bytes32[2][256] memory pub) public pure returns (bool) {
		unchecked {
    ...
    */
	...
	/*
	    ...
	    @author William Doyle
	*/
	// function addPublicKeyHashes(bytes32[] memory publicKeyHashesToAdd, bytes32[2][256] calldata publicKey, bytes[256] calldata signature) public payable redeemLamport(publicKey, signature, abi.encodePacked(publicKeyHashesToAdd, hex"341a3860")) {
    ...
    */
	...
	/*
        ...
        @author William Doyle
    */
	// function removePublicKeyHashes(bytes32[] memory publicKeyHashesToRemove, bytes32[2][256] calldata publicKey, bytes[256] calldata signature) public whenNotPaused redeemLamport(publicKey, signature, abi.encodePacked(publicKeyHashesToRemove, hex"46f5b4ad")) {
	...
	// function togglePause(bytes32[2][256] calldata publicKey, bytes[256] calldata signature) public redeemLamport(publicKey, signature, abi.encodePacked(Constants.TOGGLE_PAUSE_COMMAND))  {
...
// File contracts/AccountAbstraction/Lamport/UpgradeablePullPayments.sol

pragma solidity ^0.8.12;

abstract contract UpgradeablePullPayment {
...
// File contracts/AccountAbstraction/Lamport/LamportAccount.sol
    ...
    IERC1363Spender
{
    ...
    function initialize(
     ...
     function supportsInterface(bytes4 interfaceId) public view virtual override (ERC1155Receiver, ERC165) returns (bool) {
    ...
    function _validateSignature(
        ...
        // require(liveKeyCount > 0, "account: no live keys");

        (                                                                   //      decode signature into [key hash, lamport signature, lamport public key, ecdsa signature]
            bytes32 selectedKey,
            ...
            bytes memory ecdsaSig
                ...
                userop.signature,
                (bytes32, bytes[256], bytes32[2][256], bytes)
            );

        bytes32 hash = userOpHash.toEthSignedMessageHash();                 //      hash user op hash again to get eth signed message hash

     
        if (owner != hash.recover(ecdsaSig))                                //      check ECDSA signature
            return SIG_VALIDATION_FAILED;

		bytes32 publicKeyHash = keccak256(abi.encodePacked(publicKey));

        if (selectedKey != publicKeyHash)                                   //       keys need to match
            return SIG_VALIDATION_FAILED;

        if (!isRedeemable(publicKeyHash))  
            return SIG_VALIDATION_FAILED;

        bool isValidSig = verify_u256(
            uint256(hash),
            sig,
            publicKey
        );
        ...
        _removeSinglePublicKeyHash(selectedKey);
        emit RemovedPublicKeyHashes(liveKeyCount);
        return 0;
    }
   ...
   function executeBatch(address[] calldata dest, bytes[] calldata func) external {
     ...
     * ERC1362
     */
    function onApprovalReceived(address sender, uint256 amount, bytes calldata data) external override returns (bytes4) {
  ...
  function onTransferReceived(
	...
	function isValidSignature(bytes32 hash, bytes memory missing) public view override returns (bytes4) {
...
// File contracts/AccountAbstraction/Lamport/LamportAccountFactory.sol
...
contract LamportAccountFactory {
    ...
    function createAccount(address owner,uint256 salt, bytes32[] memory firstLamportKeys) public returns (LamportAccount ret) {
        ...
        if (codeSize > 0) {
            return LamportAccount(addr);
        }
        // ret = LamportAccount(payable(new ERC1967Proxy{salt : bytes32(salt)}(                     // works
        //         address(accountImplementation),
        //         abi.encodeCall(LamportAccount.initialize, (owner, firstLamportKeys))
        //     )));

            //  ret = LamportAccount(address(new ERC1967Proxy{salt : bytes32(salt)}(                // works
            //     address(accountImplementation),
            //     abi.encodeCall(LamportAccount.initialize, (owner, firstLamportKeys))
            // )));
        ...
        ret = LamportAccount(address(proxy));
    }
    ...
    function _addStake (uint32 unstakeDelaySec) internal {
