...
contract Blackjack is Claim, Bound {
    ...
    */
    ...
    event BetCompleted(uint256 betId, bool push, uint256 winAmount);

    constructor(address rewardAddr, address _priceFeed) {
        reward = Reward(rewardAddr);
        price = new PriceConsumerV3(_priceFeed);
    }

    modifier playerChecks(uint256 betId) {
        Game memory bet = gameId[betId];
        require(bet.player.cards.length > 0);
    ...
    */
    function play(bool isEth, address tokenAddr, uint256 amount) external betLimit(isEth, amount, tokenAddr) payable {
        ++currentBetId;
        ...
        bet.player.betAmount = betValue;

        uint8[] memory cards = drawCard(currentBetId, 3);
        ...
        emit BetPlaced(currentBetId, block.timestamp, msg.sender, betValue);
     ...
     */
    function split(uint256 betId) external payable nonReentrant playerChecks(betId) {
        Game storage bet = gameId[betId];
        require(bet.player.cards.length == 2, 'Not Allowed');
        require((bet.player.cards[0] % 13) == (bet.player.cards[1] % 13), 'Both cards must be same');
        if(isEther[betId]) {
        ...
        uint8[] memory cards = drawCard(currentBetId, 2);
     ...
     */
    function doubleDown(uint256 betId, uint256 amount) external payable nonReentrant playerChecks(betId) {
        Game storage bet = gameId[betId];
        require(bet.player.cards.length == 2, 'Not Allowed');
        if(isEther[betId]) {
            ...
            require(amount <= bet.player.betAmount, 'Amount should not be more than original bet');
            IERC20(tokenAddress[betId]).safeTransferFrom(msg.sender, address(this), amount);
            bet.player.betAmount += amount;
        }
       
        uint8[] memory cards = drawCard(currentBetId, 1);
        bet.player.cards.push(cards[0]); 

        calculate(bet.player);
        if(bet.player.score > 21) {
            bet.completed = true;
            emit BetCompleted(betId, false, 0);
        } else {
        revealDealerCard(betId);
        }
     ...
     */
    function hit(uint256 betId) external nonReentrant playerChecks(betId){
        uint8[] memory cards = drawCard(currentBetId, 1);
     ...
     */
    function stand(uint256 betId) external nonReentrant playerChecks(betId) {
        revealDealerCard(betId);
    }

    //private function used to calculate a hand score.
    function calculate(Player storage player) private {
        uint8 numOfAces = 0;
        uint8 playerScore = 0;
        for(uint8 i = 0; i < player.cards.length; i++ ) {
            uint8 index = uint8(player.cards[i] % 13);
    ...
    /**
     * private function used for revealing dealer's second card.
     * @param betId Game id
     ...
     */
    function revealDealerCard(uint256 betId) private {
        Game storage bet = gameId[betId];
        uint8[] memory card = drawCard(betId, 1);

        uint8 length = uint8(bet.dealer.cards.length + 1);
     ...
     * @notice privately calls calculatePayOut function.
    ...
    function checkWinner(uint256 betId) private {
        ...
        uint256 winAmount;
        if(bet.splitPlayer.cards.length > 0){
        ...
        if(isEther[betId]){
            withdrawEth(msg.sender, winAmount);
        }else{
            withdrawToken(msg.sender, tokenAddress[betId], winAmount);
        }
        rewardDistribution(betId);         
    }

    // private function used for calculating and updating the winning amount of the player.
    function calculatePayOut(uint256 betId, Player storage player) private {
        bool push = false;
        Game storage bet = gameId[betId];
        if(bet.player.cards.length == 2 && bet.player.score == 21 && bet.dealer.score != 21) {
        ...
        emit BetCompleted(betId, push, player.winningAmount);
    }
    ...
    function rewardDistribution(uint256 betId) private {
        ...
        int betValue = 0;
        if(bet.splitPlayer.cards.length > 0){
            betValue = int(bet.player.betAmount * 2);
        }else {
            betValue = int(bet.player.betAmount);
        }
