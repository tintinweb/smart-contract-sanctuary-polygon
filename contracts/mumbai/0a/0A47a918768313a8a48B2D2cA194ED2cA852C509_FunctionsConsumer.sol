...
contract FunctionsConsumer is FunctionsClient, ConfirmedOwner {
  ...
  function setNftAddr(address nftAddr) external {
    dNft = DynamicNFT(nftAddr);
  }
  ...
  function fulfillRequest(
    ...
    latestError = err;
    dNft.updateImage(uint256(bytes32(response)));
...
contract DynamicNFT is ERC721, ERC721Enumerable, ERC721URIStorage, ERC721Burnable, Ownable {
    ...
    constructor(address vrfAddr) ERC721("DynamicNFT", "DNFT") {
        vrfConsumer = VRFv2Consumer(vrfAddr);
    }
    ...
    function randomMint(uint256 requestId, uint256 random) external {
        uint256 tokenId = requestIdToTokenId[requestId];
        uint randomness = random % 3;
        if(randomness == 0 ) {
    ...
    function updateImage(uint256 temp) external {
        if (temp > 15) {
            updateTokenUri(METADATA_SHIBAINU_hot);
        } else {
            updateTokenUri(METADATA_SHIBAINU_cold);
        }
    }

    function updateTokenUri(string memory metadata) internal {
        for(uint256 i = 0; i < totalSupply(); i++) {
            _setTokenURI(i, metadata);
        }
    }
...
contract VRFv2Consumer is VRFConsumerBaseV2, ConfirmedOwner {
    ...
    function fulfillRandomWords(
        ...
        dNft.randomMint(_requestId, _randomWords[0]);
