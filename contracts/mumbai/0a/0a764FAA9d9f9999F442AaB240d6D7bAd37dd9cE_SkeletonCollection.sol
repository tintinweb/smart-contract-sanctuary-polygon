...
/**
   ...
   * @return partnerFee 
   */
   //function mintCardsCost(uint _amount, uint _vipCardDiscount, uint _discount, bytes32[] calldata merkleProof) public view returns (uint256 cost, uint256 collectionMintFee, uint256 partnerFee) {
function mintCardsCost(uint _amount) public view returns (uint256 cost, uint256 collectionMintFee, uint256 partnerFee) {
  // Validate amount info
 ...
 * @dev Represents a collection of card NFT in the game.
 */
contract EternityCollection is Collection, Ownable {
  using Address for address;
  using Address for address payable;
  ...
  uint256 public buyBackValue = 10000000000000000000; // 10 ETH
...
/**
  ...
  *
*/
uint256 public mintBasePrice = 97580000000000;
  ...
  for(uint8 i = 1; i < payoutDeck.length; i++) {
    ...
    for(uint8 i=0;i<13;i++){
     ...
     // comboValue = comboValue;
      ...
      buyback = (win_value * (buyBackValue / 10**18 ) * 10**18 ) / 10**8;
 ...
 * @dev Represents a collection of card NFT in the game.
 */
contract DragonCollection is Collection, Ownable {
  using Address for address;
  using Address for address payable;
...
contract IncorrectERC721ReceiverMock is IERC721Receiver {
  function onERC721Received(
    address operator,
    address from,
    uint256 tokenId,
...
contract ERC721ReceiverMock is ERC721Holder {}
