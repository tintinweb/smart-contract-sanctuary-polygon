...
// File contracts/MetaHiToken.sol
...
/**
 * The token contract itself which holds the ownership and royalty information
 */
contract MetaHiToken is ERC1155, AccessControl, Pausable, ERC1155Supply {
    ...
    mapping(uint256 => RoyaltyData) royaltyData;

    //      user    =>         tokenId    index in array
    mapping(address => mapping(uint256 => uint256)) _userTokenIndices;

    //      user       tokenId array
    mapping(address => uint256[]) _userTokenArrayMap;
    ...
    function getRoyaltyData(uint256 tokenId) public view returns(RoyaltyData memory) {
        return royaltyData[tokenId];
    }

    function changeRoyalties(uint256 tokenId, address newRoyaltyReceiver, uint256 newRoyaltyPercent) public whenNotPaused {
        require(msg.sender == royaltyData[tokenId].royaltyReceiver);
        royaltyData[tokenId].royaltyReceiver = newRoyaltyReceiver;
        royaltyData[tokenId].royaltyPercent = newRoyaltyPercent;
    }

    function mint(address account, address royaltyReceiverAddress, uint256 royaltyPercent, uint256 id, uint256 amount, uint256 supplyLimit, bytes memory data)
    public
    whenNotPaused
    onlyRole(MINTER_ROLE)
    {
        if (supplyLimit > 0) {
            require(totalSupply(id) + amount <= supplyLimit);
        }

        _mint(account, id, amount, data);
        royaltyData[id] = RoyaltyData({
            royaltyReceiver: royaltyReceiverAddress,
            royaltyPercent: royaltyPercent
        });
    }

    // Disabled. Do we have a use case for it?
...
// File contracts/MetaHiMarket.sol
...
contract MetaHiMarket is Context, Ownable, ERC1155Holder {
    string public name = "MetaHi Market V1";

    MetaHiToken tokenContract;
    ...
    mapping(address => bool) upgradeAllowances;
    address public contractUpgradeAddress; // If set, then this contract is dead

    constructor(address tokenContractAddress) {
        tokenContract = MetaHiToken(tokenContractAddress);

        // sentinel
        listings.push(ListingNode(Listing(0, payable(0), 0, 0, 0), 0, 0));
    }
     ...
     */
    function listForSale(uint256 tokenId, uint256 quantity, uint256 pricePerToken)
    public
    returns (uint256 newId)
    {
        require(isNotLocked(), "This contract has been upgraded");
        address payable caller = payable(_msgSender());
        // The below requirement is already dealt with by token contract
        // require(tokenContract.balanceOf(caller, tokenId) >= quantity, "You don't have enough tokens for this sale");

        // Make sure caller really owns the tokens - this is already covered by token contract
        // Transfer ownership to this contract
        bytes memory b = new bytes(0);
        tokenContract.safeTransferFrom(caller, address(this), tokenId, quantity, b);

        // Create a listing
        Listing memory listing = Listing(0, caller, tokenId, quantity, pricePerToken);

        // Add to listing array
        newId = _insertListingToArray(listing);

        emit ListingAdded(newId, caller, tokenId, quantity, pricePerToken);
    }
    ...
    function purchase(uint256 listingId, uint256 quantity, address receiver) public payable {
        ...
        require(listing.quantity > 0, "Listing does not exist"); // Require to exist
        ...
        // Transfer royalties
        MetaHiToken.RoyaltyData memory royaltyData = tokenContract.getRoyaltyData(listing.tokenId);
        uint256 royalties = toPay * royaltyData.royaltyPercent / 10000;
        payable(royaltyData.royaltyReceiver).transfer(royalties);
        toReturn -= royalties;

        // Transfer purchase amount
        uint256 purchaseAmount = toPay - royalties;
        listing.originalOwner.transfer(purchaseAmount);
        toReturn -= purchaseAmount;

        // Return overpay
        payable(receiver).transfer(toReturn);

        // Transfer tokens
        tokenContract.safeTransferFrom(address(this), receiver, listing.tokenId, quantity, new bytes(0));
    ...
    function removeListing(uint256 listingId) public {
        ...
        // Transfer tokens
        tokenContract.safeTransferFrom(address(this), caller, listing.tokenId, listing.quantity, new bytes(0));
    ...
    /**
     * @dev User chooses whether they allow token transfers on contract upgrade
     */
    function allowTransferOnUpgrade(bool doAllow) public {
        upgradeAllowances[_msgSender()] = doAllow;
    }

    /**
     * @dev Return whether the provided wallet allows tokens to be transferred to a new market on contract upgrade
     */
    function doesAllowTransferOnUpgrade(address seller) public view returns(bool) {
        return upgradeAllowances[_msgSender()];
    }

    /**
     * @dev If the contract is upgraded and lister allows their tokens to be transferred, this method will
     *      transfer their tokens to provided new address
     */
    function transferTokensOnUpgrade(uint256 listingId, address newOwner) public onlyOwner {
        require(!isNotLocked(), "Contract must be locked with upgrade to transfer");
        Listing storage listing = _findListing(listingId);
        require(listing.quantity > 0, "Listing does not exist");
        require(upgradeAllowances[listing.originalOwner], "Lister does not allow automatic transfer on upgrade");
        tokenContract.safeTransferFrom(address(this), newOwner, listing.tokenId, listing.quantity, new bytes(0));
    }
    ...
    function onERC1155Received(
        ...
        if (address(this) != operator) {
            return 0; // Reject token transfers not initiated by this market
        }
        return this.onERC1155Received.selector;
    }
    ...
    function onERC1155BatchReceived(
        ...
        // We don't allow receiving batch tokens
        return 0;
    }
