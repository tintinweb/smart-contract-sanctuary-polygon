...
// File: contracts/helpers/interfaces/IBAOERC20.sol
...
contract BaoTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SBAO }
    ...
    address public sBAO;
    uint public sBAOQueue; // Delays change to sBAO address
    ...
    /**
        @notice allow approved address to deposit an asset for BAO
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint BAO needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( BAO ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn BAO for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IBAOERC20( BAO ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sBAO ).balanceOf( msg.sender ); // Can only borrow against sBAO held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with BAO
        @param _amount uint
     */
    function repayDebtWithBAO( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved as a Debtor" );

        IBAOERC20( BAO ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, BAO, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( BAO ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( BAO ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns BAO valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match BAO decimals
            value_ = _amount.mul( 10 ** IERC20( BAO ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SBAO ) { // 9
            sBAOQueue = block.number.add( blocksNeededForQueue );
        } else return false;
