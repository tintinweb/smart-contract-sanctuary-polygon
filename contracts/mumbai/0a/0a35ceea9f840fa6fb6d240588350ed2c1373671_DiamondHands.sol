...
contract DiamondHands is ERC721Enumerable, Ownable{
    ...
    */
    ...
    function mint(uint256 _mintAmount) public payable {
        require(_mintAmount > 0, "bulkMint: amount cannot be 0");
        require(totalSupply() < TOTAL_SUPPLY, "bulkMint: sale has already ended");
        require(totalSupply().add(_mintAmount) <= TOTAL_SUPPLY, "bulkMint: sale has already ended");
        // require(_mintAmount <= MAX_BULK_TOKEN, "bulkMint: You may not buy more than MAX_BULK_TOKEN NFTs at once");
        // require(msg.value == _mintAmount.mul(NFT_MINT_PRICE), "bulkMint: Ether value sent is not correct");
        // uint idx = totalSupply();
    ...
    function burnBatch(uint256[] memory tokenIds) public payable{
      for (uint i = 0; i < tokenIds.length; i++) {
            burnNFT(tokenIds[i]);
        }
    }

    function burnNFT(uint256 _tokenId) public payable{
    ...
    {
        uint256[] memory tokenIds = stakers[_owner].tokenIds;
        return tokenIds;
    ...
    function unstakeBatch(uint256[] memory tokenIds) public {
        ...
        // require(
        //     (staker.stakeTime) == block.timestamp,
        //     "You have to stake for minimum 1 days."
        // );
    ...
    function _unstake(address _user,uint256 tokenIds) public {
        Staker storage staker = stakers[_user];

        uint256 lastIndex = staker.tokenIds.length;

        for(uint i = 0; i < lastIndex; i++){
            if(staker.tokenIds[i] == tokenIds){
                  delete staker.tokenIds[i];
            }
        }
        
        delete stakers[_user];
        delete tokenOwner[tokenIds];

        _transfer(contractAddress, _user, tokenIds);

        emit Unstaked(_user, tokenIds);
    }

    function setRewardPriceValue(uint256 price) public onlyOwner{
    ...
    function calculateReward(address _user) public view returns (uint256)
    {
        ...
        uint256 totalLengthArray = staker.tokenIds.length;


        uint startDate = staker.stakeTime; 
        // uint startDate =  1647757230;
        uint endDate = block.timestamp;

        uint total_day = (endDate - startDate) / 60 / 60 / 24;

        if(setRewardPrice > 0 || setRewardPrice == 0){
            require(setRewardPrice > 0,"Owner not set price"); 
        }

        uint256 reward = setRewardPrice.mul(total_day).mul(totalLengthArray);
