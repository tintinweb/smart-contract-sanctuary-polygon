...
contract LimitedEditionFacet {
    ...
    uint256 constant private SALT_1  = 1; 
    uint256 constant private SALT_2  = 2; 
    uint256 constant private SALT_3  = 3; 
    uint256 constant private SALT_4  = 4; 
    uint256 constant private SALT_5  = 5; 
    uint256 constant private SALT_6  = 6; 
    uint256 constant private SALT_7  = 7; 
    uint256 constant private SALT_8  = 8; 
    uint256 constant private SALT_9  = 9; 
    uint256 constant private SALT_10 = 10;  
    uint256 constant private SALT_11 = 11; 
    uint256 constant private SALT_12 = 12;  
    uint256 constant private SALT_13 = 13;  
    uint256 constant private SALT_14 = 14;  
    uint256 constant private SALT_15 = 15;  
    uint256 constant private SALT_16 = 16;  
    uint256 constant private SALT_17 = 17;  
    uint256 constant private SALT_18 = 18;  

    function mintLimitedEditionUnicorn(uint256 firstNameIndex, uint256 lastNameIndex, uint8 classId, string memory tokenURI, uint8[6] memory bodyPartIds) external {
        //Order: [body, face, horn, hooves, mane, tail]
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        LibDiamond.enforceIsContractOwner();
        require(classId <= 8, "LimitedEdition: Must be a valid classId");
        for(uint256 i = 0; i < 6; i++) {
            require(globalIdMatchesAValidPart(classId, i+1, bodyPartIds[i]), getErrorMessageFor(i));
        }
        require(bytes(LibUnicornNames._lookupFirstName(firstNameIndex)).length > 0 
                && bytes(LibUnicornNames._lookupLastName(lastNameIndex)).length > 0, 
                "LimitedEdition: First and last name indexes must be registered names.");

        require(!LibUnicornNames._firstNameIsAssignable(firstNameIndex) 
        ...
        dna = setBodyPartAndGenes(dna, classId, ds.bodyPartLocalIdFromGlobalId[bodyPartIds[0]], ds);
        dna = setFacePartAndGenes(dna, classId, ds.bodyPartLocalIdFromGlobalId[bodyPartIds[1]], ds);
        dna = setHornPartAndGenes(dna, classId, ds.bodyPartLocalIdFromGlobalId[bodyPartIds[2]], ds);
        dna = setHoovesPartAndGenes(dna, classId, ds.bodyPartLocalIdFromGlobalId[bodyPartIds[3]], ds);
        dna = setManePartAndGenes(dna, classId, ds.bodyPartLocalIdFromGlobalId[bodyPartIds[4]], ds);
        dna = setTailPartAndGenes(dna, classId, ds.bodyPartLocalIdFromGlobalId[bodyPartIds[5]], ds);
    ...
    function setBodyPartAndGenes(uint256 dna, uint256 classId, uint256 localPartId, LibDiamond.DiamondStorage storage ds) internal returns(uint256){
        dna = LibUnicornDNA._setBodyPart(dna, localPartId);
        dna = LibUnicornDNA._setBodyMajorGene(dna, LibHatching.getRandomGeneId(ds, classId, LibRNG.getRuntimeRNG(), SALT_1));
        dna = LibUnicornDNA._setBodyMidGene(dna, LibHatching.getRandomGeneId(ds, classId, LibRNG.getRuntimeRNG(), SALT_2));
        dna = LibUnicornDNA._setBodyMinorGene(dna, LibHatching.getRandomGeneId(ds, classId, LibRNG.getRuntimeRNG(), SALT_3));
        return dna;
    } 
    function setFacePartAndGenes(uint256 dna, uint256 classId, uint256 localPartId, LibDiamond.DiamondStorage storage ds) internal returns(uint256){
        dna = LibUnicornDNA._setFacePart(dna, localPartId);
        dna = LibUnicornDNA._setFaceMajorGene(dna, LibHatching.getRandomGeneId(ds, classId, LibRNG.getRuntimeRNG(), SALT_4));
        dna = LibUnicornDNA._setFaceMidGene(dna, LibHatching.getRandomGeneId(ds, classId, LibRNG.getRuntimeRNG(), SALT_5));
        dna = LibUnicornDNA._setFaceMinorGene(dna, LibHatching.getRandomGeneId(ds, classId, LibRNG.getRuntimeRNG(), SALT_6));
        return dna;
    }
    function setHornPartAndGenes(uint256 dna, uint256 classId, uint256 localPartId, LibDiamond.DiamondStorage storage ds) internal returns(uint256){
        dna = LibUnicornDNA._setHornPart(dna, localPartId);
        dna = LibUnicornDNA._setHornMajorGene(dna, LibHatching.getRandomGeneId(ds, classId, LibRNG.getRuntimeRNG(), SALT_7));
        dna = LibUnicornDNA._setHornMidGene(dna, LibHatching.getRandomGeneId(ds, classId, LibRNG.getRuntimeRNG(), SALT_8));
        dna = LibUnicornDNA._setHornMinorGene(dna, LibHatching.getRandomGeneId(ds, classId, LibRNG.getRuntimeRNG(), SALT_9));
        return dna;
    }
    function setHoovesPartAndGenes(uint256 dna, uint256 classId, uint256 localPartId, LibDiamond.DiamondStorage storage ds) internal returns(uint256){
        dna = LibUnicornDNA._setHoovesPart(dna, localPartId);
        dna = LibUnicornDNA._setHoovesMajorGene(dna, LibHatching.getRandomGeneId(ds, classId, LibRNG.getRuntimeRNG(), SALT_10));
        dna = LibUnicornDNA._setHoovesMidGene(dna, LibHatching.getRandomGeneId(ds, classId, LibRNG.getRuntimeRNG(), SALT_11));
        dna = LibUnicornDNA._setHoovesMinorGene(dna, LibHatching.getRandomGeneId(ds, classId, LibRNG.getRuntimeRNG(), SALT_12));
        return dna;
    }
    function setManePartAndGenes(uint256 dna, uint256 classId, uint256 localPartId, LibDiamond.DiamondStorage storage ds) internal returns(uint256){
        dna = LibUnicornDNA._setManePart(dna, localPartId);
        dna = LibUnicornDNA._setManeMajorGene(dna, LibHatching.getRandomGeneId(ds, classId, LibRNG.getRuntimeRNG(), SALT_13));
        dna = LibUnicornDNA._setManeMidGene(dna, LibHatching.getRandomGeneId(ds, classId, LibRNG.getRuntimeRNG(), SALT_14));
        dna = LibUnicornDNA._setManeMinorGene(dna, LibHatching.getRandomGeneId(ds, classId, LibRNG.getRuntimeRNG(), SALT_15));
        return dna;
    }
    function setTailPartAndGenes(uint256 dna, uint256 classId, uint256 localPartId, LibDiamond.DiamondStorage storage ds) internal returns(uint256){
        dna = LibUnicornDNA._setTailPart(dna, localPartId);
        dna = LibUnicornDNA._setTailMajorGene(dna, LibHatching.getRandomGeneId(ds, classId, LibRNG.getRuntimeRNG(), SALT_16));
        dna = LibUnicornDNA._setTailMidGene(dna, LibHatching.getRandomGeneId(ds, classId, LibRNG.getRuntimeRNG(), SALT_17));
        dna = LibUnicornDNA._setTailMinorGene(dna, LibHatching.getRandomGeneId(ds, classId, LibRNG.getRuntimeRNG(), SALT_18));
        return dna;
    }

    function globalIdMatchesAValidPart(uint256 classId, uint256 partId, uint256 globalId) internal view returns(bool isValid) {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        mapping(uint256 => uint256) storage globalIdsForClassIdAndPart = ds.bodyPartGlobalIdFromLocalId[classId][partId];
        isValid = false;
        for(uint256 localId = 0;localId <= 8 && !isValid; localId++) {
            if(globalIdsForClassIdAndPart[localId] == globalId) {
                isValid = true;
            }
        }
        return isValid;
    ...
    function getErrorMessageFor(uint256 i) internal pure returns (string memory) {
