...
/// @title NFT Admin Contract - handles the setting up / removal of Orgs and OrgAdmins
/// @author Dispatch.xyz
...
contract DispatchAdmin is NFTMetadata {
  ...
  modifier onlyAuthorised(address _orgAddress, uint256 _nftID) {
  ...
  modifier verify(address _orgAddress, uint256 _nftID) {
    // checks that the OrgAddress and nftID are in fact linked
    require(
  ...
  modifier isUnassignedToWallet(address _orgAddress) {
    // checks that the _orgAddress is NOT already linked to a walletAddress
    require(walletAddress[_orgAddress] == address(0), "A token org already exists at that address");
    _;
  }
  ...
  function removeTokenOrg(uint256 _nftID, address[] memory _adminTokensToBurn) external onlyOwner {
    // first get the tokenOrgAddr from the current tokenID <- -> tokenOrgAddr mapping
  ...
  function burnAdminNFTs(
    ...
    uint256 _nftID,
    address _orgAddress
  ) public verify(_orgAddress, _nftID) onlyAuthorised(_orgAddress, _nftID) {
    ...
    require(_adminTokenToBurn != msg.sender, "Cannot burn your own token");
  ...
  function addTeamMember(
    ...
    address _orgAddress,
    address _teamMember
  ) external verify(_orgAddress, _nftID) onlyAuthorised(_orgAddress, _nftID) {
  ...
  function mintMessage(
    ...
    uint256 _amount,
    string memory _metadata
  ) external payable onlyAuthorised(_orgAddress, _nftID) {
    (bool sent, bytes memory data) = walletAddress[_orgAddress].call{ value: msg.value }("");
