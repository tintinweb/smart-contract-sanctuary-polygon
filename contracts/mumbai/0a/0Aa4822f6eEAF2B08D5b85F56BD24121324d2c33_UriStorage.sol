    ...
    PaymentSplitter
{
    ...
    */
    function airDrop(
        address[] calldata recievers,
        string[] calldata uris
    ) external onlyOwner {
        require(recievers.length <= MAX_PER_MINT, "High Quntity");
        require(recievers.length == uris.length, "mismatch array length");

        for (uint256 i = 0; i < recievers.length; i++) {
            safeMint(recievers[i], uris[i]);
        }
    }
    ...
    function supportsInterface(
        ...
        public
        view
        override(ERC721, ERC721URIStorage, ERC721Enumerable, ERC721Royalty)
        returns (bool)
    {
        return super.supportsInterface(interfaceId);
    }
