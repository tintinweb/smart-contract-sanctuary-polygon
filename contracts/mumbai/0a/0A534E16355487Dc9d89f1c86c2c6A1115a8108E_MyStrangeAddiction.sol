...
interface ILittleSFT {

    function mint(address _to, uint _slot, uint _value) external;

    function burn(address _to, uint _slot, uint _value) external;

    function setSFTDescriptor(address _addr) external;

    function setSlotDescriptor(uint _slot, address _addr) external;

    function getFirstTokenIdOfSlot(address _user, uint _slot) external view returns (uint);

    function getSlotIndexNumber(uint _tokenId) external view returns (uint index_);
...
import "./interface/ILittleSFT.sol";

contract MyStrangeAddiction is ERC3525Upgradeable, OwnableUpgradeable, ILittleSFT {
    ...
    struct SlotData {
        ...
        mapping(uint => uint) tokenSlotIndex;
    }

    mapping(address => mapping(uint => uint)) private _userSlotFirstOf;
    mapping(uint => address) private _slotDescriptor;
    mapping(uint => SlotData) private _slotIndex;

    function initialize() initializer public {
        __Ownable_init();
        __ERC3525_init("BestLottery", "LotterySFT", 0);
    }

    function mint(address _to, uint _slot, uint _value) external override {
        _mintSFTValue(_to, _slot, _value);
    }

    function burn(address _to, uint _slot, uint _value) external override {
        _burnSFTValue(_to, _slot, _value);
    }

    function setSFTDescriptor(address _addr) external override {
        require(_addr != address(0), "ERC3525: descriptor address cannot be zero");
        ERC3525Upgradeable._setMetadataDescriptor(_addr);
    }

    function setSlotDescriptor(uint _slot, address _addr) external override {
        require(_addr != address(0), "ERC3525: descriptor address cannot be zero");
        _slotDescriptor[_slot] = _addr;
    }

    function getFirstTokenIdOfSlot(address _user, uint _slot) external view override returns (uint) {
        return _userSlotFirstOf[_user][_slot];
    }

    function getSlotIndexNumber(uint _tokenId) external view override returns (uint index_) {
        uint slot = ERC3525Upgradeable.slotOf(_tokenId);
        return _slotIndex[slot].tokenSlotIndex[_tokenId];
    }
    ...
    function _mintSFTValue(address _to, uint _slot, uint _value) internal {
        require(_to != address(0), "ERC3525: mint to the zero address");
        uint tokenId = _userSlotFirstOf[_to][_slot];

        if (tokenId == 0) {
            ERC3525Upgradeable._mint(_to, _slot, _value);
        } else {
            ERC3525Upgradeable._mintValue(tokenId, _value);
        }
    }

    function _burnSFTValue(address _to, uint _slot, uint _value) internal {
        require(_to != address(0), "ERC3525: mint to the zero address");
        require(_value != 0, "ERC3525: value cannot be zero");
        uint tokenId = _userSlotFirstOf[_to][_slot];

        ERC3525Upgradeable._burnValue(tokenId, _value);
    }
    ...
    function _beforeValueTransfer(
        ...
        if (from_ == address(0)) {
            if (_userSlotFirstOf[to_][slot_] == 0) {
                _userSlotFirstOf[to_][slot_] = toTokenId_;
            }
            return;
        }
        if (to_ == address(0) || fromTokenId_ == toTokenId_ ) {
            if (_userSlotFirstOf[to_][slot_] == fromTokenId_) {
                delete _userSlotFirstOf[to_][slot_];
            }
        }
    }
    ...
    function _afterValueTransfer(
        ...
        if (from_ == address(0)) {
            SlotData storage data = _slotIndex[slot_];
            data.counter.increment();
            data.tokenSlotIndex[toTokenId_] = data.counter.current();
            return;
        }

        if (to_ == address(0)) {
            SlotData storage data = _slotIndex[slot_];
            delete data.tokenSlotIndex[fromTokenId_];
        }
    }
}