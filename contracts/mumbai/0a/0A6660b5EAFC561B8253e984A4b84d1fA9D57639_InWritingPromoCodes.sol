...
contract InWritingPromoCodes is Ownable{
    ...
    function add_promo(string memory promo, uint256 num, uint8 percent) public onlyOwner {
        promoCodes[promo] = num;
        promoDiscount[promo] = percent;
    }

    function add_affiliate(string memory promo, address addr, uint8 percent, uint8 commission) public onlyOwner {
    ...
    function get_promo_remaining_uses(string memory promo) public view returns (uint256) {
        return promoCodes[promo];
    }

    function get_promo_discount(string memory promo) public view returns (uint8) {
        return promoDiscount[promo];
    }

    function get_affiliate_used(string memory promo) public view returns (bool) {
        return affiliateCodeUsed[promo];
    }

    function get_affiliate_address(string memory promo) public view returns (address) {
        return affiliateAddress[promo];
    }

    function get_affiliate_discount(string memory promo) public view returns (uint8) {
        return affiliateDiscount[promo];
    }

    function get_affiliate_comission(string memory promo) public view returns (uint8) {
        return affiliateCommission[promo];
    }

    function get_price_with_promo(string memory promo) public view returns (uint256) {
        return (write.get_minting_cost() * (100 - promoDiscount[promo])) / 100;
    }

    function get_price_with_affiliate(string memory promo) public view returns (uint256) {
        return (write.get_minting_cost() * (100 - affiliateDiscount[promo])) / 100;
    }

    function mint_promo_NFT(string memory str, string memory promo) public payable returns (uint256) {
        require(promoCodeUsed[keccak256(abi.encodePacked(msg.sender, promo))], "this promotional code has been used for this address already");
        require(promoCodes[promo] > 0, "this promotional code has expired");
        require(msg.value >= ((write.get_minting_cost() * (100 - promoDiscount[promo])) / 100), string(abi.encodePacked("payment not sufficient, this promotional code is only ", Strings.toString(promoDiscount[promo]), "%")));
        promoCodes[promo] -= 1;
        uint256 tokenId = write.mint_NFT{value: msg.value + write.get_minting_cost() * promoDiscount[promo] / 100}(str);
        write.transferFrom(address(this), msg.sender, tokenId);
        return tokenId;
    }

    function mint_and_send_affiliate_NFT(string memory str, string memory promo, address addr) public payable returns (uint256) {
        require(affiliateCodeUsed[promo], "this affiliate code does not exist");
        uint256 cost = ((write.get_minting_cost() * (100 - affiliateDiscount[promo])) / 100);
        require(msg.value >= cost, string(abi.encodePacked("payment not sufficient, this affiliate code is only ", Strings.toString(affiliateDiscount[promo]), "%")));
        uint256 tokenId =  write.mint_NFT{value: msg.value + write.get_minting_cost() * affiliateDiscount[promo] / 100}(str);
        write.transferFrom(address(this), addr, tokenId);
        payable(affiliateAddress[promo]).transfer(cost * affiliateCommission[promo] / 100);
        return tokenId;
    }

    function mint_affiliate_NFT(string memory str, string memory promo) public payable returns (uint256) {
