...
import "contracts/IndexAdmin/IIndex.sol";
...
contract FactoryAdmin is AccessControl, IFactory {
    ...
    function mint(uint256 startPrice, address[] memory newAssets)
        external
        onlyRole(DAO_ADMIN_ROLE)
    {
        address instance = Clones.clone(_indexMaster);
        IIndex(instance).initialize(
            _DAOAddress,
            _validator,
            _USD,
            _adapter,
...
interface IFactory {
    ...
    //     external
    //     view
    //     returns (address indexMaster, address DAOAddress);
}
