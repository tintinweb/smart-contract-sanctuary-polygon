    ...
    ConfirmedOwner
{
    ...
    mapping(address => address[]) private userToNFTs;
    ...
    event ETHWithdrawn(uint amount, address owner);
    ...
    function bytesToUint(bytes32 data) public pure returns (uint256) {
        ...
        assembly {
            returnValue := mload(add(myBytesValue, 32))
        }
        return returnValue;
    }

    function depositNFTAsCollateral(address nftAddress, uint tokenId) external {
        uint priceOfNFT;
        // todo
        if (latestResponse.length != 0) {
            priceOfNFT = bytesToUint(bytes32(latestResponse));
        }

        IERC721 nft = IERC721(nftAddress);
        nft.safeTransferFrom(msg.sender, address(this), tokenId);
        userToNFTs[msg.sender].push(nftAddress);
        collateral[msg.sender] += priceOfNFT;
    ...
    // perform upkeep should check if the ETH is in the contract
    function performUpkeep(bytes calldata /* performData */) external override {
        // perform your task here
        require(
            block.timestamp - lastUpkeepTimeStamp > updateInterval,
            "Upkeep isn't needed"
        );
        upKeepFinished = true;
        if (address(this).balance - minimumAmount > 0) {
            // allow withdrawal
            withdrawAllowed = true;
        }
    }

    // ownr should be able to withdraw if time is up, else only user should
    function withdrawNFT(uint tokenId, address nftAddress) external {
        IERC721 nft = IERC721(nftAddress);
        bool userIsInitialOwner = false;
        uint indexOfNFT = 0;
        for (uint i = 0; i < userToNFTs[msg.sender].length; i++) {
            if (userToNFTs[msg.sender][i] == nftAddress) {
                userIsInitialOwner = true;
                indexOfNFT = i;
            }
        }
        if (msg.sender == s_owner) {
            require(
                withdrawAllowed == false && upKeepFinished,
                "Withraw is only allowed for owner if time is up"
            );
        } else {
            require(withdrawAllowed, "You cannot withdraw yet");
            require(
                userIsInitialOwner,
                "You are not the initial owner of this nft"
            );
        }

        nft.safeTransferFrom(address(this), msg.sender, tokenId);
        // remove nft from array
        for (uint i = indexOfNFT; i < userToNFTs[msg.sender].length - 1; i++) {
            userToNFTs[msg.sender][i] = userToNFTs[msg.sender][i + 1];
        }

        // Remove the last element
        userToNFTs[msg.sender].pop();
    ...
    function withdrawETH() external virtual onlyOwner {
        ...
        emit ETHWithdrawn(address(this).balance, msg.sender);
    }
...
// 2. figure out how to return that to my contract
// 3. deploy keepers on the app and figure out how to function backend
