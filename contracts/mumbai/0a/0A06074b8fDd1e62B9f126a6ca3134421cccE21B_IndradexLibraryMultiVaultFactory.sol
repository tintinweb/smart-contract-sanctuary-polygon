...
abstract contract IndradexBaseVault is Ownable, ERC20 {
  ...
  */
  function requestWithdrawal(uint256 amount) external{
    ...
    emit WithdrawalRequested(sender, pendingUsersWithdrawal[sender].length - 1);
  }
...
library IndradexLibraryMultiVaultFactory{
    ...
    */
    function createVault(address newOwner, string memory name_, string memory symbol_, address payable inputToken_,
                address[] memory outputTokens_, address[] memory aggregators_, uint256 maxCapDeposit) external returns(IndradexMultiExternalVault){
        return new IndradexMultiExternalVault(newOwner, name_, symbol_, inputToken_, outputTokens_, aggregators_, maxCapDeposit);
    }
}
...
contract IndradexMultiExternalVault is IndradexBaseVault {
  ...
  /**
    ...
    */
  function setTokenExternalBalance(address token, uint256 newExternalBalance) public onlyOwner onlyPresentToken(token) {
    require(tokenInternalBalance(token) + newExternalBalance >= tokenPendingBalance(token), "Vault: new external balance is not sufficient");
