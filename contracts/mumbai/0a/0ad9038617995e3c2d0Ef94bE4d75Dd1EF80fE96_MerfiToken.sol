    ...
    GovernorTimelockControl
{
    constructor(
        ...
        Governor(_name)
        GovernorSettings(
            6575, /* voting delay: 1 day specified in blocks */
            6575, /* voting period: 1 day specified in blocks */
            0 /* Proposal Threshold */
        )
        GovernorVotes(_token)
        GovernorVotesQuorumFraction(4) // quorum as a percentage of the total supply
...
contract MerfiTreasury is Ownable {
...
import "./MerfiToken.sol";
import "./Authorizable.sol";

// MasterMerfi is the master investor of whatever investments are available.
contract MasterMerfi is Initializable, Authorizable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    using SafeERC20Upgradeable for MerfiToken;
    ...
    struct ConstructorParams {
        ...
        address comfundaddr;
        address founderaddr;
        uint256[] blockDeltaStartStage;
        uint256[] blockDeltaEndStage;
        uint256[] rewardMultiplier;
        uint256[] userFeeStage;
        uint256[] devFeeStage;
    }

    // The MERFI token
    MerfiToken public govToken;
    // Dev address.
    ...
    // MERFI created per block.
    uint256 public REWARD_PER_BLOCK;
    // Bonus multiplier for early MERFI makers.
    ...
    // The block number when MERFI mining starts.
   ...
   // Info of each pool.
    ...
    function initialize(ConstructorParams memory params) public initializer {
        ...
        liquidityaddr = params.liquidityaddr;
        comfundaddr = params.comfundaddr;
        founderaddr = params.founderaddr;
        REWARD_PER_BLOCK = params.rewardPerBlock;
        START_BLOCK = params.startBlock;
        userDepFee = params.userDepFee;
        devDepFee = params.devDepFee;
        REWARD_MULTIPLIER = params.rewardMultiplier;
        blockDeltaStartStage = params.blockDeltaStartStage;
        blockDeltaEndStage = params.blockDeltaEndStage;
        userFeeStage = params.userFeeStage;
        devFeeStage = params.devFeeStage;

        uint256 _halvingAfterBlock = params.halvingAfterBlock;

        for (uint256 i = 0; i < REWARD_MULTIPLIER.length - 1;) {
            uint256 halvingAtBlock = _halvingAfterBlock.mul(i+1).add(params.startBlock).add(1);
            HALVING_AT_BLOCK.push(halvingAtBlock);
            unchecked { i++; }
        }
        FINISH_BONUS_AT_BLOCK = _halvingAfterBlock
            .mul(REWARD_MULTIPLIER.length - 1)
            .add(params.startBlock);
    ...
    function add(uint256 _allocPoint, IERC20Upgradeable _lpToken, bool _withUpdate) public onlyOwner nonDuplicated(_lpToken) {
        require(
    ...
    // Update the given pool's MERFI allocation points. Can only be called by the owner.
    ...
    function updatePool(uint256 _pid) public {
        ...
        // Mint some new MERFI tokens for the farmer and store them in MasterMerfi.
        ...
        if (GovTokenForLP > 0) {
            ...
            //LP + Partnership fund has only xx% locked over time as most of it is needed early on for incentives and listings.
    ...
    // View function to see pending MERFI on frontend.
    ...
    function _harvest(uint256 _pid) internal {
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                ...
                // those tokens from MasterMerfi to their wallet.
                govToken.safeTransfer(msg.sender, pending);
    ...
    // Deposit LP tokens to MasterMerfi for MERFI allocation.
    ...
    // Withdraw LP tokens from MasterMerfi.
    function withdraw(uint256 _pid, uint256 _amount, address _ref) public nonReentrant {
        ...
        updatePool(_pid);
        _harvest(_pid);
        _withdrawWithFees(_amount, _pid, poolInfo[_pid], user);
    }

    function _withdrawWithFees(uint256 _amount, uint256 _pid, PoolInfo storage pool, UserInfo storage user) internal nonReentrant { 
        if (_amount > 0) {
...
contract MerfiRewardPool is LPTokenWrapper, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
...
import "lib/openzeppelin-contracts-upgradeable/contracts/token/ERC20/extensions/ERC20SnapshotUpgradeable.sol";
import "lib/openzeppelin-contracts-upgradeable/contracts/token/ERC20/extensions/ERC20CappedUpgradeable.sol";
import "lib/openzeppelin-contracts-upgradeable/contracts/token/ERC20/extensions/ERC20VotesUpgradeable.sol";
import "lib/openzeppelin-contracts-upgradeable/contracts/proxy/utils/UUPSUpgradeable.sol";
import "lib/openzeppelin-contracts-upgradeable/contracts/proxy/utils/Initializable.sol";
import "./Authorizable.sol";

contract MerfiToken is
    Initializable,
    ERC20Upgradeable,
    ...
    Authorizable
{
    ...
    /**
     * @dev Balances and total supply at the time that `snapshot` is called are recorded for later access.
     */
    ...
    /**
     * @dev Modifier to lock token transfers between non-zero addresses.
     * @param to verify if is the ZERO ADDRESS to make `burn` and `burnFrom` functions available.
     */
    modifier isLocked(address to) {
        require(
            !locked || authorized[msg.sender] || to == address(0),
            "transfer is locked"
        );
        _;
    }
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyAuthorized {
        require(_maxTransferAmountRate <= 10000, "updateMaxTransferAmountRate too high.");
        require(_maxTransferAmountRate >= 50, "updateMaxTransferAmountRate < 0.005.");
    ...
    function _beforeTokenTransfer(
        ...
        address to,
        uint256 amount
    ) internal override(ERC20Upgradeable, ERC20SnapshotUpgradeable) isLocked(to) {
        super._beforeTokenTransfer(from, to, amount);
    }
...
import "../../utils/libraries/Babylonian.sol";
import "../../utils/libraries/LowGasSafeMath.sol";
...
interface IMerfiVault is IERC20 {
...
contract MerfiZap {
    ...
    using SafeERC20 for IERC20;
    using SafeERC20 for IMerfiVault;
    ...
    function merfiInETH (address merfiVault, uint256 tokenAmountOutMin) external payable {
        require(msg.value >= minimumAmount, "Insignificant input amount");

        IWETH(WETH).deposit{value: msg.value}();

        _swapAndStake(merfiVault, tokenAmountOutMin, WETH);
    }

    function merfiIn (address merfiVault, uint256 tokenAmountOutMin, address tokenIn, uint256 tokenInAmount) external {
        ...
        _swapAndStake(merfiVault, tokenAmountOutMin, tokenIn);
    }

    function merfiOut (address merfiVault, uint256 withdrawAmount) external {
        (IMerfiVault vault, IUniswapV2Pair pair) = _getVaultPair(merfiVault);

        IERC20(merfiVault).safeTransferFrom(msg.sender, address(this), withdrawAmount);
    ...
    function merfiOutAndSwap(address merfiVault, uint256 withdrawAmount, address desiredToken, uint256 desiredTokenOutMin) external {
        (IMerfiVault vault, IUniswapV2Pair pair) = _getVaultPair(merfiVault);
        address token0 = pair.token0();
        address token1 = pair.token1();
        require(token0 == desiredToken || token1 == desiredToken, "Desired token not present in liqudity pair");
    ...
    function _getVaultPair(address merfiVault)
        private
        view
        returns (IMerfiVault vault, IUniswapV2Pair pair)
    {
        vault = IMerfiVault(merfiVault);
    ...
    function _swapAndStake(
       ...
       (IMerfiVault vault, IUniswapV2Pair pair) = _getVaultPair(merfiVault);
        ...
        require(isInputA || pair.token1() == tokenIn, "Input token not present in liqudity pair");
    ...
    {
        checkWETH();
        (, IUniswapV2Pair pair) = _getVaultPair(merfiVault);
...
interface IStrategy { 
    ...
    /// wanted/returned token
    function want() external view returns (IERC20Upgradeable);

    /// called before deposit
    ...
    /// balance of deposited tokens on the strategy
    function balanceOf() external view returns (uint256);

    /// balance of wanted tokens on the strategy
    function balanceOfWant() external view returns (uint256);

    /// balance of the pool
    function balanceOfPool() external view returns (uint256);

    /// harvest and distribute the fees/rewards among the participants
    function harvest() external;

    /// disable strategy
    function retireStrat() external;

    /// panic mode, use it when something critical happens and you need to protect the funds
    ...
    /// check whether the strategy is paused
    function paused() external view returns (bool);

    /// unirouter utilized for swapping tokens for this strategy
...
abstract contract OpsReady is Initializable {
    ...
    function _transfer(uint256 _fee, address _feeToken) internal {
        if (_feeToken == ETH) {
            ...
            SafeERC20.safeTransfer(IERC20(_feeToken), gelato, _fee);
        }
    }
...
import "lib/openzeppelin-contracts-upgradeable/contracts/token/ERC20/utils/SafeERC20Upgradeable.sol";
import "lib/openzeppelin-contracts-upgradeable/contracts/access/OwnableUpgradeable.sol";
import "../interfaces/IMerfiVault.sol";
import "../interfaces/IMerfiStrategy.sol";
import "../interfaces/IMerfiHarvester.sol";
import "./gelato/OpsReady.sol";

abstract contract MerfiHarvester is Initializable, OwnableUpgradeable, IMerfiHarvester, OpsReady {
    using SafeERC20 for IERC20;

    uint256 public delay = 10 minutes;
    
    function initialize(address _ops) public initializer { 
        __Ownable_init();
        ops = _ops;
        gelato = IOps(_ops).gelato();
    }

    function harvestVault(address _vault) external {
        IMerfiStrategy strategy = IMerfiStrategy(IMerfiVault(_vault).strategy());
        uint256 _rewardsAmount = strategy.rewardsAvailable();

        try strategy.harvest(address(this)) {
           emit HarvestSucceeded(_vault, _rewardsAmount);
        } catch {
           emit HarvestFailed(_vault, _rewardsAmount);
        }

        _payTxFee();
    }

    function willHarvestVault(address _vaultAddress)
        external
        view
        returns (
            bool _willHarvestVault
        )
    {
        bool shouldHarvestVault = _shouldHarvestVault(_vaultAddress);
        bool canHarvestVault = _canHarvestVault(_vaultAddress);

        _willHarvestVault = canHarvestVault && shouldHarvestVault;

        return _willHarvestVault;
    }

    function setDelay(uint256 _delay) external onlyOwner { 
        delay = _delay;
        emit DelayForHarvestUpdated(_delay);
    }

     function _canHarvestVault(address vaultAddress_) internal view returns (bool canHarvest_) {
        IMerfiVault vault = IMerfiVault(vaultAddress_);
        IMerfiStrategy strategy = IMerfiStrategy(vault.strategy());
        ...
        )
    {
        ...
        uint256 someTimeAgo = block.timestamp - delay;
        bool shouldHaverstAgain = strategy.lastHarvest() > someTimeAgo;
        /* solhint-enable not-rely-on-time */

        uint256 callRewardAmount_ = strategy.callReward();
        (uint256 feeAmount, ) = IOps(ops).getFeeDetails();

        bool isProfitableHarvest = shouldHaverstAgain && (callRewardAmount_ >= feeAmount);

        return (isProfitableHarvest);
    }

    function _payTxFee() internal { 
...
pragma solidity ^0.8.3;
import "lib/openzeppelin-contracts/contracts/access/Ownable.sol";
import "../interfaces/IMerfiVaultRegistry.sol";
import "../interfaces/IMerfiResolver.sol";

import "../interfaces/IMerfiVault.sol";
import "../interfaces/IMerfiHarvester.sol";

abstract contract MerfiResolver is IMerfiResolver, Ownable {
    // Contract that holds the vaults to be harvested.
    IMerfiVaultRegistry public vaultRegistry;

    // Contract that executes the harvesting
    IMerfiHarvester public harvester;

    uint256 public maxGasPriceInGwei = 60 gwei;

    constructor(IMerfiVaultRegistry _vaultRegistry,
                IMerfiHarvester _harvester){ 
        vaultRegistry = _vaultRegistry;
        harvester = _harvester;
    }

    function setMaxGasPrice(uint256 _maxGasPriceInGwei) public onlyOwner { 
        maxGasPriceInGwei = _maxGasPriceInGwei;
        emit GasPriceUpdated(_maxGasPriceInGwei);
    }

    function checker() external view returns (bool canExec, bytes memory execPayload) { 
        // get vaults to iterate over
        address[] memory vaults = vaultRegistry.allVaultAddresses();
        uint256 _maxGasPrice = maxGasPriceInGwei;

        for(uint256 i = 0; i < vaults.length;) { 
            address _vaultAddress = vaults[i];
            unchecked { i++; }

            canExec = harvester.willHarvestVault(_vaultAddress);

            execPayload = abi.encodeWithSelector(IMerfiHarvester.harvestVault.selector, _vaultAddress);

            // additional check to avoid paying a high price of gas
            // additional because `willHarvestVault` check the fee details, but not the gas.
            if(tx.gasprice > _maxGasPrice) return (false, bytes("Gas price too high"));
...
contract MerfiVaultRegistry is OwnableUpgradeable, IMerfiVaultRegistry {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    ...
    function addVaults(address[] memory _vaultAddresses) external onlyOwner {
        for (uint256 i; i < _vaultAddresses.length;) {
    ...
    function _addVault(address _vaultAddress) internal {
        ...
        IMerfiVault vault = IMerfiVault(_vaultAddress);
        IMerfiStrategy strat = _validateVault(vault);

        address[] memory tokens = _collectTokenData(strat);

        _vaultSet.add(_vaultAddress);

        for (uint8 tokenId = 0; tokenId < tokens.length;) {
    ...
    function _validateVault(IMerfiVault _vault) internal view returns (IMerfiStrategy strategy) {
        address vaultAddress = address(_vault);

        try _vault.strategy() returns (IMerfiStrategy _strategy) {
            require(IMerfiStrategy(_strategy).vault() == vaultAddress, "Vault/Strat Mismatch");
            return IMerfiStrategy(_strategy);
        } catch {
            require(false, "Address not a Vault");
        }
    }

    function _collectTokenData(IMerfiStrategy _strategy) internal view returns (address[] memory tokens) {
        ...
        )
    {
        ...
        name_ = vault.name();
        strategy_ = IMerfiStrategy(vault.strategy());
    ...
    function getVaultsForToken(address _token) external view returns (VaultInfo[] memory vaultResults) {
        ...
        for (uint256 i; i < _tokenToVaultsMap[_token].length();) {
    ...
    function getStakedVaultsForAddress(address _address) external view returns (VaultInfo[] memory stakedVaults) {
        unchecked {
            ...
            for (uint256 i; i < _vaultSet.length(); i++) {
                if (IMerfiVault(_vaultSet.at(i)).balanceOf(_address) > 0) {
                    numResults++;
                }
            }
            ...
            for (uint256 i; i < _vaultSet.length(); i++) {
                if (IMerfiVault(_vaultSet.at(i)).balanceOf(_address) > 0) {
                    stakedVaults[curResults++] = _vaultInfoMap[_vaultSet.at(i)];
                }
            }  
        }
    }
    ...
    function setVaultTokens(address _vault, address[] memory _tokens) external onlyOwner {
        ...
        for (uint256 tokenIndex; tokenIndex < currentTokens.length;) {
        ...
        for (uint256 tokenIndex; tokenIndex < _tokens.length;) {
    ...
    function setRetireStatuses(address[] memory _vaultAddresses, bool _status) external onlyOwner {
        for (uint256 vaultIndex = 0; vaultIndex < _vaultAddresses.length;) {
            _setRetireStatus(_vaultAddresses[vaultIndex], _status);
            unchecked { vaultIndex++; }
        }
...
// Interface that represents the resolver contract which handles the logic to trigger the automation.
interface IMerfiHarvester {
    // data
    struct HarvestInfo {
        bool willHarvest;
    }
    ...
    // events
    event HarvestSucceeded(address indexed vault, uint256 amount);
    event HarvestFailed(address indexed vault, uint256 amount);
    event DelayForHarvestUpdated(uint256 indexed blockNumber);

    // functions
    function harvestVault(address _vault) external;
    function willHarvestVault(address _vaultAddress) external view returns (bool _willHarvestVault);
}
...
// Interface that represents the resolver contract which handles the logic to trigger the automation.
interface IMerfiResolver {
    event GasPriceUpdated(uint256 indexed newGasPrice);
    function checker()
        external
        view
...
import "lib/openzeppelin-contracts-upgradeable/contracts/token/ERC20/IERC20Upgradeable.sol";
import "./IMerfiStrategy.sol";

interface IMerfiVault is IERC20Upgradeable {
    function name() external view returns (string memory);
    ...
    function strategy() external view returns (IMerfiStrategy);
}

// SPDX-License-Identifier: MIT
pragma solidity ^0.8.3;

// Interface to retrieve the added vault contracts from the MerfiRegistry contract
interface IMerfiVaultRegistry {
...
contract StrategyCakeLP is StratFeeManager {
    using SafeERC20 for IERC20;

    // Tokens used
    address public native;
    address public output;
    address public want; // LP TOKEN, ex: LP CAKE-BNB
    address public lpToken0;
    address public lpToken1;

    // Mergefi Contract
    address public boostStaker; // VeCakeStaker.sol, contract that deposits CAKE-BNB-LP into the MasterChefV2 of https://pancakeswap.finance/pools
    ...
    event ChargedFees(uint256 callFees, uint256 fees, uint256 strategistFees);
    event SetBoostStaker(address boostStaker);
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            IVeCakeStaker(boostStaker).withdraw(chef, poolId, _amount - wantBal);
        ...
        if (tx.origin != owner() && !paused()) {
            uint256 withdrawalFeeAmount = wantBal * withdrawalFee / WITHDRAWAL_MAX; // hold some value by applying withdraw fee amount 
    ...
    function chargeFees(address callFeeRecipient) internal {
        // transfer 4.5% of fees to this contract
        uint256 toNative = IERC20(output).balanceOf(address(this)) * 45 / DIVISOR; // retain 4.5%

        // Here we convert the Pool reward token to the native token
        // In this case: CAKE to BNB... is always SAFUr to retain the fees as native tokens.
        ...
        // reward the caller of this contract with a small fee
        IFeeConfig.FeeCategory memory fees = getFees();
        uint256 callFeeAmount = nativeBal * fees.call / DIVISOR;
        IERC20(native).safeTransfer(callFeeRecipient, callFeeAmount); // send this to the caller

        // reward the platform/treasury with a small fee
        uint256 platformFeeAmount = nativeBal * fees.merfi / DIVISOR;
        IERC20(native).safeTransfer(feeRecipient, platformFeeAmount); // send this fee to the 'Platform' or 'Treasury' management contract

        // reward the creator of this strategy with a small fee
        uint256 strategistFee = nativeBal * fees.strategist / DIVISOR; // send this to the 'Strategist' wallet.
        IERC20(native).safeTransfer(strategist, strategistFee);

        emit ChargedFees(callFeeAmount, platformFeeAmount, strategistFee);
    }
    ...
    function callReward() public view returns (uint256) {
        ...
        return _nativeBal * 45 / DIVISOR * fees.call / DIVISOR;
    }
    ...
    function setBoostStaker(address _boostStaker) external whenPaused onlyOwner {
        boostStaker = _boostStaker;
        emit SetBoostStaker(_boostStaker);
    }
...
contract VeCakeStaker is IVeCakeStaker, Pausable, ReentrancyGuard, Ownable {
    using SafeERC20 for IERC20;

    // Addresses used
    IERC20 public want;
    ICakePool public veCake; // contract that is staking CAKE with flexible/locked period: https://pancakeswap.finance/pools
    ...
    function deposit(address _cakeChef, uint256 _pid, uint256 _amount) external onlyWhitelist(_cakeChef, _pid) {
        // lp token
        address _underlying = ICakeChefV2(_cakeChef).lpToken(_pid);
    ...
    function withdraw(address _cakeChef, uint256 _pid, uint256 _amount) external onlyWhitelist(_cakeChef, _pid) {
        ...
        ICakeChefV2(_cakeChef).withdraw(_pid, _amount);
        uint256 cakeDiff = balanceOfWant() - cakeBefore; // Amount of Cakes the Chef sent us
    ...
    function emergencyWithdraw(address _cakeChef, uint256 _pid) external onlyWhitelist(_cakeChef, _pid) {
        ...
        ICakeChefV2(_cakeChef).emergencyWithdraw(_pid);
        uint256 _balance = IERC20(_underlying).balanceOf(address(this)) - _before;
        IERC20(_underlying).safeTransfer(msg.sender, _balance);
    }

    /**
     * @dev Whitelists a strategy address to interact with the Chef and gives approvals.
    ...
    function whitelistStrategy(address _strategy) external onlyManager {
        ...
        (uint256 stratBal,,) = ICakeChefV2(_cakeChef).userInfo(_pid, address(this));
...
contract DelegateManager is Ownable, Pausable {
     ...
     */
    IDelegateManager public delegateManager = IDelegateManager(0xBdaF53e588E26cF17522ee09AaeBf01ac235310e);
...
import "../../utils/gas/GasThrottler.sol";

contract StratFeeManager is Ownable, Pausable, GasThrottler {
...
contract StrategyMiniChefLP is StratFeeManager {
    using SafeERC20 for IERC20;

    struct TokensUsed { 
        // Tokens used
        address native; // native coin chain, ex: matic, bnb.
        address output; // platform output reward, ex: cake, ape, quick, etc.
        address reward; // some pools might have a second token as a reward
        address want; // lp
        address lpToken0; // token 0
        address lpToken1; // token 1
    }

    struct Routes { 
        address[] outputToNativeRoute;
        address[] rewardToOutputRoute;
    ...
    constructor(TokensUsed memory _tokens,
                Farm memory _farm, 
        ...
        require(_routes.outputToNativeRoute.length == 2, "invalid route");

        // output -> first coin of the pair
        require(_routes.outputToLp0Route[0] == _tokens.output, "invalid output");
        require(_routes.outputToLp0Route[_routes.outputToLp0Route.length - 1] == _tokens.lpToken0, "invalid route");

        // output -> second coin of the pair
        require(_routes.outputToLp1Route[0] == _tokens.output, "invalid output");
        require(_routes.outputToLp1Route[_routes.outputToLp0Route.length - 1] == _tokens.lpToken1, "invalid route");

        // reward -> output
        require(_routes.rewardToOutputRoute[_routes.rewardToOutputRoute.length - 1] == _tokens.output, "_rewardToOutputRoute != output");
    }
    
     // puts the funds to work
    function deposit() public whenNotPaused {
        // save gas
        address _want = tokens.want;
        Farm memory _farm = farm;

        uint256 wantBal = IERC20(_want).balanceOf(address(this));
        if (wantBal > 0) {
            IMiniChefV2(_farm.chef).deposit(_farm.poolId, wantBal, address(this));
        }
    }
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            IMiniChefV2(farm.chef).withdraw(farm.poolId, _amount - wantBal, address(this));
            wantBal = IERC20(_want).balanceOf(address(this));
        }
        ...
        IERC20(_want).safeTransfer(vault, wantBal);
    ...
    // compounds earnings and charges performance fee
    function _harvest(address callFeeRecipient) internal whenNotPaused {
        IMiniChefV2(farm.chef).harvest(farm.poolId, address(this));
        uint256 outputBal = IERC20(tokens.output).balanceOf(address(this));
        uint256 rewardBal = IERC20(tokens.reward).balanceOf(address(this));
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        uint256 toOutput = IERC20(tokens.reward).balanceOf(address(this));
        if (toOutput > 0) {
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(toOutput, 0, routes.rewardToOutputRoute, address(this), block.timestamp);
        }

        // transfer 4.5% of fees to this contract
        uint256 toNative = IERC20(tokens.output).balanceOf(address(this)) * 45 / DIVISOR; // retain 4.5%

        // Here we convert the Pool reward token to the native token
        // In this case: CAKE to BNB... is always SAFUr to retain the fees as native tokens.
        IUniswapRouterETH(unirouter).swapExactTokensForTokens(toNative, 0, routes.outputToNativeRoute, address(this), block.timestamp);

        uint256 nativeBal = IERC20(tokens.native).balanceOf(address(this));
        address _native = tokens.native;

        // reward the caller of this contract with a small fee
        IFeeConfig.FeeCategory memory fees = getFees();
        uint256 callFeeAmount = nativeBal * fees.call / DIVISOR;
        IERC20(_native).safeTransfer(callFeeRecipient, callFeeAmount); // send this to the caller

        // reward the platform/treasury with a small fee
        uint256 platformFeeAmount = nativeBal * fees.merfi / DIVISOR;
        IERC20(_native).safeTransfer(feeRecipient, platformFeeAmount); // send this fee to the 'Platform' or 'Treasury' management contract

        // reward the creator of this strategy with a small fee
        uint256 strategistFee = nativeBal * fees.strategist / DIVISOR; // send this to the 'Strategist' wallet.
        IERC20(_native).safeTransfer(strategist, strategistFee);
    ...
    // Adds liquidity to AMM and gets more LP tokens.
    function addLiquidity() internal {
        uint256 outputHalf = IERC20(tokens.output).balanceOf(address(this)) / 2;

        if (tokens.lpToken0 != tokens.output) {
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(outputHalf, 0, routes.outputToLp0Route, address(this), block.timestamp);
        }

        if (tokens.lpToken1 != tokens.output) {
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(outputHalf, 0, routes.outputToLp1Route, address(this), block.timestamp);
        }

        uint256 lp0Bal = IERC20(tokens.lpToken0).balanceOf(address(this));
        uint256 lp1Bal = IERC20(tokens.lpToken1).balanceOf(address(this));
        IUniswapRouterETH(unirouter).addLiquidity(tokens.lpToken0, tokens.lpToken1, lp0Bal, lp1Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    // it calculates how much 'want' this contract holds.
    function balanceOfWant() public view returns (uint256) {
        return IERC20(tokens.want).balanceOf(address(this));
    }

    // it calculates how much 'want' the strategy has working in the farm.
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount, ) = IMiniChefV2(farm.chef).userInfo(farm.poolId, address(this));
        return _amount;
    }
    ...
    function retireStrat() external {
        ...
        IMiniChefV2(farm.chef).emergencyWithdraw(farm.poolId, address(this));

        uint256 wantBal = IERC20(tokens.want).balanceOf(address(this));
        IERC20(tokens.want).transfer(vault, wantBal);
    }

    // returns rewards unharvested
    function rewardsAvailable() public view returns (uint256) {
        return IMiniChefV2(farm.chef).pendingBanana(farm.poolId, address(this));
    }
    ...
    function callReward() public view returns (uint256) {
        ...
        uint256 nativeOut;
         if (outputBal > 0) {
            try IUniswapRouterETH(unirouter).getAmountsOut(outputBal, routes.outputToNativeRoute)
        ...
        address rewarder = IMiniChefV2(farm.chef).rewarder(farm.poolId);
        if (rewarder != nullAddress) {
            pendingNative = IRewarder(rewarder).pendingToken(farm.poolId, address(this));
        } 
        
        return (pendingNative + nativeOut) / DIVISOR * fees.call / DIVISOR;
    }
    ...
    function _giveAllowances() internal {
        IERC20(tokens.want).safeApprove(farm.chef, type(uint256).max);
        IERC20(tokens.output).safeApprove(unirouter, type(uint256).max);
        IERC20(tokens.reward).safeApprove(unirouter, type(uint256).max);

        IERC20(tokens.lpToken0).safeApprove(unirouter, 0);
        IERC20(tokens.lpToken0).safeApprove(unirouter, type(uint256).max);

        IERC20(tokens.lpToken1).safeApprove(unirouter, 0);
        IERC20(tokens.lpToken1).safeApprove(unirouter, type(uint256).max);
    }

    function _removeAllowances() internal {
        IERC20(tokens.want).safeApprove(farm.chef, 0);
        IERC20(tokens.output).safeApprove(unirouter, 0);
        IERC20(tokens.reward).safeApprove(unirouter, 0);
        IERC20(tokens.lpToken0).safeApprove(unirouter, 0);
        IERC20(tokens.lpToken1).safeApprove(unirouter, 0);
    }

    function outputToNative() external view returns (address[] memory) {
        return routes.outputToNativeRoute;
    }

    function rewardToOutput() external view returns (address[] memory) {
        return routes.rewardToOutputRoute;
    }

    function outputToLp0() external view returns (address[] memory) {
        return routes.outputToLp0Route;
    }

    function outputToLp1() external view returns (address[] memory) {
        return routes.outputToLp1Route;
    }
...
contract GasPrice is Ownable {
    ...
    uint public maxGasPrice = 10 gwei; // 10 gwei
...
contract GasThrottler is Ownable {
 ...
 */
contract BaseVault is ERC20Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    ...
    function getPricePerFullShare() public view returns (uint256) {
        return totalSupply() == 0 ? 1e18 : _balance() * 1e18 / totalSupply();
    }
     ...
     */
    function depositAll() external {
        _baseDeposit(want().balanceOf(msg.sender));
    }
     ...
     */
    function _baseDeposit(uint _amount) internal nonReentrant {
        strategy.beforeDeposit();

        uint256 _pool = _balance();
        want().safeTransferFrom(msg.sender, address(this), _amount);
        _earn();
        uint256 _after = _balance();
     ...
     */
    function withdrawAll() external {
        _baseWithdraw(balanceOf(msg.sender));
    }
     ...
     */
    function _baseWithdraw(uint256 _shares) internal {
        uint256 r = (_balance() * _shares) / totalSupply();
...
contract MergeVault is BaseVault, ERC4626Upgradeable {
   using SafeERC20Upgradeable for IERC20Upgradeable;
   using MathUpgradeable for uint256;
     ...
     */
     function initialize(
        ...
        __Ownable_init();
        __ReentrancyGuard_init();
        __ERC4626_init(_strategy.want()); // asset to be deposited
        strategy = _strategy;
        approvalDelay = _approvalDelay;

        IERC20Upgradeable(asset()).safeApprove(address(this), type(uint256).max);
    }
    ...
    /**
     * @dev Deposit assets to the vault and mint an equal number of vault shares.
     * @param caller the address of the sender of the assets.
     * @param receiver the address of the receiver of vault shares.
    ...
    function _deposit(
        ...
        _baseDeposit(assets); // mint the shares and move the assets into the strategy

        emit Deposit(caller, receiver, assets, shares);
    }

    /**
     * @dev Burn the shares and withdraw assets from the vault.
...
import "./MergeVault.sol";
import "lib/openzeppelin-contracts-upgradeable/contracts/proxy/ClonesUpgradeable.sol";

// Merge Vault Proxy Factory
// Minimal proxy pattern for creating new Merge vaults
contract MergeVaultFactory {
  using ClonesUpgradeable for address;

  // Contract template for deploying proxied Merge vaults
  MergeVault public instance;

  event ProxyCreated(address proxy);

  // Initializes the Factory with an instance of the Merge Vault
  constructor(address _instance) {
    if (_instance == address(0)) {
      instance = new MergeVault();
    } else {
      instance = MergeVault(_instance);
    }
  }

  // Creates a new Merge Vault as a proxy of the template instance
  // A reference to the new proxied Merge Vault
  function cloneVault() external returns (MergeVault) {
    return MergeVault(cloneContract(address(instance)));
  }
