...
contract RewardPool is Ownable {
    ...
    constructor(address _tokenStaking, address _SBCToken) {
     ...
     */
    function approveSBC(uint256 _amount) external returns (bool) {
        require(tokenStaking != address(0x00));
        SBCToken.approve(tokenStaking, _amount);
        return true;
    }

    // Sets SBC token contract address.
    ...
    // Withdraws SBC tokens to _address.
    function withdrawSBC(address _address, uint256 _amount) external onlyOwner {
        require(_address != address(0x00));
        require(_amount > 0, "Withdraw: No balance to withdraw");
        SBCToken.transfer(_address, _amount);
    }
}
