...
// esNLP Token.
contract esNLPToken is ERC20("esNLPToken", "esNLP"), Ownable {
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        uint256 pending; // Actual pending reward after withdraw of tokens which is available for harvest.
    ...
    function updatePool(uint256 _pid) public {
        ...
        esNLP.mint(devaddr, esNLPReward.div(10));
        esNLP.mint(address(this), esNLPReward);
    ...
    /// @param to Receiver of esNLP rewards.
    function harvest(uint256 _pid, address to) public {
        UserInfo storage user = userInfo[_pid][msg.sender];
        uint256 _pendingesNLP = user.pending;

        // Effects
        user.pending = 0;
        if(_pendingesNLP > 0) {
           safeesNLPTransfer(to, _pendingesNLP);
        }

        emit Harvest(msg.sender, _pid, _pendingesNLP);
    }
    ...
    function safeesNLPTransfer(address _to, uint256 _amount) internal {
        uint256 esNLPBal = esNLP.balanceOf(address(this));
        if (_amount > esNLPBal) {
