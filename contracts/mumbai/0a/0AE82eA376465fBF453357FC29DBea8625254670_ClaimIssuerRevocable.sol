...
contract ClaimIssuerRevocable is ClaimIssuer, IClaimRevoker {
    ...
    function revokeClaim(
        ...
        bytes memory sig;
        (, , , sig, , ) = Identity(identity).getClaim(claimId);
    ...
    function revokeClaimBySignatureWithoutEvent(
        bytes memory sig
    ) external mOnlyManagementKey {
        require(!_revokedClaims[sig], "Claim already revoked");
    ...
    function unrevokeClaimBySignature(
        bytes memory sig
    ) external mOnlyManagementKey {
        require(_revokedClaims[sig], "Claim is not revoked");

        _revokedClaims[sig] = false;
        emit ClaimUnrevoked(sig);
    }

    function revokeClaimsBySignatureBatch(
        bytes calldata sigs,
        uint256 sigsCount
    ) external mOnlyManagementKey {
    ...
    function revokeClaimsBySignatureBatchWithoutEvent(
        bytes calldata sigs,
        uint256 sigsCount
    ) external mOnlyManagementKey {
        for (uint256 i = 0; i < sigsCount; i++) {
    ...
    function unrevokeClaimsBySignatureBatch(
        bytes calldata sigs,
        uint256 sigsCount
    ) external mOnlyManagementKey {
        for (uint256 i = 0; i < sigsCount; i++) {
...
interface IClaimRevoker {
    event ClaimUnrevoked(bytes sig);
    ...
    // function revokeClaimBySignature(bytes memory sig) external;

    // function unrevokeClaimBySignature(bytes memory sig) external;
