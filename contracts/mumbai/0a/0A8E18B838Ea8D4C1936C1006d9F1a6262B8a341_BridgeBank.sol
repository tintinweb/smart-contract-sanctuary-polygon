...
contract BridgeBank is Initializable, ReentrancyGuardUpgradeable {
    ...
    function lock(
        ...
        if (_tokenSource != address(0) && msg.value == 0) {
            IERC20(_tokenSource).safeTransferFrom(
                msg.sender,
                address(this),
                _amountTokenIn
            );
        ...
        paths[0] = _tokenSource;
        paths[1] = stableToken;

        // swap token -> usdc
        if (_tokenSource != stableToken) {
    ...
    function unlock(
        ...
        // Check if it is ETH
        require(
            IERC20(stableToken).balanceOf(address(this)) >= _amountStableToken - fee,
    ...
    function refund(
        ...
        require(lockCompleted[_nonce].isRefunded == false, "Refunded before");
        // Check if it is ETH
        require(
    ...
    function updateProtocolFee(uint256 protocol_fee_) public {
