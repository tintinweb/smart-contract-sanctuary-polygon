...
contract WantedTokenV2 is ERC20Upgradeable, WantedStorageV1{
    ...
    function withdraw() external returns(bool){
        ...
        (uint256 TotalLockAmount, uint256 TotalClaimedBalance, uint256 TotalUnclaimedBalance) = userTotalAmount(msg.sender);

        require(TotalUnclaimedBalance > 0, 'WantedTokenV1: release Amount is zero');

        for(uint256 i = 0; i < userInvestingIds[msg.sender].length; i++){
            _withdraw(userInvestingIds[msg.sender][i]);
        }

        _transfer(address(this), msg.sender, TotalUnclaimedBalance);
           
        emit WithdrawFromVesting(msg.sender, TotalUnclaimedBalance);
