...
// File contracts/ido/AGAllocation.sol
...
/**
 * @dev Aradena Gold Allocation Contract. Users will get tokens airdroped. Tokens will be tradable
 * but at a certain point a snapshot of owners will be done and transfer of tokens will be disabled.
 * Based on the snapshop users will be whitelisted for their total allocation on seedify IDO
 * platform.
 */
contract AGAllocation is ERC721Enumerable, Ownable, Pausable {
    ...
    onlyAuthorized()
  {
    for (uint8 i = 0; i < _amount; i++) {
      _safeMint(_to, totalSupply() + 1); // Start with ID 1
    }
  }
  ...
  /**
   * @dev Pauses transfer functionality of the NFTs.
   */
  function pause()
    external
    onlyOwner()
  ...
  /**
   * @dev Unpauses transfer functionality of the NFTs.
   */
  function unpause()
    external
    onlyOwner()
