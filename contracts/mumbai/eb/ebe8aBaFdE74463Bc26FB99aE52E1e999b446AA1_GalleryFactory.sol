...
contract Gallery is Ownable, IGallery, Multicall {
	...
	///@param _code random number used as code to verify airfrop
	///@param _tokenId token Id of artswap token to be dropped
	function manageAirDropWithVerification(uint256 _code, uint256 _tokenId) public _onlyAdminOrOwner(msg.sender) {
		listofTokenAirDropped.add(_tokenId);
		AirDropInfo storage airdrop = airDropInfo[_tokenId];
		airdrop.tokenId = _tokenId;
		airdrop.isClaimed = false;
		airdrop.verificationCode = getHash(_code);
	}
	...
	function getHash(uint256 _number) private pure returns (bytes32) {
