...
*/
  ...
  function registerByAdmin(string memory _nomalias , address payable _addr , string memory _ipfs , string memory _mail , string memory _tlf) public onlyAdmins {
    require( genteByAlias[_nomalias].addr == address(0) , 'Alias Duplicado');
    require( genteByAddr[_addr].addr == address(0) , 'Pasaporte ya creado');
    Gente memory oGente = Gente( _addr , _ipfs , _nomalias , _mail  , _tlf );  
