 ...
 */
    ...
    onlyOwner
  {
    for (uint256 i= 0; i < _amountOfNewTokenIDs; i++) {
      ...
      emit TokenAdded(nextTokenId, _collectionId, maxSup);
      nextTokenId++;   
    }
  }
  ...
  function mintBatch(
    ...
    onlyOwner 
    {
      ...
      for (uint256 i=0;i < _ids.length; i++){
        ...
        require(_amounts[i] + totalSupply(i) <= tokenIdToMaxSupply[i], "Supply must be smaller than Max");
      }
 ...
 */
  ...
  @dev token functions
 */
  function viewTokenMaxSupply(uint256 _tokenId) public view
    returns (uint256 _maxSupply)
  {
    return tokenIdToMaxSupply[_tokenId];
  }

  function exists(uint256 id) public view override returns (bool) {
      return tokenIdToMaxSupply[id] > 0;
    }
 ...
 */
  ...
  function addCollection(
    ...
    nextCollectionId++;
    emit CollectionAdded(_name, _tokenPriceInWei, _maxInvocations, true, false);
  }
 ...
 */
  ...
  event CollectionAdded(
    string _name,
    uint256 indexed _tokenPriceInWei,
    uint256 indexed _maxInvocations,
    bool _paused,
    bool locked
  );
