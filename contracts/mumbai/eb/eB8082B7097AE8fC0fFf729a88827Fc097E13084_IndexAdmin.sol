...
contract IndexAdmin is AccessControl, Initializable, IIndex {
    ...
    uint256 private _nextRebalance;
    ...
    function initialize(
        ...
        address[] memory newAssets /* initializer */
    ) public {
        ...
        internal
    {
        ...
        for (uint256 i; i < len; i++) {
            ...
            newAssets[i].totalAmount += amounts[newAssets[i].path.length - 1];
        }
    }
    ...
    function _exchangeToUSD() internal returns (uint256 sum) {
        ...
        for (uint256 i; i < len; i++) {
            ...
            sum += amounts[reversPath.length - 1];
        }
    }
    ...
    function _rebalance(Asset[] memory newAssets) internal {
        delete (_activeAssets);
        delete (_newAssets);

        uint256 len = newAssets.length;
        for (uint256 i; i < len; i++) {
            _activeAssets.push(newAssets[i]);
