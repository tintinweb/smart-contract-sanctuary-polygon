...
// can we create multiple campaigns at the same time?
struct CampaignData {
    uint256 target;
    uint256 softTarget;
...
// how a project becomes ended?
enum ProjectStatus {
    ACTIVE,
    RUNNING_CAMPAIGN,
    WAITING_CAMPAIGN_RESULTS,
    CLOSED
}
...
contract VCProject is Initializable {
    ...
    mapping(IERC20 => uint256) _balanceOutsideCampaigns;
    uint256 _balance;                       // en USD
    ...
    function activateProject() external {
        _onlyStarter();
        if (_projectStatus != ProjectStatus.WAITING_CAMPAIGN_RESULTS) {
            revert Errors.ProjFailedToActivateProject();
        }
    ...
    function closeProject() external {
        ...
        if (_projectStatus != ProjectStatus.ACTIVE) {
            revert Errors.ProjFailedToCloseProject();
        }
        _projectStatus = ProjectStatus.CLOSED;
    }

    function fundProject(uint256 _amount, IERC20 _currency) external {
        _onlyStarter();
        if (_projectStatus != ProjectStatus.ACTIVE) {
            revert Errors.ProjIsNotActive();
        }
        _raisedAmountOutsideCampaigns += _amount;
        _balanceOutsideCampaigns[_currency] += _amount;
        _balance += _amount;
    }
    ...
    function startCampaign(
        ...
        _projectStatus = ProjectStatus.RUNNING_CAMPAIGN;

        uint256 currentId = _numberOfCampaigns;
        _numberOfCampaigns++;

        _campaigns[currentId] = CampaignData(_target, _softTarget, 0, 0, _startTime, _endTime, _backersDeadline);
        return currentId;
    }

    function getFundingAmounts(uint256 _campaignId, uint256 _amount)
        external
        view
        returns (
    ...
    {
        _onlyStarter();
        CampaignStatus newStatus = getCampaignStatus(_campaignId);
        if (newStatus != CampaignStatus.ACTIVE) {
            revert Errors.ProjCampaignNotActive();
        }

        uint256 amountToTarget = _campaigns[_campaignId].target - _campaigns[_campaignId].balance;
    ...
    function fundCampaign(
        ...
        _onlyStarter();
        _backers[_campaignId][_user][_currency] += _amount;
        _updateBalances(_campaignId, _currency, _amount, true);
    }
    ...
    // this function should not receive _currency and instead interate through the supported currencies
    // and transfer all at once to the lab, EMIT an event per currency with the amount transfered
    function labCampaignWithdraw(uint256 _campaignId, IERC20 _currency)
        external
        returns (uint256 withdrawAmount, bool statusSucceeded)
    {
        _onlyStarter();

        if (getCampaignStatus(_campaignId) != CampaignStatus.FUNDED) {
            revert Errors.SttrCampaignNotFunded();
        }

        withdrawAmount = _campaignBalance[_campaignId][_currency];

        if (withdrawAmount == 0) {
            revert Errors.ProjBalanceIsZero();
        }

        _updateBalances(_campaignId, _currency, withdrawAmount, false);
        if (_campaigns[_campaignId].balance == 0) {
            statusSucceeded = true;
            _projectStatus = ProjectStatus.WAITING_CAMPAIGN_RESULTS;
        }
    ...
    function labWithdraw(IERC20 _currency) external returns (uint256 _amount) {
        ...
        _amount = _balanceOutsideCampaigns[_currency];

        if (_balanceOutsideCampaigns[_currency] == 0) {
            revert Errors.ProjBalanceIsZero();
        }
        ...
        _balanceOutsideCampaigns[_currency] = 0;
        _balance -= _amount;
    }
    ...
    function withdrawToPool(IERC20 _currency, address _receiver) external returns (uint256 amountAvailable) {
        ...
        amountAvailable =
            ...
            _balanceOutsideCampaigns[_currency];
    ...
    function transferUnclaimedFunds(
        ...
        IERC20 _currency,
        address _pool
    ) external returns (uint256 _amountToPool) {
        _onlyStarter();

        // podria estar DEFEATED? considerando que seguimos teniendo balance de los usuarios
        // creo que lo correcto seria NOTFUNDED
...
library Errors {
    ...
    error SttrResearcherAlreadyWhitelisted();
    error SttrResearcherAlreadyBlacklisted();
    error SttrFundingAmountIsZero();
    error SttrUnexistentProject();
    ...
    error ProjCampaignTargetTooHigh();
    error ProjIsNotActive();
    error ProjFailedToActivateProject();
    error ProjFailedToCloseProject();
