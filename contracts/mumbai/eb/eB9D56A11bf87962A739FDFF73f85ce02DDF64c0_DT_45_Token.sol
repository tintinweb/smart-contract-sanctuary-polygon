...
contract DT_45_Token is IDT_Token, ERC721Enumerable, IERC2981, AccessControl {
    ...
    uint256      constant       internal    collectionRoyaltyAmount = 100;
    ...
    uint256      constant       public      BATCH_LIMIT = 20;
    ...
    error CouldNotTransferTokens(uint256 amount);
   ...
   function supportsInterface(bytes4 interfaceId) public view virtual override(AccessControl, ERC721Enumerable, IERC165) returns (bool) {
    ...
    function withdrawBatch(uint256[] calldata tokenIds) external {
        ...
        for (uint256 i; i < length; i++) {
            ...
            require(_msgSender() == ownerOf(tokenId), string(abi.encodePacked("INVALID_TOKEN_OWNER ", tokenId)));
            withdrawnTokens[tokenId] = true;
            _burn(tokenId);
    ...
    function split_erc20_payment(address erc20) external {
        ...
        for (uint256 payee = 0; payee < shares.length; payee++) {
            if(!token.transfer(
                ...
                revert CouldNotTransferTokens(amount_to_pay);
        }
    }
