    ...
    IDelegator
{
    ...
    function exchangeRateStored() external view override returns (uint256 rate) {
        bytes memory data = _staticcall(abi.encodeWithSelector(
            MasterState.exchangeRateStored.selector
        ));
    ...
    function liquidateCalculateSeizeTokens(
        ...
        emit LiquidateCalculateSeizeTokens(amount);
    }
    ...
    function changeCollateralFactor(uint256 newCollateralFactor) external override {
        _delegatecall(abi.encodeWithSelector(
            MasterAdmin.changeCollateralFactor.selector,
            newCollateralFactor
        ));
    }
    ...
    MasterInternals
{
    ...
    function initialize(address middleLayerParam, address eccParam, address pTokenOracleParam) external onlyOwner() {
        middleLayer = IMiddleLayer(middleLayerParam);
        ecc = IECC(eccParam);
        borrowIndex = 1e18;
        accrualBlockNumber = block.number;
        pTokenOracle = IPTokenOracle(pTokenOracleParam);
    }
...
abstract contract MasterStorage {
    ...
    */
    ...
    IPTokenOracle internal pTokenOracle;
...
abstract contract MasterEvents {
    ...
    event ReceiveFromChain(uint256 srcChainId, address fromAddress);
...
abstract contract MasterMessageHandler is IMaster, MasterModifiers, MasterEvents {
    ...
    function borrowAllowed(
        ...
        if (shortfall == 0) {
            (uint256 accountBorrowBalance, ) = _borrowBalanceStored(params.user);

            /////////////////////////
            // EFFECTS & INTERACTIONS
            // (No safe failures beyond this point)

            accountBorrows[params.user].principal = accountBorrowBalance + params.borrowAmount;
            ...
            bytes32 payloadMetadata = ecc.preRegMsg(payload, params.user);
            assembly {
                mstore(add(payload, 0x20), payloadMetadata)
            }
    ...
    function masterRepay(
        ...
        (uint256 accountBorrowBalance,) = _borrowBalanceStored(params.borrower);

        loansOutstanding[params.borrower][chainId] -= params.amountRepaid;
        totalBorrows -= params.amountRepaid;
        accountBorrows[params.borrower].principal = accountBorrowBalance - params.amountRepaid;
...
abstract contract MasterInternals is IMaster, MasterEvents {
    ...
    function _addToMarket(
        ...
        uint8 decimals = 0;
        {
            (bool sucess, bytes memory ret) = token.staticcall(abi.encodeWithSignature("decimals()"));
            require(sucess, "TOKEN_DECIMALS");
    ...
    function _getHypotheticalAccountLiquidity(
        ...
        for (uint256 i; i < assets.length; i++) {
            ...
            vars.oraclePrice = pTokenOracle.getUnderlyingPrice(asset.chainId, markets[asset.chainId][asset.token].underlying);
    ...
    function _getHypotheticalAccountLiquidityRedeem(
        ...
        uint256 borrowOraclePrice = pTokenOracle.getUnderlyingPriceBorrow();
    ...
    function _liquidateCalculateSeizeTokens(
        ...
        // PUSD Price
        uint256 priceBorrowed = pTokenOracle.getUnderlyingPriceBorrow();
        uint256 priceCollateral = pTokenOracle.getUnderlyingPrice(chainId, markets[chainId][pTokenCollateral].underlying);
...
contract DelegatorUtil is DelegatorStorage {
    // slither-disable-next-line assembly
    function _safeRevert(bool success, bytes memory returnData) internal pure {
        if (success) return;

        // If the _res length is less than 68, then the transaction failed silently (without a revert message)
        if (returnData.length < 68) revert("Transaction reverted silently");
