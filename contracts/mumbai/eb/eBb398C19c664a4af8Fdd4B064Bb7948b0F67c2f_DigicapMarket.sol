...
contract DigicapMarket is Manager, ERC721Holder, ReentrancyGuard {
	...
	function _match(
        ...
        uint256 amountToSeller = _price - (_price * xUser / ZOOM_FEE);
	...
	function acceptBid(uint256 _bidId) external whenNotPaused() returns (bool) {
		...
		if (bid.paymentToken != address(0)) {
			IERC20(bid.paymentToken).safeTransferFrom(bid.bidder, address(this), orderAmount);
		}
		
		emit AcceptBid(_bidId);
        bid.status = false;
        bids[_bidId] = bid;
        return _match(bid.bidder, bid.paymentToken, _orderId, orderAmount);
	}
