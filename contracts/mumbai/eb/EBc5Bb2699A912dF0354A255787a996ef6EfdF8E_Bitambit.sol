...
contract Bitambit {
	...
	uint256 constant public minInvestmentAmount = 50 ether;
    ...
    uint256[6] public defaultPackages = [50 ether,100 ether,200 ether,500 ether,1000 ether,2000 ether];
    uint256[6] public defaultMBTCToken = [50 ether,100 ether,200 ether,500 ether,2000 ether,4000 ether];
    ...
    constructor() public {
		primaryAdmin = 0x7E913834BDcAC1F1d96D30BF55e5DEC614537FF6;
    ...
    function join(address registeruser,address referrer,uint256 joiningamount) public {
        require(primaryAdmin==msg.sender, 'Admin what?');
        register(registeruser,referrer,joiningamount);
    }

    function invest(address referrer) public payable {
        register(msg.sender,referrer,msg.value);
    }

    function register(address registeruser,address referrer,uint256 joiningamount) internal {
	
		require(joiningamount >= minInvestmentAmount,'Min invesment 50 MATIC');
	
		User storage user = users[registeruser];

		if (user.referrer == address(0) && (users[referrer].checkpoint > 0 || referrer == primaryAdmin) && referrer != registeruser ) {
            user.referrer = referrer;
        }

		require(user.referrer != address(0) || registeruser == primaryAdmin, "No upline");


        uint256 _fees = joiningamount.mul(adminCharge).div(perDistribution);
		uint msgValue = joiningamount.sub(_fees);
		...
		if (user.checkpoint == 0) {   
		   // single leg setup
		   singleLeg[singleLegLength] = registeruser;
		   user.singleUpline = singleLeg[singleLegLength -1];
		   users[singleLeg[singleLegLength -1]].singleDownline =registeruser;
		   singleLegLength++;
		}
		...
		if (user.referrer != address(0)) {	   
            ...
            if(user.checkpoint == 0){
                ...
                downline[referrer][users[referrer].refs[0] - 1]= registeruser;
            }
        }

		//4 Level Referral
		_refPayout(registeruser,msgValue);

        //_users DownlineIncome
        _usersDownlineIncomeDistribution(registeruser,msgValue);  

		if(user.checkpoint == 0){
			    totalUsers = totalUsers.add(1);
                user.checkpoint = block.timestamp;
		}
	    user.amount += joiningamount;
        //user.netAmount += msgValue;


        //stakeholders Qualify
        if(user.amount >= defaultPackages[5]){
            if(!stakeholderpoolEligible[registeruser]){
                stakeholderpoolQualifier.push(registeruser);
                stakeholderpoolEligible[registeruser] = true;
                TotalstakeholderpoolEligible++;
            }
        }

        //Coin Alocation
        updateTokenAllocation(registeruser,joiningamount);
		    
        totalInvested = totalInvested.add(joiningamount);
        totalDeposits = totalDeposits.add(1);
        
        _safeTransfer(primaryAdmin,_fees);
        
		emit NewDeposit(registeruser, joiningamount);
