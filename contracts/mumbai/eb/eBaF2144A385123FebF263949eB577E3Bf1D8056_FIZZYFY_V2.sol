...
contract FIZZYFY_V2 
{
	...
	function _updateReferInfo(address _user, uint256 _amount) private {
        ...
        if(user.maxDeposit == 0)
        {
            newAmount = newAmount.add(_amount);
        }
        else if(user.maxDeposit < _amount)
        {
            newAmount = newAmount.add(_amount.sub(user.maxDeposit));
        }
        ...
        for(uint256 i = 0; i < referDepth; i++)
		{
            if(upline != address(0))
			{
                userInfo[upline].teamTotalDeposit = userInfo[upline].teamTotalDeposit.add(newAmount);
                _updateLevel(upline);
                if(upline == defaultRefer) break;
                upline = userInfo[upline].referrer;
            }
