...
// File: contracts/RewardProgram.sol
...
contract RewardProgram is Ownable {
    ...
    event ReceiveReward(address target, uint256 amount, bool indexed isWithdrawnByNominator);
        ...
        )
    {
        ...
        isWithdrawnByNominator = request.isWithdrawnByNominator;
    ...
    function transferOrApproveRewardToAddress(
        ...
        receivedRewards[target].push(
            AmountWithTime(amount, block.timestamp, isWithdrawnByNominator, isFullWithdrawal)
        );
        if (isApprove) {
        ...
        emit ReceiveReward(target, amount, isWithdrawnByNominator);
    }
    ...
    function receiveResidue(
        ...
        require(isWithdrawnByNominator, "reward is not received by user");
    ...
    function receiveReward(uint256 item) public {
        ...
        require(!isWithdrawnByNominator, "reward already received");
        ...
        if (!isFullWithdrawal) {
            request.isWithdrawnByNominator = true;
