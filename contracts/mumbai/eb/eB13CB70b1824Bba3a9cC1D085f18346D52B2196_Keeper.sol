...
contract Keeper is AutomationCompatibleInterface {
    ...
    //     _m = value; // liquidationdeltapercent //
    // }
    ...
    {
        AggregatorInterface oracle = AggregatorInterface(
            0x0715A7794a1dc8e42615F059dD6e406A6594651A
        );
        ...
        // while (i < _n - 1) {
        //     points[i] = OracleUtils.getAmountOracle(oracle, oracleTimestamp);
        //     oracleTimestamp += 3600;
        //     i++;
        // }

        // points[i] = OracleUtils.getAmountOracle(oracle, block.timestamp);

        // (int256 std, int256 mu) = MargineEngine.calculateStd(points);
        // int256 lm = MargineEngine.calculateLm(_p, _m, _c, std, mu);

        uint256 latestRoundId = oracle.latestRound();

        return (latestRoundId > 0, abi.encode(latestRoundId));
    }

    function performUpkeep(bytes calldata performData) external override {
        uint256 a = abi.decode(performData, (uint256));
