...
contract RoyaltyDistributorFactory is Ownable {
    ...
    /// Event emission when new RoylatyDistributor aka clone got created.
    event RoyaltyDistributorCreated(
    ...
    /// Emit when nft factory address set.
    event NftFactoryAddressSet(address nftFactory);
    ...
    /// @notice Facilitates the creation of the RoyaltyDistributor aka clone, Only nftFactory and owner is allowed to call.
        ...
        returns (address clone)
    {
        ...
        IRoyaltyDistributor(clone).initialize(royaltyOf, owner, payees, shares);
        _createdClones.push(clone);
        // Emit event for `RoyaltyDistributor` creation.
        emit RoyaltyDistributorCreated(clone, royaltyOf, payees, shares);
    }

    /// @notice Set nft factory address, Only owner is allowed to call.
    function setNftFactoryAddress(address _nftFactory) external onlyOwner {
        nftFactory = _nftFactory;
        emit NftFactoryAddressSet(nftFactory);
    }
