...
contract Coupon is ERC721, Ownable{
    ...
    */
    ...
    function viewLiveCouponsOnly() external returns(OnlyLive[] memory data){
        uint count;
        for(uint i = 0; i < allCouponStrings.length; i++){
            if(block.timestamp > couponInformation[showCouponStringAndCouponId[allCouponStrings[i]]]._startDate && block.timestamp < couponInformation[showCouponStringAndCouponId[allCouponStrings[i]]]._endDate){
                count = i + 1;
                testvalues = count;
            }
        }
        OnlyLive[] memory liveStatus = new OnlyLive[](count);
        for(uint i = 0; i < allCouponStrings.length; i++){
            if(block.timestamp > couponInformation[showCouponStringAndCouponId[allCouponStrings[i]]]._startDate && block.timestamp < couponInformation[showCouponStringAndCouponId[allCouponStrings[i]]]._endDate){
                OnlyLive memory statusInfo;
                statusInfo._couponNameOfLive = couponInformation[showCouponStringAndCouponId[allCouponStrings[i]]].couponName;
                statusInfo._status = status_1;
                liveStatus[i] = statusInfo;
            }
        }
        return liveStatus;
    }
