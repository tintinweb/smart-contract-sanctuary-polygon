...
contract IcoSalesContract is Ownable {
  ...
  function buy() public payable returns(bool) {
    require(_msgSender() != address(0), "Address can not be null.");
    uint _tokenValue = msg.value.mul(price);
    require(_tokenValue <= tokenForSale(), "No token left for sale.");
    require(_tokenValue >= minPurchase && _tokenValue <= maxPurchase, "Purchase not allowed in this range.");
  ...
  function setMinPurchase(uint _minPurchase) public onlyOwner {
    price = _minPurchase;
  }
  
  function setMaxPurchase(uint _maxPurchase) public onlyOwner {
    price = _maxPurchase;
  }
  ...
  function updatetokenAddress(IERC20 _newTokenAddress) public onlyOwner {
