...
contract NFTCapsule_Series is Pausable, INFTCapsule {
    ...
    function getTokenId() external override returns (uint256 tokenId) {
        ...
        uint256 fragmentId = (uint256(sha256(abi.encodePacked(uint160(msg.sender) + block.timestamp + counter))) % fragmentAmount) + 1;
    ...
    //NOTE:上線前拿掉
    function getTestTokenId(uint256 picId, uint256 fragmentId) external override returns (uint256 tokenId) {
        tokenId = _buildTokenId(picId, fragmentId);
        picMintAmount[picId]++;
        fragmentMintAmount[tokenId]++;
        totalSupply++;
