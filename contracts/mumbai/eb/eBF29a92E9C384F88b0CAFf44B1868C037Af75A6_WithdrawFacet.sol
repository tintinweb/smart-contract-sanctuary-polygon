...
contract WithdrawFacet is ReentrancyGuard {
    ...
    function withdrawPendingBalance(address _recipient, uint88 _tokenId, uint8 _listingId) external nonReentrant {
    ...
    /// @param _tokenId Id of the token on its contract.
    /// @param _listingId Token's listingId.
    function quarantineBlacklistedAccount(address _recipient, uint88 _tokenId, uint8 _listingId) external nonReentrant {
...
/******************************************************************************\
/// @title Portal - Library Withdraw Balance
...
library LibWithdrawBalance {
    ...
    function _increasePendingBalance(address _to, uint8 _listingId, uint88 _tokenId, uint88 _amount) internal {
        AppStorage storage s = LibAppStorage.diamondStorage();
    ...
    /// @param _to Address of recipient.
    /// @param _tokenId Id of the token on its contract.
    function _transferERC721(IERC721 _tokenAddress, address _to, uint128 _tokenId) internal returns (uint128) {
        uint256 balanceBefore = _tokenAddress.balanceOf(address(this));
    ...
    /// @param _to Address of recipient.
    /// @param _amount Amount to transfer.
    function _transferNativeToken(address _to, uint128 _amount, uint256 _minLiquidity) internal returns (uint128) {
