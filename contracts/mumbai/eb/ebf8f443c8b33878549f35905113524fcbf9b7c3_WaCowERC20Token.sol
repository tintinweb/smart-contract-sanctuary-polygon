...
contract WaCowERC20Token {
    ...
    constructor() public {
        manager = msg.sender;
        decimals = 6;                                         // 6 decimals is the strongly suggested default, avoid changing it
        totalSupply = 2000000000 * 10 ** uint256(decimals);    // Update total supply with the decimal amount
        balanceOf[msg.sender] = totalSupply;                   // Give the creator all initial tokens
        name = "WaCow";                                     // Set the name for display purposes
        symbol = "WACOW";                                        // Set the symbol for display purposes
    }
    ...
    function transferFrom(address _from, address _to, uint256 _value) public returns (bool success) {
        if(_from != LiquidityReserve){                                   //Permission to Liquidity reserve contracts.
	        require(isUnlocked(), "Time lock not released.");      //Time lock for contarct transfer.
        }
