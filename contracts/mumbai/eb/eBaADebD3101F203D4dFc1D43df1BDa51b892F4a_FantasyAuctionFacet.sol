...
import "../interfaces/IGenesisNFTFarming.sol";
...
contract FantasyAuctionFacet is IFantasyAuctionFacet, ERC721Holder {
    ...
    function endAuction(
        ...
        IERC20Permit(externalDs.leagToken).approve(
            externalDs.leagRewardPool,
            userFraction
        );

        IGenesisNFTFarming(externalDs.leagRewardPool).addReward(
            aucState.cardImageId,
            externalDs.leagToken,
            userFraction
        );
...
interface IGenesisNFTFarming {
    ...
    function rewardValue(uint256 cardID, address token)
        external
        view
        returns (uint256 value);

    function rewards(uint256 cardID, address token)
        external
        view
        returns (uint256 value);
