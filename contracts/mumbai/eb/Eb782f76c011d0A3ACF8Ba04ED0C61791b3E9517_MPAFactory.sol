...
contract MPAFactory {
    ...
    function getOwnedMPAs(address _user) external view returns (address[] memory) {
        require(msg.sender == _user || isManagement(msg.sender), "Denied.");
    ...
    function lockMPA(address _mpa, bool _state) external {
        require(isManagement(msg.sender), "Denied.");
        MPA(payable(_mpa)).freeze(_state);
    }
