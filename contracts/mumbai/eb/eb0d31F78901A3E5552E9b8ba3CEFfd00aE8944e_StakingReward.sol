...
contract StakingReward is OwnerPausable {
    ...
    function calculateWithdrawBack(address staker) public view returns(uint256[] memory, uint256) {
        ...
        if (_userInfo[staker].amount > 0) {
            for (uint256 i; i < userOrders.length; i++) {
                ...
                if (period > 0) {
                    result[i] = userOrders[i].amount*period/RELEASE_CYCLE_TIMES - userOrders[i].withdrawn;
                    total += result[i];
                }
            }
        }
    ...
    function withdraw(uint256 amount) external {
        address staker = msg.sender;
        (uint256[] memory arr, uint256 total) = calculateWithdrawBack(staker);
        ...
        for (uint256 i; i < arr.length; i++) {
            ...
            uint256 orderWithdraw;
            if (amount - withdrawn >= arr[i]) {
                orderWithdraw = arr[i];
            } else {
                orderWithdraw = amount - withdrawn;
            }
            userOrders[i].amount = userOrders[i].amount - orderWithdraw;
            userOrders[i].withdrawn += orderWithdraw;
            withdrawn += orderWithdraw;
            if (withdrawn == amount) {
                break;
            }
        }
        ...
        emit Withdrawn(staker, withdrawn, user.amount);
    }
