...
contract ThalloBridgeRegistry is UUPSUpgradeable, OwnableUpgradeable {
    ...
    mapping(address => bool) private registryCustodian;

    event NewProject(Project newProject);
    event NewVintage(address tcc, uint24 projectId, uint16 vintage);
    ...
    function createProject(Project memory _newProject) external onlyCustodian {
        ...
        require(!projectRegistered[projKey], "TBR:Proj exists");
        unchecked {
            ++currentProjectId;
        }
    ...
    function createVintage(
        ...
        require(ITBE(thalloBridgeEvents).getVintageAddress(vintageKey) == address(0), "TBR:Vintage exists");
        TCC newTcc = new TCC();
...
contract TCC is ERC20Upgradeable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    function finalizeUncustody(address _requestor, uint256 _quantity) external onlyBridge {
        require(escrowedUncustody[_requestor] >= _quantity, "TCC: Not enough escrowed");
