...
contract PlanetsMixV5 is ERC721, Ownable, ERC2981, DefaultOperatorFilterer, ContextMixin, RoyaltiesV2Impl { 
    ...
    modifier mintCompliance(uint256 _mintAmount) {
        require(_mintAmount > 0 && _mintAmount <= maxTokenInCollection, "The mint limit for this account has been exceeded. Reduce the number and try again!");
        require(supply.current() + _mintAmount <= maxTokenInCollection, "You want more tokens to mint  than are in the collection. This is not possible. Reduce the amount and try again!");
        _;
    }
    ...
    function mintOnlyOwner(uint256 _mintAmount) public payable mintCompliance(_mintAmount) onlyOwner {
        ...
        require(msg.value >= CostForMinter * _mintAmount, "You did not send enough funds. Send the right amount and try again!");
    ...
    function isApprovedForAll(address _owner, address _operator) public view virtual override(ERC721) returns (bool isOperator) {   
