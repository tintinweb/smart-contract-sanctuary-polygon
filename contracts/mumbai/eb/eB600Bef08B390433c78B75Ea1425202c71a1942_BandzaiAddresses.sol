...
contract BandzaiAddresses is Ownable {
    ...
    event ContractSetted(AddressesInit.Addresses indexed contractSetted,string indexed contractNameSetted, address newAddress);

    event ContractUpdateinitialized(AddressesInit.Addresses indexed contractInitiate,string indexed contractNameSetted, address newAddress);
    event ContractUpdateCanceled(AddressesInit.Addresses indexed contractInitiateCanceled,string indexed contractNameSetted, address addressCanceled);
    event ContractUpdated(AddressesInit.Addresses indexed contractUpdated,string indexed contractNameSetted, address oldAddress, address newAddress);

    constructor(){
        deploymentTimestamp = block.timestamp;
    }

    modifier addressAndNameMatch(AddressesInit.Addresses _contract,string memory _contractName, address _addr){
    ...
    modifier initializationCheck(AddressesInit.Addresses _contract,string memory _contractName){
        require(
    ...
    {
        return (
            _address == addresses[AddressesInit.Addresses.NURSERY_MANAGEMENT].currentAddress ||
            _address == addresses[AddressesInit.Addresses.TRAINING_MANAGEMENT].currentAddress ||
            _address == addresses[AddressesInit.Addresses.LABO_MANAGEMENT].currentAddress ||
            _address == addresses[AddressesInit.Addresses.EGGS_NFT].currentAddress ||
            _address == addresses[AddressesInit.Addresses.MARKET_DUTCH_AUCTION_ZAI].currentAddress ||
            _address == addresses[AddressesInit.Addresses.ALCHEMY].currentAddress ||
            _address == addresses[AddressesInit.Addresses.FIGHT].currentAddress ||
        ...
        returns (bool)
    {
        return (
            ...
            _address == addresses[AddressesInit.Addresses.TRAINING_MANAGEMENT].currentAddress ||
            _address == addresses[AddressesInit.Addresses.FIGHT].currentAddress ||
            _address == addresses[AddressesInit.Addresses.RANKING].currentAddress ||
            _address == addresses[AddressesInit.Addresses.MARKET_PLACE].currentAddress ||
            _address == addresses[AddressesInit.Addresses.POTIONS_NFT].currentAddress);
    }
    ...
    function setAddress(
        ...
        external 
        onlyOwner 
        addressAndNameMatch(_contract,_contractName,_addr){
        ...
        require(addresses[_contract].currentAddress == address(0), "Contract already setted plz use initializeAddress");
    ...
    function initializeAddress(
        ...
        emit ContractUpdateinitialized(_contract,contractName[uint256(_contract)], _addr);
    ...
    function cancelInitialization(
        ...
        external 
        onlyOwner
        initializationCheck(_contract,_contractName){
        ...
        emit ContractUpdateCanceled(_contract,contractName[uint256(_contract)], _pendingAddress);
    ...
    function updateContract(
        ...
        emit ContractUpdated(_contract, _contractName, _oldAddress, c.currentAddress);
    ...
    function _updateAllInterfaces() internal {
        IZaiMeta(addresses[AddressesInit.Addresses.ZAI_META].currentAddress).updateInterfaces();
        IipfsIdStorage(addresses[AddressesInit.Addresses.IPFS_STORAGE].currentAddress).updateInterfaces();
        ILaboratory(addresses[AddressesInit.Addresses.LABO_NFT].currentAddress).updateInterfaces();
        ILabManagement(addresses[AddressesInit.Addresses.LABO_MANAGEMENT].currentAddress).updateInterfaces();
        ITraining(addresses[AddressesInit.Addresses.TRAINING_NFT].currentAddress).updateInterfaces();
        ITrainingManagement(addresses[AddressesInit.Addresses.TRAINING_MANAGEMENT].currentAddress).updateInterfaces();
        INurseryManagement(addresses[AddressesInit.Addresses.NURSERY_MANAGEMENT].currentAddress).updateInterfaces();
        IPotions(addresses[AddressesInit.Addresses.POTIONS_NFT].currentAddress).updateInterfaces();
        IOpenAndClose(addresses[AddressesInit.Addresses.OPEN_AND_CLOSE].currentAddress).updateInterfaces();
        IFighting(addresses[AddressesInit.Addresses.FIGHT].currentAddress).updateInterfaces();
        ILevelStorage(addresses[AddressesInit.Addresses.LEVEL_STORAGE].currentAddress).updateInterfaces();
        IRewardsWinningFound(addresses[AddressesInit.Addresses.REWARDS_WINNING_PVE].currentAddress).updateInterfaces();
        IRewardsRankingFound(addresses[AddressesInit.Addresses.REWARDS_RANKING].currentAddress).updateInterfaces();
        IRewardsPvP(addresses[AddressesInit.Addresses.REWARDS_PVP].currentAddress).updateInterfaces();
        IRanking(addresses[AddressesInit.Addresses.RANKING].currentAddress).updateInterfaces();
        IDelegate(addresses[AddressesInit.Addresses.DELEGATE].currentAddress).updateInterfaces();
        ILootProgress(addresses[AddressesInit.Addresses.LOOT].currentAddress).updateInterfaces();
        IMarket(addresses[AddressesInit.Addresses.MARKET_DUTCH_AUCTION_ZAI].currentAddress).updateInterfaces();
    }
