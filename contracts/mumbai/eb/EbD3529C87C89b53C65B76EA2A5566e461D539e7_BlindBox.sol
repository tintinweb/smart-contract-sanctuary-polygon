...
contract BlindBox is Ownable, ReentrancyGuard {
    ...
    constructor(ITemplar templar_) public {
        boxStatus = false;
    ...
    function blindPurchase(uint256 nftAmount) public nonReentrant {
        require(nftAmount > 0, "multiple");
        require(boxStatus, "box is pause");

        uint256 fee = nftAmount * payPrice;
        ...
        if (inviter != address(0)) {
            ...
            templar.setUserReward(msg.sender, 0, fee.mul(3).div(100));
    ...
    function psuedoRandomness(uint256 sat) public view returns (uint256) {
        return
            uint256(
                keccak256(
                    abi.encodePacked(
