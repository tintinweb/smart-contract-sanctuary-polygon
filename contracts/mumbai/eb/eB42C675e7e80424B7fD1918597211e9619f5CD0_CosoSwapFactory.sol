...
import './CosoSwapPair.sol';
import '../../interfaces/protocol/core/ICosoSwapFactory.sol';

contract CosoSwapFactory is ICosoSwapFactory {
    ...
    function createPair(address tokenA, address tokenB) external override returns (address pair) {
        ...
        pair = address(new CosoSwapPair(token0, token1));
    ...
    function setSwapFeeOverride(address _pair, uint16 _swapFeeOverride) external override onlyFeeToSetter {
        CosoSwapPair(_pair).setSwapFeeOverride(_swapFeeOverride);
    }
}
...
contract CosoSwapPair is ICosoSwapPair, ERC20WithPermit, ReentrancyGuard {
    ...
    function getSwapFee() public view override returns (uint16) {
        ...
        return _swapFeeOverride == SWAP_FEE_INHERIT ? ICosoSwapFactory(factory).swapFee() : _swapFeeOverride;
    }
    ...
    function _tryMintProtocolFee(uint112 _reserve0, uint112 _reserve1) private {
        uint _kLast = kLast;
        if (_kLast != 0) {
            ICosoSwapFactory _factory = ICosoSwapFactory(factory);
