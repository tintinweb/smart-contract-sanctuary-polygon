...
import "./interfaces/IBasePoolV1.sol";
...
import "./interfaces/IGeneralPoolV1.sol";
import "./interfaces/IERC20.sol";
import "./libraries/TransferHelper.sol";
...
contract GeneralPairPoolV1 is IGeneralPoolV1, ERC1155Supply {
    ...
    function initialize(
        ...
        question = question_;
        options = options_;
        reserves = new uint256[](options.length);
    }
    ...
    function addLiquidity(uint256 reserve0) external override onlyPoolManager whenNotPaused {
        require(tradeEndTime > block.timestamp, "end");
        require(reserves[0] == 0, "already add liquidity");
    ...
    function updateClaimTime(uint256 newClaimTime) external override onlyPoolManager {
    ...
    function submitResult(uint8 option, string calldata description) external override onlyPoolManager {
        ...
        emit SubmitResult(description, option, totalSupply(option));
    }
    ...
    function buy(
        ...
        require(reserves[0] > 0 && reserves[1] > 0, "no liquidity");
        ...
        uint256 newReserve1;
        if (option == 0) {
            newReserve1 = amountWithoutFee * tradeFactor + reserves[1];
            newReserve0 = reserves[0].mulDiv(reserves[1], newReserve1);
            shares = amountWithoutFee.mulDiv(
                newReserve0 + newReserve1,
                newReserve1
            );
        } else {
            newReserve0 = amountWithoutFee * tradeFactor + reserves[0];
            newReserve1 = reserves[0].mulDiv(reserves[1], newReserve0);
            shares = amountWithoutFee.mulDiv(
                newReserve0 + newReserve1,
                newReserve0
            );
        }
        _mint(to, option, shares, "");
        reserves[0] = newReserve0;
        reserves[1] = newReserve1;
        collateralReserve = collateralBalance;
        emit Buy(msg.sender, to, option, amount, shares, 0, reserves);
    }
    ...
    function sell(
        ...
        uint256 shares = balanceOf(address(this), option);
        uint256 newReserve0;
        uint256 newReserve1;
        if (option == 0) {
            newReserve0 = reserves[0] + shares;
            newReserve1 = reserves[0].mulDiv(reserves[1], newReserve0);
            amount = shares.mulDiv(newReserve0 + newReserve1, newReserve1);
        } else {
            newReserve1 = reserves[1] + shares;
            newReserve0 = reserves[0].mulDiv(reserves[1], newReserve1);
            amount = shares.mulDiv(newReserve0 + newReserve1, newReserve0);
        }
        _burn(address(this), option, shares);
        reserves[0] = newReserve0;
        reserves[1] = newReserve1;
        TransferHelper.safeTransfer(collateralToken, to, amount);
        collateralReserve = IERC20(collateralToken).balanceOf(address(this));
        emit Sell(msg.sender, to, option, amount, shares, 0, reserves);
    }

    function claim(address to) public override whenNotPaused returns (uint256 amount) {
        require(to != address(0), "to is address zero");
        require(
        ...
        amount = balanceOf(address(this), resultOption);
        require(amount > 0, "nothing to be claimed");
        _burn(address(this), resultOption, amount);

        TransferHelper.safeTransfer(collateralToken, to, amount);
        collateralReserve = IERC20(collateralToken).balanceOf(address(this));
        emit Claim(msg.sender, to, amount, new uint256[](0));
    }

    function claim(address to, uint256[] memory rounds) external override whenNotPaused returns (uint256 amount) {
        return claim(to);
    }
}
...
import "./GeneralPairPoolV1.sol";
import "./interfaces/IGeneralPoolManagerV1.sol";

contract GeneralPoolManagerV1 is IGeneralPoolManagerV1, BasePoolManagerV1 {
    constructor(
        address collateralToken_,
        uint16 tradeFeeRate_,
        uint8 tradeFactor_
    ...
    function createGeneralPairPool(
        ...
        string memory tag
    ) external override onlyBuilder returns (address pool) {
        require(options.length == 2, "wrong options length");
        require(
        ...
        require(getPool[poolId] == address(0), "pool exist");
        pool = address(new GeneralPairPoolV1{salt: poolId}());
        getPool[poolId] = pool;
        GeneralPairPoolV1(pool).initialize(
            collateralToken,
            tradeFeeRate,
            tradeFactor,
            tradeStartTime,
        ...
        TransferHelper.safeTransfer(collateralToken, pool, collateralAmount);
        GeneralPairPoolV1(pool).addLiquidity(reserve0);
    }

    function updateClaimTime(bytes32 poolId, uint256 newClaimTime) external override onlyBuilder {
        require(getPool[poolId] != address(0), "pool not found");
        IGeneralPoolV1 pool = IGeneralPoolV1(getPool[poolId]);
        pool.updateClaimTime(newClaimTime);
    }
    ...
    function submitResult(
        ...
        require(getPool[poolId] != address(0), "pool not found");
        IGeneralPoolV1 pool = IGeneralPoolV1(getPool[poolId]);
        pool.submitResult(option, description);
    }
    ...
    function pause(bytes32 poolId) external override onlyBuilder {
        require(getPool[poolId] != address(0), "pool not found");
        IBasePoolV1 pool = IBasePoolV1(getPool[poolId]);
        pool.pause();
        emit PoolPaused(poolId);
    }

    function unpause(bytes32 poolId) external override onlyBuilder {
        require(getPool[poolId] != address(0), "pool not found");
        IBasePoolV1 pool = IBasePoolV1(getPool[poolId]);
        pool.unpause();
...
interface IBasePoolV1 {
    ...
    event Buy(address indexed sender, address indexed to, uint8 indexed option, uint256 amount, uint256 shares, uint256 round, uint256[] reserves);
    event Sell(address indexed sender, address indexed to, uint8 indexed option, uint256 amount, uint256 shares, uint256 round, uint256[] reserves);
...
interface IGeneralPoolManagerV1 is IBasePoolManagerV1 {
    event PoolPaused(bytes32 poolId);
    event PoolUnpaused(bytes32 poolId);
    ...
    function updateClaimTime(bytes32 poolId, uint256 newClaimTime) external;
    function submitResult(bytes32 poolId, uint8 option, string calldata description) external;
...
interface IGeneralPoolV1 is IBasePoolV1 {
