...
library FixedAndVariableMath {
    ...
    function accrualFact(
        uint256 timeInSecondsAsWad
    ) internal pure returns (uint256 timeInYearsWad) {
...
interface IFactory is CustomErrors {
    ...
    function masterFCMs(
        uint8 yieldBearingProtocolID
    ) external view returns (IFCM masterFCM);
}
...
interface IMarginEngine is IPositionStructs, CustomErrors {
    ...
    function transferMarginToFCMTrader(
        address _account,
...
interface IPeriphery is CustomErrors {
    ...
    function setLPMarginCumulative(
        IVAMM vamm,
        int256 lpMarginCumulative
    ) external;
...
interface IRateOracle is CustomErrors {
    ...
    function getApyFromTo(
        uint256 from,
        uint256 to
    ) external view returns (uint256 apyFromTo);
    ...
    /// @dev Can only be set by the Factory Owner
    function setMinSecondsSinceLastUpdate(
    ...
    UUPSUpgradeable
{
    ...
    function setLPMarginCap(
        IVAMM vamm,
        int256 lpMarginCapNew
    ) external override vammOwnerOnly(vamm) {
    ...
    function mintOrBurn(
        MintOrBurnParams memory params
    ) public payable override returns (int256 positionMarginRequirement) {
    ...
    function getCurrentTick(
        IMarginEngine marginEngine
    ) external view override returns (int24 currentTick) {
        IVAMM vamm = marginEngine.vamm();
