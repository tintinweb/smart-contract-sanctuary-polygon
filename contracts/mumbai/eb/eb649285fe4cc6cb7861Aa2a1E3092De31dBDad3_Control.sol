...
contract Control is Initializable, Proxied, Governed, IControl {
    ...
    mapping(uint256 => bool) public alreadyReceivedEmptyNFT;
    ...
    function getEmptyNFT() external override returns (uint256) {
        uint256 senderId = getUsersId(msg.sender);
        ...
        for (uint256 i; i < amount; ) {
            ...
            if (metaProject.getLevelForNFT(senderId) < requeredLevels[registry.getIndexProject(i)]) {
    ...
    function mergeNFT(uint256 _level, uint256[] memory idNFT) external override returns (uint256[] memory) {
    ...
    function getUsersId(address _user) internal returns (uint256) {
