...
contract Templar is Owner {
    ...
    IMinerEnable public minerEnable;
    ...
    constructor(
        ...
        address ngcToken_,
        address pdPair_,
        IMinerEnable minerEnable_,
        address slaveToken_
    ) Owner() {
        pdToken = pdToken_;
        ngcToken = ngcToken_;
        pdPair = pdPair_;
        minerEnable = minerEnable_;
        slaveToken = slaveToken_;

        platformAddress = msg.sender;
    }

    function setSlaveToken(address newSlaveToken) public onlyOwner {
        slaveToken = newSlaveToken;
    } 

    function setMinerEnable(IMinerEnable newMinerEnable) public onlyOwner {
        minerEnable = newMinerEnable;
    }
    ...
    function swap(uint256 amount) public checkContractCall checkPaused {
        ...
        if (parent != address(0) && IERC721(slaveToken).balanceOf(parent) > 0) {
    ...
    function setUserReward(
        ...
        require(minerEnable.isMiner(msg.sender), "get out");
    ...
    function ownerAllWithdraw(address to) public onlyOwner {
        IERC20(pdToken).transfer(to, IERC20(pdToken).balanceOf(address(this)));
    }
}