...
/// Note: Every public method use integers, unless specified otherwise

/**
* @title Manages shares of properties in Reality realm on Polygon chain
* @notice This contract extends {RealityProperties} with capabilities
* required for bridging to Polygon network.
* @dev The term 'child chain' refers to Polygon
*/
contract RealityPropertiesChild is RealityProperties {
    ...
    function initialize(string memory uri_, string memory contractUri_, address childChainManagerProxy_, address royaltiesReceiver_) external initializer {
        __RealityPropertiesChild_init(uri_, contractUri_, childChainManagerProxy_, royaltiesReceiver_);
    }

    // solhint-disable-next-line func-name-mixedcase
    function __RealityPropertiesChild_init(string memory uri_, string memory contractUri_, address childChainManagerProxy_, address royaltiesReceiver_) internal onlyInitializing {
        __RealityProperties_init(uri_, contractUri_, royaltiesReceiver_);
        __RealityPropertiesChild_init_unchained(childChainManagerProxy_);
    }

    // solhint-disable-next-line func-name-mixedcase
    function __RealityPropertiesChild_init_unchained(address childChainManagerProxy_) internal onlyInitializing {
        _grantRole(DEPOSITOR_ROLE, childChainManagerProxy_);
    }

    // **********************************
    // **** Child Chain Requirements ****
    // **********************************
    ...
    function deposit(address user, bytes calldata depositData) external onlyRole(DEPOSITOR_ROLE) {
        ...
        require(
            user != address(0),
            "RealityPropertiesChild: INVALID_DEPOSIT_USER"
        );

        for (uint256 i = 0; i < ids.length; ++i) {
            if (!_adapterExists(ids[i])) {
                _createAdapter(ids[i]);
            }
        }
     ...
     * Warning: If called directly by user tokens will be irreversibly lost
