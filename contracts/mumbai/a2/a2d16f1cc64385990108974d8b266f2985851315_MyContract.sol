...
contract MyContract {
    ...
    uint256[] public luckyNumberList;
    ...
    event RandomizedUpdate(
        uint32 randomize
    );

    event RandomizedListUpdate(
        uint256[] randomize
    );
    ...
    function fetchRandomNumber() external {
        ...
        emit RandomizedUpdate(randomness);
    }

    function fetchRandomNumber(uint32 range, uint256 randomValues) external {
        assert(latestRandomizingBlock > 0);
        randomness = witnet.random(range, randomValues, latestRandomizingBlock);
        emit RandomizedUpdate(randomness);
    }

    function fetchRandomNumber(uint32 range, uint256 randomValues,uint256 _block) external returns (uint32) {
        assert(latestRandomizingBlock > 0);
        randomness = witnet.random(range, randomValues, _block);
        return randomness;
    }

    function fetchRandomNumber(uint32 range, uint256 nonce,uint256 _numberOfTokens,uint256 _block) internal returns (uint256[] memory expandedValues) {
        randomness = witnet.random(range, nonce, _block);
        expandedValues = new uint256[](_numberOfTokens);
        for (uint256 i = 0; i < _numberOfTokens; i++) {
            expandedValues[i] = uint256(keccak256(abi.encode(randomness, i)));
        }
        emit RandomizedListUpdate(expandedValues);
        return expandedValues;
    }

    function getRandomNumber(uint256 userCount,uint32 range, uint256 nonce,uint256 _numberOfTokens,uint256 _block) external returns (uint256[] memory expandedValues) {
        uint256[] memory win=fetchRandomNumber(range,nonce,_numberOfTokens,_block);
    
        for (uint256 i = 0; i < _numberOfTokens; i++) {
            uint256 indexOfWinner = win[i] % userCount;
            luckyNumberList.push(indexOfWinner);
        }
        emit LuckyListUpdate(luckyNumberList);
        return expandedValues;
    }
