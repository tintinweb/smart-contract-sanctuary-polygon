...
import "./interfaces/IAetMasterChef.sol";

contract AetVault is Ownable, Pausable {
    ...
    IAetMasterChef public masterchef;
    ...
    constructor(IAetMasterChef _masterchef) {
        masterchef = IAetMasterChef(_masterchef);
    }

    function updateMasterChef(address addr) external onlyOwner {
        masterchef = IAetMasterChef(addr);
    }
    ...
    function calculateHarvestAetRewards(uint256 _pid) external view returns (uint256) {
        uint256 amount = masterchef.payout(_pid, address(this));
        amount = amount.add(available(_pid));
