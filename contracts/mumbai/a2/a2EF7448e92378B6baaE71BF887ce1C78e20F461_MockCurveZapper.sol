...
contract MockCurveZapper is ICurveZapper {
    function add_liquidity(
        address pool,
        uint256[4] calldata deposit_amounts,
        uint256 min_mint_amount
    ) external returns (uint256) {
        MockCurvePool pool_ = MockCurvePool(pool);
        uint256 liquidity;
        for (uint256 i; i < 4; i++) {
            if (deposit_amounts[i] > 0) {
                ERC20 coin = ERC20(pool_.underlying_coins(i));
                coin.transferFrom(msg.sender, pool, deposit_amounts[i]);
                liquidity += deposit_amounts[i] * (10**(18 - coin.decimals()));
            }
        }
        pool_.mint(msg.sender, liquidity);
        return liquidity;
    }
    ...
    function exchange_underlying(
        ...
        MockCurvePool pool_ = MockCurvePool(pool);
        IERC20(pool_.underlying_coins(uint128(i))).transferFrom(
            msg.sender,
            pool,
            dx
        );
        pool_.sendTokens(msg.sender, uint128(j), min_dy);
        return min_dy;
    }
}
