...
contract Hivemind is HivemindV1Storage, HivemindI, ErrorReporter, AccessControlEnumerable, ReentrancyGuard {
  ...
  function getCollateralPriceInUnderlyingPrice(HTokenI _hToken) internal view returns (uint256) {
    uint256 nftPriceInETH = oracle.getUnderlyingFloorNFTPrice(address(_hToken.collateralToken()), _hToken.decimals());
    uint256 underlyingPriceInUSD = uint256(oracle.getUnderlyingPriceInUSD(_hToken.underlyingToken(), _hToken.decimals()));
    uint256 ethPrice = uint256(oracle.getEthPrice(_hToken.decimals()));
  ...
  function _setCollateralFactor(HTokenI _hToken, uint256 _newCollateralFactorMantissa) external {
    ...
    if (
      ...
      (oracle.getUnderlyingPriceInUSD(_hToken.underlyingToken(), _hToken.decimals()) == 0 ||
        oracle.getUnderlyingFloorNFTPrice(address(_hToken.collateralToken()), _hToken.decimals()) == 0)
    ) {
      revert MarketError(Error.MARKET_NOT_LISTED);
    }
...
interface PriceOracleI {
  function requestFloor(string calldata _collection, string calldata _pricingAsset) external;
