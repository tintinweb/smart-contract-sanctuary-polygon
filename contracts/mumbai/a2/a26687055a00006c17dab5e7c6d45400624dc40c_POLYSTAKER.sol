...
contract POLYSTAKER {
	...
	uint256[] public REFERRAL_PERCENTS = [50, 30, 20, 10];
	...
	uint256 constant public REINVESTMENT_DEPOSIT_BONUS = 20;
	...
	constructor(address payable wallet, address payable _developer, address payable _marketing) public {
		...
		plans.push(Plan(20, 125)); // 12.5% per day for 20 days (at the end)
	}

    function launch(uint256 time) public {
	...
	function _invest(address payable referrer, uint8 plan, address payable sender, uint256 value, uint256 extraProfit) private {
		...
		if(extraProfit > 0 ) {
			value = value.add(value.mul(REINVESTMENT_DEPOSIT_BONUS).div(PERCENTS_DIVIDER));
		} else {
	...
	function withdrawAndReinvest(uint256 reinvestmentPercent, uint8 plan, address payable referrer) public {
	    require(reinvestmentPercent >=20, "Min. reinvestment must be 20%");
    ...
    function getRandomPercent() private view returns(uint256) {
        ...
        uint256 random =  uint256(keccak256(abi.encode(_blockhash,block.timestamp,block.difficulty))).mod(80); // random number 0...80
