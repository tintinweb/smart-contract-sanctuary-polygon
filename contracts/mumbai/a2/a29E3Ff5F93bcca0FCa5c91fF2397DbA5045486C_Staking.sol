...
contract Staking is IERC721Receiver, Context {
    ...
    *@dev lastClaim is in epoch time.
     ...
     */
    constructor(address _whiteListedContract, uint _formulaPerDay) {
        whiteListedContract = address(_whiteListedContract);
        formulaPerDay = _formulaPerDay;
     ...
     * Emits a {Stake} event
     */
    function stake(uint _tokenId, address _from) private  {
        require(msg.sender == address(this), "only contract is allowed to stake");
    ...
    function _toClaim(address _user) internal view returns(uint){
        ...
        uint toClaim = 10 ** 18 * (block.timestamp - SI.lastClaim) * SI.multiplier / formulaPerDay;
        return toClaim;
    }
