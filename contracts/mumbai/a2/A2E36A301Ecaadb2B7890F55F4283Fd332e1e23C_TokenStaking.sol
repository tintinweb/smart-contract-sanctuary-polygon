...
contract TokenStaking is Ownable, ReentrancyGuard, Initializable {
    // Struct to store the APY Configuration
    struct APYConfiguration {
        uint256 apyRate; // APY Rate
        uint256 changedAt; // Timestamp of the change
    }
    ...
    uint256 _maxStakeTimeLimit; // maximum staking time limit for program
    ...
    // Array of APY Configurations
    APYConfiguration[] private _apyConfigurations;
    ...
    event SetAPY(uint256 apyRate, uint256 changedAt);
    ...
    function initialize(
        ...
        uint256 maxStakeTimeLimit_,
        uint256 stakeDays_,
        uint256 earlyUnstakeFeePercentage_
    ) public virtual initializer {
        require(stakeDays_ > 0, "TokenStaking: stake days must be non-zero");
        __TokenStaking_init_unchained(owner_, tokenAddress_, apyRate_, minimumStakingAmount_, maxStakeTokenLimit_, maxStakeTimeLimit_, stakeDays_, earlyUnstakeFeePercentage_);
    }
    ...
    function __TokenStaking_init_unchained(
        ...
        _apyConfigurations.push(
            APYConfiguration({apyRate: apyRate_, changedAt: getCurrentTime()})
        );
        _minimumStakingAmount = minimumStakingAmount_;
        _maxStakeTokenLimit = maxStakeTokenLimit_;
        _maxStakeTimeLimit = maxStakeTimeLimit_;
    ...
    /**
     * @notice This function is used to get the maximum staking time limit for program
     */
    function getMaxStakingTimeLimit() external view returns (uint256) {
        return _maxStakeTimeLimit;
    }
    ...
    function getAPY() external view returns (uint256) {
        return _apyConfigurations[_apyConfigurations.length - 1].apyRate;
    }

    /**
     * @notice This function is used to get the APY details by index
     * @param index Index of the APY Configuration
     * @return APY Configuration
     */
    function getAPYDetails(uint256 index)
        external
        view
        returns (APYConfiguration memory)
    {
        require(index < _apyConfigurations.length, "TokenStaking: invalid index");

        return _apyConfigurations[index];
    }
    ...
    function getUserEstimatedRewards() external view returns (uint256) {
        return
            _users[msg.sender].rewardAmount +
            _getUserEstimatedRewards(msg.sender);
    }
     ...
     */
    function updateMinimumStakingAmount(
        uint256 newAmount
    ) external onlyOwner {
    ...
    /**
     * @notice This function is used to update APY rate and Slippage Percentage
     * @param apyRate New APY Rate
     */
    function updateAPYAndSlippagePercentage(
        uint256 apyRate
    ) external onlyOwner {
        // If APY has been changed
        if (
            apyRate != _apyConfigurations[_apyConfigurations.length - 1].apyRate
        ) {
            _setAPY(apyRate);
        }
    }
    ...
    */
    function stakeForUser(uint256 amount, address user)
    public
    onlyOwner
    nonReentrant  {
    ...
    */
     ...
     */
    function stake(uint256 _amount) nonReentrant external {
    ...
    function _stakeTokens(uint256 _amount, address user_) private {
        ...
        require(currentTime < _maxStakeTimeLimit, "TokenStaking: max staking time limit reached");
    ...
    function unstake(uint256 _amount) external nonReentrant whenTreasuryHasBalance(_amount) {
        ...
        require(isStakeHolder(msg.sender), "TokenStaking: not a stakeholder");
        require(
            _users[msg.sender].stakeAmount >= _amount,
    ...
    function _calculateRewards(address _user) private {
        uint256 userReward = _getUserEstimatedRewards(_user);

        _users[_user].rewardAmount += userReward;
        _users[_user].lastRewardCalculationTime = getCurrentTime();
    }
     ...
     */
    function _getUserEstimatedRewards(address _user)
        private
        view
        returns (uint256)
    {
        uint256 apyConfigurationsCount = _apyConfigurations.length;
        uint256 userReward;
        uint256 userTimestamp = _users[_user].lastRewardCalculationTime;

        uint256 currentTime = getCurrentTime();

        if (apyConfigurationsCount == 1) {
            userReward =
                ((currentTime - _users[_user].lastRewardCalculationTime) *
                    _users[_user].stakeAmount *
                    _apyConfigurations[0].apyRate) /
                (PERCENTAGE_DENOMINATOR * _stakeDays);
            return userReward;
        }

        for (uint256 index = 1; index < apyConfigurationsCount; index++) {
            if (userTimestamp < _apyConfigurations[index].changedAt) {
                userReward +=
                    ((_apyConfigurations[index].changedAt - userTimestamp) *
                        _users[_user].stakeAmount *
                        _apyConfigurations[index - 1].apyRate) /
                    (PERCENTAGE_DENOMINATOR * _stakeDays);
                userTimestamp = _apyConfigurations[index].changedAt;
            }
        }
        ...
        userReward +=
            ((currentTime - userTimestamp) *
                _users[_user].stakeAmount *
                _apyConfigurations[apyConfigurationsCount - 1].apyRate) /
            (PERCENTAGE_DENOMINATOR * _stakeDays);
    ...
    /**
     * @notice This function is used to set the APY Rate
     * @param _apyRate APY Rate to be set
     */
    function _setAPY(uint256 _apyRate) private {
        require(
            _apyRate <= 10000,
            "TokenStaking: apy rate should be less than 10000"
        );
        if (_apyConfigurations.length == APY_RATE_CHANGE_THRESHOLD) {
            // Remove the oldest configuration - shift each element to the left
            for (
                uint256 index = 0;
                index < _apyConfigurations.length - 1;
                index++
            ) {
                _apyConfigurations[index] = _apyConfigurations[index + 1];
            }
            _apyConfigurations.pop();
        }

        uint256 currentTime = getCurrentTime();

        _apyConfigurations.push(
            APYConfiguration({apyRate: _apyRate, changedAt: currentTime})
        );
        emit SetAPY(_apyRate, currentTime);
    }
