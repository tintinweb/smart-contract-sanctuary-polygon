...
contract Pool is Initializable, AccessControlEnumerableUpgradeable, IPool {
    ...
    function onERC721Received(
        ...
        _createOrAddToPack(token, numOfRewardUnits);
    ...
    function withdraw(ITokenBundle.Token[] calldata token, address recipient) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        for (uint i = 0; i < len; i++) {
            ...
            if (t.tokenType == ITokenBundle.TokenType.ERC20) {
...
import "./ITimeboundEIP712.sol";
import "./IContractOpenablePack.sol";
import "./IPool.sol";
...
interface IPoolFactory is ITimeboundEIP712, IERC1155ReceiverUpgradeable {
    ...
    function nextPoolId() external returns(uint256);

    function poolBeacon() external returns(UpgradeableBeacon);

    function packContract() external returns(IContractOpenablePack);

    function poolContract(uint256) external returns(IPool);

    function openCount(uint256) external returns(uint256);

    function createPool(uint128 _openStartTimestamp, uint128 _amountDistributedPerOpen) external returns(address _poolAddress);
