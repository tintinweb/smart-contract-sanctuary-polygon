...
contract TokenFactory is Initializable, BasicMetaTransaction {
    ...
    function initialize(address _masterToken, address _admin)
        external
        initializer
    {
    ...
    function createClone(address _masterToken) public returns (address result) {
        bytes20 targetBytes = bytes20(_masterToken);
        assembly {
    ...
    function createToken(
        ...
        // address clone = MinimalProxy.clone(masterToken);
        address Clone = createClone(masterToken);
        // address clone = Clones.clone(masterToken);
