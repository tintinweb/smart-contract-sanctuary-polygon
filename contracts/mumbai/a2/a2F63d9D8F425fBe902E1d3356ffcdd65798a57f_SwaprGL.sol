...
interface ISwaprWallet {
    ...
    function depositNativeSwapr(address depositor, uint amount) external;
...
contract SwaprGL is BaseGovernanceUpgradeable, ListingHelper {
    ...
    function initialize(bytes calldata data) public initializer {
        ...
        swaprWallet = ISwaprWallet(payable(walletAddress));
        swaprFee = ISwaprFee(payable(swaprFeeAddress));
    ...
    function attachNewWallet(address wallet) external onlyAdmin {
        ...
        swaprWallet = ISwaprWallet(payable(wallet));
    }
    ...
    function attachNewFeeContract(address feeContract) external onlyAdmin {
        //requires a decision in case
        swaprFee = ISwaprFee(feeContract);
    }
    ...
    function _payFromWallet(PayNow memory _payOps, uint256 depositType) internal returns(bool) {
        
        address feeReceiver = swaprFee.getFeeReceiver();
        uint256 finalListingFee = depositType == 1 ? 
            swaprFee.getFinalAuctionFee(_payOps.amount, _payOps.acceptedToken) : 
            swaprFee.getFinalOrderFee(_payOps.amount, _payOps.acceptedToken);

        if(_payOps.acceptedToken == address(0)){
            swaprWallet.swapNative(_payOps.from, _payOps.receiver, _payOps.amount - finalListingFee);
            swaprWallet.releaseNative(feeReceiver, _payOps.receiver, finalListingFee);
            if(_payOps.toEOA){
                swaprWallet.releaseNative(_payOps.receiver, _payOps.receiver, _payOps.amount);
            }
        }else{
            ...
            if(_payOps.toEOA){
                swaprWallet.releaseERC(_payOps.acceptedToken, _payOps.receiver, _payOps.receiver, _payOps.amount);
            }
        }
        return true;
    }
    ...
    /// @return true if payment succeeds
    function _payUpfront(PayNow memory _payOps, uint256 depositType, uint value) internal returns(bool) {
        ...
        if(_payOps.acceptedToken == address(0)){
            require(value >= _payOps.amount, "LOW_VALUE");
            payable(feeReceiver).transfer(finalListingFee);
            payable(receiver).transfer(_payOps.amount - finalListingFee);
            if(!_payOps.toEOA){
                swaprWallet.depositNativeSwapr(_payOps.receiver, _payOps.amount - finalListingFee);
            }
        }else{
            IERC20MetadataUpgradeable paymentToken = IERC20MetadataUpgradeable(_payOps.acceptedToken);
            require(paymentToken.allowance(_payOps.from, address(this)) >= _payOps.amount, "INSUF_ALWNC");
    ...
    function depositNFTs(bytes calldata data) external {
        ...
        require(_verify(_msgSender(), data), "INCOR_SIG");
    ...
    function createListing(uint listingType, uint fee, address paymentToken, bytes calldata data) external onlyMarketplace(data) {
        address sender = _msgSender();
        (bytes memory listingData,) = abi.decode(data, (bytes, bytes));
        (bool isValid, address lock, uint nftId, , bytes memory sig) = _validateListing(sender, listingType, listingData);
        require(isValid, "INVALID_LSTNG");
    ...
    function updateListing(uint listingType, bytes calldata data) external onlyMarketplace(data) {
        ...
        require(_verify(sender, activeListingData) && _verify(sender, proposedListingData), "SNDR_MUST_BE_SLR");
        uint createdOn;
        if(listingType == 1){
            //Auction
        ...
        (bool isValid, address lock, uint nftId,, bytes memory sig) = _validateListing(sender, listingType, proposedListingData);
        require(isValid, "INVALID_LSTNG");
    ...
    function buyNowOrder(uint buyerPurchasedAmount, bytes memory data, uint split) external onlyMarketplace(data) {
        ...
        require(split > 0 && split <= EXP && split <= order.remainingPart && split <= order.maxBuyPerWallet, "INCOR_SPLIT_AMT");

        address buyer = _msgSender();
        require(buyerPurchasedAmount < order.maxBuyPerWallet, "PURCH_LMT_EXCEED");
        ...
        require(payNow(payOps, order.depositType), "PMT_FAILED");

        if(split < order.remainingPart){
            uint[] memory splitParts = new uint[](2);
            address[] memory addresses = new address[](2);
            //seller's part
            uint splitRecount = (split * EXP / order.remainingPart);
    ...
    function buyNowAuction(bytes memory data) external onlyMarketplace(data) {
        ...
        require(payNow(payOps, auction.depositType), "PMT_FAILED");
    ...
    function _withdraw(address claimant, address lock, uint nftId) internal {
        require(swaprWallet.isNFTLocked(lock, nftId), "ASSET_NOT_AVAILBL");
...
import "../interfaces/ISwaprWallet.sol";
...
contract ListingHelper {
    ...
    function _validateListing(address sender, uint listingType, bytes memory listingData) internal view returns(bool isValid, address lock, uint nftId, address paymentToken, bytes memory sig) {

        require(_verify(sender, listingData));
        (, bytes memory _sig) = abi.decode(listingData, (bytes, bytes));
        sig = _sig;
        require(listingType == 1 || listingType == 2, "INV_LISTING_TYPE");
        if(listingType == 1){
            //Auction
    ...
    returns(
        ...
        uint tokenId, address paymentToken
    ){

        Auction memory auction = _extractAuctionInfo(_listingData);

        if(swaprWallet.isNFTLocked(auction.lock, auction.nftId)){
            require(auction.activeDepositType == 0 || auction.activeDepositType == 3, "INV_DEPOSIT_TYPE");
            isValid = true;
        }else{
            require(auction.depositType == 1, "INV_DEPOSIT_TYPE");
            require(auction.seller != address(0), "ADDR_ZERO");
            require(auction.startingPrice > 0, "INV_START_PRICE");
            require(auction.buyNowPrice > auction.startingPrice, "INV_BUY_NOW_PRICE");
            require(auction.startTime + timeOffset <= auction.endTime, "INV_START_TIME");
            require(auction.endTime >= block.timestamp + timeOffset, "INV_END_TIME");
            isValid = true;
        }
    ...
    returns(
        ...
        require(order.maxTokenSell > 0 && order.maxTokenSell <= EXP &&
            order.maxBuyPerWallet > 0 && order.maxBuyPerWallet <= EXP);
        if(swaprWallet.isNFTLocked(order.lock, order.nftId)){
            require(order.activeDepositType == 0 || order.activeDepositType == 3);
            isValid = true;
        }else{
            require(order.depositType == 2);
            require(order.seller != address(0));
            require(order.fixedPrice > 0);
            require(order.maxBuyPerWallet > 0 && order.maxBuyPerWallet <= EXP);
            isValid = true;
        } 
        lockAddr = order.lock; 
        tokenId = order.nftId; 
    ...
    returns(bool isValid, Bid memory validatedBid){
        ...
        require(auction.depositType == 1);
        require(_verify(proposedBid.bidder, proposedBidData));
        if(!isActiveBid){
        ...
        require(bidderTotalBalance >= proposedBid.lockedBalance);
        require(bidderTotalBalance - proposedBid.lockedBalance >= proposedBid.offerPrice);
        isValid = true;
    ...
    returns(
        bool isValid, uint proposedEndTime
    ){
        (bytes memory listingInfo,) = abi.decode(listingData, (bytes, bytes));
        Auction memory auction = _extractAuctionInfo(listingData);
        require(_verify(auction.seller, abi.encode(listingInfo, swaprWallet.getNFT(bid.lock, bid.nftId))));
        ...
        require(bid.offerPrice >= auction.startingPrice);
        isValid = true;
    }
    ...
    returns(
        ...
        require(_verify(auction.seller, abi.encode(listingInfo, swaprWallet.getNFT(activeBid.lock, activeBid.nftId))));
        require(_verify(activeBid.bidder, activeBidData));
    ...
    function _validateClaim(address claimant, bytes memory listingData, bytes memory bidData) internal view returns(bool isValid, string memory res) {
        ...
        require(_verify(auction.seller, listingData));
        require(_verify(lastBid.bidder, bidData));
        require(block.timestamp >= lastBid.listingEndTime);
        if(_verify(claimant, listingData)){
            //for seller
            uint bidderTotalBalance = swaprWallet.getBalance(lastBid.bidder, auction.acceptedToken);
            require(bidderTotalBalance - lastBid.lockedBalance >= lastBid.offerPrice);
            isValid = true;
        }else if(_verify(claimant, bidData)){
            //for bidder/buyer
            require(swaprWallet.isNFTLocked(auction.lock, auction.nftId));
            isValid = true;
        }else{
    ...
    function _bidsCrossValidate(Auction memory auction, Bid memory activeBid, Bid memory proposedBid) internal view returns(bool isValid, uint proposedEndTime) {
        require(activeBid.listingEndTime >= auction.endTime);
        require(activeBid.listingEndTime > block.timestamp);
        proposedEndTime = _getProposedEndTime(activeBid.listingEndTime);
        require(proposedBid.offerPrice >= auction.startingPrice);
        require(activeBid.offerPrice < proposedBid.offerPrice);
        isValid = true;
    }
