...
//    }
    ...
    /**
     * @dev Returns the current price for minting a token in Wei
     * @return The current price in Matic.
     */
    function price() external view returns (uint256) {
        uint256 priceInWei = usdToWei(usdPrice, 8);
        // console.log("current price is ", Util.toString(priceInWei));
        return priceInWei;
    }
    ...
    /**
     * Converts the given USD amount to Wei using the current price from Chainlink
     * @param usdAmount amount of USD to convert
     * @param usdDecimals decimals of USD amount
     */
    function usdToWei(uint256 usdAmount, uint8 usdDecimals) internal view returns (uint256) {
        ...
        uint256 weiAmount = usdAmount * scaleFactor / uint256(rate);

        // Round up the last two decimals
        uint256 roundedWeiAmount = weiAmount + 99;
        roundedWeiAmount = roundedWeiAmount / 100 * 100;

        return roundedWeiAmount;
    }
}
