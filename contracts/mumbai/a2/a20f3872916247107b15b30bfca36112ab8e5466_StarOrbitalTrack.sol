    ...
    IERC721ReceiverUpgradeable
{
    ...
    mapping(uint256 => uint256) private planetOrbitalTrackCooldownMap;

    // event
    event ContractSettingUpdated(
        address indexed _starAddress,
        address indexed _starDataAddress,
        address indexed _planetAddress,
    ...
    event DetachCooldownUpdated(uint256 _detachCooldown);

    event OrbitalTracksListed(
    ...
    event OrbitalTracksUnlisted(uint256 _starId, uint256[] _slotIndexArray);

    event PlanetAttachedOrbitalTrack(
        uint256 indexed _starId,
        uint256 indexed _planetId,
        uint256 indexed _rentalEndTime,
        uint256 _slotIndex
    );
    ...
    /**
     * @notice Determine if Nft is own by caller
     *
    ...
    modifier isNftOwner(
        ...
        require(
            _caller == IERC721Upgradeable(_nftAddress).ownerOf(_nftId),
            "NFT is not owned"
        );
        _;
    }
     ...
     */
    function updateDetachCooldown(uint256 _detachCooldown) external onlyOwner {
        detachCooldown = _detachCooldown;
        emit DetachCooldownUpdated(_detachCooldown);
    }
    ...
    function _checkNInitOrbitalTrackData(uint256 _starId) internal {
        ...
        starDataContract.checkNInitStarOrbitalTrackData(
            _starId,
            attributes[16]
        );
    }
    ...
    /**
     * @notice attach own planet on track
     *
     * @param _starId star ID
     * @param _planetId planet ID
     */
    function attachOwnPlanet(uint256 _starId, uint256 _planetId)
        external
        isNftOwner(starAddress, _starId, msg.sender)
        isNftOwner(planetAddress, _planetId, msg.sender)
    {
        // Init Orbital Track Data
        _checkNInitOrbitalTrackData(_starId);

        // get StarOrbitalTrackData
        StarOrbitalTrackData[] memory orbitalTrackData = starDataContract
    ...
    /**
     * @notice place track for renting
     *
     * @param _starId star ID
     * @param _numberOfTracks number Of Tracks
     * @param _duration rental days
     * @param _cost cost fee
     * @param _inviteAddress invited Address, 0 will be open rent
     */
    function placeOrbitalTrackRental(
        uint256 _starId,
        ...
        address _inviteAddress,
        bool _canBeRenew
    ) external isNftOwner(starAddress, _starId, msg.sender) {
        require(_numberOfTracks > 0 && _duration > 0, "invalid argument");
        ...
        // update orbital track data
        uint256 count = 0;
        uint256[] memory resultArray = new uint256[](_numberOfTracks);
        for (uint256 i = 0; i < orbitalTrackData.length; i++) {
            ...
            if (
                ...
                // setup starData
                orbitalTrackData[i].rentingState = OrbitalTrackState.Listing;
                orbitalTrackData[i].inviteAddress = _inviteAddress;
                orbitalTrackData[i].rentalFee = _cost;
                orbitalTrackData[i].rentalDuration = _duration;
                orbitalTrackData[i].canBeRenew = _canBeRenew;

                // update starData
                starDataContract.updateStarOrbitalTrackData(
                    _starId,
                    i,
                    orbitalTrackData[i]
                );
    ...
    /**
     * @notice cancel listing track
     *
     * @param _starId star ID
     * @param _slotIndexArray target unlist slot index array
     */
    function cancelOrbitalTrackListing(
        uint256 _starId,
        uint256[] memory _slotIndexArray
    ) external isNftOwner(starAddress, _starId, msg.sender) {
        require(_slotIndexArray.length > 0, "numberOfTracks should not be 0");
        ...
        // update orbital track data
        uint256 count = 0;
        for (uint256 i = 0; i < _slotIndexArray.length; i++) {
            require(
                _slotIndexArray[i] < orbitalTrackData.length &&
                    orbitalTrackData[_slotIndexArray[i]].rentingState ==
                    OrbitalTrackState.Listing,
                "invalid argument"
            );
            // reset orbital track data
            starDataContract.resetStarOrbitalTrackData(
                _starId,
                _slotIndexArray[i],
                0
            );
        ...
        // event
        emit OrbitalTracksUnlisted(_starId, _slotIndexArray);
    }
    ...
    function rentingOrbitalTrack(
        ...
        uint256 _planetId,
        uint256 _slotIndex
    ) external isNftOwner(planetAddress, _planetId, msg.sender) {
        StarOrbitalTrackData memory orbitalTrackData = starDataContract
        ...
        if (
            ...
            if (
                orbitalTrackData.inviteAddress == address(0) ||
                orbitalTrackData.inviteAddress == msg.sender
            ) {
                uint256 platformFee = (orbitalTrackData.rentalFee *
                    platformFeePercent) / percentMaxValue;
                ...
                _attachingPlanet(
                    ...
                    _planetId,
                    _slotIndex,
                    block.timestamp + (86400 * orbitalTrackData.rentalDuration)
                );
            } else {
                revert("invalid argument");
            }
    ...
    function starOwnerDetachPlanet(
        ...
        if (
            ...
            if (orbitalTrackData.planetOwnerAddress != msg.sender) {
                uint256 rentalFeeAfterPlatform = orbitalTrackData.rentalFee -
                    ((orbitalTrackData.rentalFee * platformFeePercent) /
                        percentMaxValue);
                ...
                if (block.timestamp >= orbitalTrackData.rentalEndTime) {
                    ...
                    uint256 earlyDetachAdjustFee = (rentalFeeAfterPlatform *
                        earlyDetachFeePercent) / percentMaxValue;
                    ...
                    if (earlyDetachAdjustFee > starOwnerReceive) {
                ...
                // send token from contract to star owner
                tokenContract.safeTransfer(msg.sender, starOwnerReceive);
                // send remind token from contract to planet owner
                if (rentalFeeAfterPlatform - starOwnerReceive > 0) {
                    tokenContract.safeTransfer(
                        orbitalTrackData.planetOwnerAddress,
                        rentalFeeAfterPlatform - starOwnerReceive
                    );
                }
            } else {
                // if star owner detach own, add detach cd on planet
                planetOrbitalTrackCooldownMap[_planetId] =
                    block.timestamp +
                    detachCooldown;
            }
        ...
        if (
            ...
            if (block.timestamp >= orbitalTrackData.rentalEndTime) {
                ...
                if (
                    ...
                    starOwnerReceive = starOwnerReceive + earlyDetachAdjustFee;
                }
            }
            // send token from contract to star owner
            tokenContract.safeTransfer(
                IERC721Upgradeable(starAddress).ownerOf(_starId),
                starOwnerReceive
            );
            // send remind token from contract to planet owner
            if (rentalFeeAfterPlatform - starOwnerReceive > 0) {
    ...
    function _attachingPlanet(
        ...
        if (
            ...
            revert("star or planet is blacklisted");
        }
        ...
        IERC721Upgradeable(planetAddress).safeTransferFrom(
            _planetOwner,
            address(this),
            _planetId
        );
    ...
    function _detachingPlanet(
        ...
        if (_haveDetachCooldown) {
            ...
            planetOrbitalTrackCooldownMap[_planetId] = cooldown;
        }
        starDataContract.resetStarOrbitalTrackData(
            _starId,
            _slotIndex,
        ...
        // emit event
        emit PlanetDetachedOrbitalTrack(_starId, _planetId, _slotIndex);
    }
...
interface IApeironStar {
    ...
    function getStarGodiverseByStarId(uint256 _tokenId)
        external
        view
        returns (StarMeta.StarGodiverseData memory);
