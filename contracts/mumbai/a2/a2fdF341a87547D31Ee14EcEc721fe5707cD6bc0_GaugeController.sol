...
import { IERC20, IRebaseToken, ERC20 } from "../tokens/ERC20.sol";
...
contract GaugeController is Ownable {
    ...
    uint256 public constant EPOCH_LENGTH = 5 minutes; // 14 days in prod
    ...
    function createGauge() external onlyOwner {
        require(veToken != address(0) && 
        rewardsToken != address(0), "GC: Zero address");
    ...
    function vote(address gaugeAddress) external {
        ...
        require(_currentEpoch().end > block.timestamp, "GC: Epoch ended");
        uint256 votePower = IveToken(veToken).balanceOf(msg.sender);

        require(votePower != 0, "GC: Zero votes");
    ...
    function rebalance(address reaper) external {
        ...
        IRebaseToken(rewardsToken).rebase();
        ...
        for (uint256 i = 0; i < gauges.length; i++) {
            ...
            if (totalWeights == 0) {
                _currentRelativeWeight[gaugeAddress] = 0;
            } else {
    ...
    function _distribute(address reaper) internal {
        require(_isInitialized, "GC: Not initialized");
        uint256 currentBalance = IERC20(rewardsToken).balanceOf(address(this));
        ...
        for (uint256 i = 0; i < gauges.length; i++) {
            ...
            ERC20(rewardsToken).safeTransfer(gaugeAddress, portion);
            _resetGauge(gaugeAddress, portion);
        }
        ERC20(rewardsToken).safeTransfer(reaper, reaperReward);
    }

    function _resetGauge(address gauge, uint256 portion) internal {
        uint256 pieSize = ERC20(rewardsToken).balanceOf(gauge);
...
import { IRebaseToken, IERC20, ERC20 } from "../tokens/ERC20.sol";
import { IveToken } from "../interfaces/IveToken.sol";
import { IGauge } from "../interfaces/IGauge.sol";
import { FixedPointMathLib } from "../lib/FixedPointMathLib.sol";
import { SafeTransferLib } from "../lib/SafeTransferLib.sol";
...
contract LiquidityGauge is IGauge {
    ...
    constructor(address rewards, address ve) {
    ...
    function recordVote(address account) external onlyController {
        ...
        uint256 votingPower = IveToken(_veToken).balanceOf(account, snapshot.timestamp);
    ...
    function claim() external {
        ...
        ERC20(_rewardToken).safeTransfer(account, portion);
    }
    ...
    function balance() external view returns (uint256 blnc) {
        blnc = IERC20(_rewardToken).balanceOf(address(this));
    }
