...
contract Page is ONFT1155Core, ERC1155, IONFT1155, IPage {
    ...
    uint16 public constant FUNCTION_TYPE_TURN = uint16(uint256(keccak256("TURN")));
    ...
    modifier onlyPrinter() {
        if (_msgSender() != address(pagePrinter)) revert Page_NotAllowed();
        _;
    }
     ...
     */
    function pageNumber(uint256 _tokenId) public view returns (uint256) {
     ...
     */
    function setPagePrinter(IPagePrinter _pagePrinter) public payable onlyOwner {
     ...
     */
    function closeTurn() public payable onlyOwner {
        turnClosed = true;
    }
     ...
     */
    function closeMint() public payable onlyOwner {
        mintClosed = true;
    }
    ...
    function mint(uint256 _amount) public payable {
        ...
        if (_amount * price != msg.value) revert Page_InsufficientValue();
     ...
     */
    function turnPage(uint256 _tokenId, uint256 _amount) public payable {
        ...
        _mint(_msgSender(), page + 1, _amount);
    }
    ...
    function _debitFrom(
        ...
        override
    {
        if (_msgSender() != _from && !isApprovedForAll(_from, _msgSender())) revert Page_NotAllowed();
        ...
        override
    {
        (
            ...
            uint[] memory amounts
        ) = abi.decode(_payload, (uint16, bytes, uint[], uint[]));
        address toAddress;
        assembly {
            toAddress := mload(add(toAddressBytes, 20))
        }
    ...
    function _bumpTokenIds(uint256[] memory _tokenIds) internal view returns (uint256[] memory) {
        for (uint256 i; i < _tokenIds.length;) {
            unchecked {
                _tokenIds[i] = pageNumber(_tokenIds[i]) + 1 + CHAIN_OFFSET;
                ++i;
            }
        }
