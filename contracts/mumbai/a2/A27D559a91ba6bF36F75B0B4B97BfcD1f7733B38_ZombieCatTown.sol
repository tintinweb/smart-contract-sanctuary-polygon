...
contract ZombieCatTown is ERC721Enumerable, Ownable {
	...
	uint private constant _PRESALE_MAX_SUPPLY = 2;
    uint public constant PRICE = 0.01 ether;
    uint public constant MAX_PER_MINT = 10;

    string public baseTokenURI;

    constructor(string memory baseURI) ERC721("ZombieCatTown", "ZCT") {
        setBaseURI(baseURI);
        _tokenIds.increment();
    }
    ...
    function mintNFTs(uint _count) public payable {
        ...
        if(totalMinted < _PRESALE_MAX_SUPPLY) {
            _preSale(2);
        } else {
            _mintNFTs(_count);
        }
    ...
    // Presale mints
    function _preSale(uint _count) private {
        uint totalMinted = _tokenIds.current();
        uint preSalePrice = 0.0 ether;
        uint preSaleMaxMint = 2;

        require(totalMinted.add(_count) <= (_PRESALE_MAX_SUPPLY + 1), "Presale is over!");
    ...
    function _mintNFTs(uint _count) private {
        ...
        require(totalMinted.add(_count) > (_PRESALE_MAX_SUPPLY + 1), "Presale's now!");
