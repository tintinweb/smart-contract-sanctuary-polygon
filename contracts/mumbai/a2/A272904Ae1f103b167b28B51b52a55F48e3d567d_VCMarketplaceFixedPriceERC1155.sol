...
abstract contract VCMarketplaceBase is FeeBeneficiary, Pausable {
    ...
    function _purchase(
        ...
        _transferFee(_tokenId, _seller, _listPrice, _marketFee, _starterFee, _poolFee);
...
contract FeeBeneficiary is FeeManager, AccessControl {
    ...
    function _transferFee(
        ...
        if (_starterFee > 0) {
            ...
            // this can be done at deployment, approve infinite amount
            currency.approve(starter, _starterFee);
            _fundProjects(feesData, _price);
        }
    }
    ...
    function _fundProjects(TokenFeesData memory _feesData, uint256 _listPrice) internal {
        for (uint256 i = 0; i < _feesData.projectFeesBps.length; i++) {
            ...
            if (amount > 0) {
                _fundProject(_feesData.projects[i], amount);
            }
        }
    }
     ...
     * @dev
     */
    // FIXME change projectId to projectAddress
    function _fundProject(address _project, uint256 _amount) internal {
        IVCStarter(starter).fundProjectOnBehalf(msg.sender, _project, currency, _amount);
    }
...
interface IVCStarter {
    ...
    function forceNotFundedCampaigns(address project, uint256[] calldata campaignIds) external;
    ...
    function createProject(address _lab, uint256 _amountToRaise) external returns (address);
    ...
    // function projectInfo(uint256 _projectId) external returns (Project calldata);
}
