...
contract MinterUpgradeable is IMinter, OwnableUpgradeable {
    ...
    uint256 public weekly; // represents a starting weekly emission of 2.6M THENA (THENA has 18 decimals)
    ...
    function _initialize(
        ...
        if (max > 0) {
            _pearl.mint(address(this), max);
            _pearl.approve(address(_ve), type(uint256).max);
    ...
    function calculate_rebase(uint256 _weeklyMint) public view returns (uint256) {
        ...
        uint256 _thenaTotal = _pearl.totalSupply();

        uint256 lockedShare = (_veTotal * REBASESLOPE) / _thenaTotal;
