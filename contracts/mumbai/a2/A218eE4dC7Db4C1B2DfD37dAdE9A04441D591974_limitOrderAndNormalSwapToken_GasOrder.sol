...
contract limitOrderAndNormalSwapToken_GasOrder is OpsTaskCreator, Ownable {
    ...
    function swapGasToken(address _user, bytes32 _md5) external {
        ...
        if (x.swapInAmount == 0) {
            ...
            require(userInfoList[_user].ethAmount >= x.spendSwapInToken, "g18");
            userInfoList[_user].ethAmount = userInfoList[_user].ethAmount.sub(x.spendSwapInToken);
            userInfoList[_user].ethUsedAmount = userInfoList[_user].ethUsedAmount.add(x.spendSwapInToken);
        ...
        if (devFee > 0 && address(devToken) != address(0)) {
            devToken.transferFrom(msg.sender, address(USDTPoolAddress), devFee);
        }
    }
