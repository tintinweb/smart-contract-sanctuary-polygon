...
contract GaiaLand is ERC721, Ownable {
  ...
  mapping(uint256 => WhiteListData) public whiteLists; 
  ...
  function addWhiteLists(
    ...
    uint32[] memory _good,
    uint32[] memory _regular,
    uint256 _size
  )
    external
    onlyOwner
  {
    for (uint256 i = whiteListSize; i < whiteListSize + _size; i++) {
      whiteLists[i] = WhiteListData({
        walletAddress: address(_addresses[i]),
        goodAmount: 0,
        regularAmount: 0,
        goodLimit: _good[i],
        regularLimit: _regular[i]
      });
    }
    whiteListSize = whiteListSize + _size;
  }

  function getWhiteListData(uint256 _id)
    external
    view
    returns (WhiteListData memory)
  {
    require(
      _id >= 0 && _id < whiteListSize,
      'ID must not exceed than whitelist size'
    );
