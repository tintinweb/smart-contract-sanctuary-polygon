...
contract BridgeUpgradeable is Initializable, OwnableUpgradeable, RouterCrossTalkUpgradeable {
    ...
    // tokenTicker => userAddress => index => TransferMapping
    mapping(string => mapping(uint256 => TransferMapping)) public transferMapping;
    // unique for each transfer mapping
    // tokenTicker => userAddress => index
    mapping(string => mapping(address => uint256)) public currentTransferIndex;
    ...
    function _isBridgeActive(string memory _tokenTicker) internal view {
        require(tokenBridgeRegistryUpgradeable.isBridgeActive() 
            && tokenBridgeRegistryUpgradeable.isTokenBridgeActive(_tokenTicker), "BRIDGE_DISABLED");
    }
    ...
    function getToken(string memory _tokenTicker) public view returns (TokenUpgradeable) {
        address tokenAddress = tokenBridgeRegistryUpgradeable.getTokenAddress(_tokenTicker);
    ...
    //     uint256 _index
    // ) public view returns (uint256) {
    //     LiquidityPosition memory position = liquidityPosition[_tokenTicker][_account][_index];
    //     require(position.depositedAmount > 0, "INVALID_POSITION");

    //     uint256 feeEarned;
        
    //     // user still in starting epoch
    //     if(epochs[_tokenTicker].length == position.epochStartIndex - 1) {
    //         uint256 totalFeeCollected = feePoolUpgradeable.totalFees(_tokenTicker);
    //         uint256 totalActiveLiquidity = totalLiquidity[_tokenTicker];
            
    //         uint256 liquidityBlocks = block.number - position.blockNo;
    //         uint256 epochLength = tokenBridgeRegistryUpgradeable.getEpochLength(_tokenTicker);
    //         feeEarned = (position.depositedAmount * liquidityBlocks * totalFeeCollected) / (totalActiveLiquidity * epochLength);
    //         if(hasBooster[_tokenTicker][_account][_index][epochs[_tokenTicker].length + 1]) {
    ...
    //     if(position.epochStartIndex - 1 == position.claimedTillEpochIndex 
    //         && epochs[_tokenTicker].length > position.epochStartIndex - 1
    //     ) {
    //         Epoch memory epoch = epochs[_tokenTicker][position.epochStartIndex-1];
    //         uint256 liquidityBlocks = position.epochStartBlock + epoch.epochLength - position.blockNo;
    //         feeEarned = (position.depositedAmount * liquidityBlocks * epoch.totalFeesCollected) / (epoch.totalActiveLiquidity * epoch.epochLength);
    //         if(hasBooster[_tokenTicker][_account][_index][position.epochStartIndex]) {
    //             feeEarned = feeEarned * 3 / 2;
    //         }
    //         position.claimedTillEpochIndex += 1;
    //     }

    //     // fees for all the completed epochs
    //     for (uint256 liqIndex = position.claimedTillEpochIndex + 1; liqIndex <= epochs[_tokenTicker].length; liqIndex++) {
    //         Epoch memory epoch = epochs[_tokenTicker][liqIndex-1];
    //         feeEarned += (position.depositedAmount * epoch.totalFeesCollected / epoch.totalActiveLiquidity);
    //         if(hasBooster[_tokenTicker][_account][_index][liqIndex]) {
    //             feeEarned = feeEarned * 3 / 2;
    //         }
    //         position.claimedTillEpochIndex += 1;
    //     }
    ...
    //     if(epochs[_tokenTicker].length == position.claimedTillEpochIndex) {
    //         uint256 totalFeeCollected = feePoolUpgradeable.totalFees(_tokenTicker);
    //         uint256 totalActiveLiquidity = totalLiquidity[_tokenTicker];

    //         Epoch memory epoch = epochs[_tokenTicker][position.claimedTillEpochIndex];
    //         uint256 liquidityBlocks = block.number - (epoch.startBlock + epoch.epochLength);
    //         uint256 epochLength = tokenBridgeRegistryUpgradeable.getEpochLength(_tokenTicker);
    //         feeEarned += (position.depositedAmount * liquidityBlocks * totalFeeCollected) / (totalActiveLiquidity * epochLength);
    //         if(hasBooster[_tokenTicker][_account][_index][epochs[_tokenTicker].length + 1]) {
    //             feeEarned = feeEarned * 3 / 2;
    ...
    //     LiquidityPosition storage position = liquidityPosition[_tokenTicker][_account][_index];
    //     require(position.depositedAmount > 0, "INVALID_POSITION");

    //     uint256 startEpoch = position.claimedTillEpochIndex + 1;
    //     uint256 feeEarned = getUserConfirmedRewards(_tokenTicker, _account, _index);
    //     require(feeEarned > 0, "NO_REWARD");
    //     uint256 endEpoch = position.claimedTillEpochIndex;
    ...
    //     uint8 feeType;
    //     uint256 feeInBips;
    //     (feeType, feeInBips) = tokenBridgeRegistryUpgradeable.getFeeTypeAndFeeInBips(_tokenTicker);
    ...
    function buyBoosterPacks(
        ...
        if(position.boosterEndEpochIndex >= currentEpochIndex) {
            addBoosterMapping(_tokenTicker, _index, _quantity, position.boosterEndEpochIndex + 1);
            position.boosterEndEpochIndex += _quantity;
        }
        else {
            addBoosterMapping(_tokenTicker, _index, _quantity, currentEpochIndex);
    ...
    function calculatePassedEpoch(string memory _tokenTicker) internal returns (uint256) {
        ...
        if(block.number > nextEpochBlock[_tokenTicker]) {
            ...
            if(epochs[_tokenTicker].length > 0) {
                ...
                uint256 nextEpochStartBlock = epoch.startBlock + epoch.epochLength;
                passedEpochs = (block.number - nextEpochStartBlock) / epochLength;
                nextEpochBlock[_tokenTicker] += (passedEpochs + 1) * epochLength;
            } else {
    ...
    function addPassedEpochs(
        ...
        uint256 passedEpochs = calculatePassedEpoch(_tokenTicker);
        if(passedEpochs == 0)
            return;

        uint256 epochLength;
        (, epochLength) = tokenBridgeRegistryUpgradeable.getStartBlockAndEpochLength(_tokenTicker);
        
        uint256 totalFeesCollected = feePoolUpgradeable.totalFees(_tokenTicker);
        uint256 totalActiveLiquidity = totalLiquidity[_tokenTicker];

        uint256 noOfDepositors = tokenBridgeRegistryUpgradeable.getNoOfDepositors(_tokenTicker);
        // (, , , epochLength, , totalFeesCollected, totalActiveLiquidity, ) = tokenBridgeRegistryUpgradeable.tokenBridge(_tokenTicker);

        for (uint256 index = 0; index < passedEpochs; index++) {
            Epoch memory epoch = Epoch({
                startBlock: nextEpochBlock[_tokenTicker] + 1,
    ...
    function addLiquidity(
        ...
        if(epochs[_tokenTicker].length > 0) {
            ...
            (epochStartBlock, ) = tokenBridgeRegistryUpgradeable.getStartBlockAndEpochLength(_tokenTicker);
        }
    ...
    function withdrawLiquidity(
        ...
        uint256 noOfTokens = liquidityPosition[_tokenTicker][_msgSender()][_index].depositedAmount;
    ...
    function transferIn(
        ...
        uint8 bridgeType = tokenBridgeRegistryUpgradeable.getBridgeType(_tokenTicker);
        if(bridgeType == 0) {
            totalLiquidity[_tokenTicker] += _noOfTokens;
        }
        if(bridgeType == 1) {
        ...
        return crossChainTransferOut(_tokenTicker, _noOfTokens, _toChainId);
    }
    ...
    function crossChainTransferOut(
        ...
        uint256 transferIndex = currentTransferIndex[_tokenTicker][_msgSender()];
        ++currentTransferIndex[_tokenTicker][_msgSender()];
        bytes memory data = abi.encode(_msgSender(), _noOfTokens, _tokenTicker, transferIndex);
        bytes4 _interface = bytes4(keccak256("createCrossChainTransferMapping(address,uint256,string,uint256)"));
        // ChainID - Selector - Data - Gas Usage
        bool success = routerSend(_toChainId, _interface, data, crossChainGas );
    ...
    {
        (address userAddress, uint256 noOfTokens, string memory tokenTicker, uint256 transferIndex) = abi.decode(_data, (address, uint256, string, uint256));
        (bool success, bytes memory returnData) = 
            address(this).call( abi.encodeWithSelector(_interface, userAddress, noOfTokens, tokenTicker, transferIndex));
    ...
    function createCrossChainTransferMapping(
        ...
        require(_transferIndex == currentTransferIndex[_tokenTicker][_userAddress], "INVALID_TRANSFER_INDEX");
        ...
        transferMapping[_tokenTicker][currentTransferIndex[_tokenTicker][_userAddress]++] = transfer;
    }
    ...
    function crossChainTransferIn(
        ...
        TokenUpgradeable setuToken = getToken(concatenate("SETU", _tokenTicker));
        setuToken.mintTokens(_userAddress, _noOfTokens);
        ...
        transferOut(_userAddress, _noOfTokens, _tokenTicker);
    }
    ...
    function transferOut(
        ...
        if(bridgeType == 0) {
            ...
            setuToken.burnTokens(_userAddress, noOfTokens);
            totalLiquidity[_tokenTicker] -= (noOfTokens - feesDeducted);
            token.transferFrom(address(this), _userAddress, noOfTokens - feesDeducted);
        } 
        else if(bridgeType == 1) {
            uint256 feesDeducted = calculateBridgingFees(_tokenTicker, _noOfTokens);
            setuToken.burnTokens(_userAddress, _noOfTokens);
            totalLiquidity[_tokenTicker] -= (noOfTokens - feesDeducted);
            // transfer the available tokens from the liquidity pool
            token.transfer(_userAddress, noOfTokens - feesDeducted);
            // mint the remaining tokens
            if(_noOfTokens != noOfTokens) {
                token.mintTokens(_userAddress, _noOfTokens - noOfTokens);
            }
        }
    }
    ...
    function safeWithdrawLiquidity(
        ...
        uint256 liquidity = totalLiquidity[_tokenTicker];
        require(_noOfTokens <= liquidity, "AMOUNT_OVERFLOW");
        liquidity -= _noOfTokens;
    ...
    //     // adding any passed epochs
    //     addPassedEpochs(_tokenTicker);
    //     uint256 adminLastClaimedEpoch = adminClaimedTillEpoch[_tokenTicker];

    //     uint256 adminFees;
    //     for (uint256 epochIndex = adminLastClaimedEpoch + 1; epochIndex < epochs[_tokenTicker].length; epochIndex++) {
    //         Epoch memory epoch = epochs[_tokenTicker][epochIndex];
    //         uint256 noOfDepositors = tokenBridgeRegistryUpgradeable.getNoOfDepositors(_tokenTicker);
    //         uint256 perUserFee = epoch.totalFeesCollected/ (2 * noOfDepositors);
            
    //         uint256 normalFees = (noOfDepositors - totalBoostedUsers[_tokenTicker][epochIndex]) * perUserFee;
    //         uint256 extraBoostedFees = totalBoostedUsers[_tokenTicker][epochIndex] * perUserFee * 3 / 2;
    //         adminFees += epoch.totalFeesCollected - normalFees - extraBoostedFees;
    //     }
    ...
    //     adminClaimedTillEpoch[_tokenTicker] = epochs[_tokenTicker].length;

    //     uint8 feeType;
    //     (feeType, ) = tokenBridgeRegistryUpgradeable.getFeeTypeAndFeeInBips(_tokenTicker);
    ...
    function getEpochTotalFees(
        ...
        return epochs[_tokenTicker][_epochIndex].totalFeesCollected;
    } 
...
contract TokenBridgeRegistryUpgradeable is Initializable, OwnableUpgradeable {
    ...
    function deployChildToken(
        ...
        TransparentUpgradeableProxy transparentUpgradeableProxy = new TransparentUpgradeableProxy(address(newChildToken), address(proxyAdmin), "");
...
contract FeePoolUpgradeable is Initializable, OwnableUpgradeable {
    ...
    function transferLpFee(
        ...
        if(feeType == 0) {
            ...
            token.transferFrom(address(this), _account, feeShare);
        }
    ...
    function getUserUnconfirmedRewards(
        ...
        if(epochsLength == epochStartIndex - 1) {
            // uint256 totalFeeCollected = totalFees[_tokenTicker];
            // uint256 totalActiveLiquidity = bridgeUpgradeable.totalLiquidity(_tokenTicker);
            
            // uint256 liquidityBlocks = block.number - blockNo;
            // uint256 epochLength = tokenBridgeRegistryUpgradeable.getEpochLength(_tokenTicker);
            // feeEarned = (depositedAmount * liquidityBlocks * totalFeeCollected) / (totalActiveLiquidity * epochLength);
        ...
        if(epochStartIndex - 1 == claimedTillEpochIndex 
            ...
            // ) = bridgeUpgradeable.epochs(_tokenTicker, epochStartIndex-1);
            // uint256 liquidityBlocks = epochStartBlock + epochLength - blockNo;
            // feeEarned = (depositedAmount * liquidityBlocks * totalFeesCollected) / (totalActiveLiquidity * epochLength);
            // if(bridgeUpgradeable.hasBooster(_tokenTicker, _account, _index, epochStartIndex)) {
            //     feeEarned = feeEarned * 3 / 2;
            // }
        ...
        for (uint256 liqIndex = claimedTillEpochIndex + 1; liqIndex <= epochsLength; liqIndex++) {
            ...
            // epochFee = (depositedAmount * totalFeesCollected / totalActiveLiquidity);
        ...
        if(epochsLength == claimedTillEpochIndex) {
            ...
            // ) = bridgeUpgradeable.epochs(_tokenTicker, claimedTillEpochIndex);
            // uint256 liquidityBlocks = block.number - (startBlock + epochLength);
            // uint256 fixedEpochLength = tokenBridgeRegistryUpgradeable.getEpochLength(_tokenTicker);
            // feeEarned += (depositedAmount * liquidityBlocks * totalFeeCollected) / (totalActiveLiquidity * fixedEpochLength);
            // if(bridgeUpgradeable.hasBooster(_tokenTicker, _account, _index, epochsLength + 1)) {
            //     feeEarned = feeEarned * 3 / 2;
            // }
    ...
    function getUnconfirmedStartEpochFee(
        ...
        uint256 liquidityBlocks = block.number - blockNo;
        uint256 epochLength = tokenBridgeRegistryUpgradeable.getEpochLength(_tokenTicker);
        uint256 feeEarned = (depositedAmount * liquidityBlocks * totalFeeCollected) / (totalActiveLiquidity * epochLength);
        if(bridgeUpgradeable.hasBooster(_tokenTicker, _account, _index, epochsLength + 1)) {
            feeEarned = feeEarned * 3 / 2;
        }
        return feeEarned;
    ...
    function getConfirmedStartEpochFee(
        ...
        uint256 feeEarned = (params.depositedAmount * liquidityBlocks * totalFeesCollected) / (totalActiveLiquidity * epochLength);
    ...
    function getConfirmedEpochsFee(
        ...
        uint256 epochFee = (depositedAmount * totalFeesCollected / totalActiveLiquidity);
        if(bridgeUpgradeable.hasBooster(_tokenTicker, _account, _index, liqIndex)) {
            epochFee = epochFee * 3 / 2;
        }
        return epochFee;
    ...
    function getUnconfirmedCurrentEpochFee(
        ...
        uint256 liquidityBlocks = block.number - (startBlock + epochLength);
        uint256 fixedEpochLength = tokenBridgeRegistryUpgradeable.getEpochLength(_tokenTicker);
        uint256 feeEarned = (depositedAmount * liquidityBlocks * totalFeeCollected) / (totalActiveLiquidity * fixedEpochLength);
        if(bridgeUpgradeable.hasBooster(_tokenTicker, _account, _index, epochsLength + 1)) {
            feeEarned = feeEarned * 3 / 2;
        }
        return feeEarned;
    ...
    function claimFeeShare(
        ...
        uint256 feeEarned = getUserConfirmedRewards(_tokenTicker, _account, _index);
        require(feeEarned > 0, "NO_REWARD");
        uint256 endEpoch = claimedTillEpochIndex;
        for (uint256 epochIndex = startEpoch; epochIndex <= endEpoch; epochIndex++) {
    ...
    function getAdminTokenFees(
        ...
        for (uint256 epochIndex = adminClaimedTillEpoch[_tokenTicker] + 1; epochIndex < epochsLength; epochIndex++) {
            uint256 epochTotalFees = bridgeUpgradeable.getEpochTotalFees(_tokenTicker, epochIndex);
...
abstract contract RouterCrossTalkUpgradeable is Initializable , ContextUpgradeable , iRouterCrossTalkUpgradeable, ERC165Upgradeable {
    ...
    modifier isHandler(){
        require(_msgSender() == address(handler) , "RouterSync : Only GenericHandler can call this function" );
        _;
    }

    modifier isLinkSet(uint8 _chainID){
        require(Chain2Addr[_chainID] == address(0) , "RouterSync : Cross Chain Contract to Chain ID set" );
        _;
    }
    ...
    modifier isLinkSync( uint8 _srcChainID, address _srcAddress ){
        require(Chain2Addr[_srcChainID] == _srcAddress , "RouterSync : Source Address Not linked" );
        _;
    }
    ...
    function __RouterCrossTalkUpgradeable_init( address _handler ) internal initializer {
...
contract TokenUpgradeable is Initializable, ERC20Upgradeable, ERC20BurnableUpgradeable, OwnableUpgradeable, PausableUpgradeable {
