...
interface IOttoItemERC1155 is IOttoItem, IERC1155 {}
    ...
    IOttoV3
{
    ...
    uint256 public decayTarget;
    IOttoItemERC1155 ITEM;
    mapping(uint256 => OwnedInfo) private _ownedItems;
    // STR, DEF, DEX, INT, LUK, CON, CUTE, BRS
    mapping(uint256 => int16[8]) private _baseAttributes;
    ...
    modifier onlyOttoOwner(uint256 tokenId_) {
        require(_msgSender() == ownerOf(tokenId_), 'caller must be otto owner');
        _;
    }
    ...
    modifier onlyItemOwner(uint256 itemId_) {
        require(
            ITEM.balanceOf(_msgSender(), itemId_) > 0,
    ...
    function upgradeV3(address item_) public onlyAdmin {
    ...
    function setDecayTarget(uint256 ts_) external onlyAdmin {
        decayTarget = ts_;
    }
    ...
    function setTraitCode(
        ...
        uint8 slot_,
        uint16 code_
    ) external virtual override onlyManager validOttoId(tokenId_) {
        uint16[16] memory arr_ = traitsOf(tokenId_);
        arr_[slot_] = code_;
        infos[tokenId_].traits = toNumericTraits(arr_);
        emit TraitsChanged(tokenId_, traitsOf(tokenId_));
    }

    function _decayedCanOpenAt() private view returns (uint256) {
        uint256 peroid_ = 0;
        if (block.timestamp <= decayTarget) {
            peroid_ = decayTarget - block.timestamp;
        }
        return block.timestamp + peroid_;
    }
    ...
    function devSummon(
        ...
        infos[tokenId_].legendary = legendary_;
        emit SummonOtto(tx.origin, tokenId_, legendary_);
        emit TraitsChanged(tokenId_, traitsOf(tokenId_));
    }

    function devClose(uint256 tokenId_)
        external
        onlyManager
        validOttoId(tokenId_)
    {
        infos[tokenId_].portalStatus = PortalStatus.UNOPENED;
        infos[tokenId_].traits = 0;
        infos[tokenId_].birthday = 0;
        infos[tokenId_].summonAt = 0;
        infos[tokenId_].legendary = false;
        emit TraitsChanged(tokenId_, traitsOf(tokenId_));
    }
    ...
    function _use(uint256 ottoId_, uint256 itemId_) internal {
        int16[8] memory attrs_ = ITEM.attributesWithSideEffect(itemId_);
        for (uint8 i = 0; i < 8; i++) {
            _baseAttributes[ottoId_][i] += attrs_[i];
        }
        emit ItemUsed(ottoId_, itemId_);
        emit BaseAttributesChanged(ottoId_, _baseAttributes[ottoId_]);
    }
    ...
    function _equip(
        ...
        require(ITEM.wearable(itemId_), 'item is not wearable');

        OwnedInfo storage owned = _ownedItems[ottoId_];
        (uint8 slot_, ) = ITEM.decode(itemId_);
        if (owned.equipped[slot_]) {
            _takeoff(ottoId_, owned.itemId[slot_], from_);
        }
        owned.equipped[slot_] = true;
        owned.itemId[slot_] = itemId_;
        emit ItemEquipped(ottoId_, itemId_);
        emit TraitsChanged(ottoId_, traitsOf(ottoId_));
    }
    ...
    function _takeoff(
        ...
        OwnedInfo storage owned = _ownedItems[ottoId_];
        (uint8 slot_, ) = ITEM.decode(itemId_);
        require(owned.equipped[slot_], 'item is not equipped');

        ITEM.safeTransferFrom(address(this), to_, itemId_, 1, '');
        owned.equipped[slot_] = false;
        owned.itemId[slot_] = 0;
        emit ItemTookoff(ottoId_, itemId_);
        emit TraitsChanged(ottoId_, traitsOf(ottoId_));
    }

    /// @notice only for takeoff currently
    function transferChild(
        ...
        virtual
        override
        validOttoId(_fromTokenId)
        onlyOttoOwner(_fromTokenId)
    {
        require(
            _childContract == address(ITEM),
            '_childContract must be item contract'
        );
        _takeoff(_fromTokenId, _childTokenId, _to);
    }
    ...
    {
        OwnedInfo storage owned = _ownedItems[tokenId_];
        uint16[16] memory arr_ = rawTraitsOf(tokenId_);
        for (uint16 i = 0; i < 16; i++) {
            if (owned.equipped[i]) {
                (, uint16 code_) = ITEM.decode(owned.itemId[i]);
                arr_[i] = code_;
            }
        }
        return arr_;
    ...
    function numericTraitsOf(uint256 tokenId_)
        public
        view
        virtual
        override
    ...
    {
        return _baseAttributes[tokenId_];
    }
        ...
        returns (uint256[] memory)
    {
        ...
        uint8 index_ = 0;
        uint256[] memory itemIds_ = new uint256[](count_);
        for (uint8 i = 0; i < 16; i++) {
            if (owned.equipped[i]) {
                uint256 itemId_ = owned.itemId[i];
                itemIds_[index_] = owned.itemId[i];
                index_++;
            }
        }
        return itemIds_;
    ...
    function canOpenAt(uint256 tokenId_)
        ...
        returns (uint256)
    {
        return infos[tokenId_].canOpenAt;
    }
    ...
    function onERC1155Received(
        ...
        if (_msgSender() == address(ITEM)) {
            require(value_ == 1, 'received item value must be 1');
            uint256 ottoId_ = abi.decode(_data, (uint256));
            if (ITEM.consumable(itemId_)) {
                _use(ottoId_, itemId_);
            } else {
                _equip(ottoId_, itemId_, from_);
            }
        }
...
interface IOttoItem {
    ...
    function setAttribute(
        uint256 tokenId_,
        int16[8] calldata value_,
        uint16[8] calldata percentage_
    ) external;
    ...
    function attributesOf(uint256 tokenId_)
        external
        view
        returns (int16[8] memory, uint16[8] memory);

    function attributesWithSideEffect(uint256 tokenId_)
        external
        view
        returns (int16[8] memory);
    ...
    IERC721EnumerableUpgradeable
{
    ...
    function initialize(
        ...
        require(
            collectionSize_ > 0,
            'ERC721AUpgradeable: collection must have a nonzero supply'
        );
        require(
            maxBatchSize_ > 0,
            'ERC721AUpgradeable: max batch size must be nonzero'
        );
        _name = name_;
        _symbol = symbol_;
        ...
        returns (uint256)
    {
        require(
            index < totalSupply(),
            'ERC721AUpgradeable: global index out of bounds'
        );
        return index;
    }
    ...
    function _numberMinted(address owner) internal view returns (uint256) {
        require(
            owner != address(0),
            'ERC721AUpgradeable: number minted query for the zero address'
        );
        ...
        returns (TokenOwnership memory)
    {
        require(
            _exists(tokenId),
            'ERC721AUpgradeable: owner query for nonexistent token'
        );
        ...
        returns (address)
    {
        require(
            _exists(tokenId),
            'ERC721AUpgradeable: approved query for nonexistent token'
        );
    ...
    function _safeMint(
        ...
        require(
            ...
            'ERC721AUpgradeable: token already minted'
        );
        require(
    ...
    function _transfer(
        ...
        require(
            isApprovedOrOwner,
            'ERC721AUpgradeable: transfer caller is not owner nor approved'
        );

        require(
            prevOwnership.addr == from,
            'ERC721AUpgradeable: transfer from incorrect owner'
        );
        require(
            to != address(0),
            'ERC721AUpgradeable: transfer to the zero address'
        );
