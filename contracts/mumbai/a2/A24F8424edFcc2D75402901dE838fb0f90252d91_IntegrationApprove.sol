...
contract IntegrationApprove {
    ...
    /// @dev Get Status of Confirmation.
    /// @param _id ID of Integration Approve.
    /// @return status Status of Integration Approve.
    ...
    returns (bool) {
        ...
        if( block.number >= integrationsApprove[integrationIndexMap[_id]].block_start && block.timestamp <=integrationsApprove[integrationIndexMap[_id]].block_end )
        {
        ...
        {
            if( block.number < integrationsApprove[integrationIndexMap[_id]].block_end && block.timestamp < integrationsApprove[integrationIndexMap[_id]].block_start)
            {
    ...
    /// @dev Validate if all Groups is confirmed.
    /// @param _id ID of Integration Approve.
