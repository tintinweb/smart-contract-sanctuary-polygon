...
/******************************************************************************\
/// @title Sipher Portal - Token Management Facet
/// @author Ather Labs
/// @dev This facet provides functions to register and manage supported assets in the portal.
/******************************************************************************/
...
contract TokenManagementFacet {
    ...
    /* ------ EVENTS ------ */
    /// @dev Event emitted when pause status of a token is updated.
    event TokenPausedUpdated(uint8 lisitingId, bool paused);

    /// @dev Event emitted when a new token is listed on the portal.
    event NewToken(
    ...
    function setTokenPaused(uint8 _listingId, bool _tokenPaused) external {
        ...
        if (s.pausedTokens[_listingId] != _tokenPaused) {
            ...
            emit TokenPausedUpdated(_listingId, _tokenPaused);
        }
    }

    /// @notice Add token to the list of registered tokens.
    /// @dev Only Governor can call this function.
    /// @dev Make sure every ERC1155 token supports the IERC1155M interface.
    /// @dev New token type is not equal to 0.
    /// @dev New token lisitingId has not existed yet.
    /// @param _tokenAddress Token address.
    /// @param _tokenType Token type in number.
    /// @param _minLiquidity the balance minimum limit of this token, 0 mean no limit.
    /// @param _newListingId Token's listing Id.
    /// @param _newListingId The minimum amount of tokens required in a single deposit Tx. Only applies to Native & ERC20 tokens
    /*
    ...
    function addToken(
        ...
        s.config.minLiquidity[_newListingId] = _minLiquidity;
        s.config.minDeposit[_newListingId] = _minDeposit;
        emit NewToken(_tokenAddress, _tokenType, _newListingId, _minLiquidity, _minDeposit);
    }

    /// @notice Validate if token address is valid, except for native token which has listingId = 0 by default.
    /// @param _tokenAddr Token address.
    /// @return uint8 Token's listingId.
    function validateTokenAddress(address _tokenAddr) external view returns (uint8) {
        uint8 listingId = LibTokenManagement._validateTokenAddress(_tokenAddr);
        return listingId;
    }

    /// @notice Validate if listingId is valid, except for native token which has listingId = 0 by default.
    /// @param _listingId Token's listingId.
    /// @return tokenAddr Address of the valid listingId.
