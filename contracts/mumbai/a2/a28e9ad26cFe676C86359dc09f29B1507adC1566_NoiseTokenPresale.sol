// contracts/NoiseTokenPresale.sol
...
contract NoiseTokenPresale is Context, ReentrancyGuard, Ownable {
    ...
    uint256 public HARD_CAP = 50 ether; //@change for testing 

    uint256 constant NOISE_PER_MATIC_PRESALE = 100;
    uint256 constant NOISE_PER_MATIC_QUICKSWAP = 70;

    uint256 public QUICK_LP_MATIC = 25 ether;
    uint256 public QUICK_LP_NOISE;
    ...
    address NOISE_TOKEN_ADDRESS = 0x29dB524eA8aF45c2A683833e227fe8a880825591; //NOISE Token address

    IUniswapV2Router02 constant QUICKSWAP_V2_ADDRESS =  IUniswapV2Router02(0x8954AfA98594b838bda56FE4C12a09D7739D179b);
    ...
    address public ERC20_quickswapV2Pair; //Uniswap Pair address

    TokenTimelock public QuickLPTimeLock;
    ...
    bool public quickPairCreated = false;
    ...
    uint256 public maticSent; //MATIC Received
    
    uint256 public lockedLiquidityAmount;
    uint256 public refundTime; 

    mapping(address => uint) maticSpent;
    ...
    constructor() {
        ...
        QUICK_LP_NOISE = QUICK_LP_MATIC.mul(NOISE_PER_MATIC_QUICKSWAP);
    ...
    function buyTokens() public payable nonReentrant {
        ...
        require(msg.value >= MIN_CONTRIBUTION, "Less than 1 MATIC");
        require(msg.value <= MAX_CONTRIBUTION, "More than 10 MATIC");
        require(maticSent < HARD_CAP, "Hardcap reached");        
        require(msg.value.add(maticSent) <= HARD_CAP, "Hardcap will reached");
        require(maticSpent[_msgSender()].add(msg.value) <= MAX_CONTRIBUTION, "> 10 MATIC");

        require(!isStopped, "Presale stopped"); //@todo

        
        uint256 tokens = msg.value.mul(NOISE_PER_MATIC_PRESALE);
        require(NOISE_ERC20.balanceOf(address(this)) >= tokens, "Not enough tokens"); //@tod

        if(maticSpent[_msgSender()] == 0) {
            contributors.push(_msgSender()); //Create list of contributors    
        }
        
        maticSpent[_msgSender()] = maticSpent[_msgSender()].add(msg.value);

        tokensBought = tokensBought.add(tokens);
        maticSent = maticSent.add(msg.value);
    ...
    //@done, create quickpair first. 
    function createQuickPair() external onlyOwner {
        require(!liquidityAdded, "liquidity Already added");
        require(!quickPairCreated, "Already Created Quickpair");

        ERC20_quickswapV2Pair = quickswapFactory.createPair(address(NOISE_ERC20), QUICKSWAP_V2_ADDRESS.WETH());

        quickPairCreated = true;
    }
    ...
    function addLiquidity() external onlyOwner {
        require(!liquidityAdded, "liquidity Already added");
        require(maticSent >= HARD_CAP, "Hard cap not reached");   
        require(quickPairCreated, "Quickswap pair not created");


        NOISE_ERC20.approve(address(QUICKSWAP_V2_ADDRESS), QUICK_LP_NOISE);
        
        QUICKSWAP_V2_ADDRESS.addLiquidityETH{ value: QUICK_LP_MATIC } (
            address(NOISE_ERC20),
            QUICK_LP_NOISE,
            QUICK_LP_NOISE,
            QUICK_LP_MATIC,
    ...
    function lockLiquidity() external onlyOwner {
        ...
        //Lock the Liquidity 
        IERC20 liquidityTokens = IERC20(ERC20_quickswapV2Pair); //Get the QUICK LP token
        if(liquidityUnlockTime <= block.timestamp) {
            liquidityUnlockTime = block.timestamp.add(DURATION_LIQUIDITY_LOCK);
        }
        QuickLPTimeLock = new TokenTimelock(liquidityTokens, contract_owner, liquidityUnlockTime);
    ...
    function unlockLiquidity() external onlyOwner  {      
        QuickLPTimeLock.release();
    }

    //Check when Quickswap V2 tokens are unlocked
    function unlockLiquidityTime() external view returns(uint256) {      
        return QuickLPTimeLock.releaseTime();
    }    

    /*
    //NOISE can be claim by investors after sale success, It is optional 
    //@done
    function claimNoiseRoundOne() external nonReentrant {
        require(liquidityAdded,"NoiseCrowdsale: can only claim after listing in QUICK");  
        require(noiseHolding[_msgSender()] > 0, "NoiseCrowdsale: No NOISE token available for this address to claim");       
        uint256 tokenAmount_ = noiseBought[_msgSender()];

        tokenAmount_ = tokenAmount_.mul(UNLOCK_PERCENT_PRESALE_INITIAL).div(100);
        noiseHolding[_msgSender()] = noiseHolding[_msgSender()].sub(tokenAmount_);

        // Transfer the $NOISE to the beneficiary
        NOISE_ERC20.safeTransfer(_msgSender(), tokenAmount_);
        tokensWithdrawn = tokensWithdrawn.add(tokenAmount_);
    }

    //30% of NOISE can be claim by investors after 30 days from quicklisting
    //@done

    function claimNoiseRoundTwo() external nonReentrant {
        require(liquidityAdded,"Claimble after QUICK list");  
        require(noiseHolding[_msgSender()] > 0, "0 QUICK");
    ...
    //20% of NOISE can be claim by investors after 20 days from quicklisting
    //@done
    function claimNoiseRoundThree() external nonReentrant {
        require(liquidityAdded,"Claimble after QUICK list");  
        require(noiseHolding[_msgSender()] > 0, "0 NOISE");
    ...
    */
    ...
    function distributeTokensRoundOne() external onlyOwner {
        require(liquidityAdded, "Add QUICK Liquidity");        
    ...
    //@done
    //Withdraw the collected remaining matic
    function withdrawMatic(uint amount) external onlyOwner returns(bool){
        require(liquidityAdded,"After QUICK LP");        
    ...
    //@done
    function userMaticContribution(address user) external view returns (uint256) {
        return maticSpent[user];
    }    
    ...
    function getRefund() external nonReentrant {
        ...
        require(maticSent < HARD_CAP && block.timestamp >= refundTime, "Cannot refund");
        uint256 amount = maticSpent[_msgSender()];
    ...
    function bulkRefund() external nonReentrant {
        ...
        for (uint i=0; i<contributors.length; i++) {
            ...
            uint256 amount = maticSpent[user];
            if(amount > 0) {
                maticSpent[user] = 0;
                noiseBought[user] = 0;
                noiseHolding[user] = 0;                
