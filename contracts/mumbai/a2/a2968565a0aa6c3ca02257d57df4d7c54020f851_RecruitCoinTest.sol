...
library Math {
    ...
    function sqrt(uint256 a) internal pure returns (uint256) {
        ...
        // This gives `2**k < a <= 2**(k+1)` ΓåÆ `2**(k/2) <= sqrt(a) < 2 ** (k/2+1)`.
...
contract RecruitCoinTest is ERC20, Ownable, ReentrancyGuard {
    ...
    uint256[9] public supplies = [
        ...
        250_000e18, // SOFT_CAP
        125_000_0e18, // HARD_CAP
        125_000_00e18, // 5% PUBLIC_SALE
        250_000_00e18, // 10% PRE_SALE
        575_000_00e18, // 23% EARLY
        150_000_000e18, // 60% PUBLIC_UNCAPPED
        0, // total payment is dollar (stable coin) raised.
        500_000_0e18 // Founder Allocation
    ];
    uint256[2] public prices = [0.1 ether, 0.02 ether];
    uint256[4] public dates = [
        getCurrentTime(), // preSaleStart
        getCurrentTime().add(30 minutes), // preSaleEnd
        getCurrentTime().add(30 minutes), // publicSaleStart
        getCurrentTime().add(30 minutes) // publicSaleEnd
    ];
    ...
    struct VestingSchedule {
        ...
        uint256 amountTotal;
        // 50 % amount of tokens released in first half
        uint256 firstVestingAmount;
    ...
    function manualDeposit(
        ...
        require(depositors.length > 0, "EMPTY LIST NOT ALLOWED");
    ...
    function deposit(
        ...
        if (
            (getCurrentTime() >= dates[0] && getCurrentTime() < dates[1]) &&
            enableWhiteLister
        ) {
            require(
                MerkleProof.verify(
    ...
    function _deposit(
        ...
        if (
            ...
            _createVestingSchedule(
                ...
                ONE_MINUTE.mul(5),
                false,
                purchasedTokens.mul(1e18)
            );
        }
        /*PUBLIC SALE*/
        else if (
            (getCurrentTime() >= dates[2] && getCurrentTime() < dates[3]) ||
            publicSale3M ||
            publicSale6M
        ) {
            purchasedTokens = paymentAmount.div(prices[1]);
            uint256 purchasedTokensBonus = purchasedTokens
                .mul(bonus[getBonusWeek().sub(1)])
                .div(100);
            require(
                supplies[3] >= purchasedTokens.add(purchasedTokensBonus),
                "OVERFLOW PURCHASE"
            );
        ...
        if (msg.sender != owner()) {
            paymentTokens[pTIndex].transferFrom(
                depositor,
                address(this),
                paymentAmount
            );
        ...
        if (
            ...
            isReffered[depositor] == address(0)
        ) {
            isReffered[depositor] = referral;
            referralClaims[referral].push(
                ReferralClaim(
                    depositor,
                    purchasedTokens.mul(1e18),
                    purchasedTokens.mul(1e18).mul(bonus[4]).div(100),
                    0,
                    false
                )
            );
        ...
        onlyIfVestingScheduleNotRevoked(vestingScheduleId)
    {
        ...
        require(amount > 0, "NO TOKENS AVALIBLE");

        uint256 firstVestingAmount = 0;
        if (vestingSchedule.released == 0) {
            firstVestingAmount = vestingSchedule.amountTotal / 2;
            vestingSchedule.firstVestingAmount = firstVestingAmount;
        }

        if (((amount + firstVestingAmount) <= vestingSchedule.amountTotal)) {
            vestingSchedule.released = vestingSchedule.released.add(
            ...
            _mint(beneficiaryPayable, amount + firstVestingAmount);
            emit Claimed(amount + firstVestingAmount);
        }
    }
     ...
     */
    function claimRefferalBonus(uint256 refferalIndex) public {
        require(
            referralClaims[msg.sender][refferalIndex].refferalAddress !=
                address(0),
            "No Claim found"
        );
        require(
            !referralClaims[msg.sender][refferalIndex].claimed,
            "Already Claimed"
        );
        _mint(
            msg.sender,
    ...
    function earlyMint(
        ...
        for (uint256 i = 0; i < _recipients.length; i++) {
            ...
            uint256 purchasedTokens = 0;
            purchasedTokens = _amount[i].div(prices[0]);
            supplies[7] = supplies[7].add(purchasedTokens);
            _createVestingSchedule(
                _recipients[i],
                getCurrentTime(),
     ...
     * @param _amount : amount to mint the tokens
     */
    function founderAllocationMint(address _recipients, uint256 _amount)
        public
        onlyOwner
    {
     ...
     * @param _amount : amount to mint the tokens
     */
    function mint(address[] calldata _recipients, uint256[] calldata _amount)
        public
        onlyOwner
    {
        for (uint256 i = 0; i < _recipients.length; i++) {
    ...
    /*
     * @dev setMerkleRoot called by admin
     * @param _merkleRoot : merkle root for whitelisters
     */
     ...
     * @param merkleProof
     */
    function whiteLister(bytes32[] calldata merkleProof, address _depoositor)
        public
        view
        returns (bool)
    {
        return
            MerkleProof.verify(
                merkleProof,
                merkleRoot,
                keccak256(abi.encodePacked(_depoositor))
            );
    }
     ...
     */
    function setBonus(uint256[5] calldata _bonus) public onlyOwner {
        bonus = _bonus;
    }
     ...
     */
    function setPaymentTokens(IERC20[] calldata _paymentTokens)
        public
        onlyOwner
    {
     ...
     */
    function getPaymentTokensCount() public view returns (uint256) {
     ...
     */
    function getReferralsCount(address referral) public view returns (uint256) {
    ...
    function withdrawTokens(uint8 pTIndex, uint256 amount) public onlyOwner {
        require(
            paymentTokens[pTIndex].balanceOf(address(this)) >= amount,
        ...
        returns (uint256)
    {
        ...
        if (
            ...
            uint256 timeFromStart = currentTime.sub(vestingSchedule.start); //1611450
            uint256 secondsPerSlice = vestingSchedule.slicePeriodSeconds; // 3600
            uint256 vestedSlicePeriods = timeFromStart.div(secondsPerSlice); // //2
            vestedSlicePeriods = vestedSlicePeriods - 1; //2-1
            uint256 vestedSeconds = vestedSlicePeriods.mul(secondsPerSlice);

            uint256 vestedAmount = 0;
            uint256 reainingAmountTotal = vestingSchedule.amountTotal.div(2);

            if (vestingSchedule.firstVestingAmount >= 0) {
                vestedAmount = reainingAmountTotal.mul(vestedSeconds).div(
                    vestingSchedule.duration
                );
                vestedAmount = vestedAmount.sub(
                    vestingSchedule.released.sub(
                        vestingSchedule.firstVestingAmount
                    )
                );
                vestedAmount =  vestedAmount / 2;
            } else {
                vestedAmount = reainingAmountTotal.mul(vestedSeconds).div(
                    vestingSchedule.duration
                );
    ...
    function enableWhiteList(bool _enable) public onlyOwner {
        enableWhiteLister = _enable;
    }
