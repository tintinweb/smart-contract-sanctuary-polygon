    ...
    UUPSUpgradeable
{
    ...
    function _getChestsDrops(
        ...
        for (uint256 index = 0; index < amount; index++) {
            ...
            uint8 drop = _rollRarity(randomNumber, chestDropProbabilities);
            drops[index] = drop;
            if (drop == 0) {
                // mintGear
...
library SharedLib {
    ...
    function statsToRepresentation(Stats memory stats) internal pure returns (uint72) {
    ...
    function representationToStats(uint72 representation) internal pure returns (Stats memory stats) {
