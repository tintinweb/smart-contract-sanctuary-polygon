 ...
 */
contract FlipItGameV1 is Ownable {
    ...
    /// @param rewardId Id of the reward token.
    ...
    /// @param amount Amount of the required tokens to play.
    ...
    uint256 internal constant UPPER_THRESHOLD_OF_CHANCE_TO_WIN = 40;
    ...
    function play(uint256 thresholdId) external {
         ...
         */
        if (threshold.amount == 0) revert InvalidThreshold(thresholdId);

        if (token.balanceOf(player) < threshold.amount) revert InvalidThreshold(thresholdId);
        ...
        bool win = chance <= UPPER_THRESHOLD_OF_CHANCE_TO_WIN;

        /// Mints a reward (nft)
        uint256[] memory rewards = minter.mintIngredient(player, 1);

        /// Saves the result of the game
        games[++gameSerialId] = Game({ player: player, amount: threshold.amount, win: win, rewardId: rewards[0] });
        ...
        if (win) token.safeTransfer(player, threshold.amount);

        if (!win) token.safeTransferFrom(player, address(this), threshold.amount);
    }

    /// @notice Adds new threshold.
    /// @param amount Amount of the required tokens to play.
    /// @param level Number of nft required to play.
    function addThreshold(uint256 amount, uint256 level) external onlyOwner {
        thresholds[++thresholdSerialId] = Threshold({ amount: amount, level: level });
    }
