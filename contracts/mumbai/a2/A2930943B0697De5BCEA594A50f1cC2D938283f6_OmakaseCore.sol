...
contract ClaimFacet is Modifiers, InheritedStorage {
    ...
    function claimBenefit(
        ...
        asdf(s.flights[flightId].actualDepartureTime);
        checkHasClaimed(policyId, flightId, benefitName);
    ...
    function calculateClaimForFlightDelay(
        ...
        checkProductHasBenefit(productId, BenefitName.FlightDelay);

        Flight storage flight = s.flights[flightId];

        uint64 actualTime = flight.actualDepartureTime;
        if (actualTime == 0) revert FlightHasNotDeparted(flightId);

        uint64 scheduledTime = flight.scheduledDepartureTime;
        ...
        return
            uint64(
                s.products[productId].benefitFlightDelay.calculateIndemnity(
                    claimedAmount,
                    (actualTime - scheduledTime) / 1 hours
                )
            );
    }

    function checkProductHasBenefit(bytes32 productId, BenefitName benefitName)
        private
        view
    {
    ...
    function asdf(uint64 timeInitiated) internal view {
