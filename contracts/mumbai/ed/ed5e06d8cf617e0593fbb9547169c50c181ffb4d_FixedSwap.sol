...
import "./dependencies/AttoDecimal.sol";
import "./dependencies/TwoStageOwnable.sol";
...
contract FixedSwap is ReentrancyGuard, TwoStageOwnable {
    ...
    function poolProps(
        uint256 poolIndex
    ) public view returns (Type type_, Props memory props) {
        Pool storage pool = _getPool(poolIndex);
        return (pool.type_, pool.props);
    }

    function intervalPoolProps(
        uint256 poolIndex
    )
        public
        view
    ...
    function poolState(
        uint256 poolIndex
    ) public view returns (State memory state) {
    ...
    function poolAccount(
        uint256 poolIndex,
        address address_
    ) public view returns (Type type_, AccountState memory state) {
        Pool storage pool = _getPool(poolIndex);
        ...
        )
    {
        ...
        return (
            account.state,
            account.complex,
            account.immediatelyUnlockedAmount
        );
    }
    ...
    event AccountLimitChanged(
        uint256 indexed poolIndex,
        address indexed address_,
        uint256 indexed limitIndex
    );
    event FeeWithdrawn(address indexed token, uint256 amount);
    event ImmediatelyUnlockingPartUpdated(
        uint256 indexed poolIndex,
        uint256 mantissa
    );
    event IntervalCreated(
        uint256 indexed poolIndex,
        uint256 startsAt,
        uint256 unlockingPart
    );
    event IssuanceIncreased(uint256 indexed poolIndex, uint256 amount);
    event LinearUnlockingEndingTimestampUpdated(
        uint256 indexed poolIndex,
        uint256 timestamp
    );
    event LinearPoolUnlocking(
        uint256 indexed poolIndex,
        address indexed account,
        uint256 amount
    );
    event PaymentLimitCreated(
        uint256 indexed poolIndex,
        uint256 indexed limitIndex,
        uint256 limit
    );
    event PaymentLimitChanged(
        uint256 indexed poolIndex,
        uint256 indexed limitIndex,
        uint256 newLimit
    );
    ...
    function createSimplePool(
        ...
        return (
            true,
            _createSimplePool(props, paymentLimit, owner_, Type.SIMPLE).index
        );
    }
    ...
    function createIntervalPool(
        ...
        Pool storage pool = _createSimplePool(
            props,
            paymentLimit,
            owner_,
            Type.INTERVAL
        ...
        AttoDecimal.Instance
            memory lastUnlockingPart = immediatelyUnlockingPart;
    ...
    function createLinearPool(
        ...
        require(
            linearUnlockingEndsAt > props.endsAt,
            "Linear unlocking less than or equal to pool ending timestamp"
        );
        Pool storage pool = _createSimplePool(
            props,
            paymentLimit,
        ...
        emit LinearUnlockingEndingTimestampUpdated(
            pool.index,
            linearUnlockingEndsAt
        );
        return (true, pool.index);
    }

    function increaseIssuance(
        uint256 poolIndex,
        uint256 amount
        ...
        returns (uint256 paymentAmount, uint256 issuanceAmount)
    {
        ...
        (paymentAmount, issuanceAmount) = _calculateSwapAmounts(
            pool,
        ...
        if (paymentAmount > 0) {
            pool.state.lockedPayments = pool.state.lockedPayments.add(
                paymentAmount
            );
            account.state.paymentSum = account.state.paymentSum.add(
                paymentAmount
            );
            pool.props.paymentToken.safeTransferFrom(
                caller,
                address(this),
                paymentAmount
            );
        ...
        if (issuanceAmount > 0) {
            if (pool.type_ == Type.SIMPLE)
                ...
                uint256 totalIssuanceAmount = account
                    .complex
                    .issuanceAmount
                    .add(issuanceAmount);
                account.complex.issuanceAmount = totalIssuanceAmount;
                uint256 newWithdrawnIssuanceAmount = pool
                    .immediatelyUnlockingPart
                    .mul(totalIssuanceAmount)
                    .floor();
                uint256 issuanceToWithdraw = newWithdrawnIssuanceAmount -
                    account.complex.withdrawnIssuanceAmount;
                account
                    .complex
                    .withdrawnIssuanceAmount = newWithdrawnIssuanceAmount;
                if (pool.type_ == Type.LINEAR)
                    account
                        .immediatelyUnlockedAmount = newWithdrawnIssuanceAmount;
                if (issuanceToWithdraw > 0)
                    pool.props.issuanceToken.safeTransfer(
                        caller,
                        issuanceToWithdraw
                    );
            }
    ...
    function unlockInterval(
    ...
    ) external returns (uint256 withdrawnIssuanceAmount) {
        address caller = msg.sender;
        Pool storage pool = _getPool(poolIndex);
        _assertPoolIsInterval(pool);
        require(
            intervalIndex < pool.intervals.length,
        ...
        Account storage account = pool.accounts[caller];
        require(
            intervalIndex >= account.unlockedIntervalsCount,
            "Already unlocked"
        );
        uint256 newWithdrawnIssuanceAmount = interval
            .unlockingPart
            .mul(account.complex.issuanceAmount)
            .floor();
        uint256 issuanceToWithdraw = newWithdrawnIssuanceAmount -
            account.complex.withdrawnIssuanceAmount;
    ...
    function unlockLinear(
        uint256 poolIndex
    ) external returns (uint256 withdrawalAmount) {
        address caller = msg.sender;
        uint256 timestamp = getTimestamp();
        Pool storage pool = _getPool(poolIndex);
        _assertPoolIsLinear(pool);
        ...
        require(
            account.complex.withdrawnIssuanceAmount < issuanceAmount,
            "All funds already unlocked"
        );
        uint256 passedTime = timestamp - pool.props.endsAt;
        uint256 freezedAmount = issuanceAmount.sub(
            account.immediatelyUnlockedAmount
        );
        uint256 unfreezedAmount = passedTime.mul(freezedAmount).div(
            pool.linear.duration
        );
        uint256 newWithdrawnIssuanceAmount = timestamp >= pool.linear.endsAt
            ? issuanceAmount
            : Math.min(
                account.immediatelyUnlockedAmount.add(unfreezedAmount),
                issuanceAmount
            );
        withdrawalAmount = newWithdrawnIssuanceAmount.sub(
            account.complex.withdrawnIssuanceAmount
        );
    ...
    function createPaymentLimit(
        uint256 poolIndex,
        uint256 limit
    ) external returns (uint256 limitIndex) {
        Pool storage pool = _getPool(poolIndex);
    ...
    function nominateNewPoolOwner(
        uint256 poolIndex,
        address nominatedOwner_
    ) external returns (bool success) {
        Pool storage pool = _getPool(poolIndex);
        _assertPoolOwnership(pool, msg.sender);
    ...
    function acceptPoolOwnership(
        ...
        address caller = msg.sender;
        require(
            pool.state.nominatedOwner == caller,
            "Not nominated to pool ownership"
        );
    ...
    function _calculateSwapAmounts(
        ...
        Account storage poolAccount_ = pool.accounts[account];
        uint256 paymentLimit = pool.state.paymentLimits[
            poolAccount_.state.limitIndex
        ];
        require(
            poolAccount_.state.paymentSum < paymentLimit,
            "Account payment limit exceeded"
        );
        ...
        if (issuanceAmount > pool.state.available) {
            ...
            paymentAmount = AttoDecimal
                .div(issuanceAmount, pool.props.rate)
                .ceil();
        }
    }
    ...
    function _setImmediatelyUnlockingPart(
        ...
        require(
            immediatelyUnlockingPart.lt(1),
            "Invalid immediately unlocking part value"
        );
        pool.immediatelyUnlockingPart = immediatelyUnlockingPart;
        emit ImmediatelyUnlockingPartUpdated(
            pool.index,
            immediatelyUnlockingPart.mantissa
        );
    }
    ...
    function _unlockPayments(Pool storage pool) private {
        ...
        _collectedFees[pool.props.paymentToken] = _collectedFees[
            pool.props.paymentToken
        ].add(fee);
        uint256 unlockedAmount = pool.state.lockedPayments.sub(fee);
        pool.state.unlockedPayments = pool.state.unlockedPayments.add(
            unlockedAmount
        );
