...
/**
  @title Volume Collection Manager
  @notice Create collections and mint NFTs! Users can support their favorite artists by purchasing media NFTs!
 */
...
contract VolumeNFTManager is ERC1155URIStorage, Ownable {
  ...
  constructor (
    ...
    string memory _symbol
      ...
      symbol = _symbol;
      serviceFee = 1000;
      // testnet 0xe9A7cb72ce0Cab7B53601C75037dEEAF270F2aa4, remix 0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2, hardhat node 0x3C44CdDdB6a900fa2b585dd299e03d12FA4293BC 
  ...
  uint counter; // used to increment token ID
  address serviceManager; // address of manager's wallet
  uint serviceFee; // % with divisor of 10,000
  ...
  event CollectionCreated(uint256 tokenID, uint256 maxSupply, string uri, uint256 price, address indexed creator);
  ...
  function mint(address _to, uint256 _id, uint256 _amount) external payable {
    ...
    require(supply[_id] + _amount <= maxTokenSupply[_id], "Cannot create more than max supply");
   ...
   */
  function createCollection (uint256 _maxSupply, string memory _uri, uint256 _price) external {
    ...
    creator[counter] = msg.sender;
    emit CollectionCreated(counter, _maxSupply, _uri, _price, msg.sender);
    counter++;
  }
  ...
  // Informational Functions - consider creating info struct instead of separate functions
  /**
