...
contract PlushNFTCashbackPool is Initializable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function addTokensToAccount(address _wallet, uint256 _amount, uint256 _timeUnlock) public
    {
        idsBalances[_wallet].push(idsBalances[_wallet].length);
        balanceInfo[idsBalances[_wallet].length].balance = _amount;
        balanceInfo[idsBalances[_wallet].length].timeIsActive = block.timestamp + _timeUnlock;
    }
