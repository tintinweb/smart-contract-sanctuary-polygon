...
import "./CustomSimpleAccount.sol";

/**
 * A sample factory contract for CustomSimpleAccount
 * A UserOperations "initCode" holds the address of the factory, and a method call (to createAccount, in this sample factory).
 * The factory's createAccount returns the target account address even if it is already installed.
 * This way, the entryPoint.getSenderAddress() can be called either before or after the account is created.
 */
contract CustomSimpleAccountFactory {
    CustomSimpleAccount public immutable accountImplementation;

    constructor(IEntryPoint _entryPoint) {
        accountImplementation = new CustomSimpleAccount(_entryPoint);
    }
     ...
     */
    function createAccount(address owner,uint256 salt) public returns (CustomSimpleAccount ret) {
        address addr = getAddress(owner, salt);
        uint codeSize = addr.code.length;
        if (codeSize > 0) {
            return CustomSimpleAccount(payable(addr));
        }
        ret = CustomSimpleAccount(payable(new ERC1967Proxy{salt : bytes32(salt)}(
                address(accountImplementation),
                abi.encodeCall(CustomSimpleAccount.initialize, (owner))
            )));
    }
