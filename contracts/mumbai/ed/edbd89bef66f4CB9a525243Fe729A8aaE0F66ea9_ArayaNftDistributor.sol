...
contract ArayaNftDistributor is AccessControl, Pausable, Withdrawable, ReentrancyGuard {
    ...
    */
    event PricesChanged(uint256 tokenType, uint256 price);
    /**
    ...
    */
    function buyTokens(uint256 id, uint256 amount) external whenNotPaused nonReentrant {
        ...
        uint256 fullAmount = price.mul(amount);

        require(tx.origin == caller, "Contracts are not allowed");
        require(price > 0, "Unknown token price");
        ...
        emit TokenBought(caller, id, amount, price);
    }

    function setPrice(uint256 tokenType, uint256 price) external onlyRole(MANAGER_ROLE) {
        prices[tokenType] = price;
        tradableTokens.add(tokenType);
        emit PricesChanged(tokenType, price);
    }
}