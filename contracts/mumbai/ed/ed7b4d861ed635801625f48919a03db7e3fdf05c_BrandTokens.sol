...
// Dependency file: contracts/interfaces/IMoleculeController.sol
...
// Root file: contracts/tokens/ERC1155m_Brand_Tokens_access.sol
...
// import "@openzeppelin/contracts/utils/Strings.sol";
// import "contracts/interfaces/IMoleculeController.sol";
...
// Brand-token contract: Molecule ERC1155 token
contract BrandTokens is ERC1155, Ownable, AccessControl {
    enum MoleculeType {
        Approve,
        Burn,
        Mint,
    ...
    event TransferSingleData(address indexed operator, address indexed from, address indexed to, uint256 id, uint256 value, bytes data);
    ...
    // Both ERC1155 and AccessControl uses supportInterface
    // so requires this override
    function supportsInterface(bytes4 interfaceId) public view override(ERC1155, AccessControl) returns (bool) {
        return super.supportsInterface(interfaceId) || interfaceId == type(AccessControl).interfaceId;
    }
    ...
    // naming convention follows CoCreateERC1155.sol
    function mintBatch(
        ...
        require(recipients.length == tokenIds.length, "ERC1155m: recipients and tokenIds length mismatch");
        require(tokenIds.length == amounts.length, "ERC1155m: ids and amounts length mismatch");
        for (uint256 i = 0; i < tokenIds.length; i++) {
            if (_moleculeMint != address(0)) {
                if (!IMoleculeController(_moleculeMint).check(recipients[i])) {
                    revert AccountNotAllowedToMint(recipients[i]);
                }
            }
            _mint(recipients[i], tokenIds[i], amounts[i], data);
            emit TransferSingleData(msg.sender, recipients[i], msg.sender, tokenIds[i], amounts[i], data);
        }
    }
