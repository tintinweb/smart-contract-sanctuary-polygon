...
contract AccountRegistry is IAccountRegistry, Ownable {
    ...
    /**
     * @notice add a list of new accounts to the authorized accounts list
     */
    function addAuthorizedAccounts(address[] calldata accounts) external onlyOwner {
        for (uint256 i = 0; i < accounts.length; i++) {
            authorizedAccounts[accounts[i]] = true;
        }
        emit AccountsAdded(accounts);
    }

    /**
     * @notice remove a list of new accounts from the authorized accounts list
     */
    function removeAuthorizedAccounts(address[] calldata accounts) external onlyOwner {
        for (uint256 i = 0; i < accounts.length; i++) {
            authorizedAccounts[accounts[i]] = false;
        }
        emit AccountsRemoved(accounts);
    }
    ...
    function isAuthorized(address[] calldata accounts) external view override returns (bool) {
        for (uint256 i = 0; i < accounts.length; i++) {
            if (!authorizedAccounts[accounts[i]]) return false;
        }
        return true;
    }
}
