...
contract AuctionFactoryNzuri is Ownable, FeeCollectable, IERC721Receiver, IERC1155Receiver {
  ...
  mapping(address => mapping(address => uint256)) private fundsByBidder;
  ...
  function createAuction(
    ...
    address auctionAddress = address(
      uint160(uint(keccak256(abi.encodePacked(block.difficulty, blockhash(block.number)))))
    );
    ...
    auctionStates[auctionAddress] = AuctionState(
      controller,
      msg.sender,
      address(0),
      tokenId,
  ...
  function placeBid(address auctionAddress) payable external {
    AuctionState storage auctionState = auctionStates[auctionAddress];
    ...
    require(!auctionState.cancelled, "AuctionFactoryNzuri: auction was cancelled");
    require(!auctionState.acceptERC20, "AuctionFactoryNzuri: auction doesn't support erc20 tokens");
    require(msg.sender != auctionState.controller, "AuctionFactoryNzuri: controller can't bid on own auction");
    require(msg.sender != auctionState.beneficiary, "AuctionFactoryNzuri: beneficiary can't bid on own auction");
    ...
    if (auctionState.highestBid == 0) {
      ...
      // reject if user did not overbid
      require(totalBid >= auctionState.highestBid + parameters.bidStep, "AuctionFactoryNzuri: bid must overbid bid step");
    }
  ...
  function placeBidERC20(address auctionAddress, uint amount) external {
    ...
    require(block.timestamp < parameters.endTimestamp, "AuctionFactoryNzuri: auction has ended");
    require(!auctionState.cancelled, "AuctionFactoryNzuri: auction was cancelled");
    require(auctionState.acceptERC20, "AuctionFactoryNzuri: auction accepts only erc20 tokens");
    require(msg.sender != auctionState.controller, "AuctionFactoryNzuri: controller can't bid on own auction");
    require(msg.sender != auctionState.beneficiary, "AuctionFactoryNzuri: beneficiary can't bid on own auction");
  ...
  function claimFunds(address auctionAddress) external {
    ...
    require(
      auctionState.cancelled || block.timestamp >= parameters.endTimestamp,
    ...
    if (auctionState.cancelled) {
      ...
      require(msg.sender != auctionState.highestBidder, "AuctionFactoryNzuri: auction winner can't withdraw his bid");

      // everyone except auction winner should be allowed to withdraw their funds
      if (msg.sender == auctionState.beneficiary) {
        require(
          parameters.feeRate < 100 && !auctionState.beneficiaryClaimedFunds,
          "beneficiary already claimed his funds"
        );
        withdrawalAccount = auctionState.highestBidder;
        withdrawalAmount = auctionState.highestBid / 100 * (100 - parameters.feeRate);
        beneficiaryClaimedFunds = true;
      } else if (msg.sender == auctionState.controller) {
        require(
          parameters.feeRate > 0 && !auctionState.controllerClaimedFunds,
          "AuctionFactoryNzuri: controller already claimed his funds"
        );
        withdrawalAccount = auctionState.highestBidder;
        withdrawalAmount = auctionState.highestBid / 100 * parameters.feeRate;
  ...
  function claimItem(address auctionAddress) external {
    ...
    require(!auctionState.itemClaimed, "AuctionFactoryNzuri: item was already claimed");
    require(
      auctionState.cancelled || block.timestamp >= parameters.endTimestamp,
      "AuctionFactoryNzuri: auction is still in progress"
    );
    ...
    if (auctionState.cancelled
      || (auctionState.highestBidder == address(0) && block.timestamp >= parameters.endTimestamp)) {
      require(
        msg.sender == auctionState.beneficiary,
        "AuctionFactoryNzuri: non-beneficiary can't claim item from auction without winner"
      );
    } else {
      require(msg.sender == auctionState.highestBidder, "AuctionFactoryNzuri: only auction winner can claim item");
    }
  ...
  function cancelAuction(address auctionAddress) external {
    ...
    require(!auctionState.cancelled, "AuctionFactoryNzuri: auction already cancelled");
    require(block.timestamp < parameters.endTimestamp, "AuctionFactoryNzuri: auction has ended");
  ...
  function _placeBid(address auctionAddress, address bidder, uint totalAmount) private {
    ...
    if (bidder != auctionState.highestBidder) {
      auctionState.highestBidder = bidder;
    }

    auctionState.highestBid = totalAmount;
  }
  ...
  function _withdrawFunds(
    ...
    if (_beneficiaryClaimedFunds) {
      auctionState.beneficiaryClaimedFunds = true;
    }
    if (_controllerClaimedFunds) {
      auctionState.controllerClaimedFunds = true;
    }
    // send the funds
    if (auctionState.acceptERC20) {
  ...
  function _transferItem(address auctionAddress, address claimer) private {
    AuctionState storage auctionState = auctionStates[auctionAddress];
    if (auctionState.isErc1155) {
      erc1155NzuriContract.safeTransferFrom(address(this), claimer, auctionState.tokenId, auctionState.quantity, "");
    } else {
      erc721NzuriContract.safeTransferFrom(address(this), claimer, auctionState.tokenId);
    }

    auctionState.itemClaimed = true;
  }
  ...
  function availableFeeToken(address erc20, address shareholder) override public view returns(uint) {
    ...
    for (uint256 i = 0; i < auctionsFeePendingTotal; i++) {
      ...
      if (auctionState.controllerClaimedFunds) {
        continue;
      }
      ...
      address highestBidder = auctionState.highestBidder;
      if (highestBidder == address(0)) {
        continue;
      }

      uint256 feeAmount = auctionState.highestBid / 100 * auctionParameters[auctionAddress].feeRate;
...
contract DropMinting is ReentrancyGuard, Ownable, FeeCollectable {
  ...
  */
  constructor(
    ...
    require(address(_erc20Address) != address(0), "Invalid erc20 address");
...
abstract contract FeeCollectable {
  ...
  event FeeTokensCollected(address erc20, address to, uint256 amount);
  event FeeCollectorChanged(address oldAddress, address newAddress);
  ...
  function withdrawAllFeeToken(address erc20) virtual public {
    require(feeRates[msg.sender] > 0, "FeeCollectable: caller is not a shareholder");
  ...
  function setShareholders(address[] memory _shareholders, uint8[] memory _feeRates) virtual public {
    ...
    require(_shareholders.length > 0, "FeeCollectable: shareholders array must not be empty");
    require(_feeRates.length > 0, "FeeCollectable: fee rates array must not be empty");
    require(
