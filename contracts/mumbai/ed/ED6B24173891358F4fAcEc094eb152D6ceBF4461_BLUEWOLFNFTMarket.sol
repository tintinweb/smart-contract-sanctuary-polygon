    ...
    AccessControl
{
    ...
    function getLockedNFT(address nftContract, uint256 tokenId)
        public
        view
        returns (bool)
    {
    ...
    function getNFTOwnerDuringLockedinMarket(
        address nftContract,
        uint256 tokenId
    ...
    function createOrder(
        ...
        require(getLockedNFT(nftContract, tokenId) == false, "NFT is locked");
    ...
    function cancelOrder(
        ...
        require(
            getLockedNFT(nftContract, tokenId) == true,
            "The order not found"
        );
        require(
            orders[nftContract][tokenId][
    ...
    function Buy(
        ...
        require(getLockedNFT(nftContract, tokenId) == true, "NFT isn't locked");
        require(
            orders[nftContract][tokenId][
                orders[nftContract][tokenId].length - 1
            ].orderId == orderId,
        ...
        address buyWithTokenContract = orders[nftContract][tokenId][
            orders[nftContract][tokenId].length - 1
        ].buyWithTokenContract;
        uint256 balance = IERC20(buyWithTokenContract).balanceOf(msg.sender);
        uint256 fee = (price * feesRate) / 10000;
        address seller = orders[nftContract][tokenId][
            orders[nftContract][tokenId].length - 1
        ].seller;
    ...
    function makeOffer(
        ...
        require(price > 0, "Price must be more than 0");
        require(buyerBalance >= price, "You do not have enough token");
    ...
    function cancelOffer(
        ...
        uint256 index;
        for (uint32 i; i < offers[nftContract][tokenId].length; i++) {
            if (offers[nftContract][tokenId][i].offerId == offerId) {
                index = i;
            }
        }
        require(
        ...
        require(
            offers[nftContract][tokenId][index].active == true,
            "It's canceled item"
        );

        offers[nftContract][tokenId][index].active = false;
    ...
    function acceptOffer(
        ...
        uint256 fee = (price * feesRate) / 10000;
        require(balance >= price, "Buyer does not have enough token");
        ...
        //Update status
        if (getLockedNFT(nftContract, tokenId) == true) {
            orders[nftContract][tokenId][
                orders[nftContract][tokenId].length - 1
            ].locked = false;
            orders[nftContract][tokenId][
