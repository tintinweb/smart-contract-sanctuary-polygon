 ...
 */
contract MarketplaceModule is AccessControl, ReentrancyGuard, ErrorReporter, MarketplaceI {
  using SafeERC20 for IERC20;
  using EnumerableSet for EnumerableSet.AddressSet;
  using Arrays for uint256[];
  using Arrays for uint256[5];
  ...
  /**
   * @notice Enable liquidator for a specific token, this happens when a token is transferred to the liquidator
  ...
  function bid(
    ...
    _hToken.accrueInterestToCoupon(_collateralId);
    ...
    addToAuctionQueue(poolToTokenToAuction[_hToken][_collateralId], _hToken, msg.sender, transferAmount, underlying);
   ...
   */
  function bidCollection(HTokenI _hToken, uint256 _amount) external override nonReentrant {
    if (!hivemind.marketExists(_hToken)) revert Uninitialized();
    if (poolToAuctionDisabled[_hToken]) revert LiquidatorError(Error.AUCTION_NOT_ACTIVE);
    ...
    poolToAuction[_hToken] = addToAuctionQueue(poolToAuction[_hToken], _hToken, msg.sender, transferAmount, underlying);
  ...
  function settleAuction(
    ...
    if (settleType == SettleType.SINGLE_TOKEN) {
      ...
      poolToTokenToAuction[_hToken][_collateralId] = _removeFromAuctionQueue(singleAuction, singleAuction.bids.length - 1, false);
      ...
      poolToAuction[_hToken] = _removeFromAuctionQueue(collectionAuction, singleAuction.bids.length - 1, false);
      // collectionAuction.unlockTime = 0;
    }

    // Reset the state
    collectionToTokenToTimeReceived[collateralToken][_collateralId] = 0;
    collectionToTokenToSource[collateralToken][_collateralId] = HTokenI(address(0));
  ...
  function addToAuctionQueue(
    ...
    if (_auction.highestBidder == address(0)) {
      ...
      _auction.underlying = _underlying;
    ...
    if (present) {
      _removeFromAuctionQueue(_auction, index, true);
      params.bids = _auction.bids;
      params.bidders = _auction.bidders;
      params.unlockTimes = _auction.unlockTimes;
    }
  ...
  function isUserInArray(address[5] memory _array, address _user) internal pure returns (bool, uint256) {
  ...
  function _removeFromAuctionQueue(
    Auction storage _auction,
    uint256 _index,
    bool _refund
  ) internal returns (Auction memory) {
    // (bool present, uint256 index) = isUserInArray(_auction.bidders, _recipient);
    ...
    // If removing the highest bidder, promote the next highest if any
    if (_index == len - 1 && len > 1) {
   ...
   * @param _collateralId the tokenId of the NFT
   */
  function cancelBidSingle(HTokenI _hToken, uint256 _collateralId) external override nonReentrant {
    if (!hivemind.marketExists(_hToken)) revert Uninitialized();
    ...
    if (!present || auction.highestBidder != msg.sender) revert LiquidatorError(Error.UNAUTHORIZED);
    if (block.timestamp < auction.unlockTimes[index]) revert LiquidatorError(Error.CANCEL_TOO_SOON);

    _removeFromAuctionQueue(auction, index, true);
   ...
   * @param _hToken hToken contract address
   */
  function cancelBidCollection(HTokenI _hToken) external override nonReentrant {
    if (!hivemind.marketExists(_hToken)) revert Uninitialized();
  ...
  function viewMinimumNextBidSingle(HTokenI _hToken, uint256 _collateralId) external view override returns (uint256) {
    Auction storage auction = poolToTokenToAuction[_hToken][_collateralId];
    uint256 amount = auction.highestBid;
    uint256 minimumBid;

    if (amount > 0) {
      minimumBid = (amount * (minimumBidIncrementMantissa + 1e18)) / 1e18;
    } else {
      uint256 debt = _hToken.getBorrowAmountForCollateral(_collateralId);
   ...
   */
  function viewMinimumNextBidCollection(HTokenI _hToken) external view override returns (uint256) {
    Auction storage auction = poolToAuction[_hToken];
    uint256 amount = auction.highestBid;
    uint256 minimumBid;
   ...
   * returns an Auction
   */
  function getAllBidsOfACollection(HTokenI _hToken) external view override returns (Auction memory) {
   ...
   */
  function setAuctionStatus(HTokenI _hToken, bool _status) external nonReentrant onlyRole(DEFAULT_ADMIN_ROLE) {
   ...
   */
  function setDefaultAuctionStatus(bool _status) external nonReentrant onlyRole(DEFAULT_ADMIN_ROLE) {
   ...
   */
  function updateMinimumBidIncrement(uint256 _newMinimumBidIncrement) external nonReentrant onlyRole(DEFAULT_ADMIN_ROLE) {
   ...
   */
  function updateReservePricePadding(uint256 _newReservePricePadding) external nonReentrant onlyRole(DEFAULT_ADMIN_ROLE) {
   ...
   */
  function updateSettlementIncentive(uint256 _newSettlementIncentive) external nonReentrant onlyRole(DEFAULT_ADMIN_ROLE) {
   ...
   */
  function updateBidCooldown(uint256 _newBidCooldown) external nonReentrant onlyRole(DEFAULT_ADMIN_ROLE) {
   ...
   */
  function sweepToken(IERC20 _token) external nonReentrant onlyRole(DEFAULT_ADMIN_ROLE) {
    // if (registeredUnderlyingTokens.contains(address(_token))) revert Unauthorized(); //TODO see how to solve this to not sweep everything
   ...
   */
  function withdrawProfits(HTokenI _hToken) external nonReentrant onlyRole(DEFAULT_ADMIN_ROLE) {
  ...
  event NewAuctionWindow(uint256 _oldWindow, uint256 _newWindow);
...
interface MarketplaceI {
  ...
  function getAllBidsOfACollection(HTokenI _hToken) external view returns (Auction memory);
...
contract LiquidatorStorage {
  event DropletsSwappedToRepayBorrow(
...
abstract contract LiquidatorModuleNFT20 is AccessControl, ReentrancyGuard, LiquidatorStorage {
  ...
  // event DropletsSwappedToRepayBorrow(
  //   IERC721 indexed _collateralToken,
  //   address indexed _borrower,
  //   uint256 _collateralId,
  //   address[] _path,
  //   uint256 _amountIn,
   ...
   * @param _collateralToken Contract address of the collateral NFT
   * @param _collateralId NFT tokenId
   * @return (hToken address, vault address)
   */
  function _NFT20PreLiquidationHook(IERC721 _collateralToken, uint256 _collateralId) internal virtual returns (HTokenI, address) {}
   ...
   * @param _collateralToken address of the NFT contract
   * @param _collateralId NFT Token Id
   */
  function liquidateViaNFT20(IERC721 _collateralToken, uint256 _collateralId)
    external
    nonReentrant
    onlyRole(LIQUIDATOR_ROLE)
    returns (bool)
  {// TODO add incentivisation on executing this 
    (HTokenI hToken, address pair) = _NFT20PreLiquidationHook(_collateralToken, _collateralId);

    // Straight token transfer, IERC721Receiver hook in NFT20 pair will mint pair tokens.
    _collateralToken.safeTransferFrom(address(this), pair, _collateralId);

    emit NFT20LiquidationExecuted(pair, address(hToken), _collateralId);
  ...
  function swapNFT20DropletsAndRepayBorrow(
    ...
    emit DropletsSwappedToRepayBorrow(address(collateralToken), _borrower, _collateralId, _path, _amountIn, _amountOutMinimum);
...
abstract contract LiquidatorModuleNFTX is AccessControl, ReentrancyGuard, LiquidatorStorage {
  ...
  function _NFTXPreLiquidationHook(
    ...
    uint256 _collateralId,
    uint256 _vaultIndex
  ) internal virtual returns (HTokenI, address) {}
  ...
  function liquidateViaNFTX(
    ...
    (HTokenI hToken, address pair) = _NFTXPreLiquidationHook(_collateralToken, _collateralId, _vaultIndex);
    ...
    _collateralToken.approve(pair, _collateralId);

    INFTXVault(pair).mint(assets, amounts);

    emit NFTXLiquidationExecuted(pair, address(hToken), _collateralId);
...
library Arrays {
   ...
   * repeated elements.
   */
  function findUpperBound(uint256[5] memory array, uint256 element) internal pure returns (uint256) {
    if (array.length == 0) {
      return 0;
    }
