...
contract FileStorage is Initializable, UUPSUpgradeable, OwnableUpgradeable, EIP712Upgradeable, ERC2771ContextUpgradeable {
    using ECDSAUpgradeable for bytes32;

    // Events
    event AccountStorageUpdated(uint256 indexed accountID);

    // Structs
    struct AccountStorageItem {
        uint256 accountID;
        string uri;
        bytes rootKey;  // <encrypted root key, encrypted by secretbox()>
    }
    ...
    // Mappers
    mapping(uint256 => AccountStorageItem) private accountStorages;
    ...
    function createAccountStorage(string memory uri, bytes memory rootKey) public {
        _setAccountStorage(uri, rootKey);
    }

    function setAccountStorageURI(string memory uri) public {
        _setAccountStorage(uri, "");
    }

    function _setAccountStorage(string memory uri, bytes memory rootKey) private {
        address device = _msgSender();
        uint256 accountID = registry.getDeviceAccountID(device);
        require(accountID != 0, "Account not found");
        require(registry.isDeviceActive(accountID, device), "Must be active device of account");

        uint256 storageAccount = accountStorages[accountID].accountID;
        require(storageAccount == 0 || storageAccount == accountID, "Storage already owned");
        if (storageAccount == 0) {
            accountStorages[accountID] = AccountStorageItem(accountID, uri, rootKey);
        } else {
            accountStorages[accountID].uri = uri;
            if (keccak256(rootKey) != keccak256("")) {
                accountStorages[accountID].rootKey = rootKey;
            }
        }
        emit AccountStorageUpdated(accountID);
    }

    function getAccountStorageURI(uint256 _accountID) public view returns (string memory) {
        return accountStorages[_accountID].uri;
    }

    function getAccountStorageInfo(uint256 _accountID) public view returns (uint256 accountID, string memory uri, bytes memory rootKey) {
        AccountStorageItem storage storageItem = accountStorages[_accountID];
        return (storageItem.accountID, storageItem.uri, storageItem.rootKey);
    }
