...
contract CZM is ERC20, Ownable {
    ...
    function addFriend(address friend) external {
        require(userProfiles[msg.sender].birthdate != 0, "Profile does not exist");
        require(userProfiles[friend].birthdate != 0, "Friend's profile does not exist");

        userProfiles[msg.sender].friends.push(friend);
    }

    function getFriends(address user) external view returns (address[] memory) {
        return userProfiles[user].friends;
    }
    ...
    function userBirthdateFunction(address user) external view returns (string memory) {
        ...
        if (age > birthdate) {
            ...
            uint256 decentralizedBirthdaysRemaining = (birthdate - age) / 360;
            return string(abi.encodePacked("You haven't been born in the TimeVerse yet. Your decentralized birthday is in ", uint256ToString(decentralizedBirthdaysRemaining), " decentralized birthdays."));
        }
    }
