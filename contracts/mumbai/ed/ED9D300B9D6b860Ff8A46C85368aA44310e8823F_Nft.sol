    ...
    ONFT721Core
{
    ...
    function _debitFrom(
        ...
        require(
            ...
            "ONFT721: not owner not approved"
        );
    ...
    function buyNft(uint256 _mintAmount) public onlyWhitelisted payable  {
        ...
        require(
            tokenIdCounter.current() + _mintAmount + reservedNfts <= maxSupply,
            "limit exceeded"
        );
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(ERC4907, ERC2981, ONFT721Core)
        returns (bool)
    {
        return
        ...
        returns (string memory)
    {
        ...
        return
            bytes(currentBaseURI).length > 0
                ? string(
                    abi.encodePacked(currentBaseURI, toString(tokenId), ".json")
                )
                : "";
    }
    ...
    function airdropNft(address[] calldata _sendNftsTo, uint256 _howMany)
        external
        onlyOwner
    {
        ...
        for (uint256 i = 0; i < _sendNftsTo.length; i++)
            mintNfts(_sendNftsTo[i], _howMany);
    }
    ...
    function set_notRevealedImagesLink(string memory _notRevealedImagesLink)
        public
        onlyOwner
    {
...
contract Nft is CyberSyndicate {
    ...
    function purchaseNft(
        ...
        require(
            numberMinted(msg.sender) + _howMany <= maxMintPresales[_rootNumber],
            "exceeds max allowed"
        );
