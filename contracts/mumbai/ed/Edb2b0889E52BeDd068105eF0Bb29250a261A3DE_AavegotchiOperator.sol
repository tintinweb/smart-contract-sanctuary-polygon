...
import { IAavegotchiDiamond, AavegotchiInfo } from "./interfaces/IAavegotchiDiamond.sol";

contract AavegotchiOperator is Initializable {
    ...
    address public _gelatoOpsAddress;
    ...
    modifier onlyGelato() {
        require(msg.sender == _gelatoOpsAddress, "Only Gelato can invoke this function");
        _;
    }
    ...
    function initialize(address aavegotchiDiamondContract, uint256 max_number_aavegotchis, address gelatoOps) public initializer {
        _gelatoOpsAddress = gelatoOps;
    ...
    function listAavegotchisToPetAndAddressesToRemove() external view returns (uint256[] memory tokenIds_, address[] memory revokedAddresses_) {
        ...
        for (uint256 i; i < enabledAddresses.length; i++) {
            ...
            if (isApproved == false) {
                ...
                for (uint256 j; j < aavegotchis.length && numberOfAavegotchis < _maxNumberOfAavegotchis; j++) {
                    ...
                    if (LibAavegotchiUtils.isAavegotchiPetAble(info.lastInteracted) == true) {
        ...
        uint tokenIdCounter;
        uint256[][] memory tokenIdsOfApprovedOwners = LibArrayUtils.shortenArray(tokenIdsOfOwners, numberOfApprovedAddresses);
        tokenIds_ = new uint256[](numberOfAavegotchis);
        for (uint256 i; i< tokenIdsOfApprovedOwners.length; i++) {
            uint256[] memory tokenIds = tokenIdsOfApprovedOwners[i];
