...
import { IMarketMakerV1b, FixedProductMarketMaker } from "./FixedProductMarketMaker.sol";
...
contract FixedProductMarketMakerFactory is Errors, IMarketFactoryEvents {
    using ArrayMath for uint256[];
    using SafeERC20 for IERC20Metadata;
    using Address for address;

    /// @notice Parameters unique to a single FPMM creation
    struct MarketParams {
        QuestionID questionId;
        uint256[] distributionHint;
        uint256 haltTime;
    }
    ...
    address private immutable fpmmTemplate;

    /// @notice Invalid funds parameter for create and fund
    error InvalidFundsLength();

    constructor() {
        fpmmTemplate = address(new FixedProductMarketMaker());
    }
    ...
    function createMarket(
        uint256 fee,
        // TODO: split between condition oracle, and price oracle. See UBET-1410
        FixedProductMarketMaker.AddressParams memory addresses,
        PriceMarketParams memory params
    ) public returns (FixedProductMarketMaker) {
        uint256 outcomeSlotCount = params.fairPriceDecimals.length;
        if (outcomeSlotCount == 0) revert InvalidDistributionHint();
        ...
        // conditionalTokens is where settlement is recorded. oracle is who is
        // the authority to decide outcome. haltTime and fee should be
        // adjustable on the market itself

        bytes32 salt = marketSalt(
        ...
        ConditionID conditionId = addresses.conditionalTokens.prepareCondition(
            addresses.priceOracle,
            params.questionId,
            outcomeSlotCount
        );
        FixedProductMarketMaker.InitParams memory initParams = FixedProductMarketMaker.InitParams(
            conditionId,
            params.haltTime,
            fee,
            params.fairPriceDecimals
        );

        // Check if clone already exists for this salt. If it does, then we have already created and initialized it
        address clone = Clones.predictDeterministicAddress(fpmmTemplate, salt);
        if (clone.isContract()) {
            return FixedProductMarketMaker(clone);
        }

        address cloneActual = Clones.cloneDeterministic(fpmmTemplate, salt);
        assert(cloneActual == clone); // this always has to be true

        FixedProductMarketMaker fpmm = FixedProductMarketMaker(clone);
        fpmm.initialize(addresses, initParams);
    ...
    ) public returns (FixedProductMarketMaker[] memory) {
        if (addedFunds.length != marketParamsArray.length) revert InvalidFundsLength();

        FixedProductMarketMaker[] memory fpmms = new FixedProductMarketMaker[](marketParamsArray.length);
        address receiver = msg.sender;

        uint256 totalFunds = addedFunds.sum();
        collateralToken.safeTransferFrom(receiver, address(this), totalFunds);

        FixedProductMarketMaker.AddressParams memory addresses = FixedProductMarketMaker.AddressParams(
            conditionalTokens,
            collateralToken,
            oracle
        );
        for (uint256 i = 0; i < marketParamsArray.length; ++i) {
            fpmms[i] = _createAndFundMarketFor(receiver, fee, addedFunds[i], addresses, marketParamsArray[i]);
        }

        return fpmms;
    }

    /// @notice Creates markets and funds them in a single call. This method is
    /// deprecated, so the implementation calls createAndFundMarketsWithPrices
    /// with equal prices. Will be removed in a future version.
    /// @param marketParamsArray unique parameters for every market
    function createAndFundMarkets(
        IConditionalTokens conditionalTokens,
        IERC20Metadata collateralToken,
        address oracle,
        uint256 fee,
        ...
        PriceMarketParams[] memory priceMarketParamsArray = new PriceMarketParams[](marketParamsArray.length);
        // create PriceMarketParams from MarketParams, with fairPriceDecimals corresponding to equal prices
        for (uint256 i = 0; i < marketParamsArray.length; ++i) {
            // generate equal prices
            uint256 outcomeSlotCount = marketParamsArray[i].distributionHint.length;
            if (outcomeSlotCount == 0) revert InvalidDistributionHint();

            uint256 equalPrice = AmmMath.ONE_DECIMAL / outcomeSlotCount;
            uint256[] memory fairPriceDecimals = new uint256[](outcomeSlotCount);
            for (uint256 j = 0; j < outcomeSlotCount - 1; ++j) {
                fairPriceDecimals[j] = equalPrice;
            }
            // adjust last price to account for rounding errors
            fairPriceDecimals[outcomeSlotCount - 1] = AmmMath.ONE_DECIMAL - (equalPrice * (outcomeSlotCount - 1));

            priceMarketParamsArray[i] = PriceMarketParams(
                marketParamsArray[i].questionId,
                fairPriceDecimals,
                marketParamsArray[i].haltTime
            );
        }

        return
            createAndFundMarketsWithPrices(
                conditionalTokens,
                collateralToken,
                oracle,
                fee,
                addedFunds,
                priceMarketParamsArray
            );
    }
    ...
    function predictMarketAddress(
        ...
        bytes32 salt = marketSalt(conditionalTokens, collateralToken, oracle, questionId, outcomeSlotCount);
        return Clones.predictDeterministicAddress(fpmmTemplate, salt);
    }
    ...
    function _createAndFundMarketFor(
    ...
    ) private returns (FixedProductMarketMaker) {
        FixedProductMarketMaker fpmm = createMarket(fee, addresses, marketParams);
        ...
        if (balances.hasNonzeroEntries()) {
            // if already funded, send funds back
            // TODO: UBET-1447 do refunds in bulk
            addresses.collateralToken.safeTransfer(receiver, addedFunds);
        } else {
            // otherwise fund it
            addresses.collateralToken.safeApprove(address(fpmm), addedFunds);
            uint256[] memory emptyHint;
            fpmm.addFundingFor(receiver, addedFunds, emptyHint);
        }
    ...
    function marketSalt(
        ...
        // TODO: optimize with assembly - just hash the calldata
        return keccak256(abi.encode(conditionalTokens, collateralToken, oracle, questionId, outcomeSlotCount));
    }
}
    ...
    Errors
{
        ...
        returns (uint256[] memory sendAmounts, uint256 collateralRemovedFromFeePool)
    {
        ...
        (costBasisReduction, sendAmounts) = FundingMath.calcDefunding(
            balanceOf(owner),
            sharesToBurn,
            funderCostBasis[owner],
            totalSupply(),
            getPoolBalances()
        );
    ...
    function addFundingFor(
        ...
        mintAmount = FundingMath.calcFunding(addedFunds, totalSupply(), getPoolBalances(), fairPriceDecimals);
        funderCostBasis[receiver] += addedFunds;
        totalFunderCostBasis += addedFunds;
    ...
    function calcBuyAmount(uint256 investmentAmount, uint256 indexOut) public view returns (uint256) {
        ...
        uint256 targetBalance = totalFunderCostBasis;
...
library FundingMath {
    ...
    function calcFunding(
        ...
        if (totalSupply == 0) {
            ...
            // brings to the pool, given the estimated fair prices
            uint256 poolValue = calcPoolValue(balances, fairPriceDecimals);

            // Something is very wrong if poolValue has gone to zero
            if (poolValue == 0) revert Errors.LogicError();
            mintAmount = (addedFunds * totalSupply).ceildiv(poolValue);
        }
    }

    function calcDefunding(
        uint256 funderShares,
        uint256 sharesToBurn,
        uint256 funderCostBasis,
        uint256 totalSupply,
        uint256[] memory balances
    ) internal pure returns (uint256 costBasisReduction, uint256[] memory sendAmounts) {
        sendAmounts = new uint256[](balances.length);

        for (uint256 i = 0; i < balances.length; i++) {
            sendAmounts[i] = (balances[i] * sharesToBurn) / totalSupply;
        }

        costBasisReduction = (funderCostBasis * sharesToBurn) / funderShares;
    }
}
