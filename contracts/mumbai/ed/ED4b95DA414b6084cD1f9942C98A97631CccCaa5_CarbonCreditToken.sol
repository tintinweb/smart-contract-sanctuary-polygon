...
// Although ERC677 has not became a general standard still it is a pragmatic choice due to
// simplicity in comparison to other ERC20 improvement attempts and compatibility to ERC20.

/**
 * @dev Interface of token receiver contract.
 */
...
/**
 * @dev Base class for tokens compatible
 * with https://github.com/ethereum/EIPs/issues/677.
 */
abstract contract ERC677 is ERC20 {
    ...
    function transferAndCall(address to, uint256 amount, bytes calldata data) external virtual returns (bool success) {
        address from = _msgSender();
        // transfer tokens
        _transfer(from, to, amount);
        // emit ERC677-specific transfer event
        emit Transfer(from, to, amount, data);
        // notify receiver
        if (to.isContract()) {
            IERC677Receiver(to).onTokenTransfer(from, amount, data);
        }
        return true;
    }
}
...
// File: contracts/tokens/CarbonCreditToken.sol
...
contract CarbonCreditToken is ERC677, ERC20Burnable, ERC20Snapshot, Ownable, Pausable, Blacklist {
