...
interface IDpalToken is IERC20 {
...
contract DpnsStaking is Ownable, IERC721Receiver, ReentrancyGuard, Pausable {
    using EnumerableSet for EnumerableSet.UintSet; 
    
    //addresses 
    IERC721 public dPnToken;
    IDpalToken public dpalToken;
    ...
    modifier requireDpalToken() {
        require(address(dpalToken) != address(0), "dpalToken not set");
        _;
    }

    modifier requireDpnToken() {
        require(address(dPnToken) != address(0), "dPnToken not set");
        _;
    }
...
/* STAKING MECHANICS */
    ...
    function setDpalTokenAddress(address dpalTokenAddr) public onlyOwner() {
      dpalToken = IDpalToken(dpalTokenAddr);
    }

    function setDpnTokenAddress(address dPnAddr) public onlyOwner() {
      dPnToken = IERC721(dPnAddr);
    }
      ...
      returns (uint256[] memory)
    {
      ...
      uint256[] memory depositSetOtherContract = dpalToken.depositsOf(account);
    ...
    {
      uint256[] memory tokensInOldContract = dpalToken.depositsOf(account);
      ...
      returns (uint256[] memory rewards) 
    {
      ...
      for (uint256 i; i < tokenIds.length; i++) {
        ...
        uint256 lastBlock = arrayContains(tokensInOldContract, tokenId) ? 
          dpalToken._depositBlocks(account, tokenId) 
    ...
    function claimRewards(uint256[] calldata tokenIds) public whenNotPaused requireDpalToken {
      uint256 reward; 
      uint256 blockCur = Math.min(block.number, expiration);
      uint256[] memory tokensInOldContract = dpalToken.depositsOf(msg.sender);
      ...
      dpalToken.claimRewards(tokensInOldContract);

      if (reward > 0) {
        dpalToken.transfer(msg.sender, reward);
      }
    }

    //deposit function. 
    function deposit(uint256[] calldata tokenIds) external whenNotPaused requireDpnToken requireDpalToken {
        require(msg.sender != address(dpalToken), "Invalid address");
        claimRewards(tokenIds);
        for (uint256 i; i < tokenIds.length; i++) {
            dPnToken.safeTransferFrom(
                msg.sender,
                address(this),
                tokenIds[i],
                ""
    ...
    //withdrawal function.
    function withdraw(uint256[] calldata tokenIds) external whenNotPaused requireDpnToken requireDpalToken nonReentrant() {
        claimRewards(tokenIds);
    ...
    function withdrawTokens() external onlyOwner {
        uint256 tokenSupply = dpalToken.balanceOf(address(this));
        dpalToken.transfer(msg.sender, tokenSupply);
    }
