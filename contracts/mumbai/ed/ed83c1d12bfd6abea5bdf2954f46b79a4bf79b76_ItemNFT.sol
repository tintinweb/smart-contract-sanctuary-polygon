...
contract ItemNFT is BaseNFT {
  ...
  function mintUserItems(address[] calldata _owners, uint8[] calldata _genes, uint16[] calldata _classIds, uint _price) onlyMintAdmin external {
    for(uint i = 0; i < _owners.length; i++) {
      address owner = _owners[i];
      uint16 classId = _classIds[i];
      uint8 gene = _genes[i];
      mintItem(owner, gene, classId, _price, i);
    }
  }
