...
contract GasMonster is ERC721A, ChainlinkInfrastructureClient, TokenURIResolver, PausableMint, Withdrawable, Royalty {
	...
	modifier presaleOrPublicSale(uint256 quantity, bytes calldata signature) {
		require(block.timestamp > preSaleTimestamp, "Sale is not opened yet");
		require(balanceOf(msg.sender) + quantity <= 20, 'GasMonster: You are allowed to get only 20 NFTs');
		
		// Presale mode
		if (block.timestamp < publicSaleTimestamp) {
			// Verify signature
			require(SignerVerification.isMessageVerified(msg.sender, signature, _addressToString(msg.sender)), 'Presale signature is wrong');
	...
	function mint(uint256 quantity, bytes calldata signature) external payable whenNotPaused presaleOrPublicSale(quantity, signature) {
	...
	function testChainlinkRequest() external {
		...
		request.add('get', string(abi.encodePacked('https://my-json-server.typicode.com/lamerest/mock-backend/canUpgrade/0')));
		request.add('path', 'data');
