    ...
    VRFConsumerBaseV2
{
    ...
    function mint(
        ...
        if (!hasRole(CLAIMER_ROLE, _msgSender())) {
            require(msg.value == mintPrice * mintAmount, "Invalid msg value");
            require(
            ...
            if (subcollection.whitelistMintEnabled) {
                require(
                    _verifyProof(
                        subcollectionName,
                        keccak256(
                            bytes.concat(keccak256(abi.encode(_msgSender(), mintPrice)))
                        ),
                        proof
                    ),
                    "Not on whitelist"
...
interface ICollectionToken  {
    ...
    function mint(string memory subcollectionName, address to, bytes32[] memory proof, uint256 mintAmount, uint256 mintPrice) external payable;
