...
/**
 * @notice The Vault contract. Holds all deposited funds with minimal additional logic
 */
contract Vault is IVault, Initializable, OwnableUpgradeable, PausableUpgradeable {
    // Events //

    event LiquidityPoolSenderUpdated(address liquidityPoolSender);
    ...
    */
    ...
    address public liquidityPoolSender;
    ...
    function setLiquidityPoolSender(address _liquidityPoolSender) public onlyOwner {
        liquidityPoolSender = _liquidityPoolSender;
        emit LiquidityPoolSenderUpdated(liquidityPoolSender);
    }
    ...
    function transfer(
        ...
        require(
            msg.sender == owner() || msg.sender == liquidityPoolSender,
            "Vault: not owner or LPSender"
        );
        if (token == NATIVE) {
    ...
    function pause(bool trigger) public onlyOwnerOrOperator {
        if (trigger) _pause();
        else _unpause();
    }
}