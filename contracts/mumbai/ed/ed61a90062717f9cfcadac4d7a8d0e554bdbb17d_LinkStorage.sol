...
// File: contracts/LinkStorage.sol
...
/**
 * @title URI Storage
 * @dev Store & retrieve Links in a mapping variable by ID
 */
contract LinkStorage is Ownable{

    mapping(uint => string) public link;
    ...
    /**
     * @dev Store link in an ID mapping
     * @param _id unique link ID
     * @param _name unique link name
     * @param _uri link
     */
    function storeLink(uint _id, string memory _name, string memory _uri) public onlyAdmins{
        link[_id] = _uri;
        linkFromName[_name] = _uri;
    }

    /**
     * @dev Return Link from an ID 
     * @return link from _id
     */
    function retrieveLinkByID(uint _id) public view returns (string memory){
        return link[_id];
    }
