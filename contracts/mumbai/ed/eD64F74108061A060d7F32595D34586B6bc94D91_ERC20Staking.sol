...
contract ERC20Staking is ReentrancyGuard {
    ...
    event stakeToken(uint256 indexed stakingId, address stakerAddress, uint256 date);
    event withdrawToken(uint256 indexed stakingId, address stakerAddress, uint256 date);
    event fulfillRequest(uint256 indexed stakingId, uint256 date);
    event reStakeToken(uint256 indexed stakingId, address stakerAddress, uint256 date);
    ...
    modifier onlyFulfiller() {
        require(msg.sender == fulfiller, "You are not the fulfiller.");
        _;
    }
    ...
    function stake(uint256 _stakingId, uint256 _amount) external nonReentrant {
        ...
        updateUnclaimedRewards(msg.sender);

        // Transfer the token from the wallet to the Smart contract
        stakingToken.transferFrom(msg.sender, address(this), _amount);

        // Write the stake action into blockchain
        stakeAction(msg.sender, _stakingId, _amount);
    ...
    function fulfill(uint256 _stakingId, uint256 _rewardRate, uint256 _duration) external onlyFulfiller {

        staked[_stakingId].rewardRate = _rewardRate;
        staked[_stakingId].duration = _duration;
        emit fulfillRequest(_stakingId, block.timestamp);
    }
    ...
    function withdraw(uint256 _stakingId) external nonReentrant {
        ...
        // Write the withdraw action into blockchain
        withdrawAction(msg.sender, _stakingId, withdrawAmount);
    ...
    function reStake(uint256 _newStakingId, uint256 _oldStakingId) external nonReentrant {
        ...
        require(
            block.timestamp >= staked[_oldStakingId].stakingStartDate + staked[_oldStakingId].duration,
            "You are still in a lock period."
        );
        // Checking for oldStakingId

        uint256 totalRewards = availableRewards(msg.sender);
        uint256 oldAmount = staked[_oldStakingId].amount;
        stakeAction(msg.sender, _newStakingId, oldAmount + totalRewards);
        withdrawAction(msg.sender, _oldStakingId, oldAmount);
        stakers[msg.sender].timeOfLastUpdate = block.timestamp;
        stakers[msg.sender].unclaimedRewards = 0;
        emit reStakeToken(_newStakingId, msg.sender, block.timestamp);
    ...
    function availableRewards(address _staker) public view returns (uint256) {
        uint256 rewards = calculateAllRewards(_staker)+ stakers[_staker].unclaimedRewards;
        return rewards;
    }
    ...
    function calculateAllRewards(address _staker) internal view returns (uint256){
        ...
        if (stakers[_staker].numberStaked == 0) {
            return 0;
        }
        ...
        for (uint256 k = 0; k < length; k++){
            ...
            // Check if staking is already finished
            if (thisTimeOfLastUpdate > thisStakingStartDate + thisDuration) {
                continue;
            }

            // Calculate the APY to rewardsPerHour
            uint256 rewardsPerYear = thisamount * thisRewardRate / 100;
            uint256 rewardsPerHour = rewardsPerYear / 8760;
            // Setup amount variable
            uint256 amount = 0;

            if (block.timestamp > thisStakingStartDate + thisDuration) {
                // If the staking is ended
                uint256 timeStaked = thisStakingStartDate + thisDuration - thisTimeOfLastUpdate;
                amount =  timeStaked * rewardsPerHour / 3600;
                rewards += amount;
            } else {
                // If the staking is not ended
                uint256 timeStaked = block.timestamp - thisTimeOfLastUpdate;
                amount = timeStaked * rewardsPerHour / 3600;
                rewards += amount;
            }
        }
    ...
    function updateUnclaimedRewards(address _userAddress) internal {
        // Check the number of stake is greater than 0
        if (stakers[_userAddress].numberStaked > 0) {
            // Update the unclaimedRewards
            stakers[_userAddress].unclaimedRewards += calculateAllRewards(_userAddress);
        }
    }

    function stakeAction(address _userAddress, uint256 _stakingId, uint256 _amount) internal {
        // Create stakingInfo
        // Set the initial lock period to 2 days
        // Set the initial reward rate to 0
        StakingInfo memory stakingInfo = StakingInfo(_userAddress, true, _amount, 0, 172800, block.timestamp);

        // Add the token to the stakedTokens array
        staked[_stakingId] = stakingInfo;
        stakers[_userAddress].listStakingId.push(_stakingId);

        // Increment the amount staked for this wallet
        stakers[_userAddress].amountStaked += _amount;
        stakers[_userAddress].numberStaked ++;

    }

    function withdrawAction(address _userAddress, uint256 _stakingId, uint256 withdrawAmount) internal {
        
        // Set isActive to false
        staked[_stakingId].isActive = false;

        // Decrement the amount staked for this wallet
        stakers[_userAddress].amountStaked -= withdrawAmount;
        stakers[_userAddress].numberStaked--;

        // Update the mapping of the tokenId to the be address(0) to indicate that the token is no longer staked
        for (uint256 i = 0; i < stakers[_userAddress].listStakingId.length; i++) {
            if (stakers[_userAddress].listStakingId[i] == _stakingId) {
                removeByIndexInArray(_userAddress, i);
                break;
            }
        }
    }

    // Remove an index in array
    function removeByIndexInArray(address _userAddress, uint256 _index) internal {
        uint256[] memory _listStakingId = stakers[_userAddress].listStakingId;
        stakers[_userAddress].listStakingId[_index] =  _listStakingId[_listStakingId.length - 1];
        stakers[_userAddress].listStakingId.pop();
    } 
