...
contract DemoSenderAndReceiver is IAny2EVMMessageReceiver,AccessControlEnumerable {
    ...
    RegistryConsumer constant galaxisRegistry  = RegistryConsumer(reg);
    ...
    event MessageReceived(bytes32 messageId, bytes sender, bytes data);

    constructor()  { 
        s_chains[16015286601757825753] = "sepolia";
        s_chains[12532609583862916517] = "mumbai";
    ...
    function sendMessage(
        ...
        OmniSender omniSender = OmniSender(0x789d7f3e2eaA6de41133A7fB11d7390603645F31);
        require(
            omniSender.hasRole(CONTRACT_ADMIN,address(this)),
            "DemoSenderAndReceiver : OmniSender does not authorise this address"
        );
        ...
        require(
            IERC20(galaxisRegistry.getRegistryAddress("CCIP_FEE_TOKEN")).balanceOf(address(omniSender)) > fee,
            "DemoSenderAndReceiver : OmniSender has too low a balance"
        );
        omniSender.sendDataPayFeeToken(
    ...
    {
        emit MessageReceived(message.messageId,message.sender,message.data);
    }
    ...
    modifier onlyRouter() {
        require(msg.sender == galaxisRegistry.getRegistryAddress("CCIP_ROUTER"),"DemoSenderAndReceiver : Unauthorised");
        _;
    }
