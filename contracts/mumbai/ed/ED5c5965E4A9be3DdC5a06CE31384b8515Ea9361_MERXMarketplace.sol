...
contract MERXMarketplace is ReentrancyGuard, Pausable, AccessControl {
    ...
    uint256 public _expTime = 4320 minutes;
    // --------------------- Struct Process --------------------- //
    enum MarketType {
        Marketplace,
        Auction
    ...
    struct Order {
        ...
        uint256 insuranceRate; // insurance rate when createOrder
        bool isActive;
    ...
    function createOrder (
        ...
        emit CreateOrderEvent(nftContract, orderId, tokenId, expiration, _refundPrice, msg.sender, startPrice, tokenAddress, marketType);
        if(marketType == MarketType.Auction){
    ...
    function buyOrder( // terminate buy when it's auction
        ...
        uint256 toSeller = price * (100 - orderData.insuranceRate); // to seller
        uint256 toInsurance = price * (orderData.insuranceRate);
        ...
        IERC20(orderData.tokenAddress).transferFrom(msg.sender, _poolWallet, toInsurance); // to pool insurance
    ...
    function winnerAcceptBid(uint256 orderId) public nonReentrant {
        ...
        uint256 toSeller = orderData.currentPrice * (100 - orderData.insuranceRate);
        uint256 toInsurance = orderData.currentPrice * (orderData.insuranceRate);
        uint256 fee = toSeller * _feeRate / 100;
        require(orderData.isActive, "Order isn't active");
        ...
        IERC20(orderData.tokenAddress).transferFrom(msg.sender, _poolWallet, toInsurance);
        IERC721(orderData.nftContract).setApprovalForAll(msg.sender, true);
