...
contract ItemAcquisitionHistory is IItemAcquisitionHistory, GameBase {
    constructor(address game)
    GameBase(game) {
    }

    function getLastAcquisitionTimestamps(uint256 worldId, uint256 tokenId, uint256[] calldata itemDefinitionIds) external view override returns(uint256[] memory) {
        World world = getWorld(worldId);

        return world.itemStorage().getLastAcquisitionTimestamps(tokenId, itemDefinitionIds);
    }
}
