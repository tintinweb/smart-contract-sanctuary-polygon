...
contract lbtestmwv2 is VRFV2WrapperConsumerBase,ConfirmedOwner,ReentrancyGuard {
	using SafeMath for uint256;
    ...
    IERC20 wmatic = IERC20(0x5B67676a984807a212b1c59eBFc9B3568a474F0a); // WMATIC
    ...
    address wmaticTrading = 0x5B67676a984807a212b1c59eBFc9B3568a474F0a;

    address dailyContract;

    uint256 minimumPot = 100 * 10**18;
    ...
    uint256 prizeValueJackpot = wmatic.balanceOf(address(this)); // 47.5% of ticket bought - added every new buy - filled by prizeValue
    ...
    function setNFTPay(uint256 _percentage) public onlyOwner() {
        ...
        botReward = prizeValueJackpot.mul(_percentage).div(1000);
    ...
    function autoSpin(address _caller, uint256[] calldata _nodes) public nonReentrant {
        ...
        require(block.timestamp > lastAutoSpin + 604800, "draw timestamp still not reached");   // 604800 1 week > 605100 1 week+5mins
         ...
         if(getJackpotinUSD() >= minimumPot){ 

            if(lbNodes.getUserActivation(_caller) == false){
                lbNodes.activateNodes(_caller,_nodes,3,1);
            }

            if(block.timestamp > lastAutoSpin + 605100 && reset == false){   // if no node activates in 5 minutes period queue resets
                lbNodes.resetQueue(3,1);
                reset = true;
            } else if(lbNodes.updateNodeInfo(_caller,botReward,1,3) == true){
    ...
    function registerTickets(uint256 quantity,address user) external {
        ...
        prizeValueJackpot = wmatic.balanceOf(address(this));
     ...
     */
     function fulfillRandomWords(uint256 requestId, uint256[] memory _randomWords) internal override {
        ...
        wmatic.safeTransfer(lastWinner,prizeValueJackpot);
