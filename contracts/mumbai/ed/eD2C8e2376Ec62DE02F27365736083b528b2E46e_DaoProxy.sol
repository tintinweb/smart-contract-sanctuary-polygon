...
contract DaoProxy is BaseContract {
    ...
    function getBigNetAmountList(address[] memory _addressList) external view returns (uint256[] memory _amountList) {
        uint256 _num = _addressList.length;
        _amountList = new uint256[](_num);
        for (uint256 i = 0; i < _num; i++) {
            _amountList[i] = bigNetAmountList[_addressList[i]];
        }
    }
