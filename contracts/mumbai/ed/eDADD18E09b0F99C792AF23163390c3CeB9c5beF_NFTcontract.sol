...
Counters.Counter private _tokenIdCounter;
   ...
   event tokenMinted(uint256 tokenID, address to);
    ...
    modifier onlyAdmin{
        require(isAdmin[msg.sender]==true,"Caller is not the admin");
        _;

    }

    constructor(string memory _name, string memory _symbol,address _admin,uint _mintFee)ERC721(_name, _symbol) {
        isAdmin[_admin]=true;
        mintFee = _mintFee;
    ...
    {  
        ...
        require(msg.value==mintFee,"Minting fee INVALID");
        _safeMint(to, _tokenIdCounter.current());
        _setTokenURI(_tokenIdCounter.current(), uri);
        _tokenIdCounter.increment();
        info[_tokenIdCounter.current()] = tokenInfo(true,to);
        }else{
         address(this).call{value:mintFee};   
        _safeMint(to, _tokenIdCounter.current());
        _setTokenURI(_tokenIdCounter.current(), uri);
        _tokenIdCounter.increment();
        info[_tokenIdCounter.current()] = tokenInfo(true,to);}
    ...
    {
        require(isAdmin[msg.sender]==true || msg.sender == ownerOf(_tokenID),"Caller is not the admin nor the owner of token");
