...
import "./interface/IOazizOrganization.sol";


/**
 *      BASE:      ERC1155Base
 *      EXTENSION: override SignatureMintERC1155
 *
...
contract EventTicketsNFT is ERC1155Base, PrimarySale, SignatureMintERC1155 {

    address public organizationOwner;
    address public oazizOrganization;
     ...
     */
    mapping(string => uint256) public remainingSupplyLimit;
    /**
    ...
    constructor(
        ...
        oazizOrganization = _oazizOrganization;
    ...
    function setRemainingTokens(string memory externalId, uint256 quantity) external onlyOwner {
        remainingSupplyLimit[externalId] = quantity;
    }

    function getRemainingTokens(string memory externalId) public view returns (uint256) {
        return remainingSupplyLimit[externalId];
    }
    ...
    returns (address signer)
    {
        ...
        require(
            _req.quantity <= this.getRemainingTokens(_req.externalId),
            "Not enough remaining balance for this type of token"
        );
        remainingSupplyLimit[_req.externalId] -= _req.quantity;

        // Retrieve tokenId by request
        uint256 tokenIdToMint = _retrieveTokenId(_req);
 ...
 *  @notice           Mints tokens according to the provided mint request.
    ...
    payable
    {
        ...
        for (uint i = 0; i <= _reqs.length; i++) {
            this.mintWithSignature(_reqs[i], _signatures[i]);
        }
    }
    ...
    //
    //   #################### New/overrided internal functions
    //
    ...
    function _beforeTokenTransfer(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            require(ids[i] != 0, "This contract can not use tokenId=0");
        }
    ...
    function _collectPriceOnClaim(
        ...
        address oazizPayoutAddress = IOazizOrganization(oazizOrganization).getPayoutAddress();
        address orgPayoutAddress = IOazizOrganization(organizationOwner).getPayoutAddress();

        address commissionRecipient = oazizPayoutAddress == address(0) ? primarySaleRecipient() : oazizPayoutAddress;
        CurrencyTransferLib.transferCurrency(_currency, msg.sender, commissionRecipient, oazizCommission);

        address saleRecipient = orgPayoutAddress == address(0) ? primarySaleRecipient() : orgPayoutAddress;
        CurrencyTransferLib.transferCurrency(_currency, msg.sender, saleRecipient, totalPrice - oazizCommission);
    }

    /// @dev Collects and distributes the primary sale value of NFTs being claimed.
    function _retrieveTokenId(MintRequest calldata _req)
    internal
    returns (uint256 tokenIdToMint) {

        tokenIdToMint = tokenIdByTicketExternalId[_req.externalId];
        uint256 nextIdToMint = nextTokenIdToMint();


        if (tokenIdToMint == 0 && _req.tokenId == type(uint256).max) {
            tokenIdToMint = nextIdToMint;
            tokenIdByTicketExternalId[_req.externalId] = tokenIdToMint;
            nextTokenIdToMint_ += 1;
        } else {
            if (tokenIdToMint == 0) {
        ...
        require(tokenIdToMint != 0, "TokenId should be choosen");
    }

    //    function getTokenizeCommunityList(){}
    //    function checkTicket(){}
    //    function cancelTicket(){} How it should be work if user buy more than one ticket?
}
...
abstract contract SignatureMintERC1155 is EIP712, ISignatureMintERC1155 {
    ...
    bytes32 internal constant TYPEHASH =
        keccak256(
            "MintRequest(address to,address royaltyRecipient,uint256 royaltyBps,address primarySaleRecipient,uint256 tokenId,string uri,uint256 quantity,uint256 pricePerToken,address currency,uint128 validityStartTimestamp,uint128 validityEndTimestamp,bytes32 uid,string externalId)"
        );
