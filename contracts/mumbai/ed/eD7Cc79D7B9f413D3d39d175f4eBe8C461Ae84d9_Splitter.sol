...
contract Splitter is IERC777Recipient, Initializable, BlackHolePrevention {
    ...
    */
    ...
    /* 
    When ETH/DUST cannot be split evenly, the remainder is sent out one by one to the addresses
    ...
    this address upon the next uneven split. We keep track of bot ETH and DUST last payed indices.
    */
    uint256 _ethLastPlayedIndex = 0;
    uint256 _dustLastPayedIndex = 0;
    ...
    function _splitDust(uint256 _amount) private {
        ...
        // split remainder
        uint256 remainder = _getDustContract().balanceOf(address(this));
        for (uint256 i = 0; i < remainder; i++) {
            _getDustContract().send(wallets[(_dustLastPayedIndex + i) % wallets.length], 1, "");
        }
        // set last payed index
        _dustLastPayedIndex = (_dustLastPayedIndex + remainder) % wallets.length;
    }
