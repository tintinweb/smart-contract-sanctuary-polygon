...
library NftLib {
    function mintRequirements(uint256 id, address _entity) internal view returns (bool){
        require((id >= 1 && id <= 14), "Id out of range");
          if (id == 1) {
            require(FactoryLib.checkBytes2Game(_entity), "You must pass level 1");
        } else if (id == 2) {
            require(FactoryLib.checkFallbackGame(_entity), "You must pass level 2");
        } else if (id == 3) {
            require(
            ...
            require(FactoryLib.checkFactoryGame(_entity), "You must pass level 14");
        }
            return true;    
    }
}
...
contract NFTbadge is Initializable, ERC1155Upgradeable, OwnableUpgradeable {
    ...
    function setURI(uint256 _id, string memory _uri) external onlyOwner {
        string memory fullURI = string(abi.encodePacked("ipfs://", _uri, "/", Strings.toString(_id), ".json"));
