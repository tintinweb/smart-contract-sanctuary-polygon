...
contract LittlebitsNFT is VRFConsumerBaseV2, ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    uint private constant ME_MINT_PRICE = 0.01 ether;
    uint private ME_SUPPLY = 500;
    uint public lootboxesReserved = 0;

    // whitelist
    bool private whitelistProtectionEnabled = true;
    mapping(address => bool) public whitelistStatus;
    uint public whitelistCounter = 0;
    ...
    uint64 constant LINK_SUBID = 2858;                                                                  // MUMBAI: subscription ID 
    ...
    function ADMIN_setWhitelistProtection(bool state) public onlyOwner {
        whitelistProtectionEnabled = state;
    }

    function ADMIN_setWhitelistAddresses(address[] memory addresses) public onlyOwner {
        for (uint i = 0; i < addresses.length; i++) {
            if (whitelistStatus[addresses[i]]) {
                continue;
            } 
            whitelistStatus[addresses[i]] = true;
            whitelistCounter += 1;
        }
    }
    ...
    function _mintToken(address to) private {
        ...
        if (whitelistProtectionEnabled) {
            require(whitelistStatus[to], "Address not whitelisted");
        }
