...
contract INFINITYAPP is Owned {
    ...
    constructor() Owned (0x803c698f680e702609fb09F7782bEA2Ccd3CbdA0) public {
        address ownerAddress = 0x803c698f680e702609fb09F7782bEA2Ccd3CbdA0;
        founders = [0xDE2d50B49D1509b87eE615f7dD3d7Df11b64a281, 0xe8Bb290eeAD75E1b4f616bF0c99B7f60cf20ba93 , 0xa03E46259e7197b8623661905c081FaC9903d3e6 , 0xB705c8A741344328DbB31b108eCd93b5a7284b5C , 0x43A75F303DEa0384deaF6a11c37258b5Ac3686a8 , 0x1a376ADCf7B07eb941eE9d7807d7007Fd57889e4 , 0x36e4a289BF4176bBF0d63b9ED6D5F1142062d2eA , 0x744D5Fc65a61A859AD9e32dbcBbD5937161B63fE];
        tokenAddress = 0x4bb4978012ca5810Ac6E15D324aF99A5678d2fc5;
    ...
    function changeFounder(uint id, address founder) public onlyOwner returns(bool) {
        require(id<founders.length, "id must be less than count of founders");
        founders[id] = founder;
        return true;
    }
    ...
    function receiveTransferWithToken(address from, uint tokens, bytes memory data) public returns (string memory status) {
        require(msg.sender == tokenAddress, "Unknown token");
        ...
        if (data.length == 0) { //registration, purchase or renew cost
            ...
            if (!isUserExists(from)) {
                if (!paymentQueue[from].is_eth_paid) {
                    ...
                    paymentQueue[from].is_eth_paid = false;
                    registrationWithToken(from, paymentQueue[from].referrer);
                    result = "Registration is completed";
                }
            } else {
                require(users[from].matrices_cnt < 150 || paymentQueue[from].matrix_to_renew != 0, "max 150 hpeasy allowed");
                ...
                if (!paymentQueue[from].is_eth_paid) {
                    ...
                    paymentQueue[from].is_eth_paid = false;
                    if (paymentQueue[from].matrix_to_renew != 0) {
                        renewWithToken(from, paymentQueue[from].matrix_to_renew);
                        paymentQueue[from].matrix_to_renew = 0;
                        result = "Renew is completed";
                    } else {
                        purchaseWithToken(from);
                        result = "Purchase is completed";
                    }
                }
            }
            ...
            if (!jackpotQueue[from].is_eth_paid) {
                ...
                require(jackpotQueue[from].line == line && jackpotQueue[from].bet_size == tokens, "wrong line or bet size");
                result = addJackpotBetWithToken(jackpotQueue[from].line, jackpotQueue[from].bet_size);
                delete jackpotQueue[from];
            }
        }
    ...
    function registerWithToken(address referrer) public payable {
        require(msg.value == regCost, "not correct registration cost");
        require(!isUserExists(msg.sender), "user exists");
        require(isUserExists(referrer), "referrer not exists");
        require(!paymentQueue[msg.sender].is_eth_paid, "already paid registration cost in ETH, tokens awaiting");
        ...
        if (!useTokenPayment) {
            registrationWithToken(msg.sender, referrer);
        } else if (!paymentQueue[msg.sender].is_tokens_paid) {
            paymentQueue[msg.sender].is_eth_paid = true;
            paymentQueue[msg.sender].referrer = referrer;
        } else {
    ...
    function purchaseHpEasyPositionWithToken() public payable {
        require(msg.value == regCost, "not correct purchase cost"); 
        require(isUserExists(msg.sender), "user not exists");
        require(users[msg.sender].matrices_cnt < 150, "max 150 hpeasy allowed");
        require(!paymentQueue[msg.sender].is_eth_paid, "already paid in ETH, tokens awaiting");
    ...
    function renewMatrixWithToken(uint matrixToRenew) public payable {
        require(msg.value == regCost, "not correct renew cost"); 
        require(matrices[matrixToRenew].bunch_cycles >= bunchCyclesLimit, "matrix is active");
        require(isUserExists(msg.sender), "user not exists");
        require(!paymentQueue[msg.sender].is_eth_paid, "already paid in ETH, token payment awaiting");
        ...
        if (!useTokenPayment) { 
            renewWithToken(msg.sender, matrixToRenew);
        } else if(!paymentQueue[msg.sender].is_tokens_paid) {
            paymentQueue[msg.sender].is_eth_paid = true;
            paymentQueue[msg.sender].matrix_to_renew = matrixToRenew;
        } else {
    ...
    function registrationWithToken(address userAddress, address referrerAddress) private {
        users[userAddress] = User({
            id: lastUserId,
            referrer: referrerAddress,
            matrices_cnt: 0,
        ...
        payUserWithToken(referrerAddress, regCost * 10 / 100); 
        payFoundersWithToken(regCost * 10 / 100); 
        joinHpEasyWithToken(lastMatrixId, userAddress, false);
        fillMatrixWithToken(userAddress, referrerAddress, 0);
    ...
    function purchaseWithToken(address userAddress) private  {
        if (users[userAddress].hpeasy_cooldown_time < now - 86400) {
            users[userAddress].hpeasy_cooldown_time = now;
            users[userAddress].hpeasy_cooldown_num = 1;
        } else {
        ...
        payUserWithToken(users[userAddress].referrer, regCost * 10 / 100); 
        payFoundersWithToken(regCost * 10 / 100); 
        joinHpEasyWithToken(lastMatrixId, userAddress, false); 
        fillMatrixWithToken(userAddress, users[userAddress].referrer, 1);
    }
    ...
    function renewWithToken(address userAddress, uint matrixToRenew) private  {
        payUserWithToken(users[userAddress].referrer, regCost * 10 / 100); 
        payFoundersWithToken(regCost * 10 / 100); 
        joinHpEasyWithToken(matrixToRenew, userAddress, true); 
        payForMatrixWithToken(matrices[matrixToRenew].matrix_referrer);
    }
    ...
    function joinHpEasyWithToken(uint matrixId, address matrixOwner, bool isRenew) private {
        HpEasy[lastHpEasyId] = HpEasyLine({
            matrix_id: matrixId,
            owner: matrixOwner
            });
        ...
        if (lastHpEasyId % 2 == 0) {
            if (lastHpEasyId > 2) {
                ...
                payHpEasyWithToken(HpEasy[currentHp].owner);
                joinHpEasyWithToken(HpEasy[currentHp].matrix_id, HpEasy[currentHp].owner, false);
                payForMatrixWithToken(matrices[HpEasy[currentHp].matrix_id].matrix_referrer);
            } else {
                payHpEasyWithToken(owner);
                payForMatrixWithToken(0);
            }
        }
    }
    ...
    function payForMatrixWithToken(uint slotId) private {
        if (slotId == 0) {
            payUserWithToken(matrices[1].owner, regCost * 25 / 100); 
            return;
        }
        ...
        if (users[matrices[level1].owner].direct_referrals < 4 || matrices[level1].bunch_cycles >= bunchCyclesLimit) { 
            ...
            payUserWithToken(matrices[level1].owner, regCost * 25 / 1000);
            lastEligible = matrices[level1].owner;
        }
        ...
        if (level2 == 0) {
            if (lastEligible != address(0)) {
                payUserWithToken(lastEligible, regCost * 225 / 1000); 
            } else {
                payUserWithToken(matrices[1].owner, regCost * 25 / 100); 
            }
            return;
            ...
            payUserWithToken(matrices[level2].owner, unspent + regCost * 5 / 100);
            lastEligible = matrices[level2].owner;
            unspent = 0;
        }
        ...
        if (level3 == 0) {
            if (lastEligible != address(0)) {
                payUserWithToken(lastEligible, regCost * 175 / 1000 + unspent); 
            } else {
                payUserWithToken(matrices[1].owner, regCost * 25 / 100); 
            }
            return;
            ...
            payUserWithToken(matrices[level3].owner, unspent + regCost * 75 / 1000);
            lastEligible = matrices[level3].owner;
            unspent = 0;
        }
        ...
        if (level4 == 0) {
            if (lastEligible != address(0)) {
                payUserWithToken(lastEligible, regCost * 10 / 100 + unspent); 
            } else {
                payUserWithToken(matrices[1].owner, regCost * 25 / 100); 
            }
            return;
            ...
            payUserWithToken(matrices[level4].owner, unspent + regCost * 10 / 100);
            lastEligible = matrices[level4].owner;
            unspent = 0;
        }
        ...
        if (unspent == regCost * 25 / 100) {
            while (users[matrices[level4].owner].direct_referrals < 4 || matrices[level4].bunch_cycles >= bunchCyclesLimit) {
                if (level4 == 0) {
                    payUserWithToken(matrices[1].owner, unspent);
                    return;
                }
            ...
            payUserWithToken(matrices[level4].owner, unspent);
        } else if (unspent != 0) {
            payUserWithToken(lastEligible, unspent);
        }
    }
    ...
    function fillMatrixWithToken(address user, address referrer, uint from_hpeasy) private {
        if (referrer == address(0)) {
            referrer = usersById[1];
        }
    ...
    function payUserWithToken(address user, uint amount) private {
        payInTokensWithToken(user, amount);
    }
    ...
    function payHpEasyWithToken(address user) private {
        payInTokensWithToken(user, regCost * 30 / 100); 
    }
    ...
    function payFoundersWithToken(uint amount) private {
        uint founderAmount;
        ...
        for (uint i=0; i < founders.length; i++) {
            ...
            payInTokensWithToken(founders[i], founderAmount);
        }
    }
    ...
    function payInTokensWithToken(address to, uint amount) private returns (bool success) {
        // TODO
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = tokenAddress;
        uniswapV2Router.swapETHForExactTokens(amount, path, to, block.timestamp+1000);
        return true;
    }
    ...
    function addJackpotBetWithToken(uint line, uint betSize) private returns(string memory status) {
        bets[line][betSize].push(msg.sender);
        
        if (bets[line][betSize].length == line) { 
            defineWinnerWithToken(line);
            return ("This line winner is defined");
        } else {
            return ("Your bet is accepted");
        }
    ...
    function defineWinnerWithToken(uint line) private {  
        uint additionalSeed = uint(blockhash(block.number - 1)); 
        uint rnd = 0;
        ...
        payInTokensWithToken(winnerAddress, winnerAmount);
        emit NewJackpotWinner(line, msg.value, winnerAmount, winnerAddress);

        payFoundersWithToken(transactionalCost);
