...
contract Gomoku is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    address public gameOwner;

    // constructor() {
    //     gameOwner = owner();
    // }
    // string public string1 = "neko";//実験コード
    ...
    //盤の状態確認関数
    function getBoard() public view returns (uint8[19][19] memory){
        return board;
    }
    ...
    bool public publicstatus;// = false;//ゲームの公開状態 //initializerへ
    bool public gameOver;
    uint8 public playercount;// = 0; //initializerへ
    uint16 public turncount;// = 0;//initializerへ
    ...
    function putStone(uint8 x, uint8 y) public virtual onlyPlayer turnCheck {
...
contract GomokuV2 is Gomoku{
    //投了出来るようにする。

    enum Resignation{NoResult, BlackResign, WhiteResign}
    Resignation public gameResignation;
    ...
    function initialize() public override initializer {
        ...
        publicstatus = false;
        currentturn =0;
        gameResignation = Resignation.NoResult;
    }




    function resign() public onlyPlayer returns (Resignation) {
    
    if(msg.sender == players[0]){
        gameResignation = Resignation.BlackResign;
        return gameResignation;
        }else if (msg.sender == players[1]){
            gameResignation = Resignation.WhiteResign;
            return gameResignation;
        }else{
            return Resignation.NoResult;
        }
    ...
    function getVersion() public returns(string memory){
        string memory version = "GomokuV2";
        return version;
    }


    function putStone(uint8 x, uint8 y) public override onlyPlayer turnCheck {
    //function putStone(uint8 x, uint8 y) public onlyPlayer {

        require(gameResignation != Resignation.BlackResign, "Black resigned");
        require(gameResignation != Resignation.WhiteResign, "Black resigned");
