...
contract Trader_Testing is Ownable, ERC721, ERC721Holder{ 
    ...
    uint256 public mintPrice = 0;                   // minting price 0 ETH
    ...
    modifier isSaleActive(){
        require(saleIsActive == true, "Sale has not been started yet!.");
        _;
    }
    ...
    function mintByUser(uint256 quantity) external payable isPause isHalt isSaleActive{
    ...
    function mintByOwner(uint256 quantity) external isHalt onlyOwner isSaleActive {
    ...
    function getPaddedTokenID(uint256 tokenId) internal pure returns (string memory str){
        if(tokenId > 0 && tokenId < 10){
            str = string(abi.encodePacked('0', '0', '0', tokenId.toString()));
        }else if(tokenId > 9 && tokenId < 100){
            str = string(abi.encodePacked('0', '0', tokenId.toString()));
        }else if(tokenId > 99 && tokenId <= 999){
            str = string(abi.encodePacked('0', tokenId.toString()));
        }else if(tokenId == 1000){
            str = string(abi.encodePacked(tokenId.toString()));
        }
    } 
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if(revealed == false && (tokenId < giftMintId || tokenId > giftMintId + giftMintLimit)){
            ...
            return bytes(revealedUri).length > 0 ? string(abi.encodePacked(revealedUri, getPaddedTokenID(tokenId))) : "";
        }
    }
