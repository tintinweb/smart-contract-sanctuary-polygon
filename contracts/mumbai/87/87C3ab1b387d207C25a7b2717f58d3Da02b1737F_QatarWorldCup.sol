...
contract QatarWorldCup is ERC721Enumerable, Ownable {
    ...
    uint256 constant MAX_SUPPLY = 320;
    uint256 constant USER_MAX_SUPPLY = 256;
    uint256 constant MAX_SUPPLY_GOLD = 32;
    uint256 constant MAX_SUPPLY_SILVER = 96;
    uint256 constant MAX_SUPPLY_BRONZE = 192;
    ...
    uint256 constant GOLD_TEAM_FULL = 1;
    uint256 constant SILVER_TEAM_FULL = 4;
    uint256 constant BRONZE_TEAM_FULL = 10;
    ...
    constructor() ERC721("QATAR WORLDCUP", "QWC") {
        ...
        for(uint256 i = 0; i < teamCreating.length; i++){
            ...
            _imageIndex[teamCreating[i]][2] = 1;
            _imageIndex[teamCreating[i]][3] = 4;
        }
    }
    ...
    function mintBatch721Token(uint256 _numberOfTokens, uint256 _lowBreakpoint, uint256 _highBreakpoint) public payable {
        ...
        ( uint256 _tokenInRange, uint256 _tokenOverRange ) = _countNumberOfTokenInBreakpoint(_numberOfTokens, breakpoint[_lowBreakpoint], breakpoint[_highBreakpoint]);
        uint256 _mintPriceInRange = getMintPriceByBreakpoint(_highBreakpoint);
        if( _tokenOverRange == 0 ){
              require(_tokenInRange.mul(_mintPriceInRange) <= msg.value, "Ether value sent is not correct");
        }else{
            uint256 _mintPriceOverRange = getMintPriceByBreakpoint(_highBreakpoint++);
            require(_tokenInRange.mul(_mintPriceInRange).add(_tokenOverRange.mul(_mintPriceOverRange)) <= msg.value, "Ether value sent is not correct");
        }
    ...
    function getMintPriceByBreakpoint(uint256 _breakpoint) public view returns (uint256 _mintPrice){
            _mintPrice = mintPrice.sub(mintPrice.mul(discountByBreakpoint[breakpoint[_breakpoint]]).div(denominator));
    }
    ...
    function setBreakPoint(uint256 _breakpoint1, uint256 _breakpoint2, uint256 _breakpoint3, uint256 _breakpoint4, uint256 _breakpoint5) public onlyOwner{
        breakpoint[1] = _breakpoint1;
        breakpoint[2] = _breakpoint2;
        breakpoint[3] = _breakpoint3;
        breakpoint[4] = _breakpoint4;
        breakpoint[5] = _breakpoint5;
        breakpoint[6] = USER_MAX_SUPPLY;
    }
    ...
    function setDiscountLevel(uint256 _level_1, uint256 _level_2, uint256 _level_3, uint256 _level_4, uint256 _level_5) public onlyOwner{
        discountByBreakpoint[breakpoint[1]] = _level_1;
        discountByBreakpoint[breakpoint[2]] = _level_2;
        discountByBreakpoint[breakpoint[3]] = _level_3;
        discountByBreakpoint[breakpoint[4]] = _level_4;
        discountByBreakpoint[breakpoint[5]] = _level_5;
    }
    ...
    */
    ...
    function _countNumberOfTokenInBreakpoint(uint256 _numberOfToken, uint256 _lowBreakpoint, uint256 _hightBreakpoint) internal view returns ( uint256 _tokenInRange, uint256 _tokenOverRange){
        for(uint256 i = 0; i < _numberOfToken; i++ ){
            if(_lowBreakpoint <= currentTokenId.unsafe_add(i) && currentTokenId.unsafe_add(i) <= _hightBreakpoint){
                _tokenInRange++;
            }else{
                _tokenOverRange++;
            }
...
library UncheckedMath {
    /**
     * @dev Return unsafe increasing 1 unit of an unsigned integer
     *
     * _Available since v3.4._
     */
    function unsafe_increase(uint256 x) internal pure returns (uint) {
        unchecked { return x + 1; }
    }

    /**
     * @dev Returns the unsafe addition of two unsigned integers.
     * 
     *
     */
    function unsafe_add(uint x, uint y) internal pure returns (uint) {
        unchecked {
            return x + y;
        }
    }
    ...
    /**
     * @dev Returns the unsafe subtraction of two unsigned integers.
     * 
     *
     */
    function unsafe_sub(uint x, uint y) internal pure returns (uint) {
        unchecked {
            return x - y;
        }
    }
