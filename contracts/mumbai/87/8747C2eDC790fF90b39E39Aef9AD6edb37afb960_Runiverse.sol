...
contract Runiverse is Pausable, ReentrancyGuard, EIP712, RuniverseAdministrative, IDomainSeparator {
	...
	function startRun(uint256 gameId, SignedStartGame calldata player1, SignedStartGame calldata player2) external onlyStartRunOperator whenNotPaused nonReentrant {
        ...
        if(player2.useJolly==1)
        {
            IERC20(jollyTokenContract).safeTransferFrom(run.player2.playerAddress, address(this), bettedAmountPlayer1);
        }
        else
        {
    ...
    function getRunWinner(uint256 gameId) public view returns (address) {
        Run memory run = runs[gameId];
        return run.winner;
    }

    function getRunLoser(uint256 gameId) public view returns (address) {
        Run memory run = runs[gameId];
        return run.loser;
    }
...
abstract contract RuniverseFeeSharer is CryptorunNFTLinker  {
	using SafeMath for uint256;
	using SafeERC20 for IERC20;
