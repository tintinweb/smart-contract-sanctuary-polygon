...
contract Holder {
    ...
    function parseReceiver(address adr, uint start, uint end) public view returns (uint[] memory){
        
        uint[] memory toReturn;
        if(start > receivers[adr].size){
            toReturn = new uint[](0);
            return toReturn;
        }
        
        end = (end<receivers[adr].size) ? end : receivers[adr].size;

        toReturn = new uint[](end - start);
        for (uint i = start; i < end; i++) {
            toReturn[i-start] = receivers[adr].transactions[i];
        }
        return toReturn;
    ...
    function parseSender(address adr, uint start, uint end) public view returns (uint[] memory){
        uint[] memory toReturn;
        if(start > senders[adr].size){
            toReturn = new uint[](0);
            
            return toReturn;
        }
        
        end = (end<senders[adr].size) ? end : senders[adr].size;

        toReturn = new uint[](end - start);
        for (uint i = start; i < end; i++) {
            toReturn[i-start] = senders[adr].transactions[i];
        }
        return toReturn;
    ...
    function parsePreviousTransactionsTimestamps(uint txnId, uint start, uint end) public view returns (uint[] memory){

        uint[] memory toReturn;
        if (txnId >= transactionHolderSize || transactionsHolder[txnId].currentTransaction < start) {
            toReturn = new uint[](0);
            return toReturn;
        }
        
        end = end < transactionsHolder[txnId].currentTransaction ? end : transactionsHolder[txnId].currentTransaction ;
        toReturn = new uint[](end-start);

        for (uint i = start; i < end; i++) {
            toReturn[i-start] = transactionsHistory[txnId].previousTransactionsTimes[i];
        }
        return toReturn;
    }
