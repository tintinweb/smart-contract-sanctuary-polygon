...
contract CatAttackNFT is ERC1155LazyMint {
    ...
    event Smashed(address indexed attacker, address indexed victim, uint256 level);

    bool public isGamePaused = false;
    uint256 public constant MAX_POWERUPS = 100;
    ...
    function verifyClaim(
        ...
        require(
            ...
            "Already got a Kitten"
        );
        require(
            balanceOf[_claimer][1] == 0,
            "Already got a Grumpy cat"
        );
        require(
            balanceOf[_claimer][2] == 0,
            "Already got a Ninja cat"
        );
    }
    ...
    /**
     * @notice Use an attack power-up to attack two other players at once
     * @dev The power-up is burned after use
     * @param victim1 The first victim's address
     * @param victim2 The second victim's address
     */
    function useAttackPowerUp(address victim1, address victim2) external gameNotPaused {
        require(balanceOf[msg.sender][4] > 0, "You need an Attack power-up to perform this action");
        require(victim1 != address(0) && victim2 != address(0), "Invalid victim address");

        // Burn the attack power-up
        _burn(msg.sender, 4, 1);

        // Attack the first victim
        _attack(msg.sender, victim1);

        // Attack the second victim
        _attack(msg.sender, victim2);
    }

    /**
     * @notice Use a defense power-up to defend against two attacks
     * @dev The power-up is burned after use
     * @param attacker1 The address of the first attacker
     * @param attacker2 The address of the second attacker
     */
    function useDefensePowerUp(address attacker1, address attacker2) external gameNotPaused {
        require(balanceOf[msg.sender][5] > 0, "You need a Defense power-up to perform this action");
        require(attacker1 != address(0) && attacker2 != address(0), "Invalid attacker address");

        // Burn the defense power-up
        _burn(msg.sender, 5, 1);

        // Defend against the first attack
        _defend(msg.sender, attacker1);

        // Defend against the second attack
        _defend(msg.sender, attacker2);
    }

    /**
     * @notice Internal function to perform an attack
     * @param attacker The attacker's address
    ...
    function _attack(address attacker, address victim) internal {
        ...
        // Find the highest-level cat owned by the victim
        ...
        // Burn the victim's cat
        _burn(victim, highestLevelCat, 1);

        // Mint a badge of honor to the attacker
        _mint(attacker, 3, 1, "");

        emit Smashed(attacker, victim, highestLevelCat + 1);
    }

    /**
     * @notice Internal function to perform a defense
     * @param defender The defender's address
     * @param attacker The attacker's address
     */
    function _defend(address defender, address attacker) internal {
        // Only a grumpy cat owner can defend
        require(balanceOf[defender][1] > 0, "You need a Grumpy cat to defend!");

        // Find the highest-level cat owned by the defender
        uint256 highestLevelCat;
        for (uint256 i = 2; i >= 0; i--) {
        ...
        // Burn the attacker's cat
        _burn(attacker, highestLevelCat, 1);

        emit Smashed(defender, attacker, highestLevelCat + 1);
    }

    /**
     * @notice Get the score of a player
    ...
    function getScore(address player) public view returns (uint256) {
        ...
        return
            balanceOf[player][0] +
            2 * balanceOf[player][1] +
            3 * balanceOf[player][2] +
            balanceOf[player][3];
    }
