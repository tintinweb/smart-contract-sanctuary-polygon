...
contract DerbyStarsSoul is ERC721Enumerable {
    ...
    function getCRTBreedingFee(uint256 fatherTokenId) public view returns (uint256) {
        Properties memory fprop = derbyStarsHorse.getProperty(fatherTokenId);
        uint256 last = fprop.father_breed_timestamp;
        uint256 prev = fprop.breed_count_father;
        uint256 d = (block.timestamp - last) / 1 days;
        if (d >= prev) {
            fprop.breed_count_father = 0;
        } else {
            // this would not overflow
            require(d <= type(uint16).max, "Days calculation should not overflow");
            fprop.breed_count_father -= uint16(d);
        }
        return breedingCalculator.calculateBreedingFee(fprop, dsData);
    }
    ...
    function breed(uint256 motherTokenId, uint256 fatherTokenId) public {
        ...
        derbyStarsHorse.adjustFatherBreedingCount(fatherTokenId);
        Properties memory fprop = derbyStarsHorse.getProperty(fatherTokenId);
        ...
        // 5. increase breeding counts of motherTokenId 
        derbyStarsHorse.incrementMotherBreedingCount(motherTokenId);
        derbyStarsHorse.incrementFatherBreedingCount(fatherTokenId);
...
contract DerbyStarsHorse is ERC721, HorseProperty {
    ...
    function incrementMotherBreedingCount(uint256 tokenId) public {
        require(msg.sender == address(derbyStarsSoul));
        properties[tokenId].breed_count_mother += 1;
    }

    function adjustFatherBreedingCount(uint256 tokenId) public {
        require(msg.sender == address(derbyStarsSoul));
        uint256 last = properties[tokenId].father_breed_timestamp;
        uint256 prev = properties[tokenId].breed_count_father;
        uint256 d = (block.timestamp - last) / 1 days;
        if (d >= prev) {
            properties[tokenId].breed_count_father = 0;
        } else {
            // this would not overflow
            require(d <= type(uint16).max, "Days calculation should not overflow");
            properties[tokenId].breed_count_father -= uint16(d);
        }
    }

    function incrementFatherBreedingCount(uint256 tokenId) public {
        require(msg.sender == address(derbyStarsSoul));
        properties[tokenId].father_breed_timestamp = block.timestamp;
        properties[tokenId].breed_count_father += 1;
    }
...
contract BreedingCalculator {
    ...
    function calculateBreedingFee(
        ...
        {
            add_fee += dsData.getBreedingFee(uint256(FeeType.TALENT), father.dtrait.talent_rr);
            add_fee += dsData.getBreedingFee(uint256(FeeType.TALENT), father.dtrait.talent_fr);
            add_fee += dsData.getBreedingFee(uint256(FeeType.TALENT), father.dtrait.talent_st);
            add_fee += dsData.getBreedingFee(uint256(FeeType.TALENT), father.dtrait.talent_sr);
        }
        {
            uint256 special_rank = dsData.getSpecialSkillTier(father.dtrait.skill1);
            add_fee += dsData.getBreedingFee(uint256(FeeType.SPECIAL_SKILL), special_rank);
        }
        {
            ...
            add_fee += dsData.getBreedingFee(uint256(FeeType.NORMAL_SKILL), skill_rank_sum / 5);
        }
        if (father.breed_count_father == 0) {
            ...
            // TODO : rounding
            uint256 lhs = fee * (father.breed_count_father * 3 / 10) * father.breed_count_father;
            //(추가 비용 합산 + (추가 비용 합산 * (교배 횟수 * 교배 횟수 가중치) * 교배 횟수)
            add_fee = add_fee * 1e18;
            uint256 rhs = add_fee + (add_fee * (father.breed_count_father * 3 / 10) * father.breed_count_father);
            return fee + lhs + rhs;
        }
    }
