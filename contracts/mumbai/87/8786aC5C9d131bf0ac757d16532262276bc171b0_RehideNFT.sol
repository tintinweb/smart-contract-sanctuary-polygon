...
interface IRehideBase {
    ...
    event AddToWhitelist(address[] toAddAddresses, uint256 referrerRewardsPercentage);
    event RemoveFromWhitelist(address[] toRemoveAddresses);
}
...
abstract contract RehideBase is ERC721Enumerable, ERC721URIStorage, Pausable, Ownable, IRehideBase {
    ...
    uint256 public _readPlatformPercentage = 10; // wei = 0.001 ETH
    ...
    uint256 public _primaryReferrerPercentage = 10; // % to pay primary referrer
     ...
     * @dev Whitelist
     */ 
    mapping(address => uint256) public _whitelist; // rewards % for address
    Counters.Counter public _whitelistIds;
     ...
     */
    function addToWhitelist(address[] calldata toAddAddresses, uint256 referrerRewardsPercentage) external onlyOwner {
        require(referrerRewardsPercentage <= _maxReferrerRewardsPercentage, "RehideNFT: Referrer rewards exceed max allowed.");
        require(referrerRewardsPercentage >= _primaryReferrerPercentage, "RehideNFT: Referrer rewards less than default.");

        for (uint256 i = 0; i < toAddAddresses.length; i++) {
            _whitelist[toAddAddresses[i]] = referrerRewardsPercentage;
            _whitelistIds.increment();
        }
        emit AddToWhitelist(toAddAddresses, referrerRewardsPercentage);
    }
    ...
    function removeFromWhitelist(address[] calldata toRemoveAddresses) external onlyOwner {
        for (uint256 i = 0; i < toRemoveAddresses.length; i++) {
            if (_whitelist[toRemoveAddresses[i]] > 0) {
                _whitelist[toRemoveAddresses[i]] = 0;
                _whitelistIds.decrement();
            }
        }
        emit RemoveFromWhitelist(toRemoveAddresses);
    }
...
contract RehideNFT is IRehideNFT, RehideBase {
     ...
     * @dev Reads
     */
    mapping(address => uint256[]) internal _noteIds;
    mapping(uint256 => Note) public _notesMapping;
    mapping(uint256 => NoteRead[]) public _noteReads;
    mapping(uint256 => address[]) public _noteReadAllowlist;
    ...
    function mintNFT(
        ...
        if (_msgSender() != owner() && _whitelist[_msgSender()] < 0) {
        ...
        _noteIds[_msgSender()].push(newTokenId);
    ...
    function readSharedNote(uint256 tokenId) public payable returns (uint256 readsAvailable) {
        ...
        if (_msgSender() != owner() && _whitelist[_msgSender()] < 0 && ownerOf(tokenId) != _msgSender()) {
        ...
        if (readFee > 0) {
            ...
            payableCreator.transfer(readFee - platformFee);
            emit ReadSharedNoteRewardsTransferred(payableCreator, tokenId, readFee - platformFee);
        }
    }
    ...
    function getWalletNotes(address account)
        ...
        returns (Note[] memory)
    {
        uint256[] memory accountNoteIds = _noteIds[account];
        ...
        if (mintFee > 0) {
            ...
            if (referrer != _msgSender()) {
                ...
                if (primaryReferrer != address(0)) {
                    ...
                    uint256 whitelistReferrerRewardsPercentage = _whitelist[primaryReferrer];
                    uint256 primaryReferrerReward = mintFee.mul(_primaryReferrerPercentage).div(100);

                    // If on whitelist, give them special rewards
                    if (whitelistReferrerRewardsPercentage > 0) {
                        uint256 whitelistReferrerReward = mintFee.mul(whitelistReferrerRewardsPercentage).div(100);
