...
contract WorldEnterprise is IERC20, Authorizable, Initializable {
        ...
        if (_proposal.no < _proposal.yes && _voteThreshold <= _proposal.yes) {
            ...
            if (_proposal.pType == ProposalType.MINT) {
                if (joinPriceList[_proposalIndex] > 0) {
                    orderBook -= joinPriceList[_proposalIndex];
                }
    ...
    function createBuyOrder(uint256 amount, uint256 price) external payable {
        require(amount > 0, "WE: Amount should be greater than the zero"); //amount is in wei
        require(price > 0, "WE: Price is zero"); // price is total amount * price per share
        require(
            price == msg.value,
    ...
    function withdrawAdmin(
        ...
        require(
            address(this).balance > orderBook,
            "WE: insufficient balacne"
        );
        require(
            value <= address(this).balance - orderBook,
            "WE: Invalid amount"
        );
        to.transfer(value);
