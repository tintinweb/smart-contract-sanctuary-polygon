...
function generateStats(uint256 _tokenId) public {
require(tokenIdToLevels[_tokenId].level > 0, "This character has already generated its starting stats."); 
tokenIdToLevels[_tokenId].strength = uint(keccak256(abi.encodePacked(block.timestamp))) % 1000;
tokenIdToLevels[_tokenId].defense = uint(keccak256(abi.encodePacked(block.timestamp))) % 1000;
tokenIdToLevels[_tokenId].dexterity = uint(keccak256(abi.encodePacked(block.timestamp))) % 1000;
tokenIdToLevels[_tokenId].magic = uint(keccak256(abi.encodePacked(block.timestamp))) % 1000;
}
