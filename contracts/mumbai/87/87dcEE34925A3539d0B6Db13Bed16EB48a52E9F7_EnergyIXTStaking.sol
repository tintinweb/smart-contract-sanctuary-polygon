...
contract EnergyIXTStaking is IEnergyIXTStaking, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping(address => mapping(uint256 => uint256)) public s_userRewardPerTokenPaid;
    ...
    //ONLY FOR TESTING
    function testStakeOldIXT(uint256 _amount) external nonReentrant {
        require(_amount > 0, "ENERGY STAKING: CANT STAKE 0");
    ...
    //ONLY FOR TESTING
    function testStakeOldIXTAmelia(uint256 _amount) external nonReentrant {
        require(_amount > 0, "ENERGY STAKING: CANT STAKE 0");
    ...
    function _updateRewards(address _walletAddress) internal {
        ...
        // console.log("REWARD PER TOKEN:", s_rewardPool.rewardPerTokenStored / 1e18);
        ...
        if (_walletAddress != address(0)) {
            ...
            s_userRewardPerTokenPaid[_walletAddress][s_epochCount] = s_rewardPool.rewardPerTokenStored;

            // console.log("USER REWARD:", s_rewardPool.rewards[_walletAddress] / 1e18);
        }
    }

    /// @inheritdoc IEnergyIXTStaking
    function earned(address _walletAddress) public view returns (uint256) {
        return (userEnergyStaked[_walletAddress][s_epochCount] * (rewardPerTokenStored() - s_userRewardPerTokenPaid[_walletAddress][s_epochCount])) + s_rewardPool.rewards[_walletAddress];
    }

    function earnedAmelia(address _walletAddress) public view returns (uint256) {
        return (userEnergyStakedAmelia[_walletAddress][s_epochCount] * (rewardPerTokenStored() - s_userRewardPerTokenPaid[_walletAddress][s_epochCount])) + s_rewardPool.ameliaRewards[_walletAddress];
    }
    ...
    function getUserRewardPerTokenPaid(address _walletAddress) external view returns (uint256) {
        return s_userRewardPerTokenPaid[_walletAddress][s_epochCount];
    }
