...
contract AuctionBetting is Ownable, ReentrancyGuard {
    ...
    function closeAuction(uint256 _auctionId) external onlyOwner {
        ...
        require(auction.currentPrice > auction.minPrice, "Auction is not finished");
    ...
    function determinWiners(uint256 _auctionId) external onlyOwner nonReentrant {
        ...
        uint256 winerCount;
        if(auction.currentPrice / auction.minPrice == 1){

            winerCount = 1;
        } else winerCount = auction.currentPrice / auction.minPrice / 2;
