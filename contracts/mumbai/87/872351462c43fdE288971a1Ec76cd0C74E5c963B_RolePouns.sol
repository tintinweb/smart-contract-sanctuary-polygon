 ...
 * contract URI, burnable
 */
abstract contract PounsAbstract is AccessControl, ERC721BAbstract {
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view override(ERC721B, AccessControl) returns (bool) {
     ...
     * @dev Only callable by the owner.
     */
    function setSeeder(INounsSeeder _seeder) external onlyRole(_DAO_ROLE) {
        seeder = _seeder;
    }
...
contract RolePouns is ReentrancyGuard, PounsAbstract {
    error InvalidCall();
    ...
    constructor(
        INounsDescriptorMinimal _descriptor,
        INounsSeeder _seeder
    ) PounsAbstract(msg.sender, _descriptor, _seeder) {
    ...
    modifier mintCompliance(uint256 amount) {
        require(mintOpened, "Minting impossible");
        require(amount > 0 && amount <= maxMintAmount, "Invalid mint amount");
        require((_currentId + amount) <= MAX_SUPPLY, "Maximum mint limit");
        _;
    }
    ...
    function setCost(uint256 _newCost) external onlyRole(CURATOR_ROLE) {
        cost = _newCost;
    }
    ...
    function mint(address to, uint256 amount) external onlyRole(MINTER_ROLE) {
        if (_paused || (_currentId + amount) > MAX_SUPPLY) revert InvalidCall();
        _mintLoop(to, amount);
    }

    function mint(
        uint256 amount
    ) external payable nonReentrant mintCompliance(amount) {
        address recipient = _msgSender();
        if (_paused || recipient.code.length > 0 || (amount * cost) > msg.value)
            revert InvalidCall();
