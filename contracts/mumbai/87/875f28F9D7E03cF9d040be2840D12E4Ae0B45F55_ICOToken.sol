...
contract Crowdsale is Context, ReentrancyGuard {
	...
	/**
	 * @param ratePerWei Number of token units a buyer gets per wei
	 * @dev The rate is the conversion between wei and the smallest and indivisible
	 * token unit. So, if you are using a rate of 1 with a ERC20Detailed token
	 * with 3 decimals called TOK, 1 wei will give you 1 unit, or 0.001 TOK.
	 * @param walletOfOwner Address where collected funds will be forwarded to
	 * @param tokenAddress Address of the token being sold
	 */
	constructor(
		uint256 ratePerWei,
		address payable walletOfOwner,
		IERC20 tokenAddress
	) public {
		require(ratePerWei > 0, "Crowdsale: rate is 0");
		require(walletOfOwner != address(0), "Crowdsale: wallet is the zero address");
		require(
...
abstract contract TimedCrowdsale is Crowdsale {
	 ...
	 * @param crowdsaleOpeningTime Crowdsale opening time
	 * @param crowdsaleClosingTime Crowdsale closing time
	 */
	constructor(uint256 crowdsaleOpeningTime, uint256 crowdsaleClosingTime) {
		// solhint-disable-next-line not-rely-on-time
		require(
			crowdsaleOpeningTime >= block.timestamp,
			"TimedCrowdsale: opening time is before current time"
		);
		// solhint-disable-next-line max-line-length
		require(
			crowdsaleClosingTime > crowdsaleOpeningTime,
			"TimedCrowdsale: opening time is not before closing time"
		);

		_openingTime = crowdsaleOpeningTime;
		_closingTime = crowdsaleClosingTime;
	}
...
abstract contract AllowanceCrowdsale is Crowdsale {
	 ...
	 * @param holdTokenWallet Address holding the tokens, which has approved allowance to the crowdsale.
	 */
	constructor(address holdTokenWallet) {
		require(
			holdTokenWallet != address(0),
			"AllowanceCrowdsale: token wallet is the zero address"
		);
		_tokenWallet = holdTokenWallet;
	}
...
contract ICOToken is Crowdsale, AllowanceCrowdsale, TimedCrowdsale {
