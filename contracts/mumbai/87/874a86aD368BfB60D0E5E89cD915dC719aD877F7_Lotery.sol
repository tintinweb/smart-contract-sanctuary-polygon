...
contract Lotery is Ownable, Pausable{
  ...
  function viewReferralsData(address user) public view returns(bool,uint, uint, address ){
    return (
  ...
  function viewHistoricalTiketOwner(uint lotery, address user) public view returns(uint[] memory){
    return historicalTiketsOwner[lotery][user];
  }
  // ------------ INTERNAL FUNCTONS --------------
  ...
  // the entire reference system. special an not special
