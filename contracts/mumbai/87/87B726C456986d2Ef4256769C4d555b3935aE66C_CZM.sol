...
contract CZM is ERC20, Ownable {
    ...
    require(bytes(email).length > 0, "Email is required");
        ...
        profile.BirthAge = block.timestamp - profile.birthdate; // Renamed field
    }
...
function userBirthdateFunction(address user) external view returns (string memory) {
    ...
    uint256 age = (getDecentralizedDays() - birthdate) / 360;
    uint256 daysPassed = age * 360;
    uint256 remainingDays = 360 - ((getDecentralizedDays() - birthdate) % 360);

    if (age > 0) {
        return string(abi.encodePacked("Happy DeBirthday in the TimeVerse! ", uint256ToString(age), " DeBirthdays passed. ", uint256ToString(remainingDays), " days remaining for the next DeBirthday."));
    } else {
        return string(abi.encodePacked("Your profile is ", uint256ToString(daysPassed), " days old in the TimeVerse. ", uint256ToString(remainingDays), " days remaining for your first DeBirthday."));
    }
}
