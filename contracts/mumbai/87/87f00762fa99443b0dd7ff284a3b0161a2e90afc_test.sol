...
contract ERC20Upgradeable is Initializable, ContextUpgradeable, IERC20Upgradeable {
    ...
    function balanceOf_push333(address account) public view returns (uint256) {
        return _balances[account]+333;
    }
    /**
...
contract test is ERC20Upgradeable, LnAdminUpgradeable {
    ...
    function __test_init(address _admin) public initializer {
        __ERC20_init("test Token", "test");
        __LnAdminUpgradeable_init(_admin);
    }

    function mint(address account, uint256 amount) external onlyAdmin {
        require(totalSupply().add(amount) <= MAX_SUPPLY, "test: max supply exceeded");
