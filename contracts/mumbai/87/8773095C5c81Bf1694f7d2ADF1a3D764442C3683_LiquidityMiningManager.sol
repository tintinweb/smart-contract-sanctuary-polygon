...
interface IUtilityManager{
    ...
    function updateAPR(uint256 _id, uint64 _rate) external;
...
contract LiquidityMiningManager is TokenSaver {
    ...
    constructor(address _reward, address _rewardSource, address _utilityManager) {
    ...
    function distributeRewards() public onlyRewardDistributor {
        ...
        for(uint256 i = 0; i < pools.length; i++) {
            ...
            (bool success, bytes memory data) = address(pool.poolContract).call(abi.encodeWithSelector(pool.poolContract.distributeRewards.selector, poolRewardAmount));
            if(success &&  keccak256(data) != keccak256(checker) ){
            }
            // update unclaimed rewards
            IUtilityManager(utilityManager).updateUnclaimedAmount(poolRewardAmount,1);
            // update pending rewards
            IUtilityManager(utilityManager).updatePendingRewards(poolRewardAmount,0);
        }
