    ...
    SelfPermit
{
    ...
    function exactInputToSelf(ExactInputToSelfParams memory params)
        external
        payable
        returns (uint256 amountOut)
    {
   ...
   /// @dev Performs a single exact output swap
        ...
        returns (uint256 amountIn)
    {
       ...
       /// @dev Performs a single exact output swap
    function limitlessExactOutputInternalToSelf(
        uint256 amountOut,
        SwapCallbackData memory data
    ) private returns (uint256 amountIn) {
        (address tokenOut, address tokenIn, uint24 fee) = data.path.decodeFirstPool();
    ...
    function exactOutputToSelf(ExactOutputToSelfParams calldata params)
        external
        payable
        returns (uint256 amountIn)
    {
        // it's okay that the payer is fixed to msg.sender here, as they're only paying for the "final" exact output
        // swap, which happens first, and subsequent swaps are paid for within nested callback frames
        limitlessExactOutputInternalToSelf(
            params.amountOut,
            SwapCallbackData({path: params.path, payer: msg.sender})
        );
