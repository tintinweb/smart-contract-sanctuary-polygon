...
/// @title NEXUS World - OG Edition Mythical Buddies
/// @author Terratecc
...
abstract contract Ownable is Context {
    ...
    function _transferOwnership(address newOwner) internal virtual {
        ...
        /**
     * @dev Revokes the ownership of the new owner within a set time period
     * Can only be called by the old owner.
     */
    function revokeOwnership() public virtual {
        require(msg.sender == _oldOwner && msg.sender != _owner, "Ownable: access denied");
        require(block.timestamp < lastOwnershipTransfer + revokeWindow, "Ownable: revoke window has passed");
        _owner = _oldOwner;
        lastOwnershipTransfer = block.timestamp;
...
contract abtesteggs is ERC721Enumerable, Payable, DefaultOperatorFilterer {
    using Strings for uint256;

    uint256 public tokenPrice = 0 ether;
    uint256 public allowancePlusOne = 2;


    struct MintRound {
        uint256 roundId;
        uint256 maxSalePlusOne;
        uint256 roundStart;
        uint256 roundEnd;
        uint256 totalMinted;
        bool isMerkle;
    ...
    mapping(uint256 => MintRound) public idToMintRound;
    uint256 public lastMintRoundId;
    ...
    event Mint(uint256 tokenId, address minter, uint256 roundId, uint256 timestamp);

    constructor() ERC721("FunnyEggs", "FEG") {
     ...
     */
    modifier withinMintLimit(uint256 quantity, uint256 roundId) {
        require(roundId <= lastMintRoundId, "Invalid Id!");
        require(totalSupply() + quantity < idToMintRound[roundId].maxSalePlusOne, "Exceeds available tokens");
        _;
    }
    ...
    /**
     * See the next token id to be minted
     */
     ...
     * @param roundId The id of the mint round.
     * @param quantity The address to mint to.
     * @param quantity The number of tokens to mint.
     * @param proof The Merkle proof used to validate the leaf is in the root.
     */
    function mint(uint256 roundId, address to, uint256 quantity, bytes32[] calldata proof)
        external
        payable
        noBots
        isContractState(ContractState.ON)
        withinMintLimit(quantity, roundId)
        correctValue(tokenPrice * quantity)
    {
        require(roundId <= lastMintRoundId, "Invalid Id!");
        require(mintsPerAddress[to] + quantity < allowancePlusOne, "Exceeds allowance");
        require(block.timestamp <= idToMintRound[roundId].roundEnd, "Mint round has ended!");
        require(block.timestamp >= idToMintRound[roundId].roundStart, "Mint round has not started!");

        if (idToMintRound[roundId].isMerkle) {
            bytes32 leaf = keccak256(abi.encodePacked(to));
            require(verify(idToMintRound[roundId].merkleRoot, leaf, proof), "Not a valid proof");
        }
        
        for (uint256 i; i < quantity; i++) {
            uint256 tid = tokenId();
            mintsPerAddress[to]++;
            idToMintRound[roundId].totalMinted++;

            _safeMint(to, tid);

            emit Mint(tid, to, roundId, block.timestamp);
        }
    ...
    function mintReserved(address to, uint256 quantity) external onlyOwner {
        for (uint256 i; i < quantity; i++) {
            ...
            emit Mint(tid, to, 0, block.timestamp);
        }
    }
    ...
    function newMintRound(uint256 maxSale, uint256 roundStart, uint256 roundEnd, bool isMerkle, bytes32 merkleRoot) external onlyOwner {
        require(roundEnd > roundStart, "Invalid Timestamps!");
        lastMintRoundId++;

        idToMintRound[lastMintRoundId] = MintRound(lastMintRoundId, maxSale + 1, roundStart, roundEnd, 0, isMerkle, merkleRoot, block.timestamp);
    }
     ...
     */
    function setMaxSale(uint256 roundId, uint256 maxSale) external onlyOwner {
        require(roundId <= lastMintRoundId, "Invalid Id!");
        uint256 maxSalePlusOne_ = maxSale + 1;
        require(maxSalePlusOne_ > idToMintRound[roundId].maxSalePlusOne, "Must be above current minted!");
        idToMintRound[roundId].maxSalePlusOne = maxSalePlusOne_;
    }
     ...
     */
    function setRoundStart(uint256 roundId, uint256 newStart) external onlyOwner {
        require(roundId <= lastMintRoundId, "Invalid Id!");
        idToMintRound[roundId].roundStart = newStart;
    }
     ...
     */
    function setRoundEnd(uint256 roundId, uint256 newEnd) external onlyOwner {
        require(roundId <= lastMintRoundId, "Invalid Id!");
        idToMintRound[roundId].roundStart = newEnd;
    }
     ...
     */
    function setMerkleRoot(uint256 roundId, bytes32 merkleRoot_) external onlyOwner {
        require(roundId <= lastMintRoundId, "Invalid Id!");
        idToMintRound[roundId].merkleRoot = merkleRoot_;
    }

    /**
     * Set the need for a merkle root or not
     * @param isMerkle If a merkle root is required or not
     */
    function setIsMerkle(uint256 roundId, bool isMerkle) external onlyOwner {
        require(roundId <= lastMintRoundId, "Invalid Id!");
        idToMintRound[roundId].isMerkle = isMerkle;
    }
     ...
     * Return sale info.
     */
    function saleInfo(uint256 roundId) public view virtual returns (MintRound memory) {
        require(roundId <= lastMintRoundId, "Invalid Id!");
        return idToMintRound[roundId];
    }
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        require(_exists(uint16(_tokenId)), "URI query for nonexistent token");

        return
            bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, _tokenId.toString())) : placeholderURI;
    }
