...
interface POAPInterface {
     ...
     */
    event POAPContractUpdated(address toknContract);

    /**
     * @dev Mints a new NFT for the poll response.
     * @dev Only the polling contract can call this function.
     ...
     * @dev getTokenURI, function return the token uri by giving pollId and tokenId.
...
interface PollingInterface {
    ...
    /**
     * @dev Emitted when the competition amount transfered successfully, along with to addresses.
     */
    event TransferCompetitionAmountSuccessful(address[] toAddresses);
    ...
    /**
     * @dev Struct containing details about a poll at the end.
     */
    ...
    /**
     * @dev Struct containing details about poll reward claim.
     */
    ...
    /**
     * @dev Struct containing details about poll NFT claim.
     */
    struct PollClaimNFT {
        string tokenUri;
        uint256 pollId;
    ...
    /**
     * @dev Struct containing detail for sending competition winning details.
     */
    struct CompetitionWinningAmount {
        address toAddress;
        uint256 amount;
    }

    /**
     * @dev Struct containing detail for sending complete poll details.
     */
    struct PollDetails {
     ...
     * @param pollsReward An array of polls eligible for claiming rewards.
     */
    function claimAllRewards(PollClaimReward[] calldata pollsReward) external;

    /**
     * @dev Allows a user to claim multiple NFT's.
     * @param pollsClaimNFT An array of polls eligible for claiming NFT's.
     */
    ...
    /**
     * @dev Updates the admin address for the contract.
     * @dev Only owner can call this function.
     ...
     */
    function TOKNWithdraw(address toAddress, uint256 amount) external;

    /**
     * @dev transfer the TOKN from the owner to winner.
     * @dev Only owner or admin can call this function.
     * @param competitionWinningDetails caller giving competition winning details in struct format
     */
    function transferCompetitionWinningAmount(
        CompetitionWinningAmount[] calldata competitionWinningDetails
    ) external;
    ...
    /**
     * @dev Retrieves the address is given is admin or not.
    ...
    PollingInterface
{
    ...
    /**
     * @dev Burn percentage of the token at the time of withdraw
     */
    ...
    /**
     * @dev mapping to store polling details at the time of poll creation
     */
    mapping(uint256 => PollCreateDetails) internal createPollsDetail;

    /**
     * @dev mapping to store polling details at the time of poll end
     */
    mapping(uint256 => PollEndDetails) internal endPollsDetail;

    /**
     * @dev mapping to store has voter claimed the POAP NFT 
     */
    mapping(uint256 => mapping(address => bool)) public isVoterClaimedNFT;

    /**
     * @dev mapping to store has the winner voter claimed the poll reward
     */
    mapping(uint256 => mapping(address => bool)) public isPollRewardClaimed;

    /**
     * @dev mapping to store address is an admin
     */
    ...
    function initialize(
        ...
        require(
            ...
            "Polling: Admin address is the zero address"
        );
    ...
    function createPolls(
        ...
        require(
            polls.length > 0,
            "Polling: Polls length should be greater than zero"
        );

        require(
            timestamp > 0,
            "Polling: Timestamp should be greater than zero"
        );
        ...
        for (uint8 index = 0; index < polls.length; index++) {
            PollCreateDetails calldata pollDetail = polls[index];
            if (
                bytes(pollDetail.title).length > 0 &&
                pollDetail.choices.length > 0 &&
                timestamp < pollDetail.startTime &&
                pollDetail.startTime < pollDetail.endTime &&
                pollDetail.pollId > 0 &&
                pollDetail.creator != ZERO_ADDRESS &&
                createPollsDetail[pollDetail.pollId].startTime == 0
            ) {
                createPollsDetail[pollDetail.pollId] = PollCreateDetails({
                    ...
                    creator: pollDetail.creator
                });

                successPollIds[successIndex] = pollDetail.pollId;
                successIndex += 1;
            }
        }
    ...
    function endPolls(
        ...
        uint256[] memory successPollIds = new uint256[](polls.length);
        uint8 successIndex = 0;
        uint256 burnPercentForAll = 0;

        for (uint8 index = 0; index < polls.length; index++) {
            PollEndDetails calldata pollDetail = polls[index];
            if (
                !endPollsDetail[pollDetail.pollId].isPollEnded &&
                timestamp >
                createPollsDetail[pollDetail.pollId].endTime &&
                bytes(pollDetail.pollMetadata).length > 0 &&
                bytes(pollDetail.winningChoice).length > 0 &&
                pollDetail.winnersMerkle != ZERO_BYTES_32 &&
                pollDetail.votersMerkle != ZERO_BYTES_32 &&
                pollDetail.fee > 0
            ) {
                endPollsDetail[pollDetail.pollId] = PollEndDetails({
                    winningChoice: pollDetail.winningChoice,
                    pollMetadata: pollDetail.pollMetadata,
                    pollId: pollDetail.pollId,
                    fee: pollDetail.fee,
                    winnersMerkle: pollDetail.winnersMerkle,
                    votersMerkle: pollDetail.votersMerkle,
                    isPollEnded: true
                });
                burnPercentForAll +=
                    (pollDetail.fee * burnPercent) /
                    MAX_PERCENTAGE;
                successPollIds[successIndex] = pollDetail.pollId;
                successIndex += 1;
            }
        }
        if (burnPercentForAll > 0) {
            toknContract.burn(burnPercentForAll);
        }
    ...
    function claimAllRewards(
        ...
        require(
            pollsReward.length > 0,
            "Polling: Poll reward length should be greater than zero"
        );
        ...
        for (uint8 index = 0; index < pollsReward.length; index++) {
            ...
            if (
                pollRewardDetail.pollId > 0 &&
                !isPollRewardClaimed[pollRewardDetail.pollId][msg.sender] &&
                balanceOfContract > (pollRewardDetail.amount - 1) &&
                endPollsDetail[pollRewardDetail.pollId].isPollEnded
            ) {
                bytes32 encodedLeaf = keccak256(
                    abi.encode(msg.sender, pollRewardDetail.amount)
                );
                bool isValid = MerkleProofUpgradeable.verify(
                    pollRewardDetail.merkleProof,
                    endPollsDetail[pollRewardDetail.pollId].winnersMerkle,
                    encodedLeaf
                );
                if (isValid) {
                    isPollRewardClaimed[pollRewardDetail.pollId][
                        msg.sender
                    ] = true;
                    bool status = toknContract.transfer(
                        msg.sender,
                        pollRewardDetail.amount
                    );
                    if (status) {
                        balanceOfContract -= pollRewardDetail.amount;
                        successPollIds[successIndex] = pollRewardDetail.pollId;
                        successIndex++;
                    }
                }
            }
    ...
    function claimAllNFTs(
        ...
        for (uint8 index = 0; index < pollsClaimNFT.length; index++) {
            ...
            if (
                pollNFTDetail.pollId > 0 &&
                bytes(pollNFTDetail.tokenUri).length > 0 &&
                !isVoterClaimedNFT[pollNFTDetail.pollId][msg.sender] &&
                endPollsDetail[pollNFTDetail.pollId].isPollEnded
            ) {
                bytes32 encodedLeaf = keccak256(abi.encode(msg.sender));
                bool isValid = MerkleProofUpgradeable.verify(
                    pollNFTDetail.merkleProof,
                    endPollsDetail[pollNFTDetail.pollId].votersMerkle,
                    encodedLeaf
                );
                if (isValid) {
                    uint256 tokenId = POAPContract.mint(
                        pollNFTDetail.pollId,
                        pollNFTDetail.tokenUri,
                        msg.sender
                    );
                    isVoterClaimedNFT[pollNFTDetail.pollId][msg.sender] = true;
                    successPollIds[successIndex] = pollNFTDetail.pollId;
    ...
    function addAdmin(
        ...
        require(!isAdmin[addressOfAdmin], "Polling: Address already admin");
        require(
            addressOfAdmin != ZERO_ADDRESS,
            "Polling: Admin address is the zero address"
        );
     ...
     * @dev Remove the admin address for the contract.
     * @dev Only owner can call this function.
     * @param addressOfAdmin The old admin address for the contract.
     */
    function removeAdmin(
        address addressOfAdmin
    ) external override onlyOwner whenNotPaused nonReentrant {
        require(isAdmin[addressOfAdmin], "Polling: Address not admin");
    ...
    function TOKNWithdraw(
        ...
        require(
            ...
            "Polling: To address is the zero address"
        );
        require(
            amount > 0,
            "Polling: Amount is zero"
        );
        uint256 balanceOfContract = toknContract.balanceOf(address(this));
        require(
            balanceOfContract > (amount - 1),
            "Polling: Contract does not have sufficient balance"
        );
        bool status = toknContract.transfer(toAddress, amount);
        if (status) {
            emit TOKNWithdrawSuccessful(toAddress);
        }
    }
    ...
    function transferCompetitionWinningAmount(
        ...
        require(
            competitionWinningDetails.length > 0,
            "Polling: Competition details length should be greater than zero"
        );

        uint256 balanceOfContract = toknContract.balanceOf(address(this));
        address[] memory successReceivingAdress = new address[](competitionWinningDetails.length);
        uint8 successIndex = 0;
        for (uint8 index = 0; index < competitionWinningDetails.length; index++) {
            CompetitionWinningAmount calldata competitionWinningDetail = competitionWinningDetails[index];
            if (
                competitionWinningDetail.toAddress != ZERO_ADDRESS &&
                competitionWinningDetail.amount > 0 &&
                balanceOfContract > competitionWinningDetail.amount - 1
            ) {
                bool status = toknContract.transfer(
                    competitionWinningDetail.toAddress,
                    competitionWinningDetail.amount
                );
                if (status) {
                    balanceOfContract -= competitionWinningDetail.amount;
                    successReceivingAdress[successIndex] = competitionWinningDetail.toAddress;
                    successIndex++;
                }
            }
        }

        emit TransferCompetitionAmountSuccessful(successReceivingAdress);
    }
    ...
    modifier onlyAdminOrOwner() {
        require(
            ...
            "Polling: Caller is not admin or owner"
        );
        _;
    }
}