...
contract uTokenFactory is Ownable{
    ...
    function calculateRewardOfWinner() public view returns (uint256 ethForReward, RewardAgainstTokens[] memory rewardAgainstTokens, bool rewardPending) {
        
        uint256 _ethInPeriod = address(this).balance;
        if(_ethInPeriod > 0){
            ethForReward = _ethInPeriod; 
            rewardPending = true;
        }
        ...
        for(uint i; i < _allowedTokens.length; i++) {
            ...
            if(IERC20(_allowedTokens[i]).balanceOf(address(this)) > 0) rewardPending = true;
        }
    }
}