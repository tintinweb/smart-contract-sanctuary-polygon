...
using Address for address;
    ...
    function addCategories(
       ...
       // require((_categoriesSet == 0) && (_maxNFTs == 0), "Already Set");
        //_maxNFTs = maxNFTs;
        //Category memory category;
        //categoriesArray= new Category[] (_categoriesSet);
        for (uint8 i = 0; i < noOfCategories; i++) {
            CategoryDetails[_categoriesSet+(i+1)].categoryName = categoryNames[i];
            CategoryDetails[_categoriesSet+(i+1)].categoryNftCount = nftCounts[i];
            CategoryDetails[_categoriesSet+(i+1)].categoryMintedCount = 0;
            rarityArray[_categoriesSet+(i+1)] += (nftCounts[i]*100*(10**18)/_maxNFTs);
            sumOfWeights += rarityArray[_categoriesSet+(i+1)];

            //emit CategoriesSet(CategoryDetails[i],sumOfWeights);
            //categoriesArray.push(category);
        }
        _categoriesSet += noOfCategories;
    }
    //set maxNFTs for equal probability minting
    function setStatusMintFeeAndMaxMints(bool mintStatus, uint mintFee, uint maxMints) public onlyOwner {
        _mintFees = mintFee;
    ...
    function setTotalCategories(uint8 totalCategories) external onlyOwner {
    require(_totalCategories!=0, "Total Categories only be set once");
        _totalCategories = totalCategories;
        //set The Rarity Arrays only once
        rarityArray = new uint256[](_totalCategories);
        ...
        returns ( uint256[] memory)
    {
        ...
        string[] memory categoryNamesArr = new string[](_categoriesSet);
        for (uint16 i = 0; i < _categoriesSet; i++) {
    ...
    function updateConditions(uint256 index) internal returns (uint) {
        uint nftId;
             ...
             if(rnd < rarityArray[i]){
                flag = slotAvailable(i);
                    if(flag){
                        ...
                        TokenURI = string(abi.encodePacked("Some_Base_URI/",CategoryDetails[i].categoryName,"/",Strings.toString(nftId)));
                        CategoryDetails[i].categoryMintedCount++;
                        break;
                    }
                    else{
                        //before 2nd last full slot
                    while(!flag && i < (_categoriesSet-1)){
                        i = i+1;
                        flag = slotAvailable(i);
                        if(flag){
                                nftId = ((i+1)*100000)+CategoryDetails[i].categoryMintedCount;
                        ...
                        //last full slot
                    while(!flag && i <= (_categoriesSet-1) && i>=0){
                        flag = slotAvailable(i);
                        if(flag){
                                nftId = ((i+1)*100000)+CategoryDetails[i].categoryMintedCount;
                                TokenURI = string(abi.encodePacked("Some_Base_URI/",CategoryDetails[i].categoryName,"/",Strings.toString(nftId)));
    ...
    //     //checking whether the user_addr is whitelisted so that only one mint is allowed to him
     
    //     if(whitelistedAddresses[user_addr]==true){
    //         require(NFTsPerWallet[user_addr]<2, "Only 1 NFT mint is allowed per whitelisted wallet");
    //     }
...
function RandomMint(address user_addr, uint8 noOfMints)
        ...
        returns (uint256[] memory, string[] memory)
    {
        ...
        require(noOfMints + NFTsPerWallet[user_addr] <= _maxMints && noOfMints > 0,"You cannot mint more than max mint limit");
        ...
        for (uint i = 0; i <= noOfMints - 1; i++) {
             categoryID = random();
             tokenId = updateConditions(categoryID);
             //will return 0,1 or 2
            //categoryID = (tokenId)%(_categoriesSet);
            //tokenId = CategoryDetails[categoryID].categoryMintedCount;
        ...
        returns (uint256[] memory, string[] memory)
    {
        ...
        if (_mintEndTime > block.timestamp)
        {
            ...
            if(isDiscounted[user_addr])
                require(msg.value == (_mintFees*(noOfMints))/2, "Not Enough Amount Provided");
            else
        ...
        //previous 001 collection users will have 50% discount
            if(isDiscounted[user_addr])
                require(msg.value == ((_mintFees*(noOfMints))/100)*_DiscountPercentage, "Not Enough Amount Provided");
            else
                require(msg.value == _mintFees*(noOfMints), "Not Enough Amount Provided");

        uint256[] memory randomMintedNftIDs = new uint256[](noOfMints);
        string[] memory randomMintedNftURIs = new string[](noOfMints);
        (randomMintedNftIDs,randomMintedNftURIs) = RandomMint( user_addr,  noOfMints);
        return (randomMintedNftIDs,randomMintedNftURIs);
    }
