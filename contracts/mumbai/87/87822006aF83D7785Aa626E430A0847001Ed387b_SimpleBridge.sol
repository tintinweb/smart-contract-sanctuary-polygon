...
contract SimpleBridge {
  ...
  constructor() {
    ...
    currentId = id;
    if(id == 5) { // goerli testnet
      token = IERC20(0x7ea6eA49B0b0Ae9c5db7907d139D9Cd3439862a1);
      cost = 1.0005003e18;
      connext = IConnext(0x99A784d082476E551E5fc918ce3d849f2b8e89B6);
      destinationDomainId = 9991; //mumbai domain id

    } else if(id == 80001) { // mumbai testnet
      token = IERC20(0xeDb95D8037f769B72AAab41deeC92903A98C9E16);  
      cost = 1e18;
      connext = IConnext(0xfeBBcfe9a88aadefA6e305945F2d2011493B15b4);
      destinationDomainId = 1735353714; //goerli domain id
    }
  }

  function xSend() external payable{

    bytes memory callData = abi.encode(
      string.concat(
        "transaction number:", 
        nonceIncrement().toString(), 
        ", sender chainId:", 
        currentId.toString()
      )
    );
    ...
    connext.xcall{value: msg.value}(
      ...
      address(this),            // _to: address of the target contract
  ...
  function xReceive(
    ...
    string memory newTx = abi.decode(_callData, (string));

    inputBox = string.concat(
      newTx, 
      ", receiver chianId:",
      currentId.toString()
    );

    emit NewTx(_transferId, _amount, _asset, _originSender, _origin, _callData);
  }

  function nonceIncrement() private returns(uint256) {
    return nonce++;
  }
}