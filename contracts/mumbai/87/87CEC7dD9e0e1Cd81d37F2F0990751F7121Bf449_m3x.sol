...
contract m3x is Ownable {
    ...
    uint256 public constant minDeposit = 1e16; //15e18;

    uint256 public  tradingSplit = 3000; //split 100% amount to 70% & 30%. among which 70% for mining and 30% for AI Trade, Royalty and Stability 

    uint256 private constant timeStep = 1 minutes; //1 days;

    uint256 private constant dayPerCycle = 5 minutes;  //5 days;

    uint256 private constant dayRewardPercents = 200;

    uint256 private constant maxAddFreeze = 30 minutes; //30 days;

    uint256 private constant referDepth = 15;

    uint256 public constant directPercents = 500;

    uint256[7] private level4Percents = [200, 100, 200, 100, 100, 50, 50];

    uint256[7] private level5Percents = [75, 25, 25, 10, 10, 10, 10];

    uint256 public level4royalty = 50;

    uint256 public level4allow = 20;

    uint256 public level5royalty = 50;

    uint256 public level5allow = 35;
    ...
    uint public withdrawLimitFactor = 3;
    ...
    function register(address _referral, address _user) private {
		...
		if(_referral != address(0)){
			userInfo[_referral].myDirectCount = userInfo[_referral].myDirectCount.add(1);
		}
    ...
    function deposit(address referrer) payable external {

        address _sender = msg.sender;

        uint256 msgvalue = msg.value;
		
	uint256  _amount = msgvalue;
    ...
    function _calLevelNow(address _user) private view returns(uint256) {
        ...
        if(total >= 5000e18 && user.teamNum >= 250 && user.myDirectCount >= 10 && maxTeam >= 15000e18 && otherTeam >= 15000e18){        

            levelNow = 5;

        }else if(total >= 1000e18 && user.teamNum >= 100 && user.myDirectCount >= 5 && maxTeam >= 5000e18 && otherTeam >= 5000e18){
