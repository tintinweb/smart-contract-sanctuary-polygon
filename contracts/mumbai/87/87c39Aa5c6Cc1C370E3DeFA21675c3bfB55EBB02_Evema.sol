...
contract Evema is Ownable, ERC721URIStorage, VRFV2WrapperConsumerBase {
    ...
    constructor(
        ...
        ERC721("EvemaNFT", "EVEMA")
        VRFV2WrapperConsumerBase(_linkAddress, _wrapperAddress)
    {
        tokenId.increment();
    }

    function bookTickets(uint256 _eventId) public payable {
    ...
    function createEvent(
        ...
        getEventByIds[requestId] = _event;
        events.push(_event);
    ...
    function fulfillRandomWords(
        ...
        // Use the randomWords to generate a random event ID
         uint256 eventId = uint256(keccak256(abi.encodePacked(_randomWords)));
        EventData memory _event = getEventByIds[eventId];
        _event.eventId = _randomWords[0] % 1000;
    }
    ...
    //     require(eventId != 0, "Invalid request");
    //     EventData storage _event = getEventByIds[eventId];
    //     _event.eventId = randomness % 1000; // Modify the range as needed
    //     emit newEventCreated(_event.creator, block.timestamp, _event.eventId);
    // }
    ...
    function usersTokens(address _user) public view returns (uint256[] memory) {
        return userNftTokens[_user];
    }

    function eventsBookedByUser(address _user)
        public
        view
    ...
    function createdEvents(address _user)
        public
        view
        returns (EventData[] memory)
    {
    ...
    function ticketsSold(uint256 _eventId) public view returns (uint256) {
        return getEventByIds[_eventId].ticketsBought;
    }

    function isEventOn(uint256 _eventId) public view returns (bool) {
        return getEventByIds[_eventId].eventPast;
    }

    function getByCategory(string memory _category)
        public
        view
        returns (EventData[] memory)
    {
        ...
        for (; i < events.length; i++) {
            ...
            if (
                keccak256(abi.encodePacked(currentEvent.category)) ==
    ...
    function deleteEvent(uint256 _eventId) external {
        ...
        for (
            uint256 index = i;
            index < creatorEvents[msg.sender].length - 1;
            index++
        ) {
            creatorEvents[msg.sender][index] = creatorEvents[msg.sender][
                index + 1
            ];
        }
        ...
        for (uint256 index = envIndex; index < events.length - 1; index++) {
    ...
    function deleteEventAdmin(uint256 _eventId) external onlyOwner {
        uint256 envIndex = 0;
        // Delete from general events array
