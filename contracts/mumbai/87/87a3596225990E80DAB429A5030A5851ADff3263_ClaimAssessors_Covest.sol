...
contract ClaimAssessors_Covest is Initializable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    ...
    modifier isPolicy {
        require(AR.isPolicy(poolId,msg.sender),"AdminRouter: You aren't Policy Contract.");
        _;
    }

    modifier isSuperManager {
        require(AR.hasRole(SUPER_MANAGER_ROLE,msg.sender), "CA: You're not Super Manager.");
        _;
    }

    modifier checkAsset(address _asset) {
        require(AR.isSupportAssets(poolId,_asset),"Policy: This assets was not accept.");
        _;
    }
    ...
    function applyWorking(address _user) public  isSuperManager {
        ...
        // (,bool isaccesscaforregis) = S.accessRole(_user,IStaking.typeUser.CA);
        // require(isaccesscaforregis == true,"Staking : You not have access for register.");
    ...
    function resignation(address _user) public  isSuperManager {
        require(PDS.isBlacklistOfUser(_user) != true,"Policy: This _user are Blacklist.");
        // (,bool isAccessCA) = S.accessRole(_user,IStaking.typeUser.CA);
        // require(isAccessCA == true,"Staking : You are not UnderWriter this moment.");
    ...
    function allCA() public view returns(address[] memory){
        ...
        for (uint i = 0;i < _allCA.length;i++){
    ...
    function isClaimAssessorsNonCheckStake(address _user) public view returns(bool){
        return isCA[_user];
    }

    function isClaimAssessors(address _user) public view returns(bool) {
        (IStaking.StakingData memory _StakingData) = S.stakingData(_user,IStaking.typeUser.CA);
        (IStaking.WorkType memory _dataType) = S.workData(IStaking.typeUser.CA);
    ...
    function withdraw(address _to,address _asset, uint256 _value) public nonReentrant isSuperManager checkAsset(_asset) { // for ClaimAssessors //
        IERC20 _ERC20 = IERC20(_asset);
        require(_ERC20.balanceOf(address(this)) >= _value,"CA: Balance of Contract Not Enough.");
        require(PDS.isBlacklistOfUser(_to) != true,"Policy: This _user are Blacklist.");
        require(PDS.isBlacklistAssetsOfUser(_to,_asset) != true,"Policy: This Assets of you are Blacklist.");
        require(isClaimAssessors(_to) == true,"CA: This _user not CA.");
