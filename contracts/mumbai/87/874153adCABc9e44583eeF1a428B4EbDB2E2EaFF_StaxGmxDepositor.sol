...
contract StaxGmxDepositor is IStaxGmxDepositor, Ownable, Operators {
    ...
    function mintAndStakeGlp(uint256 fromAmount, address fromToken, uint256 minUsdg, uint256 minGlp, uint256 slippageBps) external onlyOperators returns (uint256) {
        IERC20(fromToken).safeIncreaseAllowance(glpManager, fromAmount);
        return gmxRewardRouter.mintAndStakeGlp(
            fromToken, 
            fromAmount, 
    ...
    /// @dev GMX.io takes fees dependent on the pool constituents.
    function unstakeAndRedeemGlp(uint256 glpAmount, address toToken, uint256 minOut, uint256 slippageBps, address receiver) external onlyOperators returns (uint256) {
        return gmxRewardRouter.unstakeAndRedeemGlp(
            toToken, 
            glpAmount, 
    ...
    /// for STAX's share of the upstream GMX.io rewards.
    function rewardRates(bool forStakedGlpRewards) external override view returns (
    ...
    function harvestableRewards(bool forStakedGlpRewards) external view override returns (
    ...
    function harvestRewards(FractionalAmount.Data calldata _esGmxVestingRate) external onlyOperators override returns (
        ...
        // Stack too deep
        {
            ...
            // Calculate how many ETH & esGMX rewards were awarded (shame the handleRewards fn doesn't do this for us)
            uint256 totalWrappedNativeClaimed = wrappedNativeToken.balanceOf(address(this)) - wrappedNativeBefore;
            wrappedNativeClaimedFromGmx = subtractWithFloorAtZero(totalWrappedNativeClaimed, wrappedNativeClaimedFromGlp);

            uint256 totalEsGmxClaimed = esGmxToken.balanceOf(address(this)) - esGmxBefore;
            esGmxClaimedFromGmx = subtractWithFloorAtZero(totalEsGmxClaimed, esGmxClaimedFromGlp);
