    ...
    Base
{
    ...
    /// @notice Minter role for minting tokens
    ...
    function initialize(bytes calldata _vars)
        external
        initializerERC721A
        initializer
    {
        ...
        __ERC721A_init("NFTEngage Collection", "NFTEC");
        __Ownable_init();

        _setMinter(vars.minter);
...
interface ICollectionV1 {
    ...
    event UpdatedMinter(address newMinter);
...
library DataTypes {
    /// @notice A Struct containing the parameters required for `initialize()` function.
    /// @param minter the address for the minter role.
    /// @param isMintable whether the collection's tokens are mintable or not.
    /// @param mintPrice the price for minting tokens.
    /// @param mintCurrency the currency for minting tokens.
    /// @param maxPerWallet the maximum number of token that can be minted per wallet.
