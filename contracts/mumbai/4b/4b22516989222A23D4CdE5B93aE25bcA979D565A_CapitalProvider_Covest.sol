	...
	ReentrancyGuardUpgradeable
{
	...
	struct ClaimPolicyParams {
		...
		string policyId;
		string ipfsHash;
		uint256 value; // the value decimals is 18 //
		uint256 generatedAt;
		uint256 expiresAt;
		uint8 v;
		bytes32 r;
	...
	/**
	 * @notice Get percentageWillPassWhenRequestClaim.
	 * @return The uint256 value.
	 */
	uint256 public percentageWillPassWhenRequestClaim;
	...
	function initialize(address _nameRegistry, uint256 _percentageWillPassClaim)
		public
		initializer
	{
		require(
			_percentageWillPassClaim > 0 && _percentageWillPassClaim <= 100
		);
		__ReentrancyGuard_init();
		__UUPSUpgradeable_init();
		NR = INameRegistry(_nameRegistry);
		percentageWillPassWhenRequestClaim = _percentageWillPassClaim;

		emit PercentagePassChanged(0, _percentageWillPassClaim);
	}
	 ...
	 * @param _percentageWillPassClaim The percentage of the pass claim.
	 *
	 * Requirement:
	 *  - `msg.sender` Must be a SuperManager.
	 */
	function setPassPercentage(uint256 _percentageWillPassClaim)
		public
		isSuperManager
	{
		require(
		...
		uint256 percentagebefore = percentageWillPassWhenRequestClaim;
		percentageWillPassWhenRequestClaim = _percentageWillPassClaim;

		emit PercentagePassChanged(percentagebefore, _percentageWillPassClaim);
	}
	...
	function claimIdCancel(uint256 _claimId) public nonReentrant {
		...
		uint8 _claimIdStatusClaimId = claimIdStatus(_claimId);
		require(
			_claimIdStatus[_claimId] != 5 &&
				(_claimIdStatus[_claimId] == 0 || _claimIdStatusClaimId == 6),
			"AC-NP"
		);

		require(claimIdDataThisId.asset != address(0), "IA");

		if (_claimIdStatusClaimId == 6) {
			_claimIdStatus[_claimId] = 6;
		} else {
			_claimIdStatus[_claimId] = 5;
		}
		...
		emit ClaimIdCancel(msg.sender, claimIdDataThisId.asset, _claimId);
	}
	...
	function _finalize(uint256 _claimId) private {
		...
		uint256 pass = (claimIdDataThisId.caSnapshotValue *
			percentageWillPassWhenRequestClaim) / 100;
...
interface IStaking {
	...
	function checkAccessRole(address _user, StakingTypes _type) external view returns (bool);
