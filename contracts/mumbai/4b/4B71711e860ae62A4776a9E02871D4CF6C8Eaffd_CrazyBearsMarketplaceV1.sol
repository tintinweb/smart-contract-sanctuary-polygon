...
contract CrazyBearsMarketplaceV1 is Ownable, ReentrancyGuard {
    struct Item {
        address id;
    ...
    event Purchase(address indexed user, address itemId, uint256 itemType, string itemName, uint256 timestamp);
    ...
    function add721TokenItem(string memory _name, string memory _imageUrl, address _contractAddress, uint256 _tokenId, uint256[] memory _costIds, uint256[] memory _costAmounts, bool _active) public onlyOwner {
        ...
        Item memory newItem = Item(itemId, IT_721, _name, _imageUrl, _contractAddress, _tokenId, 1, 1, 0, _costIds ,_costAmounts, 0, false, false, _active, block.timestamp);
        itemsById[itemId] = newItem;
        items.push(newItem);
    }

    function add1155TokenItem(string memory _name, string memory _imageUrl, address _contractAddress, uint256 _tokenId, uint256 _amount, uint256 _totalSupply, uint256[] memory _costIds, uint256[] memory _costAmounts, bool _mulPurchas, bool _active) public onlyOwner {
        require(_costIds.length == _costAmounts.length, "Array Error");
        address itemId = getId();
        Item memory newItem = Item(itemId, IT_1155, _name, _imageUrl, _contractAddress, _tokenId, _amount, _totalSupply, 0, _costIds ,_costAmounts, 0, _mulPurchas, false, _active, block.timestamp);
        itemsById[itemId] = newItem;
        items.push(newItem);
    }

    function addWLItem(string memory _name, string memory _imageUrl, uint256 _totalSupply, uint256[] memory _costIds, uint256[] memory _costAmounts, bool _mulPurchas, bool _active) public onlyOwner {
        require(_costIds.length == _costAmounts.length, "Array Error");
        address itemId = getId();
        Item memory newItem = Item(itemId, IT_WL, _name, _imageUrl, address(0), 0, 0, _totalSupply, 0, _costIds ,_costAmounts, 0, _mulPurchas, false, _active, block.timestamp);
        itemsById[itemId] = newItem;
        items.push(newItem);
    }

    function addRaffleEntryItem(string memory _name, string memory _imageUrl, uint256 _totalSupply, uint256[] memory _costIds, uint256[] memory _costAmounts, uint256 _numWinners, bool _mulPurchas, bool _active) public onlyOwner {
        require(_costIds.length == _costAmounts.length, "Array Error");
        address itemId = getId();
        Item memory newItem = Item(itemId, IT_RAFFLE, _name, _imageUrl, address(0), 0, 0, _totalSupply, 0, _costIds ,_costAmounts, _numWinners, _mulPurchas, false, _active, block.timestamp);
        itemsById[itemId] = newItem;
        items.push(newItem);
    }
    ...
    function beforePurchase(address _itemId) internal view {
        ...
        require(item.mulPurchas || !isPurchased(_itemId, msg.sender), "Already purchased");
    }
    ...
    function getHistory(address _user) public view returns (DetailOrder[] memory) {
        uint len = userHistory[_user].length;
        DetailOrder[] memory temp = new DetailOrder[](len);
        for (uint index = 0; index < len; index ++) {
            Order memory order = userHistory[_user][index];
            temp[index] = DetailOrder(order.id, order.user, itemsById[order.itemId], order.timestamp);
        }
        return temp;
    }

    function getPurchasedUser(address _itemId) public view returns (address[] memory) {
        uint len = purchasedUsers[_itemId].length;
        address[] memory temp = new address[](len);
        for (uint index = 0; index < len; index ++) {
            temp[index] = purchasedUsers[_itemId][index];
        }
        return temp;
    }
    ...
    function _pick(Item memory _item, bool _isReroll) internal {
        ...
        for (uint index = 0; index < entries.length; index++) {
            ...
            if (result.winners.length >= _item.numWinners) {
                break;
            }
        }
    ...
    function getRaffleResults() public view returns (RaffleResult[] memory) {
        uint len = raffleResults.length;
        RaffleResult[] memory temp = new RaffleResult[](len);
        for (uint index = 0; index < len; index ++) {
            temp[index] = raffleResults[index];
        }
        return temp;
    }
