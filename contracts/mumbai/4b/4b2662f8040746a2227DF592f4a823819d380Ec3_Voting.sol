...
contract Voting is IVoting{
   ...
   uint numberOfVoters;
    ...
    Candidates[] private aboutCandidates;

    mapping(address => VoterInfo) public voterInfoMapping;

    ElectionInfo private info;

    constructor(ElectionInfo memory _info ){
...
// getter
    function electionInfo() public view returns(ElectionInfo memory){
        return info;
    }
...
// setter
    ...
    function addCandidates(Candidates[] memory _candidates) public {
        uint i;
        for(i=0; i < _candidates.length; i+=1) {
            require(i == _candidates[i].candidateId && _candidates[i].votesReceived==0, "constructor: Wrong input for candiate");
    ...
    function addVoters(address[] memory _voters, uint[] memory _startTime, uint[] memory _endTime) public {
        require(votersAdded == false, "addVoters: Voter's list already updated");

        for(uint i=0; i <= _voters.length; i+=1) {
            address  aVoter = _voters[i];
            voterInfoMapping[aVoter] = VoterInfo(false,_startTime[i], _endTime[i]);
            voterAddressList.push(aVoter);
        }
    ...
    function castVote(uint _candidateId) public {
        require(voterInfoMapping[msg.sender].hasVoted == false && block.timestamp >= voterInfoMapping[msg.sender].startTime && block.timestamp <= voterInfoMapping[msg.sender].endTime, "castVote: Unauthorised");
