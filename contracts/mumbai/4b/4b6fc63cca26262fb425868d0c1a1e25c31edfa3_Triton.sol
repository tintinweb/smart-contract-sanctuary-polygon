...
// File: extension/interface/IDelayedReveal.sol


pragma solidity ^0.8.15;

/**
 *  Triton's `DelayedReveal` is a contract extension for base NFT contracts. It lets you create batches of
 *  'delayed-reveal' NFTs. 
 */
...
// File: extension/interface/ILazyMint.sol


pragma solidity ^0.8.15;

/**
 *  Triton's `LazyMint` is a contract extension for any base NFT contract. It lets you 'lazy mint' any number of NFTs
...
// File: extension/interface/IClaimCondition.sol
...
/**
 *  Triton's 'Drop' contracts are distribution mechanisms for tokens.
 *
...
// File: extension/interface/IDropSinglePhase.sol
...
abstract contract DropSinglePhase is IDropSinglePhase {
    using TritonBitMaps for TritonBitMaps.BitMap;
     ...
     */
    mapping(bytes32 => TritonBitMaps.BitMap) private usedAllowlistSpot;
...
/**
 *  The 'signature minting' mechanism used in triton Token smart contracts is a way for a contract admin to authorize an external party's
...
/**
 *  Triton's `Primary` is a contract extension to be used with any base contract. It exposes functions for setting and reading
...
/**
 *  Triton's `PrimarySale` is a contract extension to be used with any base contract. It exposes functions for setting and reading
 *  the recipient of primary sales, and lets the inheriting contract perform conditional logic that uses information about
 *  primary sales, if desired.
 */
...
// File: extension/interface/IPermissions.sol
...
// File: extension/interface/IPermissionsEnumerable.sol
...
// File: extension/interface/IOwnable.sol


pragma solidity ^0.8.15;

/**
 *  Triton's `Ownable` is a contract extension to be used with any base contract. It exposes functions for setting and reading
...
// File: extension/interface/IMulticall.sol
...
// File: extension/interface/IContractMetadata.sol


pragma solidity ^0.8.15;

/**
 *  Triton's `ContractMetadata` is a contract extension for any base contracts. It lets you set a metadata URI
 *  for you contract.
 *
...
// File: eip/interface/IERC165.sol
...
// File: extension/interface/IRoyalty.sol
...
/**
 *  Triton's `Royalty` is a contract extension to be used with any base contract. It exposes functions for setting and reading
...
library ECDSA {
    ...
    function toEthSignedMessageHash(bytes memory s) internal pure returns (bytes32) {
        return keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n", TritonStrings.toString(s.length), s));
    }
...
// File: openzeppelin-presets/utils/cryptography/EIP712.sol
...
// File: extension/Permissions.sol
        ...
        if (!_hasRole[role][account]) {
            revert(
                string(
                    abi.encodePacked(
                        "Permissions: account ",
                        TritonStrings.toHexString(uint160(account), 20),
                        " is missing role ",
                        TritonStrings.toHexString(uint256(role), 32)
                    )
                )
            );
        }
...
// File: openzeppelin-presets/utils/Context.sol
...
// File: openzeppelin-presets/token/ERC20/utils/SafeERC20.sol
 ...
 */
library SafeERC20 {
    using TritonAddress for address;
...
contract Multicall is IMulticall {
    ...
    function multicall(bytes[] calldata data) external virtual override returns (bytes[] memory results) {
        ...
        for (uint256 i = 0; i < data.length; i++) {
            results[i] = TritonAddress.functionDelegateCall(address(this), data[i]);
        }
        return results;
    }
}

// File: openzeppelin-presets/token/ERC721/IERC721Receiver.sol
...
// File: eip/interface/IERC721Metadata.sol
...
// File: eip/interface/ITritonERC721.sol
...
/**
 * @dev Interface of an TritonERC721 compliant contract.
 */
interface ITritonERC721 is IERC721, IERC721Metadata {
    /**
     * The caller must own the token or be an approved operator.
     */
 ...
 */
contract TritonERC721 is Context, ERC165, ITritonERC721 {
    using TritonAddress for address;
    using TritonStrings for uint256;
    ...
    function approve(address to, uint256 tokenId) public override {
        address owner = TritonERC721.ownerOf(tokenId);
...
// File: base/TritonERC721Base.sol
...
/**
 *  The `Triton` smart contract implements the ERC721 NFT standard, along with the TritonERC721 optimization to the standard.
...
contract TritonERC721Base is TritonERC721, ContractMetadata, Multicall, Ownable, Royalty, BatchMintMetadata, PermissionsEnumerable {
    using TritonStrings for uint256;
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(TritonERC721, IERC165) returns (bool) {
        return
     ...
     *  @dev            TritonERC721's `_burn(uint256,bool)` internally checks for token approvals.
     *
...
// File: base/TritonERC721SignatureMint.sol
...
/**
 *      BASE:      TritonERC721Base
 *      EXTENSION: SignatureMintERC721
 *
 *  The `Triton` contract uses the `TritonERC721Base` contract, along with the `SignatureMintERC721` extension.
 *
...
contract TritonERC721SignatureMint is TritonERC721Base, PrimarySale, SignatureMintERC721 {
    /*//////////////////////////////////////////////////////////////
                            Constructor
    //////////////////////////////////////////////////////////////*/
    ...
    ) TritonERC721Base(_name, _symbol, _royaltyRecipient, _royaltyBps) {
...
/**
 *      BASE:      TritonERC721SignatureMint
 *      EXTENSION: SignatureMintERC721, DropSinglePhase
 *
 *  The `Triton` contract uses the `TritonERC721Base` contract, along with the `SignatureMintERC721` and `DropSinglePhase` extension.
 *
 ...
 *
 *  The `Triton` contract lets you lazy mint tokens, and distribute those lazy minted tokens via signature minting, or
 *  via the drop mechanism.
 */

contract Triton is TritonERC721SignatureMint, LazyMint, DelayedReveal, DropSinglePhase {
    using TritonStrings for uint256;
    ...
    constructor(
        ...
        uint128 _royaltyBps,
        address _primarySaleRecipient
    ) TritonERC721SignatureMint(_name, _symbol, _royaltyRecipient, _royaltyBps, _primarySaleRecipient) {
    ...
    function collectPriceOnClaim(
        ...
        address _currency,
        uint256 _pricePerToken
    ) internal virtual override(DropSinglePhase, TritonERC721SignatureMint) {
        if (_pricePerToken == 0) {
            return;
        }
