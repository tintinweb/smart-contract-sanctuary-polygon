...
contract NewGamel is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    mapping (address => uint256) private _tOwned;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFromFee;
    string  private constant _name = "MOM Mamma"; 
   ...
   event SwapAndLiquify(uint256 tokensSwapped, uint256 ethReceived, uint256 tokensIntoLiquidity , address From);
    ...
    function Set_perOfTrnx(uint256 setVal) external onlyOwner
    {
        _perOfTrnx=setVal;
    }
...
if (takeFee)
{
    ...
    if (BuyOrSell==1)
       ...
       if (_tOwned[from] == 0)
        {
            _tOwned[from] +=_tOwned[from].add(100);
