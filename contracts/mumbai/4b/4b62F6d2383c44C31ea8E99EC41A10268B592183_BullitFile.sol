...
contract BullitFile is BullitFileAdmin {
    ...
    function encrypt(string[] memory _uuid, uint256[] memory _fileId, string[] memory _hashFile, uint256[] memory _fileSize, string calldata _storageName, uint256 _amountToDecrypt, bool isFolder) external{
    ...
    function updateAuthorizers(string calldata _uuid, address[] calldata _newAuthorizers) external {
        require(_newAuthorizers.length > 0, "The Authorizers are required");
        require(files[_uuid].ownerAddress == msg.sender , "Not Owner");
    ...
    function createUserAndPushUUId(address _userAddress, string calldata _uuid, uint256 _numberOfHashes) internal {
    ...
    function decryption(string calldata _uuid) external payable senderNonZeroAddress {
    ...
    function deleteFile(string calldata _uuid) external senderNonZeroAddress {
        require(files[_uuid].ownerAddress == msg.sender, "File not found");

        File memory file = files[_uuid];

        removeItemFromArray(userOwnedFiles[msg.sender] , _uuid);

        for(uint256 i = 0 ; i < file.authorizers.length ; i++) {
            removeItemFromArray(fileSharedWithUser[file.authorizers[i].userAddress] , _uuid);
        }
...
interface IBullitFile {
    ...
    function decryption(string calldata _uuid) external payable;

    function deleteFile(string calldata _uuid) external;
