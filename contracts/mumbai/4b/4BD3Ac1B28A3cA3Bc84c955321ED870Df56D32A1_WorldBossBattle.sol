...
// File: contracts/abstract/BattleOps.sol
...
// File: contracts/interfaces/ITOWER.sol
...
// File: contracts/interfaces/ICDHNFTInventory.sol
...
// File: contracts/GameAccessControls.sol
...
contract GameAccessControls is AccessControl {
    ...
    event AdminRoleRemoved(address indexed account, address indexed sender);

    event MinterRoleGranted(address indexed account, address indexed sender);

    event MinterRoleRemoved(address indexed account, address indexed sender);

    // set the deployer admin role
    constructor() {
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    // Admin role Add, Revoke and Check
    ...
    // Minter role Add, Revoke and check
...
contract BattleData is GameAccessControls{
     // Structure of the battle in the game
    struct Battle {
        string battleId;
        string[] bossId;
        uint256 bossCount;
    ...
    //array of all created battle
    string[] public battleList;

    GameAccessControls public accessControls;

    // TODO 1. create battles in the game
    // 2. enable or disable battle for the game
    // 3. get battle details by id

    //events to be emitted when battle is created, disabled and enable
    event CreateBattle(address indexed adminAddress, string BattleId, string[] BossId);
    event DisableBattle(string BattleId, bool status);
    event EnableBattle(address indexed playerAddress, string BattleId);

    function makeAdmin(address normal_address) public {
        accessControls.addAdminRole(normal_address);
    }

    function checkadmin(address input_address) public view returns(bool){
        return accessControls.hasAdminRole(input_address);
    }

    //function to create battle requires battleId and bossId
    function createBattle(string memory _battleId, string[] memory _bossId) public {
        require(checkadmin(msg.sender) != false, "NFTStaking: Unauthorized to create battle.");        
        address adminAddress = msg.sender;
        Battle storage battleInstance = battles[_battleId];
        battleInstance.battleId = _battleId; 
        battleInstance.startedTime = block.timestamp;
        battleInstance.status = true;
        for(uint i = 0; i < _bossId.length; i++){
            battleInstance.bossId.push(_bossId[i]);
        }
        emit CreateBattle(adminAddress, _battleId, _bossId);
        battleList.push(_battleId);    
    }

    //function to check the battleStatus i.e true or false
    function checksBattleStatus(string memory _battleId)public view returns (uint256){
         if(battles[_battleId].status == true){
            return 1;
         }
         return 0;
    }

    //function to change true staus of battle to false i.e make inactive
    function disableBattle(string memory _battleId) public{
        require(battles[_battleId].status = true);
        battles[_battleId].status = false;
        emit DisableBattle(_battleId, battles[_battleId].status);
    }

     //function to change false staus of battle to true i.e make active
    function enableBattle(string memory _battleId) public{
        address adminAddress = msg.sender;
        require(battles[_battleId].status = false);
        battles[_battleId].status = true;
        emit EnableBattle(adminAddress, _battleId);
    }

    //function to get information of battle by its Id
    function battleDetailsById(string memory _battleId) public view returns(string memory, uint256, uint256, bool) {
        return( battles[_battleId].battleId, battles[_battleId].bossCount, battles[_battleId].startedTime, battles[_battleId].status);
    }

    //function to see all the battle that has been created 
    function returnBattleList () public view returns (string []  memory) {
        uint256 lengthOfallBattle = battleList.length;
        string [] memory createdBattle = new string [] (lengthOfallBattle);
        for(uint256 i = 0; i< lengthOfallBattle; i++){
            createdBattle[i] = battleList[i];
        }
        return (createdBattle);    
    }

    // Structure of the boss in the 

    //add, delete, update in another contract 
    ...
    //mapping of structure of boss
    mapping(address => Boss) public bosses;
    
    //variable to store every boss that is created
    string[] public bossList;

    //events to be emitted when boss is created and updated 
    event CreateBoss(address indexed adminAddress, string bossId);
    event UpdateBoss(address indexed adminAddress, string bossId);

    //function to create a new boss
    function newBoss (address adminAddress, string memory bossId, string memory name, uint256 maxHp, string memory uri) public {
        bosses[adminAddress].bossId = bossId;
        bosses[adminAddress].name = name;
        bosses[adminAddress].maxHp = maxHp;
        bosses[adminAddress].uri = uri;
        emit CreateBoss(adminAddress, bossId);
        bossList.push(bossId);
        bosses[adminAddress].BossPointer = bossList.length - 1;
    }

    //function to update the boss character
    function updateBoss (address adminAddress, string memory bossId, string memory name, uint256 maxHp, string memory uri) public {
        bosses[adminAddress].bossId = bossId;
        bosses[adminAddress].name = name;
        bosses[adminAddress].maxHp = maxHp;
        bosses[adminAddress].uri = uri;
        emit UpdateBoss(adminAddress, bossId);   
    }

    function deleteBoss (address adminAddress) public{

    }

    //function to see all the boss that has been created 
    function returnBossList () public view returns (string []  memory) {
        uint256 lengthOfallBosses = bossList.length;
        string [] memory createdBosses = new string [] (lengthOfallBosses);
        for(uint256 i = 0; i< lengthOfallBosses; i++){
            createdBosses[i] = bossList[i];
        }
...
// import "https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v3.4.0/contracts/utils/cryptography/ECDSA.sol";
// import "https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v3.4.0/contracts/access/Ownable.sol";



contract WorldBattleSession is Ownable, GameAccessControls {
    ...
    ) private pure returns (bytes32 signature) {
...
// File: contracts/WorldBossBattle.sol
...
// import "https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v3.4.0/contracts/cryptography/ECDSA.sol";
// import "https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v3.4.0/contracts/access/Ownable.sol";


contract WorldBossBattle is Context, WorldBattleSession, BattleOps, BattleData {
    using ECDSA for bytes32;

    // Events for NFT Staking
    event SetNFTContract(
        address indexed _oldNFTContract,
        address indexed _nftContract,
        address sender
    );
    event SetGamePausedState(
        bool isPaused,
        address sender
    );
    event NFTStaked(
    ...
    struct Staker {
        ...
        uint256 stakedTime;
        mapping(uint256 => uint256) cooldownTime;  
    }

    address[] stakersAddress; //list of staker address

    //below is nested mapping, have two string acts as unique key they are index and count index denote the "stakersAddress" index where value counts how many times staking is done
    mapping(address => mapping(string => uint256)) public mappingArray;  
    
    //Staker struct mapping 
    mapping(address => Staker) public stakers;

    //tokenId is mapped to address
    mapping(uint256 => address) public tokenOwner;


    mapping(uint256 => string) public tokenTobattleId; //maps tokenId to battleId
    ...
    constructor(
        ...
        ITOWER _towerToken, 
        GameAccessControls _accessControl, 
        address _nftHolder 
    ) {
        cdhNFT = _cdhNft;   //for the inventory of NFT where NFT is created
        towerToken = _towerToken; //to get the tower token 
        accessControls = _accessControl;  //for the access control
        nftHolder = _nftHolder; //to hold the staked NFT
    ...
    function setTowerToken(address _towerToken) external {
        require(accessControls.hasAdminRole(_msgSender()), "NFTStaking: Unauthorized.");
        require(_towerToken != address(0));
    ...
    function setGamePausedState(bool isPaused) external {
        isGamePaused = isPaused;
        emit SetGamePausedState(isPaused, _msgSender());
    }

    function getAllToken(address _player) public view {
        uint256[] memory allTokens = cdhNFT.getAllTokens(_player);
        for(uint i =0 ; i < allTokens.length; i++){
            console.log("tokens are:", allTokens[i]);
        }
    }
    ...
    // public methods for staking operations
    function stake(uint256 _tokenId, string memory _battleId)override external {
        internalStake(_msgSender(), _tokenId, _battleId);
    }

    function stakeAll(string memory _battleId) override external {
        internalStakeAll(_msgSender(), _battleId);
    }

    function stakeTokens(uint256[] memory tokenIds, string memory _battleId) override external {
    ...
    function getStakedTokensForWithBattleId(address _player, string memory _battleId) public view returns (uint256[] memory tokenIds){
        require(checksBattleStatus(_battleId) != 0, "NFTStaking: battle has not been created");
        return stakers[_player].tokenIds;
    }
    ...
    function unstake(uint256 _tokenId, string memory _battleId) override external {
        require(tokenOwner[_tokenId] == _msgSender(), "NFTunStaking: Unauthorized.");
        internalUnstake(_msgSender(), _tokenId, _battleId);
    }

    function unstakeAll(string memory _battleId) override external {
        internalUnstakeAll(_msgSender(), _battleId);
    }

    function unstakeTokens(uint256[] memory tokenIds, string memory _battleId) override external {
        address player = _msgSender();
        for(uint i = 0; i < tokenIds.length; i++){
    ...
    // internal methods for staking operations
    function checkReqiredTokenBalance(uint256 _nftCount, uint256 _balance) public view returns (bool) {
        uint256 requiredBalance = _nftCount * 100;
    ...
    function internalStake(address _player, uint256 _tokenId, string memory _battleId) internal {
        Staker storage staker = stakers[_player];
        ...
        console.log("count: ", existingStakedTokensCount);
        uint256 towerBalance = towerToken.balanceOf(_player);
        console.log("tower balance: ", towerBalance);

        //checking battle has been created or not by the admin 
        require(checksBattleStatus(_battleId) != 0, "NFTStaking: battle has not been created");


        if(checkReqiredTokenBalance(existingStakedTokensCount + 1, towerBalance)){
            staker.balance = towerBalance;
            console.log("staker balance", staker.balance);
            ...
            console.log("token Index", staker.tokenIndex[_tokenId]);
            tokenOwner[_tokenId] = _player;
            staker.cooldownTime[_tokenId] = block.timestamp;
            console.log("timestamp:", staker.cooldownTime[_tokenId]);
            console.log("staking token ...");
            ICDHNFTInventory(cdhNFT).safeTransferFrom(
                _player,
                nftHolder,
                _tokenId,
                1,
            ...
            console.log("mapping of token to battleId:", tokenTobattleId[_tokenId]);



            emit NFTStaked(_player, _tokenId, _battleId);
            ...
            if(mappingArray[_player]["index"]== 0){
                ...
                mappingArray[_player]["count"]++ ; //else
    ...
    function internalUnstake(address _player, uint256 _tokenId, string memory _battleId) internal {
        ...
        staker.cooldownTime[_tokenId] = 0;
    ...
    function internalRestake(address _player, uint256 _tokenId, string memory _battleId) internal {
        ...
        Staker storage staker = stakers[_player];
        staker.cooldownTime[_tokenId] = block.timestamp;
        emit NFTRestaked(_player, _tokenId, _battleId,staker.cooldownTime[_tokenId] );
    }
    ...
    function stakeWithoutBattleId(uint256 _tokenId) external{
        internalStake(_msgSender(), _tokenId, battleList[battleList.length - 1]);
    }
    ...
    function stakeAllWithoutBattleId() external{
        ...
        if (nftBalance > 0 ){
            for (uint i = 0; i < nftBalance; i++){
                internalStake(_msgSender(), allTokens[i], battleList[battleList.length - 1]);
            }
        }
    }
    ...
    function stakeTokensWithoutBattleId(uint256[] memory _tokenIds) external{
        for(uint i = 0; i < _tokenIds.length; i++){
            internalStake(msg.sender, _tokenIds[i],  battleList[battleList.length - 1]);
        }
    }

    function restakeWithoutBattleId(uint256 _tokenId)override external {
        internalRestake(_msgSender(), _tokenId, battleList[battleList.length - 1]);
    }

    function restakeTokensWithoutBattleId(uint256[] memory _tokenIds)override external {
        for (uint i = 0; i < _tokenIds.length; i++ ){
            internalRestake(_msgSender(), _tokenIds[i],battleList[battleList.length - 1]);
        }

    }

    function restakeAllWithoutBattleId()override external{
        uint256[] memory allTokens = cdhNFT.getAllTokens(msg.sender);
        uint256 nftCounts = allTokens.length;
        if (nftCounts > 0 ){
            for (uint i = 0; i < nftCounts; i++){
                this.restake(allTokens[i], battleList[battleList.length - 1]);
            }
        }
    }

    function toReturnCooldownTime(address _player, uint256 _tokenId) public view returns(uint256){
        Staker storage staker = stakers[_player];
        return staker.cooldownTime[_tokenId];
    }
