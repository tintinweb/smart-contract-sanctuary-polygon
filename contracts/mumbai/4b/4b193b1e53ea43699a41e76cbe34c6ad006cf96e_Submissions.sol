...
contract ERC1155Base is HasSecondarySaleFees,ERC1155Metadata_URI, HasContractURI, ERC1155 {
    ...
    function _mint(address _to,uint256 _id, Fee[] memory _fees, uint256 _supply, string memory _uri) internal {
...
contract Submissions is ERC1155Base,Ownable {
    string public name;
    ...
    constructor(string memory _name,
                ...
                string memory tokenURIPrefix) ERC1155Base(contractURI, tokenURIPrefix) public {
        name = _name;
        symbol = _symbol;
        price = _price;
    ...
    function submit(string calldata uri) external payable {
      ...
      emit Submited(msg.sender,uri,uriHash);

    }

    function mint(address _to,Fee[] calldata fees,uint256 supply, string calldata uri) external onlyOwner {
        bytes32 uriHash = keccak256(abi.encodePacked(uri));
        require(uriHashUsed[uriHash],"Uri was not submited");
        _totalSupply = _totalSupply.add(1);
        _mint(_to,_totalSupply, fees, supply, uri);
        creators[_totalSupply] = submissionAuthor[uriHash];
    }
