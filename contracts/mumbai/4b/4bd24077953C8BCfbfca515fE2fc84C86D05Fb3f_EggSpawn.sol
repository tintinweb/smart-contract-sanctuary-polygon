...
contract EggSpawn is ERC721SignatureMint {
    ...
    function _createEgg(uint256 tokenId) internal {
        Egg storage newEgg = eggs[tokenId];
        newEgg.tokenId = tokenId;
        newEgg.spawnSeason = currentSeason;
        newEgg.age = currentAge;
        newEgg.tier = currentDay;
    }
    ...
    function _spawnEggs(
        ...
        for (uint256 i = 0; i < count; i++) {
            ...
            if (i > 0) {
                tokenIds = abi.encodePacked(tokenIds, ",");
            }
            tokenIds = abi.encodePacked(tokenIds, Strings.toString(tokenId));
        }
        return string(tokenIds);
    }

    function mintEggWithSignature(MintEggRequest calldata _req, bytes calldata _signature) public payable {
        // Verify the signature
        bytes32 message = keccak256(abi.encodePacked(_req.to, _req.uri, _req.pricePerToken, _req.quantity, _req.primarySaleRecipient, _req.currency, _req.royaltyBps, _req.royaltyRecipient)).toEthSignedMessageHash();
        address signer = message.recover(_signature);
        require(signer == signingAddress, "Invalid signature");

        // Check the requirements for minting eggs
        ...
        require(_req.pricePerToken * _req.quantity == msg.value, "Wrong amount sent");
        require(!usedNonces[_req.nonce], "Duplicate request");
        require(block.timestamp < _req.expires, "Signature expired");
        require(block.timestamp >= seasonStart && seasonStart != 0, "Not in season");
        require(dailyEggsSpawned + _req.quantity <= dailyEggSupply, "Daily egg supply reached");
        require(seasonEggsSpawned + _req.quantity <= seasonEggSupply, "Season egg supply reached");

         usedNonces[_req.nonce] = true;

        dailyEggsSpawned += _req.quantity;
        seasonEggsSpawned += _req.quantity;
        
        // Mint the eggs
        string memory tokenIds = _spawnEggs(_req.to, _req.quantity);

        emit SpawnEggs(_req.to, _req.quantity, _req.nonce, tokenIds, currentAge, currentSeason, currentDay);
    }
