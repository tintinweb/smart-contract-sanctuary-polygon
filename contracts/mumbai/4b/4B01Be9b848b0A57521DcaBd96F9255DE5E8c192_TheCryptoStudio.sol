...
contract TheCryptoStudio is ERC1155
{
    ...
    function DeclareArt(string memory name , string memory imageCID , string memory animationCID , string memory description ,string memory spaceName, uint256 mintPrice, bool token_type) public payable returns(uint256) {
        require(spaceInfoMap[spaceName].spaceArtists.contains(msg.sender));
        tokenID.increment();
        uint256 temp = tokenID.current();
        spaceInfoMap[spaceName].spaceTokens.add(temp);
        tokenInfoMap[temp].creator = msg.sender;
        tokenInfoMap[temp].tokenType = token_type;
        tokenInfoMap[temp].price = mintPrice;
        string memory insert_statement = SQLHelpers.insertMainStatement(MAIN_TABLE_PREFIX, mainTableID,temp,description,imageCID,name,animationCID);
    ...
    // function spaceOwnedNFTs(string memory spaceName, address sender) public view returns(uint256){
    ...
    //             sum = sum + balanceOf(sender,index);
    //         }
    //     }
    //     return sum;
    // }
    ...
    function assignAnimationURI(uint256 tokenid , string memory animationURL) public {
        ...
        tokenInfoMap[tokenid].creator = address(0);
