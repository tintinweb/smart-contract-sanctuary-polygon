    ...
    ReentrancyGuardUpgradeable
{
    ...
    function _calculateWayGateFee(
        ...
        platformFeeDistribution(_nftListingIdHash, WayGateAmount);
    ...
    function _transferAmountToSeller(
        ...
        _transferAmountHandler(_nftListingIdHash, seller, amount);
    }
    ...
    function platformFeeDistribution(
        bytes32 _nftListingIdHash,
        uint totalAmount
    ) internal {
        uint256 airdropFeeAmount = (totalAmount *
            airdropHolderReceivingPercentage) / 1000;
        _transferAmountToAirdropHolders(_nftListingIdHash, airdropFeeAmount);
        uint256 partnersFeeAmount = (totalAmount *
            partnersReceivingPrecentage) / 1000;
        _transferAmountToPartners(_nftListingIdHash, partnersFeeAmount);
        totalAmount = totalAmount - (airdropFeeAmount + partnersFeeAmount);
        _transferAmountHandler(_nftListingIdHash, platformFeeReceiver ,totalAmount);
        console.log("airdropFeeAmount",airdropFeeAmount);
        console.log("partnersFeeAmount",partnersFeeAmount);
        console.log("platformFeeReceiver",totalAmount);
    }

    function _transferAmountToPartners(
        bytes32 _nftListingIdHash,
        uint256 _partnersFee
    ) internal {
        if (wayGatePartners.length != 0) {
            uint256 feePerInvsetor = _partnersFee / wayGatePartners.length;
    ...
    function _transferAmountToAirdropHolders(
        ...
        _transferAmountHandler(
            _nftListingIdHash,
            airdropFeeDistributorAddress,
            airdropAmount
        );
    }

    function _transferAmountHandler(
        bytes32 _nftListingIdHash,
        address receiver,
        uint256 amount
    ) internal {
        if (
            ...
            wayGateToken.transfer(receiver, amount);
        }
    }
