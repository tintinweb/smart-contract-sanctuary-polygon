...
interface ILooxooryNft{
    ...
    function getNFT(uint256 _tokenId) external view returns(string memory uri,uint256 price,address creatorAddress,uint256 campaignId,uint256 numberOfNfts,uint256 platformFee);
}
...
contract LooxooryNFTMarketplace is ReentrancyGuard,Ownable {
    ...
    constructor(address _nftContract, address _nftHolder, address _randomContract) {
        ...
        require(_nftHolder!=address(0),"Invalid address");
        require(_randomContract!=address(0),"Invalid address");

        NFTContract = _nftContract;
        NFTHolderAddress = _nftHolder;
    ...
    function createMarketSale(NFTVoucher calldata voucher,bool donation) public payable{
        ...
        (string memory uri,uint256 price,address creatorAddress,uint256 campaignId,uint256 numberOfNfts,uint256 platformFee) = ILooxooryNft(NFTContract).getNFT(tokenId);
      ...
      totalSale = totalSale.add(price);
        emit Minted(tokenId,uri,price,creatorAddress,campaignId,numberOfNfts,platformFee);
    ...
    function listNftForAuction(NFTVoucher calldata voucher,uint256 bidEndTime,uint256 tokenId) external onlyOwner{
        require(!isOnAuction,"An item is already on auction");
        ...
        (uri,price,creatorAddress,campaignId,numberOfNfts,platformFee) = ILooxooryNft(NFTContract).getNFT(tokenId);
  ...
  function resetAuction() external onlyOwner{
    ...
    require(currentlyOnAuction[currentlyOnAuctionTokenID].biddingTime<block.timestamp,"Cannot reset now, bid time is not ended");
  ...
  function getWonCampaigns(address userAddress) external view returns(
    ...
    wonCampaign = wonCampaigns[userAddress];
  }

  function updateCampaignDrawTime(uint256 _campaignId,uint256 _drawTime) external onlyOwner{
    require(campaigns[_campaignId].campaignID != 0,"Invalid campaign id");
