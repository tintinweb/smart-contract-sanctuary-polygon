...
contract ERC721MintableWithSlots is ERC721Mintable {
    ...
    function initialize(string memory name_, string memory symbol_, address owner_, uint256 slotStart_,
        uint256 slotEnd_, SystemContext system_) public initializer
     {
     ...
     */
    function setSlot(uint256 slotStart_, uint256 slotEnd_) external onlyBridgeRole {
        slotStart = slotStart_;
        slotEnd = slotEnd_;
    }
...
contract ContractFactory {
    ...
    mapping(string => address) public beacons; //name -> beacon address
    ...
    function registerNewBlueprint(address blueprintAddress, string memory blueprintName, bool isNative) external onlyRole(systemContext.getAccessControlList().CONTROL_LIST_ADMIN_ROLE()) {
        require(latestBlueprintVersion[blueprintName] == 0 && blueprints[blueprintName][1] == address(0), "Contract already registered");
        ...
        beacons[blueprintName] = address(beacon);
    ...
    function registerNewBlueprintVersion(address blueprintAddress, string memory blueprintName, uint16 version) external onlyRole(systemContext.getAccessControlList().CONTROL_LIST_ADMIN_ROLE()) {
        require(latestBlueprintVersion[blueprintName] != 0, "Contract doesn't exist");
        require(blueprints[blueprintName][version-1] != address(0), "Contract doesn't exist");
    ...
    /**
     * @dev Removes blueprint
     */
    function deregisterBlueprint(string memory name) external onlyRole(systemContext.getAccessControlList().CONTROL_LIST_ADMIN_ROLE()) {
    ...
    function _deployProxy(string memory blueprintName, bytes memory constructorParams) internal returns(address) {
        return address(new BeaconProxy(beacons[blueprintName], constructorParams));
    }
