...
contract Gift is ContextUpgradeable {
    ...
    function initialize(address _settingAddress, address _nftCheckerAddress, address _signerAddress) external initializer {
    ...
    function setSignerWallet(address _newSigner) external {
        if (!setting.isSuperAdmin(_msgSender())) revert CallerIsNotTheSuperAdmin();
    ...
    function giftingNFTs(
        ...
        if (nftChecker.isERC721(_token)) {
            ...
            for (uint256 i = 0; i < _tokenIds.length; i++) {
                ...
                IOSB1155(_token).safeTransferFrom(_msgSender(), _accounts[i], _tokenIds[i], amounts[i], "");
            }
        }
    ...
    function giftingCash(address _token, address[] memory _accounts, uint256[] memory _amounts, bytes memory _signature) external {
