 ...
 * @notice User Request String2D contract for string[][] type of response using chainlink direct request
    ...
    function runRequestQuery(string memory query, string memory resourceId, string memory jobId) external nonReentrant returns(bytes32 requestId){
        require(
            getChainlinkToken().approve(
                address(getSxTRelayContract().implementationContract()), 
                getSxTRelayContract().FEE()), 
