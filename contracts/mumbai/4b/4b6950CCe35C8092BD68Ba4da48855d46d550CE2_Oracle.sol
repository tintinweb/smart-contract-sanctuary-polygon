...
contract Oracle is Ownable {
    ...
    mapping(address => address[]) public _partners;
    mapping(address => address) public _distributors;
    mapping(address => uint256) public pointAmount;

    uint16 constant BONUS_LINES_COUNT = 20;
    uint16[BONUS_LINES_COUNT] public ref_bonuses = [
        70,
        50,
        40,
        30,
    ...
    address[] newLeftPartners;
    address[] newRightPartners;
    address[] leftPartners;
    address[] rightPartners;

    mapping(address => uint16) public PositionLeftOrRight;

    /* ========== CONSTRUCTOR ========== */

    constructor(address _stakingToken, address _oracleToken) {
    ...
    function deposit(address _upline, uint256 amount) external {
        require(
            ...
            "Users must deposit in excess of the minimum deposit amount."
        );
        require(
            amount % minimumDepositAmount == 0,
            "Users must send 10 multiple USDT"
        );
        ...
        uint256 addedAmount = stakingToken.balanceOf(address(this)) -
            contractBalance;
    ...
    function _setUpline(
        ...
        require(_upline != _addr && _upline != address(0));
        if (_distributors[_addr] != _upline) {
            _distributors[_addr] = _upline;
        }
        _distributors[_addr] = _upline;
        if (_partners[_upline].length == 0) {
            _partners[_upline].push(_addr);
            
        } else if (_partners[_upline].length == 1) {
            if (_partners[_upline][0] != _addr) {
                _partners[_upline].push(_addr);

            }
        } else {
            require(
                _partners[_upline][0] == _addr || _partners[_upline][1] != _addr
            );
        }
        address levelDistributors = _upline;
        for (uint256 j = 0; j < 20; j++) {
            if (levelDistributors == address(0)) {
                levelDistributors = owner();
            }
            uint256 bonus = (_amount * ref_bonuses[j]) / PERCENT;
            _rewardAmount[levelDistributors] += bonus;
            levelDistributors = _distributors[levelDistributors];
        }
    }
    ...
    function withdraw() external {
        ...
        require(
            rewardAmount >= minimumDepositAmount,
            "You can only withdraw this rewardAmount bigger than 10 USDT"
        );
        require(
            _withdrawTime[msg.sender].add(86400) > block.timestamp,
            "You can withdraw after 1 day"
        );
        if (_withdrawTime[msg.sender].add(86400) < block.timestamp) {
            if (rewardAmount > _depositedAmount[msg.sender].mul(2)) {
                Pool3Amount += rewardAmount.sub(
                    _depositedAmount[msg.sender].mul(2)
                );
    ...
    function BinaryPlan() external {
        require(
            lastBinaryRewardTime.add(86400) < block.timestamp,
            "You can do this plan every day"
        );
        uint256 totalPoint;
        ...
        for (uint16 i = 0; i < allDistributors.length; i++) {
            ...
            if (_partners[distributor].length == 2) {
                address leftPartner = _partners[distributor][0];
                address rightPartner = _partners[distributor][1];
                _leftPartnersAmount[distributor] = _depositedAmount[leftPartner];
                _rightPartnersAmount[distributor] = _depositedAmount[rightPartner];
                leftPartners.push(leftPartner);
                rightPartners.push(rightPartner);
                for (uint16 j = 0; j < 20; j++) {
                    uint256 leftPartnersLength = leftPartners.length;
                    for (uint16 k = 0; k < leftPartnersLength; k++) {
                        _leftPartnersAmount[distributor] += _depositedAmount[leftPartners[k]];
                        for(uint16 l = 0; l < _partners[leftPartners[k]].length; l++ )
                        {
                            if (_partners[leftPartners[k]][l] != address(0)) {
                                newLeftPartners.push(_partners[leftPartners[k]][l]);
                            }
                        }

                    }
                    if (newLeftPartners.length == 0) {
                        break;
                    }
                    for (uint16 k = 0; k < leftPartnersLength; k++) {
                        leftPartners.pop();
                    }
                    uint256 newLeftPartnersLength = newLeftPartners.length;
                    for (uint8 k = 0; k < newLeftPartnersLength; k++) {
                        leftPartners.push(newLeftPartners[k]);
                    }
                    for (uint8 k = 0; k < newLeftPartnersLength; k++) {
                        newLeftPartners.pop();
                    }
                ...
                for (uint16 j = 0; j < 20; j++) {
                    uint256 rightPartnersLength = rightPartners.length;
                    
                    for (uint16 k = 0; k < rightPartnersLength; k++) {
                        _rightPartnersAmount[distributor] += _depositedAmount[rightPartners[k]];
                        for(uint16 l = 0; l < _partners[rightPartners[k]].length; l++ )
                        {
                            if (_partners[rightPartners[k]][l] != address(0)) {
                                newRightPartners.push(_partners[rightPartners[k]][l]);
                            }
                        }

                    }
                    if (newRightPartners.length == 0) {
                        break;
                    }
                    for (uint16 k = 0; k < rightPartnersLength; k++) {
                        rightPartners.pop();
                    }
                    uint256 newRightPartnersLength = newRightPartners.length;
                    for (uint8 k = 0; k < newRightPartnersLength; k++) {
                        rightPartners.push(newRightPartners[k]);
                    }
                    for (uint8 k = 0; k < newRightPartnersLength; k++) {
                        newRightPartners.pop();
                    }
                }
                if (_leftPartnersAmount[distributor] > 0 && _rightPartnersAmount[distributor] > 0) {
                    if (_leftPartnersAmount[distributor] > _rightPartnersAmount[distributor]) {
                        pointAmount[distributor] = _rightPartnersAmount[distributor].div(
                            minimumDepositAmount
                        );
                    } else {
                        pointAmount[distributor] = _leftPartnersAmount[distributor].div(
                            minimumDepositAmount
                        );
                    }
                }
                totalPoint += pointAmount[distributor];
            }
        }
        for (uint16 i = 0; i < allDistributors.length; i++) {
            _rewardAmount[allDistributors[i]] += Pool2Amount
                .mul(pointAmount[allDistributors[i]])
                .div(totalPoint);
        }
    ...
    function dividendPlan() external {
        require(
            lastRewardDividedTime.add(604800) < block.timestamp,
            "You can divide Reward every week"
        );
        for (uint16 i = 0; i < allTokenHolders.length; i++) {
            address holder;
            holder = allTokenHolders[i];
            uint256 dividendAmount = Pool3Amount
                .mul(_depositedAmount[holder])
                .mul(10 ** 12)
                .div(_totalTokenSupply);
    ...
    function ManagementBonusPlan() external {
        require(
            lastManagementBonusTime.add(2592000) < block.timestamp,
            "You can do this plan every month"
        );
        uint256 totalPoint;
        ...
        for (uint16 i = 0; i < allManageDistributor.length; i++) {
            ...
            if (_partners[distributor].length == 2) {
                address leftPartner = _partners[distributor][0];
                address rightPartner = _partners[distributor][1];
                uint256 leftPartnersAmount = _depositedAmount[leftPartner];
                uint256 rightPartnersAmount = _depositedAmount[rightPartner];
                leftPartners.push(leftPartner);
                rightPartners.push(rightPartner);
                for (uint16 j = 0; j < 20; j++) {
                    uint256 leftPartnersLength = leftPartners.length;
                    ...
                    for (uint16 k = 0; k < leftPartnersLength; k++) {
                        leftPartnersAmount += _depositedAmount[leftPartners[k]].div(minimumManageAmount);
                        for(uint16 l = 0; l < _partners[leftPartners[k]].length; l++ )
                        {
                            if (_partners[leftPartners[k]][l] != address(0)) {
                                newLeftPartners.push(_partners[leftPartners[k]][l]);
                ...
                for (uint16 j = 0; j < 20; j++) {
                    ...
                    for (uint16 k = 0; k < rightPartnersLength; k++) {
                        rightPartnersAmount += _depositedAmount[rightPartners[k]].div(minimumManageAmount);
                        for(uint16 l = 0; l < _partners[rightPartners[k]].length; l++ )
                        {
                            if (_partners[rightPartners[k]][l] != address(0)) {
                                newRightPartners.push(_partners[rightPartners[k]][l]);
                ...
                if (leftPartnersAmount > 0 && rightPartnersAmount > 0) {
                    if (leftPartnersAmount > rightPartnersAmount) {
                        pointAmount[distributor] = rightPartnersAmount;
                    } else {
                        pointAmount[distributor] = leftPartnersAmount;
                    }
                }
                totalPoint += pointAmount[distributor];
            }
        ...
        for (uint16 i = 0; i < allManageDistributor.length; i++) {
            address distributor;
            distributor = allManageDistributor[i];
            uint256 amount = Pool4Amount.mul(pointAmount[distributor]).div(totalPoint);
    ...
    function totalInfos(address _account)
        external
        view
        returns (
            // uint256,
    ...
    {
        return (
            // levelsLength[_account],
            owner(),
            _withdrawTime[_account],
            _depositedAmount[_account],
            _rewardAmount[_account],
            _leftPartnersAmount[_account],
            _rightPartnersAmount[_account]
        );
    }

    function poolInfos(address _account)
        external
        view
        returns (
            uint256,
    ...
    function getPartnersLength(address _account) external view returns(uint256)
    {
        return (_partners[_account].length);
    }
