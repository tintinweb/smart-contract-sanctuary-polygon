...
contract WaterSamples {
    ...
    function getSample(string memory _sampleAddress) public view returns (string memory sampleAddress, uint256 leadPercentage, string memory message, string memory image) {
        require(Samples[_sampleAddress].exists, "Sample does not exist");
        sample storage s = Samples[_sampleAddress];
        return(s.sampleAddress, s.lead, s.message, s.image);
    }
