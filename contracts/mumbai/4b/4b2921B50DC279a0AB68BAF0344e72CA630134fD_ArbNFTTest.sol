...
contract ArbNFTTest is ERC721, Ownable, Pausable {
    ...
    string[2][3] private bgAttributesOne;
    function setBgAttributeOne(bytes memory encodec) public onlyOwner {
        bgAttributesOne = abi.decode(encodec, (string[2][3]));
    }

    string[2][3] private bgAttributesTwo;
    function setBgAttributeTwo(bytes memory encodec) public onlyOwner {
        bgAttributesTwo = abi.decode(encodec, (string[2][3]));
    }

        string[2][3] private bgAttributesThree;
    function setBgAttributeThree(bytes memory encodec) public onlyOwner {
        bgAttributesThree = abi.decode(encodec, (string[2][3]));
    }

    function genBg(uint256 h) internal view returns(string[2] memory) {
        string[2] memory bg;
        uint8 seed = uint8(h % 9);
        if(seed <= 2) {
            bg = bgAttributesOne[seed];
        }
        else if(seed > 2 && seed <= 5){
            bg = bgAttributesTwo[seed - 3];
        } else {
            bg = bgAttributesThree[seed - 6];
        }
        
        return bg;
    }
    string bg1 = '<image width="150" height="150" xlink:href="';
    string bg2 = '"></image>';
