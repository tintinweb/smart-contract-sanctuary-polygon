    ...
    ERC1155Holder
{
        ...
        for(uint256 i ; i < offeredList[boxID].length ; ++i) {
            for(uint256 j ; j < offers[offeredList[boxID][i]].length ; ++j) {
                if(offers[offeredList[boxID][i]][j] == boxID) {
                    for(uint256 k = j ; k < offers[offeredList[boxID][i]].length - 1 ; ++k)
                        offers[offeredList[boxID][i]][k] = offers[offeredList[boxID][i]][k + 1];
                    offers[offeredList[boxID][i]].pop();
                }
            }
        }
        _deleteAssets(boxID);
    ...
    */
    ...
    function purchaseBox(uint256 BoxID) payable public isOpenForSwap {
        ...
        require(msg.sender != swapBoxes[BoxID].owner,"Can't purchase own box");
        ...
        nonReentrant
    {
        ...
        for(uint256 i ; i < offerLength ; ++i) {
            if(offers[listBoxID][i] == offerBoxID) {
                for(uint256 j = i ; j < offers[listBoxID].length - 1 ; ++j)
                    offers[listBoxID][j] = offers[listBoxID][j + 1];
                offers[listBoxID].pop();
            }
        }

        for(uint256 i ; i < offeredList[offerBoxID].length; ++i){
            if(offeredList[offerBoxID][i] == listBoxID) {
                for(uint256 j = i ; j < offeredList[offerBoxID].length - 1 ; ++j)
                    offeredList[offerBoxID][j] = offeredList[offerBoxID][j +1];
