    ...
    Oracle
{
    ...
    function transferReserved(
        ...
        /// @dev tranfer the token to the contract to the sen
        IERC721(pair.tokenA).transferFrom(address(this), _wallet, _tokenId);
    }
    ...
    function BuyWithCustomToken(
        ...
        require(!pair.isNative, "not custom token native");
        ...
        /*
         *
         * Ahora que ya tenemos los tokens, podemos hacer el swap
         */
        ...
        uint256 valueInUsd = _amountTokens.mul(latestPrice).div(1e18);

        require(
            valueInUsd >= pair.price,
            "Buy With Custom Token: The price is lower than the minimum price"
        );

        /// @dev @dev is owner of the nft
        require(
            address(this) == IERC721(pair.tokenA).ownerOf(_tokenId),
            "Buy With Custom Token: Sender must be owner"
        );

        /// @dev  transfer the token to the contract
        IERC721(pair.tokenA).transferFrom(
            address(this),
            _msgSender(),
            _tokenId
        );
    ...
    function BuyWithNativeToken(uint256 _pairId, uint256 _tokenId)
        external
        payable
        noReentrant
    {
        ...
        require(
            msg.value > 0,
            "BuyWithNativeToken: Send BNB to buy some tokens"
        );
        ...
        /// @dev check pair is native
        require(pair.isNative, "not token native");
