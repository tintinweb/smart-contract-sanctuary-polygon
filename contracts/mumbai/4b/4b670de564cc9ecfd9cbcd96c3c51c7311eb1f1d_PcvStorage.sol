...
contract PcvStorage is Ownable,PcvStruct{
    ...
    address public constant POOL_PROVIDER = 0xd05e3E715d945B59290df0ae8eF85c1BdB684744;
    ...
    address _settlement; // settlement contract
    address _comptroller; // comptroller contract
    ...
    mapping (address => bool) _autoExecute;
    // pcv => strategyId
    ...
    function addPcv(address pcvOwner,address pcv,address settleAsset,uint256 minInvest,uint256 maxInvest) external onlyFactory{
        ...
        emit addPcvEvent(pcvOwner,pcv);
    }
    event addPcvEvent(address indexed pcvOwner, address indexed pcv);
    ...
    function addStrategy(
        ...
        if(strgy.strategyId == 1 && (!strgy.closePosition)){
            ...
            _autoExecute[pcv] = true;
        }
        strategyCountMap[pcv] = strategyCountMap[pcv]+1;

        emit addStrategyEvent(pcv,strategyCountMap[pcv]);
    }
    ...
    function getParamsNeed(address protocol,bytes memory methodAbi) 
    internal 
    view 
    returns(bool[] memory amount ,bool[] memory invest) {
    ...
    // function getStrategyOperate(address pcv,uint256 id) public view returns(address[] memory protocol,string [] memory methods){
    //     PcvStrategy memory strgy = pcvStrategyMap[pcv][id];
    //     protocol = strgy.protocol;
    //     methods = strgy.methods;
    ...
    function autoExecute(address pcv) public view returns(bool){
            return _autoExecute[pcv];
    }

    function setAutoExecute(bool isAuto) external onlyPcv{
            require(isAuto != _autoExecute[_msgSender()],"value already exist");
            bool oldVal = _autoExecute[_msgSender()];
            _autoExecute[_msgSender()] = isAuto;
    ...
    function setMaxInputPercent(uint256 maxInputPercent) external onlyOwner{
    ...
    function getMaxInputPercent() public view returns(uint256){
        return _maxInputPercent;
    }
    ...
    function setComtroller(address comptroller) external {
        _comptroller = comptroller;
    }

    function getComtroller() public view returns(address) {
        return _comptroller;
    }
