...
contract VolcanoMoney is Ownable, Initializable, Pausable {
    ...
    function autoUpgrade(address _user, uint8 _currentLevel , uint8 _nextLevel) internal {
        if(users[_user].holdAmount[_currentLevel]>=levelPrice[_nextLevel] && _nextLevel<=LAST_LEVEL){
    ...
    function updateX6ReferrerSecondLevel(address userAddress, address referrerAddress, uint8 level) private {
        ...
        if (referrerAddress != owner()) {
            ...
            users[freeReferrerAddress].directIncome += levelPrice[level]*30/100;
            depositToken.safeTransfer( freeReferrerAddress, levelPrice[level]*30/100);
            emit UserIncome(referrerAddress ,freeReferrerAddress , 0 ,levelPrice[level]*30/100,"PersonalIntroducer");
