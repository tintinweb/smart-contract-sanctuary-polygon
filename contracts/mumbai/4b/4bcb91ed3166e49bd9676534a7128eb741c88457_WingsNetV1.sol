...
contract WingsNetV1 is Ownable {
    ...
    mapping(uint => address[]) public nonWorking;
    mapping(uint => address[]) public royaltyAcheivers;

    uint16[] internal levelDistribution;
    uint16[] internal nonWorkingDistribution;
    uint16[] internal royaltyDistribution;
    mapping(address => mapping (uint => address[])) internal referTeam;
    ...
    event DistributedLevel(address indexed _to, address indexed _from, uint256 _amount);
    event DistributedNonWorking(address indexed _to, address indexed _form, uint256 _amount);
    event DistributedRoyalty(address indexed _to, address indexed _form, uint256 _amount);
    ...
    //     for (uint8 j = 0; j < leaders_.length; j++) {
    //         _register(_leaders[j], _leaders[0]);
    //     }
    //     for (uint8 i = 0; i < plans.length; i++){
    //         _activate(i, owner());
    //         for (uint8 j = 0; j < leaders_.length; j++) {
    //             _activate(i, _leaders[j]);
    //         }
    //     }
    // }

   constructor(address payableToken_, address[] memory receivers_) {
        require(!initialized, "Contract is already initialized");
        initialized = true;
        PayableToken = payableToken_;
        _setReceivers(receivers_);
    ...
    function _distributeLevel(uint8 plan_, address user_) internal {
        ...
        for (uint64 i = 0; i < 20; i++) {
            ...
            if (_referCount(plan_, _referral) >= i) {
                if (_package.invest > 0 && _package.limit > 0 && _package.nonLimit > 0) {
                    ...
                    if (i < 5) {
                        ...
                        if (_amount > 0) {
                            ...
                            emit DistributedLevel(_referral, user_, _amount);
                        }
                    }
    ...
    function _distributeNonWorking(uint8 plan_, address user_) internal {
        ...
        if (_users.length > 0) {
            uint256 _amount = plans[plan_].mul(nonWorkingDistribution[plan_]).div(1e4).div(_users.length);
            
            for (uint64 i = 0; i < _users.length; i++) {
                if (_users[i] != user_) {
                    UserPackage memory package_ = userPackages[_users[i]][plan_];
                    if (package_.nonLimit < _amount) {
                        _amount = package_.nonLimit;
                        removeNonWorking(plan_, _users[i]);
                    }
                    package_.nonLimit = package_.nonLimit.sub(_amount);
                    package_.limit = package_.limit.sub(_amount);
                    package_.nonWorkingIncome = package_.nonWorkingIncome.add(_amount);
                    userPackages[_users[i]][plan_] = package_;
                    users[_users[i]].totalIncome = users[_users[i]].totalIncome.add(_amount);
                    if (_amount > 0) {
                        IERC20(PayableToken).transfer(_users[i], _amount);
                        emit DistributedNonWorking(_users[i], user_, _amount);
                    }
                }
            }
        }
    ...
    function _distributeRoyalty(uint8 plan_, address user_) internal {
        for (uint8 r = 2; r < royaltyDistribution.length; r++ ) {
            address[] memory _users = royaltyAcheivers[r];
            if (_users.length > 0) {
                uint256 _amount = plans[plan_].mul(royaltyDistribution[r]).div(1e4).div(_users.length);

                for (uint64 i = 0; i < _users.length; i++) {
                    if (_users[i] != user_) {
                        UserPackage memory _package = userPackages[_users[i]][r];
                        if (_package.limit < _amount) {
                            _amount = _package.limit;
                            removeAcheiver(r, _users[i]);
                        }
                        ...
                        if (_amount > 0) {
                            IERC20(PayableToken).transfer(_users[i], _amount);
                            emit DistributedRoyalty(_users[i], user_, _amount);
                        }
                    }
                }
            }
    ...
    function _distribute() internal {
        uint256 bal = IERC20(PayableToken).balanceOf(address(this)).div(receivers.length);
        if (bal > 0) {
    ...
    function referralExists(uint8 plan_, address user_) public view returns (bool) {
        address[] storage users_ = royaltyAcheivers[plan_];
    ...
    function removeAcheiver(uint8 plan_, address userToRemove_) internal {
        if (acheiverExists(plan_, userToRemove_)) {
            address[] storage users_ = royaltyAcheivers[plan_];
            for (uint256 i = 0; i < users_.length; i++) {
                if (users_[i] == userToRemove_) {
                    users_[i] = users_[users_.length - 1];
                    users_.pop();
                    break;
                }
            }
    ...
    function nonWorkingExists(uint8 plan_, address user_) public view returns (bool) {
        address[] storage users_ = nonWorking[plan_];
        for (uint256 i = 0; i < users_.length; i++) {
            if (users_[i] == user_) {
                return true;
            }
    ...
    function referList(uint plan_, address user_) external view returns (address[] memory) {
        return referTeam[user_][plan_];
    }

    function referCount(uint plan_, address user_) external view returns (uint256) {
        return _referCount(plan_, user_);
    }

    function _referCount(uint plan_, address user_) internal view returns (uint256) {
