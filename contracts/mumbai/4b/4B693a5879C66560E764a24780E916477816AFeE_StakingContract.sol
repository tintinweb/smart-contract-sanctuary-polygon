...
contract StakingContract {
    ...
    uint256 public apr; // APR in percentage
    uint256 public fee; // Fee in BNB
    ...
    constructor(address _tokenAddress, uint256 _apr, uint256 _fee) {
    ...
    function compoundRewards() external {
        ...
        stakedBalances[msg.sender] = stakedBalances[msg.sender].add(rewards);
    ...
    function claimRewards() external {
        ...
        IERC20 token = IERC20(tokenAddress);
        require(token.transferFrom(owner, msg.sender, rewards), "Failed to transfer rewards");
    ...
    function updateRewards(address account) internal {
        ...
        if (stakedAmount > 0) {
            uint256 rewards = calculateRewards(stakedAmount);
            rewardBalances[account] = rewardBalances[account].add(rewards);
        }
    }

    function calculateRewards(uint256 amount) internal view returns (uint256) {
        uint256 dailyRate = apr.div(365); // Daily rate based on APR
        return amount.mul(dailyRate).div(10000);
    }
    ...
    function deposit(uint256 amount) external {
        ...
        rewardPerWeek = rewardPerWeek.add(amount);

        emit Deposit(msg.sender, amount);
    }

    function withdrawAll() external {
        require(msg.sender == owner, "Only the owner can withdraw all");
    ...
    function setAPR(uint256 _apr) external {
        require(msg.sender == owner, "Only the owner can set APR");
        apr = _apr;
    }

    function setFee(uint256 _fee) external {
        require(msg.sender == owner, "Only the owner can set fee");
        fee = _fee;
    }
}