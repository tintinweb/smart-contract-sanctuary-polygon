...
contract MonwuFoundersVesting is Ownable {
  ...
  function foundersRelease(uint256 amount) external onlyOwners {
  ...
  function computeReleasableAmount() internal view returns(uint256) {
    ...
    totalReleasedTokens = (((block.timestamp - start) / releaseDuration) + 1) * (foundersAllocation / numberOfUnlocks);
