...
contract BTCLDynamicLottery is AutomationCompatible, AutomatedRandomness, ReentrancyGuard {
    ...
    uint256 public maxRoundsToDraw = 3;
    ...
    AutomatedRandomness(
        ...
        round = 1;
        // Initialize the next round number to 1
        nextRound = 1;
    ...
    function checkUpkeep(bytes calldata) external view override returns (bool upkeepNeeded, bytes memory performData) {
        uint256[] memory roundsToDraw = new uint256[](maxRoundsToDraw);
        uint256 numberOfRoundsToDraw = 0;

        for (uint256 i = nextRound; i < nextRound + maxRoundsToDraw; i++) {
            if (rounds[i].status.roundStatus == BTCLCore.Status.Open && rounds[i].status.totalBets >= maxPlayers) {
                roundsToDraw[numberOfRoundsToDraw] = i;
                numberOfRoundsToDraw++;

                if (numberOfRoundsToDraw == maxRoundsToDraw) {
                    break;
                }
            }
        }
        ...
        performData = abi.encode(roundsToDraw);
    }

    function performUpkeep(bytes calldata performData) external nonReentrant override {
        uint256[] memory roundsToDraw = abi.decode(performData, (uint256[]));

        // Request VRF numbers for all rounds that need to be drawn
        uint256 numWords = totalWinners * roundsToDraw.length; // Request 3 numWords per round winner for all rounds
        uint256 requestId = requestRandomness(uint32(numWords));

        // Loop through all rounds that need to be drawn and set the requestId
        for (uint256 i = 0; i < roundsToDraw.length; i++) {
            uint256 roundNr = roundsToDraw[i];
   ...
   function fulfillRandomWords(uint256 requestId, uint256[] memory randomness) internal override {
        ...
        uint256 numWordsPerRound = totalWinners;
        uint256 numRoundsPerBatch = maxRoundsToDraw;
        uint256 currentRound = nextRound;

        // Process rounds in batches of up to 3
        for (uint256 i = 0; i < numRoundsPerBatch && currentRound <= round && rounds[currentRound].status.requestId == requestId && rounds[currentRound].status.roundStatus == BTCLCore.Status.Drawing; i++) {
            uint256 roundNumber = currentRound;

            // Check if we received the correct number of random numbers
            require(randomness.length >= numWordsPerRound, "Invalid number of random words");

            // Set the round status to completed
            rounds[roundNumber].status.roundStatus = BTCLCore.Status.Completed;

            // Copy the randomness to the storage for the current round
            for (uint256 j = 0; j < numWordsPerRound; j++) {
                rounds[roundNumber].status.randomness[j] = randomness[numWordsPerRound * i + j];
            }

            // Emit the event
            emit BTCLCore.LotteryClosed(roundNumber, rounds[roundNumber].status.totalTickets, rounds[roundNumber].status.totalBets);

            numProcessed++;
            currentRound++;
        }

        // Check if all rounds in the current batch have been processed
        if (numProcessed == numRoundsPerBatch || (currentRound > round && rounds[nextRound].status.requestId != requestId && rounds[nextRound].status.roundStatus == BTCLCore.Status.Open)) {
            nextRound += numProcessed;
            round += numProcessed / numRoundsPerBatch;
            rounds[round].status.roundStatus = BTCLCore.Status.Open;
...
library BTCLCore {
    ...
    // event LotteryClosed(uint256[] roundNumbers, uint256[] totalTickets, uint256[] totalBets, uint256[] randomNumbers);
