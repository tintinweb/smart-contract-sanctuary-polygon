...
contract InterestEarner {
        ...
        noReentrant
    {
        ...
        uint256 _WeakTime = 300;
        uint256 _MonthTime = 600;
        ...
        // }
         if (calcTime < _WeakTime) {
          interestToPayOut = 0;
          emit intrestAmount("inside if");
          emit intrestValue(interestToPayOut);
        } 
        
        else if (calcTime > _WeakTime && calcTime < 2*_WeakTime) {
          interestToPayOut = expectedInterest[msg.sender].div(2);
          emit intrestAmount("inside else if 2");
          emit intrestValue(interestToPayOut);
        } 
        
        else if (calcTime > 2*_WeakTime && calcTime < 3*_WeakTime) {
          interestToPayOut = expectedInterest[msg.sender].div(3);
          emit intrestAmount("inside else if 3");
          emit intrestValue(interestToPayOut);
        } 
        
        else if (calcTime > 3*_WeakTime && calcTime < 4*_MonthTime) {
          interestToPayOut = expectedInterest[msg.sender];
