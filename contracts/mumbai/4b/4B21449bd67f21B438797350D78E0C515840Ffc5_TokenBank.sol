...
contract TokenBank {
  ...
  constructor(string memory name_, string memory symbol_, address nftContract_) {
    ...
    waraToken = WaraToken(nftContract_);
  }

  /// @dev NFTを所持している方のみ
  modifier onlyHasNFT() {
    require(waraToken.balanceOf(msg.sender) > 0, "Has not token");
    _;
  }
  ...
  /// @dev Tokenを移転する
  function transfer(address to, uint256 amount) public onlyHasNFT {
    if (owner == msg.sender) {
  ...
  /// @dev Tokenを預ける
  function deposit(uint256 amount) public onlyHasNFT notOwner {
    address from = msg.sender;
    address to = owner;
  ...
  /// @dev Tokenを引き出す
  function withdraw(uint256 amount) public onlyHasNFT notOwner {
    address from = owner;
