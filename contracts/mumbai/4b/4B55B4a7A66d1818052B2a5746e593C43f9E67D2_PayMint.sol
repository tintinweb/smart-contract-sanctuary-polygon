...
contract PayMint is ERC1155URIStorage, Ownable {

    string private baseURI;
    string public name;
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;
    uint256 private platformFee = 1;
    ...
    constructor() ERC1155("ipfs://ThePublicator/{id}.json") {
        setName("Crucial Test 1");
    }
    ...
    function mint(
        ...
        uint256 marketFee = nftPrice * (platformFee / 100);
        uint256 artistPrice = nftPrice - marketFee;

        require(msg.value == nftPrice, "Custom Error: Insufficient Balance");
        payable(artist).transfer(artistPrice);
        payable(marketOwner).transfer(marketFee);

        _mint(msg.sender, tokenId, noOfMints, "");
        setURI(tokenId, _newuri);
