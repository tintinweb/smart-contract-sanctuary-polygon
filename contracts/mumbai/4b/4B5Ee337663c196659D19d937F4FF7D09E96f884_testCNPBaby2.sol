...
import "./lib/CNPBabydescriptor.sol";
import "./interface/ICNPBabydescriptor.sol";
import "./CNPBabyadmin.sol";
import "./CNPBabycore.sol";

// ★あとで変更する
contract testCNPBaby2 is
    AccessControl,
    Ownable,
...
{
    // ★名前あとで変更する
    constructor() ERC721A("testCNPBaby2", "TCNPB") {
        //Role initialization
        grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        grantRole(MINTER_ROLE, msg.sender);
        grantRole(ADMIN, msg.sender);
        grantRole(ADMIN, 0x85EFB8eE85195896c14929126a4d48d0F3D9bF39); // ★

        //★URI initialization
        // setBaseURI("https://nft.naokomamablog.com/CNPBaby/json/");
    ...
    address public constant withdrawAddress =
        0x35F4A47228866399ff3A406dd11fAd80e3F74744; // ★
    ...
    function mint(
        ...
        // ミントサイトから入力されたミント数量が1トランザクション処理量を超えていないか
        require(
        ...
        require(
            _mintAmount <= maxMintAmountPerAddress - claimed[msg.sender],
    ...
    //CAL
    function setCalContract(
        IContractAllowListProxy _cal
    ) external onlyRole(ADMIN) {
        cal = _cal;
    }
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view override(AccessControl, ERC721A, ERC2981) returns (bool) {
        return
...
// import "./interface/ICNPBaby.sol";

abstract contract CNPBabyadmin is AccessControl, CNPBabycore {
...
abstract contract CNPBabycore is Ownable {
    // == Constants ==
    bytes32 public ADMIN = keccak256("ADMIN");
    bytes32 public AIRDROP_ROLE = keccak256("AIRDROP_ROLE");
    ...
    uint96 public royaltyFee = 0; // 0%, changable.

    // == For Mint ==
    uint256 public cost = 0; // freemint
    ...
    bool public burnAndMintMode = false; //バー忍モードフラグ
    bytes32 public merkleRoot; // マークルツリールート
    mapping(address => uint256) public claimed; // 前回ミント済み数
}
    ...
    CNPBabycore
{
    ...
    function _toString(
        uint256 value
    ) internal pure virtual returns (string memory ptr) {
        assembly {
    ...
    function tokenURI(
        ...
        if (address(tokenuri) == address(0)) {
            return string(abi.encodePacked(_tokenURI(tokenId), baseExtension));
        } else {
