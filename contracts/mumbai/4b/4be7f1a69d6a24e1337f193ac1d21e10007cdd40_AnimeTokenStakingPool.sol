...
contract AnimeTokenStakingPool is Ownable {
  ...
  function estimateTime(uint256 targetReward, uint256 balance)
    public
    view
    returns (uint256)
  {
    ...
    uint256 targetRewardPerToken = (targetReward / balance) * 1e18;
    return
      ((targetRewardPerToken - rewardPerTokenStored) * _totalSupply) /
      1e18 /
      rewardRate +
      lastUpdateTime;
  }

  function estimateDuration(uint256 targetReward, uint256 balance)
    external
    view
    returns (uint256)
  {
    uint256 estimatedTime = estimateTime(targetReward, balance);
    if (estimatedTime == 0) {
      return 0;
    }
