...
contract ODFDAOUpgradeableV2 is Initializable, OwnableUpgradeable, IODFDao {
    ...
    /**
     * @dev Status of the contract.
     */
    bool internal _status;
    ...
    /**
     * @dev modifier to avoid Reentrancy attacks.
     */
    modifier noReentrancy() {
        if (_status) revert ReentrantCall();
        _status = true;
        _;
        _status = false;
    }
    ...
    function vote(
        uint256 id_,
        bool approval_
    ) external virtual notFrozen noReentrancy {
        bool status = votingToken.transferFrom(
            msg.sender,
            address(this),
            1 ether
        );
        require(status, 'ODFDAO: transfer failed');
     ...
     * @param id_ ID of the proposal.
     */
    function execute(uint256 id_) external virtual {
        Proposal storage proposal = proposals[id_];
    ...
    function withdrawBalance() external virtual onlyOwner {
        ...
        if (balance > 0) require(votingToken.transfer(owner(), balance));
        balance = rewardToken.balanceOf(address(this));
        if (balance > 0) require(rewardToken.transfer(owner(), balance));
    }
...
interface IODFDao {
    ...
    /**
     * @notice Error to revert when reentrant call is detected.
     */
    error ReentrantCall();
