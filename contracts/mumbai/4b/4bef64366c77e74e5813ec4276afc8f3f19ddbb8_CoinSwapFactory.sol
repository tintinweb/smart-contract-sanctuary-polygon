...
// File: coinswap/interfaces/ICoinSwapCallee.sol
...
// File: coinswap/interfaces/IERC20.sol
...
// File: coinswap/libraries/UQ112x112.sol
...
// File: coinswap/libraries/Math.sol
...
// File: coinswap/libraries/SafeMath.sol
...
// File: coinswap/interfaces/ICoinSwapERC20.sol
...
// File: coinswap/CoinSwapERC20.sol
...
// File: coinswap/interfaces/ICoinSwapPair.sol
...
// File: coinswap/interfaces/ICoinSwapFactory.sol
...
// File: coinswap/CoinSwapPair.sol
...
contract CoinSwapPair is ICoinSwapPair, CoinSwapERC20 {
    using SafeMath  for uint;
...
// File: coinswap/CoinSwapFactory.sol

pragma solidity =0.5.16;



contract CoinSwapFactory is ICoinSwapFactory {
    bytes32 public constant INIT_CODE_PAIR_HASH = keccak256(abi.encodePacked(type(CoinSwapPair).creationCode));
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        require(tokenA != tokenB, 'CoinSwap: IDENTICAL_ADDRESSES');
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'CoinSwap: ZERO_ADDRESS');
        require(getPair[token0][token1] == address(0), 'CoinSwap: PAIR_EXISTS'); // single check is sufficient
        bytes memory bytecode = type(CoinSwapPair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        ICoinSwapPair(pair).initialize(token0, token1);
    ...
    function setFeeTo(address _feeTo) external {
        require(msg.sender == feeToSetter, 'CoinSwap: FORBIDDEN');
        feeTo = _feeTo;
    }
