...
contract MoDVault is Pausable, ReentrancyGuard, IERC721Receiver {
    ...
    constructor(address _mod){
        // Update the MoD contract address
        mod = IMoD(_mod);
        admins[_mod] = true;
    ...
    */
    ...
    function pushTheBigGreenButton() public onlyHumans nonReentrant whenNotPaused {
    ...
    function bigGreenButtonFailSafe(uint256 tokenID) public onlyHumans nonReentrant whenNotPaused {
    ...
    function addToVault(uint256 tokenID) public onlyAdmins {
...
interface IMoD {
    ...
    // Set the baseURI for a token
    function setBaseURI(string memory _baseURI, uint256 _tokenID) external;

    // Used to initialize the contract interactions
    function updateMoDContract(address _mintOfDestiny) external;
}
