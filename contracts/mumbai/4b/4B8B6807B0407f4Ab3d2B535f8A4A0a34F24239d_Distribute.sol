...
contract Distribute is Ownable {
    ...
    function batchClaimGGGenesis(string[] memory _tokenIds) public {
       for(uint8 i = 0; i < _tokenIds.length; i++){
            uint256 id = uint256(strToUint(_tokenIds[i]));
            require(msg.sender == ERC721(genContract).ownerOf(id), 'You do not own this token');
            uint256 amount = getGGAccrualGenesis(id);
            genesisTimestamp[id] = block.timestamp; 
            if (amount > 70000000000000000000000) {
                GG(GGContract).mint(70000000000000000000000, msg.sender);
                GG(GGContract).mint(((amount-70000000000000000000000)/2), CommunityWallet);
            } else {
    ...
    function batchClaimGGFounder(string[] memory _tokenIds) public {

     for(uint8 i = 0; i < _tokenIds.length; i++){
        uint256 id = uint256(strToUint(_tokenIds[i]));
        require(msg.sender == ERC721(founderContract).ownerOf(id), 'You do not own this token');
        uint256 amount = getGGAccrualFounder(id);
        foundersTimestamp[id] = block.timestamp;
        if (amount > 70000000000000000000000) {
            GG(GGContract).mint(70000000000000000000000, msg.sender);
            GG(GGContract).mint(((amount-70000000000000000000000)/2), CommunityWallet);
        } else {
