...
contract EthermonStakingData is EthermonStakingBasic {
        ...
        returns (uint256)
    {
        ...
        // totalStaked = totalStaked + data.emons;

        // EmonPerPeriod = uint64(
        //     (totalStaked * EmonPeriodMultiplier) / (365 * 24)
        // );
    ...
    function setEmonPeriodMultiplier(uint16 _EmonPeriodMultiplier)
        external
        onlyModerators
    {
        require(_EmonPeriodMultiplier > 0, "Value 0 is not valid");
        EmonPeriodMultiplier = _EmonPeriodMultiplier;
    }
    ...
    function decreaseSumTeamPower(uint256 _teamPower, uint256 _emons)
        public
        onlyModerators
    {
        require(_teamPower < NewSumTeamPower, "Wrong team power");
        require(_emons <= totalStaked, "Emons value is too high");
    ...
    function increaseSumTeamPower(uint256 _teamPower, uint256 _emons)
        public
        onlyModerators
    {
        NewSumTeamPower += _teamPower;
    ...
    function removeTokenData(TokenData memory _data) public onlyModerators {
        ...
        //require(_data.emons <= totalStaked, "Emons value is too high");
        // totalStaked = totalStaked - _data.emons;
        // EmonPerPeriod = uint64(
        //     (totalStaked * EmonPeriodMultiplier) / (365 * 24)
        // );
