...
import {RainVMIntegrity, StateConfig} from "@beehiveinnovation/rain-protocol/contracts/vm/integrity/RainVMIntegrity.sol";
import {AllStandardOps} from "@beehiveinnovation/rain-protocol/contracts/vm/ops/AllStandardOps.sol";

contract ScriptableTokenURINFT is ERC721Upgradeable, StandardVM {
    ...
    constructor(address vmIntegrity)
        StandardVM(vmIntegrity)
    {}

    function initialize(StateConfig memory config, string memory _baseURI) initializer external{
    ...
    function mintNFT() public {
        require(totalSupply < 8, "MAX_LIMIT");
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        uint256 stake = _loadVMState(toContext(ownerOf(tokenId))).eval().peek();
        return string(abi.encodePacked(baseURI, "/", tokenId.toString(), "/", stake.toString(), ".json"));
    }

    function toContext(address _owner)
        internal
        pure
        returns (uint256[] memory context_)
    {
        context_ = new uint256[](1);
        context_[0] = uint256(uint160(_owner));
    }
}
