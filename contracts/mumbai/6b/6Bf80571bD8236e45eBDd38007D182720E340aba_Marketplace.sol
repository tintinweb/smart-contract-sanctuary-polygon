...
abstract contract ERC2771ContextUpgradeableLogic {
    ...
    function __ERC2771Context_init_unchained(
        ...
        ERC2771ContextStorage.Data storage data = ERC2771ContextStorage
    ...
    /// @dev Returns whether trusted forwarder info can be set in the given execution context.
    function _canSetTrustedForwarder() internal view virtual returns (bool);

    function setTrustedForwarder(address forwarder, bool isTrusted) external {
        require(_canSetTrustedForwarder(), "ERC2771ContextUpgradeable: Not authorized");
        ERC2771ContextStorage.Data storage data = ERC2771ContextStorage
            .erc2771ContextStorage();
        data._trustedForwarder[forwarder] = isTrusted;
    }
...
abstract contract PlatformFeeLogic is IPlatformFee {
    ...
    {
        PlatformFeeStorage.Data storage data = PlatformFeeStorage
            .platformFeeStorage();
    ...
    function setPlatformFeeInfo(
        ...
        require(_canSetPlatformFeeInfo(), "PlatformFee: Not authorized");
...
abstract contract Router is Multicall, ERC165, IRouter {
    ...
    fallback() external payable virtual {
        ...
        if (_pluginAddress == address(0)) {
            _pluginAddress = IPluginMap(pluginMap).getPluginForFunction(
                msg.sig
            );
        }
    ...
    function getAllFunctionsOfPlugin(
        ...
        EnumerableSet.Bytes32Set storage selectorsForPlugin = data
            .selectorsForPlugin[_pluginAddress];
        bytes4[] memory defaultSelectors = IPluginMap(pluginMap)
            .getAllFunctionsOfPlugin(_pluginAddress);
        ...
        for (uint256 i = 0; i < len; i += 1) {
            registered[index++] = bytes4(
                data.selectorsForPlugin[_pluginAddress].at(i)
            );
        }
    }
    ...
    function getAllPlugins()
        external
        view
        returns (Plugin[] memory registered)
    {
        RouterStorage.Data storage data = RouterStorage.routerStorage();
        ...
        for (uint256 i = 0; i < overrideSelectorsLen; i += 1) {
            for (uint256 j = 0; j < defaultPluginsLen; j += 1) {
                if (
                    bytes4(overrideSelectors.at(i)) ==
                    defaultPlugins[j].functionSelector
                ) {
                    totalCount -= 1;
        ...
        for (uint256 i = 0; i < overrideSelectorsLen; i += 1) {
            registered[index] = data.pluginForSelector[
                bytes4(overrideSelectors.at(i))
            ];
            index += 1;
        }
    ...
    /// @dev View address of the plugged-in functionality contract for a given function signature.
    function _getPluginForFunction(
        bytes4 _selector
    ) public view returns (address) {
        RouterStorage.Data storage data = RouterStorage.routerStorage();
        address _pluginAddress = data
    ...
    function _addPlugin(Plugin memory _plugin) internal {
        ...
        try
            IPluginMap(pluginMap).getPluginForFunction(_plugin.functionSelector)
        returns (address) {
        ...
        data.selectorsForPlugin[_plugin.pluginAddress].add(
            bytes32(_plugin.functionSelector)
        );
    ...
    function _updatePlugin(Plugin memory _plugin) internal {
        ...
        data.allSelectors.add(bytes32(_plugin.functionSelector));
        data.pluginForSelector[_plugin.functionSelector] = _plugin;
        data.selectorsForPlugin[currentPlugin].remove(
            bytes32(_plugin.functionSelector)
        );
        data.selectorsForPlugin[_plugin.pluginAddress].add(
            bytes32(_plugin.functionSelector)
    ...
    function _removePlugin(bytes4 _selector) internal {
        ...
        require(
            currentPlugin != address(0),
            "Router: No plugin available for selector"
        );
    ...
    IERC1155Receiver
{
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view virtual override(Router, IERC165) returns (bool) {
        return
    ...
    /// @dev Checks whether trusted forwarder info can be set in the given execution context.
    function _canSetTrustedForwarder() internal view override returns (bool) {
        return hasRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }
