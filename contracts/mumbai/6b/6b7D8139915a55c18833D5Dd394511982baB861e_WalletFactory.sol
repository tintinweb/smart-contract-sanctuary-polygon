...
contract PlguinRegistry is IERC165, Ownable {
        ...
        returns (bool, bytes memory)
    {
        ...
        require(to != address(0), "PlguinRegistry: implementer not exist!");
...
contract Wallet is Ownable, PlguinRegistry, InitialInterfaces {
    using strings for *;
    IDKIM public dkim;
    uint public nonce;
    ...
    function initialize(address _dkim, address owner, uint256 _emailHash) public {
        dkim = IDKIM(_dkim);
    ...
    function verify(string calldata raw) external returns (bool) {
        (bool success, string memory from, , , string memory body) = dkim.verify(raw);
        bytes32 h = keccak256(abi.encodePacked(from));
        require(uint256(h) == emailHash, "Wallet Recovery: wrong email address!");
        require(success, "DKIM verify failed.");
        bytes memory encodedData = abi.encode(body);
        (uint256 _nonce, address decodedAddress) = abi.decode(
            encodedData,
            (uint256, address)
        );
        require(nonce + 1 == _nonce, "Invalid nonce");
        transferOwnership(decodedAddress);
        return true;
    }
    ...
    {
        return abi.encode(nonce + 1, newOwner);
    }
