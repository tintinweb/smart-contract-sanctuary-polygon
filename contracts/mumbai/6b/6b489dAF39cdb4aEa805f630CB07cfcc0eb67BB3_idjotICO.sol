...
contract idjotICO is ERC20 {
    ...
    function invest() public payable returns (bool) {
        ...
        require(
            icoState == IcoState.running,
            "idjotICO is not in running state"
        );

        // address must not have invested previously
        require(
            idjotBalance[msg.sender] == 0,
            "User must invest only once according to our policy"
        );

        require(
            msg.value >= minInvestment && msg.value <= maxInvestment,
            "Investment amount must be more than 0.05 ETH and less than 5 ETH"
        );
    ...
    function buyTokens(uint256 msgValue) internal returns (uint256) {
    ...
    function preSale(uint _msgValue) internal returns (uint256 tokens) {
        uint256 _tokens = 0;
    ...
    function seedSale(uint256 _msgValue) internal returns (uint256 tokens) {
        uint256 _tokens = 0;
        // calculated considering eth value as 2500$
    ...
    function finalSale(uint256 _msgValue) internal returns (uint256 tokens) {
        uint256 _tokens = 0;
    ...
    function successCheck() public view returns (bool) {
        require(
            ...
            "idjotICO hasn't expired yet, Try after saleEnd!"
        );
