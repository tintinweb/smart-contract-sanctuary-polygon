...
contract Market is AccessControlledAndUpgradeable {
  ...
  event NthPriceDeposit(bool isLong, uint256 depositAdded, address user, uint32 oracleUpdateIndex);

  event NthPriceRedeem(bool isLong, uint256 synthRedeemed, address user, uint32 oracleUpdateIndex);

  event NthPriceSyntheticPositionShift(
    bool isShiftFromLong,
    uint256 synthShifted,
    address user,
    uint32 oracleUpdateIndex
  );

  event ExecuteNextPriceSettlementsUserSeparateMarket(address user);
  ...
  function initialize(
    ...
    require(initialMarketSeedForEachMarketSide != 0);
    require(
  ...
  */
  ...
  function _executeOutstandingNextPriceSettlements(address user) internal virtual {
    ...
    while (userCurrentUpdateIndex != 0 && userCurrentUpdateIndex <= marketUpdateIndex) {
      ...
      emit ExecuteNextPriceSettlementsUserSeparateMarket(user);
      ++index;
      userCurrentUpdateIndex = userNextPrice_updateIndexes[user][index];
    }
  }
