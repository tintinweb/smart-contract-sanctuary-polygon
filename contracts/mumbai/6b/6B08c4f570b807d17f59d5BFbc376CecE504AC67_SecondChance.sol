...
contract SecondChance is ISecondChance, VRFConsumerBaseV2, Ownable {
    ...
    mapping(uint256 => Winner[]) s_poolWinners;
    ...
    function registerUser(
        ...
        emit SecondChance_UserRegisteredIntoPool(user, poolId);
    }
    ...
    function initiateRaffle(
        ...
        if (pools[poolId].registeredUsers.length != tokenIds.length) {
            revert SecondChance_RaffleTokenCountMismatch(poolId);
        }
        ...
        unchecked {
            ...
            while (winners[winners.length - 1] == address(0)) {
                ...
                if (!winners.contains(pool.registeredUsers[winnerIndex])) {
                    ...
                    s_poolWinners[pool.id].push(
                        Winner(
                            pool.registeredUsers[winnerIndex],
                            getPrize(iterator, pool.id)
                        )
                    );
                    iterator++;
                }
    ...
    function getPrize(
        ...
        if (index <= poolId + 3) {
            return pool.largePrize;
        }

        return pool.smallPrize;
    }
}
