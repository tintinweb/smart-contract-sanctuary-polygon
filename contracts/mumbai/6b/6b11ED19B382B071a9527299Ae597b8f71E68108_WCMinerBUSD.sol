...
contract WCMinerBUSD {
    ...
    uint256 constant launch = 1665072000;  // 6th Oct UTC: 1665072000
  	uint256 constant hardDays = 600;//86400;
    uint256 constant minStakeAmt = 0.001 ether;//20 ether
    uint256 constant percentdiv = 1000;
    uint256 devPercentage = 120;
    uint256 withdrawFee = 100;
    uint256[] ref_earnings = [3, 2, 1, 1, 1];
    ...
    constructor() {
            owner = address(0x350F84C2f5272973646342Be1AdbE232324A552E);
            dev = address(0x350F84C2f5272973646342Be1AdbE232324A552E);
            ...
            FeesKey[10] = FeesPercs(864000, 250);
            FeesKey[20] = FeesPercs(1728000, 240);
            FeesKey[30] = FeesPercs(3456000, 220);
            FeesKey[40] = FeesPercs(4320000, 200);
    }

    function fundContract(uint256 _amount) external payable {
        require(msg.value==_amount,"invalid amount");
    }

    function stakeStablecoins(address ref) external payable  {
        ...
        require(amtx >= minStakeAmt, "You should stake at least 20.");
       ...
       _setUpdaddy(msg.sender,ref);
        payReferral(msg.sender,adjustedAmt);
        ...
        payable(owner).transfer(amtx * 8/100);
        payable(dev).transfer(amtx * 4/100);
    }

     function _setUpdaddy(address _addr, address _upline) private {
        if (UsersKey[_addr].reffy == address(0) && UsersKey[_upline].keyCounter > 0) {
            UsersKey[_addr].reffy = _upline;
        }
    }
    ...
    function payReferral(
        ...
        for (uint8 i = 0; i < ref_earnings.length; i++) {
            referer = UsersKey[_user].reffy;
            if (referer == address(0)) {
                break;
            }
            UsersKey[referer].refBonus += _amount.mul(ref_earnings[i]).div(100);
            _user = referer;
        }
    }
    ...
    function withdrawDivs() external returns (uint256 withdrawAmount) {
        ...
        uint256 adjustedPercent = 1000 - withdrawFee;
        uint256 adjustedAmt = x.mul(adjustedPercent).div(percentdiv); 
    
        main.ovrTotalWiths += x;
        user.lastWith = block.timestamp;

        payable(msg.sender).transfer(adjustedAmt);
