...
contract Dice is Ownable {
    ...
    event Result(address indexed addr, uint typeRes, uint amount, uint currency, uint result, uint prediction, uint roll);
    ...
    function betSRX(uint _amount, uint _prediction, uint _roll, address _referer) public {
        ...
        if(_roll == 0) {
            ...
            if(_result < _prediction) {
                ...
                emit Result(msg.sender, 1, 1, _profit, _result, _prediction, _roll);
            } else {
                //Lose
                SRX.transferFrom(msg.sender, address(this), _amount);
                emit Result(msg.sender, 0, 1, _amount, _result, _prediction, _roll);
            }
        } 
    ...
    function betMATIC(uint _prediction, uint _roll, address _referer) payable public {
        ...
        if(_roll == 0) {
            ...
            if(_result < _prediction) {
                ...
                emit Result(msg.sender, 1, 2, _profit, _result, _prediction, _roll);
            } else {
                //Lose
                emit Result(msg.sender, 0, 2, _amount, _result, _prediction, _roll);
            }
        } 
