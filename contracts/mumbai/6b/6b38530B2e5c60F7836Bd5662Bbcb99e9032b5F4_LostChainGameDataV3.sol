...
import './interface/ILostCrystal.sol';

contract LostChainGameDataV3 is CustomERC2771ContextUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize(
        ...
        require(vaultAddress != address(0x0), 'LCGD: Vault is zero address');
        require(lostCrystalAddress != address(0x0), 'LCGD: Lost crystal is zero address');
        LCRBALinker = ILostChainRBA(rbaLinkerAddress);
        vault = vaultAddress;
        ...
        LC = ILostCrystal(lostCrystalAddress);
    ...
    function setLC(address lostCrystalAddress) public onlyRole('DEFAULT_ADMIN_ROLE', msg.sender) {
        require(lostCrystalAddress != address(0x0), 'LCGD: Lost crystal is zero address');
        LC = ILostCrystal(lostCrystalAddress);
    }
    ...
    function adventure(
        ...
        LCUD.consumeEnergy(msg.sender, _msgSender());
    ...
    function adventureComplete(
        ...
        require(curAdventure.isAdventuring, 'LCGD: User is not adventuring');
        require(curAdventure.landId == landId, 'LCGD: Land id is not the same');

        require(curAdventure.rewardList.length == rewardAmounts.length, 'LCGD: Rewards mismatch');
        require(curAdventure.heroIds.length == xpAmount.length, 'LCGD: XP mismatch');
        ...
        uint256[] memory vaultBalances = LCNFT.userNFTBalances(vault, curAdventure.rewardList);
        for (uint256 i = 0; i < vaultBalances.length; i++) {
            require(vaultBalances[i] >= rewardAmounts[i], 'LCGD: Vault has insufficient NFT');
        }
        uint256 crystalVaultBalance = LC.getUserBalance(vault);
        require(crystalVaultBalance >= crystalAmount, 'LCGD: Vault has insufficient crystal');
        ...
        // Give reward
        if (result) LCNFT.gameDataSendNFT(msg.sender, vault, _msgSender(), curAdventure.rewardList, rewardAmounts);

        // Give crystal
        if (result) {
            uint256 correctedPrice = crystalAmount * 10 ** LC.getDecimal();
            LC.gameDataTokenTransfer(msg.sender, vault, _msgSender(), correctedPrice);
        }
...
interface ILostCrystal {
    function gameDataTokenTransfer(address operator, address from, address to, uint256 amount) external;
