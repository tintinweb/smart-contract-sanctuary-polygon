...
contract TNGBLLiquidityManager is AddressAccessor, Pausable {
        ...
        )
    {
        ...
        if (token0 == tngbl) {
            uint256 required = (amount1 * (36 - underlyingDecimals)) /
            ...
            uint256 required = (amount0 * (36 - underlyingDecimals)) /
                IPriceOracle(tngblOracle).quote(1e18);
            if (required > amount1) {
                IERC20(tngbl).transferFrom(
                    treasury,
    ...
    function missingLiquidity() external view returns (uint256) {
        ...
        uint256 liquidity;
        {
            ...
            uint8 underlyingDecimals = IERC20Metadata(underlying).decimals();
            uint256 tngblBalance = IERC20(tngbl).balanceOf(uniswapV3Pool);
            liquidity =
                underlyingBalance +
        ...
        )
    {
        ...
        minSize = IERC20(usdr).totalSupply() * _minSizePercent * 1e7;
        {
            underlyingBalance = IERC20(underlying).balanceOf(uniswapV3Pool);
            uint8 underlyingDecimals = IERC20Metadata(underlying).decimals();
            tngblBalance = IERC20(tngbl).balanceOf(uniswapV3Pool);
            liquidity =
                underlyingBalance +
                (tngblBalance *
                    (tngblPrice = IPriceOracle(oracle).quote(1e18))) /
                (10**(36 - underlyingDecimals));
        }
    }
