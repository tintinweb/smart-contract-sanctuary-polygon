...
contract LendingControl is AccessControl, ReentrancyGuard, ILending {
    ...
    function updateDaysLeft(address _wallet, uint256 _idLending, uint16 _daysLeft) public onlyLending {
     ...
     function addRegistry(uint256 id, address wallet, uint256 _amount,address _token, uint256 _payPerSecond) public onlyLending nonReentrant {
        ...
        lenders[wallet][id].lastCalcTime = uint96(block.timestamp);
        lenders[wallet][id].claimTime = uint96(block.timestamp);
...
// agregar un funcionalidad que guarde los rewards generados para cuando se cambie la tasa de interes.
    ...
    function updateDaysBeforePenalization(uint16 _newDaysLeft) public onlyDev{
        daysBeforePenalization = _newDaysLeft;
    }

    function updateExtendPenalization(uint16 _newExtend) public onlyDev{
