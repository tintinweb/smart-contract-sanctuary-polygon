...
/**
 *  ERC721CompoundingDrop is an implementation of the ERC721 Drop.
 *      - The price increase with each claim (pricePerToken * tokenID)
 *
 */
contract ERC721CompoundingDrop is ERC721Drop {
    constructor(
        string memory _name,
        string memory _symbol,
        address _royaltyRecipient,
    ...
    function _getPriceBatch(
        uint256 _startTokenID,
        uint256 _quantity,
        uint256 _pricePerToken
    ) public pure returns (uint256 totalPrice) { 
        uint256 endToken = _startTokenID + _quantity;
    
        for ( uint256 i = _startTokenID; i < endToken;  i++) {
            totalPrice += _getPrice(i, _pricePerToken);       
        }
    ...
    function _getPrice(
        ...
        totalPrice = _pricePerToken * _tokenID;
    }
     ...
     function _collectPriceOnClaim(
        ...
        uint256 totalPrice = _getPriceBatch(nextTokenIdToMint(), _quantityToClaim, _pricePerToken);
