...
contract Honey is Context, IERC20, Ownable,Pausable{
    ...
    uint256 private _HiveRoyaltyTxFee = 10;
    uint256 private _TreasuryTxFee = 5;
    ...
    function foundersrolyaltyTx() public view returns (uint256) {
        return _HiveRoyaltyTxFee;
    }
      function treasuryTxFee() public view returns (uint256) {
        return _TreasuryTxFee;
    }
    ...
    function _approve(address owner, address spender, uint256 amount) private {
       ...
       function SetHiveroyaltyFeePercent(uint256 Hives) external onlyOwner() {
        _HiveRoyaltyTxFee =Hives;
    }
    
    function setTreasuryFeePercent(uint256 treasury) external onlyOwner() {
        _TreasuryTxFee = treasury;
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 initialBalance = _balances[address(this)];
    ...
    function _transferStandard(address sender, address recipient, uint256 amount) private {
        require(_isTimeLimit[sender] <= block.timestamp);
        uint256 senderBalance = _balances[sender];
        unchecked {
            _balances[sender] = senderBalance.sub(amount);
        }
        ...
        _rAndDTransfer(sender, HiveFee);
        _marketingTransfer(sender, TreasuryFee);
        _balances[recipient] = _balances[recipient].add(tAmount);
        
        _isTimeLimit[sender] = block.timestamp ;
    ...
    function _getCalculatedFees(uint256 amount) internal view returns(  uint256, uint256, uint256) {
        ...
        fee.HiveFee = amount.mul(_HiveRoyaltyTxFee).div(100);
        fee.TreasuryFee = amount.mul(_TreasuryTxFee).div(100);

        uint256 deductedAmount = amount.sub(fee.HiveFee.add(fee.TreasuryFee));

        return (deductedAmount, fee.HiveFee, fee.TreasuryFee);
    }
    function _rAndDTransfer(address sender, uint256 HiveFee) internal {
        if(HiveFee != 0) {
            _balances[HiveAcc] = _balances[HiveAcc].add(HiveFee);
            emit Transfer(sender, HiveAcc, HiveFee);
        }
    }

    function _marketingTransfer(address sender, uint256 TreasuryFee) internal {
        if(TreasuryFee != 0) {
            _balances[Treasury] = _balances[Treasury].add(TreasuryFee);
    ...
    constructor(address _Treasury, address _FoundersRoyalty) {
