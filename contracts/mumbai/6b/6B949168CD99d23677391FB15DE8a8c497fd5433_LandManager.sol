...
import "./logic/interfaces/IGameResource.sol";
    ...
    ILandManager
{
    ...
    modifier onlyGame() {
        bool valid;
        if (msg.sender == address(game) || msg.sender == stash) {
            valid = true;
        } else {
    ...
    function executeMint(
        ...
        uint256 total = support.getTotalRemain();
        uint256 number = randomNumber % total;
        (uint256 id, uint256 type_) = support.getId(number);
        require(id > 0 && id <= land.maxAmount(), "ind");
        support.updateRemain(type_);
        land.mintToken(requests[requestId].ownerRequest, id);
        infos[id].rarityType = uint16(type_);
        infos[id].floor = 1;
        infos[id].level = 1;
        infos[id].isPaid = 1;
        infos[id].richness = uint16(support.richnesses(type_, 1));
        infos[id].balance = defaultBalance;
        inventory.addInit(id, false);
        emit LandMinted(
    ...
    function nextNumber(
        ...
        uint256 number = support.numberOfRolls(info.level);
    ...
    function payTax(
        ...
        if (info.level < 12 && info.currentNumber == number - 1) {
            uint256 tax = support.taxes(info.level);
            if (info.balance < tax) {
                if (option == 1) {
                    info.currentNumber -= 1;
                    info.isPaid = 1;
                } else if (option == 2) {
                    inventory.addResource(id, 52, 66);
                    info.level += 1;
                    info.currentNumber = 0;
                    info.isPaid = 1;
                    emit UpLevel(player, id, info.level);
                } else if (option == 3) {
                    info.level += 1;
                    info.currentNumber = 0;
                    info.isPaid = 1;
            ...
            uint256 tax12 = support.taxes(12);
            uint256 over = ((tax12 + info.overTax) * increTax) / 10000;
    ...
    function reset(uint256 id) internal {
        ...
        inventory.addInit(id, true);
    }

    function gameReset(uint256 landId) external override nonReentrant whenNotPaused onlyGame {
        LandInfo storage info = infos[landId];
        if (info.richness > 0) {
            reset(landId);
        } else {
            info.isGameOver = true;
    ...
    function updateBalance(
        ...
        if (incre) {
            ...
            require(infos[id].balance >= amount, "e");
            infos[id].balance -= amount;
        }
    }
    ...
    function updateEssenceToken(
        ...
        if (incre) {
            ...
            require(infos[id].essenceToken >= amount, "e");
            infos[id].essenceToken -= amount;
        }
    }
    ...
    function updateResourceDeposit(
        ...
        require(msg.sender == stash, "s");
        infos[landId].resourceDeposit += uint64(amount);
    }

    function updateRemoveToken(
        uint256 landId, 
        uint256 amount
    ) external override nonReentrant whenNotPaused {
        require(msg.sender == address(support),"s");
        infos[landId].removeToken = amount;
    }  

    function updateNumEmpty(uint256 landId, uint256 amount) external override {
        require(msg.sender == address(inventory), "iv");
        infos[landId].numEmty += uint32(amount);
    }   

    function upgradeFloor(uint256 id) external nonReentrant whenNotPaused {
        address account = msg.sender;
        uint256 currentFloor = infos[id].floor;
        uint256 upgradeCosts = support.upgradeCosts(currentFloor);
        require(
            (account == land.ownerOf(id) || game.ownerOfLand(account, id)) && 
            token.balanceOf(account) >= upgradeCosts * 1e18,
            "ind"
        );
        LandInfo storage info = infos[id];
        ...
        info.level = 1;
        info.currentNumber = 0;
        info.richness = uint16(support.richnesses(info.rarityType, info.floor));
        token.transferFrom(
            account,
            treasury,
            upgradeCosts * 1e18
    ...
    function setInventory(address inventory_) external onlyOwner {
    ...
    function setSupport(address support_) public onlyOwner {
    ...
    function getInventory() external override view returns (address) {
