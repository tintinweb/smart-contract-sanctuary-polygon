...
/******************************************************************************
/*
Decentralized Green Energy Coin is a DAO (Decentralized Autonomous Organization) platform built on top of Binance 
SmartChain for future funding.It accelerates the growth of start-up companies by offering tools and services that save 
...
contract DecentralizedGreenEnery is Context, IERC20, Ownable {  
    ...
    uint256 public sellPrice = 0.00000002 ether;
    uint256 public priceIncrement = 0.0000000000001 ether;
    address [] public tokenHolder;
    uint256 public numberOfTokenHolders = 0;
    mapping(address => bool) private exist;
    mapping (address => bool) private _allIncludedPair;
    ...
    bool public _liquifyStatus;
    bool inSwapAndLiquify;
    bool public swapAndLiquifyEnabled = false;
    uint256 private minTokensBeforeSwap = 100;
    uint public lastAmountBought;
    event UpdateTaxFee();
    ...
    constructor () public {
        ...
        marketingwallet = 0x8EfE5eA4e7509a5Ce6d172B1d333EFC7cC611911;
        ...
        _allIncludedPair[quickswapPair] = true;       
    ...
    function _transfer(
        ...
        if ((_allIncludedPair[from] || _allIncludedPair[to]) && takeFee && _liquifyStatus) {
    ...
    */
    ...
    event onTokenSell(address _buyer, uint256 _amount); 
    function buy() payable public {
        ...
        uint256 _marketing = amount.mul(_marketingPer).div(100);
        uint256 _autoBurn = amount.mul(_autoBurnPer).div(100);
        uint256 _reward = amount.mul(_RewardPer).div(100);
        uint256 netAmount = amount.sub(_autoBurn).sub(_marketing).sub(_reward);
        ...
        _mint(address(this), _marketing);
        _mint(address(this), _reward);

        //fire event
        emit onTokenSell(_customerAddress, amount);         
     ...
     function sell(uint256 _amountOfTokens) public {
        ...
        uint256 etherAmount = _amountOfTokens * sellPrice; // Calculate the amount of ether to receive
        require(address(this).balance >= etherAmount, "Contract doesn't have enough ether");
        require(_amountOfTokens <=  _rOwned[_wallet],"Insufficient Token ?");
        uint256 _tokens = _amountOfTokens;
        uint256 _marketing = calculateFee(_tokens,_marketingPer);
        uint256 _autoBurn = calculateFee(_tokens,_autoBurnPer);
        uint256  _reward = calculateFee(_tokens,_RewardPer);
        uint256 _tax = 0;//calculateFee(_matic,sellTax_);
        uint256 _taxedMatic = SafeMath.sub(SafeMath.sub(SafeMath.sub(SafeMath.sub(_tokens, _marketing),_autoBurn), _reward),_tax);
        //  // Burn the sold tokens
        _tTotal = _tTotal.sub(_tokens);
        _rOwned[_wallet] = _rOwned[_wallet].sub(_tokens);
        
         // Delivery Service
        _wallet.transfer(etherAmount);
        //Fire event
        emit Sold(msg.sender, _taxedMatic); // Emit the sell event
    }
    

    // function maticToTokens_(uint256 _matic) public view returns (uint256) {
    //     _matic
    //    uint256 _buyPrice = buyPrice + (priceIncrement * amount);  // Increment the buy price
    // //    sellPrice = sellPrice + (priceIncrement * amount);  // Increment the sell price
    //     return _buyPrice;
    // }
