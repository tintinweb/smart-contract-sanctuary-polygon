...
contract CarbonStandardValidatorNFT is ERC721, ERC721Enumerable, ERC721Burnable, Ownable, EIP712, ERC721Votes {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;

    constructor()
        ERC721("Carbon Standard Validator NFT", "CSV")
        EIP712("Carbon Standard Validator NFT", "1")
    {}
    ...
    {
        require(!isBlacklisted(tokenId), "CarbonStandardValidatorNFT: token is blacklisted");
    ...
    // Mapping from token ID to blacklist status
    mapping (uint256 => bool) private _blacklisted;

    // Event emitted when a token is blacklisted or unblacklisted
    event BlacklistStatusChanged(uint256 indexed tokenId, bool blacklisted);

    // Returns true if the given token ID is blacklisted
    function isBlacklisted(uint256 tokenId) public view returns (bool) {
        return _blacklisted[tokenId];
    }

    // Sets the blacklist status of a token ID
    // Can only be called by the owner of the contract
    function setBlacklistStatus(uint256 tokenId, bool blacklisted) public onlyOwner {
        require(_exists(tokenId), "Nonexistent token");
        _blacklisted[tokenId] = blacklisted;
        emit BlacklistStatusChanged(tokenId, blacklisted);
    }
}