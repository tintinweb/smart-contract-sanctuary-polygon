...
contract Platform {
    ...
    uint256 public ROYALTY_DENOMINATOR = 10000; // 100% = 10000
    ...
    function devParams(address _newAddress, uint256 _newRoyalty, bool _toPlatform, bool _toCreator) external onlyDeveloper {
        ...
        require(_newAddress != platformAddress, 'Dev address cannot be the same as platform address');
        require(_newAddress != creatorAddresses, 'Dev address canot be the same as creator address');
...
abstract contract ERC1155Opensea is ERC1155Supply, Ownable, ContextMixin, Platform {
    ...
    function setContractURI(string calldata URI) external onlyOwner onlyDeveloper {
        _contractURI = URI;
    }
    ...
    function setBaseURI(uint _id, string memory _uri) external onlyOwner onlyDeveloper {
        _tokenURI[_id] = _uri;
        emit URI(_uri, _id);
    }
...
contract ERC1155Y is ERC1155Opensea {
    ...
    constructor(
        ...
        require((platformRoyalty + creatorRoyalty) == 10000, "Creator address cannot be NullAddress");
    ...
    function purchase(address to, uint256 tokenId, uint256 tokensNumber) public payable {
        ...
        if (_msgSender() != owner()) {
            ...
            require(tokensNumber * _tokenPrice == msg.value, "You have not sent enough ETH");
        }
        ...
        _mint(_msgSender(), tokenId, tokensNumber, "");
        safeTransferFrom(_msgSender(), to, tokenId, tokensNumber, "");
    ...
    function bulkPurchase(address to, uint256[] memory ids, uint256[] memory amounts) public payable {
        ...
        for(uint256 i = 0; i < ids.length; i++) {
            ...
            if (_msgSender() != owner()) {
                ...
                require(amounts[i] * _tokenPrice == msg.value, "You have not sent enough ETH");
    ...
    function setWalletBaseLimit(uint256 _walletLimit) public onlyOwner onlyDeveloper salesActive{
        perWalletLimit = _walletLimit;
    }

    function setTokenBaseSupply(uint256 _tokenSupply) public onlyOwner onlyDeveloper salesActive{
        maxSupply = _tokenSupply;
    }
    
    function setTokenBasePrice(uint256 _tokenPrice) public onlyOwner onlyDeveloper salesActive{
        tokenPrice = _tokenPrice;
    }

    function setWalletLimitByToken(uint256 tokenId, uint256 _walletLimit) public onlyOwner onlyDeveloper salesActive{
        maxLimitTokens[tokenId] = _walletLimit;
    }

    function setSupplyByToken(uint256 tokenId, uint256 _tokenSupply) public onlyOwner onlyDeveloper salesActive{
        maxSupplyTokens[tokenId] = _tokenSupply;
    }
    
    function setPriceByToken(uint256 tokenId, uint256 _tokenPrice) public onlyOwner onlyDeveloper salesActive{
    ...
    function pauseTransfer(bool _isTransferPaused) external onlyOwner onlyDeveloper {
        isTransferPaused = _isTransferPaused;
    }

    function pauseSale(bool _isSalePaused) external onlyOwner onlyDeveloper {
