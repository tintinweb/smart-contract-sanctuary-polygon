	...
	ReentrancyGuardUpgradeable
{
	 ...
	 * @param _verifyClaimRequestParams The verify request claim cover (policy) parameters as a tuple.
	 *
	 * Requirement:
	 *  - `nonReentrant` Can't call multiple when the first tx is pending of a user.
	 *  - `msg.sender` Must be a SuperManager.
	...
	function verifyClaimRequest(
		...
		return
			keccak256(
				abi.encodePacked(
					_verifyClaimRequestParams.user,
					_verifyClaimRequestParams.asset,
					_verifyClaimRequestParams.ipfsHash,
					_verifyClaimRequestParams.value,
					_verifyClaimRequestParams.generatedAt,
					_verifyClaimRequestParams.expiresAt,
					address(this)
				)
			);
	}
	 ...
	 */
	function policyClaimInternal(ClaimPolicyParams memory _claimPolicyParams)
		private
		returns (bool)
	{
		require(_claimPolicyParams.asset != address(0), "Invalid asset");

		require(_claimPolicyParams.expiresAt > block.timestamp);
		bytes32 hashMessage = verifyClaimRequest(
			verifyClaimRequestParams(
				msg.sender,
				_claimPolicyParams.asset,
				_claimPolicyParams.ipfsHash,
				_claimPolicyParams.value,
				_claimPolicyParams.generatedAt,
				_claimPolicyParams.expiresAt
			)
		);
		...
		address signer = ECDSAUpgradeable.recover(
			message,
			_claimPolicyParams.v,
			_claimPolicyParams.r,
			_claimPolicyParams.s
		);
		if (signer == V) {
	 ...
	 * @param _claimPolicyParams The claim policy parameters as a tuple.
	 *
	 * Requirement:
	 *  - `nonReentrant` Can't call multiple when the first tx is pending of a user.
	 */
		...
		nonReentrant
	{
		...
		(
			...
			bool isActive,
			bool isRedeemed
		) = PDS.policyData(_claimPolicyParams.policyId);
		require(isActive == true, "PNA");
		require(isRedeemed == false, "PIR");
		require(policyData.user == msg.sender, "YAOP");
		require(_claimPolicyParams.asset == policyData.asset, "ICA");
		require(AR.isSupportAssets(poolId, _claimPolicyParams.asset), "A");
		require(pendingClaimRequest(_claimPolicyParams.policyId) == 0, "OP");
		uint256 maxCoverageOfThisUser = policyData.maxCoverage;
		require(
			maxCoverageOfThisUser > 0 &&
				(maxCoverageOfThisUser -
					_historyOfPolicyId[_claimPolicyParams.policyId]
						.claimAmountPaid -
					_historyOfPolicyId[_claimPolicyParams.policyId]
						.claimPending >=
					_claimPolicyParams.value),
		...
		require(existsIpfs(_claimPolicyParams.ipfsHash) == false, "HD");
		// PDS decimals //
		require(_claimPolicyParams.value >= 10**18, "D18");
		require(policyClaimInternal(_claimPolicyParams), "ED");
		_existsIpfs[_claimPolicyParams.ipfsHash] = true;
		_historyOfPolicyId[_claimPolicyParams.policyId]
			.claimPending += _claimPolicyParams.value;
		...
		claimIdDataThisId.policyId = _claimPolicyParams.policyId;
		claimIdDataThisId.asset = _claimPolicyParams.asset;
		claimIdDataThisId.proofData = _claimPolicyParams.ipfsHash;
		claimIdDataThisId.timeout = block.timestamp + 1 days;
		claimIdDataThisId.requestAmount = _claimPolicyParams.value;
		_historyOfPolicyId[_claimPolicyParams.policyId]
			.claimPending += _claimPolicyParams.value;
		_historyOfPolicyId[_claimPolicyParams.policyId]
			.claimIdLasted = _claimId;

		_keepClaimIdUser[msg.sender].push(_claimId);
		_keepClaimIdPolicy[_claimPolicyParams.policyId].push(_claimId);
