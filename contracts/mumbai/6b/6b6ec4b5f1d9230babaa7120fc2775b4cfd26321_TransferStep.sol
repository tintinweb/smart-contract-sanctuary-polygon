...
contract TransferStep is BaseStep {
    ...
    // Controls how much fees to be paid and to who
    ...
    /**
     * @notice Initializes the transfer step. Only meant to be called by a vault
     * @dev Will revert if the step's node key is not the root of the step tree
     * @param _stepNodeKey The step's node key in the step tree
     */
    function init(uint256 _stepNodeKey, bytes memory) external virtual override canBeLegacied {
        // Transfer step must be the root of the stepTree
        if (_stepNodeKey != 1) revert Errors.InvalidStepPosition();
    }

    /**
     * @notice Transfers assets to the the vault and the drem collector and vault collector if fees are turned on. Only meant to be called by a vault
     * @dev Uses IVault(msg.sender) to call back on the vault. If an EOA calls this function, it will revert
     * If a malicious contract calls on this function, it will not affect any Drem vault
     * @param _caller the original msg.sender of the vault's windSteps()
     * @param _amountIn the amount send in to transfer
     */
    ...
    /**
     * @notice Transfers assets to the the caller and the drem collector and vault collector if fees are turned on. Only meant to be called by a vault
     * @dev Uses IVault(msg.sender) to call back on the vault. If an EOA calls this function, it will revert
     * If a malicious contract calls on this function, it will not affect any Drem vault
     * @param _caller the original msg.sender of the vault's windSteps()
     * @param _amountIn the amount send in to transfer
     * @return returns the total fund split that was sent to the user
     */
    function unwind(address _caller, uint256, uint256 _amountIn, DataTypes.UnwindInfo memory, bytes memory)
        external
        virtual
        override
        canBeDisabled
    ...
    /**
     * @notice returns the asset in of a vault's transfer step
     * @dev step node key does not matter here
     * @return the address of the vault's denomination asset
     */
    function assetIn(uint256, address _vault) external view virtual override returns (address) {
        return IVault(_vault).getDenominationAsset();
    }

    /**
     * @notice returns the asset out of a vault's transfer step
     * @dev step node key does not matter here
     * @return the address of the vault's denomination asset
     */
    function assetOut(uint256, address _vault) external view virtual override returns (address) {
        return IVault(_vault).getDenominationAsset();
    }

    /**
     * @notice Returns the value of the tokens produced for the transfer step. Meant to be called only by a vault
     * @dev step node key does not matter here, since there can only be one transfer step in a step tree, and it must be the root
     * Denomination asset also does not matter here: the amount of tokens produced by the transfer step will be in the denomination asset
     * @return returns the tokens produced by the vault
     */
    function value(uint256, uint256 _tokensProduced, address) external view override returns (uint256) {
        return _tokensProduced;
    }
    ...
    /**
     * @dev Admin function to set the fee controler. Only callable by the Hub owner
    ...
    function _sendFunds(address _recipient, uint256 _amount, address _denominationAsset) internal {
        ...
        if (!(transferSuccess)) revert TransferLib.SendTransferFailed();
    }
    ...
    function _receiveFunds(address _sender, address _recipient, uint256 _amount, address _denominationAsset) internal {
        ...
        // revert if this transfer has not been executed
        if (!(transferSuccess)) revert TransferLib.ReceiveTransferFailed();
    }
...
abstract contract BaseStep is IStep, HubOwnable {
    ...
    function unwind(
        ...
        uint256 stepNodeKey,
        uint256 amountIn,
        DataTypes.UnwindInfo memory unwindInfo,
        bytes memory encodedVariableArgs
    ) external virtual canBeDisabled returns (uint256) {}

    function value(uint256 stepNodeKey, uint256 tokensProduced, address denominationAsset)
        external
        view
        virtual
        returns (uint256)
    ...
    function _approveERC20ForVault(address _vault, address _erc20, address _spender, uint256 _amount) internal {
        bytes memory encodedApprovalParams = abi.encodeWithSelector(IERC20.approve.selector, _spender, _amount);

        bytes memory returnData = IVault(_vault).execute(_erc20, encodedApprovalParams);
    ...
    function _validateStepNotLegacied() internal view {
        ...
        if (DREM_HUB.getStepState(address(this)) < DataTypes.StepState.Enabled) {
            revert Errors.StepLegaciedDeprecatedOrDisabled();
        }
    }
}
...
/// @title Transfer Step Library
/// @author danyams & naveenailawadi

library TransferLib {
    /// Errors
    /**
     * The amount in passed into wind() is zero
     */
    error InsufficientFunds();

    /**
     * The transfer to send funds to the Drem collector, vault collector, or, vault failed
     */
    error SendTransferFailed();

    /**
     * The transfer for the Drem collector, vault collector, or recipient to receive funds failed
     */
    error ReceiveTransferFailed();
