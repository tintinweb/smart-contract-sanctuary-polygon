    ...
    ILand
{
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        _requireMinted(tokenId);
        uint256 floor = manager.landInfos(tokenId).floor;
        string memory baseuri = _baseURI();
        return bytes(baseuri).length > 0 ? string(abi.encodePacked(baseuri, tokenId.toString(), "/", floor.toString())) : "";
    }
    ...
    function mintToken(address to, uint256 id) external override onlyGenesisMinter nonReentrant whenNotPaused {
        require(currentAmount < maxAmount && id <= maxAmount,"exceed");
        _safeMint(to, id);
    }

    function mintBatchToken(address to, uint256[] memory ids) external override onlyGenesisMinter nonReentrant whenNotPaused {
        require(currentAmount + ids.length <= maxAmount,"exceed");
