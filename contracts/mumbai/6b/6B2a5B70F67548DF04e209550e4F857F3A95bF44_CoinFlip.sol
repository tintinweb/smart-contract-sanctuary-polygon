...
contract CoinFlip {
    ...
    uint256 constant ESTIMATED_TRANSACTION_GAS = 21000; // Placeholder value, replace with actual gas cost
    ...
    event GameCancelled(uint256 gameId, address gameCreator, uint256 betAmount, uint256 withdrawAmount, uint256 transactionFee);
 ...
 function joinGame(uint256 gameId) external payable {
    ...
    uint256 totalFunds = game.betAmount * 2;
    uint256 winningAmount = totalFunds * 95 / 100;
    bool player1Wins = (uint256(blockhash(block.number - 1)) % 2 == 0);

    if (game.player1Bet == player1Wins) {
        balances[game.player1] += winningAmount;
        emit BetResolved(game.player1, totalFunds, game.player1Bet, true);
        gameHistory[game.player1].push(GameHistory(gameId, game.player1, totalFunds, game.player1Bet, true));
    } else {
        balances[game.player2] += winningAmount;
        emit BetResolved(game.player2, totalFunds, game.player2Bet, true);
        gameHistory[game.player2].push(GameHistory(gameId, game.player2, totalFunds, game.player2Bet, true));
    }

    uint256 houseEdge = totalFunds - winningAmount;
  ...
  function getActiveGames() external view returns (Game[] memory) {
    ...
    require(gameId < games.length, "Invalid game ID");
    Game storage game = games[gameId];
    require(game.player1 == msg.sender, "Only game creator can cancel the game");
    require(game.player2 == address(0), "Cannot cancel a game with two players");

    uint256 betAmount = game.betAmount;
    require(betAmount > 0, "No bet amount to withdraw");

    // Update game state
    game.betAmount = 0;

    // Calculate the transaction fee
    uint256 transactionFee = tx.gasprice * ESTIMATED_TRANSACTION_GAS;

    // Calculate the amount to withdraw after deducting the transaction fee
    uint256 withdrawAmount = betAmount - transactionFee;

    // Transfer the bet amount minus the transaction fee to the game creator
    payable(msg.sender).transfer(withdrawAmount);

    emit GameCancelled(gameId, msg.sender, betAmount, withdrawAmount, transactionFee);
}
