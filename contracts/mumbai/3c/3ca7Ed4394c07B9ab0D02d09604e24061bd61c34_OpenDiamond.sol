...
library LibCommon {
    ...
    function _getTimelockValidityDeposit()
        internal
        view
        returns (
            /*bytes32 market, bytes32 commitment*/
            uint256
        )
    {
    ...
    function _getTimelockValidityCollateral()
        internal
        view
        returns (
            /*bytes32 market, bytes32 commitment*/
    ...
    function _getsavingsAprTimeLength(bytes32 market, bytes32 _commitment) internal view returns (uint256) {
        AppStorageOpen storage ds = diamondStorage();
        return ds.indsavingsAPRRecords[market][_commitment].time.length;
    }
...
// =========== Comptroller structs ===========
/// @notice each savingsAPR or APR struct holds the recorded changes in interest data & the
/// corresponding time for a particular commitment type.
struct savingsAPR {
    bytes32 market;
    uint256[] time; // ledger of time when the savingsAPR changes were made.
    uint256[] savingsAprChanges; // ledger of savingsAPR changes.
}

struct APR {
    bytes32 market; // validity
...
struct YieldLedger {
    ...
    uint256 oldLengthAccruedYield; // length of the savingsAPR time array.
...
struct DeductibleInterest {
    ...
    uint256 oldLengthAccruedInterest; // length of the savingsAPR time array.
    uint256 oldTime; // length of the savingsAPR time array.
...
struct AppStorageOpen {
    ...
    mapping(bytes32 => mapping(bytes32 => savingsAPR)) indsavingsAPRRecords;
