...
// File: contracts/bapevolution/Evolution.sol
    ...
    DefaultOperatorFiltererUpgradeable
{
    ...
    */
    ...
    event RoyaltyPercentageEvent(uint256 indexed _newPercentage);
    ...
    */
    ...
    function initialize() public initializer {
        __ERC721_init("Evolution", "EVO");
        __Ownable_init();
        __ERC721Burnable_init();
        __UUPSUpgradeable_init();
        __ReentrancyGuard_init();

        commonURI = "ipfs://bafybeihqqmdxzslqafnqasuawlxk32n4qhg5rhshku7dkhlaz2bm5zamyq/1";
        maxSupply = 5000;
        royaltiesPercentage = 1000;
        withdrawalAddress = 0xD4631a82423B92Fb1434403b5a3Af99eeba1d8e1;
    }
    ...
    */
    ...
    function mintForAddress(address _receiver, uint256 _mintAmount) public virtual onlyOwner {
    ...
    function airDrop(
        ...
        for (uint256 i = 0; i < _addresses.length; i++) {
            require(supply.current() + _mintAmount[i] <= maxSupply, "Max supply exceeded");
            _mintLoop(_addresses[i], _mintAmount[i]);
        }
    }
    ...
    */
    ...
    // set royalty percentage 1000 = 10%
    ...
    function setCommonURI(string memory _commonURI) external onlyOwner {
        commonURI = _commonURI;
    }

    function batchSetTokenURI(uint256[] memory _tokenIds, string[] memory _tokenURIs) external virtual onlyOwner {
        require(_tokenIds.length == _tokenURIs.length, "Array length mismatch");

        for (uint256 i = 0; i < _tokenIds.length; i++) {
            uint256 tokenId = _tokenIds[i];
            string memory newTokenURI = _tokenURIs[i];
