...
contract Staking {
    ...
    function unstakeTokens(uint256 _stakeId) external returns(bool){
        ...
        require(flagByStakeId[_stakeId] != true, "Tokens already unstaked");
        uint256 reward = amountByStakeId[_stakeId] * rewardMultiplier * (block.timestamp - timeByStakeId[_stakeId])/86400;   //multipler will be divided by 1000 in next step, in 3 decimals
        ...
        currentTotalStakesByAddress[msg.sender] -= amountByStakeId[_stakeId];
    ...
    function checkCurrentByStakeId(uint256 _stakeId) external view returns(uint256 _totalAmount) {
        if(flagByStakeId[_stakeId] == false && withdrawStakeStatus[_stakeId] == false)
        _totalAmount = amountByStakeId[_stakeId] + (amountByStakeId[_stakeId] * rewardMultiplier * (block.timestamp - timeByStakeId[_stakeId])/86400)/100000;
        else if(flagByStakeId[_stakeId] == true && withdrawStakeStatus[_stakeId] == false) 
        _totalAmount = withdrawStakeAmount[_stakeId];
        else
        _totalAmount = 0;
    }
