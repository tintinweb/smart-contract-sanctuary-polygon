 ...
 * @author Defed, inspired by the Openzeppelin ERC20 implementation
 **/
...
contract AToken is VersionedInitializable, IncentivizedERC20, IAToken {
  ...
  // Keeps track of the 'gons' deposited into the Defed system
  ...
  function _fetchExtData() internal view returns (ExtData memory) {
    ...
    (stablePrincipal, stablePrincipalScaled) = IADebtToken(STABLE_DEBT_TOKEN_ADDRESS).getAMPLBorrowData();

    uint256 variablePrincipal;
    int256 variablePrincipalScaled;
    (variablePrincipal, variablePrincipalScaled) = IADebtToken(VARIABLE_DEBT_TOKEN_ADDRESS).getAMPLBorrowData();
