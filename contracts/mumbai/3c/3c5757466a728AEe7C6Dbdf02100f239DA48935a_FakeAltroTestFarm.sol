...
contract FakeAltroTestFarm is IStakingService {
	...
	function poolInfo(uint256 pid) external view returns (PoolInfo memory) {
		...
		uint256 accFCAPerShare = 1000;
		uint32 lastRewardTime = uint32(block.number);
		uint16 allocPoint = 10;
		PoolInfo memory _poolInfo = PoolInfo(_stakingToken, stakingTokenTotalAmount, accFCAPerShare, lastRewardTime, allocPoint);
		return _poolInfo;
	}
}
