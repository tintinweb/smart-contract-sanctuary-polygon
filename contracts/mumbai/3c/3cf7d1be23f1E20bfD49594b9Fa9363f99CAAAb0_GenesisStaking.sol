...
contract GenesisStaking is Pausable {
    ...
    ///@dev params need to be uint,like 20 % need to be 20 ;
    function setDetails(
    ...
    ///@param _index 3 month = 0 ,6 month=1 , 12 month =2
    function claimReward_ROE(uint256 _index) external {
        ...
        totalStaking -= originAmount;
        bonusReward -= bonusAmount;
        stakes[msg.sender][_index].amount = 0;
    ...
    function calBonus(address _address, uint256 _index)
        ...
        returns (uint256)
    {
        return (stakes[_address][_index].amount * (bonusReward / totalStaking));
    }
...
contract ERC20Implementation is IERC20, Pausable {
    ...
    function transfer(address _to, uint256 _value) public whenNotPaused virtual override returns (bool success) {
    ...
    function transferFrom(address _from, address _to, uint256 _value) public virtual whenNotPaused override returns (bool success) {
        require(_to != address(0),INVALID_ADDRESS);
