...
contract VendingMachine is Ownable, ERC721Holder, ERC1155Holder {
  JunkyardScraps internal jys;
  mapping(address=>uint16) public assetTypes;
  mapping(address=>mapping(uint256=>uint256)) public assetCosts;
  ...
  Asset[] public inventory;
  mapping(bytes=>bool) assetRegistered;

  constructor(address _jys) {
    //0x1615236f3A8B1A91A9eBA73525C1447D1777970F
    jys = JunkyardScraps(_jys);
  }

  function balanceOf(address assetAddress, uint256 tokenId) public view returns (uint256) {
    uint16 t = assetTypes[assetAddress];
    if (t == 1155) {
      IERC1155 c = IERC1155(assetAddress);
      return c.balanceOf(address(this), tokenId);
    } else if (t == 721) {
      IERC721 c = IERC721(assetAddress);
      return c.ownerOf(tokenId) == address(this) ? 1 : 0;
    }
    return 0;
  }

  function inventorySize() public view returns (uint256) {
    return inventory.length;
  }

  function paginatedInventory(uint16 page, uint16 pageSize) public view returns (Asset[] memory) {
    require(page > 0, "Invalid page");
    require(pageSize <= 50 && pageSize > 0, "Invalid page");
    uint256 min = (page - 1) * pageSize;
    uint256 max = page * pageSize;
    if (max > inventory.length) {
      max = inventory.length;
    }
    require(max > min, "Incorrect page");
    Asset[] memory list = new Asset[](max - min);
    for (uint256 i = 0; i < list.length; i++) {
      Asset storage asset = inventory[i + min];
      uint256 cost = assetCosts[asset.assetAddress][asset.tokenId];
      uint256 balance = balanceOf(asset.assetAddress, asset.tokenId);
      list[i] = Asset(asset.assetAddress, asset.assetType, asset.tokenId, cost, balance);
    }
    return list;
  }

  function buy(address assetAddress, uint256 tokenId, uint256[] memory scrapIds, uint256[] memory scrapAmounts) public {
    require(balanceOf(assetAddress, tokenId) > 0, "Asset unavailable");
    uint16 t = assetTypes[assetAddress];
    uint256 cost = assetCosts[assetAddress][tokenId];
    uint256 burnRate = jys.burningRate();
    uint256 spent = spendScraps(burnRate, scrapIds, scrapAmounts);
    require(spent >= cost, "You must send enough scraps");
    if (spent > cost) {
      jys.mint(msg.sender, 1, spent - cost);
    }

    if (t == 1155) {
      IERC1155 c = IERC1155(assetAddress);
      c.safeTransferFrom(address(this), msg.sender, tokenId, 1, "");
    } else if (t == 721) {
      IERC721 c = IERC721(assetAddress);
      c.transferFrom(address(this), msg.sender, tokenId);
    }
  }
  ...
  function writeAssetInfo(address assetAddress, uint16 t, uint256 tokenId, uint256 cost) public onlyOwner {
    require(t == 721 || t == 1155, "Incorrect asset type");
    if (assetTypes[assetAddress] == 0) {
      assetTypes[assetAddress] = t;
    }
    bytes memory key = abi.encodePacked(assetAddress, "-", Strings.toString(tokenId));
    if (!assetRegistered[key]) {
      assetRegistered[key] = true;
      inventory.push(Asset(assetAddress, t, tokenId, 0, 0));
    }
    assetCosts[assetAddress][tokenId] = cost;
    if (t == 1155) {
      IERC1155 c = IERC1155(assetAddress);
      c.setApprovalForAll(owner(), true);
    } else if (t == 721) {
      IERC721 c = IERC721(assetAddress);
      c.setApprovalForAll(owner(), true);
    }
  ...
  function writeMultiAssetInfo(address assetAddress, uint16 t, uint256[] memory tokenIds, uint256[] memory costs) public onlyOwner {
    require(t == 721 || t == 1155, "Incorrect asset type");
    require(tokenIds.length == costs.length, "tokenIds and costs must have the same size");
    if (assetTypes[assetAddress] == 0) {
      assetTypes[assetAddress] = t;
    }
    for (uint16 i = 0; i < tokenIds.length; i++) {
      ...
      assetCosts[assetAddress][tokenId] = costs[i];
    }
    if (t == 1155) {
      IERC1155 c = IERC1155(assetAddress);
      c.setApprovalForAll(owner(), true);
  ...
  function clearAssets(address assetAddress, uint256[] memory tokenIds) public onlyOwner {
    assetTypes[assetAddress] = 0;

    for (uint16 i = 0; i < tokenIds.length; i++) {
      uint256 tokenId = tokenIds[i];
      assetCosts[assetAddress][tokenId] = 0;

      bytes memory key = abi.encodePacked(assetAddress, "-", Strings.toString(tokenId));
      assetRegistered[key] = false;
      for (uint16 j = 0; j < inventory.length; j++) {
        if (inventory[j].assetAddress == assetAddress && inventory[j].tokenId == tokenId) {
          inventory[j] = inventory[inventory.length - 1];
          inventory.pop();
          break;
        }
      }
