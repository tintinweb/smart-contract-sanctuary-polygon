...
contract MinimalAccessManagement is AccessManagement {
    constructor() withSenderAsOwner {}
}
...
abstract contract AccessManagement is IAccessManagement, ERC165, Context {
    ...
    bytes32 constant MANAGE_CAPABILITIES = keccak256("ADD_CAPABILITY");
    bytes32 constant MANAGE_ROLES = keccak256("ADD_ROLE");
    ...
    modifier withSenderAsOwner() {
        _setupOwner(_msgSender());
        _;
    }

    function _setupOwner(address account) internal virtual {
        _addCapability(OWNER, MANAGE_ROLES);
        _addCapability(OWNER, MANAGE_CAPABILITIES);
        _grantRole(OWNER, account, 1);
    }
