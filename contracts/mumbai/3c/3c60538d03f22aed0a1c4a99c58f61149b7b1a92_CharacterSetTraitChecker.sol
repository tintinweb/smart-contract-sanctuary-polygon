...
contract CharacterSetTraitChecker is Ownable, ITraitChecker {
    string public constant TRAIT_FAMILY = "Character set";
    address public emojiUtils;
    
    constructor(address _emojiUtils) Ownable() {
    ...
    function getTrait(string calldata _username) external view returns (string memory, string memory) {
        ...
        for (uint i = 0; i < b.length;) {
            ...
            //test if it is keycap or emoji
            (bool isKeycap, uint emojiSize) = EmojiUtils(emojiUtils).isValidKeyCap(b, i);
            if(isKeycap) {
            ...
            //if code is an emoji
            if(EmojiUtils(emojiUtils).isEmoji(b, i)) {
                isAlphaNumeric = false;
                isLetter = false;
                isDigit = false;
            ...
            //neither a digit nor a letter -> it is not alphanumeric
            isAlphaNumeric = false;
            // using "size" and not 1 because it could be a unicode special char
            i += size;
            
        }
        if(isDigit) return (TRAIT_FAMILY,"Digit");
        if(isLetter) return (TRAIT_FAMILY,"Letter");
        if(isAlphaNumeric) return (TRAIT_FAMILY,"Alphanumeric");
        if(isEmoji) return (TRAIT_FAMILY,"Emoji");
        return (TRAIT_FAMILY,"Mixed");
    }
...
abstract contract IEmojiIsValid {
    function matchUnicode(bytes memory b, uint index, uint32 utf8Code) internal view returns (bool) {
