...
import './UserContractMetaTx.sol';

contract YexleMetaTx is ERC2771Context, ERC721Holder, ERC721Burnable, Ownable {
    /*
    ...
    */
    ...
    // Biconomy Truster Forwarder latest : 0x69015912AA33720b842dCD6aC059Ed623F28d9f7
    constructor(string memory _metadata, address _trustedForwarder) 
    ERC2771Context(_trustedForwarder) 
    ERC721("Yexele Appian", "Yexele_Land"){
    ...
    */
    function setTokenURI(uint256 _id, string memory _tokenUri) external {
        require(_msgSender() == _owners[_id],"you are not the owner");
    ...
    function approveAndTransferLand1(approverData memory _data) external{
        if(!approverAddress[_msgSender()]){ revert notApproverAddress();}
        if(voteRecord[_msgSender()][_data._tokenId]){ revert alreadyApproved();}
        if(_data.status == true){
            approverDecision[_data._tokenId][_msgSender()] = _data.status;
            voteRecord[_msgSender()][_data._tokenId] = true;
            approvecount[_data._tokenId] += 1;
        }
        if(approvecount[_data._tokenId] == 2){
            transferFrom(_msgSender(), _data._sellingTo, _data._tokenId);
    ...
    function landDocumentViewRequestApprove(address _requester, uint tokenId, bool _status) external{
        ...
        if(!status){ revert userContractError();}
        if(_msgSender() != L1Approver){ revert notL1Approver();}
        if(_status){
    ...
    function requestLandForSale(uint _tokenId) external{
        UserContractMetaTx useC = UserContractMetaTx(userContract);
        (bool status) = useC.verifyUser(_msgSender());
        if(!status){ revert userContractError();}
        if(viewAccessGranted[_msgSender()][_tokenId]){
            landRequest[_tokenId][_msgSender()] = true;
        }else{
    ...
    function ownerDecisionforRaisedRequest(address _requester, uint _tokenId, bool _status) external{
        ...
        (bool status) = useC.verifyUser(_requester);
        if(!status){ revert userContractError();}
        require(_msgSender() == _owners[_tokenId],"you are not the owner of nft");
    ...
    function registrationForLandByBuyer(uint tokenId, string memory _DocumentUri) external{
        require(ownerAcceptLandSales[_msgSender()] == true, "registration is not possible");
    ...
    function approveByL2(approverDataForL2 memory _data) external{
        ...
        (bool status) = useC.verifyUser(_data._sellingTo);
        if(!status){ revert userContractError();}
        if(_msgSender() != L2Approver){ revert notL2Approver();}
