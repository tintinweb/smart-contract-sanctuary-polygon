...
import "contracts/@openzeppelin/contracts/utils/introspection/ERC165Checker.sol";
...
contract TokenPaymasterProxy is Ownable {
    ...
    uint256 public POST_RELAYED_CALL_GAS_LIMIT = 310000;
    uint256 public PAYMASTER_ACCEPTANCE_BUDGET = 250000; // PRE_RELAYED_CALL_GAS_LIMIT + FORWARDER_HUB_OVERHEAD
    uint256 public CALLDATA_SIZE_LIMIT = 10500;

    bool public initialized;

    constructor(address payable _impl) {
        implementation = _impl;
    }

    function setImplementation(address payable _newImpl) public onlyOwner {
        require(implementation != _newImpl);
...
contract TokenPaymasterImpl is TokenPaymasterProxy, BaseRelayRecipient, IPaymaster {
    using SafeMath for uint256;

    event AddToken(address token, address[] path);
    event RemoveToken(address token);
    event Received(address indexed sender, uint256 eth);
    event TokenPreCharge(
        address payer,
        uint256 gasPrice,
        uint256 pctRelayFee,
        uint256 baseRelayFee,
    ...
    constructor() TokenPaymasterProxy(payable(address(0))) {}
    ...
    function versionPaymaster() external view virtual override returns (string memory) {
        return "2.2.3+TokenPaymaster20220829";
    }
    ...
    function _msgSender() internal view virtual override(Context, BaseRelayRecipient) returns (address sender) {
    ...
    function trustedForwarder() public view override(IPaymaster, BaseRelayRecipient) returns (address forwarder) {
        forwarder = BaseRelayRecipient.trustedForwarder();
    }

    function setGasLimits(uint256 _PRE_RELAYED_CALL_GAS_LIMIT, uint256 _POST_RELAYED_CALL_GAS_LIMIT) public onlyOwner {
    ...
    // real path : token -> _path -> WETH
    function addToken(address _token, address[] calldata _path) external onlyOwner {
        validTokens[_token] = true;
        IERC20(_token).approve(address(router), type(uint256).max);

        paths[_token].push(_token);
        for (uint256 i = 0; i < _path.length; i++) {
            paths[_token].push(_path[i]);
        }
        paths[_token].push(weth);

        emit AddToken(_token, _path);
    }
    ...
    function removeToken(address _token) external onlyOwner {
        ...
        uint256 len = paths[_token].length;
    ...
    function _calculatePreCharge(
        ...
        tokenPreCharge = router.getAmountsIn(ethMaxCharge, paths[token])[0];
    }

    // NOTE: paymasterData = tokenAddress + permitData
    function preRelayedCall(
        GsnTypes.RelayRequest calldata relayRequest,
        bytes calldata signature,
        bytes calldata approvalData,
        ...
        require(relayRequest.relayData.paymasterData.length >= 32, "No fee token address in paymasterData");

        address token = MinLibBytes.readAddress(relayRequest.relayData.paymasterData, 12);

        require(validTokens[token], "Not Valid Token");
        if (relayRequest.relayData.paymasterData.length > 32) {
            require(
                permitMethodSignature == MinLibBytes.readBytes4(relayRequest.relayData.paymasterData, 32),
            ...
            require(success, string(abi.encodePacked("preRelayedCall: permit call reverted:")));
        }

        uint256 tokenPreCharge = _calculatePreCharge(token, relayRequest, maxPossibleGas);

        require(IERC20(token).transferFrom(payer, address(this), tokenPreCharge), "failed pre-charge");
    ...
    function postRelayedCall(
        ...
        token = MinLibBytes.readAddress(relayData.paymasterData, 12);
        uint256 ethActualCharge = relayHub.calculateCharge(gasUseWithoutPost + gasUsedByPost, relayData);

        uint256 tokenActualCharge = router.getAmountsIn(ethActualCharge, paths[token])[0];
        _depositProceedsToHub(token, tokenActualCharge);

        uint256 remainingTokenBalance = IERC20(token).balanceOf(address(this));
        require(IERC20(token).transfer(payer, remainingTokenBalance), "failed refund");
    ...
    function _depositProceedsToHub(address token, uint256 tokenActualCharge) private {
        router.swapExactTokensForETH(tokenActualCharge, 1, paths[token], address(this), block.timestamp + 600);
