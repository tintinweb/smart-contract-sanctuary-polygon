...
contract TheCryptoStudio is ERC1155
{
    ...
    string private constant MAIN_SCHEMA = "tokenID text, description text, image text, name text, audio text, animation_url text, maxSupply text, currentSupply text, mintPrice text, creator text, mutable text";
    ...
    function declareNFT(string memory name , string memory imageCID ,string memory audioCID , string memory animationCID , string memory description ,string memory spaceName, uint256 mintPrice, uint256 maxSupply,uint256 current) public   {
        ...
        runSQL(mainTableID,SQLHelpers.toInsert(
            MAIN_TABLE_PREFIX,
            mainTableID,
            "tokenID, description, image, name, audio, animation_url, maxSupply, currentSupply, mintPrice, creator, mutable",
            insertStatement
        ));
        runSQL(attributeTableID,SQLHelpers.insertAttributeStatement(
    ...
    function mint(uint256 tokenid) public payable {
        ...
        runSQL(mainTableID,SQLHelpers.toUpdate(
            MAIN_TABLE_PREFIX, 
            mainTableID, 
            string.concat("currentSupply='",Strings.toString(tokenInfoMap[tokenid].maxCap),"'"), 
    ...
    function addAttribute(uint256 tokenid , string memory trait_type , string memory value) public {
        onlyCreator(tokenid, msg.sender);
        runSQL(attributeTableID,SQLHelpers.addAttribute(
            ATTRIBUTE_TABLE_PREFIX,
            attributeTableID,
            tokenid,
            trait_type,
    ...
    function updateAudio(uint256 tokenid , string memory audioCID) public {
        ...
        runSQL(mainTableID,SQLHelpers.toUpdate(
            MAIN_TABLE_PREFIX,
            mainTableID,
            string.concat("audio","='",audioCID,"'"),
            string.concat("tokenID=",Strings.toString(tokenid))
        ));
    }

    function makeNFTImmutable(uint256 tokenid)public {
        onlyCreator(tokenid,msg.sender);
        tokenInfoMap[tokenid].creator = address(0);
        runSQL(mainTableID,SQLHelpers.toUpdate(
            MAIN_TABLE_PREFIX,
            mainTableID, 
            string.concat("mutable","='","0","'"), 
            string.concat("tokenID=",Strings.toString(tokenid))
        ));
    }
    ...
    function addSpaceArtist(string memory spaceName, address artist) public {
        ...
        runSQL(space_artists_tableID,SQLHelpers.spaceArtistInsertion(
            SPACE_ARTISTS_TABLE_PREFIX, 
            space_artists_tableID, 
            spaceName, 
            Strings.toHexString(artist)
        ));
    }
    ...
    function deleteSpaceArtist(string memory spaceName, address artist) public {
        ...
        runSQL(space_artists_tableID,SQLHelpers.toDelete(
            SPACE_ARTISTS_TABLE_PREFIX, 
            space_artists_tableID, 
            string.concat(
                "spaceName=",
                SQLHelpers.quote(spaceName)," and space_artist=",
                SQLHelpers.quote(Strings.toHexString(artist)))
            ));
    }
    ...
    function isSpaceMember(string memory spaceName, address sender) public view returns (bool){
        ...
        uint256 index;
        if(spaceInfoMap[spaceName].spaceArtists.contains(sender)){
            return true;
        }
