...
contract USDRExchange is AddressAccessor, IExchange, Pausable {
        ...
        returns (uint256)
    {
        ...
        ITreasury(treasury).withdraw(tngbl, amountOut);
        IERC20(tngbl).safeTransfer(to, amountOut);
        return amountOut;
    }
        ...
        returns (uint256)
    {
        ...
        ITreasury(treasury).withdraw(underlying, amountOut);
        IERC20(underlying).safeTransfer(to, amountOut);
        return amountOut;
    }
    ...
    function swapFromTNGBL(
        ...
        {
            // check backing
            (uint256 stable, uint256 rwa, , uint256 liquidity, uint256 debt) = ITreasury(treasury)
                .getTreasuryValue(true);
            uint256 usdrMarketCap = IStaking(staking).usdrMarketCap();
            multiplier = getTNGBLMultiplier(usdrMarketCap * 1e9, stable + rwa + liquidity - debt);
    ...
    function _checkTNGBLToUSDRMarketCapRatio() private view {
        ...
        (uint256 stable, uint256 rwa, , uint256 liquidity, uint256 debt) = ITreasury(treasury)
            .getTreasuryValue(true);
        require(
            stable + rwa + liquidity - debt >= IStaking(staking).usdrMarketCap() * 88e7,
    ...
    {
        address usdr;
        (treasury, usdr, tngbl) = abi.decode(
            addressProvider.getAddresses(
                abi.encode(TREASURY_ADDRESS, USDR_ADDRESS, TNGBL_ADDRESS)
            ),
            (address, address, address)
        );
        ...
        (uint256 stable, uint256 rwa, uint256 tngblVal, uint256 liquidity, uint256 debt) = ITreasury(treasury)
            .getTreasuryValue(true);
        require(
            stable + rwa + liquidity - debt < _scaleAmount(amountOut, tngblDecimals, 18),
            "sufficient backing"
        ...
        returns (address promissory, uint256 amountOut)
    {
        ...
        require(
            stable + rwa + liquidity - debt >=
                _scaleAmount(
                    IERC20(promissory).totalSupply() + amountOut,
                    promissoryDecimals,
                    18
                ),
            "insufficient backing"
