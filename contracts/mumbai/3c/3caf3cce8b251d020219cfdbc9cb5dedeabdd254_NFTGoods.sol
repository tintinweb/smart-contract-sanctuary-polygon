...
contract NFTGoods is INFTGoods, Initializable, Proxied, ERC721MintBatch, AccessControl {
    ...
    function mint(address to, uint96 batchSize) external override onlyRole(MINTER_ROLE) {
    ...
    function getNextTokenId() external view returns (uint96) {
    ...
    function _afterTokenTransfer(address from, address to, uint256 firstTokenId, uint256 batchSize) internal override {
        ...
        IStaking staking = IStaking(tempRegistry.getStaking());
        if (getStatus[firstTokenId] == Status.Packed && from != address(0) && msg.sender != address(staking)) {
            if (from == tempRegistry.root()) {
                staking.startCountingToPassiveStakingForNFT(firstTokenId);
            }
...
import "./openzeppelin_for_proxied/ERC721Consecutive.sol";

error ERC721MintBatchSingleMintNotSupported();
error ERC721MintBatchMintToZeroAddress();

abstract contract ERC721MintBatch is ERC721Consecutive {
    using BitMaps for BitMaps.BitMap;
    using Checkpoints for Checkpoints.Trace160;

    /**
     * @dev See {ERC721Consecutive-_mintConsecutive}.
     * Override that allows to mint not only during construction and without batchSize limit.
     */
    function _mintConsecutive(address to, uint96 batchSize) internal override returns (uint96) {
        uint96 first = _totalConsecutiveSupply();

        // minting a batch of size 0 is a no-op
        if (batchSize > 0) {
            if (to == address(0)) {
                revert ERC721MintBatchMintToZeroAddress();
            }
    ...
    function _mint(address /*to*/, uint256 /*tokenId*/) internal pure override {
        revert ERC721MintBatchSingleMintNotSupported();
    }
}
...
abstract contract ERC721Consecutive is IERC2309, ERC721 {
    ...
    // internal to access from child
    Checkpoints.Trace160 internal _sequentialOwnership;
    BitMaps.BitMap internal _sequentialBurn;
