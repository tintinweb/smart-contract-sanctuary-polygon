...
contract InWritingPromoCodes is Ownable{
    ...
    function get_price_with_promo(string memory promo) public view returns (uint256) {
        return (write.get_minting_cost() * (100 - uint256(promoDiscount[promo]))) / 100;
    }

    function get_price_with_affiliate(string memory promo) public view returns (uint256) {
        return (write.get_minting_cost() * (100 - uint256(affiliateDiscount[promo]))) / 100;
    }
    ...
    function mint_promo_NFT(string memory str, string memory promo) public payable returns (uint256) {
        ...
        uint256 cost = (write.get_minting_cost() * (100 - uint256(promoDiscount[promo]))) / 100;
    ...
    function mint_and_send_affiliate_NFT(string memory str, string memory promo, address addr) public payable returns (uint256) {
        ...
        uint256 cost = ((write.get_minting_cost() * (100 - uint256(affiliateDiscount[promo]))) / 100);
        ...
        payable(affiliateAddress[promo]).transfer(cost * uint256(affiliateCommission[promo]) / 100);
        if (msg.value > cost) {
            payable(msg.sender).transfer(msg.value - cost);
        }
