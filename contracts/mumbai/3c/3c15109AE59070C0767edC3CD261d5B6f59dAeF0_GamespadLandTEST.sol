...
contract GamespadLandTEST is ERC721Psi, AccessProtected, IERC721Receiver {
    ...
    function setCoordsToTokenId(uint256[] calldata _x, uint256[] calldata _y) external onlyAdmin {
        require(_x.length == totalSupply(), "_x Parameter length mismatch");
        require(_y.length == totalSupply(), "_y Parameter length mismatch");
        for(uint256 i = 0; i < totalSupply(); i++) {
            uint256 x = _x[i];
            uint256 y = _y[i];
            uint256 coords = formatCoordinates(x,y);
            require(_coordsToId[coords] == 0, "NFT for {x,y} already minted");
            _coordsToId[coords] = i;
            _idToCoords[i] = coords;
        }
    }
