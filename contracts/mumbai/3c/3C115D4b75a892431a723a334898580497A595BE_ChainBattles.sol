...
contract ChainBattles is ERC721URIStorage {
    ...
    function generateCharacter(uint256 tokenId) public view returns(string memory){
        ...
        bytes memory svg = abi.encodePacked(
            ...
            '<text x="50%" y="50%" class="base" dominant-baseline="middle" text-anchor="middle">', "Health: ",getHealth(tokenId),'</text>',
    ...
    function getHealth(uint256 tokenId) public view returns (string memory) {
        uint256 health = tokenIdToStats[tokenId].health;
    ...
    function mint() public {
        ...
        tokenIdToStats[newItemId].strength = 1;
        tokenIdToStats[newItemId].speed = 1;
        _setTokenURI(newItemId, getTokenURI(newItemId));
    }

    function getRandomNumber(uint256 number) internal view returns (uint) {
        return uint(keccak256(abi.encodePacked(msg.sender, block.difficulty, block.timestamp)))%number;
    }
    ...
    function train(uint256 tokenId) public {
        ...
        uint256 currentHealth = tokenIdToStats[tokenId].health;
        
        uint256 currentLevel = tokenIdToStats[tokenId].level;
        tokenIdToStats[tokenId].level = currentLevel + getRandomNumber(10);

        tokenIdToStats[tokenId].health = currentHealth + getRandomNumber(10);

        uint256 currentStrength = tokenIdToStats[tokenId].strength;
        tokenIdToStats[tokenId].strength = currentStrength + getRandomNumber(5);
        
        uint256 currentSpeed = tokenIdToStats[tokenId].speed;
        tokenIdToStats[tokenId].speed = currentSpeed + getRandomNumber(2);
