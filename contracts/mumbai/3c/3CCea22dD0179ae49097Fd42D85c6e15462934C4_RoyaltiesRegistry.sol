...
import "@akxlab/base/Royalties/LibRoyaltiesV2.sol";
import "@akxlab/base/Royalties/LibRoyaltiesV1.sol";
import "@akxlab/base/Royalties/LibRoyalties2981.sol";
import "@akxlab/base/Royalties/RoyaltiesV1.sol";
import "@akxlab/base/Royalties/RoyaltiesV2.sol";
import "@akxlab/base/Royalties/IERC2981.sol";
...
contract RoyaltiesRegistry is IRoyaltiesProvider, OwnableUpgradeable {
    ...
    function getRoyalties(address token, uint tokenId) override external returns (LibPart.Part[] memory) {
        ...
        //case royaltiesType = 2, royalties AKX v2
        if (royaltiesType == 2) {
            return getRoyaltiesAKXV2(token,tokenId);
        }

        //case royaltiesType = 3, royalties AKX v1
        if (royaltiesType == 3) {
            return getRoyaltiesAKXV1(token, tokenId);
        }
    ...
    /// @dev tries to get royalties AKX-v2 for token and tokenId
    function getRoyaltiesAKXV2(address token, uint tokenId) internal view returns (LibPart.Part[] memory) {
        try RoyaltiesV2(token).getAKXV2Royalties(tokenId) returns (LibPart.Part[] memory result) {
            return result;
        } catch {
            return new LibPart.Part[](0);
        }
    ...
    /// @dev tries to get royalties AKX-v1 for token and tokenId
    function getRoyaltiesAKXV1(address token, uint tokenId) internal view returns (LibPart.Part[] memory) {
