...
contract ERC20MerkleAirdrop is Ownable {
    ...
    function checkTime() private view {
        require(
            endAt == 0 || endAt >= block.timestamp,
            'airdrop already expired'
        );
        require(
            startAt == 0 || startAt <= block.timestamp,
    ...
    function claimLeftover(address account) public onlyOwner {
        ...
        require(
            IERC20(tokenAddress).transfer(account, leftOver),
            'transfer failed'
        );
    }
}