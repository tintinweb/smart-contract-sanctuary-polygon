...
contract Purchase is Ownable, ReentrancyGuard {
    ...
    function setRate(
        uint256 _underWritingRate,
        uint256 _surplusRate,
        uint256 _treasuryRate
    ) external onlyOwner {
        require(
            _underWritingRate.add(_surplusRate).add(_treasuryRate) == 100,
            "not equal 100"
        );
    ...
    function buyInsurance(
        ...
        require(
            divCurrencies[currency] != address(0) &&
                currency < divCurrencies.length,
            "this asset type is not supported"
        );

        if (divCurrencies[currency] == ETHEREUM) {
            require(msg.value == _premium, "not equal");
            IWETH(ETHEREUM).deposit{value: msg.value}();
            payable(underWriting).transfer(
                _premium.mul(underWritingRate).div(100)
            );
            payable(surplus).transfer(_premium.mul(surplusRate).div(100));
            payable(treasury).transfer(_premium.mul(treasuryRate).div(100));
        } else {
            IERC20(divCurrencies[currency]).safeTransferFrom(
                msg.sender,
                address(this),
                _premium
            );
            IERC20(divCurrencies[currency]).safeTransfer(
                address(underWriting),
                _premium.mul(underWritingRate).div(100)
            );
            IERC20(divCurrencies[currency]).safeTransfer(
                address(surplus),
                _premium.mul(surplusRate).div(100)
            );
            IERC20(divCurrencies[currency]).safeTransfer(
                address(treasury),
        ...
        IUnderwriting(underWriting).updateRewardPerFakt(
            _productId,
            _premium.mul(underWritingRate).div(100)
        );
