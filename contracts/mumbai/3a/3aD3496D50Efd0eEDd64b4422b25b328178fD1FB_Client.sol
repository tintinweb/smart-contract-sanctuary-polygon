...
contract Client is Initializable, Managerable {
    IGatekeeperBaseVerifier public _gatekeeper;
    ...
    function initialize(address gatekeeperAddress) external initializer {
        _gatekeeper = IGatekeeperBaseVerifier(gatekeeperAddress);
        requiredDepositClaimTopics.push(CLAIM_TOPIC_WALLET_SCREENING);

        requiredWithdrawalClaimTopics.push(CLAIM_TOPIC_KYC);
    ...
    function deposit() external payable mAddressNotZero(msg.sender) {
        require(
            _gatekeeper.isUserVerified(msg.sender, requiredDepositClaimTopics),
            "Lacking required claims"
        );
        _userBalance[msg.sender] += msg.value;
    }

    function withdraw() external mAddressNotZero(msg.sender) {
        require(
            _gatekeeper.isUserVerified(
                msg.sender,
                requiredWithdrawalClaimTopics
            ),
            "Lacking required claims"
        );
