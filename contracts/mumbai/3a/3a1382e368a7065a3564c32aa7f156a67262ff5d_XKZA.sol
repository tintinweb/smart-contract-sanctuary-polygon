...
/*
 * xKZA is Kinza's escrowed governance token obtainable by converting KZA
 * It's non-transferable, except from/to whitelisted addresses
 * It can be converted back to KZA through a vesting process
 */
contract XKZA is Ownable, ReentrancyGuard, ERC20("escrowed Kinza Token", "xKZA") {
  ...
  // uint256 public redeemVotingPower = 50; // 50%
  ...
  event Redeem(address indexed userAddress, uint256 xAmount, uint256 amount, uint256 duration);
  event FinalizeRedeem(address indexed userAddress, uint256 xAmount, uint256 amount);
  event CancelRedeem(address indexed userAddress, uint256 xAmount);
  ...
  /*
   * @dev returns redeemable KZA for "amount" of xGRAIL vested for "duration" seconds
   */
  function getKZAByVestingDuration(uint256 amount, uint256 duration) public view returns (uint256) {
   ...
   */
  function getUserRedeem(address userAddress, uint256 redeemIndex) external view validateRedeem(userAddress, redeemIndex) returns (uint256 amount, uint256 xAmount, uint256 endTime) {
  ...
  /**
   * @dev returns if "account" is allowed to send/receive xKZA
   */
   ...
   */
   ...
   function updateVoter(address newVoter) external onlyOwner {
    voter = IVoter(newVoter);
    emit NewVoter(newVoter);
   }
  function updateRedeemSettings(uint256 minRedeemRatio_, uint256 maxRedeemRatio_, uint256 minRedeemDuration_, uint256 maxRedeemDuration_) external onlyOwner {
    ...
    require(maxRedeemRatio_ <= 100, "updateRedeemSettings: wrong ratio values");
  ...
  /**
   * @dev Convert caller's "amount" of KZA to xKZA
   */
  ...
  /**
   * @dev Convert caller's "amount" of KZA to xKZA to "to" address
   */
  ...
  /**
   * @dev Initiates redeem process (xKZA to KZA)
   *
   * Handles dividends' compensation allocation during the vesting process if needed
   */
  function redeem(uint256 xAmount, uint256 duration) external nonReentrant {
    require(xAmount > 0, "redeem: xAmount cannot be null");
    require(duration >= minRedeemDuration, "redeem: duration too low");

    _transfer(msg.sender, address(this), xAmount);

    // get corresponding KZA amount
    uint256 amount = getKZAByVestingDuration(xAmount, duration);
    emit Redeem(msg.sender, xAmount, amount, duration);

    // it is possible that the minDuration is set to 0 in future
    ...
    if(duration > 0) {
      ...
      // add redeeming entry
      userRedeems[msg.sender].push(RedeemInfo(amount, xAmount, _currentBlockTimestamp().add(duration)));
    } else {
      // immediately redeem for KZA
      _finalizeRedeem(msg.sender, xAmount, amount);
    }
    ...
    if (address(voter) != address(0)) {
      ...
      // a sanity check to harden the token balance guaranteed for redeem;
  ...
  function _convert(uint256 amount, address to) internal {
    ...
    KZA.safeTransferFrom(msg.sender, address(this), amount);
    // mint new xToken
    _mint(to, amount);
   ...
   */
  function _finalizeRedeem(address userAddress, uint256 xAmount, uint256 amount) internal {
    uint256 excess = xAmount.sub(amount);

    // sends due KZA tokens
    KZA.safeTransfer(userAddress, amount);

    // burns KZA excess if any
    KZA.burn(excess);
    _burn(address(this), xAmount);

    emit FinalizeRedeem(userAddress, xAmount, amount);
  }
  ...
  function _beforeTokenTransfer(address from, address to, uint256 /*amount*/) internal view override {
    require(from == address(0) || _transferWhitelist.contains(from) || _transferWhitelist.contains(to), "xToken does not allow transfer");
  }
