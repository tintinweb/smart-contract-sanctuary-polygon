...
contract BTCLDynamicLottery is AutomationCompatible, AutomatedRandomness, ReentrancyGuard {
    ...
    function checkUpkeep(bytes calldata /* checkData */) external view override returns (bool upkeepNeeded, bytes memory performData) {
        ...
        if (upkeepNeeded) {
            uint256[] memory requestData = new uint256[](1);
            requestData[0] = nextRound;
            performData = abi.encode(requestData);
        }
    }

    function performUpkeep(bytes calldata performData) external nonReentrant override {
        uint256[] memory requestData = abi.decode(performData, (uint256[]));
        uint256 nextRoundToDraw = requestData[0];
        require(nextRoundToDraw >= nextRound && nextRoundToDraw < nextRound + maxRounds, "Invalid next round to draw");

        // Request VRF number for all rounds that need to be drawn
        uint256 numWords = maxRounds * totalWinners; // Request 3 numWords per round winner for all rounds
        ...
        for (uint256 i = 0; i < maxRounds; i++) {
            uint256 roundNr = nextRoundToDraw + i;

            if (rounds[roundNr].status.totalBets >= maxPlayers && rounds[roundNr].status.roundStatus == BTCLCore.Status.Open) {
