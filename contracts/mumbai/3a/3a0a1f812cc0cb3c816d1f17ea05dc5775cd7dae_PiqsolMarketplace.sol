...
// File: contracts/Final_PiqsolMarketplace.sol
...
contract PiqsolMarketplace is  ERC721Holder{
    //** Tax Listing **//
    uint256 listingTax;
    uint256 buyitemTax;
    uint256 biditemTax;
        ...
        FixedItemExists(id)
    {
        ...
        FixedPriceActiveItems[FixedItemsForSale[id].tokenAddress][
         ...
         IERC721(FixedItemsForSale[id].tokenAddress).safeTransferFrom(address(this), msg.sender, FixedItemsForSale[id].tokenId);
      ...
      
    {
     ...
     IERC721(AuctionItemsForSale[id].tokenAddress).safeTransferFrom(address(this), msg.sender, AuctionItemsForSale[id].tokenId);
    }
    ...
    function auctionEnd(uint256 id) public {
        ...
        if (AuctionItemsForSale[id].highestBidder == address(0)) {
            IERC721(AuctionItemsForSale[id].tokenAddress).safeTransferFrom(address(this),AuctionItemsForSale[id].beneficiary, AuctionItemsForSale[id].tokenId);
        } else {
            IERC721(AuctionItemsForSale[id].tokenAddress).safeTransferFrom(address(this),AuctionItemsForSale[id].highestBidder, AuctionItemsForSale[id].tokenId);
            if (_royaltiesRecipientAddress != address(0)) {
                uint256 royalityFee = calculateRoyaltyFee(
