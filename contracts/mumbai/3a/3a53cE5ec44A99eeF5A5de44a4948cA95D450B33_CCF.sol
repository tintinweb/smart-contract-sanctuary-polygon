...
contract CCF {
   ...
   // address constant BloodToken = 0x9c8919d6E97Be2B8B823FFfB699ef1cb3422C77F;
    address constant BloodToken = 0x0E0E66bEbE05F6078C32bB7B42Ce39fDF5Acf692;
...
// Allow _spender to withdraw from your account, multiple times, up to the _value amount.
     ...
     function approve(address _spender, uint256 _amount) public returns (bool success) {
         ERC20(BloodToken).approve(_spender, _amount);
         return true;
     }

     function IncreaseAllowance(address _spender, uint256 _amount) public returns (bool success) {
         ERC20(BloodToken).increaseAllowance(_spender, _amount);
         return true;
     }

     function allowance(address _owner, address _spender) public view returns (uint256 remaining) {
         return ERC20(BloodToken).allowance(_owner, _spender);
     }


    function deposit(address _From,address _To, uint256 amount) external {
        require(amount>0);

        balance[msg.sender] += amount;      
        ERC20(BloodToken).transferFrom(_From,_To,amount);
    }
    ...
    function withdraw(uint256 amount) external {
        ...
        balance[msg.sender] -= amount;
        ERC20(BloodToken).transfer(msg.sender, amount);
    }
