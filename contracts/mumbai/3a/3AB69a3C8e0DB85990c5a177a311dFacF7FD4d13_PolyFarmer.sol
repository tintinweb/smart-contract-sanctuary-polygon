...
contract PolyFarmer is ERC721A, Ownable, VRFConsumerBase {
  ...
  constructor(address _corn, address _metadata, address _vrf, address _link, bytes32 _keyHash, uint256 _fee) ERC721A("PolyFarmer", "PFARM") VRFConsumerBase(_vrf, _link) {
    corn = Corn(_corn);
    keyHash = _keyHash;
    fee = _fee;
  ...
  function freeMint() public {
    ...
    require(limitPerAddress[msg.sender] < 3, "Reached free mint limit");
    require(purchased < freeSupply, "Sold out");
  ...
  function purchase(uint256 amount) public {
    ...
    require(amount > 0, "Too many per tx");
  ...
  function fulfillRandomness(bytes32, uint256 randomness) internal override {
    ...
    while(true) {
      ...
      bytes32 hsh = keccak256(abi.encode(gender, skin, hair, clothes, mouth, eyes));
      if (!traitsTaken[hsh]) {
