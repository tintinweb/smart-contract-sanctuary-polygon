...
contract LOTTO {
    ...
    constructor(address payable _ownerAddress) {
        ...
        emit Register(lastId, msg.sender, address(0), block.timestamp);
        lastId++;
    }
    ...
    function buyFirstNft(address _referral) public payable {
        ...
        if (_referral != ownerAddress) {
            require(player[_referral].playerId>0, "Invalid referral");
    ...
    function buySecondNft() public payable {
        ...
        if (player[_referral].isUpgraded == true ) {
            ...
            if (player[_referral].vipReferrals % 5 > 0) {
                ...
                emit EarningEvent(secondNftBuyAmount, msg.sender, _referral,6, block.timestamp);
            }
            else {
