...
contract PrayerFlagsNFT is ERC721A, Ownable, ReentrancyGuard, ERC2981ContractRoyalties {
    
    //Boolean to control public sale
    bool private publicSaleIsOpen = false;

    //comparisons are strictly less than for gas efficiency.
    uint256 public constant MAX_SUPPLY = 1082; //1081

    uint256 public price = 200 ether;

    uint256 public max_per_wallet = 9; //10

    uint96 public royalties_points = 500; //5%
    ...
    constructor(string memory baseURI) ERC721A("PrayerFlags", "PRF") {
    ...
    function setBaseURI(string memory newTokenURI) public onlyOwner {
    ...
    /// @notice reserved mints to be minted by the team
    /// @param _receiver address that will receive the NFT
    /// @param _number of NFTs to be minted
    function reservedMints(address _receiver, uint256 _number) public onlyOwner nonReentrant {
        require(totalSupply() + _number < MAX_SUPPLY, "Not enough NFTs!");

        _mint(_receiver, _number);
    ...
    function setProjectAddress(address newprojectAdress1, address newProjectAddress2) external onlyOwner{
        projectAddress1 = newprojectAdress1;
        projectAddress2 = newProjectAddress2;
    }
    ...
    function withdraw() external onlyOwner nonReentrant {
        ...
        (bool sent1, ) = payable(projectAddress1).call{value: balance/2}("");
        (bool sent2, ) = payable(projectAddress2).call{value: balance/2}("");
...
/// @dev This is a contract used to add ERC2981 support to ERC721 and ERC115
