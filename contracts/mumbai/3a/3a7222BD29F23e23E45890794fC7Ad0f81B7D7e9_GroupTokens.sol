...
contract GroupTokens is ERC721Upgradeable, ERC721EnumerableUpgradeable, PausableUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    */
    function activateMultiSigLock(uint256 tokenID) public {
        ...
        require(multiSigLock[tokenID].lockedBy == tx.origin && !multiSigLock[tokenID].isLocked, "Not Multi-Sig Locking Address or already locked");

        // Lock the token
        multiSigLock[tokenID].isLocked = true;

        // Emit to the logs for external reference
        emit logTokenLocked(tx.origin, tokenID);
    }
    ...
    */
    function removeMultiSigLock(uint256 tokenID) public {
        ...
        // Make sure they're the owner
        require(multiSigLock[tokenID].lockedBy == tx.origin && multiSigLock[tokenID].isLocked, "Not MultiSig owner or not locked");
        ...
        // Emit to the logs for external reference
        emit logTokenUnlocked(tx.origin, tokenID);
    }


    /**
    * @dev Check if token is locked. Returns 0x0 if not locked. Only owner of token can check
