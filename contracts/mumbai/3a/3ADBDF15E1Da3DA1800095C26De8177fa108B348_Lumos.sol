    ...
    PausableUpgradeable
{
    ...
    event OnNFTTransfer(address nftContract, address indexed from, uint256 fromProfileId, address indexed to, uint256 toProfileId, uint256 tokenId);
    ...
    function onNFTTransfer(
        ...
        uint256 fromProfileId = IProfileNFT(profileNFT).profileOf(from);
        uint256 toProfileId = IProfileNFT(profileNFT).profileOf(to);
        emit OnNFTTransfer(msg.sender, from, fromProfileId, to, toProfileId, tokenId);
    }
}