...
contract NiftyCloneFactory is NiftyPermissions {
    ...
    function cloneERC721(
        ...
        require(
            IERC165(implementation).supportsInterface(
                type(IERC721Cloneable).interfaceId
            ),
            "Not a valid ERC721 Token"
        );
        ...
        if (
            IERC165(implementation).supportsInterface(
                type(INiftyEntityCloneable).interfaceId
            )
        ) {
            INiftyEntityCloneable(clone).initializeNiftyEntity(
                niftyRegistryContract_
            );
        }

        if (
            IERC165(implementation).supportsInterface(
                type(IDefaultOwnerCloneable).interfaceId
            )
        ) {
    ...
    function cloneERC1155(
        ...
        require(
            IERC165(implementation).supportsInterface(
                type(IERC1155Cloneable).interfaceId
            ),
            "Not a valid ERC1155 Token"
        );
        address clone = Clones.clone(implementation);
    ...
    function cloneMetadataGenerator(
        address implementation,
        address niftyRegistryContract_
    ) external returns (address) {
        _requireOnlyValidSender();
        require(
            ...
            "Not a valid Metadata Generator"
        );
        address clone = Clones.clone(implementation);
