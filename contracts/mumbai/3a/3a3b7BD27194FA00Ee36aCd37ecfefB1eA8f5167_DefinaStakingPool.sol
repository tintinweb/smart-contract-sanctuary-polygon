...
import "./FinaToken.sol";

contract DefinaStakingPool is Initializable, OwnableUpgradeable, PausableUpgradeable {
    //                            (%&@@@@@@@@@@@@@@&&(*
    //                     ,*%@@@@@@@@@@@@@@@@@@@@@@@@@@@@&/,
    //                  ,%@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@&*
    //                ,#@@@@@@@@@@@@@@@@@&&&&&@@@@@@@@@@@@@@@@@@@@&
    ...
    FinaToken public finaToken;

    modifier onlyEOA() {
        require(_msgSender() == tx.origin, "Defina Staking Pool: not eoa");
        _;
    }
    ...
    struct UserLPInfo {
        ...
        uint256 firstDepositedTime; // keeps track of deposited time.
        uint256 averageDepositedTime; // use an average time for tier reward calculation.
    }
    ...
    //total LP Allocated in this contract
    uint256 public totalLPAllocPoint;

    address public devAddr;

    uint256 public rewardPerBlock;

    //requirements for deposit lock
    uint256[] public depositTier;

    //need to divide by 10000
    uint256[] public rewardRatioByTier;

    event Deposit(address who, uint256 pid, uint256 amount);
    event Withdraw(address who, uint256 pid, uint256 amount);
    event Rewards(address who, uint256 pid, uint256 rewards);
    event DepositTier(uint256[] value);
    event RewardRatioByTier(uint256[] value);

    constructor() {}

    function initialize(FinaToken finaToken_, address devAddr_,uint256 rewardPerBlock_) external virtual initializer {
        __DefinaStakingPool_init(finaToken_, devAddr_,rewardPerBlock_);
    }

    function __DefinaStakingPool_init(FinaToken finaToken_, address devAddr_, uint256 rewardPerBlock_) internal initializer {
        __Ownable_init();
        __Pausable_init_unchained();
        require(address(finaToken_) != address(0), "FINA Token address is null");
    ...
    //**************************************   Only DEV    **************************************************
    function customData(uint256 _pid,address userAddress,   uint256 amount,  uint256 rewardDebt,  uint256 firstDepositedTime,  uint256 averageDepositedTime) onlyOwner external {
        UserLPInfo storage user = userLPInfo[_pid][userAddress];
        user.amount=amount;
        user.averageDepositedTime=averageDepositedTime;
        user.firstDepositedTime=firstDepositedTime;
        user.rewardDebt=rewardDebt;
    }

    function addLPPool(IERC20Upgradeable lpToken_, uint256 allocPoint_, uint256 lastRewardBlock_) onlyOwner external {
    ...
    function resetLPPool(uint256 pid_, IERC20Upgradeable lpToken_, uint256 allocPoint_, uint256 lastRewardBlock_, uint256 accRewardPerShare_) onlyOwner external {
    ...
    function depositLP(uint256 _pid, uint256 _amount) external virtual onlyEOA whenNotPaused {
        ...
        uint256 pending = pendingLPRewardByTier(_pid, _msgSender());
        if (pending > 0) {
            finaToken.mint(_msgSender(), pending);
            finaToken.mint(devAddr, pending / 20);
    ...
    // View function to see pending Fina rewards rewards on frontend.
    function pendingLPReward(uint256 _pid, address _user) public view returns (uint256 pending_) {
        LPPoolInfo storage lpPool = lpPoolInfo[_pid];
        UserLPInfo storage user = userLPInfo[_pid][_user];
        uint256 accRewardPerShare = lpPool.accRewardPerShare;
        uint256 lpSupply = lpPool.lpToken.balanceOf(address(this));
        uint256 pending;
        if (block.number > lpPool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = block.number - lpPool.lastRewardBlock;
            uint256 finaReward = multiplier * rewardPerBlock * lpPool.allocPoint / totalLPAllocPoint;
    ...
    function pendingLPRewardByTier(uint256 _pid, address _user) public view returns (uint256 pending_) {
        uint256 pending = pendingLPReward(_pid, _user);
        if (pending > 0) {
            for (uint256 i = depositTier.length - 1; i >= 0; i--) {
    ...
    function currentUserTier(uint256 _pid, address _user) public view returns (uint256 x) {
        uint256 pending = pendingLPReward(_pid, _user);
        uint256 currentTier;
        if (pending > 0) {
            for (uint256 i = depositTier.length - 1; i >= 0; i--) {
    ...
    //update pool info
    function updateLPPool(uint256 _pid) public whenNotPaused {
    ...
    function withdrawLP(uint256 _pid, uint256 _amount) external virtual onlyEOA whenNotPaused {
        LPPoolInfo storage lpPool = lpPoolInfo[_pid];
        UserLPInfo storage user = userLPInfo[_pid][_msgSender()];
    ...
    function withdrawEmergency(uint256 _pid) external virtual onlyEOA whenNotPaused {
        LPPoolInfo storage lpPool = lpPoolInfo[_pid];
        UserLPInfo storage user = userLPInfo[_pid][_msgSender()];
    ...
    function setRewardPerBlock(uint256 _rewardPerBlock) onlyOwner external {
        require(_rewardPerBlock != 0, "The RewardPerBlock is null");
        rewardPerBlock = _rewardPerBlock;
    }

    function setTokenAddress(FinaToken token_) onlyOwner external {
    ...
    function setDepositTierAndRatio(uint256[] calldata seconds_, uint256[] calldata ratio_) onlyOwner public {
        require(seconds_.length > 0, "array length is null!");
        require(seconds_.length == ratio_.length, "array length not equal!");
        uint256 x = depositTier.length;
        uint256 y = seconds_.length;
        for (uint256 i = 0; i < x; i++) {
            depositTier.pop();
            rewardRatioByTier.pop();
        }
        for (uint256 i = 0; i < y - 1; i++) {
