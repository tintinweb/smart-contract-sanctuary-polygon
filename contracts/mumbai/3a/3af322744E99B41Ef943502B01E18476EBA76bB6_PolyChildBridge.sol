...
// File: contracts/adamBridge/PolyChildBridge.sol
...
/**
 * @title PolyChildBridge
 */
contract PolyChildBridge is FxBaseChildTunnel, Create2, IERC721Receiver, Ownable {
    ...
    function _syncDeposit(bytes memory syncData) internal {
        (address rootToken, address depositor, address to, uint256 tokenId, string memory tokenURI, bytes memory depositData) = abi.decode(
            syncData,
            (address, address, address, uint256, string, bytes)
        );
        address childToken = rootToChildToken[rootToken];

        // deposit tokens
        IFxERC721 childTokenContract = IFxERC721(childToken);
        childTokenContract.mintWithURI(to, tokenId, tokenURI, depositData);
    }
