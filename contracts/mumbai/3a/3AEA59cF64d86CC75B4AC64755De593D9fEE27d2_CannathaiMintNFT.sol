...
contract CannathaiMintNFT is ERC721A, Ownable, ReentrancyGuard, AccessControl{
    ...
    mapping(address => bool) public earlyGreenListed;

    address[] public greenListedArray;
    address[] public earlyGreenListedArray;
    ...
    function mintEarlyPrivateSale(uint8 quantity) external payable nonReentrant whenPrivateSaleNotPaused{
        require(quantity + _numberMinted(msg.sender) <= MAX_MINTS, "Exceeded the limit");
        require(totalSupply() + quantity <= MAX_SUPPLY, "Not enough tokens left");
        require(msg.value >= earlyPresaleCost * quantity);
        require(earlyPrivateSaleCounters.current() <= MAX_PRIVATESALE_SUPPLY);
        require(quantity <= MAX_PRIVATESALE_MINTS, "Exceeded the Private Sale limit");
        require(earlyGreenListed[msg.sender] == true, "Address is not in early greenlist");
    ...
    function mintPublicSale(uint8 quantity) external payable nonReentrant whenPublicSaleNotPaused {
        ...
        if (msg.sender != owner()) {
            if (greenListed[msg.sender] != true) {
                ...
                require(publicSaleCounters.current() <= MAX_PUBLICSALE_SUPPLY, "Exceeded public sale supply");
                ...
                publicSaleCounters.increment();
                    ...
                    //presale
                    require(msg.value >= presaleCost * quantity, "Don't have enough money for green list sale");
                    require(privateSaleCounters.current() <= MAX_PRIVATESALE_SUPPLY, "Exceeded private sale supply");
                    require(quantity <= MAX_PRIVATESALE_MINTS, "Exceeded the Private Sale mint limit");
    ...
    function totalMint() public view returns (uint256) {
        uint256 total_mint = earlyPrivateSaleCounters.current() + privateSaleCounters.current() + publicSaleCounters.current();
        return total_mint ;
    }
    ...
    function addEarlyGreenListedUser(address _user) public onlyOwner {
        earlyGreenListed[_user] = true;
        earlyGreenListedArray.push(_user);
    }

    function removeEarlyGreenListedUser(address _user) public onlyOwner {
        earlyGreenListed[_user] = false;
        //earlyGreenListedArray[_user].pop();
    }

    function getEarlyGreenListedUser(address _user) public view returns (bool) {
        return earlyGreenListed[_user];
    }
