...
abstract contract IndradexBaseVault is Ownable, ERC20 {
   ...
   */
  function adminWithdrawToAddress(address token, uint256 amount, address receiver)
    external
    onlyOwner
    onlyPresentToken(token)
  {
  ...
  {
    PendingUserWithdrawal[] memory pendingUserWithdrawal = new PendingUserWithdrawal[](usersWithdraw[user].length());
  ...
  function getAllUsersWithdrawals()
    ...
    returns (PendingUserWithdrawal[] memory)
  {
    PendingUserWithdrawal[] memory pendingUserWithdrawal = new PendingUserWithdrawal[](pendingWithdrawID.length());
   ...
   * Internal function without access restriction.
   */
  function _setPerformanceFeePercentage(uint256 newPerformanceFeePercentage) internal {
    uint256 previousPerformanceFeePercentage = _performanceFeePercentage;
    _performanceFeePercentage = newPerformanceFeePercentage;
    emit PerformanceFeePercentageUpdated(previousPerformanceFeePercentage, newPerformanceFeePercentage);
  }
  ...
  function _feePayment(uint256 totalSupply_) internal {
    ...
    uint256 currentWatermark = _calculateCurrentWatermark(vaultTotalScaledValue_, totalSupply_);
    ...
    if (currentWatermark > previousHighWatermark) {
      ...
      uint256 fee = (currentWatermark - previousHighWatermark).mul(_performanceFeePercentage).mul(totalSupply_);
      uint256 mintAmount = fee * totalSupply_ / (vaultTotalScaledValue_ - fee);
   ...
   * Internal function without access restriction.
   */
  function _adminWithdraw(address token, uint256 amount, address receiver)
  internal virtual;
   ...
   */
  function _calculateCurrentWatermark(uint256 vaultTotalScaledValue_, uint256 totalSupply_) internal view returns (uint256) {
...
contract IndradexSingleExternalVault is IndradexBaseVault {
  ...
  function adminDeposit(address token, uint256 amount)
    ...
    onlyPresentToken(token)
    { 
      uint256 inputTokenExternalBalance_ = tokenExternalBalance(address(inputToken));
      ...
      setInputTokenExternalBalance(inputTokenExternalBalance_ - tokenAmountScaledValue_);
    }
    ...
    internal override
  {
    ...
    setInputTokenExternalBalance(tokenAmountScaledValue(token, amount) + tokenExternalBalance(address(inputToken)));
  }
  ...
  function tokenAmountScaledValue(address token, uint256 amount)
    ...
    returns (uint256)
  {
    return (amount * 10**(18 - tokenInfo[token].tokenDecimals)).mul(
              scaledChainlinkPrice(
