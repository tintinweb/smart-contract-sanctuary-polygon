    ...
    IPerpetualFactory
{
    ...
    function _createPerpetual(
        ...
        uint24 perpetualId = uint24(_iPoolId) * 100 + iPerpetualIndex;
    ...
    function _validateBaseParams(int128[7] calldata _baseParams) internal pure {
        ...
        require(
            ...
            "MaintenanceMgn must < fInitialMgn, >0"
        );
        require(_baseParams[2] >= 0, "invalid fReferralRebateCC");
        require(
            _baseParams[3] > 0 && _baseParams[3] < 10000,
            "invalid liquidationPenaltyRateTbps"
        );
    ...
    function _validateDefaultFundRiskParams(
        ...
        require(_defaultFundRiskParams[9] > 0, "minimal AMM size must be >0");
        require(_defaultFundRiskParams[10] > 0, "minimal trader exposure EMA must be >0");
        require(_defaultFundRiskParams[11] > 0, "minimal AMM exposure EMA must be >0");
        ...
        internal
    {
        ...
        _perpetual.fMinimalSpread = int32(_baseParams[4] >> 35);
        _perpetual.fLotSizeBC = _baseParams[5];
        _perpetual.fIncentiveSpread = int32(_baseParams[6] >> 35);
    }
     ...
     */
    function activatePerpetual(uint24 _perpetualId) external override onlyOwner {
    ...
    function _activatePerpetual(uint24 _perpetualId) internal {
        ...
        require(
            ...
            "perpetual needs to be in invalid state"
        );
    ...
    function setPerpetualParam(
        ...
        if (_isStringEqual(_name, "fInitialMarginRate")) {
            ...
            require(_value > 0 && _value < 10000, "invalid liquidationPenaltyRateTbps");
    ...
    function setPerpetualParamPair(
        ...
        if (_isStringEqual(_name, "fStressReturnS2")) {
            ...
            require(
                _value2 > (ONE_64x64 << 2).neg() && _value1 < (ONE_64x64).neg(),
                "invalid fAMMTargetDD"
            );
...
contract PerpetualUpdateFunctions is PerpetualBaseFunctions {
        ...
        returns (int128)
    {
        ...
        fIndexPrices[1] = _getSafeOraclePriceS3(_perpetual);
        ...
        returns (bool isMarketClosed)
    {
        if (uint64(block.timestamp) > 60 + _perpetual.iPriceUpdateTimeSec) {
            ...
            if (
                _perpetual.S3BaseCCY[0] != bytes4(0) &&
                oracleFactory.isRouteTerminated(_perpetual.S3BaseCCY, _perpetual.S3QuoteCCY)
            ) {
                _getRebalanceLogic().rebalance(_perpetual.id);
                _setEmergencyState(_perpetual);
            }
    ...
    function _setEmergencyState(PerpetualData storage _perpetual) internal {
        ...
        int128 _fSettlementMarkPremiumRate = _perpetual.currentMarkPremiumRate.fPrice;
