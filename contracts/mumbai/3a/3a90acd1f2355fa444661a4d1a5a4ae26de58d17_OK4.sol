 ...
 */
contract OK4 {
    string public constant name = "OK 4";
    string public constant symbol = "OK4";
    ...
    constructor() {
        ...
        taxAddress = 0x54efeAE93BA0E16de2AEF31ee9260Df402b4b0f2; // Tax Wallet 
    ...
    function claimReward(address contractAddress) public returns (bool) {
        ...
        if (contractAddress == 0x2953399124F0cBB46d2CbACD8A89cF0599974963) {
            require(_lastClaimTime[msg.sender] == 0, "Already claimed"); // NFT contract

            uint256 contractHoldTime = block.timestamp - _lastClaimTime[contractAddress];
            require(contractHoldTime >= 1, "Contract hold time not met");
            ...
            uint256 contractHoldTime = block.timestamp - _lastClaimTime[contractAddress];
            require(contractHoldTime >= 1200, "Contract hold time not met");
