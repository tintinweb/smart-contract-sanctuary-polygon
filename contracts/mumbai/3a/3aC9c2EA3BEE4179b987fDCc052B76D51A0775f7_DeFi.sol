...
contract DeFi is Ownable {
  ...
  uint256 private stakingRate = 15;
  ...
  function borrow(uint256 _amount, address _borrower) public onlyOwner {
    ...
    borrowing[_borrower].push(
      Borrower(_borrower, _amount, block.timestamp, 365, lendingRate)
    );
    // Minting pBorrow tokens to _borrower address
    pBorrow.mint(_borrower, _amount);
    // Minting additional token so later he can rebuy it
    pBorrow.mint(address(this), (_amount * (100 + lendingRate)) / 100);
