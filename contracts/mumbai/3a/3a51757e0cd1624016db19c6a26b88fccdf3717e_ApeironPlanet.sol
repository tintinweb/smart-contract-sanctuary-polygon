    ...
    UUPSUpgradeable
{
    ...
    function safeMint(
        ...
        if (parents.length == 2) {
            planetDataMap[tokenId].parentA = parents[0];
            planetDataMap[tokenId].parentB = parents[1];
            planetDataMap[tokenId].hasParent = true;
...
contract PlanetMeta {
    struct PlanetData {
        ...
        uint256 parentA; //parent token ids, 0: bred from zero if hasParent = false
        uint256 parentB; //parent token ids, 0: bred from zero if hasParent = false
        bool hasParent; 
