...
contract FunctionsConsumer is FunctionsClient, ConfirmedOwner {
  ...
  function createStory(
    ...
    requestToStoryNonce[reqId] = nonce;
    nonce++;
  }
  ...
  function continueStory(
    ...
    string[] memory args = new string[](3);
    args[0] = prompt;
    args[1] = stories[storyNonce].ipfs;
    args[2] = Strings.toString(storyNonce);
    stories[storyNonce].updating = true;
    bytes32 reqId = executeRequest(source, secrets, args, subscriptionId, gasLimit);
    requestToStoryNonce[reqId] = storyNonce;
  }
  ...
  function fulfillRequest(bytes32 requestId, bytes memory response, bytes memory err) internal override {
    ...
    uint256 storyNonce = requestToStoryNonce[requestId];
    Story storage s = stories[storyNonce];
    s.contribution++;
    s.functionFulfilled = true;
  }

  function fulfillApiRequest(uint256 storyNonce, string memory ipfsHash) public {
    Story storage s = stories[storyNonce];
    s.ipfs = ipfsHash;
    s.updating = false;
  }
