...
contract DEX is Ownable {
    ...
    uint256 public fee = 1010; // = 1% fee, 1007 = .3%
    uint256 public k;
 ...
 function price(uint256 xInput,uint256 xReserves,uint256 yReserves) public pure returns (uint256 yOutput) {
        ...
        return (numerator / denominator).mul(1010).div(1000); // 3988 / 23988 = 0.1622
    }
    ...
    // user sells NFT to AMM and receives eth
    // requires user to do approve or approveAll if selling more than 1
    function bulkSellNFT(uint256 _amount) public returns (uint256 ) {
    ...
    function buyNFT(uint256 amountToBuy) public payable  {
        require(msg.value == getPrice(amountToBuy), "didn't send correct eth amount");
   ...
   // adds a .003% fee
    function getPrice(uint256 _nfts) public view returns(uint256){
        ...
        // return (newY - address(this).balance).mul(1003).div(1000);                    // 1.02 / 49
    ...
    function getPrice2(uint256 _nfts) public view returns(uint256){
        require (getBalance() - _nfts >=1, "Not enough NFT liqudidity");
        uint256 currentY = k / getBalance();
        uint256 x = getBalance() - _nfts;   // 49
        uint256 newY = k  / x ;               // 50 / 49= 1.1
        // return (newY - address(this).balance).mul(1003).div(1000);                    // 1.02 / 49
        return (newY - currentY).mul(1010).div(1000);  
    }               
     ...
     */
    function deposit(uint256 _tokens) public payable onlyOwner returns (uint256) {
    ...
    */
    function withdraw(uint256 amount) public onlyOwner returns (uint256 eth_amount, uint256 token_amount) {
