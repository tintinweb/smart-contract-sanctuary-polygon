...
contract Auctionv3 is Initializable, OwnableUpgradeable {
    ...
    event onBuyOrBid(uint indexed auctionId, address imxNftAddress, uint256 price, uint256 buyDate, uint256 endSaleDate, uint tokenId, AuctionType auctionType);
    ...
    */
    ...
    uint public constant fifteenMinute = 15*60;
    ...
    modifier checkForBuyAuction(Auctions storage _auc, uint256 tokenId) {
        require(
            _auc.startTime < block.timestamp && _auc.endTime > block.timestamp && checkTokenIdExist(_auc, tokenId)
        );
        _;
    }

    modifier checkForBidAuction(Auctions storage _auc, uint256 _tokenId) {
        require(
            _auc.startTime < block.timestamp && _auc.endTime +  _auc.tokenBid[_tokenId].length * TimeExtendOnBid > block.timestamp && checkTokenIdExist(_auc, _tokenId)
        );
        _;
    }
    ...
    function placeBid(
        ...
        checkForBidAuction(auctionsDetails[auctionId], tokenId)
    {
        ...
        _auc.largestBidLength = _auc.largestBidLength < bidsLength? bidsLength : _auc.largestBidLength;

        emit onBuyOrBid(auctionId, _auc.imxNftAddress, amt, block.timestamp,_auc.endTime + bidsLength * TimeExtendOnBid, tokenId, _auc.auctionType);
    }
        ...
        checkForBuyAuction(auctionsDetails[auctionId], tokenId)
    {
        ...
        emit onBuyOrBid(auctionId, _auc.imxNftAddress, _auc.price, block.timestamp, block.timestamp, tokenId, _auc.auctionType);
    }

    function endAuction(uint256 auctionId) public onlyOwner {
        Auctions storage _auc = auctionsDetails[auctionId];
        require(
            !_auc.isEnded && _auc.endTime + _auc.largestBidLength * TimeExtendOnBid < block.timestamp,
    ...
    function endMultipleAuctions(uint256[] memory auctionIds) external onlyOwner {
        ...
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < auctionsDetails.length; i++) {
            if (auctionsDetails[i].endTime + auctionsDetails[i].largestBidLength * TimeExtendOnBid <= block.timestamp && !auctionsDetails[i].isEnded)  {
                size++;
            }
        }
        ...
        )
    {
     ...
     function checkTokenIdExist(Auctions storage _auc, uint256 tokenId) internal view returns (bool) {
        for(uint256 i = 0; i < _auc.tokenIds.length; i++) {
    ...
    function withdrawAnonymousToken(IERC20Upgradeable token, uint256 amount) external onlyOwner {
