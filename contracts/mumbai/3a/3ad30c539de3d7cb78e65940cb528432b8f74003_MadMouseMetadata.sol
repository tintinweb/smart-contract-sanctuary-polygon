...
// MMMMMMMMMMMMMMMMMMMMMMMMMMMMMMM lMMMMMMMMMMMMMMMMM]._  MMMMMMMMMMMMMMMNMMMMMMMMMMMMMM
...
// MMMMMMMMMMMMMMMMMMMM MMMMMMMM,  MM  MMMMB m  ]MMM  MMMM   MMMMMM MMMMMMMMMMMMMMMMMMMM
...
contract MadMouse is Ownable, MadMouseStaking, VRFBase {
    ...
    function reveal(string memory _imagesBaseURI, string memory secretSeed_)
        external
        payable
        onlyOwner
        whenRandomSeedSet
    {
        ...
        if (secretHash != keccak256(bytes(secretSeed_))) revert IncorrectHash();

        revealed = true;
        imagesBaseURI = _imagesBaseURI;
        _shiftRandomSeed(uint256(bytes32(bytes(secretSeed_))));
    }
