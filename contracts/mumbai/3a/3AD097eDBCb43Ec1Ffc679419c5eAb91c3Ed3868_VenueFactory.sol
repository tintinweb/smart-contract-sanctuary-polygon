...
contract EventReviewPaymaster is IEventReviewPaymaster, BasePaymaster {
    ...
    function _preRelayedCall(
        ...
        if (_balanceByVenue[venue] == 0) revert ReviewPaymasterVenueNotBalance();
        ...
        if(stars > 5) revert ReviewPaymasterWrongNumberOfStars();
...
contract VenueFactory is IVenueFactory, Ownable {
    ...
    function setRoyaltyReceiver(address royaltyReceiver) external virtual override onlyOwner {
        _royaltyReceiver = royaltyReceiver;
    }

    /// @inheritdoc IVenueFactory
    function setRoyaltyFeeNumerator(uint96 royaltyFeeNumerator) external virtual override onlyOwner {
        _royaltyFeeNumerator = royaltyFeeNumerator;
    }

    /// @inheritdoc IVenueFactory
    function setRoyaltyFeeNumeratorSecondarySales(uint96 royaltyFeeNumeratorSecondarySales) external virtual override onlyOwner {
        _royaltyFeeNumeratorSecondarySales = royaltyFeeNumeratorSecondarySales;
    }

    /// @inheritdoc IVenueFactory
    function setSplitterFoBShares(uint96 splitterFoBShares) external virtual override onlyOwner {
        _splitterFoBShares = splitterFoBShares;
    }

    /// @inheritdoc IVenueFactory
    function setRegistar(address registar) external virtual override onlyOwner {
        _registar = registar;
    }
    ...
    function deployVenue(
        ...
        if(msg.sender != _registar) revert VenueFactorySenderNotRegistar();
...
contract VenueSBT is IVenueSBT, ERC721A, IERC721Receiver, Ownable, AccessControl {
    ...
    address private constant _WETH = 0xA6FA4fB5f76172d178d61B04b0ecd319C5d1C0aa;
    address private constant _WMATIC = 0x9c3C9283D3e44854697Cd22D3Faa240Cfb032889;
    ISwapRouter private constant _SWAP_ROUTER = ISwapRouter(0xE592427A0AEce92De3Edee1F18E0157C05861564);
    ...
    uint96 private _owedTokenReviews;
    ...
    function withdrawToken(address token) external virtual override onlyOwner {
        ...
        (, address paymaster, uint256 gasPerReview) = IVenueRegistar(_venueRegistar).getTokenReviewParams();

        IERC20 tokenContract = IERC20(token);
        uint256 balance = tokenContract.balanceOf(address(this));
        uint256 owedTokenReviews = _owedTokenReviews;
        if (paymaster != address(0) && owedTokenReviews > 0) {
            uint256 totalGas = gasPerReview * owedTokenReviews;
            uint256 totalFund = tx.gasprice * totalGas;

            EventReviewPaymaster reviewEventPaymaster = EventReviewPaymaster(payable(paymaster));

            if (token != _WMATIC) {
                tokenContract.approve(address(_SWAP_ROUTER), balance);
                ...
                totalFund = _SWAP_ROUTER.exactOutputSingle(params); //_WMATIC
                
                tokenContract.transfer(_msgSender(), tokenContract.balanceOf(address(this)));
            }

            IWMATIC(_WMATIC).withdraw(totalFund);
            reviewEventPaymaster.fundVenue{value: totalFund}(address(this));
            _owedTokenReviews = 0;
        } else {
    ...
    function onERC721Received(
        ...
        //TODO: check the incoming token comes from tokenReview and add 1 to owedTokenReviews, _owedTokenReviews++
        (, address paymaster, ) = IVenueRegistar(_venueRegistar).getTokenReviewParams();
        if (_msgSender() == paymaster) {
            _owedTokenReviews += 1;
        }
