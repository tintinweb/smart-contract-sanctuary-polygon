...
contract Staking is Ownable, ReentrancyGuard, Pausable {
     ...
     * @param _tokenAmount - Amount of token to be added to pool reward.
     */
    function addPoolReward(address _ownerAddress, uint256 _tokenAmount) external onlyOwner {
        IERC20 token = IERC20(tokenAddress);
        token.safeTransferFrom(_ownerAddress, address(this), _tokenAmount);
        poolReward += _tokenAmount;

        emit PoolRewardAdded(_tokenAmount);
    }
        ...
        nonReentrant
    {
        require(
            ...
            "[Staking.stakeToken] total stake amount should be less than user limit"
        );
        require(
            _tokenAmount + poolStakeTotal <= poolStakeLimit,
            "[Staking.stakeToken] total stake amount should be less than pool limit"
        );
        IERC20 token = IERC20(tokenAddress);
    ...
    function withdrawToken() external afterStakeEnded nonReentrant {
        ...
        require(
            0 == unclaimAmount,
            "[Staking.withdrawToken] unclaim amount should be zero before user can withdraw"
        );
     ...
     * @param _tokenAmount - Amount of token to be claimed.
     */
    function claimReward(uint256 _tokenAmount)
        external
        afterStakeStarted
        nonReentrant
    {
        uint256 unclaimAmount = getUserUnclaimAmount(msg.sender);
        require(
            _tokenAmount <= unclaimAmount,
            "[Staking.claimReward] claim amount should be less than user unclaim amount"
        );
        userInfo[msg.sender].claimedAmount += _tokenAmount;
        distributedReward += _tokenAmount;
        safeTokenTransfer(msg.sender, _tokenAmount);

        emit RewardClaimed(msg.sender, _tokenAmount);
    }
