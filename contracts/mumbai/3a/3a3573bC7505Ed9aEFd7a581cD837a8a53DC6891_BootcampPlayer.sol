    ...
    FxBaseChildTunnel
{
  ...
  constructor(address _playerOracle, address _fxChild, string memory _baseImageURI) ERC721("MPLBootcamp", "BOOT") FxBaseChildTunnel(_fxChild) {
  ...
  // @notice claim an array of players and send to 
  ...
  function customise(uint256 tokenId, Stats calldata stats) public {
    ...
    require(!hasStats(tokenId), "ALREADY_CUSTOMISED");
  ...
  function getPlayerStatsArr(uint256 tokenId) public view returns (uint8[9] memory p){
  ...
  function getStatsString(uint tokenId) internal view returns(string memory s){
    ...
    for(uint i; i < 9; i++){
      // Boots are returned as strings, not numbers
      if(i == 7) {
      s = string(abi.encodePacked(s,loadTraitType(attributeTypes[i + 5], statsArr[i].toString(), false)));
      } else if(i == 8) {
      // hard coding the final entry in the attributes array so we don't have to worry about the trailing comma
      s = string(abi.encodePacked(s,'{"trait_type": "Boots","value": "',statsArr[i].toString(),'"}'));
      } else {
      s = string(abi.encodePacked(s,loadTraitType(attributeTypes[i + 5], statsArr[i].toString(), true)));
      }
    }
    return s;
  }
  ...
  function getMetadataString(uint tokenId) internal view returns(string memory){
    ...
    return string(abi.encodePacked(
      loadTraitType('Player Type', playerTypes[metadata.playerType], false),
      loadTraitType('Skill Level', levelNames[metadata.skillLevel], false),
      loadTraitType('Quirks', quirks[metadata.quirks], false),
      loadTraitType('Pet', metadata.pet ? 'has_pet' : 'no_pet', false),
      loadTraitType('Team', teamNames[getTeam(tokenId)], false)
      ));
  }

  function loadTraitType(string memory _type, string memory _value, bool isNum) internal pure returns(string memory){
    if(isNum){
  ...
  function tokenURIJson(uint256 id) internal view returns (string memory) {
        ...
        string memory description = string(abi.encodePacked('A player in the inaugural MPL Bootcamp.'));
        string memory image = string(abi.encodePacked(baseImageURI, getPlayerId(id).toString()));
