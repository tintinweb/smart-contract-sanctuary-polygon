...
contract PledgeStakeV2 {
    ...
    function _calRoundRewards(address _user, uint256 _amount) private view returns (uint256) {
        ...
        uint256 round = (block.timestamp - stakeStartTime) / roundDuration;
        uint256 roundEndTime = stakeStartTime + roundDuration * (round + 1);
    ...
    function _updateRewards(address _user, uint256 _amount) private {
        ...
        uint256 roundRewards = _calRoundRewards(_user, _amount);
        rewards[_user].roundRewards = rewards[_user].roundRewards.add(roundRewards);
        rewards[_user].totalRewards = rewards[_user].totalRewards.add(roundRewards);
        rewards[_user].availableRewards = rewards[_user].availableRewards.add(roundRewards);
    }
