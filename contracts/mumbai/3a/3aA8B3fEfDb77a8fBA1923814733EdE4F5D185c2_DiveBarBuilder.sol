...
abstract contract ReliefDAOBuilder is AbstractDynamicURI, BaseViciContract {
    ...
    function tokenURI(uint256 tokenId) public view returns (string memory) {
        CandidateInfo memory candidate = oracle.candidateNominatedBy(
            tokenId
        );
    ...
    function logo() internal virtual view returns (string memory) {
        return
            svg.g(
    ...
    function getExtraLogoPaths() internal virtual view returns (string memory paths) {
        paths = "";
        for (uint256 i = 0; i < extraLogoPaths.length; i++) {
            paths = string.concat(svg.path(extraLogoPaths[i], ""));
        }
    }
    ...
    function setExtraLogoPaths(string[] memory _logoPaths) public onlyURIManager {
...
// File: DiveBarBuilder.sol

/**
 * @title Dive Bar DAO Dynamic Content Contract
 */
contract DiveBarBuilder is ReliefDAOBuilder {
    constructor(INominator _tokens, IReliefDaoOracle _oracle)
        ReliefDAOBuilder(_tokens, _oracle)
    {}

    function _initDefaults() internal override {
        regularURL = "https://divebardao.com/";
        nominationURL = "https://divebardao.com/nominate";

        logoElement
            .dPath = "M57.508,31.132c0,4.55-.758,8.729-2.275,12.535-1.517,3.807-3.649,7.083-6.396,9.83-2.747,2.747-6.053,4.88-9.916,6.396-3.863,1.517-8.156,2.275-12.878,2.275H2.39V.096H26.043c4.722,0,9.015,.766,12.878,2.297,3.863,1.531,7.169,3.664,9.916,6.396,2.747,2.733,4.879,6.003,6.396,9.809,1.517,3.806,2.275,7.984,2.275,12.535Zm-11.848,0c0-3.405-.451-6.46-1.352-9.165-.902-2.705-2.204-4.994-3.906-6.869-1.703-1.874-3.764-3.312-6.182-4.314-2.419-1.001-5.144-1.502-8.177-1.502H13.98V52.981h12.062c3.033,0,5.759-.5,8.177-1.502,2.418-1.001,4.479-2.44,6.182-4.314,1.702-1.874,3.005-4.164,3.906-6.869,.902-2.704,1.352-5.759,1.352-9.165Z";
        logoElement
            .aPath = "M117.249,62.168h-8.929c-1.002,0-1.818-.25-2.447-.751-.63-.5-1.102-1.123-1.417-1.867l-4.636-12.663h-25.713l-4.636,12.663c-.229,.659-.673,1.26-1.331,1.803-.659,.544-1.474,.816-2.447,.816h-9.015L81.105,.096h11.762l24.382,62.072Zm-20.39-23.438l-7.555-20.648c-.372-.915-.759-2.003-1.159-3.262-.401-1.259-.802-2.619-1.202-4.078-.372,1.46-.759,2.827-1.159,4.1-.401,1.274-.787,2.383-1.159,3.327l-7.512,20.562h19.746Z";
        logoElement
            .oPath = "M145.795,0c-17.156,0-31.064,13.908-31.064,31.064s13.908,31.064,31.064,31.064,31.064-13.908,31.064-31.064S162.951,0,145.795,0Zm10.518,21.681c-2.713,3.552-9.137,10.794-9.646,14.215-.431,2.894-.313,6.473,.047,8.384,.062,.33,.403,.572,.824,.587,2.72,.095,4.729,.426,4.729,.82,0,.468-2.835,.847-6.332,.847s-6.331-.379-6.331-.847c0-.394,2.008-.725,4.728-.82,.421-.015,.761-.257,.824-.587,.361-1.911,.478-5.49,.047-8.384-.509-3.421-6.933-10.663-9.646-14.215-.043-.056-.044-.122-.003-.18,.041-.057,.113-.09,.196-.09h15.517l.728-1.099c-.681-.478-.815-1.412-.268-2.239,.548-.827,1.595-1.271,2.48-1.101l.89-1.344c.115-.174,.386-.241,.605-.15,.219,.091,.305,.305,.19,.479l-.912,1.376c.583,.498,.672,1.373,.156,2.152-.516,.78-1.477,1.219-2.326,1.124l-.53,.801h3.841c.083,0,.155,.033,.196,.09,.041,.058,.04,.123-.003,.18Z";
        logoElement.logoTransform = "translate(352 130) scale(2.1)";
        logoElement.logoTextY = "335";
        logoElement.logoTextClass = "logo";
        logoElement.logoText = "Dive Bar DAO";
        ...
        noNomElement.nomAskLinkText = "https://divebardao.com/nominate";
        noNomElement.nomAskSuffix = "";
        noNomElement.nomAskLine2 = "to nominate a dive bar";
    ...
    function getExtraLogoPaths() internal override view returns (string memory) {
        return string.concat(
            super.getExtraLogoPaths(),
            svg.line(
                Point("151.694", "21.666"),
                Point("147.649", "27.688"),
                'class="cls-7"'
            )
        );
    }
