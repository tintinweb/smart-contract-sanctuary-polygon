...
// File: contracts/Reopen/Router.sol
...
// File: contracts/Reopen/Auction.sol
...
contract Auction is Initializable, OwnableUpgradeable {
    ...
    address public reopen = 0xe583327E8D32184aA21475f98c76c6900aB40a17;

    // Modifiers
    modifier contractActive() {
        require(SeasonDataMapping[Season].auctionState == 2, "Not Active");
        require(
    ...
    function initialize(
        ...
        SeasonDataMapping[Season].projectPercent = projectPercent_;
    }

    function approveAuction() public {
        uint8 reopenCount;
        uint8 operatorCount;
        if(msg.sender == reopen && reopenCount < 1) {
            reopenCount++;
            SeasonDataMapping[Season].auctionState++;
        }
        if(msg.sender == operator && operatorCount < 1) {
            operatorCount++;
            SeasonDataMapping[Season].auctionState++;
        }
    }
    ...
    function selectWinners() public payable onlyOperator {
        ...
        for (
            ...
            nft.createToken(1,allNFTWinners[i - SeasonDataMapping[Season].lastTokenId].bidder);
    ...
    function newSeason(uint256 items_, uint256 auctionStart_, uint256 auctionEnd_)
        public
        returns (uint256)
    {
        SeasonDataMapping[Season].items =
    ...
    function checkAuctionState() public view returns (string memory) {
        if(block.timestamp > SeasonDataMapping[Season].auctionStart && block.timestamp < SeasonDataMapping[Season].auctionEnd && SeasonDataMapping[Season].auctionState == 2) {
            return "Active";
        }
    ...
    function withdraw() internal onlyOwner {
        payable(routerContract).transfer(address(this).balance);
    }
}