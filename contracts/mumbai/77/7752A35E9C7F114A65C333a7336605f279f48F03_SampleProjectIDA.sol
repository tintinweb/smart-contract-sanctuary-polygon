...
contract SampleProjectIDA {
    ...
    uint128 private unitsOwnerPer100;

    constructor(ISuperToken _spreaderToken, address _projectOwner, uint128 _unitsOwner, address plasmaDAO, uint128 unitsPlasmaDAO) {
        spreaderToken = _spreaderToken;
        unitsOwnerPer100 = _unitsOwner;
        ...
        setShares(plasmaDAO, unitsPlasmaDAO);
        // With every mint, plasmaDAO units power will be decreased by 1 until reach 0 is al
    }
    ...
    } // TODO: waiting for Constant Distribution Agreement
    ...
    function changeSharesReceiver(address from, address to) external {
        if (from == address(0)) {
            // Minted a new NFT. Adding a new subscriber
            /* Get total units
            (, , uint128 unitsApproved, uint128 unitsPending) = spreaderToken.getIndex(
                address(this),
                INDEX_ID
            );
            */
            ...
            setShares(to, 1);
            // the percentage of shares owned by the owner should be constant
            setShares(projectOwner, currentUnitsOwner + (unitsOwnerPer100 / (100 - unitsOwnerPer100)));
        } else {
            // Get current units holds by from
