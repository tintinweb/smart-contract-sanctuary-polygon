...
contract StakingAlgo2 is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    struct UserInfo {
    ...
    uint256 public depositAdminFeePc;
    uint256 public depositorFeePc;
    ...
    function allTokenWithdraw() external onlyOwner {
        ...
        rewardToken.transfer(admin, rewardToken.balanceOf(address(this)));
    }
    ...
    function deposit(uint256 _amount) external nonReentrant checkActive {
        ...
        if (_amount < _vip1MinAmount) {
            ...
            _adminFee = (_amount * depositAdminFeePc) / _DENOMINATOR;
            depositToken.transfer(admin, _adminFee);

            _depositorFee = (_amount * depositorFeePc) / _DENOMINATOR;
        ...
        int256 _updatedPayouts = (int256)(profitPerShare * _actualAmount) -
            int256(_dividends);
    ...
    function claim() external nonReentrant checkActive {
        ...
        userInfo[msg.sender].depositorFeePayouts += (int256)(
            _depositorFee * MAGNITUDE
        );
        ...
        returns (uint256)
    {
        return
            (uint256)(
                (int256)(profitPerShare * userInfo[_user].depositedAmount) -
                    userInfo[_user].depositorFeePayouts
            ) / MAGNITUDE;
    }
