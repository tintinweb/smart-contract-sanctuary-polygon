...
contract MultiChainDeployer is IAxelarExecutable  {
    ...
    function multiDeploy(bytes memory bytecode, bytes32 salt, string[] memory chainNames, address[] memory destinationAddresses) external returns (address deployedAddress_) {
        bytes memory payload = abi.encode(bytecode, salt, msg.sender);
    ...
    function execute(
        ...
        (bytes memory bytecode, bytes32 salt, address sender) = abi.decode(payload, (bytes,bytes32,address));
        address deployedAddress_ = this.deploy(bytecode, salt, sender);
