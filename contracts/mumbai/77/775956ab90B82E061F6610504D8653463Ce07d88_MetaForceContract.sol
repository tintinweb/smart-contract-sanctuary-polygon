...
contract MetaForceContract is Ownable, ReentrancyGuard, IMetaForceContract {
    ...
    function buyMFS(uint256 amountMFS) external override nonReentrant {
        ...
        metaPayment.decreaseBalance(address(stableCoin), senderId, metaPool, amountToPay);
        core.giveMFSFromPool(senderId, amountMFS);
        core.increaseTotalEmission(amountMFS);
    }

    function firstActivationPack(uint256 marketinReferrerId) external override {
    ...
    function firstActivationPackWithReplace(uint256 replaceId) external override {
        ICoreContract core = ICoreContract(registry.getCoreContract());
        uint256 senderId = getUserId(msg.sender);
        if (core.getMarketingReferrer(senderId) != 0) {
            revert MFCNotFirstActivationPack();
    ...
    function renewalPackForMFS(uint256 level, uint256 amount) external override {
        uint256 senderId = getUserId(msg.sender);
        ...
        if (core.getWorkflowStage() == Workflow.SaleOpen) {
            ...
            holding.holdOnBehalf(senderId, 1, priceRenewalPackInMFS * amount);
        } else {
        ...
        metaPayment.decreaseBalance(
            tempRegistry.getMFS(),
            senderId,
            tempRegistry.getMetaPool(),
            priceRenewalPackInMFS * amount
        );
        _giveEnergy(senderId, (priceRenewalPackInMFS * amount).mul(core.getEnergyConversionFactor()));
        distributionRewardReferal(senderId, level, priceRenewalPackInMFS * amount, priceRenewalPackInUSD * amount);
        renewalPack(level, amount, senderId);
    }
    ...
    function activationPack(uint256 level) public override nonReentrant {
        ...
        IMetaPayment metaPayment = IMetaPayment(tempRegistry.getMetaPayment());
        uint256 senderId = getUserId(msg.sender);
        uint256 pricePackInUSD = PRICE_PACK_LEVEL1_IN_USD.mul(
            uint256(SCALE * 2).pow((level - 1).convertIntToFixPoint())
        );
        uint256 pricePackInMFS;
        DatesBuyingMFS memory purchase;
        pricePackInMFS = core.calcMFSAmountForUSD(pricePackInUSD);
        address metaPool = tempRegistry.getMetaPool();
        ...
        if (core.getWorkflowStage() != Workflow.SaleOpen) {
            metaPayment.decreaseBalance(address(stableCoin), senderId, metaPool, pricePackInUSD);
            ...
            datesForBuying[senderId].push(purchase);
            core.increaseTotalEmission(pricePackInMFS);
        } else {
            metaPayment.decreaseBalance(address(mfs), senderId, metaPool, pricePackInMFS);
        }

        _giveEnergy(senderId, pricePackInMFS.mul(core.getEnergyConversionFactor()));
        core.setTimestampEndPack(senderId, level, block.timestamp + PACK_ACTIVITY_PERIOD);
        distributionRewardReferal(senderId, level, pricePackInMFS, pricePackInUSD);
        emit MFCPackIsActivated(senderId, level, block.timestamp + PACK_ACTIVITY_PERIOD);
    }
    ...
    function renewalPack(
        ...
        while (tempAmount != 0) {
            ...
            if (countRenewal[senderId][level] < HMFS_COUNT - 1) {
                ...
                renewalPack(level, tempAmount, senderId);
                tempAmount = 0;
            }
        }
        _giveEnergy(senderId, (priceRenewalPackInMFS * amount).mul(core.getEnergyConversionFactor()));
    ...
    function renewalPack(
        ...
        emit MFCPackIsRenewed(senderId, level, core.getTimestampEndPack(senderId, level));
    }
    ...
    function distributionRewardReferal(
        ...
        uint256 tempAmountUSD = amountUSD;
        {
            ...
            for (uint256 i; i < rewardsRefers.length; ) {
                ...
                if (typeReward == TypeReward.ONLY_MFS || core.getWorkflowStage() == Workflow.SaleOpen) {
                    ...
                    emit RevenueMFS(refererId, userId, dRewardMFS / 2);
                    emit RevenueStable(refererId, userId, dRewardUSD / 2);
                }
                unchecked {
                    ++i;
                }
        ...
        for (uint256 i; i < rewardsMarketingRefers.length; ) {
            ...
            while (true) {
                ...
                if (core.isPackActive(marketingRefererId, level)) {
                    break;
                }
                emit LostMoney(marketingRefererId, userId, mRewardUSD);
            }
            ...
            typeReward = core.getTypeReward(marketingRefererId);
            if (typeReward == TypeReward.ONLY_MFS || core.getWorkflowStage() == Workflow.SaleOpen) {
    ...
    function realizeMFS(uint256 amount) internal {
        ...
        IMFS mfs = IMFS(tempRegistry.getMFS());
        mfs.burn(tempRegistry.getMetaPool(), requestContract.realizeMFS(amount));
    }
