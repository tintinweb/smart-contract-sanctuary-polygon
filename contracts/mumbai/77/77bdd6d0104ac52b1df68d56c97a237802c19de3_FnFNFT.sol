...
import {IFNFNFT} from "./interfaces/IFNFNFT.sol";
import {Ownable} from "@openzeppelin/contracts/access/Ownable.sol";
import {ReentrancyGuard} from "@openzeppelin/contracts/security/ReentrancyGuard.sol";
import {SafeMath} from "@openzeppelin/contracts/utils/math/SafeMath.sol";

contract FnFNFT is ERC721URIStorage, IFNFNFT, Ownable, ReentrancyGuard {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIds;

    constructor() ERC721("FnFNFT", "FFN") Ownable() {}

    function mint(address to, string memory tokenURI) public override onlyOwner nonReentrant returns (uint256) {
        _tokenIds.increment();
        uint256 newTokenId = _tokenIds.current();
        _mint(to, newTokenId);
        _setTokenURI(newTokenId, tokenURI);
        emit MintTo(to, newTokenId, tokenURI);
        return newTokenId;
    }

    function mintTo(address to, string memory tokenURI) public override onlyOwner nonReentrant returns (uint256) {
        _tokenIds.increment();
        uint256 newTokenId = _tokenIds.current();
        _mint(to, newTokenId);
        _setTokenURI(newTokenId, tokenURI);
    ...
    function mintMultiple(uint256 numTokens, string memory tokenURI)
        public
        override
        onlyOwner
        nonReentrant
    ...
    {
        uint256[] memory mintedTokens = new uint256[](numTokens);
        for (uint256 i = 0; i < numTokens; i++) {
            mintedTokens[i] = mint(msg.sender, tokenURI);
        }
        emit MintMultiple(msg.sender, numTokens);
        return mintedTokens;
    }
    ...
    function transfer(address to, uint256 tokenId) public override nonReentrant {
...
interface IFNFNFT {
    ...
    */
    event MintTo(address indexed to, uint256 indexed tokenId, string tokenURI);
    event MintMultiple(address indexed to, uint256 numTokens);

    function mint(address to, string memory tokenURI) external returns (uint256);
    function mintTo(address to, string memory tokenURI) external returns (uint256);
    function mintMultiple(uint256 numTokens, string memory tokenURI) external returns (uint256[] memory);
