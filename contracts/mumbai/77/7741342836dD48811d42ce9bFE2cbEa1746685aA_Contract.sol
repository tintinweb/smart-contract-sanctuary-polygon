...
contract Contract is ERC721Base {
    ...
    string[] public words = ["DOOR", "BALL", "CAR", "PHONE", "LEMON", "TANGO"];
    string public activeWord = "";
    uint randNonce = 0;

    function activateNextWord() public onlyOwner {
        randNonce++;
        uint256 randomNumber = random(string(abi.encodePacked(blockhash(block.number - 1), Strings.toString(randNonce))));
        activeWord = words[randomNumber % words.length];
    } 
    ...
    function pluck(uint256 _tokenId, string[] memory _sourceArray) internal view returns(string memory) {
        uint256 randomNumber = random(string(abi.encodePacked(blockhash(block.number - 1), Strings.toString(_tokenId))));
    ...
    function tokenURI(uint256 _tokenId) override public view returns (string memory){
        ...
        parts[0] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 700 700"><style>.base { fill: white; font-family: serif; font-size: 100px; }</style><rect width="100%" height="100%" fill="black" /><text  x="50%" y="50%" class="base">';
