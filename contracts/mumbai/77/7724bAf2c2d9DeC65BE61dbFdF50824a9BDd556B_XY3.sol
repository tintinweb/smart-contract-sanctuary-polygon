 ...
 */
contract Config is AccessControl, Pausable, ReentrancyGuard, IConfig {
    bytes32 public constant PERMIT_MANAGER_ROLE = keccak256("PERMIT_MANAGER");
     ...
     */
    function Config_init(address _admin, address _addressProvider) internal {
        require(!initialized, "Config contract is already initialized");
    ...
    function setAgentPermits(
        ...
        require(
            _agents.length == _permits.length &&
     ...
     * @param _permit - The token new status, permitted or not.
     */
    function _setAgentPermit(
        address _agent,
        bytes4 _selector,
        bool _permit
    ) internal {
        require(
            _agent != address(0) && _selector != bytes4(0),
            "agent is zero address"
        );
