...
contract FarmGenesis is ReentrancyGuard, Ownable, PaymentSplitter {
    ...
    modifier WhenOpen () {
        require(open, "GENESIS SALE CLOSES");
        _;
    }

    event GenesisSaleOpen();
    event EnterGenesisSale(address who, uint256 alloc);
    ...
    )   PaymentSplitter (payees, shares) 
    {
        ...
        require(_genesisWallet != address(0));
    ...
    nonReentrant {
        ...
        require (amount <= tiersInfo[tier].maxBuy && amount >= tiersInfo[tier].minBuy, "INCORRECT AMOUNT");

        uint256 price = amount.mul(tiersInfo[tier].price);
        ...
        dragonFarmToken.safeTransferFrom(genesisWallet, _msgSender(), amount);
        emit EnterGenesisSale(_msgSender(), amount);
    }

    function startGenesisSale() external onlyOwner {
        open = true;
        emit GenesisSaleOpen();
    }
