...
import "./BellwetherInviteTokenfactory.sol";
import "@openzeppelin/contracts/utils/Strings.sol";
import "@openzeppelin/contracts/utils/Counters.sol";
import "@openzeppelin/contracts/security/ReentrancyGuard.sol";

abstract contract CasdeBellwetherOkjhgI {
    function claim(address _claimer) external payable virtual;
    function getPrice(uint256 _tokenId) public view virtual returns(uint256);
}

contract LaksewBellwetherTokenDcsfe is BellwetherTokenFactory, ReentrancyGuard{

    using Counters for Counters.Counter;
    Counters.Counter private mtCounter;
    mapping(uint256 => MintToken ) public mintTokens;
    address public NFTcontractAddress=0x556d24b41e4253817564396c4DEb0Ad32FDD4879;

    event TokenAddedToMint(uint256 tokenID, string ipfsMetadata);
    event TokenRedeemed(uint256 tokenId, address TokenOwner, address redeemTo);
    event TokenEdited(uint256 tokenId, uint256 maxSupply , string ipfsMetadataLink);
    ...
    constructor(address _admin, string memory _firstTokenURI, uint256 _firstTokenMaxSupply) ERC1155("https://base_uri/"){
        ...
        addMintToken(_firstTokenURI, _firstTokenMaxSupply);
    }
    ...
    function addMintToken(
        ...
        _update_mt(mt, _ipfsMetadataLink, _maxSupply);
        mtCounter.increment();
    ...
    function editToken(uint256 _tokenId , uint256 _max_supply , string memory _ipfsLink) external onlyAdmin {
        if (!(_tokenId < mtCounter.current() && _tokenId > 0)) revert TokenNotExist();
        if(_max_supply < mintTokens[_tokenId].mintedCount) revert MaxSupplyLessThanMinted();
        if(bytes(_ipfsLink).length==0) _ipfsLink = uri(_tokenId);
        MintToken storage mt = mintTokens[_tokenId];
        _update_mt(mt,_ipfsLink,_max_supply);
        emit TokenEdited(_tokenId, _max_supply , _ipfsLink);
    }

    function airdrop(uint256 _tokenID, uint256 _amount, address[] calldata _addrs) external onlyOwner nonReentrant {

        if(_addrs.length==0) revert BadArrayLength();
        if (!(_tokenID > 0 && _tokenID < mtCounter.current())) revert TokenNotExist();
        if ((_amount == 0 ||  mintTokens[_tokenID].mintedCount+_amount*_addrs.length > mintTokens[_tokenID].maxSupply)) revert InvalidAirdropAmount();
        mintTokens[_tokenID].mintedCount = mintTokens[_tokenID].mintedCount + _amount*_addrs.length;
        
        for(uint256 i;i<_addrs.length;i++ )
        {
            address _addr = _addrs[i];
            if (_addr == address(0)) revert AirdropToNull();
            mintTokens[_tokenID].claimedMTs[_addr] = mintTokens[_tokenID].claimedMTs[_addr]+_amount;
    ...
    function redeemForNFT(uint256 _tokenId) external payable{
        if (!(_tokenId > 0 && _tokenId < mtCounter.current())) revert TokenNotExist();
        CasdeBellwetherOkjhgI redeemerContract = CasdeBellwetherOkjhgI(NFTcontractAddress);
        redeemerContract.claim{value:msg.value}(msg.sender);
        emit TokenRedeemed(_tokenId, msg.sender, NFTcontractAddress);
        burn(msg.sender, _tokenId, 1);
    }

    function setNFTContract(address _nft_contract_address) external onlyAdmin {
        if(_nft_contract_address==address(0)) revert ZeroAddress();
        NFTcontractAddress = _nft_contract_address;
    }
}
...
abstract contract BellwetherTokenFactory is ERC1155Supply, ERC1155Burnable, AdminPermissionable {
