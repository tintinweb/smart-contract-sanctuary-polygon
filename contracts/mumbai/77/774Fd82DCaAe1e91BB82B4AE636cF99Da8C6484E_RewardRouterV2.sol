...
contract RewardRouterV2 is ReentrancyGuard, Governable {
    ...
    address public mlp; // GMX Liquidity Provider token
...
//        IRewardTracker(feeMlpTracker).stakeForAccount(account, account, mlp, mlpAmount); // TODO
...
//        emit StakeMlp(account, mlpAmount);
    ...
    function handleRewards(
        ...
        if (_shouldClaimGmx) {
            ...
            uint256 gmxAmount1 = IVester(mlpVester).claimForAccount(account, account);
        ...
        if (_shouldClaimEsGmx) {
            ...
            uint256 esGmxAmount1 = IRewardTracker(stakedMlpTracker).claimForAccount(account, account);
    ...
    function _validateReceiver(address _receiver) private view {
        ...
        require(IVester(mlpVester).transferredAverageStakedAmounts(_receiver) == 0, "RewardRouter: gmxVester.transferredAverageStakedAmounts > 0");
        require(IVester(mlpVester).transferredCumulativeRewards(_receiver) == 0, "RewardRouter: gmxVester.transferredCumulativeRewards > 0");
    ...
    function _compoundMlp(address _account) private {
        uint256 esGmxAmount = IRewardTracker(stakedMlpTracker).claimForAccount(_account, _account);
        if (esGmxAmount > 0) {
            _stakeGmx(_account, _account, esGmx, esGmxAmount);
        }
    }
