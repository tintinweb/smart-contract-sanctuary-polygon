...
contract Marimo is Ownable, ERC721A {
  ...
  mapping(uint256 => ChangeHistory[]) public tokenChangeWaterHistory; // token id => changeWaterHistory
  ...
  function changeWater(uint256 tokenId) external {
    ...
    tokenChangeWaterHistory[tokenId].push(ChangeHistory(msg.sender, block.timestamp));
    emit ChangedWater(tokenId, msg.sender, block.timestamp);
  }
  ...
  function hoge() external view returns(uint[] memory) {
    return arr2;
  }
  function fuga() external pure returns(Stats[2] memory) {
    Stats[2] memory arr3 = [Stats(1, 2, 3, 4), Stats(5, 6, 7, 8)];
    return arr3;
  }
}
