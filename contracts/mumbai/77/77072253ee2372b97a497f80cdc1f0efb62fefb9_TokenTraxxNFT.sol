...
// File @openzeppelin/contracts-upgradeable/token/ERC721/[email protected]v4.5.2
...
// File contracts/TokenTraxxNFT.sol
    ...
    ErrorCodes
{
    ...
    function setupRoles(address _owner, address _maintainer) external onlyAdmin{
        require(_maintainer != address(0) && _owner != address(0), throwError(ZERO_ADDRESS));
    ...
    function setContractURI(string memory _uri) external onlyMaintainer {
        _contractURI = _uri;
        emit TraxxURIUpdated(_uri);
    }

    function setBaseURI(string memory _uri) external onlyMaintainer {
        _baseTokenURI = _uri;
    }
