    ...
    IAccount
{
    ...
    modifier onlyRWPostPlugin(bytes32 _checkedPluginName, uint256 _version) {
        require(
            ...
            || PluginsList.SINGLE_WRITE_POST == _checkedPluginName
    ...
    modifier onlyWCommentPlugin(bytes32 _checkedPluginName, uint256 _version) {
        require(
    ...
    ) external override onlyTrustedPlugin(PluginsList.COMMUNITY_JOIN, vars.version) returns(bool) {
    ...
    function removeCommunityUser(
        DataTypes.GeneralVars calldata vars
    ) external override onlyTrustedPlugin(PluginsList.COMMUNITY_QUIT, vars.version) returns(bool) {
        (address _communityId) = abi.decode(vars.data,(address));
    ...
    function addModerator(
        DataTypes.GeneralVars calldata vars
    ) external override onlyTrustedPlugin(PluginsList.COMMUNITY_EDIT_MODERATORS, vars.version) returns(bool) {
    ...
    function addCreatedCommentIdForUser(
        DataTypes.GeneralVars calldata vars
    ) external override onlyWCommentPlugin(vars.pluginName, vars.version) returns(bool) {
