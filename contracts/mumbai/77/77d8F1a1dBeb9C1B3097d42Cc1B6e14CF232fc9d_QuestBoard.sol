...
interface IQuestBoard {
    ...
    function setQuestURI(string memory newURI) external;
...
import {IQuestCheckerCreater} from "./QuestCheckerCreater/IQuestCheckerCreater.sol";
import {IQuestBoard} from "./IQuestBoard.sol";

contract QuestBoard is IQuestBoard, Ownable, ERC1155Supply, ReentrancyGuard {
    ...
    mapping(address => mapping(uint256 => bool)) public expGained;
    ...
    function setQuestURI(string memory newURI) external onlyOwner {
        _setURI(newURI);
    }

    function setCommunityPortal(address communityPortalAddress) external onlyOwner {
    ...
    function createQuest(
        ...
        quest.exp = _exp;
        quest.creater = _creater;
        address questCheckerAddress = questCheckerCreater.createQuestChecker(
            _interfaceId,
            _target
        );
        quest.questChecker = IQuestChecker(questCheckerAddress);
    ...
    function claim(uint32 questId) external nonReentrant {
        require(balanceOf(msg.sender, questId) == 0, "You have already minted");
        IQuestChecker questChecker = questBoard[questId].questChecker;
        require(
            questChecker.checkCompleted(msg.sender),
            "You have not completed this quest yet."
        );
        if (!expGained[msg.sender][questId]) {
            expGained[msg.sender][questId] = true;
            communityPortal.addExp(
                questBoard[questId].communityId,
                msg.sender,
                questBoard[questId].exp
            );
        }
    ...
    function burn(uint32 questId) external nonReentrant {
    ...
    function checkBatchCompleted(
        ...
        for (uint256 i; i < questIds.length; i++) {
            IQuestChecker questChecker = questBoard[questIds[i]].questChecker;
