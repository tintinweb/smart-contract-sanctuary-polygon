    ...
    VRFConsumerBaseV2
{
    ...
    // Ongoing mint requests.
    mapping(string => uint256) private _liveRequests;
    ...
    event MintRequested(string subcollectionName, address to, uint256 vrfRequestId);
    ...
    function addSubcollection(
        ...
        require(bytes(params.baseCID).length != 0, "Missing base CID");
        require(bytes(params.upgradedTokenCID).length != 0, "Missing upgraded token CID");
    ...
    function mint(
        ...
        if (!hasRole(CLAIMER_ROLE, _msgSender())) {
            require(msg.value == subcollection.params.mintPrice * mintAmount, "Invalid msg value");
            require(
                _mintedBy[_msgSender()][subcollectionIdx] + mintAmount <=
                    subcollection.params.maxMintPerAddress,
                "Max supply per address exceeded"
            );
            require(
                subcollection.totalMinted + mintAmount + _liveRequests[subcollectionName] <= subcollection.params.availableMainSupply,
                "Available main supply exceeded"
            );
            if (subcollection.whitelistMintEnabled) {
                require(
                    _verifyProof(
                        subcollectionName,
                        keccak256(
                            bytes.concat(keccak256(abi.encode(_msgSender(), msg.value)))
                        ),
                        proof
                    ),
                    "Not on whitelist"
        ...
        require(
            subcollection.totalMinted + mintAmount + _liveRequests[subcollectionName] <= subcollection.params.mainSupply,
            "Subcollection is sold out"
        );
        ...
        for (uint256 i = 0; i < mintAmount; i++) {
            ...
            _liveRequests[subcollectionName] += 1;
            emit MintRequested(subcollectionName, to, vrfRequestId);
        }
    }
    ...
    function upgradeToken(
        ...
        string memory upgradedTokenCID = subcollections[subcollectionIdx].params.upgradedTokenCID;
        require(bytes(_tokenCIDs[tokenId]).length == 0, "Token is already upraded");
        _requireMinted(tokenId);
        ...
        _setTokenCID(tokenId, upgradedTokenCID);
    ...
    /**
     * @dev Set lottery address for lookup.
     * @param lotteryAddress address of the lottery contract.
     */
    function setLottery(
        address lotteryAddress
    ) external onlyRole(TRUSTED_SIGNER_ROLE) {
        require(lottery == address(0), "Can set lottery only once");
        lottery = lotteryAddress;
    }

    /**
     * @dev Sets the new subscription ID.
     * @param newSubscriptionId The new subscription ID to be set.
     */
    ...
    /**
     * @dev Sets the new key hash.
     * @param newKeyHash The new key hash to be set.
     */
    function setKeyHash(
        bytes32 newKeyHash
    ) external onlyRole(TRUSTED_SIGNER_ROLE) {
    ...
    /**
     * @dev Mints a new token from a specified subcollection.
     * @param request The mint request to process.
     * @param trueRandom The true random number used for calculating the token ID.
     */
    function _mint(
        MintRequest memory request,
        uint256 trueRandom
    ) private {
        uint256 subcollectionIdx = getSubcollectionIdxByName(request.subcollectionName);
        Subcollection storage subcollection = subcollections[subcollectionIdx];
        require(
        ...
        if (!hasRole(CLAIMER_ROLE, request.mintBy)) {
            require(
                _mintedBy[request.mintBy][subcollectionIdx] <
                    subcollection.params.maxMintPerAddress,
                "Max supply per address"
            );
            require(
                subcollection.totalMinted < subcollection.params.availableMainSupply,
                "Available main supply exceeded"
            );
        }
        ...
        // console.log(tokenId);
        _safeMint(request.mintTo, tokenId);
        _liveRequests[request.subcollectionName] -= 1;
        subcollection.totalMinted += 1;
        _mintedBy[request.mintBy][subcollectionIdx] += 1;
        _subIdByTokenId[tokenId] = subcollectionIdx;
        if (subcollection.totalMinted == subcollection.params.mainSupply) {
            emit SubcollectionSold(request.subcollectionName);
        }
    }
...
interface ICollectionToken  {
    ...
    function mint(string memory subcollectionName, address to, bytes32[] memory proof, uint256 mintAmount) external payable;
