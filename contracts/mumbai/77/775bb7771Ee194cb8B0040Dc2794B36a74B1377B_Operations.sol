...
contract Operations is VRFConsumerBaseV2, ERC2771Recipient {
    ...
    function placeBet(
        ...
        uint256 totalStake,
        uint256 totalPayout,
        bytes32[4] calldata betDetails,
        bool[4] calldata controls,
        bool callOracle
    ) public {
        if (s_placeBetKey[betDetails[3]] == true) revert Operations__Invalid();
        if (totalStake > i_core.getUserBalance(_msgSender(), token)) revert Operations__StakeMorethanbal();
        if (totalPayout > i_core.getTotalHL(token)) revert Operations__InsuffuceintHL();
        address provider;
        if (callOracle) {
            callOrcale();
            ...
            if (totalPayout > i_core.getDepositerHLBalance(provider, token)) {
                ...
                require(totalPayout <= i_core.getDepositerHLBalance(provider, token), "Insuffcient Liquidity");
            }
        } else {
            provider = i_core.getBaseProvider(token);
        ...
        if (controls[0]) i_core.handleBalance(_msgSender(), token, totalStake, 0);
        if (controls[1]) i_core.handleHL(provider, token, totalPayout, 0);
        if (controls[2]) i_core.handleStakes(_msgSender(), token, totalStake, 1);
        if (controls[3]) i_core.handlePayout(provider, token, totalPayout, 1);
    ...
    function handleWin(
        ...
        uint256 payout,
        bytes32 odds,
        address[5] calldata addList,
        uint256[5] calldata stakeList
    ) public onlySettleSigner {
        BetSlip storage betSlip = s_keyToBetSlip[betkey];
        if (betkey != betSlip.betkey || betkey == 0) revert Operations__InvalidBetId();
        if (stake == 0) {
            uint256 sumStakes = 0;
            for (uint8 i = 0; i < addList.length; i++) {
                if (stake > i_core.getUserStaked(addList[i], betSlip.token)) revert Operations__InsufficentStakes();
                if (stake > i_core.getTotalStakes(betSlip.token)) revert Operations__InsufficentStakes();
                sumStakes = sumStakes + stakeList[i];
                i_core.handleStakes(addList[i], betSlip.token, stakeList[i], 0);
            }
            i_core.handleBalance(betSlip.bettor, betSlip.token, sumStakes + payout, 1);
        } else if (stake > 0) {
            if (stake > i_core.getUserStaked(betSlip.bettor, betSlip.token)) revert Operations__InsufficentStakes();
            if (stake > i_core.getTotalStakes(betSlip.token)) revert Operations__InsufficentStakes();
            if (payout > i_core.getProviderPayout(betSlip.provider, betSlip.token)) revert Operations__InsufficentPayouts();
