...
error SuperfluidStream__NotEnoughPayment(address customer, uint256 amount);
...
contract SuperfluidStream {
    ...
    ISETHCustom private immutable i_MATICxAddress;
    uint256 private immutable i_nativeBalance;
    ...
    constructor(
        ISuperfluid host,
        ISuperToken fDAIxAddress,
        ISETHCustom MATICxAddress,
        uint256 nativeBalance,
        ISuperToken tokenAddress
    ) {
        i_fDAIxAddress = fDAIxAddress;
        i_nativeBalance = nativeBalance;
        i_MATICxAddress = MATICxAddress;
    ...
    //upgrade can refer ISETH.. need to use upgradeByETH function and pass in msg.value
    function upgradeMATIC(uint256 amount) external payable {
        // if (amount < 0) {
        //     revert SuperfluidStream__NotEnoughPayment(msg.sender, amount);
        // }
        //need to deduct amount from the msg.sender wallet?
        //this contract address will gain the msg.value?
        //Lets say someone upgrade 2ETH to ETHx, means the 2ETH will deduct from thier account and go into the contract address account??
        i_MATICxAddress.upgradeByETH{value: amount}();
    }

    function getfDAIxbalance(address account) external view returns (uint256) {
        return i_fDAIxAddress.balanceOf(account);
    }

    function getNativeBalance(address account) external view returns (uint256) {
        return i_nativeBalance;
    }

    function getBalance(address account) external view returns (uint256) {
        return i_tokenAddress.balanceOf(account);
    }
}