...
contract NFTResolver {
    ...
    // returns(uint256 []  memory arr)
    {
        ...
        for (uint256 i = 0; i < ans.length; i++) {
            ...
            uint256[] memory nfts = INFTWarranty(contract_add).getSellerNFTs(
                ans[i]
            );
            for (uint256 j = 0; j <  INFTWarranty(contract_add).getSellerNFTs(
                ans[i]
            ).length; j++) {
                //uint256 creationTime =  INFTWarranty(contract_add).getCreation(ans[i],nfts[i]);
                if (
                    (INFTWarranty(contract_add).getExpiry(ans[i], nfts[j]) <
                        block.timestamp) &&
                    (INFTWarranty(contract_add).getStatus(ans[i], nfts[j]) == 2)
                ) {
                ...
                if (
                    INFTWarranty(contract_add).getExpiry(ans[i], nfts[j]) >
                    CurrentTime ||
                    INFTWarranty(contract_add).getStatus(ans[i], nfts[j]) ==
                    0 ||
                    INFTWarranty(contract_add).getStatus(ans[i], nfts[j]) ==
                    1 ||
                    INFTWarranty(contract_add).getStatus(ans[i], nfts[j]) == 3
                ) {
