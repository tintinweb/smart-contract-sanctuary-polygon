...
// File contracts/omni-gunners/GunnersONFT721.sol

/**
 * @title GunnersONFT721
 * @dev The GunnersONFT721 contract is an ERC721-compliant contract that represents a collection of NFTs (Non-Fungible Tokens).
 * The contract supports minting of NFTs with the ability to reveal their URIs and additional features such as whitelisting and price configuration.
 * The NFTs can be minted on multiple chains, and the contract keeps track of the token IDs for each chain separately.
 * The contract extends from ONFT721Core, which handles cross-chain communication, and ERC721Enumerable, which adds enumeration support for NFTs.
 */

pragma solidity 0.8.17;

contract GunnersONFT721 is ONFT721Core, IONFT721, ERC721Enumerable {
    /**
     * @dev Token ids counters.
     * We use two counters since we mint on multiple chains:
     * one for start/current token id and
     * one for max token id on the current chain.
     */
    uint256 public nextMintId;
    uint256 public maxMint;

    // All URI vars.
    string internal baseURI; /// @notice Set at reveal.
    string internal constant UNREVEALED_URI = "ipfs://bafkreibxx7s227gxlnihol76tigj42vh53bz56j4cuwpisglnkdpoq4vuq"; // unrevealed metadata
    string internal constant URI_EXTENSION = ".json"; // for revealed metadata

    // Indicates if NFTs have been revealed.
    bool public revealed;

    // Indicates if mint has started.
    bool public mintable;

    // Mint payment.
    address public payToken; // fx. USDT on arb
    uint256 public payTokenDecimals; // fx. 6 for USDT;
    uint256 public price; // fx. 25 * 10 ** payTokenDecimals = 25 usdt.

    // Whitelist
    bool public userWhitelist; // If enabled, whitelisted users can mint first.
    mapping(address => bool) public userWhitelisted; // Maps whitelisted collections.
    uint256 public userWhitelistStartTime;
    uint256 public userWhitelistMintDuration;

    /**
     * @dev Constructor for the GunnersONFT721 contract.
     * @param _name The name of the token.
     * @param _symbol The token symbol.
     * @param _payToken The address of the token used for minting.
     * @param _payTokenDecimals The number of decimals for the token used for minting.
     * @param _price The price of minting in payToken.
     * @param _lzEndpoint Handles message transmission across chains.
     * @param _startMintId The starting mint number on this chain.
     * @param _maxMint The maximum number of mints on this chain.
     */
    constructor(string memory _name, string memory _symbol, address _payToken, uint256 _payTokenDecimals, uint256 _price, uint256 _minGasToTransfer, address _lzEndpoint, uint256 _startMintId, uint256 _maxMint) ERC721(_name, _symbol) ONFT721Core(_minGasToTransfer, _lzEndpoint) {
    ...
    // Layer Zero functionality.
    /**
     * @dev Internal function to debit tokens from the sender and transfer them to the contract.
     * @param _from The address from which to debit the tokens.
     * @param _tokenId The ID of the token being debited.
     */
    ...
    /**
     * @dev Internal function to credit tokens to the recipient.
     * @param _toAddress The address to which the tokens should be credited.
     * @param _tokenId The ID of the token being credited.
     */
    ...
    /**
     * @dev Retrieves the current owner of a token by its ID.
    ...
    // The following functions are overrides required by Solidity for the extensions.
    /**
     * @dev Checks if a given interface is supported by this contract.
     * @param interfaceId The interface identifier.
     * @return True if the interface is supported, false otherwise.
     */
    function supportsInterface(bytes4 interfaceId) public view virtual override(ONFT721Core, IERC165, ERC721Enumerable) returns (bool) {
        return interfaceId == type(IONFT721).interfaceId || super.supportsInterface(interfaceId);
    }

    // Custom functionality.
    /**
     * @dev Toggles the whitelisted status of a user.
     * @param _user The address of the user.
     */
    function toggleUserWhitelist(address _user) external onlyOwner {
        userWhitelisted[_user] = !userWhitelisted[_user];
    }

    /**
     * @dev Mints a specified amount of tokens to the sender.
     * @param _amount The amount of tokens to mint.
     */
    function mint(uint256 _amount) external {
        require(mintable, "GunnersONFT721: Mint not started!");
        require(nextMintId + _amount <= maxMint, "GunnersONFT721: max mint limit reached");

        if (userWhitelist) {
            require(block.timestamp > userWhitelistStartTime, "GunnersONFT721: too early!");
        }
        // whitelist and in whitelist mint time
        if (userWhitelist && block.timestamp > userWhitelistStartTime && block.timestamp < (userWhitelistStartTime + userWhitelistMintDuration)) {
            require(userWhitelisted[msg.sender], "GunnersONFT721: not whitelisted!");
        }

        // Transfer payment from user.
        require(IERC20(payToken).transferFrom(msg.sender, address(this), price * _amount), "GunnersONFT721: Transfer error!");

        // Mint token(s) to user.
    ...
    /**
     * @dev Sets the contract to a mintable state.
     * This function can only be called by the contract owner and can only be called once.
     */
    function setMintable() external onlyOwner {
        require(!mintable, "GunnersONFT721: Already set!");
        mintable = true;
    }

    /**
     * @dev Sets the user whitelist configuration.
     * This function can only be called by the contract owner and can only be called once.
     * @param _userWhitelistStartTime The start time for the user whitelist.
     * @param _userWhitelistMintDuration The duration of the whitelist minting period.
     */
    function setUserWhitelist(uint256 _userWhitelistStartTime, uint256 _userWhitelistMintDuration) external onlyOwner {
        require(!mintable, "GunnersONFT721: Already set!");
        require(!userWhitelist, "GunnersONFT721: Already set");
        userWhitelist = true;
        userWhitelistStartTime = _userWhitelistStartTime;
        userWhitelistMintDuration = _userWhitelistMintDuration;
    }

    /**
     * @dev Stops minting at the current token ID.
    ...
    // Opensea compatibility functions.
    /**
     * @dev Overrides the baseURI function to return the base URI for token URIs.
     * @return The base URI.
     */
    ...
    function baseTokenURI() public view returns (string memory) {
        return revealed ? _baseURI() : UNREVEALED_URI;
    }
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        require(_exists(_tokenId), "GunnersONFT721: Does not exist!");
        return revealed ? string(abi.encodePacked(baseURI, Strings.toString(_tokenId), URI_EXTENSION)) : UNREVEALED_URI;
    }
