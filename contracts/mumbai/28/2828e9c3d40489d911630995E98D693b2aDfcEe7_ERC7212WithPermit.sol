...
contract ERC7212WithPermit is ERC721, EIP712Base, AccessControl {
  ...
  bytes32 private constant MINT_PERMIT_TYPEHASH = 
    keccak256("MintWithSig(address purchaser,uint256 deadline)");
   ...
   */
  event PurchaseBlock(address indexed origin, address indexed buyer, uint256 indexed tokenId);
  ...
  function mintSigleBlockWithPermit(
    ...
    require(verifyMintSig(purchaser, deadline, signature));
    ...
    emit PurchaseBlock(address(0), purchaser, tokenId);
  }
  ...
  function mintBatchBlocksWithPermit(
    ...
    emit PurchaseBlocks(address(0), purchaser, tokenIds, tokenUris);
  }
  ...
  function verifyMintSig(
    ...
    //     DOMAIN_SEPARATOR(),
    //     keccak256(
    //       abi.encode(MINT_PERMIT_TYPEHASH, purchaser)
    //     )
    //   )
    // );
    ...
    address signer = recoverAddress(
      DOMAIN_SEPARATOR(),
      keccak256(abi.encode(MINT_PERMIT_TYPEHASH, purchaser, deadline)),
      signature
    );
