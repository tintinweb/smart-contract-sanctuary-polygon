...
///@title HousePool Contract
///@author LunaFi.io
contract HousePoolV14 is
    ERC20Upgradeable,
    ...
    ERC20VotesUpgradeable
{
    ...
    uint256 public constant MAX_PRECISION = 18;
    ...
    // Define HousePool related structures
    ...
    //Define HousePool related variables
    IStakedToken public stakedToken;
    IStakedToken public rewardToken;
    uint8 pool_decimals;
    ...
    uint256 public runningTotalDeposits;

    // Define Pool & Related variables
    ...
    // Lion Gaming contract address
    address private sportsBookContract;

    // Define HousePool custom type variables
    FarmInfo public farmInfo;

    // Define Betting custom type variables
    ValuesOfInterest private voi;

    // Define mappings related to HousePool
    mapping(address => uint256) private cooldownStartTimes;
    mapping(address => UserInfo) private userInfo;

    // Define mappings related to Pool
    ...
    modifier onlyValid(ValuesOfInterest memory data, bytes memory signature) {
        ...
        require(
            ...
            "HOUSEPOOL: invalid signature"
        );
        require(data.signer != address(0), "HOUSEPOOL: invalid signer");
        require(
            hasRole(DATA_PROVIDER_ORACLE, data.signer),
            "HOUSEPOOL: unauthorised signer"
        );
        require(
            block.number < data.deadline,
            "HOUSEPOOL: signed transaction expired"
        );
        _;
    }

    ///@notice Initializer function to be used by OZ upgradeability
    ///@param _name name of the token
    ///@param _symbol symbol of the token
    ///@param _stakedToken staked token address
    ///@param _rewardToken reward token address
    ///@param _cooldownSeconds cooldown seconds
    ///@param _unstakeWindow unstake window time
    ///@param _rewardsPerSecond reward per second
    ///@param _lpTokenPrice initial lp token price
    ///@param _poolName pool name
    function initialize(
        string memory _name,
        string memory _symbol,
        IStakedToken _stakedToken,
        IStakedToken _rewardToken,
        uint256 _cooldownSeconds,
        uint256 _unstakeWindow,
        uint256 _rewardsPerSecond,
        uint256 _lpTokenPrice,
        string memory _poolName,
        uint8 _decimals
    ) external initializer {
        require(
            address(_stakedToken) != address(0),
            "HOUSEPOOL:Staked Token Address is zero"
        );
        require(
            address(_rewardToken) != address(0),
            "HOUSEPOOL:Reward Token Address is zero"
        );
        ...
        poolName = _poolName;
        lpTokenPrice = _lpTokenPrice; // Setting initial HousePool LP token price
        initlpTokenPrice = _lpTokenPrice;
        pool_decimals = _decimals;
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(MANAGER_ROLE, msg.sender);
        _grantRole(STAKING_MANAGER, msg.sender);
        setRewardPerSecond(_rewardsPerSecond);
        createFarm();
    }
    ...
    function getUserRewardDebt(address _benefiter)
        external
        view
        returns (int256)
    {
    ...
    ///@return  pendingRewards returns  for the given _benefiter
    function getRewards(address _benefiter)
        external
        view
    ...
    function getlionGamingContract() external view returns (address) {
        return sportsBookContract;
    }

    function setlionGamingContract(address _sportsAddress) external onlyRole(MANAGER_ROLE) {
    ...
    function setUnstakeWindowTime(uint256 _unstakeWindow)
        external
        onlyRole(STAKING_MANAGER)
    {
        unstakeWindow = _unstakeWindow;
    }
    ...
    function setCooldownSeconds(uint256 _coolDownSeconds)
        external
        onlyRole(STAKING_MANAGER)
    {
        coolDownSeconds = _coolDownSeconds;
    ...
    // function setPendingStakes(int256 _pendingStakes)external  {
    //     pendingStakes = _pendingStakes;
    //     updateAttributes();
    // }
    ...
    function permitAndStake(
        ...
        IStakedToken(stakedToken).permit(
            owner,
            spender,
            value,
            deadline,
    ...
    function unstakeMax() external {
        ...
        require(unstakeValue > 0  , "HOUSEPOOL: INVALID AMOUNT");
        if (cooldownActive) {
            uint256 cooldownStartTimestamp = cooldownStartTimes[msg.sender];
            require(
                (block.timestamp) >
                    (cooldownStartTimestamp + (coolDownSeconds)),
        ...
        require(
            ...
            "HOUSEPOOL: Can't withdraw, not enough liquidity to cover me and amount"
        );
        ...
        user.rewardDebt -= int256(
            (
                (((uint256(unstakeValue)  * 10**MAX_PRECISION) * farm.accRewardsPerShare) /
                    lpTokenPrice)
            ) / ACC_REWARD_PRECISION
        );
        user.amount -= userLPTokens;
        if (cooldownActive) {
            if (balanceOfMessageSender - (userLPTokens) == 0) {
        ...
        if (totalSupply() == 0) {
            ...
            lpTokenPrice =
            (uint256(totalValueLocked) * 10**MAX_PRECISION) / totalSupply();
        }

        bool success = IStakedToken(stakedToken).transfer(msg.sender, uint256(unstakeValue));

        require(success == true,"HOUSEPOOL: Transfer was not successful");
        ...
        unStake(msg.sender, uint256(unstakeValue));
    ...
    function setCoolDownActiveState(bool _active)
        external
    ...
    function claimRewards(address to) external {
        ...
        bool success = IStakedToken(rewardToken).transfer(to, _pendingReward);
        if (success) {
    ...
    function getuserEVTrackerForTheUser(address _user)
        external
        view
        returns (int256)
    {
    ...
    function getBetInfoByID(uint256 _ID)
        external
        view
        returns (
            bool,
    ...
    ///@notice getUserLPTokenAmount function returns the LPToken amount of the _benefiter from the farm
    ///@param _benefiter address of the user
    function getUserLPTokenAmount(address _benefiter)
        external
        view
        returns (uint256)
    {
        return userInfo[_benefiter].amount;
    }
    ...
    function getMyLiquidity(address _user) external view returns (int256) {
        return ceil(int256((balanceOf(_user) * liquidity)),int256(10**(pool_decimals*2))) / int256(totalSupply());
    }
    ...
    function storeBets(BetInfo[] memory betinformation)
        external
        onlyRole(HOUSE_POOL_DATA_PROVIDER)
    {
        int256 tempPendingStakes;
            for(uint256 i= 0; i <betinformation.length; i++) {
                tempPendingStakes += int256(betinformation[i].amount);
            ...
            pendingStakes += tempPendingStakes;
            liquidity += uint256(tempPendingStakes);
            updateAttributes();
    }
    ...
    function updateBets(
        ...
        require(_Id.length == _payout.length, "HOUSEPOOL: Invalid bets");
    ...
    function settleBets(uint256[] memory _Id, uint256[] memory _result)
        external
        onlyRole(HOUSE_POOL_DATA_PROVIDER)
    {
        require(_Id.length == _result.length, "HOUSEPOOL: Invalid bets");
        for(uint256 i =0; i < _Id.length; i++) {
            if (_result[i] == 1) {
                betInfoWithId[_Id[i]].result = _result[i];
                liquidity -= (betInfoWithId[_Id[i]].payout  +
                    betInfoWithId[_Id[i]].amount);
                bool success = stakedToken.transfer(
                    sportsBookContract,
                    betInfoWithId[_Id[i]].payout
                );
                if (success) {
                    pendingStakes -= int256(betInfoWithId[_Id[i]].amount);
                ...
                betInfoWithId[_Id[i]].result = _result[i];
                liquidity += betInfoWithId[_Id[i]].amount;
                pendingStakes -= int256( betInfoWithId[_Id[i]].amount);
                updateAttributes();
            }
        }
    ...
    function stake(address onBehalfOf, uint256 amount) public {
        require(amount > 1 && amount <= stakedToken.balanceOf(msg.sender), "HOUSEPOOL: INVALID AMOUNT");
        require(onBehalfOf != address(0), "onBehalOf Address can't be zero");
        runningTotalDeposits += amount ;
        ...
        if (cooldownActive) {
            cooldownStartTimes[onBehalfOf] = getNextCooldownTimestamp(
                ...
                amount,
                onBehalfOf,
                balanceOf(msg.sender) * lpTokenPrice
            );
        }
        bool success = IStakedToken(stakedToken).transferFrom(
            msg.sender,
            address(this),
            amount
        );
    ...
    function unStake(address to, uint256 amount) public {
        require(amount > 0  , "HOUSEPOOL: INVALID AMOUNT");
        if (cooldownActive) {
            uint256 cooldownStartTimestamp = cooldownStartTimes[msg.sender];
            require(
                (block.timestamp) >
        ...
        require(
            int256(amount) <= maxUserWithdrawalAmount,
            "HOUSEPOOL: Amount is more than maxUserWithdrawalAmount"
        );
        require(
            (int256(amount) <= liquidityWithAdjustedME),
            "HOUSEPOOL: Can't withdraw, not enough liquidity to cover me and amount"
        );
        uint256 tokensToBurn = (amount * 10**MAX_PRECISION) / lpTokenPrice;
        ...
        user.rewardDebt -= int256(
            (
                (((amount  * 10**MAX_PRECISION) * farm.accRewardsPerShare) /
                    lpTokenPrice)
            ) / ACC_REWARD_PRECISION
        );
        user.amount -= tokensToBurn;
        if (cooldownActive) {
        ...
        bool success = IStakedToken(stakedToken).transfer(to, amount);
    ...
    function getMaxWithdrawal(address _user) public view returns (int256) {
        int256 userShare = int256(((balanceOf(_user) *10**pool_decimals) / totalSupply()));
        int256 maxUserWithdrawalAmount;
        if (voi.expectedValue <  userEVTracker[_user]) {
            maxUserWithdrawalAmount = (userShare) * int256(totalValueLocked) / int256(10**pool_decimals);
        } else if(voi.expectedValue ==  userEVTracker[_user]){
             maxUserWithdrawalAmount =  ceil(int256(balanceOf(_user)*lpTokenPrice),10**18)/10**18;   
        }else if(voi.expectedValue > userEVTracker[_user]) {
            int256 XtotalValueLocked =
            int256(liquidity) - pendingStakes +
            (userEVTracker[_user]) ;
            uint256 XlpTokenPrice =
            (uint256(XtotalValueLocked) * 10**MAX_PRECISION) / totalSupply();
            maxUserWithdrawalAmount = ceil(int256(XlpTokenPrice * balanceOf(_user)),10**18)/10**18;
        }
    ...
    ///@param amountToReceive amount to stake
    ///@param toAddress receiver of the stake benefits
    ///@param toBalance LpToken balance
    function getNextCooldownTimestamp(
        ...
        if (cooldownActive) {
            toCooldownTimestamp = cooldownStartTimes[toAddress];
            if (toCooldownTimestamp == 0) {
                return 0;
            }
            uint256 minimalValidCooldownTimestamp = ((block.timestamp -
                coolDownSeconds) - (unstakeWindow));
            ...
            if (minimalValidCooldownTimestamp > toCooldownTimestamp) {
                ...
                if (fromCooldownTimestamp < toCooldownTimestamp) {
                    ...
                    toCooldownTimestamp =
                        (amountToReceive * (block.timestamp)) + // block.timestamp
    ...
    function _transfer(
        ...
        if (from != to) {
            ...
            uint256 previousSenderCooldown;
            if (cooldownActive) {
                previousSenderCooldown = cooldownStartTimes[from];
