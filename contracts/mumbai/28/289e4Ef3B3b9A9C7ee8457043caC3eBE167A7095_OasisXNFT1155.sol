    ...
    PaymentSplitter
{
    ...
    function _forwardFunds(uint256 received) internal {
        ...
        (bool success, ) = protocolAddress.call{
            value: (received.div(10000)).mul(protocolFee)
        }("");
    ...
    function _checkLimit(address minter, uint256 amount)
        ...
        returns (bool)
    {
        return _minted[_phase][minter].add(amount) <= _mintsPerAddressLimit;
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (from == address(0)) {
            for (uint256 i = 0; i < ids.length; ++i) {
                _minted[_phase][to] += _minted[_phase][to].add(amounts[i]);
            }
        }
    }
}
