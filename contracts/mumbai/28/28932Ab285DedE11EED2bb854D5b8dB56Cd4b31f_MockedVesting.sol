...
contract Vesting is Operator {
    ...
    function possibleToClaim(address account, uint256 vestId) public view returns (uint256 possibleToClaimAmount) {
        require(_vesters.contains(account), "Account is not vester");
        VestingSchedule memory vestingItem = _vests[account][vestId];
        uint256 timestamp = getTimestamp();
        uint256 startTimestampVesting = timestamp + vestingItem.cliff;
    ...
    event Claimed(address[] indexed recipients, uint256[] amounts, uint256[] vestIds);
    event StartTimestampSetted(uint256 timestamp);
    event TokenWithdrawn(address recipient, uint256 amount);
    event VestingsCreated(address[] indexed recipients, uint256[] amounts, uint256[] vesting, uint256[] cliff);
    ...
    function claim(
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            ...
            token.safeTransfer(recipients[i], claimAmount);
        }
    ...
    function createVestings(
        ...
        uint256[] memory cliff
    ) external onlyOperator returns (bool) {
        uint256 length = recipients.length;
        uint256 balance = token.balanceOf(address(this));
        require(
            length == amounts.length && length == cliff.length && length == vesting.length,
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            vestingItem.monthlyAmount = vestingItem.amount / vesting[i];
            vestingItem.vesting = vesting[i] * MONTH;
            vestingItem.cliff = cliff[i] * MONTH;
            _vests[recipients[i]].push(vestingItem);
            _totalAmountVested += amounts[i];
            _vesters.add(recipients[i]);
            if (cliff[i] == 0 && vesting[i] == 0) {
                token.safeTransfer(recipients[i], amounts[i]);
                _totalAmountClaimed += amounts[i];
                vestingItem.claimAmount = amounts[i];
            }
        }
        emit VestingsCreated(recipients, amounts, vesting, cliff);
        return true;
    }

    function setStartTimestamp(uint256 timestamp) external onlyOwner returns (bool) {
        _startTimestamp = timestamp;
        emit StartTimestampSetted(timestamp);
        return true;
    }
