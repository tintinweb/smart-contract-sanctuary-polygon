...
contract BlindBox is OwnableUpgradeable, ItemPool, ReentrancyGuardUpgradeable {
    ...
    uint256 constant public all_count_n = 1000;
    uint256 constant public all_count_s = 1000;
    ...
    function random(uint boxType) public view returns (uint256)
    {
        uint init_total_weight = 0;
    ...
    function buyBox(uint amount, uint boxType) external payable nonReentrant
    {
        require(msg.value >= price * amount, "value error");
        require(_msgSender() == tx.origin, "sender error");
        require(balance[_msgSender()] + amount <= max_count_4_user, "amount limit");
        ...
        {
            uint random_val = random(boxType);
            uint256 tmp_sum = 0;
            for (uint j = 0; j < init_item_pool.length; j++)
            {
