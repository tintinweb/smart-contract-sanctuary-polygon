 ...
 */
contract FakeAltroFractionsSale is AccessControl, ReentrancyGuard, ERC721Holder, ERC1155Holder, Initializable, IFractionsSale {
	...
	/**
	 * @dev The address of the FakeAltroFractions contract instance
	 */
	...
	/**
	 * @dev The address of the FakeAltroFractionsBuyout contract
	 */
	...
	modifier onlyWhileSaleOpen(uint256 saleId) {
		require(isSaleOpen(saleId), "FakeAltroFractionsSale: sale not open");
		_;
	}
	/**
	...
	modifier onlyIfSaleClosed(uint256 saleId) {
		require(isSaleClosed(saleId), "FakeAltroFractionsSale: sale not finished yet");
		_;
	}

	/**
	 * @dev Constructor to initialize the FakeAltroFractionsSale contract
	...
	constructor(IFractions _altrFractions, address _feeManager, address _allowList, uint256[] memory _priceLimits, uint256[] memory _fractionsAmounts) {
		require(address(_altrFractions) != address(0), "FakeAltroFractionsSale: cannot be null address");
	...
	function setupSale(
		...
		require(closingTime > openingTime + MIN_OPENING_GAP, "FakeAltroFractionsSale: closing time must be greater than opening time plus min opening time period");
		require(openingTime > block.timestamp, "FakeAltroFractionsSale: opening time cannot be in the past");
		require(address(nftCollection) != address(0), "FakeAltroFractionsSale: cannot be null address");
		require(address(buyToken) != address(0), "FakeAltroFractionsSale: cannot be null address");
		require(totalPrice > 0, "FakeAltroFractionsSale: fraction price must be above 0");
		require(saleMinFractions > 0, "FakeAltroFractionsSale: sale min fractions must be above 0");

		uint256 fractionsAmount = getFractionsAmountByPrice(totalPrice);

		require(fractionsAmount > minFractionsKept, "FakeAltroFractionsSale: cannot keep more fractions than issued");
		require(fractionsAmount >= saleMinFractions, "FakeAltroFractionsSale: cannot sell more fractions than issued");
	...
	function buyFractions(uint256 saleId, uint256 amount) external nonReentrant onlyWhileSaleOpen(saleId) {
		require(allowList.isAddressAllowed(msg.sender), "FakeAltroFractionsSale: address not allowed");
		FractionsSale storage fractionsSale = fractionsSales[saleId];
		fractionsSale.fractionsSold += amount;
		require(fractionsSale.fractionsSold <= fractionsSale.fractionsAmount, "FakeAltroFractionsSale: not enough fractions available");
	...
	function withdrawFractionsKept(uint256 saleId) external onlyIfSaleClosed(saleId) {
		...
		require(fractionsSale.initiator == msg.sender, "FakeAltroFractionsSale: must be sale initiator");
		require(fractionsSale.fractionsSold >= fractionsSale.saleMinFractions, "FakeAltroFractionsSale: sale unsuccessful");
	...
	function withdrawFailedSaleNft(uint256 saleId) external onlyIfSaleClosed(saleId) {
		...
		require(fractionsSale.initiator == msg.sender, "FakeAltroFractionsSale: must be sale initiator");
		require(fractionsSale.fractionsSold < fractionsSale.saleMinFractions, "FakeAltroFractionsSale: can't trade nft back");
	...
	function setFractionsBuyout(address fractionsBuyoutAddress) external initializer onlyRole(DEFAULT_ADMIN_ROLE) {
		require(fractionsBuyoutAddress != address(0), "FakeAltroFractionsSale: cannot be null address");
	...
	function onERC721Received(address operator, address from, uint256 tokenId, bytes memory data) public override returns (bytes4) {
		require(operator == address(this), "FakeAltroFractionsSale: cannot directly send ERC721 tokens");
	...
	function onERC1155Received(address operator, address from, uint256 tokenId, uint256 amount, bytes memory data) public override returns (bytes4) {
		require(operator == address(this), "FakeAltroFractionsSale: cannot directly send ERC1155 tokens");
	...
	function _setAllowList(address _allowList) internal {
		require(_allowList != address(0), "FakeAltroFractionsSale: cannot be null address");
		require(_allowList.supportsInterface(type(IAllowList).interfaceId), "FakeAltroFractionsSale: does not support IAllowList interface");
	...
	function _setFeeManager(address _feeManager) internal {
		require(_feeManager != address(0), "FakeAltroFractionsSale: cannot be null address");
		require(_feeManager.supportsInterface(type(IFeeManager).interfaceId), "FakeAltroFractionsSale: does not support IFeeManager interface");
	...
	function _setTiers(uint256[] memory priceLimits, uint256[] memory fractionsAmounts) internal {
		require(priceLimits.length == fractionsAmounts.length, "FakeAltroFractionsSale: cannot map array of different size");
		require(priceLimits[0] == 0, "FakeAltroFractionsSale: price limits array must start with 0");
		for (uint256 i; i < priceLimits.length; i++) {
			bool success = tiers.set(priceLimits[i], fractionsAmounts[i]);
			require(success, "FakeAltroFractionsSale: tiers setting fails");
		}
	}
}
