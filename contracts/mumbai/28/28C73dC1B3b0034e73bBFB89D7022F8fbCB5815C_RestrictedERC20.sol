...
import "./SBT.sol"; // Import the SBT contract, make sure the file path is correct
...
contract RestrictedERC20 is ERC20, Ownable {
    ...
    function mint(address to, uint256 amount) public onlyOwner {
        require(isKYCVerified(to), "REST: Address must be KYC verified to mint tokens");
        _mint(to, amount);
    }
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal override {
        ...
        if (from != address(0) && to != address(0)) {
            ...
            if (from != uniswapPair && to != uniswapPair) {
                require(isKYCVerified(from), "REST: From address must be KYC verified");
                require(isKYCVerified(to), "REST: To address must be KYC verified");
            }
        }
    }
}
