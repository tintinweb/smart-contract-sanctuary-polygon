...
contract DragonReplicator is BaseAccessControl {
    
    address private _tokenContractAddress;

    event ReplicaCreated(address indexed to, uint tokenId, uint dragonValue);
    ...
    function replicate(address to, uint tokenId, uint value, string memory cid) external onlyRole(COO_ROLE) {
        DragonToken(tokenContract()).mintReplica(to, tokenId, value, cid);
        emit ReplicaCreated(to, tokenId, value);
    }
}
