...
contract WowTCommunityPoll is OwnableUpgradeable, AccessControlUpgradeable {
  ...
  event pollUpVoted(string communityName, string catagory, uint256 pollId, uint256 upVotes);
  event pollDownVoted(string communityName, string catagory, uint256 pollId, uint256 downVotes);
  event pollCommented(string communityName, string catagory, uint256 pollId, uint256 downVotes);
    ...
    function createPoll(string calldata _communityName, string calldata _question, string[] memory _options) public {
        require(
            community.checkCommunityExists(_communityName),
            "Community doesn't exist"
        );
        require(community.checkMembership(_communityName, _msgSender()), "You are not member in this community");
        require(bytes(_question).length > 0, "Invalid question");
        string memory userName = community.getUserName(
            _communityName,
            _msgSender()
        );
  ...
  emit pollCreated(_communityName, "poll", pollId);
    ...
    function vote(string calldata _communityName, uint256 _pollId, uint256 _optionIndex) public {
        require(
            community.checkCommunityExists(_communityName),
            "Community doesn't exist"
        );
  ...
  emit pollVoted(_communityName, "poll", _pollId);
    ...
    function UpVotePoll(
        ...
        require(_pollId <= pollCount, "Invalid post ID");
        Poll storage poll = communityDetail.polls[_pollId];
    ...
    function getPoll(string calldata _communityName, uint256 _pollId) public view returns (string memory, string[] memory, uint256, uint256, uint256[] memory, uint256, address, string memory) {
        ...
        return (poll.question, poll.options, poll.upVotes, poll.downVotes, voteCounts, poll.timestamp, poll.creator, poll.userName);
    }
}
