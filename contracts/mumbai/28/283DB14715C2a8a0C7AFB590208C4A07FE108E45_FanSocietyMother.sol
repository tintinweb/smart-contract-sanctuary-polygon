...
contract FanSocietyMother is ERC1155URIStorage, Ownable, PaymentSplitter {
    // "_balances" mapping is inherited via ERC1155
    // -------<tokenId>---------<owner>-----<amount of tokens>
    // mapping(uint256 => mapping(address => uint256)) private _balances;
    ...
    uint256[] private shares_;
    uint8 public tokenAmount = 1;
    ...
    // Need to provide _payees and _shares arguments in the deploy script
    constructor(address[] memory _payees, uint256[] memory _shares) ERC1155("") PaymentSplitter(_payees, _shares) payable {
    //constructor() ERC1155("") PaymentSplitter("") {
        //ownerAddr = payable(msg.sender);
        // Set Base URI at deployment
        _setBaseURI("https://ipfs.io/ipfs/");
    }

    // Create a new fan generation, ie create new _ids, increment current token id variable += 1
    // OPTIONAL: require an amount of time from previous generation creation, ie 6 months.
    function createGeneration (uint256 _tokenFee, string memory _tokenUri) public onlyOwner {
        activeGenerationId++;
        tokenFee = _tokenFee;
    ...
    function mint () public payable {
        require(activeGenerationId > 0, "FanSociety: Tokens haven't been activated for this contract.");
        require(msg.value >= tokenFee, "FanSociety: Send more ETH!");
        _mint(msg.sender, activeGenerationId, tokenAmount, "");
        //(bool sent, ) = ownerAddr.call{value: msg.value}("");
        (bool sent, ) = address(this).call{value: msg.value}("");
        require(sent, "Not enough eth sent");
        emit FanPinMinted(msg.sender, activeGenerationId, tokenFee);
     }  
    ...
    // Create raffle in separate contract
}