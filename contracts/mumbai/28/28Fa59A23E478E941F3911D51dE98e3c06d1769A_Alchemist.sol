...
contract Alchemist is ReentrancyGuard, AccessControl, ERC721Enumerable, Pausable, IncomingTransferSupport {
    ...
    address payable private _poolBank;
    address payable private _midLandBank;
    address payable private _alchemistBank;
    address private immutable _erc20TransferHelper;
    mapping(uint256 => address) private EOAOf;
    IERC20 private _MIDCoin;
    ...
    constructor(address timelockController, address erc20TransferHelper_) ERC721("Alchemist", "ALC") IncomingTransferSupport(erc20TransferHelper_) Pausable() {
     ...
     */
    function init(address payable poolBank_, address payable midLandBank_, address payable alchemistBank_, address MIDCoin_) external {
        require(hasRole(ALCHEMIST_ADMIN, msg.sender), "init only alchemist admin");
        require(_poolBank == address(0), "init already initialized");
        _poolBank = poolBank_;
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory render = ItokenURIEngine(_tokenURIEngine).render(tokenId);
