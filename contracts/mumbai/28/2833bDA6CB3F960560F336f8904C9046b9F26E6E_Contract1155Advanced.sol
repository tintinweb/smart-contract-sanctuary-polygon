...
abstract contract DigitalNFT1155Advanced is DigitalNFT1155Base, ERC1155Supply, DigitalNFTStorage, DigitalNFTMintable {
    ...
    function publicMintBatch(uint256[] memory tokenIDs, uint256[] memory amounts) external payable {
    ...
    function _priceCheckBatch(uint256 value, uint256[] memory tokenIDs, uint256[] memory amounts) private view {
    ...
    function _supplyCheckBatch(uint256[] memory tokenIDs, uint256[] memory amounts) private view {
        for (uint256 i = 0; i < tokenIDs.length; i++) {
            uint256 tokenID = tokenIDs[i];
            uint256 amount = amounts[i];
            uint256 maxSupply = getMaxSupply(tokenID);
    ...
    function adminMint(address account, uint256 id, uint256 amount) public onlyRole(ADMIN_ROLE) {
        _supplyCheck(id, amount);
        _mint(account, id, amount, "");
    }

    function adminMintBatch(address to, uint256[] memory ids, uint256[] memory amounts) public onlyRole(ADMIN_ROLE) {
        _supplyCheckBatch(ids, amounts);
        _mintBatch(to, ids, amounts, "");
    }

    function setMaxSupply(uint256 tokenID, uint256 supply) external onlyRole(ADMIN_ROLE) {
        if(_maxSupplies[tokenID] >= supply) revert supplyError(tokenID);
        _maxSupplies[tokenID] = supply;
    }

    function setMaxSupplyBatch(uint256[] memory tokenIDs, uint256[] memory supplies) public onlyRole(ADMIN_ROLE) {
...
abstract contract DigitalNFTAdvanced is DigitalNFTBase, AccessControl {
...
library DigitalNFTUtilities {
    ...
    function _duplicateCheck(uint256[] memory _tokenIDs) internal pure {
        for (uint256 i = 0; i < _tokenIDs.length; i++)
         for (uint256 j = 0; j < _tokenIDs.length; j++)
            if(i != j && _tokenIDs[i] == _tokenIDs[j] || i == j && _tokenIDs[i] != _tokenIDs[j]) revert paramsError();
    }

    function _lengthCheck(uint256[] memory _a, uint256[] memory _b) internal pure {
        if(_a.length != _b.length) revert paramsError();
    }

    function _lengthCheck(uint256[] memory _a, string[] memory _b) internal pure {
        if(_a.length != _b.length) revert paramsError();
    }

    function _lengthCheck(uint256[] memory _a, bool[] memory _b) internal pure {
        if(_a.length != _b.length) revert paramsError();
    }
}
...
abstract contract DigitalNFTMintable is DigitalNFTAdvanced {
   ...
   // ============================== Fields ============================== //
    ...
    function setPrice(uint256 tokenID, uint256 price) external onlyRole(ADMIN_ROLE) {
        _prices[tokenID] = price;
    }

    function setPriceBatch(uint256[] memory tokenIDs, uint256[] memory prices) public onlyRole(ADMIN_ROLE) {
        DigitalNFTUtilities._lengthCheck(tokenIDs, prices);
...
abstract contract DigitalNFTStorage is DigitalNFTAdvanced {
   ...
   // ============================== Fields ============================== //
    ...
    function setUri(uint256 tokenID, string memory newUri) external onlyRole(ADMIN_ROLE) {
        _uris[tokenID] = newUri;
    }

    function setUriBatch(uint256[] memory tokenIDs, string[] memory newUris) public onlyRole(ADMIN_ROLE) {
...
abstract contract DigitalNFT1155Base is ERC1155 {
...
abstract contract DigitalNFTBase {
...
contract Contract1155Advanced is DigitalNFT1155Advanced {
    ...
    constructor() ERC1155("") {
        ...
        name = "DigitalNFT1155";
        symbol = "DNFT1155";
        _contractUri = "https://raw.githubusercontent.com/flaggyverse/flaggyverse.github.io/main/Resources/Flags/Metadatas/0";
    }
}