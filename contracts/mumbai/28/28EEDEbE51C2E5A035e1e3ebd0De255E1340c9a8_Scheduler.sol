    ...
    SchedulerStorage
{
    ...
    function _createSchmint(
        ...
        address receiverAddress = _receiver == address(0) ? owner() : _receiver;
    ...
    function executeSchmint(uint256 schmintId) external nonReentrant {
        ...
        // scoping external call to one block
        {
            ...
            receiver = schmints[schmintId].receiver;
    ...
    function _checkFundsDeposit() internal {
        if (msg.value > 0) {
    ...
    function onERC721Received(
        ...
        // receiver address is set temporarily to ensure transaction is in execution right now
        // if invoked directly by someone, receiver address will be zero address, hence if block won't be executed
        // if(receiver!=address(0) && receiver!=address(this)) {
            IERC721Upgradeable(msg.sender).safeTransferFrom(address(this), receiver, tokenId);
        // }
        // returns data to notify NFT is received
...
contract SchedulerStorage {
    ...
    struct Schmint {
        ...
        address receiver; // who should receiver the NFTs
