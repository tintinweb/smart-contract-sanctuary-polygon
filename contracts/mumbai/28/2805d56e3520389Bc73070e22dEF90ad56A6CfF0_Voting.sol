...
contract Voting is GSNRecipient {
    ...
    uint[] public results = new uint[](180);

    mapping(uint => Survey) public surveys;
    mapping(uint => mapping(address => uint)) public surveyVoterResponse;
    ...
    struct Survey {
        ...
        string option1;
        string option2;
        uint timestamp; // timestamp in seconds since the epoch
        uint[] results;
    }
    ...
    function checkWhitelisted(address wallet) external view returns (bool) {
        return _whitelist[wallet];
    }

    function getVoterResponse(uint surveyId, address wallet) external view returns (uint) {
        require(surveys[surveyId].id != 0, "Voting: Survey doesn't exist!");
        return surveyVoterResponse[surveyId][wallet];
    }
    ...
    function createSurvey(
        ...
        require(surveys[id].id == 0, "Voting: Survey exists!");
        
        surveys[id].id = id;
        surveys[id].isActive = isActive;
        surveys[id].question = question;
        surveys[id].results = new uint[](180);
        surveys[id].option1 = option1;
        surveys[id].option2 = option2;
        surveys[id].timestamp = block.timestamp;
    }

    function activateSurvey(uint id) external onlyOwner {
        require(surveys[id].id != 0, "Voting: Survey doesn't exists!");
        surveys[id].isActive = true;
    }
    ...
    function getSurveyData(uint surveyId) external view returns (uint[] memory) {
        require(surveys[surveyId].id != 0, "Voting: Survey doesn't exist");
        return surveys[surveyId].results;
    }

    function registerVoter(Voter calldata voter) external onlyWhitelisted {
        require(_voters[_msgSender()].walletAddress == address(0), "Voting: Voter already registered");
        _voters[_msgSender()] = voter;
    }

    function castVote(uint surveyId, bool answer) external onlyWhitelisted {

        Voter storage voter = _voters[_msgSender()];
        require(voter.walletAddress != address(0), "Voter isn't registered");
        require(surveyVoterResponse[surveyId][_msgSender()] == 0, "Voter: Already voted on this survey"); // TODO 
        ...
        surveyVoterResponse[surveyId][_msgSender()] = index;
        surveys[surveyId].results[index]++;
    }
