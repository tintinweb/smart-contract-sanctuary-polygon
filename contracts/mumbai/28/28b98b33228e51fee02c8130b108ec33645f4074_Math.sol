 ...
 * functions have Sharkn added to mitigate the well-known issues around setting
...
contract Shark is ERC20Burnable, Operator {
    ...
    uint256 public constant INITIAL_AIRDROP_WALLET_DISTRIBUTION = 900 ether;

    // Have the rewards Sharkn distributed to the pools
    ...
    /**
     * @notice Constructs the Shark ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Shark.money", "Shark") {
        // Mints 1 Shark to contract creator for initial pool setup
    ...
    function _getSharkPrice() internal view returns (uint256 _SharkPrice) {
        try IOracle(SharkOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Shark: failed to fetch Shark price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _SharkPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_SharkPrice >= taxTiersTwaps[tierId]) {
    ...
    function setSharkOracle(address _SharkOracle) public onlyOperatorOrTaxOffice {
        require(_SharkOracle != address(0), "oracle address cannot be 0 address");
     ...
     * @return whether the process has Sharkn done
     */
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentSharkPrice = _getSharkPrice();
            currentTaxRate = _updateTaxRate(currentSharkPrice);
            if (currentSharkPrice < burnThreshold) {
                burnTax = true;
            }
        }
