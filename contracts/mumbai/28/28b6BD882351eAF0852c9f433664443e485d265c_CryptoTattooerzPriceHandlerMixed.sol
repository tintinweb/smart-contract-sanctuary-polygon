...
import "./CryptoTattooerzPriceHandlerManual.sol";

/**
 * @title This is the CryptoTattooerz implementation for Mixed Price Handling used by tokens Direct Sale contract. Mixed
 ...
 * @author tazous
 */
contract CryptoTattooerzPriceHandlerMixed is CryptoTattooerzPriceHandlerManual, PriceHandlerProxy {
    ...
    CryptoTattooerzPriceHandlerManual(TOKEN_, tokenDecimals, tokenPriceUSD, tokenDecimalsUSD, 0, 0, 0)
    ...
    function getTokenCountFully() public view returns (uint256) {
        ...
        CryptoTattooerzPriceHandler priceHandlerProxy = getPriceHandler();
...
/**
 * @title This is the CryptoTattooerz implementation for Manual Price Handling used by tokens Direct Sale contract.
 * @dev Manual price calculation is based on statically defined Currency->USD Prices
 * @author tazous
 */
contract CryptoTattooerzPriceHandlerManual is CryptoTattooerzPriceHandler{
    ...
    CryptoTattooerzPriceHandler(TOKEN_) {
...
import "cryptotattooerz-utils/contracts/CurrencyUtils.sol";
import "cryptotattooerz-utils/contracts/NumberUtils.sol";
import "cryptotattooerz-utils/contracts/ProxyUtils.sol";
import "cryptotattooerz-utils/contracts/SecurityUtils.sol";
 ...
 * @author tazous
 */
abstract contract CryptoTattooerzPriceHandler is CurrencyHandler, AccessControlImpl {
...
abstract contract PriceHandlerProxy is ProxyDiamond {
    /** @dev Key used to reference the proxied CryptoTattooerzPriceHandler contract */
     ...
     */
    function getPriceHandler() internal view virtual returns(CryptoTattooerzPriceHandler) {
        return CryptoTattooerzPriceHandler(getProxy(PROXY_PriceHandler));
    }
    function _setPriceHandlerProxy(address priceHandlerAddress_) internal virtual {
        ...
        CryptoTattooerzPriceHandler priceHandler = CryptoTattooerzPriceHandler(priceHandlerAddress_);
...
contract ProxyDiamond {
    ...
    function getProxy(bytes32 key) public view returns (address) {
     ...
     * @param adminRole Role that proxy hub administrator should be granted if adminable is activated
     */
    function _setProxy(bytes32 key, address proxyAddress, bool nullable, bool updatable, bool adminable, bytes32 adminRole) internal {
     ...
     * @param adminable Can the proxied address be updated by a proxy hub administrator
     */
    function _setProxy(bytes32 key, address proxyAddress, bool nullable, bool updatable, bool adminable) internal {
