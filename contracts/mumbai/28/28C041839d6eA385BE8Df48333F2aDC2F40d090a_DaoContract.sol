    ...
    */
    ...
    function invest(uint256 _amount) external {
        ...
        if (investIndex == 0) {
            ...
            emit RewardNewEvent(block.timestamp, investIndex, "RewardNewStaticRewad", _leftAmount);
            //todo 全网首单还有结余
            skip.currentASA = 0;
        } else {
            //todo 25%全球全球静态分红
    ...
    function _distributeDayRewardForInviteRanking() private {
        ...
        if (_amount > _totalAmount) {
            ...
            rewardNew += _leftAmount;
            emit RewardNewEvent(block.timestamp, investEndIndex, "RewardNewInviteRanking", _leftAmount);
        }
        ...
        emit DistributeEvent(_day, _timestamp, "_distributeDayRewardForInviteRanking", _amount, _totalAmount, _num, _gas0 - _gas1, _list[0]);
    }
    ...
    function _distributeDayRewardForInvestRanking() private {
        ...
        if (_amount > _totalAmount) {
            uint256 _leftAmount = _amount - _totalAmount;
            rewardNew += _leftAmount;
            emit RewardNewEvent(block.timestamp, investEndIndex, "RewardNewInvestRanking", _leftAmount);
        }
    ...
    function _distributeWeekRewardForInvest() private {
        ...
        uint256 _gas1 = gasleft();
        emit DistributeEvent(_day, _timestamp, "_distributeWeekRewardForInvest", _amount, _amount, _num, _gas0 - _gas1, firstAddress);
        // 清空地址数组
    ...
    function _distributeWeekRewardForInvite() private {
        ...
        uint256 _gas1 = gasleft();
        emit DistributeEvent(_day, _timestamp, "_distributeWeekRewardForInvite", _amount, _amount, _num, _gas0 - _gas1, firstAddress);
        // 清空地址数组
    ...
    function _updateTotalADAList(uint256 _amount, uint256 _type) private {
        ...
        if (num == 0) {
            ...
            rewardNew += _leftAmount;
            emit RewardNewEvent(block.timestamp, investEndIndex, "RewardNewDao", _leftAmount);
    ...
    function _distributeInvitationReward(uint256 _amount, address _user) private {
        ...
        for (uint256 i = 0; i < _allReferrersNum; i++) {
            ...
            if (i < 12) {
                ...
                if (i == 0) {
                    ...
                    emit DistributeInvitationRewardEvent(_ferrer, block.timestamp, investIndex, "updatedDirectReferralReward", _rewardAmount);
                } else {
                    //二级间推要求有推荐人
                    if (i == 1 || (i > 1 && referrals[_ferrer].length > 2)) {
                        ...
                        _totalAmount += _rewardAmount;
                        emit DistributeInvitationRewardEvent(_ferrer, block.timestamp, investIndex, "updatedIndirectReferralReward", _rewardAmount);
                    }
                }
            }
        ...
        rewardNew += _leftAmount;
        emit RewardNewEvent(block.timestamp, investEndIndex, "RewardNewInvite", _leftAmount);
        //更新推荐人的最大网体
