...
contract StoryChain is ERC721, ERC721URIStorage, FunctionsClient, ConfirmedOwner {
  ...
  event PromptEntered(string prompt, uint256 nonce, address author);
  ...
  function createStory(string calldata prompt, string calldata title, uint256 category, uint256 pageLimit) public {
    ...
    if (category == CATEGORY_ALL) {
      ...
      emit PromptEntered(prompt, nonce, msg.sender);
    }

    nonce++;
  }

  function continueStory(string calldata prompt, uint256 storyNonce) public {
    ...
    if (s.category == CATEGORY_ALL) {
      ...
      emit PromptEntered(prompt, storyNonce, msg.sender);
    }
  }
