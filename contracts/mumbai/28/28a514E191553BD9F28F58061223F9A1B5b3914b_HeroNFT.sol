...
contract HeroNFT is NFT {
    ...
    function mintHeroTo(address recipient) public onlyOwner {
        internalMintTo(recipient);
    }

    function mintHeroesTo(address recipient, uint count) public onlyOwner  {
        internalMintPackTo(recipient, count);
    }
}
...
contract NFT is ERC721Enumerable, PullPayment, Ownable {
    ...
    constructor() ERC721("NFTGameItem", "HZI") {
        baseTokenURI = "";
    }

    function internalMintTo(address recipient) internal returns (uint256){
    ...
    function internalMintPackTo(address recipient, uint count) internal returns (uint256, uint256){
