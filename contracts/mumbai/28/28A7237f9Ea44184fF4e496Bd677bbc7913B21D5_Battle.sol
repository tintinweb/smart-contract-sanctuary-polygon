...
contract Battle is VRFManager, GameManager, AccessControlEnumerable {
    ...
    function detectCollision(
        ...
        int256 x = _positionFire.x * MAP_SCALE;
        ...
        for (uint256 i = 0; i < MAX_TIME_STEPS; i++) {
            ...
            y += Vy;
            Vy -= _gravity;
            reducedX = x / MAP_SCALE;

            if (x < 0 || x > maxX) return false;
            else if (_positionTarget.x == reducedX) {
                if (_positionTarget.y + TANK_HEIGHT >= y) {
                    return true;
                }
                return false;
            } else if (y <= ITerrain(terrainContract).terrain(reducedX)) {
                return false;
            }
        }
        return false;
        ...
        for (; i < MAX_TIME_STEPS; ) {
            unchecked {
                ...
                if (_positionFire.x < 0 || _positionFire.x >= maxX) break;
                    ...
                    if (_positionTarget.y + TANK_HEIGHT >= _positionFire.y) {
                        i++;
                        break;
                    }
                }
