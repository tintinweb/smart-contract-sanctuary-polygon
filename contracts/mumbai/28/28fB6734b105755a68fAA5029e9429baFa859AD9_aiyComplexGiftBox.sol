...
// @name AAVE Interest Yield (AIY) Complex Gift Box
contract aiyComplexGiftBox is ComplexGiftBox {
  ...
  // @name Constructor
  constructor(address _registry, address _recipient, address _from, address _conservator, uint256 _targetDate, uint256 _targetBalance, address _targetToken, uint256 _targetTokenBalance, address _lendingPoolAddressProvider, address _wethGateway)
  ComplexGiftBox(_registry, _recipient, _from, _conservator, _targetDate, _targetBalance, _targetToken, _targetTokenBalance) {
    //Setup AAVE integration
    pool = ILendingPoolAddressesProvider(_lendingPoolAddressProvider).getLendingPool();
