...
contract L2Portal is IPortal, NonblockingLzApp {
    ...
    function setSourceChainId(uint16 _id) external onlyRole(USER_ROLE) {
        sourceChainId = _id;
        emit SourceChainIdSet(_id);
    }

    function withdrawFunds(address to) external onlyRole(USER_ROLE) {
    ...
    function bridge(
        ...
        uint256 _nonce = lzEndpoint.getOutboundNonce(sourceChainId, address(this));

        emit Bridged(_nonce, sourceChainId, msg.sender, _amountToBridge);
    }
