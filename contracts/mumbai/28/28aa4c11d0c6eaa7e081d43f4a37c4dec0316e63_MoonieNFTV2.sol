...
// File @thirdweb-dev/contracts/interfaces/drop/[email protected]
...
// File contracts/ethereum/IMoonieNFTV2.sol
...
interface IMoonieNFTV2 is IERC721Upgradeable, IDropClaimCondition {
...
// File @thirdweb-dev/contracts/feature/interface/[email protected]
...
// File @thirdweb-dev/contracts/openzeppelin-presets/metatx/[email protected]
...
// File contracts/ethereum/MoonieNFTV2.sol
    ...
    IMoonieNFTV2
{
        ...
        for (uint256 i = 0; i < baseURIIndices.length; i += 1) {
            if (_tokenId < baseURIIndices[i]) {
                if (encryptedBaseURI[baseURIIndices[i]].length != 0) {
                    return
                        string(
                            abi.encodePacked(baseURI[baseURIIndices[i]])
                        );
                } else {
                    return
                        string(
                            abi.encodePacked(
                                baseURI[baseURIIndices[i]],
                                _tokenId.toString()
                            )
                        );
                }
        ...
        nonReentrant
    {
        ...
        if (msg.sender != owner()) {
            if (onlyWhitelisted == true) {
                ...
                uint256 ownerMintedCount = addressMintedBalance[msg.sender];
                require(
                    ownerMintedCount + _quantity <= nftPerAddressLimit,
                    "max NFT per address exceeded"
                );
            }
        }
        ...
        emit TokensClaimed(_msgSender(), _receiver, tokenIdToClaim, _quantity);
    }
    ...
    function setOnlyWhitelisted(bool _state) public onlyRole(MINTER_ROLE) {
    ...
    function withdraw() public payable onlyRole(MINTER_ROLE) {
        (bool hs, ) = payable(0xEe9a4bf9835f4e5AF36522B04b317E913F2213EC).call{
    ...
    function setNftPerAddressLimit(uint256 _limit)
        public
        onlyRole(MINTER_ROLE)
    {
