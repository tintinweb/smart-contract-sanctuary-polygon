...
contract TokenSale {
    ...
    receive() external payable {
        revert("This contract does not accept Matic. Please use the buyTokensWithUSDT function.");
    }
    ...
    function buyTokensWithUSDT(uint usdtAmount) public {
        ...
        require(token.transferFrom(msg.sender, address(this), usdtAmount), "USDT transfer failed");

        tokensSold += tokenAmount;
        emit Sold(msg.sender, tokenAmount);

        // Store the token amount to be claimed by the buyer
        tokenClaims[msg.sender] += tokenAmount*1e6;
