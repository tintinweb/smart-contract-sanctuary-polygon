...
import "./security/Whitelisted.sol";
    ...
    Distribution
{
    ...
    function stakeToken(
        ...
        uint totalAmountStaking = _stakeIdAmountToken[_StakePackageId];
        _stakeIdAmountToken[_StakePackageId] = totalAmountStaking.add(
            totalAmountStaking
        );
...
contract FactoryStakeToken is Administered {
    ...
    function activeStakeToken(uint256 _id, bool _status) external onlyUser {
...
contract Distribution is Whitelisted, Chainalysis, Utils {
    /// @dev SafeMath library
    using SafeMath for uint256;

    /// @dev Events
    event ClaimEvent(
        uint256 _toDistributiom,
        uint256 _fromDistributiom,
    ...
    /// @dev Variables for distribution
    struct Pay {
        uint256 amount;
        uint256 tokenId;
        address tokenReward;
    ...
    /// @dev mapping for distribution
    mapping(address => uint256) public countClaimable;
    mapping(address => mapping(uint256 => Pay)) public claimableList;

    constructor() {}

    /// @dev send token to distribution
    function Royalties(
        uint256 _to,
        uint256 _from,
        address _tokenAddressNFt,
        uint256 _tokenId,
        bool _withPercentage,
        uint256 _fullDeposit,
        uint256 _amount,
        address _tokenAddressToPay,
        address _addr
    ) internal returns (string memory) {
        /// @dev valid
        if (Sanctions(_addr) == true) {
            emit ClaimEvent(
                _to,
                _from,
                _addr,
                _tokenAddressToPay,
                false,
                "Chainalysis: Restricted to users.",
                0
            );
            return "Chainalysis: Restricted to users.";
        } else if (whitelisted(_addr) == false) {
            emit ClaimEvent(
                _to,
                _from,
                _addr,
                _tokenAddressToPay,
                false,
                "BlackList: Restricted to White list User.",
                0
            );
            return "BlackList: Restricted to White list User.";
        } else if (_addr != IERC721(_tokenAddressNFt).ownerOf(_tokenId)) {
            emit ClaimEvent(
                _to,
                _from,
                _addr,
                _tokenAddressToPay,
                false,
                "Distribution: Sender must be owner",
                0
            );
            return "Distribution: Sender must be owner";
        }

        uint256 _amountToReward = 0;

        /// @dev is with percentage
        if (_withPercentage) {
            _amountToReward = calculatePercentage(_fullDeposit, _amount);
        } else {
            _amountToReward = _amount;
        }

        /// @dev transfer reward the token to the sender
        transferToken(_tokenAddressToPay, _addr, _amountToReward);

        /// Save Regitro
        uint256 _count = countClaimable[_addr];
        claimableList[_addr][_count] = Pay(
            _amountToReward,
            _tokenId,
            _tokenAddressToPay,
            ERC20(_tokenAddressToPay).decimals(),
            block.timestamp
        );

        /// @dev increase count
        countClaimable[_addr] = _count.add(1);
    ...
    function DistributionTokens(
        ...
        /// @dev nft contract
        IERC721 nftContract = IERC721(_tokenAddressNFt);

        /// @dev get the owner of the NFT
        IERC721Enumerable optainTotal;
        optainTotal = IERC721Enumerable(_tokenAddressNFt);

        /// @dev get the total of NFTs
        uint256 _totalSupply = optainTotal.totalSupply();

        /// @dev get the owner of the NFT
        require(_to <= _totalSupply, "Distribution: Invalid range");

        /// @dev send the reward to all the owners of the NFTs
        for (uint256 tokenId = _to; tokenId < _from; tokenId++) {
            address addressToTransfer = nftContract.ownerOf(tokenId);
    ...
    /// @dev register the distribution
    function getListClaimable(
        address _addr
    ) public view returns (Pay[] memory) {
        uint256 _count = countClaimable[_addr];

        Pay[] memory _list = new Pay[](_count);

        for (uint256 i = 0; i < _count; i++) {
            _list[i] = claimableList[_addr][i];
        }
...
contract Whitelisted is Administered {
    ...
    function addWhitelist(address account) external virtual onlyUser {
        whitelist[account] = true;
        emit WhitelistAdded(account);
    }

    function removeWhitelist(address account) external virtual onlyUser {
