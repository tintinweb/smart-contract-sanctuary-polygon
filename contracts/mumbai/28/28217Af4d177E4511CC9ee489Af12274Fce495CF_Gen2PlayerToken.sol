...
/// @title Gen2PlayerToken athletes contract.
/// @dev One division, 1 Gen2PlayerToken derived from genesis NFT image.
contract Gen2PlayerToken is ERC721, AccessControl {
    ...
    mapping(uint256 => uint256) public nftToImageId;

    /// @notice divison_Id => image id => bool
    /// @dev Prevents having 2 NFT of the same image in the same division
    mapping(uint256 => mapping(uint256 => bool)) public isImageInDivision;
    ...
    event Gen2PlayerTokenIsMinted(
        uint256 _playerImageId, uint256 _divisionId, uint256 _gen2PlayerTokenId, address _user); 
    ...
    constructor() ERC721("LeagueDAO Fantasy League", "XXXXX") {
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _setupRole(MINTER_ROLE, _msgSender());
    }

    /// @dev Method is overwriten to resolve inheretance conflict 
        ...
        external onlyRole(MINTER_ROLE) 
    {
        ...
        require(
            nomoNFT.cardImageToExistence(_playerImageId) == true, 
            'Impossible to create NFT from nonexisting image');

        require(
            isImageInDivision[_divisionId][_playerImageId] == false,
            'Impossible to have two NFT from one image in one division');
        
        _mint(_user, _tokenId); 
        isImageInDivision[_divisionId][_playerImageId] = true;
        nftIdtoDivisionId[_tokenId] = _divisionId;      // NFT belongs to division
        nftToImageId[_tokenId] = _playerImageId; // NFT has reference to images
        imageIdToAllNftIds[_playerImageId].push(_tokenId); // Store all players NFT derived from images
        emit Gen2PlayerTokenIsMinted(_playerImageId, _divisionId, _tokenId, _user) ;
    }
    
    /// @notice Grant minter role
    /// @param _addr Address to grant minter role
    function grantMinterRole(address _addr) external onlyRole(DEFAULT_ADMIN_ROLE){
        _grantRole(MINTER_ROLE, _addr);
    }
    
    /// @notice Revoke minter role
    /// @param _addr Address to revoke minter role
    function revokeMinterRole(address _addr) external onlyRole(DEFAULT_ADMIN_ROLE){
        _revokeRole(MINTER_ROLE, _addr);
    }

    /// @notice Sets genesesis nft contract address
