...
// File: presaleTeretto.sol


pragma solidity 0.8.19;

    contract Presale is IERC777Recipient, ReentrancyGuard {

    using Strings for uint256;
    address public owner;
    ERC777 private crowdsaleToken;
    ...
    event LiqudityAdded(address from, address to, uint256 maticAmount, uint256 maticRaised);

    constructor (uint256 _rate, address payable _lp, ERC777 _crowdesaleToken) {
        IERC1820Registry(0x1820a4B7618BdE71Dce8cdc73aAB6C95905faD24)
            .setInterfaceImplementer(address(this), keccak256("ERC777TokensRecipient"), address(this));
        rate = _rate;
        lp = payable(_lp);
        crowdsaleToken = _crowdesaleToken;
        owner = msg.sender;
    }
    ...
    function TokenBalance() public view returns(uint) {
        return IERC777(crowdsaleToken).balanceOf(address(this));
    }

    function buyTokens(address beneficiary) public payable nonReentrant {
        require(!hasPurchased[beneficiary], "Address has already purchased tokens");
        require(msg.value >= 1 ether && msg.value <= 10 ether, "The min is 1 MATIC, the max is 10 MATIC");
        uint256 tokenAmount = getTokenAmount(msg.value);
        maticRaised = maticRaised + msg.value;
        deliverTokens(beneficiary, tokenAmount);
        emit Purchased(beneficiary, msg.value, tokenAmount);
        forwardTokens(msg.value);
        hasPurchased[beneficiary] = true;
    }


    function deliverTokens(address beneficiary, uint256 tokenAmount) internal {
        ERC777(crowdsaleToken).transfer(beneficiary, tokenAmount);
    }
    ...
    function forwardTokens(uint256 maticAmount) internal {
        require(msg.value > 0);
        (bool success, ) = lp.call{value: maticAmount}("");
        require(success, "Failed to send Matic to AMM");
        emit LiqudityAdded(address(this), lp, maticAmount, maticRaised);
    }
