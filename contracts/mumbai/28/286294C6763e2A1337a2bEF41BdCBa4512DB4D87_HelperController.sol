...
contract HelperController is   Ownable  , AccessControl , Pausable{
  ...
  /// @dev Add an account to the admin role. Restricted to admins.
    ...
    function updateProposal ( uint256 index , address token  , uint256 fundsRequested , string memory details , bool premium , uint256 duration , uint256 profit , uint256 slice , uint256 fundsAllocatedAfterTax  , uint256 score) public whenNotPaused
    {
        require( msg.sender == proposaldb.getProposal(index).applicant , "E1");        
        proposaldb.updateProposal( index , token , fundsRequested , details , premium  ,duration ,  profit ,  slice ,  fundsAllocatedAfterTax  ,  score   );
    }
