  ...
  EIP712
{
  ...
  mapping(bytes32 => bool) internal _burnedTicket;
  ...
  function mint(Ticket calldata ticket, bytes calldata signature) public {
    ...
    require(!_ticketUsageHistory[ticket.id][_msgSender()], "Used tickets");
    require(verifyTicket(ticket, signature), "Unauthorized");
  ...
  function burnTicket(bytes memory signature) public {
    require(_msgSender() == _meishiType.author, "Unauthorized");
    require(!_burnedTicket[keccak256(signature)], "Already burned");
    _burnedTicket[keccak256(signature)] = true;
  }
...
interface ICrypteenMeishi is IMetaContext, IERC721Metadata, IERC721Enumerable {
  ...
  function burnTicket(bytes calldata signature) external;
}
