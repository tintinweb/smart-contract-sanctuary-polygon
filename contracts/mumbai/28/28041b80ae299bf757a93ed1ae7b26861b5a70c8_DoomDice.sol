...
contract DoomDice 
{
    ...
    uint256 constant TARGET_POT_SIZE = 0.002 ether;
    ...
    function mainPotRoll() private 
    {
        ...
        if(numberOfWinners == 0)
        {
            ...
            for(uint256 i = 0; i < participatingPlayers.length; i++)
            {
                ...
                amountReturned += (MIN_BET * numberOfNumbersInPot);
                payable(participatingPlayers[i]).transfer(MIN_BET * numberOfNumbersInPot);
            }

            emit MainPotNoWinners(Dice_Number, amountReturned, participatingNumberOfPlayers);
            return;
        }
    ...
    function newDiceRoll(uint8 playerGuess, address playerAddress, uint256 value) private returns(bool, uint8, uint8) 
    {
        ...
        if(didWin)
        {
            ...
            playerUsedNumbers[playerAddress].push(playerGuess);
    ...
    function resetPlayers() private {
        for (uint256 i = 0; i < participatingPlayers.length; i++) {
            delete playerUsedNumbers[participatingPlayers[i]];
