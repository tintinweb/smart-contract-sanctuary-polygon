...
// File: @openzeppelin/contracts-upgradeable/token/ERC721/IERC721.sol
...
// File: RaceKingdomNft/exchange.sol
...
contract RaceKingdomExchange is OrderState, TransferSafe {
    ...
    IRocketKingdomNft private _rocketKingdomNFT;
    address public weth;
        ...
        if (order.key.sellAsset.assetType == AssetType.ERC721 || order.key.buyAsset.assetType == AssetType.ERC721) {
            if (order.orderType == 1 || order.orderType == 3) {
                if (!isStore) {
                    ...
                    require(order.key.sellAsset.token == address(_rocketKingdomNFT), "invalid sell asset");
                    nftLazyMint(
                ...
                if (!isStore) {
                    ...
                    require(order.key.buyAsset.token == address(_rocketKingdomNFT), "invalid buy asset");
                    nftLazyMint(
    ...
    function transferErc20Fee(
        ...
        if (remaining > 0) {
            IERC20(token).safeTransferFrom(buyer, _seller, remaining);
        }
    }
    ...
    function setMintableStore(address newMintableStore) external onlyOwner {
        ...
        _rocketKingdomNFT = IRocketKingdomNft(newMintableStore);
    ...
    function NftStore() external view returns(address){
        return address(_rocketKingdomNFT);
    }
