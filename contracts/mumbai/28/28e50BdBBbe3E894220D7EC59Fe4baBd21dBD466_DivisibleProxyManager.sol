...
contract DivisibleProxyManager is Ownable {
  ...
  function divide(address _originAddress, uint256 _tokenId, uint256 _totalSupply, string memory _name, string memory _symbol) external returns(uint256) {
    ...
    emit Divide(msg.sender, _originAddress, _tokenId, divisibleProxy, divisibleCount);

    IERC721(_originAddress).safeTransferFrom(msg.sender, divisibleProxy, _tokenId);
...
contract Divisible is ERC20Upgradeable, ERC721HolderUpgradeable {
    ...
    function initialize(
        address _curator, address _originAddress, uint256 _tokenId, uint256 _totalSupply, string memory _name, string memory _symbol
