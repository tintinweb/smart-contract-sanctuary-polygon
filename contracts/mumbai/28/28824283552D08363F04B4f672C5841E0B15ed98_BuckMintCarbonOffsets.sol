...
// SPDX-FileCopyrightText: 2021 BuckMint Labs
//
...
import './interfaces/IBuckMintCarbonOffsetsFactory.sol';
import './interfaces/IBuckMintContractRegistry.sol';
import './CarbonProjectTypes.sol';
import './CarbonProjectVintageTypes.sol';
import './BuckMintCarbonOffsetsStorage.sol';
import './CarbonOffsetBatchesTypes.sol';

/// @notice Implementation contract of the BCO2 tokens (ERC20)
/// These tokenized carbon offsets are specific to a vintage and its associated attributes
/// In order to mint BCO2s a user must deposit a matching CarbonOffsetBatch
/// @dev Each BCO2 contract is deployed via a Beacon Proxy in `BuckMintCarbonOffsetsFactory`
contract BuckMintCarbonOffsets is ERC20Upgradeable, IERC721Receiver, BuckMintCarbonOffsetsStorage {
    // ----------------------------------------
    //      Events
    // ----------------------------------------
    ...
    /// @dev modifier checks whether the `BuckMintCarbonOffsetsFactory` is paused
    /// Since BCO2 contracts are permissionless, pausing does not function individually
    modifier whenNotPaused() {
        address BuckMintCarbonOffsetsFactoryAddress = IBuckMintContractRegistry(contractRegistry).buckMintCarbonOffsetsFactoryAddress();
        bool _paused = IPausable(BuckMintCarbonOffsetsFactoryAddress).paused();
        require(!_paused, 'Paused BCO2');
        _;
    }

    modifier onlyAllowed() {
        address BuckMintCarbonOffsetsFactoryAddress = IBuckMintContractRegistry(contractRegistry).buckMintCarbonOffsetsFactoryAddress();
        bool isAllowed = IBuckMintCarbonOffsetsFactory(BuckMintCarbonOffsetsFactoryAddress).allowlist(msg.sender);
    ...
    /// @notice Burn BCO2 on behalf of a user. msg.sender does not require approval
    /// by the account for the burn to be successfull. This function is exposed so it
    /// can be utilized in cross-chain transfers of BCO2 where we want to burn the
    /// BCO2 in the source chain but not retire it.
    /// @param account The user for whom to burn BCO2
    /// @param amount The amount to burn.
    function bridgeBurn(address account, uint256 amount) external virtual whenNotPaused onlyAllowed {
        _burn(account, amount);
    }

    /// @notice Mint BCO2 on behalf of a user. This function is exposed to
    /// be called by authorized message bridge systems and utilized for
    /// cross-chain transfers of BCO2 where we want to mint the BCO2 in the
    /// source chain.
    /// @param account The user for whom to mint BCO2
    /// @param amount The amount to mint.
    function bridgeMint(address account, uint256 amount) external virtual whenNotPaused onlyAllowed {
    ...
    function name() public view virtual override returns (string memory) {
        ...
        return string(abi.encodePacked('BuckMint Protocol: BCO2-', globalProjectId, '-', vintageName));
    }
    ...
    function symbol() public view virtual override returns (string memory) {
        ...
        string memory vintageName;
        (globalProjectId, vintageName) = getGlobalProjectVintageIdentifiers();
        return string(abi.encodePacked('BCO2-', globalProjectId, '-', vintageName));
    }
    ...
    function getAttributes() public view virtual returns (ProjectData memory, VintageData memory) {
        address pc = IBuckMintContractRegistry(contractRegistry).carbonProjectsAddress();
        address vc = IBuckMintContractRegistry(contractRegistry).carbonProjectVintagesAddress();

        VintageData memory vintageData = ICarbonProjectVintages(vc).getProjectVintageDataByTokenId(projectVintageTokenId);
    ...
    function onERC721Received(
        ...
        (uint256 gotVintageTokenId, uint256 quantity, RetirementStatus status) = ICarbonOffsetBatches(msg.sender).getBatchNFTData(tokenId);
        require(gotVintageTokenId == projectVintageTokenId, 'Error: non-matching NFT');
        require(status == RetirementStatus.Confirmed, 'BatchNFT not yet confirmed');

        minterToId[from] = tokenId;
        /// @dev multiply tonne quantity with decimals
        quantity = quantity * 10 ** decimals();

        require(getRemaining() >= quantity, 'Error: Quantity in batch is higher than total vintages');

        IBuckMintCarbonOffsetsFactory bco2Factory = IBuckMintCarbonOffsetsFactory(
            IBuckMintContractRegistry(contractRegistry).buckMintCarbonOffsetsFactoryAddress()
        );
        address bridgeFeeReceiver = bco2Factory.bridgeFeeReceiverAddress();
        ...
        if (bridgeFeeReceiver == address(0x0)) {
            ...
            (uint256 feeAmount, uint256 feeBurnAmount) = bco2Factory.getBridgeFeeAndBurnAmount(quantity);
            _mint(from, quantity - feeAmount);
            address bridgeFeeBurnAddress = bco2Factory.bridgeFeeBurnAddress();
    ...
    function checkWhiteListed(address collection) internal view virtual returns (bool) {
        if (collection == IBuckMintContractRegistry(contractRegistry).carbonOffsetBatchesAddress()) {
            return true;
        } else {
            return false;
        }
    ...
    /// @dev Returns the remaining space in BCO2 contract before hitting the cap
    function getRemaining() public view returns (uint256 remaining) {
        uint256 cap = getDepositCap();
        remaining = cap - totalSupply();
    }

    /// @dev Returns the cap for BCO2s based on `totalVintageQuantity`
    ...
    function convertAmountToEvent(uint256 amount) internal returns (uint256 retirementEventId) {
        retiredAmount[msg.sender] -= amount;

        address certAddr = IBuckMintContractRegistry(contractRegistry).carbonOffsetBadgesAddress();
        retirementEventId = IRetirementCertificates(certAddr).registerEvent(msg.sender, projectVintageTokenId, amount, true);
    }

    /// @notice Burn BCO2 on behalf of a user. msg.sender needs to be approved by
    ...
    /// @notice Retirement/Cancellation of BCO2 tokens (the actual offsetting),
    /// which results in the tokens being burnt
    function retire(uint256 amount) public virtual whenNotPaused returns (uint256 retirementEventId) {
        retirementEventId = _retire(msg.sender, amount);
    }

    /// @dev Allow for pools or third party contracts to retire for the user
    /// Requires approval
    function retireFrom(address account, uint256 amount) external virtual whenNotPaused returns (uint256 retirementEventId) {
        _spendAllowance(account, msg.sender, amount);
        retirementEventId = _retire(account, amount);
    }

    /// @dev Internal function for the burning of BCO2 tokens
    function _retire(address account, uint256 amount) internal virtual returns (uint256 retirementEventId) {
        _burn(account, amount);

        // Track total retirement amount in BCO2 factory
        address BCO2FactoryAddress = IBuckMintContractRegistry(contractRegistry).buckMintCarbonOffsetsFactoryAddress();
        IBuckMintCarbonOffsetsFactory(BCO2FactoryAddress).increaseTotalRetired(amount);

        // Register retirement event in the certificates contract
        address certAddr = IBuckMintContractRegistry(contractRegistry).carbonOffsetBadgesAddress();
        retirementEventId = IRetirementCertificates(certAddr).registerEvent(account, projectVintageTokenId, amount, false);
    ...
    function mintCertificateLegacy(
        ...
        _mintCertificate(retiringEntityString, beneficiary, beneficiaryString, retirementMessage, retirementEventIds);
    }
    ...
    /// @notice Retire an amount of BCO2s, register an retirement event
    ...
    function transfer(address recipient, uint256 amount) public virtual override validDestination(recipient) whenNotPaused returns (bool) {
...
contract BuckMintCarbonOffsetsStorage {
...
struct VintageData {
    ...
    /// the same project, and helps build the corresponding BCO2 name and symbol.
    string name;
...
// SPDX-FileCopyrightText: 2022 BuckMint Labs
//
// SPDX-License-Identifier: UNLICENSED

// If you encounter a vulnerability or an issue, please contact <[email protected]> or visit security.buckMint.earth
pragma solidity =0.8.9;

interface IBuckMintCarbonOffsetsFactory {
...
interface IBuckMintContractRegistry {
    ...
    function buckMintCarbonOffsetsFactoryAddress() external view returns (address);
...
interface ICarbonOffsetBatches {
    function getConfirmationStatus(uint256 tokenId) external view returns (RetirementStatus);

    function getBatchNFTData(uint256 tokenId) external view returns (uint256, uint256, RetirementStatus);
}
...
interface ICarbonProjects is IERC721Upgradeable {
    function getProjectId(uint256 tokenId) external view returns (string memory projectId);
...
// If you encounter a vulnerability or an issue, please contact or
pragma solidity =0.8.9;
