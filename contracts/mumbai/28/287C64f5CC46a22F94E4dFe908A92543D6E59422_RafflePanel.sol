...
contract RafflePanel {
    ...
    function createRaffle(uint256 tokenId, uint256 ticketPrice, uint256 maxTickets) external {
        require(maxTickets > 0, "Invalid max tickets.");
        require(ticketPrice > 0, "Invalid ticket price.");
        require(IERC721(msg.sender).ownerOf(tokenId) == msg.sender, "You must own the token.");

        uint256 raffleId = uint256(keccak256(abi.encodePacked(block.timestamp, msg.sender, tokenId)));
        ...
        emit RaffleCreated(raffleId, tokenId, ticketPrice, maxTickets, msg.sender);
    }
