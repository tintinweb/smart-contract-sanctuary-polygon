...
contract Episodes is Multicall, Permissions, BatchMintMetadata, ERC1155Drop {
    ...
    // Verifique se o usuário possui o token do episódio
    require(cards.balanceOf(msg.sender, _tokenId) > 0, "You do not own this Episode");
    ...
    // Verifique se o usuário possui todos os cartões necessários para este episódio
    for (uint i = 0; i < E1C1Requirements[_tokenId].length; i++) {
        uint256 cardBalance = cards.balanceOf(msg.sender, E1C1Requirements[_tokenId][i]);
        require(cardBalance >= 10, "You don't have all the Cards for this Episode");
    }

    // Queime o token do episódio
    burnedTokens[msg.sender][_tokenId] = true;
    super._burn(msg.sender, _tokenId, 1);
    emit Burned(msg.sender,_tokenId);

    // Cunhe o token correspondente
