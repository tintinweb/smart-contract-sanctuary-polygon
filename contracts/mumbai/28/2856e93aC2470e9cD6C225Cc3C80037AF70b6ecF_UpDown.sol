...
contract UpDown is ReentrancyGuard, Pausable{
    ...
    // user + epoch => Round
    mapping(address => mapping(uint256 => Round)) rounds;

    // id => Epoch + User
    mapping(uint256 => RoundDetailsById) roundDetailsById;
    mapping(address => uint256) currentUserEpoch;
    ...
    uint256 constant MAX_BET_MASK = 80;
    ...
    function placeBet(
        ...
        uint256 diceWinAmount = getDiceWinAmount(amount, betMask);
        uint256 diceWin = 0;
        if (diceNum < betMask) {
            diceWin = diceWinAmount;
        }
        // Send the funds to gambler.
        sendFunds(msg.sender, diceWin == 0 ? 0 wei : diceWin);
