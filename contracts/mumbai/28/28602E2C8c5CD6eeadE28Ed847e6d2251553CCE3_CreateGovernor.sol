...
contract CreateGovernor is Ownable {

    event CreateNewGovernor(address token, address locker, address governor);
    ...
    function createGovernor (
        ...
        GovernorContract governor = new GovernorContract(Token(token), TimeLock(payable(locker)), quorumPercentage, votingPeriod, votingDelay);

        emit CreateNewGovernor(address(token), locker, address(governor));
