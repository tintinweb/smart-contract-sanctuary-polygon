...
abstract contract H2HCompetition is Staker {
    ...
    /// @notice Used for points update and H2H winners evaluation. ID of a next division that will be calculated
    ...
    function saveUserWeekScore(address _user, uint256 _points) internal {
        uint256 season = seasonId.current();
        userWeeklyStats[_user][season][weekTracker.current()].points = _points;
        userSeasonStats[_user][season].totalPoints += _points;
    }
...
abstract contract Staker is StakeValidator {
    ...
    function deleteFromBench(uint256 _tokenId) private {
        ...
        uint256[] storage bench = stakedBenchPlayers[season][_msgSender()];
        for (uint256 i = 0; i < bench.length; ++i)
            if (bench[i] == _tokenId) {
                if (i == bench.length - 1) {
                    bench.pop();
                } else {
                    uint256 lastPlayer = bench[bench.length - 1];
                    bench[i] = lastPlayer;
                    bench.pop();
                }
                break;
...
abstract contract UsersNDivisions is GameProgress, AccessControlUpgradeable {
    ...
    // ____ Temporary mapping for the auction contract ____

    // Season ID => user => division ID
    mapping(uint256 => mapping(address => uint256)) public userDivisionIds;
    ...
    function shuffleUsers(uint256 _numberToShuffle) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        for (uint256 i = shuffledUserNum; i < newShuffledUserNum; ++i) {
            ...
            // Saving of user division ID
            userDivisionIds[season][user] = i / divisionSize;
        }
    ...
    function getDivisionUsers(uint256 _season, uint256 _division) public view returns (address[] memory division) {
    ...
    // ____ Temporary extra getter for the auction contract ____

    function UserToDivision(address _user) external view addedUser(_user) returns (uint256) {
        return userDivisionIds[seasonId.current()][_user];
    }
