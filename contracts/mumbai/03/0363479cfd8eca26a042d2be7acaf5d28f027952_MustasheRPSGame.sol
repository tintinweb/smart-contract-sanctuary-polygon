...
contract MustasheRPSGame {
    ...
    function createGame(uint256 _betAmount, bytes calldata _signature) external {
        ...
        bytes32 messageHash = keccak256(abi.encodePacked(gameId, _betAmount, msg.sender));
    ...
    function joinGame(uint256 _gameId, bytes calldata _signature) external {
        ...
        if (game.creator != msg.sender) {
            ...
            bytes32 messageHash = keccak256(abi.encodePacked(_gameId, game.betAmount, msg.sender));
            address signer = messageHash.recover(_signature);
            require(signer == msg.sender, "Invalid signature");
    ...
    function play(uint256 _gameId, bytes32 _choiceHash, bytes calldata _signature) external {
        Game storage game = games[_gameId];
        ...
        bytes32 messageHash = keccak256(abi.encodePacked(_gameId, _choiceHash));
        address signer = messageHash.recover(_signature);
        require(signer == msg.sender, "Invalid signature");
