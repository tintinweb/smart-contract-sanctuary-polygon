...
contract BitHotelRoomCollectionFactory is AccessControl {
    ...
    event Deployed(address newContract, address sender, string name, string symbol, uint256 replicas, uint256 price);
    ...
    function deployAuction(
        ...
        WLAcollection newCollection = new WLAcollection(name, symbol, address(this), replicas_);
        address newCollectionAddress = address(newCollection);
        uint256 index = totalDeployed() + 1; // mapping index starts with 1
        ...
        for (uint256 i = 1; i <= replicas_; i++) {
            ...
            auction.setupAuction(newCollectionAddress, i, startingBid, duration);
        }

        emit DeployedTimeAuction(newCollectionAddress, msg.sender, name, symbol, replicas_, startingBid, duration);
    ...
    function deploy(
        ...
        collectionAddress[0] = newCollectionAddress;
        category[0] = ERC721MarketplaceUpgradeable.Category(1);

        _deployedCollectionsIndex[newCollectionAddress] = index;
        _deployed.push(newCollectionAddress);
        ...
        for (uint256 i = 1; i <= replicas_; i++) {
            ...
            marketplace.addERC721Listing(_msgSender(), newCollectionAddress, i, collectionPrice);
        }

        newCollection.setController(msg.sender);

        _revokeRole(CURRATED_ROLE, _msgSender());

        emit Deployed(newCollectionAddress, msg.sender, name, symbol, replicas_, collectionPrice);
    ...
    function bulkWhitelist(address[] calldata whitelists) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        require(whitelists.length > 0, "BitHotelRoomCollectionFactory: empty address");
    ...
    function bulkRemoveWhitelist(address[] calldata whitelists) external onlyRole(DEFAULT_ADMIN_ROLE) {
        // solhint-disable-next-line reason-string
        require(whitelists.length > 0, "BitHotelRoomCollectionFactory: empty whitelist");
        for (uint256 i = 0; i < whitelists.length; i++) {
            address whitelisted_ = whitelists[i];
    ...
    function setWhitelist(address whitelist) public onlyRole(DEFAULT_ADMIN_ROLE) {
        // solhint-disable-next-line reason-string
        require(whitelist != address(0), "BitHotelRoomCollectionFactory: whitelisted is the zero address");
    ...
    function removeWhitelist(address whitelist) public onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        require(whitelist != address(0), "BitHotelRoomCollectionFactory: whitelisted is the zero address");
        // solhint-disable-next-line reason-string
        require(hasRole(WHITELISTED_ROLE, whitelist), "BitHotelRoomCollectionFactory: address not whitelisted");

        uint256 index = whitelistIndex(whitelist);
        // solhint-disable-next-line reason-string
        require(index > 0, "BitHotelRoomCollectionFactory: no whitelist index found in mapping");

        uint256 arrayIndex = index - 1;
        // solhint-disable-next-line reason-string
        require(arrayIndex >= 0, "BitHotelRoomCollectionFactory: array out-of-bounds");
...
contract WLAcollection is IWLAcollection, AccessControl, ERC721Enumerable, ERC721URIStorage, ERC2981PerTokenRoyalties {
    struct Room {
        uint256 number;
        string floorId;
        string roomTypeId;
...
contract EnglishAuction is AccessControl {
    ...
    function setupAuction(
        ...
        WLAcollection(_nftAddress).transferFrom(_msgSender(), address(this), _tokenId);
