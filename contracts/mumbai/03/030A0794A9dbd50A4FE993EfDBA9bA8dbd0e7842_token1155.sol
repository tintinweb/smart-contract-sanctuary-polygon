...
contract token1155 is ERC1155URIStorageUpgradeable{
    ...
    function mint(address _to, uint256 _tokenID, uint256 _amount, bytes memory _data) external virtual {
        require(!minted[_tokenID],"Token already minted");
        _mint(_to, _tokenID, _amount, _data);
        minted[_tokenID] = true;
    }

    function setURI(uint256 _tokenID,string memory _URI) external virtual {
        _setURI(_tokenID, _URI);
    }

    function isMinted(uint256 _tokenID) external virtual view returns(bool) {
        if(minted[_tokenID])
            return true;
        else 
            return false;    
