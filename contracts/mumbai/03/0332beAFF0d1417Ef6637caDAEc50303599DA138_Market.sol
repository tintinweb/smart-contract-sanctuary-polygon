...
contract Market is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {
  ...
  constructor(string memory _newbaseURI, address _acceptedToken) ERC721('We play For Peace', 'WPFP') {
    // URL Base
    ...
    nonReentrant
  {
    ...
    require(
      IERC20(acceptedToken).allowance(_msgSender(), address(this)) >= _amount,
      'ERC720: Insufficient balance to offer amount to buy NFT'
    );
  ...
  function buy(
    ...
    require(
      IERC20(acceptedToken).allowance(_msgSender(), address(this)) >= _tokenMeta[_tokenId].price,
    ...
    IERC20(acceptedToken).transferFrom(
      msg.sender,
      tokenSeller,
      sellerProceeds.sub(feesByTransfer)
    );
  ...
  function _payTxFee(uint256 _tokenId, bool sellByAuctions) internal returns (uint256) {
    ...
    if (tokenSeller != owner()) {
      ...
      if (sellByAuctions) {
        ...
        require(
          IERC20(acceptedToken).balanceOf(_msgSender()) >= feeSellToken,
          'ERC720: insufficient balance to transfer the NFT'
        );
    ...
    if (tokenSeller != owner() && tokenSeller != artistOf(_tokenId)) {
      ...
      if (sellByAuctions) {
        ...
        require(
          IERC20(acceptedToken).balanceOf(_msgSender()) >= royaltyFee,
          'ERC720: insufficient balance to transfer the NFT'
        );
