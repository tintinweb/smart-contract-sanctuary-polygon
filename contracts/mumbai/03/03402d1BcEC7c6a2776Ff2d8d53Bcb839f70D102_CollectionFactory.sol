...
contract CollectionFactory is Initializable, OwnableUpgradeable, PausableUpgradeable {
    ...
    function removeCollection(uint256 id) public whenNotPaused {
        ...
        (uint256 index, bool exits) = findElementInArray(collection, _userCollections[msg.sender]);
    ...
    function findElementInArray(address collection, uint256[] memory arr) private pure returns (uint256, bool) {
        bytes32 encodedElement = keccak256(abi.encode(collection));
        uint256 len = arr.length;
        for (uint256 i = 0; i < len; i++) {
            if (encodedElement == keccak256(abi.encode(collection))) {
                return (i, true);
            }
        }
        return (0, false);
