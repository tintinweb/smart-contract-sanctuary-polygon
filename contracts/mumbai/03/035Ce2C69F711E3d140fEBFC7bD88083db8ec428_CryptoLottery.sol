...
contract CryptoLottery is ERC20 {
     ...
     */
        for (uint256 i = 0; i < _tickets[_rounds.length - 1].length; i++) {
            ...
            if (
                ...
                _tickets[_rounds.length - 1][i].eth_reward = _ticket_price * 3;
                _tickets[_rounds.length - 1][i].token_reward = 3000 * 10**18;
                _token_reward += _tickets[_rounds.length - 1][i].token_reward;

                _fee_value += (_fee * (_ticket_price * 3)) / 100;
            ...
            if (
                ...
                _tickets[_rounds.length - 1][i].eth_reward =
                    _ticket_price *
                    200;
                _tickets[_rounds.length - 1][i].token_reward = 200000 * 10**18;
                _token_reward += _tickets[_rounds.length - 1][i].token_reward;

                _fee_value += ((_fee * (_ticket_price * 200)) / 100);
    ...
    function getLastRound() external view returns (Round[] memory rounds) {
        Round[] memory _rounds_array;
        _rounds_array[_rounds.length - 1] = _rounds[_rounds.length - 1];
