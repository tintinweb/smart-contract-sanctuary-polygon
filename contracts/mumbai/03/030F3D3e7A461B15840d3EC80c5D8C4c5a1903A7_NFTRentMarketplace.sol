...
contract NFTRentMarketplace is VRFConsumerBaseV2, ConfirmedOwner, IERC721Receiver {
    ...
    function fulfillRandomWords(
        ...
        // If the request was for renting an item, execute the rent
        if (s_requests[_requestId].randomWords.length == 2) {
            executeRent(_requestId, _randomWords[0]);
        }
    }
    ...
    function rentItem(uint256 _categoryId) public {
        require(pools[_categoryId].isActive, "Pool with the given category ID does not exist or is not active");
        uint256 requestId = _requestRandomWords();
        s_requests[requestId].randomWords.push(_categoryId);
    }

    function executeRent(uint256 _requestId, uint256 _randomWord) internal {
        uint256 poolId = s_requests[_requestId].randomWords[1]; // Retrieve the pool ID
        Pool storage pool = pools[poolId];

        require(pool.availableItems.length > 0, "No items available for rent");

        // Use the random index to select an item from the pool
        uint256 index = _randomWord % pool.availableItems.length;
        ...
        // Check that the item has been rented successfully
        ...
        // Check that the item has been removed from the availableItems array
        require(pool.availableItems.length == 0 || pool.availableItems[index] != selectedItemId, "Item not removed from available items");
    }
}