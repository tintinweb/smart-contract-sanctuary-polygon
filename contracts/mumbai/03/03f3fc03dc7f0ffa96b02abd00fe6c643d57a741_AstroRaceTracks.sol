    ...
    GnosisSafeUpgradeable
{
    ...
    event Mint (address indexed account, uint indexed tokenType, uint tokenId);
    ...
    function initialize(address _admin) public virtual initializer {
        __Ownable_init_unchained();
        __ERC721_init("Astro RaceTracks", "ASRT");
    ...
    function setCapacities(uint[] memory _typeIds, uint[] memory _capacities) external onlyOwner() {
        require(_typeIds.length == _capacities.length, "Mismatched data");
        uint _trackCount = trackCount();

        for (uint i = 0; i < _typeIds.length; i ++) {
            uint _typeId = _typeIds[i];
            uint _capacity = _capacities[i];
            require(_typeId < _trackCount, "Invalid model ID");
            require(tracks[_typeId].supply <= _capacity, "New capacity should be equal or greater than the current supply");

            tracks[_typeId].capacity = _capacity;
        }
    }

    function validateTypeId(uint _tokenType) internal view returns(bool) {
        return (_tokenType < trackCount());
    }

    function setTypeUri(uint _typeId, string memory _metafileUri) external onlyOwner {
        require (validateTypeId(_typeId), "Invalid type ID");
    ...
    function _mintToken(address _to, uint _tokenType) internal {
        require (validateTypeId(_tokenType), "Invalid class ID or type ID");
        require(tokenIds[_to][_tokenType] == 0, "Already minted for the type");

        Track storage track = tracks[_tokenType];
        require(track.supply < track.capacity, "Exceeds capacity");
        track.supply ++;
        ...
        emit Mint(_to, _tokenType, tokenId);
    }
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        if (_tokenId > 0 && _tokenId <= totalSupply()) {
            uint tokenType = tokenTypes[_tokenId];
            return tracks[tokenType].metafileUri;
        }
        return "";
    }
}
