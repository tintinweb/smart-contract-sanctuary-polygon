...
// File: contracts/GameContract721.sol
...
contract Astrobirdz is ERC721 {
  ...
  uint256 constant NUM_CLASSES = 4;
    ...
    // Starts From 0
    uint private constant UNIQUENFTS = 6;
    ...
    constructor() ERC721("Astrobirdz", "ABZ") {}
    ...
    function setSeed(uint _s) public {
        _seed = _s;
    } 
    ...
    //TODO RANDOM NFT AND ATTRIBUTES
    ...
    function randRarity(uint _randomNum, uint _num) internal view returns(uint) {
    ...
    function selectAttrbiutes(Attributes memory attr) internal view returns(Attributes memory){
        ...
        if(_class == Class.Common) {
            // uint rand =  uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, _seed))) % 34;
            // uint rand = randRarity(2002, 34);
            attr.rarity = 0;
            attr.babyAPY = 
            ...
            // uint rand =  uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, _seed))) % 15;
            // uint rand = randRarity(2002, 34);
            // rand = rand + 35;
            attr.rarity = 1;
           attr.cannon = randRarity(230, 15) + 35;
            ...
            // uint rand =  uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, _seed))) % 25;
            // rand = rand + 50;
            attr.rarity = 2;
    ...
    function upgrade(uint _tokenId) public {
        require(ownerOf(_tokenId) == msg.sender, "not owner");
        require(_level[_tokenId] == 1, "not level 1");

        _level[_tokenId] = 2;
    }

    // function percent() public view returns(uint){
    //     uint totalamount = 50;
    //     return totalamount.div(100).mul(80);
    // }
         ...
         string memory json = Base64.encode(
            bytes(string(
                abi.encodePacked(
                    ...
                    '{"trait_type": "Unique Attribute", "value": "', _tokenIdToAttributes[tokenId].uniqueAttribute, '"},',
