...
contract GasMonster is ERC721A, ChainlinkInfrastructureClient, TokenURIResolver, PausableMint, Withdrawable {
	...
	constructor(
		...
		uint256 _priceOfNFT
	) ChainlinkInfrastructureClient(_chainlinkConfig, _chainlinkVRFConfig) ERC721A('DynamicNFT', 'DNMC') {
    ...
    function transferFrom(
       ...
       super.transferFrom(from, to, tokenId);
        ownersNFTs[to].push(tokenId);
        for(uint i = 0; i<ownersNFTs[from].length; i++){
            if(ownersNFTs[from][i]==tokenId){
                ownersNFTs[from][i] = ownersNFTs[from].length - 1;
                ownersNFTs[from].pop();
            }
    }
    }
