...
contract Item is ERC1155, Ownable, ReentrancyGuard {
    ...
    function addItems(
        ...
        for (uint256 i = 0; i < totalSupplies.length; ++i) {
            itemTypes.push(firstId + i);
            items[firstId + i].minted = 0;
            items[firstId + i].totalSupply = totalSupplies[i];
            items[firstId + i].priceUsd = priceUsds[i];
            items[firstId + i].paused = false;
        }
    }
    ...
    function withdrawERC20(
        ...
        for (uint256 i = 0; i < tokenIds.length; ++i) {
            require(
                ERC20Tokens[tokenIds[i]].tokenContract != IERC20(address(0)),
                "token not set"
            );

            uint256 balance = ERC20Tokens[tokenIds[i]].tokenContract.balanceOf(
                address(this)
            );
            require(balance > 0, "balance can't be zero");

            uint256 founderAmount = balance.mul(4).div(10);
            ERC20Tokens[tokenIds[i]].tokenContract.transfer(
                0xfE38200d6206fcf177c8C2A25300FeeC3E3803F3,
                founderAmount
            );
