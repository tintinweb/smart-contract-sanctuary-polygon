...
interface IMyLimeStore is IUpgradeable {
    ...
    function getEventUpdatesById(
        uint112 fullId
    ) external view returns (uint32[] memory);
}


// File contracts/utils/Converter.sol
...
contract Converter is IConverter {
    function indexOf(
        string memory what,
        string memory where
    ) public pure returns (uint256 found) {
    ...
    function toHexDigit(uint8 d, bool upperCase) public pure returns (bytes1) {
        if (0 <= d && d <= 9) {
            return bytes1(uint8(bytes1("0")) + d);
        } else if (10 <= uint8(d) && uint8(d) <= 15) {
            return
                bytes1(uint8(upperCase ? bytes1("A") : bytes1("a")) + d - 10);
        }
    ...
    function toHexString(
        ...
        for (uint256 i = 0; i < count; ++i) {
            b = a % 16;
            res[count - i - 1] = toHexDigit(uint8(b), upperCase);
            a /= 16;
        }
        return string(res);
    }

    function numberFromAscII(bytes1 b) public pure returns (uint8 res) {
        if (b >= "0" && b <= "9") {
    ...
    function isLowerCaseAndDigits(
        ...
        if (b[0] < "a" || b[0] > "z") return false;
        for (uint256 i = 1; i < b.length; i++) {
            if (
                !((b[i] >= "a" && b[i] <= "z") || (b[i] >= "0" && b[i] <= "9"))
            ) {
                return false;
            }
        }
...
// File contracts/utils/Encoder.sol
    ...
    OwnableUpgradeable
{
    ...
    function methodIdByName(string memory name) public pure returns (uint8) {
        // method names and ID are hardcoded because to change them
        // we need to release a new version of the Encoder
        if (isEqual(name, "create")) return 1;
        if (isEqual(name, "update")) return 2;
        return 0;
    }

    function methodNameById(uint mId) public pure returns (string memory) {
        return mId == 1 ? "create" : mId == 2 ? "update" : "";
    }
    ...
    function uintToTag(
        ...
        string memory hexTag = toHexString(tag, true);
        if (bytes(hexTag).length < 14) {
            uint padding = 14 - bytes(hexTag).length;
            for (uint i = 0; i < padding; i++) {
                hexTag = string(abi.encodePacked("0", hexTag));
            }
        }
        return
            string(
                abi.encodePacked(
                    substring(hexTag, 0, 2),
                    ":",
                    substring(hexTag, 2, 4),
                    ":",
                    substring(hexTag, 4, 6),
                    ":",
                    substring(hexTag, 6, 8),
                    ":",
                    substring(hexTag, 8, 10),
                    ":",
                    substring(hexTag, 10, 12),
                    ":",
                    substring(hexTag, 12, 14)
                )
            );
    }
    ...
    function decode(
        uint256 data
    ) public pure override returns (string[5] memory) {
        uint8 method = getMethod(data);
        if (method > 2) revert UnsupportedMethod();
        return [
            StringsUpgradeable.toString(uint8(data)),
            methodNameById(method),
            toHexString(uint96(data >> 32), false),
            StringsUpgradeable.toString(uint24(data >> 128)),
            method == 1
                ? uintToTag(uint56(data >> 152))
                : StringsUpgradeable.toString(uint32(data >> 152))
        ];
    }

    function getVersion(uint256 data) public pure override returns (uint8) {
        return uint8(data);
    }

    function getMethod(uint256 data) public pure override returns (uint8) {
        return uint8(data >> 8);
    }

    function getCustomerId(uint256 data) public pure override returns (uint16) {
        return uint16(data >> 16);
    }

    function getObjectId(uint256 data) public pure override returns (uint96) {
        return uint96(data >> 32);
    }

    function getId(uint256 data) public pure override returns (uint112) {
        return uint112(data >> 16);
    }

    function getUserId(uint256 data) public pure override returns (uint24) {
        return uint24(data >> 128);
    }

    function getUpdateId(uint256 data) public pure override returns (uint32) {
        if (uint8(data >> 8) != 2) revert UnsupportedMethod();
        return uint32(data >> 152);
    }

    function getTag(uint256 data) public pure override returns (uint56) {
        if (uint8(data >> 8) != 1) revert UnsupportedMethod();
        return uint56(data >> 152);
    }
}