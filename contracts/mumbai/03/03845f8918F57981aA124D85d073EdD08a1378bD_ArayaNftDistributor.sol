 ...
 */
contract ArayaNftDistributor is AccessControl, Pausable, Withdrawable, ReentrancyGuard {
    ...
    */
    ...
    constructor(address arayaNftContractAddress, address fearContractAddress) {
        arayaNftContract = IArayaNft(arayaNftContractAddress);
    ...
    */
    function buyTokens(uint256 id, uint256 amount) external whenNotPaused nonReentrant {
        ...
        uint256 userBalance = fearContract.balanceOf(_msgSender());
        uint256 allowance = fearContract.allowance(_msgSender(), address(this));
        ...
        fearContract.transferFrom(_msgSender(), address(this), fullAmount);
        arayaNftContract.initialMint(caller, id, amount, "");

        emit TokenBought(caller, id, amount, tokenPrice);
    }

    function setPrice(uint256 id, uint256 tokenPrice, uint256 limitPerWallet) external onlyRole(MANAGER_ROLE) {
        price[id] = tokenPrice;
