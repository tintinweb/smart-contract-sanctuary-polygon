...
import "./interfaces/IFarmingGalileanPolygonBridge.sol";
    ...
    ERC721EnumerableUpgradeable
{
    ...
    uint16 public constant GEN_ZERO_CUTOFF_ID = 15000;

    uint16 public numRubyMints;
    uint16 public maxRubyMints;

    IFarmingGalileanPolygonBridge bridge;
    ...
    function bridgeMint(address user, uint256 tokenId)
        external
        onlyGameContract
    {
        _mint(user, tokenId);
    ...
    function burn(uint256 tokenId) external onlyGameContract {
        // assume ownership check has already been done
        _burn(tokenId);
    }

    function bridgeToL1(uint256 tokenId) external onlyEOA {
        require(_msgSender() == ownerOf(tokenId), "Not token owner");
        require(tokenId < GEN_ZERO_CUTOFF_ID, "Only gen zero can bridge back");
        bridge.withdrawWithMetadata(tokenId);
    }
    ...
    function tokenURI(uint256 tokenId)
        public
        view
        override(ERC721Upgradeable, IFarmingGalilean)
    ...
    function setContracts(
        ...
        bridge = IFarmingGalileanPolygonBridge(bridgeAddress);
        gameContracts[randomizerAddress] = true;
        gameContracts[farmlandAddress] = true;
        gameContracts[bridgeAddress] = true;
    }
...
interface IFarmingGalilean is IERC721EnumerableUpgradeable {
    ...
    function GEN_ZERO_CUTOFF_ID() external view returns (uint16);
    function numGenZeroGalileansOfUser(address user) external view returns (uint16);
    function burn(uint256 tokenId) external;
    function bridgeMint(address user, uint256 tokenId) external;
    function tokenURI(uint256 tokenId) external view returns (string memory);
}
...
interface IFarmingGalileanPolygonBridge {
