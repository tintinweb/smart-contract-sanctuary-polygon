...
contract SuiBirdsNFT is TinyERC721 {
    ...
    constructor() TinyERC721("SuiBirdsNFT", "SuiBirds", 10) {}

    function PublicMint(uint256 _amt) public payable {
        if (_amt <= 0 || _amt > maxMintPerTx) revert MaxMintPerTx();
        if (totalSupply() + _amt > maxSupply) revert MaxSupplyReached();
        if (msg.value < mintPrice * _amt) revert InvalidPrice();
    ...
    function tokenURI(
        ...
        return "https://suibirds.s3.eu-central-1.amazonaws.com/suibirds/metadata/main.json";
    }
