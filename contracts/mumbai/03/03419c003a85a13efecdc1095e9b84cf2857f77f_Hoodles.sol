...
contract Hoodles is Ownable, ReentrancyGuard, ERC721ABurnable {
    ...
    function setGiveawayAddresses(
        ...
        require(
            _addresses.length == _tokenAmounts.length,
            "array size mismatch"
        );
        for (uint256 i = 0; i < _addresses.length; i++) {
    ...
    function claimGiveaway() external nonReentrant {
        require(
            !isGiveawayClaimed[msg.sender],
            "user already claimed giveaway"
        );
        ...
        returns (string memory)
    {
        ...
        if (!revealStatus) {
            return
                string(
                    abi.encodePacked(hiddenTokenURI, _tokenId.toString(), json)
                );
        } else {
            return
                string(
