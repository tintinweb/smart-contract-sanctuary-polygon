...
/**
 * @title NFT factory contract. We are using it to deploy new NFT collections.
 */
contract ERC721Factory is Administrated {
     ...
     */
    function createTestERC721(
        string memory _name,
        string memory _symbol,
        uint256 tokenprice
    ) external onlyAdmin {
        TestERC721 _new = new TestERC721(
            _name,
            _symbol,
            owner(),
            weth,
...
/// @title Red Kite NFT contract based on OpenZeppelin contracts
contract TestERC721 is ERC721URIStorage, ERC721Enumerable, Administrated {
    ...
    /// @dev Prevent to buy more than allowed
    modifier lessThanMaxId() {
        require(totalSupply() < maxTokenId, "Max token ID reached");
        _;
    }
     ...
     * @param _tokenprice token's price
     */
    constructor(
        string memory _name,
        string memory _symbol,
    ...
    /**
     * @dev Token buy functionality
     * @param _tokenURI Token URI String
     * @return _tokenId - ID of new token
     */
    function _buy(string memory _tokenURI)
        private
     ...
     */
    function mintBatches(string[] memory _tokenURIs) external onlyAdmin {
