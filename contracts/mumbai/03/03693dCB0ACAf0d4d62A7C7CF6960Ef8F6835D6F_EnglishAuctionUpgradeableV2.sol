...
contract EnglishAuctionUpgradeableV2 is
    Initializable,
    OwnableUpgradeable,
    UUPSUpgradeable
{
    ...
    function verSubastasPasadas() public view returns (bytes32[] memory) {
        bytes32[] memory _pastAuctionIds = new bytes32[](pastAuctions.length);
        for (uint256 i = 0; i < pastAuctions.length; i++) {
            _pastAuctionIds[i] = pastAuctions[i];
        }
        return _pastAuctionIds;
    }

    function version() public pure returns (string memory) {
        return "English Auction Version 2.0";
    }
