...
contract DoubleUp is Ownable, ReentrancyGuard {
    ...
    function enterTesting(uint256 entry, uint256 immunityLevel, address msgSender) payable external nonReentrant {
        require(refundsActive == false, "Contract retired.");
        require(entry >= minimumEntry && entry <= maximumEntry, "Out of range entry.");
        require(immunityLevel <= immunityMax, "Immunity too high.");
        ...
        if(entriesLength >= 4){
            ...
            if(next.next == address(0)){
                ...
                current.next = next.next; // Clip
                entries[current.next].next = msgSender;
            }
        }else{
            entriesLength++;
            if(head == address(0)){
        ...
        tail = msgSender;
        uint immunityLevelFinal = immunityLevel > 0 ? (immunityLevel * 2) + (entry / 2) : 0;
        entries[msgSender] = Entry(address(0), collateral, immunityLevelFinal);
        emit Entered(msgSender, collateral, immunityLevelFinal);
    }
