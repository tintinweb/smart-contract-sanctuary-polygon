...
contract DragonCrossbreed is BaseAuctionReceiver, ReentrancyGuard, RandomConsumerBase {
    ...
    function processERC721(address from, uint tokenId, bytes calldata data) 
        ...
        require(dt.breedsCount(tokenId) < _platformPrices.length, 
    ...
    whenNotPaused {
        ...
        address owner = dragonToken.ownerOf(myDragonId);
        require(
            owner == _msgSender(), 
        ...
        require(
            dragonToken.breedsCount(myDragonId) < _platformPrices.length && 
            dragonToken.breedsCount(anotherDragonId) < _platformPrices.length, 
            "DC: crossbreed limit exceeded"
        );
    ...
    function fulfillRandomness(bytes32 requestId, uint randomness) internal override {
        ...
        DragonCreator(dragonCreatorAddress()).giveBirth(
            ...
            newGenes, 
            _owners[breedDetails.dragon1Id]
        );

        if (isLocked(breedDetails.dragon2Id) && 
            dragonToken.breedsCount(breedDetails.dragon2Id) >= _platformPrices.length) {
            _transferTokenToHolder(breedDetails.dragon2Id);
        }
        
        delete _requestData[requestId];
        delete _owners[breedDetails.dragon1Id];
    }
