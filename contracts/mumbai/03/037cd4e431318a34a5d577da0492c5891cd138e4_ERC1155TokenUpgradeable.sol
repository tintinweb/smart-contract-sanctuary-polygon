...
import "../ONFT//ONFT1155Core.sol";

// import "@openzeppelin/contracts/proxy/transparent/ProxyAdmin.sol";
// import "@openzeppelin/contracts/proxy/transparent/TransparentUpgradeableProxy.sol";

contract ERC1155TokenUpgradeable is ONFT1155Core,IERC1155Mint,ERC1155Upgradeable, ERC1155ExtensionsUpgradeable, ERC1155BurnableUpgradeable, ERC1155SupplyUpgradeable {
    ...
    function initialize(string memory name_, string memory symbol_,address owner,address _lzEndpoint) public initializer {
        ...
        __ERC1155Supply_init();
         __ONFT1155Core_init( _lzEndpoint);
        ...
        returns(uint256)
    {
     ...
     function supportsInterface(bytes4 interfaceId)
        public
        view
        override(ERC1155Upgradeable,ONFT1155Core)
        returns (bool)
    {
        ...
        override(ERC1155Upgradeable, ERC1155SupplyUpgradeable)
    {
     ...
     function _creditTo(uint16, address _toAddress, uint[] memory _tokenIds, uint[] memory _amounts, string[] memory uris) internal virtual  {
        _mintBatch(_toAddress, _tokenIds, _amounts, "");
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            _setURI(_tokenIds[i], uris[i]);
    ...
    function _sendBatch(address _from, uint16 _dstChainId, bytes memory _toAddress, uint[] memory _tokenIds, uint[] memory _amounts, address payable _refundAddress, address _zroPaymentAddress, bytes memory _adapterParams) internal virtual override {
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
           _uris[i]=uri( _tokenIds[i]);
            //uris.push(uri( _tokenIds[i]));
        }
  ...
  function _nonblockingLzReceive(uint16 _srcChainId, bytes memory _srcAddress, uint64 _nonce, bytes memory _payload) internal virtual override {
        ...
        (bytes memory toAddressBytes, uint[] memory tokenIds, uint[] memory amounts, string[] memory uris) = abi.decode(_payload, (bytes, uint[], uint[],string[]));
        address toAddress;
        assembly {
            toAddress := mload(add(toAddressBytes, 20))
        }

        _creditTo(_srcChainId, toAddress, tokenIds, amounts,uris);
...
abstract contract ONFT1155Core is Initializable,NonblockingLzApp, ERC165, IONFT1155Core {
    // constructor(address _lzEndpoint) NonblockingLzApp(_lzEndpoint) {}

    function __ONFT1155Core_init(address _lzEndpoint) internal onlyInitializing {
