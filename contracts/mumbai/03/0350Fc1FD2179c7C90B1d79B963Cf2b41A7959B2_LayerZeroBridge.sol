...
contract LayerZeroBridge is ILayerZeroReceiver, IBridge {
    ...
    function _deployNonNativeWrapper(DeployExternalParams memory deployData, bytes32 collectionId) internal {
        systemContext.getContractFactory().createNonNativeContractInstanceByHash(
            deployData.wrapperBytecodeHash, abi.encode(systemContext, deployData.originalCollection), collectionId, deployData.collectionName, deployData.owner, deployData.originalCollection
        );
    }
    ...
    function _handleDeploy(DeployData memory deployData, uint16 srcChainId, bytes memory fromAddress, uint64 nonce) internal {
        ...
        bytes memory rawCallData = abi.encodeWithSelector(
            contractFactory.createNativeContractInstanceByHash.selector, deployData.bytecodeHash,
    ...
    function _handleERC721Bridge(ERC721BridgeData memory callData, uint16 srcChainId, bytes memory fromAddress, uint64 nonce) internal returns (address) {
        ...
        (address newAddress) = systemContext.getContractFactory().createNativeContractInstanceByHash(
    ...
    function deployNativeCollection(
        ...
        for (uint256 i = 0; i < deploymentParams_.length; i++) {
            ...
            if(deploymentParams_[i].chainId == systemContext.chainId()) {
                systemContext.getContractFactory().createNativeContractInstanceByHash(
...
contract ContractFactory {
    ...
    * @dev Creates native contract instance for whitelisted byteCode.
     ...
     */
    function createNativeContractInstanceByHash(bytes32 bytecodeHash, bytes memory constructorParams, bytes32 collectionId, string calldata name, address owner) external onlyBridgeRole returns(address) {
        ...
        require(contractNative[contractName[bytecodeHash]], "Contract registered as non-native");
    ...
    function _registerNonNativeToken(address newContract, bytes32 bytecodeHash, bytes32 collectionId, string calldata name, address owner, address originalAddress) internal {
    ...
    /**
    * @dev Creates NON-native contract instance for whitelisted byteCode.
     * @param bytecodeHash - hash of a contract bytecode.
     * @param constructorParams - encoded constructor params.
     * @param collectionId - unique collection identifier.
     * @param name - human readable collection name.
     * @param owner - owner of the collection.
     */
    function createNonNativeContractInstanceByHash(bytes32 bytecodeHash, bytes memory constructorParams, bytes32 collectionId, string calldata name, address owner, address originalAddress) external onlyBridgeRole returns(address) {
        require (hashRegistered[bytecodeHash], "Contract not registered");

        address newContract = _deploy(contractsBytecode[bytecodeHash], constructorParams, collectionId);

        require(contractNative[contractName[bytecodeHash]] == false, "Contract registered as native");
        systemContext.getAccessControlList().grantNonNativeTokenRole(newContract);
        _registerNonNativeToken(newContract, bytecodeHash, collectionId, name, owner, originalAddress);
