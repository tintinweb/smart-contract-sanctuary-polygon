...
// File: contracts/NFTuobitaMatic.sol
...
contract Oubita is ERC721, Ownable {
   ...
   //Cost in MATIC
    ...
    uint public costMatic_Whitelist = 200000000000000;
    uint public costMatic_Special = 1500000000000000;
    uint public costMatic_Normal = 2000000000000000;

    //cost in TOKEN NATIVO  = ETHER EN RINKEBY

    uint public costEth_Whitelist= 200000000000000;
    uint public costEth_Special = 1500000000000000;
    uint public costEth_Normal= 2000000000000000;
    ...
    function mint(uint tipo, uint _mintAmount) public payable{
        ...
        if(msg.sender != owner()){
            ...
            if(onlyWhitelisted == true){
                    ...
                    multiTokensTransfer(pagos, tipo, _mintAmount, counter);
    ...
    function multiTokensTransfer(address _to, uint _type, uint _amount, uint _counter) public payable{
        if(_type == 1){
            if(onlyWhitelisted == true){
                ...
                require(msg.value >= costMatic_Whitelist * _amount, "You dont have enough balance");
            }else if(_counter < specialSale){
                
                require(msg.value >= costMatic_Special * _amount, "You dont have enough balance");
            }else if(_counter >= specialSale){
                
                require(msg.value >= costMatic_Normal * _amount, "You dont have enough balance");
            }
            
        }else if (_type == 2){
            if(onlyWhitelisted == true){
                uint precio = costEth_Whitelist * _amount;
                IERC20(tokenContract).transferFrom(msg.sender, _to, precio);
            }else if(_counter < specialSale){
                uint precio = costEth_Special * _amount;
                IERC20(tokenContract).transferFrom(msg.sender, _to, precio);
            }else if(_counter >= specialSale){
                uint precio = costEth_Normal * _amount;
                IERC20(tokenContract).transferFrom(msg.sender, _to, precio);
            }
        }
    } 
