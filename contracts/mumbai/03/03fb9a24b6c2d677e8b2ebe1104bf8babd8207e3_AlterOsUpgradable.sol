...
// File: contracts/AlterOsUpgradable.sol
...
interface NftContractInterFace {
    function getCreator(uint256) external returns (address);
}

contract AlterOsUpgradable is
    Initializable,
    OwnableUpgradeable,
    ReentrancyGuardUpgradeable
{
    ...
    function createListing(
        ...
        if (!isErc721) {
            ...
            for (uint256 i; i < tokenIds.length; ) {
                require(
                    IERC1155Upgradeable(nftAddress).balanceOf(
                        msg.sender,
                        tokenIds[i]
                    ) >= amount,
            ...
            for (uint256 i; i < tokenIds.length; i++) {
                require(
                    IERC721Upgradeable(nftAddress).ownerOf(tokenIds[i]) ==
                        msg.sender,
                    "ERC721 token does not belong to the author."
                );
            }
        }
        address _creator = NftContractInterFace(nftAddress).getCreator(
            tokenIds[0]
        );
    ...
    function buyAssetFromListing(
        ...
        _toRemove(listingId);
        emit ListingSold(msg.sender, listingId, block.timestamp, assetId);
    }
        ...
        nonReentrant
    {
        ...
        _toRemove(listingId);
        emit ListingCancelled(listingId, block.timestamp, assetId);
    }
