...
contract NakshMarketplace is Ownable {
    ...
    */
    ...
    function updateSaleData(address _nftAddress, uint _tokenId) internal {
        uint leng = OnSaleNFTs.length;
        for (uint i = 0; i <= leng;) {
            if (OnSaleNFTs[i].nft.nftAddress == _nftAddress && OnSaleNFTs[i].nft.tokenId == _tokenId) {
                delete OnSaleNFTs[i];
            }
            unchecked {++i;}
        }
    ...
    function getSaleData(address _nft, uint256 _tokenId) external view returns (SaleData memory) {
    ...
    */
    ...
    function startAuction(address _nftAddress, uint _tokenId, uint _price, uint _auctionTime) external onlyOwnerOf(_nftAddress, _tokenId) returns (bool) {
        ...
        saleData[_nftAddress][_tokenId].nft = NakshNFT(_nftAddress).getNFTData(_tokenId);
