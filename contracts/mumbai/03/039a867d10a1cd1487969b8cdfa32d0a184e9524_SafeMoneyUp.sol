...
contract SafeMoneyUp is ERC20, Ownable {
    ...
    uint256 public adminBuyFee = 50; // 0.5% (50/10000)
    uint256 public adminSellFee = 50; // 0.5% (50/10000)
    uint256 public liquidityBuyFee = 200; // 2% (200/10000)
    uint256 public liquiditySellFee = 200; // 2% (200/10000)

    event TokenBought(uint256 eth, uint256 tokens);
    event TokenSold(uint256 eth, uint256 tokens);
    ...
    function buyTokens() public payable {
        ...
        uint256 totalFee = adminBuyFee + liquidityBuyFee;

        if (!excludedFromFees[msg.sender]) {
            uint256 ethForAdmin = (msg.value * adminBuyFee) / 10000;
            uint256 ethForLiquidity = (msg.value * liquidityBuyFee) / 10000;

            (bool sent,) = owner().call{value: ethForAdmin}("");
            require(sent, "ETH fees to admin failed");

            tokensToBuy = (tokensToBuy * (10000 - totalFee)) / 10000;
        }
    ...
    function sellTokens(uint256 tokensToSell) public {
        ...
        uint256 totalFee = adminSellFee + liquiditySellFee;

        _transfer(msg.sender, address(this), tokensToSell);

        if (!excludedFromFees[msg.sender]) {
            uint256 ethForAdmin = (ethAmount * adminSellFee) / 10000;
            uint256 ethForLiquidity = (ethAmount * liquiditySellFee) / 10000;

            uint256 tokensForAdmin = (tokensToSell * adminSellFee) / 10000;
            adminFeeTokens += tokensForAdmin;
            _transfer(address(this), owner(), tokensForAdmin);

            ethAmount = (ethAmount * (10000 - totalFee)) / 10000;
    ...
    function setAdminBuyFee(uint256 newAdminBuyFee) public onlyOwner {
        require(newAdminBuyFee >= 0 && newAdminBuyFee <= 1000, "Invalid fee value");
        adminBuyFee = newAdminBuyFee;
    }

    function setAdminSellFee(uint256 newAdminSellFee) public onlyOwner {
        require(newAdminSellFee >= 0 && newAdminSellFee <= 1000, "Invalid fee value");
        adminSellFee = newAdminSellFee;
    }

    function setLiquidityBuyFee(uint256 newLiquidityBuyFee) public onlyOwner {
        require(newLiquidityBuyFee >= 0 && newLiquidityBuyFee <= 1000, "Invalid fee value");
        liquidityBuyFee = newLiquidityBuyFee;
    }

    function setLiquiditySellFee(uint256 newLiquiditySellFee) public onlyOwner {
        require(newLiquiditySellFee >= 0 && newLiquiditySellFee <= 1000, "Invalid fee value");
        liquiditySellFee = newLiquiditySellFee;
    }
