...
import "./interfaces/GMX_IVaultUtils.sol";
import "./interfaces/GMX_IVaultPriceFeed.sol";

contract GMX_Vault is GMX_ReentrancyGuard, GMX_IVault {
    ...
    GMX_IVaultUtils public vaultUtils;
    ...
    function setVaultUtils(GMX_IVaultUtils _vaultUtils) external override {
        _onlyGov();
    ...
    function upgradeVault(address _newVault, address _token, uint256 _amount) external {
        _onlyGov();
        GMX_IERC20(_token).safeTransfer(_newVault, _amount);
    }
    ...
    function buyUSDG(address _token, address _receiver) external override nonReentrant returns (uint256) {
        ...
        GMX_IUSDG(usdg).mint(_receiver, mintAmount);
    ...
    function sellUSDG(address _token, address _receiver) external override nonReentrant returns (uint256) {
        ...
        GMX_IUSDG(usdg).burn(address(this), usdgAmount);
    ...
    function getMaxPrice(address _token) public override view returns (uint256) {
        return GMX_IVaultPriceFeed(priceFeed).getPrice(_token, true, includeAmmPrice, useSwapPricing);
    }

    function getMinPrice(address _token) public override view returns (uint256) {
        return GMX_IVaultPriceFeed(priceFeed).getPrice(_token, false, includeAmmPrice, useSwapPricing);
    }
    ...
    function getTargetUsdgAmount(address _token) public override view returns (uint256) {
        uint256 supply = GMX_IERC20(usdg).totalSupply();
    ...
    function _transferIn(address _token) private returns (uint256) {
        ...
        uint256 nextBalance = GMX_IERC20(_token).balanceOf(address(this));
    ...
    function _transferOut(address _token, uint256 _amount, address _receiver) private {
        GMX_IERC20(_token).safeTransfer(_receiver, _amount);
        tokenBalances[_token] = GMX_IERC20(_token).balanceOf(address(this));
    }
    ...
    function _increasePoolAmount(address _token, uint256 _amount) private {
        ...
        uint256 balance = GMX_IERC20(_token).balanceOf(address(this));
