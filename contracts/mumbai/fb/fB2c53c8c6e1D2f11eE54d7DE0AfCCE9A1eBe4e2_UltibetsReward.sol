...
contract UltibetsReward is CustomAdmin {
    ...
    mapping(address => uint256) public claimedRewardAmount;
    mapping(address => uint256) public claimableRewardAmount;
    ...
    event ReleaseReward(address bettor, uint256 amount);
    event UpgradeTier(address bettor, uint256 reward);
    ...
    function updateRewardTier(uint256 _amount) external onlyDailyBets {
        ...
        if (
            ...
            totalBetAmount[tx.origin] >= thresholdByTier[currentTier + 1]
        ) {
            currentTierOfBettor[tx.origin] += 1;
            claimableRewardAmount[tx.origin] += rewardByTier[currentTier + 1];

            emit UpgradeTier(tx.origin, claimableRewardAmount[tx.origin]);
        }
    }
    ...
    function claimReward() external {
        require(
            IUltiBetsToken(ultiBetsToken).balanceOf(address(this)) >=
                claimableRewardAmount[msg.sender],
            "Deposit UTBETS to this contract."
        );

        uint256 reward = claimableRewardAmount[msg.sender];
        claimableRewardAmount[msg.sender] = 0;
        claimedRewardAmount[msg.sender] += reward;

        IUltiBetsToken(ultiBetsToken).transfer(msg.sender, reward);

        emit ReleaseReward(msg.sender, reward);
    }
