    ...
    **/
    ...
    function tax( uint256 taxAmount, address sender) internal {
        ...
        if(IUniswapV2Router02(ApeswapRouter).getAmountsOut(balanceOf(address(this)),path)[1] >= USDY_ThresholdLimit){
            ...
            (uint amount, uint leftAmount) = IReferralNFT(Admin.Refferal()).rewardDistribution(sender, IERC20(BUSD).balanceOf((address(this))));
            if(amount != 0){
                IERC20(address(this)).transfer(Admin.Refferal(),amount);
            }
            if(leftAmount != 0){
                IERC20(address(this)).transfer(Admin.Treasury(),leftAmount/2);
                IERC20(address(this)).transfer(Admin.TeamAddress(),leftAmount/2);
            }
