...
contract Trade {
    ...
    event TestEmitStr(string val);
    event TestEmitUnit(uint val);
    ...
    function getFees(uint paymentAmt, BuyingAssetType buyingAssetType, address buyingAssetAddress, uint tokenId) public view returns(Fee memory){
        ...
        uint256 buyerFee = percent(paymentAmt, buyerFeePermille);
        uint256 sellerFee = percent(paymentAmt, sellerFeePermille);
        uint256 royaltyFee = percent(paymentAmt, royaltyPermille);
        uint256 platformFee = buyerFee.add(sellerFee);
        uint256 price = paymentAmt.sub(buyerFee);
        uint256 assetFee = price.sub(sellerFee).sub(royaltyFee);
    ...
    function executeBidEmit(Order memory order) public returns(bool) {
        Fee memory fee = getFees(order.amount, order.nftType, order.nftAddress, order.tokenId);
        emit ExecuteBid(msg.sender , order.tokenId, order.qty, order.buyer);

        emit TestEmitUnit(fee.price);

        if(order.nftType == BuyingAssetType.ERC721) {
            emit TestEmitStr('721111');
        }
        else if(order.nftType == BuyingAssetType.ERC1155) {
            emit TestEmitStr('1165555');
        }
        else {
            emit TestEmitStr('last');
        }
        return true;
    }
