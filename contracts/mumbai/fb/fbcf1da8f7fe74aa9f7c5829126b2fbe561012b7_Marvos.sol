...
interface MarvosInterface {
    ...
    /**
     * @dev Create an offer with the provided offer data using contract token balance when available.
     *
     * Requirements:
     ...
     * - If `useBalance` is true, then `totalAmount` - `balance` is transferred (zero if `balance` is greater).
     */
    function createOffer(Offer calldata offer, bool useBalance) external payable;

    /**
     * @dev Pause or cancel an offer. Cancellations are non-reversible.
     *
     * Requirements:
     ...
     * - Bids cannot be placed on offers with any status value other than `Active`.
     */
    function updateOfferStatus(uint256 offerId, OfferStatus status) external;

    /**
     * @dev Place a compatible bid on an offer.
     *
     * Requirements:
