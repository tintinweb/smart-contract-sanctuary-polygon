...
contract BLKZToken is BEP20('BLKZ', 'BLKZ') {
    using SafeMath for uint256;
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "BLKZSwap::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "BLKZSwap::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "BLKZSwap::delegateBySig: signature expired");
     ...
     * @dev Block number must be a Blkzalized block or else this function will revert to prevent misinformation.
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "BLKZSwap::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "BLKZSwap::_writeCheckpoint: block number exceeds 32 bits");
...
contract Stakerv2 is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. BLKZs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that BLKZs distribution occurs.
        uint256 accRewardPerShare;   // Accumulated BLKZs per share, times 1e12. See below.
    ...
    // The Reward TOKEN!
    BLKZToken public rwd;
    // Dev address.
    address public devaddr;
    // BLKZ tokens created per block.
  //  uint256 public RewardPerBlock;
   ...
   // uint256 public intervalBlock = 86400;
    ...
    // The block number when BLKZ mining starts.
...
//         devaddr = 0x025c1667471685c323808647299e5DbF9d6AdcC9;
//         feeAddress = 0x025c1667471685c323808647299e5DbF9d6AdcC9;
//         startBlock = 47114495;
//     }
...
//add new pool _allocPoint - Multiplier , _withUpdate - true, _isFixed - , _emisson - per block reward, _interval - if fixed interval reward, _poolLimit - stak token count
    function add(uint256 _allocPoint, IBEP20 _lpToken, uint16 _depositFeeBP, bool _withUpdate, BLKZToken _rwdToken,uint256 _lockedPeriod,bool _isFixed,uint256 _emisson,uint256 _interval,uint256 _poolLimit) public  {
...
//Edit pool
    function set(uint256 _pid, uint256 _allocPoint, uint16 _depositFeeBP, bool _withUpdate,BLKZToken _rwdToken,uint256 _lockedPeriod,uint256 _interval,uint256 _poolLimit) public onlyOwner {
    ...
    // Deposit LP tokens to MasterChef for BLKZ allocation.
    ...
    // Safe rwd transfer function, just in case if rounding error causes pool to not have enough BLKZs.
