...
import "../Crowdfunding/IVaultCrowdfunding.sol";
import "../Crowdfunding/Calcs.sol";
...
contract Market is MarketEvents, Assistants, ReentrancyGuard, Calcs {
    ...
    address private crowdCollection;
    ...
    uint256 private feePanoram = 150; //150 is equal to 1.5%
    bool paused = false;
    ...
    constructor(address _tokenInfo, address token, address _mainVaultAddress, address _lendersRewards, 
    address _walletPanoram, address _crowdControl) Calcs(_crowdControl) {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(DEV_ROLE, msg.sender);
        ...
        permissions(token,_lendersRewards,_mainVaultAddress);
        }else{
            revert ("address Zero");
        }
    } 

    function crowdfunding(address _owner, uint256 _quantity ,uint256 _value, address _token) public isPaused validToken(_token) notZero(_value) nonReentrant {
        if(ICollectionNFT(crowdCollection).getPresaleStatus()){
            revert ("Presale Open");
        }
        LocalMint memory locals;
        (locals.pan, ) = calcFees(_value);
        locals.sell = _value + locals.pan;
        IERC20(_token).safeTransferFrom(msg.sender, address(this), locals.sell);
        
        (address vCrowdfunding,) = tokenInfo.getCrowdVaults(_token);
        
        locals.value = _value / _quantity;
        IVaultCrowdfunding(vCrowdfunding).deposit(_value, _token);
        IERC20(_token).safeTransfer(walletPanoram, locals.pan);
        uint256 _id = addRegistryCrowdFund(_owner, _value, _safeCastToUint32(_quantity));
        for(uint256 i=1; i <= _quantity;){
            locals._nftId = ICollectionNFT(crowdCollection).redeem(_owner, locals.value);
            emit NFTMinted(crowdCollection, locals._nftId, _owner);
            unchecked {
             ++i;
            }
        }

        emit Crowdfunding(_id, crowdCollection, _quantity,_value, _owner);
    }

    function mintingMortgage(address _collection, address _owner,uint256 _value) public isPaused onlyMortgage nonReentrant returns(uint256 _nftId){
        if(_collection == crowdCollection){
            revert ("Crowdfunding Collection not allowed");
        }
        if(ICollectionNFT(_collection).getPresaleStatus()){
            revert ("Presale Open");
        }
        _nftId = ICollectionNFT(_collection).redeem(_owner, _value);
        emit NFTMinted(_collection, _nftId, _owner);
    }

    function mintingPresaleMortgage(address _collection, address _owner,uint256 _value) public isPaused onlyMortgage nonReentrant returns(uint256 _nftId){
        if(_collection == crowdCollection){
            revert ("Crowdfunding Collection not allowed");
        }
        if(!ICollectionNFT(_collection).getPresaleStatus()){
        ...
        _nftId = ICollectionNFT(_collection).preSale(_owner, _value);
        emit NFTMinted(_collection, _nftId, _owner);
    }
    ...
    function minting(address _collection, address _owner, uint256 _value, uint256 _fee, address _token) public isPaused validToken(_token) nonReentrant {
        ...
        IVaultCrowdfunding(vCrowdfunding).deposit(sell, _token);
        IVaultRewards(lendersVault).deposit(lenders, _token);
        IERC20(_token).safeTransfer(walletPanoram, pan);
        
        uint256 _nftId = ICollectionNFT(_collection).redeem(_owner, sell);
    ...
    uint256 _fee, address _token) public isPaused validToken(_token) nonReentrant {
        ...
        IVaultCrowdfunding(vCrowdfunding).deposit(locals.sell, _token);
        IVaultRewards(lendersVault).deposit(locals.lenders, _token);
        IERC20(_token).safeTransfer(walletPanoram, locals.pan);
       
        for(uint256 i=1; i <= _amount;){
            locals._nftId = ICollectionNFT(_collection).redeem(_owner, locals.value);
    ...
    validToken(_token) nonReentrant {
        ...
        uint256 _nftId = ICollectionNFT(_collection).preSale(_owner, sell);
    ...
    uint256 _fee, address _token) public isPaused validToken(_token) nonReentrant {
        ...
        (vars.vCrowdfunding,) = tokenInfo.getCrowdVaults(_token);
        (locals.pan, locals.lenders) = calcFees(_fee);
        locals.sell = _value - _fee;
        IVaultCrowdfunding(vars.vCrowdfunding).deposit(locals.sell, _token);
        IVaultRewards(vars.lendersVault).deposit(locals.lenders, _token);
        IERC20(_token).safeTransfer(walletPanoram, locals.pan);

        for(uint256 i=1; i <= _amount;){
            uint256 _nftId = ICollectionNFT(_collection).preSale(_owner, locals.sell);
            emit NFTPresale(_collection, _nftId, _owner);
            unchecked {
             ++i;
            }
    ...
    function addRegistryCrowdFund(address wallet, uint256 _amount, uint32 _totalNFTsBuyed) internal returns (uint256 _id){
        uint256 _payPerSecond = calcInterestForSecond(_amount, ICrowdfundingControl(CrowdControl).getInterest());
        uint256 _totalToPaid = calcTotalPayInterest(_amount, ICrowdfundingControl(CrowdControl).getInterest());
        _id = ICrowdfundingControl(CrowdControl).addRegistry(wallet, _amount, _payPerSecond, _totalToPaid, _totalNFTsBuyed);
        ICrowdfundingControl(CrowdControl).addInfo(_id,wallet);
        return _id;
    }
    ...
    function setCrowdCollection(address _crowdCollection) public onlydev{
        crowdCollection = _crowdCollection;
    }

    function permissions(address _token, address _lenderRwards, address _mainVault) public onlydev validToken(_token) {
        IERC20(_token).approve(_lenderRwards, 2**255);
        IERC20(_token).approve(_mainVault, 2**255);
    }
...
abstract contract MarketEvents {
    ...
    event Crowdfunding(uint256 _crowdId, address _crowdCollection, uint256 _NFTQuantity, uint256 _value, address _owner);
