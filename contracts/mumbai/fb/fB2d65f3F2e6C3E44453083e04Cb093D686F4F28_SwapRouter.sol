    ...
    ManageableUpgradeable
{
    ...
    function swapTokenForToken(
        ...
        uint256 feeAmount = (_realAmountIn * BURN_PERC) / PRECISION;
    ...
    function swapTokenForETH(
        ...
        if (tokenIn == ROUTER.WETH()) {
            ...
            uint256 feeAmount = (adjustedAmountIn * BURN_PERC) / PRECISION;
    ...
    function swapETHForToken(
        ...
        if (tokenOut == ROUTER.WETH()) {
            ...
            uint256 feeAmount = (amountIn * BURN_PERC) / PRECISION;
    ...
    function _burn(uint256 feeAmount, address[] memory _path) internal {
        address tokenIn = _path[0];
        uint256 _firstFeeAmount = feeAmount / 3;
        uint256 _secondFeeAmount = feeAmount - _firstFeeAmount;
        uint256 _thirdFeeAmount = feeAmount - _secondFeeAmount;
        ...
        if (tokenIn == ROUTER.WETH()) {
            ...
            // BUY BACK FUNDS
            _safeTransfer(GROWTH, _secondFeeAmount + _thirdFeeAmount);
        } else {
            // BURN INPUT TOKEN
            IERC20(tokenIn).transfer(DEAD_ADDRESS, _firstFeeAmount);
            ...
            if (wethPath.length >= 2) {
                ...
                IERC20(tokenIn).transfer(GROWTH, _thirdFeeAmount);
            }
        }
    }
