...
contract Auction is Ownable {
    ...
    mapping(string => DataStructure.autionStruct) public auctionIdToAuction;
    mapping(string => address[]) public autionIdToParticipants;
    mapping(address => uint256) public userCancelOfferNFT;

    mapping(string => DataStructure.Listing) public Listings;
    mapping(string => DataStructure.Offer) public Offers;
    ...
    modifier checkStateOfAution(string memory _auctionId) {
        require(
    ...
    function getTokenIdFromListing(string memory _listingId)
        internal
        view
        returns (uint256)
    {
    ...
    {
        DataStructure.Offer storage offerInstance = Offers[_auctionId];

        DataStructure.subOffer memory subOfferInstance = offerInstance
            .subOffers[_subOfferId];
        string memory _idcollection = collectionNFT.tokenIdToCollectionId(
    ...
    function getInforOfferPvP(string memory _nftId, string memory _subOfferId)
        public
        view
        returns (
            uint256,
    ...
    {
        DataStructure.Offer storage offerInstance = Offers[_nftId];
    ...
    function purchaseProcessing(
        ...
        require(
            stanFund[_buyer] - (_amount + _fee) >= 0,
        ...
        require(
            _amount - creatorAmount - creatorStan - fee > 0,
    ...
    function saveOffer(
        ...
        Offers[_indexId].nftID = isAuction ? "" : _indexId;
        Offers[_indexId].auctionID = isAuction ? _indexId : "";
    ...
    function buyFixPriceProcessing(
        address _seller,
        address _buyer,
        uint256 _amount,
        uint256 _tokenId,
    ...
    function listingNFTFixedPrice(
        ...
        string[] memory arrayString = new string[](0);
    ...
    function cancelListingFixedPrice(
        ...
        DataStructure.dataStructure memory dataInstance = DataStructure
            .dataStructure(tokenId, arrayString);
    ...
    function makeOfferFixedPrice(
        ...
        address _owner = stanNFT.ownerOf(_tokenId);
        saveOffer(
            _subOfferId,
            _nftID,
            _tokenId,
    ...
    function placeBidAuction(
        ...
        auctionIdToAuction[_auctionId].offerIdToIndex[
            _subOfferId
    ...
    function acceptOfferPvP(
        ...
        (
            ...
            DataStructure.StateOfOffer state
        ) = getInforOfferPvP(_nftId, _subOfferId);
        ...
        purchaseProcessing(_collectionId, ownerNFT, maker, amount, 0, tokenId);
    ...
    function cancelOfferPvP(
        string memory _nftId,
        string memory _subOffersId,
        string memory _requestId
    ) external {
        address maker = Offers[_nftId].subOffers[_subOffersId].maker;
        uint256 tokenId = Offers[_nftId].tokenId;
        require(
            msg.sender == maker,
            "You are not allowed to preoceed this function"
        );

        delete Offers[_nftId].subOffers[_subOffersId];
    ...
    function cancelOfferPvPAuto(
        string memory _nftId,
        string[] memory subOffersIdParam,
        string memory _requestId
    ) external onlyOwner {
        ...
        DataStructure.dataStructure memory dataInstance = DataStructure
            .dataStructure(Offers[_nftId].tokenId, subOffersIdParam);
    ...
    function transferNFTPvP(
        ...
        chargeFeeTransferNFT(_seller, fee);
        stanNFT.updateOwnerNFTAndTransferNFT(
            _idCollection,
            _seller,
            _buyer,
    ...
    function deposit(uint256 _amount, string memory _requestId)
        external
    ...
    function withdraw(uint256 _amount, string memory _requestId) external {
        require(
    ...
    function findTheBestFitWinner(string memory _auctionId)
        internal
        view
        returns (uint256)
    {
    ...
    function processFinishAuction(
        string memory _auctionId,
        string memory _winnerSubOfferId,
        string memory _requestId
    ...
    function finishAution(string memory _auctionId, string memory _requestId)
        external
        onlyOwner
        ...
        checkStateOfAution(_auctionId)
    {
        ...
        string memory winnerSubOfferId = auctionIdToAuction[_auctionId]
            .offerIds[winnerIndex];
...
contract StanNFT is ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    function createNFT(
        ...
        string[] memory arrayString = new string[](0);
        DataStructure.dataStructure memory dataInstance = DataStructure
            .dataStructure(newTokenId, arrayString);
    ...
    function updateOwnerNFTAndTransferNFT(
        ...
        require(
            msg.sender == auctionAddress || ownerOf(_tokenId) == msg.sender,
            "You are not allowed to call this functions"
        );
        collectionNFT.updateOwnerNFT(_idCollection, _from, _to, _requestId);
...
contract CollectionNFT is Ownable {
    ...
    mapping(uint256 => string) public tokenIdToCollectionId;
    mapping(string => DataStructure.stateCollection) internal _collectionNFT;
...
library DataStructure {
    ...
    struct autionStruct {
        ...
        string[] offerIds;
        Type _type;
        mapping(string => uint256) offerIdToIndex;
    ...
    struct Offer {
        uint256 tokenId;
        mapping(string => subOffer) subOffers;
        string nftID;
        string auctionID;
    }
