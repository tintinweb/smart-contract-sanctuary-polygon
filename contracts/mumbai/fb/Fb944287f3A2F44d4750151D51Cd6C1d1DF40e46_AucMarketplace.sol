...
contract AucMarketplace is ReentrancyGuard, AccessControl {
     ...
     */
     ...
     function deListNft(uint256 _tokenId, address _tokenAddress) external nonReentrant{
        require(itemsForAuction[_tokenId][_tokenAddress].owner == msg.sender, '');
        IERC721(_tokenAddress).transferFrom(address(this), itemsForAuction[_tokenId][_tokenAddress].owner, _tokenId);
     ...
     */
     ...
     function deListBulkNft(uint256[] calldata _tokenId, address _tokenAddress) external nonReentrant{
        for (uint256 i = 0; i < _tokenId.length; i++) {
            require(itemsForAuction[_tokenId[i]][_tokenAddress].owner == msg.sender, '');
            IERC721(_tokenAddress).transferFrom(address(this), itemsForAuction[_tokenId[i]][_tokenAddress].owner, _tokenId[i]);
        }
