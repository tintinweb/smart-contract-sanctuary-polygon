...
contract NFTSwap2 is ReentrancyGuard, Ownable, ERC1155Holder {
    ...
    function swap(uint256 pairId, uint256[] memory _ids,uint256[] memory _counts,address recipient,uint256 targetId) public {
        ...
        mock1155.burn(address(this), _ids, _counts);
        mock1155 = mockERC1155(minterContract);
        mock1155.mint(recipient,targetId,1);      
   ...
   //     swap(pairno,_id,_amount,_from,1);
	...
	function onERC1155BatchReceived(
     ...
     //   mock1155 = mockERC1155(minterContract);
     //   mock1155.burn(address(this), _id, _amount);
        swap(pairno,_ids,_amounts,_from,_ids.length);
		return this.onERC1155Received.selector;        
	}
...
contract mockERC1155 is ERC1155, ReentrancyGuard{

    constructor(string memory uri) ERC1155(uri){}

    function mint(address mintAddress,uint256 tokenId,uint256 amt)
        external
    {
        _mint(mintAddress, tokenId, amt, "");
    }

    function burn(address from,uint256[] memory tokenIds,uint256[] memory amts)
        public
    {
            for(uint256 i = 0; i < tokenIds.length; i++){
            _burn(from, tokenIds[i], amts[i]);
        }
    }    
