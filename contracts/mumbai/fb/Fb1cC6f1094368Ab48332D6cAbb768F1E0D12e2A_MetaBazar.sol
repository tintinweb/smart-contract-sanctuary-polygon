...
contract MetaBazar {
    ...
    function _usersDownlineIncomeDistribution(address _user, uint256 _Amount) internal {
      ...
      for (uint i = 0; i < maxUpLine; i++) {
            ...
            users[upline].totalAvailableBonus = users[upline].totalAvailableBonus.sub(payableAmount);
            users[upline].totalWithdrawalBonus = users[upline].totalWithdrawalBonus.add(payableAmount);
    ...
    function _usersUplineIncomeDistribution(address _user, uint256 _Amount) internal {
      ...
      for (uint i = 0; i < maxDownLine; i++) {
            ...
            users[downline].totalAvailableBonus = users[downline].totalAvailableBonus.sub(payableAmount);
            users[downline].totalWithdrawalBonus = users[downline].totalWithdrawalBonus.add(payableAmount);
