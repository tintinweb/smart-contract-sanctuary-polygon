...
import './interface/ITokenEscrow.sol';
import './interface/ITokenEscrowNFT.sol';

contract TokenEscrow is AccessControl, ITokenEscrow {
    ...
    uint8 public constant ESCROW_ID_SUFFIX = 10;
    ...
    ITokenEscrowNFT public immutable escrowNFT;
    IERC20 public immutable earthaToken;

    mapping(uint256 => EscrowDetail) private _escrowDetail;
    Counters.Counter private _escrowIdTracker = Counters.Counter(1);

    mapping(IERC20 => uint256) public unpaidCreativeRewards;

    constructor(ITokenEscrowNFT escrowNFT_, IERC20 earthaToken_) AccessControl() {
    ...
    function buyerSettlement(uint256 escrowId) external virtual override {
        ...
        unpaidCreativeRewards[ed.token] += (ed.creativeReward - ed.buyerIncentive);

        uint256 value = ed.value;
        if (ed.isSellerBurden) {
            value -= ed.creativeReward;
        }

        ed.token.transfer(recipientAddress, value);
        ed.token.transfer(createrAddress, ed.buyerIncentive);

        emit BuyerSettlement(escrowId, createrAddress, recipientAddress, ed);
    }
    ...
    function sellerSettlement(uint256 escrowId) external virtual override {
        ...
        unpaidCreativeRewards[ed.token] += ed.creativeReward;
        ...
        emit SellerSettlement(escrowId, createrAddress, recipientAddress, ed);
    }
    ...
    function refund(uint256 escrowId) external virtual override {
        ...
        uint256 total = ed.value + ed.creativeReward;
        if (ed.isSellerBurden) {
            total = ed.value;
        }
    ...
    function withdrawCreativeRewards(address recipient, IERC20 token) external virtual override withdrawerOnly() {
    ...
    function setCreativeRewordPercent(uint256 reword) external virtual override setterOnly() {
        creativeRewordPercent = reword;
    }

    function setDiscountCreativeRewordPercent(uint256 discount) external virtual override setterOnly() {
...
interface ITokenEscrow {
    ...
    function withdrawCreativeRewards(address recipient, IERC20 token) external;
    ...
    function setCreativeRewordPercent(uint256 reword) external;
