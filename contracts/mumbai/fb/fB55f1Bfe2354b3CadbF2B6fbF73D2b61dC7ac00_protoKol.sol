...
contract protoKol is iProtoKol {
    ...
    address public immutable kolToken;
    ...
    // address private constant FACTORY = 0x9E6Fa3FB6e1E2AA49B709f7dd91fdC722e96bAD7;
   ...
   */
    function generateTGE(
        ...
        require(campaigns[_campaignId].campaignData.preSaleToken != address(0x0), "Protokol:update PST first");
   ...
   */
    function depositPreSaleTokens(uint256 _campaignId, uint256 _amount) external {
        ...
        uint256 _investment = campaigns[_campaignId].campaignData.enteredInvestmentAgainstMarketingBudget +
            ...
            (campaigns[_campaignId].campaignData.marketingBudget); //* 10**IERC20(token).decimals()) /10**18;
   ...
   */
    function claimPreSaleTokens(
        ...
        if (campaigns[_campaignId].vestingData.isVestingInEnabled) {
            ...
            // console.log("investmentDetails.lastVestingClaimed", block.timestamp - investmentDetails.lastVestingClaimed);
            // require(
            ...
            bool timePassed = _vesting >= campaigns[_campaignId].vestingData.NumberOfvestings;
            ...
            if (
                blackListedKOL[msg.sender][_campaignId] ||
                (timePassed && campaigns[_campaignId].campaignData.presaleAmount < _reward)
            ) {
                _blackListedKOLFundsHandling(_campaignId, _reward, preSale);
                return;
            }
            uint256 convertInto18 = (1 * 10**18) / 10**IERC20(preSale).decimals();

            _reward = _reward * convertInto18;
            _vesting = _vesting * convertInto18;
            ...
            // console.log("_reward", (_reward * 10**IERC20(preSale).decimals()) / 10**18);
            _safeTransfer(preSale, msg.sender, (_reward * 10**IERC20(preSale).decimals()) / 10**18);

            // (% of deposited amount presale * Reward / marketing budget ) - claimed reward
            // enteredMarkt/requiredBudget = % of deposited *(eligiblereward - claimedReward)
