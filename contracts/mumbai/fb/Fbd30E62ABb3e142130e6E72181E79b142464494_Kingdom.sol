...
contract Kingdom is Ownable, IERC721Receiver, Pausable {  
    ...
    event StakeDiceRolled(address owner, uint256[] scores);
    ...
    function addManyToKingdomAndPack(address account, uint16[] memory tokenIds)
        external
        returns(uint256[] memory dice_scores)
    {
        require(
        ...
        uint256[] memory scores = new uint256[](tokenIds.length);
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if (isMerry(tokenIds[i])) _addMerrymanToPack(account, tokenIds[i]);
                ...
                uint256 score = _addHenchmanToKingdom(account, tokenIds[i]);
                scores[i] = score;
            }
        }
        emit StakeDiceRolled(msg.sender, scores);
        dice_scores = scores;
    }
