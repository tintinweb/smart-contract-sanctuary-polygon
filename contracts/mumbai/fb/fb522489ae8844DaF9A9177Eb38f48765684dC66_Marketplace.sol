...
contract Marketplace is AdminAccess, ITangibleMarketplace, IERC721Receiver {
    ...
    function buyFraction(
        ...
        if (existing.seller == initialReSeller) {
            require(
                IInitialReSellerExt(initialReSeller)
                    .saleData(ftnft)
        ...
        if (share == remainingShare) {
            ...
            //to send usdc that marketplace received after fractionalizing
            usdc.safeTransfer(existing.seller, usdc.balanceOf(address(this)));
            //if initial resseler
    ...
    function buy(
        ...
        uint256 lockedAmount;
        if (tokenPrice == 0) {
            (tokenPrice, , , , lockedAmount) = _itemPrice(nft, tokenId, false);
        }
    ...
    function buyUnminted(
        ...
        (
            ...
            ,
            uint256 lockedAmount
        ) = _itemPrice(nft, _fingerprint, true);
    ...
    function setPaymentTokensRouterApprove(
        address _router,
        IERC20[] calldata approvedTokens
    ) external onlyAdmin {
        uint256 length = approvedTokens.length;
        for (uint256 i; i < length; i++) {
            approvedTokens[i].approve(_router, type(uint256).max);
        }
    }
        ...
        return
            fromFingerprints
                ? factory
                    ...
                    .usdcPrice(nft, data, 0)
                : factory
                    .priceManager()
                    .getPriceOracleForCategory(nft)
                    .usdcPrice(nft, 0, data);
    }
    ...
    function _buy(
        ...
        if (cost == 0) {
            (cost, , , , ) = _itemPrice(nft, tokenId, false);
        }
