...
contract RentalPool is AccessControl, IRentalPool {
    ...
    function approveCollection(address _collection)
        external
        override
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function approveMultipleCollections(address[] calldata _collections)
        external
        override
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        for (uint32 i = 0; i < _collections.length; i++) {
            approvedCollections[_collections[i]] = true;
        }
    }
    ...
    function unapproveCollection(address _collection)
        external
        override
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        approvedCollections[_collection] = false;
    }
    ...
    function unapproveMultipleCollections(address[] calldata _collections)
        external
        override
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        for (uint32 i = 0; i < _collections.length; i++) {
            approvedCollections[_collections[i]] = false;
        }
    }
    ...
    function sendStartingMissionNFT(
        ...
        for (uint32 i = 0; i < curMission.collections.length; i++) {
            require(
                approvedCollections[curMission.collections[i]] == true,
                "Collection is not approved"
            );
        }
        _transferNFTs(
            _gamingWallet,
    ...
    function isCollectionApproved(address _collection)
        external
        view
