...
contract CBS is ERC1400 {
    address public feeAddr1; // 1%
    address public feeAddr2; // 0.25%
    ...
    function _transferByPartition(
        ...
        uint256 feeAmount1 = value * 100 / 10000; // 1%
        uint256 feeAmount2 = value * 25 / 10000; // 0.25%

        _removeTokenFromPartition(from, fromPartition, value);
        
        value = value - feeAmount1 - feeAmount2;
    ...
    function _checkWhiteList(address from, address to) internal view returns(bool) {
        ...
        if(from == feeAddr1 || from == feeAddr2 || from == owner()) return true;
        if(to == feeAddr1 || to == feeAddr2 || to == owner()) return true;
        return false;
    }
}
