...
contract Automation is AutomationCompatibleInterface {
    ...
    function checkUpkeep(
        ...
        if (keccak256(checkData) == keccak256(bytes("request"))) {
        ...
        if (keccak256(checkData) == keccak256(bytes("pick"))) {
            bool isInterval = (block.timestamp - lottery.getLatestCheckpoint() >=
                lottery.getInterval());
    ...
    function performUpkeep(bytes calldata performData) external override {
        if (keccak256(performData) == keccak256(bytes("request"))) {
            if (
        ...
        } else if (keccak256(performData) == keccak256(bytes("pick"))) {
            if (
