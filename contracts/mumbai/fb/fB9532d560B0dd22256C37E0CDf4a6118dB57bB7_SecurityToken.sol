  ...
  ERC20SnapshotUpgradeable
{
  ...
  mapping(address => mapping(uint256 => bool)) dividendSent;
  ...
  function distributeDividends(uint256 _startIndex, uint256 _endIndex, uint256 snapshotId, uint256 totalDividendVal)
        external
        onlyRole(MANAGER_ROLE)
    {
        ...
        uint256[] memory dividendValues = new uint256[](size);
        ...
        for (uint256 i = startIndex; i <= _endIndex; i++) {
            ...
            address key = accounts[i];
            uint256 balance = balanceOfAt(key, snapshotId);
            if(!dividendSent[key][snapshotId]) {
              if (balance > 0) {
                  uint256 value = (balance * totalDividendVal) / totalSupply;
                  IERC20(dividendToken).safeTransfer(key, value);
                  investorsAddress[i] = key;
                  dividendValues[i] = value;
              }
              dividendSent[key][snapshotId] = true;
            }
        }
  ...
  function _beforeTokenTransfer(
    ...
    address _accessControlAddress = getRoleAddr();
    bool isBlocklisted = IAccessControl(_accessControlAddress).hasRole(
      BLOCKLISTED_ROLE,
      from
    );
    bool isAdmin = IAccessControl(_accessControlAddress).hasRole(
      MANAGER_ROLE,
      to
    );

    bool isWhitelisted = IAccessControl(_accessControlAddress).hasRole(
      WHITELISTED_ROLE,
      to
    );

    require(!isBlocklisted || (isBlocklisted && isAdmin) || isWhitelisted, "_beforeTokenTransfer: not allowed");
    super._beforeTokenTransfer(from, to, amount);
  }
