 ...
 */
contract ControlPanel is AccessControl {
  ...
  /// @notice Hivemind address new HTokens will be deployed with
  ...
  uint256 public defaultBorrowFeeMantissa = 0.02e18;
  /// @notice Default referred borrow fee new HTokens will be deployed with
  uint256 public defaultReferredBorrowFeeMantissa = 0.02e18;
  /// @notice Default hive fee new HTokens will be deployed with
  uint256 public defaultHiveFeeMantissa = 0.02e18;
  /// @notice Default reserve factor new HTokens will be deployed with
  uint256 public defaultReserveFactorMantissa = 0.02e18;
  /// @notice Default initial exchange rate new HTokens will be deployed with
  uint256 public defaultInitialExchangeRateMantissa = 0.5e18;
  ...
  function deployHToken(
    ...
    string memory symbol = string.concat("h", IERC20Metadata(_underlyingToken).symbol(), IERC721Metadata(_collateralToken).symbol());
    ...
    HTokenI(hToken)._setProtocolFees(defaultHiveFeeMantissa, defaultReserveFactorMantissa);
    ...
    hivemind._setPriceOracle(HTokenI(hToken), PriceOracleI(_oracle));
    hivemind._supportMarket(HTokenI(hToken));
    hivemind._setFactors(HTokenI(hToken), _maxLTVFactorMantissa, _collateralFactorMantissa);
    hivemind._setBorrowFeePerMarket(HTokenI(hToken), defaultBorrowFeeMantissa, defaultReferredBorrowFeeMantissa);

    // Set up liquidator
    liquidator._initializeHToken(HTokenI(hToken));

    emit HTokenDeployed(msg.sender, hToken, _underlyingToken, _collateralToken);
  }
  ...
  function setPriceOracle(address _hToken, address _newOracle) external {
    ...
    hivemind._setPriceOracle(HTokenI(_hToken), PriceOracleI(_newOracle));
  }
  ...
  function setFactors(
    ...
    if (!ownedHTokens[msg.sender].contains(_hToken)) revert Unauthorized();
    hivemind._setFactors(HTokenI(_hToken), _newMaxLTVFactorMantissa, _newCollateralFactorMantissa);
  }
   ...
   */
  function setAdminFee(address _hToken, uint256 _newAdminFeeMantissa) external {
    if (!ownedHTokens[msg.sender].contains(_hToken)) revert Unauthorized();
    HTokenI(_hToken)._setAdminFee(_newAdminFeeMantissa);
  }
  ...
  function pauseComponent(
    ...
    if (!ownedHTokens[msg.sender].contains(_hToken)) revert Unauthorized();
    hivemind._pauseComponent(HTokenI(_hToken), _state, _target);
  }
   ...
   * @param   _defaultHiveFeeMantissa               Hive fee in mantissa format
  ...
  function _setDefaults(
    ...
    uint256 _defaultHiveFeeMantissa,
    ...
    defaultHiveFeeMantissa = _defaultHiveFeeMantissa;
  ...
  event HTokenDeployed(address _deployer, address indexed _hToken, address indexed _underlying, address indexed _collateral);
