...
contract CentenarioCelta is ERC721, ERC721Enumerable, ERC721Burnable, AccessControl {
    ...
    uint256 public priceInDolar; // 10 MATIC
    uint256 public priceUSDC = 10 * 10 ** 6; // 10 USDC (because usdc is a 6 decimal ERC20 token)
    ...
    constructor(address _crossmint, address _usdcAddress, uint256 _priceInDolar, uint256 _slippage, address priceFeedAddress) ERC721("Centenario Celta", "CLT") {
    ...
    function getPriceInMatic() public view returns (uint256 weiPrice){
        return priceInDolar * 1e26 / uint256(getLatestPrice()); // 26 -> 18 decimals eth + 8 decimals price feed.
    }
    function crossmint(address _to) external payable {
        require(getMinPriceAccepted() <= msg.value, "Value send to low");
        require(getMaxPriceAccepted() >= msg.value, "Value send to high");
   ...
   function getTolerance() public view returns(uint256) {
        uint256 tolerance = getPriceInMatic() * slippage/100;
        return tolerance;
    }
    function getMinPriceAccepted() public view returns(uint256) {
        uint256 variance = getTolerance();
        return (getPriceInMatic() - variance);
    }
    function getMaxPriceAccepted() public view returns(uint256) {
        uint256 variance = getTolerance();
        return (getPriceInMatic() + variance);
    }
    ...
    //     require(_tokenIdCounter.current() + 1 <= MAX_SUPPLY, "No more left");
    //     require(msg.sender == crossmintAddress, "This function is for Crossmint only.");
    //     // require(usdc.allowance(msg.sender, address(this)), "Allowance is not set");
