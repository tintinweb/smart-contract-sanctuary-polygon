...
/**
  *  @title TestNFT contract for creating ERC721 standard (Non-Fungible) Token
...
contract TestNFT is ERC721, Pausable, Ownable, ITestNFT, BlackList {
    ...
    /**
     * @dev callerAddress for maintaining mint function caller address
     */
    ...
    function tokenURI(uint id) public view  override returns (string memory) {
        require(_exists(id), "TestNFT: Non-existent token.");
        return uri[id];
    }
     ...
     */
    function safeMint(address to, string memory tokenUri) external override whenNotPaused whenNotBlackListedUser(to) {
        require(!_isBlackListUser(_msgSender()), "TestNFT: Caller address is blacklisted");
        require(msg.sender != callerAddress,"TestNFT: Invalid caller");
    ...
    /**
     * @dev setMintFunctionCaller, function is used to set caller for calling mint function from another contract.
     * @param _callerAddress, it is account address which you set for call mint function.
     */
    function setMintFunctionCaller(address _callerAddress) external  whenNotPaused whenNotBlackListedUser(_callerAddress){
    ...
    function airDrop(address[] calldata account, string[] memory tokenUri) external override whenNotPaused onlyOwner {
        require(account.length == tokenUri.length, "TestNFT: Incorrect parameter length");
        for(uint16 i = 0; i < account.length; i++){
            require(address(account[i]) != ZERO_ADDRESS , "TestNFT: Address Cannot be Zero Address");
            uint256 tokenId = _tokenIdCounter.current();
            _tokenIdCounter.increment();
    ...
    function withdraw() external override whenNotPaused onlyOwner {
        require(balanceOfContract() > 0, "TestNFT: Insufficient balance");
    ...
    function updateTokenPrice(uint256 _tokenPrice) public override onlyOwner{
        require(_tokenPrice != tokenPrice,"TestNFT: Current token price should be greater then existing");
        tokenPrice = _tokenPrice;
