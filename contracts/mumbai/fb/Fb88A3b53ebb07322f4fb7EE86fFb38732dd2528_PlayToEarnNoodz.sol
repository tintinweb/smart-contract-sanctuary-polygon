...
contract PlayToEarnNoodz is Ownable {
  ...
  constructor(address _noodzTokenAddress) {
    admin = msg.sender;
    gameId = 0;
    questId = 0;
    noodzToken = _noodzTokenAddress;
  }
    ...
    returns (bool)
  {
    ...
    token.transfer(_player, balances[_player][_questId].balance);
  ...
  function withdrawIfUnlocked(uint256 _gameId) external returns (bool) {
    ...
    NoodzToken token = NoodzToken(noodzToken);
    token.transfer(msg.sender, gameBalances[msg.sender][_gameId].balance);
  ...
  function withdrawQuest(uint256 _questId) external returns (bool) {
    ...
    NoodzToken token = NoodzToken(noodzToken);
    token.transfer(msg.sender, balances[msg.sender][_questId].balance);
