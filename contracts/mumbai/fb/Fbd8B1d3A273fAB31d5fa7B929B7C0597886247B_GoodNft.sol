    ...
    ERC721URIStorageUpgradeable
{
    ...
    IERC20Upgradeable public goodTokenAddress;
    ...
    event EmergencyWithdrawalERC20(address receiver, address tokenAddress);
    ...
    function initialize(
        ...
        IERC20Upgradeable _goodTokenAddress,
        ...
        __ERC721Holder_init();
        goodTokenAddress = _goodTokenAddress;
    ...
    /**
     * @notice  Provides functionality to get USDT worth of Good token based on the amount .
     * @param   _pathaddress  .
     * @param   amountIn  .
     * @return  uint  .
     */
    function getUniswapv3Amount(
        ...
        try quoterV2.quoteExactInput(path, amountIn) returns (
            uint256 amountOut,
            uint160[] memory sqrtPriceX96AfterList,
            uint32[] memory initializedTicksCrossedList,
            uint256 gasEstimate
    ...
    /**
     * @notice Provides functionality to purchase NFT by giving Good token equivalent to the price of NFT in USDT .
     * @param   _purchaseTokenId  .
     * @param   _amount  .
     */
    function purchase(uint _purchaseTokenId, uint _amount) external {
        NftListing memory listingData = _tokenIdToListing[_purchaseTokenId];
        require(listingData.priceInUSD > 0, "Listing doesnot exist");

        address[] memory path = new address[](2);
        path[0] = address(goodTokenAddress);
        path[1] = address(usdc);
        uint usdc_equivalent_value = getUniswapv3Amount(path, _amount);
        require(usdc_equivalent_value == 0, "Zero amount");
        require(
            usdc_equivalent_value < listingData.priceInUSD,
            "Good token amount less than nft price"
        );

        goodTokenAddress.safeTransferFrom(msg.sender, address(this), _amount);
        ...
        goodTokenAddress.safeTransfer(listingData.charity, charityShare);
        goodTokenAddress.safeTransfer(
            listingData.charityCoordinator,
            charityCoordinatorShare
        );
        goodTokenAddress.safeTransfer(listingData.lister, listorShare);
        goodTokenAddress.safeTransfer(treasuryContractAddress, platformShare);
    ...
    /**
     * @notice  Provides functionality to emergency withdraw any ERC20 token locked on the contract,caller must have Admin role .
    ...
    function emergencyWithdrawERC20(
        ...
        emit EmergencyWithdrawalERC20(_recieverAddress, _tokenAddress);
    }
