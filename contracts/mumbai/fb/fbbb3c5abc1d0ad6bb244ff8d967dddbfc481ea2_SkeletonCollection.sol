...
/**
 ...
 function depositForFreeMint(uint32 _mintCount) public payable{
    require(_mintCount >= 1 && _mintCount <= 1000, "wrong count");
...
function mintJoker(uint8 _amount, Coupon memory _coupon, uint8 _availableAmount) public {
  require(_amount >= 1 && _amount <= 100, "wrong amount");
  require (_amount <= jokersCount, "not enough joker");
...
function commit(bytes32 dataHash, uint8 _amount, Coupon memory _coupon, uint8 _availableAmount, bool _freeMint) public payable{
    require(mintingPaused == false, "on P");
    require(commits[msg.sender].amount == 0, "C already exists");
    require(_amount >= 1 && _amount <= 100, "max 100");
...
function reveal(bytes32 revealHash) public {
    require(commits[msg.sender].revealed==false, "revealed");
    
    require(getHash(revealHash)==commits[msg.sender].commit, "wrong C");
    //require that the block number is greater than the original block
    require(uint64(block.number)>commits[msg.sender].block, "wrong block");
...
/**
  ...
  function upgradeCards(uint160[] calldata _tokenIds, uint8 _targetCardType, uint8 _collectionId,  Coupon memory _coupon, uint _discount) public payable validCardType(_targetCardType) {
    ...
    require(upgradeinfo.collectionBuyBack >= buyBackValue, "wrong collection");
    ...
    require(upgradecost.highestCardValue <= cardValue[_targetCardType], "wrong target C");
...
function calculateBuyback(uint160[] calldata _tokenIds) public view returns (uint256 floorValue, uint256 bonusEarned, uint256 buyback, bool allAces) {
  ...
  function claimBuyBack (uint160[] calldata _tokenIds) public {
    ...
    if(buyback > 0){
       ...
       uint256 epsilon = 100000000000000; //wei
      if(gametreasury.balance() < (buyback - epsilon)){
          
          buybackFromTreasuryReserve = buyback - gametreasury.balance() + epsilon;
  ...
  function claimBuyBackCommission(uint256 claimCommission, uint256 partnerClaimCommission) private {
    ...
    uint256 epsilon = 100000000; //wei
    if(claimCommission > 0){
      if(gametreasury.balance() < claimCommission - epsilon){
          uint256 claimCommissionFromTreasuryReserve = claimCommission + epsilon - gametreasury.balance();
          treasuryreserve.claimCommission(daoAddress, claimCommissionFromTreasuryReserve);
          claimCommission -= (claimCommissionFromTreasuryReserve + epsilon);
      }
      if(claimCommission > 0){
    ...
    if(partnerClaimCommission > 0){
        if(gametreasury.balance() < partnerClaimCommission + epsilon){
          uint256 partnerClaimCommissionFromTreasuryReserve = partnerClaimCommission + epsilon - gametreasury.balance();
          treasuryreserve.claimCommission(partnerDAOAddress, partnerClaimCommissionFromTreasuryReserve);
          partnerClaimCommission -= partnerClaimCommissionFromTreasuryReserve - epsilon;
      }
      if(claimCommission > 0){
  ...
  /**
	...
	*/
	function setPartnerCommission(uint _newCommission) external onlyOwner {
		// Update partnerFee
    require(_newCommission < 100, "< 100");
...
/**
	...
	*/
    ...
    require(_newPartnerMarketplaceFee < 100, "< 100");
