...
contract Mint is Ownable {
    ...
    function updateState(
        ...
        // IERC20Extended(swapToToken).safeApprove(address(lock), 0);
        // IERC20Extended(swapToToken).safeApprove(swapRouter, 0);
        asset = IAsset(asset_);
        ...
        feeTo = feeTo_;
        IERC20Extended(swapToToken).approve(address(lock), MAX_UINT256);
        IERC20Extended(swapToToken).approve(swapRouter, MAX_UINT256);
    }
    ...
    function mint(
        ...
        if (swapToToken == address(1)) {
            ...
            uint256 bal = IERC20Upgradeable(swapToToken).balanceOf(address(this));
    ...
    function _getPrice(IERC20Extended token, bool isCollateral)
        private
        view
