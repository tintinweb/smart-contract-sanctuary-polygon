...
import "./market/template/MarketTieredLeverage.sol";
import "./v0.1/interfaces/IStaker.sol";
import "./oracles/template/OracleManagerFixedEpoch.sol";
import "./interfaces/ILongShort.sol";

contract KeeperArctic is AccessControlledAndUpgradeable {
  LongShort public longShort;
  ...
  function initialize(address _admin, address _longShort) external virtual initializer {
    longShort = LongShort(_longShort);

    _AccessControlledAndUpgradeable_init(_admin);
  }

  // Functian that the keeper calls
  function updateSystemStateForMarket(MarketTieredLeverage market) external {
    market.updateSystemState();
  }

  function shouldUpdateMarket() external view returns (bool canExec, bytes memory execPayload) {
    uint256 latestMarket = longShort.latestMarket();
    for (uint32 index = 1; index <= latestMarket; index++) {
      MarketTieredLeverage market = MarketTieredLeverage(longShort.separateMarketContracts(index));
      (, , uint32 latestExecutedEpochIndex, ) = market.epochInfo();

      OracleManagerFixedEpoch oracleManager = market.oracleManager();

      if (latestExecutedEpochIndex < oracleManager.getLatestEpochIndex()) {
        uint32 epochTimestamp = uint32(oracleManager.getEpochStartTimestamp());

        bool shouldUpdate = oracleManager.shouldUpdatePrice(epochTimestamp);

        if (shouldUpdate) {
          return (true, abi.encodeCall(this.updateSystemStateForMarket, market));
        }
      }
    }
    return (false, "");
  ...
  // Test code
  event TestUpdateSystemStateForMarket(address indexed market);

  // Functian that the keeper calls
  function testUpdateSystemStateForMarket(MarketTieredLeverage market) external {
    emit TestUpdateSystemStateForMarket(address(market));
  }

  function testShouldUpdateMarket() external view returns (bool canExec, bytes memory execPayload) {
    uint256 latestMarket = longShort.latestMarket();
    for (uint32 index = 1; index <= latestMarket; index++) {
      MarketTieredLeverage market = MarketTieredLeverage(longShort.separateMarketContracts(index));
      (, , uint32 latestExecutedEpochIndex, ) = market.epochInfo();
      ...
      if (latestExecutedEpochIndex < oracleManager.getLatestEpochIndex()) {
        ...
        if (shouldUpdate) {
          return (true, abi.encodeCall(this.testUpdateSystemStateForMarket, market));
        }
      }
    }
    return (false, "");
  ...
  function testGelato() external view returns (bool, bytes memory execPayload) {
...
contract LongShort is ILongShort, AccessControlledAndUpgradeable {
  ...
  address public staker_DEPRECATED;
  ...
  mapping(uint32 => uint256) public marketLastUpdateTimestamp;

  uint256[43] private __marketStateGap;
  ...
  uint256[40] private __separateMarketContractsStateGap;
  ...
  function SEPARATE_MARKET_CONTRACT_INDEX_START() internal pure virtual returns (uint256) {
    // If this vaulue isn't set this contract won't work!
    return 0;
  }
  ...
  function initialize(address _admin, address _gems) public virtual initializer {
  ...
  function registerPoolMarketContract(
    ...
    GEMS(gems).grantRole(GEMS(gems).GEM_ROLE(), address(marketContract));
  }
  ...
  */
  ...
  function _calculateFundingAmount(
    ...
    uint256 lastUpdateTimestamp = marketLastUpdateTimestamp[marketIndex];
  ...
  /// NB this function adjusts the value of the long and short side based on the latest
  ...
  function _rebalanceMarket(uint32 marketIndex, int256 newAssetPrice)
    internal
    virtual
  ...
  mapping(uint32 => bool) finalMarketPriceReceived;
  ...
  function _updateSystemStateInternal(uint32 marketIndex)
    ...
    requireMarketExists(marketIndex)
  {
    require(!finalMarketPriceReceived[marketIndex], "This market is migrated");
    if (marketIndex < SEPARATE_MARKET_CONTRACT_INDEX_START()) {
      ...
      if (assetPriceHasChanged) {
        finalMarketPriceReceived[marketIndex] = true;

        (uint256 newLongPoolValue, uint256 newShortPoolValue) = _rebalanceMarket(
          marketIndex,
          newAssetPrice
        );
        ...
        marketLastUpdateTimestamp[marketIndex] = block.timestamp;
      ...
      //TODO update when update state implemented on market
      MarketTieredLeverage(separateMarketContracts[marketIndex]).updateSystemState();
    }
  }
  ...
  function mintLong(
    ...
    MarketTieredLeverage(separateMarketContracts[marketIndex]).mintLongFor(
      amount,
      pool,
      msg.sender
    );
  ...
  function mintShort(
    ...
    MarketTieredLeverage(separateMarketContracts[marketIndex]).mintShortFor(
      amount,
      pool,
      msg.sender
    );
  ...
  function redeemLong(
    ...
    MarketTieredLeverage(separateMarketContracts[marketIndex]).redeemLongFor(
      tokens_redeem,
      pool,
      msg.sender
    );
  ...
  function redeemShort(
    ...
    MarketTieredLeverage(separateMarketContracts[marketIndex]).redeemShortFor(
      tokens_redeem,
      pool,
      msg.sender
    );
  ...
  function shiftPositionNextPrice(
    ...
    revert("No shifting allowed until after upgrade");
  }
  ...
  function shiftPositionFromLongNextPrice(uint32 marketIndex, uint256 amountSyntheticTokensToShift)
    ...
    override
  {
    separateMarketContracts[marketIndex].shiftPositionNextPriceFor(
 ...
 */
    ...
    virtual
  {
    if (marketIndex < SEPARATE_MARKET_CONTRACT_INDEX_START()) {
      ...
      MarketTieredLeverage(separateMarketContracts[marketIndex])
        .executeOutstandingNextPriceSettlementsUser(user);
    }
  }
  ...
  function executeOutstandingNextPriceSettlementsUserMulti(
    ...
    for (uint256 index = 0; index < users.length; index++) {
      _executeOutstandingNextPriceSettlements(users[index], marketIndex);
    }
  }
...
import "../../oracles/template/OracleManagerFixedEpoch.sol";
import "../../longShort/template/LongShort.sol";
import "../../MIA/SyntheticTokenMarketUpgradeable.sol";
...
contract MarketTieredLeverage is AccessControlledAndUpgradeable {
  ...
  */
  ...
  event MarketFundingRateMultiplerChanged(uint256 fundingRateMultiplier_e18);
  ...
  LongShort public immutable longShort; // original core contract
    ...
    adminOnly
  {
    ...
    emit MarketFundingRateMultiplerChanged(_fundingRateMultiplier_e18);
  }

  function initializeEpochInfo(OracleManagerFixedEpoch _oracleManager) internal returns (int256) {
    // Set this value to one initially - 0 is a null value and thus potentially bug prone.
    epochInfo.epochIndex = uint32(_oracleManager.getLatestEpochIndex());
    epochInfo.epochTimestamp = uint32(_oracleManager.getEpochStartTimestamp());
  ...
  function initializePools(
    PoolInfo[] memory initPools,
    uint256 initialEffectiveLiquidityToSeedEachPool,
    address seederAndAdmin,
    uint32 _marketIndex,
    address _oracleManager,
    address _yieldManager
  ) external initializer longShortOnly returns (bool initializationSuccess) {
    require(
    ...
    PoolInfo[] memory poolInfo = new PoolInfo[](initPools.length);
    ...
    for (uint256 i = 0; i < initPools.length; i++) {
      ...
      seedAmount = calculateLiquidityForPool(
        initialEffectiveLiquidityToSeedEachPool,
        initPools[i].leverage
      );
      amountToLockInYieldManager += seedAmount;

      SyntheticTokenMarketUpgradeable(initPools[i].token).initialize(
        initPools[i],
        seederAndAdmin,
        marketIndex
      );
      SyntheticTokenMarketUpgradeable(initPools[i].token).mint(
        PERMANENT_INITIAL_LIQUIDITY_HOLDER,
        seedAmount
      );
  ...
  /// @dev Can only be called by the current admin.
  /// @param _newOracleManager Address of the replacement oracle manager.
  function updateMarketOracle(OracleManagerFixedEpoch _newOracleManager) external adminOnly {
    // NOTE: we could also upgrade this contract to reference the new oracle potentially and have it as immutable
    // If not a oracle contract this would break things.. Test's arn't validating this
    // Ie require isOracle interface - ERC165
    OracleManagerFixedEpoch previousOracleManager = oracleManager;
  ...
  function get_pool_leverage(PoolType poolType, uint256 pool) external view returns (uint256) {
    ...
    returns (uint256 totalValueRealizedInMarket)
  {
    for (uint8 poolType = uint8(PoolType.SHORT); poolType <= uint8(PoolType.LONG); poolType++) {
      ...
      for (uint256 poolIndex = 0; poolIndex < maxPoolIndex; poolIndex++) {
        totalValueRealizedInMarket += pools[PoolType(poolType)][poolIndex].value;
      }
    }
  }
  ...
  function calculateEffectiveLiquidityForPool(uint256 value, uint256 leverage)
    public
    pure
  ...
  function calculateLiquidityForPool(uint256 effectiveLiquidity, uint256 leverage)
    public
    pure
    ...
    returns (uint256[2] memory effectiveLiquidityPoolType)
  {
    for (uint8 poolType = uint8(PoolType.SHORT); poolType <= uint8(PoolType.LONG); poolType++) {
      uint256 maxPoolIndex = numberOfPoolsOfType[poolType];
      for (uint256 poolIndex = 0; poolIndex < maxPoolIndex; poolIndex++) {
        effectiveLiquidityPoolType[poolType] += calculateEffectiveLiquidityForPool(
  ...
  function getValueChange(
    ...
    // TODO URGENT: check value change is less than the highest percentage change.
    // TODO URGENT: add funding rate (will need to use the epoch length since we rebalance precisely at the end of an epoch)
  }
  ...
  function getValueChangeForPool(
    ...
    valueChangeForPool =
      (int256(calculateEffectiveLiquidityForPool(poolValue, leverage)) *
        valueChange *
        directionScalar) /
      int256(totalEffectiveLiquidity);
  }

  // This is an example struct that we could use for later optimization
    ...
    for (uint8 poolType = uint8(PoolType.SHORT); poolType <= uint8(PoolType.LONG); poolType++) {
      ...
      for (uint8 poolIndex = 0; poolIndex < maxPoolIndex; poolIndex++) {
        int256 valueChangeForPool = getValueChangeForPool(
          totalEffectiveLiquidityPoolType[uint256(PoolType(poolType))],
          valueChange,
          pools[PoolType(poolType)][poolIndex].value,
          pools[PoolType(poolType)][poolIndex].leverage,
          PoolType(poolType)
        ...
        pools[PoolType(poolType)][poolIndex].value = uint256(
          int256(pools[PoolType(poolType)][poolIndex].value) + valueChangeForPool
        );
  ...
  function _mint(
    ...
    // TODO URGENT: handle edge case where existing deposit exists in pool for users and it is currently the EWT: if (isNextEpochAmount) {
  ...
  function mintLongFor(
    ...
    _mint(amount, user, PoolType.LONG, pool);
  }
  ...
  function mintShortFor(
    ...
    _mint(amount, user, PoolType.SHORT, pool);
  }
  ...
  function redeemLongFor(
    ...
    _redeem(amount, user, PoolType.LONG, pool);
  }
  ...
  function redeemShortFor(
    ...
    _redeem(amount, user, PoolType.SHORT, pool);
  }
    ...
    if (currentUserAction.amount > 0 && currentUserAction.correspondingEpoch <= epochIndex) {
      ...
      if (currentUserAction.correspondingEpoch > epochInfo.latestExecutedEpochIndex) {
        ...
        if (!isCurrentEpochPriceUpdated) {
          ...
          // TODO URGENT: we would need to add the users action to the `nextEpochAmount`
        }
      }
      ...
      if (currentUserAction.nextEpochAmount > 0) {
        // TODO URGENT: handle this edge case!
...
// AggregatorV3Interface(AggregatorProxy(proxy).aggregator())
/*
...
contract OracleManagerFixedEpoch {
  ...
  function getMissedEpochPriceUpdates(
    ...
    (uint80 latestRoundId, , , , ) = chainlinkOracle.latestRoundData(); // TODO: this function call should be integrated into the oracle more efficiently.
    ...
    while (_isSearchingForMissedEpochExecutionInfo && latestRoundId >= _currentOracleIndex) {
...
abstract contract IYieldManager {
  ...
  */
  function distributeYieldToTreasury() external virtual;
  ...
  */
  ...
  function _updateSystemStateInternal() internal virtual {
    ...
    if (
      ...
      MarketSideValueInPaymentToken memory sideValues = marketSideValueInPaymentToken;
      _rebalanceMarkets(oracleUpdates, sideValues);

      emit SystemStateUpdatedSeparateMarket(
        marketUpdateIndex,
        int256(int128(oracleUpdates.currentPrice)),
        sideValues.value_long,
        sideValues.value_short,
  ...
  AccessControlledAndUpgradeable
{
  ...
  address _minterDeprecated;
  address _stakerDeprecated;
  ...
  MarketTieredLeverage.PoolType public poolType;
  ...
  // TODO: create a modifier called `onlyMarket` and use that instead of the minter role.
  ...
  function initialize(
    MarketTieredLeverage.PoolInfo memory poolInfo,
    address upgrader,
    uint32 _marketIndex
  ...
  */
  ...
  function burn(uint256 amount) public override(ERC20BurnableUpgradeable, ISyntheticToken) {
    require(msg.sender == market, "Only minter contract");
  ...
  */
  function _beforeTokenTransfer(
    ...
    if (sender != market && sender != address(0)) {
      Market(market).executeOutstandingNextPriceSettlementsUser(sender);
    }
  ...
  */
  function balanceOf(address account) public view virtual override returns (uint256) {
    ...
    // TODO: fiix this fuction:
    // Market(minter).getUsersConfirmedButNotSettledSynthBalance(account, isLong);
  }
}
