...
contract Registration is Ownable {
    ...
    function advertiserRegister(string memory name_, string memory category_)
        external onlyOwner
    {
        require(isNotRegistred(msg.sender));
        Register storage register = registered[msg.sender];
        register.advertiserID = uniqueID;
        register.name = name_;
        register.category = category_;
        uniqueID++;
        emit Registered(msg.sender, name_, category_);
    }
