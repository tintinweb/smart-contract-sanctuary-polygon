...
contract Staking is IERC165, IERC721Receiver, IERC1155Receiver, Ownable, Initializable {
    ...
    /**
     * @dev Since we will not interact with this contract directly, but instead use it as implementation referenced for a Proxy,
     * we simply lock this contract so no one can call `initialize`.
     * This step is not critical, since the state of the implementation contract is never used, but recommended as a best practice.
     */
    constructor() {
    ...
    /**
     * @dev Pseudo-constructor for this contract. Since this contract serves as implementation for a Proxy contract, we can not use a real constructor,
     * as we would not be able to run it in the context of the proxy.
     ...
     */
    function initialize(address _validator, address _owner, uint256 _unstakePeriodSeconds) initializer external {
        unstakePeriodSeconds = _unstakePeriodSeconds;
        validator = IStakeValidator(_validator);
        // Ownable implicitly grants ownership to the deployer in the constructor. Since constructors are not working in combination with Proxies, we do it here
        // explicitly. We could use msg.sender but this should be handled carefully as the admin of the proxy can not be the owner
