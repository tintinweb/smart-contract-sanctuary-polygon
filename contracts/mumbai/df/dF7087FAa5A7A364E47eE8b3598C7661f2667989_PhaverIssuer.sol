...
error InvalidProof();
error HashAlreadyUsed(bytes32 usedHash);
...
contract PhaverIssuer is Pausable, Ownable {
  AccountBoundToken public ABT;

  mapping(bytes32 => bool) private UsedTwitterIdHash; // for deduplication
  mapping(bytes32 => bool) private UsedDiscordIdHash; // for deduplication

  /// @notice ABT address is set in the constructor.
  constructor(address ABTAddress) {
    ABT = AccountBoundToken(ABTAddress);
  }
  ...
  /// @notice Issues a Phaver ABT to the function caller.
  /// @param twitterIdHash Hash of the caller's twitter credential.
  /// @param discordIdHash Hash of the caller's discord credential.
  /// @param credentialURL URL of the caller's credentials.
  /// @param proof Proof the caller own's the credentials.
  function issueCredential(
    ...
    if (ABT.hasRole(ABT.DEFAULT_ADMIN_ROLE(), issuer) == false)
      revert InvalidProof();

    _deduplicateCredential(twitterIdHash, discordIdHash);
    // force id 1 as phaver issuer can only issuer phaver SBT
    ABT.issue(to, 1, credentialURL);
  }

  /// @notice Updates the credential URL of the caller's ABT.
  /// @param credentialURL URL of the caller's updated credentials.
  /// @param proof Proof the caller own's the updated credentials.
  function updateCredential(string memory credentialURL, Proof memory proof)
    external
    whenNotPaused
  {
    address receiver = msg.sender;
    bytes32 digest = keccak256(
      abi.encode(1, receiver, credentialURL, block.chainid, address(this))
    );
    address issuer = _recoverSigner(digest, proof);
    if (ABT.hasRole(ABT.DEFAULT_ADMIN_ROLE(), issuer) == false)
      revert InvalidProof();

    // force id 1 as phaver issuer can only update phaver SBT
    ABT.update(receiver, 1, credentialURL);
  }
  ...
  /// @notice Reverts if the twitter and discord hashes have already been used.
  /// @param twitterIdHash Hash of the caller's twitter credential.
  /// @param discordIdHash Hash of the caller's discord credential.
  function _deduplicateCredential(bytes32 twitterIdHash, bytes32 discordIdHash)
    internal
  {
    if (UsedTwitterIdHash[twitterIdHash]) revert HashAlreadyUsed(twitterIdHash);
    if (UsedDiscordIdHash[discordIdHash]) revert HashAlreadyUsed(discordIdHash);

    UsedTwitterIdHash[twitterIdHash] = true;
    UsedDiscordIdHash[discordIdHash] = true;
  }
  ...
  AccessControlUpgradeable
{
  ...
  /// @notice Initializes the ABT, AccessControle and sets the roles to the deployer.
  ...
  /// @param id The id of the ABT.
  function _isRevoked(address account, uint256 id)
    internal
    view
    returns (bool)
  {
    bytes32 credentialHash = keccak256(abi.encode(_credentialURL[account][id]));
...
interface IAccountBoundToken {
  ...
  /// @dev This emits when updating the credential URL for an existing ABT.
