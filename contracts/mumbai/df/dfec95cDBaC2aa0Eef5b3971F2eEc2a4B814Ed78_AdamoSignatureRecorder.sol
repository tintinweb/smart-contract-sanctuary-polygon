...
contract AdamoSignatureRecorder is Owned {
    struct SignatureMetadata {
        ...
        uint256 signatureLastUpdateTimestamp;
        string signatureOwnerEncryptedString;
        uint256 signatureLastUpdateBlockNumber;
    ...
    function registerSignature(
        ...
        s_signatureMetadata[_signatureOwnerDocumentNumber]
            .signatureLastUpdateTimestamp = block.timestamp;
        s_signatureMetadata[_signatureOwnerDocumentNumber]
            .signatureLastUpdateBlockNumber = block.number;
        s_signatureMetadata[_signatureOwnerDocumentNumber]
    ...
    function deleteSignature(
        ...
        if (!isRegisteredSignature[_signatureDocumentNumberToDelete])
            revert SignatureToUpdateIsNotRegistered();
    ...
    function updateSignatureData(
        uint64 _signatureDocumentNumberToUpdate,
        string memory _signatureOwnerNameToUpdate,
        string memory _signatureOwnerDocumentTypeToUpdate
    ) external onlySignatureRecorderOwner {
        if (!isRegisteredSignature[_signatureDocumentNumberToUpdate])
            revert SignatureToUpdateIsNotRegistered();

        s_signatureMetadata[_signatureDocumentNumberToUpdate]
            .signatureLastUpdateBlockNumber = block.number;
        s_signatureMetadata[_signatureDocumentNumberToUpdate]
            .signatureLastUpdateTimestamp = block.timestamp;
        s_signatureMetadata[_signatureDocumentNumberToUpdate]
            .signatureOwnerName = _signatureOwnerNameToUpdate;
        s_signatureMetadata[_signatureDocumentNumberToUpdate]
            .signatureOwnerDocumentType = _signatureOwnerDocumentTypeToUpdate;

        emit SignatureUpdated(_signatureDocumentNumberToUpdate);
    }
