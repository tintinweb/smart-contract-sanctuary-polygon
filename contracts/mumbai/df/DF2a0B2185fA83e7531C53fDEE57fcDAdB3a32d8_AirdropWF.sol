...
contract AirdropWF is Pausable, Ownable {
    ...
    uint256 public currentDropId = 0;
    uint public eachMatic = 0;
    bool public claimEnabled = false;
    uint public totalMatic = 0;
    uint public remBalance = 0 ;

    mapping(uint256=>mapping(uint256=>bool)) public claimedRewards;
    mapping(uint256=>uint) public previousEachMatic;
    mapping(uint256=>uint) public previousRemainingMatic;
    ...
    function startClaim() public onlyOwner whenNotPaused {
        ...
        //Total Matic for Current Drop
        totalMatic =  payable(address(this)).balance - remBalance;
        ...
        //Add previous Matics to each drop
        previousEachMatic[currentDropId] = eachMatic;
        claimEnabled = true;
    ...
    function stopClaim () public onlyOwner {
      ...
      //Add remaining balance of current drop minus the prevois ones
        previousRemainingMatic[currentDropId] = payable(address(this)).balance - remBalance;

         //Sum Prevoius Drop Balance
        for(uint256 i=1;i<=currentDropId;i++){
                remBalance += previousRemainingMatic[i];
            } 
    ...
    function claimReward (uint256[] memory _nftId, uint256 _dropId) public {
        ...
        for(uint i = 0 ; i<_nftId.length; i++) {
            ...
            require(!claimedRewards[_dropId][_nftId[i]], 'Already Claimed');

             if (_dropId == currentDropId) {
                // do something 
                totalClaim+=eachMatic;
            }
            else {
                 totalClaim+=previousEachMatic[_dropId];
            }

            claimedRewards[_dropId][_nftId[i]] = true;
