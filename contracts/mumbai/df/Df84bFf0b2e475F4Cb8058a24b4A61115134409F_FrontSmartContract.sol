...
contract CollectionERC1155 is ERC1155Supply, Ownable {
    ...
    function mint(address to, uint256 tokenId, uint256 amountOfItems) external payable {
        require(msg.value >= amountOfItems * idToPriceMap[tokenId], "Insufficient MATIC");
        _mint(to, tokenId, amountOfItems, "");
    }

    function crearToken(uint256 tokenSupply, uint256 mintPrice) public onlyOwner(collectionOwner) {
    ...
    function isPaused(address callerAddress) public view onlyAdmin(callerAddress) returns(bool)  {
        return paused;
    }

    function pause(address callerAddress) public onlyAdmin(callerAddress) {
        paused = !paused;
    }

     function reveal(address callerAddress) public onlyAdmin(callerAddress) {
        revealed = !revealed;
    }

    function isRevealed(address callerAddress) public view onlyAdmin(callerAddress) returns(bool)  {
        return revealed;
    }
 
    function whiteListUser(address callerAddress, address userAddressTowhiteList) public onlyAdmin(callerAddress) {
...
contract FrontSmartContract {
    ...
    function createCollectionToken(
     ...
     function _createCollectionToken( 
       ...
       /* require(
            checkParams(
                collectionName, collectionSymbol, metadataBaseUri, maxCollectionSupply, mintPrice
            ) == true, 
            "Icorrect params"
        );*/
        ...
        //TODO: emit event of contract address created
         for (uint i = 0; i < maxCollectionSupply; i++) {
            createdContract.crearToken(maxTokenSupply[i], mintPrice[i]);
        }
    ...
    function _whiteListUser(uint256 collectionId, address userAddressTowhiteList) public {
    ...
    /*function checkParams(
        ...
        string memory collectionSymbol,
        string memory metadataBaseUri,
        uint256 maxItemAmount, //maximo de los items creados para la coleccion
        uint256 mintPrice
