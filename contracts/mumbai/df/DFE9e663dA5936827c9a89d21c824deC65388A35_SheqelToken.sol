...
contract SheqelToken is Context, IERC20, IERC20Metadata {
    ...
    function _transfer(
        ...
        if(recipient != reserveAddress && sender != reserveAddress && recipient != address(0)){
    ...
    function swapTokenToUSDC(address recipient, uint256 amount) internal {
        ...
        try uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            ...
            // If the DEX does not have enogh liquidity, sell SHQ to the reserve
            _approve(address(this), address(reserveContract), amount);
