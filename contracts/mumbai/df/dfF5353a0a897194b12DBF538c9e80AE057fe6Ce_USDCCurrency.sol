...
/// @title USDCCurrency
/// @notice This contract is a specific implementation of the BaseCurrency for USDC.
/// @dev Since USDC is already in USDC value, this simply returns the given amount
contract USDCCurrency is BaseCurrency {
    /// @notice Constructor sets the address for the ERC20 token.
    /// @param _erc20 The address of the ERC20 token to be used as the base currency.
    // solhint-disable-next-line no-empty-blocks
    constructor(address _erc20) BaseCurrency(_erc20) {}

    /// @notice Returns the estimated value in USDC of an amount of the base currency.
    /// @dev For USDC, the return value is the same as the input value (_amount).
    /// @param _amount The amount of base currency to estimate the value of.
    /// @return The estimated value in USDC of the specified amount of base currency.
    function getEstimatedUSDCValue(uint256 _amount) public view virtual override returns (uint256) {
        return _amount;
    }
}
