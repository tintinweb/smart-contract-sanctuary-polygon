    ...
    ERC1155Holder
{
    ...
    function factory() public view returns (ILSSVMPairFactoryLike _factory) {
        return ILSSVMPairFactoryLike(_pool.factory());
    }
...
abstract contract LSSVMPairEnumerable is LSSVMPair {
    ...
    function _sendAnyNFTsToRecipient(
        ...
        uint256 lastIndex = nft().balanceOf(address(this)) - 1;
        for (uint256 i = 0; i < numNFTs; ) {
            uint256 nftId = IERC721Enumerable(address(nft()))
                .tokenOfOwnerByIndex(address(this), lastIndex);
            nft().safeTransferFrom(address(this), nftRecipient, nftId);
    ...
    function _sendSpecificNFTsToRecipient(
        ...
        uint256 numNFTs = nftIds.length;
        for (uint256 i; i < numNFTs; ) {
            nft().safeTransferFrom(address(this), nftRecipient, nftIds[i]);
...
interface IUpshotPool {
    ...
    function nft() external view returns (address _nft);
    ...
    /// @notice Find the owner of an encapsulated pair
    /// @param _pair The address for the encapsulated pair
    /// @return owner The address of the owner
    function ownerOf(address _pair) external view returns (address owner);

    function spotPrice() external view returns (uint128 _spotPrice);

    function factory() external view returns (address _factory);

    function pairOf(address _owner) external view returns (address pair);
    ...
    function addLiquidityETH(uint256[] calldata tokenIdList, uint256 valueInWei) external payable;

    function addLiquidityERC20(uint256[] calldata tokenIdList, uint256 countERC20) external;

    function pullLiquidityETH(uint256[] calldata tokenIdList, uint256 valueInWei) external;

    function pullLiquidityERC20(uint256[] calldata tokenIdList, uint256 countERC20) external;
    ...
    //TODO: keep this or axe it depending on LP token
    function changeAssetRecipient(address payable newRecipient, address pair) external;
