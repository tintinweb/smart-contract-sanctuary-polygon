...
///      The Aggregator contract should be deployed first since Tellers and Auctioneers
///      all require it in their constructors.
///
        ...
        for (uint256 i; i < len; ++i) {
            ...
            if (expiry <= maxExpiry_) {
                if (minAmountOut_ <= maxPayout) {
                    try auctioneer.payoutFor(amountIn_, ids[i]) returns (
                        uint256 payout
                    ) {
...
interface IBondTeller {
    /// @notice                 Exchange quote tokens for a bond (ERC4626 shares) in a specified market
    /// @param recipient_       Address of recipient of bond. Allows deposits for other addresses
    /// @param id_              ID of the Market the bond is being purchased from
    /// @param amount_          Amount to deposit in exchange for bond (ERC4626 shares)
    /// @param minAmountOut_    Minimum acceptable amount of tokens vested in vault. Prevents frontrunning
    /// @return                 Amount of payout token to be vested in the vault
    /// @return                 Amount of vault shares received for vesting of payout tokens
    function purchase(
        address recipient_,
        uint256 id_,
        uint256 amount_,
...
interface IBondAuctioneer {
    ...
    /// @param id_              Market ID
    /// @return owner           Address of the market owner (payout tokens transferred from this address)
