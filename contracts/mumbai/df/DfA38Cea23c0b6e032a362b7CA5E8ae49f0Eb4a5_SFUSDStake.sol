...
contract SFUSDStake is Ownable {
    ...
    uint256 public constant WITHDRAW_PERIOD = 70 seconds;
    ...
    modifier onlyModerator() {
        require(
            owner() == msg.sender || moderatorAddress == msg.sender,
            "Only owner or moderator can do this transaction"
        );
        _;
    }
    ...
    modifier onlyAdmin() {
        require(
                ...
                isAdmin[msg.sender],
            "Only admin or moderator can do this transaction"
        );
        _;
    }
    ...
    constructor(
        ...
        require(
            ...
            "Invalid moderator wallet address"
        );
    ...
    function setContractClosed(bool _contractClosed) external onlyModerator{
        contractClosed = _contractClosed;
    }

    // set SFUSD Address in the contract
    function setSFUSDaddress(address _SFUSDaddress) external onlyOwner {
        require(_SFUSDaddress != address(0), "Invalid tokenAddress");
        SFUSDaddress = _SFUSDaddress;
    }

    // change moderator address
    ...
    function deposit(uint256 _amount) external {
        ...
        uint256 depositFee = _amount.mul(DEPOSIT_FEE).div(TOTAL_PERCENTAGE);

        uint256 amountToDeposit = _amount.sub(depositFee);
        ...
        depositsLength[msg.sender] = depositsLength[msg.sender].add(1);
        ...
        if (investors[msg.sender].totalRewardAmount >= _amount) {
            rewardPool = rewardPool.add(amountToDeposit);
            investors[msg.sender].totalRewardAmount = investors[msg.sender]
                .totalRewardAmount
                .sub(_amount);
            depositState[_id].compoundedInvestment = amountToDeposit;
        } else {
            capitalLockedPool = capitalLockedPool.add(
                amountToDeposit.sub(
                    investors[msg.sender].totalRewardAmount
                )
            );
            depositState[_id].pocketInvestment = amountToDeposit.sub(investors[msg.sender].totalRewardAmount);
            rewardPool = rewardPool.add(
                investors[msg.sender].totalRewardAmount
            );
            
            depositState[_id].compoundedInvestment = investors[msg.sender].totalRewardAmount;

            
            investors[msg.sender].pocketInvestment = investors[msg.sender].pocketInvestment.add(amountToDeposit.sub(investors[msg.sender].totalRewardAmount));
            investors[msg.sender].totalRewardAmount = 0;
        }

        investors[msg.sender].nextClaimDate = block.timestamp.add(WITHDRAW_PERIOD);
    ...
    function claimRewards() external {
        ...
        require(
            investors[msg.sender].nextClaimDate <=
                block.timestamp,
        ...
        rewardPool = rewardPool.sub(claimableAmount);
    ...
    function withdrawCapital(uint256 id) external {
        ...
        capitalLockedPool = capitalLockedPool.sub(
            depositState[id].pocketInvestment
        );
        rewardPool = rewardPool.sub(claimableReward);

        investors[msg.sender].pocketInvestment = investors[msg.sender].pocketInvestment.sub(depositState[id].pocketInvestment);
        ...
        capitalWithdrawn = capitalWithdrawn.add(depositState[id].depositAmount);
    ...
    function withdrawPocketInvestment() external {
        ...
        require(IERC20(SFUSDaddress).balanceOf(address(this))>=refundAmount,"Not enough SFUSD");
    ...
    function getClaimableReward(
        ...
        if(lastROIDate>=depositState[_id].depositAt){
                ...
                .div(TOTAL_PERCENTAGE.mul(WITHDRAW_PERIOD));
        }else{
               profit = depositState[_id]
                .depositAmount
                .mul(DAILY_ROI * 7)
    ...
    function migrateInvestorToV2(
        ...
        if (investors[_investor].totalRewardAmount >= _amount) {
            rewardPool = rewardPool.add(_amount);
            investors[_investor].totalRewardAmount = investors[_investor]
                .totalRewardAmount
                .sub(_amount);
            
            depositState[_id].compoundedInvestment = _amount;
        } else {
            uint256 _pocketAmount = _amount.sub(investors[_investor].totalRewardAmount);
            ...
            rewardPool = rewardPool.add(investors[_investor].totalRewardAmount);
            depositState[_id].compoundedInvestment = investors[_investor].totalRewardAmount;
            

            
            investors[_investor].pocketInvestment = investors[_investor].pocketInvestment.add(_pocketAmount);
            investors[_investor].totalRewardAmount = 0;
        }

        depositsLength[_investor] = depositsLength[_investor].add(1);
        investors[_investor].nextClaimDate = _depositAt.add(WITHDRAW_PERIOD);
        totalInvested = totalInvested.add(_amount);
    ...
    function removeInvestorOnWithdraw(address _investor) private{
        require(_investor!=address(0),"Invalid investor address");
        require(investors[_investor].investor!=address(0),"This address has no investment in the contract");
        ...
        for(uint256 i = 0;i<length;i++){
            if(depositState[currentInvestorDeposits[i]].state){
                totalAmountLocked = totalAmountLocked.add(depositState[currentInvestorDeposits[i]].depositAmount);
            }
        ...
        capitalLockedPool = capitalLockedPool.sub(investors[_investor].pocketInvestment);
        totalInvestors = totalInvestors.sub(1);
        totalInvested = totalInvested.sub(totalAmountLocked.mul(TOTAL_PERCENTAGE).div(TOTAL_PERCENTAGE.sub(DEPOSIT_FEE)));
