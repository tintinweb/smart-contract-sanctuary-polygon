...
// File: chainlink/VRFConsumerBaseV2.sol
    ...
    ReentrancyGuard
{
    ...
    //Token address that validates the winner, wrapper and wallet functions
    address NULL_ADDRESS = 0x0000000000000000000000000000000000000000;

    //Wallet office that receives the fee rates
    address _office = 0x71061fBB51c8c243c08D80056B1fE933E5500779;
    ...
    //Raffle token is native
    bool public isWrapper = false;
    ...
    //Run only owner
    modifier onlyOwner() {
        require(_operador == _manager.getOwner());
        _;
    }
    ...
    /// @param manager contract address Manager
   
    constructor(address manager) ERC721("GoldNumber", "luckynumber.gg")
    {
    ...
    function getOperador() public view returns (address)
    {
        return _operador;
    } 
    ...
    /// @param value state
    function setState(uint value) public onlyManager
    {
        state = value;
    }
    ...
    function setResult(uint256 randomWords) public onlyManager
    {
        //tokenCount total number purchased in the raffle
        uint result = (randomWords % tokenCount);
    ...
    /// @param _id raffle contract
    /// @param _index raffle contract
    /// @param _raffleName raffle title
    /// @param _quant total numbers
    /// @param _startTime date start
    /// @param _finishTime date finish
    ...
    function init(
        ...
        uint _finishTime, 
        uint256 _price, 
        address _token) public onlyManager
    {
       ...
       isWrapper = _token == NULL_ADDRESS ? true : false;
       ...
       quant = _quant;
       price = _price;
       tokenSymbol = isWrapper == true ? "MATIC" : IERC20(token).symbol();
       state = 1;
    ...
    /// @notice modify office address and fee
    /// @param office wallet or contract that receives the fees
    /// @param fee tax
    ...
    {
        return isWrapper ? address(this).balance : IERC20(token).balanceOf(address(this));
    }
    /// @dev function used to validate the randomization of the number drawn
    /// @return true or false
    function validateRequest() public view returns(bool)
    {
        /**
        ...
        */
        if(block.timestamp > finishTime || quant == tokenCount && state == 1)
        {
            return true;
        }
        return false;
    ...
    function claimReward() public onlyOneBlock nonReentrant
    {
        /**
        * @dev check if you are the champion. check if you have already claimed the rewards.
        */
        require(msg.sender == winner && claimed == false && closed == true, "not winner");
    ...
    function setClose() public onlyManager
    {
         if(tokenCount == 0)
         {
            closed = true;
    ...
    function mint(uint[] memory ids) public payable onlyOneBlock nonReentrant
    {
        /**
        * @dev image hardcode
        */
        string memory metadata = "https://luckynumber.gg/metadata.json";
        ...
        */
        require(isContract(msg.sender) == false, "Not contract");
        /**
        * @dev checks state
        */
        ...
        uint[] memory tokensId = new uint[](ids.length);

        while (i < ids.length) 
        { 
            uint itemId = ids[i];
            /**
            * @dev check the number has already been purchased
            */
            if(indexTokens[itemId] == 0)
            {
                /**
                * @dev checks if the purchased number is greater than the quantity
                */
                require(itemId <= quant, "greater than quantity");
                _mint(itemId, metadata);
                tokensId[j] = itemId;
                j++;
            }
            else 
            {
                /**
                * @dev remove from the list the number that has already been purchased
                */
                delete tokensId[tokensId.length-1];
            }
            i++;
        }
        require(j > 0, "numbers purchased");

        /**
        * @dev checks if the purchased number is greater than the quantity
        */
        uint256 total = price.mul(i);
    ...
    /// @notice deposit 
    /// @dev deposits the value of the NFT of the numbers purchased in the contract
    /// @param total total number purchased * price
    function deposit(uint256 total) private
    {
        require(msg.value >= total, "not amount");
        /**
        * @dev transfers the fee amount to the office contract
        */
        payable(_office).transfer(total.div(_fee));
    ...
    {
       uint fee = total.div(_fee);
       /**
       * @dev transfers the fee amount to the office contract
       */
       IERC20(token).transferFrom(msg.sender, _office, fee);

       /**
       * @dev transfers the fee amount to the office contract
       */
       IERC20(token).transferFrom(msg.sender, address(this), total.sub(fee));
      
    }
    /// @notice mint the nft of the purchased number
    /// @param itemId purchased number
    /// @param _tokenURI metadata json
    /// @return NFT index
    function _mint(uint itemId, string memory _tokenURI) internal returns(uint) 
    {
        _safeMint(msg.sender, tokenCount);
        _setTokenURI(tokenCount, _tokenURI);
        
        items[tokenCount] = Item(itemId, block.timestamp, payable(msg.sender));
        indexTokens[itemId] = tokenCount;
        tokenCount++;
