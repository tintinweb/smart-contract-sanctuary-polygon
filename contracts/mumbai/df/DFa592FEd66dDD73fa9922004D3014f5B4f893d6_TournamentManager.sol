...
contract TournamentManager is VRFConsumerBaseV2 {
    ...
    event RoundFinished(uint256 round, uint256[] looserTeamIds);
    event tournamentFinished(uint256 winner);
    ...
    function submitRound(uint256[] memory loosers) public onlyOwner {
        ...
        for (uint256 i = 0; i < loosers.length; i++) {
            require(
                ...
                "One or more losers already lost or are not in the competition"
            );
        }
        removeLosers(loosers);
        round_state = ROUND_STATE.PREPARATION;
        emit RoundFinished(currentRound, loosers);
        currentRound++;
    }
    ...
    function closeTournament() public onlyOwner {
        ...
        emit tournamentFinished(getAllTeamIdStillInCompetition()[0]);
    }
    ...
    function fulfillRandomWords(
        ...
        if (requestShuffle == true) {
            shuffleRandom = randomWords[0];
            emit ReturnedShuffleRandomness(randomWords[0]);
            requestShuffle = false;
        }

        if (requestRound == true) {
            roundRandom.push(randomWords[0]);
            emit ReturnedRoundRandomness(roundRandom[currentRound]);
            requestRound = false;
        }
    ...
    function createTeam(
        ...
        uint256[5] memory gotchiSpecials,
        uint256 gotchiLeader,
        ...
        require(
            gotchiSpecialsAreValid(gotchiSpecials),
            "Gotchi speical invalid"
        );
        require(
            checkGotchiFormationValidity(gotchiFormation, gotchis) == true,
    ...
    function kickTeam(uint256 teamId) public onlyOwner {
        Team memory teamToRemove = teamIdToTeam[teamId];
    ...
    function wrapToTeam(
        ...
        newTeam.gotchiSpecials = gotchiSpecials;
    ...
    function modiftyTeamAfterInscription(
        ...
        require(
            checkGotchiFormationValidity(
                gotchiFormation,
                getTeamGotchiTeam(teamIndex)
            ) == true,
            "Gotchi formation not valid"
        );
        require(
    ...
    function checkGotchiFormationValidity(
        uint256[10] memory gotchiFormation,
        uint256[5] memory gotchiTeam
    ) public view returns (bool) {
        // Check if all gotchis from gotchiTeam are present in gotchiFormation
        for (uint256 i = 0; i < 5; i++) {
            bool found = false;
            for (uint256 j = 0; j < 10; j++) {
                if (gotchiFormation[j] == gotchiTeam[i]) {
                    found = true;
                    break;
                }
            }
            if (!found) {
                return false; // A gotchi from gotchiTeam is not present in gotchiFormation
            }
        }

        // Check that empty slots in gotchiFormation are set to zero
        for (uint256 k = 0; k < 10; k++) {
            if (
                gotchiFormation[k] != 0 &&
                !contains(gotchiTeam, gotchiFormation[k])
            ) {
                return false; // A gotchi not included in gotchiTeam occupies a position in gotchiFormation
            }
        }

        // Check that no gotchi appears twice in gotchiFormation
        for (uint256 m = 0; m < 10; m++) {
            for (uint256 n = m + 1; n < 10; n++) {
                if (
                    gotchiFormation[m] != 0 &&
                    gotchiFormation[m] == gotchiFormation[n]
                ) {
                    return false; // A gotchi is duplicated in gotchiFormation
                }
            }
        }

        return true; // The gotchiFormation is valid
    }

    // Utility function to check if an element is present in a uint256[] array
    function contains(
    ...
    3=brn
    */
    function gotchiSpecialsAreValid(
        uint256[5] memory gotchiSpecials
    ) public view returns (bool) {
        // Check if each element in the array is within the valid range (0 to 4, inclusive).
        for (uint256 i = 0; i < gotchiSpecials.length; i++) {
            if (gotchiSpecials[i] < 0 || gotchiSpecials[i] > 4) {
                return false; // If the element is outside the valid range, return false.
            }
        }
        return true; // If all elements are valid, return true.
    }
    ...
    function getTeamGotchiSpecials(
        uint256 teamId
    ) public view returns (uint256[5] memory) {
        return teamIdToTeam[teamId].gotchiSpecials;
    }
