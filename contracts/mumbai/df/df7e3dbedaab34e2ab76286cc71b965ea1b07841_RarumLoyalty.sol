...
import "./IRarumLoyalty.sol";

/**
 * @dev ERC721-compatible contract to control on-chain benefits
 */
contract RarumLoyalty is IRarumLoyalty, TieredMetadataToken {

    using ModifiedEnumerableMapUpgradeable for ModifiedEnumerableMapUpgradeable.StringToBytesMap;

    bytes4 public constant RARUM_LOYALTY_INTERFACE_ID = type(IRarumLoyalty).interfaceId;
    ...
    {
        return
            interfaceId == RARUM_LOYALTY_INTERFACE_ID ||
    ...
    GenericTypedMessage
{
    ...
    function _addTierURI(string memory _newURI) internal {
        emit NewTier(tierCIDs.length);
    ...
    function tierList() external view returns (string[] memory) {
        return tierCIDs;
    }

    function balancesOf(address _owner)
        external
        view
...
import "../ERC721/IERC721GenericMetadata.sol";
interface IRarumLoyalty is IGenericMetadata {
 ...
 */
contract ERC721GenericMetadata is Initializable, IGenericMetadata, AccessControlEnumerableUpgradeable, Roles {
