...
contract ItemsMetadataHandler is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    mapping(uint256 => ItemMetadata) itemsMetadata;
    ...
    function setItemMetadata(uint256 tokenId, ItemMetadata calldata metadata) external onlyOwner{
        itemsMetadata[tokenId] = metadata;
    }

    function getItemMetadata(uint256 tokenId) external view returns(string memory){
        bytes memory name = abi.encodePacked(itemsMetadata[tokenId].name);
        bytes memory description = abi.encodePacked(itemsMetadata[tokenId].description);
        bytes memory image = _buildSVG(_buildImage(itemsMetadata[tokenId].image));
        return string(_buildJSON(name, description, image));
    }
...
interface MetadataHandlerMigratedLike {
