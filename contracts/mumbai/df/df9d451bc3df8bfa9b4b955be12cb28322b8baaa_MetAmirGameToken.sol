...
contract MetAmirGameToken is EIP20Interface {
    ...
    function deleteExpiredHoldLimits(address _owner) public {
        if (holdLimits[_owner].length > 0) {
           bool isDelete; 
           for (uint i = 0; i < holdLimits[_owner].length; i++) {               
               if(isDelete){
                   i--;
               ...
               if (block.timestamp >= holdLimits[_owner][i].Expire) {
                   for (uint j = i; j < holdLimits[_owner].length - 1; j++) {
                       holdLimits[_owner][j] = holdLimits[_owner][j + 1];
                   }
                   delete holdLimits[_owner][holdLimits[_owner].length - 1];
                   holdLimits[_owner].length--;
                   isDelete = true;
               }
            }
        }
