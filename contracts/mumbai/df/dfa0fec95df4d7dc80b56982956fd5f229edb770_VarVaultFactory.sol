...
import "./AutomatedVarVault.sol";
import "clones-with-immutable-args/ClonesWithImmutableArgs.sol";
import "@openzeppelin/contracts/utils/Context.sol";

contract VarVaultFactory is Context {
    event VarVaultDeployed(
        address creator,
        address newVarVault,
    ...
    function createVarVault(
        address m_depositToken,
        address m_assetToken,
        address m_assetOracle
    ) public returns (address) {
        AutomatedVarVault clone;
        bytes memory data = abi.encodePacked(
            m_depositToken,
            m_assetToken,
            m_assetOracle
        );
        clone = AutomatedVarVault(implementation.clone(data));

        //emit Event
        emit VarVaultDeployed(
            _msgSender(),
            address(clone),
            m_depositToken,
            m_assetToken
