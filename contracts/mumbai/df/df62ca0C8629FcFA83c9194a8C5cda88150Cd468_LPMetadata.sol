...
contract LPMetadata is VRFConsumerBase, Ownable {
    ...
    uint256 totalEntries = 2435;

	event RandomnessRequested(bytes32 indexed requestId, address indexed roller);
	event RandomnessFulfilled(bytes32 indexed requestId, uint256 indexed result);
	...
	function requestRandomness() external onlyOwner returns (bytes32 requestId) {
		require(LINK.balanceOf(address(this)) >= linkFee, "Not enough LINK in the contract");
	...
	function defineShift() external onlyOwner {
        shift = (uint256(keccak256(abi.encode(vrfResult, 0))) % totalEntries) + 1;
		emit ShiftDefined(vrfResult, shift);
	}
