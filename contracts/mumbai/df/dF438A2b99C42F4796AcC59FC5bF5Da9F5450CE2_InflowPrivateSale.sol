...
contract InflowPrivateSale is Ownable, ReentrancyGuard {
    ...
    function setReward(
        ...
        for (uint256 i = 0; i < airdropAddress.length; i++) {
            ...
            if (holderMap[airdropAddress[i]].amountTotal == 0) {
                ...
                holderMap[airdropAddress[i]].amountTotal =
                    holderMap[airdropAddress[i]].amountTotal +
                    amount[i];
            }
        ...
        nonReentrant
    {
        ...
        require(
            vestedAmount >= amount,
            string.concat(
                "releaseInternal: cannot release tokens, not enough vested tokens",
                Strings.toString(vestedAmount),
    ...
    function releaseMultiple(address beneficiary, uint256 amount) public {
        require(
            amount <= holderMap[beneficiary].amountTotal,
            "releaseMultiple: cannot release tokens, not enough tokens"
        );

        uint256 rewardCount = _getVestingSchedulesCountByBeneficiary(
            beneficiary
        );
        ...
        if (rewardCount == 1) {
            bytes32 vestingScheduleId = this
                .computeVestingScheduleIdForAddressAndIndex(beneficiary, 0);
            ...
            for (uint256 i = 0; i < rewardCount; i++) {
                ...
                uint256 remainingBalance = _computeReleasableAmount(
                    vestingSchedule
                );
    ...
    function updateAdminWalletAddress(address newAdminAddr) public onlyOwner {
