...
// File: VlinnNFTMarketplaceV2.sol
...
contract VlinnNFTMarketplaceV2 is Initializable, PausableUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    IVlinnNFTFactory private vlinnNFTFactory;
    bytes4 private constant _INTERFACE_ID_ERC2981 = 0x2a55205a;
    uint96 private platformFee;
...
/// @custom:oz-upgrades-unsafe-allow constructor
    ...
    function initialize(uint96 _platformFee,address _feeRecipient, IVlinnNFTFactory _vlinnNFTFactory) initializer public {
        require(_platformFee <= 1000 && _platformFee>=0, "can't > 10 percent");
        __Pausable_init();
        __Ownable_init();
    ...
    function listNft(
        ...
        emit ListedNFT(_nft, _tokenId, _price, _msgSender());
    }
    ...
    {
        ListNFT memory listedNFT = listNfts[_nft][_tokenId];
        require(listedNFT.seller == _msgSender(), "not listed owner");
        IERC721(_nft).transferFrom(address(this), _msgSender(), _tokenId);
        delete listNfts[_nft][_tokenId];
        emit UnlistedNFT(_nft, _tokenId, _msgSender());
    }
    ...
    function buyNFT(
        ...
        //Send Royalty
        totalPrice = sendRoyaltyFee(_nft, listedNft.price, totalPrice);
        
        //Send Payment
        sendPayment2Seller(listedNft.seller, totalPrice);
        
        // Transfer NFT to buyer
        sendNFT2Buyer(listedNft.nft, listedNft.tokenId, _msgSender());
    ...
    function buyNFTFromCrossmint(
        ...
        // Transfer NFT to buyer
        sendNFT2Buyer(listedNft.nft, listedNft.tokenId, _to);
    ...
    function sendRoyaltyFee(address _nft, uint256 _price, uint256 totalPrice )
        internal
        returns (uint256)
    {
    ...
    function sendNFT2Buyer(address _nft, uint256 _tokenId, address _to)
    internal
    {
    ...
    function updatePlatformFee(uint96 _platformFee) external onlyOwner {
        require(_platformFee <= 1000 && _platformFee >= 0, "can't more than 10 percent");
        platformFee = _platformFee;
    }
