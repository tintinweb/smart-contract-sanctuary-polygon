...
import {UserInventoryStorage} from "./UserInventoryStorage.sol";
...
contract UserInventoryFacet is Base {
    ...
    function getAllowlists(address user_)
        external
        view
        returns (uint256[] memory allowlistIds, uint256[] memory balances)
    {
        if (_usersFacet().isUserFrozen(user_)) {
            revert UserIsFrozen();
        }
        UserInventoryStorage storage userInventory = s.userInventory[user_];
        allowlistIds = userInventory.allowlistsIds;
        balances = new uint256[](allowlistIds.length);
        for (uint256 i = 0; i < allowlistIds.length; i++) {
            balances[i] = userInventory.allowlistsBalance[allowlistIds[i]];
        }
    }
    ...
    function getRaffles(address user_) 
        external 
        view 
        returns (uint256[] memory raffleIds, uint256[] memory balances) 
    {
         if (_usersFacet().isUserFrozen(user_)) {
            revert UserIsFrozen();
        }
    ...
    function getRafflesWon(address user_) 
        external 
        view 
        returns (uint256[] memory raffleIds, uint256[] memory balances) 
    {
        ...
        UserInventoryStorage storage userInventory = s.userInventory[user_];
        raffleIds = userInventory.raffleWonIds;
        balances = new uint256[](raffleIds.length);
        for (uint256 i = 0; i < raffleIds.length; i++) {
            balances[i] = userInventory.raffleWonBalance[raffleIds[i]];
        }
    }
