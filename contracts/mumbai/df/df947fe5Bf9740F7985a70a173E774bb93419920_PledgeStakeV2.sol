 ...
 contract PledgeStakeV2 {
    ...
    function stake(uint256 _amount) external {
     ...
     function _updateRewards(address _user) private {
        uint256 directRewards = rewards[_user].directRewards;
        ...
        rewards[_user].totalRewards = directRewards.add(roundRewards).add(levelRewards);
     ...
     function getRoundRewards(address _user) public view returns (uint256) {
        ...
        for (uint256 i = lastProcessedStakeIndex[_user]; i < stakes[_user].length; i++) {
            ...
            uint256 roundNumber = (block.timestamp - userStake.startTime) / ROUND_DURATION;
