    ...
    UUPSUpgradeable
{
    ...
    function mint(
        ...
        address erc20,
        address paidTo,
        uint8 cut, // 10 for 10 % cut
        bytes memory signature
    ) public payable onlyNewhash(signature, validUntil) {
        if (
            recover(tokenId, validUntil, amount, price, erc20, signature) ==
            false
        ) revert InvalidSignature();
        if (price > 0) {
            uint256 royalty = (price * cut) / 100;
            if (erc20 == address(0)) {
                if (price == msg.value)
                    payable(paidTo).transfer(price - royalty);
                else revert WrongETHValue();
            } else if (erc20 != address(0)) {
                IERC20(erc20).transferFrom(msg.sender, paidTo, price - royalty);
                IERC20(erc20).transferFrom(msg.sender, address(this), royalty);
            }
        }
        _mintBatch(account, tokenId, amount, "0x");
    }
