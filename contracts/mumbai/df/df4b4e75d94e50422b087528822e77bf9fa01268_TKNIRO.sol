...
contract TKNIRO is ERC20,AccessControl, Ownable {
    ...
    constructor(uint256 _maxSupply) ERC20("TKNIRO", "IRO") {
    ...
    function claimTokens(uint256 _amount, bytes32 _signature, bytes32[] calldata _merkleProof) public  {
        ...
        require(_amount > 0, "IRO: Invalid amount");
        require(totalSupply() + _amount * (10 ** decimals()) <= maxSupply, "Max supply exceeded!");
        require(!txnClaimed[_signature],  "IRO: Expired transaction");

        bytes32 leaf = keccak256(abi.encode(_signature, _amount, msg.sender));
        require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), "IRO: Unauthorized");
    ...
    function payment(address _reciever, uint256 tax, uint256 price) public{
        _transfer(msg.sender, owner(), tax * (10 ** decimals()));
        _transfer(msg.sender, _reciever, price * (10 ** decimals()));
    }
