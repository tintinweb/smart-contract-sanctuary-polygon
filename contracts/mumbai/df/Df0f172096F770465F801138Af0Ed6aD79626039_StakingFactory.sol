...
contract StakingFactory is AccessControl, Ownable {
    ...
    event StakingContractStakingOwnerUpdated(address indexed stakingAddress, address stakingOwner);

    /**
     * @dev Setup role for deployer.
     * Setup contract params.
     * @param _balanceVaultAddress - balance vault address.
     * @param _adminAddress - fees reciever.
     */
    constructor(address _balanceVaultAddress, address _adminAddress) {
    ...
    function approveStaking(
        ...
        if (_fees != 0) {
            require(
                balanceVault.getBalance(_stakingOwner) >= _fees,
                "[StakingFactory.approveStaking] Insufficient user balance"
            );
            balanceVault.decreaseBalance(_stakingOwner, _fees);
     ...
     * @param _stakingOwner - Purchased user address (staking owner).
     */
    function changeStakingContractStakingOwner(address _stakingAddress, address _stakingOwner) external onlyOwner {
    ...
    /**
     * @dev Get length of created staking contract array.
     */
    function getStakingArrayLength() public view returns (uint256) {
...
contract StakingV2 is AccessControl, Ownable, Pausable, ReentrancyGuard {
    ...
    event WithdrawnToken(address indexed userAddress, uint256 tokenAmount);
    event RewardClaimed(address indexed userAddress, uint256 tokenAmount);
    event RewardRefunded(address indexed userAddress, uint256 refundAmount);
     ...
     */
    function addPoolReward() external beforeStakeStarted onlyRole(STAKINGOWNER) {
        uint256 maxPoolReward = poolStakeLimit * percentage * duration / 100 / 365;
        IERC20 token = IERC20(tokenAddress);
        require(token.balanceOf(msg.sender) >= maxPoolReward, "[StakingV2.addPoolReward] Not enought tokens to add");
        token.safeTransferFrom(msg.sender, address(this), maxPoolReward);
     ...
     */
    function withdrawExceedPoolReward() external onlyRole(STAKINGOWNER) afterStakeStarted {
        uint256 actualRewardAmount = (poolStakeTotal * percentage * duration) /
            100 /
            365;
        require(
            poolReward >= actualRewardAmount,
            "[StakingV2.withdrawExceedPoolReward] Pool reward should be more than actual reward amount"
        );
        uint256 exceedAmount = poolReward - actualRewardAmount;
        poolReward -= exceedAmount;
        safeTokenTransfer(msg.sender, exceedAmount);
     ...
     */
    function withdrawToken() external afterStakeEnded nonReentrant {
        ...
        safeTokenTransfer(msg.sender, userInfo[msg.sender].stakeAmount);
        emit WithdrawnToken(msg.sender, userInfo[msg.sender].stakeAmount);
        ...
        nonReentrant
    {
        if (poolReward == 0) {
            ...
            safeTokenTransfer(msg.sender, refundAmount);

            emit RewardRefunded(msg.sender, refundAmount);
        }
        else {
            uint256 unclaimAmount = getUserUnclaimAmount(msg.sender);
            require(
            ...
            safeTokenTransfer(msg.sender, unclaimAmount);
    ...
    function safeTokenTransfer(address _userAddress, uint256 _tokenAmount)
        internal
    {
        IERC20 token = IERC20(tokenAddress);
        ...
        if (_tokenAmount >= tokenBalance) {
            token.safeTransfer(_userAddress, tokenBalance);
        } else {
