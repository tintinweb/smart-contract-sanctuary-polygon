...
contract LULU is ERC721Enumerable, Ownable {
 ...
 // event welcome_user_reward(address payable indexed indUser, bool friend1mint, bool friend2mint, uint256 new_reward);
  ...
  // function to put on or off the trigger

  function settrigger(bool _setting) public onlyOwner{
    trigger_on = _setting;
...
// BURN FNIC1 AND BECOME A PR


function BECOMEAPR(uint256 _mintAmount,  address whitelisted1, address whitelisted2)public payable {
      require(state == 0 || state == 1, "the contract is not in state 0 or 1");
      require(msg.value >= cost*_mintAmount , "you need to pay more for this nft");
      uint256 supply = totalSupply();
      IERC721Burnable burn_contract = IERC721Burnable(address(fnic1));
      require(burn_contract.balanceOf(msg.sender)>0 , "not FNIC1");
      uint256 ownerMintedCount = addressMintedBalance[msg.sender];
      require(ownerMintedCount + _mintAmount <= nftPerPRLimit, "max NFT per address exceeded");
      require((isPR(payable(msg.sender)) == false), "YOU ARE A PR");
      require(isWhitelisted(whitelisted1)== false, "first address is already whitelisted");
      require(isWhitelisted(whitelisted2) == false, "second address is already whitelisted");
...
/// MINT FOR PR
 ...
 function PRmint(uint256 _mintAmount,  address whitelisted1, address whitelisted2) public payable {
    require(msg.value >= cost*_mintAmount , "you need to pay more for this nft");
     require(state == 0 || state == 1 || state == 2, "the contract is not in state 0 1 2");
    uint256 supply = totalSupply();
    require(_mintAmount > 0, "need to mint at least 1 NFT");
    require(_mintAmount <= maxMintAmount, "max mint amount per session exceeded");
    require(supply + _mintAmount <= maxSupply, "max NFT limit exceeded");
    ...
    require(isWhitelisted(whitelisted1)== false, "first address is already whitelisted");
    require(isWhitelisted(whitelisted2) == false, "second address is already whitelisted");
    require(isPR(payable (msg.sender)), "you are not a PR");
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 /*function mint(uint256 _mintAmount,  address whitelisted1, address whitelisted2) public payable {
  ...
  /// set state trigger
    ...
    function trigger_on_off(uint256 _trigger) public onlyOwner{
      trigger = _trigger;
    }
  ...
  function privatemint () public payable {
    ...
    require(state == 1 || state==2, "it's not time for private mint");
    require(isWhitelisted(msg.sender), "user is not whitelisted");
    require(!paused, "the contract is paused");
    uint256 supply = totalSupply();
    uint256 ownerMintedCount = addressMintedBalance[msg.sender];
    require(ownerMintedCount + 1 <= nftPerAddressLimit, "max NFT per address exceed");
    //require(_mintAmount > 0, "need to mint at least 1 NFT");
    //require(_mintAmount <= maxMintAmount, "max mint amount per session exceeded");
  ...
  function publicmint (uint256 _mintAmount) public payable {
    ...
    require(state == 2, "it's not time for public sale");
    require(!paused, "the contract is paused");
    uint256 supply = totalSupply();
    require(_mintAmount > 0, "need to mint at least 1 NFT");
    require(_mintAmount <= maxMintAmount, "max mint amount per session exceeded");
