...
interface IDeployer {
    function changeLaunchpadState(address _token) external;
...
contract LaunchPad is Ownable, Pausable, ReentrancyGuard, IEnums {
    ...
    bool public kycRequest;
    bool public auditRequest;
    ...
    event launchpadActionChanged(bool usingWhitelist, uint256 endOfWhitelistTime);
    event launchpadWhitelistUsersChanged(address[] users, uint256 action);
    ...
    event UpdateKycStatus(bool newStatus);
    event UpdateAuditStatus(bool newStatus);
    ...
    function claimFund() external nonReentrant {
        ...
        if (launchPadType == LAUNCHPAD_TYPE.NORMAL) {
            ...
            amountEarned =
                (depositedAmount[msg.sender] * totalSellingAmount) /
                totalDeposits;
    ...
    function finishSale() external restricted nonReentrant {
        ...
        uint256 liquidityTokenPayment = launchPadType == IEnums.LAUNCHPAD_TYPE.NORMAL?
            (totalDeposits - tokenPaymentfee) * liquidityPercent / 100000 :
            (totalDeposits - tokenPaymentfee);
        uint256 liquidityToken = launchPadType == IEnums.LAUNCHPAD_TYPE.NORMAL? 
            liquidityTokenPayment * listingRate * 10**ERC20(tokenSale).decimals() / (1e18) : 
            totalSellingAmount;
        ...
        deployer.changeLaunchpadState(tokenSale);
    ...
    function updateKycStatus(bool _newStatus) public onlyAdmin {
        kyc = _newStatus;
        emit UpdateKycStatus(_newStatus);
    }

    function updateAuditStatus(bool _newStatus) public onlyAdmin {
        audit = _newStatus;
        emit UpdateAuditStatus(_newStatus);
    }
    ...
    function grantWhitelist(address[] calldata _users) external onlyOwner {
        ...
        emit launchpadWhitelistUsersChanged(users, 0);
    }
    ...
    function revokeWhitelist(address[] calldata _users) external onlyOwner {
        ...
        emit launchpadWhitelistUsersChanged(users, 1);
    }
...
contract LaunchpadDeployer is OwnableUpgradeable {
    ...
    function changeLaunchpadState(address _token)
        external
    {
        require(
    ...
    function _sendTokenToLaunchContract(
        ...
        if(_launchpadType == IEnums.LAUNCHPAD_TYPE.NORMAL) {
            ...
            uint256 tokensToLiquidity = (_listingRate *
                _cap *
                _liquidityPercent / 100000 *
                10**ERC20(_tokenSale).decimals()) / 10**18;
            ...
            ERC20(_tokenSale).transferFrom(
                msg.sender,
                _launchpad,
                tokensToDistribute + tokensToLiquidity + adminTokenSaleFee
            );
        } else {
            ...
            if (_adminTokenSaleFee > 0) {
                adminTokenSaleFee =
                    (_totalSellingAmount * _adminTokenSaleFee) /
                    10000;
            }
            ERC20(_tokenSale).transferFrom(
                msg.sender,
