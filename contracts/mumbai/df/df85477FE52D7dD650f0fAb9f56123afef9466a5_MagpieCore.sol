...
contract MagpieCore is ReentrancyGuard, Ownable, Pausable, IMagpieCore {
  ...
  function swapIn(SwapInArgs calldata args,bool tokenBridge) public payable ensure(args.swapArgs.deadline) override returns(uint256[] memory amountOuts, uint64, uint64) {
    ...
    if(tokenBridge){
      ...
      return (amountOuts, coreSequence, tokenSequence);
       ...
       emit SwappedIn(args, amountOuts, coreSequence, 0, msg.sender);
  ...
  function swapOut(
    ...
    if (payload.destGasTokenAmount > 0){
      SwapArgs memory sourceToNative;

      uint256 i = 0;
      for(i=0; i<args.swapArgs.routes.length; ++i){
        sourceToNative.routes[i].amountIn = payload.destGasTokenAmount;
        sourceToNative.routes[i].hops[i]=args.swapArgs.routes[i].hops[i];
      }

      sourceToNative.assets[0]=payload.fromAssetId;
      sourceToNative.assets[1]= 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE;
      sourceToNative.to = payable(payload.to);
      sourceToNative.amountOutMin = args.swapArgs.amountOutMin;
      sourceToNative.deadline = args.swapArgs.deadline;
