...
contract Factory is IFactory, IOwnable {
        ...
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < tokenIdsLength; i++) {
            rentShare.forToken(address(voucher.token), tokenIds[i]).updateShare(
    ...
    function seizeTnft(ITangibleNFT tnft, uint256[] memory tokenIds)external onlyOwner{
        uint256 length = tokenIds.length;
        for(uint256 i = 0; i < length; i++){
            uint256 token = tokenIds[i];
            bool ableToSeize = tnft.storageEndTime(token) + 180 days < block.timestamp;
            require(ableToSeize);
    ...
    function seizeFractionTnft(ITangibleFractionsNFT ftnft, uint256[] calldata fractionTokenIds)external onlyOwner{
        ...
        bool ninetyPassed = (tnft.storageEndTime(tnftTokenId) + 90 days < block.timestamp);
