...
contract NoWar is Ownable, ERC721Royalty, PullPayment, ContextMixin {

    using Counters for Counters.Counter;

    uint256 constant internal MAX_TOKENS = 100_000;
    uint256 private _tokenSalePrice = 1 ether;
    ...
    constructor() ERC721("Million Voices", "NOWAR") {
        _setDefaultRoyalty(_msgSender(), 480);
		_baseTokenExtension = '';
    }
    ...
    function buyToken(uint _tokenId) external payable {
        ...
        // first sell
        require(ownerOf(_tokenId) == contractOwner, "Token already sold");
        require(msg.value >= _tokenSalePrice, "Not enough funds");

        safeTransferFrom(contractOwner, _msgSender(), _tokenId);
    }
    ...
    function setSalePrice(uint256 tokenSalePrice) external onlyOwner {
        _tokenSalePrice = tokenSalePrice;
    }
