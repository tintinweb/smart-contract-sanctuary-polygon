...
// File: github.com/OpenZeppelin/openzeppelin-contracts/blob/master/contracts/token/ERC20/utils/SafeERC20.sol
...
interface IHangman{
    ...
    function supportsInterface(bytes4 interfaceId) external view returns (bool);
    function tokenURI(uint256 tokenId) external view returns (string memory);
    function createToken(address _to, string memory metadataCID, uint tokenId) external view returns (string memory);
    function burnToken(uint256 tokenId) external;
    function createMultipleToken(address _to, string memory metadataCID, uint start, uint end) external;
}

contract InnovfinMarket is ReentrancyGuard {
    using Counters for Counters.Counter;
    Counters.Counter private _itemIds;
    Counters.Counter private _itemsSold;
    Counters.Counter private _requests;
    ...
    struct MintRequest{
     ...
     mapping(uint256 => MintRequest) private idToMintRequest;
    ...
    function createMarketItemERC20(
        address nftContract,
        uint256 tokenId,
        uint256 price
        // IERC20 token
    ...
    function createMintRequest(
        ...
        _requests.increment();
        uint256 requestId = _requests.current();

        idToMintRequest[requestId] = MintRequest(
            requestId,
            nftContract,
            msg.sender,
            uri,
        ...
        //transfer iedu
        token.transferFrom(msg.sender, ieduTo, ieduPrice);
    ...
    function createMarketSaleERC20(
        address nftContract,
        uint256 itemId,
        IERC20 token
        ) public nonReentrant {
            ...
            //require(price <= token.balanceOf(idToMarketItem[itemId].seller), "Balance too low");
            emit MarketItemSold(
                itemId,
                msg.sender
                );
    ...
    function marketMint(
        ...
        return nftToken.createToken(msg.sender, uri, tokenId);
    }

    function marketMintIEDU(
        IHangman nftToken,
        uint tokenId,
        string memory uri,
        IERC20 token,
        ...
        token.transferFrom(msg.sender, toUser, ieduCost);
        return nftToken.createToken(msg.sender, uri, tokenId);
    }
