...
/**
 *Submitted for verification at cronoscan.com on 2022-05-09
*/
...
ReentrancyGuard
{
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. ABS to distribute per block.
        uint256 lastRewardTime; // Last block number that ABS distribution occurs.
        uint256 accSkyGPerShare; // Accumulated ABS per share, times 1e18. See below.
    ...
    function initialize(
        ...
        rewardPerSecond = TOTAL_REWARDS.div(DEFAULT_RUNNING_TIME); // =0.0009499924000607996 ABS/sec = 60000 ABS / (731 days * 24h * 60min * 60sec)
    ...
    returns (uint256)
    {
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            ...
            uint256 _absReward = _generatedReward.mul(pool.allocPoint).div(
                totalAllocPoint_
            );
            accSkyGPerShare = accSkyGPerShare.add(
                _absReward.mul(1e18).div(lpSupply)
            );
        }
    ...
    // Update the given pool's ABS allocation point and deposit fee. Can only be called by the owner.
    function setPool(
        uint256 _pid,
        uint256 _allocPoint,
        uint16 _depositFeeBP,
    ...
    function depositNFT(
        ...
        user.rewardDebt = user.amount.mul(poolInfo[_pid].accSkyGPerShare).div(
            1e18
        );
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint_ > 0) {
            ...
            pool.accSkyGPerShare = pool.accSkyGPerShare.add(
                _absReward.mul(1e18).div(lpSupply)
            );
        }
        pool.lastRewardTime = block.timestamp;
    ...
    function _harvestReward(
        ...
        PoolInfo memory pool = poolInfo[_pid];
        uint256 _pending = user.amount.mul(pool.accSkyGPerShare).div(1e18).sub(
            user.rewardDebt
        );
        if (_pending > 0) {
            if (_taxedReward) {
                ...
                _safeSkyGTransfer(insuranceFund, _half_boosted_pending);
    ...
    // Deposit LP tokens to MasterChef for ABS allocation.
    ...
    // Safe ABS transfer function, just in case if rounding error causes pool to not have enough ABS.
    function _safeSkyGTransfer(address _to, uint256 _amount) internal {
        IERC20 _abs = IERC20(abs);
        uint256 _bal = _abs.balanceOf(address(this));
        if (_amount > _bal) _amount = _bal;
        if (_amount > 0) {
            _abs.transfer(_to, _amount);
        }
    }
