...
contract PolygonBSGTest {
    ...
    uint256 private constant maxAddFreeze = 12 minutes; 
    ...
    uint256 private constant boosterPoolTimeLimit = 2 hours;
    ...
    function _isBooster(address _user, uint256 _prevMax) private {
        uint256 toCheck;
        ...
        if(orderInfos[_user].length > toCheck && !(userAchieve[_user].isbooster) && myTeamUsers[_user].length >= 1) {
            if(block.timestamp >= orderInfos[_user][toCheck-1].unfreeze && userInfo[_user].maxDeposit >= 500e6)  {
                uint256 volume;
                ...
                if(volume >= userInfo[_user].maxDeposit) {
                    ...
                    userAchieve[_user].boosterPoolTill = 3 hours;
                    userAchieve[_user].boosterAcheivedAmount = _prevMax;
        ...
        for (uint256 i = 0; i < orderInfos[_user].length; i++) {
            ...
            if (block.timestamp > order.unfreeze && !order.isClaimed) {
                ...
                if(userAchieve[_user].isbooster){
                    ...
                    if(userAchieve[_user].boosterAcheivedAmount < order.amount) {
                        boosterIncome = userAchieve[_user].boosterAcheivedAmount.mul(boostercycleRewardPercents).div(baseDivider);
                        staticReward = (order.amount.sub(userAchieve[_user].boosterAcheivedAmount)).mul(normalcycleRewardPercents).div(baseDivider);
                ...
                bool isEligible = _isEligibleForDirect(_user);
                if(!isEligible && !(userAchieve[_user].isbooster) && user.totalRevenue > user.maxDeposit.mul(limitProfit).div(baseDivider)){
                    staticReward = 0;
                } else if(!isEligible && userAchieve[_user].isbooster && user.level < 4 && user.totalRevenue > user.maxDeposit.mul(boosterLimitProfit).div(baseDivider)) {
                    staticReward = 0;
                }
    ...
    function _isEligibleForDirect(address _user) private view returns(bool) {
        bool isEligible;
        uint256 volume;
        ...
        if(volume >= 500e6 && myTeamUsers[_user].length >= 5) {
            isEligible = true;
        }
    ...
    function _updateReward(address _user, uint256 _amount, uint256 _prevMax) private {
        ...
        for(uint256 i = 0; i < referDepth; i++){
            if(upline != address(0)){
                ...
                if(isStopLoss40ofATH && !(user.start > lastFreezed)) {
                    newAmount = 0;
                } else if(isStopLoss20ofATH && !(user.start > lastFreezed)) {
                    newAmount = newAmount.div(2);
                } else if(newAmount > 0 && lastFreezed > 0 && userInfo[upline].start <= lastFreezed) {
                    newAmount = newAmount.div(2);
                }
                ...
                if(i > 4) {
                    ...
                    if(_isEligibleForDirect(upline)){
                        reward = newAmount.mul(directPercents).div(baseDivider);
                        upRewards.directs = upRewards.directs.add(reward);
                        userInfo[upline].totalRevenue = userInfo[upline].totalRevenue.add(reward);

                    } else if(userAchieve[upline].isbooster && user.totalRevenue < userInfo[upline].maxDeposit.mul(boosterLimitProfit).div(baseDivider)) {
                        reward = newAmount.mul(directPercents).div(baseDivider);
                        upRewards.directs = upRewards.directs.add(reward);
                        userInfo[upline].totalRevenue = userInfo[upline].totalRevenue.add(reward);
                        
                    } else if(user.totalRevenue < userInfo[upline].maxDeposit.mul(limitProfit).div(baseDivider)) {
                        reward = newAmount.mul(directPercents).div(baseDivider);
                        upRewards.directs = upRewards.directs.add(reward);
                        userInfo[upline].totalRevenue = userInfo[upline].totalRevenue.add(reward);
                    }
    ...
    function _balActived(uint256 _bal) private {
        if(_bal <= AllTimeHigh.mul(ATHSTOPLOSS40).div(baseDivider)){
            isStopLoss40ofATH = true;
            stopLossHitAt = _bal;
            lastFreezed = block.timestamp;
        }else if(_bal >= stopLossHitAt.mul(1000).div(baseDivider) && isStopLoss40ofATH){
            isStopLoss40ofATH = false;
            isRecovered = true;
        } else {
            if(_bal <= AllTimeHigh.mul(ATHSTOPLOSS20).div(baseDivider)){
                isStopLoss20ofATH = true;    
                depositFromInsurance();
                stopLossHitAt = _bal;
                lastFreezed = block.timestamp;
            }else if(_bal >= stopLossHitAt.mul(1000).div(baseDivider) && isStopLoss20ofATH){
