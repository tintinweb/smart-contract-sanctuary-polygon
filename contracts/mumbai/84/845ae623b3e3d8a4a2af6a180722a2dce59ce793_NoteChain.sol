...
contract NoteChain {
    ...
    function setIpfsFileHash(string calldata _newIPFSFileHash) external payable requireValidOrigin requireRegisteredAuthor requireEditFee {
        ...
        authorProfile[msg.sender].ipfsFileHash = _newIPFSFileHash;
        authorProfile[msg.sender].lastEditTime = block.timestamp;
    ...
    function getAuthorProfile() external view requireValidCaller requireRegisteredAuthor returns (Profile memory) {

        return authorProfile[msg.sender];
    }

    function getSpecificAuthorProfile(address _authorAddress) external view requireValidCaller requireContractManager returns (Profile memory) {

        return authorProfile[_authorAddress];
    }

    function getBootstrap() external view requireValidCaller requireRegisteredAuthor returns (uint256, uint256, Profile memory) {
