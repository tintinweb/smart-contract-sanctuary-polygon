...
contract FunctionsConsumer is FunctionsClient, ConfirmedOwner {
  ...
  function addSimulatedRequestId(address oracleAddress, bytes32 requestId) public onlyOwner {
    ...
    function withdraw(address _profile) public onlyOwner {
        Profile profile = Profile(_profile);
        profile.withdraw();
    }

    function donate(address _profile,  string calldata source, bytes calldata secrets, string[] calldata args, uint64 subscriptionId, uint32 gasLimit) public payable {

        Profile profile = Profile(_profile);
        profile.donate{value: msg.value}();
        ...
        //args[6] = bytes32ToString(transactionHash);
