...
contract Withdraw is Initializable, UUPSUpgradeable, ReentrancyGuardUpgradeable, AccessControlUpgradeable, PausableUpgradeable {

    // ERR-1-BW         Invalid address
    // ERR-2-BW         Withdraw not available for this address
    // ERR-3-BW         Nothing to withdraw
    ...
    event WithdrawRoyaltyRegistered(
        address funder,
        uint value,
        address receiver,
        uint royaltyId,
        uint timestampAvailability,
        uint timestamp
    );
    ...
    function registerWithdraw(address _address) 
        external 
        ...
        payable
    {
        require(_address != address(0), "ERR-1-BW");
        pendingWithdrawals[_address] += msg.value;

        emit WithdrawRegistered(
            msg.sender, 
            msg.value, 
            _address, 
            block.timestamp);
    ...
    function registerRoyaltyWithdraw(address _address) 
        external 
        onlyRole(FUNDER_ROLE)
        nonReentrant 
        whenNotPaused
        payable
    {
        ...
        uint royaltyId = _royaltyId.current();

        uint timestamp = block.timestamp;
        pendingWithdrawalsRoyalty[royaltyId] = RoyaltyWithdraw(
            timestamp + withdrawRoyaltyDelay,
            msg.value,
            _address
        );

        emit WithdrawRoyaltyRegistered(
            msg.sender, 
            msg.value, 
            _address, 
            royaltyId, 
    ...
    function _authorizeUpgrade(address newImplementation) 
        ...
        view 
    {
        require(newImplementation != address(0), "ERR-6-BA");
    }
