...
contract NftContract is ERC721Enumerable, AccessControl {
    ...
    /// @notice Event triggered when an address is trying to access a non-existent token
    error NonExistentToken();

    /// @notice Event triggered when parts of an avatarType are not defined
    error PartsNotDefined();

    /// @notice Event triggered when mintType was not defined
    ...
    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        ...
        writer = writer.writeStringProperty("external_url", cachedBaseURI);
        writer = writer.writeStringProperty("image", cachedBaseURI);
        ...
        string[] memory parts = avatarTypeConfig[currentAvatar.avatarType].partsName;
        ...
        writer = writer.writeStartObject();
        writer = writer.writeStringProperty("trait_type", "avatarType");
        writer = writer.writeStringProperty("value", avatarTypeConfig[currentAvatar.avatarType].name);
        writer = writer.writeEndObject();

        writer = writer.writeStartObject();
        writer = writer.writeStringProperty("trait_type", "mintType");
        writer = writer.writeStringProperty(
            "value",
            bytes32ToString(mintTypeNames.get(bytes32(uint256(currentAvatar.mintType))))
        );
        writer = writer.writeEndObject();
