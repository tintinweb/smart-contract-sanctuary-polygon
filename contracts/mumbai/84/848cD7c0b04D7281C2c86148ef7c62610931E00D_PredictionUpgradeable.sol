 ...
 * @title PredictionV2
 */
contract PredictionUpgradeable is Ownable, Pausable, ReentrancyGuard,KeeperCompatibleInterface {
    ...
    address tokenAddress = 0xC2Afd86e21eA4082f2581D4F248AcFD64aAfB84c;//token contract address
    address blackHoleAddress = 0x000000000000000000000000000000000000dEaD;

    uint256 public decrementPeriod = 274910;//decreasing reward period

    uint256 public averageMining = 10 * 10 ** 18;//the current amount of CPT mined per assetUnit

    uint256 public minMining = 1 * 10 ** 18;//the min amount of PT mined per assetUnit

    uint256 public assetUnit = 1 * 10 ** 18;//1 = ?wei

    uint256 public decreasingRatio = 1000;//decreasing ratio

    uint256 public miningStartBlock = 0;//mining start block

    address public PledgeContractAddress;//pledge contract address

    uint256 public rewardCardProportion = 5000;//Pledge distribution ratio

    uint256 public minBalance = 10000 * 10 ** 18;//contract minimum balance
    ...
    event ClaimTreasury(uint256 teamAmount);
    ...
    function setMinBalance(uint256 _minBalance) external onlyAdmin {
    ...
    function _claimTreasury() internal {
        uint256 team = treasuryAmount;
        _safeTransfer(teamAddress, team);
        treasuryAmount=0;
        emit ClaimTreasury(team);
    }
    ...
    constructor(
        ...
        symbolOracle['MATIC'] = 0x5B30f7b6AB2C441Bc6B09c74cB4caFBC0f555e56;
        symbolOracle['ETH'] = 0x1FD7a622cC8dFF46cf21EB340dbEAFE516FEb27D;
        symbolOracle['BTC'] = 0x0E9B68AdEE218056586bc21d0B25767981665c94;
        symbolSwitch['MATIC'] = 1;
        symbolSwitch['ETH'] = 1;
        symbolSwitch['BTC'] = 1;
        intervalSeconds['BTC'] = 180;
        intervalSeconds['ETH'] = 180;
        intervalSeconds['MATIC'] = 180;
        bufferSeconds['BTC'] = 100;
        bufferSeconds['ETH'] = 100;
        bufferSeconds['MATIC'] = 100;
        symbols.push('MATIC');
        symbols.push('ETH');
        symbols.push('BTC');
    ...
    function miningRewardsBalance(string memory _symbol, uint256 epoch) public view returns (uint256){
        if(IERC20(tokenAddress).balanceOf(address(this)) < minBalance){
            return 0;
        }
            ...
            for (uint256 i = 0; i < epochs.length; i++) {
                ...
                if (rounds[_symbol][epochs[i]].oracleCalled) {
                    ...
                    if (block.number > miningStartBlock + decrementPeriod) {
                        ...
                        averageMining = averageMining - (averageMining * (10000 - decreasingRatio) / 10000);
                        if (averageMining < minMining) {
                            averageMining = minMining;
                        }
                    }
    ...
    function extract(uint256 value) public virtual onlyOwner(){
    ...
    function _calculateRewards(string memory _symbol, uint256 epoch) internal {
        ...
        if (treasuryAmt > 0) {
            thePledgeToInject = treasuryAmt * rewardCardProportion / 10000;
