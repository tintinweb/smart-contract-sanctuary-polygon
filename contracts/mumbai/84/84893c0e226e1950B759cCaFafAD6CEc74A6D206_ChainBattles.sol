...
contract ChainBattles is ERC721URIStorage {
    ...
    function generateCharacter(uint256 tokenId) public returns(string memory){
        ...
        bytes memory svg = abi.encodePacked(
            ...
            getSvg(class),
            '<text x="0%" y="0%" class="base" dominant-baseline="middle" text-anchor="middle">',class,'</text>',
            '<text x="0%" y="5%" class="base" dominant-baseline="middle" text-anchor="middle">', "Levels: ",getLevels(tokenId),'</text>',
            '<text x="0%" y="10%" class="base" dominant-baseline="middle" text-anchor="middle">', "Strength: ",getStrength(tokenId),'</text>',
            '<text x="0%" y="15%" class="base" dominant-baseline="middle" text-anchor="middle">', "Speed: ",getSpeed(tokenId),'</text>',
            '<text x="0%" y="20%" class="base" dominant-baseline="middle" text-anchor="middle">', "Life: ",getLife(tokenId),'</text>',
            '</svg>'
        );
        return string(
            abi.encodePacked(
    ...
    function getSvg(uint256 number) private view returns (string memory) {
        return classSvgs[number];
    }
    ...
    function levelUp(Character memory character) internal {
        character.level += 1;
