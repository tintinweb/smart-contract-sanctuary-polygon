...
contract VRFPizza is VRFv2Consumer {
    uint256 public slices;
    event IncreasePizza(uint256 reqId, uint256[] numSlices);
   ...
   function setPizzaSliceNumber() external {
   ...
   function refillSlice() external {
        ...
        if (fulfilled) {
            emit IncreasePizza(reqId,numSlices);
            slices += numSlices[0];
        }
