    ...
    ERC20VotesUpgradeable
{
	...
	//set as public by etherauthority to use while unstake
    function getRewards(
        address _benefiter
    ) public view returns (uint256 pending) {
        if(balanceOf(_benefiter) > 0 ){
    ...
    function unstakeMax() public {
		//added by etherauthority
        require((tx.origin == msg.sender && !FrozenAccount[msg.sender]) || whitelistedAccount[msg.sender], "Invalid caller");
        ...
        // Farm Related Logic
        farmUtil(userLPTokens, msg.sender);
        if (cooldownActive) {
	...
	//added by etherauthority
    ///@notice function to set the whitelistedAccount state
    ///param _active the value whether true/false to set the cooldown state to active or now
    function setWhitelistAccount(
        address _user, bool _state
    ) external onlyRole(MANAGER_ROLE) {
        whitelistedAccount[_user] = _state;
    }
	//added by etherauthority
    ///@notice function to set the FrozenAccount state
    ///param _active the value whether true/false to set the cooldown state to active or now
    function setFrozenAccount(
        address _user, bool _state
    ) external onlyRole(MANAGER_ROLE) {
        FrozenAccount[_user] = _state;
    }
    ...
    function storeBets(
        ...
        for (uint256 i = 0; i < betinformation.length; i++) {
			//added by etherauthority
            if(betinformation[i].Id > 0 && betInfoWithId[betinformation[i].Id].Id == 0 ){
    ...
    function updateBets(
        ...
        for (uint256 i = 0; i < _Id.length; i++) {
		     //added by etherauthority to check for result
            if (betInfoWithId[_Id[i]].parlay && betInfoWithId[_Id[i]].result == 0) {
    ...
    function settleBets(
        ...
        for (uint256 i = 0; i < _Id.length; i++) {
	//added by etherauthority
            if(betInfoWithId[_Id[i]].result == 0 || betInfoWithId[_Id[i]].result > 3){
    ...
    function stake(address onBehalfOf, uint256 amount) public {
        ...
        userEVTracker[onBehalfOf] = calculateNewEVValue(
            lpTokensToMint,
            voi.expectedValue,
            onBehalfOf
        );
        _mint(onBehalfOf, lpTokensToMint);
        farmUtil(lpTokensToMint, onBehalfOf);
        liquidity += amount;
        updateAttributes();
	...
	//set as external by etherauthority
    function unStake(address to, uint256 amount) external {
        ...
        if (int256(amount) == maxUserWithdrawalAmount) {
            ...
            farmUtil(tokensToBurn, msg.sender);
    ...
    ///@param _user address for whome details are going to set
    function farmUtil(uint256 _amount, address _user) internal {
        // Farm Related Logic
        FarmInfo memory farm = updateFarm();
        ...
        if (balanceOf(_user) > 0) {
            uint256 pending = uint256(
                int256(
                    (balanceOf(_user) * farm.accRewardsPerShare) /
                        ACC_REWARD_PRECISION
                ) - user.rewardDebt
            );
            if (pending > 0) {
