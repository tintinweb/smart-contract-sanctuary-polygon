...
contract MetaFarm1155 is ERC1155 {
    ...
    modifier onlyManager() {
        require(gov.hasRole(MANAGER_ROLE, _msgSender()), "Caller is not Manager");
        _;
    }

    modifier onlyMinter() {
        require(gov.hasRole(AUTHENTICATOR_ROLE, _msgSender()), "Caller is not AUTHENTICATOR");
        _;
    }

    modifier onMaintenance() {
        require(!gov.locked(), "Under Maintenance");
        _;
    }

    event Breed(address indexed token, address indexed to, uint256 indexed tokenId, uint256 amount);
    ...
    */
    function updateBaseURI(string calldata _newBaseURI) external onlyManager {
        require(
            bytes(_newBaseURI).length != 0, "Empty BaseURI"
        );
        _setURI(_newBaseURI);
    }
    ...
    */
    function tokenURI(uint256 _tokenId) external view returns (string memory _tokenURI) {
        _tokenURI = string(
            abi.encodePacked(
                uri(_tokenId), _tokenId.toString()
            )
        );
    }

    function mint(address _to, uint256 _tokenId, uint256 _amount) external onMaintenance onlyMinter  {
        //  when `maxSupply[id] = 0`, allow unlimited minting
        uint256 _max =  maxSupply[_tokenId];
        uint256 _minted = minted[_tokenId];
        require(
            _max == 0 || _minted + _amount <= _max, "Reach max_supply"
        );
    ...
    */
    ...
    function _beforeTokenTransfer(
        ...
        require(
            !gov.blacklist(from) && !gov.blacklist(to), "User in the blacklist"
        );
