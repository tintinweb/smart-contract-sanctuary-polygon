//do not use this coin... is for testing purposes
...
contract testCoin is ERC20, AccessControl   {
    ...
    uint256 public tokensPerMATIC = 1; // tokens sent by every matic paid
    uint256 public tokensFaucetMint = 2100; //amount requested in faucet
    ...
    mapping(address => uint) public TotalMLM1lvl;
    uint256 public MATICxFullTransfer = 1 * (10 ** decimals()); // MATIC amount for full transfer

    constructor() ERC20("test Coin", "test") {

        totalSupplyToken = 100 * (10 ** decimals());
    ...
    function mint(address to, uint256 amount) public onlyRole(MINTER_ROLE) {
        require(amount < remainYear(currentYear), "MINT_YEAR_END");
        _mint(to, amount);
        setUpLine(to, WithoutMLM);
        totalSupplyYear[currentYear] += amount;
    }

    function mintYear(address to, uint256 amount, uint _Year) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _mint(to, amount);
        setUpLine(to, WithoutMLM);
        totalSupplyYear[_Year] += amount;
    }

    function remainYear(uint _Year) public view returns (uint256) {
        uint256 _rest;
        if ((_Year > 2020) && (_Year < 2032)) {
            _rest = 210000000000000 - totalSupplyYear[_Year];
        }
        return _rest;
    }
    
    function faucetMint() public {
        require(tokensFaucetMint < remainYear(currentYear), "FAUCET_END");
        _mint(msg.sender, tokensFaucetMint);
        totalSupplyYear[currentYear] += tokensFaucetMint;
        uint256 _div21 = ((tokensFaucetMint / 100) * percentForUpline) / 21;
        setUpLine(msg.sender, WithoutMLM);

        totalSupplyYear[currentYear] += (_div21 * 21);

        _mint(usUp[msg.sender].u01, _div21);
        _mint(usUp[msg.sender].u02, _div21);
        _mint(usUp[msg.sender].u03, _div21);
        _mint(usUp[msg.sender].u04, _div21);
        _mint(usUp[msg.sender].u05, _div21);
        _mint(usUp[msg.sender].u06, _div21);
        _mint(usUp[msg.sender].u07, _div21);
        _mint(usUp[msg.sender].u08, _div21);
        _mint(usUp[msg.sender].u09, _div21);
        _mint(usUp[msg.sender].u10, _div21);
        _mint(usUp[msg.sender].u11, _div21);
        _mint(usUp[msg.sender].u12, _div21);
        _mint(usUp[msg.sender].u13, _div21);
        _mint(usUp[msg.sender].u14, _div21);
        _mint(usUp[msg.sender].u15, _div21);
        _mint(usUp[msg.sender].u16, _div21);
        _mint(usUp[msg.sender].u17, _div21);
        _mint(usUp[msg.sender].u18, _div21);
        _mint(usUp[msg.sender].u19, _div21);
        _mint(usUp[msg.sender].u20, _div21);
        _mint(usUp[msg.sender].u21, _div21);
    ...
    function setUpLine(address new_acc, address father) internal virtual returns (bool) {
        if (usUp[new_acc].u01 == address(0)) {
            TotalMLM1lvl[new_acc] = 0;
            TotalMLM1lvl[father] += 1;
    ...
    function getBalanceYear(uint _Year) public view returns (uint256) {
        return totalSupplyYear[_Year];
    }

    //admin functions
    function updateTokensPerMATIC(uint256 amount) public onlyRole(DEFAULT_ADMIN_ROLE){
        require(amount > 0, "NO_VALID_AMOUNT");
        tokensPerMATIC = amount;
    }

    function updatePriceFullTransfer(uint256 amount) public onlyRole(DEFAULT_ADMIN_ROLE){
        require(amount > 0, "NO_VALID_AMOUNT");
    ...
    function updateTokensMintFaucet(uint256 amount) public onlyRole(DEFAULT_ADMIN_ROLE){
        require(amount > 0, "NO_VALID_AMOUNT");
    ...
    function buy() public payable returns (bool){
        ...
        uint256 _tokensToBuy = (msg.value / 100) / (tokensPerMATIC * (10 ** decimals()));

        require(_tokensToBuy < remainYear(currentYear), "EXCEEDED_AMOUNT_TO_BUY_THIS_YEAR");
    ...
    /*function sell(uint256 _amount) external {
      ...
      //  payable(msg.sender).transfer(_amount * tokensPerMATIC);
    //}
