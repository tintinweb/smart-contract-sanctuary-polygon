...
contract Roulette is VRFConsumerBaseV2, Ownable {
  ...
  */
  ...
  function bet(uint256 betType, uint256 number, bool _isMatic, address ERC20Address, uint256 amount) external payable {
      require(block.timestamp < betIdToBets[currentBetId].time + 6000000000, 'deadline for this bet is passed');
    ...
    emit BetPlacedInToken(currentBetId, msg.sender, ERC20Address, amount, betType, number, true );  
    ...
    override
  {
    ...
    function checkWinner(uint number, uint _requestId) internal {
      ...
      if (won) {
        winnings[b.bettorAddress] += b.betAmount * payouts[b.betType];
