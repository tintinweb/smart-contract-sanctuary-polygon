...
interface MBO_GENESIS_NFT_CONTRACT {
    function ownerOf(uint256 tokenId) external view returns (address owner);
}

contract MBOSpecialCollection is ERC721Opensea {
    event Mint(
    ...
    uint16 public GENESIS_CLAIM_LIMIT;

    MBO_GENESIS_NFT_CONTRACT public MBOGenesisNFTContract;

    mapping(uint256 => address) public genesisClaimedIdToOwner;
    mapping(uint256 => bool) public genesisTokensClaimed;
    mapping(uint256 => PhygitalClaim) public phygitalClaimData;

    uint256 public regularClaimTotalQty = 0;
    uint256 public genesisClaimTotalQty = 0;
    uint256 public freebiesClaimTotalQty = 0;
    uint256 public phygitalClaimTotalQty = 0;

    // =============== MINTING ===============
    function getBatchMintPrice(MintRequest[] calldata mintRequest) public pure returns (uint256) {
    ...
    function singleMint(
        ...
        // Check genesis token conditions
        if(mintRequest.isWithGenesis){
            // Check if the genesis claim limit has been reached
            require(
                genesisClaimTotalQty < GENESIS_CLAIM_LIMIT,
                "Genesis claim limit has been reached"
            );

            // Check if the genesis token has already been claimed
            require(
                !checkIsGenesisClaimed(mintRequest.genesisTokenId),
                "Genesis NFT has already been used for claiming"
            );
        }
        ...
        if(mintRequest.isWithGenesis){
            genesisTokensClaimed[mintRequest.genesisTokenId] = true;
            genesisClaimTotalQty += 1;
        }else{
            regularClaimTotalQty += 1;
        }

        emit Mint(
            msg.sender,
            totalSupply(),
            mintRequest.isWithGenesis,
            mintRequest.genesisTokenId
        );
    }

    /**
     * @dev Batch mint of NFT, discount automatically applied if genesis token is used
    ...
    function batchMint(
        ...
        uint256 totalMintPrice = getBatchMintPrice(mintRequests);

        require(
            msg.value >= totalMintPrice,
            "Insufficient ETH sent for minting"
        );
    ...
    function claimPhygital(
        ...
        // check mint signature
        require(
            checkMintSignature(phygitalClaimRequest.uid),
    ...
    function totalClaimed() public view returns (uint256) {
        return regularClaimTotalQty + genesisClaimTotalQty;
    }

    /**
     * @dev Checks if the genesis token has already been claimed
     * @param genesisTokenId The ID of the genesis token
     * @return bool True if the genesis token has already been claimed
     */
    function checkIsGenesisClaimed(
        uint256 genesisTokenId
    ) public view returns (bool) {
        // return genesisClaimedIdToOwner[genesisTokenId] != address(0);
        return genesisTokensClaimed[genesisTokenId];
    }

    // =============== Owner Functions ===============
    /**
     * @dev Changes the address of the MBO Genesis NFT contract
     * @param newGenesisNFTContract The address of the new MBO Genesis NFT contract
     */
    function changeGenesisNFTContract(
        address newGenesisNFTContract
    ) public onlyOwner {
    ...
    constructor(
        ...
        GENESIS_CLAIM_LIMIT = genesisClaimLimit;

        PUBLIC_SALES_START_TIMESTAMP = publicSalesStartTimestamp;

        MBOGenesisNFTContract = MBO_GENESIS_NFT_CONTRACT(genesisNFTAddress);
    }
}
...
contract MintSignatureVerifier {
    ...
    bytes32 private constant MINT_REQUEST_TYPEHASH =
        keccak256(
            "MintRequest(address to,string uri,bytes32 uid,uint256 price,bool isWithGenesis,uint256 genesisTokenId)"
        );
    bytes32 private constant DOMAIN_TYPEHASH =
        keccak256(
            "EIP712Domain(string name,string version,uint256 chainId,address verifyingContract)"
    ...
    function checkMintSignature(bytes32 uid) public view returns (bool) {
        // Check if the signature has been used before
