...
contract A4AAuction is Ownable {
    using SafeMath for uint256;

    address private a4A_NFT_address = 0x3a068D98752a5c0B19550b467c653739d2de9FAe;
    ...
    mapping(uint256 => address) private _maxBidder;
    ...
    function listItemForAuction(uint256 itemID, uint256 minPrice, uint256 endDate) external {
        require(a4A_items.ownerOf(itemID) == _msgSender());
        ...
        a4A_items.transferFrom(_msgSender(), address(this), itemID);
    ...
    function cancelAuction(uint256 itemID) external {
        ...
        if (_maxBidder[itemID] != address(0)){
    ...
    function bidForItem(uint256 itemID) external payable {
        require(block.timestamp < _auctionsEndDate[itemID]);
        require(msg.value > _listedItemsMinPrice[itemID]);
...
*/
    ...
    function claimAuctionResult(uint256 itemID) external {
        ...
        if(_maxBidder[itemID] == address(0)){
            ...
            uint256 sellerAmt = _itemPrice - marketAmt;
            (bool sentValue, ) = seller.call{value: sellerAmt}("");
            require(sentValue, "Failed to send BNB to the item owner.");

            //Transfer NFT to the buyer
            a4A_items.safeTransferFrom(address(this), _maxBidder[itemID], itemID);

            _listedItemsOwner[itemID] = address(0);
            _listedItemsMaxPrice[itemID] = 0;

            //check New owner & Not for sale
            require(a4A_items.ownerOf(itemID) == _maxBidder[itemID], "New owner not assigned.");
