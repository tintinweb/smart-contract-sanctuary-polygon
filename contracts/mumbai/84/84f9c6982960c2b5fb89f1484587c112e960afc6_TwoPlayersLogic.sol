...
contract TwoPlayersLogic is VRFConsumerBaseV2 {
    ...
    struct Game {
        ...
        uint256 time2;
        uint256 random;
        bytes32 proof;  // keccak256("word1_word2_word3")
    ...
    constructor(uint256 time_) VRFConsumerBaseV2(VRF) {
        time = time_;
    ...
    function move1(
        Name name_,
        bytes32 cipher_
    ) public payable returns(Game memory) {
        ...
        return games[name_][room][id[name_][room]];
    }
    ...
    function move2(
        ...
        require(games[name_][room][id_].move2 == 0, "Move 2 exists");

        games[name_][room][id_].player2 = msg.sender;
        games[name_][room][id_].move2 = uint256(move_);
        ...
        return games[name_][room][id_];
    }

    function getRoom() public payable returns(Room room) {
        if (msg.value == 1) {
            room = Room.One;
    ...
    function getPrize(Room room_) public pure returns(uint256 prize) {
        if (room_ == Room.One) {
            prize = 2;
    ...
    function winnerAddress(
        ...
        bytes32 cipher = keccak256(abi.encodePacked(move_, proof_));
    ...
    function claim(
        ...
        uint256 move_,
        bytes32 proof_
    ) public payable returns(address winner) {
        ...
        require(
            msg.sender == game.player2 && block.timestamp <= game.time2 + time,
            "Result is not ready"
        );

        if (game.player1 == msg.sender) {
            winner = winnerAddress(name_, room_, id_, move_, proof_);
            require(winner != address(0), "Claim error");
            game.proof = proof_;
            game.move1 = move_;
            game.winner = payable(winner);
        } else {
            game.winner = payable(game.player2);
        }
