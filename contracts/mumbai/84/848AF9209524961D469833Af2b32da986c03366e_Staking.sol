...
contract Staking {
    ...
    mapping(address => bool) public _addressToIsDelegator;
    mapping(address => uint256) public _addressToDelegatorStakedAmount;
    mapping(address => uint256) public _addressToDelegatorIndex;
    mapping(address => address) public _delegatorToSupportedValidator;
    ...
    modifier onlyDelegator() {
        require(_isDelegator(msg.sender), "Only delegator can call function");
        _;
    }
    ...
    function delegatorUnstake() public onlyEOA onlyDelegator {
        _delegatorUnstake();
    }

    function _delegateTo(address delegatingValidator) private {
        _stakedAmount += msg.value;
        _addressToDelegatorStakedAmount[msg.sender] += msg.value;

        if (_canBecomeDelegator(msg.sender)) {
    ...
    function _delegatorUnstake() private {
        uint256 amount = _addressToDelegatorStakedAmount[msg.sender];

        _addressToDelegatorStakedAmount[msg.sender] = 0;
        _stakedAmount -= amount;

        if (_isDelegator(msg.sender)) {
            _deleteFromDelegators(msg.sender);
        }
    ...
    function _deleteFromDelegators(address delegator) private {
        address delegatingValidator = _delegatorToSupportedValidator[delegator];
        require(
        ...
        require(
            _addressToDelegatorIndex[delegator] < _addressToDelegators[delegatingValidator].length,
            "index out of range"
        );

        // index of removed address
        uint256 index = _addressToDelegatorIndex[delegator];
        ...
        if (index != lastIndex) {
            ...
            _addressToDelegatorIndex[lastAddr] = index;
        }

        _addressToIsDelegator[delegator] = false;
        _addressToDelegatorIndex[delegator] = 0;
    ...
    function _appendToDelegatorSet(address newDelegator, address delegatingValidator) private {
        require(
            ...
            "Delegator set has reached full capacity. Please support someone else"
        );

        _addressToIsValidator[newDelegator] = true;
        _addressToValidatorIndex[newDelegator] = _addressToDelegators[delegatingValidator].length;
        _addressToDelegators[delegatingValidator].push(newDelegator);
    }

    function _isDelegator(address account) private view returns (bool) {
        return _addressToIsDelegator[account];
    }

    function _canBecomeDelegator(address account) private view returns (bool) {
        return !_isDelegator(account) && _addressToDelegatorStakedAmount[account] >= DELEGATION_THRESHOLD;
    }
}