...
contract tods is ERC1155, Ownable, Pausable {
    using SafeMath for uint256;
    uint256 public TOKEN_ID = 0;
    uint256 public minted;
    event ReturnEvent(uint256 ID);

    constructor() ERC1155("https://kculture.metadata.block-stars-dev.com/metadata/getmetadata/{id}"){
    ...
    function bulk_mint(uint amount, uint nft_amount) public payable{
        
        for(uint i = 0; i<=nft_amount;i++){
            TOKEN_ID = TOKEN_ID + 1;
            _mint(msg.sender, TOKEN_ID, amount, "");
            minted = minted+1;
            emit ReturnEvent(TOKEN_ID);
