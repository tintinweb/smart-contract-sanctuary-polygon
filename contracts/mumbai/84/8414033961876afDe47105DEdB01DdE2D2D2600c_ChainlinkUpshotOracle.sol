...
contract ChainlinkUpshotOracle is PriceOracleI, ChainlinkClient, AccessControlEnumerable {
  ...
  bytes32 public constant jobIdFloor = 0x6238643736396534653237643436373138633633616338653339346262613237;
  bytes32 public constant jobIdIndividual = 0x6231373436376661366366393431383439306561353231653966306331346662;
   ...
   */
  function fulfillFloor(bytes32 _requestId, uint256 _result) external recordChainlinkFulfillment(_requestId) {
    ...
    emit RequestFloorFulfill(_requestId, uint128(block.timestamp), uint128(_result), request.collection, request.pricingAsset);
  }
  ...
  function getUnderlyingFloorNFTPrice(address _collection, uint256 _decimals) external view override returns (uint128, uint128) {
    ...
    ResponseFloor memory response = floorPrices[_collection];

    uint256 oracleDecimals = 18;
    if (oracleDecimals < _decimals) {
      _decimals -= oracleDecimals;
      return (uint128(response.estimate * 10**_decimals), response.lastUpdated);
  ...
  function getUnderlyingIndividualNFTPrice(
    ...
    uint256 price = individualPrices[_collection][_tokenId].estimate;
