...
contract SphereToken is ERC20Detailed, Ownable {
    ...
    bool public approveRouter;
    bool public approvePairStableCoin;
    bool public allowStableStashForOwnContract;
    ...
    constructor() ERC20Detailed('Sphere Finance', 'SPHERE', uint8(DECIMALS)) {
        ...
        approveRouter = IERC20(stableCoin).approve(address(router), uint256(-1));
        approvePairStableCoin = IERC20(stableCoin).approve(address(pairStableCoin), uint256(-1));
        allowStableStashForOwnContract = IERC20(stableCoin).approve(address(this), uint256(-1));
    ...
    function _transferFrom(
        ...
        if ((partyArrayCheck[sender] || automatedMarketMakerPairs[recipient])) {
            require(
                amount <= maxSellTransactionAmount.div(partyListDivisor),
                'Party Sell Error amount'
            );
        }
        ...
        if (partyArrayCheck[sender] || partyArrayCheck[recipient]) {
            require(
                amount <= maxBuyTransactionAmount.div(partyListDivisor),
                'Party Buy Error amount'
            );
        }
    ...
    {
        if (!isPartyOver) {
            addPartyAddies(msg.sender, true);
        }
