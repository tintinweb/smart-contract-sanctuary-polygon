...
contract NFT is ERC721, Ownable {
    ...
    uint public MAX_TOKENS = 1000;
    uint private constant TOKENS_RESERVED = 20;
    uint public wlPrice = 0;
    uint public publicPrice = 5000000000000000;
    ...
    function Mint(uint256 _numTokens) external payable{
        if (WL[msg.sender] && _numTokens==1) {
            wlMint(_numTokens);
        }
        else if(WL[msg.sender] && _numTokens>1){
            wlMint(1);
            publicMint(_numTokens-1);
        }
        else{
    ...
    function publicMint(uint256 _numTokens) private {
        ...
        require(msg.value == _numTokens * publicPrice , "Insufficient funds.");
    ...
    function wlMint(uint256 _numTokens) private {
        require(isWLSaleActive, "The sale is paused.");
        require(_numTokens <= MAX_MINT_PER_TX, "You cannot mint that many in one transaction.");
        require(mintedPerWallet[msg.sender] + _numTokens <= MAX_MINT_PER_TX, "You cannot mint that many total.");
        uint256 curTotalSupply = totalSupply;
        require(curTotalSupply + _numTokens <= MAX_TOKENS, "Exceeds total supply.");
