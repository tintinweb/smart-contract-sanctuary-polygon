...
contract PaymentContract is Ownable {
    ...
    event PaymentSuccessful(uint256 amount, address token, address receiver);
    ...
    function payForOrder(
        ...
        uint256 expectedAmountWithPriceFeed = _amount * uint256(latestPrice);
        ...
        if (_token != stableCoin && _token != address(0)) {
            ...
            require(
                _checkedTokenAmount >= expectedAmountWithPriceFeed,
                "Insufficient amount!"
            );
    ...
    function requiredTokenAmount(uint256 _amount, address _token)
        public
        view
    ...
    function updateSlippage(uint256 _slippage) public onlyOwner {
        uint256 prevSlippage = slippage;
        slippage = _slippage;
        emit SlippageUpdated(prevSlippage, slippage);
    }

    function updatePriceFeed(address _priceFeedAddress) public onlyOwner {
        address prevPriceFeed = priceFeed;
        priceFeed = _priceFeedAddress;
        emit PriceFeedUpdated(prevPriceFeed, priceFeed);
    }
