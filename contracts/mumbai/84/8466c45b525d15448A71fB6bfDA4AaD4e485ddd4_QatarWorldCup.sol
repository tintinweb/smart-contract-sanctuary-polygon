...
contract QatarWorldCup is ERC721Enumerable, Ownable {
    /**
     * NOTE
     * Description: NFT Football Cup
     * Mint Teams, Win Prizes
     * Released 32,000 NFTs representing 32 teams participating in WC
     * Offer free mint random on matic for users with the cost around $5
     * 70% of the revenue is for reward users who victory NFT The remaining 30% is used for developing future projects of the team
     */

    using SafeMath for uint256;

    uint256 constant MAX_SUPPLY = 32000;
    uint256 constant USER_MAX_SUPPLY = 25600;
    ...
    uint256 constant BREAKPOINT_LEVEL_1 = 1000;
    uint256 constant BREAKPOINT_LEVEL_2 = 2000;
    uint256 constant BREAKPOINT_LEVEL_3 = 3000;
    uint256 constant BREAKPOINT_LEVEL_4 = 4000;
    uint256 constant BREAKPOINT_LEVEL_5 = 5000;
    ...
    uint256 public  mintPrice = 60000000000000000; //6 MATIC

    // mappings token to metadata URI
    ...
    //mapping team and rank to amount
    mapping(uint256 => mapping(uint256 => uint256)) private _amountNFTByTeamRank;

    // mapping team and rank to imageIndex (_teams -> _rank -> _imageIndex)
    ...
    //mapping id to breakpoint
    mapping(uint256 => uint256) public breakpoint;

    //mapping breakpoint discount 
    mapping(uint256 => uint256) public discountByBreakpoint;


    uint256 public maxQWCPurchase = 10;
    ...
    constructor() ERC721("QATAR WORLDCUP", "QWC") {
        ...
        // set rate discount level
        setDiscountLevel(50000, 40000, 30000, 20000, 10000);

    }

    modifier hasMintRunning() {
        uint256 currentTime = block.timestamp;
        require(mintStartTime > 0, "QWC: Mint has yet to be started");
        require(
            (mintStartTime <= currentTime && currentTime <= mintEndTime) ,
            "QWC: Now is not a mint time"
        );
        _;
    }
    ...
    function _mint721Token(address _to) internal{
        ...
        */
        ...
        // random rank
        uint256 _seed = IRandom(random).random(currentTokenId);
        if (
    ...
    function mintBatch721Token(uint256 _numberOfTokens) public payable {
        require(_numberOfTokens <= maxQWCPurchase, "QWC: Mint more than allowed at a time");
        require(totalSupply().add(_numberOfTokens) <= MAX_SUPPLY, "QWC: Purchase would exceed max supply of QWC");
        if(_msgSender() != owner()){
            require(totalSupply().add(_numberOfTokens) <= USER_MAX_SUPPLY, "QWC: Minting user limit reached");
        }

        // check token in range of breakpoint or not;
        uint256 _mintPriceInRange = getMintPriceByCurrentTokenId();
        require(_numberOfTokens.mul(_mintPriceInRange) <= msg.value, "Ether value sent is not correct");
    ...
    function burn(uint256 tokenId) external {
        require(
            ...
            "QWC: Only the TokenId holder can burn this tokenId"
        );
        _burn(tokenId);
    }
    ...
    function getNFTProperty(uint256 _tokenId) public view returns ( uint256 teams, uint256 rank, uint256 imageIndex){
        NFTProperty memory nftProperty = _nftProperty[_tokenId];
        return (nftProperty.teams, nftProperty.rank, nftProperty.imageIndex);
    }

    function getAmountNFTByTeamAndRank(uint256 _team, uint256 _rank) public view returns ( uint256 amount){
        return _amountNFTByTeamRank[_team][_rank];
    }

    function getMintPriceByCurrentTokenId() public view returns (uint256 _mintPrice){
        if( currentTokenId < BREAKPOINT_LEVEL_1){
            _mintPrice = mintPrice.sub(mintPrice.mul(discountByBreakpoint[BREAKPOINT_LEVEL_1]).div(denominator));
        }else if(currentTokenId < BREAKPOINT_LEVEL_2){
            _mintPrice = mintPrice.sub(mintPrice.mul(discountByBreakpoint[BREAKPOINT_LEVEL_2]).div(denominator));
        }else if(currentTokenId < BREAKPOINT_LEVEL_3){
            _mintPrice = mintPrice.sub(mintPrice.mul(discountByBreakpoint[BREAKPOINT_LEVEL_3]).div(denominator));
        }else if(currentTokenId < BREAKPOINT_LEVEL_4){
            _mintPrice = mintPrice.sub(mintPrice.mul(discountByBreakpoint[BREAKPOINT_LEVEL_4]).div(denominator));
        }else if(currentTokenId < BREAKPOINT_LEVEL_5){
            _mintPrice = mintPrice.sub(mintPrice.mul(discountByBreakpoint[BREAKPOINT_LEVEL_5]).div(denominator));
        }else{
    ...
    function setDiscountLevel(uint256 _level_1, uint256 _level_2, uint256 _level_3, uint256 _level_4, uint256 _level_5) public onlyOwner{
        discountByBreakpoint[BREAKPOINT_LEVEL_1] = _level_1;
        discountByBreakpoint[BREAKPOINT_LEVEL_2] = _level_2;
        discountByBreakpoint[BREAKPOINT_LEVEL_3] = _level_3;
        discountByBreakpoint[BREAKPOINT_LEVEL_4] = _level_4;
        discountByBreakpoint[BREAKPOINT_LEVEL_5] = _level_5;
    }
    ...
    */
    function setMintTime(
        ...
        require(
            ...
            _mintStartTime < _mintEndTime ,
            "QWC: Invalid mint time"
        );
    ...
    function _roll_team(
        ...
        if (
            ...
            _amountNFTByTeamRank[_teams[currentTokenId]][_nftProperty[currentTokenId].rank ]++;
...
pragma solidity ^0.8.4;
interface IRandom {
    function random(uint256 _bump) external returns (uint256 seed_);
}
