...
/// @title Pharo Phinance Contract
/// @author Aphilos
/// @notice Manages all financial transactions
contract PharoPhinance is IPharoPhinance, PharoEmits, Conversions, Ownable, AccessControl
{
    ...
    // need to update this to the DAO War Chest for production polygon
    address public constant pharoPheeAddress =
        0x3f15B8c6F9939879Cb030D6dd935348E57109637;
    uint16 public phee = 20; // 20% fee on all transactions.
    ...
    // todo: Jason, should this be inherited from Obeisk? No, should be deployed.
    // todo: Sean, we should use some sort of access control...

    constructor (address _riskPoolAddress,
                 address _reservePoolAddress,
                 address _phroTokenAddress,
                 address _newOwner)
    {
        ...
        _setupRole(PharoConstants.OPERATOR_ROLE, 0x3f15B8c6F9939879Cb030D6dd935348E57109637);
        _setupRole(PharoConstants.OPERATOR_ROLE, 0xc316Efa8aD1046b0DEf528e17E6f4136072731F7); // jax foundry dev address
        _transferOwnership(_newOwner);
    }

    function grantOperatorRole(address newOperator) public onlyOwner
    {
        _setupRole( PharoConstants.OPERATOR_ROLE, newOperator );
    }

    function getCBApproval(uint256 pharoId,
                           address token, 
                           uint256 approvalAmount, 
                           address coverBuyer) external returns(bool)
    {
        bool success = IERC20(token).approve( coverBuyer, approvalAmount ); // Emits an Approval event: https://docs.openzeppelin.com/contracts/2.x/api/token/erc20#IERC20-approve-address-uint256-
        emit RiskPoolApproved( approvalAmount, pharoId );

        return success;
    }

    function verifyFinancials(/*uint256 pharo_id,*/
                              address token, // PHRO Token for now
                              uint256 amount_req, 
                              address wallet_id) external view returns(bool)
    {
        uint256 amount_available = IERC20(token).allowance(wallet_id, riskPoolAddress);
        return amount_available >= amount_req;
    }

    /// @dev Moves the assets to the risk and reserve pools and the treasury
    function executeCBFinancials(uint256 pharoId,
                                 address token, // PHRO Token for now
                                 uint256 stakeAmount, 
                                 address coverBuyer) external returns(bool) 
    {
        // With sufficient funds, The CB stake is immediately transferred to pools and LPs
        uint256 funds_remaining = IERC20(token).allowance(address(coverBuyer), address(this));
        if (funds_remaining < stakeAmount) { return false; }

        // Tokens transferred to contract. Paid with PHRO for testing...
        IERC20(token).transferFrom(coverBuyer, address(this), stakeAmount);

        // RiskPool 50%
        uint256 riskPoolDepAmt = stakeAmount.div(100).mul(50);
        IERC20(token).approve(riskPoolAddress, riskPoolDepAmt);
        console.log("Risk Pool Approved");

        riskPool.deposit(pharoId, token, riskPoolDepAmt);
        console.log("Deposted to Risk Pool");

        emit RiskPoolDeposit(riskPoolDepAmt, coverBuyer);

        // todo: Market Maker/LP's should get paid here... **30% for mock
        // todo: send out the tokens for LP/MM
        //uint256 lpShare = stakeAmount.div(100).mul(30); // 15%
        // uint256 mmShare = stakeAmount.div(100).mul(15); // 15%

        // ReservePool 10%
        uint256 reservePoolDepAmt = stakeAmount.div(100).mul(10);
        IERC20(token).approve(reservePoolAddress, reservePoolDepAmt);
        console.log("Reserve Pool Approved");

        reservePool.deposit(pharoId, reservePoolDepAmt, token);
        console.log("Deposited to Reserve Pool");

        emit ReservePoolDeposit(reservePoolDepAmt, coverBuyer);

        // Pharo Phee 10%
        uint256 pharoPheeDepAmt = stakeAmount.div(100).mul(phee);
        IERC20(token).transfer(pharoPheeAddress, pharoPheeDepAmt);
        console.log(
            "Pharo Phee Transferred to Treasury. Phee: ",
            pharoPheeDepAmt
        );

        emit TreasuryDeposit(pharoPheeDepAmt, "Fees");

        return true;
    }

    /// @dev execute the lp transactions
    /// @dev Approve on the front-end for the stakeAmount
    /// @param providerAddress lp address
    /// @param stakeAmount the amount staked
    /// @param asset the staked asset
    /// @param pharoId the id of the Pharo
    function executeLPFinancials(address providerAddress, 
                                 uint256 stakeAmount, 
                                 address asset, 
                                 /*uint256 poolId,*/
                                 uint256 pharoId) external
    {
        // LP financials need to be placed in a pool

        // Approve on the front-end for the stakeAmount
        IERC20(asset).transferFrom(msg.sender, address(this), stakeAmount);

        // now we store the money for payouts if/when they happen
        IERC20(asset).approve(riskPoolAddress, stakeAmount);
        riskPool.deposit(pharoId, asset, stakeAmount);

        emit RiskPoolDeposit(stakeAmount, providerAddress);
    }

    /// @dev public function for Keeper contract to exectute payouts
    /// @param wallet_id the buyers address
    /// @param amount the amount to pay the buyer from this pool
    function payoutCoverPolicy(uint256 pharo_id, address payable wallet_id, /*address tokenAddress,*/ uint256 amount) 
        external returns(bool success) 
    {
        // rewards are not sourced from LPs, so they either need to be minted or...
        riskPool.withdraw(pharo_id, wallet_id, /*tokenAddress,*/ amount, "Withdraw from LP");

        return true;
    }

    // ---------- THESE FUNCTIONS ARE INCOMPLETE - ?? DELETE ?? ---------- //
    ...
    /// @dev remove liquidity from a Pharo/market
    /// @param pharoId the id of the Pharo/Market
    /// @param providerAddress the address of the provider
    /// @param amount how much to withdrawl
    function removeLiquidityFromPharo(/*uint256 poolId,*/
                                      uint256 pharoId,
                                      address payable wallet_id,
                                      /*address tokenAddress,*/
                                      uint256 amount,
                                      address providerAddress) public
    {
        string memory memo = "Withdraw from LP";
        riskPool.withdraw(pharoId, wallet_id, /*tokenAddress,*/ amount, memo);

        emit LiquidityRemoved(pharoId, providerAddress, amount);
    }
}
