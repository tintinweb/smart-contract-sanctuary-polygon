    ...
    ProjectUtils
{
    ...
    function initialize() external virtual initializer {
        ...
        __ERC721_init_unchained('BuckMint Protocol: Carbon Project Vintages', 'BUCKMINT-CPV');
        __Ownable_init_unchained();
        __Pausable_init_unchained();
    ...
    function setBuckMintContractRegistry(address _address) external virtual onlyOwner {
        contractRegistry = _address;
    }

    /// @notice Adds a new carbon project-vintage along with attributes/data
    /// @dev vintages can be added by data-managers
    function addNewVintage(address to, VintageData memory _vintageData) external virtual override onlyManagers whenNotPaused returns (uint256) {
    ...
    function updateProjectVintage(uint256 tokenId, VintageData memory _vintageData) external virtual onlyManagers whenNotPaused {
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view virtual override(AccessControlUpgradeable, ERC721Upgradeable, IERC165Upgradeable) returns (bool) {
...
contract Modifiers {
    modifier onlyBy(address _contractRegistry, address _owner) {
        require(_contractRegistry == msg.sender || _owner == msg.sender, 'Caller is not the registry, nor owner');
        _;
    }
}
...
contract ProjectUtils {
    function checkProjectTokenExists(address contractRegistry, uint256 tokenId) internal virtual {
        address c = IBuckMintContractRegistry(contractRegistry).carbonProjectsAddress();
    ...
    function getProjectId(address contractRegistry, uint256 tokenId) internal view virtual returns (string memory) {
        address c = IBuckMintContractRegistry(contractRegistry).carbonProjectsAddress();
