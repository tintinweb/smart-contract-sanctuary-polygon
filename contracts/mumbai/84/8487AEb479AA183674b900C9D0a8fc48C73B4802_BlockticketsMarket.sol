...
interface Blocktickets{

    function isTicket(uint256 tokenId) external returns(bool ticket);

    function idToOrganizerRoyalty(uint256 ticket) external view returns(address creator, uint256 royalty);
}

 contract BlockticketsMarket is IERC1155Receiver, Ownable, ReentrancyGuard{
  ...
  event ItemBought(
    ...
    require(Blocktickets(mintingContract).isTicket(tokenId),"Nft not a ticket");
   ...
   function hashSaleTransaction(
    ...
    (address minterAdd, uint256 royaltyPercentage) = Blocktickets(mintingContract).idToOrganizerRoyalty(tokenId);
  ...
  function EndSale(uint256 itemId) external nonReentrant {
    ...
    function gethfghg(uint256 ticket) public view returns(address , uint256 ){
        (address minterAdd, uint256 royaltyPercentage) = Blocktickets(mintingContract).idToOrganizerRoyalty(ticket); 
        return(minterAdd, royaltyPercentage);
    }
