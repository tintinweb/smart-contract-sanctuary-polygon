...
contract MetaForceContract is Initializable, Proxied, Governed, ReentrancyGuard, IMetaForceContract {
    ...
    uint256 public override distributionLoopsLimit;

    constructor() {}

    function setDistributionLoopsLimit(uint256 newLimit) external override onlyGovernor {
        distributionLoopsLimit = newLimit;
    }
    ...
    function distributionRewardReferal(
        ...
        uint256 count = 0;
        {
            ...
            for (uint256 i; i < rewardsRefers.length; ) {
                ...
                while (true) {
                    ...
                    emit LostMoney(refererId, _userId, dRewardUSD, false);

                    unchecked {
                        count += REFERRAL_TREE_STEP;
                    }

                    if (count > distributionLoopsLimit) {
                        emit OutOfGas(_userId, refererId, activate, _level, tempAmountMFS, false, i);
                        return;
                    }
                }
                ...
                if (activate) {
                    ...
                    if (typeReward == TypeReward.ONLY_MFS || core.getWorkflowStage() == Workflow.SaleOpen) {
                        ...
                        emit RevenueMFS(refererId, _userId, dRewardMFS, _level, true, false);
                    } else if (typeReward == TypeReward.ONLY_USD) {
                        core.giveStableFromPool(refererId, dRewardUSD);
                        amountMFSForRealization += dRewardMFS;
                        emit RevenueStable(refererId, _userId, dRewardUSD, _level, true, false);
                    } else {
                        ...
                        emit RevenueMFS(refererId, _userId, dRewardMFS / 2, _level, true, false);
                        emit RevenueStable(refererId, _userId, dRewardUSD / 2, _level, true, false);
                    }
                } else {
                    core.giveMFSFromPool(refererId, dRewardMFS);
                    emit RevenueMFS(refererId, _userId, dRewardMFS, _level, false, false);
                }
                unchecked {
                    ++i;
                }
        ...
        }
        {
            ...
            uint256 marketingRefererId = _userId;
            for (uint256 i; i < rewardsMarketingRefers.length; ) {
                ...
                while (true) {
                    ...
                    emit LostMoney(marketingRefererId, _userId, mRewardUSD, true);

                    unchecked {
                        count += MARKETING_TREE_STEP;
                    }

                    if (count > distributionLoopsLimit) {
                        emit OutOfGas(_userId, marketingRefererId, activate, _level, tempAmountMFS, true, i);
                        return;
                    }
                }
                ...
                if (activate) {
                    ...
                    if (typeReward == TypeReward.ONLY_MFS || core.getWorkflowStage() == Workflow.SaleOpen) {
                        ...
                        emit RevenueMFS(marketingRefererId, _userId, mRewardMFS, _level, true, true);
                    } else if (typeReward == TypeReward.ONLY_USD) {
                        core.giveStableFromPool(marketingRefererId, mRewardUSD);
                        amountMFSForRealization += mRewardMFS;
                        emit RevenueStable(marketingRefererId, _userId, mRewardUSD, _level, true, true);
                    } else {
                        ...
                        emit RevenueMFS(marketingRefererId, _userId, mRewardMFS / 2, _level, true, true);
                        emit RevenueStable(marketingRefererId, _userId, mRewardUSD / 2, _level, true, true);
                    }
                } else {
                    core.giveMFSFromPool(marketingRefererId, mRewardMFS);
                    emit RevenueMFS(marketingRefererId, _userId, mRewardMFS, _level, false, true);
                }
                unchecked {
                    ++i;
                }
