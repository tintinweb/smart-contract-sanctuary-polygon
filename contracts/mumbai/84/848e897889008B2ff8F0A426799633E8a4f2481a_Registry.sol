...
contract Registry {
    ...
    function registerGame(address _registryAddr, address _gameAddr) external {
    ...
    ) external view returns (GameReg[] memory) {
        RegistryState storage state = registries[_registryAddr];
        require(state.isInitialized, "uninitialized");
        return state.games;
    }

    function unregisterGame(address _registryAddr, address _gameAddr) external {
        RegistryState storage state = registries[_registryAddr];
