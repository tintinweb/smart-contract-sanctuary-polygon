...
contract HSAgency is Initializable, AccessControlUpgradeable, IHSAgency {
    ...
    function initialize(
        HSReferralsInterfaceV6_2 _referralContract,
    ...
    function updateDiscountFeeP(
        uint256 _discountFeeP
    ) external only(DEFAULT_ADMIN_ROLE) {
        discountFeeP = _discountFeeP;
    }

    function updateDistributionP(
        uint256[2] memory _distributionP
    ) external only(DEFAULT_ADMIN_ROLE) {
    ...
    function addRoot(address _level0) external only(MANAGER_ROLE) {
        ...
        require(
            address(referralContract) == address(0) ||
                !referralContract.checkReferrerActive(_level0),
            "IS_REFERRER"
        );
    ...
    function registerReferrer(address _addressReferrer) external {
        require(
            address(referralContract) == address(0) ||
                !referralContract.checkReferrerActive(_addressReferrer),
            "IS_REFERRER"
        );
        require(rootReferrer[_addressReferrer] != address(0), "OUT_OF_NETWORK");
        require(
            rootStatus[rootReferrer[_addressReferrer]],
            "LEV0_WERE_DISABLED"
        );
        require(
            directReferrer[msg.sender].referrer == address(0),
            "ON_NETWORK"
        );
        require(
            rootReferrer[_addressReferrer] == _addressReferrer ||
                (directReferrer[_addressReferrer].referrer != address(0) &&
                    uint(directReferrer[_addressReferrer].level) <
                    uint(Level.LEVEL2)),
            "INVALID_REFERRER"
        );
        ...
        DirectReferrer memory referrer = DirectReferrer(
            _addressReferrer,
            refLevel
        );
        ...
        emit ReferrerRegistered(
            msg.sender,
            _addressReferrer,
            rootReferrer[_addressReferrer],
            refLevel
        );
    }

    function getDirectReferrer(
        address referrer
    ) public view override returns (DirectReferrer memory) {
        return directReferrer[referrer];
    }

    function getDistributionP(
        address _user
    ) public view override returns (uint256 feeP1, uint256 feeP2) {
        if (
            _user == address(0) ||
        ...
        if (
            directReferrer[_user].level == Level.LEVEL1 ||
            directReferrer[_user].level == Level.LEVEL2
        ) {
    ...
    function calulateFee(
        uint256 _vaultOpenFeeP,
        address _user
    ) public view override returns (uint256) {
        if (
            rootReferrer[_user] != address(0) && rootStatus[rootReferrer[_user]]
        ) {
            return
                _vaultOpenFeeP > discountFeeP
                    ? _vaultOpenFeeP - discountFeeP
                    : _vaultOpenFeeP;
        }
    ...
    function distributeReward(
        ...
        if (feeP2 > 0) {
            ...
            emit RewardDistribution(
                _user,
                referralFee2,
                directReferrer[_user].referrer,
                directReferrer[_user].level
            );
        }
        return referralFee;
    }
    ...
    function claimRewards() external {
        ...
        reward[msg.sender] = 0;
        //change to method bot get reward from HS to usdc
