...
abstract contract ERC721AMarketplace is
    IERC721AMarketplace,
    ERC721AOperator,
    ReentrancyGuard
{
    ...
    //in the forom of tokenId => offerIds
    mapping(uint256 => uint256[]) public offersByTokenId;

    uint256 offersCount;

    /// @inheritdoc IERC721AMarketplace
    function versionERC721AMarketplace()
        external
        pure
        virtual
        override
    ...
    /** Buyer */
    function buyBatch(uint256[] memory tokenIds, uint256[] memory sellPrices)
        external
        virtual
        override
    {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            _transferTokens(tokenId, seller, buyer, buyPrice);

            //sets new selling price
            uint256 sellPrice = sellPrices[i];
            if(sellPrice == 0) revert ERC721AMKInvalidSellPrice();
    ...
    /** Buyer sets Offers for batches*/
    function setOffers(
        uint256[] memory tokenIds,
        uint256[] memory offerPrices,
        uint64 deadline
    ) external virtual override {
        if(tokenIds.length != offerPrices.length) revert ERC721AMKInvalidBatchLengths();

        if(deadline <= block.timestamp) revert ERC721AMKWrongDeadline();

        for (uint256 i; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            if(!_exists(tokenId)) revert ERC721AMKUnexistentToken();
            uint256 offerPrice = offerPrices[i];
            if(offerPrice == 0) revert ERC721AMKInvalidOfferPrice();
            address buyer = _msgSender();
            uint256 offerId = offersCount += 1;
            Offer memory offer = Offer(tokenId, offerPrice, buyer, deadline);
            offers[offerId] = offer;
            offersByBuyer[buyer].push(offerId);
            offersByTokenId[tokenId].push(offerId);
            emit NewOffer(buyer, tokenId, offerId);
        }
    }

    /** Seller */
    function acceptOffers(uint256[] memory offerIds)
        external
        virtual
        override
        nonReentrant
    {
        for (uint256 i; i < offerIds.length; i++) {
            ...
            if(buyer == address(0)) revert ERC721AMKUnexistentOffer();

            uint256 tokenId = offer.tokenId;
            address seller = _msgSender();
            if(ownerOf(tokenId) != seller) revert ERC721AMKNotTokenOwner();

            if(offer.offerDeadline < block.timestamp) revert ERC721AMKOfferDeadlineOver();
           
            offer.offerDeadline = 0;
            _transferTokens(tokenId, seller, buyer, offer.offerPrice);
            emit AcceptOffer(offerId);
        }
    }

    /** Buyer */
    function cancelOffers(uint256[] memory offerIds) external virtual override {
        for (uint256 i; i < offerIds.length; i++) {
            uint256 offerId = offerIds[i];
            Offer memory offer = offers[offerId];
            address buyer = offer.buyer;
            address sender = _msgSender();
            if(buyer != sender) revert ERC721AMKNotOfferOwner();
    ...
    function getOffersByToken(
        ...
        uint256[] memory offersByToken = offersByTokenId[tokenId];
        return _activeOffers(offersByToken, active, expired);
    }
    ...
    function getOffersByBuyer(
        ...
        uint256[] memory offersByBuyerCount = offersByBuyer[buyer];
        return _activeOffers(offersByBuyerCount, active, expired);
    }
    ...
    function _activeOffers(
        ...
        for (uint256 i; i < offers_.length; i++) {
            Offer memory offer = offers[offers_[i]];
            if (
                (offer.offerDeadline > block.timestamp && active) ||
                (offer.offerDeadline < block.timestamp && expired)
            ) {
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(ERC721AOperator)
        returns (bool)
    {
        return
            interfaceId == type(IERC721AMarketplace).interfaceId ||
            super.supportsInterface(interfaceId);
    }
}
...
abstract contract ERC721AOperator is IERC721AOperator, ERC721ARoyalty {
    ...
    mapping(uint256 => uint256) internal _ticketByEntryPos;

    // in the form of tokenId => paymentPrice
    mapping(uint256 => uint256) public priceByToken;

    function versionERC721AOperator() virtual external pure override returns (string memory){
    ...
    function setSellingPrice(uint256[] memory tokenIds, uint256[] memory sellPrices) external virtual override {
        if(tokenIds.length != sellPrices.length) revert ERC721AOPWrongBatchLengths();
        for(uint256 i; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            uint256 sellPrice = sellPrices[i];

            if(ownerOf(tokenId) != _msgSender()) revert ERC721AOPNotOwnedToken();

            if(sellPrice == 0) revert ERC721AOPInvalidSellPrice();
    ...
    function _saveTickets(
        ...
        for(uint256 i; i < amount; i++) {
            ...
            _ticketByEntryPos[tokenId] = entryPos;
            uint256 sellPrice = sellingPrices[i];
            if(sellPrice == 0) revert ERC721AOPInvalidSellPrice();
            priceByToken[tokenId] = sellPrice;
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721ARoyalty) returns (bool) {
        return interfaceId == type(IERC721AOperator).interfaceId ||
            super.supportsInterface(interfaceId);
    }
}
...
contract ERC721EFactory is ERC721FactoryWide, IERC721EFactory, Ownable {
    ...
    function versionERC721EFactory() override virtual external pure returns (string memory){
        return "1.0.0-beta.0+fob.rsv.iERC721EFactory";
    }

    function setTrustedForwarer(address forwarder) external virtual override onlyOwner {
    ...
    function deploy(
        ...
        emit EventDeployed(venue, contractAddress, block.number);
        return contractAddress;
    }


    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return interfaceId == type(IERC721EFactory).interfaceId ||
        super.supportsInterface(interfaceId);
    }
}
...
interface IERC721AMarketplace {
    ...
    event NewOffer(address indexed buyer, uint256 indexed tokenId, uint256 indexed offerId);
    event AcceptOffer(uint256 indexed offerId);
    event CancelOffer(uint256 indexed offerId);

    error ERC721AMKInvalidBatchLengths();
    error ERC721AMKInvalidSellPrice();
    ...
    error ERC721AMKUnexistentToken();
    ...
    function acceptOffers(uint256[] memory offerIds) external;
...
interface IERC721AOperator {
    ...
    error ERC721AOPWrongBatchLengths();
    error ERC721AOPNotOwnedToken();
    error ERC721AOPInvalidSellPrice();
...
interface IERC721E {
    ...
    event MintTickets(address indexed buyer, uint256 indexed entryType, uint256 amount);

    error ERC721EWrongPrice();
    error ERC721EWrongMaxSupply();
    error ERC721EWrongBatchLengths();
    error ERC721EReachedMaxSupply();
    error ERC721ETooManyTickets();
    error ERC721ENonExistentToken();
    error ERC721EWrongEntryPos();
    error ERC721WrongEventDate();
    error ERC721EWrongSaleEnd();
    error ERC721ESaleEnded();   
    ...
    /// @param sellingPrices array of prices the sender will sell the token/ticket
    function buyTickets(
        address receiver,
        uint256 userId,
        uint256 entryPos,
...
interface IERC721EFactory {
    ...
    function versionERC721EFactory() external pure returns (string memory);
...
contract ERC721E is IERC721E, ERC721AMarketplace, Ownable, AccessControl {
    ...
    constructor(
        ...
        ) = abi.decode(data, (string, address, address, address, address, address, uint96, uint96, bytes));

        _baseUri = baseUri;
        paymentToken = paymentToken_;

        _setupRole(DEFAULT_ADMIN_ROLE, accessControl);
        _setupRole(MINTER_ROLE, accessControl);
        _setupRole(CREATOR_ROLE, accessControl);
    ...
    function setEventDate(uint256 _eventDate) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function createEntry(
        ...
        uint256 maxBuy,
        uint256 saleEnd
    ) external virtual override onlyRole(CREATOR_ROLE) {
        if (price == 0) revert ERC721EWrongPrice();
        if (saleEnd < block.timestamp) revert ERC721EWrongSaleEnd();
        if (maxSupply == 0) revert ERC721EWrongMaxSupply();
        entriesCount += 1;
    ...
    /// @inheritdoc IERC721E
    function editEntryName(uint256 entryPos, string memory name) external virtual override onlyRole(CREATOR_ROLE) {
        if (entryPos > entriesCount) revert ERC721EWrongEntryPos();
        EntryType storage entry = _entryByPos[entryPos];
        entry.name = name;
    }

    /// @inheritdoc IERC721E
    function editEntryPrice(uint256 entryPos, uint256 price) external virtual override onlyRole(CREATOR_ROLE) {
        if (entryPos > entriesCount) revert ERC721EWrongEntryPos();
        if (price == 0) revert ERC721EWrongPrice();
        EntryType storage entry = _entryByPos[entryPos];
        entry.price = price;
    ...
    /// @inheritdoc IERC721E
    function editEntryMaxSupply(uint256 entryPos, uint256 maxSupply) external virtual override onlyRole(CREATOR_ROLE) {
        if (entryPos > entriesCount) revert ERC721EWrongEntryPos();
        EntryType storage entry = _entryByPos[entryPos];
        if (maxSupply < entry.sold) revert ERC721EWrongMaxSupply();
        entry.maxSupply = maxSupply;
    }

    /// @inheritdoc IERC721E
    function editEntryMaxBuy(uint256 entryPos, uint256 maxBuy) external virtual override onlyRole(CREATOR_ROLE) {
        if (entryPos > entriesCount) revert ERC721EWrongEntryPos();
        EntryType storage entry = _entryByPos[entryPos];
        entry.maxBuy = maxBuy;
    }
    ...
    /// @inheritdoc IERC721E
    function editEntrySaleEnd(uint256 entryPos, uint256 saleEnd) external virtual override onlyRole(CREATOR_ROLE) {
        if (entryPos > entriesCount) revert ERC721EWrongEntryPos();
        if (saleEnd < block.timestamp) revert ERC721EWrongSaleEnd();
        EntryType storage entry = _entryByPos[entryPos];
    ...
    function buyTickets(
        ...
        if (amount != sellingPrices.length) revert ERC721EWrongBatchLengths();

        EntryType memory entryType = _entryByPos[entryPos];

        if (entryType.saleEnd < block.timestamp) revert ERC721ESaleEnded();

        uint256 soldTickets = entryType.sold;
        uint256 maxSupply = entryType.maxSupply;
        if (soldTickets + amount > maxSupply) revert ERC721EReachedMaxSupply();

        uint256 maxBuy = entryType.maxBuy;
        if (_userTicketsBought[userId][entryPos] + amount > maxBuy) revert ERC721ETooManyTickets();
        ...
        _saveTickets(_currentIndex, amount, entryPos, sellingPrices);
        _userTicketsBought[userId][entryPos] += amount;

        //Implement royalties
        if (price > 0 && _msgSender() != owner()) {
        ...
        emit MintTickets(receiver, entryPos, amount);
    }

    /// @inheritdoc IERC721E
    function getTicketInfo(uint256 tokenId) external view virtual override returns (EntryType memory) {
        if (!_exists(tokenId)) revert ERC721ENonExistentToken();
        uint256 entryPos = _ticketByEntryPos[tokenId];
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(ERC721AMarketplace, AccessControl)
        returns (bool)
    {
        return
