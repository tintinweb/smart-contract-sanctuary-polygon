...
//-------------------------------ADMIN CALLER FUNCTION -----------------------------------
   ...
   //Incomming transaction
    ...
    function payNbirth(address user,uint recParentIndx) internal returns(uint) {
        ...
        if (is2xBirth){
            ...
            if (lastIDCount!=1){
               ...
               payUserPosition(recMindex, payUser ,toUser, autopool2xCycle[recMindex].cycle);
    ...
    function payNbirth3x(address _user, uint recParentIndx ) internal {
        ...
        // get all data of last parent
        uint recIndex = autoPool3xDataList[recParentIndx].mIndex;         

        address recUser = userAddressByID[autoPool3xDataList[recParentIndx].userID];
        address toUser    = userAddressByID[autoPool2xDataList[recIndex].userID];
        ...
        if (is3xBirth){
            ...
            if (lastIDCount!=1){
                ...
                userGains[toUser].total3xPoolGains+=POOL_3X_PAY;
