...
contract Product020802 {
    ...
    enum TemporalityOfDays{ PAST, SAME, FUTURE }
    ...
    function computeCoverageStartDate(uint256 requestedCoverageStartDate, uint256 subscriptionDate)
        ...
        returns (uint256)
    {
        TemporalityOfDays requestedCoverageStartDateComparedToSubscriptionDate = checkTheTemporalityOfDays(requestedCoverageStartDate, subscriptionDate);
        require(requestedCoverageStartDateComparedToSubscriptionDate != TemporalityOfDays.PAST, "requestedCoverageStartDate cannot be in the past");
        if(requestedCoverageStartDateComparedToSubscriptionDate == TemporalityOfDays.SAME){
          return subscriptionDate;
        }else {
          uint256 requestedCoverageStartDateAtMidnight = setDateAtMidnight(requestedCoverageStartDate);
    ...
    function checkTheTemporalityOfDays(uint256 referenceDate, uint256 dateToBeCompare) private pure returns (TemporalityOfDays){
        uint256 ReferenceDateAtMidnight = setDateAtMidnight(referenceDate);
        uint256 DateToBeCompareAtMidnight = setDateAtMidnight(dateToBeCompare);

        if(ReferenceDateAtMidnight < DateToBeCompareAtMidnight){
          return TemporalityOfDays.PAST;
        } else if (ReferenceDateAtMidnight > DateToBeCompareAtMidnight) {
          return TemporalityOfDays.FUTURE;
        } 
        return TemporalityOfDays.SAME;
    }

    function setDateAtMidnight(uint256 date) private pure returns(uint256){
        uint256 totalSecondsInDay = date % 1 days ; 
        return date - totalSecondsInDay;
    }
