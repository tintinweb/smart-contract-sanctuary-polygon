    ...
    CoreRoles
{
    ...
    function mint(
        address to,
        uint256 amount
    ) external override onlyOperatorOrSale nonReentrant {
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view virtual override(RoyaltyInfo, ERC721) returns (bool) {
        return
            interfaceId == type(IERC721).interfaceId ||
            super.supportsInterface(interfaceId);
    }
...
//       maxSupply を少なくしてある
contract TestSmallToken is ERC721CoreOpenSea {
    constructor()
        ERC721CoreOpenSea("TestSmallToken", "TST", address(0), 0, 11, "", "")
    {}
}
