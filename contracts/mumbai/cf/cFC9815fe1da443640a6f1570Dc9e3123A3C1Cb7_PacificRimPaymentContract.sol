...
contract PacificRimPaymentContract is Ownable, AccessControl, ReentrancyGuard {
    ...
    uint256 private perTransactionMintLimit;
    ...
    event perTransactionMintLimitUpdated(uint256 indexed _perTransactionMintLimit);
    ...
    function publicMint(uint256[] memory _tokenId, bytes32 _hash, bytes memory _signature) public payable{
        ...
        require(recover(_hash,_signature) == signatureAddress,"Dapp: Invalid signature!");
        require(!signatures[_signature],"Dapp: Signature already used!");
        require(perTransactionMintLimit <= _tokenId.length,"Dapp: Token id length greater than per transacton mint limit!");
    ...
    function updatePresaleTime(uint256 _presaleTime) public {
        ...
        require(hasRole(ADMIN_ROLE, _msgSender()),"Landsale: Must have admin role to set eth toggle.");
    ...
    function airdrop(address[] memory to, uint256[] memory tokenId) public {
        require(hasRole(MINTER_ROLE, _msgSender()),"Landsale: Must have price update role to mint.");
    ...
    function updateCappedValue(uint256 _value) public  {
    ...
    function updatePerTransactionMintLimit(uint256 _perTransactionMintLimit) public {
        require(_perTransactionMintLimit>0,"Dapp: Invalid value!");
        require(_perTransactionMintLimit!=perTransactionMintLimit,"Dapp: Limit value is same sa previous!");
        require(hasRole(ADMIN_ROLE, _msgSender()),"Landsale: Must have admin role to set eth toggle.");

        perTransactionMintLimit = _perTransactionMintLimit;

        emit perTransactionMintLimitUpdated(_perTransactionMintLimit);
    }
    ...
    function getPerTransactionMintLimit() public view returns(uint256){
        return perTransactionMintLimit;
    }
