...
contract YieldManagerMock is IYieldManager {
  ...
  function distributeYieldToTreasury() external override {
    ...
    uint256 totalValueRealizedForMarket = MarketTieredLeverageInternalStateSetters(market)
...
contract MarketTieredLeverageInternalStateSetters is MarketTieredLeverage {
  constructor(address _paymentToken, ILongShort _longShort)
    MarketTieredLeverage(_paymentToken, _longShort)
  {}
  ...
  function get_pool_token(PoolType poolType, uint256 pool) external view returns (address) {
  ...
  function syntheticToken_priceSnapshot(
    ...
    return _syntheticToken_priceSnapshot[epoch][poolType][pool];
  }
...
contract MarketTieredLeverage is AccessControlledAndUpgradeable, IMarketTieredLeverage {
  ...
  */
  ...
  mapping(uint256 => mapping(PoolType => uint256[8])) _syntheticToken_priceSnapshot;
  ...
  modifier _longShortOnly() {
    require(msg.sender == address(_longShort), "Not LongShort");
    _;
  }
  ...
  function initializePools(
    IMarketTieredLeverage.PoolInfo[] memory initPools,
    uint256 initialEffectiveLiquidityToSeedEachPool,
    address seederAndAdmin,
    uint32 _marketIndex,
    address _oracleManager,
    address yieldManager
  ) external override initializer _longShortOnly returns (bool initializationSuccess) {
    require(
    ...
    require(
      seederAndAdmin != address(0) && _oracleManager != address(0) && yieldManager != address(0)
    );
    ...
    for (uint256 i = 0; i < initPools.length; i++) {
      ...
      seedAmount = _calculateLiquidityForPool(
  ...
  function _rebalanceMarketPools(
    ...
    for (uint8 poolType = uint8(PoolType.SHORT); poolType <= uint8(PoolType.LONG); poolType++) {
      ...
      for (uint8 poolIndex = 0; poolIndex < maxPoolIndex; poolIndex++) {
        ...
        _syntheticToken_priceSnapshot[epochToExecute.associatedEpochIndex][PoolType(poolType)][
          poolIndex
        ] = price;
      }
    }
  ...
  function _batchConfirmOutstandingPendingActionsPool(
    ...
    if (
      ...
      if (
        ...
        pool.batched_amountPaymentToken_deposit = pool.nextEpoch_batched_amountPaymentToken_deposit;
        pool.nextEpoch_batched_amountPaymentToken_deposit = 0;
        _syntheticToken_priceSnapshot[associatedEpochIndex][PoolType(poolType)][poolIndex] = price;
      }
    }
