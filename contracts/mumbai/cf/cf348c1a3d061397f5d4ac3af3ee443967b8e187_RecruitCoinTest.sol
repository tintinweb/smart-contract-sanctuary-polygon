...
contract RecruitCoinTest is ERC20, Ownable, ReentrancyGuard {
  ...
  function _deposit(
    ...
    /*PUBLIC SALE*/
      ...
      if (getBonusWeek() <= 2 || publicSale3M) {
        _createVestingSchedule(depositor, getCurrentTime(), ONE_MINUTE.mul(15), 1, 1, false, purchasedTokens.mul(1e18));
      } else {
        _createVestingSchedule(depositor, getCurrentTime(), ONE_MINUTE.mul(30), 1, 1, false, purchasedTokens.mul(1e18));
      }
    } else {
    ...
    if (referral != address(0) && isReffered[depositor] != referral && isReffered[depositor] == address(0)) {
      ...
      referralClaims[referral].push(ReferralClaim(depositor, purchasedTokens.mul(1e18), purchasedTokens.mul(1e18).mul(bonus[4]).div(100), 0, false));
    }
  ...
  function claim(bytes32 vestingScheduleId) public nonReentrant onlyIfVestingScheduleNotRevoked(vestingScheduleId) {
    ...
    if(vestingSchedule.firstVestingAmount > 0){
        _mint(beneficiaryPayable, amount + vestingSchedule.firstVestingAmount);
        vestingSchedule.firstVestingAmount = 0;
    }else{
   ...
   * @param merkleProof
   */
  function whiteLister(bytes32[] calldata merkleProof, address _depoositor) public view returns (bool) {
    return MerkleProof.verify(merkleProof, merkleRoot, keccak256(abi.encodePacked(_depoositor)));
  }
  ...
  function computeReleasableAmount(VestingSchedule memory vestingSchedule) public view returns (uint256) {
    ...
    if ((currentTime < vestingSchedule.cliff) || vestingSchedule.revoked == true) {
      ...
      uint256 vestedSeconds = vestedSlicePeriods.mul(secondsPerSlice);
        // uint256 _firstVestingAmount =0;
        // if(vestingSchedule.released == 0 ){
        //     _firstVestingAmount = vestingSchedule.firstVestingAmount;
        // }
      uint256 vestedAmount = vestingSchedule.amountTotal.mul(vestedSeconds).div(vestingSchedule.duration);
      vestedAmount = vestedAmount.sub(vestingSchedule.released);
      return vestedAmount;
    //   return vestedAmount + _firstVestingAmount;
    }
  }
