...
contract ELSCargoXPool is ReentrancyGuard, Ownable, DSMath {
    ...
    enum AddressStatus{ ACTIVE, DELETED }
    ...
    // get address status
    function getAddressesStatus(address _userPublicAddress) external view returns (AddressStatus) {
    ...
    function getUserStakedBalance(address _userPublicAddress) external view returns (uint256) {
        return userStakedBalance[_userPublicAddress];
    }

    // get user reward tally
    function getUserRewardTally(address _userPublicAddress) external view returns (uint256) {
        return userRewardTally[_userPublicAddress];
    }

    // get user rewards
    function getUserPaidRewards(address _userPublicAddress) external view returns (uint256) {
        return rewardsPaid[_userPublicAddress];
    }

    // get user earned rewards
    function getUserEarnedRewards(address _userPublicAddress) public view returns (uint256) {
        return DSMath.sub(DSMath.wmul(userStakedBalance[_userPublicAddress], rewardPerToken), userRewardTally[_userPublicAddress]);
    }

    // owner of the pool can change fee, but not more than 10%
    function changeFee(uint256 _newFee) external onlyOwner {
        require(_newFee <= 10, "Fee cannot be more that 10%.");
    ...
    // return all assets to stakers. If pool will close.
    function returnAllAssetsToStakers(address[] memory _addresses) external onlyOwner nonReentrant {
        uint256 userStakedBalanceAmount;

        for(uint i = 0; i < _addresses.length; i++) {
            if(userStakedBalance[_addresses[i]] > 0) {
                userStakedBalanceAmount = userStakedBalance[_addresses[i]];
                userStakedBalance[_addresses[i]] = 0;

                totalStakedBalance = DSMath.sub(totalStakedBalance, userStakedBalanceAmount);

                SafeERC20.safeTransfer(token, _addresses[i], userStakedBalanceAmount);
            }
        }
        
        emit AllAssetsReturnedToStakers();
    }

    // distribute rewards to stakers
    function distributeRewards() external nonReentrant {
        uint256 totalRewardsIncludingFee = DSMath.sub(DSMath.sub(token.balanceOf(address(this)), totalStakedBalance), totalDistributedRewards);
        emit LogAmount("totalRewardsIncludingFee", totalRewardsIncludingFee);

        require(totalRewardsIncludingFee >= 10, "There is no enough rewards to distribute.");

        uint256 distributionFee = DSMath.wdiv(DSMath.wmul(totalRewardsIncludingFee, fee), 100);
        emit LogAmount("distributionFee", distributionFee);

        // remove fee distributionFee from totalRewards for distribution
        uint256 totalRewards = DSMath.sub(totalRewardsIncludingFee, distributionFee);

        totalDistributedRewards = DSMath.add(totalDistributedRewards, totalRewards);

        // send distributionFee to pool owner
        SafeERC20.safeTransfer(token, owner(), distributionFee);

        // calculate reward per token
        rewardPerToken = DSMath.add(rewardPerToken, DSMath.wdiv(totalRewards, totalStakedBalance));
        emit LogAmount("rewardPerToken", rewardPerToken);
    ...
    function saveStakingAddress(address _userPublicAddress) private {
        ...
        if(addressID > 0) {
            addressesByAddress[_userPublicAddress].status = AddressStatus.ACTIVE;
        } else {
            totalAddresses++;

            StakeAddress memory stakeAddress = StakeAddress(totalAddresses, _userPublicAddress, AddressStatus.ACTIVE);
    ...
    function stake(uint256 _amount) external nonReentrant {
        require(_amount >= 10, "Minimum stake is 10 CXO.");
        require(DSMath.add(_amount, totalStakedBalance) > 250000, "Maximum staking amount for this pool is 250000 CXO.");

        uint256 userWalletBalance = token.balanceOf(msg.sender);
        require(_amount <= userWalletBalance, "You dont have enough CXO in your wallet?");
        ...
        userStakedBalance[msg.sender] = DSMath.add(userStakedBalance[msg.sender], _amount);
        userRewardTally[msg.sender] = DSMath.add(userRewardTally[msg.sender], DSMath.wmul(rewardPerToken, _amount));
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        require(_amount <= totalStakedBalance, "Total supply of staked CXO is not sufficient?");
        require(_amount <= userStakedBalance[msg.sender], "You don't have staked amount you want to withdraw!");
        ...
        userStakedBalance[msg.sender] = DSMath.sub(userStakedBalance[msg.sender], _amount);
        userRewardTally[msg.sender] = 0;
    ...
    function withdrawRewards() public nonReentrant {
        ...
        require(reward <= DSMath.sub(token.balanceOf(address(this)), totalStakedBalance), "Withdraw amount is bigger than it should be?");

        userRewardTally[msg.sender] = DSMath.wmul(userStakedBalance[msg.sender], rewardPerToken);
    ...
    event FeeRewardsPaid(address indexed ownerAddress, uint256 amount);
    event AllAssetsReturnedToStakers();
    event LogAmount(string title, uint256 amount);
}
