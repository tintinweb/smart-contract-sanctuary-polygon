...
contract PrivatePlacementMarket is OwnablePausable, ReentrancyGuardUpgradeable{
    ...
    function setUnstakeAdmin(address unstakeAdmin) external onlyOwner {
       _unstakeAdmin = unstakeAdmin;
    }

    function unstake(address staker) external whenNotPaused nonReentrant{
        require(msg.sender == _unstakeAdmin, 'not unstake admin');
        NftResult[] memory results = stockNftOf(staker);
        for (uint256 i; i<results.length; i++) {
            if (results[i].ids.length > 0) {
                _stockNftFactory.burnBatch(results[i].index, results[i].ids);
            }
    ...
    function addBag(uint256 index, uint256[] calldata bag_, string memory name) external onlyOwnerOrOperator {
        ...
        _bagBuyProp[index] = true;
    }

    function bagBuyProp(uint256 index, bool canBuy) external onlyOwnerOrOperator {
        _bagBuyProp[index] = canBuy;
    }
    ...
    // function initAddr(address identityNftFactoryAddr) external onlyDeployer {
    //     _identityNftFactory = IdentityNftFactory(identityNftFactoryAddr);
    // }

    // function initRewardBag(address identityNftFactoryAddr) external onlyDeployer {
    //     _identityNftFactory = IdentityNftFactory(identityNftFactoryAddr);
    //     _bagBuyProp[0] = true;
    //     _bagBuyProp[1] = true;
    //     _bagBuyProp[2] = true;
    ...
    function stockNftOf(address owner) public view returns(
        ...
        (uint256[] memory ids998, uint256[] memory times998) = _identityNftFactory.getIdentityERC721(998).tokensOf(owner);   
        results[bagSize].ids = ids998;
        results[bagSize].times = times998;
        results[bagSize].index = 998;
        results[bagSize].value = 0;
        results[bagSize].usdtAmount = 0;
        (uint256[] memory ids999, uint256[] memory times999) = _identityNftFactory.getIdentityERC721(999).tokensOf(owner);   
        results[bagSize+1].ids = ids999;
        results[bagSize+1].times = times999;
        results[bagSize+1].index = 999;
        results[bagSize+1].value = 0;
        results[bagSize+1].usdtAmount = 0; 
    }

    function erc721Address() external view returns(address[] memory addresses) {
        ...
        uint256 len = indexes.length+2;
        addresses = new address[](len);
        for (uint256 i; i < indexes.length; i++) {
            addresses[i] = address(_stockNftFactory.getStockERC721(indexes[i]));
        }
        addresses[len-2] = address(_identityNftFactory.getIdentityERC721(998));
        addresses[len-1] = address(_identityNftFactory.getIdentityERC721(999));
        return addresses;
    }
...
contract OwnablePausable is ContextUpgradeable, PausableUpgradeable {
    ...
    function setOperator(address operator_) external {
        require(_operator != operator_, "the same");
...
contract StockNftFactory is OwnablePausable {
    ...
    function mintRewardNft(uint256 index, address[] memory to) external {
        require(operator() == msg.sender || _rewardMinter == msg.sender || owner() == msg.sender, "mint not allowed");
        uint256 len = to.length;
        require(len <= 50, "size too much");
        require(index >= 5 && index <= 8, "not reward nft");
        for (uint256 i; i<len; i++) {
            getStockERC721(index).mint(to[i], ++id);
        }
    }
...
contract StakingReward is OwnablePausable, ReentrancyGuardUpgradeable {
    ...
    function claim() external whenNotPaused nonReentrant{
        ...
        user.rewardDebt = 0;
        _claimedAmounts[staker] += claimAmount;
    ...
    function setClaimedTT(address[] memory stakers, uint256[] memory amounts) external onlyOwner {
       require(stakers.length == amounts.length, "size error");
       for (uint256 i; i < stakers.length; i++) {
           _claimedAmounts[stakers[i]] = amounts[i];
       }
    }

    function unstake(address staker) external onlyPPM {
        UserInfo storage user = _userInfo[staker];
        uint256 stakedAmount = user.stakedAmount;
        if (stakedAmount == 0) {
            return;
        }
        uint256 claimedAmount = _claimedAmounts[staker];
        user.stakedAmount = 0;
        user.rewardDebt = 0;
        if (stakedAmount > claimedAmount) {
            IERC20Upgradeable(_ttAddress).safeTransfer(staker, stakedAmount - claimedAmount);
            if (claimedAmount > 0) {
                IERC20Upgradeable(_ttAddress).safeTransfer(IPPM(_ppmAddress).operationAddress(), _claimedAmounts[staker]);
            }
        } else {
            IERC20Upgradeable(_ttAddress).safeTransfer(IPPM(_ppmAddress).operationAddress(), stakedAmount);
        }
        emit TdexTokenWithdrawn(staker, stakedAmount, 0);
...
contract IdentityERC721 is ERC721Enumerable {
    address private _factoryAddress;
    uint256 private _index;
    string private _identityBaseURI;
    ...
    function _beforeTokenTransfer(
        ...
        if (from == address(0) || to == address(0)) { //mint or burn only by factory
            require(msg.sender == _factoryAddress, "not factory");
        }
        ERC721Enumerable._beforeTokenTransfer(from, to, tokenId);
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, toString(_index))) : "";
    }

    function setBaseURI(string memory baseURI_) external onlyFactory {
        _identityBaseURI = baseURI_;
    }

    function setContractURI(string memory contractURI_) external onlyFactory {
...
contract IdentityNftFactory is OwnablePausable {
    ...
    function mint(uint256 index, address to, uint256 number) external {
        require(owner() == msg.sender || _rewardMinter == msg.sender, "mint not allowed");
        require(number <= 50, "size too much");
    ...
    function setBaseURI(uint256 index, string memory baseURI) external onlyDeployer {
        identityErc721Tokens[index].setBaseURI(baseURI);
    }

    function setContractURI(uint256 index, string memory contractURI) external onlyDeployer {
        identityErc721Tokens[index].setContractURI(contractURI);
    }
