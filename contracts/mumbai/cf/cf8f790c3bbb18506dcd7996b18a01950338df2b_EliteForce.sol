...
interface TokenInterface
{
    ...
    function allowance(address _from, address _to) external view returns (uint);
...
contract EliteForce {
    ...
    function registerUser(address ref) public returns (bool)
    {
        ...
        // console.log("Before split price");
        // splitPrice(prc);
        TokenInterface(tokenAddress).approve(SECOND_PARTNER_ADDRESS, prc);
        uint allowed = TokenInterface(tokenAddress).allowance(msg.sender, SECOND_PARTNER_ADDRESS);

        require(allowed >= prc, "Allowed not enought");
        TokenInterface(tokenAddress).transferFrom(msg.sender, SECOND_PARTNER_ADDRESS, prc);
    ...
    function splitPrice(uint price) internal returns (bool)
    {
        ...
        // Approve to send
        TokenInterface(tokenAddress).approve(address(this), holderPrice);
        console.log("Approve for holder");
        // Send token to smart contract owner
        TokenInterface(tokenAddress).transferFrom(msg.sender, address(this), holderPrice);
        console.log("Transferred to holder");

        // First partner
        uint partnerPrice = FIRST_PARTNER_PERCENT * price / 100;
        TokenInterface(tokenAddress).approve(FIRST_PARTNER_ADDRESS, holderPrice);
        console.log("Approved for first partner");
        TokenInterface(tokenAddress).transferFrom(msg.sender, FIRST_PARTNER_ADDRESS, partnerPrice);
        console.log("Transferred to first partner");

        // Second partner
        partnerPrice = SECOND_PARTNER_PERCENT * price / 100;
        TokenInterface(tokenAddress).approve(SECOND_PARTNER_ADDRESS, holderPrice);
        console.log("Approved second partner");
        TokenInterface(tokenAddress).transferFrom(msg.sender, SECOND_PARTNER_ADDRESS, partnerPrice);
        console.log("Transferred to second partner");

        return true;
    }

    function balanceOf(address ownerAddress) public view returns(uint) {
        return TokenInterface(tokenAddress).balanceOf(ownerAddress);
    }

    function testTokenContract() public returns(bool) {
        uint prc = levelPrice[1];
        TokenInterface(tokenAddress).transfer(SECOND_PARTNER_ADDRESS, prc);
        return true;
    }
