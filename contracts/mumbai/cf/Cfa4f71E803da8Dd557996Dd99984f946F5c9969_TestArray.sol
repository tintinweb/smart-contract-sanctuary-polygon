...
contract TestArray {
    ...
    function encodeArray(address[] memory addrArray) public view returns (bytes memory) {
        return abi.encode(addrArray);
    }

    function decodeArray(bytes memory encodedArrayData) public view returns (address[] memory) {
