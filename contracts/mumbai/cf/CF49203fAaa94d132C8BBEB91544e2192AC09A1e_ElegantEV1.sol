...
contract ElegantEV1 is Ownable, AccessControl {
    ...
    function signUp (uint[] calldata _racers, uint _event) public payable{
        ...
        uint moolah = rulesByRace[_event].devMat;
        uint nards = rulesByRace[_event].refNut;
        uint coffee = rulesByRace[_event].cofMat;
        uint bowel = rulesByRace[_event].cofNut;
    ...
    function returnToken(uint256 _tokenID) public {
        ...
        racersByEvent[_ev][i] = racersByEvent[_ev][(racersByEvent[_ev].length - 1)];
        racersByEvent[_ev].pop();
