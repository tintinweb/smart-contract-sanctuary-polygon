...
interface ISWAP {
    ...
    function swapOut(address toToken, uint256 usdeWei) external returns (bool);
}
...
interface IOFCHAIN is ISWAP {
    ...
    function withdraw(uint256 usdeWei) external payable returns (bool);
...
interface ILOCK {
    ...
    function unLockFrom(address from, uint256 usdeWei) external returns (bool);
...
interface IOWNER is ILOCK {
    ...
    function mint(address to, uint256 usdeWei) external returns (bool);

    function burnFrom(address from, uint256 usdeWei) external returns (bool);
...
contract USDE is IERC20, IUSDE, IOWNER {
    ...
    function addToken(address token) external virtual returns (bool) {
        ...
        if (notThis_ && _isWZero(Token()) && _isAdmin()) {
            BTC = IBTC(token);
            emit And(token);
            return true;
        }
    ...
    function stake(
        ...
        if (_hasWei(staker, usdeWei)) {
            _transfer(staker, _contract, usdeWei);
        }
        _stake(pool, staker, usdeWei, stake_Month);
        return true;
    }
    ...
    function donate(
        ...
        if (isMonth && _hasWei(_sender(), usdeWei)) {
            ...
            _transfer(to, _contract, usdeWei);
    ...
    function deposit() public payable virtual override returns (bool) {
        ...
        bool isValue = msg.value > 0 && usdeWei > 0;
        // bool canBuy = _maxCap >= _totalSupply() + usdeWei && isValue;
    ...
    function withdraw(uint256 usdeWei)
        public
        payable
        virtual
        override
        returns (bool)
    {
        ...
        if (canSwap_ && _hasWei(_sender(), usdeWei)) {
    ...
    function swapOut(address toToken, uint256 usdeWei)
        public
        virtual
        override
        returns (bool)
    {
        ...
        return _swapOut(toToken, usdeWei);
    }
    ...
    function _incStake(address staker_, uint256 usdeWei) internal {
        ...
        emit Staking(staker_, usdeWei, _stakeAt(staker_));
    }
    ...
    function _mint(address to, uint256 usdeWei_) internal returns (bool) {
        uint256 usdeWei = _ParseWei(usdeWei_);

        if (!_isWallet(to) || usdeWei < 1) {
            return false;
        }
        _incTotalExchange(usdeWei);
    ...
    function _swapOut(address toToken, uint256 usdeWei)
        internal
        returns (bool)
    {
    ...
    function _fee_swap(uint256 usdeWei) internal view returns (uint256) {
        uint256 mulOf_ = usdeWei * _oracle(_FEE_SWAP, "USD");
        return _ParseWei(mulOf_ / _10Pow(_decimal));
    }

    function _ParseWei(uint256 usdeWei_) internal pure returns (uint256) {
        uint256 usdeWei8 = usdeWei_ / _10Pow(12);
        return usdeWei8 * _10Pow(12);
    }
