...
contract HEFTYArtBadgeNFT is ERC721, EIP712MetaTransaction("HEFTYArt Badge Minter", "1"), Ownable {
    ...
    constructor() public ERC721("HEFTYArt Badge", "HEFTYArt") {}

    function mintToCaller(address caller) 
        public
        returns (uint256)
    {
        require(isWhitelisted[caller], "HEFTYArt: caller not authorised"); // Makes sure that only the discord members can mint the badge
        require(isMinted[caller] == false, "HEFTYArt: NFT already minted");
        _tokenIds.increment();
    ...
    function setWhitelist(bytes32 userId, address user) public onlyOwner {
        require(!isUserAdded[userId], "HEFTYArt: user already added");
