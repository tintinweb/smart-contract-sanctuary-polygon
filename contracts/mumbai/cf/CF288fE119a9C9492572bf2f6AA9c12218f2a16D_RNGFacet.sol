...
contract RNGFacet {
	function setChainlinkVRFKeyhash(bytes32 _keyhash) external {
		...
		LibRNG.rngStorage().chainlinkVRFKeyhash = _keyhash;
    }

    function getChainlinkVRFKeyhash() external view returns (bytes32) {
		return LibRNG.rngStorage().chainlinkVRFKeyhash;
    }

    function setChainlinkVRFFee(uint256 _fee) external {
		LibShadowcorn.enforceIsContractOwner();
		LibRNG.rngStorage().chainlinkVRFFee = _fee;
    }

    function getChainlinkVRFFee() external view returns (uint256) {
        return LibRNG.rngStorage().chainlinkVRFFee;
    }

	function setChainlinkVRFCoordinator(address vrfCoordinator) external {
		LibShadowcorn.enforceIsContractOwner();
		LibRNG.rngStorage().vrfCoordinator = vrfCoordinator;
    }

	function getChainlinkVRFCoordinator() external view returns(address) {
		return LibRNG.rngStorage().vrfCoordinator;
    }
	...
	function rawFulfillRandomness(bytes32 vrfRequestId, uint256 randomness) external {
        ...
        require(msg.sender == rs.vrfCoordinator, "RNGFacet: Only VRFCoordinator can fulfill");
		rs.randomness[vrfRequestId] = randomness;
		uint256 mechanicId = rs.mechanicIdByVRFRequestId[vrfRequestId];
		if(mechanicId == LibRNG.RNG_HATCHING) {
			LibHatching.hatchingFulfillRandomness(vrfRequestId);
		}

		delete rs.mechanicIdByVRFRequestId[vrfRequestId];
    }

	function setVRFBlocksToRespond(uint256 vrfBlocksToRespond) external {
		LibShadowcorn.enforceIsContractOwner();
		LibRNG.rngStorage().vrfBlocksToRespond = vrfBlocksToRespond;
	}

	function getVRFBlocksToRespond() public view returns(uint256) {
		return LibRNG.rngStorage().vrfBlocksToRespond;
	}
}
