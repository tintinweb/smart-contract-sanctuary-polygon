...
import "../interface/IGiveaway.sol";
import "../interface/IGiveawayFactory.sol";

contract GiveawayFactory is
  Initializable,
  UUPSUpgradeable,
  OwnableUpgradeable,
  PausableUpgradeable,
  ...
  IGiveawayFactory
{
  ...
  function init(
    address _giveawayImplementation,
    address _stakingV3
    ...
    stakingV3 = _stakingV3;
    beacon = address(new UpgradeableBeacon(_giveawayImplementation));

    UpgradeableBeacon(beacon).transferOwnership(_msgSender());
  }

  function _authorizeUpgrade(address newImplementation) internal virtual override proxied {}

  function allGiveawaysLength() external view returns (uint256) {
    return allGiveaways.length;
  }
  ...
  function createGiveaway(
  ...
  ) external onlyOwner returns (address giveaway) {
    bytes memory data = abi.encodeWithSelector(
      IGiveaway.init.selector,
      _token,
      _cutoffAt,
      _claimAt,
      _amountToShare
    ...
    giveaway = address(new BeaconProxy(UpgradeableBeacon(beacon).implementation(), data));
    allGiveaways.push(giveaway);

    emit GiveawayCreated(
      _token,
      allGiveaways.length - 1,
      block.timestamp,
      giveaway
    );
  }
  ...
  function owner() public view override(IGiveawayFactory, OwnableUpgradeable) returns (address){
    return super.owner();
  }
