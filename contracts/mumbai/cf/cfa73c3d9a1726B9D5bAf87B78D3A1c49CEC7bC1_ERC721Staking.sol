...
contract ERC721Staking is ReentrancyGuard {
    ...
    // Allow only the deployer to set the rewards per hour
    modifier isDeployer() {
        require(msg.sender == address(this), "Only the deployer can call this function");
        _;
    }
    // functino set rewardsPerhour
    function SetRewardsPerHour(uint256 _rewardsPerHour) public isDeployer {
        rewardsPerHour = _rewardsPerHour; 
    }

    // function to view the currenct rewardsPerhours
    function viewRewardsPerHour() public view returns (uint256) {
        return rewardsPerHour;
    }


    // Rewards per hour per token deposited in wei.
    //uint256 private rewardsPerHour = _rewardsPerHour;
