...
/*    function RewardCalcTimeChange(uint256 time)public onlyAdmin
    {
	RewardCalcTime = time;
    }
*/
	...
	IToken(lp2).transferFrom(_msgSender(),address(this),amount);

	nn = StakeNum[addr]*1 + 1;
	StakeNum[addr] = nn;
	StakeUser[addr][nn].addr 	= lp;
...
/*
    function StakeAmountFill(uint8 grp,uint256 amount)private
    {
	uint256 i;
	uint256 last = Utime(0);
	uint256 start;
	if(StakeAmountLastTime[grp] > 0)
	{
	start = StakeAmountLastTime[grp];
	}
	else
	start = RewardStartTime;

	uint256 a = StakeAmount[grp][last];
	for(i = start;i <= last;i+=RewardCalcTime)
	{
	    if(i <= StakeAmount[grp][last])continue;
	    StakeAmount[grp][i] += a;
	}
	StakeAmount[grp][last] += amount;
	StakeAmountLastTime[grp] = last;
    }
*/
...
//	require(BalanceLP[StakeUser[addr][nn].grp]>=amount,'');
...
//	IToken(StakeUser[addr][nn].addr).transferFrom(address(this),_msgSender(),amount);
	IToken(StakeUser[addr][nn].addr).transfer(_msgSender(),amount);
    }
    //mapping(uint8 => uint256)public BalanceLP;
...
//    function RewardAdd(address tkn,uint256 amount,uint256 interval,uint256 koef1,uint256 koef2,uint256 koef3)public 
    function RewardAdd(address tkn,uint256 amount,uint256 interval)public 
    {
	require(IToken(tkn).allowance(_msgSender(),address(this)) >= amount,"Check allowance from you LP to this contract.");
	RewardNum++;
...
//	RewardData[RewardNum].koef1 	= koef1;
//	RewardData[RewardNum].koef2 	= koef2;
//	RewardData[RewardNum].koef3 	= koef3;
//	RewardData[RewardNum].koef 	= koef1 + koef2 + koef3;
    }
...
//        RewardData[num].koef  = koef1 + koef2 + koef3;
    }
	...
	uint256 amount = IToken(RewardData[num].tkn).balanceOf(address(this));
	IToken(RewardData[num].tkn).transfer(addr,amount);
    }


    function RewardSummaryOnTime(uint8 grp,uint256 num,uint256 time)public view returns(uint256)
    {
	if(RewardData[num].hidden == true)return 0;

	if(time == 0)time = Utime(block.timestamp);
	uint256 i;
	uint256 t  = Utime(RewardData[num].start_time);
	uint256 t2 = RewardData[num].start_time + RewardData[num].interval * RewardStartTime;
	...
	for(i = t;i <= t2;i += RewardStartTime)
	{
	    if(i == time)
	    {
...
/*
    function RewardList(address addr,uint256 time)public view returns(uint256 count, address[] memory tkns, uint256 memory amounts[])
    {
	...
	{
	    if(RewardData[i].hidden == true)continue;
	    if(RewardData[i].stoped == true)
	    {
	    last = Utime(RewardData[i].stoped_time);
		if(last < time)time = last;
	    }
	    for(j = Utime(RewardData[i].start_time);j <= time;j += 86400)
	    {
	    }
	}
    }
*/
    function Utime(uint256 time)public pure returns(uint256 out)
    {
//	out = time / 86400 * 86400;
	...
	uint256 stake = 0;
	uint256 reward = 0;
	uint8 grp = StakeUser[addr][nn].grp;
	for(i = t;i <= time;i += RewardCalcTime)
	{
	    if(StakeAmount[grp][i] > 0)
	    stake_all = StakeAmount[grp][i];

	    stake = StakeUser[addr][nn].amount * 10*18 / stake_all;
//	    StakeUser[addr][nn].amount
//function RewardSummaryOnTime(uint8 grp,uint256 num,uint256 time)public view returns(uint256)
	    t2 = RewardSummaryOnTime(grp,num,time);
	    t2 = t2 * stake / 10**18;
	    reward += t2;
	}
	return reward;
    }
    function ClaimReward(uint256 num,uint256 nn)public
    {
	address addr = _msgSender();
	...
	uint256 amount = RewardNumByAddr(num,addr,nn,time);
