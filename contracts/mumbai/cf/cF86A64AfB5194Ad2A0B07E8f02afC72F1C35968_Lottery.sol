...
// -------------------------EVENTS -------------------------------
   ...
   constructor() ERC721("NFT Lotto", "flr") {
      promo = "Crypto Lotto is in Beta. Winning numbers are generated through Chainlink VRF.";
     }
...
// SPDX-License-Identifier: MIT
  ...
  / @dev uses chainlink VRF2 to get random number
   ...
   Contract needs to be seeded with the initial prizepool amount of .1 eth
 */
pragma solidity ^0.8.0;
...
contract Lottery is VRFConsumerBaseV2, ConfirmedOwner, ReentrancyGuard {
   ...
   // address VRF_POLYGON_ADDRESS =  0xAE975071Be8F8eE67addBC1A82488F1C24858067;
    ...
    constructor(uint64 subscriptionId, address _floorContract) VRFConsumerBaseV2(VRF_MUMBAI_ADDRESS) ConfirmedOwner(msg.sender)
    {
        floor101Contract = Floor101(_floorContract);
        COORDINATOR = VRFCoordinatorV2Interface(VRF_MUMBAI_ADDRESS);
...
// the percentage of funds kept aside for next draw
 ...
 function checkWinners(uint256 _drawNumber, string memory _numbers) internal {
    ...
    if (numbersToAddress[_drawNumber][_numbers].length > 0) {
        // Calculate the amount to be sent to each winner (80% of the contract balance divided equally)
        ...
        // No winner of the lottery
        s_lotteryNumber++;  // Increment the lottery draw number
        // Keep 20% of fees aside to accommodate the next draw when there are winners of the lottery
        addLotteryInfo(s_lotteryNumber,  "", false);
    }
