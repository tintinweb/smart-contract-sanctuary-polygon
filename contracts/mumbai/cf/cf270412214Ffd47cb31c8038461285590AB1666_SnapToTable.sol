...
contract SnapToTable is Initializable, ERC721Upgradeable, OwnableUpgradeable, UUPSUpgradeable, ERC721HolderUpgradeable {
    ...
    function initialize() initializer public {
        __ERC721_init("SnapToTable", "SNP");
        __Ownable_init();
        __UUPSUpgradeable_init();
        _tableland = TablelandDeployments.get();
    }

    function createTable() private returns (uint256) {
        string memory create = string.concat(
            "CREATE TABLE snapshot_",
        ...
        uint256 _snapshot = _tableland.createTable(
            address(this),
            create
        );
        emit sqlUpdate(create,"create",_msgSender());
        return _snapshot;
    }

    function getName(uint256 tableId) private view returns(string memory) {
    ...
    function insertRow(uint256 _snapshot, string memory _snapName, string memory insert) private {        
        _tableland.runSQL(
            address(this),
            _snapshot,
            string.concat(
                "INSERT INTO ",
                _snapName,
                "(tokenId,owner,block) VALUES ",
                insert,
                ";"
            )
        );
        emit sqlUpdate("0","insert",_msgSender());
    }

    function getRow(IERC721EnumerableUpgradeable nft, uint256 index) private view returns (string memory){
        // uint256 token = nft.tokenByIndex(index);
        address owner = nft.ownerOf(index);
        return string.concat(
            "(",
            StringsUpgradeable.toString(index),
            ",'",
    ...
    // collection is the address of an ERC721 NFT.
    function safeMint(address nftContract, uint256 offset) public onlyOwner {
        IERC721EnumerableUpgradeable nft = ERC721EnumerableUpgradeable(nftContract);
        uint256 supply = nft.totalSupply();
        // ensure at least 1 row
        string memory insert = getRow(nft, 0 + offset);
        string memory splitter = ",";
        // only then create the table
        uint256 _snapshot = createTable();
        string memory _snapName = getName(_snapshot);

        for (uint256 i = 1; i < supply; i++) {
            string memory next = getRow(nft, i + offset);
            emit sqlUpdate(next,"row",_msgSender());
            insert = string.concat(insert, splitter, next);
            splitter = ",";
            if (i % 50 == 0) {
                insertRow(_snapshot, _snapName, insert);
                insert = "";
                splitter = "";
            }
        }
        // insert any left over rows
        if (supply % 50 != 0) {
            insertRow(_snapshot, _snapName, insert);
        }
