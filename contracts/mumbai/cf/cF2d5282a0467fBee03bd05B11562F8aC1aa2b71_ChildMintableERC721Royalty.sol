 ...
 * _Available since v4.5._
 */
contract ChildMintableERC721Royalty is
IERC2981,
ERC721,
IChildToken,
AccessControlMixin,
...
ContextMixin
{
    ...
    constructor(
        ...
        uint96 defaultRoyalty_ // Add default choice to user at constructor
    ) ERC721(name_, symbol_) {
        _setupContractId(contractId_);
        _setupRole(DEPOSITOR_ROLE, childChainManagerProxy_);
        _setupRole(MINT_ROLE, minter_);
        _setupRole(OWNER, minter_);
        _initializeEIP712(name_);
        _setDefaultRoyalty(minter_, defaultRoyalty_); // Set the default Royalty at Zero for NFT on the gallery
    }

    // This is to support Native meta transactions
    // never use msg.sender directly, use _msgSender() instead
    function _msgSender() internal override view virtual returns (address payable sender) {
    ...
    function resetTokenRoyalty(uint256 tokenId_) public only(OWNER) {
    ...
    */
    ...
    ) public only(MINT_ROLE) returns (uint256) {
        _tokenIds.increment();
        uint256 newItemId = _tokenIds.current();
        _mint(recipient, newItemId);
        _setTokenURI(newItemId, uri);
        _setDepositTokenURI(newItemId, uri);
        _setTokenRoyalty(newItemId, royaltyReceiver_, feeNumerator_);
