...
contract PresaleSRX is Ownable {
    ...
    //Presale token price 0.5 usd in usdc = 5 * 10 ** 17
    uint256 public srxPrice = 5 * 10 ** 17;

    //Presale hardcap 1.8M SRX
    uint256 public hardCap = 1800000 * 10 ** 18;

    //Presale softcap 300k SRX
    uint256 public softCap = 300000 * 10 ** 18;
    ...
    event PresaleRefunded(address user, uint256 amount);
    ...
    constructor() {
        ...
        btcPriceFeed = AggregatorV3Interface(
            0x26c5287a4cF44548B2B1C22fAFD1b0c0d271df87
        );
        bnbPriceFeed = AggregatorV3Interface(
            0x8dBa379e5FA089BDA80009972381887984a75E19
        );
        usdcPriceFeed = AggregatorV3Interface(
            0x7D0570486dE61a9Df950DF86bf38178aAf4AA0DC
        );
        busdPriceFeed = AggregatorV3Interface(
            0xF63a83bd4980C8E4473B7849277E9245D0EfB2a2
        );
    ...
    function buySRXWithToken(
        ...
        uint256 tokenPrice = getTokenPrice(_tokenAddress);
        uint256 tokenAmount = ((_amount * 10 ** 18) * tokenPrice) / srxPrice;
        if (_tokenAddress == btcAddress) {
    ...
    function burnAllSRX() external onlyOwner {
        ...
        require(
            IERC20(srxAddress).transfer(
                address(0),
                IERC20(srxAddress).balanceOf(address(this))
            ),
            "Presale: Transfer failed"
        );
