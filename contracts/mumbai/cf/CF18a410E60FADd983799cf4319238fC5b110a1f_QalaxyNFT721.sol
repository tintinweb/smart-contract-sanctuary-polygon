    ...
    AccessControl
{
    ...
    function adminMint(uint256 qty) external onlyRole("ADMIN_ROLE") {
        uint256[] memory randomness = new uint256[](qty);
        randomness = getNumber(block.number, qty);
        for(uint256 i = 0; i < randomness.length; i++) {
            _safeMint(creator, randomness[i]);
        }
    }
    ...
    function mint(
        ...
        uint256[] memory randomness = new uint256[](qty);
        require(qty <= 5, "Mint invalid quantity");
        require(paymentToken.balanceOf(_msgSender()) >= mintingFee * qty,"Mint: insufficient balance");
        require(paymentToken.transferFrom(_msgSender(), creator, mintingFee * qty),"Mint: failed on transfer");
        randomness = getNumber(block.number, qty);
        for(uint256 i = 0; i < randomness.length; i++) {
            _safeMint(_msgSender(), randomness[i]);
        }
        return true;
    }

    function setTokenRoyalty(address _receiver, uint96 fee) external onlyRole('ADMIN_ROLE') {
        _setRoyalty(_receiver, fee);
    }
    ...
    function tokenURI(uint256 tokenId)
        ...
        returns (string memory)
    {
        require(_exists(tokenId),"tokenURI: non exitent token");
