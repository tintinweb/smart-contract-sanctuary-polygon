...
contract DAFactory is ERC721, Ownable, Pausable {
    ...
    /// @notice Emitted during the punishment of a token.
    /// @param punishedTokenOwner Owner of the punished token
    /// @param tokenId ID of the token punished
    event HasBeenPunished(address indexed punishedTokenOwner, uint256 tokenId);
    ...
    /// @param questionId ID of the question of the vote (indexed for search)
    ...
    constructor(
        ...
        require(
            ...
            "PaymentContractAddressCantBeNull"
        );
    ...
    function _validTokenOnly() private view {
        require(isNftOwnerOfValidToken(msg.sender), DAFLib.NEED_VALID_TOKEN);
    }

    modifier validTokenOnly() {
        _validTokenOnly();
        _;
    }
    ...
    modifier requireAccountToHaveToken(address targetAddress) {
        if (targetAddress != address(0)) {
            require(
                balanceOf(targetAddress) > 0,
                DAFLib.ACCOUNT_MUST_HAVE_TOKEN
            );
        }
        _;
    }
    ...
    function isNftOwnerOfValidToken(address wallet) public view returns (bool) {
        require(wallet != address(0), DAFLib.ZERO_ADDRESS_IS_FORBIDDEN);
    ...
    {
        require(
            dafStorage.questionsById[id].dateOfCreation != 0,
            DAFLib.QUESTION_DOES_NOT_EXIST
        );
    ...
    function setStartScore(uint256 _newStartScore) external onlyOwner {
        require(
            _newStartScore > 0 && _newStartScore <= 100,
            DAFLib.INVALID_START_SCORE_VALUE
        );
        dafStorage.startScore = _newStartScore;
    }

    function setScoreToUpgrade(uint256 _newScoreToUpgrade) external onlyOwner {
        require(_newScoreToUpgrade > 0, DAFLib.INVALID_UPGRADE_SCORE);
    ...
    function setBaseAnswers(string[] memory _baseAnswers) external onlyOwner {
        require(_baseAnswers.length > 1, "Must have at least 2 answers");
        dafStorage.baseAnswers = _baseAnswers;
    }
    ...
    /// @notice Mint a contributor token for the invoker. We have a total of `tokenMaxSupply` tokens.
    ...
    function mint() public requireAccountNotToHaveToken(msg.sender) {
        require(getNumberOfNFTsLeft() > 0, DAFLib.TOKEN_MAX_SUPPLY_REACHED);
        require(
            address(dafStorage.dafSubscriptionsContract) != address(0),
            "DAFSubscriptionContractNotSet"
        );
        ...
        if (dafStorage.nextId >= dafStorage.dafContributorTokens.length) {
            dafStorage.dafContributorTokens.push(newToken);
        } else {
            dafStorage.dafContributorTokens[dafStorage.nextId] = newToken;
        }
        dafStorage.ownerToToken[msg.sender] = dafStorage.nextId;
        _mint(msg.sender, dafStorage.nextId);
        emit DAFContributorTokenEmitted(msg.sender, dafStorage.nextId);
    ...
    function createQuestion(
        ...
        require(
            isQuestionPublisher(msg.sender),
            DAFLib.RESTRICTED_TO_QUESTION_PUBLISHERS
        );
    ...
    function withdrawErroneousTokenSent(address _tokenContract) external {
        require(
            _tokenContract != address(paymentTokenContract),
            "CannotWithdrawTokenBalance"
        );
    ...
    {
        require(msg.sender != address(0), DAFLib.ZERO_ADDRESS_IS_FORBIDDEN);
    ...
    function punishInactiveToken(
        uint256 punishedTokenId,
        string calldata questionId
    ) external whenNotPaused validTokenOnly {
        ...
        emit TokenPunished(
            ...
            dafStorage.ownerToToken[msg.sender],
            dafStorage.rewardPointDeltaForPunishment,
            dafStorage.dafContributorTokens[punishedTokenId].owner,
            punishedTokenId,
            dafStorage.punishmentPointDelta
        );
    }
    ...
    function upgrade() external whenNotPaused validTokenOnly {
        uint256 tokenId = dafStorage.ownerToToken[msg.sender];
        DAFLib.upgrade(tokenId, dafStorage, true);
        emit HasBecomeCoreMember(msg.sender, tokenId);
    }

    /// @notice Allows a contributor to withdraw all or part of their winnings.
    /// @param points Expresses the proportion of gain in number of points to withdraw. The token must obviously have this number of points, otherwise the transaction will fail.
    function withdraw(uint256 points)
        external
        whenNotPausedOrMigrationStarted
        validTokenOnly
    {
        require(points > 0, DAFLib.CANT_WITHDRAW_NEGATIVE_OR_ZERO);
        uint256 tokenId = dafStorage.ownerToToken[msg.sender];
        uint256 amountToWithdraw = DAFLib.withdraw(
            dafStorage.dafContributorTokens[tokenId],
            points,
            dafStorage.startScore,
            dafStorage.totalOfPoints,
    ...
    /// @notice Return the owner of a contributor token.
    /// @param tokenId Contributor token ID.
    /// @return Address of the token owner.
    function ownerOf(uint256 tokenId) public view override returns (address) {
        return dafStorage.dafContributorTokens[tokenId].owner;
    }
    ...
    modifier onlyMigrationTargetContract() {
        //slither-disable-next-line incorrect-equality https://github.com/crytic/slither/wiki/Detector-Documentation#dangerous-strict-equalities
        require(
            msg.sender == dafStorage.migrationVersionAddress,
            DAFLib.MIGRATION_INVOKER_IS_NOT_NEW_VERSION
        );
        _;
    }
    ...
    modifier onlyIfMigrationStateIsValid() {
        ...
        require(
            ...
            DAFLib.MIGRATION_NOT_STARTED
        );
        require(
            dafStorage.migrationVersionAddress != address(0),
            DAFLib.MIGRATION_CONTRACT_UNDEFINED
        );
        _;
    }
        ...
        requireAccountNotToHaveToken(msg.sender)
    {
        ...
        //Update token with new id
        uint256 tokenNewId = dafStorage.nextId;
        token.id = tokenNewId;
        ...
        if (token.score == 0) {
            uint256 previousVersionPauseStartTime = previousVersion
                .pauseStartTime();
            int256 remainingTime = int256(token.dateOfDeath) +
        ...
        // Mint the new corresponding token
        mint();
        ...
        // If the token has core contributor role, emit the corresponding event
        //slither-disable-next-line incorrect-equality: https://github.com/crytic/slither/wiki/Detector-Documentation#dangerous-strict-equalities
        if (token.role == Role.Core) {
            emit HasBecomeCoreMember(msg.sender, tokenNewId);
        }
