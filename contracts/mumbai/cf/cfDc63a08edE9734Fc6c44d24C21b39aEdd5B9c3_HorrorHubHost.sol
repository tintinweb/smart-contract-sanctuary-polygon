...
contract HorrorHubHost is AccessControl, ContextMixin, NativeMetaTransaction {
    ...
    struct GameInfo {
        string id;
        string metadataUrl;
        uint256 price; // price in USDT (will be paid in FEAR according to the current rate)
    }

    GameInfo[] public games;
    mapping (string => GameInfo) public gamesById;
    mapping (address => mapping (string => bool)) public userInstallations;
    mapping (address => mapping (string => bool)) public userPurchases;


    constructor(address fearContractAddress, address muonContractAddress) {
    ...
    function _getUserGames(address user, bool purchased) private view returns(string[] memory) {
        uint256 userGamesCount;
        for (uint i = 0; i < games.length; i++) {
            if ((purchased && userPurchases[user][games[i].id]) || (!purchased && userInstallations[user][games[i].id]))
                userGamesCount++;
        }
    ...
    function getUserPurchasedGames(address user) external view returns(string[] memory) {
        return _getUserGames(user, true);
    }

    function getUserInstalledGames(address user) external view returns(string[] memory) {
        return _getUserGames(user, false);
    }

    function getGamePriceInFear(string memory gameId, uint256 fearPrice) public view returns(uint256) {
        require(bytes(gamesById[gameId].id).length > 0, "Game not found");

        uint256 price = gamesById[gameId].price;

        uint256 interPrice = price.mul(10 ** 18).mul(10 ** 18);
        uint256 gamePriceInFear = interPrice.div(fearPrice);

        return gamePriceInFear;
    }

    function registerInstallation(string memory gameId) external {
        address user = _msgSender();

        require(bytes(gamesById[gameId].id).length > 0, "Game not found");
        require(gamesById[gameId].price == 0 || userPurchases[user][gameId], "Must purchase first");

        userInstallations[user][gameId] = true;
    }

    function registerUninstallation(string memory gameId) external {
        address user = _msgSender();
        userInstallations[user][gameId] = false;
    }
    ...
    function purchaseGame(
        ...
        require(!userPurchases[user][gameId], "Already purchased");
        require(block.timestamp - muonTimestamp < acceptableMuonDelay, "Data expired");
        ...
        require(muon.verify(toBytes(requestId), uint256(hash), signatures), "Verification fail");

        uint256 fearAmount = getGamePriceInFear(gameId, fearPrice);
        ...
        userPurchases[user][gameId] = true;
    }

    //// ADMIN FUNCTIONS
   
    function addGame(string memory id, string memory metadataUrl, uint256 price) external onlyRole(MANAGER_ROLE) {
    ...
    function editGame(string memory id, string memory metadataUrl, uint256 price) external onlyRole(MANAGER_ROLE) {
