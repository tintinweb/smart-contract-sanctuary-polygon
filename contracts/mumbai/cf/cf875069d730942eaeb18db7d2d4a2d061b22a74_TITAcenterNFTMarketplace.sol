...
contract TITAcenterNFTMarketplace is ERC721URIStorage {
    ...
    constructor(address _address) ERC721("TITAcenterNFTMarketplace", "TITA") {
    ...
    function createToken(
        ...
        createMarketItem(newTokenId, price, metadata);
    ...
    function toggleSale(uint256 tokenId) external {
        ...
        address tokenOwner = MarketItems[tokenId].seller;
        require(msg.sender == tokenOwner, "Not owner of this token");
        if (MarketItems[tokenId].sale) {
            MarketItems[tokenId].sale = false;
            _transfer(address(this), msg.sender, tokenId);
        } else {
            MarketItems[tokenId].sale = true;
    ...
    function buy(uint256 tokenId) public payable {
        ...
        uint256 price = MarketItems[tokenId].price + gasFee;
        address payable seller = MarketItems[tokenId].seller;
        require(MarketItems[tokenId].sale, "It's not ready for sale");
        require(
            msg.value >= price,
            "Please submit the asking price in order to complete the purchase"
        );
        _transfer(tokenOwner, msg.sender, tokenId);
        payable(seller).transfer(msg.value - gasFee);
        payable(owner).transfer(gasFee);
        MarketItems[tokenId].seller = payable(msg.sender);
        MarketItems[tokenId].sale = false;
        emit NftBought(seller, msg.sender, msg.value, tokenId);
    }
    ...
    function transferToken(address to, uint256 tokenId) public payable {
        ...
        payable(owner).transfer(gasFee);
        MarketItems[tokenId].seller = payable(to);
    }

    /* Returns all ready to sale market items */
    ...
    function myItems() public view returns (MarketItem[] memory) {
        ...
        uint256 currentIndex = 0;
        for (uint256 i = 0; i < totalItemCount; i++) {
            address seller = MarketItems[i + 1].seller;
    ...
    function getAllTokens() public view returns (MarketItem[] memory) {
        ...
        for (uint256 i = 0; i < totalItems; i++) {
            uint256 currentId = MarketItems[i + 1].tokenId;
            MarketItem storage currentItem = MarketItems[currentId];
            items[currentIndex] = currentItem;
            currentIndex += 1;
        }
