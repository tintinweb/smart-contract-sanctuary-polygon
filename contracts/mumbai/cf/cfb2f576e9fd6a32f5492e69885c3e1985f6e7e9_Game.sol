...
contract Game is IGame, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable{
    ...
    mapping (address => FreeAffiliate) public freeAffiliate;
    ...
    function canShoot(address account) external override view returns (bool) {
        return !requestIdUsed[userRequestIds[account]] && userRequestIds[account] != 0;
    }
    ...
    function arrangeMissile(uint256[12] memory positions) external payable override nonZero(positions) nonReentrant whenNotPaused {
        (uint256 price, uint256[] memory events, UseCache[] memory useCache, uint256 amountAff) = calPrice(positions, msg.sender);
        ...
        if (amountAff > 0) {
            freeAffiliate[msg.sender].amountMissile -= uint128(amountAff);
        }
    ...
    function calPrice(uint256[12] memory positions, address account) public override view returns (uint256, uint256[] memory, UseCache[] memory, uint256 amountAff) {
        uint256 value;
        uint256[] memory events = getFreeEventPerAccount(account);
        ...
        for (uint256 i = 0; i < 12; i++) {
            ...
            if (!free) {
                if (freeAffiliate[account].typeMissile == positions[i] && freeAffiliate[account].amountMissile > amountAff) {
                    amountAff += 1;
                } else {
        ...
        return (value * cost, events, useCache, amountAff);
    }

    function getFreeEventPerAccount(address account) public override view returns (uint256[] memory) {
    ...
    function getFreeMissilePerAccount(address account) public override view returns (uint256[] memory) {
        uint256[] memory events = getFreeEventPerAccount(account);
        ...
        amountFree[freeAffiliate[account].typeMissile] += freeAffiliate[account].amountMissile;
        return amountFree;
    }
    ...
    function clickAffiliate(address endUser, address publisher) external onlyOperator {
        ...
        freeAffiliate[endUser].typeMissile = uint128(typeFreeAffiliate);
        freeAffiliate[endUser].amountMissile = uint128(numberFreeAffilate);
    }
...
pragma solidity ^0.8.0; 
 ...
 interface IGame {
    ...
    struct FreeAffiliate {
        uint128 typeMissile;
        uint128 amountMissile;
    }
    ...
    function canShoot(address account) external view returns (bool);
    ...
    function calPrice(uint256[12] memory positions, address account) external view returns (uint256, uint256[] memory, UseCache[] memory, uint256 amountAff);

    function getFreeEventPerAccount(address account) external view returns (uint256[] memory);
