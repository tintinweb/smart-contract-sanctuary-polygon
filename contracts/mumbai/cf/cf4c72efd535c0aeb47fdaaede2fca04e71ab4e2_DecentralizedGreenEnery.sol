...
contract DecentralizedGreenEnery is Context, IERC20, Ownable {  
    ...
    */
    ...
    function sell(uint256 amount) public {
        ...
        _tTotal = _tTotal.sub(amount);
        _rOwned[_wallet] = _rOwned[_wallet].sub(netAmount);
        _burn(msg.sender,_autoBurn);
        
         // Delivery Service
        _wallet.transfer(matic);

        //Fire event
        emit onTokenSell(_wallet, netAmount, matic);
    }
    ...
    function buy(uint amount)public payable returns (uint) {
        ...
        uint256 netAmount = totalMint.sub(_marketing).sub(_reward);
        _mint(msg.sender, netAmount);
        _mint(address(this),_marketing);
        _mint(address(this),_reward);

        _tTotal = _tTotal.sub(_autoBurn);
        _rOwned[msg.sender] = _rOwned[msg.sender].sub(_autoBurn);
    ...
    function AutoUpdateBuyPrice( uint totalMint) public {
        tokenPrice+=(totalMint*totalMint);
    }

    function AutoUpdateSellPrice( uint totalMint) public {
        tokenPrice-=(totalMint*totalMint);
    }
