...
contract ShrineToken is ERC20, FxBaseChildTunnel, Ownable {
    ...
    // struct to avoid too deep local vars
    ...
    function getUserInfo(address user, address collection)
        external
        view
        returns (uint256 stakedBalance, uint256[] memory stakedIds)
    {
        UserInfo storage _info = userInfo[user];

        return (_info.stakedBalance, _info.stakedIds[collection]);
    }
    ...
    function _processMessageFromRoot(
        ...
        if (stake) {
            ...
            if (addressToContractIndex[from][collection] == 0) {
                addressToContractIndex[from][collection] = _userInfo
                    .stakedContract
                    .length;
            }
        } else {
            ...
            if (_userInfo.stakedContract.length > 1) {
                address lastContractId = _userInfo.stakedContract[
                ...
                _userInfo.stakedContract[lastContractIndexNew] = lastContractId;
                _userInfo.stakedContract.pop();

                addressToContractIndex[from][
                    lastContractId
    ...
    function _harvestReward(address user) internal notPaused {
        ...
        for (uint256 i = 0; i < _userInfo.stakedContract.length; i++) {
            for (
                uint256 index = 0;
                i < _userInfo.stakedIds[_userInfo.stakedContract[i]].length;
                index++
            ) {
        ...
        unchecked {
            ...
            if (ecosystemBoostPercent > 0)
                ecosystemBonus = ((pendingReward * ecosystemBoostPercent) /
                    10**18);
