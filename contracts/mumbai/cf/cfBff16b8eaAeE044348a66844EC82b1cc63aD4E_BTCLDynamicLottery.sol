...
contract BTCLDynamicLottery is AutomationCompatible, AutomatedRandomness, ReentrancyGuard {
    ...
    function checkUpkeep(bytes calldata /* checkData */) external view override returns (bool upkeepNeeded, bytes memory performData) {
        ...
        for (uint256 i = nextRound + 1; i < nextRound + maxRounds; i++) {
        ...
        performData = abi.encode(nextRound, roundsToProcess);
    }

    function performUpkeep(bytes calldata performData) external nonReentrant override {
        (uint256 currentRound, uint256[] memory roundsToProcess) = abi.decode(performData, (uint256, uint256[]));
        require(roundsToProcess.length > 0, "No rounds to draw");

        // Request VRF number for all rounds that need to be drawn
        uint256 numWords = totalWinners * roundsToProcess.length; // Request 3 numWords per round winner for all rounds
        ...
        for (uint256 i = 0; i < roundsToProcess.length; i++) {
            ...
            rounds[roundNr + 1].status.roundStatus = BTCLCore.Status.Drawing;
            rounds[roundNr + 1].status.requestId = requestId;
            emit BTCLCore.LotteryClosed(roundNr + 1, rounds[roundNr + 1].status.totalTickets, rounds[roundNr + 1].status.totalBets);
        }

        // Increment the next round if all rounds have been drawn
        if (roundsToProcess[roundsToProcess.length - 1] + 1 == currentRound + maxRounds) {
            nextRound += maxRounds;
            round++;
            rounds[round + 1].status.roundStatus = BTCLCore.Status.Open;
            emit BTCLCore.LotteryOpened(round + 1);
        }
    }
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomness) internal override {
        ...
        for (uint256 i = nextRound; i < nextRound + maxRounds; i++) {
            if (rounds[i + 1].status.requestId == requestId && rounds[i + 1].status.roundStatus == BTCLCore.Status.Drawing) {
                roundsProcessed++;

                rounds[i + 1].status.roundStatus = BTCLCore.Status.Completed;

                // Copy 3 elements from the randomness array to the storage array for the current round
                rounds[i + 1].status.randomness[0] = randomness[startIndex];
                rounds[i + 1].status.randomness[1] = randomness[startIndex + 1];
                rounds[i + 1].status.randomness[2] = randomness[startIndex + 2];

                emit BTCLCore.LotteryOpened(i + 1);
