...
contract LootBoxNFT is ILootBoxNFT, Initializable, Proxied, ERC721ForProxied, AccessControl {
    ...
    mapping(uint256 => uint256) public override collectionTotalSupply;

    mapping(uint256 => string) private _collectionURI;
    mapping(address => mapping(uint256 => uint256)) private _collectionBalances;
    mapping(uint256 => uint256) private _collectionNextTokenId;

    constructor() {}

    function changeCollectionURI(uint256 collectionId, string memory newURI) external override onlyRole(MINTER_ROLE) {
        _collectionURI[collectionId] = newURI;
        emit CollectionURIChanged(collectionId, newURI);
    }

    function mint(address to, uint256 collectionId) external override onlyRole(MINTER_ROLE) {
        uint256 baseId = collectionId << 128;
        uint256 fullId = baseId + _collectionNextTokenId[collectionId];
        _safeMint(to, fullId);
    }
    ...
    function balanceOf(address owner, uint256 collectionId) external view override returns (uint256) {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        return _collectionURI[collectionId];
    }
    ...
    function getCollectionId(uint256 tokenId) public pure returns (uint256) {
        return tokenId >> 128;
    }
    ...
    function _afterTokenTransfer(
        ...
        uint256 collectionId = getCollectionId(firstTokenId);
        if (from == address(0)) {
            unchecked {
                ++totalSupply;
                ++collectionTotalSupply[collectionId];
                ++_collectionBalances[to][collectionId];
                ++_collectionNextTokenId[collectionId];
            }
        } else if (to == address(0)) {
            unchecked {
                --totalSupply;
                --collectionTotalSupply[collectionId];
                --_collectionBalances[from][collectionId];
            }
        } else {
            unchecked {
                ++_collectionBalances[to][collectionId];
