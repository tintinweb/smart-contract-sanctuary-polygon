...
contract DOTCManager is ERC1155Holder, IdOTC, AdminFunctions, ReentrancyGuard {
        ...
        returns (uint256 takenOderId)
    {
        ...
        require(
            standardiseNumber(_amount, allOffers[offerId].tokenOutAddress) <= allOffers[offerId].amountOut,
            "Amount is greater than offer"
        );
        ...
        if (_amount == allOffers[offerId].amountOut) {
            amountToReceiveByTaker = allOffers[offerId].amountOut;
            allOffers[offerId].amountOut = 0;
            allOffers[offerId].fullyTaken = true;
            emit CompletedOffer(offerId);
        } else {
            amountToReceiveByTaker = standardiseNumber(_amount, allOffers[offerId].tokenOutAddress)
                .mul(10**DECIMAL)
                .div(allOffers[offerId].unitPrice);
