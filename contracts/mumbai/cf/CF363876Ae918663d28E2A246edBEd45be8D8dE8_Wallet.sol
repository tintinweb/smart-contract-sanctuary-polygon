...
import "./IEIP4337Wallet.sol";
import "../../entrypoint/IEntryPoint.sol";

/**
 * @dev Basice implementation of a EIP4337 compatible wallet.
 */
abstract contract EIP4337Wallet is IEIP4337Wallet {
    IEntryPoint public entryPoint;
    mapping(uint256 => bool) private walletNonce;
    ...
    /**
     * @dev Wallet's entryPoint changed event.
     */
    event EntryPointChanged(address indexed oldEntryPoint, address indexed newEntryPoint);

    /**
     * @dev Ensure the call only from the owner.
     * Wallet must override this modifier.
     */
    ...
    /**
     * @dev Ensure the call only from the entryPoint.
     * Wallet must override this modifier.
     */
     ...
     */
    function updateEntryPoint(address newEntryPoint) external onlyEntryPoint {
     ...
     */
    function payPrefund(uint256 missingWalletFunds) internal virtual {
        if (missingWalletFunds == 0) return;
        // solhint-disable-next-line avoid-low-level-calls
    ...
    function validateAndUpdateNonce(UserOperation calldata userOp) internal {
        require(walletNonce[userOp.nonce] == false, "EIP4337Wallet: invalid nonce");
        walletNonce[userOp.nonce] = true;
    }
...
import "./EIP4337Wallet/EIP4337Wallet.sol";
import "./WalletManager/BaseWallet.sol";
import "./WalletManager/TransactionManager.sol";
import "./WalletManager/SecurityManager.sol";
import "./WalletManager/GuardianManager.sol";
 ...
 */
contract Wallet is EIP4337Wallet, BaseWallet, Initializable, TransactionManager, SecurityManager, GuardianManager {
    enum OwnerSignature {
    ...
    modifier onlyOwner() override(BaseWallet, EIP4337Wallet) {
        if (msg.sender != getOwner()) revert NotTheOwner();
        _;
    }

    modifier onlyEntryPoint() override(BaseWallet, EIP4337Wallet) {
        require(msg.sender == address(entryPoint), "Wallet: caller must be an entrypoint");
        _;
    }
    modifier onlyOwnerOrEntryPoint() override(BaseWallet) {
        if (msg.sender != owner && msg.sender != address(entryPoint)) revert NotTheOwnerOrTheEntryPoint();
        _;
    }

    /**
     * @dev Setup the verifirst data for wallet. Can only call one time
     * @param walletEntryPoint Entrypoint of the wallet
     * @param walletOwner EOA owner of the wallet
     * @param firstGuardian The first guardian
     */
    function initialize(
        address walletEntryPoint,
        address walletOwner,
        address firstGuardian
    ) external initializer {
        addGuardianIntoStorage(firstGuardian);
        entryPoint = IEntryPoint(walletEntryPoint);
        owner = walletOwner;
    }
    ...
    function validateSignature(
        ...
        (aggregator);
        bytes32 signHash = Utils.toEthSignedMessageHash(requestId);
        bytes memory signatures = userOp.signature;
        if (userOp.initCode.length > 0) {
            require(owner == Utils.recoverSigner(signHash, signatures, 0), "wallet: wrong signature");
            return;
        }
        (uint256 requiredSignatures, OwnerSignature option) = getRequiredSignatures(userOp.callData);
        validSignatureFormat(requiredSignatures, signatures.length);
        validateSigner(signHash, signatures, option);
    }
    ...
    function validateSigner(
        ...
        require(signatures.length >= 65, "Wallet: invalid signature length");
        ...
        for (uint256 i = 0; i < signatures.length / 65; i++) {
            ...
            if (i == 0) {
                if (option == OwnerSignature.Required) {
                    ...
                    require(getOwner() == signer, "Wallet: the head signature must be owner");
                    continue;
                }
            }
            require(signer > lastSigner, "Wallet: signers must be different");
            lastSigner = signer;
            (isGuardian, guardians) = Utils.isGuardianOrGuardianSigner(guardians, signer);
            require(isGuardian, "Wallet: signer is not wallet's guardian");
        }
    }
    ...
    function getRequiredSignatures(bytes calldata data) public view returns (uint256, OwnerSignature) {
        ...
        require(methodId != 0x00, "Wallet: invalid data format");
        if (
            methodId == TransactionManager.execFromEntryPoint.selector ||
            methodId == TransactionManager.execBatchFromEntryPoint.selector ||
        ...
        if (
            ...
            methodId == EIP4337Wallet.updateEntryPoint.selector
        ) {
            // owner + majority of guardians
            uint256 majorityGuardians = majorityOfGuardians();
        ...
        if (methodId == SecurityManager.executeRecovery.selector) {
            ...
            uint256 numberOfSignaturesRequired = majorityOfGuardians();
            return (numberOfSignaturesRequired, OwnerSignature.Disallowed);
        }

        revert("Wallet: unknown method");
    }
     ...
     */
    function validSignatureFormat(uint256 requireSignature, uint256 signatureLength) internal pure {
        require(requireSignature > 0, "Wallet: wrong signature requirement");
        require(requireSignature * 65 == signatureLength, "Wallet: wrong signature number");
    }
}
...
/**
 * @dev Basic contract for implementing wallet's features.
 */
abstract contract BaseWallet {
    ...
    // The security window
    uint256 internal constant SECURITY_WINDOW = 4 hours;

    /**
     * @notice The owner of wallet.
     */
    ...
    /**
     * @notice wallet specific storage
     */
    mapping(address => uint256) internal whitelist;

    /**
     * @notice count address in whitelist
     */
    uint256 internal whitelistCount;
    ...
    /**
     * @dev Ensure the call only from the owner or the wallet itselft.
     */
    ...
    /**
     * @dev Ensure the call only available when wallet locked.
     */
    modifier onlyWhenLocked() {
        if (isLocked() == false) revert NotLocked();
        _;
    }

    /**
     * @dev Ensure the call only available when wallet unlocked.
     */
    modifier onlyWhenUnlocked() {
        if (isLocked() == true) revert NotUnlocked();
        _;
    }

    /**
     * @dev Ensure the call only available when target whitelisted.
     */
    modifier checkSetupWhitelist(address target) {
        if (whitelistCount != 0 && isWhitelisted(target) == false && target != address(this)) {
    ...
    /**
     * @notice Returns the number of guardians for this wallet.
    ...
    /**
     * @notice Checks if an account is active guardian for this wallet.
     * @param guardian The account.
     * @return true if the account is an active guardian for this wallet.
     */
    ...
    /**
     * @dev Helper method to check if a wallet is locked.
     */
    function isLocked() public view returns (bool) {
        return lockReleaseTime > block.timestamp; // solhint-disable-line not-rely-on-time
    }

    /**
     * @dev Helper method to lock the wallet
     */
    function setLock(uint256 releaseAfter, bytes4 lockerFunction) internal {
        lockReleaseTime = releaseAfter;
...
/**
 * @dev Implementation of guardian-related features of the wallet.
 */
abstract contract GuardianManager is BaseWallet {
    // The security period to add/remove guardians
    uint256 internal constant SECURITY_PERIOD = 36 hours;

    // *************** Errors *************************** //
    /* solhint-disable */
    error GuardianCannotBeOwner();
    error DuplicateGuardian();
    error IsNotAnEOAOrSmartWallet();
    ...
    /**
     * @notice Lets add a guardian to this wallet.
     ...
     */
    function addGuardian(address guardian) external onlyEntryPoint onlyWhenUnlocked {
        ...
        if (!success) revert IsNotAnEOAOrSmartWallet();
    ...
    /**
     * @notice Confirms the pending addition of a guardian to this wallet.
     ...
     */
    function finalizeGuardianAddition(address guardian) external onlyWhenUnlocked {
        bytes32 id = keccak256(abi.encodePacked(guardian, "addition"));
     ...
     * @param guardian The guardian.
     */
    function cancelGuardianAddition(address guardian) external onlyEntryPoint onlyWhenUnlocked {
        bytes32 id = keccak256(abi.encodePacked(guardian, "addition"));
        if (pendingGuardianPeriod[id] == 0) revert UnknownPendingAddition();
        delete pendingGuardianPeriod[id];
    ...
    /**
     * @notice Lets revoke a guardian from its wallet.
     ...
     * @param guardian The guardian.
     */
    function cancelGuardianRevokation(address guardian) external onlyEntryPoint onlyWhenUnlocked {
        bytes32 id = keccak256(abi.encodePacked(guardian, "revokation"));
        if (pendingGuardianPeriod[id] == 0) revert UnknownPendingRevokation();
        delete pendingGuardianPeriod[id];
    ...
    /**
     * @notice Gets the info of an guaridan for this wallet.
     * @param guardian The guardian.
...
/**
 * @dev Implementation of transaction-related features of the wallet.
 */
abstract contract SecurityManager is BaseWallet {
    uint256 internal constant LOCK_PERIOD = 36 hours;
    uint256 internal constant RECOVERY_PERIOD = 36 hours;

    // Recovery related variables
    /**
     * @notice new owner of the wallet if succeed to recovery.
     */
    ...
    /* solhint-enable */
    event Locked(uint256 releaseTime);
    event Unlocked();
    ...
    /**
     * @notice Throws if there is no ongoing recovery procedure or recovery expired.
     */
     ...
     */
    function lock() external onlyOwnerOrEntryPoint onlyWhenUnlocked {
        /* solhint-disable not-rely-on-time */
        setLock(block.timestamp + LOCK_PERIOD, SecurityManager.lock.selector);
        emit Locked(block.timestamp + LOCK_PERIOD);
        /* solhint-enable not-rely-on-time */
    }

    /**
     * @notice Unlock the wallet.
     */
    function unlock() external onlyEntryPoint onlyWhenLocked {
     ...
     */
    function executeRecovery(address recovery) external onlyEntryPoint notWhenRecovery {
        ...
        // solhint-disable-next-line not-rely-on-time
        setLock(block.timestamp + LOCK_PERIOD, SecurityManager.executeRecovery.selector);
     ...
     */
    function finalizeRecovery() external onlyWhenRecovery {
        // solhint-disable-next-line not-rely-on-time
     ...
     */
    function cancelRecovery() external onlyEntryPoint onlyWhenRecovery {
 ...
 * @dev Implementation of transaction-related features of the wallet.
 */
abstract contract TransactionManager is BaseWallet {
    /* solhint-disable */
    error WrongArrayLength();
    /* solhint-enable */
    uint256 internal constant WHITELIST_PERIOD = 36 hours;
     ...
     */
    event BatchInvoked(address[] indexed target, uint256 value, bytes[] data);
    event WhitelistAdded(address[] indexed target, uint256 whitelistTime);
    ...
    function invoke(
        ...
        uint256 value,
        bytes memory data
    ) internal checkSetupWhitelist(target) onlyWhenUnlocked {
        // solhint-disable-next-line avoid-low-level-calls
    ...
    function isWhitelisted(address[] calldata targets) external view returns (bool) {
        for (uint256 i = 0; i < targets.length; i++) {
     ...
     */
    function addToWhitelist(address[] calldata targets) external onlyEntryPoint onlyWhenUnlocked {
        /* solhint-disable not-rely-on-time */
     ...
     */
    function removeFromWhitelist(address[] calldata targets) external onlyEntryPoint onlyWhenUnlocked {
