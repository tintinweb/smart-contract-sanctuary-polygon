...
contract TechBookFestFriends is ERC1155, Ownable {
    uint256 public constant TBF13 = 13;
    ...
    function mintBatch(address[] memory tos) public onlyOwner {
        _mintBatch(tos, TBF13);
    }

    function _mintBatch(address[] memory tos, uint256 id) internal {
        for (uint256 idx = 0; idx < tos.length; idx++) {
            address to = tos[idx];

            require(to != address(0), "ERC1155: mint to the zero address");
            require(balanceOf(to, id) != 0, "to address has NFT already");
