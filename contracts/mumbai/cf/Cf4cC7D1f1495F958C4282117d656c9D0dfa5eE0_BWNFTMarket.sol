...
interface IERC721 {
    ...
    function getNFTStatus(uint256 _tokenId) external view returns (bool);
}
...
contract BWNFTMarket is ReentrancyGuard, ERC721Holder, Pausable, AccessControl {
    ...
    function createOrder(
        ...
        require(orders[tokenId].locked == false, "NFT is locked");
        require(price > 0, "Price must be more than 0");
        require(
            IERC721(nftContract).getNFTOwner(tokenId) == msg.sender,
        ...
        IERC721(nftContract).lockNft(tokenId);
    ...
    function cancelOrder(uint256 tokenId, uint256 orderId) public nonReentrant {
        require(
            orders[tokenId].seller == msg.sender,
            "You don't own the order"
        );
        require(orders[tokenId].locked == true, "NFT isn't locked");
        require(
            orders[tokenId].orderId == orderId,
            "This is not latest orderId"
        );

        IERC721(orders[tokenId].nftContract).unlockNft(tokenId);
        orders[tokenId].locked = false;

        emit OrderCanceledEvent(
            orders[tokenId].nftContract,
            orderId,
            tokenId,
            msg.sender,
            orders[tokenId].buyWithTokenContract,
            false,
            false
        );
    }
    ...
    function Buy(uint256 tokenId, uint256 orderId) public nonReentrant {
        ...
        address buyWithTokenContract = orders[tokenId].buyWithTokenContract;
        ...
        address nftContract = orders[tokenId].nftContract;
        require(
            balance >= amount,
    ...
    function acceptOffer(
        ...
        if (IERC721(nftContract).getNFTStatus(tokenId) == true) {
            require(orders[tokenId].locked == true, "NFT isn't locked");
            IERC721(nftContract).unlockNft(tokenId);
            orders[tokenId].locked = false;
            orders[tokenId].sold = true;
        ...
        IERC721(nftContract).safeTransferFrom(msg.sender, buyerAddr, tokenId);
