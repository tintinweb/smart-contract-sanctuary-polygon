...
// File contracts/interfaces/ISiloManager.sol
...
// File contracts/interfaces/ILinkToken.sol
...
// File contracts/interfaces/IKeepersRegistry.sol
...
// File contracts/DeFi/Silos/SiloManager.sol
...
contract SiloManager is KeeperCompatibleInterface{
    ...
    function performUpkeep(bytes calldata performData) external override {
        ...
        //if funds excede threshold, add them to  upkeep
        if(upkeepId > 0){//only check this if the upkeepId is set
            //check if ERC20 balance of Link is enough
            uint ERC20LinkBal = ERC20Link.balanceOf(address(this));
            if(ERC20LinkBal >= addFundsThreshold && ERC20LinkBal <= PegSwap.getSwappableAmount(address(ERC20Link), address(ERC677Link))){
            ...
            if(ERC677Link.balanceOf(address(this)) >= addFundsThreshold){
    ...
    function setUpkeepId(uint id) external{
        require(msg.sender == address(ManagerFactory) || msg.sender == owner, "Only factory or owner can set upkeep id");
        upkeepId = id;
    }
}
...
// File contracts/DeFi/Silos/SiloManagerFactory.sol
...
contract SiloManagerFactory is Ownable, KeeperCompatibleInterface{
    ...
    uint public minFundingAmount = 1000000000000000000;//1 Link
    uint lastCheckedUpkeepId = 48;
    uint maxBatchSize = 100;
    uint32 public GAS_LIMIT = 2500000;
    ...
    function createSiloManager(uint _amount) external{
        ...
        bytes memory data = abi.encodeWithSelector(FUNC_SELECTOR, name, hex"", manager, GAS_LIMIT, msg.sender, hex"", amount, SOURCE);
    ...
    {   uint registryCount = Registry.getUpkeepCount() - 1;
        if(registryCount > lastCheckedUpkeepId){
            ...
            if(isManager[potentialManager] && gas == GAS_LIMIT && check.length == 0 && admin == ISiloManager(potentialManager).owner()){//next upkeep id is a manager
            ...
            for(uint i=0; i<batchSize; i++){
                (address potentialManager,,,,,,) = Registry.getUpkeep(lastCheckedUpkeepId + i + 1);
                if(isManager[potentialManager]){//next upkeep id is a manager
    ...
    function performUpkeep(bytes calldata performData) external override {
        ...
        (address silo, uint id) = abi.decode(performData, (address,uint));
        ...
        if(silo != address(0)){
            ...
            require(silo == target, "Manager address does not match upkeep id target");
            ISiloManager(silo).setUpkeepId(id);
        }
    }
