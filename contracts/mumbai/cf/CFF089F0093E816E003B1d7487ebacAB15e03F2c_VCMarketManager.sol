...
contract VCMarketManager is IVCMarketManager, FeeManager, CanWithdrawERC20 {
    error MktMgrUnexpectedAddress();
    error MktMgrOnlyAdminAllowed();
    error MktNoRoyalties();
    error MktMgrOnlyMarketplaceAllowed();
    error MktMgrInvalidPoolAmount();
    error MktMgrFailedToClaim();
    error MktMgrCallContractFailed();
    ...
    function setPool(address _pool) external {
        ...
        _setTo(_pool);
        pool = _pool;
        IVCMarketplace(marketplaceFixedPriceERC1155).setPool(_pool);
        IVCMarketplace(marketplaceFixedPriceERC721).setPool(_pool);
        IVCMarketplace(marketplaceAuctionERC721).setPool(_pool);
        IVCMarketplace(marketplaceFixedPricePocNft).setPool(_pool);
        IVCMarketplace(marketplaceAuctionPocNft).setPool(_pool);
    }

    function setStarter(address _starter) external {
        _onlyAdmin();
        _checkAddress(_starter);
        IVCMarketplace(marketplaceFixedPriceERC1155).setStarter(_starter);
        IVCMarketplace(marketplaceFixedPriceERC721).setStarter(_starter);
        IVCMarketplace(marketplaceAuctionERC721).setStarter(_starter);
        IVCMarketplace(marketplaceFixedPricePocNft).setStarter(_starter);
        IVCMarketplace(marketplaceAuctionPocNft).setStarter(_starter);
    }

    function changeMarketManager(address _mktManager) external {
        _onlyAdmin();
        _checkAddress(_mktManager);
        IVCMarketplace(marketplaceFixedPriceERC1155).setAdmin(_mktManager);
        IVCMarketplace(marketplaceFixedPriceERC721).setAdmin(_mktManager);
        IVCMarketplace(marketplaceAuctionERC721).setAdmin(_mktManager);
        IVCMarketplace(marketplaceFixedPricePocNft).setAdmin(_mktManager);
        IVCMarketplace(marketplaceAuctionPocNft).setAdmin(_mktManager);
    }
    ...
    function setMarketplaceAuctionERC1155(address _marketplaceAuctionERC1155) external {
        _onlyAdmin();
        _checkAddress(_marketplaceAuctionERC1155);
        marketplaceAuctionERC721 = _marketplaceAuctionERC1155;
    }
    ...
    function callContract(address _contract, bytes calldata _data) external {
        ...
        if (!status) {
            revert MktMgrCallContractFailed();
        }
    }
    ...
    function setCurrency(IERC20 _currency) external {
        _onlyAdmin();
        currency = _currency;
        IVCMarketplace(marketplaceAuctionERC721).setCurrency(_currency);
        IVCMarketplace(marketplaceFixedPriceERC1155).setCurrency(_currency);
        IVCMarketplace(marketplaceFixedPriceERC721).setCurrency(_currency);
        IVCMarketplace(marketplaceFixedPricePocNft).setCurrency(_currency);
        IVCMarketplace(marketplaceAuctionPocNft).setCurrency(_currency);
    }

    function setPoCNft(address _pocNft) external {
        _onlyAdmin();
        pocNft = IPoCNft(_pocNft);
        IVCMarketplace(marketplaceAuctionERC721).setPoCNft(_pocNft);
        IVCMarketplace(marketplaceFixedPriceERC1155).setPoCNft(_pocNft);
        IVCMarketplace(marketplaceFixedPriceERC721).setPoCNft(_pocNft);
        IVCMarketplace(marketplaceFixedPricePocNft).setPoCNft(_pocNft);
        IVCMarketplace(marketplaceAuctionPocNft).setPoCNft(_pocNft);
    }

    function setMinPoolFeeBps(uint96 _minPoolFeeBps) external {
        _onlyAdmin();
        IVCMarketplace(marketplaceFixedPriceERC1155).setMinPoolFeeBps(_minPoolFeeBps);
        IVCMarketplace(marketplaceFixedPriceERC721).setMinPoolFeeBps(_minPoolFeeBps);
        IVCMarketplace(marketplaceAuctionERC721).setMinPoolFeeBps(_minPoolFeeBps);
        IVCMarketplace(marketplaceFixedPricePocNft).setMinPoolFeeBps(_minPoolFeeBps);
        IVCMarketplace(marketplaceAuctionPocNft).setMinPoolFeeBps(_minPoolFeeBps);
    }

    function setMarketplaceFeeBps(uint256 _marketplaceFeeBps) external {
        _onlyAdmin();
        IVCMarketplace(marketplaceFixedPriceERC1155).setMarketplaceFeeBps(_marketplaceFeeBps);
        IVCMarketplace(marketplaceFixedPriceERC721).setMarketplaceFeeBps(_marketplaceFeeBps);
        IVCMarketplace(marketplaceAuctionERC721).setMarketplaceFeeBps(_marketplaceFeeBps);
        IVCMarketplace(marketplaceFixedPricePocNft).setMarketplaceFeeBps(_marketplaceFeeBps);
        IVCMarketplace(marketplaceAuctionPocNft).setMarketplaceFeeBps(_marketplaceFeeBps);
    }

    function setMaxBeneficiaryProjects(uint96 _maxBeneficiaryProjects) external {
        _onlyAdmin();
        IVCMarketplace(marketplaceFixedPriceERC1155).setMaxBeneficiaryProjects(_maxBeneficiaryProjects);
        IVCMarketplace(marketplaceFixedPriceERC721).setMaxBeneficiaryProjects(_maxBeneficiaryProjects);
        IVCMarketplace(marketplaceAuctionERC721).setMaxBeneficiaryProjects(_maxBeneficiaryProjects);
        IVCMarketplace(marketplaceFixedPricePocNft).setMaxBeneficiaryProjects(_maxBeneficiaryProjects);
        IVCMarketplace(marketplaceAuctionPocNft).setMaxBeneficiaryProjects(_maxBeneficiaryProjects);
    }
    ...
    function _checkAddress(address _addr) internal view {
        if (_addr == address(this) || _addr == address(0)) {
            revert MktMgrUnexpectedAddress();
        }
    }
}
