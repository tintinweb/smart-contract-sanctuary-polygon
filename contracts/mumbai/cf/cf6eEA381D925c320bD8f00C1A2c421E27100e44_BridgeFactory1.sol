...
contract BridgeFactory1 {
   ...
   function __create_bridge (address token) public {
       ...
       if(_bridges[token] == address(0)) {
          ...
          Bridge1 bridge = new Bridge1(token, custodian);
...
contract Bridge1 {
    ...
    function __lock_tokens (uint _bridgedAmount, bytes32 _mainDepositHash) external {
        
        IERC20(token).transferFrom(msg.sender, address(this), _bridgedAmount);
     
        emit TokensLocked(msg.sender, _mainDepositHash, _bridgedAmount, block.timestamp);
    }

    function __unlock_tokens (address reciver, uint _bridgedAmount, bytes32 _sideDepositHash) external {

        if(msg.sender != custodian) {
            revert("Not a custodian");
        }

        IERC20(token).transfer(reciver, _bridgedAmount);

        emit TokensUnlocked(reciver, _sideDepositHash, _bridgedAmount, block.timestamp);
    } 
