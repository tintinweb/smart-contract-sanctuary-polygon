...
/// @notice Thrown when liquidity pool of index `index` is deactivated
error DEX__LiquidityPoolDeActivated(uint256 index);
/// @notice Thrown when liquidity pool of index `index` is activated
error DEX__LiquidityPoolIsActivated(uint256 index);
/// @notice Thrown when `index` cannot be found in array
error DEX__IndexOutOfBounds(uint256 index);
 ...
 * added, deactivated and removed through access-restricted functions, favourably
 ...
 * allow owner features, while still keeping upgradeablity functionality.
 *
...
contract DEX is OwnableUpgradeable {
    ...
    /// @dev Defines the structure for a liquidity pool stored on the DEX
    ...
    LiquidityPool[] private s_liquidityPools;
    ...
    /// @notice Event emitted when a liquidity pool is deactivated
    event LiquidityPoolDeActivated(uint256 index, address liquidityPoolAddress);
    /// @notice Event emitted when a liquidity pool is reactivated
    event LiquidityPoolReActivated(uint256 index, address liquidityPoolAddress);
    /// @notice Event emitted when a liquidity pool is removed from the DEX
    event LiquidityPoolRemoved(uint256 index, address liquidityPoolAddress);
    ...
    /// @notice Checks if liquidity pool at `index` is not deactivated
    modifier notDeActivated(uint256 index) {
        if (s_liquidityPools[index].deActivated) revert DEX__LiquidityPoolDeActivated(index);
        _;
    }

    /// @notice Checks if liquidity pool at `index` is listed on the DEX
    modifier poolExists(uint256 index) {
        if (index + 1 > s_liquidityPools.length) revert DEX__IndexOutOfBounds(index);
        _;
    }
     ...
     * functionality.
     * Sets the `msg.sender` as owner of the contract
     */
    ...
    /**
     * @notice Function for adding an NativeLiquidityPool to the DEX
     ...
     *
     * Note that already liquidity pools with the same parameters as an existing one may
     * still be added to the DEX.
     *
    ...
    function addNativeLiquidityPool(
        ...
        ILiquidityPool newLiquidityPoolInterface = ILiquidityPool(address(newLiquidityPool));
        s_liquidityPools.push(LiquidityPool(newLiquidityPoolInterface, false));

        emit LiquidityPoolAdded(
            address(newLiquidityPoolInterface),
            ILiquidityPool.Kind.NativeLiquidityPool
        );
    }

    /**
     * @notice Function for adding a TokenLiquidityPool to the DEX
     * @param xAddress is the address of the x token in the new TokenLiquidityPool
     * @param yAddress is the address of the y token in the new TokenLiquidityPool
     * @param swapFee is the swapFee to be used for swapping in the pool
    ...
    function addTokenLiquidityPool(
        ...
        emit LiquidityPoolAdded(
            address(newLiquidityPoolInterface),
            ILiquidityPool.Kind.TokenLiquidityPool
        );
    }

    /**
     * @notice Function for deactivating a liquidity pool
     * @param index is the index of the liquidity pool
     * @dev The function can only be called by the owner and if the DEX is not
     * CLOSED. This implemenation will check for the existence of a liquidity pool at
     * `index`. In case of the liquidity pool at `index` being deactivated, the
     * function will revert.
     *
     * This function emits the {LiquidityPoolDeActivated} event.
     */
    function deActivateLiquidityPool(uint256 index)
        external
        onlyOwner
        stateIsNot(State.CLOSED)
        poolExists(index)
        notDeActivated(index)
    {
        s_liquidityPools[index].deActivated = true;
        emit LiquidityPoolDeActivated(index, address(s_liquidityPools[index].liquidityPool));
    }

    /**
     * @notice Function for reactivating a deactivated liquidity pool
     * @param index is the index of the liquidity pool
     * @dev The function can only be called by the owner and if the DEX is not
     * CLOSED. This implemenation will check for the existence of a liquidity pool at
     * `index`. In case of the liquidity pool at `index` already being activated, the
     * function will revert.
     *
     * This function emits the {LiquidityPoolReActivated} event.
     */
    function reActivateLiquidityPool(uint256 index)
        external
        onlyOwner
        stateIsNot(State.CLOSED)
        poolExists(index)
    {
        if (s_liquidityPools[index].deActivated == false)
            revert DEX__LiquidityPoolIsActivated(index);

        s_liquidityPools[index].deActivated = false;
        emit LiquidityPoolReActivated(index, address(s_liquidityPools[index].liquidityPool));
    }

    /**
     * @notice Function for removing a liquidity pool from the DEX
     * @param index is the index of the liquidity pool
     * @dev The function can only be called by the owner and if the DEX is not
     * CLOSED. This implemenation will check for the existence of a liquidity pool at
     * `index`.
     *
     * The function in effect resizes the liquidity pool array in the DEX, while removing the
     * liquidity pool at `index` from the storage variable. As this process is more
     * gas intensive than deactivating liquidity pools, it is advised to instead use the
     * deActivateLiquidityPool function when suitable.
     *
     * This function emits the {LiquidityPoolRemoved} event.
     */
    function removeLiquidityPool(uint256 index)
        external
        onlyOwner
        stateIsNot(State.CLOSED)
        poolExists(index)
    {
        address lpAddress = address(s_liquidityPools[index].liquidityPool);

        s_liquidityPools[index] = s_liquidityPools[s_liquidityPools.length - 1];
        s_liquidityPools.pop();
        emit LiquidityPoolRemoved(index, lpAddress);
    }

    /**
     * @notice Function for swapping at liquidity pool at `index`
     * @param index is the index of the liquidity pool
     * @param tokenAmount is the amount of tokens to be swapped.
     * Note that `tokenAmount` can be zero if swapping eth for tokens (xToY, EthLiquidityPool)
     * @dev The function calls the swapFrom function of the liquidity pool at `index` (see
     * ILiquidityPool for more documentation). In order for the DEX to be able to swap from
     ...
     * check for the existence of a liquidity pool at `index`. In case of the liquidity pool
     * at `index` being deactivated, the function will revert.
     */
    function swapAt(uint256 index, uint256 tokenAmount)
        external
        payable
        stateIs(State.OPEN)
        poolExists(index)
        notDeActivated(index)
    {
        s_liquidityPools[index].liquidityPool.swapFrom{value: msg.value}(msg.sender, tokenAmount);
    }
    ...
    /**
     * @notice Function for retrieving the address of liquidity pool at `index`
     * @param index is the index of the liquidity pool
     * @return Address of the liquidity pool at `index`
     */
    function getLiquidityPool(uint256 index) public view returns (address) {
        return address(s_liquidityPools[index].liquidityPool);
    }

    /**
     * @notice Function for retrieving if liquidity pool at `index` is
     * activated
     * @param index is the index of the liquidity pool
     * @return Activation value of liquidity pool at `index`
     */
    function isActivated(uint256 index) public view returns (bool) {
        return !s_liquidityPools[index].deActivated;
    }
...
/**
 * @dev This is an interface for TokenLiquidityPool and the NativeLiquidityPool to implement
 * standardized functionality of both liquidity pools. Using this interface, third party contracts
 * can use liquidity pools of both types, while keeping the most vital features standardized.
 */
interface ILiquidityPool {
    ...
    /**
     * @dev Defines the direction a swap should be executed in.
    ...
    struct Allowance {
        /// @dev specifies the amount of the asset can be swapped
        uint256 amount;
...
contract NativeLiquidityPool is Ownable, Initializable, ILiquidityPool {
    ...
    constructor(address tokenAddress, uint256 swapFee) {
        i_token = IERC20(tokenAddress);
        i_swapFee = swapFee;
    }

    /**
     * @notice Function for setting up (initializing) the liquidity pool
     ...
     * if swapping `amount`. The fee `fee` will be substracted from the
     * output
     */
     ...
     * @return Amount of tokens expected to be recieved for swap
     */
     ...
     * @param tokenAmount is the amount of tokens to be swapped
     * @return Amount of native expected to be recieved for swap
     */
...
/// @notice Thrown when caller sends eth to the contract
error TokenLiquidityPool__EthNotAccepted();
/// @notice Thrown when requested amount to swap is smaller than allowance
 ...
 * over time from the swapping fee. Liquidity can be withdrawn at all times. Users can
...
contract TokenLiquidityPool is Ownable, Initializable, ILiquidityPool {
     ...
     */
    function swap(uint256 tokenAmount, SwapDirection direction) external payable override {
        if (msg.value > 0) revert TokenLiquidityPool__EthNotAccepted();
     ...
     * if swapping `amount`. The fee `fee` will be substracted from the token
     * output
     */
     ...
     * @return Amount of y tokens expected to be recieved for swap
     */
    ...
    /**
     * @notice Function for retrieving x token output for swap of y tokens `xAmount`
     * Note that token output fluctuates with price and therefore changes constantly
     * @param yAmount is the amount of y tokens to be swapped
     * @return Amount of x tokens expected to be recieved for swap
     */
