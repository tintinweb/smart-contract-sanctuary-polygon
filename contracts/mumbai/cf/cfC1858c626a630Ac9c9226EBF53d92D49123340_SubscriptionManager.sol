...
contract SubscriptionManager {
    ...
    function onKeyPurchase(
        ...
        address prevPlanAddr = address(subscriptions[botOwner].plan);

        subscriptions[botOwner].tokenId = tokenId;
        subscriptions[botOwner].plan = ILock(newPlanAddr);

        refundFromPreviousPlan(prevPlanAddr, prevTokenId);
    }

    function refundFromPreviousPlan(address prevPlanAddr, uint256 prevTokenId) internal {
        if (prevPlanAddr == address(0) || prevTokenId == 0) {
            return;
        }

        // cancel and refund remaining amount if there are valid subscriptions in this plan
        ILock prevPlan = ILock(prevPlanAddr);
        if (prevPlan.isValidKey(prevTokenId)) {
            uint256 refundValue = prevPlan.getCancelAndRefundValue(prevTokenId);
            prevPlan.expireAndRefundFor(prevTokenId, refundValue);
        }
    }
