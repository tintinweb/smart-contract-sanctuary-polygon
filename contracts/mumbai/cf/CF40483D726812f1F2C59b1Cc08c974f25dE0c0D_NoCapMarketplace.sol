...
contract NoCapMarketplace is Ownable, Initializable, EIP712Upgradeable, ReentrancyGuardUpgradeable {
    ...
    function buyNFT(Voucher.NFTVoucher memory voucher,
        ...
        if(isPrimary) {
           (,uint amount,) = calculateTotalAmount(voucher,isPrimary);
            ...
            if(_currency==address(1)){
                ...
                saleTransaction(voucher.NFTAddress, voucher.seller, voucher.tokenId,voucher.fractions, amount, (voucher.pricePerFraction)*voucher.fractions, _currency);
                if(msg.value > amount){
            ...
            platformCollection[_currency]+= (platformFeePercent*voucher.pricePerFraction*voucher.fractions)/10000;
            ...
            (address receiver,uint amount,uint royaltyAmount) = calculateTotalAmount(voucher,isPrimary);
            if(_currency==address(1)) {
                require(msg.value >= amount,"Invalid amount.");
                ...
                IERC20(_currency).transferFrom(msg.sender, address(this), (platformFeePercent*voucher.pricePerFraction*voucher.fractions)/10000);
                platformCollection[_currency] += (platformFeePercent*voucher.pricePerFraction*voucher.fractions)/10000;
    ...
    function calculateTotalAmount(Voucher.NFTVoucher memory voucher,bool _isPrimary) public view returns(address, uint256, uint256){
        uint platformAmount = (platformFeePercent*voucher.pricePerFraction*voucher.fractions)/10000;
        uint totalAmount;
        address receiver;
        uint royaltyAmount;
        if(_isPrimary){
        totalAmount = platformAmount+(voucher.pricePerFraction)*voucher.fractions;
        }
        else{
        ...
        totalAmount = platformAmount+((voucher.pricePerFraction)*voucher.fractions)+royaltyAmount;
        receiver = receivers;
