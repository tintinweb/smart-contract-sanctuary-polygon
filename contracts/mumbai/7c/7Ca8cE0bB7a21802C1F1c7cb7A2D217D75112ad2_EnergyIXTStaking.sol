...
contract EnergyIXTStaking is IEnergyIXTStaking, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function earned(address _walletAddress) public view returns (uint256) {
        return (userStakedAmount[_walletAddress] * (rewardPerTokenStored() - s_rewardPool.userRewardPerTokenPaid[msg.sender])) + s_rewardPool.rewards[msg.sender];
    }

    function earnedAmelia(address _walletAddress) public view returns (uint256) {
        return (userAmeliaCommitment[_walletAddress] * (rewardPerTokenStored() - s_rewardPool.userRewardPerTokenPaid[msg.sender])) + s_rewardPool.ameliaRewards[msg.sender];
    }
