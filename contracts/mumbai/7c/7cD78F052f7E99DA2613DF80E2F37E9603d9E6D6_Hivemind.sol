...
contract Hivemind is HivemindV1Storage, HivemindInterface, ComptrollerErrorReporter, ExponentialNoError, Ownable {
  ...
  event CompBorrowSpeedUpdated(HTokenI indexed hToken, uint256 newSpeed);

  /// @notice Emitted when a new supply-side COMP speed is calculated for a market
  event CompSupplySpeedUpdated(HTokenI indexed hToken, uint256 newSpeed);

  /// @notice Emitted when a new COMP speed is set for a contributor
  event ContributorCompSpeedUpdated(address indexed contributor, uint256 newSpeed);

  /// @notice Emitted when COMP is distributed to a supplier
  event DistributedSupplierComp(HTokenI indexed hToken, address indexed supplier, uint256 compDelta, uint256 compSupplyIndex);

  /// @notice Emitted when COMP is distributed to a borrower
  event DistributedBorrowerComp(HTokenI indexed hToken, address indexed borrower, uint256 compDelta, uint256 compBorrowIndex);
...
interface HTokenI {
  ...
  function getUserCoupons(address user) external view returns (Coupon[] memory);

  function searchCoupon(address user, uint256 _tokenId) external view returns (uint256);

  function getSpecificCoupon(uint256 _couponNFTId) external view returns (Coupon memory);

  function accrueInterestToCoupons() external returns (uint256, uint256);

  function accrueInterestToCoupon(uint256 index) external returns (uint256);

  function closeoutLiquidation(address borrower, uint256 _tokenId) external returns (bool);

  function withdrawalAllowed(address user, uint256 _tokenId) external view returns (bool);
...
interface PriceOracleI {
  function getUnderlyingPrice(IHERC20 hToken) external view returns (uint256);
