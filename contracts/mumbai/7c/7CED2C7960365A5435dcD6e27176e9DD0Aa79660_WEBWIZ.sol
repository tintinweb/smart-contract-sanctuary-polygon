...
contract WEBWIZ is ERC721Enumerable, Ownable, IRamperInterface721 {
    ...
    function price() override external view returns (uint256) {
        require(address(priceOracle) != address(0), "Price oracle address not set");
        return priceOracle.getCurrentPrice();
    }

    function calculateTotalPrice(uint256 _quantity) internal view returns (uint256) {
        require(address(priceOracle) != address(0), "Price oracle address not set");
        uint256 currentPrice = priceOracle.getCurrentPrice();
        uint256 totalPrice = currentPrice * _quantity;
        return totalPrice;
    }

    function mint(address _userWallet, uint256 _quantity) override external payable {
        uint256 totalPrice = calculateTotalPrice(_quantity);
        require(msg.value >= totalPrice, "Insufficient funds for requested tokens");
