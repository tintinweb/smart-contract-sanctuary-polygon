...
abstract contract ConsumerInterface {
    function init(
...
import "./ConsumerInterface.sol";
import "@openzeppelin/contracts/access/Ownable.sol";
import "@chainlink/contracts/src/v0.8/interfaces/AggregatorV3Interface.sol";
import "./SToken.sol";
import "./StonkslyInterface.sol";
...
contract Stonksly is StonkslyInterface, Ownable {
    enum RequestType {
        PURCHASE,
        SALE
    }
    ...
    ConsumerInterface s_purchaseConsumer;
    ConsumerInterface s_saleConsumer;
    ...
    modifier sTokenExists(address _sToken) {
    ...
    function initPurchase(
        address _sToken
    ) external payable sTokenExists(_sToken) {
    ...
    function finalizeSale(
        ...
        // //1% fee
        // uint256 maticToWithdraw = ((maticAmount * 99) / 100);
        // s_collectedFees = request.amount - maticToWithdraw;

        // SToken(request.sToken).burn(address(this), request.amount);
        sendMatic(request.account, maticAmount);
    ...
    function setPurchaseConsumer(ConsumerInterface _purchaseConsumer) external {
    ...
    function setSaleConsumer(ConsumerInterface _saleConsumer) external {
    ...
    function withdrawFees() external onlyAllowed(s_stonkslyWallet) {
