    ...
    Ownable
{
    ...
    function manulDistribute(uint256 time,uint256 amount) external onlyOwner {
        ...
        ClaimRecord memory record = ClaimRecord(week, block.number,totalSupply,totalSupply);
        systemClaimRecords[week] = record;
        emit ManulDistributed(week, block.number, totalSupply, totalSupply);
    ...
    function claim() external {
        ...
        for(uint256 i = 0; i< cycle;i++){
            ...
            uint256 currentDisAmount = balanceOf * claimRecord.remainAmount / claimRecord.veSupply;
    ...
    function _checkpointDistribute() internal {
        ...
        if (tokensPerWeek[thisWeek] == 0 && toDistribute > 0) {
    ...
    */
        ...
        returns (Claimable memory)
    {
        ...
        uint256 weekCursor  = (block.timestamp / WEEK) * WEEK;
        ...
        // require(claimRecords[_addr][weekCursor] == false,"current week has been claimed");
        ...
        uint256 _veSupply = veSupply[weekCursor];
        // uint256 balanceOf = veDEFED.balanceOf(_addr);
