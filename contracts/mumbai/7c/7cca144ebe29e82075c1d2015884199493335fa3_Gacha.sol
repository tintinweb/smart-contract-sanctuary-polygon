...
import "src/YourNFT.sol";
import "src/randVRF.sol";
import "src/rand.sol";
contract Gacha{
    uint256 private startId;
    uint256 private endId;
    uint256 private ZanNft;
    YourNFT nftContract;
//    RandomNumberConsumer RNCVRFContract;
    RandomNumberGenerator RNGContract;
    constructor(uint256 num){
        nftContract = new YourNFT();
//        RNCVRFContract = new RandomNumberConsumer();
        RNGContract = new RandomNumberGenerator();
        ZanNft = num;
        (startId,endId)= nftContract.mintItem(address(this),ZanNft);
        for(uint256 i = 0;i < num; i++)
        {
            /*  ERC1155
                uint256 amount = nftContract.balanceOf(address(this), startId+i);
                require(amount == 1, "mint error");
            */
            //ERC721パターン
            require(nftContract.ownerOf(startId+i) == address(this),"mint address error");
        }
        require((endId - startId) == num,"mint num error");
    }
    ...
    function nftGacha() public payable {
        require(msg.value % 1e15 == 0 , "number of digits error");
        //require(msg.value <= 0.5 ether , "limit 0.5 ether");
        uint256 loopnum = msg.value / 0.001 ether;
        console.log(ZanNft);
        require(loopnum <= ZanNft,"NFT shortage");
        uint256 bonusLoopNum = msg.value / 0.05 ether;
        for(uint256 i = 0; i < bonusLoopNum; i++)
        {
            RNGContract.setLotNum(10);
            uint requestId = RNGContract.lotStart(1);
            uint256 rand = RNGContract.getlotResult(requestId);
            require(( 0 < rand ) && ( 10 >= rand ),"Bonus amount error");
            loopnum += rand;
            if( loopnum >= ZanNft )
            {
                loopnum  = ZanNft;
        ...
        require( loopnum <= ZanNft ,"NFT shortage2");

        for(uint i = 0 ;i < loopnum; i++)
       {
            RNGContract.setLotNum(ZanNft);
            uint requestId = RNGContract.lotStart(1);
            uint256 rand = RNGContract.getlotResult(requestId);
            nftContract.transferItem(address(this),msg.sender,rand);
            ZanNft--;
       }
    }

    function NftNumChk() public view returns(uint256) 
    {
        return ZanNft;
    }
/*
    function getNftStatus(uint256 tokenId) public view returns (nftContract.nftStruct memory)
    {
        nftContract.nftStruct
        return nftContract.getYourNftStatus(tokenId);
    }
    */
}
...
contract YourNFT is ERC721, Ownable{
    ...
    mapping(uint256=>uint256) nftlist;
    mapping(uint256=>nftStruct) nftStatus;
    mapping(address=>uint256) hasNftList;
    function mintItem(address to,uint256 num) public 
    onlyOwner
    returns (uint256,uint256)
    {
        ...
        for(uint i = 1; i <= num ; i++)
        {
            ...
            _mint(to,id);
            nftlist[i] = id;
            nftStatus[nftlist[i]].beforeDraw = true;
/*            uint8 Rarity = 1;
            assembly{
                switch mod(id, 10)
                case 1 {
                    Rarity := calldataload(Rarity2)
                }
                case 2 {
                }
            }
            */
            uint8 onesPlace = uint8(nftlist[i] % 10);
            if (onesPlace == 2 || onesPlace == 5 || onesPlace == 9) {
                nftStatus[nftlist[i]].rarity = 1;
            } else if (onesPlace == 1 || onesPlace == 4 || onesPlace == 6) {
                nftStatus[nftlist[i]].rarity = 2;
            } else if (onesPlace == 0 || onesPlace == 8) {
                nftStatus[nftlist[i]].rarity = 3;
            } else if (onesPlace == 3) {
                nftStatus[nftlist[i]].rarity = 4;
            } else if (onesPlace == 7) {
                nftStatus[nftlist[i]].rarity = 5;
            } else {
    ...
    function transferItem(address from, address to,uint256 num) public
    {
        require(ownerOf(nftlist[num]) == from ,"not owner");
        require(nftStatus[num].beforeDraw == true, "Already minted");
        approve(address(this),nftlist[num]);
        transferFrom(from,to,nftlist[num]);
        if(num != endId)
        {
            nftlist[num] = nftlist[endId];
            nftlist[endId] = 0;
            nftStatus[nftlist[endId]].beforeDraw = false;
        }
        else
        {
            nftlist[endId] = 0;
    ...
    function getYourNftStatus(uint256 tokenId) public view returns (nftStruct memory)
    {
...
contract RandomNumberConsumer is VRFConsumerBaseV2 {
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        ...
        uint256 Value = randomWords[0] % lotnum + 1;
        lotResult[requestId] = Value;
        lotEnd[requestId] = true;
        // emitting event to signal that dice landed
        emit LotEnd(requestId, Value);
    }

    // house function
    function getlotResult(uint256 requestId) public view returns (uint256) {
        require(lotEnd[requestId] == true, "not lot end");
        return lotResult[requestId];
    }
