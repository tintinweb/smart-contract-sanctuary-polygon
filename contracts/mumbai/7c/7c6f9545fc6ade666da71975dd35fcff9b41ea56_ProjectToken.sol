...
contract ERC20Upgradeable is Initializable, ContextUpgradeable, IERC20Upgradeable, IERC20MetadataUpgradeable {
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        if (_lock[from].lockupList.length != 0) {
            ...
            for (uint8 i=0; i<_lock[from].lockupList.length;){ 
                ...
                for (uint8 j=0; j< _initialLock[from][i_initialLockTime].lockupPlanList.length;){
                ...
                if ((i_unlockedAmount == i_lockupAmount) || (_initialLock[from][i_initialLockTime].lockupPlanList.length == 0 && i_initialLockTime < block.timestamp) ) {
            ...
            require(_balances[from]-(lockupAmount_Total-unlockedAmount_Total) >= amount,"Transmittable amount exceeded");  
        }
    ...
    /*==================================================================
                            잠금 삭제 
    ==================================================================*/
    function lockup_delete(address account,uint32 lockupTime) external returns (bool) { 
        _ProjectFunctions();
        uint128 lockupAmount = _initialLock[account][lockupTime].lockupAmount;
        require(lockupAmount != 0,"lockupTime is not set");
    ...
    /*==================================================================
                            잠금플랜 설정
    ==================================================================*/ 
    function lockupPlan_setting(address account, uint32 lockupTime, uint128[] calldata lockupPlanTime, uint128[] calldata lockupPlanAmount) external returns (bool) {
        _ProjectFunctions();
        uint128 lockTotalAmount = _initialLock[account][lockupTime].lockupPlanTotalAmount;
        require(_initialLock[account][lockupTime].lockupAmount != 0,"lockupTime is not set");
        require(lockupPlanTime.length == lockupPlanAmount.length ,"The length of the array is different");
        if (lockupTime < block.timestamp){
            require(_initialLock[account][lockupTime].lockupPlanList.length != 0,"Cannot set for lockupTime");
        }
        uint128 addAmount;
        for (uint32 i=0; i<lockupPlanTime.length;){
            require(_initialLock[account][lockupTime]._lockupPlan[lockupPlanTime[i]].lockupPlanAmount == 0 && lockupPlanTime[i] >= lockupTime,"lockupTime is not correct");
            unchecked{addAmount += lockupPlanAmount[i];}
            _initialLock[account][lockupTime]._lockupPlan[lockupPlanTime[i]] = lockupPlanInfo(uint32(_initialLock[account][lockupTime].lockupPlanList.length),lockupPlanAmount[i]);
            _initialLock[account][lockupTime].lockupPlanList.push(lockupPlanTime[i]);
            unchecked{ i++; }
        }
        require(_initialLock[account][lockupTime].lockupAmount >= lockTotalAmount+addAmount ,"lockupPlanTotalAmount exceeded");
    ...
    /*==================================================================
                            잠금플랜 삭제 
    ==================================================================*/
    function lockupPlan_delete(address account,uint32 lockupTime,uint32 lockupPlanTime) external returns (bool) {
        _ProjectFunctions();
        uint128 addLockAmount = _initialLock[account][lockupTime]._lockupPlan[lockupPlanTime].lockupPlanAmount;
        require(addLockAmount != 0 && lockupPlanTime > block.timestamp,"lockupPlanTime is not correct");
