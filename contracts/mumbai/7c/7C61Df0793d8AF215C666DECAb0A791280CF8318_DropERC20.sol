 ...
 *  @notice  Gitshock Edgeware `ContractMetadata` is a contract extension for any base contracts. It lets you set a metadata URI
 *           for you contract.
...
/**
 *  The interface `IClaimCondition` is written for Gitshock Edgeware 'Drop' contracts, which are distribution mechanisms for tokens.
 *
...
/**
 *  The interface `IClaimConditionMultiPhase` is written for Gitshock Edgeware 'Drop' contracts, which are distribution mechanisms for tokens.
 *
...
/**
 *  Gitshock Edgeware `ContractMetadata` is a contract extension for any base contracts. It lets you set a metadata URI
 *  for you contract.
 *
 *  Additionally, `ContractMetadata` is necessary for NFT contracts that want royalties to get distributed on OpenSea.
 */
...
/**
 *  The interface `IDrop` is written for Gitshock Edgeware 'Drop' contracts, which are distribution mechanisms for tokens.
 *
 *  An authorized wallet can set a series of claim conditions, ordered by their respective `startTimestamp`.
 *  A claim condition defines criteria under which accounts can mint tokens. Claim conditions can be overwritten
 *  or added to by the contract admin. At any moment, there is only one active claim condition.
...
/**
 *  Gitshock Edgeware `PlatformFee` is a contract extension to be used with any base contract. It exposes functions for setting and reading
...
/**
 *  Gitshock Edgeware `Primary` is a contract extension to be used with any base contract. It exposes functions for setting and reading
        ...
        if (!_hasRole[role][account]) {
            revert(
                string(
                    abi.encodePacked(
                        "Permissions: account ",
                        GEStrings.toHexString(uint160(account), 20),
                        " is missing role ",
                        GEStrings.toHexString(uint256(role), 32)
                    )
                )
            );
        }
...
/**
 *  @title   Platform Fee
 *  @notice  Gitshock Edgeware `PlatformFee` is a contract extension to be used with any base contract. It exposes functions for setting and reading
 *           the recipient of platform fee and the platform fee basis points, and lets the inheriting contract perform conditional logic
 *           that uses information about platform fees, if desired.
 */
...
/**
 *  @title   Primary Sale
 *  @notice  Gitshock Edgeware `PrimarySale` is a contract extension to be used with any base contract. It exposes functions for setting and reading
 *           the recipient of primary sales, and lets the inheriting contract perform conditional logic that uses information about
 *           primary sales, if desired.
 */
