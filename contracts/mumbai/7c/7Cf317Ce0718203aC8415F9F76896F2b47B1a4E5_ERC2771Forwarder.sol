...
contract ERC2771Forwarder is EIP712 {
    ...
    bytes32 private constant _FORWARD_REQUEST_TYPEHASH =
        keccak256(
            "ForwardRequest(address from,address to,uint256 value,uint256 gas,uint48 deadline,bytes data)"
        );
     ...
     */
    event ExecutedForwardRequest(address indexed signer, bool success);
    ...
    function execute(
        ...
        if (!success) {
            if (returndata.length < 68) {
                revert FailCall("Something went wrong!");
            }
            assembly {
                // Slice the sighash.
    ...
    function _execute(
        ...
        if (signerMatch && alive) {
            ...
            emit ExecutedForwardRequest(signer, success);
        }
    }
