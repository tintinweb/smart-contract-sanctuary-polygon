...
contract UCO is ERC721AUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, "Not the Owner of the Gallery");
        _;
    }

    function initialize(string memory _name, string memory _symbol, string memory _projectID, address _admin) initializerERC721A initializer public {
    ...
    function mint(address owner, uint256 quantity, string[] memory cidArr) external payable onlyAdmin nonReentrant {
        _safeMint(owner, quantity, '');

        for (uint i = 0; i < cidArr.length; ++i) {
            cidOfTokenId[cidTokenCounter.current()] = cidArr[i];
            tokenState[cidTokenCounter.current()] = 0;
    ...
    function getTokenState(uint256 _id) public view returns (uint256) {
        return tokenState[_id];
    }

    function setTokenState(uint256 _state) public onlyAdmin {
