...
contract swapHelperDiyTask2 is OpsTaskCreator, Ownable {
    ...
    function createTask(string memory _taskName, IAocoRouter02 _routerAddress, address[] memory _swapRouter, address[] memory _swapRouter2, uint256 _swapAmount, uint256 _interval, uint256[] memory _start_end_Time, uint256[] memory _timeList) external payable returns (bytes32) {
        ...
        **/
        ...
        _createTaskItem._taskConfig._swapAmount = _swapAmount;
    ...
    modifier onlyTime(bytes32 _md5) {
        ...
        if (y._taskData._timeIntervalList.length == 0) {
            ...
            _interval = y._taskData._timeIntervalList[lastTimeIntervalIndexList[_md5]];
        }
        require(block.timestamp >= (lastExecutedTimeList[_md5]).add(_interval), "t001");
    ...
    function swapToken(address _user, bytes32 _md5) external onlyDedicatedMsgSender onlyTime(_md5) {
        ...
        lastTimeIntervalIndexList[_md5] = lastTimeIntervalIndexList[_md5] + 1;
        if (lastTimeIntervalIndexList[_md5] >= y._taskData._timeIntervalList.length) {
            lastTimeIntervalIndexList[_md5] = 0;
        }
    }
