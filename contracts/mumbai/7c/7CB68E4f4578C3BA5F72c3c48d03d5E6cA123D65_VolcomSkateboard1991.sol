...
contract VolcomSkateboard1991 is PunchableERC721{
    ...
    bool public eIDEnabled;

    uint16 maxSkateboards = 1991;

    constructor(uint _reservedIDs, string memory name, string memory symbol) PunchableERC721(_reservedIDs, maxSkateboards, name, symbol){}

    function setEIDEnabled(bool active) external onlyOwner{
        eIDEnabled = active;
    }

    function tokenExperience(uint tokenId) public virtual override view returns (uint) {
        return eIDEnabled ? EID[tokenId] : 0;
    }
}
