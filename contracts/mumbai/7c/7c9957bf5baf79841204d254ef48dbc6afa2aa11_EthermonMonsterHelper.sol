...
contract EthermonMonsterHelper is Ownable {
    ...
    function monsterWorldTokenOfOwnerByIndex(address addr, uint256 index)
        public
        view
        returns(uint64 monId, uint32 classId, address trainer, string memory name, uint32 exp, uint256[] memory baseStats, uint256[] memory currentStats)
        {
            uint256 resp = EthermonMonsterContract.tokenOfOwnerByIndex(addr, index);
            (monId, classId, trainer, name, exp,,,) = EthermonDataContract.monsterWorld(uint64(resp));
