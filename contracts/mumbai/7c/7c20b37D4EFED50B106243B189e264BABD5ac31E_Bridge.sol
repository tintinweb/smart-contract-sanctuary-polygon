...
contract Bridge is Ownable, Pausable {
    ...
    constructor(address _WMATIC, address[] memory _signers, uint256 _required) {
        require(isContract(_WMATIC), "not a contract");
    ...
    function depositETH() external payable whenNotPaused {
        ...
        if (msg.value != 0) {
            IWETH(payable(WMATIC)).deposit{value: ETHAmount}();
        }
        require(
            IWETH(payable(WMATIC)).balanceOf(address(this)) >= ETHAmount,
            "Ethereum not deposited"
        );
        IWETH(payable(WMATIC)).transfer(msg.sender, ETHAmount);
        IWETH(payable(WMATIC)).transferFrom(
            address(msg.sender),
            address(this),
            ETHAmount
        );
        emit Deposit(msg.sender, WMATIC, ETHAmount);
    }

    function deposit(
        address token,
        uint256 amount
    ) external checkAmount(amount) whenNotPaused {
    ...
    function withdraw(
        ...
        if (token == WMATIC) {
            require(
                IWETH(payable(WMATIC)).balanceOf(address(this)) >= amount,
                "insufficient balance"
            );

            if (verify(signatures, message)) {
                txHashs[txhash] = true;
                IWETH(payable(WMATIC)).withdraw(amount);
                payable(msg.sender).transfer(address(this).balance);
                emit Withdraw(msg.sender, WMATIC, amount, message);
            } else {
