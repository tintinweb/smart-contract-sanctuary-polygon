...
contract FeeManager is ReentrancyGuard {
    ...
    /*** STATE VARIABLES ***/
    address private immutable i_protocolManager;
    ...
    constructor(
        ...
        i_protocolManager = _protocolManager;
    ...
    function collectFee(uint256 depositAmount, address tokenAddress) external returns (uint256) {
        require(depositAmount != 0, "FeeManager: Zero amount");

        uint256 realAmount = (depositAmount * s_transactionFee) / 100;
    ...
    function setNewFeeManager(address _newManager) external onlyProtocolManager {
        require(
            _newManager != address(0x00) &&
                _newManager != address(this) &&
                _newManager != msg.sender,
     ...
     */
    function setNewTransactionFee(
        uint8 _newTransactionFee
    ) external onlyFeeManager onlyProtocolManager {
        require(_newTransactionFee != 0, "FeeManager: Invalid fee");
