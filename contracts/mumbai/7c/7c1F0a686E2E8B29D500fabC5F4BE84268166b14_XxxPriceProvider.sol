...
contract XxxPriceProvider is PriceOracle {
    ...
    mapping(address => AggregatorV3Interface) public assetsPriceSources;
    bool public paused = false;
    string public usdtSymbol;
    ...
    event PriceAlert(address priceAdmin, address asset, uint256 previousPriceMantissa, uint256 newPriceMantissa);
    event AssetPriceSourceUpdated(address indexed asset, address indexed source);

    modifier onlyGovernance {
        require(msg.sender == governance, "only governance may call");
        _;
    }

    modifier onlyPriceAdmin {
        require(priceAdmin[msg.sender], "only price admin may call");
        _;
    }
    ...
    constructor(string memory _usdtSymbol) public {
        governance = msg.sender;
        priceAdmin[governance] = true;
        usdtSymbol = _usdtSymbol;
    }

    function getUnderlyingPrice(CToken cToken) external view returns (uint) {
        //eToken设置价格小数点=36-标的资产小数位 bsc网络的usdt小数点为18
        uint256 price = 0;

        address underlying = address(0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE);
        string memory symbol = cToken.symbol();
        if (compareStrings(symbol, usdtSymbol)) {
            return 1e18;
        }else if (!compareStrings(symbol, "eBNB")) {
            underlying = address(CErc20(address(cToken)).underlying());
        }

        address asset = underlying;
        AggregatorV3Interface priceSource = assetsPriceSources[asset];
        price = getChainlinkPrice(priceSource);
        if (price > 0) {
            return price;
        } else{
           // price = getCustomerPrice(asset);
        }
    ...
    function getChainlinkPrice(AggregatorV3Interface priceSource) public view returns (uint256) {
        // Chainlink USD-denominated feeds store answers at 8 decimals
        uint256 decimalDelta = uint(18).sub(priceSource.decimals());
    ...
    function getCustomerPrice(address _asset) public view returns (uint256) {

        uint256 price = assetPrices(_asset);
        if (price > 0) {
            //return: (price / 1e8) * (1e36 / baseUnit) ==> price * 1e28 / baseUnit
            return uint(price).mul(1e28).div(1e18);
        } else {
            return 0;
        }
    }

    function isValidPrice(address _asset, uint256 _price) public view returns (bool) {
        // initial price is 0
        if (prices[_asset].price == 0) return true;

        uint256 min = prices[_asset].price.div(2);
        uint256 max = prices[_asset].price.mul(2);
    ...
    function setAssetSources(address[] calldata assets, address[] calldata sources) external onlyGovernance {
        _setAssetsSourcesInternal(assets, sources);
    }

    function _setAssetsSourcesInternal(address[] memory assets, address[] memory sources) internal {
        require(assets.length == sources.length, 'Inconsistent parameter length');
        for (uint256 i = 0; i < assets.length; i++) {
            assetsPriceSources[assets[i]] = AggregatorV3Interface(sources[i]);
            emit AssetPriceSourceUpdated(assets[i], sources[i]);
        }
    }

    function setDirectPrice(address _asset, uint256 _price) public onlyPriceAdmin whenNotPaused {
        uint256 previousPrice = prices[_asset].price;
    ...
    function setPrice(address _asset, uint256 _price) private onlyPriceAdmin {
        uint256 previousPrice = prices[_asset].price;
        uint256 newPrice = _price;
        prices[_asset] = PriceInfo(newPrice, block.timestamp, msg.sender);
        emit PricePosted(_asset, previousPrice, newPrice,  prices[_asset].price);
    ...
    function getPriceInfo(address asset) external view returns (uint256 price, uint256 updatedAt, address reportedBy) {
