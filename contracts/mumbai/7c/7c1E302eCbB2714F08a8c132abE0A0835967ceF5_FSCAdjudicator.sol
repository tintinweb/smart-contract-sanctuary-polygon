...
// content bonder funds are accounted for in the bonder structure as freeBondAmount until an adjudication is underway, and then
// they are accounted for (still in the bonder structure) as inAdjudicationAmount. once a contentBond is challenged, the adjudication
// is underway; and from that time the bonder can no longer withdraw his contentBond, until the end of the adjudication.
//
// the challenger bond funds are managed differently: challenger funds are immiediatly moved into the content structure and accounted
// as content.holdoverAmount. from here funds are deducted as they are distributed for eg. LINK fees. note that at the end of the
// adjudication, in thise cases in which the content was judged to be not-free-speech, the content-bonder funds are also moved from
// the bonder.inAdjudicationAmount into the content.holdoverAmount. regardless of the outcome of the adjudication, the vindication
// award, challenge award, lottery award and and revealer award are all paid out if the content.holdoverAmount.
//
...
contract FSCAdjudicator is AccessControl, VRFConsumerBase {
  ...
  // etherscan.io's Event Log API does not have an option to match multiple values in an individual topic. so in order to match any one of three
  // message hash's we duplicate the content hash into 3 topic positions
  event ContentBondEvent(uint256 indexed _hash1, uint256 indexed _hash2, uint256 indexed _hash3, bytes title, bytes comment);
  event ChallengeBondEvent(uint256 indexed _hash1, uint256 indexed _hash2, uint256 indexed _hash3, bytes title, bytes comment);
  ...
  // return values for isContentBonded(uint256 contentHash)
  uint8   constant NOT_BONDED              = 0x00;
  uint8   constant PROVISIONALLY_BONDED    = 0x01;
  uint8   constant IS_BONDED               = 0x02;
  ...
  uint8   constant VERDICT_FREE_SPEECH     = 0x05;
  ...
  // dividend/burn: 3.9 to 8.9 ether
  ...
  function _postBond(Bonder storage _bonder, uint256 amount)
    internal
  {
    ...
    public
  {
    ...
    emit ContentBondEvent(contentHash, contentHash, contentHash, title, comment);
  }
  ...
  function postChallengeBond(uint256 contentHash, address bonderAddr, bytes memory title, bytes memory comment)
    public
  {
    Content storage _content = contents[contentHash];
    ...
    emit ChallengeBondEvent(contentHash, contentHash, contentHash, title, comment);
  ...
  // same as postContentBond, but approval provided via draft-IERC20Permit
  function postChallengeBondAuthorized(uint256 contentHash, address bonderAddr, bytes memory title, bytes memory comment,
				       uint256 deadline, uint8 v, bytes32 r, bytes32 s)
    external
  {
    fscToken.permit(msg.sender, address(this), challengeBondAmount, deadline, v, r, s);
    postChallengeBond(contentHash, bonderAddr, title, comment);
  }
    ...
    onlyRole(REVEALER_ROLE)
  {
    ...
    uint8 jurorVote = ((certifiedVote & VOTE_FREE_SPEECH_MASK) == VOTE_FREE_SPEECH_MASK) ? VERDICT_FREE_SPEECH : VERDICT_NOT_FREE_SPEECH;
