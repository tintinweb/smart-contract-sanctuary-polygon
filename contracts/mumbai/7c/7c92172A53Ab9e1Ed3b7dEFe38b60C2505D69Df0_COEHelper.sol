...
contract COEHelper {
  ...
  function purchaseWithToken(
    uint256 _totalCost, //in USD from the pack struct * amount of packs purchasing - fixed point with 3 decimal places
  ...
  function editPaymentTokenByAddress(
    ...
    // Edit the price feed and token in the mapping
    priceFeeds[_tokenAddress] = AggregatorV3Interface(_priceFeedAddress);
    //if the token is not in the allowedTokens array, add it
    if (!checkAllowedToken(_tokenAddress)) {
      allowedTokens.push(_tokenAddress);
    }
  }

  //TODO: REMOVE PAYMENT TOKEN
  function removePaymentToken(address _tokenAddress) public onlyAuthed {
    // Remove the price feed and token from the mapping
    delete priceFeeds[_tokenAddress];
    //if the token is in the allowedTokens array, remove it
    if (checkAllowedToken(_tokenAddress)) {
      for (uint256 i = 0; i < allowedTokens.length; i++) {
        if (allowedTokens[i] == _tokenAddress) {
