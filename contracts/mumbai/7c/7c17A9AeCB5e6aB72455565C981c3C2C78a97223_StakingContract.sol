...
contract StakingContract is Ownable {
    ...
    address public rtoken = 0xE137BBF9A0444316c0d20EF05a2173bdb0E27510;
    address public goldenAddress = 0x624756418495F737729864CCd5704Ec0D947A48C;
    ...
    function changeAPETokenAddress(address _apeTokenAddress) public onlyOwner {
        goldenAddress = _apeTokenAddress;
    }
    ...
    function stake(uint256[] memory tokenIds) public {
        for(uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if(IERC721(goldenAddress).ownerOf(tokenIds[i]) != msg.sender) continue;

            IERC721(goldenAddress).transferFrom(address(msg.sender), address(this), tokenIds[i]);
    ...
    function unstake(uint256[] memory tokenIds) public {
        ...
        for(uint256 i = 0; i < tokenIds.length; i++) {
            (bool _isStaked,) = getStakingItemInfo(msg.sender, tokenIds[i]);
            if(!_isStaked) continue;
            if(IERC721(goldenAddress).ownerOf(tokenIds[i]) != address(this)) continue;
            ...
            if(stakingAmount[msg.sender] > 0)
                stakingAmount[msg.sender] = stakingAmount[msg.sender] - 1;
            IERC721(goldenAddress).transferFrom(address(this), msg.sender, tokenIds[i]);
