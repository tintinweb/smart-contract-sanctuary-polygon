...
interface ILiquidityV1 {
    ...
    function payForBet(uint256 _amount) external returns (bool);
    ...
    function getCoreContractAddress() external returns (address);
    function setCoreContractAddress(address _core) external returns (address);
...
//import "https://github.com/OpenZeppelin/openzeppelin-contracts/blob/release-v4.9/contracts/proxy/utils/UUPSUpgradeable.sol";
...
handleTax
triggerTax
setCoreContractAddress
*/
//TODO: revisar modificadores de acceso en las funciones

contract LiquidityV1 is UUPSUpgradeable, OwnableUpgradeable {

	modifier onlyManager() {
        require(managers[0] == msg.sender || managers[1] == liquidityTaxWallet || managers[2] == administrationTaxWallet, "Core: Not manager");
        _;
    }
	
    //ERC20 public immutable ERC20TokenAddress;
	ERC20 public ERC20TokenAddress;
	uint256 public ERC20TokenDecimals;
	uint256 private _approveMax;
    ...
    address public administrationTaxWallet;
	...
	function initialize(address _token) public initializer {
		...
		/*ERC20TokenAddress = ERC20(0x0FA8781a83E46826621b3BC094Ea2A0212e71B23); //0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;
		ERC20TokenDecimals = ERC20TokenAddress.decimals();//_transferOwnership(msg.sender);
		_approveMax = 10000000*(10**ERC20TokenDecimals);*/

		ERC20TokenAddress = ERC20(_token);//ERC20(0x7c9F903c888d2B76BbeaCB11E292b8410eD41619);//ERC20(0x0FA8781a83E46826621b3BC094Ea2A0212e71B23); //0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;
		ERC20TokenDecimals = ERC20TokenAddress.decimals();//_transferOwnership(msg.sender);
		_approveMax = 10000000*(10**ERC20TokenDecimals);

		liquidityTaxWallet = msg.sender;
    	administrationTaxWallet = msg.sender;
		
		managers[0] = msg.sender;
		managers[1] = liquidityTaxWallet;
	...
	/**
	* @dev Adds ERC20 tokens liquidity to this contract.
	*/
	function addLiquidity(uint256 _amount) public /*external*/ returns (bool) {
		//require(ERC20TokenAddress.allowance(msg.sender, address(this)) >= _amount, "Not enough allowance");
		ERC20TokenAddress.transferFrom(msg.sender, address(this), _amount);
		return true;
	}
	...
	*/
	function withdrawLiquidity(uint256 _amount) public /*external*/ returns (bool) {
		require(ERC20TokenAddress.allowance(address(this), msg.sender) >= _amount, "Not enough allowance");
		ERC20TokenAddress.transferFrom(address(this), msg.sender, _amount);
		return true;
	}

	/**
	* @dev Sets `_amount` as the allowance of bettor over the contract's tokens.
	*/
	function _approveWithdrawal(uint256 _amount) private returns (bool) { //Llamar cuando reclame sus USDCs desde otra funcion de este contrato
		ERC20TokenAddress.approve(msg.sender, _amount);
		return true;
	}

	/**
	* @dev Withdraws `_amount` from contract to bettor.
	*/
	function withdraw(uint256 _amount) public /*external*/ /*¿onlyProxy?*/ returns (bool) {
		ERC20TokenAddress.transferFrom(address(this), msg.sender, _amount);
		return true;
	}
	
	/**
	* @dev Pays to this contract in order to make a bet.
	*/
	function payForBet(uint256 _amount) external /*¿onlyProxy?*/ returns (bool) {
		//require(ERC20TokenAddress.allowance(msg.sender, address(this), _amount) >= _amount, "Not enough allowance");
		if(ERC20TokenAddress.allowance(msg.sender, address(this)) < _amount){
			//_approveContract();
			return false;
		}else{
			ERC20TokenAddress.transferFrom(msg.sender, address(this), _amount);
			//Comprobar que se ha enviado los tokens a este contract??
			//TODO: Make bet calling Core contract
		}
		return true;
	}
	
	/**
	* @dev Pays won bets to bettor.
	*/
	function redeemWonBets() external /*¿onlyProxy?*/ returns (bool) {
		//TODO
		//_coreAddress.
	}
	
	/**
	* @dev Calculates the tax, transfer it to the contract. If the bettor is betting, and the amount threshold is met, it executes the tax.
	*/
	function handleTax(address from, address to, uint256 amount) private returns (uint256) { //TODO        
		/*if(!isExcluded(from)) {
			uint256 tax;
	...
	*/
	...
	function setCoreContractAddress(address _core) public onlyManager returns (address) {
		_coreAddress = _core;
		return _coreAddress;
	}
	
	/**
	* @dev Sets tax for payouts.
	*/
	function setPayoutTax(uint256 _liquidity, uint256 _administration) public /*onlyOwner*/ {
		payoutTaxes["liquidity"] = _liquidity;
		payoutTaxes["administration"] = _administration;
	}
	
	/**
	* @dev Sets wallets for taxes.
	*/
	function setTaxWallets(address _liquidity, address _administration) public /*onlyOwner*/ {
