	...
	UUPSUpgradeable
{
	...
	struct SalePhase {
        uint256 category;
        uint256 salesType;      // 1: Fixed, 2: Dutch Auction(percent decrease), 3: Dutch Auction(linear decrease)
		uint256 startDate;
		uint256 endDate;
		uint256 mintPrice;
		uint256 maxPool;
    ...
    // total salePhase count
    uint256 public totalSalePhaseCount;
    // creator faktura payment address
    address private creatorFakturaAddress;
    // mapping to all categories
    mapping(uint256 => Category) public categories;
    // mapping to all salePhases
	mapping(uint256 => SalePhase) public salePhases;
	// mapping to rootHash for salePhase
    ...
    function initialize(
        ...
        for (uint8 index = 0; index < _salesPhases.length; index++) {
            salePhases[index] = _salesPhases[index];
        }
        ...
        // set the total categories count
        totalCategoryCount = _categories.length;
        // set the total salephases count
        totalSalePhaseCount = _salesPhases.length;
        // set the Creator Faktura Address
        creatorFakturaAddress = _creatorFakturaAddress;

        require(maxSupply > 0,
            "maxSupply: should be higher than 0"
        );
    }
    ...
    function getCategory(uint256 _cat) public view returns (Category memory) {
        return categories[_cat];
    }

    function getSalePhase(uint256 _sales) public view returns (SalePhase memory) {
        return salePhases[_sales];
    }

    function getPrice(uint8 n, uint256 sales) public view returns (uint256) {
        return n * salePhases[sales].mintPrice;
    }
    ...
    function getMintedBySales(uint256 sales) public view returns (uint256) {
        return _totalMintedBySales(sales);
    }

    function getMintedByCategory(uint256 cat) public view returns (uint256) {
        return _totalMintedByCategories(cat);
    }

    function safeMint(address to, uint8 amount, uint256 sales, uint256 category, bytes32[] memory _merkleProof) public payable whenNotPaused {
        require(
        	category < totalCategoryCount,
        	"Category not exists"
        );
        require(
            salePhases[sales].salesType > 0,
            "SalePhases not exist."
        );
        require(
            salePhases[sales].startDate <= block.timestamp,
            "SalePhases not started."
        );
        require(
            salePhases[sales].endDate > block.timestamp || 
            salePhases[sales].endDate == 0,
            "SalePhases finished."
        );
        require(
            _totalMinted() + amount <= maxSupply,
            "Collection soldout or invalid amount"
        );
        require(
        	_totalMintedByCategories(category) + amount <= categories[category].tokenSupply,
        	"Category soldout or invalid amount"
        );
        require(
            _totalMintedBySales(sales) + amount <= salePhases[sales].maxPool || 
            salePhases[sales].maxPool == 0,
            "SalePhases soldout or invalid amount"
        );
        require(
            _totalMintedByAddress(to, sales) + amount <= salePhases[sales].maxPerWallet || 
            salePhases[sales].maxPerWallet == 0,
            "maxPerWallet limit"
        );
        require(
            msg.value >= getPrice(amount, sales),
            "Insufficient mint price"
        );

        /*
        if(salePhases[sales].hasWhitelist) {
        ...
        */
        ...
        if (salePhases[sales].hasWhitelist) {
        	...
        	require(MerkleProof.verify(_merkleProof, rootHashes[sales], leaf), "Incorrect proof");
        }
    ...
    function setCategory(uint256 _cat, uint256 _tokenSupply, string calldata _name) external onlyRole(ADMIN_ROLE) {
        require(_cat <= totalCategoryCount, "Invalid category index");
        require(_tokenSupply > 0, "Invalid token supply");

        categories[_cat].name = _name;
    	categories[_cat].tokenSupply = _tokenSupply;

        if (_cat == totalCategoryCount) {
            totalCategoryCount = _cat + 1;
        }
    }

    function setSalePhase(uint256 _sales, SalePhase calldata _salePhase) external onlyRole(ADMIN_ROLE) {
        require(_sales <= totalSalePhaseCount, "Invalid salephase index");
        require(_salePhase.category < totalCategoryCount, "Invalid category");

        salePhases[_sales] = _salePhase;

        if (_sales == totalSalePhaseCount) {
            totalSalePhaseCount = _sales + 1;
        }
    }

    function setMerkleTreeRoot(uint256 _sales, bytes32 _rootHash) external onlyRole(ADMIN_ROLE) {
    	rootHashes[_sales] = _rootHash;
    }
