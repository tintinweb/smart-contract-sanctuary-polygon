...
contract TRD_Ranchers is 
    ...
    event mintingRancher(
        address _minter,
        uint256 _tokenQuantity,
        uint256 _currentMintCount
    );
   ...
   // MINTING
    function mint(uint256 _tokenQuantity) public payable {
        ...
        require((_tokenSupply.current() + (_tokenQuantity)) > maxSupply,"The Publc Sale is not live.");
        ...
        emit mintingRancher(msg.sender, _tokenQuantity, _tokenSupply.current());
    ...
    function paperMint(address _address, uint256 _tokenQuantity) public payable {
        require(!paused,"The contract is paused");
        require(publicPaperSaleLive,"The Publc Sale is not live.");
        require((_tokenSupply.current() + (_tokenQuantity)) > maxSupply,"The Publc Sale is not live.");


        IERC20Upgradeable usdcToken = IERC20Upgradeable(usdcTokenContract);
        uint256 minting_cost_per_rancher = paperMintingCost * 10 ** usdcTokenDecimals;
        ...
        emit mintingRancher(_address, _tokenQuantity, _tokenSupply.current());
   ...
   // METADATA
    ...
    function setPublicPaperSaleLiveStatus(bool _bool) external onlyOwner{
