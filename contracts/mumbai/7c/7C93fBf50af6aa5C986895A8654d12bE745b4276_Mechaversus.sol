...
contract Mechaversus is Initializable, OwnableUpgradeable {
    ...
    modifier callerIsPlayer( uint256 match_id ) {
        bool founded = false;
        for( uint256 i = 0; i < PLAYERS_COUNT; i++) founded = founded || _matches[ match_id ]._activePlayers[i] == msg.sender;
    ...
    function getMatchArena( uint256 match_id ) public view returns ( uint256 ) {
        return _matches[ match_id ]._arenaId;
    }

    function getMatchActivePlayers( uint256 match_id ) public view returns ( address[] memory ) {
        return _matches[ match_id ]._activePlayers;
    }

    function getMatchPlayers( uint256 match_id ) public view returns ( address[] memory ) {
        return _matches[ match_id ]._players;
    }

    function getMatchDecks( uint256 match_id ) public view returns ( uint256[] memory ) {
        uint256[] memory emptyUint256s;
        return _matches[ match_id ]._performersOrder.length == PLAYERS_COUNT * DECK_SIZE
        ? _matches[ match_id ]._decks : emptyUint256s;
    }

    function getMatchStakeAmount( uint256 match_id ) public view returns ( uint16 ) {
        return _matches[ match_id ]._stakeAmount;
    }

    function getMatchPerformersOrder( uint256 match_id ) public view returns ( uint256[] memory ) {
        return _matches[ match_id ]._performersOrder;
    }

    function getMatchActionCount( uint256 match_id ) public view returns ( uint256 ) {
        return _matches[ match_id ]._lastActionId;
    }

    function isMatchHalted( uint256 match_id ) public view returns ( bool ) {
        return _matches[ match_id ]._halted;
    }

    function getMatchWinner( uint256 match_id ) public view returns ( address ) {
        return _matches[ match_id ]._winner;
    }

    function getMatchActions( uint256 match_id ) public view returns ( Action[] memory ) {
        Action[] memory actions = new Action[]( _matches[ match_id ]._lastActionId );
        for ( uint256 i = 0; i < _matches[ match_id ]._lastActionId; i++ ){
            actions[i] = _matchActions[ match_id ][i+1];
        }
        return actions;
    }

    function getMatchAction( uint256 match_id, uint256 action_id ) public view returns ( Action memory ) {
        return _matchActions[ match_id ][ action_id ];
    }

    function isDeckUsed( uint256 deck_id ) public view returns ( bool ) {
        return bool( _usedDeck[ deck_id ] );
    }

    function getEntityEP( uint256 match_id, uint256 entity_id ) public view returns ( int256 ) {
        return _entities[ match_id ][ entity_id ];
    }

    // @dev Generate a Match with a stake amount and the block until player can respond
    function generateMatch( uint16 stake_amount, uint256 block_to_respond ) public virtual {

        IERC20Upgradeable _Mechadium = _Arenas._Decks()._Mechas()._Cores()._Industries()._Mechadium();

        require( stake_amount >= MIN_STAKE_AMOUNT, "Stake amount lower than minimum required for generate Match" );
        require( _Mechadium.balanceOf( msg.sender ) >= stake_amount, "Insufficient Mechadium balance for create a Match with this stake amount" );
        ...
        emit GenerateMatch( matchId, arenaId, msg.sender, stake_amount, block_to_respond );
    }

    // @dev Destroy an empty Match if you are its creator or the blocks of Match life are passed
    function destroyMatch( uint256 match_id ) public virtual {

        address creator = _matches[ match_id ]._creator;

        require( _matches[ match_id ]._startBlock + MATCH_LIFE_BLOCKS <= block.number ||
            creator == msg.sender, "Only the creator can Destroy this Match before its min lifetime runs out" );
        require( _matches[ match_id ]._players.length == 0 , "All players should leave before destroy this Match" );

        delete _matches[ match_id ];
        _activeMatch[ creator ] = Arrays.popUint256( _activeMatch[ creator ], match_id );

        emit DestroyMatch( match_id );
    }

    // @dev The Player can join the Match before it started by sending stake amount of Mechadium
    function joinMatch( uint256 match_id, uint256 deck_id ) public virtual {

        Decks _Decks = _Arenas._Decks();
        IERC20Upgradeable _Mechadium = _Decks._Mechas()._Cores()._Industries()._Mechadium();

        require( _matches[ match_id ]._winner == address(0x0) , "This Match is already ended" );
        require( match_id < _matchCounter.current(), "This Match does not exist");
        require( !_usedDeck[ deck_id ] , "This Deck is already used in another Match");
        require( _matches[ match_id ]._players.length < PLAYERS_COUNT, "All Players have already join this Match" );
        _assetsOwnershipChecks( deck_id );

        // @dev Player send Mechadium to the reserve
        require( _Mechadium.allowance( msg.sender, mechadium_reserve ) >= _matches[ match_id ]._stakeAmount,
            'Mechaversus should be approved on MECHA from all players');
        _Mechadium.transferFrom( msg.sender, mechadium_reserve, _matches[ match_id ]._stakeAmount );

        _matches[ match_id ]._activePlayers.push( msg.sender );
        _matches[ match_id ]._players.push( msg.sender );
        _matches[ match_id ]._decks.push( deck_id );
        _usedDeck[ deck_id ] = true;

        emit JoinMatch( match_id, _matches[ match_id ]._arenaId, msg.sender, _matches[ match_id ]._stakeAmount );
    }

    // @dev The Player can leave the Match before it started with unstake of Mechadium amount
    function leaveMatch( uint256 match_id ) public virtual callerIsPlayer(match_id){

        IERC20Upgradeable _Mechadium = _Arenas._Decks()._Mechas()._Cores()._Industries()._Mechadium();

        require( _matches[ match_id ]._performersOrder.length == 0, "Players cannot Leave a started Match" );

        // @dev Reserve resend Mechadium to the player
        _Mechadium.transfer( msg.sender, _matches[ match_id ]._stakeAmount );

        for( uint256 i = 0; i < _matches[ match_id ]._players.length; i++ ){
            if( _matches[ match_id ]._players[i] == msg.sender ) {
                _matches[ match_id ]._activePlayers = Arrays.popAddress( _matches[ match_id ]._activePlayers, _matches[ match_id ]._activePlayers[i] );
                _matches[ match_id ]._players = Arrays.popAddress( _matches[ match_id ]._players, _matches[ match_id ]._players[i] );
                _usedDeck[ _matches[ match_id ]._decks[i] ] = false;
                _matches[ match_id ]._decks = Arrays.popUint256( _matches[ match_id ]._decks, _matches[ match_id ]._decks[i] );
            }
        }

        emit LeaveMatch( match_id, msg.sender );
    }

    // @dev The Player with fastest Mecha should set the actions order before Match starting
    function setPerformersOrder( uint256 match_id, uint256[] memory performers_order ) public virtual callerIsPlayer(match_id){

        require( _matches[ match_id ]._performersOrder.length == 0, "Performers order already set in this Match");
        require( performers_order.length == PLAYERS_COUNT * DECK_SIZE, "Performers order provided have a wrong length");

        uint256 i = 0;
        uint256 j = 0;
        while( i < performers_order.length ){
            require( performers_order[i] != performers_order[j] || i == j, "Duplicated Performer id founded in order array" );
            i++;
            if( i == performers_order.length - 1 ){
                j++;
                i = j;
            }
        }

        _matches[ match_id ]._performersOrder = performers_order;

        emit StartMatch( match_id, performers_order, _matches[ match_id ]._decks );
    }

    // @dev Every player turn the Match is updated with the action performed
    function updateMatch( uint256 match_id, uint256 performer_id, uint8[] memory action_type, bytes2[] memory coordinates,
        uint256[] memory target_ids, int256[] memory target_eps ) public virtual callerIsPlayer(match_id){

        require( !_matches[ match_id ]._halted, "This Match is Halted for suspicious activities" );
        require( _matches[ match_id ]._performersOrder.length == PLAYERS_COUNT * DECK_SIZE , "Players cannot update Match before it has started");
        require(
            action_type.length == coordinates.length &&
            action_type.length == target_ids.length &&
            action_type.length == target_eps.length,
            "Arrays lengths mismatch"
        );
        require( performer_id == _getNextPerformerId( match_id ), "Performer provided is not the next in Match order");

        for( uint256 i = 0; i < action_type.length; i++){

            require( coordinates[i] <= ARENA_SIZE, "Coordinate provided should be inside of Arena" );

            uint256 actionId = _matches[ match_id ]._lastActionId + 1;
            _matches[ match_id ]._lastActionId = actionId;
            _entities[ match_id ][ target_ids[i] ] = target_eps[i];

            _matchActions[ match_id ][ actionId ] = Action({
                _player: msg.sender,
                _block: block.number,
                _performerId: performer_id,
                _type: action_type[i],
                _coordinates: coordinates[i],
                _targetId: target_ids[i],
                _targetEP: target_eps[i]
            });
        }

        emit UpdateMatch( match_id, msg.sender, performer_id, action_type, coordinates, target_ids, target_eps);
    }

    // @dev Freeze the Match for suspicious activities
    function haltMatch( uint256 match_id ) public virtual callerIsPlayer(match_id){

        require( _matches[ match_id ]._performersOrder.length == PLAYERS_COUNT * DECK_SIZE , "Players cannot Halt the Match before starting" );

        uint256 lastActionId = _matches[ match_id ]._lastActionId;

        require( _matchActions[ match_id ][ lastActionId ]._player != msg.sender, "Last action performer cannot call halt match" );
        _matches[ match_id ]._halted = true;

        emit HaltMatch( match_id, msg.sender );
    }

    // @dev Check if the end Match condition rules are satisfied
    function surrendMatch( uint256 match_id ) public virtual callerIsPlayer(match_id){

        require( _matches[ match_id ]._performersOrder.length == PLAYERS_COUNT * DECK_SIZE , "Players cannot surrender Match before it has started");

        _loseMatch( match_id, msg.sender );

        emit SurrendMatch( match_id, msg.sender );
    }

    // @dev Check if the end Match condition rules are satisfied
    function endMatchConditionsChecks( uint256 match_id ) public virtual callerIsPlayer(match_id) {

        Decks _Decks = _Arenas._Decks();
        uint256 lastActionId = _matches[ match_id ]._lastActionId;

        require( _matches[ match_id ]._performersOrder.length == PLAYERS_COUNT * DECK_SIZE , "Players cannot end Match before it has started");
        require( _matchActions[ match_id ][ lastActionId ]._player != msg.sender
            || _matchActions[ match_id ][ lastActionId ]._block + _matches[ match_id ]._blockToRespond >= block.number,
            "Cannot call this checks before blocks to respond are passed");

        for( uint256 i = 0; i < _matches[ match_id ]._activePlayers.length; i++){
            uint256 idx = 0;
            for( uint256 j = 0; j < _matches[ match_id ]._players.length; j++) {
                if( _matches[ match_id ]._players[j] == _matches[ match_id ]._activePlayers[i] ) idx = j;
            }

            uint256 nexus_id = _Decks.getNexusId( _matches[ match_id ]._decks[ idx ] );
            if( _entities[ match_id ][ nexus_id ] == int256(-1) ) _loseMatch( match_id, _matches[ match_id ]._players[ idx ] );

            bool allMechaBroken = true;
            uint256[] memory mechas_ids = _Decks.getMechaIds( _matches[ match_id ]._decks[ idx ] );
            for( uint256 j = 0; j < DECK_SIZE; j++){
                allMechaBroken = allMechaBroken && _entities[ match_id ][ mechas_ids[j] ] == int256(-1);
            }
            if( allMechaBroken ) _loseMatch( match_id, _matches[ match_id ]._players[ idx ] );
        }
    }

    function _loseMatch( uint256 match_id, address player ) internal virtual {

        _matches[ match_id ]._activePlayers = Arrays.popAddress( _matches[ match_id ]._activePlayers, player );
        if( _matches[ match_id ]._activePlayers.length == 1 ) _endMatch( match_id, _matches[ match_id ]._activePlayers[0] );
    }

    function _endMatch( uint256 match_id, address winner ) internal virtual {

        _matches[ match_id ]._winner = winner;

        _activeMatch[ _matches[ match_id ]._creator ] = Arrays.popUint256( _activeMatch[ _matches[ match_id ]._creator ], match_id );

        for( uint256 i = 0; i < _matches[ match_id ]._decks.length; i++ ) _usedDeck[ _matches[ match_id ]._decks[i] ] = false;

        _updateBreakingLoads( match_id );

        _rewardsDistribution( match_id, _matches[ match_id ]._arenaId, winner );

        emit EndMatch( match_id, winner );
    }

    function _getNextPerformerId( uint256 match_id ) internal view virtual returns ( uint256 ){

        uint256 nextPerformerId = 0;
        uint256 lastActionId = _matches[ match_id ]._lastActionId;
        if ( lastActionId == 0 ) nextPerformerId = _matches[ match_id ]._performersOrder[0];

        for (uint256 i = 0; i < _matches[ match_id ]._performersOrder.length; i++) {
            if ( _matches[ match_id ]._performersOrder[i] == _matchActions[ match_id ][ lastActionId ]._performerId ){
                if ( i == _matches[ match_id ]._performersOrder.length - 1 ) nextPerformerId = _matches[ match_id ]._performersOrder[ 0 ];
                else nextPerformerId = _matches[ match_id ]._performersOrder[ i + 1 ];
                break;
            }
        }
    ...
    function _assetsOwnershipChecks( uint256 deck_id ) internal view {
    ...
    function _rewardsDistribution( uint256 match_id, uint256 arena_id, address winner ) internal {

        IERC20Upgradeable _Mechadium = _Arenas._Decks()._Mechas()._Cores()._Industries()._Mechadium();

        uint256 amount = _matches[ match_id ]._stakeAmount * PLAYERS_COUNT;
    ...
    function _updateBreakingLoads( uint256 match_id ) internal {
        ...
        for( uint256 i = 0; i < PLAYERS_COUNT; i++){
            ...
            address player = _matches[ match_id ]._players[i];
            uint256 degradationReduction = player == _matches[ match_id ]._winner ? DEGRADATION_REDUCTION : 1;
            uint256 nexusErosion = NEXUS_DEGRADATION / degradationReduction;
            uint256 mechaErosion = ( MECHA_DEGRADATION + _matches[ match_id ]._lastActionId ) / degradationReduction;

            uint256 nexus_id = _Decks.getNexusId( _matches[ match_id ]._decks[i] );
            _Industries.updateBreakingLoad( nexus_id, nexusErosion );

            uint256[] memory mechas_ids = _Decks.getMechaIds( _matches[ match_id ]._decks[i] );
            for( uint256 j = 0; j < DECK_SIZE; j++) {
                uint256 core_id = _Mechas.getMechaCoreId( mechas_ids[j] );
                if( _entities[ match_id ][ mechas_ids[j] ] == int256(-1) )
