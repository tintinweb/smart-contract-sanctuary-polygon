...
contract YexleAppian is ERC721Burnable, Ownable {
    ...
    */
    ...
    /** 
        ...
        *@param _oldOwner - address of the seller/owner of the land
    ...
    */
    function ownerDecisionforRaisedRequest(address _oldOwner, address _requester, uint _tokenId, bool _status) external onlyAdmin{
        UserContract useC = UserContract(userContract);
        (bool status) = useC.verifyUser(_requester);
        if(!status){ revert userContractError();}
        require(_oldOwner == _owners[_tokenId],"you are not the owner of nft");
