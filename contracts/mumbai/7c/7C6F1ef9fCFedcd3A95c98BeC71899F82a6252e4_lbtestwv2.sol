...
contract lbtestwv2 is VRFV2WrapperConsumerBase,ConfirmedOwner,ReentrancyGuard {
	using SafeMath for uint256;
    ...
    */
    ...
    uint16 percentageToNFTs = 95; // 9,50%

    uint256 prizeValueJackpot = btc.balanceOf(address(this)); // 47.5% of ticket bought - added every new buy - filled by prizeValue
    uint256 botReward = prizeValueJackpot.mul(percentageToNFTs).div(1000); // Default: 9,50% of weekly jackpot goes to NFTNode Golden 
        ...
        VRFV2WrapperConsumerBase(linkAddress, wrapperAddress) 
    {
        ...
        lbNodes = IluckblocksNodes(0x3A7ebDC5251D95BE2eA87c3815D5819d101cE22D);
    ...
    function getJackpotinUSD() public view returns (uint256) {
       ...
       uint256 result = ((jackpot * 10**10) * tprice) / 10**18; 
    ...
    function setNFTPay(uint16 _percentage) public onlyOwner() {
  ...
  function autoSpin(address _caller, uint8[] calldata _nodes) public nonReentrant {
        ...
        if(lbNodes.getUserActivation(_caller,1) == false){
            lbNodes.activateNodes(_caller,_nodes,1,1);
        }else{
            ...
            uint256 amountInUSD = getOutputAmount(btcTrading,usdTrading,botReward,0);            
            ...
            if(getJackpotinUSD() >= minimumPot && registeredsAccts.length > 0){ 
                ...
                if(block.timestamp > lastAutoSpin + 605100 && reset == false){   // if no node activates in 5 minutes period queue resets
                    lbNodes.resetQueue(1,1);
                    reset = true;
                } else if(lbNodes.updateNodeInfo(_caller,amountInUSD,1,1) == true){
    ...
    function registerTickets(uint256 quantity,address user) external {
         ...
         for(uint i = 0; i < quantity; i++){
            ...
            uint16 indexCheck = incrementTicketsIds;
            ...
            if (userTickets[requestCounter][user].length == 0) {
                userTickets[requestCounter][user] = new uint16[](0);
            }
        ...
        prizeValueJackpot = btc.balanceOf(address(this));
        botReward = prizeValueJackpot.mul(percentageToNFTs).div(1000); // Default: 9,50% of weekly jackpot goes to NFTNode Golden
    }
     ...
     */
     function fulfillRandomWords(uint256 requestId, uint256[] memory _randomWords) internal override {
        ...
        raffle = uint16(_randomWords[0] % registeredsAccts.length);
        ...
        emit LotteryLog(block.timestamp,userWallet, "We have a new Weekly Raffle Winner",prizePaid);
    ...
    function getLastUserTicket(uint256 drawId,address userWallet) internal view returns(uint16) {
    ...
    function ForfeitTicket(address user) external nonReentrant {
        ...
        uint16 ticketId = getLastUserTicket(requestCounter,user);
