...
contract FactoryFacet {
    ...
    constructor(){
        ...
        f.founderz[msg.sender] = true; 
        BigBoardManagerLib.setFactoryHostToken(0xA14e84B1B1Cdd66aDFD97e11B3cc4b75e9499116);

    }

    modifier onlyFounder {
        BigBoardManagerLib.onlyFounder();
        _;
    }

    function addNewFounder(address _newFounder) external onlyFounder {
        BigBoardManagerLib.Factory storage f  = BigBoardManagerLib.getFactory();
        f.founderz[_newFounder] = true; 
    }

    function setBBHToken(address _token) external onlyFounder {
        BigBoardManagerLib.setFactoryHostToken(_token);
    }
...
contract Board is Ownable{
                    ...
                    if(across[c] != game.score_home_by_period[q] % 10){
                        ...
                        for (uint r = 0; r < down.length; r++){
                            if(down[r] != game.score_away_by_period[q] % 10){
                                ...
                                if( squareBuyerz[selectedRow][selectedCell].purchaser != address(0x0) ){
                                    // get the purchased square that has a cell == across index and row == down index
                                    BigBoardManagerLib.Square memory winningSquare = squareBuyerz[selectedRow][selectedCell];
                                    winners[winningSquare.purchaser].purchaser = winningSquare.purchaser;
                                    winners[winningSquare.purchaser].totalWinnings = winners[winningSquare.purchaser].totalWinnings + forwardPrize;
                                    winners[winningSquare.purchaser].winningForwardSquarez.push(squareBuyerz[selectedRow][selectedCell]);
                                }
                                
                                // check for backward win
                                if( squareBuyerz[selectedCell][selectedRow].purchaser != address(0x0) ){
                                    // get the purchased square that has a cell == down index and row == across index
                                    BigBoardManagerLib.Square memory winningSquare = squareBuyerz[selectedCell][selectedRow];
                                    winners[winningSquare.purchaser].purchaser = winningSquare.purchaser;
                                    winners[winningSquare.purchaser].totalWinnings = winners[winningSquare.purchaser].totalWinnings + forwardPrize;
                                    winners[winningSquare.purchaser].winningForwardSquarez.push(squareBuyerz[selectedCell][selectedRow]);
                                }
                            }
                        }
    ...
    function collectPrize() public winnerOnly {
        ...
        if(prizeCollectedByWinner[msg.sender]){
            ...
            uint256 total = currentWinner.totalWinnings;
            if(total > 0 ){
                //
...
library BigBoardManagerLib {
    ...
    function onlyHost() internal view {
        Factory storage f = getFactory();
        require(f.hostToken.balanceOf(msg.sender) > 0);
    }

    function onlyFounder() internal view{
        Factory storage f = getFactory();
        require(f.founderz[msg.sender]);
    }

    function founderOrHostOnly() internal view{
        Factory storage f = getFactory();
        require(f.founderz[msg.sender] || f.hostToken.balanceOf(msg.sender) > 0);
