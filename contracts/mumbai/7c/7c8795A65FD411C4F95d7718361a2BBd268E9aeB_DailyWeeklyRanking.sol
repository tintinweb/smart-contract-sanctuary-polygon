...
contract DailyWeeklyRanking is Ownable{
    ...
    mapping(uint256 => ChallengeDatas[10]) private _pastsDailyChallenge;
    mapping(uint256 => uint256) private _dailyRewardsOfPastChallenge;
    mapping(uint256 => ChallengeDatas[10]) private _pastsWeeklyChallenge;
    ...
    function getPastDailyRanking(uint256 _dayNum) external view returns(ChallengeDatas[10] memory ranking, uint256 rewards){
        return (_pastsDailyChallenge[_dayNum],_dailyRewardsOfPastChallenge[_dayNum]); 
    }

    function getPastWeeklyRanking(uint256 _weekNum) external view returns(ChallengeDatas[10] memory ranking, uint256 rewards){
        return (_pastsWeeklyChallenge[_weekNum],_weeklyRewardsOfPastChallenge[_weekNum]); 
    }
    ...
    function _updateDayAndWeekBeginning() internal returns(bool){
        if(dayBeginingTimestamp != _getDayBegining()){
            ...
            _pastsDailyChallenge[_dayNumber] = _dailyChallenge;
            _dayNumber +=1;
            _payDailyWinners();
        }
        if(weekBeginingTimestamp != _getWeekBegining()){
            ...
            _pastsWeeklyChallenge[_weekNumber] = _weeklyChallenge;
            _weekNumber +=1;
            _payWeeklyWinners();
        }
        return true;
    ...
    function _payDailyWinners() internal {
        ...
        IPayments IPay =  IPayments(paymentAddress);
