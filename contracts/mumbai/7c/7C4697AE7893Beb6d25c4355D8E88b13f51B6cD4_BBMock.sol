...
contract BBMock is BaseBuildingBlock {
    ...
    constructor() {
        // remotes[11] = address ActionPool in Optimism Mainnet
    }
    ...
    function getData(uint16 recipientLZId,
                    ...
                    uint256 stableAmount) public view returns(bytes memory data) {
        data = abi.encode(recipientLZId,nativeStableToken, destinationStableToken, sgBridge, targetRouter, stableAmount);
    }

    
    function getData(address p0, uint256 p1) public view returns(bytes memory data) {
    ...
    function lzReceive(
        ...
        address destinationStableToken,
        address nativeStableToken) = abi.decode(actionData, (uint256, uint16, address, address, address));
