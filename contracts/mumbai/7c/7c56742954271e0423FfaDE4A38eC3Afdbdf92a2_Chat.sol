  ...
  //--------------------------------------------------------------------------------------   
   ...
   require(walletAddress != address(0), "User with given address does not exist.");
    ...
    function sendMessage(address receiver, string calldata subject, string memory message, string memory fileHash) external {
        ...
        function sendMessageToGroup(address[] memory receiver, string calldata subject, string []memory message, string memory fileHash) external {
        require(
            checkUserExists(msg.sender) == true,
            "You must have an account"
        );
        for(uint i = 0; i<receiver.length; i++){
            ...
            Message memory message = Message(msg.sender, receiver[i], subject, message[i], block.timestamp, false, fileHash);
        messages.push(message);
        }
