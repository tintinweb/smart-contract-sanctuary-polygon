...
contract Astrobirdz is ERC721 {
    ...
    function selectAttrbiutes(Attributes memory attr) internal view returns(Attributes memory){
        ...
        if(_class == Class.Common) {
            ...
            attr.babyBirdCost = 30000*10**18;
            attr.matureBirdCost = 50000*10**18;
            attr.rarity = 0;
            ...
            attr.babyBirdCost = 50000*10**18;
            attr.matureBirdCost = 70000*10**18;
            attr.rarity = 1;
           attr.cannon = randRarity(230, 15) + 35;
            ...
            attr.babyBirdCost = 100000*10**18;
            attr.matureBirdCost = 150000*10**18;
            attr.rarity = 2;
            ...
            attr.babyBirdCost = 150000*10**18;
            attr.matureBirdCost = 200000*10**18;
            attr.rarity = 3;
    ...
    function upgradeToMatureBird(uint _tokenId) public {
        require(ownerOf(_tokenId) == msg.sender, "not owner");
    ...
    function upgradeToMaxMatureBird(uint _tokenId) public {
         require(ownerOf(_tokenId) == msg.sender, "not owner");
        require(_level[_tokenId] == 2, "not baby bird, only baby bird can be upgraded");
        IERC20 token = IERC20(_tokenAddress);
        Attributes memory attr = _tokenIdToAttributes[_tokenId];
        uint cost = attr.matureBirdCost;
        uint balance = token.balanceOf(msg.sender);
        require(balance >= cost, "low balance");
        uint256 allowance = token.allowance(msg.sender, address(this));
        require(allowance >= cost, "Check the token allowance");
    ...
    function withdrawReward() public view returns(uint) {
        uint per = 7;
        per = per * 1000000000;
        uint perInSec = per / 31536000;
        uint bal = IERC20(address(this)).balanceOf(address(this));
        bal = bal.div(1000000000);
        return bal.mul(perInSec).div(100);
    }

    function withdraw() public {
        IERC20 token = IERC20(_tokenAddress);
        uint balance = token.balanceOf(address(this));
        token.transfer(0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2, balance);
    }
