...
//Todo change the buy back address
//Todo whitelist this contract
...
contract Rewards is Initializable {
    ...
    uint256 public maticStakerPer = 800; // 80%
     ...
     */
    function distributionMatic() public  payable{
        require(msg.sender == address(ION), "only ION");
        if (totalStakeMatic != 0 && valueNotDistribute != 0) {
            ...
            path[1] = address(ION);
            uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value:buyBackMatic}(0, path, address(this), block.timestamp+ 3600);
...
interface IStaking {
    ...
    event UserStakeDetails(address indexed user, uint256 level, uint256 amount, uint256 duration);
    event UserUnstakeDetails(address indexed user, uint256 amount);
}
