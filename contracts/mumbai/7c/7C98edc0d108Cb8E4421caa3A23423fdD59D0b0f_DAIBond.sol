...
contract DAIBond is AddressAccessor, Pausable {
    ...
    function claimable(address account) public view returns (uint256) {
        ...
        return v.fullyVestedAmount * vestingDuration / maturity;
    }
    ...
    function claim(uint256 amount) public {
        ...
        (address usdr, address susdr, address staking) = abi.decode(
        ...
        uint totalClaimed = v.claimed + amount;
        if (totalClaimed == v.fullyVestedAmount) {
            delete vesting[who];
        } else {
            vesting[who].claimed = totalClaimed;
        }
        uint256 balance = IERC20(susdr).balanceOf(address(this));
        if (balance >= amount) {
            IStaking(staking).unstake(amount);
        } else {
            if (balance > 0) {
                IStaking(staking).unstake(balance);
                IERC20(usdr).transfer(who, balance);
            }
            IMintableERC20(usdr).mint(who, amount - balance);
        }
    }
    ...
    function deposit(uint256 amount) external whenNotPaused {
        (address bonding, address underlying, address exchange, address usdr, address staking) = abi.decode(
            addressProvider.getAddresses(
                abi.encode(
                    USDR_BONDING_ADDRESS,
                    UNDERLYING_ADDRESS,
        ...
        uint256 available = IExchange(exchange).scaleToUnderlying(USDRBonding(bonding).available());
        require(available >= amount, "amount exceeds availability");
        address who = msg.sender;
        IERC20(underlying).transferFrom(who, address(this), amount);
        IERC20(underlying).approve(exchange, amount);
        uint256 amountOut = IExchange(exchange).swapFromUnderlying(amount, address(this));
        USDRBonding(bonding).update(amountOut);
        IERC20(usdr).approve(staking, amountOut);
        IStaking(staking).stake(amountOut);
        Vesting memory v = vesting[who];
        v.initialAmount = v.fullyVestedAmount - v.claimed + amountOut;
...
bytes32 constant USDR_BONDING_ADDRESS = bytes32(
    keccak256("USDRBonding")
);
...
contract USDRBonding is AddressAccessor {
    ...
    function reset() external {
        ...
        uint256 marketCap = IStaking(staking).usdrMarketCap() * 1e5;
        available = marketCap * percentage;
    }
    ...
    function update(uint256 delta) external onlyRole(CONTROLLER_ROLE) {
        available -= delta;
    }
}
