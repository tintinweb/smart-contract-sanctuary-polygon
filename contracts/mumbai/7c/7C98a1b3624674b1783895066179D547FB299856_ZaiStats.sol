...
contract ZaiStats is Ownable{
    ...
    event newRandomKing(uint256 newKing, uint256 lastKing, uint256 during);
    ...
    function getZaiStats(uint256 _zaiId) external view returns( 
        ...
        uint256 zaiTotalRandomSelected){
            ...
            return(     
                ...
                s.zaiTotalLoss,
                s.zaiTotalFights,
                s.zaiTotalRandomSelected);
    }

    function updateCounterWinLoss(uint256 _zaiId,uint256[30] memory _fightProgress) external onlyGame returns(bool){
        require(_zaiId != 0, "Zai doesn't exist");
        ZaiStruct.Stats storage p1 = _zaiStats[_zaiId];
        ZaiStruct.Stats storage p2 = _zaiStats[_fightProgress[0]];
        p1.zaiTotalFights = p1.zaiTotalFights + 1;

        if(p1.zaiTotalFights > totalFightKing.totalScore){
            emit newTotalKing(_zaiId, totalFightKing.actualKing, block.timestamp - totalFightKing.kingSince);
            totalFightKing.totalScore = p1.zaiTotalFights;
            totalFightKing.kingSince = block.timestamp;
            totalFightKing.actualKing = _zaiId;
        }

        p2.zaiTotalFights = p2.zaiTotalFights + 1;

        if(p2.zaiTotalFights > totalFightKing.totalScore){
            emit newTotalKing(_fightProgress[0], totalFightKing.actualKing, block.timestamp - totalFightKing.kingSince);
            totalFightKing.totalScore = p2.zaiTotalFights;
            totalFightKing.kingSince = block.timestamp;
            totalFightKing.actualKing = _fightProgress[0];
        }
        
        p2.zaiTotalRandomSelected= p2.zaiTotalRandomSelected + 1;

        if(p2.zaiTotalRandomSelected > randomKing.totalScore){
            emit newRandomKing(_fightProgress[0], randomKing.actualKing, block.timestamp - randomKing.kingSince);
            randomKing.totalScore = p2.zaiTotalRandomSelected;
            randomKing.kingSince = block.timestamp;
            randomKing.actualKing = _fightProgress[0];
        }

        if(_fightProgress[1] > _fightProgress[2]){
            p1.zaiTotalWins = p1.zaiTotalWins + 1;

            if(p1.zaiTotalWins > winKing.totalScore){
                emit newWinKing(_zaiId, winKing.actualKing, block.timestamp - winKing.kingSince);
            ...
            p1.zaiTotalDraw= p1.zaiTotalDraw + 1;
            if(p1.zaiTotalDraw > drawKing.totalScore){
                emit newDrawKing(_zaiId, drawKing.actualKing, block.timestamp - drawKing.kingSince);
            ...
            p1.zaiTotalLoss = p1.zaiTotalLoss + 1;
            if(p1.zaiTotalLoss > lossKing.totalScore){
                emit newDrawKing(_zaiId, lossKing.actualKing, block.timestamp - lossKing.kingSince);
    ...
    function updateAllPowersInGame(ZaiStruct.Powers memory toAdd) external returns(bool){
        require(msg.sender == gameAddresses.getZaiAddress(), "Not authorized");
