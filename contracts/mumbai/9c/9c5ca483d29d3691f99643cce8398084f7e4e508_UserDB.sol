...
contract UserDB {
    ...
    struct User {
        ...
        bool paymentPrefrences; // if true its privateKey payment
    }
    ...
    **/
    function addUser(string memory name ,string memory profile_pic , string memory phone_number , string memory erc20_wallet_address) public {
        ...
        users[userCount].paymentPrefrences = false;
    ...
    users[user_id].privateKeys.push(private_key);
     users[user_id].mainPrivateKey = private_key;
}
...
function changePaymentPrefrences(uint user_id, bool payment_prefrences) public {
    users[user_id].paymentPrefrences = payment_prefrences;
}

function getPaymentPrefrences(uint user_id) public view returns(bool) {
    return users[user_id].paymentPrefrences;
}
