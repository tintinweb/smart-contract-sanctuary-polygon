...
contract PlayerRental is IPlayerRental {
    ...
    constructor(address _addressRegistry) {
        ...
        address pxpAddress = addressRegistry.pxp();
        if (pxpAddress == address(0)) revert ZeroAddress();
        pxp = IPXP(pxpAddress);
    }
    ...
    function rentAsset(address _asset, uint256 _tokenId, uint256 _amount, uint256 _duration, bool _isERC721) external {
        ...
        (address assetPool721, address assetPool1155, address riskStrategy) = _checkPlayerRental();
        ...
        // pay the rental
        _payRental(riskStrategy, msg.sender, _amount, _duration, totalLiquidity, totalBorrows);
    ...
    function _checkPlayerRental()
        internal
        view
        returns (address assetPool721, address assetPool1155, address riskStrategy)
    {
        address playerRental;
        ...
        (, assetPool721, assetPool1155, playerRental, riskStrategy) = poolManagement.getPoolInfo(pid);
    ...
    function _payRental(
        ...
        uint256 actualUtilizationRate = IRiskStrategy(riskStrategy).getActualUtilizationRate(
            _totalLiquidity,
            _totalBorrows
        );
        uint256 dueAmount = IRiskStrategy(riskStrategy).getPXPCost(actualUtilizationRate) * _amount * _duration;
