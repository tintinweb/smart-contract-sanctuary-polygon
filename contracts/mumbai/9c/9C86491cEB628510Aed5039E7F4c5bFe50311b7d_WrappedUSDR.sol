...
contract wUSDR is ERC20PermitUpgradeable, OwnableUpgradeable {
    error StrategyFailedError(uint256 missingAmount);
    error TokenApprovalError(uint256 missingAmount);
    ...
    uint256 public strategyFee; // 100 = 1%
    ...
    function setStrategyFee(uint256 fee) external onlyOwner {
        strategyFee = fee;
    }

    /**
     * @dev Provides an USDR flash loan for the given `amount`.
     * The loan and the fee have to be paid back by the time the `strategy` is completed.
     * If the `amount` exceeds 50% of the `USDR` held by the `wUSDR` contract, strategists
     * can use `approvedTokens` to request a temporary approval for any token other than
     * `USDR` that may be available in the `wUSDR` contract. The strategy will then have
     * access to those tokens for the duration of this call.
     */ /*
    function runStrategy(
        ...
        uint256 initialBalance = USDR.balanceOf(address(this));
        uint256 fee;
        if (tx.origin != feeRecipient) {
            fee = (amount * fee) / 10000;
        }
        bool approveTokens = _verifyTokenApprovals(
            amount,
            initialBalance,
            approvedTokens.length
        );
        if (approveTokens) {
            _approveTokensForStrategy(
                strategy,
                type(uint256).max,
                approvedTokens
            );
        }
        USDR.transfer(strategy, amount);
        IStrategy(strategy).run(strategist, amount, data);
        uint256 newBalance = USDR.balanceOf(address(this));
        if (newBalance < initialBalance + fee) {
            revert StrategyFailedError(initialBalance + fee - newBalance);
        }
        _payFee(newBalance - initialBalance);
        if (approveTokens) {
            _approveTokensForStrategy(strategy, 0, approvedTokens);
        }
        emit StrategyExecuted(strategist, strategy, amount, fee);
    }

    function wrap(uint256 amount) external returns (uint256 value) {
        USDR.transferFrom(msg.sender, address(this), amount);
        value = wrappedAmount(amount);
        _mint(msg.sender, value);
    }

    function unwrap(uint256 amount) external returns (uint256 value) {
        _burn(msg.sender, amount);
        value = unwrappedAmount(amount);
        USDR.transfer(msg.sender, value);
    }

    function unwrappedAmount(uint256 amount) public view returns (uint256) {
        return (amount * USDR.liquidityIndex()) / 1e36;
    }

    function wrappedAmount(uint256 amount) public view returns (uint256) {
        return (amount * 1e36) / USDR.liquidityIndex();
    }

    function virtualPrice() external view returns (uint256) {
        return (USDR.balanceOf(address(this)) * 1e27) / totalSupply();
    }

    function _approveTokensForStrategy(
        address strategy,
        uint256 amount,
        ...
        for (uint256 i; i < numTokens; i++) {
            ...
            if (token != address(USDR)) {
                ...
                if (amount != 0) {
                    emit TokenApproved(msg.sender, strategy, token);
                }
            }
        }
    }
    ...
    function _payFee(uint256 fee) private {
        if (fee > 0) {
            uint256 value = wrappedAmount(fee);
            _mint(feeRecipient, value);
        }
    }

    function _verifyTokenApprovals(
        uint256 amount,
        uint256 balance,
        uint256 numApprovals
    ) private pure returns (bool) {
        if (numApprovals > 0) {
            if (amount * 2 >= balance) return true;
            revert TokenApprovalError(balance - amount * 2);
        }
        return false;
    }
}
