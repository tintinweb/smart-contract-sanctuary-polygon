...
contract Profile is Initializable, EIP712Upgradeable, OwnableUpgradeable {
    ...
    bytes32 public constant BIO_KEY = keccak256("bio");
    bytes32 public constant PROFILE_PICTURE_KEY = keccak256("profilePicture");
    ...
    /// @notice A struct to remove a field
    /// @param key The key of the field to update.
    /// @param value The value of the field to update.
    /// @param uid A unique ID that identifies the request. Used to prevent replay attacks.
    struct RemoveField {
    ...
    /// @notice Emitted when a profile is changed.
    /// @param sets The sets that were made.
    /// @param removals The removals that were made.
    event ProfileUpdated(address owner, SetField[] sets, RemoveField[] removals);
    ...
    function updateProfile(
        ...
        // Remove fields first
        for (uint256 i = 0; i < removals.length; ) {
            bytes32 key = removals[i].key;
            if (key == USERNAME_KEY) {
                bytes32 valueHash = _checkUsernameOwnership(removals[i].value);
                delete userNames[valueHash];
            } else if (key != BIO_KEY && key != PROFILE_PICTURE_KEY) {
        ...
        emit ProfileUpdated(owner, sets, removals);
    }

    function _checkUsernameOwnership(string calldata userName
    ) internal view returns (bytes32 userNameHash) {
        userNameHash = keccak256(bytes(userName));

        address existingOwner = userNames[userNameHash];
        if (existingOwner != _msgSender()) {
