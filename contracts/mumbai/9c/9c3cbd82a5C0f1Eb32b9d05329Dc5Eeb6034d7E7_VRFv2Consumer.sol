...
contract VRFv2Consumer is VRFConsumerBaseV2, ConfirmedOwner {
    ...
    mapping(uint256 => uint256) private words_reuse_count; 
    ...
    function RefreshRandomness(uint32 numWords) external onlyOwner returns (uint256){
        require(numWords > 0,"ZERO_WORDS");
        uint256 requestId = COORDINATOR.requestRandomWords(gas_lane_hash,subscription_id,request_confirmations,callback_gas_limit,numWords);
        requested[requestId] = true;
        last_requested_id = requestId;
        emit OnRandomnessRequested(requestId, numWords);
        return requestId;
    }
    ...
    function random(uint256 range) external returns (uint256){
        require(fullfiled[last_fullfilled_id],'NO_RANDOMNESS');
        uint256 _rand_value = words[last_fullfilled_id][nounces[last_fullfilled_id]] % range;
        nounces[last_fullfilled_id] = (nounces[last_fullfilled_id] + 1) % words[last_fullfilled_id].length;
        uint256 _new_reuse_count = uint256(nounces[last_fullfilled_id] / words[last_fullfilled_id].length);
        if(_new_reuse_count > words_reuse_count[last_fullfilled_id]){
            words_reuse_count[last_fullfilled_id] = _new_reuse_count;
            shuffle(last_fullfilled_id);
        }
        return _rand_value;
    }
    ...
    function shuffle(uint256 request_id) internal {
        for (uint256 i = 0; i < words[request_id].length; i++) {
            uint256 n = i + uint256(keccak256(abi.encodePacked(words_reuse_count[last_fullfilled_id]))) % (words[request_id].length - i);
            uint256 temp = words[request_id][n];
            words[request_id][n] = words[request_id][i];
            words[request_id][i] = temp;
        }
    }
}