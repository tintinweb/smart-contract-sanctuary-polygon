    ...
    function generateSVGImage(URIParams memory params) internal pure returns (string memory svg) {
        SVGSupplier.SVGParams memory svgParams =
            SVGSupplier.SVGParams({
                ...
                releaseTimestamp: params.releaseTimestamp,
...
library SVGSupplier {
    ...
    function generateSVGText(SVGParams memory params) private pure returns (string memory svg) {
        svg = string(
            abi.encodePacked(
                ...
                params.tokenSymbol,
                '</text><text x="20" y="70">Bond value, GTON: ',
    ...
    function generateDateScript(SVGParams memory params) private pure returns (string memory svg) {
        svg = string(
            abi.encodePacked(
                '<text transform="matrix(1 0 0 1 263.4561 55.7754)" class="st1 st2"></text>',
                '<script type="text/javascript"><![CDATA[',
                'var text = document.getElementsByTagName(\'text\')[2];',
                'var time = new Date(', params.releaseTimestamp.toString(), ' * 1000);',
                'var textnode = document.createTextNode(time.toDateString());',
