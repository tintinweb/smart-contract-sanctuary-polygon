...
contract MetaOMaticMain  is MetaOMaticUniversal {
    ...
    function _verifyId(address user,address referrer,bool allpackage,uint package) public {
        ...
        if(allpackage==false) {
          ...
          if(package==0){_PlaceInUp15(user,1);}
          else if(package==1){_PlaceInUp30(user,1);}
          else if(package==2){_PlaceInUp60(user,1);}
          else if(package==3){_PlaceInUp120(user,1);}
          else if(package==4){_PlaceInUp240(user,1);}
          else if(package==5){_PlaceInUp480(user,1);}
          ...
          _refPayout(user,((packageprice*m6per)/100));
            ...
            for (uint8 i = 0; i < totalPackage; i++) {
               ...
               _UserAffiliateDetails[user].selfInvestment+=packagePrice[i];
                if(i==0){_PlaceInUp15(user,1);}
                else if(i==1){_PlaceInUp30(user,1);}
                else if(i==2){_PlaceInUp60(user,1);}
                else if(i==3){_PlaceInUp120(user,1);}
                else if(i==4){_PlaceInUp240(user,1);}
                else if(i==5){_PlaceInUp480(user,1);}
    ...
    function _Withdrawal() public {  
       uint256 rewardM6=m6contract.getM6Bous(msg.sender);
      _UserIncomeDetails[msg.sender].totalM6Bonus += rewardM6;
      _UserIncomeDetails[msg.sender].totalBonus += rewardM6;
      _UserIncomeDetails[msg.sender].creditedWallet += rewardM6;
      _UserIncomeDetails[msg.sender].availableWallet += rewardM6;
      m6contract.updateM6Bonus(msg.sender);
      uint256 amount = _UserIncomeDetails[msg.sender].availableWallet;
      require(amount >= minimumWithdrawal,'You Must Enter Minimum Withdrawal Amount !');
      require(amount >= minimumWithdrawal,'You Must Have Minimum Withdrawal Amount !');
      uint256 _fees = (amount*adminChargePer)/100;
      ...
      _SafeTransfer(msg.sender,actualAmountToSend);
    ...
    function UpdateUpBonus(address user,uint256 amount,uint UpNo) private {
      _UserUPBonusDetails[user].totalBonus += amount;
      ...
      _SystemUpBonusDetails[0].totalBonus += amount;
    ...
    function _PlaceInUp15(address user,uint noofentry) private {
      for(uint I=1;I<=noofentry;I++){
       Up15List.push(user);
      _SystemUpTotalId[0].totalId += 1;
      _SystemUpTotalId[0].totalUp15Id += 1;
    ...
    function _PlaceInUp30(address user,uint noofentry) private {
      for(uint I=1;I<=noofentry;I++){
       Up30List.push(user);
      _SystemUpTotalId[0].totalId += 1;
      _SystemUpTotalId[0].totalUp30Id += 1;
    ...
    function _PlaceInUp60(address user,uint noofentry) private {
      for(uint I=1;I<=noofentry;I++){
       Up60List.push(user);
      _SystemUpTotalId[0].totalId += 1;
      _SystemUpTotalId[0].totalUp60Id += 1;
    ...
    function _PlaceInUp120(address user,uint noofentry) private {
      for(uint I=1;I<=noofentry;I++){
       Up120List.push(user);
      _SystemUpTotalId[0].totalId += 1;
      _SystemUpTotalId[0].totalUp120Id += 1;
    ...
    function _PlaceInUp240(address user,uint noofentry) private {
      for(uint I=1;I<=noofentry;I++){
       Up240List.push(user);
      _SystemUpTotalId[0].totalId += 1;
      _SystemUpTotalId[0].totalUp240Id += 1;
    ...
    function _PlaceInUp480(address user,uint noofentry) private {
      for(uint I=1;I<=noofentry;I++){
       Up480List.push(user);
      _SystemUpTotalId[0].totalId += 1;
      _SystemUpTotalId[0].totalUp480Id += 1;
