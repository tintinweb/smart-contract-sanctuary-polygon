...
contract PlanetRegistry {
    ...
    event PlanetCodeChanged(bytes32 indexed oldCode, bytes32 indexed newCode);
    ...
    modifier onlyRegister {
        require(
            registers[msg.sender] || accessControl.hasRole(accessControl.defaultAdminRole(), msg.sender),
            "unauthorized"
        );
        _;
    }

    modifier onlyApplicantAndRegister(bytes32 _code) {
        require(
            planetInfo[_code].applicant == msg.sender || accessControl.hasRole(accessControl.defaultAdminRole(), msg.sender) || registers[msg.sender],
            "unauthorized"
        );
        _;
    }
    ...
    function unregisterPlanet(bytes32 _code)
        public
        onlyApplicantAndRegister(_code)
    ...
    function changePlanetCode(bytes32 _oldCode, bytes32 _newCode) public onlyRegister {

        require(
            planetInfo[_oldCode].applicant != address(0),
            "planet doesn't exist"
        );
        require(_newCode != "", "empty code");
        require(
            planetInfo[_newCode].applicant == address(0),
            "code is taken"
        );

        planetInfo[_newCode] = planetInfo[_oldCode];
        planetInfo[_newCode].planetCode = _newCode;
        shareholdersInfo[_newCode] = shareholdersInfo[_oldCode];

        delete planetInfo[_oldCode];
        delete planetInfo[_oldCode];

        emit PlanetCodeChanged(_oldCode, _newCode);
