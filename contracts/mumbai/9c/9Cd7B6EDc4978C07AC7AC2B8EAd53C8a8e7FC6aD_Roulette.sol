...
contract Roulette is Ownable {
  ...
  */
  ...
  function spinWheel(uint256 _betId, bool isMatic, address ERC20Address) internal  {
    ...
    uint256 num = uint(keccak256(abi.encodePacked(block.timestamp,block.difficulty,msg.sender, _betId))) % 36; 
    ...
    bool won = false;
      ...
      if (won && isMatic) {
        winningsInMatic[bet.bettorAddress] += bet.betAmount[i] * payouts[typeOfBet];
      ...
      emit WinAmount(_betId, bet.winningAmount);
    } 
  }
