...
contract Trade {
    ...
    function buyAsset(Order memory order, uint256 assetlength, Sign memory sign) public returns(bool) {
        require(!usedNonce[sign.nonce],"Nonce: Invalid Nonce");
        updatingNonce(order, assetlength, sign.nonce);
    ...
    function executeBid(Order memory order, uint256 assetlength, Sign memory sign) public returns(bool) {
        require(!usedNonce[sign.nonce],"Nonce: Invalid Nonce");
        updatingNonce(order, assetlength, sign.nonce);
        Fee memory fee = getFees(order);
    ...
    function updatingNonce(Order memory order, uint256 assetlength, uint256 nonce) internal returns(bool) {
        if(!(nftListStatus[msg.sender][order._orderItems])) {
            nftlist[msg.sender][order._orderItems] = assetlength;
            nftListStatus[msg.sender][order._orderItems] = true;
        }
        
        OrderItem[] memory orderValues = abi.decode(order._orderItems, (OrderItem[]));

        for(uint i = 0; i < orderValues.length; i++) {
            if(order.tokenId == orderValues[i].tokenId && order.nftAddress == orderValues[i].nftAddress) {
                nftlist[msg.sender][order._orderItems] -= 1;
                if(nftlist[msg.sender][order._orderItems] == 0) 
