...
contract GenArtHub is ERC721Enumerable, ReentrancyGuard, Ownable, IGenArtHub {
  ...
  */
  ...
  // TODO: REMOVE **********************************************
  // buyer address => projectId => number of mints
  mapping(address => mapping (uint256 => uint256)) projectMintCounter;
  ...
  */
  ...
  modifier onlyArtist(uint256 _projectId) {
    if (_msgSender() != projectIdToArtistAddress[_projectId]) revert NotAllowed();
    _;
  }

  // Only whitelisted or artists
  modifier onlyArtistOrWhitelisted(uint256 _projectId) {
    if (!whitelist[_msgSender()] && _msgSender() != projectIdToArtistAddress[_projectId]) revert NotAllowed();
    _;
  }
  ...
  */
  ...
  /**
    Minting
  */
  function mint(address _to, uint256 _projectId) override external payable returns (uint256 tokenId) {
    if (projects[_projectId].mints >= projects[_projectId].maxMints) revert MaxMintsReached();
    if (!projects[_projectId].active) revert InvalidProject();
    if (projects[_projectId].paused) revert PausedProject();

    // TODO: REMOVE **********************************************
    // if (msg.value < projects[_projectId].price) revert NotAllowed();
    // if (projectMintLimit[_projectId] > 0) {
    //   if (projectMintCounter[_msgSender()][_projectId] < projectMintLimit[_projectId]) {
    //     projectMintCounter[_msgSender()][_projectId]++;
    //   } else {
  ...
  // TODO: REMOVE **********************************************
  function _splitFunds(uint _projectId) internal {
    if (msg.value > 0) {
      Project storage project = projects[_projectId];

      uint256 refund = msg.value - project.price;
      if (refund > 0) {
        (bool _success,) = _msgSender().call{value: refund}("");
        require(_success, "Refund failed");
      }

      uint256 treasuryAmount = (project.price * treasuryPercentage) / 100;
      uint256 artistFunds = project.price - treasuryAmount;

      if (artistFunds > 0) {
        (bool success_,) = projectIdToArtistAddress[_projectId].call{value: artistFunds}("");
        require(success_, "Artist payment failed");
      }
    }
  }

  function setProjectMintLimit(uint256 _projectId, uint8 _limit) external onlyArtistOrWhitelisted(_projectId) {
  ...
  function updatePricePerTokenInWei(
    ...
    projects[_projectId].price = _pricePerTokenInWei;
  }
  // **********************************************
  ...
  */
  ...
  /**
    Admin
  */
  function updateTreasuryAddress(address payable _treasuryAddress) public onlyOwner {
    treasuryAddress = _treasuryAddress;
  }

  function updateTreasuryPercentage(uint256 _treasuryPercentage) public onlyOwner () {
  ...
  function updateRandomizerAddress(address _randomizerAddress) public onlyWhitelist {
  ...
  /**
    Project
  */
  function addProject(string memory _projectName, address payable _artistAddress) public onlyWhitelist returns (uint256 _projectId) {
    ...
    // TODO: REMOVE **********************************************
    projects[projectId].price = 0.01 ether;
  ...
  function toggleProjectIsLocked(uint56 _projectId) public onlyWhitelist onlyUnlocked(_projectId) {
  ...
  */
  ...
  function updateProjectArtistName(uint256 _projectId, string memory _artistName) public onlyUnlocked(_projectId) onlyArtistOrWhitelisted(_projectId) {
    projects[_projectId].artist = _artistName;
  }

  function updateProjectDescription(uint256 _projectId, string memory _description) public onlyUnlocked(_projectId) onlyArtistOrWhitelisted(_projectId) {
    projects[_projectId].description = _description;
  }

  function updateProjectBaseURI(uint256 _projectId, string memory _baseURI) public onlyUnlocked(_projectId) onlyArtistOrWhitelisted(_projectId) {
    projects[_projectId].baseURI = _baseURI;
  }

  function updateProjectMaxMints(uint256 _projectId, uint256 _maxMints) public onlyArtist(_projectId) {
    if (projects[_projectId].locked || _maxMints > projects[_projectId].maxMints) revert NotAllowed();
  ...
  /**
    Getters
  */
  function projectDetails(uint256 _projectId) public view returns (
    ...
    price = projects[_projectId].price;
  }
  ...
  function projectScriptInfo(uint256 _projectId) external view  returns (
  ...
  function projectScriptByIndex(uint256 _projectId, uint256 _index) external view returns (
    string memory
  ) {
    return projects[_projectId].scripts[_index];
  }

  function projectURIInfo(uint256 _projectId) external view returns (string memory baseURI) {
    baseURI = projects[_projectId].baseURI;
  }

  function tokenIdToProjectId(uint256 _tokenId) external pure  returns (uint256 _projectId) {
...
interface IGenArtHub {
  ...
  // TODO: Update to add _by, and remove payable
  function mint(address _to, uint256 _projectId) external payable returns (uint256 tokenId);
}
