...
import "@superfluid-finance/ethereum-contracts/contracts/tokens/PureSuperToken.sol";
import "@superfluid-finance/ethereum-contracts/contracts/interfaces/superfluid/ISuperToken.sol";

import {SuperTokenV1Library} from "@superfluid-finance/ethereum-contracts/contracts/apps/SuperTokenV1Library.sol";

contract SuperNFTWrapper is Ownable, ERC721URIStorage, ERC721Holder {
    using SuperTokenV1Library for ISuperToken;
    IERC721 private _nft;

    mapping(uint256 => PureSuperToken) public FFTMappings; //mapping from tokenId to Fluid Fraction Tokens

    constructor(IERC721 nft_) ERC721("SuperSliceNFT", "SSNFT") {
        _nft = nft_;
    }

    function wrapNFT(uint256 tokenId_, string memory tokenURI_) external {
        require(
            _nft.ownerOf(tokenId_) == msg.sender,
        ...
        //Create a Super Token Contract for this newly minted NFT
        PureSuperToken FFT = (new PureSuperToken)();
        FFT.initialize("Fluid Fraction Token", "FFT", 100 * 1e18); //initialize
        ISuperToken(address(FFT)).transfer(msg.sender, 100 * 1e18);

        FFTMappings[tokenId_] = FFT; //bind the fractional token address to this NFT token just minted
    }

    function unwrapNFT(uint256 tokenId_) external {
        require(
            ISuperToken(address(FFTMappings[tokenId_])).balanceOf(msg.sender) ==
                100 * 1e18,
            "Only the owner of the wrapped NFT can unwrap it"
        ); //ownership of a wrappedNFT is represented by the holders FFT balance.
        _burn(tokenId_);
        _nft.safeTransferFrom(address(this), msg.sender, tokenId_);
    }
}