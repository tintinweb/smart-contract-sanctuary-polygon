...
// Smart Contract v2.0.0 - (May 9, 2023). - Changes in the way we calculate mortgage payments through autotasks.
...
contract Payments is ReentrancyGuard, AccessControl, IMortgageInterest {
    ...
    // Payments, Liquidation, auction, Mortage Control, Mortgages, Rewards payer, Helper Rewards Payer

    address internal relayAddress = 0x1921a154365A82b8d54a3Cb6e2Fd7488cD0FFd23; //test wallet foundry:0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266; //polygon mainnet: 0x988F94C0Ef61815AacD775789Af297713408D3B8;
    address public walletPanoram = 0x30268390218B20226FC101cD5651A51b12C07470; // CAMBIAR POR EL MULTISIGN
    uint8 public amountPerPage = 50;
    ...
    event successfullAdvancePayment(address indexed client, uint256 _IdMortgage, uint256 _amount);
    event MortgagePaid(address indexed wallet, uint256 _IdMortgage, uint256 _amount);
    event successfullBatchPayment(address indexed wallet, uint256[] _IdMortgages, uint256 _amountPaid);
    event MortgageLiquidated(address indexed user, uint256 mortgageID);
    event CalcMortgagesPaymentsforSegment(address wallet, uint256 mortgageID);
    ...
    /// @dev Function to give the Front the debt accumulated by a mortgage.
    /// @param _IdMortgage - the Mortgage's ID to query.
    /// @param _user - the user's address.
    /// @return CapitalPay - The accumulated capital to pay from the last payment to the moment of query.
    /// @return Interest - The accumulated interest to pay from the last payment to the moment of query.
    function getMortgageDailyDebt(uint256 _IdMortgage, address _user)
        public
        view
        returns (uint256 CapitalPay, uint256 Interest)
    {
        uint256 LenderRewardsPay;
        uint256 PanoramPay;

        MortgageInterest memory mortgage = mortgageControl.getuserToMortgageInterest(_user, _IdMortgage);
        (CapitalPay, LenderRewardsPay, PanoramPay) = calcPayment(mortgage);

        Interest = LenderRewardsPay + PanoramPay;
    }
        ...
        nonReentrant
    {
        ...
        // check if mortgage is payed or liquidated and revert if it is.
        isPaidOrLiquidated(mortgage);

        // Calcular el pago tanto si es normal como delayed
        (uint256 Capital, uint256 lenderRewardPay, uint256 panoramPay) = calcPayment(mortgage);
        uint256 totalToPay = Capital + lenderRewardPay + panoramPay;

        if (totalToPay == 0) {
            revert("You Already Pay");
        }
        // actualizar la deuda total
        if (Capital >= mortgage.totalDebt) {
            mortgage.totalDebt = 0;
            mortgageControl.updateMortgageState(_IdMortgage, msg.sender, true);
        } else {
            mortgage.totalDebt -= Capital;
        }
        mortgageControl.updateTotalDebt(msg.sender, _IdMortgage, mortgage.totalDebt); // actualiza campo totalDebt
        // recalcular y registra los nuevos pagos de intereses diarios, intereses panoram y lenders normales y moratorios
        mortgageRegister.registerInitialMortgageData(_IdMortgage, msg.sender, mortgage.totalDebt);
        // guardar la fecha de pago y resetear los strikes y el monthy payment.
        mortgageControl.updateLastTimePayment(msg.sender, _IdMortgage, block.timestamp);
        mortgageControl.resetStrikes(_IdMortgage, msg.sender);
        mortgageControl.updateMonthlyPaymentDelayed(msg.sender, _IdMortgage, false);
        mortgageControl.updateMortgagePayment(_IdMortgage, msg.sender); // actualiza contador de pagos

        updateRetainedRents(mortgage, _IdMortgage);

        // enviar el valor a pagar a la funcion de handle payment para realizar las transferencias
        handlePayment(_token, totalToPay, Capital, lenderRewardPay, panoramPay);
        emit successfullPayment(msg.sender, _IdMortgage, totalToPay);
    }
        ...
        nonReentrant
    {
        ...
        // validar que no tiene deuda y tampoco esta liquidado para que pueda hacer un pago adelantado
        MortgageInterest memory mortgage = mortgageControl.getuserToMortgageInterest(msg.sender, _IdMortgage);
        isPaidOrLiquidated(mortgage);
        ...
        // descontarlo del total debt y registrar la nueva deuda
        if (_amountToPay >= mortgage.totalDebt) {
            mortgage.totalDebt = 0;
            mortgageControl.updateMortgageState(_IdMortgage, msg.sender, true);
        } else {
        ...
        mortgageControl.updateTotalDebt(msg.sender, _IdMortgage, mortgage.totalDebt);
        mortgageControl.updateMortgagePayment(_IdMortgage, msg.sender);
        mortgageControl.updateLastTimePayment(msg.sender, _IdMortgage, block.timestamp);
        // recalcular el pago de capital y los intereses normales y moratorios y registrarlos en el mortgage control
        mortgageRegister.registerInitialMortgageData(_IdMortgage, msg.sender, mortgage.totalDebt);
        ...
        // transferir el _amountToPay que quiere adelantar como pago
        ...
        // emitimos evento con el pago adelantado realizado
        emit successfullAdvancePayment(msg.sender, _IdMortgage, _amountToPay);
    }
    ...
    function liquidationDebt(uint256 _IdMortgage, address _token) external validToken(_token) isPaused nonReentrant {
        ...
        // calcular el pago total a realizar, total debt + intereses acumulados desde el ultimo pago al dia de hoy
        (, uint256 lenderRewardPay, uint256 panoramPay) = calcPayment(mortgage);
        uint256 totalToLiquidateDebt = mortgage.totalDebt + lenderRewardPay + panoramPay;

        // guardar la fecha de pago y resetear los strikes.
        mortgageControl.updateLastTimePayment(msg.sender, _IdMortgage, block.timestamp);
        mortgageControl.resetStrikes(_IdMortgage, msg.sender);
        mortgageControl.updateMonthlyPaymentDelayed(msg.sender, _IdMortgage, false);
        ...
        // enviar el valor a pagar a la funcion de handle payment para realizar las transferencias
        handlePayment(_token, totalToLiquidateDebt, mortgage.totalDebt, lenderRewardPay, panoramPay);

        mortgageControl.resetDebt(msg.sender, _IdMortgage); // manda total debt a 0

        emit MortgagePaid(msg.sender, _IdMortgage, totalToLiquidateDebt);
    }
        ...
        nonReentrant
    {
        ...
        if (_IdMortgages.length > amountPerPage) {
            revert("Array Length Must Not Exceed Amount Per Page");
        }
        ...
        for (uint16 i = 0; i < _IdMortgages.length;) {
            ...
            // solo si mortgage no esta liquidada o tiene dias pendientes de pago
            if (!mortgage.liquidate && mortgage.strikes != 0) {
                // calcular el monto a pagar de capital, intereses panoram y lender rewards por cada hipoteca.
                (uint256 capitalMortgage, uint256 LenderRewards, uint256 ForPanoram) = calcPayment(mortgage);
                capitalPay += capitalMortgage;
                payForLenderRewards += LenderRewards;
                payForPanoram += ForPanoram;
                // actualizar el total debt de cada hipoteca y su estado de isPay
                if (capitalMortgage >= mortgage.totalDebt) {
                    mortgage.totalDebt = 0;
                    mortgageControl.updateMortgageState(_IdMortgages[i], msg.sender, true); // isPay
                } else {
                    mortgage.totalDebt -= capitalMortgage;
                }
                // actualiza campo totalDebt
                mortgageControl.updateTotalDebt(msg.sender, _IdMortgages[i], mortgage.totalDebt);
                // resetear los strikes, guardar la fecha de pago, resetear el monthly payment delayed y actualizar el contador de pagos
                mortgageControl.updateLastTimePayment(msg.sender, _IdMortgages[i], block.timestamp);
                mortgageControl.resetStrikes(_IdMortgages[i], msg.sender);
                mortgageControl.updateMonthlyPaymentDelayed(msg.sender, _IdMortgages[i], false);
                mortgageControl.updateMortgagePayment(_IdMortgages[i], msg.sender);

                // recalcular los intereses de cada hipoteca y guardalo en el mortgage control.
                mortgageRegister.registerInitialMortgageData(_IdMortgages[i], msg.sender, mortgage.totalDebt);

                updateRetainedRents(mortgage, _IdMortgages[i]);
            }
            unchecked {
                ++i;
            }
        }
        // transferir el dinero del user al contrato y repartir los pagos respectivos a cada vault y a panoram.
        finalPayment = capitalPay + payForLenderRewards + payForPanoram;
        handlePayment(_token, finalPayment, capitalPay, payForLenderRewards, payForPanoram);

        // emitir evento de pago exitoso.
        emit successfullBatchPayment(msg.sender, _IdMortgages, finalPayment);
    }

    ///@dev Function to update the strikes and payment status for a mortgage.
    /**
     * @notice
     * cuando el array de wallets sea muy grande hay que llamar a la funcion por lotes de wallets para evitar que consuma todo el gas durante su ejecucion,
     * the relayer should prepare the arrays to avoid duplicate wallets, or mortgages already paid or liquidated.
     */
    function UpdateStrikesRelayer(address[] calldata _userWallet, uint256[] calldata _idMortgage)
        external
        onlyRelayer
        isPaused
    {
        uint256 length = _userWallet.length;
        if (length != _idMortgage.length) {
        ...
        if (length != 0) {
            ...
            for (uint16 i = 0; i < length;) {
                collection = mortgageControl.getCollection(_userWallet[i], _idMortgage[i]);
                payPerNFT = RControl.getPayPerNFT(_userWallet[i], collection);
                (isLiquidated, amountRetained) =
                    mortgageControl.updateStrikes(_idMortgage[i], _userWallet[i], payPerNFT, collection);

                if (isLiquidated) {
                    RControl.updateAccumulatedRent(_userWallet[i], collection, amountRetained);
                    RControl.updateRentRetained(_userWallet[i], collection, amountRetained);
            ...
            emit CalcMortgagesPaymentsforSegment(_userWallet[length - 1], _idMortgage[length - 1]);
        } else {
            emit CalcMortgagesPaymentsforSegment(address(0), 0);
        }
    }

    /// @dev Function to pay the Interest Accumulated with the retained rents of the user liquidated.
    /// @notice Esta funcion se debe llamar en el Autotask de liquidaciones.
    function payInterestWithRentsRetained(address _user, uint256 _idMortgage, address _collection, address _token)
        external
        onlyRelayer
        validToken(_token)
        isPaused
    {
        // obtener las rentas retenidas.
        uint256 rentRetained = mortgageControl.getRetainedRent(_user, _collection, _idMortgage);
        uint256 rent = rentRetained;
        // llamar a la funcion para calcular los intereses que debe a lenders y a Panoram.
        MortgageInterest memory mortgage = mortgageControl.getuserToMortgageInterest(_user, _idMortgage);
        (, uint256 LendersInterest, uint256 PanoramInterest) = calcPayment(mortgage);

        uint256 amountLender;
        uint256 amountPanoram;
        // validar si alcanza para pagar los intereses de lenders: si si alcanza pagarlos todos, si no pagar para lo que alcance.
        if (rentRetained >= LendersInterest) {
            // registrar la cantidad que se abono al pago de intereses a los lenders en el mortgage control.
            mortgageControl.setInterestLendersPaidOnLiquidation(_idMortgage, _user, LendersInterest);
            amountLender = LendersInterest;
            rentRetained -= LendersInterest;
        } else {
            mortgageControl.setInterestLendersPaidOnLiquidation(_idMortgage, _user, rentRetained);
            amountLender = rentRetained;
            rentRetained = 0;
        }

        // si sobro dinero, validar si alcanza para abonar el pago de los intereses para Panoram.
        if (rentRetained > 0) {
            if (rentRetained >= PanoramInterest) {
                // registrar cuanto se abono de los intereses de panoram en el mortgage control.
                mortgageControl.setInterestPanoramPaidOnLiquidation(_idMortgage, _user, PanoramInterest);
                amountPanoram = PanoramInterest;
                rentRetained -= PanoramInterest;
                // si sobra
                amountLender += rentRetained;
            } else {
                mortgageControl.setInterestPanoramPaidOnLiquidation(_idMortgage, _user, rentRetained);
                amountPanoram = rentRetained;
                rentRetained = 0;
            }
        }

        // retirar las rentas del vault de rentas.
        address VaultRent = tokenInfo.getVaultRent(_collection);
        IVaultRent(VaultRent).withdraw(rent, _token);

        // transferir la cantidad abonada al vault de lenders rewards.
        (, address vaultRewardsLenders,) = tokenInfo.getVaultInfo(_token);
        IVaultLendersRewards(vaultRewardsLenders).deposit(amountLender, _token);
        // transferir la cantidad abonada a la wallet de panoram.
        IERC20(_token).safeTransfer(walletPanoram, amountPanoram);
    }

    /// @dev Helper Function to calculate the amount the user should pay for the mortgage.
    function calcPayment(MortgageInterest memory _mortgage)
        private
        pure
        returns (uint256 CapitalPay, uint256 LenderRewardPay, uint256 PanoramPay)
    {
        // validar el numero de dias a calcular. Si strikes = 0 no calcular, retornar 0
        if (_mortgage.strikes == 0) {
            CapitalPay = LenderRewardPay = PanoramPay = 0;
        } else if (_mortgage.strikes > 0 && _mortgage.strikes <= 30) {
            // calcular el pago de los dias normales.
            CapitalPay = _mortgage.capitalDailyPay * _mortgage.strikes;
            LenderRewardPay = _mortgage.interestRewardLender * _mortgage.strikes;
            PanoramPay = _mortgage.interestPanoramDaily * _mortgage.strikes;
        } else {
            // si esta delayed calcular el pago de los dias delayed y sumarlo con los dias normales
            // quitamos los 30 que ya calculamos para calcular los dias restantes
            uint8 delayedDays = _mortgage.strikes - 30;
            CapitalPay = _mortgage.capitalDailyPay * _mortgage.strikes;
            LenderRewardPay = (_mortgage.interestRewardLender * 30) + (_mortgage.lateInterestRewardLender * delayedDays);
            PanoramPay = (_mortgage.interestPanoramDaily * 30) + (_mortgage.lateInterestPanoramDaily * delayedDays);
        }
    }

    ///@dev Helper Function to handle the internal transfers and payments that happen when an user pay his mortgage debt.
    function handlePayment(
        address _token,
        uint256 _totalToPay,
        uint256 _capital,
        uint256 _lenderRewardPay,
        uint256 _panoramPay
    ) private {
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _totalToPay);

        (address vaultLenders, address vaultRewardsLenders,) = tokenInfo.getVaultInfo(_token);

        IVaultLenders(vaultLenders).depositCapital(_capital, _token);
        IVaultLendersRewards(vaultRewardsLenders).deposit(_lenderRewardPay, _token);
        IERC20(_token).safeTransfer(walletPanoram, _panoramPay);
    }

    /// @dev Helper Function to check if a mortgage total debt is payed or if it's liquidated.
    function isPaidOrLiquidated(MortgageInterest memory _mortgage) private pure {
        if (_mortgage.totalDebt == 0) {
            revert("The Mortgage is Paid or This is not your Mortgage");
        }
        if (_mortgage.liquidate) {
            revert("Liquidated Mortgage");
        }
    }

    /// @dev Function to update the related info to Retained Rents in Mortgage Control and Rewards Control
    function updateRetainedRents(MortgageInterest memory _mortgage, uint256 _IdMortgage) private {
        if (_mortgage.strikes > 30) {
            address collection = mortgageControl.getCollection(msg.sender, _IdMortgage);
            mortgageControl.reduceDelayedMortgages(msg.sender, collection);
            // get el valor guardado de rentas retenidas para esta hipoteca en mortgage control
            uint256 retainedRent = mortgageControl.getRetainedRent(msg.sender, collection, _IdMortgage);
            // resta y Actualiza el rents retained en rewards control con el nuevo valor
            RControl.updateRentRetained(msg.sender, collection, retainedRent);
            // resetar a 0 el rent retained en mortgage control
            mortgageControl.resetRentRetained(msg.sender, collection, _IdMortgage);
        }
    }
    ...
    function updateAmountPerPage(uint8 _newAmountPerPage) external onlyDev {
        amountPerPage = _newAmountPerPage;
    }
