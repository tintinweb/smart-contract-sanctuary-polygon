...
//导入一个帮助文件
//一个自定义函数来检查它们的长度。这个函数首先将它们转换为字节

//编译文件


contract Domains is ERC721URIStorage {
    // OpenZeppelin为我们提供tokenId
    ...
    // 我们通过将 this 添加到构造函数中来使合约“可支付”
    constructor(string memory _soul) payable ERC721("Pa Name Service", "PA") {
        owner = payable(msg.sender);
        // 一个名为的公共字符串soul- 这将记录您的域以什么结尾
        soul = _soul;
        console.log("%s name service deployed", _soul);
    }

    //根据域的长度返回价格。域名越短越贵
    //由于 MATIC 代币有 18 位小数，我们需要放在* 10**18价格的末尾。
    function price(string calldata name) public pure returns (uint256) {
        uint256 len = StringUtils.strlen(name);
        require(len > 0);
        if (len == 3) {
            return 5 * 10**10; // 5 MATIC = 5 000 000 000 000 000 000（18 位小数）。我们将使用 0.5 Matic，因为水龙头不会提供太多
        } else if (len == 4) {
            return 3 * 10**10; //要收取较小的金额，请减少小数。这是 0.3
        } else {
            return 1 * 10**10;
        }
    }
    ...
    //注册域名函数
    // calldata name  存储参数的“位置”
    function register(string calldata name) public payable {
        // 检查名称是否未注册
        if (domains[name] != address(0)) revert AlreadyRegistered();
        if (!valid(name)) revert InvalidName(name);
        //检查发送的“msg”的“值”是否高于一定数量
        uint256 _price = price(name);
        //检查交易中有没有足够的 Matic 支付,“Value”是发送的 Matic 数量，“msg”是交易
        require(msg.value >= _price, "Not enough Matic paid");
        // 合并域名
        string memory _name = string(abi.encodePacked(name, ".", soul));
        // 合并SVG
