...
contract Escrow {
    ...
    modifier onlyBuyer(uint256 _purchaseId) {
        require(
            msg.sender == purchases[_purchaseId].buyer,
     ...
     modifier onlySeller(uint256 _purchaseId) {
        require(
            msg.sender == purchases[_purchaseId].seller,
            "Only the seller can call this function."
        );
        _;
    }

    modifier shipmentConfirmed(uint256 _purchaseId) {
        require(purchases[_purchaseId].isShipmentConfirmed, "Shipment is not confirmed yet.");
        _;
    }

    function createPurchase(address _buyer, address _seller, uint256 _totalPayoutAmount) external {
        uint256 newPurchaseId = purchaseCount;
        purchases[newPurchaseId] = Purchase(_buyer, _seller, _totalPayoutAmount, false);
        purchaseCount++;
    }

    function confirmShipment(uint256 _purchaseId) external onlyBuyer(_purchaseId) {
        require(!purchases[_purchaseId].isShipmentConfirmed, "Shipment has already been confirmed.");
        // Add your logic for confirming shipment, e.g., emitting an event.
        purchases[_purchaseId].isShipmentConfirmed = true;
    }

    function releaseFunds(uint256 _purchaseId) external onlySeller(_purchaseId) shipmentConfirmed(_purchaseId) {
        require(address(this).balance >= purchases[_purchaseId].totalPayoutAmount, "Insufficient balance in escrow.");

        // Transfer the funds to the seller
        (bool success, ) = payable(purchases[_purchaseId].seller).call{value: purchases[_purchaseId].totalPayoutAmount}("");
        require(success, "Failed to transfer funds to the seller.");

        // Add any additional logic you need after the funds are released

        // Emit an event or update state variables if necessary
    }
}
...
interface IMarketplace is IThirdwebContract {
    ...
    IERC1155ReceiverUpgradeable
{
    ...
    escrow.createPurchase(_payer, _payee, _totalPayoutAmount);
