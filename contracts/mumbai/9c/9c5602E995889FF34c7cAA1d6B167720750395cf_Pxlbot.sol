...
contract Pxlbot is ERC721APlayable, IPxlbot, BaseAttributeCoordinator, Payable {
  ...
  function renameBot(uint256 tokenId, string memory name) override external payable {
    require(isController(_msgSender()) || ownerOf(tokenId) == _msgSender(), 'Only owner or controller can rename bot');
