    ...
    ERC721Burnable
{
    ...
    error ZeroAddress(address caller, string message);

    /**n @dev ============================================================ State Variables Ends ============================================================= */

    /**@notice =============================================================== Constructor ================================================================== */

    /** removed from construct as not using this EIP712(SIGNING_DOMAIN, SIGNATURE_VERSION)*/

    constructor() ERC721("BlockVareNFT", "BV") {
    ...
    function setMarketPlaceAddress(address _marketPlaceAddress) external {
        require(
            ...
            "BlockVier: only admin allowed!"
        );
        require(isContract(_marketPlaceAddress), "invalid address.");
        emit MarketPlaceAddressChanged(_marketPlaceAddress);

        marketAddress = _marketPlaceAddress;
    }

    function setListingFee(uint96 _listingFee) external {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, msg.sender),
            "BlockVier: only admin allowed!"
        );
    ...
    function setTokenURI(uint256 tokenId, string memory _tokenURI) public {
        require(msg.sender == ownerOf(tokenId), "BlockVier: only NFT owner!");
    ...
    function redeem(
        ...
        if (redeemer == address(0)) {
            revert ZeroAddress(redeemer, "zero address");
        }
        ...
        // require(signer != redeemer, "LazyNFT: redeemer is voucher signer");
        // require(signer != msg.sender, "LazyNFT: caller is vouher signer");
        // console.log("Signer from NFT platform:", signer);

        // // make sure that the signer is authorized to mint NFTs
        // require(
        //     hasRole(MINTER_ROLE, signer),
        //     "LazyNFT: Signature invalid or unauthorized"
        // );
        ...
        _safeMint(signer, _newTokenId);
        _setTokenURI(_newTokenId, voucher.uri);

        // transfer the token to the redeemer
        _transfer(signer, redeemer, _newTokenId);

        _setApprovalForAll(redeemer, address(this), true);

        require(isApprovedForAll(redeemer, address(this)));

        // setting royalty here
        uint256 _royaltyInBP = voucher.royaltyBPS;
        ...
        royalties[_newTokenId] = RoyaltyReceiver({
    ...
    function mint(string memory _uri, uint256 _royaltyInBP)
        external
        onlyRole(MINTER_ROLE)
        returns (uint256 _tokenId)
    {
        if (msg.sender == address(0)) {
            revert ZeroAddress(msg.sender, "zero address");
        }
    ...
    /**  Transfers all pending withdrawal balance to the AUTHORIZED caller. Reverts if not AUTHORIZED. */
    ...
    //     emit EtherWithdrawn(receiver, amount);
    ...
    /** =========================================================== State Modifiying Funcitions Ends ======================================================== */
    ...
    function withDrawFee() external whenNotPaused {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, msg.sender),
            "BlockVareNFT: only DEFAULT_ADMIN_ROLE"
        );

        address payable receiver = payable(msg.sender);

        require(receiver != address(0), "caller 0 address!");
    ...
    fallback() external payable {
        emit EtherReceivedFallback(_msgSender(), msg.value);
    }

    /** =========================================================== Internal and Private Funcitions =========================================================== */
    ...
    //                 abi.encode(
    //                     keccak256(
    //                         "NFTVoucher(address creator,uint256 tokenId,uint256 minPrice,string uri)"
    //                     ),
    //                     voucher.creator,
    //                     voucher.tokenId,
    //                     voucher.minPrice,
