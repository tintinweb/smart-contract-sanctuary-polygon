...
contract CashBoxV2 is ERC20Detailed {
    ...
    event CashTokensRedeemed(
        address indexed user,
        uint256 redeeemedCashAmount,
        uint256 outputAssetAmount,
        uint256 mintedCashboxPoolAmount
    );
    event CashboxPoolTokensBurnt(
        address indexed user,
        uint256 burntCashboxPoolAmount,
        uint256 outputAssetAmount,
        uint256 outputCashAmount
    );
    event AssetTokensRedeemed(
        address indexed user,
        uint256 redeemedAssetToken,
        uint256 outputCashAmount,
        uint256 mintedAssetPoolAmount
    );
    event AssetPoolTokensBurnt(
        address indexed user,
        uint256 burntAssetPoolToken,
        uint256 outputAssetAmount,
        uint256 outputCashAmount
    );
    ...
    constructor(
        ...
        require(
            ...
            "assetToken is the zero address"
        );
        ...
        assetToCashRate = ((10**(cashDecimals)).mul(_assetToCashRate)).div(
            1e18
        );
    ...
    function contractAssetTokenBalance()
        public
        view
        returns (uint256 assetTokenBalance)
    {
    ...
    function assetTokenCashValuation() internal view returns (uint256) {
        uint256 cashEquivalent = (
            contractAssetTokenBalance().mul(assetToCashRate)
        ).div(assetTokenMultiplier);
    ...
    function accountHoldsNFT(address _address)
        public
        view
        returns (bool)
    {
        return
            IERC721(nftFactoryAddress).balanceOf(
                _address
            ) > 0;
    }

    function depositCash(uint256 inputCashAmount) external {
        require(accountHoldsNFT(msg.sender), "Not allowed");
        ...
        uint256 assetToRedeem = (actualCashReceived.mul(assetTokenMultiplier))
            .div(assetToCashRate);
        ...
        if (assetToRedeem <= contractAssetTokenBalance()) {
            ...
            uint256 remainingAsCashTokens = (
                (assetToRedeem.sub(outputAssetAmount)).mul(assetToCashRate)
            ).div(assetTokenMultiplier);
    ...
    function burnCashboxPoolToken(uint256 cashboxPoolTokenAmount) external {
        ...
        uint256 assetToRedeem = cashboxPoolTokenAmount
            .mul(assetTokenMultiplier)
            .div(assetToCashRate);
        ...
        emit CashboxPoolTokensBurnt(
            sender,
            cashboxPoolTokenAmount,
            outputAssetToken,
            outputCashAmount
        );
    }
    ...
    function depositAsset(uint256 inputAssetTokenAmount) external {
        ...
        uint256 cashToRedeem = (actualAssetReceived.mul(assetToCashRate)).div(
            assetTokenMultiplier
        );
        ...
        if (cashToRedeem <= contractCashBalance()) {
            ...
            uint256 remainingAsAssetTokens = (
                (cashToRedeem.sub(outputCashAmount)).mul(assetTokenMultiplier)
            ).div(assetToCashRate);
        ...
        emit AssetTokensRedeemed(
            sender,
            actualAssetReceived,
            outputCashAmount,
            mintableAssetPoolTokens
        );
    }
    ...
    function burnAssetPoolToken(uint256 assetPoolTokenAmount) external {
        ...
        uint256 cashToRedeem = (assetPoolTokenAmount.mul(assetToCashRate)).div(
            assetTokenMultiplier
        );
        ...
        if (cashToRedeem <= contractCashBalance()) {
            ...
            outputAssetToken = (remainingCash.mul(assetTokenMultiplier)).div(
                assetToCashRate
            );
            doTransferOut(address(assetToken), sender, outputAssetToken);
        }
        emit AssetPoolTokensBurnt(
            sender,
            assetPoolTokenAmount,
            outputAssetToken,
            outputCashAmount
        );
    }
