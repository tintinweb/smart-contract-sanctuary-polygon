...
/**
 * @dev This is the base contract for whitelisted allowance management. It is based on underlying proxied IWhitelist &
 * IAllowance implementations which means that every whitelists defined will need their own buckets from which to get allowance
 * from
 */
contract WhitelistedAllowanceProxy is WhitelistProxy, AllowanceProxy {
    ...
    function _addOrUseAllowance(address address_, bytes32 bucketName, uint256 amount, bool add/*, bool checkRole*/) internal override
    returns (uint256 allowance, Bucket memory bucket) {
        checkWhitelisted(address_, bucketName);
        return super._addOrUseAllowance(address_, bucketName, amount, add);
    }
...
contract WhitelistHandler is BaseWhitelistHandler {
    ...
    returns (bool[] memory whitelisted, Whitelist[] memory whitelistDef) {
        ...
        bool[] memory whitelisted_ = new bool[](addresses.length);
        Whitelist[] memory whitelistDef_ = new Whitelist[](addresses.length);
        for(uint256 i = 0 ; i < addresses.length ; i++) {
            // Get current whitelisting status
            (bool isWhitelisted_, Whitelist memory whitelist_) = isWhitelisted(addresses[i], whitelistNames[i]);
            whitelisted_[i] = isWhitelisted_;
            whitelistDef_[i] = whitelist_;
            // Nothing to update
            if(doWhitelist == isWhitelisted_) continue;
            // Check whitelister role
            _checkWhitelister(_msgSender(), whitelist_);
            /*if(!hasRole(WL_ADMIN_ROLE, _msgSender())) {
                _checkRole(whitelist_.whitelisterRole);
            }*/
        ...
        return (whitelisted_, whitelistDef_);
    }
...
abstract contract WhitelistProxy is ProxyDiamond, BaseWhitelistHandler {
    /** @dev Key used to reference the proxied IWhitelist contract */
    bytes32 public constant PROXY_IWhitelist = keccak256("IWhitelistProxy");
    ...
    constructor(address whitelistProxyAddress_, bool nullable, bool updatable, bool adminable) {
        _setWhitelistProxy(PROXY_IWhitelist, whitelistProxyAddress_, nullable, updatable, adminable);
    }
    ...
    function _setWhitelisted(address[] memory addresses, bytes32[] memory whitelistNames, bool doWhitelist) internal override whenNotPaused()
    returns (bool[] memory whitelisted, Whitelist[] memory whitelistDef) {
        (bool[] memory whitelistedStatuses_, Whitelist[] memory whitelistDefs_) = doWhitelist ? getWhitelistProxy().whitelistAll(addresses, whitelistNames) :
                                                                                                getWhitelistProxy().unwhitelistAll(addresses, whitelistNames);
        for(uint256 i = 0 ; i < whitelistedStatuses_.length ; i++) {
            // Address wasn't already whitelisted, role has to be checked against caller's ones
            if(whitelistedStatuses_[i] != doWhitelist) {
                //_checkRole(whitelistDef_[i].whitelisterRole);
                _checkWhitelister(_msgSender(), whitelistDefs_[i]);
            }
        }
        return (whitelistedStatuses_, whitelistDefs_);
    }
    function getWhitelistedCount(bytes32 whitelistName) external view returns (uint256) {
    ...
    function getWhitelistProxy() internal view returns (IWhitelist) {
        return IWhitelist(getProxy(PROXY_IWhitelist));
    }
    function _setWhitelistProxy(bytes32 key, address whitelistProxyAddress_, bool nullable, bool updatable, bool adminable) internal {
        _setProxy(key, whitelistProxyAddress_, IWhitelistInterfaceId, nullable, updatable, adminable);
    }
}
