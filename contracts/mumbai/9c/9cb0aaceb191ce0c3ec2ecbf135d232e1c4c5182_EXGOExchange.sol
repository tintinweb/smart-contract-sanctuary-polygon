...
contract EXGOExchange is OrderState, TransferSafe {
    ...
    function fiatMethod(uint256 _type,address _nftAddress , address _from , uint256 _id,string memory uri ,uint _nftType,uint amount)external payable{
        ...
        if(_type == 1){
            require(_nftAddress == address(_EXGOStore),"Invalid Token Address");
             BEP1155safeMintwithuriTransferFrom(IEXGOStore(_EXGOStore),owner(),_msgSender(),_id,1,uri);  
             require(payable(owner()).send(amount),"Transaction Failed 1");
             emit Fiat(_type,_nftAddress,owner(),_msgSender(),_id,_nftType,amount,block.timestamp);
        }
        else{
            require(_nftAddress != address(0x00),"Invalid Token Address");
            require(_nftType == 1 || _nftType == 2,"Invalid NFT Type");
            if (_nftType == 1){
                 BEP721safeTransferFrom(IBEP721(_nftAddress),_from,_msgSender(),_id);
            }
            else{
                BEP1155safeTransferFrom(IBEP1155(_nftAddress),_from,_msgSender(),_id,1);
            }
            require(payable(_from).send(amount),"Transaction Failed 2");
            emit Fiat(_type,_nftAddress,_from,_msgSender(),_id,_nftType,amount,block.timestamp);
        }
