...
contract TravelmythNFTs is ERC1155, IERC2981, Ownable, Pausable, ERC1155Burnable, ERC1155Supply {
    ...
    // No NFT will be minted during deployment, hundreds of NFTs will be minted later as needed.
    constructor() ERC1155("") {
        name = "Travelmyth NFTs";
        symbol = "TMNFT";
    }

    // The images will be temporarily stored locally and later (at the end of each year?) will be transferred to IPFS or elsewhere
    // This contract will be deployed on Polygon without bridge to Ethereum, so there's no interest to optimize it for low gas fees.
    ...
    // Set and view royalties
    function _setRoyalties(address newRoyaltyReceiver, uint256 newRoyaltyBps) internal {
        royaltyReceiver = newRoyaltyReceiver;
        royaltyBps = newRoyaltyBps;
    }

    function setRoyalties(address newRoyaltyReceiver, uint256 newRoyaltyBps) external onlyOwner {
        _setRoyalties(newRoyaltyReceiver,newRoyaltyBps);
    }
