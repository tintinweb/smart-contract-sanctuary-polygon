...
// File: contracts/interfaces/ISxTRelay.sol
...
// File: contracts/interfaces/ISxTRelayProxy.sol
...
// File: contracts/UserRequest.sol
...
// File: contracts/UserRequestBytesWithDecode.sol
...
contract UserRequestBytes is UserRequest, ConfirmedOwner {
    ...
    string[][] public currentResponseString2D;
    string public currentResponseString;
    uint256 public currentResponseUint256;
     ...
     */
    function runRequestQueryBytes(string memory query, string memory resourceId, string memory jobId) external nonReentrant returns(bytes32 requestId){
        require(
        ...
        requestId = getSxTRelayContract().requestQueryBytes(query, resourceId, address(this), this.saveQueryBytesResponse.selector, jobId);
    }

    function runRequestQueryString(string memory query, string memory resourceId, string memory jobId) external nonReentrant returns(bytes32 requestId){
        require(
            getChainlinkToken().approve(
                address(getSxTRelayContract().impl()), 
                getSxTRelayContract().feeInLinkToken()), 
        ...
        requestId = getSxTRelayContract().requestQueryString(query, resourceId, address(this), this.saveQueryStringResponse.selector, jobId);
    }

    function runRequestQueryString2D(string memory query, string memory resourceId, string memory jobId) external nonReentrant returns(bytes32 requestId){
        require(
            getChainlinkToken().approve(
                address(getSxTRelayContract().impl()), 
                getSxTRelayContract().feeInLinkToken()), 
        ...
        requestId = getSxTRelayContract().requestQueryString2D(query, resourceId, address(this), this.saveQueryString2DResponse.selector, jobId);
    }
    /**
    ...
    function runRequestView(string memory viewName, string memory jobId) external nonReentrant returns(bytes32 requestId){
        ...
        requestId = getSxTRelayContract().requestViewBytes(viewName, address(this), this.saveQueryBytesResponse.selector, jobId);
    }
     ...
     */
    function saveQueryBytesResponse(bytes32 requestId, bytes calldata data, string calldata errorMessage) external {
    ...
    function saveQueryStringResponse(bytes32 requestId, bytes calldata data, string calldata errorMessage) external {
        currentRequestId = requestId;
        // Store response
        latestError = errorMessage;
        currentResponseString = abi.decode(data, (string));
    }

    function saveQueryString2DResponse(bytes32 requestId, bytes calldata data, string calldata errorMessage) external {
        currentRequestId = requestId;
        // Store response
        latestError = errorMessage;
        // delete currentResponseString2D;
        currentResponseString2D = abi.decode(data, (string[][]));
        // string[][] memory newResponseString2D =  abi.decode(data, (string[][]));
        // for (uint256 i = 0; i < newResponseString2D.length; i++) {
        //     uint256 inLength = newResponseString2D[i].length;
        //     string[] memory row = new string[](inLength);
        //     for (uint256 j = 0; j < inLength; j++) {
        //         row[j] = newResponseString2D[i][j];
        //     }
        //     currentResponseString2D.push(row);
        // }
    }
     ...
     */
    function getDecodedStringResponse() public returns (string memory){
        currentResponseString = abi.decode(currentResponse, (string));
        return currentResponseString;
    }

    /**
     * @dev This function is used for decoding the latest bytes response to int
     * @return decodedStringResponse - decoded string response
     */
    function getDecodedUint256Response() public returns( uint256) {
        currentResponseUint256 = abi.decode(currentResponse, (uint256));
        return currentResponseUint256;
    }

    /**
     * @dev This function is used for decoding the latest bytes response to 2DString
     * @return decodedStringResponse - decoded string response
     */
    function getDecodedString2DResponse() public returns(string[][] memory) {
        // delete currentResponseString2D;
        currentResponseString2D =  abi.decode(currentResponse, (string[][]));
        return currentResponseString2D;
    }
}