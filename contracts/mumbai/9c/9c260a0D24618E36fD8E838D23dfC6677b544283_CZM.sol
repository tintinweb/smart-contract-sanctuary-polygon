...
contract CZM is ERC20, Ownable {
    ...
    function getAge(address user) external view returns (uint256) {
        ...
        uint256 ageInSeconds = currentTime - birthdate;
        uint256 ageIn360DayYears = ageInSeconds / (360 days);
        return ageIn360DayYears;
    }

    function userBirthdayFunction(address user) external view returns (string memory) {
        uint256 birthdate = userProfiles[user].birthdate;
        uint256 currentDecentralizedAge = getDecentralizedAge();
        uint256 nextBirthday = birthdate + (360 days);

        if (currentDecentralizedAge >= nextBirthday) {
            // User has already celebrated their decentralized birthday
            return "You were born in the TimeVerse.";
        } else if (currentDecentralizedAge == birthdate) {
            // User is celebrating their decentralized birthday today
...
function aiFunction() external view returns (string memory) {
    require(aiEnabled, "Timeverse Assistant is disabled");

    uint256 currentDecentralizedAge = getDecentralizedAge();

    if (currentDecentralizedAge >= CYCLE_ZERO_START) {
        uint256 currentCycle = (currentDecentralizedAge - CYCLE_ZERO_START) / CYCLE_DURATION;
        uint256 nextNewYear = CYCLE_ZERO_START + (currentCycle + 1) * CYCLE_DURATION;

        if (currentDecentralizedAge == nextNewYear - 360 days) {
            return "Happy new year in the TimeVerse!";
        } else {
            uint256 remainingDays = nextNewYear - currentDecentralizedAge;
            return string(abi.encodePacked("Remaining days until the next new year in the TimeVerse: ", uint256ToString(remainingDays)));
        }
    }
