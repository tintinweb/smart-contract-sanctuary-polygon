...
contract MerkleTraitChecker is Ownable, ITraitChecker {
    ...
    string public trait;
    string public traitCategory;

    constructor(string memory _trait, string memory _traitCategory) Ownable() {
        trait = _trait;
        traitCategory = _traitCategory;
    }
    ...
    function getTraitWithProof(string calldata _username, bytes32[] calldata proof) external view returns (string memory, string memory) {
        require(initialized, 'MerkleTraitChecker: not initialized');
        bytes32 leaf = keccak256(bytes.concat(keccak256(abi.encode(_username))));
