...
library CoreTools {
     ...
     */
    function marginAdjustedOdds(uint256[] memory probabilities, uint256 margin)
        internal
        pure
        returns (uint256[] memory odds)
    {
        ...
        for (; iteration < MAX_ITERATIONS; ++iteration) {
            ...
            uint256 oddsSpread = FixedMath.ONE - FixedMath.ONE.div(spread);
    ...
    /**
     * @notice Calculate the betting odds with bookmaker commission `margin` for each outcome of condition.
     */
    function calcOdds(uint128[] memory funds, uint256 margin)
        internal
        pure
        returns (uint256[] memory odds)
    {
        ...
        if (margin > 0) {
            ...
            for (uint256 i = 0; i < length; ++i) {
                probabilities[i] = calcProbability(funds[i], fund);
            }
            odds = marginAdjustedOdds(probabilities, margin);
        } else {
            odds = new uint256[](length);
            for (uint256 i = 0; i < length; ++i) {
                uint256 odds_ = (fund).div(funds[i]);
                if (odds_ == FixedMath.ONE) revert LargeFundsRatio();
     ...
     */
    function calcProbability(uint256 outcomeFund, uint256 fund)
        internal
        pure
        returns (uint256 probability)
    {
        probability = outcomeFund.div(fund);
        if (probability < 1000) revert LargeFundsRatio(); // This constraint helps to avoid errors in marginAdjustedOdds call
    }
}
...
contract PrematchCore is CoreBase, IPrematchCore {
        ...
        uint128 payout = odds.mul(amount).toUint128();
        {
            ...
            for (uint256 i = 0; i < length; ++i) {
                if (i != outcomeIndex) {
                    ...
                    CoreTools.calcProbability(virtualFunds[i], virtualFund);
                }
            }
        }

        condition.virtualFunds = virtualFunds;
        _changeFunds(condition, outcomeIndex, amount, payout, amount, false);
     ...
     * @param  conditionId the match or condition ID
     * @param  outcomeWin the ID of the winning outcome of the condition
     */
        ...
        override
    {
        ...
        _resolveCondition(
            ...
            outcomeWin,
            condition.payouts[getOutcomeIndex(conditionId, outcomeWin)]
        );
    }
