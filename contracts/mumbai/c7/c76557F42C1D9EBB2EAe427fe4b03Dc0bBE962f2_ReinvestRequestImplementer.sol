...
contract ReinvestRequestImplementer {
    ...
    mapping ( uint32 => mapping ( uint16 => mapping (uint32 => mapping ( bool => uint256) ) ) ) public dataUserReinvetments; //fromProjId=>tokenId=>toProjId=>isCrypto=>value
    ...
    function incrementValue(uint16 tokenId, uint32 fromProjId, uint32 toProjId, bool isCrypto, uint256 value) public onlyAllowed {
        if (dataUserReinvetments[fromProjId][tokenId][toProjId][isCrypto] == 0) {
            dataDestProjects[fromProjId][tokenId].push(toProjId);
        }
        dataUserReinvetments[fromProjId][tokenId][toProjId][isCrypto] += value;
    }

    function decrementValue(uint16 tokenId, uint32 fromProjId, uint32 toProjId, bool isCrypto, uint256 value) public onlyAllowed {
        require(dataUserReinvetments[fromProjId][tokenId][toProjId][isCrypto] >= value, "ReinvestImplementer: Not enough reinvestment");
        dataUserReinvetments[fromProjId][tokenId][toProjId][isCrypto] -= value;
        if( dataUserReinvetments[fromProjId][tokenId][toProjId][false] == 0 && dataUserReinvetments[fromProjId][tokenId][toProjId][true] == 0) {
    ...
    function getValue(uint16 tokenId, uint32 fromProjId, uint32 toProjId, bool isCrypto) public view returns (uint256) {
        return dataUserReinvetments[fromProjId][tokenId][toProjId][isCrypto];
    }

    function getUserTotalReinvest(uint16 tokenId, uint32 fromProjId) public view returns (uint256) {
        uint256 totalReinvest = 0;
        uint32[] storage destProjects = dataDestProjects[fromProjId][tokenId];
        for (uint i = 0; i < destProjects.length; i++) {
            totalReinvest += dataUserReinvetments[fromProjId][tokenId][destProjects[i]][false];
            totalReinvest += dataUserReinvetments[fromProjId][tokenId][destProjects[i]][true];
        }
        return totalReinvest;
    }

    function getUserTotalReinvestToProj(uint16 tokenId, uint32 fromProjId, uint32 toProjId) public view returns (uint256) {
        return dataUserReinvetments[fromProjId][tokenId][toProjId][false] + dataUserReinvetments[fromProjId][tokenId][toProjId][true];
    }
