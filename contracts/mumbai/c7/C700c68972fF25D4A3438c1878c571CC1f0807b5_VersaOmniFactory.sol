...
abstract contract LzApp is Ownable, ILayerZeroReceiver, ILayerZeroUserApplicationConfig {
    ...
    function setTrustedRemote(uint16 _remoteChainId, bytes memory _path) public onlyOwner {
        trustedRemoteLookup[_remoteChainId] = _path;
        emit SetTrustedRemote(_remoteChainId, _path);
    }

    function setTrustedRemoteAddress(uint16 _remoteChainId, bytes memory _remoteAddress) public onlyOwner {
    ...
    function setMinDstGas(uint16 _dstChainId, uint16 _packetType, uint _minGas) public onlyOwner {
    ...
    function setPayloadSizeLimit(uint16 _dstChainId, uint _size) public onlyOwner {
 ...
 * @notice The validator is an extension of a `module` which implements `IOmniValidator` interface
 ...
 */
abstract contract OmniValidatorManager is SelfAuthorized {
    ...
    function _enableValidator(address validator, ValidatorType validatorType, bytes memory initData) internal {
        ...
        IOmniValidator(validator).initWalletConfig(initData);
    ...
    function _disableValidator(address prevValidator, address validator) internal {
        try IOmniValidator(validator).clearWalletConfig() {
