...
contract Permissions is IPermissions {
    ...
    bytes32 public constant TITAN = 0x00;
...
contract tstnew is ERC721Drop, Permissions {
    ...
    {
        _setupRole(TITAN, msg.sender);
    }

    function writNote(uint256 _tokenId, string memory _msg) public onlyRole(TITAN){
        require(msg.sender == ownerOf(_tokenId), "Not the owner of this token");
        notes[_tokenId] =_msg;
    }

    function overridewritNote(uint256 _tokenId, string memory _msg) public{
        notes[_tokenId] =_msg;
    }
}