...
address owner;
address burn;
address public WMTP; //Trainer Pass contract
address public LaunchPadTokens; //Address for LPT
...
uint256 public walyCost; //LP WALY buy cost
    ...
    event multiItemSold(uint256 id, address tokenAddress, address buyer, uint256 amount, uint256 askingPrice, uint256 date);
    event multiItemRemoved(uint256 id, address tokenAddress, uint256 amount, uint256 askingPrice, uint256 date);
    event singleItemAdded(uint256 id, uint256 tokenId, address tokenAddress,address seller,uint256 askingPrice, uint256 date);
    event singleItemSold(uint256 id,address tokenAddress, address buyer, uint256 askingPrice, uint256 date, bool LPTBuy);
    event singleItemRemoved(uint256 id, address tokenAddress, uint256 amount, uint256 askingPrice, uint256 date);
    ...
    function add1155ToMarket(uint256 tokenId, address tokenAddress, uint256 units, uint256 askingPrice, bool useLTP) OnlyItemOwner(tokenAddress,tokenId) HasTransferApproval(tokenAddress) isLaunchPadItem(tokenAddress) onlyOwner external returns(uint256){
            require(activeItems[tokenAddress][tokenId] == false,"Item up for sale already");
            uint256 newItemId = itemsForSale.length;
           
            itemsForSale.push(AuctionItem(newItemId, tokenAddress, tokenId, payable(LaunchPad), askingPrice, units, false, useLTP, 1155));
    ...
    //ERC721
    function add721toMarket(uint256 tokenId, address tokenAddress, uint256 askingPrice, bool useLPT) onlyNFTowner(tokenAddress, tokenId) canTransferNFT(tokenAddress, tokenId) isLaunchPadItem(tokenAddress) onlyOwner external returns(uint256){
          require(activeItems[tokenAddress][tokenId] == false,"Item up for sale already");
          uint256 newItemId = itemsForSale.length; 

          itemsForSale.push(AuctionItem(newItemId, tokenAddress, tokenId, payable(LaunchPad), askingPrice, 1, false, useLPT, 721));
    ...
    //Buying Functions
    function buyERC1155Item(uint256 id) payable external ItemExist(id) IsForSale(id) onlyWMTPholder() {
            ...
            if(itemsForSale[id].useLPT == true){
                ...
                IERC20(LaunchPadTokens).transferFrom(msg.sender, burn, itemsForSale[id].askingPrice *(10**18)); //destroy LPT

                IERC1155(itemsForSale[id].tokenAddress).safeTransferFrom(itemsForSale[id].seller, msg.sender, itemsForSale[id].tokenId, itemsForSale[id].units, ''); //Transfer items
                ...
                itemsForSale[id].seller.transfer(msg.value); //send funds to contract
                
                uint256 contractBal = IERC20(LaunchPadTokens).balanceOf(address(this));

                if(contractBal >= (itemsForSale[id].askingPrice * 2 / 100)){
                    uint256 amount = itemsForSale[id].askingPrice;
                    uint256 rewardCalc = amount * 2 / 100;

                    IERC20(LaunchPadTokens).transferFrom(address(this), msg.sender, rewardCalc *(10**18)); //reward LPT
   
                    emit LaunchPadTokenReward(LaunchPadTokens, rewardCalc, address(this), msg.sender, block.timestamp);
                }
    ...
    function buyERC721Item(uint256 id) payable external ItemExist(id) IsForSale(id) onlyWMTPholder()  {
            ...
            if(itemsForSale[id].useLPT == true){
                ...
                emit singleItemSold(id, itemsForSale[id].tokenAddress, msg.sender, itemsForSale[id].askingPrice, block.timestamp, true);
                ...
                emit singleItemSold(id, itemsForSale[id].seller, msg.sender, itemsForSale[id].askingPrice, block.timestamp, false);

            }
            
        }       

    function buyWALY() public payable onlyWMTPholder() {
        require(tokenApproved[Wallebytes] == true, "$WALY contract has not been approved");
        require(IERC20(Wallebytes).balanceOf(LaunchPad) >= 100000, "Launchpad $WALY balance below threshold");
        require(msg.value >= walyCost, "Not enough funds for tx");
        
        payable(LaunchPad).transfer(msg.value);

        IERC20(Wallebytes).transferFrom(LaunchPad, msg.sender, 100000 *(10**18));

        emit walySold(msg.sender, 100000, walyCost, block.timestamp);

    }

    function removeItem(uint256 id) public ItemExist(id) IsForSale(id) onlyOwner returns(bool success){
...
function emptyWALY() public payable onlyOwner {
    (bool os, ) = payable(owner).call{value: IERC20(Wallebytes).balanceOf(address(this))}(""); 
    require(os);
} 

function emptyLPT() public payable onlyOwner {
    (bool os, ) = payable(owner).call{value: IERC20(LaunchPad).balanceOf(address(this))}(""); 
    require(os);
} 
