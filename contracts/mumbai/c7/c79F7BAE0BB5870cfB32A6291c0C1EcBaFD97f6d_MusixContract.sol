...
contract MusixContract is ERC721Enumerable,Ownable {
    ...
    uint256 public startDate = 1655290800;
    ...
    function getCurrentPrice() public view returns (uint){
        uint daysAfterLaunch = 0;
        if(block.timestamp - startDate > 0){
            daysAfterLaunch = (block.timestamp - startDate) / (1* 1 days);
        }
        
        uint priceCalculation = MIN_PRICE+(daysAfterLaunch* 0.01168 ether);
    ...
    function tryOut() public view returns (uint256){
            return block.timestamp;
    }
    function tryOutSecond() public view returns (uint256){
            return startDate;
    }
