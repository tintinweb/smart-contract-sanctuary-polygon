...
contract NFT150 is ERC1155Tradeble {
	...
	mapping(address => mapping(uint256 => bool)) public migratedBalance;
	mapping(uint256 => bool) public migratedFromOrderId;
	...
	function migrateBalancesFromOrder(
		address _oldMarket,
		uint256 _orderId
	) public onlyOwner {

		require(migratedFromOrderId[_orderId] == false, "migrateBalancesFromOrder: new 1155 for this order already minted");
		...
		(
			...
			) = IPolkaMarketVersion(_oldMarket).orders(_orderId);

		require(isOnsale == true, "migrateBalancesFromOrder: isOnsale must be true");
		require(isERC721 == false, "migrateBalancesFromOrder: This order should be a 1155 order");
		require(quantity > 0, "migrateBalancesFromOrder: quantity must be grater than 0");

		migratedFromOrderId[_orderId] = true;
		_mint(owner, tokenId, quantity, '0x');
	}
