...
interface _Connector {
    function createNFT(address creator, string memory NFTData, uint256 numberOfTokens) external returns (address);
}
contract NFTMarket is ERC1155Holder {
    ...
    /* returns all NFTs */
    function fetchNFTs() public view returns (uint, _NFT[] memory, address[] memory, string[] memory, uint[] memory) {
        _NFT[] memory items = new _NFT[](_NFTIds);
        address[] memory creators = new address[](_NFTIds);
        string[] memory uris = new string[](_NFTIds);
        uint[] memory ids = new uint[](_NFTIds);
        for (uint256 NFTID=1; NFTID <= _NFTIds; NFTID++) {
            items[NFTID-1] = _NFT(idToNFT[NFTID]);
            creators[NFTID-1] = _NFT(idToNFT[NFTID]).getCreator();
            uris[NFTID-1] = _NFT(idToNFT[NFTID]).getUri();
            ids[NFTID-1] = NFTID;
        }
        return (_NFTIds, items, creators, uris, ids);
    }
    /* gets all user's nfts */
    function fetchUserNFTs(address user) public view returns (uint, _NFT[] memory, address[] memory, string[] memory, uint[] memory) {
        ...
        for (uint NFTId=1; NFTId<=_NFTIds; NFTId++) {
            ...
            for (uint tokenId=1; tokenId<=amount; tokenId++) {
                if (_NFT(idToNFT[NFTId]).getOwner(tokenId) == user){
                    ...
                    creators[count] = _NFT(idToNFT[NFTId]).getCreator();
                    uris[count] = _NFT(idToNFT[NFTId]).getUri();
                    ids[count] = NFTId;
                    count++;
                    break;
                }
    ...
    function fetchCreatedByUserNFTs(address user) public view returns (uint, _NFT[] memory, address[] memory, string[] memory, uint[] memory) {
        uint count = 0; // counting nfts
        for (uint NFTId=1; NFTId<=_NFTIds; NFTId++) {
    ...
    function fetchNFTsOnSale() public view returns (uint, _NFT[] memory, address[] memory, string[] memory, uint[] memory, uint256[] memory, uint[] memory) {
        uint count = 0; // counting nfts
        for (uint NFTId=1; NFTId<=_NFTIds; NFTId++) {
            uint amount = _NFT(idToNFT[NFTId]).getAmount();
            for (uint tokenId=1; tokenId<=amount; tokenId++) {
        ...
        string[] memory uris = new string[](count);
        uint[] memory ids = new uint[](count);
        uint256[] memory minTokenPrices = new uint[](count);
        uint[] memory tokensOnSale = new uint[](count);
        count = 0;
        for (uint NFTId=1; NFTId<=_NFTIds; NFTId++) {
            uint amount = _NFT(idToNFT[NFTId]).getAmount();
            // if any token on sale, adding this nft to nft array
            uint256 min_price = 0;
            uint countTokensOnSale = 0;
            for (uint tokenId=1; tokenId<=amount; tokenId++) {
                if (_NFT(idToNFT[NFTId]).tokenOnSale(tokenId)){
                    uint256 tokenPrice = _NFT(idToNFT[NFTId]).getPrice(tokenId);
                    if (countTokensOnSale == 0) {
                        min_price = tokenPrice;
                    } else if (min_price > tokenPrice) {
                        min_price = tokenPrice;
                    }
                    countTokensOnSale++;
                    items[count] = _NFT(idToNFT[NFTId]);
                    ...
                    uris[count] = _NFT(idToNFT[NFTId]).getUri();
                    ids[count] = NFTId;
                    minTokenPrices[count] = tokenPrice;
                    tokensOnSale[count] = countTokensOnSale;
                    if (countTokensOnSale == 1) {
                        count++;
                    }
        ...
        return (count, items, creators, uris, ids, minTokenPrices, tokensOnSale);
    }
    /* creates NFT */
    function createNFT(string memory NFTData, uint256 numberOfTokens) public payable returns (uint256) {
        ...
        idToNFT[newNFTId] = _Connector(connectorAddr).createNFT(msg.sender, NFTData, numberOfTokens);
        return newNFTId;
    }
    /* creates NFT Kit */
