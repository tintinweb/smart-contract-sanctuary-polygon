...
contract Marketplace is Ownable {
	...
	*/    
    ...
    event ProviderModified(bytes32 id, string name, string description, string logo, string banner, address addr, uint8 commission, uint256 dateCreated, bool isEnabled, bool isNew);
    event ArtModified(bytes32 id, bytes32 providerID, string name, string description, string uri, string metadataUri, uint16 totalSupply, uint256 price, uint256 dateCreated, bool isEnabled, bool isNew);
    ...
    * @param id Provider id, zero will create a new one
    * @param name Provider name 
	* @param description Provider Description 
	* @param logo The provider's logo
	...
	*/
    ...
    function addEditProvider(bytes32 id, string memory name, string memory description, string memory logo, string memory banner, address addr, uint8 commission) public notStopped {
        ...
        if(id == 0) {
            ...
            bytes32 pid = keccak256(abi.encodePacked(msg.sender, name, block.timestamp));
            Provider memory p = Provider(pid, name, description, logo, banner, payable(addr), commission, block.timestamp, true);

            providers.push(pid);
            providerByOwner[addr] = pid;
            providerByID[pid] = p;

            emit ProviderModified(p.id, p.name, p.description, p.logo, p.banner, p.addr, p.commission, p.dateCreated, p.isEnabled, true);
        } else {
            ...
            emit ProviderModified(p.id, p.name, p.description, p.logo, p.banner, p.addr, p.commission, p.dateCreated, p.isEnabled, false);
        }
    }
    ...
    */
    ...
    function addEditArtwork(bytes32 id, string memory name, string memory description, string memory uri, string memory metadataUri, uint256 price, uint16 totalSupply) notStopped public {
        require(bytes(name).length > 0, "The name can't be empty");
        require(bytes(description).length > 0, "The description can't be empty");
        ...
        bytes32 artID;
        if(id == 0) {
            artID = keccak256(abi.encodePacked(msg.sender, name, block.timestamp));
            Art memory art = Art(artID, providerID, name, description, uri, metadataUri, totalSupply, 0, price, block.timestamp, true);

            artsByProvider[providerID].push(artID);
            artByID[artID] = art;

            emit ArtModified(art.id, art.providerID, art.name, art.description, art.uri, art.metadataUri, art.totalSupply, art.price, art.dateCreated, art.isEnabled, true);
        } else {
            artID = id;

            Art memory art = artByID[id];

            require(art.providerID == providerID, "only the owner can edit an art");

            art.name = name;
            art.description = description;
            art.uri = uri;
            art.metadataUri = metadataUri;
            art.totalSupply = totalSupply;
            art.price = price;

            artByID[id] = art;
            
            emit ArtModified(art.id, art.providerID, art.name, art.description, art.uri, art.metadataUri, art.totalSupply, art.price, art.dateCreated, art.isEnabled, false);
        }
    }

    /** @dev Enables / Disables an Art Listing
    * @param id The provider to edit
    * @param value true -> enabled, false -> disabled
	*/
    function setIsArtEnabled(bytes32 id, bool value) public notStopped {
        Art memory a = artByID[id];
        bytes32 providerID = getProviderByAddress(msg.sender);

        require(a.id != 0, "The art doesn't exist");
        require(owner() == msg.sender || a.providerID == providerID, "");

        a.isEnabled = value;
        artByID[id] = a;

        emit ArtModified(a.id, a.providerID, a.name, a.description, a.uri, a.metadataUri, a.totalSupply, a.price, a.dateCreated, a.isEnabled, false);
    }
