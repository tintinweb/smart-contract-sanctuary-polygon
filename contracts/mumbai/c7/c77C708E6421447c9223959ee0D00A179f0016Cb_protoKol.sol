...
contract protoKol is iProtoKol {
    ...
    //In Binance
    // address private constant UNISWAP_ROUTER_ADDRESS = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
    // address private factory = 0xB7926C0430Afb07AA7DEfDE6DA862aE0Bde767bc;
    ...
    //CONSTRUCTOR
    constructor(address _admin, address _kolToken,address _usdt,address _staking) {
        admin = _admin;
        kolToken = _kolToken;
        _USDT = _usdt;
   ...
   */
    function createCampaign(
        ...
        _newCampaign.requiredInvestment =
            (_newCampaignInput.requiredInvestment * 10**IERC20(_USDT).decimals()) /
            10**18;
   ...
   */
    function investInCampaign(
        ...
        _investment = (_investment * 10**IERC20(tokenAddress).decimals()) / 10**18;
        // Campaign memory campaignDetails = retriveCampaign(_campaignId);
        if (campaigns[_campaignId].campaignData.preSaleToken != _USDT) {
            ...
            if (tokenAddress == kolToken) {
                ...
                // console.log("Amount Out ",amount[1]);
                require(
                    (amount[1] * (10000 - _transactionPer)) / 10000 <=
                ...
                amount = _swapRouter(_investment, amount[1], path, address(this));
                _investment = amount[1];
                //console.log("Amount Out 2 ",amount[1]);
            } else {
                _investment = (_investment * (10000 - _transactionPer)) / 10000;
                require(
                ...
                _safeTransferFrom(_USDT, msg.sender, address(this), _investment);
                _safeTransferFrom(_USDT, msg.sender, _vault, _totalInvestment - _investment);
            }
        }
   ...
   */
    function depositPreSaleTokens(uint256 _campaignId, uint256 _amount) external {
        ...
        IPST(token).transferFrom(msg.sender, address(this), _amount);
   ...
   */
    function claimPreSaleTokens(
        ...
        if (campaigns[_campaignId].vestingData.isVestingInEnabled) {
            ...
            uint256 totalReward = (investmentDetails.eligibleReward * 10**IERC20(_USDT).decimals() * progress) /
                (10000 * 10**18);
            ...
            require(IPST(preSale).balanceOf(address(this)) >= _reward, "Protokol:Not enough funds");
            ...
            require(IPST(preSale).balanceOf(address(this)) >= eligibleRwd, "Protokol:Not enough funds");
   ...
   */
    function claimBackInvestmentByKOL(
        ...
        path[0] = _USDT;
        path[1] = kolToken;
        uint256[] memory amount = UniswapV2Library.getAmountsOut(factory, _invest, path);
   ...
   */
    function _blackListedKOLFundsHandling(
        ...
        uint256 balance = IPST(preSale).balanceOf(address(this));
        if (balance < _reward) {
            // console.log(balance);
            IPST(preSale).transfer(msg.sender, balance);
        ...
        if (campaigns[_campaignId].campaignData.preSaleToken != _USDT) {
            ...
            if (balance < investedCampaignDetails[msg.sender][_campaignId].leftOverInvestment) {
                ...
                _safeTransfer(_USDT, msg.sender, investedCampaignDetails[msg.sender][_campaignId].leftOverInvestment);
   ...
   */
    ...
    //for testing
    function updateKolToken(address _newAddress) external {
        kolToken = _newAddress;
    }
}
