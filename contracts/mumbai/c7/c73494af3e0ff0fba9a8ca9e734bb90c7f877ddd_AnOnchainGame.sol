...
contract AnOnchainGame is EssentialERC2771Context {
    ...
    function setMap(Space[] calldata spaces, uint256[][] calldata startSpaces) external onlyOwner {
        ...
        for (uint256 index = 0; index < teamCount; index++) {
            teamStartingSpaces[uint8(index)] = startSpaces[index];
        }
    }
