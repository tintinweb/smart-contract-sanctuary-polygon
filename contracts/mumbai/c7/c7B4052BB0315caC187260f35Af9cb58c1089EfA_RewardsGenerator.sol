...
contract RewardsGenerator is IRewardsGenerator, OwnableUpgradeable, AbstractDependant {
    ...
    uint256 public policyBookCustodianRatio; // % with precision 10^25
    ...
    event PercentageCustSet(uint256 policyBookCustodianRatio);
    ...
    function setPolicyBookCustodianRatio(uint256 _policyBookCustodianRatio) external onlyOwner {
        policyBookCustodianRatio = _policyBookCustodianRatio;

        _updateCumulativeSum(address(0));

        emit PercentageCustSet(_policyBookCustodianRatio);
    }
    ...
    function updatePolicyBookShare(
        ...
        if (contractType == IPolicyBookFabric.ContractType.STABLECOIN) {
            newRewardMultiplier = newRewardMultiplier.mul(policyBookStableRatio).uncheckedDiv(
                PERCENTAGE_100
            );
        } else if (contractType == IPolicyBookFabric.ContractType.CUSTODIAN) {
            newRewardMultiplier = newRewardMultiplier.mul(policyBookCustodianRatio).uncheckedDiv(
                PERCENTAGE_100
            );
        }
