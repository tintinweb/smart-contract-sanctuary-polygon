...
contract MarketplaceStorage {
  ...
  struct Order {
    ...
    address nftAddress;
    // Price (in Shiv Token) for the published item
    uint256 price;
  ...
  uint256 public publicationFeeInShivToken;
...
contract Marketplace is Ownable, Pausable, MarketplaceStorage {
  ...
  /**
    ...
    * @param _publicationFee - Fee amount in Shiv Token this contract charges to publish an item
    */
  function setPublicationFee(uint256 _publicationFee) external onlyOwner {
    publicationFeeInShivToken = _publicationFee;
    emit ChangedPublicationFee(publicationFeeInShivToken);
  }
  ...
  /**
    ...
    * @param priceInShivToken - Price in Shiv Token for the supported coin
    ...
    internal
  {
    ...
    require(priceInShivToken > 0, "Price should be bigger than 0");
    ...
    orderByAssetId[nftAddress][assetId] = Order({
      ...
      price: priceInShivToken,
      expiresAt: expiresAt
    });

    // Check if there's a publication fee and
    // transfer the amount to marketplace owner
    if (publicationFeeInShivToken > 0) {
      require(
        acceptedToken.transferFrom(sender, owner(), publicationFeeInShivToken),
