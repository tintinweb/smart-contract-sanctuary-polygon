...
contract VCRegistry is Initializable,
    OwnableUpgradeable,
    UUPSUpgradeable,
    ...
    struct CredentialSubject { //VC Data
        //string id;
        address ethereumAddress;
        string _type;
        //string typeSchema;
    ...
    struct VerifiableCredential { //VC W3C Standard
        //string _context;
        //string _type;
        //string id;
        Issuer issuer;
    ...
    mapping(address => mapping(uint256 => VerifiableCredential)) public VCRegistryTable;
    mapping(address => bool) public authorizedIssuers;
    mapping(uint256 => bool) hasMintedVC;
    ...
    function setAuthorizedSigner(address newSignerAddress, bool isAuthorized) public onlyOwner {
        authorizedIssuers[newSignerAddress] = isAuthorized;
    }

    function isValidVC(VerifiableCredential memory vc, bytes memory signature) internal view returns(bool) {
        ...
        address signer = ECDSAUpgradeable.recover(prefixedHashMessage, signature);
        return(authorizedIssuers[signer]==true);
    }
   
    function registerVC(VerifiableCredential memory vc, bytes memory signature) public {
        require(isValidVC(vc, signature) == true,"Not Valid VC");

        bytes32 uuidBytes = keccak256(abi.encode(vc));
        uint256 uuid = uint256(uuidBytes);
        require(hasMintedVC[uuid] != true, "Has Exist");

        hasMintedVC[uuid] = true;
        VCRegistryTable[vc.credentialSubject.ethereumAddress][uuid] = vc;
        _safeMint(vc.credentialSubject.ethereumAddress, vcIdCounter.current());
        vcIdCounter.increment();
    }

    function registerVCBatch(VerifiableCredential[] memory vcs, bytes[] memory signatures) external {
    
        require(vcs.length == signatures.length, "No Matched Data Length");
        for(uint i = 0; i < vcs.length; i++) {
            registerVC(vcs[i], signatures[i]);
        }
    }

    function getVC(address user, uint256 uuid) public view returns (VerifiableCredential memory) {
        return VCRegistryTable[user][uuid];
    }

    function updateVC(address user, uint256 uuid, VerifiableCredential memory newVC) public {
        require(authorizedIssuers[msg.sender] == true, "No Access to Update");
        VCRegistryTable[user][uuid] = newVC;
    }

    function revokeVC(address user, uint256 uuid) public {
        require(authorizedIssuers[msg.sender] == true, "No Access to Delete");
