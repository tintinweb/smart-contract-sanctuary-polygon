...
5/1號 ：15:00 空投一次 
2022年5月1日星期日下午3点00分 GMT+08:00
1651388400

5/2號18:00～5/3號21:00 第一次白名單 
2022年5月2日星期一晚上6点00分 GMT+08:00
1651485600

2022年5月3日星期二晚上9点00分 GMT+08:00
1651582800

5/4號21:30～5/5號21:30 第二次白名單 
2022年5月4日星期三晚上9点30分 GMT+08:00
1651671000

2022年5月5日星期四晚上9点30分 GMT+08:00
1651757400

公售。5/10號 時間：18:00～不截止 價格：0.069 ETH
2022年5月10日星期二晚上6点00分 GMT+08:00

1652176800

收取空投的地址
0x5809426f4700e104976BDA3a3547f85C292f091A
...
// •	NFT能不能被銷毀？是NFT擁有者可以銷毀，還是合約擁有者才能銷毀？合約擁有者
// •	用戶鑄造的時候能不能選TokenID?不可以，隨機的
*/

contract mf is ERC721Enumerable, Ownable {
    using Strings for uint256;

    //   using SafeMath for uint256;

    constructor() ERC721("TBC-name", "TBC-symbol") {
    ...
    //首次白名单价，二次白名单价，开售价
    uint256 public firstWlCost = 50000000000000000;
    uint256 public secondWlCost = 55000000000000000;
    uint256 public finalCost = 69000000000000000;

    //时间设定
    //1651388400 5/1號 15:00
    //1651485600 5/2號 18:00
    //1651582800 5/3號 21:00
    //1651671000 5/4號 21:30
    //1651757400 5/5號 21:30
    //1652176800 5/10號 18:00
    ...
    //收款钱包，可修改
    address public payoutAddress = 0x70E1eF93a64BC090E10517F5E381f4fa6c21B86A;
    ...
    //取随机不重复值 + 前面要保留多少号
    function randomIndex() public returns (uint256) {
        ...
        returns (uint256)
    {
        ...
        while (i < _ids.length) {
            if (ownerOf(_ids[i]) == msg.sender) {
                ERC721(address(this)).transferFrom(msg.sender, _to, _ids[i]);
            }
            i += 1;
        }
        return (i);
