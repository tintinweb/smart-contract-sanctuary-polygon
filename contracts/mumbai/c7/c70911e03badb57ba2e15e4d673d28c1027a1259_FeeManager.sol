...
contract FeeManager is IFeeManager, Ownable {
     ...
     */
    function setCollectionFee(address collection, uint16 fee)
        external
        override
        onlyOwner
    {
     ...
     * @param collection address of the collection (ERC-721 or ERC-1155)
     */
    function removeCollectionFee(address collection)
        external
        override
        onlyOwner
    {
    ...
    function getFeeAmount(address collection, uint256 amount)
        ...
        returns (uint256)
    {
        return amount.mul(getFee(collection)).div(PERCENTS_DIVIDER);
    }
}