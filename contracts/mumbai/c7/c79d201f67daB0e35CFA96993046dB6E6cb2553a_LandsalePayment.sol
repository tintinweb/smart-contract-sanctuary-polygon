...
contract LandsalePayment is AccessControl, Ownable, ReentrancyGuard {
    ...
    event landCategorySupplyIncreased(
        uint256 indexed additionalSupply,
        string indexed category
    );
    event landCategorySupplyDecreased(
        uint256 indexed additionalSupply,
        string indexed category
    );
    event newSlotAdded(
    ...
    function adminMint(
        ...
        require(
            totalSupply.add(_tokenId.length) <= maxSupply,
    ...
    function updateTVKperUSDprice(uint256 _TVKperUSDprice) public {
        require(
            hasRole(admin_role, _msgSender()),
            "Landsale: Must have admin role to update tvk price"
        );
    ...
    function updateETHperUSDprice(uint256 _ETHperUSDprice) public {
        require(
            hasRole(admin_role, _msgSender()),
            "Landsale: Must have admin role to update eth price"
        );
    ...
    function getTokenLandCategory(uint256 _tokenId)
        public
        view
