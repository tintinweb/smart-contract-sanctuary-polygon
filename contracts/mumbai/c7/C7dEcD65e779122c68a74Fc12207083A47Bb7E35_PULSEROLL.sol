...
contract PULSEROLL is VRFConsumerBase {
    ...
    returns(bytes32)
    {
        ...
        maxPendingPayouts = maxPendingPayouts + playerBetValue[requestId];
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal payoutsAreActive override 
    {
        ...
        maxPendingPayouts = maxPendingPayouts - playerBetValue[requestId]; //reduce the payoutsfor theuser
    ...
    function ownerRefundPlayer(
        ...
        require(originalPlayerBetId != bytes32(0),"The bytes should not be empty");
        require(sendTo != address(0),"address should not be equal to zero address");
        require(originalPlayerProfit != 0,"User will have some profit if he betted with right parameters");
        require(originalPlayerBetValue != 0,"Bet value should not be zero");
        require(originalPlayerBetValue <= address(this).balance ,"Contract Doesn't have that much amount");
        /* safely reduce pendingPayouts by playerProfit[rngId] */
        maxPendingPayouts = (maxPendingPayouts - originalPlayerBetValue);//take out the player profit from maxpendingpayouts
        /* send refund */
