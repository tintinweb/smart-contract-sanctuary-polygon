...
contract ContentTracer is ProvidersWrapper, OwnableUpgradeable {
	...
	/// @param content ipfs content
	/// @param size ipfs content size
	event Remove(address provider, bytes32 account, string content, uint256 size);
	...
	function _remove(
		...
		uint256 size = contentSizes[provider][account][content];
		delete contentSizes[provider][account][content];

		emit Remove(provider, account, content, size);
	}
