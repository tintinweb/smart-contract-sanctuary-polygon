...
contract Contract is ERC1155Holder {
    DropERC1155 public immutable Win5Badge;
    TokenERC20 public immutable PlayToken;
    TokenERC20 public immutable WinToken;

    constructor(
        DropERC1155 win5badgeAddress,
        TokenERC20 playtokenAddress,
        TokenERC20 wintokenAddress
    ) {
        Win5Badge = win5badgeAddress;
        PlayToken = playtokenAddress;
        WinToken = wintokenAddress;
    }

    mapping(address => bool) public isPlaying;

    function checkPlayTokens() external view returns (uint) {
        return PlayToken.balanceOf(msg.sender);
    }

    function inGame() external view returns (bool) {
        return isPlaying[msg.sender];
    }

    function buyPlayToken() external {
        PlayToken.approve(address(this), 5);
        PlayToken.approve(msg.sender, 5);
        PlayToken.mintTo(msg.sender, 5);
    }

    function burnPlayToken() external {
        require(
            PlayToken.balanceOf(msg.sender) > 0,
            "You need at least 1 play token"
        );
        PlayToken.approve(msg.sender, 1);
        PlayToken.burnFrom(msg.sender, 1);
        isPlaying[msg.sender] = true;
    }

    function wonGame() external {
        require(
            isPlaying[msg.sender],
            "You have to have played a game to get a win token"
        );
        WinToken.transfer(msg.sender, 1);
        isPlaying[msg.sender] = false;
    }

    function checkWinTokens() external view returns (uint) {
        return WinToken.balanceOf(msg.sender);
    }

    function buyBadge() external {
        require(
            WinToken.balanceOf(msg.sender) >= 5,
            "You need at least 5 win tokens to buy a badge"
        );
        Win5Badge.safeTransferFrom(address(this), msg.sender, 0, 1, "");
    }

    function checkBadges() external view returns (uint) {
        return Win5Badge.balanceOf(msg.sender, 0);
    }
}