...
contract GToken is ERC20Upgradeable, ERC4626Upgradeable, OwnableUpgradeable, IGToken {
    ...
    function collateralizationP() public view returns(uint) { // PRECISION (%)
        return accPnlPerTokenUsed > 0 ?
            (PRECISION - uint(accPnlPerTokenUsed)) * 100 :
            (PRECISION + uint(accPnlPerTokenUsed * (-1))) * 100;
    }
    ...
    function updateAccPnlPerTokenUsed(
        ...
        int maxDelta = int(MathUpgradeable.min(
            uint(int(PRECISION) - accPnlPerToken) * supply / PRECISION,
    ...
    function availableAssets() public view returns(uint){
        return uint(int(PRECISION) - accPnlPerTokenUsed) * totalSupply() / PRECISION;
    }
