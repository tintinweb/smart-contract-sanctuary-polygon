...
contract Reel_Crypto_Token is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    uint256 private _tTotal = 15000000000 * 1e18; // 15 billion total supply
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Reel Crypto Token"; // token name
    string private _symbol = "RCT"; // token ticker
    ...
    uint256 public maxTxLimit = _tTotal.div(1000); // maximum Holding limit is 1% percent of total supply
    uint256 public minTokenToSwap = 10000 ether; // 1M amount will trigger the swap and add liquidity
    ...
    uint256 public marketFeeOnBuying = 40; // 4% will go to the award pool address
    uint256 public buyBackandBurnOnBuying = 20; // 2% will go to the earth bank wallet address
    ...
    uint256 public marketFeeOnSelling = 40; // 4% will go to the award pool address
    uint256 public buyBackandBurnOnSelling = 30; // 3% will go to the earth bank wallet address
    ...
    uint256 public marketFee = 20; // 2% will go to the award pool address
    uint256 public buyBackandBurn = 10; // 1% will go to the earth bank wallet address
    ...
    constructor(address payable _market)
    {
        ...
        _isExcludedFromMaxTxLimit[dexPair] = true;
        _isExcludedFromMaxTxLimit[burnAddress] = true;
    ...
    function transfer(address recipient, uint256 amount)
        ...
        returns (bool)
    {
        if(!_isExcludedFromMaxTxLimit[msg.sender])
        {
            require(amount<maxTxLimit , "Transfer amount exceeds than the max limit");
        }
