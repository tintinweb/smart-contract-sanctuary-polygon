...
contract HyNFTTest is ERC721Enumerable, Ownable, ERC721Burnable, EIP712Whitelist {
    ...
    Counters.Counter private claimTokenId;
    ...
    function airdrop(address _to, bytes calldata signature) public payable {
        uint256 currentClaimTokenId = claimTokenId.current();
        require(isWhitelisted(_to) == false, "Address already claimed airdrop");
        require(simpleVerify(signature), "Invalid Signature");
        require(currentClaimTokenId < maxSupply, "No remained NFT to claim");

        claimTokenId.increment();
        

        approve(_to, claimTokenId.current());
        safeTransferFrom(owner(), _to, claimTokenId.current());
    }
}