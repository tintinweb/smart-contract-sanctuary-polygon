...
contract PricesSimple is IPrices, Ownable {
    ...
    function setPrice(uint256 price_) external onlyOwner {
        if (price_ == 0) revert NoLiquidity();
        price = price_;
    }
    ...
    function getSpot(address token1, address token2) public view returns(uint256) {
        return price;
    }

    /**
    * @notice Returns price of a token in a stablecoin
    * @param path unused
    * @return uint256 Price of Token in USD
    */
    function getPrice(address[] memory path) external returns(uint256) {
        return price;
    }
