...
/// @title RentLendAutomation contract
/// @author LYNC WORLD (https://lync.world)
/// @notice This contract is a chainlink compatible contract used for automation of the RentLendMarketplace contract.
/// @dev This contract is used to automate the return of expired rentings and refund required rentings.
contract RentLendAutomation is AutomationCompatibleInterface {

    /// @notice RentLendMarketplace contract address
    RentLendMarketplace public rentLendMktAddress;

    /// @notice Owner of the contract
    address public owner;

    /// @notice Event emitted when the RentLendMarketplace contract address is changed
    event RentLendMarketplaceChanged(
        address indexed previousAddress,
        address indexed newAddress
    );

    /// @notice Event emitted when the owner of the contract is changed
    event OwnerChanged(address indexed previousOwner, address indexed newOwner);

    /// @notice Constructor of the contract to set the RentLendMarketplace contract address and the owner of the contract
    /// @param _rentLendMktAddress Address of the RentLendMarketplace contract
    constructor(address payable _rentLendMktAddress) {
        owner = msg.sender;
        rentLendMktAddress = RentLendMarketplace(_rentLendMktAddress);
    }

    /// @notice Function to check if the upkeep of the RentLendMarketplace contract is needed
    /// @dev This function is called by the Chainlink Keeper network
    /// @dev It uses the getExpiredRentings and getRefundRentings functions of the RentLendMarketplace contract to check if the upkeep is needed
    ...
    /// @notice Function to perform the upkeep of the RentLendMarketplace contract
    /// @dev This function is called by the Chainlink Keeper network
    function performUpkeep(bytes calldata /*performData*/) external override {
        rentLendMktAddress.checkReturnRefundAutomation();
    }

    /// @notice Function to set the RentLendMarketplace contract address
    /// @param _newAddress Address of the new RentLendMarketplace contract
    /// @dev Only the owner of the contract can call this function
    function setLyncRentLendMarketplace(address payable _newAddress) external {
        require(msg.sender == owner, "Only owner can call tis function");
        require(
            _newAddress != address(0),
            "New address cannot be the zero address"
        );
        emit RentLendMarketplaceChanged(
            address(rentLendMktAddress),
            _newAddress
        );
