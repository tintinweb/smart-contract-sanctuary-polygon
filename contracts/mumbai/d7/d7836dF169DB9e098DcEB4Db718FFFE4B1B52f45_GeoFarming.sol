...
interface GeoPolyNFT {
    function getMintingString(uint256 tokenID)
        external
        view
        returns (string memory);

    function balanceOf(address wallet, uint256 tokenID)
        external
        returns (uint256);
    ...
    function getWalletNFTs(address wallet)
        external
        view
...
library GeopolyFarmingHelper {
    function findIndexInArr(uint256 val, uint256[] memory arr)
        public
        pure
...
abstract contract GeopolyRoles is Ownable {
    ...
    modifier isGeopolyContract() {
        require(
            geopolyContracts[_msgSender()],
            "GeopolyRoles: Address not a geopoly contract"
        );
        _;
    }
}
...
interface GeoSpeical {
    function doAll(string calldata _in)
        external
        pure
        returns (
            uint256,
...
contract GeoFarming is GeopolyRoles {
    event FarmingStarted(
        uint256 tokenID,
        address owner,
        uint256 unlockTime,
        uint256 spinsAmount
    );
    event FarmingHarvested(
        uint256 tokenID,
        address owner,
        uint256 timestamp,
        uint256 spinsAmount
    ...
    // MUMBAI ADDRESS
    address NFT = 0x3eaDfA50F47b5503c0CeDF8813B2dDc85eBa4730;
    ...
    function changeRewardBasis(
        uint256 _nBaseMultiplier,
        uint256 _nBaseDivisor,
        uint256 _nBaseOffset
    ) public isAdmin {
    ...
    function nftOwner(uint256 tokenID) public view returns (address) {
        require(
            isNFTFarming(tokenID),
            "GeopolyFarms: This token is not currently farming"
        );
        return (_farmings[tokenID].owner);
    }

    // safe add farming to the tokenID
    function _safeAddFarming(
        uint256 tokenID,
        address owner,
        uint256 lockTS,
        uint256 unlockTS
    ) internal {
        _farmings[tokenID] = Farmings(
            owner,
            lockTS,
            unlockTS,
            getRewards(tokenID)
    ...
    function _safeRemoveFarming(uint256 tokenID) internal {
        ...
        (bool sucess, uint256 index) = GeopolyFarmingHelper.findIndexInArr(
            tokenID,
            tokensFarming
        );
        if (sucess) {
    ...
    function Farm(uint256 tokenID) external payable {
        require(
            farmingStatus,
            "GeopolyFarms: Sorry Farming is under maintainance"
        );
        require(
            !isNFTFarming(tokenID),
            "GeopolyFarms: This NFT has already started farming"
        );
        require(
            msg.value >= farmingFee,
            "GeopolyFarms: You need to pay a farming fee to start farming your NFT"
        );
        require(
            recieve1155Token(msg.sender, tokenID),
            "GeopolyFarms: Cannot Farm your NFT"
        );
        _safeAddFarming(
            tokenID,
            msg.sender,
            block.timestamp,
            (block.timestamp + nftLockTime)
        );
    }
    ...
    function Harvest(uint256 tokenID) external {
        require(
            ...
            "GeopolyFarms: Cannot harvest an NFT that is not your own"
        );
        require(
            block.timestamp >= farmingTime(tokenID),
            "GeopolyFarms: This NFT is not done farming yet"
        );
        require(
            send1155Token(msg.sender, tokenID),
            "GeopolyFarms: Cannot send back the NFT"
        );
        _safeRemoveFarming(tokenID);
    }

    //get all the current nfts farming
    function getWalletFarmings(address wallet)
        public
        view
        returns (uint256[] memory walletFarmings)
    {
        uint256 count = 0;
        for (uint256 i = 0; i < tokensFarming.length; i++) {
    ...
    function getWalletNFTs(address owner)
        public
        view
        returns (uint256[] memory allNFTs)
    {
     ...
     */
    function send1155Token(address owner, uint256 tokenID)
        internal
        returns (bool)
    {
        require(
            GeoPolyNFT(NFT).balanceOf(address(this), tokenID) > 0,
            "GeopolyFarms: We do not own this NFT."
        );
     ...
     */
    function recieve1155Token(address owner, uint256 tokenID)
        internal
        returns (bool)
    {
        require(
            GeoPolyNFT(NFT).balanceOf(owner, tokenID) > 0,
            "GeopolyFarms: Cannot farm NFT you dont own"
        );
        require(
            GeoPolyNFT(NFT).isApprovedForAll(owner, address(this)),
            "GeopolyFarms: Need to approve us to farm your NFT"
        );
     ...
     */
    function send20Token(
        address token,
        address reciever,
        uint256 amount
    ) internal returns (bool) {
        require(
            Geos20(token).balanceOf(address(this)) > amount,
            "No enough balance"
        );
        require(
            Geos20(token).transfer(reciever, amount),
            "Cannot currently transfer"
        );
        return true;
    }
     ...
     */
    function recieve20Token(
        address token,
        address sender,
        uint256 amount
    ) internal returns (bool) {
        require(
            Geos20(token).allowance(sender, address(this)) >= amount,
            "Need to approve us for WIZZY to farm"
        );
        require(
            Geos20(token).transferFrom(sender, address(this), amount),
            "Need to pay us WIZZY to go farming"
        );
        return true;
    }
        ...
        returns (uint256 decimals)
    {
        ...
        require(
            decimals != 0,
            "GeopolyFarms: Cannot pay with a 0 decimal token"
        );
    }
    ...
    function _getCost(address token) internal view returns (uint256 cost) {
        if (token == address(this)) {
            ...
            require(
                ...
                "GeopolyFarms: This token is not supported"
            );
    ...
    function buySpins(uint256 amount, address token) external payable {
        ...
        if (token == address(0)) {
            require(
                msg.value >= (_getCost(address(this)) * amount),
                "GeopolyFarms: need to pay cost for geospins in matic"
            );
        } else {
            require(
                recieve20Token(token, msg.sender, (_getCost(token)) * amount),
                "GeopolyFarms: need to pay cost for geospins in tokens"
            );
        }
    ...
    function removeSpin(address owner, uint256 amount)
        external
        isGeopolyContract
    {
    ...
    function _checkNFTProps(uint256 tokenID)
        internal
        view
        returns (uint256 category, uint256 tier)
    {
    ...
    function withdrawNative(uint256 amount, address to) external onlyOwner {
        require(
            ...
            "GeopolyFarms: Not enough balance"
        );
        require(
            payable(to).send(amount),
            "GeopolyFarms: Cannot process withdrawal to this address"
        );
    }

    function withdraw20Tokens(
        address token,
        uint256 amount,
        address to
    ) external onlyOwner {
