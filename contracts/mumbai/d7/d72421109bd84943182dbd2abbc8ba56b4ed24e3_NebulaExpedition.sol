    ...
    StakeAssetMeta
{
    ...
    function _isExpeditionClaimable(uint256 _expeditionId)
        internal
        view
        returns (bool)
    {
        return getExpeditionState(_expeditionId) == STATE_CLAIMABLE;
    }
    ...
    function unstakeNFT(
        ...
        require(
            (_isExpeditionPeriodAndNotJoined(_expeditionId) ||
                _isExpeditionClaimable(_expeditionId)) &&
                (_expeditionId == lastExpeditionId ||
                    ...
                    _isExpeditionClaimable(lastExpeditionId)),
    ...
    StakeAssetMeta
{
    ...
    function stakeAssets(
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            for (uint256 j = 0; j < stakedAssets[_stakeholder].length; j++) {
                if (
