    ...
    Whitelist
{
    ...
    modifier timeLimitExceeds(uint256 saleId) {
        ...
        // console.log(deadline, reserveSale[saleId].startTime);
        require( deadline < reserveSale[saleId].startTime, 'DistinctMarket: time over');
        _;
    }

    modifier validateSaleId(uint256 saleId) {
        require(reserveSale[saleId].status == SaleStatus.open, 'Distinct: Sale not open');
        _;
    }
    ...
    modifier onlySeller(uint256 saleId) {
        require(reserveSale[saleId].seller == _msgSender(), "Distinct: not seller");
        _;
    }
    ...
    function updateSale(
        ...
        uint256 startTime, 
        uint256 endTime
    ) external validateSaleId(saleId) validateInputs(startTime,endTime) onlySeller(saleId){
    ...
    function buyDropItems(
        ...
        require(tokenIds.length == amounts.length, "Distinct: ids and amounts length mismatch");
        ...
        if(totalSellerRevenue > 0) {
            IERC20Upgradeable(paymentToken).safeTransferFrom(_msgSender(), drop.seller, totalSellerRevenue);
        }
        if(totalDistinctFee > 0){
            IERC20Upgradeable(paymentToken).safeTransferFrom(_msgSender(), address(this), totalDistinctFee);
        }
    ...
    function updateTimelimit(uint256 _maxTimelimit) external{
    ...
    function joinWhitelist(uint256 saleId) external  validateSaleId(saleId) timeLimitExceeds(saleId){
