...
contract Runiverse is Pausable, ReentrancyGuard, EIP712, RuniverseAdministrative, IDomainSeparator {
    ...
    bytes32 private constant _PLAYER_TYPEHASH =
        keccak256("StartRun(address sender,address tokenChosen,uint256 amountBetted,uint256 nonce,uint256 deadline)");
    
    bytes32 private constant _JOLLYUSED_TYPEHASH =
        keccak256("StartRun(address sender,address player,address tokenChosen,uint256 amountBetted,uint256 nonce,uint256 deadline)");
	...
	function startRun(uint256 gameId, SignedStartGame calldata player1, SignedStartGame calldata player2) external onlyOperator whenNotPaused nonReentrant {
		...
		checkSignature(player1.signature, player1);

        checkSignature(player2.signature, player2);
        ...
        if(player2.useJolly==1)
        {
            bettedAmountPlayer1 = ((10**player2PaymentTokenDecimals) * ticketCost) / player2.jollyPrice;
		...
		if(player1.useJolly==1)
        {
            IERC20(jollyTokenContract).safeTransferFrom(operatorAddress, address(this), bettedAmountPlayer1);
        }
        else
        {
    ...
    function checkSignature(bytes memory signature, SignedStartGame calldata player) internal {
        
        (bytes32 r, bytes32 s, uint8 v) = splitSignature(signature);

		//check signature of both players
        if(player.useJolly==0)
        {
            bytes32 structHash = keccak256(abi.encode(_PLAYER_TYPEHASH, player.sender, player.tokenChosen,player.amountBetted, _useNonce(player.playerAddress), player.deadline));
            address signer = ECDSA.recover(_hashTypedDataV4(structHash), v, r, s);
            require(signer == player.sender, "invalid signature");
        }
        else
        {
            bytes32 structHash = keccak256(abi.encode(_JOLLYUSED_TYPEHASH,player.sender, player.playerAddress, player.tokenChosen,player.amountBetted, _useNonce(player.playerAddress), player.deadline));
            address signer = ECDSA.recover(_hashTypedDataV4(structHash), v, r, s);
            require(signer == player.sender, "invalid signature");
        } 
    ...
    function GetSigLenght(bytes memory signature) public view returns (uint256 length)
    {
        uint256 ris = signature.length;
        return ris;
    }

    function GetSplitSignature(bytes memory signature) external {
        require(signature.length == 65, "invalid signature length");
...
abstract contract RuniverseAdministrative is Pausable, Ownable, CryptorunFeeSharer {
	using SafeMath for uint256;
	using SafeERC20 for IERC20;
