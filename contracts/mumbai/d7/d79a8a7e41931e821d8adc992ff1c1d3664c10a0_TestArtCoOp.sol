...
contract TestArtCoOp is ERC1155, Ownable, DefaultOperatorFilterer, ReentrancyGuard, ERC2981 {
    using Strings for uint256;

    string public baseURI = "ipfs://bafybeifyne7flhgsjjk42fhgrybclwwappc2oopgqqd5zju4axo2tmgcgi/";
    string public name = "Test Art CoOp";
    ...
    mapping(uint8 => bool) public tokenIdsBurnToClaimStatus;
    mapping(uint8 => uint256 ) public tokenIdsBurnToClaimAmount;
 ...
 /**
    ...
    function updateTokenIdsBurnToClaimAmount (uint8[] memory _tokenIds,uint256[] memory _claimAmount) public onlyOwner {
        for (uint256 i = 0; i<_tokenIds.length;i++){
            tokenIdsBurnToClaimAmount[_tokenIds[i]] = _claimAmount[i];
        }        
    }

    function updateTokenIdsBurnToClaimStatus (uint8 _tokenId, bool _status) public onlyOwner {
            tokenIdsBurnToClaimStatus[_tokenId] = _status;      
    }
    ...
    external payable nonReentrant {
        ...
        require( (tokenIdsBurnsNeeded[_redeemTokenID] * _redeemQuantity) == (_burnQuantity) );
    ...
    external payable nonReentrant {
        require(tokenIdsBurnToClaimStatus[_burnTokenID] == true);
        _burn(msg.sender, _burnTokenID, _burnQuantity);
         payable(msg.sender).transfer(tokenIdsBurnToClaimAmount[_burnTokenID]);
    }
   ...
   /**
    ...
    function withdraw_amt(uint256 amt) external onlyOwner {
