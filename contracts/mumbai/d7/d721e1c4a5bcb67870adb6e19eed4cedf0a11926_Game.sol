...
contract Game {
    ...
    function battle(string memory _name, int _strength, int _agility, int _wisdom) public {
        ...
        uint firstAttackIndex = attacks.length-1;
        ...
        if (players[currentIndex].health <= 0 && spawns[currentIndex].health <= 0){
            addRound(currentIndex, attackCounter, firstAttackIndex, firstAttackIndex+attackCounter, false, 5);
        } else if (players[currentIndex].health > 0 && spawns[currentIndex].health <= 0){
            addRound(currentIndex, attackCounter, firstAttackIndex,  firstAttackIndex+attackCounter, true, 20);
        } else if (players[currentIndex].health <= 0 && spawns[currentIndex].health > 0){
            addRound(currentIndex, attackCounter, firstAttackIndex, firstAttackIndex+attackCounter, false, 0);
        }
    }
}