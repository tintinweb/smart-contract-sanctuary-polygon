...
contract FearWolfSeller is AccessControl, Pausable, Withdrawable {

    using SafeMath for uint256;

    IERC20 public fearContract;
    IFearWolf public fearWolfContract;
    IMuonV02 public muonContract;


    uint256 public fearWolfPrice;
    
    uint256 public muonAppId = 10; // ???
    uint256 public acceptableMuonDelay = 2 minutes;

    constructor(address fearContractAddress, address fearWolfContractAddress, address muonContractAddress) {
        fearContract = IERC20(fearContractAddress);
        fearWolfContract = IFearWolf(fearWolfContractAddress);
        muonContract = IMuonV02(muonContractAddress);
        fearWolfPrice = 2 * 10 ** 17;
    }
    ...
    function buyWolves(
        ...
        require(block.timestamp - _muonTimestamp < acceptableMuonDelay, "expired");
        require(_sigs.length > 0, "!sigs");

        bytes32 hash = keccak256(abi.encodePacked(muonAppId, fearPrice, _muonTimestamp));
        require(muonContract.verify(_reqId, uint256(hash), _sigs), "!verified");

        uint256 fearAmount = amount.mul(fearWolfPrice.div(fearPrice));

        uint256 userBalance = fearContract.balanceOf(caller);
        require(userBalance >= fearAmount, "Insufficient balance");

        uint256 contractAllowance = fearContract.allowance(caller, address(this));
        require(contractAllowance >= fearAmount, "Insufficient allowance");

        fearContract.transferFrom(caller, address(this), fearAmount);
        fearWolfContract.setInitialOwner(caller, amount);
    }

    function setFearWolfPrice(uint256 _fearWolfPrice) external onlyRole(MANAGER_ROLE) {
        fearWolfPrice = _fearWolfPrice;
    }

    function setMuonAppId(uint256 _muonAppId) external onlyRole(MANAGER_ROLE) {
        muonAppId = _muonAppId;
    }

    function setMuonContractAddress(address _muonContractAddress) external onlyRole(MANAGER_ROLE) {
        muonContract = IMuonV02(_muonContractAddress);
    }
}