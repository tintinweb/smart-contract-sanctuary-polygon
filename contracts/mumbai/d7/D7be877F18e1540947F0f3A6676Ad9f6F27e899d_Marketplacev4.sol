    ...
    UUPSUpgradeable
{
    ...
    function aggregatedSBT(
        ...
        } else _totalRENWFunded[_to] += _RENWFunded;
    }

    // function to be called only once
    // after ASBT upgrade
    function initializeIpfsId() public onlyRole(DEFAULT_ADMIN_ROLE) {
        uint256 _totalSupply = _tokenIdCounter.current();

        for (uint256 i = 1; i < _totalSupply; i++) {
            _ipfsIdToTokenId[i] = i;
        }
    }
    ...
    UUPSUpgradeable
{
    ...
    struct retainer {
        uint256 retainerID;
        address payable retainerAddress;
    }

    struct retainerProject {
        uint256 retainerID;
        uint256 retainedForID;
        // if isOwned - false, retained in behalf of someone
        bool isOwned;
        uint256[] reservedYears;
        mapping(uint256 => uint256) amountPaidByYear;
        mapping(uint256 => uint256) amountReservedByYear;
    }

    mapping(uint256 => retainer) public _retainers;
    // each project can have multiple retainers
    // every buyer can retain several times
    mapping(uint256 => retainerProject[]) public _projectRetainers;
    mapping(uint256 => CountersUpgradeable.Counter)
        public _numberOfRetainersByProject;
    mapping(uint256 => mapping(uint256 => CountersUpgradeable.Counter))
        public _numberOfUnpaidRetainersByYear;
    mapping(uint256 => mapping(uint256 => mapping(uint256 => bool)))
        public approvedRetainerOnProjectByYear;
    ...
    function updateReservedProjectStatus(
        uint256 _projectId,
        bool _canBeReserved
    ) public isProjectValid(_projectId) onlyRole(LIST_PROJECT_ROLE) {
        _projects[_projectId].canBeReserved = _canBeReserved;
    }
    ...
    function updateProjectPausedStatus(uint256 _projectID, bool paused)
        public
        isProjectValid(_projectID)
    ...
    function addMWhToReservedProject(
        uint256 _projectID,
        uint256 _additionalMWhProduced,
        uint256 _projectYear,
        string[] memory _ipfsHashSBTs,
        string[] memory _ipfsHashASBTs,
        uint256[] memory _ipfsTokenIDsSBTs,
        uint256[] memory _ipfsTokenIDsASBTs,
        string memory _projectIpfsHash
    ) public onlyRole(LIST_PROJECT_ROLE) {
        require(
            _projects[_projectID].canBeReserved,
            "This project can't be reserved"
        );
        addMWhToProject(
            _projectID,
            _additionalMWhProduced,
        ...
        //get the number of retainers for the current project
        uint256 numberOfRetainers = _numberOfRetainersByProject[_projectID]
            .current();

        //total MWh available for sale at this moment for the current project
        //produced  - funded
        //compute the equal amount which must be sent to each retainer
        uint256 splittedAmount = (_projectVintageMWhProduced[_projectID][
            _projectYear
        ] - _projectVintageMWhFunded[_projectID][_projectYear]) /
            _numberOfUnpaidRetainersByYear[_projectID][_projectYear].current();

        //convert to integer value
        splittedAmount = (splittedAmount / 1 ether) * 1e18;

        uint256 MWhFunded;
        //add this to avoid stack too deep
        uint256 projectID = _projectID;

        for (uint256 i = 0; i < numberOfRetainers; i++) {
            retainerProject storage Retainer = _projectRetainers[projectID][i];

            //check if the current retainer is valid
            if (
                approvedRetainerOnProjectByYear[projectID][Retainer.retainerID][
                    _projectYear
                ] && Retainer.amountReservedByYear[_projectYear] > 0
            ) {
                //how much more has to be paid to the current retainer
                //store projectYear like this to avoid stack too deep
                uint256 projectYear = _projectYear;
                uint256 computedAmountToBePaid = Retainer.amountReservedByYear[
                    projectYear
                ] - Retainer.amountPaidByYear[projectYear];

                //check if the project still has to pay any amount to the current retainer
                if (computedAmountToBePaid > 0) {
                    uint256 amountToBePaid = splittedAmount;

                    //check if the remaining amount to be paid is lower or equal to the equally computed amount
                    if (computedAmountToBePaid <= splittedAmount) {
                        amountToBePaid = computedAmountToBePaid;

                        //reserved amount will be fully paid after this round for the current retainer
                        _numberOfUnpaidRetainersByYear[projectID][projectYear]
                            .decrement();
                    }

                    MWhFunded += amountToBePaid;

                    //update amount paid by year for current retainer
                    Retainer.amountPaidByYear[projectYear] += amountToBePaid;

                    //burn the paid amount
                    RToken.burn(treasuryWallet, amountToBePaid);

                    // check if user funded a similar project before (country, energy type)
                    countryChecker(
                        _retainers[Retainer.retainerID].retainerAddress,
                        _projects[projectID].projectCountry
                    );
                    energyTypesChecker(
                        _retainers[Retainer.retainerID].retainerAddress,
                        _projects[projectID].projectSourceType
                    );

                    //mint SBTs/ASBTs
                    //send sbtHash, asbtHash, ipfsTokenIDSBT and ipfsTokenIDASBT
                    //in this way to avoid stack too deep

                    string memory sbtHash = _ipfsHashSBTs[i];
                    string memory asbtHash = _ipfsHashASBTs[i];
                    uint256 ipfsTokenIDSBT = _ipfsTokenIDsSBTs[i];
                    uint256 ipfsTokenIDASBT = _ipfsTokenIDsASBTs[i];
        ...
        _projectVintageMWhFunded[projectID][_projectYear] += MWhFunded;
        _projects[projectID].MWhFunded += MWhFunded;
    }

    function fundWithBasket(
        address _paymentAddress,
        uint256[] memory _projectIDs,
        uint256[] memory _RENWFunded,
        uint256[] memory _projectsVintage,
        string[] memory _ipfsHashSBTs,
        string[] memory _ipfsHashASBTs,
        uint256[] memory _ipfsSBTIds,
        uint256[] memory _ipfsASBTIds
    ) public payable {
        for (uint256 i = 0; i < _projectIDs.length; i++) {
            require(
                _projects[_projectIDs[i]].providerAddress != address(0),
                "This project hasn't been registered"
            );
        }
    ...
    function reserveRECs(
        uint256 _projectID,
        uint256 _retainerID,
        address _retainerAddress,
        bool _isOwned,
        uint256 _retainedForID,
        uint256[] memory _reservedYears,
        uint256[] memory _amountReserved
    ) public isProjectValid(_projectID) onlyRole(LIST_PROJECT_ROLE) {
        require(
            _projects[_projectID].canBeReserved,
            "Project can't be reserved"
        );

        if (_retainers[_retainerID].retainerAddress == address(0)) {
            _retainers[_retainerID] = retainer(
                _retainerID,
                payable(_retainerAddress)
            );
        }

        retainerProject storage newRetainer = _projectRetainers[_projectID]
            .push();

        for (uint256 i = 0; i < _reservedYears.length; i++) {
            newRetainer.amountReservedByYear[
                _reservedYears[i]
            ] = _amountReserved[i];

            _numberOfUnpaidRetainersByYear[_projectID][_reservedYears[i]]
                .increment();
        }
        newRetainer.reservedYears = _reservedYears;
        newRetainer.retainerID = _retainerID;

        newRetainer.isOwned = _isOwned;
        newRetainer.retainedForID = _isOwned ? 0 : _retainedForID;

        //approve the first year by default
        approvedRetainerOnProjectByYear[_projectID][_retainerID][
            _reservedYears[0]
        ] = true;

        _numberOfRetainersByProject[_projectID].increment();
    }

    function getReservedYearsByRetainer(uint256 _projectID, uint256 _retainerID)
        public
        view
        isProjectValid(_projectID)
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < numberOfRetainers; i++) {
            retainerProject storage Retainer = _projectRetainers[_projectID][i];
            if (Retainer.retainerID == _retainerID) {
                return Retainer.reservedYears;
            }
        }
        return new uint256[](0);
    }

    function getPaidAndReservedAmountsByYear(
        uint256 _projectID,
        uint256 _retainerID,
        uint256 _projectYear
    )
        ...
        )
    {
        ...
        for (uint256 i = 0; i < numberOfRetainers; i++) {
            ...
            if (Retainer.retainerID == _retainerID) {
                return (
                    Retainer.amountPaidByYear[_projectYear],
                    Retainer.amountReservedByYear[_projectYear],
                    approvedRetainerOnProjectByYear[_projectID][_retainerID][
                        _projectYear
                    ]
                );
    ...
    function updateRetainerAddress(
        uint256 _retainerID,
        address _retainerAddress
    ) public onlyRole(LIST_PROJECT_ROLE) returns (bool success) {
        require(
            _retainers[_retainerID].retainerAddress != address(0),
            "Invalid retainer"
        );
        _retainers[_retainerID].retainerAddress = payable(_retainerAddress);
        return true;
    }

    function updateApprovedStatusOfRetainersByYear(
        uint256 _projectID,
        uint256 _projectYear,
        bool _status,
        uint256[] memory _retainersIds
    ) public isProjectValid(_projectID) onlyRole(LIST_PROJECT_ROLE) {
        for (uint256 i = 0; i < _retainersIds.length; i++) {
            approvedRetainerOnProjectByYear[_projectID][_retainersIds[i]][
                _projectYear
            ] = _status;
        }
    }
    ...
    function getCountriesByAddress(address _user)
        public
        view
        returns (string[] memory)
    {
    ...
    function getEnergyTypesByAddress(address _user)
        public
        view
        returns (string[] memory)
    {
    ...
    function updateProjectData(
        uint256 _projectId,
        uint256 _projectNewSCADAID,
        string memory _projectSDG,
        string memory _projectIpfsHash
    ) public isProjectValid(_projectId) onlyRole(LIST_PROJECT_ROLE) {
        if (_projects[_projectId].projectSCADAID != _projectNewSCADAID) {
            _projects[_projectId].projectSCADAID = _projectNewSCADAID;
        }

        if (
            keccak256(abi.encodePacked(_projects[_projectId].projectSDG)) !=
            keccak256(abi.encodePacked(_projectSDG))
        ) {
    ...
    function howManyMatic(uint256 _amount) public view returns (uint256) {
        ...
        uint256 amountUSDGWEI = amountUSDDecimals * 1e13;
    ...
    UUPSUpgradeable
{
    ...
    /**
     * @notice Mint a new SBT for a specific buyer both in a custodial and non-custodial way
     * @param _to Address to transfer funds to
     * @param _isCustodial Represents the mint type (true - {custodial for company X or on behalf of company Y}, false - non-custodial)
     * @param _ownerID Final owner id (0 if custodial mint on behalf of current buyer)
     */
    function mintSBT(
        address _to,
        bool _isCustodial,
    ...
    // when a corporate buyer updates its wallet address
    // to a non-custodial one
    ...
    function getSBTsHeldBy(address _owner)
        public
        view
        returns (uint256[] memory)
    {
    ...
    function getTokenURIsOfSBTsHeldBy(address _owner)
        public
        view
        returns (string[] memory)
    {
    ...
    function getSBTsHeldByFor(address _owner, uint256 _ownerID)
        public
        view
        returns (uint256[] memory)
    {
    ...
    function getTokenURIsOfSBTsHeldInCustody(address _owner)
        public
        view
        returns (string[] memory)
    {
    ...
    function getSBTsHeldInCustody(address _owner)
        public
        view
        returns (uint256[] memory)
    {
    ...
    function getSBTsOfNonCustodialBuyer(address _owner)
        public
        view
        returns (uint256[] memory)
    {
    ...
    function getTokenURIsOfSBTsOfNonCustodialBuyer(address _owner)
        public
        view
        returns (string[] memory)
    {
