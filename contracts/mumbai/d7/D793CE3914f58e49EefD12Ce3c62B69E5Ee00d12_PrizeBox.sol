    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize(string memory baseURI, address signerAddress)
        public
        initializer
    {
        ERC721x.__ERC721x_init("PrizeBox", "PrizeBox");
        ...
        returns (string memory)
    {
        return
            string.concat(
                ...
                "/",
                Strings.toString(boxRarity[_tokenId])
            );
    }
    ...
    {
        toTokenId = Math.min(toTokenId, _totalMinted());
        uint256[] memory part = new uint256[](toTokenId - fromTokenId);
        for (uint256 i = 0; i < toTokenId - fromTokenId; i++) {
            part[i] = boxRarity[i + fromTokenId];
        }
        return part;
    }
