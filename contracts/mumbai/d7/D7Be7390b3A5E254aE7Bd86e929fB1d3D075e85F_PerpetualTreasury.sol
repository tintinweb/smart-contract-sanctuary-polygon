...
contract PerpetualTreasury is PerpetualBaseFunctions, IFunctionList, IPerpetualTreasury {
    ...
    mapping(address => mapping(uint8 => WithdrawRequest)) lpWithdrawMap;
     ...
     */
    function addLiquidity(uint8 _poolId, uint256 _tokenAmount) external override nonReentrant {
        ...
        emit LiquidityAdded(_poolId, user, _tokenAmount, fShareToMint.toUDec18());
        ...
        nonReentrant
    {
        ...
        if (_shareAmount > shareToken.balanceOf(user)) {
    ...
    function executeLiquidityWithdrawal(uint8 _poolId) external override nonReentrant {
        address user = msg.sender;
        require(lpWithdrawMap[user][_poolId].shareTokens > 0, "init withdrwl");
        require(
            block.timestamp >= lpWithdrawMap[user][_poolId].withdrawTimestamp + _getDelay(),
            "too early"
        );
        //inlines the modifier updateFundingAndPrices: updateFundingAndPrices(uint24(0), _poolId)
        uint24 iPerpetualId;
        ...
        LiquidityPoolData storage pool = liquidityPools[_poolId];
        _isLPWithdrawValid(pool);
        uint256 shareAmount = lpWithdrawMap[user][_poolId].shareTokens;
        delete lpWithdrawMap[user][_poolId];

        int128 fTokenAmountToReturn = _getTokenAmountToReturn(
            pool,
            int256(shareAmount).fromDec18()
        );
        IShareToken(pool.shareTokenAddress).burn(user, shareAmount);
        pool.totalSupplyShareToken = uint128(IShareToken(pool.shareTokenAddress).totalSupply());
        ...
        emit LiquidityRemoved(_poolId, user, fTokenAmountToReturn.toUDec18(), shareAmount);
