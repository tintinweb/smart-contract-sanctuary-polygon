...
contract ERC721SelfMinter is ERC721, ReentrancyGuard, Ownable {
    ...
    function mint(address mintAddress) external payable onlyStarted {
        require(
    ...
    function batchmint(address[] memory mintAddresses)
        external
        onlyMultiSig(msg.sender)
    {
        require(
            mintedAmount + mintAddresses.length <= maxSupply,
            "Exception: Reached the minting limit for the contract. Can't mint no more"
        );
        for (uint256 i = 0; i < mintAddresses.length; i++) {
            _mint(mintAddresses[i], currentId);
            currentId++;
        }
        mintedAmount = mintedAmount + mintAddresses.length;
    }
    ...
    function uri(uint256 tokenId) public view returns (string memory) {
        ...
        return
            string(abi.encodePacked(baseURI, tokenId.toString(), metaDataExt));
    }
}
