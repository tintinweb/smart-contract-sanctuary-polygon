...
contract ERC1155ORBUser is OwnableUpgradeable, ERC1155BurnableUpgradeable, ERC1155Lazy, HasContractURI {

    event CreateERC1155ORBUser(address owner, string name, string symbol);

    string public name;
    string public symbol;

    function __ERC1155ORBUser_init(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, address[] memory operators) external initializer {
        ...
        __ERC1155ORBUser_init_unchained(_name, _symbol);
        _setBaseURI(baseURI);
        for(uint i = 0; i < operators.length; i++) {
            setApprovalForAll(operators[i], true);
        }
        emit CreateERC1155ORBUser(_msgSender(), _name, _symbol);
    }

    function __ERC1155ORBUser_init_unchained(string memory _name, string memory _symbol) internal initializer {
        name = _name;
        symbol = _symbol;
    }
    ...
    function uri(uint tokenId) public view override(ERC1155BaseURI, ERC1155Upgradeable) virtual returns (string memory) {
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC1155Lazy, ERC1155Upgradeable, ERC165StorageUpgradeable) returns (bool) {
...
abstract contract AbstractRoyalties {
    ...
    event RoyaltiesToTokenId (uint256 tokenId, LibPart.Part[] royaltiesValue);
    ...
    function _saveRoyalties(uint256 id, LibPart.Part[] memory _royalties) internal {
        ...
        emit RoyaltiesToTokenId(id,royalties[id]);
    }
...
abstract contract ERC1271Validator is EIP712Upgradeable {
    using AddressUpgradable for address;
...
abstract contract ERC1155Lazy is IERC1155LazyMint, ERC1155BaseURI, Mint1155Validator, RoyaltiesV2Upgradeable, RoyaltiesV2Impl {
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC165StorageUpgradeable, ERC1155Upgradeable, IERC165Upgradeable) returns (bool) {
