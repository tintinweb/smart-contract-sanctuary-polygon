...
contract InvestmentContract is ERC721 {
    ...
    function distributeProfitsAndLosses(int256 percentage) external onlyOwner {
        ...
        for (uint256 i = 1; i < tokenIdCounter; i++) {
            ...
            if (_isApprovedOrOwner(investor, tokenId)) {
                int256 payment = (int256(totalInvested) * percentage) / totalPercentage;
                uint256 totalPayment = totalInvested + uint256(payment);

                if (percentage < 0) {
                    require(totalPayment <= totalInvested, "Cannot distribute more than invested amount");
                }
    ...
    function getInvestedAmount(address investor) external view returns (uint256) {
        return hasInvested[investor] ? totalInvested : 0;
    }
