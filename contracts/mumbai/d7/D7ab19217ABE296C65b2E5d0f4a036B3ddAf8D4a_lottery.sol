...
contract lottery is ReentrancyGuard,VRFConsumerBaseV2,ConfirmedOwner {
   ...
   return count;
    ...
    function getwinnerscountandamount(uint256 roundno,uint digit,uint position) public view returns(uint256 count,uint256 amount) {
        if(digit == 4){
           count = Rounds[roundno].winnerscount4D[position];
           amount = Rounds[roundno].winnersamount4D[position]; 
          return (count,amount);
        }
        else if(digit == 4){
          count = Rounds[roundno].winnerscount6D[position];
          amount = Rounds[roundno].winnersamount6D[position]; 
          return (count,amount);
        }
    }  
    ...
    function withdrawtokens(address payable _paymentReceiver,uint256 _amt,uint256 nonce,bytes memory signature1,bytes memory signature2,bytes memory signature3) notContract nonReentrant external {
