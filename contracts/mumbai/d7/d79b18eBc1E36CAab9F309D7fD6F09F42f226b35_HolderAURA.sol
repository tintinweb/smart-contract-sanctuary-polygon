...
contract HolderAURA is Ownable, Pausable {
    ...
    uint256 public auraPoolPID;

    uint256 public constant MAX_WITHDRAW_FEE = 3000; // 30%
    uint256 public constant MAX_WITHDRAW_FEE_PERIOD = 120 days; // 120 days
    uint256 public constant WITHDRAW_PENALTY_PERIOD = 7 days; // 7 days
    ...
    function init(IERC20 dummyToken) external onlyOwner {
        ...
        masterchef.deposit(auraPoolPID, balance);
        emit Init();
    }
    ...
    function deposit(uint256 _amount) external whenNotPaused notContract {
        ...
        // Handle stock funds.
        if (totalShares == 0) {
            token.safeTransfer(treasury, pool);
        }
        ...
        user.AuraAtLastUserAction = user.shares.mul(balanceOf()).div(totalShares);
    ...
    /**
     * @notice Withdraws funds from the Aura Vault
    ...
    function withdraw(uint256 _shares) public notContract {
        ...
        if (block.timestamp < user.lastDepositedTime.add(withdrawFeePeriod)) {
            uint256 userWithdrawFee = withdrawFee;
            if (block.timestamp < user.lastDepositedTime.add(WITHDRAW_PENALTY_PERIOD)) {
                userWithdrawFee = 5000; //50% penalty if withdraw within 7 days after deposit
            }
            uint256 currentWithdrawFee = currentAmount.mul(userWithdrawFee).div(10000);
    ...
    /**
     * @notice Harvest pending AURA tokens from MasterChef
     */
    function harvest() internal {
        (uint256 pendingAura,,,) = masterchef.pendingTokens(auraPoolPID, address(this));
        if (pendingAura > 0) {
            uint256 balBefore = balanceOf();
            masterchef.withdraw(auraPoolPID, 0);
     ...
     */
    function calculateTotalPendingAuraRewards() public view returns (uint256) {
        (uint256 amount,,,) = masterchef.pendingTokens(auraPoolPID, address(this));
        return amount;
    }
    ...
    function getPricePerFullShare() external view returns (uint256) {
        return totalShares == 0 ? 1e18 : (((balanceOf() + calculateTotalPendingAuraRewards()) * (1e18)) / totalShares);
    }
