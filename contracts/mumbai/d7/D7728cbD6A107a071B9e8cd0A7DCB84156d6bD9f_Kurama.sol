...
contract Kurama is Ownable, VRFConsumerBaseV2 {
    //  <-  Define Raffle State  ->  //
    enum RaffleState {
        Open,
        Close
    ...
    */
    ...
    //  <-  Mapping of User entries with Raffle ID  ->  //
    mapping(uint256 => address[]) private entries_mapping;
    //  <-  List of all Raffles  ->  //
    ...
    modifier onlyAdminOrOwner() {
        require(
            ...
            "Function only accessible to owner or admin"
        );
        _;
    }
    ...
    function enterRaffle(uint256 _raffleId) external payable {
        ...
        entries_mapping[_raffleId].push(msg.sender);
    }

    //  <-  ChainLink Keeper => determine if performUpkeep should be executed on-chain  ->  //
    function checkUpkeep(
        ...
        for (uint256 i = 0; i < raffles.length; i++) {
            ...
            uint256 raffleEntries = entries_mapping[raffle.id].length;
    ...
    //  <-  ChainLink Keeper => When checkUpkeep returns upkeepNeeded == true, the Keeper node broadcasts a transaction to the blockchain to execute performUpkeep  ->  //
    function performUpkeep(
        ...
        raffles[raffleId].state = RaffleState.Close;
        ...
        vrfCoordinatorV2.requestRandomWords(
            ...
            callbackGasLimit,
            raffles[raffleId].maxWinners
        );
    }

    //  <-  Receives random values and perform operation with random values  ->  //
    function fulfillRandomWords(
        ...
        for (uint256 i = 0; i < randomWords.length; i++) {
            uint256 indexOfWinner = randomWords[i] %
                entries_mapping[raffleId].length;
            raffle.winners.push(entries_mapping[raffleId][indexOfWinner]);
        }
    }

    function getRaffleEntries(uint256 _raffleId)
        external
        view
        returns (address[] memory)
    {
        return entries_mapping[_raffleId];
    }
    ...
    function getRaffleEntriesLength(uint256 _raffleId)
        ...
        returns (uint256)
    {
        return entries_mapping[_raffleId].length;
    }
    ...
    function getRaffleWinners(uint256 _raffleId)
        external
        view
        returns (address[] memory)
    {
