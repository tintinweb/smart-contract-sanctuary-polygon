...
contract BandContract is  AccessControlUpgradeable {
  ...
  // TODO update to mainet address
    address public splits; // testnet polygon - 0x2ed6c4B5dA6378c7897AC67Ba9e43102Feb694EE
    

// address of alive house royalty receiver

    address public aliveHouse; // add to registry Todo
    ...
    // event SongCreationConstructor(address beacon, bytes data);
...
// to update the split's implementation address for creating split wallets
    ...
    function initialize (uint _id, address _bandAdmin, string memory _bandName, address _aliveManager, address aliveRoyaltyReceiver, address[] calldata imps) external initializer{
        bandId = _id;
       ...
       for (uint i = 0; i < imps.length; i++) {
         nftBeacon.push(new NFTBeacon(imps[i]));
         }
       }
    ...
    //  function createSongFixed( address _royaltyReceiverSecondary, address royaltyReceiverMint, uint96 _offchainRoyalty, string memory _uri ,string memory _contractURI, uint96 _royaltyFeesInBips, uint maxFixedMints, uint fixedMintPrice , address _artist) public onlyGovernor returns(address) {
    //     uint id = allSongs.length+1;
    //     AliveFixed song = new AliveFixed(_royaltyReceiverSecondary, _royaltyFeesInBips, maxFixedMints, fixedMintPrice, royaltyReceiverMint,_offchainRoyalty, _uri,_contractURI, id , _artist, address(msg.sender));
    ...
    // }
     ...
     function createSongFixed( address _royaltyReceiverSecondary, address royaltyReceiverMint, uint96 _offchainRoyalty, string memory _uri ,string memory _contractURI, uint96 _royaltyFeesInBips, uint maxFixedMints, uint fixedMintPrice , address _artist) public onlyGovernor returns(address) {
        ...
        uint id = allSongs.length+1;
        bytes memory data = abi.encodeWithSelector(AliveFixedUpgradeable(address(0)).initialize.selector, _royaltyReceiverSecondary, _royaltyFeesInBips, maxFixedMints, fixedMintPrice, royaltyReceiverMint,_offchainRoyalty, _uri,_contractURI, id , _artist, address(msg.sender) );
        BeaconProxy song = new BeaconProxy(address(nftBeacon[0]), data);
     ...
     function createSongBonded( address _royaltyReceiverSecondary, address royaltyReceiverMint, uint96 _offchainRoyalty, string memory _uri ,string memory _contractURI, uint96 _royaltyFeesInBips, address _artist, uint[3] calldata _configureBondedCurve) public onlyGovernor returns(address) {

        uint id = allSongs.length+1;
        bytes memory data = abi.encodeWithSelector(AliveBondedUpgradeable(address(0)).initialize.selector, _royaltyReceiverSecondary, _royaltyFeesInBips, royaltyReceiverMint,_offchainRoyalty, _uri,_contractURI, id , _artist, address(msg.sender) , _configureBondedCurve);
        BeaconProxy song = new BeaconProxy(address(nftBeacon[1]), data);
     ...
     function createSongDutch( address _royaltyReceiverSecondary, address royaltyReceiverMint, uint96 _offchainRoyalty, string memory _uri ,string memory _contractURI, uint96 _royaltyFeesInBips, address _artist, uint[6] calldata _configureDutchAuction) public onlyGovernor returns(address) {
        
        uint id = allSongs.length+1;
        bytes memory data = abi.encodeWithSelector(AliveDutchUpgradeable(address(0)).initialize.selector, _royaltyReceiverSecondary, _royaltyFeesInBips, royaltyReceiverMint,_offchainRoyalty, _uri,_contractURI, id , _artist, address(msg.sender), _configureDutchAuction );

        BeaconProxy song = new BeaconProxy(address(nftBeacon[2]), data);
    ...
    // TODO - revert if wrong arguments passed, do a checking. 
    //  function createSongBonded( address _royaltyReceiverSecondary, address royaltyReceiverMint, uint96 _offchainRoyalty, string memory _uri ,string memory _contractURI, uint96 _royaltyFeesInBips, address _artist) public onlyGovernor returns(address) {
    //     uint id = allSongs.length+1;
       
    //     AliveBonded song = new AliveBonded(_royaltyReceiverSecondary, _royaltyFeesInBips, royaltyReceiverMint,_offchainRoyalty, _uri,_contractURI, id , _artist, address(msg.sender));
    //     Songs memory newSong;
    //     newSong.nftContract = address(song);
    //     newSong.songId = id;
    //     newSong.songType = 2;
    ...
    //  function createSongProxyNonFixed(uint _typeId,uint _typeName,  address _royaltyReceiverSecondary, address royaltyReceiverMint, string memory _uri ,string memory _contractURI, uint96 _royaltyFeesInBips) public onlyGovernor returns(address) {
    //     require(_typeId > 3, "Function only for atleast 1 fixed mint type nfts");
    //     require(_typeName== 555 || _typeName == 556 || _typeName == 566 || _typeName == 666, "invalid type name");
    //     uint id = allSongs.length+1;
    //     bytes memory data = abi.encodeWithSelector(N456(address(0)).initialize.selector, _royaltyReceiverSecondary, _royaltyFeesInBips, royaltyReceiverMint, _uri,_contractURI, id );
    //     BeaconProxy song = new BeaconProxy(address(nftBeacon[_typeId]), data);
    //     Songs memory newSong;
    //     newSong.nftContract = address(song);
    //     newSong.songId = id;
    //     newSong.songType = _typeName;
    //     newSong.splitSecondaryReceiverAddress = _royaltyReceiverSecondary;
    //     newSong.splitMintReceiverAddress = royaltyReceiverMint;
    //     allSongs.push(newSong);
...
contract AliveFixedUpgradeable is  ERC721AUpgradeable, ERC2981Upgradeable, AccessControlUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable
                           {
    bytes32  constant GOVERNANCE_ROLE = keccak256("GOVERNANCE_ROLE");
 ...
 function setURI(string memory _uri) public onlyRole(GOVERNANCE_ROLE) {
  ...
  function contractURI() public view returns (string memory) {
    ...
    {   uint price = fixedMintPrice* amount;
        ...
        uint fee = price/10; // TODO - upgraderable fee to alive
        payable(aliveHouse).transfer(fee); // Todo - Upgradeble to alive, use a beacon meachnaism
...
contract AliveBondedUpgradeable is ERC721AUpgradeable, ERC2981Upgradeable, AccessControlUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable
    ...
    {   require(msg.value >= currentPriceBondedCurve*amount, "value < mint price");
        require(totalSupply() + amount <= maxBondedCurve, "max supply reached");
        require(amount>0);
         
         uint fee = currentPriceBondedCurve/10; // TODO - remove hardcoding of fees 
         payable(aliveHouse).transfer(fee);
         payable(mintRoyaltyReceiver).transfer(currentPriceBondedCurve - fee);

        if(msg.value > currentPriceBondedCurve) {
              payable(msg.sender).transfer(msg.value-currentPriceBondedCurve);
        }
        emit bondedCurveMintEvent(to, amount, currentPriceBondedCurve);
 ...
 function setURI(string memory _uri) public onlyRole(GOVERNANCE_ROLE) {
  ...
  function contractURI() public view returns (string memory) {
    ...
    function setContractURI(string memory _uri) external onlyRole(GOVERNANCE_ROLE) {
        contracturi = _uri;
    }
...
contract AliveDutchUpgradeable is ERC721AUpgradeable, ERC2981Upgradeable, AccessControlUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable
                         {
    bytes32  constant GOVERNANCE_ROLE = keccak256("GOVERNANCE_ROLE");
    bytes32  constant PAUSER_ROLE = keccak256("PAUSER_ROLE");

    uint public songId;
    uint public edition = 3;
    address payable aliveHouse;
    address public aliveManager;
    address public mintRoyaltyReceiver;
    uint96 public offchainRoyaltyPercentageInBips;
    ...
    function initialize(
        ...
        address artist, 
        address _alivemanager,
        uint[6] memory _configureDutchAuction // total supply, price increase per sale, start price
    ) public initializer initializerERC721A {
 ...
 function getPrice() public view returns (uint) {
    ...
    {     require(startAt < block.timestamp);
          ...
          require(totalSupply() + amount <= maxdutchAuction, "can't mint more than supply");
          super._mint(to, amount);
         ...
         uint fee = price/10; // TODO upgradeable
         payable(aliveHouse).transfer(fee);
         payable(mintRoyaltyReceiver).transfer(price - fee);
   ...
   string public contracturi;
    ...
    function batchAirDrop(address[] memory _arrayOfReceivers, uint _amountForEach) public  onlyRole(GOVERNANCE_ROLE) {
       ...
       require(totalSupply() + _arrayOfReceivers.length <= maxdutchAuction, "can't mint more than supply");
        for (uint256 index = 0; index < _arrayOfReceivers.length;) {
 ...
 function setURI(string memory _uri) public onlyRole(GOVERNANCE_ROLE) {
  ...
  function contractURI() public view returns (string memory) {
    ...
    function pause() public  {
        require(hasRole(PAUSER_ROLE, msg.sender), "Pausable: must have pauser role to pause");
        _pause();
    }

    function unpause() public  {
        require(hasRole(PAUSER_ROLE, msg.sender), "Pausable: must have pauser role to unpause");
        _unpause();
    }
