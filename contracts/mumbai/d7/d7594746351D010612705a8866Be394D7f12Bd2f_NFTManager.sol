...
contract NFTManager is ERC2771TrustForwarderChangeable {
    ...
    function isOwner(
        ...
        return ERC721(nft_contract).ownerOf(token_id) == owner;
    }
    ...
    function transferNFTs(
        ...
        for (uint256 i = 0; i < transfers.length; i++) {
            bool isOwned = isOwner(transfers[i]._contract, transfers[i].tokenId, transfers[i].owner);
            if (isOwned) {
                ERC721(transfers[i]._contract).transferByPermitOperator(
                    $store,
                    transfers[i].owner,
