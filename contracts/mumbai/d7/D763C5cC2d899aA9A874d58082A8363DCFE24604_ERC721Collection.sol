...
contract ERC721Collection is AccessControl, ERC721, ReentrancyGuard, Ownable {
  ...
  function setURI(string memory newuri) public onlyAdmin {
    require(bytes(newuri).length != 0, 'Invalid String');
    baseUri = newuri;
  }
  ...
  function setContractURI(string memory _contractUri) public onlyAdmin {
    require(bytes(_contractUri).length != 0, 'Invalid String');
    contractUri = _contractUri;
  }
  ...
  function burnNFT(address from, uint256 id) external nonReentrant {
    require(from == _msgSender() || hasRole(OPERATOR_ROLE, _msgSender()), 'Not owner or operator');
    require (ownerOf(id) == from, 'Not token owner');
    _burn(id);
  }
  ...
  function safeTransferFrom(
    ...
    require(from != address(0), 'Invalid Address');
    require(to != address(0), 'Invalid Address');
    require (_msgSender() == from || _isApprovedOrOwner(_msgSender(), id), 'Unapproved');
    _safeTransfer(from, to, id, '');
  }
  ...
  function setApprovalForAll(
    ...
    require(_msgSender() == account, 'Not token owner');
