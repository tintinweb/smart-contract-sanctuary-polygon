...
import "./royalties/BaseRoyalties.sol";
...
contract NightbirdFuturists is ERC721A, Ownable, BaseRoyalties {
    ...
    uint256 constant public MAX_SUPPLY = 5556;
    uint256 constant public  MAX_TOKENS_PER_WALLET = 11;
    string public _baseTokenURI;

    //Free Mint Stage
    uint256 constant public MAX_FREE_MINT_SUPPLY = 201;
    uint256 constant public FREE_MINT_PRICE = 0 ether;

    //Pre Mint Stage
    uint256 constant public MAX_PRE_MINT_SUPPLY = 801;
    uint256 constant public PRE_MINT_PRICE = 0.15 ether;

    //Public Mint
    uint256 constant public PUBLIC_MINT_PRICE = 0.22 ether;
    uint256 constant public MAX_PUBLIC_MINT_SUPPLY_PHASE_1 = 1001;
    uint256 constant public MAX_PUBLIC_MINT_SUPPLY_PHASE_2 = 1001;
    uint256 constant public MAX_PUBLIC_MINT_SUPPLY_PHASE_3 = 1001;

    //Reserved Mints
    uint256 constant public MAX_RESERVED_MINTS = 1501;
    ...
    uint256 public mintingPhase; //need open x minting phase by owner
    
    //Mappings to control minting phases
    mapping(uint256 =>uint256) tokensLimitPerPhase;
    mapping(uint256 => uint256) tokensMintedPerPhase; //phase 1 => minted 234 require < max_mint
    mapping(uint256 => uint256) pricePerMintingPhase; //initialize in constructor

    //Royalties. Address placeholder of developer address
    address projectAddress = 0xfeB31F196D4d9115642c505C6efe95be0b75B664;
    ...
    //modifier to ensure that the contract is only called by humans
    ...
    constructor (string memory _unrevealedBaseTokenUri) ERC721A("NightBirdFuturists", "NBF"){
        ...
        setRoyalties(projectAddress, ROYALTIES_POINTS);
    ...
    ///@notice helps set new project address in case of compromised wallet
    function setNewProjectAddress(address _newProjectAddress) external onlyOwner{
    ...
    function reservedMint(uint256 _number) external onlyOwner {
        require(reservedTokensMinted + _number < MAX_RESERVED_MINTS, "Not enough Reserved NFTs left");

        _mint(projectAddress, _number);
        unchecked {
            reservedTokensMinted += _number;
        }

        emit nftMinted(_number, totalSupply(), projectAddress);
    ...
    ///@dev retrieve funds obtained during minting (ETH)
    ...
    ///@dev allows to withdraw ERC20 sent to the contract
    function withdrawERC20(IERC20 _erc20Token) external onlyOwner {
        _erc20Token.transfer(projectAddress, _erc20Token.balanceOf(address(this)));
    }
...
abstract contract BaseRoyalties is ERC165, IERC2981Royalties {
    ...
    function royaltyInfo(uint256, uint256 _value) external view override
