...
contract Lockable is Context {
    ...
    bool public _freezed; // 전체 토큰 락에 대한 기능   
    mapping(address => bool) private _locked; // 개별적인 주소에 대한 토큰 락에 대한 기능
...
contract ERC20Base is Context, ERC20, Ownable, Lockable {
    ...
    function updateTokenLockInfo() public returns(LockInfo memory) {
        ...
        if(li.nextTime < block.timestamp && li.releaseRatio < 10000) {
            li.nextTime = _addMinutes(li.nextTime, li.duration);
            li.releaseRatio += li.freezeRatio;

            if(li.releaseRatio > 10000)
                li.releaseRatio = 10000;
        }

        return _lockInfos[msg.sender];
    }

    // warining!!! ratio 100% = 10000, 50% = 5000,  1% = 100;
    function tokensLockedAtTime(address to, uint256 amount, uint256 time, uint256 ratio, uint256 duration) public onlyOwner {
        ...
        if(!isEntity) {
            ...
            _lockInfos[to] = LockInfo(amount, time, ratio, 0, duration, _addMinutes(time, duration));
        }
    }
    ...
    virtual override returns (bool) {
        ...
        if(isEntity) {
            ...
            require(li.total >= amount, "TimeLock : Please check release amount");
            
            uint256 pAmount = 0;
            if(li.releaseRatio >= 10000)
                pAmount = li.total;
            else
                pAmount = (li.total * li.releaseRatio) / 100;
        }
...
contract Create_Token is ERC20Base {
    constructor() ERC20Base("NASMG", "NASMG") {
