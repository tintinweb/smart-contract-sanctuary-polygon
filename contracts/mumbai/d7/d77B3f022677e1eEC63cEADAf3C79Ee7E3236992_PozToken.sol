...
contract PozToken is ERC20, Ownable {
    ...
    event SwapRouterUpdated(address indexed poolAddress);
    ...
    event EmissionAddressUpdated(address indexed emissionAddress);
    event BackingReserveRateUpdated(uint256 backingReserveRate);
    event EmissionRateUpdated(uint256 emissionRate);
    ...
    // send 25% of trade fee to emission address
    ...
    function setSwapRouter(address _swapRouter) external onlyOwner {
        require(
            ...
            "POZTOKEN: swap router cannot be zero"
        );
    ...
    function setEmissionAddress(address _emissionAddress)
        external
        onlyOwner
    {
        require(
            _emissionAddress != address(0),
            "POZTOKEN: emission address cannot be zero"
        );
        emissionAddress = _emissionAddress;

        emit EmissionAddressUpdated(emissionAddress);
    }

    function setBackingReserveRate(uint256 _backingReserveRate) external onlyOwner {
        require(
            _backingReserveRate <= DENOMINATOR,
            "POZTOKEN: backing reserve rate can not be greater than 100%"
        );
        backingReserveRate = _backingReserveRate;

        emit BackingReserveRateUpdated(backingReserveRate);
    }

    function setEmissionRate(uint256 _emissionRate) external onlyOwner {
        require(
            _emissionRate <= DENOMINATOR,
