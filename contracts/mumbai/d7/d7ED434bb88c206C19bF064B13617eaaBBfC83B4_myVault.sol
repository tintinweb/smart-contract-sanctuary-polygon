...
contract myVault {
  ...
  address public chainLinkETHUSDAddress = 0xd0D5e3DB44DE05E9F294BB0a3bEEaF030DE24Ada;
  ...
  function getTotalBalance() public view returns(uint) {
    ...
    uint wmaticUSD = wmaticBalance * maticPrice; // assumes both assets have 18 decimals
    uint totalBalance = wmaticUSD + linkBalance;
  ...
  function updateMaticPriceChainlink() public returns(uint) {
    int256 chainLinkmaticPrice = EACAggregatorProxy(chainLinkETHUSDAddress).latestAnswer();
    maticPrice = uint(chainLinkmaticPrice / 100000000);
    console.log("maticprice", maticPrice);
    return maticPrice;
  }

  function buyWmatic(uint amountUSD) public {
    ...
    require(linkToken.approve(address(uinswapV3RouterAddress), amountIn), 'DAI approve failed');
  ...
  function sellWmatic(uint amountUSD) public {
    uint256 deadline = block.timestamp + 15;
    uint24 fee = 3000;
    address recipient = address(this);
    ...
    uint160 sqrtPriceLimitX96 = 0;
    require(wmaticToken.approve(address(uinswapV3RouterAddress), amountOut), 'WETH approve failed');
  ...
  function rebalance() public {
    ...
    uint usdBalance = getlinkBalance();//0
  ...
  function wrapMATIC() public {
    ...
    emit myVaultLog('wrapETH', maticBalance);
