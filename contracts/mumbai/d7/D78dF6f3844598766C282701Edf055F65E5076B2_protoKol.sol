...
contract protoKol is iProtoKol {
   ...
   */
    function _swapRouter(
        ...
        _safeApprove(path[0], UNISWAP_ROUTER_ADDRESS, _investment);
    ...
    function investInCampaign(
        ...
        if (campaigns[_campaignId].campaignData.preSaleToken != _USDT) {
            ...
            if (tokenAddress == kolToken) {
                ...
                uint256 kolValue = IOracle(oracle).fetchKOLPrice();
                kolValue = (kolValue * _investment) / 1e18;

                require(
                    (kolValue * (10000 - _transactionPer)) / 10000 <=
                ...
                uint256[] memory amount = _swapRouter(_investment, kolValue, path, address(this));
   ...
   */
    function claimKOLInvestment(uint256 _campaignId) external {
        //Campaign memory campaignDetails = retriveCampaign(_campaignId);
        // require(campaigns[_campaignId].tgeDetails.isTGE == true, "Protokol:!TGE"); //// #change 3
        require(
            campaigns[_campaignId].campaignData.campaignOwner == msg.sender ||
                campaigns[_campaignId].campaignData.secondOwner == msg.sender,
            "Protokol:UnAuthorized"
        ...
        if (_investment > campaigns[_campaignId].campaignData.enteredInvestmentAgainstMarketingBudget)
            _investment =
                ...
                campaigns[_campaignId].campaignData.investmentClaimed;
        // uint256[] memory amount = UniswapV2Library.getAmountsOut(factory, _investment, path);
        uint256 usdtPricePerKOL = IOracle(oracle).fetch(_USDT);
        usdtPricePerKOL = usdtPricePerKOL * _investment;
        usdtPricePerKOL = usdtPricePerKOL / 1e18;
        // uint256[] memory amount =
        _swapRouter(_investment, usdtPricePerKOL, path, msg.sender);
   ...
   */
    function claimBackInvestmentByKOL(
        ...
        // amount =
        _swapRouter(_invest, usdtPricePerKOL, path, msg.sender);
   ...
   */
    ...
    //for testing
    function updateKolToken(address _newAddress) external onlyAdmin {
    ...
    function _getAmountOut(address tokenAddress, uint256 amount) public view returns (uint256[] memory amounts) {
        address[] memory path = new address[](2);
        path[0] = tokenAddress;
        path[1] = _USDT;
...
interface IOracle {
    ...
    function fetchKOLPrice() external returns (uint256 price);
}
