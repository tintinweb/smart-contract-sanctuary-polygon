...
contract IDGAF is ERC721, ERC721Enumerable, Ownable {
    ...
    uint256 public salePrice = 40 ether;
    ...
    ) ERC721("idgafrog", "IDGAF") {
        RESERVED = _reserveAmount;
        remainingReserved = _reserveAmount; //for tracking reserve
    ...
    function mintIDGAF(uint256 numberOfTokens) external payable {
        ...
        require(numberOfTokens > 0, "Must not be zero");
        require(_mintStatus[msg.sender] + numberOfTokens <= maxMintPerUser, "Exceeds maximum allowded mint");
        require(totalSupply() + numberOfTokens <= MAX_SUPPLY - remainingReserved, "Exceeds Max Supply");
        require(msg.value >= salePrice * numberOfTokens, "Ether sent is not correct");
    ...
    //update ipfs after all minting finish
    ...
    function giveAway(address _toAddress, uint256 numberOfTokens) external onlyOwner {
        require(numberOfTokens <= remainingReserved, "Exceeds reserved supply");

        remainingReserved -= numberOfTokens;
