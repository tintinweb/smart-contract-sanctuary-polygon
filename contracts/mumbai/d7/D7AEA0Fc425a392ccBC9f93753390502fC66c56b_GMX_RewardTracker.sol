...
import "./interfaces/GMX_IRewardDistributor.sol";
import "./interfaces/GMX_IRewardTracker.sol";
import "../access/GMX_Governable.sol";

contract GMX_RewardTracker is GMX_IERC20, GMX_ReentrancyGuard, GMX_IRewardTracker, GMX_Governable {
    ...
    function tokensPerInterval() external override view returns (uint256) {
        return GMX_IRewardDistributor(distributor).tokensPerInterval();
    }
    ...
    function claimable(address _account) public override view returns (uint256) {
        ...
        uint256 pendingRewards = GMX_IRewardDistributor(distributor).pendingRewards().mul(PRECISION);
    ...
    function rewardToken() public view returns (address) {
        return GMX_IRewardDistributor(distributor).rewardToken();
    }
    ...
    function _claim(address _account, address _receiver) private returns (uint256) {
        ...
        if (tokenAmount > 0) {
            GMX_IERC20(rewardToken()).safeTransfer(_receiver, tokenAmount);
    ...
    function _stake(address _fundingAccount, address _account, address _depositToken, uint256 _amount) private {
        ...
        GMX_IERC20(_depositToken).safeTransferFrom(_fundingAccount, address(this), _amount);
    ...
    function _unstake(address _account, address _depositToken, uint256 _amount, address _receiver) private {
        ...
        GMX_IERC20(_depositToken).safeTransfer(_receiver, _amount);
    }

    function _updateRewards(address _account) private {
        uint256 blockReward = GMX_IRewardDistributor(distributor).distribute();
...
interface GMX_IRewardDistributor {
