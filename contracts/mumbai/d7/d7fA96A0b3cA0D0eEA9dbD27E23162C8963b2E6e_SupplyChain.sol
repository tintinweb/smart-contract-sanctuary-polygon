...
contract SupplyChain {
    ...
    function getAllocation(uint _id) public view returns (uint, uint, uint,uint,string memory,address, address) {
        ...
        return (allocate.id, allocate.deliveryId, allocate.totalQuantity,allocate.allocatedQuantity,allocate.status, allocate.owner, allocate.brand);
    }
    ...
    function addAllocation(uint _id, uint _deliveryId, uint _allocatedQuantity, address _brand) public {
        ...
        allocations.push(Allocate(_id, _deliveryId, product.totalQuantity, _allocatedQuantity,'CREATED', msg.sender, _brand));
        allocationIdToIndex[_id] = products.length - 1;
    }

    // Accept the allocation by the Brand it was allocated too
    function acceptAllocation(uint _id) public {
        uint allocationIndex = allocationIdToIndex[_id];
        Allocate storage allocate = allocations[allocationIndex];
        // Check that the product exists and the sender is the owner
        require(allocate.id == _id, "Allocation does not exist");
        require(allocate.brand == msg.sender, "Sender is not the brand of the allocation");
        allocate.status = 'ACCEPTED';
    }

    // Reject the allocation by the Brand it was allocated too
    function rejectAllocation(uint _id) public {
        uint allocationIndex = allocationIdToIndex[_id];
        Allocate storage allocate = allocations[allocationIndex];
        // Check that the product exists and the sender is the owner
        require(allocate.id == _id, "Allocation does not exist");
        require(allocate.brand == msg.sender, "Sender is not the brand of the allocation");
        allocate.status = 'REJECTED';
    }
}