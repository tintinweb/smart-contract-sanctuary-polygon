...
abstract contract ERC721BaseCollection is
    ...
    OwnableUpgradeable
{
    ...
    modifier onlyWhitelist() {
        require(whitelists[msg.sender], "NOT_IN_WHITELIST");
        _;
    }
    ...
    function addWhitelists(address[] calldata _whitelists) external onlyOwner {
        for (uint256 i = 0; i < _whitelists.length; i++) {
            address minter = _whitelists[i];
            require(
                minter != address(0),
                "addWhitelists: INVALID_MINTER_ADDRESS"
            );

            whitelists[minter] = true;
            emit Allowed(minter, true);
        }
    }
    ...
    function removeWhitelists(
        ...
        for (uint256 i = 0; i < _whitelists.length; i++) {
            address minter = _whitelists[i];
            whitelists[minter] = false;
            emit Allowed(minter, false);
        }
    }
     ...
     * @param _receiver - owner of the token
    ...
    function _issueToken(
        ...
        _safeMint(_receiver, _itemId);

        // Log
        emit Issue(_receiver, _itemId, _sender);
    }

    /**
     * @notice Issue tokens.
     * @param receivers - owner of the tokens
     * @param quantities - token quantities
     */
    function issueTokens(
        ...
        for (uint256 i = 0; i < receivers.length; i++) {
            ...
            for (uint256 j = 0; j < quantities[i]; j++) {
                _issueToken(receiver, nextTokenId++, sender);
            }
        }
    }
    ...
    /**
     * @notice Set Base URI
     * @param _uri - base URI for token URIs
     */
    function setBaseURI(string calldata _uri) public onlyOwner {
        emit BaseURI(_baseURI(), _uri);
        baseURI = _uri;
    }
...
contract VieNFT is ERC721BaseCollection, UUPSUpgradeable {
    ...
    function initialize(
        ...
        super.initialize(name_, symbol_, uri_);
    ...
    function burn(uint256 tokenId) public {
        require(_isApprovedOrOwner(msg.sender, tokenId), "NOT_ALLOWANCE");
        _burn(tokenId);
    }
