    ...
    NaviMarginable
{
    ...
    function canBorrow(address from, uint256 amount)
        private
        view
        returns (bool)
    {
        return
            _parent.hasEnoughMargin(
                from,
                wdiv(wmul(amount, _oracle.price(_id)), _LTV)
            );
    }

    function canRedeem(address from, uint256 amount)
        private
        view
        returns (bool)
    {
        return
            _parent.hasEnoughMargin(
                from,
                wmul(wmul(amount, _oracle.price(_id)), _LTV)
            );
    }

    function hasEnoughMargin(address account, uint256 amount)
        private
        view
        returns (bool)
    {
        if (balanceOfUnderlying(account) > amount)
            return canRedeem(account, amount);
        return canBorrow(account, amount);
    }
        ...
        onlyBridge
    {
        ...
        require(hasEnoughMargin(account, amount), "Not enough margin");
        require(
    ...
    function redeemPrivate(
        ...
        require(
            skipMarginCheck || canRedeem(redeemer, redeemTokens),
    ...
    function borrowPrivate(
        ...
        require(
            skipMarginCheck || canBorrow(borrower, borrowAmount),
...
/**
 * @title nTokenForERC777
 * @dev ERC777 Recipient nToken
 */
contract nTokenForERC777 is IERC777Recipient, nToken {
    IERC777 internal immutable _token;
    ...
    function tokensReceivedInternal(
        ...
        (uint8 operation, address param) = abi.decode(
            userData,
            (uint8, address)
        );
        require(operation < 4, "nTokenForERC777: Unsupported operation");

        // op 0. Sender supplies assets into the market and receives nTokens in exchange
        // op 1. Sender repays their own borrow
        // op 2. Sender repays a borrow belonging to some other borrower
        if (operation == 0) mintInternal(from, amount);
        if (operation == 1) repayBorrowBehalfInternal(from, from, amount);
        if (operation == 2) repayBorrowBehalfInternal(from, param, amount);
        if (operation == 3) addReservesInternal(amount);
    }
    ...
    function tokensReceived(
        ...
        //we can receive debt token for burn
        if (msg.sender == _oracle.tokenAddress(2)) return;
        if (userData.length == 0) return;
        require(
            msg.sender == address(_token),
            "nTokenForERC777: Invalid token received"
        );
        require(to == address(this), "nTokenForERC777: what the?");

        tokensReceivedInternal(from, amount, userData);

        emit TokenSupplied(operator, from, to, amount, userData, operatorData);
    }

    function doTransferIn(address, uint256 amount)
        internal
        pure
        override
        returns (uint256)
    {
        return amount; //tokens already received
    }
    ...
    function doTransferOut(address to, uint256 amount) internal override {
        IERC20(address(_token)).transfer(to, amount);
        //send() fails if recipient contract has no implementer for ERC777TokensRecipient
    }

    function sweepToken(IERC20 token) external {
        require(
            address(token) != address(_token),
            "nTokenForERC20::sweepToken: can not sweep underlying token"
        );
...
/**
 * @title nTokenForERC777withoutReserves
 * @dev NaviDAO ERC777 Recipient nToken, no need to hold reserves in our own token, just burn them
 */
contract nTokenForNaviDAO is nTokenForERC777 {
    constructor(
        uint32 id,
        string memory name,
        string memory symbol,
    ...
    function burnDebtFor(address, uint256) external pure override {
        revert("Reserves are not claimable from this nToken");
    }

    function getTotalReserves() external pure override returns (uint256) {
        return 0;
    }

    function burnReserves() public nonReentrant {
        accrueInterest();
        uint256 amountBurned = _totalReserves;
        _token.burn(amountBurned, "");
        _totalReserves = 0;
        emit ReservesReduced(msg.sender, amountBurned, _totalReserves);
    }
}