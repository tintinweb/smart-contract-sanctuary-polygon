...
contract RandomARRC is VRFConsumerBaseV2, ConfirmedOwner {
    ...
    event RandomNumberReceived(uint256 indexed tokenId, address user, uint256 indexed requestId, uint256 randomNumber);
    ...
    function fulfillRandomWords(uint256 _requestId, uint256[] memory _randomWords) internal override {
        ...
        emit RandomNumberReceived(tokenId, user, _requestId, randomResult);
    }
...
contract TreasureChest is ERC721URIStorage, Ownable {
    ...
    constructor(
        ...
        //uint256[] memory _levelUpPrices,
        //uint256[] memory _levelCaps
    ...
    function levelUp(uint256 tokenId, uint256 targetLevel) external payable {
        ...
        require(keccak256(bytes(treasureChests[tokenId].state)) == keccak256(bytes("chest")), "Chest is already opened");
        ...
        require(levelCount[nextLevel] < levelCap[nextLevel], "Next level is fully minted");
        uint256 totalPrice = calculateTotalPrice(currentLevel, targetLevel);
    ...
    function calculateTotalPrice(uint256 currentLevel, uint256 targetLevel) internal view returns (uint256) {
        uint256 totalPrice = 0;
        for (uint256 level = currentLevel + 1; level <= targetLevel; level++) {
            uint256 price = levelUpPrice[level];
            require(price > 0, "Level up price is not set");
            totalPrice += price;
        }
        return totalPrice;
    }
    ...
    function open(uint256 tokenId) external {
        ...
        if (treasureChests[tokenId].level > 1) {
    ...
    function updatePublicMintingEnabled(bool enabled) external onlyOwner {
        isPublicMintingEnabled = enabled;
    }

    function updateWhitelistMintingEnabled(bool enabled) external onlyOwner {
        isWhitelistMintingEnabled = enabled;
    }

    function updateOpenEnabled(bool enabled) external onlyOwner {
        isOpenEnabled = enabled;
    }
