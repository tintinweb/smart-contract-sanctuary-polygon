    ...
    function burnPirates(uint256[] memory tokenIDs) public {
        for (uint256 i = 0; i < tokenIDs.length; i++){
            ...
            if (isActivePirate[tokenIDs[i]]) {
                tokenIndex[activePiratesByOwner[msg.sender][activePiratesByOwner[msg.sender].length - 1]] = tokenIndex[tokenIDs[i]];
                activePiratesByOwner[msg.sender][tokenIndex[tokenIDs[i]]] = activePiratesByOwner[msg.sender][activePiratesByOwner[msg.sender].length - 1];
