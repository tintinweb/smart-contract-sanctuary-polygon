...
import {Direction, Order, VaultSwapParams} from './utils/Structures.sol';
import {IVaultLimitUpgradeable2} from './interfaces/IVaultLimitUpgradeable.sol';
...
// contracts
import {VaultVolumeUpgradeable} from './VaultVolumeUpgradeable.sol';

/**
 * @title VaultLimitUpgradeable
 * @author gotbit
 * @notice The contract responsible for placing, removing and fulfilling
 * trade orders with funds transferred to it.
 * @dev Inherits from the VaultVolumeUpgradeable contract.
 */

contract VaultLimitUpgradeable is VaultVolumeUpgradeable, IVaultLimitUpgradeable2 {
    using SafeERC20 for IERC20;

    /// @inheritdoc IVaultLimitUpgradeable2
    function init(
        address manager_
    )
        external
        virtual
        override(IVaultLimitUpgradeable2, VaultVolumeUpgradeable)
        initializer
    {
        __Vault_init(manager_, VaultType.LIMIT);
    }

    /// @notice Returns order info: ID, sender, direction, price_min, price_max, volume.
    Order[] public orders;
    ...
    function placeOrder(
        ...
        localRoles[0] = EXECUTOR_LIMIT_ROLE;
    ...
    function removeOrder(
        ...
        emit RemoveOrder(id, msg.sender);
    }
    ...
    function fulfillOrder(
        ...
        require(amountOut <= ord.volume, 'volume too big');
...
import {Direction, Order} from '../utils/Structures.sol';
import {IVaultVolumeUpgradeable} from './IVaultVolumeUpgradeable.sol';
import {OraclePermission} from '../utils/OraclePermission.sol';

/**
 * @title IVaultLimitUpgradeable
 * @author gotbit
 * @notice Interface for the VaultLimitUpgradeable contract.
 * @dev The VaultLimitUpgradeable contract is responsible for placing,
 * removing and fulfilling trade orders with funds transferred to it.
 * It inherits from the VaultVolumeUpgradeable contract.
 * The orders() function is separate because of the solc bug that
 * prevents us from inheriting the interface otherwise.
 */

interface IVaultLimitUpgradeable2 is IVaultVolumeUpgradeable {
    ...
    event RemoveOrder(uint256 indexed id, address indexed sender);

    /// @notice Initializes the vault.
    function init(address manager_) external override(IVaultVolumeUpgradeable);

    /// @notice Returns the amount of orders created, both active and inactive.
    function ordersLength() external view returns (uint256);

    /// @notice Places an order.
    /// @param ord The order to be placed.
    ...
    /// @notice Fulfills an order, checking the price and volume.
    /// @param id The ID of the order to be fulfilled.
    /// @param useReceiver Whether to use a receiver.
    /// @param amountIn The amount of tokens to be sent ("quote" token).
    /// @param amountOut The amount of tokens to be received ("base" token).
...
interface IVaultLimitUpgradeable is IVaultLimitUpgradeable2 {
    /// @notice Returns order info: ID, sender, direction, price_min, price_max, volume.
