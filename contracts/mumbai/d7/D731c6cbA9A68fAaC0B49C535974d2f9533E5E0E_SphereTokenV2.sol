...
contract SphereTokenV2 is ERC20Detailed, Ownable {
    ...
    uint256 private constant MAX_PARTY_ARRAY = 490;
    ...
    function setWhitelistSetters(address _addr, bool _value, uint256 _type) external onlyOwner {
        if (_type == 1) {
    ...
    function manualRebase() external {
        ...
        while (nextRebase < block.timestamp);
    ...
    function setRebaseType(bool _value) external onlyOwner {
        rebaseType = _value;

        emit SetRebaseType(_value);
    }
    ...
    function setWallDivisor(uint256 _wallDivisor, uint256 _wallMultiplier) external onlyOwner {
        wallDivisor = _wallDivisor;
        wallMultiplier = _wallMultiplier;
        emit SetWallDivisor(_wallDivisor, _wallMultiplier);
    }
    ...
    event SetRebaseType(bool indexed value);
