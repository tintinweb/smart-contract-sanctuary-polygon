...
contract NFTRentMarketplace is VRFConsumerBaseV2, ConfirmedOwner, IERC721Receiver {
  ...
  function fulfillRandomWords(uint256 _requestId, uint256[] memory _randomWords) internal override {
    require(rents[_requestId].status == RentStatus.REQUESTED, "Request not found or already fulfilled");
  ...
  function startRent(uint256 requestId) public {
    require(rents[requestId].status == RentStatus.REQUEST_FULFILLED, "This Rent cannot start");
    ...
    require(
      pool.availableItems.length == 0 || pool.availableItems[index] != selectedItemId,
  ...
  function finishRent(uint256 requestId) public {
    require(rents[requestId].status == RentStatus.ACTIVE, "This Rent is not Active");
    Rent storage rent = rents[requestId];
