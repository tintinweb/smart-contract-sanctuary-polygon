...
contract StakingFactory is Ownable, IStakingFactory {
    ...
    /**
     * @notice total timestamps of 52 weeks
     * timestamp value not block count, because of backend referring we can't change the name as like totalSecondsPerYear
     */
    uint256 public constant totalBlocksPerYear = 364 * 24 * 60 * 60; 
    ...
    /**
     * @notice check the ufo pool is already setup or not
     */
    bool public isUfoPoolSetup;

    /**
     * @notice check the lp pool is already setup or not
     */     
    bool public isLpPoolSetup;
    ...
    constructor(address _admin, uint256 _ufoRewardsForUfoPoolsPools, uint256 _ufoRewardsForLpPoolsPools, address _rewardToken ) {
        ...
        require(_rewardToken != address(0), Errors.SHOULD_BE_NON_ZERO);
        Ownable.transferOwnership(_admin);
        ufoRewardPerSecondOfUfoPool = _ufoRewardsForUfoPoolsPools / totalBlocksPerYear;
        ufoRewardPerSecondOfLpPool = _ufoRewardsForLpPoolsPools / totalBlocksPerYear;

        rewardToken = _rewardToken;
        startTimeStamp = block.timestamp;
        uint256 allocPoint;
        for(uint256 i = 0; i < totalPools; i = i + 2) {
            allocPoint += 1e18 + (i * 19230769230769230);
        }
        totalAllocPoint = allocPoint;
    }

    /**
     * @notice initialize ufo the staking contract
     * @param _beacon address of the beacon contract
     * @param _ufoToken Address of the ufo token    
     */
    function setupUFOPools(address _beacon, address _ufoToken) external onlyOwner {
        require(isUfoPoolSetup == false, Errors.ALREADY_SETUP);
        isUfoPoolSetup = true;
        _createUfoAndLpPools(_beacon, true, _ufoToken, owner());
    }

    /**
     * @notice initialize lp the staking contract
     * @param _beacon address of the beacon contract     
     * @param _lpToken Address of the lp token
     */
    function setupLPPools(address _beacon, address _lpToken) external onlyOwner {
        require(isLpPoolSetup == false, Errors.ALREADY_SETUP);
        isLpPoolSetup = true;
        _createUfoAndLpPools(_beacon, false, _lpToken, owner());
    }

    function _createUfoAndLpPools(address _beacon, bool isUfoPool,address _Token,address _admin) internal {
        uint256 timeStampsPerWeek = totalBlocksPerYear / 52; //timestamps per week (1 year is 52 weeks)        
        for(uint256 index = 0; index < totalPools; index = index + 2) {
            if(isUfoPool == true)
                _createPool(_beacon, index, _Token, timeStampsPerWeek * index + 1, _admin, 1e18 + (index * 19230769230769230));
            else 
                _createPool(_beacon, index + 1, _Token, timeStampsPerWeek * index + 1, _admin, 1e18 + (index * 19230769230769230));            
        }
        if(isUfoPoolSetup == true && isLpPoolSetup == true) emit FinishCreatePool();
    }
