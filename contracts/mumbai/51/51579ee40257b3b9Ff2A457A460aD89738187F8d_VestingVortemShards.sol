...
/**
 * @title Vesting Vortem Shards Smart Contract
 */
...
contract VestingVortemShards is Ownable, ReentrancyGuard, Pausable {
    enum VestingType {
        PUBLIC_SALE,
        STRATEGIC,
        SEED,
     ...
     */
    struct UserInfo {
        mapping(VestingType => VestAllocation) detail;
        VestingType[] vestIndexID;
        bool isVesting;
    }

    /**
     * @notice vortem shards have 18 of decimal
     */
     ...
     */
    mapping(address => UserInfo) private walletToVestAllocations;
    ...
    modifier checkUserStatus(address _user) {
        require(
            walletToVestAllocations[_user].isVesting,
            "checkUserStatus: User not recipient"
        );
        _;
    }
     ...
     * @param _vortemShard Address of token.
     * @param _vestStartTime Vesting Start time event unix timestamp.
     */
    constructor(IERC20 _vortemShard, uint256 _vestStartTime) {
        ...
        nonReentrant
    {
        ...
        UserInfo storage user = walletToVestAllocations[_msgSender()];
        uint256 totalToPay;

        for (uint256 i = 0; i < user.vestIndexID.length; i++) {
            uint256 amountToPay = _calculateClaimableTokens(
                user.detail[user.vestIndexID[i]]
            );
            totalToPay += amountToPay;
            user
                .detail[user.vestIndexID[i]]
                .totalVestTokensClaimed += amountToPay;
        }
        require(totalToPay > 0, "claimVesting: Nothing to withdraw");

        vortemShard.transfer(_msgSender(), totalToPay);
    ...
    function addRecipients(
        UserImport[] calldata _userImport,
    ...
    function updatePercentageRecipient(
        ...
        UserInfo storage user = walletToVestAllocations[_user];
    ...
    function withdrawTokens() external onlyOwner {
        ...
        vortemShard.transfer(_msgSender(), tokenSupply);
    }
    ...
    /**
     * @notice to get detail recipient information
     * @return VestAllocation[], totalClaimed => array of list vesting info and total claim user
     */
    function getRecipientInfo(address _user)
        external
        view
        returns (
            VestAllocation[] memory,
        ...
        )
    {
        ...
        VestAllocation[] memory userDetail = new VestAllocation[](
            user.vestIndexID.length
        );

        for (uint256 i; i < user.vestIndexID.length; i++) {
            ...
            lockedBalance += (user
                .detail[user.vestIndexID[i]]
                .totalTokenAllocated - unlockedBalance);
            totalClaimed += user
                .detail[user.vestIndexID[i]]
                .totalVestTokensClaimed;
        }

        return (userDetail, vestingStartTime, lockedBalance, unlockedBalance, totalClaimed);
    }
     ...
     */
    function _calculateClaimableTokens(VestAllocation memory _userVestData)
        internal
        view
        returns (uint256)
    {
     ...
     */
    function _calculateAvailableTokens(VestAllocation memory _userVestData)
        internal
        view
        returns (uint256)
    {
         ...
         */
            ...
            if (vestData.lockPeriod > 0) {
                 ...
                 * after 22 months. then next has 3 month a period interval
                 */
                if (
                    _userVestData.vestIndexID == VestingType.STRATEGIC &&
                    (totalMonthsElapsed > 22)
                ) {
                    /**
                     * 22 subtract monthly start reward to calculate the number of months before month 22
                     * after that the number of months is subtracted by 22
    ...
    function _addRecipient(
        ...
        user.vestIndexID.push(_vestingIndex);
        user.isVesting = true;
