...
interface Target {
    ...
    function getPlant(uint256 _tokenId) external view returns (uint256);
}
...
contract TheBusiness is Ownable {
    ...
    Target private target = Target(0x13de43C1f2b1dedD575aC9f14cd2F8364C991a46);
    Auction private auction = Auction(0xBAA5574c48aa2aE290e1505497F1f432641b14B4);
    ...
    function areOnSell(address owner, uint256[] memory tokens) public view returns(bool[] memory) {
        bool[] memory onSells = new bool[](tokens.length);

        for(uint256 i = 0; i < tokens.length; i++) {
            onSells[i] = isOnSell(owner, tokens[i]);
        }

        return onSells;
    }

    function isOnSell(address owner, uint256 _tokenId) public view returns(bool) {
        try auction.getAuction(_tokenId) returns (address seller, uint256, uint256, uint256, uint256) {
            if (seller != owner) {
                return false;
            }
            return true;
    ...
    function getTokens(address _owner) public view returns(Info[] memory) {
        uint256 tokenQty = target.balanceOf(_owner);
        Info[] memory infos = new Info[](tokenQty);
        uint256[] memory tokens = target.tokenOfOwner(_owner);

        for(uint256 i = 0; i < tokenQty; i++) {
            uint256 plantId = target.getPlant(tokens[i]);
            infos[i] = Info({
                tokenId: tokens[i],
                plantId: plantId
            });
    ...
    function getID(address _owner, uint256 randomNumber) public view returns(uint256){
        ...
        if(number <= 9600){
            ...
            uint256 treeID = 2_000_000_000 + (number + 1) % 9 * 10**8;
            
            if (number <= 9850) {
                return treeID + 1010101; // common
            } else if (number <= 9950) {
                return treeID + 2020102; // rare
            } else if (number <= 9990) {
                return treeID + 3030103; // epic
            } else {
                return treeID + 4040104; // legendary
            }
        }
    } 
}