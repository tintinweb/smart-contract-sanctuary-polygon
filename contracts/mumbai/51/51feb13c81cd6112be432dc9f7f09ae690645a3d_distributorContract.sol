...
contract distributorContract is ReentrancyGuardUpgradeable,OwnableUpgradeable {
        ...
        function deposit(uint poolId, uint amount)external{
                logic(poolId, amount,true,0);
        }
        function investBack(uint poolId, uint _nftId,uint desiredPool) external {
         uint amount= getRewardDetails(poolId, _nftId);// 32600000000000000000
          ...
          logic(desiredPool,amount,false,poolId);
          detailsPerNftId[_nftId][poolId].lastClaimTime=block.timestamp;
        }

        function logic (uint poolId,uint tokenAmounts,bool status, uint actualOne) internal nonReentrant {
               ...
               IGovernanceNFT(governanceTokenAddress[pool]).mintTokens(msg.sender, nftIndexPerPool[pool]);
