...
contract NftFacet is ERC721AUpgradeable {
   error ProfileNFT__isAddress0();
    ...
    function awardNFT(address recipient, string memory tokenURI) callingFromDiamond public returns (uint256) {
		
		_safeMint(recipient, 1);
        uint256 newItemId = s()._currentIndex;
		setTokenURI(newItemId, tokenURI);

		return newItemId;
	}

    function setTokenURI(uint256 tokenId, string memory _tokenURI) callingFromDiamond idDoExists(tokenId) public onlyOwnerCan {
        s()._tokenURIs[tokenId] = _tokenURI;

        // emit MetadataUpdate(tokenId);
    }

    function tokenURI(uint256 tokenId) callingFromDiamond idDoExists(tokenId) public view virtual returns (string memory) {
    ...
    function tokenProgress(uint256 tokenId) callingFromDiamond idDoExists(tokenId) public view virtual returns (uint256) {
        return s()._progress[tokenId];
    }

    function setTokenProgress(uint256 tokenId, uint256 progress) callingFromDiamond idDoExists(tokenId) public onlyOwnerCan {
