...
contract TokenLock {
    ...
    function lock(uint _lockedAmount, uint _releaseStartTime, uint _releaseEndTime, address _locker) external {
        require(IERC20(lockedToken).balanceOf(address(this)) == _lockedAmount, "lockedAmount invalid");
        require(_releaseStartTime > block.timestamp, "releaseStartTime must be greater than current time");
        require(_releaseStartTime < _releaseEndTime, "releaseStartTime must be less than releaseEndTime");
    ...
    function canClaim() public view returns (uint) {
        ...
        uint canClaimTotal = (block.timestamp - releaseStartTime) * releaseRate;
        return canClaimTotal - claimed;
    }

    function claim() public returns (uint) {
        require(msg.sender == locker, "Only locker");
        uint canClaimAmount = canClaim();
...
contract TokenLockFactory is ITokenLockFactory {

    function createTokenLock(address _lockedToken) external override returns (address) {
        TokenLock tokenLock = new TokenLock(_lockedToken);
        return address(tokenLock);
    }
}
