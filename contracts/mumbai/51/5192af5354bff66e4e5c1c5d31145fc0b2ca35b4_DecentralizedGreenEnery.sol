...
contract DecentralizedGreenEnery is Context, IERC20, Ownable {  
    ...
    uint256 public Price = 0.00000001 ether;
    uint256 public priceIncrement = 0.0000000000001 ether;
    
    address [] public tokenHolder;
    uint256 private numberOfTokenHolders = 0;
    ...
    event onTokenSell(address indexed customerAddress,uint256 tokensBurned,uint256 _taxedMatic);
    ...
    function updateprice(uint _Price_,uint PriceIncremental_) onlyOwner public {
        Price =_Price_;
        priceIncrement= PriceIncremental_; 
        emit Updateprice();
    }
    ...
    */
    ...
    function sell(uint256 amount) public { 
        ...
        require(amount <=  _rOwned[_wallet],"Insufficient Token ?");
        uint tokensReceived = (amount * priceIncrement) / (10**18);
        Price -= tokensReceived;
        // uint256 _tokens = amount;
        // uint256 _matic = AutoUpdateSellPrice(_tokens);
        // uint256 _marketing = calculateFee(_matic,_marketingPer);
        // uint256 _autoBurn = calculateFee(_matic,_autoBurnPer);
        // uint256  _reward = calculateFee(_matic,_RewardPer);
        // uint256 _taxedMatic = SafeMath.sub(SafeMath.sub(SafeMath.sub(_matic, _marketing),_autoBurn), _reward);
        ...
        uint256 netAmount = amount.sub(_marketing).sub(_reward);
        uint256 totalburn =netAmount+_autoBurn;
        _burn(msg.sender, totalburn);
        require( transfer (address(this),(_marketing+_reward)),"Tax amount cannot be  transfer successfully");
        _wallet.transfer(tokensReceived);
        //Fire event
        emit onTokenSell(_wallet, amount,tokensReceived); 
    }

    

    function buy(uint amount)public payable returns (uint) {
        require(msg.value > Price, "Insufficient payment");
        require(!paused, "Function is paused don't sell token");
        uint256 matic = Price * amount;
        uint256 _marketing = amount.mul(_marketingPer).div(100);
        uint256 _autoBurn = amount.mul(_autoBurnPer).div(100);
        uint256 _reward = amount.mul(_RewardPer).div(100);
        uint256 totalMint = amount;
        uint256 netAmount = totalMint.sub(_marketing).sub(_reward).sub(_autoBurn);
        ...
        emit onTokenPurchase(msg.sender, matic, amount);
        AutoUpdateBuyPrice(amount);
    }

   
    function AutoUpdateBuyPrice( uint amount)  internal  {
        Price+=((amount / 10**18)*priceIncrement);
    }


    function AutoUpdateSellPrice( uint amount)  internal  returns(uint){
       uint maticreceived= Price-=((amount / 10**18)*priceIncrement);
       return maticreceived;
    }


    function GetBuyTokenPrice(uint amount) public view returns (uint){
        uint256 Getmatic =Price*amount;
        return Getmatic;
    }
