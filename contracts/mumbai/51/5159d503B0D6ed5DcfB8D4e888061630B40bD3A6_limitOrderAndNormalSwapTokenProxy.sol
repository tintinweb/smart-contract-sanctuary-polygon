...
contract limitOrderAndNormalSwapTokenProxy is OpsTaskCreator, Ownable {
    ...
    function massCancelTask(bytes32[] memory _taskIDList) external {
        (bool success,bytes memory result) = OrderAdminAddress.delegatecall(abi.encodeWithSelector(limitOrderAndNormalSwapToken.massCancelTask.selector, _taskIDList));
        _getRevertMsg(success, result);
    }

    function massRestartTask(bytes32[] memory _taskIDList) external {
        (bool success,bytes memory result) = OrderAdminAddress.delegatecall(abi.encodeWithSelector(limitOrderAndNormalSwapToken.massRestartTask.selector, _taskIDList));
        _getRevertMsg(success, result);
    }
