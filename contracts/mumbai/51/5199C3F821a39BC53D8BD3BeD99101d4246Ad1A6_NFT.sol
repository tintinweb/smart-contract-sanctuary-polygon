...
contract NFT is ERC1155 ,Ownable{
    ...
    function burn(uint256 _id,uint256 _quantity) external existCheck(_id) createrCheck(msg.sender,_id) {
        ...
        require(_quantity != 0,"quantity required.");
        require(_balance >= _quantity,"Quantity exceeds holdings.");

        _burn(msg.sender, _id, _quantity);
        emit Burn(_id, _quantity);
    }

    function balanceOfCreater(uint256 _id,address creater) external view returns(uint256){
        uint256 _balance = balanceOf(creater,_id);
        return _balance;
    }

    function setURI(string memory _uri) external {
        require(keccak256(abi.encodePacked(_uri)) != keccak256(abi.encodePacked("")),"No URI entered.");
        baseMetadataURIPrefix[msg.sender] = _uri;
        emit SetURI(_uri);
    }

    function getURI() external view returns(string memory){
        return baseMetadataURIPrefix[msg.sender];
    }
