...
contract NFTMarketplace {
    ...
    function buyMergeNFT(uint256 _id) external payable {
        (, uint256 tokenId) = _buyNFT(_id);
    ...
    function sellMergeNFT(uint256 linkedListId, uint256 nftPrice) external {
        ...
        address ownerOfLinkedList = oneForge.listIdOwner(linkedListId);

        require(linkListSize != 0, "Not contain any data");
        require(msg.sender == ownerOfLinkedList, "Not owned NFT");

        _sellFixed(linkedListId, nftPrice, linkListSize);
    }
    ...
    {
        require(msg.sender == mintNft.ownerOf(_tokenId), "Not called by owner");
        _sellFloat(_tokenId, _premiumValue, 1);
    }

    function sellMergeNFTForFloatPrice(
        uint256 linkedListId,
        uint256 _premiumValue
    ) external {
        uint256 linkedListSize = oneForge.sizeOfList(linkedListId);
        address ownerOfLinkedList = oneForge.listIdOwner(linkedListId);

        require(linkedListSize != 0, "Not contain any data");
        require(msg.sender == ownerOfLinkedList, "Not owned NFT");
        _sellFloat(linkedListId, _premiumValue, linkedListSize);
    }
...
interface IOneForge {
    ...
    function setLinkedListOwner(uint256 _linkedListId, address user) external;
