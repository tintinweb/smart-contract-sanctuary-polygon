    ...
    */
    ...
    function openChests(uint256 _numChests) external {
        ...
        for (uint256 i = 0; i < _numChests; i++) {
            // Explicit reference :(
            uint256 reqId = VRFManager(vrfCoordinator).getRandomNumber(1);
     ...
     */
    function _drawContentAmt(uint256 _randomWord) internal returns (uint256 _amount) {
     ...
     */
    function _drawTreasure(uint256 _randomWord)
        internal
        returns (uint256 _assetId, uint256 _assetAmount)
    {
