...
contract Vault{
   ...
   */
   function depositTokens(uint _packageId, address _contractAd, uint _tokens) external{
      ...
      uint packageVal = opc.packageTotalValue(_packageId);
      uint packageTotalValue = package[_packageId].fieldVal * packageVal;
   ...
   function transferpackageTokensValue(uint _packageId) external{
      ...
      for(uint i = 0; i < addresses.length; i++){
         if(!userRewardStatus[_packageId][addresses[i]]){
            uint userValue = opc.userTransferredDataValue(addresses[i], _packageId); // eg: 15 
            uint userTotalValue = package[_packageId].fieldVal * userValue; // 15 * 20 = 300
