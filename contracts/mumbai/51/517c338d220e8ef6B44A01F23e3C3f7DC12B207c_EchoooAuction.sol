    ...
    Pausable,Initializable,UUPSUpgradeable
{
    ...
    event EventHighestBidIncreased(address,uint256, address, address, uint256);
    ...
    function englishAuctionERC721Bid(Order memory order,uint256 _price, bytes32 orderHash) external payable{
        ...
        emit EventHighestBidIncreased(order.tokenAddress,order.tokenId,_owner,msg.sender,msg.value);
    }
    ...
    function englishAuctionERC1155Bid(Order memory order, uint tokenAmount, uint _price, bytes32 orderHash) external payable {
        ...
        emit EventHighestBidIncreased(order.tokenAddress,order.tokenId,order.user,msg.sender,msg.value);
    }
    ...
    function englishAuctionBuy(Input memory input, bytes32 orderHash) public {
        ...
        require(order.status == Status.AUCTION && orderStatus[orderHash] == Status.NEW,'Auction:order already exist');
        require(_isNative(IERC20(order.currencyAddress)), 'settle only in ETH');
        require(order.tokenAddress == input.settle.tokenAddress && order.tokenId == input.settle.tokenId && order.amount == input.settle.amount,'Invalid order params');
        require(
            address(order.executionDelegate) != address(0) && 
            delegates[address(order.executionDelegate)],
            'Auction:invalid delegateAddress'
    ...
    function DutchAuctionBuy(Input memory input) public payable{
        ...
        //settlement price (all protocol fee, royalty gonna deduct from this price)
        uint _price = getCurrentPrice(order.startTime,order.endTime,order.lowestPrice, order.price);
        ...
        if(_isNative(IERC20(order.currencyAddress))) {
            ...
            require(input.settle.price >= _price, "amount should be greater than price of token");
        }
        orderStatus[orderHash] = Status.COMPLETE;
    ...
    /** get Current Time for dutch auction */
    function getCurrentPrice(uint startTime, uint endTime, uint endPrice, uint startPrice) public view returns (uint256) {
        ...
        if (elapsed >= duration) {
            ...
            uint256 priceDecrement = priceDiff * elapsed / duration;
    ...
    function _distributeFeeAndProfit(
        ...
        //marketplace fee
        if(paymentReceiver != address(0) && paymentReceiverFee > 0){
