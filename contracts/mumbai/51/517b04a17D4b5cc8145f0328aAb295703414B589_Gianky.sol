...
contract Gianky is ERC721, ERC721URIStorage {
    ...
    uint256 private constant STARTER_PRICE = 0.01 * 10 ** 18; // 0.01 BNB on BSC
    uint256 private constant BASIC_PRICE = 0.02 * 10 ** 18; // 0.02 BNB on BSC
    uint256 private constant STANDARD_PRICE = 0.03 * 10 ** 18; // 0.03 BNB on BSC
    uint256 private constant VIP_PRICE = 0.04 * 10 ** 18; // 0.04 BNB on BSC
    uint256 private constant PREMIUM_PRICE = 0.05 * 10 ** 18; // 0.05 BNB on BSC
    uint256 private constant DIAMOND_PRICE = 0.06 * 10 ** 18; // 0.06 BNB on BSC
    ...
    uint256 public GIANKY_REWARD_RATE = 10000; // 0.005 dollars per Gianky token
    ...
    string private constant STARTER_URI =
        "https://ipfs.io/ipfs/QmZBgSq4rF3dmS4m9KjVa6MeJgCtxGzNsMsLsg7HTxjRvB/0";
    string private constant BASIC_URI =
        "https://ipfs.io/ipfs/QmRZpRMTC9aUrUBG44FZTrLuwkkpEyMWeucbrnfoXAraN8/1";
    string private constant STANDARD_URI =
        "https://ipfs.io/ipfs/QmQzSWR8XXfTzQQkYL5ZssZmFtezeNYvZ7UyA9mibHPsxD/2";
    string private constant VIP_URI =
        "https://ipfs.io/ipfs/QmTuXktKUqdirDL7P8ryfa96mbt7NTnkHpW2ubNJ9Hfbea/3";
    string private constant PREMIUM_URI =
        "https://ipfs.io/ipfs/Qmb5EqoT4VPqSBSQgZBgSuzr6c5bsBkUBqvRfH3BPdJCP7/4";
    string private constant DIAMOND_URI =
        "https://ipfs.io/ipfs/QmP76DQ5yTCfifjFrqa6ZAWy5KySCuCoeXShWR1Jk7UygN/5";
    ...
    function mintStarter(uint256 _referralId) public payable returns (uint256) {
        ...
        require(_referralId >= STARTER_MIN_ID && _referralId <= DIAMOND_MAX_ID);
    ...
    function mintBasic(uint256 _referralId) public payable returns (uint256) {
        address _to = msg.sender;
        require(_referralId >= BASIC_MIN_ID && _referralId <= DIAMOND_MAX_ID);
    ...
    function mintVIP(uint256 _referralId) public payable returns (uint256) {
        address _to = msg.sender;
        require(_referralId >= VIP_MIN_ID && _referralId <= DIAMOND_MAX_ID);
    ...
    function mintDiamond(uint256 _referralId) public payable returns (uint256) {
        address _to = msg.sender;
        require(_referralId >= DIAMOND_MIN_ID && _referralId <= DIAMOND_MAX_ID);
