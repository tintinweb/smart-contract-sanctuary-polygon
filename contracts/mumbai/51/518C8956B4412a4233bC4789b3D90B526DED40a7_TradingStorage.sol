...
contract TradingStorage {
    ...
    TokenInterface public constant dai = TokenInterface(0x8f3Cf7ad23Cd3CaDbD9735AFf958023239c6A063);
    TokenInterface public constant linkErc677 = TokenInterface(0xb0897686c545045aFc77CF20eC7A532E3120E0F1);

    // Contracts (updatable)
    AggregatorInterface public priceAggregator;
    PoolInterface public pool;
    PausableInterface public trading;
    PausableInterface public callbacks;
    TokenInterface public skel = TokenInterface(0x0000000000000000000000000000000000000000);
    NftInterface[1] public nfts = [
    ...
    uint[1] public spreadReductionsP = [25];             // %, array to allow new NFTs in future

    // Gov & dev addresses (updatable)
    address public gov = 0x1989d0CCCCA5530f48c3abB36A95841A21BacD95;
    address public dev = 0x1989d0CCCCA5530f48c3abB36A95841A21BacD95;
    ...
    event NftsUpdated(NftInterface[1] nfts);
    event NumberUpdated(string name,uint value);
    event NumberUpdatedPair(string name,uint pairIndex,uint value);
    event SpreadReductionsUpdated(uint[1]);

    constructor() {}

    // Modifiers
    modifier onlyGov(){ require(msg.sender == gov); _; }
    modifier onlyTrading(){ require(isTradingContract[msg.sender] && skel.hasRole(MINTER_ROLE, msg.sender)); _; }
    ...
    function updateToken(TokenInterface _newToken) external onlyGov{
        ...
        skel = _newToken;
        emit AddressUpdated("skel", address(_newToken));
    }
    function updateNfts(NftInterface[1] memory _nfts) external onlyGov{
    ...
    function addTradingContract(address _trading) external onlyGov{
        require(skel.hasRole(MINTER_ROLE, _trading), "NOT_MINTER");
    ...
    function setTrading(address _trading) external onlyGov{
        require(_trading != address(0));
        trading = PausableInterface(_trading);
    ...
    function setCallbacks(address _callbacks) external onlyGov{
        ...
        callbacks = PausableInterface(_callbacks);
    ...
    function setSpreadReductionsP(uint[1] calldata _r) external onlyGov{
        require(_r[0] > 0);
    ...
    function claimFees() external onlyGov{
        skel.mint(dev, devFeesToken);
        skel.mint(gov, govFeesToken);
    ...
    function handleTokens(address _a, uint _amount, bool _mint) external onlyTrading{ 
        if(_mint){ skel.mint(_a, _amount); tokensMinted += _amount; } 
        else { skel.burn(_a, _amount); tokensBurned += _amount; } 
    }
    ...
    function getSpreadReductionsArray() external view returns(uint[1] memory){
