    ...
    Initializable
{
    ...
    function sellNft(
        ...
        if (
            (_marketplaceManager.isBlocked(msg.sender) == true)
    ...
    function mintSellNft(
        ...
        uint256 tokenId = ArtfiIMarketplace(_marketplace).mintNft(
            mintData_
        );
        tokenId_ = tokenId;
    ...
    function updateSalePrice(
        ...
        if (_marketplaceManager.isPaused())
            revert GeneralError("NS:128");
        if (
            (_marketplaceManager.isBlocked(msg.sender) == true)
        ) revert GeneralError("NS:126");
        ArtfiIMarketplace.Offer memory offer = ArtfiIMarketplace(
            _marketplace
