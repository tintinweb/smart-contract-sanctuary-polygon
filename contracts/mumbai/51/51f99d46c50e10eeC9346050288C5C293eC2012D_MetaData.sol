...
contract MetaData is IMetaData, VRFConsumerBaseV2, Ownable {
    ...
    function requestNextStepFor(uint256 tokenId) public onlyOwner {
        ...
        if (metaData.status == NFTStatus.DONE) {
    ...
    function fulfillRandomWords(
        ...
        if (metaData.inProgress && metaData.status == NFTStatus.MINTED) {
            ArtworkUtils.Artwork memory artwork;

            uint256 legendary_roll = randomWords[0] %
                (i_totalSupply - s_revealCount);

            uint256 legendaryId = s_legendaryIndex;

            if (
                legendaryId <= i_legendaries &&
                legendary_roll < i_legendaries - legendaryId
            ) {
            ...
            s_revealCount = s_revealCount + 1;
            s_artworkRegistry[artwork.id] = true;
            metaData.artwork = artwork;
            metaData.status = NFTStatus.REVEALED;
        ...
        if (metaData.inProgress && metaData.status == NFTStatus.WINNER) {
            ...
            if (s_prizes[prizeIndex][1] == 0) {
                ...
                emit MetaData_PrizeCategoryExhaused(prize);
            }
    ...
    function setClaimed(uint256 tokenId) external onlyOwner {
        TokenMetaData memory metaData = s_tokenMetaData[tokenId];
