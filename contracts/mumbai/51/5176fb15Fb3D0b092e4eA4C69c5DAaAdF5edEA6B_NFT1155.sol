...
contract NFT1155 is ERC1155, ERC1155URIStorage, ERC1155Pausable, ERC1155Burnable, ERC1155Supply, ERC2981, Ownable{
    
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIdCounter;
    
    uint[] private _memoryTokenId;
    constructor(string memory _uri) ERC1155(_uri){}
    
    function mint(address to, uint256 amount, bytes memory data, string memory uri_)public onlyOwner returns(uint id){
        _tokenIdCounter.increment();
        id = _tokenIdCounter.current();
        _mint(to, id, amount, data);
        _setURI(id, uri_);
    }
    function mintRoyalty(address to, address royaltyReceiver, uint amount, uint96 feeNumerator, bytes memory data, string memory uri_)public onlyOwner returns(uint id){
        _tokenIdCounter.increment();
        id = _tokenIdCounter.current();
        _mint(to, id, amount, data);
    ...
    function mintBatch(address to, uint256[] memory amounts, bytes memory data)public onlyOwner {
        ...
        for(uint i; i<amounts.length; i++){ 
            _tokenIdCounter.increment();
            id = _tokenIdCounter.current();
            _memoryTokenId.push(id);
        }
        _mintBatch(to, _memoryTokenId, amounts, data);
        delete _memoryTokenId;
    }
    ...
    (address to, address[] memory royaltyReceiver, uint[] memory ids, uint[] memory amounts, uint96[] memory feeNumerator, bytes memory data)
    public onlyOwner{
        _mintBatch(to, ids, amounts, data);
        for(uint i; i<ids.length; i++){
            _setTokenRoyalty(ids[i], royaltyReceiver[i], feeNumerator[i]);
        }
    }
...
//Overrides functions
    function burn(address account, uint256 id, uint256 value)public override{
        ...
        _resetTokenRoyaltyForBurn(id);
    }
    function burnBatch(address account, uint256[] memory ids, uint256[] memory values)public override{
        super.burnBatch(account, ids, values);
        for(uint i; i<ids.length; i++){
            _resetTokenRoyaltyForBurn(ids[i]);
        }
    }
    function _resetTokenRoyaltyForBurn(uint id)internal{
        if(totalSupply(id) == 0){
