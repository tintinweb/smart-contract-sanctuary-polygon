...
interface IVeDefed {
    ...
    function increaseUnlockTime(uint256 _unlockTime,address _depositFor) external;
...
contract VeDefed is IVeDefed, ReentrancyGuard, Ownable {
    ...
    function setBridgeControl(address _bridgeControl) external onlyOwner {
     ...
     */
    function increaseUnlockTime(uint256 _unlockTime,address _beneficiary)
        external
        override
    ...
    function withdraw(address userProxy) external override nonReentrant onlyOperator{
        ...
        IERC20(token).safeTransfer(bridgeControl, _value);
