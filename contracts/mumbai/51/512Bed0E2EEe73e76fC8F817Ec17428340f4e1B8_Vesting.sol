...
contract Vesting is AccessControl, IVesting {
    ...
    uint256 private _startVesting;
    address private _MGBAddress;
    uint256 private _distributedAmount;
    uint256 private _distributedImmediately;
    ...
    constructor(
        ...
        _startVesting = startVesting;
        _MGBAddress = tokenAddr;

        _distributedImmediately = _calcPercent(
            _totalAmount,
     ...
     */
    function claim(uint256 amountMGB) external isWhiteList(msg.sender) {
        require(_calcAvaiableAmount() >= amountMGB, "No amount available");

        _distributedAmount += amountMGB;

        IERC20(_MGBAddress).safeTransfer(msg.sender, amountMGB);

        emit Claim(msg.sender, amountMGB);
    }

    /**
     * @dev See {IVesting-setWhiteList}.
     */
    ...
    /**
     * @dev See {IVesting-withdrawToken}.
     */
    ...
    /**
     * @dev See {IVesting-checkUserStatus}.
     */
    ...
    /**
     * @dev See {IVesting-getAvaiableAmount}.
     */
    function getAvaiableAmount() external view returns (uint256) {
        return (_calcAvaiableAmount());
    }

    /**
     * @dev See {IVesting-getInfoVesting}.
     */
    function getInfoVesting() external view returns (InfoVesting memory) {
        return
            InfoVesting(
                ...
                _MGBAddress,
                _distributedAmount,
                _distributedImmediately
            );
    }

    function _calcAvaiableAmount()
        internal
        view
        returns (uint256 avaiableAmount)
    {
        ...
        uint256 months = (block.timestamp - _startVesting) / 30 days;
        if (months > _vestingDuration) {
            months = _vestingDuration;
        }
        avaiableAmount =
            _distributedImmediately +
            ((_totalAmount - _distributedImmediately) / _vestingDuration) *
            months -
            _distributedAmount;
    }
...
interface IVesting {
     ...
     */
    event Claim(address indexed account, uint256 amountMGB);

    /**
     *@dev Users who are on the whitelist can pick up the available reward
     */
    function claim(uint256 amountMGB) external;
