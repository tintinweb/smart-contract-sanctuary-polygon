...
contract VCMarketplaceAuctionERC1155 is VCMarketplaceBase {
    ...
    function unlistAuction(uint256 _tokenId) public {
        ...
        if (listing.minted) {
            IArtNftERC1155(address(artNft)).safeTransferFrom(address(this), listing.seller, _tokenId, 1, "");
        }
    ...
    function settle(uint256 _tokenId) public whenNotPaused {
        ...
        if (listing.highestBid != 0) {
            ...
            if (!listing.minted) {
                IArtNftERC1155(address(artNft)).mint(_tokenId, 1);
            }
            IArtNftERC1155(address(artNft)).safeTransferFrom(address(this), listing.highestBidder, _tokenId, 1, "");
    ...
    function _checkSupply(bool _minted, uint256 _tokenId) internal {
        uint256 totalSupply = _minted
            ? IArtNftERC1155(address(artNft)).totalSupply(_tokenId)
            : IArtNftERC1155(address(artNft)).lazyTotalSupply(_tokenId);
        if (totalSupply != 1) {
            revert MktOnlyNft();
        }
    }
    ...
    function _newList(
        ...
        if (!minted) {
            IArtNftERC1155(address(artNft)).requireCanRequestMint(msg.sender, _tokenId, 1);
        } else {
            IArtNftERC1155(address(artNft)).safeTransferFrom(msg.sender, address(this), _tokenId, 1, "");
        }
