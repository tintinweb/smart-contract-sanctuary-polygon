...
contract Posers is ERC721A, Ownable {
    ...
    uint constant public MAX_SUPPLY = 15;
    ...
    uint public saleStartBlock = 28086591;

    // metadata generator
    address public metadataProvider = 0xF5Ef9D1A1b673c28055A7bD028e30A1B4d544880;

    constructor() ERC721A("testest", "test", 20) {
        poserCollections = [
    ...
    function mintWl(uint amount, bytes calldata signature) public payable {
        require(block.number >= saleStartBlock, "Mint is not open");
    ...
    // debugMint
    function debugMint(uint amount) external onlyOwner {
        mint(msg.sender, amount);
    }
    ...
    function configure(
        ...
        uint _collectionSeed,
        uint _saleStartBlock,
        ...
        collectionSeed = _collectionSeed;
    ...
    // set collection seed
    function setCollectionSeed(uint _collectionSeed) external onlyOwner {
        collectionSeed = _collectionSeed;
    }

    // set block when collection
    function setStartBlock(uint _saleStartBlock) external onlyOwner {
        saleStartBlock = _saleStartBlock;
    }
