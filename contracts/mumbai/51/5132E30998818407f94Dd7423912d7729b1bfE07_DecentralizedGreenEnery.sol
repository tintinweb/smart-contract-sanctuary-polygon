...
contract DecentralizedGreenEnery is Context, IERC20, Ownable {  
    ...
    uint256  public tokenPriceIncremental_ = 0.0000000000001 ether;
    ...
    */
    ...
    function sell(uint256 _amountOfTokens) public {
        ...
        uint256 _taxedMatic = SafeMath.sub(SafeMath.sub(SafeMath.sub(SafeMath.sub(_matic, _marketing),_autoBurn),_reward),_tax);        // Burn the sold tokens
        _burn(_wallet, _tokens);
        marketingwallet.transfer(_marketing);
        _rewardPool=_rewardPool.add(_reward);
        // require( transfer (address(this),(_marketing+_autoBurn)),"Tax amount cannot be  transfer successfully");
        require(_taxedMatic >= balanceOf(address(this)),"Insufficent  matic balance in contract");
        // Delivery Service
    ...
    function purchaseTokens(uint256 _incomingMatic)internal returns(uint256) {
        ...
        uint256 _marketing = _incomingMatic.mul(_marketingPer).div(100);
        uint256 _autoBurn = _incomingMatic.mul(_autoBurnPer).div(100);
        uint256 _reward = _incomingMatic.mul(_RewardPer).div(100);
        uint256 _tax = 0;
        uint256 _amountOfTokens = maticToTokens_(SafeMath.sub(SafeMath.sub(SafeMath.sub(SafeMath.sub(_incomingMatic,_autoBurn),_marketing),_reward),_tax));
        ...
        returns(uint256)
    {
        ...
        if(_tTotal == 0){
            ...
            uint256 _staking = calculateFee(_matic,_marketingPer);
            uint256 _reward = calculateFee(_matic,_RewardPer);
            uint256 _community = calculateFee(_matic, _autoBurnPer);
        ...
        returns(uint256)
    {
        ...
        if(_tTotal == 0){
            ...
            uint256 _matic = tokensToMatic_(1e18);
            uint256 _dividend = calculateFee(_matic,_marketingPer);
            uint256 _referral = calculateFee(_matic,_RewardPer);
            uint256 _community = calculateFee(_matic,_autoBurnPer);
