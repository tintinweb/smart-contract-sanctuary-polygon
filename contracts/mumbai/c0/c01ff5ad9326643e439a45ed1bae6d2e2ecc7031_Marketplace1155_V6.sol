...
contract Marketplace1155_V6 is 
  Initializable, 
  OwnableUpgradeable, 
  ...
  UUPSUpgradeable 
{
    ...
    whenNotPaused
  {
    ...
    emit MarketItemCreated(itemID, nftContract, tokenID, amount, msg.sender, address(0), price, royaltyReceiver, royaltyInBips, ListingStatus.Active);
  }
    ...
    whenNotPaused
  {
    ...
    emit MarketItemCreated(itemID, nftContract, tokenID, 0, minter, address(0), price,address(0), 0, ListingStatus.Active);
    ...
    emit NewResellOffer(itemID, 1, msg.sender, amount, price, ListingStatus.Active);
  }
    ...
    whenNotPaused
  {
    ...
    emit MarketItemModified(itemID, cancelledAmount, marketItem.amountToSell, previousPrice, marketItem.price, msg.sender, royaltyReceiver, royaltyInBips);
  }
    ...
    whenNotPaused
  {
    ...
    emit MarketItemCancelled(marketItem.itemID, marketItem.seller, cancelledAmount, ListingStatus.Cancelled);
  }
    ...
    whenNotPaused
  {
    ...
    uint256 royaltyFee = msg.value * marketItem.royaltyInBips / 10000;
    address royaltyReceiver = marketItem.royaltyReceiver;
    if (marketItem.royaltyInBips == 0) {
      (royaltyReceiver, royaltyFee) = getRoyaltyInfo(itemID, msg.value);
    }

    uint256 marketFee = msg.value * MARKET_FEE_IN_BIPS / 10000;
    uint256 mainFee = msg.value - royaltyFee - marketFee;

    emit MarketItemSale(itemID, msg.sender, marketItem.seller, amount, price, mainFee, marketFee, royaltyFee, royaltyReceiver);
    ...
    if (marketItem.amountToSell == 0) {
      ...
      emit MarketItemSold(itemID, marketItem.seller, ListingStatus.Sold);
    }
  }
    ...
    whenNotPaused
  {
    ...
    emit NewResellOffer(itemID, resellID, msg.sender, amount, price, ListingStatus.Active);
  }
    ...
    whenNotPaused
  {
    ...
    emit ResellOfferModified(itemID, resellID,  amount, offer.price,  msg.sender, offer.status);
  }
    ...
    whenNotPaused
  {
    ...
    emit ResellOfferModified(itemID, resellID, offer.amountToSell, offer.price, offer.seller, offer.status);
  }
    ...
    whenNotPaused
  {
    ...
    emit ResellItemSale(itemID, resellID, msg.sender, offer.seller, amount, price, mainFee, marketFee, royaltyFee, royaltyReceiver);
    ...
    if (offer.amountToSell == 0) {
      ...
      emit ResellItemSold(itemID, resellID, offer.seller, offer.price, offer.status);
    }
  }
    ...
    onlyRole(CENSOR_ROLE)
  {
    ...
    emit MarketItemBlacklisted(itemID, marketItem.nftContract, marketItem.tokenID, marketItem.seller, marketItem.amountToSell, ListingStatus.Blacklisted);
  }
