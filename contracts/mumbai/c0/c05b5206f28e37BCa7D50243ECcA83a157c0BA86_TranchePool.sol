...
contract TranchePool is ITranchePool, TrancheStorage, OwnableUpgradeable, ERC721Upgradeable, ERC1155HolderUpgradeable {
  ...
  function claim(bool isSeniorTranche) public {
    ...
    _claim(amount, isSeniorTranche, epochCounter - 1); // gives airthmetic overflow error if first epoch ( acceptable since no profits made in first epoch so code has to break anyway )
    tranche.profitsEpoch = epochCounter;
  }
  ...
  function processPendingClaims(bool isSeniorTranche) external override(ITranchePool) {
    ...
    _claim(request.amount, isSeniorTranche, request.epoch);
  ...
  function _claim(
    ...
    if (isSeniorTranche) {
      if (globalTrancheDetails.totalSeniorProfitsAvailable >= amount) {
        ...
        IERC20Upgradeable(pool.asset).transfer(msg.sender, globalTrancheDetails.totalSeniorProfitsAvailable + globalTrancheDetails.reservor);

        pendingClaimRequestSenior[msg.sender] = ClaimRequests(
      ...
      if (globalTrancheDetails.totalJuniorProfitsAvailable >= amount) {
        ...
        IERC20Upgradeable(pool.asset).transfer(msg.sender, globalTrancheDetails.totalJuniorProfitsAvailable + globalTrancheDetails.reservor);
        pendingClaimRequestJunior[msg.sender] = ClaimRequests(
    ...
    if (isSeniorPool) {
      ...
      while (true) {
        ...
        if (srTranche.amountToRedeem > queuedAccount.pendingSwap) {
          ...
          _claim(queuedAccount.amount, true, epochCounter - 1);
      ...
      while (true) {
        ...
        if (jrTranche.amountToRedeem > queuedAccount.pendingSwap) {
          IERC20Upgradeable(pool.asset).transfer(account, queuedAccount.pendingSwap);
          _claim(queuedAccount.amount, false, epochCounter - 1);
          jrTranche.amount += queuedAccount.pendingSwap;
  ...
  function getAPY(
    ...
    uint256 endEpoch,
    bool senior
  ) public view override(ITranchePool) returns (uint256) {
