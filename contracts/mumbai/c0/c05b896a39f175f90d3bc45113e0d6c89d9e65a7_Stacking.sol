...
contract Stacking is Ownable {
    ...
    IBEP20 public stakedToken=IBEP20(0x87a661AF73a73e31bf8A4F2C56e2DB1A899a8488);
    ...
    function deposite(uint256 amount,uint8 _plan) public {
        ...
        require(plans[_plan].isActive,"plan is not activated");
        require(
    ...
    */
    ...
    // ADD PLANS
    function addPlans(uint256 _days,uint256 _apy,bool _active) public onlyOwner {
        require(_days>0,"Days must be greater then zero");
        require(_apy>0,"APY must be greater then zero");
        plans.push(Plan(730, 20,_active));
    }

    // UPDATE PLAN STATUS
    function updatePlanStatus(uint256 _plan,bool _active) public onlyOwner{
        plans[_plan].isActive = _active;
    }
   
    constructor() {
        admins.push(msg.sender);
        plans.push(Plan(730, 20,true));
        plans.push(Plan(1095, 23,true));
        plans.push(Plan(1703, 30,true));  
    }
}