...
contract Swap is Ownable, ERC1155Holder, ERC721Holder, BasicMetaTransaction {
    ...
    */
    ...
    function swap(
        ...
        require(
            GTP.balanceOf(_msgSender(), tokenId) >= quantity,
            "swap: insufficient gtp in user account"
        );
    ...
    function withdrawVCCTokensFromSwap(
        uint256 _gtpProjectTokenId,
        uint256 _vccTokenIdIndex,
        address _receiverAddress
    ) external onlyOwner {
        require(
            _gtpProjectTokenId != 0,
            "withdrawVCCTokensFromSwap: gtp token Id cannot be zero"
        );
        uint256 vccTokenId = VCC.updateVCCGTPStorageAfterWithdrawl(
            _gtpProjectTokenId,
            _vccTokenIdIndex
        );
        VCC.safeTransferFrom(address(this), _receiverAddress, vccTokenId);
    }
