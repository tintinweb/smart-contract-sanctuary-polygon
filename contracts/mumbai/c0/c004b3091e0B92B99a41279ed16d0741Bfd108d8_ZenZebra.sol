...
contract ZenZebra is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    function deposit() external payable onlySender nonReentrant {
        ...
        for (uint256 i = 0; i < 3; i++) {
            for (uint256 j = 1; j < 5; j++) {
                for (uint256 k = UNITS_BATCH[i][j-1]; k < _tokenIdTracker[i][j]; k++) {
                    rewards[k+1] += _rewards * LEVEL_POINT[j] * LEVEL_PERCENT[j] / (_totalPoint * 4);
                }
            }
        }
    }
    ...
    // ===== Distribute to the team and owner =====
