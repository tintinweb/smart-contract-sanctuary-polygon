    ...
    */
    ...
    function payBillInstallmentNFT(bytes32 _billId , uint256 _period) external whenInstallmentNotPaused {
        ...
        if(bill.periodBalance == 0){
            IERC721Upgradeable(bill.nftAddress).transferFrom(
                address(this),
                bill.buyer,
                bill.tokenId
            );
    ...
    function cancelOrderInstallmentNFT(bytes32 _orderId) external  whenInstallmentNotPaused{
        ...
        IERC721Upgradeable(order.nftAddress).transferFrom(
            address(this),
            order.seller,
            order.tokenId
        );
    ...
    function calculateInstallmentNFT (address _sellerAccount, bytes32 _orderId , uint256 _period) external view returns(uint totalbill ,uint prepay,uint totalBillPayByperiod) {
        Order memory order = orderByOrderId[_sellerAccount][_orderId];
        uint calculatedInterestRate;
        uint totalInterest;
        calculatedInterestRate = (_period / 3) + 3 + (4 * (_period - 6)/3);
        totalInterest = (order.price * calculatedInterestRate) / 100;
        totalbill = order.price + totalInterest;
        totalBillPayByperiod  = totalbill / _period;
        prepay = (totalBillPayByperiod / 30) * 40;
