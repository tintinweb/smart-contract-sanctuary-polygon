...
IERC20 eft;
 /**
  ...
  eft = IERC20(0x3Aa5ebB10DC797CAC828524e59A333d0A371443c);
  ...
  function upgradeCards(uint160[] calldata _tokenIds, uint8 _targetCardType, uint8 _collectionId,  Coupon memory _coupon, uint _discount, uint256 _value) public  validCardType(_targetCardType) {
    ...
    
    {
      if(upgradeFeeCost > 0){
        ...
        gametreasury.upgradeCommission(daoAddress, upgradeFeeCost);
      }
      if(partnerFeeCost > 0){
      ...
      //claim commitions
      claimUpgradeCommission(cardsTypeValueDiffCost, refundValue);
    }
