...
// Substituted tokenholders array with a mapping
// addTokenHolder and removeTokenHolder functions adjusted to work with a mapping
// searchTokenHolder function no longer needed - hence deleted

contract EditorialV3 is ERC1155, Ownable, Pausable, ReentrancyGuard, AccessControl {
    ...
    /**
     * @dev mapping to store all current NFT holders
     */
    mapping(address => uint256) public tokenHolders;

    /**
     * @dev number of current NFT holders
     */
    ...
    function _addTokenHolder( address _holderAddress, uint256 _id ) internal {
        tokenHolders[_holderAddress] = _id;
        numTokenHolders++;
    }

    /**
     * @dev Remove the address from the Token Holders mapping
     *      by switching to false
     */
    function _removeTokenHolder( address _holderAddress ) internal {        
        tokenHolders[_holderAddress] = 0;
        numTokenHolders--;
    ...
    function addBook(
        ...
        // params: acct, id, amount, bytes internal data
        ...
        _addTokenHolder( msg.sender, newBookId );

        return newBookId;
    }

    /**
     * @dev Transfer NFT and update address in tokenHolders array
    */
    function transferNFT( address _from, address _to, uint256 _id, uint256 _amount ) external  {
        // require(hasRole(ADMIN_ROLE, msg.sender), "Caller is not an admin");
        _removeTokenHolder( _from );
        _addTokenHolder( _to, _id );
