...
import "../libraries/ArpeggiStaticData.sol";

import "../interfaces/IAudioRelationshipProtocol.sol";

/// @notice ERC-721 token contract for sounds (samples, loops, and stems) created in Arpeggi Studio (arpeggi.io/studio). Minted sounds are automatically registered to the Audio Relationship Protocol.
/// @author Alec Papierniak <[email protected]>, Kyle Dhillon <[email protected]>
contract ArpeggiSound is
    Initializable,
    ERC721Upgradeable,
    ...
    ReentrancyGuardUpgradeable
{
    ...
    uint256 public _numSounds;
    mapping(uint256 => bytes) public _compositions;
    bool public _requireAuthorizedWriter;

    function initialize(address adminAddress) public initializer {
        __ERC721_init("ArpeggiSound", "ARPSOUND");
        ...
        _setupRole(Roles.ARPEGGI_ADMIN_ROLE, adminAddress);

        _setRoleAdmin(Roles.ARPEGGI_ADMIN_ROLE, Roles.ARPEGGI_ADMIN_ROLE);
        _setRoleAdmin(Roles.PAUSER_ROLE, Roles.ARPEGGI_ADMIN_ROLE);
        _setRoleAdmin(Roles.ARPEGGI_CONTRACT_ROLE, Roles.ARPEGGI_ADMIN_ROLE);
        _setRoleAdmin(Roles.AUTHORIZED_WRITER_ROLE, Roles.ARPEGGI_ADMIN_ROLE);

        _requireAuthorizedWriter = true;
    }

    /// @dev Address of other Arpeggi contracts. Keys are from `../libraries/ArpeggiStaticdata.sol`
    ...
    function mintStem(
        ...
        bytes calldata composition
    ) external whenNotPaused nonReentrant returns (uint256) {
        if (_requireAuthorizedWriter) {
            require(hasRole(Roles.AUTHORIZED_WRITER_ROLE, _msgSender()), "ArpeggiSong: Unauthorized write.");
        }

        uint256 numSounds = _numSounds + 1;

        _safeMint(tokenOwner, numSounds);
        _compositions[numSounds] = composition;

        IAudioRelationshipProtocol(_contractAddresses[ContractNames.AUDIO_REGISTRY_PROTOCOL]).registerMedia({
            artistAddress: artistAddress,
            dataUri: dataUri,
            metadataUri: metadataUri,
            subcomponents: subcomponents,
            originContractAddress: address(this),
            originTokenId: numSounds,
            originChainId: block.chainid,
            originType: 0 // PRIMARY
        });
    ...
    function setContractAddress(bytes32 contractName, address contractAddress) external onlyRole(Roles.ARPEGGI_ADMIN_ROLE) {
        _contractAddresses[contractName] = contractAddress;
    }

    function setBaseUri(string calldata baseUri) external onlyRole(Roles.ARPEGGI_ADMIN_ROLE) {
        _baseUri = baseUri;
    }
    ...
    function setRequireAuthorizedWriter(bool requireAuthorizedWriter) external onlyRole(Roles.ARPEGGI_ADMIN_ROLE) {
        _requireAuthorizedWriter = requireAuthorizedWriter;
    }

    function _authorizeUpgrade(address newImplementation) internal override onlyRole(Roles.ARPEGGI_ADMIN_ROLE) {}
...
library ContractNames {
    bytes32 public constant ARPEGGI_SONG = keccak256("ARPEGGI_SONG");
    bytes32 public constant ARPEGGI_SOUND = keccak256("ARPEGGI_SOUND");
    bytes32 public constant AUDIO_REGISTRY_PROTOCOL = keccak256("AUDIO_REGISTRY_PROTOCOL");
}

library Roles {
    bytes32 public constant ARPEGGI_ADMIN_ROLE = keccak256("ARPEGGI_ADMIN_ROLE");
    bytes32 public constant ARPEGGI_CONTRACT_ROLE = keccak256("ARPEGGI_CONTRACT_ROLE");
