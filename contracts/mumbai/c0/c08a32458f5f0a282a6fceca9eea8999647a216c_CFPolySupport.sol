...
// File contracts/CFPolySupport.sol
...
ReentrancyGuard
{
    ...
    {
        return string.concat(uri(0), "CFPSB.json");
    }

    function tokenURI(uint256 tokenId) external view returns (string memory)
    {
        string memory _URI = (tokenId >= (1 << 128)) ? string.concat(uri(0), "pb_data/", Strings.toString(tokenId), ".json") :
        (tokenId < (10000 << 64))? string.concat(uri(0), "b_data/", Strings.toString(tokenId), ".json") : string.concat(uri(0), "b_data/badge.json");
        return _URI;
    }
