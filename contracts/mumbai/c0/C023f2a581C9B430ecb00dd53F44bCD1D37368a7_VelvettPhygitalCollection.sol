...
import {ERC721PartnerSeaDropUpgradeable} from "seadrop/src-upgradeable/src/ERC721PartnerSeaDropUpgradeable.sol";

library VelvettMembershipStorage {
    struct Layout {
    ...
    bytes32 internal constant STORAGE_SLOT =
        keccak256("seaDrop.contracts.storage.velvettMembership");
 ...
 */
contract VelvettMembership is ERC721PartnerSeaDropUpgradeable {
    using VelvettMembershipStorage for VelvettMembershipStorage.Layout;
    ...
    /**
     * @dev Modifier that ensures a user can only transact one token at a time and does not already own a membership token of this type.
     * @param to The address of the recipient.
     * @param quantity The number of tokens to be transferred.
     */
    modifier onlyOneToken(address to, uint256 quantity) {
        require(quantity == 1, "You can only transact one token at a time");
        require(
            balanceOf(to) == 0,
            "You already own a membership token of this type"
        );
        _;
    }

    /**
     * @notice Initializes the token contract with its name, symbol, administrator, allowed SeaDrop addresses, and the VelvettMembership contract address.
     * @dev This function is marked as `initializer` to ensure it is only called once.
     * @param name The name of the token.
     * @param symbol The symbol of the token.
     * @param administrator The address of the token administrator.
     * @param allowedSeaDrop An array of addresses allowed to perform SeaDrop actions.
     */
    function initialize(
        string memory name,
    ...
    /**
     * @dev Internal function called before the token transfer occurs.
     * @notice This function overrides the base implementation to include the `onlyOneToken` modifier.
     * @param from The address of the sender.
     * @param to The address of the recipient.
     * @param startTokenId The starting token ID in the range of tokens to be transferred.
     * @param quantity The number of tokens to be transferred.
     */
    function _beforeTokenTransfers(
        ...
        uint256 quantity
    ) internal override onlyOneToken(to, quantity) {}
    ...
    function setBurnAddress(address newBurnAddress) external onlyOwner {
        VelvettMembershipStorage.layout().burnAddress = newBurnAddress;
    }
    ...
    function getBurnAddress() public view returns (address) {
        return VelvettMembershipStorage.layout().burnAddress;
    }

    /**
     * @notice Destroys the token with the given `tokenId`. Only callable by the set burn address.
     * @dev This function can only be called by the address set as the burn address, otherwise it emits the `BurnIncorrectSender` error.
     * @param tokenId The token ID of the token to be destroyed (burned).
     */
    function burn(uint256 tokenId) external {
        if (msg.sender != VelvettMembershipStorage.layout().burnAddress) {
            revert BurnIncorrectSender(tokenId, msg.sender);
        }
...
import {ERC721PartnerSeaDropUpgradeable} from "seadrop/src-upgradeable/src/ERC721PartnerSeaDropUpgradeable.sol";
import {VelvettMembership} from "./VelvettMembership.sol";

library VelvettPhygitalCollectionStorage {
    struct Layout {
        /// @notice The only address that can burn tokens on this contract.
        address burnAddress;
        /// @notice The addresses for the VelvettMembership contracts.
        address goldVelvettMembershipContract;
        address silverVelvettMembershipContract;
    }

    bytes32 internal constant STORAGE_SLOT =
        keccak256("seaDrop.contracts.storage.velvettPhygitalCollection");
 ...
 *         The set burn address is the only sender that can burn tokens.
 */
contract VelvettPhygitalCollection is ERC721PartnerSeaDropUpgradeable {
    using VelvettPhygitalCollectionStorage for VelvettPhygitalCollectionStorage.Layout;
    ...
    /**
     * @notice Modifier to check whether the `from` and `to` addresses own a membership token.
     * @dev This modifier requires that both `from` and `to` addresses own a membership token, or be the zero address.
     * @param from The address tokens are being transferred from.
     * @param to The address tokens are being transferred to.
     */
    modifier isMember(address from, address to) {
        require(
            from == address(0) || ownsMembershipNFT(from),
            "You must own a membership token"
        );
        require(
            to == address(0) || ownsMembershipNFT(to),
            "Reciever must own a membership token"
        );
        _;
    }

    /**
     * @notice Initializes the token contract with its name, symbol, administrator, allowed SeaDrop addresses, and the VelvettMembership contract addresses for gold and silver memberships.
     * @dev This function is marked as `initializer` to ensure it is only called once.
     * @param name The name of the token.
     * @param symbol The symbol of the token.
     * @param administrator The address of the token administrator.
     * @param allowedSeaDrop An array of addresses allowed to perform SeaDrop actions.
     * @param _goldVelvettMembershipContract The address of the Gold Velvett Membership contract.
     * @param _silverVelvettMembershipContract The address of the Silver Velvett Membership contract.
     */
    function initialize(
        string memory name,
        string memory symbol,
        address administrator,
        address[] memory allowedSeaDrop,
        address _goldVelvettMembershipContract,
        address _silverVelvettMembershipContract
    ) external initializer initializerERC721A {
        ERC721PartnerSeaDropUpgradeable.__ERC721PartnerSeaDrop_init(
            name,
            symbol,
        ...
        VelvettPhygitalCollectionStorage
            .layout()
            .goldVelvettMembershipContract = _goldVelvettMembershipContract;
        VelvettPhygitalCollectionStorage
            .layout()
            .silverVelvettMembershipContract = _silverVelvettMembershipContract;
    }

    /**
     * @notice Checks whether the specified `account` owns a GOLD or SILVER membership token.
     * @dev This function performs an external call to the VelvettMembership contract and decodes the returned data.
     ...
     */
    function ownsMembershipNFT(address account) public view returns (bool) {
        bool doesOwnMembership = (VelvettMembership(
            VelvettPhygitalCollectionStorage
                .layout()
                .goldVelvettMembershipContract
        ).balanceOf(account) == 1) ||
            (VelvettMembership(
                VelvettPhygitalCollectionStorage
                    .layout()
                    .silverVelvettMembershipContract
            ).balanceOf(account) == 1);

        return doesOwnMembership;
    }

    /**
     * @notice Hook that is called before a token transfer takes place.
     * @dev This function enforces the `isMember` modifier for both `from` and `to` addresses.
     * @param from The address tokens are being transferred from.
     * @param to The address tokens are being transferred to.
    ...
    function _beforeTokenTransfers(
        ...
        uint256 startTokenId,
        uint256 quantity
    ) internal override isMember(from, to) {}

    /**
     * @notice THIS IS A TESTING FUNCTION. REMOVE BEFORE DEPLOYMENT.
     * @notice Mints `quantity` tokens to `minter`. Only used for testing, remove before deployment.
     * @dev This function is marked as `public` and can be called by any address. It enforces the `isMember` modifier for the `minter` address.
     * @param minter The address of the account to receive the minted tokens.
     * @param quantity The number of tokens to be minted.
     */
    function mint(address minter, uint256 quantity) public {
     ...
     */
    function setBurnAddress(address newBurnAddress) external onlyOwner {
        VelvettPhygitalCollectionStorage.layout().burnAddress = newBurnAddress;
    }
     ...
     */
    function getBurnAddress() public view returns (address) {
        return VelvettPhygitalCollectionStorage.layout().burnAddress;
    }
    ...
    function burn(uint256 tokenId) external {
        if (
            msg.sender != VelvettPhygitalCollectionStorage.layout().burnAddress
        ) {
            revert BurnIncorrectSender(tokenId, msg.sender);
        }
