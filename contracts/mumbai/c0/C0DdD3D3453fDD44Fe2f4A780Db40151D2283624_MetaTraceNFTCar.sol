...
contract MetaTraceNFTCar is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, PausableUpgradeable, OwnableUpgradeable, ERC721BurnableUpgradeable, UUPSUpgradeable {
    using CountersUpgradeable for CountersUpgradeable.Counter;
    string public secret;
    uint256 public secretNum;
    ...
    function initialize() initializer public {
        __ERC721_init("MetaTrace NFT Car", "CAR");
    ...
    function changeBaseURI(string memory _base) external virtual onlyOwner {
        base = _base;
    ...
    function safeMintUser(bytes memory _signature, uint256 _secretNum) external virtual {
        secretNum = _signerCheck(_signature, _secretNum);
        _safeMintInternal(msg.sender);
    }

    function _safeMintInternal(address to) internal virtual onlyOwner {
    ...
    function safeMintWithTokenId(address to, uint256 tokenId) external virtual onlyOwner {
        _safeMint(to, tokenId);
    }

    function safeMintWithTokenIdUser(bytes memory _signature, 
        uint256 tokenId, uint256 _secretNum) external virtual {
        secretNum = _signerCheck(_signature, _secretNum);
    ...
    function safeMintBatch(address[] memory recipients, uint256[] memory amounts) external virtual onlyOwner {
    ...
    function increment() internal onlyOwner {
        _tokenIdCounter.increment();
    }
