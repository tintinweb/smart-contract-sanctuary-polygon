...
interface IRehideNFT {
    ...
    event SetTier(uint256 indexed tierId, uint256 rebate, uint256 discount);
}
...
contract RehideNFT is IRehideNFT, RehideBase {
    ...
    mapping(uint256 => Tier) public _tierList; 
     ...
     */
    function setTier(uint256 tierId, uint256 rebate, uint256 discount) external onlyOwner {
        require(rebate <= _maxReferrerRewardsPercentage, "RehideNFT: Referrer rebate exceed max allowed.");
        require(discount <= _maxReferrerRewardsPercentage, "RehideNFT: Referrer discount exceed max allowed.");

        if (_tierList[tierId].rebate == 0) {
        ...
        _tierList[tierId] = tier;

        emit SetTier(tierId, rebate, discount);
    }
    ...
    function addToReferrerTierList(address[] calldata toAddAddresses, uint256 tierId) external onlyOwner {
        require(_tierList[tierId].rebate > 0, "RehideNFT: Invalid tier");
    ...
    function mintNFT(
        ...
        if (_msgSender() != owner() && _referrerTierList[_msgSender()] < 3) {
    ...
    function readSharedNote(uint256 tokenId) public payable returns (uint256 readsAvailable) {
        ...
        if (_msgSender() != owner() && _referrerTierList[_msgSender()] < 3 && ownerOf(tokenId) != _msgSender()) {
    ...
    function addToReadAllowlist(uint256 tokenId, address[] calldata toAddAddresses) public {
        ...
        for (uint256 i = 0; i < toAddAddresses.length; i++) {
            _noteReadAllowlistAddresses[tokenId].push(toAddAddresses[i]);
        }
        ...
        if (mintFee > 0) {
            ...
            if (referrer != _msgSender()) {
                ...
                if (primaryReferrer != address(0)) {
                    ...
                    if (referrerTierId > 0) {
                        Tier memory tier = _tierList[referrerTierId];
                        uint256 whitelistReferrerReward = mintFee.mul(tier.rebate).div(100);
