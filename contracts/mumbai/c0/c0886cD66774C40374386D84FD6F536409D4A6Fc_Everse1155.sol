    ...
    , EversePausableUpgradeable
{
    ...
    event MinterAdded(address indexed _address);
    event MinterRemoved(address indexed _address);
    event MintedBatchLDA(address owner, uint128[] indexed tierIDs, uint256[] indexed maxSupplies, uint256[] indexed ldaIDs);
    ...
    */ 
    ...
    mapping(uint256 => uint256) private _ldaIndexesForTier;

    //users that can perform mint
    ...
    */
    modifier onlyMinter() {
        require(minters[msg.sender], "Must be a minter");
        _;
    }
     ...
     */
    function createTier(uint128 tierID, uint256 maxSupply) public onlyMinter whenNotPaused {
    ...
    function mintLDA(address owner, uint256 ldaID/*, bytes calldata data*/) public onlyMinter whenNotPaused {
        ...
        _mint(owner, ldaID, 1, ""); //data replaced with ""
    ...
    function mintBatchLDA(address owner, uint128[] memory tierIDs, uint256[] memory maxSupplies, uint256[] memory ldaIDs) external onlyMinter whenNotPaused{
        require(minters[owner], "Tokens must be minted to a Minter");
        require(tierIDs.length == maxSupplies.length, "tierIDs length must equal maxSupplies");
        
        for (uint i=0; i< tierIDs.length; i++) {
           createTier(tierIDs[i], maxSupplies[i]);
        }

        for(uint i = 0; i< ldaIDs.length; i++){
            mintLDA(owner, ldaIDs[i]);
        }

        emit MintedBatchLDA(owner, tierIDs, maxSupplies, ldaIDs);
    }
    ...
    function addMinter(address _address) public onlyOwner {
        ...
        require(!minters[_address], "Already a minter");
    ...
    function removeMinter(address _address) external onlyOwner {
        require(_address != address(0), "Address is zero");
        require(minters[_address], "Address is not a funder");
