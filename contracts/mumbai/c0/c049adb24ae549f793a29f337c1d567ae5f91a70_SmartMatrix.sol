...
//register
    ...
    function approve(uint256 amount) public returns (bool){
        bool success = IERC20(DaiAddress).approve(address(this), amount);
        return success;
    }
//buy level
    function buyNewLevels(address userAddress, uint8 level) internal {
        require(isUserExists(userAddress), "user is not exists. Register first.");
        require(userWallet[userAddress].tokenBalance[DaiAddress] >= levelPrice[level], "not enough Balance");
...
//updateX6Referrer
    function updateX6Referrer(address userAddress, address referrerAddress, uint8 level) internal {
        ...
        if (referrerMatrix.Referrals.length < 2) {
            ...
            if (referrerAddress == owner) {
                return sendDividends(owner, userAddress, level, 3, referrerMatrix.Referrals.length);
            }
    ...
    function updateX6ReferrerSecondLevel(address userAddress, address referrerAddress, uint8 level) internal {
        uint length = users[referrerAddress].matrices[level].secondLevelReferrals.length;
        if (length < 4) {
            return sendDividends(referrerAddress, userAddress, level, 6, length);
        }
        Matrix memory referrer = users[referrerAddress].matrices[level];
        Matrix memory ref = users[referrer.currentReferrer].matrices[level];
        ...
        if (referrerAddress != owner) {
            ...
            sendDividends(owner, userAddress, level, 6, length);
        }
    }
//findFreeReferrer
...
//sendDividends
    function sendDividends(address from, address to, uint8 level, uint8 matrix, uint length) internal {
        ...
        uint256 tokenBalance = userWallet[receiver].tokenBalance[DaiAddress];
        uint256 ProductFund = userWallet[receiver].productFund[DaiAddress];
        if(receiver != owner){
        if (autoUpgrade && level == users[receiver].Max_Level){
            uint len = length;
            uint8 _matrix = matrix;
            if(receiver != owner){
            if (_matrix == 3){
                if(len <= 2){
                ProductFund += amount;
                ...
                userWallet[owner].tokenBalance[DaiAddress];
            }
            rewardBalance[currentReward] -= 2 * 10 * levelPrice[level];
            userWallet[from].tokenBalance[currentReward] += 10 * levelPrice[level];
            userWallet[receiver].tokenBalance[currentReward] += 10 * levelPrice[level];
            }
//set AutoUpgrade
