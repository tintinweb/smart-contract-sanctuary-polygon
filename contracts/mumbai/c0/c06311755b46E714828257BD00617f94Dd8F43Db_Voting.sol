...
// setter
    ...
    function addVoters(address[] memory _voters, uint[] memory _startTime, uint[] memory _endTime) public {
        ...
        // uint[] memory startTime = _startTime;
        // uint[] memory endTime = _endTime;
        // address[] memory _voterAddressList;
        for(uint i=0; i < _voters.length; i+=1) {
            // address  aVoter = voters[i];
            voterInfoMapping[_voters[i]] = VoterInfo(false, _startTime[i], _endTime[i]);
            voterAddressList.push(_voters[i]);
            // _voterAddressList[i] = voters
        }

        // voterAddressList = _voterAddressList;
