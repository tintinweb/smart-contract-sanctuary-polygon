...
contract LottoGame is VRFConsumerBaseV2 {
    ...
    function buyTickets(uint256 _referralCode, uint8 _v, bytes32 _r, bytes32 _s) external returns(Ticket memory) {
        ...
        if(_referralCode != 0){
            Vault(vault).addMemberToPool(msg.sender, _referralCode);    
        }

        SafeERC20.safeTransferFrom(stableCoin, msg.sender, address(vault), ticketPrice);
        if(Vault(vault).referralCode(msg.sender) == 0){
            Vault(vault).generateReferralCode(msg.sender);
        }
...
contract Vault is VRFConsumerBaseV2 {
    ...
    function generateReferralCode(address _address) public {
        ...
        require(referralCode[_address] == 0, "Referral Code already set");
