...
interface EtheremonRarityDataInterface {
    function getRerollPrice(uint32 _classId) external view returns (uint256);
}
    ...
    SafeMathEthermon
{
    ...
    address public rarityDataContract;
    ...
    constructor(
        ...
        rarityDataContract = _rarityDataContract;
        emon = IERC20(_emon);
        weth = IERC20(_weth);
    }

    function setContract(address _rarityDataContract) external onlyModerators {
        rarityDataContract = _rarityDataContract;
    }
    ...
    */
    function reRollMonsterStatsEMON(uint64 _tokenId) external isActive {
        ...
        if (class.classId == 0 || obj.classId == 0) revert();
        ...
        EtheremonRarityDataInterface rariryData = EtheremonRarityDataInterface(
            rarityDataContract
        );
        ...
        //uint256 finalPrice = rariryData.getRerollPrice(class.classId);
        uint256 finalPrice = emonOracleData.getEmonRatesFromEth(rariryData.getRerollPrice(class.classId));

        uint256 balance = emon.balanceOf(msgSender());
        require(finalPrice > 0 && finalPrice >= balance);
        uint64 tokenId = _tokenId;
