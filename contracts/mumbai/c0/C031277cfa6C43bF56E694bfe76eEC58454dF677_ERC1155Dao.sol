...
contract Ownable is IOwnable {
    ...
    /// @notice `1` Not Owner, `2` -> Invalid Address. Signature : `0x30cd7471`
    error InvalidOperation(uint8 status);
     ...
     */
    modifier onlyOwner() {
        if (_owner != msg.sender) revert InvalidOperation(1);
        _;
    }
    ...
    modifier checkForInvalidAddress(address newOwner) {
        if (newOwner == address(0)) revert InvalidOperation(2);
        _;
    }
...
contract ERC1155Dao is IERC1155Dao, Ownable, Pausable, Auction {
    ...
    /// `1` -> token Owner is allowed, `2` -> token owner is not allowed, `3` -> must end lending first
    /// `4` -> token is expired, `5` -> not enough life, `6` -> value is out of range
    /// `7` -> transfer or bidding before sale is not allowed, `8` -> token already active
    /// `9` -> token already inactive, `10` -> life must be multiple of the day
    /// `11` -> auction inactive, `12` -> auction ended, `13` -> borrower active
    /// `14` -> only winner can claim, `15` -> already listed for fixed price or auction
    /// `16` -> already in lending, `17` -> not listed for fixed price or auction
    /// `18` -> not minted, `19` -> already minted
    error NotAvailableForOperation(uint status);
    ...
    function _checkforExpired(
        ...
        if (startOfLife == 0 && block.timestamp > expireOn)
            revert NotAvailableForOperation(4);
        if (startOfLife != 0 && block.timestamp + period > endOfLife)
            revert NotAvailableForOperation(5);
    }
    ...
    function _getPrice(
        ...
        unchecked {
            if (startOfLife == 0) {
                ...
                return
                    ((tokenPrice / 100) * modify) *
                    (((endOfLife - startOfLife) / 100) * endOfLife);
            }
        }
    }
    ...
    function mint(
        ...
        if (_tokenDetails[tokenId].isMinted)
            revert NotAvailableForOperation(19);
    ...
    function buyFromContract(
        ...
        if (serialNo == 0) revert NotAvailableForOperation(6);
    ...
    function listTokenForFixedPrice(
        ...
        if (tokenOwner.lendingStatus) revert NotAvailableForOperation(3);
        ...
        emit Listing(tokenId, serialNo, true, block.timestamp);
    }
    ...
    function changeListingForFixedPrice(
        ...
        if (tokenOwner.user != msg.sender) revert NotAvailableForOperation(1);
        if (tokenOwner.fixedOrAuction != 1) revert NotAvailableForOperation(17);
    ...
    function unlistTokenFromFixedPrice(
        ...
        emit Listing(tokenId, serialNo, false, block.timestamp);
    }
    ...
    function buyFromUser(
        ...
        if (tokenOwner.user == msg.sender) revert NotAvailableForOperation(2);
        if (tokenOwner.fixedOrAuction != 1) revert NotAvailableForOperation(15);
    ...
    function listForAuction(
        ...
        emit AuctionState(tokenId, serialNo, true, block.timestamp);
    }
    ...
    function placeBidInAuction(
        ...
        if (tokenOwner.fixedOrAuction != 2) revert NotAvailableForOperation(11);
        if (tokenOwner.biddingLife > block.timestamp)
            revert NotAvailableForOperation(12);
    ...
    function claimTokenFromAuction(
        ...
        if (highestBidder != msg.sender) revert NotAvailableForOperation(14);
        ...
        emit AuctionState(tokenId, serialNo, false, block.timestamp);
    ...
    function unlistFromLending(
        ...
        if (
            ...
            block.timestamp
        ) revert NotAvailableForOperation(13);
    ...
    function borrowToken(
        ...
        emit Borrow(tokenId, serialNo, block.timestamp);
    }
    ...
    function activateToken(
        ...
        if (tokenOwner.user != msg.sender) revert NotAvailableForOperation(1);
        if (tokenOwner.isActivated) revert NotAvailableForOperation(8);
        if (tokenOwner.fixedOrAuction != 0) revert NotAvailableForOperation(15);
        if (tokenOwner.lendingStatus) revert NotAvailableForOperation(16);
    ...
    function transferContractTotalAmount(
        ...
        if (amount > _totalAmount) revert NotAvailableForOperation(6);
...
contract Auction is IAuction {
     ...
     *
     * Signature for Bid(uint256,uint256,bool) : `0x4593ac8d`
     *
    ...
    /// @notice `1` -> No bid, `2` bid lower than previous, `3` highest bidder cannot revoke. Signature : `0x1c65b3ea`
    error BidError(uint8 status);
    ...
    function _placeBid(
        ...
        emit Bid(tokenId, serialNo, true);
    }
    ...
    function _revokeBid(uint256 tokenId, uint256 serialNo) internal virtual {
        ...
        if (bidderAmout == 0) revert BidError(1);
        if (_highestBidder[tokenId][serialNo] == msg.sender) revert BidError(3);

        _otherBidders[tokenId][serialNo][msg.sender] = 0;

        payable(msg.sender).transfer(bidderAmout);

        emit Bid(tokenId, serialNo, false);
    }
