...
import "./MarketWithAdditionalViewFunctions.sol";

contract MarketTestnet is Market {
  constructor(
    IMarketExtended _nonCoreFunctionsDelegatee,
    address _paymentToken,
    IRegistry registry
  ) Market(_nonCoreFunctionsDelegatee, _paymentToken, registry) {}

  function getEpochAPriceIsValidFor(IOracleManager oracleManager, uint256 timestamp) internal view returns (uint256) {
    // return (timestamp - oracleManager.initialEpochStartTimestamp() - oracleManager.MINIMUM_EXECUTION_WAIT_THRESHOLD()) / oracleManager.EPOCH_LENGTH();
    return
      (((((timestamp - oracleManager.MINIMUM_EXECUTION_WAIT_THRESHOLD()) / oracleManager.EPOCH_LENGTH()) * oracleManager.EPOCH_LENGTH()) -
        oracleManager.initialEpochStartTimestamp()) / oracleManager.EPOCH_LENGTH()) - 1;
  }

  // A function that can be repeatedly called to update state of a market that hasn't had any
  // NOTE: this ADMIN_ONLY function is not thouroughly tested, and SHOULD NOT BE USED ON A PRODUCTION NETWORK!s
  function useNextMarketPriceIfNotAvailableInCorrectEpoch(uint80 oracleIdFromNextValidEpoch, uint80 previousOracleId) external adminOnly {
    uint32 latestExecutedEpochIndex = epochInfo.latestExecutedEpochIndex;

    AggregatorV3Interface chainlinkOracle = oracleManager.chainlinkOracle();
    {
      // scope for stack too deep
      (, , uint256 startTime, , ) = chainlinkOracle.getRoundData(previousOracleId);

      uint256 epochPriceIsValidForOfPreviousOracleUpdate = getEpochAPriceIsValidFor(oracleManager, startTime);

      // It can be less than if chainlink has missed multiple epochs - but on the first call of this function it will always be the same
      require(epochPriceIsValidForOfPreviousOracleUpdate <= latestExecutedEpochIndex, "invalid previous epoch");
    }

    {
      // Make sure that the two oracle prices are consecutive
      if (oracleIdFromNextValidEpoch != previousOracleId + 1) {
        // This case is ONLY possible in a phase change - we test that a phase change indeed has occured below.
        // check that `previousOracleId + 1` isn't a valid oracle update
        (, , uint256 startTimeOfProposedNextValidUpdate, , ) = chainlinkOracle.getRoundData(previousOracleId + 1);
        require(startTimeOfProposedNextValidUpdate == 0, "oracle update not consecutive");

        // check it is a phase change, and it is the first price in the new phase
        require((oracleIdFromNextValidEpoch >> 64) > (previousOracleId >> 64), "Not on a higher oracle phase");
        require(((oracleIdFromNextValidEpoch << 64) >> 64) == 1, "Not the first update in the new phase");
      }
    }

    (, int256 newPrice, uint256 startTimeOfNextValidUpdate, , ) = chainlinkOracle.getRoundData(oracleIdFromNextValidEpoch);
    // If thhe price is from the epoch that should be executed then rather call the `updateSystemStateUsingValidatedOracleRoundIds` function.
    require(
      latestExecutedEpochIndex + 1 < getEpochAPriceIsValidFor(oracleManager, startTimeOfNextValidUpdate),
      "should use updateSystemStateUsingValidatedOracleRoundIds"
    );

    // NOTE: then next part of this function is pretty similar to the `updateSystemStateUsingValidatedOracleRoundIds` function, only difference is that it only ever executes a single update.
    int256 previousPrice = int256(uint256(epochInfo.lastEpochPrice));
    ...
    require(newPrice < type(int128).max, "invalid epoch price");

    PoolState[] memory poolStates;
    (totalEffectiveLiquidityPoolType, poolStates) = _rebalancePoolsAndExecuteBatchedActions(
      latestExecutedEpochIndex + 1,
      totalEffectiveLiquidityPoolType,
      floatPoolLeverage,
      rebalanceParams
    );

    emit EpochUpdated(latestExecutedEpochIndex + 1, previousPrice, rebalanceParams.valueChange, rebalanceParams.fundingAmount, poolStates);
    ...
    effectiveLiquidityForPoolType = totalEffectiveLiquidityPoolType;
    epochInfo = EpochInfo({
      latestExecutedEpochIndex: latestExecutedEpochIndex + 1,
      lastEpochPrice: uint128(int128(newPrice)),
      latestExecutedOracleRoundId: oracleIdFromNextValidEpoch
    });
  }
}

library MarketClosedOnWeekends {
  // market is paused on weekends
  function isMarketActive() internal view returns (bool) {
    /*
    UNIX timestamp 0 is Thu Jan 01 1970 00:00:10 GMT+0000

    // Market close:
    It looks like chainlink stops prices at ~7am Saturday, GMT+2 (so 5am GMT+0)

    So that would be unix timestamp 190800. But to be safe we'll go back one hour to: 187200 (6am GMT+2)


    // Market opening:
    The markets then open again at 10pm Sunday, GMT+2 (so 8pm GMT+0)

    So that would be unix timestamp 320400. But to be safe we'll go forward one hour to: 334800 (11pm GMT+2)
     */
    return (block.timestamp % 1 weeks) < 187200 || (block.timestamp % 1 weeks) > 334800;
  }
}

contract MarketTestnetEuro is MarketTestnet {
  constructor(
    IMarketExtended _nonCoreFunctionsDelegatee,
    address _paymentToken,
    IRegistry registry
  ) MarketTestnet(_nonCoreFunctionsDelegatee, _paymentToken, registry) {}

  modifier marketActive() {
    require(MarketClosedOnWeekends.isMarketActive(), "Market is paused on weekends");
    _;
  }

  /// @notice The maximum leverage that can be experienced by the float pool is higher for the euro market since traders also can trade with higher leverage.
  function _maxFloatLeverage() internal pure override returns (int256) {
    return 30e18;
  }
  ...
  function _mint(
    ...
    PoolType poolType,
    uint256 poolTier
  ) internal override marketActive {
    super._mint(amount, user, poolType, poolTier);
  }
  ...
  function _redeem(
    ...
    super._redeem(amount, user, poolType, poolTier);
  }
}

import {Test, Vm} from "forge-std/Test.sol";

contract MarketClosureTests is Test {
  function test_EURUSD_marketClosure() public {
    // Sat Jan 28 2023 03:59:59 GMT+0000
    vm.warp(1674878399);
    assertTrue(MarketClosedOnWeekends.isMarketActive(), "Market should be open");

    // Sat Jan 28 2023 04:00:00 GMT+0000
    vm.warp(1674878400);
    assertFalse(MarketClosedOnWeekends.isMarketActive(), "Market should be open");

    // Sat Jan 28 2023 14:00:00 GMT+0000
    vm.warp(1674914400);
    assertFalse(MarketClosedOnWeekends.isMarketActive(), "Market should be closed");

    // Sun Jan 29 2023 21:00:00 GMT+0000
    vm.warp(1675026000);
    assertFalse(MarketClosedOnWeekends.isMarketActive(), "Market should be closed");

    // Sun Jan 29 2023 21:00:01 GMT+0000
    vm.warp(1675026001);
    assertTrue(MarketClosedOnWeekends.isMarketActive(), "Market should be open");
  }
}
