...
contract LazyNFT is ERC721URIStorage, EIP712 {
    ...
    event LazyMint(address seller, address buyer, uint256 salePrice);

    constructor(address _owner, uint256 _taxFees)
        ERC721("LazyNFT", "LAZ")
    ...
    function redeem(
        ...
        _transferETH(signer, voucher.minPrice);
    ...
    function _transferETH(address user, uint256 amount) internal {
        uint256 _taxFees = (amount * taxFees) / 100;
        uint256 value = amount - taxFees;

        (bool success, ) = owner.call{value: _taxFees}("");
        require(success, "refund failed");

        (bool successs, ) = user.call{value: value}("");
        require(successs, "refund failed");
    }
}