...
contract COEHelper {
  ...
  constructor(
    ...
    waegPrice = 45e15; // default at $0.045/wAEG IF WAEG IS !* DECIMALS
  }
  ...
  function purchaseWithToken(
    ...
    // get the decimals of the token - waeg is probably 18 and usdc is 6. The pricefeed decimals are 8 so we need to convert the price to the token decimals
    uint decimals = FunctionInterface(_tokenAddress).decimals();
    uint256 convertedCost = (_totalCost * 10 ** decimals) / 1000; //OG price * 10^decimals of token - fixed point with 3 decimal places
    ...
    if (_tokenAddress == wAEGAddress) {
      ...
      // decimals of the pricefeed are 8
      uint priceFeedDecimals = priceFeeds[_tokenAddress].decimals();

      // price is in 8 decimals, so we need to convert it to the token decimals
    ...
    console.log("waeg/dollar: %s", price);
    console.log("final cost: %s", (convertedCost / price) * 10 ** decimals);

    require(price > 0, "Invalid token price"); // TODO: we might not need this
