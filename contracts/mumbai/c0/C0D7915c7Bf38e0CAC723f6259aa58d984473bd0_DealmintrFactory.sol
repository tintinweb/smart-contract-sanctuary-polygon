...
import './Dealmintr.sol';

contract DealmintrFactory is Ownable {
    
    struct DealmintrNFT{
        string  c_name;
        string name;
        string symbol;
        address c_collection;
        address controller;
    ...
    mapping(uint256 => DealmintrNFT) public dealmintrNfts;
    uint256 index = 0;
    event CreateCollection(address to,address from,uint256 index);
    constructor(string memory _baseURI){
     ...
     uint256 price
        ...
        Dealmintr _dealmintr = new Dealmintr(name,symbol,baseURI,maxBatchSize,controller,companyAddress,price);
        dealmintrs.push(address(_dealmintr));
        index ++;
        DealmintrNFT memory _dealmintrNft = DealmintrNFT(c_name,name,symbol,address(_dealmintr),controller,companyAddress,price);
        dealmintrNfts[index] = _dealmintrNft;
        indexes[msg.sender].push(index);
        emit CreateCollection(address(this),address(_dealmintr),index);
     }
    ...
    function getAllCollection(uint256 id) external view returns(string memory,string memory,string memory,address,address,address,uint256){

       return (
           dealmintrNfts[id].c_name,
           dealmintrNfts[id].name,
           dealmintrNfts[id].symbol,
           dealmintrNfts[id].c_collection,
           dealmintrNfts[id].controller,
           dealmintrNfts[id].companyAddress,
           dealmintrNfts[id].price
           );
    }

    function getCollectionId(address cAddress) external view returns(uint256[] memory){

        return indexes[cAddress];
    }
}
    ...
    Ownable
{
    ...
    modifier onlyController{
        require(msg.sender == controller,"This user is not a controller.");
        _;
    }

    modifier onlyCompany{
        require(msg.sender == companyAddress,"This account is not for the company.");
        _; 
    }
    ...
    function mint(
        ...
        require(_price == price,"not match the price.");
        require(maxBatchSize <= quantity,"cannot mint more than maxBatchSize.");
        _mint(_to, _tokenId, royaltyRecipient, royaltyValue);
    ...
    function setUpdatePrice(uint256 newPrice) external onlyCompany{
        require(newPrice > 0, "The new price cannot zero.");
        price = newPrice;
    }
    ...
    function batchmint(
        ...
        require(_quantity == maxBatchSize, "cannot mint more than maxBatchSize");
        for (uint256 i = 1; i <= _quantity; i++) {
            _mint(to, i, royaltyRecipient, royaltyValue);
            _setTokenURI(i, _tokenURI);
        }
    }

    function getBatchMaxSize() external view returns(uint256){
        return maxBatchSize;
    }
    ...
    function supportsInterface(bytes4 _interfaceId) public view virtual override(ERC721Base, ERC721, ERC721Enumerable) returns (bool) {
    ...
    function burn(uint256 tokenId) public virtual override onlyCompany{
        _burn(tokenId);
    }
}
    ...
    ERC2981PerTokenRoyalties
{
    ...
    function supportsInterface(bytes4 _interfaceId)
        ...
        view
        virtual
        override(ERC721, ERC165Storage, ERC2981Base)
        returns (bool) 
    {
        return super.supportsInterface(_interfaceId);
    }
