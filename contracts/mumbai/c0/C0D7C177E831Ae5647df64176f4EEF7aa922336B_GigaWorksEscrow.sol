...
//add an limit to how high this can be
    function updateClosingFee(uint _closingFee) public {
...
// function to dispute & resolve dispute
    ...
    function disputeEscrow(uint256 escrowId) public returns (bool) {
        ...
        if(_address != escrow.buyer && _address != escrow.seller) revert YouCantDispute('wrong role'); 
    ...
    function resolveDispute(uint256 escrowId) public returns (bool) {
        require(hasRole(ARBITRATOR_ROLE, msg.sender));
        Escrow storage escrow = idToEscrow[escrowId];
        ...
        payable(teamWallet).transfer(escrow.amount);
