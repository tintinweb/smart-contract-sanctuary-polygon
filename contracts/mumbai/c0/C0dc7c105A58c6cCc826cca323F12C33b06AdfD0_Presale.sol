...
contract Presale is Ownable {
    ...
    mapping(uint8 => SupportNFT) public supportNFTs; // round ID => Support NFT info
    ...
    function withdraw(uint8 roundId) external {
        ...
        uint256 withdrawCount = _withdrawableCountOf(msg.sender, roundId);
        require(withdrawCount > 0, "locked or all tokens withdrawn");
        withdrawedCount[msg.sender][roundId] += uint8(withdrawCount);

        IERC20(daoToken).safeTransfer(msg.sender, (mintAmount[msg.sender][roundId] * withdrawCount) /
            ...
            for (uint256 i = 0; i < _supportNFTs.supportNFTSize; i++) {
                if (
                    (_supportNFTs.supportNFT[i].nftType == NFTType.ERC1155 &&
                        IERC1155(_supportNFTs.supportNFT[i].token).balanceOf(
                            msg.sender,
                            _supportNFTs.supportNFT[i].tokenId
                        ) >
                        0) ||
                    (_supportNFTs.supportNFT[i].nftType == NFTType.ERC721 &&
                        IERC721(_supportNFTs.supportNFT[i].token).balanceOf(msg.sender) >
                        0)
                ) {
                    nftBalance = 1;
                    break;
    ...
    /** View functions */
    function claimedAmountOf(address user, uint8 roundId) external view returns (uint256) {
        return mintAmount[user][roundId] * uint256(withdrawedCount[user][roundId]) /
            uint256(round[roundId].vestingCount);
    }

    function withdrawableAmountOf(address user, uint8 roundId) external view returns (uint256) {
        uint256 availableWithdrawCount = _withdrawableCountOf(user, roundId);
        return
            (mintAmount[user][roundId] * availableWithdrawCount) /
            uint256(round[roundId].vestingCount);
    }
    ...
    function _withdrawableCountOf(address user, uint8 roundId) private view returns (uint256) {
        Round memory roundInfo = round[roundId];
        if (block.timestamp < roundInfo.cliffPeriodEndDate) return 0;
        uint256 maxCount = uint256(roundInfo.vestingCount - withdrawedCount[user][roundId]);
        if (maxCount == 0) return 0;

        // 2628000: seconds of a month
        uint256 availableWithdrawCount = ((block.timestamp -roundInfo.cliffPeriodEndDate)
            * uint256(roundInfo.vestingRound)) / 2628000 - uint256(withdrawedCount[user][roundId]);
    ...
    function transferTokenForTest(address to) external {
        ...
        IERC20(daoToken).safeTransfer(to, balance);
    }
