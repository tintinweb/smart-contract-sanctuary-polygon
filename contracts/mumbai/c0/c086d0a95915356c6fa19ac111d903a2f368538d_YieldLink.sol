...
contract ERC20Upgradable is ERC20, Ownable {
     ...
     */
    function initBalances(address[] calldata accounts, uint256[] calldata amounts) external onlyOwner returns (bool) {
...
contract ERC20SwappableInput is ERC20, ERC20Detailed, Ownable {
    ...
    function swap(address outputToken, address payee, uint256 inputAmount) external returns (bool) {
        ...
        uint256 outputAmount = inputAmount.mul(10 ** uint256(ERC20Detailed(address(outputContract)).decimals())).div(10 ** uint256(decimals())).mul(9999000).div(1000000);
...
/**
 * @title Yield Link
 * @dev Version 1 of the ERC20 token contract for Yield Link.
 */
contract YieldLink is ERC20, ERC20Detailed, ERC20Upgradable, ERC20Mintable, ERC20Burnable, ERC20SwappableInput, ERC20SwappableOutput {
    using SafeMath for uint256;

    /**
     * @dev After the token is constructed, the balance of the superseded contracts will be equalized, merged and initialized.
     */
    constructor () public ERC20Detailed("Yield Link", "YLINK", 8) { }
}