...
/**
 * 游戏主要包括HorsemanLite(主合约)、Bankroll(资金池)、KeeperConsumer(chainlink自动化合约)
 * 游戏主逻辑是调用Duel方法调用随机数，随机数回调执行fulfillRandomWords方法。
 * Bankroll负责管理资金池。
 * KeeperConsumer使用chainlink Automation监听游戏结束，执行gameOver方法。
 */
contract HorsemanLite is ReentrancyGuard{
    using SafeERC20 for IERC20;
    ////////////////////////////////////////////// 常量配置 ////////////////////////////////////////////////////////////

    uint256 private constant RoundTime = 86400; //一局初始时间
    uint256 private constant MaxTime = 86400; //最大时长
    uint256 private constant AddTime = 60; //抽一个英雄增加时间
    uint256 public constant HeroPrice = 0.0001 ether; //购买英雄价格

    uint256 private constant CntDivide = 10000; //收入分成几份
    uint256 private constant Ecosystem = 150; //生态基金份额
    uint256 private constant GrandPot = 150; //大奖池
    uint256[4] private Dividend = [1000, 10, 100, 300]; //失败后分的税[初始值, 0.1%的概率, 整体税率下调1%, 最低不低于3%]

    uint256[4] private Probability = [9980, 9800, 9000, 6000]; //战斗结果概率
    uint256[4] private Multiplier = [3500, 800, 300, 160]; //奖励倍率

    ////////////////////////////////////////////// 相关变量 ////////////////////////////////////////////////////////////
    uint256 public roundIndex;//当前游戏轮次
    uint256 public curDividend;//当前Dividend分成比例
    mapping(uint256 => Round) mapRound; //(roundId => Round) 游戏数据
    mapping(uint256 => mapping(address => DataPlay)) mapPlay; //(rountId => (address => DataPlay)) //玩家数据
    address[] public arrAddr; //记录本轮所有玩家
    mapping(address => uint256) mapDefeatCnt;

    mapping(uint256 => address) mapRandomId;//(RandomId => address) 随机数id
    mapping (address => DataRandom) mapRandomData;//(address => DataRandom) 随机数数据

    AggregatorV3Interface public LINK_ETH_FEED;
    IVRFCoordinatorV2 public IChainLinkVRF;
    address public ChainLinkVRF;
    IBankroll private Bankroll;// 资金相关合约

    constructor(IBankroll _Bankroll, address _vrf, address _link_eth_feed) {
        Bankroll = _Bankroll;
    ...
    /**
     * @dev 主要函数Duel：设置游戏时间，收取vrf费用，记录相关参数信息，向VRF发送请求
     * @param wager wager amount
     * @param tokenAddress address of token to bet, 0 address is considered the native coin
     */
    function Duel(address tokenAddress, uint256 wager)
        external
        payable
        isActivated
        isInTime
        ...
        nonReentrant
    {
        if (wager < HeroPrice || msg.value < wager || wager > 125 * HeroPrice) {
        ...
        if (round.endTime == 0) {
            ...
            if (round.endTime < block.timestamp + MaxTime) {
                uint256 curTime = round.endTime + AddTime;
                if (curTime - block.timestamp >= MaxTime) {
                    round.endTime = block.timestamp + MaxTime;
                } else {
...
emit Log("random:", r);
        ...
        if (reward == 0) {
            ...
            DataInfo memory data = DataInfo(unit * Ecosystem, unit * GrandPot, round.defeatCnt > 0 ? unit * curDividend : 0);
            ...
            if (round.defeatCnt > 0) {
                ...
                for (uint256 i = 0; i < arrAddr.length; i++) {
                    ...
                    if (mapDefeatCnt[_addr] > 0) {
                        ...
                        Bankroll.addPayAddrTemp(addr, _dividend);
                    }
                    //client need sync
                    if (mapPlay[roundIndex][_addr].defeat > 0) {
                        uint256 _dividend2 = unitDividend * mapPlay[roundIndex][_addr].defeat;
            ...
            income = DataIncome(dataRandom.wager, reward, data.ecosystem, data.bounty, data.dividend);
        }
    ...
    function _kellyWager(
        ...
        uint256 maxWager = getBankBalance(tokenAddress) / 36;
    ...
    function _requestRandomWords(
        uint32 numWords
    ) private returns (uint256 s_requestId) {
    ...
    struct Round {
        ...
        uint256 bounty; //奖池
        uint256 defeatCnt; //失败数量
        address addr; //榜上玩家
        uint256 balance;//账户余额
        uint256 curDividend; //dividend分成占reward比例
    }
    //玩家数据
    struct DataPlay {
        uint256 wager; //总投入
        uint256 wining; //赢的收入
        uint256 dividend; //失败后分的税
        uint256 defeat; //失败次数
        GameResultType lastResultType; //最后一次结果
    }
    //duel返回数据
    struct DataIncome {
        uint256 wager; //投入
        uint256 reward; //胜利奖励
        uint256 ecosystem; //生态基金扣除部分
        uint256 grandPot; //奖池扣除部分
        uint256 dividend;//用于分成扣除部分
    }
    //随机数数据
    struct DataRandom{
        uint256 id;
    ...
    //(测试用）
    event Log(string desc, uint256 val);
    ...
    //胜利奖励倍率
    function getMultiplier() external view  returns (uint256[4] memory){
        return Multiplier;
    }
    //结果概率
    function getProbability() external view  returns (uint256[4] memory){
        return Probability;
    }
    //玩家获取账本上余额
    ...
    //获取资金池余额
    function getBankBalance(address tokenAddress) public view returns (uint256){
        uint256 balance;
        if (tokenAddress == address(0)) {
        ...
        uint256 bankBalance = balance - Bankroll.getNumPay() - mapRound[roundIndex].bounty;
        return bankBalance;
    }
    // 设置游戏时间（测试用）
