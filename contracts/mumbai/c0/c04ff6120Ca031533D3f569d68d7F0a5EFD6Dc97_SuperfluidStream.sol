...
contract SuperfluidStream {
    //employer
    //convert USDC to USDCx
    //pay salary in stream
    //update salary
    //delete

    /**State Variable */
    ISuperToken private immutable i_fDAIxAddress;

    using CFAv1Library for CFAv1Library.InitData;
    CFAv1Library.InitData public cfaLib;

    constructor(ISuperfluid host, ISuperToken fDAIxAddress) {
        i_fDAIxAddress = fDAIxAddress;
    ...
    function sendLumpSumIntoContract(
        uint256 amount,
        ISuperToken token
    ) external {
    ...
    /// @param flowRate Flow rate of Wei per second to stream. Always in int96
    function createFlowFromContract(
        address receiver,
        ISuperToken token,
        int96 flowRate
    ) external {
        cfaLib.createFlow(receiver, token, flowRate);
    }
    ...
    function updateFlowFromContract(
        ...
        cfaLib.updateFlow(receiver, token, flowRate);
    }
    ...
    function deleteFlowFromContract(
        ...
        cfaLib.deleteFlow(address(this), receiver, token);
    }
    ...
    function createFlowIntoContract(
        ...
        cfaLib.createFlowByOperator(msg.sender, address(this), token, flowRate);
    }
    ...
    function updateFlowIntoContract(
        ...
        cfaLib.updateFlowByOperator(msg.sender, address(this), token, flowRate);
    }

    function deleteFlowIntoContract(ISuperToken token) external {
        cfaLib.deleteFlowByOperator(msg.sender, address(this), token);
    }

    function withdrawFundsOutOfContract(
        uint256 amount,
        ISuperToken token
    ) external {
    ...
    function getfDAIxbalance(address account) public view returns (uint256) {
        return i_fDAIxAddress.balanceOf(account);
    }
}