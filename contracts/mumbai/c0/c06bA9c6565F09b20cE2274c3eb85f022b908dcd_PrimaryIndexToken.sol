...
contract PrimaryIndexToken is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable
{
    ...
    function getCollateralAvailableToWithdraw(address account, address projectToken, address lendingToken) public view returns(uint collateralProjectToWithdraw) {
        ...
        DepositPosition memory _depositPosition = depositPosition[account][projectToken][usdcToken];

        //13010276000000
        uint256 totalDepositedCollateral = _depositPosition.depositedProjectTokenAmount;
        //499950000000
        uint256 _totalOutstandingInUSD = lendingToken == address(0) ? 0 : totalOutstandingInUSD(account, projectToken, lendingToken);
        // 499950000000 * 100 * 10e18/2010602254877388125/81 = 3069837500000
        ...
        collateralProjectToWithdraw =  totalDepositedCollateral > depositedAmountSatifyHF? totalDepositedCollateral-depositedAmountSatifyHF: 0;
    }

    function getCollateralAvailableToWithdrawTest(address account, address projectToken, address lendingToken) public view returns(uint, uint, uint, uint) {
        (uint256 lvrNumerator, uint256 lvrDenominator) = getLoanToValueRatio(projectToken, lendingToken);
        DepositPosition memory _depositPosition = depositPosition[account][projectToken][usdcToken];
        ...
        uint256 collateralProjectToWithdraw =  totalDepositedCollateral > depositedAmountSatifyHF? totalDepositedCollateral-depositedAmountSatifyHF: 0;
        return (totalDepositedCollateral,_totalOutstandingInUSD, depositedAmountSatifyHF, collateralProjectToWithdraw );
    }
