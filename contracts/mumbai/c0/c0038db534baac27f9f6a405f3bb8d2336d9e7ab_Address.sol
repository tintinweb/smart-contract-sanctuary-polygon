...
// File: tests/MetaMooNodeManager.sol
...
contract MetaMooNodeManager is Ownable {
    ...
    struct Node {
      ...
      uint256 MetaMooUpgradeStartTimestamp;
    ...
    function _createNode(address user, string memory name, uint256 nodeTypeId) public onlyOwner {
      ...
      usersNodes[user].push(Node({
        ...
        MetaMooUpgradeStartTimestamp: 0,
        availableRewards: 0
      }));
    ...
    function _upgradeNode(address user, uint256 id) external onlyOwner returns (bool) {
      ...
      node.upgrading = true;
      node.MetaMooUpgradeStartTimestamp = block.timestamp;
    ...
    function _claimUpgrade(address user, uint256 id) external onlyOwner returns (bool) {
      ...
      node.MetaMooUpgradeStartTimestamp = 0;
    ...
    function _getClaimableUpgrades(address user) external onlyOwner view returns (uint256[] memory) {
      ...
      for (uint256 i = 0; i < nodes.length; i++) {
        ...
        if(node.upgrading) {
          if(block.timestamp >= node.MetaMooUpgradeStartTimestamp + nodeType.timeToUpgrade) {
    ...
    function canClaimUpgrade(Node memory node) private view returns (bool) {
      ...
      return node.upgrading && block.timestamp >= node.MetaMooUpgradeStartTimestamp + nodeType.timeToUpgrade;
    }
...
contract MetaMoo is ERC20, Ownable {
    using SafeMath for uint256;

    MetaMooNodeManager private nodeManager;
    ...
    // Pools & Wallets
    address public _futurUsePool = 0x2E9fb249dBA16D95b7D821EB44349930b77D28AF;
    address public _distributionPool = 0x8726006AB8B34Df5Bd9EA3582c64197BDd30a3DC;
    ...
    constructor() ERC20("MetaMoo Nodes", "MetaMoo") {
        nodeManager = new MetaMooNodeManager();
    ...
    function buyNode(string memory name, uint256 nodeType) external  {
      ...
      uint256 contractMetaMooBalance = balanceOf(address(this));
      bool canProcess = contractMetaMooBalance >= swapTokensAtAmount;
      ...
      if (canProcess && !swapping) {
          ...
          if(_futurUseNodeFee > 0) {
            uint256 futurUseTokens = contractMetaMooBalance.mul(_futurUseNodeFee).div(100);
            swapAndSendToFee(_futurUsePool, futurUseTokens);
          }

          // Liquidity
          if(_liquidityPoolFee > 0) {
            uint256 swapTokens = contractMetaMooBalance.mul(_liquidityPoolFee).div(100);
    ...
    function getNodes(address user) external view returns (MetaMooNodeManager.Node[] memory) {
      return nodeManager._getNodes(user);
    }

    function getNodeTypes() external onlyOwner view returns (MetaMooNodeManager.NodeType[] memory) {
      return nodeManager._getNodeTypes();
    }

    function getBoosterDatas(address user) external view returns (MetaMooNodeManager.BoosterData memory) {
