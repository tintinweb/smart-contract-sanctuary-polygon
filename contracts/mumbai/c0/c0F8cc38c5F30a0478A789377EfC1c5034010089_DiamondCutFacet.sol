...
import {IDiamondCut} from "../interfaces/IDiamondCut.sol";
import {IPermissionsRegistry} from "../../permissions/interfaces/IPermissionsRegistry.sol";
...
library LibDiamond {
    ...
    event PermissionsRegistryUpdated(address indexed previousRegistry, address indexed newRegistry);

    function setPermissionsRegistry(address _newRegistry) internal {
        DiamondStorage storage ds = diamondStorage();
        address previousRegistry = ds.permissionsRegistry;
        ds.permissionsRegistry = _newRegistry;
        emit PermissionsRegistryUpdated(previousRegistry, _newRegistry);
    }

    function enforceIsPlatform() internal view {
        require(IPermissionsRegistry(
            diamondStorage().permissionsRegistry
        ).isPlatformExecutor(LibMeta.msgSender()), "LibDiamond: Must be platform executor");
    }
...
interface IPermissionsRegistry {
     ...
     */
    function initialize(address _initialExecutor, address _platformVault) external;
