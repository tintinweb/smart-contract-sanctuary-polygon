...
contract Dao {
  ...
  event proposalCount(
    ...
    function createProposal(string memory _description, uint256  _deadline) public {
        ...
        require(balance > 0);
        require((_deadline > (block.timestamp.add(1 days))) && (_deadline < block.timestamp.add(4 weeks)));
    ...
    function viewTime() public view returns (uint256 _time, uint256 _timeClose, uint256 _timeFar) {
        _time = block.timestamp;
        _timeClose = (block.timestamp + 1 days);
        _timeFar = (block.timestamp + 4 weeks);
    }
}
