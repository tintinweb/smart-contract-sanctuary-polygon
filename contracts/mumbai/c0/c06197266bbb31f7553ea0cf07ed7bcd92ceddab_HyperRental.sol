...
contract HyperRental is ERC721Holder, AutomationCompatibleInterface {
    ...
    mapping(bytes32 => bytes[]) private _idToRentalAssets; // rentalId => abi.encodePacked(contractAddress,tokenId,amount);
    ...
    function createRentalPack() public returns (uint256, address) {
        uint256 tokenId = RentalPackNFT(rentalPackAddress).safeMint(msg.sender);
    ...
    function _recordRentalInfo(
        uint256 rentalPackTokenId,
        RentalCondition memory condition,
    ...
    function _batchDepositAssets(uint256 rentalPackTokenId, bytes[] memory assetDatas) private {
        ...
        for (uint256 i; i < assetDatas.length; i++) {
            (address contractAddress, uint256 tokenId, uint256 amount) = abi.decode(assetDatas[i], (address, uint256, uint256));
            if (IERC165(contractAddress).supportsInterface(type(IERC3525).interfaceId)) {
                IERC3525(contractAddress).transferFrom(tokenId, tokenBoundAddress, amount);
            } else if (IERC165(contractAddress).supportsInterface(type(IERC1155).interfaceId)) {
                IERC1155(contractAddress).safeTransferFrom(
                    msg.sender, tokenBoundAddress, tokenId, amount, "0x"
                );
            } else if (IERC165(contractAddress).supportsInterface(type(IERC721).interfaceId)) {
                IERC721(contractAddress).transferFrom(msg.sender, tokenBoundAddress, tokenId);
            } else if (IERC165(contractAddress).supportsInterface(type(IERC20).interfaceId)) {
                IERC20(contractAddress).transferFrom(msg.sender, tokenBoundAddress, amount);
            } else {
                revert NotSupportedToken({contractAddress: contractAddress, tokenId: tokenId});
            }
        }
    }
    ...
    function _lockTokenBoundAccount(uint256 rentalPackTokenId) private {
        ...
        ILockable(tokenBoundAccount).lock();
    }

    function _listRentalPack(uint256 rentalPackTokenId) private {
        IERC721(rentalPackAddress).safeTransferFrom(msg.sender, address(this), rentalPackTokenId, "0x");
    }
    ...
    {
        require(
            IERC721(rentalPackAddress).ownerOf(rentalPackTokenId) == msg.sender, "msg.sender is not rentalPack owner"
        );
    ...
    function _unlockTokenBoundAccount(uint256 rentalPackTokenId) private {
        address tokenBoundAccount = IRegistry(tokenBoundAccountRegistry).account(rentalPackAddress, rentalPackTokenId);
        ILockable(tokenBoundAccount).unlock();
    }
    ...
    function _withdrawAllAssets(uint256 rentalPackTokenId) private {
        ...
        for (uint256 i; i < assetDatas.length; i++) {
            ...
            if (IERC165(contractAddress).supportsInterface(type(IERC3525).interfaceId)) {
                IAccount(tokenBoundAddress).executeCall(
                    ...
                    abi.encodeWithSignature(
                        "transferFrom(uint256,address,uint256)", tokenId, msg.sender, amount
                    )
                );
            }
            ...
            if (IERC165(contractAddress).supportsInterface(type(IERC20).interfaceId)) {
                IAccount(tokenBoundAddress).executeCall(
                    contractAddress,
                    0,
                    abi.encodeWithSignature(
    ...
    function _delistRentalPack(uint256 rentalPackTokenId) private {
        IERC721(rentalPackAddress).safeTransferFrom(address(this), msg.sender, rentalPackTokenId, "0x");
    }

    function _refreshRentalRecord(uint256 rentalPackTokenId) private {
        bytes32 rentalId = keccak256(abi.encodePacked(rentalPackAddress, rentalPackTokenId));
        _idToRentalConditions[rentalId] = RentalCondition(address(0), 0, 0, 0);
    ...
    function cancelLending(uint256 rentalPackTokenId) external {
        bytes32 rentalId = keccak256(abi.encodePacked(rentalPackAddress, rentalPackTokenId));
        require(_idToRentalConditions[rentalId].lender != address(0), "the given RentalPack is invalid");
        require(msg.sender == _idToRentalConditions[rentalId].lender, "msg.sender is not a lender");
    ...
    function rent(uint256 rentalPackTokenId, uint256 rentalHour, address receiver) public payable {
        ...
        require(condition.lender != address(0), "the given RentalPack is invalid");
        require(condition.feePerHour * rentalHour <= msg.value, "msg.value is insufficient");
        require(condition.mimHour <= rentalHour, "the given rental hour is too shoot");
        ...
        payable(condition.lender).transfer(msg.value);
    ...
    /* ========== ERRORS ========== */
    error NotSupportedToken(address contractAddress, uint256 tokenId);
}
