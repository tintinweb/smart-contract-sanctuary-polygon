...
contract Locker is ERC20 {
    ...
    function holdDeposit(
        ...
        if (hodler.balance > 0) {
            ...
            if (hodler.unlockTime < _unlockTime) {
                hodler.unlockTime = _unlockTime;
                hodler.rate = options[id]._rate;
            }
        } else {
            uint256 userepoch = (block.timestamp - startTime) / epoch + 1;
            hodlers[sender] = Hodler(
                msg.sender,
                amount,
                _unlockTime,
                options[id]._rate,
    ...
    function withdraw(uint256 _amount) public {
        ...
        require(
            block.timestamp > hodler.unlockTime,
            "Unlock time not reached yet."
        );
        require(hodler.balance > _amount, "Invalied balance");
        uint256 userEpochTimes = (block.timestamp - startTime) /
            epoch -
            hodler.epochTime;
        uint256 amount = (_amount *
            (100000 + userRate[msg.sender] * userEpochTimes)) / 100000;
        hodler.balance -= _amount;
    ...
    function getReward() public {
        ...
        require(
            hodler.epochTime > block.timestamp,
            "Invalied time for get reward."
        );
        uint256 userEpochTimes = (block.timestamp - startTime) /
            epoch -
            hodler.epochTime;
        uint256 amount = (hodler.balance *
            userRate[msg.sender] *
            userEpochTimes) / 100000;
        hodler.epochTime = userEpochTimes + 1;
        IERC20(vsqAddress).transfer(msg.sender, amount);
