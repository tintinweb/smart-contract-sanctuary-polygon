...
contract ComposableChildrenERC1155 is ERC1155PresetMinterPauser, ERC2771Context  {
    using SafeMath for uint256;

    event BatchMintedEngPoints(address[] to,uint256[] amount);
    event MintedEngPoints(address _to, uint _amount);
    event sNFTUpgraded(uint256 _composableId,uint256 _upgradeToTierId,string _uri, address _mintedFor); 
    ...
    mapping(address => mapping(uint256 => bool)) private _ownerToUpgradeInitiated;
    uint public composableIdInCsnft;
        ...
        ERC2771Context(TRUSTED_FORWARDER)
    {
        ...
        composableIdInCsnft = csnftContract.getComposableId(_msgSender());
    }
    ...
    function mintEngagementPoints(
        ...
        emit MintedEngPoints(_to, _amount);
    }

    function mintBatchEngagementPoints(
        address[] memory to,
        uint256[] memory amount,
        bytes[] memory data
    ) public virtual {
        require(hasRole(MINTER_ROLE, _msgSender()), "ERC1155PresetMinterPauser: must have minter role to mint");
        require(to.length == amount.length, "to.length must be equal to id.length");
        require(amount.length == data.length, "amount.length must be equal to data.length");

        for(uint i = 0 ; i < to.length; i++)
        {
            mint(to[i],0, amount[i], data[i]);
        }
        
        emit BatchMintedEngPoints(to, amount);
    }
    ...
    function upgradeSNFT(
        ...
        emit sNFTUpgraded(_composableId, _upgradeToTierId, _uri, _msgSender());
        return true;
    }
 ...
 */
contract ERC1155PresetMinterPauser is AccessControlEnumerable, ERC1155Burnable, ERC1155Pausable {
    ...
    function uri(uint256 tokenId, uint composableId) internal virtual view returns (string memory) {
        string memory tokenURI = _tokenURIs[composableId][tokenId];
     ...
     */
    function _setURI(uint256 tokenId, uint256 composableId, string memory tokenURI) internal virtual {
        _tokenURIs[composableId][tokenId] = tokenURI;
