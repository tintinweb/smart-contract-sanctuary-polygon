...
contract LostChainEntityData is CustomERC2771ContextUpgradeable, UUPSUpgradeable {
    ...
    struct Hero {
        ...
        uint256 entityDataId;
        string metadata;
        string minBaseHealth;
        string maxBaseHealth;
        string minBaseDefense;
        string maxBaseDefense;
        string minBaseDamage;
        string maxBaseDamage;
        string minBaseAttackSpeed;
        string maxBaseAttackSpeed;
        string minBaseCriticalChance;
        string maxBaseCriticalChance;
    }
    mapping(uint256 => Hero) heroEntities;
    CountersUpgradeable.Counter private _heroIdCounter;

    function addWeapon(
        string memory minLevel,
        ...
        require(bytes(minLevel).length > 0, 'LCED: Min level cannot be empty');
        ...
        require(!weaponEntities[weaponId].exists, 'LCED: Weapon already exists with that id');

        weaponEntities[weaponId].exists = true;
        weaponEntities[weaponId].entityDataId = weaponId;

        weaponEntities[weaponId].minLevel = minLevel;
    ...
    function getAllWeaponEntites() public view returns (Weapon[] memory) {
        ...
        require(maxWeaponId > 0, 'LCED: No weapon available');

        Weapon[] memory tempRet = new Weapon[](maxWeaponId);
        uint256 totalIndex = 0;
        for (uint256 i = 0; i < maxWeaponId; i++) {
            Weapon storage weapon = weaponEntities[i];
            if (!weapon.disabled) {
                tempRet[totalIndex] = weapon;
                totalIndex++;
            }
        }

        // Only return non empty index
        require(totalIndex > 0, 'LCED: No weapon available');
        Weapon[] memory ret = new Weapon[](totalIndex);
    ...
    function getWeapon(uint256 weaponId) public view returns (Weapon memory) {
        require(weaponEntities[weaponId].exists, 'LCED: Weapon does not exist');
        require(!weaponEntities[weaponId].disabled, 'LCED: Weapon is removed');

        return weaponEntities[weaponId];
    }

    function disableWeapon(uint256 weaponId) public onlyRole('GAME_MANAGER', _msgSender()) {
        require(weaponEntities[weaponId].exists, 'LCED: Weapon does not exist');
        require(!weaponEntities[weaponId].disabled, 'LCED: Weapon is already disabled');

        weaponEntities[weaponId].disabled = true;
    }

    function enableWeapon(uint256 weaponId) public onlyRole('GAME_MANAGER', _msgSender()) {
        require(weaponEntities[weaponId].exists, 'LCED: Weapon does not exist');
        require(weaponEntities[weaponId].disabled, 'LCED: Weapon is not disabled');

        weaponEntities[weaponId].disabled = false;
    }
    ...
    function addArmor(
        ...
        require(!armorEntites[armorId].exists, 'LCED: Armor already exists with that id');

        armorEntites[armorId].exists = true;
        armorEntites[armorId].entityDataId = armorId;

        armorEntites[armorId].minLevel = minLevel;
    ...
    function getAllArmorEntities() public view returns (Armor[] memory) {
        ...
        Armor[] memory tempRet = new Armor[](maxArmorId);
        uint256 totalIndex = 0;
        for (uint256 i = 0; i < maxArmorId; i++) {
            Armor storage armor = armorEntites[i];
            if (!armor.disabled) {
                tempRet[totalIndex] = armor;
                totalIndex++;
            }
        }

        // Only return non empty index
        require(totalIndex > 0, 'LCED: No armor available');
        Armor[] memory ret = new Armor[](totalIndex);
        for (uint256 i = 0; i < totalIndex; i++) {
            ret[i] = tempRet[i];
        }
    ...
    function getArmor(uint256 armorId) public view returns (Armor memory) {
        require(armorEntites[armorId].exists, 'LCED: Armor does not exist');
        require(!armorEntites[armorId].disabled, 'LCED: Armor is disabled');

        return armorEntites[armorId];
    }

    function disableArmor(uint256 armorId) public onlyRole('GAME_MANAGER', _msgSender()) {
        require(armorEntites[armorId].exists, 'LCED: Armor does not exist');
        require(!armorEntites[armorId].disabled, 'LCED: Armor is already disabled');

        armorEntites[armorId].disabled = true;
    }

    function enableArmor(uint256 armorId) public onlyRole('GAME_MANAGER', _msgSender()) {
        require(armorEntites[armorId].exists, 'LCED: Armor does not exist');
        require(armorEntites[armorId].disabled, 'LCED: Armor is not disabled');

        armorEntites[armorId].disabled = false;
    }

    function addHero(
        string memory minBaseHealth,
        string memory maxBaseHealth,
        string memory minBaseDefense,
        string memory maxBaseDefense,
        string memory minBaseDamage,
        string memory maxBaseDamage,
        string memory minBaseAttackSpeed,
        string memory maxBaseAttackSpeed,
        string memory minBaseCriticalChance,
        string memory maxBaseCriticalChance,
        string memory metadata
    ) public onlyRole('GAME_MANAGER', _msgSender()) {
        require(bytes(minBaseHealth).length > 0, 'LCED: Min base health cannot be empty');
        require(bytes(maxBaseHealth).length > 0, 'LCED: Max base health cannot be empty');
        require(bytes(minBaseDefense).length > 0, 'LCED: Min damage cannot be empty');
        require(bytes(maxBaseDefense).length > 0, 'LCED: Max damage cannot be empty');
        require(bytes(minBaseDamage).length > 0, 'LCED: Min attack speed cannot be empty');
        require(bytes(maxBaseDamage).length > 0, 'LCED: Max attack speed cannot be empty');
        require(bytes(minBaseAttackSpeed).length > 0, 'LCED: Min critical chance cannot be empty');
        require(bytes(maxBaseAttackSpeed).length > 0, 'LCED: Max critical chance cannot be empty');
        require(bytes(minBaseCriticalChance).length > 0, 'LCED: Min critical chance cannot be empty');
        require(bytes(maxBaseCriticalChance).length > 0, 'LCED: Max critical chance cannot be empty');
        require(bytes(metadata).length > 0, 'LCED: Metadata link cannot be empty');

        uint256 heroId = _heroIdCounter.current();
        _heroIdCounter.increment();
        require(!heroEntities[heroId].exists, 'LCED: Hero already exists with that id');

        heroEntities[heroId].exists = true;
        heroEntities[heroId].entityDataId = heroId;

        heroEntities[heroId].minBaseHealth = minBaseHealth;
        heroEntities[heroId].maxBaseHealth = maxBaseHealth;
        heroEntities[heroId].minBaseDefense = minBaseDefense;
        heroEntities[heroId].maxBaseDefense = maxBaseDefense;
        heroEntities[heroId].minBaseDamage = minBaseDamage;
        heroEntities[heroId].maxBaseDamage = maxBaseDamage;
        heroEntities[heroId].minBaseAttackSpeed = minBaseAttackSpeed;
        heroEntities[heroId].maxBaseAttackSpeed = maxBaseAttackSpeed;
        heroEntities[heroId].minBaseCriticalChance = minBaseCriticalChance;
        heroEntities[heroId].maxBaseCriticalChance = maxBaseCriticalChance;
        heroEntities[heroId].metadata = metadata;
    }

    function getAllHeroEntities() public view returns (Hero[] memory) {
        uint256 maxHeroId = _heroIdCounter.current();
        require(maxHeroId > 0, 'LCED: No hero available');

        Hero[] memory tempRet = new Hero[](maxHeroId);
        uint256 totalIndex = 0;
        for (uint256 i = 0; i < maxHeroId; i++) {
            Hero storage hero = heroEntities[i];
            if (!hero.disabled) {
                tempRet[totalIndex] = hero;
                totalIndex++;
            }
        }

        // Only return non empty index
        require(totalIndex > 0, 'LCED: No hero available');
        Hero[] memory ret = new Hero[](totalIndex);
        for (uint256 i = 0; i < totalIndex; i++) {
            ret[i] = tempRet[i];
        }
    ...
    function getHero(uint256 heroId) public view returns (Hero memory) {
        require(heroEntities[heroId].exists, 'LCED: Hero does not exist');
        require(!heroEntities[heroId].disabled, 'LCED: Hero is disabled');

        return heroEntities[heroId];
    }

    function disableHero(uint256 heroId) public onlyRole('GAME_MANAGER', _msgSender()) {
        require(heroEntities[heroId].exists, 'LCED: Hero does not exist');
        require(!heroEntities[heroId].disabled, 'LCED: Hero is already disabled');

        heroEntities[heroId].disabled = true;
    }

    function enableHero(uint256 heroId) public onlyRole('GAME_MANAGER', _msgSender()) {
        require(heroEntities[heroId].exists, 'LCED: Hero does not exist');
        require(heroEntities[heroId].disabled, 'LCED: Hero is not disabled');

        heroEntities[heroId].disabled = false;
    }

    // TEST FUNCTION TO ALTER BYTECODE (RE-VERIFY)
    function TESTING58() external pure returns (bool) {
        return true;
    }
}
