...
contract DigitalAssetsLending is AccessControl {
    ...
    struct LoanDetail {
        ...
        uint256 loanDuration;
        address loanPaymentAddress;
        uint256 qty;
        uint256 loanInterestForDuration;
    }

    constructor(uint256 _buyerFee, uint256 _sellerFee) {
        buyerFee = _buyerFee;
    ...
    function verifyLenderSign(address lender, address borrower, address nftAddress, uint256 tokenId, uint256 amount, uint256 qty, Sign memory sign)
        internal
        view
    {
    ...
    function initiateLend(Order memory order, Sign memory sign)
        external
        returns(bool)
    {
        ...
        uint256 interest = price * order.interestRateDuration / 1000;
        ...
        emit Lend(id, msg.sender, order.lender, order.nftAddress, order.tokenId, assetPrice, platformFee);
        return true;
    }

    function loanRepayment(uint256 _loanId, Sign memory sign)
        external
        returns(bool)
    {
        require(!usedNonce[sign.nonce],"Nonce: Invalid Nonce");
        ...
        require(block.timestamp <= loanMaturityDate, "loan overdued");
        uint256 timeDiff = block.timestamp - loans[_loanId].loanStartTime;
        uint256 interestDue = calculateInterest(
                                loans[_loanId].loanPrincipalAmount,
                                loans[_loanId].loanRepaymentAmount,
                                timeDiff,
                                loans[_loanId].loanDuration,
                                loans[_loanId].loanInterestForDuration
                            );
        uint256 amount = loans[_loanId].loanPrincipalAmount + interestDue;
        if(amount > 0) {
            IERC20(loans[_loanId].loanPaymentAddress).transferFrom(
        ...
        if(loans[_loanId].nftType == AssetType.ERC721) {
            IERC721(loans[_loanId].nftAddress).safeTransferFrom(
                address(this),
                loans[_loanId].borrower,
                loans[_loanId].tokenId
            );
        }
        if(loans[_loanId].nftType == AssetType.ERC1155) {
            IERC1155(loans[_loanId].nftAddress).safeTransferFrom(
                address(this),
                loans[_loanId].borrower,
                loans[_loanId].tokenId,
                loans[_loanId].qty,
        ...
        totalActiveLoans -= 1;
        emit LoanRepayment(_loanId, loans[_loanId].borrower, loans[_loanId].lender, loans[_loanId].nftAddress, loans[_loanId].tokenId, interestDue, loans[_loanId].loanPrincipalAmount);
        delete loans[_loanId];
        return true;
    }

    function loanOverdue(uint256 _loanId, Sign memory sign)
        external
        returns(bool)
    {
        require(!usedNonce[sign.nonce],"Nonce: Invalid Nonce");
        ...
        loanIdStatus[_loanId] = false;
        loanRepaidOrLiquidated[_loanId] = true;
        emit LoanOverdued(_loanId, loans[_loanId].borrower, loans[_loanId].lender, loans[_loanId].nftAddress, loans[_loanId].tokenId);
        return true;
    }

    function calculateInterest(uint256 amount, uint256 repaymentAmount, uint256 timeDiff, uint256 loanDuration, uint256 interestRate)
        internal
        pure
        returns(uint256)
    {
    ...
    function getInterest(uint256 _loanId)
        external
    ...
    function setBuyerFee(uint256 _buyerFee)
        external
        onlyRole(ADMIN_ROLE)
        returns(bool)
    {
    ...
    function setSellerFee(uint256 _sellerFee)
        external
        onlyRole(ADMIN_ROLE)
        returns(bool)
    {
    ...
    function getUserDetails(uint256 _loanId)
        external
        view
