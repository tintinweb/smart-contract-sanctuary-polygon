...
contract swapHelperDiyTask is OpsTaskCreator, Ownable {
    ...
    event CounterTaskCreated(uint256 _taskAmount, string _taskId, taskConfig _taskConfig);
    event swapCotE(address _tx_origin, address _msg_sender, uint256 _gasUsed, uint256 _spendSwapInToken, uint256 _timestamp, address _user, string _md5, string _taskID);
    ...
    function createTask(IAocoRouter02 _routerAddress, address[] memory _swapRouter, address[] memory _swapRouter2, uint256 _swapAmount, uint256 _interval, uint256 _salt) external payable {
        ...
        md5TaskList[md5] = id;
        taskConfig memory k = taskConfig(_routerAddress, _swapRouter, _swapRouter2, _swapAmount, _interval, _salt, 0, string(abi.encodePacked(md5)));
        taskConfigList[id] = k;
        emit CounterTaskCreated(taskAmount, string(abi.encodePacked(id)), k);
    ...
    function swapToken(address _user, bytes32 _md5) external onlyDedicatedMsgSender {
        ...
        if (x._swapIn == 0) {
            ...
            emit swapCotE(tx.origin, msg.sender, x.gasUsed, x.spendSwapInToken, block.timestamp, _user, string(abi.encodePacked(_md5)), string(abi.encodePacked(md5TaskList[_md5])));
