...
///      Allows owner to pause/unpause purchases, mint, withdraw funds, change expiry and royalty info
    ...
    IMemberships
{
    ...
    /// @notice Emitted when initialized by other than registered factory contract
    error Unauthorized();

    /// @notice Emitted when incorrect msg.value is passed during purcha
    ...
    /// @notice Emitted during extendExpiry() when new expiry is not greater than current expiry
    error InvalidExpiry();
    ...
    event MembershipPurchased(uint256 indexed tokenId, address indexed recipient);

    /// @notice Emitted when `tokenId` is minted by `owner` for `recipient`
    /// @param tokenId tokenId of membership minted
    /// @param recipient recipient of the membership
    event MembershipMinted(uint256 indexed tokenId, address indexed recipient);
    ...
    event Withdrawal(uint256 amount, address indexed treasury, uint256 fee, address feeTreasury);

    /// @notice Emitted when expiry is changed by owner
    /// @param oldExpiry old expiry timestamp
    /// @param newExpiry new expiry timestamp
    event ExpiryExtended(uint256 oldExpiry, uint256 newExpiry);
    ...
    /// @notice Membership expiry timestamp in seconds
    ...
    /// @notice Total Memberships minted
    uint256 public totalSupply;

    /// @dev Only factory can initialize
    ...
    /// @param _membership membership parameters including price, expiry, cap, airdropToken, airdropAmount
    function initialize(
        address _owner,
    ...
    /// @notice Pause Membership purchases
    /// @dev Callable only by `owner`
    function pause() external onlyOwner {
        _pause();
    }

    /// @notice Unpause Membership purchases
    /// @dev Callable only by `owner`
    ...
    function purchase(address recipient) external payable whenNotPaused returns (uint256) {
    ...
    function mint(address recipient) external onlyOwner returns (uint256) {
        uint256 tokenId = _mintMembership(recipient);

        _sendAirdropTokens(recipient);

        emit MembershipMinted(tokenId, recipient);
    ...
    /// @notice Change expiry of membership from `expiry` to `_newExpiry`
    /// @dev Callable only by `owner`.
    /// @param _newExpiry new expiry to set
    function extendExpiry(uint256 _newExpiry) external onlyOwner {
        if (!(_newExpiry > expiry)) revert InvalidExpiry();

        emit ExpiryExtended(expiry, _newExpiry);
    ...
    function _sendEther(address payable recipient_, uint256 amount) internal {
        ...
        (bool success, ) = recipient_.call{ value: amount }(""); // solhint-disable avoid-low-level-calls
        if (!success) revert TransferFailed();
    }

    /// @dev Mints membership and emits MembershipPurchased event
    /// @return tokenId of the minted membership
    function _purchase(address recipient) internal returns (uint256) {
        ...
        emit MembershipPurchased(tokenId, recipient);

        return tokenId;
    }

    /// @dev Mints membership by checking totalSupply against `cap`.
    ///      Reverts if all memberships have already been minted
    /// @return tokenId of the minted membership
    function _mintMembership(address recipient) internal returns (uint256) {
        uint256 tokenId = ++totalSupply;

        // if cap != 0, memberships is capped
        if (cap != 0)
            if (tokenId > cap)
    ...
    /// @dev Safe transfers `airdropAmount` of `airdropToken` to recipient,
    ///      iff `airdropToken` and `airdropAmount` are set.
    function _sendAirdropTokens(address recipient) internal {
...
interface IMemberships {
    struct Membership {
        uint256 price; // price
        uint256 expiry; // expiry timestamp in seconds
        uint256 cap; // total number of memberships. cap = 0 for unlimited supply
