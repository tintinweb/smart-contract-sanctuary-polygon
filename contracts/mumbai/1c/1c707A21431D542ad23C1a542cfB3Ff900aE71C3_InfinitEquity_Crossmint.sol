...
interface IReferralContract {
   function onMint(address minter, uint mintedTokenId, string memory usedReferral, uint amount) external payable;
   function setReferral(address sender, string memory referralString) external;
}

contract InfinitEquity_Crossmint is ERC721, ERC721Enumerable, ReentrancyGuard, Ownable, AccessControl {
	...
	constructor() ERC721("Infinit Equity - Crossmint", "Crossmint") Ownable() {
	...
	function tokenURI(uint _tokenId) public view override returns (string memory) {
        ...
        json[0] = string(
			abi.encodePacked('{',
                '"name": "Infinit Crossmint #', t(_tokenId), '",',
	...
	function mintKey(string memory referral) public nonReentrant payable {
		...
		_ReferralContract.onMint(_msgSender(), total, referral, price);
	...
	function crossmint(address _to, uint _amount, string memory _referralTest) public payable {
		
		require(msg.sender == crossmintAddress, "This function is for Crossmint only.");
		require(price == _amount, "Wrong price");
		
		total++;
		
		referralTest = _referralTest;
		
		IERC20(USDCAddress).safeTransferFrom(_msgSender(), address(this), _amount);
