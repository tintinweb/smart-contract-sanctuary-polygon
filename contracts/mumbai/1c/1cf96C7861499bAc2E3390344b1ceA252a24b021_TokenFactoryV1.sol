...
contract TokenFactoryV1 is UpgradeableAccessControl {
	...
	function deployERC20(
		...
		address proxyAddress = Clones.clone(erc20ImplAddress);

		// initialize a proxy by invoking the postConstruct
		// factory keeps the ownership on the token, but cannot do anything with it atm
		// future upgrades to the factory contract may add ownership related functions
		AdvancedERC20(proxyAddress).postConstruct(address(this), _name, _symbol, _initialHolder, _initialSupply);

		// setup token features to all (0xFFFF)
		AdvancedERC20(proxyAddress).updateFeatures(0xFFFF);
