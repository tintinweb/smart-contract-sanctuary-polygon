...
contract MainController is MainControllerStorageV1, MainControllerInterface, Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    function getUpline(address account) external override view returns (address)
    {
    	return uplineDict[account];
    }

    function getUplineFee() external override view returns (uint)
    {
    	return first_ref_fee;
    }

    function getUpline2Fee() external override view returns (uint)
    {
    ...
    function setUpline(address userAddress, address uplineAddress) external override 
    {
    	require(msg.sender == admin || matchInfos[msg.sender].isUsed, "only match contract can update this dictionary");
    ...
    // set team name
    function setTeamName(uint8 _teamNo, string memory  _teamName) override external
    {
    	require(msg.sender == admin, "only admin can set team name");
	teamNamesDict[_teamNo] = _teamName;
    }
