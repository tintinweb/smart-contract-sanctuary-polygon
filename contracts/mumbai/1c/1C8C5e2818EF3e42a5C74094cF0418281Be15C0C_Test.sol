...
contract Test{    
    ...
    event MatrixBonus(address indexed receiver,  uint256 _amount, uint8 level);
    ...
    function registration(address userAddress, address referrerAddress, uint256 id) private{
        ...
        for(uint8 i=0;i<7;i++)
        {
            if(ref!=address(0)) 
            {
                ...
                emit LevelBonus(ref, msg.sender, (((levelUSD*REFERRAL_PERCENTS[i])/1000)*1e18)/maticRate, i+1);
                ref=users[ref].referrer;
            }
            else
            break;
    ...
    function updateX6Referrer(address userAddress, address referrerAddress, uint8 level, uint256 amount) private{
        ...
        {          
          users[referrerAddress].holdAmount[level]+=amount;
        ...
        if(!users[_user].activeX3Levels[level])
        {
            ...
            for(uint8 i=0;i<7;i++)
            {
                if(referrerAddress!=address(0)) 
                {
                    ...
                    emit LevelBonus(referrerAddress, msg.sender, ((levelUSD*(REFERRAL_PERCENTS[i+1]/1000))/1e20)*maticPerDollor, i+1);
    ...
    function autoUpgrade(address _user, uint8 level) private{
             ...
             emit MatrixBonus(_user, userAmount, (level-1));
