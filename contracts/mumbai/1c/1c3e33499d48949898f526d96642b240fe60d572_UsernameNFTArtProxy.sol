...
contract UsernameNFTArtProxy is IUsernameArtProxy, OwnableUpgradeable {
    ...
    function _getImageSVG(string memory _username) public pure returns (string memory) {
        return string(
            abi.encodePacked(
                '"image": "data:image/svg+xml;base64,', Base64.encode(
                    abi.encodePacked(
                        '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: green; font-family: serif; font-size: 14px; }</style><rect width="100%" height="100%" fill="red" />',
