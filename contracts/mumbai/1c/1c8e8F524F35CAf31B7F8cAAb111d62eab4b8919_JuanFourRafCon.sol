...
contract JuanFourRafCon is VRFConsumerBaseV2, ReentrancyGuard {
    ...
    event TransferReceived(uint room_number,address _from, uint _amount, uint playersLength,address[] s_players);
    ...
    event NewLease(uint room_number,address lessee,uint newEntryfee,uint pot,uint limit,bool active,uint grandPrize,uint firstPrize, uint secondPrize,uint leaseType); // 0 = standard, 1 = sponsored
    event ExtendLease(uint room_number,uint time);
    event BurnedToken(uint room_number, uint amount, uint time);
    event RoomMarketState(uint room_number, uint activity_type); // activity_types , 0 = sell room, 1 = buy room, 2 = void selling
    event LoadRoom(uint room_number, uint playAmount);
    ...
    function join(uint entryFee, uint room_number) public nonReentrant {
        ...
        // if(raffle_room[room_number].s_player_wallets[msg.sender] == 0){
        //     raffle_room[room_number].s_player_wallets[msg.sender] = 0;
        // }
        emit TransferReceived(room_number,msg.sender, entryFee, raffle_room[room_number].s_players.length,raffle_room[room_number].s_players);
    ...
    function hostARaffle(uint newEntryfee, uint room_number, uint grandPrize,uint limit) public {
        ...
        emit NewLease(room_number,msg.sender,newEntryfee,raffle_room[room_number].pot,limit,true,raffle_room[room_number].prizes[0],raffle_room[room_number].prizes[1],raffle_room[room_number].prizes[2],0);
    ...
    function loadSlots(uint room_number,uint amount) public{
        ...
        uint playAmount = amount/100000000000000000000;
        raffle_room[room_number].playingSlot = raffle_room[room_number].playingSlot + playAmount;
        emit LoadRoom(room_number,playAmount);
    }
    ...
    function sellRaffleRoom(uint room_number, uint price) public{
        ...
        raffle_room[room_number].active = false;
        emit RoomMarketState(room_number,0);
    }
    ...
    function voidRoomSelling(uint room_number) public{
        ...
        raffle_room[room_number].active = true;
        emit RoomMarketState(room_number,2);
    }
