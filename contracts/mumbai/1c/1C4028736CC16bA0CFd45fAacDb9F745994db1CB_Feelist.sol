...
contract Feelist is ERC20, ERC20Burnable, Pausable, Ownable {
    ...
    function setTier(
        ...
        tiers[id].fixedFeeVolume = fixedFeeVolume;
        tiers[id].fixedFee = fixedFee;
        uint256 newdynamicId = tiers[id].dynamicId++;
        tierVolumes[id][newdynamicId].tierVolume = tierVolume;
        tierVolumes[id][newdynamicId].tierPercentage = tierPercentage;
    }
    ...
    function addTierVolume(
        ...
        require (tierVolume > tierVolumes[id][dynamicId].tierVolume, "The Volume must be higher than the last setted");
        uint256 newdynamicId = tiers[id].dynamicId++;
        tierVolumes[id][newdynamicId].tierVolume = tierVolume;
        tierVolumes[id][newdynamicId].tierPercentage = tierPercentage;
    }
    ...
    function setFlyDiscount(
        ...
        require(tiers[id].id == id, "id does not exist");

        tiers[id].flyWindow = flyWindow;
        tiers[id].minFlyamount = minFlyamount;
        tiers[id].percentageFly = percentageFly;
    ...
    function setSpecialFee(
        ...
        specialFee storage specialfees = specialFees[wallets];
    ...
    function getTierVolume(uint256 _id) public view returns (TierVolume[] memory) {
        uint256 tierVolumeqty = tiers[_id].dynamicId;
        TierVolume[] memory tierVolumesArr = new TierVolume[](tierVolumeqty);
        
        for (uint256 i = 0; i < tierVolumeqty; i++) {
            tierVolumesArr[i] = TierVolume(tierVolumes[_id][i].tierVolume, tierVolumes[_id][i].tierPercentage);
        }
    ...
    function getTierPercentage(uint256 _id, uint256 _tierVolume) public view returns (uint256) {
        uint256 highestVolume = 0;
        uint256 highestPercentage = 0;
        uint256 tierVolumeqty = tiers[_id].dynamicId;
        
        for (uint256 i = 0; i < tierVolumeqty; i++) {
            if (_tierVolume <= tierVolumes[_id][i].tierVolume) {
                return tierVolumes[_id][i].tierPercentage;
            }
            else if (tierVolumes[_id][i].tierVolume > highestVolume) {
                highestVolume = tierVolumes[_id][i].tierVolume;
                highestPercentage = tierVolumes[_id][i].tierPercentage;
            }
        }
    ...
    function tokenTransfer(address to, uint256 amount) public {
        uint256 idTier = walletTiers[msg.sender].id;
        ...
        else if (amount <= walletTiers[msg.sender].fixedFeeVolume) {
            transferFrom(msg.sender, feeVault, walletTiers[msg.sender].fixedFee);
        }else{
            uint256 feePayment = getPortion(amount, getTierPercentage(idTier, amount));
