...
import "./interfaces/ICoGResources.sol";
...
contract BaseLogic is Initializable, OwnableUpgradeable, IBaseLogic {
    using Resources for Resources.ID;
    using Items for Items.ID;
    ILandManager public landManager;
    ICoGResources public gameResource;
    IGameController public gameController;
    address[] public resourceLogic;
    ...
    modifier onlyGameContract() {
        ...
        for (uint256 i = 0; i < resourceLogic.length; ++i) {
            if (msg.sender == resourceLogic[i]) {
                valid = true;
            }
        }
        if (msg.sender == address(gameController)) {
            valid = true;
        }
        require(valid, "Invalid caller");
        _;
    }
    ...
    function initialize(
        ...
        gameResource = _resources;
    }

    function setController(address _controller) public onlyOwner {
        gameController = IGameController(_controller);
    }

    function setLandManager(address _manager) public onlyOwner {
        landManager = ILandManager(_manager);
    }

    function setResource(address _resource) public onlyOwner {
        gameResource = ICoGResources(_resource);
    }

    function setLogic(address[] memory _logic) public onlyOwner {
        resourceLogic = _logic;
    }
    ...
    function s4_grow(
        ...
        landManager.removeResource(
            id,
            uint256(Resources.ID.S4),
            gameController.getIndex(id, position)
        );
        uint256 rand;
        uint256 resourceId;
        // Grow to Rare or better res
        if (landManager.isItemExist(id, uint256(Items.ID.IF1))) {
            uint256 length_rare = gameResource.categoryLength("rare_fruits");
            uint256 length_very_rare = gameResource.categoryLength(
                "very_rare_fruits"
            );
            rand =
                ((random / 100 ** position) % 400) %
                (length_rare + length_very_rare);
            if (rand < length_rare) {
                resourceId = gameResource.at("rare_fruits", rand);
            } else {
                resourceId = gameResource.at("very_rare_fruits", rand);
            }
        } else if (landManager.isItemExist(id, uint256(Items.ID.IC11))) {
            uint256 length_uncommon = gameResource.categoryLength(
                "uncommon_fruits"
            );
            uint256 length_rare = gameResource.categoryLength("rare_fruits");
            uint256 length_very_rare = gameResource.categoryLength(
            ...
            rand =
                ((random / 100 ** (position + 3)) % 900) %
                (length_uncommon + length_rare + length_very_rare);
            if (rand < length_uncommon) {
                resourceId = gameResource.at("uncommon_fruits", rand);
            } else if (
                rand > length_uncommon && rand < length_uncommon + length_rare
            ) {
                resourceId = gameResource.at("rare_fruits", rand);
            } else {
                resourceId = gameResource.at("very_rare_fruits", rand);
            ...
            rand =
                ((random / 100 ** position) % 100) %
                gameResource.categoryLength("fruits");
            resourceId = gameResource.at("fruits", rand);
        }
        _addNewResource(id, resourceId, 1);
        uint256[] memory positions = new uint256[](0);
        positions[0] = position;
        emit Synergy(id, Effect.CHANGE, positions);
    }
    ...
    function boostPayout(
        ...
        return _boostPayout(id, resourceId, position, addedValue);
    }
    ...
    function addNewResource(
        ...
        _addNewResource(id, resourceId, quantity);
    }

    function checkAllAdjacent(
        uint256 id,
        uint16 position
    ) external override onlyGameContract returns (bool) {
        return _checkAllAdjacent(id, position);
    }
    ...
    function arrow(
        ...
        uint8 sort,
        uint256 randomNumber
    ) external override onlyGameContract returns (uint16[] memory destroyPos) {
        return _arrow(id, uint8(position), sort, randomNumber);
    }
    ...
    function _arrow(
        ...
        uint8 countDestroy;
        uint8 time = 1;
        if (landManager.isItemExist(_id, uint256(Items.ID.IQ3))) {
            time = 2;
        }
        uint8 prevDirection = 8;
        while (time-- != 0) {
            uint8 direction = uint8(
                (randomNumber / (1e20 * 10 ** position)) % 8
            ); // 0 up, 1 down, 2 left, 3 right, 4 up-left, 5 up-right, 6 down-left, 7 down-right
            while (prevDirection == direction) {
                direction = uint8(
                    (direction + (randomNumber / (1e40 * 10 ** position))) % 8
                );
            }
            prevDirection = direction;
            uint16[] memory listPositionPointedTo = Helper.getArrowPointed(
                position,
                direction
            );

            for (uint8 i = 0; i < listPositionPointedTo.length; ++i) {
                uint16 positionPointedTo = listPositionPointedTo[i];
                Resources.ID resourcePointed = Resources.findResource(
                    gameController.atPosition(_id, positionPointedTo)
                );
                if (resourcePointed == Resources.ID.T1) {
                    destroyPos[countDestroy] = positionPointedTo;
                    ++countDestroy;
                } else {
                    if (sort == 1) {
                        gameController.updatePayout(
                            _id,
                            positionPointedTo,
                            2,
                            0,
                ...
                if (
                    resourcePointed == Resources.ID.A1 ||
                    resourcePointed == Resources.ID.P2
                ) {
                    _boostPayout(
                        _id,
                        uint16(resourcePointed),
                        positionPointedTo,
                        1
                    );
                }
    ...
    function _doll(
        ...
        uint16 position,
        uint256 value
    ) public override onlyGameContract returns (uint16[] memory destroyPos) {
        uint256 countSpin = gameController.getProp(id, position);
        ++countSpin;
        if (
            (countSpin == value - 2 &&
                landManager.isItemExist(id, uint256(Items.ID.IT4))) ||
            countSpin == value
        ) {
            destroyPos[0] = position;
            if (value == 3) {
                _addNewResource(id, uint256(Resources.ID.M5), 1);
            } else if (value == 5) {
                _addNewResource(id, uint256(Resources.ID.M6), 1);
            } else if (value == 7) {
                _addNewResource(id, uint256(Resources.ID.M7), 1);
            } else if (value == 9) {
                _addNewResource(id, uint256(Resources.ID.M8), 1);
            }
            uint256[] memory positions = new uint256[](0);
            positions[0] = position;
            emit Synergy(id, Effect.CHANGE, positions);
        ...
        gameController.updateProp(id, position, countSpin);
    }
    ...
    function _rollDice(
        ...
        uint256 random;
        if (landManager.isItemExist(id, uint256(Items.ID.IL9))) {
            random = typeDice;
        } else
            random =
                ((((randomNumber / 10000) * position) % 200) % typeDice) +
                1;
        if (random == 1) {
            uint16[] memory posGambler = gameController.findPosition(
                id,
                uint256(Resources.ID.G1)
            );
            for (uint256 i = 0; i < posGambler.length; ++i) {
                destroyPos[i] = posGambler[i];
            }
        }
        gameController.updatePayout(id, position, 0, 0, int256(random));
    }
    ...
    function _suits(
        ...
        uint16[] memory adjacentPosition = Helper.getAdjacentPositions(
            position,
            _checkAllAdjacent(id, position)
        );
        int256 coin;
        bool isItemA1Exist = landManager.isItemExist(id, uint256(Items.ID.IA1));
        for (uint8 i = 0; i < adjacentPosition.length; ++i) {
            ...
            Resources.ID adjResource = Resources.findResource(
                gameController.atPosition(id, adjPosition)
            );
            if (
                (sort == 1 &&
                    (adjResource == Resources.ID.C10 ||
                        adjResource == Resources.ID.S9)) ||
                (sort == 2 &&
                    (adjResource == Resources.ID.D3 ||
                        adjResource == Resources.ID.H1))
            ) {
                coin += 1;
            } else if (
                ((isItemA1Exist &&
        ...
        uint256 count = gameController.countResource(
            id,
            uint256(Resources.ID.C10)
        );
        count += gameController.countResource(id, uint256(Resources.ID.S9));
        count += gameController.countResource(id, uint256(Resources.ID.H1));
        count += gameController.countResource(id, uint256(Resources.ID.D3));

        if (count > 2) {
            coin += 1;
        }
        gameController.updatePayout(id, position, 0, 0, coin);
    }

    function _checkAllAdjacent(
        uint256 id,
        uint256 resPos
    ...
    function _boostPayout(
        ...
        gameController.updatePayout(id, position, 0, 0, 1);
        ILandManager.SpecialResource memory resource = landManager
            .getASpecialResource(id, resourceId, index);

        landManager.updateSpecialResource(
            id,
            resourceId,
            index,
            resource.payout + addedValue,
            resource.prop
        );
    }

    function _addNewResource(
        uint256 id,
        uint256 resourceId,
        uint256 quantity
    ) internal {
        gameController.updateNewAddedResource(id);
        landManager.addResource(id, resourceId, quantity, 0, 0);
    }
}
