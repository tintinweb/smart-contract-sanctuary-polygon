...
interface INFTFactory {
    ...
    event TokenMinted(uint256 collectionId, uint256 tokenId);
    event TransactionProcessed(uint256 transactionId);
...
contract NFT is INFT, Ownable, ERC721URIStorage {
    // solhint-disable-next-line no-empty-blocks
    constructor(string memory name_, string memory symbol_) ERC721(name_, symbol_) {}

    function mint(address recipient, uint256 tokenId, string calldata tokenURI) external onlyOwner {
...
error SignerNotVerified(address caller, bytes32 hash, bytes signature);
    ...
    PullPaymentUpgradeable
{
    ...
    event NativeTokenSet(bytes32 nativeToken);
    ...
    function initialize(address verifier_, uint8 feeCountLimit_, bytes32 nativeToken_) external initializer {
    ...
    function createCollection(
        ...
        bytes32 hash = _createCollectionHash(transactionId, collectionId, name, symbol, itemLimit, fees, msg.sender);
        bool verified = verifier.verifySigner(hash, signature);
        if (!verified) {
            revert SignerNotVerified(msg.sender, hash, signature);
        }

        _createCollection(collectionId, name, symbol, itemLimit);
        _transferFees(fees, msg.sender);
    ...
    function mintItem(
        ...
        _setItemSupply(collectionId, deduplicationId, maxItemSupply);
        _mintItem(collectionId, recipient, tokenId, tokenURI);
        _transferFees(fees, msg.sender);

        emit TokenMinted(collectionId, tokenId);
        emit TransactionProcessed(transactionId);
    }
    ...
    function computeCollectionAddress(
        ...
        bytes32 bytecodeHash = keccak256(abi.encodePacked(type(NFT).creationCode, abi.encode(name, symbol)));
    ...
    function createCollectionHash(
        ...
        return _createCollectionHash(transactionId, collectionId, name, symbol, itemLimit, fees, sender);
    }
    ...
    function _createCollection(
        ...
        NFT nft = new NFT{ salt: bytes32(collectionId) }(name, symbol);
    ...
    function _mintItem(uint256 collectionId, address recipient, uint256 tokenId, string calldata tokenURI) private {
        ...
        INFT(collection.nft).mint(recipient, tokenId, tokenURI);
        collection.itemCount.increment();
    }

    function _transferFees(Fee[] calldata fees, address beneficiary) private {
        ...
        if (msg.value > totalFeeAmount) {
            _asyncTransfer(beneficiary, msg.value - totalFeeAmount);
        }
    }
    ...
    function _createCollectionHash(
        ...
        return
            keccak256(
                abi.encodePacked(transactionId, collectionId, name, symbol, itemLimit, _encodeFees(fees), sender)
            );
    }
    ...
    function _encodeFees(Fee[] calldata fees) private pure returns (bytes memory encodedFees) {
        uint256 feeCount = fees.length;
