...
import {IOpenverseNFTExchange} from "./interfaces/IOpenverseNFTExchange.sol";
...
contract OpenverseNFTExchange is IOpenverseNFTExchange, ReentrancyGuard, Ownable {
    using SafeERC20 for IERC20;

    using OrderTypes for OrderTypes.MakerOrder;
    using OrderTypes for OrderTypes.TakerOrder;

    address public immutable ETH = 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE;  // default native token address expression
    ...
    constructor(
        ...
        DOMAIN_SEPARATOR = keccak256(
            abi.encode(
                ...
                0x24dd4310babf66e2fb14c70b19a54132010aaf7f497239269917c41df219312e, // keccak256("OpenverseNFTExchange")
    ...
    /**
     * @notice Match ask with a taker bid order using partial ETH and partial WETH
    ...
    /**
     * @notice Match ask with a taker bid order using ETH only
     * @param takerBid taker bid order
     * @param makerAsk maker ask order
     */
    function matchAskWithTakerBidUsingETH(
        ...
        require((makerAsk.isOrderAsk) && (!takerBid.isOrderAsk), "Order: Wrong sides");
        require(makerAsk.currency == ETH, "Order: Currency must be ETH");
        require(msg.sender == takerBid.taker, "Order: Taker must be the sender");
        require(takerBid.price == msg.value, "Order: Msg.value too high");
    ...
    /**
     * @notice Transfer fees and funds to royalty recipient, protocol, and seller by using ETH only
     * @param strategy address of the execution strategy
     * @param collection non fungible token address for the transfer
     * @param tokenId tokenId
     * @param to seller's recipient
    ...
    function _transferFeesAndFundsWithETH(
        ...
        // 1. Protocol fee
        {
            ...
            if ((protocolFeeRecipient != address(0)) && (protocolFeeAmount != 0)) {
                ...
                require(success, "_transferFeesAndFundsWithETH: transfer ETH not successful");
                finalSellerAmount -= protocolFeeAmount;
            }
        }
        ...
        // 2. Royalty fee
        {
            ...
            if ((royaltyFeeRecipient != address(0)) && (royaltyFeeAmount != 0)) {
                ...
                emit RoyaltyPayment(collection, tokenId, royaltyFeeRecipient, address(ETH), royaltyFeeAmount);
            }
        }
        ...
        // 3. Transfer final amount (post-fees) to seller
        {
            (bool success, ) = to.call{value: finalSellerAmount}("");
            require(success, "_transferFeesAndFundsWithETH: transfer ETH not successful");
        }
    }
...
/**
 * @title OrderTypes
 * @notice This library contains order types for the exchange.
 */
library OrderTypes {
...
interface IOpenverseNFTExchange {
    function matchAskWithTakerBidUsingETHAndWETH(
        OrderTypes.TakerOrder calldata takerBid,
        OrderTypes.MakerOrder calldata makerAsk
    ) external payable;
