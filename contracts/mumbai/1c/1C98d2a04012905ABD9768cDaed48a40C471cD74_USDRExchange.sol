...
contract USDRExchange is IExchange, Ownable, Pausable {
        ...
        returns (uint256)
    {
        (
            ...
            address tngbl,
            uint256 amountOut
        ) = _prepareTNGBLWithdrawal(amountIn, to);
        ...
        returns (uint256)
    {
        (
            ...
            address underlying,
            uint256 amountOut
        ) = _prepareUnderlyingWithdrawal(amountIn, to);
    ...
    function updateUnderlying() external onlyOwner {
        (address usdr, address underlying) = abi.decode(
            addressProvider.getAddresses(
                abi.encode(USDR_ADDRESS, UNDERLYING_ADDRESS)
            ),
    ...
    function _checkTNGBLToUSDRMarketCapRatio() private {
        (
            address treasury,
            address staking
        ) = abi.decode(
    ...
    function _prepareTNGBLWithdrawal(uint256 amountIn, address to)
        private
        returns (
            address treasury,
            address tngbl,
    ...
    function _preparePromissoryWithdrawal(uint256 amountIn, address to)
        private
    ...
    function _prepareUnderlyingWithdrawal(uint256 amountIn, address to)
        private
        returns (
            address treasury,
            address underlying,
