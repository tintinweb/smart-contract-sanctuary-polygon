...
contract MockOneInch is Context {
    ...
    function token2ReceiveAmount(uint256 token1Amount) public view returns (uint256) {
        uint256 token1Balance = IERC20(token1).balanceOf(address(this));
        uint256 token2Balance = IERC20(token2).balanceOf(address(this));
        if (token2Balance == 0) {
            return token1Amount;
        }
        uint256 denominator = (token1Amount + token1Balance);
        return ((token2Balance * (denominator - token1Balance)) / denominator);
    }

    function token1ReceiveAmount(uint256 token2Amount) public view returns (uint256) {
        uint256 token2Balance = IERC20(token2).balanceOf(address(this));
        uint256 token1Balance = IERC20(token1).balanceOf(address(this));
        if (token1Balance == 0) {
            return token2Amount;
        }
        uint256 denominator = (token2Amount + token2Balance);
        return ((token1Balance * (denominator - token2Balance)) / denominator);
    }

    function depositToken1(uint256 token1Amount) public {
        IERC20(token1).transferFrom(_msgSender(), address(this), token1Amount);
    }

    function depositToken2(uint256 token2Amount) public {
        IERC20(token2).transferFrom(_msgSender(), address(this), token2Amount);
    }
    ...
    function swap(
        ...
        require(desc.srcToken == token1 || desc.srcToken == token2, "Unsupported fromToken");
        require(desc.dstToken == token1 || desc.dstToken == token2, "Unsupported toToken");
        require(desc.srcToken != desc.dstToken, "Can't swap token for itself");

        uint256 toTokenAmount = 0;
        if (desc.srcToken == token1) {
            toTokenAmount = token2ReceiveAmount(desc.amount);
        } else {
            toTokenAmount = token1ReceiveAmount(desc.amount);
        }
        console.log("toTokenAmount", toTokenAmount);

        IERC20(desc.srcToken).transferFrom(_msgSender(), address(this), desc.amount);
        IERC20(desc.dstToken).transfer(desc.dstReceiver, toTokenAmount);
...
import {IERC20} from "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import {Context} from "@openzeppelin/contracts/utils/Context.sol";
import {MockOneInch, SwapDescription} from "./MockOneInch.sol";
...
contract OneInchAdapter is Context {
    ...
    function swap(
        ...
        uint256 amount = desc.srcToken.allowance(_msgSender(), address(this));
        desc.srcToken.transferFrom(_msgSender(), address(this), amount);
