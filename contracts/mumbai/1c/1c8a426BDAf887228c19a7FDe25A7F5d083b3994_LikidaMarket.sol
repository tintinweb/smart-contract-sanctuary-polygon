...
contract LikidaMarket is IERC721Receiver, ERC1155Holder, Pausable, AccessControl, ReentrancyGuard {
  ...
  function changeState(uint256 offerId, bool bidding) external whenNotPaused validItem(offerId) isSeller(offerId) {
    require(_idToMarketItem[offerId].price != 0, "LKMKT: Price must be greater");
    require(_idToMarketItem[offerId].bidder == address(0), "LKMKT: Shoudn't have a bidder");
    _idToMarketItem[offerId].inBidding = bidding;
  }
  ...
  function makeOffer(uint256 offerId) external payable whenNotPaused validItem(offerId) inBidding(true, offerId, msg.value) {
    _idToMarketItem[offerId].bidder = msg.sender;
    _idToMarketItem[offerId].price = msg.value;
    emit OfferMade(msg.sender, offerId, msg.value);
  }
  ...
  function cancelOffer(uint256 offerId) external nonReentrant whenNotPaused validItem(offerId) inBidding(false,offerId,0) {
    ...
    if(_idToMarketItem[offerId].seller == msg.sender || _idToMarketItem[offerId].bidder == msg.sender){
      uint256 price = _idToMarketItem[offerId].price;
      address bidder = _idToMarketItem[offerId].bidder;
      
      _idToMarketItem[offerId].price = 0;
      _idToMarketItem[offerId].bidder = address(0);
  ...
  function acceptOffer(uint256 offerId, bytes memory data) external nonReentrant whenNotPaused validItem(offerId) inBidding(false,offerId,0) isSeller(offerId) {
    ...
    uint256 toSeller = offer.price;
    uint256 likidaFee = calculatePercentage(toSeller, fees);
    uint256 royaltyFee = _royaltyPay(offer.token.contractAddress, offer.token.tokenId, toSeller);

    payable(feeReceiver).transfer(likidaFee);
    payable(offer.seller).transfer(toSeller - likidaFee - royaltyFee);
   ...
   *!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! @dev Change to uint[]
   */
  function getAllMarketItems() external view returns(MarketStorage.MarketItem[] memory) {
    uint256 itemCount       = _tokenIdCounter.current();
    MarketStorage.MarketItem[] memory items = new MarketStorage.MarketItem[](itemCount);
