...
contract NftFactory is Ownable, ERC721, ProxyTarget {
    ...
    IWhitelist whitelistChecker;
    IERC20 public paymentsToken;

    function initialize(address _gameAddress, address _tokenAddress,address _whitelistChecker,address _metadata,address _vrf, address _paymentsToken) external {
        ...
        whitelistChecker = IWhitelist(_whitelistChecker);
    ...
    // function setContract(address _gameAddress, address _tokenAddress, address _whitelistChecker, address _metadata,address _vrf, address _paymentsToken) external onlyOwner {
    //     game = IGameEngine(_gameAddress);
    //     SUP = IERC20(_tokenAddress);
    //     whitelistChecker = IWhitelist(_whitelistChecker);
    //     metadataHandler = IMetadata(_metadata);
    ...
    function presaleBuyAndStake(bool stake,uint8 tokenType, uint tokenAmount,bytes memory signature) external {
    ...
    // available at this link: https://hungerbrainz.com/terms 
        ...
        require(isPresale == true, "Not presale");
        require(userPurchase[msg.sender] + tokenAmount <= 3,"Purchase limit");
        require(whitelistChecker.isValidSignature(msg.sender, signature) == true, "Invalid signature");
        paymentsToken.transferFrom(msg.sender, address(this), salePrice*tokenAmount);
        ...
        userPurchase[msg.sender] += tokenAmount;
        if(stake) {
    ...
    function buyAndStake(bool stake,uint8 tokenType, uint tokenAmount) external {
    //  By calling this function, you agreed that you have read and accepted the terms & conditions
    // available at this link: https://hungerbrainz.com/terms 
        require(tokenType < 2,"Invalid type");
        require(tokenId_.current() <= SUP_THRESHOLD,"Buy using SUP");
        require(isPresale == false, "Now is presale");
...
interface IWhitelist{
    function isValidSignature(address user, bytes memory signature) external view returns(bool);
}
