...
contract testContract is Ownable, KeeperCompatibleInterface {
    ...
    function CreateNewProgram(
        uint256 fundGoal,
        // uint256 fees,
        string calldata programDataCID // uint256 votesRequired, // uint256 votingTime,
    ) external payable // uint256 RaisingFundsTime,
    // uint256 currentFunds,
    // uint256 CurrentVotes,
    // uint256 Rejects
    {
    ...
    function getAllUserVotes(bytes32 programId) external returns (UserVote) {
        return program_Adress_ToVotes[programId][msg.sender];
    }

    function getAllUserDonations(bytes32 programId) external returns (uint256) {
