...
abstract contract Protected is Ownable, EIP712 {
    ...
    bytes32 private constant _ORDER_TYPEHASH =
        keccak256(
            "OrderInfo(address tokenAddress,uint256 tokenId,uint256 totalTrading,address currency,uint96 feeRate,uint256 price)"
        );
    bytes32 internal constant _MINT_TYPEHASH =
        // prettier-ignore
        keccak256(
            "BuyWithMintParams(address seller,uint256 listingId,OrderInfo order,uint256 startTime,uint256 endTime,uint256 totalSupply,string meta,address royaltyReceiver,uint96 royalty)"
            "OrderInfo(address tokenAddress,uint256 tokenId,uint256 totalTrading,address currency,uint96 feeRate,uint256 price)"
        );
    bytes32 internal constant _BUY_TYPEHASH =
        // prettier-ignore
        keccak256(
            "BuyParams(address seller,uint256 listingId,OrderInfo order,uint256 startTime,uint256 endTime)"
            "OrderInfo(address tokenAddress,uint256 tokenId,uint256 totalTrading,address currency,uint96 feeRate,uint256 price)"
        );
    bytes32 internal constant _MINT_OFFER_TYPEHASH =
        // prettier-ignore
        keccak256(
            "SellWithMintParams(address buyer,uint256 offerId,OrderInfo order,uint256 endTime,uint256 totalSupply,string meta,address royaltyReceiver,uint96 royalty)"
            "OrderInfo(address tokenAddress,uint256 tokenId,uint256 totalTrading,address currency,uint96 feeRate,uint256 price)"
        );
    bytes32 internal constant _BUY_OFFER_TYPEHASH =
        // prettier-ignore
        keccak256(
            "SellParams(address buyer,uint256 offerId,OrderInfo order,uint256 endTime)"
            "OrderInfo(address tokenAddress,uint256 tokenId,uint256 totalTrading,address currency,uint96 feeRate,uint256 price)"
        );
    ...
    /// @param mintData Mint params
    /// @param buyerSig Signature of seller
    /// @return True, if signature signer matches the seller
    ...
    /// @notice Сheck the signature of the platform (Direct Sale & Auction Mint)
    ...
    function _getMintHashStruct(
        ...
        bytes32 orderHashStruct = _getOrderHashStruct(mintData.order);
        return
            keccak256(
                abi.encode(
                    _MINT_TYPEHASH,
    ...
    function _getBuyHashStruct(
        ...
        bytes32 orderHashStruct = _getOrderHashStruct(buyData.order);
        return
            keccak256(
                abi.encode(
                    _BUY_TYPEHASH,
...
contract Marketplace is Protected, Pausable {
    ...
    function buyWithMint(
        ...
        uint256 payment = mintData.order.price * platformData.editionsToBuy;
        _checkMoney(_msgSender(), mintData.order.currency, payment);
        ...
        uint256 fee = (payment * mintData.order.feeRate) / FEE_DENOMINATOR;

        if (
            !IMintable(mintData.order.tokenAddress).exists(
                mintData.order.tokenId
            )
        ) {
            _mint(
                mintData.seller,
                mintData.order.tokenAddress,
                mintData.order.tokenId,
                mintData.totalSupply,
                mintData.meta,
                mintData.royaltyReceiver,
    ...
    function buy(
        ...
        uint256 payment = buyData.order.price * platformData.editionsToBuy;
        _checkMoney(_msgSender(), buyData.order.currency, payment);
        ...
        uint256 fee = (payment * buyData.order.feeRate) / FEE_DENOMINATOR;
        address royaltyReceiver;
        uint256 royalty;
        ...
        _transfer(
            buyData.order.tokenAddress,
            buyData.seller,
            platformData.receiver,
            buyData.order.tokenId,
            platformData.editionsToBuy
        );
        ...
        if (
            IERC165(buyData.order.tokenAddress).supportsInterface(
                type(IERC2981).interfaceId
            )
        ) {
            (royaltyReceiver, royalty) = IERC2981(buyData.order.tokenAddress)
                .royaltyInfo(buyData.order.tokenId, payment);
        } else {
    ...
    function sellWithMint(
        ...
        uint256 payment = mintData.order.price * platformData.editionsToSell;
        _checkMoney(mintData.buyer, mintData.order.currency, payment);
    ...
    function sell(
        ...
        uint256 payment = buyData.order.price * platformData.editionsToSell;

        _checkMoney(buyData.buyer, buyData.order.currency, payment);
        if (!_checkPlatformOfferSignature(platformData, platformSignature))
            revert WrongPlatformSignature();
    ...
    function _cancelAuction(
        ...
        if (IVault(vault).isBidExist(buyData.listingId))
            IVault(vault).refundBid(buyData.listingId, buyData.order.currency);
    ...
    function _acceptBidWithMint(
        ...
        if (mintData.order.totalTrading != 1) revert TokenIsNotUnique();
        ...
        if (IVault(vault).isBidExist(mintData.listingId)) {
            ...
            uint256 bidFee = (bidPrice * mintData.order.feeRate) /
                FEE_DENOMINATOR;
    ...
    function _acceptBid(
        ...
        if (buyData.order.totalTrading != 1) revert TokenIsNotUnique();
        ...
        if (IVault(vault).isBidExist(buyData.listingId)) {
            ...
            uint256 bidFee = (bidPrice * buyData.order.feeRate) /
                FEE_DENOMINATOR;
            address royaltyReceiver;
            uint256 royalty;
            ...
            if (
                ...
                (royaltyReceiver, royalty) = IERC2981(
                    buyData.order.tokenAddress
                ).royaltyInfo(buyData.order.tokenId, bidPrice);
            } else {
                (royaltyReceiver, royalty) = IRoyalties(royalties).royaltyInfo(
                    buyData.order.tokenAddress,
                    buyData.order.price
    ...
    function mintFree(
        ...
        _mint(receiver, token, id, editions, meta, royaltyReceiver, royalty);
    }
    ...
    function withdrawNative(address payable receiver) external onlyOwner {
