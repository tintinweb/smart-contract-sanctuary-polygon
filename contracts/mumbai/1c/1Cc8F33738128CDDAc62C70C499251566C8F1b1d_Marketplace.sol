...
contract Marketplace {
    ...
    // Mapping from seller address to list of their item IDs
    ...
    function listMarketItem(
        ...
        // Add the item to the list of the seller's items
    ...
    function unlistMarketItem(uint256 itemId) public {
        ...
        // Ensuring the item is not already sold
        require(marketItems[itemId].isSold == false, "Item is already sold");

        // Ensuring the item is owned by the user attempting to unlist it
        require(marketItems[itemId].seller == msg.sender, "Only the seller can unlist the item");

        // Unlisting the item
        marketItems[itemId].isPresent = false;

        // Transferring the NFT back to the seller
    ...
    function getSellerItems(address seller) 
        ...
        returns (MarketItem[] memory items) 
    {
        uint256[] memory itemIds = sellerItems[seller];
        items = new MarketItem[](itemIds.length);

        for (uint256 i = 0; i < itemIds.length; i++) {
            if(marketItems[itemIds[i]].isSold == false && marketItems[itemIds[i]].isPresent == true) {
