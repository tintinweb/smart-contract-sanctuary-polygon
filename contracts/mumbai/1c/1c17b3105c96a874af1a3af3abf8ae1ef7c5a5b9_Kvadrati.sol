...
contract Kvadrati is ERC721, ERC721Enumerable, Ownable {
    using SafeMath for uint256;

    uint256 public constant MAX_ALLOW_LIST_MINT = 2;
    uint256 public constant MAX_TO_MINT=1;
    ...
    constructor() ERC721("Kvadrati", "Kv") {}
    ...
    function mint(uint256 numTokens) external payable {
        ...
        require(numTokens <= MAX_SUPPLY, "Invalid number of tokens");
        require(ts.add(numTokens) <= MAX_SUPPLY, "Purchase would exceed max tokens");
        require(msg.value >= MINT_PRICE.mul(numTokens), "Ether value sent is below the required price");
