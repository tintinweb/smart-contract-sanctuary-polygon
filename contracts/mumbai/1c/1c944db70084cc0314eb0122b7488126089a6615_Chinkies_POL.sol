...
// File: contracts/tokenomemenft_eth.sol
...
contract Chinkies_POL is ERC721, Ownable, ReentrancyGuard, ContextMixin {
    using Strings for uint256;
    ...
    uint256 public _price = 60000000000000000; // 0.06 ETH

    Counters.Counter private _tokenIds;

    constructor(string memory uri, address _proxyRegistryAddress)
        ERC721("Tokenomemenft", "TKMNFT")
    ...
    function publicSaleMint(uint256 _amount) external payable onlyAccounts {
        require(publicM, "Chinkies_POL: PublicSale is OFF");
        require(!paused, "Chinkies_POL: Contract is paused");
        require(_amount > 0, "Chinkies_POL: zero amount");

        uint256 current = _tokenIds.current();

        require(
            current + _amount <= maxSupply,
            "Chinkies_POL: Max supply exceeded"
        );
        require(
            _price * _amount <= msg.value,
            "Chinkies_POL: Not enough ethers sent"
        );
