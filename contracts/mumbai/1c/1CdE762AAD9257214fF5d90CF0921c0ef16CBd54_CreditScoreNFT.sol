...
contract CreditScoreNFT is ERC721, ReentrancyGuard {
    ...
    uint256 private constant nftPrice = 0.5 ether;

    constructor() ERC721("CreditScoreNFT", "CSNFT") {}
    ...
    function buyCreditScoreNFT() external payable nonReentrant {
        ...
        CreditScoreData memory newCreditScore = CreditScoreData(initialCreditScore, msg.sender, block.timestamp);
    ...
    function revokeApproval(address _contract) external {
    ...
    function updateCreditScore(uint256 tokenId, int256 scoreChange) external {
        ...
        uint256 timeSinceLastUpdate = block.timestamp - creditScoreData.lastUpdated;

        // Check if at least 1 day (86400 seconds) has passed since the last update
        require(timeSinceLastUpdate >= 86400, "Credit score can only be updated once a day");
    ...
    function getCreditScoreByAddress(address walletAddress) public view returns (uint256 creditScore, uint256 lastUpdated) {
        ...
        require(tokenId != 0, "User does not own a CreditScoreNFT");
