 ...
 * @author mario
 */
contract Hivemind is HivemindV1Storage, HivemindI, ErrorReporter, ExponentialNoError, Ownable {
   ...
   */
  function exitMarket(HTokenI _hToken) external override {
  ...
  function borrowAllowed(
    ...
    if (borrowGuardianPaused[_hToken]) revert Paused();
  ...
  function transferAllowed(
    ...
    _dst;
    // Pausing is a very serious situation - we revert to sound the alarms
    if (transferGuardianPaused) revert Paused();

    // Currently the only consideration is whether or not
    //  the src is allowed to redeem this many tokens
    redeemAllowed(_hToken, _src, _transferTokens);
  }
  ...
  function getHypotheticalAccountLiquidity(
    ...
    if (_redeemTokens > 0)
      (liquidity, shortfall) = getHypotheticalAccountLiquidityRedeemInternal(_account, _hToken, _redeemTokens);
    else if (_collateralId > 0)
      (liquidity, shortfall) = getHypotheticalAccountLiquidityBorrowInternal(_account, _hToken, _collateralId, _borrowAmount);
  }
   ...
   * @param _hToken The market to hypothetically redeem/borrow in
   * @param _account The account to determine liquidity for
   * @param _collateralId The 721 Collateral that is used to compute the price
   * @param _borrowAmount The amount of underlying to hypothetically borrow
  ...
  function getHypotheticalAccountLiquidityBorrowInternal(
    ...
    uint256 sumCollateral = oracle.getUnderlyingNFTPrice(_collateralId, _hToken.decimals());

    // Read the balances and exchange rate from the hToken
    (, uint256 borrowBalance, ) = _hToken.getAccountSnapshot(_account);
    ...
    // console.log("collateralFactorUint ", collateralFactorUint);
  ...
  function getHypotheticalAccountLiquidityRedeemInternal(
    ...
    for (i; i < assets.length; i++) {
      ...
      uint256 tokensToDenom = (markets[asset].collateralFactorMantissa * exchangeRateMantissa * oraclePriceMantissa) /
        _hToken.decimals();
   ...
   */
  function _setPauseGuardian(address _newPauseGuardian) public {
    if (msg.sender != owner()) revert Unauthorized();
  ...
  function _setBorrowPaused(HTokenI _hToken, bool _state) public returns (bool) {
    if (!markets[_hToken].isListed) revert MarketError(Error.MARKET_ALREADY_LISTED);
    if (msg.sender != pauseGuardian && msg.sender != owner()) revert Unauthorized();
    if (msg.sender == pauseGuardian && !_state) revert Unauthorized();
...
interface PriceOracleI {
  ...
  function getUnderlyingNFTPrice(uint256 _tokenId, uint256 _decimals) external view returns (uint256);
