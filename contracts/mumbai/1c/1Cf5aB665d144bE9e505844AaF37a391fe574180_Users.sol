...
function create_user(address _user_address ,uint256 _user_point,uint256 _user_stock_price  ) 
        ...
        user memory _user = user( _user_address,_user_point);

        user_id[_user_address] =users.length+1;
...
function number_of_INA_stocks  (address _user ) public  returns (uint256){
        require(user_id[_user]>=0,"Account did not Exist  ");
        uint256 user_ID =get_userID(_user);
        uint256 _user_points = users[user_ID].user_point  ;
                 //uint256 _number_of_INA = (_user_points - user_last_point[_user])/500 ;
