...
interface IPool {
    function transferFunds(address _to, uint _amount ) external;
...
contract distributorContract is ReentrancyGuardUpgradeable,OwnableUpgradeable {
        ...
        function deposit (uint poolId,uint tokenAmount) public nonReentrant {
                ...
                if (poolId == 1) {
                        insuranceAmount = treasury *1/100;
               ...
               emit Deposit(msg.sender, collectionPools[poolId],total, poolId,currentIndexOfPool);
        }
        ...
        function claimFunds (uint[] memory poolIds, uint[] memory _nftIds) external nonReentrant {
                ...
                for (uint i=0; i< poolIds.length; i++) {
                        ...
                        if(poolIds[i]==1){
                            ...
                            //   token.transfer(msg.sender,insure);
                        }else{
      ...
      function investBack(uint poolId, uint _nftId,uint PoolIdToInvest) external{
          uint amount= getRewardDetails(poolId, _nftId);
          require(amount>0,"Not Enough Rewards");
          require(detailsPerNftId[_nftId][poolId]._currentOwner==msg.sender,"not a NFT Owner");
            deposit(PoolIdToInvest, amount);
        }
        ...
        function unstakeTokens (uint[] memory poolIds, uint[] memory _nftIds) external nonReentrant {
                ...
                for (uint i=0; i< poolIds.length; i++) {
                        ...
                        if(poolIds[i]==1){
                           ...
                           IPool(collectionPools[poolId]).transferFunds(insuranceFundAddress,insurance); 
                        }
                        ...
                        IPool(collectionPools[poolId]).transferFunds(msg.sender,finalAmount);
