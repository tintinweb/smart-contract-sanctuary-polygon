...
contract CancelPolicyManager is
    SwixContract,
    ReentrancyGuard
{
    ...
    /// Describes different cancellation policy options
    struct Policy {
        /// Guest can get full refund until how many days before check-in
        uint256 fullRefundUntil;
        /// Guest can get full refund in this much time after booking
        uint256 fullRefundIn;
        /// Last moment which will allow fullRefundIn
        uint256 fullRefundInDeadline;
        /// Guest can get half refund until how many days before check-in
        uint256 halfRefundUntil;
        /// Guest can get half refund in this much time after booking
        uint256 halfRefundIn;
        /// Last moment which will allow halfRefundIn
        uint256 halfRefundInDeadline;
    }
    ...
    /// Array of all active cancellation policies within Swix system
    ...
    /// add Policy
    /// @param newPolicy - the new policy to add to policies 
    function addPolicy(Policy calldata newPolicy)
        external
        nonReentrant
        onlyLeaseManager
    {
    ...
    /// retrieve cancelTime based on policyIndex and start timestamp
    /// @param policyIndex - index of the cancelPolicy chosen
    /// @param start - the checkin timestamp
    /// @return fullRefundUntil - absolute fullRefundUntil timestamp
    /// @return halfRefundUntil - absolute halfRefundUntil timestamp
    function getCancelTimes(uint256 policyIndex, uint256 start)
        external
        view
        returns (uint256 fullRefundUntil, uint256 halfRefundUntil)
    {
        /// Set the return variables to default of 0
        fullRefundUntil = 0;
        halfRefundUntil = 0;

        // If policy is NON-REFUNDABLE, return (0,0)
        if (policyIndex == 0) return (fullRefundUntil, halfRefundUntil);

        // Get chosen policy
        Policy memory policy = policies[policyIndex];
        // Get current timestamp
        uint256 timestamp = block.timestamp;

        // If `fullRefundUntil` is set for a chosen policy
        if (policy.fullRefundUntil > 0) {
            // If now is before `fullRefundUntil` deadline for chosen policy
            if (timestamp < start - policy.fullRefundUntil) {
                // Set `fullRefundUntil` as full refund deadline for chosen policy
                fullRefundUntil = start - policy.fullRefundUntil;
            }
        }

        // If `fullRefundInDeadline` is set for a chosen policy
        if (policy.fullRefundInDeadline > 0) {
            // If now is before `fullRefundIn` deadline for chosen policy
            if (timestamp + policy.fullRefundIn < start - policy.fullRefundInDeadline) {
                // Check if current `fullRefundUntil` is earlier that the deadline from `fullRefundIn` policy
                if (fullRefundUntil < timestamp + policy.fullRefundIn) {
                    // Set the more favourable deadline
                    fullRefundUntil = timestamp + policy.fullRefundIn;
                }
            }
        }

        // If `halfRefundUntil` is set for a chosen policy
        if (policy.halfRefundUntil > 0) {
            // If now is before `halfRefundUntil` deadline for chosen policy
            if (timestamp < start - policy.halfRefundUntil) {
                // Set `halfRefundUntil` as half refund deadline for chosen policy
                halfRefundUntil = start - policy.halfRefundUntil;
            }
        }

        // If `halfRefundInDeadline` is set for a chosen policy
        if (policy.halfRefundInDeadline > 0) {
            // If now is before `halfRefundIn` deadline for chosen policy
            if (timestamp + policy.halfRefundIn < start - policy.halfRefundInDeadline) {
                // Check if current `halfRefundUntil` is earlier that the deadline from `halfRefundIn` policy
                if (halfRefundUntil < timestamp + policy.halfRefundIn) {
                    // Set the more favourable deadline
                    halfRefundUntil = timestamp + policy.halfRefundIn;
                }
            }
        }
    }    
