...
contract Ewallet {
    ...
    modifier lesserThanMiniPoolProfitTracker(uint256 amount) {
     ...
     function getTeamIDByName(string memory _name) public view returns(uint256){
      ...
      function getVoucherIDByVoucherCode(string memory _voucherCode) public view returns(uint256){
        uint256 localVoucherCodeID;
        for(uint256 i = 1; i <= voucherCodeCount; i++) {
            if(keccak256(abi.encodePacked(voucherCodeIDToVoucherCode[i])) == keccak256(abi.encodePacked(_voucherCode))) {
                localVoucherCodeID = i;
                break;
            }
        }

        return localVoucherCodeID;
     }
