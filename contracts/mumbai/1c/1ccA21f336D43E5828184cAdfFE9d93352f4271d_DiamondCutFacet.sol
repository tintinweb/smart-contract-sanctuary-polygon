...
/**
 * @title DiamondCutFacet - A contract implementing the diamond cut function
 * @dev This facet is responsible for adding, updating, and removing functions in a diamond.
 * It also allows for an optional delegatecall to be executed as part of the diamond cut.
 */
contract DiamondCutFacet is IDiamondCut {
    /**
     * @notice Modify diamond functions by adding, replacing, or removing them and
     *         optionally execute a delegatecall on a specified contract or facet.
     *
     * @param diamondCut_ An array containing the facet addresses and function selectors
     * @param init_ The address of the contract or facet to execute calldata_ using delegatecall
     * @param calldata_ Encoded function call, including function selector and arguments,
     *                  to be executed using delegatecall on init_
     *
    ...
    function diamondCut(
        ...
        /// Check if the last selector slot is not full
        if (selectorCount & 7 > 0) {
            selectorSlot = ds.selectorSlots[selectorCount >> 3];
        }

        /// Iterate through the diamond cuts array
        ...
        /// Update selector count if it has some changes
        if (selectorCount != originalSelectorCount) {
            ds.selectorCount = uint16(selectorCount);
        }

        /// Update the last selector slot if it is not full
        if (selectorCount & 7 > 0) {
            ds.selectorSlots[selectorCount >> 3] = selectorSlot;
        }

        emit DiamondCut(diamondCut_, init_, calldata_);
        LibDiamond.initializeDiamondCut(init_, calldata_);
    }
}
