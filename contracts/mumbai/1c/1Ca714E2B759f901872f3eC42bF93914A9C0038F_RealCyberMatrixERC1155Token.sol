...
abstract contract ERC1155Base is HasSecondarySaleFees, Ownable, ERC1155Metadata_URI, HasContractURI, ERC1155 {
    ...
    function _mint(Fee[] memory _fees, uint256 _supply, string memory _uri) internal {
        require(
        ...
        // emit TransferSingle(msg.sender, address(0x0), msg.sender, _id, _supply); // this event emitted by ERC1155
        emit URI(_uri, _id);
    }
    ...
    function burn(address _owner, uint256 _id, uint256 _value) external {
        ...
        _amount[_id].sub(_value); // _burn method already checked the negative amount
    ...
    // return non-zero-amount tokenIds
    function getTokenIds() public view returns (uint256[] memory) {
        uint length = 0;
        for (uint i = 0; i < _tokenIds.length; i++) {
            if (_amount[_tokenIds[i]] > 0) {
                length++;
            }
        }
...
contract RealCyberMatrixERC1155Token is ERC1155Base, SignerRole {
    ...
    */
    ...
    function mint(bytes memory _signature, Fee[] memory _fees, uint256 _supply, string memory _uri) public {
        require(
            isSigner(
