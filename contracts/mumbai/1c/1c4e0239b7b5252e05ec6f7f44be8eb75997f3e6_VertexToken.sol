...
contract VertexToken is ERC20, SpenderRole {
    address payable public admin;
    uint256 private immutable _cap;
    uint256 public _base_price;

    constructor(uint256 cap_, uint256 base_price_)
        ERC20("VertexVerse Token", "VRTX")
    {
        require(cap_ > 0, "ERC20Capped: cap is 0");
        require(base_price_ > 0, "ERC20Base: Base Price cant be 0");
        _cap = cap_;
        _base_price = base_price_;
        _mint(msg.sender, 1000000 * 10**18);
        admin = payable(msg.sender);
    }

    function vrtxPrice() public view virtual returns (uint256) {
        return _base_price;
    }
    ...
    function buyVRTX() public payable {
        uint256 amountToMint = _base_price * msg.value;
        require(
            ERC20.totalSupply() + amountToMint <= maxSupply(),
