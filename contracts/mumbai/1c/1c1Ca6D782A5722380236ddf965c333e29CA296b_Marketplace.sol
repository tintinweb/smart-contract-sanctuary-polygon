...
contract Marketplace is Ownable {
	...
	*/    
    ...
    event ArtModified(bytes32 id, bytes32 providerID, string name, string description, string category, string uri, string metadataUri, uint16 totalSupply, uint256 price, uint256 dateCreated, bool isEnabled, bool isNew);
    ...
    */
    ...
    @param category The artwork description
    ...
    */
    ...
    function addEditArtwork(bytes32 id, string memory name, string memory description, string memory category, string memory uri, string memory metadataUri, uint256 price, uint16 totalSupply) notStopped public {
        require(bytes(name).length > 0, "The name can't be empty");
        require(bytes(description).length > 0, "The description can't be empty");
        ...
        if(id == 0) {
            ...
            Art memory art = Art(artID, providerID, name, description, category, uri, metadataUri, totalSupply, 0, price, block.timestamp, true);

            artsByProvider[providerID].push(artID);
            artByID[artID] = art;

            emit ArtModified(art.id, art.providerID, art.name, art.description, art.category, art.uri, art.metadataUri, art.totalSupply, art.price, art.dateCreated, art.isEnabled, true);
        } else {
            artID = id;
            ...
            emit ArtModified(art.id, art.providerID, art.name, art.description, art.category, art.uri, art.metadataUri, art.totalSupply, art.price, art.dateCreated, art.isEnabled, false);
        }
    }
	...
	*/
    function setIsArtEnabled(bytes32 id, bool value) public notStopped {
        ...
        emit ArtModified(a.id, a.providerID, a.name, a.description, a.category, a.uri, a.metadataUri, a.totalSupply, a.price, a.dateCreated, a.isEnabled, false);
    }
...
contract NFTToken is ERC721URIStorage, Ownable {
    ...
    constructor() Ownable() ERC721("Cellars NFT", "CNFT")  {
    }
