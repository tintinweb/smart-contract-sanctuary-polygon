...
//Function to start the claim
    function startClaim() public onlyOwner {
        ...
        require(payable(address(this)).balance>0);
        uint totalMatic = payable(address(this)).balance;
        eachMatic = totalMatic / 5000;
        claimEnabled = true;
...
//Claim the reward Manually
    function claimReward (uint256[] memory _nftId) public {

        uint totalClaim=0;

        for(uint i = 0 ; i<_nftId.length; i++) {
            address isOwner = IERC721(0xd4bBbbc27fDA78572477Ed068BAc747A42aFfafB).ownerOf(_nftId[i]);
            require(isOwner == msg.sender);
            require(_nftId[i]>=0 || _nftId[i]<=5000, 'Id must be between 0-5000');
            require(!claimedRewards[_nftId[i]], 'Already Claimed');

            

            totalClaim+=eachMatic;
            claimedRewards[_nftId[i]] = true;
