...
contract BCEvents is Ownable {
    ...
    function  C(uint _id) public view returns (BCEvent memory) {
...
contract BCGames is Ownable {
    ...
    function _hurtPlayer(uint _playerId, uint _gameId, uint _amount, bool _hazard) internal {
        ...
        if(currentPlayer.healthDmgTaken > int8(uint8(currentChar.traits.health))) {
            _respawnPlayer(_playerId, games[_gameId]);
        }
    }

    function _respawnPlayer(uint _playerId, BCTypes.Game storage _game) internal {
        BCTypes.Player storage currentPlayer = players[_playerId];
        ...
        _movePlayer(_game.mapId, StartLocation, _playerId);
    }

    function extApplyEffect(uint _playerId, uint _gameId, uint _eventId, uint _effectNum) public {
        require(msg.sender == actionsAddress, "Not actions contract");
        ...
        _applyEffect(_playerId, _gameId, effect);
    }

    function _applyEffect(uint _playerId, uint _gameId, BCEvents.BCEffect memory _effect) internal {
        ...
        if(_effect.placeHazard) {
            maps.extSetHazardStatus(games[_gameId].mapId, games[_gameId].eventPosition.row, games[_gameId].eventPosition.col, true);
        }
        ...
        if(_effect.hazardDamage > 0) {
            _hurtPlayer(_playerId, _gameId, _effect.hazardDamage, true);
        }
    ...
    function resolveEvent(uint _gameId, uint _choice) public {
        ...
        if(eventGame.eventIsTile) {
            currentEvent = bcEvents.extGetRoomEvent(eventGame.eventNumber);
        } else {
            // TODO:
            return;
            // currentEvent = bcEvents.extGetCardEvent(eventGame.eventNumber);
        }
        BCEvents.BCEffect[] memory currentEffects = bcEvents.extGetEffects(eventGame.eventIsTile, eventGame.eventNumber);
        ...
        if(currentEvent.rollForLow == 0 && currentEvent.rollForHigh == 0) {
            ...
            _applyEffect(eventGame.eventPlayerId, _gameId, currentEffects[0]);
        } else {
            uint roll = utils.roll();
            if(currentEvent.rollForLow != 0 && roll <= currentEvent.rollForLow) {
                // Rolled for lowEffect
                _applyEffect(eventGame.eventPlayerId, _gameId, currentEffects[1]);
            } else if (currentEvent.rollForHigh != 0 && roll >= currentEvent.rollForHigh){
                // Rolled for highEffect
                _applyEffect(eventGame.eventPlayerId, _gameId, currentEffects[2]);
            } else {
    ...
    function _endPlayerRound(uint _gameId, uint _mapId) internal {
        ...
        maps.extSpreadHazards(_mapId, 0);
        // TODO: Hazards should damage denizens
        for(uint i = 0; i <= currentGame.playerIndexes.length-1; i++) {
            BCTypes.Player storage currentPlayer = players[currentGame.playerIndexes[i]];
            ...
            if(currentGameTile.hasHazard) {
                _hurtPlayer(currentGame.playerIndexes[i], _gameId, 1, true); // TODO: Hardcoded
            }
        }
    }
    ...
    function _movePlayer(uint _gameId, BCTypes.Position memory _position, uint _playerId) internal {
        ...
        if(maps.extGetGameTile(games[_gameId].mapId, _position.row, _position.col).roomId == SPACE_ROOM) {
            _respawnPlayer(_playerId, games[_gameId]);
        }
