...
// File: CelebrationPassNFT_Polygon.sol
...
contract PWDcelebrationPass is Ownable, ERC721, ERC721Enumerable, BridgeNFT, MinterRole {

    string private _nft_meta_ipfs_folder = "bafybeibp2u67n4uqbfkwpncf3nmihes6ai72ogbhev76fbyi6uunp5uwju";
    uint private _maxTokenID = 301;

    constructor() ERC721("PWD Cel Pass Test Bridge 2", "TPASSB2") { }
    ...
    // Needed for the BridgeNFT interface and other bridges 
    ...
    function tokenURI(uint256 tokenId) override(ERC721) public view returns (string memory) {
        return super.tokenURI(tokenId);     // default of ERC721 implementation:  string(abi.encodePacked(baseURI, tokenId.toString()))
    }    


    // Needed for the BridgeNFT interface
	function mint(address to, uint256 id, bytes calldata) override external onlyMinter {
        require(id > 0 && id <= _maxTokenID, "Invalid id");
		_safeMint(to, id);
	}

    // Needed for the CrossChain bridge
    ...
    // Needed for the BridgeNFT interface
	function burnFor(address from, uint256 id) override external onlyMinter {
        require(ownerOf(id) == from, "You don't own this nft!");
		_burn(id);
	}

    // Needed for CrossChain bridge
