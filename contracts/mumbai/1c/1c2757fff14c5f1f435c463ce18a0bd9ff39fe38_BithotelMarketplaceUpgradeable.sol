    ...
    UUPSUpgradeable
{
    ...
    function addERC721Listing(
        ...
        require(
            hasRole(INCLUDED_COLLECTIONS, _msgSender()) || getIncludedCollectionIndex(erc721TokenAddress) != 0,
            "Not allowed to list"
        );
        address owner = owner_;
        address deployer = _msgSender();
        _beforeAddERC721Listing(erc721TokenAddress, erc721TokenId, priceInWei, deployer);
        ...
        _receiveERC721(erc721TokenAddress, deployer, _thisAddress, erc721TokenId);
    ...
    function executeERC721Listing(
        ...
        uint256 tax = ((listing.priceInWei * listingTax()) / _DIV); // 100%
        uint256 taxMarketplace = (tax / 5) * 3; // 60%
        uint256 taxCreator = (tax / 5) * 2; // 40%
        uint256 amountAfterTax = listing.priceInWei - tax;

        require(msg.value == listing.priceInWei, "Price doesn't match");

        payable(_wallet).transfer(taxMarketplace);
        payable(creator).transfer(taxCreator);

        if (physicalArt == true) {
            payable(_wallet).transfer(amountAfterTax);
            _sendERC721(listing.erc721TokenAddress, _thisAddress, _wallet, listing.erc721TokenId);
        } else {
    ...
    function _beforeExecuteERC721Listing(
        ...
        // solhint-disable-next-line reason-string
        require(buyer.balance > listing.priceInWei, "buyer has not enough balance");
        require(msg.value == listing.priceInWei, "wrong price");
    ...
    function _beforeCancelERC721Listing(uint256 mListingId, address mOwner) internal virtual {
        ...
        require(listing.seller == mOwner || hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "msg.sender is not seller");
    }
