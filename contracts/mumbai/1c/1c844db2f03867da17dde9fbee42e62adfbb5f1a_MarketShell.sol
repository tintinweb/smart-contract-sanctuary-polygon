...
contract MarketShell is AccessControlEnumerable, IMarket {
    ...
    string public constant VERSION = "1";
     ...
     constructor(address _paymentAccount_address, address _mys_address, address _advance_address, address _whitelist_address, address _itemCollection_address) {
    ...
    function updateAddresses(address paymentAccount, address mys_nft, address marketAdvance, address whitelist, address itemCollection) public onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function addToken(address _tokenAddress, uint256 _tokenId, uint256 _price, uint256 amount) public returns (uint256)
    {
        require(amount > 0, "Amount value must be > 0");
        if (IERC165(_tokenAddress).supportsInterface(type(IERC1155).interfaceId)) {
            return _add_erc_1155(_tokenAddress, _tokenId, _price, msg.sender, amount);

        } else if (IERC165(_tokenAddress).supportsInterface(type(IERC721).interfaceId)) {
            require(amount == 1, "ERC721: amount must be = 1");
            return _add_erc_721(_tokenAddress, _tokenId, _price, msg.sender);
        }
    ...
    function buy(uint256 itemId) public payable {
        ...
        _buy(item, msg.sender, 1, msg.value);
    }
    ...
    function _buy_MYS_ERC_1155(ItemInfo memory item, IERC1155 nft, uint256 _amount, uint256 _tokenCount, address _buyer) private {
        ...
        if (toUser > 0) {
            _mysteriousPaymentShareOut(item, SystemWideRoyalties_ERC1155(item.tokenAddress), toUser);
        }
    ...
    function _mysteriousPaymentShareOut(ItemInfo memory item, SystemWideRoyalties_ERC1155 nft, uint256 amount) private 
    {
        ...
        if (sellerReceiver == creatorReceiver) {
            _paymentShareOutBetweenMembers(item, nft, amount);
        } else {
            if (creatorRoyaltyAmount > 0) {
                _paymentShareOutBetweenMembers(item, nft, creatorRoyaltyAmount);
            }
    ...
    function _paymentShareOutBetweenMembers(ItemInfo memory item, SystemWideRoyalties_ERC1155 nft, uint256 amount) private 
    {
    ...
    function _removeItem(uint256 _itemId, address _sender) private {
     ...
     function _add_erc_1155(address _tokenAddress, uint256 _tokenId, uint256 _price, address sender, uint256 amount) private returns (uint256) {
        ...
        return _addItem(sender, _tokenAddress, _tokenId, _price, amount);
    }

    function _add_erc_721(address _tokenAddress, uint256 _tokenId, uint256 _price, address sender) private returns (uint256) {
        ...
        return _addItem(sender, _tokenAddress, _tokenId, _price, 1);
    }

    function _addItem(address sender, address _tokenAddress, uint256 _tokenId, uint256 _price, uint256 amount) private returns (uint256) {
        uint256 itemId = _itemCollection.addItem(sender, _tokenAddress, _tokenId, address(0x0), _price, amount);
