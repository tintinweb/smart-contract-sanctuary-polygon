...
contract BHeroS is Initializable, AccessControlUpgradeable, UUPSUpgradeable {
  ...
  function _calculateFusion(uint256[] calldata mainMaterials, uint256[] calldata buffMaterials)
    internal
    view
    returns (
      uint256,
...
contract BHeroToken is ERC721Upgradeable, AccessControlUpgradeable, PausableUpgradeable, UUPSUpgradeable {
  ...
  event FusionSuccess(address indexed to, uint256 countFusionSuccess, uint256 timestamp);
  ...
  function getPendingTokens(address to) external view returns (uint256, uint256) {
    uint256 totalHeros;
  ...
  function processTokenRequests() external returns (bool) {
    ...
    if (countFusionSuccess > 0) {
      emit FusionSuccess(to, countFusionSuccess, block.timestamp);
    }
    return true;
  }
