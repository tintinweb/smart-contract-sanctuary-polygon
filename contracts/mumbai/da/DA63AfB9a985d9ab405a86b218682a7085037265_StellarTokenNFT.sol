...
contract StellarTokenNFT is ERC721URIStorage {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        parts[1] = "    <defs><style>@import url('https://fonts.googleapis.com/css2?family=Press+Start+2P');.base{font-family : 'Press Start 2P', Roboto;font-size: 7px;fill: #fff;}</style></defs>";
        parts[2] = string(abi.encodePacked('<image href="', levelImages[tokenIdToMetadata[tokenId].level-1], '" height="200" width="200" />'));
        parts[3] = '    <text x="10" y="15" class="base big">#: ';
        parts[4] = toString(tokenId);
        parts[5] = '</text><text x="10" y="25" class="base">Level: ';
        parts[6] = toString(tokenIdToMetadata[tokenId].level);
        parts[7] = '</text><text x="50%" y="190" dominant-baseline="middle" text-anchor="middle" class="base">Rarity: ';
        parts[8] = tokenIdToMetadata[tokenId].rarity;
        parts[9] = '</text>';
        parts[10] = '</svg>';
