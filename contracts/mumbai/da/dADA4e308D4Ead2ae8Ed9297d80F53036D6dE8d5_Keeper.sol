...
import {SD59x18, sd} from "./../../margine/prb/SD59x18.sol";
...
contract Keeper is AutomationCompatibleInterface {
    ...
    mapping(uint256 => int256) _amounts;
    uint256 public _lastTimestamp;
    uint256 public _n = 48;
    int256 public _std;

    constructor() {
        oracle = AggregatorInterface(
    ...
    function getAmountOracle(
        uint256 timestamp
    ) public view returns (int256 amount) {
        ...
        // console.log("roundIndex %s", roundIndex);

        amount = 1; //oracle.getAnswer(roundIndex) * 1e10;
    }

    function setN(uint256 n) external {
        _n = n;
    }
        ...
        returns (bool upkeepNeeded, bytes memory performData)
    {
        ...
        uint256 k = _lastTimestamp == 0 ? _n : (time - _lastTimestamp) / 3600;

        uint256 startTimestamp = time - 3600 * (_n - 1);
        uint256 i = 0;
        uint256 j = k;
        ...
        while (j < _n) {
            points[i] = _amounts[startTimestamp];
        ...
        while (i < _n) {
            points[i] = getAmountOracle(startTimestamp);
            startTimestamp += 3600;
            i++;
        }

        (int256 std, ) = _calculateSdt(points);

        return (k > 0, abi.encode(time, std, k, points));
    }
    ...
    function performUpkeep(bytes calldata performData) external override {
        ...
        ) = abi.decode(performData, (uint256, int256, uint256, int256[]));

        for (uint i = _n - k; i < _n; i++) {
            uint a = lastTimestamp - (_n - i - 1) * 3600;
            _amounts[a] = points[i];
        }
    ...
    function _calculateSdt(
        int256[] memory x
    ) public view returns (int256 std, int256 muRes) {
        int256 mu;
