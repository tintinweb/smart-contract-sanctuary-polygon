...
import "GovernorContractAggregate.sol";
    ...
    GovernorContractAggregate
{
    ...
    ///  proposalSnapshot - Block Start for voting.
    ///  proposalDeadline - Block End for voting. 
    ///  getVotes - Voting power of account on block, when voting is started. , ProposalAggregatePagination memory
    function getAllProposalAggregatePaginate(uint256 first, uint256 offset)
    public
    view
    returns (uint256 total, uint256 per_page, uint256 last_page, uint256 current_page) {
        uint256 total = proposals.length;
        ProposalAggregatePagination memory pagination;

        if (total == 0){
            pagination.data = new ProposalAggregate[](0);
            return (0,0,0,0 );
        }
        uint256 per_page = first;
        uint256 last_page = total/per_page;
        uint256 current_page = total-(offset-1)*per_page;

        //ProposalAggregate [] memory  proposal_aggregate = new ProposalAggregate[](total-(offset-1)*per_page);
       

        //for(uint256 i=0;per_page*offset<total;i++){
        //    proposal_aggregate[i]  = getSingleProposalAggregate(proposals[i].proposalID);
        //}
        //pagination.data = proposal_aggregate;
        return (total,per_page,last_page,current_page);
    }
...
/// @notice GovernorContract implementation from openzeppelin
/// @dev GovernorContract implementation from openzeppelin
contract GovernorContractAggregate 
{
