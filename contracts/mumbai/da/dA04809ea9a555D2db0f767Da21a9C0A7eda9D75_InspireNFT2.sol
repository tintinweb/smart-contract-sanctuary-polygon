    ...
    ERC721AUpgradeable
{
    ...
    event MintedNFT(uint256 id, uint256 uploadTime, address minter);
    event NewCollection(
        uint256 id,
        string name,
        string description,
    ...
    function batchmint(address _wallet, uint256 NFTamount) public onlyOwner {
        uint256 tokenId = _tokenIdCounter.current();
        emit MintedNFT(tokenId, block.timestamp, msg.sender);
        nfts[tokenId] = Nft(tokenId, block.timestamp, msg.sender);
