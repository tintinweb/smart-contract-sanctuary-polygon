...
contract BetVerseToken is ERC20Upgradeable, OwnableUpgradeable, Interest, ReentrancyGuardUpgradeable {
    ...
    // Fix :: minAmountToStake to avoid DDOS
    ...
    // This contains the list of locked addresses in the tokenomics and the team member, area, and etc
    mapping(address => Internal) public internalAddresses;

    // FIX: We changed to mapping since arrays could cause DDOS attacks.
    ...
    // Fix :: It was possible to mint amounts higher than the max amount manually mintable.
    ...
    // FIX:: Before we were able to burn external coins via OWNER;
    // FIX:: The list of internal addresses are using mapping also for faster confirmations;
    ...
    // FIX::  To stake, the user now has to have a min number of coins. It would prevent DDOS attacks
    // FIX:: We also changed to mapping so it would be way faster
    // Stress Test:: Using ganache, we mint and stake 100K addresses locally
    // FIX: isNotAnInternalAddress is now a mapping also, so a KEY VALUE to check if the user is not a team member
    ...
    function setInternalAddress(address _address) external onlyOwner {
        ...
        emit setInternalAddressSuccess();
    }
    ...
    //FIX:: It was before possible to burn some coins and mint more than the max amount allowed to mint
    modifier maxAmountManuallyMintableNotReached(uint256 _amountToMint) {
        ...
        require(
            ...
            'You can only create more coins to the max amount manually allowed is reached.'
        );
        _;
    }
    ...
    event setInternalAddressSuccess();
