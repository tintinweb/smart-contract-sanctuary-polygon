...
contract GFCGenesisLootBox is Ownable, Whitelist, VRFConsumerBase{
    ...
    /**
        First Step on opening loot box
        Burn keys and request random number from Chainlink
     */
    function getRandom(uint256 tier, uint256 amount) public {
        ...
        getRandNum4LootBox();
        addressKeyInfo[msg.sender] = KeyInfo(tier, amount);
    }
     ...
     */
    function openLootBox(uint256 transactionId) public{
        require(!paused, "The contract have been paused");
        uint256 randNum = getResult(msg.sender);
        KeyInfo storage kInfo = addressKeyInfo[msg.sender];
        uint256[] memory randomNums = expand(randNum, kInfo.amount, BURNT_KEYS);
        BURNT_KEYS += kInfo.amount;
        if(kInfo.tier == 1) {
            openTier1LootBox(transactionId, kInfo.amount, randomNums);
        }else if(kInfo.tier == 2) {
            openTier2LootBox(transactionId, kInfo.amount, randomNums);
        }else if(kInfo.tier == 3) {
            openTier3LootBox(transactionId, kInfo.amount, randomNums);
        }
    ...
    /** 
     * Requests randomness (Chainlink VRF)
     */
    function getRandNum4LootBox() internal {
        require(LINK.balanceOf(address(this)) >= fee, "Not enough LINK to pay fee");
        require(s_results[msg.sender] == 0, "Already rolled");
        bytes32 requestId = requestRandomness(keyHash, fee);
        s_rollers[requestId] = msg.sender;
