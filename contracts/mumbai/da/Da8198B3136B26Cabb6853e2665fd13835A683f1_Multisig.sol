...
import "IApyFlowRootVault.sol";
import "IApyFlowSingleAssetVault.sol";
import "IApyFlowAssetConverter.sol";
...
contract Multisig is Multiownable
{
	...
	modifier onlyNotExpired()
	{
		...
		if (initiationTimeByOperation[operation] - block.timestamp > timeout)
		{
	...
	function voteForCall(address to, bytes data) external onlyNotExpired() onlyManyOwners()
	{
...
interface IApyFlowRootVault
{
	function setNewFeeDestination(address newFeeDestination) external;

	function setNewFeeValue(uint256 newFeeValue) external;
...
interface IApyFlowSingleAssetVault
{
	function addVault(address vault) external;
...
interface IApyFlowAssetConverter
{
	function updateConverter(address source, address destination, address newConverter) external;
}