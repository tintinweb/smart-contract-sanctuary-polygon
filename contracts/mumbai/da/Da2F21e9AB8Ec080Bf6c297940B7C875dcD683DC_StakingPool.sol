...
contract StakingPool is Ownable, ReentrancyGuard, Metadata {
    ...
    function init(bytes memory extraData) external {
        ...
        require(
            address(_localVars._rewardToken) != address(0) &&
                address(farmInfo.inputToken) != address(0),
        ...
        require(_localVars._amount > 0, "Allocate some amount for Pool");

        require(_localVars._blockReward > 0, "Block Rewards cant be zero");

        require(
            _localVars._startTimestamp >= block.timestamp,
