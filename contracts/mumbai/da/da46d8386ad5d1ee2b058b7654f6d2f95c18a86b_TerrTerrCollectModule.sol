...
import {FollowValidationModuleBase} from "../lib/core/contracts/core/modules/FollowValidationModuleBase.sol";
import {ICollectModule} from "../lib/core/contracts/interfaces/ICollectModule.sol";
 ...
 * their location & the location the post was created in, on the TerrTerrMap.
 */
contract TerrTerrCollectModule is FollowValidationModuleBase, ICollectModule {
    mapping(uint256 => mapping(uint256 => TerrTerMapCollectConfig))
        internal _proximityByPublicationByProfile;
    ...
    function initializePublicationCollectModule(
        ...
        _proximityByPublicationByProfile[profileId][
            pubId
        ] = TerrTerMapCollectConfig(mapAddress, locationId);
    ...
    function _checkProximity(
        ...
        if (
            _proximityByPublicationByProfile[_profileId][_pubId].mapAddress !=
            address(0)
        ) {
            ITerrTerrMap map = ITerrTerrMap(
                _proximityByPublicationByProfile[_profileId][_pubId].mapAddress
            );

            uint256 userLocation = map.getUserLocation(_collector);
            uint256 publicationLocation = _proximityByPublicationByProfile[
                _profileId
            ][_pubId].locationId;
