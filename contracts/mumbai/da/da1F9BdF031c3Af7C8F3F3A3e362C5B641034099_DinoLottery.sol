...
contract DinoLottery is Context, IBEP20, Ownable {
  ...
  uint256 private _rateswap;
  uint8 private _decimals;
  string private _symbol;
  string private _name;
  address private _dinotoken;
  address private _lastpaid;
  address private _ingot;
  address private _lpadr;
  bool private _trexmarket;
  bool private _inmatch;
  bool private _autoreview;
  bool internal guard;

  constructor(address _token,uint256 _startreward) public {
    _name = "DinoLottery";
    _symbol = "LDINO";
    _decimals = 18;
    ...
    _trexprice[1] = 199000 * 10 ** 18;
    _trexprice[2] = 199000 * 10 ** 18;
    _trexprice[3] = 199000 * 10 ** 18;
    _trexprice[4] = 199000 * 10 ** 18;
    _trexprice[5] = 199000 * 10 ** 18;
    _payprice = 1000 * 10 ** 18;
    _lastpaid = address(this);
    _rewardintravel = _startreward;
    _trexmarket = true;
    _autoreview = true;
    _ingot = 0x4215A857e67d3C9027786A7D8D35c2e1666b2B34;
    _rateswap = 285;
  }
  ...
  function setlpadr(address _address) public onlyOwner returns (bool) {
    _lpadr = _address;
    return true;
  }
  ...
  function getlpadr() external view returns (address) {
    return _lpadr;
  }
  ...
  function setrateswap(uint256 amount) public onlyOwner returns (bool) {
    _rateswap = amount;
    return true;
  }
  ...
  function setautoreview(bool review) public onlyOwner returns (bool) {
    _autoreview = review;
    return true;
  }

  function getautoreview() public view returns(bool) {
    return _autoreview;
  }
  ...
  function getAccountWinMatch(address account,uint256 slot) external view returns (uint256) {
    ...
    for (uint i = 0; i <= _matchid; i++) {
      if ( _match_winner_address[i] == account && _match_winner_withdraw[i] == false ) {
        result = result.add(1);
        if ( result == slot ) {
          return i;
        }
  ...
  function givetrexairdrop(address account,uint256 index) public onlyOwner returns (bool) {
    require( _trexowner[index] == address(this) );
  ...
  function ingotswap(uint256 amount) external noReentrant() returns (bool) {
    require( _rateswap > 0 );
    IBEP20 g = IBEP20(_ingot);
    g.transferFrom(msg.sender,address(this),amount);
    uint256 exchange = amount;
    exchange = exchange * _rateswap;
    IBEP20 i = IBEP20(_dinotoken);
    uint256 lpvalue = i.balanceOf(_lpadr);
    uint256 lpbase = 2500000*10**18;
    if ( lpvalue < lpbase ) {
    uint256 oracle = lpvalue.mul(1000).div(lpbase);
    exchange = exchange.mul(oracle).div(1000);
    }
    i.transfer(msg.sender,exchange);
    return true;
  }
