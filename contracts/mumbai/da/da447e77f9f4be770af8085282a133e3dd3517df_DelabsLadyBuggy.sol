...
import "./ERC4906.sol";
import "./operator-filter-registry-main/src/DefaultOperatorFilterer.sol";


contract DelabsLadyBuggy is ERC721A, ERC721AQueryable, ERC721ABurnable, ReentrancyGuard,
     Ownable, ERC2981, ERC4906, DefaultOperatorFilterer {
    ...
    constructor() ERC721A("DelabsLadyBuggy", "LADYBUGGY") {
        ...
        signVerifier = 0x3Da49Ca1E078C464a1F3E943078FbA067F9B5Fb2;
    }

    function mint(uint256 _numOfMints, uint256 _nonce, address _contractAddress, bytes memory sig) external nonReentrant {
        require(!usedNonces[_nonce], "Signature Has Already Been Used");
        usedNonces[_nonce] = true;

        bytes32 message = keccak256(abi.encodePacked(msg.sender, _numOfMints, _nonce, _contractAddress));
        require(recoverSigner(message, sig) == signVerifier, "Invalid Signature");

        _mintNFT(msg.sender, _numOfMints);

    }
    


    function whitelistMintNfts(address[] calldata wAddresses, uint256[] calldata quantity) external nonReentrant  {
        require(whitelistAddress[msg.sender], "NOT_IN_WHITELIST");
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(IERC721A, ERC721A, ERC2981, ERC4906)
        returns (bool)
    {
