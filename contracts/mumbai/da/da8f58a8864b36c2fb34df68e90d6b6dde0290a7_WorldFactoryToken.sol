...
// File: contracts/stateERC721forERC20.sol
...
contract WorldFactoryToken is ERC20Burnable, Ownable {

    IERC20 public rewardsToken;
    IERC20 public stakingToken;
    IERC1155 public worldFactoryNFTs;
...
//    IERC1155 worldFactoryNFTs = IERC1155(0x540d7E428D5207B30EE03F2551Cbb5751D3c7569);
    ...
    constructor() ERC20("WorldFactoryToken", "WFT") {}

    function claimAll() external updateReward(msg.sender) {
        ...
        //Transfer rewards from staking ERC20 tokens - "uint reward" it the rewardRate per second defined in contract - divided amongst ALL the staked ERC20 tokens
        //Special local calculations needs to be calculated for Utilitiy Tokens
        //the _mint(msg.sender, totalRewards) is the minting of the WorldFactoryGameToken, which is determined by the EMISSIONS_RATE which is per staked ERC721
        uint reward = rewards[msg.sender];
        rewards[msg.sender] = 0;


        uint256 factory = worldFactoryNFTs.balanceOf(msg.sender, 2);
        uint256 continents = worldFactoryNFTs.balanceOf(msg.sender, 3);
        ...
        uint kion = (reward * 1);
//        uint kion2 = (reward * 2);
        rewardsToken.transfer(msg.sender, kion);
//        rewardsToken.transfer(msg.sender, kion2);
//        rewardsToken.transfer(msg.sender, (continents * 1e18)); //should result in 1 token with 18 decimals if continents = 1

        _mint(msg.sender, totalRewards);
    }

    function setworldFactoryNFTs(address _worldFactoryNFTs) public onlyOwner {
        worldFactoryNFTs = IERC1155(_worldFactoryNFTs);
        return;
    }
    ...
    function withdrawRewardsToken(uint _amount) external updateReward(msg.sender) {
        _totalSupply -= _amount;
        _balances[msg.sender] -= _amount;
    ...
    function setEMISSIONS_RATE(uint256 _EMISSIONS_RATE) public onlyOwner {
        EMISSIONS_RATE = _EMISSIONS_RATE;
        return;
    }
    ...
    function unstakeAll() public payable {
        ...
        require(paused == false, "Unstaking is currently disabled");
        require(msg.value >= cost);
        uint256 totalRewards = 0;
    ...
    function unstakeByIds(uint256[] memory tokenIds) public payable {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            IERC721(anonymiceAddress).transferFrom(address(this), msg.sender, tokenIds[i]);
    ...
    function claimByTokenId(uint256 tokenId) public payable{
    ...
    function withdrawBalanceFromContract() public payable onlyOwner {
