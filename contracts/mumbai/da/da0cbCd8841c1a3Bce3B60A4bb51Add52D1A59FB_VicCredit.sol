...
contract VicCredit is ERC721, AccessControl {
    string[] private guidArray;
    uint256 private mintPrice;
    uint256 private transferFee;
    mapping (string => uint256) private guidToTokenId;
    mapping (uint256 => uint256) private tokenToPrice;

    constructor(uint256 mp) ERC721("VicCredit", "VICC") { 
    ...
    function isClaimAvailable(string memory _guid) external view returns (bool) {
        bytes memory guidBytes = abi.encodePacked(_guid);
        string memory guidString = string(guidBytes);
        return guidToTokenId[guidString] == uint256(0);
    }

    function getTokenId(string memory _guid) external view returns (uint256) {
        bytes memory guidBytes = abi.encodePacked(_guid);
        string memory guidString = string(guidBytes);
        require(guidToTokenId[guidString] != uint256(0), "Viccredit: There is no claim therefore no tokenId.");
        return guidToTokenId[guidString];
    }

    function getSalePrice(string memory _guid) external view returns (uint256) {
        //bytes memory guidBytes = abi.encodePacked(_guid);
        //string memory guidString = string(guidBytes);
        uint256 tokenId = guidToTokenId[_guid];
        return tokenToPrice[tokenId];
    }

    function mintCredit(string memory _guid, uint256 _price) whenNotPaused external payable {
        require(msg.value == mintPrice, "VicCredit: Cost to mint credit must equal mint price.");
        //mint
        //bytes memory guidBytes = abi.encodePacked(_guid);
        string memory guidString = string(_guid);
        require(guidToTokenId[guidString] == uint256(0));

        guidArray.push(guidString);
        uint256 id = guidArray.length;
        guidToTokenId[guidString] = id;
        tokenToPrice[id] = _price;
        _safeMint(msg.sender, id);
        _setTokenURI(id,guidString);

        financialOfficerAddress.transfer(msg.value);
    }

    function purchaseCredit(string memory _guid, uint256 _price) whenNotPaused external payable {
        require(msg.value > 0, "VicCredit: Purchase price cannot be zero. Use transfer for no cost ownership changes.");
        bytes memory guidBytes = abi.encodePacked(_guid);
        string memory guidString = string(guidBytes);
        uint256 tokenId = guidToTokenId[guidString];

        require(msg.value >= tokenToPrice[tokenId], "VicCredit: Amount offered must be >= price set by owner.");
        ...
        emit SaleComplete(guidString, toOwner, toContract);
    }

    function sellCredit(string memory _guid, uint256 _newPrice, address _purchaser) whenNotPaused external payable {
        uint256 tokenId = guidToTokenId[_guid];
        require(msg.value > 0, "VicCredit: Sale price cannot be zero. Use transfer for no cost ownership changes.");
        ...
        require((msg.value / 10000) * 10000 == msg.value, 'VicCredit: Sale price is too small.');
        uint256 toContract = msg.value * transferFee / 10000;
        uint256 toOwner = msg.value - toContract;
        ...
        emit SaleComplete(_guid, toOwner, toContract);
    }
    ...
    function tokensOfOwner(address _owner) external view returns(uint256[] memory ownerTokens) {
        ...
        if (tokenCount == 0) {
            ...
            uint256 viccreditId;
            for (viccreditId = 1; viccreditId <= total; viccreditId++) {
                if(ownerOf(viccreditId) == _owner){
                    result[resultIndex] = viccreditId;
                    resultIndex++;
    ...
    function setSalePrice(uint256 tokenId, uint256 price) external {
        ...
        require(price >= 0, "VicCredit: Price must be positive number.");
        tokenToPrice[tokenId] = price;
        require(tokenToPrice[tokenId] == price, "VicCredit: Price must be properly set.");
    ...
    event SaleComplete(string guid, uint256 price, uint256 fee);
}