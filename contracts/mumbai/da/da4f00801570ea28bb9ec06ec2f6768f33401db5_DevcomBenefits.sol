...
// File: contracts\DevcomBenefits.sol


pragma solidity ^0.8.17;
/// @custom:security-contact [email protected]
contract DevcomBenefits is ERC721, ERC721URIStorage, PullPayment, Ownable {
    using Counters for Counters.Counter;

    uint256 private constant _totalSupply = 1000;
    string private constant _baseTokenURI = "https://devcom-web3.s3.amazonaws.com/json/";
    string private constant _contractURI = "https://devcom-web3.s3.amazonaws.com/contract-metadata.json";

    Counters.Counter private supply;

    constructor() ERC721("DevcomBenefits", "DBT") { }
    ...
    function tokensOwners() public view returns (address[] memory) {
        uint256 tokenSupply = supply.current();
        address[] memory owners = new address[](tokenSupply);
        uint256 currentTokenId = 1;
        uint256 tokenIndex = 0;

        while (currentTokenId <= tokenSupply) {
            address currentTokenOwner = ownerOf(currentTokenId);
            owners[tokenIndex] = currentTokenOwner;
            tokenIndex++;
            currentTokenId++;
        }
