...
contract Membership is Initializable, Pausable, AccessControlUpgradeable {

    bytes32 public constant ADMIN_ROLE = keccak256("MEMBERSHIP_ADMIN_ROLE");
    ...
    event MembershipCreated(bytes32 _type, uint256 tokenId, uint256 price);

    event MembershipDisabled(bytes32 _type);
    ...
    function mintMembership(bytes32 _type, string memory _uri, uint256 _price) external priceGreaterThanZero(_price)
    returns (bool, uint256)
    {
        ...
        emit MembershipCreated(_type, tokenId, _price);

        return (true, tokenId);
    }

    function disableMembership(bytes32 _type) external returns (bool)
    {
        require(hasRole(ADMIN_ROLE, _msgSender()), "must have admin role");

        require(_membershipTypeToTokenId[_type] > 0, "Membership type not found");
        require(_membershipTokenLeft[_membershipTypeToTokenId[_type]] == 1000, "Membership already used");

        for (uint8 i = 0; i < membershipTypesIds.length; i++) {
            if (membershipTypesIds[i] == _membershipTypeToTokenId[_type]) {
                membershipTypesIds[i] = membershipTypesIds[membershipTypesIds.length - 1];
            }
        }

        membershipTypesIds.pop();
        _membershipTypeToTokenId[_type] = 0;

        emit MembershipDisabled(_type);
        return (true);
    }
