    ...
    OwnableUpgradeable
{
    ...
    function convertToOrdKubz(
        ...
        uint256 ordKubzCount = avycTokenIds.length / avycRequiredPerOrdKubz;
        require(
            ordKubzCount * avycRequiredPerOrdKubz == avycTokenIds.length,
            "Non divisible count of AVYC provided"
        );
        require(ordKubzCount > 0, "No AVYC provided");
        uint256 l = availOrdKubzIds.length();
        require(l >= ordKubzCount, "Not enough availOrdKubzIds left");
        _burnAVYC(avycTokenIds);
        for (uint256 i = 0; i < ordKubzCount; i++) {
            uint256 randOrdKubzId = availOrdKubzIds.at(
                rng(avycTokenIds) % (l - i)
            );
            availOrdKubzIds.remove(randOrdKubzId);
            ordKubzContract.transferFrom(
                vaultAddress,
                msg.sender,
                randOrdKubzId
            );
    ...
    function convertToKubz(
        ...
        uint256 kubzCount = avycTokenIds.length / avycRequiredPerKubz;
        require(
            kubzCount * avycRequiredPerKubz == avycTokenIds.length,
            "Non divisible count of AVYC provided"
        );
        require(kubzCount > 0, "No AVYC provided");
        uint256 l = availKubzIds.length();
        require(l >= kubzCount, "Not enough availKubzIds left");
        _burnAVYC(avycTokenIds);
        for (uint256 i = 0; i < kubzCount; i++) {
            uint256 randKubzId = availKubzIds.at(rng(avycTokenIds) % (l - i));
