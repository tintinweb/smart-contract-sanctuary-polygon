...
contract SheepionWL is ERC1155Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function initialize() initializer public {
    ...
    walletMaster = payable(0x52BD82C6B851AdAC6A77BC0F9520e5A062CD9a78);
  }
  ...
  */
  function uri(uint256 _collectionId) public view virtual override returns (string memory) {
    if (_collectionId == BOOSTER_COLLECTION_ID)   return boosterUri;
    if (_collectionId == BATTLE_COLLECTION_ID)    return battleUri;
    if (_collectionId == HERD_COLLECTION_ID)      return herdUri;
