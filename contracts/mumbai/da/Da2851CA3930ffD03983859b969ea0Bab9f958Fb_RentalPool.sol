...
contract RentalPool is AccessControl, IRentalPool {
    ...
    function postMissions(NFTRental.Mission[] calldata newMission)
        external
        override
    {
        for (uint256 i; i < newMission.length; i++) {
            _postMission(newMission[i]);
        }
    }
    ...
    function cancelReadyMission(string calldata _uuid) external override {
        _cancelReadyMission(_uuid);
    }

    function batchCancelReadyMission(string[] calldata _uuid)
        external
        override
    {
        for (uint32 i; i < _uuid.length; i++) {
            _cancelReadyMission(_uuid[i]);
        }
    }
    ...
    function _postMission(NFTRental.Mission calldata newMission) internal {
        ...
        require(
            !ownerReadyMission[msg.sender][newMission.tenant][
                newMission.dappId
            ],
            "Owner already have ready mission for tenant and dapp"
        );
        require(
            !IMissionManager(missionManager).isMissionPosted(newMission.uuid),
    ...
    function _cancelReadyMission(string calldata _uuid) internal {
        NFTRental.Mission memory curMission = IMissionManager(missionManager)
            .getReadyMission(_uuid);
        ...
        delete ownerReadyMission[msg.sender][curMission.tenant][
            curMission.dappId
        ];
    }
