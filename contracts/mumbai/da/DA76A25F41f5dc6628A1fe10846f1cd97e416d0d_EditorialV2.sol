...
// 1) Add Mapping to store the list of owners
// 2) Add function to search address within the mapping of owners
// 3) Create a function with Base64 to store a prefix on the token type on the Base URI

contract EditorialV2 is ERC1155, Ownable, Pausable, ReentrancyGuard, AccessControl {
    ...
    /**
     * @dev array to store all current NFT holders
     */
    ...
    /**
     * @dev Add a new address into the list of token holders
     */

    function addTokenHolder( address _holderAddress ) public {
        tokenHolders.push( _holderAddress );
    }


    /**
     * @dev Check if the connected wallet is part of the token holders array
     * @dev returns the index of the array where the tokenholder address was stored
     * @param result returns either the index where the address was found or defaults to 0 if it was not found
     */

    function searchTokenHolder( address _holderAddress ) public view returns( uint result ) {
        {   uint i;

            for ( i = 0; i < tokenHolders.length; i++ )
            {
                if( tokenHolders[i] == _holderAddress ) {
                    return i;
                }

                if( i >= tokenHolders.length ) {
                    return 0;
                }
            }
    ...
    /**
     * @dev Remove the address from the Token Holders array
     *      by switching the token holder address to 0x0 address
     */
    function removeTokenHolder( uint _tokenHolderIndex ) public {
        tokenHolders[_tokenHolderIndex] = 0x0000000000000000000000000000000000000000;
    }
