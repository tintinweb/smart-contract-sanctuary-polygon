...
contract DragonArena is LockableReceiver, ReentrancyGuard { 
    ...
    function deposit(address to, uint amount) external nonReentrant {
        require(!Address.isContract(to), BAD_ADDRESS_ERROR);
        require(amount > 0, BAD_AMOUNT_ERROR);
    ...
    function withdrawDeposit(address to, uint amount) external nonReentrant {
