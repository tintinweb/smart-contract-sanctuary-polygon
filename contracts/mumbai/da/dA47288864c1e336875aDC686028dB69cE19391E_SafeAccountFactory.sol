...
contract EIP4337Manager is IAccount, SafeStorage, Executor {
    ...
    constructor(address anEntryPoint, address anRecoveryModule, address anSpendingLimitModule) {
     ...
     * @param _userOp validate the userOp.signature field
     * @param _userOpHash convenient field: the hash of the request, to check the signature against
     *          (also hashes the entrypoint and chain id)
     * @return _validationData signature and time-range of this operation
    ...
    function _validateSignature(
        ...
        address recovered = hash.recover(_userOp.signature);
        if (!safe.isOwner(recovered)) {
     ...
     *
     * @param _userOp validate the userOp.nonce field
     *
     * solhint-disable-next-line no-empty-blocks
     */
    function _validateMethod(UserOperation calldata _userOp) internal view {
        bytes4 sig = bytes4(_userOp.callData[:4]);
        if (whetherLimitRouter()) {
            require(
                sig == DELAY_TRANSACTION || sig == NORMAL_TRANSACTION || sig == SUPER_TRANSACTION,
                "account: invalid method"
            );
        }
        uint192 key = uint192(_userOp.nonce >> 64);
        if (key != 0) {
            require(!isDisableSchedule, "account: schedule module disabled");
            require(sig == DELAY_TRANSACTION, "exec: illegal call");
        }
    }
