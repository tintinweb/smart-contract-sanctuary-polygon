...
contract Dao {
  ...
  event proposalCount(
    ...
    function voteOnProposal(uint256 _id, uint256 _votesUp, uint256 _votesDown) public {
        ...
        require( (proposalToId[_id].individualVotes[msg.sender] += (_votesUp+_votesDown)) <= proposalToId[_id].individualVotes[msg.sender] );
        require(block.number <= proposalToId[_id].deadline, "The deadline has passed for this Proposal");
    ...
    function _vote(uint256 _id, uint256 _votesUp, uint256 _votesDown, address _voter, uint256 maxVotesAvailable) internal {
        ...
        p.individualVotes[_voter] += (_votesUp+_votesDown);    
        p.votesUp += _votesUp;
        p.votesDown += _votesDown;
        p.individualChoices[_voter][true] += _votesUp;
        p.individualChoices[_voter][false] += _votesDown;
        if(p.individualVotes[_voter] == maxVotesAvailable){
            p.voteStatus[_voter] == true;
        }
    ...
    function _checkVoteAvailability(uint256 _id, address _checkVoter) public view returns (uint256 _votesAvailable){
