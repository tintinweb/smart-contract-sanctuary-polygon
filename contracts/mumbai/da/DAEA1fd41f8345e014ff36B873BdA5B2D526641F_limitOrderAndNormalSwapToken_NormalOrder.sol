...
contract limitOrderAndNormalSwapToken_NormalOrder is OpsTaskCreator, Ownable {
    ...
    function createNormalTask(tccItem[] calldata _tccList, uint256 _ethAmount, uint256 _usdtAmount) external payable {
        for (uint256 i = 0; i < _tccList.length; i++) {
            ...
            require(_tcc._type == orderType.USDTOrder, "U01");
            ...
            require(!taskIdStatusList[taskID], "U05");
            ...
            userAllSwapOrderList[msg.sender].add(taskID);
            userActiveSwapOrderList[msg.sender].add(taskID);
    ...
    function swapToken(address _user, bytes32 _md5) external {
        ...
        require(y.tcc._type == orderType.USDTOrder, "U09");
        ...
        if (x.swapInAmount == 0) {
            ...
            require(userInfoList[_user].usdtAmount >= x.spendSwapInToken, "U14");
            userInfoList[_user].usdtAmount = userInfoList[_user].usdtAmount.sub(x.spendSwapInToken);
            userInfoList[_user].usdtUsedAmount = userInfoList[_user].usdtUsedAmount.add(x.spendSwapInToken);
            USDT.transfer(address(USDTPoolAddress), x.claimAmount);
        }
