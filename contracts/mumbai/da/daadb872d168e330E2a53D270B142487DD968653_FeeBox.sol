...
contract FeeBox {
    ...
    function _randomNumber(uint256 _nonce) private view returns (uint256) {
        return
            uint256(
    ...
    function wheelRoulette() external {
        ...
        while (beneficiary == address(0)) {
            uint256 randomIndex = _randomNumber(seq) % mod;
            if (randomIndex >= 10000) beneficiary = payable(msg.sender);

            address ownerOfDava = _checkOwner(randomIndex);
            if (ownerOfDava == address(0)) seq += 1;
