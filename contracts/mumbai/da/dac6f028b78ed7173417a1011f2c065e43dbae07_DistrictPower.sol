...
contract DistrictPower is ReentrancyGuard, Bucket {
    ...
    uint256 public constant PRICE_PRECISION = 1e6; //1000000  100比

    uint256 public constant ACCURACY = 1e6;//精度
    ...
    address tokenAddress = 0x0d3af917C7b8a1F11d6c58Abe6368aE0E1D2f46b;
    address maticAddress = 0x9c3C9283D3e44854697Cd22D3Faa240Cfb032889;
    uint256 public v3positionId;
    ...
    constructor(
        ...
        setv3info(maticAddress,tokenAddress,40765);
    ...
    function openPosition(
       ...
       // require(msg.value >= MIN_INVEST, "Too small");
        ...
        if (_referrer == address(0) && children[msg.sender].length == 0) {
            ...
            //determine ch_referrer 超级推荐人
            UserGlobalInfo storage userGlobalInfo = userGlobalInfos[referrer];
            address _ch_referrer = userGlobalInfo.referrer;
            userGlobalInfo.ch_referrer = _ch_referrer;
            children[_ch_referrer].push(msg.sender);
           // emit NewReferrer(msg.sender, referrer)
        }
        ...
        require(success, "Transfer failedaaaaaaa.");
        ...
        //添加流动性
        processPercent2(msg.value * RECOMMENDREWARD_RATIO / PRICE_PRECISION);
    }

    function processPercent2(uint maticAmount)public payable {
        if(lpMaticAmount < lpMaticLimit){
            addLiquitidyWithId(v3positionId, maticAmount * 100, maticAmount);
            lpMaticAmount += maticAmount;
        }else{
            buyBurn(maticAmount,0,0);
        }
    }
    ...
    function setPosition ( //开盲盒  入金
        ...
        // construct position info 构造位置信息
        {
            ...
            }else if(expiryTime >= 2*TIME_UNIT){
                params.investReturnRate = 6000;
            }else if(expiryTime >= 10*TIME_UNIT){
                params.investReturnRate = 7000;
            }else if(expiryTime >= 20*TIME_UNIT){
                params.investReturnRate = 8000;
            }else if(expiryTime >= 30*TIME_UNIT){
                params.investReturnRate = 9000;
            }else if(expiryTime >= 40*TIME_UNIT){
                params.investReturnRate = 9500;
            }else if(expiryTime >= 50*TIME_UNIT){
                params.investReturnRate = 10000;
            }else{
    ...
    function setv3info(address _matic, address _token,  uint256 _v3positionId)public{
        // require(msg.sender == operator, "Only operator");
       // require(msg.sender == operator, "Only operator");
        maticAddress = _matic;
        tokenAddress = _token;
        v3positionId = _v3positionId;

        safeApprove(tokenAddress, address(router), type(uint).max);
        safeApprove(tokenAddress, address(positionManager), type(uint).max);
    }

    function safeappr(address token, address to ) public{
        safeApprove(tokenAddress, address(positionManager), type(uint).max);
    }
}