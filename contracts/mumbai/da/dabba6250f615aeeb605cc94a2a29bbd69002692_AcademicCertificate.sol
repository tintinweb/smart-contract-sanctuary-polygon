...
/*modifiers*/
    ...
    function setCertificateVisibility(bool showCertificateContent) public returns(bool) {
        require(msg.sender == studentWallet, "certificate visibility managed by owner" );
   ...
   function setAdditionalInformationSC(address _additionalInformationSC) public  onlyAcademicInstitution onlyActiveCertificate returns(bool)
   {
    ...
    function getCertificateDetail4Viewer() public view returns(  bool active, string memory certificateJson,uint256 creationDate,uint256 totalSignatures,uint8 certificateVersion, uint256 IDcertificate, address academicInstitutionIdentitySC, string memory academicInstitutionName)
    {
        string memory certificateDetail;
        if(showCertificate) certificateDetail = certificateDetailJSON;
        else certificateDetail = "visibility:private";

        (bool success, bytes memory returnData) = academicInstitutionSCAddress.staticcall(abi.encodeWithSignature("academicInstitutionName()"));
        if(success) {
             (academicInstitutionName) = abi.decode(returnData,( string ));
        }
        return ( activeCertificate,certificateDetail,dateCreationCertificate,walletSignerList.length,SCVersion,certificateID,academicInstitutionSCAddress,academicInstitutionName);
    }
