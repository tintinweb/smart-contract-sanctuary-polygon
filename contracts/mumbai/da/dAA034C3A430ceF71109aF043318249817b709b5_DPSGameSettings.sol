...
contract DPSGameSettings is Ownable {
     ...
     */
    uint16 public maxSkillsCap = 570;

    /**
     * @notice max points the causality can generate
     */
    uint16 public maxRollCap = 600;
    ...
    /**
     * @notice min/max Doubloons rewards per chest type
     */
    mapping(VOYAGE_TYPE => uint256[]) public doubloonsPerChest;
    ...
    /**
     * @notice tmaps per doubloons
     */
    uint256 tmapPerDoubloon;

    /**
     * @notice support ships contract
     */
    ...
    constructor() {
        ...
        doubloonsPerChest[VOYAGE_TYPE.EASY] = [1 * 1e18, 5 * 1e18];
        doubloonsPerChest[VOYAGE_TYPE.MEDIUM] = [2 * 1e18, 10 * 1e18];
        doubloonsPerChest[VOYAGE_TYPE.HARD] = [3 * 1e18, 20 * 1e18];
        doubloonsPerChest[VOYAGE_TYPE.LEGENDARY] = [4 * 1e18, 30 * 1e18];
    ...
    function setDoubloonsPerChest(VOYAGE_TYPE _type, uint256[] memory _amounts) external onlyOwner {
        require(_amounts.length == 2, "Invalid amounts");
        require(_amounts[0] < _amounts[1], "Min rewards needs to be greater than max rewards");
        doubloonsPerChest[_type] = _amounts;
    }
    ...
    function getDoubloonsRewardsPerChest(VOYAGE_TYPE _type) external view returns (uint256[] memory) {
        return doubloonsPerChest[_type];
    }
