...
contract ION is ERC20, AccessControl {
    ...
    constructor(
        ...
        buySellTime = block.timestamp + 1800;
    ...
    function getTokenPrice() public view returns (uint256) {
        ...
        path[2] = USDC; //Todo change this address for mainnet
    ...
    function _setEpoch() internal {
        epochEndAmt = (marketCap() * marketCapPer) / (PCT_BASE); //todo Remove 1000 value
        buySellTime = block.timestamp + twentyFourHours;
        epochCurrentAmt = 0;
    }
    ...
    function _setEpochEndAmt() internal {
        epochEndAmt = (marketCap() * marketCapPer) / (PCT_BASE); //todo remove 1000 for deployment
    }
    ...
    function _transfer(
        ...
        if (isblacklist[_sender] || isblacklist[_recipient]) {
            ...
            _isExcludedFromFee[_sender] || _isExcludedFromFee[_recipient]
        ) {
            ...
            console.log("enter", remThreePerTaxAmount);
    ...
    function _userSellLimit(address user, uint256 transactAmount) internal {
        if (
            userLimits[user].startTime == 0 ||
            userLimits[user].startTime + 24 hours >= block.timestamp
        ) {
    ...
    function _distribution() internal {
        if (checkEpoch()) {
            if (superChgReward != 0 || burnRebaseReward != 0) {
                ...
                if (enableSwapAndLiquify) {
                    ...
                    uint256 maticAmount = uniswapV2Router.swapExactTokensForETH(
                        swapAmount,
                        1,
                        path,
                        address(this),
        ...
        if (block.timestamp >= trickleBaseTime + trickleTime) {
            if (trickleChgReward != 0 && devReward != 0) {
                super._transfer(
                    address(this),
                    rewardsContract,
                    trickleChgReward
