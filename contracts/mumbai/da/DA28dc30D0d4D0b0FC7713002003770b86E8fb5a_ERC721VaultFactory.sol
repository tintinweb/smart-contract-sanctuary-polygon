...
interface ItokenCenter {
    function exchangeToken( address _curator,address _originalToken, uint256 _supply,uint256 _ratio) external;
}

contract TokenVault is ERC20Upgradeable, ERC721HolderUpgradeable,ReentrancyGuardUpgradeable {
    using Address for address;
    ...
    /// @notice weth address
    address public constant weth = 0x190411Af0c058f08555B80C335d7009ad6791588;
    ...
    /// @notice the ratio between original token and new token
    ...
    //constant variable
    address public immutable tokenCenter;
    ...
    constructor(address _settings,address _tokenCenter) {
        settings = _settings;
        tokenCenter=_tokenCenter;
    }
    ...
    function initialize(address _curator, address _token, uint256 _id, uint256 _supply, uint256 _listPrice, uint256 _fee, string memory _name, string memory _symbol) external initializer {
        ...
        // exchangeToERC20Token(_curator,_supply);

    }

    function exchangeToERC20Token(
        address _curator, 
        uint256 _supply
    ) internal nonReentrant returns (bool) {
        // Transfers tokens to TokenCenter contract, assume that buyer has given allownace[buyer][address(this)]
        TransferFromHelper.safeTransferFrom(address(this), _curator,tokenCenter, _supply);
        
        //Call contract to mint token
        try ItokenCenter(tokenCenter).exchangeToken(_curator, address(this), _supply, ratio) {}
        catch {
            revert("Error: failed to mint general tokens");
        }
    ...
    function burn(uint256 amount) public returns (bool) {
        require(tokenCenter == _msgSender(), "You don't have right to burn token.");
...
contract ERC721VaultFactory is Ownable, Pausable {
  ...
  constructor(address _settings,address _tokenCenter) {
    settings = _settings;
    tokenCenter = _tokenCenter;
    logic = address(new TokenVault(_settings,_tokenCenter));
  }
