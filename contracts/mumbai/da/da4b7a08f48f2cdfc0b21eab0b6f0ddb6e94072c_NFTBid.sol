...
contract NFTBid is ERC721URIStorage {  // Definisce il contratto GameItem e lo eredita da ERC721URIStorage
    ...
    constructor() ERC721("NFTBid", "ITM") {}  // Costruttore del contratto che chiama il costruttore del contratto ERC721 e gli assegna il nome "GameItem" e il simbolo "ITM"

    function awardItem(address bidder, string memory tokenURI)
        public
        returns (uint256)
    {
        uint256 newItemId = _tokenIds.current();  // Crea un nuovo ID del token tramite il contatore
        _mint(bidder, newItemId);  // Crea un nuovo token e lo assegna al giocatore specificato
