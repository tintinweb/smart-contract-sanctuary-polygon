...
contract Calculations is Initializable, OwnableUpgradeable {
    ...
    function estimateChainlinkFee(
    ...
    ) public view returns (uint256 nativePrice, uint256 linkPrice) {
        require(
            amountOfBets > 0,
        ...
        currentGasPrice = currentGasPrice + ((currentGasPrice * 10) / 100); // Adds 10% to the current gas price
        uint256 gasCost = currentGasPrice *
            (maxVerificationGas + callbackGasLimit) *
            amountOfBets;

        uint256 gasCostInLink = price > 0 ? (gasCost * BP) / uint256(price) : 0;
        uint256 totalInLink = gasCostInLink + linkPremium_;
        uint256[] memory amounts = sushiswap.getAmountsIn(
            totalInLink,
            nativeToLinkPath
        );
        return (amounts[0], totalInLink);
    }
