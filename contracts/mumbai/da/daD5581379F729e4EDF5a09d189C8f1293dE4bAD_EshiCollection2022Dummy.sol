...
contract EshiCollection2022Dummy is BaseNFT {
    ...
    function mintByType(uint256 tokenType_) external whenNotPaused {
        uint256 tokenID = _tokenIDCounter.current();

        _mint(msg.sender, tokenID, tokenType_);
...
contract BaseNFT is ERC721Enumerable, ERC2981, IERC4907, Ownable, Pausable {
    ...
    bool internal _isMinterFrozen;

    modifier onlyMinter() {
        require(msg.sender == _minter, "BaseNFT: caller is not the minter");
        _;
    }
    ...
    function assignMinter(address minter_) external onlyOwner {
        require(
            minter_ != address(0),
            "BaseNFT: new minter is the zero address"
        );
        _requireMinterNotFrozen();

        _setMinter(minter_);
    }

    function unassignMinter() external onlyOwner {
        _requireMinterNotFrozen();
    ...
    function _requireMinterNotFrozen() internal view {
        require(!_isMinterFrozen, "BaseNFT: minter frozen");
    }
