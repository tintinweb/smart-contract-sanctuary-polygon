...
contract Domains is ERC721URIStorage {
  ...
  event RecordSet(string name, Record record);
  ...
  constructor(string memory _tld) payable ERC721("PolyNames", "PNS") {
    console.log("THIS IS MY DOMAINS CONTRACT. NICE. THIS IS THE NEW META");
    tld = _tld;
  }
  ...
  function register(string calldata name) external payable {
    ...
    string memory json = Base64.encode(
      abi.encodePacked(
        '{"name": "',
        _name,
        '", "description": "A domain on the poly name service", "image": "data:image/svg+xml;base64,',
        Base64.encode(bytes(finalSvg)),
        '","length":"',
        Strings.toString(nameLength),
        '","username":"", "content":"", "twitter":"", "github":"", "website":"", "email":""}'
      )
    );
  ...
  function setRecord(
    ...
    require(domains[name] == msg.sender, "Not Owner");
    string memory _name = string(abi.encodePacked(name, ".", tld));
    // Create the SVG (image) for the NFT with the name
    string memory finalSvg = string(
      abi.encodePacked(svgPartOne, _name, svgPartTwo)
    ...
    // update tokenURI metadata
    // emit event for successful record set
  ...
  //   add contract uri function here
}
