...
contract SkyToken is ERC20, FxBaseChildTunnel, Ownable {
    ...
    event ProcessedMessage(address from, uint256 count, bool stake);
    ...
    // staking bonus based on days range staked threshold
    ...
    function toggleBonusStack() external onlyOwner {
        bonusStack = !bonusStack;
    }

    function setMultiplier(uint256 stakeDays, uint256 multiplierEther) external onlyOwner{
        uint256 daysToUnix = stakeDays * 1 days;
        uint256 multiplierWei = multiplierEther * 1 ether;
        stakingBonus[daysToUnix] = multiplierWei;
        thresholdRecord.push(daysToUnix);
    }

    function setBaseReward(uint256 rate) external onlyOwner {
        baseReward = rate;
    }
    ...
    function gaslessHarvest(
        ...
        _validateSignature(user, amount, signature);
    ...
    function _processMessageFromRoot(
        ...
        (address from, uint256 count, bool stake) = abi.decode(
            message,
        ...
        stake
            ? _userInfo.stakedBalance += count
            : _userInfo.stakedBalance -= count;

        emit ProcessedMessage(from, count, stake);
    }
    ...
    function _harvestReward(address user) internal notPaused {
        ...
        uint256 pendingReward = _getPendingRewards(user);
        _userInfo.lastClaimed = block.timestamp;

        if (pendingReward > 0) {
            transfer(user, pendingReward);
            emit Transfer(address(this), user, pendingReward);
        }
    }

    function _bonusThreshold(uint256 stakingDays) internal view returns (uint256) {
        uint256 bonus = 0;

        for(uint256 i = 0; i < thresholdRecord.length; i++){
            if(stakingDays >= thresholdRecord[i]){
                if(stakingBonus[thresholdRecord[i]] > bonus) bonusStack 
                    ? bonus += thresholdRecord[i]
                    : bonus = thresholdRecord[i];
            }
        }
    ...
    function _getPendingRewards(address user) internal view returns (uint256) {
        UserInfo storage _userInfo = userInfo[user];
        uint256 dayRate = 0;
        uint256 daysDifference = block.timestamp - _userInfo.lastClaimed;

        unchecked {
            dayRate += (baseReward * _userInfo.stakedBalance);
        }

        if (dayRate > 0) {
            uint256 bonus = _bonusThreshold(daysDifference);
            return
                ((dayRate * daysDifference) / 86400) + bonus;
        }
