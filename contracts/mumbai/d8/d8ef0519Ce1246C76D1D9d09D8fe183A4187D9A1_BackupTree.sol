...
// File contracts/trees/BackupTree.sol
...
contract BackupTree is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, ERC721BurnableUpgradeable, AccessControlUpgradeable {
    event BatchCreated(uint256 indexed, uint256 indexed);
    event BatchMinted(uint256 indexed, address indexed, uint256, uint256 indexed);
    ...
    function initialize(Batch calldata _batch, address admin, address _treesFactory) initializer public {
        batch = _batch;
        treesFactory = ITreesFactory(_treesFactory);

        // B - stands for batch
        // CTB - stands for Carbify Tree Batch
        string memory _name  = string(abi.encodePacked("Carbify Foundation Backup Tree B", Strings.toString(batch.no)));
        string memory _symbol = string(abi.encodePacked("BTB", Strings.toString(batch.no)));
        ...
        emit BatchCreated(batch.no, block.timestamp);
    }
    ...
    function safeMintBatch(address to, uint256 quantity) public onlyRole(MINTER_ROLE) {
        require (quantity <= treesFactory.batchMintLimit(), "Quantity exceeds max batch mint limit");
        require(_currentIndex + quantity <= batch.maxSupply, "Token ID exceeds max supply for Backup NFT");
        for (uint256 i = 0; i < quantity; i++) {
            _safeMint(to, _currentIndex + i);
        }
        _currentIndex += quantity;

        emit BatchMinted(batch.no, to, quantity, block.timestamp);
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        return bytes(uri).length != 0 ? string(abi.encodePacked(uri, "/", Strings.toString(tokenId), batch.baseExtension)) : '';
    }
