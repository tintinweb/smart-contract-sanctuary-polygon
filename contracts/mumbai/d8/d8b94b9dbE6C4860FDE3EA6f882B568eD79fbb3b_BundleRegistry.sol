    ...
    PausableUpgradeable
{
    ...
    /// @dev Registers an execution bundle, making data accessible to nodes. Once registered, each bundle is immutable. Only the 
    /// contract owner can change it, and they can only deactivate/reactivate it if necessary.
    ...
    function register(string calldata _bundle, string calldata _source, string calldata _host, string calldata _output, bool _active)
        external
        whenNotPaused
    {
        require(isIPFS(_bundle), "Bundle must be an IPFS CID hash");

        bytes32 bundleHash = keccak256(abi.encode(_bundle, _source, _host, _output));

        // Check that bundle does not yet have an author--proxy to check whether the bundle was already registered.

        require(bundles[bundleHash].author == address(0), "Bundle already registered");
        ...
        emit BundleRegistered(bundleHash, _bundle, _host, _source, _output, _active, msg.sender);
    }
     ...
     * @param _remainActive is whether to pause or unpause; false to pause.
     */
    function pauseAdapter(bytes32 _adapter, bool _remainActive) external onlyOwner {
