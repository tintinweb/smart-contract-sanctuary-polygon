...
contract FetcchRegistry is OwnableUpgradeable, helper {
    /// @dev address of provider registry
    address public providerRegistryImplementation;
    ...
    /// @dev array of providers info
    ProvidersInfo[] internal Providers;

    ///@dev mapping to check if provider exist
    mapping(string => bool) internal providerExists;

    ///@dev errors
    error InvalidAddress();

    function initialize(address _proxyContract) public initializer {
        providerRegistryImplementation = _proxyContract;
        __Ownable_init();
        __Ownable_init_unchained();
    }

    /// @notice This function is responsible for registring provider and deploys new proxy
    /// @dev only owner can call this function
    /// @param _providerName provider name
    /// @param _owner provider address
    /// @param _fetcchAdmin address of fetcch admin
    /// @param _logo logo of provider
    /// @param _metadata metadata of provider
    /// @param _type metadata type of provider
    function acquireNamespace(
        ...
        string memory _name = _toLower(_providerName);
        require(checkStr(_name), "Invalid Id");
        if (_fetcchAdmin == address(0)) revert InvalidAddress();
        if (_owner == address(0)) revert InvalidAddress();

        require(!providerExists[_name], "Provider already exists");
        providerExists[_name] = true;

        bytes memory data = abi.encodeWithSignature(
            "initialize(string,address,address,string,uint8)",
            _name,
            _owner,
            _fetcchAdmin,
            _metadata,
        ...
        address proxy = address(
            new TransparentUpgradeableProxy(
                providerRegistryImplementation,
                msg.sender,
                data
            )
        );

        ProvidersInfo memory _providerInfo = ProvidersInfo(
            _name,
            _owner,
            _logo,
            proxy,
        ...
        Providers.push(_providerInfo);

        emit ProviderCreated(_name, _owner, _logo, proxy, _metadata, _type);
    }

    /// @notice This function returns if a provider exist
    /// @param _name Provider name
    function checkProvider(string memory _name) external view returns (bool) {
        return providerExists[_name];
    }

    /// @notice This function is responsible for changing provider registry
    /// @dev only owner can call this function
    /// @param _providerRegistry address of new provider registry
    function changeProviderRegistry(
        address _providerRegistry
    ) external onlyOwner {
        if (_providerRegistry == address(0)) revert InvalidAddress();
        providerRegistryImplementation = _providerRegistry;
    }

    /// @notice This function returns all data of providers
    function getProviders() external view returns (ProvidersInfo[] memory) {
        return Providers;
    }
}
