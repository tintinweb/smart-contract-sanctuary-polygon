...
contract RewardsWinningFound is Ownable { 
    ...
    uint256 public rewardPortion = 1000000; // 0.0001%  //1000000 => Arround 50 BZAI at begining
    ...
    modifier onlyGame(){
        require(
            msg.sender == gameAddresses.getGameAddress()
            ,"only Game auth");
        _;
    }
    ...
    function setRewardPortion(uint256 _rewardPortion) external onlyOwner{
        require(_rewardPortion >= 10000 && _rewardPortion <= 100000000,"Value forbiden");
        rewardPortion = _rewardPortion;
    } 

    function _updateRewards() private returns(bool){
        IReserveForWinRewards I = IReserveForWinRewards(gameAddresses.getReserveWinAddress());
        ...
        }else if(block.timestamp >= (_lastRewardUpdate + 3600)){
            _lastRewardUpdate = block.timestamp;
    ...
    // TODO use the zai level to get more reward 
    function getWinningRewards(uint256 level) external onlyGame returns(uint256){
        if(!reserveFinished){
            _updateRewards();
        }
        uint256 multiplier;
        ...
        //each level add to multiplier 0.018 => 0.1(level 0) => 1(level 50) 
        uint256 zaiRewardPortion = rewardPortion * 1000 / multiplier;

        uint256 _toSend = BZAI.balanceOf(address(this)) / zaiRewardPortion; 
