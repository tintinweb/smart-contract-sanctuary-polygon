...
contract XVRPetraNFTSale is
    IERC721Receiver,
    Pausable,
    Ownable,
    ReentrancyGuard
{
    event BuyNFT(address buyer, uint256 amountOfETH);
    event AuctionBid(address bidder, uint256 amountOfETH);
    event AuctionEnd(uint256 tokenId, address winner, uint256 amountOfETH);

    bool auctionLive = false;

    XVRPetraNFT public PETRA;

    uint256 price = 0.1 ether;
    uint256 nftsForSale = 10;

    uint256 startDate = 0;
    uint256 endDate = 0;

    Bid public highestBid = Bid(address(0), 0, 0);

    constructor(address payable petraAddress) {
        // NOTE: You must set this as the owner of the Petra NFT contract and give it the 0th token
        PETRA = XVRPetraNFT(petraAddress);
    }

    // ********** NFT purchase functions **********

    function buy() external payable whenNotPaused {
        require(msg.value >= price, "Must send at least the price");
        require(
            PETRA.totalMinted() <= nftsForSale,
            "All tokens have been minted"
        );
        require(
            PETRA.balanceOf(msg.sender) < 5,
            "You already own more than 5 tokens"
        );

        emit BuyNFT(msg.sender, msg.value);

        PETRA.safeMint(msg.sender);
    }

    // ********** Auction functions **********

    function bid() external payable nonReentrant whenNotPaused {
        require(auctionLive, "Auction has not started");
        require(msg.value > highestBid.bidAmount, "Bid must be higher");
        require(checkAuction(), "Auction is not live");

        emit AuctionBid(msg.sender, msg.value);

        // Refund previous highest bidder. The failure of this function should not block the auction, so we don't check the return value.
        bool _ignored = payable(highestBid.bidder).send(highestBid.bidAmount);
        _ignored = _ignored;

        highestBid = Bid(msg.sender, msg.value, block.timestamp);
    }

    function concludeAuction() external nonReentrant whenNotPaused {
        require(auctionLive, "Auction has been stopped");
        require(block.timestamp > endDate, "Auction is still live");
        require(highestBid.bidder != address(0), "No bids have been made");

        auctionLive = false;

        emit AuctionEnd(0, highestBid.bidder, highestBid.bidAmount);

        PETRA.transferFrom(address(this), highestBid.bidder, 0);
    }

    // ********** Owner functions **********

    function startAuction(uint256 _endDate) external onlyOwner whenNotPaused {
        require(auctionLive == false, "Auction is already live");
        require(
            _endDate > block.timestamp,
            "End date must be later in time than start date"
        );
    ...
    function stopAuction() external onlyOwner whenNotPaused {
        require(block.timestamp < endDate, "Auction already ended");
        auctionLive = false;
    }
    ...
    function withdraw() external onlyOwner whenNotPaused {
        ...
        PETRA.withdraw(payable(msg.sender));
    }

    function setSalePrice(uint256 _price) external onlyOwner whenNotPaused {
        price = _price;
    }

    function setNFTsForSale(uint256 _nftsForSale)
        external
        onlyOwner
        whenNotPaused
    {
    ...
    function checkAuction() public view returns (bool) {
        require(startDate != 0 && endDate != 0, "Auction has not started");
        require(auctionLive, "Auction has been stopped");
    ...
    function onERC721Received(
        ...
        // We want to receive the 0th token from the Petra NFT contract
        // so that we can transfer it to the winner of the auction

        require(msg.sender == address(PETRA), "Only Petra NFTs can be sent");
        require(tokenId == 0, "Only token 0 can be sent");
...
/// @custom:security-contact [email protected]
contract XVRPetraNFT is ERC721Enumerable, ERC2981, Ownable {
    using Strings for uint256;
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;

    constructor() ERC721("XVR Petra NFT", "PETRA") Ownable() {
        // 10% royalty
        _setDefaultRoyalty(address(this), 1000);
    }

    function _baseURI() internal pure override returns (string memory) {
        return "https://xvrm.parlour.construction/a/";
    }
...
// SPDX-License-Identifier: MIT
// OpenZeppelin Contracts (last updated v4.8.0-rc.0) (token/ERC721/extensions/ERC721Consecutive.sol)
...
// SPDX-License-Identifier: MIT
// OpenZeppelin Contracts (last updated v4.8.0-rc.0) (token/ERC721/ERC721.sol)
...
// SPDX-License-Identifier: MIT
// OpenZeppelin Contracts (last updated v4.8.0-rc.0) (interfaces/IERC2309.sol)
...
// SPDX-License-Identifier: MIT
// OpenZeppelin Contracts (last updated v4.8.0-rc.0) (utils/Checkpoints.sol)
...
// SPDX-License-Identifier: MIT
// OpenZeppelin Contracts (last updated v4.8.0-rc.0) (utils/math/SafeCast.sol)
