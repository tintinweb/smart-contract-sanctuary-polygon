...
interface IUtilityManager{
    ...
    function updateAPR(uint256 _id, uint256 _rate) external;
...
abstract contract AbstractRewards is IAbstractRewards {
    ...
    uint256 constant VESTING_PERIOD = 600;
...
abstract contract BasePool is ERC20Votes, AbstractRewards, IBasePool, TokenSaver {
    ...
    function checkClaimsVestingPeriod(address _receiver) view external returns(bool){
        bool isReadyForTransfer;
        
        for(uint256 i = 0; i < depositsOf[_receiver].length; i++) {
            if(block.timestamp >= depositsOf[_receiver][i].end){
    ...
    function claimRewards(address _receiver) external {
        ...
        if(rewardAmount != 0){
            ...
            withdrawnRewards[_receiver] = withdrawnRewards[_receiver] + rewardAmount;
            rewardToken.safeTransfer(_receiver, rewardAmount);
            // update claimed rewards
            IUtilityManager(utilityManager).updateClaimedAmount(rewardAmount);
            // update unclaimed rewards
            IUtilityManager(utilityManager).updateUnclaimedAmount(rewardAmount,0);

            for(uint256 i=0; i < depositsOf[_receiver].length; i++) {
                if(block.timestamp >= depositsOf[_receiver][i].end){
                    totalAmount += depositsOf[_receiver][i].amount;
                }
            }

            IUtilityManager(utilityManager).updateAPR(0,rewardAmount);
        }
...
contract TimeLockPool is BasePool, ITimeLockPool {
    ...
    constructor(
        ...
        address _utilityManager
    ) BasePool(_name, _symbol, _depositToken, _rewardToken, _utilityManager) {
    ...
    function deposit(uint256 _amount, uint256 _duration, address _receiver) external override {
        ...
        IUtilityManager(utilityManager).updateStakedAmount(0, _amount);
        IUtilityManager(utilityManager).updateTotalValueLockedAmount(0, _amount, 1);
    ...
    function withdraw(uint256 _depositId, address _receiver) external {
        ...
        IUtilityManager(utilityManager).updateTotalValueLockedAmount(0, userDeposit.amount, 0);
...
contract FossilPool is TimeLockPool {
    constructor(
        string memory _name,
        string memory _symbol,
        address _depositToken,
        ...
        uint256 _maxLockDuration,
        address _utilityManager
    ) TimeLockPool(_name, _symbol, _depositToken, _rewardToken, _maxBonus, _maxLockDuration, _utilityManager) {
