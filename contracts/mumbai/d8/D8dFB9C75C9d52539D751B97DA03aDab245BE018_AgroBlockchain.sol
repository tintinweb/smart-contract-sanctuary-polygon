...
contract AgroBlockchain is ReentrancyGuard, SoftAdministered, ERC721A {
    ...
    // Price of each token
    uint256 public initial_price = 0.002 ether;
    uint256 public price;

    // USDC token contract
    IERC20 public usdc;

    // Conversion factor between Ether and USDC
    uint256 private constant ETHER_TO_USDC = 10 ** 6;
    ...
    function mintToken(uint256 _amount) public {
        require(_amount > 0, "mintToken: Amount must be greater than 0");

        /// @dev Get the total supply
        uint256 supply = totalSupply();

        /// @dev Check if the sale is active
        require(saleActive, "mintToken: Sale isn't active");

        /// @dev Check if  total supply + amount is less than max supply
        require(
            supply + _amount <= MAX_SUPPLY,
            "mintToken: Can't mint more than max supply"
        );

        // Calculate the total amount of USDC required
        uint256 usdcAmount = (price * _amount * ETHER_TO_USDC) / 1 ether;

        // Transfer USDC tokens from the caller to the contract
        require(
            usdc.transferFrom(msg.sender, address(this), usdcAmount),
            "mintToken: Failed to transfer USDC tokens"
        );
    ...
    function mintReserved(
        ...
        require(
            supply + _amount <= MAX_SUPPLY,
            "mintReserved: Can't mint more than max supply"
        );
        ...
        require(
            usdc.transferFrom(msg.sender, address(this), usdcAmount),
            "mintReserved: Failed to transfer USDC tokens"
        );
        /// @dev Minting
    ...
    /**
     * @dev function return all tokens owned by an address
     * @param _owner The owner to get the tokens of
     */
    ...
    /**
     * @dev function to set the sale active or not
     * @param val The value to set the sale to
     */
     ...
     * @param _baseTokenURI The base URI to set
     */
    ...
    /**
     * @dev Returns the base URI set via {_setBaseURI} or the empty string if no
     */
     ...
     * @param _addr The address to withdraw to
    ...
    function withdraw(address _addr, uint256 _amount) external onlyOwner {
        ...
        require(
            ...
            "withdraw: Insufficient USDC balance"
        );

        require(
            usdc.transfer(_addr, _amount),
            "withdraw: Failed to send USDC tokens"
        );
    }
}
