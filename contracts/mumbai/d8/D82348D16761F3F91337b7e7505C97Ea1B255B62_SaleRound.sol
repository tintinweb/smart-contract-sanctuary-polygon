...
contract SaleRound is ISaleRound, Ownable {
    ...
    constructor(
        ...
        _availableCurrency[usdt] = true;
        _availableCurrency[usdc] = true;
    ...
    /**
     * @dev See {IPrivateSale-buyMGB}.
     */
    function buyMGB() external payable isFinish {
        uint256 amountMATIC = msg.value;
        uint256 amountUSD = _getPrice(amountMATIC);

        if (amountUSD > _maxContribution || amountUSD < _minContribution) {
        ...
        if (_soldAmount > _totalAmount) {
            revert ExceedingMaxSold(_totalAmount - _soldAmount);
        }

        UserData storage userData = _userData[msg.sender];
        userData.buyAmount += amountMGB;
        uint256 avaiAmount = _calcAvaiableAmount(msg.sender);
        if (avaiAmount == 0) {
            revert ZeroAmount();
        }
     ...
     * @dev See {IPrivateSale-buyMGB}.
     */
    function buyMGB(address usdAddr, uint256 usdAmount)
        external
        ...
        isFinish
    {
        ...
        if (usdAmount > _maxContribution || usdAmount < _minContribution) {
            revert MinMaxContribution();
        }
        ...
        IERC20(usdAddr).safeTransferFrom(msg.sender, address(this), usdAmount);
        IERC20(_MGBAddress).safeTransfer(msg.sender, avaiAmount);

        emit BuyMGBOfUSD(msg.sender, amountMGB, usdAmount, avaiAmount);
    }
     ...
     */
    function getAvaiableAmount(address account)
        external
        view
        returns (uint256)
    {
        return (_calcAvaiableAmount(account));
    }
     ...
     */
    function getInfo() external view returns (PrivateSaleInfo memory) {
        return
            PrivateSaleInfo(
                _MGBAddress,
                _totalAmount,
    ...
    function _calcAvaiableAmount(address account)
        internal
        view
    ...
    function _getPrice(uint256 amount) internal view returns (uint256) {
        ...
        address usd = 0xc2132D05D31c914a87C6611C10748AEb04B58e8F;
...
interface ISaleRound {
     ...
     */
    function buyMGB() external payable;
     ...
     */
    function buyMGB(address usdAddr, uint256 usdAmount) external;
