...
import "./MagnetCoreLib.sol";

library ProgramsLib {
    bytes32 internal constant NAMESPACE = keccak256("magnet.lib.programs");

    struct Storage {
        address rootP;
        address ownerP;
        address tokenWalletP; // Адрес кошелька для токена
        address presentsWalletP; // Адрес кошелька для розыгрышей
        bool lockedP; // No reentrance
        uint[16] levelsPrice; // TODO: public Стоимости уровней
        uint8 maxLevel; // Максимальный уровень, который можно купить
        uint8 groupBuyLimiter; // Уровень до которого будут покупаться все 3 программы
        bool initialized;
    }
    ...
    function getProgramsRoot() internal view returns (address) {
        return getStorage().rootP;
    }

    function getProgramsOwner() internal view returns (address) {
        return getStorage().ownerP;
    }

    function getGroupBuyLimiter() internal view returns (uint8) {
        return getStorage().groupBuyLimiter;
    }

    function setGroupBuyLimiter(uint8 _newGroupBuyLimiter) internal {
        Storage storage s = getStorage();
        s.groupBuyLimiter = _newGroupBuyLimiter;
    }

    function getLevelsPrice() internal view returns (uint[16] memory) {
        return getStorage().levelsPrice;
    }

    function setLevelsPrice(uint[16] calldata _newPrices) internal {
        Storage storage s = getStorage();
        s.levelsPrice = _newPrices;
    }

    function getMaxLevel() internal view returns (uint8) {
        return getStorage().maxLevel;
    }

    function setMaxLevel(uint8 _newMaxLevel) internal {
        Storage storage s = getStorage();
        s.maxLevel = _newMaxLevel;
    }

    // modifier NoReentrancy() {
    //     require(!lockedP, "No re-entrancy");
    //     lockedP = true;
    //     _;
    //     lockedP = false;
    // }

    function _checkGroupBuy(
        uint8 prgm,
        uint userId
    ) external view returns (uint8) {
        uint8 l1 = MagnetCoreLib.getUserLevel(userId, 0);
        uint8 l2 = MagnetCoreLib.getUserLevel(userId, 1);
        uint8 l3 = MagnetCoreLib.getUserLevel(userId, 2);
        uint8 groupBuyLimiter = ProgramsLib.getGroupBuyLimiter();
        // 0 - все уровни выше лимитера
        if (
            groupBuyLimiter <= l1 &&
            groupBuyLimiter <= l2 &&
            groupBuyLimiter <= l3
        ) return 0;
        // 1 - все уровни одинаковые и ниже лимитера
        if (l1 == l2 && l2 == l3 && l3 < groupBuyLimiter) return 1;
        // 0 - Покупается минимальный уровень в одной программе (в других уже есть этот уровень) - возвращаем тоже 0
        if (prgm == 0 && l1 < l2 && l1 < l3) return 0;
        if (prgm == 1 && l2 < l1 && l2 < l3) return 0;
        if (prgm == 2 && l3 < l1 && l3 < l2) return 0;
        // 2 - Покупается два минимальных уровня (в одной программе уже есть этот уровень)
        if (prgm == 0 && l1 == l2 && l1 < l3) return 12;
        if (prgm == 1 && l2 == l3 && l2 < l1) return 23;
        if (prgm == 2 && l3 == l1 && l3 < l2) return 31;
        // 3 - Возврат с ошибкой
        return 3;
    }

    function _findNewProgramSecondReferrer(
        uint[] memory _binarReferrers,
        uint8 levelNumber
    ) public view returns (uint) {
        uint findedReferrer;
        ...
        for (
            uint i = 0;
            i < _binarReferrers.length || findedReferrer != 0;
            i++
        ) {
            if (!_existsBinarPlace(_binarReferrers[i], levelNumber)) {
                newBinarSublineLength += MagnetCoreLib.getP2ReferralsCount(
                    _binarReferrers[i],
                    levelNumber
                );
            } else {
        ...
        if (findedReferrer == 0) {
            ...
            for (
                ...
                uint bLength = MagnetCoreLib.getP2ReferralsCount(
                    _binarReferrers[i],
                    levelNumber
                );
                for (uint a = 0; a < bLength; a++) {
                    _newBinarSubLine[j] = MagnetCoreLib.getP2Referral(
                        _binarReferrers[i],
                        levelNumber,
                        a
                    );
            ...
            findedReferrer = _findNewProgramSecondReferrer(
                _newBinarSubLine,
                levelNumber
            );
        }
    ...
    function _existsBinarPlace(
        uint uid,
        uint8 levelNumber
    ) internal view returns (bool) {
        return
            MagnetCoreLib.getP2ReferralsCount(uid, levelNumber) < 2
                ? true
                : false;
    }

    function _setLevelsProgramSecond(
        uint uid,
        uint referrer,
        uint8 levelNumber
    ) external {
        ...
        uint binarReferrer = _findNewProgramSecondReferrer(
            _binarPopentialReferrers,
            levelNumber
        );
        MagnetCoreLib.addUserProgramIds(1, levelNumber, uid);
        MagnetCoreLib.setUserProgramId(
            1,
            levelNumber,
            uid,
            MagnetCoreLib.getProgramIdsLength(1, levelNumber) - 1
        );
        MagnetCoreLib.setProgramSecondReferrer(uid, levelNumber, binarReferrer);
        MagnetCoreLib.addProgramSecondReferral(binarReferrer, levelNumber, uid);
    }

    function _setLevelProgramThird(uint uid, uint8 levelNumber) external {
        //Проверяем закрыт ли ряд тринара
        uint row = MagnetCoreLib.getUsersP3Length(levelNumber) - 1;
        if (!MagnetCoreLib.isP3BlockClosed(levelNumber, row)) {
            if (MagnetCoreLib.getUserP3(levelNumber, row, 1) == 0) {
                MagnetCoreLib.setUserP3(levelNumber, row, 1, uid);
                MagnetCoreLib.setP3UserId(uid, levelNumber, 1);
            } else {
                MagnetCoreLib.setUserP3(levelNumber, row, 2, uid);
                MagnetCoreLib.setP3UserId(uid, levelNumber, 2);
                MagnetCoreLib.setP3BlockClosed(levelNumber, row);
            }
        } else {
            row++;
            MagnetCoreLib.addP3NewRow(levelNumber, uid);
            MagnetCoreLib.setP3UserId(uid, levelNumber, 0);
        }
        MagnetCoreLib.setP3UserRow(uid, levelNumber, row);
        MagnetCoreLib.addUserProgramIds(2, levelNumber, uid);
        MagnetCoreLib.setUserProgramId(
            2,
            levelNumber,
            uid,
            MagnetCoreLib.getProgramIdsLength(2, levelNumber) - 1
        );
    }
    ...
    function initialize(
        address _rootP,
        address _ownerP,
        address _tokenWalletP,
        address _presentsWalletP
    ) internal {
        // TODO: add all params in function arguments
        Storage storage s = getStorage();
        require(!s.initialized, "already initialized");
        s.levelsPrice[1] = 5;
        for (uint8 i = 2; i < 16; i++) {
            s.levelsPrice[i] = s.levelsPrice[i - 1] * 2;
        }
        s.rootP = _rootP;
        s.ownerP = _ownerP;
        s.tokenWalletP = _tokenWalletP;
        s.presentsWalletP = _presentsWalletP;
        s.maxLevel = 12;
        s.initialized = true;
        MagnetCoreLib.incrementId();
        MagnetCoreLib.addUserIds(1, _rootP);
        MagnetCoreLib.addUserMainAddress(1, _rootP);
        MagnetCoreLib.setProgramLevel(0, 1, 15);
        MagnetCoreLib.setProgramLevel(1, 1, 15);
        MagnetCoreLib.setProgramLevel(2, 1, 15);
        MagnetCoreLib.setUserMainReferrer(1, 0);
    }
}