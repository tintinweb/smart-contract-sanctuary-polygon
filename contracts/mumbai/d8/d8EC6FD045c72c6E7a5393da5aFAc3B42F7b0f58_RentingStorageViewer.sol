...
contract RentingStorageViewer is IRentingContract {
    ...
    function getFinishedRentPeriods(uint256 searchIdxFrom, uint256 searchIdxTo) external view returns (uint256[] memory) {
        require(searchIdxTo > searchIdxFrom, "Incorrect parameters");
        uint256[] memory ids = new uint[](searchIdxTo - searchIdxFrom);
        uint finishedCounter = 0;
        for (uint i = searchIdxFrom; i < searchIdxTo; i++) {
            uint256 landId = storageContract.rentedLandByIdx(i);
    ...
    function getTotalRentings() external view returns (uint256) {
        return storageContract.getTotalRentings();
    }
