    ...
    IContractOpenablePack721
{
    ...
    function createOrAddERC721Bundle(
        ...
        uint256 sumOfRewardUnits = tokenIds.length;
        ...
        if (isUpdate) {
            _addTokenInBundle(tokenIds, packId);
        } else {
            _createBundle(tokenIds, packId);
        }
    }
    function _createBundle(uint256[] calldata tokenIds, uint256 packId) internal {
        _addTokenInBundle(tokenIds, packId);
    }
    function _addTokenInBundle(uint256[] calldata tokenIds, uint256 packId) internal {
        uint256 len = tokenIds.length;

        for (uint256 t = 0; t < len; t++) {
            packTokens[packId].add(t);
        }
