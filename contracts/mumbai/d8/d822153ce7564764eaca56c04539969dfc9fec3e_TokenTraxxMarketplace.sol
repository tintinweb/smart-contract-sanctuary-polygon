    ...
    ErrorCodes
{
    ...
    function bootstrap() external onlyAdmin {
        ...
        require(
            _tokenListings[currentListing.assetContract][currentListing.tokenId].length == 0,
            throwError(CURRENCY_ALREADY_WHITELISTED) 
        );
        _tokenListings[currentListing.assetContract][currentListing.tokenId].push(i);
        for (i=1; i < listingIdTracker; i++){
    ...
    function getTokenListings(address _assetcontract, uint256 _tokenId) public view onlyAdmin returns (uint256[] memory) {
        require(_isListedAlready(_assetcontract, _tokenId), "asset not listed");
