    ...
    ITokenERC721
{
    ...
    function mint(
        ...
        if(bytes(updatedTokens[_tokenId]).length > 0) {
            ...
            require(updatedTokenPrice[_tokenId] == msg.value, "Invalid token price added.");
        } else {
    ...
    function addNewToken(uint256 _price, string calldata _uri) external  onlyRole(DEFAULT_ADMIN_ROLE) returns (uint256) {
        ...
        if(addedTokens == 0) addedTokens += 1; 
    ...
    function withdrawToken(address _receiver, address _token) external onlyRole(DEFAULT_ADMIN_ROLE) {}
