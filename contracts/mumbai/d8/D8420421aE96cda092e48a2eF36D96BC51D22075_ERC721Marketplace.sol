...
contract Auction is Initializable {
    ...
    mapping(uint256 => Types.Auction) public auctions;
    mapping(uint256 => Types.Bid[]) public bids;

    event AuctionCreated(Types.Auction auction);
    event AuctionCanceled(uint256 _auctionId);
    event AuctionFinalized(Types.Bid bid, Types.Auction auction);
    event BidSuccess(address _from, uint256 _auctionId, uint256 _amount, uint256 _bidIndex);

    function __Auction_init(address _validator_) internal onlyInitializing {
    ...
    function createAuction(
        ...
        (bool success, bytes memory result) = address(validator).call(
            abi.encodeWithSignature(
                "createAuction(address,uint256,address,uint256,address)",
                _collection,
                _id,
                _currency,
                _endTime,
        ...
        auctions[auctionIndex] = Types.Auction(
            auctionIndex,
            _collection,
            _id,
            _startTime,
        ...
        emit AuctionCreated(auctions[auctionIndex]);
    }
    ...
    function bidOnAuction(uint256 _id, uint256 amount) external payable {
        Types.Bid memory lastBid;

        if (bids[_id].length > 0) {
            lastBid = bids[_id][bids[_id].length - 1];
        }

        bids[_id].push(Types.Bid(msg.sender, amount));

        (bool success, bytes memory result) = address(validator).call{value: msg.value}(
            abi.encodeWithSignature(
                "bidOnAuction(uint256,address,uint256,uint256,uint256,bool,address,address,uint256,address)",
                amount,
                auctions[_id].currency,
                auctions[_id].startTime,
                auctions[_id].endTime,
                auctions[_id].startPrice,
                auctions[_id].valid,
                auctions[_id].owner,
                lastBid.from,
                lastBid.bidPrice,
        ...
        emit BidSuccess(msg.sender, _id, amount, bids[_id].length);
    }
     ...
     * @param _id auction id
     */
    function finalizeAuction(uint256 _id) external {
        Types.Bid memory lastBid;
        ...
        bool _valid = auctions[_id].valid;

        auctions[_id].valid = false;

        (bool success, bytes memory result) = address(validator).call(
            abi.encodeWithSignature(
                "finalizeAuction(uint256,address,address,bool,address,address,uint256,address)",
                auctions[_id].token_id,
        ...
        if (bids[_id].length == 0) {
            ...
            emit AuctionFinalized(lastBid, auctions[_id]);
        }
    }

    /**
     * @dev Function to claim NFT from auction
     * @param _id auction id
     */
    function claimAuction(uint256 _id) external {
        Types.Bid memory lastBid;
        ...
        (bool success, bytes memory result) = address(validator).call(
            abi.encodeWithSignature(
                "claimAuction(uint256,address,address,bool,address,uint256,address,uint256,address)",
                auctions[_id].token_id,
                auctions[_id].collection,
                auctions[_id].currency,
                _valid,
...
import {Auction} from "./executions/Auction.sol";
 ...
 */
contract ERC721Marketplace is Initializable, ERC721HolderUpgradeable, Auth, Swap, Auction, Offer, CollectionOffer {
