...
// 
contract Amos is ERC721EnumerableUpgradeable, IncrementableCounter {
    ...
    function initialize(
        ...
        require(
            _ACL != address(0),
            "Amos: initialize ACL for the zero address"
        );
    ...
    function setTokenURI(uint256 tokenId, string calldata uri)
        ...
        onlyController
    {
        require(_exists(tokenId), "Amos: set token URI for nonexistent token");
        _uris[tokenId] = uri;
    }
    ...
    function tokenOf(address owner) external view returns (uint256) {
        if(ERC721Upgradeable.balanceOf(owner) == 0)
            return 0;
    ...
    function _beforeTokenTransfer(
        ...
        require(
            balanceOf(to) == 0,
            "Amos: transfer token to address that holded an token"
        );
        super._beforeTokenTransfer(from, to, tokenId);
    }
}