...
contract Prediction is AccessProtected, Pausable {
    ...
    event LeagueUpdated(uint8 indexed leagueId, string name, string sport);
    ...
    function _addLeague(League calldata _league) private {
        require(
    ...
    function addLeagues(League[] calldata _leagues)
        external
        onlyAdmin
        whenNotPaused
    {
        for (uint256 i = 0; i < _leagues.length; i++) {
            _addLeague(_leagues[i]);
        }
    }
        ...
        whenNotPaused
    {
        ...
        leagues[_id] = _league;
        emit LeagueUpdated(_id, _league.name, _league.sport);
    }

    function addPool(Pool calldata _pool) external onlyAdmin whenNotPaused {
        require(_pool.matchIds.length > 0, "No Matches Present");
        require(_pool.totalRewards == 0, "Incorrect Rewards");
        require(
        ...
        for (uint256 i = 0; i < _pool.matchIds.length; i++) {
            ...
            require(
                matches.length >= matchId && matches[matchId].espnMatchId != 0,
                "Match Not present"
            );
        }
        pools.push(_pool);
        ...
        whenNotPaused
    {
        ...
        delete matchIndex[matches[matchId].leagueId][
            matches[matchId].espnMatchId
        ];
        matches[matchId] = matchData;
        matchIndex[leagueId][espnMatchId] = matchId;
        emit MatchUpdated(leagueId, espnMatchId, matchId);
    }
    ...
    function _addMatch(Match memory matchData) private {
        ...
        emit MatchAdded(leagueId, matchData.espnMatchId, matches.length - 1);
    }
        ...
        whenNotPaused
    {
        ...
        if (treasury != address(0) && treasuryPercentage != 0) {
            ...
            pools[prediction.poolId].totalRewards +=
            ...
            pools[prediction.poolId].totalRewards += pools[prediction.poolId]
                .fee;
        }
        emit PoolPredicted(
