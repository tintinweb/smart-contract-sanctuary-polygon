...
contract Neuromod is ERC721, AccessControlEnumerable, Ownable {
    ...
    function mint(address _owner, uint256 _tokenId) external {
        if (!hasRole(MINTER_ROLE, _msgSender())) revert MissingRole();
        if (_tokenId == 0 || _tokenId > MAX_SUPPLY) revert OutOfRange();
    ...
    function setBaseUri(string calldata _newBaseUri) external {
        if (!hasRole(DEFAULT_ADMIN_ROLE, _msgSender())) revert MissingRole();
    ...
    /**
     * @dev returns token ids owned by a owner, don t use this in another contract tho as it's gas intensive
     */
