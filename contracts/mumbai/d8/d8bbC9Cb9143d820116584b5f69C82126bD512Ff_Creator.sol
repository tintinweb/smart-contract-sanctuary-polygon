...
contract Creator is ERC1155, Ownable, ERC1155Supply {
    ...
    mapping(uint => launchpad_data) public launchpad_id;
    mapping(uint256 => string) private _uri;
    mapping(address => uint256[]) private ownerMint;
    mapping(uint256 => bool) public isMint;
    constructor(uint256 _fee, address _WETH, address _receiver, address _demask, uint _feeLockDemask, address _referral) ERC1155(""){
        _name = 'DeMask Creator';
        _symbol = 'DRC';
        feeProtocol = _fee; // 1% -> 100
        WETH = _WETH;
        receiver = _receiver;
        demask = _demask;
    ...
    function launchpad_submit(uint _id, uint _initial, uint _price, address _token, uint _totalSupply, uint _startTime, uint _endTime, string memory _url, bytes memory data) external IsMint(_id) VerifyTime(_startTime, _endTime){
        require(_price > 0 && _totalSupply > 0);
        launchpad_id[_id] = launchpad_data(msg.sender, _initial, (_totalSupply - _initial) , _totalSupply, _token, _price, _startTime, _endTime, data, _url);
        _setURI(_id, _url);
        if(_initial > 0) _mint(msg.sender, _id, _initial, data);
        emit LaunchpadSubmit(msg.sender, _id, _initial, _price, _token, _totalSupply, _startTime, _endTime, _url, data, block.timestamp);
    }
    
    function launchpad(uint _id, uint amount) external payable {
        launchpad_data storage launchpaddata = launchpad_id[_id];
        require(
           launchpaddata.startTime < block.timestamp && launchpaddata.endTime >= block.timestamp, "Creator: launchpad not available"
        );
        require(launchpaddata.available >= amount, "Creator: launchpad not enough");
        require(amount > 0);
        uint total = amount * launchpaddata.price;
        if(launchpaddata.token == WETH){
            if (msg.value > total) TransferHelper.safeTransferETH(msg.sender, msg.value - total);
            IWETH(WETH).deposit{value: total}();
        }
        _safeTransafer(launchpaddata.token, launchpaddata.creator, total);
        _mint(msg.sender, _id, amount, launchpaddata.data);
        launchpaddata.available -= amount;  
        emit Launchpad(msg.sender, launchpaddata.token, _id, total, amount, block.timestamp); 
    }
    ...
    function _safeTransafer(address _token, address _to , uint amount) internal {
        (uint amountFee, uint amountFeeReferral, uint amountReceive) = getAmount(amount);
        TransferHelper.safeTransferFrom(_token, msg.sender, _to, amountReceive); // owner
        TransferHelper.safeTransferFrom(_token, msg.sender, receiver, amountFee); // protocol
        TransferHelper.safeTransferFrom(_token, msg.sender, IReferral(referral).getReceiver(msg.sender), amountFeeReferral); // referral
    }
    function getAmount(uint amount) internal view returns (uint, uint, uint){
        uint amountFee = amount * feeProtocol / 10000;
        uint amountFeeReferral = amount * IReferral(referral).getFee() / 10000;
        uint amountReceive = amount - amountFee - amountFeeReferral;
        return (amountFee, amountFeeReferral, amountReceive);
    }
