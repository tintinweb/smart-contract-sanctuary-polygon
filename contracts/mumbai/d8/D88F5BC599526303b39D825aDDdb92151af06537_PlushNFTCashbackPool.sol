...
contract PlushNFTCashbackPool is Initializable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function initialize(Plush _plushCoin, uint256 _lockedTokens, uint256 _timeUnlock) initializer public
    {
        token = _plushCoin;
    ...
    function addTokensToAccount(address _wallet) public onlyRole(OPERATOR_ROLE)
    {
        if(getFreeTokens() >= lockedTokens){
            uint256 id = idsBalances[_wallet].length;

            allIds.push(id);
            idsBalances[_wallet].push(id);
            balanceInfo[id].balance = lockedTokens;
    ...
    function withdraw(uint256 _amount) external
    {
        ...
        require(getAvailableBalance(msg.sender) >= _amount, "Not enough balance.");
        require(token.transfer(msg.sender, _amount), "Transaction error.");

        decreaseWalletAmount(msg.sender, _amount);
    }

    function setLockedTokens(uint256 _amount) public onlyRole(OPERATOR_ROLE)
    {
    ...
    function getFreeTokens() private view returns(uint256)
    {
    ...
    function getAvailableBalance(address _wallet) private view returns(uint256)
    {
