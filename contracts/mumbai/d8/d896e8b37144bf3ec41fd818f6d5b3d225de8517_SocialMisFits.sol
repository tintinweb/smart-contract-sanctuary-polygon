...
contract SocialMisFits is ERC721Enumerable, Ownable, Pausable {
    ...
    mapping(address => string[]) private gameAttributes;
    ...
    function preSaleMint(address _to, uint256 _count) public payable whenNotPaused{
        ...
        require(msg.value >= _count * salePrice, "Less Amount");
    ...
    function upgradeAttributes(string memory _attribute) public onlyOwner {        
        attributes.push(_attribute);
    }

    function removeAttributes(string memory _attribute) public onlyOwner {        
        for(uint256 i = 0; i < attributes.length; i++){
            if (keccak256(abi.encodePacked(attributes[i])) == keccak256(abi.encodePacked(_attribute))) {
                attributes[i] = attributes[attributes.length-1];
    ...
    function userUpgradeAttributes(string memory _attribute) public {        
        gameAttributes[msg.sender].push(_attribute);
    }

    function userRemoveAttributes(string memory _attribute) public {        
        for(uint256 i = 0; i < gameAttributes[msg.sender].length; i++){
            if (keccak256(abi.encodePacked(gameAttributes[msg.sender][i])) == keccak256(abi.encodePacked(_attribute))) {
                gameAttributes[msg.sender][i] = gameAttributes[msg.sender][gameAttributes[msg.sender].length-1];
                delete gameAttributes[msg.sender][gameAttributes[msg.sender].length-1];
                break;
            }   
        }
    }

    function getAttributes(address _account) public view returns(string[] memory){       
        return gameAttributes[_account];
    }
