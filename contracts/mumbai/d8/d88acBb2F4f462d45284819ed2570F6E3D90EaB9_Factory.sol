    ...
    AccessControlUpgradeable
{
    ...
    function mintLoanNft(
        ...
        if (_details.isRefinance) {
            ...
            if (
                ...
                ).transferRewards(_details.borrower, _details.refinanceTokenID, _details.bank);
                ...
                ).transferRewards(_details.borrower, _details.refinanceTokenID, address(0));
            }
        }
        emit LoanCreated(
            _details.borrower,
    ...
    function prepaidPazaBuy(
        ...
        if(auctionreward){
            ...
            LoanLib.Loan memory temp = IPool(pools[_lender]).getUserDetails(_borrower,_loanId);
            pazaAmount = temp.discount - temp.borrowerShare;
        } else {
            receiver = _lender;
            pazaAmount = _amount;
        }
        if (usdc.balanceOf(prepaidUsdcWallet) >= (_amount * 10 ** usdc.decimals() ) /
            (Paza.getTokenPrice())) {
            trade.buyWithApproval(prepaidUsdcWallet, receiver, _amount);

            emit PazaBuyFromUsdcWallet(_serialNo, receiver, _amount);
        } else {
            ...
            emit PazaBuyFromCoinbase(_serialNo, receiver, (_amount));
        }
    }
    ...
    //     address _auctionWinner
    // ) external onlyRole(DEFAULT_ADMIN_ROLE) {
    //     require(isPoolExist[_auctionWinner] == true, "Bank Not Whitelisted");
