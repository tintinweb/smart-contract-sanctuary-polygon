    ...
    ReentrancyGuardUpgradeable
{
    ...
    // Mapping from address to mint possible count
    ...
    function mint(uint256 _mintAmount) public payable whenNotPaused {
        ...
        require(!whitelisted, "Only whitelisted can mint");
        require(_mintAmount > 0, "You need to mint more than one");
        require(
            _mintAmount <= maxMintAmount,
            "You have exceeded max mint amount"
        );
        require(supply + _mintAmount <= maxSupply, "Mint has been completed");
    ...
    function mintWhitelist(uint256 _mintAmount) external payable whenNotPaused {
        uint256 supply = totalSupply();

        require(whitelisted, "Whitelist is not active");
        require(
            _mintAmount <= maxMintAmount,
            "You have exceeded max mint amount"
        );
        ...
        require(_whiteList[msg.sender] >= 0, "Cannot mint anymore");
    ...
    function getWhitelistCount(address whiteListAddress)
        external
        view
        returns (uint256)
    {
    ...
    function removeWhitelist(address whitelistAddress) external onlyOwner {
        delete _whiteList[whitelistAddress];
    }
    ...
    {
        require(
            numAllowedToMint <= maxMintAmount,
            "Max Mint Amount Exceeded for Whitelisted User"
        );
    ...
    function setWhitelisted(bool _whitelist) public onlyOwner {
    ...
    function withdraw() external payable onlyOwner {
        require(
            ...
            "The balance of this address is zero"
        );
