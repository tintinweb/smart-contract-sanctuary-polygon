...
contract BattleSetOperator is Context, IRentingTypes, ReentrancyGuard, Pausable, Ownable {
    ...
    event BattleSetCreated(uint256 indexed landId, uint256[] botIds, address owner, Coin coin,
        uint256 price, bool perpetual, RentingType rt, uint share);
...
//    Coin[] private fixedPriceRentalsSupportedCoins = new Coin[](0);
    ...
    function setSupportedCoins(Coin[] memory fixedPriceCoins, Coin[] memory revShareCoins) external onlyOwner {
        fixedPriceRentalsSupportedCoins = fixedPriceCoins;
        revShareRentalsSupportedCoins = revShareCoins;
    }

    function setMarketplaceAddress(address xoiliumMarketplaceAddress) external onlyOwner {
        xoiliumMarketplace = IXoiliumMarketplace(xoiliumMarketplaceAddress);
    }
    ...
    function listBattleSetRevenueShared(uint256 landId, uint256[] memory botIds, Coin coin, uint256 guaranteeAmount,
        uint share, bool perpetual, address[] memory wallets) whenNotPaused external {
        require(guaranteeAmount != 0, "Guarantee amount should be grater then 0");
    ...
    function listBattleSetFixedPrice(uint256 landId, uint256[] memory botIds, Coin coin, uint256 price,
        bool perpetual, address[] memory wallets) whenNotPaused external {
    ...
    function listBattleSet(uint256 landId, uint256[] memory botIds, Coin coin, uint256 price,
        ...
        emit BattleSetCreated(landId, botIds,  _msgSender(), coin, price, perpetual, rt, share);
    }
