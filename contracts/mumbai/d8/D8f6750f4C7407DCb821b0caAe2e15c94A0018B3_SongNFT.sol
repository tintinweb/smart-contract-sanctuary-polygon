...
contract SongNFT is ERC721 {
    IManager private manager;
    string private baseURI;

    event NewSongId(uint256 tokenId, string SongId, uint256 timestamp);

    constructor(address _manager, string memory _newBaseURI)
        ERC721("SongToken", "SOTO")
    {
        manager = IManager(_manager);
        baseURI = _newBaseURI;
    }

    function mint(address to, uint256 tokenId, string memory SongId)
        external
        isPaused
        isManager(_msgSender())
    {
        _safeMint(to, tokenId);
        emit NewSongId(tokenId, SongId, block.timestamp);
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (from != address(0)) {
            require(
                manager.isNFTopenForAll() || manager.isNFTManager(_msgSender())
            );
        }
    ...
    modifier isManager(address _sender) {
        require(manager.isNFTManager(_sender));
        _;
    }
}
