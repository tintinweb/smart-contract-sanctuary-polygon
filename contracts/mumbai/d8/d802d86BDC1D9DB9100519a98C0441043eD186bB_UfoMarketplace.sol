    ...
    ReentrancyGuardUpgradeable
{
    ...
    event AcceptBider(address indexed _nftAddress, uint256 indexed _tokenId, address _bider);

    event FixedItemSuccessful(address indexed _nftAddress, uint256 indexed _tokenId, uint256 _totalPrice, address _buyer);
    ...
    function setWETHAddress(address wethAddr) external onlyOwner {
        require(weth_addr != wethAddr, Errors.SHOULD_NOT_SAME);
        weth_addr = wethAddr;
    }
    ...
    function buySellItem(
        ...
        require(_checkSignature(v, r, s, _info), Errors.SIGNATURE_IS_WRONG);
        
        IERC20Upgradeable crypto = IERC20Upgradeable(weth_addr);
        crypto.safeTransferFrom(msg.sender, _info.seller, _info.price);
        _transfer(_info.nftContract, address(this), msg.sender, _info.nftId);
        emit FixedItemSuccessful(_info.nftContract, _info.nftId, _info.price, msg.sender);
   }

    // function acceptBidonAuction(address _nftAddress, uint256 _tokenId) external nonReentrant {
    //     AuctionItem memory _auction = auctionItems[_nftAddress][_tokenId];
    //     require(_auction.startedAt > 0, Errors.SHOULD_BE_ON_SELL_STATUS);
    //     require(msg.sender == _auction.seller, Errors.ONLY_OWNER);
    //     require(_auction.bider != address(0), Errors.AT_LEAST_ONE_BIDER);

    //     _transfer(_nftAddress, _auction.seller, _auction.bider, _tokenId);
    //     payable(_auction.seller).transfer(_auction.price);

    //     emit AcceptBider(_nftAddress, _tokenId, _auction.bider);
    // }
...
abstract contract EIP712Signature {
    ...
    function _checkSignature(
        ...
        bytes32 hashStruct = keccak256(
            abi.encode(
                keccak256(
                    'NftInfo(uint256 nftId,address nftContract,string sellType,address seller,uint256 price,uint256 reservePrice,uint256 start,uint256 end,uint256 salt)'
                ),
                info.nftId,
                info.nftContract,
