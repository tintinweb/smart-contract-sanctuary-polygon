...
contract Counter is AutomationCompatibleInterface {
    ...
    {
        upkeepNeeded = airbnb.checkAndreturn()>0;
...
contract AirBNB is Ownable{
        ...
        for(uint i = 0;i<count;i++){
           ...
           if(block.timestamp > _bookingDetails.endTimeStamp){
            //    _bookingDetails.isBooked = false;
               ...
               PropertyDetails storage _Propertydetails = PropertyDetailsId[_bookingDetails.propertyId];
    ...
    function checkAndreturn() public view returns(uint){
        ...
        for(uint i = 0;i<count;i++){
           ...
           PropertyDetails memory property = PropertyDetailsId[bookingDetails.propertyId];

           if(block.timestamp > bookingDetails.endTimeStamp && property.isBooked){
               currentId++;
           }
