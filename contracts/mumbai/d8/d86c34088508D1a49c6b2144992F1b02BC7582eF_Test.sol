    ...
    function autoUpgradeLevel(address _user, uint8 level) private{
        if(!users[_user].activeX3Levels[level])
        {
            ...
            for(uint8 i=0;i<7;i++)
            {
                ...
                {
                    users[referrerAddress].withdrawable+=((levelUSD*(REFERRAL_PERCENTS[i]/1000))/1e20)*maticPerDollor;                
                    emit LevelBonus(referrerAddress, msg.sender,(levelUSD*REFERRAL_PERCENTS[i])/1000, ((levelUSD*REFERRAL_PERCENTS[i])/1e21)*maticPerDollor, i+1);
