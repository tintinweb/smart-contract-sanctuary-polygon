...
contract MasterProxy {
    ...
    // Storage position of the owner of the contract
    bytes32 private constant initializePosition = keccak256("com.lithium.proxy.initialize");

    function initialize(address owner) external{
        require(!initialized(),"already initialized");
        setUpgradeabilityOwner(owner);
        bytes32 position = initializePosition;
        assembly {
            sstore(position, true)
        }
    }

    /**
     * @dev Tells if contract is on maintenance
     * @return _initialized if contract is on maintenance
     */
    function initialized() internal view returns (bool _initialized) {
        bytes32 position = initializePosition;
        assembly {
            _initialized := sload(position)
        }
    }
