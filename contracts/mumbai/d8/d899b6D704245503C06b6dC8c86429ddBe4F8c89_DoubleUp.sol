...
contract DoubleUp is Ownable, ReentrancyGuard {
    ...
    function currentTarget() view public returns(uint256) {
        if(head == address(0)){return 0;}

        uint256 winMultiple = (winnerCounter % winnersUntilTriple) == 0 ? 3 : 2;
        return entries[head].collateral * winMultiple;
    }

    function entryState() view external returns(address[] memory addresses, uint[] memory immunities, uint[] memory amounts, string[] memory messages, uint256 balance, uint256 bonus, uint256 target){
        ...
        if(entriesLength == 0){
            return (addresses, immunities, amounts, messages, accumulatedBalance(), bonusPrize, currentTarget());
        }
