	...
	ReentrancyGuardUpgradeable
{
	...
	function fundOutFlow(
		...
		require(pendingClaimRequest(_policyId) == 0, "PCRN0");
		if (_value > 0) {
	...
	function withdraw(
		...
		require(PDS.isBlacklistUser(_to) != true, "BU");
		require(PDS.isBlacklistAssetsUser(_to, _asset) != true, "BAU");
	...
	function policyClaimInternal(ClaimPolicyParams memory _claimPolicyParams)
		...
		returns (bool)
	{
		require(_claimPolicyParams.asset != address(0), "IA");

		require(_claimPolicyParams.expiresAt > block.timestamp, "ET");
		...
		returns (uint256 claimId)
	{
		...
		require(
			isActive == true || block.timestamp >= policyData.claimExpires,
			"PNA"
		);
		...
		claimId = countClaimForRequest + 1;

		countClaimForRequest += 1;
		ClaimIdRequestData storage claimIdDataThisId = _claimIdData[claimId];
		require(setCAvoting(claimId), "SE");
		claimIdDataThisId.caSnapshotCount = CA.countCA();
		claimIdDataThisId.caSnapshotValue = S
			.workData(IStaking.StakingTypes.CA)
			.powerCount;

		_claimIdStatus[claimId] = 0;
		...
		_historyOfPolicyId[_claimPolicyParams.policyId]
			.claimPending += _claimPolicyParams.value;
		_historyOfPolicyId[_claimPolicyParams.policyId].claimIdLasted = claimId;

		_keepClaimIdUser[msg.sender].push(claimId);
		_keepClaimIdPolicy[_claimPolicyParams.policyId].push(claimId);
	 ...
	 */
	function cancelClaim(uint256 _claimId) public nonReentrant {
		require(NR.paused() == false, "CL");
		ClaimIdRequestData storage claimIdDataThisId = _claimIdData[_claimId];
		require(
			claimIdDataThisId.user == msg.sender ||
	...
	/**
	 * @notice canCancelClaim.
	 * @param _claimId The number value of claimId.
	 * @return The boolean value that the caller can cancel this claimId or not.
	 *
	 * Requirement:
	 *  - `msg.sender` must be your own claimId.
	 */
	function canCancelClaim(uint256 _claimId) public view returns (bool) {
		ClaimIdRequestData storage claimIdDataThisId = _claimIdData[_claimId];
		uint8 _claimIdStatusClaimId = claimIdStatus(_claimId);
		...
		if (
			...
			if (
				...
				if (claimIdDataThisId.asset != address(0)) {
					return true;
				}
			}
		}
	...
	function claimIdStatus(uint256 _claimId) public view returns (uint8) {
		require(_claimIdData[_claimId].user != address(0), "UZ");
		if (
