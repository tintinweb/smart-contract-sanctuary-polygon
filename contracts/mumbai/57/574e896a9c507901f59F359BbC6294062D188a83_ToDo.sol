...
contract ToDo is BaseRelayRecipient {
  ...
  function addToDo(string calldata _todo) public {
    _todoList[tx.origin].push(Memo(false, _todo));
  }

  function toggleToDoCompleted(uint8 index) public {
    _todoList[tx.origin][index].completed = !_todoList[tx.origin][index].completed;
  }

  function deleteToDo(uint8 index) public {
    uint256 end = _todoList[tx.origin].length - 1;
    for (uint8 i = index; i < end; i++) {
      _todoList[tx.origin][i] = _todoList[tx.origin][i + 1];
    }
    _todoList[tx.origin].pop();
  }

  function todoList() public view returns (Memo[] memory) {
    return _todoList[tx.origin];
  }
