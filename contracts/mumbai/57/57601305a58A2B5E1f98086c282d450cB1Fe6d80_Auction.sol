...
contract Auction is EIP712MetaTransaction, ReentrancyGuard {
  ...
  function bid(
    ...
    _transferBidToAuction(bidder, _wethAmount);
  ...
  function _transferBidToAuction(address _bidder, uint256 _wethAmount) private {
    require(
      weth.balanceOf(_bidder) >= _wethAmount,
      "Auction#_transferBidToAuction: Insufficient funds"
    );
    require(
      weth.allowance(_bidder, address(this)) >= _wethAmount,
