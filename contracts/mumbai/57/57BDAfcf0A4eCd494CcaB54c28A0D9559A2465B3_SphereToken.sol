...
contract SphereToken is ERC20Detailed, Ownable {
    ...
    function userTotalTax(address who) public view returns (uint256) {
        return calculateAdditionalTax(who).add(totalSellFee);
    }
    ...
    function calculateAdditionalTax(address userAddress)
        public
        view
        returns (uint256)
    {
        //make sure if the tax is not enabled, then return 0
        if (!isTaxBracket()) return 0;
        ...
        uint256 totalCap = userTotal * 100 / totalLiq;
    ...
    function getOwnAddressInIDexPair() internal view returns (uint256) {
        ...
        require(!isNone, 'LP does not have Sphere');
        return 0;
    }
