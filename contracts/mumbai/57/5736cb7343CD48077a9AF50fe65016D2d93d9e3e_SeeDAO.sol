...
contract SeeDAO is ERC1155, Ownable{
    ...
    event  OwnerMint(address indexed fromAddress,address indexed ownerMintAddress,uint256 indexed id,uint256 num);
    ...
    event SetTokenUri(uint256 indexed id,string tokenUri);
    ...
    //铸造新 ID NFT（onlyOwner 测试的时候不加）
    function ownerMint(string memory _tokenURI,uint256 _tokenID,uint256 _amount,uint256 _totalAmount) public { 
        require( _totalAmount >= _amount,"Sorry,the number of totalAmount should higher than the amount!");
        tokenIdAccountBool[msg.sender] = true;
        
        _mint(_ownerAccount, _tokenID, _amount, "");
       
        totalSupply[_tokenID] = _totalAmount;
        mintedNumber[_tokenID] += _amount;
       
        _setTokenUri(_tokenID, _tokenURI); 

        emit OwnerMint(address(this),msg.sender,_tokenID,_amount);
    } 
