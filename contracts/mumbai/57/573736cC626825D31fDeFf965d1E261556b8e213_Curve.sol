...
contract Curve is VRFConsumerBase {
    ...
    */
    ...
    function burn(uint256 tokenId) public virtual {
        ...
        if(isRareCenter(tokenId)) {
            burnPrice = getCurrentPriceToBurn().mul(100);
       } else if(isRare(tokenId)) {
            burnPrice = getCurrentPriceToBurn().mul(10);
        } else {
    ...
    function isRareCenter(uint256 tokenId) public pure returns (bool) {
        bytes memory bhash = abi.encodePacked(bytes32(tokenId));
        if(toUint8(bhash,4)/51 == 5) {
            return true;
        }  
        return false;
    }
    ...
    function generateSVGFromHash(bytes32 _hash) public virtual view returns (string memory) {
        ...
        */
        string memory svg = string(abi.encodePacked(
            "<svg viewBox='0 0 4000 2416'>", 
            string(abi.encodePacked("<path fill='",palette[toUint8(bhash,0)/51],"' stroke='#000' stroke-width='3' d='m779.1 1653.9 233.2-404.1 416.5-239.8 233.5 403.9v240z'/>")), // tile 1
            string(abi.encodePacked("<path fill='",palette[toUint8(bhash,1)/51],"' stroke='#000' stroke-width='3' d='m779.1 1670.5 235.3 401.8 412.3 239.8 235.6-410.2v-231.4z'/>")), // tile 2
            string(abi.encodePacked("<path fill='",palette[toUint8(bhash,2)/51],"' stroke='#000' stroke-width='3' d='M1677 1670.5h887.6l-231.3 393.4-414.4 248.2-241.9-401.8z'/>")), // tile 3
            string(abi.encodePacked("<path fill='",palette[toUint8(bhash,3)/51],"' stroke='#000' stroke-width='3' d='m2577.3 1670.5 410.1 239.8 233.5 401.8h-885.5v-229.3z'/>")), // tile 4
            string(abi.encodePacked("<path fill='",palette[toUint8(bhash,4)/51],"' stroke='#000' stroke-width='3' d='m2576.4 1654.2-230-405h873V1485l-232.5 406.4z'/>")), // tile 5
            string(abi.encodePacked("<path fill='",palette[toUint8(bhash,5)/51],"' stroke='#000' stroke-width='3' d='m1916.3 1010 411.7 237.7 230.3 406.2H1677v-240.6z'/>")), // tile 6
            string(abi.encodePacked("<path fill='",palette[toUint8(bhash,6)/51],"' stroke='#000' stroke-width='3' d='m1230.5 640.9 201.8-115.8h471.4v471.4l-233.5 398.1z'/>")), // tile 7
            string(abi.encodePacked("<path fill='",palette[toUint8(bhash,7)/51],"' stroke='#000' stroke-width='3' d='M1922.5 514.5 2156.6 106l400.7 235.7 123.6 203.4-758.4 442.8z'/>")), // tile 8
            string(abi.encodePacked("<path fill='",palette[toUint8(bhash,8)/51],"' stroke='#000' stroke-width='3' d='m2334.7 1230.4-406.5-228.3 762.1-440.2L2802.9 759v471.4z'/>")), // tile 9
            "</svg>"
        ));
