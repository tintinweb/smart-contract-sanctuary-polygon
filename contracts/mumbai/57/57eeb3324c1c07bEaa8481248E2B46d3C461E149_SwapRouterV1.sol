...
interface IGeneralPoolV1 is IBasePoolV1 {
    ...
    event SubmitResult(uint8 option, uint256 totalShares);
    ...
    function submitResult(uint8 option) external;
    function initialize(
...
interface ISwapRouterV1 {
    function generalPoolManager() external view returns (address);
    function fixedCryptoPoolManager() external view returns (address);

    function buy(bytes32 poolId, address to, uint8 poolType, uint8 option, uint256 amount) external returns (uint256 shares);
    function sell(bytes32 poolId, address to, uint8 poolType, uint8 option, uint256 shares) external returns (uint256 amount);
    function claimGeneralPool(bytes32 poolId, address to) external returns (uint256 amount);
    function claimFixedPool(bytes32 poolId, address to, uint256[] memory rounds) external returns (uint256 amount);

    function estimateBuy(bytes32 poolId, uint8 poolType, uint8 option, uint256 amount) external view returns (uint256 shares, uint256[] memory newReserves);
    function estimateSell(bytes32 poolId, uint8 poolType, uint8 option, uint256 shares) external view returns (uint256 amount, uint256[] memory newReserves);
}
...
contract SwapRouterV1 is ISwapRouterV1 {
    ...
    address public immutable override generalPoolManager;
    address public immutable override fixedCryptoPoolManager;

    constructor(address generalPoolManager_, address fixedCryptoPoolManager_) {
        generalPoolManager = generalPoolManager_;
        fixedCryptoPoolManager = fixedCryptoPoolManager_;
    }
    ...
    function buy(
        ...
        address poolManager;
        if (poolType == 0) poolManager = generalPoolManager;
        if (poolType == 1) poolManager = fixedCryptoPoolManager;
        address pool = IBasePoolManagerV1(poolManager).getPool(poolId);
        require(pool != address(0), "pool not found");
        address collateralToken = IBasePoolV1(pool).collateralToken();
        ...
        shares = IBasePoolV1(pool).buy(to, option);
    }
    ...
    function sell(
        ...
        address pool = IBasePoolManagerV1(poolManager).getPool(poolId);
        require(pool != address(0), "pool not found");
        IERC1155(pool).safeTransferFrom(msg.sender, pool, option, shares, "");
        amount = IBasePoolV1(pool).sell(to, option);
    }

    function claimGeneralPool(
        bytes32 poolId,
        address to
    ) external override returns (uint256 amount) {
        address pool = IBasePoolManagerV1(generalPoolManager).getPool(poolId);
        require(pool != address(0), "pool not found");
        uint8 option = IGeneralPoolV1(pool).resultOption();
        uint256 shares = IERC1155(pool).balanceOf(msg.sender, option);
        IERC1155(pool).safeTransferFrom(msg.sender, pool, option, shares, "");
        amount = IGeneralPoolV1(pool).claim(to);
    }
    ...
    function claimFixedPool(
        ...
        address pool = IBasePoolManagerV1(fixedCryptoPoolManager).getPool(poolId);
        require(pool != address(0), "pool not found");
        (uint256[] memory ids, uint256[] memory amounts) = IFixedCryptoPoolV1(pool).getClaimable(msg.sender, rounds);
        IERC1155(pool).safeBatchTransferFrom(msg.sender, pool, ids, amounts, "");
        return IFixedCryptoPoolV1(pool).claim(to, rounds);
    }
    ...
    function estimateBuy(
        ...
        uint8 option, 
        uint256 amount
    ) external view override returns (uint256 shares, uint256[] memory newReserves) {
        address poolManager;
        if (poolType == 0) poolManager = generalPoolManager;
        if (poolType == 1) poolManager = fixedCryptoPoolManager;
        IBasePoolV1 pool = IBasePoolV1(IBasePoolManagerV1(poolManager).getPool(poolId));
        require(address(pool) != address(0), "pool not found");
        require(option <= 1, "wrong option");
        uint16 tradeFeeRate = pool.tradeFeeRate();
        uint8 tradeFactor = pool.tradeFactor();
    ...
    function estimateSell(
        ...
        uint8 option, 
        uint256 shares
    ) external view override returns (uint256 amount, uint256[] memory newReserves) {
        address poolManager;
        if (poolType == 0) poolManager = generalPoolManager;
        if (poolType == 1) poolManager = fixedCryptoPoolManager;
        IBasePoolV1 pool = IBasePoolV1(IBasePoolManagerV1(poolManager).getPool(poolId));
        ...
        if (option == 0) {
            ...
            newReserves[1] = reserves[0].mulDiv(reserves[1], newReserves[0]);
            amount = shares.mulDiv(newReserves[0] + newReserves[1], newReserves[1]);
        } else {
            newReserves[1] = reserves[1] + shares;
            newReserves[0] = reserves[0].mulDiv(reserves[1], newReserves[1]);
            amount = shares.mulDiv(newReserves[0] + newReserves[1], newReserves[0]);
        }
    }
}