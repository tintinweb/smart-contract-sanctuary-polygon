...
contract RaffleFacet is RaffleModifiers {
    ...
    function pickWinners(uint256 _raffleId, uint256 weed) external isRaffled(_raffleId) returns (address[] memory winners){
       ...
       purchasable( _raffleId)
    {
        ...
        _fundManagerFacet().charge(s.raffles[_raffleId].price);
    }
        ...
        startTimeBeforeEndTime( _startTime, _endTime)
    {
        ...
        s.raffles[s.rafflesCount].paused = _paused;
    }
        ...
        
    {
        ...
        s.raffles[_raffleId].paused = _paused;
    }
...
contract RaffleModifiers is IRaffleFacet, Listing {
    ...
    modifier isActive(uint256 raffleId_) override {
        if( s.raffles[raffleId_].paused) revert QueryNonActiveRaffle();
        _;
    }
