...
contract DAIBond is AddressAccessor, Pausable {
    ...
    function deposit(uint256 amount) external whenNotPaused {
        ...
        USDRBonding(bonding).update(amount);
    ...
    {
        require(
            paused() || token != addressProvider.getAddress(UNDERLYING_ADDRESS)
        );
    ...
    function claim(uint256 amount) public {
        ...
        (address usdr, address susdr, address staking) = abi.decode(
            addressProvider.getAddresses(
                abi.encode(USDR_ADDRESS, sUSDR_ADDRESS, STAKING_ADDRESS)
            ),
        ...
        if (balance >= amount) {
            IERC20(susdr).approve(staking, amount);
            IStaking(staking).unstake(amount);
            IERC20(usdr).transfer(who, amount);
        } else {
            if (balance > 0) {
                IERC20(susdr).approve(staking, balance);
