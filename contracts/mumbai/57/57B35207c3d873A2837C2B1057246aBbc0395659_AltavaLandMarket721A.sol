...
contract AltavaLandMarket721A is
  Initializable,
  OwnableUpgradeable,
  UUPSUpgradeable,
  ...
  PausableUpgradeable
{
  ...
  event mintLandsAndRegisterForSaleEvent(
    address indexed to,
    uint256 indexed beginTokenId,
    uint256 quantity,
  ...
  function initialize(address landContract) public initializer {
    __Ownable_init();
  ...
  function mintLands(
    ...
    for (uint256 i = 0; i < quantity; i++) {
      ...
      Land memory newNftItem = Land({ tokenId: _tokenId, coinContract: coinContract, price: price });
   ...
   * 랜드 리스트 갯수 반환
   */
  function getLandListCount() public view virtual returns (uint256) {
   ...
   * 랜드 리스트 반환
   */
  function getLandList(uint256 start, uint256 end) public view virtual returns (Land[] memory) {
   ...
   * 랜드 반환
   */
  function getLand(uint256 tokenId) public view virtual returns (Land memory) {
    require(getLandListCount() > 0, 'landList is empty');
   ...
   * 랜드 리스트의 인덱스 반환
   */
  function getLandIndex(uint256 tokenId) public view virtual returns (uint256) {
    require(getLandListCount() > 0, 'landList is empty');
  ...
  // 판매 정보 반환
  function getSalesInfo() public view virtual returns (uint8, uint256, uint256) {
  ...
  function registerForSale(uint256 tokenId, address coinContract, uint256 price) public virtual nonReentrant {
    ...
    Land memory newNftItem = Land({ tokenId: tokenId, coinContract: coinContract, price: price });
    landList.push(newNftItem);
   ...
   */
  function buyByETH(uint256 tokenId, bytes32[] calldata proof) public payable virtual nonReentrant whenNotPaused {
    require(getLandListCount() > 0, 'landList is empty');

    // Public(0), etc(1), ...
    if (_salesType > 0) {
      // 둘 다 0이면 영역체크 안하는 상황이라고 가정
      if (!(_salesBeginId == 0 && _salesEndId == 0))
  ...
  ReentrancyGuardUpgradeable
{
  ...
  // totalSupplyLimit 수정
  function setTotalSupplyLimit(uint256 amount) public virtual onlyOwner {
    _totalSupplyLimit = amount;
  }
