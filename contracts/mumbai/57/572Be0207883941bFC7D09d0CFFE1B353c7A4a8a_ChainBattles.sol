...
contract ChainBattles is ERC721URIStorage {
    ...
    function getSkillPoints(uint256 tokenId) public view returns (string memory) {
        Stats memory _stats = tokenIdtoStats[tokenId];
    ...
    function train(uint256 tokenId) public {
        ...
        require(_stats.skillPoints > 0, "The Skill points of this NFT are finished, you can not train it anymore!");

        uint256 currentLevel = _stats.level;
        _stats.level = currentLevel + 1;

        uint256 currentAttack = _stats.attack;
        _stats.attack = currentAttack + getRandomNumber(currentAttack);

         uint256 currentDefense = _stats.defense;
        _stats.defense = currentDefense + getRandomNumber(currentDefense);

         uint256 currentSkillPoints = _stats.defense;
