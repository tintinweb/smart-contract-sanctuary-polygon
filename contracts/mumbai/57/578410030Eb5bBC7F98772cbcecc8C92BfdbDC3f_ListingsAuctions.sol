...
contract ListingsAuctions is OwnableUpgradeable, PausableUpgradeable, ERC2771Recipient, MulticallUpgradeable, ISubMarket {
    ...
    mapping(address => BitMaps.BitMap) private listed;                                                          // listed NFTs
    mapping(address => mapping(uint256 => Listing)) public listings;                                            // listings for each NFT

    mapping(address => BitMaps.BitMap) private auctioned;                                                       // nft for auction
     ...
     * @dev The owner of an NFT could unapprove the marketplace, which would cause this function to fail
     */
    function buyListedItemWithLowerPrice(address _nftAddress, uint256 _tokenId, uint256 _price, uint256 _creationTime, bytes calldata _signature, address _token) external {
    ...
    function _buyListedItem(address _nftAddress, uint256 _tokenId, uint256 _price, address _token) internal onlyListed(_nftAddress, _tokenId) whenNotPaused {
        ...
        marketplace.doTransferOfNFT(_nftAddress, _tokenId, listings[_nftAddress][_tokenId].seller, _msgSender());
     ...
     * @dev Can't be cancelled if there is a bid !
     */
    ...
    /**
     * @notice Method to cancel an ongoing auction if it has not been bidded yet
     * @param _nftAddress Address of the NFT
     * @param _tokenId Token ID of NFT
     */
        ...
        whenNotPaused 
    {
        ...
        emit AuctionCancelled(_msgSender(), _nftAddress, _tokenId);
    }
