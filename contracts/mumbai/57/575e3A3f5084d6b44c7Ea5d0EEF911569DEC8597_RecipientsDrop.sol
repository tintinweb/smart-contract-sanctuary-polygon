...
import "./MarketplaceRole.sol";

/// @author Amit Molek
/// @dev This base contract provides a guard check for enforcing only
/// approved marketplaces.
abstract contract BaseMarketplaceAccess is MarketplaceRole {
    ...
    function _checkAccessGuard(address operator, bool approved)
        ...
        virtual
    {
        bool isApproved = hasRole(MARKETPLACE_ROLE, operator);

        // After the operator's marketplace role was revoked,
        // the user can only unapprove it
        if (isApproved == false && approved == true) {
...
contract MarketplaceRole is AccessControl {
