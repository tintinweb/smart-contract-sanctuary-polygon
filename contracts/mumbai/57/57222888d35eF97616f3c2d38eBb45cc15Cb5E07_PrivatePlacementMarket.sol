...
contract PrivatePlacementMarket is OwnablePausable, ReentrancyGuardUpgradeable{
    ...
    event ClaimAmbassadorReward(address indexed ambassador, uint256 reward);
    struct Bag {
        uint256 amount;
        uint256 ttValue;
        uint256 stockValue;
    ...
    function setBag(uint256 index, uint256[] calldata bag_) external onlyOwnerOrOperator {
        Bag memory oldBag = _bags[index];
        if (oldBag.amount == 0) {
            bagSize += 1;
        }
        _bags[index] = Bag(bag_[0], bag_[1], bag_[2], bag_[3]);
        _stockNftIds.add(bag_[2]);
    }
    ...
    function claimAmbassadorReward() external whenNotPaused nonReentrant {
        ...
        emit ClaimAmbassadorReward(ambassador, reward);
    }
...
contract StakingReward is OwnablePausable, ReentrancyGuardUpgradeable {
    ...
    uint256 public BASE_REWARD_PER_SECOND; //1个代币单位 1%的apy 对应1个区块的奖励
    ...
    function calculatePendingRewards(address staker, uint256 toTimestamp) public view returns(uint256 rewardDebt, uint256 pendingRewards) {
    ...
    function _withdraw(address staker, uint256 amount) internal returns(uint256 withdrawn) {
        ...
        for (uint256 i; i < len; i++) {
            ...
            // if (userOrders[i].remainedAmount == 0) { //
            //     _finishedOrders[staker].push(userOrders[i]);
            ...
            //     len -= 1;
            //     i -= 1;
            //     require(userOrders.length == 5, "hahah");
            // }
    ...
    function apy() external view returns(uint256[] memory) {
        return _apys;
    }
...
interface ITdex {
    ...
    function setPrice(address tokenContract, uint256 price) external pure returns(uint256);
