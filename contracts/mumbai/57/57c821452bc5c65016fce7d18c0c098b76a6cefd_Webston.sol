...
contract Webston {
    ...
    function _setb14self(uint256 poolNo,address _referral,address _addr,uint256 _amount) private{
        ...
        if(players[_referral].b14_level[poolNo][0]<2){
            ...
            address _ref=players[_referral].b14_lr[poolNo][0];
            while(players[_ref].b14_lr[poolNo][0] != address(0)){
                if(players[_ref].b14_level[poolNo][0]<2){
                    _parent=players[_ref].b14_lr[poolNo][0];
                    break;
                }else{
                    _ref=players[_ref].b14_lr[poolNo][0];
                }
            }
        }
