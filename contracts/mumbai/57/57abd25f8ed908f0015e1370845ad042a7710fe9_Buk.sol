...
contract Buk is ERC1155, IERC1155Receiver, Ownable, ReentrancyGuard{
    ...
    function bookRoom(Booking[] memory booking,address user, bytes memory signature, string memory nonce) external {                 
        ...
        for(uint256 i=0; i< totalNfts; i++){
           ...
           // uint256 bukFee  = (booking[i].base_price * _bukCommission / 100);
            uint256 bukActualTotal = ((booking[i]._baseprice * platformFee / 100))- booking[i].discount;
    ...
    function cancelBookingUser(uint256 id, address user, string memory  nonce, bytes memory signature) external {
        ...
        uint256 bukComm = ((bookingDetails[id]._baseprice * platformFee / 100))- bookingDetails[id].discount; // bukFee and discounts deducted from Total paid amount 
        ...
        if( owner() == msg.sender || msg.sender == idToHotel[bookingDetails[id].hotelId].hotelManager){
            ...
            uint256 userAmount = ActualAmount - (bookingDetails[id].buk_cancellationFee + bookingDetails[id].Hotel_cancellationfee);
           
            IERC20(currency).transfer(treasury, bookingDetails[id].buk_cancellationFee); 
            IERC20(currency).transfer(idToHotel[bookingDetails[id].hotelId].hotelTreasury, bookingDetails[id].Hotel_cancellationfee);
