...
contract ChainBattles is ERC721URIStorage {
    ...
    mapping(uint256 => Skill) public tokenIdToLevels;
  ...
  function getLevels(uint256 tokenId)view public  returns (string memory) {
    Skill memory level = tokenIdToLevels[tokenId];
    return string(
        abi.encodePacked(
            level.level.toString(),
            ", Speed: ",
            level.speed.toString(),
            ", Strength: ",
            level.strength.toString(),
            ", Life: ",
            level.life.toString()
        )
    );
  ...
  function mint() public {
    ...
    tokenIdToLevels[tokenId] = skillInit;
