...
abstract contract IERC20Token is IERC20, Tokenomics, Liquifier, UnStucker {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        bool takeFee = true;
        if (isInPresale || inSwapAndLiquify) { takeFee = false; 
        } else {
...
contract BabyBurnCoin is IERC20Token {
    using SafeMath for uint256;
...
abstract contract Liquifier is Ownable {
    ...
    function liquify(uint256 contractTokenBalance, address sender) internal returns (bool) {
        if (numberOfTokensToSwapToLiquidity == 0) return false;
...
abstract contract Tokenomics is Antiwhale, Ownable {
    ...
    bool public isInPresale = true; // Prevetn Liqudity & Pre-Sale glitch
    // --------------------- Fees Settings ------------------- //
    ...
    uint256 internal totPendFees;
    uint256 public maxTotalFees = 200; // Max Settable Fees 20%

    // Fees Buy
    uint256 public feeRedistBuy = 10; // Redistribution 1%
    uint256 public feeCharityBuy = 80; // Charity 8%
    uint256 public feeMarketingBuy = 10; // Marketing 1%
    // Fees Sell
    uint256 public feeRedistSell = 20; // Redistribution 2%
    uint256 public feeCharitySell = 100; // Charity 10%
    uint256 public feeMarketingSell = 40; // Marketing 4%
    uint256 public feeLiquiditySell = 10; // Marketing 1%
    ...
    constructor(string memory tName,string memory tSymbol, uint totalAmount) Antiwhale(totalAmount) {
        ...
        charityAddress = 0xE4682db2A4D1A8833B977E8fb477A7b7e17956FB;
        marketingAddress = 0xfF064a27EdB1D3D015C65dA0b97E410f0902501C;
        burnAddress = address(0);

        _addFeesBuy(); _addFeesSell();
    }
    ...
    function _addFee(FeeType name, uint256 value, address recipient, bool buy) private {
        ...
        if (buy) {
            ...
            sumOfFeesBuy += value;
            require(sumOfFeesBuy <= maxTotalFees, "MAX: 20%");
        } else {
            feesSell.push(Fee(name, value, recipient, 0));
            sumOfFeesSell += value;
            require(sumOfFeesSell <= maxTotalFees, "MAX: 20%");
        }
    }
    
    function _addFeesBuy() private {
        delete feesBuy; delete sumOfFeesBuy;
        _addFee(FeeType.Rfi, feeRedistBuy, address(this), true);
        _addFee(FeeType.External, feeCharityBuy, charityAddress, true);
        _addFee(FeeType.External, feeMarketingBuy, marketingAddress, true);
    }

    function _addFeesSell() private {
        delete feesSell; delete sumOfFeesSell;
        _addFee(FeeType.Rfi, feeRedistSell, address(this), false);
        _addFee(FeeType.External, feeCharitySell, charityAddress, false);
        _addFee(FeeType.External, feeMarketingSell, marketingAddress, false);
        _addFee(FeeType.Liquidity, feeLiquiditySell, address(this), false); 
    }
    ...
    function setBuys(uint256 _redistribution, uint256 _charity, uint256 _marketing) external onlyOwner {
        feeRedistBuy = _redistribution;
        feeCharityBuy = _charity;
        feeMarketingBuy = _marketing;
        _addFeesBuy();
    }

    function setSells(uint256 _redistribution, uint256 _charity, uint256 _marketing, uint256 _liquidity) external onlyOwner {
        feeRedistSell = _redistribution;
        feeCharitySell = _charity;
        feeMarketingSell = _marketing;
        feeLiquiditySell = _liquidity;
        _addFeesSell();
    }
    ...
    function setConvBNB(bool value) external onlyOwner {
        feeInBNB = value;
    }
}
