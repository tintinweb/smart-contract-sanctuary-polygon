...
contract CryptoLoan is Ownable {
   ...
   function addUser(string memory email, string memory phone, uint256 loanAmount, uint256 endDate, uint256 period, uint256 interestRate, uint256 currentAssetValue, uint256[] memory installmentAmounts, uint256[] memory installmentDueDates) public payable{

        // Check if email is already in use
        require(bytes(addressToEmail[msg.sender]).length == 0, "Address already in use");
        ...
        require(
            (installmentAmounts.length == installmentDueDates.length), "Invalid Payment Schedule Details");
        // Add loan installments
        for (uint256 i = 0; i < installmentAmounts.length; i++) {
            uint256 installmentIndex = installments.length;
            Installment memory installment = Installment(installmentAmounts[i], installmentDueDates[i], 0, false);
    ...
    function getUserDetails(string memory email) public view returns (address, string memory, string memory, uint256, uint256, uint256, uint256, uint256, string memory, bool) {
        // Check if user exists
        require(bytes(users[email].phone).length != 0, "User email does not exist");
        ...
        return (getAddressByEmail(email), email, user.phone, user.collateralAmount, user.loanAmount, user.initialMarketPrice, user.startDate, user.endDate, user.remark, user.active);
    }

    // Function to get User Payment schedule
    function getUserPaymentSchedule(string memory email) public view returns(uint256[] memory installmentAmounts, uint256[] memory installmentDueDates, uint256[] memory installmentPaidDates, bool[] memory installmentPaid, uint256 unpaidInstallments){
        ...
        for (uint256 i = 0; i < installmentIndexes; ) {
            Installment storage installment = installments[user.installmentIndexes[i]];
    ...
    function editUser(string memory email, uint256 loanAmount, uint256 startDate, uint256 endDate, string memory remark, bool active) public onlyOwner{
        // Check if user exists
    ...
    function editPaymentSchedule(string memory email, uint256 index, uint256 paidDueDate, bool _paid) public onlyOwner{
        // Check if user exists
    ...
    function getAddressByEmail(string memory email) private view returns (address) {
        for (uint i = 0; i < userAddress.length; i++) {
            if (keccak256(bytes(addressToEmail[userAddress[i]])) == keccak256(bytes(email))) {
    ...
    function getEmailbyAddress(address _userAddress) public view onlyOwner returns (string memory) {
    ...
    function isAddressExist(address connectedAddress) public view returns(bool){
    ...
    function releaseCollateraltoBorrower(string memory email) external onlyOwner{
        User storage user = users[email];
        require(bytes(users[email].phone).length != 0, "User email does not exist");
    ...
    // Amount Repayment
    function loanAmountRepayment(string memory email, uint256 amount, uint256 index) private {
    ...
    function transferEthtoAddress(address payable recipient, uint256 amount) internal onlyOwner{
    ...
    function removeUserByEmail(string memory email) public onlyOwner {
        ...
        require(bytes(users[email].phone).length != 0, "User email does not exist");
        require(bytes(addressToEmail[userAddressToRemove]).length != 0, "User address does not exist");
        ...
        for (uint256 i = 0; i < userKeys.length; ) {
            if (keccak256(abi.encodePacked(userKeys[i])) == keccak256(abi.encodePacked(userKey))) {
                delete userKeys[i];
                break;
            }
            unchecked {
