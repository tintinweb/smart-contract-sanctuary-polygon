...
contract LostWorldValidator is CustomERC2771ContextUpgradeable, UUPSUpgradeable {
    ...
    function initialize(
        ...
        require(rbaLinkerAddress != address(0x0), 'LCED: RBA linker is zero address');
        require(nftAddress != address(0x0), 'LCED: NFT  is zero address');
        require(worldDataAddress != address(0x0), 'LCED: World data is zero address');
        LCRBALinker = ILostChainRBA(rbaLinkerAddress);
    ...
    function setLWD(address lostWorldAddress) public onlyRole('DEFAULT_ADMIN_ROLE', msg.sender) {
        require(lostWorldAddress != address(0x0), 'LWNFT: Lost chain NFT is zero address');
