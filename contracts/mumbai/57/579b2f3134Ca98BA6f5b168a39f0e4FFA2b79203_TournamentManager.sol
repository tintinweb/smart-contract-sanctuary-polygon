...
contract TournamentManager is VRFConsumerBaseV2 {
  ...
  struct Team {
    ...
    /* gotchiFormation[0][0] is the X position of first gotchi
        gotchiFormation[2][1] is the Y position of the third gotchi
     */
    uint32[5][2] gotchiFormation;    
    address owner;
    string name;
    uint256 teamId;
  }
  ...
  mapping(uint256 => bool) private gotchiRegistered;
  mapping(address => Team[]) private participantToTeams;
  ...
  Team[] private participantTeams;
  mapping(uint256 => Team) private teamIdToTeam;
  mapping(uint256 => bool) private teamIdStillInCompetition;
  ...
  constructor(
    ...
    gotchiDiamond = IAavegotchiDiamond(address(0x83e73D9CF22dFc3A767EA1cE0611F7f50306622e));
    s_keyHash = keyHash;
  ...
  function openEntry() public onlyOwner() {
    require(tournament_state == TOURNAMENT_STATE.CLOSED, "Tournament is not closed");
    tournament_state = TOURNAMENT_STATE.INSCRIPTION_OPEN;
  }
 
  function closeEntryAndSnapshot() public onlyOwner {
    require(tournament_state == TOURNAMENT_STATE.INSCRIPTION_OPEN, "Tournament is not open");
    tournament_state = TOURNAMENT_STATE.SHUFFLING;
    emit SnapshotTournament();
  }

  function shuffleAndStartTournament() public onlyOwner {
    require(tournament_state == TOURNAMENT_STATE.SHUFFLING, "Tournament not in shuffle mode");
    requestShuffleSeed();
  }

  function startRound() public onlyOwner {
    require(tournament_state == TOURNAMENT_STATE.BATTLE, "Tournament not in battle mode");
    require(round_state == ROUND_STATE.PREPARATION, "Round not in preparation");
    requestRoundSeed();
    round_state = ROUND_STATE.CALCULATING_WINNER;

  }

  function submitRound(address[] memory loosers) public onlyOwner {
    require(tournament_state == TOURNAMENT_STATE.BATTLE, "Tournament not in battle mode");
    require(round_state == ROUND_STATE.CALCULATING_WINNER, "Round not finished");
    removeLoosers(loosers);
    round_state = ROUND_STATE.PREPARATION;
    currentRound++;
  }

  function removeLoosers(address[] memory loosers) public onlyOwner{
    return;
  }

  function closeTournament() external onlyOwner() {
    require(tournament_state == TOURNAMENT_STATE.BATTLE, "Tournament not in battle mode");
    //require(PARTICIPANT RESTANT . LENGTH == 1)
  ...
  function removeParticipants(address[] memory addressesToRemove) public onlyOwner {
        for (uint256 i = 0; i < addressesToRemove.length; i++) {
            participants[addressesToRemove[i]] = false;
        }
        uint256 j = 0;
        for (uint256 i = 0; i < participantList.length; i++) {
            if (!participants[participantList[i]]) {
                continue;
            }
            if (i != j) {
                participantList[j] = participantList[i];
            }
            j++;
        }
    }
  ...
  function requestShuffleSeed() public onlyOwner {
  require(tournament_state == TOURNAMENT_STATE.SHUFFLING, "Tournament not in shuffle mode");
  require(randomRequest == false, "Wait until random request is done to request another random number");
  ...
  function requestRoundSeed() public onlyOwner {
    require(tournament_state == TOURNAMENT_STATE.BATTLE, "Tournament not in battle mode");
    require(randomRequest == false, "Wait until random request is done to request another random number");
  ...
  function cleanAllParticipants() public onlyOwner() {
    for (uint256 i = 0; i < participantList.length; i++) {
      delete participants[participantList[i]];
    }
  ...
  function enter() external {
    require(tournament_state == TOURNAMENT_STATE.INSCRIPTION_OPEN, "Tournament not open");
    require(feeToken.balanceOf(msg.sender) >= entryPrice, "Not enough GHST to pay for entrance fee");
    require(participants[msg.sender] == false,"User already registered");
    registerParticipant();
  }

  function registerParticipant() internal {
    feeToken.transferFrom(msg.sender, address(this), entryPrice);
    participants[msg.sender] = true;
    participantList.push(msg.sender);
  }

  function createTeam(uint256[5] memory gotchis, uint256 gotchiLeader, uint32[5][2] memory gotchiFormation, string memory teamName) public {
    require(gotchis.length == 5, "Invalid number of Gotchi IDs provided.");
    //require(checkGotchiFormationValidity1(gotchiFormation) == true, "Gotchi formation not valid");
    //require(checkGotchiFormationValidity2(gotchiFormation) == true, "Gotchi formation not valid");
    require(isGotchiLeaderInTeam(gotchis, gotchiLeader) == true, "Gotchi leader is not in the team");
    require(tournament_state == TOURNAMENT_STATE.INSCRIPTION_OPEN, "Tournament not open");
    require(participants[(msg.sender)] == true, "Participant didn't enter");
    require(checkGotchiTeamOwned(gotchis, msg.sender) == true, "Participant not owner of the gotchis");
    require(isGotchiTeamRegistered(gotchis) == false, "One or more gotchis already registered");
    registerTeam(wrapToTeam(gotchis, gotchiLeader, gotchiFormation, msg.sender, teamName));
  }


  // Push the team on the mapping address => Team[]
  // Register the gotchis so they can't be used again
  ...
  /*Remove the team teamId */
  function deleteTeam(uint256 teamId) internal {
    ...
    for (uint256 i = 0; i < participantTeams.length; i++) {
        if (participantTeams[i].teamId == teamId) {
            ...
            participantTeams[i] = participantTeams[participantTeams.length - 1];
  ...
  function wrapToTeam(uint256[5] memory gotchis, uint256 gotchiLeader, uint32[5][2] memory gotchiFormation, address owner, string memory teamName) internal pure returns (Team memory) {
    Team memory newTeam;
  ...
  function modifyTeamBeforeBattle(uint256 teamIndex, uint256[5] memory gotchis, uint256 gotchiLeader, uint32[5][2] memory gotchiFormation, string memory teamName) external {
    require(gotchis.length == 5, "Invalid number of Gotchi IDs provided.");
    require(tournament_state == TOURNAMENT_STATE.INSCRIPTION_OPEN, "Tournament not open");
    require(participants[(msg.sender)] == true, "Participants didn't enter");
    // TODO CHECK THAT GOTCHIS BELONG TO THE PEOPLE
    // Check that gotchis not already entered
    // If a gotchi is sent, if the new proprietaire enter his team, the gotchis should quit the last team
    participantToTeams[(msg.sender)][teamIndex] = wrapToTeam(gotchis, gotchiLeader, gotchiFormation, msg.sender, teamName);
  }

  function modiftyTeamAfterBattle(uint256 teamIndex, uint256[5] memory gotchis, uint256 gotchiLeader, uint32[5][2] memory gotchiFormation) external {
    require(gotchis.length == 5, "Invalid number of Gotchi IDs provided.");
    require(tournament_state != TOURNAMENT_STATE.CLOSED, "Tournament not open");
    require(participants[(msg.sender)] == true, "Participants didn't enter or is eliminated");
    // CHECK THAT THE 5 NEWS GOTCHIS ARE THE SAME (In different order) than the 5 olds gotchis
  }
  ...
  function getTeam(address participant, uint256 index) external view returns (Team memory) {
  ...
  function checkGotchiTeamOwned(uint256[5] memory gotchisId, address participant) internal view returns (bool)  {
    address realOwner;
    for (uint256 i = 0; i < 5 ; i ++)  {
      realOwner = gotchiDiamond.ownerOf(gotchisId[i]);
      if (realOwner != participant)
        return false;
    }
    return true;
  }

  function checkGotchiOwned(uint256 gotchiId, address participant) internal view returns (bool)  {
  ...
  function isGotchiTeamRegistered(uint256[5] memory gotchiId) public view returns (bool) {
    for (uint256 i = 0; i < 5 ; i ++)  {
      if (gotchiRegistered[gotchiId[i]] == true)
        return true;
    }
    return false;
  }

  function getTeamOwner(Team memory team) public view returns (address){
    return team.owner;
  }
  ...
  function getTeamName(Team memory team) public view returns (string memory){
    return team.name;
  }
  ...
  function getTeamFormation(uint256 teamId) public view returns (uint32[5][2] memory){
  ...
  function getTeamsStillInCompetition() public view returns(Team[] memory){
    // TODO
  }
  ...
  function checkGotchiFormationValidity1(uint32[5][2] memory gotchiFormation) public view returns (bool) {
    // Check X and Y positions range
    for (uint32 i = 0; i < 5; i++) {
        // Check X position range
        if (gotchiFormation[i][0] < 1 || gotchiFormation[i][0] > 5) {
            return false; // Invalid X position
        }
        
        // Check Y position range
  ...
  function checkGotchiFormationValidity2(uint32[5][2] memory gotchiFormation) public view returns (bool) {
    for (uint32 i = 0; i < 4; i++) {
        for (uint32 j = i + 1; j < 5; j++) {
            if (gotchiFormation[i][0] == gotchiFormation[j][0] && gotchiFormation[i][1] == gotchiFormation[j][1]) {
  ...
  function isGotchiLeaderInTeam(uint256[5] memory gotchis, uint256 gotchiLeader) internal pure returns (bool) {
