...
import "@openzeppelin/contracts/utils/math/SafeMath.sol";
 contract PledgeStakeV2 {
    ...
    // Calculate level rewards for the user's upline
    // Calculate level rewards for the user's upline
    function getLevelReward(address _user, uint256 _index) public view returns (uint256) {
        UserInfo storage user = userInfo[_user];
        address upline = user.referrer;
        ...
        for (uint256 i = 0; upline != address(0) && i < LEVEL_COUNT; i++) {
            ...
            if (userInfo[upline].level > i && userInfo[upline].totalStake > 0) {
                levelReward = levelReward.add(roundReward.mul(levelRewardPercentage).div(100));
            } else if (upline == defaultReferrer) {
                levelReward = levelReward.add(roundReward.mul(levelRewardPercentage).div(100));
            }
    ...
    function getTotalLevelRewards(address _user) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < stakes[_user].length; i++) {
            StakeInfo storage stake = stakes[_user][i];
            if (!stake.unstaked) {
                uint256 levelReward = getLevelReward(_user, i);
                totalLevelRewards = totalLevelRewards.add(levelReward);
            }
        }
