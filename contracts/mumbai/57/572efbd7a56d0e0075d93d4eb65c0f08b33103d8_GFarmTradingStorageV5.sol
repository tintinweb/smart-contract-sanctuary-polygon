...
contract GFarmTradingStorageV5 {
    ...
    // List of open pairs that the trader has open positions on
    ...
    function setMaxOpenInterestDai(uint256 _pairIndex, uint256 _newMaxOpenInterest) external onlyGov {
    ...
    function unregisterTrade(address trader, uint256 pairIndex, uint256 index) external onlyTrading {
        ...
        // if the trader only has one position of the current pair index
    ...
    function increaseReferralRewards(address _referral, uint256 _amount) external onlyTrading {
    ...
    // Manage dev & gov fees
    function handleDevGovFees(uint256 _pairIndex, uint256 _leveragedPositionSize, bool _dai, bool _fullFee)
        external
        onlyTrading
        returns (uint256 fee)
    {
    ...
    function transferLinkToAggregator(address _from, uint256 _pairIndex, uint256 _leveragedPosDai)
        external
        onlyTrading
    {
    ...
    // Traverse all trades indicies within a pair until the first empty index, returning its value.
...
interface AggregatorInterfaceV5 {
    ...
    function pairMinOpenLimitSlippageP(uint256) external view returns (uint256);
    function closeFeeP(uint256) external view returns (uint256);
    ...
    function referralP(uint256) external view returns (uint256);
    function nftLimitOrderFeeP(uint256) external view returns (uint256);
}
