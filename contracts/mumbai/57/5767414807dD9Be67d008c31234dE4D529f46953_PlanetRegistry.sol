 ...
 *
 */
contract PlanetRegistry is Ownable {
    ...
    event PlanetCreated(bytes32 indexed _planetCode);

    event PlanetUnregistered(bytes32 indexed _planetCode);
    ...
    modifier onlyApplicant(bytes32 _code) {
        require(
            planetInfo[_code].applicant == _msgSender() || accessControl.hasRole(accessControl.defaultAdminRole(), _msgSender()),
            "PR: unauthorized"
        );
        _;
    }
    ...
    modifier onlyShareHoldersAndAdmin(bytes32 _code) {
        require(
            _shareholderExists(_code, msg.sender) || accessControl.hasRole(accessControl.defaultAdminRole(), _msgSender()),
            "PR: unauthorized"
        );
        _;
    }
    ...
    modifier onlyShareHolderAndAdmin(bytes32 _code, address account) {
        require(
            (_shareholderExists(_code, msg.sender) && account == _msgSender()) ||
            planetInfo[_code].applicant == _msgSender() ||
    ...
    modifier onlyAdmin() {
        require(accessControl.hasRole(accessControl.defaultAdminRole(), _msgSender()), "PR: unauthorized");
        _;
    }

    modifier onlyPlanetAdmin(bytes32 _code) {
        require(accessControl.isAdmin(_code, _msgSender()), "PR: need admin right");
        _;
    }

    modifier onlyPlanetOperator(bytes32 _code) {
        require(accessControl.hasRole(_code, _msgSender()) || accessControl.isAdmin(_code, _msgSender()), "PR: address unauthorized");
        _;
    }
     ...
     * Admin Functions               *
     *********************************/
    function setSharesCollector(address _collector) public onlyOwner {
        require( _collector != address(0), "PR: zero address");
    ...
    function setAllowedToken(address _token, bool isAllowed) public onlyAdmin {
        require(_token != address(0), "PR: zero address");
        allowedTokens[_token] = isAllowed;
        emit AllowedTokensUpdated(_token, isAllowed);
    }

    function setAllowedDepositToken(address _token, bool isAllowed) public onlyAdmin {
    ...
    function registerPlanet(
        ...
        require(_planetCode != 0, "PR: empty code");
        require(
            _applicant != address(0),
            "PR: applicant is zero address"
        );
        require(allowedTokens[_token], "PR: planet not supported");
        require(
            (_tokenSupply % 100) == 0,
            "PR: shares not multiple of 100"
        );
        require(_tokenSupply >= 1000, "PR: shares less than 1000");
        require(
            allowedDepositTokens[_depositToken],
            "PR: depositToken not supported"
        );
        require(_depositRequirement > 0, "PR: zero deposit requirement");
        require(_chargeShares <= 20, "PR: charge shares exceeds" );
    ...
    function addShareholder(
        ...
        onlyApplicant(_code)
    {
        require(_shares > 0 && _shares <= 100, "PR: invalid shares");
        
        PlanetData storage planetData = planetInfo[_code];
        ShareholderData[] storage shareholders = shareholdersInfo[_code];
        ShareholderData storage applicant = shareholders[0];

        require(!_shareholderExists(_code, _shareholder), "PR: shareholder exists");
        ...
        if (applicant.depositedAmount > 0) {
            ...
            require(
                ...
                "PR: applicant deposit exceeds shares"
            );
        }
        ...
        onlyShareHolderAndAdmin(_code, _shareholder)
    {
        ...
        require(_shareholder != applicant.shareholder, "PR: applicant removal not allowed");
        ...
        revert("PR: shareholder not found");
    }
