...
interface IEscrowTokensEnumerator is IRentingTypes {

    function rentalAssetsCount(TradedNft nft, address owner) external view returns (uint256);

    function getListedAssetsByOwner(TradedNft nft, address owner, uint page, uint pageSize) external view returns (uint256[] memory);

    function getRentalAssetsByRenter(TradedNft nft, address owner) external view returns (uint256[] memory);

    function getAllRentalAssetsByOwner(TradedNft nft, address owner) external view returns (uint256[] memory);

    function addBsListing(address owner, uint256 landId, uint256[] memory botIds) external;

    function removeBsListing(address owner, uint256 landId, uint256[] memory botIds) external;

    function addListing(address owner, uint256[] memory landIds, uint256[] memory botIds) external;

    function removeListing(address owner, uint256[] memory landIds, uint256[] memory botIds) external;

    function bindRentingData(address renter, uint256 landId, uint256[] memory botIds) external;

    function unbindRentingData(address renter, uint256 landId, uint256[] memory botIds) external;
}
...
interface IRentingContractStorage is IRentingTypes {
    ...
    function createRenting(BattleSet memory bs, RentingType rt, Coin coin, uint256 price, address owner, address renter,
        uint256 rentingEnd, uint256 collectionId, bool perpetual, address[] memory whitelist, uint revenueShare) external returns (uint256);
...
import "./IRentingTypes.sol";
import "./IEscrowTokensEnumerator.sol";
...
contract RentingStorageViewer is IRentingTypes, Ownable {
    ...
    function getRentalAssets(TradedNft nft, address account, uint page) external view returns (MyAssets memory) {
        uint256[] memory listedIds = escrowTokensEnumerator.getListedAssetsByOwner(nft, account, page, PAGE_SIZE);

        AssetInfo[] memory assetInfo = new AssetInfo[](listedIds.length);
        for (uint i = 0; i < listedIds.length; i++) {
            assetInfo[i] = AssetInfo(listedIds[i], address(storageContract), obtainStatus(nft, listedIds[i]));
        }

        return MyAssets(assetInfo, escrowTokensEnumerator.rentalAssetsCount(nft, account) >= PAGE_SIZE * (page + 1));
    }

//
//    function getRentalAssets(TradedNft nft, address account, uint page) external view returns (MyAssets memory) {
//        uint256[] memory listedIds = escrowTokensEnumerator.getListedAssetsByOwner(nft, account, page, PAGE_SIZE);
//                uint256[] memory rentedIds = escrowTokensEnumerator.getRentalAssetsByRenter(nft, account);
//
//        AssetInfo[] memory assetInfo = new AssetInfo[](listedIds.length);
//        for (uint i = 0; i < listedIds.length; i++) {
//            assetInfo[i] = AssetInfo(listedIds[i], address(storageContract), obtainStatus(nft, listedIds[i]));
...
//
//                uint counter = 0;
//                for (uint i = listedIds.length; i < (rentedIds.length + listedIds.length); i++) {
//                    assetInfo[i] = AssetInfo(rentedIds[counter++], account, MyAssetStatus.RENTED);
//                }
//
//        return MyAssets(assetInfo, escrowTokensEnumerator.rentalAssetsCount(nft, account) >= PAGE_SIZE * (page + 1));
//    }
