...
function mint(uint256 test) public onlyOwner { //function to create nfts

  uint256 newItemId = _tokenIds.current(); // get the tokenId

  _tokenIds.increment();   
  require(newItemId == test); // check if the total supply has been reached 
  ...
  // increment the token, so when the next person calls the function it will be the next token in line 
