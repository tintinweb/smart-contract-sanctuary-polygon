    ...
    ERC1155HolderUpgradeable
{
    ...
    function addTickets(uint256 _tickets) external whenNotPaused {
        ...
        ticket.safeTransferFrom(msg.sender, address(this), info.ticketId, _tickets, "");
    ...
    function addPrize(
        ...
        require(length == _amounts.length, "invalid length");
    ...
    function draw(uint256 _day) external override whenNotPaused {
        ...
        uint256 requestID = getRandomNumber(_day);
     ...
     */
    function getRandomNumber(uint256 _day) internal returns (uint256 requestID) {
        require(keyHash != bytes32(0), "Must have valid key hash");

        uint256 numWords;
        for (uint256 i = 0; i < raffleLandmarkIds[_day].length; i++) {
            numWords += raffleLandmarkAmounts[_day][raffleLandmarkIds[_day][i]];
        }
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        ...
        for (i = 0; i < raffleLandmarkIds[day].length; i++) {
            id = raffleLandmarkIds[day][i];
            for (j = 0; j < raffleLandmarkAmounts[day][id]; j++) {
                rand = randomWords[index++] % info.totalTickets;
                left = 0;
