...
contract KeepBoredPass is ERC721A, Ownable {
    ...
    constructor(address signer_, address ogsigner_, string memory baseURI_,
    address _cpWallet,address _dev, address _amgm) ERC721A("Keep Bored Pass", "KBP") {
        _baseTokenURI = baseURI_;
        _WLsignerAddress = signer_;
        _OgsignerAddress = ogsigner_;
        COMPANY_WALLET = _cpWallet;
        //Reserved Supply OG
        _safeMint(_amgm, 2); //amanda, jaime
        _safeMint(_dev, 3); //dev
        _safeMint(0x88b0416738Cd0bEb64a67fa02959A398Da351219, 1); //KADES
        _safeMint(0x488cb8682983eE8C0add26E06C21146342588E25, 1); //Foxkids
        _safeMint(0x0a89A0fC542350B228112C95Affd883767A5772c, 1); // Chellyn
        _safeMint(0x3F5Bf050b409B06F92Df0203dE01813E19dBA72f, 1); // Lzfavan
        _safeMint(0x6a33aF6f10B4a27d3981855f7DA297a8C1481aD3, 1); // Solaire
        _safeMint(0x6d612F1C62AEE6D725aD85b084e56B482060CE4d, 1); // Gyurca

        //Reserved Supply bored
        _safeMint(0x08D46B6fAFc5E8E1B64929A987f61f68ffD2D64F, 1); //Odalax
        _safeMint(0x60aFc25cA04db6DC57e984471F7694D3b07cb298, 1); //Cusatis
        _safeMint(0xb03257180aA2104299F9E4145e02d7F443fcfF1a, 1); //Mr sangria
        _safeMint(0xBD7360855dFd1FD666Fc4dbe858c4E1F6FE4DbE9, 1); //Pandora
        _safeMint(0x0441D7f668ADFb0Bef68c74b19a527d2BaD17dbD, 1); //ChrisNFT
    ...
    function dropnotMinted() external onlyOwner {
         ...
         require(totalminted < TOTAL_SUPPLY - (RSupply + ROGSupply),"Max supply exceeded!");
         ...
         for (uint256 i = 0; i < ogtoMint; ++i) { //mint Remain OG
            tokenType[totalminted+i] = 1;
        }

         for (uint256 i = 0; i < toMint; ++i) { //mint Remain
