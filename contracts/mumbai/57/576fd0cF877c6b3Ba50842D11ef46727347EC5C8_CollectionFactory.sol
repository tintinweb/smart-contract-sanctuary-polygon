...
contract NiftySouqCollectionV2 is ERC721Upgradeable, AccessControlUpgradeable {
    ...
    string private _coverImageUri;
    string private _baseTokenURI;

    bytes32 public constant COLLECTION_OWNER_ROLE =
        keccak256("COLLECTION_OWNER_ROLE");
    bytes32 public constant NIFTYSOUQ_ADMIN_ROLE =
        keccak256("NIFTYSOUQ_ADMIN_ROLE");
    bytes32 public constant NIFTYSOUQ_MARKETPLACE_ROLE =
        keccak256("NIFTYSOUQ_MARKETPLACE_ROLE");
    ...
    modifier isNiftySouqAdmin() {
        if (
            !hasRole(DEFAULT_ADMIN_ROLE, msg.sender) &&
            !hasRole(NIFTYSOUQ_ADMIN_ROLE, msg.sender)
        ) revert GeneralError("NS:102");
        _;
    }

    modifier isNiftySouqAdminOrOwner() {
        if (
            !hasRole(DEFAULT_ADMIN_ROLE, msg.sender) &&
            !hasRole(NIFTYSOUQ_ADMIN_ROLE, msg.sender) &&
    ...
    modifier isNiftyMarketplace() {
        if (!hasRole(NIFTYSOUQ_MARKETPLACE_ROLE, msg.sender))
            revert GeneralError("NS:106");
        _;
    }
    ...
    //*********************** Admin Functions ***********************//
    /**
     * @notice Initializes the contract by setting 'name','symbol','baseURI','title','description','imageURI','coverImageURI' of collection and addresses of 'niftySouqAdmin','niftySouqMarketplace'.
     ...
     * @param title_ title for collection.
     * @param description_ description for collection.
     * @param imageURI_ URI of the image for collection.
     * @param coverImageURI_ URI of coverimage for collection.
     * @param owner_ address of collection owner.
     * @param niftySouqAdmin_ address of nifty souq admin.
     * @param niftySouqMarketplace_ address of niftySouqMarketplace.
     */
    function initialize(
        uint256 version_,
        string memory name_,
        ...
        string memory coverImageURI_,
        address owner_,
        address niftySouqAdmin_,
        address niftySouqMarketplace_
    ) external initializer {
        __ERC721_init(name_, symbol_);
        __AccessControl_init();
        _setupRole(DEFAULT_ADMIN_ROLE, niftySouqAdmin_);
        _setupRole(NIFTYSOUQ_ADMIN_ROLE, niftySouqAdmin_);
        _setupRole(COLLECTION_OWNER_ROLE, owner_);
        _setupRole(NIFTYSOUQ_MARKETPLACE_ROLE, niftySouqMarketplace_);

        _title = title_;
        _description = description_;
        _imageUri = imageURI_;
        _coverImageUri = coverImageURI_;
    ...
    function updateMetadata(
        ...
        string memory imageURI_,
        string memory coverImageURI_
    ) external isNiftySouqAdminOrOwner {
        _title = title_;
        _description = description_;
        _imageUri = imageURI_;
        _coverImageUri = coverImageURI_;
     ...
     * @dev only nifty admin can change uri.
     * @param baseUri_ baseuri of token.
     */
    function setBaseURI(string memory baseUri_) external isNiftySouqAdmin {
        if (
    ...
    *@param uri_ URI of token.
        ...
        )
    {
        ...
        coverImageURI_ = string(
            abi.encodePacked(_baseTokenURI, _coverImageUri)
        );
    }
     ...
     *@dev works only if tokenId exist otherwise throws an error-NS:206'tokenId doesnot exist'.
    ...
    function isCollectionContract() external pure returns (bool) {
        return true;
    }
    ...
    {
        if (!hasRole(COLLECTION_OWNER_ROLE, mintData_.minter))
            revert GeneralError("NS:104");
        if (
    ...
    function transferNft(
        ...
        if (_nftInfos[tokenId_].isFirstSale)
            _nftInfos[tokenId_].isFirstSale = false;
    }
...
import "./NiftySouq-CollectionProxy.sol";
import "./NiftySouq-Collection.sol";
...
contract CollectionFactory is Initializable {
    error GeneralError(string errorCode);
    address private _niftySouqMarketplace;
    ...
    /** @notice Initializes the contract by setting the address of niftysouq Marketplace.
     * @dev used instead of constructor.
     * @param niftySouqMarketplace_ address of niftysouq Marketplace.
     */
    function initialize(address niftySouqMarketplace_) external initializer {
        _niftySouqMarketplace = niftySouqMarketplace_;
        _admin = msg.sender;
        _version = 1;
    }
     ...
     * @param  niftySouqMarketplace_ address of niftysouq Marketplace.
     */
    function updateMarketplaceaddress(address niftySouqMarketplace_)
        external
        onlyAdmin
    {
        _niftySouqMarketplace = niftySouqMarketplace_;
    ...
    function createNewCollection(
        ...
        NiftySouqCollectionV2 collectionV2 = new NiftySouqCollectionV2();
        NiftySouqProxy collectionProxy = new NiftySouqProxy(
            address(collectionV2),
            _admin,
            abi.encodeWithSelector(
                NiftySouqCollectionV2(address(0)).initialize.selector,
                _version,
                name_,
                symbol_,
                baseURI_,
 ...
 */
contract NiftySouqProxy is ERC1967Proxy {
    error GeneralError(string errorCode);
     ...
     */
    function changeAdmin(address newAdmin_) external virtual ifAdmin {
     ...
     */
    function upgradeTo(address newImplementation_) external ifAdmin {
        _upgradeToAndCall(newImplementation_, bytes(""), false);
    }
     ...
     */
    function upgradeToAndCall(address newImplementation_, bytes calldata data_)
        external
        payable
        ifAdmin
    {
        _upgradeToAndCall(newImplementation_, data_, true);
    }
    ...
    function _beforeFallback() internal virtual override {
        if (msg.sender == _getAdmin()) revert GeneralError("NS:601");
