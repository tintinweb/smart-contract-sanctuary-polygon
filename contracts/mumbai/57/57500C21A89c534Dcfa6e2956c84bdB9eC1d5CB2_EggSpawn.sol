...
contract EggSpawn is ERC721SignatureMint {
    using ECDSAHelper for bytes32;
    using TWStrings for uint256;
    ...
    uint256 public seasonDuration = 10;

    uint256 private _currentDay = 0;
    uint256 public dailyEggsSpawned = 0;
    uint256 public seasonEggsSpawned = 0;

    uint256 public dailyEggSupply;
    uint256 public seasonEggSupply;
    ...
    function _createEgg(uint256 tokenId) internal {
        _eggs[tokenId] = Egg(
            tokenId,
            currentAge,
            _currentDay,
    ...
    function _spawnEggs(
        ...
        for (uint256 i = 0; i < count; i++) {
            _createEgg(startTokenId + i);
            tokenIds[i] = startTokenId + i;
        }
        return tokenIds;
    }

    function mintEggWithSignature(
        uint256 expires,
        uint256 nonce,
        uint256 count,
        uint256 payment,
        ...
        uint256 day = (block.timestamp - seasonStart) / uint256(60) / uint256(60) / uint256(24);
        if (_currentDay != day) {
            dailyEggsSpawned = 0;
            _currentDay = day;
        }

        bytes32 message = keccak256(abi.encodePacked(msg.sender, expires, nonce, count, payment)).toEthSignedMessageHash();
        address signer = message.recover(signature);
        require(signer == signingAddress, "Invalid signature");

        require(payment == msg.value, "Wrong amount sent");
        require(!usedNonces[nonce], "Duplicate request");
        require(block.timestamp < expires, "Signature expired");
        require(block.timestamp >= seasonStart && seasonStart != 0, "Spawning closed");
        if (_currentDay < 10) {
            require(dailyEggsSpawned + count <= dailyEggSupply, "Daily egg supply reached");
        }
        require(seasonEggsSpawned + count <= seasonEggSupply, "Season egg supply reached");

        usedNonces[nonce] = true;

        dailyEggsSpawned += count;
        seasonEggsSpawned += count;
        uint256[] memory tokenIds = _spawnEggs(msg.sender, count);

        emit SpawnEggs(msg.sender, count, nonce, tokenIds, currentAge, currentSeason, _currentDay);
    }

    function setSeasonStart(uint256 start, uint256 age, Season season, uint256 eggDailyCnt) external onlyOwner {
        dailyEggsSpawned = 0;
        seasonEggsSpawned = 0;
        ...
        dailyEggSupply = eggDailyCnt;
        seasonEggSupply = dailyEggSupply * seasonDuration;
    }
    ...
    function setDrakariaService(address _drakariaService) external onlyOwner {
        drakariaService = _drakariaService;
    }
    ...
    function setEggMetadata(uint256 tokenId, Season house, Season spawnElement) external onlyOwner {
        Egg storage egg = _eggs[tokenId];
        egg.house = house;
        egg.spawnElement = spawnElement;
    }

    function getEgg(uint256 _tokenId) external view
    returns (
        uint256 tokenId,
        uint256 age,
        uint256 tier,
        ...
        if (_exists(_tokenId)) {
            return (
            _eggs[_tokenId].tokenId,
            _eggs[_tokenId].age,
            _eggs[_tokenId].tier,
            _eggs[_tokenId].spawnSeason,
            _eggs[_tokenId].house,
            _eggs[_tokenId].spawnElement
            );
        }
        revert("Invalid tokenId");
    }
    ...
    function getCurrentDay() public view returns (uint256) {
        if (seasonStart == 0) return 0;

        uint256 day = (block.timestamp - seasonStart) / uint256(60) / uint256(60) / uint256(24);
        if (_currentDay != day) return day;
        return _currentDay;
    }

    error EggQueryForNonexistentToken();
}