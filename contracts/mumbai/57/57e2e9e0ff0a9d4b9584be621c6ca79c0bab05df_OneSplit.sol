...
contract OneSplitRoot {
    ...
    IWETH internal constant wmatic = IWETH(0x4c28f48448720e9000907BC2611F73022fdcE1fA);
...
contract OneSplit is Initializable, IOneSplit, OneSplitRoot, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    function swap(
        ...
        _swapFloor(fromToken, destToken, confirmed, distribution, flags);
    ...
    function swapMulti(
        ...
        for (uint256 i = 1; i < tokens.length; i++) {
            ...
            _swapFloor(tokens[i - 1], tokens[i], returnAmount, dist, flags[i - 1]);
    ...
    function _swapFloor(
        ...
        _swap(fromToken, destToken, amount, 0, distribution, flags);
    }

    //[rusdt, usdt, route]
    //[rmatic, wmatic, route]
    //[usdt, rmatic, route] - not possible
    //[route, wmatic , rmatic]
    //[route, matic]
    //[rmatic, MATIC] - already

    function _swap(
        IERC20Upgradeable fromToken, //usdt, rusdc
        IERC20Upgradeable destToken, //rusdt, usdc
        uint256 amount,
        uint256 minReturn,
        uint256[] memory distribution,
        uint256 flags // See constants in IOneSplit.sol
        ...
        //rusdc to usdc || rmatic to wmatic
        if (
        ...
        //usdc to rusdc || wmatic to rmatic
        ...
        function(IERC20Upgradeable, IERC20Upgradeable, uint256, uint256)[DEXES_COUNT] memory reserves = [
            _swapOnUniswap,
            _swapOnUniswapV2,
            _swapOnDfyn,
            _swapOnPancakeSwap
        ...
        require(distribution.length <= reserves.length, "Distribution array should not exceed reserves array size");
...
library UniversalERC20 {
    ...
    IERC20Upgradeable private constant ETH_ADDRESS = IERC20Upgradeable(0x0000000000000000000000000000000000000000);
