...
contract distributorContract is ReentrancyGuardUpgradeable,OwnableUpgradeable {
        ...
        // events
        event Deposit(address from,  address to, uint amount, uint poolID);
        event getNftId(address from , address to , uint tokenID);
        ...
        function deposit (uint poolId,uint tokenAmount) external nonReentrant {
               ...
               emit Deposit(msg.sender, collectionPools[poolId],total, poolId );
               emit getNftId(address(0), msg.sender, nftIndexPerPool[poolId]);
        }
