...
contract ListingV2 is Initializable {
    ...
    mapping(bytes => AuctionStructure.Listing) public Listings;
    ...
    {
        AuctionStructure.Listing memory listing = Listings[_listing];
        return listing;
    }

    function _processCancelListing(bytes calldata _listingId) private {
        require(
            Listings[_listingId].state !=
                AuctionStructure.StateOfListing.INACTIVE,
            "AlreadyInActive"
        );

        Listings[_listingId].state = AuctionStructure.StateOfListing.INACTIVE;
    }
    ...
    function listingNFTFixedPrice(
        ...
        AuctionStructure.Listing memory newInstance = AuctionStructure.Listing(
            paramListing.indexId,
            paramListing.maker,
            _stanNFT,
            false,
            paramListing.expirationTime,
            paramListing.amount,
            paramListing.tokenId,
            AuctionStructure.StateOfListing.ACTIVE,
            NULL,
            paramListing.nftId,
            paramListing.currency
        );
        Listings[paramListing.indexId] = newInstance;
    }

    function listingNFTAuction(
        AuctionStructure.paramListing calldata _paramListing,
        address _stanNFT
    ) external onlyAuthorizer returns (address) {
        AuctionStructure.Listing memory newInstance = AuctionStructure.Listing(
            _paramListing.indexId,
            _paramListing.maker,
            _stanNFT,
            true,
            _paramListing.expirationTime,
            _paramListing.amount,
        ...
        Listings[_paramListing.indexId] = newInstance;
        Listings[_paramListing.indexId].state = AuctionStructure
            .StateOfListing
            .ACTIVE;

        return _paramListing.maker;
    }

    function cancelListingFixedPrice(bytes calldata _listingId)
        external
        onlyAuthorizer
    {
        _processCancelListing(_listingId);
    }

    function cancelListingAuction(bytes calldata _listingId)
        external
        onlyAuthorizer
    {
        _processCancelListing(_listingId);
    ...
    function updateListing(
        ...
        Listings[_listingId].state = state;
    }

    function expiredListing(bytes[] calldata listingIds)
        external
        onlyAuthorizer
    {
        for (uint256 i = 0; i < listingIds.length; ) {
            Listings[listingIds[i]].state = AuctionStructure
                .StateOfListing
                .EXPIRED;
