...
contract Tagged {
    ...
    */
    ...
    event logAddTags(uint256 indexed msgID, address[] indexed target);
    event logRemoveTags(uint256 indexed msgID, address[] indexed target);
    ...
    */
    function getTaggedMsgIDs(address usrAddress, uint256 startFrom) public view returns(uint256[] memory) {
    ...
    function removeTags(uint256 msgID, address[] calldata addressesTagged) public onlyAdmins {
        ...
        for (uint h=0; h < addressesTagged.length; h++) {
            ...
            emit logRemoveTags(msgID, addressesTagged);

            delete addressTaggedStr;
            delete bucketKeyID;
        }
    }
    ...
    function addTags(uint256 msgID, address[] calldata addressesTagged) public onlyAdmins {
        ...
        for (uint h=0; h < addressesTagged.length; h++) {
            // Make sure it's min length to be ENS at least
            require(bytes(KUtils.addressToString(addressesTagged[h])).length >= 7, "Tags must be ENS name or ETH address");
            ...
            // Emit to the logs for external reference
            emit logAddTags(msgID, addressesTagged);

            delete addressTaggedStr;
            delete taggedBucketKey;
            delete bucketKeyID;
        }
    }
