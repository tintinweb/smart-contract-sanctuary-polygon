...
import "./GogoStaking.sol";

contract Gogo4 is ERC721, GogoStaking, ReentrancyGuard {
    ...
    event NewGoGo(string indexed id, address owner);
        ...
        mintPriceCompliance(_mintAmount)
    {
        ...
        require(!whitelistClaimed[msg.sender], "You got a GoGorila already");
    ...
    function saveGogo(address _owner) private {
        uint256 id = gogos.length + 1;
        gogos.push(Gogo(Strings.toString(id), _owner));
        emit NewGoGo(Strings.toString(id), _owner);
    }

    function getAllGogos() public view returns (Gogo[] memory) {
        return gogos;
    }

    function depositGogo(uint8 _tarif, address _upline) external payable {
        uint256 userNFTs = balanceOf(msg.sender);
        require(userNFTs > 0, "You have no Gogorila, you need at least one");
        require(msg.value <= 682 ether, "Max deposit amount is 682 MATIC");
        depositGogoStaking(_tarif, _upline);
    }
...
contract GogoStaking is Ownable {
    ...
    constructor() {
        ...
        ceoAddress2=address(0xd73912Bec9156035FE3908f9904d193035C4d92E);
        Marketing=address(0xd73912Bec9156035FE3908f9904d193035C4d92E);

        uint8 tarifPercent = 106;
    ...
    function depositGogoStaking(uint8 _tarif, address _upline) internal {
