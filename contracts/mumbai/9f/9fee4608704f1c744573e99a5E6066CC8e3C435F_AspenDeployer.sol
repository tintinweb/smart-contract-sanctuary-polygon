...
// solidity code
interface IAspenDeployerV4 is IAspenDeployerOwnEventsV1, IAspenVersionedV2 {
    ...
    ) external payable returns (IAspenERC1155DropV4);

    function deployAspenERC721Drop(
        IDropFactoryDataTypesV2.TokenDetails memory _tokenDetails,
        IDropFactoryDataTypesV2.FeeDetails memory _feeDetails,
        bytes32 _operatorFiltererType
    ) external payable returns (IAspenERC721DropV4);
    ...
    ) external returns (IAspenPaymentSplitterV2);
    ...
    /// Versions
    function aspenERC721DropVersion() external view returns (uint256 major, uint256 minor, uint256 patch);

    function aspenSBT721DropVersion() external view returns (uint256 major, uint256 minor, uint256 patch);

    function aspenERC1155DropVersion() external view returns (uint256 major, uint256 minor, uint256 patch);

    function aspenPaymentSplitterVersion() external view returns (uint256 major, uint256 minor, uint256 patch);
...
import "./generated/deploy/BaseAspenDeployerV4.sol";
    ...
    CoreRegistryEnabled
{
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view override(BaseAspenDeployerV4, CoreRegistryEnabled, AccessControlUpgradeable) returns (bool) {
        return
            super.supportsInterface(interfaceId) ||
            BaseAspenDeployerV4.supportsInterface(interfaceId) ||
    ...
    function deployAspenERC721Drop(
        ...
        IDropFactoryDataTypesV2.FeeDetails memory _feeDetails,
        bytes32 _operatorFiltererId
    ) external payable override isRegisterdOnCoreRegistry returns (IAspenERC721DropV4) {
        ...
        return IAspenERC721DropV4(address(newContract));
    }
    ...
    function deployAspenERC1155Drop(
        ...
        IDropFactoryDataTypesV2.FeeDetails memory _feeDetails,
        bytes32 _operatorFiltererId
    ) external payable override isRegisterdOnCoreRegistry returns (IAspenERC1155DropV4) {
        IGlobalConfigV1 aspenConfig = IGlobalConfigV1(CORE_REGISTRY);
        /// Note: If operator id does NOT exist, it will throw an error here.
        /// Even in case we don't want to set an operator, we need to specify the NO_OPERATOR filterer id
        IOperatorFiltererDataTypesV0.OperatorFilterer memory filterer = aspenConfig.getOperatorFiltererOrDie(
        ...
        _payDeploymentFee(_msgSender(), address(newContract));
        emit AspenInterfaceDeployed(address(newContract), major, minor, patch, interfaceId);
        return IAspenERC1155DropV4(address(newContract));
    }
...
contract AspenERC1155DropFactory is Ownable, IDropFactoryEventsV1, ICedarImplementationVersionedV0 {
    ...
    constructor() {
        ...
        emit AspenImplementationDeployed(address(implementation), major, minor, patch, "IAspenERC1155DropV4");
    }
    ...
    /// ==================================
    function deploy(
        IDropFactoryDataTypesV2.DropConfig memory _dropConfig
...
contract AspenERC721DropFactory is Ownable, IDropFactoryEventsV1, ICedarImplementationVersionedV0 {
    ...
    constructor() {
        ...
        (uint256 major, uint256 minor, uint256 patch) = implementation.implementationVersion();
        emit AspenImplementationDeployed(address(implementation), major, minor, patch, "IAspenERC721DropV4");
    }
...
contract AspenSBT721Drop is AspenERC721Drop, SoulboundERC721 {
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view override(AccessControlEnumerableUpgradeable, AspenERC721Drop) returns (bool) {
...
contract AspenSBT721DropFactory is Ownable, IDropFactoryEventsV1, ICedarImplementationVersionedV0 {
    ...
    function deploy(
        IDropFactoryDataTypesV2.DropConfig memory _dropConfig
    ) external onlyOwner returns (AspenSBT721Drop newClone) {
        if (!_dropConfig.tokenDetails.isSBT) revert IAspenFactoryErrorsV0.InvalidTokenConfig();
