 ...
 * over 5% for each fee type (max 15%)
 *
...
abstract contract Tokenomics is Ownable {
    ...
    string internal constant SYMBOL = "ISV";
    ...
    function _addFees() private {
        ...
        _addFee(FeeType.External, 10, stakingAddress);
        _addFee(FeeType.External, 10, animeAddress);
    }
    ...
    function changeStakingFee(uint256 value) external onlyOwner {
        require(value <= 50, "Fee can't be over 5%");
        uint256 sumWithoutMarketing = sumOfFees - fees[1].value;
        fees[1].value = value;
        sumOfFees = sumWithoutMarketing + value;
    }

    function changeStakingAddress(address account) external onlyOwner {
        fees[1].recipient = account;
    }

    function changeAnimeFee(uint256 value) external onlyOwner {
        require(value <= 50, "Fee can't be over 5%");
        uint256 sumWithoutAnime = sumOfFees - fees[2].value;
        fees[2].value = value;
...
abstract contract Liquifier is IERC20, Ownable {
    ...
    // Quickswap V2 Polygon
    address private _testnetMumbaiRouterV2Address =
        0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff;
    // Testnet
    ...
    function initializeLiquiditySwapper(
        ...
        } else if (_env == Env.TestnetBsc) {
            _setRouterAddress(_testnetBscRouterAddress, address(usdtContract));
        }
        /*(_env == Env.TestnetMumbai)*/
        else {
            _setRouterAddress(
                _testnetMumbaiRouterV2Address,
                address(usdtContract)
            );
        }
...
contract IsekaiCoin is IsekaiCoinBase {
    constructor(IERC20 _usdtContract)
        IsekaiCoinBase(Env.TestnetMumbai, _usdtContract)
    {
