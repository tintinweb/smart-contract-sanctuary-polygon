...
contract MessageFormat {
    ...
    */
    ...
    function buildMsg(uint256[] memory msgData, string memory message, address[2] memory postedBy, string[] memory hashtags, address[] memory taggedAccounts, string memory uri, uint256[] memory inGroups) public view returns (string memory){
        ...
        for (uint h=0; h < hashtags.length; h++) {
            if (h == 0){
                hashtagsStr = append('"', hashtags[h], '"', '', '');
            } else {
                hashtagsStr = append(hashtagsStr, ',"', hashtags[h], '"', '');
            }
        }

        for (uint h=0; h < taggedAccounts.length; h++) {
            if (h == 0){
                taggedStr = append('"', KUtils.addressToString(taggedAccounts[h]), '"', '', '');
            } else {
                taggedStr = append(taggedStr, ',"', KUtils.addressToString(taggedAccounts[h]), '"', '');
            }
        }

        for (uint h=0; h < inGroups.length; h++) {
            if (h == 0){
                inGroupsStr = append('"',KUtils.toString(inGroups[h]),'"','','');
            } else {
                inGroupsStr = append(taggedStr, ',"', KUtils.toString(inGroups[h]), '"', '');
            }
        }
        ...
        Attribs memory attribs;
        
        attribs.msgID = KUtils.toString(msgData[0]);
        attribs.commentLevel = KUtils.toString(msgData[11]);
        attribs.tip = KUtils.toString(msgData[3]);
        attribs.paid = KUtils.toString(msgData[4]);
        attribs.postByContract = KUtils.toString(msgData[5]);
        attribs.likes = KUtils.toString(msgData[6]);
        attribs.comments = KUtils.toString(msgData[8]);
        attribs.reposts = KUtils.toString(msgData[7]);
        attribs.block = KUtils.toString(msgData[2]);
        attribs.timestamp = KUtils.toString(msgData[1]);
        attribs.isCommentOf = KUtils.toString(msgData[9]);
        attribs.isRepostOf = KUtils.toString(msgData[10]);
        attribs.asGroup = KUtils.toString(msgData[12]);
        ...
        */
        ...
        string memory thisMsg = string(abi.encodePacked(
            '{"msgID": ', attribs.msgID,
            ',"poster": {"address": "', KUtils.addressToString(postedBy[0]),
            '","handle": "', userDetails[0],
            '","verification": "', userDetails[4],
            '","avatarURI": "', userDetails[5],
            '","avatarMetadata": "', userDetails[6],
            '","avatarContract": "', userDetails[7]
        ));

        thisMsg = string(abi.encodePacked(thisMsg,
            '"},"message": "', message,
            '","commentLevel": ', attribs.commentLevel,
            ',"hashtags": [', hashtagsStr,
            '],"taggedAccounts": [', taggedStr,
            '],"postByContract": ', attribs.postByContract,
            ',"likes": ', attribs.likes,
            ',"comments": ', attribs.comments,
            ',"reposts": ', attribs.reposts));

        thisMsg = string(abi.encodePacked(thisMsg,
            ',"isCommentOf": ', attribs.isCommentOf,
            ',"isRepostOf": ', attribs.isRepostOf,
            ',"asGroup": "', attribs.asGroup,
            '","inGroups": [', inGroupsStr,
            '],"uri": "', uri,
            '","tips": ', attribs.tip,
            ',"paid": ', attribs.paid,
            ',"block": ', attribs.block,
            ',"timestamp": ', attribs.timestamp));

        thisMsg = string(abi.encodePacked(thisMsg,
            ',"postProxy": "', KUtils.addressToString(postedBy[1]),
            '"}'));
