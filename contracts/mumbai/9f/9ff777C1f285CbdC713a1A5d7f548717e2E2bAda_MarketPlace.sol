...
contract MarketPlace is ERC2771Context,IAuctionHouse, ReentrancyGuard,Whitelist,ERC721Holder{
    ...
    function cancelOrder(uint256 orderId) external nonReentrant orderExists(orderId) {
        require(
            auctions[orderId].tokenOwner == _msgSender(),
            "Can only be called by auction creator or curator"
        );
        _cancelOrder(orderId);
        emit OrderCanceled(orderId,auctions[orderId].tokenId,auctions[orderId].tokenContract,auctions[orderId].tokenOwner);
    }
    ...
    function setAuctionReservePrice(uint256 auctionId, uint256 reservePrice) external override auctionExists(auctionId) {
        ...
        require(_msgSender() == auctions[auctionId].tokenOwner, "Must be auction curator or token owner");
    ...
    function endAuction(uint256 auctionId) external override auctionExists(auctionId) nonReentrant {
        ...
        if(checkRoyalties(auctions[auctionId].tokenContract)){
            ...
            //console.log("Royality Receiver---------->%s",reciver);
            //console.log("royaltyAmount----------->%s",royaltyAmount);
            if(reciver != address(0)){
                _handleOutgoingBid(reciver, royaltyAmount,LNQTokenAddress);
