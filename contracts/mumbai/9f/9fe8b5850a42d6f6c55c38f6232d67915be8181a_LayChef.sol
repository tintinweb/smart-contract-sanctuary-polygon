...
// LayChef is the chef of new tokens. She can make yummy food and she is a fair lady as well as MasterChef.
contract LayChef is ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    // Info of each user.
    struct UserInfo {
        uint256 amount; // How many LAY tokens the user has provided.
    ...
    uint256 public rewardPerBlock = 16e18; // 16 LAY per block

    // Info.
    PoolInfo public poolInfo;
    // Info of each user that stakes Lay tokens.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 stakedSupply = lay.balanceOf(address(this));
    ...
    function updatePool() public {
        ...
        uint256 laySupply = lay.balanceOf(address(this));
        if (laySupply == 0) {
        ...
        poolInfo.accRewardPerShare = poolInfo.accRewardPerShare.add(
            tokenReward.mul(1e12).div(laySupply)
        );
        poolInfo.lastRewardBlock = block.number;
    }

    // Deposit Lay tokens to LayChef for Reward allocation.
    function deposit(uint256 _amount) public nonReentrant {
        require(_amount > 0, "amount 0");
        UserInfo storage user = userInfo[msg.sender];
        updatePool();
        lay.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    // Withdraw Lay tokens from LayChef.
    function withdraw(uint256 _amount) public nonReentrant {
        ...
        updatePool();
        lay.safeTransfer(address(msg.sender), _amount);
    ...
    function emergencyWithdraw() public nonReentrant {
        ...
        lay.safeTransfer(address(msg.sender), user.amount);
