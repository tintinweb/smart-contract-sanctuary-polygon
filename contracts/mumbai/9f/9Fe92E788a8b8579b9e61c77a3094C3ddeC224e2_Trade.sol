...
contract Trade is AccessControl {
    ...
    mapping(address => mapping (address => mapping(uint256 => bool))) isOnSale;

    mapping(address => uint256) public lazymintListingFee;
    ...
    */
    ...
    constructor(
        ...
        owner = msg.sender;
        signer = msg.sender;
        lazymintListingFee[weth] = 5;
        lazymintListingFee[harvest] = 5;
    ...
    */
    ...
    function setLazymintlistingfee(address tokenAddress, uint256 _listingFee)
        external
        onlyRole(ADMIN_ROLE)
        returns (bool)
    {
        lazymintListingFee[tokenAddress] = _listingFee;
        return true;
    }
    ...
    */
    ...
    function putOnSale(uint256 _tokenId, address nftAddress, BuyingAssetType nftType, uint256 qty) external payable onlyRole(OPERATOR_ROLE) {
        if(nftType == BuyingAssetType.ERC721) {
            require(IERC721(nftAddress).ownerOf(_tokenId) == msg.sender,"ERC721: caller is not Owner");
        }
        if(nftType == BuyingAssetType.ERC1155) {
            require(IERC1155(nftAddress).balanceOf(msg.sender, _tokenId) <= qty ,"ERC1155: caller is not Owner");
        }
        require(!isOnSale[msg.sender][nftAddress][_tokenId],"ERC721: token already in sale");
        require(msg.value == listingFee,"Listing: value must be equal to ListingFee");
        payable(owner).transfer(msg.value);
        isOnSale[msg.sender][nftAddress][_tokenId] = true;
    }
    /**
    ...
    function getFees(
        ...
        if(order.nftType == BuyingAssetType.LazyERC721 || order.nftType == BuyingAssetType.LazyERC1155) {
            uint256 value =  assetFee * lazymintListingFee[order.erc20Address] / 100;
            require(assetFee >= value, "Invalid payment amount");
            assetFee -= value;
        }
