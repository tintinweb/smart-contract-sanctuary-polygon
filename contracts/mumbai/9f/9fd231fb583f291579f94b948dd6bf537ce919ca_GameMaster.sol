...
contract GameMaster is Initializable, AccessControlUpgradeable, PausableUpgradeable, ICharacter, IGameMaster {
    ...
    function performFight(
        ...
        crafting.lootItems(char, tokens, msg.sender);
        ...
        if (playerRoll > monsterRoll) {
            ...
            if (
                overflowReward > 0 && RandomUtil.combineSeeds(seed, block.timestamp).mod(chanceOfWinningOverflow) == 0
            ) {
    ...
    ICharacter
{
    ...
    event SetParts(address sender, uint256 parts);
    event SetFireCrystals(address sender, uint256 fireCrystals);
    event SetWaterCrystals(address sender, uint256 waterCrystals);
    event SetEarthCrystals(address sender, uint256 earthCrystals);
    event SetLightningCrystals(address sender, uint256 lightningCrystals);
    event Salvage(address sender, uint256 reward);
    ...
    function initialize(
        ...
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(GAME_ADMIN, msg.sender);
        _setupRole(CRAFTING_ITEMS_ADMIN, msg.sender);
    ...
    function salvage(uint256 weapon, address user) external override {
        require(weapons.ownerOf(weapon) == msg.sender, 'Not the weapon owner');
        ...
        // burn weapon
        weapons.safeTransferFrom(msg.sender, 0x0000000000000000000000000000000000000001, weapon);
        emit Salvage(msg.sender, reward);
    }
    ...
    function lootItems(
        ...
        // require(hasRole(CRAFTING_ITEMS_ADMIN, msg.sender), 'Sender cannot loot items');
        ...
        // emit LootItems(
        //     msg.sender,
        //     craftingItems.balanceOfBatch(
        //         [msg.sender, msg.sender, msg.sender, msg.sender, msg.sender],
        //         [PART, FIRE_CRYSTAL, LIGHTNING_CRYSTAL, WATER_CRYSTAL, EARTH_CRYSTAL]
        //     )
        // );
    }

    function addParts(uint256 amount, address user) public {
        craftingItems.mintParts(user, amount);
        emit SetParts(user, craftingItems.balanceOf(user, PART));
    }

    function addFireCrystals(uint256 amount, address user) public {
        craftingItems.mintFireCrystals(user, amount);
        emit SetFireCrystals(user, craftingItems.balanceOf(user, FIRE_CRYSTAL));
    }

    function addWaterCrystals(uint256 amount, address user) public {
        craftingItems.mintWaterCrystals(user, amount);
        emit SetWaterCrystals(user, craftingItems.balanceOf(user, WATER_CRYSTAL));
    }

    function addEarthCrystals(uint256 amount, address user) public {
        craftingItems.mintEarthCrystal(user, amount);
        emit SetEarthCrystals(user, craftingItems.balanceOf(user, EARTH_CRYSTAL));
    }

    function addLightningCrystals(uint256 amount, address user) public {
        craftingItems.mintLightningCrystals(user, amount);
        emit SetLightningCrystals(user, craftingItems.balanceOf(user, LIGHTNING_CRYSTAL));
    }

    function subParts(uint256 amount, address user) public {
        require(craftingItems.balanceOf(user, PART).sub(amount) >= 0, 'Burn amount exceeds balance');
        craftingItems.burnParts(user, amount);
        emit SetParts(user, craftingItems.balanceOf(user, PART));
    }

    function subFireCrystals(uint256 amount, address user) public {
        require(craftingItems.balanceOf(user, FIRE_CRYSTAL).sub(amount) >= 0, 'Burn amount exceeds balance');
        craftingItems.burnFireCrystals(user, amount);
        emit SetFireCrystals(user, craftingItems.balanceOf(user, FIRE_CRYSTAL));
    }

    function subLightningCrystals(uint256 amount, address user) public {
        require(craftingItems.balanceOf(user, LIGHTNING_CRYSTAL).sub(amount) >= 0, 'Burn amount exceeds balance');
        craftingItems.burnLightningCrystals(user, amount);
        emit SetLightningCrystals(user, craftingItems.balanceOf(user, LIGHTNING_CRYSTAL));
    }

    function subWaterCrystals(uint256 amount, address user) public {
        require(craftingItems.balanceOf(user, WATER_CRYSTAL).sub(amount) >= 0, 'Burn amount exceeds balance');
        craftingItems.burnWaterCrystals(user, amount);
        emit SetWaterCrystals(user, craftingItems.balanceOf(user, WATER_CRYSTAL));
    }

    function subEarthCrystals(uint256 amount, address user) public {
        require(craftingItems.balanceOf(user, EARTH_CRYSTAL).sub(amount) >= 0, 'Burn amount exceeds balance');
