...
contract Cards is Permissions, ReentrancyGuard, ERC1155Drop {

    mapping(uint256 => string[]) private unlockableContent;
    mapping(address => bool) public hasMintedSpecial;
    uint256[] private specialTokenIdsCards;

    event SpecialClaimed(address indexed sender);
    ...
    function setUnlockableContent(uint256[] memory tokenIds, string[][] memory content) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(tokenIds.length == content.length, "The arrays must have the same size");
            for (uint i = 0; i < tokenIds.length; i++) {
                unlockableContent[tokenIds[i]] = content[i];
            }
    }

    function getUnlockableContent(address account, uint256 tokenId) public view returns (string[] memory) {
        require(balanceOf[account][tokenId] > 0, "You are not the owner of this Card");
        return unlockableContent[tokenId];
    }

    function setSpecialTokens(uint256[] calldata tokenIdsCards) external onlyRole(DEFAULT_ADMIN_ROLE) {
        for (uint256 i = 0; i < tokenIdsCards.length; i++) {
            specialTokenIdsCards.push(tokenIdsCards[i]);
        }
    }

    function hasEligibleTokenCards() public view returns (bool) {
        // Check if the sender has any of the special card tokens
        for (uint256 i = 0; i < specialTokenIdsCards.length; i++) {
            if (balanceOf[msg.sender][specialTokenIdsCards[i]] > 0) {
                return true;
            }
        }
        return false;
    ...
    function claimSpecial() external nonReentrant {
        require(!hasMintedSpecial[msg.sender], "You have already minted the Card #0");
        require(hasEligibleTokenCards(), "You do not have an Special Card");
        hasMintedSpecial[msg.sender] = true;
        super._mint(msg.sender, 0, 1, "");
        emit SpecialClaimed(msg.sender);
    }
}