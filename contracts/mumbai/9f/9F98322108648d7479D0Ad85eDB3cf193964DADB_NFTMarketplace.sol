...
interface IDistantCollections {
    function getTokenCreatorData(uint256 id)
        external
        view
        returns (TokenCreatorData memory);

    function getTokenCreatorDataFee(uint256 id) external view returns (uint256);

    function getTokenCreator(uint256 id) external view returns (address);
}
...
contract NFTMarketplace is Context, Ownable, Pausable, ReentrancyGuard {
    ...
    event CollectionAdded(address indexed collection, address collectionFeeCollector, uint256 royaltyFees);
    event CollectionUpdated(address indexed collection, address collectionFeeCollector, uint256 royaltyFees);
    event CollectionVerificationStatus(address indexed collection, Status status, string ipfs);
    ...
    /// All read functions
    function getAccountRevenue(address _account)
        external
        view
        returns (uint256)
    {
    ...
    modifier isProtocolCall() {
        require(isProtocol[_msgSender()], "Only in-House protocols can call");
        _;
    }

    // Write functions

    function delist(address _collection, uint256 _tokenId) external isProtocolCall {
        _delist(_collection, _tokenId);
    }

    function _delist(address _collection, uint256 _tokenId)
        private
        whenNotPaused
    {
     ...
     * @param _tokenId uint256 of the tokenId
     */
    function buyWithKCS(address _collection, uint256 _tokenId)
        external
        payable
        whenNotPaused
        nonReentrant
    ...
    function _buyNFT(
        ...
        (
            ...
            uint256 tokenFee
        ) = calculateFee(_collection, _tokenId, _price, tradeFee);
    ...
    function _updateRevenue(
        ...
        if (tokenFee != 0) {
            address creator = IDistantCollections(DistantCollections)
                .getTokenCreator(_tokenId);
            revenue[creator] += tokenFee;
        }
    }
        ...
        )
    {
        ...
        if (_collection == DistantCollections) {
            uint256 _tokenFee = IDistantCollections(DistantCollections)
                .getTokenCreatorDataFee(tokenId);
            tokenFee = (_price * _tokenFee) / 10000;
        }
        amount = _price - (marketplaceFee + collectionFee + tokenFee);
        return (amount, marketplaceFee, collectionFee, tokenFee);
    }
    ...
    */
    function updateCollection(
        ...
        address _collectionAddress,
        uint256 _royaltyFees
    ) external whenNotPaused isCollection(_collection) {
        require(
            _royaltyFees >= minFees && _royaltyFees <= (maxFees - tradeFee),
            "high fees"
        );
    ...
    */
    ...
    /** 
        ...
        @param _collection address of the collection
    */
    function verifyCollectionStatus(address _collection, string calldata _ipfsHash)
        external
        isAdmin
        isCollection(_collection)
    {
        ...
        emit CollectionVerificationStatus(_collection, collectionStatus.status, _ipfsHash);
    }
    ...
    */
        ...
        nonReentrant
    {
        ...
        bool singleApproval = __collection.getApproved(_tokenId) == address(this);
    ...
    */
    ...
    function updateProtocol(address newProtocol) external onlyOwner {
        isProtocol[newProtocol] = !isProtocol[newProtocol];
    }
    ...
    */
    ...
    function updateDistantCollectionAddress(address _DCL) external onlyOwner {
        DistantCollections = _DCL;
    }
}