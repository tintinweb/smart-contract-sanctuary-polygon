...
contract TokenSale {
    ...
    IERC20 public token; // Your token contract
    ...
    constructor(address _usdtTokenAddress, address _tokenAddress, uint _price) {
    ...
    receive() external payable {
        revert("This contract does not accept Ether. Please use the buyTokensWithUSDT function.");
    }

    function buyTokensWithUSDT(uint usdtAmount) public {
        require(!saleEnded, "The sale has ended");

        uint tokenAmount = usdtAmount * price;
        require(tokenAmount > 0, "Insufficient payment");

        // Transfer USDT tokens from the buyer to the contract
        ...
        tokenClaims[msg.sender] += tokenAmount;
