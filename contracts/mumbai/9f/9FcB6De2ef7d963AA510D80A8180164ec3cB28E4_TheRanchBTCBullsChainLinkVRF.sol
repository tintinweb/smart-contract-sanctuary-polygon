...
contract TheRanchBTCBullsChainLinkVRF is VRFConsumerBaseV2, Ownable {
    ...
    */
    function fulfillRandomWords(
        ...
        uint256 indexOfWinner = randomWords[0] % dailyRafflePlayersCount;
...
error Maintenance_FeeOverCeiling();
...
contract TheRanchBTCBullsCommunity is 
    ...
    // Raffle Variables
    address[] internal dailyRafflePlayers;
    ...
    */
    ...
    struct BTCBullOwner {
        ...
        uint256 maintenanceFeesStanding;     // how many months are they behind on the maintenance fees? 0 means all paid up, 4 gets liquidated.
        uint256 lastRewardDate;              // this tracks when the last time I rewarded them. Aug 2022 would be 0822, Mar 2023 would be 0323. 
        uint256 liquidationCount;            // How many times has this account been liquidated before?
    }
    ...
    uint256 public maint_fee_ceiling; // monthly maintenance fees per nft can't exceed this value
    ...
    */
    ...
    /**
     * @dev Excluded NFTs are a special role and incentive within the project to be excluded from maintenance fees 
    */
    mapping(uint256 => bool) public excludedNftIndexes;
    uint256 public excludedNftIndexesCount;
   ...
   // MINTING
     ...
     * 5. If msg.sender elects to enter raffle, 90% goes to btcMinersFund, if they do not, 93% does. 
    */
    function mint(uint256 _tokenQuantity, bool _enterRaffle) public payable {
        ...
        uint256 btcMinersSafeAmt = totalTransactionCost - (referralFundAmt + raffleFundAmt + marketingSafeAmt + hostingSafeAmt); 
        ...
        // if (dailyRafflePlayers.length >= 5  || _tokenSupply.current() == 10000) {
        //     startRaffle();
        // }
    ...
    */
    ...
    function mintingRaffle(uint256 _winningIndex) external {
    ...
    function resetUserInDailyRaffle() internal {
        for (uint256 i=0; i< dailyRafflePlayers.length ; i++){
    ...
    */
    ...
    function rewardBulls(uint256 _stockyardNumber) public ADMIN_OR_DEFENDER {
        ...
        for( uint256 i = startingIndex; i <= endingIndex; i++) {
            ...
            if (bullOwnerAddress != address(0)){
                ...
                if (btcBullOwners[bullOwnerAddress].lastRewardDate != currentRewardingDate) {
                    ...
                    uint256 _nftCountForMaintenance = maintenanceFeeCount(bullOwnerAddress);
                    ...
                    uint256 _maint_fee_to_add = (_nftCountForMaintenance * calculatedMonthlyMaintenanceFee);
                    _bullOwner.maintenanceFeeBalance += _maint_fee_to_add;
         

                    // update the maintenanceFeesStanding for the _bullOwner
                    if (_maint_fee_to_add > 0) {
                        _bullOwner.maintenanceFeesStanding += 1;
                    } else if (_bullOwner.maintenanceFeeBalance > 0){
                        _bullOwner.maintenanceFeesStanding += 1;
                    } 


                    // Check if _bullOwner is 3 rewards cycles behind on the account 
                    if (_bullOwner.maintenanceFeesStanding == 3){
    ...
    * to update there maintenanceFeesStanding by 1 and add to liquidation list if they reach 3 in that category 
    */
    function updateMaintenanceStandingOfThoseNotRewardedThisMonth() external ADMIN_OR_DEFENDER {
        for( uint256 i; i < rewardedAddresses.length; i++) {
            address _address = rewardedAddresses[i];
            if (btcBullOwners[_address].WBTC_Balance > 0){
                if (btcBullOwners[_address].maintenanceFeeBalance != 0){
                    if (btcBullOwners[_address].lastRewardDate != currentRewardingDate) {
                        
                        // take action and add one to maintenanceFeesStanding
                        btcBullOwners[_address].maintenanceFeesStanding += 1;
                        
                        if (btcBullOwners[_address].maintenanceFeesStanding == 3){
    ...
    /**
     * @dev If the user has been added to the liquidityArray, that means they are 3 months behind on paying their maintenance fees
    ...
    **/
    function liquidateOutstandingAccounts() external ADMIN_OR_DEFENDER {
        ...
        for( uint256 i; i < upForLiquidation.length; i++) {
            address _culprit = upForLiquidation[i];


            BTCBullOwner storage _bullOwner = btcBullOwners[_culprit];

            // Does the _culprit have 45 dollars in their USDC.e rewards account? 
            // If they do, subtract it out and avoid liquidation. 
            uint256 _neededAmountToAvoidLiquidation = 45 * 10 ** 6;

            if (_bullOwner.USDC_Balance >= _neededAmountToAvoidLiquidation ) {
                _bullOwner.USDC_Balance -= _neededAmountToAvoidLiquidation;
                hostingSafeBalance += _neededAmountToAvoidLiquidation; 
                _bullOwner.maintenanceFeeBalance = 0;
                _bullOwner.maintenanceFeesStanding = 0;
                continue;
            } else {
                uint256 _amount = _bullOwner.WBTC_Balance;
                _bullOwner.WBTC_Balance = 0;
                totalAmountLiquidated += _amount; 

                // reset fees and months behind increment 
                _bullOwner.maintenanceFeeBalance = 0;
                _bullOwner.maintenanceFeesStanding = 0;  
                _bullOwner.liquidationCount += 1; 
                // emit event
    ...
    function getLiquidatedArrayLength() external view ADMIN_OR_DEFENDER returns (uint) {
    ...
    */
    function updateUsdcBonusFromAnotherContract(address[] memory _ownersOfTheNFTs, uint256 _amountToAdd) payable external {
       ...
       require(msg.value == 0, "funds do not add up");
        ...
        for( uint256 i; i < _ownersOfTheNFTs.length; i++) {
            ...
            uint256 referralAmt = _amountToAdd * 1 / 100;
    ...
    **/
    ...
    function add_to_usdc_rewards (uint256 _amountToDeposit) public payable nonReentrant {
        IERC20Upgradeable usdcToken = IERC20Upgradeable(usdcTokenContract);
        usdcToken.safeTransferFrom(msg.sender, address(this), (_amountToDeposit));

        btcBullOwners[msg.sender].USDC_Balance += _amountToDeposit;
        emit addedToUSDCBalance(_amountToDeposit, msg.sender);
    }
    ...
    function withdrawToken(address _tokenContract) external onlyOwner {
        ...
        if (_tokenContract == usdcTokenContract){
            _amt = tokenContract.balanceOf(address(this)) - (btcMinersSafeBalance + hostingSafeBalance + USDCRewardsBalance + marketingSafeBalance);
    ...
    function withdrawMarketingSafeBalance() external ADMIN_OR_DEFENDER {
        IERC20Upgradeable tokenContract = IERC20Upgradeable(usdcTokenContract);
        uint256 amtToTransfer = marketingSafeBalance;
        tokenContract.approve(address(this), amtToTransfer);
        tokenContract.safeTransferFrom(address(this), marketingSafe, amtToTransfer);
        marketingSafeBalance -= amtToTransfer;
    }
    ...
    function getUserAlreadyInDailyRaffleStatus(address _address) internal view returns (bool) {
   ...
   // METADATA
    ...
    function setPauseStatus(bool _paused) external ADMIN_OR_DEFENDER{
        ...
        if(address(ChainLinkVRFContract) == address(0)) { revert Pause_MustSetAllVariablesFirst();}
    ...
    function setSafeAddresses(address _hostingSafe, address _btcMinersSafe, address _marketingSafe, address _chainlinkVRF) external onlyOwner {
        ...
        ChainLinkVRFContract = _chainlinkVRF;
    }

    function setUsdcTokenAddress(address _address) external onlyOwner {
        if (address(_address ) == address(0)) { revert Address_CantBeAddressZero();}
        usdcTokenContract = _address;
    }

    function setWbtcTokenAddress(address _address) external onlyOwner {
        if (address(_address ) == address(0)) { revert Address_CantBeAddressZero();}
    ...
    function setMaintenanceFeeCeiling(uint256 _ceiling) external onlyOwner {
        maint_fee_ceiling = _ceiling;
    }
    
    function setMonthlyMaintenanceFeePerNFT(uint256 _monthly_maint_fee_per_nft) external onlyOwner {
        if (_monthly_maint_fee_per_nft > maint_fee_ceiling) { revert Maintenance_FeeOverCeiling();}
    ...
    function setExcludedMaintenanceNFTs(uint256 _exemptIndex, bool _bool) external onlyOwner {
        excludedNftIndexes[_exemptIndex] = _bool;
        excludedNftIndexesCount += 1;
        
    }

    /**
     * @dev Used to see how  many NFTs they own then subtract any NFT's that are 
     * in the excludedMaintenanceNFTS mapping so it will reduce the amount of maintenance fees
     * that this person will have to pay monthly.
    **/
    function maintenanceFeeCount(address _owner) public view returns (uint256 _returnCount) {
        uint256 _count = balanceOf(_owner);
        for (uint256 i; i < _count; i++) {
            if (excludedNftIndexes[(tokenOfOwnerByIndex(_owner, i))] == true) {
                _count -= 1;
            }
        }
        return _count;
    ...
    function getliquidationCountForAddress(address _addressToCheck) external view returns (uint256) {
        return btcBullOwners[_addressToCheck].liquidationCount;
    }   
