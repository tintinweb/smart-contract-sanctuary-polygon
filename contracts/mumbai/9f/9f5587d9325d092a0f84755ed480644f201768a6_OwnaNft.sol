...
contract OwnaNft is ERC721, ERC721Burnable, Ownable, ERC721URIStorage {
  ...
  // ============ Mutable Variables ============    
  uint256 public noOfTokenId;  
    
  // ============ Events ============ 
  event Minted(uint256 tokenId, string nftTitle, string uri, address Owna, address borrower);

  // ============ Mappings ============
  mapping (uint256 => address) public OwnaDetails;
    
  constructor() ERC721("NFT Loans", "PWNL") {}

  // ============ Functions ============
  function mint(address ownaContrat, address _borrower, string memory nftTitle, string memory uri) external onlyOwner {
    require(bytes(uri).length > 0, "Invalid URI");
    require(_borrower != address(0) && ownaContrat != address(0), "Invalid address");
    require(bytes(nftTitle).length > 0, "Invalid NFT title");

    uint256 tokenId = ++noOfTokenId;
    _mint(ownaContrat, tokenId);
    _setTokenURI(tokenId, uri);
    
    OwnaDetails[tokenId] = _borrower;

    emit Minted(tokenId, nftTitle, uri, ownaContrat, _borrower );
  }
  ...
  function transferNft(address to, uint256 tokenId) external onlyOwner{
    ...
    _safeTransfer(address(this), to, tokenId,"0x00");
  }
  ...
  function burn(uint256 nftID) public override {
    _burn(nftID);
    OwnaDetails[nftID] = address(0x0);
  }

  // Will return tokenURI of the NFT 
