...
contract MultiDexRouter is Ownable {
    ...
    function _swapSupportingFeeOnTransferTokensForMultiDex(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            address _factory = _routerAddressList.length > 1 ? checkRouter(_routerAddressList[i]) : checkRouter(_routerAddressList[0]);
            ...
            x.path_[0] = x.input;
            x.path_[1] = x.output;
            x.amountOutput = _routerAddressList.length > 1 ? ((Irouter(_routerAddressList[i]).getAmountsOut(x.amountInput, x.path_))[1]) : (Irouter(_routerAddressList[0]).getAmountsOut(x.amountInput, x.path_))[1];
            (x.amount0Out, x.amount1Out) = x.input == x.token0 ? (zero, x.amountOutput) : (x.amountOutput, zero);
            if (i < path.length - 2) {
                address _factory2 = _routerAddressList.length > 1 ? checkRouter(_routerAddressList[i + 1]) : checkRouter(_routerAddressList[0]);
    ...
    function checkAr(
        ...
        if (_routerAddressList.length > 1) {
            require(_routerAddressList.length == path.length.sub(1), "multiDexRouter: INVALID_ROUTER_ADDRESS_LIST");
        }
        if (_routerAddressList.length == 1) {
            require(path[path.length - 1] != path[0], "multiDexRouter: INVALID_SWAP_TOKEN");
        }
    }
    ...
    function massGetPairInfo(PairStruct0[] memory pairInfoList_) external view returns (PairStruct1[] memory _pairInfoList){
        _pairInfoList = new PairStruct1[](pairInfoList_.length);
        for (uint256 i = 0; i < pairInfoList_.length; i++) {
