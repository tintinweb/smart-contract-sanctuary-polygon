...
contract BoardMember{
    ...
    */
    ...
    Manager public manager = Manager(abi.encode("manager"), abi.encode("Tyler"), 0xb94ae34DE09B1EeF75E18e8Ed17F91C32E9B0A9f);
    ...
    function terminationOfDuties(address _in, string memory _name, string memory _position) public {
    ...
    function transferRights(address _from, address _to, uint amount) public notBlacklisted(_to){
        IERC20(BSR).transferFrom(_from, _to, amount);
        emit TRANSFER_OF_RIGHTS(_from, _to);
    }

    function dissociation(address _who, string memory _reason) public {
    ...
    function dissociationReassign(address _who) public {
        blacklist[_who] = true;
    }

    function amendArticles() public {
        emit AMEND_ARTICLES(counter, true);
        counter++;
    }
    function mortgageManners(string memory _type) public {
        emit MORTGAGE_MANNERS(abi.encode(_type), true);
    }
    function merger(string memory _who) public {
        emit MERGER(abi.encode(_who), true);
    }
    function incurDebt(uint _amt) public {
        emit INDEBTEDNESS(_amt, true);
    }
    function changeBusinessNature(string memory _to) public {
        emit CHANGE_NATURE_OF_BUSINESS(abi.encode(_to), true);
    }
    function commenceBankruptcy() public {
        emit COMMENCE_VOLUNTARY_BANKRUPTCY(true);
    }
    function changeAmountManagers(string memory _addSub, uint _amt) public {
        emit CHANGE_AMT_MANAGERS(abi.encode(_addSub), true, _amt);
    }
    function addShares(string memory _class, uint _amount) public {
