...
contract AuctionHouse is VRFConsumerBaseV2, Ownable, AccessControl {
  ...
  constructor(
    ...
    updateChainlinkConfiguration(_keyHash, _subscriptionId);
  }
  ...
  function enterAuction(uint id, address sender, uint amount, uint256[] memory tokenIds, bytes calldata signature, uint sigMessageLength) public onlyController {
    ...
    if (auction._type == auction_types.REGULAR) {
      ...
      CURRENCY.transferFrom(sender, address(this), amount * 1 ether);
      ...
      if (entries[id].length > auction.amountOfWinners) {
        ...
        CURRENCY.transfer(entry.bidder, entry.amount);
      }
      ...
      CURRENCY.transferFrom(sender, address(this), price * _amount * 1 ether);
      ...
      CURRENCY.transferFrom(sender, address(this), auction.ticketPrice * amount * 1 ether);
  ...
  function requestRandomnessForAuction(uint id) public onlyController {
    ...
    vrfRequests[COORDINATOR.requestRandomWords(keyHash, subscriptionId, requestConfirmations, callbackGasLimit, 1)] = id;
  ...
  function updateChainlinkConfiguration(bytes32 _keyHash, uint64 _subscriptionId) public onlyOwner {
    keyHash = _keyHash;
  ...
  function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
    ...
    auction.vrfRandomness = randomWords[0];
  }
}
