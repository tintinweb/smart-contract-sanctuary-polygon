...
contract Rewards is Ownable {
    ...
    /**
     * @notice Called by contract owner to submit new Asset pack along with POW collateral
     * @dev Assumes owner has approved Rewards contract to take POW
     * @param _powIn Amount of POW collateral to send to Rewards contract
     * @param _resourceIds Array of IDs of resources to add
     * @param _resourceAmounts Array of amounts of each resource to add
     * @param _itemIds Array of IDs of items to add
     * @param _itemAmounts Array of amounts of each item to add
     */
    function submitAssetPack(
        uint256 _powIn,
        ...
        // Transfer POW from owner to this contract
        IERC20(pow).transferFrom(msg.sender, address(this), _powIn);

        // Add Resources to Asset Pool
        resources.addResources(_resourceIds, _resourceAmounts);

        // Add Items to Asset Pool
        gameItems.addItems(_itemIds, _itemAmounts);
    ...
    function claimGameItemRewards(
        ...
        for (uint i = 0; i < len; ) {
            ...
            uint256 _powIn = assetPool.calcPowInFromResourcesOut(
                _reward.resourceIds, 
                _reward.amounts
            );
            if (_powIn > _reward.maxPowVal) revert SlippageExceeded();

            uint256 _powFee = _powIn * craftingFee / 1000000000000000000;
            IERC20(pow).transfer(address(assetPool), _powIn - _powFee);
...
contract AssetPool is Ownable, ERC1155Holder {
    ...
    function calcResourcesOutFromPOWIn(
        ...
        if (_len != _powSplitUp.length) revert LengthMismatch();
        ...
        for (uint256 i = 0; i < _len; ) {
            ...
            _amounts[i] = Balancer.outGivenIn(
                resources.balanceOf(address(this), _id), //balanceO
    ...
    function calcPOWOutFromResourcesIn(
        ...
        for (uint256 i = 0; i < _len; ) {
            ...
            uint256 _amountO = Balancer.outGivenIn(
                ...
                resources.balanceOf(address(this), _id), // balanceI
                _amountsIn[i]
            );
...
contract Resources is ERC1155, Ownable {
    ...
    constructor(
        ...
        _mintBatch(_assetPool, _ids, _amounts, "");
    ...
    function addResources(
        ...
        _mintBatch(assetPool, _ids, _amounts, "");
    }
...
contract Crafting is Ownable {
    ...
    function destroyForResources(
        ...
        for (uint i = 0; i < len; ) {
            ...
            //Burn Game Item
            gameItems.burn(_request.prey, _request.itemId, 1);

            // If we're sending the resources back to AssetPool, return locked POW to Rewards contract
            if (_request.to == address(assetPool)) {
                uint256 _powVal = assetPool.calcPOWOutFromResourcesIn(_request.resourceIds, _request.resAmounts);
    ...
    function destroyForPow(
        ...
        for (uint i = 0; i < len; ) {
            ...
            if (_request.to == rewards) {
                ...
                resources.burnBatch(lockedResources, _request.resourceIds, _request.resAmounts);
            }
...
contract GameItems is ERC1155, Ownable {
     ...
     * @param _ids Array of item IDs to add
     * @param _amounts Array of amounts of each item to add
     */
    function addItems(
        uint256[] calldata _ids,
        uint256[] calldata _amounts
     ...
     * @param _amount Amount of item to burn
     */
    function burn(
        address _from, 
        uint256 _id,
     ...
     * @param _amounts Array of amounts of each Game Item to burn
     */
    function burnBatch(
        address _from, 
        uint256[] calldata _ids,
