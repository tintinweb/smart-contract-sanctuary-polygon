...
contract StakingGDEX is Ownable {
    ...
    uint public constant MAX_PENALTY_PERCENT = 3000;

    mapping(address => UserStakingInfo) public usersStakingInfo;
    mapping(address => mapping(uint => UserLockInfo)) public usersLockInfo;
    mapping(address => UnstakingStatus) public userUnstakingStatus;
    ...
    event Locked(address indexed user, uint amount, uint period);
    event RequestedUnstaking(address indexed user, bool isRequested);
    event Unstaked(address indexed user);
    event Unlocked(address indexed user, uint period);
    event UpgradedLock(address indexed user, uint previousPeriod, uint newPeriod);
    event ForceUnlock(address indexed user, uint period);
    ...
    function stake(uint amount) public {
        ...
        UserStakingInfo memory user = usersStakingInfo[msg.sender];

        user.stakedAmount += amount;
        user.updatedTime = block.timestamp;

        usersStakingInfo[msg.sender] = user;
        IERC20(stakedToken).transferFrom(msg.sender, address(this), amount);

        if (userUnstakingStatus[msg.sender].isRequested) {
            userUnstakingStatus[msg.sender].isRequested = false;
        }

        emit Staked(msg.sender, amount);
    }

    function lock(uint amount, uint period) public isValidPeriod(period) {
        require(amount > 0, "Invalid amount");

        UserLockInfo memory user = usersLockInfo[msg.sender][period];
        ...
        usersLockInfo[msg.sender][period] = user;

        emit Locked(msg.sender, amount, period);
    }

    function requestUnstake(bool isUnstaking) public {
        require(usersStakingInfo[msg.sender].stakedAmount > 0, "Invalid staked amount");
        UnstakingStatus memory userStatus = userUnstakingStatus[msg.sender];
        ...
        userUnstakingStatus[msg.sender] = userStatus;
        emit RequestedUnstaking(msg.sender, isUnstaking);
    }

    function unStake() public {
        UserStakingInfo memory user = usersStakingInfo[msg.sender];
        require(user.stakedAmount > 0, "No staked amount");
        require(userUnstakingStatus[msg.sender].isRequested, "No requested");
        require(block.timestamp - userUnstakingStatus[msg.sender].requestedTime >= cooldownPeriod, "No passed cooldown period");
        
        delete usersStakingInfo[msg.sender];

        IERC20(stakedToken).transfer(msg.sender, user.stakedAmount);
        emit Unstaked(msg.sender);
    }

    function unLock(uint period) public isValidPeriod(period) {
        UserLockInfo memory user = usersLockInfo[msg.sender][period];
        ...
        delete usersLockInfo[msg.sender][period];

        IERC20(stakedToken).transfer(msg.sender, user.lockedAmount);
        emit Unlocked(msg.sender, period);
    }

    function convertStakingToLock(uint period) public isValidPeriod(period) {
        UserStakingInfo memory user = usersStakingInfo[msg.sender];
        require(user.stakedAmount > 0, "No staked amount");
        delete usersStakingInfo[msg.sender];
        
        UserLockInfo memory userLock = usersLockInfo[msg.sender][period];

        userLock.lockedAmount += user.stakedAmount;
        userLock.lockedTime = block.timestamp;

        emit UpgradedLock(msg.sender, 0, period);
    }

    function upgradeLock(uint oldPeriod, uint newLockedPeriod) public isValidPeriod(newLockedPeriod) {
        require(newLockedPeriod > oldPeriod, "Only available for increasing locked period");

        UserLockInfo memory user = usersLockInfo[msg.sender][oldPeriod];
        require(user.lockedAmount > 0, "No locked amount");
        delete usersLockInfo[msg.sender][oldPeriod];

        UserLockInfo memory newPeriodLockInfo = usersLockInfo[msg.sender][newLockedPeriod];
        newPeriodLockInfo.lockedTime = block.timestamp;
        newPeriodLockInfo.lockedAmount += user.lockedAmount;

        usersLockInfo[msg.sender][newLockedPeriod] = newPeriodLockInfo;
        emit UpgradedLock(msg.sender, oldPeriod, newLockedPeriod);
    }

    function forceUnLock(uint period) public {
        UserLockInfo memory user = usersLockInfo[msg.sender][period];
        require(user.lockedAmount > 0, "No locked amount");
