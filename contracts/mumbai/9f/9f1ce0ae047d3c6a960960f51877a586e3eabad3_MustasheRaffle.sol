...
contract MustasheRaffle is ERC721Holder, Ownable {
    ...
    address public tokenAddress = 0x8909fCF37B1A7382dBA034CC7bC71BEc6E79811F; 
    address public feeAddress = 0x5CEe0e6D261dA886aa4F02FB47f45E1E9fa4991b;
   ...
   function createRaffle(
    ...
    function participateRaffle(uint256 raffleId, uint256 ticketCount) external {
        ...
        // Check if the raffle is fully sold out and end it if true
    ...
    function endRaffleAfterExpiry(uint256 raffleId) external {
        ...
        uint256 ticketSupply = raffles[raffleId].ticketSupply;
        ...
        if (ticketSold > 0) {
            ...
            uint256 winningTicket = generateRandomNumber(raffleId, ticketSupply);
            raffles[raffleId].winnerTicket = winningTicket;
            ...
            IERC20 token = IERC20(tokenAddress);
            uint256 prizeAmount = raffles[raffleId].ticketPrice.mul(ticketSold).mul(TOKEN_DECIMALS);
            uint256 feeAmount = prizeAmount.mul(FEE_PERCENTAGE).div(100);
            ...
            require(raffles[raffleId].creator == msg.sender || raffles[raffleId].ticketSupply > 0, "Only the creator can end the raffle after expiry when no tickets were sold");
