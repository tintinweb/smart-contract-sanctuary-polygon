...
import "../interfaces/IERC1155Cloneable.sol";
...
contract NiftyCloneFactory is NiftyPermissions {
    ...
    event ClonedERC1155(address newToken);    
    event ClonedMetadataGenerator(address metadataGenerator);    
    ...
    function cloneERC721(address implementation, address niftyRegistryContract_, address defaultOwner_, string calldata name_, string calldata symbol_, string calldata baseURI_) external returns (address) {
        ...
        if(IERC165(implementation).supportsInterface(type(IDefaultOwnerCloneable).interfaceId)) {
            IDefaultOwnerCloneable(clone).initializeDefaultOwner(defaultOwner_);
        }        

        return clone;
    }

    function cloneERC1155(address implementation, address niftyRegistryContract_, address defaultOwner_, string calldata baseURI_) external returns (address) {
        _requireOnlyValidSender();
        require(IERC165(implementation).supportsInterface(type(IERC1155Cloneable).interfaceId), "Not a valid ERC1155 Token");        
        address clone = Clones.clone(implementation);

        emit ClonedERC1155(clone);

        IERC1155Cloneable(clone).initializeERC1155(baseURI_);        
    ...
    function cloneMetadataGenerator(address implementation, address niftyRegistryContract_) external returns (address) {
        _requireOnlyValidSender();
        require(IERC165(implementation).supportsInterface(type(IMetadataGenerator).interfaceId), "Not a valid Metadata Generator");
        address clone = Clones.clone(implementation);        

        emit ClonedMetadataGenerator(clone);
...
interface IERC1155Cloneable is IERC1155 {
    function initializeERC1155(string calldata baseURI_) external;
}
