...
contract RangoMultichainFacet is IRango, ReentrancyGuard, IRangoMultichain {
    ...
    event MultichainBridge(
        MultichainBridgeType actionType,
    ...
    function multichainSwapAndBridge(
        ...
        if (bridgeRequest.actionType == MultichainBridgeType.OUT_NATIVE) {
            require(request.toToken == LibSwapper.ETH, 'Invalid token, it must be equal to null address');
        }
    ...
    function multichainBridge(
        ...
        if (request.actionType == MultichainBridgeType.OUT_NATIVE) {
            require(msg.value >= amount, 'Insufficient ETH sent for OUT_NATIVE action');
            require(token == LibSwapper.ETH, 'Invalid token, it must be equal to null address');
        }
