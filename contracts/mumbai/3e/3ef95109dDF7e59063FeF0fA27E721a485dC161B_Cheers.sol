...
contract Cheers is ICheers, AccessControl {
  // CHERトークンのアドレス
  IERC20 public cher;
  ...
  modifier onlyHasIAmCheersNFT() {
    _checkHasIAmCheersNFT(msg.sender);
    _;
  }

  function _checkHasIAmCheersNFT(address owner) internal view {
    require(iAmCheersNFT.balanceOf(owner) != 0, "You don't have IAmCheersNFT");
  }

  // CHERトークンのアドレスをセット
  // @note 管理者のみ実行　管理者とは？
  function setCherTokenAddress(address _cherTokenAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
  ...
  // @note 管理者のみ実行　管理者とは？
  function setIAmCheersNFTAddress(address _iAmCheersNFT) external onlyRole(DEFAULT_ADMIN_ROLE) {
    iAmCheersNFT = IIAmCheersNFT(_iAmCheersNFT);
  }

  // cheersDataのアドレスを変更
  // @note 管理者のみ実行　管理者とは？
  function setCheersDataAddress(address _cheersData) external onlyRole(DEFAULT_ADMIN_ROLE) {
  ...
  // 登録したUser/DAOのみ実行可能
	...
	) external onlyHasIAmCheersNFT returns (address) {
  ...
  // cheerする
  // 登録したUser/DAOのみ実行可能
  function cheer(address projectPoolAddress, uint256 _cheerCherAmount, string memory _cheerMessage) external onlyHasIAmCheersNFT {
