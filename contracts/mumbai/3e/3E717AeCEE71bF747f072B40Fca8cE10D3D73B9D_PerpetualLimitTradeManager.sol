    ...
    IPerpetualLimitTradeManager
{
        ...
        */
        ...
        if (_order.flags.isStopOrder()) {
            ...
            _getTradeLogic().validateStopPrice(
                _order.fAmount > 0,
                _getPerpetualMarkPrice(perpetual),
    ...
    function chargePostingFee(Order calldata _order, uint16 _feeTbps) external override {
        ...
        perpetual.fAMMFundCashCC = perpetual.fAMMFundCashCC.add(feeCC);
        pool.fAMMFundCashCC = pool.fAMMFundCashCC.add(feeCC);
        _transferFromUserToVault(pool.marginTokenAddress, _order.traderAddr, feeCC);
        emit UpdateAMMFundCash(_order.iPerpetualId, perpetual.fAMMFundCashCC, pool.fAMMFundCashCC);
    }
    ...
    function rebatePostingFee(Order calldata _order, uint16 _feeTbps) external override {
        ...
        int128 feeCC = fAmount.mul(ConverterDec18.tbpsToABDK(_feeTbps));
        perpetual.fAMMFundCashCC = perpetual.fAMMFundCashCC.sub(feeCC);
        pool.fAMMFundCashCC = pool.fAMMFundCashCC.sub(feeCC);
        _transferFromVaultToUser(pool.marginTokenAddress, _order.traderAddr, feeCC);
        emit UpdateAMMFundCash(_order.iPerpetualId, perpetual.fAMMFundCashCC, pool.fAMMFundCashCC);
    }
