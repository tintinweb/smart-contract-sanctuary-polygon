...
contract EnglishAuction is AccessControl {
    ...
    constructor(uint256 sListingTax_, address wla_) {
        wla = wla_;
        _sListingTax = sListingTax_;
    ...
    function end(
        ...
        if (accept == true) {
            if (auction.highestBidder != address(0)) {
                uint256 tax = ((auction.highestBid * listingTax()) / _DIV); // 100%
                uint256 taxMarketplace = (((auction.highestBid * listingTax()) / _DIV) * 10) / 5; // 20%
                uint256 taxCreator = taxMarketplace * 2; // 40%

                uint256 amountAfterTax = auction.highestBid - tax;

                auction.nft.safeTransferFrom(address(this), auction.highestBidder, auction.tokenId);

                payable(wla).transfer(taxCreator + taxMarketplace);
                payable(creator).transfer(taxCreator);
                payable(auction.seller).transfer(amountAfterTax);
            } else {
