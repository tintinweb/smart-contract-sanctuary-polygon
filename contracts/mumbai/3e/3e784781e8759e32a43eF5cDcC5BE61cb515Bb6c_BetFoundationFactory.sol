...
contract BetFoundationFactory is
    TokenConfig,
    ReentrancyGuard,
    IBetFoundationFactory
{
    ...
    constructor(
        ...
        require(config_ != address(0), "Zero Address");
        require(aggregator_ != address(0), "Zero Address");
        admin = admin_;
        config = config_;
    ...
    function updateConfig(address config_)
        external
        isAdmin(admin)
        returns (bool)
    {
    ...
    function updateAggregator(address aggregator_)
        external
        isAdmin(admin)
        returns (bool)
    {
    ...
    function setDisputeResolver(address resolver_)
        external
        isAdmin(admin)
        returns (bool)
    {
        ...
        returns (bool _status, address _betTrendSetter)
    {
        ...
        require(betId_ != address(0), "ZERO ADDRESS");
        require(betTakerRequiredLiquidity_ != 0, "ZERO TAKER LIQUIDITY");
        require(totalBetOptions_ != 0, "ZERO BET OPTION");
        require(tokenLiqidity_ != 0, "ZERO CREATION LIQUIDITY");
        ...
        returns (bool)
    {
        ...
        if (
            ...
            uint256(betDetails[betAddress_].betEndingTime - block.timestamp)
        ) {
    ...
    function raiseDispute(address betAddress_)
        ...
        returns (bool)
    {
        require(betAddress_ != address(0), "ZERO ADDRESS");
