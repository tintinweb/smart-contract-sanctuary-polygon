...
contract PartyKarts is ERC721Drop {
    ...
    function submitRaceResults(string memory raceResultsJson) public {
        bytes memory input = bytes(raceResultsJson);
        bytes memory encoded = abi.encode(input);
        RaceResult[] memory decoded = abi.decode(encoded, (RaceResult[]));
        for (uint i = 0; i < decoded.length; i++) {
            raceResults[msg.sender][decoded[i].name] = decoded[i];
        }
        // Do something with the race results
    }
