...
contract InsidersPass is InsidersPassAuction {
        ...
        if (_msgSender() != owner()) {
            if (
             ...
             if (promoCodeDetails[_promoCode].discountIsPercentage) {
                        uint256 newDiscountedAmount = ticketTypeDetails[ticketID].ticketFixPrice - ((promoCodeDetails[_promoCode].discountPercentage /    100) * (ticketTypeDetails[ticketID].ticketFixPrice));
                        if (promoCodeDetails[_promoCode].maxDiscountAmount > newDiscountedAmount ) {
                            if(newDiscountedAmount * noOfCopies > promoCodeDetails[_promoCode].maxDiscountAmount ){
                                newDiscountedAmount = promoCodeDetails[_promoCode].maxDiscountAmount ;
                           //add this if to set maxDiscountAmount
                            }
                            checkPrice(((newDiscountedAmount * noOfCopies)));
                            promoCodeDetails[_promoCode].claimedUsers++;
                        } else {
    ...
    /// here is new funtion update promoCode
