...
/**
 * @title UserRegistry SC.
 * @author pchn
 * @notice Errors and events are specified in IUserRegistry
 * interface (./contracts/interfaces/IUserRegistry.sol)
 */
contract UserRegistry is OwnableUpgradeable, IUserRegistry {
    // STATE VARIABLES

    /// @inheritdoc IUserRegistry
    mapping(address => uint256) public override ticketsBalance;
    //user address => tickets balance

    uint256 ticketPriceInGasToken;
    //multiplied by 1e18

    uint256 ticketPriceInErc20Token;
    //multiplied by 1e18

    address public override paymentTokenAddress;
    //ERC20 token to buy tickets for
    ...
    modifier onlyBattleContract() {
        if (msg.sender != battleContractAddress) revert CallRestricted();
        _;
    }
    ...
    function initialize(
        ...
        ticketPriceInGasToken = _ticketPriceInGasToken;
        ticketPriceInErc20Token = _ticketPriceInErc20Token;
        paymentTokenAddress = _paymentTokenAddress;
        battleContractAddress = _battleContractAddress;

        __Ownable_init();

        emit TicketPriceInGasTokenChanged({newPrice: ticketPriceInGasToken});
        emit TicketPriceInERC20TokenChanged({
            newPrice: ticketPriceInErc20Token
        });
    }
    ...
    /// @inheritdoc IUserRegistry
    function buyTickets(uint256 ticketsAmount) external payable override {
        uint256 tokensAmount = 0;
        if (msg.value > 0) {
            tokensAmount =
                (ticketsAmount * ticketPriceInGasToken) /
                Decimals.priceDecimals;

            if (msg.value < tokensAmount)
                revert InsufficientFunds({
                    requiredFunds: tokensAmount,
                    providedFunds: msg.value
                });

            ticketsBalance[msg.sender] += ticketsAmount;

            // refund dust eth, if any
            if (msg.value > tokensAmount) {
                TransferHelper.smartTransferETH(
                    msg.sender,
                    msg.value - tokensAmount
                );
            }
            emit TicketsBoughtForEth({
                buyer: msg.sender,
                amount: ticketsAmount,
                ticketPrice: ticketPriceInGasToken
            });
        } else {
            tokensAmount =
                (ticketsAmount * ticketPriceInErc20Token) /
                Decimals.priceDecimals;

            uint256 balanceBefore = IERC20(paymentTokenAddress).balanceOf(
                address(this)
            );
            ...
            uint256 balanceAfter = IERC20(paymentTokenAddress).balanceOf(
                address(this)
            );

            if (balanceAfter < balanceBefore + tokensAmount)
                revert InsufficientFunds({
                    requiredFunds: tokensAmount,
                    providedFunds: balanceAfter > balanceBefore
                        ? balanceAfter - balanceBefore
                        : 0
                });

            ticketsBalance[msg.sender] += ticketsAmount;
            emit TicketsBoughtForERC20({
                buyer: msg.sender,
                amount: ticketsAmount,
                ticketPrice: ticketPriceInErc20Token
            });
        }
    }
    ...
    ) external override onlyBattleContract {
        if (ticketsBalance[userAddress] < ticketsAmount)
            revert InsufficientFunds({
                requiredFunds: ticketsAmount,
                providedFunds: ticketsBalance[userAddress]
            });
        ticketsBalance[userAddress] -= ticketsAmount;
        emit TicketsSpent({userAddress: userAddress, amount: ticketsAmount});
    }

    //END PUBLIC STATE-CHANGING FUNCTIONS

    //MAINTENANCE FUNCTIONS

    /// @inheritdoc IUserRegistry
    function updateTicketPriceInGasToken(
        uint256 newPrice
    ) external override onlyBattleContract {
        ticketPriceInGasToken = newPrice;
        //TODO: Add moving average price?

        emit TicketPriceInGasTokenChanged({newPrice: newPrice});
    }

    /**
     * Maintanence function used to change ticket`s price in native token.
     * @param newPrice new ticket`s price.
     * @dev Throws error if caller address differs from owner`s address
     * (see OwnableUpgradeable.owner()).
     * @dev Emits {IUserRegistry.TicketPriceInGasTokenChanged} event.
     */
    function setTicketPriceInGasToken(uint256 newPrice) external onlyOwner {
        ticketPriceInGasToken = newPrice;
        emit TicketPriceInGasTokenChanged({newPrice: newPrice});
    }

    /**
     * Maintanence function used to change ticket`s price in payment ERC20 token
     * (see IUserRegistry.paymentTokenAddress).
     * @param newPrice new ticket`s price.
     * @dev Throws error if caller address differs from owner`s address
     * (see OwnableUpgradeable.owner()).
     * @dev Emits {IUserRegistry.TicketPriceInERC20TokenChanged} event.
     */
    function setTicketPriceInErc20Token(uint256 newPrice) external onlyOwner {
        ticketPriceInErc20Token = newPrice;
        emit TicketPriceInERC20TokenChanged({newPrice: newPrice});
    }
    ...
    {
        _ticketPriceInGasToken = ticketPriceInGasToken;
        _ticketPriceInErc20Token = ticketPriceInErc20Token;
    }
