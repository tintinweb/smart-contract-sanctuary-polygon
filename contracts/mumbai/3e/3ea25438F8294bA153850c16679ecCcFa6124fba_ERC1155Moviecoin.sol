...
abstract contract ERC1155Lazy is IERC1155LazyMint, ERC1155BaseURI, Mint1155Validator, RoyaltiesV2Upgradeable, RoyaltiesV2Impl {
    ...
    function mintAndTransfer(LibERC1155LazyMint.Mint1155Data memory data, address to, uint256 _amount) public override virtual {
        ...
        if (supply[data.tokenId] == 0) {
            ...
            if(movieIdToProducer[data.movieId] == address(0)) _setMovieProducer(data.movieId, data.producer);
        }
    ...
    function _setMovieId(uint256 tokenId, string memory movieId) internal {
        tokenIdToMovieId[tokenId] = movieId;
    }

    function _setMovieRevealTime(string memory movieId, uint256 time) internal {
        nftRevealTime[movieId] = time;
    }

    function _setMovieProducer(string memory movieId, address producer) internal {
...
contract ProducerTokenUriValidator is Initializable {
    bytes32 public constant PRODUCER_TOKENURI_TYPEHASH = keccak256("ProducerTokenUri(uint256 tokenId,string movieId,string tokenUri)");
