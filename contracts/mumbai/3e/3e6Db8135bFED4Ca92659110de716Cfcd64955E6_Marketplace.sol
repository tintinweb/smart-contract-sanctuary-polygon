...
contract Marketplace is ERC721Holder {
    ...
    constructor(IERC20 _maticToken, uint256 _commissionRate) {
    ...
    function buy(address nftContract, uint256 itemId) external payable {
        ...
        require(msg.value >= item.price + calculateCommission(item.price), "Insufficient funds");
        ...
        // Transfer commission fee to the marketplace contract in MATIC tokens
        require(maticToken.transferFrom(msg.sender, address(this), commission), "Transfer of commission fee failed");
    }
    ...
    function withdrawCommission() external onlyMarketplaceOwner {
        uint256 commissionBalance = maticToken.balanceOf(address(this));
        require(commissionBalance > 0, "No commission to withdraw");

        require(maticToken.transfer(marketplaceOwner, commissionBalance), "Commission withdrawal failed");
    }

    function calculateCommission(uint256 price) internal view returns (uint256) {
