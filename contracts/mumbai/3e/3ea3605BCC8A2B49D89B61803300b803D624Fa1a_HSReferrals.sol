...
contract HSReferrals is Initializable {
    // CONSTANTS
    ...
    uint256 public targetVolumeUsdc; // USDC (to reach maximum referral system fee, eg. 1e8)
    ...
    event UpdatedTargetVolumeUsdc(uint256 value);
    ...
    function initialize(
        ...
        openFeeP = _openFeeP;
        targetVolumeUsdc = _targetVolumeUsdc;
    }
    ...
    function updateTargetVolumeUsdc(uint256 value) external onlyGov {
        require(value > 0, "VALUE_0");

        targetVolumeUsdc = value;

        emit UpdatedTargetVolumeUsdc(value);
    }
    ...
    function whitelistReferrer(address referrer, address ally) external {
        ...
        require(
            address(storageT.hsAgency()) == address(0) ||
                storageT.hsAgency().rootReferrer(referrer) == address(0),
            "EXISTED_IN_AGENCY"
        );
    ...
    function distributePotentialReward(
        ...
        uint256 referrerRewardValueUsdc = (volumeUsdc *
            getReferrerFeeP(pairOpenFeeP, r.volumeReferredUsdc)) /
            PRECISION /
            100;

        uint256 referrerRewardToken = referrerRewardValueUsdc;
        //storageT.handleTokens(address(this), referrerRewardToken, true);

        AllyDetails storage a = allyDetails[r.ally];

        uint256 allyRewardValueUsdc;
        ...
        return referrerRewardValueUsdc + allyRewardValueUsdc;
    }
    ...
    function claimAllyRewards() external {
        ...
        storageT.transferUsdc(address(storageT), msg.sender, rewardsToken);
    ...
    function getReferrerFeeP(
        ...
        uint256 feeP = minFeeP +
            ((maxReferrerFeeP - minFeeP) * volumeReferredUsdc) /
            1e18 /
            targetVolumeUsdc;
    ...
    function getPercentOfOpenFeeP(
        ...
        return
            getPercentOfOpenFeeP_calc(
                referrerDetails[referrerByTrader[trader]].volumeReferredUsdc
            );
    }
    ...
    function checkReferrerActive(
        address referrer
    ) external view returns (bool) {
        return referrerDetails[referrer].active;
    }
}
