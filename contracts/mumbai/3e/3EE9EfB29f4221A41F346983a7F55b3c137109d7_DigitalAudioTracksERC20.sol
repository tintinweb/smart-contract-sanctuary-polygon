...
contract DigitalAudioTracksERC20 is ERC1155 {
    ...
    bytes32 private constant FINANCE_ROLE = keccak256("FINANCE");
    //PaymentToken
    IERC20 private paymentToken;
    mapping(address => uint256) private paymentBalances;
    //Token Consumption
    event TokenIssued(
        uint256 tokenId,
        uint32 launchDate,
    ...
    mapping(uint256 => bytes32) private tokenToProject;
    // TODO: Only allow approved Mapping from token ID to approved address
    // All owners
    //Fees
    uint private standardMintingPercentage = 15;
    //Project Fees
    mapping(bytes32 => bool) private hasRegisteredMintingPercentage;
    mapping(bytes32 => uint) private registeredMintingPercentage;

    constructor(
        address _paymentToken
        //uint _standardMintingPercentage, //Amount we take from minting.
    )  ERC1155("https://app.paradimeaudio.com/api/NFT/MATIC/TESTNET/DAT/USDC/Token/{id}.json")  {
        baseURI = "https://app.paradimeaudio.com/api/NFT/DAT/Token/{id}.json";//_baseURI;
        standardMintingPercentage = 15;//_standardMintingPercentage;
    ...
    function approveIssuer(address _issuer) public onlyProjectAdmin {
        roleDeligations[APPROVED_ISSUER][_issuer] = true;
    }

    function approveAdmin(address _admin) public onlyProjectAdmin {
        roleDeligations[ADMIN_ROLE][_admin] = true;
    }

    function approveFinanceAdmin(address _admin) public onlyProjectAdmin {
        roleDeligations[FINANCE_ROLE][_admin] = true;
    }

    function removeIssuer(address _issuer) public onlyProjectAdmin {
        roleDeligations[APPROVED_ISSUER][_issuer] = false;
    }

    function removeAdmin(address _admin) public onlyProjectAdmin {
        roleDeligations[ADMIN_ROLE][_admin] = false;
    } 

    function removeFinanceAdmin(address _admin) public onlyProjectAdmin {
        roleDeligations[FINANCE_ROLE][_admin] = false;
    }  

    function getIssuerOfToken(bytes32 tokenId) public view returns (address) {
        return tokens[tokenId].issuer;
    }
    ...
    modifier isValidCreatorStakes(ParadimeTokens.Shares[] memory _owners,
        ...
        require(ParadimeTokens.isBalancedOwnership(_owners), "Unbalanced ownership stake, the total share amount must equal 100");
        require(ParadimeTokens.isBalancedOwnership(_residualOwners), "Unbalanced residual ownership stake, the total share amount must equal 100");
        _;
    }

    function uri(uint256 _tokenId)  public override view returns (string memory) {
        return string(bytes.concat(bytes(baseURI), tokenToProject[_tokenId]));
    }
    ...
    function issueToken(
        ...
        require(tokens[_projectId].owner == address(0), "A License was issued for this project already.");
        
        projectToTokens[_projectId] = issuedTokens.current();
        tokenToProject[issuedTokens.current()] = _projectId;
        tokens[_projectId] = ParadimeTokens.TokenDefinition({
            issuer: _issuer,
            owner: address(_license),
            tokenId: _projectId,
            listing: listing,
            totalMinted: 0,
            isPaused: false
    ...
    function getArtistAgreement(bytes32 _projectId)  public view returns (address) {
        //token
        return tokens[_projectId].owner;
    }

    function getTokenProject(uint256 _tokenId) public view returns (bytes32) {
        return tokenToProject[_tokenId];
    }

    function mint(bytes32 _tokenId, uint256 _amount) tokenExists(_tokenId) public {
        //Make User pay gas fee
        require(_amount > 0, "The specified amount must be greater than zero");
        require(getTokenPrice(_tokenId) <= _amount, "Incorrect amount issued");
        ...
        //Collect Payment in USDC, Update: Artist Balance + Standard Fee Balance
        
        // //Update Balance for Internal
        uint256 paradimeFee = (_price * standardMintingPercentage / 100);
        // ArtistAgreement _agreement = ArtistAgreement(getArtistAgreement(_tokenId));
        uint256 artistFee = _price - paradimeFee;//(_price * (100 - standardMintingPercentage) / 100);
        paymentBalances[address(this)] += paradimeFee;
        ArtistAgreement _agreement = ArtistAgreement(getArtistAgreement(_tokenId));
        paymentToken.transferFrom(msg.sender, address(this), paradimeFee);
        _agreement.depositToOwners(address(paymentToken), msg.sender, artistFee);
        //Update Supply
    ...
    function getTokenPrice(bytes32 _projectId) public view returns (uint256) {
        return tokens[_projectId].listing.price;
    }

    function getTokenTotalSupply(bytes32 _projectId) public view returns (uint256) {
        return tokens[_projectId].listing.supply;
    }

    function getTokenAvailableSupply(bytes32 _projectId) public view returns (uint256) {
        return tokens[_projectId].listing.supply - tokens[_projectId].totalMinted;
    }

    function getTokenTotalMinted(bytes32 _projectId) public view returns (uint256) {
        return tokens[_projectId].totalMinted;
    }
    ...
    function withdraw() public onlyProjectAdmin {
        require(roleDeligations[FINANCE_ROLE][msg.sender], "Only finance admins can perform this task");
        uint256 _accountBalance = paymentBalances[address(this)];
        paymentBalances[address(this)] = 0;
        paymentToken.transferFrom(address(this), msg.sender, _accountBalance);
    }
}
...
contract ArtistAgreement is ERC1155 {
    ...
    //Permissions
    TokenConsumptionTerms.PermissionsAndRoyalties private usageTerms;
    //Utility Tokens: TokenType => ID => Ownership
    mapping (uint256 => mapping(bytes32 => mapping(address => uint256))) utilityTokenBalances;
    //ERC20 Payments: Token => Address => Amount
    mapping(address => mapping(address => uint256)) private erc20Balances;
    //ERC20 Payments: Token => Address => Amount
    mapping(address => mapping(address => uint256)) private approvedERC20Tokens;
    ...
    constructor(
        ...
        ParadimeTokens.Shares[] memory _residualOwners,
        //Royalties
        uint[] memory _royalties,
        bool[] memory _permissions
    ) 
        ERC1155("https://app.paradimeaudio.com/api/NFT/ArtistAgreement/Token/{id}.json") { 
        //DAT_MANAGER = _manager;
        //tokenId = _baseTokenId;
        issuedBy = _issuer;
        //Royalties
        //TODO Add Cover.
        usageTerms.royalties[TokenConsumptionTerms.EXPORT_ACTION] = _royalties[0];//_exportRoyalty;
        usageTerms.royalties[TokenConsumptionTerms.SAMPLE_ACTION] = _royalties[1];//_sampleRoyalty;
        usageTerms.royalties[TokenConsumptionTerms.REMIX_ACTION] = _royalties[2];//_remixRoyalty;
        usageTerms.royalties[TokenConsumptionTerms.RESELL_ACTION] = _royalties[3];//_resaleRoyalty;
        usageTerms.royalties[TokenConsumptionTerms.COVER_ACTION] = _royalties[4];//_coverRoyalty;
        // //Permissions
        usageTerms.permissions[TokenConsumptionTerms.PUBLISH_ACTION] = _permissions[0];//_publishPermission; 
        usageTerms.permissions[TokenConsumptionTerms.EXPORT_ACTION] = _permissions[1];//_exportPermission;
        usageTerms.permissions[TokenConsumptionTerms.SAMPLE_ACTION] = _permissions[2];//_samplePermission;
        usageTerms.permissions[TokenConsumptionTerms.REMIX_ACTION] = _permissions[3];//_remixPermission; 
        usageTerms.permissions[TokenConsumptionTerms.COVER_ACTION] = _permissions[4];//_coverPermission;
        //Add Admins
        _mintOwnershipTokens(_owners);
        _mintResidualRoyaltyTokens(_residualOwners);
        usageTerms.addAdmin(_issuer);
    }
    ...
    function _mintOwnershipTokens(ParadimeTokens.Shares[] memory _shares) private {
        for (uint256 i = 0; i < _shares.length; i++) {
            ...
            stakeHolders.addOwner(_shares[i].entity);
        }
    }
    ...
    function _mintResidualRoyaltyTokens(ParadimeTokens.Shares[] memory _shares) private {
        for (uint256 i = 0; i < _shares.length; i++) {
            ...
            stakeHolders.addResidualOwner(_shares[i].entity);
        }
    }
    ...
    function mint(uint256 _tokenId ,uint256 _amount) public {
        require(_tokenId != OWNERSHIP_STAKE_TOKEN, "Ownership stake tokens can not be minted after an artist agreement is issued");
        require(_tokenId != RESIDUAL_STAKE_TOKEN, "Ownership stake tokens can not be minted after an artist agreement is issued");
        _mint(
            msg.sender,
            _tokenId,
            _amount,
    ...
    function addTokenAdmin(address _admin) public {
        require(isTokenAdmin(), "Only token admins can perform this action");
        usageTerms.addAdmin(_admin);
    }

    function removeTokenAdmin(address _admin) public {
        require(isTokenAdmin(), "Only token admins can perform this action");
        usageTerms.removeAdmin(_admin);
    }
    ...
    function canCover() public view returns (bool) {
        return usageTerms.permissions[TokenConsumptionTerms.COVER_ACTION];
    }    
    ...
    function getCoverRoyalty() public view returns (uint) {
        return usageTerms.royalties[TokenConsumptionTerms.COVER_ACTION];
    }  
    ...
    function safeTransferFrom(
        ...
        if (isRemoveOwner) {
            if (id == OWNERSHIP_STAKE_TOKEN) {
                stakeHolders.removeOwner(from);
            }   
            if (id == RESIDUAL_STAKE_TOKEN) {
                stakeHolders.removeResidualOwner(from);
            }
        }
    }
    ...
    function depositToOwners(
        ...
        IERC20(_paymentToken).transferFrom(_from, address(this), _amount);

        for(uint256 i = 0; i <= stakeHolders.owners.length; i++) {
            currentUserBalance = balanceOf(stakeHolders.owners[i], OWNERSHIP_STAKE_TOKEN);
            if (currentUserBalance > 0) {
                userPaymentAmount = (_amount * currentUserBalance / 100);
                erc20Balances[_paymentToken][stakeHolders.residualOwners[i]] += userPaymentAmount;
                 //TODO: STORE BALANCE IF PAYMENT NOT SUCCESSFUL.
                emit PaymentIssued(
                    paymentTransactionIndex.current(),
                    OWNERSHIP_STAKE_TOKEN,
    ...
    function depositToResidualOwners(
        address _paymentToken,
        address _from,
        uint256 _amount
    ) public {
    ...
    function getDepositBalance(address _paymentToken) public view returns (uint256) {
        return erc20Balances[_paymentToken][msg.sender];
    }

    function withdrawERC20Balance(address _paymentToken) public {
        require(erc20Balances[_paymentToken][msg.sender] > 0, "No balance for this user");
        uint256 currentBalance = erc20Balances[_paymentToken][msg.sender];
        erc20Balances[_paymentToken][msg.sender] = 0;
        IERC20(_paymentToken).transferFrom(address(this), msg.sender, currentBalance);
    }

    modifier onlyIssuer() {
        require(msg.sender == issuedBy, "Only the issuing admin can perform this action");
        _;
    }

    function issueUtilityToken(
        uint256 availableSupply,
        uint256 totalMinted,
        uint256 price,
        uint256 launchDate
...
library ParadimeTokens {
    ...
    // modifier isValidCreatorStakes(ParadimeTokens.Shares[] memory _owners,
    //     ParadimeTokens.Shares[] memory _residualOwners) external {
    //     //TODO: Make sure stakes = 100 || 1000 || 10000
    ...
    function mintAmount(TokenDefinition storage self, uint256 amount) external {
        ...
        //self.listing.supply -= amount;
    }

    function isBalancedOwnership(Shares[] memory _shares) external pure returns (bool) {
        uint16 counter = 0;
        for(uint16 i = 0; i <= _shares.length; i++) {
            counter += _shares[i].allocation;
        }

        return counter == uint16(100);
    }
}
...
library BundleDefinition {
    ...
    function getSize(Bundle storage self) external view returns (uint256) {
...
library TokenConsumptionTerms {
    ...
    bytes32 internal constant COVER_ACTION = keccak256("COVER");
    //Permissions
    ...
    */
    ...
    function isStandardPermission(bytes32 _permission) external pure returns (bool) {
    ...
    function hasPermission(PermissionsAndRoyalties storage self, bytes32 _permission) external view returns (bool) {
    ...
    function getActionFee(PermissionsAndRoyalties storage self, bytes32 permission) external view returns (uint256) {
