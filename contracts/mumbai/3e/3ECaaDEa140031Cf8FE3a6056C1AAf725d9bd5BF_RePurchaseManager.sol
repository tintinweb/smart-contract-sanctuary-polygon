...
contract RePurchaseManager is PurchaseManager, IERC721Receiver {
    ...
    function purchaseFtnftCb(
        ...
        if (
            ...
            IERC20(paymentToken).safeTransfer(treasury, amountToPay);
...
abstract contract PurchaseManager is AddressAccessor {
    function _validatePurchase(uint256 amount) internal view {
        ...
        (uint256 stable, uint256 rwa, , , ) = ITreasury(treasury)
            .getTreasuryValue(false);
        uint256 threshold = uint256(ITreasury(treasury).purchaseThreshold());
        uint256 stableNotToGoBelow = ((stable + rwa) * threshold) / 100;
        uint256 ableToSpend = stable < stableNotToGoBelow
            ? 0
            : stable - stableNotToGoBelow;

        require(ableToSpend >= amount, "Stable req not met");
    }
}
