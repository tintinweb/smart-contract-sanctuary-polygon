...
interface IFluencr {
    /**
     *
     * @notice *** IMPORTANT *** If the NFT contract for the product is ERC721 set tokenId = 0, if it is ERC1155 set id equal to that token id
     * @param id - correlationId for the product
     * @param url - url for the associated token
     * @param price - price for the associated token
     * @param contractAddress - contract address for the actual NFT contract. Can be the same as seller contract but this allows for different sale and NFT contract
     * @param tokenId - id for associated ERC1155 token. Leave as 0 if ERC721 contract. Used for balance checks.
     */
    struct Product {
        uint256 id;
        string url;
    ...
    function listProducts() external view returns (Product[] memory products);
}
...
contract FluencrSale is OwnableUpgradeable, PausableUpgradeable, Fluencr {
    ...
    event LimitSet(address contractAddress, uint256 tokenId, uint256 limit);
    ...
    mapping(address => mapping(uint256 => uint256)) public saleLimits;
    ...
    function createProduct(
        ...
        Product memory product = Product(totalProducts, _url, _price, _contractAddress, _tokenId);
        products[totalProducts] = product;
        saleAmounts[totalProducts] = _amount;
     ...
     */
    function setSaleLimits(address _contractAddress, uint256 _tokenId, uint256 _limit) external onlyOwner {
        saleLimits[_contractAddress][_tokenId] = _limit;

        emit LimitSet(_contractAddress, _tokenId, _limit);
    }
    ...
    /// @inheritdoc IFluencr
    function getProduct(uint256 id) external view override(IFluencr) returns (Product memory) {
    ...
    function _sell(address buyer, uint256 id) internal override(Fluencr) whenNotPaused {
        ...
        Product memory product = products[id];
        if (saleLimits[product.contractAddress][product.tokenId] < sales[product.contractAddress][product.tokenId] + saleAmounts[id]) {
            revert SaleLimitExceeded();
        }

        sales[product.contractAddress][product.tokenId] += saleAmounts[id];
        ITrustedMintable(product.contractAddress).trustedMint(buyer, product.tokenId, saleAmounts[id]);

        emit ProductSold(id, buyer, product.contractAddress, product.tokenId, saleAmounts[id]);
    }

    /// @inheritdoc IFluencr
    function listProducts() external view override(IFluencr) returns (Product[] memory) {
