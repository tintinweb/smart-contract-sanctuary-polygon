...
import "../interfaces/IGeosOracleK.sol";
import "../interfaces/IGeos20K.sol";

import "../libraries/GeoSpecial.sol";
import "../libraries/Nums.sol";
...
contract GeoPoly is ERC1155, ReentrancyGuard, Ownable, Roles {
    ...
    address gToken = 0x0D54568be2871879d4f300Dd7255dc6a19a21455;

    // an address for geosOracle
    address gOracle = 0xb850e98b0afc39B0dad74511c1DA9c8DD8Cb916c;
    ...
    function getCategory(string memory _categoryName)
        public
        view
        returns (uint256 categoryID)
    {
        ...
        for (uint256 i = 0; i < catIdx.current(); i++) {
            if (
                GeoSpecial.compareStrings(
                    _categoryName,
                    _avalibleCategory[i]._categoryName
                )
            ) {
                categoryID = i;
                break;
    ...
    function getCategoryName(uint256 _categoryID)
        external
        view
    ...
    function getAvaliableTiers(uint256 _categoryID)
        public
        view
    ...
    function getTierPrices(uint256 _categoryID)
        public
        view
        returns (uint256[] memory)
    {
    ...
    function getTokenVars(uint256 _tokenID)
        public
        view
        returns (NFTVars memory _out)
    {
    ...
    function getMintingString(uint256 _tokenID)
        external
        view
        returns (string memory)
    {
    ...
    function getWalletNFTs(address _wallet)
        external
        view
        returns (uint256[] memory)
    {
    ...
    function getNFTVars(string memory _mintString)
        public
        view
        returns (NFTVars memory _out)
    {
    ...
    function deserilazeNFTProps(string memory _mintString)
        public
        pure
        returns (NFTVars memory _out)
    {
    ...
    function oracleUSDTcalc(uint256 _humanUSDT) public view returns (uint256) {
        return ((_humanUSDT * (10**18)) /
            GeosOracle(gOracle).getExchangeRate("WMATIC"));
    }
    ...
    function calcUpgradeCost(
        ...
        require(
            _nextTier <= category._avaliableTiers[_avalLen - 1],
        ...
        for (uint256 i = 0; i < _avalLen; i++) {
            if (
                category._avaliableTiers[i] <= _nextTier &&
                category._avaliableTiers[i] > _currentTier
            ) {
    ...
    function upgradeNFT(uint256 _tokenID, uint256 _newTier)
        external
        nonReentrant
    {
        ...
        require(
            ...
            "Need To be the owner to upgrade the NFT"
        );
        NFTVars memory _nft = getTokenVars(_tokenID);
        checkGeosPayment(
            msg.sender,
            calcUpgradeCost(_nft._categoryID, _nft._currentTier, _newTier)
        );
    ...
    function geoMint(string memory _props, address _to)
        external
        payable
        nonReentrant
    {
        require(
            allowNFTSales || (privateSaleLive && isWhitelisted(msg.sender)),
            "NFT sales are currently not allowed"
        );
        NFTVars memory _nft = getNFTVars(_props);
        if (msg.sender == owner()) {
            require(
                oMints.current() < ownerNFTCap,
                "GeoPoly: Owner cannot mint anymore"
            );
            oMints.increment();
        } else {
            require(
                checkPayment(msg.sender, uint256(msg.value), _nft._price),
                "Payment is required to mint an NFT"
            );
        }
    ...
    // Reserve an NFT
    function reserveNFT(string memory _props, address _to)
        external
        payable
        nonReentrant
    {
        ...
        require(
            _reservedNFTs.length <= reservedCap,
            "Maximum limit for reservations, try later"
        );
        require(
            checkPayment(
                msg.sender,
                uint256(msg.value),
                deserilazeNFTProps(_props)._price
            ),
            "Payment is required to reserve an NFT"
        );
        require(
            _resNFTs[_props] == address(0),
            "This NFT has already been reserved"
        );
    ...
    function checkPayment(
        address _sender,
        uint256 _senderMsgVal,
        uint256 _priceUSDT
    ) internal returns (bool) {
        if (_senderMsgVal >= oracleUSDTcalc(_priceUSDT)) {
            return true;
        } else if (
            GEOS20(USDTToken).allowance(_sender, address(this)) >= _priceUSDT
        ) {
            if (
                GEOS20(USDTToken).transferFrom(
                    _sender,
                    address(this),
                    _priceUSDT
                )
    ...
    function checkGeosPayment(address _sender, uint256 _amount)
        internal
        returns (bool)
    {
        require(
            GEOS20(gToken).allowance(_sender, address(this)) >= _amount,
            "You need to allow us to spend your GEO$ for upgrades"
        );
        require(
            GEOS20(gToken).transferFrom(_sender, address(this), _amount),
            "You have to pay to upgrade your NFT"
        );
        return true;
    }
    ...
    function getReservedNFTs()
        external
        view
        isAdmin
        returns (string[] memory, address[] memory)
    {
        address[] memory _reservedAddresses = new address[](
            _reservedNFTs.length
        );
    ...
    function addCategory(
        string memory _categoryNew,
        uint256[] memory _tiersUpgrades,
        uint256[] memory _tiersAva
        ...
        _avalibleCategory[_categoryID] = Category(
            _tiersUpgrades,
            _tiersAva,
            _categoryNew
        );
    ...
    // change the season
    function changeSeason(uint256 _seasonNum, uint256 _nftsPer)
        external
        isAdmin
    {
    ...
    function resetReservedNFTs(
        string[] memory _nfts,
        address[] memory _reservations
    ) external isAdmin {
        require(
            _nfts.length == _reservations.length,
            "NFTs and RESERVATIONs needs to be the same length"
        );
        _reservedNFTs = _nfts;
    ...
    function withdraw(uint256 amount) external onlyOwner {
        require(
            ...
            "Not enough balance to withdraw this much"
        );
    ...
    function withdrawTokens(address tokenAddr) external onlyOwner {
        require(
            GEOS20(tokenAddr).transfer(
                msg.sender,
                GEOS20(tokenAddr).balanceOf(address(this))
            )
        );
    }
    ...
    function withdrawAll() external onlyOwner {
        ...
        require(
            GEOS20(gToken).transfer(
                msg.sender,
                GEOS20(gToken).balanceOf(address(this))
            )
        );
        require(
            GEOS20(USDTToken).transfer(
                msg.sender,
                GEOS20(USDTToken).balanceOf(address(this))
            )
        );
    }
}
...
interface GeosOracle {
    function getExchangeRate(string calldata tokenName)
        external
        view
        returns (uint256);
}
