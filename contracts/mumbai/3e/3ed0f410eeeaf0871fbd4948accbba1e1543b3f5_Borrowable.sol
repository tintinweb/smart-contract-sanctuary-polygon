...
contract SharkERC20 {
    ...
    function _transfer(
        ...
        balanceOf[from] = balanceOf[from].sub(value, "Shark: TRANSFER_TOO_HIGH");
        balanceOf[to] = balanceOf[to].add(value);
    ...
    function _checkSignature(
        ...
        require(deadline >= block.timestamp, "Shark: EXPIRED");
...
contract PoolToken is IPoolToken, SharkERC20 {
    ...
    function _setFactory() external {
        require(factory == address(0), "Shark: FACTORY_ALREADY_SET");
        factory = msg.sender;
    }
        ...
        returns (uint256 mintTokens)
    {
        ...
        require(mintTokens > 0, "Shark: MINT_AMOUNT_ZERO");
        ...
        returns (uint256 redeemAmount)
    {
        ...
        require(redeemAmount > 0, "Shark: REDEEM_AMOUNT_ZERO");
        require(redeemAmount <= totalBalance, "Shark: INSUFFICIENT_CASH");
    ...
    modifier nonReentrant() {
        require(_notEntered, "Shark: REENTERED");
        _notEntered = false;
        _;
        _notEntered = true;
    }
...
contract BAllowance is PoolToken, BStorage {
    ...
    function _checkBorrowAllowance(
        ...
        if (spender != owner && _borrowAllowance != uint256(-1)) {
            require(_borrowAllowance >= value, "Shark: BORROW_NOT_ALLOWED");
...
interface IFactory {
    ...
    function SharkPriceOracle() external view returns (address);
...
contract BSetter is PoolToken, BStorage {
    ...
    function _initialize(
        ...
        require(msg.sender == factory, "Shark: UNAUTHORIZED"); // sufficient check
    ...
    function _checkSetting(
        ...
        _checkAdmin();
        require(parameter >= min, "Shark: INVALID_SETTING");
        require(parameter <= max, "Shark: INVALID_SETTING");
    }

    function _checkAdmin() internal view {
        require(msg.sender == IFactory(factory).admin(), "Shark: UNAUTHORIZED");
    }
}
...
// File contracts/interfaces/ISharkCallee.sol

pragma solidity >=0.5.0;

interface ISharkCallee {
    function SharkBorrow(
        address sender,
        address borrower,
        uint256 borrowAmount,
        bytes calldata data
