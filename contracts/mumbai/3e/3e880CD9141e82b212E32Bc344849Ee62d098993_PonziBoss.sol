    ...
    ERC721HolderUpgradeable
{
    ...
    function getPrice(uint256 tokenId) public view returns (uint256) {
        uint256 timeElapsed = getBlocksRemaining(tokenId);
        if (stored.duration - timeElapsed <= 0) {
            return 0;
        }
        uint256 price = (stored.auctions[tokenId].startPrice * (stored.duration - timeElapsed)) / stored.duration;
    ...
    function getBlocksRemaining(uint256 tokenId) public view returns (uint256) {
        uint256 timeElapsed = uint32(block.timestamp) - stored.auctions[tokenId].startBlock;
        return stored.duration - timeElapsed;
    }
    ...
    function resolveMatch(uint256 challengerId, uint256 player2Id) public {
        Player memory challenger = stored.players[challengerId];
        Player memory player2 = stored.players[player2Id];
        ...
        uint256 level1 = calculateLevel(challenger.score);
        uint256 level2 = calculateLevel(player2.score);

        // Calculate the difference in levels
        uint256 levelDifference = level1 - level2;
        ...
        if (roll < probability1) { // Challenger wins!
            _payWinner(player2Id, challengerId, 1 + (player2.score / 2));
            _checkKill(player2Id);
            _checkBoss(player2Id, challengerId);
        } else { // Player2 Wins!
            _payWinner(challengerId, player2Id, 1 + (challenger.score / 4));
            _checkKill(challengerId);
            _checkBoss(challengerId, player2Id);
        }
        // Mark both players as having played in the current block
        stored.players[challengerId].block = block.number;
        stored.players[player2Id].block = block.number;
    }

    // Calculates a player's level based on their score
    function calculateLevel(uint256 score) public view returns (uint256) {
        // Calculate the percentage of the total game score that the player's score represents
        uint256 percentage = (score * 100) / stored.points * 10000;
