...
contract DrainedPlanets is ERC721A, Ownable, ReentrancyGuard {
    ...
    string public constant NAME = "Drained Planets";
    ...
    uint256 public constant ROYALTY_PERCENTAGE = 75; // 7.5%
    ...
    function mint() public payable {
        ...
        uint256 tokensToMint = msg.value / PRICE + _freeTokens;
        require(
            tokensToMint <= MAX_TOKENS_PER_PURCHASE,
            "Max tokens per purchase exceeded"
        );
        
        require(
            _mintedCount[msg.sender] + tokensToMint <= MAX_TOKENS_PER_PURCHASE,
            "Max tokens minted"
        );
        require(
            totalSupply() + tokensToMint <= MAX_TOKENS,
            "Max tokens supply"
        );
        if (isWhiteListSale) {
            require(_whitelist[msg.sender], "You are not in whitelist");
            _freeMinted[msg.sender] = true;
            _mintedCount[msg.sender] += tokensToMint;
    ...
    {
        royaltyAmount = (value * ROYALTY_PERCENTAGE) / 1000;
        receiver = owner();
    }
