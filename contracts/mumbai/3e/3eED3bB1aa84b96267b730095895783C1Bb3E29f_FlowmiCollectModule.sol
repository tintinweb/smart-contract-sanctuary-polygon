 ...
 */
contract FlowmiCollectModule is
    VRFConsumerBaseV2,
    FeeModuleBase,
    ICollectModule,
    FollowValidationModuleBase
{
    ...
    mapping(address => mapping(uint256 => address payable)) private s_profileToCollectors; // mapping of profile to index to collector address
    mapping(address => uint256) private s_profileToCollectorsCount; // mapping to know the amount of collectors an account has
    ...
    mapping(address => uint256) private s_profileToLastWinnerPrize; // mapping of profile to index to collector address
    mapping(address => address) private s_profileToLastWinnerAddress; // mapping of profile to index to collector address
    ...
    function _processCollect(
        ...
        profileid = payable(_dataByPublicationByProfile[profileId][pubId].recipient);

        s_index = s_profileToCollectorsCount[profileid]; // Reads previous amount of flowmiCollectors
        ...
        s_profileToCollectors[profileid][s_index] = payable(collector); // Stores address as collector of profile
        s_index++;
        s_profileToCollectorsCount[profileid] = s_index; // Updates amount of flowmiCollectors

        iWmatic.safeTransferFrom(collector, address(this), requested);
        ...
        if (s_index % i_goal == 0 && s_profileToCollectorsCount[profileid] != 0) {
        ...
        override
    {
        ...
        s_indexOfWinner = s_profileToCollectorsCount[profileid] - s_indexOfWinner - 1;
        s_recentWinner = (s_profileToCollectors[profileid][s_indexOfWinner]);
     ...
     *  @param _winner is the address given by the mapping of collectors in the index given by the VRF
     */
    ...
    /** @notice Retrieve the goal of collectors when the raffle is activated
    ...
    /** @notice Gets the number of collectors a profile has
     * @param _profileid is the profile
     * @return s_profileToCollectorsCount of the profile
     */

    function getNumberOfCollectors(address _profileid) public view returns (uint256) {
        return s_profileToCollectorsCount[_profileid];
    }

    /** @notice Gets the address of a collector by index of flowmi collect
     * @param _profileid is the profile requested
     * @param _index is the index given to the collector when started flowmi collecting
     * @return s_profileToCollectors in the profileid location
     */
    function getCollectorOfIndex(address _profileid, uint256 _index) public view returns (address) {
        return s_profileToCollectors[_profileid][_index];
    }

    /** @notice Gets number of collectors in the current raffle
     * @param _profileid is the profile requested
     * @return s_profileToCollectorCount[_profileid] % i_goal, total amount of collectors related to the profile
     * "modulo" the goal of the raffle, so it only counts whats haven't been raffled yet
     */
    function getCollectsToRaffle(address _profileid) public view returns (uint256) {
        return s_profileToCollectorsCount[_profileid] % i_goal;
    }
