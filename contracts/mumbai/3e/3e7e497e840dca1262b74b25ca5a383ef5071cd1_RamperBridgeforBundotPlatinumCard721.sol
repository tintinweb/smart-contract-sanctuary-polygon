...
//SilverCard
abstract contract BundotMembershipPlatinumCard is ERC721Burnable, Ownable, ReentrancyGuarded{
...
contract RamperBridgeforBundotPlatinumCard721 is IRamperBridgeInterface721{

BundotMembershipPlatinumCard public platinumcardaddress;

constructor(BundotMembershipPlatinumCard _existingcontractaddress){
    platinumcardaddress = _existingcontractaddress;
}

 function availableTokens(address) override external pure returns (uint256 quantity) {
...
function price() external view returns (uint256)
{
    return platinumcardaddress.getPrice();
}

function mint(address _userWallet, uint256 _quantity) override external payable {
        require(_quantity == 1);
        platinumcardaddress.mintToken{value:msg.value}(_userWallet);
    }

/// @notice Helper Transfer function to allow multiple tokens to be transferred at once
    function safeBulkTransferFrom(address _from, address _to, uint256[] memory _tokenIds) external{
        for(uint256 i;i < _tokenIds.length; i++)
               platinumcardaddress.safeTransferFrom(_from,_to,_tokenIds[i]);
    }    
