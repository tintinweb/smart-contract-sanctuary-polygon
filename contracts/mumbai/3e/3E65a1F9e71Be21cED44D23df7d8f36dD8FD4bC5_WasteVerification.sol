...
contract WasteVerification is Ownable {
    ...
    */
    ...
    mapping(string => TripInfo) public tripInfo;
    ...
    function upsertTripData(
        ...
        TripInfo storage trip = tripInfo[_transId];
        trip.tripId = _tripId;
        ...
        emit TripData(_transId, _tripId, _houseList.length);
    }
    ...
    /// @notice Fetch all trip data based on transId
    /// @param _transId TransId Key of the dumpyard details db.
    /// @return trip TripData for the transId
    function getTripData(string memory _transId) external view returns (TripInfo memory trip) {
        trip = tripInfo[_transId];
    }

    /// @notice Function to calculate tentative waste possible for houses.
    /// @param _totalHouses Number of houses
    /// @return totalTentativeWasteUpperLimit Tentative waste possible for the number of houses (Upper Limit)
    /// @return totalTentativeWasteLowerLimit Tentative waste possible for the number of houses (Lower Limit)
    function calclulateTentativeWasteForHouses(uint256 _totalHouses)
        public
        view
        returns (uint256 totalTentativeWasteUpperLimit, uint256 totalTentativeWasteLowerLimit)
    {
        totalTentativeWasteUpperLimit =
            (_totalHouses * averageWasteCollectedPerHouse * (100 + tolerancePercentage)) /
            100;

        totalTentativeWasteLowerLimit =
            (_totalHouses * averageWasteCollectedPerHouse * (100 - tolerancePercentage)) /
            100;
    }
    ...
    function processGarbageQuantityVerificationStatus(
        ...
        (
            uint256 totalTentativeWasteUpperLimit,
            uint256 totalTentativeWasteLowerLimit
        ) = calclulateTentativeWasteForHouses(_totalHouses);
        garbageQuantityVerificationStatus = (_totalCollectedWaste <=
            totalTentativeWasteUpperLimit &&
            _totalCollectedWaste >= totalTentativeWasteLowerLimit)
