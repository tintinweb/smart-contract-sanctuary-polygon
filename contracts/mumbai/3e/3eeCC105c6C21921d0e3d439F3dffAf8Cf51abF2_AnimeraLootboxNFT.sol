...
contract AnimeraLootboxNFT is VRFConsumerBaseV2, ERC721URIStorage, Ownable {
    enum Breed {
        PUG,
        SHIBA_INU,
        ST_BERNARD
    ...
    function adminMint(address _to) public onlyOwner {
        _mint(_to, 1);
    }

    function requestRandom() internal {
      for (uint index = 0; index < lootboxWhitelistedUsers.length; index ++) {
        ...
        requestIdToSender[s_requestId] = lootboxWhitelistedUsers[index];
        emit requestConfirmationEvent(lootboxWhitelistedUsers[index], s_requestId);
      }
    }
    ...
    function setWhitelist(address[] memory _whitelist) public onlyOwner {
      lootboxWhitelistedUsers = _whitelist;
    }
}
