...
import {AdvancedConfig, Data, BaseConfig, DropuStorage} from "../libraries/DropuStorage.sol";
import {IERC721} from "@openzeppelin/contracts/token/ERC721/IERC721.sol";

contract DropuERC721AExtensionC is DropuERC721AUpgradeable {
    ...
    function setTokenURIs(
        ...
        require(!DropuStorage.state().advCfg.metadataFrozen, "NOT_ACTIVE");
        uint256 tokenIdsLength = _tokenIds.length;
        require(tokenIdsLength == _newURIs.length);
        for (uint256 i = 0; i < tokenIdsLength; i++) {
            DropuStorage.state().data.tokenURIs[_tokenIds[i]] = _newURIs[i];
        }
    }

    function baseTokenURI() external view returns (string memory) {
        return DropuStorage.state().cfg.uriBase;
    }
    ...
    function _publicPriceInWei() internal view returns (uint256) {
        return uint256(DropuStorage.state().cfg.publicPrice) * 10 ** 13;
    }
    ...
    function creditCardMint(
        ...
        if (state.cfg.dropuFeeActive) {
            ...
            require(
                msg.value == publicPrice * _numTokens + dropuFee,
                "INVALID_PRICE_PAID"
            );
    ...
    function setSoulbindingState(bool _soulbindingActive) external onlyOwner {
        DropuStorage.state().cfg.soulbindingActive = _soulbindingActive;
    }
    ...
    function setStakingState(bool _stakingState) external onlyOwner {
        DropuStorage.state().advCfg.stakingActive = _stakingState;
    }
    ...
    function adminUnstake(uint256 _tokenId) external onlyOwner {
        Data storage data = DropuStorage.state().data;
        require(DropuStorage.state().data.currentTimeStaked[_tokenId] != 0);
    ...
    function currentTokenStakeTime(
        ...
        uint256 currentStakeStartTime = DropuStorage
        .state()
        .data
    ...
    function updateMintsPerFreeClaim(
        ...
        DropuStorage.state().advCfg.mintsPerFreeClaim = _mintsPerFreeClaim;
    }
    ...
    function freeClaim(
        ...
        if (state.cfg.dropuFeeActive) {
            uint256 dropuFee = totalMints * dropuFeePerToken();
