...
contract BSBBattleUpgradeable is Initializable, BSBBaseGameModeUpgradeable, VRFConsumerBaseUpgradeable {
    ...
    function stackInSquad(CollectionItems calldata collectionItems_, uint leaderId_) external {
        require(collectionItems_.collection == contracts["gen1"], "BSB400");

        _checkIfOwnsItems(collectionItems_, _msgSender());
    ...
    function unStackFromSquad(CollectionItems calldata collectionItems_) external {
        _checkIfOwnsItems(collectionItems_, _msgSender());

        bool applyBuff_ = BSBMintPassBuffCaller.getBuffBalance(_msgSender()) != 0;
        uint accumulatedYield_ = _claimRewards(collectionItems_, applyBuff_);

        for (uint i = 0; i < collectionItems_.assets.length; ++i) {
            _unstackToken(collectionItems_.collection, collectionItems_.assets[i]);
        }
    ...
    function _removeFromElitesAndUnstackIfNecessary(CollectionItems calldata collectionItem_) internal {
        if (collectionItem_.collection == contracts["gen0"] || collectionItem_.collection == contracts["gen1"]) {
            ...
            for (uint i = 0; i < collectionItem_.assets.length; ++i) {
                require(getLeaderSquadSize(collectionItem_.assets[i]) == 0, "BSB450");
    ...
    function _stackToken(address collection_, uint tokenId_, uint leaderId_) internal {
        require(!tokenSquadAppartenance[collection_][tokenId_], "BSB400");
    ...
    function claimRewards(CollectionItems[] calldata collectionItems_) public returns (uint) {
    ...
    function _claimRewards(CollectionItems calldata collectionItems_, bool applyBuff_) internal returns (uint) {
        ...
        for (uint i = 0; i < collectionItems_.assets.length; ++i) {
            ...
            collectionAccumulatedRewards_ += _applyRisk(
                collectionItems_.collection,
                collectionItems_.assets[i],
    ...
    function _getLevelImpact(uint level_) public view returns (ImpactType memory) {
    ...
    function _getBloodShardWinChancePercentage(
        ...
        _getLevelImpact(level_).bloodShardBoost +
    ...
    function calculateReward(address collection_, uint tokenId_) public view returns (uint, uint) {
        require(EnumerableSetUpgradeable.contains(allowedStakeCollections, collection_), "BSB400");
    ...
    function _calculateYieldReward(
        ...
        uint baseYield = daysLifetime_ * yield_ / yieldDaysRate;
