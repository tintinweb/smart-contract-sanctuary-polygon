...
contract TimeForDay {
    ...
    function getYearMonthDay(uint256 _timestamp, uint256 _adjustHour) internal pure returns (uint256) {
        _DateTime memory dt = parseTimestamp(_timestamp + _adjustHour * 8);
...
contract T222 is Ownable, TimeForDay, SortList {
    ...
    */
    ...
    // 领取邀请奖励
    function claimInvitationReward() external {
        // 获取用户地址
        address _user = msg.sender;
        // 更新所有奖励
        updateAllRewards(_user);
        // 获取待领取的邀请奖励
        uint256 _pendingReward = userInfoList[_user]._updatedRewardItem._updatedInvitationReward;
        // 获取用户剩余的授信额度
        uint256 _userRemainingCredit = getUserRemainingCredit(_user);
        // 如果待领取奖励大于用户剩余授信额度，则将待领取奖励设为用户剩余授信额度
        if (_pendingReward > _userRemainingCredit) {
        ...
        // 转移奖励给用户
        transferReward(_user, _pendingReward, "claimInvitationReward");
        // 更新用户已领取的邀请奖励
        userInfoList[_user]._claimedRewardItem._claimedInvitationReward += _pendingReward;
        // 更新用户待领取的邀请奖励
        userInfoList[_user]._updatedRewardItem._updatedInvitationReward -= _pendingReward;
    }
     ...
     * @param _user 用户地址
     */
    function updateInvitationRewad(address _user) private {
        // 获取用户的邀请奖励
        uint256 _totalReward = userInfoList[_user]._invitationReward;
        // 将用户的邀请奖励添加到更新后的奖励项中
        userInfoList[_user]._updatedRewardItem._updatedInvitationReward += _totalReward;
        // 将用户的邀请奖励清零
        userInfoList[_user]._invitationReward = 0;
    }
    ...
    function getInvitationReward(address _user) internal view returns (uint256) {
        // 获取用户的推广收益
        return userInfoList[_user]._invitationReward;
    }
    ...
    function getAllInvitationReward(address _user) public view returns (uint256) {
        // 获取用户的推广收益
        uint256 _totalReward = getInvitationReward(_user);
        // 返回用户的推广收益加上已更新的静态收益
    ...
    function distributeInvitationReward(uint256 _amount, address _user) private {
        ...
        for (uint256 i = 0; i < _allReferrersNum; i++) {
            ...
            // 更新当前推荐人的邀请奖励和网体投资额
            userInfoList[_allReferrers[i]]._invitationReward += _rewardAmount;
