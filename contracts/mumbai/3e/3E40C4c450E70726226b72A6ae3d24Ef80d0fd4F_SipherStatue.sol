...
contract SipherStatue is ERC1155, Pausable, Ownable {
    ...
    //  admin lock an user and emit the event
    event lockUser(address user);
    //  admin unlock an user and emit the event
     ...
     */
    constructor(string memory baseSipherURI) public ERC1155(baseSipherURI) {
        admin = msg.sender;
    }
    
    /**
     * @dev modify function to restrict locked user (locked by admin)
     */
    ...
    /**
     * @dev lock user, the user after being locked by admin, cannot do anything with their assets.
     * Requirements:
     * - Only admin
     * - admin cannot lock his own address.
     */
    function adminLockUser(address user) external onlyOwner{
        require(admin != user, "Hello Admin, you lock your own address!");
        isBlacklist[user] = true;
        emit lockUser(user);
    }

    /**
     * @dev unlock user, the user after being unlocked by admin, they can free to transfer or claim with their assets
     * Requirements:
     * - Only admin
     */
