...
contract JustGems is Ownable, AccessControl, ERC721 {
  ...
  bytes32 internal constant _CONSUMER_ROLE = keccak256("CONSUMER_ROLE");

  // ============ Storage ============

  //the last token mintable
  ...
  /**
   * @dev Returns all the tokens that are mintable
   */
  function mintable() external view returns(Token[] memory) {
    Token[] memory tokens = new Token[](totalMintable());
    uint256 index;
    do {
      if (mintable(++index)) {
    ...
    } while(index < _lastTokenId);
    return tokens;
  }

  /**
   * @dev Returns true if tokenId is mintable
   */
  function mintable(uint256 tokenId) public view returns(bool) {
    return !_exists(tokenId) && tokenId <= _lastTokenId;
  }

  /**
   * @dev Returns all the tokens that are mintable
   */
  function mintable(IERC20 token) external view returns(Token[] memory) {
    Token[] memory tokens = new Token[](totalMintable());
    uint256 index;
    do {
      if (mintable(++index)) {
  ...
  /**
   * @dev Returns the last mintable token id
   */
  ...
  /**
   * @dev Returns the total amount of tokens that are mintable
   */
  function totalMintable() public view returns(uint256) {
    return _lastTokenId - _totalMinted;
  }
  ...
  function tokenURI(
    ...
    return bytes(_uri[tokenId]).length > 0 
      ? _uri[tokenId]
  ...
  function burn(uint256 tokenId) external {
    ...
    require(
      //caller should have consumer role
      hasRole(_CONSUMER_ROLE, caller)
      //or caller is owner or approved
      || _isApprovedOrOwner(caller, tokenId), 
  ...
  function mint(uint256 tokenId, address recipient) external payable {
    require(
      _price[tokenId] > 0 && msg.value >= _price[tokenId],
      "ERC721Mintable: Invalid price amount"
    );
  ...
  function mint(
    ...
    require(
      _erc20Price[tokenId][token] > 0,
      "ERC721Mintable: No price set"
    );
  ...
  /**
   * @dev Sets the `uri` and `price` for `tokenId`
   */
  function mintable(
    string memory uri,
    uint256 price
  ) external onlyRole(_CURATOR_ROLE) {
    _uri[++_lastTokenId] = uri;
    _price[_lastTokenId] = price;
  }

  /**
   * @dev Sets the `uri` and `token` `price` for `tokenId`
   */
  function mintable(
    string memory uri,
    IERC20 token,
    ...
    _uri[++_lastTokenId] = uri;
    _erc20Price[_lastTokenId][token] = price;
  }
