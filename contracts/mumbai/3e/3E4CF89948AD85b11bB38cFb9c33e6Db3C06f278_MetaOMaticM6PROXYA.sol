...
contract MetaOMaticM6PROXYA is  IPOLYCONTRACTM6PROXYA {
   ...
   //Get Level Downline With No of Id & Investments
    function generate_report(uint package,address user,uint cycle) view public returns(uint currentrecycle,address parent,address left,address right,uint noofidinfirstlevel){
      if(package==0){
          return(_M615Details[user].currentrecycle,_M615Details[user].parent[cycle],_M615Details[user].left[cycle],_M615Details[user].right[cycle],_M615Details[user].noofidinfirstlevel[cycle]);
      }
      else if(package==1){
          return(_M630Details[user].currentrecycle,_M630Details[user].parent[cycle],_M630Details[user].left[cycle],_M630Details[user].right[cycle],_M630Details[user].noofidinfirstlevel[cycle]);
      }
    }
