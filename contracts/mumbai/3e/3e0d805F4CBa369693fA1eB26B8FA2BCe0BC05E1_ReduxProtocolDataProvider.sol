...
/**
 * @title ReduxProtocolDataProvider
 * @author Aave
 * @notice Peripheral contract to collect and pre-process information from the Pool.
 */
contract ReduxProtocolDataProvider is IPoolDataProvider {
  ...
  /**
    * @notice Returns interests paid daily 
    * @return nativeInterestPaid The native interest paid daily
    * @return prxyInterestPaid The prxy interest paid daily
  */
  function getInterestsPaidDaily(address asset) external view returns (uint256 nativeInterestPaid, uint256 prxyInterestPaid) {
    address supplyingAsset = asset;
    IPool POOL = IPool(ADDRESSES_PROVIDER.getPool());
    DataTypes.ReserveData memory reserve = POOL.getReserveData(supplyingAsset);

    nativeInterestPaid = reserve.nativeInterestPaid;
    prxyInterestPaid = reserve.prxyInterestPaid;
  }
   ...
   * @return prxymode2rate The prxy rate of the reserve
   * @return prxymode3rate The collateral prxy rate of the reserve
    ...
    )
  {
    ...
    prxymode2rate = reserve.prxymode2rate;
    prxymode3rate = reserve.prxymode3rate;
    ...
    )
  {
    ...
    DataTypes.ReserveData memory reserve = IPool(ADDRESSES_PROVIDER.getPool()).getReserveData(_asset);
    ...
    currentPTokenBalance = IPToken(reserve.pTokenAddress).balanceOf(_user);
    for(uint256 i = 0; i < reserveList.length; ++i) {
      currentBorrowBalances[i] = IPToken(reserve.pTokenAddress).borrowBalanceOf(_user, reserveList[i]);
    }

    currentPrxyBalance = IPToken(reserve.pTokenAddress).prxyBalanceOf(_user);
    currentCollateralBalance = IPToken(reserve.pTokenAddress).collateralBalanceOf(_user);
    currentUncollateralBalance = currentPTokenBalance - currentCollateralBalance;
    supplyRate = reserve.supplyRate;
    borrowRate = reserve.borrowRate;
    prxymode2rate = reserve.prxymode2rate;
    prxymode3rate = reserve.prxymode3rate;
    lastUpdateTimestamp = IPToken(reserve.pTokenAddress).lastUpdateTimestampOf(_user);
    supplyMode = IPToken(reserve.pTokenAddress).supplyMode(_user);
  }
