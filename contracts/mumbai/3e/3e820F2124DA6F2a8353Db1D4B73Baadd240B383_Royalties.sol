    ...
    ReentrancyGuardUpgradeable
{
    ...
    uint256 constant public EIGHTEEN_MONTHS = 547 days;
        ...
        returns (uint256[] memory)
    {
        ...
        for (i = 0; i < tierIds.length; i++) {
            ...
            uint256[] memory ownedTokens = _LDA_.getOwnedTokens(tierId, claimer);
            ...
            for (uint256 j = 0; j < numOwnedTokens; j++) {
                (, , uint128 tokenId) = RoyalUtil.decomposeLDA_ID(ownedTokens[j]);
        ...
        returns (uint256)
    {
        ...
        uint256[] memory ownedTokens = _LDA_.getOwnedTokens(tierId, ldaHolder);
        uint256 numOwnedTokens = ownedTokens.length;
