...
contract Oracle is Ownable {
    ...
    uint16[BONUS_LINES_COUNT] public ref_bonuses = [70, 50, 40, 30, 30, 30, 20, 20, 20, 20, 10, 10, 10, 10, 5, 5, 5, 5, 5, 5];
    ...
    function _setUpline(address _addr, address _upline, uint256 _amount) private {
        ...
        {
            if(levels[_upline][i].length < (i+1) *2 +1 )
            {   
                levels[_upline][i].push(_addr);
                currentLevel = i;
                levelsLength[_upline] = i + 1;
                currentPosition = levels[_upline][i].length;
                if(currentPosition / 2 > (i * 2 +1))
                {
        ...
        if(currentLevel != 0)
        {
            for(uint256 j = currentLevel - 1; j >= 0 ; j--)
            {
