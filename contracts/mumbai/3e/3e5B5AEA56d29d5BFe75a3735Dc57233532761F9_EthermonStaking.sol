...
contract EthermonStaking is EthermonStakingBasic {
    ...
    function updateTokens(
        ...
        //Have to think about this it, rn its without comment
    ...
    function updateStakingData(
        ...
        require(
            keccak256(abi.encodePacked(_depositeToken._appSecret)) == appHash,
            "Application hash doesn't match"
        );
        uint256 currentTime = now;
        ...
        require(data.endTime > currentTime, "Staking time ended");
        require(
            _depositeToken._amount >= data.emons,
            "Emons can only be added."
        );
        ...
        if (_depositeToken._day > data.duration) {
            ...
            // Add remaing to endtime
            data.endTime = remainingTime + updatedTime;
            // Check if duration should be > before and also add time on top of it or just increase to the new duration
            // staked for 30 days (25 days passed) => increase to 60 days (Will we do 60 - 25 = 35 + 5(days left to stake)) OR 25(lastcalled) + 60
