...
contract Test {
    ...
    function entry(Data memory data) public view {
        add(data);
        console.logUint(data.offer.length);
    }

    function entry2(uint256 a) public view {
        add2(a);
        console.logUint(a);
    }

    function add(Data memory data) internal pure {
        OfferItem[] memory newOffer = new OfferItem[](data.offer.length + 1);

        OfferItem memory newOfferItem = OfferItem(10);

        newOffer[data.offer.length] = newOfferItem;
        data.offer = newOffer;
    }

    function add2(uint256 a) internal pure {
        a = a + 1;
    }
}
