...
function updateX3Referrer(address userAddress, address referrerAddress, uint8 level) private {
    ...
    function updateX6Referrer(address userAddress, address referrerAddress, uint8 level) private {
        ...
        address[] storage refRef = referrerMatrix.Referrals;

        if (referrerLen < 2) {
            refRef.push(userAddress);
            emit NewUserPlace(userAddress, referrerAddress, level, uint8(referrerLen + 1));
            ...
            if ((referrerLen == 1 || referrerLen == 2) && refRef[0] == referrerAddress) {
            ...
            updateX6ReferrerSecondLevel(userAddress, referrerMatrix.currentReferrer, level);
        } else {
            Matrix storage referrer = users[referrerAddress].matrices[level];
            referrer.secondLevelReferrals.push(userAddress);

            if ((referrer.Referrals[0] == referrer.Referrals[1]) && (referrer.Referrals[0] == referrer.closedPart)) {
    ...
    function updateX6(address userAddress, address referrerAddress, uint8 level, bool x2) private {
        Matrix storage ref = users[referrerAddress].matrices[level];
        address[] storage referrals = users[ref.Referrals[x2 ? 1 : 0]].matrices[level].Referrals;
        uint len = referrals.length;

        referrals.push(userAddress);
        emit NewUserPlace(userAddress, ref.Referrals[x2 ? 1 : 0], level, uint8(len + 1));
        emit NewUserPlace(userAddress, referrerAddress, level, 2 + uint8(len + 1));

        users[userAddress].matrices[level].currentReferrer = ref.Referrals[x2 ? 1 : 0];
    }
    ...
    function updateX6ReferrerSecondLevel(address userAddress, address referrerAddress, uint8 level) private {
        ...
        Matrix storage ref = users[users[referrerAddress].matrices[level].currentReferrer].matrices[level];
        address[] storage x6 = users[ref.Referrals[0]].matrices[level].Referrals;
        ...
        referrer.secondLevelReferrals.length = 0;
