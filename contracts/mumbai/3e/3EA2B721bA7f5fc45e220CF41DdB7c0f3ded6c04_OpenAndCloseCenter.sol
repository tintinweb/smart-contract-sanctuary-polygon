...
contract OpenAndCloseCenter is Ownable {
    ...
    mapping(uint256 => uint256) public nurseryMaturityTime;
    ...
    function changeNurseryName(string memory _name, uint256 _tokenId) external {
        require(
            INursery(gameAddresses.getNurseryAddress()).ownerOf(_tokenId) == msg.sender ||
            msg.sender == owner(),
            "Not authorized");
    ...
    function changeTrainingCenterName(string memory _name, uint256 _tokenId) external {
        require(
            ITraining(gameAddresses.getTrainingNFTAddress()).ownerOf(_tokenId) == msg.sender ||
            msg.sender == owner(),
            "Not authorized");
    ...
    function changeLaboratoryName(string memory _name, uint256 _tokenId) external {
        require(
            ILaboratory(gameAddresses.getLaboratoryNFTAddress()).ownerOf(_tokenId) == msg.sender ||
            msg.sender == owner(),
            "Not authorized");
    ...
    function canNurserySell(uint256 _tokenId) external view returns (bool) {
        bool result;
        if(_tokenId > 0 && _tokenId <= INursery(gameAddresses.getNurseryAddress()).getPreMintNumber()){
            result = true;
        }else if(
    ...
    function canTrain(uint256 _tokenId) external view returns (bool) {
        bool result;
        if(_tokenId > 0 && _tokenId <= ITraining(gameAddresses.getTrainingNFTAddress()).getPreMintNumber()){
            result = true;
        }else if(
    ...
    function canLaboSell(uint256 _tokenId) external view returns (bool) {
        bool result;
        if(_tokenId > 0 && _tokenId <= ILaboratory(gameAddresses.getLaboratoryNFTAddress()).getPreMintNumber()){
            result = true;
        }else if(
    ...
    function closeNursery(uint256 _tokenId) external {
        ...
        require(_tokenId > INurs.getPreMintNumber(), "This center can't be closed");
        ClosingProcess storage c = closingProcesses[1][_tokenId];
        require(
            INurs.ownerOf(_tokenId) == msg.sender,
            "Not your Nursery"
        );
    ...
    function closeTrainingCenter(uint256 _tokenId) external {
        ...
        require(_tokenId > ITrain.getPreMintNumber(), "This center can't be closed");
        ClosingProcess storage c = closingProcesses[2][_tokenId];
        require(ITraining(gameAddresses.getTrainingCenterAddress()).ownerOf(_tokenId) == msg.sender, "not your Center");
        require(!c.isClosing, "Already in closing process");

        require(ITrainingManagement(gameAddresses.getTrainingCenterAddress()).cleanSlotsBeforeClosing(_tokenId));
    ...
    function closeLabo(uint256 _tokenId) public {
        ...
        require(_tokenId > ILab.getPreMintNumber(), "This center can't be closed");
        ClosingProcess storage c = closingProcesses[3][_tokenId];
        require(
            ILab.ownerOf(_tokenId) == msg.sender,
            "Not your labo"
        );
        require(!c.isClosing, "Already in closing process");
        require(ILabManagement(gameAddresses.getLaboratoryAddress()).cleanSlotsBeforeClosing(_tokenId));
        c.isClosing = true;
        c.timestampClosedActed = block.timestamp + closingHousesDuration;
    }
