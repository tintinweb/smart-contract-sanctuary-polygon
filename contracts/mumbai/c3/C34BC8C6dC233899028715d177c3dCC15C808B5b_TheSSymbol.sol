...
contract TheSSymbol is ERC721Enumerable, Ownable {
    using Strings for uint256;

    event PublicSaleStateUpdate(bool publicSaleActive);

    string public baseURI;
    string public unrevealedURI = "ipfs://QmWp22S7uXvUVqBmDxEvHChVLgGR19tqVRsRyrbGuK5Bej";

    bool public publicSaleActive;
    bool public whitelistActive;

    uint256 public constant MAX_SUPPLY = 7000;
    uint256 public constant INDEX_OFFSET = 1;
    ...
    constructor() ERC721("TheSSymbol", "SYMBOL") {}
    ...
    function giveAway(address[] memory users) external onlyOwner {
        for (uint256 i; i < users.length; i++) _mintBatchTo(users[i], 1);
    }

    function whitelist(address[] memory users) external onlyOwner {
        for (uint256 i; i < users.length; i++) whitelisted[users[i]] = true;
    }
