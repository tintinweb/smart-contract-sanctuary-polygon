...
contract MasterWhitelist is Ownable, IMasterWhitelist {
     ...
     */
    constructor(address _passport, address _reader, bytes32[] memory _countryBlacklist) {
        KYCPassport = IQuadPassport(_passport);
        KYCReader = IQuadReader(_reader);
        for(uint i = 0; i < _countryBlacklist.length; i++) {
    ...
    function addUserToWhitelist(address _user) external onlyLawyer {
        ...
        kycProviders[_user] = keccak256("Manual");
    ...
    /**
     * @notice Adds provider for a user manually
     * @param _user is the User address
     * @param _provider is the kyc provider name hashed with keccak256
     */
    function setKYCProvider(address _user, bytes32 _provider) external onlyLawyer {
    ...
    function addUserToWhitelistUsingPassport(address _user) external payable {
        ...
        whitelistedUsers[_user] = true;
        kycProviders[_user] = keccak256("Quadrata");
        emit UserAddedToWhitelist(_user, 0);
    }
