...
contract AirdropQatar is Ownable, ReentrancyGuard {
    ...
    function claimAllAirdrop(uint256[] memory _cids, uint256[] memory _tokenIds) public nonReentrant returns(uint256[] memory _amountList , uint256 _total){ 
        address _sender = msg.sender;
        require(_cids.length == _tokenIds.length, "Airdrop: diff length");
        for (uint i = 0; i < _cids.length; i++) {
            CampaignInfo storage campaign = campaignInfo[_cids[i]];
            ClaimInfo storage _claim = claim[_cids[i]][_sender];
            IWhitelist whitelist = IWhitelist(campaign.whitelist);
            ...
            emit Claimed(_cids[i], _sender, _claim.amount);
        }
