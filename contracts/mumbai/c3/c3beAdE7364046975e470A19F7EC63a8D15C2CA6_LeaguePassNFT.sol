...
contract LeaguePassNFT is ERC721Upgradeable, OwnableUpgradeable {
    using CountersUpgradeable for CountersUpgradeable.Counter;
    using SafeERC20Upgradeable for IERC20Upgradeable;

    CountersUpgradeable.Counter private _tokenIds;

    /// @notice Token to be transfered to users after using NFT pass to join the league
    ...
    /// @notice Addresses which are joined the league 
    /// @dev True if user joined the league. Prevents users from joining twice. 
    mapping(address => bool) public leagueParticipants;
    ...
    function mint(IERC20Upgradeable _token) external {
        ...
        _token.safeTransferFrom(msg.sender, address(this), passPriceList[address(_token)]);    
        _tokenIds.increment();
        _mint(msg.sender, _tokenIds.current());
        emit NewPassCreated(_tokenIds.current(), msg.sender, passPriceList[address(_token)], address(_token));
    }
    ...
    function joinTheLeague(uint256 _nftId) external {
        ...
        require(leagueParticipants[msg.sender] == false, "You have already joined the league");
        require(balanceOf(msg.sender) >= 1, "You do not have the pass to join the league");
        leagueParticipants[msg.sender] = true;
        _burn(_nftId);
        dLeagueToken.mint(msg.sender, dLeagAmount);
        emit NewPlayerJoinedTheLeague(msg.sender, _nftId);
    }
    ...
    function setPaymentToken(address _token, uint256 _NFTprice) external onlyOwner {
        passPriceList[_token] = _NFTprice;
        emit PaymentTokenIsSet(_token, _NFTprice);
    }

    /// @notice Sets token address to be transfered to users after using NFT pass
    /// @param _token Address of token to be transfered to users after using NFT pass
    function setDleagueToken(address _token) external onlyOwner {
        dLeagueToken = IMintableToken(_token);
        emit DleagueTokenIsSet(_token);
    }

    /// @notice Sets amount of tokens to be transfered to users after using NFT pass
    /// @param _amount Amount of tokens to be transfered to users after using NFT pass
    function setDleagueTokenAmount(uint256 _amount) external onlyOwner {
        dLeagAmount = _amount;
    ...
    function setLeagueStartDate(uint256 _date) external onlyOwner{
    ...
    function setWhitelisteAddr(address _addr, bool _val) external onlyOwner{
