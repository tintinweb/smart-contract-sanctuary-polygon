...
contract ToryNFT is ItnmetaNFT, Ownable, ReentrancyGuard, Pausable, ERC721WithPermit {
    ...
    constructor() ERC721("ToryNFT","TNFT") Ownable() {
        Admin = _msgSender();
    }

    modifier onlyOwnerOrAdmin() {
        if (Admin == address(0)) revert BadRequest("Not allowed address");
        require(Admin == _msgSender() || owner() == _msgSender(), "caller is not the Owner or Admin");
        _;
    }

    function changeAdmin(address newAdmin) public onlyOwner {
        if(owner() != _msgSender()) revert NotOwnerNorAdmin("caller is not the Admin");
        Admin = newAdmin;
    }

    function mint(address _to, uint256 quantity) external onlyOwnerOrAdmin whenNotPaused {
            for (uint256 i = 0; i < quantity; i++) {
            lastTokenId++;
            if(!safeMint(_to, lastTokenId)) revert MintingFailed();
        }
    }

    function safeMint(address _to, uint256 _tokenId) private nonReentrant returns (bool) {
