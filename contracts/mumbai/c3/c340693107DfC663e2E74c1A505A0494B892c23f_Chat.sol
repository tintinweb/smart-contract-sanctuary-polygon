  ...
  //--------------------------------------------------------------------------------------   
   ...
   require(walletAddress != address(0), "User with given address does not exist.");
    ...
    function sendMessage(address receiver, string calldata subject, string memory message, string memory fileHash, string memory receiverGroup) external {
        ...
        function sendMessageToGroup(address[] memory receiver, string calldata subject, string []memory message, string memory fileHash, string memory emailGroup, address[] memory cciReceivers) external {
        require(
            checkUserExists(msg.sender) == true,
            "You must have an account"
        );
        ...
        for(uint i = 0; i<cciReceivers.length; i++){
            ...
            Message memory message = Message(messageCount, msg.sender, cciReceivers[i], subject, message[i], block.timestamp, false, fileHash, '',DeletionStatus.NotDeleted);
        messages.push(message);
        messageCount++;
        }
