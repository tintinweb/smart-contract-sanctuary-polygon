...
contract LBMultiCall is Ownable {
    ...
    function getLotteryInfos() external view returns (lotteryInfo[] memory){
            ...
            for (uint256 i = 0; i < contracts.length; i++) {
                ...
                string memory formattedJpWei = formatUnits(jpWei, contractAddress, 0);
                string memory formattedJpUsd = formatUnits(jpUsd, contractAddress, 1);
...
function formatUnits(uint256 value, address contractAddress, uint256 jptype) internal view returns (string memory) {
    uint256 decimalPlaces;
    
    if(jptype == 0){
        if (contractAddress == contractsAddresses[0]) {
            decimalPlaces = 8; // Bitcoin contract has 8 decimal places
        } else {
            decimalPlaces = 18; // Other contracts have 18 decimal places
        }
    } else{
         decimalPlaces = 18; // Other contracts have 18 decimal places
    }
    
    uint256 intValue = value / (10**decimalPlaces);
    uint256 fractionalPart = value % (10**decimalPlaces);
    
    string memory formattedValue = string(
        abi.encodePacked(
            uintToString(intValue),
            ".",
            uintToString(fractionalPart, decimalPlaces)
        )
    );
...
function uintToString(uint256 value) internal pure returns (string memory) {
    ...
    bytes memory buffer = new bytes(digitCount);
    uint256 index = digitCount;
...
function uintToString(uint256 value, uint256 decimalPlaces) internal pure returns (string memory) {
    string memory stringValue = uintToString(value);
    ...
    if (decimalPlaces > length) {
        uint256 zerosToAdd = decimalPlaces - length;
        string memory zeros = new string(zerosToAdd);
        stringValue = string(abi.encodePacked(zeros, stringValue));
    }
