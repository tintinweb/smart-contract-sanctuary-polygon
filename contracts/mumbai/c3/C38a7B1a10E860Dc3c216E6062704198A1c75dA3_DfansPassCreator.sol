...
contract DfansPassCreator is Ownable {
  ...
  constructor() {
      implementation = address(new DfansPass('DFANS','D'));
  }
  ...
  function createDfansPass(
      ...
      dp.initialize(name, symbol, collectionId, royaltyFraction, beneficiary, initialPublish);
      dp.transferOwnership(_msgSender());
      emit ContractDeployed(_msgSender(), clone);
      return clone;
  }
}
