...
// Thirdweb top-level
import "../interfaces/IFantasySportFee.sol";
    ...
    ITokenERC1155
{
    ...
    constructor(address _fantasySportFee) initializer {
    ...
    /// @dev Returns the market fee bps and recipient.
    ...
    function collectPrice(MintRequest memory _req) internal {
        ...
        uint256 marketFees = (totalPrice * marketFeeBps) / MAX_BPS;
        (address upfrontFeeRecipient, uint256 upfrontBps, , ) = fantasySportFee
            .getFeeInfo(address(this), FeeType.MARKET_SALE);

        uint256 upfrontSalesCut = (marketFees * upfrontBps) / MAX_BPS;
    ...
    IRoyalty
{
    ...
    /// @dev Emitted when a new upfront sales fee is set.
    event UpfrontSalesBpsUpdated(uint256 upfrontFeeBps);

    /// @dev Emitted when a new royalty sales fee is set.
    event RoyaltySalesBpsUpdated(uint256 royaltyFeeBps);
