...
contract FixlineBetting is Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        nextRewardTime = block.timestamp + (20 minutes / maxRewardsInDay);
    ...
    function sendRewardsToStackingContract(
        ...
        if (
            ...
            (block.timestamp >= (nextRewardTime + 30 minutes))
        ) {
            uint amountForCapsStacking;
            // Make Sure To Give Approval To WETH And Swap Router Contract
            if (
                (totalPendingStableCoinsToSwapForCapsStackingContract +
                    totalPendingStableCoinsToSwapForFixlineStackingContract) >=
                minStableCoinSwapToETH
                ...
                IWETH(WETHAddress).withdraw(amountOut);
                 amountForCapsStacking=((((amountOut * 1e18) / (totalPendingStableCoinsToSwapForCapsStackingContract + totalPendingStableCoinsToSwapForFixlineStackingContract)) * totalPendingStableCoinsToSwapForCapsStackingContract) /
                    1e18);
