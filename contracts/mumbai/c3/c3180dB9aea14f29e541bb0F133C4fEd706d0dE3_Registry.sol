...
contract Registry is IRegistry, Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function setEconomics(address _economics) external isContract(_economics) onlyOwner {
        emit UpdateEconomics(address(economics), _economics);
        economics = IEconomics(_economics);
    }

    function setEventFactory(address _eventFactory) external isContract(_eventFactory) onlyOwner {
        emit UpdateEventFactory(address(eventFactory), _eventFactory);
    ...
    modifier isContract(address account) {
        require(account.isContract(), "Registry: address not a contract");
        _;
    }
