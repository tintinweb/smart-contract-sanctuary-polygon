    ...
    Ownable
{
    ...
    function mint(address _to, string memory uri) external override {
        ...
        setRoyalty(tokenId, msg.sender, royaltyFee);

        emit Minted(msg.sender, _to, uri, tokenId);
    }

    function multiMint(string[] calldata URIs, address _to) external override {
    ...
    function updateRoyaltyFee(uint16 _royaltyFee) external onlyOwner {
        require(_royaltyFee <= MAX_ROYALTY, "Invalid fee rate");
