...
contract ForgeToken is ERC721Enumerable, Ownable {
    ...
    /// @param sig The signature of the recipient of the minted NFT
    //-------------------------------------------------------------------------
    function safeMint(uint256 nonce, bytes memory sig) external {
        require(!usedNonces[nonce], "Nonce already used");

        bytes32 message = _prefixed(keccak256(abi.encodePacked(msg.sender, nonce, this)));
        require(_recoverSigner(message, sig) == minterAddress, "Signer address mismatch");
