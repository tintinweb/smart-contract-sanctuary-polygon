...
contract Runiverse is Pausable, ReentrancyGuard, EIP712, RuniverseAdministrative, IDomainSeparator {
	...
	function startRun(uint256 gameId, SignedStartGame calldata player1, SignedStartGame calldata player2) external onlyOperator whenNotPaused nonReentrant {
        ...
        if(player1.useJolly==0)
        {
            bytes32 structHash = keccak256(abi.encode(_PLAYER_TYPEHASH, player1.sender, player1.tokenChosen,player1.amountBetted, _useNonce(player1.playerAddress), player1.deadline));
            //check signature
            checkSignature(structHash,player1.v,player1.r,player1.s,player1.sender);
        }
        else
        {
            // nonce = _useNonce(player1.sender);
            bytes32 structHash = keccak256(abi.encode(_JOLLYUSED_TYPEHASH,player1.sender, player1.playerAddress, player1.tokenChosen,player1.amountBetted, _useNonce(player1.playerAddress), player1.deadline));
            checkSignature(structHash,player1.v,player1.r,player1.s,player1.sender);
        }

        if(player2.useJolly==0)
        {
            bytes32 structHash = keccak256(abi.encode(_PLAYER_TYPEHASH, player2.sender, player2.tokenChosen,player2.amountBetted, _useNonce(player2.playerAddress), player2.deadline));
            //check signature
            checkSignature(structHash,player2.v,player2.r,player2.s,player2.sender);
        }
        else
        {
            bytes32 structHash = keccak256(abi.encode(_JOLLYUSED_TYPEHASH,player2.sender, player2.playerAddress, player2.tokenChosen,player2.amountBetted, _useNonce(player2.playerAddress), player2.deadline));
            checkSignature(structHash,player2.v,player2.r,player2.s,player2.sender);
        }
    ...
    function checkSignature(bytes32 signature,uint8 v,bytes32 r, bytes32 s, address sender) internal view{ 
        // console.log(sender);
        address signer = ECDSA.recover(_hashTypedDataV4(signature), v, r, s);
        // console.log(signer);
