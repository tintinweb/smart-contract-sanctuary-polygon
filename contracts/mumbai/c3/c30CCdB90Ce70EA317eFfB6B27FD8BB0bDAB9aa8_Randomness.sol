...
contract Randomness is VRFConsumerBase {
     ...
     * 得到随机数,不用自己提供种子，种子为hash生成
       @param bonusRound => 对应储蓄抽奖轮数 => 这个合约里面记录，所以lottofund合约的记录可以删掉了
     */
    function getRandom(uint256 bonusRound) external {
        //一轮只能获取一个随机数 => 测试可屏蔽
        ...
        //获取requestId,并告知需要获取一个随机数 
        bytes32 _requestId = requestRandomness(keyHash, fee);
        //fullfill函数在这里执行
        
        requestIds[_requestId] = bonusRound;
    }
    ...
    //function rawFulfillRandomness(bytes32 requestId, uint256 randomness) external override {
        //require(_vrfCoordinator == msg.sender, "Fulfilment only permitted by Coordinator");
        //fulfillRandomness(requestId, randomness);
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override{
        ...
        //记录id
        uint bonusround= requestIds[requestId];
        randomNumber[bonusround] = randomness;
    }
    
    //外部获取
    function getRandomNumber(uint256 bonusRound) external view returns (uint256){
        return randomNumber[bonusRound];
    }
}