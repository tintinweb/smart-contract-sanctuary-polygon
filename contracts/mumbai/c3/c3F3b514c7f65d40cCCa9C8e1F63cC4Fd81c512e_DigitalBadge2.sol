    ...
    UUPSUpgradeable
{
    ...
    function valueBadge(uint256 _tokenId) public view returns (uint256) {
        return metadataToken[_tokenId].amountUSDC;
        
    }
    function quemar(uint256 tokenId)  public {
        uint256 _valueBadge = metadataToken[tokenId].amountUSDC;
        address owner = super.ownerOf(tokenId);
        require( usdcToken.allowance(msg.sender, owner) >= _valueBadge,"allowance");
        require( usdcToken.balanceOf(msg.sender) >= _valueBadge,"Not enough balance");
        _burn(tokenId);
    }
    ...
    function burn(uint256 tokenId) public  onlyRole(VERIFIER_ROLE) {
        ...
        //require( usdcToken.allowance(msg.sender, owner) >= 1000, "Dig Ba: Not enough allw");
        //require( usdcToken.balanceOf(msg.sender) >= 1000,  "Dig Ba: Not enough balance");
        
        //usdcToken.transferFrom(msg.sender, owner, 1000);
