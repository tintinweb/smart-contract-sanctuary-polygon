...
import "../LushLinkNftV3.sol";

contract CarkayousExplorersDEV is LushLinkNftV3 {
    mapping(uint => string) private overrideIds;
    constructor(address _ownerAddress) LushLinkNftV3(
        "Carkayous Explorers (DEV)",
        "CARKAYOUS_EXPLORERS_DEV",
        "https://www.develop-sr3snxi-telwwy7z25xi4.us-4.platformsh.site/collection/cllJYTBSQlA5U3RldHA3QmM2a1U1QT09/nft",
        "https://www.develop-sr3snxi-telwwy7z25xi4.us-4.platformsh.site/sites/default/files/collections/7295/pre-release-images/Explorers-prereveal-image-V1_1.jpg",
        _ownerAddress,
        10000
    ){
...
contract LushLinkNftV3 is ERC721Enumerable, ERC2981, Hierarchy {
    ...
    uint256 public minReveal;
    mapping(uint256 => string) public reveal;
    mapping(uint => string) private overrideIds;
    ...
    // Set admin role | reduced byte size
    ...
    // Supply
    function setMaxSupply(uint256 value) public onlyAdmin {
        // require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Caller is not allowed to set max supply");
        require((_currentId + 1) < value, "Max supply can not be less than current supply");
        maxSupply = value;
        // emit MaxSupplyUpdated(msg.sender, value);
    }
    ...
    function setRegularCost(uint256 _cost) public onlyAdmin {
    ...
    // Minting.
    function setAllowPublicMint(bool _allowed) public onlyAdmin {
        // require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Caller is not allowed to set public mint.");
        allowPublicMint = _allowed;
    }
    ...
    function publicMint(uint256 _mintAmount) public payable {
        require(allowPublicMint, "Caller is not allowed to mint");
        require(_mintAmount > 0, "need to mint at least 1 NFT");
    ...
    function airdropMint(address _to, uint256 _amount) public onlyAdmin {
        // require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Caller is not allowed to airdrop");
        require(_currentId + _amount <= maxSupply, "Will exceed maximum supply");
    ...
    function airdropMintBatch(address[] memory destinations) public onlyAdmin {
        // require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Caller is not allowed to airdrop");
        require(_currentId + destinations.length <= maxSupply, "Will exceed maximum supply");
    ...
    function withdraw() public payable onlyAdmin {
        // require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Caller is not allowed to withdraw");
    ...
    function burn(uint256 tokenId) public {
        ...
        _burn(tokenId);
        // If the burned token ID is the latest minted token ID, update the latestMintedTokenId variable
        if (tokenId == latestMintedTokenId) {
            latestMintedTokenId--;
        }
    }
    ...
    function tokenURI(uint _tokenId) public view virtual override returns (string memory) {
        ...
        if (keccak256(bytes(reveal[_tokenId])) == keccak256("reveal")) {
    ...
    function setNotRevealedURI(string memory _notRevealedURI) public onlyAdmin {
        // require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Caller is not allowed to set notRevealURI");
        notRevealedURI = _notRevealedURI;
    }

    function revealToggle( uint256 _minReveal, uint256 _maxReveal) public onlyAdmin {

        // maxReveal
        if (_maxReveal > maxReveal) {
            for (uint256 i = maxReveal; i <= _maxReveal ; i++) {
                reveal[i] = "reveal";
            }

        } else {
            for (uint256 i = (_maxReveal+1); i <= maxReveal; i++) {
                reveal[i] = "hidden";
            }
        }

        // minReveal
        // make very first minReveal = 0
        if (_minReveal > minReveal) {
            for (uint256 i = minReveal; i < _minReveal; i++) {
                reveal[i] = "hidden";
            }
        } else {
            for (uint256 i = _minReveal; i < minReveal; i++) {
                reveal[i] = "reveal";
            }
        }

        maxReveal = _maxReveal;
        minReveal = _minReveal;
    }
    ...
    function removeAdminRole(address account) public {
        require(account != ownerAddress, "Can not remove Admin role from owner.");
