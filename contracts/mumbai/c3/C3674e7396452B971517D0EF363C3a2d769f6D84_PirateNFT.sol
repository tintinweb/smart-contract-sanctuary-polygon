    ...
    IRandomizerCallback
{
    ...
    function isTrustedForwarder(address forwarder)
        ...
        returns (bool)
    {
        return _hasAccessRole(TRUSTED_FORWARDER_ROLE, forwarder);
    }
...
interface ITraitsConsumer {
    /** @return TraitsProvider for this consumer */
...
interface ILockingSystem is ISystem, IERC165 {
     ...
     * Locks an NFT
     *
     * @param account        Account that is taking action
...
contract GameNFT is TraitsConsumer, IGameNFT, ERC721BridgableChild {
    ...
    /// @notice Only LockingSystem can call lock/unlock token
    error OnlyLockingSystemCanCall();
    ...
    function lockToken(uint256 tokenId) external override {
        if (_msgSender() != address(_lockingSystem())) {
            revert OnlyLockingSystemCanCall();
        }
        _lockToken(tokenId);
    }
    ...
    /** @return Whether or not a token is soulbound */
    ...
    function _beforeTokenTransfer(
        ...
        if (from != address(0)) {
            ...
            if (to != address(0) && _isSoulbound[tokenId] == true) {
...
import {IERC721Lockable} from "../lockable/IERC721Lockable.sol";
...
import {IERC721Lockable} from "./IERC721Lockable.sol";
...
abstract contract ERC721Lockable is IERC721Lockable, ERC721Enumerable {
    ...
    /// @notice Rescue mode not enabled
    error RescueNotEnabled();

    /// @notice Call was not made by owner
    error NotOwner();

    /// @notice Token is locked and cannot be transferred or cannot be locked
    error IsLocked();

    /// @notice Token is not locked and cannot be unlocked
    error IsAlreadyUnlocked();

    /// @notice Token is not unlocked and cannot be locked
    error IsAlreadyLocked();
    ...
    function rescueUnlock(uint256 tokenId) external {
        if (getRescueUnlockEnabled() == false) {
    ...
    function _lockToken(uint256 tokenId) internal virtual {
        if (_isLocked[tokenId] == true) {
            revert IsAlreadyLocked();
        }
    ...
    function _unlockToken(uint256 tokenId) internal virtual {
        if (_isLocked[tokenId] == false) {
            revert IsAlreadyUnlocked();
        }
