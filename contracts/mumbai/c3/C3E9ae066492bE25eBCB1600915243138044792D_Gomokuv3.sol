...
contract Gomokuv3 is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    function ChanegeOpen() external onlyGameOwner(){
    ...
    function EntryGame() external {
        // 開放状態を確認
        require(dataGomoku[nonce].OpenStatus == true, "Gama is CLOSE");
        // 終了状態確認
        require(dataGomoku[nonce].flgEnd == false, "Gama is END");
    ...
    function StoneHaiti(uint8 x, uint8 y) external{
        ...
        if(x == 15 && y == 15){
            emit evTouryou();
            if(msg.sender == dataGomoku[nonce].Player1Addr){
                dataGomoku[nonce].winPlayerAddr = dataGomoku[nonce].Player2Addr;
            }
            else
            {
