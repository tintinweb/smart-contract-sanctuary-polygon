...
contract Marimo is Ownable, ERC721A {
  ...
  mapping(uint256 => uint256[]) public tokenHistoryIds; // token id => changeWaterHistory
  ChangeWaterHistory[] private _changeWaterHistories;

  function getHistoryIds(uint256 tokenId) public view returns (uint256[] memory) {
    return tokenHistoryIds[tokenId];
  }

  function getHistory(uint256 historyId) public view returns (ChangeWaterHistory memory) {
    return _changeWaterHistories[historyId];
  }
  ...
  function changeWater(uint256 tokenId) external {
    ...
    _changeWaterHistories.push(ChangeWaterHistory(msg.sender, block.timestamp));
    _historyIndex += 1;
    tokenHistoryIds[tokenId].push(_historyIndex);
