...
contract BatchTransfer is Ownable {
	function batchTransfer(
		...
		require(
			erc20Token.balanceOf(address(this)) >= totalAmount,
			'Contract has insufficient balance.'
		);

		for (uint256 i = 0; i < recipients.length; i++) {
			require(
				erc20Token.transferFrom(address(this), recipients[i], amounts[i]),
	...
	function withdraw(address token) external onlyOwner {
		...
		require(erc20Token.balanceOf(address(this)) > 0, 'Contract has no balance of such token.');
