...
import {ITamswapFactory} from "../interfaces/ITamswapFactory.sol";
import {ITamswapCallee} from "../interfaces/ITamswapCallee.sol";
import {ITamswapPair} from "../interfaces/ITamswapPair.sol";
import {Math} from "../libraries/Math.sol";
import {UQ112x112} from "../libraries/UQ112x112.sol";
import {SafeMath} from "../libraries/SafeMath.sol";
import {TamswapERC20} from "./TamswapERC20.sol";

contract TamswapPair is TamswapERC20{
    ...
    function _safeTransfer(address token, address to, uint value) private {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'Tamswap: TRANSFER_FAILED');
    }
    ...
    function initialize(address _token0, address _token1) external {
        require(msg.sender == factory, 'Tamswap: FORBIDDEN'); // sufficient check
        token0 = _token0;
        token1 = _token1;
    }
    ...
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= type(uint112).max && balance1 <= type(uint112).max, 'Tamswap: OVERFLOW');
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        address feeTo = ITamswapFactory(factory).feeTo();
...
// SPDX-License-Identifier: SEE LICENSE IN LICENSE
pragma solidity ^0.8.0;
import {ITamswapERC20} from "../interfaces/ITamswapERC20.sol";
import {SafeMath} from "../libraries/SafeMath.sol";

contract TamswapERC20{
    ...
    string public constant name = "Tamswap";
    string public constant symbol = "TAM";
    ...
    function permit(address owner, address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
        require(deadline >= block.timestamp, 'Tamswap: EXPIRED');
        ...
        require(recoveredAddress != address(0) && recoveredAddress == owner, 'Tamswap: INVALID_SIGNATURE');
