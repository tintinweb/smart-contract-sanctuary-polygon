...
contract Marketplace is Ownable, Pausable, MarketplaceStorage, NativeMetaTransaction {
  ...
  function getassetOwner(address add, uint256 id) public view returns(address){
      ERC721Interface nftRegistry = ERC721Interface(add);
        address assetOwner = nftRegistry.ownerOf(id);
    return assetOwner;
  }
