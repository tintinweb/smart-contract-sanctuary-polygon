...
contract EthPlays is ERC20, Ownable {
  ...
  /// @notice Count of votes for each button index by input index
  mapping(uint256 => uint256[8]) public inputVotes;
  /// @notice The block timestamp of the previous input
  uint256 public previousInputTimestamp;
  ...
  uint256 private alignmentDecayPerBlock = 980;
  ...
  function submitButtonInput(uint256 buttonIndex) external onlyActive onlyRegistered {
    ...
    if (block.timestamp <= previousIndividualControlStart + 30 seconds) {
      // Individual control.
      ...
      inputVotes[inputIndex][buttonIndex]++;

      _mint(msg.sender, orderReward);
      emit InputVote(inputIndex, msg.sender, buttonIndex);

      if (block.timestamp >= previousInputTimestamp + 20) {
        // If 20 seconds have passed since the previous input, execute.
        uint256 bestButtonIndex = 0;
        uint256 bestButtonIndexVoteCount = 0;

        for (uint256 i = 0; i < 8; i++) {
          if (inputVotes[inputIndex][i] > bestButtonIndexVoteCount) {
            bestButtonIndex = i;
            bestButtonIndexVoteCount = inputVotes[inputIndex][i];
          }
        }

        previousInputTimestamp = block.timestamp;
  ...
  /// @param vote The alignment vote. True corresponds to order, false to chaos.
  function submitAlignmentVote(bool vote) external onlyActive onlyRegistered {
    if (block.number - previousAlignmentDecayBlock < 100) {
      // If players are active, reduce alignment magnitude by 2% on each vote
    ...
    alignment += vote ? int256(1000) : -1000;

    previousAlignmentDecayBlock = block.number;

    emit AlignmentVote(msg.sender, vote, alignment);
  }
