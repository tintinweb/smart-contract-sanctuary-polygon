...
contract GroupChallenge is Initializable, VerifyData, UUPSUpgradeable {
    using SafeMath for uint256;

    enum UserStatus {NotYet, AlreadyEntry, Successed, Failed, GiveUp}

    enum TypePayment {Normal, NftMetri, Random}

    enum TypeGiveup {GiveupOption1, GiveupOption2}
    ...
    struct ChallengeInfor {
        ...
        uint256 entryFee; // 8
        bool isAllowGiveUp; // 9
        address token20Address; // 10
    }
    // [1678025148, 1678025400, 1678025400, 1678025700, 120, 6000, 1, 1,9000000000000000000]
    ...
    modifier onTimeSendResult() {
        require(getChallengeStatus() == 3 || getChallengeStatus() == 2, "NOT TIME TO SEND DAILY RESULT.");
        _;
    }
    
    modifier onTimeForPayment() {
        require( getChallengeStatus() == 4, "NOT TIME TO PAYMENT TO ACHIEVED USER.");
        _;
    }

    modifier onTime() {
        require(getChallengeStatus() == 2, "NOT TIME TO SEND GIVE UP.");
        _;
    }
    ...
    function initialize(
        ...
        address _nftHold,
        TypeGiveup _typeGiveup,
        uint256[] memory _percentGiveup
    ) external initializer {
        __UUPSUpgradeable_init();
        __Verify_init();
        updateChallengeInfo(_challengeInfor, _token20Address, _isAllowGiveUp); 
        ...
        require(checkPercentPayment(percentRateSendDaily), "INVALID PERCENT RATE SEND DAILY."); 
        ...
        require(checkPercenGiveup(percentGiveup), "INVALID PERCENT RATE PERCENT GIVEUP."); 
        
        adminAddress = _msgSender();
    }

    function getChallengeStatus() public view returns(uint256) {
        if(
        ...
        if(
            block.timestamp <= challengeInfor.challengeFinish.add(challengeInfor.endDaySendStepData) &&
            block.timestamp >= challengeInfor.challengeStart
        ) {
            return 3;
        }

        if(block.timestamp > challengeInfor.challengeFinish.add(challengeInfor.endDaySendStepData)) {
            return 4;
        }
    ...
    function checkPercentPayment(PercentRateSendDaily memory _percentRateSendDaily) internal pure returns(bool) {
        uint256 totalPaymentPercent = _percentRateSendDaily.adminPercent.add(_percentRateSendDaily.successPercent)
            .add(_percentRateSendDaily.holdNftPercent).add(_percentRateSendDaily.goldPercent)
            .add(_percentRateSendDaily.silverPercent).add(_percentRateSendDaily.bronzePercent);

        require(totalPaymentPercent == 100, "INVALID TOTAL PERCENT FOR PAYMENT.");

        if(_percentRateSendDaily.typePayment == TypePayment.Normal) {
            if(_percentRateSendDaily.adminPercent.add(_percentRateSendDaily.successPercent) == 100) {
                return true;
            }  
        }

        if(_percentRateSendDaily.typePayment == TypePayment.NftMetri) {
            if(
                _percentRateSendDaily.adminPercent.add(_percentRateSendDaily.successPercent)
                .add(_percentRateSendDaily.holdNftPercent) == 100 && _percentRateSendDaily.nftHold != address(0)
            ) {
                return true;
            }
        }

        if(_percentRateSendDaily.typePayment == TypePayment.Random) {
            if(
                totalPaymentPercent == 100 &&
                _percentRateSendDaily.nftHold != address(0)
            ) {
                return true;
            }
        }
    ...
    function checkPercenGiveup(PercentGiveup memory _percentGiveup) internal pure returns(bool) {
        uint256 totalGiveupPercent = _percentGiveup.adminGiveupPercent.add(_percentGiveup.successGiveupPercent)
            .add(_percentGiveup.giveUpPercent);

        require(totalGiveupPercent == 100, "INVALID TOTAL PERCENT FOR GIVEUP.");  

        if(_percentGiveup.typeGiveup == TypeGiveup.GiveupOption1) {
            if(_percentGiveup.adminGiveupPercent.add(_percentGiveup.successGiveupPercent) == 100) {
                return true;
            }
        }

        if(_percentGiveup.typeGiveup == TypeGiveup.GiveupOption2) {
            if(
                _percentGiveup.adminGiveupPercent.add(_percentGiveup.successGiveupPercent).add(_percentGiveup.giveUpPercent) == 100
            ) {
                return true;
            }
        }
    ...
    function userEntryChallenge() external {
        require(
            userInfor[_msgSender()].userStatus == UserStatus.NotYet,
        ...
        userInfor[_msgSender()].userStatus = UserStatus.AlreadyEntry;
        ...
        challengeStatus.listUser.push(_msgSender());
        challengeStatus.totalAmountCollected = challengeStatus
            .totalAmountCollected
            .add(challengeInfor.entryFee);

        emit UserEntryChallenge(_msgSender(), challengeInfor.entryFee);
    }
    ...
    function giveUp() external canGiveUp onTime {
        ...
        userInfor[_msgSender()].userStatus = UserStatus.GiveUp;

        emit GiveUp(_msgSender());
    }

    function sendDailyResult(
        uint256[] memory _day, 
        uint256[] memory _stepIndex
    ) external onTimeSendResult {
        require(
            userInfor[_msgSender()].userStatus == UserStatus.AlreadyEntry,
        ...
        for (uint256 i = 0; i < _day.length; i++) {
            require(userInfor[_msgSender()].stepOn[_day[i]] == 0, "THIS DAY'S DATA HAD ALREADY UPDATED.");
            userInfor[_msgSender()].stepOn[_day[i]] = _stepIndex[i];
            userInfor[_msgSender()].historyDate.push(_day[i]);
            userInfor[_msgSender()].historyData.push(_stepIndex[i]);
            if(
                _stepIndex[i] >= challengeInfor.targetStepsPerDay && 
                userInfor[_msgSender()].currentStatus < challengeInfor.dayRequired
            ) {
                userInfor[_msgSender()].currentStatus =  userInfor[_msgSender()].currentStatus.add(1);
            }
        }

        userInfor[_msgSender()].sequence = userInfor[_msgSender()].sequence.add(_day.length);
        if (
            userInfor[_msgSender()].sequence.sub(userInfor[_msgSender()].currentStatus) > 
            challengeInfor.challengeDuration.sub(challengeInfor.dayRequired)
        ){
            userInfor[_msgSender()].userStatus = UserStatus.Failed;
        } else {
            if (userInfor[_msgSender()].currentStatus >= challengeInfor.dayRequired) {
                userInfor[_msgSender()].userStatus = UserStatus.Successed;
            }
        }

        emit SendDailyResult(_msgSender(), _day, _stepIndex); 
    }
    ...
    function paymentToAchievedUser() external onTimeForPayment {
        ...
        //     if (_msgSender() == challengeStatus.listUser[i]) {
        //         isUserHasEntry = true;
        //         break;
        //     }
        // }
        ...
        // require(
        //     isUserHasEntry || adminAddress == _msgSender() || checkOperator(_msgSender()),
        //     "ONLY USER HAS ENTRY OR ADMIN CAN CALL THIS FUNCTION."
        // );

        // for (uint256 i = 0; i < challengeStatus.listUser.length; i++) {
        //     if (
        //         userInfor[challengeStatus.listUser[i]].userStatus == UserStatus.Successed
        //     ) {
        //         challengeStatus.listUserAchieved.push(challengeStatus.listUser[i]);
        //     } else if (
        //         userInfor[challengeStatus.listUser[i]].userStatus == UserStatus.Failed ||
        //         userInfor[challengeStatus.listUser[i]].userStatus == UserStatus.AlreadyEntry
        //     ) {
        //         challengeStatus.listUserNotAchieved.push(challengeStatus.listUser[i]);
        //     }
        // }

        if(percentRateSendDaily.typePayment == TypePayment.Normal) {
            normalPayment();
        }
    ...
    function normalPayment() internal {
        uint256 totalAmountSuccess;
        uint256 amountForAdmin;
        if(challengeStatus.listUserNotAchieved.length > 0) {
            totalAmountSuccess = challengeStatus.listUserNotAchieved.length.mul(challengeInfor.entryFee);
            amountForAdmin = totalAmountSuccess.mul(percentRateSendDaily.adminPercent).div(100);
        ...
        uint256 totalAmountForWinner = totalAmountSuccess.sub(amountForAdmin);
        uint256 amountForEachOtherAdmin;
        if(challengeStatus.listUserAchieved.length > 0){
            amountForEachOtherAdmin = totalAmountForWinner.div(challengeStatus.listUserAchieved.length).add(challengeInfor.entryFee);
        } else {
            amountForEachOtherAdmin = challengeInfor.entryFee;
        }
    ...
    function updateChallengeInfo(
        ...
        require(
            _challengeInfor[5] > 0 && _challengeInfor[6] > 0 &&
            _challengeInfor[7] > 0 && _challengeInfor[8] > 0,
    ...
    function checkRandomNumber(uint256 _limitValue) internal view returns (uint256) {
        uint256 randomValue = uint256(
            keccak256(
                abi.encodePacked(block.number, block.difficulty, _msgSender())
            )
        ) % _limitValue;
        return randomValue;
    }
...
abstract contract VerifyData is Initializable, Context {
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    modifier onlyOperator() {
        require(operator.contains(_msgSender()), "NOT OPERATOR.");
        _;
    }
    ...
    function updateOperator(address _operatorAddr, bool _flag) external onlyOperator {
        require(operator.values().length != 0 || operator.contains(_msgSender()) ,"NOT OPERATOR.");
    ...
    function checkOperator(address _operatorAddr) internal view returns(bool){
        return operator.contains(_operatorAddr);
    }
}