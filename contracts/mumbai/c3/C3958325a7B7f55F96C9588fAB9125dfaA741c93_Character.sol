...
contract Character is ERC721URIStorage, Ownable {
  ...
  // charcter traits
  // id,level from 1 - 5 , evac, tokenURI
  struct Char {
    uint256 id;
    uint8 level;
    uint256 evac;
  ...
  event NewChar(address indexed owner, uint256 id);
  ...
  modifier mintCompliance() {
    ...
    // condition: total minted + this potential tx is under maxsupply
    require(
      _tokenIds.current().add(1) < maxSupply,
      "Max supply exceeded."
    );
    ...
    mintCompliance()
  {
    ...
    emit NewChar(msg.sender, newCharID);
  }
  ...
  /** it should be onlyOwner(contractOwner address) funcs but according to test requirement it is user initiated function
  ...
  function LevelUp(
    ...
    require(char.level <= 5 , "level cannot be more than 5");
    // level up
    if (_levelUp == true) {
      char.level++;
    }
