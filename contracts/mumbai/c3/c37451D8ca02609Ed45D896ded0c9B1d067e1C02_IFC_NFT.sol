...
contract IFC_NFT is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public mintPrice = 100000000000000; // 0.0001 MATIC
    ...
    function setRevealed(uint256 _tokenId) public isOperator {
        revealed[_tokenId] = true;
    }

    function getRevealed(uint256 _tokenId) public view returns (bool) {
    ...
    function setTokenUri(uint256 _tokenId, string memory _uri) public isOperator {
        tokenURIs[_tokenId] = _uri;
    }

    /// Required for paper.xyz
    ...
    function getClaimIneligibilityReason(address _userWallet, uint256 _quantity) public view returns (string memory) {
        if (!isActive) {
            return 'Contract not live yet';
        } else if (_quantity > maxMintAmountPerTx) {
            return 'Max mint amount per wallet exceeded';
        } else if (totalSupply() + _quantity > maxSupply) {
            return 'Not enough supply';
        } else if (_numberMinted(_userWallet) + _quantity > maxMintPerWallet) {
            return 'Cannot buy this quantity of nft';
        }
        return '';
    }
    ...
    function giveawayTo(address _userWallet, uint256 _quantity)
        public
        isOperator
        mintCompliance(_quantity)
    {
     ...
     */
    function withdraw(address _wallet) public onlyOwner nonReentrant {
