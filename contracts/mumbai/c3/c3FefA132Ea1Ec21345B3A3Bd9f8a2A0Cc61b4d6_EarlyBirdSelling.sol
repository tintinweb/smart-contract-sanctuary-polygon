...
contract ERC20Implementation is ERC20, Pausable {
    ...
    uint256 _totalSupply = 300000000 * 1e6;
...
contract EarlyBirdSelling is Pausable {
    ...
    //Matic swap rate, default:2
    uint256 swapRate = 2;
    //USDT swap rate，default:1.25, so below have to divided by 100
    uint256 swapRate_USDT = 125;
    //WETH swap rate，default:3750
    uint256 swapRate_WETH = 3750;

    uint256 maxRelease = 500000 * 1e6;
    uint256 restRelease = 500000 * 1e6;
    uint256 accountMaximum = 100 * 1e6;
    ...
    modifier basicCkeck(uint256 _amount) {
    ...
    function buy(uint256 _amount) external payable basicCkeck(_amount) whenNotPaused {
        uint256 paymentTobuy = (_amount * 1e12) / swapRate;
        require(
    ...
    function buyWithUSDT(uint256 _buyamount, uint256 _payamount)
        external
        basicCkeck(_buyamount)
        whenNotPaused
    {
        uint256 paymentTobuy = (_buyamount / swapRate_USDT) * 100;
    ...
    function buyWithWETH(uint256 _buyamount, uint256 _payamount)
        ...
        whenNotPaused
    {
        uint256 paymentTobuy = (_buyamount * 1e12) / swapRate_WETH;
        require(_payamount >= paymentTobuy, "The payment is not enough");
