...
import "./UmiverseUser1155Token.sol";
...
contract Factory1155 {
    ...
    modifier onlyOwner() {
        require(msg.sender == owner, "ownable: caller doesn't have role");
        _;
    }

    constructor(address _escrow, address _proxy) {
        escrow = _escrow;
        proxy = _proxy;
    }

    function changeProxy(address _proxy) external onlyOwner {
    ...
    function deploy(
        ...
        addr = address(
            new UmiverseUser1155Token{salt: _salt}(name, symbol, tokenURIPrefix, escrow, proxy)
        );
        UmiverseUser1155Token token = UmiverseUser1155Token(address(addr));
...
contract UmiverseUser1155Token is
    Context,
    ERC1155Burnable,
    ERC1155Supply,
    AccessControl
{
    ...
    function isApprovedForAll(address _owner, address operator) public view override(ERC1155) returns(bool) {
    ...
    function _beforeTokenTransfer(
        ...
        for(uint256 i = 0;i<ids.length; i++) {
            require(IEscrow(escrow).isRented(address(this), from, ids[i], amounts[i]), "ERC721Rental: NFT is on Rental");
        }
        }
