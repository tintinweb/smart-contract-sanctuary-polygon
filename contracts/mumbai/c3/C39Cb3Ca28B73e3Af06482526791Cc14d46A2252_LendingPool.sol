...
// File: contracts3/libraries/ABDKMath64x64.sol
...
// File: contracts3/libraries/ConstantLib.sol


pragma solidity ^0.8.8;

uint256 constant DECIMAL_FACTOR = 1e18;
uint256 constant SECONDES_PER_YEAR = 356 days;

library ConstantLib {}

// File: contracts3/TokenPrice.sol
...
// File: contracts3/InterestRate.sol
    ...
    function CalculateInterestRate(
        ...
        uint256 interestRate_
            ...
            if (utilizationRate_ < optimalUtilizationRate_){
                slope_  =  
                 (optimalUtilizationRate_ - utilizationRate_) * (interestRate_ - minValue_) / (DECIMAL_FACTOR - utilizationRate_);
                newInterestRate_ = interestRate_ - slope_;

            } else { 
                slope_  =  
                (utilizationRate_ - optimalUtilizationRate_) * (maxValue_ - interestRate_) / utilizationRate_;
                newInterestRate_=  interestRate_ + slope_;
            }
...
// File: contracts3/libraries/AssetTransfer.sol
...
// File: contracts3/LendingPool.sol
...
contract LendingPool is AccessControlEnumerable, Pausable {
    ...
    DDeployer public dDeployer;
    LDeployer public lDeployer;
    ...
    event UpdateModelParams(address indexed asset_);
    event AddAsset(
        address indexed asset_,
    ...
    event UpdateCollateral(address indexed asset_, address indexed token_, bool state_);
    ...
    function _supply(address asset_, uint256 amount_, address to_) internal {
        ...
        if (asset_ != address(0)) {
            ...
            lenderSupply[asset_][to_]+= amount_;
    ...
    function _withdraw(
        ...
        require(reserveInfo_.isActivated, "Securd: asset is not listed");
        require(
            assetAmount_ <= reserveInfo_.supply,
        ...
        DToken(dToken_).burn(msg.sender, dTokenAmount_);
         lenderSupply[asset_][msg.sender]-= Math.min(lenderSupply[asset_][msg.sender], assetAmount_);
    ...
    function _updateModelParams(address asset_) internal {
        ...
        if (block.timestamp > reserveInfo_.lastTime) {
            ...
            if (supplyWithInterests_!=0) {
                utilizationRate_ = (debtWithInterests_ * DECIMAL_FACTOR) /
    ...
    function addAsset(
        ...
        reserveInfos[asset_].interestRateInfo.interestRate = initialInterestRate_;
    ...
    function updateProtocoleFee(address asset_, Fee memory fee_) external {
        require(
            reserveInfos[asset_].isActivated,
    ...
    function updateSupplyCap(address asset_, uint256 supplyCap_) external {
        require(
            reserveInfos[asset_].isActivated,
            "Securd: asset is not listed"
        );
    ...
    function sendAsset(address asset_, uint256 amount_, address to_) external {
        require(to_ != address(0), "Securd: zero address");
        require(
            reserveInfos[asset_].isActivated,
            "Securd: asset is not listed"
        ...
        require(
            amount_ <= reserveInfos[asset_].supply,
            "Securd: not enough liquidity"
        );
    ...
    function mintOrBurnLToken(
        ...
        if (direction_) {
            ...
            reserveInfos[asset_].debt -= Math.min(reserveInfos[asset_].debt, amountAsset_);
        }
    }

    function getLTokenPrice(
        address asset_
    ...
    function getLenderSupply(
        ...
        return lenderSupply[asset_][lender_];
    }
