...
import {IMarket, IERC165} from "./IMarket.sol";

/// @title BaseMarketUpgradeable
/// @dev This is an abstract contract for the marketplace.
abstract contract BaseMarketUpgradeable is IMarket, MarketHubRegistrarUpgradeable, SignatureValidator, UUPSUpgradeable {
    ...
    /// @dev Throws if the new implementation is not the target implementation.
    ...
    /// @notice Creates a new sale in the marketplace.
    /// @dev Throws if currency or collection is not registered, if seller or buyer is not registered, or if the seller is the buyer.
    ...
    function supportsInterface(
        ...
        return
            _interfaceId == type(IMarket).interfaceId ||
            _interfaceId == type(IERC165).interfaceId ||
     ...
     * @param _saleId The id of the sale that has closed.
     */
    function handleSaleClosed(uint256 _saleId) external virtual override;
}
...
import {IOfferMarket} from "./IOfferMarket.sol";
import {BaseMarketUpgradeable} from "./BaseMarketUpgradeable.sol";
import {IColleCollection} from "../collections/IColleCollection.sol";
import {IEscrow} from "../escrow/IEscrow.sol";

/**
 * @title An upgradable offer market contract
 * @dev This contract is part of the Colle protocol and governs all operations related to offering a token for sale.
 */
contract OfferMarketUpgradeable is IOfferMarket, BaseMarketUpgradeable {
    // Nested mapping of offers, organized by collection, token id, buyer, and erc20 token
    mapping(address => mapping(uint256 => mapping(address => mapping(address => Offer)))) internal offers;

    /// @dev An unsafe operation that initializes the contract
    /// @custom:oz-upgrades-unsafe-allow constructor
    constructor() {
    ...
    /**
     * @notice Offer a token for sale
     * @dev The function is publicly accessible and makes a new offer
     * @param _collection The collection that the token belongs to
     * @param _tokenId The ID of the token to offer
     * @param _erc20 The address of the ERC20 token to use as payment
     * @param _price The price at which to offer the token
     * @param _payNow Whether or not payment should be made immediately
     */
    function offer(address _collection, uint256 _tokenId, address _erc20, uint256 _price, bool _payNow) public virtual {
        _offer(msg.sender, msg.sender, _collection, _tokenId, _erc20, _price, _payNow);
    }

    /**
     * @notice Offer a token for sale with a signature
     * @dev The function is publicly accessible, verifies the signature and makes a new offer
     ...
     * @param _price The price at which to offer the token
     * @param _payNow Whether or not payment should be made immediately
     * @param _deadline The time when the permission to offer expires
     * @param _signature The signature that approves the offer
     */
    function permitOffer(
        address _buyer,
        address _collection,
    ...
    /**
     * @notice Offer a token for sale with off-chain payment and a signature
     * @dev The function is publicly accessible, verifies the signature and makes a new offer with off-chain payment
     * @param _buyer The address of the buyer
     * @param _collection The collection that the token belongs to
     * @param _tokenId The ID of the token to offer
     * @param _erc20 The address of the ERC20 token to use as payment
     ...
     * @dev The function is publicly accessible and revokes an offer
     * @param _collection The collection that the token belongs to
     * @param _tokenId The ID of the token whose offer to revoke
    ...
    /**
     * @notice Revoke an offer with a signature
     * @dev The function is publicly accessible, verifies the signature and revokes an offer
     * @param _buyer The address of the buyer
     * @param _collection The collection that the token belongs to
     * @param _tokenId The ID of the token whose offer to revoke
     * @param _erc20 The address of the ERC20 token used in the offer
     * @param _deadline The time when the permission to revoke expires
     * @param _signature The signature that approves the revocation
     */
    function permitRevokeOffer(
        address _buyer,
        address _collection,
     ...
     * @dev The function is publicly accessible and accepts an offer
     * @param _buyer The address of the buyer
     * @param _erc20 The address of the ERC20 token used in the offer
     * @param _collection The collection that the token belongs to
     * @param _tokenId The ID of the token whose offer to accept
     */
    ...
    /**
     * @notice Accept an offer with a signature
     * @dev The function is publicly accessible, verifies the signature and accepts an offer
     * @param _seller The address of the seller
     * @param _buyer The address of the buyer
     * @param _erc20 The address of the ERC20 token used in the offer
     * @param _collection The collection that the token belongs to
     * @param _tokenId The ID of the token whose offer to accept
     * @param _deadline The time when the permission to accept expires
     * @param _signature The signature that approves the acceptance
     */
    function permitAcceptOffer(
        address _seller,
        address _buyer,
     ...
     * @param _saleId The id of the sale that has closed.
     */
    function handleSaleClosed(uint256 _saleId) external virtual override onlyMarketHub {
        IEscrow.Sale memory sale = marketHub.getEscrow().getSale(_saleId);

        // Change of owner only affects the buyers' offer. Other actors offers are still valid.
        address[] memory erc20s = marketHub.getERC20s();
        for (uint i = 0; i < erc20s.length; ++i) {
            address erc20 = erc20s[i];
            if (offers[sale.erc721][sale.tokenId][sale.buyer][erc20].price > 0) {
                emit OfferRevoked(sale.buyer, sale.erc721, sale.tokenId, erc20);
                delete offers[sale.erc721][sale.tokenId][sale.buyer][erc20];
            }
        }
    }
    ...
    function supportsInterface(bytes4 _interfaceId) public view virtual override returns (bool) {
        return _interfaceId == type(IOfferMarket).interfaceId || super.supportsInterface(_interfaceId);
    }

    /**
     * @notice Get an offer
     * @dev The function is publicly accessible and returns an offer
     * @param _collection The collection that the token belongs to
     * @param _tokenId The ID of the token for which to get the offer
     * @param _buyer The address of the buyer
     * @param _erc20 The address of the ERC20 token used in the offer
     * @return The offer
     */
     ...
     * @dev The function is internally accessible and creates an offer
     * @param _buyer The address of the buyer
     ...
     * @param _erc20 The address of the ERC20 token to use as payment
     * @param _price The price at which to offer the token
     * @param _payNow Whether the payment should be made immediately
     */
    function _offer(
        address _buyer,
        address _spender,
        ...
        require(marketHub.isAccountActive(_buyer), "Buyer is not registered or is haulted/banned");

        IColleCollection collection = marketHub.getCollection(_collection);
        address seller = collection.ownerOf(_tokenId);
        require(seller != _buyer, "Cannot buy your own product");
        uint256 usdcPrice = marketHub.getCurrencyByERC20(_erc20).getEstimatedUSDCValue(_price);
        require(usdcPrice >= marketHub.getMinUSDCPrice(), "Price must be above minimum price");
    ...
    /**
     * @notice Revoke an offer
     * @dev The function is internally accessible and revokes an offer
     * @param _buyer The address of the buyer
     * @param _collection The collection that the token belongs to
     * @param _tokenId The ID of the token whose offer to revoke
     * @param _erc20 The address of the ERC20 token used in the offer
    ...
    function _revokeOffer(address _buyer, address _collection, uint256 _tokenId, address _erc20) internal virtual {
        require(offers[_collection][_tokenId][_buyer][_erc20].price > 0, "Offer does not exist");
    ...
    /**
     * @notice Accept an offer
     * @dev The function is internally accessible and accepts an offer
     * @param _seller The address of the seller
     * @param _buyer The address of the buyer
     * @param _erc20 The address of the ERC20 token used in the offer
     * @param _collection The collection that the token belongs to
    ...
    function _acceptOffer(
        ...
        require(marketHub.isAccountActive(_seller), "Seller is not registered or is haulted/banned");

        IColleCollection collection = marketHub.getCollection(_collection);
        Offer memory currentOffer = offers[_collection][_tokenId][_buyer][_erc20];
        require(currentOffer.price > 0, "Offer does not exist");
        require(_seller == collection.ownerOf(_tokenId), "Only the token owner can call this function");
        _createSale(
            _seller,
            _buyer,
    ...
    /**
     * @dev Generates a hashed representation of a permit offer.
     * @param _buyer The address of the buyer.
     * @param _collection The address of the NFT collection.
     * @param _tokenId The identifier for an NFT.
     * @param _erc20 The address of the ERC20 token.
     * @param _price The price for the NFT.
     * @param _payNow Indicates whether payment will be made now.
     * @param _deadline The time at which the offer expires.
     * @return A bytes32 hash of the permit offer.
     */
    ...
    /**
     * @dev Generates a hashed representation of an off-chain payment offer.
     * @param _buyer The address of the buyer.
     * @param _collection The address of the NFT collection.
     * @param _tokenId The identifier for an NFT.
     * @param _erc20 The address of the ERC20 token.
     * @param _price The price for the NFT.
     * @param _deadline The time at which the offer expires.
     * @return A bytes32 hash of the off-chain payment offer.
     */
    ...
    /**
     * @dev Generates a hashed representation of a permit revoke offer.
     * @param _buyer The address of the buyer.
     * @param _collection The address of the NFT collection.
     * @param _tokenId The identifier for an NFT.
     * @param _erc20 The address of the ERC20 token.
     * @param _deadline The time at which the offer expires.
     * @return A bytes32 hash of the permit revoke offer.
     */
    ...
    /**
     * @dev Generates a hashed representation of a permit accept offer.
     * @param _seller The address of the seller.
     * @param _buyer The address of the buyer.
     * @param _erc20 The address of the ERC20 token.
     * @param _collection The address of the NFT collection.
     * @param _tokenId The identifier for an NFT.
     * @param _deadline The time at which the offer expires.
     * @return A bytes32 hash of the permit accept offer.
     */
