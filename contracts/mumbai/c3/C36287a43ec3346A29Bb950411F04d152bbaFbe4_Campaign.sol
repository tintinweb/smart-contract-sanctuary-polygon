...
contract Campaign is CampaignMetadata, CompareString {
    ...
    event CampaignUpdated(address advertiser, string status);
    ...
    function updateCampaign(uint256 campaignID_, string memory status_)
        external
    {
        ...
        emit CampaignUpdated(msg.sender, status_);
    }

    function isValidCampaign(uint256 campaignID_) public view returns (bool) {
        if (
            _exists(campaignID_) &&
            CompareString.compare(getInfo[campaignID_].status, "Active")
        ) return true;
