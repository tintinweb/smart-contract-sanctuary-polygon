...
contract FeeCollectHook {
    ...
    mapping(uint256 => FeeControl) public feeControls;
    ...
    function startup(
        ...
        (address feeToken, uint256 collectFee) = abi.decode(
            initialize,
            (address, uint256)
        );
        feeControls[ownerId] = FeeControl({
            feeToken: feeToken,
            feeReceiver: owner,
        ...
        onlyHookManager
    {
        ...
        FeeControl memory feeControl = feeControls[calleeId];
        ...
        if (feeControl.feeToken == address(0)) {
            ...
            if (refund > 0) {
                _transfer(feeControl.feeToken, sender, refund);
            }
        } else {
