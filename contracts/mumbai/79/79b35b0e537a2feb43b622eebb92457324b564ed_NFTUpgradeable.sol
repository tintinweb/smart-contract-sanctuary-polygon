...
contract NFTUpgradeable is
    ...
    OwnableUpgradeable,ReentrancyGuard
{
    using CountersUpgradeable for CountersUpgradeable.Counter;
    CountersUpgradeable.Counter private _nftIdCounter;
    uint256 public constant MAX_SUPPLY = 90000;
    uint256 public pricePresale = 0.19 ether;
    uint256 public priceSale = 0.25 ether;
    ...
    mapping(uint256 => bool) nftsURL;

  

      function initialize(string memory _theBaseURI, string memory _notRevealedURI) public initializer {
    __ERC721_init("InfinityTest", "INFINITYNFT");
    ...
    function changePricePresale(uint256 _pricePresale) external onlyOwner {
      ...
      function setWeb(uint256 _web) external onlyOwner {
        WEB = _web;
    }
    ...
    function revealNft(uint256 start, uint256 end) external onlyOwner {
    ...
    function setUpExchange() external onlyOwner {
        sellingStep = Steps.Exchange;
    }
    ...
    function stopPreSell() external onlyOwner {
        require(sellingStep == Steps.Presale, 'Presale Step');
        sellingStep = Steps.Before;
    }

    function stopSell() external onlyOwner {
        require(sellingStep == Steps.Sale, 'Sale Step.');
        sellingStep = Steps.Before;
    }

    function stopExchange() external onlyOwner {
        require(sellingStep == Steps.Exchange, 'Exchange Step.');
        sellingStep = Steps.Before;
    }
    ...
    function presaleMint(uint256 _ammount) external payable nonReentrant {
        ...
        require(whitelisted[msg.sender] == true, 'Only Whitelisted Users');
    ...
    function exhangeNFT(address[] calldata _users) public onlyOwner {
        require(sellingStep == Steps.Exchange, 'Sorry, Exchange not Active');
        uint256 numberNftSold = totalSupply();
        for (uint256 i = 0; i < _users.length; i++) {
             _safeMint(_users[i], numberNftSold + i);
