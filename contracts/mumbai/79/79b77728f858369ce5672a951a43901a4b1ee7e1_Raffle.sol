...
import {IRaffle, VRFCoordinatorV2Interface} from './interfaces/IRaffle.sol';
import {Raffle} from './Raffle.sol';
import {TicketTracker} from './TicketTracker.sol';
...
/**
 * Contract used to create raffles
 */
// cl configs: https://docs.chain.link/docs/vrf-contracts/#configurations
contract Raffle is
  IRaffle,
  Ownable,
  ERC721('RAFFLY', 'RFY'),
  ...
  ERC721Burnable
{
  ...
  VRFCoordinatorV2Interface public immutable CHAINLINK_COORDINATOR;

  // initializing to 1 as its cheaper to pass from 1 to 2 than from 0 to 1
  uint256 public tokenIdCounter = 1;
  bytes32 public vrfKeyHash; // The gas lane to use, which specifies the maximum gas price to bump to.
  uint256 public raffleFee;
  address public feeCollector;

  uint64 public clSubscriptionId;
  string public raffleUri;
  bool public stopProtocol = false;

  // raffle nft Id => raffle configuration
  mapping(uint256 => RaffleConfiguration) public _raffleConfigs;
  mapping(uint256 => uint256) public _vrfRequestIdToRaffleNftId;
  mapping(uint256 => bool) public _emergencyStop;

  constructor(
    uint256 raffleFee_,
    address feeCollector_,
    string memory raffleUri_,
    address clVRFCoordinator,
    bytes32 vrfKeyHash_
  ) VRFConsumerBaseV2(clVRFCoordinator) {
    raffleFee = raffleFee_;
    feeCollector = feeCollector_;
    raffleUri = raffleUri_;
    CHAINLINK_COORDINATOR = VRFCoordinatorV2Interface(clVRFCoordinator);
  ...
  {
    return _raffleConfigs[raffleId];
  }
  ...
  function getVRFRequestIdForRaffleNftId(uint256 raffleId)
    ...
    returns (uint256)
  {
    return _vrfRequestIdToRaffleNftId[raffleId];
  }

  // Cancel the subscription and send the remaining LINK to a wallet address.
  function cancelSubscription(address receivingWallet) external onlyOwner {
    CHAINLINK_COORDINATOR.cancelSubscription(clSubscriptionId, receivingWallet);
    clSubscriptionId = 0;
  }
  ...
  function create(
    ...
    uint256 _fullSellPrice,
    uint256 _discountedSellPrice,
    uint256 _maxTickets
  ) external returns (uint256) {
    require(stopProtocol == false, 'PROTOCOL_STOPPED');

    IERC721(_priceNFTAddress).safeTransferFrom(
      msg.sender,
      address(this),
      _priceNFTId
    );
    ...
    address tickets = address(
      new TicketTracker(tokenIdCounter, (_fullSellPrice / _maxTickets))
    );
    _raffleConfigs[tokenIdCounter] = RaffleConfiguration({
      raffleId: tokenIdCounter,
      priceNFTId: _priceNFTId,
      priceNFTAddress: _priceNFTAddress,
      expirationDate: _expirationDate,
      fullSellPrice: _fullSellPrice,
      discountedSellPrice: _discountedSellPrice,
  ...
  function getRaffleStateById(uint256 raffleId)
    public
    view
    returns (RaffleStates)
  {
    RaffleConfiguration memory raffleConfig = _raffleConfigs[raffleId];
    return getRaffleState(raffleConfig);
  }
  ...
  function getRaffleState(RaffleConfiguration memory raffleConfig)
    ...
    returns (RaffleStates)
  {
    if (_emergencyStop[raffleConfig.raffleId] == true)
      return RaffleStates.EXPIRED;
    if (raffleConfig.winner != address(0)) {
      return RaffleStates.FINISHED;
    } else if (
      TicketTracker(raffleConfig.ticketTracker).totalMinted() ==
      raffleConfig.maxTickets ||
      (getRaisedAmount(raffleConfig) > raffleConfig.discountedSellPrice &&
        raffleConfig.expirationDate > block.timestamp)
    ) {
      return RaffleStates.RAFFLE_SUCCESSFUL;
    } else if (raffleConfig.expirationDate > block.timestamp) {
      return RaffleStates.EXPIRED;
    } else {
  ...
  function getRaisedAmountById(uint256 raffleId) public view returns (uint256) {
    RaffleConfiguration memory raffleConfig = _raffleConfigs[raffleId];
    return getRaisedAmount(raffleConfig);
  }
    ...
    returns (uint256)
  {
    return
      TicketTracker(raffleConfig.ticketTracker).totalMinted() *
      TicketTracker(raffleConfig.ticketTracker).getPriceTicket();
  }
  ...
  function buyTickets(uint256 raffleNftId, uint256 ticketsAmount)
    ...
    payable
  {
    RaffleConfiguration memory raffleConfig = _raffleConfigs[raffleNftId];
    // Check state
    require(getRaffleState(raffleConfig) == RaffleStates.ACTIVE, 'NOT_ACTIVE');
    // Check not surpass the number max of tickets
    require(
      TicketTracker(raffleConfig.ticketTracker).totalMinted() + ticketsAmount <=
        raffleConfig.maxTickets,
      'MAX_TICKETS'
    );

    uint256 priceAmount = ticketsAmount *
      TicketTracker(raffleConfig.ticketTracker).getPriceTicket();

    require(msg.value == priceAmount, 'NOT_ENOUGT_ETH');

    TicketTracker(raffleConfig.ticketTracker).mint(msg.sender, ticketsAmount);

    emit IRaffle.NewTicket(raffleNftId, ticketsAmount, msg.sender);
  }
  ...
  function claimNFTBack(uint256 raffleNftId) external {
    ...
    require(getRaffleState(raffleConfig) == RaffleStates.EXPIRED, 'NO_WINNER');
    // Owner of the raffle
    require(ownerOf(raffleNftId) == msg.sender, 'NOT_THE_OWNER');
    // Return NFT
    IERC721(raffleConfig.priceNFTAddress).safeTransferFrom(
      address(this),
      msg.sender,
      raffleConfig.priceNFTId
    );

    // Burn this nft
    this.burn(raffleNftId);
    emit IRaffle.NFTBack(raffleNftId, getRaffleState(raffleConfig), msg.sender);
  }

  // Iterate all the NFTs claim amount back and burn
  function claimAmountTicketBack(uint256 raffleNftId, uint256[] memory tokensId)
    external
  {
    RaffleConfiguration memory raffleConfig = _raffleConfigs[raffleNftId];
    // Need to be expired to claim the money
    require(getRaffleState(raffleConfig) == RaffleStates.EXPIRED, 'NO_WINNER');
    uint256 ticketPrice = TicketTracker(raffleConfig.ticketTracker)
      .getPriceTicket();

    for (uint256 i = 0; i < tokensId.length; i++) {
      // Check the ownership of the tocken
      require(
        TicketTracker(raffleConfig.ticketTracker).ownerOf(tokensId[i]) ==
          msg.sender,
        'NOT_THE_OWNER'
      );
      // Return the ticket price to the owner
      _safeTransferETH(msg.sender, ticketPrice);
      // Burn the ticket used
      TicketTracker(raffleConfig.ticketTracker).burn(tokensId[i]);
      emit IRaffle.AmountBack(raffleNftId, ticketPrice, msg.sender);
    }
  }
  ...
  function redeemPriceNft(uint256 raffleNftId) external {
    RaffleConfiguration memory raffleConfig = _raffleConfigs[raffleNftId];
    require(getRaffleState(raffleConfig) == RaffleStates.FINISHED, 'NO_WINNER');
    // Owner of the raffle
    require(ownerOf(raffleNftId) == msg.sender, 'NOT_THE_OWNER');
    address winner = raffleConfig.winner;
    require(winner != address(0), 'NOT_ADDRESS_WINNER');

    uint256 raffleAmount = getRaisedAmount(raffleConfig);

    uint256 fee = (raffleAmount * raffleFee) / 100;
    // transfer winnings
    _safeTransferETH(ownerOf(raffleNftId), raffleAmount - fee);
    ...
    // transfer fee
    _safeTransferETH(feeCollector, fee);

    // Burn this nft
    this.burn(raffleNftId);
    // receiver:  field can be it will be in the case of success address of the winner or in expired the tickets owners.
    emit IRaffle.RedeemNFT(raffleNftId, RaffleStates.FINISHED, winner);
  }

  function chooseWinner(uint256 raffleNftId) external {
    RaffleConfiguration storage raffleConfig = _raffleConfigs[raffleNftId];

    require(
      getRaffleState(raffleConfig) == RaffleStates.RAFFLE_SUCCESSFUL,
      'NOT_ACTIVE'
    );

    // choose winner on chainlink vfr
    require(clSubscriptionId > 0, 'CL_VRF_SUBSCRIPTION_NOT_SET');
    require(raffleConfig.vrfRequestId > 0, 'VRF_ALREADY_REQUESTED');

    uint256 vrfRequestId = CHAINLINK_COORDINATOR.requestRandomWords(
      vrfKeyHash,
      clSubscriptionId,
    ...
    raffleConfig.vrfRequestId = vrfRequestId;
    _vrfRequestIdToRaffleNftId[vrfRequestId] = raffleNftId;
  }

  function emergencyStop(uint256 raffleId) external onlyOwner {
    _emergencyStop[raffleId] = true;
  }

  function stopAllProtocol() external onlyOwner {
    stopProtocol = true;
  }
  ...
  function _createNewSubscription() internal {
    clSubscriptionId = CHAINLINK_COORDINATOR.createSubscription();
    CHAINLINK_COORDINATOR.addConsumer(clSubscriptionId, address(this));
  }
  ...
  function fulfillRandomWords(
    ...
    uint256 raffleNftId = _vrfRequestIdToRaffleNftId[vrfRequestId];
    RaffleConfiguration storage raffleConfig = _raffleConfigs[raffleNftId];

    require(raffleConfig.winner != address(0), 'WINNER_ALREADY_CHOSEN');

    uint256 numberOfTicketsSold = TicketTracker(raffleConfig.ticketTracker)
      .totalSupply();
    // Assuming only one random word was requested.
    uint256 tokenIdwinner = (randomWords[0] % numberOfTicketsSold) + 1;

    raffleConfig.winner = TicketTracker(raffleConfig.ticketTracker).ownerOf(
      tokenIdwinner
    );

    emit IRaffle.Winner(raffleNftId, tokenIdwinner, raffleConfig.winner);
  }
...
contract TicketTracker is ERC721A {
  address internal raffleAddress;
  uint256 internal raffleId;
  uint256 internal priceTicket;

  constructor(uint256 _raffleId, uint256 _priceTicket)
    ERC721A('TicketTracker', 'TKT')
  {
    raffleAddress = msg.sender;
    raffleId = _raffleId;
    priceTicket = _priceTicket;
  }

  function mint(address sender, uint256 quantity) external payable {
    // `_mint`'s second argument now takes in a `quantity`, not a `tokenId`.
    _mint(sender, quantity);
  }

  function getRaffleAddress() external view returns (address) {
  ...
  function burn(uint256 tokenId) external {
    require(raffleAddress == msg.sender, 'ONLY_CAN_BURN_THE_RAFFLE');
    _burn(tokenId);
  }
  ...
  //     }
  // https://github.com/PatrickAlphaC/all-on-chain-generated-nft/blob/main/contracts/RandomSVG.sol
