...
contract Escrow {
    ...
    function withdraw(uint w_amount) isNgo public {
        require(w_amount <= amount, "IS/B");
        Txn memory n_tx = Txn(block.timestamp, ngo, address(this), w_amount, TxnType.DEBIT);
...
contract Bitgive {
    ...
    function withdrawFromCampaign(uint256 _id, uint256 _amt) public {
        Campaign storage campaign = campaigns[_id];
        campaign.e.withdraw(_amt);
    }
