...
interface IFarmController is IMarginGroup {
...
contract NaviUi is Ownable {
    ...
    function getOracle() public view returns (bytes[] memory out) {
        ...
        for (uint32 i = 0; i < length; i++) {
            if (oracle.hasPriceFeed(i))
                out[i] = abi.encode(oracle.tokenAddress(i), oracle.price(i));
            else out[i] = abi.encode(oracle.tokenAddress(i));
        }
    }
    ...
    function getAll(address account)
        public
        view
        returns (bytes[] memory groups, bytes[] memory farms)
    {
        groups = getGroup(account, controller);
        farms = getFarms(account);
    }
        ...
        returns (bytes[] memory out)
    {
        ...
        out = new bytes[](length);
        for (uint32 i = 0; i < length; i++) {
            (address item, ) = group.getMarginable(i);
            out[i] = getOne(account, item);
        }
    }
    ...
    function getOne(address account, address item)
        public
        view
        returns (bytes memory out)
    {
