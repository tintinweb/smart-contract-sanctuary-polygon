...
contract CCSwap {
    ...
    address public INATokenAddress;
    IERC20 public inaToken;
    constructor (address _INATokenAddrss){
        INATokenAddress = _INATokenAddrss;
        inaToken = IERC20(INATokenAddress);
    }
    ...
    function swap(address _fromCC, address _toCC, uint256 _amount) public returns(uint256){
        ...
        fromCC.transferFrom(msg.sender, address(this), _amount);
        uint256 inaAmount = fromCC.sellCC(_amount);
        inaToken.approve(address(toCC), inaAmount);
        uint256 swappedAmount = toCC.buyCC(inaAmount);
        toCC.transfer(msg.sender, swappedAmount);
...
contract INAToken is ERC20("INANI token", "INA"), Ownable {
    ...
    function finishPresale() onlyOwner public {
        require(block.timestamp > (privateSaleStartTime + privateSaleMaxDuration/86400), "within max duration!");
    ...
    function _beforeTokenTransfer(
        ...
        if (from == founderTeamWallet){
            require (block.timestamp > founderTeamReleaseTime/86400, "not allowed to transfer team and founder token yet");
        }
