        ...
        .`.    |         `.             /         :_,'.'
          \ `...\   _     ,'-.        .'         /_.-'
           `-.__ `,  `'   .  _.>----''.  _  __  /
               ...
               /_|.-'\ ,".             '.'`__'-( \
                 / ,"'"\,'               `/  `-.|" mh
EGT fork to send Vitalik a dragon :)
*/
import "./SuperSecretDragonUtils.sol";

pragma solidity ^0.8.12;

contract VTD is Context, IERC20, AccessControl {
    using SafeMath for uint256;
    using Address for address;

    bytes32 public constant ADMIN_WRITE = keccak256("ADMIN_WRITE");
    ...
    constructor() {
        ...
        _grantRole(ADMIN_WRITE, msg.sender);

        _name = "Vitalik The Dragon";
        _symbol = "VTD";
        ...
        //UNISWAP V2
        _setRouterAddress(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff); //shared router contract on ETH: 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D, on quickswap/polygon:0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff  
        _setOperatorsAddresses(
            payable(0xf0dbd59b214405FF0b2364dac77Ae14662CcfcDD), //it4i-deployer
            payable(0x81a387dDB3C2f119341c3e34D565Ab00E6e181Dc), //burner_2
            payable(0x61f629FCF26409f9F48C8758d2dA32D891dceb3a) //burner_3
        );
    ...
    function manualSwapAndLiquify() external onlyRole(ADMIN_WRITE) {
    ...
    function manualSwapAndLiquify(uint256 tokenAmountToSwap) external onlyRole(ADMIN_WRITE) {
        if (!inSwapAndLiquify && !inSplitShares) {
            uint256 contractTokenBalance = balanceOf(address(this)).sub(
                _deltaLPReserve
            );
    ...
    function manualWithdrawTokens(uint256 tokenAmount) external onlyRole(ADMIN_WRITE) {
    ...
    function distributeTax() external onlyRole(ADMIN_WRITE) {
        _distributeTax();
    }
    ...
    function includeInFee(address account) external onlyRole(ADMIN_WRITE) {
        excludedFromFee[account] = false;
    }

    function excludeFromFee(address account) external onlyRole(ADMIN_WRITE) {
    ...
    function enableBlockedBotAccount(address account) external onlyRole(ADMIN_WRITE) {
        _lockedBot[account] = false;
    }

    function blockBotAccount(address account) external onlyRole(ADMIN_WRITE) {
     ...
     *
     */
    function setDevelopmentAddress(address payable _developmentAddress) external onlyRole(ADMIN_WRITE) {
    ...
    function setBuyBackShare(uint256 buyBackShare) external onlyRole(ADMIN_WRITE) {
    ...
    function setMarketingShare(uint256 marketingShare) external onlyRole(ADMIN_WRITE) {
    ...
    function setDevelopmentShare(uint256 developmentShare) external onlyRole(ADMIN_WRITE) {
    ...
    function setLPShare(uint256 lpShare) external onlyRole(ADMIN_WRITE) {
    ...
    function setBuyTax(uint256 buyTax) external onlyRole(ADMIN_WRITE) {
    ...
    function setSellTax(uint256 sellTax) external onlyRole(ADMIN_WRITE) {
     ...
     *
     */
    function setSwapAndLiquifyEnabled(bool _enabled) external onlyRole(ADMIN_WRITE) {
        swapAndLiquifyEnabled = _enabled;
        emit SwapAndLiquifyEnabledUpdated(_enabled);
    }

    function setAutoSplitSharesEnables(bool _enabled) external onlyRole(ADMIN_WRITE) {
        autoSplitShares = _enabled;
    }

    function enableUniswap() external onlyRole(ADMIN_WRITE) {
        require(!enableUniSwap, "Already enabled!");
        enableUniSwap = true;
        emit EnabledUniswap();
    }

    function setAcceptedSlippage(uint256 accepted) external onlyRole(ADMIN_WRITE) {
        require(accepted <= 9, "Cannot set above 9");
        acceptSlippageReduceFactor = accepted;
    }

    function getAcceptedSlippage() external view onlyRole(ADMIN_WRITE) returns (uint256) {
        return acceptSlippageReduceFactor;
    }

    function setAcceptedFeeOnAdd(uint256 accepted) external onlyRole(ADMIN_WRITE) {
        acceptFeeOnAddLP = accepted;
    }

    function getAcceptedFeeOnAdd() external view onlyRole(ADMIN_WRITE) returns (uint256) {
     ...
     *
     */
    function setRouterAddress(address newRouter) external onlyRole(ADMIN_WRITE) {
        _setRouterAddress(newRouter);
    }

    function totalDevelopmentTaxCollected() external view onlyRole(ADMIN_WRITE) returns (uint256) {
    ...
    function totalLPTaxCollected() external view onlyRole(ADMIN_WRITE) returns (uint256) {
        return _lpTaxCollected;
    }

    function totalTaxCollected() external view onlyRole(ADMIN_WRITE) returns (uint256) {
        return
            _marketingTaxCollected +
            _developmentTaxCollected +
            _buyBackTaxCollected +
