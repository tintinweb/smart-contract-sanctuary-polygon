...
contract ERC165Upgradeable is IERC165Upgradeable {
...
contract ERC1155Upgradeable is  ERC165Upgradeable, IERC1155Upgradeable, IERC2981Upgradeable,OwnableUpgradeable {
   ...
   mapping(uint256 => string) private _tokenURIs;
    ...
    string private _name = "TariNFT";

    string private _symbol = "TARI";
    ...
    function _setTokenURI(uint256 tokenId, string memory __uri) internal {
        require(_exists(tokenId), "ERC1155Metadata: URI query for nonexistent token");
        _tokenURIs[tokenId] = __uri;
    }
...
contract TariNFT1155 is ERC1155Upgradeable {
    ...
    function init(address _operator)external initializer{
               operator=_operator;
                __Ownable_init();
    }
    ...
    function mint(address creator, string memory _uri, uint256 supply, uint96 fee)  public returns(uint256, bool){
        require(msg.sender == operator,"ERC1155: caller doesn't have operator Role");
        require(!isDuplicate(_uri), "Minting: Duplicate Minting");
        usedURIs[_uri] = true;
        uint256 tokenId = tokenCounter;
        tokenURIs[tokenId]=_uri;
        _mint(creator, tokenId, supply, _uri);
