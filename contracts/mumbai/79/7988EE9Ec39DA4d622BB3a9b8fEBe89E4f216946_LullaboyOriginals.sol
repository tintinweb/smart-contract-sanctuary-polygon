...
import "./libs/ERC721Rewards.sol";

/// @custom:security-contact [email protected]
contract LullaboyOriginals is ERC2981, ERC721Rewards, ERC721URIStorage, Pausable {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;
    uint256 private _currentSupply = 1;
    uint256 private _nftCost = 0.01 ether;
    uint256 public total = 25;
    address[] private whitelist;
    

    constructor() ERC2981("LullaboyOriginals", "LBO", msg.sender) {
        _safeMint(msg.sender, 0);
        addMetadata(0, "ipfs://QmeSjSinHpPnmXmspMjwiXyN6zS4E9zccariGR3jxcaWtq/0");
    ...
    function setMaxMint(uint256 newLimit) public onlyOwner {
        require(_currentSupply < newLimit, "Limit is too low");
        _currentSupply = newLimit;
    }
    ...
    function addMetadata(uint256 tokenId, string memory uri) public whenNotPaused onlyOwner {
    ...
    function _isWhitelisted(address minter) private view returns (bool,uint){
        for (uint i = 0; i < whitelist.length; i++){
            if (minter == whitelist[i]) {
                return (true,i);
            }
        }
        return (false,whitelist.length);
    }

    function _deleteWhitelisted(uint idx) private {
    ...
    function safeMint() public payable whenNotPaused {
        ...
        require(tokenId < _currentSupply, "Minting not yet available.");
        require(msg.value >= _nftCost, "Not enough Ether.");
        (bool canMint, uint indexAddr) = _isWhitelisted(msg.sender);
        require(canMint,"Not in whitelist");
        _updateMint(tokenId, msg.value);
        _deleteWhitelisted(indexAddr);
        _safeMint(msg.sender, tokenId); //Triggers _beforeTokenTransfer
        _tokenIdCounter.increment();
    }
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId)
        ...
        override
    {
        _updateRewards(_tokenIdCounter.current());
        uint256 amount = getRewardByToken(tokenId);
        flushRewards(tokenId);
        if (amount > 0) {
    ...
    */
    ...
    function claimRewards(uint256 tokenId) public whenNotPaused override {
        require(ownerOf(tokenId)==msg.sender, "Not allowed");
        _updateRewards(_tokenIdCounter.current());
        uint256 amount = getRewardByToken(tokenId);
        console.log(amount);
        flushRewards(tokenId);
        require(amount > 0, "Not enough rewards");
    ...
    function fillWhitelist(address[] memory addrs) public whenNotPaused onlyOwner { 
        for (uint i = 0; i < addrs.length; i++){
            whitelist.push(addrs[i]);
        }
    }
 ...
 */
contract ERC2981 is ERC721, IEIP2981, Ownable {
    ...
    constructor(
        ...
        defaultRoyalty = TokenRoyalty(_royaltyRecipient, 1000);
    }
...
abstract contract ERC721Rewards {
    ...
    uint256 private _etherMint = 0 ether;
    uint256 private _etherTotal = 0 ether;
    mapping(uint256 => uint256) private _tokenRewards;

    function _updateMint(uint256 currentSupply, uint256 payment) internal {
        _etherMint += payment;
        _etherTotal += payment;
        _updateRewards(currentSupply);
    }

    function _updateRewards(uint256 currentSupply) internal {
        if ( address(this).balance <= _etherTotal ) {
            return;
        }

        uint256 newFees = (address(this).balance - _etherTotal) / currentSupply;
        for (uint256 j = 0; j < currentSupply; j++) {
            _tokenRewards[j] += newFees;         
        }
        _etherTotal = address(this).balance;
    }

    function flushRewards(uint256 tokenId) internal  {
        _tokenRewards[tokenId] = 0;
    }

    function getRewardByToken(uint256 tokenId) public view returns (uint256) {
        return _tokenRewards[tokenId];
    }

    function claimRewards(uint256) public virtual;

    function _ownerWithdraw() internal {
        require(_etherMint > 0, "Nothing to withdraw");
        uint256 amount = _etherMint;
        _etherTotal -= _etherMint;
        _etherMint = 0;
        (bool success, ) = payable(msg.sender).call{value: amount}("");
        require(success, "Transfer failed.");
    }
