...
/*
  ...
  $$ |\$$$ |$$ |         $$ | \____$$\       $$ |     $$  __$$ | $$ |$$\ $$  __$$ |$$ | $$ | $$ |
  $$ | \$$ |$$ |         $$ |$$$$$$$  |      $$$$$$$$\\$$$$$$$ | \$$$$  |\$$$$$$$ |$$ | $$ | $$ |
  \__|  \__|\__|         \__|\_______/       \________|\_______|  \____/  \_______|\__| \__| \__|
...
interface ITokenERC721 is IERC721Upgradeable {
    ...
    /// @dev Emitted when an account with MINTER_ROLE mints an item for the slotmachine NFT.
    event ItemsMinted(uint256 indexed itemIdMinted, bytes onchainItem);
...
/**
 *  `Primary Sale` is a contract extension to be used with any base contract. It exposes functions for setting and reading
...
/**
 *  `Royalty` is a contract extension to be used with any base contract. It exposes functions for setting and reading
...
library FeeType {
    uint256 internal constant PRIMARY_SALE = 500;
    ...
    ITokenERC721
{
    ...
    bytes32 private constant MODULE_TYPE = bytes32("NFTs_Latam_Slot");
    ...
    constructor() initializer {
        ...
        __EIP712_init("NFTs_Latam_Slot", "2");
    ...
    function buildNFT(
        ...
        require(
            ...
            "Slot id need to be different of 0."
        );
        ...
        parts[1] = string(onchainItems[_slotId1]); ///data base64 of the image
    ...
    function changeUseState(uint256 _tokenId, uint16[] memory slotsIds)
        ...
        returns (string memory)
    {
        require(_ownerOf(_tokenId) != address(0), "No claimet yet");
        require(!useState[_tokenId], "You spent your slots opportunity!.");
        require(
            slotsIds[0] > 0 &&
    ...
    function claimPrize(uint256 _tokenId) external returns (string memory) {
        require(_ownerOf(_tokenId) == _msgSender(), "No claimet yet");
        require(useState[_tokenId], "First use your slots opportunity!.");

        //prizesRegister[_tokenId];
    ...
    function getClaimState(uint256 _tokenId) public view returns (bool) {
        require(_ownerOf(_tokenId) != address(0), "No claimet yet");
        return claimNFT[_tokenId];
    }
        ...
        returns (uint256 tokenIdToMint)
    {
        ...
        //string memory json = string(abi.encodePacked('{"name": "', name(), '#', toString(tokenIdToMint), '", "description": "Tulip generated and stored on chain.", "image": "data:image/svg+xml;base64,', _onchain, '"}'));

        //onchain[tokenIdToMint] = bytes(base64Decode(_onchain));
        prizesRegister[tokenIdToMint] = [0, 0, 0];
        useState[tokenIdToMint] = false;
        claimNFT[tokenIdToMint] = false;
        ...
        returns (uint256 itemIdToMint)
    {
        ...
        //onchain[itemIdToMint] = bytes(base64Decode(_onchain));
    ...
    function collectPrice(MintRequest memory _req) internal {
        ...
        (address nlFeeRecipient, uint256 nlFeeBps) = nftslatamFee.getFeeInfo(
            address(this),
            FeeType.PRIMARY_SALE
        );
        uint256 nlFee = (totalPrice * nlFeeBps) / MAX_BPS;
    ...
    event tokenClaimed(uint256 indexed index);
    event tokenUseState(uint256 indexed index);
}
