...
interface EthermonStakingInterface {
    ...
    function updateTokenData(bytes calldata _data, uint256 _timeElapsed)
        external;
}

interface EthermonStakingDumpInterface {
    function dumpStaker(bytes calldata _data) external;
...
contract EthermonStaking is EthermonStakingBasic {
    ...
    address public stakingDumpContract;
    ...
    function depositeTokens(
        ...
        data.level = _level;
        uint256 rarity = weight.getClassWeight(data.classId);
        data.validTeam = 1;
    ...
    function updateTokens(
        ...
        stakingData.updateTokenData(encoded, timeElapsed);
    ...
    function dumpStakingData(uint256 _lockId) external onlyModerators {
        EthermonStakingDumpInterface stakingDumpContract = EthermonStakingDumpInterface(
                stakingDumpContract
            );
        EthermonStakingInterface stakingData = EthermonStakingInterface(
            stakingDataContract
        ...
        TokenData memory data = stakingData.getTokenData(_lockId);
        require(
            data.lockId != 0 && data.owner != address(0),
            "Token Data is invlid"
        );
        bytes memory encoded = abi.encode(data);
        stakingDumpContract.dumpStaker(encoded);
        stakingData.removeTokenData(data);
    }

    function fetchStaking(uint256 _lockId, address _owner)
        external
        onlyModerators
    {
        require(
            _lockId > 0 && _owner != address(0),
            "LockID or Owner address is wrong"
        );
        EthermonStakingDumpInterface stakingDumpContract = EthermonStakingDumpInterface(
                stakingDumpContract
            );
        ...
        TokenData memory data = stakingDumpContract.fetchStaker(
            _lockId,
            _owner
        );
        require(
        ...
        stakingData.updateTokenData(encoded, 0);
    }
    ...
    function withdrawEmon(address _sendTo) external onlyModerators {
        //Moderator withdraw will only take emons deposite by mods;
