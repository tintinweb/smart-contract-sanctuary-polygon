...
contract PetInteractionHandler is HSystemChecker, Multicall {
    ...
    constructor(address itemFactoryContractAddress, address systemCheckerContractAddress)
    ...
    function interact(
        ...
        require(
            _petInteractions[petTokenId].length < _finalFormInteractions,
            'PI 100 - Pet has reached final form'
        );

        // Save interaction data
        _petInteractions[petTokenId].push(
            InteractionStruct(from, uint48(itemTokenId), uint48(block.timestamp))
        );
    ...
    function _incrementDailyInteractionCount(uint256 tokenId) internal {
        ...
        // This means the interaction counter can be reset as it is a new day
            ...
            data.timestamp =
                data.timestamp +
                (((currentTime - data.timestamp) / DAY_IN_SECONDS) * DAY_IN_SECONDS);
            data.dailyInteractionCount = 1;
        }
        ...
        // of the daily allowance
            ...
            require(
                data.dailyInteractionCount < _dailyInteractionAllowance,
                'PI 101 - Pet has exceeded their daily interaction allowance'
            );
    ...
    function getDailyInteractionCount(uint256 tokenId)
        external
        view
        returns (uint256 dailyInteractionCount)
    {
    ...
    function setStageInteractions(
        ...
        require(
            blobOneInteractions < blobTwoInteractions,
            'PI 400 - Blob one must come before blob two'
        );
        require(
            blobOneInteractions < finalFormInteractions,
            'PI 401 - Blob one must come before final form'
        );
        require(
            blobTwoInteractions < finalFormInteractions,
            'PI 402 - Blob two must come before final form'
        );
        ...
        emit LogSetStageInteractionsEvent(
            blobOneInteractions,
            blobTwoInteractions,
            finalFormInteractions
        );
    }
