 ...
 */
contract DarkNFT is Context, AccessControlEnumerable, ERC721Enumerable {
    ...
    /// @notice Get level of a specific ID
    /// @param tokenId The ID of the NFA to check
    /// @return (uint)
    function getTokenLevel(uint256 tokenId) external view returns (uint32) {
        NFADetails memory currentNFADetails = getNFADetailsById[tokenId];
        return currentNFADetails.level;
    }
    ...
    function mint(
        ...
        _mint(to, tokenId);
        getNFADetailsById[tokenId] = NFADetails(
            name,
            level,
            attack,
            luck,
