...
contract UnikNFTMarketPlace is ownable{
    ...
    function createToken(string memory creator, string memory tokenURI, uint256 price, uint tier, uint num,  uint _time) public onlyOwner returns (uint) {  
        ...
        for(uint i = 0; i < num; i++)
        {
          ...
          createMarketItem(tokenURINums[tokenURI] + 1,newTokenId, price, flag, _tokenURI, _time);

          mintedCount++;
          mintedAddr = msg.sender;
        }

        tokenURINums[tokenURI] += num;
        return num;
    }

    function getMintedCount() public view returns (uint256 count,address addr){
      NFTToken NftTkn = NFTToken(NftAddr);
    ...
    function createMarketSale(
      uint256 [] calldata tokenIds,address token_to
              ...
              if(idToMarketItem[tokenId].isMarket == true){
                if(pricefrom >= price){
                  ...
                  if(token_to.balance >= price){
                    ...
                    emit MarketTrx(tokenId,seller,token_to,price,
                                  "Sold",
                                  block.timestamp
                    );
                  }
    ...
    /* Returns all NFT under an Influencer */
    function fetchNFTItemsByInfluencer(string memory _influencer) public view returns (MarketItem[] memory) {
        NFTToken NftTkn = NFTToken(NftAddr);
