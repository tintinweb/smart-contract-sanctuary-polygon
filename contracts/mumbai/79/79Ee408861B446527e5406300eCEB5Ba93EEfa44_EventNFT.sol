...
contract EventNFT {
    ...
    function _deploy()
        ...
        returns (ERC721Psi)
    { 
        ERC721Psi contract_ = new ERC721Psi(address(this)); // the number of salt determines the address of the contract that will be deployed
        return contract_;
    }

    function _deployWithComputedAddress(bytes32 signatureA, bytes32 signatureB)
        public
        returns (ERC721Psi)
    {
        bytes32 salt = keccak256(
            abi.encodePacked(signatureA, signatureB)
        );
        ERC721Psi contract_ = new ERC721Psi{salt: bytes32(salt)}(address(this)); // the number of salt determines the address of the contract that will be deployed
        return contract_;
    }
