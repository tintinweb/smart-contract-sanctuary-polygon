...
contract maticTest is ERC721Enumerable, Ownable, ERC721URIStorage {
    using Counters for Counters.Counter;
    using SafeCast for int256;
    using SafeMath for uint256;

    Counters.Counter private _tokenIdCounter;
    AggregatorInterface internal maticUSDAPI;

    int256 private fiveLetter= 6;
    int256 private fourLetter= 25;
    int256 private threeLetter= 95;
    int256 private threeYearsDiscount= 10;
    int256 private fiveYearsDiscount= 30;
    int256 private tenYearsDiscount= 50;
    ...
    constructor() ERC721("TEST1 Domain Name Service", "TDNS") {
        maticUSDAPI = AggregatorInterface(0xAB594600376Ec9fD91F8e885dADF0CE036862dE0);
    }
    ...
    function maticUSD() public view returns (int256) {
        return maticUSDAPI.latestAnswer();
    }
    ...
    function renew(string memory domainName, uint256 year) public payable{
        ...
        require((msg.value * 10 ** 12) >= (price * tolerance) / 100 , "Check register price. Error #32");
    ...
    function ownerMint(address to, string memory domainName, uint256 year) public onlyOwner {
        ...
        string memory metadataURI = string(abi.encodePacked("https://ferma2exchange.com/metadata/json/", tokenIdStr));
    ...
    function register(string memory domainName , uint256 year) public payable{
        ...
        require(checkDomainAvaliable(domainName) == true , "Domain unavaliable (Has been registered). Error #41");
        uint256 price = domainPrice(domainName, year);
        require((msg.value * 10 ** 12) >= (price * tolerance) / 100 , "Check register price. Error #3");
        uint256 tokenId = _tokenIdCounter.current();
    ...
    function domainPrice(string memory domainName, uint256 year) public view returns(uint256){
        ...
        if (lenCount <= 3){
            domainPriceValue = ((threeLetter * 10 ** 20) / maticUSD()).toUint256();
        }
        else if(lenCount == 4){
            domainPriceValue = ((fourLetter * 10 ** 20) / maticUSD()).toUint256();
        }
        else if(lenCount >= 5){
            domainPriceValue = ((fiveLetter * 10 ** 20) / maticUSD()).toUint256();
        }
        uint256 price = domainPriceValue * year - (domainDiscountValue > 0 ? (domainPriceValue * domainDiscountValue) / 100 : 0);
        return price;
    }

    function domainDiscount(uint256 year) public view returns(uint256){
        if (year == 3){
            return threeYearsDiscount.toUint256();
        }
        else if(year == 5){
            return fiveYearsDiscount.toUint256();
        }
        else if(year == 10){
            return tenYearsDiscount.toUint256();
        }
        return 0;
    }
    ...
    function changeDiscount(int256 threeYears , int256 fiveYears , int256 tenYears) public onlyOwner{
        if (threeYears != 0){
