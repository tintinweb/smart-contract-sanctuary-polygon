    ...
    ReentrancyGuard
{
    ...
    function initialize(address _trustForwarderAddress, address _capeToken) public initializer {
    ...
    modifier marketplaceCompliance(uint256 _amount, uint256 _tokenid) {
        ...
        require(IERC1155(address(this)).balanceOf(msg.sender, _tokenid) >= _amount,
    ...
    function updateSalePrice(
        ...
        require(IERC1155(address(this)).balanceOf(msg.sender, _tokenid) >=
                        _amount,
            "ERC1155 token balance is not sufficient for the seller.."
        );
        require(_amount > 0, "Amount should be not less than zero");
    ...
    {
        bytes memory data = "";
        require(IERC1155(address(this)).balanceOf(msg.sender, _tokenid) >= 1,
            "ERC1155 token balance is not sufficient for the seller.."
        );
        // require(
        //     salesList[_listingId].owner == msg.sender,
    ...
    function getItemPrice(uint256 _itemId) public view returns (uint256) {
        uint256 fee = salesList[_itemId].amount *
            ((feePercent / 1000000000000000000) / 100);
        return (salesList[_itemId].amount + fee);
    }
