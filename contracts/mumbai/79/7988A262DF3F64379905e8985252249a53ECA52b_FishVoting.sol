...
contract FishVoting is Ownable, GelatoRelayContext {
    //  the Currency
    struct tokenInfo {
        IERC20 payToken;
        uint256 voteRate;
    ...
    // mapping((uint256 => address) => bool) public voted;

    uint256 public voters;

    uint256 public singleVote;

    uint256 public doubleVote;
    ...
    function addFeeToken(IERC20 _payToken, uint256 _voteRate) public onlyOwner {
        AllowedFee.push(tokenInfo({payToken: _payToken, voteRate: _voteRate}));
    }
    ...
    // Function to set Timer for voting

    function startVotingTimer(uint256 durationMinutes) public onlyOwner {
        votingEnd = block.timestamp + (durationMinutes * 1 minutes);
    }

    // event votedEvent(uint256 indexed _proposalsId);

    // Function to Add Proposal / Options
    ...
    // Function to reset Proposal

    function resetProposals() public onlyOwner {
        proposalsCount = 0;
        votingEnd = 0;
        // for(range =1; range <= proposalsCount; range++ ){
        //     proposals[range] = Proposal(0, "", 0, "", "");
        // }
        for (range = 0; range < voters; ) {
            voted[voterList[range]] = false;
            unchecked {
                range++;
            }
        }
    ...
    // Function to Perform Voting operation
    function vote(
        uint256 proposalsId,
        uint256 _proposalsId,
        uint256 _pid,
        uint256 _voteFee,
        ...
        _transferRelayFee();
        tokenInfo memory tokens = AllowedFee[_pid];
        IERC20 payToken = tokens.payToken;
        address orgSender = _verifyMessage(_hashedMessage, _v, _r, _s);

        require(!voted[msg.sender], "Owner not allowed to Vote");
        require(
            payToken.balanceOf(orgSender) >= _voteFee,
            "Not enough Tokens to complete transaction."
        );

        require(votingEnd != 0, "Voting yet to be Started");
        require(block.timestamp <= votingEnd, "Voting time is over");
        require(!voted[msg.sender]);
        voted[msg.sender] = true;
        voterList[voters] = msg.sender;

        if (_proposalsId <= 0 && proposalsId > 0) {
            require(proposalsId > 0 && proposalsId <= proposalsCount);
            proposals[proposalsId].voteCount++;
            singleVote++;
        } else {
            // require(_proposalsId > 0 && _proposalsId <= proposalsCount);
            proposals[proposalsId].voteCount++;
            proposals[_proposalsId].voteCount++;
            doubleVote++;
        }
        voters++;
        // emit votedEvent(_proposalsId);
    }

    // Function to verify the encoded message
    ...
    function winningProposal() public view returns (uint256) {
        ...
        uint256 winningProposals = 0;
        for (uint256 _range = 0; _range < proposalsCount; ) {
            if (proposals[_range].voteCount > winningVoteCount) {
                winningVoteCount = proposals[_range].voteCount;
                winningProposals = _range;
            }
            unchecked {
                _range++;
            }
    ...
    // Function to withdraw ERC20 tokens from the Voting Contract
    function withdraw(uint256 _pid) public payable onlyOwner {
        tokenInfo storage tokens = AllowedFee[_pid];
        IERC20 payToken = tokens.payToken;
