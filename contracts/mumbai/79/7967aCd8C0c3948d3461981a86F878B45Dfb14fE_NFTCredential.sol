...
contract NFTCredential is ERC721A, Ownable {
        ...
        returns (string[] memory)
    {
        ...
        for (uint i = 0; i <= total_token_num; i++) {
            if (_exists(i)) {
                address _tokenOwner = ownerOf(i);
                if (_msgSender() == _tokenOwner) {
                    string memory json = generateNFTInfo(i);
                    nfts[nft_num] = json;
                    nft_num++;
                }
            }
        ...
        returns (string[] memory)
    {
        ...
        for (uint i = 0; i <= total_token_num; i++) {
            if (_exists(i)) {
                if (!getShowFlg(i) || _msgSender() == ownerOf(i)) {
                    continue;
                }

                uint list_length = _showAddressList[i].length;
                address[] memory addressList = new address[](list_length);
                addressList = _showAddressList[i];
                
                for (uint j = 0; j < addressList.length; j++) {
                    if (_msgSender() == addressList[j]) {
                        string memory json = generateNFTInfo(i);
                        nfts[nft_num] = json;
                        nft_num++;
        ...
        returns (string[] memory)
    {
        ...
        for (uint i = 0; i <= total_token_num; i++) {
            if (_exists(i)) {
                address _tokenCreator = getCreator(i);
                if (_msgSender() == _tokenCreator) {
                    string memory json = generateNFTInfo(i);
                    nfts[nft_num] = json;
                    nft_num++;
                }
    ...
    function generateNFTInfo(
        uint256 tokenId
    ) internal view returns (string memory) {
        bool showflg = getShowFlg(tokenId);
        string memory strShowFlg = "0";
        if (showflg) {
            strShowFlg = "1";
        }
