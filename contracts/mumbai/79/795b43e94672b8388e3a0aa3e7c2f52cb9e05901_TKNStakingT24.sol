...
contract TKNStakingT24 is Ownable {
    ...
    uint256 public allUsersTotalStakeAmount;
    // total locked weight across all users
    uint256 public allUsersTotalStakeWeight;
    ...
    BasicUserInfo[] private listOfStakers;
    ...
    constructor(address _token, uint256 _rewardPerBlock, uint256 _startBlock) {
        ...
        rewardBonusWeight[0] = 0; // 14 day lock 100% rewards
        rewardBonusWeight[1] = 10; // 30 day lock 110% rewards
        rewardBonusWeight[2] = 25; // 90 day lock 125% rewards
        rewardBonusWeight[3] = 60; // 180 day lock 160% rewards
        lockDurations[0] = 14;
        lockDurations[1] = 30;
        lockDurations[2] = 90;
        lockDurations[3] = 180;
    ...
    function balanceOf(address _user) external view returns (uint256) {
        return userInfo[_user].tokenAddrAmount;
    }
    ...
    function getAllUsers() external view returns (BasicUserInfo[] memory) {
        require(getUsersAllowed, "Get users not allowed");
        return listOfStakers;
    }

    // Returns indexed user
    function getUserByIndex(uint256 _index) external view returns (BasicUserInfo memory) {
        require(getUsersAllowed, "Get users not allowed");
        return listOfStakers[_index];
    }

    // Returns users indexed from lower and upper bounds specified
    function getUsersInRange(uint256 _lower, uint256 _upper) external view returns (BasicUserInfo[] memory) {
        require(getUsersAllowed, "Get users not allowed");
        require(_lower < _upper && _lower >= 0);
        BasicUserInfo[] memory users = new BasicUserInfo[]((_upper) - (_lower) + 1);
        for (uint256 i = _lower; i < _upper; i++) {
            users[i] = listOfStakers[i];
        }
        return users;
    }
    ...
    function getPendingRewardOf(address _staker, uint256 _depositId) external view returns(uint256) {
        ...
        uint256 _accTokenPerUnitWeight = accTokenPerUnitWeight + (tokenReward * MULTIPLIER) / allUsersTotalStakeWeight;
    ...
    function _sync() internal {
        ...
        uint256 _weightLocked = allUsersTotalStakeWeight;
        if (_weightLocked == 0) {
        ...
        uint256 surplusToken = token.balanceOf(rewardVault) - allUsersTotalStakeAmount - unclaimedTokenRewards;
    ...
    function _stake(address _staker, uint256 _amount, uint256 _lockMode) internal {
        ...
        if (user.indexPosition == 0) // Set the user index position so we can track user's basic info later
        {
            user.indexPosition = (listOfStakers.length - 1);
        }

        // If user has no stake prior, then create a basic user info entry for enumerable rewards reading/tracking (i.e. for Remnant off-chain backend or partnered games to give in-game rewards for staking)
        if (user.deposits.length == 1)  // Condition: User has no stake prior to this stake
        {
            BasicUserInfo memory basicUserInfo =
                BasicUserInfo({
                    userAddress: address(_staker),
                    totalStakeWeighted: stakeWeight,
                    indexPosition: (listOfStakers.length - 1)
                });

            listOfStakers.push(basicUserInfo);  // Action: Add enumerable staking trackability
        }
        else if (user.deposits.length > 1)  // Condition: User has stakes already
        {                                   // Action: Update total stake by adding to the amount
            listOfStakers[user.indexPosition].totalStakeWeighted += stakeWeight;
        }

        user.tokenAddrAmount += _amount;
        user.totalWeight += stakeWeight;

        // update global variable
        allUsersTotalStakeWeight += stakeWeight;
        allUsersTotalStakeAmount += _amount;
    ...
    function _unstake(address _staker, uint256 _depositId, bool _sendRewards) internal {
        ...
        listOfStakers[user.indexPosition].totalStakeWeighted -= _weight; // update enumerable record for off-chain reading

        // update global variable
        allUsersTotalStakeWeight -= _weight;
        allUsersTotalStakeAmount -= _amount;
