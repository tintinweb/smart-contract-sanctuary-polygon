...
contract protoKol is iProtoKol {
   ...
   */
    function claimPreSaleTokens(
        ...
        if (campaigns[_campaignId].vestingData.isVestingInEnabled) {
            ...
            console.log("726 eligibleRwd", eligibleRwd);
   ...
   */
    function _blackListedKOLFundsHandling(
        ...
        uint256 balance = campaigns[_campaignId].campaignData.presaleAmount; //IPST(preSale).balanceOf(address(this)); //change here1
        if (balance < _reward) {
            if (balance > 0) {
