...
interface ILockdropPhase1 {
    ...
    function calculateRewardRates() external;

    function calculateTotalReward(
        address _wallet
    ) external view returns (uint256);

    function toClaim(address _wallet) external view returns (uint256[] memory);
    ...
    function calculateAndStoreTotalReward(
        address _wallet
    ) external returns (uint256);
}
...
contract LockdropPhase1 is ILockdropPhase1, Lockdrop {
    ...
    bool internal rewardRateCalculated_ = false;
    ...
    uint256[] public lockingExpirationTimestamps;

    LockingToken[] public lockingTokens;
    ...
    mapping(uint32 => uint256) public rewardPerPointInTimeRange;
    ...
    constructor(
        ...
        notZeroAddress(address(_phase2))
        notZeroAddress(address(_vesting))
    {
        ...
        for (uint256 i = 0; i < lockingExpirationTimestamps.length; ++i) {
            if (prev >= lockingExpirationTimestamps[i]) {
                revert WrongExpirationTimestamps();
            }
            prev = lockingExpirationTimestamps[i];
        }
        lockingExpirationTimestamps = _lockingExpirationTimestamps;
        ...
        for (
            ...
            lockingTokenId < _lockingTokens.length;
            ++lockingTokenId
        ) {
    ...
    function totalPoints() public view override returns (uint256) {
        return totalPointsInRound[0];
    }
        ...
        notZeroTokens(_value)
    {
        ...
        for (
            uint32 stampId = 0;
            stampId <= allocation.unlockTimestampIndex;
            ++stampId
        ) {
        ...
        returns (uint256)
    {
        ...
        for (
            ...
            for (
                uint32 timeIndex = 0;
                timeIndex <= unlockTimestampIndex;
                ++timeIndex
            ) {
                reward +=
                    rewardPerPointInTimeRange[timeIndex] *
                    allocation.points;
            }
        }
        ...
        returns (uint256)
    {
        ...
        return
            calculateAndStoreTotalRewardInChunks(
                _wallet,
                userAllocationsCount[_wallet]
            );
    }
        ...
        returns (uint256)
    {
        ...
        for (uint allocationId = start; allocationId <= stop; ++allocationId) {
            UserAllocation memory allocation = userAllocations[_wallet][
                allocationId
            ];
    ...
    function toClaim(
        ...
        uint256 walletAllocationsLength = userAllocationsCount[_wallet];
        uint256[] memory toClaimValues = new uint256[](walletAllocationsLength);

        if (
            walletAllocationsLength == 0 ||
            state() != LockdropState.TARGET_LIQUIDITY_PROVIDED
        ) {
            return toClaimValues;
        }

        for (
            uint256 allocationId = 1;
            allocationId < walletAllocationsLength;
            ++allocationId
        ) {
            UserAllocation memory allocation = userAllocations[msg.sender][
                allocationId
            ];
            uint256 toTakeFromAllocation = 0;
            if (!allocation.taken) {
                uint256 unlockTime = lockingExpirationTimestamps[
                    allocation.unlockTimestampIndex
                ];
                if (block.timestamp >= unlockTime) {
                    toTakeFromAllocation =
                        (allocation.points * initialLpTokensBalance) /
                        totalPoints();
                }
            }

            toClaimValues[allocationId] = toTakeFromAllocation;
        }
    ...
    function withdrawExchangedTokens(
        ...
        for (
            ...
            uint256 tokensToWithdraw = (allocation.points *
                initialLpTokensBalance) / totalPoints();

            totalLpToTransfer += tokensToWithdraw;
            userAllocations[msg.sender][allocationIndex].taken = true;
        }

        if (!exchangedPair().transfer(msg.sender, totalLpToTransfer)) {
            revert TransferFailed();
        }
    }
    ...
    function allocateRewardOnLockdropPhase2(
        uint256 _amount
    ) external atLeastTheLockdropState(LockdropState.REWARD_RATES_CALCULATED) {
        if (!isRewardCalculated(msg.sender)) {
            revert RewardNotCalculated();
        }

        uint256 totalReward = _totalRewardPerWallet[msg.sender];
        uint256 toAllocateOnPhase2Max = totalReward / 2;
        ...
        onlyLockdropPhase1Resolver
    {
        ...
        if (balance != 0) {
            if (
                !lockingTokens[removedLiquidityCounter_].token.approve(
    ...
    function state() public view returns (LockdropState) {
        ...
        if (allocationState == AllocationState.NOT_STARTED) {
            ...
            } else if (rewardRateCalculated_) {
                return LockdropState.REWARD_RATES_CALCULATED;
            }
        }
    ...
    function _getPoints(
        ...
        return (_getLPTokenPrice(_tokenIndex) * _amount) / 1 ether;
    }

    function calculateBoost(uint256 _basePoints) public view returns (uint256) {
        uint256 numerator = _basePoints * 100 * (lockingEnd - block.timestamp);
