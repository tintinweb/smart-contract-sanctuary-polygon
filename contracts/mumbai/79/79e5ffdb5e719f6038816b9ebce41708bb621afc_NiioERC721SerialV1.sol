...
RolesUpgradeable {
    ...
    modifier seriesExist(uint256 seriesId) {
        require(_seriesInfo[seriesId].maxEditions > 0, "Series does not exist");
        _;
    }
    ...
    modifier validEdition(uint256 seriesId, uint256 tokenId, uint256 minTokenId, uint256 maxTokenId) {
        ...
        require(seriesInfo.maxEditions > 0, "Series does not exist");
        require(seriesInfo.mintedEditions < seriesInfo.maxEditions, "Series is fully minted");
        require(tokenId > minTokenId && tokenId < maxTokenId, "TokenId does not belong to the given seriesId");
        _;
    }

    modifier validNumberOfEditions(uint256 maxEditions, uint256 numberOfMintedEditions) {
        require(numberOfMintedEditions > 0, "Must have a positive number of minted editions");
        require(maxEditions >= numberOfMintedEditions, "Number of minted editions  must be smaller than maxEditions");
        _;
    }
    ...
    validNumberOfEditions(maxEditions, tokensIds.length) {
        ...
        for (uint i = 0; i < tokensIds.length; i++) {
            require(tokensIds[i] > minTokenId && tokensIds[i] < maxTokenId, "TokenId does not belong to the given seriesId");
     ...
     */
    function mintNewSeriesAndTransferByGallery(uint256 seriesId, uint256 maxEditions, uint256[] calldata tokensIds, string[] calldata tokensMetadataHashes, string memory seriesMetadataHash, address creator, address gallery, address[] memory buyers, uint256[] calldata tokensToTransfer)
    external {
        require(buyers.length == tokensToTransfer.length, "buyers must have the same length as tokensToTransfer");
        mintNewSeries(seriesId, maxEditions, tokensIds, tokensMetadataHashes, seriesMetadataHash, creator);
        for (uint i = 0; i < tokensToTransfer.length; i++) {
    ...
    /**
     * @notice Burns the given series along with the given tokens.
     * The given tokens must be all the minted tokens of the given series.
     */
    function burnSeriesAndGivenTokens(uint256 seriesId, uint256[] calldata tokensIds)
    external
    onlyRole(SUPER_ADMIN_ROLE) {
        uint256 numberOfMintedEditions = _seriesInfo[seriesId].mintedEditions;
        require(numberOfMintedEditions == tokensIds.length, "The series has a different amount of tokens");
        (uint256 minTokenId, uint256 maxTokenId) = getTokensRange(seriesId);
    ...
    override {
        ...
        uint256 seriesId = getTokenSeriesId(tokenId);
        require(_seriesInfo[seriesId].mintedEditions > 0, "The series that refers to this token contains 0 tokens");
        _seriesInfo[seriesId].mintedEditions--;
        _burn(tokenId);
    }
    ...
    returns (RoyaltyInfo memory){
        return _seriesRoyaltyInfo[seriesId];
    }
}
...
abstract contract SerialERC2981Upgradeable is Initializable, IERC2981Upgradeable, ERC165Upgradeable {
    ...
    {
        uint256 seriesNumber = getTokenSeriesId(_tokenId);
        RoyaltyInfo memory royalty = _seriesRoyaltyInfo[seriesNumber];
    ...
    function _setSeriesRoyalty(
        ...
        _seriesRoyaltyInfo[seriesNumber] = RoyaltyInfo(receiver, feeNumerator);
    }

    /**
     * @dev Resets royalty information for the token id back to the global default.
     */
    function _resetSeriesRoyalty(uint256 seriesNumber) internal virtual {
        delete _seriesRoyaltyInfo[seriesNumber];
    }
    ...
    pure
    returns (uint256) {
        require(tokenId > SERIES_FACTOR, "tokensId must must be greater than 100,000");
