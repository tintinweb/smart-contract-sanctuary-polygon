...
contract RangoCBridgeFacet is IRango, IRangoCBridge, ReentrancyGuard {
    ...
    function cBridgeSwapAndBridge(
        ...
        if (request.toToken == LibSwapper.ETH) {
            ...
            SafeERC20.safeTransfer(IERC20(request.toToken), middleware, out);
        }
    ...
    function cBridgeBridge(
        ...
        if (token == LibSwapper.ETH) {
            require(msg.value >= amount + sumFees, "Insufficient ETH sent for bridging");
            value = amount;
        } else {
            ...
            SafeERC20.safeTransferFrom(IERC20(token), msg.sender, middleware, amount);
    ...
    function cBridgeSwapAndIM(
        ...
        if (request.toToken == LibSwapper.ETH && msg.value == 0) {
            ...
            (uint out,) = LibSwapper.onChainSwapsPreBridge(request, calls, sgnFee);
            bridgeAmount = out;
        }
    ...
    function cBridgeIM(
        ...
        if (token == LibSwapper.ETH) {
            uint amountWithFee = amount + request.affiliateFee + request.platformFee + request.destinationExecutorFee;
        ...
        RangoCBridgeMiddleware(middleware).doCBridgeIM{value : token == LibSwapper.ETH ? amount : sgnFee}(
            token,
            amount,
            bridgeRequest.receiver,
            bridgeRequest.dstChainId,
...
contract RangoCBridgeMiddleware is RangoBaseInterchainMiddleware, MessageSenderApp, MessageReceiverApp, ReentrancyGuard {
