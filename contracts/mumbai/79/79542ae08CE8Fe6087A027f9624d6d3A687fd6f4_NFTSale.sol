...
contract NFTSale {
  ...
  uint64 public start;
  uint64 public end;
  uint32 public nftType;
  uint32 public fee_d2; // in percent 2 decimal
  ...
  mapping(uint32 => Detail) public nft;
  mapping(address => Invoice[]) public invoices;
  mapping(address => string) public recipient;
  mapping(uint32 => uint128) public nftSold;
  mapping(address => mapping(uint32 => uint128)) public userBought;
  ...
  event NFTBought(uint32 indexed nftType, address indexed buyer, uint128 nftReceived, uint128 buyAmount, uint128 feeCharged);
  ...
  function initialize(
    ...
    nftType = uint32(_sale.length);

    for(uint32 i=0; i<nftType; ++i){
      nft[i].sale = _sale[i];
  ...
  /**
    ...
    */
  function getUserAllocation(address _user, uint32 _nftType) public view returns(uint128 userAlloc){
    uint128 min = minKom;
    if(min > 0){
  ...
  function amountInCalc(
    ...
    amountInFinal = _nftReceived * price;
    ...
    if(userPurchase + _nftReceived > alloc) amountInFinal = (alloc - userPurchase) * price;

    nftReceivedFinal = amountInFinal / price;
  }
  ...
  /**
    ...
    * @param _nftType NFT type
    */
  function buyToken(uint32 _nftType, uint128 _amountIn) external isOnGoing isNotPaused {
    if(minKom > 0) require(setAllocation(msg.sender, _nftType), "!eligible");

    uint32 buyerId = setBuyer(msg.sender);

    uint128 nftReceived = _amountIn / nft[_nftType].price;
    
    (uint128 amountInFinal, uint128 nftReceivedFinal) = amountInCalc(nftReceived, msg.sender, _nftType);
    
    uint128 feeCharged = (amountInFinal * fee_d2) / 10000;
  ...
  /**
    ...
    * @param _nftType NFT type
    */
  function teamBuy(uint32 _nftType, uint128 _nftAmount) external isOnGoing isNotPaused {
  ...
  /**
    ...
    * @param _nftType NFT type
    */
  function setAllocation(address _user, uint32 _nftType) private returns(bool) {
  ...
  function setPrice(uint128[] calldata _price) external onlyOwner {
     ...
     * @param _fee_d2 Fee project percent in 2 decimal
     */
    function setFee_d2(uint32 _fee_d2) external onlyOwner {
      require(uint64(block.timestamp) < start, "bad");
...
interface INFTSaleFactory {
  ...
  function createProject(uint64, uint64, uint64, uint128, uint128[] calldata, uint128[] calldata, uint32, address) external returns (address);
