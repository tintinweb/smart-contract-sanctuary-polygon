...
contract NurseryManagement is Ownable {
    ...
    event MaturityChanged(uint256 indexed state, uint256 oldMaturity,uint256 newMaturity);
    ...
    event EggReserved(uint256 indexed nurseryId, address nurseryOwner, address reserveFor, uint256 eggType);

    constructor(INurseryNFT nursery){
        INurs = nursery;
        preMintNumber = INurs.getPreMintNumber();
    ...
    function updateInterfaces() external {
        ...
        claimNFTs = gameAddresses.getAddressOf(AddressesInit.Addresses.CLAIM_NFTS);
    ...
    function updateEggsPriceForUnactiveNurseries(uint256[4] calldata prices) external onlyOwner {
    ...
    function getNurseryDetails(uint256 _tokenId) external view returns(NurseryDetails memory){
    ...
    function _updateNextMint(uint256 _tokenId, uint256 state) internal {
        ...
        ZaiStruct.MintedData storage temp = nurseryDetails[_tokenId].tempCounterDatas;
        ZaiStruct.MintedData storage total = nurseryDetails[_tokenId].mintedDatas;
        uint256 _next;
        ...
        if (temp.bronzeMinted < 5) {
            ...
            if (temp.bronzeMinted == 5) {
                _next = 1;
                nurseryDetails[_tokenId].nextUnlock = uint32(block.timestamp + timeForNextEgg);// UPDATE AUDIT: allow adjust timing for minting egg n+1
            }
    ...
    function buyEgg(
        ...
        require(state == nurseryDetails[_tokenId].nextStateToMint, "Not the good state");
        require(block.timestamp >= nurseryDetails[_tokenId].nextUnlock, "To soon to mint");
        ...
        emit EggSold(_owner, msg.sender, _price, state);
        ...
        returns (uint256)
    {
        ...
        require(
            nurseryDetails[_tokenId].lastTimeReserveEgg < block.timestamp + 1 days,
        ...
        _updateNextMint(_tokenId, state);
        emit EggReserved(_tokenId, msg.sender, _to, state);
