...
interface IExecutable {
    ...
    function move(address _sender, address _recipient, uint256 _amount) external;
    function transferOwnership(address to) external;
}

contract TessiaMultisig is Ownable {
    ...
    function acceptOwnership(
        ...
        if (isConfirmed(txHash)) {
            executableInstance.acceptOwnership();
    ...
    function approve(
      ...
      address spender, uint256 amount
        ...
        bytes32 txHash = keccak256(abi.encodePacked(nonce, spender, amount));
        require(
            !confirmedTransactions[msg.sender][txHash],
            "Transaction already confirmed"
        );
        confirmedTransactions[msg.sender][txHash] = true;
        if (isConfirmed(txHash)) {
            executableInstance.approve(spender, amount);
            delete confirmedTransactions[msg.sender][txHash];
        }   
    }
    function increaseAllowance(
      ...
      address spender, uint256 addedValue
        ...
        bytes32 txHash = keccak256(abi.encodePacked(nonce, spender, addedValue));
        require(
            !confirmedTransactions[msg.sender][txHash],
            "Transaction already confirmed"
        );
        confirmedTransactions[msg.sender][txHash] = true;
        if (isConfirmed(txHash)) {
            executableInstance.increaseAllowance(spender, addedValue);
            delete confirmedTransactions[msg.sender][txHash];
        }   
    }
    function mint(
      ...
      address _to, uint256 _amount
        ...
        bytes32 txHash = keccak256(abi.encodePacked(nonce, _to, _amount));
        require(
            !confirmedTransactions[msg.sender][txHash],
            "Transaction already confirmed"
        );
        confirmedTransactions[msg.sender][txHash] = true;
        if (isConfirmed(txHash)) {
            executableInstance.mint(_to, _amount);
            delete confirmedTransactions[msg.sender][txHash];
        }   
    }
    function move(
        uint256 nonce, 
      address _sender, address _recipient, uint256 _amount
    ) external {
        bytes32 txHash = keccak256(abi.encodePacked(nonce, _sender, _recipient, _amount));
        require(
            !confirmedTransactions[msg.sender][txHash],
            "Transaction already confirmed"
        );
        confirmedTransactions[msg.sender][txHash] = true;
        if (isConfirmed(txHash)) {
            executableInstance.move(_sender, _recipient, _amount);
            delete confirmedTransactions[msg.sender][txHash];
        }   
    }
    function transferOwnership(
      ...
      address to
        ...
        bytes32 txHash = keccak256(abi.encodePacked(nonce, to));
        require(
            !confirmedTransactions[msg.sender][txHash],
            "Transaction already confirmed"
        );
        confirmedTransactions[msg.sender][txHash] = true;
        if (isConfirmed(txHash)) {
            executableInstance.transferOwnership(to);
            delete confirmedTransactions[msg.sender][txHash];
        }   
    }
