...
/// @dev テスト用の NFT コントラクト（錦NFT）
contract DeployedNFTMock is IDeployedNFTMock, ERC721, ReentrancyGuard, Ownable {

    using Strings for uint256;

    uint256 public constant PRICE = 0.01 ether;
    uint256 public constant MAX_SUPPLY = 100;
    uint256 private _nextReservedTokenId = 90;
    uint256 private _remainingAmountForPromotion = 10;
    uint256 private _remainingAmountForSale = 90;
    ...
    ERC721("DeployedNFTMock", "DNM")
    {
        require(__recipient != address(0), "DeployedNFTMock: Invalid address");
        _recipient = __recipient;
        __baseURI = baseURI;
    }

    /// @dev 残りのプロモーション用のトークンの発行数を返す
    function remainingAmountForPromotion() external override view returns (uint256) {
        return _remainingAmountForPromotion;
    }

    /// @dev 残りの販売用のトークンの発行数を返す 9800 - 1470
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        require(_exists(tokenId), "DeployedNFTMock: URI query for nonexistent token");
    ...
    function buy(uint256 tokenId) external override nonReentrant payable {
        require(_isOnSale, "DeployedNFTMock: Not on sale");
        require(msg.value == PRICE, "DeployedNFTMock: Invalid value");
        require(tokenId < _nextReservedTokenId, "DeployedNFTMock: Invalid id");
    ...
    function buyBundle(uint256[] memory tokenIdList) external override nonReentrant payable {
        ...
        require(_isOnSale, "DeployedNFTMock: Not on sale");
        require(msg.value == PRICE * count, "DeployedNFTMock: Invalid value");
        _remainingAmountForSale -= count;

        for (uint256 i; i < count; i++) {
            require(tokenIdList[i] < _nextReservedTokenId, "DeployedNFTMock: Invalid id");
    ...
    /// @dev メタデータのベースURIを更新する
    function updateBaseURI(string calldata newBaseURI) external override onlyOwner {
        require(!_isMetadataFroze, "DeployedNFTMock: Metadata is froze");
        __baseURI = newBaseURI;
    }

    function freezeMetadata() external override onlyOwner {
        require(!_isMetadataFroze, "DeployedNFTMock: Already froze");
        _isMetadataFroze = true;
    }

    /// @dev プロモーション用のトークンを発行する（ownerMint）
    function mintForPromotion(
        address to,
        uint256 amount
    ...
    /// @dev ETHを受け取る。ただし、_recipientは変更不可
