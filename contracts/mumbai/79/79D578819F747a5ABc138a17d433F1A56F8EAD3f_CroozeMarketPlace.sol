...
contract CroozeMarketPlace {
    ...
    /// @dev The minimum price must be 80% of the buyNowPrice(if set).
    ...
    /**
     * @dev The bid amount was either equal the buyNowPrice or it must be higher than the previous
    ...
    /// @dev check if the highest bidder can purchase this NFT.
    ...
    /**
     * @dev Payment is accepted if the payment is made in the ERC20 token or ETH specified by the seller.
    ...
    /**
     * @dev Check if a bid has been made. This is applicable in the early bid scenario
    ...
    /// @dev if the minPrice is set by the seller, check that the highest bid meets or exceeds that price.
    ...
    /**
     * @dev An NFT is up for sale if the buyNowPrice is set, but the minPrice is not set.
    ...
    /**
     * @dev The highest bidder is allowed to purchase the NFT if
    ...
    /// @dev The default value for the NFT recipient is the highest bidder
    ...
    /// @param _erc20Token ERC20 Token for payment (if specified by the seller)
    /// @param _minPrice minimum price
    /// @param _buyNowPrice buy now price
    /// @param _feeRecipients fee recipients
    /// @param _feePercentages fee percentage
    ...
    /**
     * @dev Create an auction that uses the default bid increase percentage
    ...
    /**
     * @dev Allows for a standard sale mechanism where the NFT seller can
    ...
    /**
     * @dev Make bids with ETH or an ERC20 Token specified by the NFT seller.
    ...
    /**
     * @dev Settle an auction or sale if the buyNowPrice is met or set
    ...
    /**
     * @dev Reset all auction related parameters for an NFT.
    ...
    /**
     * @dev Reset all bid related parameters for an NFT.
    ...
    /// @dev update bid parameters to ensure contract only holds the highest bid.
    ...
    /// @dev reverse bids to ensure contract only holds the highest bid.
    ...
    function _payout(
        ...
        if (_isERC20Auction(auctionERC20Token)) {
             ...
             * ---> Maybe this function need to use reentrancy guard <---
             */
