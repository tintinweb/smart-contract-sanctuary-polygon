...
contract FlowmiFollowModule is VRFConsumerBaseV2, FeeModuleBase, FollowValidatorFollowModuleBase {
    ...
    mapping(address => uint256) private s_profileToAverageFunding; // mapping to know how many tokens are in the current raffle
    ...
    function processFollow(
        ...
        // Update total amount of funds for profile
        s_profileToAverageFunding[profileid] += requested;
        ...
        if (s_index % i_goal == 0 && s_profileToFollowersCount[profileid] != 0) {
            ...
            requestRandomWords();
            s_profileToAverageFunding[profileid] = 0;
        }
    }
    ...
    function payAtokens(address _winner) private {
        bool success = iaWmatic.transfer(_winner, s_profileToAverageFunding[profileid] / i_goal);
        if (!success) {
            revert Flowmi__TransferFailed();
        }
    }
