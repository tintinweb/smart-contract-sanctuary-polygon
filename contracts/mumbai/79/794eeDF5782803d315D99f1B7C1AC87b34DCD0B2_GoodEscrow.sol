...
contract GoodEscrow is Initializable, ContextUpgradeable, UUPSUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    address public treasuryContractAddress;
    address public goodTokenContractAddress;
    ...
    event UpdatedTreasuryAndTokenAddress(
        address treasuryAddress,
        address tokenAddress
    );
    event AdminDeposited(address indexed payee, uint256 weiAmount);
    ...
    function initialize(
        ...
        address _goodTokenContractAddress,
        uint256 _allowancePerHour
    ) public initializer {
        _updateTreasuryAndTokenAddress(
            _treasuryContractAddress,
            _goodTokenContractAddress
        );
    ...
    function adminDepositFunds(uint256 _amount) external isAdmin {
        require(_amount > 0, "GoodEscrow: amount should be greater than 0");
        IERC20Upgradeable(goodTokenContractAddress).safeTransferFrom(
            _msgSender(),
            address(this),
            _amount
        );
        emit AdminDeposited(_msgSender(), _amount);
    }
    ...
    function withdraw(uint256 _amount) external {
        ...
        IERC20Upgradeable(goodTokenContractAddress).safeTransfer(
            _msgSender(),
            _amount
        );
    ...
    function EmergencyWithdrawFunds(
        ...
        require(
            (_to != address(0)) && (_tokenAddress != address(0)),
            "GoodEscrow: address zero not allowed"
        );
        require(_amount > 0, "GoodEscrow: amount should be greater than 0");
        IERC20Upgradeable(_tokenAddress).safeTransfer(_to, _amount);
        emit EmergencyFundWithdrawn(_tokenAddress, _to, _amount);
    }
     ...
     */
    function updateTreasuryAndTokenAddress(
        address _treasuryContractAddress,
        address _goodTokenContractAddress
    ) external isAdmin {
        _updateTreasuryAndTokenAddress(
    ...
    function _updateTreasuryAndTokenAddress(
        address _treasuryContractAddress,
        address _goodTokenContractAddress
    ) internal {
        treasuryContractAddress = _treasuryContractAddress;
        goodTokenContractAddress = _goodTokenContractAddress;
        emit UpdatedTreasuryAndTokenAddress(
            _treasuryContractAddress,
            _goodTokenContractAddress
        );
    }
