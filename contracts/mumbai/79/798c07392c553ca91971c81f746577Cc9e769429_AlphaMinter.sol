...
contract AlphaMinter is Ownable, IERC721Receiver, Controllable, PausableContract, IMinter {
  ...
  mapping(uint256 => address) public og_token_owners;
  ...
  function mintScion() public payable whenNotPaused {
    require(
      canMintBot(1) == true,
      'unable to mint; either wrong caller, minting disabled or bot allowance exceeded'
    );
    (
      uint256 parent_id,
    ...
    //name based on how many scions there already are
    uint256 number_with_scions = scionCoordinator.getScionCount(parent_id) + 1;
    pxlbot.renameBot(tokenId, string(abi.encodePacked('SCION ',Strings.toString(parent_id),'-',getPaddedStr(number_with_scions))));
  ...
  function claimFromCollectible(uint256 collectible_token_id) external payable {
    require(
      isController(_msgSender()) || og_token_owners[collectible_token_id] == _msgSender(),
      'Minter: Caller is not controller or does not own the collectible.'
    );
    require(
    ...
    //mint the bot and assign attrs (no degradation)
    pxlbot.mintScion(og_token_owners[collectible_token_id], collectible_token_id, attr_ids, attr_values);
    uint256 tokenId = pxlbot.totalSupply();
    //name bot
    pxlbot.renameBot(tokenId, string(abi.encodePacked('SCION ',Strings.toString(collectible_token_id),'-001')));
    //put the token in play
    pxlbot.addTokenToGameplay(tokenId);
  ...
  function setDefaultDegradation(uint8 _new_degrade) external onlyController {
    default_degradation = _new_degrade;
  }
  ...
  function getPaddedStr(uint256 _num) internal returns (string memory) {
    string memory _strnum = Strings.toString(_num);
    if(_num > 9 && _num <= 99) {
      return string(abi.encodePacked('0',_strnum));
    }
    if (_num > 99 && _num <= 999) {
      return _strnum;
    }
    return string(abi.encodePacked('00',_strnum));
  }
