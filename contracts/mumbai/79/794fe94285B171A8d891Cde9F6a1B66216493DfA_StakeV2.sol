...
contract Stake is Initializable, AccessControlUpgradeable, UUPSUpgradeable, LicenseBase, OwnableUpgradeable {
    /* ========== STATE VARIABLES ========== */
    ...
    function stake(uint256 _amount) external {
        ...
        require(_amount >= minStakeAmount, "amount below the mininum staking requirement");
        ...
        require(dimoToken.transferFrom(msg.sender, address(this), _amount), "token transfer failed");
     ...
     */
    function setNewMinStakeAmount(uint256 _amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
     ...
     */
    function unstake(address user, uint256 _amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        // update the balance of the user's staked tokens
        require(stakedBalanceOf[user] >= _amount, "User does not have enough staked balance");
     ...
     */
    function emergencyWithdraw(address user, uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function mint(address to) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(checkUserIsWhitelisted(to), "User has not staked the required DIMO tokens");
     ...
     */
    function setDimoURI(string memory dimoURI) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _setDimoURI(dimoURI);
    }
     ...
     */
    function checkUserStakedBalance(address user) external view returns (uint256) {
...
contract LicenseBase is Initializable {
     ...
     */
    function __LicenseBase_init(string memory name_, string memory symbol_) internal initializer {
        _name = name_;
        _symbol = symbol_;
    }
