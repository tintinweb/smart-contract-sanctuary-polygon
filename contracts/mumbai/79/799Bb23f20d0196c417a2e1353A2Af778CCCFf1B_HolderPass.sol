...
contract HolderPass is SolidStateERC1155, AccessControl, Ownable{
    ...
    /**
     * set imageURI which is the directry path where goldPass and silverPass image files.
     */
    function setImageURI(string memory _imageURI) public onlyRole(MINTER_ROLE) {
        imageURI = _imageURI;
    }

    /**
     * @dev return DataURI which is base64 encoded Json matadata
     * @notice inheritdoc IERC1155Metadata
     */
    function uri(uint256 tokenId) override(IERC1155Metadata) public view returns (string memory) {
        string memory imageURI_gold = string(abi.encodePacked(imageURI, "gold"));
        string memory imageURI_silver = string(abi.encodePacked(imageURI, "silver"));

        // TODO
        if (tokenId > 10000) {
            return imageURI_gold;
        } else {
            return imageURI_silver;
        }
    }
    ...
    function supportsInterface(bytes4 interfaceId)
        public view virtual override(IERC165, AccessControl)
        returns (bool) {
        return 
