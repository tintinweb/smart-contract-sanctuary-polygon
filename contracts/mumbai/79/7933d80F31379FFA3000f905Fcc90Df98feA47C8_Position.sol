...
contract Position is ReentrancyGuard, Ownable, PositionStorage {
    ...
    function updateMarginContract(address _marginContract) external onlyOwner {
        require(
            _marginContract.isContract(),
    ...
    function createPositionAndTrade(address tokenAddress, address targetToken, uint256 amount) external {
       (uint256 availableMargin,
        uint256 loanAmount,
        ,
        ) = IMargin(marginContract).getDepositAmounts(msg.sender, tokenAddress);
        uint256 ltmRatio = IMargin(marginContract).ltmRatio(msg.sender, tokenAddress);
        ...
        IMargin(marginContract).setPositionAmount(user, tokenAddress, marginUsed, loanUsed);
    ...
    function trade(uint256 tradeAmount, address[] memory path) internal returns(uint256, uint256) {
        ...
        uint256 protocolFees = (expectedUnits[1] * 20) / 10000;
        //expectedUnits is in expectedUnits[1] 
        if(path[0] == uniswapV2Router.WETH()) {
            // Trade and get asset out
            //internalCall to marginFunds
            IMarginFunds(marginFundsContract).withdrawFunds(payable(address(this)), tradeAmount);
            ...
            IMarginFunds(marginFundsContract).withdrawERC20Tokens(path[0], address(this), tradeAmount);
        ...
        //this protocol fees should be transferred to treasury contract
    ...
    function closePosition(uint256 positionID) external {
        ...
        positionValue[positionID].lossValue = 0;
        positionValue[positionID].gainValue = 0;
        //PositionValue storage positionValue = 
        ...
        if(positionInfo.assetAmount >= _result) {
            ...
            positionValue[positionID].lossValue = temp;
            ...
            positionValue[positionID].gainValue = temp;
            marginAmount = positionInfo.marginUsed + temp;
        }
        address user = msg.sender;
        IMargin(marginContract).closePositionAmount(user, positionInfo.asset, positionInfo.marginUsed ,positionInfo.loanUsed, marginAmount);
        require(IERC20(positionInfo.asset).balanceOf(address(this)) >= _result, "Position: No balance");
        IERC20(positionInfo.asset).transfer(marginFundsContract, _result);
    ...
    function addMargin(uint256 positionID, uint256 marginAmount) external {
        ...
        address user = msg.sender;
        IMargin(marginContract).setPositionAmount(user, positionInfo.asset, marginAmount, 0);
        address[] memory path = new address[](2);
    ...
    function moveMargin(uint256 positionID, uint256 marginAmount) external {
        ...
        address user = msg.sender;
        IMargin(marginContract).removeMargin(user, positionInfo.asset, marginAmount);
    ...
    function getLiquidationPrice(uint256 tradeAmount, uint256 marginAmount, uint256 targetAmount, address[] memory path) public view returns(uint256 liquidationPrice) {
        ...
        uint256 maxLoss = (convertedMargin * 80)/100;
    ...
    function getMarginPrice(uint256 tradeAmount, uint256 marginAmount, uint256 targetAmount, address[] memory path) public view returns(uint256 marginPrice) {
        ...
        uint256 maxMarginLoss = (convertedMargin * 70)/100; 
        if(targetDecimals == 18) {
...
contract PositionStorage {
    ...
    address internal marginContract;
    ...
    mapping(uint256 => uint256) public marginAdded;

    mapping(uint256 => uint256) public marginMoved;

    mapping(uint256 => PositionValue) public positionValue;  
