...
contract Betgame is Ownable {
    ...
    function collectWinnings() external{
        ...
        */
        ...
        bool success =purchaseToken.transfer(msg.sender, getWinningsForWallet());
        require(success, "Failed to Collect!");
    }
    ...
    function getTicketsForWallet() public view returns (uint256) {
        require(userData[msg.sender].numTickets>0,"Wallet not registered!");
        return userData[msg.sender].numTickets;
    }
    //Get selected team for user wallet
    function getTeamForWallet() public view returns (string memory) {
        require(userData[msg.sender].numTickets>0,"Wallet not registered!");
        return teamList[userData[msg.sender].teamId];
    }
    //Get winnings for user wallet
    function getWinningsForWallet() public view returns (uint256){
        require(gameEnded == true, "Game needs to end!");
        require(userData[msg.sender].numTickets>0,"Wallet not registered!");
        require(userData[msg.sender].teamId==winnerId,"Your team didnt win!");
        ...
        return ticketShare*userData[msg.sender].numTickets;
    }
