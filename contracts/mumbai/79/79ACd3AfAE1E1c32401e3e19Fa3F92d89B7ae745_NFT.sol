...
contract NFT is ERC721, Ownable {
    ...
    function transfer(address from, address to, uint256 tokenId) public onlyOwner {
        ...
        require(ownerOf(tokenId) == from, "You are not the owner of this smart contract");
        // Transfer the token to `to`
        _transfer(from, to, tokenId);
    }

    function transferContractOwnership(address newOwner) public onlyOwner {
        // Transfer contract ownership to `newOwner`
        transferOwnership(newOwner);
    }
    
    function mint(address to) public onlyOwner {
        // Increment the total NFTs counter
        totalNFTs++;
        // Mint a new token to `to`
        _safeMint(to, totalNFTs);
    }
}
