    ...
    UUPSUpgradeable
{
    ...
    uint256 public normalMintPrice;
    ...
    event Paid(uint256 required, uint256 paid);

    function initialize(
        uint256 maxSupply,
        uint256 _normalMintPrice,
        uint256 _whitelistedMintPrice,
        address payable _wallet1
    ) public initializer {
        __ERC721_init("CryElePants", "CEN");
    ...
    function whitelistMint(
        ...
        require(_isWhiteListed[msg.sender], "Account is not whitelisted");
        require(tokenId < MAX_SUPPLY, "Invalid tokenId");
        uint256 requiredPayment = whitelistedMintPrice;
    ...
    function batchWhitelistMint(
        ...
        uint256 requiredPayment = whitelistedMintPrice * quantity;
        require(msg.value >= requiredPayment, "Insufficient payment");
        ...
        if (msg.value > requiredPayment) {
            (bool success, ) = msg.sender.call{
                value: msg.value.sub(requiredPayment)
            }("");
            require(success, "Refund failed");
        }

        emit Paid(requiredPayment, msg.value);
    }
    ...
    function mint(
        ...
        uint256 requiredPayment = normalMintPrice;
        require(msg.value >= requiredPayment, "Insufficient payment");
    ...
    function batchMint(
        ...
        require(mintEnabled, "Batch minting is not enabled");
        require(tokenIds.length > 0, "No tokenIds provided");
        require(
            tokenIds.length == tokenURIs.length,
            "TokenIds and URIs length mismatch"
        ...
        uint256 requiredPayment = normalMintPrice * quantity;
        require(msg.value >= requiredPayment, "Insufficient payment");
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            require(tokenId < MAX_SUPPLY, "Invalid tokenId");
    ...
    function tokensOfOwnerWithURIs(
        address owner
    ...
    function withdrawFunds() public onlyOwner {
        ...
        wallet1.transfer(balance);
    }

    function updateWallet1(address payable newWallet1) public onlyOwner {
        require(newWallet1 != address(0), "New wallet1 address cannot be zero");
        wallet1 = newWallet1;
    }
    ...
    function _setTokenURI(
        ...
        // ERC721URIStorageUpgradeable's _setTokenURI() call
