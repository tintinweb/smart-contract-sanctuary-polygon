...
contract test002 is VRFV2WrapperConsumerBase {
    ...
    mapping(uint8 => mapping(uint8 => bool)) public drawIdProcessed;

    bool public dLBPProcessed;
    ...
    function dNP(uint8 drawId, uint16[] memory tokenIds) external onlyOwner {

        // Check if the drawId has already been processed
        require(!drawIdProcessed[currentCritRound][drawId], "This drawId has already been processed.");

        // Mark drawId as processed
        drawIdProcessed[currentCritRound][drawId] = true;
    ...
    function dLNP(uint8 drawId, uint16[] memory tokenIds) external onlyOwner {
    ...
    function tNP(uint8 _drawId) external onlyOwner {
        uint16 tokenId;
        address winner;
        uint256 mintPrice;
        uint256 multiplierPercentage;
    ...
    function dLBP(uint8 drawIdLength) external onlyOwner {
        // Check if the dLBP has already been processed
        require(!dLBPProcessed, "dLBP has already been processed.");

        // Mark dLBP as processed
        dLBPProcessed = true;
        ...
        LBPWinners.push() = tokenId;
        LBPPrizes.push() = remainingPrizeAmount;
    ...
    function getNPWinners(uint8 _currentCritRound, uint8 _drawId) public view returns (uint16[] memory) {
        return roundWinners[_currentCritRound][_drawId];
    }

    function getNPPlayers(uint8 _currentCritRound, uint8 _drawId) public  view returns (uint16[] memory) {
        return roundPlayers[_currentCritRound][_drawId];
    }

    function setNPProcessState(uint8 _drawId, bool _processed) external onlyOwner {
        drawIdProcessed[currentCritRound][_drawId] = _processed;
    }

    function getBPWinners() public view returns (uint16[] memory) {
        return LBPWinners;
    }

    function setBPPProcessState(bool _processed) external onlyOwner {
        dLBPProcessed = _processed;
    }
