...
contract NFTS is ERC1155, Ownable {
    ...
    mapping(address=>mapping(uint=>uint)) private nftsOwned;
    ...
    function createToken(uint256 _amount, string memory _uri) external {
        ...
        nftsOwned[_msgSender()][tokenIds.current()] = _amount;
    ...
    function totalNftOwned(address wallet) public view returns(uint[] memory ids, uint [] memory quantity ){
        uint len = 0;
        for(uint i = 1; i <= tokenIds.current(); i++){
            len += nftsOwned[wallet][i] > 0 ? 1 : 0;
        }
        if(len < 1)
            return (new uint[](0),new uint[](0));
        uint [] memory nftsId = new uint[](len);
        uint [] memory qty = new uint[](len);
        uint index = 0;
        for(uint i = 1; i <= tokenIds.current(); i++){
            if(nftsOwned[wallet][i] > 0){
                nftsId[index] = i;
                qty[index] = nftsOwned[wallet][i];
                index++;
            }
        }
    ...
    function safeTransferFrom(
        ...
        nftsOwned[from][id] -= amount;
        nftsOwned[to][id] += amount;
        _safeTransferFrom(from, to, id, amount, data);
    } 
