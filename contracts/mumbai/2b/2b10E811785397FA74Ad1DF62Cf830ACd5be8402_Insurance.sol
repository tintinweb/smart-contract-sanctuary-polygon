...
contract Insurance is FunctionsClient, VRFConsumerBaseV2, ConfirmedOwner {
  ...
  mapping(uint256 => address) public randomnessRequestSenders;
  ...
  function requestRandomWords() public returns (uint256 requestId) {
    ...
    randomnessRequestSenders[requestId] = msg.sender;
  ...
  function fulfillRandomWords(uint256 _requestId, uint256[] memory _randomWords) internal override {
    ...
    address originalSender = randomnessRequestSenders[_requestId];

    // Update the latest insurance data for the sender with random points
    InsuranceData[] storage policies = insuranceDataMapping[originalSender];
    require(policies.length > 0, "No insurance data found for the request sender");

    InsuranceData storage latestPolicy = policies[policies.length - 1];
    for (uint i = 0; i < 3; i++) {
      latestPolicy.randomPoints[i] = _randomWords[i];
    }
  }
