...
contract MonsterPartData is Ownable
{
    ...
    uint8 public constant EVOLVE_LEVEL_REQUIRE = 5;
    ...
    function getEvolveRequireLevel(uint8[5] memory _decodeGene, uint8 evolve) external view returns(uint8)
    {
        ...
        for (uint8 i = 0; i < _decodeGene.length; i++)
        {
            ...
            if((rarity > 1 && evolve < 3) || (rarity > 0 && evolve < 2))
            {
    ...
    function getBaseStatPoints(uint8[5] memory _decodeGene) external view returns(uint8[5] memory)
    {
        ...
        for(uint8 i =0;i<_decodeGene.length;i++)
        {
            ...
            uint supportIndex1 = i-1;
            uint supportIndex2 = i -2;
            if(i < 2)
            {
                supportIndex1 = (_decodeGene.length-1)*i;
                supportIndex2 = (_decodeGene.length - i -1);
            }
            ...
            if(supportPartClass1 - partClass == 1 || supportPartClass1 - partClass == 4)
                baseStats[partClass - 1] += 3;
            if(supportPartClass2 - partClass == 1 || supportPartClass2 - partClass == 4)
