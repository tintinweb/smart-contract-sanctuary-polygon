...
contract NftContract is ERC721Enumerable,Ownable {
    ...
    function mintNft(uint256 _quantity) external onlyOwner {
        uint256 _totalMinted = _tokenIds.current();
        require(_totalMinted.add(_quantity) <= MAX_SUPPLY, "Quantity Exceed MAX SUPPLY");
        ...
        for (uint256 i = 0; i < _quantity; i++) {
            ...
            //string memory _temp_uri = string(abi.encodePacked(_baseURI(), _totalMinted.toString(),".json"));
            //emit PermanentURI(_temp_uri, _totalMinted);
            _tokenIds.increment();
        }
