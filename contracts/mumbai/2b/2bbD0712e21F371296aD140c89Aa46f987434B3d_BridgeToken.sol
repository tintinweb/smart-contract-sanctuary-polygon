...
contract BridgeToken is ERC20Bridge, IChildToken, EIP712MetaTransaction {
    ...
    error BridgeToken__MintAddressMismatch(
        uint256 mintAddressesLen,
        uint256 mintAmountsLen
    );
    error BridgeToken__InvalidManagerProxy();
    error BridgeToken__TotalSupplyExceedsCap();
    error BridgeToken__NotAllowedToDeposit(address sender);
    error ERC20Capped__CapIsZero();
    error ERC20Capped__CapExceeded();
    ...
    constructor(
        ...
        EIP712MetaTransaction(_domainSeparatorName, _domainSeparatorVersion)
    {
        uint256 _mintAddressesLen = _mintAddresses.length;
        uint256 _mintAmountsLen = _mintAmounts.length;
        if (_mintAddressesLen != _mintAmountsLen) {
            revert BridgeToken__MintAddressMismatch({
                mintAddressesLen: _mintAddressesLen,
                mintAmountsLen: _mintAmountsLen
            });
        }
        if (address(0) == _childChainManagerProxy) {
            revert BridgeToken__InvalidManagerProxy();
        }
        if (cap_ <= 0) {
            revert ERC20Capped__CapIsZero();
        }
        _cap = cap_;

        childChainManagerProxy = _childChainManagerProxy;

        for (uint256 i; i < _mintAddressesLen; ++i) {
            address _mintAddress = _mintAddresses[i];
            _mint(_mintAddress, _mintAmounts[i]);
        }

        uint256 _totalSupply = totalSupply();
        if (cap_ < _totalSupply) {
            revert BridgeToken__TotalSupplyExceedsCap();
        }
    }
    ...
    function deposit(
        ...
        if (_msgSender() != childChainManagerProxy) {
            revert BridgeToken__NotAllowedToDeposit(_msgSender());
        }

        uint256 amount = abi.decode(depositData, (uint256));
        uint256 _totalAmount = this.totalSupply() + amount;
        if (_totalAmount > _cap) {
            revert ERC20Capped__CapExceeded();
        }
...
abstract contract EIP712Base {
    error EIP712Base__DomainSeparatorAlreadySet();
    error EIP712Base__StringIsEmpty(string field);
    ...
    /**
        @notice Set domain separator (initializer), only once
    ...
    */
    function _setDomainSeparator(
        ...
        if (domainSeparator != bytes32(0)) {
            revert EIP712Base__DomainSeparatorAlreadySet();
        }

        if (bytes(_name).length == 0) {
            revert EIP712Base__StringIsEmpty("_name");
        }

        if (bytes(_version).length == 0) {
            revert EIP712Base__StringIsEmpty("_version");
        }
    ...
    function toTypedMessageHash(
        ...
        return
            keccak256(
                abi.encodePacked("\x19\x01", getDomainSeparator(), _messageHash)
            );
    }
}
...
abstract contract EIP712MetaTransaction is EIP712Base {
    ...
    event MetaTransactionExecuted(
        ...
        address payable indexed _relayerAddress,
    ...
    /**
    @notice constructor that init domain separator
    @param _name Name of the domain
    @param _version The version of the contract
     */
    constructor(string memory _name, string memory _version) {
        _setDomainSeparator(_name, _version);
    }
    ...
    function verify(
        ...
        return
            _signer ==
            ecrecover(
                toTypedMessageHash(hashMetaTransaction(_metaTx)),
                _sigV,
                _sigR,
                _sigS
            );
