...
contract PINFTRevenueReader is  AdminAccess {
    ...
    function getLocksBatch(uint256[] calldata tokenIds) external view returns(PINFTExt.Lock[] memory locksBatch){
    ...
    function getSharesBatch(uint256[] calldata tokenIds, address fromAddress) external view returns(int256[] memory sharesBatch, uint256 totalShare){
        uint256 length = tokenIds.length;
        sharesBatch = new int256[](length);
        RevenueShareExt revenueShare = RevenueShareExt(address(factory.revenueShare()));
