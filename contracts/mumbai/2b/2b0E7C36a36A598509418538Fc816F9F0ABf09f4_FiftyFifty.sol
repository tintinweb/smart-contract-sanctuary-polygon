...
contract FiftyFifty is ERC721Enumerable, VRFConsumerBase, ReentrancyGuard, Ownable {
...
*/
    ...
    bool public drawingInProgress = false;
...
*/
    ...
    function mint(uint8 qty) public nonReentrant{
        ...
        require(IERC20(wethAddress).balanceOf(msg.sender) >= cost * qty, "Insufficient WETH balance");
        uint256 allowance = IERC20(wethAddress).allowance(msg.sender, address(this));
        require(allowance >= cost*qty, "Check the token allowance");

        // transfer WETH to contract
        uint256 ffsplit = cost * qty * 1/2;
        IERC20(wethAddress).transferFrom(msg.sender, address(this), ffsplit);
        IERC20(wethAddress).transferFrom(msg.sender, ownerAddress, ffsplit);

        weeklyPot[currentWeek] += (cost*qty) * 1/2;
        ...
        for(uint8 i=0; i<qty; i++){
            ...
            emit MintInProgress(_requestId);
            _tokenIds++;
        }
    }
...
*/
    ...
    // potential problems - if we're doing a yearly drawing prior to a minimum of 4 quarters we will lock up
    // must fix this
    ...
    // once seed is set do drawing, increment whichever drawing we just did, adjust _tokenIds if necessary
    // and re-enable minting.
    ...
    function weeklyDrawing() internal{
        ...
        require(IERC20(wethAddress).balanceOf(address(this)) >= weeklyPot[currentWeek], "Contract has insufficient WETH balance");

        uint256 min = 1000*currentWeek;
        uint256 winner = (weeklySeed[currentWeek] % (_tokenIds - min)) + min;
        address currentHolder = ownerOf(winner);

        IERC20(wethAddress).transfer(currentHolder, weeklyPot[currentWeek]);
