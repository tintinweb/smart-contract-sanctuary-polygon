...
contract Tickets is ERC721Enumerable, ERC721Burnable, ReentrancyGuard, AccessControl {
	...
	uint256 voteTime;
	uint256 voteAmount;
	uint8 winFlag; // 1 - win, 2 -lose, 0 - even
	uint8 teamA;
	uint8 teamB;
	uint8 scoreA;
	uint8 scoreB;
    ...
    // issue a ticket
    function newTicket(address voter, address matchAddress, MainControllerInterface.VoteRecord memory record) external nonReentrant returns (uint256) 
    {
	require(hasRole(MINTERS, msg.sender), "only minters can issue ticket");
	_tokenIds.increment();
        uint256 newId = _tokenIds.current();
        _safeMint(msg.sender, newId);
	records[newId] = TicketInfo(voter, matchAddress, record.voteTime, record.voteAmount, record.flag, record.teamA, record.teamB, record.scoreA, record.scoreB, record.isCommonVote, false, true);
        return newId;
    }
    // checkout tickets
    ...
    // get ticket info
    function getTicketInfo(uint256 tokenId) external view returns(address, address, uint8, uint8, uint8, uint8, uint8, uint256, uint256, bool, bool)
    {
	require(records[tokenId].isUsed == true, "invalid tokenId");
	TicketInfo memory info = records[tokenId];
	return (info.voter, info.matchAddress, info.winFlag, info.teamA, info.teamB, info.scoreA, info.scoreB, info.voteTime, info.voteAmount, info.isClaimed, info.isCommonVote);
    }
}
