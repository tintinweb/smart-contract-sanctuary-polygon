    ...
    BuddyAdminRole
{
     ...
     */
     function mint(
        ...
        tokenId = nextTokenId++;
        if(tokenIds[0]==0) {
            if(transferTokenId < NFTContract(NFTAddress).getNextTokenId()) {
            
                NFTContract(NFTAddress).transferFrom(NFTAdminAddress,msg.sender,transferTokenId);

                NFTContract(NFTAddress).lock(transferTokenId);

                mapTokenIds[tokenId][NFTAddress].push(transferTokenId);
                transferTokenId++;
            }
        }
        else {
    ...
    NFT721Mint    
{
    ...
    function initialize(
        ...
        adminUpdateConfig("https://ipfs.io/ipfs/",_NFTAddress,_NFTAdminAddress);
     ...
     */
    function adminUpdateConfig(string memory baseURI, address _NFTAddress, address _NFTAdminAddress)
        public
        onlyBuddyAdmin
    {
        _updateBaseURI(baseURI);
        NFTAddress = _NFTAddress;
        NFTAdminAddress = _NFTAdminAddress;
