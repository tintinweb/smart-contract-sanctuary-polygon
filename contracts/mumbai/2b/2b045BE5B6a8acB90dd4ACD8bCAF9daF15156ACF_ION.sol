...
contract ION is ERC20, AccessControl {
    ...
    uint256[] public rewardsAmt = new uint256[](5); //multiple of 10
    uint256 oneYearTime = 600;
    uint256 airdropMATICamount;
    uint256 public epochEndTime;
    // uint256 public slippageProtectionPercent = 40; //4%
    uint256 public marketCapPer = 30; //3%
    ...
    function setRewards(uint256[] calldata _rewardsAmt) public onlyAdmin {
    ...
    function setSellTax(uint256 _sellMaxTax, uint256 _sellMinTax)
        ...
        onlyAdmin
    {
        require(_sellMaxTax>_sellMinTax,"Invalid Value");
 ...
 //Percent base is in 1000
    function mint(address to, uint256 percent) public onlyAdmin {
            ...
            require(percent <= 100, "Percent exceeded");
    ...
    function _transfer(
        ...
        if (isblacklist[_sender] || isblacklist[_recipient] ) {
            ...
            // uint256 prevBal = IERC20(address(this)).balanceOf(uniswapV2Pair);
            super._transfer(_sender, _recipient, remAmount);
            epochCurrentAmt += ((remAmount * tokenPrice) / 10**18);
            super._transfer(_sender, address(this), sellTaxAmt);
            // uint256 currBal = IERC20(address(this)).balanceOf(uniswapV2Pair);
            // require(((prevBal + (prevBal * slippageProtectionPercent)/1000) >= currBal)
            //     && ((prevBal - (prevBal * slippageProtectionPercent)/1000) <= currBal), "Slippage Protection");
    ...
    function _distribution() internal {
        if (checkEpoch()) {
            if (superChgReward != 0 && burnRebaseReward != 0) {
                ...
                ISuperCharge(admin.superCharge()).addSupercharge(superChgReward);
                //swap and liquify
                if(enableSwapAndLiquify){
                    require(liquidityReward > 0, "liquidity not set");
                    address[] memory path = new address[](2);
                    path[0] = address(this);
                    path[1] = uniswapV2Router.WETH();
                ...
                uniswapV2Router.addLiquidityETH{value:maticAmount}(address(this), swapAmount, 1, 1, owner, block.timestamp+3600);
                }
            }
            _setEpoch();
        }
        ...
        if(block.timestamp >= maticDistribution + sevenDays){
