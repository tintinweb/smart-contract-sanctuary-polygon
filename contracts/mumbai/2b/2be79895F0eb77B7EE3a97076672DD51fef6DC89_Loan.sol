...
contract Loan is AccessProtected, ReentrancyGuard, BaseRelayRecipient, IERC721Receiver, EIP712Base, Pausable {
    ...
    function updateNFTRewardsClaimer(uint256 _loanId, bool _isLoanerNFTRewardsClaimer) external whenNotPaused {
        ...
        if (_isLoanerNFTRewardsClaimer) {
            require(
                ...
                "Loanee is already giving out nftRewards"
            );
            loanItems[_loanId].claimer = NFTRewardsClaimer.loaner;
            addressIsGivingAwayNFTRewardsTo[loanItems[_loanId].loanee] = _loanId;
        } else {
            loanItems[_loanId].claimer = NFTRewardsClaimer.loanee;

            if (loanItems[_loanId].claimer == NFTRewardsClaimer.loaner && _isLoanerNFTRewardsClaimer == false) {
                addressIsGivingAwayNFTRewardsTo[loanItems[_loanId].loanee] = 0;
            }
        }
    }
    ...
    function issueLoanFromOffer(
        ...
        checkLoanRewardPercentageLimit(sender, offer.percentageRewards);
