...
interface ILock is IERC721Upgradeable {
     ...
     */
    function getInfoBySingleID(
        uint id
    ) external view returns (address, uint, uint, uint, uint);
     ...
     */
    function getInfoByManyIDs(
        uint[] memory ids
    ) external view returns (address[] memory, uint[] memory, uint[] memory);
    ...
    ERC721HolderUpgradeable
{
    ...
    function updateLockedNFT(
        bytes calldata sig,
        address lock,
        uint nftId
    ) external onlySwapr {
        require(
            _getNFTOwner(lock, nftId) == address(this),
            "WALLET_IS_NOT_OWNER"
        );
        _lockedNFTs[lock][nftId] = sig;
    }
    ...
    function getLockedPart(
        address lock,
        uint nftId
    ) public view returns (uint256 lockedPart) {
        ILock lockContract = ILock(lock);
    ...
    function releaseERC(
        ...
        IERC20MetadataUpgradeable paymentToken = IERC20MetadataUpgradeable(
            token
        );
