...
contract ChainBattles is ERC721URIStorage {
    ...
    function train(uint256 tokenId) public {
        ...
        _attributes.levels = _attributes.levels + rand;
        _attributes.hp = _attributes.hp + rand;
        _attributes.strength = _attributes.strength + rand;
        _attributes.speed = _attributes.speed + rand;
        tokenIdToAttributes[tokenId] = _attributes;
        _setTokenURI(tokenId, getTokenURI(tokenId));
    }

    function generateRandom() private returns (uint256) {
        return uint256(keccak256(abi.encodePacked(block.timestamp))) % modulus;
    }
}
