...
contract luckBlocksNodesCFGD is Ownable,ReentrancyGuard {
  ...
  // initialize contract while deployment
   ...
   function activateNodes(address caller, uint256[] calldata _nodes , uint lottery,uint weekly) external {
        ...
        // require(block.timestamp > lbcontract.autoSpinTimestamp() + 6048,"autoSpin parameters not met");
         ...
         // require(block.timestamp > lbcontract.autoSpinTimestamp() + 864,"autoSpin parameters not met");
  ...
  function resetQueue(uint lottery, uint weekly) external {
      ...
      if(weekly == 1){
        require(block.timestamp > lbcontract.autoSpinTimestamp() + 25500,"autoSpin parameters not met");
        ...
        require(block.timestamp > lbcontract.autoSpinTimestamp() + 1164,"autoSpin parameters not met");
        //send the waiting nodes to the active nodes array and delist the active stucked nodes
        uint[] memory _activenodes = activenodes;
        uint[] memory _waitingnodes = waitingnodes;
