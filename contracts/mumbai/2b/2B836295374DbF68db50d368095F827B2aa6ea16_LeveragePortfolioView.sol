...
contract LeveragePortfolioView is ILeveragePortfolioView, AbstractDependant {
        ...
        returns (uint256)
    {
        ...
        uint256 _poolUR = _poolCoverToken.mul(PERCENTAGE_100).tryDiv(_poolTotalLiquidity);
        res = _poolUR
            .mul(_poolTotalLiquidity)
        ...
        returns (uint256 _amountToDeploy, uint256 _maxAmount)
    {
        ...
        uint256 _netMPL =
            _mpl.mul(ILeveragePortfolio(msg.sender).totalLiquidity()).uncheckedDiv(PERCENTAGE_100);
        ...
        _amountToDeploy = result1.mul(ILeveragePortfolio(msg.sender).totalLiquidity()).tryDiv(
            vStableWeight
        );
    }
