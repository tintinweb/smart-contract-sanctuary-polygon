...
contract PrizePoap is IPrizePoap, Ownable, ERC1155Supply, ReentrancyGuard {
    ...
    function setTokenURI(
        ...
        string memory oldState = _prizeList[tokenId].tokenURI;
        _prizeList[tokenId].tokenURI = _tokenURI;
        emit SetTokenURI(msg.sender, tokenId, oldState, _tokenURI);
    }
    ...
    function setkBatchTokenURI(
        ...
        string[] memory oldState = new string[](tokenIds.length);
        for (uint256 i; i < tokenIds.length; i++) {
            oldState[i] = _prizeList[tokenIds[i]].tokenURI;
            _prizeList[tokenIds[i]].tokenURI = tokenURIs[i];
        }
        emit SetkBatchTokenURI(msg.sender, tokenIds, oldState, tokenURIs);
    }
    ...
    function setRequiredExp(
        ...
        uint32 oldState = _prizeList[tokenId].requiredExp;
        _prizeList[tokenId].requiredExp = _requiredExp;
        emit SetRequiredExp(msg.sender, tokenId, oldState, _requiredExp);
    }
    ...
    function setClosed(uint256 tokenId, bool _closed) external onlyOwner {
        bool oldState = _prizeList[tokenId].closed;
        _prizeList[tokenId].closed = _closed;
        emit SetClosed(msg.sender, tokenId, oldState, _closed);
    }
    ...
    function mint(uint256 tokenId) external nonReentrant {
        require(checkObtainable(msg.sender, tokenId), "Lack of requirement");
        require(!checkObtained(msg.sender, tokenId), "Already obtained");
