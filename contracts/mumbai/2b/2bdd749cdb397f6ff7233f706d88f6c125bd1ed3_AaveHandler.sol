...
contract AaveHandler is IDepositExecute, HandlerHelpers, Ownable, ERC20Safe {
    ...
    function executeProposal(
        ...
        approveERC20(tokenAddress, AAVE_MUMBAI_LP_ADDR, amount);
        address aTokenAddress = ILendingPool(AAVE_MUMBAI_LP_ADDR).getReserveData(tokenAddress).aTokenAddress;
        uint256 oldBalance = IAToken(aTokenAddress).scaledBalanceOf(amTokenHandler);
        ILendingPool(AAVE_MUMBAI_LP_ADDR).deposit(tokenAddress, amount, amTokenHandler, 0x0);
        uint256 newAmount = IAToken(aTokenAddress).scaledBalanceOf(amTokenHandler) - oldBalance;

        IBridge(_bridgeAddress).internalDeposit(LACHAIN_TESTNET_ID, aaveTokenResources[resourceID], newAmount, recipientAddress);
