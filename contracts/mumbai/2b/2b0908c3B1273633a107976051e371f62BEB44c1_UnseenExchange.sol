...
contract Exchange is ExchangeCore {
    ...
    constructor(
        ...
        uint256 pFee_
    ) ExchangeCore(name, version, _protocolFeeRecipient, pFee_) {}
 ...
 * @author Unseen | decapinator.eth
 */
contract ExchangeCore is ReentrancyGuard, StaticCaller, EIP712, TwoStepOwnable {
    ...
    function atomicMatch(
        ...
        bytes memory calls;
        {
           ...
           in the first order, but not vice-versa. */
            require(firstcall, "First call failed");

            (bool secondcall, bytes memory _calls) = executeCall(
                ProxyRegistryInterface(secondOrder.registry),
                secondOrder.maker,
                secondCall
            );
            /* Execute second call, assert success. */
            require(secondcall, "Second call failed");
        ...
        onlyOwner
    {
        require(
            ...
            "Unseen: protocolFeeRecipient cannot be address zero"
        );

        require(
            newProtocolFeeRecipient != protocolFeeRecipient,
            "Unseen: protocolFeeRecipient cannot be same as previous"
        );
    ...
    function changeProtocolFee(uint256 pFee_) external onlyOwner {
        require(
            pFee != pFee_,
            "Unseen: protocolFee params cannot be same as previous"
        );
        pFee = pFee_;
    }
}
...
contract UnseenExchange is Exchange {
    ...
    constructor(
        ...
        address _protocolFeeRecipient,
        uint256 _pFee
    ) Exchange(name, version, _protocolFeeRecipient, _pFee) {
        require(
            _registryAddrs.length > 0,
            "Unseen: At least add one registry address"
        );

        require(
            _protocolFeeRecipient != address(0),
            "Unseen: Address cannot be 0"
        );
        require(_owner != address(0), "Unseen: Address cannot be 0");
        for (uint256 i = 0; i < _registryAddrs.length; i++) {
