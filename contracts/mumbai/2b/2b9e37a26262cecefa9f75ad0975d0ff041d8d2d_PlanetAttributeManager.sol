...
contract PlanetAttributeManager is BreedPlanetBase {
    ...
    function setUpBloodline(
        ...
        // planetTagsPerBloodline[bloodline] = planetTagArray;
        for (uint256 i = 0; i < planetTagArray.length; i++) {
            planetTagsPerBloodline[bloodline][i] = planetTagArray[i];
        }
    }

    // function resetBloodline(uint256 bloodline) external onlyOwner {
    //     delete (planetTagsPerBloodline[bloodline]);
    // }
    ...
    function breed(
        ...
        if (shouldUseMiniBlockhole) {
            ...
            // require(
            //     breedContract.getApeironGodiverseCollection().isApprovedForAll(
            //         userAddress,
            //         address(this)
            //     ) &&
            //         breedContract.getApeironGodiverseCollection().balanceOf(
            //             userAddress,
            //             breedContract.miniBlackholeTokenId()
            //         ) >=
            //         targetApeironGodiverseCollectionNumber,
            //     "MBH approval or balance is not correct"
            // );
            if (
                !(breedContract
                    .getApeironGodiverseCollection()
                    .isApprovedForAll(userAddress, address(this)) &&
                    breedContract.getApeironGodiverseCollection().balanceOf(
                        userAddress,
                        breedContract.miniBlackholeTokenId()
                    ) >=
