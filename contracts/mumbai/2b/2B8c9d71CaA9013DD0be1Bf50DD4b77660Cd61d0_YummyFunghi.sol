...
contract YummyFunghi is ERC721Enumerable, EIP712, Ownable {

    using Counters for Counters.Counter;
    string private constant SIGNING_DOMAIN = "YUFI";
    ...
    bool private _whitelistPaused = false;
    ...
    address private _signer = 0xB9CDCA792D0057de8Cf02511af1e449756EBf6A6;
    mapping (uint256 => bool) public redeemed;

    event YUFICreated(address mintedBy, uint256 indexed id);
    ...
    constructor(uint256 maxSupply, uint256 promoTickets, string memory initialBaseURI, string memory initialContractURI/*, address[] memory team*/) ERC721("Yummy Funghi", "YUFI") EIP712(SIGNING_DOMAIN, SIGNING_VERSION)  {
        MAX_SUPPLY = maxSupply;
        RESERVED = promoTickets;
    ...
    function mint(uint256 num) external payable nonReentrant {
        require( !_salePaused,                                       "Sale paused." );
        require( num <= _maxBookingTransactionSize,                  "You can't book that many in one transaction." );
        require( msg.value >= PRICE * num,                          "The Amount of Ether sent is not correct" );
        require( !_isSoldOut(),                                      "Sold out." );

        _mintInternal(msg.sender, num);
    }

    function mintWhitelist(uint256 num, uint256 id, bytes memory signature) external payable nonReentrant {
        require( !_whitelistPaused,                                     "Whitelist sale paused." );
        require( msg.value >= PRICE * num,                              "Ether sent is not correct" );
        require( !_isSoldOut(),                                         "Sold out." );
        require(check(id, signature) == _signer,                  "Invalid Voucher" );
    ...
    */
    ...
    function airdropToOne(address to, uint256 amount) external onlyOwner {
        require( amount <= RESERVED, "This would exceed reserved supply" );
    ...
    function airdrop(address[] memory addresses) external onlyOwner {
        require( addresses.length <= RESERVED, "This would exceed reserved supply" );

        for (uint256 i = 0; i < addresses.length; i++) {
            _mintInternal(addresses[i], 1);
            RESERVED--;
        }
    }
    ...
    function mintTeam() external onlyOwner {
        for(uint256 i = 0; i < _team.length; i++) {
            _mintInternal(_team[i], 1);
        }
        RESERVED -= _team.length;
    }
    ...
    */
    ...
    function _isSoldOut() private view returns (bool) {
        return !(totalSupply() < MAX_SUPPLY - RESERVED);
    }
    ...
    function _mintInternal(address to, uint256 amount) private {
        ...
        for(uint256 i = 0; i < amount; i++){
            uint256 id = supply + i;
            _safeMint( to, id );
            emit YUFICreated(msg.sender, id);
        }
    }
