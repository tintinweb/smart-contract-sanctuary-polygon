...
contract FPMMDeterministicFactory is Roles, ERC1155Receiver {
    ...
    IGovernor public governance;
    FixedProductMarketMaker public implementationMaster;
    address internal currentFunder;
    address public masterPoint;

    constructor(
        FixedProductMarketMaker _impl,
        address[] memory _admins,
        IGovernor _governance,
        address _masterPoint
    ) public Roles(_admins)
    {
        governance = _governance;
        implementationMaster = _impl;
        masterPoint = _masterPoint;
    }

    function changeGovernorAddress(IGovernor _newGovernor) public onlyRole(GUARDIAN_ROLE) {
        governance = _newGovernor;
    }

    function changeMasterPointAddress(address _masterPoint) public onlyRole(GUARDIAN_ROLE) {
        masterPoint = _masterPoint;
    }
    ...
    function predictDeterministicAddress(bytes32 _salt) public view returns (address) {
        return Clones.predictDeterministicAddress(address(implementationMaster), _salt);
    }
    ...
    returns (address)
    {
        require(isGuardian(msg.sender) || msg.sender == address(governance), "Invalid creator!");
        currentFunder = msg.sender;
        address fixedProductMarketMaker = _initMarket(
            salt,
            conditionalTokens,
            collateralToken,
            proposer,
        ...
        if (address(collateralToken) == masterPoint) {
            IPoint(address(collateralToken)).grantWhitelist(fixedProductMarketMaker);
            IPoint(address(collateralToken)).grantWhitelist(address(conditionalTokens));
        }

        if (initialFunds > 0) {
            uint allowance = collateralToken.allowance(currentFunder, address(this));
            collateralToken.transferFrom(currentFunder, address(this), initialFunds);
            collateralToken.approve(address(fixedProductMarketMaker), initialFunds);
            IFixedProductMarketMaker(fixedProductMarketMaker).addFunding(initialFunds, distributionHint);
            IERC20(fixedProductMarketMaker).transfer(currentFunder, IERC20(fixedProductMarketMaker).balanceOf(address(this)));
    ...
    function _initMarket(
        ...
        address fixedProductMarketMaker = Clones.cloneDeterministic(address(implementationMaster), _salt);

        IFixedProductMarketMaker(fixedProductMarketMaker).initialize(
            conditionalTokens,
            collateralToken,
            proposer,
            treasury,
...
contract ConditionalTokens is Roles, ERC1155 {
    ...
    /// @param questionId An identifier for the question to be answered by the oracle.
    /// @param outcomeSlotCount The number of outcome slots which should be used for this condition. Must not exceed 256.
    function prepareCondition(address oracle, bytes32 questionId, uint outcomeSlotCount) external onlyRole(GUARDIAN_ROLE) {
    ...
    function splitPosition(
        ...
        if (freeIndexSet == 0) {
            ...
            if (parentCollectionId == bytes32(0)) {
                // TODO: cannot call if address(this) wasn't configured as WHITELIST_ROLE
    ...
    function redeemPositions(IERC20 collateralToken, bytes32 parentCollectionId, bytes32 conditionId, uint[] calldata indexSets, uint outcomeIndex, address leader) external {
        require(payoutDenominator[conditionId] > 0, "result for condition not received yet");
        uint outcomeSlotCount = payoutNumerators[conditionId].length;
        require(outcomeSlotCount > 0, "condition not prepared yet");
        
        (uint totalPayout, uint[] memory payoutStakes) = _redeemPositions(
            collateralToken,
            parentCollectionId,
            conditionId, 
            indexSets,
        ...
        emit PayoutRedemption(msg.sender, collateralToken, parentCollectionId, conditionId, indexSets, totalPayout, payoutStakes, outcomeIndex, leader);
    }
    ...
    function _redeemPositions(
        ...
        uint outcomeSlotCount,
        uint fullIndexSet
    ) internal returns (uint totalPayout, uint[] memory payoutStakes) {
        payoutStakes = new uint[](indexSets.length);
...
contract FixedProductMarketMaker is ERC20, ERC1155Receiver, Initializable {
    ...
    event FPMMProposerFeeRedeemed(
        address indexed proposer,
        uint totalProposerFee
    );
    ...
    uint internal totalProposerFee;
    ...
    private
    {
        uint conditionsLength = conditionIds.length;

        while (conditionsLength > 0) {
            uint index = conditionsLength - 1;
    ...
    function collectedProposerFee() external view returns (uint) {
        return totalProposerFee;
    }
    ...
    function redeemProposerFee() public {
        require(msg.sender == proposer, "Only proposer can redeem proposer fee!");

        for (uint i = 0; i < conditionIds.length; i++) {
            require(conditionalTokens.payoutDenominator(conditionIds[i]) > 0, "result for condition not received yet");
        }

        require(collateralToken.transfer(msg.sender, totalProposerFee), "withdrawal transfer failed");
        emit FPMMProposerFeeRedeemed(msg.sender, totalProposerFee);
    }
    ...
    function calcBuyAmount(uint investmentAmount, uint outcomeIndex) public view returns (uint) {
        ...
        uint[] memory poolBalances = getPoolBalances();
        uint investmentAmountMinusFees = investmentAmount.sub(investmentAmount.mul(fee.add(tradingFee)) / ONE);
    ...
    function calcSellAmount(uint returnAmount, uint outcomeIndex) public view returns (uint outcomeTokenSellAmount) {
        ...
        uint[] memory poolBalances = getPoolBalances();
        uint returnAmountPlusFees = returnAmount.mul(ONE) / ONE.sub(fee.add(tradingFee));
    ...
    function buy(uint investmentAmount, uint outcomeIndex, uint minOutcomeTokensToBuy, bool isStake, address leader) external {
        ...
        uint feeAmount = investmentAmount.mul(fee.add(tradingFee)) / ONE;
        ...
        uint balanceOf = conditionalTokens.balanceOf(address(this), positionIds[outcomeIndex]);

        // Transfer back ConditionalTokens to user
        conditionalTokens.safeTransferFrom(address(this), msg.sender, positionIds[outcomeIndex], outcomeTokensToBuy, "");

        // Do all the thing related to fee
        _conductFeeTransfer(feeAmount);
        if (isStake) {
            require(leader != address(0), "invalid leader");
            userStakeAmount[msg.sender][outcomeIndex] = userStakeAmount[msg.sender][outcomeIndex].add(outcomeTokensToBuy);
            emit FPMMStake(msg.sender, investmentAmount, feeAmount, outcomeIndex, outcomeTokensToBuy, leader);
        } else {
            emit FPMMBuy(msg.sender, investmentAmount, feeAmount, outcomeIndex, outcomeTokensToBuy);
        }
    }

    function sell(uint returnAmount, uint outcomeIndex, uint maxOutcomeTokensToSell, bool isUnstake, address leader) external {
        ...
        if (isUnstake) {
            require(leader != address(0), "invalid leader");
            require(returnAmount <= userStakeAmount[msg.sender][outcomeIndex], "maximum unstake amount exceed");
            userStakeAmount[msg.sender][outcomeIndex] = userStakeAmount[msg.sender][outcomeIndex].sub(outcomeTokensToSell);
        }
        conditionalTokens.safeTransferFrom(msg.sender, address(this), positionIds[outcomeIndex], outcomeTokensToSell, "");

        uint feeAmount = returnAmount.mul(fee) / (ONE.sub(fee.add(tradingFee)));
        ...
        _conductFeeTransfer(feeAmount);
        if (isUnstake) {
            emit FPMMUnstake(msg.sender, returnAmount, feeAmount, outcomeIndex, outcomeTokensToSell, leader);
        } else {
    ...
    function _conductFeeTransfer(uint feeAmount) internal {
        uint tradingFeeAmount = feeAmount.mul(tradingFee.mul(ONE).div(tradingFee.add(fee))) / ONE;
        uint proposerFeeAmount = tradingFeeAmount.mul(proposerFee).div(ONE);
        uint deductedFeeAmount = feeAmount.sub(tradingFeeAmount);

        feePoolWeight = feePoolWeight.add(deductedFeeAmount);
        totalProposerFee = totalProposerFee.add(proposerFeeAmount);
        tradingFeeAmount = tradingFeeAmount.sub(proposerFeeAmount);

        // Transfer deducted collateral amount from swap fee to treasury
        require(collateralToken.transfer(treasury, tradingFeeAmount), "cost transfer failed");
    }
}
...
contract Roles is AccessControl {
    ...
    function grantWhitelist(address _whitelist) external onlyRole(GUARDIAN_ROLE) {
...
interface IPoint {
    function grantWhitelist(address _whitelist) external;
