    ...
    ERC20VotesUpgradeable
{
    ...
    uint8 private precisionDifference;
    ...
    mapping(address => int256) private userEVShareTracker;
    ...
    ///@param _precisionDifference precision difference
    ///@param _poolName pool name
    ///@param _sportsBookContract sports book contract address
    function initialize(
        string memory _name,
        ...
        uint8 _precisionDifference,
        string memory _poolName,
        address _sportsBookContract
        ...
        require(
            address(_sportsBookContract) != address(0),
            "HOUSEPOOL:Sports Book contract Address is zero"
        );
        ...
        poolName = _poolName;
        sportsBookContract = _sportsBookContract;
    ...
    function getuserEVTrackerForTheUser(address _user)
        ...
        returns (int256)
    {
        return userEVShareTracker[_user];
    }
    ...
    {
        require(betinformation.length <= 25, "HOUSEPOOL: can process only 25 bets or less");
    ...
    function updateBets(
        ...
        require(_Id.length <= 25 && _payout.length <= 25, "HOUSEPOOL: can process only 25 bets or less");
        ...
        onlyRole(HOUSE_POOL_DATA_PROVIDER)
    {
        ...
        require(_Id.length <= 25 && _result.length <= 25, "HOUSEPOOL: can process only 25 bets or less");
        for (uint256 i = 0; i < _Id.length; i++) {
            if (_result[i] == 1) {
    ...
    function calculateNewEVValue(
        ...
        int256 newEVForUser = ((int256(_newAmount) * _newEV) +
            (int256(balanceOf(_toUser)) * userEVShareTracker[_toUser])) /
    ...
    function stake(address onBehalfOf, uint256 amount) public {
        ...
        userEVShareTracker[msg.sender] = calculateNewEVValue(
            lpTokensToMint,
            voi.expectedValue,
            msg.sender
        );
        _mint(onBehalfOf, lpTokensToMint);
        liquidity += uint256(int256(runningTotalDeposits) + pendingStakes);
        updateAttributes();
        FarmInfo memory farm = updateFarm();
        UserInfo storage user = userInfo[msg.sender];
    ...
    function unStake(address to, uint256 amount) public {
        ...
        require(
            int256(amount) <= ceil(maxUserWithdrawalAmount, 10**0),
    ...
    function getMaxWithdrawal(address _user) public view returns (int256) {
        int256 userShare = int256(
            ((balanceOf(_user) * 10**MAX_PRECISION) / totalSupply())
        );
        int256 maxUserWithdrawalAmount;
        if (((userShare * voi.expectedValue) / 10**18) < 0) {
            maxUserWithdrawalAmount = ((userShare * int256(totalValueLocked)) /
                10**18);
        } else if (
            (((userShare) * voi.expectedValue) / 10**18 >= 0) &&
            (((userShare) * voi.expectedValue) / 10**18 <
                userEVShareTracker[_user]) &&
            (userEVShareTracker[_user]) >= 0
        ) {
            maxUserWithdrawalAmount = ((userShare * int256(totalValueLocked)) /
                10**18);
        } else if (
            ...
            maxUserWithdrawalAmount =
                ((userShare * (int256(totalValueLocked) - voi.expectedValue)) /
                    10**18) +
                (userEVShareTracker[_user]);
        }
    ...
    function _transfer(
        ...
        if (from != to) {
            ...
            userEVShareTracker[to] = calculateNewEVValue(
                amount,
                userEVShareTracker[from],
                to
            );
    ...
    /// @notice internal function to verify
    ///@param _amount value that is required to perfrom a stake
    function verify(uint256 _amount) internal view {
        require(_amount  > 1 * 10**18, "HOUSEPOOL: INVALID AMOUNT");
        require(
            _amount  != 0 && _amount  <= stakedToken.balanceOf(msg.sender),
            "HOUSEPOOL:INVALID_AMOUNT"
        );
    }
