...
contract JuanFourRafCon is VRFConsumerBaseV2 {
    ...
    constructor(uint64 subscriptionId,IERC20 lease_token,IERC20 main_token) VRFConsumerBaseV2(vrfCoordinator) {
        COORDINATOR = VRFCoordinatorV2Interface(vrfCoordinator);
        s_owner = payable(msg.sender);
        s_subscriptionId = subscriptionId;
        token_address = main_token;
        lease_address = lease_token;
        prizeSelection[50] = true;
        prizeSelection[75] = true;
        prizeSelection[90] = true;
    }
    ...
    function rakeCollection(uint room_number) private{
        ...
        // token_address.transfer(raffle_room[room_number].lessee,raffle_room[room_number].l_rake);
        emit RakeTransfer(room_number,rakeRecord[room_number].l_rakeCollected,raffle_room[room_number].lessee);   
    }

    function roomLeaseCheck(uint room_number) public{
        if(raffle_room[room_number].lease_duration < block.timestamp){
          emit OpenRaffleLease(room_number,block.timestamp);
        }
    }
    ...
    //TODO:
    //1. convert amount into second //issue is uint
    function hostARaffle(uint _amount, uint newEntryfee, uint room_number, uint grandPrize,uint limit) public {
        ...
        lease_address.transferFrom(msg.sender, address(this), _amount);
        raffle_room[room_number].active = true;
        ...
        raffle_room[room_number].objid = 0;
        ...
        if(grandPrize == 90){
            computePrizes(room_number,90,5,5,fullrake);
        }
        ...
        emit BurnedToken(room_number,_amount,block.timestamp);
        resetContract(room_number);
    }

    function sponsorARaffle(uint _amount,uint prize_amount, uint ticker, uint room_number, uint grandPrize,uint limit,uint recordid) public {
        require(isActive == true, "This room is paused at the moment.");
        uint rentTime = raffle_room[room_number].lessee_updatedAt + raffle_room[room_number].lease_duration;
        require(block.timestamp > rentTime,"Lease is still active");
        uint256 allowance = lease_address.allowance(msg.sender,address(this));
        require(allowance > 0, "You don't have enough allowance to host.");
        require(_amount > raffle_room[room_number].lease_duration , "Please enter enough amount");
        require(limit >= 3, "Limit should be 3 or higher");
        require(limit <= 10, "Limit should not be more than 10");
        require(prizeSelection[grandPrize] == true, "Invalid grand prize");
       
        lease_address.transferFrom(msg.sender, address(this), _amount);
        token_address.transferFrom(msg.sender, address(this), prize_amount); //
        RoomPrizePool[room_number] = prize_amount;
        raffle_room[room_number].active = true;
        raffle_room[room_number].lessee = msg.sender;
        // raffle_room[room_number].entryfee //default is 1 ether, if sponsors want it free, edit through admin
        raffle_room[room_number].limit = limit;
        raffle_room[room_number].pot = ticker * limit; //ticker * the number of participants, ex: 1 * 10 = 10 SWRTC. 
        raffle_room[room_number].burn_rate = raffle_room[room_number].pot * 1 / 100; //1% of of the whole balance is burned... as there is no way to withdraw.
        raffle_room[room_number].lessee_updatedAt = block.timestamp;
        raffle_room[room_number].lease_duration = _amount;
        raffle_room[room_number].objid = recordid;
        ...
        emit NewLease(room_number,msg.sender,ticker,raffle_room[room_number].pot,limit,true,raffle_room[room_number].prizes[0],1);
        resetContract(room_number);
    }
    ...
    function breakLease(uint room_number) public{
        ...
        token_address.transfer(msg.sender, rakeRecord[room_number].l_rakeCollected);
        rakeRecord[room_number].l_rakeCollected = 0;
        raffle_room[room_number].active = false;
        raffle_room[room_number].lessee = s_owner;
        raffle_room[room_number].lease_duration = 0;
        raffle_room[room_number].objid = 0;
        emit OpenRaffleLease(room_number,block.timestamp);
    }

    function extendLease(uint room_number,uint amount) public{
        require(msg.sender == raffle_room[room_number].lessee,"This account is not allowed to break this contract.");
        require(amount > 0,"Please enter a valid amount");
        token_address.transfer(msg.sender, rakeRecord[room_number].l_rakeCollected);
        raffle_room[room_number].lease_duration = raffle_room[room_number].lease_duration + amount;
    }
