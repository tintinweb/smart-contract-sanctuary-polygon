...
contract Deposit is ReentrancyGuard {
    ...
    uint256 constant BASIS_POINTS_MIL = 1000000;
    ...
    constructor(
        ...
        if (stakePlan == StakePlanContext.StakePlan.FLEXI_PLAN) {
            addInterestAllocation((_amount.mul(_interestRate).div(BASIS_POINTS_MIL).mul(3)), depositDate.add(_harvestInterval.mul(3)));
            addInterestAllocation((_amount.mul(_interestRate).div(BASIS_POINTS_MIL)), depositDate.add(_harvestInterval.mul(4)));
            addInterestAllocation((_amount.mul(_interestRate).div(BASIS_POINTS_MIL)), depositDate.add(_harvestInterval.mul(5)));
            addInterestAllocation((_amount.mul(_interestRate).div(BASIS_POINTS_MIL)), depositDate.add(_harvestInterval.mul(6)));
        } else {
        addInterestAllocation((_amount.mul(_interestRate).div(BASIS_POINTS_MIL)), depositDate.add(_harvestInterval.mul(12)));
        addInterestAllocation((_amount.mul(_interestRate).div(BASIS_POINTS_MIL)), depositDate.add(_harvestInterval.mul(13)));
        addInterestAllocation((_amount.mul(_interestRate).div(BASIS_POINTS_MIL)), depositDate.add(_harvestInterval.mul(14)));
        addInterestAllocation((_amount.mul(_interestRate).div(BASIS_POINTS_MIL)), depositDate.add(_harvestInterval.mul(15)));            
        }
    }
    ...
    function closeDeposit() external nonReentrant onlyStakingContract { 
        ...
        if (unstakePenaltyUntilDate() < block.timestamp) {
            ...
            uint256 penalty = (amount * unstakePenaltyRateBP) / BASIS_POINTS_MIL;
