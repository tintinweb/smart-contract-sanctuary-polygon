...
contract NFTCapsule_Series is Pausable, INFTCapsule {
    ...
    function getTokenId() external override whenNotPaused returns (uint256 tokenId) {
        ...
        // 取得隨機編號
        uint256 picId = (uint256(sha256(abi.encodePacked(uint160(msg.sender) + block.timestamp + totalSupply))) % picAmount) + 1;
        uint256 fragmentId = (uint256(sha256(abi.encodePacked(uint160(msg.sender) + block.timestamp + picId + totalSupply))) % fragmentAmount) + 1;
