...
contract mPCKT is Ownable, IERC20, IERC20Metadata, AccessControlEnumerable, ReentrancyGuard, Pausable {
...
contract VaultMiner is Context, Ownable, IERC1155Receiver, ReentrancyGuard {
   ...
   // event SetHarvestToken(address indexed user, IERC20 token);
    ...
    function sellWorkers() public isInitialized nonReentrant {
        require(activeFeatures.vaultActive && activeFeatures.minerBuy && (!minerSettings.noSell || block.timestamp > (userStats[msg.sender].lastSell + minerSettings.sellDuration)), 'too soon to sell');
        ...
        if(harvestToken[msg.sender] == IERC20(address(0))){
            // send to the user
            (sent,) = payable(msg.sender).call{value: (workerValue-fee)}("");
            require(sent,"send failed");
        } else {
            _swapNativeForToken(workerValue-fee, harvestToken[msg.sender], msg.sender); 
        }
    ...
    function contractBuyWorkers(address _user, address _ref) public payable isInitialized nonReentrant {
    ...
    function _buyWorkers(address user, uint256 amount, address ref,  bool fromSwap) internal {
        ...
        require(activeFeatures.vaultActive && activeFeatures.minerBuy && amount >= minerSettings.minBuy && amount <= minerSettings.maxBuy, 'Cant Buy');
...
contract Vault is VaultMiner {
    ...
    // Nft Rewards Address
    address public nftRewardsAddress;
    ...
    uint256 public burnThresh = 100;
    uint256 public shareMod = 50;

    // scale value of shares for the token
    uint256 public tokenShare = 5;
    uint256 public tokenPercent = 10;
...
//    event Claimed(address indexed account, uint256 nftId, uint256 amount);
    ...
    constructor (
        ...
        address _lpAddress, 
        address _router
    ) VaultMiner(_devWallet, _treasuryWallet, _investWallet, _token, _nftContract, _router ) {
        // nftContract = _nftContract;
    ...
    function setThresholds(uint256 _packThresh, uint256 _burnThresh, uint256 _shareMod, uint256 _tokenmShare, uint256 _tokenPercent) public onlyOwner {
        ...
        burnThresh = _burnThresh;
        shareMod = _shareMod;
        tokenShare = _tokenmShare;
    ...
    function setAddresses(
        ...
        nftRewardsAddress = _nftRewardsAddress;
...
/*
    ...
    function lock(uint256 _nftId) public nonReentrant {
        ...
        require(
            ...
            (nftContract.balanceOf(address(this), _nftId) - nftInfo[_nftId].lockedNfts) > 0, 'Cant Lock');
    ...
    function claimLock(uint256 _nftId) public nonReentrant {
        require(activeFeatures.vaultActive && activeFeatures.giveNfts && userLocks[msg.sender][_nftId].tokenAmount > 0, 'Not Locked');
    ...
    function tokensForShares(uint256 _amount) public nonReentrant {
        ...
        _addShares(msg.sender,adjustedShares * tokenBurnMultiplier, true );
    ...
    function giveNft(uint256 _nftId, uint256 _amount) public nonReentrant {
        require(activeFeatures.vaultActive && activeFeatures.giveNfts && nftInfo[_nftId].sharePoints > 0  && !nftInfo[_nftId].isDisabled && nftContract.balanceOf(address(msg.sender), _nftId) >= _amount ,'cant give');
        ...
        if(nftInfo[_nftId].toBurn && (nftContract.maxSupply(_nftId) - nftContract.balanceOf(address(burnAddress), _nftId) ) <= burnThresh){
        ...
        if(nftInfo[_nftId].toBurn){
            ...
            if(inNftRewards[_nftId] && (nftContract.balanceOf(address(this), _nftId) - nftInfo[_nftId].lockedNfts) >= packThresh){
                toSend = address(nftRewardsAddress);
            }
    ...
    function adjustTokenShares(uint256 _amount) public view returns(uint256){
        return ((_calcAdjustedShares(tokenShare, tokenPercent) * _amount) * 10) / shareMod;
    }

    function adjustNftShares(uint256 _nftId) public view returns(uint256) {
        return ((_calcAdjustedShares(nftInfo[_nftId].sharePoints, nftInfo[_nftId].sharePercent ) * 1 ether) * 10) / shareMod;
    }
/*
    ...
    function useNft(uint256 _nftId) public isInitialized nonReentrant{
        require(activeFeatures.vaultActive && activeFeatures.useNft && minerNftInfo[_nftId].nftType > 0 && minerNftInfo[_nftId].nftType < 3 && !minerNftInfo[_nftId].isDisabled && nftContract.balanceOf(msg.sender,_nftId) > 0,'Cant Use');
    ...
    function GTFO() public nonReentrant {
        require(userStats[msg.sender].purchaseValue > 0, 'Nothing invested');
    ...
    function _addLiquidity(uint256 tokenamount, uint256 nativeamount) private {
        ...
        // tokenIds[1].approve(address(pancakeRouter), tokenamount);
    ...
    receive() external payable {
        ...
        if(!activeFeatures.lpEnabled || msg.sender == address(pancakeRouter) || msg.sender == address(gatewayAddress)){
            lpBal = 0;
            shareBal = msg.value;
        } else {
