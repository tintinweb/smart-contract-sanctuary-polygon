...
contract A4AAuction is Ownable, IERC721Receiver {
    ...
    //event ExtendAuction(uint256 tokenId);
    ...
    function cancelAuction(uint256 itemID) external {
        require(_listedItemsOwner[itemID] == _msgSender(), "Only the owner can cancel the auction.");
        //require(block.timestamp < _auctionsEndDate[itemID], "The auction is already closed.");
...
/*
    function extendAuction(uint256 itemID, uint256 newEndDate) external {
        require(_listedItemsOwner[itemID] == _msgSender(), "Only the owner can extend the auction.");
        require(block.timestamp < _auctionsEndDate[itemID], "The auction is already closed.");
        require(block.timestamp < newEndDate, "The auction's end date cannot be a past date.");
    ...
    */
    ...
    function claimAuctionResult(uint256 itemID) external onlyOwner {
        ...
        if(_maxBidderByItemID[itemID] == address(0) || _listedItemsMaxPrice[itemID] < _listedItemsMinPrice[itemID]){
            ...
            uint256 auctionAmt = 0;
            if (_auctionFee > 0){
                auctionAmt = _auctionFee * _itemPrice / 100;
                (bool sentFee, ) = _a4A_wallet.call{value: auctionAmt}("");
            ...
            if (authorFee > 0){
                authorAmt = authorFee * _itemPrice / 100;
            ...
            uint256 sellerAmt = _itemPrice - auctionAmt - authorAmt;
