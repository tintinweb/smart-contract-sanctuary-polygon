...
// File: SpaceBank/SpaceBankPolygon.sol
...
contract SpaceBankPolygon is ERC721Enumerable,Ownable{

    uint public MAX_SUPPLY = 5000;
    uint public price = 150 ether;
    uint public tokenId = 111;
    ...
    constructor() ERC721("SpaceBank MobBosses","SBMB"){
        base = "https://spacebank-backend.onrender.com/metadata/";
    }

    function mint(uint _amount) external payable{
        require(!paused,"Execution paused");
        require(_amount <= 10,"Only 10 mints max at a time");
        require(tokenId + _amount <= MAX_SUPPLY,"Max supply exceeded");
        require(msg.value >= price*_amount,"Underpaid");
    ...
    function claimToken(uint[] memory tokenIds,address to) external{
