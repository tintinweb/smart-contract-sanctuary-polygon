    ...
    function withdraw(uint256 _percentage) public payable nonReentrant {
        ...
        if(deposited[msg.sender]) {
            ...
            if (_percentage == (100*(10**18))) {
                ...
                uint256 amountToBeWithdrawn = ((dusdUserBalanceInCon * _percentage)/100);
...
string constant name_ = "DeflenderNFT";
string constant symbol_ = "DFLL";

contract DeflenderNFT is ERC721(name_, symbol_){
    //The library of String operations and used here for uint256
    using Strings for uint256;
    ...
    // returning the base URI unto which tokenId is added to give URL of each NFT
    function _baseURI() internal view virtual override returns (string memory) {
        return "https://ipfs.io/ipfs/Deflend-lender/";
    }
...
contract DefToken is Context, IERC20, IERC20Metadata{
    address _owner;
    mapping(address => uint256) private _balances;
...
contract Dethereum is Context, IERC20, IERC20Metadata{
    address _owner;
    mapping(address => uint256) private _balances;
...
contract Dusd is Context, IERC20, IERC20Metadata{
    address _owner;
    mapping(address => uint256) private _balances;
