...
contract COGGame {
    ...
    mapping(uint256 => mapping(uint256 => SymbolInstance)) public gameMap; // LandId => position => symbol
    ...
    function initGame(uint256 id) public {
        int256 initialPayout = getInitialPayout(uint256(Symbols.Symbols.F1));
        gameMap[id][5] = SymbolInstance(Symbols.Symbols.F1, initialPayout, 0);
        initialPayout = getInitialPayout(uint256(Symbols.Symbols.S1));
        gameMap[id][11] = SymbolInstance(Symbols.Symbols.S1, initialPayout, 0);
        initialPayout = getInitialPayout(uint256(Symbols.Symbols.B1));
        gameMap[id][7] = SymbolInstance(Symbols.Symbols.B1, initialPayout, 0);
        initialPayout = getInitialPayout(uint256(Symbols.Symbols.A3));
        gameMap[id][13] = SymbolInstance(Symbols.Symbols.A3, initialPayout, 0);
        initialPayout = getInitialPayout(uint256(Symbols.Symbols.B4));
        gameMap[id][9] = SymbolInstance(Symbols.Symbols.B4, initialPayout, 0);

        for (uint256 i = 0; i < 20; ++i) {
            if (i != 5 && i != 11 && i != 7 && i != 13 && i != 9) {
                gameMap[id][i] = SymbolInstance(Symbols.Symbols.E4, 0, 0);
            }
        }
        iventory[id].set(uint256(Symbols.Symbols.F1), 1);
        iventory[id].set(uint256(Symbols.Symbols.S1), 1);
        iventory[id].set(uint256(Symbols.Symbols.B1), 1);
        iventory[id].set(uint256(Symbols.Symbols.A3), 1);
        iventory[id].set(uint256(Symbols.Symbols.B4), 1);
        iventory[id].set(uint256(Symbols.Symbols.E4), 15);
    }

    function spin(uint256 id, Symbols.Symbols[] memory symbols) public {
        setMap(id, symbols);
        int256 coin = 0;
        for (uint256 i = 0; i < 20; ++i) {
            coin += getPayout(id, i);
    ...
    function setMap(uint256 id, Symbols.Symbols[] memory symbolsAdd) internal {
        for (uint256 i = 0; i < symbolsAdd.length; ++i) {
            int256 initialPayout = getInitialPayout(uint256(symbolsAdd[i]));
            // gameMap.set(positions[i], Convert.abiEncode(symbolsAdd[i]));
            gameMap[id][i] = SymbolInstance(symbolsAdd[i], initialPayout, 0);
        }
    }

    function updateBalance(uint256 id, int256 coin) public returns (bool) {
        if (coin >= 0) {
    ...
    function getMap(uint256 id) public view returns (string[] memory) {
        ...
        for (uint256 i = 0; i < 20; ++i) {
            SymbolInstance memory symbol = gameMap[id][i];
    ...
    function getInventory(
        ...
        string[] memory sb = new string[](iventory[id].length());
        uint256[] memory qt = new uint256[](iventory[id].length());
        for (uint256 i = 0; i < iventory[id].length(); ++i) {
            (uint256 index, uint256 quantity) = iventory[id].at(i);
            Symbols.Symbols symbol = Symbols.findSymbol(index);
    ...
    function atPosition(
        ...
        return uint256(gameMap[id][position].symbol);
    }
    ...
    function isPositionEmpty(
        ...
        if (gameMap[id][position].symbol != Symbols.Symbols.E4) return false;
        return true;
    }

    function removeSymbol(uint256 id, uint256 position) external {
        Symbols.Symbols symbol = gameMap[id][position].symbol;

        uint256 quantity = iventory[id].get(uint256(symbol));
        iventory[id].set(uint256(symbol), quantity - 1);

        quantity = removedResource[id].get(uint256(symbol));
        removedResource[id].set(uint256(symbol), quantity + 1);
        gameMap[id][position] = SymbolInstance(Symbols.Symbols.E4, 0, 0);
    }

    function destroySymbol(uint256 id, uint256 position) external {
        Symbols.Symbols symbol = gameMap[id][position].symbol;
        uint256 quantity = iventory[id].get(uint256(symbol));
        iventory[id].set(uint256(symbol), quantity - 1);
    ...
    function addSymbol(uint256 id, uint256 position, uint256 index) external {
        Symbols.Symbols symbol = Symbols.findSymbol(index);
        (, uint256 quantity) = iventory[id].tryGet(uint256(symbol));
        iventory[id].set(uint256(symbol), quantity + 1);
        int256 initialPayout = getInitialPayout(uint256(symbol));
        gameMap[id][position] = SymbolInstance(symbol, initialPayout, 0);
    }
    ...
    function addToInventory(
        ...
        Symbols.Symbols symbol = Symbols.findSymbol(index);
        if (iventory[id].contains(uint256(symbol))) {
            quantity += iventory[id].get(uint256(symbol));
        }
        iventory[id].set(uint256(symbol), quantity);
    }

    function getEmptyPosition(uint256 id) external returns (uint256[] memory) {
        delete positions;
        uint256 count = 0;
        for (uint256 i = 0; i < 20; ++i) {
            if (gameMap[id][i].symbol == Symbols.Symbols.E4) {
                positions[count] = i;
            }
        }
        return positions;
