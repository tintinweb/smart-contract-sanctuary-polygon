...
contract RewardProtocol is OwnableUpgradeable {
    ...
    function _createVestingSchedule(
        ...
        // we need to calculate who pays in case of adv or not
        if(_advCardId != 0){
            vestingDuration = _calculateVestableDuration(_advCollectionAddress, _advCardId, _trackId);
            vestingAmount   = _calcualteVestableAmount(_advCollectionAddress, _advCardId, _trackId);

            //nftAddress = _advCollectionAddress;
            //nftTokenId = _advCardId;
        }else{
            vestingDuration = _calculateVestableDuration(_nftContract, _tokenId, _trackId);
            vestingAmount   = _calcualteVestableAmount(_nftContract, _tokenId, _trackId);
    ...
    function createVestingSchedule(
        ...
        bytes32 currentVestingScheduleId = _createVestingSchedule(
            _beneficiary,
            _tokenId,
            _nftContract, 
            _nftCreator, 
