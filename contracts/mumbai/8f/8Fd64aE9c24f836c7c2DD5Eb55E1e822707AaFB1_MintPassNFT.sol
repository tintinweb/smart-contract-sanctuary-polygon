...
//contract MintPassNFT is ERC721, Pausable, Ownable, ERC721Burnable, ERC2981, DefaultOperatorFilterer {    
    ...
    constructor(address _systemAddress, address _mintingFeeRecipient, string memory _uri, uint256 _mintingFee,  uint96 _loyaltyFee) ERC721("MintPass", "MPASS") {
    ...
    function mintMintPass(bytes32 hash, bytes memory signature) public payable returns(uint256){
        ...
        // set metadata
        _setTokenURI(tokenId, string.concat(uri,Strings.toString(tokenId)));
        //_setBaseURI(uri);
