...
contract MetaproofRegistry is AccessControlUpgradeable, PausableUpgradeable {
    bytes32 public constant CONTROLLER_ROLE = keccak256("CONTROLLER_ROLE");
    bytes32 public constant ISSUER_ROLE = keccak256("ISSUER_ROLE");

    event PaymentCurrencySet(address indexed _tokenAddress);
    event ReceiverWalletSet(address indexed _walletAddress);
    event MinimumAmountSet(uint _amount);
    event VerificationAdded(address indexed _address, bytes32 _credentialType, bytes32 uuid);
    event VerificationRemoved(bytes32 uuid);
    event VerificationRevoked(bytes32 uuid);
    event CredentialPaid(address indexed _address, uint _amount);
    ...
    function initialize(address _tokenAddress, address _walletAddress, uint _amount) public initializer {
        __AccessControl_init();
        __Pausable_init();

        _verificationRecordCount = 0;
        receiverWallet = _walletAddress;
        paymentCurrency = _tokenAddress;
        minAmount = _amount;
    ...
    modifier onlyControllers() {
        require(isControllerRole(msg.sender), "Restricted to controllers");
        _;
    }

    modifier onlyIssuers() {
        require(isIssuerRole(msg.sender), "Restricted to issuers");
        _;
    }

    modifier onlyIssuersAndControllers() {
        require(isIssuerRole(msg.sender) || isControllerRole(msg.sender), "Restricted");
        _;
    }

    function isControllerRole(address account) public virtual view returns (bool) {
        return hasRole(CONTROLLER_ROLE, account);
    }

    function isIssuerRole(address account) public virtual view returns (bool) {
        return hasRole(ISSUER_ROLE, account);
    }

    function setPaymentCurrency(address _tokenAddress) public onlyControllers {
        paymentCurrency = _tokenAddress;
        emit PaymentCurrencySet(_tokenAddress);
    }
    ...
    function setReceiverWallet(address _walletAddress) public onlyControllers {
        receiverWallet = _walletAddress;
        emit ReceiverWalletSet(_walletAddress);
    }

    function setMinimumAmount(uint _amount) public onlyControllers {
        minAmount = _amount;
        emit MinimumAmountSet(_amount);
    }
    ...
    function isVerified(address subject, bytes32 credentialType) external view returns (bool) {
        ...
        for (uint i=0; i<subjectRecords.length; i++) {
            ...
            if (!record.revoked && record.expirationTime > block.timestamp && record.credentialType == credentialType) {
                return true;
            }
        }
        return false;
    ...
    /**
     * A function to add an address to the registry
     *
     * The `external` modifier makes a function *only* callable from outside
     * the contract.
     * The onlyOwner modifier is part of Ownable.sol and only owner is allowed to call this function
     */
    function registerVerification(address _address, bytes32 _credentialType, uint256 _expirationTime) external onlyIssuersAndControllers whenNotPaused {
        ...
        emit VerificationAdded(_address, _credentialType, uuid);
    }

    function removeVerification(bytes32 uuid) external onlyIssuersAndControllers {
    ...
    function revokeVerification(bytes32 uuid) external onlyIssuersAndControllers {
    ...
    function pay(uint _amount, bytes32 _credentialType) public whenNotPaused {
        require(_amount >= minAmount, "Invalid amount");
        require(_amount <= getAllowance(), "Not enough allowance");

        _cancelActivePayments(msg.sender, _credentialType);
        IERC20(paymentCurrency).transferFrom(msg.sender, receiverWallet, _amount);

        _paymentRegistry[msg.sender].push(PaymentInfo(block.timestamp, _credentialType, true));
        emit CredentialPaid(msg.sender, _amount);
    }

    function getAllowance() public view returns(uint256) {
        IERC20 token = IERC20(paymentCurrency);
    ...
    function cancelPayment(address _address, bytes32 _credentialType, uint256 _timestamp) external onlyIssuersAndControllers {
        uint count = _paymentRegistry[_address].length;
        for(uint i=0; i<count; i++) {
            PaymentInfo memory record = _paymentRegistry[_address][i];
