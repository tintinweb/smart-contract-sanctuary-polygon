...
contract ValueShareContract is IERC721Receiver {
    ...
    mapping(address => DistributionData) _distribution;
    uint public _totalDistribution;
    ...
    modifier authDistributor() {
        require(_distribution[msg.sender].flag == 1, "Not distributor");
        _;
    }
    ...
    constructor(address erc721FactoryAddress, address fixedExchangeAddress, address oceanAddress, address deployer) {
        _owner = deployer;
    ...
    function addDistribution(address distributor, uint amount) public onlyOwner returns (string memory) {
        if (_distribution[distributor].flag == 1) {
            return "Already added!";
        }
        _distribution[distributor] = DistributionData(amount, 1);
        _totalDistribution += amount;
        emit DistributionAdded();
        return "Added!";
    }

    function getDistribution(address distributor) public view onlyOwner returns (DistributionData memory) {
        return _distribution[distributor];
    }

    function updateDistribution(address distributor, uint amount) public onlyOwner returns (string memory) {
        if (_distribution[distributor].flag == 1) {
            _totalDistribution -= _distribution[distributor].amount;
            _distribution[distributor] = DistributionData(amount, 1);
            _totalDistribution += amount;
    ...
    function getDistribution() public view authDistributor returns (DistributionData memory) {
        return _distribution[msg.sender];
    }

    function claim() public authDistributor {
        IERC20 oceanToken = IERC20(_oceanAddress);
        uint256 balance = oceanToken.balanceOf(address(this));
        uint256 amount = balance * _distribution[msg.sender].amount / _totalDistribution;
        oceanToken.transfer(msg.sender, amount);
        _totalDistribution -= _distribution[msg.sender].amount;
        _distribution[msg.sender].amount = 0;
        emit Claimed();
    }
    ...
    */
    function _setMetaData(
        ...
        string calldata metaDataDecryptorUrl, // https://v4.provider.rinkeby.oceanprotocol.com
