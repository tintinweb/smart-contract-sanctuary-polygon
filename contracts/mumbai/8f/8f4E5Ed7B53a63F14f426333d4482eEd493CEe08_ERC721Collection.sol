...
import "./lib/Config.sol"; // ! REDO: change configuration for minting and other contract calls
    ...
    DefaultOperatorFiltererUpgradeable
{
    ...
    function initialize(
        ...
        // collectionType = _isProduct;

        // _setupContractURI(_contractURI);
        _setupPrimarySaleRecipient(_msgSender(), _floorPrice);
        // setup royalties

        decirContract = _decirContract;
    }

    // ! REPLACE: membership collection should be implemented with 1155
    modifier onlyMembership() {
        // ? False if collection is membership collection 
      require(!collectionType, "Collection: Tiers are applied only for membership NFT");
      _;
    }
    ...
    // TODO this is primary sales and should also support Batch minting

    // ? mint tokens with quantity

    // withdraw from collection
    function withdraw() external onlyOwner {

        uint256 balance = address(this).balance;
        DecirTreasury _decir = DecirTreasury(decirContract);
        uint256 _fees;
    ...
    function mint(uint256 _quantity) external payable {
        ...
        // require(FLOOR_PRICE * _quantity == msg.value, "MINT: Value should not be lower");
        require(totalSupply() + _quantity <=  MAX_SUPPLY, "MINT: Cannot be higher than  max supply");
    ...
    function mintToCaller(
        ...
        // ! REQUIRE: quantity <= maxPublicTx
        // ! REQUIRE: quantity <= maxAllowed
        // ! REQUIRE: totalSupply()  <= max_supply
        bool isProduct = collectionType;
        ...
        if(isProduct) {
            ...
            require(TierInfo[_tierId].totalSupply - 1 >= _suppliedTokens[_tierId], "Membership: Sold out.");
            require(TierInfo[_tierId].floorPrice == msg.value, "Membership: Price does not match");
    ...
    function burn(uint256 _tokenId) public  {
        ...
        require(
            ERC721AUpgradeable.ownerOf(_tokenId) == owner(),
    ...
    */
    ...
    function setTiers(Config.TierConfig[] memory _tierConfigs) public onlyOwner onlyMembership {
        for (
            uint256 tierIndex = 0;
            tierIndex < _tierConfigs.length;
            tierIndex++
        ) {
            bytes32 _givenTier = sha256(abi.encode(_tierConfigs[tierIndex].tierId));
    ...
    function setTierInfo(bytes32 _givenTier, uint256 _floorPrice, uint256 _totalSupply) internal  {
    ...
    function GetTierStatus(string memory _tierId) public view returns(uint256) {
        bytes32 _givenTier = sha256(abi.encode(_tierId));
