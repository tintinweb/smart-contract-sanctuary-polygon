...
contract TheRanchBullsMintAndReward is 
    ...
    uint public bulls_in_existence = 2500;
    ...
    mapping(address => uint256) public myDroveTeamCount;   // partner mapping; msg.sender  ==> who referred them

   
    // raffle slippage winners 
    address public usdcTaxWinner;
    address public wbtcTaxWinner;
    ...
    event WinnerPicked(
        ...
        address indexed usdcTaxWinner, 
        address indexed wbtcTaxWinner, 
    ...
    event withdrawUSDCRewardsForAddressEvent(
        ...
        uint256 indexed taxCollectedAmt
    );
    ...
    constructor(
        ...
        string memory _initGraveyardURI,
        ...
        usdcTaxWinner = _coreteam1;
        wbtcTaxWinner = _coreteam1;
    ...
    */
    function mint(uint256 _tokenQuantity, bool _enterRaffle) public payable {
        ...
        mintingToken.approve(msg.sender,totalTransactionCost);
    ...
    address[] addressesToPayMaintenanceFees; 
    ...
    address[] allAddressThatHaveEverBeenRewarded; 
    ...
    function updateMaintenanceFeesForTheMonth() external onlyOwner {
        ...
        for( uint i; i < addressesToPayMaintenanceFees.length; i++) {
            address _ownerOfNFTs = addressesToPayMaintenanceFees[i];
        ...
        updateMonthsBehindMaintenanceFeeDueDate();
    }


    function updateMonthsBehindMaintenanceFeeDueDate() internal onlyOwner {
        for( uint i; i < allAddressThatHaveEverBeenRewarded.length; i++) {
            address _address = allAddressThatHaveEverBeenRewarded[i];
    ...
    function fundAndRewardBulls(uint _startingIndex, uint _endingIndex, uint256 _totalAmountToDeposit) public payable onlyOwner {
        ...
        uint startIndex = _startingIndex;
        uint endIndex = _endingIndex;

        coreTeam_1_amt += _totalAmountToDeposit * coreTeam_1_percent / 100;
        coreTeam_2_amt += _totalAmountToDeposit * coreTeam_2_percent / 100;
        uint256 disperableRewardBalance = _totalAmountToDeposit * (100 - (coreTeam_1_percent + coreTeam_2_percent)) / 100; 
        uint256 payout_per_nft = disperableRewardBalance / ((endIndex - startIndex) + 1);
        ...
        emit fundingEvent(_totalAmountToDeposit);
    }
    ...
    **/
    function payMaintanenceFees() external nonReentrant {
        ...
        if (_balance >= _feesDue){
            ...
            mintingToken.approve(msg.sender,amt_needed);
    ...
    function fulfillRandomWords(
        ...
        address recentWinner = dailyRafflePlayers[indexOfWinner];
        ...
        updateUsdcBonus(recentRaffleWinner, dailyRaffleBalance);
        ...
        // USDC TAX WINNER // 
        uint256 usdcTaxWinnerIndex =  randomWords[1] % _tokenSupply.current();
        address usdcTaxWinner = ownerOf(usdcTaxWinnerIndex);


        // WBTC TAX WINNER // 
        uint256 wbtcTaxWinnerIndex =  randomWords[2] % _tokenSupply.current();
        address wbtcTaxWinner = ownerOf(wbtcTaxWinnerIndex);
        ...
        lastTimeStamp = block.timestamp;
        raffleMintState = RaffleMintState.OPEN;
        emit WinnerPicked(recentWinner,usdcTaxWinner, wbtcTaxWinner, raffleWinningAmount);
    ...
    //     uint256 numberToUse = 1231656564651654;
    //     uint256 indexOfWinner = numberToUse % dailyRafflePlayers.length;
    //     address recentWinner = dailyRafflePlayers[indexOfWinner];
    //     recentRaffleWinner = recentWinner;
    ...
    //     /**
    //     * @dev Only pick the tax collector winners if we have more more than 5 nfts to choose from. 
    //     * This should be always happen but doing this to avoid edgecase when we start the project. 
    //     */


    //     if ((_tokenSupply.current() - activeNFTStartIndex) > 10) {

    //         uint usdcTaxWinnerIndex = ((numberToUse * 2) % _tokenSupply.current());

    //         if (usdcTaxWinnerIndex < activeNFTStartIndex) {
    //             usdc_tax_winner_index = (activeNFTStartIndex + _tokenSupply.current()) / 2 ; 
    //         } else {
    //             usdc_tax_winner_index = usdcTaxWinnerIndex;
    //         }

    //         //update wbtc tax winner
    //         uint wbtcTaxWinnerIndex = (numberToUse * 5) % _tokenSupply.current();

    //         if (wbtcTaxWinnerIndex < activeNFTStartIndex) {
    //             wbtc_tax_winner_index = ((activeNFTStartIndex + _tokenSupply.current()) / 2) - 3 ; 
    //         } else {
    //             wbtc_tax_winner_index = wbtcTaxWinnerIndex;
    //         }
    //     }
    ...
    function setPartnerAddress(address _newPartner)  public {
        ...
        if (currentPartner == address(0)){
            myPartner[msg.sender] = _newPartner;
            myDroveTeamCount[_newPartner] += 1;
        } else {
            myPartner[msg.sender] = _newPartner;
            myDroveTeamCount[currentPartner] -= 1;
            myDroveTeamCount[_newPartner] += 1;
        }
    }
    ...
    function withdrawWbtcForWalletAddress() external nonReentrant {
        ...
        if (msg.sender != coreTeam_1 && msg.sender != coreTeam_2) {
            _tax = ((myBalance / 100) / 2 ); // 0.5%  of the transaction
            _withdrawableAmount = myBalance - _tax;
        } else {
            _withdrawableAmount = myBalance;

        }
        
        // Transfer Balance 
        IERC20(wbtcTokenContract).safeTransfer(msg.sender, _withdrawableAmount);

        // update wbtc balance for nft owner
        WBTCRewardsForAddress[msg.sender] = 0;
        

        // give the Tax Collector Winner the tax 
        updateWBTCRewardBalanceForAddress(wbtcTaxWinner, _tax); 

        emit withdrawWbtcRewardsEvent(msg.sender, _withdrawableAmount, _tax);
    ...
    function withdrawUsdcRewardBalance() external nonReentrant {
        ...
        if (msg.sender != coreTeam_1 && msg.sender != coreTeam_2) {
            ...
            // Transfer Balance 
            IERC20(usdcTokenContract).safeTransfer(msg.sender, _withdrawableAmount);
            // update mapping on contract 
            USDCRewardsForAddress[msg.sender] = 0;

            // update USDC Rewards Balance Total
            USDCRewardsBalanceTotal -= _withdrawableAmount;

            // update tax value for the current tax_collector 
            updateUsdcBonus(usdcTaxWinner,_tax);
        ...
        // emit event
        emit withdrawUSDCRewardsForAddressEvent(msg.sender, _withdrawableAmount, _tax);
    ...
    /**
     * @dev returns how many people are using them as their partner
     */
    function getDroveTeamCount() public view returns (uint) {
        return myDroveTeamCount[msg.sender];
    }

    /**
     * @dev checks if an address is using them as their partner.
     */
    function getAreTheyOnMyDroveTeam(address _adressToCheck) public view returns (bool) {
   ...
   // METADATA
    ...
    function togglePublicSaleStatus() external onlyOwner {
        ...
        require(address(usdcTaxWinner) != address(0), "ERROR: The usdcTaxWinner address must be set prior to any minting");
        require(address(wbtcTaxWinner) != address(0), "ERROR: The wbtcTaxWinner address must be set prior to any minting");
