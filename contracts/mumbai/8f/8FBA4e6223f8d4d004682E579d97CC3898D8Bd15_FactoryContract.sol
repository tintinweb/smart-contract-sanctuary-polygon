...
contract FactoryContract {
    ...
    event ContractCreated(address indexed campaignOwner, address newContract, uint256 timestamp);
    ...
    function createContract(
        ...
        require(_validateSignature(_signature, _orderId, msg.sender, _backerNftConstructor.transferable, _backerNftConstructor.baseUri), "Invalid signature");
        excutedOrderIds[_orderId] = true;
        BackerNFT newContract = new BackerNFT(
            _backerNftConstructor.nftName, 
            _backerNftConstructor.nftSymbol, 
    ...
    function _validateSignature(
        ...
        bytes32 hashValue = keccak256(
            abi.encodePacked(_orderId, _campaignOwner, _transferable, _baseUri)
        );
...
import "./interface/IBackerNFT.sol";

contract BackerNFT is ERC721Enumerable, Pausable, IBackerNFT {
    ...
    constructor(string memory _nftName, string memory _nftSymbol, string memory _baseUri, bool _transferable) ERC721(_nftName, _nftSymbol) {
        baseUri = _baseUri;
    ...
    function mint(address _receiver) public override onlyMinter returns (uint256 newTokenId) {
        newTokenId = tokenId++;
        _safeMint(_receiver, tokenId);
        emit TokenCreated(_receiver, tokenId, block.timestamp);
    }

    function mintBatch(address _receiver, uint256 _amount) external override onlyMinter returns (uint256[] memory) {
        uint256[] memory ids = new uint256[](_amount);
        require(_receiver != address(0), "Receiver can not be zero address");
        for(uint256 i = 0; i < ids.length; i++){
            uint256 newTokenId = mint(_receiver);
            ids[i] = newTokenId;
        }
        return ids;
    }
    ...
    {  
        ...
        for (uint256 index = 0; index < balanceOf(_address); index++) {
            uint256 nftId = tokenOfOwnerByIndex(_address, index);
            require(nftId > 0, "TokenId is must be greater than 0");
            nftIds[index] = nftId;
...
interface IBackerNFT {
    ...
    function mintBatch(address _account, uint256 _amount) external returns (uint256[] memory);
