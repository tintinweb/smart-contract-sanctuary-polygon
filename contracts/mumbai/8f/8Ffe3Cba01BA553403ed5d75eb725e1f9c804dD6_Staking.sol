...
contract Staking {
    ...
    // Parameters
    uint128 public constant VALIDATOR_THRESHOLD = 100000000000000; // 1 ether
    ...
    function _unstake() private {
        ...
        uint256 amount = _addressToStakedAmount[signer];

        _addressToStakedAmount[signer] = 0;
        _stakedAmount -= amount;

        if (_isValidator(signer)) {
            _deleteFromValidators(signer);
        }

        payable(staker).transfer(amount);
        emit Unstaked(signer, amount);
    }
