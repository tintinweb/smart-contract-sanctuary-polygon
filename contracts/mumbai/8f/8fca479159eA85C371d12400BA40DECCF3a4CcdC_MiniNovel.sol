...
/* *
   ...
   * @dev support recover and svg background
   *
...
contract MiniNovel is OwnableUpgradeable, ERC721Upgradeable, ERC721EnumerableUpgradeable, ERC2771ContextUpgradeable, Governance{
    ...
    uint256 public missionSpeed = 1; //400;
    uint256 public missionGrow = 2; //36;
    ...
    function _gethealth(uint256 tokenId) internal view returns (string memory) {
        if(getHealth(tokenId)) return "Yes";
        else return "No";
    }
    ...
    function getHunger(uint256 tokenId) public view returns (uint256) {
        ...
        if( love[tokenId] > missionGrow ) 
        {
           ...
           return _hunger[tokenId] + ((block.number+48 - _lastEatBlock[tokenId]) / missionSpeed);
    }
    ...
    function getStrength(uint256 tokenId) public view returns (uint256) {
        ...
        if( love[tokenId] > missionGrow ) 
        {
          ...
          return _strength[tokenId] + ((block.number+48 - _lastWorkBlock[tokenId]) / missionSpeed);
    }
    ...
    function getBoredom(uint256 tokenId) public view returns (uint256) {
        ...
        if( love[tokenId] > missionGrow ) 
        {
          ...
          return _boredom[tokenId] + ((block.number+48 - _lastPlayBlock[tokenId]) / missionSpeed);
    }
    ...
    function getSleepiness(uint256 tokenId) public view returns (uint256) {
        ...
        if( love[tokenId] > missionGrow ) 
        {
           ...
           return _sleepiness[tokenId] + ((block.number+48 - _lastSleepBlock[tokenId]) / missionSpeed);
    }

    function achieve(uint256 tokenId) internal {
        _sleepiness[tokenId] = (uint8)(_sleepiness[tokenId] + ((block.number+48 - _lastSleepBlock[tokenId]) / missionSpeed));
        _boredom[tokenId] = (uint8)(_boredom[tokenId] + ((block.number+48 - _lastPlayBlock[tokenId]) / missionSpeed));
        _strength[tokenId] = (uint8)(_strength[tokenId] + ((block.number+48 - _lastWorkBlock[tokenId]) / missionSpeed));
        _hunger[tokenId] =  (uint8)(_hunger[tokenId] + ((block.number+48 - _lastEatBlock[tokenId]) / missionSpeed));
   ...
   function section_1(uint256 id) private view returns (bytes memory) {
         ...
         abi.encodePacked(
                    ...
                    '<rect fill="url(#gradient)" width="500" height="500"/><text class="n"><tspan class="n" text-anchor="middle" x="50%" y="42%">',
    ...
    { 
        ...
        bytes memory now_trait = abi.encodePacked('{"trait_type":"leaf","value":"',tokens[id].toString(),'",\
"trait_type":"Health","value":"',_gethealth(id),'",\
"trait_type":"love","value":"',love[id].toString(),'",\
"trait_type":"name","value":"',getName(id),'"}');
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                             ...
                             '{"name":"',
                                "Mini Novel", "",
                                '", "description":"Onchain the mini-novel of Lofitown",',
                                '"attributes": [',
                                now_trait,
                                '],"image":"',
