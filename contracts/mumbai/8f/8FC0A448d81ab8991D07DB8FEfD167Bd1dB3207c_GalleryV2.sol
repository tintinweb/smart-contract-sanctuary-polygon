...
contract GalleryV2 is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function initialize(address bitflix_, uint256 gracePeriod_) public initializer {
    bitflix = bitflix_;
  ...
  function createListing(
    ...
    return
      _createListing(listingType, collection, msg.sender, tokenId, timeStart, timeEnd, currency, minimalBid, false);
  }
  ...
  function buyETH(uint256 listingId) external payable nonReentrant {
    ...
    require(msg.value >= listings[listingId].minimalBid, "Not enough ETH passed");
  ...
  function priceReduction(uint256 listingId, uint256 price) external nonReentrant {
    require(
      ...
      "Price changes are carried out only at a fixed price"
    );
    require(listings[listingId].seller == msg.sender, "Only the seller can change the price");
    require(price < listings[listingId].minimalBid, "Price should be less than the current one");
    listings[listingId].minimalBid = price;

    emit PriceReduction(listingId, price);
  }

  function revertSell(uint256 listingId) external nonReentrant {
    address seller = _getSellerByToken(listingId);
    require(seller == msg.sender, "Only the seller can cancel");
  ...
  function setCollection(address _collection, bool _status) external onlyOwner {
  ...
  function _revertTokenToTarget(uint256 listingId) internal {
    ...
    listings[listingId].collection.transferFrom(address(this), seller, listings[listingId].tokenId);
    emit RevertToken(listingId, seller);
  }
  ...
  function _buy(uint256 listingId) private {
    ...
    listings[listingId].claimed = true;
    listings[listingId].collection.transferFrom(address(this), msg.sender, listings[listingId].tokenId);
  ...
  function _distributeValue(uint256 listingId) private {
    ...
    _transferCurrency(listingId, listings[listingId].collection.artists(listings[listingId].tokenId), artistValue);
    _transferCurrency(listingId, listings[listingId].collection.celebs(listings[listingId].tokenId), celebValue);
    _transferCurrency(listingId, bitflix, bitflixValue);
    _transferCurrency(
      listingId,
      listings[listingId].seller,
      listings[listingId].lastBid - artistValue - celebValue - bitflixValue
    );
  }
