...
contract CryptoChimeraFactory is Ownable {
    ...
    struct TaskType {
        ...
        uint256 priceWeis;  // not supported in zkSync2
    }
    mapping(uint8 => TaskType) public taskTypes;

    // TODO: should emit an event instead of using blockchain storage for reduce your smart contract gas fees
    ...
    struct TaskInQueue {
        ...
        //uint256 deliveryPrice; // ? addToDeliveryPrice to TokenForDelivery.price
        uint256[] arguments;
    }
    ...
    */
    ...
    event TaskInQueueAdded(uint8 taskTypeId, uint8 addedTasksBefore, uint256 totalPrice, address indexed recipient, uint256 arg2, uint256 arg3);
    ...
    function setTaskType(uint8 taskTypeId, uint8 numberArguments, uint256 priceWeis) external onlyOwner {
        if (taskTypes[taskTypeId].numberArguments == 0) {
            taskTypes[taskTypeId] = TaskType(
                numberArguments,
                priceWeis
            );
        } else {
            // rewrite existing instance
            TaskType storage taskType = taskTypes[taskTypeId];
            taskType.numberArguments = numberArguments;
            taskType.priceWeis = priceWeis;
        }
    }
    ...
    function addTask(uint8 taskTypeId, address currencyAddress, uint256 arg2, uint256 arg3) external payable {
        ...
        CurrencyCoinType storage choosedCurrency = taskPricesForCurrencies[currencyAddress];
        require(choosedCurrency.currencyType != 0, "CryptoChimeraFactory: unknown choosed currency");
        if (choosedCurrency.currencyType == 1) {
        ...
        emit TaskInQueueAdded(taskTypeId, addedTasksBefore, taskType.priceWeis, _msgSender(), arg2, arg3);
    }
