    ...
    UUPSUpgradeable
{
    ...
    modifier checkIsAlphaBurn() {
        if (_isAlpha) {
            IPeriphery _periphery = _factory.periphery();
            require(
                msg.sender == address(_periphery) ||
                    msg.sender == address(_marginEngine),
                "pphry only"
            );
        }
        ...
        initializer
    {
        ...
        require(
            __tickSpacing > 0 && __tickSpacing < Tick.MAXIMUM_TICK_SPACING,
            "TSOOB"
        );
        ...
        _maxLiquidityPerTick = Tick.tickSpacingToMaxLiquidityPerTick(
            _tickSpacing
        );
    ...
    /// @inheritdoc IVAMM
    function refreshRateOracle() external override onlyOwner {
        rateOracle = _marginEngine.rateOracle();
    }
    ...
    /// @inheritdoc IVAMM
    function fixedTokenGrowthGlobalX128()
        external
        view
        override
        returns (int256)
    ...
    /// @inheritdoc IVAMM
    function variableTokenGrowthGlobalX128()
        external
        view
        override
        returns (int256)
        ...
        onlyMarginEngine
    {
        if (_protocolFees < protocolFeesCollected) {
            revert CustomErrors.NotEnoughFunds(
    ...
    function initializeVAMM(uint160 sqrtPriceX96) external override {
        ...
        if (_vammVars.sqrtPriceX96 != 0) {
            revert CustomErrors.ExpectedSqrtPriceZeroBeforeInit(
                _vammVars.sqrtPriceX96
            );
        }
    ...
    function setFeeProtocol(uint8 feeProtocol) external override onlyOwner {
        require(
            feeProtocol == 0 || (feeProtocol >= 3 && feeProtocol <= 50),
            "PR range"
        );
        ...
        returns (int256 positionMarginRequirement)
    {
        ...
        require(
            (msg.sender == recipient) ||
                (msg.sender == address(_marginEngine)) ||
                _factory.isApproved(recipient, msg.sender),
            "MS or ME"
        );
    ...
    function updatePosition(ModifyPositionParams memory params)
        private
        returns (int256 positionMarginRequirement)
    {
        ...
        if (msg.sender != address(_marginEngine)) {
            ...
            positionMarginRequirement = _marginEngine
                .updatePositionPostVAMMInducedMintBurn(params);
        }
        ...
        if (params.liquidityDelta != 0) {
            if (
                (lvammVars.tick >= params.tickLower) &&
                (lvammVars.tick < params.tickUpper)
            ) {
        ...
        )
    {
        ...
        if (
            !(msg.sender == address(_marginEngine) ||
                msg.sender == address(_marginEngine.fcm()))
        ) {
            require(
                msg.sender == params.recipient ||
                    _factory.isApproved(params.recipient, msg.sender),
            ...
            while (
                ...
                (
                    ...
                    SwapMath.SwapStepParams({
                        ...
                        timeToMaturityInSecondsWad: termEndTimestampWad -
                ...
                if (state.liquidity > 0) {
                    ...
                    state.variableTokenDeltaCumulative -= step
                        .variableTokenDelta; // opposite sign from that of the LP's

                    // necessary for testing purposes, also handy to quickly compute the fixed rate at which an interest rate swap is created
                    state.fixedTokenDeltaUnbalancedCumulative -= step
        ...
        if (
            ...
            marginRequirement = _marginEngine.updatePositionPostVAMMInducedSwap(
                    params.recipient,
                    params.tickLower,
                    params.tickUpper,
                    state.fixedTokenDeltaCumulative,
                    state.variableTokenDeltaCumulative,
                    state.cumulativeFeeIncurred,
                    state.fixedTokenDeltaUnbalancedCumulative
                );
        }
        ...
        )
    {
        ...
        stateVariableTokenGrowthGlobalX128 =
            state.variableTokenGrowthGlobalX128 +
            FullMath.mulDivSigned(
                step.variableTokenDelta,
                FixedPoint128.Q128,
                state.liquidity
            );

        stateFixedTokenGrowthGlobalX128 =
            state.fixedTokenGrowthGlobalX128 +
            FullMath.mulDivSigned(
                fixedTokenDelta,
                FixedPoint128.Q128,
...
interface IFCM is CustomErrors {
    ...
    function initialize(IVAMM __vamm, IMarginEngine __marginEngine) external;
...
interface IRateOracle is CustomErrors {
    ...
    event RateCardinalityNext(uint16 observationCardinalityNextNew);
    ...
    function getRateFrom(uint256 _from) external view returns (uint256);
    ...
    function UNDERLYING_YIELD_BEARING_PROTOCOL_ID()
        external
        view
        returns (uint8 yieldBearingProtocolID);
    ...
    function getLastRateApy() external view returns (uint256 apyWad);
...
contract VAMMStorageV1 {
    ...
    bool public paused;
    uint256 internal _maturityBufferWad;
}
