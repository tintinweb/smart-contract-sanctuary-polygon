...
library UniswapV2Library {
    ...
    function getAmountsInOneD(address router,address factory,address[] memory pairs, uint amountOut, address[] memory path) internal view returns (uint[] memory amounts) {
        ...
        for (uint i = path.length - 1; i > 0; i--) {
            ...
            //without fee
            amounts[i - 1] = IUniswapV2Router01(router).getAmountIn(amounts[i], reserveIn, reserveOut);
        }
    }
}
...
function defiCall(
    ...
    require(curTokenPath.length>0, 'TOKEN_PATH_ZERO');
    require(curRouterPath.length>0, 'ROUTER_PATH_ZERO');
    require(curFactoryPath.length>0, 'FACTORY_PATH_ZERO');

    uint amountLoan = _amount0 == 0 ? _amount1 : _amount0;
    address[] memory path = new address[](2);
    path[0] = curTokenPath[curTokenPath.length-1];
    path[1] = curTokenPath[0];

    uint amountReim = getAmountsInOneD(
        curRouterPath[curRouterPath.length-1], 
        curFactoryPath[curFactoryPath.length-1], 
        amountLoan, 
        path
    )[0];
    ...
    require(amountLoan!=888, 'CHECK_888');

    IERC20 tokenReim = IERC20(curTokenPath[curTokenPath.length-1]);
    require(amountReceived >= amountReim, 'INSUFFICIENT_AMOUNT_REIMBURSE');
    tokenReim.transfer(msg.sender, amountReim);
    tokenReim.transfer(tx.origin, amountReceived - amountReim);
  }
