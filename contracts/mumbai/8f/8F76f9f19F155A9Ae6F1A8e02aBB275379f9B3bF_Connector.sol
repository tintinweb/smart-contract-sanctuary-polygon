...
import "./NFT.sol";
contract Connector {
    function createNFT(address creator, string memory NFTData, uint256 numberOfTokens) public returns (address) {
...
contract NFT is ERC1155 {
    ...
    string public link;
    bool public Kit = false;
    mapping (uint256 => Link) public HyperLink;
    constructor(address account, uint256 _amount, string memory uri_, address _market) payable ERC1155("") {
        creator = account;
        amount = _amount;
        market = _market;
        link = uri_;
    ...
    function getAll(uint256 tokenId) public view returns (string memory, address, uint, bool, address, bool, uint256) {
        return(link, creator, amount, Kit, getOwner(tokenId), tokenOnSale(tokenId), getPrice(tokenId));
    }
    ...
    function publishToken(uint256 id, uint256 price) public payable {
        ...
        HyperLink[id].cost = 1 gwei * price;
    }
