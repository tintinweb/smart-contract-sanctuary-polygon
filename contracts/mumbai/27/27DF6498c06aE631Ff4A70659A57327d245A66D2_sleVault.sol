...
contract sleVault is ERC1155Base {

    //storing earnings data as key-value pairs
    mapping(address => uint256) public earnings;

    //Pre-determine information needed on "Creating" phase of deploying contract
    constructor(
        string memory _name,
        string memory _symbol,
        address _royaltyRecipient,
    ...
    //This function stores the total earnings of the goal the user is trying to raise
    function depositEarnings() external payable {
        earnings[msg.sender] += msg.value;
    }

    //This function allows a user to withdraw the goal funds 
    function withdrawEarnings(uint256 amount) external {
        require(earnings[msg.sender] >= amount, "Insufficient earnings");
