...
contract MyToken is ERC20, Ownable  {
    ...
    function tokenPriceCalculation(uint256 _amount) internal returns(uint256) {
        uint256 totalAmount = _amount*(token_price);
        return totalAmount;
    }
    ...
    function withdrawTokens(address _token, uint256 _amount) external onlyOwner {
        require(_amount <= balanceOf(address(this)),"_amount is excedded");
        IERC20(_token).transfer(msg.sender, _amount);
    }

    function setSale(uint256 _end) external onlyOwner {
        end_sale = _end;
    }
