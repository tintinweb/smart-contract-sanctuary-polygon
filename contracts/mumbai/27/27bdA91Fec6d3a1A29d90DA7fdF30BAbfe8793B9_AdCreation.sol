...
// File contracts/utils/ERC721Upgradeable.sol
 ...
 */
abstract contract NFT721Metadata is ERC721Upgradeable {
...
// File contracts/interface/IRegistration.sol
...
// File contracts/CreateAd.sol

// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.9;
contract AdCreation is NFT721Metadata {
    using AddressUpgradeable for address;
    ...
    mapping(uint256 => AdInfo) public adInfos;

    mapping(address => uint[]) public userAds;

    error NotAValidAd(uint256 tokenID, address user);

    event AdCreated(address, uint256, string, string[6]);

    function init(
        address registrationRole_,
        string memory name_,
        string memory symbol_
    ) external initializer {
        _initializeAdvertiserRegistration(registrationRole_);
        __ERC721_init(name_, symbol_);
        NFT721Metadata._initializeNFT721Mint();
        _updateBaseURI("");
    }
     ...
     */
    function _initializeAdvertiserRegistration(address _registry) internal {
        //onlyInitializing
        require(
     ...
     */
    function getRegistrationRole() public view returns (address) {
        return registry;
    }

    // adDetails[0] = Ad Title
    // adDetails[1] = Ad Target Link
    // adDetails[2] = Ad category
    // adDetails[3] = Ad Content
    // adDetails[4] = Ad layout
    // adDetails[5] = Ad Target Platform
    function createAd(string memory _tokenIPFSPath, string[6] memory adDetails)
        external
    {
        IRegistration(getRegistrationRole()).isValidAdvertiser(msg.sender);

        uint256 mintedTokenID = _mintInternal(_tokenIPFSPath);

        AdInfo storage adInfo = adInfos[mintedTokenID];
        adInfo.title = adDetails[0];
        adInfo.targetLink = adDetails[1];
        adInfo.category = adDetails[2];
        adInfo.content = adDetails[3];
        adInfo.layout = adDetails[4];
        adInfo.targetPlatform = adDetails[5];
        adInfo.tokenId = mintedTokenID;
        adInfo.advertiser = msg.sender;
        userAds[msg.sender].push(mintedTokenID);

        emit AdCreated(msg.sender, mintedTokenID, _tokenIPFSPath, adDetails);
    }

    function isValidAd(uint256 tokenID_, address advertiser_)
        public
        view
        returns (bool)
    {
        if (_exists(tokenID_) && adInfos[tokenID_].advertiser == advertiser_)
            return true;
        else revert NotAValidAd(tokenID_, advertiser_);
    }
}