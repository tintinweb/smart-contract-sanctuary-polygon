...
//   modifier isClaimed() {
//         _validateClaimIsDone(); //checkAddress is included in storage?
//         _;
//     }
    ...
    //function readTree
       function getParentById(uint256 _tokenId)  external view  returns (uint256){
        return storeParent[_tokenId];
        
    }


    function claimProfileByNFT(
        DataTypes.CreateProfileData calldata vars,
    uint256 GenesisId /// GENESIS-NFT
    //NFTprofileId,
    )
        external
        
        // isClaimed //NOT CLAIMED YET
        // whenNotPaused
        // onlyWhitelistedProfileCreator
    {
        //1. check owning NFT //exploit: other address can use other NFTID, how to restrict?

        //2. check profileId has address claimed? -> _recordOfTree
        // store

        //3. profile from genesis has EXTRA feature?

         uint256 profileId = ++_profileCounter;
        storeByAddress[msg.sender].push(profileId);
        storeById[profileId] = msg.sender;
        storeChildren[0].push(profileId);
        storeParent[profileId] = 0;
        storeGenesisNFT[GenesisId].push(profileId);
    ...
    // function claimProfileByRC(DataTypes.CreateProfileData calldata vars,
    // uint256 NFTProfileId //referralCode = profileId now
    // )
    //     external
        
    //     // isClaimed
    // {
    //     //1. check profileId has address  claimed? -> _recordOfTree -> isClaimed
    //     //2. check referral code --> 
    //     //3. profile from referral code  carry parent

    //     uint256 profileId = ++_profileCounter;
    //     storeByAddress[msg.sender].push(profileId);
    //     storeById[profileId] = msg.sender;
    //     storeChildren[NFTProfileId].push(profileId);
    //     storeParent[profileId] = NFTProfileId;

    //     //then referralCode check
    ...
    // function _validateClaimIsDone() internal view {
    //     if (!_profileCreatorWhitelisted[msg.sender]) revert Errors.ProfileCreatorNotWhitelisted();
    // }
...
contract LensHubStorage {
    ...
    mapping(uint256 => address) internal  storeById;
    mapping(address => uint256[]) internal  storeByAddress;
    mapping(uint256 => uint256) internal storeParent;
    mapping(uint256 => uint256[]) internal  storeChildren;
    mapping(uint256 => uint256[]) internal storeGenesisNFT;
}
