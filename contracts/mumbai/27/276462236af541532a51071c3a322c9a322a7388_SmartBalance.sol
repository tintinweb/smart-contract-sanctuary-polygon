...
contract SmartBalance is Initializable, AccessControlUpgradeable {
    IERC20Upgradeable public MCO2;
    ICarbonChain public carbonChain;
    ...
    function initialize(
        address _offsetter,
        address _admin,
        IERC20Upgradeable _MCO2,
        ICarbonChain _CarbonChain,
        string calldata _clientId
    ) public initializer {
        MCO2 = _MCO2;
        carbonChain = _CarbonChain;
        clientId = _clientId;

        MCO2.approve(address(_CarbonChain), type(uint256).max);

        _setupRole(OFFSETTER_ROLE, _offsetter);
        _setupRole(ADMIN_ROLE, _admin);
        _setupRole(DEFAULT_ADMIN_ROLE, _admin);
    }

    function sweep() external onlyRole(ADMIN_ROLE) {
        MCO2.transfer(msg.sender, MCO2.balanceOf(address(this)));
    }

    function changeCarbonChain(ICarbonChain _newCarbonChain)
        external
        onlyRole(ADMIN_ROLE)
    {
        carbonChain = _newCarbonChain;
    }
    ...
    function offsetCarbon(
        ...
        carbonChain.offsetCarbon(_carbonTon, _transactionInfo, _onBehalfOf);
    }
}
