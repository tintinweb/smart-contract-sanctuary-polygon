...
contract mQuark1155Implementation is ImQuarkNFT, ERC1155, Ownable, Initializable, ReentrancyGuard, NoDelegateCall {
  event TokenMint(uint64 projectId, uint256 templateId, uint64 collecitonId, uint256 amount, string uri);
  ...
  mapping(uint256 => mapping(uint256 => TokenSubscriptionInfo)) private s_tokenSubscriptions; //formerly _tokenProjectURIs
  ...
  function _validityChecks() internal view returns (Collection memory m_tempData) {
    ...
    if (m_tempData.collectionURIs.length == 0) revert CollectionURIZero("B1155");
    if (m_tempData.totalSupply <= m_tempData.mintCount) revert CollectionIsSoldOut("C1155");
  ...
  function mintWithURI(
    ...
    emit TokenMint(m_tempData.projectId, m_tempData.templateId, m_tempData.collectionId, msg.value, _uri);
  }
  ...
  function updateURISlot(
    ...
    if (balanceOf(_owner, _tokenId) != 1) revert NotOwner("I1155", _tokenId);
    if (!s_tokenSubscriptions[_tokenId][_projectId].isSubscribed) revert Unsubscribed("J1155", _tokenId, _projectId);
  ...
  function withdraw() external onlyOwner nonReentrant noDelegateCall {
    _withdraw(false);
  }

  function protocolWithdraw() external onlyAuthorized nonReentrant noDelegateCall {
    _withdraw(true);
  }
