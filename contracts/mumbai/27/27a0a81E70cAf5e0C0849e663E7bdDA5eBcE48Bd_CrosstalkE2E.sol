...
/*
    Cross talk E2E cases handled
    - Happy Paths
        1 Single Request with Ack
        2 Multiple Request Atomic
        3 Multiple Request Non Atomic
        4 Multiple Request with Ack
    - Error Paths
        5 Gas Limit Errors Single Request
        6 Gas Price Errors Single Request
        7 ACK Gas Price Error
        8 ACK Gas Limit Error
        9 Error ACK

*/

contract CrosstalkE2E is AccessControl , ICrossTalkApplication {
    using Counters for Counters.Counter;

    Counters.Counter private _requestId;

    mapping (bytes32 => receiptStruct ) private receipts;
    ...
    string public stringPayload;

    event CrossTalkE2ESend( uint64 indexed requestId , uint8 _type );

    event CrossTalkE2EReceive( uint64 indexed requestId , uint8 _type , uint i );

    event CrossTalkE2EACKReceive( uint64 indexed requestId , uint8 _type , bool _success , uint i );

    error CustomRevert( uint64 requestId , uint8 _type , uint i);
    ...
    function setMetadata(string memory feePayer) external {
        gateway.setDappMetadata(feePayer);
    }

    function setDestGasLimit ( uint64 _gasLimit ) external {
        destGasLimit = _gasLimit;
    }

    function setAckGasLimit ( uint64 _gasLimit ) external {
        ackGasLimit = _gasLimit;
    }

    function setStringPayload ( string memory _payload ) external {
        stringPayload = _payload;
    }

    function testCrossTalk(
        uint64 chainType,
        string memory chainId,
        uint8 reqType,
        uint64 destGasPrice,
        uint64 ackGasPrice,
        address[] memory destAddress,
        ...
        bytes[] memory addresses = new bytes[](destAddress.length);
        bytes[] memory payloads = new bytes[](destAddress.length);
        for (uint i = 0; i < destAddress.length; i++) {
            if ( ( i == 1 ) && ( ( reqType == 2 ) || ( reqType == 3 ) || ( reqType == 4 ) ) ){
                payload = abi.encode(_requestId.current(), reqType , true ,stringPayload , i);
            }else {
                payload = abi.encode(_requestId.current(), reqType , false ,stringPayload , i );
            }
            addresses[i] = toBytes(destAddress[i]);
            payloads[i] = payload;
        }
        bool atomic;
        if ( reqType == 3 ) {
        ...
        emit CrossTalkE2ESend( currentRequestId , reqType );
    }

    function _crossTalkSend(
    ...
    function handleRequestFromSource(
        ...
        (uint64 reqID, uint8 reqType , bool error , string memory strPayload , uint i) = abi.decode(
            payload,
            (uint64, uint8 , bool , string , uint)
        );
        emit CrossTalkE2EReceive( reqID , reqType , i );
        if (error) {
            revert CustomRevert( reqID  , reqType , i );
        }
        if (reqType == 1 ) {
            returnPayload = abi.encode( reqID , reqType , true, i );
    ...
    function handleCrossTalkAck(
        ...
        (uint64 reqID, uint8 reqType , bool success , uint i) = abi.decode(
            _execData,
            (uint64, uint8, bool, uint)
        );
        if ( reqType == 9 ){
            revert CustomRevert(reqID , reqType , i);
        }

        emit CrossTalkE2EACKReceive(reqID, reqType , success , i);
    }
