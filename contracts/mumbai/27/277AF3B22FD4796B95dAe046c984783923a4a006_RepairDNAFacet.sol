...
contract RepairDNAFacet {

    function adminGetHatchDetails(uint256 tokenId) external view returns (
        uint256 roundTripId,
        ...
        birthday = lhs.birthdayByVRFRequestId[vrfRequestId];
        shouldUsePredictiveDNA = LibHatching.shouldUsePredictiveDNA(tokenId);
    }

    function adminForceDNA(uint256 tokenId, uint256 dna) external {
        LibDiamond.enforceIsContractOwner();
        LibUnicornDNA._setDNA(tokenId, dna);
    }

    function adminForceIdempotenceState(uint256 tokenId, uint256 idmp) external {
        LibDiamond.enforceIsContractOwner();
