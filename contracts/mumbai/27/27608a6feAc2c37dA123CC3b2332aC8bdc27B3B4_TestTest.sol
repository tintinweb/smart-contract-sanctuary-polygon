...
interface IConstants is ISismoStructs {
    ...
    struct arbitrationResolutionVoting {
        uint256 requestID;
        bool resolutionType; // True if it is for CreateDB false if it is for a contribution
...
contract TestTest is Ownable, ERC1155, AccessControl, IConstants {
    ...
    event contributionAssertionCreated(uint256 requestID, bytes32 assertionID);
    ...
    event datasetAssertionCreated(uint256 requestID, bytes32 assertionID);
    ...
    constructor(ISismoVerifier _SISMO_VERIFIER, ISender _sender) ERC1155("") {
        sender = _sender;
        SISMO_VERIFIER = _SISMO_VERIFIER;
        // Goerli
    ...
    function getAssertions(uint256 _requestID) public view returns (bytes32[] memory) {
        RequestInfo memory requestinfo = requestInfo[_requestID];
        bytes32[] memory assertions = new bytes32[](requestinfo.details.assertions.length);
        for (uint i = 0; i < requestinfo.details.assertions.length; i++) {
            assertions[i] = requestinfo.details.assertions[i];
        }
        return assertions;
    }
    ...
    function RequestDB(
        ...
        for (uint i = 0; i < _claims.length; ) {
            request.requestRequiredClaims.push(_claims[i]);
            unchecked {
                ++i;
            }
        }
    ...
    function getRequestRequiredClaims(uint256 _requestID) external view returns(ClaimRequest[] memory) {
        return requestInfo[_requestID].requestRequiredClaims;
    }
    ...
    function assertContribution(
        ...
        // Verifying Claims
        if (request.requestRequiredClaims.length > 0) {
            SISMO_VERIFIER.verifySismoProof(proofs, request.requestRequiredClaims);
        }
        // UMA assert valid contribution
        assertContributionTruth(_requestID, Contribution(_requestID, msg.sender, dataCID, 1));
    }
    ...
    function assertContributionTruth(
        ...
        // );
        bytes memory assertedClaim = abi.encodePacked(
            "assert subbbb on ID testttt 0x",
            AncillaryData.toUtf8BytesUint(_requestID)
            // " with data : ",
            // contribution.dataCID
        );
        ...
        request.details.assertions.push(assertionID);

        assertionInfo[assertionID].requestID = _requestID;
        assertionInfo[assertionID].assertionType = false;
        assertionToContribution[assertionID] = contribution;
        emit contributionAssertionCreated(_requestID, assertionID);
    }
    ...
    function assertDataset(
        ...
        uint256 mintPrice,
        address tokenTreasury
    ) public exists(_requestID) onlyRequestAdmins(_requestID) {
        RequestInfo storage request = requestInfo[_requestID];
        require(request.status != RequestStatus.Mintable);
        ...
        // );
        bytes memory assertedClaim = abi.encodePacked(
            "Assertion of TokenID testing 0x",
            AncillaryData.toUtf8BytesUint(_requestID)
            // " with data : ",
            // datasetCID
        );
        ...
        assertionToCreateDB[assertionID] = DB(_requestID, datasetCID, mintPrice, tokenTreasury);

        assertionInfo[assertionID].requestID = _requestID;
        assertionInfo[assertionID].assertionType = true;
        emit datasetAssertionCreated(_requestID, assertionID);
    }
    ...
    function assertionResolvedCallback(
        ...
        if (contribution.contributor != address(0)) {
            if (assertedTruthfully) {
                ...
                _grantRole(
                    getRequestContributorRole(assertionInfo[assertionId].requestID),
            ...
            if (assertedTruthfully) {
                ...
                requestInfo[db.tokenID].tokenTreasury = db.tokenTreasury;
                requestInfo[db.tokenID].mintPrice = db.mintPrice;
                emit DatasetCreated(db.tokenID, db.dbCID, db.mintPrice, db.tokenTreasury);
            }
        }
    }
    ...
    function startResolutionVoting(
        ...
        uint256 _requestID = assertionInfo[assertionId].requestID;
        uint256 votingEndTime = (requestInfo[_requestID].details.resolutionDays / 2) +
            block.timestamp;
        if (assertionInfo[assertionId].assertionType) {
            assertionVoting.requestID = assertionInfo[assertionId].requestID;
            assertionVoting.time = time;
            assertionVoting.votingEndTime = votingEndTime;
            assertionVoting.ancillaryData = ancillaryData;
            assertionVoting.resolutionType = true; // DB resolution
        } else {
            assertionVoting.requestID = assertionInfo[assertionId].requestID;
            assertionVoting.time = time;
            assertionVoting.votingEndTime = votingEndTime;
    ...
    function voteOnAssertionResolution(
        ...
        external
        // ) external onlyRequestAdmins(assertionInfo[assertionId].requestID) {
        onlyRequestMembers(assertionInfo[assertionId].requestID)
    {
        arbitrationResolutionVoting storage assertionVoting = assertionVotingPhase[assertionId];
    ...
    function settleAssertions(uint256 _requestID) external {
        RequestInfo storage requestinfo = requestInfo[_requestID];
        uint256 numberOfAssertions = requestinfo.details.assertions.length;
        bytes32 assertionId;
        for (uint i = 0; i < numberOfAssertions; ) {
            assertionId = requestinfo.details.assertions[i];
    ...
    function getAvailableAssertions(uint256 _requestID) external view returns(bytes32[] memory){
        RequestInfo storage requestinfo = requestInfo[_requestID];
        uint256 numberOfAssertions = requestinfo.details.assertions.length;
    ...
    function mintDatasetNFT(uint256 tokenID) external payable exists(tokenID) {
        require(requestInfo[tokenID].status == RequestStatus.Mintable, "DB not still mintable");
        require(requestInfo[tokenID].mintPrice == msg.value, "wrong price");
        address payable to = payable(requestInfo[tokenID].tokenTreasury);
        if (isContract(requestInfo[tokenID].tokenTreasury)) {
    ...
    function hasAccess(address user, uint256 _requestID) public view returns (bool) {
        if (requestInfo[_requestID].status == RequestStatus.Mintable) {
            return balanceOf(user, _requestID) > 0;
        }
        return requestAccess(user, _requestID);
    }

    function requestAccess(address user, uint256 _requestID) public view returns (bool) {
        return
            hasRole(getRequestContributorRole(_requestID), user) ||
            hasRole(getRequestAdminRole(_requestID), user);
    }

    function requestAdmin(address user, uint256 _requestID) public view returns (bool) {
        return hasRole(getRequestAdminRole(_requestID), user);
    }
    ...
    function getRequestContributorRole(uint256 _requestID) public pure returns (bytes32) {
        return keccak256(abi.encodePacked(_requestID, "REQUEST_CONTRIBUTOR_ROLE"));
    }
    ...
    function isDisputeAllowed(bytes32 assertionID, address disputer) public view returns (bool) {
        ...
        if (
            hasRole(getRequestAdminRole(_requestID), disputer) ||
            hasRole(getRequestContributorRole(_requestID), disputer)
        ) {
            return true;
        }
        return false;
    ...
    modifier onlyRequestMembers(uint256 _requestID) {
        require(requestAccess(msg.sender, _requestID), "anothorized request action");
        _;
    }

    modifier onlyRequestAdmins(uint256 _requestID) {
        require(requestAdmin(msg.sender, _requestID), "anothorized request action");
        _;
    }
