...
contract TaxToken is ERC20 ,Ownable{
    uint public Tax; //Default tax set to 10%|| 10% =10000 in bips.
    bool private istax;
    ...
    modifier whennottaxed (){
        require(!_istax(),"Taxable:Tax off");
        _;
    }   

    modifier whentaxed () {
        require(_istax(),"TAxable: Tax on");
        _;
    } 
    // Read functions 
    ...
    function _istax() public view returns(bool){
        return istax;
    }

    function _TaxOn()public virtual onlyOwner whennottaxed{
        istax=true;
    } 

    function Taxoff()public virtual onlyOwner whentaxed {
        istax =false;
    }

    function taxdenominator() internal pure returns(uint96) {
        return 10000;
    ...
    function transfer(address to, uint256 amount) public  override returns (bool) {
        //Make sure sender have enough money to transfer
        //Take out 10 % cut from it 
        //Transfer the remaining 90% to the receipient
        //emit the transfer event 

        uint balancesender = balanceOf(msg.sender); 
        require (balancesender>=amount,"ERC20:Not enough balance to transfer the amount");
        //solidity does not support floating type number
        if(!_istax()){
            _transfer(msg.sender,to,amount);
        }else{
            uint taxAmount = (amount * Tax)/ taxdenominator();
            uint transferamount = amount - taxAmount;
            _transfer(msg.sender,to,transferamount);
            _transfer(msg.sender,taxdestination(),taxAmount);
        }
