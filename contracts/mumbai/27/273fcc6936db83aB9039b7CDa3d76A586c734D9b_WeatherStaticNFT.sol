...
import {CBOR, Buffer} from "./dev/vendor/solidity-cborutils/2.0.0/CBOR.sol";
...
contract WeatherStaticNFT is FunctionsClient, ConfirmedOwner {
  ...
  function ok(string memory zipcode, string memory countryCode) public view returns (CBOR.CBORBuffer memory, bytes memory) {
    string[2] memory args = [zipcode, countryCode]; 
    if (args.length > 0) {
      CBOR.writeString(buffer, "args");
      CBOR.startArray(buffer);
