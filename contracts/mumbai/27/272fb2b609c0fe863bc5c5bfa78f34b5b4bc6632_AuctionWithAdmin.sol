...
contract AuctionWithAdmin is Ownable {
    ...
    // Declare a set state variable
    EnumerableMap.UintToAddressMap private offerId;
    ...
    function dealDetails(uint256 index) public view returns (DealDetails memory dealInfo, uint256 tokenId, uint256 price){
        (uint256 id,) = EnumerableMap.at(dealId, index);
        return (deal[id], id, token_price[id]);
    }
    ...
    function auctionDetails(uint256 index) public view returns (AuctionDetails memory auctionInfo, uint256 tokenId , uint256 price){
        (uint256 id,) =  EnumerableMap.at(auctionId, index);        
        return (auction[id], id, token_price[id]);
    }
}