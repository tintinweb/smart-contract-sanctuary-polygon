...
contract ZKBadgeMinter is IZKBadgeMinter, Initializable, AccessControl, ERC1155, SismoConnect {
  ...
  bytes32 public constant CONFIG_REGISTRANT_ROLE = keccak256("CONFIG_REGISTRANT_ROLE");
  ...
  // Each tokenId is mapped to a ZK Badge configuration. The config can be registered one time only.
  ...
  mapping(uint256 => ZKBadgeConfig) private _zkBadgeConfigs;

  // mapping to quickly know if a vaultId has minted a ZK Badge
  // the vaultId is the anonymous identifier of a user's vault for a specific app
  // vaultId = hash(userVaultSecret, appId)
  // The vaultId is used to prevent double spending of a ZK Badge
  mapping(uint256 => mapping(uint256 => bool)) private _isVaultIdUsedForTokenId;
  ...
  ) ERC1155(legacyUri) SismoConnect(buildConfig(bytes16(0), false)) {
    initialize(legacyUri, owner);
  }
  ...
  /**
   * @dev Mints a ZK Badge based on a Sismo Connect response
   ...
   */
  function claimWithSismoConnect(bytes memory responseBytes, address to, uint256 tokenId) external {
    SismoConnectResponse memory response = abi.decode(responseBytes, (SismoConnectResponse));

    ZKBadgeConfig memory zkBadgeConfig = _zkBadgeConfigs[tokenId];
    ...
    SismoConnectRequest memory request = buildRequest({
      ...
      signature: buildSignature({message: abi.encode(to, tokenId)})
    });

    SismoConnectVerifiedResult memory result = _sismoConnectVerifier.verify({
      response: response,
      request: request,
      config: _zkBadgeConfigs[tokenId].config
    });

    // check if vaultId is already used for this appId
    // The vaultId is used to prevent double spending of a ZK Badge for a specific appId
    uint256 vaultId = result.getUserId(AuthType.VAULT);
    if (_isVaultIdUsedForTokenId[tokenId][vaultId]) {
      revert VaultIdAlreadyUsedForTokenId(tokenId, vaultId);
    }

    // update state to keep track of the vaultId
    _isVaultIdUsedForTokenId[tokenId][vaultId] = true;
  ...
  /**
   * @dev Registers a ZK Badge configuration
   ...
   * @param baseTokenURI uri of the ZK Badge metadata
  ...
  function registerZKBadgeConfig(
    ...
    address owner,
    uint256 tokenId
  ) external onlyRole(CONFIG_REGISTRANT_ROLE) {
    if (_zkBadgeConfigs[tokenId].owner != address(0)) {
      revert ZKBadgeConfigAlreadyRegistered(tokenId);
    }
    _createZKBadge(
      appId,
      isImpersonationMode,
  ...
  /**
   * @dev Updates a ZK Badge configuration
  ...
  function updateZKBadgeConfig(
    ...
    address configOwner = _zkBadgeConfigs[tokenId].owner;
  ...
  /**
   * @dev Returns the ZK Badge configuration based on its appId
   * @param tokenId tokenId of the ZK Badge
   */
  function getZKBadgeConfig(uint256 tokenId) external view returns (ZKBadgeConfig memory) {
    return _zkBadgeConfigs[tokenId];
  }

  /**
   * @dev Returns if weither or not a vaultId has minted a specific ZK Badge
   * @param tokenId tokenId of the ZK Badge
   * @param vaultId anonymous identifier of a user's vault for a specific app
   */
  function isVaultIdUsedForTokenId(uint256 tokenId, uint256 vaultId) external view returns (bool) {
    return _isVaultIdUsedForTokenId[tokenId][vaultId];
  }

  /**
   * @dev Returns the uri of a ZK Badge based on its tokenId
   * @param tokenId tokenId of the ZK Badge
   */
  function uri(uint256 tokenId) public view override(ERC1155) returns (string memory) {
    ...
    return _zkBadgeConfigs[tokenId].uri;
  }

  /**
   * @dev Creates a ZK Badge configuration
   * @param appId id of the Sismo Connect App from which the proofs are required
   * @param isImpersonationMode if True, the ZK Badge can be minted thanks to impersonated proofs
   * @param baseTokenURI uri of the ZK Badge metadata
   * @param authRequests list of dataSource ownerships required to mint the ZK Badge
  ...
  function _setZKBadge(
    ...
    _zkBadgeConfigs[tokenId].config.appId = appId;
    _zkBadgeConfigs[tokenId].config.vault.isImpersonationMode = isImpersonationMode;
    _zkBadgeConfigs[tokenId].uri = baseTokenURI;
    _setAuths(tokenId, authRequests);
    _setClaims(tokenId, claimRequests);
    _zkBadgeConfigs[tokenId].owner = owner;
  }

  /**
   * @dev Sets the auths of a ZK Badge configuration
   * @param tokenId tokenId of the ZK Badge
   * @param auths list of dataSource ownerships required to mint the ZK Badge
   */
  function _setAuths(uint256 tokenId, AuthRequest[] memory auths) private {
    AuthRequest[] storage _authRequests = _zkBadgeConfigs[tokenId].authRequests;
  ...
  /**
   * @dev Sets the claims of a ZK Badge configuration
   * @param tokenId tokenId of the ZK Badge
   * @param claims list of group memberships required to mint the ZK Badge
   */
  function _setClaims(uint256 tokenId, ClaimRequest[] memory claims) private {
    ClaimRequest[] storage _claimRequests = _zkBadgeConfigs[tokenId].claimRequests;
  ...
  /**
   * @dev Sets the tokenId of a ZK Badge configuration
   * @param tokenId tokenId of the ZK Badge
   * @notice The tokenId should not be used by another ZK Badge
   */
  function _setTokenId(uint256 tokenId) private {
    if (_isTokenIdFromLegacy(tokenId)) {
      revert TokenIdAlreadyUsedFromLegacy(tokenId);
    }

    _zkBadgeConfigs[tokenId].tokenId = tokenId;
  }

  /**
   * @dev Returns if a tokenId is already reserved for a legacy ZK Badge
   * @param tokenId tokenId of the ZK Badge
   */
   ...
   * @param legacyUri new URI
   */
  function setUri(string memory legacyUri) external override onlyRole(DEFAULT_ADMIN_ROLE) {
    _setURI(legacyUri);
  }
...
interface IZKBadgeMinter {
  ...
  error ZKBadgeConfigAlreadyRegistered(uint256 tokenId);
  error NotZKBadgeConfigOwner(bytes16 appId, address configOwner, address attacker);
  error VaultIdAlreadyUsedForTokenId(uint256 tokenId, uint256 vaultId);
  error TokenIdAlreadyUsedFromLegacy(uint256 tokenId);
