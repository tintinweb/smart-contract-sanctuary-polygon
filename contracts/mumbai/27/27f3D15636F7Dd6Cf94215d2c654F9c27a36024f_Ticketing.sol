...
contract Ticketing is ERC721, Ownable{
    ...
    event NewTicketCreated(uint indexed eventID, uint indexed creatorAddress, uint indexed ticketsCount);
    event NFTMinted(uint indexed tokenId);
    event Purchased(uint indexed eventId, address indexed _ad, bool indexed status);
    ...
    mapping(uint => mapping(address => bool)) private forQrVerification;
    ...
    function createEvent(EventInformation memory _event) public onlyAdmin{
        ...
        autoIds.push(autoGenerateId);
        emit NewTicketCreated(autoGenerateId, _event.publishTime, _event.ticketsQuantity);
    }
    ...
    function buyTickets( uint _eventId, uint _ticketCount) public {
        ...
        forQrVerification[_eventId][msg.sender] = true;
        emit Purchased(_eventId, msg.sender, true);
    }
    ...
    /**
     * QrVerification
     * @param eventId - Pass the event ID.
     * @param userAddress - Pass the user address.
     */
    function QrVerification(uint eventId, address userAddress) external view returns(bool paymentStatus){
        return forQrVerification[eventId][userAddress];
    }
