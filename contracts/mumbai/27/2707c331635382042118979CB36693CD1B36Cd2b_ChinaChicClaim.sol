...
/**
 * @title ChinaChic Claim contract
 * @dev Contract that distrubute earning made by China Chic DAO
 */
contract ChinaChicClaim is Ownable, Pausable, ReentrancyGuard {
    ...
    function deposit(uint256 amount) public onlyOwner {
        ...
        uint256 split = amount / MAX_SUPPLY;
        balances.push(split);
    }
        ...
        returns (string memory)
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            output = abi.encodePacked(output, "-", tokenIds[i]);
        }
    ...
    function claim(
        ...
        require(matchSigner(hash, signature), "Signature not authenticated");
        require(!nonces[nonce], "Hash reused");

        string memory ids = concat(tokenIds);
        require(hashMessage(_msgSender(), nonce, ids) == hash, "Hash failed");

        nonces[nonce] = true;
        uint256 totalDeposit = getTotalDeposit();
        require(claimed[_msgSender()] < totalDeposit, "Already claimed");

        uint256 remaining = totalDeposit - claimed[_msgSender()];
        claimed[_msgSender()] = totalDeposit;
        ERC20(token).transfer(msg.sender, remaining);
    }
}
