...
contract Demo is MaxAccess
  ...
  constructor() ERC721("Demo 721", "Demo") {}

  modifier presaleChecks() {
    if (balanceOf(msg.sender) >= 4) {
      revert MaxSplaining({
        reason: "Token: Tap the hate brakes Thanos, 4 is the max for now, no cap fr fr!"
      });
    }
    _;
  }
  ...
  modifier saleChecks() {
    if (balanceOf(msg.sender) >= 10) {
      revert MaxSplaining({
        reason: "Token: Ok boomer, 10 is now the max, you have it already."
      });
    }
    _;
  }
  ...
  // @notice: This sets the contract as royalty reciever (useful with PaymentSplitterV2.sol)
  ...
  // @notice: This sets royalties per EIP2981
  ...
  // @notice: This clears all EIP2981 royalties (address(0) and 0%)
   ...
   *  -#+%%+. -#%+   %@  -%%= [email protected]*@*@@=     =-  :#%+:#%%=  *@+%*.     -**-   *@%@[email protected]%:    :#%#-  
   *
   * @dev: This is the internal -> external functions for ContractURI.sol
   */
  ...
  function supportsInterface(
    ...
    return (
      ...
      interfaceId == type(TimeCop).interfaceId  ||
      ...
      interfaceId == type(lzLlamas).interfaceId  ||
      interfaceId == type(ERC721Burnable).interfaceId  ||
      interfaceId == type(NonblockingReceiver).interfaceId  ||
...
abstract contract lzLlamas is IMAX721, ILlamas {
...
abstract contract TimeCop is MaxAccess
  ...
  function showPresaleTimes()
    ...
    override (ITimeCop)
    returns (uint, uint) {
    return (startPresale, startPresale + presaleDuration);
  }
...
abstract contract PaymentSplitterV2 is MaxErrors {
  ...
  function claim()
    ...
    if (_shares[check] == 0) {
      revert MaxSplaining({
        reason: "PaymentSplitter: You have no shares"
      });
    }
    ...
    if (payment == 0) {
      revert MaxSplaining({
        reason: "PaymentSplitter: You are not due payment"
      });
    }
  ...
  , uint256 shares_
    ...
    if (account == address(0)) {
      ...
      revert MaxSplaining({
        reason: "PaymentSplitter: account already has shares"
      });
    }
 ...
 * @title: MaxErrors.sol
 * @author: Max Flow O2 -> @MaxFlowO2 on bird app/GitHub
 * @notice: Custom errors for all contracts
 */
...
abstract contract MaxErrors {
  ...
  // @dev: this is Unauthorized(), basically a catch all, and no fucks given
...
abstract contract ERC2981Collection is MaxErrors, IERC2981 {
  ...
  if (_permille >= 1000 || _permille == 0) {
    revert MaxSplaining({
      reason: "ERC2981Collection: Your permille is out of bounds, try again!"
    });
  }
...
abstract contract MaxAccess is MaxErrors
  ...
  function init() internal {
    contractRoles.add(OWNERS, TheOwner);
    contractRoles.add(OWNERS, TheDev);
    contractRoles.add(DEVS, TheDev);
  }

  // @dev: returns standard owner() for people
  // @return: address "TheOwner"
  function owner()
    external
    view
    virtual
  ...
  modifier onlyOwner() {
    ...
    if (!check) {
      revert MaxSplaining({
        reason: "MaxAccess: You unfortunately do not possess the might nor the power of onlyOwner()"
      });
    }
    _;
  }

  // @dev: sets display owner
  // @param _user: address to display
  ...
  function acceptOwner()
    ...
    if (!contractRoles.has(PENDING_OWNERS, msg.sender)) {
      revert MaxSplaining({
        reason: "MaxAccess: You unfortunately do not possess the might nor the power of a pending onlyOwner()"
      });
    }
  ...
  modifier onlyDev() {
    ...
    if (!check) {
      revert MaxSplaining({
        reason: "MaxAccess: You unfortunately do not possess the might nor the power of onlyDev()"
      });
    }
    _;
  }
  ...
  function acceptDeveloper()
    ...
    if (!contractRoles.has(PENDING_DEVS, msg.sender)) {
      revert MaxSplaining({
        reason: "MaxAccess: You unfortunately do not possess the might nor the power of a pending onlyDev()"
      });
    }
...
interface IOwner is IERC165 {
  ...
  // @dev: New isOwner from EIP173 roles
  // @param user: address to check
  // @return: bool for query
  function isOwner(
    address user
  ) external
    view
  ...
  // @dev: This is the classic EIP173 method of setting onlyOwner()
  // @param newOwner: addres of new pending owner role
  function pushOwner(
    address newOwner
  ) external;
...
interface IDeveloper is IERC165 {
  ...
  // @dev: New isDeveloper from EIP173 roles
  // @param user: address to check
  // @return: bool for query
  function isDeveloper(
    address user
  ...
  // @dev: This renounces your role as onlyDeveloper()
  function renounceDeveloper()
    external;

  // @dev: This starts the push-pull method of onlyDeveloper()
  // @param newDeveloper: addres of new pending Developer role
  function transferDeveloper(
    address newDeveloper
  ) external;

  // @dev: This accepts the push-pull method of onlyDeveloper()
  function acceptDeveloper()
    external;

  // @dev: This declines the push-pull method of onlyDeveloper()
  function declineDeveloper()
    external;

  // @dev: This is the classic EIP173 method of setting onlyDeveloper()
  // @param newDeveloper: addres of new pending Developer role
  function pushDeveloper(
    address newDeveloper
  ) external;
