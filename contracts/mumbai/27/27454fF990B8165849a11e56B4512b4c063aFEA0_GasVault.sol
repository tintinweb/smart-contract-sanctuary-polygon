    ...
    UUPSUpgradeable
{
    ...
    /**
     * @dev deposit more eth to be used in jobs. Can only be withdrawn by governance and the given vault, so in most cases these funds are unretrievable.
    ...
    /**
     * @dev withdraws eth from given vault. Can only be called by governance. Meant to be used for dead vaults and large amounts of mistakenly-deposited funds.
     * @param amount is amount of ETH to withdraw from given vault account
     * @param targetAddress is address to withdraw
     */
    function withdraw(
        uint256 amount,
    ...
    /**
     * @return transactions remaining, assuming upper limit estimate of gas price is used for the transaction
     * @param targetAddress the address of the vault in question
     * @param highGasEstimate an estimate of the highest reasonable gas price which a transaction will cost, in terms of wei. 
        In other words, given a bad gas price, how many more times can a strategy be run.
     */
     ...
     * @dev Orchestrator calls this function in order to reimburse tx.origin for method gas. First it checks that all parameters are correct (gas price isn't too high),
        and then it returns as much gas as is available to use in the transaction.
     * @dev note that this function will revert if the gas price is too high for the strategy. 
        ...
        returns (uint256)
    {
        ...
        // Ensure requested gas use is acceptable. wei / gas must be less than maxGasCost, and GasVault must have enough ether allotted to pay for action.
    ...
    /**
     * @param targetAddress the address which the action was performed upon. The reimbursement will come from its gas fund.
     * @param originalGas is how much gas there was at the start of the action (before any action was called)
     * @param jobHash is the hash of the job which was performed. All vaults other than DynamicJobs can only have one job, so in this case jobHash will just be actionHash.
