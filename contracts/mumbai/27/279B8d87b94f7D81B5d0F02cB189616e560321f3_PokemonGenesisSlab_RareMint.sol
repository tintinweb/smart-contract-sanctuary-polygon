...
/**
 * @title NFT Drop "Pokemon Genesis Slab", by RareMint, on Polygon Mainnet.
 */
contract PokemonGenesisSlab_RareMint is TokenDropBase {
    /// @dev Polygon Mainnet constructor.
    /// See {https://docs.chain.link/docs/vrf-contracts/v1/#polygon-matic-mainnet} for VRF parameters.
    constructor()
        TokenDropBase(
            "Pokemon Genesis Slab Tester | by RareMint",
            2, // Max 2 tokens per transaction
            100, // Max 100 tokens per address
            0.9 * 10**18, // 0.2 ETH on Polygon (subject to be updated before the drop)
            0x879bad9DcD7e7f79B598a632103984FC090DA00D, // TESTTOKEN on Polygon Testnet
            0x7637cE4C37A796d8cECd92C0c2441ab62c16EF79, // RareMint Engineering Multisig on Polygon Testnet
            0x2Ce391164Dd179F0B7736fc77E3753e878A123C2, // RareMint Registry deployed on Polygon Testnet
            0x8C7382F9D8f56b33781fE506E897a4F1e2d17255, // VRFCoordinator on Polygon Testnet
            0x326C977E6efc84E512bB9C30f76E30c160eD06FB, // LINK Token on Polygon Testnet
            0x6e75b569a01ef56d18cab6a8e71e6600d6ce853834d4a5748b720d06f878b3a4, // Chainlink keyHash on Polygon Testnet
...
abstract contract TokenDropBase is Context, ReentrancyGuard, Ownable, ERC20PaymentProcessor, PresaleList, TokenMintingPool {
    ...
    constructor(
        ...
        PresaleList()
        TokenMintingPool(tokenRegistryAddress, vrfCoordinator, linkToken, vrfKeyHash, vrfLinkFee)
    {
        name = dropName;
        maxPerTx = _maxPerTx;
     ...
     */
    function _mintableAmountForAddress(
        address minter,
     ...
     *  - Mints the tokens to the minter.
     */
    function _processMintRequestCC(address minter, address destination, uint256 requestedAmount) private {
        uint256 remainingSupply = remainingSupplyCount();
        require(requestedAmount > 0, "TokenDropBase: Invalid request for zero tokens.");
        require(remainingSupply > 0, "TokenDropBase: No more tokens left.");
        require(_mintsPerAddress[destination] < maxPerAddress, "TokenDropBase: Token limit per wallet already reached.");
        require(requestedAmount <= maxPerTx, "TokenDropBase: Token limit per transaction exceeded.");

        uint256 mintableAmount = _mintableAmountForAddress(destination, requestedAmount, remainingSupply);
        _mintsPerAddress[destination] += mintableAmount;
        _processPayment(minter, mintableAmount * mintPrice);
        _mintTokens(destination, mintableAmount);
    }
    ...
    function claimTokens(uint256 amount) external nonReentrant {
        ...
        /**
     * @dev Claim tokens to a specific address so cc checkout can work.
     *
     * Requirements:
     *  - if the public sale is not open:
     *      - the presale must be open
     ...
     *  - see {_processMintRequest}
     */
    function claimTokensToAddress(address to, uint256 amount) external nonReentrant {
        address caller = _msgSender();
        if (!_publicSaleIsOpen) {
            require(_presaleIsOpen, "TokenDropBase: The presale is not open yet.");
            require(_hasPresaleAccess(caller), "TokenDropBase: The public sale is not open yet and caller does not have presale address.");
...
abstract contract PresaleList {
    ...
    function _addAddressesToPresale(bytes32[] memory addressHashes) internal {
        ...
        for (uint256 ii = 0; ii < addressHashes.length; ii++) {
...
abstract contract TokenMintingPool is VRFConsumerBase {
    ...
    constructor(
        ...
        bytes32 vrfKeyHash,
        uint256 vrfLinkFee
    ) onlyValidRegistry(tokenRegistryAddress) VRFConsumerBase(vrfCoordinator, linkToken) {
