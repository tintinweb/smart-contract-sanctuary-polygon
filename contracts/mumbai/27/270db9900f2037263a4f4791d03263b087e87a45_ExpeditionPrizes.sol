    ...
    ExpeditionMeta
{
    ...
    uint256 internal constant PRIZE_TYPE_TRANSFER_NONE = 0;
    uint256 internal constant PRIZE_TYPE_TRANSFER_ERC20 = 1;
    uint256 internal constant PRIZE_TYPE_TRANSFER_ERC721 = 2;
    uint256 internal constant PRIZE_TYPE_TRANSFER_ERC1155 = 3;
    uint256 internal constant PRIZE_TYPE_MINTABLE_STAR = 4;
    uint256 internal constant PRIZE_TYPE_MINTABLE_GODIVERSE_COLLECTION = 5;
    ...
    /// @notice Link up the NubulaExpedition contract
    ...
    function claimTreasure(uint256 _treasureId) external nonReentrant {
        ...
        for (uint256 i = 0; i < prizes.length; i++) {
            if (prizes[i].prizeType == PRIZE_TYPE_TRANSFER_ERC20) {
                ...
                require(erc20.transfer(msg.sender, prizes[i].amount));
            } else if (prizes[i].prizeType == PRIZE_TYPE_TRANSFER_ERC721) {
            ...
            } else if (prizes[i].prizeType == PRIZE_TYPE_TRANSFER_ERC1155) {
            ...
            } else if (prizes[i].prizeType == PRIZE_TYPE_MINTABLE_STAR) {
                ...
                prizes[i].prizeType == PRIZE_TYPE_MINTABLE_GODIVERSE_COLLECTION
            ) {
    ...
    StakeAssetMeta
{
    ...
    /// @dev Determine if expedition is started or not
    /// @param _expeditionId Expedition id
    modifier isExpeditionPeriod(uint256 _expeditionId) {
        require(
    ...
    /// @dev Determine if expedition is claimable or not
    /// @param _expeditionId Expedition id
    modifier isExpeditionClaimable(uint256 _expeditionId) {
        require(
            getExpeditionState(_expeditionId) == STATE_CLAIMABLE,
            "Expedition is not claimable"
        );
        _;
    }

    /// @dev Determine if user is joined to expedition or not
    /// @param _expeditionId Expedition id
    modifier isJoinedExpedition(uint256 _expeditionId) {
        require(
            userExpeditions[msg.sender][_expeditionId].joinedAt > 0,
            "User require to join the expedition"
        );
        _;
    }
    ...
    /// @dev Determine if user is joined to expedition or not
    /// @param _expeditionId Expedition id
    modifier notJoinedExpedition(uint256 _expeditionId) {
        require(
            userExpeditions[msg.sender][_expeditionId].joinedAt == 0,
            "User is not joined to expedition"
        );
        _;
    }
    ...
    /// @notice Link up the ExpeditionStakingAndKeys contract
    /// @param _address ExpeditionStakingAndKeys contract address
    ...
    function joinExpedition(
        ...
        external
        isExpeditionPeriod(_expeditionId)
        notJoinedExpedition(_expeditionId)
    ...
    function stakeNFTBeforeExpeditionEnd(
        uint256 _expeditionId,
        ...
        if (
            ...
            for (
                ...
                for (uint256 j = 0; j < _tokenIds.length; j++) {
                    if (
                        expeditionInfoList[_expeditionId]
                            .optionalAssetWhitelistIds[i] == _tokenIds[j]
                    ) {
                        isExists = true;
                        break;
                    }
    ...
    /// @notice Let user to unstake NFT (planets or optional assets) before expedition
    /// @param _expeditionId Expedition id
    /// @param _assetAddress NFT address
    /// @param _tokenIds NFT token ids
    function unstakeNFTBeforeExpeditionEnd(
        uint256 _expeditionId,
        address _assetAddress,
        uint256[] memory _tokenIds
    )
        ...
        notJoinedExpedition(_expeditionId)
    {
        _unstakeNFTForExpedition(_expeditionId, _assetAddress, _tokenIds);
    }

    /// @notice Let user to unstake NFT (planets or optional assets) after expedition
    /// @param _expeditionId Expedition id
    /// @param _assetAddress NFT address
    /// @param _tokenIds NFT token ids
    function unstakeNFTAfterExpeditionEnd(
        uint256 _expeditionId,
        address _assetAddress,
        uint256[] memory _tokenIds
    )
        external
        nonReentrant
        isExpeditionClaimable(_expeditionId)
        isJoinedExpedition(_expeditionId)
    {
        _unstakeNFTForExpedition(_expeditionId, _assetAddress, _tokenIds);
    }

    /// @notice Do unstake NFT stuff
    function _unstakeNFTForExpedition(
        uint256 _expeditionId,
        address _assetAddress,
        uint256[] memory _tokenIds
    ) internal {
    ...
    StakeAssetMeta
{
    ...
    /// @param amounts Array of amounts of the asset
    event StakedAssets(
        address indexed user,
    ...
    function stakeAssets(
        ...
        if (assetTypes[_assetAddress] == ASSET_TYPE_ERC20) {
            ...
            for (uint i = 0; i < _tokenIds.length; i++) {
                ...
                require(
                    erc20.allowance(_stakeholder, address(this)) >=
                        _amounts[i] &&
                        erc20.balanceOf(_stakeholder) >= _amounts[i],
            ...
            require(
                erc721.isApprovedForAll(_stakeholder, address(this)),
                "Not approved"
            );
            ...
            for (uint i = 0; i < _tokenIds.length; i++) {
                require(
                    erc721.ownerOf(_tokenIds[i]) == _stakeholder &&
                        _amounts[i] == 1,
                    "Not owner"
                );
            }
            ...
            //check the approval of the token
            require(
                erc1155.isApprovedForAll(_stakeholder, address(this)),
                "Not approved"
            );
            ...
            for (uint i = 0; i < _tokenIds.length; i++) {
                require(
                    erc1155.balanceOf(_stakeholder, _tokenIds[i]) >=
                        _amounts[i],
                    "Not enough balance"
                );
            }
        ...
        if (assetTypes[_assetAddress] == ASSET_TYPE_ERC20) {
            ...
            for (uint i = 0; i < _tokenIds.length; i++) {
                //stake the asset
                require(
                    erc20.transferFrom(_stakeholder, address(this), _amounts[i])
                );
            }
        } else if (assetTypes[_assetAddress] == ASSET_TYPE_ERC721) {
            IERC721 erc721 = IERC721(_assetAddress);
    ...
    function unstakeAssets(
        ...
        if (assetTypes[_assetAddress] == ASSET_TYPE_ERC20) {
            ...
            for (uint i = 0; i < _tokenIds.length; i++) {
                require(erc20.transfer(_stakeholder, _amounts[i]));
            }
        } else if (assetTypes[_assetAddress] == ASSET_TYPE_ERC721) {
            IERC721 erc721 = IERC721(_assetAddress);
    ...
    function setupExchangeConfig(
        ...
        require(
            _exchangeRateForKey > 0,
            "exchangeRateForKey must be greater than 0"
        );
    ...
    function exchangeKeys(
        ...
        if (tokenToKey) {
            ...
            require(exchangeToken.transfer(_stakeholder, tokenAmount));
        }
    }
        ...
        onlyOwner
    {
        ...
        require(exchangeToken.transfer(_wallet, _amount));
    }
}
