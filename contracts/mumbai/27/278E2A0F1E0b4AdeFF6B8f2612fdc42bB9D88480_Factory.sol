...
contract Factory 
{
    ...
    {
        require(subscription != address(0), "SNS"); // Subscription not starte
        require(ISubscription(subscription).getActivationRemainingTime(SUBSCRIPTION_PRODUCT_ID, msg.sender) > 0, "ANA"); // Account not active
        _;
    }
    ...
    {
        require(reentrancyForTokenOwnerStatus[msg.sender] != STATUS_OWNER_ENTERED, "REETOWN");
    ...
    modifier validToken(address tokenAddress) 
    {
        ...
        require(tokenActive[tokenAddress] == 1, "INVTK"); // Invalid Token
        _;
    }
    ...
    modifier deployValidation(uint256 initialSupply, uint256 primaryDistributionAmount, uint256 feePercentForLiquidity)
    {
        ...
        require(feePercentForLiquidity <= ONE_HUNDRED, 'INFPL'); // Invalid fee percent for liquidity
    ...
    {
        require(tokenRegistered[tokenAddress] == 0, 'AREG'); // Already registered
        _;
    }
    ...
    modifier initialPriceValidation(uint256[] memory initialPrice, address[] memory initialPriceReceivingToken)
    {
        ...
        {
            require(initialPriceReceivingToken[ix] != address(0), "TKINVAD"); // Token invalid address
    ...
    {
        require(tokenPrimaryDistributionPeriod[tokenAddress][0] <= block.timestamp && tokenPrimaryDistributionPeriod[tokenAddress][1] >= block.timestamp, "PDNSOC"); // Primary Distribution Not Started Or Closed
        require(sourceAmount > 0, "ZSAM"); // Zero Source Amount
        require(tokenInitialPrice[tokenAddress][tokenSource] > 0, "UNDP"); // Undefined Price

        // PrimaryDistributionForecast memory forecast = getForecastBuyFromPrimaryDistribution( [tokenAddress, tokenSource], sourceAmount);
        // require(IERC20(tokenAddress).balanceOf( address(this) ) >= forecast.amountForBuyerToReceive, "NEB"); // Not enough balance
        _;
    }
    ...
    modifier changePrimateDistributionTimeValidation(address tokenAddress, uint[2] memory newValue)
    {
        ...
        if(tokenPrimaryDistributionPeriod[tokenAddress][0] <= block.timestamp)
        {
            ...
            require(newValue[0] == tokenPrimaryDistributionPeriod[tokenAddress][0], "SDCBM"); // Start Date Cannot be modified
        }
        require(newValue[0] < newValue[1], "ISET"); // Invalid Stard and End Time
        require(newValue[1] > block.timestamp, "IPDT"); // Invalid Primary Distribution Time
        _;
    }
    ...
    */
    ...
    function registerToken(address tokenAddress, uint256[4] memory valuedParameters, string memory data, address tokenOwnerAddress, uint256[] memory initialPrice, address[] memory initialPriceReceivingToken) public validAddress(tokenAddress) registerValidation(tokenAddress) allowanceForTransferFrom([tokenAddress, msg.sender, address(this)], valuedParameters[0])
    {
    ...
    */
    function setPlatformOwner(address newValue) external onlyPlatformOwner noReentrancyForPlatformOwner validAddress(newValue) validWallet
    {
        platformOwner = newValue;
    }

    function setSubscription(address value) external onlyPlatformOwner noReentrancyForPlatformOwner validWallet validAddress(value)
    {
        subscription = value;
    }

    function setPlatformFeePercent(uint256 newValue) external onlyPlatformOwner noReentrancyForPlatformOwner validWallet
    {
        platformFeePercent = newValue;
    }

    function setPlatformFeeAddress(address newValue) external onlyPlatformOwner noReentrancyForPlatformOwner validAddress(newValue) validWallet
    {
        platformFeeAddress = newValue;
    }

    function transferFund(address tokenAddress, address to, uint256 amountInWei) external onlyPlatformOwner noReentrancyForPlatformOwner validAddress(tokenAddress) validAddress(to) validWallet
    {
        //Withdraw token
    ...
    */
    ...
    function getForecastBuyFromPrimaryDistribution(address[2] memory addressesParameter, uint256 sourceAmount) public view returns (PrimaryDistributionForecast memory)
    {
        ...
        uint256 amountForBuyerToReceive = SafeMath.safeMulFloat(tokenInitialPrice[ addressesParameter[0] ][ addressesParameter[1] ], amountToBuyUsingSource, IERC20( addressesParameter[1] ).decimals() );
        ...
        PrimaryDistributionForecast memory forecast = PrimaryDistributionForecast({
            ...
            amountForTokenOwner: amountForBuyerToReceive - liquidityFeeValue
        });
    ...
    function buyFromPrimaryDistribution(address[3] memory addressesParameters, uint256 sourceAmount) external validAddress(addressesParameters[0]) validAddress(addressesParameters[1]) validToken(addressesParameters[0]) validWallet buyFromPrimaryDistributionValidation(addressesParameters[0], addressesParameters[1], sourceAmount) allowanceForTransferFrom([addressesParameters[1], msg.sender, address(this)], sourceAmount)
    {
        ...
        require(IERC20(addressesParameters[0]).balanceOf( address(this) ) >= forecast.amountForBuyerToReceive, "NEB"); // Not enough balance
    ...
    // Commented below cause it's alredy in getReceivingTokenDetails function
    // function getLiquidityBagAmount(address tokenAddress, address receivingTokenAddress) public view returns (uint256)
    // {
    //     return tokenLiquidityBag[tokenAddress][receivingTokenAddress];
    // }
