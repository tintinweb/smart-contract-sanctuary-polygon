    ...
    IBKZOpaqueAuctionInfo
{
    ...
    uint256 public BID_SLOTS; // 3

    uint256 public finalPrice;
    uint8 public auctionState;
    address public signer;

    // public bid, 1-3 (BID_SLOTS) items per address
    uint256 public totalDepositedETH;
    EnumerableSet.AddressSet totalBidders;
    mapping(address => uint256[]) public biddedSlots; // address => list of bidded slot
    mapping(uint256 => address[]) public slotBidders; // slot ID => bidders
    mapping(uint256 => mapping(address => uint256)) public slotUserBidETH; // slot ID => address => bid price/deposited in wei
    mapping(uint256 => mapping(address => bool)) public slotIsUserRefunded; // slot ID => address => user has refunded, whether as winner or loser
    ...
    function addBid(uint256 slot) external payable {
        require(auctionState == 1, "Auction not in bidding state");
        require(msg.value > 0, "msg.value must > 0");
        require(slot <= BID_SLOTS - 1, "invalid slot");

        mapping(address => uint256) storage userBidETH = slotUserBidETH[slot];
        ...
        if (userBidETH[msg.sender] == 0) {
            address[] storage bidders = slotBidders[slot];
            bidders.push(msg.sender);
            emit UserEnteredBid(msg.sender, newAmount, slot, true);
        } else {
            emit UserEnteredBid(msg.sender, newAmount, slot, false);
        }
        totalBidders.add(msg.sender);
    ...
    function refund(
        ...
        mapping(address => uint256) storage userBidETH = slotUserBidETH[slot];
        mapping(address => bool) storage isUserRefunded = slotIsUserRefunded[
            slot
        ];

        require(userBidETH[msg.sender] > 0, "No bid record");
        require(!isUserRefunded[msg.sender], "Already refunded");
        require(wonItemsCount <= 1, "Too many won items");

        checkValidity(
            signature,
            string.concat(
                "bkz:auction:refund:",
                Strings.toString(slot),
                ":won-items:",
        ...
        emit UserRefunded(msg.sender, refundAmount, slot, wonItemsCount);
    ...
    function getTotalBiddersLength() external view returns (uint256) {
        return totalBidders.length();
    }

    function getUsersBids(uint256 slot, address[] calldata bidders)
        external
        view
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < bidders.length; i++) {
            ...
            bids[i] = slotUserBidETH[slot][bidder];
        }
        return bids;
    }

    function getTotalBidders(uint256 fromIdx, uint256 toIdx)
        external
        view
        returns (address[] memory)
    {
        toIdx = Math.min(toIdx, totalBidders.length());
        address[] memory part = new address[](toIdx - fromIdx);
        for (uint256 i = 0; i < toIdx - fromIdx; i++) {
            part[i] = totalBidders.at(i + fromIdx);
        }
        return part;
    }

    function getBiddersLength(uint256 slot) external view returns (uint256) {
        address[] storage bidders = slotBidders[slot];
    ...
    function getBiddersAll(uint256 slot)
        external
        view
        returns (address[] memory)
    {
    ...
    function changeBidSlots(uint256 slots) external onlyOwner {
        require(auctionState == 0, "Auction already started");
        BID_SLOTS = slots;
    }
