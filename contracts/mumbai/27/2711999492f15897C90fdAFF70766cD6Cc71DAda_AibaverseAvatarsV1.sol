...
import "./interfaces/IAibaverseAvatars.sol";

contract AibaverseAvatarsV1 is
    Initializable,
    UUPSUpgradeable,
    OwnableUpgradeable,
    ERC2981Upgradeable,
    ...
    IAibaverseAvatars
{
    ...
    function initialize(
    ...
    ) public virtual initializerERC721A initializer {
        __ERC721A_init("Aibaverse Avatars", "AIBA");
        __Ownable_init();
        __ERC2981_init();

        privatePrice = 50 ether;
        publicPrice = 75 ether;
        ...
        _setDefaultRoyalty(_royaltiesReceiver, 1000);
    }
    ...
    modifier verifyWalletSupply(uint256 amount) {
        if (_numberMinted(msg.sender) + amount > MAX_MINTS)
    ...
    function updateBaseURI(string memory uri) external onlyOwner {
        if (!revealed) revert NotRevealedError();
        baseURI = uri;
    }
    ...
    function mintPrivate(
        ...
        verifyAmount(_amount, privatePrice)
        verifyWalletSupply(_amount)
    ...
    function mintOwner(
        address _to,
        uint256 _amount
    ) external onlyOwner verifyAvailableSupply(_amount) {
        _mint(_to, _amount);
    }
    ...
    function commit(
        ...
        if (revealed) revert AlreadyRevealedError();
        provenanceHash = _provenanceHash;
        futureBlockToUse = block.number + 5;
    }

    function reveal(string memory uri) external payable onlyOwner {
        if (revealed) revert AlreadyRevealedError();
        if (futureBlockToUse == 0) revert NotCommittedError();
        if (block.number < futureBlockToUse) revert TooEarlyForRevealError();
        ...
        return
            bytes(uri).length != 0
                ? string(
                    abi.encodePacked(
                        uri,
                        _toString((tokenId + tokenIdShift) % MAX_SUPPLY)
                    )
                )
                : hiddenURI;
    }
...
interface IAibaverseAvatars is IERC721AUpgradeable {
    ...
    error AlreadyRevealedError();
    error AlreadyCommittedError();
    error NotCommittedError();
    error TooEarlyForRevealError();
