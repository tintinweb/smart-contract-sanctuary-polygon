...
contract ERC20InvictusFundV3 is OwnableUpgradeable, ERC20PresetMinterPauserUpgradeable {
  ...
  address public withheldAddress;
  uint24 public requeueNonce;
  ...
  event RequeueRedemptionEvent(uint256 withdrawAmount, string ticker, address participant, uint24 requeueNonce);
  ...
  /**
   * @dev Updates the withheld address.
   * @param newWithheldAddress The new address where withheld funds are sent to.
   */
  function updateWithheldAddress(address newWithheldAddress) public onlyOwner {
    require(newWithheldAddress != address(0), "Invalid address");
    withheldAddress = newWithheldAddress;
  }
  ...
  /**
   * @dev Updates the requeue nonce. (Increments every time a redemption is kicked off the queue)
   * @param _requeueNonce New nonce assigned to a requeue event.
   */
  function updateRequeNonce(uint24 _requeueNonce) internal {
    requeueNonce = _requeueNonce;
  }
  ...
  function _handleRedemptionsDealing() private returns (bool) {
    ...
    for (uint256 i = redemptionsLength(); i > endingIndex; i--) {
      ...
      if (!InvictusWhitelist(whitelistContract).isWhitelisted(redemption.investor)) {
        redemptions.pop();
        transferFrom(withheldAddress, redemption.investor, redemption.amount);
        ...
        redemptions.pop();
        emit RequeueRedemptionEvent(redemption.amount, redemption.ticker, redemption.investor, requeueNonce);
        updateDealingNonce(requeueNonce = 1);
      }
    }
