...
contract ProtocolManager is IProtocolManager, AccessControlEnumerable {
    ...
    modifier onlyMaintainerOrDeployer() {
        require(
            hasRole(MAINTAINER_ROLE, msg.sender)
                || protocolFinder.getImplementationAddress(ProtocolInterfaces.Deployer) == msg.sender,
    ...
    /**
     * @notice Constructs the Protocol Manager contract
     * @param _protocolFinder Protocol finder contract
        ...
        onlyMaintainerOrDeployer
    {
        ...
        require(rolesCount == accounts.length, 'Number of roles and accounts must be the same');
        require(rolesCount == contracts.length, 'Number of roles and contracts must be the same');
     ...
     * @param roles Roles id
     */
    function renounceProtocolRole(address[] calldata contracts, bytes32[] calldata roles)
        external
        override
        onlyMaintainerOrDeployer
    {
        ...
        require(rolesCount == contracts.length, 'Number of roles and contracts must be the same');
        for (uint256 i; i < rolesCount; i++) {
            IAccessControlEnumerable(contracts[i]).renounceRole(roles[i], address(this));
        }
    }
     ...
     */
    function emergencyShutdown(IEmergencyShutdown[] calldata contracts) external override onlyMaintainer {
