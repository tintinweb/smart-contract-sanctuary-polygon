...
contract PerpetualTradeLogic is PerpetualUpdateFunctions, IFunctionList, IPerpetualTradeLogic {
        ...
        returns (int128)
    {
        ...
        if ((isLong && fkStar < 0) || (!isLong && fkStar > 0)) {
            ...
            int128 fTotalDefaultFunds = liqPool.fDefaultFundCashCC;
            int128 fPnLParticipantCash = _getCollateralTokenAmountForPricing(liqPool);
            // account for excess LP funds
            if (fPnLParticipantCash > liqPool.fTargetAMMFundSize) {
                fTotalDefaultFunds = fTotalDefaultFunds.add(
                    fPnLParticipantCash.sub(liqPool.fTargetAMMFundSize)
                );
            }
            int128 fundingRatio = fTotalDefaultFunds.div(liqPool.fTargetDFSize);
    ...
    function _transferProtocolFee(
        ...
        emit UpdateDefaultFundCash(_liqPool.id, _fTreasuryFee, _liqPool.fDefaultFundCashCC);

        // executor gets margin token
        _transferFromVaultToUser(_liqPool, _executorAddr, _fReferralRebate);
        emit TransferFeeToexecutor(_perpetual.id, _traderAddr, _executorAddr, _fReferralRebate);
    }
     ...
     * @param   _traderAddr         The address of trader.
     * @param   _executorAddr       The address of executor who will get rebate from the deal.
     * @param   _brokerAddr         The address of executor who will get rebate from the deal.
