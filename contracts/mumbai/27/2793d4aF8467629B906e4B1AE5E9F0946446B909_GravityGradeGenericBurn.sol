...
contract GravityGradeGenericBurn is IGravityGradeGenericBurn, OwnableUpgradeable {
    ...
    function burnPack(
        ...
        uint256 requestId = s_randNumOracle.getRandomNumber(randWordsRequest);
        ...
        onlyOracle
    {
        ...
        for (uint256 i; i < categories.length; i++) {
            ...
            for (uint256 j; j < openings; j++) {
                uint256 target = randomWords[j] % categories[i].contentAmountsTotalWeight;
                ...
                for (uint256 k; k < draws; k++) {
                    uint256 target = randomWords[k] % categories[i].contentsTotalWeight;
                    uint256 index = _binarySearch(categories[i].tokenWeights, target);
                    _mintReward(user, categories[i], index);
                }
            }
        }
