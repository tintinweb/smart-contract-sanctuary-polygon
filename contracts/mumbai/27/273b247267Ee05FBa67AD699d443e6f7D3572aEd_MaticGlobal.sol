...
contract MaticGlobal {
    ...
    uint256[6] public noWorkingCapping = [20 ether,50 ether,100 ether,200 ether,1000 ether,2000 ether];
  ...
  function updateNonWorkingCapping(address _user,uint256 _Amount) internal{  
      ...
      if(_Amount==defaultPackages[0]){
           user.nonworkingIncomeCapping += noWorkingCapping[0];
      }
      else  if(_Amount ==defaultPackages[1]){
           user.nonworkingIncomeCapping += noWorkingCapping[1];
      }
      else  if(_Amount ==defaultPackages[2]){
           user.nonworkingIncomeCapping += noWorkingCapping[2];
      }
      else  if(_Amount ==defaultPackages[3]){
           user.nonworkingIncomeCapping += noWorkingCapping[3];
      }
      else  if(_Amount ==defaultPackages[4]){
           user.nonworkingIncomeCapping += noWorkingCapping[4];
      }
      else  if(_Amount ==defaultPackages[5]){
           user.nonworkingIncomeCapping += noWorkingCapping[5];
      }  
  }
  ...
  function getEligibleWithdrawal(address _user) public view returns(uint8 reivest, uint8 withdrwal){  
   ...
   function getRandom(uint _FromSingleLegLength,uint _ToSingleLegLength) public view returns (uint) {
     uint randomnumber = uint(keccak256(abi.encodePacked(now, msg.sender))) % _ToSingleLegLength;
     randomnumber = randomnumber + _FromSingleLegLength;
     return randomnumber;
   }

   function _distributeNonWorkingFromPool(uint256 _Amount,uint _TotalNoofUser,uint _FromSingleLegLength,uint _ToSingleLegLength) internal{
        if(_Amount <= availablepool_amount){
        uint256 payableAmount = _Amount.div(_TotalNoofUser);
        for (uint i = 0; i < _TotalNoofUser; i++) {
            address _user=singleLeg[getRandom(_FromSingleLegLength,_ToSingleLegLength)];
            if (singleLeg[getRandom(_FromSingleLegLength,_ToSingleLegLength)] != address(0)) {
   ...
   function distributeNonWorkingFromPool(uint256 _Amount,uint _TotalNoofUser,uint _FromSingleLegLength,uint _ToSingleLegLength) external {
	     require(primaryAdmin==msg.sender, 'Admin what?');
	    _distributeNonWorkingFromPool(_Amount,_TotalNoofUser,_FromSingleLegLength,_ToSingleLegLength);	    
   }
