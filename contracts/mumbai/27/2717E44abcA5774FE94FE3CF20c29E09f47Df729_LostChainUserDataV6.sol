...
contract LostChainUserDataV6 is CustomERC2771ContextUpgradeable, UUPSUpgradeable {
    ...
    ) internal override onlyRole('DEFAULT_ADMIN_ROLE', _msgSender()) {}
    ...
    function getCurrentEnergy(address playerAddress) internal view returns (uint8) {
        ...
        // Refilled
        if (
            playerEnergy[playerAddress].usedAt != 0 &&
            block.timestamp >= playerEnergy[playerAddress].usedAt + LCGS.getRechargeDuration()
    ...
    function getEnergyDetail(address playerAddress) public view returns (uint8, uint256) {
        require(playerAddress != address(0x0), 'LCUD: Addess is zero address');
        uint8 currentEnergy = getCurrentEnergy(playerAddress);
        uint256 nextRechargeTime = getNextRechargeTime(playerAddress);
        if (nextRechargeTime == 0) currentEnergy = LCGS.getMaxEnergy();

        return (currentEnergy, nextRechargeTime);
    }
    ...
    function consumeEnergy(address operator, address playerAddress) external onlyRole('TRUSTED_FORWARDER', operator) {
        ...
        if (nextRechargeTime == 0) {
            ...
            require(currentEnergy > 0, 'LCUD: Out of energy');
            if (currentEnergy > LCGS.getMaxEnergy()) currentEnergy = LCGS.getMaxEnergy();

            playerEnergy[playerAddress].energy = currentEnergy - 1;
        }
    }
