 ...
 */
error NotMinter();
error InvalidArg_TokenAndAddressLengthMismatch();

contract Structured10MRelease is ERC1155Upgradeable, OwnableUpgradeable, BaseRelayRecipient {
  ...
  function initialize(address routerAddress, address minterAddress, address owner, address forwarder) public initializer {
    __ERC1155_init("_");
  ...
  function batchMint(uint256[] calldata tokenIds, address[] calldata ownerIds) public onlyMinter {
    if (tokenIds.length != ownerIds.length) {
      revert InvalidArg_TokenAndAddressLengthMismatch();
    }
...
error UndefinedRouteForItemIndex(uint256 itemIndex, uint totalRoutes, uint256 itemsPerRoute);
error NotRouteRegisterer();
error InvalidArg_AddressZero();
error InvalidArg_ItemsPerRouteZero();
...
contract Structured10MRouter is OwnableUpgradeable {
  ...
  string[] public _routes;
  uint256 public _itemsPerRoute;
  ...
  function registerRoute(uint256 index, string memory cid) public {
    if (_msgSender() != _routeRegisterer) {
      revert NotRouteRegisterer();
    }
  ...
  function lookup(uint256 itemIndex) public view returns (string memory) {
    ...
    if (bytes(cid).length == 0) {
      revert UndefinedRouteForItemIndex({itemIndex: itemIndex, totalRoutes: _routes.length, itemsPerRoute: _itemsPerRoute});
    }
    return cid;
  }
  ...
  function setRouteRegisterer(address account) public onlyOwner {
    ...
    emit RouteRegistererUpdated({current: account, previous: _routeRegisterer});
    _routeRegisterer = account;
  }

  /**
   * Sets `_itemsPerRoute` (note that only the `owner` wallet can execute this action)
   * This function should be used cautiously. This redistributes tokenIDs to potentially different CIDs.
   */
  function setItemsPerRoute(uint256 itemsPerRoute) public onlyOwner {
    if (itemsPerRoute == 0){
      revert InvalidArg_ItemsPerRouteZero();
    }
    emit ItemsPerRouteUpdated({current: itemsPerRoute, previous: _itemsPerRoute});
    _itemsPerRoute = itemsPerRoute;
  }
}
