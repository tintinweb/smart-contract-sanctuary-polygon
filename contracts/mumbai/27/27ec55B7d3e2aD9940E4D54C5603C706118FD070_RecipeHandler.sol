...
contract RecipeHandler is Ownable, StudioSignature {
    ...
    event SummonSuccess(address indexed account, uint256[] hunterSeeds);
    ...
    constructor(
        ...
        for (uint256 i = 0; i < _chestGasLimits.length; i++) {
            chestGasLimits[i] = _chestGasLimits[i];
        }
    ...
    function summon(RecipeSignature calldata packet) external {
        ...
        emit SummonSuccess(user, outHunterSeeds);
        ...
        // OUT - Hunters
        hunters.studioMint(user, outHunterSeeds.length);
    }
