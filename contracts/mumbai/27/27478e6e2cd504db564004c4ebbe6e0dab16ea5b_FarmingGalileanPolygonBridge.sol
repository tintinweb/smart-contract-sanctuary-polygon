...
contract FarmingGalileanPolygonBridge is
    Initializable,
    GameContract,
    AccessControlUpgradeable
{
    ...
    function deposit(address user, bytes calldata depositData) external {
        ...
        if (depositData.length == 32) {
            ...
            galilean.bridgeMint(user, tokenId);
            ...
            for (uint256 i; i < length; i++) {
                galilean.bridgeMint(user, tokenIds[i]);
            }
        }
    }

    function withdrawWithMetadata(uint256 tokenId) external onlyGameContract {
        // ownership already checked

        // Encoding metadata associated with tokenId & emitting event
        emit TransferWithMetadata(
            galilean.ownerOf(tokenId),
            address(0),
            tokenId,
            abi.encode(galilean.tokenURI(tokenId))
        );

        galilean.burn(tokenId);
    }
