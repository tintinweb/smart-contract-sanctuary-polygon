...
contract DrownTheClown is ReentrancyGuard {
    ...
    */
    ...
    // Write out the round, address, MATIC winnings
    event RoundWinner(uint, address payable, uint);

    // Log the winner of an NFT
    event TokenAwarded(uint, address, uint);
    ...
    function pickWinner(uint newCostPerPlayer,uint newTotalPlayerLimit, uint256[3] calldata purses, uint newSideBetCost, uint seed) public onlyOwner nonReentrant {
        ...
        // Loop through and get the 3 winners
        ...
        if (tokens.tokensRemaining() > 0){
            ...
            emit TokenAwarded(gameNumber, firstPlace, tokenWon);
        }
