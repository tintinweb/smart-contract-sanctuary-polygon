...
// File: RestaurantReliefBuilder.sol

/**
 * @title Restaurant Relief Builder
 * @dev builds the dynamic content of the Restaurant Relief NFTs
 */
contract RestaurantReliefBuilder is AbstractDynamicURI, BaseViciContract {
     ...
     * - caller MUST be the contract owner or have the URI_MANAGER role.
     */
    function setFooter(string memory newFooter) public onlyURIManager {
        footer = newFooter;
    }
        ...
        returns (string memory)
    {
        ...
        return
            string.concat(
                svg.text(textProps("total", "590", "amt"), oracle.progress()),
                svg.text(
                    textProps("label", "660", "norm"),
                    "Restaurant Relief Fund"
                ),
                svg.text(textProps("yours", "790", rClass), rName),
                svg.text(
                    textProps("noms", "864", "vote"),
                    string.concat(
                        Strings.toString(restaurant.votes),
                        " Nominations"
                    )
                ),
                svg.text(textProps("foot", "1000", "norm"), footer)
            );
    }
