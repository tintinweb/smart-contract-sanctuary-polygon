...
contract Vesting is IVesting, Ownable {
  ...
  function revokeVestingOfUser(address _beneficiary, uint256 _vestingId)
    external
    onlyOwner
  {
    require(_vestingId < vestings[_beneficiary].length, "Incorrect vesting id");
    require(
      vestings[_beneficiary][_vestingId].revocable,
      "This vesting is irrevocable"
    );
    require(
  ...
  function getWithdrawableAmount(address _beneficiary)
    external
    view
    returns (uint256)
  {
  ...
  function _createVesting(
    ...
    require(
      _vestingSchedule.vestingDuration > 0,
      "Duration should be bigger than zero"
    );
    require(
      _initialPercentUnlock <= PRECISION,
      "Initial percentage exceed 100%"
    );
    // solhint-disable-next-line not-rely-on-time
    uint256 currentTime = block.timestamp;
    require(
      _vestingSchedule.startDate +
        _vestingSchedule.vestingDuration +
  ...
  function _getVesting(address _beneficiary)
    internal
    view
    returns (Vesting[] storage)
  {
    require(
      vestings[_beneficiary].length != 0,
      "There is no vesting for release"
    );
  ...
  function _calculateAvailableAmount(Vesting memory _vesting)
    internal
    view
    returns (uint256)
  {
    ...
    uint256 startAt = _vesting.vestingSchedule.startDate +
      _vesting.vestingSchedule.lockPeriod;
