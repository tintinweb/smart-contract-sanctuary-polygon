...
library CFPS {
    ...
    string constant STR_MINIMUM_DONATION = "Minimal donation is 10000 Wei";
...
ReentrancyGuard
{
    ...
    uint256 runningCampaigns = 0;


    uint256 private campaignId = 1;
    uint256 private donationId = 1;
    uint256 private commission = 100;
    uint256 private commissionBalance = 0;
    uint256 private tokenUriMethod = 0;


    address _royaltiesRecipient;
    string public name = "Crowdfunding on Polygon";
    string public symbol = "CFPT";

    function getCampaignInfo(uint256 cid)public view returns(uint256 cId, address cPublisher, uint256 cBalance, uint256 cTarget, uint cStart, uint cEnd)
    {
        return (cid, campaignId2Publisher[cid], campaign2Balance[cid] - campaign2Balance[cid]/commission, campaign2Target[cid], campaign2Start[cid], campaign2End[cid]);
    }
    ...
    function revokeDonation(uint256 targetDonationId) payable public
    {
        ...
        campaign2Balance[donation2Campaign[targetDonationId]] -= donationAmount[targetDonationId];
    ...
    /*
        The existingDonationId parameter should be 0 unless a backer wishes to donate more to
        a campaign he/she has already donated to.
    */
    function donate(uint256 targetCampaignId, uint256 existingDonationId) payable public
    {
        require(msg.value >= 10000, CFPS.STR_MINIMUM_DONATION);
    ...
    function dissolveCampaign(uint256 cid) public onlyOwner
    {
        ...
        campaignDissolved[cid] = 1;
        campaignId2Publisher[cid] = address(0);
        campaign2End[cid] = 0;
        runningCampaigns--;
    }
    ...
    function closeCampaign(uint256 cid) public
    {
        require(campaign2End[cid] != 0, CFPS.STR_CAMPAIGN_CLOSED);
        ...
        uint256 campaignCommission = campaign2Balance[cid] / commission;
        uint256 dueBalance = campaign2Balance[cid] - campaignCommission;

        if (dueBalance > 0)
        {
            address payable recipient = payable(campaignId2Publisher[cid]);
            recipient.transfer(dueBalance);
            commissionBalance += campaignCommission;
        }
    ...
    function _clearCampaignMappings(uint256 cid) internal
    {
        delete publisher2Campaign[campaignId2Publisher[cid]];
        delete campaignId2Publisher[cid];
        delete campaignDonationCount[cid];
        delete campaign2Balance[cid];
        delete campaign2Target[cid];
    ...
    function createCampaign(uint startDate, uint endDate, uint256 target) public whenNotPaused
    {
        ...
        require(target > commission, CFPS.STR_TARGET_TOO_LOW);
    ...
    function getDissolvedStatus(uint256 cid)public view returns(uint256)
    {
        return campaignDissolved[cid];
    }
    ...
    function campaignPublisher(uint256 cid) public view returns (address)
    {
        return campaignId2Publisher[cid];
    }
    ...
    function getCommissionBalance() public onlyOwner view returns (uint256)
    {
    ...
    {
        require(commissionBalance > 0, CFPS.STR_NO_FUNDS);
        payable(_royaltiesRecipient).transfer(commissionBalance);
        commissionBalance = 0;
    }
    ...
    function getCommissionRate() public view onlyOwner returns(uint256)
    {
        return commission;
    }
