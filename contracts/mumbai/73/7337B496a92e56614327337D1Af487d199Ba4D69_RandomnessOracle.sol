...
contract RandomnessOracle is AccessControlEnumerable, IRandomnessOracle {
  bytes32 public constant REQUESTER_ROLE = keccak256("REQUESTER_ROLE");

  VRFCoordinatorV2Interface public override coordinator;
  ...
  function topUpSubscription(uint64 subscriptionId, uint256 amount) external {
  ...
  function removeConsumer(uint64 subscriptionId, address consumerAddress)
    external
  ...
  function cancelSubscription(uint64 subscriptionId, address receivingWallet)
    external
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
