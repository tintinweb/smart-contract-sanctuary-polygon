...
contract ProofOfCrypto is AccessControl, ERC2771Context {
     ...
     */
    function setNFTAddress(address _nftAddress) public onlyPoCAdmin {
        require(_nftAddress != address(0), "PoC: Invalid address");
    ...
    function mintNFT(
        ...
        require(
            ...
            "PoC: Only CREATOR with valid signature"
        );
        nftAddress.mintNFT(to, id, amount, data, tokenUri);
    }

    /**
     * @dev Public function to mint a single new token and approve PoC contract
     * Reverts if the caller is not trusted forwarder contract
     * @param to The address to which the newly minted tokens will be assigned.
     * Requirements -
     *   to cannot be a null address
     ...
     * Reverts if the caller is not PoC Admin
    ...
    function burnNFT(
        ...
        require(
            ...
            "PoC: Only token owner with valid signature can burn his tokens"
        );
    ...
    function transferNFT(
        ...
        require(_isContract(splitter), "PoC: Invalid splitter address");
        ...
        musdcAddress.transferFrom(to, splitter, value);
    ...
    function approveAndTransferNFT(
        ...
        require(
            from == _msgSender(),
            "PoC: Only token owner with valid signature"
        );
    ...
    /**
     * @dev Public function to mint new tokens in batches and approve them to the PoC contract
     * Reverts if the caller is not trusted forwarder contract
     * Reverts if the length of ids and amounts is not equal
     * @param to The address to which the newly minted tokens will be assigned.
     * Requirements -
    ...
    function burnNFTBatch(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            require(
                ...
                "PoC: Only token owner with sufficient balance can burn his tokens"
            );
        }
    ...
    function transferNFTBatch(
        ...
        // transfer USDC to creator
        musdcAddress.transferFrom(to, splitter, total);
