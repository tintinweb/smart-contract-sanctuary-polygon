...
// File: contracts/OracleAccessControl.sol

pragma solidity ^0.8.0;


abstract contract OracleAccessControl is AccessControl {
    ...
    modifier whenNotPaused {
        require(!isPaused, "Oracle Functionality  is paused");
        _;
    }
    ...
    function updateDefaultOwner(address _newOwner) public onlyRole(DEFAULT_ADMIN_ROLE) {
        renounceRole(DEFAULT_ADMIN_ROLE, defaultOwner);
        defaultOwner = _newOwner;
...
contract OracleProxy is OracleAccessControl{
    ...
    function setUseOracle(bool _status)public onlyRole(OWNER_ROLE){
        useOracle = _status;
    }
    function setDefaultPrice(uint256 _def)public onlyRole(OWNER_ROLE){
        defaultPrice = _def;
    }

    function setOracle(AggregatorV3Interface _ora)public onlyRole(OWNER_ROLE){
        oracle = _ora;
    }
    ...
    function getONEUSD() public view returns (uint256,uint8) {
