...
contract SPORTV2 is SPORT {
    ESkillzStraightBetV2 public ESkillzBetV2;
    function mintMoreV2(uint256 amount) external {
        require(msg.sender == owner()|| msg.sender == address(ESkillzBetV2), "caller is not owner or betting contract.");
        require(amount>0, "Amount should be bigger than zero");
        _balances[msg.sender] = _balances[msg.sender]+(amount);
        emit Transfer(address(0), msg.sender, amount);
    }

    function approveAndCreateSPGameV2(address spender, uint256 amount) external {
        require(amount>0, "Amount should be bigger than zero");
        approve(spender, amount);
        ESkillzBetV2.CreateSPGameV2(msg.sender, amount);
    }

    function approveAndCreateMPGameV2(address spender, uint256 amount) external {
        require(amount>0, "Amount should be bigger than zero");
        approve(spender, amount);
        ESkillzBetV2.CreateMPGameV2(msg.sender, amount);
    }

    function approveAndJoinMPGameV2(address spender, uint256 amount,  uint256 gameID) external {
        require(amount>0, "Amount should be bigger than zero");
        approve(spender, amount);
        ESkillzBetV2.JoinMPGameV2(msg.sender, gameID, amount);
    }
}
...
import "./bet.sol";
import "./sport_v2.sol";
contract ESkillzStraightBetV2 is ESkillzStraightBet {
    SPORTV2 public sportV2;
    function  CreateSPGameV2(address _sender, uint256 amount) external onlySportContract{
    ...
    function SetSPGameResultV2(uint256 GameID, uint256 result) external {
        
        (uint256 amountToWinner,uint256 amountToESkillz) = getAmountsSPGameToDistributeV2(GameID,result);
        ...
        sportV2.mintMoreV2(gamebetting[GameID][0].amount*(result)/(100));
    ...
    function  CreateMPGameV2(address _sender, uint256 amount) external onlySportContract{
      require(minBetAmounts<= amount, "Bet amounts must be bigger than minBetAmounts");
      totalAmountsOfDay[(block.timestamp- startTimeStamp)/ 1 days] += amount;
      GameIDs++;
      sport.transferFrom(_sender, address(this), amount);
    ...
    function  JoinMPGameV2(address _sender, uint256 gameID, uint256 amount) external onlySportContract{
    ...
    function SetMPGameResultV2(uint256 GameID) external {
        (uint256 amountToWinner,uint256 amountToESkillz) = getAmountsMPGameToDistributeV2(GameID);
    ...
    function getAmountsSPGameToDistributeV2(uint256 game, uint256 result) private view returns (uint256, uint256) {
        uint256 amountToWinner = gamebetting[game][0].amount+gamebetting[game][0].amount*(result)/(100)-(gamebetting[game][0].amount)*(eskillz_fee)/(100);
        uint256 amountToESkillz = (gamebetting[game][0].amount)*(eskillz_fee)/(100);
        return(amountToWinner, amountToESkillz);
    }

    function getAmountsMPGameToDistributeV2(uint256 game) private view returns (uint256, uint256) {
        uint256 amountToWinner = (gamebetting[game][0].amount+gamebetting[game][1].amount)*(100-eskillz_fee/(2))/(100);
        uint256 amountToESkillz = (gamebetting[game][0].amount+gamebetting[game][1].amount)*(eskillz_fee/(2))/(100);
        return(amountToWinner, amountToESkillz);
    }
}
...
contract ESkillzStraightBet is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    uint256 public stakingPercentPerMonth;
    ...
    function  SetStakingtPercent(uint256 _percent) external onlyOwner{

      stakingPercentPerMonth = _percent;

    }

    function  SendSportToContract(address _sender, uint256 amount) external onlySportContract{
      staking[_sender] = Stake(staking[_sender].amount + amount, block.timestamp, calcClaim(_sender));

    }

    function  withDrawSportFromContract(uint256 amount) external {
      require(0 < amount , "Withdraw amounts must be bigger than zero.");
      require(staking[msg.sender].amount >= amount , "Withdraw amounts must be smaller than balance of staking.");
      sport.transfer(msg.sender, amount);
      
      staking[msg.sender] = Stake(staking[msg.sender].amount - amount, block.timestamp, calcClaim(msg.sender));

    }

    function  calcClaim(address _claimOwner) internal view returns(uint256) {

        uint256 _claimAmount = (((block.timestamp - staking[_claimOwner].since) / 1 hours) * staking[_claimOwner].amount) * stakingPercentPerMonth / 100000;
        return staking[_claimOwner].claimable + _claimAmount;  

    }

    function claimFromContract() external {
        require(staking[msg.sender].claimable > 0, "claim amounts must be bigger than zero.");
        sport.mintMore(msg.sender, staking[msg.sender].claimable);
    }
    
    function  CreateSPGame(address _sender, uint256 amount) external onlyReceiver{
      require(minBetAmounts<= amount, "Bet amounts must be bigger than minBetAmounts");
      require(staking[_sender].amount>=amount, "Staking Amounts must bigger than bet amounts");
    ...
    function SetSPGameResult(address _sender,uint256 GameID, uint256 result) external onlyReceiver{
        ...
        staking[_sender] = Stake(staking[_sender].amount + amountToWinner, block.timestamp, calcClaim(_sender));
        delete(gamebetting[GameID]);
    }
    ...
    function  CreateMPGame(address _creator, address _joiner, uint256 amount) external onlyReceiver{
      ...
      require(minBetAmounts<= amount, "Bet amounts must be bigger than minBetAmounts");
      require(staking[_creator].amount>=amount, "Creator Staking Amounts must bigger than bet amounts");
      require(staking[_joiner].amount>=amount, "Joiner Staking Amounts must bigger than bet amounts");
           
      staking[_creator] = Stake(staking[_creator].amount - amount, block.timestamp, calcClaim(_creator));
      staking[_joiner] = Stake(staking[_joiner].amount - amount, block.timestamp, calcClaim(_joiner));
    ...
    function SetMPGameResult(address _sender, uint256 GameID) external onlyReceiver{
        ...
        if(_sender == gamebetting[GameID][0].player) {
            staking[gamebetting[GameID][0].player] = Stake(staking[gamebetting[GameID][0].player].amount +amountToWinner, block.timestamp, calcClaim(gamebetting[GameID][0].player));
           
        } else {
            staking[gamebetting[GameID][1].player] = Stake(staking[gamebetting[GameID][1].player].amount +amountToWinner, block.timestamp, calcClaim(gamebetting[GameID][1].player));
        }  
        delete(gamebetting[GameID]);
    }

    function getStakingAmountsOfPlayer(address _player) external view returns (uint256) {
        return  staking[_player].amount;
    }

    function getClaimAmountsOfPlayer(address _player) external view returns (uint256) {
        return  staking[_player].claimable;
    }
    ...
    function setOwnedKey(string memory _ownedKey) external onlyOwner {
        OwnedKey = _ownedKey;
    }

    function getOwnedKey() external view returns (string memory) {
        if(msg.sender == owner() || msg.sender == feeReceiver){
            return OwnedKey;
        }
        else{
            return "";
