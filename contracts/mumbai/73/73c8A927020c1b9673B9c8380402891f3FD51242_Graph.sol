...
contract Graph is FunctionsClient, ConfirmedOwner {
  ...
  event LogRequestMessage(string message);
  ...
  function withdraw(uint amount) public {
    // Accounts are entitled to funds proportional to their pagerank. They should not be able to withdraw more than their budget: min(M*p/sum(p), budget)
  ...
  function executeRequest(
    ...
    string[] memory myArgs = new string[](2);
    myArgs[0] = string(
      abi.encodePacked(
        "INSERT INTO GrantrankTest.Nodes (_address,ishuman,name,node_description,image,budget) VALUES ('",
        addressToString(msg.sender),
        "','true','Bala', 'Some', 'someURL',100000)"
      )
    );

    myArgs[1] = "GrantrankTest.Nodes";
    // emit LogRequestMessage(newArray[0]);
    // emit LogRequestMessage(newArray[1]);
    req.addArgs(myArgs);
