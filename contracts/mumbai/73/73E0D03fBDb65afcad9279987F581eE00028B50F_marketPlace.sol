...
contract marketPlace is OwnableUpgradeable, SignerImplementation {
        ...
        function mintNewTokens(Signer memory signer,string[] memory _uri) external {
                ...
                INft(signer._nftAddress).mintTokens(signer._user,signer.totalTokensToMint,_uri);
        }
        ...
        function executeOrder(Signer memory signer) external payable {
                ...
                nonceStatus[msg.sender][signer.nonce] = true;
                if (!signer.inEth)
                        fee  = calculateFees(signer.amount, signer._nftAddress);
                else
