    ...
    ProtocolFees
{
    ...
    event Fill(
        ...
        uint256 salt, // unique for each order
        uint256 filled
    );
    ...
    function _fillOrder(
        ...
        // Fetch order info
        LibOrder.OrderInfo memory orderInfo = getOrderInfo(
            order,
            makerAssetFillAmount
        );
        ...
        _updateFilledState(
            ...
            takerAddress,
            orderInfo.orderHash,
            orderInfo.orderMakerAssetFilledAmount,
            fillResults
        );

        // Settle order
        if (!order.isNative) {
            if (order.orderType == LibOrder.OrderType.MINT) {
                _dispatchMintTo(
                ...
                _settleOrder(order, takerAddress, fillResults);
            }
        } else {
            if (
                msg.value <
                takerAssetFillAmount.safeAdd(fillResults.takerFeePaid)
            ) {
    ...
    function getOrderInfo(
        ...
        if (
            orderInfo.orderMakerAssetFilledAmount.safeAdd(
                makerAssetFillAmount
            ) > order.oData.mAmount
        ) {
    ...
    function _getOrderHashAndFilledAmount(LibOrder.Order memory order)
        internal
        view
        returns (bytes32 orderHash, uint256 orderMakerAssetFilledAmount)
    {
        orderHash = order.getTypedDataHash(EIP712_EXCHANGE_DOMAIN_HASH);
        orderMakerAssetFilledAmount = filled[orderHash];
        return (orderHash, orderMakerAssetFilledAmount);
    }
    ...
    function _assertFillableOrder(
        ...
        if (
            ...
            takerAssetFillAmount < order.oData.tAmount * makerAssetFillAmount
        ) {
    ...
    function _updateFilledState(
        ...
        address takerAddress,
        bytes32 orderHash,
        uint256 orderMakerAssetFilledAmount,
        LibFillResults.FillResults memory fillResults
    ) internal {
        // Update state
        filled[orderHash] = orderMakerAssetFilledAmount.safeAdd(
            fillResults.makerAssetFilledAmount
        );
        emit Fill(
            order.oData.maker,
    ...
    function _settleOrder(
        ...
        for (uint256 i = 0; i < order.oData.others.length; ++i) {
            _dispatchTransferFrom(
                ...
                fillResults.othersFillAmount[i]
            );
        }
    ...
    function _cancelOrder(LibOrder.Order memory order) internal {
        ...
        LibOrder.OrderInfo memory orderInfo = getOrderInfo(order, 0);
...
library LibOrder {
    ...
    struct Order {
        ...
        ItemType itemType; //0: erc1155, 1:erc1155
        OrderData oData;
    }
    ...
    struct OrderData {
        ...
        uint256 tAmount; // Amount of takerAsset being bid on per 1 item. Must be greater than 0.
    ...
    struct OrderInfo {
        ...
        uint256 orderMakerAssetFilledAmount; // Amount of order that has already been filled.
    }
...
library LibFillResults {
    ...
    function calculateFillResults(
        ...
        fillResults.othersFillAmount = new uint256[](order.oData.others.length);
        for (uint256 i = 0; i < order.oData.others.length; ++i) {
            fillResults.othersFillAmount[i] = (order.oData.others[i].amount *
                makerAssetFillAmount);
            fillResults.takerAssetFilledAmount = fillResults
                .takerAssetFilledAmount
                .safeSub(order.oData.others[i].amount * makerAssetFillAmount);
        }

        fillResults.makerFeePaid = order.oData.mFee * makerAssetFillAmount;
        fillResults.takerFeePaid = order.oData.tFee * makerAssetFillAmount;
...
contract AssetProxyDispatcher is Ownable, IAssetProxyDispatcher {
    ...
    function _dispatchMintTo(
        ...
        if (amount > 0) {
            ...
            IAssetProxy(assetProxy).mint(assetData, to, amount);
        }
    }
}
