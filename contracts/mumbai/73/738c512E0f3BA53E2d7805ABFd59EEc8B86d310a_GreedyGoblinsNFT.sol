...
import { GreedyGoblinsLottery } from "./GreedyGoblinsLottery.sol";
import "@openzeppelin/contracts/utils/Counters.sol";

contract GreedyGoblinsNFT is Ownable, ERC721Checkpointable, GreedyGoblinsLottery {
    ...
    bool public presSaleIsActive = false;
    ...
    mapping(address => uint256) public _LotteryAllow;
    mapping(address => uint256) public _mintWhitelist;
 

    constructor()
      ERC721("Greedy Goblins NFT", "GGNFT") {
       setBaseURI("ipfs://QmYR1eqCKCxq5aZVoB2JLruNHjqFPx8nqVXfyYMJCKjqhd/");
     }

     function LotteryWinner(uint lottery) public onlyOwner returns (address payable) {
        winner = lotteryHistory[lottery];
        _LotteryAllow[winner]=1;
     ...
     function _mintWhiteList(uint8 numberOfTokens) private {
        uint256 ts = totalSupply();
     ...
     function _mintLottery(uint8 numberOfTokens) private {
        uint256 ts = totalSupply();
        uint256 LotteryTokens = 1;
        require(_LotteryAllow[winner]==1);
        require(LotteryIsActive, "Allow list is not active");
        require(ts + LotteryTokens <= MAX_SUPPLY, "Purchase would exceed max tokens");
        ...
        for (uint256 i = 1; i <= LotteryTokens; i++) {
         _safeMint(msg.sender, ts + i);
         _LotteryAllow[winner]=0;
         }
     }

     function mintNFT(uint8 numberOfTokens) public payable {
     uint8 number = _allowList[msg.sender];
     uint256 numberNFT = balanceOf(msg.sender);

     require(numberNFT+numberOfTokens<=200,"Maximum amount of Nft-s per account is 10");

     if ( _LotteryAllow[winner]==1) {
         _mintLottery(numberOfTokens); 
         }
     
     else if ( number!=0 ) {
        
      _mintWhiteList(numberOfTokens);
     } 


      else if(presSaleIsActive) {
...
contract GreedyGoblinsLottery is VRFConsumerBase {
    ...
    function pickWinner() public onlyLotteryAdmin {
        getRandomNumber();
    }
    ...
    function newLotteryAdmin(address newLotteryAdminAddress) public virtual onlyLotteryAdmin returns (address) {
        LotteryAdmin = newLotteryAdminAddress;
        return LotteryAdmin;
    }
}
