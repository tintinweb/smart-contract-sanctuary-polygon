...
contract PremioLoteria is VRFConsumerBase, Ownable{
    // Chainlink variables.
    ...
    event FirstPrizeWinner(address winner);
    event SecondPrizeWinner(address winner);
    event ThirdPrizeWinner(address winner);
    ...
    function addPlayers(address[] memory array) public onlyOwner{
    ...
    */
    function getRandomWinner() public onlyOwner returns (bytes32 requestId) {
   ...
   */
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal virtual override  {
        ...
        emitFirstPrizeWinner(randomness);
        emitSecondPrizeWinner(randomness);
        emitThirdPrizeWinner(randomness);
    }

    function emitFirstPrizeWinner(uint256 randomness) private{
        uint256 firstPrizeWinnerIndex = randomness % players.length;
        address winner = players[firstPrizeWinnerIndex];
        emit FirstPrizeWinner(winner);
    }

    function emitSecondPrizeWinner(uint256 randomness) private{
        uint256 secondPrizeWinnerIndex = randomness+1 % players.length;
        address winner = players[secondPrizeWinnerIndex];
        emit SecondPrizeWinner(winner);
    }

    function emitThirdPrizeWinner(uint256 randomness) private{
        uint256 thirdPrizeWinnerIndex = randomness-1 % players.length;
        address winner = players[thirdPrizeWinnerIndex];
        emit ThirdPrizeWinner(winner);
    }
