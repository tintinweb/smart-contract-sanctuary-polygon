...
contract Web3liminator_V0_1 is Initializable {
    ...
    function getAllEliminatedStatus() public returns (bool[] memory) {
    ...
    function bulkAddGames(
        ...
        for (uint256 i; i < 1; i++) {
            ...
            NFLGame memory newGame;
            newGame.week = game_weeks[i];
            newGame.homeTeam = home_teams[i];
            newGame.awayTeam = away_teams[i];
            newGame.kickoffTime = kickoff_times[i];
    ...
    modifier onlyOwner() {
        require(
            ...
            "Not the owner of the contract while trying to call onlyOwner function"
        );
        _;
    }
}
