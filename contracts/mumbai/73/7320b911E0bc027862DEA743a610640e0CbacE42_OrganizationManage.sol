 ...
 */
contract OazizOrganization is ERC725XCore, ERC725YCore {
    string public name;
    string public symbol;
    address public OrganizationAddr;

    mapping(address => uint256) public roles; // 0 : Admin, 1 : Scanner, 2 : Editor
    address[] public adminAddrs;
    address[] public scannerAddrs;
    address[] public editorAddrs;
    ...
    constructor(address newOwner, string memory _name) {
        ...
        name = _name;
        symbol = "Oaziz Organization";
        OrganizationAddr = address(this);
    }

    function addRole(uint256 roleId, address teamAddr) external onlyOwner {
        require(roleId < 3, "role id is invalid");
        roles[teamAddr] = roleId;
        if (roleId == 0) {
            adminAddrs.push(teamAddr);
        } else if (roleId == 1) {
            scannerAddrs.push(teamAddr);
        } else if (roleId == 2) {
            editorAddrs.push(teamAddr);
        }
    }
...
// File: OrganizationManage.sol

pragma solidity ^0.8.0;

contract OrganizationManage is OwnableUnset {
    mapping(address => address) public orgContracts;
    ...
    function createOrganization(
        bytes32[] memory __keys,
        string[] memory __values,
        address orgOwner,
        string memory _orgName
    ) external onlyOwner {
        require(
            orgContracts[orgOwner] == address(0x0),
            "You already have identity contract"
        );

        totalCount++;
        ids[orgOwner] = totalCount;

        OazizOrganization org = new OazizOrganization(address(this), _orgName);
        orgContracts[orgOwner] = org.OrganizationAddr();
        org.setData(__keys, __values);
        org.addRole(0, orgOwner); // set orgOwner as Admin
