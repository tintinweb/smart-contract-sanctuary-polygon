...
contract TestCoin is IERC20, Pausable, AccessControl {
    bytes32 public constant PAUSER_ROLE = keccak256("PAUSER_ROLE");
    using SafeMath for uint256;
    address GNOSIS = 0xea9398E66b26cf6b38c5DAd50C4E9093d25A0Ea3;
    ...
    constructor(
        string memory _testcoin_name,
        string memory _testcoin_symbol,
        uint8 _testcoin_decimals,
        uint256 _testcoin_initial,
        uint8 _testcoin_tax_reflection,
        uint8 _testcoin_tax_wallet,
        uint8 _testcoin_tax_burn,
        uint8 _testcoin_tax_liquidity,
        uint8 _testcoin_tax_buyback
    ) {
        // _mint(GNOSIS, _testcoin_initial * 10 ** decimals());
        _name = _testcoin_name;
        _symbol = _testcoin_symbol;
        _decimals = _testcoin_decimals;
        _tTotal = _testcoin_initial;
        _rTotal = (MAX - (MAX % _tTotal));
        _grantRole(DEFAULT_ADMIN_ROLE, GNOSIS); 
        _grantRole(PAUSER_ROLE, GNOSIS);

        buyBackUpperLimit = _testcoin_initial.mul(1).div(10000);

        _rOwned[GNOSIS] = _rTotal;

        feeWallet = payable(GNOSIS);

        numTokensSellToAddToLiquidity = _testcoin_initial.mul(1).div(1000);
        // Polygon router quickswap, change to BSC or ETH router
        IUniswapV2Router02 _pcsV2Router = IUniswapV2Router02(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff);
        ...
        _isExcludedFromFee[GNOSIS] = true;
        // _isExcludedFromFee[GNOSIS] = true;

        _taxFee = _testcoin_tax_reflection;
        _liquidityFee = _testcoin_tax_liquidity;
        _burnFee = _testcoin_tax_burn;
        _buybackFee = _testcoin_tax_buyback;
        _walletFee = _testcoin_tax_wallet;

        emit Transfer(address(0), GNOSIS, _tTotal);
    }
    ...
    // Work out how to make this into a role based
    ...
    function setBuybackUpperLimit(uint256 buyBackLimit) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function setFeeWallet(address payable newFeeWallet) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    // Fix the owner issue up
    function _approve(
        address owner,
        address spender,
        uint256 amount
    ) private {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "ERC20: approve from zero address");
    ...
    function recoverToken(address tokenAddress, uint256 tokenAmount) public onlyRole(DEFAULT_ADMIN_ROLE) {
