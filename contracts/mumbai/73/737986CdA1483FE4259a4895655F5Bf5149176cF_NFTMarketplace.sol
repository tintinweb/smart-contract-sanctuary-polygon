...
contract NFTMarketplace is IERC721Receiver {
    ...
    constructor() {
        // whoever deploys the contract is the owner
    ...
    function createAuction(
        ...
        // can't start auction if not owner of nft. ownerOf also checks if id is valid
        if (msg.sender != _nft.ownerOf(_nftId)) {
            revert NOT__NFTOWNER();
        }
        ...
        // if owner and nft id is valid, create market item
    ...
    function bid(uint256 _nftId) external payable {
        ...
        if (idMarketItem[_nftId].highestBidder.bidder == address(0)) {
            if (msg.value >= idMarketItem[_nftId].initialBid) {
            ...
            if (msg.value > idMarketItem[_nftId].highestBidder.bidValue) {
                // do before setting new highestBidder
                bidders[_nftId][
