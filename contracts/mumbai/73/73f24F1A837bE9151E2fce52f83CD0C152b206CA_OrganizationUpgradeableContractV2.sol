...
contract OrganizationUpgradeableContractV2 is Initializable, AccessControlUpgradeable, ERC1155Upgradeable {
    ...
    uint256 public constant JAVA_PROFESSIONAL_CERTIFICATE = 1;

    mapping(address => MintGuard) javaTokens;
    ...
    struct MintGuard {
        bool claimed;
        mapping(bytes32 => GrantRole) roles;
    }
    ...
    function uri(uint256 _tokenId) override public pure returns (string memory) {
        return string(
            abi.encodePacked(
                "https:/2dsadas/ipfs.io/ipfs/bafybeihjjkwdrxxjnuwevlqtqmh3iegcadc32sio4wmo7bv2gbf34qs34adasdas/",
                StringsUpgradeable.toString(_tokenId), ".json"
            )
        );
    }

    function claimReady(address to) public view returns (bool){
        MintGuard storage mintGuard = javaTokens[to];
        return mintGuard.roles[TEAM_LEADER_ROLE].granted
        && mintGuard.roles[MANAGER_ROLE].granted;
    }

    function checkRole() internal view virtual returns (bytes32) {
    ...
    function mint(address to, uint256 id, uint256 amount, bytes memory data) public  {
        if (msg.sender == to && claimReady(to)) {
            super._mint(to, id, amount, data);
        } else {
            javaTokens[to].roles[checkRole()] = GrantRole(true, msg.sender);
        }
    }
