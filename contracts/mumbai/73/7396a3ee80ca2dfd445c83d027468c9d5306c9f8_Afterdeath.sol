...
contract Afterdeath is IAfterdeath, IDescriptorUser, Initializable {
  ...
  address private _admin; // _UNUSED << ZERO!
  uint private _BurialDelayInSeconds; 
  address private _treasury;
  address private _game;
  // address private _sge
  uint private _fusionProposalExpirationPeriodInSeconds; // _UNUSED2 << ZERO!
  ...
  function initialize2(
    address admin,
    uint fusionProposalExpirationPeriodInSeconds
  ) public {
    require(_inited2 == false, 'Already inited');
    _admin = admin;
    _fusionProposalExpirationPeriodInSeconds = fusionProposalExpirationPeriodInSeconds;
    _inited2 = true;
  }
  ...
  function resurrect(uint256 tokenId) external override {
    ...
    d.deathTime = 0; // prevents multiple resurrections
    ...
    d.skinId = d.onResurrectionSkinId;
    ...
    ISnookGame(_game).increaseSpecialSkinIdCounter(d.onResurrectionSkinId);
  ...
  {
    Descriptor memory descriptor = _state.getDescriptor(tokenId);
    return _getResurrectionPrice(descriptor);
  }

  function _getResurrectionPrice(Descriptor memory descriptor) private view returns (uint)
  {
    uint256 k = _uniswap.getSnookPriceInSkills(); // in wei
    int128 d = _getResurrectionDifficulty(descriptor); 
  ...
  function _getResurrectionDifficulty(Descriptor memory descriptor) private view returns (int128) {
    uint bin = descriptor.traitCount;
  ...
  function getFusionProposalExpirationPeriodInSeconds() override external view returns(uint) {
    return _fusionProposalExpirationPeriodInSeconds;
  }
  function setFusionProposalExpirationPeriodInSeconds(uint secs) onlyAdmin() override external {
    _fusionProposalExpirationPeriodInSeconds = secs;
  } 

  function getFusionProposal(uint[2] calldata snookIds) external override view 
    returns (uint, uint[2] memory) 
  {
    (uint price, Descriptor[3] memory descriptors) = _getFusionProposal(snookIds);
    uint[2] memory snookTraitCounts;
    for (uint i=0; i<snookIds.length; i++) {
      snookTraitCounts[i] = descriptors[i].traitCount;
    }
    return (price, snookTraitCounts);
  }

  function _getFusionProposal(uint[2] calldata snookIds) internal view returns(uint, Descriptor[3] memory) {
    uint8 ASCII0 = 48;
    Descriptor[3] memory descriptors;
    uint traitCount = 0;
    uint i = 0;
    for (; i<snookIds.length; i++) {
      //  each of the snooks should 
      // 1. exist 2. belong to the caller 3. not be locked (either game or sale) 
      uint snookId = snookIds[i];
      require(_snook.exists(snookId), string(abi.encodePacked("snook ", snookId+ASCII0, " does not exist")));
      require(_snook.isLocked(snookId) == false, string(abi.encodePacked("snook ", snookId+ASCII0, " is locked")));
      descriptors[i] = _state.getDescriptor(snookId);
      require(descriptors[i].deathTime == 0, string(abi.encodePacked("snook ", snookId+ASCII0, " is dead")));
      traitCount += descriptors[i].traitCount;
    }
    ...
    uint price = _getResurrectionPrice(descriptors[i]);
    return (price, descriptors);
  }
  ...
  function _isSignatureValid(
    ...
    // signer has Killer role, so sent message is not temperated
    bool rv = AccessControlEnumerableUpgradeable(_game).hasRole(ISnookGame(_game).getKillerRole(), signer);
    return rv;
  }
  ...
  function fuseSnooks(
    ...
    (, Descriptor[3] memory descriptors) = _getFusionProposal(snookIds);
    require(
      block.timestamp <= proposalTimestamp + _fusionProposalExpirationPeriodInSeconds * 1 seconds, 
      'Proposal expired'
    );
    for (uint j=0; j<snookIds.length; j++) {
      require(
        snookTraitCounts[j]== descriptors[j].traitCount, 
        string(abi.encodePacked('Snook trait count changed at i=',j))
      );
    }
    require(_skill.transferFrom(msg.sender, address(this), price), "Not enough funds");
    _skill.approve(_treasury, price);
    ITreasury(_treasury).acceptResurrectionFunds(price);
  
    uint i=0;
    for (; i<snookIds.length; i++) {
      _snook.burn(snookIds[i]);
      _updateOnDeath(descriptors[i].traitCount);
      ISnookGame(_game).decreaseSpecialSkinIdCounter(descriptors[i].skinId);
      _state.deleteDescriptor(snookIds[i]);
    }
    uint tokenId = _snook.mint(msg.sender, tokenURI);
    _state.setDescriptor(tokenId, descriptors[i]);
    _updateOnMint(descriptors[i].traitCount, 1);
    ISnookGame(_game).increaseSpecialSkinIdCounter(skinId);
