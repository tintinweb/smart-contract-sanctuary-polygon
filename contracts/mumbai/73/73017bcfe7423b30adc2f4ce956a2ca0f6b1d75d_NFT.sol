...
interface INFTPayoutsUpgradeable is IERC165Upgradeable {
     ...
     * @dev TODO
     */
    function payoutCount(uint256 _tokenId, uint256 _tier)
        external
        view
        returns (uint256);
...
interface IERC721FullAmberfiContentFactory is INFTPayoutsUpgradeable {
    struct PayoutTier {
...
{
    mapping(uint256 => mapping(uint256 => PayoutInfo[]))
        private _tokenPayoutInfo;
    mapping(uint256 => mapping(uint256 => uint256)) private _tokenPayoutCount;
    ...
    function payoutCount(uint256 _tokenId, uint256 _tier)
        ...
        returns (uint256)
    {
        return _tokenPayoutCount[_tier][_tokenId];
    }
    ...
    function payoutInfo(
        ...
        returns (address[] memory receivers, uint256[] memory payoutAmounts)
    {
        uint256 count = payoutCount(_tokenId, _tier);
        require(count == 0, "NFTPayouts: no payout");

        PayoutInfo[] memory payouts = _tokenPayoutInfo[_tier][_tokenId];
    ...
    function _setTokenPayout(
        ...
        _resetTokenPayout(tokenId, tier);

        uint256 payoutTotal;

        _tokenPayoutCount[tier][tokenId] = payoutCount_;
        ...
        for (uint256 i; i < payoutCount_; ) {
            _tokenPayoutInfo[tier][tokenId].push(payoutInfo_[i]);
     ...
     */
    function _resetTokenPayout(uint256 tokenId, uint256 tier) internal virtual {
        delete _tokenPayoutCount[tier][tokenId];
        delete _tokenPayoutInfo[tier][tokenId];
    }
