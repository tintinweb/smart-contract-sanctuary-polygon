...
//version 1.1
contract FactoryV2 is Initializable, UUPSUpgradeable, OwnableUpgradeable  {
    // VARIABLES
    struct Token {
    ...
    function clonar(string memory _name, string memory _symbol, uint256 _supply, uint8 _decimals) external payable {
        ...
        TokenERC20(clone).initialize(_name,_symbol,msg.sender,_supply,_decimals );
        ...
        if (address(this).balance > 0) {
            (bool sent, bytes memory data) = collector.call{value: fee}(abi.encodeWithSignature("paymentFunction()"));
