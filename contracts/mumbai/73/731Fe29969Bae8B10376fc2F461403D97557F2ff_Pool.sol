...
function claimableAmount(address _borrower, uint256 _loanId)
        ...
        returns (uint256)
    {
        ...
        // uint discountInPaza = paza.getContinuousMintReward(temp.discount);
        uint rewardInUSDC = temp.discount - temp.borrowerShare;
        uint256 reward = paza.getContinuousMintReward(rewardInUSDC);
...
function transferStakedAmount(
        ...
        paza.transfer(_bank, stakedAmount[_user][_loanId]);
...
function unstake(address _borrower, uint256 _loanId) public bankPoolStatus {
        ...
        paza.transfer(_borrower, amount + amountStaked);
