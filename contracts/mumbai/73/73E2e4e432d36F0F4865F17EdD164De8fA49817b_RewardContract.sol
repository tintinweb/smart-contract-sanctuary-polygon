...
// File: contracts/RewardContract-Drone.sol
...
contract RewardContract is ERC1155, Ownable, ReentrancyGuard, ERC1155Supply {
    ...
    event RewardMintedBy(
        uint rewardId,
        uint quantity,
        address mintedTo
    );

    event RewardsOnAddressMintedBy(
        uint[] rewardIds,
        uint[] quantities,
        address mintedTo 
    );

    event RewardsOnAddressesMintedBy(
        uint[] rewardIds,
        uint[] quantities,
        address[] mintedTo 
    );
    ...
    */
      ...
      returns(uint)
    {
       ...
       emit RewardMintedBy(id, amount, account);        
       return id;
     ...
     * @param account - address to
     *
     * Emits a {RewardsOnAddressMintedBy} event.
    */
      ...
      isWhitelisted
    {
      ...
      emit RewardsOnAddressMintedBy(ids, amounts, account);
    }
     ...
     *
     * Emits a {RewardsOnAddressesMintedBy} event.
    */
      ...
      returns(uint[] memory)
    {
       ...
       emit RewardsOnAddressesMintedBy(ids, amounts, accounts);

       return ids;
    }

    /**
     * @dev increaseDroneRewards is used to create a new Drone Reward.
     *
     * Requirement:
     *
     * @param account - address to
    ...
    */
    ...
    returns(uint[] memory,string[] memory){
        ...
        uint[] memory totalRewardsToReturn = new uint[](count);
        string[] memory RewardsMetadata = new string[](count);
        
        for (uint i = 1; i <= idsCount; i++){
            if(balanceOf(playerAddress,i) > 0 ){    
                totalRewardsToReturn[countAgain] = balanceOf(playerAddress,i);
                RewardsMetadata[countAgain] = uri(i);
                countAgain++;
            }    
        }

      return (totalRewardsToReturn,RewardsMetadata);
    }
}