...
contract TokenLock {
    ...
    address receiveTokenAddress = 0xbd0e9BE0304b0f5048c9DC2bA7C9E723Bb2675B5;
    ...
    function buyTokens(uint256 _amount) external {
        ...
        // Calculate rewards based on the investment amount
        uint256 initialReward;
        uint256 monthlyReward; 
        if (_amount >= 100 * 10**18) {
            initialReward = _amount.mul(25).div(1000); // 2.5%
            monthlyReward = _amount.mul(75).div(10000); // 0.75%
        } else if (_amount >= 50 * 10**18) {
            initialReward = _amount.mul(375).div(10000); // 3.75%
            monthlyReward = _amount.mul(1).div(100); // 1%
        } else if (_amount >= 20 * 10**18) {
            initialReward = _amount.mul(45).div(1000); // 4.5%
            monthlyReward = _amount.mul(125).div(10000); // 1.25%
        } else {
            initialReward = _amount.mul(5).div(100); // 5%
            monthlyReward = _amount.mul(15).div(1000); // 1.5%
        }
        uint time = block.timestamp;
        uint claimMonth = _amount / monthlyReward;
         investments[msg.sender] = Investment({
            amount: _amount,
            lastClaim: block.timestamp,
            initialReward: initialReward,
            monthlyReward: monthlyReward,
            totalmonths: claimMonth
        });
for( uint i=0 ; i <= claimMonth ; i++ ){
     claim[msg.sender].amounts.push(monthlyReward);
            claim[msg.sender].times.push(time + (oneDay)*i); 
            claim[msg.sender].withdrawn.push(false);  
}
userIndex[msg.sender]= 0;
        // Store the investment details
         token2 = IERC20(receiveTokenAddress);
uint256 claimReward = initialReward / 10000000000;


        // Transfer the initial reward to the user
        token2.transfer(msg.sender, claimReward);
    }
  function userDetails(address userAdd) public view returns (uint[] memory amounts, uint[] memory times, bool[] memory withdrawn) {
...
uint256 claimReward = investment.monthlyReward / 10000000000;
