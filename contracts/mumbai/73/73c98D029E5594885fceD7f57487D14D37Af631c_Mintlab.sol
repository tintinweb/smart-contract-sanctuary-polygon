  ...
  BasicMetaTransaction
{
  ...
  bytes32 public constant UNAPPROVE_MARKET_ROLE =
    keccak256('UNAPPROVE_MARKET_ROLE');
  ...
  function initialize(
    ...
    _grantRole(UNAPPROVE_MARKET_ROLE, _proxyManager);
    _setRoleAdmin(UNAPPROVE_MARKET_ROLE, ADMIN_ROLE);
    ...
    roleNames[UNAPPROVE_MARKET_ROLE] = 'UNAPPROVE_MARKET_ROLE';
  ...
  function setMarket(address _market) public onlyRole(ADMIN_ROLE) {
    require(
      _market != address(0),
      'Marketplace address cannot be zero address'
    );
    marketplace = _market;
  }

  function unApproveMarket(uint256 _tokenId)
    public
    onlyRole(UNAPPROVE_MARKET_ROLE)
  {
    address owner = super.ownerOf(_tokenId);

    if (getApproved(_tokenId) == marketplace) {
      _approve(owner, _tokenId);
    }
    if (isApprovedForAll(owner, marketplace)) {
      _setApprovalForAll(owner, marketplace, false);
    }
  }

  function initMarket(address _market) public onlyRole(ADMIN_ROLE) {
    marketplace = _market;
    roleNames[UNAPPROVE_MARKET_ROLE] = 'UNAPPROVE_MARKET_ROLE';
    if (!hasRole(UNAPPROVE_MARKET_ROLE, proxyManager)) {
      _grantRole(UNAPPROVE_MARKET_ROLE, proxyManager);
      _setRoleAdmin(UNAPPROVE_MARKET_ROLE, ADMIN_ROLE);
      _pushRole(
        proxyUsersRole,
        name(),
