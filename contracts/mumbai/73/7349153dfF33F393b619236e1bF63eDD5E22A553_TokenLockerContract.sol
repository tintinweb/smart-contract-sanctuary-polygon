...
contract TokenLockerContract is ERC721Holder, Context, ReentrancyGuard{
    ...
    IMainContract public MainContract; // Main Contract address for the IMainContract interface

    address public DutchAuction; // Dutch Auction contract address
    ...
    event Deposit( bytes32 indexed lockId, address indexed user, address indexed protocol, address collection, uint[] tokens, uint lockPeriod);
    ...
    modifier isProtocolMod(){
        if(!isProtocol[_msgSender()]){
            revert Unauthorized(_msgSender());
        }
        _;
    }
    ...
    function _withdraw(address _collection, uint[] storage _tokens, address _recipient) private {
    ...
    function deposit(address _collection, uint[] calldata _tokens, uint _unlock, address _ownerOf) external nonReentrant isSupportedCollection(_collection) returns(bytes32 lockId) {
        require(_tokens.length > 0);
        lockId = keccak256(abi.encode(_ownerOf, _collection, _tokens, _unlock, block.number));
        ...
        emit Deposit(lockId, _ownerOf, __protocol, _collection, _tokens, __lockTime);
    }
    ...
    function liquidate(bytes32 _lockId, address _recipient) external nonReentrant isProtocolMod{
        LockedTokens storage lockTokens = lockedTokens[_lockId];
        require(lockTokens.lockTime == 0);
    ...
    function updateMainContract(address _MainContract) external isAdmin{
        MainContract = IMainContract(_MainContract);
    }
    ...
    function getLockedIds(address locker) external view returns (bytes32[] memory _lockIds){
    ...
    function getLockedTokensData(bytes32 _id) external view returns(LockedTokens memory lockTokens){
