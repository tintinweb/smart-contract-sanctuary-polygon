...
interface IKittieNode is IERC721 {
  ...
  function tokenLastTransferredAt(uint256 tokenId) external view returns (uint256);
  ...
  function mint(uint256[] memory tierId, uint256[] memory amount) external payable;
}
...
interface IPriceOracle {
  function getMainPriceViaNativePair(address token) external view returns (uint256);
}
 ...
 * KittieNode
 */
contract KittieNode is IKittieNode, Ownable, ERC721Burnable, ERC721Enumerable, ERC721Pausable {
  ...
  // Payment address
  address public paymentAddress = 0x27DDd8645b1930F01d79bD6e48ed424F5D1E58Dc;

  // Royalties address
  address public royaltyAddress = 0x27DDd8645b1930F01d79bD6e48ed424F5D1E58Dc;

  // Royalties basis points (percentage using 2 decimals - 10000 = 100, 5000 = 50, 0 = 0)
  uint256 private royaltyBasisPoints = 300; // 3%

  // Token info
  string public constant TOKEN_NAME = "KittieNode";
  string public constant TOKEN_SYMBOL = "KittieNode";
  ...
  modifier whenPublicSaleNotActive() {
    require(!publicSaleActive && publicSaleStartTime == 0, "Public sale is already active");
    _;
  }

  // Owner or public sale active modifier
  modifier whenOwnerOrPublicSaleActive() {
    require(owner() == _msgSender() || publicSaleActive, "Public sale is not active");
    _;
  }
  ...
  constructor(
    ...
    Tier[3] memory initTiers = [
      Tier({id: 1, name: "Loshad''", tokenPrice: 1_000_000, stablePrice: 10, dailyReturnPercent: 100}),
      Tier({id: 2, name: "Gopnik Kittie", tokenPrice: 900_000, stablePrice: 9, dailyReturnPercent: 80}),
      Tier({id: 3, name: "Authentic Gopnik", tokenPrice: 700_000, stablePrice: 7, dailyReturnPercent: 50})
    ];
  ...
  function royaltyInfo(uint256, uint256 _salePrice) external view returns (address receiver, uint256 royaltyAmount) {
    return (royaltyAddress, (_salePrice.mul(royaltyBasisPoints)).div(PERCENT_DENOMENATOR));
  }
  ...
  function getElapsedSaleTime() public view returns (uint256) {
    return publicSaleStartTime > 0 ? block.timestamp.sub(publicSaleStartTime) : 0;
  }
  ...
  {
    require(_tierIds.length == _amounts.length, "Tier and amounts per tier should be the same");
    ...
    for (uint256 _i = 0; _i < _tierIds.length; _i++) {
      ...
      for (uint256 i = 0; i < _amount; i++) {
        ...
        (uint256 mainPriceUSD18, uint256 pricePaid) = _getTotalPaidUSD18(tier.id);
        pricePaidUSD18[_tokenIds.current()] = pricePaid;

        tokenPerDayReturn[_tokenIds.current()] = getPerDayReturnFromTier(tier.id, mainPriceUSD18);
  ...
  function payToMint(uint256 _tierId, uint256 amount) internal whenOwnerOrPublicSaleActive {
    ...
    uint256 totalMainTokenPrice = tier.tokenPrice.mul(10**main.decimals()).mul(amount);
    uint256 totalStablePrice = tier.stablePrice.mul(10**stable.decimals()).mul(amount);

    require(main.balanceOf(msg.sender) >= totalMainTokenPrice, "not enough main token balance to mint");
    require(stable.balanceOf(msg.sender) >= totalStablePrice, "not enough stable token balance to mint");
  ...
  function setDexUtils(address _newUtils) external onlyOwner {
    priceOracle = IPriceOracle(_newUtils);
  }
  ...
  function updateNodePerDayReturn(uint256 tokenId, uint256 amount) external onlyOwner {
    tokenPerDayReturn[tokenId] = amount;
  }

  function getPerDayReturnFromTier(uint256 tierId, uint256 mainPriceUSD18) public view returns (uint256) {
    Tier memory tier = allTiers[tierId];
    IERC20Decimals main = IERC20Decimals(mainToken);
    (, uint256 totalUSDPaid18) = _getTotalPaidUSD18(tierId);
    uint256 perDayUSDRewards18 = totalUSDPaid18.mul(tier.dailyReturnPercent).div(PERCENT_DENOMENATOR);
    return perDayUSDRewards18.mul(10**main.decimals()).div(mainPriceUSD18);
  }

  function _getTotalPaidUSD18(uint256 tierId) internal view returns (uint256, uint256) {
    Tier memory tier = allTiers[tierId];
    IERC20Decimals main = IERC20Decimals(mainToken);
    IERC20Decimals stable = IERC20Decimals(stableToken);
    ...
    uint256 mainPriceUSD18 = priceOracle.getMainPriceViaNativePair(mainToken);
    uint256 mainUSDPaid18 = mainPriceUSD18.mul(mainIncDecimals.mul(10**18).div(10**main.decimals())).div(10**18);
    return (mainPriceUSD18, mainUSDPaid18.add(stableIncDecimals.mul(10**18).div(10**stable.decimals())));
  }
