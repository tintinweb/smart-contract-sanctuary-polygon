...
contract swapHelperDiyTask is OpsTaskCreator, Ownable {
    ...
    event CounterTaskCreated(uint256 _taskAmount, bytes32 _taskId, taskConfig _taskConfig);
    ...
    function createTask(IAocoRouter02 _routerAddress, address[] memory _swapRouter, address[] memory _swapRouter2, uint256 _swapAmount, uint256 _INTERVAL, uint256 _salt) external payable {
        ...
        md5TaskList[md5] = id;
        taskConfig memory k = taskConfig(_routerAddress, _swapRouter, _swapRouter2, _swapAmount, _INTERVAL, _salt, 0, md5);
        taskConfigList[id] = k;
        emit CounterTaskCreated(taskAmount, id, k);
