...
//            uint amount = getRewards(tokenId,msg.sender);
            uint amount = getRewards(tokenIds,msg.sender);//20

            uint nftAmount = (count* tokentierToPrice[sustain.tokenType])* 1 ether;//>>2
            require(nftAmount>=amount,"!enough rewards");

            uint price = amount - tokentierToPrice[sustain.tokenType] * nftAmount;
...
//            mintNFT(sustain,_tokenTier);
            for (uint i = 0; i< nftAmount; i++) {
                mintNFT(sustain.tokenType, true);
            }
        }
