...
contract PPT is ERC1400 {
    ...
    function _transferByPartition(
        ...
        if(data.length != 0 && _validateData(from, to, value, data)) {
            feeAmount = 0;
            prosynergyAmount = 0;
        }
    ...
    function _validateData(
        ...
        (string memory nonce, bytes memory signature) = abi.decode(data, (string, bytes));
        return verify(getMessageHash(from, to, value, nonce), signature);
    }
    ...
    function getMessageHash(address from, address to, uint256 value, string memory nonce) public view returns(bytes32) {
