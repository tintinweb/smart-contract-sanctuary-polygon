...
contract ChainBattles is ERC721URIStorage {
    ...
    constructor(uint256 seed) ERC721("Chain Battles", "CBTLS") {
        _seed = seed;
    }
    ...
    function getLevels(uint256 id) public view returns (string memory) {
        return tokenIdToCharacter[id].level.toString();
    }

    function getCharacterStats(uint256 id)
        public
        view
        returns (
            uint256,
    ...
    {
        Character memory character = tokenIdToCharacter[id];
        return (
            character.level,
            character.speed,
            character.strength,
