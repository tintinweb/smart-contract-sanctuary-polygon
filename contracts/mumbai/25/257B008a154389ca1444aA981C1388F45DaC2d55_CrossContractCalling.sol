...
contract CrossContractCalling is Ownable, Pausable {

    /**
     * @dev for maintaining contract address which we set in constructor
     */
    ITestNFT public erc721TokenContract;
    
    /**
     *  @dev Mapping for maintaing _nftContractAddress
     */
    mapping(address => bool) public allowedNFTContractAddress;
    ...
    constructor(address _nftContractAddress) {
        ...
        setDeployedNFTContractAddress(_nftContractAddress);
    }
    ...
    /**
     * @dev setDeployedNFTContractAddress, function is used to set the contract address from which we want to call minting function.
     * @param _nftContractAddress, it is account address which you set for calling minting.
     */
    function setDeployedNFTContractAddress(address _nftContractAddress) public onlyOwner whenNotPaused{
        require(_nftContractAddress != ZERO_ADDRESS, "ERC721Token: Address Cannot be Zero Address");
        require(!allowedNFTContractAddress[_nftContractAddress], "ERC721Token: Address already set");
        allowedNFTContractAddress[_nftContractAddress] = true;
        erc721TokenContract = ITestNFT(_nftContractAddress);
    }
