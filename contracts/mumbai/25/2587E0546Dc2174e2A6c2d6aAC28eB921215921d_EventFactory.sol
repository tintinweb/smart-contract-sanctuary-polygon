...
contract EventFactory is CloneFactory, Ownable {
    ...
    address[] public EventAccountants;
    SponsoredEvent[] public sponsoredEvents;
    ...
    event NewEvent(address eventAddress, address accountHandler);

    event NewEventSponsored(SponsoredEvent);
    ...
    function sponsorEvent(address _eventAddress) public payable {
        require(msg.value > 0, "Must send ether");
        if (sponsoredEvents.length < 3) {
            sponsoredEvents.push(
                SponsoredEvent(_eventAddress, _msgSender(), msg.value)
            );
            emit NewEventSponsored(sponsoredEvents[sponsoredEvents.length - 1]);
        } else {
            require(
                _eligibleToSponsor(msg.value),
                "Not enough value to sponsor"
            );
            for (uint256 index = 0; index < sponsoredEvents.length; index++) {
                if (msg.value >= sponsoredEvents[index].deposit + 0.01 ether) {
                    (bool sent, bytes memory data) = (
                        sponsoredEvents[index].sponsor
                    ).call{value: (sponsoredEvents[index].deposit * 95) / 100}(
                        ""
                    );
                    require(sent, "Failed to send Ether");
                    sponsoredEvents[index].deposit = msg.value;
                    sponsoredEvents[index].eventAddress = _eventAddress;
                    sponsoredEvents[index].sponsor = _msgSender();
                    emit NewEventSponsored(sponsoredEvents[index]);
                }
            }
        }
    }

    function _eligibleToSponsor(uint256 _value) internal returns (bool) {
        for (uint256 index = 0; index < sponsoredEvents.length; index++) {
            if (msg.value >= sponsoredEvents[index].deposit + 0.01 ether) {
                return true;
            }
        ...
        isDev
    {
        ...
        EventAccountants.push(address(newSplitter));
    }
...
contract TicketEx is Context, ERC165, IERC1155, ERC2981, Initializable {
    ...
    function setPrice(uint256[] memory _ticketIds, uint256[] memory _prices)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < _ticketIds.length; i++) {
