...
contract YSLBUSDVault is Initializable,ReentrancyGuard, Pausable,IEvents {
    ...
    */
    ...
    function deposit(address user, uint amount,uint32 level,bool isBUSD) nonReentrant external _lastExchangeRatio() whenNotPaused() {
        ...
        if((!Admin.lpDeposit()) || isBUSD){
            IERC20(Admin.BUSD()).transferFrom(user,address(this),amount);
            IERC20(Admin.BUSD()).approve(Admin.liquidityProvider(),amount);
            Price = ILiquidityProvider(Admin.liquidityProvider()).depositBUSDToLP(user,amount,address(yslBUSD));
        }
        else{
        IERC20(address(yslBUSD)).transferFrom(msg.sender, Admin.Treasury(), amount); //transferring LPtokens at this contract address
        Price = lpDollarValue(address(yslBUSD),Admin.YSL(),Admin.BUSD());
        }
        ...
        path[0] = Admin.BUSD(); 
        path[1] = Admin.USDy();
        uint USDyAmount=IUniswapV2Router02(router).getAmountsOut(10 ** 18,path)[1];
        IReceipt(yslBUSD_A).mint(address(this),(((Price * USDyAmount) / 10 ** 18) * (100 - depositTax) * (10 ** 18))/ (exchangeRatio * 100));
        UserDeposit[user] += (((Price * USDyAmount) / 10 ** 18) * (100 - depositTax) * (10 ** 18))/ (exchangeRatio * 100);
        USDyOfDollarValue += (Price * USDyAmount) / 10 ** 18;
        IReceipt(Admin.USDy()).mint(address(this),(Price * USDyAmount) / 10 ** 18);
    ...
    */
    ...
    function withdraw(address user, bool isReciept,uint _amount,address sendTo) nonReentrant external _lastExchangeRatio() whenNotPaused(){
        ...
        if(isReciept){
            IERC20(yslBUSD_A).safeTransfer(sendTo,_amount);
            USDyOfDollarValue -= _amount;
        }
        else{
            uint balance = (_amount * exchangeRatio)/10 ** 18;
            IERC20(Admin.USDy()).safeTransfer(sendTo,balance);
    ...
    */
    ...
    function purchase(address user, uint amount, uint minAmount) external nonReentrant _lastExchangeRatio() whenNotPaused() returns(uint){
        if(!IWhitelist(Admin.whitelist()).getAddresses(msg.sender)) {
            require(restrictTransfer[msg.sender] != block.number, "YSLBUSDVault: you can't interact in same block");
        }
        ...
        IReceipt(yslBUSD_A).mint(user, balance); //minting receipt tokens in users account
        exchangeRatio = (IERC20(address(yslBUSD)).balanceOf(address(this)) * (10 ** 18))/(IERC20(yslBUSD_A).totalSupply()); //calculating new exchange ratio
        restrictTransfer[msg.sender]= block.number; 
        return(balance);
        emit purchaseORsell("YSLbusdVault",user,amount,block.number,block.timestamp);
    ...
    */
    ...
    function sell(address user, uint amount, uint minAmount) external nonReentrant _lastExchangeRatio() whenNotPaused() returns(uint){
        ...
        IReceipt(yslBUSD_A).burn(user,amount);
...
interface ILiquidityProvider {
    ...
    function addLiquidityEth(address token, uint amount) external payable;
