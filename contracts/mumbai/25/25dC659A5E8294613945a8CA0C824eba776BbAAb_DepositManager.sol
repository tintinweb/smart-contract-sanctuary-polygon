...
contract DepositManager is OwnableUpgradeable, UUPSUpgradeable {
    ...
    function ManagerAddDeposit(
        address user,
        address _tokenAddress,
        uint256 _amount
    ) external ManagerorOwner {
        _addDeposit(user, _tokenAddress, _amount);
    }

    function addDeposit(address _tokenAddress, uint256 _amount) external {
        _addDeposit(msg.sender, _tokenAddress, _amount);
    }
    ...
    function _addDeposit(
        ...
        require(stableTokenRate[_tokenAddress] != 0, "suggest token to Admin");
        require(IERC20(_tokenAddress).balanceOf(wallet) >= _amount, "insufficent Balance");
        uint256 storagePurchased = _amount
            .mul(stableTokenRate[_tokenAddress])
            .mul(costOfStorage())
            .div(10**decimals)
            .div(10**6);
        deposits[wallet].push(Deposit(block.timestamp, _amount, storagePurchased));
        _updateAvailableStorage(wallet, storagePurchased);
        IERC20(_tokenAddress).transferFrom(wallet, address(this), _amount);
        emit AddDepositEvent(wallet, _tokenAddress, _amount, _costOfStorage, storagePurchased);
    }

    /** 
    * @dev the function set the storage Size new User will start with

    * @param _initialDataCapForNewUsers value
    */
    function setDataCapForNewUsers(uint256 _initialDataCapForNewUsers) public onlyOwner {
        initialDataCapForNewUsers = _initialDataCapForNewUsers;
    }

    /** 
    * @dev the function initializated the priceFeed Aggregator
    * see {chainlink for more}

    * @param aggregatorAddressFeed designated chainlink priceFeed address
    */
    function setPriceFeed(address aggregatorAddressFeed) public onlyOwner {
    ...
    */
    function setCostOfStorage(uint256 newCost) public ManagerorOwner {
    ...
    */
    function getStorageCostForStableCoins(address tokenAddress, uint256 size) public view returns (uint256) {
        require(stableTokenRate[tokenAddress] != 0, "Token Rate Not set");
        return size.mul(1 ether).mul(10**6).div(_costOfStorage.mul(stableTokenRate[tokenAddress]));
    }
    ...
    */
    ...
    /**
     * @dev
     * give approval to designated Account to spend an amount
     */
    function giveApprovalToSpendToken(
        address _tokenAddress,
        address wallet,
        uint256 amount
    ) external onlyOwner {
        require(amount <= IERC20(_tokenAddress).balanceOf(address(this)));
        IERC20(_tokenAddress).approve(wallet, amount);
        emit withdrawApproval(wallet, _tokenAddress, amount, false);
    }
    ...
    function transferAmount(
        ...
        require(amount <= IERC20(_tokenAddress).balanceOf(address(this)));
        IERC20(_tokenAddress).transfer(wallet, amount);
        emit withdrawApproval(wallet, _tokenAddress, amount, true);
    }
     ...
     * 
     * @param _tokenAddress token address on the Network
     ...
     */
    function addToken(address _tokenAddress, uint256 rate) external onlyOwner {
        require(_tokenAddress != address(0), "Address can't be zero");
        require(rate != 0, "rate can't be zero");
        stableTokenRate[_tokenAddress] = rate;
    }
     ...
     * @param _tokenAddress token on the Network
     *
     *
     * Requirement:
     * - only callable by owner
     * - revert if the tokenAddress rate is already set to zero
     *
     */
    function removeToken(address _tokenAddress) external onlyOwner {
        require(_tokenAddress != address(0), "Address can't be zero");
        require(stableTokenRate[_tokenAddress] != 0, "token already disabled");
        stableTokenRate[_tokenAddress] = 0;
    }
    ...
    function updateStorage(
        ...
        if (!storageList[user].isKnownUser) {
            _updateAvailableStorage(user, initialDataCapForNewUsers);
     ...
     */
    function setWhiteListAddress(address _address, bool _status) external onlyOwner {
    ...
    function claimEth(address payable _to, uint256 _amount) external onlyOwner {
        ...
        emit withdrawApproval(_to, address(0), _amount, true);
    }
