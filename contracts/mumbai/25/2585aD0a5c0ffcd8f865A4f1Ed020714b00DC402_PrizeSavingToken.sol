    ...
    BatchMintMetadata
{
    ...
    uint256 public numberOfWinningTickets;
    uint256 public originalBalance;

    mapping(address => uint256) public numberOfTicketsPerAddress;
    ...
    function mintTicket(
        ...
        numberOfTicketsPerAddress[_to] += _amount;
    }
        ...
        onlyAdmin
    {
        ...
        address jackpotPrizeWinner = _receivers[_random(_receivers.length)];
        require(
            balanceOf[address(jackpotPrizeWinner)][_tokenId] > 0,
            "No tickets issued for this draw"
        );

        uint256 mainPrizeAmount = balanceOf[address(jackpotPrizeWinner)][_tokenId] * 1250 * 10**18;
        ...
        for (uint256 index = 0; index < smallPrizeWinners; index++) {
            address smallWinner = _receivers[_random(smallPrizeWinners)];
            require(
