...
contract AntbTreasury is Ownable, IAntbTreasury {
    ...
    address public sANTB;
    uint256 public sANTBQueue; // Delays change to sANTB address
    ...
    constructor(
        ...
        isLiquidityToken[_ANTBDAI] = true;
        liquidityTokens.push(_ANTBDAI);
        bondCalculator[_ANTBDAI] = _bondCalculator;
        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for ANTB
        @param _amount uint
    ...
    function deposit(
        ...
        // mint ANTB needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        IERC20Mintable(ANTB).mint(msg.sender, send_);
    ...
    /**
        @notice allow approved address to burn ANTB for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw(uint256 _amount, address _token) external override {
        ...
        uint256 value = valueOfToken(_token, _amount);
        IANTBERC20(ANTB).burnFrom(msg.sender, value);
    ...
    function incurDebt(uint256 _amount, address _token) external {
        ...
        uint256 maximumDebt = IERC20(sANTB).balanceOf(msg.sender); // Can only borrow against sANTB held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with ANTB
        @param _amount uint
     */
    function repayDebtWithANTB(uint256 _amount) external {
        require(isDebtor[msg.sender], 'Not Appv');

        IANTBERC20(ANTB).burnFrom(msg.sender, _amount);

        debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
        totalDebt = totalDebt.sub(_amount);

        emit RepayDebt(msg.sender, ANTB, _amount, _amount);
    }
        ...
        override
    {
        ...
        IERC20Mintable(ANTB).mint(_recipient, _amount);
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(IERC20(ANTB).totalSupply().sub(totalDebt));
    }
    ...
    /**
        @notice returns ANTB valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    ...
    {
        if (isReserveToken[_token]) {
            // convert amount to match ANTB decimals
            value_ = _amount.mul(10**IERC20(ANTB).decimals()).div(
        ...
        returns (bool)
    {
        ...
        } else if (_managing == MANAGING.SANTB) {
            // 9
            sANTBQueue = block.number.add(blocksNeededForQueue);
        } else return false;
