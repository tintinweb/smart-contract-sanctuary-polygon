...
import {ITransferManager} from "./interfaces/ITransferManagerNFT.sol";
import "./interfaces/ICollection.sol";
// libraries
import {OrderTypes} from "./libraries/OrderTypes.sol";

/**
 * @title slabs Exchange
    ...
    ReentrancyGuard
{
    ...
    ) ERC2771Context(_trustedForwarder) EIP712("Slabs-Exchange", "1") {
    ...
    function cancelAllOrdersForSender(uint256 minNonce) external {
        require(
            minNonce > userMinOrderNonce[_msgSender()],
            "Cancel: Order nonce lower than current"
        );
        require(
            minNonce < userMinOrderNonce[_msgSender()] + 500000,
            "Cancel: Cannot cancel more orders"
        );
        userMinOrderNonce[_msgSender()] = minNonce;
        emit CancelAllOrders(_msgSender(), minNonce);
    }
    ...
    function cancelMultipleMakerOrders(
        ...
        for (uint256 i = 0; i < orderNonces.length; i++) {
            require(
                orderNonces[i] >= userMinOrderNonce[_msgSender()],
                "Cancel: Order nonce lower than current"
            );
            _isUserOrderNonceExecutedOrCancelled[_msgSender()][
                orderNonces[i]
            ] = true;
        }

        emit CancelMultipleOrders(_msgSender(), orderNonces);
    }
    ...
    /**
     * @notice Match a makerAsk with a makerBid
     * @param makerAsk maker ask order
     * @param makerBid maker bid order
     */
    function matchMakerBidWithMakerAsk(
        OrderTypes.MakerOrder calldata makerAsk,
        OrderTypes.MakerOrder calldata makerBid
    ) external nonReentrant {
        require(
            _msgSender() == owner() || makerAsk.signer == _msgSender(),
            "not an auction admin"
        );
        require(
        ...
        require(makerBid.currency == makerAsk.currency, "Currency Mismatch");
        require(makerBid.strategy == makerAsk.strategy, "Strategy Mismatch");

        // Check the maker bid order
        bytes32 orderHash = makerBid.hash();
        _validateOrder(makerBid, orderHash);

        orderHash = makerAsk.hash();
        _validateOrder(makerAsk, orderHash);

        (
            bool isExecutionValid,
            uint256 tokenId,
            uint256 amount
        ) = IExecutionStrategy(makerBid.strategy).canExecuteMakerBid(
                makerAsk,
                makerBid
            );
    ...
    /**
     * @notice Update transfer manager
    ...
    function _transferFeesAndFunds(
        ...
        // 2. Royalty fee
        {
            address[] memory royaltyFeeRecipient;
            uint256[] memory royaltyFeeAmount;

            if (ICollection(collection).supportsInterface(ERC998_MAGIC_VALUE)) {
                (royaltyFeeRecipient, royaltyFeeAmount) = royaltyFeeManager
                    .calculateRoyaltyForBundle(collection, tokenId, amount);
            } else {
                (royaltyFeeRecipient, royaltyFeeAmount) = royaltyFeeManager
            ...
            if (royaltyFeeRecipient.length > 0) {
                for (uint256 i = 0; i < royaltyFeeRecipient.length; i++) {
                    if (
                        (royaltyFeeRecipient[i] != address(0)) &&
                        (royaltyFeeAmount[i] != 0)
                    ) {
                        IERC20(currency).safeTransferFrom(
                            from,
                            royaltyFeeRecipient[i],
                            royaltyFeeAmount[i]
                        );
                        finalSellerAmount -= royaltyFeeAmount[i];
                        emit RoyaltyPayment(
                            collection,
                            tokenId,
                            royaltyFeeRecipient[i],
...
interface IExecutionStrategy {
    ...
    function changeFee(uint256 _protocolFee) external;
