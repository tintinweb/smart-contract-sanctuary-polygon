...
╚═╝     ╚═╝╚═╝╚═╝  ╚═══╝╚═╝╚═╝  ╚═══╝ ╚═════╝ ╚═╝ ╚═════╝ ╚═╝  ╚═╝╚═╝     ╚═╝╚══════╝
    ...
    This is the NFT staking contract.
    @custom:security-contact [email protected]

  Read more on https://mining.game/
...
contract NftStaker is Ownable{
    ...
    // this address is for sending rewards to deposited users
    ...
    constructor(IERC1155 _nft, IERC20 _rewardsToken) {        
        nft = _nft;
    ...
    function setRewardsWallet(address _newRewardsWallet) external onlyOwner {
    ...
    function stake(uint256 tokenId, uint256 amount) public {
        if (tokenId==1) {
            require(amount==1,"You can't stake more than 1 for tokenID 1");
        }
    ...
    function stakeBatch(uint256[] memory tokenIds, uint256[] memory amounts) public {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            if (tokenIds[i]==1) {
                require(amounts[i]==1,"You can't stake more than 1 for tokenID 1");
            }
            _stake(tokenIds[i], amounts[i]);
        }
    }

    function _stake(uint256 _tokenId,uint256 amount) internal {
    ...
    function unstake(uint256 depositId) public {        
        require(nft.balanceOf(msg.sender, nftDeposits[depositId].tokenId) != 0, "User must be the owner of the staked nft");
        _unstake(depositId);
    }
    ...
    function _unstake(uint256 depositId) internal {
    ...
    function withdrawRewards(uint256 depositId, uint256 _amount) external {
        require(depositId <= nftDeposits.length);

        require(_amount > 0, "Amound should be greater than zero");

        require(nftDeposits[depositId].rewardsEarned > _amount, "Rewards token amount is not enough.");

        rewardsToken.transferFrom(rewardsWallet, msg.sender, _amount);

        nftDeposits[depositId].rewardsEarned -= _amount;

        nftDeposits[depositId].rewardsReleased += _amount;
