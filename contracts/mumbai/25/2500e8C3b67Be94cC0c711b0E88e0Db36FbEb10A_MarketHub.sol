...
import "./ICollectionRegistry.sol";

contract CollectionRegistry is ICollectionRegistry, ColleAccess {
    // e.g. collection address => isRegistered
    mapping(address => bool) internal collections;

    constructor() {}

    function registerCollection(address _collection) public onlyColle {
    ...
    function unregisterCollection(address _collection) public onlyColle {
...
contract CurrencyRegistry is ICurrencyRegistry, ColleAccess {
    ...
    function registerERC20(address _currency) public onlyColle {
        require(_currency != address(0), "Currency address cannot be 0");
    ...
    function unregisterERC20(address _currency) public onlyColle {
        require(_currency != address(0), "Currency address cannot be 0");
        ...
        require(_erc20sByCurrency[_currency] != address(0), "ERC20 is not registered");
...
import "../../utils/ColleAccess.sol";
import "./IEscrow.sol";
import "./IEscrowRegistry.sol";

contract EscrowRegistry is IEscrowRegistry, ColleAccess {
    IEscrow internal escrow;

    constructor() {}

    function registerEscrow(address _escrow) public onlyColle {
        require(address(escrow) == address(0), "Escrow is already registered");
        require(_escrow != address(0), "Escrow address cannot be 0");

        escrow = IEscrow(_escrow);
        escrow.register();
    }

    function unregisterEscrow() public onlyColle {
        require(address(escrow) != address(0), "Escrow is not registered");

        escrow.unregister();
        escrow = IEscrow(address(0));
    }
...
contract KYCRegistry is IKYCRegistry, ColleAccess {
    mapping(address => Account) internal _accounts;

    constructor() {}

    function registerAccount(address _account, bytes32 _tier) public onlyColle {
    ...
    function updateTier(address _account, bytes32 _tier) public onlyColle {
    ...
    function haultAccount(address _account) public onlyColle {
        require(_accounts[_account].account != address(0), "Account not registered");
    ...
    function unhaultAccount(address _account) public onlyColle {
        require(_accounts[_account].account != address(0), "Account not registered");
    ...
    function banAccount(address _account) public onlyColle {
        require(_accounts[_account].account != address(0), "Account not registered");
        require(_accounts[_account].status == AccountStatus.HAULTED, "Account not haulted");
    ...
    function unbanAccount(address _account) public onlyColle {
        require(_accounts[_account].account != address(0), "Account not registered");
...
import "./currencies/CurrencyRegistry.sol";
import "./royalties/RoyaltyRegistry.sol";
import "./kycs/KYCRegistry.sol";
import "./vaults/VaultRegistry.sol";
import "./escrow/EscrowRegistry.sol";
import "./collections/CollectionRegistry.sol";
    ...
    Pausable
{
    ...
    IUpgradeGatekeeper private _upgradeGatekeeper;

    constructor(address upgradeGatekeeper) {
        __SignatureValidator_init("Colle Marketplace", "v0.1");
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        minUSDCPrice = 1e6;
        _upgradeGatekeeper = IUpgradeGatekeeper(upgradeGatekeeper);
    }

    function notifyTokenIdNoLongerAvailable(address _seller, address _erc721, uint256 _tokenId) public {
        ...
        for (uint256 i = 0; i < marketNames.length; ++i) {
            IMarket market = getMarket(marketNames[i]);
            market.handleTokenIdNoLongerAvailable(_seller, _erc721, _tokenId);
        }
    }

    function setMinUSDCPrice(uint256 _minUSDCPrice) public onlyColle {
    ...
    function getUpgradeGatekeeper() public view returns (IUpgradeGatekeeper) {
        return _upgradeGatekeeper;
    }

    function pauseNewSales() public onlyColle {
        _pause();
    }

    function unpauseNewSales() public onlyColle {
        _unpause();
    }
...
contract MarketRegistry is IMarketRegistry, ColleAccess {
    bytes32 public constant MARKET_ROLE = keccak256("MARKET_ROLE");

    // e.g. "Listing" => 0xabcd, "Offer" => 0xefgh
    mapping(bytes32 => address) internal _markets;
    bytes32[] internal _marketNames;

    constructor() {}

    function registerMarket(address _marketAddress, bytes32 _marketName) public onlyColle {
        require(_markets[_marketName] == address(0), "Market already registered");
        require(_marketAddress != address(0), "Market address cannot be 0");
    ...
    function unregisterMarket(address _marketAddress, bytes32 _marketName) public onlyColle {
    ...
    function getMarket(bytes32 _marketName) public view returns (IMarket) {
        return IMarket(_markets[_marketName]);
    }
...
contract RoyaltyRegistry is IRoyaltyRegistry {
    ...
    function registerRoyalty(bytes32 _accountTier, address _royalty) external {
        require(royalties[_accountTier] == address(0), "RoyaltyRegistry: royalty already registered");
        require(_royalty != address(0), "RoyaltyRegistry: royalty address cannot be 0");
    ...
    function unregisterRoyalty(bytes32 _accountTier) external {
        require(royalties[_accountTier] != address(0), "RoyaltyRegistry: royalty not registered");
    ...
    function registerRoyaltyPool(address _royaltyPool) external {
        require(address(royaltyPool) == address(0), "RoyaltyRegistry: royalty pool already registered");
...
contract VaultRegistry is IVaultRegistry, ColleAccess {
    IVault internal _vault;

    constructor() {}

    function registerVault(address _vaultAddress) public onlyColle {
        require(address(_vault) == address(0), "Vault already registered");
    ...
    function unregisterVault(address _vaultAddress) public onlyColle {
        require(address(_vault) == _vaultAddress, "Vault not registered");
        require(_vaultAddress != address(0), "Vault address cannot be 0");
