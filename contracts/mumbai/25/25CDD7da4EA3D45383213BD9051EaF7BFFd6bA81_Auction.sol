...
contract Auction {
    ...
    IERC20 private paymentToken; //setter later
    uint112 private tokenBal;

    //Declare an Event
    event AuctionCreated(address indexed _client, uint256 _minPrice, uint32 noOfCopies, AuctionState _auctionState);
    event BidPlaced(address indexed _bidder, uint256 _value);
    event BiddingEnded();
    ...
    event BidDealUnsuccessfulRefund(address indexed _bidder, uint256 _refundAmount, uint256 _paidAmount);
    event VerificationEndedSuccess();
    event VerificationEndedCancelled();

    constructor(IERC20 _paymentToken, uint256 _minPrice, uint32 _noOfCopies, address _client, address _admin) {
        admin = _admin;
        ...
        console.log("Auction deployed for admin: ", admin);
    }
    ...
    function selectBid(address selectedAddress) public onlyClientOrAdmin {
        ...
        require(b.bidState == BidState.BIDDING, "Deal state not bidding");
        b.isSelected = true;
    ...
    // admin select all bids once time is up
    // function autoSelectBids() public {
    // }

    function refundUnsuccessfulBids() public onlyAdmin {
        uint32 count = 0;
        for (uint8 i = 0; i < bidders.length; i++) {
            ...
            if (!b.isSelected) {
                ...
                b.bidAmount = 0;
                b.bidState = BidState.NOT_SELECTED_REFUNDED;
                count ++;
            }
        }
    ...
    function setBidDealSuccess(address bidder) public {
        ...
        require(msg.sender == admin || msg.sender == bidder, "Txn sender not admin or SP");
    ...
    function setBidDealRefund(address bidder, uint256 refundAmount) public onlyAdmin {
        Bid storage b = bids[bidder];
        require(b.bidState == BidState.SELECTED, "Deal not selected");
        require(refundAmount <= b.bidAmount, "Refund amount greater than bid amount");
        ...
        emit BidDealUnsuccessfulRefund(bidder, refundAmount, b.bidAmount - refundAmount);
    }

    function endVerificationSuccess() public onlyAdmin {
        auctionState = AuctionState.COMPLETED;
        emit VerificationEndedSuccess();
    }

    function endVerificationCancelled() public onlyAdmin {
        auctionState = AuctionState.CANCELLED;
        emit VerificationEndedCancelled();
    }
    ...
    modifier onlyClientOrAdmin {
      require(msg.sender == client || msg.sender == admin, "Txn sender not admin or client");
      _;
   }
}
