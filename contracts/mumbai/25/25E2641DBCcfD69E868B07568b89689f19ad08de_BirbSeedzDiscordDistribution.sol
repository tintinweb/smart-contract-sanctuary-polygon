...
contract EligibleUser {
    ...
    constructor(string memory _discordName){
...
contract BirbSeedzDiscordDistribution is Ownable {
    ...
    function claimTokens() public {
        ...
        uint _claimableTokens = getClaimableTokens(msg.sender);
        require(_claimableTokens != 0, "You have no tokens to claim yet");
        transfer(msg.sender, _claimableTokens);
    }

    function addUserByOwner(address _userAddr, string memory _discordName) public onlyOwner{
        EligibleUser newUser = new EligibleUser(_discordName);
        eligibleUsers[_userAddr] = newUser;
    }

    function removeUserByOwner(address _userAddr) public onlyOwner{
        eligibleUsers[_userAddr] = new EligibleUser("null");
    }

    uint public tokenAccumulationSpeed =  10000000000000000; // per day in wei
    ...
    function getClaimableTokens(address addr) public view returns (uint){
        return ((block.timestamp - eligibleUsers[addr].claimedTime()) / 1 minutes) * (tokenAccumulationSpeed);// / 1440); // 1440 minutes in day / 86400 seconds
    }
