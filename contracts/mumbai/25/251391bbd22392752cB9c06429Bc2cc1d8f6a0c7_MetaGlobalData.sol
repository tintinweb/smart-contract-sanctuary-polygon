...
import "../storage/MetaGlobalDataStorage.sol";
import "../helpers/MetaGovernanceInternal.sol";
import "../../interfaces/IMetaGlobalData.sol";

contract MetaGlobalData is IMetaGlobalData, MetaGovernanceInternal {
    using MetaGlobalDataStorage for MetaGlobalDataStorage.Layout;

    function getGlobalDataBytes32(bytes32 prop, bytes32 key) external view override returns(bytes32) {
        return MetaGlobalDataStorage.layout().getDataBytes32(prop, key);
    }
    function setGlobalDataBytes32(bytes32 prop, bytes32 key, bytes32 value) external override onlyPropertyManager(prop) {
        MetaGlobalDataStorage.layout().setDataBytes32(prop, key, value);
    }

    function getGlobalDataBytes(bytes32 prop, bytes32 key) external view override returns(bytes memory) {
        return MetaGlobalDataStorage.layout().getDataBytes(prop, key);
    }
    function setGlobalDataBytes(bytes32 prop, bytes32 key, bytes calldata value) external override onlyPropertyManager(prop) {
        MetaGlobalDataStorage.layout().setDataBytes(prop, key, value);
    }


    function getGlobalDataSetContainsValue(bytes32 prop, bytes32 key, bytes32 value) external view override returns(bool) {
        return MetaGlobalDataStorage.layout().getDataSetContainsValue(prop, key, value);
    }
    function getGlobalDataSetLength(bytes32 prop, bytes32 key) external view override returns(uint256) {
        return MetaGlobalDataStorage.layout().getDataSetLength(prop, key);
    }
    function getGlobalDataSetAllValues(bytes32 prop, bytes32 key) external view override returns(bytes32[] memory) {
        return MetaGlobalDataStorage.layout().getDataSetAllValues(prop, key);
    }
    function setGlobalDataSetAddValue(bytes32 prop, bytes32 key, bytes32 value) external override {
        MetaGlobalDataStorage.layout().setDataSetAddValue(prop, key, value);
    }
    function setGlobalDataSetRemoveValue(bytes32 prop, bytes32 key, bytes32 value) external override {
        MetaGlobalDataStorage.layout().setDataSetRemoveValue(prop, key, value);
    }

    function getGlobalDataMapValue(bytes32 prop, bytes32 key, bytes32 vKey) external view override returns(bytes32) {
        return MetaGlobalDataStorage.layout().getDataMapValue(prop, key, vKey);
    }
    function getGlobalDataMapLength(bytes32 prop, bytes32 key) external view override returns(uint256) {
        return MetaGlobalDataStorage.layout().getDataMapLength(prop, key);
    }
    function getGlobalDataMapAllEntries(bytes32 prop, bytes32 key) external view override returns(bytes32[] memory, bytes32[] memory) {
        return MetaGlobalDataStorage.layout().getDataMapAllEntries(prop, key);
    }
    function setGlobalDataMapSetValue(bytes32 prop, bytes32 key, bytes32 vKey, bytes32 vValue) external override {
        MetaGlobalDataStorage.layout().setDataMapSetValue(prop, key, vKey, vValue);
    }
...
library MetaGlobalDataStorage {
    ...
    struct Layout {
        mapping(bytes32 => DataStorageLib.DataStorage) data; //Category -> DataStorage
    }
    ...
    function getDataBytes32(Layout storage l, bytes32 prop, bytes32 key) internal view returns(bytes32) {
        return l.data[prop].getBytes32Value(key);
    }
    function setDataBytes32(Layout storage l, bytes32 prop, bytes32 key, bytes32 value) internal {
        l.data[prop].setValue(key, value);
    }

    function getDataBytes(Layout storage l, bytes32 prop, bytes32 key) internal view returns(bytes memory) {
        return l.data[prop].getBytesValue(key);
    }
    function setDataBytes(Layout storage l, bytes32 prop, bytes32 key, bytes memory value) internal {
        l.data[prop].setValue(key, value);
    }

    function getDataSetContainsValue(Layout storage l, bytes32 prop, bytes32 key, bytes32 value) internal view returns(bool) {
        return l.data[prop].getSetContainsValue(key, value);
    }
    function getDataSetLength(Layout storage l, bytes32 prop, bytes32 key) internal view returns(uint256) {
        return l.data[prop].getSetLength(key);
    }
    function getDataSetAllValues(Layout storage l, bytes32 prop, bytes32 key) internal view returns(bytes32[] memory) {
        return l.data[prop].getSetAllValues(key);
    }
    function setDataSetAddValue(Layout storage l, bytes32 prop, bytes32 key, bytes32 value) internal {
        l.data[prop].setValue(key, value, false);
    }
    function setDataSetRemoveValue(Layout storage l, bytes32 prop, bytes32 key, bytes32 value) internal {
        l.data[prop].setValue(key, value, true);
    }

    function getDataMapValue(Layout storage l, bytes32 prop, bytes32 key, bytes32 vKey) internal view returns(bytes32) {
        return l.data[prop].getMapValue(key, vKey);
    }
    function getDataMapLength(Layout storage l, bytes32 prop, bytes32 key) internal view returns(uint256) {
        return l.data[prop].getMapLength(key);
    }
    function getDataMapAllEntries(Layout storage l, bytes32 prop, bytes32 key) internal view returns(bytes32[] memory, bytes32[] memory) {
        return l.data[prop].getMapAllEntries(key);
    }
    function setDataMapSetValue(Layout storage l, bytes32 prop, bytes32 key, bytes32 vKey, bytes32 vValue) internal {
        l.data[prop].setValue(key, vKey, vValue);
    }
}
