...
contract LazyTrade is Initializable, OwnableUpgradeable, VerifySignature {
    ...
    function saleToken(
        ...
        if (ids[3] == 721) {
            ...
            if (IERC721Upgradeable(_conAddr).ownerOf(ids[0]) == from) {
    ...
    function acceptBId(
        ...
        if (ids[3] == 721) {
            ...
            if (IERC721Upgradeable(_conAddr).ownerOf(ids[0]) == msg.sender) {
    ...
    // ipfsmetadata[0] - meta, ipfsmetadata[1] - datas, ipfsmetadata[2] - status, ipfsmetadata[3] - _message, 
    // users[0] - from, users[1] - to
    // datas[0] - supply, datas[1] - nftType, datas[2] -  royal, datas[3] - price, datas[4] - total, datas[5] - _nonce
    ...
    function lazyMint(
        ...
        require(verify(users[0], datas[3], ipfsmetadata[3], datas[5], signature) == true,"Not vaild User");
    ...
    function setCollectionAddress(address _ERC721, address _ERC1155)
        public
        onlyOwner
    {
        lazy721 = _ERC721;
