...
/**
 * @title ERC721MintableWithSlots
 * ERC721MintableWithSlots - ERC721 contract that allows to mint some tokens with id range
 */
contract ERC721MintableWithSlots is ERC721Mintable {
    ...
    constructor(string memory name_, string memory symbol_, bytes32 collectionId_,
        address owner_, uint256 slotStart_, uint256 slotEnd_, SystemContext system_)
    ERC721Mintable(name_, symbol_, collectionId_, owner_, system_) {
    ...
    function _validateMintForNonBridgeCaller(uint256 tokenId) internal view override {
...
contract ERC721Mintable is ERC721Enumerable, AccessControl {
    ...
    */
    ...
    function moveTo(uint16 _l0ChainId, bytes calldata _destinationBridge, bytes calldata _destinationContract, uint256 _tokenId) external payable {
        require(_isApprovedOrOwner(msg.sender, _tokenId), "ERC721: transfer caller is not owner nor approved");
        transferFrom(msg.sender, address(this), _tokenId);

        system.getBridge().mintOnSecondChain{value : msg.value}(_l0ChainId, _destinationBridge, _destinationContract, msg.sender, msg.sender, _tokenId, _tokenURIs[_tokenId]);
    }
...
contract LayerZeroBridge is ILayerZeroReceiver {
    ...
    function mintOnSecondChain(
        ...
        _sendMessage(chainId_, bridge_, buffer, refundAddress_);
    }
        ...
        for (uint256 i = 0; i < deploymentParams_.length; i++) {
            bytes memory buffer = abi.encode(
                Data({
                    ...
                    apiVersion: apiVersion,
                    rawData: abi.encode(DeployData({bytecodeHash: bytecodeHash_,
