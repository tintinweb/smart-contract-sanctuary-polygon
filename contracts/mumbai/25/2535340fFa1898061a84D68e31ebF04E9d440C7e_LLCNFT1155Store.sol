...
abstract contract ERC1155URIStorage is ERC1155 {
    ...
    function uri(uint256 id) public view override returns (string memory) {
        return string(abi.encodePacked(super.uri(0),_tokenURIs[id]));
    }
...
contract LLCNFT1155Store is Ownable, ERC1155URIStorage {
    string public name;
    ...
    function mint(
        ...
        require(
            ecrecover(msgHash, v, r, s) == signer,
            "signer should sign tokenId"
        );

        require((creator[id] == address(0)) || (creator[id] == to), "invalid creator");
        require(!messageHash[msgHash], "signature duplicate");

        messageHash[msgHash] = true;

        if(creator[id] == address(0)) creator[id] = to;
    ...
    function mintWithSignature(
        ...
        bytes32 msgHash = prepareMintHash(to, id, blockExpiry);
    ...
    function burn(uint tokenId, uint amount) public {
        ...
        return
            keccak256(
                abi.encodePacked(
                    ...
                    keccak256(
                        abi.encodePacked(
                            abi.encodePacked(this, id, blockExpiry)
                        )
                    )
                )
            );
