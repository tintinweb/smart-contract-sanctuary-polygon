...
contract BTCLDynamicLottery is AutomationCompatible, AutomatedRandomness, ReentrancyGuard {
    ...
    function checkUpkeep(bytes calldata /* checkData */) external view override returns (bool upkeepNeeded, bytes memory performData) {
        ...
        uint256 currentNextRound = nextRound;

        for (uint256 i = currentNextRound; i < currentNextRound + maxRounds; i++) {
        ...
        performData = abi.encode(currentNextRound, round, roundsToProcess);
    }

    function performUpkeep(bytes calldata performData) external nonReentrant override {
        (uint256 currentNextRound, uint256 currentRound, uint256[] memory roundsToProcess) = abi.decode(performData, (uint256, uint256, uint256[]));
        ...
        if (roundsToProcess[roundsToProcess.length - 1] + 1 == currentNextRound + maxRounds) {
            nextRound = currentNextRound + maxRounds;
