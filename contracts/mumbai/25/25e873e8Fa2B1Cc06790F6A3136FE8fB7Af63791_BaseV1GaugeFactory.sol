...
contract Gauge is IGauge {
  ...
  /// @dev Default snx staking contract implementation
  ...
  function getPriorRewardPerToken(address token, uint timestamp) public view returns (uint, uint) {
    ...
    if (rewardPerTokenCheckpoints[token][nCheckpoints - 1].timestamp <= timestamp) {
      return (rewardPerTokenCheckpoints[token][nCheckpoints - 1].rewardPerToken,
    ...
    return (rewardPerTokenCheckpoints[token][lower].rewardPerToken,
  ...
  function getReward(address account, address[] memory tokens) external lock override {
