...
/** 
 *  SourceUnit: /Users/iggyi/Git/culture-vault/airdrop/contracts/WertMarket.sol
*/
...
contract WertMarket is Market, ERC1155Holder, ERC721Holder, Ownable {
    ...
    function sendOrderMatch(Market _market, address nftOwner, Market.Order memory sellOrder, bytes memory sellerSignature, Market.Order memory buyOrder, bytes memory buyerSignature, uint256 amount) public payable nonReentrant{
        _market.orderMatch{value: msg.value}(sellOrder, sellerSignature, buyOrder, buyerSignature, amount);
    ...
    function _transfer(
        ...
        uint256 tokenStandard = _market.getTokenStandard(tokenAddress);
        require(tokenStandard != 0, "Not support.");
        if (tokenStandard == 721) {
            IERC721(tokenAddress).safeTransferFrom(from, to, tokenId);
        } else if (tokenStandard == 1155) {
    ...
    function verifiedTransfer(Market _market, uint256 orderId, address tokenAddress, uint256 tokenId, address to, uint256 amount) public nonReentrant {
    ...
    function forceWithdraw(Market _market,address tokenAddress, uint256 tokenId, uint256 amount, address receiverAddress) public nonReentrant onlyOwner {
        uint256 tokenStandard = _market.getTokenStandard(tokenAddress);
        require(tokenStandard != 0, "Not support.");
        if (tokenStandard == 721) {
