...
contract Messenger is NonblockingLzApp {
    bytes public constant PAYLOAD = "any 32-byte data----------------";
    string public someMessage;

    constructor(address _lzEndpoint) NonblockingLzApp(_lzEndpoint) {}

    function sendMessage(uint16 _dstChainId, string calldata message) external payable {
        _lzSend(_dstChainId, bytes(message), payable(msg.sender), address(0x0), bytes(""), msg.value);
    }

    function _nonblockingLzReceive(uint16, bytes memory, uint64, bytes memory payload) internal override {
        someMessage = string(payload);
    }
