...
contract Web3CertIdentityRegistry is IWeb3CertIdentityRegistry, AccessControl {
     ...
     */
    function registerIdentityAndManagementKey(address _user, string calldata _name, string calldata _url, address _management, bytes calldata _permit) external override onlyRole(VERIFIER_ROLE) {
        require(_user!=address(0), "zero address");
        // verify signature
        ...
        // approve identity
        identities[identityKey].approved = block.timestamp;
        emit IdentityApproved(identityKey);
        // add management key
