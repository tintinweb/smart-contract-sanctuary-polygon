...
contract COEHelper {
  ...
  // a test event to see if the contract is working
  event TestEvent(
    uint256 totalCost,
    uint256 price,
    uint256 convertedCost,
    uint256 decimals
  );
  ...
  function purchaseWithToken(
    ...
    uint256 convertedCost = _totalCost *
      10 ** FunctionInterface(_tokenAddress).decimals();

    // TODO: CONVERT TO AMOUNT OF AEG CURRENTLY JUST USD
    ...
    if (_tokenAddress == aegAddress) {
      ...
      // emit an event to show price
      emit TestEvent(
        _totalCost,
        _totalCost,
        convertedCost,
        FunctionInterface(_tokenAddress).decimals()
      );
    ...
    uint256 price = getPrice(_tokenAddress);
