...
contract Market is Ownable {
	...
	mapping(address => mapping(uint256 => uint256)) nftLatestAuction;
	...
	function listToken(address token, uint tokenId, uint price) external {
		...
		nftLatestAuction[token][tokenId] = _listingId;
	...
	function getNftLatestAuction(address token, uint256 tokenId) public view returns (uint256) {
		return nftLatestAuction[token][tokenId];
	}
	...
	function buyToken(uint listingId) external payable {
		...
		nftLatestAuction[listing.token][listing.tokenId] = 0;
