 ...
 */
contract TitleEscrowFactory is ITitleEscrowFactory, TitleEscrowFactoryErrors, RelayRecipientV2 {
  ...
  function create(uint256 tokenId) external override returns (address) {
    ...
    bytes32 salt = keccak256(abi.encodePacked(_msgSender(), tokenId));
    address titleEscrow = Clones.cloneDeterministic(implementation, salt);
    TitleEscrow(titleEscrow).initialize(_msgSender(), tokenId);

    emit TitleEscrowCreated(titleEscrow, _msgSender(), tokenId);
