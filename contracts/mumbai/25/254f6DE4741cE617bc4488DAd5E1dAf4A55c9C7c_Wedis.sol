...
// Tableland contract and metadata table for the contract. 
struct StoredData {
    uint256 _tableId;
    string _table;
    ITablelandTables _tableland;
    mapping(uint256 => string) _tables; // list of all tokens by their tablename
    mapping(uint256 => string) _names; // list of all tokens by their name
    mapping(uint256 => mapping(address => bool)) _grantees; // will hold a map of all who have access to the kv
}
  ...
  UUPSUpgradeable, ERC721HolderUpgradeable {
    ...
    */
    function _createKV(address owner, string memory name) private returns (uint256 tokenId) {
        ...
        stored._tables[tokenId] = userKvTable;
        stored._names[tokenId] = name;
    ...
    */
    function hasAccessTo(uint256 tokenId, address from) public view returns (bool) {
        return stored._grantees[tokenId][from];
    }

    function changeOwner(uint256 tokenId, address from, address to) private {
        stored._grantees[tokenId][to] = true;
        stored._grantees[tokenId][from] = false;
    }
    ...
    * executing the query. This works well because the owner of the token is this smart contract.
    */
    function runSQL(
        address caller,
        uint256 tableId,
    ...
    */
    ...
    function easyMint() public onlyOwner returns (uint256) {
        return createTable(_msgSender(), "tada");
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns(string memory) {
        ...
        string memory name = stored._names[tokenId];
		return string.concat(
            'data:application/json,{"name":"',
            name,
            '","external_url":"https://testnet.tableland.network/chain/80001/tables/4427","image":"https://render.tableland.xyz/80001/4427","animation_url":"https://render.tableland.xyz/anim/?chain=80001\u0026id=4427","attributes":[{"display_type":"date","trait_type":"created","value":1670957593}]}'
		);
	}
}