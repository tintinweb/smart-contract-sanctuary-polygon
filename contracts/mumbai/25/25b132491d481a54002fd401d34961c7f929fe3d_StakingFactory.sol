...
contract StakingFactory {
    ...
    error TimeIsZero();
    error AmountIsZero();
    error StartIsMoreThanFinish(uint _startAt, uint _finishAt);
    error FeeNotPayed();
    ...
    function createStakePool(
        ...
        if (_start >= _finish) {
            revert StartIsMoreThanFinish(_start, _finish);
        }
        ...
        returns (StakingContract[] memory, uint[] memory)
    {
        ...
        for (uint i; i < stakingContracts.length; ++i) {
...
contract Staking is Ownable, IStaking {
    ...
    constructor(
        ...
        require(_startAt < _finishAt, "startAt must be before finishAt");
    ...
    function lastTimeRewardApplicable() public view returns (uint) {
        uint minFinishStart = Math.min(finishAt, block.timestamp);
    ...
    function emergencyWithdraw(uint _amount) public updateReward(msg.sender) {
        ...
        uint fee = 0;
        if (block.timestamp < finishAt) {
            fee = (_amount * emergencyWithdrawPercentage) / 100;
            amount = _amount - fee;
        } else {
            amount = _amount;
        }
    ...
    function notifyRewardAmount(
        ...
        if (rewardsNotified) revert RewardsAlreadyNotified();
        ...
        if (rewardRate == 0) revert RewardRateIsZero();
