...
contract CreatorTokenAirdrop is Ownable, ReentrancyGuard {
    ...
    // token => beneficiary => airdrop
    mapping(address => mapping(address => Airdrop)) private airdrops;
    // token => operator
    mapping(address => address) private approvedOperators;
    // token => isAdded
    mapping(address => bool) private tokenAdded;
    // beneficiary => isAdded
    mapping(address => bool) private beneficiaryAdded;
    ...
    modifier onlyOperator(address _token) {
        require(
            approvedOperators[_token] == msg.sender || owner() == msg.sender
        );
        _;
    }
    ...
    function addAirdrop(
        ...
        if (airdrops[_token][_to].amount == 0) {
            airdrops[_token][_to] = Airdrop(
                _amount,
                _token,
                _to,
                _isApproved,
            ...
            airdrops[_token][_to].amount += _amount;
        }

        if (!tokenAdded[_token]) {
            tokens.push(_token);
            tokenAdded[_token] = true;
        }

        if (!beneficiaryAdded[_to]) {
            beneficiaries.push(_to);
            beneficiaryAdded[_to] = true;
        }
        ...
        onlyOperator(_token)
    {
        ...
        airdrops[_token][_to].amount = 0;
        airdrops[_token][_to].isApproved = false;
        airdrops[_token][_to].beneficiary = address(0);
        airdrops[_token][_to].token = address(0);
    ...
    function addApprovedOperator(address _token, address _owner)
        external
        onlyOwner
    {
    ...
    function setApproveAirdrop(
        ...
        address _to,
        bool _approve
    ) external onlyOperator(_token) {
        airdrops[_token][_to].isApproved = _approve;
    ...
    function claimAirdrop(address _token) external nonReentrant {
        ...
        airdrops[_token][sender].amount = 0;
        airdrops[_token][sender].isClaimed = true;
        airdrops[_token][sender].isApproved = false;
        airdrops[_token][sender].dateClaimed = block.timestamp;
    ...
    {
        return airdrops[_token][_to].amount;
    }
    ...
    {
        return airdrops[_token][_to].isApproved;
    }

    function getAirdropInfo(address _token, address _to)
        public
        view
    ...
    function getAirdropsByToken(address _token)
        external
        view
        returns (Airdrop[] memory)
    {
        ...
        for (uint256 i = 0; i < beneficiaries.length; i++) {
            token = airdrops[_token][beneficiaries[i]].token;
            beneficiary = airdrops[_token][beneficiaries[i]].beneficiary;
        ...
        Airdrop[] memory airdropArray = new Airdrop[](size);
        ...
        for (uint256 i = 0; i < beneficiaries.length; i++) {
            ...
            if (token != address(0) && beneficiary != address(0)) {
                airdropArray[i] = airdrops[_token][beneficiaries[i]];
            }
        }
        return airdropArray;
    }
