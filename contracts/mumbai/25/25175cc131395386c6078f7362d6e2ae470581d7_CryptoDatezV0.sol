...
contract CryptoDatezV0 is ERC721EnumerableUpgradeable, OwnableUpgradeable {
    ...
    //     init(_name, _symbol, _initBaseURI, _trustedForwarder);
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return
            bytes(currentBaseURI).length > 0
                ? string(abi.encodePacked(currentBaseURI)) // ? string(abi.encodePacked(currentBaseURI, tokenId.toString(), baseExtension))
                : '';
    }
    ...
    //     return ERC721EnumerableUpgradeable._beforeTokenTransfer(from, to, tokenId);
    // }
