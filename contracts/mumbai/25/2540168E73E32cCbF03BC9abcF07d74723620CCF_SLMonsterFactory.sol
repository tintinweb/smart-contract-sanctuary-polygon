...
import {Monster} from "./Monster.sol";

abstract contract Trait is Monster {
    using Unsafe for uint256;
    using CountersUpgradeable for CountersUpgradeable.Counter;
    using EnumerableSetUpgradeable for EnumerableSetUpgradeable.UintSet;
     ...
     *  TraitType
     */
    function addMonsterTraitType(string calldata _name) external onlyOperator {
        ...
        emitCreate("MonsterTraitType", typeId);
    }
    ...
    function removeMonsterTraitValue(uint256 _valueId) external onlyOperator {
        uint256 monsterLength = getMonsterLengthOfTraitValue(_valueId, true) +
            getMonsterLengthOfTraitValue(_valueId, false);

        if (monsterLength != 0) revert AlreadyExistMonster();
    ...
    function addMonsterOfTrait(
        uint256 _typeId,
        TraitMonsterSet[] calldata _traitMonsterSets
    ) external onlyOperator {
        if (!isExistTraitTypeById(_typeId)) revert InvalidTraitTypeId();

        for (uint256 i = 0; i < _traitMonsterSets.length; i = i.increment()) {
            if (!_isValidTraitMonsterSet(_typeId, _traitMonsterSets[i]))
                revert InvalidTraitMonsterSet();

            TraitMonsterSet calldata traitMonsterSet = _traitMonsterSets[i];

            if (traitMonsterSet.normalMonsterIds.length > 0) {
                _addMonsterOfTrait(
                    _typeId,
                    traitMonsterSet.valueId,
                    false,
                    traitMonsterSet.normalMonsterIds
                );
            }

            if (traitMonsterSet.shadowMonsterIds.length > 0) {
                _addMonsterOfTrait(
                    _typeId,
                    traitMonsterSet.valueId,
                    true,
                    traitMonsterSet.shadowMonsterIds
                );
            }
        }
    }
    ...
    function removeMonsterOfTrait(
        ...
        for (uint256 i = 0; i < _monsterIds.length; i = i.increment()) {
            monsterOfTraitType[typeId][_isShadow].remove(_monsterIds[i]);
            monsterOfTraitValue[_valueId][_isShadow].remove(_monsterIds[i]);
        }
    }

    function _addMonsterOfTrait(
        uint256 _typeId,
        uint256 _valueId,
        bool _isShadow,
        uint256[] calldata _monsterIds
        ...
        for (uint256 i = 0; i < _monsterIds.length; i = i.increment()) {
            if (isContainMonsterOfTraitType(_typeId, _isShadow, _monsterIds[i]))
                revert AlreadyExistMonster();

            monsterOfTraitType[_typeId][_isShadow].add(_monsterIds[i]);
            monsterOfTraitValue[_valueId][_isShadow].add(_monsterIds[i]);
        }
    }

    function _isValidTraitMonsterSet(
        uint256 _typeId,
        TraitMonsterSet calldata _traitMonsterSet
    ) private view returns (bool) {
        if (!isContainTraitValueOfType(_typeId, _traitMonsterSet.valueId))
            return false;

        if (!isExistMonsterBatch(true, _traitMonsterSet.shadowMonsterIds))
            return false;

        if (!isExistMonsterBatch(false, _traitMonsterSet.normalMonsterIds))
            return false;
    ...
    function isContainMonsterOfTraitType(
        ...
        return monsterOfTraitType[_typeId][_isShadow].contains(_monsterId);
    }
    ...
    function isContainMonsterOfTraitTypeBatch(
        ...
        for (uint256 i = 0; i < _monsterIds.length; i++) {
            if (
                !isContainMonsterOfTraitType(_typeId, _isShadow, _monsterIds[i])
            ) return false;
        }
    ...
    function isContainMonsterOfTraitValue(
        ...
        return monsterOfTraitValue[_valueId][_isShadow].contains(_monsterId);
    }
    ...
    function getMonsterLengthOfTraitValue(
        ...
        return monsterOfTraitValue[_valueId][_isShadow].length();
    }
    ...
    function getMonsterIdOfTraitType(
        ...
        return monsterOfTraitType[_typeId][_isShadow].values();
    }
    ...
    function getMonsterIdOfTraitValue(
        ...
        return monsterOfTraitValue[_valueId][_isShadow].values();
    }
}
...
import {Trait} from "./parts/Trait.sol";
import {ISLProject} from "../project/ISLProject.sol";

contract SLMonsterFactory is Trait, UUPSUpgradeable {
