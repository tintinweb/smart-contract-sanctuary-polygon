...
contract GabrielV3 is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    function enableUnstaking(uint256 _pid) external onlyOwner {
        PoolInfo storage pool = poolInfo[_pid];
        pool.unstakeDisabled = false;
    }
    ...
    function harvest(uint256 _pid, bool compound, bool leaveRewards) external {
        ...
        update(_pid);
        if (!(pool.unstakeDisabled)) { disableUnstake(_pid); }
        uint256 NORT = pool.NORT;
        ...
        if (compound && leaveRewards) {
            for (uint256 i; i < NORT; i++) {
                ...
                if (arrayLength > 0 && arrayLength == NORT) {
                    if (i == 0) {
                        uint256 newPending = pending.add(user.nonHarvestedRewards[i]);
                        if (newPending <= pool.maxStake) {
                            pending = pending.add(user.nonHarvestedRewards[i]);
                            user.nonHarvestedRewards[i] = 0;
                        } else {
                            uint256 toMax = pool.maxStake.sub(pending);
                            pending = pending.add(toMax);
                            user.nonHarvestedRewards[i] = user.nonHarvestedRewards[i].sub(toMax);
                        }
                    } else {
                        pending = pending.add(user.nonHarvestedRewards[i]);
                        user.nonHarvestedRewards[i] = 0;
                ...
                if (pending > 0) {
                    ...
                    if (i == 0) {
                        storeUnclaimedReward(_pid, msg.sender, i);
                        stakeFrom(msg.sender, _pid, pending, compound);
                    } else {
                        storeUnclaimedReward(_pid, msg.sender, i);
                    }
                }
    ...
    function reset(uint256 _pid, uint256 startIndex, uint256 endIndex) external onlyOwner {
        PoolInfo storage pool = poolInfo[_pid];
        address[] memory harvestList = pool.harvestList;
        uint256 arrayLength = harvestList.length;
        for (uint256 i = startIndex; i < endIndex; i++) {
            UserInfo storage user = userInfo[_pid][harvestList[i]];
            user.harvested = false;
        }

        address lastArgAddr = harvestList[endIndex - 1];
        address lastHarvester = harvestList[arrayLength - 1];
    ...
    function stakeFrom(address _from, uint256 _pid, uint256 _amount, bool compound) internal {
        ...
        require(_amount <= pool.maxStake, "you cannot stake more than the maximum");
        ...
        if ((pool.unstakeDisabled)) { enableUnstake(_pid); }
    ...
    function storeUnclaimedReward(uint256 _pid, address userAddress, uint256 index) internal {
        ...
        if (arrayLength == 0) {
            ...
            uint256 reward = user.amount * pool.staticRewardsInPool[index];
            uint256 lpSupply = pool.staked;
            uint256 pending = reward.div(lpSupply);
            if (pending > 0) {
                if (index == 0) {
                    user.nonHarvestedRewards[index] = 0;
                } else {
                    user.nonHarvestedRewards[index] = pending;
                }
            }
        }
        if (arrayLength == NORT) {
            ...
            if (pending > 0) {
                if (index == 0) {
                    user.nonHarvestedRewards[index] = user.nonHarvestedRewards[index].add(0);
                } else {
                    user.nonHarvestedRewards[index] = user.nonHarvestedRewards[index].add(pending);
                }
            }
        }
    }
    ...
    /**
     * prevent people from unstaking untill after calling this function
     * if they unstake before this, then reward calculation will be in accurate
     * unstake() is automatically disabled after the first harvest
     * after this calculation is completed, unstake will be enabled by the admin
     * before enabling, make sure to prepare the pool for the next round of staking.
    */
    ...
    function checkIfAlreadyInAPool(address user) public view returns (bool inAnotherPool, uint256 pid) {
