...
contract NFTRaffle is AccessControl, ReentrancyGuard, VRFConsumerBaseV2, ConfirmedOwner {
    ...
    function entry(uint64 _raffleId, uint16 _qty, address _receiptAddress, uint256 _price) internal {
        ...
        if (checkHolder(raffle.holder_1, msg.sender, true) || checkHolder(raffle.holder_2, msg.sender, false) || checkHolder(raffle.holder_3, msg.sender, false)) {
            ...
            IERC20(TokenContract).safeTransferFrom(msg.sender, address(this), _price);
            // token transfer
    ...
    function winnerClaim(uint64 _raffleId) internal nonReentrant {
        RaffleStruct memory raffle = raffleMap[_raffleId];
    ...
    unchecked {
        ...
        uint256 amountRaised = uint256(raffle.price) * raffle.currentSupply * 1e18;
        ...
        if (amountRaised > amountForPlatform) {
            if (contractBalance < amountRaised) revert("Not enough balance");

            IERC20(TokenContract).safeTransfer(config.platformFeeReceiver, amountForPlatform);

            uint256 amountForCreator = amountRaised - amountForPlatform;
            IERC20(TokenContract).safeTransfer(raffle.creator, amountForCreator);
        } else {
            // sales can not cover the platform fee & chainlink fee
            IERC20(TokenContract).safeTransfer(config.platformFeeReceiver, amountRaised);
        }
    }
