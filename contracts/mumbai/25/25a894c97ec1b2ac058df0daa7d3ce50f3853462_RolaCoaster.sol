...
import "./AbstractRolaCoasterUpgradeable.sol";

contract RolaCoaster is AbstractRolaCoasterUpgradeable{
    ...
    // Static ID alloted to ROLA token
    uint8 constant ROLA = 0;

    // Static mint amount for ROLAGRAM
    uint8 constant ROLAGRAM_MINT_AMOUNT = 1;

    // Decimals for the ROLA token
    ...
    function mintRola(address account, uint256 amountRola) external override onlyMaintainer whenNotPaused nonReentrant whenNotBlackListedUser(account) {
        ...
        emit RolaMinted(account, amountRola*(10**rolaDecimals));
    }
    ...
    function initializeMintRolaGram(uint256 amountRola) external override whenNotPaused nonReentrant {
        require(amountRola > 0, "RolaCoaster: ROLA amount should be greater than zero.");
        safeTransferFrom(_msgSender(), treasury, ROLA, amountRola*(10**rolaDecimals), '');
        emit InitializeRolaGramMint(_msgSender(), amountRola*(10**rolaDecimals));
    }
    ...
    function endMintRolaGram(address account, string memory tokenUri) external override onlyMaintainer whenNotPaused nonReentrant whenNotBlackListedUser(account) {
        require(account != ZERO_ADDRESS, "RolaCoaster: Cannot mint RolaGram to Zero Address.");
    ...
    function airdropRola(address[] memory assigned, uint16[] memory quantity) external override onlyOwner whenNotPaused nonReentrant {
        ...
        for (uint8 index = 0; index < assigned.length; index++) {
            if(!_isBlackListUser(assigned[index])){
                _mint(assigned[index], ROLA, quantity[index]*(10**rolaDecimals), "");
            }
        }
    }
...
abstract contract AbstractRolaCoasterUpgradeable is ERC1155SupplyUpgradeable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable, BlackListUpgradeable, IRolaCoasterUpgradeable {
