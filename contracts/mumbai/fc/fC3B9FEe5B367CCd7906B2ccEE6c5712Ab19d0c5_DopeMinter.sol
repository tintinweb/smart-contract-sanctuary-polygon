...
contract DopeMinter is Ownable, ReentrancyGuard {
  ...
  function mint(
    ...
    // send the rest to the DAO address
    (bool sent, ) = payable(daoAddress).call{value: address(this).balance}("");
    require(sent, "Failed to send domain payment to DAO address");
  ...
  /// @notice This changes prices in the minter contract
  ...
  function changeRoyaltyFee(uint256 _royaltyFee) external {
    require(_royaltyFee <= 2000, "Cannot exceed 20%");
