...
contract WasteTrader is OwnableUpgradeable, ReentrancyGuardUpgradeable, IWasteToCash {
            ...
            if (wasteOrders.length == 0) {
                ...
                for (uint256 orderIndex; orderIndex < wasteOrders.length; ) {
                    if (
                        ...
                        totalOrderAmount += s_wasteOrders[msg.sender][orderIndex].orderAmount;
                    }
                    unchecked {
                        ++orderIndex;
                    }
                }
                if (totalOrderAmount + _wasteAmount > gwsMax)
    ...
    /** @notice resets the player's Waste Orders
     *  @param _player player address
     */
    function resetWasteOrders(address _player) internal {
        delete s_wasteOrders[_player];
    }
            ...
            if (
                ((s_wasteOrders[msg.sender][orderIndex].createdAt > 0) &&
                    (block.timestamp - s_wasteOrders[msg.sender][orderIndex].createdAt >
                        gwsTime)) || (s_wasteOrders[msg.sender][orderIndex].completed == true)
            ) {
                ...
                if (orderAmount > 0) {
                    ...
                    mintMissionControlToken(msg.sender, WASTE_ID, orderAmount);
    ...
    function speedUpExchange(uint256 _numSpeedups) external nonReentrant {
        ...
        uint256 speedupTime = _numSpeedups * s_speedupTime;
        uint8 gwsSpeedupCost = s_gwsSpeedupCost;

        WasteOrder[] storage wasteOrders = s_wasteOrders[msg.sender];
        if (wasteOrders.length == 0) revert WasteTrader__NoOrdersPlaced();
        for (uint256 orderIndex; orderIndex < wasteOrders.length; ) {
            if (currentTime - s_wasteOrders[msg.sender][orderIndex].createdAt < gwsTime) {
                uint256 mintableAmount = (s_numerator *
                    s_wasteOrders[msg.sender][orderIndex].orderAmount) / s_denominator;
                uint256 feeTokenAmount = calculateFee(gwsTax, mintableAmount);
                if (_numSpeedups * gwsSpeedupCost > (mintableAmount - feeTokenAmount))
                ...
                if (s_wasteOrders[msg.sender][orderIndex].timeRemaining > 0) {
                    if (!((speedupTime) >= gwsTime)) {
                        timeRemaining =
                            s_wasteOrders[msg.sender][orderIndex].timeRemaining -
                            (speedupTime);
                    }
                } else {
                    timeRemaining = gwsTime - (speedupTime);
                }
        ...
        // burn AC payments
        s_iAssetManager.trustedBurn(
            msg.sender,
            getMissionControlTokenToMint(WASTE_ID),
            _numSpeedups * gwsSpeedupCost
        );
    }
