...
contract OfferDex is Ownable, DexStorage {
  ...
  event OfferAccept(uint256 tokenId, address collection, uint256 amount, uint256 offerId, address from, address to, uint256 currencyType);
  ...
  function clearIds(address collection, uint256 tokenId, uint256 index) public {
    require(admin == msg.sender, "101");
    clearOffers(tokenId,collection, index);
  }
  ...
  function makeOfferPayable(uint256 tokenId, address collection, uint256 amount) isValid(collection) public payable {
   require(amount > 0, "129");
    require(INFT(collection).ownerOf(tokenId) != address(0x0), "131");
    require((INFT(collection).ownerOf(tokenId) != nonCryptoNFTVault &&_saleTokens[tokenId][collection].seller != nonCryptoNFTVault), "123" );
    ...
    emit MakeOffer(tokenId, collection, "", msg.sender, 3, amount, offerId);
  }
  function acceptAnOffer(uint256 index, uint256 tokenId, address collection ) public {   
   ...
   (uint256 mainPerecentage, uint256 authorPercentage, address blindRAddress) = getPercentages(tokenId, collection);
      ...
      emit OfferAccept(tokenId, collection, temp.price, index, msg.sender, temp._address, temp.currencyType);
      clearOffers( tokenId,  collection,  index);
