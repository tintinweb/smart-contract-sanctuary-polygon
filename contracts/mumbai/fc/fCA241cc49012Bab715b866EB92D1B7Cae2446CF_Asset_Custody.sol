...
interface IDelegator{
    ...
    function OnSetText(address _contract, string memory newstr) external;
    event Delegator_Minted(address _contract, address _to, uint256 tokenId);
}
...
contract Asset_Custody is Ownable, IERC721Receiver{
    ...
    event NFTTransfer(address pre_owner, address cur_owner, uint256 tokenId);
    ...
    function SetRealOwner(address _contract, address buyer_addr, uint256 _tokenId) public onlyOwner{
    ...
    function nftMint(address _contract, string memory token_uri) public onlyOwner{
