...
contract GuildID is Modifiers, Initializable, ERC721Upgradeable {
    ...
    function getImageURL() external view returns (string memory) {
        return GuildIDStorage.layout().imageUrl;
    }

    function getMinRankForTransfers() external view returns (uint256) {
        return GuildIDStorage.layout().minRankForTransfers;
    }

    function getMaxRank() external view returns (uint256) {
        return GuildIDStorage.layout().maxRank;
    }
    ...
    function _transferrable(uint256 _tokenId) private view returns (bool) {
        if (!_exists(_tokenId)) revert DoesNotExist();
        return GuildIDStorage.layout().data[_tokenId].rank >= GuildIDStorage.layout().minRankForTransfers;
    }
...
contract Guild is Modifiers {
    ...
    function setTotalXPEarnedPerRank(uint256[] calldata totalXPEarnedPerRank) external onlyAdmin {
        if (totalXPEarnedPerRank.length != GuildIDStorage.layout().maxRank + 1) revert InvalidArrayLength();
        gs.totalXPEarnedPerRank = totalXPEarnedPerRank;
    }

    function setFees(GuildFees memory fees) external onlyAdmin {
        gs.fees = fees;
    }
