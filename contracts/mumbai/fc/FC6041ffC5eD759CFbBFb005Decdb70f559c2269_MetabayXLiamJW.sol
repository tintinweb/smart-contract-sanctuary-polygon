...
contract MetabayXLiamJW is ERC2981, ERC721Enumerable, Ownable, Pausable {
    using Strings for uint;
    IPaperKeyManager paperKeyManager;

    // IERC20 public constant USDC = IERC20(0x4E0068513994fD4526AEc7f558Ee572234AeeFB8); //0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174

    // uint16[1001] public ids;
    // uint16 private index;
    uint256 public immutable price = 0 ether; // 105000000 wei; // USDC 6 decimals, 105 USDC
    uint256 public immutable MAX_SUPPLY = 1000;

    uint256 private _royaltyFee = 500; //5%
    ...
    ) ERC721("Metabay X Liam J. Ward", "MLJW") {
    ...
    modifier mintCompliant(
        ...
        require(totalSupply() + _quantity <= MAX_SUPPLY, "MINT:Max supply exceeded, contact support!");
        _;
    }

    // // Random id logic
    // function _pickRandomUniqueId(uint256 _seed) private returns (uint256 id) {
    //     uint256 len = ids.length - index;
    //     require(len > 0, "No NFTs left");
    //     uint256 randomIndex = _seed % len;
    ...
    function claimTo(
    ...
    ) external payable mintCompliant(_recipient, _quantity) onlyPaper(keccak256(abi.encode(_recipient, _quantity)), _nonce, _signature) whenNotPaused {
        uint256 supply = totalSupply();
        // require(USDC.transferFrom(msg.sender, address(this), price * _quantity), "MINT:Insufficient USDC");
        for (uint256 i = 1; i <= _quantity; i++) {
            ...
            // )));
            // _safeMint(_recipient, _pickRandomUniqueId(seed)+1);
    ...
    function getClaimIneligibilityReason(
        ...
        if ( totalSupply() + _quantity > MAX_SUPPLY ) {
            ...
            return "Contract is paused, sale is over";
        }
        return "";
    }
    ...
    function tokenURI(uint _tokenId) public view virtual override returns (string memory) {
        ...
        return bytes(currentURI).length > 0 ? string(abi.encodePacked(currentURI, _tokenId.toString(), ".json")) : "";
    }
    ...
    function royaltyInfo(uint256 _tokenId, uint256 _salePrice) public override view returns (
        ...
        return (owner(), (_salePrice * _royaltyFee) / 10000);
    }

    function pauseSale(bool pause) onlyOwner public {
        if (pause) _pause();
        else _unpause();
    }
    ...
    // Set tranferable if not soldout
    ...
    function _beforeTokenTransfer(
        ...
        if ( totalSupply() + batchSize == MAX_SUPPLY ) {
            soldOut();
        }
