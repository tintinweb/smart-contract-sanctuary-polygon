...
contract DavaParts is PartCollection {
    function name() public pure returns (string memory) {
        return "dava-wearables";
    }
}
    ...
    UUPSUpgradeable
{
    ...
    string public davaApiURI;

    PartInfo private _partInfo;
    CollectionInfo private _collectionInfo;

    uint256 public override numberOfParts;
    EnumerableSetUpgradeable.Bytes32Set private _supportedCategoryIds;

    event PartCreated(uint256 partId);
    event PartUpdated(uint256 partId);
    event SetIpfsBaseURI(string ipfsBaseURI);
    event SetDavaApiURI(string davaApiURI);

    /// @custom:oz-upgrades-unsafe-allow constructor
    constructor() initializer {}

    function initialize(address dava_) public initializer {
        ...
        davaApiURI = "https://api.davaproject.com";
    ...
    function setIpfsBaseURI(string calldata ipfsBaseURI_)
        external
        onlyRole(OPERATOR_ROLE)
    {
        ipfsBaseURI = ipfsBaseURI_;
        emit SetIpfsBaseURI(ipfsBaseURI_);
    }

    function setDavaApiURI(string calldata davaApiURI_)
        external
        onlyRole(OPERATOR_ROLE)
    {
        davaApiURI = davaApiURI_;
        emit SetDavaApiURI(davaApiURI_);
    }
    ...
    function createPart(
        ...
        if (categoryId_ == DEFAULT_CATEGORY) {
            require(maxSupply_ == 0, "Part: maxSupply should be zero");
        } else {
            require(
                maxSupply_ != 0,
    ...
    function updatePart(
        ...
        Attribute[] memory attributes,
        uint256 maxSupply_
    ) public virtual onlyRole(CREATOR_ROLE) {
        require(tokenId_ < numberOfParts, "PartCollection: non existent part");

        if (bytes(title_).length > 0) {
            _partInfo.titles[tokenId_] = title_;
        }
        if (bytes(description_).length > 0) {
            _partInfo.descriptions[tokenId_] = description_;
        }
        if (bytes(ipfsHash_).length > 0) {
            _partInfo.ipfsHashes[tokenId_] = ipfsHash_;
        }
        if (maxSupply_ > 0) {
            _partInfo.maxSupply[tokenId_] = maxSupply_;
        }

        if (attributes.length > 0) {
            delete _partInfo.attributes[tokenId_];
            for (uint256 i = 0; i < attributes.length; i += 1) {
                _partInfo.attributes[tokenId_].push(attributes[i]);
            }
        }

        emit PartUpdated(tokenId_);
    }
    ...
    function uri(uint256 tokenId) public view override returns (string memory) {
        ...
        string memory shortenedAddress = (uint256(uint160(address(this))) >>
            (8 * 16)).toHexString(4);
        string[] memory imgParams = new string[](1);
        imgParams[0] = "v1/images";
        string[] memory infoParams = new string[](3);
        infoParams[0] = "v1/info";
        infoParams[1] = uint256(uint160(address(this))).toHexString(20);
        ...
        queries[1] = URICompiler.Query(
            "z1",
            string(
                abi.encodePacked(shortenedAddress, "[", tokenId.toString(), "]")
            )
        );
        ...
        return
            OnchainMetadata.toMetadata(
                ...
                imgURIs,
                URICompiler.getFullUri(davaApiURI, imgParams, queries),
        ...
        )
    {
        ...
        if (IDavaV2(dava).isAvatar(from)) {
            IDavaV2(dava).updateUniqueness(from);
        }
    }
...
library OnchainMetadata {
    ...
    function toMetadata(
        ...
        bytes memory prefix = abi.encodePacked('data:application/json;base64,');
        ...
        string memory base64MetaData= Base64Upgradeable.encode(metadata);

        return _append(string(prefix), base64MetaData);
    }
