...
contract Finish is IExecutePlugin, BasePlugin {
    using MakeTokenId for address;
    ISoulBound public soulBound;

    constructor(address _registry) {
        PLUGIN_VERSION = 1;
        PLUGIN_NAME = PluginsList.SAFE_DEAL_FINISH;
    ...
    function execute(
        ...
        ISafeDeal safeDeal = ISafeDeal(registry.safeDeal());

        require(safeDeal.finishDeal(vars),
            "Finish: wrong exec"
        );

        DataTypes.MinSimpleVars memory readingVars;
        readingVars.pluginName = PLUGIN_NAME;
        readingVars.version = PLUGIN_VERSION;
        readingVars.data = _data;
        DataTypes.SafeDeal memory dealInfo = safeDeal.readDeal(readingVars);

        makeMint(_executedId, dealInfo.guarantor, uint256(DataTypes.UserRatesType.FOR_DEAL_GUARANTOR));
        makeMint(_executedId, dealInfo.seller, uint256(DataTypes.UserRatesType.FOR_DEAL_SELLER));
        makeMint(_executedId, dealInfo.buyer, uint256(DataTypes.UserRatesType.FOR_DEAL_BUYER));
