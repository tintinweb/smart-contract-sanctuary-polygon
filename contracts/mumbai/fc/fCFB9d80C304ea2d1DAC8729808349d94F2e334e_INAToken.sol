...
contract INAToken is IINAToken, ERC20, ERC20Burnable, AccessControl, Pausable, ERC20Permit, ReentrancyGuard  {    
    ...
    public 
    view returns (uint256 usdPrice, bytes32 salingId) {
    ...
    internal {
        (uint256 usdPrice, bytes32 salingId) = getTokenPrice();
        require(usdPrice > 0, "INA token isn't saling");

        if (_payType == PayType.payMatic) {
            (, int price, , , ) = _feedMaticUsd.latestRoundData();
            uint256 maticPrice = _amount * usdPrice / uint256(price) / 10 ** (decimals() - _feedMaticUsd.decimals());
            ...
            uint256 usdtPrice = _amount * usdPrice / uint256(price) / 10 ** (decimals() - _feedUsdtUsd.decimals());            
            require(
            ...
            uint256 wethPrice = _amount * usdPrice / uint256(price) / 10 ** (decimals() - _feedEthUsd.decimals());            
            require(
...
interface IINAToken {
    event CreatedSale(bytes32 ssId);
    event CreatedVest(bytes32 vsId);
}
