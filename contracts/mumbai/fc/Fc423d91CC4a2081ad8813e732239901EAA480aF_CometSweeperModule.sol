...
contract CometSweeperModule is WithStorageComet {
    ...
    function withdrawAndSwapAllIn(AllInputMoneyMarketMultiParams memory params) external returns (uint256 amountOut) {
        ...
        amountOut = IMinimalSwapRouter(router).exactInput(
            ExactInputParams({path: params.path, amountIn: amountIn, recipient: params.recipient})
        );
    ...
    function swapETHAndRepayAllOut(AllOutputMultiParamsBase calldata params) external payable returns (uint256 amountIn) {
        ...
        require(amountIn <= params.amountInMaximum, "hd to pay too much");
    ...
    function swapCollateralAllIn(AllInputSingleParamsBase calldata params) external returns (uint256 amountOut) {
        MarginCallbackData memory data = MarginCallbackData({
        ...
        uint256 amountIn = IComet(cos().comet[params.cometId]).borrowBalanceOf(msg.sender);
        ...
        require(amountOut >= params.amountOutMinimum, "Withdrew too much");
    }

    // swaps the collateral from one token (tokenIn) to another (tokenOut) provided tokenOut amount
    function swapCollateralAllInMulti(AllInputMoneyMarketMultiParams calldata params) external returns (uint256 amountOut) {
    ...
    function trimMarginPositionAllIn(MarginSwapParamsAllIn calldata params) external returns (uint256 amountOut) {
        MarginCallbackData memory data = MarginCallbackData({
            path: abi.encodePacked(params.tokenIn, params.fee, params.tokenOut),
            tradeType: 7,
            user: msg.sender,
    ...
    function trimMarginPositionAllOut(MarginSwapParamsAllOut calldata params) external returns (uint256 amountIn) {
        MarginCallbackData memory data = MarginCallbackData({
            path: abi.encodePacked(params.tokenIn, params.fee, params.tokenOut),
            tradeType: 7,
            user: msg.sender,
        ...
        require(amountIn <= params.amountInMaximum, "Had to withdraw too much");
    }

    // decrease the margin position - use the collateral (tokenIn) to pay back a borrow (tokenOut)
    function trimMarginPositionAllInMulti(AllInputMultiParamsBase calldata params) external returns (uint256 amountOut) {
        (address tokenIn, address tokenOut, uint24 fee) = params.path.decodeFirstPool();
    ...
    function trimMarginPositionAllOutMulti(AllOutputMultiParamsBase calldata params) external returns (uint256 amountIn) {
        (address tokenOut, address tokenIn, uint24 fee) = params.path.decodeFirstPool();
        ...
        // it's technically possible to not receive the full output amount,
        // so if no price limit has been specified, require this possibility away
        require((zeroForOne ? uint256(-amount1) : uint256(-amount0)) == amountOut);
