...
contract Domains is ERC721URIStorage, Ownable {
    ...
    uint256 public domainLengthPriceUpperBound = 7;
    uint256 public maxDomainLength = 12;
    ...
    function setMaxLeaseTime(uint256 newLeaseTime) external onlyOwner {
        if (newLeaseTime < 182 days || newLeaseTime > 730 days) {
            revert InvalidLeaseTime();
        }
        ...
        private
    {
        ...
        string memory svg = string.concat(svgStart, domainName, svgEnd);
        _setTokenURI(tokenId, getJsonUri(domainName, svg));
