...
contract MLDStaking is Ownable {
    ...
    function stake(uint256 _mldAmount, uint256 _stakeDurationInSec) external {
        ...
        uint maturityTime = block.timestamp + _stakeDurationInSec;
        ...
        emit Staked(msg.sender, _mldAmount, block.timestamp, maturityTime);
    }
    ...
    function withdraw(uint256 _index) external {
        ...
        DepositInfo memory userDepositInfo = userDeposits[user][_index];

        uint256 mldStaked = userDepositInfo.mldStaked;
        uint256 lordStaked = userDepositInfo.lordStaked;

        totalMLDLocked -= mldStaked;
        userTotalDeposits[user].totalMLDStaked -= mldStaked;
        userTotalDeposits[user].totalLordStaked -= lordStaked;

        uint lastIndex = userDeposits[user].length - 1;
        userDeposits[user][_index] = userDeposits[user][lastIndex];
        ...
        if (block.timestamp >= userDepositInfo.maturity) {
            ...
            uint256 lordFees = (lordStaked * unstakeLordFeesPercentage) / 100;

            mldToken.transfer(user, returnAmount);
            lordToken.burn(lordFees);
            lordToken.transfer(user, lordStaked - lordFees);
            ...
            uint256 mldPenalty = (mldStaked * preMatureMLDPenalty) / 100;
            uint256 withdrawableAmount = mldStaked - mldPenalty;

            mldToken.transfer(user, withdrawableAmount);
            mldToken.transfer(feeReciever, mldPenalty);
            lordToken.burn(lordStaked);
