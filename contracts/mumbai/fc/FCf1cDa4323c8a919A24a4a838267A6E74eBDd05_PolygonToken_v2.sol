...
abstract contract Rescuer is Pauser  {
    ...
    event Rescue(address indexed tokenAddr, address indexed toAddr, uint256 amount);
    ...
    function rescue(IERC20 _tokenContract, address _to, uint256 _amount) public whenNotPaused onlyRescuer {
        ...
        emit Rescue(address(_tokenContract), _to, _amount);
    }
}
...
contract PolygonToken_v2 is PolygonToken_v1, Owner {
    ...
    function updateMetadata(string calldata _newName, string calldata _newSymbol)
        external
        onlyRescuer
    {
        setName(_newName);
        setSymbol(_newSymbol);
        _setDomainSeperator(_newName, ERC712_VERSION);
    }
    ...
    function permit(
        ...
        require(_owner == ecrecover(digest, v, r, s), "INVALID-PERMIT");
        require(now <= _deadline, "PERMIT-EXPIRED");

        _approve(_owner, _spender, _value);
    }

    function transfer(address _recipient, uint256 _amount) whenNotPaused onlyNotProhibited(_msgSender()) onlyNotProhibited(_recipient) isNaturalNumber(_amount) public virtual override returns (bool) {
        _transfer(_msgSender(), _recipient, _amount);
        return true;
    }

    function transferFrom(address _sender, address _recipient, uint256 _amount) whenNotPaused onlyNotProhibited(_sender) onlyNotProhibited(_recipient) isNaturalNumber(_amount) public virtual override returns (bool) {
        return super.transferFrom(_sender, _recipient, _amount);
    }

    function rescueNative(address _to, uint256 _amount) public whenNotPaused onlyRescuer {
        _transfer(address(this), _to, _amount);
        emit Rescue(address(this), _to, _amount);
    }
    ...
    function withdraw(uint256 amount) external override{
        _burn(_msgSender(), amount);
        emit Burn(_msgSender(), amount, _msgSender());
    }
}