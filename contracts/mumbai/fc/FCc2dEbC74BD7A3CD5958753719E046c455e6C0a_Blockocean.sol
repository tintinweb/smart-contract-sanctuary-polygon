...
contract Blockocean {
    ...
    function _buySlot(address _user, uint256 _slot) private {
        ...
        if(_slot > 1 && user.directTeam >= GPDirectRequired) _updateAP4x2(_user, _slot);
        user.slots += 1;
        ...
        if(user.mainUpline != address(0) && userInfo[user.mainUpline].slots >= user.slots) {
            ...
            if(rewardInfo[user.mainUpline].serial[_slot - 1] == 0) {
                ...
                curPercent = refPercent;
            } else if(rewardInfo[user.mainUpline].serial[_slot - 1] > 1 && rewardInfo[user.mainUpline].selfUpgrade < slots[_slot] && user.slots == userInfo[user.mainUpline].slots && _slot < 7) {
    ...
    function calculateLayer4x2(uint256 _slot) public view returns (uint256 layer, uint256 start, uint256 end) {
        uint256 currentUsers = 1;
        uint256 prevUsers = 0;
        while (currentUsers < globalpool4x2[_slot].length) {
    ...
    function calculateLayer3x3(uint256 _slot) public view returns (uint256 layer, uint256 start, uint256 end) {
        uint256 currentUsers = 1;
        uint256 prevUsers = 0;
        while (currentUsers < globalpool3x3[_slot].length) {
            prevUsers = currentUsers;
            currentUsers *= 3;
            layer++;
        }
