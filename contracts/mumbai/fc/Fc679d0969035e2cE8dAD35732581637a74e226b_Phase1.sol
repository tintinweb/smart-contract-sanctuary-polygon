...
contract Phase1 is ReentrancyGuard {
	...
	address private _presaleMainContract;
	...
	function setPresaleInstance(uint startTime, uint duration, uint price, uint _qtyPerEth, uint supply) public onlyOwner(msg.sender) {
		_supply = supply;
		_presale = AkxPresale(_presaleMainContract);
		_presale.setPhase1(startTime, duration, price, _qtyPerEth, supply);
	}
