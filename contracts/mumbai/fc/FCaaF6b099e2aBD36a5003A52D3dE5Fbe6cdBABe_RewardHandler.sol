...
//    mapping (address => uint256) public lpTokenBalances;
    // Assigned but still unclaimed rewards per user.
    mapping (address => uint256) public claimableRewards;
    // The balance for when rewards we last assigned, per user.
    mapping (address => uint256) public assignedCumulativeRewards;
...
*/
        ...
        returns(uint256 amount)
    {
        ...
        uint256 _cumulativeRewards = cumulativeRewards;
        uint256 supply = distToken.totalSupply();

        // cumulativeRewards can only increase, so if _cumulativeRewards
        // is zero, it means there are no rewards yet.
        // If supply is 0, there is nothing to distribute yet.
        if (_cumulativeRewards == 0 || supply == 0) {
            return 0;
        }

        uint256 addedRewards =
            _cumulativeRewards.sub(assignedCumulativeRewards[account]);
        if (addedRewards > 0) {
            uint256 lpTokenBalance = distToken.balanceOf(account);
            if (lpTokenBalance > 0) {
                uint256 addedAccountReward =
                    lpTokenBalance.mul(addedRewards).div(supply);
    ...
    function _updateClaimableRewards(address account) private {
        ...
        if (addedRewards > 0) {
            ...
            assignedCumulativeRewards[account] = _cumulativeRewards;
        }
    }
