    ...
    OwnableUpgradeable
{
    ...
    mapping(uint256 => uint256) public lockupBonus;
    ...
    AggregatorV3Interface internal oracleInterface;
    // https://docs.chain.link/docs/ethereum-addresses/ => (MATIC / USD)
    ...
    function initialize(address _usdt, address _oracle) public initializer {
        ...
        oracleInterface = AggregatorV3Interface(_oracle);
        ...
        tokenQuantityBonus = [
            ...
            [500, 1000, 1200, 1500, 2000, 2200, 2500]
        ];
    ...
    modifier checkSaleState(uint256 amount, address _msgSender) {
        ...
        require(
            maxTotalSold >= amount + totalTokensSoldInStage,
            "There are no free tokens for sale"
        );
        require(
            !saleAccessOn || isSaleAccess[_msgSender],
            "Sender not in sale access list"
        );
        _;
    }
    ...
    function deposits(
        address user
    ) external view returns (UserDeposits[] memory) {
     ...
     * Para verificar la cantidad total de tokens de bonificación que el usuario obtendrá por una cantidad particular y meses bloqueados
     ...
     * Para verificar la cantidad de tokens de bonificación que el usuario obtendrá por una cantidad particular y meses bloqueados
     * @param amount amount of tokens to be locked
     * @param lockup_months number of months tokens will be locked
     */
    function checkLockupBonus(
        uint256 amount,
        uint256 lockup_months
    ) public view returns (uint256 bonus, uint256 timeLockedFor) {
        if (lockupBonus[lockup_months] == 0) {
            return (0, 0);
        } else {
            bonus = ((amount * decimals) * lockupBonus[lockup_months]) / 10000;
     ...
     * Para verificar la cantidad de tokens de bonificación que el usuario obtendrá por una cantidad particular comprada
     * comparado con la cantidad equivalente de tokens en USDT
     * @param amount amount of tokens to be locked
     */
    function checkTokenQuantityBonus(
        uint256 amount
    ) public view returns (uint256) {
        uint256 price = (amount * stagePrice) / decimals;
        if (price < tokenQuantityBonus[0][0]) return 0;
        if (price >= tokenQuantityBonus[0][tokenQuantityBonus[0].length - 1])
            return
                ((amount * decimals) *
                    tokenQuantityBonus[1][tokenQuantityBonus[1].length - 1]) /
                10000;

        uint256 bonus;

        for (uint256 i = 0; i < (tokenQuantityBonus[0].length); i++) {
            if (price < tokenQuantityBonus[0][i]) {
                bonus = (tokenQuantityBonus[1][i - 1]);
                break;
            } else if (price == tokenQuantityBonus[0][i]) {
                bonus = (tokenQuantityBonus[1][i]);
                break;
            }
        }

        return ((amount * decimals) * bonus) / 10000;
    }
    ...
    function ethBuyHelper(
        ...
        ethAmount = (usdPrice * decimals) / getLatestPrice();
    }
    ...
    function buyWithUSDT(
        ...
        external
        checkSaleState(amount, _msgSender())
        whenNotPaused
        returns (bool)
    {
    ...
    function update(
        ...
        (uint256 lockup_bonus, uint256 time) = checkLockupBonus(
            amount,
            lockup_months
        );
        ...
        uint256 newBonus = quantityBonus + lockup_bonus;
        ...
        userDeposits[_user].push(
            UserDeposits(
                amount * decimals,
                newBonus,
                (((amount * decimals) + newBonus) * initialClaimPercent) /
                    10000,
                0,
                time
            )
     ...
     */
    function updateInvestmentBonus(
        ...
        tokenQuantityBonus = _tokenQuantity;
    }
    ...
    function updateLockUpBonus(uint256[][] memory _lockup) public onlyOwner {
        ...
        for (uint256 i; i < _lockup[0].length; i++) {
            lockupBonus[_lockup[0][i]] = _lockup[1][i];
        }
    }
    ...
    function switchSaleAccessAddresses(
    ...
    function startClaim(
        ...
        require(
            noOfTokens >= (totalTokensSold * decimals) + totalBonus,
    ...
    function claim(uint256 _id) public whenNotPaused returns (bool) {
        ...
        if (!newUser[_msgSender()][_id]) {
            userDeposits[_msgSender()][_id].claimedAmount +=
                tokens -
                ((tokens * 10) / 100);
        } else {
