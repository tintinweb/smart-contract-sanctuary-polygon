...
contract Ghost is IFlashLoanReceiver {
  ...
  function executeOperation(
    ...
    (address _sender, address _recipient, BorrowToken[] memory _DebtTokenBalance, SupplyToken[] memory _aTokenBalance) = abi.decode(params, (address, address, BorrowToken[], SupplyToken[]));
    ...
    for(uint i=0; i<_DebtTokenBalance.length; i++) {
      ...
      if (_DebtTokenBalance[i].stableDebtTokenBalance != 0) {
        IERC20(_DebtTokenBalance[i].tokenAddress).approve(address(AaveV3Mumbai.POOL), _DebtTokenBalance[i].stableDebtTokenBalance);
        AaveV3Mumbai.POOL.repay(_DebtTokenBalance[i].tokenAddress, _DebtTokenBalance[i].stableDebtTokenBalance, 1, _sender);
      }
      // Repay VariableDebt
      if (_DebtTokenBalance[i].variableDebtTokenBalance != 0) {
        IERC20(_DebtTokenBalance[i].tokenAddress).approve(address(AaveV3Mumbai.POOL), _DebtTokenBalance[i].variableDebtTokenBalance);
        AaveV3Mumbai.POOL.repay(_DebtTokenBalance[i].tokenAddress, _DebtTokenBalance[i].variableDebtTokenBalance, 2, _sender);
      }
    }
  ...
  function merge(
    ...
    BorrowToken[] memory _borrowToken,
    SupplyToken[] memory _supplyToken
  ) external {
    require(_recipientAccount != msg.sender, "Invalid recipient address");
    bytes memory params = abi.encode(msg.sender, _recipientAccount, _borrowToken, _supplyToken);
    address[] memory assets = new address[](_borrowToken.length);
    uint256[] memory amounts = new uint256[](_borrowToken.length);

    // Flash Loan all the borrowed assets
    for (uint i=0; i<_borrowToken.length; i++) {
      assets[i] = _borrowToken[i].tokenAddress;
      amounts[i] = (_borrowToken[i].stableDebtTokenBalance).add(_borrowToken[i].variableDebtTokenBalance);
    }
    _flashloan(_recipientAccount, assets, amounts, params);
  }

  function _flashloan(address recipient, address[] memory assets, uint256[] memory amounts, bytes memory _params)
  internal
  {
