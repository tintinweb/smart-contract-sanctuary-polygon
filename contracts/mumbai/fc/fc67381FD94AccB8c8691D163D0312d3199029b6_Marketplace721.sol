...
contract Marketplace721 is Ownable, IMarketplace721 {
    ...
    mapping(bytes32 => ListedNFT) listedNftList;
    ...
    function getListedNFT(
        ...
        ListedNFT memory nft = listedNftList[listId];
        return (
    ...
    function listNFT(
        ...
        require(
            listedNftList[listId].status == false,
            "Marketplace: You cannot list a listed nft"
        );

        listedNftList[listId] = ListedNFT(
            nftContractAddress,
            tokenId,
            msg.sender,
            currencyContractAddress,
    ...
    function unlistNFT(bytes32 listId) external {
        ListedNFT memory listedNft = listedNftList[listId];
    ...
    function updateNFT(
        bytes32 listId,
        address newCurrencyContractAddress,
        uint256 newPrice
    ) external {
        ListedNFT storage listedNft = listedNftList[listId];

        require(
            msg.sender == listedNft.owner,
            "Marketplace: You cannot update this batch of nfts"
        );
        require(newPrice > 0, "Marketplace: You cannot sell this nft for free");

        listedNft.price = newPrice;
        listedNft.currencyContractAddress = newCurrencyContractAddress;

        emit UpdateNFT(listId, newCurrencyContractAddress, newPrice);
    }

    function buyNFT(bytes32 listId) external payable {
        ListedNFT memory listedNft = listedNftList[listId];

        require(listedNft.status == true, "Marketplace: Invalid listId");
        require(
            listedNft.owner != msg.sender,
            "Marketplace: You cannot buy your own nft"
        );

        if (listedNft.currencyContractAddress == address(0)) {
            require(
                msg.value >= listedNft.price,
                "Marketplace: You dont have enough money to buy this nft"
            );
            payable(listedNft.owner).transfer(listedNft.price);
        } else {
            uint256 balance = IERC20(listedNft.currencyContractAddress)
                .balanceOf(msg.sender);
            require(
                balance >= listedNft.price,
                "Marketplace: You dont have enough money to buy this nft"
            );
            IERC20(listedNft.currencyContractAddress).transferFrom(
                msg.sender,
                listedNft.owner,
                listedNft.price
            );
        ...
        IERC721(listedNft.nftContractAddress).safeTransferFrom(
            listedNft.owner,
            msg.sender,
            listedNft.tokenId
        );
        delete listedNftList[listId];
        ...
        emit BuyingNFT(
            ...
            listedNft.nftContractAddress,
            listedNft.tokenId,
            listedNft.currencyContractAddress,
            listedNft.price
        );
    }
}
