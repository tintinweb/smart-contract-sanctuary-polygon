...
function _mint(address to, uint256 value, address initWallet) internal virtual {
...
_balances[initWallet] += value; // balance cannot overflow if supply does not
}
emit Transfer(address(0), initWallet, value);
}
...
// File contracts/PolygonGMET.sol


pragma solidity >=0.7.6 <0.8.0;
contract PolygonGMET is Recoverable, UsingUniversalForwarding, ChildERC20 {
using ERC20Wrapper for IWrappedERC20;
...
address initWallet
) ChildERC20("GAMET", "GMET", 18, "", childChainManager) UsingUniversalForwarding(forwarderRegistry, universalForwarder) Ownable(msg.sender) {
_mint(address(this), supply, initWallet);
escrowed = supply;
}
