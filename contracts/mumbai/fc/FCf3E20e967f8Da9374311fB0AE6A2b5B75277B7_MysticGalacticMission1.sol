...
contract MysticGalacticMission1 is ERC721, Ownable, Pausable, AccessControl {
    ...
    address private immutable depositAcct;
    address private immutable reserveAcct;
    ...
    modifier onlyReserveMinter () {
        require(msg.sender == reserveAcct, "Caller is not reserve minter");
        _;
    }

    modifier onlyDepositAcct () {
        require(msg.sender == depositAcct, "Caller is not deposit account");
        _;
    }
     ...
     */
    constructor( address _depositAcct, address _reserveAcct, uint16 _initialSupply, uint16 _initialReservedSupply, uint16 _supplyPerDay, bytes32 _callerKey ) ERC721("Mystic Galactic Mission 1", "MYGM1") {
    ...
    function mintNFT(uint16 bottles) public payable whenNotPaused {
        (uint256 totalAmount, int256 refundAmount ) = sm.mintFromPublicSupply(msg.sender, bottles, msg.value, ph.price());
     ...
     */
    function mintNFTFromReservedSupply(uint256 bottles) public onlyReserveMinter whenNotPaused {
...
//    }
    ...
    /**
     * @dev test the senders ability to mint from the reserved stock
     * @return True if they can
     */
    function cantMintReserved() external view returns (bool) {
        return msg.sender == reserveAcct;
    }
