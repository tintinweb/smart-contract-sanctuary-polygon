...
contract TurtleShellToken is CustomERC4671, EIP712, Ownable {
	...
	/**
	 * @notice Event emitted when new Audit Badge is minted
	 * @param initiator is the address that initiated the mint
	 * @param contractAddress is the contract address having received the NFT
	 * @param tokenURI is the token URI of the new NFT
	 */
	event Mint(address indexed initiator, address indexed contractAddress, string indexed tokenURI);
	...
	function mint(MintRequest calldata mintRequest, bytes calldata signature) external {
		...
		emit Mint(msg.sender, mintRequest.to, mintRequest.tokenURI);
	}
