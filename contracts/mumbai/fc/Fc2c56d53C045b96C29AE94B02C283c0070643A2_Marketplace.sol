    ...
    IERC1155ReceiverUpgradeable
{
    ...
    /*///////////////////////////////////////////////////////////////
                    Read Only Mappings For Listings
    //////////////////////////////////////////////////////////////*/

    // Mapping to track the number of listings owned by each address
    mapping(address => uint256) public _ownerListingsBal;

    // Mapping to store the listings owned by each address
    mapping(address => mapping(uint256 => uint256)) public _ownedListings;

    // Mapping to store the index of each listing ID in the _ownedListings mapping for a particular address
    mapping(uint256 => uint256) private _ownedListingIndex;

    // function listingBalanceOfOwner(
    //     address owner
    // ) public view returns (uint256) {
    //     require(owner != address(0), "address zero is not a valid owner");
    //     return _ownerListingsBal[owner];
    // }

    function listingOfOwnerByIndex(
        address owner,
        uint256 index
    ) public view returns (uint256) {
        require(index < _ownerListingsBal[owner], "owner index out of bounds");
        return _ownedListings[owner][index];
    }

    function _addListingToOwnerEnumeration(uint256 listingID) internal {
        uint256 currentListings = _ownerListingsBal[msg.sender];
        _ownedListings[msg.sender][currentListings] = listingID;
        _ownedListingIndex[listingID] = currentListings;
        _ownerListingsBal[msg.sender]++;
    }

    // function _removeListingFromOwnerEnumerationForUser(
    //     uint256 listingId,
    //     address User
    // ) internal {
    //     uint256 indexToRemove = _ownedListingIndex[listingId];
    //     require(
    //         _ownedListings[User][indexToRemove] == listingId,
    //         "Listing not found"
    //     );

    //     uint256 lastListingId = _ownedListings[User][
    //         _ownerListingsBal[User] - 1
    //     ];
    //     _ownedListings[User][indexToRemove] = lastListingId;
    //     _ownedListingIndex[lastListingId] = indexToRemove;

    //     delete _ownedListings[User][_ownerListingsBal[User] - 1];
    //     delete _ownedListingIndex[listingId];

    //     _ownerListingsBal[User]--;
    // }

    // function _removeListingFromOwnerEnumeration(
    //     uint256 listingId,
    //     address User
    // ) internal {
    //     uint256 indexToRemove = _ownedListingIndex[listingId];
    ...
    function _removeListingFromOwnerEnumeration(
        ...
        uint256 indexToRemove = _ownedListingIndex[listingId];
        require(indexToRemove < _ownerListingsBal[user], "Listing not found");
    ...
    function listingsOfOwner(
        address owner
    ) external view returns (Listing[] memory) {
        uint256 listingCount = _ownerListingsBal[owner];
        if (listingCount == 0) {
            return new Listing[](0);
        } else {
            Listing[] memory result = new Listing[](listingCount);
            for (uint256 index; index < listingCount; index++) {
                result[index] = getListings(
                    listingOfOwnerByIndex(owner, index)
                );
            }
            return result;
        }
    ...
    /*///////////////////////////////////////////////////////////////
                    No Direct Listing Can be created Twice For 1 NFT
    //////////////////////////////////////////////////////////////*/
    // owner => contractAddress => TokenId =>Bool
    mapping(address => mapping(address => mapping(uint256 => bool))) _ListingExists;

    // mapping(address => mapping(address => mapping(uint256 => bool))) _ListingExists1155;

    // Check if a listing with the same asset contract and token ID already exists
    function isListingExists(
        address assetContract,
        uint256 tokenId
    ) internal view returns (bool) {
        return _ListingExists[_msgSender()][assetContract][tokenId];
    }

    function _addtoListingExists(
        address assetContract,
        uint256 tokenId
    ) internal {
        _ListingExists[_msgSender()][assetContract][tokenId] = true;
    }

    function _removefromListingExists(
        address assetContract,
        uint256 tokenId
    ) internal {
        delete _ListingExists[_msgSender()][assetContract][tokenId];
    }
    ...
    function _removefromListingExists(
        ...
        delete _ListingExists[Owner][assetContract][tokenId];
    }

    /*///////////////////////////////////////////////////////////////
                Get All Offerer's Offers                   
    //////////////////////////////////////////////////////////////*/
    // listing ID => Account => Offer
    // mapping(uint256 => mapping(address => Offer)) public offers;
    // uint256 _OfferId;

    // // Mapping to track the number of Offers made by each address
    // mapping(address => uint256) public _accountOfferBal;

    // // Mapping to store the listings owned by each address
    // mapping(address => mapping(uint256 => uint256)) public _accountOffers;

    // // Mapping to store the index of each listing ID in the _ownedListings mapping for a particular address
    // mapping(uint256 => uint256) private _accountOfferIndex;
    ...
    function createListing(ListingParameters memory _params) external override {
        ...
        // Check if a listing with the same asset contract and token ID already exists
        require(
            !isListingExists(_params.assetContract, _params.tokenId),
            "DUPLICATE"
        );
        _addtoListingExists(_params.assetContract, _params.tokenId);
        /* -------------------------------------------------------------------------- */
        ...
        /* -------------------------------------------------------------------------- */
        /*                                new Addition                                */
        _addListingToOwnerEnumeration(listingId);
        /* -------------------------------------------------------------------------- */
    ...
    function cancelDirectListing(
        ...
        /* -------------------------------------------------------------------------- */
        /*                                new Addition                                */
        _removeListingFromOwnerEnumeration(
            targetListing.listingId,
            _msgSender()
        );

        require(
            isListingExists(targetListing.assetContract, targetListing.tokenId),
            "Non Existent Listing"
        );

        _removefromListingExists(
            targetListing.assetContract,
            targetListing.tokenId
        );
...
// TODO:
// Offers Emunerable... 1 listing ... Offers , Offers List...
// 1 Listing...
// mapping contract tokenId

// User -> Lisitings enumerable.
// user->3
// address-> 54,54,66

// Valid Listings

// ** ✅ user jo list krega uski sari listings ajaen ek call se
// ** user k pass jo offers ayengi wo sari ajaen per nft
// ** offeror jo offer dega usko offers show hon
// ** ✅ No Listing Can be created Twice For 1 NFT
// TODO:
// TEST OF MATIC,, Remove its Support.
// Check For Multicall
// two offers for SAME LISTING?

// TEST
// DIrect listing ... then recreate... complete listing ... New listing from new Onwer ... Sell To old and old try to relist