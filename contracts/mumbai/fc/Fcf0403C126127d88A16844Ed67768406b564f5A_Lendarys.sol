...
contract Lendarys is Ownable, ERC721Enumerable {
        ...
        payable
    {
        ...
        Variables memory vBuffer = v;
        require(ts + amount <= vBuffer.wlMaxSupply, "Exceed available supply");
        require(
    ...
    function publicSaleMint(uint256 amount) external payable {
        ...
        require(ts + amount <= vBuffer.maxSupply, "Exceed available supply");
        require(msg.value >= vBuffer.regularPrice * amount, "Not enough funds");
        mint(msg.sender, amount, ts);
    }
    ...
    function setMaxSupply(uint16 maxSupply) external onlyOwner {
        require(totalSupply() <= v.maxSupply, "Max supply below ts");
        v.maxSupply = maxSupply;
    }

    function setWlDate(uint64 wlDate) external onlyOwner {
        v.wlDate = wlDate;
    }

    function setRaffleDate(uint64 date) external onlyOwner {
        v.raffleDate = date;
    }
