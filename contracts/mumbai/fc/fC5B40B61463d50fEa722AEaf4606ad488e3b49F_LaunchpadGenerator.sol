 ...
 * creation charges a fee in GLMR which is defined in the launchpad settings.
 */
...
contract LaunchpadGenerator {
     ...
     *@param _wglmr address of the wrapped glmr contract
     ...
     * @dev requires to receive the creation fee in GLMR (defined in settings)
...
/**
 * @title Interface of a wrapped GLMR token
 * @dev This interface describes a wrapping ERC20 token for native GLMR currency
 */
interface IWGLMR {
    /**
...
interface ILaunchpadSettings {
    ...
    /**
     * @notice returns the absolute fee in GLMR for launchpad creation
     */
...
contract Launchpad is ReentrancyGuard {
    ...
    struct LaunchpadInfo {
        ...
        uint256 startTime; // the timestamp to start the launchpad
        uint256 endTime; // the timestamp to end the launchpad (max length of settings contract must be met)
        bool isGLMR; // if the base token is native currency
    }
    ...
    function init2(
        ...
        launchpadInfo.isGLMR = address(_baseToken) == address(WGLMR);
    ...
    function addLiquidity() external nonReentrant {
        ...
        // wrap GLMR if base token is GLMR
        if (launchpadInfo.isGLMR) {
            WGLMR.deposit{value: baseLiquidity}();
        }
        ...
        uint256 remainingBaseBalance = launchpadInfo.isGLMR
     ...
     * @dev this function uses msg.value and ignores _amount param for GLMR as base token.
    ...
    function userDeposit(uint256 _amount) external payable nonReentrant {
        ...
        uint256 amount_in = launchpadInfo.isGLMR ? msg.value : _amount;
        ...
        // transfer unused GLMR back to user if base token is GLMR
        if (launchpadInfo.isGLMR && amount_in < msg.value) {
            msg.sender.transfer(msg.value.sub(amount_in));
        }

        // transfer non GLMR base token to launchpad
        if (!launchpadInfo.isGLMR) {
            TransferHelper.safeTransferFrom(
                address(launchpadInfo.bToken),
                msg.sender,
