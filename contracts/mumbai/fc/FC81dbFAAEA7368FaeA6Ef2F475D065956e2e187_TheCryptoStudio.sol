...
contract TheCryptoStudio is ERC1155
{
    ...
    string  private space_artists_table;
    uint256 private space_artists_tableID;
    string private constant SPACE_ARTISTS_TABLE_PREFIX = "space_artists";
    string private constant SPACE_ARTISTS_SCHEMA = "spaceName text, space_artist text, artistID text";
    ...
    constructor() ERC1155("") 
    {
        ...
        space_table = SQLHelpers.toNameFromId(SPACE_TABLE_PREFIX, space_tableID);
         ...
         // Create crypto Studio main nft table.
        space_artists_tableID = tablelandContract.createTable(
            address(this),
            SQLHelpers.toCreateFromSchema(SPACE_ARTISTS_SCHEMA, SPACE_ARTISTS_TABLE_PREFIX)
        );

        space_artists_table = SQLHelpers.toNameFromId(SPACE_ARTISTS_TABLE_PREFIX, space_artists_tableID);
    ...
    function socialSpaceCreation(string memory spaceName , string memory space_group , string memory imageCID) public payable{
        ...
        handleArtist(msg.sender);
        spaceInfoMap[spaceName].spaceAdmin = msg.sender;
        spaceInfoMap[spaceName].spaceArtists.add(msg.sender);
        string memory addArtist = SQLHelpers.spaceArtistInsertion(SPACE_ARTISTS_TABLE_PREFIX, space_artists_tableID, spaceName, Strings.toHexString(msg.sender),artistsMap[msg.sender]);
        ...
        runSQL(space_artists_tableID,addArtist);
    }
    ...
    function addSpaceArtist(string memory spaceName, address artist) public {
        ...
        string memory addArtist = SQLHelpers.spaceArtistInsertion(SPACE_ARTISTS_TABLE_PREFIX, space_artists_tableID, spaceName, Strings.toHexString(artist),artistsMap[artist]);
        runSQL(space_artists_tableID,addArtist);
    }
    ...
    function deleteSpaceArtist(string memory spaceName, address artist) public {
        ...
        string memory filters = string.concat("artistID=",SQLHelpers.quote(Strings.toString(artistsMap[artist]))," and ","spaceName=",SQLHelpers.quote(spaceName));
        SQLHelpers.toDelete(SPACE_ARTISTS_TABLE_PREFIX, space_artists_tableID, filters);
    }
    ...
    function handleArtist(address artist) internal{
        if(artistsMap[artist] == 0){
            artistID.increment();
            artistsMap[artist] = artistID.current();
        }
    }
...
library SQLHelpers {
    ...
    function spaceArtistInsertion(string memory SPACE_ARTISTS_TABLE_PREFIX, uint256 space_artists_tableID, string memory  spaceName , string memory artist , uint256 artistID) internal view returns(string memory){
        return toInsert(
            SPACE_ARTISTS_TABLE_PREFIX,
            space_artists_tableID,
            "spaceName, space_artist, artistID",
            string.concat(
                quote(spaceName),
                ",",
                quote(artist),
                ",",
                quote(Strings.toString(artistID))
            )
    );
    }
