...
contract SphereToken is ERC20Detailed, Ownable {
    ...
    function takeFee(
        ...
        if (
            ...
            //make sure if it's send outside of LP to be taxed too
            if (
                !automatedMarketMakerPairs[recipient] &&
                !automatedMarketMakerPairs[sender] &&
                taxNonMarketMaker
            ) {
                iDexAddress = mainLPAddress;
            }

            //calculate Tax
            if (isTaxBracketEnabled) {
                //params
                IDexPair iDexFeeCalculator = IDexPair(iDexAddress);
                uint112 reserve0;
                uint112 reserve1;
                uint32 blockTimestampLast;
                ...
                uint256 userTotal = getBalanceOfAllSubContracts(sender);
    ...
    function setMainLPAddress(address _mainLPAddress) external onlyOwner {
        require(_mainLPAddress != address(0), 'Main LP not set');
        mainLPAddress = _mainLPAddress;
        emit MainLPAddressSet(_mainLPAddress, block.timestamp);
    }
