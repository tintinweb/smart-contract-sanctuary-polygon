...
contract CryptoRouterV2 is RouterV2, ICryptoRouter {
    ...
    function _executeOp(
        ...
        (chainIdTo, updatedParams, maskedParams, result) = super._executeOp(isOpHalfDone, op, params, prevMaskedParams);
    ...
    function _addCrypto(AddCryptoParams memory p) internal returns (uint256 amountOut) {
        address poolAdapter = IAddressBook(addressBook).cryptoPoolAdapter();
    ...
    function _executeOp(
        ...
        if (PERMIT_CODE == op) {
            ...
            if (isOpHalfDone == false) {
                ...
                maskedParams.amountOut = _mint(p);
                maskedParams.to = p.to;
                maskedParams.emergencyTo = p.emergencyTo;
            }
            ...
            if (isOpHalfDone == false) {
                ...
                processedOps[currentRequestId] = true;
                maskedParams.amountOut = _unlock(p);
                maskedParams.to = p.to;
                maskedParams.emergencyTo = p.emergencyTo;
            }
            ...
            maskedParams.amountOut = _wrap(p);
            maskedParams.to = p.to;
            ...
            address tmp;
            (p.amountIn, p.from, tmp) = _checkMaskedParams(p.amountIn, p.from, address(0), prevMaskedParams);
            maskedParams.amountOut = _unwrap(p);
            maskedParams.to = p.to;
            maskedParams.emergencyTo = prevMaskedParams.emergencyTo;
