...
// File: contracts/GoldMintNFT.sol
...
contract GLDMNFT is ERC721Enumerable, Ownable {
  ...
  // initialize
  function init(address _devWallet, string memory baseURI_, string memory dummyURI_) internal {
    CHEST = 1;
    BAR = 2;
    SACK = 3;
    COIN = 4;
  ...
  constructor(
    ...
    string memory dummyURI_,
    address _devWallet) ERC721(name_, symbol_) {
      init(_devWallet, baseURI_, dummyURI_);
  }
  ...
  function getRandomizedTier() public view returns(uint) {
    ...
    uint rndScaleIn = randomNumber % maxSupply;
    uint id;
  ...
  function mintAll() public onlyOwner {
    uint i;
    for(i = totalSupply(); i < maxSupply; i ++)
      mint();
  }
  ...
  function reveal(bool flag) public onlyOwner {
    require(totalSupply() == maxSupply, "[GoldMintNFT] Not all NFTs have been minted yet!");
    revealed = flag;
  }
