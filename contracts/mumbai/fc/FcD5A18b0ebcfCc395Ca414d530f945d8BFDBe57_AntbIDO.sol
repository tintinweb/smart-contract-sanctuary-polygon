...
contract AntbIDO is Ownable {
    ...
    mapping(address => bool) public boughtANTB;
    ...
    constructor(
        ...
        require(_daiANTBLP != address(0));

        ANTB = _ANTB;
        DAI = _DAI;
        treasury = _treasury;
        daiANTBLP = _daiANTBLP;
        staking = _staking;
        cancelled = false;
        finalized = false;
    }
    ...
    function initialize(
        ...
        initialTotalAmount = _totalAmount;
    ...
    function purchaseANTB(uint256 _amountDAI) external returns (bool) {
        require(saleStarted() == true, 'Not started');
        require(
            !whiteListEnabled || whiteListed[msg.sender] == true,
            'Not whitelisted'
        );
        require(boughtANTB[msg.sender] == false, 'Already participated');

        boughtANTB[msg.sender] = true;
    ...
    function finalize(address _receipt) external onlyOwner {
        require(totalAmount == 0, 'need all antb to be sold');

        uint256 daiInTreasure = 250000 * 1e18;

        IERC20(DAI).approve(treasury, daiInTreasure);
        uint256 antbMinted = IAntbTreasury(treasury).deposit(
            daiInTreasure,
            DAI,
            0
        );

        require(antbMinted == 250000 * 1e9);

        // dev: create lp with 15 DAI per ANTB
        IERC20(DAI).transfer(daiANTBLP, 750000 * 1e18);
        IERC20(ANTB).transfer(daiANTBLP, 50000 * 1e9);
        uint256 lpBalance = IUniswapV2Pair(daiANTBLP).mint(address(this));
        uint256 valueOfToken = IAntbTreasury(treasury).valueOfToken(
            daiANTBLP,
            lpBalance
        );

        IUniswapV2Pair(daiANTBLP).approve(treasury, lpBalance);
        uint256 zeroMinted = IAntbTreasury(treasury).deposit(
            lpBalance,
            daiANTBLP,
            valueOfToken
        );
        require(zeroMinted == 0, 'should not mint any ANTB');
        IERC20(ANTB).approve(staking, antbMinted);
