...
contract ServiceNFT is ERC721URIStorage{
    ...
    mapping(uint256 => Service[]) ServicesOfBusiness;
    mapping(uint256 => string) serviceName;
    mapping(address => Service[]) public userServices; // your Service


    function makeService(uint256 servicePrice, string memory _serviceDescription, string memory tokenURI) public {
    ...
    function getService(uint256 _serviceId) public view returns(uint256, uint256, address, string memory, bool ){
     //   Service memory service = userServices[msg.sender];
    ...
    function listMyServices(uint256 _businessId) public view returns (Service[] memory){
        return ServicesOfBusiness[_businessId];
    }

    function getSoldNFTs() public view returns(uint256){
        return soldNFTs;
    }
}
...
contract BusinessNFT is ERC721URIStorage {
   ...
   /* function setPunkcityContract(address _punkcityAddress) public {
    ...
    function createBusiness(string memory cityName, string memory _businessType, string memory kind, string memory description,  string memory name, string memory _googleAddress, string[] memory _services) public {
        ...
        businessDetails memory nextBusiness = businessDetails(businesses.length, cityName, _businessType, kind, description, name, msg.sender, _googleAddress, block.timestamp, _services);
...
contract Vault {
    ...
    ServiceNFT private _ServiceNFT;
    //testBuying private _testBuying;

    constructor(address _ServiceAddress){
        setServiceContract(_ServiceAddress);
    }
    
    function setServiceContract(address _serviceAddress) public {
        _ServiceNFT = ServiceNFT(_serviceAddress);
        //_testBuying = testBuying(_serviceAddress);
    }
    
    function buy(uint256 serviceId) public payable {
        //require(_testBuying.getOwnerOfService(serviceId) != msg.sender, "You cannot buy your own service");
        //require(msg.value >= _testBuying.getPriceForAService(serviceId), "you want to pay less than the actual price");

        //address payable receiver = _testBuying.getOwnerOfService(serviceId);
        //_testBuying.buyService(serviceId);
        require(_ServiceNFT.getOwnerOfService(serviceId) != msg.sender, "You cannot buy your own service");
        // require for nonexistentBusiness
        require(msg.value >= _ServiceNFT.getPriceForAService(serviceId), "you want to pay less than the actual price");
        address payable receiver = _ServiceNFT.getOwnerOfService(serviceId);
        receiver.transfer(msg.value / 10 * 9); // 90% to seller 10% to Vault
        _ServiceNFT.buyService(serviceId);
    }
    ...
    // function borrow => Valaki kölcsönkér, itt a collateralt kéne megtudni mi lenne az értelme
    // function flashLoan => 


    //TODO: AAVE Part ( credit Delegation, any DEX part if we implement, the chips or energy ( if we implement these what
    // reasonable thing they should be good for ), borrowings, flash loan maybe, any more ideas.
}
