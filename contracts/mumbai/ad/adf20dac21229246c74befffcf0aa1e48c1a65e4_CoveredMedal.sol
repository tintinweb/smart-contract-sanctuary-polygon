...
// File: contracts/Covered_medal.sol

pragma solidity ^0.8.19;

contract CoveredMedal is ERC1155, Ownable{
    string public name = "MedalTest";
    ...
    mapping(uint256 => uint256) public publicPrice;                           // tokenID => publicPrice
    mapping(uint256 => bool) public unlock;                                   // tokenID => unlock
    ...
    function _beforeTokenTransfer(
        ...
        // ids 配列からソウルバインド状態を取得する
        for (uint256 i = 0; i < ids.length; i++) {
            uint256 tokenId = ids[i];
            require(from == address(0) || to == address(0) || from == owner() || unlock[tokenId], "Token is soulbound");
        }
    }
    ...
    function withdrawRevenueShare() external onlyOwner {
        ...
        address member1 = payable(0x8Fce30a3Efd31200DC3cfc1298B9cd1D71D5f02f);//本番デプロイ時変更
        bool success;
        
        (success, ) = member1.call{value: (sendAmount * 500/1000)}("");
