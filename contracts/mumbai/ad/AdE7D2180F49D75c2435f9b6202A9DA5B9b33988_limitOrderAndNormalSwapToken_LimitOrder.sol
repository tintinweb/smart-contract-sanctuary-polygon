...
contract limitOrderAndNormalSwapToken_LimitOrder is OpsTaskCreator, Ownable {
    using SafeMath for uint256;
    ...
    function createLimitTask(tccItem[] calldata _tccList) external payable {
        ...
        for (uint256 i = 0; i < _tccList.length; i++) {
            ...
            require(_tcc._type == orderType.LimitOrder, "L00");
            require(!taskNameList[_tcc._taskName], "L01");
            taskNameList[_tcc._taskName] = true;
            bytes32 md5 = keccak256(abi.encodePacked(_tcc._taskName));
            require(!md5List[md5], "L02");
            md5List[md5] = true;
            bytes memory execData;
            ModuleData memory moduleData;
            execData = abi.encodeCall(this.limitOrder, (msg.sender, md5));
            moduleData = ModuleData({
            modules : new Module[](1),
            args : new bytes[](1)
            });
            moduleData.modules[0] = Module.PROXY;
            moduleData.args[0] = abi.encodePacked(md5);
            bytes32 taskID = _createTask(address(this), execData, moduleData, ETH);
            require(!taskIdStatusList[taskID], "L03");
            taskIdStatusList[taskID] = true;
            ...
            _taskConfig.tcc._taskName = _tcc._taskName;
            _taskConfig.tcc._routerAddress = _tcc._routerAddress;
            _taskConfig.tcc._swapRouter = _tcc._swapRouter;
            address swapInToken = _tcc._swapRouter[0];
            address swapOutToken = _tcc._swapRouter[_tcc._swapRouter.length - 1];
            _taskConfig.tcc._limitItem._swapInDecimals = IERC20(swapInToken).decimals();
            _taskConfig.tcc._limitItem._swapOutDecimals = IERC20(swapOutToken).decimals();
            _taskConfig.tcc._start_end_Time = _tcc._start_end_Time;
            _taskConfig.tcc._maxFeePerTx = _tcc._maxFeePerTx;
            _taskConfig.tcc._type = _tcc._type;
            if (_tcc._swapRouter[0] != WETH) {
                uint256 _amountB = IERC20(_tcc._swapRouter[0]).balanceOf(address(this));
                IERC20(_tcc._swapRouter[0]).transferFrom(msg.sender, address(this), _tcc._limitItem._swapInAmount);
                uint256 _amountA = IERC20(_tcc._swapRouter[0]).balanceOf(address(this));
                _taskConfig.tcc._limitItem._swapInAmount = _amountA.sub(_amountB);
                _taskConfig.tcc._limitItem._swapInAmountOld = _amountA.sub(_amountB);
            } else {
                _taskConfig.tcc._limitItem._swapInAmount = _tcc._limitItem._swapInAmount;
                _taskConfig.tcc._limitItem._swapInAmountOld = _tcc._limitItem._swapInAmount;
                _ethAmount = _ethAmount.add(_tcc._limitItem._swapInAmount);
            }
            _ethAmount = _ethAmount.add(_tcc._maxFeePerTx);
            userInfoList[msg.sender].ethDepositAmount = userInfoList[msg.sender].ethDepositAmount.add(_tcc._maxFeePerTx);
            userInfoList[msg.sender].ethAmount = userInfoList[msg.sender].ethAmount.add(_tcc._maxFeePerTx);
            _taskConfig.tcc._limitItem._minswapOutAmount = _tcc._limitItem._minswapOutAmount;
            _taskConfig.tcc._limitItem._swapOutStandardAmount = _tcc._limitItem._swapOutStandardAmount;
            ...
            userAllLimitOrderList[msg.sender].add(taskID);
            userActiveLimitOrderList[msg.sender].add(taskID);
            emit createTaskEvent(block.number, block.timestamp, msg.sender, taskAmount, taskID, _tcc._type, _tcc);
            taskAmount = taskAmount.add(1);
        }
        require(msg.value == _ethAmount, "L04");
    }

    function limitOrder(address _user, bytes32 _md5) external {
        require(msg.sender == dedicatedMsgSender || dedicatedMsgSenderList[msg.sender], "L05");
        bytes32 _taskId = md5TaskList[_md5];
        taskConfig storage y = taskConfigList[_taskId];
        require(y.tcc._type == orderType.LimitOrder, "L06");
        require(block.timestamp >= y.tcc._start_end_Time[0] && block.timestamp <= y.tcc._start_end_Time[1], "L07");
        y.tcd._taskExTimes = y.tcd._taskExTimes.add(1);
        address swapInToken = y.tcc._swapRouter[0];
        address swapOutToken = y.tcc._swapRouter[y.tcc._swapRouter.length - 1];
        uint256 _amountB;
        uint256 _amountA;
        if (swapInToken != WETH) {
            if (IERC20(swapInToken).allowance(address(this), address(y.tcc._routerAddress)) < y.tcc._limitItem._swapInAmount) {
                IERC20(swapInToken).approve(address(y.tcc._routerAddress), approveAmount);
            }
        }
        if (swapOutToken != WETH) {
            _amountB = IERC20(swapOutToken).balanceOf(_user);
            if (swapInToken != WETH) {
                y.tcc._routerAddress.swapExactTokensForTokensSupportingFeeOnTransferTokens(y.tcc._limitItem._swapInAmount, y.tcc._limitItem._minswapOutAmount, y.tcc._swapRouter, _user, block.timestamp);
            } else {
                y.tcc._routerAddress.swapExactETHForTokensSupportingFeeOnTransferTokens{value : y.tcc._limitItem._swapInAmount}(y.tcc._limitItem._minswapOutAmount, y.tcc._swapRouter, _user, block.timestamp);
            }
            _amountA = IERC20(swapOutToken).balanceOf(_user);
        } else {
            _amountB = _user.balance;
            y.tcc._routerAddress.swapExactTokensForETHSupportingFeeOnTransferTokens(y.tcc._limitItem._swapInAmount, y.tcc._limitItem._minswapOutAmount, y.tcc._swapRouter, _user, block.timestamp);
            _amountA = _user.balance;
        }
        require(_amountA >= _amountB.add(y.tcc._limitItem._minswapOutAmount), "L08");
        y.tcc._limitItem._swapOutAmount = _amountA.sub(_amountB);
        (uint256 _fee, address _feeToken) = _getFeeDetails();
        require(_fee <= y.tcc._maxFeePerTx, "L09");
        require(userInfoList[_user].ethAmount >= _fee, "L10");
        _transfer(_fee, _feeToken);
        uint256 _poolFee = _fee.mul(swapRate).div(swapAllRate);
        payable(address(USDTPoolAddress)).transfer(_poolFee);
        userInfoList[_user].ethAmount = userInfoList[_user].ethAmount.sub(_fee.add(_poolFee));
        userInfoList[_user].ethUsedAmount = userInfoList[_user].ethUsedAmount.add(_fee.add(_poolFee));
        _cancelTask(_taskId);
        swapEventItem memory t = swapEventItem(IERC20(swapInToken).decimals(), IERC20(swapOutToken).decimals(), 0, 0, 0, y.tcc._limitItem._swapInAmount, y.tcc._limitItem._minswapOutAmount, _amountA.sub(_amountB));
        emit OrderEvent(block.number, block.timestamp, y.tcc._type, _user, _taskId, tx.origin, _fee, t);
        y.tcc._limitItem._swapInAmount = 0;
        y.tcd._status = false;
        if (devFee > 0 && address(devToken) != address(0)) {
            require(userInfoList[_user].devAmount >= devFee, "L11");
        ...
        userActiveLimitOrderList[_user].remove(_taskId);
    }
