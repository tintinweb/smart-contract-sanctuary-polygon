...
//MPW Token
    ...
    uint256 public minimumMwpAmount;
...
//TOTAL SOLD PER BACTH
    ...
    bytes32 public constant BUY_PASS_ROLE = keccak256("BUY_PASS_ROLE");
    ...
    modifier minMatic(uint amount){
        if(amount < minimumMwpAmount){
            revert MinMatic(amount);
        }
        else{
            _;
        }
...
//--------------------------------------------------------------------------------
    ...
    function setMinimumMwpAmount(uint256 amount) external onlyRole(MIN_MWP_WRITER_ROLE) {
        if (amount < 10**18) {
            revert MinimumMwpAmount(amount, 10**18);
        }
        minimumMwpAmount = amount;
    }
//----------------------------------------------------------------------
...
//----------------------------Handle With WhiteList------------------------------
    ...
    function removeAddressFromWaitingList(address _address) external onlyRole(WAITING_LIST_WRITER_ROLE) {
...
//-------------------------------------
    function writeOrder(address _account, uint _tokens, Batch _batch, PurchaseMethod _method) private {
        ...
        ordersIndex++;
        if(_batch == Batch.BATCH_PUBLIC_SALE){
            totalSoldWL+= _tokens;
        }
...
//---------------------------------------------------- Buy With Matic -----------------------------------
    ...
    function BuyMwpWithMatic() public payable minMatic(msg.value) {
    ...
    function buyMwpInWL(address _account, uint _value, PurchaseMethod _method) private forbidZeroAddress(_account) {
        ...
        if(_method == matic){
            ...
            if(_value > mwpRemains){
                ...
                buyMwpInBacthPL(_account, _value, pix);
            }
        }
    }
       ...
       require(!isPausePL, "PL is Pause");
        ...
        if(_method == matic){
            ...
            if(remainsInMatic < _value){
                isPausePL = true;
                uint nextBatchinMatic = _value - remainsInMatic;
                uint mwpAmmountWL = mwpAmountGivenMatic(remainsInMatic, PL);
                writeOrder(_account, mwpAmmountWL, PL, matic);
                (bool sent, ) = _account.call{value: nextBatchinMatic }("");
                if(sent == false){
                    revert FailSendEdgePL(nextBatchinMatic);
                }
                if(sent == true){
                    emit MaticReturnsPL(_account, remainsInMatic, nextBatchinMatic);
                } 
            }
            else{
