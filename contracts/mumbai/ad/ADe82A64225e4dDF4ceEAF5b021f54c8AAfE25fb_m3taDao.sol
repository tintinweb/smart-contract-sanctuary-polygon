    ...
    Ownable
{
    ...
    mapping(address => uint256) profileMapping;
    ...
    constructor(Im3taQuery initQueryContract)
    {
        ...
        _profileTablePrefix = "M3taProfile";
        ...
        _profileTableId = tablelandContract.createTable(
            address(this),
            queryContract.getCreateProfileTableStatement(
                _profileTablePrefix,
                _chainID
            )
        );
    ...
    function createProfile(DataTypes.ProfStruct memory vars) public {
        vars.profileAddress = msg.sender;
        require(
            profileMapping[vars.profileAddress] == 0, "Only one profile per Wallet address"
        );
        profileID.increment();
        vars.profileID = profileID.current();
        profileMapping[vars.profileAddress] = vars.profileID;
        vars.metadataTable = _profileTable;
        string memory statement = queryContract.getProfileInsertStatement(vars);
        runSQL(_profileTableId,statement);
    }
        ...
        payable
    {
        ...
        require(
            profileMapping[vars.founderAddress] > 0,
    ...
    function rejectHiringRequest(uint256 accountID , uint256 hireId) public {
        ...
        string memory statement = queryContract.getRejectHiringRequestStatement(_profileTable,hireId);
         runSQL(_profileTableId,statement);
        //Assigning the null address for Avoiding conflicts
        hireMapping[hireId] = address(0);
    }
...
interface Im3taQuery  {
    ...
    function getProfileInsertStatement(DataTypes.ProfStruct memory vars)
    external pure returns (string memory);
