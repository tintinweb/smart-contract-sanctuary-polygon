...
contract TicketMeTest is ERC721Enumerable {
    uint256 public nextTokenId = 1;

    constructor()
        ERC721("TicketMeTest", "TMT")
    {}
    ...
    function _getJson(uint256 tokenId, string memory svg) private pure returns (string memory) {

        bytes memory json = abi.encodePacked(
            '{"name": "Ticket #',tokenId,
            '", "description": "TicketMe is super web3 product!", "image": "data:image/svg+xml;base64,',Base64.encode(bytes(svg)),
            '"}'
        );
