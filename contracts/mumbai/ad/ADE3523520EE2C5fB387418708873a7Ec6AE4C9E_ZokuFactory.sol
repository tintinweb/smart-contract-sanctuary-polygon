...
contract ZokuFactory is OwnableUpgradeable {
  ...
  function create(
    ...
    ISettings(settings).setMaxPerMint(5);
  ...
  function settingsAddAdmin(address _token, address _admin) external onlyAdmin {
    Project storage project = _projects[_token];
    ISettings(project.settings).addAdmin(_admin);
  }
  ...
  function settingsSetRefundable(
    address _token,
    bool _refundable,
    uint256 _refundFixedFee,
    uint256 _refundPercentageFee
  ) external onlyAdmin {
    Project storage project = _projects[_token];
    ISettings(project.settings).setRefundable(_refundable, _refundFixedFee, _refundPercentageFee);
  }
  ...
  function settingsSetMaxPerMint(address _token, uint256 _quantity) external onlyAdmin {
    Project storage project = _projects[_token];
    ISettings(project.settings).setMaxPerMint(_quantity);
  }
  ...
  function salesRenounceOwnership(address _token) external onlyAdmin {
    Project storage project = _projects[_token];
    ISales(project.sales).renounceOwnership();
  }
   ...
   ****************************************************
   */
  function tokenSetMinter(address _token, address _minter) external onlyAdmin {
    Project storage project = _projects[_token];
    IERC721Standard(project.token).setMinter(_minter);
  }

  function tokenTransferOwnership(address _token, address _newOwner) external onlyAdmin {
    Project storage project = _projects[_token];
    IERC721Standard(project.token).transferOwnership(_newOwner);
  }

  function tokenRenounceOwnership(address _token) external onlyAdmin {
    Project storage project = _projects[_token];
    IERC721Standard(project.token).renounceOwnership();
  }
}
