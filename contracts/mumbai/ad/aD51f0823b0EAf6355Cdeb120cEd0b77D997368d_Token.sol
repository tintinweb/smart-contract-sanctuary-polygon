...
abstract contract Taxable is Context {
  ...
  uint private _maxtax;
  uint private _mintax;
...
contract Token is ERC20, ERC20Burnable, ReentrancyGuard, Taxable, AccessControl {
  /* Variables */
  ...
  constructor(uint initialSupply) ERC20("Two Lands Token", "LANDS") {
  ...
  function _transfer(address from, address to, uint256 amount) internal virtual override(ERC20) nonReentrant {
    if (initialLiquidityProvided) {
      if (hasRole(EXCLUDED_ROLE, from) || hasRole(EXCLUDED_ROLE, to) || !taxed()) {
        ...
        console.log("Original Amount is %s", amount);
        console.log("Taxed amount is %s", taxedAmount);
        console.log("Remaining amount is %s", remainingAmount);

        require(balanceOf(from) >= amount, "ERC20: transfer amount exceeds balance");

        /* @dev: Amount of tokens transferred may not exceed % of totalSupply */
        ...
        console.log("Emitting %s", taxedAmount);
        emit Transfer(from, destination, taxedAmount);
        super._transfer(from, to, remainingAmount);
        console.log("Emitting %s", remainingAmount);
