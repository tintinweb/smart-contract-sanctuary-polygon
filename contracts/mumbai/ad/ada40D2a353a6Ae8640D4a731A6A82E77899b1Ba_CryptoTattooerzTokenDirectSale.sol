...
contract CryptoTattooerzTokenDirectSale is CryptoTattooerzCurrencyHandler, Pausable, AccessControlEnumerable{
     ...
     * @param discountRate Discount rate applied during the purchase
     * @param discountDecimals Discount rate applicable decimals
     */
    event TokensPurchased(address indexed beneficiary, uint256 amount, bytes32 indexed currency, uint256 price, uint256 discountRate, uint8 discountDecimals);
    /**
...
//            require(currency != getPriceHandler().TOKEN(), "CryptoTattooerzTokenDirectSale: Cannot change TOKEN address");
    ...
    function buyWithCoins() public payable returns (uint256) {
        ...
        (uint256 tokenAmount, uint256 discountRate, uint8 discountDecimals) =
            priceHandler.transform(priceHandler.COIN(), priceHandler.TOKEN(), msg.value);
        // Perform the final buy
        _buy(msg.sender, tokenAmount, priceHandler.COIN(), msg.value, discountRate, discountDecimals);
        return tokenAmount;
    }
    ...
    function buyWithTokens(bytes32 currency, uint256 amount) public returns (uint256) {
        ...
        CryptoTattooerzPriceHandler priceHandler = getPriceHandler();
        (uint256 tokenAmount, uint256 discountRate, uint8 discountDecimals) =
            priceHandler.transform(currency, priceHandler.TOKEN(), amount);
        ...
        // Perform the final buy
        _buy(msg.sender, tokenAmount, currency, amount, discountRate, discountDecimals);
        return tokenAmount;
    }
     ...
     * paused
     */
    function _buy(address buyer, uint256 amount, bytes32 currency, uint256 price, uint256 discountRate, uint8 discountDecimals) private whenNotPaused() {
        ...
        emit TokensPurchased(buyer, amount, currency, price, discountRate, discountDecimals);
    }
