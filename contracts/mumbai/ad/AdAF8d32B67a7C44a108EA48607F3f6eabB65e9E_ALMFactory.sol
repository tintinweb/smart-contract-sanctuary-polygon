...
contract ALMFactory is Ownable {
    ...
    address[] public almTokenArray;
    mapping(uint32 => address) public releaseIdMapping;
    event ALMCreated(uint32 indexed releaseId, string indexed symbol, address almAddress);
    
    /**
        Fallback function that reverts any value sent
    */
    fallback() payable external   {
        require(false,'Fallback method does not allow for MATIC to be sent');
    }
    ...
    */
    function createALM (uint32 releaseId_, address owner_, string memory name_, string memory symbol_) external onlyOwner returns(address){
        ...
        require(releaseIdMapping[releaseId_] == address(0),'The release id mapping already exisits');

        // Creates new ALM token
        ALM almToken = new ALM(releaseId_, owner_, name_, symbol_, address(this));

        // Sets mapping, emits ALMCreated, and transfers ownership to msg.sender
        releaseIdMapping[releaseId_] = address(almToken);
        almTokenArray.push(address(almToken));
    ...
    */
    function provideReward(ALM almToken, IERC20 token, uint256 amount) payable onlyOwner external {
    ...
    */
    ...
    /**
        ALM token mapping length
    */
    function almTokenArrayLength() view external returns(uint256){
        return almTokenArray.length;
    }
}
...
contract ALM is ERC20, Ownable, ReentrancyGuard {
    ...
    address private constant MATIC = 0x0000000000000000000000000000000000001010;
    address public immutable almFactoryAddress;
    ...
    constructor(uint32 releaseId_, address owner_, string memory name_, string memory symbol_, address almFactoryAddress_) ERC20(name_, symbol_) {
        ...
        almFactoryAddress = almFactoryAddress_;
    }
    ...
    */
    function initializeReward(IERC20 token, uint256 amount) payable external onlyWhenWindowIsNotOpen {
        require(address(msg.sender) == almFactoryAddress || address(msg.sender) == owner(), 'You are not allowed to initialize the reward.');
