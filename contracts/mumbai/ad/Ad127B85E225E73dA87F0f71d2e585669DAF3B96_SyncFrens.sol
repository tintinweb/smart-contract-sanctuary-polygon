...
contract SyncFrens is ERC721, Ownable, ERC721Enumerable, Pausable, ERC2981ContractWideRoyalties{
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIdCounter;
    
    uint8 public MAX_PER_WALLET = 1;
    uint8 public maxSupply = 150;
     // epoch timestamp
    uint256 public whitelistWindowOpens = 1687428184;
    uint256 public publicWindowOpens    = 1687514584;
    ...
    //@notice mint during whitelist period with whitelisted address 
    ...
    function mint() private {
        require(totalSupply() < maxSupply, "Sorry, all the SyncFrens have already been minted.");
        require(balanceOf(msg.sender) < MAX_PER_WALLET, "You've already minted a SyncFren");
    ...
    */
    // calldata is coming from the transaction
    ...
    /**
    * @notice returns true or false depending on if address is on whitelist
    */
    function isWhitelisted() external view  returns (bool) {
     ...
     //take out the cash from the contract
    function withdraw(address _addr) external onlyOwner {
        uint256 balance = address(this).balance;
        payable(_addr).transfer(balance);
    }

    //royalties for ongoing sales
  function setRoyalties (address recipient, uint value) external onlyOwner {
