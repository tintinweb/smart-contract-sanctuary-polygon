...
contract Vault is Ownable, AccessControlEnumerable {
    ...
    function deposit(uint256 _amount) external {
        console.log("deposit owner: %s", msg.sender);
        console.log("balanceOf: %s", token.balanceOf(msg.sender));
        require(
            token.balanceOf(msg.sender) >= _amount,
            "Insufficient account balance"
        );
        console.log("deposit _amount: %s, token.balanceOf: %s", _amount, token.balanceOf(msg.sender));
