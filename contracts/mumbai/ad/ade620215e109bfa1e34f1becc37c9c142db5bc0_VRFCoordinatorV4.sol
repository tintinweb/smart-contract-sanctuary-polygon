...
// File contracts/VRFCoordinatorV4.sol
...
contract VRFCoordinatorV4 is VRF, ConfirmedOwner, TypeAndVersionInterface, VRFCoordinatorV4Interface {
  WrappedNativeInterface public immutable WrapperToken;
  ...
  mapping(address => uint256) /* oracle */ /* LINK balance */ private s_withdrawableTokens;
  ...
  constructor(address wrapperToken, address blockhashStore) ConfirmedOwner(msg.sender) {
    WrapperToken = WrappedNativeInterface(wrapperToken);
  ...
  function recoverFunds(address to) external onlyOwner {
    ...
    uint256 externalBalance = WrapperToken.balanceOf(address(this));
    ...
    if (internalBalance < externalBalance) {
      ...
      WrapperToken.transfer(to, amount);
  ...
  function oracleWithdraw(address recipient, uint256 amount) external nonReentrant {
    ...
    if (!WrapperToken.transfer(recipient, amount)) {
      revert InsufficientBalance();
    }
  }
  ...
  function fundSubscription(uint64 subId, uint256 amount) external override {
    fundSubscriptionInternal(subId, amount, false);
  }

  /**
   * @inheritdoc VRFCoordinatorV4Interface
   */
  function fundSubscriptionNative(uint64 subId) external payable override {
    fundSubscriptionInternal(subId, msg.value, true);
  }

  function fundSubscriptionInternal(uint64 subId, uint256 amount, bool isNative) internal nonReentrant {
    if (isNative) {
      // Convert received native token to wrapped token
      WrapperToken.deposit{value: amount}();
    } else {
      // only wkcs
      bool isSuccess = WrapperToken.transferFrom(msg.sender, address(this), amount);
      if (!isSuccess) {
        revert InsufficientBalance();
      }
    }
  ...
  //    // anyone can fund a subscription.
  //    uint256 oldBalance = s_subscriptions[subId].balance;
  //    s_subscriptions[subId].balance += uint256(amount);
  //    s_totalBalance += uint256(amount);
  //    emit SubscriptionFunded(subId, oldBalance, oldBalance + amount);
  //  }
  ...
  function cancelSubscriptionHelper(uint64 subId, address to) private nonReentrant {
    ...
    if (!WrapperToken.transfer(to, balance)) {
