    ...
    ERC2981Upgradeable
{
    ...
    ///@notice SFTRecommendation pattern
    ...
    function mint(
        ...
        if (_coin == IManagement.Coin.ETH_COIN) {
            isAmountLowerThanPrice = msg.value < pricePerCoin[_coin];
        } else {
        ...
        _doAllTransfersMint(_coin, pricePerCoin[_coin], _tokenId);

        _safeMint(msg.sender, _tokenId);

        __increaseScore(msg.sender, 1);
    ...
    function mintForCrowdfund(
        ...
        if (_tokenIds.length != _scores.length) {
        ...
        for (uint256 ii; ii < _tokenIds.length; ++ii) {
            ...
            __increaseScore(_to, _scores[ii]);
        }
    }
    ...
    function _coinTransfer(
        ...
        if (_coin == IManagement.Coin.ETH_COIN) {
            ...
            management.tokenContract(_coin).transferFrom(_from, _payee, _value);
        }
    }
    ...
    /** @dev increases user score on Management contract
        ...
        @param _score: score to be added */
    function __increaseScore(address _user, uint256 _score) private {
        management.increaseScore(_user, _score);
    }
    ...
    function safeTransferFrom(
        ...
        address to,
        uint256 tokenId
    ) public override(IERC721Art, ERC721Upgradeable) {
        __nonReentrant();
        __whenNotPaused();
    ...
    function isApprovedForAll(
        address _owner,
        address operator
    ) public view override(IERC721Art, ERC721Upgradeable) returns (bool) {
    ...
    function getApproved(
        ...
        public
        view
        override(IERC721Art, ERC721Upgradeable)
        returns (address operator)
    {
