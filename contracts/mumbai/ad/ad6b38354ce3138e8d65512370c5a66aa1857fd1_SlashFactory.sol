...
contract MerchantProperty is IMerchantProperty, Ownable {
    ...
    bool internal _isReceiveOnContract = false;

    address internal _slashCore;
    IMerchantProperty internal _sharedProperty;
    ISlashController internal _slashController;

    event ReceiveAddressUpdated(address oldAddress, address newAddress, bool isContract);
    ...
    function viewCashBackPercentWithTime() external view returns (uint256 cashBackPercent, uint256 lastModified) {
        if (_specificProps[Property.CASH_BACK_PERCENT]) {
            return (_chashBackPercent, _lastModified[Property.CASH_BACK_PERCENT]);
        } else {
            return (_sharedProperty.viewCashBackPercent(), 0);
        }
    }
    ...
    function viewMerchantReceiveWallet() external override view returns (address) {
        return _merchantReceiveWallet;
    }

    function viewMerchantReceiveContract() external override view returns (address) {
        return _receiveContractAddress;
    }

    function viewReceiveAddress() external override view returns (
    ...
    function isReceiveOnContract() external override view returns (bool) {
        return _isReceiveOnContract;
    }

    function updateReceiveAddress(address receiveAddress, bool isContract) public {
        ...
        if (isContract) {
            require(isContractAddress(receiveAddress), "this address is not contract address");
            emit ReceiveAddressUpdated(_receiveContractAddress, receiveAddress, isContract);
            _receiveContractAddress = receiveAddress;
        } else {
            emit ReceiveAddressUpdated(_merchantReceiveWallet, receiveAddress, isContract);
