...
import "../../contracts/libraries/OSB721.sol";
import "../../contracts/interfaces/IOSBFactory.sol";
...
import {RoyaltyInput, CollectionInput} from "../interfaces/IOSBFactory.sol";
...
contract OSB721 is ERC721EnumerableUpgradeable, ERC2981Upgradeable, OwnableUpgradeable {
    ...
    event MintWithRoyalty(
        address indexed to,
        uint256 tokenId,
        string tokenUri,
        address indexed receiverRoyalty,
        uint96 indexed percentageRoyalty
    );
    ...
    function initialize(
        ...
        __ERC721_init(_collectionInput.name, _collectionInput.symbol);
        __Ownable_init();
    ...
    ) public exceededTotalSupply(1) onlyOwnerOrController returns (uint256 tokenId) {
        require(bytes(_tokenUri).length > 0, "Invalid tokenUri");
        lastId.increment();
    ...
    ) external exceededTotalSupply(_tokenUris.length) onlyOwnerOrController returns (uint256[] memory tokenIds) {
