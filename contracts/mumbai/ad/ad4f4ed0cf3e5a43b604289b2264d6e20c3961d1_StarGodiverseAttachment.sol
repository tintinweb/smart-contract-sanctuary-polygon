    ...
    IERC1155ReceiverUpgradeable
{
    ...
    address public godiverseAddress;
    ...
    event ContractSettingupdated(
        ...
        address indexed _godiverseAddress,
        address _blacklistAddress
    );
    event TokenReceiptContractsUpdated(address indexed _tokenReceipt);
    event StarAttachedGodiverse(uint256 indexed _starId, uint256 _godiverseId);
    event StarDetachedGodiverse(uint256 indexed _starId, uint256 _godiverseId);
    ...
    modifier isStarOwner(uint256 _nftId, address _caller) {
        require(
            _caller == IERC721Upgradeable(starAddress).ownerOf(_nftId) ||
                _caller ==
    ...
    modifier isGodiverseOwner(uint256 _nftId, address _caller) {
        require(
            IERC1155Upgradeable(godiverseAddress).balanceOf(_caller, _nftId) >=
                1,
    ...
    function updateContractSetting(
        ...
        godiverseAddress = _godiverseAddress;
    ...
    function _checkNInitStarGodiverseMap(
        ...
        StarGodiverseData memory starGodiverseData = starDataContract
            .getGodiverseByStarId(_starId);
        uint256 targetValue = 0;
        if (_slot == GodiverseSlot.Sun) {
            if (starGodiverseData.sunSlot.length == 0) {
                targetValue = _attributes[4];
            }
        } else if (_slot == GodiverseSlot.AsteroidBelt) {
            if (starGodiverseData.asteroidBeltSlot.length == 0) {
                targetValue = _attributes[3];
            }
        } else if (_slot == GodiverseSlot.KuiperBelt) {
            if (starGodiverseData.kuiperBeltSlot.length == 0) {
                targetValue = _attributes[2];
            }
        } else if (_slot == GodiverseSlot.Comet) {
            if (starGodiverseData.cometSlot.length == 0) {
                targetValue = _attributes[1];
            }
        } else {
            // fulfill coverage, if (_slot == GodiverseSlot.Supernova)
            if (starGodiverseData.supernovaSlot.length == 0) {
                targetValue = _attributes[0];
            }
        }
        // if mapData length is 0, init it
        if (targetValue > 0) {
            starDataContract.initStarGodiverseMap(_starId, _slot, targetValue);
        }
    }
    ...
    function _checkCanBeAttach(
        ...
        if (
            ...
            blacklistContract.blacklistedNFT(msg.sender, starAddress, _starId)
        ) {
            return false;
        }

        // check _slotIndex is free slot or not
        StarGodiverseData memory godiverseData = starDataContract
            .getGodiverseByStarId(_starId);
        if (_slot == GodiverseSlot.Sun) {
            if (godiverseData.sunSlot[_slotIndex] != 0) {
                return false;
            }
        } else if (_slot == GodiverseSlot.AsteroidBelt) {
            if (godiverseData.asteroidBeltSlot[_slotIndex] != 0) {
                return false;
            }
        } else if (_slot == GodiverseSlot.KuiperBelt) {
            if (godiverseData.kuiperBeltSlot[_slotIndex] != 0) {
                return false;
            }
        } else if (_slot == GodiverseSlot.Comet) {
            if (godiverseData.cometSlot[_slotIndex] != 0) {
                return false;
            }
        } else {
            // fulfill coverage, if (_slot == GodiverseSlot.Supernova)
            if (godiverseData.supernovaSlot[_slotIndex] != 0) {
                return false;
            }
        }
    ...
    function attachGodiverse(
        ...
        external
        isStarOwner(_starId, msg.sender)
        isGodiverseOwner(_godiverseId, msg.sender)
    {
        ...
        if (godiverseInfo.additionalOribitalTrack > 0) {
            ...
            // add the additionalOribitalTrack
            starDataContract.addStarOrbitalTrackData(
                _starId,
                godiverseInfo.additionalOribitalTrack
            );
        }
        ...
        IERC1155Upgradeable(godiverseAddress).safeTransferFrom(
            msg.sender,
            address(this),
            _godiverseId,
            1,
        ...
        // emit event
        emit StarAttachedGodiverse(_starId, _godiverseId);
    }
    ...
    function detachGodiverse(
        ...
        StarGodiverseData memory starGodiverse = starContract
            .getStarGodiverseByStarId(_starId);
        bool isAttached = false;
        if (_slot == GodiverseSlot.Sun) {
            if (starGodiverse.sunSlot[_slotIndex] == _godiverseId) {
                isAttached = true;
            }
        } else if (_slot == GodiverseSlot.AsteroidBelt) {
            if (starGodiverse.asteroidBeltSlot[_slotIndex] == _godiverseId) {
                isAttached = true;
            }
        } else if (_slot == GodiverseSlot.KuiperBelt) {
            if (starGodiverse.kuiperBeltSlot[_slotIndex] == _godiverseId) {
                isAttached = true;
            }
        } else if (_slot == GodiverseSlot.Comet) {
            if (starGodiverse.cometSlot[_slotIndex] == _godiverseId) {
                isAttached = true;
            }
        } else {
            // fulfill coverage, if (_slot == GodiverseSlot.Supernova)
            if (starGodiverse.supernovaSlot[_slotIndex] == _godiverseId) {
                isAttached = true;
            }
        }
        require(isAttached, "Godiverse is not attached");
        ...
        // Items Attached cannot be detached if any orbital tracks are rented out
        ...
        // update orbital track slot
        // since already check no orbital tracks are rented, just pop the orbital track slot
        GodiverseInfo memory godiverseInfo = starDataContract.getGodiverseInfo(
            _godiverseId
        );
        if (godiverseInfo.additionalOribitalTrack > 0) {
            starDataContract.removeStarOrbitalTrackSlot(
                _starId,
                godiverseInfo.additionalOribitalTrack
            );
        }
        ...
        // emit event
        emit StarDetachedGodiverse(_starId, _godiverseId);
    }
