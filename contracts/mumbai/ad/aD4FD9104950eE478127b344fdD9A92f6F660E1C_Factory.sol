...
contract DMLToken is ERC7254, ReentrancyGuard, ERC1155Holder {
    ...
    constructor(
        ...
        uint _fee
    ) ERC7254(name, symbol, _tokenReward){
        token = _token;
        nft = _nft;
        id = _id;
...
contract Factory is Ownable{
    ...
    event DmlTokenCreated(address creator,address erc, address nft, uint256 id, address dmlToken, uint256 length);
    ...
    function createDMLToken(address creator,address token, address nft, uint256 id) external returns (address) {
        ...
        emit DmlTokenCreated(creator, token, nft, id, address(_dmltoken), allDmlTokens.length);
