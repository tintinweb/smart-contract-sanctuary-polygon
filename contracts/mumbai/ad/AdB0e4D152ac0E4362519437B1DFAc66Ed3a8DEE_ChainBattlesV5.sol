...
contract ChainBattlesV5 is ERC721URIStorage, Ownable {
    using Strings for uint256;
    ...
    string[] public availableClasses = ["Warrior", "Mage", "Rogue", "Hunter"];

    constructor() ERC721("Chain Battles", "CBTLS") {}

    function addClass(string memory _newClass) public onlyOwner {
        availableClasses.push(_newClass);
    }

    function removeClass(string memory value) public onlyOwner {
        for (uint i = 0; i < availableClasses.length; i++) {
            if (
                keccak256(bytes(availableClasses[i])) == keccak256(bytes(value))
            ) {
                if (i != availableClasses.length - 1) {
                    availableClasses[i] = availableClasses[
                        availableClasses.length - 1
                    ];
                }
                availableClasses.pop();
                return;
            }
        }
    }
    ...
    function generateCharacter(uint256 tokenId) public returns (string memory) {
        ...
        bytes memory svg = abi.encodePacked(
            ...
            '"/>',
            "<g>",
            '<svg x="30%" y="23%" fill="#000000" width="60px" height="60px" viewBox="0 0 128 128" enable-background="new 0 0 128 128" version="1.1" xml:space="preserve" xmlns="http://www.w3.org/2000/svg">',
            iconForNft,
            "</svg>",
            "</g>",
            '<text x="56%" y="32%" class="base" dominant-baseline="middle" text-anchor="middle">',
            getClass(tokenId),
            "</text>",
            ...
            getLevels(tokenId),
            "</text>",
            '<text x="50%" y="51%" class="base" dominant-baseline="middle" text-anchor="middle">',
            "Strength: ",
            getStr(tokenId),
            "</text>",
            '<text x="50%" y="57%" class="base" dominant-baseline="middle" text-anchor="middle">',
            "Agility: ",
            getAgi(tokenId),
            "</text>",
            '<text x="50%" y="63%" class="base" dominant-baseline="middle" text-anchor="middle">',
            "Intelligence: ",
            getInt(tokenId),
            "</text>",
            '<text x="50%" y="69%" class="base" dominant-baseline="middle" text-anchor="middle">',
            "Spirit: ",
            getSpi(tokenId),
            "</text>",
    ...
    function getClass(uint256 tokenId) public view returns (string memory) {
        string memory class = tokenIdToStats[tokenId].class;
        return class;
    }
    ...
    function getStr(uint256 tokenId) public view returns (string memory) {
        uint256 str = tokenIdToStats[tokenId].strength;
        return str.toString();
    }

    function getAgi(uint256 tokenId) public view returns (string memory) {
        uint256 agi = tokenIdToStats[tokenId].agility;
        return agi.toString();
    }

    function getInt(uint256 tokenId) public view returns (string memory) {
        uint256 inte = tokenIdToStats[tokenId].intelligence;
        return inte.toString();
    }

    function getSpi(uint256 tokenId) public view returns (string memory) {
        uint256 spi = tokenIdToStats[tokenId].spirit;
        return spi.toString();
    }

    function getSta(uint256 tokenId) public view returns (string memory) {
        uint256 sta = tokenIdToStats[tokenId].stamina;
        return sta.toString();
    }
    ...
    function mint(string memory _class) public {
        _tokenIds.increment();
    ...
    function train(uint256 tokenId) public {
        ...
        uint256 currentStr = tokenIdToStats[tokenId].strength;
        tokenIdToStats[tokenId].strength = currentStr + 3;

        uint256 currentAgi = tokenIdToStats[tokenId].agility;
        tokenIdToStats[tokenId].agility = currentAgi + 2;

        uint256 currentInt = tokenIdToStats[tokenId].intelligence;
        tokenIdToStats[tokenId].intelligence = currentInt + 1;

        uint256 currentSpi = tokenIdToStats[tokenId].spirit;
        tokenIdToStats[tokenId].spirit = currentSpi + 2;

        uint256 currentSta = tokenIdToStats[tokenId].stamina;
        tokenIdToStats[tokenId].stamina = currentSta + 3;
