...
// File: contracts/RunnerableUpgradeable.sol
 ...
 * the owner.
 */
abstract contract RunnerableUpgradeable is OwnableUpgradeable {

    address private _runner;

    event RunnershipTransferred(address indexed previousRunner, address indexed newRunner);

    /**
     * @dev Initializes the contract setting the deployer as the initial owner.
     */
    function __Runnerable_init() internal onlyInitializing {
        __Ownable_init();
    }

    /**
     * @dev Throws if called by any account other than the runner.
     */
    modifier onlyRunner() {
        _checkRunner();
        _;
    ...
    /**
 * @dev Returns the address of the current runner.
     */
    function runner() public view virtual returns (address) {
        return _runner;
    }
 ...
 * @dev Throws if the sender is not the owner.
     */
    function _checkRunner() internal view virtual {
        require((owner() == _msgSender())||(runner() == _msgSender()), "Runner: caller is not the runner");
    }
     ...
     */
    function renounceRunnership() public virtual onlyOwner {
        _transferRunnership(address(0));
    }
     ...
     * Can only be called by the current owner.
     */
    function transferRunnership(address newRunner) public virtual onlyOwner {
        require(newRunner != address(0), "Runnable: new runner is the zero address");
        _transferRunnership(newRunner);
    }
     ...
     * Internal function without access restriction.
     */
    function _transferRunnership(address newRunner) internal virtual {
        address oldRunner = _runner;
        _runner = newRunner;
        emit RunnershipTransferred(oldRunner, newRunner);
    }
...
contract TIM721 is ERC721Upgradeable, ERC721HolderUpgradeable, ERC721URIStorageUpgradeable, ERC721EnumerableUpgradeable, ERC721BurnableUpgradeable, EIP712Upgradeable,RunnerableUpgradeable {

    string private _baseUri;
    string private _newName;
    string private _newSymbol;
    string private _version = "2022120801";

    constructor() initializer {}

    // goto online tool: https://abi.hashex.org/ to get the calldata
    function initialize(string memory iName, string memory iSymbol, string memory iBaseURI ) initializer public {
        __ERC721_init(iName, iSymbol);
        __ERC721Enumerable_init();
        __ERC721Holder_init();
        __ERC721URIStorage_init();
        __Runnerable_init();
        __ERC721Burnable_init();
        __EIP712_init(iSymbol, _version);
        _newName = iName;
        _newSymbol = iSymbol;
        _baseUri = iBaseURI;
    ...
    // when upgrade with call, 0x48c02dc1, always increase the date and number before compile and upgrade
    function updateVersion() public
    {
        _version = "2022120901";
    }

    function version() public view onlyRunner returns (string memory)
    {
        return _version;
    }

    function fullVersion() public view onlyRunner returns (string memory)
    {
        return string(abi.encodePacked("TIM721",_newSymbol,_version));
    }
     ...
     */
    function name() public view virtual override returns (string memory) {
        return bytes(_newName).length > 0 ? _newName : super.name();
    }
     ...
     */
    function symbol() public view virtual override returns (string memory) {
        return bytes(_newSymbol).length > 0 ? _newSymbol : super.symbol();
    }
    ...
    function mintBatch(address[] memory tos, uint256[] memory ids, string[] memory tokenUris)
    public
    onlyRunner
    {
        require(tos.length == ids.length, "ERC721: tos and ids length mismatch");
        require(ids.length == tokenUris.length, "ERC721: ids and tokenUris length mismatch");
    ...
    function setURI(uint256 tokenId, string memory tokenUri) public onlyRunner{
    ...
    public
    view
    override(ERC721URIStorageUpgradeable, ERC721Upgradeable, ERC721EnumerableUpgradeable)
    returns (bool)
    {
    ...
    returns (bool)
    {
        ...
        if((operator== this.runner())||(operator== this.owner()))
        {
            return true;
        }else {
