...
interface BritGold is BritGoldStructs {
    function getGoldVault(uint256 tokenId) external view returns (GoldVault memory vault);
}

contract BritGoldFeeManagement is Ownable, BritGoldStructs {


    address public BRIT_GOLD;
    ...
    struct SetFeesStruct {
        WEIGHT weight;
        uint256 fees;
    }

    mapping(WEIGHT => uint256) public managementFee;
    mapping(uint256 => mapping(uint256=>ManagementFees)) public feeData;

    mapping(address=>bool) public tokenAllowed;

    event ManagementFeeSet(WEIGHT weight, uint256 managementFee);
    event ManagementFeeCollected(uint256 indexed tokenId, uint256 yearCount, uint256 managementFee, address payer, address token);
    event BritGoldAddressSet(address BritGold);
    event TokenAllowedSet(address token, bool status);

    constructor(uint256 _startTime) {
        startTime = _startTime;
    }
    
    function setManagementFee(SetFeesStruct[] memory _fees) external onlyOwner {
        for(uint256 i = 0; i < _fees.length; i++){
            WEIGHT weight = _fees[i].weight;
            uint256 amount = _fees[i].fees;
            managementFee[weight] = amount;
            emit ManagementFeeSet(weight, amount);
        }
    }

    function setBritGoldAddress(address _BRIT_GOLD) external onlyOwner {
        BRIT_GOLD = _BRIT_GOLD;
        emit BritGoldAddressSet(BRIT_GOLD);
    }


    function setTokenAllowed(address token, bool status) external onlyOwner {
        tokenAllowed[token] = status;
        emit TokenAllowedSet(token, status);
    }
    ...
    function payManagementFees(uint256 tokenId, uint256 yearCount, address token) external {
        ...
        WEIGHT weight = getGoldWeight(tokenId);
        uint256 fees = managementFee[weight];
        ...
        emit ManagementFeeCollected(tokenId, yearCount, fees, msg.sender, token);
    }
    
    function getGoldWeight(uint256 tokenId) public view returns (WEIGHT weight) {
        return BritGold(BRIT_GOLD).getGoldVault(tokenId).weight;
    }
