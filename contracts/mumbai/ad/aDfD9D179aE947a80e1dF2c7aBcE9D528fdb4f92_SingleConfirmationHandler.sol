...
import '../interfaces/IConfirmationHandler.sol';
import '../interfaces/IConfigurationHandler.sol';
import '../interfaces/IStorage.sol';

import 'hardhat/console.sol';

contract SingleConfirmationHandler is IConfirmationHandler, IStorage {
    function confirmProgress(IBillingUnits.Message[] memory _messages) external override {
        console.log('[ConfirmationHandler] entering confirmProgress');

        Metadata storage md = metadata();
        address _bconContract = md.bconContract;

        BillingData storage bd = billingData();

        // ReportConfig laden
        IConfigurationHandler.ReportConfig memory config = IBconContract(_bconContract)
            .getReportConfig();
        require(config.bconContract == _bconContract);

        require(_messages.length > 0, 'empty messagelist');

        for (uint256 i = 0; i < _messages.length; i++) {
            IBillingUnits.Message memory message = _messages[i];

            // minimal check
            require(
                message.msgType == SharedStates.MessageType.ConfirmationOkay ||
                    message.msgType == SharedStates.MessageType.ConfirmationNotOkay ||
                    message.msgType == SharedStates.MessageType.ReportRejected,
                'invalid confirmation type'
            );
            require(msg.sender == _bconContract, 'sender must be bconContract');

            string memory _itemId = message.billingUnitItemId;
            IBillingUnits.BillingUnitItem storage item = bd.billingUnitItems[_itemId];

            require(
                item.state == SharedStates.BillingUnitItemState.CompletionStarted ||
                    item.state == SharedStates.BillingUnitItemState.CompletionClaimed ||
                    item.state == SharedStates.BillingUnitItemState.CompletionPartiallyConfirmed,
                'invalid item state'
            );

            if (bd.billingUnits[item.billingUnitId].origin == SharedStates.Origin.Issue) {
                item.rectificationCounter++;
                if (item.rectificationCounter == config.maxNumberOfRectifications) {
                    console.log(
                        '[ConfirmationHandler] maxNumberOfRectifications reached; exit for item!'
                    );
                    item.state = SharedStates.BillingUnitItemState.CancelledWithIssue;
                    continue;
                }
            }

            item.outstandingReports -= 1;
            item.completionRateClaimed -= message.completionRate;

            if (message.msgType != SharedStates.MessageType.ReportRejected) {
                item.completionRateTotal += message.completionRate;
                item.quantityTotal += message.completionQuantity;

                bd.billingUnits[item.billingUnitId].completionRateClaimed += message.completionRate;
            } else {
                // TODO add functionality for report rejection
                // TODO undo temporary changes from message
                // bd.billingUnitItems[_itemId].state = SharedStates.BillingUnitItemState.CompletionClaimed;
                // if((item.completionRateTotal + item.completionRateClaimed + message.completionRate) == 1000000000) {
                // bd.billingUnitItems[_itemId].state = SharedStates.BillingUnitItemState.CompletionClaimed;
            }

            // 1000000000 = 100%
            if (item.outstandingReports == 0 && item.completionRateTotal == 1000000000) {
                item.state = SharedStates.BillingUnitItemState.CompletionConfirmed;
            } else {
                item.state = SharedStates.BillingUnitItemState.CompletionPartiallyConfirmed;
            }

            if (message.msgType == SharedStates.MessageType.ConfirmationOkay) {
                // positive confirmation

                bd.billingUnits[item.billingUnitId].paymentClaimed += (item.price *
                    message.completionQuantity);

                // throw event in BconContract
                console.log('[ConfirmationHandler] throw BillingUnitItemConfirmed in bconContract');
                IBconContract(_bconContract).throwBillingUnitItemConfirmed(
                    IBconContract.BillingUnitItemEventPayload({
                        billingUnitItem: item,
                        timestamp: block.timestamp,
                        reportId: message.id,
                        sender: md.client,
                        reportType: SharedStates.MessageType.ConfirmationOkay,
                        fileIds: message.fileIds
                    })
                );

                console.log(
                    '[ConfirmationHandler] throw BillingUnitPaymentClaimed in bconContract'
                );
                IBconContract(_bconContract).throwBillingUnitPaymentClaimed(
                    IBconContract.BillingUnitEventPayload({
                        billingUnit: bd.billingUnits[item.billingUnitId],
                        timestamp: block.timestamp,
                        sender: md.client,
                        reportType: SharedStates.MessageType.ConfirmationOkay
                    }),
                    bd.billingUnits[item.billingUnitId].paymentClaimed
                );
            } else if (message.msgType == SharedStates.MessageType.ConfirmationNotOkay) {
                // neue Mängelmeldung

                uint256 totalIssueReductionAmount = message.issueReductionAmount;
                console.log(
                    '[ConfirmationHandler] confirmation not okay; issueReductionAmount is',
                    totalIssueReductionAmount
                );

                //item.state = SharedStates.BillingUnitItemState.RectificationProcess;
                bd.billingUnits[item.billingUnitId].paymentClaimed += ((item.price *
                    message.completionQuantity) - totalIssueReductionAmount);

                // split billing unit by introducing new unit for issue reduction amount
                IBillingUnits(config.billingUnits).splitBillingUnitWithIssue(
                    _itemId,
                    totalIssueReductionAmount
                );

                console.log(
                    '[ConfirmationHandler] throw BillingUnitItemConfirmedWithIssueReductionAmount in bconContract'
                );
                IBconContract(_bconContract).throwBillingUnitItemConfirmedWithIssueReductionAmount(
                    IBconContract.BillingUnitItemEventPayload({
                        billingUnitItem: item,
                        timestamp: block.timestamp,
                        reportId: message.id,
                        sender: md.client,
                        reportType: SharedStates.MessageType.ConfirmationNotOkay,
                        fileIds: message.fileIds
                    }),
                    totalIssueReductionAmount
                );

                // notifiy about oldUnit linked to issueUnitId, new Unit will be signaled by event BillingUnitCreated
                console.log('[ConfirmationHandler] throw BillingUnitSplit in bconContract');
                IBconContract(_bconContract).throwBillingUnitSplit(
                    IBconContract.BillingUnitEventPayload({
                        billingUnit: bd.billingUnits[item.billingUnitId],
                        timestamp: block.timestamp,
                        sender: md.client,
            ...
            // check if payment should be triggered
            console.log('[ConfirmationHandler] trigger checkForPayment in PaymentHandler');
            checkForPayment(config.paymentHandler, item.billingUnitId);

            // Message sichern
            bd.itemMessages[message.billingUnitItemId].push(message);
            IBconContract(_bconContract).throwNewMessage(message);
            console.log('[BillingUnits] added new item', message.billingUnitItemId, message.id);
        }
    }

    function checkForPayment(address paymentHandler, string memory billingUnitId) internal {
        bytes memory rawData = abi.encodeWithSignature('checkForPayment(string)', billingUnitId);
        (bool success, bytes memory results) = paymentHandler.delegatecall(rawData);
        require(success == true, '[ConfirmationHandler] checking for payment failed');
    }
}
...
abstract contract IPaymentHandler {
    function confirmPayment(IBillingUnits.Message[] memory _reports, address _bconContract)
        external
        virtual;

    function checkForPayment(string memory unitId) public virtual;
}
...
abstract contract IConfirmationHandler {
    function confirmProgress(IBillingUnits.Message[] memory _messages) external virtual;
}
