...
interface ISLSystem {
     ...
     *  Combine
     */
    function soulStoneUpgrade(
        BaseStorage.RankType _stoneRank,
        uint256 _requestAmount
    ) external;

    function soulStoneCombine(
        BaseStorage.RankType _stoneRank,
        uint256 _requestAmount,
        bytes[] calldata _normalMonsterSignatures,
        SystemBase.ShadowMonsterSignature[] calldata _shadowMonsterSignatures
    ) external;

    function strangeStoneTransform(
        uint256 _requestAmount,
     ...
     */
    // E - A
    function setRequiredSoulStoneForUpgrade(
        uint256[5] calldata _requiredSoulStones
    ) external;

    // E - S
    function setRequiredSoulStoneForCombine(
        uint256[6] calldata _requiredSoulStones
    ) external;
    ...
    // E - S
    function setPercentageForSoulStone(
        uint256[6] calldata _percentages
    ) external;

    function setRequiredStrangeStoneForTransform(
        uint256 _requiredStrangeStone
    ) external;

    function getRequiredSoulStoneForUpgrade()
        external
        view
        returns (uint256[5] memory);

    function getRequiredSoulStoneForCombine()
        external
        view
    ...
    function getPercentageForSoulStone()
        external
        view
        returns (uint256[6] memory);

    function getRequiredStrangeStoneForTransform()
        external
        view
        returns (uint256);

    function getHunterSoulStoneUpgradeCount(
        address _hunter,
        BaseStorage.RankType _stoneRank
    ) external view returns (uint256);

    function getHunterSoulStoneCombineCount(
        address _hunter,
        BaseStorage.RankType _stoneRank
    ) external view returns (uint256);
    ...
    function monsterSeparateBatch(
        SystemBase.MonsterSet calldata _monsterSet
    ) external;

    function getHunterMonsterSeparateCount(
        address _hunter,
        bool _isShadow,
        BaseStorage.RankType _monsterRank
    ) external view returns (uint256);
     ...
     *  Separate Base
     */
    function setStrangeStoneForSeparate(
        uint256[6] calldata _normalMonsterStrnageStones,
        uint256[3] calldata _shadowMonsterStrangeStones
    ) external;

    function getStrangeStoneForSeparate()
        external
        view
        returns (uint256[6] memory, uint256[3] memory);
    ...
    function setSoulStoneCollectionId(uint256 _soulStoneCollectionId) external;

    function getMonsterCollectionId(
        bool _isShadow
    ) external view returns (uint256);

    function getSoulStoneCollectionId() external view returns (uint256);
...
abstract contract Combine is ISLSystem, SystemBase {
    ...
    function soulStoneUpgrade(
        ...
        ISLMT soulStone = ISLMT(
            projectContract.getTokenContractByCollectionId(
                soulStoneCollectionId
            )
        );

        soulStone.burn(hunter, uint256(_stoneRank), requiredAmount);

        soulStone.mint(hunter, uint256(nextStoneRank), _requestAmount);

        soulStoneUpgradeCount[hunter][_stoneRank] += _requestAmount;

        emit SoulStoneUpgraded({
            hunter: hunter,
    ...
    function soulStoneCombine(
        ...
        uint256 _requestAmount,
        bytes[] calldata _normalMonsterSignatures,
        ShadowMonsterSignature[] calldata _shadowMonsterSignatures
    ) external {
        if (
            _requestAmount != _normalMonsterSignatures.length ||
            _requestAmount != _shadowMonsterSignatures.length
        ) revert InvalidArgument();

        if (_stoneRank >= RankType.B) {
            if (_normalMonsterSignatures.length > 0) revert InvalidArgument();
        } else {
            if (_shadowMonsterSignatures.length > 0) revert InvalidArgument();
        }
        ...
        SoulStoneCombineResult[]
            memory combineResults = new SoulStoneCombineResult[](
                _requestAmount
            );

        if (_stoneRank >= RankType.B) {
            combineResults = _shadowRankSoulStoneCombine(
                hunter,
                _stoneRank,
                _requestAmount,
                _shadowMonsterSignatures
            );
        } else {
            combineResults = _normalRankSoulStoneCombine(
                hunter,
                _stoneRank,
                _requestAmount,
                _normalMonsterSignatures
            );
        }

        soulStoneCombineCount[hunter][_stoneRank] += _requestAmount;

        emit SoulStoneCombined({
            hunter: hunter,
            monsterRank: _stoneRank,
            combineAmount: _requestAmount,
            burnAmount: requiredAmount,
    ...
    function strangeStoneTransform(
        ...
        uint256 requiredAmount = requiredStrangeStoneForTransform *
            _requestAmount;
        ...
        soulStone.burn(hunter, uint256(StoneType.Strange), requiredAmount);

        StrangeStoneTransformResult[]
            memory transformResults = new StrangeStoneTransformResult[](
                _requestAmount
            );
        ...
        for (uint256 i = 0; i < _requestAmount; i = i.increment()) {
            ...
            for (
                ...
                percentage += percentageForSoulStone[RankType(j)];
            ...
            transformResults[i] = StrangeStoneTransformResult({
        ...
        soulStone.mintBatch(hunter, tokenIds, _asUintArray(1, tokenIds.length));

        emit StrangeStoneTransformed({
            hunter: hunter,
            transformAmount: _requestAmount,
            burnAmount: requiredAmount,
    ...
    function _shadowRankSoulStoneCombine(
        address _hunter,
        RankType _stoneRank,
        uint256 _requestAmount,
        ShadowMonsterSignature[] calldata _shadowMonsterSignatures
    ) private returns (SoulStoneCombineResult[] memory) {
        SoulStoneCombineResult[]
            memory combineResults = new SoulStoneCombineResult[](
                _requestAmount
            );
        ...
        ISLMT monsterContract;

        for (uint256 i = 0; i < _requestAmount; i = i.increment()) {
            _verifyRandomSignature(
                _hunter,
                _shadowMonsterSignatures[i].shadowSignature
            );
            _verifyRandomSignature(
                _hunter,
                _shadowMonsterSignatures[i].monsterSignature
            );

            // to solve the stack too deep error
            address hunter = _hunter;
            RankType stoneRank = _stoneRank;
            bytes memory shadowSignature = _shadowMonsterSignatures[i]
                .shadowSignature;
            bytes memory monsterSignature = _shadowMonsterSignatures[i]
                .monsterSignature;
            ...
            uint256[] memory monsters = monsterFactoryContract
                .getMonsterIdOfRankType(stoneRank, isShadow);
            uint256 max = monsters.length - 1;
            uint256 numberForMonster = uint256(keccak256(monsterSignature)) %
                max;

            combineResults[i] = SoulStoneCombineResult({
            ...
            monsterContract.mint(hunter, monsters[numberForMonster], 1);
        }

        return combineResults;
    }

    function _normalRankSoulStoneCombine(
        address _hunter,
        RankType _stoneRank,
        uint256 _requestAmount,
        ...
        for (uint256 i = 0; i < _requestAmount; i = i.increment()) {
            ...
            monsterContract.mint(_hunter, monsters[numberForMonster], 1);
        }
    ...
    function setRequiredSoulStoneForUpgrade(
        ...
        for (uint256 i = 0; i < _requiredSoulStones.length; i = i.increment()) {
            requiredStoneForStoneUpgrade[RankType(i)] = _requiredSoulStones[i];
        }
    }
    ...
    function setRequiredSoulStoneForCombine(
        ...
        for (uint256 i = 0; i < _requiredSoulStones.length; i = i.increment()) {
            requiredStoneForMonsterMint[RankType(i)] = _requiredSoulStones[i];
        }
    }
    ...
    function setPercentageForSoulStone(
        ...
        for (uint256 i = 0; i < _percentages.length; i = i.increment()) {
            ...
            if (totalPercentage > denominator) revert ExceedDenominator();

            percentageForSoulStone[RankType(i)] = _percentages[i];
        }
    ...
    function setRequiredStrangeStoneForTransform(
        ...
        requiredStrangeStoneForTransform = _requiredStrangeStone;
    }
    ...
    function getRequiredSoulStoneForUpgrade()
        ...
        returns (uint256[5] memory)
    {
        uint256[5] memory requiredSoulStones;

        for (uint256 i = 0; i < 5; i = i.increment()) {
            requiredSoulStones[i] = requiredStoneForStoneUpgrade[RankType(i)];
        }

        return requiredSoulStones;
    }
    ...
    function getRequiredSoulStoneForCombine()
        ...
        returns (uint256[6] memory)
    {
        uint256[6] memory requiredSoulStones;

        for (uint256 i = 0; i < 6; i = i.increment()) {
            requiredSoulStones[i] = requiredStoneForMonsterMint[RankType(i)];
        }
        ...
        returns (uint256[6] memory)
    {
        ...
        for (uint256 i = 0; i < 6; i = i.increment()) {
            percentages[i] = percentageForSoulStone[RankType(i)];
        }
    ...
    function getRequiredStrangeStoneForTransform()
        ...
        returns (uint256)
    {
        return requiredStrangeStoneForTransform;
    }
    ...
    function getHunterSoulStoneUpgradeCount(
        ...
        return soulStoneUpgradeCount[_hunter][_stoneRank];
    }
    ...
    function getHunterSoulStoneCombineCount(
        ...
        return soulStoneCombineCount[_hunter][_stoneRank];
    }
}
...
abstract contract Separate is Combine {
    using Unsafe for uint256;
    ...
    function monsterSeparate(
        ...
        uint256 amountPerSeparated = _isShadow
            ? strangeStoneWhenShadowMonsterSeparated[_monsterRank]
            : strangeStoneWhenNormalMonsterSeparated[_monsterRank];

        uint256 strangeStoneAmount = monsterAmount * amountPerSeparated;
        ...
        soulStone.mint(hunter, uint256(StoneType.Strange), strangeStoneAmount);

        monsterSeparateCount[hunter][_isShadow][_monsterRank] += monsterAmount;

        emit MonsterSeparated(
            hunter,
            _monsterRank,
            _isShadow,
            strangeStoneAmount,
    ...
    function monsterSeparateBatch(MonsterSet calldata _monsterSet) external {
        if (!_checkMonsterSet(_monsterSet)) revert InvalidMonster();

        address hunter = _msgSender();

        uint256 strangeStoneAmount;

        if (_monsterSet.normalMonsterIds.length > 0) {
            strangeStoneAmount += _separateMonsterAndCalculateStone(
                hunter,
        ...
        emit MonsterSeparatedBatch(hunter, strangeStoneAmount, _monsterSet);
    }
    ...
    function _separateMonsterAndCalculateStone(
        address _hunter,
        uint256[] calldata _monsterIds,
        uint256[] calldata _monsterAmounts,
        bool _isShadow
        ...
        for (uint256 i = 0; i < monsterRankTypes.length; i = i.increment()) {
            ...
            uint256 amountPerSeparated = isShadow
                ? strangeStoneWhenShadowMonsterSeparated[monsterRank]
                : strangeStoneWhenNormalMonsterSeparated[monsterRank];

            strangeStoneAmount += (amountPerSeparated * monsterAmount);

            monsterSeparateCount[hunter][isShadow][
                monsterRank
            ] += monsterAmount;
        }

        return strangeStoneAmount;
    }
    ...
    function setStrangeStoneForSeparate(
        ...
        for (
            uint256 i = 0;
            i < _normalMonsterStrnageStones.length;
            i = i.increment()
        ) {
            strangeStoneWhenNormalMonsterSeparated[
                RankType(i)
            ] = _normalMonsterStrnageStones[i];
        }
        ...
        for (
            uint256 i = 0;
            i < _shadowMonsterStrangeStones.length;
            i = i.increment()
        ) {
            strangeStoneWhenShadowMonsterSeparated[
                RankType(startRankType + i)
            ] = _shadowMonsterStrangeStones[i];
        }
    }
    ...
    function getStrangeStoneForSeparate()
        ...
        returns (uint256[6] memory, uint256[3] memory)
    {
        uint256[6] memory normalMonsterStones;
        uint256[3] memory shadowMonsterStones;

        for (uint256 i = 0; i < 6; i = i.increment()) {
            normalMonsterStones[i] = strangeStoneWhenNormalMonsterSeparated[
                RankType(i)
            ];
        }
        ...
        for (uint256 i = 0; i < 3; i = i.increment()) {
            shadowMonsterStones[i] = strangeStoneWhenShadowMonsterSeparated[
                RankType(startRankType + i)
            ];
        }

        return (normalMonsterStones, shadowMonsterStones);
    }
    ...
    function getHunterMonsterSeparateCount(
        ...
        return monsterSeparateCount[_hunter][_isShadow][_monsterRank];
    }
}
...
import {Separate} from "./parts/Separate.sol";
import {ISLProject} from "../project/ISLProject.sol";
import {ISLMonsterFactory} from "../monsterFactory/ISLMonsterFactory.sol";
import {ISLRandom} from "../random/ISLRandom.sol";

contract SLSystem is Separate, UUPSUpgradeable {
    function initialize(
        ISLProject _projectContract,
        ISLMonsterFactory _monsterFactoryContract,
        ISLRandom _randomContract,
        uint256 _soulStoneCollectionId,
        ...
        __percentageForSoulStone_init();
        __strangeStoneWhenNormalMonsterSeparated_init();
        __strangeStoneWhenShadowMonsterSeparated_init();

        monsterFactoryContract = _monsterFactoryContract;
        randomContract = _randomContract;

        soulStoneCollectionId = _soulStoneCollectionId;
        normalMonsterCollectionId = _normalMonsterCollectionId;
        shadowMonsterCollectionId = _shadowMonsterCollectionId;

        requiredStrangeStoneForTransform = 5;
    }
    ...
    function __percentageForSoulStone_init() private {
        percentageForSoulStone[RankType.E] = 35_00000;
        percentageForSoulStone[RankType.D] = 25_00000;
        percentageForSoulStone[RankType.C] = 20_00000;
        percentageForSoulStone[RankType.B] = 10_00000;
        percentageForSoulStone[RankType.A] = 7_00000;
        percentageForSoulStone[RankType.S] = 3_00000;
    }

    function __strangeStoneWhenNormalMonsterSeparated_init() private {
        strangeStoneWhenNormalMonsterSeparated[RankType.E] = 5;
        strangeStoneWhenNormalMonsterSeparated[RankType.D] = 10;
        strangeStoneWhenNormalMonsterSeparated[RankType.C] = 20;
        strangeStoneWhenNormalMonsterSeparated[RankType.B] = 40;
        strangeStoneWhenNormalMonsterSeparated[RankType.A] = 80;
        strangeStoneWhenNormalMonsterSeparated[RankType.S] = 160;
    }

    function __strangeStoneWhenShadowMonsterSeparated_init() private {
        strangeStoneWhenShadowMonsterSeparated[RankType.B] = 200;
        strangeStoneWhenShadowMonsterSeparated[RankType.A] = 400;
        strangeStoneWhenShadowMonsterSeparated[RankType.S] = 800;
    }
    ...
    function setSoulStoneCollectionId(
        uint256 _soulStoneCollectionId
    ) external onlyOperator {
        if (projectContract.isActiveCollection(_soulStoneCollectionId))
            revert InvalidCollectionId();

        TokenType tokenType = projectContract.getCollectionTypeByCollectionId(
            _soulStoneCollectionId
        );
    ...
    SystemError
{
    ...
    // collectionId
    uint256 internal soulStoneCollectionId;
    uint256 internal normalMonsterCollectionId;
    uint256 internal shadowMonsterCollectionId;

    // strangeStone
    uint256 internal requiredStrangeStoneForTransform;

    /*
     *  Struct
     */
    struct SoulStoneCombineResult {
    ...
    struct StrangeStoneTransformResult {
     ...
     *  Mapping
     */
    /// @notice RankType to required soul stone count for stone upgrade
    mapping(RankType => uint256) internal requiredStoneForStoneUpgrade; // E-A soulStone

    /// @notice RankType to required soul stone count for monster mint
    mapping(RankType => uint256) internal requiredStoneForMonsterMint; // E-S soulStone

    /// @notice RankType to shadow monster percentage when soul stone combined
    mapping(RankType => uint256) internal percentageForShadowMonster; // B-S shadowMonster

    /// @notice RankType to soul stone percentage when strange stone transformed
    mapping(RankType => uint256) internal percentageForSoulStone; // E-S soulStone

    /// @notice RankType to strange stone count when 1 normal monster separated
    mapping(RankType => uint256)
        internal strangeStoneWhenNormalMonsterSeparated; // E-S normalMonster

    /// @notice RankType to strange stone count when 1 shadow monster separated
    mapping(RankType => uint256)
        internal strangeStoneWhenShadowMonsterSeparated; // B-S shadowMonster

    /// @notice hunter to RankType to soul stone upgrade count
    mapping(address => mapping(RankType => uint256))
        internal soulStoneUpgradeCount;

    /// @notice hunter to RankType to soul stone combine count
    mapping(address => mapping(RankType => uint256))
        internal soulStoneCombineCount;

    /// @notice hunter to to isShadow to RankType to monster separate count
    mapping(address => mapping(bool => mapping(RankType => uint256)))
        internal monsterSeparateCount;

    /*
     *  Event
     */
    event SoulStoneUpgraded(
    ...
    event SoulStoneCombined(
        address indexed hunter,
        RankType indexed monsterRank,
        uint256 combineAmount,
        uint256 burnAmount,
        SoulStoneCombineResult[] combineResults,
        uint256 timestamp
    );

    event StrangeStoneTransformed(
        address indexed hunter,
        uint256 transformAmount,
        uint256 burnAmount,
        StrangeStoneTransformResult[] transformResults,
        uint256 timestamp
    );

    event MonsterSeparated(
        address indexed hunter,
        RankType indexed monsterRank,
        bool indexed isShadow,
        uint256 strangeStone,
        uint256[] monsterIds,
        uint256[] monsterAmounts
    );

    event MonsterSeparatedBatch(
        address indexed hunter,
        uint256 strangeStone,
        MonsterSet separatedMonster
    );
}
