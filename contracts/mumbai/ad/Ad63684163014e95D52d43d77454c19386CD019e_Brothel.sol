...
contract Brothel is ERC721, ERC721Enumerable, AccessControl, ReentrancyGuard {
    ...
    uint256 internal NFTPrice = 10000000000000000; // 0.01 ETH
    uint256 internal MAX_NFT = 500;
    uint256 internal looter = 100 * 10**18;
    uint256 internal levelToMint = 20;
    ...
    mapping(address => bool) internal isStakerAlreadyMint;
    ...
    function mintBrothelWithTavern(uint256 tavernId) external {
        require(
            royalty.addressStakedBalance(msg.sender) >= looter,
            "Only Flag!"
        );
        IRhumProduction.TavernInfos memory tavernInfos = rhumProd
            .getTavernInfos(tavernId);
        require(isStakerAlreadyMint[msg.sender] == false, "Already Mint!");
        require(tavernInfos.level >= levelToMint, "Tavern level too low!");
        require(totalSupply() < MAX_NFT, "Sold out!");
        safeMint(msg.sender);
        isStakerAlreadyMint[msg.sender] = true;
    }
    ...
    function getMAX_NFT() external view returns (uint256) {
        return MAX_NFT;
    }

    function getLevelToMint() external view returns (uint256) {
        return levelToMint;
    }

    function getIsStakerAlreadyMint(address user) external view returns (bool) {
        return isStakerAlreadyMint[user];
    }
    ...
    function setRoyalty(address _royalty)
        external
    ...
    function setTreasuryGuild(address _treasuryGuild)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        treasuryGuild = _treasuryGuild;
    ...
    function setRhumProd(address _rhumProd)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        rhumProd = IRhumProduction(_rhumProd);
    ...
    function setCharacters(address _characters)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        characters = ICharacter(_characters);
