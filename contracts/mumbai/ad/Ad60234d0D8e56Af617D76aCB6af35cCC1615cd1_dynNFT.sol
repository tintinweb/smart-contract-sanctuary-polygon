...
contract dynNFT is ERC721, IERC721Receiver, Ownable {
    ...
    // Tableland-specific data
    uint256 private _flowersTableId;
    uint256 private _tokensTableId;
    string private constant _FLOWERS_TABLE_PREFIX = "flowers";
    string private constant _TOKENS_TABLE_PREFIX = "tokens";
    string private _baseURIString = "http://testnets.tableland.network/query?extract=true&unwrap=true&s=";

    constructor() ERC721("dNFTs", "dNFT") {
        interval = 30; // Hardcode some interval value (seconds)
        lastTimeStamp = block.timestamp;
    }

    function initTables() public {
        _flowersTableId = TablelandDeployments.get().createTable(
            address(this),
            SQLHelpers.toCreateFromSchema(
                "id int primary key," // An ID for the trait row
                "stage text not null," // The traits will be stored here
                "color text not null," // The value of the trait's color (unknown, purple, etc.)
                "cid text not null", // For each trait's image, store the IPFS CID
                _FLOWERS_TABLE_PREFIX // Prefix (human readable name) for the table
            )
        );
        string[] memory values = new string[](3);
        values[0] = "0,'seed','unknown','QmNpAiQZjkoLCb3MRR8jFJEDpw7YWcSSGMPLzyU5rvNTNg'";
        ...
        TablelandDeployments.get().runSQL(
            ...
            SQLHelpers.toBatchInsert(
                ...
                _flowersTableId,
                "id,stage,color,cid", // Columns to insert into
    ...
    function safeMint(address to) public onlyOwner {
        ...
        TablelandDeployments.get().runSQL(
            ...
            SQLHelpers.toInsert(
                _TOKENS_TABLE_PREFIX,
                _tokensTableId,
                "id," // Columns to insert into
                "stage_id", // Intial image stage (i.e., it starts as a seed and then grows)
        ...
        returns (string memory)
    {
         ...
         *
         */
         string memory tokens = SQLHelpers.toNameFromId(_TOKENS_TABLE_PREFIX, _tokensTableId);
         string memory flowers = SQLHelpers.toNameFromId(_FLOWERS_TABLE_PREFIX, _flowersTableId);
