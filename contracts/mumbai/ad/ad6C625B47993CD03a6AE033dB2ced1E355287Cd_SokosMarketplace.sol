...
import {MarketplaceLib} from "./library/MarketplaceLib.sol";

contract SokosMarketplace is
    Initializable,
    OwnableUpgradeable,
    UUPSUpgradeable,
    ...
    ERC721HolderUpgradeable
{
    ...
    mapping(address => mapping(uint256 => mapping(address => MarketplaceLib.Listing)))
        public listings;

    /// @notice NftAddress -> Token ID -> Offer
    mapping(address => mapping(uint256 => MarketplaceLib.Offer)) public offers;
    ...
    function initialize(address _addressRegistry) public initializer {
        ...
        init(payable(_msgSender()), 200, _addressRegistry, _msgSender());
    }
    ...
    function mintSokosTradables(
        ...
        if (mintFee > 0) {
            ...
            AddressUpgradeable.sendValue(feeReceipient, msg.value);
        }
        ...
        IERC1155Upgradeable nft = IERC1155Upgradeable(_nftRegistry);
    ...
    function CreateListing(
        ...
        listings[_nftAddress][_tokenId][_msgSender()] = MarketplaceLib.Listing(
            _quantity,
            _pricePerItem,
            _startingTime,
            _expiresAt,
        ...
        emit MarketplaceLib.ListingCreated(
            _msgSender(),
            _nftAddress,
            _tokenId,
            _quantity,
    ...
    {
        _cancelOffers(_nftAddress, _tokenId);
    ...
    /// @param _nftAddress Address of NFT contract
    /// @param _tokenId Token ID of NFT
    function _cancelOffers(address _nftAddress, uint256 _tokenId) internal {
        MarketplaceLib.Offer memory currentOffer = offers[_nftAddress][
            _tokenId
        ];
    ...
    function updateListing(
        ...
        MarketplaceLib.Listing storage listedItem = listings[_nftAddress][
            _tokenId
        ][_msgSender()];

        listedItem.price = _newPrice;
        emit MarketplaceLib.ListingUpdated(
            _msgSender(),
            _nftAddress,
            _tokenId,
            _newPrice
    ...
    function buyItem(
        ...
        address _payToken,
        uint256 _quantity
    ) external payable nonReentrant isListed(_nftAddress, _tokenId, _owner) {
        MarketplaceLib.Listing memory listedItem = listings[_nftAddress][
            _tokenId
        ][_owner];
        ...
        if (msg.value > 0) {
            _buyItem(_nftAddress, _tokenId, _owner, _quantity, listedItem);
        } else {
            require(
                tokenToFeed[_payToken] != address(0),
    ...
    function _buyItem(
        ...
        address _owner,
        uint256 _quantity,
        MarketplaceLib.Listing memory listedItem
    ) private {
        ...
        // PRICE PAID
        uint256 feeAmount = uint256(msg.value).mul(platformFee).div(1e4);
        AddressUpgradeable.sendValue(payable(feeReceipient), feeAmount);
        ...
        AddressUpgradeable.sendValue(payable(_owner), price);

        uint256 _stock = uint256(listedItem.quantity).sub(_quantity);
        // Transfer NFT to buyer
        _sendNFT(_nftAddress, _tokenId, _msgSender(), _quantity);

        listings[_nftAddress][_tokenId][_owner].quantity = _stock;

        emit MarketplaceLib.ItemSold(
            _owner,
            _msgSender(),
            _nftAddress,
            _tokenId,
    ...
    function _buyItem(
        ...
        uint256 price = listedItem.price.mul(_quantity).mul(10**18);

        require(
            getTokenConversionRate(
                IERC20Upgradeable(_payToken).allowance(
                    _msgSender(),
                    address(this)
                ),
                _payToken
            ) >= (price + price.mul(platformFee).div(1e4)),
            "Marketplace: tokens spend approved not enough"
        );

        uint256 tokensAsPerPrice = (price.mul(10**18)).div(
            getPrice(tokenToFeed[_payToken])
        );
        ...
        IERC20Upgradeable(_payToken).safeTransferFrom(
            _msgSender(),
            feeReceipient,
            (price.mul(platformFee).div(1e4).mul(10**18)).div(
                getPrice(tokenToFeed[_payToken])
            )
        );

        if (_checkRoyalties(_nftAddress)) {
            tokensAsPerPrice = _deduceRoyalties(
                _nftAddress,
                _tokenId,
                tokensAsPerPrice,
                _payToken
        ...
        IERC20Upgradeable(_payToken).safeTransfer(_owner, tokensAsPerPrice);
    ...
    function safePlaceBid(
        ...
        require(
            _owner != _msgSender(),
            "Marketplace: Seller's not allowed to bid"
        );

        MarketplaceLib.Listing memory listedItem = listings[_nftAddress][
            _tokenId
        ][_owner];
        MarketplaceLib.Offer memory offer = offers[_nftAddress][_tokenId];
        ...
        offers[_nftAddress][_tokenId] = MarketplaceLib.Offer(
            _msgSender(),
        ...
        emit MarketplaceLib.BidCreated(
            _msgSender(),
            _nftAddress,
            _owner,
            _tokenId,
    ...
    function cancelOffer(
        ...
        uint256 _tokenId,
        address _nftOwner
    ) internal offerExists(_nftAddress, _tokenId, _nftOwner) {
        MarketplaceLib.Offer memory offer = offers[_nftAddress][_tokenId];
        offer.payToken.safeTransfer(offer.offerer, offer.paidTokens);
        delete (offers[_nftAddress][_tokenId]);
        emit MarketplaceLib.BidCancelled(
            offer.offerer,
            _nftAddress,
            _nftOwner,
            _tokenId
    ...
    /// @param _nftAddress NFT contract address
    /// @param _tokenId TokenId
    function acceptOffer(address _nftAddress, uint256 _tokenId)
        external
        nonReentrant
        offerExists(_nftAddress, _tokenId, _msgSender())
    {
        ...
        uint256 feeAmount = offer.paidTokens.mul(platformFee).div(1e4);
        uint256 price = offer.paidTokens.sub(feeAmount);

        offer.payToken.safeTransfer(feeReceipient, feeAmount);

        if (_checkRoyalties(_nftAddress)) {
            price = _deduceRoyalties(
                _nftAddress,
                _tokenId,
                price,
        ...
        offer.payToken.safeTransfer(_msgSender(), price);

        _sendNFT(_nftAddress, _tokenId, offer.offerer, 1);
        ...
        emit MarketplaceLib.BidAccepted(
            offer.offerer,
            _nftAddress,
            _msgSender(),
            _tokenId,
    ...
    function createCollection(
        ...
        address newCollection = sokosAddressRegistry.createCollection(
            _name,
            _symbol,
            _isPublic
        );
        emit MarketplaceLib.CollectionCreated(newCollection, _name, _symbol);
    }
     ...
     */
    function updatePlatformFee(uint16 _platformFee)
        external
        nonReentrant
        onlyOwner
    {
        platformFee = _platformFee;
        emit MarketplaceLib.UpdatePlatformFee(_platformFee);
    }

    function updatePlatformMintFee(uint256 _mintFee)
        external
        nonReentrant
        onlyOwner
    {
        mintFee = _mintFee;
        emit MarketplaceLib.UpdatePlatformMintFee(_mintFee);
    }
        ...
        onlyOwner
    {
        ...
        emit MarketplaceLib.UpdatePlatformFeeRecipient(_platformFeeRecipient);
    }
    ...
    function withdraw(address _token) public onlyOwner nonReentrant {
        IERC20Upgradeable acceptedToken = IERC20Upgradeable(_token);
    ...
    function addTokenFeed(address _token, address feed) public onlyOwner {
        ...
        tokenToFeed[_token] = feed;
        emit MarketplaceLib.PaymentOptionAdded(_token);
    }

    function removePaymentOption(address _token) public onlyOwner {
        delete tokenToFeed[_token];
        emit MarketplaceLib.PaymentOptionRemoved(_token);
    }
    ...
    function getConversionRate(uint256 maticAmount)
        ...
        returns (uint256)
    {
        uint256 ethPrice = getPrice(sokosAddressRegistry.sokosMaticPriceFeed()); // 262784346 ;
    ...
    {
        require(
            tokenToFeed[_token] != address(0),
            "Marketplace: Token not acceptable as payment"
        );
        uint256 tokenPrice = getPrice(tokenToFeed[_token]); // 262784346 ;
    ...
    function _deduceRoyalties(
        ...
        ) = IERC2981Upgradeable(nft).royaltyInfo(tokenId, grossSaleValue);
        ...
        if (royaltiesAmount > 0) {
            if (_paytoken == address(0)) {
                ...
                IERC20Upgradeable(_paytoken).safeTransfer(
                    royaltiesReceiver,
                    royaltiesAmount
                );
            }
        ...
        emit MarketplaceLib.RoyaltiesPaid(nft, tokenId, royaltiesAmount);
        return netSaleValue;
    }
    ...
    function _checkRoyalties(address _contract) internal view returns (bool) {
        bool success = IERC2981Upgradeable(_contract).supportsInterface(
    ...
    function _cancelListing(
        ...
        delete (listings[_nftAddress][_tokenId][_owner]);
        emit MarketplaceLib.ListingCancelled(_owner, _nftAddress, _tokenId);
    }

    /// @notice Method for returning the NFT to owner
    /// @param _nftAddress Address of NFT contract
    /// @param _tokenId Token ID of NFT
    /// @param benefeciery Initial owner of NFT
    function _returnNftToOwner(
        address _nftAddress,
        uint256 _tokenId,
        address benefeciery
    ) internal {
    ...
    modifier isSokosNFT(address _nft) {
        require(
            sokosAddressRegistry.isSokosNFT(_nft),
            "Marketplace: Invalid NFT address"
        );
        _;
    }
    ...
    modifier isListed(
        ...
        MarketplaceLib.Listing memory listing = listings[_nftAddress][_tokenId][
            _owner
        ];
        require(listing.quantity > 0, "not listed item/sold out");
        _;
    }
    ...
    modifier offerExists(
        ...
        MarketplaceLib.Offer memory offer = offers[_nftAddress][_tokenId];
        require(offer.offerer != address(0), "offer not exists");
        _;
    }
}