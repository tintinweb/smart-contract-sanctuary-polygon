...
contract CreatorDAOUpgradeable is
    Initializable,
    ContextUpgradeable,
    GranularRolesUpgradeable
{
    ...
    event CreatedDAO(string name, address indexed treauryAddress);

    function initialize(
        Config.DAOConfig memory daoConfig,
        RoleAddresses[] memory _roleAddresses
    ) public initializer {
        _name = daoConfig.name;
        _owner = daoConfig.owner;
        _treasuryAddress = daoConfig.treasuryAddress;

        _initRoles(daoConfig.owner, _roleAddresses);

        emit CreatedDAO(_name, _treasuryAddress);
    }
    ...
    function setTreasury(address treasuryAddress_) public onlyRole(ADMIN_ROLE) {
        _treasuryAddress = treasuryAddress_;
    }

    function changeOwner(address owner_) public onlyRole(ADMIN_ROLE) {
        _owner = owner_;
    }

    // DAO should have withdraw function to withdraw funds from wallet
}
