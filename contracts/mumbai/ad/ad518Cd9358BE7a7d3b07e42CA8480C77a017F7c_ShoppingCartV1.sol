...
contract BacanoGoGiveawayV1 is BacanoGoCollection {
    ...
    function mint(address _recipient, uint32 _quantity) public {
        require(
            _msgSender() == owner() || contractWhitelist[_msgSender()],
            "Not authorized"
        );
    ...
    ERC2771ContextUpgradeable
{
    ...
    function contractInfo(
        ...
        returns (
            ...
            string memory,
            string memory,
            DroppableUpgradeable.MintPhase memory
        )
    {
        require(isWhitelisted[contractAddress], "Contract is not whitelisted");
        DroppableUpgradeable droppable = DroppableUpgradeable(contractAddress);
        ERC721Upgradeable erc721 = ERC721Upgradeable(contractAddress);
        (DroppableUpgradeable.MintPhase memory phase, ) = droppable
            .getActiveMintPhase();
        return (
            erc721.name(),
            erc721.symbol(),
            IContractMetadata(contractAddress).contractURI(),
            phase
        );
    }
