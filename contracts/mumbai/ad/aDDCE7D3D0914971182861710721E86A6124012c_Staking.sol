...
contract Staking {
    ...
    enum Status { Empty, Stake, Unstake, Suspend, Ban, Pause }
    ...
    // Parameters
    uint128 public constant VALIDATOR_THRESHOLD = 10 ether; // 10 ether
    ...
    uint64  public constant MAXIMUM_VALIDATORSUBSET_SIZE = 150;
    ...
    // proposer delegation
    uint128 public constant DELEGATION_THRESHOLD = 0.1 ether; // 0.1 ether
    ...
    uint256 public _maximumNumDelegators = 1000;
    ...
    modifier onlyStaker() {
        require(
            _isStaker(msg.sender) || _addressToStakedAmount[msg.sender] > 0,
    ...
    modifier onlyBlockProposer() {
        require(
            ...
            "Only block proposer can call function"
        );
        _;
    }
    ...
    modifier onlyDelegator(address signer) {
        require(
            _addressToDelegatorInfo[msg.sender].validatorToStakedAmount[
                signer
            ] > 0,
    ...
    //     bytes[] memory keys = new bytes[](vLen);
    ...
    function getValidatorsStakeInfo()
        public
        view
        returns (AccountStakingInfo[] memory)
    {
        AccountStakingInfo[] memory rs = new AccountStakingInfo[](
            _validators.length
        );
        for (uint64 i = 0; i < _validators.length; i++) {
            ...
            AccountStakingInfo memory valInfo = AccountStakingInfo(
                _validators[i],
                _addressToStakedAmount[_validators[i]],
                state,
                timeLock
            );
            rs[i] = valInfo;
    ...
    function ban(address badValidator, uint64 validatorsSubsetSize) public onlyBlockProposer {
        _ban(badValidator, validatorsSubsetSize);
    }
    ...
    function suspend(
        ...
        emit Suspended(
            badValidator,
            block.number,
            _validatorsState[badValidator].timeLock,
            _validatorSubsetCounter[indexValidator]
        );
    }
    ...
    function checkStateValidators() public onlyBlockProposer {
    ...
    function _stake() private {
        require(msg.value > VALIDATOR_THRESHOLD, "Amount must be greater than validator threshold");
    ...
    function _unstake() private {
        ...
        uint256 timeLock = _validatorsState[signer].timeLock;
        // uint256 blockPresent = block.number;
        require(timeLock < block.number, "staker is banned or suspended");
    ...
    function _ban(address badValidator, uint64 validatorsSubsetSize) private {
        // add co check max validator subset size
        if (
            validatorsSubsetSize > MINIMUM_VALIDATORSUBSET_SIZE &&
            validatorsSubsetSize <= MAXIMUM_VALIDATORSUBSET_SIZE
        ) {
            //set state == "ban"
        ...
        if (_addressToDelegators[badValidator].length > 0) {
            ...
            for (
                uint64 i = 0;
                i < _addressToDelegators[badValidator].length;
                i++
            ) {
                uint256 amountDelegator = _addressToDelegatorInfo[
                    delegatorInfo[i]
                ].validatorToStakedAmount[badValidator];
                uint256 amountDelegatorSlashing = (amountDelegator *
                    PERCENTAGE_TOKEN_SLASHING) / 100;
                _addressToDelegatorInfo[delegatorInfo[i]]
                    .validatorToStakedAmount[badValidator] =
                    amountDelegator -
                    amountDelegatorSlashing;
    ...
    function delegatorUnstake(
        address signer
    ) public onlyEOA onlyDelegator(signer) {
    ...
    function getTotalDelegatorOfValidator(
        address account
    ) public view returns (uint256) {
        return _addressToDelegators[account].length;
    }

    function getDelegatorsInfoOfValidator(
        address signer
    ) public view returns (AccountStakingInfo[] memory) {
        ...
        for (uint64 i = 0; i < _size; i++) {
            ...
            uint256 delegatedAmount = _addressToDelegatorInfo[delegatorAccount]
                .validatorToStakedAmount[signer];
    ...
    function _delegate(address signer) private {
        require(msg.value > DELEGATION_THRESHOLD, "Amount must be greater than delegator threshold");
        require(
            _isValidator(signer),
            "cannot delegate tokens to someone who is not validator"
        );
    ...
    function _delegatorUnstake(address account, address signer) private {
        uint256 amount = _addressToDelegatorInfo[account]
            .validatorToStakedAmount[signer];
    ...
    function _deleteFromDelegatorsOfValidator(
        address account,
        address signer
    ) private {
        require(
        ...
        require(
            _addressToDelegatorInfo[account].delegatorIndex[signer] <
                _addressToDelegators[signer].length,
    ...
    function _appendToDelegatorSet(address account, address signer) private {
        ...
        _addressToDelegatorInfo[account].delegatorIndex[
            signer
        ] = _addressToDelegators[signer].length;
        _addressToDelegators[signer].push(account);
    }

    function _isDelegator(
        address account,
        address signer
    ...
    function _canBecomeDelegator(
        address account,
        address signer
    ) private view returns (bool) {
        return
            !_isDelegator(account, signer) &&
            _addressToDelegatorInfo[account].validatorToStakedAmount[signer] >=
