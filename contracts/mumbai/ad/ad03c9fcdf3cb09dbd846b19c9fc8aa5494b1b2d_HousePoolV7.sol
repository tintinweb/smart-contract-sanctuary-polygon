    ...
    function getMaxWithdrawal(address _user) public view returns (int256) {
        ...
        if (((userShare * voi.expectedValue)) < userShare * userEVTracker[_user]) {
            maxUserWithdrawalAmount = (userShare) * int256(totalValueLocked) / int256(10**pool_decimals);
             ...
             maxUserWithdrawalAmount =  ((userShare * (int256(totalValueLocked) - voi.expectedValue)) +
                (userShare * userEVTracker[_user]));
            console.log("Max Withdrawal Amount from contract now is:");
            console.logInt(maxUserWithdrawalAmount);
            maxUserWithdrawalAmount = ceil(maxUserWithdrawalAmount,int256(10**(pool_decimals*2))) / int256(10**pool_decimals);
        }
        if (maxUserWithdrawalAmount >= int256(liquidity)) {
            maxUserWithdrawalAmount = int256(liquidity);
        }
        //return ceil(maxUserWithdrawalAmount,int256(10**pool_decimals));
