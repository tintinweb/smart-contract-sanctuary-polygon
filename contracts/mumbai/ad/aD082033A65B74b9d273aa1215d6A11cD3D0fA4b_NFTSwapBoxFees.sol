...
contract NFTSwapBoxFees is INFTSwap,AccessControl {
    ...
    */
    ...
    function _checkPaymentTokenFee(
        ...
        uint256 userDiscount = INFTSwapFeeDiscount(nftSwapFeeDiscount).getUserDiscount(buyer);
        if(_fixedPrice.tokenAddr == address(0)) {
            return _fixedPrice.amount * nativePaymentFee - _fixedPrice.amount * nativePaymentFee * userDiscount / 1000;
        } else {
            return _fixedPrice.amount * _whiteListTokenFee(_fixedPrice.tokenAddr) - _fixedPrice.amount * _whiteListTokenFee(_fixedPrice.tokenAddr) * userDiscount / 10000;
        }
    }
