...
contract Poster is Context {
    ...
    event NewThreshold(uint256 previousThreshold ,uint256 newThreshold);
    event NewTokenAddress(address previousTokenAddress ,address newTokenAddress);
    ...
    function setTokenAddress(address _newTokenAddress) public virtual onlyOwner {
        ...
        emit NewTokenAddress(oldAddress, tokenAddress);
    }

    function setThreshold(uint256 _newThreshold) public virtual onlyOwner {
        uint256 oldfTreshhold = threshold;
        threshold = _newThreshold;
        emit NewThreshold(oldfTreshhold, threshold);
    }
    ...
    function thresholdOf() public view returns (uint256) {
    return threshold;
    }

    function tokenAddressOf() public view returns (address) {
    return tokenAddress;
    }
}
