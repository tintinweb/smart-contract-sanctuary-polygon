...
contract Jackpot is VRFV2WrapperConsumerBase, ConfirmedOwner {
    //Chainlink variables
    ...
    // emitted when someone request to joins a game
    event PlayerRequestJoin(address player, uint256 gameId);
    // emitted when someone joins a game
    event PlayerJoined(uint256 gameId, address player);
    // emettied when VRF send the random value
    event ReceiveRndValue(uint256 gameId, uint256[] rndNumbers);
    ...
    /**
     * @param wrapperAddress address of wrapper contract
    ...
    function joinGame() public payable {
        ...
        emit PlayerRequestJoin(msg.sender, msg.value);
     ...
     * @param _requestId  this ID is unique for the request we sent to the VRF Coordinator
     * @param _randomWords this is a random unit256 generated and returned to us by the VRF Coordinator
     */
    function fulfillRandomWords(
        uint256 _requestId,
        uint256[] memory _randomWords
    ) internal virtual override {
        emit ReceiveRndValue(_requestId, _randomWords);
        ...
        emit GameEnded(gameId, winner, _requestId, _randomWords[0]);
