...
contract MultiCall is KeyEnumBase {
        ...
        returns (uint256 state, bytes32 agentNode, uint256 ordersCost, uint256 registersCost, uint256 reservesCost)
    {
        ...
        if (orders[0].length > 0) {
            (state, ordersCost) = checkOrders(orders[0], false); // for DB
            if (state != 0) {
        ...
        if (orders[1].length > 0) {
            (state, ordersCost) = checkOrders(orders[0], true); // for BUFFER
            if (state != 0) {
                return (state, bytes32(0), 0, 0, 0);
            }
        }
        ...
        )
    {
        ...
        if (owner != address(0)) {
            ...
            uint64 expireR;
            (, takerR, expireR, priceR) = searchOrder(targetNode, owner, BUFFER);
