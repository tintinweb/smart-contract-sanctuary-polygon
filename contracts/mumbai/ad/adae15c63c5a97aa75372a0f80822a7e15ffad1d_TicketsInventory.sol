...
// File contracts/token/ERC1155/TicketsInventory.sol
pragma solidity ^0.8.0;



contract TicketsInventory is ERC1155, Pausable, InventoryAccessControl {
    ...
    function setSigner(address _signer) external onlyAdmin(DEFAULT_ADMIN_ROLE) {
        signer = _signer;
        emit SetSigner(_signer, block.timestamp);
    }

    function getSigner() external view onlyAdmin(DEFAULT_ADMIN_ROLE) returns (address) {
        return signer;
    }

    function setBaseUri(string memory _uri) external onlyAdmin(DEFAULT_ADMIN_ROLE) {
        baseUri = _uri;
    }

    function setTokenTypes(uint256 _tokenTypeId, string memory _tokenType) external onlyAdmin(DEFAULT_ADMIN_ROLE) {
    ...
    function setTokenIdTypes(uint256 _tokenId, uint256 _tokenType) internal {
