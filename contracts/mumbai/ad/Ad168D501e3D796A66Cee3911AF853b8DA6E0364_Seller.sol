...
import "./interfaces/ITractor.sol";
...
contract Seller {
    ...
    ITractor internal tractors;

    /// @notice price of Tractor in USD (Tractor/USD)    
    uint256 public basePrice;

    /// @notice decimals of basePrice. It's better to use decimals not too small
    /// (can be the reason of the zero price in ETH or other expensive token)
    uint8 public baseDecimals;

    /// @notice Official Wrapped Ether 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2
    address public immutable WETH; 
    /// @notice mapping: Address of token => the address of Chainlink aggregator of price 
    /// (token/USD Ethereum Mainnet)
    ...
    constructor(
        ...
        address _aggregatorAddress) {
            ...
            tractors = ITractor(_addressOfTractors);    
    ...
    /**
     * @notice Set the price of Tractor in USD. 
     * Function can be called only by SuperAdmin
     * @param _baseTractors The price of Tractor in USD wich will be set.
     * @param _priceDecimals The decimals of Tractor price.
    */  
    function setThePriceOfTractor(uint256 _baseTractors, uint8 _priceDecimals) external returns (bool){         
        require (controller.isSuperAdmin(msg.sender), "Seller: Only SuperAdmin can set the Price of Tractor");         
        basePrice = _baseTractors; 
    ...
    */
    ...
    /**
     * @notice Get the price of Tractor in proper token.  
     * @param _token Token in wich you want to know the price of Tractor.
     * @param _decimals The decimals of Tractor price.
    */  
        ...
        returns (uint256)
    {
        ...
        uint256 tractorUSDPrice = _scalePrice(basePrice, baseDecimals, _decimals);
        ...
        return tractorUSDPrice * decimals / tokenUSDPrice;
    }    

    /**
     * @notice Get the price of Tractor in ETH.       
    */  
    ...
    */  
    ...
    /**
     * @notice Function to buy Tractor for ETH. 
     * It mints Tractor to the address of msg.sender and send ETH to the Treasure. 
     * Function can be called only by Whitelisted address (SC Controller, _whitelist) 
    */
    function buyForETH(uint256 NFTAmount) public payable returns (bool) {        
        require (controller.isAddressWhitelisted(msg.sender), "Seller: You are not whitelisted, you can't buy Tractor");  
        uint256 amountETH = getETHPrice() * NFTAmount;        
        address to = controller.getTreasureContract();                       
        assert(IWETH(WETH).transferFrom(msg.sender, to, amountETH));
        _mintTractors(msg.sender, NFTAmount);
        return true;
    }

    /**
     * @notice Function to buy Tractor for ERC20-token. 
     * It mints Tractor to the address of msg.sender and send ERC20-tokens to the Treasure. 
     * Function can be called only by Whitelisted address (SC Controller, _whitelist)     
     * @param _tokenToPay Address of the token in which Tractor is bought. 
     * @param _decimals The decimals you want to use in price   
    */
    function buyForToken(address _tokenToPay, uint8 _decimals, uint256 NFTAmount) public returns (bool) {        
        require (controller.isAddressWhitelisted(msg.sender), "Seller: You are not whitelisted, you can't buy Tractor");        
        
        uint256 amountOfToken = getTokenPrice(_tokenToPay, _decimals) * NFTAmount;  
        address payable treasury = payable(controller.getTreasureContract());
        require (IERC20(_tokenToPay).balanceOf(msg.sender) >= amountOfToken, "Insufficient balance to buy Tractor");
        IERC20(_tokenToPay).safeTransferFrom(msg.sender, treasury, amountOfToken);
        _mintTractors(msg.sender, NFTAmount);
        return true;
    }
    ...
    */  
    ...
    //Function will be created after SC Tractor
    function _mintTractors(address _mintTractorTo, uint256 amount) internal returns (bool){
        for(uint i = 0; i < amount; ++i) {
            tractors.mint(_mintTractorTo);
        }
        return true;
    }
}
...
interface ITractor {
    ...
    * _newURL - URL of the token.
    * Requirements:
    * sender must have a NFTManager role.
    */
    function setTokenUri(uint256 tokenId, string memory _newURL) external;
