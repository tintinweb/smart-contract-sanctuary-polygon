...
contract Keeper is AutomationCompatibleInterface, Ownable {
    mapping(uint256 => int256) _prices;
    uint256 public _lastTimestamp;

    uint256 public _lastVariance;
    uint256 public _lastRate;
    uint256 public _lastMu;
    ...
    function setExchange(IExchange exchange) external onlyOwner {
        _exchange = exchange;
    }
        ...
        returns (bool upkeepNeeded, bytes memory performData)
    {
        ...
        int256 lastMu = _factory.getPairMu(token0, token1);
        int256 lastVariance = _factory.getPairVariance(token0, token1);

        int256 rate = int256(_exchange.getAmountOut(token0, token1, 1e18));

        int256 mu = 96e16 * lastMu + 4e16 * rate;
        int256 variance = 96e16 *
            lastVariance +
            4e16 *
            (rate - mu) *
            (rate - mu);

        int256 std = this.calculateStd(variance);
        int256 lm = 0; //this.calculateLm(_p, _m, _c, std, mu);

        return (
            k > 0,
            abi.encode(token0, token1, block.timestamp, lm, mu, variance)
        );
    }

    function calculateStd(int256 variance) external view returns (int256 std) {
        SD59x18 sdVariance = sd(variance);
        SD59x18 std = sdVariance.pow(sd(5e17));

        return SD59x18.unwrap(std);
    }
    ...
    //     int std,
    //     int mu
    // ) external view returns (int256 lm) {
    //     int mu_x_c = mu * c;
    //     int std_x_c = std * c;
    ...
    function performUpkeep(bytes calldata performData) external override {
        (
            ...
            int256 variance
                ...
                performData,
                (address, address, uint256, int256, int256, int256)
            );

        _lastTimestamp = lastTimestamp;

        _factory.updatePairLeverage(token0, token1, lm);
        _factory.updateLastTimestamp(token0, token1, _lastTimestamp);
        _factory.updatePairMu(token0, token1, mu);
        _factory.updatePairVariance(token0, token1, variance);
    }
}
...
interface IFactory {
    ...
    function updatePairMu(address token0, address token1, int256 mu) external;

    function updatePairVariance(
        address token0,
        address token1,
        int256 variance
    ) external;
