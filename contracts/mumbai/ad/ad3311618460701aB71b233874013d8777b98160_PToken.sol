...
contract PToken is VersionedInitializable, IPToken {
  ...
  function prxyBalanceOf(address user) external view returns (uint256) {
    ...
    if(_userState[user].referralCode == 1 && _userState[user].supplyBalance > 0) {
      uint256 prxyInterestAmount = _userState[user].uncollateralBalance.rayMul(_userState[user].prxyInterestRate) * dt;
  ...
  function supply(address user, uint256 amount, uint16 referralCode) external virtual override onlyPool {
    ...
    if (_userState[user].supplyBalance == 0) {
      require(_userState[user].borrowBalance == 0, Errors.MUST_ZERO_BORROW_BALANCE);
  ...
  function borrow(address user, uint256 amount) external virtual override onlyPool {
    ...
    _userState[user].borrowBalance += borrowAmount;
  ...
  function withdraw(address user, uint256 amount) external virtual override onlyPool {
    ...
    if(_userState[user].supplyBalance - withdrawAmount < WadRayMath.RAY) {
      reduxInterest.reduxAmount += _userState[user].supplyBalance - withdrawAmount;
  ...
  function repay(address user, uint256 amount) external virtual override onlyPool {
    ...
    require (_userState[user].borrowBalance >= repayAmount, Errors.INSUFFICIENT_USER_BORROW_BALANCE);
    require (_borrowTotalBalance >= repayAmount, Errors.INSUFFICIENT_TOTAL_BORROW_BALANCE);

    _userState[user].borrowBalance -= repayAmount;
    _borrowTotalBalance -= repayAmount;
  ...
  function liquidate(address user) external virtual override onlyPool {
    ...
    require (_borrowTotalBalance >= _userState[user].borrowBalance, Errors.INSUFFICIENT_TOTAL_BORROW_BALANCE);

    _supplyTotalBalance -= _userState[user].supplyBalance;
    _borrowTotalBalance -= _userState[user].borrowBalance;

    uint256 transferAmount = _userState[user].supplyBalance / WadRayMath.RAY;
    reduxInterest.reduxAmount += _userState[user].supplyBalance - transferAmount * WadRayMath.RAY;
  ...
  /// @inheritdoc IPToken
  function reduxInterestToTreasury(uint256 amount, uint256 treasurySplit) external virtual override onlyPool  {
    _updateReduxInterest();
    
    if(treasurySplit > 0) IERC20(_underlyingAsset).safeTransfer(_treasury, treasurySplit);
    if(amount > 0) reduxInterest.reduxAmount -= amount *  WadRayMath.RAY;
    reduxInterest.reduxTimestamp = uint40(block.timestamp);
  }

  /// @inheritdoc IPToken
  function getReduxInterest() external virtual override returns(uint256, uint256) {
    uint256 elapsedTime = block.timestamp - reduxInterest.lastUpdateTimestamp;
    uint256 amount = (reduxInterest.reduxAmount + _borrowTotalInterestAmount * elapsedTime) / WadRayMath.RAY;
    return (amount, block.timestamp - reduxInterest.reduxTimestamp);
  }
  ...
  function _updateInterestRate(address user) internal {
    if(_userState[user].supplyBalance > 0) {
      ...
      uint256 collateralBalance = _userState[user].borrowBalance.rayMul(POOL.getCollateralRate(_underlyingAsset));
      require(_userState[user].supplyBalance >= collateralBalance, Errors.INVALID_USER_BALANCE);
      _userState[user].uncollateralBalance = _userState[user].supplyBalance - collateralBalance;

      if(_userState[user].referralCode == 1) {
        _userState[user].prxyInterestRate = POOL.getPrxyInterestRate(_underlyingAsset);
        _prxyTotalInterestAmount += _userState[user].uncollateralBalance.rayMul(_userState[user].prxyInterestRate);
      }

      if(_userState[user].borrowBalance > 0) {
        _userState[user].borrowInterestRate = POOL.getBorrowInterestRate(_underlyingAsset);
        _borrowTotalInterestAmount += _userState[user].borrowBalance.rayMul(_userState[user].borrowInterestRate);
      }
    }
  }
  function _updateState(address user) internal {
    ...
    if(_userState[user].supplyBalance > 0) {
      ...
      if(_userState[user].referralCode == 1) {
        uint256 prxyInterestAmount = _userState[user].uncollateralBalance.rayMul(_userState[user].prxyInterestRate);
        _prxyTotalInterestAmount -= prxyInterestAmount;
        _userState[user].prxyBalance += prxyInterestAmount * dt;
      }

      if(_userState[user].borrowBalance > 0) {
        uint256 borrowInterestAmount = _userState[user].borrowBalance.rayMul(_userState[user].borrowInterestRate);
        _borrowTotalInterestAmount -= borrowInterestAmount;
        uint256 reduceAmount = borrowInterestAmount * dt;
