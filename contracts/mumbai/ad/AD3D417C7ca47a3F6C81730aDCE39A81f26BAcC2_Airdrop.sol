...
contract Airdrop is IAccessControlHolder, IAirdrop, WithFees {
    ...
    function claimTokens() external payable override onlyWithFees {
        ...
        uint256 onLockdropPhase2 = lockdrop.walletSpartaLocked(msg.sender);
        uint256 total = amount + onLockdropPhase2;
        uint256 onVesting = total / 2;
        uint256 toSend = total - onVesting;
        ...
        if (!sparta.transfer(msg.sender, toSend)) {
            revert TransferFailed();
        }
        if (!sparta.approve(address(vesting), onVesting)) {
