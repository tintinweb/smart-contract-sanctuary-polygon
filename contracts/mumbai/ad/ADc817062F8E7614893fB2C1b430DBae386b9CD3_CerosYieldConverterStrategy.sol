...
contract CerosYieldConverterStrategy is BaseStrategy {
    ...
    function _deposit(uint256 amount) internal returns (uint256 value) {
        ...
        (uint256 amountOut, bool enoughLiquidity) = ISwapPool(_swapPool).getAmountOut(true, amount, true); // (amount * ratio) / 1e18
        if (enoughLiquidity && amountOut >= (amount * _certToken.ratio()) / 1e18) {
    ...
    function _withdraw(uint256 amount) internal returns (uint256 value) {
        ...
        if(amount < wethBalance) {
            ...
            uint256 fee = _assessSwapFee(amount);
            return amount - fee;
        }

        // unstakeFeeAmt = (amount * ISwapPool(_swapPool).unstakeFee()) / ISwapPool(_swapPool).(FEE_MAX);
        
        (uint256 amountOut, bool enoughLiquidity) = ISwapPool(_swapPool).getAmountOut(false, ((amount - wethBalance ) * _certToken.ratio()) / 1e18, true); // (amount * ratio) / 1e18
        if (enoughLiquidity) {
            value = _ceRouter.withdrawWithSlippage(address(this), amount - wethBalance, amountOut);
            uint256 fee;
            if(wethBalance > 0) {
                fee = _assessSwapFee(amount);
            }
            amount = wethBalance + value - fee ;
            underlying.transferFrom(address(this), address(vault), amount);
            return amount;
        }
    }
    ...
    //         return amount;
    //     }
    //     // unstakeFeeAmt = (amountIn * unstakeFee) / FEE_MAX;
    //     (uint256 amountOut, bool enoughLiquidity) = ISwapPool(_swapPool).getAmountOut(false, ((amount - wethBalance ) * _certToken.ratio()) / 1e18, true); // (amount * ratio) / 1e18
    //     if (enoughLiquidity && (amountOut + 100 >= amount)) {
    //         value = _ceRouter.withdrawWithSlippage(address(this), amount - wethBalance, amountOut);
    ...
    function _assessSwapFee(uint256 amount) private returns(uint256) {
        if(ISwapPool(_swapPool).unstakeFee() > 0) {
            uint256 swapFee =  (amount * ISwapPool(_swapPool).unstakeFee()) / ISwapPool(_swapPool).FEE_MAX();
            underlying.transfer(address(_rewardsPool), amount);
...
interface ISwapPool {
    ...
    function getAmountOut(
        ...
        external returns(uint amountOut, bool enoughLiquidity);
    
    function unstakeFee() external returns (uint24);

    function FEE_MAX() external returns (uint24);
}
