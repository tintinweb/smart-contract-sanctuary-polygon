...
contract Minter is AccessControlEnumerableUpgradeable, Provenance, PaymentSplitterUpgradeable {
    ...
    //need an additional function that allows the server to mint an nft to a user for a price of zero eth.
    //is it possible for us to make it so you cannot change the tokenUri for a token after it has been set unless
    //a boolean has been enabled.  thinking this could be an extra security feature if the server/dev makes a mistake
