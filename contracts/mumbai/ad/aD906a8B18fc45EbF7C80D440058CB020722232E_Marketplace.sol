...
contract Marketplace is AccessControl, ReentrancyGuard, Pausable, MarketplaceI {
  ...
  /// @notice Honey protocol Hivemind
  ...
  function bidSingle(
    ...
    if (transferAmount < viewMinimumNextBidSingle(_hToken, _collateralId)) revert LiquidatorError(Error.AUCTION_BID_TOO_LOW);
  ...
  function increaseBidSingle(
    ...
    if (transferAmount + currentBid < viewMinimumNextBidSingle(_hToken, _collateralId)) revert LiquidatorError(Error.AUCTION_BID_TOO_LOW);

    tokenToTotalBids[underlying] += transferAmount;

    addToAuctionQueue(poolToTokenToAuction[_hToken][_collateralId], _hToken, msg.sender, transferAmount + currentBid, false);
   ...
   */
  function increaseBidCollection(HTokenI _hToken, uint256 _increaseAmount) external override nonReentrant whenNotPaused {
    if (!hivemind.marketExists(_hToken)) revert Uninitialized();
    if (poolToAuctionDisabled[_hToken]) revert LiquidatorError(Error.AUCTION_NOT_ACTIVE);
    ...
    IERC20 underlying = _hToken.underlyingToken();
    uint256 transferAmount = doUnderlyingTransferIn(underlying, msg.sender, _increaseAmount);
    if (transferAmount + currentBid < viewMinimumNextBidCollection(_hToken)) revert LiquidatorError(Error.AUCTION_BID_TOO_LOW);
  ...
  function settleAuction(
    ...
    if (singleAuction.highestBidder != address(0) || collectionAuction.highestBidder != address(0)) {
      ...
      settleType = (singleAuction.highestBid >= collectionAuction.highestBid) ? SettleType.SINGLE_TOKEN : SettleType.COLLECTION;
    } else {
  ...
  function viewMinimumNextBidSingle(HTokenI _hToken, uint256 _collateralId) public view override returns (uint256) {
    ...
    uint256 minimumBid;
    uint256 debtWithPadding = (_hToken.getDebtForCollateral(_collateralId) * (reservePricePaddingMantissa + 1e18)) / 1e18;
    if (firstNonZero == 0) {
  ...
  /**
   * @notice  Set the Hivemind contract address
   * @param   _newHivemind The new Hivemind address
   */
  function _setHivemind(HivemindI _newHivemind) external onlyRole(DEFAULT_ADMIN_ROLE) {
    if (address(_newHivemind) == address(0)) revert WrongParams();
  ...
  event SingleBidEntered(address indexed _hToken, address indexed _user, uint256 indexed _collateralId, uint256 _amount);
  event SingleBidIncreased(address indexed _hToken, address indexed _user, uint256 indexed _collateralId, uint256 _amount);
  ...
  event HivemindUpdated(HivemindI _oldHivemind, HivemindI _newHivemind);
