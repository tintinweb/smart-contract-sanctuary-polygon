...
contract PerpetualDepositManager is
    ...
    IPerpetualDepositManager
{
     ...
     */
    function deposit(uint24 _iPerpetualId, int128 _fAmount) external override nonReentrant {
        ...
        LiquidityPoolData storage liqPool = liquidityPools[perpetual.poolId];
        ...
        emit TokensDeposited(_iPerpetualId, traderAddr, _fAmount);
    }

    /**
     * Deposit to the default fund.
     * No choice of collateral.
     * @param   _poolId      Reference to liquidity pool
     * @param   _fAmount     The amount of collateral in 'collateral currency' to deposit.
     */
    function depositToDefaultFund(uint8 _poolId, int128 _fAmount) external override onlyOwner {
        ...
        _transferFromUserToVault(pool.marginTokenAddress, msg.sender, _fAmount);
    ...
    /**
     * To become a broker, deposit to the default fund.
     * No choice of collateral.
     * @param   _poolId   Reference to liquidity pool
     * @param   _iLots    Number of collateral units as specified per liquidity pool
     *                    of 'collateral currency' to deposit. Integer number
     *                    (not ABDK)
     */
    function brokerDepositToDefaultFund(uint8 _poolId, uint32 _iLots) external override {
        require(_poolId <= iPoolCount, "pool index out of range");
        require(_iLots > 0, "invalid lots");
        PerpStorage.LiquidityPoolData storage pool = liquidityPools[_poolId];
        int128 fAmount = pool.fBrokerCollateralLotSize.mul(int128(int32(_iLots)) * ONE_64x64);
        _transferFromUserToVault(pool.marginTokenAddress, msg.sender, fAmount);
        pool.fDefaultFundCashCC = pool.fDefaultFundCashCC.add(fAmount);

        // add to broker credentials
        uint32 brokerLots = brokerMap[_poolId][msg.sender];
        // first time this broker deposits?
        if (brokerLots == 0) {
            uint16 _feeTbps = _getBrokerFeeLogic().getFeeForBrokerDesignation(_iLots);
            _getBrokerFeeLogic().setInitialVolumeForFee(_poolId, msg.sender, _feeTbps);
        }
        brokerMap[_poolId][msg.sender] = brokerLots + _iLots;

        emit UpdateBrokerAddedCash(_poolId, _iLots, brokerLots + _iLots);
        emit UpdateDefaultFundCash(_poolId, fAmount, pool.fDefaultFundCashCC);
    }

    /**
     * Withdraw from the default fund. Only allowed if pool not running.
     * No choice of collateral token
     * @param   _poolId      Reference to liquidity pool
    ...
    function _deposit(
        ...
        uint24 iOtherPerpId;
        (, iOtherPerpId) = _selectPerpetualIds(_perpetual.id, _perpetual.poolId);
        _getUpdateLogic().updateFundingAndPricesBefore(_perpetual.id, iOtherPerpId);
        // main part
        ...
        _getUpdateLogic().updateFundingAndPricesAfter(_perpetual.id, iOtherPerpId);
    }
     ...
     *
     */
    function transferEarningsToTreasury(uint8 _poolId, int128 _fAmount)
        external
        override
        onlyOwner
    {
    ...
    function getFunctionList() external pure virtual override returns (bytes4[] memory, bytes32) {
        ...
        functionList[4] = this.brokerDepositToDefaultFund.selector;
