...
contract Mintiero is ERC1155, Ownable {
    ...
    function mint(address _userWallet, uint256 _quantity, uint256 _tokenId) public onlyCrossmint {
        require(this.availableTokens(msg.sender, _tokenId) >= _quantity, "Requested number of tokens is over limit for minting");

        uint256 totalPrice = tokenPrices[_tokenId] * _quantity;
        tokenInstance.transferFrom(msg.sender, address(this), totalPrice / 10**11);
