...
contract RentingContractStorage is Context, IRentingContract, RBAC {
    ...
    function createListingInfo(BattleSet memory bs, RentingType rt, address owner, Coin coin, uint256 _price, bool _perpetual, address[] memory whitelist) external onlyManager {
        _createListingInfo(bs, rt, owner, coin, _price, _perpetual, whitelist);
    }

    function _createListingInfo(BattleSet memory bs, RentingType rt, address owner, Coin coin, uint256 _price, bool _perpetual, address[] memory whitelist) private {
    ...
    function createRenting(BattleSet memory bs, RentingType rt, Coin coin, uint256 price, address owner, address renter,
        uint256 rentingEnd, uint256 collectionId, bool perpetual, address[] memory whitelist) external onlyManager {
    ...
    function createCollection(address assetsOwner, uint256[] memory landIds, uint256[] memory botIds,
        ...
        collection.perpetual = _perpetual;
    ...
    function processCollectionRentalEnd(RentingInfo memory ri) private {
        ...
        if (collection.disbandTs != 0 && collection.rentedLandIds.length == 0) {
            ...
            delete allCollections[ri.collectionId];
        }
    }
    ...
    function completeRenting(uint256 landId) external onlyManager {
        ...
        if (ri.collectionId != 0) {
            processCollectionRentalEnd(ri);
        } else {
            if (ri.cancelTs == 0 && ri.perpetual) {
                _createListingInfo(ri.battleSet, ri.rentingType, ri.owner, ri.chargeCoin, ri.price, ri.perpetual, ri.whitelist);
            } else {
