...
interface CollectionType1 {
    // RUM, TMAP, Doubloon, FLagship, DPS
    function mint(address to, uint256 amount) external;
}

interface CollectionType2 {
    // artifact, supportShip, lockedBox
    function mint(address addr, uint256 assetType, uint256 amount) external;
}

interface CollectionType3 {
    // Voyage
    function mint(address owner, uint256 tokenId, VoyageConfigV2 calldata config) external;
}

interface CollectionType4 {
    // chest
    function mint(address addr, uint256 assetType, uint256 amount) external;
}
...
contract Airdrop is Ownable {
    event AirdropCollectionType1(uint256 index, address owner, uint256 amount);

    event AirdropCollectionType2(uint256 index, address owner, uint256 assetType, uint256 amount);

    event AirdropCollectionType3(uint256 index, address owner, uint256 assetType, uint256 boughtAt);

    event AirdropCollectionType4(uint256 index, address owner, uint256 assetType, uint256 amount);

    event Gas(uint256 gas);

    function airdropCollectionType1(address collection, ParamType1[] calldata params) external onlyOwner {
        unchecked {
            uint256 i;
            uint256 oldGas = gasleft();
            CollectionType1(collection).mint(params[i].owner, params[i].amount);
            ++i;
            uint256 gas = oldGas - gasleft();

            uint256 minimum = params.length < gasleft() / gas ? params.length : gasleft() / gas;
            console.log(minimum);
            ...
            emit AirdropCollectionType1(i - 1, params[i - 1].owner, params[i - 1].amount);
        }
    }

    function airdropCollectionType2(address collection, ParamType2[] calldata params) external onlyOwner {
        unchecked {
            uint256 i;
            uint256 oldGas = gasleft();
            CollectionType2(collection).mint(params[i].owner, params[i].assetType, params[i].amount);
            ++i;
            uint256 gas = oldGas - gasleft();
            uint256 minimum = params.length < gasleft() / gas ? params.length : gasleft() / gas;
            for (i; i < minimum; ++i) {
            ...
            emit AirdropCollectionType2(i - 1, params[i - 1].owner, params[i - 1].assetType, params[i - 1].amount);
        }
    }

    function airdropCollectionType3(address collection, ParamType3[] calldata params) external onlyOwner {
        unchecked {
            uint256 i;
            uint256 oldGas = gasleft();
            CollectionType3(collection).mint(params[i].owner, params[i].tokenId, params[i].config);
            ++i;
            uint256 gas = oldGas - gasleft();
            uint256 minimum = params.length < gasleft() / gas ? params.length : gasleft() / gas;
            for (i; i < minimum; ++i) {
            ...
            emit AirdropCollectionType3(i - 1, params[i - 1].owner, params[i - 1].tokenId, params[i - 1].config.boughtAt);
        }
    }

    function airdropCollectionType4(address collection, ParamType4[] calldata params) external onlyOwner {
        unchecked {
            uint256 i;
            uint256 oldGas = gasleft();
            CollectionType4(collection).mint(params[i].owner, params[i].assetType, params[i].amount);
            ++i;
            uint256 gas = oldGas - gasleft();
            uint256 minimum = params.length < gasleft() / gas ? params.length : gasleft() / gas;
            for (i; i < minimum; ++i) {
            ...
            emit AirdropCollectionType4(i - 1, params[i - 1].owner, params[i - 1].assetType, params[i - 1].amount);
        }
    }
}
