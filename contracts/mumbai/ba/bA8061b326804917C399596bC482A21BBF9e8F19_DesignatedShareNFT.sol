...
contract DesignatedShareNFT is ERC721 {
    ...
    function transferShare(address _from, uint256 _tokenId, address _to, uint256 _amount) external {
        ...
        if (getShareExit(_tokenId, _to)) {
            ...
            uint256  index = getShareIndex(_tokenId, _to);
            tokenIdToShares[_tokenId][index] = share;
        } else {
            Share memory newShare = Share({
                owner : _to,
                amount : _amount
        ...
        uint256  index = getShareIndex(_tokenId, _from);
        tokenIdToShares[_tokenId][index] = share;

        _transfer(_from, _to, _tokenId);
    }

    function existTokenId(uint256[] memory _tokenIds, uint256 _tokenId) public pure returns (bool) {
    ...
    function getTokenIdCount(uint256 _tokenId, address owner, uint256 _amount) public view returns (bool){
    ...
    function getShareExit(uint256 _tokenId, address owner) public view returns (bool){
        Share[] memory shares = tokenIdToShares[_tokenId];
        for (uint i = 0; i < shares.length; i++) {
            if (shares[i].owner == owner) {
                return true;
    ...
    function getShareEntity(uint256 _tokenId, address owner) public view returns (Share memory){
        Share  memory share;
    ...
    function getShareIndex(uint256 _tokenId, address owner) public view returns (uint256){
        uint256   index;
        Share[] memory shareList = tokenIdToShares[_tokenId];
        for (uint i = 0; i < shareList.length; i++) {
            if (shareList[i].owner == owner) {
