...
error ForceNFTControlUserBlocked(address _user);
...
import "./interfaces/IBurnableEnergyCoin.sol";
...
contract Control is Initializable, Proxied, Governed, IControl {
    ...
    mapping(address => bool) private blockedUsers;
    ...
    modifier notBlockedUser() {
        if (blockedUsers[msg.sender]) {
            revert ForceNFTControlUserBlocked(msg.sender);
        }
        _;
    }
    ...
    function blockUsers(address[] memory _user) external onlyGovernor {
        for (uint256 i; i < _user.length; ) {
            blockedUsers[_user[i]] = true;
            unchecked {
                ++i;
            }
        }
    ...
    function unblockUsers(address[] memory _user) external onlyGovernor {
        for (uint256 i; i < _user.length; ) {
            blockedUsers[_user[i]] = false;
            unchecked {
                ++i;
            }
        }
    ...
    function getEmptyNFT() external override returns (uint256) {
        if (alreadyReceivedEmpty[msg.sender]) {
        ...
        alreadyReceivedEmpty[msg.sender] = true;
        ...
        if (nftEnergy + amounts[last] < getPriceUpToFirstLevel) {
            ...
            for (uint256 i; i < amountMetaProject; ) {
                ...
                if (energyConversionFactor != 0) {
                    IBurnableEnergyCoin energyCoin = IBurnableEnergyCoin(registry.getEnergyToken(i));
                    amount = amounts[i].mul(energyConversionFactor);
                    if (nftEnergy + amount < getPriceUpToFirstLevel) {
                        // require(
                        //     IEnergyCoin(energyCoin).transferFrom(msg.sender, address(this), amounts[i]),
                        //     "Transfer failed"
                        // );
                        IBurnableEnergyCoin(energyCoin).burnFrom(msg.sender, amounts[i]);
                        nftEnergy += amount;
                    } else {
                        uint256 burnable = (getPriceUpToFirstLevel - nftEnergy).div(energyConversionFactor);
                        // require(
                        //     IEnergyCoin(energyCoin).transferFrom(msg.sender, address(this), burnable),
                        //     "Transfer failed"
                        // );
                        IBurnableEnergyCoin(energyCoin).burnFrom(msg.sender, burnable);
...
interface IBurnableEnergyCoin is IERC20 {
...
interface IMetaProject {
    function getLevel(address _idUser) external view returns (uint256);
}
