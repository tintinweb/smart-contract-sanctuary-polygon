...
contract GamespadLand is ERC721Psi, AccessProtected {
    ...
    mapping(uint256 => uint256) public _coordsToId;    // {x,y} => nftId mapping
    mapping(uint256 => uint256) private _idToCoords;    // nftId => {x,y} mapping

    constructor(string memory _name, string memory _symbol, string memory _uri) ERC721Psi(_name, _symbol) {
        baseURI = _uri;
    }
     ...
     * @param y - y coordinate of property
     */
    function formatCoordinates(uint256 x, uint256 y) public pure returns (uint256) {
