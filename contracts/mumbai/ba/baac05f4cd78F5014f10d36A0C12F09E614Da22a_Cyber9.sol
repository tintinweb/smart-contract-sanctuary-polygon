...
contract Cyber9 is ERC721Enumerable, Ownable, RNG, VRFConsumerBase {
    ...
    function levelUp(uint256 _tokenId) external status(_tokenId) {
        require(characters[_tokenId].level < MAX_LEVEL+1, "C");
        require(characters[_tokenId].hp > 0, "D");
        require(msg.sender == ownerOf(_tokenId), "B");
    ...
    function spawnEnemy() external {
        ...
        require(isSpawned[msg.sender] == false, "G");
    ...
    function attackEnemy(uint256[] memory _tokensId) external {
        require(isSpawned[msg.sender] == true, "G");

        for(uint256 i = 0; i<_tokensId.length;i++) {
            require(characters[_tokensId[i]].hp > 0, "D");
            require(msg.sender == ownerOf(_tokensId[i]), "B");
            ...
            uint256 _enemyDmg = uint256(roll(enemy[msg.sender].attack, 2));
            ...
            characters[_tokensId[i]].experience += ((characters[_tokensId[i]].level * 400) - expRequired(_tokensId[i])) + walletOfOwner(msg.sender).c9Bonus();
            ...
            if(enemy[msg.sender].health <= 0 ) {    
                cyber9token.mintBei(msg.sender, enemy[msg.sender].attack * 20);
                emit EnemyDefeated(msg.sender, _tokensId[i], enemy[msg.sender].attack * 20);
    ...
    function stake(uint256[] memory _tokenIds) external {
        ...
        for(uint256 i = 0; i<_tokenIds.length; i++) {
            ...
            require(msg.sender == ownerOf(_tokenIds[i]), "B");
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId) public virtual override status(tokenId) {
        require(characters[tokenId].hp > 0, "D");
