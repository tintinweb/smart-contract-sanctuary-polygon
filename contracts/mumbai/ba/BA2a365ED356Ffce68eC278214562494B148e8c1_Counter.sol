...
// File: contracts/recent/Automation.sol
...
contract Counter is AutomationCompatible {
    ...
    event LogCounter(uint currentCounter);
    ...
    function triggerStart() external {
        start = !start;
    }
    ...
    function buyTicket() external {
        ticketCounter += 1;
        ticketsIds.push(ticketCounter);
    }

    function getTicketsCount() external view returns(uint256 count) {
        ...
        returns (bool upkeepNeeded, bytes memory /* performData */)
    {
        ...
        bool isThereAnyTickets = ticketsIds.length > 0; 
        upkeepNeeded = timestampCheck && start && isThereAnyTickets;
    ...
    function performUpkeep(bytes calldata /* performData */) external override {
        ...
        bool timestampCheck = (block.timestamp - lastTimeStamp) > interval;
        bool isThereAnyTickets = ticketsIds.length > 0; 
        bool trigger = timestampCheck && start && isThereAnyTickets;
