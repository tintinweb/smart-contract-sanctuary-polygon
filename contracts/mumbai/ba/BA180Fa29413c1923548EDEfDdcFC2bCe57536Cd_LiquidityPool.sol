    ...
    LiquidStorage
{
     ...
     */
    function setUserDepositData(uint256 _poolID, uint256 _timePeriod, address _userAddress) internal {
        LiquidTypes.UserInfo storage user = LiquidStorage.userInfo[_poolID][
            msg.sender
        ];
        ...
        LiquidStorage.providersAdressess[LiquidStorage.numberOfLiquidityProviders] = _userAddress;
        LiquidStorage.numberOfLiquidityProviders += 1;
        LiquidStorage.listLPProviders.push(msg.sender);
    }
    ...
    function swapTokens(uint256 _poolID, uint256 _purchaseAmount) internal {
        ...
        console.log("Tax reward wallet to receive: " , taxAmount);
    ...
    function sellTokens(
        ...
        if (_sellAll) {
            ...
            console.log("Tax reward wallet to receive on sell: " , calculateTaxPercent(
                _poolID,
                _sellAmount,
                false
            ));
    ...
    function calculateTaxPercent(
        ...
        if (_withdraw) {
            console.log("It is withdraw and this is info: ");
            console.log("Withdraw tax: " , taxPairs.withdrawTax);
            console.log("Amount withdraw: ", _amount);
            taxableAmount = (_amount.mul(taxPairs.withdrawTax).div(100));
            return taxableAmount;
        } else {
            console.log("It is deposit and this is info: ");
            console.log("Deposit tax: " , taxPairs.depositTax);
    ...
    function splitBetweenWallets(
        uint256 _poolID,
        uint256 _walletID,
        uint256 _amountToSplit
    ) internal {
        LiquidTypes.Wallet storage wallets = LiquidStorage.wallets[_walletID];
        LiquidTypes.PoolPair storage pairs = LiquidStorage.poolPairs[_poolID];
        ...
        //     _amountToSplit
        // );
        wallets.amountHeld+=_amountToSplit;
    }
    ...
    function addNewWallet(
        ...
        console.log(_walletName);
        // LiquidStorage.wallets.push(
        //     LiquidTypes.Wallet({
        //         id: _walletId,
        //         amountHeld: 0,
    ...
    LiquidCalculate
{
    ...
    /**
     * @notice Changes the operations wallet for the pool
     * @param _operationsWallet The wallet address of the operations wallet 
     * @dev The operations wallet is not set by default, after deployment the operations wallet must be set.
     */

    function changeOperationsWallet(address _operationsWallet) external onlyOwnerr {
        
        if(_operationsWallet == address(0)) revert ("Reward wallet can't be null address");

        LiquidStorage.operationsWallet = _operationsWallet;
        emit LiquidEvents.operationWalletChanged(_operationsWallet);
    }
     ...
     */
    function changeRewardWallet(uint256 _poolID, address _newWallet) external onlyOwnerr {
        ...
        emit LiquidEvents.rewardWalletChanged(_newWallet);
    ...
    * @dev Used mainly for testing purposes, but can be used by admins for lower costs, doesn't do extra calculations
    ...
    function deposit(
        ...
        LiquidCalculate.setUserDepositData(_poolID, _months, msg.sender);
    ...
    function returnLiquidityProviders() external view returns(address[] memory) {
        return LiquidStorage.listLPProviders;    
    }

    /**
     * @notice Updates the payment interval for reward payouts
     * @param _poolID The id of pool whose payment interval is getting updated
     * @param _paymentInterval the interval of payments, can be 1, 4, 12
     * @dev 1 represents monthly payouts, 4 quarterly and 12 yearly. 
     */
    function setPaymentInterval(uint256 _poolID, uint256 _paymentInterval) external onlyOwnerr{
        LiquidTypes.PoolPair storage pairs = LiquidStorage.poolPairs[_poolID];

        pairs.paymentInterval = _paymentInterval;
    }

    // function splitBetweenWallet(uint256 _poolID, uint256 _walletId, uint256 _amount) external onlyOwnerr {
    //     LiquidCalculate.splitBetweenWallets(_poolID, _walletId, _amount);
    // }
    // function addNewWallets(uint256 _walletId, string memory _walletName, address _walletAddr) external{
    //     LiquidCalculate.addNewWallet(_walletId, _walletName, _walletAddr);
    // }
    // function getOtherWalletBalances(uint256 _walletId) external onlyOwnerr returns(uint256)  {
    //     LiquidTypes.Wallet storage wallets = LiquidStorage.wallets[_walletId];
    //     return wallets.amountHeld;
    // }
}
...
library LiquidEvents { 
    ...
    // Event for paying deposit tax
    event DepositTax(uint256 indexed pid, uint256 newTax);

    // Event for paying withdraw tax
    event WithdrawTax(uint256 indexed pid, uint256 newTax);

    // Event for changing reward wallet
    event rewardWalletChanged(address _newRewardAddress);

    // Event for changing operations wallet
    event operationWalletChanged(address _newOperationWallet);
}
...
contract LiquidStorage {
    ...
    // Mapping of providers containing all user addresess 
    mapping(uint256 => address) public providersAdressess;
    ...
    // Address of the operations wallet.
    address public operationsWallet;
    
    uint256 public constantProductMarketMaker;

    // The number of liquidity providers (Users, used to retrieve all users that are providers)
    uint256 public numberOfLiquidityProviders = 0;
    
    // Array of LiquidityProviders
    address[] public listLPProviders;

    LiquidTypes.TaxInfo[] public taxPairs;

    LiquidTypes.Wallet[] public wallets;
}
