...
interface ISport {
    ...
    function mintMore(uint256 amount) external returns (bool);
}
...
contract ESkillzStraightBet is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    function initialize(address _sport) public initializer {
        sport = _sport;  
        eskillz_fee = 500; 
    ...
    function SetSPGameResult(uint256 GameID, uint256 result) external {
        ...
        ISport(sport).mintMore(gamebetting[GameID][0].amount.mul(result).div(100));
     ...
     function SetSPGameResultdasi(uint256 GameID, uint256 result) external {
        
        (uint256 amountToWinner,uint256 amountToESkillz) = getAmountsSPGameToDistribute(GameID,result);
        require(gamebetting[GameID][0].player == msg.sender, "Other Players can not access.");
        require(gamebetting[GameID][0].gameType == 0, "You can set the SP game only");
        //ISport(sport).mintMore(gamebetting[GameID][0].amount.mul(result).div(100));
        ISport(sport).transfer(gamebetting[GameID][0].player, gamebetting[GameID][0].amount-amountToESkillz);
        ISport(sport).transfer(feeReceiver, amountToESkillz);
        totalAmountsOfDay[(block.timestamp- startTimeStamp)/ 1 days] -=(gamebetting[GameID][0].amount);
        delete(gamebetting[GameID]);
    }

    function mintMoreSport(uint256 amount, uint256 pro) external {
        
        
        ISport(sport).mintMore(amount.mul(pro).div(100));
    ...
    function getAmountsSPGameToDistribute(uint256 game, uint256 result) private view returns (uint256, uint256) {
        uint256 amountToWinner = gamebetting[game][0].amount+gamebetting[game][0].amount.mul(result).div(100)-(gamebetting[game][0].amount).mul(eskillz_fee).div(feeDenominator);
        uint256 amountToESkillz = (gamebetting[game][0].amount).mul(eskillz_fee).div(feeDenominator);
    ...
    function getAmountsMPGameToDistribute(uint256 game) private view returns (uint256, uint256) {
        uint256 amountToWinner = (gamebetting[game][0].amount+gamebetting[game][1].amount).mul(feeDenominator-eskillz_fee.div(2)).div(feeDenominator);
        uint256 amountToESkillz = (gamebetting[game][0].amount+gamebetting[game][1].amount).mul(eskillz_fee.div(2)).div(feeDenominator);
        return(amountToWinner, amountToESkillz);
    }
    ...
    function setSportAddress(address _sportAddress) external onlyOwner {
        sport = _sportAddress;
    }
