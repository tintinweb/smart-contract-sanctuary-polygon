 ...
 *
 * @dev Implements Ownable, but only to set the organization's "collection wallet" as a nominal owner for
 * external metadata edits on collection sites like OpenSea.
 * @dev All functionality related to minting, token transfers, ownership transfer, and state changes must be sent
    ...
    Redeemable
{
    ...
    /// @dev the service account of the contract, set during initialization after cloning
    ...
    /// @dev the base URI for this contract's metadata
    ...
    /// @dev Event emitted after contractURI is set
    event SetContractURI(string uri);

    /// @dev Event emitted after service is set
    event SetService(address service);
    ...
    /// @dev Only used in unit tests. In deployment this contract is initialized not constructed.
    constructor(address _service, address _collectionWallet)
        ERC721("NOOP", "NOOP")
    {
        service = _service;
        _transferOwnership(_collectionWallet);
    }

    /// @dev checks message sender is the service account, which is allowed to mint
     ...
     * @param _service web3 service account to perform minting and other state modifications
     * @param _collectionWallet the organization's "public" account, used on OpenSea
    ...
    function initialize(
        ...
        // Set up base contract state
        service = _service;
        account = _account;
        nftName = _nftName;
        nftSymbol = _nftSymbol;

        // Org's collectionWallet should own the contract for metadata edits only
        _transferOwnership(_collectionWallet);
        require(owner() == _collectionWallet, "OWNER_NOT_COLLECTION");

        // Initialize token counter at 1
     ...
     * @param _ipfsMetadataUri the metadata uri on IPFS for the NFT being minting
    ...
    function safeMint(
        ...
        _setTokenURI(tokenId, _ipfsMetadataUri);
        emit TaggedTransfer(
            bytes(_orgId),
            bytes(_collectionId),
            _ipfsMetadataUri,
     ...
     * @param _orgId the account who owns this contract
     * @param _collectionId the asset collection associated with this clone contract
     * @param _ipfsMetadataUris IPFS metadata uris for the NFTs to mint
     * @param _to the address to mint the NFT to
     */
    function batchMint(
        string memory _orgId,
        string memory _collectionId,
        string[] calldata _ipfsMetadataUris,
        address _to
    ) public onlyService {
        for (uint32 i; i < _ipfsMetadataUris.length; i++) {
            safeMint(_orgId, _collectionId, _ipfsMetadataUris[i], _to);
        }
    }
    ...
    /**
     * @notice set the new service accout of the contract
     * @notice emits SetService
     * @param _service wallet address of the service account for the contract
     */
    function setService(address _service) external onlyService {
        service = _service;
        emit SetService(service);
    }
     ...
     */
    function setMintActive(bool _mintActive) external onlyService {
     ...
     */
    function setBaseURI(string calldata _URI) external onlyService {
        baseURI = _URI;
     ...
     */
    function setMaxRedemptions(uint32 _maxRedemptions) external onlyService {
     ...
     */
    function redeem(uint256 _tokenId, uint32 _count) external onlyService {
        _redeem(_tokenId, _count);
    }

    /// @notice allow owner to change the name
    function setName(string calldata _name) external onlyService {
        nftName = _name;
    }

    /// @notice allow owner to change the symbol
    function setSymbol(string calldata _symbol) external onlyService {
        nftSymbol = _symbol;
    }

    /// @notice allow owner to change the contract's metadata URI
    function setContractURI(string calldata _uri) external onlyService {
        contractMetadataURI = _uri;
        emit SetContractURI(_uri);
    }

    /**
     * @notice returns this contract's metadata URI, per OpenSea standards.
     * See [OpenSea](https://docs.opensea.io/docs/contract-level-metadata) for details.
     */
    ...
    function pause() public onlyService {
        _pause();
    }

    /// @notice unpause trading for the NFT collection
    function unpause() public onlyService {
        _unpause();
    }
    ...
    /**
     * @notice Overrides with `onlyService`, as the owner on this contract is
     * only nominal and should not have access to state changing functionality.
     * @param newOwner the address to transfer ownership to
     */
    function transferOwnership(address newOwner) public override onlyService {
     ...
     * only nominal and should not have access to state changing functionality.
     */
    function renounceOwnership() public override onlyService {
        _transferOwnership(address(0));
    }
...
/**
 * @notice This is a thin wrapper around ERC1967Proxy, used only by `cloneNFTContractERC1967`.
 */
contract NFTAssetProxy is ERC1967Proxy {
...
import "../Assets/NFT/NFTAsset.sol";
import "../Assets/NFT/NFTAssetProxy.sol";
import "hardhat/console.sol";

/**
 * @title AssetFactory
 * @notice AssetFactory is used to create cloned digital asset contracts such as ERC-20, ERC-721, and ERC-1155.
...
contract AssetFactory {
     ...
     * @param collectionWallet the address of the org's collection (metadata editor) wallet
     * @param instance the address of the created NFT contract
     */
    event CloneNFT(
        bytes indexed orgId,
        bytes indexed collectionId,
        address indexed collectionWallet,
        address instance
    );
     ...
     * @param _collectionWallet the organization's collection wallet which will "own" the
     * cloned contract for metadata editing only
     */
...
/**
 * @dev Contract module which allows children to implement a redeem
 * capability that lets children mark a token (i.e. ERC721) as redeemed.
 *
 * This module is used through inheritance.  Each token will have the
 * default number of redemptions and allow the child contract to increment
 * the tokens redeemed counter until all redemptions are used.
 */
abstract contract Redeemable {
    /// @dev max redemptions default to 1, defaults to 0 after cloning
    uint32 public maxRedemptions = 1;

    /// @dev the redemptions used per token
    mapping(uint256 => uint32) public tokenRedemptions;

    /**
     * @notice event emitted when creating new NFT contracts
     * @param tokenId the ID of the token being redeemed
     * @param count the current number of redemptions
     */
    event Redeem(uint256 indexed tokenId, uint32 count);

    /**
     * @dev set the maximum number of redemptions
     * @param _maxRedemptions the desired maximum number of redemptions
     */
    function _setMaxRedemptions(uint32 _maxRedemptions) internal {
        maxRedemptions = _maxRedemptions;
    }

    /**
     * @dev increment the number of redemptions used by the token
     * @param _tokenId the token to redeem
     * @param _count the number of redemptions to use
     */
    function _redeem(uint256 _tokenId, uint32 _count) internal {
        uint32 currentRedemptions = tokenRedemptions[_tokenId];
        require(
            currentRedemptions + _count <= maxRedemptions,
            "REDEMPTIONS_USED"
        );
        tokenRedemptions[_tokenId] += _count;
        emit Redeem(_tokenId, _count);
    }
}
