...
contract Marketplace is ERC721Holder {
    ...
    function buy(address nftContract, uint256 itemId) external payable {
        ...
        require(msg.value >= item.price + 1, "Insufficient funds");

        address seller = item.seller;
        uint256 price = item.price;
        uint256 commissionAmount = 1;
        ...
        (success, ) = payable(address(this)).call{value: commissionAmount}("");
        require(success, "Transfer of commission fee failed");

        // Refund excess payment to the buyer
        if (msg.value > price + 1) {
            (success, ) = payable(msg.sender).call{value: msg.value - price - 1}("");
