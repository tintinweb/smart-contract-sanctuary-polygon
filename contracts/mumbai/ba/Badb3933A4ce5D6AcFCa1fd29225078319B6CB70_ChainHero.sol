...
contract ChainHero is ERC721URIStorage, Ownable {
    ...
    function getTokenURI(uint256 tokenId) public view returns (string memory){
        bytes memory dataURI = abi.encodePacked(
            ...
            '"attributes": [',
            '{"trait_type": "Damage","value": ', _getDamage(tokenId), ', "max_value": 100},',
//            '{"trait_type": "Agility","value": ', _getAgility(tokenId), ', "max_value": 100},',
            '{"trait_type": "Level", "value": ', _getLevel(tokenId), ', "display_type": "number"}',
            '],',
            '}'
        );
        return string(
    ...
    function _getWeaponImage(uint tokenId) private view returns (string memory){
        ...
        if (item.itemAddress == address(0)) {
            ...
            string memory imageUrl = IChainHeroItem(item.itemAddress).getImageUri(item.itemTokenId);
            return string.concat('<image href="', imageUrl, '"/>');
        }
    }
...
interface IChainHeroItem {
    ...
    function getImageUri(uint tokenId) external view returns(string memory);
