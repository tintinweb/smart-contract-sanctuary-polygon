...
contract LongTermStaking is Ownable {
    ...
    struct PoolInfo {
        ...
        uint128 maxSlot; // max slot can stake
        uint128 currentSlot; // current slot of pool
    ...
    event WithdrawAndClaim(uint256 _pid, uint256 amount);
    ...
    event AddPool(uint256 _rewardPool, uint256 _minDeposit, uint256 _maxDeposit, uint128 _maxSlot, uint64 _APR, uint128 _period);       
    event SetPool(uint256 _pid, uint256 _minDeposit, uint256 _maxDeposit, uint128 _maxSlot);
    ...
    function addPool(uint256 _rewardPool, uint256 _minDeposit, uint256 _maxDeposit, uint128 _maxSlot, uint64 _APR, uint128 _period) external onlySystemAdmin {        
        ...
        emit AddPool(_rewardPool, _minDeposit, _maxDeposit, _maxSlot, _APR, _period);
    }

    function setPool(uint256 _pid, uint256 _minDeposit, uint256 _maxDeposit, uint128 _maxSlot) external onlySystemAdmin {
        ...
        emit SetPool(_pid, _minDeposit, _maxDeposit, _maxSlot);
    }
    ...
    function APR(uint256 _pid) external view returns (uint256) {
        return poolInfo[_pid].APR;
    }

    function poolCurrentSlot(uint256 _pid) external view returns (uint256) {
        return poolInfo[_pid].currentSlot;
    }

    function poolMaxSlot(uint256 _pid) external view returns (uint256) {
        return poolInfo[_pid].maxSlot;
    }

    function poolMinDeposit(uint256 _pid) external view returns (uint256) {
        return poolInfo[_pid].minDeposit;
    }

    function poolMaxDeposit(uint256 _pid) external view returns (uint256) {
        return poolInfo[_pid].maxDeposit;
    }

    function stakeAmout(uint256 _pid) external view returns(uint256) {
    ...
    function deposit(uint256 _pid, uint256 _amount) external {
        ...
        require(pool.currentSlot <= pool.maxSlot, "Full Slot");
    ...
    function withdrawAndClaim(uint256 _pid) external {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        if (pool.status == true) {
            if (userTimeStake >= pool.period) {
                ...
                plastik.safeTransferFrom(rewardDistributionAddress, msg.sender, pending);
            }
        } 
        else {
            pending = earned(_pid, msg.sender);
        ...
        plastik.safeTransfer(msg.sender, user.amount);

        _totalSupply = _totalSupply.sub(user.amount);
        user.amount = user.amount.sub(user.amount);
        pool.currentSlot -= 1;

        emit WithdrawAndClaim(_pid, user.amount);
    }

    function withdrawFunds(uint256 _amount) external onlySystemAdmin {
        plastik.safeTransfer(msg.sender, _amount);
