...
contract Treasury is EIP712, Pausable, Roles {
    using SafeERC20 for IERC20;

    // keccak256("Claim(address from,address token,uint amount,uint nonce)").
    bytes32 public constant CLAIM_TYPEHASH = 0x2d68a28dbe8246775237073b2b4bdcff37b030f33b9e36f9121fdfd9b8c39782;
    ...
    event Claimed(address indexed user, address token, uint amount);
    ...
    function recoverFunds(
        ...
        uint fundBalance = IERC20(_token).balanceOf(address(this));
        require(fundBalance > 0, "Noting to recover!");
        IERC20(_token).safeTransfer(msg.sender, fundBalance);
    }
    ...
    function claim(
        ...
        require(
            _nonce == _nonces[_from],
            "Invalid nonce!"
        );
        require(
            _verify(
                _hash(_from, _token, _amount, _nonce),
                _signature
            ),
            "Not approve by our system"
        );
        _nonces[_from] = _nonce + 1;

        uint toClaim = _amount - claimed[_from];

        require(toClaim > 0, "Nothing to claim!");
        require(IERC20(_token).balanceOf(address(this)) >= toClaim, "Exceeds current fund!");

        IERC20(_token).safeTransfer(_from, toClaim);
        claimed[_from] += toClaim;
        totalClaimed += toClaim;

        emit Claimed(_from, _token, toClaim);
    }

    function doOverride(address[] memory _users, uint[] memory _values) external onlyRole(BIG_GUARDIAN_ROLE) {
    ...
    function togglePauseState() public onlyRole(GUARDIAN_ROLE) {
        paused() ? _unpause() : _pause();
    }

    function newSigner(address signer_) public onlyRole(BIG_GUARDIAN_ROLE) {
        require(_signer != signer_, "Identical signer!");
        _signer = signer_;
    }
