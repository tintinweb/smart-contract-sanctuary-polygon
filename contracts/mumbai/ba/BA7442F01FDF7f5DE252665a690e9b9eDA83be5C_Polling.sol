...
interface PollingInterface {
     ...
     * @param pollFeesForPoll The poll fees paid at the time of poll creation.
     */
    function createPoll(
        uint256 pollId,
     ...
     * @param startDate The start date of the poll.
     * @param endDate The end date of the poll.
     * @param pollFeesForPoll The array of poll fees paid at the time of poll creation.
     */
    function createPolls(
        ...
        uint256[] calldata pollFeesForPoll
    ) external;

    /**
     * @dev Ends the specified polls by providing the necessary information.
     * @dev Only owner can call this function.
     * @param pollingIds The unique identifier of the poll to be ended.
     * @param winningMerkle The winning Merkle root for the poll.
     * @param results The result url for the poll.
     * @param winningChoice The winning choice for the poll.
     */
    function endPoll(
        uint256 pollingIds,
        bytes32 winningMerkle,
    ...
    ReentrancyGuardUpgradeable
{
    ...
    /**
     * @dev Minimum poll fees for the contract
     */
    uint256 internal minPollFees;

    /**
     * @dev Burn percentage of the token
     */
    uint32 internal burnPercent;
     ...
     * @param minFees - Minimum fees for the contract.
    ...
    function initialize(
        ...
        uint32 minFees,
        uint32 percentForBurn
    ) public initializer {
        require(
            minFees > 0,
            "Polling: Minimum Fees should be greater than zero"
        );
        require(
            minFees <= feesForPoll,
            "Polling: Fees should be greater than minimum fees"
        );
        require(
     ...
     * @param startDate The start date of the poll.
     * @param endDate The end date of the poll.
     * @param feesForPoll The poll fees paid at the time of poll creation.
     */
    function createPoll(
        uint256 pollId,
        string memory pollTitle,
     ...
     * @param startDate The start date of the poll.
     * @param endDate The end date of the poll.
     * @param feesForPoll The array of poll fees paid at the time of poll creation.
     */
    function createPolls(
        uint256[] memory pollingIds,
        string[] memory pollTitle,
        string[][] memory pollChoices,
        address[] memory pollCreator,
        uint256[] memory startDate,
        uint256[] memory endDate,
        uint256[] memory feesForPoll
    ) external override onlyOwner whenNotPaused nonReentrant {
        require(
    ...
    function endPoll(
        ...
        require(
            pollingDetails[pollingIds].startDate != 0,
            "Polling: Poll id not found"
        );
        require(
            winningMerkle != ZERO_BYTES_32,
            "Polling: Merkle root can't be empty"
        );
        require(bytes(results).length != 0, "Polling: Results can't be empty");
        require(
            bytes(winningChoice).length > 0,
            "Polling: Winning choice can't be empty"
        );
        require(
            block.timestamp >= pollingDetails[pollingIds].endDate,
            "Polling: End date of poll is later"
        );

        PollingDetail storage updatePoll = pollingDetails[pollingIds];
        updatePoll.result = results;
        updatePoll.winnerMerkle = winningMerkle;
        updatePoll.winningChoice = winningChoice;
        updatePoll.isPollEnded = true;

        toknContract.burn((updatePoll.pollFees * burnPercent) / MAX_PERCENTAGE);
    }
    ...
    function endPolls(
        ...
        for (uint8 index = 0; index < pollingIds.length; index++) {
            if (
                ...
                toknContract.burn(
                    (updatePoll.pollFees * burnPercent) / MAX_PERCENTAGE
                );
