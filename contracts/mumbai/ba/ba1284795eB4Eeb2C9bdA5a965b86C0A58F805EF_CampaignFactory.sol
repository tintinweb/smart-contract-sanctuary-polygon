...
contract Campaign {
    ...
    function contribute(uint256 _amount) external {
        ...
        for (uint i = 0; i < rewards.length; i++) {
            if (
                _amount == rewards[i].contribution &&
                rewards[i].claimed < rewards[i].maxNumber
            ) {
                campaignNFT.setURI(rewards[i].ImageLink);
                campaignNFT.mint(msg.sender, approversCount, 1);
                rewards[i].claimed++;
            }
        }
    }
    ...
    function createReward(
        ...
        _createReward(_contribution, _maxNumber, _imageLink, _claimed);
    }
    ...
    function finalizeRequest(uint256 index) public restricted {
        ...
        require(request.approvalCount > (approversCount / 4));
    ...
    function getRewardsCount() public view returns (uint256) {
