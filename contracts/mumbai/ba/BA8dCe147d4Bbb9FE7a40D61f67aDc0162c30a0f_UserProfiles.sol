...
contract UserProfiles {
    ...
    */
    function updateProfile(string calldata handle, string calldata location, string calldata avatar, string calldata _uri, string calldata _bio, bool isGroup) public {
        ...
        if (isGroup){
            ...
            require(msg.sender == Groups.getOwnerOfGroupByID(Groups.getGroupIDFromAddress(profileAddress)), "You are not the owner of this group");
    ...
    */
    function getUserDetails(address usrAddress) public view returns(string[] memory){
        ...
        string[] memory userDetails = new string[](17);
        ...
        userDetails[8] = KUtils.toString(usrProfileMap[usrAddress].userAvatar.tokenID);
        userDetails[9] = usrProfileMap[usrAddress].uri;
        userDetails[10] = usrProfileMap[usrAddress].bio;
        userDetails[11] = usrProfileMap[usrAddress].location;
        userDetails[12] = KUtils.toString(usrProfileMap[usrAddress].joinBlock);
        userDetails[13] = KUtils.toString(usrProfileMap[usrAddress].joinTime);
        userDetails[14] = KUtils.toString(usrProfileMap[usrAddress].followLimit);
        userDetails[15] = KUtils.toString(usrProfileMap[usrAddress].userStats.tipsReceived);
        userDetails[16] = KUtils.toString(usrProfileMap[usrAddress].userStats.tipsSent);
