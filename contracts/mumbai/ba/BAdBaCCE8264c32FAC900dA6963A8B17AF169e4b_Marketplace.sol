...
error OnlyCollectionWithModels();
error OnlySubmarket();
error NotOwner();
error UnknownCallError();

contract Marketplace is OwnableUpgradeable, MulticallUpgradeable {
    ...
    address public swapper;                                                                             // address of the swapper

    mapping(address => bool) public collectionsWithModels;                                              // collections with models id to allow offer for one model
    mapping(address => bytes4) public collectionsWithModelsSelector;                                    // selectors returned by the collections to get the model id

    address[] private subMarkets;                                                                       // submarkets addresses

    modifier onlySubMarketplace(){
        bool found = false;
        uint256 subMarketsLen = subMarkets.length;
        for(uint256 i = 0; i < subMarketsLen;){

            if(subMarkets[i] == _msgSender()){
                found = true;
                break;
            }
    ...
    function initialize(address[] calldata _nftAddresses, address _swapperAddress, address _multisig) public initializer {
        ...
        uint256 addressesLen = _nftAddresses.length;
        for(uint256 i = 0; i < addressesLen;){
        ...
        subMarkets = new address[](0);

        transferOwnership(_multisig);
    }

    /**
     * @notice Check if the NFT is allowed
    ...
    /**
     * @notice Check if the owner of the NFT is the sender or the origin
     * @param _nftAddress Address of the collection
     * @param _tokenId Id of the NFT
     */
    function checkNFTOwner(address _nftAddress, uint256 _tokenId, address _owner) external view {
         if(IERC721(_nftAddress).ownerOf(_tokenId) != _owner && IERC721(_nftAddress).ownerOf(_tokenId) != tx.origin){
            revert NotOwner();
        }
    }

    /**
     * @notice Check if the price and the duration are correct
     * @param _price Price
    ...
    /**
     * @notice Check if the NFT is allowed to be traded on this marketplace
     * @param _nftAddress Address of the collection
     * @param _tokenId Token ID of the NFT
     */
    function checkApprovedMarketplace(address _nftAddress, uint256 _tokenId) external view {
        IERC721 nft = IERC721(_nftAddress);
        if (!nft.isApprovedForAll(_msgSender(), address(this)) && nft.getApproved(_tokenId) != address(this)) {
            revert NotApprovedForMarketplace();
        }
    }

    /**
     * @notice Check if the NFT collection supports models
     * @param _nftAddress Address of the collection
     */
    function checkCollectionWithModels(address _nftAddress) external view {
        if(!collectionsWithModels[_nftAddress]){
            revert OnlyCollectionWithModels();
        }
    }

    /**
     * @notice Perform a transfer of NFT for a submarketplace
     * @param _nftAddress Address of the NFT
     * @param _tokenId Token ID of NFT
     * @param _from holder of the NFT
     * @param _to recipient of the NFT
     */
    function doTransferOfNFT(address _nftAddress, uint256 _tokenId, address _from, address _to) external onlySubMarketplace {
        IERC721(_nftAddress).safeTransferFrom(_from, _to, _tokenId);
    }

    /**
     * @notice Get the model id of a given NFT
     * @param _nftAddress Address of the collection
     * @param _tokenId id of the NFT
     */
    function getModelIdForNFT(address _nftAddress, uint256 _tokenId) external view returns (uint256) {
        if(!collectionsWithModels[_nftAddress]){
         ...
         (bool success, bytes memory data) = _nftAddress.staticcall(
            abi.encodeWithSelector(collectionsWithModelsSelector[_nftAddress], _tokenId)
        );
    ...
    /**
     * @notice Check if the NFT is free on all submarkets
     * @param _nftAddress Address of the collection
     * @param _tokenId id of the NFT
     */
    function isNFTFree(address _nftAddress, uint256 _tokenId) external view returns (bool) {
        uint256 subMarketsLen = subMarkets.length;
        for(uint256 i = 0; i < subMarketsLen;){
            if(!ISubMarket(subMarkets[i]).isFree(_nftAddress, _tokenId)){
                return false;
            }
    ...
    /**
     * @notice Allows the owner to update submarkets
     * @param _submarkets Address of the submarket
     * @dev Can only be called by the owner
     */
    function setSubMarkets(address[] calldata _submarkets) external onlyOwner {
        subMarkets = _submarkets;
    }

    /**
     * @notice Allows the owner to update the parameters of the shop
     * @param _listingMaxDuration The new max duration
     * @param _listingMinPrice The new min price
     * @dev Can only be called by the owner
     */
    function updateParamListing(uint256 _listingMaxDuration, uint256 _listingMinPrice) external onlyOwner {
     ...
     * @param _selector selector of the function to call to get the model if of a given NFT
     * @dev Can only be called by the owner
     */
    function setCollectionWithModelsId(address _nftAddress, bool _allowed, bytes4 _selector) external onlyOwner {
        if(!allowedNFTs[_nftAddress])
            revert CollectionNotAllowed();

        collectionsWithModels[_nftAddress] = _allowed;
        collectionsWithModelsSelector[_nftAddress] = _selector;
    }
    ...
    receive() external payable {
        (bool ok, ) = swapper.call{value: msg.value}("");
        if(!ok){
            revert UnknownCallError();
        }
    }
