...
contract BaseContract is Ownable, TimeForDay, StructForDao, SortList {
    ...
    // DAO1的升级基数
    uint256 internal dao1Amount = 5000000 * 10 ** 18;
    // DAO2的升级基数
    uint256 internal dao2Amount = 1000000 * 10 ** 18;
    // DAO3的升级基数
    uint256 internal dao3Amount = 200000 * 10 ** 18;
    // DAO4的升级基数
    uint256 internal dao4Amount = 50000 * 10 ** 18;
    // 总补偿金额
    ...
    // 奖励比例列表
    uint256[20] internal rewardRateList = [250000000000, 100000000000, 50000000000, 25000000000, 12500000000, 6250000000, 3125000000, 1562500000, 781250000, 390625000, 195312500, 97656250];
    // 奖励比例列表
    ...
    event UpgradeDaoEvent(address _user,uint256 _time,uint256 _investIndex,string _type);
...
contract DaoContract is BaseContract {
    ...
    */
    ...
    function invest(uint256 _amount) external {
        ...
        _totalAD1A : totalShareAllocationList[investEndIndex]._totalASA,
        _totalAD2A : totalShareAllocationList[investEndIndex]._totalASA,
        _totalAD3A : totalShareAllocationList[investEndIndex]._totalASA,
        _totalAD4A : totalShareAllocationList[investEndIndex]._totalASA,
    ...
    function _distributeDayRewardForInviteRanking() private {
        ...
        emit DistributeEvent(_day, _timestamp, "_distributeDayRewardForInviteRanking", _amount, _totalAmount, _num, _gas0 - _gas1, firstAddress);
    }
    ...
    function _distributeDayRewardForInvestRanking() private {
        ...
        address firstAddress = _list[0];
        //将第一名加入到周排行榜地址
        weekTop1AddressList.push(firstAddress);
        uint256 _totalAmount;
        for (uint256 i = 0; i < _num; i++) {
            address _user = _list[i];
        ...
        uint256 _gas1 = gasleft();
        emit DistributeEvent(_day, _timestamp, "_distributeDayRewardForInvestRanking", _amount, _totalAmount, _num, _gas0 - _gas1, firstAddress);
    }
    ...
    function _distributeCompensation() private {
        ...
        uint256 _gas1 = gasleft();
        emit DistributeEvent(_day, _timestamp, "_distributeCompensation", _amount, _amount, _num, _gas0 - _gas1, firstAddress);
    }
    ...
    // 领取DAO奖励
    function _claimDaoReward(uint256 _type) private {
    ...
    function _distributeInvitationReward(uint256 _amount, address _user) private {
        ...
        for (uint256 i = 0; i < _allReferrersNum; i++) {
            ...
            //更新推荐人上级的最大网体
            _updateBigNetAmountList(_allReferrers[i]);
    ...
    //根据记录的最大网体直接判断是否可以升级
    function _canUpgrade2(address _user, uint256 _allNetAmount, uint256 _daoAmount) private view returns (bool canUpgrade_) {
    ...
    function _updateDao(address _user, uint256 _investStartIndex) private {
        ...
        if (_num < 3 || userDaoList[_user]._daoType == 1) {
            return;
        }
        ...
        if (_allNetAmount < dao4Amount * 2) {
            return;
        }
        // 根据用户当前的等级进行升级操作
        if (userDaoList[_user]._daoType == 0) {
            // 如果用户当前等级为0，则判断是否可以升级到等级4
            //            bool canUpgrade_ = _canUpgrade(_referrals, _num, _allNetAmount, dao4Amount);
            bool canUpgrade_ = _canUpgrade2(_user, _allNetAmount, dao4Amount);
            if (canUpgrade_) {
                Dao4Set.add(_user);
            ...
            bool canUpgrade_ = _canUpgrade2(_user, _allNetAmount, dao3Amount);
            if (canUpgrade_) {
                Dao4Set.add(_user);
                Dao3Set.add(_user);
            ...
            bool canUpgrade_ = _canUpgrade2(_user, _allNetAmount, dao2Amount);
            if (canUpgrade_) {
                Dao3Set.add(_user);
                Dao2Set.add(_user);
                _updateDaoRewad(_user, 3);
                userDaoList[_user] = UserDaoItem(_investStartIndex, 2);
                emit UpgradeDaoEvent(_user, block.timestamp, _investStartIndex, "Dao2");
            }
            ...
            bool canUpgrade_ = _canUpgrade2(_user, _allNetAmount, dao1Amount);
            if (canUpgrade_) {
                Dao2Set.add(_user);
                Dao1Set.add(_user);
