...
contract BurnTickets is ERC721Drop {
    ERC721LazyMint public immutable FablesContract;
    ...
    constructor(
        ...
        FablesContract = ERC721LazyMint(_fablesContract);
    }
    ...
    function burnToClaimTicket(uint256 _fablesTokenId) external {
        require(FablesContract.ownerOf(_fablesTokenId) == msg.sender, "BurnTickets: caller is not the owner of the ticket");
        FablesContract.burn(_fablesTokenId);
        _mint(msg.sender,1);
    }
    
    function burnTicketToClaimFables(uint256 _ticketTokenId, address _receiver) external {
        require(pause == false, "BurnTickets: contract is paused");
        require(ownerOf(_ticketTokenId) == msg.sender, "BurnTickets: caller is not the owner of the ticket");
        _burn(_ticketTokenId);
        FablesContract.claim(_receiver, _ticketTokenId);
    }
}