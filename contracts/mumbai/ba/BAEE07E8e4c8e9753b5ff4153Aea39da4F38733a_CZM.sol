...
contract CZM is ERC20, Ownable {
    ...
    struct Post {
        address author;
        string content;
        string mediaUrl; // Add the mediaUrl field
        uint256 timestamp;
        uint256 likes;
        bool isPrivate;
    }
...
function createProfile(string calldata email, string calldata name, string calldata imageUrl) external returns (uint256) {
    ...
    profile.email = email;
    profile.name = name;
    profile.birthdate = block.timestamp - decentralizedAge; // Assign the current decentralized age to the birthdate using block.timestamp
    ...
    if (isFirstYear) {
        // Check if the user is in their first decentralized year

        // Assign VIP status and additional privileges to the user
        profile.isVIP = true;
    ...
    function getReceivedMessages(address user) external view returns (uint256[] memory, string memory) {
        string memory username = addressToName[user];
        require(bytes(username).length > 0, "User profile does not exist");

        return (userReceivedMessages[user], username);
    }
...
function getUserPosts(address user) external view returns (string memory, uint256, Post[] memory) {
    ...
    // Count the number of public posts and calculate total likes
    ...
    // Create a new array for public posts
    Post[] memory publicPosts = new Post[](publicPostCount);
    uint256 currentIndex = 0;

    // Store public posts in the new array
    for (uint256 i = 0; i < posts.length; i++) {
        if (!posts[i].isPrivate) {
...
function likePost(uint256 postIndex) external {
    require(postIndex < userPosts[msg.sender].length, "Invalid post index");

    Post storage likedPost = userPosts[msg.sender][postIndex];
...
function getAge(address user) external view returns (uint256) {
    ...
    function aiFunction() public view returns (string memory) {
        if (aiEnabled) {
            ...
            if (decentralizedAge < D_WEEK_DURATION) {
                return "Welcome to the TimeVerse! You are in the first Year of CycleZero!! Congratulations :)";
            } else if (decentralizedAge < D_MONTH_DURATION) {
                uint256 currentDecentralizedWeek = getDecentralizedWeek();
                uint256 remainingWeeks = (D_MONTH_DURATION - decentralizedAge) / D_WEEK_DURATION;

                return string(abi.encodePacked(
                    "Congratulations! You have completed week ", uint256ToString(currentDecentralizedWeek),
                    " of CycleZero. ", uint256ToString(remainingWeeks), " week(s) remaining in this month."
                ));
            } else if (decentralizedAge < D_YEAR_DURATION) {
                uint256 currentDecentralizedMonth = getDecentralizedMonth();
                uint256 remainingMonths = (D_YEAR_DURATION - decentralizedAge) / D_MONTH_DURATION;

                return string(abi.encodePacked(
                    "Great progress! You have completed month ", uint256ToString(currentDecentralizedMonth),
                    " of CycleZero. ", uint256ToString(remainingMonths), " month(s) remaining in this year."
                ));
            } else {
                uint256 currentDecentralizedYear = getDecentralizedYear();
                uint256 remainingYears = (CYCLE_DURATION - decentralizedAge) / D_YEAR_DURATION;

                return string(abi.encodePacked(
                    "Keep up the good work! You have completed year ", uint256ToString(currentDecentralizedYear),
                    " of CycleZero. ", uint256ToString(remainingYears), " year(s) remaining in CycleZero."
                ));
            }
        } else {
    ...
    function userBirthdayFunction(address user) external view returns (string memory) {
        ...
        if (currentDecentralizedAge >= nextBirthday) {
            ...
            return string(abi.encodePacked("Remaining seconds until your next decentralized birthday: ", uint256ToString(remainingDays)));
        }
    }
    ...
    function isCycleHero() public view returns (bool, string memory) {
        uint256 currentTime = block.timestamp;
        bool result = currentTime >= CYCLE_HERO_START && currentTime < CYCLE_HERO_END;

        if (result) {
            return (result, "Congratulations! You have completed one cycle. You win!");
        } else {
            uint256 remainingSeconds = CYCLE_HERO_START - currentTime;
            string memory message = string(abi.encodePacked("CycleHero starts in ", uint256ToString(remainingSeconds), " seconds. Keep exploring and participating in the TimeVerse to unlock more achievements!"));
