...
contract Governance is TellorVars {
    ...
    constructor() {
        bytes4[12] memory _funcs = [
            ...
            0xa1bdb15e, // setEmissionRate(uint256)
            0xde52fc90, // setMinimumtip(uint256)
            0x6274885f // issueTreasury(uint256,uint256,uint256)//addtosetminimumtips
        ];
...
contract TellorVars is TellorVariables {
    ...
    address constant TELLOR_ADDRESS =
         0x6A555779845939ba42Fa4b3eFeBDA8A122177C20; // Address of main Tellor Contract 
        //todo change address
...
interface IOracle{
    ...
    function getValueByTimestamp(bytes32 _queryId, uint256 _timestamp) external view returns(uint256);
    ...
    function submitValue(bytes32 _queryId, uint256 _value, uint256 _nonce, bytes memory _queryData) external;
