...
contract NotAScam is Ownable, ERC2981, ERC721 {
  using Strings for uint256;
    ...
    function mint(uint256 numberOfTokens) public payable {
      if(!saleOpen) revert("Sale not open");
        require(totalSupply < MAX_SUPPLY, "Max not a scam supply reached");
        require(claimedAmount[msg.sender] + numberOfTokens < MAX_MINT, "Exceeds your not a scam quota");
        if(msg.value < numberOfTokens* 10**17 ) revert("insufficient payment");
        claimedAmount[msg.sender] += numberOfTokens;
        uint256 curr = totalSupply+1;
        totalSupply += numberOfTokens;

        for(uint256 i=0; i<numberOfTokens; i++)
        _mint(msg.sender, curr+i);
    }
  ...
  constructor() ERC721("NotAScam", "NOTSCAM"){
