...
contract SavageStreetTester is ERC721r, Ownable, Pausable, ReentrancyGuard {
  ...
  enum Stage { whitelistSale, publicSale }
  
  address buyer = msg.sender;
  bool WLstate = true;

  function setWhitelistSale(bool _newState) public view onlyOwner {
    _newState = WLstate;
  }
  function checkStage() public view returns(Stage stage) {
    if(WLstate == true) {
      stage = Stage.whitelistSale;
      return stage;
    }
    else if (WLstate == false) {
      stage = Stage.publicSale;
      return stage;
    }
  }
  modifier checkAndMint(uint _quantity, address _buyer) {
    if(checkStage() == Stage.whitelistSale) {
    ...
    //mint section
    function mint (uint quantity) external payable checkAndMint(quantity, buyer){  
      _mintRandom(buyer, quantity);
    }
