...
contract MaticProV2copy is Context, Ownable, IBEP20 {
    ...
    uint256 public constant plan1_minimum = 0.05 ether;
    uint256 public constant plan1_maximum = 500 ether;

    uint256 public constant plan2_minimum = 0.05 ether;
    uint256 public constant plan2_maximum = 500 ether;
    
    uint256 public constant plan3_minimum = 0.05 ether;
    ...
    uint256 sellRate = 500;
    uint256 constant RATE = 50;
    ...
    constructor() {
        ...
        _totalSupply =  1_000_000_000 * 10**uint(_decimals); // 1B
        
        //5% for the pool
        _balances[address(this)] = 200_000_000 * 10**uint(_decimals); 
		emit Transfer(address(0), address(this), _balances[address(this)]);    
		
        _balances[msg.sender] = 800_000_000 * 10**uint(_decimals); 
    ...
    function MakeDeposit(address referral, uint256 _tarif) public payable {
        ...
        if(_tarif == 50) {
            ...
            require(msg.value <= plan2_maximum, "Maximum Deposit for Plan 2 is 500 MATIC!");
        }else if(_tarif == 75) {
            require(msg.value >= plan1_minimum, "Minimum Deposit for Plan 1 is 5 MATIC!");
            require(msg.value <= plan1_maximum, "Maximum Deposit for Plan 1 is 100 MATIC!");
        }else{
