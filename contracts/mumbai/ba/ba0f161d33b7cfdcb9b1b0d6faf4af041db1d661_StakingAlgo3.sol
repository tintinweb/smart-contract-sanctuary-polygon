...
contract StakingAlgo3 is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    struct UserInfo {
        ...
        // total reward amount at last deposit time
        // ALA is the abbreviation for At the time of the Latest Action
        uint256 totalRewardALA;
        // only 1 refer address can be added per user.
        // if this value is true, you can't add refer anymore
        bool isAddRefer;
    }
    ...
    uint256 public rewardPerRefer;
    uint256 public rewardPer10Refer;
    uint256 public rewardPer100Refer;
    ...
    function initialize(
        ...
        vip1MinAmount = 10000000 * 10**18; // 10m HRT, decimals is 18
        vip1LockingPeriod = 60 * 60 * 24 * 365; // 1 year
        vip1RewardPercent = 85; // 85% of totalReward
        vip2MinAmount = 100000000 * 10**18; // 100m HRT, decimals is 18
        vip2LockingPeriod = 60 * 60 * 24 * 365 * 3; // 3 year
        vip2RewardPercent = 100; // 100% of totalReward
        rewardPercent = 70; // 70% of totalReward
        minDepositAmount = 10000 * 10**18; // 10000 HRT, decimals is 18
        // to refer address, give instance 100 HRT reward
        rewardPerRefer = 100;
        // when 10 refers, then give more 100*10 HRT rewards
        rewardPer10Refer = 1000;
        // when 100 refers, then give more 100*100 HRT rewards
        rewardPer100Refer = 10000;
        rewardCycle = 7; // 7 days
        depositFeePercent = 10; // 10%
        withdrawFeePercent = 10; // 10%
    ...
    function changeToken(address _hrt) external onlyOwner {
        require(_hrt != address(0), "can't be zero address");
        HRT = IERC20(_hrt);
    }
    ...
    function allTokenWithdraw() external onlyOwner {
        ...
        HRT.transfer(admin, HRT.balanceOf(address(this)));
    }

    function addRewardNotRefer(uint256 _amount) external onlyAdmin {
        HRT.transferFrom(msg.sender, address(this), _amount);

        uint256 _burnAmount = HRT.transferFeeAmount(msg.sender, _amount);
    ...
    function deposit(uint256 _amount, address _refer)
        external
        nonReentrant
        checkActive
    {
        require(
            _amount >= getDepositIn(msg.sender, minDepositAmount),
        ...
        uint256 _fee = depositHRT(_newAmount);
    ...
    function withdraw() external nonReentrant checkActive {
        ...
        if (_withdrawAmount < vip1MinAmount) {
            ...
            HRT.transfer(admin, _withdrawFee);
        }
        HRT.transfer(msg.sender, _withdrawAmount + _rewardAmount);
    ...
    function claim() external nonReentrant checkActive {
        ...
        HRT.transfer(msg.sender, _rewardAmount);
    ...
    function redeposit() external nonReentrant checkActive {
        ...
        require(
            _redepositAmount >= getDepositIn(msg.sender, minDepositAmount),
            "amount is less than min deposit amount"
        );
        uint256 _fee = depositHRT(_redepositAmount);
        ...
        returns (uint256)
    {
        ...
        if (_newReward > 0 && _depositedAmount > 0) {
            ...
            _rewardAmount +=
                (_newReward * _rewardPercent * _depositedAmount) /
                (totalDeposit * 100);
        }
    ...
    /** private functions */
    function depositHRT(uint256 _amount) private returns (uint256) {
        // deposit fee
        uint256 fee = 0;
    ...
    function claimReward() private returns (uint256) {
        ...
        userInfo[msg.sender].refersPayouts = userInfo[msg.sender].totalRefersCount;
    ...
    function getMinDepositAmount(address _caller) external view returns (uint256) {
        return getDepositIn(_caller, minDepositAmount);
    }

    function getVip1MinAmount(address _caller) external view returns (uint256) {
        return getDepositIn(_caller, vip1MinAmount);
    }

    function getVip2MinAmount(address _caller) external view returns (uint256) {
        return getDepositIn(_caller, vip2MinAmount);
    }

    function getDepositIn(address _caller, uint256 _amount) private view returns (uint256) {
        uint256 _fee = HRT.transferFeeAmount(_caller, _amount);
        uint256 FEEDENOMINATOR = 10000;
        uint256 _feePc = _fee * FEEDENOMINATOR / _amount;
        return _amount * FEEDENOMINATOR / (FEEDENOMINATOR - _feePc);
    }
}
