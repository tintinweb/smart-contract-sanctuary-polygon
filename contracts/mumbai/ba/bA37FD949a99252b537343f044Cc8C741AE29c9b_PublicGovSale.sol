...
contract PublicGovSale is Initializable, UUPSUpgradeable, PausableUpgradeable, OwnableUpgradeable, AdminProxyManager {
    ...
    function buyTokenInner(
        uint128 _amountIn,
        address _buyer
    ) internal whenNotPaused returns(uint128 amountDebt) {
    ...
    function buyToken(uint128 _amountIn) external whenNotPaused {
        uint128 amountDebt = buyTokenInner(_amountIn, _msgSender());
    ...
    function _buyToken(
        ...
        require(_msgSender() == factory.crossChainManager(), "who");
        buyTokenInner(_amountIn, _buyer);
    }
