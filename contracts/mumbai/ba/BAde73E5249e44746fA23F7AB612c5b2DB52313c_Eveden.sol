...
import "./MetaTransactionExecutor.sol";

contract Eveden is Ownable, ERC1155, ERC2981, MetaTransactionExecutor {
  constructor(string memory uri_) ERC1155(uri_) EIP712("Eveden", "1") {
  }
  ...
  function mintBatch(address to, uint[] memory ids, uint[] memory amounts) external onlyOwner {
  ...
  function _msgSender() internal view override (Context, MetaTransactionExecutor) returns (address) {
    return MetaTransactionExecutor._msgSender();
  }

  function _msgData() internal view override (Context, MetaTransactionExecutor) returns (bytes calldata) {
    return MetaTransactionExecutor._msgData();
  }
