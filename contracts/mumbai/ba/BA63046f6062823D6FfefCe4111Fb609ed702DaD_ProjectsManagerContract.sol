...
contract ProjectsManagerContract is Initializable, Common {
    ...
    function initialize(IERC20Upgradeable _wozxToken) public initializer {
        // 0x2aac70d576489F519880Ab5314687C79e3755632 fake wozx token
    ...
    function updateProject(
        ...
        p.nftValue = _nftValue;
        p.totalNFT = _totalNFT;
    }
    ...
    modifier canMintNft(uint256 _id, uint256 _totalNftToMint) {
        require(
            projects[_id].totalMintedNFT + _totalNftToMint >=
                projects[_id].totalNFT,
    ...
    function contribute(
        ...
        console.log( _amount >= (projects[_id].nftValue * _totalNftToMint));
         console.log( _amount, (projects[_id].nftValue * _totalNftToMint));
        require(
            _amount >= (projects[_id].nftValue * _totalNftToMint),
            "Not enough woxz"
        );
    ...
    function statsPerUser() external view returns (uint256 sum, uint256 withdrawable) {
        for (uint256 i = 0; i < projects.length; i++) {
            Project project = Project(projects[i].addr);
            sum += project.getLockedAmounts(msg.sender);
        }
...
contract Project is Initializable, Common {
    ...
    function contribute(
        ...
        if (contributions[_address].amount > 0) {
            contributions[_address].nbOfNftToMint += _totalNftToMint;
