...
contract BasicERC721Extendable is ERC721, ERC721Royalty, AccessContrownable, ReentrancyGuard, IBaseERC721Extendable {
    ...
    error NewMaxSupplyMustBeSmallerThanCurrentMaxSupply(uint256 newMaxSupply, uint256 currentMaxSupply);
    error CurrentSupplyMustBeEqualOrLowerThanNewMaxSupply(uint256 newMaxSupply, uint256 currentSupply);
    error TokenWasNotCreatedByThisExtension(address callingExtension, address expectedExtension, uint256 tokenId);
    ...
    modifier extensionRequired() {
        if (extensions.contains(msg.sender) == false) {
            revert MustBeARegisteredExtension();
        }
        _;
    }

    constructor(address _admin, address _minter, string memory _name, string memory _symbol, string memory _baseURI, uint256 _maxSupply, address _royalReceiver, uint96 _royalFeeNumerator) AccessContrownable(_admin) ERC721(_name, _symbol) payable {
        baseURI = _baseURI;
    ...
    returns (bool)
    {
        return interfaceId == type(IBaseERC721Extendable).interfaceId || super.supportsInterface(interfaceId);
    }

    /**
    * @dev See {IBaseERC721Extendable-getExtensions}.
     */
    function getExtensions() external view override returns (address[] memory _extensions) {
     ...
     */
    function registerExtension(address _extension, string calldata _baseURI) external override onlyRole(DEFAULT_ADMIN_ROLE) {
     ...
     */
    function unregisterExtension(address _extension) external override onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function tokenURI(uint256 _tokenId) public view override(ERC721) returns (string memory) {
        ...
        if (extension != address(0)) {
            ...
            if (bytes(tokenURIMap[_tokenId]).length != 0) {
                ...
                return string(abi.encodePacked(_extensionBaseURI, tokenURIMap[_tokenId]));
            }
            ...
            if (bytes(_extensionBaseURI).length != 0) {
                ...
                return string(abi.encodePacked(_extensionBaseURI, uint2str(_tokenId)));
            }
    ...
    function reduceMaxSupply(uint256 _newReducedMaxSupply) public onlyRole(DEFAULT_ADMIN_ROLE) {
        if (_newReducedMaxSupply >= maxSupply) {
            revert NewMaxSupplyMustBeSmallerThanCurrentMaxSupply(_newReducedMaxSupply, maxSupply);
        }
        if (currentSupply > _newReducedMaxSupply) {
            revert CurrentSupplyMustBeEqualOrLowerThanNewMaxSupply(_newReducedMaxSupply, currentSupply);
        }
    ...
    function updateBaseURI(string memory _baseURI, bool _emitEvent) public onlyRole(MINTER_ROLE) metadataNotLocked {
        baseURI = _baseURI;
        if (_emitEvent) {
            _emitBatchMetadataUpdatedEvent(1, type(uint256).max);
        }
        emit BaseURISet(_baseURI);
    }

    function updateBaseURIExtension(string memory _baseURI, bool _emitEvent) public virtual override nonReentrant metadataNotLocked extensionRequired {
    ...
    function batchMintExtension(address[] calldata _tos, uint256[] calldata _tokenIds, string[] calldata _tokenUris) public virtual override nonReentrant extensionRequired {
        ...
        for (uint i = 0; i < len; i++) {
            tokenExtension[_tokenIds[i]] = address(msg.sender);
        }
        _batchMint(_tos, _tokenIds, _tokenUris);
    }

    function mintExtension(address _to, uint256 _tokenId, string calldata _tokenUri) public virtual override nonReentrant extensionRequired {
        tokenExtension[_tokenId] = address(msg.sender);
        _mint(_to, _tokenId, _tokenUri);
    }

    function mint(address _to, uint256 _tokenId, string calldata _tokenUri) public onlyRole(MINTER_ROLE) {
        _mint(_to, _tokenId, _tokenUri);

    }

    function batchUpdateTokenUriExtension(uint256[] calldata _tokenIds, string[] calldata _tokenUris, bool _emitEvent) public virtual nonReentrant override extensionRequired {
        uint256 len = _tokenIds.length;
        for (uint i = 0; i < len; i++) {
            if (tokenExtension[_tokenIds[i]] != address(msg.sender)) {
                revert TokenWasNotCreatedByThisExtension(msg.sender, tokenExtension[_tokenIds[i]], _tokenIds[i]);
            }
        }
        _batchUpdateTokenUri(_tokenIds, _tokenUris, _emitEvent);
    }

    function batchUpdateTokenUri(uint256[] calldata _tokenIds, string[] calldata _tokenUris, bool _emitEvent) public onlyRole(MINTER_ROLE) {
        _batchUpdateTokenUri(_tokenIds, _tokenUris, _emitEvent);
    }
    ...
    function lockMetadata(uint256[] calldata _tokenIdsForMetadataLockedEvent) public onlyRole(DEFAULT_ADMIN_ROLE) metadataNotLocked {
        metadataLocked = true;
        emit MetadataLocked();
    ...
    function _mint(address _to, uint256 _tokenId, string calldata _tokenUri) internal virtual {
    ...
    function _batchMint(address[] calldata _tos, uint256[] calldata _tokenIds, string[] calldata _tokenUris) internal virtual {
        ...
        if (len != _tokenIds.length || (_tokenUris.length > 0 && len != _tokenUris.length)) {
    ...
    function _batchUpdateTokenUri(uint256[] calldata _tokenIds, string[] calldata _tokenUris, bool _emitEvent) internal metadataNotLocked {
        uint256 len = _tokenIds.length;
        ...
        if (_tokenUris.length == 0) {
            for (uint256 i; i < len; ++i) {
                ...
                tokenURIMap[tokenId] = '';
                if (_emitEvent) {
                    _emitMetadataUpdatedEvent(tokenId);
                }
            }
        } else {
            for (uint256 i; i < len; ++i) {
    ...
    function emitMetadataUpdatedEvent(uint256 _tokenId) public onlyRole(MINTER_ROLE) {
        _emitMetadataUpdatedEvent(_tokenId);
    }

    function emitBatchMetadataUpdatedEvent(uint256 _fromTokenId, uint256 _toTokenId) public onlyRole(MINTER_ROLE) {
        _emitBatchMetadataUpdatedEvent(_fromTokenId, _toTokenId);
    }

    function _emitMetadataUpdatedEvent(uint256 _tokenId) internal {
        emit MetadataUpdate(_tokenId);
    }

    function _emitBatchMetadataUpdatedEvent(uint256 _fromTokenId, uint256 _toTokenId) internal {
     ...
     */
    function _registerExtension(address _extension, string calldata _baseURI) internal {
...
/**
 * @title A contract that combines `Ownable` with `AccessControl`.
...
abstract contract AccessContrownable is Ownable, AccessControl {
    address private _defaultAdminAddr;
    ...
    constructor(address admin_) {
        ...
        _defaultAdminAddr = admin_;
    }

    function owner() public view override returns (address) {
        return _defaultAdminAddr;
    }
    ...
    function grantRole(
         ...
         * previous owner and set it to the new `_defaultAdminAddr`, which is used to
         * track the `owner`.
         */
        if (role == DEFAULT_ADMIN_ROLE) {
            _revokeRole(role, _defaultAdminAddr);
            _defaultAdminAddr = account;
    ...
    function revokeRole(
        ...
        /**
         * Revoking the `DEFAULT_ADMIN_ROLE` role is disabled.
         */
        if (role == DEFAULT_ADMIN_ROLE) {
    ...
    function renounceRole(bytes32 role, address account) public override {
        /**
         * Renouncing the `DEFAULT_ADMIN_ROLE` role is disabled.
         */
        if (role == DEFAULT_ADMIN_ROLE) {
            revert OperationNotAllowed();
        }
