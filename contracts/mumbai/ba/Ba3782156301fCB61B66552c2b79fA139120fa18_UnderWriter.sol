	...
	ReentrancyGuardUpgradeable
{
	...
	function resignation(address _user) public isSuperManager {
		...
		for (uint256 i = 0; i < _allUW.length; i++) {
			if (_user == _allUW[i]) {
				_allUW[i] = _allUW[_allUW.length - 1];
	...
	/**
	 * @notice Get isUnderWriter.
	 * @dev Get isUnderWriter that mean the user isUW and already stake of the minUserStake.
	 * @return The boolean value.
	 */
	function isUnderWriter(address _user) public view returns (bool) {
		IStaking.StakingData memory _StakingData = S.stakingData(
			_user,
			IStaking.StakingTypes.UW
		);
		IStaking.WorkType memory _dataType = S.workData(
			IStaking.StakingTypes.UW
		);
		return
			_isUnderWritier[_user] &&
			(_StakingData.balance >= _dataType.minUserStake);
	}

	/**
	 * @notice Get isUnderWriterNonCheckStake.
	 * @dev Get isUnderWriterNonCheckStake that mean the user isUW.
	 * @return The boolean value.
	 */
	 ...
	 *  - `_worker` Must worker of UnderWriter.
	 */
	function withdraw(
		address _worker,
		address _asset,
		...
		require(
			...
			"Policy: This worker is Blacklist."
		);
		require(
			PDS.isBlacklistAssetsUser(_worker, _asset) != true,
			"Policy: This Assets of worker is Blacklist."
		);
		require(
			isUnderWriter(_worker) == true,
			"This user Doesn't Underwriter."
		);
