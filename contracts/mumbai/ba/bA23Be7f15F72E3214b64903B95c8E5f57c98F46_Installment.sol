...
contract Installment is Ownable, AccessControl, ReentrancyGuard{
    ...
    */
     ...
     * Seller need to setApprovallForAll this contract in landNFT contract for begin createOrder.
    ...
    function payBill(address _buyerAccount, bytes32 _billId , uint256 _period) external  {
        ...
        uint totalPay = bill.payByperiod * _period;
        vaultBalance.decreaseBalance(msg.sender, totalPay);
        vaultBalance.increaseBalance(bill.seller, totalPay);
    ...
    function cancelOrder(bytes32 _orderId) external {
        ...
        landNFT.transferFrom(admin ,msg.sender, order.tokenId);
        delete orderByOrderId[order.seller][_orderId];
