...
contract Shopify is ERC1155, ERC2981, IShopify, DefaultOperatorFilterer, AccessControl  {
    ...
    IShopifyERC721 public shopifyNFT;
    ...
    // collectionId => Reward NFT
    mapping(uint256 => ShopifyNFT) private shopifies;
    ...
    function mint(
        ...
        payable 
    {
        ShopifyNFT memory item = shopifies[_itemId];
        ...
        onlyExistingItem(_itemId)
    {
        ...
        shopifies[_itemId].minted = item.minted + _quantity;
        ...
        if (collectionType == CollectionType.Limited) {
            shopifyNFT.mint(_to, _itemId, _quantity);
        } else {
    ...
    function getCollectionInfo(uint256 itemId) external view onlyExistingItem(itemId) returns (ShopifyNFT memory) 
    {
        return shopifies[itemId];
    }
    ...
    function enableCollection(uint256 _id) 
        ...
        onlyHasRole(ADMIN_ROLE) 
    {
        require(!shopifies[_id].enabled, "Collection already enabled");
        shopifies[_id].enabled = true;
    }
    ...
    function disableCollection(uint256 _id) 
        ...
        onlyHasRole(ADMIN_ROLE) 
    {
        require(shopifies[_id].enabled, "Collection already disabled");
        shopifies[_id].enabled = false;
    }

    function updateShopifyNFT(address _nft) external onlyHasRole(ADMIN_ROLE) {
        shopifyNFT = IShopifyERC721(_nft);
    }
    ...
    function updateShopifyData(
        ...
        onlyExistingItem(_id) 
    {
        if (shopifies[_id].minted >= _cap) {
            revert UpdateLessThanCap();
        }
        shopifies[_id].cap = _cap;
        shopifies[_id].maxMintPerAddress = _maxMintPerAddress;
        shopifies[_id].maxMintPerTx = _maxMintPerTx;
    }
        ...
        onlyExistingItem(_id) 
    {
        ...
        shopifies[_id].tokenGated = _token;
        shopifies[_id].tokenGatedId = _tokenGatedId;
        shopifies[_id].enabledTokenGated = _enabled;
    }

    /// @notice update price when needed
    function updateItemsMintPrice(
        ...
        onlyExistingItem(_id) 
    {
        shopifies[_id].mintPriceInWei = _mintPriceInWei;
    }
    ...
    function updateItemCap(
        ...
        onlyExistingItem(_id) 
    {
        if (shopifies[_id].minted >= cap) {
            revert UpdateLessThanCap();
        }

        shopifies[_id].cap = cap;
    }
