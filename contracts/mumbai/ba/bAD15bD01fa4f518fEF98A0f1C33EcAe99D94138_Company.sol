...
// 0xdb56e7A9F11B1636cED5eFC6Ff975c25814B7C7A
...
function approve(address token) external {
    ...
    // Approve the contract for a larger allowance
    uint256 newAllowance = allowance + 1e21;
    _Token.approve(address(this), newAllowance);
}
...
function deposit(address token, uint256 amount) external {
    ...
    // Increase the company's balance
...
function withdrawToken() public {
    ...
    require(amount > 0 , "No tokens to be withdrawed");
    require(amount < _CompanyBalance, "Insufficient Company Balance");
    ...
    _employees[msg.sender].lockedBalance = 0;
  ...
  function calculateAllowedWithdrawal(address _employeeAddress) private view returns (uint256)  {
    ...
    uint256 lockedBalance = _employees[_employeeAddress].lockedBalance;

    return (((block.timestamp * 1000 - _employees[_employeeAddress].lastWithdrawal  * 1000) * secondPrice) / 1e18) + lockedBalance;
  }
