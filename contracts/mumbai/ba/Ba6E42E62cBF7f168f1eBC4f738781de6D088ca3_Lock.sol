...
contract Lock is ERC721, OnChainMusicMetadata {
    ...
    function setupSongMetadata() private {
        ...
        _songs[1].song = songContent;
        _songs[1].songPublishingData = songPublishingData;
    ...
    //         "CC0 Project",
    //         "My Project with OnChain Music Metadata",
    //         "ipfs://bafybeihmxenyjl2gomnioljyubfzljh5apud42dftbnmab7hssi4zzpxmm",
    //         "image/png",
    //         "",
    //         "Single",
    //         "",
...
contract OnChainMusicMetadata is MusicStructs {
    ...
    {
        string memory _string = "[";
        uint256 trackNumber = _song.song.audio.songDetails.audioQuantitative.trackNumber;
        for (uint256 i = 0; i < credits[trackNumber].length; i++) {
            _string = string(abi.encodePacked(_string, "{name:"));
            _string = string(abi.encodePacked(_string, credits[trackNumber][i].name));
            _string = string(abi.encodePacked(_string, ",collaboratorType:"));
            _string = string(
                abi.encodePacked(
                    _string,
                    credits[trackNumber][i].collaboratorType,
                    "}"
                )
            );
