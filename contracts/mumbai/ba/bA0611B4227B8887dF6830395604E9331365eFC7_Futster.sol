...
contract Futster is ERC721, ERC721Enumerable, Pausable, ERC721Burnable, ERC721Royalty, AccessControlled {
    ...
    modifier onlyOwnerApproveControl() {
        require(onlyOwnerApprove == false, "Only owner can receive approval");
        _;
    }


    function setOnlyOwnerApprove(bool _approval) public onlyGovernor {
    ...
    function setApproveToContractOwner(uint256 _tokenId, bool _status) public {
        ...
        if (_status) {
            _approve(authority.governor(), _tokenId);
        } else {
...
contract Timelock is VerifySignature, AccessControlled {
    ...
    function timeLockGrantUpdate(uint blocknumber, uint nonce, bytes32 digest, bytes memory signature) external returns (bool){
        require(verify(blocknumber, authority.governor(), nonce, digest, signature, authority.guardian()), "Signed Message is not message sent");
