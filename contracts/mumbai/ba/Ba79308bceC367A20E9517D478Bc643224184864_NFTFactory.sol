...
interface INFTFactory {
    ...
    function isCollection(address _contract) external view returns (bool);

    function getCollectionId(address _contract) external view returns (uint256);
}
...
contract NFTCollection is ERC721Enumerable, IERC2981, AccessControl, INFTCollection {
    using Strings for uint256;
    ...
    constructor(
        ...
        _setupRole(ADMIN_ROLE, _mainAdmin);
        setCreator(_mainAdmin);
        setBaseURI(_initBaseURI);
        setNotRevealedURI(_initNotRevealedUri);
        revealed = _revealed;
        royaltyFee = _royaltyFee;
        beneficiary = _royaltyRecipient;
    ...
    {
        uint256 royaltyAmount = (_salePrice * royaltyFee) / 100;
    ...
    function mint(uint256 _mintAmount) public payable {
        ...
        require(hasRole(ADMIN_ROLE, msg.sender), "Only admin can use this function");
...
contract NFTFactory is AccessControl, INFTFactory {
    ...
    mapping(address => uint256) public reverseCollections;
    mapping(address => bool) private collectionExists;
    ...
    function createCollection(
        ...
        newCollection.baseURI = _initBaseURI;
        newCollection.notRevealedUri = _initNotRevealedUri;
        newCollection.isRevealed = _revealed;
        nftContract = new NFTCollection(
            _name,
            _symbol,
            _initBaseURI,
            _initNotRevealedUri,
        ...
        newCollection.NFTContract = address(nftContract);
        ownedCollections[msg.sender].push(address(nftContract));
        collectionExists[address(nftContract)] = true;
        reverseCollections[address(nftContract)]=collectionId;
        emit CollectionCreated(collectionId, _name, msg.sender);
    }

    function getOwnedCollections() external view returns (address[] memory) {
        return ownedCollections[msg.sender];
    }
    function getCollectionId(address _contract) external view returns (uint256) {
        return reverseCollections[_contract];
    }

    function isCollection(address _contract) external view returns (bool) {
        return collectionExists[_contract];
    }
