...
contract LandNFTPass is ERC721Enumerable, Ownable {
	...
	constructor(PassportMint _passport, IERC20 _matic) ERC721("Actocracy Land NFT Pass", "LandNFTPassMint") {
		pos = 0;
	...
	function mint(uint256 nftAmount) external payable {
        ...
        (, , string memory _referer, , , uint256 referralCount, uint256 level) = passport._totalInfo(passportID);
        if(level == 1) {
        ...
        if(keccak256(abi.encodePacked(_referer)) != keccak256(abi.encodePacked("no"))) {
            ...
            (address _fownerAddr, , string memory _freferer, , , ,) = passport._totalInfo(fpassportID);
            ...
            if(keccak256(abi.encodePacked(_freferer)) != keccak256(abi.encodePacked("no"))) {
                ...
                (address _sownerAddr,, string memory _sreferer, , , , ) = passport._totalInfo(spassportID);
        ...
        payable(address(this)).transfer(msg.value - feeAmount);
