  ...
  OwnableUpgradeable
{
  ...
  function fetchSingleProxyUserList(address _proxy)
    ...
    returns (usersRoleInfo[] memory)
  {
    uint256 totalUserRoles = 0;
    for (uint256 i = 0; i < proxyUsersRole.length; i++) {
      if (proxyUsersRole[i].contractAddr == _proxy) {
        totalUserRoles++;
      }
    }
    usersRoleInfo[] memory res = new usersRoleInfo[](totalUserRoles);
    uint256 j = 0;
    for (uint256 i = 0; i < proxyUsersRole.length; i++) {
      if (proxyUsersRole[i].contractAddr == _proxy) {
        res[j] = proxyUsersRole[i];
        j++;
      }
    }
    return res;
    ...
    for (uint256 i = 0; i < loopLimit; i++) {
      ...
      if (
        (_role == NFT(proxy).DEFAULT_ADMIN_ROLE() &&
          _account == address(this)) && (msg.sender != owner())
      ) {
        revert(
