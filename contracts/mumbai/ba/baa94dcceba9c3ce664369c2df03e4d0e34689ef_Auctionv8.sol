...
contract Auctionv8 is Initializable, OwnableUpgradeable {
    ...
    event ClaimableAuctionInfo(
        uint256 indexed claimableAuctionIndex,
        uint256 endTime,
        uint256 auctionId
    );
    ...
    */
    ...
    function claimNFT(uint256 claimableAuctionIndex, uint256 tokenId) public {
        ClaimableAuction storage _claimableAuction = claimableAuctions[
            claimableAuctionIndex
        ];
        require(
            _claimableAuction.endTime <= block.timestamp,
            "Can't claim now"
        );
        Auctions storage _auc = auctionsDetails[_claimableAuction.auctionId];
        Bid[] memory bids = _auc.tokenBid[tokenId];
        uint256 winnerIndex = bids.length - 1;
        int256 index = findUserBidIndex(bids, msg.sender);
        require(index != -1);
        uint256 userBidIndex = uint256(index);
        require(userBidIndex == winnerIndex, "You can not claim");
        require(!_claimableAuction.isClaimed[msg.sender], "already claimed");
        _claimableAuction.isClaimed[msg.sender] = true;
        _auc.paymentToken.safeTransfer(
            bids[userBidIndex].userAddress,
            bids[userBidIndex].amt
        );
        emit UserClaimed(msg.sender, claimableAuctionIndex, tokenId);
    }
    ...
    function endAuction(uint256 auctionId) public onlyOwner {
        ...
        if (_auc.auctionType == AuctionType.STAKE) {
            ClaimableAuction storage _claimbleAuction = claimableAuctions
                .push();
            _claimbleAuction.endTime = block.timestamp + stakeDuration;
            _claimbleAuction.auctionId = auctionId;
            emit ClaimableAuctionInfo(
                claimableAuctions.length - 1,
                _claimbleAuction.endTime,
                _claimbleAuction.auctionId
            );
        } else {
    ...
    struct ClaimableAuction {
        uint256 endTime;
        uint256 auctionId;
        mapping(address => bool) isClaimed;
    }
       //how much time NFT wilbe locked 6 month	
    uint256 public constant stakeDuration = (30 days) * 6;	
    ClaimableAuction[] public claimableAuctions;	
