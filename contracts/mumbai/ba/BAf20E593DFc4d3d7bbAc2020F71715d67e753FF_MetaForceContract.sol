...
contract MetaForceContract is Ownable, ReentrancyGuard, IMetaForceContract {
    ...
    function distibuteEmission() external override nonReentrant {
    ...
    function buyMFS(uint256 amountMFS) external override nonReentrant {
        ...
        for (uint256 i = lastIndexBuying[msg.sender]; i < datesForBuying[msg.sender].length; ) {
            if (datesForBuying[msg.sender][i].date < block.timestamp) {
                ...
                datesForBuying[msg.sender][i].amount = datesForBuying[msg.sender][i].amount - tempAmount;
                tempAmount = 0;
            }
            if (tempAmount == 0) {
        ...
        stableCoin.safeTransferFrom(msg.sender, registry.getMetaPool(), amountToPay);
        core.mintMFS(msg.sender, amountMFS);
    }
    ...
    function renewalPack(
        ...
        if (core.getWorkflowStage() == Workflow.SaleOpen) {
            if (typeCurrency == TypeRenewalCurrency.MFS) {
                ...
                core.transferMFSFrom(msg.sender, address(this), priceRenewalPackInMFS * amount);
                core.transferMFSFrom(msg.sender, registry.getMetaPool(), priceRenewalPackInMFS * amount);
    ...
    function activationPack(uint256 level) public override nonReentrant {
        ...
        if (core.getWorkflowStage() != Workflow.SaleOpen) {
            stableCoin.safeTransferFrom(msg.sender, registry.getMetaPool(), pricePackInUSD);
            unchecked {
                purchase.date = block.timestamp + PURCHASE_TIME_LIMIT_PERIOD;
            }
            purchase.amount = pricePackInMFS;
            datesForBuying[msg.sender].push(purchase);
            core.mintMFS(registry.getMetaPool(), pricePackInMFS);
        } else {
            mfs.safeTransferFrom(msg.sender, registry.getMetaPool(), pricePackInMFS);
        }
        IEnergy energy = IEnergy(registry.getEnergyCoin());
    ...
    function renewalPack(
        ...
        while (tempAmount != 0) {
            ...
            if (countRenewal[msg.sender][level] < HMFS_COUNT - 1) {
                for (uint256 i = countRenewal[msg.sender][level]; i < amountsCurrency.length; ) {
                ...
                renewalPack(level, 1);
                countRenewal[msg.sender][level] = countRenewal[msg.sender][level] + 1;
                tempAmount--;
            } else {
                if (amountsCurrency[HMFS_COUNT - 1] < tempPrice * tempAmount) {
                    revert MFCNotEnoughHMFSNeedLevel();
                }
                IHMFS hFMS = IHMFS(registry.getHMFS(HMFS_COUNT));
                hFMS.burn(msg.sender, amountsCurrency[HMFS_COUNT - 1]);
                renewalPack(level, tempAmount);
                tempAmount = 0;
            }
        }
        IEnergy energy = IEnergy(registry.getEnergyCoin());
    ...
    function cashingFrozenMFS() public nonReentrant {
        ICoreContract core = ICoreContract(registry.getCoreContract());
        if (core.getWorkflowStage() != Workflow.SaleOpen) {
        ...
        core.transferMFSFrom(registry.getMetaPool(), msg.sender, core.getAmountFrozenMFS(msg.sender));
        core.decreaseFrozenMFS(msg.sender, core.getAmountFrozenMFS(msg.sender));
    }

    function renewalPack(uint256 level, uint256 amount) internal nonReentrant {
        ICoreContract core = ICoreContract(registry.getCoreContract());
    ...
    function distributionRewardReferal(
...

        {
            ...
            for (uint256 i; i < referrers.length; ) {
                ...
                if (typeReward == TypeReward.ONLY_MFS || core.getWorkflowStage() == Workflow.SaleOpen) {
                    core.transferMFSFrom(metaPool, referrers[i], dRewardMFS);
                } else if (typeReward == TypeReward.ONLY_USD) {
                    stableCoin.safeTransferFrom(metaPool, referrers[i], dRewardUSD);
                    amountMFSForRealization += dRewardMFS;
                } else {
                    uint256 tempAmount = dRewardMFS / 2;
                    core.transferMFSFrom(metaPool, referrers[i], tempAmount);
                    amountMFSForRealization += tempAmount;
                    tempAmount = dRewardUSD / 2;
                    stableCoin.safeTransferFrom(metaPool, referrers[i], tempAmount);
                }
                unchecked {
                    ++i;
                }
        ...
        for (uint256 i; i < marketingReferrers.length; ) {
            ...
            if (typeReward == TypeReward.ONLY_MFS || core.getWorkflowStage() == Workflow.SaleOpen) {
                core.transferMFSFrom(metaPool, marketingReferrers[i], mRewardMFS);
            } else if (typeReward == TypeReward.ONLY_USD) {
                stableCoin.safeTransferFrom(metaPool, marketingReferrers[i], mRewardUSD);
                amountMFSForRealization += mRewardMFS;
            } else {
                uint256 tempAmount = mRewardMFS / 2;
                core.transferMFSFrom(metaPool, marketingReferrers[i], tempAmount);
                amountMFSForRealization += tempAmount;
                tempAmount = mRewardUSD / 2;
                stableCoin.safeTransferFrom(metaPool, marketingReferrers[i], tempAmount);
            }
            unchecked {
                ++i;
            }
    ...
    function calcMFSAmountForUSD(uint256 amountUSD) internal view returns (uint256 amount) {
        ICoreContract core = ICoreContract(registry.getCoreContract());
        uint256 priceMFSInUSD = core.priceMFSInUSD();
        uint256 endSmallBlock = core.endSmallBlock();
        uint256 totalEmissionMFS = core.totalEmissionMFS();
        ...
        if (totalEmissionMFS + amount > endSmallBlock) {
            ...
            uint256 amountInNewPrice = balance.div(core.calculateNextMFSPrice());
    ...
    function calcUSDAmountForMFS(uint256 amountMFS) internal view returns (uint256 amountUSD) {
        ...
        if (totalEmissionMFS + amountMFS > endSmallBlock) {
            ...
            uint256 amountInNewPrice = amountAfter.mul(core.calculateNextMFSPrice());
