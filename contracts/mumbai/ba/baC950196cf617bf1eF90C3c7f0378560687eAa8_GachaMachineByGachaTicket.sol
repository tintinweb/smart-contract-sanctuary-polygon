...
// File: contracts/LUXON/myPage/centralization/ERC721Centralization.sol
...
// File: contracts/LUXON/store/desperado/GachaMachineByGachaTicket.sol
...
contract GachaMachineByGachaTicket is ReentrancyGuard, LuxOnLive, ERC1155Holder, LuxOnData {
    event GachaByGachaTicket(address indexed owner, uint256 indexed gachaTicketTokenId, uint256 indexed tokenId);
    ...
    string public centralizationData = "ERC721Centralization";
    ...
    function name() public pure returns (string memory) {
        return "Gacha Machine By Gacha Ticket";
    }
    ...
    function gacha(GachaInfo memory _gachaInfo) external isLive {
        ...
        for (uint256 i = 0; i < _gachaInfo.amount; i++) {
            ...
            tokenIds[i] = tokenId;
            emit GachaByGachaTicket(msg.sender, _gachaInfo.tokenId, tokenId);
        }
        ERC721Centralization(getDataAddress(centralizationData)).setRealOwner(mintAddress, tokenIds, msg.sender);
    }
    ...
    function mintPay(GachaInfo memory _gachaInfo) private {
        DspGachaData.GachaInfo memory gachaInfo = DspGachaData(getDataAddress(gachaData)).getGachaInfo(_gachaInfo.tokenId);
