...
contract Tournament  is Ownable {
    ...
    mapping(address => Payment) private latestPayment;
    ...
    event TournamentCreated(uint256 tournamentId, uint256 startDate, uint256 endDate, uint256 fee, bool nftpass);
    event PayTournament (address sender , uint256 currentTime ,  uint256 fee);

    function isActive(uint256 tournamentId) private view returns (bool) 
    {
        // Get the tournament details for the given ID
        TournamentDetails storage tournament = tournaments[tournamentId];
    ...
    function deposit() public payable returns(bool) {
        ...
        {
            require(0<ERC1155(wolf2dContractAddress).balanceOf(msg.sender, tokenId));
        }
    ...
    {
        Payment memory lastPayment = latestPayment[_sender];
        return (lastPayment.currentTime);
    }

    
    function addScoreT(string memory _encryptedScore) public {
        require(isActive(activeTournament),"Out of Time");
    ...
    function createTournament(string memory _name,uint256 _startDate, uint256 _endDate, uint256 _fee, bool _nftpass) public onlyOwner
    {
        ...
        emit TournamentCreated(tournamentId, _startDate, _endDate, _fee,  _nftpass);
    ...
    function setnftContractAddress(address address2dWolf, uint256 _tokenId) public onlyOwner {
