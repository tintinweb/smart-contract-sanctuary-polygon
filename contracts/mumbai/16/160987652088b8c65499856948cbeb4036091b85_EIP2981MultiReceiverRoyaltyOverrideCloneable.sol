    ...
    OwnableUpgradeable
{
     ...
     */
    function setDefaultRoyalty(uint16 bps, Recipient[] calldata recipients)
        external
        override
        onlyOwner
    {
...
abstract contract EIP2981MultiReceiverRoyaltyMultiReceiverOverrideCore is IEIP2981, IEIP2981MultiReceiverRoyaltyOverride, ERC165 {
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC165, IERC165) returns (bool) {
        return interfaceId == type(IEIP2981).interfaceId || interfaceId == type(IEIP2981MultiReceiverRoyaltyOverride).interfaceId || super.supportsInterface(interfaceId);
    }
    ...
    function _setTokenRoyalties(TokenRoyaltyConfig[] memory royaltyConfigs) internal {
        for (uint i; i < royaltyConfigs.length; i++) {
     ...
     */
    function getTokenRoyalties() external override view returns(TokenRoyaltyConfig[] memory royaltyConfigs) {
        royaltyConfigs = new TokenRoyaltyConfig[](_tokensWithRoyalties.length);
        for (uint i; i < _tokensWithRoyalties.length; ++i) {
     ...
     */
    function getDefaultRoyalty() external override view returns(uint16 bps, Recipient[] memory recipients) {
        if (_defaultRoyaltySplitter != address(0)) {
     ...
     */
    function getAllSplits() external override view returns(address payable[] memory splits) {
        uint startingIndex;
        uint endingIndex = _tokensWithRoyalties.length;
        if (_defaultRoyaltySplitter != address(0)) {
