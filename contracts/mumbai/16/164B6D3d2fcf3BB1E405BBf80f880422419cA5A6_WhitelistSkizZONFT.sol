...
contract WhitelistSkizZONFT is IERC721, Ownable {
    ...
    event TixelNoLongerForDrop(uint indexed TixelIndex);

    /**
     * Event emitted when the public Drop begins.
     */
    event DropChange();
    ...
    string internal nftName = "WhitelistSkizZONFT";
    string internal nftSymbol = "WhtNft";
    ...
    uint public DropStartTime;
    uint public maxBatchMints = 250; // 2.5%
    uint public currentBatchMints = 0;
    ...
    function publicDropState(bool _isRunning) external onlyOwner {
        DropStartTime = block.timestamp;
        publicDrop = _isRunning;
        emit DropChange();
    }
    ...
    function mintsRemaining() external view returns (uint) {
        return TOKEN_LIMIT.sub(numTokens); //ski. aggiunto subBatchMints
    }
    ...
    function mint(uint256 numberOfNfts) external payable reentrancyGuard {
        ...
        require(publicDrop, "Drop not started.");
        require(numberOfNfts > 0, "numberOfNfts cannot be 0");
        require(numberOfNfts <= 50, "You can not buy more than 50 NFTs at once");
        require(totalSupply().add(numberOfNfts) <= TOKEN_LIMIT-maxBatchMints, "Exceeds TOKEN_LIMIT");
    ...
    function BatchMint(uint256 numberOfNfts, address to) external reentrancyGuard onlyOwner {
        require(currentBatchMints + numberOfNfts < maxBatchMints);
