...
contract YBCGovernorERC721Upgradeable is
    Initializable,
    GovernorUpgradeable,
    ...
    UUPSUpgradeable
{
    ...
    uint8 membershipBalance;

    modifier onlyMember() {
        require(_isMember(_msgSender()), "Low Balance");
        _;
    }
    /// @custom:oz-upgrades-unsafe-allow constructor
    constructor() {
    ...
    function _isMember(address _user) internal virtual returns (bool) {
        return IERC721Upgradeable(membershipToken).balanceOf(_user) > membershipBalance;
    }
