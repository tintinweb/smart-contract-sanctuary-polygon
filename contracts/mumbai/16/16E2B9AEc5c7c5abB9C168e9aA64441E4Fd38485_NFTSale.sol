...
import "./interface/IBigFootNFT.sol";
import "./util/WhitelistPriority.sol";

contract NFTSale is WhitelistPriority, ReentrancyGuard {
    ...
    mapping(uint256 => BigFootItem) private idToBigFootItem;
    ...
    constructor(
        ...
        _placeholderURIs[NFTType.AQUA] = placeholderURIs[1];
        _placeholderURIs[NFTType.FROST] = placeholderURIs[2];
        _placeholderURIs[NFTType.GROUND] = placeholderURIs[3];
        _placeholderURIs[NFTType.TOXIC] = placeholderURIs[4];
        _placeholderURIs[NFTType.GHOST] = placeholderURIs[5];
        _placeholderURIs[NFTType.FOREST] = placeholderURIs[6];
    ...
    modifier onlyItemOwner(uint256 nftId) {
        require(
            NFT.getOwnerOfToken(nftId) == _msgSender(),
            "Only product owner can do this operation"
        );
        _;
    }

    function createBigFootItem(
        NFTType nftType,
        uint256 tokenId,
        uint256 price
    ) private {
         uint256 nftPrice = prices[nftType];
        require(price > 0, "Price must be at least 1 wei");
        require(msg.value == nftPrice, "Price must be equal to nft price");

        idToBigFootItem[tokenId] = BigFootItem(
            tokenId,
            nftType,
            payable(_msgSender()),
        ...
        onlyWhitelisted
    {
        ...
        NFT.mint(_msgSender(), nftId, URI);

        typeCounter[nftType] = typeCounter[nftType].add(1);

        userPurchaseCounter[_msgSender()] = userPurchaseCounter[_msgSender()]
            .add(1);

        createBigFootItem(nftType, nftId, price);

        emit Sold(nftId, buyer, price);
    }

    function putItemToResell(uint256 nftId, uint256 price)
        public
        payable
        nonReentrant
        onlyItemOwner(nftId)
    {
        NFTType _nftType = idToBigFootItem[nftId].nftType;
        uint256 nftPrice = prices[_nftType];
        require(
            idToBigFootItem[nftId].owner == _msgSender(),
            "Only item owner can perform this operation"
        );
        require(msg.value == nftPrice, "Price must be equal to nft price");
        idToBigFootItem[nftId].sold = false;
        idToBigFootItem[nftId].price = price;
        idToBigFootItem[nftId].seller = payable(_msgSender());
        idToBigFootItem[nftId].owner = payable(address(this));
        _itemsSold.decrement();

        NFT.transferToken(_msgSender(), address(this), nftId);
    }

    /* Creates the sale of a marketplace item */
    /* Transfers ownership of the item, as well as funds between parties */

    function createMarketSale(uint256 nftId) public payable {
        uint256 price = idToBigFootItem[nftId].price;
        address seller = idToBigFootItem[nftId].seller;
        NFTType _nftType = idToBigFootItem[nftId].nftType;
        uint256 nftPrice = prices[_nftType];
        require(
            msg.value == price,
        ...
        idToBigFootItem[nftId].owner = payable(_msgSender());
        idToBigFootItem[nftId].sold = true;
        idToBigFootItem[nftId].seller = payable(address(0));
        _itemsSold.increment();
        NFT.transferToken(address(this), _msgSender(), nftId);
