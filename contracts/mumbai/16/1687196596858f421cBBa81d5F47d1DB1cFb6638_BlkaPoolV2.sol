...
contract BlkaPoolV2 is Ownable, Pausable {
    using SafeERC20 for IERC20;

    struct UserInfo {
        uint256 shares; // number of shares for a user.
        uint256 lastDepositedTime; // keep track of deposited time for potential penalty.
        uint256 blkaAtLastUserAction; // keep track of blka deposited at the last user action.
    ...
    IERC20 public immutable token; // blka token.
    ...
    /**
     * @notice Constructor
     * @param _token: Blka token contract
     * @param _masterchef: MasterChef contract
     * @param _admin: address of the admin
     * @param _treasury: address of the treasury (collects fees)
     * @param _operator: address of operator
     * @param _pid: blka pool ID in MasterChef
     */
    constructor(
        IERC20 _token,
    ...
    function init(IERC20 dummyToken) external onlyOwner {
        ...
        masterchef.deposit(blkaPoolPID, balance, 0);
        emit Init();
    }
    ...
    /**
     * @notice Checks if the msg.sender is either the blka owner address or the operator address.
     */
    modifier onlyOperatorOrBlkaOwner(address _user) {
        require(msg.sender == _user || msg.sender == operator, "Not operator or blka owner");
        _;
    }
    ...
    function updateUserShare(address _user) internal {
        ...
        if (user.shares > 0) {
            if (user.locked) {
                ...
                totalShares -= user.shares;
                user.shares = 0;
                uint256 earnAmount = totalAmount - user.blkaAtLastUserAction;
    ...
    /**
     * @notice Unlock user blka funds.
     * @dev Only possible when contract not paused.
     * @param _user: User address
     */
    function unlock(address _user) external onlyOperatorOrBlkaOwner(_user) whenNotPaused {
        UserInfo storage user = userInfo[_user];
    ...
    /**
     * @notice Deposit funds into the Blka Pool.
     * @dev Only possible when contract not paused.
    ...
    function depositOperation(
        ...
        user.blkaAtLastUserAction = (user.shares * balanceOf()) / totalShares - user.userBoostedShare;
    ...
    /**
     * @notice Withdraw funds from the Blka Pool.
    ...
    function withdrawOperation(uint256 _shares, uint256 _amount) internal {
        ...
        if (user.shares > 0) {
            user.blkaAtLastUserAction = (user.shares * balanceOf()) / totalShares;
        } else {
            user.blkaAtLastUserAction = 0;
        }
    ...
    function harvest() internal {
        uint256 pendingBlka = masterchef.pendingBlka(blkaPoolPID, address(this));
        if (pendingBlka > 0) {
            uint256 balBefore = available();
            masterchef.withdraw(blkaPoolPID, 0, 0);
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Blka Pool
     */
    ...
    function calculatePerformanceFee(address _user) public view returns (uint256) {
        ...
        if (user.shares > 0 && !user.locked && !freePerformanceFeeUsers[_user]) {
            uint256 pool = balanceOf() + calculateTotalPendingBlkaRewards();
     ...
     * @return Returns total pending blka rewards
     */
    function calculateTotalPendingBlkaRewards() public view returns (uint256) {
        uint256 amount = masterchef.pendingBlka(blkaPoolPID, address(this));
        return amount;
    }

    function getPricePerFullShare() external view returns (uint256) {
        return totalShares == 0 ? 1e18 : (((balanceOf() + calculateTotalPendingBlkaRewards()) * (1e18)) / totalShares);
    }
...
interface IMasterChefV2 {
    function deposit(uint256 _pid, uint256 _amount, uint256 _boost) external;

    function withdraw(uint256 _pid, uint256 _amount, uint256 _boost) external;
