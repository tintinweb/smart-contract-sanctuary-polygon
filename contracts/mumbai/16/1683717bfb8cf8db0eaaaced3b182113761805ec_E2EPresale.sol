...
contract E2EPresale is Ownable {
    using Address for address payable;
    using SafeERC20 for IERC20;

    event E2EPresalePurchase(address indexed payee, address indexed token, uint256 indexed amount);
    event TokenWithdrawal(address indexed payee, address indexed token, uint256 indexed amount);
    ...
    constructor(
        ...
        require(_e2e != address(0), "Invalid e2e address");
    ...
    function setERC20(address token, uint256 conversionRate) public onlyOwner {
        if(conversionRate != 0) {
    ...
    function setMinimumPurchase(uint256 newValue) public onlyOwner {
        minimumTicket = newValue;
    }
    
    function TokenPurchase(address token, uint256 amount) public whenNotPaused {
        require(whitelistedTokens[token], "This token is not whitelisted");
        ...
        uint256 amountPurchased = amount * conversionRates[token];
        require(amountPurchased >= minimumTicket, "Minimum ticket not satisfied");
        require(amountPurchased <= e2e.balanceOf(address(this)), "Amount exceeds contract's reserve");
        
        IERC20(token).safeTransferFrom(msg.sender, address(this), amount);
        e2e.safeTransfer(msg.sender, amountPurchased);

        emit E2EPresalePurchase(msg.sender, token, amount);
    }
    ...
    function withdrawTokens(address token, address recipient) public onlyOwner {
        ...
        emit TokenWithdrawal(recipient, token, IERC20(token).balanceOf(address(this)));
    }
}