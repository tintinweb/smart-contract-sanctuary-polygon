...
contract Game {
    ...
    function getStrength(uint256 _index) internal view returns (int) {
        return pluck(_index, "STRENGTH", attributesArray);
    }
    
    function getAgility(uint256 _index) internal view returns (int) {
        return pluck(_index, "AGILITY", attributesArray);
    }
    
    function getWisdom(uint256 _index) internal view returns (int) {
    ...
    function battle(string memory _name, int _str, int _agi, int _wis) public {
        ...
        // initialize spawn
        addSpawn(100, getStrength(currentIndex), getAgility(currentIndex), getWisdom(currentIndex));
