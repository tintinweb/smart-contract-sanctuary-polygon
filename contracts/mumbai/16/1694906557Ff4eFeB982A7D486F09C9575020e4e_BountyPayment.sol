...
contract BountyPayment is FunctionsClient {
  ...
  function createBounty(
    ...
    address ownerAddress,
    string calldata ownerUsername,
    BountyContributor[] calldata initialContributors
  ) external payable {
    require(msg.value >= 0, "Insufficient ETH to create the bounty");

    Bounty storage bounty = bounties[bountyCounter];
    bounty.issueNumber = issueNumber;
    bounty.bountyAmount = msg.value;
    bounty.repo = repo;
    bounty.ownerAddress = ownerAddress;
    bounty.ownerUsername = ownerUsername;

    for (uint256 i = 0; i < initialContributors.length; i++) {
      BountyContributor memory contributor = initialContributors[i];
      bounty.contributors[contributor.walletAddress] = true;
      bounty.contributorUsernames[contributor.walletAddress] = contributor.username;
      bounty.contributorsCount++;
    }

    bountyCounter++;
  }

  function applyToBounty(uint256 bountyId, string calldata githubUsername) external payable {
    require(bountyId < bountyCounter, "Invalid bounty ID");
    Bounty storage bounty = bounties[bountyId];

    require(bounty.contributors[msg.sender] == false, "You have already applied to this bounty");
    require(msg.sender != bounty.ownerAddress, "Owner cannot apply to their own bounty");

    bounty.contributors[msg.sender] = true;
    bounty.contributorUsernames[msg.sender] = githubUsername;
    bounty.contributorsCount++;
  }
  ...
  function executeRequest(
    ...
    //require(bounty.contributors[msg.sender] == true, "You are not authorized to execute this bounty");
  ...
  function fulfillRequest(bytes32 requestId, bytes memory response, bytes memory err) internal override {
    ...
    if (uint256(bytes32(response)) == 1) {
      payoutContract(msg.sender);
    } else {
      revert("No contribution found");
    }
  }

  function payoutContract(address contributor) internal onlyOwner {
    require(contributor != address(0), "Invalid contributor address");
    Bounty storage bounty = bounties[bountyCounter - 1];
    require(bounty.contributors[contributor] == true, "Invalid contributor");

    uint256 payoutAmount = bounty.bountyAmount;
    (bool sent, ) = contributor.call{value: payoutAmount}("");
    require(sent, "Failed to send Ether to the contributor");
  }
