...
contract testContract is ERC721, Ownable, PaymentSplitter {
    ...
    constructor(address[] memory _payees, uint256[] memory _shares) ERC721("test contract", "tst") PaymentSplitter(_payees, _shares) payable {
        //  Max amount of tokens
        maxAmntTokens = 100;
    ...
    function whitelistAddresses(address[] calldata _listAddressses) public onlyOwner {
        for(uint i = 0; i < _listAddressses.length; i++) {
            whitelistedUsers[_listAddressses[i]] = true;
        }    
    }

    //  Remove user from whitelist
    function removeWhitelist(address addressToRemove) public onlyOwner {
        whitelistedUsers[addressToRemove] = false;
    }
    ...
    function mintNFT(uint256 amountTokens) public payable {
        ...
        //  If presale is active requires that the msg.sender is whitelisted
        if (presaleIsActive) {
            require (whitelistedUsers[msg.sender], "You are not allowed in the presale");
        }
