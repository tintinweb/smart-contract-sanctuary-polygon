...
// File: bsc-library/contracts/IERC20.sol
...
contract PryzStakingContract is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    event NewAPY(uint256 rewardAPY);
    event Withdraw(address indexed user, uint256 amount);

    constructor(pinctoken _ipinctoken) {
        pryzToken = IERC20(0xFEACB1C56A2637990a1c9e486908eDAC54ee9709);
        rewardAPY = 20; // 20%
        startTime = block.timestamp;
        PRECISION_FACTOR = uint256(10**12);
        lastRewardTime = startTime;
        ipinctoken = pinctoken(_ipinctoken);
        
    }

     function SetSOS (bool _value) public onlyOwner {
        SOS = _value;
    }
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                pryzToken.safeTransfer(address(msg.sender), pending);
            }
        }

        if (_amount > 0) {
            user.amount = user.amount.add(_amount);
            pryzToken.safeTransferFrom(address(msg.sender), address(this), _amount);
            ipinctoken.mint(address(msg.sender), _amount);
        }
    ...
    function SOSdeposit(uint256 _amount) external nonReentrant {
        require(SOS == true);
        UserInfo storage user = userInfo[msg.sender];
        require(_amount > 0, "Can't deposit zero amount");
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        require(block.timestamp >= user.lastDeposit.add(vestingTime), "Vesting time requires!");
        ...
        if (_amount > 0) {
            ...
            pryzToken.safeTransfer(address(msg.sender), _amount);
            ipinctoken.burnFrom(address(msg.sender),_amount);
        }
     ...
     function SOSwithdraw(uint256 _amount) external nonReentrant {
        require(SOS == true);
        UserInfo storage user = userInfo[msg.sender];
        require(user.amount >= _amount, "Amount to withdraw too high");
        require(block.timestamp >= user.lastDeposit.add(vestingTime), "Vesting time requires!");
    ...
    function emergencyWithdraw() external nonReentrant {
        ...
        if (amountToTransfer > 0) {
            ...
            pryzToken.safeTransfer(address(msg.sender), amountToTransfer);
            ipinctoken.burnFrom(address(msg.sender),amountToTransfer);
        }
     ...
     * @dev Needs to be for emergency.
     */
    function SOSemergencyWithdraw() external nonReentrant {
        require(SOS == true);
        UserInfo storage user = userInfo[msg.sender];
        require(block.timestamp >= user.lastDeposit.add(vestingTime), "Vesting time requires!");
        uint256 amountToTransfer = user.amount;
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(pryzToken), "Cannot be staked token");
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.timestamp > lastRewardTime && stakedTokenSupply != 0) {
            uint256 currentRewardPerSecond = rewardAPY.mul(stakedTokenSupply).div(100).div(365 days);
            uint256 multiplier = _getMultiplier(lastRewardTime, block.timestamp);
            uint256 rewardAmount = multiplier.mul(currentRewardPerSecond);
    ...
    function withdrawReward(uint256 amount) external onlyOwner {
        ...
        pryzToken.safeTransfer(msg.sender, amount);
    }
