...
interface IRequestManager {
    ...
    function ForwardCreateBet(   
        ...
        uint tokenLiqidity_) external payable returns (bool _status,address _betTrendSetter);
    ...
    function ForwardResolveBet(address betAddress_,uint finalOption_,bytes32[] memory hash_,bytes memory maker_,bytes memory taker_,bool isCustomized_) external returns(bool);
    function ForwardWithdrawLiquidity(address betAddress_) external payable returns (bool);
    function ForwardBanBet(address betAddress_) external returns (bool _status);
...
interface IBetFoundationFactory {
    ...
    function banBet(address betAddress_) external returns(bool);
}
...
contract BetLiquidityHolder is IBetLiquidityHolder,ReentrancyGuard {
    ...
    event PostDrawDistribution(address betMaker_,address betTaker_,address admin_,uint betMakerAmount_,uint betTakerAmount_,uint adminAmount_);
    ...
    function processAaveDistributionForDraw(address registry_) public payable returns(bool) {
        if(tokenAddress == address(0)) {
            if(address(this).balance > totalAvailableLiquidity) {
                ...
                emit PostDrawDistribution(betCreator, betTaker, IConfig(registry_).getAdmin(), userLiquidity[betCreator], userLiquidity[betTaker], _adminAmount);
            } else {
                ...
                emit PostDrawDistribution(betCreator, betTaker, IConfig(registry_).getAdmin(), _bal/2, _bal/2, 0);
            }
        } else {
            ...
            if(_currentBal > totalAvailableLiquidity) {
                ...
                emit PostDrawDistribution(betCreator, betTaker, IConfig(registry_).getAdmin(), _currentBal/2, _currentBal/2, 0);
            }
        }
...
contract RequestManager is IRequestManager {
    ...
    function ForwardCreateBet(   
        ...
        uint totalBetOptions_,
        uint selectedOptionByUser_,
        uint tokenLiqidity_) external payable override returns (bool _status,address _betTrendSetter) {
            BetLiquidityHolder _blh = new BetLiquidityHolder();
            address __blh = address(_blh);
            (_status,_betTrendSetter) = IBetFoundationFactory(FoundationFactory).createBet{value:msg.value}(parentBet_,__blh, betTakerRequiredLiquidity_, betEndingTime_, tokenId_, totalBetOptions_, selectedOptionByUser_, tokenLiqidity_);
    ...
    function ForwardResolveBet(address betAddress_,uint finalOption_,bytes32[] memory hash_,bytes memory maker_,bytes memory taker_,bool isCustomized_) external override returns(bool _status) {
        (_status) = IBetFoundationFactory(FoundationFactory).resolveBet(betAddress_,finalOption_,hash_,maker_,taker_,isCustomized_);
    ...
    function ForwardBanBet(address betAddress_) external override returns (bool _status) {
        (_status) = IBetFoundationFactory(FoundationFactory).banBet(betAddress_);
