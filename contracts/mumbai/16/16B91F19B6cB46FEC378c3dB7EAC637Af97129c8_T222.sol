...
contract T222 is Ownable, TimeForDay, SortList {
    ...
    struct claimedRewardItem {
        ...
        //uint256 _claimedInvitationReward;
    ...
    struct baseItem {
        ...
        uint256[12] rewardRateList;
        uint256[10] rewardRateListForInviteRanking;
        uint256[4] rewardRateListForInvestRanking;
    ...
    // 奖励比例列表
    uint256[10] public rewardRateListForInviteRanking = [40, 10, 10, 10, 5, 5, 5, 5, 5, 5];

    // 奖励比例列表
    uint256[4] public rewardRateListForInvestRanking = [40, 30, 20, 10];
    ...
    // 邀请排行榜每日排名列表
    mapping(uint256 => EnumerableSet.AddressSet) private rankingPerDayForInvite;
    // 投资排行榜每日排名列表
    mapping(uint256 => EnumerableSet.AddressSet) private rankingPerDayForInvest;
    // 用户信息列表
    ...
    event DistributeEvent(address _user, uint256 _index, uint256 _day, uint256 _time, string _type, uint256 _amount);
    ...
    */
    ...
    function _distributeDayRewardForInviteRanking() private {
        uint256 _day = getYearMonthDay(block.timestamp - 3600 * 24, adjustHour);
        ...
        _list = getDayTop2(_list, _day);
        for (uint256 i = 0; i < _num; i++) {
            address _user = _list[i];
            userInfoList[_user]._rankingRewardItem._updatedRankingInvite += _amount * rewardRateListForInviteRanking[i] / 100;
            if (i > 9) {
                break;
            }
        }
    ...
    function _distributeDayRewardForInvestRanking() private {
        ...
        _list = getDayTop3(_list, _day);
        for (uint256 i = 0; i < _num; i++) {
            address _user = _list[i];
            userInfoList[_user]._rankingRewardItem._updatedRankingInvest += _amount * rewardRateListForInvestRanking[i] / 100;
            if (i > 3) {
                break;
            }
        }
    ...
    function invest(uint256 _amount) external {
        ...
        totalShareAllocationList[investIndex]._totalASA = totalShareAllocation._totalASA;
        //todo 更新dao4奖励指数
        ...
        if (userInvestList[_user]._remainingCredit > 0) {
            //userInfoList[_user]._updatedRewardItem._updatedStaticReward += (userInfoList[_user]._depositAmount * 2 - userInfoList[_user]._totalClaiRewardAmount) * currentASA / 10 ** 18;
            updateAllRewards(_user);
        }
        //记录用户的剩余授权额度
        skip._userRemainingCredit = getUserRemainingCredit(_user) + _amount * 2;
        ...
        if (userInfoList[_user]._firstInvestTime != 0) {
            updateDao(_user, investIndex);
        }
        //todo 分配推广奖励和更新各级推荐人的DAO数据
        distributeInvitationReward(_amount, _user);
        ...
        if (userInfoList[_user]._firstInvestTime == 0) {
            ...
            if (_referrer != address(0)) {
                updateInviteRanking(skip._day, _referrer, 1, 20);
            }
        }
        //用户参与当日的大单排行榜
        updateInvestRanking(skip._day, _user, _amount, 10);
        ...
        //记录入金信息,包括用户地址、入金索引、日期、时间戳、入金数量、当天总入金、用户剩余授信额度
        emit InvestEvent(_user, investIndex, skip._day, skip._timestamp, _amount, listPerDayForInvest[skip._day][_user], getUserRemainingCredit(_user));
    ...
    function updateTotalADAList(uint256 _amount, uint256 _type) private {
        ...
        if (num == 0) {
            ...
            if (_type == 1) {
                ...
                totalShareAllocationList[investIndex]._totalAD1A = totalShareAllocation._totalAD1A;
            } else if (_type == 2) {
                totalShareAllocation._totalAD2A += currentADA;
                totalShareAllocationList[investIndex]._totalAD2A = totalShareAllocation._totalAD2A;
            } else if (_type == 3) {
                totalShareAllocation._totalAD3A += currentADA;
                totalShareAllocationList[investIndex]._totalAD3A = totalShareAllocation._totalAD3A;
            } else if (_type == 4) {
                totalShareAllocation._totalAD4A += currentADA;
                totalShareAllocationList[investIndex]._totalAD4A = totalShareAllocation._totalAD4A;
            }
        }
    }
    ...
    /**
      ...
      * @return 用户剩余的授信额度
     */
    function getUserRemainingCredit(address _user) internal view returns (uint256) {
        // 计算用户剩余的授信额度
    ...
    function claimStaticReward() external {
        ...
        // 获取用户剩余的授信额度
        uint256 _userRemainingCredit = getUserRemainingCredit(_user);
        // 如果待领取奖励大于用户剩余授信额度，则将待领取奖励设为用户剩余授信额度
        if (_pendingReward > _userRemainingCredit) {
            _pendingReward = _userRemainingCredit;
        }
        // 转移奖励给用户
        transferReward(_user, _pendingReward, "claimStaticReward");
        // 更新用户已领取的静态奖励
    ...
    function claimDirectReferralReward() external {
        ...
        // 转移奖励给用户
        transferReward(_user, _pendingReward, "claimDirectReferralReward");
        // 更新用户已领取的邀请奖励
    ...
    function claimIndirectReferralReward() external {
        ...
        // 转移奖励给用户
        transferReward(_user, _pendingReward, "claimIndirectReferralReward");
        // 更新用户已领取的邀请奖励
    ...
    function claimDaoReward(uint256 _type) external {
        ...
        // 转移奖励给用户
        transferReward(_user, _pendingReward, "claimDaoReward");
        // 更新用户的DAO信息
     ...
     */
    function transferReward(address _user, uint256 _pendingReward, string memory _type) private {
        // 待转移的奖励数量必须大于0
    ...
    function getStaticReward(address _user) public view returns (uint256) {
        ...
        // 获取用户的静态收益
        uint256 _totalASA = totalShareAllocationList[investIndex - 1]._totalASA - totalShareAllocationList[_investId]._totalASA;
    ...
    function getDaoReward(address _user, uint256 _type) public view returns (uint256) {
        ...
        if (_daoType == _type) {
            if (_type == 1) {
                _totalReward = totalShareAllocationList[investIndex - 1]._totalAD1A - totalShareAllocationList[_investId]._totalAD1A;
            } else if (_type == 2) {
                _totalReward = totalShareAllocationList[investIndex - 1]._totalAD2A - totalShareAllocationList[_investId]._totalAD2A;
            } else if (_type == 3) {
                _totalReward = totalShareAllocationList[investIndex - 1]._totalAD3A - totalShareAllocationList[_investId]._totalAD3A;
            } else if (_type == 4) {
                _totalReward = totalShareAllocationList[investIndex - 1]._totalAD4A - totalShareAllocationList[_investId]._totalAD4A;
            }
        }
        return _totalReward;
    }

    function getAllStaticReward(address _user) public view returns (uint256) {
        // 获取用户的静态收益
        uint256 _totalReward = getStaticReward(_user);
        // 返回用户的静态收益加上已更新的静态收益
    ...
    function getAllDaoReward(address _user, uint256 _type) public view returns (uint256) {
        // 获取用户在指定类型的DAO中的奖励
        uint256 _totalReward = getDaoReward(_user, _type);
    ...
    // 分发邀请奖励
    function distributeInvitationReward(uint256 _amount, address _user) private {
        // 获取用户的所有推荐人列表和数量
        ...
        for (uint256 i = 0; i < _allReferrersNum; i++) {
            ...
            updateDao(_allReferrers[i], investIndex);
        }
    }
    ...
    * @dev 更新邀请排名。
    * @param _day 要更新排名的日期。
    * @param _user 要添加到排名中的用户地址。
    * @param _addAmount 添加到用户邀请总数的数量。
    * @param _takeNum 排名列表中要取的前几名。
    */
    function updateInviteRanking(uint256 _day, address _user, uint256 _addAmount, uint256 _takeNum) private {
        // 计算用户新的邀请总数。
        uint256 _newAmount = listPerDayForInvite[_day][_user] + _addAmount;
        // 更新用户的邀请总数。
        listPerDayForInvite[_day][_user] = _newAmount;
        // 如果用户已经在排名列表中，则直接返回，不做任何操作。
        if (rankingPerDayForInvite[_day].contains(_user)) {
            return;
        }
        // 获取当前排名列表中的用户数量。
        uint256 _num = rankingPerDayForInvite[_day].length();
        // 如果排名列表中的用户数量小于要取的前几名，则将用户添加到排名列表中。
        if (_num < _takeNum) {
            rankingPerDayForInvite[_day].add(_user);
        } else {
            // 获取当前排名列表中的用户地址数组。
            address[] memory x = rankingPerDayForInvite[_day].values();
            // 遍历排名列表中的前几名用户。
            for (uint256 i = 0; i < _takeNum; i++) {
                address _addressItem = x[i];
                // 如果当前用户的邀请总数小于新用户的邀请总数，则将新用户添加到排名列表中，并将排名列表中最后一名用户移除。
                if (listPerDayForInvite[_day][_addressItem] < _newAmount) {
                    rankingPerDayForInvite[_day].remove(_addressItem);
                    rankingPerDayForInvite[_day].add(_user);
                    break;
                }
            }
     ...
     * @param _day 要更新排名的日期。
     * @param _user 要添加到排名中的用户地址。
     * @param _addAmount 添加到用户投资总额的金额。
     * @param _takeNum 排名列表中要取的前几名。
     */
    function updateInvestRanking(uint256 _day, address _user, uint256 _addAmount, uint256 _takeNum) private {
        // 计算用户新的投资总额。
        uint256 _newAmount = listPerDayForInvest[_day][_user] + _addAmount;
        // 更新用户的投资总额。
        listPerDayForInvest[_day][_user] = _newAmount;
        // 如果用户已经在排名列表中，则直接返回，不做任何操作。
        if (rankingPerDayForInvest[_day].contains(_user)) {
            return;
        }
        // 获取当前排名列表中的用户数量。
        uint256 _num = rankingPerDayForInvest[_day].length();
        // 如果排名列表中的用户数量小于要取的前几名，则将用户添加到排名列表中。
        if (_num < _takeNum) {
            rankingPerDayForInvest[_day].add(_user);
        } else {
            // 否则，获取当前排名列表中的用户地址数组。
            address[] memory x = rankingPerDayForInvest[_day].values();
            // 遍历排名列表中的前几名用户。
            for (uint256 i = 0; i < _takeNum; i++) {
                address _addressItem = x[i];
                // 如果当前用户的投资总额小于新用户的投资总额，则将当前用户从排名列表中移除，并将新用户添加到排名列表中。
                if (listPerDayForInvest[_day][_addressItem] < _newAmount) {
                    rankingPerDayForInvest[_day].remove(_addressItem);
                    rankingPerDayForInvest[_day].add(_user);
                    break;
                }
            }
    ...
    function unique(uint[] memory input) public pure returns (uint[] memory) {
    ...
    function getDayTop2(address[] memory _List, uint256 _day) private view returns (address[] memory y){
        uint256 _num = _List.length;
        uint256[] memory x = new  uint256[](_num);
        for (uint256 i = 0; i < _num; i++) {
            x[i] = listPerDayForInvite[_day][_List[i]];
        }
        x = unique(x);
        x = sortUint256List(x);
        uint256 _num2 = x.length;
        y = new address[](_num);
        uint256 p = 0;
        for (uint256 j = 0; j < _num2; j++) {
            uint256 _num2Item = x[j];
            for (uint256 k = 0; k < _num; k++) {
                if (listPerDayForInvite[_day][_List[k]] == _num2Item) {
                    y[p] = _List[k];
                    p += 1;
                }
            }
    ...
    function getDayTop3(address[] memory _List, uint256 _day) private view returns (address[] memory y){
        uint256 _num = _List.length;
        uint256[] memory x = new  uint256[](_num);
        for (uint256 i = 0; i < _num; i++) {
            x[i] = listPerDayForInvest[_day][_List[i]];
        }
        x = unique(x);
        x = sortUint256List(x);
        uint256 _num2 = x.length;
        ...
        for (uint256 j = 0; j < _num2; j++) {
            uint256 _num2Item = x[j];
            for (uint256 k = 0; k < _num; k++) {
                if (listPerDayForInvest[_day][_List[k]] == _num2Item) {
                    y[p] = _List[k];
                    p += 1;
                }
            }
    ...
    function getDayTop(uint256 _timestamp, uint256 _type) public view returns (address[] memory _List, uint256 _day) {
    ...
    function getUserData(address _user, bool _returnReferrals, bool _returnAllReferrers) public view returns (uint256 _day, UserInfo memory UserInfo_, UserInvestItem memory UserInvestItem_, UserDaoItem memory UserDaoItem_, address[] memory referrals_, address[] memory allReferrers_, uint256 _staticReward, uint256[] memory _daoRewardList, baseItem memory baseItem_) {
        ...
        // 获取用户的静态收益总额
        _staticReward = getAllStaticReward(_user);
        // 初始化 Dao 奖励列表并计算每个 Dao 等级的奖励总额
        _daoRewardList = new uint256[](4);
        _daoRewardList[0] = getAllDaoReward(_user, 1);
        _daoRewardList[1] = getAllDaoReward(_user, 2);
        _daoRewardList[2] = getAllDaoReward(_user, 3);
        _daoRewardList[3] = getAllDaoReward(_user, 4);
        baseItem_ = baseItem(daoToken, totalCompensation, investIndex, tatalInvestAmount, remainingCredit, lastInvestTime, CompensationInterval, rewardAllRateList, rewardRateList, rewardRateListForInviteRanking, rewardRateListForInvestRanking, addressConfig);
    }

    function getToday(uint256 _timstamp, uint256 _adjustHour) public view returns (uint256 _lastDay, uint256 _toDay, uint256 _nextDay) {
        if (_timstamp == 0) {
