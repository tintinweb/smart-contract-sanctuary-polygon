...
contract Tournaments is Ownable {
    ...
    event TournamentAdded(uint256 id, uint256 lobbySize, uint256 startTime, uint256 endTime);
    ...
    modifier tourIsGoing(uint256 _tournamentId) {
    ...
    modifier isJoined(uint256 _tournamentId) {
    ...
    modifier isTourFull(uint256 _tournamentId) {
        Tournament memory tournament = tournaments[_tournamentId - 1];
        require(tourParticipants[_tournamentId].length < tournament.lobbySize, "Tournament is full");
        _;
    }
    
    function addTournament(uint256 _lobbySize, uint256 _startTime, uint256 _endTime) public onlyOwner {
        uint256 id = tournaments.length + 1;
        tournaments.push(Tournament(id, _lobbySize, _startTime, _endTime));
        emit TournamentAdded(id, _lobbySize, _startTime, _endTime);
    }
    ...
    function getActiveTournaments() public view returns (Tournament[] memory) {
        ...
        for (uint256 i = 0; i < tournaments.length; i++) {
            if ((block.timestamp >= tournaments[i].startTime) && (block.timestamp <= tournaments[i].endTime)) {
                count++;
            }
        }
    ...
    function joinTournament(uint256 _tournamentId) public tourIsGoing(_tournamentId) isJoined(_tournamentId) isTourFull(_tournamentId) returns(bool) {
        Tournament memory tournament = tournaments[_tournamentId - 1];
    ...
    function addScore(uint256 _tournamentId, address _user, uint _score) public tourIsGoing(_tournamentId) onlyOwner returns(bool _done) {
    ...
    function getLeaderboard(uint256 _tournamentId) public view returns(User[] memory) {
        Tournament memory tournament = tournaments[_tournamentId - 1];
        require(block.timestamp > tournament.endTime, "Tournament has not ended yet");
