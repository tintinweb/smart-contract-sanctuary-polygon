    ...
    CrowdsaleAccessControl
{
    ...
    /**@dev admin can reserve tokens with free mint.
     * @param _tokenAmount no. of tokens to mint
     */

    function reserveTokens(uint256 _tokenAmount) public onlyOwner {
        _updatePurchasingState(msg.sender, _tokenAmount, 0);

        _processPurchase(msg.sender, _tokenAmount);

        emit TokenPurchase(msg.sender, 0, _tokenAmount);
    }
