 ...
 *
 */
error Structured10MRelease__NotMinter();
error Structured10MRelease__InvalidArg_TokenAndAddressLengthMismatch();
error Structured10MRelease__InvalidArg_AddressZero();
error Structured10MRelease__InvalidArg_SellerFeeBasisPoints(uint16 basisPoints);
error Structured10MRelease__AlreadyMinted(uint256 tokenId);

contract Structured10MRelease is ERC1155Upgradeable, OwnableUpgradeable, BaseRelayRecipient, IERC2981 {
  struct RoyaltyInfo {
    address recipient;
    uint16 basisPoints;
  }
   ...
   */
  Structured10MRouter public _router;
  ...
  modifier onlyMinter() {
    ...
    if (sender != minter && sender != prevMinter) {
      revert Structured10MRelease__NotMinter();
    }
    _;
  }
  ...
  function initialize(RoyaltyInfo calldata _royalties, address routerAddress, address minterAddress, address owner, address forwarder, string calldata contracturi) public initializer {
    if(_royalties.basisPoints > 10000){
      revert Structured10MRelease__InvalidArg_SellerFeeBasisPoints(_royalties.basisPoints);
    }
    __ERC1155_init("_");
  ...
  function _setRouterAddress(address routerAddress) internal {
    if (routerAddress == address(0)){
      revert Structured10MRelease__InvalidArg_AddressZero();
    }
  ...
  function mintNFT(uint256 tokenId, address ownerId) public onlyMinter {
    if (_balances[tokenId][ownerId] >= 1) {
      revert Structured10MRelease__AlreadyMinted(tokenId);
    }
  ...
  function batchMint(uint256[] calldata tokenIds, address[] calldata ownerIds) public onlyMinter {
    if (tokenIds.length != ownerIds.length) {
      revert Structured10MRelease__InvalidArg_TokenAndAddressLengthMismatch();
    }
    for (uint256 i=0; i < tokenIds.length; i++) {
      if (_balances[tokenIds[i]][ownerIds[i]] >= 1) {
        revert Structured10MRelease__AlreadyMinted(tokenIds[i]);
      }
...
error Structured10MRouter__UndefinedRouteForItemIndex(uint256 itemIndex, uint totalRoutes, uint256 itemsPerRoute);
error Structured10MRouter__NotRouteRegisterer();
error Structured10MRouter__InvalidArg_AddressZero();
error Structured10MRouter__InvalidArg_ItemsPerRouteZero();
...
contract Structured10MRouter is OwnableUpgradeable {
   ...
   */
  function initialize(uint256 initialRoutesCount, uint256 _itemsPerRoute, address _routeRegisterer, address owner) public initializer {
    routes = new string[](initialRoutesCount);
  ...
  function registerRoute(uint256 index, string calldata cid) public {
    if (_msgSender() != routeRegisterer) {
      revert Structured10MRouter__NotRouteRegisterer();
    }
  ...
  function lookup(uint256 itemIndex) public view returns (string memory) {
    ...
    string memory cid = index < routes.length ? routes[index] : "";
    if (bytes(cid).length == 0) {
      revert Structured10MRouter__UndefinedRouteForItemIndex({itemIndex: itemIndex, totalRoutes: routes.length, itemsPerRoute: itemsPerRoute});
    }
    return cid;
  }
  ...
  function setRouteRegisterer(address account) public onlyOwner {
    if (account == address(0)){
      revert Structured10MRouter__InvalidArg_AddressZero();
    }
  ...
  function setItemsPerRoute(uint256 _itemsPerRoute) public onlyOwner {
    if (_itemsPerRoute == 0){
      revert Structured10MRouter__InvalidArg_ItemsPerRouteZero();
    }
