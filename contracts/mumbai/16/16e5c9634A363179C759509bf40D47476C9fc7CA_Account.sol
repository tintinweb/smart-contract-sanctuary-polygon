...
contract Account is Ownable {
    event UserCreated(string _email, string imageUrl, bytes32 _nickname);
    event UserDeleted(string _email);
    ...
    function createUser(
        ...
        require(isEmailUnique(_email), "Account: user already exists");
        UserData memory userData = UserData(_logoUrl, _nickname);
        emailToUserData[_email] = userData;
        usersCount++;
        emit UserCreated(_email, _logoUrl, _nickname);
    }
     ...
     */
    function isEmailUnique(string calldata _email)
        internal
        view
        returns (bool)
    {
        return (emailToUserData[_email].nickname[0].length == 0);
    }
     ...
     */
    function getUser(string calldata _email)
        external
        view
        onlyOwner
        returns (UserData memory)
    {
        require(!isEmailUnique(_email), "Account: user doesn't exist");
        return emailToUserData[_email];
    }
     ...
     * @param _logoUrl string user's logo url
     */
    function modifyLogo(string calldata _email, string calldata _logoUrl)
        external
        onlyOwner
    {
        require(!isEmailUnique(_email), "Account: user doesn't exist");
        emailToUserData[_email].logoUrl = _logoUrl;
    }

    /**
     * @dev Modifies users logo
     * @param _email string user's email
     * @param _nickname bytes32 user's image url
     */
    function modifyNickname(string calldata _email, bytes32 _nickname)
        external
        onlyOwner
    {
        require(!isEmailUnique(_email), "Account: user doesn't exist");
        emailToUserData[_email].nickname = _nickname;
    }
     ...
     * @param _email string user's email
     */
    function deleteUser(string calldata _email) external onlyOwner {
