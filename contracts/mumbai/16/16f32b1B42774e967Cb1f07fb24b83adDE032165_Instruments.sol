...
contract Instruments is ERC721Base {
    struct Token {
    ...
    mapping (uint256 => Token[]) private _certTokens;
    ...
    function getCertTokens(uint256 tokenId) public view returns (Token[] memory) {
        return _certTokens[tokenId];
    }

    function addCertToken(uint256 tokenId, address contractAddress, uint256 certTokenId) public {
        require(_exists(tokenId), "Not exist");

        require(IERC721(contractAddress).ownerOf(certTokenId) == msg.sender, "Not owner of cert token");
        require(IERC721(address(this)).ownerOf(tokenId) == msg.sender, "Not owner of instrument token");

        _certTokens[tokenId].push(Token(contractAddress, certTokenId, msg.sender));

        IERC721(contractAddress).transferFrom(msg.sender, address(this), certTokenId);
       
        emit CertTokenAdded(tokenId, contractAddress, certTokenId);
    }

    function removeCertToken(uint256 tokenId, address contractAddress, uint256 certTokenId) public {
        require(_exists(tokenId), "Not exist");
        require(_existsCertToken(tokenId, contractAddress, certTokenId), "Token not exist");

        uint256 certTokenIndex = _certTokenIndex(tokenId, contractAddress, certTokenId);

        Token memory certToken = _certTokens[tokenId][certTokenIndex];

        require(certToken.owner == msg.sender,
            "Cert token owner not match");

        IERC721(certToken.contractAddress).transferFrom(address(this), msg.sender, certToken.tokenId);

        _certTokens[tokenId][certTokenIndex] = _certTokens[tokenId][_certTokens[tokenId].length - 1];
        _certTokens[tokenId].pop();
        emit CertTokenRemoved(tokenId, certToken.contractAddress, certToken.tokenId);
    }

    event CertTokenAdded(uint256 indexed tokenId, address indexed contractAddress, uint256 indexed certTokenId);
    event CertTokenRemoved(uint256 indexed tokenId, address indexed contractAddress, uint256 indexed certTokenId);

    function _existsCertToken(uint256 tokenId, address contractAddress, uint256 certTokenId) public view returns (bool) {
        Token[] memory certTokens = _certTokens[tokenId];

        for (uint i = 0; i < certTokens.length; i++) {
            if (certTokens[i].tokenId == certTokenId &&
                certTokens[i].contractAddress == contractAddress
            )  {
                return true;
            }
        }
    ...
    function _certTokenIndex(uint256 tokenId, address contractAddress, uint256 certTokenId) internal view returns (uint256) {
        Token[] memory certTokens = _certTokens[tokenId];

        for (uint256 i = 0; i < certTokens.length; i++) {
            if (certTokens[i].tokenId == certTokenId &&
                certTokens[i].contractAddress == contractAddress
            )  {
                return i;
    ...
    function _afterTokenTransfers(address from, address to, uint256 startTokenId,  uint256 quantity) internal override {
        ...
        if (from != address(0)) {   // bypass minting transfers
            // console.log("Transferring from %s to %s tokenId %s", from, to, startTokenId);

            Token[] memory certTokens = getCertTokens(startTokenId);

            for (uint i = 0; i < certTokens.length; i++) {
                _certTokens[startTokenId][i].owner = to;            
            }
        }
    }
}
