...
contract Comments is Ownable {
  ...
  uint256 public _registryTableId;
  string public _registryTable;
  uint256 public _commentsTableId;
  ...
  function createRegistryTable() public onlyOwner returns (uint256) {
    _createRegistryTable();
    return _registryTableId;
  }

  function createCommentsTable() public onlyOwner returns (uint256) {
    _createCommentsTable();
    return _commentsTableId;
  }

  function addCommentSql(uint256 id, uint256 created_at, string memory authorAddress, string memory body, uint256 contract_id, uint256 token_id, uint256 parent_id) public view returns (string memory) {
    return string.concat(
      "INSERT INTO ",
      _commentsTable,
      " (id, created_at, address, body, contract_id, token_id, parent_id) VALUES (",
      Strings.toString(id), ',',
  ...
  function addComment(uint256 id, uint256 created_at, string memory authorAddress, string memory body, uint256 contract_id, uint256 token_id, uint256 parent_id) public {
    _tableland.runSQL(
      address(this),
      _commentsTableId,
      addCommentSql(id, created_at, authorAddress, body, contract_id, token_id, parent_id)
    );
  }

  function addCollectionSql(uint256 id, string memory contractAddress, uint256 chain_id, string memory collectionName, string memory collectionSymbol) public view returns (string memory) {
    return string.concat(
      "INSERT INTO ",
      _registryTable,
  ...
  function addCollection(uint256 id, string memory contractAddress, uint256 chain_id, string memory collectionName, string memory collectionSymbol) public {
    _tableland.runSQL(
      address(this),
      _registryTableId,
      addCollectionSql(id, contractAddress, chain_id, collectionName, collectionSymbol)
    );
  }
  ...
  function createRegistryTableSql() public view returns (string memory) {
    return string.concat(
      "CREATE TABLE ",
      "nfcregistry_",
  ...
  function _createRegistryTable() internal {
    ...
    _registryTableId = _tableland.createTable(
      address(this),
      'CREATE TABLE nfcregistry_80001 (id INT, address TEXT, chain_id INT, name TEXT, symbol TEXT);' // createRegistryTableSql()
    );
