...
contract SummoningService is Base, ISummoningService {
  ...
  function summonNewChampion(
    ...
    // mint champion
    uint256 newChampionId = ISummonChampion(summonChampion).mintTo(_owner, _uriName);
  ...
  function summon(
    ...
    (
      ...
      ChampionAttributeTypes.GeneralAttributes memory attributes,
      string memory uriName,
      address summonOwner,
      uint256 timer
    ) = abi.decode(_params, (SummonTypes.SummonSessionInfo, ChampionAttributeTypes.GeneralAttributes, string, address, uint256));
    ...
    require(IChampionUtils(championUtils).isOwnerOf(signer, sessionInfo.parents[1].champion_id), "TRV: Require owner");
    require(IChampionUtils(championUtils).isOwnerOf(sessionInfo.parents[0].owner, sessionInfo.parents[0].champion_id), "TRV: Donor delisted");
    ...
    summonNewChampion(sessionInfo, summonOwner, uriName, attributes, _parents);
  }
  ...
  function eligibleSummon(SummonTypes.SummonSessionInfo memory _sessionInfo) public view returns (bool, string memory) {
    ...
    if (ISummoningState(summoningState).getTotalChampionsSummonedInSession(_sessionInfo.id) >= _sessionInfo.max_champions_summoned) {
      return (false, "Exceed max summoning champions in session");
    }
    ...
    for (uint256 i; i < _sessionInfo.parents.length; i++) {
      ...
      if (
        ...
        return (false, "Exceed max participate in per champion");
      }
      ...
      if (
        ...
        return (false, "Exceed max summoning per champion");
      }
