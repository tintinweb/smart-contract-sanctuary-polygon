...
contract Lending {
    ...
    function inititateLend (Order memory order, Sign memory sign) external returns(bool){
        ...
        uint price = order.unitPrice * 1000 / (1000 + buyerFee);
        ...
        require(price >= platformFee, "Subtraction overflow");
        uint256 assetPrice = price - platformFee;
        ...
        LoanDetail memory details = LoanDetail({
            ...
            borrower: msg.sender,
            lender: order.lender,
            loanPrincipalAmount: order.unitPrice,
            loanRepaymentAmount: order.unitPrice + interest,
        ...
        if(platformFee > 0) {
            IERC20(order.erc20Address).transferFrom(order.borrower, owner, platformFee);
        }
    ...
    function loanRepayment(uint256 _loanId, Sign memory sign) external returns(bool) {
        ...
        if(interestDue > 0) {
            IERC20(loans[_loanId].loanERC20Address).transferFrom(loans[_loanId].borrower, loans[_loanId].lender, interestDue);
        }
