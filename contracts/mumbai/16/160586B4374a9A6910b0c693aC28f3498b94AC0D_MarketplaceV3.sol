...
abstract contract PlatformFeeLogic is IPlatformFee {
    ...
    function getPlatformFeeInfo(address _currency) public view override returns (address, uint16) {
        ...
        if (_currency == data.discountCurrency) {
            return (data.platformFeeRecipient, uint16(data.discountBps));
        }
    ...
    function _setupPlatformFeeInfo(address _platformFeeRecipient, uint256 _platformFeeBps, address _discountCurrency, uint256 _discountBps) internal {
        ...
        data.discountCurrency = _discountCurrency;
        data.discountBps = uint16(_discountBps);
...
library PlatformFeeStorage {
    ...
    struct Data {
        ...
        /// @dev The currency address that results in a discounted % platform fee.
        address discountCurrency;
        /// @dev The % discounted platform fee.
        uint16 discountBps;
    }
