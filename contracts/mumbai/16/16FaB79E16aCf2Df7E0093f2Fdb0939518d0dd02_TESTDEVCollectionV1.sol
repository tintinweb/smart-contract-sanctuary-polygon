/** @notice The  TEST contracts have the responceblity for Minting NFTs , transferring , burning NFTs from the wallets
...
//@title TEST having the nature of ERC721A with Royalties.
//@dev see{ERC721A, ERC2981, ReentrancyGuard, AccessControl}.

contract TESTDEVCollectionV1 is ERC721A, ERC2981, ReentrancyGuard, AccessControl {

    //@notice _tokenURIs is an array value used for assigning tokenURIs to tokenIds.
    ...
    //@notice signer holds current contract signer.
    ...
    //@notice TESTProxy holds current TESTProxy address.

    address public TESTProxy;
    ...
    constructor() ERC721A("NFT_TEST", "TEST") {
        owner = _msgSender();
        signer = _msgSender();
    ...
    //@notice changeSigner for transferring contract signer position to new signer address.
    //@param newSigner address of new signer.
    //@return bool value always true. 
    /** restriction: the ADMIN_ROLE address only has the permission to 
    chnage the contract signer position to new wallet address.*/
    //@dev see{Accesscontrol}.
    // emits {SignerChanged} event.
    ...
    function contractURI() external pure returns (string memory) {
        return "https://testipfs.infura-ipfs.io/ipfs/QmfFV9NLNxhKHNqnwuWqWgTxK3Xnaec62f6azu8BV5Twyk";
    }

    function setRoyaltyReceiver(address _receiver) external onlyRole(ADMIN_ROLE){
     ...
     * @param royalty array of royaltyFee permiles.
     ...
     * The royalty is cleared when the token is burned.
     *
     * Requirements:
     *
     * - `tokenId` must exist.
    ...
    */
    ...
    function verifySign(
        ...
        bytes32 hash = keccak256(
            abi.encodePacked(this, _msgSender(), _tokenURI, sign.nonce)
        );
