...
/// @dev status == 0 Contract is in Migrate mode, disable deposit-related features, keeping all withdrawal-related features enabled during the migration time. Prepare for migrating to a new contract version.
/// @dev status == 1 Contract is Active mode, allow all features.
/// @dev status == 2 Contract is Maintenance mode, disable all deposit & withdrawal-related features. Anyone can turn off Maintenance mode after it reachs the expiration time.
/// @dev status == 3 Contract is MassExit mode, enable exit features, only available when no operations are performed before the expiration time.
...
contract StatusControlFacet {
    ...
    /// @notice
    event UpdateDefaultMaintenanceTime(uint256 defautMaintenanceTime);

    /// @notice
    event UpdateDefaultMigrationTime(uint256 defautMigrationTime);

    /* ------ EXTERNAL FUNTIONS ------ */

    /// @notice
    function setDefaultMaintenanceTime(uint256 _time) external {
        LibAccessControl._requireGov();
        s.defaultMaintenanceTime = _time;
        emit UpdateDefaultMaintenanceTime(_time);
    }

    /// @notice
    function setDefaultMigrationTime(uint256 _time) external {
        LibAccessControl._requireGov();
        s.defaultMigrationTime = _time;
        emit UpdateDefaultMigrationTime(_time);
    }
    ...
    function enableMigrate() external {
        ...
        s.status = 0;
        s.migrationExpirationTime = block.timestamp + s.defaultMigrationTime;
        emit Migrate();
    }
    ...
    function switchMaintenance() external {
        if (s.status == 2) {
            ...
            s.status = 2;
            s.maintenanceExpirationTime = block.timestamp + s.defaultMaintenanceTime;
            emit Maintenance();
        }
    }
}
