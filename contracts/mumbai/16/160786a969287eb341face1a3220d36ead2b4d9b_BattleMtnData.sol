...
// File: final contracts/BattleMtnData.sol
...
contract BattleMtnData is AccessControl, VRFConsumerBase {
    ...
    function setSlogan(uint8 position, string memory _slogan) public {
        IABToken ABTokenData = IABToken(ABTokenDataContract);
        require(ABTokenData.ownerOf(MTZion[position].angelId) == msg.sender, 'not owner');
        ...
        )
    {
        ...
        //defend
        if (desiredAction == 2) {
            uint8 extraDefense = getRandomNumber(10, 5, msg.sender);
            action = 4;
