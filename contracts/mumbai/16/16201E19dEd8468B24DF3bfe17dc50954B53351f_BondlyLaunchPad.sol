...
contract BondlyLaunchPad is Ownable {
    ...
    event CardPriceAdded(
        uint256 _cardId,
        address _paymentToken,
        uint256 _newPrice
    );
    event CardPriceRemoved(uint256 _cardId, address _paymentToken);
    ...
    mapping(uint256 => mapping(address => bool)) public _cardIsAvailableToken;
    mapping(uint256 => address[]) public _cardPaymentTokens;
    mapping(uint256 => mapping(address => uint256)) public _cardPrice;
    ...
    function purchaseNFT(
        ...
        require(
            _cardIsAvailableToken[_cardId][_paymentToken],
        ...
        uint256 _price = _cardPrice[_cardId][_paymentToken] *
            _availableForPurchase;
    ...
    function addCardPrice(
        uint256 _cardId,
        address _paymentToken,
        uint256 _newPrice
    ) external onlyOwner {
        _cardPrice[_cardId][_paymentToken] = _newPrice;
        if (!_cardIsAvailableToken[_cardId][_paymentToken]) {
            _cardPaymentTokens[_cardId].push(_paymentToken);
            _cardIsAvailableToken[_cardId][_paymentToken] = true;
        }

        emit CardPriceAdded(_cardId, _paymentToken, _newPrice);
    }

    function removeCardPrice(uint256 _cardId, address _paymentToken)
        external
        onlyOwner
    {
        if (_cardIsAvailableToken[_cardId][_paymentToken]) {
            delete _cardIsAvailableToken[_cardId][_paymentToken];
            delete _cardPrice[_cardId][_paymentToken];

            for (uint256 i = 0; i < _cardPaymentTokens[_cardId].length; i++) {
                if (_cardPaymentTokens[_cardId][i] == _paymentToken) {
                    _cardPaymentTokens[_cardId][i] = _cardPaymentTokens[
                        _cardId
                    ][_cardPaymentTokens[_cardId].length - 1];
                    _cardPaymentTokens[_cardId].pop();
                }
            }

            emit CardPriceRemoved(_cardId, _paymentToken);
        }
    }
    ...
    function isCardFree(uint256 _cardId, address _paymentToken)
        public
        view
        returns (bool)
    {
        return _cardPrice[_cardId][_paymentToken] == 0;
    }
    ...
    {
        return _cardPaymentTokens[_cardId];
    }
    ...
    function getCardBasePrice(uint256 _cardId, address _paymentToken)
        public
        view
        returns (uint256)
    {
        return _cardPrice[_cardId][_paymentToken];
    }
