...
interface ITokenGated
{
    ...
    function updateGate(bytes4 name, address tokenAddress) external;
}
...
contract WhitelistHandler is BaseWhitelistHandler
{
     ...
     */
    event Whitelisted(address indexed admin, address indexed whitelisted, bytes4 indexed whitelist);
    /**
     ...
     */
    event Unwhitelisted(address indexed admin, address indexed unwhitelisted, bytes4 indexed whitelist);
    ...
    returns (bool[] memory whitelisted, Whitelist[] memory whitelistDef)
    {
        ...
        for(uint256 i = 0 ; i < addresses.length ; i++)
        {
            ...
            (bool isWhitelisted_, Whitelist memory whitelist_) = isWhitelisted(
                addresses[i], whitelistNames[i],
                additionalData.length == 0 ? new bytes(0) : additionalData[i]);
