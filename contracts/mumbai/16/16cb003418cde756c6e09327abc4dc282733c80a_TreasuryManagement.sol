...
contract TreasuryManagement is Initializable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    /// @notice An address source for the contract to send all collected fees to
    address payable public verumTreasuryAddress;
    ...
    function delegatedConstructor(
        ...
        address payable _verumTreasuryAddress,
        uint256 _mintFee,
        ...
        mintFee = _mintFee;
        verumTreasuryAddress = _verumTreasuryAddress;
    ...
    fallback() external payable {
        ...
        verumTreasuryAddress.transfer((msg.value * 10).div(100));
        emit etherSent(
        ...
        returns (bool success, uint256 _fee)
    {
        ...
        if (!IWhitelist(addressRegistry.whitelistContract()).hasRoleAdmin(_sender)) {
            ...
            token.transferFrom(_sender, verumTreasuryAddress, (fee * 10).div(100));
    ...
    function getAllowance(address _token) public view returns (uint256) {
        ...
        return IERC20Upgradeable(_token).allowance(sender, address(this));
    }

    function getTokenByIndex(uint256 _index) public view returns (address tokenAddress) {
        require(_index < whitelistedTokenAddresses.length, "Index out of bounds");
    ...
    function getPriceOfTokenByIndex(uint256 _index) public returns (int256 value) {
        return (getPriceOfTokenByAddress(getTokenByIndex(_index)));
    }

    function getOracleByIndex(uint256 _index) public view returns (address tokenAddress) {
