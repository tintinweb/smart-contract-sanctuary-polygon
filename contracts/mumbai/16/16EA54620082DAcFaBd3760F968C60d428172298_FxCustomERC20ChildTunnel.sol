 ...
 */
contract FxCustomERC20ChildTunnel is FxBaseChildTunnel {
    ...
    function _mapToken(bytes memory syncData) internal returns (address) {
        (address rootToken, string memory name, string memory symbol, uint8 decimals, address childToken) = abi.decode(
            syncData,
            (address, string, string, uint8, address)
        );

        // check if it's already mapped
        require(childToken == address(0x0), "FxCustomERC20ChildTunnel: ALREADY_MAPPED");
    ...
    function _withdraw(
        ...
        require(
            ...
            "FxCustomERC20ChildTunnel: NO_MAPPED_TOKEN"
        );
