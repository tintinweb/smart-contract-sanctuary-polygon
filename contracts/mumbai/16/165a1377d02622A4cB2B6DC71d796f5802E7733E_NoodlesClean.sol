...
contract NoodlesClean is ERC721, ERC721Enumerable, Ownable {
    ...
    constructor(string memory baseURI_, address beneficiary_)
        ERC721("Noodles", "NOODLE")
    {
        baseURI = baseURI_;
    ...
    function mint(bytes32[] calldata merkleProof) external payable {
        ...
        require(!alreadyMinted[msg.sender], "Only 1 mint per address allowed");
        require(
            MerkleProof.verify(
                merkleProof,
                merkleRoot,
        ...
        require(
            ...
            "Minting from smart contract not allowed"
        );

        uint256 tokenId = totalSupply() + 1;
        require(tokenId <= MAX_SUPPLY, "Minting would exceed max supply");
