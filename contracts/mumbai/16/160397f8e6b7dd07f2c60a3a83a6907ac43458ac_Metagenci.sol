...
contract Metagenci is ERC1155 {
   ...
   function TokenMintBatch(address account,uint256[] memory newItemId, uint256[] memory amounts, string[] memory metadata, string[] memory name) public onlyOwner returns (uint256[] memory){
     uint256 _length= amounts.length;
     uint256[] memory arrIds = new uint256[](_length);
     delete _lastGeneratedToken;
     
      for(uint i = 0; i < amounts.length; i++){
	      require(!tokenCheck[newItemId[i]],"TokenID already exists");
          arrIds[i]=newItemId[i];
          tokenCheck[newItemId[i]] = true;
		  tokenHash[newItemId[i]] = metadata[i];
		  tokenName[newItemId[i]] = name[i];
        
          _lastGeneratedToken.push(newItemId[i]);
      }
       _mintBatch(account,arrIds,amounts, '');       
       return arrIds;
    ...
    function TokenBurnBatch(address account,uint256[] memory id, uint256[] memory amount) public onlyOwner returns (bool){   
