...
contract CorporationAvatars is ERC1155SupplyUpgradeable, OwnableUpgradeable, IAssetManager, ITrustedMintable {
     ...
     */
    function setMaxSupplies(uint256[] calldata _tokenIds, uint256[] calldata _maxSupplies)
        external
        onlyOwner
    {
        ...
        for (uint256 i; i < suppliesMem.length; i++) {
            require(totalSupply(tokenIdsMem[i]) <= suppliesMem[i], "CA: AMOUNT_TOO_SMALL");
    ...
    function _safeMint(
        ...
        uint256 idToMint;
        for(uint i = indexOfTokenId; i < s_tokensByRarity.length; i++) {
