...
contract PledgeStakeV2 {
    ...
    mapping(address => GameBet[]) public gameBets;
    struct StakeInfo {
        uint256 amount;
        uint256 startTime;
        bool unstaked;
    ...
    function _updateRewards(address _user) private {
        ...
        for (uint256 i = 0; i < stakeIds[_user].length; i++) {
            ...
            if (!stake.unstaked) { // Add this condition
    ...
    // Function to calculate level rewards for a user
    ...
    // View function to get user's stake list
    function getUserStakes(address _user) external view returns (StakeInfo[] memory) {
        return stakes[_user];
    }

    function placeBet(uint256 _number, uint256 _betAmount) external {
        require(_number >= 1 && _number <= 100, "Invalid number");
        require(_betAmount > 0, "Bet amount must be greater than zero");
        require(rewardInfo[msg.sender].availableRewards >= _betAmount, "Insufficient rewards balance");

        rewardInfo[msg.sender].availableRewards = rewardInfo[msg.sender].availableRewards.sub(_betAmount);
        gameBets[msg.sender].push(GameBet(_number, _betAmount));
    }
    ...
    function playGame() external {
        ...
        for (uint256 i = 0; i < gameBets[msg.sender].length; i++) {
            totalBetAmount = totalBetAmount.add(gameBets[msg.sender][i].betAmount);
            if (gameBets[msg.sender][i].number == winningNumber) {
                winnersCount = winnersCount.add(1);
            }
        }

        uint256 winningAmount = totalBetAmount.div(10).mul(winnersCount);
        if (winningAmount > 0) {
            rewardInfo[msg.sender].availableRewards = rewardInfo[msg.sender].availableRewards.add(winningAmount);
        }

        // Reset user's game bets
        delete gameBets[msg.sender];
    }
