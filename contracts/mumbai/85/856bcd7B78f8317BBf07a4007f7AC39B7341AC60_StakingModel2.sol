...
contract StakingModel2 is StakingCalculations{
    ...
    function checkEarlyRewardSeparate(uint256 id) public view returns(uint256){
        address staker = msg.sender;
        ...
        uint256 calcStakes = stakerDetails[id].stakeAmount;

        // Total days staked and calculate early reward
        uint256 noOfDays = block.timestamp + (864000); //10 days
        noOfDays = getNumberOfDays(noOfDays.sub(stakerDetails[id].maturityTime));
        return calculateEarlyWithdrawReward(noOfDays, calcStakes, daysOfMaturity, earlyRewardAPY);
    }

    // @notice checkEarlyRewardCombined function calculates the amount of early reward tokens for combined model
    // @param id (uint256): id of staked tokens
    // @return Returns uint256 amount of rewards calculated
    function checkEarlyRewardCombined(uint256 id) public view returns(uint256){
        address staker = msg.sender;
    ...
    // @return Returns uint256 amount of rewards calculated
    function checkAfterMaturityCombinedReward() public view returns(uint256){
        address staker = msg.sender;
    ...
    // @notice checkAfterMaturitySeparateRewardAmount function calculates the amount of reward tokens after maturity for separate model
    // @param id (uint256): id of staked tokens
    // @return Returns uint256 amount of rewards calculated
    function checkAfterMaturitySeparateRewardAmount(uint256 id) public view returns(uint256){
