...
contract OriumFactory is Initializable, AccessControlUpgradeable {
    ...
    mapping(uint256 => mapping(uint256 => uint256)) public platformToEventIdToSharesLength;
    ...
    function addPlatform(
        ...
        for (uint256 i = 0; i < _sharesLength.length; i++) {
            platformToEventIdToSharesLength[_platform][i + 1] = _sharesLength[i];
        }
     ...
     * @return platform shares length
     */
    function getPlatformSharesLengthOfEventId(uint256 _platform, uint256 _eventId)
        external
        view
        returns (uint256)
    {
        return platformToEventIdToSharesLength[_platform][_eventId];
    }
