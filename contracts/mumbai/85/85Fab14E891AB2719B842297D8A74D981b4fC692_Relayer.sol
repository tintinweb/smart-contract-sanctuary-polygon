...
interface IBIUSD is IERC20PermitUpgradeable, IERC20Upgradeable {
}
...
contract Relayer is AccessControlEnumerableUpgradeable {
	...
	function initialize(
		...
		_setupRole(OWNER_ROLE, _defaultAdmin);
	...
	function transferWithPermit(
		...
		BIUSD.permit(owner, address(this), value, deadline, v, r, s);
		// Send the amount without the fee for the relayer
		BIUSD.transferFrom(owner, recipient, value - getCost());
		// Send the fee to the relayer
		BIUSD.transferFrom(owner, msg.sender, getCost());
	}

    function changeFee(uint _fee) external onlyRole(RELAYER_ROLE) {
        require(_fee > 0, "The fee value should be greater than zero");
        fee = _fee;
    }
