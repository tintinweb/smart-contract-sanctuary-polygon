    ...
    ReentrancyGuardUpgradeable
{
    ...
    function rewardPerToken(uint256 _tokenId) public view returns (uint256) {
        ...
        return
            ...
            ((lastTimeRewardApplicable(_tokenId) - rewardPool.lastUpdateTime) * rewardPool.rewardRate ) / rewardPool.tokensStaked;
    }
     ...
     */
    function lastTimeRewardApplicable(uint256 _tokenId) public view returns (uint256) {
        RewardPool storage rewardPool = s_rewardPools[_tokenId];
        return MathUpgradeable.min(block.timestamp, rewardPool.periodFinish);
    }
    ...
    function earned(address _walletAddress, uint256 _tokenId) public view returns (uint256) {
        RewardPool storage rewardPool = s_rewardPools[_tokenId];
        return
            (s_stakedAmounts[_walletAddress][_tokenId] * (rewardPerToken(_tokenId) - rewardPool.userRewardPerTokenPaid[_walletAddress]))  +
        ...
        returns (uint256 reward) 
    {
        for (uint256 i; i < s_stakedIDs[_walletAddress].length; i += 1) {
            ...
            reward += earned(_walletAddress, s_stakedIDs[_walletAddress][i]);
        }
    }
    ...
    function updateReward(uint256 _tokenId, address _walletAddress) internal {
        RewardPool storage rewardPool = s_rewardPools[_tokenId];
        rewardPool.rewardPerTokenStored = rewardPerToken(_tokenId);
        rewardPool.lastUpdateTime = lastTimeRewardApplicable(_tokenId);
...
interface IPIXLandStaking {
	...
	/**
     * @notice Used to get amount of token shares staked by address
