...
contract SwapContract is Ownable, ReentrancyGuard{
    ...
    */
    function cancelSwap(uint256 swapId) external {
        ...
        if (listIndex > 0) {
            ...
            IWithdraw(marketplace.getEscrow()).claimNFTback(sp.owner, sp.tokenContract, sp.tokenId);
    ...
    */
    function swapToken(uint256 tokenId,
        ...
        IERC721(tokenContract).transferFrom(tokenOwner, marketplace.getEscrow(), tokenId);
    ...
    function wishFulfill(uint256 swapId) external {
        ...
        uint256 length = wishlist[swapId].length;
        uint256 check = 0;

        for(uint i = 0; i < length; i++) {
            NFTtype memory nft = wishlist[swapId][i];
            require(nft.tokenContract != address(0), "Invalid Contract");
            require(nft.tokenId > 0 || nft.amount > 0, "Invalid");

            _wishFulfill(nft, swapOriginal.owner);
            check += 1;
        }
        if (check == length)
        IWithdraw(marketplace.getEscrow()).claimNFTback(msg.sender, swapOriginal.tokenContract, swapOriginal.tokenId);
    }

    function _wishFulfill(NFTtype memory nft, address owner) internal {

        if(nft.erc == ErcStandard.erc721) {
            IERC721 con = IERC721(nft.tokenContract);
            require(con.ownerOf(nft.tokenId) == msg.sender, "Invalid sender");
            con.safeTransferFrom(msg.sender, owner, nft.tokenId);
        } else if(nft.erc == ErcStandard.erc20) {

            IERC20 con = IERC20(nft.tokenContract);
            require(con.balanceOf(msg.sender) >= nft.amount, "Invalid sender");
            con.safeTransferFrom(msg.sender, owner, nft.amount);
        }
    }
    ...
    */
    ...
    function _swapping(Offers storage off, Swap storage swapOriginal, uint256 offerId, uint256 swapId) internal {
        ...
        if (off.exchangeValue > 0) {
            ...
            IWithdraw(marketplace.getEscrow()).transferCurrency{value: msg.value}(off.owner, uint256(off.exchangeValue), off.usdt, false);

            // updates the balance of the owner and treasury for them to claim later on
            if (off.usdt) {

                IWithdraw(marketplace.getEscrow()).updateIncomingToken(swapOriginal.owner, uint256(off.exchangeValue), true);

            } else {

                IWithdraw(marketplace.getEscrow()).updateIncomingToken(swapOriginal.owner, uint256(off.exchangeValue), false);
            }
        }
        ...
        IWithdraw(marketplace.getEscrow()).claimNFTback(off.owner, swapOriginal.tokenContract, swapOriginal.tokenId);
