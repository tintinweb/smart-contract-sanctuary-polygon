...
contract Game {
    ...
    function battle(string memory _name, int _strength, int _agility, int _wisdom) public {
        ...
        uint startAttackIndex = attacks.length-1;
        ...
        if (players[currentIndex].health <= 0 && spawns[currentIndex].health <= 0){
            addRound(currentIndex, attackCounter, startAttackIndex, startAttackIndex+attackCounter, false, 5);
        } else if (players[currentIndex].health > 0 && spawns[currentIndex].health <= 0){
            addRound(currentIndex, attackCounter, startAttackIndex,  startAttackIndex+attackCounter, true, 20);
        } else if (players[currentIndex].health <= 0 && spawns[currentIndex].health > 0){
            addRound(currentIndex, attackCounter, startAttackIndex, startAttackIndex+attackCounter, false, 0);
        }
    }
}