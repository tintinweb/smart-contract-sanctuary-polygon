...
/// @title Shoply Domain Registry
contract DomainRegistry is IDomainRegistry, ERC721, AccessControl {
    using SafeERC20 for IERC20;

    /// @notice The role for minting domains
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    address public feeAddress;
    address public feeToken;
    address public feeTokenOracle;
    uint8 public feeTokenDecimals;
    uint256 public domainPrice;

    /// @notice Mapping of registered domains to their owners
    mapping (bytes32 => address) public domainOwner;

    constructor(address _feeAddress, address _feeToken, address _feeTokenOracle, uint8 _feeTokenDecimals, uint256 _domainPrice) ERC721("Shoply Domains", "SHOP") {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(MINTER_ROLE, msg.sender);

        feeAddress = _feeAddress;
        feeToken = _feeToken;
        feeTokenOracle = _feeTokenOracle;
        feeTokenDecimals = _feeTokenDecimals;
        domainPrice = _domainPrice;
    }

    /// @notice Register a domain
    /// @dev Emits a `DomainRegistered` event
    /// @param _domain The domain to register
    function registerDomain(bytes32 _domain) external {
        // get price from feeTokenOracle
        AggregatorV3Interface oracle = AggregatorV3Interface(feeTokenOracle);
        (, int256 price, , , ) = oracle.latestRoundData();
        if (price <= 0) {
            revert DomainPriceOracleError();
        }
        // get the oracle decimals
        uint8 oracleDecimals = oracle.decimals();
        // get domain price in feeToken
        uint256 domainPriceInFeeToken = domainPrice * 10**feeTokenDecimals * 10**oracleDecimals / uint256(price);

        IERC20(feeToken).safeTransferFrom(msg.sender, feeAddress, domainPriceInFeeToken);
        _registerDomain(_domain, msg.sender);
    }

    /// @notice Mint a domain without requesting it
    /// @dev Emits a `DomainRegistered` event
    /// @param _domain The domain to mint
    /// @param _domainOwner The domain owner
    function adminMintDomain(bytes32 _domain, address _domainOwner) external onlyRole(MINTER_ROLE) {
        _registerDomain(_domain, _domainOwner);
    }

    function adminMintDomainBatch(bytes32[] calldata _domains, address[] calldata _domainOwners) external onlyRole(MINTER_ROLE) {
        if (_domains.length != _domainOwners.length) {
            revert InvalidArrays();
        }
        for (uint256 i = 0; i < _domains.length; i++) {
            _registerDomain(_domains[i], _domainOwners[i]);
        }
    }
    ...
    function setFeeToken(address _feeToken, uint8 _feeTokenDecimals) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (_feeToken == address(0)) {
            revert InvalidFeeToken();
        }
        feeToken = _feeToken;
        feeTokenDecimals = _feeTokenDecimals;
        emit FeeTokenChanged(_feeToken, _feeTokenDecimals);
    }

    function setFeeTokenOracle(address _feeTokenOracle) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (_feeTokenOracle == address(0)) {
            revert InvalidFeeTokenOracle();
        }
        feeTokenOracle = _feeTokenOracle;
        emit FeeTokenOracleChanged(_feeTokenOracle);
    }

    function setDomainPrice(uint256 _domainPrice) external onlyRole(DEFAULT_ADMIN_ROLE) {
        domainPrice = _domainPrice;
        emit DomainPriceChanged(_domainPrice);
    }

    /// @notice Check if this contract supports interface `interfaceId`
    /// @param interfaceId The interface to check
    /// @return True if this contract supports interface `interfaceId`
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721, AccessControl) returns (bool) {
        return super.supportsInterface(interfaceId);
    }

    function _registerDomain(bytes32 _domain, address _domainOwner) internal {
        if (_domain == bytes32(0)) {
            revert InvalidDomain();
        }
        if (domainOwner[_domain] != address(0)) {
            revert DomainAlreadyRegistered();
        }

        domainOwner[_domain] = _domainOwner;
        _mint(_domainOwner, uint256(_domain)); // QUESTION: Should the tokenID be the domain hash or sequential?
        emit DomainRegistered(_domain, _domainOwner);
    }

    /// @dev override _beforeTokenTransfer hook to create soulbound tokens
    function _beforeTokenTransfer(address _from, address, uint256, uint256) internal virtual override {
        if (_from != address(0)) {
            revert TransferNotAllowed();
        }
        // do not allow transfer of domains
        // NFTs are soul bound
    }
