...
contract VolcomSkateboardMinter is Ownable{
    ...
    function setDistributionRule(uint _maxDistribution) external onlyOwner {
        maxDistribution = _maxDistribution;
    }

    function setMaxDistribution(uint _eID, uint amount) external onlyOwner {
        distribution[_eID] = amount;
    }

    function getDistributionRule(uint _eID) public returns (uint){
