...
contract BorrowerOps is  OrumBase, Ownable, CheckContract, IBorrowerOps {
    ...
    function openVault(uint _maxFeePercentage, uint _USDCAmount, uint _colloMATICamount , address _upperHint, address _lowerHint) external payable override {
        ...
        require( oMaticToken.allowance(msg.sender, address(this)) >= _colloMATICamount  , "BorrowerOps: not allowed to spend oMATIC of user");
        ...
        _transferoMATICFromSender( oMaticToken , msg.sender, address(activePool), vars.netColl);
    ...
    function addColl(uint _collAddition, address _upperHint, address _lowerHint) external payable override  { 
        require( oMaticToken.allowance(msg.sender, address(this)) >= _collAddition  , "BorrowerOps: not allowed to spend oMATIC of user");
    ...
    */
        ...
        internal
    {
        ...
        if (_isCollIncrease) {
            _transferoMATICFromSender(oMaticToken , msg.sender,address(activePool), _collChange );
