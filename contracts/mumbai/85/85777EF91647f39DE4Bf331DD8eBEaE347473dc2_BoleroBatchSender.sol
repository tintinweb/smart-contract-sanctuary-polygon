...
contract BoleroBatchSender is ExternalStorage {
    ...
    */
    ...
    function multisendToken(address token,Transfer[] memory batchData) public payable {
            require(batchData.length <= arrayLimit());
            ERC20 erc20token = ERC20(token);
            uint256 total = 0;
            uint8 i = 0;
            for(i; i < batchData.length; i++){
                erc20token.transferFrom(msg.sender, batchData[i].reciever , batchData[i].balance);
                total += batchData[i].balance;
            }
            emit Multisended(total, msg.sender, batchData[i].reciever);
            setTxCount(msg.sender, txCount(msg.sender) + 1);
    }
    
    function multisendMultiToken(BatchTransfer[] memory batchData) public payable {
            require(batchData.length <= arrayLimit());
            uint8 i = 0;
            for (i; i < batchData.length; i++) {
               multisendToken(batchData[i].tokenAddress, batchData[i].operations);
            }
            setTxCount(msg.sender, txCount(msg.sender) + 1);
    }
