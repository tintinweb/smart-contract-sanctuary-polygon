...
contract T222 is Ownable, TimeForDay, SortList {
    ...
    */
    ...
    function invest(uint256 _amount) external {
        ...
        userInvestList[_user] = UserInvestItem(investIndex, _userRemainingCredit);
        ...
        if (userInfoList[_user]._firstInvestTime == 0) {
            ...
            if (_referrer != address(0)) {
                updateInviteRanking(_day, _referrer, 1, 20);
            }
        }
        //用户参与当日的大单排行榜
        updateInvestRanking(_day, _user, _amount, 10);
        ...
        //记录入金信息,包括用户地址、入金索引、日期、时间戳、入金数量、当天总入金、用户剩余授信额度
        emit InvestEvent(_user, investIndex, _day, block.timestamp, _amount, listPerDayForInvest[_day][_user], getUserRemainingCredit(_user));
    ...
    function transferReward(address _user, uint256 _pendingReward, string memory _type) private {
        ...
        // 计算用户剩余的授信额度
        uint256 _userRemainingCredit = getUserRemainingCredit(_user);
        emit ClaimRewardEvent(_user, investIndex - 1, getYearMonthDay(block.timestamp), block.timestamp, _type, _userRemainingCredit);
        // 更新用户的投资列表
