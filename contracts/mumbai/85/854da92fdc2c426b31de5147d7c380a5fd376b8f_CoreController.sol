...
contract CoreController is Initializable, UUPSUpgradeable, ICoreController {
	...
	function getVaults() external view override returns (address[] memory) {
		uint256 totalVaults = _vaultsList.length;
		uint256 totalDroppedVaults = 0;
		address[] memory vaultsList = new address[](totalVaults);

		for (uint256 i = 0; i < totalVaults; i++) {
			if (_vaultsList[i] != address(0)) {
				vaultsList[i - totalDroppedVaults] = _vaultsList[i];
			} else {
				totalDroppedVaults++;
			}
		}

		// Reduces the length of the vaults array by `totalDroppedVaults`
		assembly {
			mstore(vaultsList, sub(totalVaults, totalDroppedVaults))
		}
		return vaultsList;
	}
	...
	///@inheritdoc ICoreController
	function dropVault(address underlyingAsset)
		external
		override
		onlyConfigurator
	{
		CoreLogic.executeDropVault(underlyingAsset, _vaultsList, _vaults);
	}
...
library CoreLogic {
	...
	/// A Partner Has Liquidity
	error PartnerHasLiquidity();
	...
	function executeInitializeVault(
		...
		bool isFirstVault = vaultsList.length > 0 && vaultsList[0] == asset;

		if (vaultExists || isFirstVault) {
			revert VaultAlreadyInitialized();
		}

		vaultsList.push(asset);
		vaultData.id = uint8(vaultsList.length - 1);
	}

	/**
	 * @notice Implements the vault deletion feature of the Group.
	 * @dev Drops a vault
	 * @param vaults The vault datas
	 * @param asset The underlying asset
	 * @param vaultsList The active vaults
	 **/
	function executeDropVault(
		address asset,
		address[] storage vaultsList,
		mapping(address => VaultModel.Data) storage vaults
	) external {
		VaultModel.Data memory vaultData = vaults[asset];

		if (asset == address(0)) {
			revert InvalidAddress();
		}

		if (vaultData.id == 0 && vaultsList[0] != asset) revert UnlistedAsset();

		if (IERC20Upgradeable(vaultData.vaultToken).totalSupply() != 0)
			revert PartnerHasLiquidity();

		if (IERC20Upgradeable(vaultData.variableCreditToken).totalSupply() != 0)
			revert PartnerHasLiquidity();

		if (IERC20Upgradeable(vaultData.stableCreditToken).totalSupply() != 0)
			revert PartnerHasLiquidity();

		vaultsList[vaultData.id] = address(0);
		delete vaults[asset];
	}
