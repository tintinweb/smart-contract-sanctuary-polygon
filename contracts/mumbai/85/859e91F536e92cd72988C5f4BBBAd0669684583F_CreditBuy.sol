    ...
    OwnableUpgradeable
{
    ...
    function buyCredits(uint256 _planId) external {
        ...
        require(
            paymentToken.allowance(msg.sender, address(this)) >= plan.paymentTokensRequired,
        ...
        require(
            paymentToken.transferFrom(msg.sender, paymentWallet, plan.paymentTokensRequired),
     ...
     */
    function batchActivateCreditBuyPlans(uint256[] memory _planIds) external onlyAdminOrOwner {
     ...
     */
    function batchDeactivateCreditBuyPlans(uint256[] memory _planIds) external onlyAdminOrOwner {
        for(uint256 i=0; i<_planIds.length; i++) {
        ...
        emit DeactivatedCreditBuyPlans(
            msg.sender,
            _planIds,
            block.timestamp
        );
     ...
     */
    function addNewCreditBuyPlans(CreditBuyPlan[] memory _creditBuyPlans)
        public
        onlyAdminOrOwner
    {
     ...
     */
    function getAllCreditBuyPlans() external view returns(CreditBuyPlan[] memory) {
        uint256 totalPlans = creditBuyPlansAvailable;
