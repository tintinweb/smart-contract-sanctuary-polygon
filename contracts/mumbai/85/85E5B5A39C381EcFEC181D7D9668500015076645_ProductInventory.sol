...
contract ProductInventory is ERC721 {
    address admin;
    
    //Struct for add new product descriptions
    struct Product {
        uint256 code;
        string name;
        uint256 price;             
    ...
    //Mapping for new Product
    mapping(uint256 => Product) public products;
    
    //Mapping for user token amount
    mapping(address => uint256) public usersTokenAmount;
    
    constructor() ERC721("BlokFarm" , "BFA"){
        admin = msg.sender;
    }
    
    //Function for add new product by giving code, name, price and quantity in parameters (only admin can add product)
    function addProduct(uint256 _code, string memory _name, uint256 _price, uint256 _quantity) public  {
        require(msg.sender == admin, "Only Admin can add the product");
        
         Product memory _product = products[_code];
        _product.code = _code;
        _product.name = _name;
        _product.price = _price * 1 wei;
        _product.quantity = _quantity;
         products[_code] = _product;
    }
    
    //Function for removing product by giving code in parameters (only admin can remove the product)
    ...
    //Function for end userbuy the product by giving code and quantity of the product in parameters
    ...
    //Funtion for withdraw the amount (only admin or owner can call this function)
