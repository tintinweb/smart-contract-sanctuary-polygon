...
contract ERC721PsiMaxSupply is ERC721Psi, Ownable {
    ...
    /**
     * @notice Minting the requested amount would exceed the max supply allowed
     *
     * @param counterfactualNewSupply Total supply if requested amount minted
     */
    error MaximumSupplyExceeded(uint256 counterfactualNewSupply, uint256 maxSupply);
    ...
    /// @notice Alerts of minted tokens and their recipient
    ...
    /**
     * @notice Mints `quantity` of tokens if available
     *
     * @param to Address to receive minted tokens
     * @param quantity Number of tokens to mint
     */
    function mint(address to, uint256 quantity) public onlyOwner {
        if (0 == quantity) revert ZeroMintsRequested();

        uint256 newSupply  = totalSupply() + quantity;
        if (newSupply > maxSupply) revert MaximumSupplyExceeded(newSupply, maxSupply);
    ...
    /**
     * @notice Mints the max number of tokens available
     *
     * @param recipient Address to receive minted tokens
     */
    function mintMax(address recipient) external onlyOwner {
        mint(recipient, maxSupply - totalSupply());
    }
    ...
    /**
     * @notice Token URI for requested token if exists
     *
