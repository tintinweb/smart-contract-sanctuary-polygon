...
contract DeeDooTeacherTest04 is ERC721A, Ownable, ReentrancyGuard {
  ...
  /**
   * Storign the course name on chain for every token
   */
  mapping(uint256 => string) public mapCourseName;
  ...
  bool public teacherMintEnabled = true;
  ...
  function teacherMint(string memory _courseName) public payable mintCompliance(1) {
    require(teacherMintEnabled, 'The Teacher mint is not enabled!');
    
    /**
     * Storing the coursename
     */
    mapCourseName[totalSupply() + 1] = _courseName; 
    /**
  ...
  function getCourseName(uint256 _tokenId) public view virtual returns (string memory) {
    return mapCourseName[_tokenId];
  }
