    ...
    Ownable
{
    ...
    function _mint(
        address to,
        uint256 tokenId
    ) internal override virtual whenNotPaused(MintPauseTopicID) {
    ...
    function _beforeTokenTransfer(
        ...
        uint256 batchSize
    ) internal override(ERC721Enumerable) onlyAllowedOperator(from) {
...
/*
 ...
 |_.__/ \__, |                                            
    __  |___/            _                   _     _      
    \ \   / /__ _ __ ___(_)_   _  ___  _ __ | |__ (_)_ __ 
     \ \ / / _ \ '__/ __| | | | |/ _ \| '_ \| '_ \| | '__|
      \ V /  __/ |  \__ \ | |_| | (_) | | | | |_) | | |   
       \_/ \___|_|  |___/_|\__, |\___/|_| |_|_.__/|_|_|   
     _____    _            |___/       _ _                
    |_   _|__| | ___ __   ___ | | ___ (_|_)               
      ...
      |_|\___|_|\_\_| |_|\___/|_|\___// |_|               
                                    |__/                                
*/
contract UnfetteredWhitelist is UnfetteredBaseToken, PhasedSales {
    constructor(
        address accountOwner,
        string memory name,
        string memory symbol,
...
abstract contract MerkleVerifier {
    ...
    function verifyMerkle(
        ...
        if (!MerkleProof.verifyCalldata(_merkleProof, _merkleRootHash, _leaf))
    ...
    function isMerkleValid(
        ...
        return MerkleProof.verifyCalldata(_merkleProof, _merkleRootHash, _leaf);
    }
}
...
abstract contract CashCollector {
    ...
    function _addRemovePaymentToken(
        IERC20 paymentToken,
        bool remove
    ) internal {
...
abstract contract PhasedSales is MerkleVerifier {
    SalesPhase[] _salesPhases;
    mapping(bytes32 => mapping(address => uint256)) _usages;
    IERC20 _salesToken;
    ...
    */
    function _buy(
        ...
        bytes32 hash = keccak256(abi.encodePacked(phaseIndex));
        if (_usages[hash][msg.sender] >= amount) revert AlreadyExist();
    ...
    function _buy(
        ...
        if (price == 0 && salesPhase.merkleRootHash == bytes32(0x0)) revert InvalidValue();

        // check period
        TimeRangeLib.check(salesPhase.timeRange);

        if (_usages[salesPhase.merkleRootHash][msg.sender] >= amount)
            revert AlreadyExist();

        bytes32 leaf = keccak256(abi.encodePacked(msg.sender, amount));
        bool valid = isMerkleValid(leaf, merkleProof, salesPhase.merkleRootHash);
        if (!valid && phaseIndex > 0) {
            // check old phases
            for (uint i = phaseIndex - 1; i >= 0; i--) {
                salesPhase = _salesPhases[i];
                ...
                if (salesPhase.amountLimiting) {
                    uint256 usedAmount = _usages[salesPhase.merkleRootHash][msg.sender];
                ...
                if (amountOK) {
                    valid = isMerkleValid(leaf, merkleProof, salesPhase.merkleRootHash);
                    if (valid) break;
                }
            }
        }
    ...
    function _receivePayment(
        ...
        if (amountLimiting) {
            uint256 usedAmount = _usages[hash][msg.sender];
            uint256 remainingAmount = amount - usedAmount;
            if (requestedAmount > remainingAmount) revert InvalidAmount();
        }

        _usages[hash][msg.sender] += requestedAmount;
