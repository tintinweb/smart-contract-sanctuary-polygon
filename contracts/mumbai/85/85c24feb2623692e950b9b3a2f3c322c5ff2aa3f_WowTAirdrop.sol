  ...
  ERC721URIStorageUpgradeable
{
  ...
  /* constructor() ERC721("wowT Aidrop", "wowT") {} */
  ...
  function initialize(
    ...
    __Ownable_init();
    baseAmount = 50000000000000000; //0.05 matic
    nftPoints = _nftPoints;
  ...
  function mintTo(
    ...
    require(points.getPoints(to) > nftPoints, 'Insufficient points');
    string memory userName = community.getUserName(_communityName, to);
    ...
    rentables[id] = RentableItem({ rentable: false, amountPerDay: baseAmount });
    tokenToCommunity[id] = _communityName;
    points.reducePoints(to, nftPoints, 'mint');
  }
  ...
  function UpVoteNFT(string calldata _communityName, uint256 _tokenId) public {
    ...
    uint256 upVotePoints = points.getPointValue(5);
    points.addPoints(_msgSender(), upVotePoints, 'upVote');
  ...
  function downVoteNFT(
    ...
    nft.downVotes++;
    nft.preference[_msgSender()] = true;
    uint256 downVotePoints = points.getPointValue(6);
    points.addPoints(_msgSender(), downVotePoints, 'downVote');
  ...
  function commentNFT(string calldata _communityName, uint256 _tokenId) public {
    ...
    NFT storage nft = communityDetail.nfts[_tokenId];
    nft.comments++;
    uint256 commentPoints = points.getPointValue(7);
    points.addPoints(_msgSender(), commentPoints, 'comment');
  ...
  function setNftPoints(uint256 _nftPoints) external onlyOwner {
    nftPoints = _nftPoints;
  }
        ...
        if (isNewLeader) {
            ...
            if (topLeaderBoard.length > topLeaderboardSize) {
                removeLeaderBoardEntry(
                    topLeaderBoard[topLeaderBoard.length - 1]
                );
            }
        }
    }
