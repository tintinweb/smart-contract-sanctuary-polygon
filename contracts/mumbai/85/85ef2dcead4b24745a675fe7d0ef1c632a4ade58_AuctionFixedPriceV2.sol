...
contract AuctionFixedPriceV2 is Initializable {
    ...
    function buyFixPrice(
        ...
        if (listingInstance.isAuction) revert NFTAreOnAuction();
    ...
    function makeOfferFixedPrice(
        ...
        if (
            stanFund.get(_paramsOffer.maker) < _paramsOffer.amount &&
            !_isWhiteList
        ) revert InvalidBalance();
        if (_paramsOffer.expiTime <= block.timestamp) revert InvalidTimestamp();
    ...
    function acceptOfferPvP(
        ...
        if (!_isWhiteList && _sender != inforInstance.owner)
            revert InvalidOwner();
        if (inforInstance.state != AuctionStructure.StateOfOffer.ACTIVE)
            revert InvalidState();
        if (block.timestamp >= inforInstance.expirationTime)
    ...
    function transferNFTPvP(
        ...
        if (stanFund.get(_maker) < feeSystem.stanFee && !_isWhiteList)
