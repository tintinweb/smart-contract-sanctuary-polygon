...
contract Token is Context, IERC20, Ownable{
    ...
    uint256 private constant _decimals = 7;
   ...
   function decimals () public view returns (uint256){
    ...
    function _transfer (address from, address to, uint256 amount) internal {
        ...
        require(amount >= balances[from], "TTS: you do not have enough tokens");
    ...
    function approve (address spender, uint256 amount) external override returns (bool) {
        require(_msgSender() != address(0), "TTS: contracts addreses should not approve");
        require(spender != address(0), "TTS: token should not be sent to contracts");
        require(amount >= balances[_msgSender()], "TTS: you do not have enough tokens");
        allowances[_msgSender()][spender].add(amount);
    ...
    function transferFrom (address from, address to, uint256 amount) external override returns (bool){
        ...
        allowances[from][spender].sub(amount);
