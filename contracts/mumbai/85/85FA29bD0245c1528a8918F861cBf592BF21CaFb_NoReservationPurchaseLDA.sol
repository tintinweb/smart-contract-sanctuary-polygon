...
contract NoReservationPurchaseLDA is RoyalPausableUpgradeable {
    ...
    /// @dev (tierID => max tokenID to sell) Allows for maxSupply of tier to be different
    ///       from the number of tokens that we sell. maxSupply - maxTokenIDForSale = N. This
    ///       means last N tokens of tier will be held out and not sold.
    ///       DEFAULT VALUE: set to maxSupply; can only be changed w/ registration update
    mapping(uint128 => uint256) public maxTokenIDForSale;
    ...
    function registerTierForPurchase(
        ...
        uint256 curTierMaxSupply = Royal1155LDA(ldaContract).tierMaxSupply(
            tierID
        );
        maxSupplyForTier[tierID] = curTierMaxSupply;

        // Default the tokens for sale to max supply. Can be updated later.
        maxTokenIDForSale[tierID] = curTierMaxSupply;
    }
    ...
    function updateTierPurchaseRegistration(
        ...
        address ldaContract,
        address ldaHolderForTier,
        uint256 numMaxTokenIDForSale
    ) external onlyOwner whenNotPaused {
        require(
            numMaxTokenIDForSale <= maxSupplyForTier[tierID],
            "Can't sell more than maxSupply"
        );
        ...
        tierLDAContract[tierID] = ldaContract;
        tierLDAHolder[tierID] = ldaHolderForTier;
        maxTokenIDForSale[tierID] = numMaxTokenIDForSale;
    }
    ...
    function buy(
        ...
        if (nextTokenToSellForTier[tierID] > maxTokenIDForSale[tierID]) {
            emit TierSoldOut(
                tierID,
                ldaContractAddress,
