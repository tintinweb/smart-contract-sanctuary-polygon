...
contract Calamus is Initializable, ICalamus, IPlatformFee, ReentrancyGuardUpgradeable, CarefulMath, PausableUpgradeable {
    ...
    EnumerableMapUpgradeable.AddressToUintMap userFees;
    EnumerableMapUpgradeable.AddressToUintMap tokenFees;
    ...
    function _feeOf(address userAddress, address tokenAddress) private view returns (uint256 fee) {
        FeeVars memory vars;
        (vars.exists, vars.value) = userFees.tryGet(userAddress);
        if (vars.exists) {
            return vars.value;
        }
        (vars.exists, vars.value) = tokenFees.tryGet(tokenAddress);
        if (vars.exists) {
            return vars.value;
        }
    ...
    function addWhitelistToken(address tokenAddress, uint256 fee) public onlyOwner {
        tokenFees.set(tokenAddress, fee);
    }

    function addWhitelistUser(address userAddress, uint256 fee) public onlyOwner {
        userFees.set(userAddress, fee);
    }

    function removeWhitelisToken(address tokenAddress) public onlyOwner {
        tokenFees.remove(tokenAddress);
    }

    function removeWhitelisUser(address userAddress) public onlyOwner {
        userFees.remove(userAddress);
    }
}
