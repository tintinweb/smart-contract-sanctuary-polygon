...
//    uint256 public cancellationFeeBuk = 2000;// Cancellation fee when bookings are cancelled, received by the platform treasury
    ...
    struct Booking{
        ...
        uint256 discount; //discount on platformFee
        uint256 hotelId; // hotel Id of the room booked
        uint256 buk_cancellationFee; //cancellation fee of the bukprotocol
        uint256 Hotel_cancellationfee;//cancellationFee of the Hotel in which room was booked
...
/*
    // Function to update hotel taxes
    function updateHotelTaxes(uint256 tax) external onlyOwner{
       hotelTaxes = tax;
       emit HotelTaxesUpdated(tax); 
    }
*/
    ...
    function cancelBookingUser(uint256 id, address user, string memory  nonce, bytes memory signature) external {
        ...
        if( owner() == msg.sender || msg.sender == idToHotel[bookingDetails[id].hotelId].hotelManager){
            ...
            if(block.timestamp < bookingDetails[id].time - checkInTime){
             uint256 feeHotel =  ActualAmount*bookingDetails[id].Hotel_cancellationfee/10000;
            uint256 feeBuk = ActualAmount*bookingDetails[id].buk_cancellationFee/10000;    
