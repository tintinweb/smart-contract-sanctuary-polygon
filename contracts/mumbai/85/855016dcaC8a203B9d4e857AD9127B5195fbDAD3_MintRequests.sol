...
contract MintRequests is Initializable, ReentrancyGuardUpgradeable, PausableUpgradeable, EIP712Upgradeable {
    ...
    function getReqDataRoyaltyRecipient(uint256 _tokenId)
        external
        view
        returns (address payable[10] memory recipients)
    {
        return requestsData[_tokenId].royaltyRecipient;
        // uint _trueLength = 0;
        // for (uint i=0; i <= requestsData[_tokenId].royaltyBps.length - 1; i++) {
        //     if (requestsData[_tokenId].royaltyBps[i] == 0) {
        //         _trueLength = i + 1;
        //     }
        // }
        // recipients = new address payable[](_trueLength);
        // for (uint i=0; i <= _trueLength; i++) {
    ...
    function getReqDataRoyaltyBps(uint256 _tokenId) external view returns (uint256[10] memory basisPoints) {
        return requestsData[_tokenId].royaltyBps;
        // uint _trueLength = 0;
        // for (uint i=0; i <= requestsData[_tokenId].royaltyBps.length - 1; i++) {
        //     if (requestsData[_tokenId].royaltyBps[i] == 0) {
        //         _trueLength = i + 1;
    ...
    */
        ...
        returns (uint256 requestID)
    {
        ...
        for (uint256 i = 0; i < 10; i++) {
            _requestData.royaltyBps[i] = voucher.royaltyBps[i];
            _requestData.royaltyRecipient[i] = voucher.royaltyRecipient[i];
        }
