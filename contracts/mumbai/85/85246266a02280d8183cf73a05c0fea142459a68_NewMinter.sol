...
contract NewMinter is IMinter, Ownable {
    ...
    event ERC20TokenChanged(address newToken);
    event ERC20MintPriceChanged(uint256 newPrice);
    ...
    constructor(address nft_, uint16 devReserve_, IERC20 erc20Token_) {
        _panda = nft_;
        _signer = msg.sender;
        require(
        ...
        erc20MintPrice = 1000;  // Set initial ERC20 mint price, can be changed later
    }
    ...
    // /**
    //  * publicMint with ERC20
    //  */

        //  function to change ERC20 token
    function setERC20Token(IERC20 newERC20Token) external onlyOwner {
        erc20Token = newERC20Token;
        emit ERC20TokenChanged(address(newERC20Token));
    }

    //  function to set ERC20 mint price
    function setERC20MintPrice(uint256 newPrice) external onlyOwner {
        erc20MintPrice = newPrice;
        emit ERC20MintPriceChanged(newPrice);
    }

    function publicMintWithERC20(uint16 quantity, address to) external {
        require(publicMintActive, "Public mint is not active");
        uint256 totalRequired = erc20MintPrice * quantity;
        require(erc20Token.balanceOf(msg.sender) >= totalRequired, "Insufficient ERC20 token balance");
        
        // Transfer the ERC20 tokens from the sender to this contract
        erc20Token.transferFrom(msg.sender, address(this), totalRequired);
