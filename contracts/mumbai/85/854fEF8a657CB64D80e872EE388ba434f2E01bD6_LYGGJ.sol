...
/**
 * @title LYGGJ
 * @dev The LYGGJ is non-transferable token.
 * It is possible to gradually convert from since to until period to YGGJ.
 */
contract LYGGJ is ERC20 {
    /**********
     * Struct *
     **********/
    ...
    constructor() ERC20("Locked YGGJ", "LYGGJ") {}
    ...
    /**
     * Mint the LYGGJ by depositing the YGGJ.
     * @param to Destination address for the LYGGJ.
     * @param since Unixtime to start converting the LYGGJ to the YGGJ.
     * @param until Unixtime when all the LYGGJ can be converted to the YGGJ
     */
    function mint(
        address to,
        uint64 since,
        ...
        if (since <= block.timestamp || since >= until)
    ...
    /**
     * Convert the LYGGJ to the YGGJ.
     * @param amount Amount of the LYGGJ.
     */
    function claim(uint256 amount) external {
        ...
        uint256 currentClaimableYGGJ = getClaimableYGGJ(
            originalClaimer[msg.sender]
        ) - originalClaimInfo.claimed;
        if (amount > currentClaimableYGGJ) revert OverAmount();
    ...
    /**
     * Return the LYGGJ as the YGGJ to the address that minted it.
     * @param amount Amount of the LYGGJ.
     */
        ...
        returns (bool)
    {
        require(
            ...
            "LYGGJ: bulk transfer args must be equals"
        );
        address owner = _msgSender();
    ...
    function transferFrom(
        ...
        require(
            ...
            "LYGGJ: bulk transferFrom args must be equals"
        );
    ...
    /**
     * Get current amount of the LYGGJ available for conversion.
     * @param original Holder of the LYGGJ token.
     */
    function getClaimableYGGJ(address original) public view returns (uint256) {
        ...
        uint256 amount = (originalClaimInfo.amount *
            (block.timestamp - originalClaimInfo.since)) /
    ...
    /**
     * The LYGGJ is transferable to allowed addresses.
     */
     ...
     */
    function _contains(address[] memory list, address item)
        internal
        pure
        returns (bool)
    {
