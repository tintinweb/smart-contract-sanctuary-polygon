...
interface IBorrowerOps {
    ...
    event Amatic_TokenAddressChanged(address _amaticTokenAddress);
    ...
    // --- Functions ---
    function openVault(uint _maxFee, uint _debtAmount, uint _collAMATICamount, address _upperHint, address _lowerHint) external payable;
    // function addColl(address _upperHint, address _lowerHint) external payable;
    function addColl(uint _collAddition, address _upperHint, address _lowerHint) external payable;
    function moveROSEGainToVault(address _user,uint depositorROSEGain, address _upperHint, address _lowerHint) external payable;
...
interface ICollSurplusPool {
    ...
    event CollSurplusROSEBalanceUpdated(uint _ROSE);
    ...
    function receive_amatic_CollSurplusPool(uint new_coll) external;
}
...
contract BorrowerOps is OrumBase, Ownable, CheckContract, IBorrowerOps {
    ...
    function openVault(uint _maxFeePercentage, uint _OSDAmount, uint _collAMATICamount , address _upperHint, address _lowerHint) external payable override {
        require(_collAMATICamount <= amatic_Token.balanceOf(msg.sender) , "borrower dont have enough amatic");
        require( amatic_Token.allowance(msg.sender, address(this)) >= _collAMATICamount  , "BorrowerOps: not allowed to spend amatic of user");
        ...
        vars.netColl = _collAMATICamount;
        ...
        if (!isRecoveryMode) {
            ...
            _transferAmaticFromSender( amatic_Token , msg.sender, orumFeeDistributionAddress, vars.ROSEFee);
        ...
        // sends aMATIC to active pool
        _transferAmaticFromSender( amatic_Token , msg.sender, address(activePool), vars.netColl);
        activePool.receive_amatic(vars.netColl);
    ...
    function addColl(uint _collAddition, address _upperHint, address _lowerHint) external payable override  { 
        require( amatic_Token.allowance(msg.sender, address(this)) >= _collAddition  , "BorrowerOps: not allowed to spend amatic of user");
        _adjustVault(msg.sender, _collAddition, true, 0, false, _upperHint, _lowerHint, 0);
    }

    // sends amatic to active pool
    function _transferAmaticFromSender(IaMATICToken _amatic_Token, address _from, address _to, uint _amount) internal {
        bool success = _amatic_Token.transferFrom(_from, _to, _amount);
        require(success, "BorrowerOps: error in transfering amatic to active pool or orum_revenue");
    ...
    function moveROSEGainToVault(address _borrower, uint depositorROSEGain, address _upperHint, address _lowerHint) external payable override {
    ...
    function withdrawOSD(uint _maxFeePercentage, uint _OSDAmount, address _upperHint, address _lowerHint) external override {
        ...
        _adjustVault(msg.sender, 0, false, _OSDAmount, true, _upperHint, _lowerHint, _maxFeePercentage);
    }
    ...
    function repayOSD(uint _OSDAmount, address _upperHint, address _lowerHint) external override {
        ...
        _adjustVault(msg.sender, 0, false, _OSDAmount, false, _upperHint, _lowerHint, 0);
    }
    ...
    */
    function _adjustVault(address _borrower, uint _collChange, bool _isCollIncrease, uint _OSDChange, bool _isDebtIncrease, address _upperHint, address _lowerHint, uint _maxFeePercentage) internal {
        ContractsCache memory contractsCache = ContractsCache(vaultManager, activePool, osdToken, amatic_Token);
        ...
        // Check if he has enough smatic balance to add collateral
        // if(_isCollIncrease && contractsCache.amatic_Token.balanceOf(msg.sender) >= _collChange){
        //     _transferAmaticFromSender( amatic_Token , msg.sender, address(activePool), _collChange);
        // }
        
        // _requireSingularCollChange(_collChange,_isCollIncrease);    // TODO no need to call this
        _requireNonZeroAdjustment(_isCollIncrease, _collChange, _OSDChange);
        ...
        assert(msg.sender == _borrower || (msg.sender == stabilityPoolAddress && _collChange > 0 && _OSDChange == 0));
        ...
        if (_isDebtIncrease && !isRecoveryMode) { 
            vars.ROSEFee = _triggerBorrowingFee(contractsCache.vaultManager, _OSDChange, _maxFeePercentage, vars.price); // TODO check while withdraw/add Debt OSD
        }
        ...
        vars.newICR = _getNewICRFromVaultChange(vars.coll, vars.debt, vars.collChange, vars.isCollIncrease, vars.netDebtChange, _isDebtIncrease, vars.ROSEFee, vars.price); // TODO check while withdraw/add Debt OSD
        ...
        internal
    {
        if (_isDebtIncrease) {
            ...
            _activePool.sendROSE(_amaticToken, orumFeeDistributionAddress, _borrowFee);
        ...
        if (_isCollIncrease) {
            _transferAmaticFromSender(amatic_Token , msg.sender,address(activePool), _collChange );
            _activePool.receive_amatic(_collChange);
            // _activePoolAddColl(_activePool, _collChange);
        } else {
            _activePool.sendROSE(_amaticToken, _borrower, _collChange); 
        }
    }
    ...
    function _requireSingularCollChange(uint _collWithdrawal, bool _isCollIncrease) internal view {
        require(_isCollIncrease == true || _collWithdrawal == 0, "BorrowerOps: Cannot withdraw and add coll"); // TODO TOOD check again
    }
    ...
    function _requireNonZeroAdjustment(bool _isCollIncrease, uint _collWithdrawal, uint _OSDChange) internal view {
        require( _isCollIncrease == false || _collWithdrawal != 0 || _OSDChange != 0, "BorrowerOps: There must be either a collateral change or a debt change");
    }
        ...
        view 
    {
        ...
        */
        if (_isRecoveryMode) {
            ...
            _vars.newTCR = _getNewTCRFromVaultChange(_vars.collChange, _vars.isCollIncrease, _vars.netDebtChange, _isDebtIncrease, _vars.ROSEFee, _vars.price); // TODO check for borrowing fees
