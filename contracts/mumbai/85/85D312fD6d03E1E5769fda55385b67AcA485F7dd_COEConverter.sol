...
contract COEConverter {
  ...
  function convertCards(
    ...
    uint256[] memory _prices // (ex. 45 = .045, 500 = .5, 1000 = 1, 10000 = 10)
    ...
    //convert prices to aeg assuming aeg is .045 cents per token and has 18 decimals
    totalPrice = (totalPrice * 10 ** 18 * 1000) / 45; // THOUSANDTHS :: we multiply by 1000 to get the decimals right and divide by 45 to get the price in aeg at .045 cents per token

    //tranfer AEG/wAEG from user to this contrac
    require(
      AEGInterface(aegToken).balanceOf(_userAddress) >= totalPrice,
    ...
    for (uint256 i = 0; i < _ids.length; i++) {
      _convertCard(_assetAddress, _userAddress, _ids[i], _amounts[i]);
    }

    emit ConvertedCards(_assetAddress, _userAddress, _ids, _amounts);
  }

  function _convertCard(
    address _assetAddress,
    address _userAddress,
    uint256 _type,
    uint256 _amount
    ...
    NftInterface(_assetAddress).adminMint(_userAddress, _type, 1, _amount);
  }
