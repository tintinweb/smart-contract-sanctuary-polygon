...
contract SquidBets is SquidBetsCore, ReentrancyGuard {
    ...
    ///@param _eventID is the eventID, _result is the decision
    ///@param _result is the decision

    function placeBet(uint256 _eventID, RoundResult _result) external payable {
        require(
            msg.value == eventData[_eventID].roundBetAmount,
    ...
    function reportResult(
        ...
        payable(prizePool).transfer(poolAmount);
    }

    function transferRegisterFeeToTreasuryAndPrizePool(
        uint256 _eventID
    ...
    function withdrawAllFundWhenEmergency() public override onlyAdmin {
        payable(ultibetsTreasury).transfer(address(this).balance);
    }
}
...
contract SquidBetsCore is ISBCNFTType, CustomAdmin {
    ...
    address public ultibetsTreasury; /// address of the UltibetsTreasury contract
    ...
    event UpdateTotalRound(uint256 eventID, uint8 totalRound);
    event WinnerClaimedPrize(
        uint256 eventID,
        EventState state,
        uint16 claimedNumber
    );
    ...
    function updateEventTotalRound(
        ...
        emit UpdateTotalRound(_eventID, _totalRound);
    }
    ...
    function addRound(
        ...
        uint16 playersOnRound = uint16(
            playersByRound[_eventID][newLevel].length()
        );

        if (currentLevel == 0) {
            transferRegisterFeeToTreasuryAndPrizePool(_eventID);
        }

        emit RoundAdded(_eventID, newLevel, playersOnRound, _desc, _startTime);
    }
    ...
    function reActiveCanceledRound(
        ...
        emit RoundUpdated(_eventID, currentLevel, _desc, _startTime);
    }
    ...
    function getWinnersNumber(
        ...
        require(eventData[_eventID].eventID > 0, "Not created event!");
    ...
    function resultVote(uint256 _eventID) external onlyAdmin {
        ...
        emit ReportVoteResult(
            _eventID,
            eventData[_eventID].state,
            prizeOfEvent[_eventID],
            eventData[_eventID].vResult,
    ...
    function withdrawAllFundWhenEmergency() public virtual {}
