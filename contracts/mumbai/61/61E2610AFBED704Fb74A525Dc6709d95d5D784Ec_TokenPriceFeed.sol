...
contract TokenPriceFeed is ChainlinkClient, Ownable {
    ...
    string private tokenFeedEndpoint; 
    ...
    event Admin_Owner_Updated(address indexed newAdminOwnerAddress);
    event Oracle_Updated(address newOracleAddress);
    event Endpoint_Set(string newEndpoint);
    event JobId_Set(bytes32 newJobId);
    event Fee_Set(uint256 newFee);
    ...
    constructor(address adminAddress) {
        ...
        tokenFeedEndpoint = "https://data.messari.io/api/v1/assets/shib/metrics";
    ...
    {        
        if(!currentRound.requesting){
            ...
            Chainlink.add(request, "get", tokenFeedEndpoint);
    ...
    function updateAdminAddress(address newAdminAddress) external onlyOwner {
        admin = newAdminAddress;

        emit Admin_Owner_Updated(admin);
    }
    
    function setSHIBAFeedEndpoint(string memory newEndpoint) external onlyOwner {
        tokenFeedEndpoint = newEndpoint;

        emit Endpoint_Set(tokenFeedEndpoint);
    }
    
    function setOracle(address newOracleAddress) external onlyOwner {
        oracle = newOracleAddress;

        emit Oracle_Updated(oracle);
    }
    
    function setJobId(bytes32 newJobId) external onlyOwner {
        jobId = newJobId;

        emit JobId_Set(jobId);
    }
