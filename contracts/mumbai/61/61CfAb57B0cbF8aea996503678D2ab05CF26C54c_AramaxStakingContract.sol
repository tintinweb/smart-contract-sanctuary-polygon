...
contract AramaxStakingContract {
    ...
    uint256 public rewardPercentage = 24;
    ...
    function getRewards(
        ...
        uint256 rewardAmount = (users[account].amount * rewardPercentage * timeDiff / 365 days) / 100;
        return rewardAmount;
    }
    ...
    function withdraw(
        ...
        require(amount <= users[msg.sender].amount, "amount should be lesser than stakedAmount");
        uint256 rewardAmount = getRewards(msg.sender);
        uint256 totalAmount = rewardAmount + amount;
        token.transfer(msg.sender, totalAmount);
        uint256 rAmount = rewardAmount + users[msg.sender].rewardAmount;
        uint256 wAmount = totalAmount + users[msg.sender].withdrawAmount;
        bool _status = (amount > 0) ?true :false;
        users[msg.sender] = UserDetail( 
        ...
        emit Withdraw(msg.sender, totalAmount, rewardAmount);
        return true;
    }
