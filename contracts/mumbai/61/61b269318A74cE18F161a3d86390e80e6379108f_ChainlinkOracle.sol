...
contract ChainlinkOracle is PriceOracleI, ChainlinkClient, AccessControlEnumerable {
   ...
   */
  function requestFloor(string calldata _collection, string calldata _pricingAsset) external override onlyRole(REQUESTOR_ROLE) {
  ...
  function getUnderlyingPriceInUSD(IERC20 _erc20, uint256 _decimals) public view override returns (uint256) {
    ...
    if (oracleDecimals < _decimals) _decimals -= oracleDecimals;
    (
      ,
      /*uint80 roundID*/
