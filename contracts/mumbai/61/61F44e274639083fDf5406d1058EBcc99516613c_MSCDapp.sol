...
contract MSCDapp is Contractable {
    using SafeMath for uint256; 
    IERC20 public Dai;
    uint256 private constant baseDivider = 10000;

    uint256 private constant feePercents1 = 500;
    uint256 private constant feePercents2 = 2000;
    ...
    uint256 private constant uct = 90 minutes;
    ...
    struct UserInfo {
        ...
        bool isactive;
        bool isWithdrawalStop;
    }

    mapping(address=>UserInfo) public userInfo;
    mapping(address=>uint256) public prevCurSplit;
    ...
    constructor(address _usdtAddr, address _defaultRefer, address _admin1, address _admin2, address _contractAddress)   {
        Dai = IERC20(_usdtAddr);      
        feeReceivers[0] = address(_admin1);
        feeReceivers[1] = address(_admin2); 
    ...
    function deposit(uint256 _amount) external {
        ...
        uint256 amt = getCurSplit(msg.sender);    
        _deposit(msg.sender, _amount.add(amt));
        prevCurSplit[msg.sender] = prevCurSplit[msg.sender].add(amt);
    ...
    function _deposit(address _user, uint256 _amount) private {
        ...
        _unfreezeFundAndUpdateReward(msg.sender, _amount, dayRewardPercents);
    ...
    function _distributeDeposit(uint256 _amount) private {
        Dai.transfer(feeReceivers[0], _amount.mul(feePercents1).div(baseDivider));
        Dai.transfer(feeReceivers[1], _amount.mul(feePercents2).div(baseDivider));
    ...
    function depositByGrowth(uint256 _amount) external {
        ...
        require(splitLeft >= userInfo[msg.sender].totalDeposit, "Insufficient amt");
    ...
    function withdrawPrincipalAmount() external {
        require(!userInfo[msg.sender].isWithdrawalStop,"You can't withdrawal");
        distributePoolRewards();
    ...
    function withdrawIncome() external {
        ...
        uint256 withdrawable = (dynamicReward);
        uint256 splitAmt = (dynamicSplit);
    ...
    function getCurSplit(address _user) public view returns(uint256){
        ...
        return rewardInfo[_user].split.add(staticSplit).add(dynamicSplit).sub(rewardInfo[_user].splitDebt).sub(prevCurSplit[_user]);
    }
    ...
    function _unfreezeFundAndUpdateReward(address _user, uint256 _amount, uint256 _reward) private {
        UserInfo storage user = userInfo[_user];
        ...
        for(uint256 i = 0; i < orderInfos[_user].length; i++){
            ...
            if(block.timestamp > order.unfreeze  && order.isUnfreezed == false && _amount >= order.amount){
                ...
                uint256 staticReward = order.amount.mul(_reward).mul(dayPerCycle).div(timeStep).div(baseDivider);
                if(isFreezeReward){
    ...
    function transferOwnerShip(address _owner) public onlyContract{
        _contract = _owner;
    }
