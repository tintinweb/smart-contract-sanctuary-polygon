    ...
    UUPSUpgradeable
{
    ...
    function openChests() external {
        ...
        for (uint256 index = 0; index < blessingEvent.amount; index++) {
            uint256 randomNumber = _getRandomNumber(randomSeed, index, 100);
            uint8 chestDrop = _rollRarity(randomNumber, chestDropProbabilities);
            _mintChestDrop(chestDrop, blessingEvent.id, randomSeed, index);
        }
    }

    function previewChests() external returns (uint8[] memory) {
        BlessingEvent memory blessingEvent = _blessingEvents[msg.sender];
        uint256 randomSeed = IRandomOracle(randomOracleAddress).getRandomNumber(blessingEvent.requestId);
        require(blessingEvent.revealBlock < block.number, "not blessed");
        require(randomSeed > 0, "random seed not ready");
        _burn(msg.sender, blessingEvent.id, blessingEvent.amount);
        uint8[] memory results = new uint8[](blessingEvent.amount);
        for (uint256 index = 0; index < blessingEvent.amount; index++) {
            uint256 randomNumber = _getRandomNumber(randomSeed, index, 100);
            uint8 chestDrop = _rollRarity(randomNumber, chestDropProbabilities);
            results[index] = chestDrop;
        }
        return results;
    }

    function _mintChestDrop(
        uint8 chestDrop,
        uint256 id,
        uint256 randomSeed,
        uint256 nonce
    ...
    function _mintGear(
        ...
        IRagnarokItem(itemsAddress).mintRandom(msg.sender, rarity, randomSeed);
    }
...
interface IRagnarokItem {
    ...
    // function getItemStats(uint256 itemId) external view returns (SharedLib.Stats memory);

    // function getItemType(uint256 itemId) external view returns (SharedLib.ItemType memory);
}
