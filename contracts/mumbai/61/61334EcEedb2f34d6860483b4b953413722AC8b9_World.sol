...
contract World is IWorld, Ownable {
    ...
    mapping(int256 => mapping(int256 => mapping(address => uint256)))
        public elementCountInCellByAddress;

    mapping(int256 => mapping(int256 => address)) public elementRightsHolder;
    ...
    function placeElement(
        ...
        require(
            lastPlacedTime[_msgSender()] + timeCondition >= block.timestamp,
        ...
        emit ElementPlaced(elementID, x, y, value, rightsHolder);
        ...
        (int256 x_CU, int256 y_CU) = CellMath.belongsToCell(
            x,
            y,
            CellMath.cellDiameter(worldRadius)
        );

        // old ownerovi zmensim pocet owned elemetov . v cell
        if (elementRightsHolder[x][y] != address(0)) {
            // niekto tam uz je
            elementCountInCellByAddress[x_CU][y_CU][
                elementRightsHolder[x][y]
            ]--;
        }

        // novi owner zvysim pocet elementov v cell
        elementCountInCellByAddress[x_CU][y_CU][rightsHolder]++;

        // poznacim do latet owners
        elementRightsHolder[x][y] = rightsHolder;
    }
...
library CellMath {
    ...
    function belongsToCell(
        ...
        int256 d = int256(diameter);
        int256 resultX = closestNumber(x, d) / d;
        int256 resultY = closestNumber(y, d) / d;
        return (resultX, resultY);
    }

    function cellDiameter(uint256 cellRadius) public pure returns (uint256) {
        return cellRadius * 2 + 1;
    }
...
interface IWorld {
    ...
    function elementCountInCellByAddress(
        int256 cellX,
        int256 cellY,
        address rightsHolder
