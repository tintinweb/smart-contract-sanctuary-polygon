...
contract Arenas is Initializable, OwnableUpgradeable {
    ...
    function getArena( uint256 arena_index ) public view returns ( Arena memory ) {
        return _arenas[ arena_index ];
    }
    ...
    function genesysArena( address to, uint256 arena_id ) public virtual {
        ...
        uint256 arenaIndex = _arenasCounter.current();
        _arenasCounter.increment();

        _arenas[ arenaIndex ] = Arena ({
            _id: arena_id,
...
contract Decks is Initializable, OwnableUpgradeable {
    ...
    function getOwnedHelmetsIds( address owner ) public view returns ( uint256[] memory ) {
        NFT_1155 nft_1155 = _Mechas._Cores()._Industries()._Validators()._NFT_1155();

        uint256 idx = 0;
        uint256[] memory helmetIDs = _helmetsOwned[ owner ];
        uint256[] memory helmet_ids = new uint256[]( helmetIDs.length );

        for ( uint256 i = 0; i < helmetIDs.length; i++ ){
            if ( nft_1155.ownerOfNF( helmetIDs[ i ] ) == owner ) {
                helmet_ids[ idx ] = helmetIDs[ i ];
                idx++;
            }
        }
        return helmet_ids;
...
contract Industries is Initializable, OwnableUpgradeable {
    ...
    uint8 public _industries_version;

    uint256 public constant NFT_AMOUNT = 1;
    ...
    function createPart( address to ) public onlyOwner {
        NFT_1155 nft_1155 = _Validators._NFT_1155();
        nft_1155.mint( to, NFT_AMOUNT );
    }

    function createLand( address to ) public onlyOwner {
        NFT_1155 nft_1155 = _Validators._NFT_1155();
    ...
    function createLands( address to, uint256 quantity ) public onlyOwner {
        for (uint256 i = 0; i < quantity; i++) createLand( to );
    }

    function createArena( address to ) public onlyOwner {
        NFT_1155 nft_1155 = _Validators._NFT_1155();
    ...
    function createArenas( address to, uint256 quantity ) public onlyOwner {
        for (uint256 i = 0; i < quantity; i++) createArena( to );
    }

    function createHelmet( address to ) public onlyOwner {
        NFT_1155 nft_1155 = _Validators._NFT_1155();
    ...
    function createHelmets( address to, uint256 quantity ) public onlyOwner {
        for (uint256 i = 0; i < quantity; i++) createHelmet( to );
    }

    function createNexus( address to ) public onlyOwner {
        NFT_1155 nft_1155 = _Validators._NFT_1155();
    ...
    function createNexuses( address to, uint256 quantity ) public onlyOwner {
        for (uint256 i = 0; i < quantity; i++) createNexus( to );
    }

    function createCore( address to ) public onlyOwner {
        NFT_1155 nft_1155 = _Validators._NFT_1155();
    ...
    function createCores( address to, uint256 quantity ) public onlyOwner {
        for (uint256 i = 0; i < quantity; i++) createCore( to );
    }

    function createWeapon( address to ) public onlyOwner {
        NFT_1155 nft_1155 = _Validators._NFT_1155();
    ...
    function createWeapons( address to, uint256 quantity ) public onlyOwner {
        for (uint256 i = 0; i < quantity; i++) createWeapon( to );
    }

    function createAccessory( address to ) public onlyOwner {
        NFT_1155 nft_1155 = _Validators._NFT_1155();
    ...
    function createAccessories( address to, uint256 quantity ) public onlyOwner {
        for (uint256 i = 0; i < quantity; i++) createAccessory( to );
    }

    function createMotion( address to ) public onlyOwner {
        NFT_1155 nft_1155 = _Validators._NFT_1155();
    ...
    function createMotions( address to, uint256 quantity ) public onlyOwner {
        for (uint256 i = 0; i < quantity; i++) createMotion( to );
    }

    function destroyPart( address from, uint256 part_id ) public onlyOwner {
        NFT_1155 nft_1155 = _Validators._NFT_1155();
        nft_1155.burn( from, part_id, NFT_AMOUNT );
    }

    function destroyLand( address from, uint256 land_id ) public onlyOwner {
        NFT_1155 nft_1155 = _Validators._NFT_1155();

        require( _Validators.isValidLand( land_id ), "Provided land_id is not valid" );
        destroyPart( from, land_id );

        // Check if is NFT and then invalidate part
        if( nft_1155.isNotFungible( land_id )) invalidateLand( land_id );

        emit DestroyPart( "Land", from, land_id );
    }

    function destroyLands( address from, uint256[] memory land_ids ) public onlyOwner {
        for (uint256 i = 0; i < land_ids.length; i++) destroyLand( from, land_ids[i] );
    }

    function destroyArena( address from, uint256 arena_id ) public onlyOwner {
        NFT_1155 nft_1155 = _Validators._NFT_1155();

        require( _Validators.isValidArena( arena_id ), "Provided arena_id is not valid" );
        destroyPart( from, arena_id );

        // Check if is NFT and then invalidate part
        if( nft_1155.isNotFungible( arena_id )) invalidateArena( arena_id );

        emit DestroyPart( "Arena", from, arena_id );
    }

    function destroyArenas( address from, uint256[] memory arena_ids ) public onlyOwner {
        for (uint256 i = 0; i < arena_ids.length; i++) destroyArena( from, arena_ids[i] );
    }

    function destroyHelmet( address from, uint256 helmet_id ) public onlyOwner {
        NFT_1155 nft_1155 = _Validators._NFT_1155();

        require( _Validators.isValidHelmet( helmet_id ), "Provided helmet_id is not valid" );
        destroyPart( from, helmet_id );

        // Check if is NFT and then invalidate part
        if( nft_1155.isNotFungible( helmet_id )) invalidateHelmet( helmet_id );

        emit DestroyPart( "Helmet", from, helmet_id );
    }

    function destroyHelmets( address from, uint256[] memory helmet_ids ) public onlyOwner {
        for (uint256 i = 0; i < helmet_ids.length; i++) destroyHelmet( from, helmet_ids[i] );
    }

    function destroyNexus( address from, uint256 nexus_id ) public onlyOwner {
        NFT_1155 nft_1155 = _Validators._NFT_1155();

        require( _Validators.isValidNexus( nexus_id ), "Provided nexus_id is not valid" );
        destroyPart( from, nexus_id );

        // Check if is NFT and then invalidate part
        if( nft_1155.isNotFungible( nexus_id )) invalidateNexus( nexus_id );

        emit DestroyPart( "Nexus", from, nexus_id );
    }

    function destroyNexuses( address from, uint256[] memory nexus_ids ) public onlyOwner {
        for (uint256 i = 0; i < nexus_ids.length; i++) destroyNexus( from, nexus_ids[i] );
    }

    function destroyCore( address from, uint256 core_id ) public onlyOwner {
        NFT_1155 nft_1155 = _Validators._NFT_1155();

        require( _Validators.isValidCore( core_id ), "Provided core_id is not valid" );
        destroyPart( from, core_id );

        // Check if is NFT and then invalidate part
        if( nft_1155.isNotFungible( core_id )) invalidateCore( core_id );

        emit DestroyPart( "Core", from, core_id );
    }

    function destroyCores( address from, uint256[] memory core_ids ) public onlyOwner {
        for (uint256 i = 0; i < core_ids.length; i++) destroyCore( from, core_ids[i] );
    }

    function destroyWeapon( address from, uint256 weapon_id ) public onlyOwner {
        NFT_1155 nft_1155 = _Validators._NFT_1155();

        require( _Validators.isValidWeapon( weapon_id ), "Provided weapon_id is not valid" );
        destroyPart( from, weapon_id );

        // Check if is NFT and then invalidate part
        if( nft_1155.isNotFungible( weapon_id )) invalidateWeapon( weapon_id );

        emit DestroyPart( "Weapon", from, weapon_id );
    }

    function destroyWeapons( address from, uint256[] memory weapon_ids ) public onlyOwner {
        for (uint256 i = 0; i < weapon_ids.length; i++) destroyWeapon( from, weapon_ids[i] );
    }

    function destroyAccessory( address from, uint256 accessory_id ) public onlyOwner {
        NFT_1155 nft_1155 = _Validators._NFT_1155();

        require( _Validators.isValidAccessory( accessory_id ), "Provided accessory_id is not valid" );
        destroyPart( from, accessory_id );

        // Check if is NFT and then invalidate part
        if( nft_1155.isNotFungible( accessory_id )) invalidateAccessory( accessory_id );

        emit DestroyPart( "Accessory", from, accessory_id );
    }

    function destroyAccessories( address from, uint256[] memory accessory_ids ) public onlyOwner {
        for (uint256 i = 0; i < accessory_ids.length; i++) destroyAccessory( from, accessory_ids[i] );
    }

    function destroyMotion( address from, uint256 motion_id ) public onlyOwner {
        NFT_1155 nft_1155 = _Validators._NFT_1155();

        require( _Validators.isValidMotion( motion_id ), "Provided motion_id is not valid" );
        destroyPart( from, motion_id );

        // Check if is NFT and then invalidate part
        if( nft_1155.isNotFungible( motion_id )) invalidateMotion( motion_id );

        emit DestroyPart( "Motion", from, motion_id );
    }

    function destroyMotions( address from, uint256[] memory motion_ids ) public onlyOwner {
        for (uint256 i = 0; i < motion_ids.length; i++) destroyMotion( from, motion_ids[i] );
    }
