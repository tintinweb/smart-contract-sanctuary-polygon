...
// File: contracts/interfaces/Planet-Universe-Interface-Traits.sol



pragma solidity ^0.8.0;

interface PlanetUniverseInterfaceTraits {
...
// File: contracts/Planet-Universe-Traits.sol
...
contract Traits is Ownable, PlanetUniverseInterfaceTraits {
...
constructor() {}
  ...
  function setPlanetNFT(address _PlanetUniverseInterface) external onlyOwner {
    PlanetUniverseNFT = PlanetUniverseInterface(_PlanetUniverseInterface);
  }
...
// * generates a base64 encoded metadata response without referencing off-chain content * @param [tokenId] the ID of the token to generate the metadata for * @return a [base64 encoded JSON dictionary of the token's metadata and SVG]
  function tokenURI(uint256 tokenId) public view override returns (string memory) {
    ...
    string memory metadata = string(abi.encodePacked(
      '{"name": "',
      'Planet Universe Gen0/1 #',
      tokenId.toString(),
      '", "description": "Planet Universe Gen 0 and 1 Collection | NO IPFS | NO API just the Polygon blockchain | P2E the right way with Planets and Aliens", "image": "data:image/svg+xml;base64,',
...
// Compile the base64 string and decode it Custom Ratigan @Planet Universe 2022 © No other game uses true SVG's we do!
  function compileBase64(uint256 tokenId) internal view returns (string memory) {
    ...
    if(s.isPlanet) {  traits = string(abi.encodePacked(
        ...
        Base64.decode(startbase()),
        Base64.decode(singleAttributeBase64(traitData[9][s.traitarray[9]].svg)), 
        Base64.decode(singleAttributeBase64(traitData[10][s.traitarray[10]].svg)),
        Base64.decode(singleAttributeBase64(traitData[11][s.traitarray[11]].svg)),
        Base64.decode(singleAttributeBase64(traitData[12][s.traitarray[12]].svg)),
        Base64.decode(singleAttributeBase64(traitData[13][s.traitarray[13]].svg)),
        Base64.decode(singleAttributeBase64(traitData[14][s.traitarray[14]].svg)),
        Base64.decode(singleAttributeBase64(traitData[15][s.traitarray[15]].svg)),
        Base64.decode(singleAttributeBase64(traitData[16][s.traitarray[16]].svg))
        ));}
...
// *generates an array composed of all the individual ATTRIBUTES * @param [tokenId] the ID of the token to compose the metadata for * @return a JSON array of all of the attributes for given token ID
  function compileAttributes(uint256 tokenId) internal view returns (string memory) {
    ...
    if(s.isPlanet) {  traits = string(abi.encodePacked(
        ...
        attributeForTypeAndValue(_traitTypes[7], traitData[7][s.traitarray[7]].name)               
        ));}
        else {
        traits = string(abi.encodePacked(
        attributeForTypeAndValue(_traitTypes[9], traitData[9][s.traitarray[9]].name), ',',
        attributeForTypeAndValue(_traitTypes[10], traitData[10][s.traitarray[10]].name), ',',
        attributeForTypeAndValue(_traitTypes[11], traitData[11][s.traitarray[11]].name), ',',
        attributeForTypeAndValue(_traitTypes[12], traitData[12][s.traitarray[12]].name), ',',
        attributeForTypeAndValue(_traitTypes[13], traitData[13][s.traitarray[13]].name), ',',
        attributeForTypeAndValue(_traitTypes[14], traitData[14][s.traitarray[14]].name), ',',
        attributeForTypeAndValue(_traitTypes[15], traitData[15][s.traitarray[15]].name), ',',
   ...
   return string(abi.encodePacked(
      ...
      Strings.toString(s.generation),
      '},{"display_type": "number", ',' "trait_type":"Building Plots","value":',
      Strings.toString(s.buildingPlots),
      '},{"trait_type":"Breeding Amount","value":',
      Strings.toString(s.breedingAmount),
      '},{"trait_type":"Type","value":',
      s.isPlanet ? '"Planet"' : '"Alien"', '}'
   ));
  }
}