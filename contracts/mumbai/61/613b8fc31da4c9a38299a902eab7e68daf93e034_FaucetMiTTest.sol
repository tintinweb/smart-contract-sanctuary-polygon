...
contract FaucetMiTTest is Ownable{
    ...
    function withdrawMaticByUser(string memory code) public onlyWhiteList{
        uint balance = balanceOfContract();
        require(balance >= (MaxQuantity),"There is not enought tokens to retrieve");
        require(bytes(userCode[msg.sender]).length > 0,"User doesnt have a code");
    ...
    function withDrawMatic(address wallet, string memory code)public onlyOwner{
        require(activeCode[code], "Code already active");
        require(whiteList[wallet].canRetrieve,"User can retrieve tokens");
        require(bytes(userCode[wallet]).length > 0,"User doesnt have a code!");
        require(bytes(code).length > 0,"Code is empty!");
        uint balance = balanceOfContract();
        require(balance >= (MaxQuantity),"There is not enought tokens to retrieve");
