...
contract ComplianceProvider_Covest is Initializable, UUPSUpgradeable, ReentrancyGuardUpgradeable  {

    mapping(string  => uint256) internal historyValueOfUser;
    mapping(address => mapping(address => uint256)) internal valueOfCP;
    ...
    function initialize(address _nameRegistry,address _whoIsCP,string memory _signatureIs) public initializer {
    ...
    function fundInFlow(address _caller,address _asset,string memory _policyId, uint256 _value) public isPolicy { // only Policy can Manage //
        ...
        valueOfCP[_asset][whoIsCP] += _value;
    ...
    function withdraw(address _asset, uint256 _value) public nonReentrant checkAsset(_asset) { // for CP //
        IERC20 _ERC20 = IERC20(_asset);
        require(PDS.isBlacklistOfUser(msg.sender) != true,"Policy: This user are Blacklist.");
        require(PDS.isBlacklistAssetsOfUser(msg.sender,_asset) != true,"Policy: This Assets of you are Blacklist.");
        require(_ERC20.balanceOf(address(this)) >= _value,"Balance of Contract Not Enough.");
        require(whoIsCP == msg.sender,"CP: You are not CP.");
        require(valueOfCP[_asset][msg.sender] >= _value,"CP_User: Not Enough.");

        valueOfCP[_asset][whoIsCP] -= _value;
        _ERC20.transfer(msg.sender,_value);
   
        emit Withdraw(_asset,msg.sender,_value,valueOfCP[_asset][msg.sender]);
    }
    ...
    function balanceOfComplianceProvider(address _asset,address _user) public view returns(uint256) {
        return valueOfCP[_asset][_user];
    }

    function whoIsComplianceProvider() public view returns(address,string memory) {
        return (whoIsCP,signature);
    }
