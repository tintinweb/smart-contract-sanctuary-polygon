...
contract LAND is ERC721, Ownable {
    ...
    uint256 internal constant GRID_SIZE_Y = 140;

    address EstateRegistryAddress = 0x1b3A447EFcc29E8d57Db9276cA0651083EBCb2AE;

    IEstateRegistry estateInterface = IEstateRegistry(EstateRegistryAddress);
    ...
    constructor() ERC721("TestLand", "TSL") {
        setBaseURI("https://62cd7d9b066bd2b69926fd66.mockapi.io/api/v1/parcel/");
    }
    ...
    function mintToken(
        ...
        _safeMint(to, getTokenId(x, y));
        _setTokenURI(getTokenId(x, y), Strings.toString(getTokenId(x, y)));
    }
    ...
    function mintMultipleTokens(
        ...
        for (uint256 i = 0; i < x.length; i++) {
            _safeMint(to, getTokenId(x[i], y[i]));
            _setTokenURI(getTokenId(x[i], y[i]), Strings.toString(getTokenId(x[i], y[i])));
        }
    }
    ...
    function getTokenId(int256 x, int256 y) public pure returns (uint256) {
    ...
    /**
     * @dev Transfer a parcel to another address using the parcel's toke id.
     */
    ...
    function _createEstate(
        ...
        uint256 estateTokenId = estateInterface.mint(to, metadata);

        for (uint256 i = 0; i < x.length; i++) {
            uint256 tokenId = getTokenId(x[i], y[i]);
            safeTransferFrom(ownerOf(tokenId), address(EstateRegistryAddress), tokenId);
        }
