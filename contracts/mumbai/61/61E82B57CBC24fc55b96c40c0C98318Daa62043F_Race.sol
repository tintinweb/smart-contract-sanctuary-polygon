...
contract Race {
    ...
    function getNFTGameList(address nftAddress) external view returns (GameState[] memory)  {
    ...
    function settle(
        ...
        for (uint256 i = 0; i < params.length; i++) {
            int256 idx = _findPlayer(gameState.players, params[i].player);
            require(idx >= 0, "Invalid settle player address");
            PlayerJoin storage playerJoin = gameState.players[i];
            if (params[i].operator == SettleOp.Add) {
                // 加
                playerJoin.balance = playerJoin.balance.add(params[i].amount);
                sum = sum.add(params[i].amount);
            } else if (params[i].operator == SettleOp.Sub) {
                // 减
                playerJoin.balance = playerJoin.balance.sub(params[i].amount);
                sum = sum.sub(params[i].amount);
            } else if (params[i].operator == SettleOp.Eject) {
                // transfer balance
    ...
    function getOwnerNFTList(
        ...
        for (uint256 i = 0; i < len; i++) {
            NFTTemplate nftContract = NFTTemplate(nftAddressList[i]);
            (
                string memory name,
