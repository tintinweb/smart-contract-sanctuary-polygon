...
import {VaultDeployerLib} from "./VaultDeployerLib.sol";

contract VaultDeployer is IVaultDeployer, StateAware, UUPSUpgradeable {
    using Clones for address;

    address immutable ASSET_REGISTRY_ADDRESS;
    address immutable PRICE_AGGREGATOR_ADDRESS;
    address immutable FEE_CONTROLLER_ADDRESS;
    ...
    constructor(
        ...
        if (
            _assetRegistryAddress == address(0) || _priceAggregatorAddress == address(0)
                || _feeControllerAddress == address(0)
        ) revert Errors.ZeroAddress();

        ASSET_REGISTRY_ADDRESS = _assetRegistryAddress;
        PRICE_AGGREGATOR_ADDRESS = _priceAggregatorAddress;
        FEE_CONTROLLER_ADDRESS = _feeControllerAddress;

        _disableInitializers();
    }

    function init() external initializer {
        vaultImplementation = VaultDeployerLib.deployNewVault(
            address(DREM_HUB), ASSET_REGISTRY_ADDRESS, PRICE_AGGREGATOR_ADDRESS, FEE_CONTROLLER_ADDRESS
        );
    }
     ...
     * @dev The 'pausauble' modifier is inherited from StateAware. It does not refer to OZ's pausauble implementation
     * @param _variableEncodedArgs The variable argument data used in the initial wind
     * @return The address of the newly created vault
     */
    function deployVault(DataTypes.DeploymentInfo calldata _deploymentInfo, bytes[] calldata _variableEncodedArgs)
        external
        pausable
        returns (address)
    {
        // The remaining input validation is completed during the vault initialization
        if (_deploymentInfo.admin == address(0)) revert Errors.ZeroAddress();

        /// Move funds into the vault deployer
        /// The transfer step will always be the first step
        // Therefore, the amount in will always be the first argument
        uint256 amountIn = abi.decode(_variableEncodedArgs[0], (uint256));
        IERC20(_deploymentInfo.denominationAsset).transferFrom(msg.sender, address(this), amountIn);
        ...
        // Init
        IVault(_vault).init(
            _deploymentInfo.admin,
            _deploymentInfo.name,
            _deploymentInfo.symbol,
            _deploymentInfo.denominationAsset,
            _deploymentInfo.steps,
            _deploymentInfo.feeInfo
        );

        // Wind the steps
        IERC20(_deploymentInfo.denominationAsset).approve(_vault, amountIn);
        IVault(_vault).windSteps(amountIn, _variableEncodedArgs);

        // Transfer vault shares to the msg.sender
        IERC20(_vault).transfer(msg.sender, IERC20(_vault).balanceOf(address(this)));

        // Emit event
        emit Events.VaultDeployed(msg.sender, _vault, _deploymentInfo.name, _deploymentInfo.symbol);
