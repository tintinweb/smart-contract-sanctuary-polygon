...
contract Slots is VRFConsumerBaseV2 {
        ...
        )
    {
        ...
        return (
            ...
            gamesResult[player][length].randNumber1,
            gamesResult[player][length].randNumber2,
            gamesResult[player][length].randNumber3
        );
    }

    // ------------------------------------------------------------------------------------------------------------------------------------

    function startRoll() external payable returns (uint256 requestId) {
        ...
        gamesStart[requestId] = GameStart(msg.sender, msg.value);
    ...
    function fulfillRandomWords(uint256 _requestId, uint256[] memory _randomWords) internal override {
        ...
        endRoll(
            _requestId,
            (_randomWords[0] - (_randomWords[0] / 10 * 10)) > 5 ? (_randomWords[0] - (_randomWords[0] / 10 * 10)) - 5 : (_randomWords[0] - (_randomWords[0] / 10 * 10)),
            ((_randomWords[0] - (_randomWords[0] / 100 * 100)) / 10) > 5 ? ((_randomWords[0] - (_randomWords[0] / 100 * 100)) / 10) - 5 : ((_randomWords[0] - (_randomWords[0] / 100 * 100)) / 10),
            ((_randomWords[0] - (_randomWords[0] / 1000) * 1000) / 100) > 5 ?((_randomWords[0] - (_randomWords[0] / 1000) * 1000) / 100) - 5 : ((_randomWords[0] - (_randomWords[0] / 1000) * 1000) / 100)
            );
    }
    ...
    function endRoll(uint256 _requestId,uint256 firstDegigt, uint256 secondDigit, uint256 thirdDigit) internal {

        uint256 result = calculatePrize(firstDegigt, secondDigit, thirdDigit, gamesStart[_requestId].deposit);
        if (result != 0) {
            totalWinsOfPlayer[gamesStart[_requestId].player] += result;
            (bool success, ) = pool.call(abi.encodeWithSignature("getReward(uint256,address)", result, gamesStart[_requestId].player));
            require(success);
        }
        gamesResult[gamesStart[_requestId].player].push(
            GameRes(result, firstDegigt, secondDigit, thirdDigit)
        );
    }
