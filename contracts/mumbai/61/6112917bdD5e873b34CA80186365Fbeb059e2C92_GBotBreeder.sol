...
contract GBotBreeder is ERC721Receiver, Ownable, Pausable, GBotBabyLock, BreedingProperties {
  ...
  event BreedingStarted(address indexed owner, uint256[] parents, uint256 tokenId, uint256 lockTimestamp);
    ...
    function breed(address sender, bytes calldata sig, uint256[] memory tokenIds) public virtual whenNotPaused {
        ...
        for (uint i=0; i < tokenIds.length; i++) {
          uint256 energyCores = getMetadataForProperty(parentMetadatas[i], ENERGY_CORES_BITS);
          GBotContract.upgradeGBot(tokenIds[i], burnEnergyCores(parentMetadatas[i], energyCores -1));
        }        
    ...
    function burnEnergyCores(uint256 metadata, uint256 propertyValue) public pure returns (uint256 newMetadata){
       // 8 bits and 191 is the position of the energyCores
       require(0 < 8 && 191 < 256 && 191 + 8 <= 256, "GetMetadata: Incorrect number of bits when burning energy cores");
       metadata |= (propertyValue << 191);
       return metadata;
    }
