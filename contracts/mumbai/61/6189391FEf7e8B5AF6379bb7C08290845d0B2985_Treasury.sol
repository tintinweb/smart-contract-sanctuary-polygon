...
contract Treasury is HSystemChecker, Multicall {
    ...
    constructor(address systemCheckerContractAddress, address milkChildContractAddress)
    ...
    function claim(
        ...
        for (uint256 i; i < catIds.length; i++) {
            ...
            unchecked {
                reward +=
                    ((_baseRate + _catClassBonus[catClass]) * (currentTime - timeToken)) /
                    86400;
            }
    ...
    function calcClaim(uint256[] calldata catIds, uint256[] calldata catClasses)
        public
        view
        returns (uint256 reward)
    {
        ...
        for (uint256 i; i < catIds.length; i++) {
            ...
            unchecked {
                reward +=
                    ((_baseRate + _catClassBonus[catClass]) * (block.timestamp - timeToken)) /
                    86400;
            }
        }
    ...
    /// @param catClass[] - Cat classes
    /// @param bonus[] - Daily bonus per class, respective to the first param index
    function setCatClassBonus(uint256[] calldata catClass, uint256[] calldata bonus)
        external
        onlyRole(ADMIN_ROLE)
    {
    ...
    function setMilkChildContractAddress(address milkChildContractAddress)
        external
        onlyRole(ADMIN_ROLE)
    {
        _milkChildContractAddress = milkChildContractAddress;
