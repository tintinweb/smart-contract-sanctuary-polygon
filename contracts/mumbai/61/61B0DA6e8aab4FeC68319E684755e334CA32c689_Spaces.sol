...
contract Spaces {
  ...
  function verifySpace(string memory _id) public payable _ownerOnly {
    // find space with name
    ParkingSpace memory space = carPark[_id];
    // pre-guards
  ...
  function toggleSpaceEnabled(string memory _id, bool enable) public payable {
    // find space with name
    ParkingSpace memory space = carPark[_id];
  ...
  function updateLocationHint(string memory _id, string memory _locationHint) public payable {
    ParkingSpace memory space = carPark[_id];
    require(space.landlord != address(0), 'NOT_REGISTERED');
  ...
  function updateRules(string memory _id, string memory _rules) public payable {
    ParkingSpace memory space = carPark[_id];
    require(space.landlord != address(0), 'NOT_REGISTERED');
    require(space.landlord == msg.sender, 'NOT_LANDLORD');
  ...
  function transferToNewOwner(string memory _id, address _newLandlord) public payable {
    ParkingSpace memory space = carPark[_id];
    require(space.landlord != address(0), 'NOT_REGISTERED');
    require(space.landlord == msg.sender, 'NOT_LANDLORD');
  ...
  function removeSpace(string memory _id) public payable {
    ParkingSpace memory space = carPark[_id];
    require(space.landlord != address(0), 'NOT_REGISTERED');
    require(space.landlord == msg.sender, 'NOT_LANDLORD');
    delete carPark[_id];
