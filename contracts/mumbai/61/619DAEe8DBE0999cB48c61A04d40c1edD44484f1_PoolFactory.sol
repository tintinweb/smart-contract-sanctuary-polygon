...
contract IDOPool is Ownable, Pausable, KLAYDAOWhitelist, IERC1155Receiver, VRFConsumerBase {
    ...
    function buyTicket(
        ...
        require(_verifyMintTicket(msg.sender, _amount, currentTicketID, _quantity, proof), "POOL:INVALID_MERKLE");

        _forwardTokenFunds(_amount);

        ITicket(ticket).mint(address(this), currentTicketID, _quantity, _data);

        userTicket[currentTicketID][msg.sender] += _quantity;
        currentTicketID += 1;

        emit BuyTicket(msg.sender, _amount, currentTicketID, _quantity);
    }
