...
contract HPirates is IHPirates {
    ...
    string[1] _traitTypes = ["Legendary"];

    // storage of each traits name and base64 PNG data
    mapping(uint8 => mapping(uint8 => Trait)) public traitData;

    IPirates public pirNFT;
    ...
    function setPirates(address _pirNFT) external onlyOwner {
        pirNFT = IPirates(_pirNFT);
    }
     ...
     * @return a valid SVG of the Ai Lax Pirate
     */
    function drawSVG(uint256 tokenId) public view returns (string memory) {
        IPirates.HPirates memory r = pirNFT.getTokenTraitsHonors(tokenId);
        string memory svgString = string(
            abi.encodePacked(drawTrait(traitData[0][r.Legendary]))
        );
        return
            string(
                abi.encodePacked(
                    '<svg id="shatteredEon" width="100%" height="100%" version="1.1" viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">',
                    svgString,
                    "</svg>"
                )
            );
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    '"Pirate"',
                    '},{"trait_type":"Rank","value":"Dreadnought"',
                    "}]"
                )
            );
    }
    ...
    function tokenURI(uint256 tokenId)
        ...
        returns (string memory)
    {
        require(msg.sender == address(pirNFT), "no no");
        string memory tokenName = pirNFT.tokenNameByIndex(tokenId);
        bool hasName = pirNFT.hasBeenNamed(tokenId);
