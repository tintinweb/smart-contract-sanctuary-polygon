...
// File: contracts/QueryToxicity.sol

pragma solidity 0.8.5;


contract QueryToxicity is UsingTellor {
    constructor(address payable _tellor) UsingTellor(_tellor) {}

    function getToxicity(string memory _collateralAsset, string memory _loanAsset) public view returns(bool _success, uint256 _toxicity) {
        // generate queryId for lending pair toxicity
        bytes memory _queryData = abi.encode("LendingPairToxicity", abi.encode(_collateralAsset, _loanAsset));
        bytes32 _queryId = keccak256(_queryData);
        // retrieve value from oracle
        (, bytes memory _valueRetrieved, uint256 _timestampRetrieved) = getDataBefore(_queryId, block.timestamp - 2 hours);
        // handle no value retrieved
        if(_timestampRetrieved == 0) {
            return (false, 0);
        }
        // decode bytes into uint256
        _toxicity = abi.decode(_valueRetrieved, (uint256));
        _success = true;
    } 
}