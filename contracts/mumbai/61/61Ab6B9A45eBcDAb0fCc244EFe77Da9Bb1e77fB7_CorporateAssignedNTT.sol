...
interface IERC721Compatibility {
   ...
   *
   * Improves compatibilty with IERC721
   */
   ...
   * @dev Improves compatibilty with IERC721Enumerable
   */
...
/**
 * This interface reflects the fact that persons can win the monthly & yearly MVP
 */
interface IMVP {
  enum Month {
    January,
  ...
  /**
   * Sets the latest MVP of the year
   *
   * @param winner The winner of the MVP
   * @param tokenId The winning token ID
   */
  function setMvpOfTheYear(address winner, uint256 tokenId) external;

  /**
   * Reports the latest token ID to win the MVP of the year
   *
   * @return The latest token ID to win the MVP
   */
  function getMvpOfTheYear() external view returns (MVP memory);

  /**
   * Sets the latest MVP of the month for a given month
   *
   * @param tokenId The Token ID to mark as the MVP
   * @param month The month
   */
  function setMvpOfTheMonth(address winner, uint256 tokenId, Month month) external;

  /**
   * Reports the latest token ID that won the MVP for each month
   *
   * @return The latest token IDs that won the MVP per month (Note: 0 demarcates no winner yet)
   */
  function getMontlhyMvps() external view returns (MVP[12] memory);
}
 ...
 */
interface ICorporateAssignedNTT is IERC4671Metadata, IERC4671Enumerable, ILimitedTokenGenerator, IERC721Compatibility {
  /**
...
error CorporateAssignedNTT__InvalidOwnerAndTokenCombination();

/**
 * Corporate assigned, non-tradeable tokens
 */
contract CorporateAssignedNTT is ICorporateAssignedNTT, ERC165, IMVP {
  // Token name
  ...
  // Latest winner of the yearly MVP
  MVP private s_yearlyMvp;

  // Latest winners of the monthly MVP, by month
  MVP[12] private s_monthlyMvps;
  ...
  /**
   * @dev See {IMVP-setMvpOfTheYear}.
   */
  function setMvpOfTheYear(address winner, uint256 tokenId) external onlyContractOwner {
    address owner = s_owners[tokenId];
    ...
    if (owner != winner) {
      revert CorporateAssignedNTT__InvalidOwnerAndTokenCombination();
    }

    s_yearlyMvp.tokenId = tokenId;
    s_yearlyMvp.winner = winner;
  }

  /**
   * @dev See {IMVP-getMvpOfTheYear}.
   */
  function getMvpOfTheYear() external view returns (MVP memory) {
    return s_yearlyMvp;
  }
  ...
  /**
   * @dev See {IMVP-setMvpOfTheMonth}.
   */
  function setMvpOfTheMonth(address winner, uint256 tokenId, Month month) external onlyContractOwner {
    address owner = s_owners[tokenId];
    ...
    s_monthlyMvps[uint(month)].tokenId = tokenId;
    s_monthlyMvps[uint(month)].winner = winner;
  }

  /**
   * @dev See {IMVP-getMontlhyMvps}.
   */
  function getMontlhyMvps() external view returns (MVP[12] memory) {
    return s_monthlyMvps;
  }
}