...
contract EndersBattlePass is ERC1155Supply, ReentrancyGuard, AccessControl {
    ...
    function buyRewards(address tokenToPay) public payable {
        uint256 id = seasons[currentSeason].rewardId;
        uint256 cost = getPrice(tokenToPay, 1);
    ...
    function getPrice(address tokenToGet, uint256 quantity)
        public
        view
        returns (uint256)
    {
    ...
    function addSeason(uint256 id, uint256 priceUSD)
        external
        onlyRole(SEASON_ROLE)
    {
        require(
    ...
    function editSeason(uint256 id, uint256 priceUSD)
        external
        onlyRole(SEASON_ROLE)
    {
        seasons[currentSeason].priceUSD = priceUSD;
