...
contract MustasheRPSGame {
    ...
    function play(uint256 _gameId, string memory _choice) external {
        ...
        require(
            bytes(game.choices[msg.sender]).length == 0,
        ...
        if (
            bytes(game.choices[game.creator]).length > 0 &&
            bytes(game.choices[game.opponent]).length > 0
        ) {
    ...
    function revealChoices(uint256 _gameId) private {
        ...
        if (keccak256(bytes(choice1)) == keccak256(bytes(choice2))) {
            ...
            if (
                (keccak256(bytes(choice1)) == keccak256(bytes("rock")) &&
                    keccak256(bytes(choice2)) == keccak256(bytes("scissors"))) ||
                (keccak256(bytes(choice1)) == keccak256(bytes("scissors")) &&
                    keccak256(bytes(choice2)) == keccak256(bytes("paper"))) ||
                (keccak256(bytes(choice1)) == keccak256(bytes("paper")) &&
                    keccak256(bytes(choice2)) == keccak256(bytes("rock")))
            ) {
                game.wins[creator]++;
            } else {
                game.wins[opponent]++;
