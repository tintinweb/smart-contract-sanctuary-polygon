...
contract EthermonStaking is BasicAccessControl, EthermonStakingData {
    using SafeERC20 for IERC20;

    event Withdraw(address _to, uint256 _amount);
    event Deposite(address _to, uint256 _amount);
    ...
    function depositeTokens(uint256 _day, uint256 _amount) external {
        ...
        emit Deposite(stakingDataContract, _amount);
    }

    function withdrawTokens(address _owner) external {
        EthermonStakingData stakingData = EthermonStakingData(
            stakingDataContract
        );
        TokenData[] memory data = stakingData.getTokenData(_owner);
        for (uint256 i = 0; i < data.length; i++) {
            
            if (data[i].day <= block.timestamp) {
                uint256 balance = emon.balanceOf(stakingDataContract);
                require(balance > 0);
                
                stakingData.removeTokenData(_owner, i);
                ...
                emit Withdraw(stakingDataContract, data[i].amount);
                break;
            }
        }
    }
