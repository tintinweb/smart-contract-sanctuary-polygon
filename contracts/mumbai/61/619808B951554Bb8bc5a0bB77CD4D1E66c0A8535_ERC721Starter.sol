...
contract ERC721Starter is ERC721Enumerable, Ownable, PrivateSale, Airdrop {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIds; // library buat naikin number (increment/decrement)

    string public baseTokenURI;
    uint256 public constant PRICE = 0.1 ether; // akan ngikutin native currency dari blockchain

    constructor(
        string memory name,
        string memory symbol,
        string memory _baseTokenURI,
        uint256 _privateSaleStartTimestamp,
        uint256 _privateSaleEndTimestamp
    )
        ERC721(name, symbol)
        PrivateSale(_privateSaleStartTimestamp, _privateSaleEndTimestamp)
    {
    ...
    {
        require(
            _exists(tokenId), // token exist atau engga -> kalo tokennya gada gabisa di return uri nya
    ...
    function mintPrivateSale() public payable duringPrivateSale {
        require(msg.value == PRIVATE_SALE_PRICE, "ether must be same as price");
        require(addressToMintQty[msg.sender] != 0, "not allowed mint");
        require(
            !addressToDoneMinting[msg.sender],
            "had minted during private sale"
        );

        uint256 qtyToMint = addressToMintQty[msg.sender];
    ...
    function sendAirdropForEachAddress() public onlyOwner {
        for (uint256 i = 0; i < addressesForAirdrop.length; i++) {
            if (
                addressToAllowedAirdrop[addressesForAirdrop[i]] == true &&
                addressHasReceiveAirdrop[addressesForAirdrop[i]] == false
            ) {
                addressHasReceiveAirdrop[addressesForAirdrop[i]] = true;
                uint256 currentTokenId = _tokenIds.current();
                _safeMint(addressesForAirdrop[i], currentTokenId);
                _tokenIds.increment();
            }
        }
    }
    ...
    function claimAirdrop() public {
        ...
        require(
            !addressHasReceiveAirdrop[msg.sender],
    ...
    function mint() public payable {
        require(msg.value == PRICE, "ether must be same as price"); // condition ether yg dikirim harus sama dengan price kalo ga sama/false akan nge revert

        uint256 currentTokenId = _tokenIds.current();
        _safeMint(msg.sender, currentTokenId); // function yg disediakan erc721 akan mint nft sekian untuk addres ini (msg.sender)
        _tokenIds.increment(); // token akan di increment karna sudah di mint 1
    }
...
contract PrivateSale is Ownable {
    ...
    mapping(address => bool) public addressToDoneMinting; // false

    uint256 public constant PRIVATE_SALE_PRICE = 0.3 ether;
    ...
    function addAddressesToWhitelist(
        ...
        require(
            _addresses.length == _qtys.length,
            "addresses length is not the same with qtys"
        );
    ...
    function whitelistedAddressCount() public view returns (uint256) {
