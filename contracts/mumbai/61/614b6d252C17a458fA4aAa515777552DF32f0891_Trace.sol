...
contract Trace is AccessControl {
    ...
    constructor() {
        // ADMIN akan langsung diberikan kepada account
        // yang melakukan deployment
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
    }

    // Fungsi registrasi stakeholder
    function inputStakeholder(
        address account,
        bytes32 jenis_akses,
        string memory no_sertifikat
    ) public onlyRole(DEFAULT_ADMIN_ROLE) {
        // Memberikan akses kepada operator
        _setupRole(jenis_akses, account);

        stakeholder[account] = Stakeholder(
            account,
            string(abi.encodePacked(jenis_akses)),
            no_sertifikat,
            block.timestamp
        );
    }
    ...
    function getDetailStakeholder(address account) public view returns (
        Stakeholder memory
    )
    {
        Stakeholder memory detail = stakeholder[account];
        return (detail);
    }
}