...
contract Factory is ReentrancyGuard {
    ...
    function createNew(
        ...
        require(
            _totalAmount > 0,
            "Total amount payable should be greater than 0"
        );
        POT t = new POT(
            msg.sender,
            _passenger,
...
contract POT is ReentrancyGuard {
    ...
    function paymentByPassenger(uint256 amountPayable) public onlyPassenger {
        // Comprobar si el contrato no ha sido activado
        require(!contractActivated, "Contract already activated");
        require(amountPayable == totalAmount, "Amount not correct");
    ...
    function finishTrip() internal {
        require(!isTripFinished && activatedByPassenger, "Trip already finished");
            if (currState == State.COMPLETE && deliveryByPassenger) {
                token.transfer(driver, totalAmount / 90);
                token.transfer(arbitrator, totalAmount / 10);
            }
            if (
                currState == State.COMPLETE &&
                deliveryByDriver &&
