...
contract InvaderMarketplace is ReentrancyGuard, Owned {
    ...
    function delistNFT (
        ...
        emit MarketItemDelisted(itemId, msg.sender);
    ...
    function getListedItems() public view returns(MarketplaceItem[] memory) {
        uint amtListed = listedItems - soldItems - delistedItems;
        uint count = 0;

        MarketplaceItem[] memory items = new MarketplaceItem[](amtListed);
        for (uint i = 0; i < listedItems; i++) {
            if (idToItem[i + 1].sold == false && idToItem[i + 1].forSale == true) {
                MarketplaceItem storage currentItem = idToItem[i + 1];
