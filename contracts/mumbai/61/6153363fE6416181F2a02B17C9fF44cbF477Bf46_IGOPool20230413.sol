...
import {ERC721Enumerable as IERC721Enumerable} from "@openzeppelin/contracts/token/ERC721/extensions/ERC721Enumerable.sol";
...
contract IGOPool20230413 is Ownable, ReentrancyGuard, structItem {
    ...
    function igo(uint256 idoNum, uint256 _cosoID) external payable nonReentrant {
        ...
        for (uint256 i = 0; i < idoNum; i++) {
            //从NFT所有者转账NFT给用户
            uint256 _token_id = fk1.nftToken.tokenOfOwnerByIndex(nftOwner, 0);
            fk1.nftToken.transferFrom(nftOwner, msg.sender, _token_id);
