...
contract StrategyDutchAuction is IExecutionStrategy, Ownable {
    ...
    event NewMinimumAuctionLengthInSeconds(
        uint256 minimumAuctionLengthInSeconds
    );
        ...
        )
    {
        ...
        require(
            endTime >= (startTime + minimumAuctionLengthInSeconds),
        ...
        uint256 currentAuctionPrice = startPrice -
            (((startPrice - endPrice) * (block.timestamp - startTime)) /
                (endTime - startTime));
     ...
     */
    function updateMinimumAuctionLength(uint256 _minimumAuctionLengthInSeconds)
        external
        onlyOwner
    {
        require(
