...
contract Marketplace is Initializable, Pausable, AccessControlUpgradeable {
    ...
    struct AuctionState {
        ...
        uint32 duration; // not applicable yet
        uint256 price;
        uint256 reservePrice; // not applicable yet
        address seller;
        bool resell;
    }
    ...
    function initialize(address _nft) public initializer {
        ...
        foundationFee = 200; // 2% foundation on only initial sale
        adminFee = 800; // 8% admin  =>. adminAddress on initial sale

        adminFeeOnResell = 250; // 2.5% admin  =>. adminAddress on resell
        creatorFee = 750; // 7.5% creator (init artist) on resell
