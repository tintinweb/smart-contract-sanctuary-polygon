...
contract Treasury is Ownable {
    ...
    modifier onlyAuthorized() {
        require(msg.sender == owner() || msg.sender == rewardContract, "Not Authorized");
        _;
    }

    function updateRewardContract(address rewardContract_)
        external
        onlyOwner
    {
        //onlyInitializing
        require(
            rewardContract_.isContract(),
            "Campaign: Address is not a contract"
        );
        rewardContract = rewardContract_;
    }

    function getRewardContract() public view returns (address) {
    ...
    function deposit(uint amount_, uint campaignID_) external payable {
        if(msg.value > 0) {
            tokens[address(0)][campaignID_] += msg.value;
        }
    }
     ...
     */
    function withdrawFunds(address payable to, uint256 amount, uint campaignID_)
        public
        onlyAuthorized
    {
        require(tokens[address(0)][campaignID_] >= amount, "Insufficient deposit");
        tokens[address(0)][campaignID_] -= amount;
        to.sendValue(amount);

        emit FundsWithdrawn(campaignID_, to, amount); 
    }
