...
contract Position is ReentrancyGuard, Ownable, PositionStorage {
    ...
    function addMargin(uint256 positionID, uint256 marginAmount, uint256 source) external payable {
        ...
        if(source == 1) {
            ...
            uint256 maxAddMargin = getMaxAddMargin(availableMargin, availableLoan) ;
    ...
    function getMaxAddMargin(uint256 margin, uint256 loan) public view returns(uint256) {
        uint256 maxLeverage = IAdminFunctions(adminContract).getBaseLtm();
        uint256 maxMargin = (maxLeverage * margin - loan)/maxLeverage;
        return maxMargin;
    }
