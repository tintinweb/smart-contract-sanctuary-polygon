...
contract StakingContract {
    ...
    function updateDirectRefPercent(
        uint256 poolIndex,
        uint256[15] memory rewardPercentages
    ) external onlyAdmin {
        require(poolIndex < poolsCount, "Invalid Pool");
        pools[poolIndex].levelRewards = rewardPercentages;
    }

    function togglePoolStatus(uint256 poolIndex) external onlyAdmin {
        require(poolIndex < poolsCount, "Invalid Pool");
        pools[poolIndex].isActive = !pools[poolIndex].isActive;
    }

    function updateMaxPoolSize(uint256 poolIndex, uint256 maxPoolSize)
        external
        onlyAdmin
    {
        require(poolIndex < poolsCount, "Invalid Pool");
        pools[poolIndex].maxPoolSize = maxPoolSize;
    }
    ...
    function stake(
        ...
        require(referrer != address(0) || msg.sender==admin, "Invalid referrer");
        require(
            stakingAmount >= pools[poolIndex].minAmount,
            "Stake Amount Is very Less"
        );
        require(pools[poolIndex].isActive==true,"Pool is Not Active");
        require(users[referrer].isActive == true, "Referrer is not active");
        if (users[msg.sender].referrer != address(0) || msg.sender == admin) {
    ...
    function claimRewards(address user, uint256 poolIndex) public {
        ...
        users[msg.sender].deposits[poolIndex].totalWithdrawn +=
            (timePassed *
                users[user].deposits[poolIndex].stakedAmount *
                pools[poolIndex].roiPercent) /
            10000 /
    ...
    function unstake(uint256 poolIndex) external {
        ...
        totalDeposits -= users[msg.sender].deposits[poolIndex].stakedAmount;
        token.transfer(
            msg.sender,
            users[msg.sender].deposits[poolIndex].stakedAmount
        );
    ...
    function getLevelWiseIncome(address user, uint256 level)
        external
        view
        returns (uint256)
    {
    ...
    function getTotalPendingReward(address user)
        external
        view
        returns (uint256)
    {
    ...
    function withdrawERC20(address receiver, IERC20 _token) external onlyAdmin {
        token.transfer(receiver, _token.balanceOf(address(this)));
    }
