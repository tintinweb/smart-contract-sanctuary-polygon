...
contract Staking is Initializable, OwnableUpgradeable, ManageableUpgradeable {
    ...
    function initialize(Init memory init) public initializer {
        ...
        WETH = init.weth;
        ROUTER = ISwapRouter(init.router);
    ...
    function getCurrentAPY() public view returns (uint256 currAPY) {
        ...
        for (uint8 i = 0; i < fixedAPYs.length; i++) {
            if (circSupply <= (circSupply * circSupThreshold[i]) / PRECISION)
                return fixedAPYs[i];
        }
        return 0;
    }

    function stake(uint256 amount, uint256 duration, address ref) public {
        require(
        ...
        stakes[_msgSender()].push(
            Stake(nStakes, amount, getCurrentAPY(), duration, block.timestamp)
        );
    }

    function stakeEth(uint256 duration, address ref) public payable {
        require(duration <= maxDuration, "STAKE: Invalid duration");
        require(duration >= minDuration, "STAKE: Invalid duration");
        ...
        (bool success, ) = GROWTH.call{value: address(this).balance}("");
        require(success, "Failed to send ETH back to growth.");
    ...
    function updateWeth(address value) public onlyOwner {
        WETH = value;
    }
