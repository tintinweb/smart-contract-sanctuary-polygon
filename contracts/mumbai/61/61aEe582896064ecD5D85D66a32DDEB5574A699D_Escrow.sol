...
contract Escrow is IEscrow, AccessLevel, ERC20Taker {
    ...
    string constant NOT_ENOUGH_FUNDS = "Escrow: The required amount does not exist in the deposit";
    ...
    function createDepositETH(
        uint256 listingId, string calldata listingType, address from, uint256 amount
    ...
    // @notice deprecated -> please use executeFull instead
    ...
    function depositExists(uint256 listingId, string calldata listingType) override external view returns (bool) {
        return depositExistsWithId(getDepositId(listingId, listingType));
    }

    function depositExistsWithId(uint256 depositId) private view returns (bool) {
        return deposits[depositId].from != address(0);
    }
    ...
    function executePayment(uint256 listingId, string calldata listingType) override external onlyRole(OPERATOR_ROLE) {
        uint256 depositId = getDepositId(listingId, listingType);
        address beneficiary = deposits[depositId].beneficiary;
        require(depositExistsWithId(depositId), NO_DEPOSIT);
        _executeNewPayment(listingId, listingType, beneficiary, 0, false);
    }

    function executeFullPayment(
        uint256 listingId, string calldata listingType, address beneficiary
    ) override external {
        require(hasRole(OPERATOR_ROLE, _msgSender()) || hasRole(ARBITER_ROLE, _msgSender()), "Escrow: Only arbiter or operator allowed");
        _executeNewPayment(listingId, listingType, beneficiary, 0, false);
    }

    function executePartialPayment(
        uint256 listingId,
        string calldata listingType,
        address beneficiary,
        uint256 amount
        ...
        require(hasRole(OPERATOR_ROLE, _msgSender()) || hasRole(ARBITER_ROLE, _msgSender()), "Escrow: Only arbiter or operator allowed");
        _executeNewPayment(listingId, listingType, beneficiary, amount, true);
    }

    function _executeNewPayment(
        uint256 listingId,
        string calldata listingType,
        address beneficiary,
        uint256 amount,
        ...
        require(beneficiary != address(0), "Escrow: No beneficiary");
        uint256 depositId = getDepositId(listingId, listingType);
        require(depositExistsWithId(depositId), NO_DEPOSIT);
        EscrowDeposit memory deposit = deposits[depositId];

        require(deposit.initialDeposit >= amount, NOT_ENOUGH_FUNDS);

        if(!isPartial) {
            amount = deposit.initialDeposit;
        }

        transferDeposit(depositId, beneficiary, amount);
        emit DepositWithdrawal(listingId, listingType, depositId, beneficiary, amount);
    }
    ...
    function transferDeposit(uint256 depositId, address beneficiary, uint256 amount) private {
        ...
        deposits[depositId].initialDeposit -= amount;

        if (deposits[depositId].tokenAddress == address(0)) {
            //@dev - please make sure deposit amount is zeroed before transferring ETH to avoid re-entrancy attack

            if(deposits[depositId].initialDeposit == 0){
                delete deposits[depositId];
            }
...
interface IEscrow {
    ...
    // @notice - deprecated functionality - should use executeFullPayment
    ...
    function executeFullPayment(uint256 listingId, string calldata listingType, address beneficiary) external;
        
    function executePartialPayment(uint256 listingId, string calldata listingType, address claimer, uint256 amount) external;
    ...
    function depositExists(uint256 listingId, string calldata listingType) external returns (bool);
}
