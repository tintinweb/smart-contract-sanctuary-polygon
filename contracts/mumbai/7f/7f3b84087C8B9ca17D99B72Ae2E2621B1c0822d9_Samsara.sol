...
contract Samsara is Ownable {
  ...
  function isSubscribed(uint256 projectId, uint16 tierIndex, address donator) view public returns (bool) {
    ...
    return subscriberIndex(projectId, tierIndex, donator) < projects[projectId].subscriptions.length;
  }

  function subscriberIndex(uint256 projectId, uint32 tierIndex, address donator) public view returns (uint256 result) {
    require(projectId < projectCounter, 'Invalid project id');
