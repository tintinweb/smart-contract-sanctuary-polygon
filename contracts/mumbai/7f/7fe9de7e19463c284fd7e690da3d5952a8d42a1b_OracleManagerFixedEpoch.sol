...
contract OracleManagerFixedEpoch is IOracleManagerFixedEpoch {
  ...
  // uint256 public immutable deploymentTimeWithOffset;
  ...
  function getEpochEndTimestamp() public view returns (uint256) {
    return (getEpochStartTimestamp()) + EPOCH_LENGTH;
  }
  ...
  // TODO: make name of this function more descriptive and aligned!
  // TODO: should we not be passing around epoch indexes rather than timestamps?
  // TODO: make naming more clear by specifying renaming 'currentEpochTimestamp' to 'currentEpochStartTimestamp'
  ...
  // TODO: should we not be passing around epoch indexes rather than timestamps?
  // TODO: Document and improve naming of this function
  //        - eg this function reverts if you ask for a historical price update round
  // TODO: Can this function be removed? Was written as a function to use in the non-linear search version of this function.
  //       or for finding specific entries.
  ...
  function latestRoundID() external view returns (uint80 latestRoundId) {
    (latestRoundId, , , , ) = chainlinkOracle.latestRoundData();
  }
  ...
  function _shouldCurrentPriceUpdateExecuteEpoch(
    ...
    console2.log("");
    console2.log(">>>>>>>>>>_shouldCurrentPriceUpdateExecuteEpoch>>>>>>>>>>");
    ...
    bool one = _priceUpdateIsBeforeMinExecutionWaitThreshold(
      currentEpochStartTimestamp,
      previousOracleUpdateTimestamp
    );
    bool two = _priceUpdateIsBeforeMinExecutionWaitThreshold(
      currentEpochStartTimestamp,
      currentOracleUpdateTimestamp
    );
    console2.log(
      "_shouldCurrentPriceUpdateExecuteEpoch _priceUpdateIsBeforeMinExecutionWaitThreshold previous",
      one
    );
    console2.log(
      "_shouldCurrentPriceUpdateExecuteEpoch _priceUpdateIsBeforeMinExecutionWaitThreshold current",
      two
    );
    console2.log("<<<<<<<<<<_shouldCurrentPriceUpdateExecuteEpoch<<<<<<<<<<");
    return one && !two;
  ...
  function inMewt() public view returns (bool) {
    return block.timestamp < getEpochStartTimestamp() + MINIMUM_EXECUTION_WAIT_THRESHOLD;
  }

  function priceAtIndex(uint80 index) public view virtual returns (int256 price) {
  ...
  /// @notice It is "maximum" as this is just the upper
  ...
  function _getMaximumNumberOfMissedEpochs(
    ...
    console2.log("");
    console2.log(">>>>>>>>>>_getNumberOfMissedEpochs>>>>>>>>>>");
    uint256 _currentEpochIndex = getCurrentEpochIndex();
    console2.log("_getNumberOfMissedEpochs _currentEpochIndex", _currentEpochIndex);
    console2.log("_getNumberOfMissedEpochs _latestExecutedEpochIndex", _latestExecutedEpochIndex);

    uint256 _numberOfMissedEpochs = _currentEpochIndex - _latestExecutedEpochIndex - 1;
    ...
    if (
      _priceUpdateIsBeforeMinExecutionWaitThreshold(
        ...
        latestOraclePriceUpdateTime
      )
    ) {
      console2.log("_getNumberOfMissedEpochs inside if");
  ...
  struct MissedEpochExecution {
    bool _isSearchingForMissedEpochExecutionInfo;
    uint80 _currentOracleIndex;
  ...
  function getMissedEpochPriceUpdates(
  ...
  ) public view virtual returns (MissedEpochExecutionInfo[] memory) {
    (uint80 latestRoundId, , uint256 timestampReusableVariable, , ) = chainlinkOracle
      .latestRoundData();
    uint256 _numberOfMissedEpochs = _getMaximumNumberOfMissedEpochs(
      _latestExecutedEpochIndex,
      timestampReusableVariable
    );
    ...
    MissedEpochExecution memory _missedEpochExecution = MissedEpochExecution({
      _isSearchingForMissedEpochExecutionInfo: true,
      _currentOracleIndex: _previousOracleUpdateIndex + 1,
    ...
    timestampReusableVariable =
      (uint256(_latestExecutedEpochIndex) + 2) *
      EPOCH_LENGTH +
    ...
    (, , uint256 _previousOracleUpdateTimestamp, , ) = chainlinkOracle.getRoundData(
      _previousOracleUpdateIndex
    );

    while (
      _missedEpochExecution._isSearchingForMissedEpochExecutionInfo &&
      latestRoundId >= _missedEpochExecution._currentOracleIndex
    ) {
      (, int256 _currentOraclePrice, uint256 _currentOracleUpdateTimestamp, , ) = chainlinkOracle
        .getRoundData(_missedEpochExecution._currentOracleIndex);
      if (
        ...
        console2.log("getMissedEpochPriceUpdates while inside if");
        _missedEpochPriceUpdates[
          _missedEpochExecution._currentMissedEpochPriceUpdatesArrayIndex
        ] = MissedEpochExecutionInfo({
          oracleUpdateIndex: _missedEpochExecution._currentOracleIndex,
        ...
        timestampReusableVariable += uint32(EPOCH_LENGTH);

        // Check that we have retrieved all the missed epoch updates that we are searching
        // for and end the while loop
        if (
          _missedEpochExecution._currentMissedEpochPriceUpdatesArrayIndex == _numberOfMissedEpochs
        ) {
          _missedEpochExecution._isSearchingForMissedEpochExecutionInfo = false;
        }
      }
      ...
      _missedEpochExecution._currentOracleIndex++;
    }
  ...
  // TODO worst case if we miss tons of updates, this might run out of gas.
  // Need to have a clearing function
    ...
    )
  {
    (currentEpochTimestamp, numberOfEpochsSinceLastEpoch) = updateCurrentEpochTimestamp(
      _previousEpochEndTimestamp
    );

    // TODO possibly (may be worse) optimize by saving as state variable in epochInfo
    previousPrice = priceAtIndex(_previousExecutionPriceOracleIdentifier);

    _missedEpochPriceUpdates = getMissedEpochPriceUpdates(
      _latestExecutedEpochIndex,
      _previousExecutionPriceOracleIdentifier
    );
  }
}
