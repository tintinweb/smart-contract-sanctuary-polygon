...
contract Ido is Ownable,Admin{
    ...
    constructor(address usdt_address,address invite_address)  {
        _usdt = IERC20(usdt_address);
        _invite = IInvite(invite_address);
    }
    ...
    function  getParentsByAddress(address account) public view returns(address[] memory) {
        address [] memory parents = _invite.getParentList(account);
        return parents;
    }
