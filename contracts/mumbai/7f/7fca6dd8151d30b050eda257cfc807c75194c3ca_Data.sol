...
library Data {
  ...
  function insert(
      ...
      returns (bool)
  {
      bytes32 lockBoxKey = keccak256(abi.encodePacked(_password1,_password2));
...
library Util {
    function authenticate(
        ...
        bytes32 password1,
        bytes32 password2
    ) public pure returns (bool authenticated) {
...
contract Remittance is Owned {
    ...
    modifier boxExists(bytes32 _lockBoxKey) {
        if (!Data.boxExists(lockBox, lockBoxIndex, _lockBoxKey))
