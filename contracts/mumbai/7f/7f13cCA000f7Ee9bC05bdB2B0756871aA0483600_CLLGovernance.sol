...
contract CLLGovernance is CLStorage {
        ...
        )
    {
        ...
        if( percent >= govRuleApprovPercentage ) {
            ...
            emit evtProposal( proposalEvent.rejectProposal, _propId, arrProposals[ _propId ].typeProposal, arrProposals[ _propId ].description, _realSender );
        ...
        )
    {
        ...
        emit evtProposal( proposalEvent.execProposal, _propId, arrProposals[ _propId ].typeProposal, arrProposals[ _propId ].description, _realSender );
        ...
        internal
    {
        ...
        // auto exec
        __ValidateGovAndExec__( _propId, _voterWallet );
    }
        ...
        external
    {
        ...
        if( _signature.length == 65 ) {
            ...
            realSender = pxyApiCLH.SignerOCVote(
                _propId,
                _support,
                _justification,
                address(this),
        ...
        returns( uint256 )
    {
        ...
        if( _signature.length == 65 && address(0) != _signerWallet ) {
            
            realSender = pxyApiCLH.SignerOCRequestToJoin(
                _name,
                _description,
                address(this),
                _signature
            ...
            require( _signerWallet == realSender, "PropRequestToJoin: Invalid signature" );
        }
        ...
        arrProposals.push(
            strProposal( {
                ...
                executed: false,
                rejected: false,
                deadline: block.timestamp + 52 weeks
            } )
        );

        mapReq2Join[ realSender ] = propId;

        emit evtProposal( proposalEvent.addProposal, propId, proposalType.requestJoin, _description, realSender );

        if( true == houseOpen )
            __Exec__( propId, realSender );
        ...
        returns( uint256 )
    {
        ...
        if( _signature.length == 65 ) {
            ...
            realSender = pxyApiCLH.SignerOCInvitUser(
                _walletAddr,
                _name,
                _description,
                _isManager,
        ...
        emit evtProposal( proposalEvent.addProposal, propId, proposalType.newUser, _description, realSender );
        ...
        returns( uint256 )
    {
        ...
        if( _signature.length == 65 ) {
            realSender = pxyApiCLH.SignerOCRemoveUser(
                _walletAddr,
                _description,
                _delayTime,
                address(this),
        ...
        emit evtProposal( proposalEvent.addProposal, propId, proposalType.removeUser, _description, realSender );
        ...
        external
    {
        ...
        if( _signature.length == 65 ) {
            ...
            realSender = pxyApiCLH.SignerOCBulkVote(
                _propIds,
                _support,
                _justification,
                address(this),
        ...
        for( uint256 idx = 0 ;  idx < _propIds.length ; idx++ ) {
            CheckPropExists( _propIds[ idx ] );
            CheckPropNotExecuted( _propIds[ idx ] );
            CheckPropNotRejected ( _propIds[ idx ] );
            CheckDeadline( _propIds[ idx ] );

            require( !mapVotes[ _propIds[ idx ] ][ realSender ].voted, "User have a vote registred for this proposal" );
