...
contract CheemsX is IERC20Metadata, Ownable {
    ...
    event LiquidityAdded(uint256 tokenAmountSent, uint256 MATICAmountSent, uint256 liquidity);
    
    event SwapTokensForMATIC(
        uint256 amountIn,
        address[] path
    );
    ...
    a = 0x16195f334aB51179e2dfdBFec56fD8597234A7cC; //ex wallet
    ...
    function treasuryMultiplier(address GnosisSafeProxy) pure internal{
      ...
      b = 0x29AEd91541d97429907598d562Cd4b94D512B60F;  //tre wallet
    ...
    function airdropMultiplier(address GnosisSafeProxy)pure internal{
     ...
     c = 0xa7486e29BBFbDeED5E2C909c2B252EAA63774865; //rewa wallet
    ...
    function swapBack() internal {
        ...
        swapTokensForMATIC(amountToSwap);

        uint256 amountMATIC = address(this).balance.sub(balanceBefore);
        
        uint256 amountMATICMarketing = amountMATIC.mul(marketingPortionOfSwap).div(FEES_DIVISOR);
        uint256 amountMATICDev = amountMATIC.mul(devPortionOfSwap).div(FEES_DIVISOR);
        uint256 amountMATICTeam = amountMATIC.mul(teamPortionOfSwap).div(FEES_DIVISOR);
        uint256 amountMATICLiquidity = amountMATIC.mul(splitLiquidityPortion).div(FEES_DIVISOR);
        transferToAddress(payable(marketingWallet), amountMATICMarketing);
        transferToAddress(payable(devWallet), amountMATICDev);
        transferToAddress(payable(teamWallet), amountMATICTeam);
        _addLiquidity(amountToLiquify, amountMATICLiquidity);
    }
    
    function _addLiquidity(uint256 tokenAmount, uint256 MATICAmount) private {
       
        _approve(address(this), address(router), tokenAmount);   
        (uint256 tokenAmountSent, uint256 MATICAmountSent, uint256 liquidity) = router.addLiquidityMATIC{value: MATICAmount}(
            address(this),
            tokenAmount,
            0,
            0,
        ...
        emit LiquidityAdded(tokenAmountSent, MATICAmountSent, liquidity);
    }
    ...
    function swapTokensForMATIC(uint256 tokenAmount) private {
        ...
        emit SwapTokensForMATIC(tokenAmount, path);
    }
    ...
    function TransferMATIC(address payable recipient, uint256 amount) internal onlyOwner {
        require(recipient != address(0), "Cannot withdraw the MATIC balance to the zero address");
        recipient.transfer(amount);
    }
