...
contract ROIContract {
    ...
    //         uint level1ReferralAmount = (investmentAmount * referralLevel1Rate) / 100;
    //         referralAmount += level1ReferralAmount;
            
    //         address referrerLevel2 = referrals[referrer];
    //         if (referrerLevel2 != address(0)) {
    //             uint level2ReferralAmount = (investmentAmount * referralLevel2Rate) / 100;
    //             referralAmount += level2ReferralAmount;
                
    //             address referrerLevel3 = userReferral[referrerLevel2];
    //             if (referrerLevel3 != address(0)) {
    //                 uint level3ReferralAmount = (investmentAmount * referralLevel3Rate) / 100;
    ...
    //         uint256 activeDays = (DURATION - day);
            
    //         if (month == month2 && year == year2) {
    //             // Calculate profit based on the monthly percentage and active days
    ...
    for (uint256 j = 0; j < monthlyPercentages.length; j++) {
        ...
        // Check if the monthlyPercentage timestamp is in the same month as the investment start date
        if (month == month2 && year == year2) {
            // Calculate the active days in the current month
            uint256 activeDays = (day2 - day) / DURATION;
            if (activeDays > remainingDays) {
                activeDays = remainingDays; // Cap the active days to the remaining days in the month
            }
