    ...
    PausableUpgradeable
{
    ...
    function _createAuction(
        ...
        require(!_hasAuctionCreated(auction), "NFTAuctionV2: Auction exists");
        require(
            endTime >= 1 minutes,
            "NFTAuctionV2: Auction duration must be atleast 1 minutes"
        );
    ...
    function _bid(uint256 tokenId, uint256 bidAmount) internal {
        ...
        require(
            ...
            "NFTAuctionV2: The auction has not created"
        );
        require(
            _isAuctionActive(auction),
            "NFTAuctionV2: The Auction has ended"
        );
        require(
            msg.sender != auction.seller.seller,
            "NFTAuctionV2: Auction owner cannot bid"
        );
        require(
            auction.startingPrice <= bidAmount,
            "NFTAuctionV2: The amount should be greater than starting price"
        );
        require(
            auction.currentPrice < bidAmount,
            "NFTAuctionV2: The bid amount should be greater than the previous bid amount"
        );
    ...
    function _withdrawSellerCuts(uint256 tokenId) internal {
        ...
        require(
            _hasAuctionCreated(auction),
            "NFTAuctionV2: Auction does not exist"
        );
        require(
            auction.seller.seller == msg.sender,
            "NFTAuctionV2: Only Auction creator can call"
        );
        require(
            !_isAuctionActive(auction),
            "NFTAuctionV2: Auction has not ended yet"
        );
        require(
            !auction.seller.claimedCuts,
            "NFTAuctionV2: Seller has already withdrew cut"
        );
        require(
            auction.highestBidder != address(0),
            "NFTAuctionV2: The Auction does not have any bids"
        );

        (bool succeed, bytes memory data) = msg.sender.call{
            value: auction.currentPrice
        }("");
        require(succeed, "NFTAuctionV2: Failed to withdraw");
    }
    ...
    function claimNFT(uint256 tokenId) public {
        ...
        require(
            _hasAuctionCreated(auction),
            "NFTAuctionV2: Auction has not been created yet"
        );
        require(
            auction.highestBidder == msg.sender,
            "NFTAuctionV2: Winner can only claim NFT"
        );
        require(
            !_isAuctionActive(auction),
            "NFTAuctionV2: Auction has not ended"
        );
        require(
            !_isOwnerOf(msg.sender, tokenId),
            "NFTAuctionV2: Already claimed the NFT"
        );
    ...
    function setNFTInterface(address nftContract) external onlyOwner {
        require(nftContract != address(0), "NFTAuctionV2: Invalid Address");
    ...
    function withdrawBiddingAmount(uint256 tokenId) public {
        ...
        require(
            !_isAuctionActive(auction),
            "NFTAuctionV2: The auction is not ended yet"
        );
        require(
            auction.highestBidder != msg.sender,
            "NFTAuctionV2: Winner cannot withdraw"
        );
        ...
        for (uint256 i = 1; i < auction.bidderSize; i++) {
            if (auction.bidders[i].bidder == msg.sender) {
                require(
                    ...
                    "NFTAuctionV2: Already withdrew"
                );
                require(
                    auction.bidders[i].bidAmount < address(this).balance,
                    "NFTAuctionV2: Insufficient contract balance"
                );
                (bool succeed, bytes memory data) = msg.sender.call{
    ...
    function createAuction(
        ...
        require(
            ...
            "NFTAuctionV2: Only NFT owner can create auction"
        );
        ...
        )
    {
        ...
        require(
            _hasAuctionCreated(auction),
            "NFTAuctionV2: Auction not created"
        );
        return (
    ...
    function cancelAuction(uint256 tokenId) external {
        ...
        require(_isAuctionActive(auction), "NFTAuctionV2: Auction has ended");

        require(
            msg.sender == auction.seller.seller,
            "NFTauctionV2: Only auction creator can cancel the auction"
        );

        if (block.timestamp >= auction.startedAt) {
            require(
                auction.bidderSize == 0,
                "NFTauctionV2: Auction with bidders cannot be cancelled"
            );
