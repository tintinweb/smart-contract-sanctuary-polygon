...
contract HYIP is Ownable {
    ...
    function invest(GardenTier _pool, uint256 _amount) payable external {
        //investment in eth
        if(msg.value > 0) {
        ...
        //investment in usdt
            ...
            if(previousInvestment.amount == 0) {
                investment = Investment(block.timestamp, block.timestamp + pool.balanceLockedTime, 0, PaymentMode.usdt, amountInvested);
            }
            else {
                investment = Investment(previousInvestment.startTime, block.timestamp + pool.balanceLockedTime, previousInvestment.lastClaimTime, PaymentMode.usdt, amountInvested + previousInvestment.amount);
            }
