...
contract CryptoHunters is ERC721AQueryable, Ownable {
    ...
    string public baseUri =
        "https://storage.googleapis.com/crypto-hunters-test/";
    ...
    function ownerMint(uint256 quantity) external onlyOwner {
        require(
            ...
            "CryptoHunters: supply limit exceeded"
        );
    ...
    function mint(
        ...
        require(
            quantity > 0,
            "CryptoHunters: quantity should be greater then 0"
        );
        ...
        if (!isPublicMintEnabled) {
            ...
            require(
                signatureAddress == messageSigner,
                "CryptoHunters: invalid whitelist signature"
            );

            require(
                (whitelist[msg.sender] += quantity) <= whitelistMintLimit,
                "CryptoHunters: mint limit exceeded"
            );
        } else {
            require(
                publicMint[msg.sender] + quantity <= publicMintLimit,
                "CryptoHunters: mint limit exceeded"
            );
            publicMint[msg.sender] += quantity;
        }
        uint256 price = (isPublicMintEnabled ? publicPrice : whitelistPrice) *
            quantity;
