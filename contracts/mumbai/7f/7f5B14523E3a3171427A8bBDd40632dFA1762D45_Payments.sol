...
// File: piggy_new/PiggyBankNFT.sol
...
contract PiggyBankNFT is ERC721, Ownable, ERC721Burnable, ERC721URIStorage {
    ...
    event TokensLocked(address user, uint amount, uint lockingPeriod);
 ...
 */
    constructor(address _token) ERC721("PiggyBankNFT", "PiggyBankNFT") {
        token = PiggyToken(_token);
    }
    ...
    function setTiers(uint _tierId, uint _lockingPeriod) external onlyOwner {
        ...
        tiers[_tierId] = PiggyBankTier(_lockingPeriod);
    }
...
*/
    function lock(address _user, uint _amount, uint _nftId) external {
        ...
        emit TokensLocked(_user, _amount, tiers[_tierId].lockingPeriod);
    }
...
*/
    ...
    function cashOut(uint _tokenId) external {
        ...
        emit NFTCashedOut(_tokenId, msg.sender, depo.amount);
    }
...
contract Payments is Ownable, ERC721Holder {
    ...
    event JackpotNFTTransferred(address to, uint tokenId);
    event SpecialNFTTransferred(address to, uint tokenId);
...
[1,2,3,4,5][100,80,70,50,40][
*/

    function transferWinnerPayments(
        address[] memory _winnerAddress,
        uint[] memory _winnerAmount, //[10,20,10,10,10]
        uint _burnAmount, //20
        uint _treasuryAmount, //20
        address _treasuryAddress,
        address _piggyBankNFT,
        uint piggyBanklockAmount,//50
        uint[] memory _piggyBankNFTId //[1,25...]
    ) external onlyOwner {
        require(_winnerAddress.length == _winnerAmount.length, "Invalid array");

        token.burn(_burnAmount); //piggy token burn
        token.transfer(_treasuryAddress, _treasuryAmount);
        for (uint i = 0; i < _winnerAddress.length; i++) {
            token.transfer(_winnerAddress[i], _winnerAmount[i]);
            token.transfer(_piggyBankNFT, piggyBanklockAmount);
            piggyNFT.lock(
                _winnerAddress[i],
                piggyBanklockAmount,
                _piggyBankNFTId[i]
...
@param _jackpotTreasury address of jackpot nft contract
*/

    function transferUplinePayments  (
        address[] memory uplineAddress,
        uint[] memory uplineAmount,//[10,20,50,40]
        uint[] memory _piggyBankNFTId,
        uint[] memory _piggyBanklockAmount,//[1,20,30..]
        address _piggyBankNFT,
        ...
        for (uint i = 0; i < uplineAddress.length; i++) {
            if (isActive[i]) {
                token.transfer(
                    uplineAddress[i],
                    uplineAmount[i] - _piggyBanklockAmount[i]
                );

                token.transfer(_piggyBankNFT, _piggyBanklockAmount[i]);
                piggyNFT.lock(
                    uplineAddress[i],
                    _piggyBanklockAmount[i],
                    _piggyBankNFTId[i]
                );
            } else {
                token.transfer(_jackpotTreasury, uplineAmount[i]);
            }
        }
    }
    ...
    function transferSpecialNFT(
        uint _tokenId,
        address _winner
    ) public onlyOwner {
        specialNFT.transferFrom(msg.sender, _winner, _tokenId);
        emit SpecialNFTTransferred(_winner, _tokenId);
    }

    /**
@dev Function to transfer funds to special NFT
@param _specialNFT The addresses of the NFT
@param _lockingAmount The amount of tokens to be locked in special NFT
...
*/
    ...
    function transferToSpecialNFT(
        ...
        token.transfer(_specialNFT, _lockingAmount);
        specialNFT.lock(msg.sender, _lockingAmount, _nftID, _lockingPeriod);
    }
