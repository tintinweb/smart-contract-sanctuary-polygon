...
contract ERC721r is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function _mintRandom(address to/*, uint _numToMint*/) internal virtual {
        ...
        require(_mintPause == false, "Random minting is paused");
    ...
    function _unpauseRandomMint() internal virtual {
        require(_mintPause == true, "Mint is already unpaused");
        _mintPause = false;
        _numAvailableTokensSubCollection = _subCollectionSupply;
    ...
    function _mintAtIndex(address to, uint index) internal virtual {
        require(_mintPause == false, "Minting is paused");
        require(_msgSender() == tx.origin, "Contracts cannot mint");
        require(to != address(0), "ERC721: mint to the zero address");
...
contract WUBI is ERC721rEnumerable, Ownable {
    ...
    /*function addAirdropAddressList(address[] memory _addressList) external onlyOwner {
        require(_addressList.length == SUBCOLLECTION_SUPPLY,"Address list too short");
        uint256 i;
        for(i = 1; i <= SUBCOLLECTION_SUPPLY; i++) {
            airdropAddressList[i] = _addressList[i];
        }
    }*/

    /*function airdrop() external onlyOwner {
        for(uint256 i = SUBCOLLECTION_SUPPLY; i > 0; i--) {
            address addressToAirdrop = airdropAddressList[i];
            _mintAtIndex(addressToAirdrop, i);
        }
    }*/
