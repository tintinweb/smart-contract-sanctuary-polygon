...
contract Project {
    ...
    event ContributorRefunded(address contributor, uint256 amount);

    // Modifier to check current state
    modifier inState(State _state) {
        require(
            uint256(state) == uint256(_state),
            "Project is not in the correct state"
        );
        _;
    }
    ...
    function contribute() external payable inState(State.Fundraising) {
        require(msg.sender != creator, "Cannot contribute to your own project");
