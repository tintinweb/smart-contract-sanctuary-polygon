...
// File contracts/AcceptedToken.sol
...
// File contracts/KhizaTokenVendor.sol
...
contract KhizaTokenVendor is Ownable, ReentrancyGuard {
    ...
    function setTokenToSellDecimals(uint8 _tokenToSellDecimals) public onlyOwner {
        tokenToSellDecimals = _tokenToSellDecimals;
    }
    ...
    function valueToPay(uint amountToBuy) public view returns (uint amountToPay) {
        amountToPay = (amountToBuy * pricePerToken) / (10 ** tokenToSellDecimals);
    }
    ...
    function buyTokens(uint amountToBuy) public nonReentrant notPaused whitelistCompliance(amountToBuy) timeCompliance {
        ...
        uint amountToPay = (amountToBuy * pricePerToken) / (10 ** tokenToSellDecimals);
        require(amountToPay > 0, 'NoFreeLunch');
        ...
        uint currentAllowance = tokenToPay.allowance(_msgSender(), address(this));
        ...
        uint currentBuyerBalance = tokenToPay.balanceOf(_msgSender());
        ...
        tokenToPay.safeTransferFrom(_msgSender(), owner(), amountToPay);

        // Transfer token to the msg.sender
        tokenToSell.safeTransfer(_msgSender(), amountToBuy);
    ...
    event WhitelistAdded(address _address, uint _limit);
    ...
    function setWhitelistAddress(address _address, uint _limit) onlyOwner public {
        emit WhitelistAdded(_address, _limit);
        whitelistWithLimits[_address] = _limit;
    }

    function setMultiWhitelist(address[] memory _addresses, uint[] memory _limits) onlyOwner public {
        for (uint i=0; i<_addresses.length; i++) {
            setWhitelistAddress(_addresses[i], _limits[i]);
        }
    }
