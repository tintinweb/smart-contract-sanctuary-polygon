...
contract Reward is Initializable, AccessControlUpgradeable, UUPSUpgradeable {
   ...
   * @param vehicleNodeId The node id minted by the registry contract for vehicles
   * @param sanctionsContractAddress The contract address for chainalysis sanctions oracle
   */
  function initialize(
    address tokenAddress,
    address registryContractAddress,
    uint256 vehicleNodeId,
    ...
    vehicleNodeType = vehicleNodeId;
    DIMORegistryContract = IDIMORegistry(registryContractAddress);
  ...
  function batchTransfer(
    ...
    for (uint256 i = 0; i < arrayLength; i++) {
      vehicleOwner = DIMORegistryContract.ownerOf(vehicleIds[i]);
      ...
      if (
        ...
        vehicleNodeType == DIMORegistryContract.getNodeType(vehicleIds[i])
      ) {
