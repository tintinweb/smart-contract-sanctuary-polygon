...
abstract contract ERC721Time is Context, ERC165, IERC721Time, IERC721Metadata {
    ...
    function __ERC721_Init_Constructor(string memory name, string memory symbol) internal {
        _name = name;
        _symbol = symbol;
    }
    ...
    function tokenDataOf(
        uint256 tokenId
    ) public view virtual override returns (IERC721Time.TokenData memory) {
...
abstract contract OxtripMultiState {
    DataTypes.ProtocolState private _state;
    DataTypes.WhitelistedState private _whiteState;
     ...
     *      2: Paused
     */
    function getWhiteState() external view returns (DataTypes.WhitelistedState) {
        return _whiteState;
    }

    function _getWhiteState() internal view returns (DataTypes.WhitelistedState) {
        return _whiteState;
    }

    function _setWhiteState(DataTypes.WhitelistedState newState) internal {
        // DataTypes.WhitelistedState prevState = _whiteState;
        _whiteState = newState;
...
import {IOxtripNFTBase} from '../../interfaces/IOxtripNFTBase.sol';
import {Errors} from '../../libraries/Errors.sol';
import {DataTypes} from '../../libraries/DataTypes.sol';
import {Events} from '../../libraries/Events.sol';
...
abstract contract OxtripNFTBase is ERC721Enumerable, IOxtripNFTBase {
    ...
    function _initialize_constructor(string memory name, string memory symbol) internal {
        ERC721Time.__ERC721_Init_Constructor(name, symbol);
        (name, symbol);
        emit Events.BaseInitialized(name, symbol, block.timestamp);
    }
...
import {OxtripNFTBase} from './base/OxtripNFTBase.sol';
import {OxtripMultiState} from './base/OxtripMultiState.sol';
import {OxtripHubStorage} from './storage/OxtripHubStorage.sol';
import {VersionedInitializable} from '../upgradeability/VersionedInitializable.sol';
import {IERC721Enumerable} from '@openzeppelin/contracts/token/ERC721/extensions/IERC721Enumerable.sol';
import {IPostNFT} from '../interfaces/IPostNFT.sol';
import {MetaTxLib} from '../libraries/MetaTxLib.sol';

contract OxtripHub is OxtripNFTBase, VersionedInitializable, OxtripMultiState, OxtripHubStorage, IOxtripHub {
    uint256 internal constant REVISION = 3;

    address internal immutable FOLLOW_NFT_IMPL;
    address internal immutable COLLECT_NFT_IMPL;
    address internal immutable POST_NFT_IMPL;
    ...
    constructor(address followNFTImpl, address collectNFTImpl, address postNFTImpl) {
        if (followNFTImpl == address(0)) revert Errors.InitParamsInvalid();
        if (collectNFTImpl == address(0)) revert Errors.InitParamsInvalid();
        if (postNFTImpl == address(0)) revert Errors.InitParamsInvalid();

        FOLLOW_NFT_IMPL = followNFTImpl;
        COLLECT_NFT_IMPL = collectNFTImpl;
        POST_NFT_IMPL = postNFTImpl;
    }
    ...
    function initialize(
        ...
        _setWhiteState(DataTypes.WhitelistedState.UnEnable);
    ...
    function setWhiteState(DataTypes.WhitelistedState newState) external override {
        if (msg.sender != _governance) {
            revert Errors.NotGovernanceOrEmergencyAdmin();
        }
        _setWhiteState(newState);
    }
    ...
    function createProfile(
        ...
        if (!IOxtripHub(address(this)).isProfileCreatorWhitelisted(msg.sender))
    ...
    function isProfileCreatorWhitelisted(
        ...
        if (_getWhiteState() == DataTypes.WhitelistedState.UnEnable) {
            return true;
        }
    ...
    function getPostContentURI(uint256 pubId) external view override returns (string memory) {
        return _postById[pubId].contentURI;
    }
    ...
    function _createPost(
        ...
        unchecked {
            ...
            address postAddress = ownerOf(profileId);

            if (POST_NFT_IMPL != address(0)) {
                //mint
                uint256 postId = ++_postCounter;
                IPostNFT(POST_NFT_IMPL).mint(postAddress, postId);
                _postById[postId].contentURI = contentURI;
                _pubByIdByProfile[profileId][pubId].postNftId = postId;
            }
...
/**
 * @title OxtripHubStorage
 * @author  Oxtrip Protocol
 *
 * @notice This is an abstract contract that *only* contains storage for the OxtripHub contract. This
 * *must* be inherited last (bar interfaces) in order to preserve the OxtripHub storage layout. Adding
 * storage variables should be done solely at the bottom of this contract.
 */
abstract contract OxtripHubStorage {
    ...
    uint256 internal _postCounter;
    //帖子对应的content uri
    mapping(uint256 => DataTypes.PostNFTData) internal _postById;
}
 ...
 * @author  Oxtrip Protocol
 *
 * @notice This is the standard interface for all  Oxtrip-compatible CollectModules.
 */
interface ICollectModule {
     ...
     * @param pubId The associated publication's OxtripHub publication ID.
     ...
     *
     * @param referrerProfileId The OxtripHub profile token ID of the referrer's profile (only different in case of mirrors).
     * @param collector The collector address.
     * @param profileId The token ID of the profile associated with the publication being collected.
     * @param pubId The OxtripHub publication ID associated with the publication being collected.
...
interface ICollectNFT {
     ...
     * and initializing the name and symbol in the OxtripNFTBase contract.
     *
 ...
 * @author  Oxtrip Protocol
 *
 * @notice This is the standard interface for all  Oxtrip-compatible FollowModules.
 */
interface IFollowModule {
    /**
     * @notice Initializes a follow module for a given  Oxtrip profile. This can only be called by the hub contract.
     *
    ...
    /**
     * @notice Processes a given follow, this can only be called from the OxtripHub contract.
     *
     ...
     * only be called from the OxtripHub contract.
     *
...
/**
 * @title IOxtripNFTBase
 * @author  Oxtrip Protocol
 *
 * @notice This is the interface for the OxtripNFTBase contract, from which all  Oxtrip NFTs inherit.
 ...
 * @author  Oxtrip Protocol
 *
 * @notice This is the interface for the IPostNFT contract.
 */
interface IPostNFT {
    /**
     * @notice Initializes the collect NFT, setting the feed as the privileged minter, storing the collected publication pointer
     ...
     * @return uint256 An interger representing the minted token ID.
     */
    function mint(address to, uint256 pubId) external returns (uint256);
}
 ...
 * @author  Oxtrip Protocol
 *
 * @notice This is the standard interface for all  Oxtrip-compatible ReferenceModules.
 */
...
library Events {
    ...
    event PostNFTInitialized(uint256 timestamp);
    // Module-Specific
     ...
     *
     * @param hub The OxtripHub contract address used.
     * @param timestamp The current block timestamp.
     */
    ...
    /**
     * @dev Emitted when the user wants to enable or disable follows in the `OxtripPeriphery`.
     *
    ...
    /**
     * @dev Emitted when the metadata associated with a profile is set in the `OxtripPeriphery`.
     *
...
library ProfileTokenURILogic {
    ...
    function getProfileTokenURI(
        ...
        string memory imageData = _getSVGImageBase64Encoded(handleWithAtSymbol, imageURI);
        return
            string(
                abi.encodePacked(
                    ...
                    Base64.encode(
                        abi.encodePacked(
                            ...
                            '","description":"',
                            handleWithAtSymbol,
                            ' - 0xtrip Protocol Profile","image":"data:image/svg+xml;base64,',
                            imageData,
        ...
        return
            Base64.encode(
                abi.encodePacked(
                    ...
                    handleWithAtSymbol,
                    '</text><rect id="background-border" x="2.5" y="2.5" width="444" height="444" rx="13" stroke="url(#rounded-border-transparency-detail)" stroke-width="5"/></g></g></svg>'
                )
            );
    }
...
contract FollowNFTProxy is Proxy {
    ...
    constructor(bytes memory data) {
        HUB = msg.sender;
        IOxtripHub(msg.sender).getFollowNFTImpl().functionDelegateCall(data);
    }

    function _implementation() internal view override returns (address) {
        return IOxtripHub(HUB).getFollowNFTImpl();
    }
}
 ...
 *
 * @author  Oxtrip Protocol, inspired by Aave's implementation, which is in turn inspired by OpenZeppelin's
