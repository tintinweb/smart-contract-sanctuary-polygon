...
contract TokenSale is Initializable, OwnableUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function buy(address _token, uint256 _amount, address referrer) public returns (uint256) {
        ...
        if (referrer != address(0) && stakers[referrer].amount >= 100_000 * 10 ** token.decimals()) {
    ...
    function sell(address _token, uint256 amount) public returns (uint256) {
        require(price[_token] > 0, "Invalid token");
    ...
    function stake(uint256 amount) public {
        ...
        totalRewardRate = uint16(
            (totalStakedAmount.mul(totalRewardRate).add(stakerData.amount.mul(stakerData.rewardRate))).div(
    ...
    function calcReward(address account) public view returns (uint256) {
        ...
        uint256 diffDays = (block.timestamp.sub(stakerData.lastClaimedAt)).div(SECONDS_PER_DAY);
        return
            stakerData
                .reward
                .add(MathUpgradeable.mulDiv(stakerData.amount, stakerData.rewardRate, DECIMALS).mul(diffDays).div(30))
