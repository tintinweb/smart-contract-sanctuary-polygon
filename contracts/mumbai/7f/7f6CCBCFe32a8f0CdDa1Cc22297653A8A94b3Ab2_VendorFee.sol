...
contract VendorFee is Ownable {
    TokenERC20 yourToken;

    uint256 public tokensPerMatic = 10000; //10,000
    uint public fee = 3; // 3%

    constructor(address tokenAddress) {
        yourToken = TokenERC20(tokenAddress);
    }
    ...
    function buyTokens() public payable returns (uint256 tokenAmount) {
        ...
        require(vendorBalance >= amountToBuy, "Vendor contract doesn't have ");
...
uint256 feeAmount = (amountOfMATICToTransfer * fee) / 100; // คำนวณค่า fee
amountOfMATICToTransfer -= feeAmount; // ลบค่า fee ออกจากค่าเงินที่จะโอน
uint256 ownerMATICBalance = address(this).balance;
require(ownerMATICBalance >= amountOfMATICToTransfer, "Vendor doesn't have ");
