...
abstract contract ReliefDAOBuilder is AbstractDynamicURI, BaseViciContract {
    ...
    function buildDefs() internal virtual view returns (string memory) {
...
// File: PodcastSpotlightBuilder.sol

/**
 * @title Podcast Spotlight DAO Dynamic Content Contract
 */
contract PodcastSpotlightBuilder is ReliefDAOBuilder {
    ...
    function logo() internal view override returns (string memory) {
        return
            svg.g(
                svg.prop("id", "full-logo"),
                string.concat(
                    svg.g(
                        ...
                        string.concat(
                            svg.path(logoElement.dPath, 'id="logo"'),
                            svg.path(logoElement.aPath, 'class="mic"'),
                            svg.path(logoElement.oPath, 'class="mic"'),
                            getExtraLogoPaths()
                        )
                    ),
                    svg.text(
    ...
    function _initDefaults() internal override {
        regularURL = "https://podcastdao.io/";
        nominationURL = "https://podcastdao.io/nominate";

        gradientStart = "#2b51bc";
        gradientStop = "#c1498a";

        logoElement
            .dPath = "M51.75,18.094c-1.484-3.722-3.568-6.92-6.255-9.593-2.687-2.672-5.919-4.758-9.698-6.255-3.778-1.497-7.977-2.246-12.595-2.246H.07V60.706H23.202c4.618,0,8.816-.741,12.595-2.225,3.778-1.483,7.011-3.568,9.698-6.255,2.687-2.687,4.771-5.891,6.255-9.614,1.483-3.722,2.225-7.809,2.225-12.259s-.742-8.536-2.225-12.259Zm-10.684,21.222c-.882,2.645-2.156,4.885-3.82,6.717-1.665,1.833-3.681,3.24-6.045,4.219-2.365,.98-5.031,1.469-7.997,1.469H11.405V8.984h11.797c2.966,0,5.632,.49,7.997,1.469,2.365,.98,4.38,2.386,6.045,4.219,1.665,1.834,2.939,4.072,3.82,6.717,.882,2.645,1.322,5.633,1.322,8.963s-.441,6.318-1.322,8.963ZM77.052,0l-23.888,60.706h8.816c.951,0,1.749-.266,2.393-.798,.643-.531,1.077-1.119,1.301-1.763l4.534-12.385h25.147l4.534,12.385c.308,.728,.769,1.337,1.385,1.826,.615,.49,1.413,.735,2.393,.735h8.732L88.555,0h-11.503Zm-3.904,37.784l7.347-20.109c.363-.924,.741-2.008,1.134-3.254,.392-1.245,.769-2.582,1.134-4.009,.391,1.427,.783,2.757,1.175,3.988,.392,1.232,.769,2.295,1.134,3.191l7.389,20.193h-19.312Zm97.338-7.513c0,16.563-13.427,29.99-29.991,29.99s-29.99-13.427-29.99-29.99S123.932,.28,140.495,.28s29.991,13.427,29.991,29.99Z";
        logoElement
            .aPath = "M140.153,16.661h0c2.196,0,3.993,2.035,3.993,4.522v11.249c0,2.487-1.797,4.522-3.993,4.522h0c-2.196,0-3.993-2.035-3.993-4.522v-11.249c0-2.487,1.797-4.522,3.993-4.522Z";
        logoElement
            .oPath = "M147.34,26.933v6.197c0,2.058-.811,3.931-2.119,5.291l-.002,.002h-.001c-1.091,1.134-2.525,1.908-4.12,2.134v2.366h3.08c.454,0,.826,.386,.826,.858h0c0,.472-.372,.858-.826,.858h-8.106c-.454,0-.826-.386-.826-.858h0c0-.472,.372-.858,.826-.858h3.079v-2.366c-1.595-.226-3.031-1.001-4.122-2.134h-.001c-1.307-1.356-2.119-3.23-2.119-5.292v-6.197h1.533v6.197c0,1.623,.64,3.099,1.67,4.168v.003c1.028,1.068,2.449,1.731,4.013,1.731s2.985-.663,4.013-1.731l.003-.003c1.028-1.068,1.667-2.543,1.667-4.168v-6.197h1.533Z";
        logoElement.logoTransform = "translate(375 150) scale(2)";
        logoElement.logoTextY = "330";
        logoElement.logoTextClass = "logo";
        logoElement.logoText = "Podcast Spotlight";
        ...
        noNomElement.nomAskLinkText = "https://podcastdao.io/nominate";
        noNomElement.nomAskSuffix = "";
        noNomElement.nomAskLine2 = "to nominate a podcast";
