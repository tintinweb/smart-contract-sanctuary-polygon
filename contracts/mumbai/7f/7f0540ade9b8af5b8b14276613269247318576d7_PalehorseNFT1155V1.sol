...
contract ERC1155 is Context, ERC165, IERC1155, IERC1155MetadataURI {
    ...
    */    
    ...
    function updateTokenSupply( uint256 _supply, uint256 _tokenId  , address account)  internal {
        _balances[_tokenId][account]=  _balances[_tokenId][account]+_supply;
    }
    /**
    ...
    */
    ...
    function _mint(address tokenOwner,uint256 tokenId, uint256 _supply, string memory _uri, uint256 _fee) internal {
        require(!_exists(tokenId), "ERC1155: token already minted");
        require(_supply != 0, "Supply should be positive");
        require(bytes(_uri).length > 0, "uri should be set");

        creators[tokenId] = tokenOwner;
        _tokenOwners.set(tokenId,tokenOwner);
        _royaltyFee[tokenId] = _fee;
        _balances[tokenId][tokenOwner] = _supply;
        _setTokenURI(tokenId, _uri);

        emit TransferSingle(tokenOwner, address(0x0),tokenOwner, tokenId, _supply);
...
contract PalehorseNFT1155V1 is ERC1155 {
...
// Removing functionality of whitelist users
    ...
    function LazyMint(address payable creator, NFTVoucher calldata voucher, uint256 fee, uint supply)
        public
        payable
        returns (uint256)
    {
        ...
        require(msg.value >= (voucher.minPrice) * supply, "Insufficient funds to redeem");
        require(
            voucher.royality <= maximumRoyality,
            "royality of creater is too high"
        );
      // first assign the token to the signer, to establish provenance on-chain  
      //    _mint(creator,tokenCounter, supply, voucher.tokenUri, fee);
    
      
    //   To create multiple copies of same tokenid on same nonce
       if(nonceToTokenId[voucher.vNonce] == 0) 
        {
            _mint(msg.sender, tokenCounter, supply, voucher.tokenUri, fee);
        nonceToTokenId[voucher.vNonce] = tokenCounter;
        ...
        {
            _mint(msg.sender, nonceToTokenId[voucher.vNonce], supply, voucher.tokenUri, fee);
        creator.transfer(msg.value);
        return nonceToTokenId[voucher.vNonce];
        }
      

        // _setTokenURI(newItemId, voucher.tokenUri);
        // _doSafeBatchTransferAcceptanceCheck(operator, from, to, newItemId, amount, data);
        // _transfer(creator, msg.sender, tokenCounter, supply, "0x");
