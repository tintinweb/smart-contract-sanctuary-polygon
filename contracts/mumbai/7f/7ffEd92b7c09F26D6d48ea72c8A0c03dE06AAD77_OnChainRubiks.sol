...
contract OnChainRubiks is 
    ...
    modifier isNotations(string memory _notation ) {
        require(GetOCR.notationsCheck(_notation)==true, "invalid notations!");
        _;
    }

    function _mintRubik(string memory _name, string memory _scramble) public mintVerify(1, false) isNotations(_scramble) payable {
        string memory dna = GetOCR.getDna(_scramble);
        require(isDna[dna].exist!=true, "Dna exists!");
        rubiks[_tokenIdTracker.current()] = Rubik(string(abi.encodePacked("oCR#",Strings.toString(_tokenIdTracker.current())," ", _name)), _scramble, dna, _msgSender());
        isDna[dna] = Dna(true);
    ...
    function _mintRandom(uint8  _itemsamount, uint8  _notationsamount) public mintVerify(_itemsamount, true) payable {
        for (uint i=0; i < _itemsamount; i++) {
            string memory _scramble = GetOCR.randomNotations(_tokenIdTracker.current(), i, _notationsamount);
            string memory dna = GetOCR.getDna(_scramble);
            require(isDna[dna].exist!=true, "Dna exists!");            
...
library GetOCR {
    function notationsCheck(string memory _notation) public pure returns (bool) {
    ...
    function getRubiks(uint256 _tokenId, string[2][54] memory getnewposes) public pure returns (string memory) {
        ...
        //string[2][54] memory getnewposes = BaseRubiks.scramble(rubiks[_tokenId].scramble);        
...
library BaseRubiks {
    ...
    function permut(bytes32 _key ) public pure returns (uint8[4][5] memory) {
        ...
        //		           👇️ face permut cw         👇️ face permut ccw
        ...
        //		           👇️ center permut cw       👇️ center permut ccw
