    ...
    PullPaymentUpgradeable
{
    ...
    function permitBurnItems(
        ...
        address claimedOwner = verifier.recoverSigner(messageHash, signature);

        _setItemUsage(deduplicationId, maxUsage);
        _burnItems(nftItems, claimedOwner);
        feeDistributor.distributeFees{ value: msg.value }(fees);
    ...
    function _burnItems(NFTItem[] calldata nftItems, address claimedOwner) private {
        ...
        for (uint256 i = 0; i < nftItemCount; ) {
            ...
            if (claimedOwner != address(0)) {
                address currentOwner = INFT(nftItem.collection).ownerOf(nftItem.tokenId);
...
interface IVerifier {
    ...
    function verifySigner(bytes32 hash, bytes memory signature) external returns (bool);
