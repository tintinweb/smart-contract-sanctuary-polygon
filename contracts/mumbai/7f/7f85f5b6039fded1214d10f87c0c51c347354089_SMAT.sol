...
contract SMAT is ERC721A,  Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        uint256 _priceWhitelist) ERC721A ("Smat token", "SMAT") {
        setBaseURI(baseURI);
    ...
    modifier isMaxMintsPerTXExceeded(uint amount) {
        require(addressToMintCount[msg.sender] + amount <= maxMintsPerTX, "Exceeds max mint per tx!");
        _;
    }
    ...
    function mintWhitelist(bytes32[] calldata merkleProof, uint256 quantity) public payable 
        ...
        isMaxMintsPerTXExceeded(quantity)
        nonReentrant {
