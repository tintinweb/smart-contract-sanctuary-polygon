...
contract Derivswap {
    ...
    mapping(address => uint256) public protocol_earnings_by_asset;
    ...
    address[] public whitelisted_coins = [
    ...
    function owner_whitelist_collateral(address _tokenAddress) public{
        require(msg.sender == owner);
        whitelisted_collateral[_tokenAddress] = true;
        whitelisted_coins.push(_tokenAddress);
        protocol_earnings_by_asset[_tokenAddress] = 0;
    }
    ...
    function whitelisted_collateral_list() external view returns(address[] memory){
        address[] memory collateral_list;
        for(uint x=0; x<whitelisted_coins.length;x++){
            collateral_list[x] = whitelisted_coins[x];
        }
...
import "./Payout.sol";
import './Derivswap.sol';
contract Orderbook {
    IPyth pyth;
    Derivswap public vault; 
    ...
    function owner_set_hedgex(address DRVS_VAULT_ADDRESS) public {
        ...
        vault = Derivswap(payable(DRVS_VAULT_ADDRESS));
    }
    ...
    function fetchFeedLiquidityAllCollateral(bytes32[] memory ids) public view returns(uint256[][] memory){
        uint256[][] memory volumes = new uint256[][](ids.length);
        address[] memory collateral_list = vault.whitelisted_collateral_list();
        for (uint256 i = 0; i < ids.length; i++) {
            volumes[i] = new uint256[](collateral_list.length);
            for(uint x=0; x<collateral_list.length; x++){
                volumes[i][x] = feedBidLiquidityByAsset[ids[i]][collateral_list[x]];
            }
        }
        return (volumes);
    }
