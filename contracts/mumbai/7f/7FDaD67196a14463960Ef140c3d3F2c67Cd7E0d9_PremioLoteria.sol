...
contract PremioLoteria is VRFConsumerBase, Ownable{
    ...
    function prepare(address[] memory array) public onlyOwner{
   ...
   */
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal virtual override  {
        ...
        uint256 secondPrizeWinnerIndex;
        uint256 thirdPrizeWinnerIndex;
        if(firstPrizeWinnerIndex > 0 && firstPrizeWinnerIndex < players.length - 1){
            secondPrizeWinnerIndex = firstPrizeWinnerIndex + 1;
            thirdPrizeWinnerIndex = firstPrizeWinnerIndex - 1;
        }else if(firstPrizeWinnerIndex == 0){
            secondPrizeWinnerIndex = firstPrizeWinnerIndex + 1;
            thirdPrizeWinnerIndex = firstPrizeWinnerIndex + 2;
        }else{
            secondPrizeWinnerIndex = firstPrizeWinnerIndex - 1;
            thirdPrizeWinnerIndex = firstPrizeWinnerIndex - 2;
        }
        emitFirstPrizeWinner(firstPrizeWinnerIndex);
        emitSecondPrizeWinner(secondPrizeWinnerIndex);
        emitThirdPrizeWinner(thirdPrizeWinnerIndex);
    }

    function emitFirstPrizeWinner(uint256 index) private{
        address winner = players[index];
        emit FirstPrizeWinner(winner);
    }

    function emitSecondPrizeWinner(uint256 index) private{
        address winner = players[index];
        emit SecondPrizeWinner(winner);
    }

    function emitThirdPrizeWinner(uint256 index) private{
        address winner = players[index];
