...
contract Marriage is ERC721URIStorage {
    ...
    uint private _startTime = 1648821600;
    uint private _endTime = 1648908000;

    mapping(string=>string) public _vowsByBothParties;
    
    constructor(string memory _husband, string memory _wife) ERC721("M", "Marriage") {
        _vowsByBothParties[husband] = _husband;
        _vowsByBothParties[wife] = _wife;

    }

    function sendRing(string memory tokenURI) external {
        require(_tokenIds.current() < 1, "cannot mint another ring");
        _tokenIds.increment();
         uint256 newItemId = _tokenIds.current();
         address wifeAddress = 0x040506C383b4342F613279a83A347dD443984232;
        _mint(wifeAddress, newItemId);
