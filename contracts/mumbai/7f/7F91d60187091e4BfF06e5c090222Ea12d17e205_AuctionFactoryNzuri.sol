...
import "./AuctionNzuri.sol";


contract AuctionFactoryNzuri {
  ...
  external
  {
    ...
    AuctionNzuri newAuction = new AuctionNzuri(
      controller,
      msg.sender,
      acceptERC20,
      isErc1155,
      tokenId,
      quantity,
      address(erc20NzuriContract),
      address(erc721NzuriContract),
      address(erc1155NzuriContract)
    );
    ...
    if (isErc1155) {
      erc1155NzuriContract.safeTransferFrom(msg.sender, address(newAuction), tokenId, quantity, "");
    } else {
      erc721NzuriContract.safeTransferFrom(msg.sender, address(newAuction), tokenId);
    }
  ...
  external
  {
    AuctionNzuri auction = AuctionNzuri(auctionAddress);
  ...
  external
  {
    ...
    require(erc20NzuriContract.transferFrom(msg.sender, auctionAddress, amount));
    auction.placeBid(msg.sender, totalBid);
  ...
  public
  {
    require(feeRate <= 100, "AuctionFactoryNzuri: feeRate mustn't be greater than 100");
    feeRate = _feeRate;
  }
...
contract AuctionNzuri is IERC721Receiver, IERC1155Receiver {
  modifier onlyOwner {
    require(msg.sender == owner);
    _;
  }
  ...
  IERC20 erc20NzuriContract;
  IERC721 erc721NzuriContract;
  IERC1155 erc1155NzuriContract;
  ...
  constructor(
    ...
    uint _tokenId,
    uint _quantity,
    address erc20NzuriAddress,
    address erc721NzuriAddress,
    address erc1155NzuriAddress
  ) {
    owner = msg.sender;
    controller = _controller;
    ...
    if (acceptERC20) {
      erc20NzuriContract = IERC20(erc20NzuriAddress);
    }

    if (isErc1155) {
      erc1155NzuriContract = IERC1155(erc1155NzuriAddress);
    } else {
      erc721NzuriContract = IERC721(erc721NzuriAddress);
    }
  }
  ...
  external
  {
    ...
    // send the funds
    if (acceptERC20) {
      require(erc20NzuriContract.transfer(claimer, withdrawalAmount));
    } else {
  ...
  {
    if (isErc1155) {
      erc1155NzuriContract.safeTransferFrom(address(this), claimer, tokenId, quantity, "");
    } else {
      erc721NzuriContract.safeTransferFrom(address(this), claimer, tokenId);
    }
