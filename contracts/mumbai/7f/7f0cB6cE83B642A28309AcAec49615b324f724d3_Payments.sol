...
// 0x7f0cB6cE83B642A28309AcAec49615b324f724d3
...
contract Payments is ReentrancyGuard, AccessControl, IMortgageInterest {
   ...
   address private relayAddress = 0x1921a154365A82b8d54a3Cb6e2Fd7488cD0FFd23;
    ...
    function getDebt(uint256 _IdMortgage, address _walletUser) public onlyDev returns(uint256) {
        ...
        // cuando paga capital se le reducen los pagos de capital mensuales agregar que recalcule los pagos a capital cuando da pagos avanzados
        // dividir la deuda total actualizada entre los meses pendientes por pagar
    function advancePayment(address _token,uint256 _IdMortgage,uint256 _amountToPay) public nonReentrant isPaused {
        ...
        bool success = false;
            if (_token == address(Busd)) {
                success = Busd.transferFrom(msg.sender,address(this),_amountToPay);
                if (success) {
   ...
   ///@dev function to pay and Calc interes when the users decide to pay.
    ...
    function CalcDailyInterest(uint256 _IdMortgage) private isPaused {
        ...
        if (!mortgage.liquidate) {
            ...
            uint256 numdays = (block.timestamp - mortgage.lastTimeCalc);// sacamos el timestamp
            if (numdays > 1 days) {
                (uint256 loan,,uint64 interestrate,uint256 payCounter) = mortgageControl.getDebtInfo(msg.sender, _IdMortgage);
                if (payCounter == 0) {
                    mortgage.totalDebt = loan;
                }
                uint256 daysLastCalc = numdays / 86400;

                if (!mortgage.isMonthlyPaymentDelayed) {
                    //uint256 dailyInterestToPay = ((mortgage.totalDebt * interestrate) / 100000);
                     if(mortgage.strikes > 0){
                        uint256 dailyDelayedInterestToPay = (((mortgage.totalDebt * interestrate) * penalization) / 10000);
                        uint256 newAmountVault = mortgageControl.getCapitalPay(msg.sender, _IdMortgage);
                        mortgage.amountToVault += newAmountVault;
                        mortgage.totalMonthlyPay += (dailyDelayedInterestToPay + newAmountVault) * daysLastCalc;
                        mortgage.amountToPanoram += ((dailyDelayedInterestToPay * percentagePanoramDelayedPay) / 10000) * daysLastCalc;
                        mortgage.amountToPool += ((dailyDelayedInterestToPay * percentagePoolDelayedPay) / 10000) * daysLastCalc;
                    } else{
                        uint256 dailyInterestToPay = ((mortgage.totalDebt * interestrate) / 10000);
                        /* uint256 newAmountVault = loan / (period * 30); 
                        mortgage.amountToVault += newAmountVault; */
                        uint256 newAmountVault = mortgageControl.getCapitalPay(msg.sender, _IdMortgage);
                        mortgage.amountToVault += newAmountVault;
                        //There is no problem for the division to follow the multiplication, since this data is stored directly.
        ...
        for (uint16 i = 0; i < _userWallet.length; i++) {
            ...
            if (!mortgage.liquidate) {
                ...
                if(!mortgageControl.getMortgageStatus(_userWallet[i], _idMortgage[i])){
                    ...
                    uint256 numdays = (block.timestamp - mortgage.lastTimeCalc);
                    if (numdays > 1 days) {
                        uint256 daysLastCalc = numdays / 86400;
    ...
    function updatePaused(bool status) public {
        if (!hasRole(DEV_ROLE, msg.sender)) {
  ...
  ///@dev Funcion para actualizar los porcentajes a pagar a Panoram y al Pool en pago moratorio
    ...
    function updateMortgageControl(address _newMortgageControl) public onlyDev isPaused {
        ...
        // SOLO PARA TESTS ,ELIMINAR ANTES DE PRODUCCION
        
    function updatePaidStatusPruebas(address _wallet, uint256 _IdMortgage,uint256 _lastTimePayment, uint256 _lastTimeCalc, uint8 _strikes,
    bool _isMonthlyPaymentPayed,bool _isMonthlyPaymentDelayed, bool _liquidate) public onlyDev{
        MortgageInterest memory mort = mortgageControl.getuserToMortgageInterest(_wallet, _IdMortgage);
        mort.lastTimePayment = _lastTimePayment;
        mort.lastTimeCalc = _lastTimeCalc;
        mort.strikes = _strikes;
        mort.isMonthlyPaymentPayed = _isMonthlyPaymentPayed;
        mort.isMonthlyPaymentDelayed = _isMonthlyPaymentDelayed;
        mort.liquidate = _liquidate;
        mortgageControl.updatePaidStatusPruebas(_wallet, _IdMortgage,mort);
    }
...
interface IMortgageControl is IMortgageInterest {
    ...
    ///@dev ONLY TEST BORRAR EN PRODUCCION
    ...
    function updatePaidStatusPruebas(address _wallet, uint256 _IdMortgage, MortgageInterest memory mort) external ;
}
...
interface IMortgageInterest {
    ...
    struct Information {
        ...
        uint256 downPay;
        uint256 capitalPay; // Daily Capital Pay
        uint256 price;
        uint256 startDate;
