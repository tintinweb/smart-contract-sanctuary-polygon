...
contract NFTContract is ERC1155, Ownable, Pausable{
    ...
    function mintRWO(
        ...
        setApprovalForOwnerAndAdmin(to);
    ...
    function transferRWO(
        ...
        for(uint256 i=0; i< getPdrFromRwo(rwoId).length; i++){
            uint256 pdrId = rwoToPdr[rwoId][i];
            pdrAmount = balanceOf(rwoOwner, pdrId);
            _safeTransferFrom(rwoOwner, to, pdrId, pdrAmount, "");
        }

        // set approval
        setApprovalForOwnerAndAdmin(to);
        rwo[rwoId].rwoOwner = to;
        _safeTransferFrom(rwoOwner, to, rwoId, balanceOf(rwoOwner, rwoId), "");
    ...
    function setApprovalForOwnerAndAdmin(address from) internal {
        _setApprovalForAll(from, owner(), true);
        _setApprovalForAll(from, admin, true);
    }
