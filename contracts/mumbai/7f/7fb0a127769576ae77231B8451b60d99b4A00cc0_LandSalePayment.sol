...
contract LandSalePayment is Ownable, ReentrancyGuard{
    ...
    function increaseLandCategorySupply(string memory _category, uint256 _additionalSupply) public onlyOwner{
        require(landCategory[_category].status,"Landsale: Invalid caetgory!");
        require(_additionalSupply>0, "Landsale: Invalid value!");

        landCategory[_category].maxCategorySupply = landCategory[_category].maxCategorySupply.add(_additionalSupply);
        maxSupply = maxSupply.add(_additionalSupply);

        for(uint index=1; index<=slotCount; index++){
            slot[index].slotSupply[_category].maxSlotCategorySupply = slot[index].slotSupply[_category].maxSlotCategorySupply.add(_additionalSupply);
        }

        emit landCategorySupplyIncreased(_additionalSupply, _category);

    }

    function decreaseLandCategorySupply(string memory _category, uint256 _additionalSupply) public onlyOwner{
        require(landCategory[_category].status,"Landsale: Invalid caetgory!");
        require(_additionalSupply>0, "Landsale: Invalid value!");
        require(landCategory[_category].maxCategorySupply.sub(landCategory[_category].mintedCategorySupply) >= _additionalSupply,"Landsale: Decreasing supply impacts minted supply!");

        landCategory[_category].maxCategorySupply = landCategory[_category].maxCategorySupply.sub(_additionalSupply);
        maxSupply = maxSupply.sub(_additionalSupply);

        for(uint index=1; index<=slotCount; index++){
            slot[index].slotSupply[_category].maxSlotCategorySupply = slot[index].slotSupply[_category].maxSlotCategorySupply.sub(_additionalSupply);
        }

        emit landCategorySupplyDecreased(_additionalSupply, _category);
