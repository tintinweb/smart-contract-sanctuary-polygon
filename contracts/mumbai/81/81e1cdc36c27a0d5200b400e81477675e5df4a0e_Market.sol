    ...
    ReentrancyGuardUpgradeable
{
        ...
        isValidAuction(auctionId_)
    {
        ...
        if (
            ...
            (royaltyReceiver, royaltyAmount) = IERC2981PayoutUpgradeable(
                auction.nftContract
            ).royaltyInfo(auction.tokenId, highestBid.bidAmount);

            // TODO
            uint256 bidAmountWithoutFee = highestBid.bidAmount * 10000 / (10000 + getServiceFeePercent());

            (payoutReceivers, payoutShares) = IERC2981PayoutUpgradeable(
                auction.nftContract
            ).payoutInfo(auction.tokenId, bidAmountWithoutFee);
        }
        ...
        if (_paymentTokens[highestBid.paymentTokenIndex].native) {
            if (auction.creator == owner()) {
                ...
                payable(auction.creator).transfer(
                    highestBid.bidAmount - royaltyAmount - (serviceFee / 2)
                );
            }
