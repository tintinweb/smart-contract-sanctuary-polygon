...
contract MysterBox2 is Initializable, IMB.IMysteryBox {
  ///@dev connect libs
  ...
  mapping(address => uint) public _lastContextId;
  ...
  function getBox(address collection, uint contextId, uint256 boxTokenId) external view override returns (Box memory) {
    return _getBox(collection, contextId, boxTokenId);
  }

  function getAllSeries(address collection, uint contextId) external view override returns (string[] memory) {
  ...
  function createContextBox(
    address collection,
    InputBoxSeries[] calldata inputBoxes
  ...
  function openBox(IMB.ICollection collection, uint256 contextId, uint256 tokenBoxId) external override {
    return _openBox(collection, contextId, tokenBoxId, msg.sender);
  }

  ///@dev private functions

  function _openBox(IMB.ICollection collection, uint256 contextId, uint256 tokenBoxId, address recepient) internal {
    ...
    uint256 randomPercentage = _randomRange(1, 100);
    uint sumPercentageOfLuck = 0;
    uint256 droppedTokenId;
    for (uint i = 0; i < context.seriesKeys.length; i++) {
      BoxSeries storage series = context.series[context.seriesKeys[i]];
      if (
        randomPercentage > sumPercentageOfLuck &&
        randomPercentage <= sumPercentageOfLuck + series.probability &&
        series.tokens.length > 0
      ) {
        uint256 randomItemIndex = _randomRange(0, series.tokens.length);
  ...
  function _createContextBox(address collection, InputBoxSeries[] calldata inputBoxes) internal returns (uint256) {
    uint lastContextId = _getLastContextAndIncrement(collection);

    for (uint i = 0; i < inputBoxes.length; i++) {
    ...
    require(_validateSeries(collection, lastContextId), "MB: Incorrect token loss percentage amount");
    emit CreateContextBox(lastContextId, collection);
    return lastContextId;
  }

  function _getAllSeries(address collection, uint contextId) private view returns (string[] memory) {
    return contextsBox[collection][contextId].seriesKeys;
  }

  function _getBox(address collection, uint contextId, uint256 boxTokenId) private view returns (Box memory) {
  ...
  function _getLastContextAndIncrement(address collection) private returns (uint) {
...
interface IMysteryBox {
  ...
  function getSeries(address collection, uint contextId, string calldata series) external returns (BoxSeries memory);

  function getBox(address collection, uint contextId, uint256 boxTokenId) external returns (Box memory);

  function getAllSeries(address collection, uint contextId) external view returns (string[] memory);

  ///@dev external functions

  function createContextBox(address collection, InputBoxSeries[] calldata inputBoxes) external returns (uint256);

  function mintOneBox(ICollection collection, string calldata boxURI, uint256 templateId) external returns (uint256);
  ...
  function openBox(ICollection collection, uint256 templateId, uint256 tokenBoxId) external;
}
