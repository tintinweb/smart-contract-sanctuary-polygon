...
// TestToken Custodial
contract TestTokenCustodialV2 is NativeMetaTransaction, ReentrancyGuardUpgradeable, UUPSUpgradeable  {
    IERC20 public token;

    function initialize(address _token) public virtual initializer  {
        _initializeEIP712("TestToken", "1");
        __Ownable_init();
    ...
    //error WithdrawEtherFailed(string message, string reason);
    event WithdrawWBET(address indexed userAddress, uint256 amount);
    event WithdrawEther(address indexed userAddress, uint256 amount);
    event EtherDeposited(address indexed from, uint256 amount);

    function withdrawEther(address payable user, uint256 amount)
        external
        onlyOwner
        nonReentrant
    {
        require(amount <= address(this).balance, "Not enough ether to withdraw from the contract");
        (bool sent, bytes memory data) = user.call{value: amount}("");
        //string memory response = string(data);
        require(sent, "Failed to Send Ether");
        //if (!sent) {
            //revert WithdrawEtherFailed("Failed to Send Ether", response);
        //}
        emit WithdrawEther(user, amount);
    }

    function withdrawWBET(address user, uint256 amount)
        external
        onlyOwner
        nonReentrant
    {
        token.transfer(user, amount);
        emit WithdrawWBET(user, amount);
    }

    /// Approve Transfer
    function approve(address target, uint256 amount) external onlyOwner nonReentrant {
 ...
 */
contract NativeMetaTransaction is EIP712Base, OwnableUpgradeable, ContextMixin {
