...
contract Factory is IFactory, Ownable {
    ...
    function setApproval(address intAddress, bool allowIntegration)
        external
        override
    {
    ...
    function isApproved(address _owner, address _intAddress)
        public
        view
        override
        returns (bool)
    ...
    constructor(IMarginEngine _masterMarginEngine, IVAMM _masterVAMM) {
        require(
            address(_masterMarginEngine) != address(0),
            "master me must exist"
        );
    ...
    function setMasterFCM(IFCM _masterFCM, uint8 _yieldBearingProtocolID)
        external
        override
        onlyOwner
    {
    ...
    function setMasterMarginEngine(IMarginEngine _masterMarginEngine)
        external
        override
        onlyOwner
    {
    ...
    {
        IMarginEngine marginEngine = IMarginEngine(
            address(new VoltzERC1967Proxy(address(masterMarginEngine), ""))
        );
        IVAMM vamm = IVAMM(
            address(new VoltzERC1967Proxy(address(masterVAMM), ""))
        );
        marginEngine.initialize(
            _underlyingToken,
            _rateOracle,
