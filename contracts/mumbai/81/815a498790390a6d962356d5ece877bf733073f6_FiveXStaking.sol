...
contract FiveXStaking is Ownable, ReentrancyGuard {
    ...
    function getTotalStats(address investor) public view returns (TotalStats memory stats) {
        uint256 deposited;
        uint256 claimed;
        uint256[] memory ids = getOwnedDeposits(investor);
            for(uint256 i = 0 ; i< ids.length; i++){
                if(depositState[ids[i]].state){
                deposited += depositState[ids[i]].depositAmount;
                claimed += depositState[ids[i]].claimedAmount;
            }
        }
        return TotalStats(deposited, claimed, getAllClaimableReward(investor));
    }

    function getAllDeposit(address investor) public view returns (DepositStruct[] memory) {
        uint256[] memory ids = getOwnedDeposits(investor);
        DepositStruct[] memory deposits = new DepositStruct[](ids.length);
        for(uint256 i = 0; i<ids.length; i++){
            deposits[i] = depositState[ids[i]];
        }
        return deposits;
    }
