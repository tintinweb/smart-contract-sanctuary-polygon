...
// File: contracts/SWCT_ICO_v2.sol
...
contract PresaleContract is ReentrancyGuard, Ownable, Pausable {
    ...
    IERC20Metadata public usdc_interface;
    ...
    // _pay_method = ( 1:matic, 2:matic_card, 3:usdt )
    ...
    constructor(
        ...
        usdc_interface = IERC20Metadata(usdc_);
    ...
    function get_matic_in_usdt() internal view returns (uint256) {
    ...
    function buy_with_usdc(uint256 amount_) external nonReentrant whenNotPaused canPurchase(_msgSender(), amount_) returns (bool) {
        uint256 amount_in_usdt = amount_;
        require(round_list[current_round_index].usdt_round_raised + amount_in_usdt < round_list[current_round_index].usdt_round_cap,
            "BUY ERROR : Too much money already deposited."
        );
    ...
    function buy_with_matic() external payable nonReentrant whenNotPaused canPurchase(_msgSender(), msg.value) returns (bool) {

        uint256 amount_in_usdt = (msg.value * get_matic_in_usdt()) / 1e30;
        require(round_list[current_round_index].usdt_round_raised + amount_in_usdt < round_list[current_round_index].usdt_round_cap,
            "BUY ERROR : Too much money already deposited."
        );
    ...
    function buy_with_matic_wert(address user) external payable nonReentrant whenNotPaused canPurchase(user, msg.value) returns (bool) {
