...
contract Auction is Initializable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function delegatedConstructor(address _addressRegistry, uint256 _minBidIncrement) public initializer {
    ...
    function createAuction(
        ...
        if (IERC165(_nftAddress).supportsInterface(INTERFACE_ID_ERC1155)) {
            ...
            require(
                ...
                "Auction: Multitoken Wasn't minted on Veritone"
            );
            ...
            require(INFTFactory(addressRegistry.factory()).exists(_nftAddress), "Auction: NFT Wasn't minted on Veritone");
        }
        require(
            _payToken == address(0) ||
        ...
        _createAuction(_nftAddress, _tokenId, _payToken, _reservePrice, _startTimestamp, _endTimestamp);
    }
    ...
    function createAuctionAndBidWithVoucher(
        ...
        if (_voucher.deployAndMint) {
            ...
            if (_voucher.isMultiToken) {
                (_nftAddress, _tokenId) = IMultiTokenFactory(addressRegistry.MultiTokenFactory())
                    .createNFTContractFromRequest(_requestID, _payToken, msg.sender);
            } else {
                (_nftAddress, _tokenId) = INFTFactory(addressRegistry.factory()).createNFTContractFromRequest(
                    _requestID,
                    _payToken,
                    msg.sender
                );
    ...
    /**
     ...
     @param _tokenId Token ID of the item being auctioned
     */
    function withdrawBid(address _nftAddress, uint256 _tokenId) external whenNotPaused nonReentrant onlyAdmin {
        HighestBid storage highestBid = highestBids[_nftAddress][_tokenId];
        // Ensure highest bidder is the caller
        require(highestBid.bidder != address(0), "Auction: No bid has been made");

        uint256 _endTime = auctions[_nftAddress][_tokenId].endTime;

        require(
            _getNow() > _endTime && (_getNow() - _endTime >= 43200),
            "Auction: can withdraw only after 12 hours (after auction ended)"
        );

        uint256 previousBid = highestBid.bid;

        // Refund the top bidder
        _refundHighestBidder(_nftAddress, _tokenId, highestBid.bidder, previousBid);
    ...
    function cancelAuction(address _nftAddress, uint256 _tokenId) external nonReentrant whenNotPaused {
        ...
        require(
            ...
            "Auction: sender must be owner"
        );
        // Check auction is real
        require(auction.endTime > 0, "Auction: no auction exists");
        // Check auction not already resulted
        require(!auction.resulted, "Auction: auction already resulted");
    ...
    /**
     ...
     @param _tokenId Token ID of the item being auctioned
     */
    function resultAuction(address _nftAddress, uint256 _tokenId) external whenNotPaused nonReentrant {
        // Check the auction to see if it can be resulted
        AuctionData storage auction = auctions[_nftAddress][_tokenId];

        require(INFT(_nftAddress).ownerOf(_tokenId) == auction.owner, "Auction: sender must be item owner");

        // Check the auction real
        require(auction.endTime > 0, "Auction: no auction exists");

        // Check the auction has ended
        require(_getNow() > auction.endTime, "Auction: auction not ended");
        ...
        require(winner != address(0), "Auction: no open bids");
        require(winningBid >= auction.reservePrice, "Auction: highest bid is below reservePrice");

        // Ensure this contract is approved to move the token
        require(INFT(_nftAddress).isApprovedForAll(auction.owner, address(this)), "Auction: auction not approved");
        ...
        require(total <= winningBid, "Auction: Royalty splitup shouldn't be greater than 99.99% of actual price");
        IERC20Upgradeable(auction.payToken).safeTransferFrom(msg.sender, auction.owner, winningBid - total);

        // Transfer the token to the winner

        bool isMultiToken = IERC165(_nftAddress).supportsInterface(INTERFACE_ID_ERC1155);
        if (isMultiToken) {
            IMultiToken(_nftAddress).safeTransferFrom(
                IMultiToken(_nftAddress).creators(_tokenId),
                winner,
                _tokenId,
                1,
                "0x00"
    ...
    function _refundHighestBidder(
        ...
        AuctionData memory auction = auctions[_nftAddress][_tokenId];
        IERC20Upgradeable payToken = IERC20Upgradeable(auction.payToken);
        require(payToken.transfer(_currentHighestBidder, _currentHighestBid), "Auction: failed to refund previous bidder");
...
interface INFTFactory {
  ...
  function createNFTContractFromRequest ( uint256 _requestId, address _token, address _sender ) external returns ( address, uint256 );
  function delegatedConstructor ( address _addressRegistry ) external;
...
interface INFT {
  ...
  function mint ( address _to, string memory _tokenUri, address _ERC20Token, address _sender, address[] memory royaltyRecipient, uint256[] memory royaltyBps ) external returns ( uint256 );
...
interface IMultiToken {
  ...
  function creators ( uint256 ) external view returns ( address );
  function getRoyalties ( uint256 tokenId ) external view returns ( address[] memory, uint256[] memory );
  function mint ( address _to, uint256 _supply, string memory _uri, address _ERC20Token, address _sender, address[] memory royaltyRecipient, uint256[] memory basisPoints ) external returns ( uint256 );
  function mintFromRequest ( uint256 _requestId, address _ERC20Token, address _sender ) external returns ( uint256 );
  function owner (  ) external view returns ( address );
  function updateAddressRegistry( address _addressRegistry ) external;
}
...
interface IMultiTokenFactory {
  function createNFTContract ( string calldata _name, string calldata _symbol, string calldata _uri, address _token ) external returns ( address );
  function createNFTContractFromRequest ( uint256 _requestId, address _token, address _sender ) external returns ( address, uint256 );
  function delegatedConstructor ( address _addressRegistry ) external;
  function disableTokenContract ( address tokenContractAddress ) external;
  function exists ( address ) external view returns ( bool );
