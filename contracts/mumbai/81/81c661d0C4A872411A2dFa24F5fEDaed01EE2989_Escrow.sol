...
contract Escrow is ERC721Holder, ERC1155Receiver, ERC1155Holder {
    ...
    function rent(bytes32 lendId, uint256 qty, uint256 duration, Sign calldata sign) external {
        ...
        uint256 fee = getFees(lendingDetails[lendId].dailyRent, duration, qty);
    ...
    function claim(bytes32 rentalId, Sign calldata sign) external {
        ...
        uint8 nftType = getValidType(lendingDetails[lendId].nftAddress); 
        uint256 fee = getFees(lendingDetails[lendId].dailyRent, rentalDetails[rentalId].duration, rentalDetails[rentalId].rentedQuantity);
    ...
    function safeTransfer(address nftAddress, address paymentAddress, address caller, address callee, uint256 tokenId, uint256 lendingAmount, uint8 nftType, uint256 amount, bool isClaim) internal {
        ...
        if(amount > 0) {
            if(isClaim) { 
                ...
                (_royaltyFee, _tokenCreator, royalty) = IRoyaltyInfo(nftAddress)
                ...
                for( uint96 i = 0; i < _royaltyFee.length; i++) {
                    transferProxy.erc20safeTransferFrom(IERC20(paymentAddress), callee, _tokenCreator[i], _royaltyFee[i]);
                }
    ...
    function getFees(uint256 dailyRent, uint256 duration, uint256 qty) internal pure returns(uint256) {
       return (dailyRent * duration * qty);
    }
