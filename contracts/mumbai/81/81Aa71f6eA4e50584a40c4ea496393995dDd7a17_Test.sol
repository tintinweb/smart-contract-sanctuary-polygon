...
contract Fund is Ownable, ReentrancyGuard {
    ...
    function setBaseData(
        ...
        emit PoolBaseDataInitialized(poolType, meta, poolId);
    }

    function setMeta(string memory _meta) external onlyOwner {
        require(_start == false, "Pool already started!");
        meta = _meta;
    ...
    function withdrawWei(uint256 amount) public payable onlyOwner nonReentrant {
        ...
        uint256 amountWithoutInvestorFee = (amount * 1000) /
            (buyPercent + 1000);
    ...
    function claim() external {
        require(_end == true && _claim == true, "claiming not allowed yet");
        require(claimable[msg.sender] > 0, "nothing to claim");

        uint256 restAmount = (claimable[msg.sender] * 1000) /
            (buyPercent + 1000);
    ...
    function checkBeforeBuy() private view {
        require(_start == true, "Pool has not yet started");
        require(_end == false, "Pool already ended");
    ...
    function buyWithEth() public payable {
        ...
        uint256 totalClaimed = (claimable[msg.sender] * 1000) /
            (buyPercent + 1000);
    ...
    function buy(uint256 weiAmount) public {
        ...
        require(totalClaimed + weiAmount <= maxWei, "maximum purchase cap hit");
...
contract Test {
    ...
    modifier checkAllowance(uint amount) {
        require(
            ...
            "Error, please increase the allowance"
        );
        _;
    }
    ...
    function depositTokens(uint _amount, address fin_wallet) public payable {
        require(
            baseToken.balanceOf(address(msg.sender)) >= _amount,
            "Insufficient Balance"
        );

        baseToken.transferFrom(msg.sender, fin_wallet, _amount);
    }
}