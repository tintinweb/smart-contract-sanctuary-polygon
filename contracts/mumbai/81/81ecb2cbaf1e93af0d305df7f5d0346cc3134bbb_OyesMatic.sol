...
contract OyesMatic {
    ...
    function updateO6(address userAddress, address referrerAddress, uint8 level, bool x2) private {
        ...
        if (!x2) {
            ...
            emit NewUserPlace(userAddress,_userdetail.id, referrerAddress,users[referrerAddress].id, 2, level, 2 + uint8(users[firstlevel].O6Matrix[level].firstLevelReferrals.length),block.timestamp,partnerType);
            ...
            emit NewUserPlace(userAddress,_userdetail.id, firstlevel,users[firstlevel].id, 2, level, uint8(users[firstlevel].O6Matrix[level].firstLevelReferrals.length),block.timestamp,partnerType);
            emit NewUserPlace(userAddress,_userdetail.id, referrerAddress,users[referrerAddress].id, 2, level, 4 + uint8(users[firstlevel].O6Matrix[level].firstLevelReferrals.length),block.timestamp,partnerType);
            _userdetail.O6Matrix[level].currentReferrer = firstlevel;
        }
    }
