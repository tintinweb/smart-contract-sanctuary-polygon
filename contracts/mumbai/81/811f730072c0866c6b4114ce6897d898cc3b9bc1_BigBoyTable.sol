...
interface IBigBoyTable {
  ...
  function getRoomCreationPriceInUSDC() external pure returns (uint);
  function getExtractorRole() external view returns(bytes32);
  function getStakingWindowInSeconds() external pure returns(uint);
  function getWinningFeeInPercents() external pure returns(uint);
}
...
contract BigBoyTable is IBigBoyTable, AccessControlEnumerableUpgradeable, PausableUpgradeable {
  ...
  // TODO: should be settable by admin
  // percent of SNK or Native, depending on room currency
  uint private constant WINNING_FEE_IN_PERCENTS = 5; 
  uint private constant ROOM_CREATION_PRICE_IN_USDC = 5; 
  uint private constant STAKING_WINDOW_IN_SECONDS = 5*60;
  ...
  modifier whenStakePossible(uint roomId) {
    ...
    require(
      block.timestamp <= room.lockedAt + STAKING_WINDOW_IN_SECONDS,
  ...
  function getWinningFeeInPercents() external override pure returns(uint) {
    return WINNING_FEE_IN_PERCENTS;
  }

  function getStakingWindowInSeconds() external override pure returns(uint) {
    return STAKING_WINDOW_IN_SECONDS;
  }

  function getRoomCreationPriceInUSDC() external override pure returns(uint) {
    return ROOM_CREATION_PRICE_IN_USDC;
  }
  ...
  function _getRoomCreationPriceInSkill() internal view returns(uint price) {
    // NOTE: priceInSNK = priceInUSDC * k / 0.25
    uint k = _uniswap.getSnookPriceInSkills();
    price = ROOM_CREATION_PRICE_IN_USDC * k * 100  / 25;
  }
  ...
  ) external override onlyActiveRoom(roomId) onlyRole(EXTRACTOR_ROLE) 
  {
    ...
    require(
      block.timestamp > room.createdAt + STAKING_WINDOW_IN_SECONDS,
    ...
    uint winningFeeAmount = room.totalStake * WINNING_FEE_IN_PERCENTS / 100;
