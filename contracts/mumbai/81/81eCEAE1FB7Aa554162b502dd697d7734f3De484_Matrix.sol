...
contract Matrix {
  ...
  uint256 GOLD_MATRIX_PRICE = 1 ether ; 
  ...
  event MatrixClosed(address indexed referrer, uint8 level, uint256 reinvestCount);
  ...
  function registration(address userAddress, address referrerAddress) private {
    require(msg.value == 1 ether, "registration cost 1 TRX");
  ...
  function updateMatrixReferrerSecondLevel(address userAddress, address referrerAddress, uint8 level) private {
    ...
    emit MatrixClosed(referrerAddress, level, users[referrerAddress].matrix[level].reinvestCount);
  ...
  function updateGoldMatrixReferrerSecondLevel(address userAddress, address referrerAddress) private {
    ...
    emit MatrixClosed(referrerAddress, 0, goldMatrix[referrerAddress].reinvestCount);
