...
interface IScrap is IERC20 {
    ...
    /* ========== EVENTS ========== */
    event NonceInvalidation(address indexed signer, uint256 nonce);
...
/**
    @author The Calystral Team
    @title  Synergy of Serra in-game market.
*/
contract SynergyOfSerraMarket is
    EIP712,
    ...
    RegistrableContractState
{
    ...
    ==============================*/
    event NonceInvalidation(address indexed signer, uint256 nonce);
    event FeeUpdate(uint256 feePercentage);
    event Trade(
    ...
    bytes32 public constant SELL_TYPEHASH =
        keccak256(
            "Sell(address seller,uint256 nftId,uint256 scrapNetPrice,bytes data,uint256 nonce)"
        );
    bytes32 public constant BUY_TYPEHASH =
        keccak256(
            "Buy(address buyer,uint256 nftId,uint256 scrapGrossPrice,bytes data,uint256 nonce)"
        );
    /*==============================
    =       DATA STRUCTURES        =
    ==============================*/
    ...
    =          MODIFIERS           =
    ==============================*/
    modifier isAuthorizedMarketAccount() {
        _isAuthorizedMarketAccount();
        _;
    }
    ...
    */
    constructor(
        ...
        EIP712("Synergy of Serra Market", "1")
    ...
    function onERC1155Received(
        ...
        uint256,
        bytes calldata
    ) external view override(IERC1155TokenReceiver) isActive returns (bytes4) {
    ...
    function trade(
        ...
        // S E L L E R
        address signerSeller = ECDSA.recover(_digest(sell), signatureSeller);
        require(sell.seller == signerSeller, "Invalid seller signature");
        require(
            isMetaNonceUsed[signerSeller][sell.nonce] == false,
            "This transaction was executed by the seller already"
        );
        _invalidateNonce(signerSeller, sell.nonce);

        // B U Y E R
        address signerBuyer = ECDSA.recover(_digest(buy), signatureBuyer);
        require(buy.buyer == signerBuyer, "Invalid buyer signature");
        require(
            isMetaNonceUsed[signerBuyer][buy.nonce] == false,
            "This transaction was executed by the buyer already"
        );
        _invalidateNonce(signerBuyer, buy.nonce);

        //check T R A D E
        uint256 tradeFee = ((sell.scrapNetPrice * feePercentage) / 100 / 1e18) *
            1e18;
        require(
            sell.scrapNetPrice + tradeFee == buy.scrapGrossPrice &&
                sell.nftId == buy.nftId,
            "Invalid trade"
        );

        // T R A N S F E R S
        IERC1155(_getAssetsContractAddress()).safeTransferFrom(
            signerSeller,
            signerBuyer,
            sell.nftId,
            1,
        ...
        IScrap(_getScrapContractAddress()).transferFrom(
            signerBuyer,
            signerSeller,
            sell.scrapNetPrice
        );
        IScrap(_getScrapContractAddress()).burnFrom(signerBuyer, tradeFee);
    ...
    function batchTrade(
        bytes[] calldata signaturesSeller,
        Sell[] calldata sells,
        bytes[] calldata signaturesBuyer,
        Buy[] calldata buys
    ) external {
        for (uint256 i = 0; i < signaturesSeller.length; i++) {
            trade(signaturesSeller[i], sells[i], signaturesBuyer[i], buys[i]);
        }
    }
    ...
    function updateFee(uint256 value) external isAuthorizedAdmin {
        feePercentage = value;
        emit FeeUpdate(value);
    }
    ...
    =      INTERNAL & PRIVATE      =
    ==============================*/
    function _digest(Sell calldata sell) internal view returns (bytes32) {
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
    ...
    function _digest(Buy calldata buy) internal view returns (bytes32) {
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
    ...
    function _getAssetsContractAddress() private view returns (address) {
        return getContractAddress(2);
    }

    function _getScrapContractAddress() private view returns (address) {
        return getContractAddress(10);
    }

    function _invalidateNonce(address signer, uint256 nonce) private {
        isMetaNonceUsed[signer][nonce] = true;
        emit NonceInvalidation(signer, nonce);
    }

    /**
        @dev Checks if the msg.sender is the MarketManager.
        Reverts if msg.sender is not the MarketManager.
    */
    function _isAuthorizedMarketAccount() private view {
        require(
            getContractAddress(1009) == msg.sender,
