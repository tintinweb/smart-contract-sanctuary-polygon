...
contract CapsStaking is Ownable {
    ...
    event WithdrawReward(address indexed by, uint256 amount, uint256 at);
    event Claimed(
        address indexed by,
        uint256 tokenAmount,
    ...
    struct User {
        ...
        uint256 rewardFrom;
        bool isActiveUser;
        mapping(uint256 => uint256) stackedAt;
        mapping(uint256 => uint256) rewardPerStake;
    ...
    struct RewardHistory {
        uint256 rewardAmount;
        uint256 totalStackedTokens;
    }

    mapping(address => User) private usersData;
    mapping(uint256 => RewardHistory) public rewardHistory;
    address[] private activeStackers;
    uint256 private totalStakedAmount;
    uint256 private availableRewards;
    uint256 public maxDataFetchSizeLimit = 20;
    uint256 public claimTime = 1 days;
    uint256 public totalRewardCount;
    ICapsToken private capsTokenContract_Ins;

    constructor(address _capsTokenContract) {
        capsTokenContract_Ins = ICapsToken(_capsTokenContract);
    }
    ...
    function stake(uint256 amount) external returns (bool) {
        ...
        uint256 curStakeCount = usersData[msgSender].totalStakes + 1;
        ...
        if (availableRewards != 0) {
            tempUserData.rewardPerStake[curStakeCount] = availableRewards;
            availableRewards = 0;
        }
        tempUserData.rewardFrom = totalRewardCount + 1;

        tempUserData.totalStakes = curStakeCount;
        tempUserData.amountStaked[curStakeCount] = amount;
        tempUserData.stackedAt[curStakeCount] = curTime;

        SafeERC20.safeTransferFrom(
            IERC20(address(capsTokenContract_Ins)),
            msgSender,
            address(this),
            amount
        ); // Adding SafeTransfer Method Here Because Of Auditor Recommendation
        emit Stacked(msgSender, curStakeCount, amount, curTime);
        return true;
    }
    ...
    function setClaimTime(uint256 newTime) external onlyOwner {
        claimTime = newTime;
    }

    function setMaxDataFetchSizeLimit(uint256 newSize) external onlyOwner {
    ...
    function calculateRewardPerStack(address userAddress)
        ...
        returns (uint256)
    {
        uint256 totalRewardPerStake;
        uint256 _totalRewardCount = totalRewardCount;
        uint256 _userTotalStackedAmount = getUserTotalStackedAmount(
            userAddress
        );
        ...
        for (
            ...
            uint256 _rewardAmount = rewardHistory[i].rewardAmount;
            uint256 _totalStakedAmount = rewardHistory[i].totalStackedTokens;
            if (_userTotalStackedAmount != 0) {
                if (_totalStakedAmount != 0) {
                    uint256 userRewardAmount = uint256(
    ...
    function getStackData(address userAddress, uint256 stackNum)
        external
        view
    ...
    {
        User storage tempUserData = usersData[userAddress];
        int256 arrSize = int256(
            (tempUserData.totalStakes - tempUserData.claimFrom) + 1
        );
        if (arrSize < 0) {
            arrSize = 0;
        }
        stackData[] memory tempStackData = new stackData[](uint256(arrSize));
        uint256 j;
        for (
            uint256 i = tempUserData.claimFrom;
            i <= tempUserData.totalStakes;
