 ...
 * @dev This contract acts as intermediary between NaviController and nTokens distributing messages
 * between different the levels of the hierarhy
 **/
contract NaviControllerGroup is NaviMarginGroup, NaviMarginable, IMarginGroup {
    constructor(
        address oracle,
        address parent,
        uint256 ltv
    ) NaviMarginGroup(oracle) NaviMarginable(ltv, parent) {
        _erc1820.setInterfaceImplementer(
            address(this),
    ...
    function setLTV(uint256 ltv) external onlyOwner {
        ...
        require(ltv >= 0.5 ether, "Loan to value ratio must be >= 50%");
    ...
    /**
     * @notice This function checks if account has enough margin to borrow or redeem this amount of margin.
     * @dev If account has both borrows and deposits in this group, we must estimate how
     * net position will change in order to determine if this is a borrow or a redeem
     */
    function hasEnoughMargin(address account, uint256 amount)
        ...
        returns (bool)
    {
        if (netPosition(account, 0) >= int256(amount))
            return _parent.hasEnoughMargin(account, wmul(amount, _LTV)); //redeem
        return _parent.hasEnoughMargin(account, wdiv(amount, _LTV)); //borrow
    }
    ...
    //no need to keep track of strategies in groups
    function authorizeStrategy(
        address strategy,
        address signatory,
        bool authorize
     ...
     * net depositer(borrows=0). Unused argument is required for compartability with IMarginable.
     */
    function netPosition(address from, uint32) public view returns (int256) {
        int256 total = 0;
        for (uint32 i = 0; i < _members.length; i++)
            total += _members[i].netPosition(from, _data[i]);

        if (total >= 0) return int256(wmul(uint256(total), _LTV));
        return -int256(wdiv(uint256(-total), _LTV));
    }

    function balanceUpdated(uint8, address) external {}
}