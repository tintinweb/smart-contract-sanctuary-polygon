...
contract DepensionPlanV1 is AccessControlUpgradeable {
    ...
    event     Plan(uint32 planId, address customer, uint256 amount, uint16 planType, uint16 rate);
    event  Deposit(uint32 planId, address customer, uint256 amount, uint32 planType, uint16 rate, address token);
    event Withdraw(uint32 planId, address customer, uint256 amount, uint32 planType, uint16 rate, address token);
    ...
    function transferToken(address from, address to, address token, uint256 amount) public onlyRole(roleAccess) {
        ...
        if (from == address(this)) {
            approveTokenTransfer(token, to, amount);
    ...
    function approveTokenTransfer(address token, address recipient, uint256 amount) public onlyRole(roleAccess) {
        IERC20Upgradeable token_ = IERC20Upgradeable(token);

        uint256 allowance = token_.allowance(address(this), recipient);
        if (allowance < amount) {
            token_.safeIncreaseAllowance(recipient, amount-allowance);
        }
    }
    ...
    function newPlan(address customer, uint256 amount, uint16 planType, uint16 rate) public onlyRole(roleAccess) returns(uint32) {
        _planId +=1;
        ...
        emit Plan(_planId, customer, amount, planType, rate);
        return _planId;
    }
    ...
    function deposit(
        ...
        require(inputAmount > 0 && rate > 0, "InvalidInput");
        ...
        uint32 planId = newPlan(customer, inputAmount, planType, rate);
        emit Deposit(planId, customer, inputAmount, planType, rate, token);
    }
    ...
    function withdraw(
        ...
        _removePlan(planId);
        emit Withdraw(planId, customer, outputAmount, planInfo.planType, planInfo.rate, token);
    }
}
