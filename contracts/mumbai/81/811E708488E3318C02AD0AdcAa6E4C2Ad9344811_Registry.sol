...
import "../interfaces/IRecord.sol";

abstract contract Record is IRecord {
  
  mapping(uint256 => uint256) internal _tokenPresets; // Presets. E.g _tokenPresets[tokenId] = presetId
  mapping(uint256 => mapping(uint256 => string)) internal _records; // Records. E.g _records[presetId]] = value
  mapping(uint256 => string) private _keys; // Keys. E.g _keys[keyHash] = 'crypto.BTC.holland'


  // Getters

  // Key -> crypto.BTC.address (creo que puede ser tanto addresses como cualquier otro valor a referenciar (por ejemplo: ownerAddress)) TODO: Check
  ...
  function _get(string calldata key, uint256 tokenId) private view returns (string memory value) {
    return _get(uint256(keccak256(abi.encodePacked(key))), tokenId); // Encode key to get keyHash
  }

  function _get(uint256 keyHash, uint256 tokenId) private view returns (string memory value) {
    return _records[_presetOf(tokenId)][keyHash]; // view on records mapping after getting the tokenPresets mapping
  }
  ...
  function _set(
    ...
    if (bytes(_records[_presetOf(tokenId)][keyHash]).length == 0) {
      emit NewKey(tokenId, key, key); // If the record doesnt already exists -> Emit new key event
    }
    _records[_presetOf(tokenId)][keyHash] = value; // Set the new value or replace the value
...
contract Registry is ERC721Upgradeable, IRegistry, Record {

  address internal _mintingManagerAddress;
  mapping(address => uint256) internal _reverses;
  mapping(uint256 => string) internal _tokenNames;
  
  /** 
   * @dev Methods only allowed for the minting contract
   */
  modifier onlyMintingManager() {
    require(_msgSender() == _mintingManagerAddress, 'Registry: SENDER_IS_NOT_MINTING_MANAGER');
    _;
  }

  /** 
   * @dev Methods only allowed for the owner or approved addresses
   */
  ...
  function initialize(
    address mintingManagerAddress
  ) public initializer {
    _mintingManagerAddress = mintingManagerAddress;
   __ERC721_init_unchained("DotMundo", "DTM");
  }

  function exists(uint256 tokenId) external view override(IRegistry) returns (bool) {
    return _exists(tokenId);
  }

  function mintTLD(uint256 tokenId, string calldata uri) external override onlyMintingManager {
    _mint(_mintingManagerAddress, tokenId);
  ...
  function reverseNameOf(address addr) external view override returns (string memory) {
    ...
    string memory domain = _tokenNames[tokenId];
    require(keccak256(abi.encodePacked(domain)) != keccak256(abi.encodePacked('')), 'Registry: REVERSE_IS_NOT_ALLOWED');
    return domain;
  }

  //--------------- INTERNAL ----------------

  /** Receive a tokenId and a label. Returns a node
   *  @param labels string[] -> array with the labels. E.g: ['henri', 'holland']
   *  @return node -> hashing of the whole domain
   */
  function _namehash(string[] memory labels) internal pure returns(uint256) {
    uint256 node = 0x0; // Node base hash (node = null) only namehash the label because there's no parent node

    // Iterate over the labels and call the _namehash method
  ...
  /** Receive a tokenId and a label. Returns a node
   *  @param tokenId -> Is the tokenId of the token
   *  @param label -> E.g: 'henri' (of henri.holland)
   *  @return node -> namehash of the domain (might be the tokenId to for the mint)
   */
  function _namehash(uint256 tokenId, string memory label) internal pure returns(uint256) {
    require(bytes(label).length != 0, "Registry: LABEL_EMPTY"); // Check if label is not empty
    return uint256(keccak256(abi.encodePacked(tokenId, keccak256(abi.encodePacked(label)))));
  }

  /** Set Reverse for address
   * @param addr address of the reverse record.
   * @param tokenId token Id for the domain name
   * @param uri Domain name associated with the address
   */
  ...
  /** Receive a string of labels and returns the URI
   *  @param labels string[] -> array with the labels. E.g: ['henri', 'holland']
   *  @return uri -> Uri based on the labels. E.g: 'henri.holland'
   */
