...
contract Marketplace {
    ...
    mapping(uint256 => utilities.Order) public IdtoOrder;
    mapping(address => uint256) nonces;
    mapping(address => mapping(uint256 => bool)) public isListed;
    event Listing(address maker, uint256 NFTId, bool isListed);
    event CancelListing(bool isListed);
    event AtomicMatch(
        address maker,
        address taker,
        uint256 NFTId,
    ...
    uint8 public creator_fee;
    uint8 public system_fee;
    ...
    function listing(address _maker, uint256 _NFTId) public onlyDev {
        isListed[_maker][_NFTId] = true;
        emit Listing(_maker, _NFTId, true);
    }

    function checkListed(address _maker, uint256 _NFTId)
        public
        view
        virtual
        returns (bool)
    {
        return isListed[_maker][_NFTId];
    }

    function cancelListing(address _maker, uint256 _NFTId) public {
        isListed[_maker][_NFTId] = false;
        emit CancelListing(false);
    }
    ...
    function _transferToken(
        ...
        if (_payment_token != address(0)) {
            uint256 amountToSystem = (_price * system_fee) / 1000;
            uint256 amountToCreator = (_price * creator_fee) / 1000;
    ...
    function _atomicMatch(
        ...
        require(isListed[_sell.maker][_sell.NFTId], "not listed");
        ...
        isListed[_sell.maker][_sell.NFTId] = false;
        isListed[_sell.taker][_sell.NFTId] = false;
        emit AtomicMatch(
            _sell.maker,
            _sell.taker,
            _sell.NFTId,
