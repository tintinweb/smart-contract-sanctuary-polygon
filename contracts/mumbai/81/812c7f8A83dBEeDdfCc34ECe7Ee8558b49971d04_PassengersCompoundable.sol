...
abstract contract RevokableOperatorFiltererUpgradeable is OperatorFiltererUpgradeable {
    ...
    function __RevokableOperatorFilterer_init(address subscriptionOrRegistrantToCopy, bool subscribe) internal onlyInitializing{
...
RevokableDefaultOperatorFiltererUpgradeable {
	...
	*/
	...
	function detachAndBurnTSD(uint256 passengerId, uint256 tsdID) external {
			...
			delete headOf_721[crateContract][tsdID];
	...
	function burnTSD(uint256[] memory TSDs) external {
		for (uint256 i=0;i<TSDs.length;i++) {
			require(
				IERC721Upgradeable(crateContract).ownerOf(TSDs[i]) == msg.sender,
				"Burn: Not the true owner of TSD"
			);
			IPassengersCrate(crateContract).burnToken(TSDs[i]);
		}
	}
	
	function turnOnOS() external onlyController {
