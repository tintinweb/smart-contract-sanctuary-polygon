...
contract ERC721Token is ERC721URIStorage, Ownable {
    ...
    function mint(
        ...
        uint newItemId = _minting(recipient, _tokenURI);
        // nftOwner[newItemId] = recipient;
        return newItemId;
    }

    // function mintInContract(
    //     address _mintContract,
    //     address _to,
    //     string memory _tokenURI
    // ) external returns (uint) {
    //     uint newItemId = _minting(_mintContract, _tokenURI);
    //     nftOwner[newItemId] = _to;
    //     return newItemId;
    // }
...
contract NestedNft {
    ...
    uint256 public _directOwnerId;

    // store parent for single child to get who is the parent of a child.
    mapping(uint256 => uint) private _parentOfChild;

    // store child indexing id in a pendingChild list
    mapping(uint256 => uint) private _pendingChildIndex;

    // store child indexing id in a activeChild list
    mapping(uint256 => uint) private _activeChildIndex;
    ...
    // Used to notify listeners that all pending child of a given parent have been rejected.
    event AllChildrenRejected(uint256 indexed parentId);
    ...
    modifier onlyParent(uint256 parentId) {
        require(
            _directOwners[parentId].ownerAddress == msg.sender,
            "Only Parent can do this action!"
        );
        _;
    }

    modifier onlyChild(uint256 childId) {
        require(
            _directOwners[childId].ownerAddress == msg.sender,
            "Only Child can do this action!"
        );
        _;
    }
    ...
    function getPendingChildIndex(uint childId) external view returns (uint) {
        return (_pendingChildIndex[childId] - 1);
    }

    // Show indexing id of a child to active child list
    function getActiveChildIndex(uint childId) external view returns (uint) {
        return (_activeChildIndex[childId] - 1);
    }

    // show parent id of a child
    function getParentOfChild(uint childId) external view returns (uint) {
    ...
    function nestMint(
        ...
        _addChild(_parentId, _directOwnerId);
    }
    ...
    function transferChild(
        ...
        uint256 _childId,
        bool _isPending
    ) external virtual onlyParent(_parentId) {
        _transferChild(
            _parentId,
            _newParentId,
            _childIndex,
    ...
    function rejectAllChildren(
        ...
        _rejectAllChildren(_parentId, maxRejections);
    }

    // remove/delete minted parent nft and also its child and grand child
    function burn(
        uint256 parentId
    ) external virtual onlyParent(parentId) returns (uint256) {
    ...
    function _existsChildId(
        ...
        if (_isPending) {
            if (_pendingChildren[_parentId][_childIndex].childId == _childId) {
                return true;
            }
    ...
    function _innermint(address _to, string memory _tokenURI) private {
        uint256 nftId = ERC721Token(nfttoken).mint(_to, _tokenURI);

        _directOwnerId++;
        _directOwners[_directOwnerId] = DirectOwner({
            nftId: nftId,
            ownerAddress: _to
        });
    ...
    function _addChild(uint256 _parentId, uint256 _childId) private {
        require(
            _existsId(_parentId) && _existsId(_childId),
        ...
        address childAdd = _directOwners[_childId].ownerAddress;
        Child memory child = Child({childId: _childId, childAddress: childAdd});

        _pendingChildren[_parentId].push(child);

        _parentOfChild[_childId] = _parentId;

        _pendingChildIndex[_childId] = _pendingChildren[_parentId].length;

        emit ChildProposed(
            _parentId,
            _pendingChildren[_parentId].length - 1,
            childAdd,
            _childId
        );
    }
    ...
    function _acceptChild(
        ...
        _activeChildIndex[_childId] = _pendingChildren[_parentId].length;

        delete _pendingChildIndex[_childId];

        _removeChildByIndex(_pendingChildren[_parentId], _childIndex);

        emit ChildAccepted(_parentId, _childIndex, _childAddress, _childId);
    }
    ...
    function _transferChild(
        ...
        if (_isPending) {
            ...
            _removeChildByIndex(_activeChildren[_parentId], _childIndex);
    ...
    function _rejectAllChildren(
        ...
        require(
            _pendingChildren[_parentId].length > maxRejections,
            "Max Rejections is Exceed!"
        );

        delete _pendingChildren[_parentId];
        emit AllChildrenRejected(_parentId);
    }

    // delete parent and its child and also grand child for both list pending and accept.
    function _burn(uint256 parentId) private returns (uint256) {
        Child[] memory pendingChild = pendingChildrensOf(parentId);
        Child[] memory activeChild = childrensOf(parentId);

        if (pendingChild.length > 0) {
            _recursiveBurn(parentId, pendingChild.length);
        }

        if (activeChild.length > 0) {
            _recursiveBurnActiveChild(parentId, activeChild.length);
        }

        // get parent Id of a child and removed child from parent
        uint parentIdOfChild = _parentOfChild[parentId];
        if (parentIdOfChild != 0) {
            if (_pendingChildIndex[parentId] > 0) {
                _removeChildByIndex(
                    _pendingChildren[parentIdOfChild],
                    _pendingChildIndex[parentId] - 1
                );
            } else if (_activeChildIndex[parentId] > 0) {
                _removeChildByIndex(
                    _activeChildren[parentIdOfChild],
                    _activeChildIndex[parentId] - 1
                );
            }
        }

        delete _activeChildren[parentId];
        delete _pendingChildren[parentId];
        delete _directOwners[parentId];

        emit burning(parentId, msg.sender);

        return 0;
    }

    // delete childs and grand child into pending child list.
    function _recursiveBurn(uint256 parentId, uint length) private {
        for (uint i = 0; i < length; ) {
            Child[] memory children = pendingChildrensOf(parentId);
            uint newParentId = children[i].childId;

            if (children.length > 0) {
                _recursiveBurn(
                    newParentId,
                    pendingChildrensOf(newParentId).length
                );
            }

            delete _pendingChildren[newParentId];
            delete _directOwners[newParentId];
            delete _childIsInActive[children[i].childAddress][
                children[i].childId
            ];
    ...
    // delete childs and grand child into accept child list.
    function _recursiveBurnActiveChild(uint256 parentId, uint length) private {
        for (uint i = 0; i < length; ) {
            Child[] memory children = childrensOf(parentId);
            uint newParentId = children[i].childId;

            if (children.length > 0) {
                _recursiveBurnActiveChild(
                    newParentId,
                    childrensOf(newParentId).length
                );
            }

            delete _activeChildren[newParentId];
            delete _directOwners[newParentId];
            delete _childIsInActive[children[i].childAddress][
                children[i].childId
            ];
