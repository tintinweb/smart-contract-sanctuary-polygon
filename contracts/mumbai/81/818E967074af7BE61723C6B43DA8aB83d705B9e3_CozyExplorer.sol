...
/**
 * The CozyExplorer NFT contract.
 */

 // TODO - Chainlink VRF integration for reveal and gold claims
contract CozyExplorer is ERC721, IERC721Enumerable, Ownable {
  IERC20 cozyGold;
  using Strings for uint;

  string private constant NAME = "Cozy Explorer";
  string private constant SYMBOL = "CZEX";
  ...
  uint public immutable maxAdminMint;

  uint public adminMinted;
  uint public totalTokenSupply;
  bool public locked;
  bool public proposedLock;

  uint public mintCostGold;
  uint public costMultiplier = 10005;
  uint public costDivisor = 10000;

  constructor(
    uint _maxTokens,
    uint _maxAdminMint,
    address _cozyGoldAddress,
    uint _mintCostGold,
    string memory _unknownTokenUri
  ) ERC721(NAME, SYMBOL) {
    cozyGold = IERC20(_cozyGoldAddress);
    maxTokens = _maxTokens;
    maxAdminMint = _maxAdminMint;
    adminMinted = 0;
    mintCostGold = _mintCostGold;
    unknownTokenUri = _unknownTokenUri;
  }
  ...
  function mintAdmin(uint numberOfTokens, address recipient) external onlyOwner {
    require(totalTokenSupply + numberOfTokens <= maxTokens, "Not enough tokens");
    require(adminMinted + numberOfTokens <= maxAdminMint, "Admin reached capped mint");
    _mintMultiple(numberOfTokens, recipient);
  }

  function mint() public returns(bool) {
    require(totalTokenSupply < maxTokens, "Not enough tokens");
    require(mintCostGold <= cozyGold.allowance(msg.sender, address(this)), "Please approve enough tokens before transferring");
    
    cozyGold.transferFrom(msg.sender, address(this), mintCostGold);
    mintCostGold = (mintCostGold * costMultiplier / costDivisor) / 10 ** 18 + 1 ** 18;
  ...
  function proposeLock() external onlyOwner {
    proposedLock = true;
  }

  function cancelProposeLock() external onlyOwner {
    proposedLock = false;
  }

  function lock() external onlyOwner {
    require(proposedLock, "proposeLock is false");
    locked = true;
  }
