...
// File: contracts/LCT/IERC721LUXON.sol


pragma solidity ^0.8.13;

interface IERC721LUXON {
    function mintLuxOn(address mintUser, uint256 quantity, string[] memory gachaIds) external;
}
// File: contracts/LCT/IERC20LUXON.sol
...
// File: contracts/LCT/SuperOperators.sol
...
// File: contracts/LCT/Payback.sol
...
// File: contracts/LCT/LCTGachaMachine.sol
...
contract LCTGachaMachine is Payback, SuperOperators, ReentrancyGuard {
    ...
    mapping(uint8 => string[]) private characterInfoTable;
    mapping(string => uint8) private characterInfo;
    ...
    function getTierRatio() public view returns (uint32[] memory, uint256) {
        return (tierRatio, tierRatioSum);
    }

    function getCharacterInfo(string memory name) public view returns (uint8) {
    ...
    function getCharacterCount() public view returns (uint256) {
        ...
        for (uint8 i = 0; i < tierRatio.length; i++) {
            characterCount += characterInfoTable[i].length;
        }
        return characterCount;
    }

    function getCharacterCountByTireAndGachaGrade(uint8 _tier) public view returns (uint256, string[] memory) {
        return (characterInfoTable[_tier].length, characterInfoTable[_tier]);
    }
    ...
    function setCharacterInfo(uint8 _tier, string[] memory _characterNames) external onlyOwner {
        characterInfoTable[_tier] = _characterNames;
        for (uint8 i = 0; i < _characterNames.length; i++) {
            characterInfo[_characterNames[i]] = _tier;
        }
    }

    //------------------ gacha ------------------//

    function gacha(address mintUser, uint256 quantity) external onlySuperOperator {
    ...
    function mintPay(address mintUser, uint256 quantity) private {
        ...
        string[] memory gachaIds = new string[](quantity);
        for (uint8 i = 0; i < quantity; i++) {
            gachaIds[i] = characterGacha(i);
        }
        IERC721LUXON(mintAddress).mintLuxOn(mintUser, quantity, gachaIds);
    }

    function getGachaIds(uint256 quantity) public view returns (string[] memory) {
        string[] memory gachaIds = new string[](quantity);
        for (uint8 i = 0; i < quantity; i++) {
            gachaIds[i] = characterGacha(i);
        }
    ...
    function characterGacha(uint8 seed) public view returns (string memory) {
        uint8 _tier = randomTier(seed);
        return randomCharacterNum(_tier, seed);
    }
    ...
    function randomCharacterNum(uint8 _tier, uint8 seed) public view returns (string memory) {
        uint256 characterCount = characterInfoTable[_tier].length;
        uint index = uint(keccak256(abi.encodePacked(block.timestamp, msg.sender, seed * 2))) % characterCount;
        return characterInfoTable[_tier][index];
    }
}