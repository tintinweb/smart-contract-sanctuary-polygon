...
contract Erc1155CooperatableDriverContract is TokenCooperatableDriverContract {
...
import "../base/Erc1155CooperatableDriverContract.sol";
import "../Interfaces/IERC1155InteractionDriver.sol";

contract Erc1155InteractionDriver is Erc1155CooperatableDriverContract,IERC1155InteractionDriver {
    
    constructor(string memory name_,string memory desc_,address token_contract_) 
        Erc1155CooperatableDriverContract(name_,desc_,token_contract_) {
    }
    
    function mintNFTsFor(address _addr,uint256[] memory tokenIds,uint256[] memory amounts) external virtual onlyOwnerAndOperatableTemplate {
        address _token_contract = get_token_contract_address();
        require(_token_contract != address(0),"NO_TOKEN_CONTRACT");
        
        IERC1155TokenContract _tokenContract = IERC1155TokenContract(_token_contract);
        _tokenContract.mintNFTsFor(_addr,tokenIds,amounts);
    }
    function burnNFTsFor(address _addr,uint256[] memory tokenIds,uint256[] memory amounts) external virtual onlyOwnerAndOperatableTemplate {
        address _token_contract = get_token_contract_address();
        require(_token_contract != address(0),"NO_TOKEN_CONTRACT");
        
        IERC1155TokenContract _tokenContract = IERC1155TokenContract(_token_contract);
        _tokenContract.burnNFTsFor(_addr,tokenIds,amounts);
    }
    function mintNFTFor(address _addr,uint256 tokenId,uint256 amount) external virtual onlyOwnerAndOperatableTemplate {
        address _token_contract = get_token_contract_address();
        require(_token_contract != address(0),"NO_TOKEN_CONTRACT");

        IERC1155TokenContract _tokenContract = IERC1155TokenContract(_token_contract);
        _tokenContract.mintNFTFor(_addr,tokenId,amount);
    }
    function burnNFTFor(address _addr,uint256 tokenId,uint256 amount) external virtual onlyOwnerAndOperatableTemplate {
        address _token_contract = get_token_contract_address();
        require(_token_contract != address(0),"NO_TOKEN_CONTRACT");

        IERC1155TokenContract _tokenContract = IERC1155TokenContract(_token_contract);
        _tokenContract.burnNFTFor(_addr,tokenId,amount);
    }
    function get_driver_token_contract() external view virtual override returns (address){return get_token_contract_address();}
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return
            interfaceId == type(IERC1155InteractionDriver).interfaceId ||
            super.supportsInterface(interfaceId);
    }
}
