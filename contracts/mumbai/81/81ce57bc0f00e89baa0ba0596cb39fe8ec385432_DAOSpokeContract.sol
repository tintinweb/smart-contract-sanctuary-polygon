    ...
    */
    function receiveMessage(bytes memory VAA) public {
        ...
        if (option == 0) {
            ...
            coreBridge.publishMessage(nonce, payload, 200);
            nonce++;
...
contract MetaHumanGovernor is Governor, GovernorSettings, CrossChainGovernorCountingSimple, GovernorVotes, GovernorVotesQuorumFraction, GovernorTimelockControl {
    /**
     ...
     @param _wormholeCoreBridgeAddress The address of the core bridge contract used for cross-chain communication.
    */
    constructor(IVotes _token, TimelockController _timelock, CrossChainAddress[] memory _spokeContracts, uint16 _chainId, address _wormholeCoreBridgeAddress)
    Governor("MetaHumanGovernor")
    GovernorSettings(1 /* 1 block */, 5 * 4 /* 4 minutes */, 0) //TODO in production voting period needs to be changed to some reasonable value, right now it's 5 blocks for testing. Also remember about proposal threshold
...
/**
  @title IWormhole
  @dev The interface for the Wormhole contract.
  Copied from [Wormhole repository](https://github.com/wormhole-foundation/wormhole/blob/main/ethereum/contracts/interfaces/IWormhole.sol).
 */
interface IWormhole {
    struct GuardianSet {
        address[] keys;
