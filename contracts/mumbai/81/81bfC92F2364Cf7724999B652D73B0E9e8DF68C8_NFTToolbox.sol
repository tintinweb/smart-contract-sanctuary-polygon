...
contract NFTToolbox is ERC721Enumerable, Ownable { 
    ...
    function batchMint(uint16[] calldata _quantities, address[] calldata recipients) public onlyAllowed {
        for(uint16 i = 0; i < recipients.length; i++) {
            for(uint8 z = 0; z < _quantities[i]; z++) {
                uint256 newId = ++_counter + z;
                _mint(recipients[i], newId);
            }           
        }
    }
