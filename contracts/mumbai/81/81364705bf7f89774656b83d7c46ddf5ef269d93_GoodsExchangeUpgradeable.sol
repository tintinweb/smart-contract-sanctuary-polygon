    ...
    ChainLinkPriceOracleUpgradeable
{
    ...
    uint256 public paymentPriceUsdPerItem;
    mapping(address => BitMaps.BitMap) private _isCollectionUsed;

    function __GoodsPayment_init(uint256 paymentPriceUsdPerItem_) internal onlyInitializing {
        __GoodsPayment_init_unchained(paymentPriceUsdPerItem_);
    }

    function __GoodsPayment_init_unchained(uint256 paymentPriceUsdPerItem_) internal onlyInitializing {
        paymentPriceUsdPerItem = paymentPriceUsdPerItem_;
    }
    ...
    function calculatePaymentAmounts(
        ...
        uint256 paymentAmount = _getTokenAmountUp(paymentTokens_, paymentPriceUsdPerItem * total);
    ...
    function setPaymentPriceUsdPerItem(uint256 amount_) external onlyRole(OPERATOR_ROLE) {
        paymentPriceUsdPerItem = amount_;
    }

    function pay(address paymentToken_, address collection_, uint256[] calldata tokenIds_) external payable {
    ...
    function _processPayment(address paymentToken_, address collection_, uint256[] calldata tokenIds_) internal {
        ...
        uint256 paymentAmount = _calculatePaymentAmounts(paymentToken_, collection_, tokenIds_);

        if (paymentToken_ == address(0)) {
            require(msg.value >= paymentAmount, "IB");
