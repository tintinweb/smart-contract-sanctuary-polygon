...
library Signature {
    function verifyBuyMessage(
        ...
        uint256 _taxBuyerPercent,
        uint256 _taxSellerPercent,
        StructLib.TaxReceiver[] memory _taxReceivers,
        uint256 _signatureExpTime,
        address _nFTAddress,
        address _verifier
...
import "../interfaces/IVerdantRouter.sol";
import "../interfaces/IVerdantCore.sol";
import "../libraries/Signature.sol";

import {EZeroAddress} from "../libraries/Error.sol";

contract VerdantRouter is IVerdantRouter, Initializable {
    IVerdantCore public verdantCore;
    IVerdantManager public verdantManager;

    function initialize(
        address _verdantCore,
        address _verdantManager
    ) public initializer {
        verdantCore = IVerdantCore(_verdantCore);
    ...
    function setVerdantCore(address _verdantCore) external onlyOwner {
        if (_verdantCore == address(0)) {
            revert EZeroAddress();
        }
        verdantCore = IVerdantCore(_verdantCore);
    }
    ...
    function createOrder(
        ...
        require(
            Signature.verifyOrderMessage(
                ...
                verdantManager.verifier()
            ),
            "Invalid signature"
        );

        return
            verdantCore.createOrder(
                _paymentToken,
                _tokenId,
                _price,
                _sellerDiscountPercent,
    ...
    function createBid(
        ...
        uint256 _signatureExpTime,
        bytes calldata _signature
    ) external override returns (uint256 _bidId) {
        require(
    ...
    function acceptBid(
        ...
        verdantCore.acceptBid(_bidId, _taxInfo, _taxReceivers, msg.sender);
    }

    function cancelOrder(uint256 _orderId) external override {
        verdantCore.cancelOrder(_orderId, msg.sender);
    }

    function cancelBid(uint256 _bidId) external override {
        verdantCore.cancelBid(_bidId, msg.sender);
    }
    ...
    function updateBid(
        ...
        require(
            Signature.verifyBidMessage(
                _signature,
                msg.sender,
                verdantCore.bids(_bidId).paymentToken,
                _bidId,
                _bidPrice,
                _serviceTaxPercentBuyer,
                _signatureExpTime,
