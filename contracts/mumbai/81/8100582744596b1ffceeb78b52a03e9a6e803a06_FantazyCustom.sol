...
///////////////////
    ...
    function CustomwithdrawPrizes2(address[] calldata users, uint256[] calldata amounts, uint256 eventID, uint256 poolid) external onlyOwner{
        require(!pools1[poolid].prizesPaid, "The prizes have already been paid.");
        require(users.length == amounts.length,"Must be equal");
        //require(users.length <= 10,"");
        uint256[] memory me = pools1[poolid].teamsarray;
        uint256 us = paidcount1[poolid];
        require(us < me.length,"Must be less than");
        ...
        for(uint256 i = 0; i < users.length; i++){
            ...
            eventpaidforuser1[eventID][users[i]] = true;
        }
        ...
        paidcount1[poolid] += users.length;
