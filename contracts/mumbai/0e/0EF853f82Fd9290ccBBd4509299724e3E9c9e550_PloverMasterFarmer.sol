...
// File: PloverToken.sol

pragma solidity 0.6.12;


// PloverToken with Governance.
contract PloverToken is ERC20("Panda Token", "PNDA"), Ownable, Authorizable {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "Plover::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "Plover::delegateBySig: invalid nonce");
        require(now <= expiry, "Plover::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "Plover::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "Plover::_writeCheckpoint: block number exceeds 32 bits");
...
// File: browser/PloverMasterFarmer.sol

pragma solidity 0.6.12;


interface IMigratorToPloverSwap {
    // Perform LP token migration from legacy UniswapV2 to PloverSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to UniswapV2 LP tokens.
    // PloverSwap must mint EXACTLY the same amount of PloverSwap LP tokens or
    // else something bad will happen. Traditional UniswapV2 does not
    // do that so be careful!
    function migrate(IERC20 token) external returns (IERC20);
}

// PloverMasterFarmer is the master of Plover. He can make Plover and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once Plover is sufficiently
// distributed and the community can show to govern itself.
//
contract PloverMasterFarmer is Ownable, Authorizable {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of Plovers
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accPloverPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accPloverPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. Plovers to distribute per block.
        uint256 lastRewardBlock;  // Last block number that Plovers distribution occurs.
        uint256 accPloverPerShare; // Accumulated Plovers per share, times 1e12. See below.
    }

    // The Plover TOKEN!
    PloverToken public Plover;
    ...
    // The block number when Plover mining starts.
    ...
    IMigratorToPloverSwap public migrator;
    ...
    event SendPloverReward(address indexed user, uint256 indexed pid, uint256 amount, uint256 lockAmount);
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate) public onlyOwner {
        require(poolId1[address(_lpToken)] == 0, "PloverMasterFarmer::add: lp is already in pool");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    // Update the given pool's Plover allocation point. Can only be called by the owner.
    ...
    function setMigrator(IMigratorToPloverSwap _migrator) public onlyOwner {
        migrator = _migrator;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 PloverForDev;
        uint256 PloverForFarmer;
		uint256 PloverForLP;
		uint256 PloverForCom;
		uint256 PloverForFounders;
        (PloverForDev, PloverForFarmer, PloverForLP, PloverForCom, PloverForFounders) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);
        Plover.mint(address(this), PloverForFarmer);
        pool.accPloverPerShare = pool.accPloverPerShare.add(PloverForFarmer.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
		if (PloverForCom > 0) {
            Plover.mint(comfundaddr, PloverForCom);
			//Community fund treasury for non-technical bounties, contests, promotions, grants and non-technical hiring.
            if (block.number <= FINISH_BONUS_AT_BLOCK) {
                Plover.lock(address(comfundaddr), PloverForCom.mul(75).div(100));
            }
        }
		if (PloverForDev > 0) {
            Plover.mint(devaddr, PloverForDev);
			//Plover rewards to go to the dev treasury to support technical bounties, hiring, infrastructure costs and other on-going fees.
			if (block.number <= FINISH_BONUS_AT_BLOCK) {
                Plover.lock(address(devaddr), PloverForDev.mul(50).div(100));
            }
        }
        if (PloverForFounders > 0) {
            Plover.mint(founderaddr, PloverForFounders);
			//Plover rewards for the founding dev team members directly. Unlocks at the same pace as user tokens.
			if (block.number <= FINISH_BONUS_AT_BLOCK) {
                Plover.lock(address(founderaddr), PloverForFounders.mul(75).div(100));
            }
        }
        if (PloverForLP > 0) {
            Plover.mint(liquidityaddr, PloverForLP);
			//Plover rewards for Bao holders which will be dripped out over time in staking contract on mainnet. It will be dripped out over many years based on staking of Bao, lockedBao, BaoNFTs and other franchise tokens.
			if (block.number <= FINISH_BONUS_AT_BLOCK) {
                Plover.lock(address(liquidityaddr), PloverForLP.mul(50).div(100));
            }
        }
    ...
    function getPoolReward(uint256 _from, uint256 _to, uint256 _allocPoint) public view returns (uint256 forDev, uint256 forFarmer, uint256 forLP, uint256 forCom, uint256 forFounders) {
        ...
        uint256 PloverCanMint = Plover.cap().sub(Plover.totalSupply());

        if (PloverCanMint < amount) {
            forDev = 0;
			forFarmer = PloverCanMint;
			forLP = 0;
			forCom = 0;
			forFounders = 0;
        }
        else {
            forDev = amount.mul(10).div(100);
			forFarmer = amount;
			forLP = amount.mul(25).div(100);
			forCom = amount.mul(5).div(100);
			forFounders = amount.mul(10).div(100);
        }
    }

    // View function to see pending Plovers on frontend.
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accPloverPerShare = pool.accPloverPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply > 0) {
            uint256 PloverForFarmer;
            (, PloverForFarmer, , ,) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);
            accPloverPerShare = accPloverPerShare.add(PloverForFarmer.mul(1e12).div(lpSupply));

        }
        return user.amount.mul(accPloverPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function _harvest(uint256 _pid) internal {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accPloverPerShare).div(1e12).sub(user.rewardDebt);
            uint256 masterBal = Plover.balanceOf(address(this));
            ...
            if(pending > 0) {
                Plover.transfer(msg.sender, pending);
                uint256 lockAmount = 0;
                if (user.rewardDebtAtBlock <= FINISH_BONUS_AT_BLOCK) {
                    lockAmount = pending.mul(PERCENT_LOCK_BONUS_REWARD).div(100);
                    Plover.lock(msg.sender, lockAmount);
                }

                user.rewardDebtAtBlock = block.number;

                emit SendPloverReward(msg.sender, _pid, pending, lockAmount);
            }

            user.rewardDebt = user.amount.mul(pool.accPloverPerShare).div(1e12);
        }
    }
    ...
    // Deposit LP tokens to PloverMasterFarmer for $Plover allocation.
    function deposit(uint256 _pid, uint256 _amount, address _ref) public {
        require(_amount > 0, "PloverMasterFarmer::deposit: amount must be greater than 0");
        ...
        devr.rewardDebt = devr.amount.mul(pool.accPloverPerShare).div(1e12);
  ...
  // Withdraw LP tokens from PloverMasterFarmer.
    function withdraw(uint256 _pid, uint256 _amount, address _ref) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        UserGlobalInfo storage refer = userGlobalInfo[_ref];
        UserGlobalInfo storage current = userGlobalInfo[msg.sender];
        require(user.amount >= _amount, "PloverMasterFarmer::withdraw: not good");
        if(_ref != address(0)){
                refer.referrals[msg.sender] = refer.referrals[msg.sender] - _amount;
                refer.globalRefAmount = refer.globalRefAmount - _amount;
            }
        ...
        if(_amount > 0) {
			...
			if(user.blockdelta == blockDeltaStartStage[0] || block.number == user.lastDepositBlock){
				...
				pool.lpToken.safeTransfer(address(msg.sender), _amount.mul(1).div(100));
				pool.lpToken.safeTransfer(address(devaddr), _amount.mul(99).div(100));
				...
				pool.lpToken.safeTransfer(address(msg.sender), _amount.mul(50).div(100));
				pool.lpToken.safeTransfer(address(devaddr), _amount.mul(50).div(100));
				...
				pool.lpToken.safeTransfer(address(msg.sender), _amount.mul(88).div(100));
				pool.lpToken.safeTransfer(address(devaddr), _amount.mul(12).div(100));
				...
				//1% fee if a user deposits and withdraws after 4 weeks.
    ...
    // Safe Plover transfer function, just in case if rounding error causes pool to not have enough Plovers.
    function safePloverTransfer(address _to, uint256 _amount) internal {
        uint256 PloverBal = Plover.balanceOf(address(this));
        if (_amount > PloverBal) {
            Plover.transfer(_to, PloverBal);
        } else {
            Plover.transfer(_to, _amount);
        }
    }
