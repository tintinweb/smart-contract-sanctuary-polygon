...
contract RecruitmentATSSmartContract {
    ...
    mapping(address => Application[]) public applicationsByEmployer;

    mapping(address => Application[]) public  applicationsByApplicant;
 ...
 // Modifier to restrict access to applicant-only functions
    ...
    function applyJob(uint jobId, uint matchScore, address applicantAddress) public {
        ...
        applicationsByEmployer[jobs[jobId].employerAddress].push(Application(
            msg.sender,
            jobs[jobId].employerAddress,
            jobId,
            false,
        ...
        applicationsByApplicant[applicantAddress].push(Application(
            msg.sender,
            jobs[jobId].employerAddress,
            jobId,
            false,
    ...
    function getApplicantionDetailsByEmployer(
        address employerAddress
    ) public view returns (Application[] memory) {
        return applicationsByEmployer[employerAddress];
    }

    function getApplicantionDetailsByApplicant(
        address applicantAddress
    ) public view returns (Application[] memory) {
        return applicationsByApplicant[applicantAddress];
    }
