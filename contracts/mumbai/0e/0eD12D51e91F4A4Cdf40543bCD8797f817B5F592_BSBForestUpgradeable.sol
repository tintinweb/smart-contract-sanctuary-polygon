...
contract BSBForestUpgradeable is Initializable, BSBBaseGameModeUpgradeable, VRFConsumerBaseUpgradeable {
    ...
    uint public treeHouseStaminaBuff;
    ...
    function setTreeHouseStaminaBuff(uint value_) external onlyOwner {
        treeHouseStaminaBuff = value_;
    }
    ...
    function _claimTokenGeneratorRewards(
        ...
        for (uint i = 0; i < collectionItems_.assets.length; ++i) {
            uint item_ = collectionItems_.assets[i];
            address collection_ = collectionItems_.collection;
            if (tokenHouseAppartenance[collection_][item_]) {
                ...
                if (yield > 0) {
                    emit BloodTokensClaimed(collection_, item_, yield);
                }

                accumulatedYield += yield;
            }

            stateChangeDates[collection_][item_] = block.timestamp;
