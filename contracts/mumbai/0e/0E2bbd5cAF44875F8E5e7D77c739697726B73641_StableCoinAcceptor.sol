...
contract StableCoinAcceptor {
    ...
    /**
     * @dev dev descr
     * @notice Whether provided stablecoin is supported.
     * @param _address Address to lookup.
     * @return bool True if stablecoin added, false otherwise
     */
    ...
    /**
        ...
        @return bool True if stablecoin added, false otherwise
     */
    function supportsCoin2(IERC20 _address) public view returns (bool) {
        uint256 len = stablecoins.length;
        for(uint256 i; i < len; i++) {
            if(_address == stablecoins[i]) {
                return true;
    ...
    /// @param _address Address to lookup.
    /// @return bool True if stablecoin added, false otherwise
    function supportsCoin3(IERC20 _address) public view returns (bool) {
        uint256 len = stablecoins.length;
        for(uint256 i; i < len; i++) {
            if(_address == stablecoins[i]) {
                return true;
    ...
    function supportsCoin4(IERC20 _address) public view returns (bool) {
        uint256 len = stablecoins.length;
        for(uint256 i; i < len; i++) {
            if(_address == stablecoins[i]) {
                return true;
