...
/// @notice Verify if the time setted on the order is valid
/// @param time Order expiration time
    modifier expirationChecker(uint256 time) {
        ...
        (uint year, uint month, uint day, uint hour, uint minute) = date_time.timestampToDateTime(time);
        uint formated_timestamp = date_time.timestampFromDateTime(year, month, day, hour);
        require(formated_timestamp - 1500  >= block.timestamp , "You cannot place the order for this finishing expiration!");
        _;
     
    }

/// @notice Set a new address for the active price feed of this contract
/// @param _new_address The new address for the price feed
/// @return true if successful
    function setPriceFeedAddress(address _new_address) external onlyOwner returns (bool) {
        active = _new_address;
        return true;
    }
...
/// @notice Format the timestamp to the round hour timestamp
/// @param _time The user imput timestamp
/// @return time The processed timestamp
...
/// @notice Function for place and order that will be called by the user
/// @param _price The active price that will be used for the order
/// @param _value The amount of currency the user will spend on the order
/// @param _time The timestamp that the order will be finished
/// @param call_put The type of the order (true for call, false for put)
...
/// @dev Function must be called by the cron (UpKeepers)
/// @return success If success
