 ...
 */
contract LimitlessCollection is NFTBase, Ownable, Pausable, EIP712 {
    ...
    string private constant SIGNING_DOMAIN = "LimitLess-Voucher";
    ...
    constructor(
        ...
        uint256 daysUntilReveal
    ) NFTBase(name, symbol) EIP712(SIGNING_DOMAIN, SIGNATURE_VERSION) {
        MAX_NFTS = maxNftSupply;
        REVEAL_TIMESTAMP = saleStart + (daysUntilReveal * (1 days)); //(86400 * 9);
        COLLECTION_PROVENANCE = provenanceHash;
        nftPrice = _nftPrice;
    }

    function setPuchaseLimitByWallet(uint256 newLimit) public onlyOwner {
    ...
    function setDistributionPhase(DistributionPhase _phase) external onlyOwner {
