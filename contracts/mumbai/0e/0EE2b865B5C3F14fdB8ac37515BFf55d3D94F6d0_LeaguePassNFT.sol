...
contract LeaguePassNFT is ERC721Upgradeable, OwnableUpgradeable {
    ...
    event DleagueTokenIsSet(address _adr);
    event DleagueTokenAmountIsSet(uint256 _amount);
    event WhitelistChanged(address _adr, bool _val);
    ...
    function mint(IERC20Upgradeable _token) public {
    ...
    /// @param _adr Address of the user
    /// @param _val If value is true address is whitelisted, if value is false address is not whitelisted
    function setWhitelisteAddr(address _adr, bool _val) external onlyOwner{
        whitelist[_adr] = _val;
        emit WhitelistChanged(_adr, _val);
    }
}
