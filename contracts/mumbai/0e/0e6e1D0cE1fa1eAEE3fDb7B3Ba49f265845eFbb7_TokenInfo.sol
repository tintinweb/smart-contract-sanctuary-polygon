...
contract TokenInfo is AccessControl {
   ...
   address public walletPanoram = 0x30268390218B20226FC101cD5651A51b12C07470; 
    ...
    function addVaultRegistry(address _token, address _lender,address _lenderRewards,address _rewards) public onlydev validToken(_token) {
        vaultsInfo[_token].lender = _lender;
        vaultsInfo[_token].lenderRewards = _lenderRewards;
        vaultsInfo[_token].rewards = _rewards;
    }

    function removeVaultRegistry(address _token) public onlydev validToken(_token) {
    ...
    function permissions(address _token) public onlydev validToken(_token) {
        address _lender = vaultsInfo[_token].lender;
        address _lenderRewards = vaultsInfo[_token].lenderRewards;
        address _rewards = vaultsInfo[_token].rewards;
        IERC20(_token).approve(_lender, 2**255);
        IERC20(_token).approve(_lenderRewards, 2**255);
