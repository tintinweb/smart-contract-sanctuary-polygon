...
contract NFTFactory is NFTokenMetadata, Ownable {

    mapping(address => bool) approval;

    modifier onlyApproval() {
        require(approval[msg.sender], "Only allowed account");
        _;
    }

    constructor() {
        nftName = "Crypto Catapult";
        nftSymbol = "CYP";
        approval[msg.sender] = true;
    }

    function mint(address _to, uint256 _tokenId, string calldata _url) external onlyApproval {
        super._mint(_to, _tokenId);
        super._setTokenUri(_tokenId, _url);
    }

    function setApproval(address account) public onlyOwner {
        approval[account] = true;
    }

    function isApprove(address account) public view returns (bool) {
        return approval[account] == true;
    }

    function burn(uint256 _tokenId) public onlyApproval {
        _burn(_tokenId);
    }
