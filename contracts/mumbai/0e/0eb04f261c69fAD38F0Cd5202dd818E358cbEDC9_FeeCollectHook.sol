...
contract FeeCollectHook {
    ...
    //Content Owner Id => FeeControl
    mapping(uint256 => mapping(uint256 => FeeControl)) public feeControls;
    ...
    function startup(
        ...
        (
            ...
            uint256 collectFee,
            uint256 supply
        ) = abi.decode(initData, (uint8, address, uint256, uint256));

        feeControls[ownerId][contentId] = FeeControl({
            feeToken: feeToken,
            feeReceiver: owner,
        ...
        onlyHookManager
    {
        ...
        FeeControl storage feeControl = feeControls[calleeId][contentId];

        if (feeControl.allow == CollectAllow.followers) {
            require(
        ...
        if (feeControl.supply != type(uint256).max)
            require(
                feeControl.supply > feeControl.actSupply,
                "not shares to collect"
            );
        feeControl.actSupply = feeControl.actSupply + 1;

        if (feeControl.feeToken == address(0)) {
            require(
                msg.value >= feeControl.collectFee,
                "CollectFee insufficient"
            );
            _transfer(
                feeControl.feeToken,
                feeControl.feeReceiver,
                feeControl.collectFee
            );
            uint256 refund = msg.value - feeControl.collectFee;
            if (refund > 0) {
                _transfer(feeControl.feeToken, collector, refund);
            }
        } else {
            IERC20(feeControl.feeToken).safeTransferFrom(
                collector,
                feeControl.feeReceiver,
                feeControl.collectFee
            );
