    ...
    Ownable
{
    ...
    mapping(address => uint256) private profileIdMapping;

    string[] private profileDID;
    string[] private profileGroupID;
    address[] private profileOwners;

    // string[]  private accountsGroupID;
    // string[] private m3tadaoOrganizations;
...
// 0xD504d012D78B81fA27288628f3fC89B0e2f56e24
    ...
    function indexProfile(string memory profiledid , string memory groupID ) public {
        // only one profile per address
        require(profileExists[msg.sender] == false , "only one profile per address");
        profileDID.push(profiledid);
        profileGroupID.push(groupID);
        profileOwners.push(msg.sender);
        profileIdMapping[msg.sender] = profileID.current();
        profileID.increment();        
    ...
    function getM3tadaoUserIDs(address user) public view returns (string memory did, string memory groupid){
        require(profileExists[user] == false, "not a m3tadao User");
        uint256 id = profileIdMapping[user];
        return (profileDID[id], profileGroupID[id]);
    }

    function getM3tadaoUsers() public view returns (string[] memory dids , string[] memory goupIds , address[] memory profileAddresses){
        return (profileDID,profileGroupID,profileOwners);
    }
