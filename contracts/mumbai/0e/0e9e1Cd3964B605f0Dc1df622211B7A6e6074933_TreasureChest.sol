...
contract TreasureChest is ERC721URIStorage, Ownable {
	...
	function getLevelUpPrices() public view returns (uint256[] memory) {
        uint256[] memory prices = new uint256[](MAX_LEVEL - 1);
        for (uint256 i = 2; i <= MAX_LEVEL; i++) {
            prices[i - 2] = levelUpPrice[i];
        }
        return prices;
    ...
    function getLevelCounts() public view returns (uint256[] memory) {
        uint256[] memory counts = new uint256[](MAX_LEVEL);
        for (uint256 i = 1; i <= MAX_LEVEL; i++) {
            counts[i - 1] = levelCount[i];
        }
        return counts;
	...
	function getTokenInfoList() public view returns (TokenInfo[] memory) {
        uint256[] memory tokenIds = getOwnedTokens(msg.sender);
