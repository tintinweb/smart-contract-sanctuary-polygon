    ...
    AccessControlUpgradeable
{
    ...
    */
    uint256 constant public feePercentage = 1470;
    ...
    modifier isFactoryOrUser() {
        require(
            hasRole(ALFRED_FACTORY_ROLE, msg.sender) || hasRole(USER_ROLE, msg.sender),
           "NOT_AUTHORIZED"
        );
        _;
    }
    ...
    /// @notice Function to deploy vault and invest in a strategy
    ...
    /// Public function only callable by the user or factory
    ...
    function addFund(
        ...
        ExtensionArgs[] memory _swapArgs,
        ExtensionArgs[] memory _positionArgs
    ) public isFactoryOrUser returns (uint256 sharesReceived_)
    {
    ...
    function depositUSDC(
        ...
        (bool successfulTransfer) = IERC20(USDC).transferFrom(USER, address(this), _amount);
    ...
    function redeemSharesAndWithdrawInUSDC(
        ...
        _payoutAssetPercentages[0] = 1000; /// @custom:@TODO check percentage calculations
    ...
    function claimOwnerShip() external {
        ...
        IVault(VAULT_PROXY).transfer(nominatedOwner, balance);
    ...
    /// @notice interacts with enzyme's extensions    
    /// @dev external function to interact with enyme's extension functions and adapters for misc purposes
    /// only callable by the user
    /// @param _args The structured data to call on an extension of the enzyme protocol
    function callOnExtension(
        ExtensionArgs memory _args
    ) external isUser {
        IComptroller(COMPTROLLER_PROXY).callOnExtension(_args._extension, _args._actionId, _args._callArgs);
    }

    /// @notice calls a specific function on enzyme
    /// @dev external payable   function with low level call on a given enzyme contract
    /// only callable by the user
    /// @param _target the taget contract address
    //  @param _value the value to be sent (if payable)
    /// @param _signature the function signature
    /// @param _data the encoded data of the arguments to be passed with the function
    /// @return response_ the respons or the return data of the function called
    function callOnEnzyme(address _target, string memory _signature, bytes memory _data) external payable isUser returns (bytes memory response_) {
        require(address(0) != _target, "INVALID_ADDRESS");
        ...
        (bool success, bytes memory response) = _target.call{value: msg.value}(callData);
