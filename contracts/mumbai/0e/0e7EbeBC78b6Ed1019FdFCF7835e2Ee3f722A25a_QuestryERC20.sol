  ...
  ERC2771Context
{
  ...
  /** @dev Compliance control function for AML / CFT */
  
  function RegistBlacklist(address account)
    public
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
    _registerBlacklist(account);
  }

  function DeleteBlacklist(address account)
    public
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
    _deleteBlacklist(account);
  }

  function _registerBlacklist(address account)
    internal
  {
    blacklists[account] = true;
  }

  function _deleteBlacklist(address account)
    internal
  {
    blacklists[account] = false;
  }

  function ConfirmBlacklist(address account)
    public
    view
    returns (bool)
  {
  ...
  /** override ERC20 function for AML/CFT Control */
  /**
     * @dev 
     * Custome for AML/CFT Validation/
     */
    function _transfer(
        address from,
        address to,
        ...
        require(!blacklists[from],'non-compliant address');
        require(!blacklists[to],'non-compliant address');
        super._transfer(from, to, amount);
    }


  /** @dev _msgSender & _msgData override for metaTX  */
