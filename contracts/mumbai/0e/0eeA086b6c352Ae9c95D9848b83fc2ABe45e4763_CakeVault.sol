...
// File: contracts/LP Farms/LPVault.sol
...
contract CakeVault is Ownable, Pausable {
    ...
    struct UserInfo {
        ...
        uint256 cakeAtLastUserAction; // keeps track of lp deposited at the last user action
    ...
    uint routerDeadlineDuration = 300;
    uint256 public pid;
    ...
    constructor(
        ...
        pid = _pid;
        earnedToToken0Path = [_earnedAddress, _token0Address];
        earnedToToken1Path = [_earnedAddress, _token1Address];

        // Infinite approve
        IERC20(_token).safeApprove(address(_masterchef), type(uint).max);
    }
    ...
    function harvest() external notContract whenNotPaused {
        ...
        uint256 bal = rewardAvailable();
        uint256 currentPerformanceFee = bal.mul(performanceFee).div(10000);
        rewardToken.safeTransfer(treasury, currentPerformanceFee);
        
        uint256 currentCallFee = bal.mul(callFee).div(10000);

        bal = bal.sub(currentPerformanceFee).sub(currentCallFee);


        rewardToken.safeTransfer(msg.sender, currentCallFee);
    ...
    function calculateHarvestCakeRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingCake(pid, address(this));
    ...
    function _swapToLP(uint _balWithdraw, bool _withdraw) internal {
        if (_withdraw){
            IMasterChef(masterchef).withdraw(pid, _balWithdraw);
        }
        uint lps = available();
