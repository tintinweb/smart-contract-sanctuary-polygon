...
contract ZaiNFT is ERC721Enumerable, Ownable {
    ...
    constructor(address _zaiMeta, address _levelStorage) ERC721("BandZai_NFT_ZAI", "ZAI") {
    ...
    mapping(uint256 => bool) _isGod;

    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ZaiStruct.Zai memory z = _zai[tokenId];
        return IipfsIdStorage(gameAddresses.getIpfsStorageAddress()).getTokenURI(z.metadata.seasonOf,z.metadata.state,z.metadata.ipfsPathId);
    }
    ...
    function _safeMintZai(address _to, uint256 _level)internal returns(uint256) {
        _tokenIds.increment();
        uint256 _tokenId = _tokenIds.current();
        _safeMint(_to, _tokenId);
        require(ILevelStorage(levelStorage).addFighter(_level,_tokenId));
        return _tokenId;
    }
    ...
    function mintZai(
        ...
        uint256 _newItemId = _safeMintZai(_to,0);
        _createZaiDatas(_newItemId, _name, _state, _to, 0);

        return (_newItemId);
    }

    function _createZaiDatas(uint256 _newItemId, string memory _name, uint256 _state, address _to, uint256 _level) internal{
        IipfsIdStorage I = IipfsIdStorage(gameAddresses.getIpfsStorageAddress());
        ...
        if(_state == 3 && _ipfsId <= 7){
            z.name = zaiMeta.godNames(_ipfsId);
            z.powers = zaiMeta.getGodsPowers(_ipfsId);
            _isGod[_newItemId] = true;
        }else{
            z.name = _name;
            _distributePoints(z, _points, random);
        }
        _updateAllPowers(z.powers);
    }

    function _updateAllPowers(ZaiStruct.Powers memory powers) internal {
        require(IStats(gameAddresses.getZaiStatsAddress()).updateAllPowersInGame(powers));
    }

    function _preMintZai(uint256 _level) private {
        uint256 _newItemId = _safeMintZai(levelStorage, _level);

        _createZaiDatas(_newItemId, "challenger", 0, levelStorage, _level);
        ZaiStruct.Zai storage z = _zai[_newItemId];

        z.level = _level;
        z.xp = zaiMeta.getNextLevelUpPoints(_level);

    }

    function _distributePoints(ZaiStruct.Zai storage z ,uint256 _points, uint256 random) internal{
        z.powers = zaiMeta.getRandomPowers(z.level,_points,random);
    }
    ...
    function updateXp(
        uint256 _id,
        uint256 _xp
    ) external onlyAuth returns (uint256 level) {
        ZaiStruct.Zai storage z = _zai[_id];
        z.xp += _xp;
        // update level
        level = zaiMeta.getLevel(z.xp);
        ILevelStorage ILevel = ILevelStorage(levelStorage);
    ...
    function updatePowers(
        ...
        require(IDelegate(gameAddresses.getDelegateZaiAddress()).canUseZai(_zaiId,msg.sender), "Not your zai nor delegated");
        z.creditForUpgrade -= (_water + _fire + _metal + _air + _stone);
        z.powers = zaiMeta.updatePowers(z.level, z.powers, ZaiStruct.Powers(_water,_fire,_metal,_air,_stone), _isGod[_zaiId]);
        _updateAllPowers(ZaiStruct.Powers(_water,_fire,_metal,_air,_stone));
    }

    function updateMana(uint256 _tokenId, uint256 _manaUp, uint256 _manaDown, uint256 _maxUp) external onlyAuth returns(bool){
    ...
    function updateStatus(uint256 _tokenId, uint256 _newStatusID, uint256 _center) external onlyAuth {
    ...
    function getStatus(uint256 _tokenId) external view returns(uint256[2] memory){
        return [_zai[_tokenId].activity.statusId,_zai[_tokenId].activity.onCenter];
    }
    ...
    function getZaiState(uint256 _tokenId) external view returns(string memory){
        return zaiMeta.getState(_zai[_tokenId].metadata.state);
    }

    function burnZai(uint256 _tokenId) external onlyAuth {
        _burn(_tokenId);
    }

    function _getRandom(address _to, uint256 _id) internal returns(uint256){
        _nonce = IOracle(gameAddresses.getOracleAddress()).getRandom(keccak256(abi.encodePacked(_to, _id, _nonce)));
        return _nonce;
    }
