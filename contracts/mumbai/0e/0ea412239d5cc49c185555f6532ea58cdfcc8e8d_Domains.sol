...
constructor(string memory _tld) payable ERC721("Doge Name Servies","DNS"){
    tld = _tld;
}

function price(string calldata name )public pure returns(uint){
    uint lent =StringUtils.strlen(name);
    require(lent >0,'Name length must tran 0');
    if(lent == 3){
        return 5 * 10**16;
    }else if(lent == 4){
        return 3 *10**16;
  ...
  function register(string calldata name) public payable {
        ...
        bytes(
            ...
            abi.encodePacked(
                '{"name": "',
                _name,
                '", "description": "A domain on the Doge name service", "image": "data:image/svg+xml;base64,',
  ...
  function setRecord(string calldata name,string calldata record) public{
      require(domains[name] == msg.sender,'error ');
      records[name] = record;
  }
