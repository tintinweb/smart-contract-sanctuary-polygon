...
contract Web3RSVP {
    ...
    struct CreateEvent {
        bytes32 eventId;
        string eventDataCID; //IPFS hash with events' data, to avoid entering this info on-chain (expensive)
        address eventOwner;
        uint256 eventTimestamp;
        uint256 deposit;
        uint256 maxCapacity;
    ...
    mapping(bytes32 => CreateEvent) public idToEvent; //Relations a unique eventID to the struct defined

    function createNewEvent(
        uint256 eventTimestamp,  //When the event will start
        uint256 deposit,         //Deposit required to RSVP to this event
        uint256 maxCapacity,
        string calldata eventDataCID  //IPFS hash containing information like the event name and description
    ) external { //external is highly performant and saves on gas.
