...
//  mapping(eventId => associated promoCodes[])
    ...
    function createEvent(Event calldata eventDetails, TicketType[] calldata ticketTypes)public onlyOwner{
    ...
    function createTicketTypes(uint eventId, TicketType[] calldata ticketTypes) internal returns(uint[] memory){
    ...
    function createPromoCodes(PromoCode[] calldata promoCodes) public onlyOwner{
    ...
    function updatePromoCode(uint promoCodeId, PromoCode calldata updatedPromoCode) public onlyOwner {
    ...
    function deletePromocode(uint promoCodeId) public onlyOwner{
        checkPromoCodeExists(promoCodeId);
    ...
    function buyTicketInCrypto(uint eventId, uint ticketTypeId, uint amountToBuy, uint promoCodeId) public payable{
        uint currentTimestamp = block.timestamp;
    ...
    function getReqTicketPrice(uint eventId, uint ticketTypeId, uint promoCodeId) public view returns (uint, uint){
        isValidDiscountAmouont(_ticketTypeDetails[ticketTypeId], _promoCodeDetails[promoCodeId]);
        ...
        return promoCodeId == 0 ? (actualPrice, serviceFee) : (discountedPrice, serviceFee);
    }

    function buyTicketInFiat(address buyer, uint eventId, uint ticketTypeId, uint amountToBuy) public onlyOwner{
        uint currentTimestamp = block.timestamp;
    ...
    function updateEvent(uint eventId, Event calldata updatedEventDetails) public onlyOwner{
        checkEventExists(eventId);
    ...
    function updateTicketType(uint eventId, uint ticketTypeId, TicketType memory updatedTicketType) public onlyOwner {
        checkEventExists(eventId);
        checkEventNotPaused(eventId);
        checkTicketTypeExists(ticketTypeId);
        checkEventHasTicketTypeId(eventId, ticketTypeId);
    ...
    function addTicketTypes(uint eventId, TicketType[] calldata ticketTypes) public onlyOwner{
        checkEventExists(eventId);
        checkEventNotPaused(eventId);
        isValidTicketType(_eventDetails[eventId], ticketTypes);
    ...
    function deleteTicketType(uint eventId, uint ticketTypeId) public onlyOwner{
        checkEventExists(eventId);
        checkEventNotPaused(eventId);
        checkTicketTypeExists(ticketTypeId);
        checkEventHasTicketTypeId(eventId, ticketTypeId);
    ...
    function updateEventStatus(uint eventId, bool status) public onlyOwner{
        checkEventExists(eventId);
    ...
    function isValidEvent(Event calldata eventDetails) private view {
        ...
        if (eventDetails.startDate < currentTimestamp || eventDetails.endDate < currentTimestamp || eventDetails.startDate >= eventDetails.endDate || eventDetails.servicePercentage == 0 || eventDetails.royaltyPercentage == 0){
    ...
    function associatePromoCodeWithEvents(PromoCode calldata promoCode) private{
        for(uint i; i < promoCode.eventIds.length; i++){
...
contract Reselling is Ownable, Auctions{
    ...
    function createEventPackageWithFixedPrice(EventPkg calldata pkg) public {
        isValidEventPkg(pkg);
    ...
    function createEventPackageWithAuction(EventPkg calldata pkg, Auction calldata auction) public {
        isValidEventPkg(pkg);
        isEligibleForPkgCreation(pkg);
    ...
    function updateEventPkg(uint eventPkgId, EventPkg calldata updatedPkg) public{
    ...
    function deleteEventPkg(uint eventPkgId) public {
        checkEventPkgExists(eventPkgId);
        require(msg.sender == _eventPkgDetails[eventPkgId].seller, "Reselling: Not an owner of package");
    ...
    function buyEventPkgWithFixedPrice(uint eventPkgId) public payable{
        checkEventPkgExists(eventPkgId);
        ...
        if (_eventPkgDetails[eventPkgId].reSaleType == ReSaleType.fixedPrice){
            EventPkg memory pkg = _eventPkgDetails[eventPkgId];
            ...
            _ticketsERC1155Contract.safeBatchTransferFrom(
                pkg.seller,
                msg.sender,
                pkg.ticketTypeIds,
                pkg.copies,
    ...
    function endAuctionForEventPkg(uint eventPkgId) public{
        checkEventPkgExists(eventPkgId);
        require(msg.sender == _eventPkgDetails[eventPkgId].seller, "Reselling: Not an owner of package");
        checkUserBalance(_eventPkgDetails[eventPkgId].seller, _eventPkgDetails[eventPkgId].ticketTypeIds, _eventPkgDetails[eventPkgId].copies);
    ...
    // function updateLockedCopies(uint eventPkgId, EventPkg calldata pkg) private{
    //     uint ticketTypeIdsLength = _eventPkgDetails[eventPkgId].ticketTypeIds.length;
        
    //     for (uint i; i < ticketTypeIdsLength; i++){
    //         _lockedCopies[pkg.seller][_eventPkgDetails[eventPkgId].ticketTypeIds[i]] -= _eventPkgDetails[eventPkgId].copies[i];
    //     }
...
contract TicketsERC1155 is ERC1155, Ownable, Pausable, ERC1155Burnable {
    ...
    modifier onlyAllowed(address caller){
        require(caller == _adminContract || caller == owner(), "TicketsERC1155: Caller is not authorized");
        _;
    }
    ...
    function setAdminContract(address adminContract) public onlyOwner{
        require(adminContract != address(0), "TicketsERC1155: Admin contract's address argument is null");
        _adminContract = adminContract;
    }

    function setResellingContract(address resellingContract) public onlyOwner{
        require(resellingContract != address(0), "TicketsERC1155: Reselling contract's address argument is null");
        ...
        onlyAllowed(msg.sender) isValidAdminContract
    {
        ...
        if (!isApprovedForAll(account, _adminContract) || !isApprovedForAll(account, _resellingContract)){
