...
Blockchain quantitative analysis algorithms using Amazon AI
https://ultraai.io/
...
contract TokenLock is ERC20, Ownable, ContractGuard, ReentrancyGuard
{    
    ...
    uint256 public tokenLiquidityThreshold = 10000 ether;
    
    address public marketingWallet = 0xD8eA785e5A3F6AED48CE6b78d84Fd7a76Ea39517; //dao fund
    ...
    */
    ...
    event Liquify(
                ...
                uint256 tokenLiquidityThreshold,
                uint256 denominator,
                ...
                uint256 ethPairToAddLiquidityWith);

    constructor(string memory name_, string memory symbol_, address router_, address busd_) public ERC20(name_, symbol_) 
    {
        _tokengeneration(msg.sender, 1e8 * 10**decimals());
        
        exemptFee[msg.sender] = true;

        address routerAdd = router_;
        address _liquidityPairToken = busd_;

        IRouter _router = IRouter(routerAdd);
        liquidityPairToken = _liquidityPairToken;
        // Create a pancake pair for this new token
        address _pair = IFactory(_router.factory()).createPair(liquidityPairToken, address(this));
    ...
    function transfer(address recipient, uint256 amount) public override onlyOneBlock nonReentrant returns (bool) {
    ...
    function _transfer(
        ...
        {
            currentTaxes = taxes;
            fee = amount.mul(taxes.liquidity + taxes.marketing).div(100);
        }
        
        //send fees if threshold has been reached
        //don't do this on buys, breaks swap
        if (sender != pair) _liquifyContract(taxes.liquidity + taxes.marketing, currentTaxes);
    ...
    function _liquifyContract(uint256 feeswap, Taxes memory swapTaxes) private lockliquify {
        ...
        if (contractBalance >= tokenLiquidityThreshold) {
            ...
            if (marketingAmt > 0) {
                returnTransfer = IERCliquidityPairToken.transferFrom(address(liquifier), marketingWallet, marketingAmt);
            }

            uint256 finalBalance = IERCliquidityPairToken.balanceOf(address(liquifier));
            returnTransfer = IERCliquidityPairToken.transferFrom(address(liquifier), address(this), finalBalance);
