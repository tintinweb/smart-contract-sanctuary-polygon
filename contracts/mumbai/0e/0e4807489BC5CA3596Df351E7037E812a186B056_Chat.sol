...
contract Chat {
    ...
    function incrementGrpID() public returns (uint){
        uint OldGroupID = GroupID;
        GroupID = GroupID + 1;
        return OldGroupID;
    }
  ...
  //--------------------------------------------------------------------------------------   
   ...
   require(walletAddress != address(0), "User with given address does not exist.");
    ...
    function sendMessage(address receiver, string calldata subject, string memory message, string memory fileHash, uint GroupID) external {
        require(
