...
contract DecentralizedAI {
    ...
    event ApiKeyGenerated(address indexed user, bytes32 indexed modelId, address indexed modelOwner, bytes32 apiKey, address secondPerson);
    ...
    function generateApiKey(bytes32 modelId, uint256 maxTokens, address modelOwner, address secondPerson) public returns (bytes32) {
        ...
        // Make the API key visible to the selected person running the model and the user generating the API key, and the specified second person
        require(msg.sender == modelOwner || msg.sender == tx.origin || msg.sender == secondPerson, "Unauthorized access to API key");
        allowances[msg.sender][modelId] -= maxTokens;
        emit ApiKeyGenerated(msg.sender, modelId, modelOwner, apiKey, secondPerson);
        return apiKey;
    }
