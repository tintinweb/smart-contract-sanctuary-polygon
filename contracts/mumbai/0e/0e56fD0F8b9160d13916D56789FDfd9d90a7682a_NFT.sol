...
contract NFT is ERC721Enumerable, Ownable {
    ...
    function mint(string calldata _message, string calldata _color) external {
        require(keccak256(abi.encodePacked(_color)) == keccak256(abi.encodePacked("red")), "COLOR_IS_NOT_VALID");
    ...
    function buildMetadata(uint256 _tokenId) private view returns (string memory) {
        ...
        return string(
            abi.encodePacked(
                ...
                Base64.encode(
                    bytes(
                        abi.encodePacked('{"name":"', _word.name, '", "description":"', _word.description, '", "image": "', 'data:image/svg+xml;base64,', image, '"}')
                    )
                )
            )
        );
    ...
    function buildImage(string memory _message, string memory _color) private pure returns (string memory) {
        return
            Base64.encode(
                bytes(
                    abi.encodePacked(
