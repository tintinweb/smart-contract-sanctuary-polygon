...
contract Greeter is RouterCrossTalk {
    ...
    function greetINT() public view returns (uint) {
        return greetingINT;
    }
    ...
    function setGreetingINTCrossChain(
        uint8 _chainID,
        uint _greetingINT,
        uint256 _crossChainGasLimit,
        uint256 _crossChainGasPrice
        ...
        nonce = nonce + 1;
        bytes memory data = abi.encode(_greetingINT);
        bytes4 _selector = bytes4(keccak256("setGreetingINT(uint)"));
        (bool success, bytes32 hash) = routerSend(
            _chainID,
            _selector,
            data,
    ...
    function _routerSyncHandler(bytes4 _selector, bytes memory _data)
        ...
        returns (bool, bytes memory)
    {
        uint _greetingINT = abi.decode(_data, (uint));
        (bool success, bytes memory data) = address(this).call(
            abi.encodeWithSelector(_selector, _greetingINT)
        );
        return (success, data);
    }
    ...
    function setGreetingINT(uint _greetingINT) external isSelf {
        console.log("Changing greetingINT from '%n' to '%n'", greetingINT, _greetingINT);
        greetingINT = _greetingINT;
    }
