...
contract Ticketing is ERC721, Ownable{
    ...
    constructor(address _registrationContract, address _usdtTokenAddress) ERC721("EventBriteTickets", "EVNT-TIC") {
    ...
    EventBriteCoin usdt = EventBriteCoin(usdtTokenAddress);
    ...
    function buyTickets(uint _userId, uint _eventId, uint _ticketCount) public {
            ...
            require(usdt.balanceOf(msg.sender) >= ticketPrice, "Insufficient USDT balance"); 
            ...
            // It will deduct the USDT balance from the user
            uint totalPrice = ticketPrice * _ticketCount;
            IERC20(usdt).approve(address(this), totalPrice);
            IERC20(usdt).transferFrom(msg.sender, address(this), totalPrice);
            

            // Minting NFTs as per user request
    ...
    function getUserDetailsStruct(uint256 _userId) external view returns(
        EventInformation memory _userDetails){
        return (
        linkIdToEvent[_userId]
        );
    }
}