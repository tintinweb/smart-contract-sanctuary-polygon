...
contract StakingSBT is ERC721Psi, IERC4906, Ownable {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return renderer.tokenUri(uint16(thisRank-1));
    }

    function existsCheck(uint256 tokenId) external view returns (bool) {
        return _exists(tokenId);
    }

    function getURIOnly(uint256 tokenId) external view returns (string memory) {
        return renderer.tokenUri(uint16(tokenId));
    }
