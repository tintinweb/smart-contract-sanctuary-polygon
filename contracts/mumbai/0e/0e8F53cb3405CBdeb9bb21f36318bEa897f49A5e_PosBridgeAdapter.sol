...
contract PosBridgeAdapter is IBridgeAdapter, Ownable {
    ...
    function sendAssets(
        ...
        require(
            (to != address(0)) || (notifyVault),
            "Can't deposit to vault without notification"
        );
        bytes memory data = abi.encode(
            keccak256(
                abi.encode(nonce++, uint256(block.chainid), address(this))
            ),
            dstChainId,
            value,
            dstAsset,
    ...
    function pullAssets(
    ...
    ) external returns (uint256 pooled) {
        bytes32 transferId = keccak256(data);
        (
            ,
            uint256 neededChainId,
            uint256 value,
            address asset,
            address receiver,
            bool notifyVault
        ...
        require(
            neededChainId == block.chainid,
            "that is not the destination chain"
        );
...
contract Worker is Ownable {
    ...
    function pushTransfer() public {
        require(
            ...
            "Can be called only by external account"
        );
