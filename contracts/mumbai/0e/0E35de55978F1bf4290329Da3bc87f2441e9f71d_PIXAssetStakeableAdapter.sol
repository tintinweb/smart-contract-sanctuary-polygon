...
contract PIXAssetStakeableAdapter is IMissionControlStakeable, OwnableUpgradeable, ERC1155HolderUpgradeable {
    enum Distribution {
        Poisson,
        LinearDeterministic,
        Facility
    ...
    uint256[] private leadTimes;
    uint256[] private prodAmounts;
    ...
    function setTokenInfo(
        ...
        tokenInfo[_tokenId][_x][_y].isStakeable = _isStakeable;
        tokenInfo[_tokenId][_x][_y].isBase = _isBase;
        tokenInfo[_tokenId][_x][_y].isRaidable = _isRaidable;
        tokenInfo[_tokenId][_x][_y].class = _class;
        tokenInfo[_tokenId][_x][_y].resourceEV = _resourceEv;
        tokenInfo[_tokenId][_x][_y].resourceId = _resourceId;
        tokenInfo[_tokenId][_x][_y].resourceCap = _resourceCap;
        //cutoff should update by tile
        __update_cutoff(_tokenId, _resourceEv, _resourceCap, _x, _y);
        emit TokenInfoUpdated(_isStakeable, _isBase, _isRaidable, _class, _resourceEv, _resourceId, _resourceCap);
    }

    function setStructureBonuses(
        int256 _x,
        int256 _y,
        uint256 _bonus
    ) external onlyOwner {
    ...
    function __update_cutoff(
        ...
        cutoffs[_id][_x][_y] = CUTOFF_COEF * (_cap / _ev) + CUTOFF_INTERCEPT;
    }
    ...
    function setERC1155(address _erc1155) external onlyOwner {
        require(address(erc1155Token) == address(0), "MC1155_ADAPTER: Erc1155 has already been set");
    ...
    function setDroneTier(uint256[] memory _droneId, uint256[] memory _droneTier) external onlyOwner {
    ...
    function isBase(
        ...
        _isBase = tokenInfo[_tokenId][_x][_y].isBase;
    }
    ...
    function isRaidable(
        ...
        _isRaidable = tokenInfo[_tokenId][_x][_y].isRaidable;
    }
    ...
    function onStaked(
        ...
        require(tokenInfo[_tokenId][_x][_y].isStakeable, "MCS1155_ADAPTER: Token not stakeable");
        require(seeds[_currentOwner][_nonce] == 0, "MCS1155_ADAPTER: Nonce already used");
        if (_besideToken == address(this))
            require(_tokenId != _besideTokenId, "MCS1155_ADAPTER: Trying to stake duplicate tech");
        if (tokenInfo[_tokenId][_x][_y].class == Distribution.Facility) {
            require(
                __handleFacilitySetup(_tokenId, _currentOwner, _underlyingToken, _underlyingTokenId, _nonce),
                "MCS1155_ADAPTER: Facility must be staked on correct tier"
            );
            // Facility can't be beside anything
            require(_besideToken == address(0), "MCS1155_ADAPTER: Facility Cannot Be Staked alongside anything else");
        }
        // Facility can't be beside anything
        require(
            tokenInfo[_besideTokenId][_x][_y].class != Distribution.Facility,
        ...
        // Drone buff logic, need to replace tokenId with the relevant tier
        if (
            droneTier[_tokenId] != 0 &&
            _besideToken == rover &&
    ...
    /// @inheritdoc IMissionControlStakeable
    function checkTimestamp(address _userAddress, uint256 _nonce)
        external
        view
        override
        onlyMissionControl
    ...
    /// @inheritdoc IMissionControlStakeable
    function onUnstaked(address _newOwner, uint256 _nonce)
        external
        override
        onlyMissionControl
        onlyStakedTokens(_newOwner, _nonce)
    {
    ...
    function __timeLeft(
        ...
        Distribution class = tokenInfo[tokenId][_x][_y].class;
    ...
    function __timeLeft_deterministic_linear(
        ...
        uint256 completionTime = (DAY_IN_SEC / tokenInfo[tokenId][_x][_y].resourceEV) *
            tokenInfo[tokenId][_x][_y].resourceCap +
            localSeed;
    ...
    function __timeLeft_facility(
        ...
        uint256 output = (facilityOutput[_userAddress][_nonce] * structureBonuses[_x][_y]) /
            STRUCTURE_BONUS_DENOMINATOR;

        uint256 completionTime = (tokenInfo[tokenId][_x][_y].resourceCap * facilityLeadTime[_userAddress][_nonce]) /
            output +
            localSeed;
        int256 timeRemaining = int256(completionTime) - int256(block.timestamp);
        return timeRemaining > 0 ? timeRemaining : int256(0);
    ...
    function __check_tile(
        ...
        if (class == Distribution.LinearDeterministic) {
            return __checkTile_deterministic_linear(_userAddress, tokenId, _nonce, _pausedAt, _x, _y);
        }
        if (class == Distribution.Facility) {
    ...
    function __checkTile_facility(
        ...
        uint256 ret = (facilityOutput[_userAddress][_nonce] *
            ((untilTimestamp - localSeed) / facilityLeadTime[_userAddress][_nonce]));

        if (ret > tokenInfo[tokenId][_x][_y].resourceCap) {
            ret = tokenInfo[tokenId][_x][_y].resourceCap;
        }

        ret = (ret * structureBonuses[_x][_y]) / STRUCTURE_BONUS_DENOMINATOR;
        return (ret, tokenInfo[tokenId][_x][_y].resourceId);
    }
    ...
    function __checkTile_deterministic_linear(
        ...
        require(localSeed != 0, "MCS1155_ADAPTER: No such token staked");
        uint256 untilTimestamp = _pausedAt > 0 ? _pausedAt : block.timestamp;
        uint256 ret = (tokenInfo[tokenId][_x][_y].resourceEV * (untilTimestamp - localSeed)) / DAY_IN_SEC;

        if (ret > tokenInfo[tokenId][_x][_y].resourceCap) {
            ret = tokenInfo[tokenId][_x][_y].resourceCap;
        }

        if (droneBuffApplied[_userAddress][_nonce]) ret *= DRONE_BUFF / DRONE_BUFF_DENOMINATOR;
    ...
    function __checkTile_Poisson(
        ...
        require(localSeed != 0, "MCS1155_ADAPTER: No such token staked");
        uint256 ret = 0;
        if (tokenInfo[tokenId][_x][_y].resourceEV == 0) {
            return (ret, tokenInfo[tokenId][_x][_y].resourceId);
        }
        uint256 untilTimestamp = _pausedAt > 0 ? _pausedAt : block.timestamp;
        if (uint256(int256(untilTimestamp) - localSeed) > cutoffs[tokenId][_x][_y]) {
            return (tokenInfo[tokenId][_x][_y].resourceCap, tokenInfo[tokenId][_x][_y].resourceId);
        }
        uint256 localEV = tokenInfo[tokenId][_x][_y].resourceEV;
        uint256 localCap = tokenInfo[tokenId][_x][_y].resourceCap;
    ...
    */
    ...
    function _exponential_2(
        ...
        exponential_sample = -_max_ln(
            MAX_SCALE -
                _max_div(MAX_SCALE * int256(_uniform(_playerAddress, tokenId, seed)), MAX_SCALE * int256(PROBA_RES))
        );
    }
