...
contract BKSNFT is ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    constructor(address busdAddr, address oracleAddr) ERC721("BKSNFT", "NFT") {
        owner_ = msg.sender;
        BUSD = ERC20(busdAddr);
        _ORACLE = IStdReference(oracleAddr);
        priceFeed = AggregatorV3Interface(0x0bf79F617988C472DcA68ff41eFe1338955b9A80);
    }
    ...
    function mintNFT(address recipient, string memory _tokenURI, string memory eventName) public onlyOwner {
        // require(!tokenURIExists[_tokenURI], "This ticket already minted");
        require(eventInfo[eventName].number > eventInfo[eventName].minted, "You can't mint any more!");
        eventInfo[eventName].minted += 1;
        _tokenIds.increment();
    ...
    function addEvent(
        ...
        emit EventAdded(eventName, walletAddress, payees, fees, price, ticketNumber);
    }
    ...
    function updateEvent(
        ...
        emit EventUpdated(eventName, msg.sender, payees, fees, price);
    }
    ...
    function getUSDRate(uint256 eur) public view returns (uint256) {
        ...
        return (((eur * 100 / 100 *  DOUBLE)) * DECIMAL) / DOUBLE;
    }
    ...
    function payWithBUSD(
        ...
        // uint256 amount = (getUSDRate(eventInfo[eventName].price) * _amount) /
        //     DOUBLE;
        uint256 amount = uint256(getLatestPrice()) * eventInfo[eventName].price * _amount * 10000000;
