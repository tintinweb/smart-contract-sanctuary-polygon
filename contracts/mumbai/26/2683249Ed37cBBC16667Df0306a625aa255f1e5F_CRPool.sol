 ...
 * @notice PCToken is the "Swarm Markets Smart Pool Token" token (transferred upon finalization)
...
contract CRPool is ICRPool, PCToken, OwnableUpgradeable, BalancerReentrancyGuard {
    ...
    event WhitelistedLiquidityProvider(address indexed caller, address indexed provider);
    event RemovedWhitelistedLiquidityProvider(address indexed caller, address indexed provider);
     ...
     *      createPool()
     *
     * NOTE: this function is originally onlyOwner, but we need to remove it in order to keep CRPool ownership
     ...
     */
    function getUpdateWeightCost(address token, uint256 newWeight) external view virtual override returns (uint256) {
        return SmartPoolManager.getUpdateWeightCost(IConfigurableRightsPool(address(this)), bPool, token, newWeight);
    }
    ...
    function commitAddToken(
        ...
        emit NewTokenCommitted(token, balance, denormalizedWeight, address(this), msg.sender);
    ...
    function applyAddToken() external virtual override logs nonReentrant needsBPool {
        ...
        emit NewTokenApplied(_newToken.addr, _newToken.balance, _newToken.denorm, address(this), msg.sender);
     ...
     */
    function getRemoveTokenCost(address token) external view virtual override returns (uint256) {
        return SmartPoolManager.getRemoveTokenCost(IConfigurableRightsPool(address(this)), bPool, token);
    }
    ...
    function whitelistLiquidityProvider(address provider) external onlyOwner nonReentrant logs {
        ...
        emit WhitelistedLiquidityProvider(_msgSender(), provider);
    ...
    function removeWhitelistedLiquidityProvider(address provider) external onlyOwner nonReentrant logs {
        ...
        emit RemovedWhitelistedLiquidityProvider(_msgSender(), provider);
...
contract PCToken is IERC20, Initializable {
    ...
    // State variables
    string public constant NAME = "Swarm Markets Smart Pool Token";
