...
/*
   "Distribute WEF Tokens to Borrowers".
   Borrowers of Davos token against collaterals are incentivized 
   to get WEF Tokens.
*/

contract WEFRewards is IRewards, Initializable {
    // --- Wrapper ---
    ...
    function initPool(address token, bytes32 ilk, uint256 rate) external auth {
        require(IERC20Upgradeable(wefToken).balanceOf(address(this)) >= poolLimit, "Reward/not-enough-reward-token");
    ...
    function setWEFToken(address wefToken_) external auth {
        require(wefToken_ != address(0), "Reward/invalid-token");
        wefToken = wefToken_;

        emit WEFTokenChanged(wefToken);
    }
    function setRewardsMaxLimit(uint256 newLimit) external auth {
        require(IERC20Upgradeable(wefToken).balanceOf(address(this)) >= newLimit, "Reward/not-enough-reward-token");
        poolLimit = newLimit;
    ...
    function setOracle(address oracle_) external auth {
        ...
        emit WEFOracleChanged(address(oracle));
    }
    ...
    // --- View ---
    function wefPrice() public view returns(uint256) {
        // 1 DAVOS is wefPrice() wefs
    ...
    function unrealisedRewards(address token, address usr) public poolInit(token) view returns(uint256) {
        ...
        return FullMath.mulDiv(rewards, wefPrice(), 10 ** 18);                           // WEF Tokens
    }
    ...
    function claim(uint256 amount) external {
        ...
        IERC20Upgradeable(wefToken).safeTransfer(msg.sender, amount);
