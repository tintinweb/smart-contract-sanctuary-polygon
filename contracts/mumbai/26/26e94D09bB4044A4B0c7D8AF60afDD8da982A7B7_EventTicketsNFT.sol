...
contract EventTicketsNFT is ERC1155Short, SignatureMintERC1155, EventOrgAccess, ERC1155Holder {
    ...
    struct EmailAddressLinkObject {
        string email;
        address ownerAddress;
    }

    struct AddressAmountByEmail {
        uint256 amount;
        address ownerAddress;
    }

    mapping(string => mapping(uint256 => AddressAmountByEmail)) public balanceOfByEmail;
    mapping(uint => EmailAddressLinkObject[]) private emailsStructByTokenId;
    ...
    function getEmailStructsByTokenId(uint256 _tokenId, uint256 offset, uint256 _limit) external view returns(EmailAddressLinkObject[] memory) {
        uint256 limit = _limit;


        if (offset > emailsStructByTokenId[_tokenId].length) {
            return new EmailAddressLinkObject[](0);
        }

        if (limit > emailsStructByTokenId[_tokenId].length - offset) {
            limit = emailsStructByTokenId[_tokenId].length - offset;
        }

        EmailAddressLinkObject[] memory values = new EmailAddressLinkObject[](limit);
        for (uint256 i = 0; i < limit; i++) {
            values[i] = emailsStructByTokenId[_tokenId][offset + i];
        }
    ...
    function _mintProcess(MintRequest calldata _req, address signerOrMinter) internal {
        ...
        if (bytes(_req.email).length > 0) {
            emailsStructByTokenId[tokenIdToMint].push(EmailAddressLinkObject(_req.email, _req.to));
        }

        if (_req.to == address(0)) {
            require(bytes(_req.email).length > 0, "Email must be set");
            balanceOfByEmail[_req.email][tokenIdToMint].amount += _req.quantity;
            balanceOfByEmail[_req.email][tokenIdToMint].ownerAddress = _req.to;
    ...
    function transferFromEmail(string memory email, address _to, uint tokenId) external {
        ...
        uint amount = balanceOfByEmail[email][tokenId].amount;
        _safeTransferFrom(address(this), _to, tokenId, amount, "");
        balanceOfByEmail[email][tokenId].amount -= amount;
        balanceOfByEmail[email][tokenId].ownerAddress = _to;
    }
...
contract ERC1155Short is ERC1155, Ownable, Royalty, Multicall {
