...
contract ERC1155 is Context, ERC165, IERC1155, IERC1155MetadataURI {
     ...
     *
     * - `to` cannot be the zero address.
     * - `from` must have a balancesetApprovalFor of tokens of type `id` of at least `amount`.
     * - If `to` refers to a smart contract, it must implement {IERC1155Receiver-onERC1155Received} and return the
     * acceptance magic value.
     */
...
// File: ERC1155/ERC1155Supply.sol
...
// File: ERC1155/ERC1155Burnable.sol
...
contract MYS_ERC1155 is Context, AccessControlEnumerable, ERC1155Burnable, SystemWideRoyalties_ERC1155, IMYS_ERC1155 {
    bytes32 private constant MINTER_ROLE = keccak256("MINTER_ROLE");

    enum Role { admin, minter }

    uint256 private _tokenIDs;
    
    mapping(uint256 => uint256) private _tokenToReleaseTokenDate;
    ...
    constructor(address _payment, string memory contractMetadataUri, string memory _name, string memory _symbol) SystemWideRoyalties_ERC1155(_payment) {
        name = _name;
        symbol = _symbol;
    ...
    function isTokenReleased(uint256 _tokenId) view external returns(bool) {
        return block.timestamp >= _tokenToReleaseTokenDate[_tokenId];
    }
    
    function updateReleaseDate(uint256 _tokenId, uint256 _newDate) public onlyRole(MINTER_ROLE) {
        _tokenToReleaseTokenDate[_tokenId] = _newDate;
    }

    function getTokenReleaseDate(uint256 _tokenId) view public onlyRole(MINTER_ROLE) returns(uint256) {
        return _tokenToReleaseTokenDate[_tokenId];
    }
    ...
    function updatePlatform(address _address) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(IERC165(_address).supportsInterface(type(IPaymentAccount).interfaceId), "Invalid IPaymentAccount address");
        _platform = IPaymentAccount(_address);
    }
    ...
    function mint(
        ...
        require(hasRole(MINTER_ROLE, _msgSender()), "MYS_ERC1155: must have minter role to mint");
        require(receivers.length > 0, "Receivers must not be empty");

        uint256 id = _tokenIDs;

        _setRoyalties(id, receivers, basisPoints);
        _tokenIDs++;
        _setURI(uri, id);
        _mint(to, id, amount, data);
        _tokenToReleaseTokenDate[id] = releaseDate;
        return id;
    }
    ...
    function mintBatch(
        ...
        for (uint256 i; i < count; i++) {
            ...
            ids[i] = tokenId;
            _setURI(uris[i], tokenId);
            _setRoyalties(tokenId, receivers, basisPoints);
            _tokenToReleaseTokenDate[tokenId] = releaseDate;
        }
        _tokenIDs = _tokenIDs + count;
        _mintBatch(to, ids, amounts, data);
        return ids;
    }

    function setURI(string memory uri, uint256 id) external {
        require(hasRole(MINTER_ROLE, _msgSender()), "MYS_ERC1155: must have minter role to set uri");
        _setURI(uri, id);
    }
    ...
    function setContractURI(string memory uri) external {
        require(hasRole(MINTER_ROLE, _msgSender()), "MYS_ERC1155: must have minter role to set contractUri");
        _contractURI = uri;
    }
    ...
    function setRoyalties(uint256 tokenId, address payable[] calldata receivers, uint16[] calldata basisPoints) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _setRoyalties(tokenId, receivers, basisPoints);
    }
    ...
    view
    virtual
    override(ERC1155, SystemWideRoyalties_ERC1155, AccessControlEnumerable, IERC165)
    returns (bool)
    {
        return interfaceId == type(ISystemWideRoyalties).interfaceId ||
               interfaceId == type(IERC1155).interfaceId ||
               interfaceId == type(IERC1155MetadataURI).interfaceId || 
               interfaceId == type(IERC2981).interfaceId ||
               interfaceId == type(IMYS_ERC1155).interfaceId ||
               interfaceId == type(IERC165).interfaceId ||
    ...
    function _beforeTokenTransfer(
        ...
        if (this.isTokenReleased(ids[0]) == false) {
            require(hasRole(MINTER_ROLE, _msgSender()), "MYS_ERC1155: must have minter role to transfer unreleased token");
        }
        super._beforeTokenTransfer(operator, from, to, ids, amounts, data);
    }

    /**
    * @dev See {SystemWideRoyalties}
    */

    function getCreatorAddress(uint256 tokenId) internal view virtual override returns (address) {
        return _creator[tokenId];
    }

    function setupRoles(Role[] memory roles, address account) public onlyRole(DEFAULT_ADMIN_ROLE) {
        for (uint16 i; i < roles.length; i++) {
            _setupRole(_to(roles[i]), account);
        }
    }

    function revokeRoles(Role[] memory roles, address account) public onlyRole(DEFAULT_ADMIN_ROLE) {
        for (uint16 i; i < roles.length; i++) {
             _revokeRole(_to(roles[i]), account);
        }
    }

    function _to(Role role) pure private returns(bytes32) {
        if (role == Role.minter) {
            return MINTER_ROLE;
        }
