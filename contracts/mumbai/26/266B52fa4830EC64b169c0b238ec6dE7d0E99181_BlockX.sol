...
contract BlockX is ERC20, ERC20Burnable, Ownable {
    ...
    function lockAndTransfer(address walletAddress, uint256 amount, uint256 vestStart, uint256 vestPeriod)  public onlyAdmin returns (bool) {
    ...
    function multipleLockAndTransfer(address[] memory walletAddresses, uint256[] memory amounts, uint256 vestStart, uint256 vestPeriod)  public onlyAdmin returns (bool) {
        require(vestStart > block.timestamp, "Vest Start Date should be later than now");
        require(vestPeriod > 0, "Vest Period is too short");
    ...
    function getLockedPerAgreement(address walletAddress, uint index) public view returns (uint256) {
        ...
        if (block.timestamp > currentVest.vestStart) {
            if (((block.timestamp - currentVest.vestStart) / 60) <= currentVest.vestPeriod) {
                unLockedAmount = currentVest.totalAmount * ((block.timestamp - currentVest.vestStart) / 60) / currentVest.vestPeriod;
