...
contract PolicyDetails_Covest is Initializable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event BlacklistAssetsUserChanged(address[] assets, address[] user, bool[] boolean);
    ...
    function isPolicyActive(string memory _policyId) public view returns(bool isActive) {
         if (
            ...
            isBlacklistAssetsUser(_policyIdData[_policyId].user,_policyIdData[_policyId].asset) == true
        ){
            return false;
        } else if (
     ...
     */
    function _setFundAllocationWeight(uint256[] memory _fundAllocationWeight) private {
    ...
    function setFundAllocationWeight(uint256[] memory _fundAllocationWeight)
        ...
        isSuperManager
    {
        require(_fundAllocationWeight.length == 7, "Percent should be have 7 arguments.");
    ...
    function issuePolicy(bytes calldata _data) public isReady onlyPolicyManager {
        ...
        require(mempercent.length == 7,"Invaild Length of fundAllocationWeight");
        ...
        returns (bool, string memory)
    {
        ...
        if (
            !((
                _policyIdData[_policyId].fundAllocationWeight
            ).length == 7)
        ) {
        ...
        )
    {
        ...
        _weightRequest[_asset][redeemId] = _policyIdData[_policyId].fundAllocationWeight;
