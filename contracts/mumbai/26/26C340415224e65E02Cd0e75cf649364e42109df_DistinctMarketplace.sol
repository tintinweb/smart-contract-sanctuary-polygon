    ...
    Whitelist
{
    ...
    function buyDropItemsWorking(
        ...
        require(_isUSDT(paymentToken) || _isWETH(paymentToken), 'Distinct: not WETH or USDT');

        (
            uint256 totalSellerRevenue,
            uint256 totalDistinctFee
        ) = getSellerRevenueAndDistinctFeeTotal(
            drop,
            _msgSender(),
            tokenIds,
            amounts,
        ...
        IERC20Upgradeable(paymentToken).transferFrom(_msgSender(), drop.seller, totalSellerRevenue);
        IERC20Upgradeable(paymentToken).transferFrom(_msgSender(), address(this), totalDistinctFee);
        ...
        //     IERC20Upgradeable(paymentToken).transferFrom(_msgSender(), drop.seller, dropItem.priceInUSDT*amounts[i]);
        // }
    ...
    function _isWETH(address paymentToken) internal view returns (bool) {
        return paymentToken == address(WETH);
    }
    ...
    function getSellerRevenueAndDistinctFeeTotal(
        ReserveSale storage drop,
        address account,
        uint256[] memory tokenIds,
        uint256[] memory amounts,
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            totalSellerRevenue = itemsPrice - 10;
            totalDistinctFee = 10;
        }
    }
    ...
    function getItemsAmount(uint256 saleId) external view returns(uint256 [] memory amounts){
        return _getItemsAmount(saleId);
    }

    function _getItemsAmount(uint256 saleId) internal view returns(uint256 [] memory amounts) {
