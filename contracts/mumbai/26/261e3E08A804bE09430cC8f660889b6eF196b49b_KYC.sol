...
contract KYC is BaseContract {
    ...
    struct User {
        ...
        string profession;
        bool smoking;
        bool insurancePlan; // Address of the hospital only if KYC gets verified
    ...
    event NewUserRegistered(address);
    event UserInfoUpdated(address);
    ...
    function getAllUserInfo() external view returns (User[] memory) {
        User[] memory users = new User[](userList.length);
        for (uint256 i = 0; i < userList.length; i++) {
            users[i] = userInfo[userList[i]];
        }
        return users;
    }
    ...
    function isUserRegistered(address _user) public view returns (bool) {
        for (uint256 i = 0; i < userList.length; i++) {
            if (_user == userList[i]) return true;
        }
        return false;
    }

    function updateUserStatus(address _user, ActiveStatus _status)
        external
        isAdmin
    {
        require(isUserRegistered(_user), "Unregistered User");
        userInfo[_user].active = _status;
    }
