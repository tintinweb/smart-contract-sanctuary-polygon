...
contract Utopia42Verse is AccessControl{
    ...
    function adminUpdateLands(
        ...
        ownerLands[_owner].push(landId);
        emit Assign(landId, x1, x2, y1, y2, _owner, _hash);
    }

    // function adminBatchUpdateLands(
    ...
    //     uint256[] memory ownerIndex
    // ) public onlyUtopia42DAO {
    //     for (uint i = 0; i < landId.length; i++) {
    //         lands[landId[landId[i]]] = Land(
    //             landId[i],
    //             x1[i],
    //             x2[i],
    //             y1[i],
    ...
    //             ownerIndex[i]
    //         );
    //         ownerLands[_owner[i]].push(landId[i]);
    //         emit Assign(landId[i], x1[i], x2[i], y1[i], y2[i], _owner[i], _hash[i]);
    //     }
    // }
