    ...
    Base
{
    ...
    function getStakedSoldiersByLevel(uint8 level)
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            tokenIds[i] = _soldiersStakedLevelMapping[level][tokenIds[i]];
        }        
    }
        ...
        onlyOwner
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 earnings = (totalShinMinted *
                ...
                100 / NUMBER_OF_GENESIS_LEGENDS -
                _chibiLegendClaimed[tokenIds[i]];
    ...
    function _claimCitizen(
        ...
        require(
            stakedDuration >= minimumTimeToClaim,
            string(
                abi.encodePacked(
        ...
        if (unstake) {
            _updateExpAndLevel(citizen, stakedDuration);
            if (isSoldier) {
    ...
    function _updateExpAndLevel(
        ...
        uint16 newExp = citizen.exp +
            uint16((EXP_PER_TIME_BLOCK * stakedDuration) / timeBlock);
        ...
        chibiCitizenContract.setLevel(citizen.tokenId, newLevel, newExp);
    }
     ...
     */
    function _payTaxToSoldiers(uint256 amount) internal {
...
interface IStakeChibiCitizen {
    /**
     * struct to store worker stake info
     */
    struct Stake {
        uint16 tokenId;
