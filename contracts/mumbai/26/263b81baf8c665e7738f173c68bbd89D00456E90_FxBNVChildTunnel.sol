...
contract FxBNVChildTunnel is FxBaseChildTunnel, Ownable, ReentrancyGuard {
    address private childToken;
    address private tokenSafe;
    ...
    function setChildToken(address _childToken) public onlyOwner {
        childToken = _childToken;
    }

    function _setFxRootTunnel(address _fxRootTunnel) public onlyOwner {
        fxRootTunnel = _fxRootTunnel;
    }

    function setTokenSafe(address _tokenSafe) public onlyOwner {
        tokenSafe = _tokenSafe;
    }

    function encodeMessage(address to, uint256 tokenId)
        internal
        pure
    ...
    function _processMessageFromRoot(
        ...
        (address to, uint256 tokenId) = decodeMessage(data);
        require(
            IBNVTokenBase(childToken).exists(tokenId),
            "Token does not exist!"
        );
        require(
            ERC721(childToken).ownerOf(tokenId) == tokenSafe,
            "Token is not in the safe!"
        );
        ERC721(childToken).safeTransferFrom(tokenSafe, to, tokenId);
    ...
    function withdraw(uint256 tokenId) external {
        ...
        require(
            msg.sender == IERC721(childToken).ownerOf(tokenId),
            "You are not the owner!"
        );
        bytes memory message = encodeMessage(msg.sender, tokenId);
        // withdraw tokens
        ERC721(childToken).safeTransferFrom(msg.sender, tokenSafe, tokenId);
