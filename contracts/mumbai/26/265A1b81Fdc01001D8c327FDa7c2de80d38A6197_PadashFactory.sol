...
import "./Padash.sol";
import "../../interface/IPadashFactory.sol";

contract PadashFactory is IPadashFactory {
  address public lastPeimaneh;

  event PadashCreated(address value);

  function createPadash(address[] memory _allowedRewardTokens) external override returns (address) {
    address _lastPeimaneh = address(new Padash(
        msg.sender,
        _allowedRewardTokens
      ));
    lastPeimaneh = _lastPeimaneh;
    emit PadashCreated(_lastPeimaneh);
    return _lastPeimaneh;
  }
}
...
import "../../interface/IPadash.sol";
import "../../interface/IERC721.sol";
import "../../interface/IVoter.sol";
import "../../interface/IVe.sol";
import "./MultiRewardsPoolBase.sol";

/// @title Padashs pay out rewards for a given pool based on the votes
///        that were received from the user (goes hand in hand with Peimanehs.vote())
contract Padash is IPadash, MultiRewardsPoolBase {
  ...
  ///      since it can only be called "internally" from Peimanehs
  ...
  /// @dev Used to notify a peimaneh/padash of a given reward,
...
interface IPadashFactory {
  function createPadash(address[] memory _allowedRewardTokens) external returns (address);
}
...
interface IVoter {
  ...
  function attachTokenToPeimaneh(uint _tokenId, address account) external;

  function detachTokenFromPeimaneh(uint _tokenId, address account) external;

  function emitDeposit(uint _tokenId, address account, uint amount) external;

  function emitWithdraw(uint _tokenId, address account, uint amount) external;

  function distribute(address _peimaneh) external;
...
abstract contract MultiRewardsPoolBase is Reentrancy, IMultiRewardsPool {
  ...
  function removeRewardToken(address token) external onlyOperator {
    ...
    // assume it will be FARS + pool tokens
    uint i = 3;
    bool found = false;
    for (; i < length; i++) {
