    ...
    ReentrancyGuardUpgradeable
{
    ...
    modifier isUWOrCAOrReferralOrReserve() {
        require(
            NR.getContract("UW") == msg.sender ||
                ...
                NR.getContract("R") == msg.sender,
            "Staking: you are not UW or CA or Referral or RiskManager or Reserve."
        );
        _;
    }
        ...
        assetsMI
    {
        ...
        _userStakingData[MITokenAddress][msg.sender][_type]
            .balance += _amount;
        _workData[_type].powerCount += _amount;
     ...
     *  - `isReady` The pool must not paused.
     */
    function payPenaltyBehalf(address _user,uint256 _amount, StakingTypes _type)
        public
        nonReentrant
        isReady
    {   
         address MITokenAddress = address(MI);
        require(
            checkRoleAccess(_user, _type) == true,
            "User are not access"
        );
        require(
            MI.allowance(msg.sender, address(this)) >= _amount,
        ...
        MI.transferFrom(msg.sender, address(this), _amount);
        _userStakingData[MITokenAddress][_user][_type]
            .penalty -= _amount;
...
interface IReferral {
	...
	function balanceOfReferral(address _user, address _asset) external view returns (uint256);
	...
	function isReferralNonCheckStake(address _user) external view returns (bool);

	function resignation(address _user) external;

	function whoIsRefer(string memory _policyId) external view returns (address);
