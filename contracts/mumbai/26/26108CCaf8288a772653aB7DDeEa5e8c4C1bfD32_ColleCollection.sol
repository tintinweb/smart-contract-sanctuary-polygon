...
contract ColleCollection is IColleCollection, SignatureValidator, ERC721URIStorage, MarketHubRegistrar {
    ...
    mapping(uint256 => string) internal tokenSaleMetadataIPFS;
    ...
    ) public override isValidSignature(_signature, _getPermitApproveHash(_signature, _to, _tokenId)) {
        address owner = ERC721.ownerOf(_tokenId);
        require(_to != owner, "ERC721: approval to current owner");
        require(
            _signature.signer == owner || isApprovedForAll(owner, _signature.signer),
            "ERC721: approve caller is not token owner or approved for all"
        );
    ...
    function permitSetApprovalForAll(
        ...
        bool _approve,
        Signature memory _signature
    ) public override isValidSignature(_signature, _getPermitSetApprovalForAllHash(_signature, _operator, _approve)) {
        _setApprovalForAll(_signature.signer, _operator, _approve);
    }
    ...
    function permitSafeTransfer(
        ...
        uint256 _tokenId,
        Signature memory _signature
    ) public override isValidSignature(_signature, _getPermitSafeTransferHash(_signature, _to, _tokenId)) {
        _safeTransfer(_signature.signer, _to, _tokenId, "");
    }
    ...
    function _getPermitApproveHash(
        ...
        return
            keccak256(
                abi.encode(
                    keccak256("Approve(address from,address to,uint256 tokenId,uint256 deadline,uint256 nonce)"),
                    _signature.signer,
                    _to,
                    _tokenId,
                    _signature.deadline,
    ...
    /**
     * @dev Internal function to get the expected EIP-712 signed hash to permit approving a token transfer
     * @param _signature The signature, deadline and signer from the owner of the tokens.
     * @param _operator The address to approve.
     * @param _approve Whether to approve or revoke approval
     */
    ...
    function _getPermitSafeTransferHash(
        ...
        return
            keccak256(
                abi.encode(
                    keccak256("SafeTransfer(address from,address to,uint256 tokenId,uint256 deadline,uint256 nonce)"),
                    _signature.signer,
                    _to,
                    _tokenId,
                    _signature.deadline,
