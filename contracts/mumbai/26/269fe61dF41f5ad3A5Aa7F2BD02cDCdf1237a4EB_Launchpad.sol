...
contract Launchpad {
    ...
    function getPresaleDetail1(uint256 _counter) internal view returns(address _tokenAddress,
    address _seller,string memory _imageurl){
       details storage pre = presaleDetails[(_counter)];
       return(pre.tokenAddress,pre.seller,pre.imageURL);
    }
    function getPresaleDetail2(uint256 _counter) internal view returns(uint256 _prelaunchprice,uint256 _startTime,
    ...
    uint256 _remainingAmount,uint256 price){
       details storage pre = presaleDetails[(_counter)];
       return(pre.prelaunchprice,pre.startTime,pre.endTime,pre.softCap,
       pre.hardCap,pre.minimumInvestment,pre.maximumInvestment,pre.remainingAmount,pre.price);
    }
    function getpresale(uint256 _counter) public view returns(bool){
        getPresaleDetail1(_counter);
        getPresaleDetail2(_counter);
        return true;
    }
