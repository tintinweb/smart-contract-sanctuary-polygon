...
contract SkillsInterface {
    ...
    uint8[MAX_SKILLS][NUMBER_OF_CLASSES] public validSkillsForClass = [
     ...
     function rollSkillsForPlayer(uint256 entropy, uint8 classId, uint8 tier) public view returns (uint8[MAX_SKILLS] memory skillsToRoll) {
        ...
        uint8 seed = 1;
        uint8 skillIndex = 0;
        rolledSkills[0] = uint8(deriveRandomnessFromEntropy(entropy, seed) % MAX_SKILLS);
        skillIndex++;
        seed++;
         for(uint8 index = 1; index <= tier; index++) {
               seed++;
               rolledSkills[index] = uint8(deriveRandomnessFromEntropy(entropy, seed) % MAX_SKILLS);
               bool done = false;
               while(!done) {
                  seed++;
                  uint8 newDerivedRandomness = uint8(deriveRandomnessFromEntropy(entropy, seed * index));

                  uint8 newValue = uint8(newDerivedRandomness % MAX_SKILLS);
