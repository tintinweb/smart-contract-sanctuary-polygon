...
contract SubManager is ISubManager, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event DisablePlan(uint256 indexed merchantTokenId, uint256 planIndex);
    ...
    uint256 BillBufferTime = 43200; // 12 hours

    address public platformFeeAddress;
    uint256 public platformFeeRate;

    mapping(uint256 => uint256[]) public merchantSubscriptions;
    mapping(uint256 => mapping(uint256 => uint256[]))
    public merchantPlanSubscriptions;
    ...
    returns (uint256)
    {
        ...
        emit CreateMerchant(msg.sender, name, merchantTokenId);
    ...
    function updateMerchant(
        ...
        emit UpdateMerchant(merchantOwner, name, merchantTokenId);
    }
    ...
    function createPlan(
        ...
        uint256 planIndex = IPlanManager(planManager).createPlan(
            merchantTokenId,
            name,
            description,
            billingPeriod,
    ...
    {
//        IMerchantToken.Merchant memory merchant = IMerchantToken(
//            merchantManager
//        ).getMerchant(merchantTokenId);
        ...
        uint256 billTime = Period.getPeriodTimestamp(
            plan.billingPeriod,
            block.timestamp
        );
        //        uint termTime = endTime;
        uint256 subscriptionTokenId = ISubTokenManager(subTokenManager)
        .mintSubToken(msg.sender);
        merchantSubscriptions[merchantTokenId].push(subscriptionTokenId);
        merchantPlanSubscriptions[merchantTokenId][planIndex].push(
            subscriptionTokenId
        );
        ...
        _charge(merchantTokenId, subscriptionTokenId, plan.payeeAddress, plan.paymentToken, plan.pricePerBillingPeriod, block.timestamp);
    ...
    function charge(uint256 subscriptionTokenId) external override nonReentrant {
        ...
        _charge(subInfo.merchantTokenId, subscriptionTokenId, plan.payeeAddress, plan.paymentToken, plan.pricePerBillingPeriod, billingTime);
    }

    function _charge(uint merchantTokenId, uint subscriptionTokenId, address payeeAddress, address paymentToken, uint pricePerBillingPeriod, uint billingTime) internal {
        uint256 platformFee = _getPlatformFee(pricePerBillingPeriod);

        uint256 adjustedPrice = pricePerBillingPeriod.sub(platformFee);
    ...
    returns (uint256)
    {
        return pricePerBillingPeriod.mul(platformFeeRate).div(10000);
    }
    ...
    function cancelSubscription(uint256 subscriptionTokenId) external override {
        ...
        emit CancelSubscription(subInfo.merchantTokenId, subscriptionTokenId);
    }
    ...
    function disablePlan(uint256 merchantTokenId, uint256 planIndex) external override {
        ...
        emit DisablePlan(merchantTokenId, planIndex);
    }
}
