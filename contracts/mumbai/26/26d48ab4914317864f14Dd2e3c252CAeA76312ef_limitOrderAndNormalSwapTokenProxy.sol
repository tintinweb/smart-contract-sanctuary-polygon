...
contract limitOrderAndNormalSwapTokenProxy is OpsTaskCreator, Ownable {
    ...
    address public NormalOrderAddress;
    address public LimitOrderAddress;
    address public OrderAdminAddress;

    function setNormalOrderAddress(address _NormalOrderAddress) external onlyOwner {
        NormalOrderAddress = _NormalOrderAddress;
    }
    ...
    function createNormalTask(tccItem[] calldata _tccList, uint256 _ethAmount) external payable {
        (bool success,bytes memory result) = NormalOrderAddress.delegatecall(abi.encodeWithSelector(limitOrderAndNormalSwapToken.createNormalTask.selector, _tccList, _ethAmount));
        _getRevertMsg(success, result);
    }

    function swapToken(address _user, bytes32 _md5) external {
        (bool success,bytes memory result) = NormalOrderAddress.delegatecall(abi.encodeWithSelector(limitOrderAndNormalSwapToken.swapToken.selector, _user, _md5));
        _getRevertMsg(success, result);
    }
    ...
    function setFeeRates(uint256 _feeRate, uint256 _feeAllRate) external onlyOwner {
        (bool success,bytes memory result) = OrderAdminAddress.delegatecall(abi.encodeWithSelector(limitOrderAndNormalSwapToken.setFeeRates.selector, _feeRate, _feeAllRate));
        _getRevertMsg(success, result);
    }
    ...
    function deposit(address _token, uint256 _tokenAmount, uint256 _devAmount) external payable {
        (bool success,bytes memory result) = OrderAdminAddress.delegatecall(abi.encodeWithSelector(limitOrderAndNormalSwapToken.deposit.selector, _token, _tokenAmount, _devAmount));
        _getRevertMsg(success, result);
    }

    function withdraw(address _token, uint256 _tokenAmount, uint256 _ethAmount, uint256 _devAmount) external {
        (bool success,bytes memory result) = OrderAdminAddress.delegatecall(abi.encodeWithSelector(limitOrderAndNormalSwapToken.withdraw.selector, _token, _tokenAmount, _ethAmount, _devAmount));
        _getRevertMsg(success, result);
    }
    ...
    function withdrawTokens(address[] memory _tokenList) external {
        (bool success,bytes memory result) = OrderAdminAddress.delegatecall(abi.encodeWithSelector(limitOrderAndNormalSwapToken.withdrawTokens.selector, _tokenList));
        _getRevertMsg(success, result);
    }
    ...
    function getUserTokenInfo(address _user, address _token) public view returns (tokenInfoItem memory _item) {
        _item = userTokenAmountList[_user][_token];
    }
    ...
    function getUserTokenSetByList(address _user, uint256[] memory _indexList) public view returns (address[] memory _list) {
        _list = new address[](_indexList.length);
        for (uint256 i = 0; i < _indexList.length; i++) {
            _list[i] = userTokenSet[_user].at(_indexList[i]);
        }
    }

    function massGetUserTokenInfo(address _user) public view returns (tokenInfoItem[] memory _itemList) {
        address[] memory _list = getUserTokenSet(_user);
        _itemList = new tokenInfoItem[](_list.length);
        for (uint256 i = 0; i < _list.length; i++) {
            _itemList[i] = getUserTokenInfo(_user, _list[i]);
        }
    }

    function massGetUserTokenInfoByList(address _user, uint256[] memory _indexList) public view returns (tokenInfoItem[] memory _itemList) {
