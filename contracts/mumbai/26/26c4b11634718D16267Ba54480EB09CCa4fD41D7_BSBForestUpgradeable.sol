...
contract BSBForestUpgradeable is Initializable, BSBBaseGameModeUpgradeable, VRFConsumerBaseUpgradeable {
    ...
    function getTokenGeneratorBonus(
        address collection_,
        uint tokenId_
    ...
    function getTokenGeneratorBonusForMany(address collection_, uint[] calldata tokenIds_) external view returns (uint[] memory) {
        uint[] memory tokenGeneratorBonuses = new uint[](tokenIds_.length);
        for (uint i = 0; i < tokenIds_.length; i++) {
            tokenGeneratorBonuses[i] = getTokenGeneratorBonus(collection_, tokenIds_[i]);
        }
    ...
    function _calculateTokenGeneratorYield(
        ...
        if (applyBuff_) {
            ...
            yieldBonus_ = getTokenGeneratorBonus(collection_, tokenId_);
        }
