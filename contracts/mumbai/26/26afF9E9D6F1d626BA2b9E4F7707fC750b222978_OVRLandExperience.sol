...
contract OVRLandExperience is AccessControl {
    ...
    IERC721 public OVRLandContainer;
    IOVRLandRenting public OVRLandRenting;

    // Owner Experience
    mapping(IERC721 => mapping(uint256 => string)) public experiences;

    // Renter Experience
    mapping(IERC721 => mapping(uint256 => string)) public rentingExperiences;
    mapping(IERC721 => mapping(uint256 => uint256)) public rentingDates;
    mapping(IERC721 => mapping(uint256 => uint256)) public months;
    mapping(IERC721 => mapping(uint256 => address)) public renter;

    // prettier-ignore
    constructor(IERC721 _OVRLandAddress, IOVRLandRenting _ovrLandRentingAddress, IERC721 _OVRLandContainer) {
    ...
    // prettier-ignore
    event ExperienceUpdated(IERC721 indexed contractAddress, uint256 indexed nftId, address indexed sender, uint256 timestamp, bool renting, string uri);
    // prettier-ignore
    event TokenRented(IERC721 indexed contractAddress, uint256 indexed nftId, address indexed renter, uint256 startDate, uint256 months, string uri);

    modifier isAllowedContractAddress(IERC721 _contractAddress) {
        // prettier-ignore
     ...
     * @return bool Rented
     */
    function isTokenRented(IERC721 _contractAddress, uint256 _nftId)
        public
        view
     ...
     * @return string URI
     */
    function experienceURI(IERC721 _contractAddress, uint256 _nftId)
        public
        view
        isAllowedContractAddress(_contractAddress)
     ...
     */
    function rentingExpiration(IERC721 _contractAddress, uint256 _nftId)
        public
        view
        isAllowedContractAddress(_contractAddress)
        returns (uint256)
