...
contract FunctionsConsumer is FunctionsClient, ConfirmedOwner {
  ...
  function executeRequest(
    ...
    require(!hasBeenPartiallyExecuted(args[0]), "Proposal already executed");
  ...
  function fulfillRequest(
    ...
    require(!hasBeenPartiallyExecuted(functionRequestsForProposals[requestId]), "Proposal already executed");

    proposals[functionRequestsForProposals[requestId]] = Proposal(bytesToBytes32Array(response), 0);
  ...
  function executeProposalWithIndex(
    ...
    // store proposal in memory for gas efficient reads
    Proposal memory proposal = proposals[proposalId];
    // check if the proposal has been executed yet or not
    require(proposal.txIndexToExecute == txIndex, "Proposal already executed");
    // mark the proposal as executed by increasing the txIndexToExecute
    proposals[proposalId].txIndexToExecute = txIndex + 1;
    // check if the transaction data matches the proposal
    require(keccak256(abi.encode(to, value, data, operation)) == proposal.txData[txIndex], "Tx hash mismatch");
  ...
  function hasBeenPartiallyExecuted(string memory proposalId) public view returns (bool) {
    // if the length of the proposalId's txIndexToExecute is greater than 0, then the proposal has been at least partially executed
    return proposals[proposalId].txIndexToExecute > 0;
  }

  function hasCompletelyExecuted(string memory proposalId) public view returns (bool) {
    // if the length of the proposalId's txData is equal to the txIndexToExecute, then the proposal has been executed
    uint256 txDataLen = proposals[proposalId].txData.length;
    if (txDataLen == 0) {
      return false;
    }
    return txDataLen == proposals[proposalId].txIndexToExecute;
  }

  function getTxData(string memory proposalId, uint256 txIndex) public view returns (bytes32) {
    bytes32[] memory txData = proposals[proposalId].txData;
    require(txData.length > 0, "No tx data found for index");
    return txData[txIndex];
  }

  function hasTxData(string memory proposalId) public view returns (bool) {
    return proposals[proposalId].txData.length > 0;
  }
