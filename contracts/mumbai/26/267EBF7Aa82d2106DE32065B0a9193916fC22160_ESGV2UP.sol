...
contract ESGV2UP is Initializable,PausableUpgradeable, AccessControlUpgradeable {
	...
	event DocumentCreated(bytes32 indexed documentId, Document documentCreated);
	event DocumentPublished(bytes32 indexed documentId, Document documentPublished);
	event DocumentWithdrawn(bytes32 indexed documentId, Document documentWithdrawn);
	
	
	function initialize() public virtual initializer{
		//        Here we can give (at least) admin and other desired roles to the contract deployer. ADMIN will be able to, at any point, remove
		//        and grant roles to other addresses
	...
	function createDocument(
	...
	) public onlyRole(SUBMITTER_ROLE) returns (bytes32){
        if(statementId_ != 0x0){
            require(Documents[statementId_].creationTimestamp > 0, "The statement you are referencing doesn't exist.");
        }
		...
		Documents[documentId_] = newDocument;
		emit DocumentCreated(newDocument.documentId, newDocument);
		return newDocument.documentId;
	}
	
	function permanentPublishDocument(bytes32 documentId_, bytes32 documentHash_, uint8 hashFunction_, uint8 hashSize_) public
	whenNotPaused onlyRole(PUBLISHER_ROLE) returns (Document memory){
	...
	function withdrawDocument(bytes32 documentId_) public whenNotPaused onlyRole(WITHDRAWER_ROLE) returns (Document memory){
		require(Documents[documentId_].creationTimestamp > 0, "Document does not exists!");
