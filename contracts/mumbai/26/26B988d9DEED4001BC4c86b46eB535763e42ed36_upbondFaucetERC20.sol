...
contract upbondFaucetERC20 is upbondFactoryController, Initializable, ReentrancyGuard{
    ...
    function claimFaucet(
        ...
        require(
            IERC20(faucetToken).balanceOf(address(this)) >= faucetConfig.reward,
            "upbondFaucet : Reward is out of stock"
        );

        IERC20(faucetToken).safeTransfer(
            _msgSender(),
            faucetConfig.reward
        );
    ...
    function emergencyWithdrawFaucet() external virtual onlyFactoryOwner nonReentrant {
        uint256 allBalance = IERC20(faucetToken).balanceOf(address(this));
