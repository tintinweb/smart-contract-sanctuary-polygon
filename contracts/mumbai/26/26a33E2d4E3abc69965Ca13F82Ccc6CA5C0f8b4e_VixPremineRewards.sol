...
contract VixPremineRewards is OwnableUpgradeable, PausableUpgradeable {
  ...
  function setPolyTrade(address _ptPool) external onlyOwner {
    require(_ptPool != address(0), "invalid address.");
    ptPool = _ptPool;
  }

  function ptAddRewards(address user, uint256 amount) external {
    require(msg.sender == ptPool, "only polyTrade contract can call");
