...
contract WantedTokenV2 is ERC20Upgradeable, WantedStorageV2{
    ...
    function randomnumber(uint256 numOfHolders) public view returns (uint256[] memory Numbers){
        ...
        uint256 nounce = 10000;
        bytes32 seed = keccak256(abi.encode(block.timestamp * numOfHolders));

        for(uint256 i = 0; i < 200; i++){
            seed = keccak256(abi.encode(block.timestamp * i , nounce));
            numbers[i] = uint256(keccak256(abi.encodePacked(block.difficulty, seed)))%1000;
            nounce += numbers[i];
        }
