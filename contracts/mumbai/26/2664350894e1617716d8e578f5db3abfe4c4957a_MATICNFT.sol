...
contract MATICNFT is BlackLkisted,ERC721Enumerable {
	using SafeMath for uint256;
	IBEP20 dart;
	constructor() payable ERC721('Matic NFT ', 'Matic') {
		sellingFess = 5;
	}
    uint256 public sellingFess;
    uint256 public creationFess = 0;
    uint256 public referenceFees = 1;
    uint256 public  royalty=1;
	...
	function setContractFees(uint256 _creationFess,uint256 _sellingFess,uint256 _referralFees,uint256 _royalties) public onlyOwner {
	...
	function mintNFTWithMatic(string memory _tokenURI, uint256 tokenId, uint256 price) public payable {
        ...
        nftINFO[tokenId].isToken=false;
        nftINFO[tokenId].paymentCurrency=IBEP20(address(this));
        nftINFO[tokenId].sell=true;
	}
	function mintNFTWithToken(string memory _tokenURI,uint256 amount, uint256 tokenId, bool isToken,uint256 price,IBEP20 paymentCurrency) public  {
		require(!isBlacklisted[msg.sender], "caller is backlisted");  
        require(checkExitscurrency(paymentCurrency),"Invalid Payment Currency");
        require(hasSaleStarted == true, "Sale hasn't started");
        if(creationFess > 0) {
    ...
    function buyWithTokens(uint256 tokenId,address payable _reference,uint256 amount) internal {
		...
		if(_reference==address(0)){
		    owner_fees = uint256(100) - sellingFess - royalty;
		}else{
		    owner_fees = uint256(100) - sellingFess - royalty - referenceFees;
		...
		if(royalty > 0) {
		dart.transferFrom(msg.sender, nftINFO[tokenId].creator_address, (nftINFO[tokenId].price * royalty) / 100);
		}
		if(owner_fees > 0) {
	...
	function buyWithBNB(uint256 tokenId,address payable _reference) public payable {
		...
		payable(nftINFO[tokenId].creator_address).transfer((msg.value * royalty) / (100));
        _transfer(_owner,msg.sender,tokenId);
        nftINFO[tokenId].sell=false;
        nftINFO[tokenId].owner_address=msg.sender;
	}
