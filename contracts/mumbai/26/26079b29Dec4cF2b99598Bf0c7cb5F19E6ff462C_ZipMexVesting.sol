...
contract ZipMexVesting is Initializable, OwnableUpgradeable{
    ...
    */
    ...
    {
        Pool storage p = vestingPools[_poolIndex];
        Beneficiary memory b = p.beneficiaries[_address];
        uint unlockedTokens = 0;
        uint currentVestingIndex = b.currentIndex;

        require(
            currentVestingIndex < vestingPools[_poolIndex].releaseTimes.length,
            "ZipMexVesting: No more withdrawals scheduled for the current vesting schedule."
        );

        if (block.timestamp < vestingPools[_poolIndex].releaseTimes[currentVestingIndex]) { 
        ...
        { 
            require(
                vestingPools[_poolIndex].beneficiaries[_address].claimedTotalTokenAmount < 
                vestingPools[_poolIndex].beneficiaries[_address].totalTokens,
                "ZipMexVesting: Cannot claim more than invested."
            );
            uint percentage = vestingPools[_poolIndex].releaseAmountPercentage[currentVestingIndex];
            uint multiplier = percentage*100;

            unlockedTokens = vestingPools[_poolIndex].beneficiaries[_address].totalTokens * multiplier /10000;
