...
contract RewardsWolferFinance is Pausable, Ownable {
   ...
   struct Aidrop {
    ...
    mapping(uint256=>Aidrop) public Rewards;
    ...
    function startClaim() public onlyKeeperRegistry whenNotPaused {
        airDropIdNo++;
        require(!claimEnabled);
        require(payable(address(this)).balance>0);
        //Total Matic for Current Drop
        uint totalMatic =  payable(address(this)).balance - totalRemMatic;

            //Each Matic for Current Drop
        uint eachMatic = totalMatic / 5000;

           Aidrop storage newReward = Rewards[airDropIdNo];
           
            newReward.airDropId= airDropIdNo;
            newReward.eachMatic= eachMatic;
            newReward.totalMatic= totalMatic;
    ...
    //Stop Function should be executed before next Drop
    function stopClaim () public onlyOwner {

       
       Rewards[airDropIdNo].remMatic =  payable(address(this)).balance - totalRemMatic;
         totalRemMatic= 0;

          for (uint i=1 ;i<=airDropIdNo;i++){
            totalRemMatic+=Rewards[i].remMatic;
        }
    ...
    function claimReward (uint256[] memory _nftId, uint256 _dropId)  public {
        ...
        for(uint i = 0 ; i<_nftId.length; i++) {
           ...
           totalClaim+=Rewards[_dropId].eachMatic;

        }
        
        payable(msg.sender).transfer(totalClaim);
        if(airDropIdNo>1 && _dropId <airDropIdNo)  Rewards[_dropId].remMatic -= totalClaim;
