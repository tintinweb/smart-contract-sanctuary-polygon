...
contract Redemptions is AragonApp {
    ...
    string private constant ERROR_VAULT_IS_NOT_CONTRACT =
        "REDEMPTIONS_VAULT_NOT_CONTRACT";
    string private constant ERROR_TOKEN_MANAGER_IS_NOT_CONTRACT =
        "REDEMPTIONS_TOKEN_MANAGER_NOT_CONTRACT";
    string private constant ERROR_REDEEMABLE_TOKEN_LIST_FULL =
        "REDEMPTIONS_REDEEMABLE_TOKEN_LIST_FULL";
    string private constant ERROR_DUPLICATE_REDEEMABLE_TOKEN =
        "REDEMPTIONS_DUPLICATE_REDEEMABLE_TOKEN";
    string private constant ERROR_TOKEN_ALREADY_ADDED =
        "REDEMPTIONS_TOKEN_ALREADY_ADDED";
    string private constant ERROR_TOKEN_NOT_CONTRACT =
        "REDEMPTIONS_TOKEN_NOT_CONTRACT";
    string private constant ERROR_TOKEN_NOT_ADDED =
    ...
    string private constant ERROR_CANNOT_REDEEM_ZERO =
    ...
    function initialize(
        ...
        require(
            _redeemableTokens.length <= REDEEMABLE_TOKENS_MAX_SIZE,
            ERROR_REDEEMABLE_TOKEN_LIST_FULL
        );
        ...
        for (uint256 i = 0; i < _redeemableTokens.length; i++) {
            ...
            require(
                ...
                ERROR_DUPLICATE_REDEEMABLE_TOKEN
            );
            if (token != ETH) {
    ...
    function addRedeemableToken(address _token) external auth(ADD_TOKEN_ROLE) {
        require(
            redeemableTokens.length < REDEEMABLE_TOKENS_MAX_SIZE,
            ERROR_REDEEMABLE_TOKEN_LIST_FULL
        );
     ...
     * @param _token Token address
     */
    function removeRedeemableToken(address _token)
        external
        auth(REMOVE_TOKEN_ROLE)
    {
     ...
     */
    function redeem(uint256 _burnableAmount)
        external
        authP(REDEEM_ROLE, arr(msg.sender))
    {
        require(_burnableAmount > 0, ERROR_CANNOT_BURN_ZERO);
        require(
            tokenManager.spendableBalanceOf(msg.sender) >= _burnableAmount,
        ...
        for (uint256 i = 0; i < redeemableTokens.length; i++) {
            ...
            redemptionAmount = _burnableAmount.mul(vaultTokenBalance).div(
                burnableTokenTotalSupply
            );
