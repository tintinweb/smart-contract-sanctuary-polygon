...
contract MissionImpl is MissionBase, RewardManager {
...
abstract contract RewardManager is Ownable, InterfaceManager {
    ...
    function mintRewards() external onlyOwner {
        this.mintBronzeRewards();
        this.mintSilverRewards();
        this.mintGoldRewards();
    }

    function mintGoldRewards() external onlyOwner {
    ...
    function mintSilverRewards() external onlyOwner {
        for (uint i = 0; i < 13; i++) {
    ...
    function mintBronzeRewards() external onlyOwner {
        for (uint i = 0; i < 5; i++) {
