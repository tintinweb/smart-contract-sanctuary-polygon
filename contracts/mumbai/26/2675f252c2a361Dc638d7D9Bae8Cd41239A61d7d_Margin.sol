...
interface ICErc20Delegator {
    function borrow(address payable holder, uint borrowAmount) external returns(uint);
...
contract Margin is ReentrancyGuard, MarginStorage, Ownable {
    ...
    function deposit(
        ...
        depositInfo.interestAccumulated += getInterestAccumulated(
            msg.sender,
            assetAddress,
        ...
        nonReentrant
    {
        ...
        // uint256 interestAmount = getInterestAccumulated(
        //     msg.sender,
        //     assetAddress,
        //     loanAllocated
        // );
    ...
    function repay(
        ...
        if (source == 1) {
            if (assetAddress == address(0)) {
                ...
                //ICEther(delegator).repayBorrow{value: msg.value};
                require(
                    address(this).balance >= repayAmount,
                    "Margin: contract has less balance"
                );
                ...
                //amount is getting transferred from the contract
                IERC20(assetAddress).transferFrom(
                    msg.sender,
                    address(this),
                    repayAmount
        ...
        //using the margin account
            ...
            if (assetAddress == address(0)) {
                //need to check contract should have repayAmount as balance
                require(
                    address(this).balance >= repayAmount,
                    "Margin: contract has less balance"
                );
    ...
    function setPositionAmount(address user, address assetAddress, uint256 marginUsed, uint256 loanUsed) internal {
        Deposit storage depositInfo = depositDetails[user][assetAddress];
    ...
    function getInterestAccumulated(
        address userAddress,
        address assetAddress,
        uint256 loanAmount
