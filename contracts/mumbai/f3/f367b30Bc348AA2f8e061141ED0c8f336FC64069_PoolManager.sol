...
contract PoolManager is OwnableUpgradeable, IPoolManager, Utility {
    using EnumerableSet for EnumerableSet.AddressSet;
    using SafeMath for uint256;
    using SafeERC20Upgradeable for IERC20Upgradeable;
    ...
    mapping(address => uint8) private _poolState;
    mapping(address => address) private _poolForToken;
    mapping(string => address) private _privatePoolForToken;
    ...
    event EmergencyWithdrawFeesChanged(address indexed pool_address, uint256 _old, uint256 _new);
    ...
    function setEmergencyWithdrawFees(uint256 _fees) external onlyOwner validAmount(_fees) {
        emit EmergencyWithdrawFeesChanged(address(this), emergencyWithdrawFees, _fees);
        emergencyWithdrawFees = _fees;
    }

    function getEmergencyWithdrawFees() external view override returns (uint256) {
        return emergencyWithdrawFees;
    }
    ...
    function privatePoolForToken(string memory tokenName)
    external
    view
    override
    returns (address)
    {
        return _privatePoolForToken[tokenName];
    }
    ...
    function registerPrivatePool(
        ...
        _poolsForVersion[version].add(pool);
        _poolsOf[owner].add(pool);
        _privatePoolForToken[tokenName] = pool;
        _poolState[pool] = 2;
    }
    ...
    function removePrivatePoolForToken(string memory tokenName, address pool)
    external
    override
    onlyAllowedFactory
    {
        _privatePoolForToken[tokenName] = address(0);
        emit PrivatePoolForTokenRemoved(tokenName, pool);
    }
    ...
    function addTopPool(address poolAddress, uint256 raisedAmount) external override onlyAllowedFactory {
        if(raisedAmount >= _topPools[49].totalRaised) {
            ...
            status = false;
            TopPoolInfo memory tmp = TopPoolInfo(0, address(0));
            for(uint256 i = 0; i < 50; i++)
            {
    ...
    returns (CumulativeLockInfo[] memory)
    {
        ...
        uint8[] memory saleType = new uint8[](3);
        uint256[] memory info = new uint256[](11);
        string memory name = "";
        string memory symbol = "";
        string memory poolDetail = "";

        for (uint256 i = start; i <= end; i++) {
            if(_poolState[_pools.at(i)] == 1) {
                (
                token,
                saleType,
                info,
                ...
                name,
                poolDetail
                ) = IPrivatePool(_pools.at(i)).getPoolInfo();
                lockInfo[currentIndex] = CumulativeLockInfo(
                    _pools.at(i),
                    address(0),
                    saleType[0],
