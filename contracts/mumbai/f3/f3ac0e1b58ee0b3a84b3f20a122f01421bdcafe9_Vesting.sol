...
contract Vesting is Ownable, ReentrancyGuard {
	...
	function _claimTo(address to) internal {
		...
		require(
			block.timestamp > startTime + vestingSchedulesById[id].vestingStart,
			"Vesting is not started"
		);

		uint256 _amount = computeClaimableAmount(id);

		require(remainingVestingAmount > _amount, "No remained vesting token");
		...
		returns (uint256 _amount)
	{
		if (vestingSchedulesById[id].isEnded) {
			...
			if (block.timestamp < startTime) {
				...
				if (
					block.timestamp >=
					startTime + vestingSchedulesById[id].vestingStart
				) {
					_amount =
						(vestingSchedulesById[id].allocatedAmount *
							(block.timestamp -
