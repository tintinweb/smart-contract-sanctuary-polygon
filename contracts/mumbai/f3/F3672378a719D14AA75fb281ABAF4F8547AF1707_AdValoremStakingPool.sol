...
contract AdValoremStakingPool is OwnableUpgradeable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    event PoolAdded(uint256 apr, uint256 timeLocked, string label);
    event SetPoolInfo(uint256 pid, uint256 apr, uint256 timeLocked, string label, bool isActive);
    ...
    function initialize(address _adValoremToken)
        external
        initializer
    {
        __Ownable_init();
        adValoremToken = _adValoremToken;
        ...
        poolInfo.push(
            PoolInfo({
                totalSupply: 0,
                apr: 500,
                timeLocked: 30 * 24 * 60 * 60,// 30 days timestamp
                label: "Pool One",
                isActive: true
            })
        );
        poolInfo.push(
            PoolInfo({
                totalSupply: 0,
                apr: 1000,
                timeLocked: 60 * 24 * 60 * 60,// 60 days  timestamp
                label: "Pool two",
                isActive: true
            })
        );
        poolInfo.push(
            PoolInfo({
                totalSupply: 0,
                apr: 1500,
                timeLocked: 180 * 24 * 60 * 60,// 180 days  timestamp
                label: "Pool Three",
                isActive: true
            })
        );
        poolInfo.push(
            PoolInfo({
                totalSupply: 0,
                apr: 2000,
                timeLocked: 360 * 24 * 60 * 60, // 360 days  timestamp
                label: "Pool Four",
                isActive: true
            })
        );
        ...
        returns (uint256)
    {
        ...
        if (user.timeClaimed.sub(user.timeDeposited) <= pool.timeLocked) {
            if (block.timestamp.sub(user.timeClaimed) <= pool.timeLocked) {
                rewardDuration = block.timestamp - user.timeClaimed;
            } else {
                rewardDuration = pool.timeLocked;
            }
        } else {
            rewardDuration = 0;
        }

        uint256 pendingUserAdValorem = user
            .amount
            .mul(rewardDuration)
            .mul(pool.apr)
            .div(PRECISION)
            .div(YEAR_TIME);
        return pendingUserAdValorem;
    }
    ...
    function _deposit(uint256 _pid, uint256 _amount) private {
        ...
        require(pool.isActive, "deposit: This Pool is non activated");
        ...
        if (_amount > 0) {
            uint256 before = IBEP20(adValoremToken).balanceOf(address(this));
            IBEP20(adValoremToken).safeTransferFrom(
                address(msg.sender),
                address(this),
                _amount
            );
            uint256 post = IBEP20(adValoremToken).balanceOf(address(this));
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            ...
            IBEP20(adValoremToken).safeTransfer(address(msg.sender), _amount);
        }
    }
    ...
    function _claimPendingReward(uint256 _pid, address _address) private {
        ...
        require(
            ...
            "claimPendingReward: Should charge reward token"
        );

        totalRewardSupply = totalRewardSupply.sub(rewardAmount);
        if (rewardAmount != 0) {
            IBEP20(adValoremToken).safeTransfer(_address, rewardAmount);
        }
        user.timeClaimed = block.timestamp;
    ...
    function harvest(uint256 _pid) external {
        PoolInfo storage pool = poolInfo[_pid];
        UserPoolInfo storage user = userPoolInfo[_pid][address(msg.sender)];
        require(pool.isActive, "harvest: this Pool is non activated");
    ...
    function compound(uint256 _pid) external {
        PoolInfo storage pool = poolInfo[_pid];
        UserPoolInfo storage user = userPoolInfo[_pid][address(msg.sender)];
        require(pool.isActive, "compound: This Pool is non activated");
        
        address _address = address(msg.sender);
        uint256 rewardAmount = pendingReward(_pid, _address);
        if (rewardAmount > 0) {
            _deposit(_pid, rewardAmount);
            user.timeDeposited = block.timestamp;
        }
    }
    
    // Add a new lp to the pool. Can only be called by the owner.
    // XXX DO NOT add the same VLR token more than once. Rewards will be messed up if you do.
    function addPool(
        uint256 _apr,
        uint256 _timeLocked,
        ...
        emit PoolAdded(_apr, _timeLocked, _label);
    }

    // Update the given pool info. Can only be called by the owner.
    function setPoolInfo(uint256 _pid, uint256 _apr, uint256 _timeLocked, string memory _label, bool _isActive)
        external
        onlyOwner
    {
        poolInfo[_pid].apr = _apr;
        poolInfo[_pid].timeLocked = _timeLocked;
        poolInfo[_pid].label = _label;
        poolInfo[_pid].isActive = _isActive;
        emit SetPoolInfo(_pid, _apr, _timeLocked, _label, _isActive);
    }

    // Deposite tokens for reward
    function depositRewardToken(uint256 _amount) external onlyOwner {
        uint256 originalBalance = IBEP20(adValoremToken).balanceOf(address(this));
        IBEP20(adValoremToken).safeTransferFrom(
            msg.sender,
            address(this),
            _amount
        );
        uint256 currentBalance = IBEP20(adValoremToken).balanceOf(address(this));
    ...
    function withdrawRewardToken() external onlyOwner {
        ...
        totalRewardSupply = 0;
        IBEP20(adValoremToken).safeTransfer(address(msg.sender), amount);
