...
contract AuctionFixedPriceV2 is Initializable {
    ...
    function makeOfferFixedPrice(
        ...
        address ownerOfNFT = stanNFT.ownerOf(_paramsOffer.tokenId);
    ...
    function acceptOfferPvP(
        ...
        if (
            listingInstance.ListingID.length != 0 &&
            listingInstance.state != AuctionStructure.StateOfListing.INACTIVE
        ) {
            listing.cancelListingFixedPrice(listingInstance.ListingID);
        }

        address ownerOfOffer = offer.acceptOfferPvP(_nftId, _subOfferId);
        AuctionStructure.Currency fromCurrency = inforInstance.currency;
    ...
    function cancelOfferPvP(
        ...
        if (_currency == AuctionStructure.Currency.CRYPTO) {
            ...
            stanFund.set(fee, AuctionStructure.Operator.MINUS, infoOffer.maker);
        }
