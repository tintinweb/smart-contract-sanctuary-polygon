...
contract mumbaiPresaleAri1 is ERC721A, ERC2981, Ownable {
    ...
    mapping(address => uint256) internal mintQuantities;
    ...
    */
    ...
    /**
    * @notice Get the caller's current number of mints
    * @return current number of mints
    */
    function getMintQuantity() external view returns (uint256) {
        return mintQuantities[msg.sender];
    }
    ...
    */
    function mint(uint256 quantity) external payable {
        ...
        if (_publicPerWalletMaximum > 0 && mintQuantities[msg.sender] + quantity > _publicPerWalletMaximum) revert InvalidQuantity();
        if (_publicPerTransactionMaximum > 0 && quantity > _publicPerTransactionMaximum) revert InvalidQuantity();

        _safeMint(msg.sender, quantity);

        mintQuantities[msg.sender] += quantity;
    ...
    */
    ...
    returns (bool)
    {
        ...
        uint256 internal constant _presaleSupply = 9;
    uint256 internal _preSalePrice = 0.00001 ether;
    uint256 internal _prePerWalletMaximum = 0;
    uint256 internal _prePerTransactionMaximum = 0;
    ...
    */
    function getPreSaleSupply() external view returns (uint256){
    ...
    */
    function presale(
        ...
        if (msg.value != _preSalePrice * quantity) revert InvalidPrice();
                if (!existsInAllowList()) revert NotExistInArrowList();


        if (_prePerWalletMaximum > 0 && mintQuantities[msg.sender] + quantity > _prePerWalletMaximum) revert InvalidQuantity();
    ...
    event UpdatedAllowList();
    error EmptyAllowList();
    error NotExistInArrowList();

    /**
    * @notice Determine if the specified address exists in the arrow list
    * @return true if it exists
    */
    function existsInAllowList() internal view returns(bool) {
        bool result = false;
    ...
    /**
    * @notice get allow address list
    * @return allow list
    */
    function getAllowList() external view onlyOwner returns(address[] memory) {
        address[] memory allowListLengthFixed = new address[](_allowList.length);
        allowListLengthFixed = _allowList;
        return allowListLengthFixed;
    }

    /**
    * @notice Set new Allow List
    * @param allowList allow addresses
    */
    function setAllowList(address[] calldata allowList) external onlyOwner{
        if(allowList.length == 0) revert EmptyAllowList();
        _allowList = allowList;
        emit UpdatedAllowList();
    }
