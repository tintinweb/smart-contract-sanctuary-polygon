...
//   uint public consumeNum = 1*1e16;                                              //todo
  ...
  mapping (address => mapping (address => bool)) internal ownerToOperators;
    ...
    recordItem[] public recordTransfer;
  ...
  function _transfer(
    ...
    internal
  {
    recordItem memory temp;
    temp._tokenId = _tokenId;
    temp._from = _from;
    temp._to = _to;
    recordTransfer.push(temp);
...
contract zNFTCONTRACT is Mdate,Ownable{
    ...
    function getRecordLength() public view returns (uint) {
        return recordTransfer.length;
    }

    function getNFTRecord(uint indx) public view returns (uint tid,address _from,address _to,string memory _ipfs) {
        tid = recordTransfer[indx]._tokenId;
        _from = recordTransfer[indx]._from;
        _to = recordTransfer[indx]._to;
        _ipfs = all[recordTransfer[indx]._tokenId]._uri;
    }
}