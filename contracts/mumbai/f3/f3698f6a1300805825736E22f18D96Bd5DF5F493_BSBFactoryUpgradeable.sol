...
contract BSBFactoryUpgradeable is Initializable, OwnableUpgradeable, IBSBTypes, BSBBaseUpgradeable, VRFConsumerBaseUpgradeable {
    ...
    mapping(uint => uint) public treeHouseLevelCostsMapping;
    ...
    function __BSBFactory_init(
        ...
        bloodShardTokenId = 0;
        cappedLevelForBloodTokenSpending = 3;
    ...
    function setTreeHouseLevelCosts(uint[] calldata levels_, uint[] calldata costs_) external onlyOwner {
        require(levels_.length == costs_.length, "BSB400");

        for (uint i = 0; i < levels_.length; ++i) {
            treeHouseLevelCostsMapping[levels_[i]] = costs_[i];
        }
    }
    ...
    function mergeTreeHouses(uint[] calldata housesIds_) external {
        ...
        _spendTreeHouseMergingCosts(treeHousesStats_[treeHouseToBeUpgradedIndex_].size.value);

        uint resultingSize_ = treeHousesStats_[treeHouseToBeUpgradedIndex_].size.value + 1;
        treeHousesStats_[treeHouseToBeUpgradedIndex_].size.value = resultingSize_ <= treeHousesStats_[treeHouseToBeUpgradedIndex_].size.max ?
        resultingSize_ : treeHousesStats_[treeHouseToBeUpgradedIndex_].size.max;
    ...
    function _spendTreeHouseMergingCosts(uint houseLevel_) internal {
        if (houseLevel_ <= cappedLevelForBloodTokenSpending) {
            BSBBloodTokenCaller.spend(treeHouseLevelCostsMapping[houseLevel_] * ETHER, _msgSender());
        } else {
            BSBBloodShardCaller.burn(bloodShardTokenId, treeHouseLevelCostsMapping[houseLevel_], _msgSender());
        }
    }
    ...
    function reRoll(uint toBeUpgradedTokenId_, uint[] calldata gen1Tokens_) external {
        ...
        _runReRoll(gen1Address_, toBeUpgradedTokenId_, gen1Tokens_);
    ...
    function _runReRoll(
        ...
        require(gen1Tokens_.length >= 2, "BSB400");

        BearStats[] memory bearStats_ = BSBStorageCaller.getBearsStats(gen1Address_, gen1Tokens_);
        ...
        uint reRollSuccessPercentage_ = _getLevelImpact(average).reRollSuccessBoostPercentage;
        uint totalSuccessPercentage_ = _calculateTotalReRollSuccessPercentage(toBeUpgradedTokenId_, reRollSuccessPercentage_);
        uint additionalTax_ = _calculateAdditionalTax(gen1Tokens_);
        uint reRollPrice_ = _calculatePrice(toBeUpgradedTokenId_, additionalTax_);

        bool isReRollSuccessful = _isReRollSuccessful(totalSuccessPercentage_, toBeUpgradedTokenId_, reRollSuccessPercentage_);
        if (isReRollSuccessful) {
            toBeUpgradedBearStats_.isElite = true;
            delete gen1BonusPercentages[toBeUpgradedTokenId_];
        } else {
            gen1BonusPercentages[toBeUpgradedTokenId_] += reRollSuccessPercentage_;
            gen1ReRollAdditionalTax[toBeUpgradedTokenId_] += additionalTax_;
        ...
        delete reRollSuccessPercentage_;
        delete totalSuccessPercentage_;
        delete additionalTax_;
        delete reRollPrice_;
        delete bearStats_;
    ...
    function _calculateAdditionalTax(uint[] memory tokenIds_) internal pure returns(uint) {
        return 10000 * tokenIds_.length;
    }
