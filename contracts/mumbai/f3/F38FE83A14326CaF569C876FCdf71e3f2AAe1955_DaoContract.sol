...
contract BaseContract is Ownable, TimeForDay, StructForDao, SortList {
    ...
    //    // DAO5地址列表
    //    EnumerableSet.AddressSet internal Dao5Set;
    ...
    uint256[10] internal daoAmountList = [5 * (10 ** 4) * (10 ** 18), 20 * (10 ** 4) * (10 ** 18), 100 * (10 ** 4) * (10 ** 18), 500 * (10 ** 4) * (10 ** 18), 1000 * (10 ** 4) * (10 ** 18)];
    string[10] internal daoEventList = ["RewardNewDao1","RewardNewDao2","RewardNewDao3","RewardNewDao4","RewardNewDao5"];

    //    uint256 internal dao1Amount = 50000 * 10 ** 18;
    //    uint256 internal dao2Amount = 200000 * 10 ** 18;
    //    uint256 internal dao3Amount = 1000000 * 10 ** 18;
    //    uint256 internal dao4Amount = 5000000 * 10 ** 18;
    //    uint256 internal dao5Amount = 10000000 * 10 ** 18;
    // 总补偿金额
    ...
    uint256 public maxDaoType = 5;
    uint256 public rewardNew;
    ...
    // 奖励比例列表
    uint256[10] rewardRateListForDao = [50, 30, 20, 20, 15];
    ...
    //可升级参数
    TotalItemList public totalConfig;

    //    address[] public weekTop1AddressList;
    //    uint256 public weekReward;

    //周投资奖
    address[] internal weekTop1AddressListForInvest;
    uint256 internal weekRewardForInvest;

    //周邀请奖
    address[] internal weekTop1AddressListForInvite;
    uint256 internal weekRewardForInvite;


    //平台总投资人数
    uint256 public totalInvestMember;
    ...
    //已经用户最大的最大区网体入金
    mapping(address => uint256) public bigNetAmountList;
    //统计dao所有奖励和已发放的奖励
    ...
    function _getDaoReward(address _user, uint256 _type) internal view returns (uint256) {
        ...
        if (investIndex == 0 || _type < 1 || _type > 5 || userDaoList[_user]._daoType == 0) {
            return 0;
        }
        // 获取用户的DAO信息
        ...
        if (_daoType == _type) {
            ...
            //            } else if (_type == 5) {
            //                _totalReward = totalShareAllocationList[investEndIndex]._totalAD5A - totalShareAllocationList[_daoStartIndex]._totalAD5A;
            //            }
        }
        return _totalReward;
    }
    ...
    function _getAllDaoReward(address _user, uint256 _type) internal view returns (uint256) {
        ...
        //        if (_type == 1) {
        _totalReward += userInfoList[_user]._updatedRewardItem._updatedDaoRewardList[_type - 1];
        //        } else if (_type == 2) {
        ...
        //        } else if (_type == 5) {
        //            _totalReward += userInfoList[_user]._updatedRewardItem._updatedDao5Reward;
        //        }
        return _totalReward;
    }
...
contract DaoContract is BaseContract {
    ...
    */
    ...
    function invest(uint256 _amount) external {
        ...
        //todo 更新静态奖励指数
        if (investIndex == 0) {
            uint256 _leftAmount = _amount * 25 / 100;
            rewardNew += _leftAmount;
            emit RewardNewEvent(block.timestamp, investIndex, "RewardNewStaticRewad", _leftAmount / (10 ** 18));
            //todo 全网首单还有结余
            skip.currentASA = 0;
        } else {
            //todo 25%全球全球静态分红
        ...
        //todo 更新dao1奖励指数
        _updateTotalADAList(_amount, 5);
        //todo dao管理奖 13.5%
        //        daoRewardData._allDao1Reward += _amount * 50 / 1000;
        //        daoRewardData._allDao2Reward += _amount * 30 / 1000;
        //        daoRewardData._allDao3Reward += _amount * 20 / 1000;
        //        daoRewardData._allDao4Reward += _amount * 20 / 1000;
        //        daoRewardData._allDao5Reward += _amount * 15 / 1000;
        //        daoRewardData._allDao5Reward += _amount*15/1000;
        ...
        //todo 分配推广奖励和更新各级推荐人的DAO数据(直推22%+间推10%+团队10%) --此处会产生结余
        ...
        //        weekReward += _amount * 15 / 1000;
        weekRewardForInvest += _amount * 75 / 10000;
        weekRewardForInvite += _amount * 75 / 10000;
        //更新总补偿池数据
        ...
        emit InvestEvent(_user, investIndex, skip._day, skip._timestamp, _amount/(10**18), listPerDayForInvest[skip._day][_user]/(10**18), _getUserRemainingCredit(_user)/(10**18));
        //入金索引+1
        investIndex += 1;
    }

    function _forInvite(uint256 _amount, uint256 _day, address[] memory _list, uint256 _startIndex, uint256 _endIndex, uint256 _rewardRate) private returns (uint256 _rewardAmount){
        uint256 _totalR;
        for (uint256 k = _startIndex; k < _endIndex; k++) {
            _totalR += listPerDayForInvite[_day][_list[k]];
        }
        for (uint256 k = _startIndex; k < _endIndex; k++) {
            uint256 _rewardAmountItem = _amount * _rewardRate * listPerDayForInvite[_day][_list[k]] / (100 * _totalR);
            userInfoList[_list[k]]._rankingRewardItem._updatedRankingInvite += _rewardAmountItem;
    ...
    function _distributeDayRewardForInviteRanking() private {
        ...
        //第1名
        _totalAmount += _forInvite(_amount, _day, _list, 0, 1, 40);
        //第2、3、4
        _totalAmount += _forInvite(_amount, _day, _list, 1, 4, 30);
        //第5、6、7、8、9、10
        _totalAmount += _forInvite(_amount, _day, _list, 4, 10, 30);
        if (_amount > _totalAmount) {
            uint256 _leftAmount = _amount - _totalAmount;
            rewardNew += _leftAmount;
            emit RewardNewEvent(block.timestamp, investEndIndex, "RewardNewInviteRanking", _leftAmount / (10 ** 18));
        }
        ...
        emit DistributeEvent(_day, _timestamp, "_distributeDayRewardForInviteRanking", _amount/(10**18), _totalAmount/(10**18), _num, _gas0 - _gas1, _list[0]);
    }
    ...
    function _distributeDayRewardForInvestRanking() private {
        ...
        //将第一名加入到周排行榜地址
        weekTop1AddressListForInvest.push(firstAddress);
        uint256 _totalAmount;
        for (uint256 i = 0; i < _num; i++) {
            address _user = _list[i];
        ...
        if (_amount > _totalAmount) {
            uint256 _leftAmount = _amount - _totalAmount;
            rewardNew += _leftAmount;
            emit RewardNewEvent(block.timestamp, investEndIndex, "RewardNewInvestRanking", _leftAmount / (10 ** 18));
        }
        DayRewardForInvestRankingStatus[_day] = true;
        uint256 _gas1 = gasleft();
        emit DistributeEvent(_day, _timestamp, "_distributeDayRewardForInvestRanking", _amount/(10**18), _totalAmount/(10**18), _num, _gas0 - _gas1, firstAddress);
    }
    ...
    function _distributeCompensation() private {
        ...
        address firstAddress = _list[0];
        userInfoList[firstAddress]._rankingRewardItem._updatedCompensation += _amountForTop;
        if (_num > 1) {
        ...
        uint256 _gas1 = gasleft();
        emit DistributeEvent(_day, _timestamp, "_distributeCompensation", _amount/(10**18), _amount/(10**18), _num, _gas0 - _gas1, firstAddress);
    }
    ...
    function _distributeWeekRewardForInvest() private {
        ...
        uint256 _timestamp = block.timestamp;
        uint256 _day = getYearMonthDay(_timestamp, adjustHour);
        uint256 _num = weekTop1AddressListForInvest.length;
        uint256 _amount = weekRewardForInvest * 50 / 100;
        // 如果本周排名人数小于7人，不进行奖励分配
        if (_num < 7) {
            return;
        }
        // 检查前7个地址是否全部相同，只有在这种情况下才会进行奖励分配
        address firstAddress = weekTop1AddressListForInvest[0];
        bool _canDistribute = true;
        for (uint256 i = 1; i < 7; i++) {
            if (weekTop1AddressListForInvest[i] != firstAddress) {
                _canDistribute = false;
            }
        }
        if (_canDistribute) {
            // 对第一名进行奖励分配
            userInfoList[firstAddress]._rankingRewardItem._updatedWeeKReward += _amount;
            weekRewardForInvest -= _amount;
        }
        uint256 _gas1 = gasleft();
        emit DistributeEvent(_day, _timestamp, "_distributeWeekRewardForInvest", _amount/(10**18), _amount/(10**18), _num, _gas0 - _gas1, firstAddress);
        // 清空地址数组
        weekTop1AddressListForInvest = new address[](0);
    }
    ...
    function _distributeWeekRewardForInvite() private {
        ...
        uint256 _timestamp = block.timestamp;
        uint256 _day = getYearMonthDay(_timestamp, adjustHour);
        uint256 _num = weekTop1AddressListForInvite.length;
        uint256 _amount = weekRewardForInvite * 50 / 100;
        // 如果本周排名人数小于7人，不进行奖励分配
        if (_num < 7) {
            return;
        }
        // 检查前7个地址是否全部相同，只有在这种情况下才会进行奖励分配
        address firstAddress = weekTop1AddressListForInvite[0];
        bool _canDistribute = true;
        for (uint256 i = 1; i < 7; i++) {
            if (weekTop1AddressListForInvite[i] != firstAddress) {
                _canDistribute = false;
            }
        }
        if (_canDistribute) {
            // 对第一名进行奖励分配
            userInfoList[firstAddress]._rankingRewardItem._updatedWeeKReward += _amount;
            weekRewardForInvite -= _amount;
        }
        uint256 _gas1 = gasleft();
        emit DistributeEvent(_day, _timestamp, "_distributeWeekRewardForInvite", _amount/(10**18), _amount/(10**18), _num, _gas0 - _gas1, firstAddress);
        // 清空地址数组
    ...
    //    // 分配本周奖励
    //    function _distributeWeekReward() private {
    //        uint256 _gas0 = gasleft();
    //        uint256 _timestamp = block.timestamp;
    //        uint256 _day = getYearMonthDay(_timestamp, adjustHour);
    //        uint256 _num = weekTop1AddressList.length;
    //        uint256 _amount = weekReward * 50 / 100;
    //        // 如果本周排名人数小于7人，不进行奖励分配
    //        if (_num < 7) {
    //            return;
    //        }
    //        // 检查前7个地址是否全部相同，只有在这种情况下才会进行奖励分配
    //        address firstAddress = weekTop1AddressList[0];
    //        bool _canDistribute = true;
    //        for (uint256 i = 1; i < 7; i++) {
    //            if (weekTop1AddressList[i] != firstAddress) {
    //                _canDistribute = false;
    //            }
    //        }
    //        if (_canDistribute) {
    ...
    //        }
    //        uint256 _gas1 = gasleft();
    //        emit DistributeEvent(_day, _timestamp, "_distributeWeekReward", _amount, _amount, _num, _gas0 - _gas1, firstAddress);
    //        // 清空地址数组
    //        weekTop1AddressList = new address[](0);
    //    }
    ...
    function _distributeAll() private {
        ...
        //        _distributeWeekReward();
        _distributeWeekRewardForInvest();
        _distributeWeekRewardForInvite();
    }
    ...
    function _updateTotalADAList(uint256 _amount, uint256 _type) private {
        ...
        //        uint256[10] rateList = [50, 30, 20, 20, 15];
        ...
        if (num == 0) {
            ...
            rewardNew += _leftAmount;
            emit RewardNewEvent(block.timestamp, investEndIndex, daoEventList[_type - 1], _leftAmount / (10 ** 18));
            ...
            //            } else if (_type == 5) {
            //                totalShareAllocation._totalAD5A += currentADA;
            //                totalShareAllocationList[investEndIndex]._totalAD5A = totalShareAllocation._totalAD5A;
            //            }
        }
    }
    ...
    function _claimDaoReward(address _user, uint256 _type) private {
        ...
        //        if (_type == 1) {
        _pendingReward = userInfoList[_user]._updatedRewardItem._updatedDaoRewardList[_type - 1];
        //        } else if (_type == 2) {
        ...
        //        } else if (_type == 5) {
        //            _pendingReward = userInfoList[_user]._updatedRewardItem._updatedDao5Reward;
        //        }
        // 获取用户剩余的授信额度
        uint256 _userRemainingCredit = _getUserRemainingCredit(_user);
        // 如果待领取奖励大于用户剩余授信额度，则将待领取奖励设为用户剩余授信额度
        ...
        userInfoList[_user]._claimedRewardItem._claimedDaoRewardList[_type - 1] += _pendingReward;
        userInfoList[_user]._updatedRewardItem._updatedDaoRewardList[_type - 1] -= _pendingReward;
        //        } else if (_type == 2) {
        //            daoRewardData._claimedDao2Reward += _pendingReward;
        ...
        //        } else if (_type == 3) {
        //            daoRewardData._claimedDao3Reward += _pendingReward;
        ...
        //        } else if (_type == 4) {
        //            daoRewardData._claimedDao4Reward += _pendingReward;
        ...
        //        } else if (_type == 5) {
        //            daoRewardData._claimedDao5Reward += _pendingReward;
        //            userInfoList[_user]._claimedRewardItem._claimedDao5Reward += _pendingReward;
        //            userInfoList[_user]._updatedRewardItem._updatedDao5Reward -= _pendingReward;
        //        }
        // 转移奖励给用户
    ...
    function claimDaoReward() external {
        ...
        for (uint56 i = 0; i < maxDaoType; i++) {
            ...
            //            _claimDaoReward(_user, 2);
            //            _claimDaoReward(_user, 3);
            //            _claimDaoReward(_user, 4);
            //            _claimDaoReward(_user, 5);
        }
    }
    ...
    function claimWeekReward() external {
        address _user = msg.sender;
        // 获取用户待领取的奖励金额
        uint256 _pendingReward = userInfoList[_user]._rankingRewardItem._updatedWeeKReward;
        // 将奖励转账给用户
        IERC20(daoToken).transfer(_user, _pendingReward);
        // 更新用户已领取的奖励金额
        userInfoList[_user]._rankingRewardItem._claimedWeekReward += _pendingReward;
        // 清空用户待领取的奖励金额
        userInfoList[_user]._rankingRewardItem._updatedWeeKReward -= _pendingReward;
    }
    ...
    function _transferReward(address _user, uint256 _pendingReward, string memory _type) private {
        ...
        // 计算用户剩余的授信额度
        uint256 _userRemainingCredit = _getUserRemainingCredit(_user);
        emit ClaimRewardEvent(_user, investIndex - 1, getYearMonthDay(block.timestamp, adjustHour), block.timestamp, _type, _userRemainingCredit/(10**18));
        // 更新用户的投资列表
    ...
    function _updateDaoRewad(address _user, uint256 _type) private {
        ...
        // 根据DAO类型更新用户的奖励
        //        if (_type == 1) {
        userInfoList[_user]._updatedRewardItem._updatedDaoRewardList[_type - 1] += _totalReward;
        //        } else if (_type == 2) {
        ...
        //        } else if (_type == 5) {
        //            userInfoList[_user]._updatedRewardItem._updatedDao5Reward += _totalReward;
        //        }
    }
    ...
    function _distributeInvitationReward(uint256 _amount, address _user) private {
        ...
        for (uint256 i = 0; i < _allReferrersNum; i++) {
            ...
            if (i < 12) {
                ...
                // 更新当前推荐人的邀请奖励和网体投资额
                if (i == 0) {
                    userInfoList[_ferrer]._updatedRewardItem._updatedDirectReferralReward += _rewardAmount;
                    _totalAmount += _rewardAmount;
                    emit DistributeInvitationRewardEvent(_ferrer, block.timestamp, investIndex, "updatedDirectReferralReward", _rewardAmount/(10**18));
                } else {
                    //二级间推要求有推荐人
                    if (i == 1 || (i > 1 && referrals[_ferrer].length > 2)) {
                        userInfoList[_ferrer]._updatedRewardItem._updatedIndirectReferralReward += _rewardAmount;
                        _totalAmount += _rewardAmount;
                        emit DistributeInvitationRewardEvent(_ferrer, block.timestamp, investIndex, "updatedIndirectReferralReward", _rewardAmount/(10**18));
                    }
                }
            }
        ...
        rewardNew += _leftAmount;
        emit RewardNewEvent(block.timestamp, investEndIndex, "RewardNewInvite", _leftAmount / (10 ** 18));
        //更新推荐人的最大网体
        _updateBigNetAmountList(_user);
    }

    //    function getMax(address[] memory _referrals, uint256 _num) private view returns (uint256) {
    ...
    //    }
    //
    //    function getMax2(address[] memory _referrals, uint256 _num) private view returns (uint256) {
    //        uint256 maxVal = 0;
    //        for (uint256 i = 0; i < _num; i++) {
    //            maxVal = userInfoList[_referrals[i]]._totalNetInvestment > maxVal ? userInfoList[_referrals[i]]._totalNetInvestment : maxVal;
    //        }
    //        return maxVal;
    //    }
    ...
    //    //遍历下级所有的网体，获得最大网体，再判断是否可以升级
    //    function _canUpgrade(address[] memory _referrals, uint256 _num, uint256 _allNetAmount, uint256 _daoAmount) private view returns (bool canUpgrade_) {
    //        uint256 totalSmallAmount;
    //        uint256 okAmount;
    //        for (uint256 i = 0; i < _num; i++) {
    //            if (userInfoList[_referrals[i]]._totalNetInvestment >= _daoAmount) {
    //                okAmount = userInfoList[_referrals[i]]._totalNetInvestment;
    //                break;
    //            }
    //        }
    //        totalSmallAmount = _allNetAmount - okAmount;
    //        if (okAmount >= _daoAmount && totalSmallAmount >= _daoAmount) {
    //            canUpgrade_ = true;
    //        }
    //    }
    ...
    function _updateDao(address _user, uint256 _investStartIndex) private {
        ...
        //获取用户当前等级
        uint256 _daoType = userDaoList[_user]._daoType;
        ...
        // 根据用户当前的等级进行升级操作
        //        if (_daoType == 0) {
        //            // 如果用户当前等级为0，则判断是否可以升级到等级4
        //            //            bool canUpgrade_ = _canUpgrade(_referrals, _num, _allNetAmount, dao4Amount);
        //            bool canUpgrade_ = _canUpgrade2(_user, _allNetAmount, daoAmountList[0]);
        //            if (canUpgrade_) {
        //                Dao1Set.add(_user);
        //                userDaoList[_user] = UserDaoItem(_investStartIndex, 1);
        //                emit UpgradeDaoEvent(_user, block.timestamp, _investStartIndex, "Dao1");
        //            }
        //        } else if (_daoType > 0) {
        // 如果用户当前等级为4，则判断是否可以升级到等级3
        //            bool canUpgrade_ = _canUpgrade(_referrals, _num, _allNetAmount, dao3Amount);
        ...
        if (canUpgrade_) {
            ...
            userDaoList[_user] = UserDaoItem(_investStartIndex, _daoType + 1);
            emit UpgradeDaoEvent(_user, block.timestamp, _investStartIndex, _daoType + 1);
        }
        //        } else if (userDaoList[_user]._daoType == 2) {
        //            // 如果用户当前等级为3，则判断是否可以升级到等级2
        //            //            bool canUpgrade_ = _canUpgrade(_referrals, _num, _allNetAmount, dao2Amount);
        //            bool canUpgrade_ = _canUpgrade2(_user, _allNetAmount, daoAmountList[2]);
        //            if (canUpgrade_) {
        //                Dao2Set.remove(_user);
        //                Dao3Set.add(_user);
        //                _updateDaoRewad(_user, 2);
        //                userDaoList[_user] = UserDaoItem(_investStartIndex, 3);
        //                emit UpgradeDaoEvent(_user, block.timestamp, _investStartIndex, "Dao3");
        //            }
        //        } else if (userDaoList[_user]._daoType == 3) {
        //            // 如果用户当前等级为2，则判断是否可以升级到等级1
        //            //            bool canUpgrade_ = _canUpgrade(_referrals, _num, _allNetAmount, dao1Amount);
        //            bool canUpgrade_ = _canUpgrade2(_user, _allNetAmount, daoAmountList[3]);
        //            if (canUpgrade_) {
        //                Dao3Set.remove(_user);
        //                Dao4Set.add(_user);
        //                _updateDaoRewad(_user, 3);
        //                userDaoList[_user] = UserDaoItem(_investStartIndex, 1);
        //                emit UpgradeDaoEvent(_user, block.timestamp, _investStartIndex, "Dao4");
        //            }
        //        } else if (userDaoList[_user]._daoType == 4) {
        //            // 如果用户当前等级为2，则判断是否可以升级到等级1
        //            //            bool canUpgrade_ = _canUpgrade(_referrals, _num, _allNetAmount, dao1Amount);
        //            bool canUpgrade_ = _canUpgrade2(_user, _allNetAmount, daoAmountList[4]);
        //            if (canUpgrade_) {
        //                Dao4Set.remove(_user);
        //                Dao5Set.add(_user);
        //                _updateDaoRewad(_user, 4);
        //                userDaoList[_user] = UserDaoItem(_investStartIndex, 5);
        //                emit UpgradeDaoEvent(_user, block.timestamp, _investStartIndex, "Dao5");
        //            }
        //        }
    }
...
contract SortList {
    function sortUint256List(uint256[] memory a) public pure returns (uint256[] memory){
...
contract StructForDao {
    ...
    struct BaseItem4 {
        ...
        address[] weekTop1AddressListForInvite;
        address[] weekTop1AddressListForInvest;
    }

    struct BaseItem3 {
        //        uint256 _allDao1Reward;
        //        uint256 _allDao2Reward;
        //        uint256 _allDao3Reward;
        //        uint256 _allDao4Reward;
        //        uint256 _allDao5Reward;
    ...
    struct ReturnItem {
        ...
        BaseItem3 baseItem3_;
        BaseItem4 BaseItem4_;
        address[] referrals_;
