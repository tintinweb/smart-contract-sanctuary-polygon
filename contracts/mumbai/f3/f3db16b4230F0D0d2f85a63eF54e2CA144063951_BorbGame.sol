...
contract BorbGame {
    ...
    event AddNewUser(address indexed user, address ref);
    event NewBetAdded(
    ...
    event BetClaimed(address indexed user, uint256 indexed betId);
    ...
    function makeBet(
        ...
        if (users[msg.sender] == false) {
            ...
            emit AddNewUser(msg.sender, _ref);
        }
    ...
    function claim(
        ...
        if (bets[_timeframe][_betId].claimed) {
        ...
        if (
            block.timestamp <
            bets[_timeframe][_betId].lockTimestamp +
        ...
        int256 closePrice = 0;
        uint256 houseFee = (bets[_timeframe][_betId].amount * 100) / 10000;
        ...
        address ref = referals[bets[_timeframe][_betId].user];
        //if user win (user bet up and lockPrice<closePrice or user bet down and lockPrice>closePrice)
        if (
            (bets[_timeframe][_betId].betType == BetType.Up &&
                bets[_timeframe][_betId].lockPrice < closePrice) ||
            (bets[_timeframe][_betId].betType == BetType.Down &&
                bets[_timeframe][_betId].lockPrice > closePrice)
        ) {
            pool.transferReward(
                bets[_timeframe][_betId].user,
                bets[_timeframe][_betId].potentialReward,
                houseFee,
                ref
            );
        } else {
            pool.unlock(bets[_timeframe][_betId].amount, houseFee, ref);
        }
        emit BetClaimed(bets[_timeframe][_betId].user, _betId);
    }
    ...
    function getClosePrice(
        ...
        do {
            ...
            roundId += 1;
            (, closePrice, priceTime, , ) = priceFeed.getRoundData(roundId);
        } while (priceTime < closeTime);
        return closePrice;
    }
...
//import "./BorbInvestNFT.sol";
import "./TokenPlus.sol";
import "hardhat/console.sol";

contract Pool {
    //1 usdt = 1 000 000
    //1 usdt = 1 000 000 000 usdt+
    uint256 public constant MIN_USDT_WEI_PRICE = 100; //_000_000;
    ...
    function makeDeposit(uint256 _amount) external payable {
        ...
        tokenPlus.mint(msg.sender, _amount / getTokenPlusBuyPrice());
        emit BuyPriceChanged(getTokenPlusBuyPrice());
        emit InvestmentAdded(msg.sender, _amount * getTokenPlusBuyPrice());
    }
uint256 public tokenPlusBuyPrice = MIN_USDT_WEI_PRICE;
    /*
     *  Получает цену для покупки токена. Если цена слишком низкая,
     */
    function getTokenPlusBuyPrice() public returns (uint256) {
        uint256 currentFreePoolBalance = usd.balanceOf(address(this)) -
            blockedUsdCount;

        if (currentFreePoolBalance == 0 || tokenPlus.totalSupply() == 0) {
            return MIN_USDT_WEI_PRICE;
        }            console.log("currentFreePoolBalance",currentFreePoolBalance * MIN_USDT_WEI_PRICE);
            console.log("ts",tokenPlus.totalSupply());
        uint256 price = (currentFreePoolBalance * tokenPlusBuyPrice) /
            tokenPlus.totalSupply();
                        console.log("price",price);
        tokenPlusBuyPrice = price < MIN_USDT_WEI_PRICE ? MIN_USDT_WEI_PRICE : price;
        return tokenPlusBuyPrice;
    }
    ...
    function getTokenPlusSellPrice() public view returns (uint256) {
        uint256 currentFreePoolBalance = usd.balanceOf(address(this)) -
            blockedUsdCount;
        return
            (currentFreePoolBalance * MIN_USDT_WEI_PRICE) /
            tokenPlus.totalSupply();
    }
     ...
     */
    function withdraw(uint256 _tokenPlusAmount) external {
