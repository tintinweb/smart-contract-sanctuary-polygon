...
import "./Interfaces/IContractNFTOffset.sol";

/*
    It is the single Verified Carbon Units to check
*/
contract C3VerifiedCarbonUnitsOffset is
    IContractNFTOffset,
    ...
    Common
{
    ...
    event VCUOMinted(address sender, uint256 tokenId);
    event VCUOMetaDataUpdated(uint256 tokenId, string url);

    mapping(uint256 => OffsetData) public list;
    ...
    function initialize(address _address) public virtual initializer {
        ...
        __ERC721_init_unchained(
            'C3: Verified Carbon Units Offset',
            'C3-VCUO'
        );
        orchestratorAddress = _address;
    }

    function mintOffset(address to, string memory _project, string memory _transferee, string memory _reason, uint256 vintage, uint256 amount) external override virtual whenNotPaused {

        // check if project id is real who call the method
        address projectFactory = IContractAddresses(orchestratorAddress).projectFactory();
        require(
            IContractProjectFactory(projectFactory).checkProject(_project, _msgSender()),
            'The caller is not the project linked to address'
        );

        _tokenIds.increment();
        uint256 newTokenId = _tokenIds.current();
        _safeMint(to, newTokenId);
        list[newTokenId].transferee = _transferee;
        list[newTokenId].reason = _reason;
        list[newTokenId].projectId = _project;
        list[newTokenId].projectAddress = _msgSender();
        list[newTokenId].vintage = vintage;
        list[newTokenId].amount = amount;
        emit VCUOMinted(to, newTokenId);
    ...
    **/
    function setMetadata(
        ...
        emit VCUOMetaDataUpdated(tokenId, uri);
...
import "../C3DefinitionsProject.sol";
...
interface IContractProject {
    ...
    function getProjectData(string memory _projectKey) external view returns(ProjectData memory);
...
interface IContractProjectFactory {
    ...
    function checkProject(string memory _project, address _address) external view returns (bool);
    function isTokenExists(address _address) external view returns (bool);
...
interface IContractNFTOffset {

    function mintOffset(address to, string memory _project, string memory _transferee, string memory _reason, uint256 vintage, uint256 amount) external;
