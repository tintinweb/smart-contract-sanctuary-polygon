...
interface IGravityGrade is IERC1155Upgradeable {
     ...
     */
    function setAllowedPaymentCurrencies(uint256 _saleId, address[] calldata _currencyAddresses) external;
...
contract EnergyIXTStakingGG is IEnergyIXTStakingGG, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function addReward(GGRewards memory _rewards) external onlyMod returns (uint256 _rewardId) {
        ...
        allRewards.push(ggRewards[s_rewardId]);
    ...
    function getAllRewards() external view returns (GGRewards[] memory) {
        return allRewards;
    }
}