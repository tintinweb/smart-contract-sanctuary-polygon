...
contract PubExchange is Ownable, ContractMetadata, PrimarySale {
    ...
    event BuyTokens(address buyer, uint256 tokenAmount, uint256 amount);
    event SellTokens(address seller, uint256 tokenAmount, uint256 amount);
    ...
    function tokenBalance() public view onlyOwner returns (uint256 _balance) {
    ...
    function buyToken(uint256 _amount) public payable returns (bool success) {
        uint256 _tokenAmount = (_amount * tokenPrice) /
            10 ** tokenDecimals /
            (10 ** (18 - tokenDecimals));
        ...
        bool received = token.transferFrom(msg.sender, recipient, _tokenAmount);
        require(received, "Failed to receive token from user");

        bool sent = PGC.transfer(msg.sender, _amount);
        require(sent, "Failed to send token to user");

        emit BuyTokens(msg.sender, _tokenAmount, _amount);

        return received && sent;
    }

    function sellToken(uint256 _amount) public payable returns (bool success) {
        uint256 _tokenAmount = (_amount * tokenPrice) /
            10 ** tokenDecimals /
            (10 ** (18 - tokenDecimals));
        uint256 allowance = PGC.allowance(msg.sender, address(this));
        ...
        bool received = PGC.transferFrom(msg.sender, recipient, _amount);
        require(received, "Failed to receive token from user");

        bool sent = token.transfer(msg.sender, _tokenAmount);
        require(sent, "Failed to send token to user");

        emit SellTokens(msg.sender, _tokenAmount, _amount);
