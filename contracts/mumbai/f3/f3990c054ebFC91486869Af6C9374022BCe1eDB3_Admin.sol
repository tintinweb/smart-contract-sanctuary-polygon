...
contract Admin is Initializable, AccessControl, BasicMetaTransaction, ISafe {
    ...
    function tradingBuy(
        ...
        uint256 tokenValue = oraclePrice(_amount * _gramsValue);
        if (tokenValue > 0) {
            // usdc.approve(tradingContract,_amount);
            usdc.transferFrom(_owner,roobaWallet, uint256(tokenValue));
            emit BuyTrading(_amount, _owner,tnxId);
        }
        else {
    ...
    function oraclePrice(uint256 _amount) public returns (uint256) {
        uint256 priceOfOne = priceFeed.price();
        price = ((priceOfOne * _amount)) / (31103 * 10**9);
        return price;
    }
