...
contract ProjectsManagerContract is Initializable, Common {
    ...
    function isWhitelisted(uint256 _projectId) external view returns (uint256) {
        ...
        uint256 lockedAmount = project.getLockedAmounts(
            msg.sender
        );
    ...
    function totalTokensLockedPerUser() external view returns (uint256) {
        uint256 _total = 0;
        for (uint256 i = 0; i < projects.length; i++) {
            Project project = Project(projects[i].addr);
            _total.add(project.getLockedAmounts(msg.sender));
        }
