...
library ERC721AStorage {
    ...
    address internal constant OWNER_ADDRESS = 0xD223573Cd4E758a0EE69A46B42EDA53d217d66aE;

    function diamondAddress() internal pure returns (address) {
...
error AvatarNFT__notCalledFromDiamond();
...
contract InitFacet is ERC721AUpgradeable {
    ...
    modifier callingFromDiamond() {
        if(address(this) != ERC721AStorage.diamondAddress()) revert AvatarNFT__notCalledFromDiamond();
        _;
    }
    function init() external onlyOwnerCan callingFromDiamond {
    ...
    function tokenURI(uint256 tokenId) callingFromDiamond public view virtual returns (string memory) {
     ...
     * - `tokenId` must exist.
     */
    function setTokenURI(uint256 tokenId, string memory _tokenURI) callingFromDiamond public onlyOwnerCan {
        if(tokenId > s()._currentIndex) revert AvatarNFT__tokenIdNotExists();
