...
contract UniqueSwapV1 {
    ...
    function _swap(
        ...
        console.log("token0: ", token0);
        console.log("amounts[1]: ", amountOut);

        (uint256 amount0Out, uint256 amount1Out) = input == token0
            ? (uint256(0), amountOut)
            : (amountOut, uint256(0));
        console.log("final amount0Out: ", amount0Out);
        console.log("finalamount1Out: ", amount1Out);
    ...
    function swapExactETHForTokens(
        ...
        payable(WMATIC).transfer(msg.value);
        IERC20(WMATIC).inM(address(this), msg.value);
        swap(uint256(0), amounts[1], msg.sender);
    ...
    function swapExactTokensForETH(
        ...
        swap(amounts[1], uint256(0), msg.sender);
        // _swap(amounts, path, msg.sender);
    }
    ...
    function mint(address to) internal lock returns (uint256 liquidity) {
        ...
        if (_totalSupply == 0) {
            ...
            IERC20(ULPT).relayMint(WMATIC, MINIMUM_LIQUIDITY); // here WMATIC is temporary address instead of zero address
        } else {
            liquidity = Math.min(
    ...
    function swap(
        ...
        uint256 balance1;
        {
            ...
            if (amount0Out > 0) {
                ...
                IERC20(_token0).ethTransfer(address(this), amount0Out);
