...
// import "https://github.com/axelarnetwork/axelar-gmp-sdk-solidity/blob/726020fd893059f9a469ca049f3cf5dd4b3d11c6/contracts/StringAddressUtils.sol";
...
contract FundRaiser is ERC721, IAxelarExecutable {
    ...
    ) ERC721("Top Contributor", "VIP") IAxelarExecutable(gateway_) {
    ...
    // * Modifiers
    // - onlyOwner => Only Owner can call these functions
    modifier onlyOwner() {
    ...
    function claimNFT() public {
        ...
        require(funds > 0.05 ether, "Not enough Donation");
    ...
    function _execute(
        ...
        if (keccak256(bytes(conditional)) == keccak256(bytes("fundsUpdate"))) {
            (string memory temp, uint newFunds, uint fundsOnSourceChain) = abi
                .decode(payload_, (string, uint, uint));
            // Update Funds Data on Current Chain
            fundsOnDestinationChain = fundsOnSourceChain;
            totalFunds += newFunds;

            // Emit Received Event
            emit fundsUpdateReceived(newFunds, sourceChain_, chainName);
        }
        // statusUpdate from Other Chain
        // ? Update status of the contract
        else if (
            keccak256(bytes(conditional)) == keccak256(bytes("statusUpdate"))
        ) {
            // Update Status on Current Chain
            (string memory temp, Status newStatus) = abi.decode(
                payload_,
                (string, Status)
            );
            status = newStatus;
        ...
        // ? Update totalFunds, fundsonOtherChain
            ...
            keccak256(bytes(conditional)) == keccak256(bytes("nftTransfer"))
        ) {
            (string memory temp, string memory _reciever, uint _tokenId) = abi
                .decode(payload_, (string, string, uint));
