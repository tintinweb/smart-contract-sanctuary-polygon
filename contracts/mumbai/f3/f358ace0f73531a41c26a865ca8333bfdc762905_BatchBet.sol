...
contract BatchBet is Errors {
    IERC20 public immutable token;
    bytes4 private immutable marketMakerV1a = 0x8be0519d;
    ...
    // TODO: does it need nonReentrant
    function batchBuy(
        ...
        uint256 totalInvestment = _investmentAmount.sum();
        token.safeTransferFrom(msg.sender, address(this), totalInvestment);

        for (uint256 i = 0; i < length; i++) {
            address market = _markets[i];
            uint256 investmentAmount = _investmentAmount[i];
            uint256 outcomeIndex = _outcomeIndex[i];
            uint256 minOutcomeTokensToBuy = _minOutcomeTokensToBuy[i];

            if (!market.supportsInterface(marketMakerV1a)) {
                revert NotAMarket(market);
            }
            IMarketMakerV1a amm = IMarketMakerV1a(market);

            token.safeApprove(address(amm), investmentAmount);

            // TODO: get fees
            // TODO: meta-transactions
            amm.buyFor(msg.sender, investmentAmount, outcomeIndex, minOutcomeTokensToBuy);
        }
    }
}
...
/// pieces
interface IMarketMakerV1b is IMarketMakerV1a, Errors {
