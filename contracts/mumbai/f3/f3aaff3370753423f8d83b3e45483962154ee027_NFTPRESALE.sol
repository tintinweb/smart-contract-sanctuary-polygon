...
// File: contracts/ERC721Registry/IOperatorFilterRegistry.sol
...
// File: contracts/ERC721Registry/OperatorFilterer.sol
...
// File: contracts/ERC721Registry/DefaultOperatorFilterer.sol
...
// File: contracts/ERC721Registry/ERC721Operator.sol
 ...
 */
abstract contract ERC721Operator is ERC721, DefaultOperatorFilterer, Ownable {
...
contract NFTPRESALE is ERC721Operator {
    using Strings for uint256;
    ...
    uint256 public preMintTokenLimit;
    ...
    constructor(string memory name, string memory symbol, uint256 _preTokenLimit, uint256 _tokenMaxLimit , uint256 _tokenPrice ) 
                ERC721(name, symbol) {
        ...
        preMintTokenLimit = _preTokenLimit;
        tokenCap = _tokenMaxLimit;
        tokenPrice = _tokenPrice; //0.07-ETH
    }
    ...
    function setMetaDataURL(uint _tokenId, string memory _name, string memory _imageUrl, string memory _metaDataUrl) public {
        require(whitelistAdmin == msg.sender, "Caller is not authorized");
        require(_exists(_tokenId),"ERC721Metadata: URI set of nonexistent token");
        tokenMetadata[_tokenId] = Metadata(_name, _imageUrl, _metaDataUrl);
        setTokenURL(_tokenId, _metaDataUrl);
    }

    //Below function is used Set _tokenURI as the tokenURI of the token
    function setTokenURL(uint256 _tokenId, string memory _tokenURI) internal {
        //only admin can set tokenURI
        require(whitelistAdmin == msg.sender, "User Not Authorized");
        //check the tokenId is exist before set the tokenURI for that tokenId
        require(_exists(_tokenId),"ERC721Metadata: URI set of nonexistent token");
        //if tokeid is exist then set the tokenURI as tokenURI for the given tokenId
    ...
    function preMint(
         ...
         uint256 royal
            ...
            require(NFTtokenId.current() <= preMintTokenLimit, "Token reaches the initial limit");
            NFTtokenId.increment();
            uint256 tokenId = NFTtokenId.current();
             tokenMetadata[tokenId] = Metadata(name, imageUrl, metadataUrl);
             _safeMint(msg.sender, tokenId);
             setTokenURL(tokenId, metadataUrl);
            if(royal > 0) {
                _royal[tokenId] = royal * 1e18;
             }
             //caller is going to mint only one token
             balances[tokenId][msg.sender] = 1;
             //transfer the service amount to the owner
             address payable admin = payable(owner());
             if(tokenPrice > 0) {
                admin.transfer(tokenPrice);
             }
    ...
    //         //check the caller/minter is whitelisted user
    //         require(whiteListedUser[msg.sender] == true, "Caller is not white listed");
    //         require(NFTtokenId.current() <= (preMintTokenLimit + tokenCap), "Token reaches the cap limit");
    //         NFTtokenId.increment();
    //         uint256 tokenId = NFTtokenId.current();
    //          tokenMetadata[tokenId] = Metadata(name, imageUrl, metadataUrl);
    //          _safeMint(msg.sender, tokenId);
