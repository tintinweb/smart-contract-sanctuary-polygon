...
contract Rewarder is IRewarder, SybelAccessControlUpgradeable, ContentBadges, ListenerBadges, PushPullReward {
    ...
    /**
     * @dev All the pending content pool rewards
     */
    mapping(uint256 => uint256) private pendingContentPoolRewards;
    ...
    /**
     * @dev Send all the pending content pools rewards
     */
    function sendContentPoolRewards(uint256 contentPoolId) external onlyRole(SybelRoles.REWARDER) whenNotPaused {
        // Find the reward and ensure it's present
        uint256 pendingReward = pendingContentPoolRewards[contentPoolId];
        if(pendingReward == 0) revert InvalidReward();

        // When good, reset it and send it to the content contentPool
        pendingContentPoolRewards[contentPoolId] = 0;
        contentPool.addReward(contentPoolId, pendingReward);
    }
}
