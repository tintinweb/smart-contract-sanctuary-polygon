...
contract Exchanger is Ownable, ReentrancyGuard {
    ...
    mapping(IERC20 => uint256) internal balances;
    mapping(IERC20 => bool) internal authorizedTokens;
     ...
     * @param amount - amount of tokens to send
     */
    function sendTo(address receiver, uint256 amount)
        external
        onlyKeeper
        onlyActive
        nonReentrant
     ...
     * @notice deposit eth
     */
    function depositEth() external payable onlyActive zeroAddress(msg.sender) {
     ...
     */
    function depositTokens(IERC20 token, uint256 amount) external nonReentrant onlyActive {
        if (!authorizedTokens[token]) revert NotAuthorizedToken();
        if (amount == 0) revert IncorrectAmount();
        ...
        emit DepositTokens(token, msg.sender, amount);
    }
    ...
    function withdrawTokens(IERC20 token, uint256 _amount) external onlyOwner {
        ...
        emit WithdrawToken(token, _amount);
    }

    /**
     * @dev only owner can set authorized tokens
     *
     * @param tokens - array of tokens
     * @param status - true or false status of token
     */
    function setAuthorizedToken(IERC20[] memory tokens, bool status) external onlyOwner {
        for (uint256 i = 0; i < tokens.length; i++) {
            if (IERC20(tokens[i]) != IERC20(address(0))) {
                authorizedTokens[tokens[i]] = status;

                emit ChangedAuthorizedTokens(tokens[i], status);
            }
        }
    }
    ...
    function _setKeeper(address _keeper) internal onlyOwner zeroAddress(_keeper) {
        keeper = _keeper;
    ...
    /**
     * @dev return balance amount in specified token
     *
     * @param token - token address
     */
     ...
     * @param token - token address
     */
    function isTokenAuthorized(IERC20 token) external view returns (bool) {
