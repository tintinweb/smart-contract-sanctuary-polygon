...
contract NextMetaverse is ERC721Enumerable, Ownable, ChainlinkClient {
    using Chainlink for Chainlink.Request;
    using Strings for uint256;
    string public baseURI;
    string public baseExtension = ".json";
    uint256 public maxSupply = 9999;
    uint256 public maxLimitWhitelist = 10;
    uint256 public minLimit = 10;

    uint256 public maxLimitNonWhitelist = 2;
    ...
    mapping(bytes32 => mintInfo) public requestMapping;
    ...
    function requestPrice(
        ...
        string memory url = string(
            abi.encodePacked(
                "https://api.nextmetaverse.io/buildings/",
                _id.toString()
            )
        );
        request.add("get", url);
        ...
        requestMapping[_requestId] = mintInfo(_to, _id, _value);
        return _requestId;
    }
    ...
    {
        price = _price;
        require(requestMapping[_requestId].value == _price, "pay price of nft");
        _safeMint(
            requestMapping[_requestId].mintTo,
            requestMapping[_requestId].tokenId
        );
    }
    ...
    function mint(address _to, uint256 _id) public payable {
        ...
        require(!_exists(_id), "token id already exist");
        require(supply + 1 <= maxSupply);

        if (msg.sender != owner()) {
            require(msg.value > minLimit, "insufficient funds");
            uint256 minted = balanceOf(_to);
            if (whitelist[_to] == true) {
                require(
                    maxLimitWhitelist >= minted + 1,
                    "cannot mint more than limit"
                );
            } else {
                require(
                    onlyWhiteList == false,
                    "only whitelist addresses can mint"
                );
                require(
                    maxLimitNonWhitelist >= minted + 1,
                    "cannot mint more than limit"
                );
            }
            requestPrice(_to, _id, msg.value);
        } else {
            _safeMint(_to, _id);
        }
    }
    ...
    function setMinLimit(uint256 _minLimit) public onlyOwner {
        minLimit = _minLimit;
    }
    ...
    function addWhitelistAccounts(address[] memory _accounts) public onlyOwner {
