...
contract Challenge {
    ...
    uint256 public entryFee = 1;
        // entry fee to challenge
    uint256 public maxRewardedParticipants = 1;
    // challenge state
    bool public isFinished = false;

    // store participant addresses 
    address[] public participantWallets;
    ...
    function award() public senderIsAddress onlyOwner challengeIsActive {
        ...
        for (uint256 i = 0; i < sortedParticipants.length; i++) {
            (bool success, ) = payable(sortedParticipants[i].wallet).call{value: this.getRewards(sortedParticipants.length)}("");
    ...
    function getRewards(uint256 currentParticipantsCount) public view returns (uint256) {
        return address(this).balance/currentParticipantsCount;
    }
}