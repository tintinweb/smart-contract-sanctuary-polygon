...
contract Lottery {
    ...
    function distributeUSDT(uint256 _amount, address[] memory _participants, uint256[] memory _prizePercentages) public returns (address[] memory) {
        ...
        require(usdtToken.allowance(msg.sender, address(this)) >= _amount, "Contract not allowed to transfer enough USDT");
        require(usdtToken.balanceOf(msg.sender) >= _amount, "Not enough USDT balance");
        ...
        for (uint256 i = 0; i < _prizePercentages.length; i++) {
            ...
            winners[i] = winner;
            usdtToken.transferFrom(msg.sender, winner, prizeAmount);
