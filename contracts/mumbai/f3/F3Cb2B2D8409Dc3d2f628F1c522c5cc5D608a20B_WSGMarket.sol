...
contract WSGMarket is Context, ReentrancyGuard, Ownable {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;
    Counters.Counter private _tokensSold;

    address private _nftContractAddress;
    uint256 private _listingPrice = 0.045 ether;
    ...
    modifier hasNFTContractAddress() {
        require(_nftContractAddress != address(0), "NFT contract required");
        _;
    }

    function setupNFTContractAddress(address nftContractAddress_) external {
    ...
    function createMarketToken(uint256 tokenId, uint256 price)
        external
        payable
        nonReentrant
        hasNFTContractAddress
    {
        // nonReentrant is a modifier to prevent reentry attack
        require(price > 0, "Price must be at least one wei");
        require(msg.value == _listingPrice, "Price must equal listing price");
        ...
        hasNFTContractAddress
    {
        ...
        MarketToken memory token = _idToMarketToken[itemId];

        // transfer the amount to the seller
        token.seller.transfer(msg.value);
        token.owner = payable(_msgSender());
        token.sold = true;
        ...
        // buyer (owner) pays listing price to wsg market
        payable(owner()).transfer(_listingPrice);
    }

    // function for returning an array of created nfts for sales
    function fetchMarketTokens() external view returns (MarketToken[] memory) {
        ...
        for (uint256 i = 1; i <= itemCount; i++) {
            if (_idToMarketToken[i].owner == address(0)) {
                MarketToken memory currentItem = _idToMarketToken[i];
    ...
    function fetchItemsCreated() external view returns (MarketToken[] memory) {
        ...
        for (uint256 i = 1; i <= totalItemCount; i++) {
            if (_idToMarketToken[i].seller == msg.sender) {
                itemCount += 1;
            }
        }
        ...
        for (uint256 i = 0; i < totalItemCount; i++) {
            if (_idToMarketToken[i].seller == msg.sender) {
                uint256 currentId = _idToMarketToken[i].itemId;
