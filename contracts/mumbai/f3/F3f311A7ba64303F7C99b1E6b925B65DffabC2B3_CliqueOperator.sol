...
contract CliqueOperator is CliqueClient, Ownable {
  ...
  function sendOracleRequest(uint256 taskId, bytes memory data) public {
  ...
  function fulfill(uint256 _requestId) public recordFulfillment(_requestId) {}
}
...
/// @title CliqueClient abstract contract
/// @author Clique
/// @custom:coauthor ollie (eillo.eth)
/// @notice This contract is to be inherited by Clique clients.
abstract contract CliqueClient {
  ...
  modifier recordFulfillment(uint256 requestId) {
    if (msg.sender != _pendingRequests[requestId])
      revert InvalidSource(_pendingRequests[requestId], msg.sender);
