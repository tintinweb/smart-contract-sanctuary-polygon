...
contract Board is Ownable{
    ...
    modifier winnerOnly {
        ...
        require(winners[msg.sender].totalWinnings > 0 /* || backwardWinners(msg.sender) > 0 */);
        _;
    }

    function buySquare(uint row, uint cell) external payable {
        require(boardStatus != BoardStatus.CANCELED, "Board CANCELED. NO MORE PURCHASES ALLOWED.");
        require(boardStatus != BoardStatus.SOLDOUT, "Board SOLDOUT. NO OPEN SQUAREZ AVAILABLE.");
        require(buyerSquareCount[msg.sender] < 10,"You can not buy more than 10 Squarez on one board.");
        ...
        if(selectedSquare.purchaser == address(0x0)){
            // is available
            require(feeToken.transferFrom(msg.sender, address(this), boardFee), "Please pay full board fee.");
    ...
    function scrambleDownAndAcross() public adminOrHostOnly {
        if(shuffled){
            revert("Board already shuffled. awaiting finals scores...");
        } else {
    ...
    function getGameScore() public adminOrHostOnly {
        //BigBoardzFactory scoreSource = BigBoardzFactory(admin);
        //BigBoardManagerLib.Factory storage f = BigBoardManagerLib.getFactory();

        game = BigBoardManagerLib.getGameFromScores(gameId);
        
        game.status = BigBoardManagerLib.GameStatus.OVER;
    }
    ...
    function collectPrize() public winnerOnly {
        ...
        if(prizeCollectedByWinner[msg.sender]){
            revert("You already collected your winnings. Thanks for playing.");
        } else{
    ...
    function collectHostFee() external onlyHost {
        ...
        uint256 hostFeeAmount =  boardFee * (10 - availableSquarezCount);
        feeToken.transfer(msg.sender, hostFeeAmount);
        hostFeeCollected = true;
    }

    function collectRefund() external onlyBuyer {
        uint256 refundAmount = buyerSquareCount[msg.sender] * boardFee;
        feeToken.transfer(msg.sender, refundAmount);
