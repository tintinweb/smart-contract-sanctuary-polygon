...
contract ffwc22lottoRouter is ChainlinkClient {
    ...
    string private _MATCHID;
    // bytes32 private _MATCHIDReqId;
    //team name in final
    ...
    // AWAY starting XI
    uint8 private _AWAYNO1;
    bytes32 private _AWAYNO1ReqId ;
    uint8 private _AWAYNO2;
    bytes32 private _AWAYNO2ReqId ;
    uint8 private _AWAYNO3;
    bytes32 private _AWAYNO3ReqId ;
    uint8 private _AWAYNO4;
    bytes32 private _AWAYNO4ReqId ;
    uint8 private _AWAYNO5;
    bytes32 private _AWAYNO5ReqId ;
    uint8 private _AWAYNO6;
    bytes32 private _AWAYNO6ReqId ;
    uint8 private _AWAYNO7;
    bytes32 private _AWAYNO7ReqId ;
    uint8 private _AWAYNO8;
    bytes32 private _AWAYNO8ReqId ;
    uint8 private _AWAYNO9;
    bytes32 private _AWAYNO9ReqId ;
    uint8 private _AWAYNO10;
    bytes32 private _AWAYNO10ReqId ;
    uint8 private _AWAYNO11;
    bytes32 private _AWAYNO11ReqId ;
    ...
    //old winning prize (COPA2021)
    uint16 private _superChampionCodeCOPA21;
    uint8 private _championNationIdCOPA21;
    uint8 private _runnerupNationIdCOPA21;
    uint32 private _lastFulFillTimestampCOPA21;

    //sportapi MatchId
    string WC22FinalMatchID = "429770"; 
    uint32 WC22FinalMatchIDInt = 429770; 
    string COPA21FinalMatchID = "314832"; 
    uint32 COPA21FinalMatchIDInt = 314832; 
    ...
    constructor() {
        ...
        //set sale period
        _closedSaleTimestamp = 1671375600; // FFwc 22 Final start
    ...
    //         if (_isTicketClaimed[number] == false) {
    ...
    //     }
    //     //check if this ticket is claimed
    //     if (_isTicketClaimed[number] == true) {
    //         return 0;
    //     }
    //     claimable = 0;
    ...
    function getCOPA21()
        ...
        virtual
        returns (
            uint32 lastFulFillTimestampCOPA21,
            uint16 superChampionCodeCOPA21,
            uint8 championNationIdCOPA21,
            uint8 runnerupNationIdCOPA21
        )
    {
        return ( _lastFulFillTimestampCOPA21, _superChampionCodeCOPA21, _championNationIdCOPA21, _runnerupNationIdCOPA21 );
    }

    function getJERSEYNos()
        public
        view
        virtual
        returns (
    ...
    function getMULJERSEY()
        ...
        returns (
            ...
            uint8 HOMEGOAL,
            uint8 AWAYGOAL,
            uint256 MULAWAYJERSEY){

        MULAWAYJERSEY = uint256 (
            _AWAYNO1 * _AWAYNO2 * _AWAYNO3 * _AWAYNO4 * _AWAYNO5 * _AWAYNO6 * 
            _AWAYNO7 * _AWAYNO8 * _AWAYNO9 * _AWAYNO10 * _AWAYNO11
        );
        ...
        returns (bool success)
    {
        ...
        return reqSportdataWithChainLink( sportdataAPIKEY, WC22FinalMatchID);
    }
    function reqCOPA21(string memory sportdataAPIKEY, uint32 deadline)
        external
        virtual
        ensure(deadline)
        returns (bool success)
    {
        ...
        //chainlink => sportdataapi
        return reqSportdataWithChainLink( sportdataAPIKEY, COPA21FinalMatchID);
    }

    function reqSportdataWithChainLink(string memory APIKEY,string memory matchID)
        private
        returns (bool success)
    {
        string memory matchUrl = string( abi.encodePacked( "https://app.sportdataapi.com/api/v1/soccer/matches/", matchID, "?apikey=", APIKEY ) );        
        ...
        //get 2 team nation code (String)
        {
            ...
            req.add("path", "data,home_team,short_code");
            ...
            req.add("get", matchUrl);
            req.add("path", "data,away_team,short_code");
        ...
        //get 2 team #goal (Int)
        {
            ...
            req.add("get", matchUrl);
            req.add("path", "data,match_statistics,0,goals");
            req.addInt("times", 1);
            ...
            req.add("get", matchUrl);
            req.add("path", "data,match_statistics,1,goals");
            req.addInt("times", 1);
            _AWAYGOALReqId = sendChainlinkRequest(req, LINK_fee);
        }

        //reset 11 players jersey No.  (Int)
        _AWAYNO1 = 0;
        _AWAYNO2 = 0;
        _AWAYNO3 = 0;
        _AWAYNO4 = 0;
        ...
        //get 11 players jersey No.  (Int)
        {
            ...
            req.add("get", matchUrl);
            req.add("path", "data,lineups,1,players,0,detailed,number");
            req.addInt("times", 1);
            _AWAYNO1ReqId  = sendChainlinkRequest(req, LINK_fee);
            ...
            req.add("get", matchUrl);
            req.add("path", "data,lineups,1,players,1,detailed,number");
            req.addInt("times", 1);
            _AWAYNO2ReqId  = sendChainlinkRequest(req, LINK_fee);
            ...
            req.add("get", matchUrl);
            req.add("path", "data,lineups,1,players,2,detailed,number");
            req.addInt("times", 1);
           _AWAYNO3ReqId  = sendChainlinkRequest(req, LINK_fee);
            ...
            req.add("get", matchUrl);
            req.add("path", "data,lineups,1,players,3,detailed,number");
            req.addInt("times", 1);
           _AWAYNO4ReqId  = sendChainlinkRequest(req, LINK_fee);
            ...
            req.add("get", matchUrl);
            req.add("path", "data,lineups,1,players,4,detailed,number");
            req.addInt("times", 1);
           _AWAYNO5ReqId  = sendChainlinkRequest(req, LINK_fee);
            ...
            req.add("get", matchUrl);
            req.add("path", "data,lineups,1,players,5,detailed,number");
            req.addInt("times", 1);
           _AWAYNO6ReqId  = sendChainlinkRequest(req, LINK_fee);
            ...
            req.add("get", matchUrl);
            req.add("path", "data,lineups,1,players,6,detailed,number");
            req.addInt("times", 1);
           _AWAYNO7ReqId  = sendChainlinkRequest(req, LINK_fee);
            ...
            req.add("get", matchUrl);
            req.add("path", "data,lineups,1,players,7,detailed,number");
            req.addInt("times", 1);
           _AWAYNO8ReqId  = sendChainlinkRequest(req, LINK_fee);
            ...
            req.add("get", matchUrl);
            req.add("path", "data,lineups,1,players,8,detailed,number");
            req.addInt("times", 1);
           _AWAYNO9ReqId  = sendChainlinkRequest(req, LINK_fee);
            ...
            req.add("get", matchUrl);
            req.add("path", "data,lineups,1,players,9,detailed,number");
            req.addInt("times", 1);
           _AWAYNO10ReqId  = sendChainlinkRequest(req, LINK_fee);
            ...
            req.add("get", matchUrl);
            req.add("path", "data,lineups,1,players,10,detailed,number");
            req.addInt("times", 1);
           _AWAYNO11ReqId  = sendChainlinkRequest(req, LINK_fee);    
        }
        ...
        recordChainlinkFulfillment(requestId)
    {
        ...
        } else if (requestId == _AWAYNO1ReqId ) {
            _AWAYNO1 = uint8(response);
        } else if (requestId == _AWAYNO2ReqId ) {
            _AWAYNO2 = uint8(response);
        } else if (requestId == _AWAYNO3ReqId ) {
            _AWAYNO3 = uint8(response);
        } else if (requestId == _AWAYNO4ReqId ) {
            _AWAYNO4 = uint8(response);
        } else if (requestId == _AWAYNO5ReqId ) {
            _AWAYNO5 = uint8(response);
        } else if (requestId == _AWAYNO6ReqId ) {
            _AWAYNO6 = uint8(response);
        } else if (requestId == _AWAYNO7ReqId ) {
            _AWAYNO7 = uint8(response);
        } else if (requestId == _AWAYNO8ReqId ) {
            _AWAYNO8 = uint8(response);
        } else if (requestId == _AWAYNO9ReqId ) {
            _AWAYNO9 = uint8(response);
        } else if (requestId == _AWAYNO10ReqId ) {
            _AWAYNO10 = uint8(response);
        } else if (requestId == _AWAYNO11ReqId ) {
            _AWAYNO11 = uint8(response);
        }
    ...
    function updateIffullyfulfill() private {
        ...
        //all jersey no. multiply each other , if where is any 0 => result = 0
        uint256 MULAWAYJERSEY = uint256 (
            _AWAYNO1 * _AWAYNO2 * _AWAYNO3 * _AWAYNO4 * _AWAYNO5 * _AWAYNO6 * 
            _AWAYNO7 * _AWAYNO8 * _AWAYNO9 * _AWAYNO10 * _AWAYNO11
        );
        ...
        if(
            MULAWAYJERSEY > 0 && //all jersey Nos must not be 0
            // (_MATCHID == WC22FinalMatchID || _MATCHID == COPA21FinalMatchID) && 
            (
                keccak256(abi.encodePacked(_MATCHID)) == keccak256(abi.encodePacked(WC22FinalMatchID)) || 
                keccak256(abi.encodePacked(_MATCHID)) == keccak256(abi.encodePacked(COPA21FinalMatchID))
            ) && 
            ...
            //READY TO ANNOUNCE
            // bool isWC22 = _MATCHID == WC22FinalMatchID; // else COPA21
            bool isWC22 =  (keccak256(abi.encodePacked((_MATCHID))) == keccak256(abi.encodePacked((WC22FinalMatchID)))); // else COPA21
            ...
            if(isWC22){//save data for WC22
                ...
                _superChampionCodeWC22 = uint16(3);                            
                _lastFulFillTimestampWC22 = uint32( block.timestamp );
        
                //mark that the result is announced
                _isAnnounced = true;
            }else{//save data for COPA21
                _championNationIdCOPA21 = championNationId;
                _runnerupNationIdCOPA21 = runnerupNationId;
                _superChampionCodeCOPA21 = uint16(3);
                _lastFulFillTimestampCOPA21 = uint32( block.timestamp );
            }
        }
    }
}