...
contract MyToken is Context, Ownable, EventToken {
    ...
    function safeTransfer(address from, address to, uint256 tokenId, uint256 price) public {
        require(balances[to] >= price, "Khong hop le");
        unchecked {
            _balances[from] -= 1;
            _balances[to] += 1;
            balances[to] = balances[to].sub(price);
            balances[from] = balances[from].add(price);
        }
        _owners[tokenId] = to;
    }
