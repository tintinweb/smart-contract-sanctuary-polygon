...
contract MustasheRaffle is ERC721Holder, Ownable, AutomationCompatibleInterface {
    ...
    address public tokenAddress = 0x8909fCF37B1A7382dBA034CC7bC71BEc6E79811F; // $AMBER address
    ...
    uint256 public checkRaffleExpiryInterval; // Interval in seconds to check for Raffle expiry
    uint256 public lastExpiryCheckTime; // Timestamp of the last time Raffles were checked for expiry
    ...
    constructor(address _chainlinkKeeper) {
        ...
        checkRaffleExpiryInterval = 600; // Set the interval to check for Raffle expiry to 10 min.
        lastExpiryCheckTime = block.timestamp; // Initialize the last expiry check time to the contract's deployment time
    }
    ...
    modifier onlyKeeper() {
        require(msg.sender == address(chainlinkKeeper), "Only Chainlink Keeper can call this function");
        _;
    }
    ...
    function performUpkeep(bytes calldata) external override {
        if (block.timestamp >= lastExpiryCheckTime + checkRaffleExpiryInterval) {
            for (uint256 i = 1; i <= raffleCount; i++) {
                if (!raffles[i].ended && raffles[i].endTime <= block.timestamp) {
                    endRaffleIfExpiredAndNotFullySoldOut(i);
                }
            }
            lastExpiryCheckTime = block.timestamp;
        }
    }

    function endRaffleIfExpiredAndNotFullySoldOut(uint256 raffleId) internal raffleNotEnded(raffleId) {
