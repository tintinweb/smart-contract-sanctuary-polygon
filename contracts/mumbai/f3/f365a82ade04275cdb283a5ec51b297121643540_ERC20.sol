...
// File: contracts/ShipAgeFuel.sol


pragma solidity ^0.8.2;


contract ShipAgeFuel is ERC20, AccessControl, ERC721Holder {
    bytes32 public constant CONTRACT_ADMIN_ROLE = keccak256("CONTRACT_ADMIN_ROLE");
    bytes32 public constant BURNER_ROLE = keccak256("BURNER_ROLE");
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    address public collectionAddressShipAgeNFT;
    address public collectionAddressNFTv2;

    uint256 public numberOfBlocksPerRewardUnit;

    uint256 public coinAmountPerRewardUnitShipAgeNFT;
    uint256 public coinAmountPerRewardUnitNFTv2;

    uint256 public welcomeBonusAmountShipAgeNFT;
    uint256 public welcomeBonusAmountNFTv2;

    uint256 public amountOfStakersShipAgeNFT;
    uint256 public amountOfStakersNFTv2;

    uint256 public tokensStakedShipAgeNFT;
    ...
    mapping (address => mapping(uint256 => StakeInfo)) public stakeLogShipAgeNFT;

        /// owner => tokenId => StakeInfo
    mapping (address => mapping(uint256 => StakeInfo)) public stakeLogNFTv2;

    /// owner => #NFTsStaked
    mapping (address => uint256) public tokensStakedByUserShipAgeNFT;
    ...
    mapping (uint256 => bool) public welcomeBonusCollectedShipAgeNFT;

        /// tokenId => true/false (true if welcome bonus has been collected for a specific tokenId)
    mapping (uint256 => bool) public welcomeBonusCollectedNFTv2;

    /// owner => list of all tokenIds that the user has staked
    mapping (address => uint256[]) public stakePortfolioByUserShipAgeNFT;

        /// owner => list of all tokenIds that the user has staked
    mapping (address => uint256[]) public stakePortfolioByUserNFTv2;

    /// tokenId => indexInStakePortfolio
    mapping(uint256 => uint256) public indexOfTokenIdInStakePortfolioShipAgeNFT;

        /// tokenId => indexInStakePortfolio
    mapping(uint256 => uint256) public indexOfTokenIdInStakePortfolioNFTv2;

    event RewardsHarvestedShipAgeNFT (address owner, uint256 amount);
    event NFTStakedShipAgeNFT (address owner, uint256 tokenId);
    event NFTUnstakedShipAgeNFT (address owner, uint256 tokenId);
    ...
    constructor(address owner, address _collectionAddressShipAgeNFT, address _collectionAddressNFTv2) ERC20("ShipAgeFuel", "SFUEL") AccessControl(){
        _mint(owner, 100000 * 10 ** 18);
        _setupRole(DEFAULT_ADMIN_ROLE, owner);
        _setupRole(CONTRACT_ADMIN_ROLE, owner);
        _setupRole(BURNER_ROLE, owner);
        _setupRole(MINTER_ROLE, owner);
        collectionAddressShipAgeNFT = _collectionAddressShipAgeNFT;
        collectionAddressNFTv2 = _collectionAddressNFTv2;
        contractCreationBlock = block.number;
        coinAmountPerRewardUnitShipAgeNFT = 5 * 10 ** 18; // 5 ERC20 coins per rewardUnit, may be changed later on
        ...
        welcomeBonusAmountShipAgeNFT = 300 * 10 ** 18; // 300 tokens welcome bonus, only paid once per tokenId
        welcomeBonusAmountNFTv2 = 600 * 10 ** 18; // 600 tokens welcome bonus, only paid once per tokenId
    }

    function stakedNFTSByUserShipAgeNFT(address owner) external view returns (uint256[] memory){
        return stakePortfolioByUserShipAgeNFT[owner];
    }
    ...
    function pendingRewardsShipAgeNFT(address owner, uint256 tokenId) public view returns (uint256){
        StakeInfo memory info = stakeLogShipAgeNFT[owner][tokenId];
        ...
        return rewardAmount * coinAmountPerRewardUnitShipAgeNFT;
    ...
    function stakeShipAgeNFT(uint256 tokenId) public {
        IERC721(collectionAddressShipAgeNFT).safeTransferFrom(_msgSender(), address(this), tokenId);
        require(IERC721(collectionAddressShipAgeNFT).ownerOf(tokenId) == address(this),
            "ShipAgeFuel: Error while transferring token");
        StakeInfo storage info = stakeLogShipAgeNFT[_msgSender()][tokenId];
        info.stakedAtBlock = block.number;
        info.lastHarvestBlock = block.number;
        info.currentlyStaked = true;
        if(tokensStakedByUserShipAgeNFT[_msgSender()] == 0){
            amountOfStakersShipAgeNFT += 1;
        }
        tokensStakedByUserShipAgeNFT[_msgSender()] += 1;
        tokensStakedShipAgeNFT += 1;
        stakePortfolioByUserShipAgeNFT[_msgSender()].push(tokenId);
        uint256 indexOfNewElement = stakePortfolioByUserShipAgeNFT[_msgSender()].length - 1;
        indexOfTokenIdInStakePortfolioShipAgeNFT[tokenId] = indexOfNewElement;
        if(!welcomeBonusCollectedShipAgeNFT[tokenId]) {
            _mint(_msgSender(), welcomeBonusAmountShipAgeNFT);
            welcomeBonusCollectedShipAgeNFT[tokenId] = true;
        }

        emit NFTStakedShipAgeNFT(_msgSender(), tokenId);
    }
    ...
    function stakeBatchShipAgeNFT(uint256[] memory tokenIds) external {
        for(uint currentId = 0; currentId < tokenIds.length; currentId++) {
            if(tokenIds[currentId] == 0) {
                continue;
            }
            stakeShipAgeNFT(tokenIds[currentId]);
        }
    }
    ...
    function unstakeBatchShipAgeNFT(uint256[] memory tokenIds) external {
        for(uint currentId = 0; currentId < tokenIds.length; currentId++) {
            if(tokenIds[currentId] == 0) {
                continue;
            }
            unstakeShipAgeNFT(tokenIds[currentId]);
        }
    }
    ...
    function unstakeShipAgeNFT(uint256 tokenId) public {
        if(pendingRewardsShipAgeNFT(_msgSender(), tokenId) > 0){
            harvestShipAgeNFT(tokenId);
        }
        StakeInfo storage info = stakeLogShipAgeNFT[_msgSender()][tokenId];
        info.currentlyStaked = true;
        IERC721(collectionAddressShipAgeNFT).safeTransferFrom(address(this), _msgSender(), tokenId);
        require(IERC721(collectionAddressShipAgeNFT).ownerOf(tokenId) == _msgSender(),
            "ShipAgeFuel: Error while transferring token");
        if(tokensStakedByUserShipAgeNFT[_msgSender()] == 1){
            amountOfStakersShipAgeNFT -= 1;
        }
        tokensStakedByUserShipAgeNFT[_msgSender()] -= 1;
        tokensStakedShipAgeNFT -= 1;
        stakePortfolioByUserShipAgeNFT[_msgSender()][indexOfTokenIdInStakePortfolioShipAgeNFT[tokenId]] = 0;
        emit NFTUnstakedShipAgeNFT(_msgSender(), tokenId);
    }
    ...
    function harvestShipAgeNFT(uint256 tokenId) public {
        StakeInfo storage info = stakeLogShipAgeNFT[_msgSender()][tokenId];
        uint256 rewardAmountInERC20Tokens = pendingRewardsShipAgeNFT(_msgSender(), tokenId);
        if(rewardAmountInERC20Tokens > 0) {
            info.lastHarvestBlock = block.number;
            _mint(_msgSender(), rewardAmountInERC20Tokens);
            emit RewardsHarvestedShipAgeNFT(_msgSender(), rewardAmountInERC20Tokens);
        }
    }
    ...
    function harvestBatchShipAgeNFT(address user) external {
        uint256[] memory tokenIds = stakePortfolioByUserShipAgeNFT[user];

        for(uint currentId = 0; currentId < tokenIds.length; currentId++) {
            if(tokenIds[currentId] == 0) {
                continue;
            }
            harvestShipAgeNFT(tokenIds[currentId]);
        }
    }
    ...
    function setCoinAmountPerRewardUnitShipAgeNFT(uint256 coinAmount) external onlyRole(CONTRACT_ADMIN_ROLE){
        coinAmountPerRewardUnitShipAgeNFT = coinAmount;
    }
    ...
    function setWelcomeBonusAmountShipAgeNFT(uint256 coinAmount) external onlyRole(CONTRACT_ADMIN_ROLE){
        welcomeBonusAmountShipAgeNFT = coinAmount;
    }
    ...
    function setCollectionAddressShipAgeNFT(address newAddress) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddress != address(0), "VIBE: update to zero address not possible");
        collectionAddressShipAgeNFT = newAddress;
    }
