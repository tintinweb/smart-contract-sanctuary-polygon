...
contract NFTMarketplace is ReentrancyGuard, Ownable {
    ...
    address payable public feeAccount; // the account that recieves fees
    ...
    address payable public royaltyRecipient = payable(address(0));
    uint public royaltyPercent = 1000;
    ...
    function createMarketSale(address nftContract, uint256 itemId) public payable nonReentrant {
        ...
        payable(royaltyRecipient).transfer(royaltyPercent * price / 10000);
        uint256 feeAmount = feePercent * price / 10000;
        // transfer the (item price - royalty amount - fee amount) to the seller
        item.seller.transfer(price - royaltyPercent * price / 10000 - feeAmount);

        IERC721(nftContract).transferFrom(address(this), msg.sender, tokenId);

        _itemsSold.increment();
        if (listingPrice != 0) payable(feeAccount).transfer(listingPrice);
        payable(feeAccount).transfer(royaltyPercent * price / 10000);
