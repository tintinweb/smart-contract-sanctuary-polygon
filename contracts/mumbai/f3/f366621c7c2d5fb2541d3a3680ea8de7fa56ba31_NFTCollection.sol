...
import "../SplitterContract/SplitterContract.sol";
    ...
    ReentrancyGuardUpgradeable
{
    ...
    address public adminRoyaltyAccount;
    address[] private allCollections; // all created collections through this contract
    address[] private payees;

    uint96 public royaltyFee; // royalty fee percentage
    uint256 public platformFee; // platform fee
    uint256 public adminRoyaltyPercentage;
    ...
    mapping(address => mapping(address => string[]))
        public collectionAttributes; // maps the user account with his/her created collection address and it's attributes array
    mapping(address => bool) public isCollectionDestroyed;
    ...
    event AdminRoyaltyAccountUpdated(address indexed account);
    event AdminRoyaltyPercentageUpdated(uint256 _newPercentage);
     ...
     * @param erc721BeaconProxyAddress, address of the beacon proxy.
     * @param _platformFee platform fee.
     * @param _royaltyFee royalty fee percentage.
     */
    function initialize(
        address erc721BeaconAddress,
        address erc721BeaconProxyAddress,
        address _adminRoyaltyAccount,
        uint256 _adminRoyaltyPercentage,
        ...
        require(
            ...
            "Factory: Beacon address cant be zero address"
        );
        require(
            erc721BeaconProxyAddress != address(0),
            "Factory: Beacon Proxy address cant be zero address"
        );
        ...
        admin = owner();
        adminRoyaltyAccount = _adminRoyaltyAccount;
        adminRoyaltyPercentage = _adminRoyaltyPercentage;
    }
    ...
    /**
     * @dev Returns created collections of `_address`
     * @param _address user wallet address
     */
    function getUserCreatedCollections(address _address)
        external
        view
        returns (address[] memory)
    {
        return userCreatedCollections[_address];
    }
     ...
     */
    function getCollectionAttributes(address account, address collection)
        external
        view
        returns (string[] memory)
    {
        return collectionAttributes[account][collection];
    }
    ...
    function createERC721Collection(
        ...
        uint256 collectionOwnerRoyaltyPercentage = 100 - adminRoyaltyPercentage;
        payees = [adminRoyaltyAccount, _receiver];
        shares = [adminRoyaltyPercentage, collectionOwnerRoyaltyPercentage];
        SplitterContract paymentSplitter = new SplitterContract(payees,shares,admin);
        ...
        allCollections.push(address(token));
     ...
     * Requirements:
     *
     * - `account` should not be zero address
    ...
    function setCollectionAttributes(
        ...
        require(account != address(0), "Account should not be zero address");
        require(
            collection != address(0),
            "Collection address should not be zero address"
        );
        collectionAttributes[account][collection] = _collectionAttributes;
     ...
     */
    function setRoyaltyFee(uint96 _newFee) external onlyOwner whenNotPaused {
        require(_newFee > 0, "Royalty Fee must be greater than zero.");
        royaltyFee = _newFee;
     ...
     * - Caller must be the owner of this contract
     */
    function setPlatformFee(uint256 _newPlatformFee)
        external
        onlyOwner
        whenNotPaused
    {
    ...
    function setCollectionIsDestroyed(address collection)
        external
        whenNotPaused
    {
        require(
            msg.sender == collection,
            "Error : caller has no access to destroy collection."
        );
        isCollectionDestroyed[collection] = true;
        emit CollectionDestroyed(collection);
    }

    function isAccountWhitelisted(address account)
        external
        view
        returns (bool)
    {
    ...
    */
     ...
     */
    function updateBeaconProxy(address erc721BeaconProxyAddress)
        external
        onlyOwner
    {
        require(
    ...
    function updateAdminRoyaltyAccount(address account) external onlyOwner {
        require(account != address(0), "Factory: Cant be zero address");
        adminRoyaltyAccount = account;
        emit AdminRoyaltyAccountUpdated(adminRoyaltyAccount);
    }

    function updateAdminRoyaltyPercentage(uint256 newPercentage)
        external
        onlyOwner
    {
        adminRoyaltyPercentage = newPercentage;
        emit AdminRoyaltyPercentageUpdated(adminRoyaltyPercentage);
    }
...
import "../Create-collection/NFTCollection.sol";
    ...
    ERC2981Upgradeable
{
    ...
    mapping(uint256 => string[]) public tokenIdAttributes; // maps token id with it's corresponding attributes
    mapping(uint256 => mapping(string => string)) public tokenIdAttributeValues; // maps token id with it's attribute and value
    ...
    /**
     * @dev Returns the attributes of `tokenId`
     * @param tokenId.
     */
    function getTokenAttributes(uint256 tokenId)
        ...
        returns (string[] memory)
    {
        return tokenIdAttributes[tokenId];
    }
     ...
     * Requirements:
     *
     * - The caller account must be whitelisted
    ...
    function updateTokenUri(
        ...
        require(
            NFTCollection(factory).isAccountWhitelisted(msg.sender),
            "Account is not whitelisted."
        );
        require(
            msg.sender == ERC721Upgradeable.ownerOf(tokenId) ||
                msg.sender == creator[tokenId],
            "Caller has been restricted from updating token id."
        );

        tokenIdAttributes[tokenId] = attributes;
        for (uint256 i = 0; i < attributes.length; i++) {
            tokenIdAttributeValues[tokenId][attributes[i]] = attributeValues[i];
        }
     ...
     * @param to - account
     * @param uri - token uri
     * @param attributes - token id's attribute array
     * @param attributeValues - token id's attribute values array
     */
    function safeMint(
        address to,
        string memory uri,
    ...
    function destroyCollection() public onlyAdmin {
        NFTCollection(factory).setCollectionIsDestroyed(address(this));
