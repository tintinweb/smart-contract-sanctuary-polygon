...
contract TheBusiness is Ownable {
    ...
    function getIDs(address _owner, uint256[] memory randomNumbers) public view returns(uint256[] memory) {
        uint256[] memory ids = new uint256[](randomNumbers.length);
        for(uint256 i = 0; i < randomNumbers.length; i++){
            ids[i] = getID(_owner, randomNumbers[i]);
        }
        return ids;
    }
