...
ContextMixin
{
    ...
    event CreateSale(address indexed nft, uint256 indexed tokenId, bytes32 saleId, uint128 price, uint256 endDate);
    ...
    function createSale(
        ...
        IERC721(_nft).safeTransferFrom(msgSender(), address(this), _tokenId);
    ...
    function executeSale(address _nft, uint256 _tokenId) external {
        ...
        if(royaltyIsSupported){
            // Get Royalty Info
            (address receiver, uint256 royaltyAmount) = IERC2981(_nft).royaltyInfo(_tokenId, sale.price);
        ...
        emit ExecuteSale(_nft, _tokenId, saleId, sale.seller, sale.maxBidUser, sale.price);
    }
     ...
     */
    function cancelSale(bytes32 _saleId) public {
        ...
        emit CancelSale(sale.nft, sale.tokenId, _saleId);
    }
