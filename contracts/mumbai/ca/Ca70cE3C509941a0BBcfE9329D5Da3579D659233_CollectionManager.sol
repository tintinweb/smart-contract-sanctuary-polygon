...
import {UnexpectedCaller} from "./error/Errors.sol";
import {ICollectionManager} from "./interfaces/ICollectionManager.sol";
import {CollectionV1} from "./CollectionV1.sol";
import {CollectionSingletonV1} from "./CollectionSingletonV1.sol";
...
contract CollectionManager is ICollectionManager, Ownable, Base {
    /// @notice Creator role for creating Collections
    /// @dev Initially the Rubix Admin wallet
    address public creator;

    /// @notice Address of the Collection implementation
    CollectionSingletonV1 public collectionImpl;
    ...
    constructor(address _creator, address _collectionSingleton)
        checkNonZeroAddress(_creator)
        checkNonZeroAddress(_collectionSingleton)
    {
        creator = _creator;
        collectionImpl = CollectionSingletonV1(_collectionSingleton);
    }

    /// @notice Creates a new Collection, creating a completely new deployment
    function createCollection()
        external
        onlyExpectedCaller(creator)
        returns (CollectionV1 newCollection)
    {
        newCollection = new CollectionV1(msg.sender);

        collectionCreationData[address(newCollection)] = CreationData(
    ...
    /// @notice Created a new Collection, deploying a clone proxy from an existing singleton implementation
    function createCollectionClone()
        external
        onlyExpectedCaller(creator)
        returns (CollectionSingletonV1 clone)
    {
        clone = CollectionSingletonV1(Clones.clone(address(collectionImpl)));

        collectionCreationData[address(clone)] = CreationData(
            address(collectionImpl)
        );

        clone.initialize(msg.sender);

        emit CollectionCreated(address(clone));
    }
...
abstract contract Base {
    ...
    modifier onlyExpectedCaller(address expected) {
        if (msg.sender != expected)
            revert UnexpectedCaller(msg.sender, expected);
        _;
    }
}
...
/// @notice Emitted when the contract has already been initialized
...
/// @title CollectionV1
/// @notice This is simple collection contract, meant to be used by the CollectionManager to deploy new from scratch.
contract CollectionV1 is ERC721, Base {
    ...
    event UpdatedMinter(address indexed newMinter);

    constructor(address _minter) ERC721("NFTEngage Collection", "NFTEC") {
...
import {Base} from "./abstract/Base.sol";
import {AlreadyInitialized} from "./error/Errors.sol";
import {ERC721Upgradeable} from "@openzeppelin/contracts-upgradeable/token/ERC721/ERC721Upgradeable.sol";
import {Initializable} from "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";

/// @title CollectionSingletonV1
/// @notice This contract is a singleton to be used by the manager to deploy clone contracts.
contract CollectionSingletonV1 is Initializable, ERC721Upgradeable, Base {
    address public owner;
    ...
    function initialize(address _minter) external initializer {
        __ERC721_init("NFTEngage Collection", "NFTEC");
        _setOwner(msg.sender);
        _setMinter(_minter);
    }
