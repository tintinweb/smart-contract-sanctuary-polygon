...
contract PrivatePlacement is OwnerPausable {
    ...
    bytes32 private _buyMerkleRoot;
    ...
    function setBuyMerkleRoot(bytes32 merkleRoot) external onlyOperator {
        _buyMerkleRoot = merkleRoot;
    }

    function buy(uint256 index, bytes32[] calldata merkleProof) external whenNotPaused {
        bytes32 node = keccak256(abi.encodePacked(msg.sender));
        bool canBuy = MerkleProof.verify(merkleProof, _buyMerkleRoot, node);
        require(canBuy, "not in white list");
        buy(index);
    }
    ...
    function buy(uint256 index) public whenNotPaused {
        ...
        //IERC20(_usdtAddress).safeTransfer(address(_stockMargin), requireUsdt/2);
...
contract StockERC1155 is ERC1155Supply, OwnerPausable {
    ...
    bytes32 private _transferMerkleRoot;
    ...
    function _beforeTokenTransfer(
        ...
        if (from != address(0)) {//除mint外 只有白名单可进行transfer
            if (address(_transferProxy) == address(0)) {
                bytes32[] memory merkleProof = abi.decode(data, (bytes32[]));
                bytes32 node = keccak256(abi.encodePacked(msg.sender));
                bool canTransfer = MerkleProof.verify(merkleProof, _transferMerkleRoot, node);
                require(canTransfer, "transfer not supported");
            } else {
    ...
    function setMerkleRoot(bytes32 merkleRoot) external onlyOperator {
        _transferMerkleRoot = merkleRoot;
    }
...
contract StockMargin is OwnerPausable {
    ...
    function addStockMargin(address user, uint256 margin) external {
        require(msg.sender == _privatePlacementAddress, "not PrivatePlacement");
    ...
    function withdrawMargin() external whenNotPaused {
        require(_launchResult == 2, "launch result not failed");
