...
contract AkxPresale is  Initializable,PausableUpgradeable, Presale, AccessControlEnumerableUpgradeable {
	...
	function setPresaleToken(address _token) public onlyRole(PRESALE_OPERATOR) {
	...
	function setVestingWallets(address[] memory _vw, uint256[] memory _amounts, uint256 duration) public onlyRole(PRESALE_OPERATOR) {
		vestingWallets = _vw;
	...
	function setRefundableEscrow(address _escrowImplementation) public onlyRole(PRESALE_OPERATOR) {
		_holdersEscrows[msg.sender] = _escrowImplementation;
	}

	function setPriceForOneToken(uint256 price) public onlyRole(PRESALE_OPERATOR) {
		require(init == false, "already done");

	}

	function setQtyPerETH(uint256 qty) public onlyRole(PRESALE_OPERATOR) {
		require(init == false, "already done");
		//qtyPerEth = qty;
	}

	function setPhase1(uint startTime, uint duration, uint price, uint _qtyPerEth, uint supply) public onlyRole(PRESALE_OPERATOR) {
		require(init == false, "already done");
	...
	function startPhase1() public onlyRole(PRESALE_OPERATOR) {
	...
	function endPhase1() public onlyRole(PRESALE_OPERATOR){
	...
	function setPhase2(uint startTime, uint duration, uint price, uint _qtyPerEth, uint supply) public onlyRole(PRESALE_OPERATOR) {
	...
	function startPhase2() public onlyRole(PRESALE_OPERATOR) {
		require(presalePhase == 0, "already started");
		presalePhase = 2;
		unpauseContract();
		emit PhaseStarted(1, block.number);
