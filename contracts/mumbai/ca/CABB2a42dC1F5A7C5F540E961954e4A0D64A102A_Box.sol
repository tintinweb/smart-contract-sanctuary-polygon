    ...
    ReentrancyGuard
{
    ...
    function _mintToQuantity(address to, uint256 quantity) private {
        PhaseConfig storage phaseConfig = phaseConfigs[activePhase];

        require(
            phaseConfigs[activePhase].addressesMintQtyAllowed[msg.sender] > 0,
            "You are not allowed to mint in this phase"
        );

        require(
            phaseConfigs[activePhase].addressesMintQtyAllowed[msg.sender] >=
                phaseConfigs[activePhase].addressesMintedCount[msg.sender] +
                    quantity,
            "You are trying to mint too many"
        );

        require(
            phaseConfig.phaseStartTime <= block.timestamp &&
                phaseConfig.phaseEndTime >= block.timestamp,
            "Minting is not available at this time"
        );

        require(
            phaseConfig.maxPerMintTx >= quantity,
    ...
    /**
     * @notice adds a new phase to the contract to a new value
     */
    function addPhase(
        string calldata phaseName,
        address[] calldata addresses,
        ...
        phaseConfig.maxPerMintTx = maxPerMintTx;
    ...
    /**
     * @notice Sets the metadata filetype to a new value - for testing with OS previews
     * @param _fileType the new filetype extension to use with metadata
     */
    function setMetadataFileType(string calldata _fileType) external onlyOwner {
        fileType = _fileType;
    }
    ...
    function tokenURI(
        ...
        bytes memory dataURI = abi.encodePacked(
            "{",
            '"name": "Mysterious Box",',
            '"description": "A box, nothing else",',
            '"image": "https://cryptodadsnft.nyc3.cdn.digitaloceanspaces.com/box/box.',
            fileType,
            '",',
            '"animation_url": "https://cryptodadsnft.nyc3.cdn.digitaloceanspaces.com/box/box.',
            fileType,
            '",',
            '"attributes": [{"trait_type": "A Box?", "value": "Yep"}]',
            "}"
        );
        return
            string(
