...
contract Layer is AccessControl, ERC721Enumerable, DateUtil,ReentrancyGuard{
    ...
    mapping(string => IncomsDetail) public Incoms;
    ...
    function transfer(uint amount) external nonReentrant() returns (bool result)  {
       ...
       if(!Incoms[date].isUsed){
            IncomsDetail storage incomsDetail= Incoms[date];
            incomsDetail.amount = 0;
            incomsDetail.numTimes = 0;
            incomsDetail.isUsed = true;
        }       
       IncomsDetail storage iDetail= Incoms[date];
    ...
    function dividend() external {
        ...
        uint amount = Incoms[date].amount;
        uint in_class1;
        uint in_class2;
        uint in_class3;
        ...
        // for (uint i=0;i<class1[date].length;i++){
        //      _devidendSend(class1[date][i],in_class1);
        // }
        // for (uint i=0;i<class2[date].length;i++){
        //     _devidendSend(class1[date][i],in_class2);
        // }
        // for (uint i=0;i<class3[date].length;i++){
        //     _devidendSend(class1[date][i],in_class3);
        // }
    ...
    function dividendOf(address userAddress) public view returns(uint){
        uint lastDealtime = dealTimeList[userAddress];
        if (lastDealtime == 0) {
            lastDealtime = startDate;
        }
        //ת��0��
        lastDealtime = getday0(lastDealtime);
        ...
        //�����Ƿ��Ѿ��ֺ�
        // if(dividendByDayList[date].class1 == 0 && dividendByDayList[date].class2 == 0 ){
        //     today = today - 1 days;
        // }
        for (;lastDealtime <= today - 1 days ;lastDealtime + 1 days) {
    ...
    function buyRB(uint Rbnum) external{
        ...
        string memory date = DateUtil.daysToDateString(int(block.timestamp),8);   
        require(classList[date][msg.sender] != 0,"(buyRb) user error");
        uint total = 1000;
        if(classList[date][msg.sender] == 2){
            total = 5000;
    ...
    // function _checkExist(address[] addList,address add)private returns (bool){
    //     string memory date = DateUtil.daysToDateString(int(block.timestamp - 1 days),8);
    //     for (uint i=0;i<class1[date][i].length;i++){
    //          if(class1[date][i] == add){
    //             return true;
    //          }
    //     }
    //     for (uint i=0;i<class2[date][i].length;i++){
    //          if(class2[date][i] == add){
    //             return true;
    //          }
    //     }
    //     for (uint i=0;i<class3[date][i].length;i++){
    //          if(class3[date][i] == add){
    //             return true;
    //          }
    //     }
    //     return false;
