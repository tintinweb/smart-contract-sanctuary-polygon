...
contract Vault {
    ...
    constructor(MarketTradingControl _tradingControl, 
    ...
    function openMarketPriceOrder(
                ...
                int _collateral, 
                int _takeProfit, 
                int _stopLoss) public returns(uint) {

        DAI.transferFrom(msg.sender, address(this), uint(_collateral));

        uint tradeId = marketTradingControl.openMarketOrder(
                         msg.sender, 
                         _pair, 
                         orderType, 
                         _leverageAmount, 
    ...
    function closeOrder(uint _tradeId) public returns(int) {
        (int profit, uint closeTradeIndex) = marketTradingControl.closeMarketOrder(_tradeId, msg.sender);

        int collateral = tradingStorage.getClosedTradeDetails(closeTradeIndex, msg.sender).collateral;

        int pnl = collateral + profit;
        if (profit > 0) {
            DAI.transfer(msg.sender, uint(pnl));
        } else if (profit < 0 && pnl > 0) {
            DAI.transfer(msg.sender, uint(pnl));
        } 
...
contract BancorFormula is IBancorFormula, Power {
     ...
     * .      = 1 * ((1 + 1,700,000/ 10)^ (0.5)) . = 412.31
              = 1 * ((1 + 1 / 10)^ 0.5)
           1   = 1 * ((1 + 1700000/x)^0.5 ) - 1 
...
contract TradingStorage {
    ...
    mapping(address => closedTrade[]) public usersAllClosedTrades;
    ...
    {
        return usersAllClosedTrades[_trader];
    }
    ...
    function updateTrade(
        ...
        usersAllClosedTrades[_trader].push(_closedTrade);
...
contract MarketTradingControl {
    ...
    function closeMarketOrder(uint _tradeId, address _trader) public returns(int, uint) {   // need to do checking for the owner of the trade
        int profit;
        ...
        uint closeTradeIndex = tradingStorage.updateTrade(_tradeId, _trader, closePrice, profit);


        return (profit, closeTradeIndex);
    }
}
