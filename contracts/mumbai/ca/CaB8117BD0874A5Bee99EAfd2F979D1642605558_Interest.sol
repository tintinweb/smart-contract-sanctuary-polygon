...
import "@thirdweb-dev/contracts/drop/DropERC1155.sol"; // For Memberships
import "@thirdweb-dev/contracts/token/TokenERC20.sol"; // For Money
import "@thirdweb-dev/contracts/openzeppelin-presets/utils/ERC1155/ERC1155Holder.sol"; // for Trust

// OpenZeppelin (ReentrancyGuard)
import "@openzeppelin/contracts/security/ReentrancyGuard.sol";

contract Interest is ReentrancyGuard, ERC1155Holder {
    // Store our two other contracts here (Edition Drop and Token)
    DropERC1155 public immutable Membership;
    TokenERC20 public immutable Money;

    // Constructor function to set the Money and the Membership
    constructor(DropERC1155 MembershipContractAddress, TokenERC20 MoneyContractAddress) {
        Membership = MembershipContractAddress;
        Money = MoneyContractAddress;
    }
    ...
    // Mapping of Member addresses to their current Money
    // By default, Member has no Money. They will not be in the mapping.
    // Mapping of address to Money is not set until they stake the Money.
    // In this example, the tokenId of the Money is the multiplier for the Interest reward.
    mapping (address => MoneyValue) public Member;

    // Mapping of Member address until last time they staked/withdrew/claimed their Interest rewards
    // By default, Member has no last time. They will not be in the mapping.
    mapping (address => MoneyValue) public MemberLastUpdate;

    function stake(uint256 _tokenId) external nonReentrant {
        // Ensure the Member has at least 1 of the Money they are trying to stake
        require(Membership.balanceOf(msg.sender, _tokenId) >= 1, "You must have at least 1 Money to stake");

        // If they have a Membership already, send it back to them.
        if (Member[msg.sender].isData) {
            // Transfer using safeTransfer
            Membership.safeTransferFrom(address(this), msg.sender, Member[msg.sender].value, 1, "Returning your Money");
        }

        // Calculate the Interest Reward they are owed, and pay them out.
        uint256 reward = calculateRewards(msg.sender);
        Money.transfer(msg.sender, reward);

        // Transfer the Money to the contract
        Membership.safeTransferFrom(msg.sender, address(this), _tokenId, 1, "Staking your Money");

        // Update the Members mapping
        Member[msg.sender].value = _tokenId;
        Member[msg.sender].isData = true;

        // Update the MemberLastUpdate mapping
        MemberLastUpdate[msg.sender].isData = true;
        MemberLastUpdate[msg.sender].value = block.timestamp;
    }


    function withdraw() external nonReentrant {
        // Ensure the Member has Money
        require(Member[msg.sender].isData, "You do not have Money to withdraw.");

        // Calculate the Interest rewards they are owed, and pay them out.
        uint256 reward = calculateRewards(msg.sender);
        Money.transfer(msg.sender, reward);

        // Send the Money back to the Member
        Membership.safeTransferFrom(address(this), msg.sender, Member[msg.sender].value, 1, "Returning your Money");

        // Update the Member mapping
        Member[msg.sender].isData = false;
    ...
    // Calculate the Interest rewards the Member is owed since last time they were paid out
    // The Interest rewards rate is 0.0001 per block.
    // This is calculated using block.timestamp and the MemberLastUpdate.
    // If MemberLastUpdate or Money is not set, then the Member has no Interest rewards.
    function calculateRewards(address _player)
        public
        view
        returns (uint256 _rewards)
    {
        // If MemberLastUpdate or Money is not set, then the Member has no Intrest rewards.
        if (!MemberLastUpdate[_player].isData || !Member[_player].isData) {
            return 0;
        }

        // Calculate the time difference between now and the last time they staked/withdrew/claimed their Interest rewards
        uint256 timeDifference = block.timestamp - MemberLastUpdate[_player].value;

        // Calculate the rewards they are owed
        uint256 rewards = timeDifference * 1 * (Member[_player].value + 1);

        // Return the Interest rewards
        return rewards;
