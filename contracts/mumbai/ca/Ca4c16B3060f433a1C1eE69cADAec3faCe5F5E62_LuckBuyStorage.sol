...
contract LuckBuyStorage {
  ...
  function rewardItem(uint256 itemID,uint256 winnerIndex, address platform) public onlyLogicContract returns (bool, address, uint256) {
    require(items[itemID].itemID != 0, "item not exist"); 
    require(items[itemID].startTime <= block.timestamp, "item not start"); 
    ...
    uint256 bonus = items[itemID].bonusAmount;
