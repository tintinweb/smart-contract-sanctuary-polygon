...
contract Flipcoin {
    ...
    function bet(uint256 betAmount) external gasLimit(200000) {
        ...
        uint256 totalAmount = betAmount * (10**_tokenDecimals) + gasFee;
        ...
        if (isWin) {
            require(_amberToken.transfer(msg.sender, winnings * (10**_tokenDecimals)), "Failed to transfer winnings");
        } else {
            _balances[msg.sender] += betAmount * (10**_tokenDecimals);
        }
    ...
    function depositFunds(uint256 amount) external gasLimit(100000) {
        ...
        require(_amberToken.transferFrom(msg.sender, address(this), amount * (10**_tokenDecimals)), "Failed to transfer AMBER tokens");
        require(_amberToken.approve(address(this), amount * (10**_tokenDecimals)), "Failed to approve AMBER token transfer");
