...
function getAudioChatById(bytes32 audio_event_id) public view returns ( bytes32,
        uint256 created_at,
        uint256 start_at,
        string memory cid_metadata,
        stateOptions state,
    ...
    CreateAudioChat memory audio_chat = id_to_audio_chat[audio_event_id];
...
function getAllRecordingsByWalletAddress(address creator) public view returns (CreateAudioChat[] memory){
    require(creator == msg.sender, "Not the owner of this chat");

    uint256 array_len;
    for (uint256 i; address_to_audio_chat[creator].length > i; i++){
        if (keccak256(abi.encodePacked(id_to_audio_chat[address_to_audio_chat[creator][i]].recording_arweave_transaction_id)) != ""){
            array_len += 1;
        }
    }
...
function updateAudioChat(bytes32 audio_event_id, 
    ...
    emit handleAudioChatUpdated(
        audio_event_id,
        id_to_audio_chat[audio_event_id].start_at,
        id_to_audio_chat[audio_event_id].created_at,
        id_to_audio_chat[audio_event_id].cid_metadata,
        id_to_audio_chat[audio_event_id].state,
        id_to_audio_chat[audio_event_id].is_indexed,
        id_to_audio_chat[audio_event_id].recording_arweave_transaction_id
    );
