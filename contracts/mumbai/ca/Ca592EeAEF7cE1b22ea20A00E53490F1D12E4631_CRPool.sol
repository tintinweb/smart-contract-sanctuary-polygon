...
contract CRPool is ICRPool, PCToken, OwnableUpgradeable, BalancerReentrancyGuard {
    ...
    event NewTokenCommitted(address indexed token, uint256 weight, address indexed pool, address indexed caller);

    event NewTokenApplied(address indexed token, uint256 weight, address indexed pool, address indexed caller);

    event StartedGradualWeightsUpdate(
        uint256[] newWeights,
        uint256 startBlock,
        uint256 endBlock,
    ...
    event FinishedGradualWeightsUpdate(
        uint256[] endWeights,
        uint256 startBlock,
        uint256 endBlock,
        address indexed pool,
    ...
    function updateWeightsGradually(
        ...
        emit StartedGradualWeightsUpdate(newWeights, startBlock, endBlock, address(this), msg.sender);
    ...
    function pokeWeights() external virtual logs nonReentrant needsBPool {
        ...
        // Delegate to library to save space
        bool isFinished = SmartPoolManager.pokeWeights(bPool, gradualUpdate);

        if (isFinished) {
            emit FinishedGradualWeightsUpdate(
    ...
    function commitAddToken(
        ...
        emit NewTokenCommitted(token, denormalizedWeight, address(this), msg.sender);
    ...
    function applyAddToken() external virtual override logs nonReentrant needsBPool {
        ...
        emit NewTokenApplied(_newToken.addr, _newToken.denorm, address(this), msg.sender);
