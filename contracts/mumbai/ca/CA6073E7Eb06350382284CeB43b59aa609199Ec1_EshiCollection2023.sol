...
contract EshiCollection2023 is BaseNFT {
    using Counters for Counters.Counter;
    using Strings for uint256;

    Counters.Counter private _tokenIDCounter;

    mapping(uint256 => uint256) private _typeMaxSupplies;
    mapping(uint256 => mapping(address => bool)) private _isAirdroppeds;

    constructor(uint256 royalMembershipMaxSupply, uint256 membershipMaxSupply)
        BaseNFT("Eshi Collection 2023", "EC2023")
    {
        for (uint256 i = 1; i <= 4; i++) {
            _typeMaxSupplies[i] = royalMembershipMaxSupply;
        }
        for (uint256 j = 5; j <= 8; j++) {
            _typeMaxSupplies[j] = membershipMaxSupply;
        }
    }

    function typeMaxSupply(uint256 tokenType_) external view returns (uint256) {
        _requireValidTokenType(tokenType_);

        return _typeMaxSupplies[tokenType_];
    }
        ...
        whenNotPaused
    {
        ...
        require(
            _typeSupplies[tokenType_] < _typeMaxSupplies[tokenType_],
            "EC2023: supply has reached the upper limit"
        );
        require(!_isAirdroppeds[tokenType_][to_], "EC2023: already airdropped");
...
contract BaseNFT is ERC721Enumerable, ERC2981, IERC4907, Ownable, Pausable {
    using Strings for uint256;
    ...
    mapping(uint256 => bool) internal _isTokenURIFrozens;

    mapping(uint256 => uint256) internal _tokenTypes;
    mapping(uint256 => uint256) internal _typeSupplies;
    mapping(address => mapping(uint256 => uint256)) internal _typeBalances;

    mapping(uint256 => uint256) internal _holdingStarteds;
    mapping(uint256 => address) internal _firstOwners;
    ...
    function freezeTokenURI(uint256 tokenID_, string calldata uri_)
        external
        onlyOwner
    {
        _requireMinted(tokenID_);
