...
contract Marketplace is Initializable, ReentrancyGuardUpgradeable, PausableUpgradeable {
     ...
     */
    function updatePlatformFeeRecipient(address payable _platformFeeRecipient) external onlyAdmin {
    ...
    function _cancelListing(
        ...
        if (IERC165Upgradeable(_nftAddress).supportsInterface(INTERFACE_ID_ERC1155)) {
            ...
            if (IERC721Upgradeable(_nftAddress).balanceOf(address(this)) > 0) {
                ...
                IERC721Upgradeable(_nftAddress).burn(_tokenId);
            }
        }
        //(IAddressRegistry(addressRegistry).mintRequest(requestCreatedFromVoucher(string.concat(voucher.tokenUri, StringsUpgradeable.toHexString(voucher.recipient)))))
