...
contract Faucet {
    ...
    // tracks requestor's next request time
    ...
    function withdrawBalance(address to) external onlyOwner {
        token.safeTransfer(to, getBalance());
    }
    ...
    function requestTokens() external {
        ...
        require(timestamp > lockTime[requestor], "Request limit exceeded");
        require(getBalance() >= amountAllowed, "Not enough funds in the faucet");

        token.safeTransfer(requestor, amountAllowed);

        lockTime[requestor] = timestamp + period;
    }
