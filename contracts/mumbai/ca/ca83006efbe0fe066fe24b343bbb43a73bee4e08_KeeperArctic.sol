...
contract KeeperArctic is AccessControlledAndUpgradeable {
  ...
  function initialize(address _admin, address _longShort) external virtual initializer {
    longShort = IRegistry(_longShort);

    _AccessControlledAndUpgradeable_init(_admin);
  }

  function setLongShort(address _longShort) external onlyRole(ADMIN_ROLE) {
    longShort = IRegistry(_longShort);
  }

  // Functian that the keeper calls
  function updateSystemStateForMarket(IMarketTieredLeverage market) external {
  ...
  function shouldUpdateMarketCore()
    public
    view
    returns (bool shouldUpdate, IMarketTieredLeverage market)
  {
    uint256 latestMarket = longShort.latestMarket();
    for (uint32 index = 1; index <= latestMarket; index++) {
      market = IMarketTieredLeverage(longShort.separateMarketContracts(index));
      uint32 latestExecutedEpochIndex = market.epochInfo().latestExecutedEpochIndex;

      IOracleManagerFixedEpoch oracleManager = market.oracleManager();

      if (latestExecutedEpochIndex < oracleManager.getCurrentEpochIndex()) {
        uint32 epochTimestamp = uint32(oracleManager.getEpochStartTimestamp());
        shouldUpdate = oracleManager.shouldUpdatePrice(epochTimestamp);

        if (shouldUpdate) {
          return (shouldUpdate, market);
        }
      }
    }
  }
  ...
  function shouldUpdateMarketCallable() external returns (bool canExec, bytes memory execPayload) {
    (bool shouldUpdate, IMarketTieredLeverage market) = shouldUpdateMarketCore();
    if (shouldUpdate) {
      return (true, abi.encodeCall(this.updateSystemStateForMarket, market));
    }
    longShort = longShort; //prevents warning about view function
    return (false, "");
  }
  ...
  // Functian that the keeper calls
  function updateSystemStateForMarketTest(IMarketTieredLeverage market) external {
  ...
  function shouldUpdateMarketTest() external view returns (bool canExec, bytes memory execPayload) {
    ...
    if (shouldUpdate) {
      return (true, abi.encodeCall(this.updateSystemStateForMarketTest, market));
    }
