...
contract LandStaking is Ownable, ReentrancyGuard {
    ...
    // staking plan => data for staking
    mapping(uint8 => StakeData) public stakingData;
    // staker address => land id => already stake or not
    mapping(address => mapping(uint256 => bool)) public isStaked;
    // land id => staking plan
    mapping(uint256 => uint8) private landStakingPlan;
    ...
    event StakingPlanUpdated(uint8 indexed stakingPlan, uint8[] types, uint256[] amounts);
     ...
     **/
    function stake(uint256 id, uint8 stakingPlan) external nonReentrant {
        require(_getBlockTime() >= stakingData[stakingPlan].stakingStartTime, 'not now');
        rewardLocked += _stakeLand(_msgSender(), id, stakingPlan);
    }
     ...
     **/
    function stakeBatch(uint256[] memory ids, uint8[] memory stakingPlans) external nonReentrant {
        require(ids.length == stakingPlans.length, 'invalid length');
        address sender = _msgSender();
        uint256 rewardXPAmount;
        for (uint256 i = 0; i < ids.length; i++) {
            require(_getBlockTime() >= stakingData[stakingPlans[i]].stakingStartTime, 'not now');
            rewardXPAmount += _stakeLand(sender, ids[i], stakingPlans[i]);
        }
    ...
    function claim(uint256 id) external nonReentrant {
        address sender = _msgSender();
        require(isStaked[sender][id], 'invalid claim info');
        uint8 stakingPlan = landStakingPlan[id];
        require(_getBlockTime() >= stakingData[stakingPlan].stakingEndTime, 'not now');
        landContract.transferFrom(address(this), sender, id);
        uint256 reward = getRewardAmount(typeOfLand[id], stakingPlan);
        xpToken.transfer(sender, reward);
        rewardLocked -= reward;
        emit Claimed(id, reward);
    }
    ...
    function unstake(uint256 id) external nonReentrant notPaused {
        address sender = _msgSender();
        require(isStaked[sender][id], 'invalid unstake info');
        uint8 stakingPlan = landStakingPlan[id];
        require(_getBlockTime() < stakingData[stakingPlan].stakingEndTime, 'not now');
        landContract.transferFrom(address(this), sender, id);
        uint256 reward = getRewardAmount(typeOfLand[id], stakingPlan);
    ...
    function claimBatch(uint256[] memory ids) external nonReentrant {
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            require(isStaked[sender][ids[i]], 'invalid claim info');
            uint8 stakingPlan = landStakingPlan[ids[i]];
            require(_getBlockTime() >= stakingData[stakingPlan].stakingEndTime, 'not now');
            landContract.transferFrom(address(this), sender, ids[i]);
            uint256 reward = getRewardAmount(typeOfLand[ids[i]], stakingPlan);
            totalRewardTransfer += reward;
            emit Claimed(ids[i], reward);
        }
    ...
    function unstakeBatch(uint256[] memory ids) external nonReentrant notPaused {
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            require(isStaked[sender][ids[i]], 'invalid unstake info');
            uint8 stakingPlan = landStakingPlan[ids[i]];
            require(_getBlockTime() < stakingData[stakingPlan].stakingEndTime, 'not now');
            landContract.transferFrom(address(this), sender, ids[i]);
            uint256 reward = getRewardAmount(typeOfLand[ids[i]], stakingPlan);
    ...
    function setStakingPlan(
        ...
        for (uint256 i = 0; i < types.length; i++) {
            ...
            stakingData[stakingPlan].rewardAmount[types[i]] = amounts[i];
        }
        stakingData[stakingPlan].stakingStartTime = startTime;
        stakingData[stakingPlan].stakingEndTime = endTime;
        emit StakingPlanUpdated(stakingPlan, types, amounts);
    }
    ...
    function _stakeLand(
        ...
        require(!isStaked[sender][landId], 'already stake');
        ...
        isStaked[sender][landId] = true;
        landStakingPlan[landId] = stakingPlan;
        emit Staked(landId, landType, stakingPlan, rewardXPAmount);
    ...
    {
        amount = stakingData[stakingPlan].rewardAmount[landType];
    }
