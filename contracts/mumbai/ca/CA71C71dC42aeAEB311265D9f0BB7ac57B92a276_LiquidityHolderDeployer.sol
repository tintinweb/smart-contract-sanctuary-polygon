...
contract BetLiquidityHolder is IBetLiquidityHolder, ReentrancyGuard {
    ...
    mapping(address => bool) internal userWithdrown;
    ...
    function receiveLiquidityTaker(
        ...
        require(
            ...
            "Holder:Caller Is Not Instance Deployer"
        );
        totalAvailableLiquidity += tokenLiquidity_;
        betTaker = betTaker_;
        ...
        returns (bool)
    {
        ...
        address treasury_;
        (, , , treasury_) = IConfig(registry_).getWalletAddress();
        if (betTrendSetter != address(0)) {
            uint256 _trasuryValue = IFeeAgreegator(agreegatorAddress_)
    ...
    function processFinalLiquidityDistribution(
        ...
        address burnAddress;
        address poolAddress;
        (, burnAddress, poolAddress, ) = IConfig(registry_).getWalletAddress();
        if (betTrendSetter != address(0)) {
            (
    ...
    function payReward(
        ...
        payable(treasury_).transfer(treasuryValue_);
    ...
    function withdrawLiquidity(address user_) external payable override {
        ...
        require(
            user_ == betCreator || user_ == betTaker,
            "User must be bet creator or bet taker"
        );
        require(
            !userWithdrown[user_],
            "User allready have withdrawn liquidity"
        );
        require(betCreator != address(0), "Invalid Bet");
        require(betTaker == address(0), "Bet Is Ongoing");
        userWithdrown[user_] = true;
        if (tokenAddress == address(0))
    ...
    function processAaveDistributionForDraw(
        ...
        if (tokenAddress == address(0)) {
            if (address(this).balance > totalAvailableLiquidity) {
                        ...
                        totalAvailableLiquidity;
                    payable(treasury_).transfer(_adminAmount);
            ...
            if (_currentBal > totalAvailableLiquidity) {
                        ...
                        totalAvailableLiquidity;
                    IERC20(tokenAddress).transfer(treasury_, _adminAmount);
                    IERC20(tokenAddress).transfer(
                        betCreator,
                        userLiquidity[betCreator]
                    );
        ...
        returns (bool)
    {
        ...
        address developer_;
        (developer_, , , ) = IConfig(registry_).getWalletAddress();
        if (tokenAddress == address(0)) {
            payable(developer_).transfer(address(this).balance);
        } else {
            uint256 _bal = IERC20(tokenAddress).balanceOf(address(this));
            IERC20(tokenAddress).transfer(developer_, _bal);
        }
