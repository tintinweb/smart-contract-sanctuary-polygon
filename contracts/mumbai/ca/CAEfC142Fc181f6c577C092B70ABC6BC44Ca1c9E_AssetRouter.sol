...
contract AssetRouter is IAssetRouter, AccessControl, ReentrancyGuard {
    ...
    function swapRemote(
        SwapMessage memory receivedMessage
    ) external nonReentrant onlyRole(BRIDGE_ROLE) returns (SwapMessage memory) {
        ...
        IAssetV2(pool.poolAddress).release(receivedMessage.receiver, receivedMessage.amount);
