    ...
    ReentrancyGuardUpgradeable
{
     ...
     * @dev Pause Market
     */
    function pause() external onlyOwner onlyRole(AmberfiLib.MARKET_OWNER) {
     ...
     * @dev Unpause Market
     */
    function unpause() external onlyOwner onlyRole(AmberfiLib.MARKET_OWNER) {
        uint256 marketId = getMarketId();
    ...
    function addPaymentToken(
        ...
        bool native_
    ) external onlyOwner onlyRole(AmberfiLib.MARKET_OWNER) {
        ...
        for (uint256 i; i < length; i++) {
            if (
                ...
                AmberfiLib.PaymentToken
                    storage existingPaymentToken = _paymentTokens[i];
                {
                    // In case they had to update existing PaymentToken detail
                    existingPaymentToken.token = token_;
                    existingPaymentToken.tokenDecimals = tokenDecimals;
                    existingPaymentToken.aggregator = aggregator_;
                    existingPaymentToken
                        .aggregatorDecimals = aggregatorDecimals_;
                    existingPaymentToken.native = native_;
                }
                existingPaymentToken.enabled = true;
        ...
        AmberfiLib.PaymentToken memory newPaymentToken = AmberfiLib
            .PaymentToken(
                token_,
                tokenDecimals,
                aggregator_,
        ...
        _paymentTokens.push(newPaymentToken);
    ...
    function removePaymentToken(uint256 index_)
        external
        onlyOwner
        onlyRole(AmberfiLib.MARKET_OWNER)
    {
        ...
        AmberfiLib.Auction[] memory activeAuctions = getActiveAuctions(index_);
     ...
     * @param startPrice_ (uint256) Start price
     * @param reservedPrice_ (uint256) Reserved price
     * @param minIncrementAmount_ (uint256) Minimum price increment amount
     ...
     * @param instantSale_ (bool) Boolean for instant sale
     */
    function createAuction(
        ...
        if (startTime_ < block.timestamp) {
            revert AmberfiLib.Market_AuctionCreateFailed(
                uint8(AmberfiLib.AuctionCreateErrorType.INVALID_START_TIME)
            );
        }

        if (endTime_ <= startTime_) {
            revert AmberfiLib.Market_AuctionCreateFailed(
                uint8(AmberfiLib.AuctionCreateErrorType.INVALID_END_TIME)
            );
        }
        ...
        if (minIncrementAmount_ == 0) {
            revert AmberfiLib.Market_AuctionCreateFailed(
                uint8(AmberfiLib.AuctionCreateErrorType.INVALID_MIN_INCREMENT)
            );
        }

        if (paymentTokenIndex_ >= _paymentTokens.length) {
            revert AmberfiLib.Market_AuctionCreateFailed(
                uint8(
                    AmberfiLib
     ...
     * @param auctionId_ (uint256) Auction ID
     */
    function buyNow(uint256 auctionId_)
        external
        payable
        nonReentrant
        whenNotPaused
        isValidAuction(auctionId_)
    {
        ...
        if (auction.startPrice != msg.value) {
            revert AmberfiLib.Market_AuctionBuyNowFailed(
        ...
        _bidsPerAuctionId[auctionId_].push(
            AmberfiLib.Bid(_msgSender(), msg.value)
        );

        AmberfiLib.Auction storage auctionToSave = _auctions[auctionId_];
        auctionToSave.highestBidAmount = msg.value; // Set new bid amount
        auctionToSave.highestBidder = _msgSender(); // Set new bidder

        emit AmberfiLib.BoughtNFT(_msgSender(), msg.value);
    }

    /**
     * @dev Place bid to an auction (ERC20 PaymentToken)
     * @param auctionId_ (uint256) Auction ID
     * @param auctionId_ (uint256) Bid amount in PaymentToken
     */
    function buyNow(uint256 auctionId_, uint256 bidAmount_)
        external
        nonReentrant
        whenNotPaused
        isValidAuction(auctionId_)
    {
        AmberfiLib.Auction memory auction = _auctions[auctionId_];
        uint256 paymentTokenIndex = auction.paymentTokenIndex;
        AmberfiLib.AuctionState auctionState = auction.auctionState;
        ...
        if (getRate(auction.startPrice, paymentTokenIndex) != bidAmount_) {
            revert AmberfiLib.Market_AuctionBuyNowFailed(
                uint8(AmberfiLib.AuctionBuyNowErrorType.INVALID_PAYMENT_AMOUNT)
            );
        }
        ...
        _bidsPerAuctionId[auctionId_].push(
            AmberfiLib.Bid(_msgSender(), bidAmount_)
        );

        AmberfiLib.Auction storage auctionToSave = _auctions[auctionId_];
        auctionToSave.highestBidAmount = bidAmount_; // Set new bid amount
        auctionToSave.highestBidder = _msgSender(); // Set new bidder

        emit AmberfiLib.BoughtNFT(_msgSender(), bidAmount_);
    }
        ...
        isValidAuction(auctionId_)
    {
        ...
        uint256 currentBidAmount = highestBidAmount > 0
            ? highestBidAmount
            : auction.startPrice;
        if (msg.value < currentBidAmount + auction.minIncrementAmount) {
            revert AmberfiLib.Market_AuctionBidFailed(
        ...
        uint256 prevCurrentBidAmount = prevHighestBidAmount > 0
            ? prevHighestBidAmount
            : auction.startPrice;
        if (prevCurrentBidAmount + auction.minIncrementAmount > msg.value) {
            revert AmberfiLib.Market_AuctionBidFailed(
                uint8(AmberfiLib.AuctionBidErrorType.INVALID_BID_AMOUNT)
            );
        }
        ...
        emit AmberfiLib.BidPlaced(_msgSender(), msg.value);
    }
        ...
        isValidAuction(auctionId_)
    {
        ...
        uint256 currentBidAmount = highestBidAmount > 0
            ? highestBidAmount
            : auction.startPrice;
        if (bidAmount_ < currentBidAmount + auction.minIncrementAmount) {
            revert AmberfiLib.Market_AuctionBidFailed(
                uint8(AmberfiLib.AuctionBidErrorType.INVALID_BID_AMOUNT)
            );
        }
        ...
        uint256 prevCurrentBidAmount = prevHighestBidAmount > 0
            ? prevHighestBidAmount
            : auction.startPrice;
        if (prevCurrentBidAmount + auction.minIncrementAmount > bidAmount_) {
            revert AmberfiLib.Market_AuctionBidFailed(
                uint8(AmberfiLib.AuctionBidErrorType.INVALID_BID_AMOUNT)
            );
        }
        ...
        if (prevHighestBidAmount > 0 && prevHighestBidder != address(0)) {
            address token = _paymentTokens[paymentTokenIndex].token;
        ...
        emit AmberfiLib.BidPlaced(_msgSender(), bidAmount_);
    }
        ...
        isValidAuction(auctionId_)
    {
        ...
        AmberfiLib.AuctionState auctionState = auction.auctionState;
        AmberfiLib.ClaimState claimState = _claimStatePerAuctionId[auctionId_];
        ...
        if (
            block.timestamp < auction.endTime &&
            auction.reservedPrice > auction.highestBidAmount
        ) {
            revert AmberfiLib.Market_AuctionRedeemNFTFailed(
                uint8(AmberfiLib.AuctionRedeemNFTErrorType.AUCTION_NOT_ENDED)
            );
        }
        if (claimState == AmberfiLib.ClaimState.REDEEMED) {
            revert AmberfiLib.Market_AuctionRedeemNFTFailed(
        ...
        emit AmberfiLib.NFTRedeemed(highestBidder);
    }
        ...
        isValidAuction(auctionId_)
    {
        ...
        if (
            block.timestamp < auction.endTime &&
            auction.reservedPrice > highestBidAmount
        ) {
            revert AmberfiLib.Market_AuctionClaimPaymentFailed(
                uint8(AmberfiLib.AuctionRedeemNFTErrorType.AUCTION_NOT_ENDED)
            );
        ...
        if (
            ...
            (royaltyReceiver, royaltyAmount) = IERC2981Upgradeable(
                auction.nftContract
            ).royaltyInfo(auction.tokenId, highestBidAmount);
        }

        uint256 serviceFee = (highestBidAmount * _serviceFeePercent) / 10000;

        if (_paymentTokens[paymentTokenIndex].native) {
            if (auction.creator == owner()) {
                payable(auction.creator).transfer(
                    highestBidAmount - royaltyAmount
                );
            } else {
                payable(auction.creator).transfer(
                    highestBidAmount - royaltyAmount - serviceFee
                );
            }
        ...
        AmberfiLib.Auction storage auctionToSave = _auctions[auctionId_];
        auctionToSave.auctionState = AmberfiLib.AuctionState.COMPLETED;
        _claimStatePerAuctionId[auctionId_] = AmberfiLib.ClaimState.CLAIMED;
        _serviceFeesPerPaymentTokenId[auction.paymentTokenIndex] += serviceFee;

        emit AmberfiLib.PaymentClaimed(_msgSender(), highestBidAmount);
    }
        ...
        isValidAuction(auctionId_)
    {
        ...
        if (highestBidAmount >= auction.reservedPrice) {
            revert AmberfiLib.Market_AuctionCancelFailed(
                uint8(AmberfiLib.AuctionCancelErrorType.RESERVED_PRICE_REACHED)
            );
        }

        AmberfiLib.Auction storage auctionToSave = _auctions[auctionId_];
        auctionToSave.auctionState = AmberfiLib.AuctionState.CANCELLED;

        IERC721Upgradeable nftContract = IERC721Upgradeable(
            auction.nftContract
        );
        ...
        if (_paymentTokens[paymentTokenIndex].native) {
            ...
            IERC20Upgradeable(_paymentTokens[paymentTokenIndex].token)
                .transferFrom(address(this), highestBidder, highestBidAmount);
        }
     ...
     * @return (uint256) Minimum increment amount
     ...
     * @return (address) Highest bidder address
     * @return (uint256) PaymentToken index
    ...
    /**
     * @dev Get all active auctions with specific PaymentToken index
     * @param paymentTokenIndex_ (uint256) PaymentToken index
     * @return auctions (Auction[] memory) Auctions
     */
    function getActiveAuctions(uint256 paymentTokenIndex_)
        public
        view
        returns (AmberfiLib.Auction[] memory auctions)
    {
        ...
        for (uint256 i; i < getAuctionsCount(); i++) {
            if (
                _auctions[i].auctionState == AmberfiLib.AuctionState.STARTED &&
                paymentTokenIndex_ == _auctions[i].paymentTokenIndex
            ) {
    ...
    function getMarketId() public view returns (uint256) {
        return _marketRegistry.marketId(address(this));
    }
