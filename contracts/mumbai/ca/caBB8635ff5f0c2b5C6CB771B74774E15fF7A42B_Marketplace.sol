...
contract Marketplace is ReentrancyGuard, Ownable, Pausable {
    ...
    function claimPendingRevenue(address unitAddress) external nonReentrant {
        ...
        _transferByUnit(address(this),msg.sender, revenueToClaim, unitAddress);
    ...
    function createOrder(
        ...
        // TODO: cancel prev order if tokenId exist
        ...
        whenNotPaused
    {
        ...
        emit MarketItemCancel(marketItemId, nftContractAddress, tokenId);
    }
    ...
    function _buy(uint256 marketItemId, uint256 amount) internal {
        ...
        // Transfer to seller
        _transferByUnit(msg.sender, seller, netPrice, unitAddress);
     ...
     */
    function _transferByUnit(address fromAddress, address toAddress, uint256 amount, address unitAddress) internal returns(bool) {
        if(unitAddress == address(0)) {
            payable(address(toAddress)).transfer(amount);
        }
        if(unitAddress != address(0)) {
            if(fromAddress == address(this)) {
                IERC20(unitAddress).safeTransfer(toAddress, amount);
            }
            if(fromAddress != address(this)) {
                IERC20(unitAddress).safeTransferFrom(fromAddress, toAddress, amount);
            }
        }
