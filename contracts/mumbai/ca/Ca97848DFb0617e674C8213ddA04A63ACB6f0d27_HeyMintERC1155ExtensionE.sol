...
contract HeyMintERC1155ExtensionE is HeyMintERC1155Upgradeable {
     ...
     */
    function setSoulbindingState(uint16 _tokenId, bool _soulbindingActive) external onlyOwner {
        HeyMintStorage.state().tokens[_tokenId].soulbindingActive = _soulbindingActive;
    }
    ...
    function refundPriceInWei(uint16 _tokenId) public view returns (uint256) {
        return uint256(HeyMintStorage.state().tokens[_tokenId].refundPrice) * 10 ** 13;
    }
     ...
     */
    function increaseRefundEndsAt(uint16 _tokenId, uint32 _newRefundEndsAt) external onlyOwner {
        HeyMintStorage.State storage state = HeyMintStorage.state();
        require(
    ...
    function refund(uint16 _tokenId, uint256 _numTokens) external nonReentrant {
        ...
        require(balanceOf(msg.sender, _tokenId) >= _numTokens, "NOT_ENOUGH_TOKENS_OWNED");
        HeyMintStorage.State storage state = HeyMintStorage.state();
        ...
        safeTransferFrom(msg.sender, addressToSendToken, _tokenId, _numTokens, "");

        (bool success,) = payable(msg.sender).call{value: refundPriceInWei(_tokenId) * _numTokens}("");
    ...
    function burnPaymentInWei(uint16 _tokenId) public view returns (uint256) {
        return uint256(HeyMintStorage.state().tokens[_tokenId].burnPayment) * 10 ** 13;
    }
     ...
     * @notice To be updated by contract owner to allow burning to claim a token
         ...
         */
    function setBurnClaimState(uint16 _tokenId, bool _burnClaimActive) external onlyOwner {
        HeyMintStorage.State storage state = HeyMintStorage.state();
    ...
    function updateBurnTokens(
        ...
        BurnToken[] storage burnTokens = HeyMintStorage.state().burnTokens[_tokenId];
     ...
     * @notice Update the number of free mints claimable per token burned
         ...
         */
    function updateMintsPerBurn(uint16 _tokenId, uint8 _mintsPerBurn) external onlyOwner {
     ...
     * @notice Update the price required to be paid alongside a burn tx to mint (payment is per tx, not per token in the case of >1 mintsPerBurn)
         ...
         */
    function updatePaymentPerBurn(uint16 _tokenId, uint32 _burnPayment) external onlyOwner {
    ...
    function burnToMint(
        ...
        require(_contracts.length == state.burnTokens[_tokenId].length, "ARRAY_LENGTHS_MUST_MATCH");
        require(
            state.data.totalSupply[_tokenId] + _tokensToMint <= state.tokens[_tokenId].maxSupply,
            "MAX_SUPPLY_EXCEEDED"
        );
        uint256 burnPayment = burnPaymentInWei(_tokenId);
        uint256 burnPaymentTotal = burnPayment * (_tokensToMint / state.tokens[_tokenId].mintsPerBurn);
        ...
        for (uint256 i = 0; i < state.burnTokens[_tokenId].length; i++) {
            ...
            if (burnToken.tokenType == 1) {
                ...
                require(
                    (_tokenIdsToBurnLength / burnToken.tokensPerBurn) * state.tokens[_tokenId].mintsPerBurn == _tokensToMint,
                ...
                require(
                    (amountToBurn / burnToken.tokensPerBurn) * state.tokens[_tokenId].mintsPerBurn == _tokensToMint,
                    "INCORRECT_NO_OF_TOKENS_TO_BURN"
                );
        ...
        state.data.totalPaidPerToken[_tokenId] += msg.value;
