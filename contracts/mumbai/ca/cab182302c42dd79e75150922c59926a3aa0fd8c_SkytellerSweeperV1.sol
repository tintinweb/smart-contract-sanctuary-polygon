 ...
 * @notice Version 1 of the sweeper for Skyteller
 *         This contract is responsible for sweeping tokens on behalf of others,
...
contract SkytellerSweeperV1 is ISkytellerSweepDelegate, Ownable2Step, Pausable, Initializable {
    ...
    /// @dev Default minimum amount out for conducting swaps (in bips)
    uint16 internal constant MINIMUM_SWAP_PERCENT_OUT_BIPS = 9850;

    /// @dev Precision for calculations with bips
    uint16 internal constant BIPS_PRECISION = 10000;

    /// @dev Number of decimals in a returned price derivation
    uint8 internal constant PRICE_DERIVATION_DECIMALS = 18;

    /// @dev Precision for price derivation
    uint256 internal constant PRICE_DERIVATION_PRECISION = 10 ** PRICE_DERIVATION_DECIMALS;

    /// @dev Default skyteller swap fee (1 bip == 100)
    uint24 internal constant DEFAULT_SKYTELLER_SWAP_FEE = 7500;

    /// @dev Fee precision for Uniswap V3, which we're also using for the Skyteller fee
    ...
    /**
     * @notice Initialize the sweeper. Sets owner and initial values.
     ...
     * @param _usdFeed The address of the USD price feed for the token
     * @param _ethFeed The address of the ETH price feed for the token
     * @param _usdPegged Whether the token should be treated as always pegged to USD
     */
    function setTokenPriceFeeds(address _token, address _usdFeed, address _ethFeed, bool _usdPegged)
        external
        onlyOwner
    {
        _tokens.add(_token);
        tokenPriceFeedConfig[_token] =
            TokenPriceFeedConfig({usdPegged: _usdPegged, usd: _usdFeed, eth: _ethFeed});
    }
...
/**
 *  @title Skyteller Sweep Delegate Interface
 * @notice Interface for a contract that can sweep tokens
 */
