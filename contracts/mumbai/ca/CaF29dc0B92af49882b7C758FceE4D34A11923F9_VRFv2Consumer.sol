...
contract VRFv2Consumer is VRFConsumerBaseV2, ConfirmedOwner {
    ...
    function random(uint256 range) external returns (uint256){
        ...
        bool shouldRefreshRandomness = nounces[last_fullfilled_id] >= words[last_fullfilled_id].length;
        uint256 nounceOffset = shouldRefreshRandomness ? nounces[last_fullfilled_id] : 0;
        uint256 index = nounces[last_fullfilled_id] % words[last_fullfilled_id].length;
        uint256 _rand_value = (words[last_fullfilled_id][index] + nounceOffset) % range;
        nounces[last_fullfilled_id] = nounces[last_fullfilled_id].add(1);
        if(shouldRefreshRandomness)
            refresh_randomness_internal();
