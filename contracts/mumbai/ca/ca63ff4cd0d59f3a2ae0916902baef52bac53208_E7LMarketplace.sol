...
contract E7LMarketplace is ReentrancyGuard, Ownable {
  ...
  function makeBidUp(
    ...
    uint tokenId,
    uint bidPrice
  ) external payable isPaytokenValid(paytoken) nonReentrant {
   ...
   * @param tokenId Token ID of NFT
   */
  function acceptBidUp(uint tokenId) external isOwner(tokenId, msg.sender) nonReentrant {
