...
//tester baseuri ipfs://QmVYn6bCqSiQJkg9M2yGHyEaDnoYT8415Po8Z72hophYhj/ 
...
contract SavageStreetTester is ERC721r, Ownable, Pausable, ReentrancyGuard {
  ...
  function mint (uint quantity) external payable {
    if(stage == Stages.whitelistSale) {
      ...
      require(msg.value >= prePrice * quantity,"Not enough matic sent... use the damn dapp wanna be hacker.");
      require(quantity <= mintThrottle, "Sorry the throttle is set to 2 per TX to be fair.");
      require(quantity + mintCountPerOwner[msg.sender] <= maxMintForWhitelist,"You've minted the max amount during pre-sale");
    }
    require(msg.value >= price * quantity); 
    mintCountPerOwner[msg.sender] += quantity;
    _mintRandom(msg.sender, quantity);
  }
  function mint2 (uint quantity) public payable {
    if(stage == Stages.whitelistSale) {
      require(isWhitelisted(msg.sender) == true, "Ah Ah Ah, you're not on the whitelist.");
      require(msg.value >= prePrice * quantity,"Not enough matic sent... use the damn dapp wanna be hacker.");
      require(quantity <= mintThrottle, "Sorry the throttle is set to 2 per TX to be fair.");
  ...
  function promoGiveawayMint(uint quantity) public onlyOwner {
    _mintRandom(msg.sender, quantity);
  }
