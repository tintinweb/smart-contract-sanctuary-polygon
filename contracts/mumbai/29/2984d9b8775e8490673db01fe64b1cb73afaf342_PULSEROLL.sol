...
contract PULSEROLL is VRFConsumerBase {
    ...
    modifier CheckRangeSelect(uint _rangelower,uint _rangeupper,uint _rollUnder)
    {
    ...
    function playerRollDice(uint256 rollUnder,uint _OddEvenStatus ,uint rangeLowerLimit,uint rangeUpperLimit) public payable gameIsActive  CheckOddEven(_OddEvenStatus) CheckRangeSelect(rangeLowerLimit,rangeUpperLimit,rollUnder) betIsValid(rollUnder,_OddEvenStatus,rangeLowerLimit,rangeUpperLimit) CheckZero 
    returns(bytes32)
    {
    ...
    //Get functions 
    function GetMultiplier(uint _RollUnder,uint _OddEvenStatus,bool _IsRangeTrue,uint RangeLow,uint RangeHigh)public view CheckOddEven(_OddEvenStatus) CheckRangeSelect(RangeLow,RangeHigh,_RollUnder) returns(uint)
    {
        require(_RollUnder >= minNumber &&  _RollUnder <= maxNumber);
        if(RangeLow == 0 || RangeHigh ==0)
        {
            require(_IsRangeTrue == false);
        }
        ...
        if(_IsRangeTrue == true)
        {
            if(_OddEvenStatus ==1 || _OddEvenStatus == 2)
            {
                for(uint i=RangeLow;i<=RangeHigh;i++)
                {
                    ...
                    else if(_OddEvenStatus ==2)
                    {
                        bool check=CheckEven(i);
                        if(check)
                        {
                            range+=1;
                        }
            ...
            uint totalChances = ((100 - range)*(1e18)) / BaseRangeDivisor;//as the number is less they need to get high multipliers
            Multiplier =Multiplier+totalChances;
        }
        return Multiplier;
    }

    function GetProfit(uint _RollUnder,uint _OddEvenStatus,uint RangeLow,uint RangeHigh,uint betValue )public view CheckOddEven(_OddEvenStatus) CheckRangeSelect(RangeLow,RangeHigh,_RollUnder) returns(uint )
    {
        require(_RollUnder >= minNumber &&  _RollUnder <= maxNumber);
        bool _IsRangeTrue;
        if(RangeLow !=0 && RangeHigh !=0 )
