...
abstract contract AllowlistModifiers is IAllowlistFacet, Listing {
    ...
    modifier purchasable(uint256 allowlistId_) override {
        ...
        uint256 projectBalance = s.userInventory[msg.sender].projectBalance[projectId];
        _purchasable( allowlistId_, projectId);
        if (projectBalance >= s.projects[projectId].maxPerWallet ) revert MaxPerWalletReached();
        _;
    }
