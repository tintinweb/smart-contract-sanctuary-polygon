...
contract AntbBondingCalculator is IAntbBondingCalculator {
    ...
    function markdown(address _pair) external view override returns (uint256) {
        ...
        uint256 reserve;
        if (IUniswapV2Pair(_pair).token0() == ANTB) {
            reserve = reserve1;
        } else {
            reserve = reserve0;
        }
        return
            reserve.mul(2 * (10**IERC20(ANTB).decimals())).div(
                getTotalValue(_pair)
            );
    }
}
