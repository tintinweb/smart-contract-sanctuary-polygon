...
contract StakingContract is ERC20 {
    ...
    mapping(address => stakeStruct) staker;
    ...
    event claimed(address Owner, uint reward);
    ...
    function stake(uint256 _stakingAmount) external {
        ...
        transferFrom(msg.sender, address(this), _stakingAmount);
        staker[msg.sender] = stakeStruct(_stakingAmount, block.timestamp);
    ...
    function unStake() external {
        ...
        staker[msg.sender] = stakeStruct(0, 0);
        (uint256 endTime, uint256 totalStakedTime) = rewardCalculate(
            _amount,
            staker[msg.sender].startTime
        );

        transferFrom(address(this), msg.sender, _amount);
        emit Unstaked(_amount, totalStakedTime, endTime);
    }
    ...
    function rewardCalculate(
        ...
        uint256 totalstakedTime = endTime - _startTime;
