...
contract SuperfluidOps {
    ...
    function doAction(
        address _user,
        bytes calldata _callAgreementData,
        uint256 _actionTime
    ) external onlyOps {
        ...
        require(
            _opsTaskIds[_user].contains(opsTaskId),
        ...
        superfluid.callAgreement(cfa, _callAgreementData, ctx);
    ...
    function checker(
        ...
        execPayload = bytes("SuperfluidOps: Time not elapsed");
    ...
    function _getCallAgreementData(
        ...
        if (_action == OperatorAction.CREATE_FLOW)
        ...
        if (_action == OperatorAction.UPDATE_FLOW)
            data = abi.encodeWithSelector(
        ...
        if (_action == OperatorAction.DELETE_FLOW)
            data = abi.encodeWithSelector(
    ...
    function _getOpsTaskId(
        address _user,
        bytes memory _callAgreementData,
        uint256 _actionTime
