...
*/
    ...
    function buyTokens(uint256 _amountToBuy) external nonReentrant{
        require(block.timestamp < duration,"Launchpad Ended!");
        uint256 finalPrice = price*10**priceDecimals;
        uint256 amount = _amountToBuy / finalPrice;
        emit tokenSale(_amountToBuy, amount);
        //The transfer requires approval from currency smart contract
        currency.transferFrom(msg.sender, address(this), amount);
        sellerVault += amount;
        buyer[msg.sender].tokenAmount = _amountToBuy;
        buyer[msg.sender].currencyPaid = amount;
        boughtAmounts += _amountToBuy;
...
contract PolyKick_Factory{
    ...
    function startLaunchpad(
        ...
        pkLaunchpad = new PolyKick_Launchpad(_factory, _token, _tokenAmount, _currency, _price, _priceDecimals, _target, _duration);
