...
contract MissionControl is IMissionControl, OwnableUpgradeable, EIP712Upgradeable {
    ...
    function placeNFT(
        ...
        // Should only allow radius 1 tiles on this function
        require((abs(order.x) + abs(order.y) + abs(order.z)) / 2 == 1, "MISSION_CONTROL: RADIUS 1 ONLY");
    ...
    function placeNFTs(PlaceOrder[] memory _placeOrders) external override {
        for (uint256 i; i < _placeOrders.length; i++) {
            require(
                (abs(_placeOrders[i].order.x) + abs(_placeOrders[i].order.y) + abs(_placeOrders[i].order.z)) / 2 == 1,
                "MISSION_CONTROL: RADIUS 1 ONLY"
            );
