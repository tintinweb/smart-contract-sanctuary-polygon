...
contract ApeironPlanetPresaleBurner is Ownable {
    ...
    function reserveAndBurn(
        ...
        require(_tokenIds.length > 0, "invalid tokenIds size");
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            //approve the token to operate
            planetContract.approve(address(this), _tokenIds[i]);

            //burn the token
            planetContract.burn(_tokenIds[i]);
            
            emit Burn(_tokenIds[i]);
        }
    }
}
