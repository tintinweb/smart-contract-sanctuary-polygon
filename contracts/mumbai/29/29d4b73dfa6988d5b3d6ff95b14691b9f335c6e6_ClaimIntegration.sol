...
// File: contracts/ClaimIntegration.sol
...
contract ClaimIntegration is Ownable {
    ...
    event DistributeRewards(address[] _addresses, uint[] _amounts, address _tokenAddress);
    ...
    function setReveneueAddress(address _address) onlyOwner public {
        require(_address != reveneuAddress, "same addresses");
    ...
    function getReveneueAddress() public view returns (address) {
        return reveneuAddress;
    }
    ...
    function distributeRewards(address[] memory _addresses, uint[] memory _amounts, address _tokenAddress, address _masterAddress) public {
        require(_addresses.length == _amounts.length, "addresses and amounts are not the same length");
        ...
        for (uint i = 0; i < _addresses.length; i++) {
          ...
          // send fee to reveneu address
          IERC20(_tokenAddress).transferFrom(_addresses[i], reveneuAddress, currentFee);
          IERC20(_tokenAddress).transferFrom(_addresses[i], _masterAddress, amount);
        }

        emit DistributeRewards(_addresses, _amounts, _tokenAddress);
    }
}