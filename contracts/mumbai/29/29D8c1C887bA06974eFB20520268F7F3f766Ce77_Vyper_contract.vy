# @dev Implementation of Storefront
...
id_to_uri: HashMap[uint256, String[256]]
...
@external
def __init__():
    self.name = "ARTPEDIA DEMO Yellow"
    self.symbol = "ART-Y"
...
@internal
def _mint(token_uri: String[256], _to: address, sender: address):
    ...
    self.id_to_uri[token_id] = token_uri
...
def tokenURI(_tokenId: uint256) -> String[256]:
    owner: address = self.id_to_owner[_tokenId]
    assert owner != ZERO_ADDRESS, "ERC721: owner query for nonexistent token"
    return self.id_to_uri[_tokenId]


@external
def mint(token_uri: String[256], _to: address) -> bool:
    """
    ...
    """
    self._mint(token_uri, _to, msg.sender)
    return True


@external
def safeMint(token_uri: String[256], _to: address, _data: Bytes[1024] = b"") -> bool:
    """
    @dev Function to mint tokens
         Throws if `msg.sender` is not the minter.
         Throws if `_to` is zero address.
