...
interface IERC20Basics {
    function decimals() external view returns (uint8);
}

contract TreasuryManagement is Initializable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    address payable public veritoneTreasuryAddress;

    /// @notice An address source for the contract to send all collected fees to
    address payable public partnerTreasuryAddress;
    ...
    modifier onlyAdmin() {
        ...
        require(hasAdmin, "TreasuryManagement: Not an Admin");
        _;
    }
    ...
    function delegatedConstructor(
        ...
        address payable _veritoneTreasuryAddress,
        address payable _partnerTreasuryAddress,
        uint256 _mintFee,
        ...
        for (uint256 i = 0; i < whitelistedTokenAddresses.length; i++) {
            dataFeedSourceAddress[whitelistedTokenAddresses[i]] = _dataFeedAddresses[i];
        }
        mintFee = _mintFee;
        partnerTreasuryAddress = _partnerTreasuryAddress;
        veritoneTreasuryAddress = _veritoneTreasuryAddress;
    ...
    fallback() external payable {
        veritoneTreasuryAddress.transfer((msg.value * 90).div(100));
        partnerTreasuryAddress.transfer((msg.value * 10).div(100));
        emit etherSent(
            "The contract does not support direct payments, your Ether has been transferred to Treasury Address"
        );
    }
    ...
    {
        if (!IWhitelist(addressRegistry.whitelistContract()).hasRoleAdmin(_sender)) {
            IERC20Upgradeable token = IERC20Upgradeable(_token);
            int256 price = getPriceOfTokenByAddress(_token);
            uint256 fee = ((mintFee * 10**8) / uint256(price)) * 10**(uint256(IERC20Basics(_token).decimals()));
            require(token.balanceOf(_sender) >= fee, "TreasuryManagement: Not Enough ERC20 Balance");
            require(token.allowance(_sender, address(this)) >= fee, "TreasuryManagement: Not Enough Allowance");

            token.transferFrom(_sender, veritoneTreasuryAddress, (fee * 90).div(100));
            token.transferFrom(_sender, partnerTreasuryAddress, (fee * 10).div(100));
        ...
        returns (bool success, uint256 _fee)
    {
        if (!IWhitelist(addressRegistry.whitelistContract()).hasRoleAdmin(_sender)) {
            IERC20Upgradeable token = IERC20Upgradeable(_token);
            int256 price = getPriceOfTokenByAddress(_token);
            uint256 fee = ((platformFee * 10**8) / uint256(price)) * 10**(uint256(IERC20Basics(_token).decimals()));
            require(token.balanceOf(_sender) >= fee, "Not Enough ERC20 Balance");
            require(token.allowance(_sender, address(this)) >= fee, "Not Enough Allowance");
            // require(fee);
            token.transferFrom(_sender, veritoneTreasuryAddress, (fee * 90).div(100));
            token.transferFrom(_sender, partnerTreasuryAddress, (fee * 10).div(100));
            emit chargeForMintingEvent(fee, true);
    ...
    function addTokenSupport(address _tokenAddress, address _OracleAddress) public onlyAdmin {
        require(dataFeedSourceAddress[_tokenAddress] == address(0), "Token Already Exists");
    ...
    function removeTokenSupport(address _tokenAddress) public onlyAdmin {
        require(dataFeedSourceAddress[_tokenAddress] != address(0), "Token Doesn't Exist");
        dataFeedSourceAddress[_tokenAddress] = address(0);
        uint256 _index = getIndexByToken(_tokenAddress);
        for (uint256 i = _index; i < whitelistedTokenAddresses.length - 1; i++) {
    ...
    /// @notice updates Veritone Treasury Address mint fee
    function updateVeritoneTreasuryAddress(address payable _veritoneTreasuryAddress) public onlyAdmin {
        veritoneTreasuryAddress = _veritoneTreasuryAddress;
    }

    /// @notice updates Partner Treasury Address mint fee
    function updatePartnerTreasuryAddress(address payable _partnerTreasuryAddress) public onlyAdmin {
        partnerTreasuryAddress = _partnerTreasuryAddress;
    }
    ...
    function getIndexByToken(address _tokenAddress) public view returns (uint256 value) {
        for (uint256 i = 0; i < whitelistedTokenAddresses.length; i++) {
    ...
    function getAllowance(address _token) public view returns (uint256) {
        return IERC20Upgradeable(_token).allowance(msg.sender, address(this));
    }

    /// @notice Uses the price oracle to retrive the tokenaddress provided.
    function getPriceOfTokenByAddress(address _tokenAddress) public returns (int256 value) {
    ...
    function getOracleByAddress(address _token) public view returns (address oracleAddress) {
