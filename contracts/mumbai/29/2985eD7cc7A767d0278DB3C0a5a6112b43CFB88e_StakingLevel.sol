...
contract StakingLevel is Ownable, AccessControl, ReentrancyGuard, Pausable {
    ...
    constructor(
        ...
        upoToken = IERC20(_upoTokenAddress);
     ...
     */
    function addPoolReward(uint256 _upoAmount)
        external
        whenNotPaused
        onlyRole(WORKER)
    {
        totalPoolReward += _upoAmount;
        upoToken.safeTransferFrom(msg.sender, address(this), _upoAmount);
    ...
    function stakeUpOnly(uint256 _upoAmount) external whenNotPaused {
        ...
        // Update last valid to current if not pending
    ...
    function withdrawUpOnly(uint256 _upoAmount) external nonReentrant {
        ...
        totalPoolStake -= _upoAmount;
        user.currentStakeAmount -= _upoAmount;
        safeUpOnlyTransfer(msg.sender, _upoAmount);
    ...
    function claimReward() public nonReentrant {
        UserInfo storage user = userInfo[msg.sender];
        (uint256 validClaimHour, uint256 unclaimAmount) = getUserUnclaimInfo(msg.sender);
        require(
            unclaimAmount > 0,
        ...
        safeUpOnlyTransfer(msg.sender, unclaimAmount);
    ...
    /**
     * @dev Transfer uponly safely.
     * @param _userAddress - user address.
     * @param _upoAmount - amount to be transfer.
     */
    function safeUpOnlyTransfer(address _userAddress, uint256 _upoAmount)
        internal
    {
        uint256 upoBalance = upoToken.balanceOf(address(this));
        if (_upoAmount >= upoBalance) {
            upoToken.safeTransfer(_userAddress, upoBalance);
        } else {
            upoToken.safeTransfer(_userAddress, _upoAmount);
        }
    }
    ...
    /**
     * @dev Set staking level in to pause state (only withdraw, claim reward and getter is allowed).
     */
    ...
    /**
     * @dev Set staking level in to normal state.
     */
