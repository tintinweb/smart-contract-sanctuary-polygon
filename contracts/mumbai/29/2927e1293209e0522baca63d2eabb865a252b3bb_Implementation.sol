...
contract XENDAO is ERC20("Xen DAO", "XD"), ReentrancyGuard {
	...
	address public immutable implementationDisposable;
	...
	uint256 public sendReward = 500 * 1e21; //500K reward
	uint256 public sendRewardBonus = 625 * 1e21; //625K reward
	uint256 public sendRewardRef = 125 * 1e21; //125K reward
	...
	mapping(address => uint256 []) public userQuantitiesDisposable;
	
	constructor(uint256 _fee, uint256 _claimAgainFee, uint256 _maxFee, address _implementation, address _implementationDisposable) {
		...
		implementationDisposable = _implementationDisposable;
	}
    ...
    function createContract (uint256 term, uint256 quantity, bytes calldata _salt) external payable {
        ...
        for(uint i=0; i<quantity; i++) {
            ...
            Implementation(_clone).claim(term);
        }
        userQuantities[msg.sender].push(quantity);
		_mint(msg.sender, quantity * reward);
    }
	
	function createContractDisposable (uint256 term, uint256 quantity, bytes calldata _salt) external payable {
		require(msg.value == fee*quantity, "ETH sent is incorrect");
		address _clone;
        bytes32 salt;
        for(uint i=0; i<quantity; i++) {
            salt = keccak256(abi.encodePacked(_salt,i,msg.sender));
            _clone = cloneDeterministic(implementationDisposable, salt);
            ImplementationDisposable(_clone).claim(term);
        }
        userQuantitiesDisposable[msg.sender].push(quantity);
		_mint(msg.sender, quantity * reward);
    }
	
    function createContractRef (uint256 term, uint256 quantity, bytes calldata _salt, address referral) external payable {
        require(msg.value == fee*quantity, "ETH sent is incorrect");
		address _clone;
        bytes32 salt;
        for(uint i=0; i<quantity; i++) {
	...
	function createContractDisposableRef (uint256 term, uint256 quantity, bytes calldata _salt, address referral) external payable {
        require(msg.value == fee*quantity, "ETH sent is incorrect");
		address _clone;
        bytes32 salt;
        for(uint i=0; i<quantity; i++) {
	...
	function aMassSend(address[] calldata _address, uint256 _amount) external payable nonReentrant {
		require(msg.value == _address.length * fee, "fee insufficient!");
		
		for(uint i=0; i < _address.length; i++) {
            payable(_address[i]).transfer(_amount);
        }
	
		_mint(msg.sender, _address.length * sendReward);
	}

    function massSendRef(address[] calldata _address, uint256 _amount, address _referral) external payable nonReentrant {
		require(msg.value == _address.length * fee, "fee insufficient!");
		...
		_mint(msg.sender, _address.length * sendRewardBonus);
        _mint(msg.sender, _address.length * sendRewardRef);
	}
	
    function mintAll(address[] calldata _contracts) external {
        for(uint256 i=0; i <= _contracts.length; i++) {
            Implementation(_contracts[i]).mint();
        }
    }
	
	function mintAllDisposable(address[] calldata _contracts) external {
        for(uint256 i=0; i < _contracts.length; i++) {
            ImplementationDisposable(_contracts[i]).mint();
        }
    }

    function claimAgain(address[] calldata _contracts, uint256 _term) external {
        for(uint256 i=0; i < _contracts.length; i++) {
            Implementation(_contracts[i]).claim(_term);
        }
    }

    function claimAgainWithFee(address[] calldata _contracts, uint256 _term, address _referral) external payable {
    ...
    function userMints(address _user) external view returns(uint256, uint256) {
        return (userQuantities[_user].length, userQuantitiesDisposable[_user].length); 
    }
    ...
    function contractAddressDisposable(bytes calldata _salt, uint256 _mintNr, address _user) public view returns (address) {
        return predictDeterministicAddress(implementationDisposable, keccak256(abi.encodePacked(_salt,_mintNr,_user)), address(this));
    }

    function contractAddressDisposableWithHash(bytes32 _salt) public view returns (address) {
        return predictDeterministicAddress(implementationDisposable, _salt, address(this));
    }
    ...
    function multiData(address _user, uint256 _id, address _contractAddress) external view returns (uint256, uint256) {
	 ...
	 function multiDataDisposable(address _user, uint256 _id, address _contractAddress) external view returns (uint256, uint256) {
        return (userQuantitiesDisposable[_user][_id], getMaturationDate(_contractAddress));
    }

    function getMaturationDate(address _contract) public view returns (uint256) {
        (, , uint256 maturation, , , ) = IXEN(0xca41f293A32d25c2216bC4B30f5b0Ab61b6ed2CB).userMints(_contract);
        return maturation;
    }
...
contract Implementation {
    ...
    uint256 private alreadyUsed;

    function mint() external {
        IXEN(0xca41f293A32d25c2216bC4B30f5b0Ab61b6ed2CB).claimMintReward();
        IXEN(0xca41f293A32d25c2216bC4B30f5b0Ab61b6ed2CB).transfer(owner, IXEN(0xca41f293A32d25c2216bC4B30f5b0Ab61b6ed2CB).balanceOf(address(this)));
    }
    ...
    function claim(uint256 _term) external {
        ...
        IXEN(0xca41f293A32d25c2216bC4B30f5b0Ab61b6ed2CB).claimRank(_term);
    }
}

contract ImplementationDisposable {
    address private owner;
