...
contract Presale is Ownable {
    ...
    // Matic has been exchanged for tokens    
    ...
    // track contributions to enforce per-address maximums
    mapping(address => uint256) private _contributions;

    // maximum amount an address can exchange
    uint256 private _maximum;

    // exchange disabled when paused
    bool private _paused = false;
    
    // initialExchangeRate Number of Tokens provided for 1 Matic
    constructor(address token, uint256 initialExchangeRate) {
        ...
        _minimum = 10000000000000000;  // 0.01 Matic
        _maximum = 3000000000000000000; // 3 Matic
    ...
    // exchange the value of Matic sent in the transaction for tokens
    ...
    // set the minimum amount of (wei) per transaction
    function setMinimumExchange(uint256 newMinimum) external onlyOwner {
        _minimum = newMinimum;
    }  

    // set the maximum amount of (wei) each address can exchange
    ...
    // send contract Polygon to the owner
