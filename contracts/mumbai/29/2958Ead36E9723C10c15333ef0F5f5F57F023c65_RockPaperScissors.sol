    ...
    IRockPaperScissors
{
    ...
    uint256 private s_comissionPerentage = 10;
    ...
    function _createBetAndSettle(
        ...
        uint256 comission = getComissionFromBet(msg.value);

        Results _result = _winOrLoose(_playerChoice, _outcome);

        if (_result == Results.Win) {
            (bool hs, ) = payable(_player).call{value: (winAmount - comission)}(
                ""
            );
        ...
        if (_result == Results.Tie) {
            (bool hs, ) = payable(_player).call{
                value: (_betAmount - comission)
            }("");
    ...
    function getComissionFromBet(uint256 _bet)
        internal
        view
        returns (uint256 _comission)
    {
        _comission = (_bet * (s_comissionPerentage / 100));
    }

    function getComissionPercentage()
        external
        view
        returns (uint256 _comissionPercentage)
    {
        _comissionPercentage = s_comissionPerentage;
    }
    ...
    function updateComissionPercentage(uint256 _comissionPercentage)
        external
        onlyOwner
    {
        s_comissionPerentage = _comissionPercentage;
        emit ComissionPercentageUpdated(_comissionPercentage);
    }
