...
contract CheckExecutor is Executor, SecuredTokenTransfer, Ownable {
    ...
    event EnableCheckLock(bool lock, uint timestamp);
    event DisableCheckLock(bool lock, uint timestamp);

    event EnableBatch( uint256 batch, bool result, uint timestamp);
    event DisableBatch( uint256 batch, bool result, uint timestamp);
    ...
    // bytes32 private constant SAFE_CHECK_EXECUTION_TYPEHASH = keccak256(
    //     "SafeCheckExecution(address checkOwner,address to,uint256 value,bytes data,uint8 operation,uint256 startTime,uint256 expirationTime,uint256 batch,uint256 safeTxGas,uint256 baseGas,uint256 gasPrice,address gasToken,address refundReceiver,uint256 nonce)"
    // );

    bytes32 private constant SAFE_CHECK_EXECUTION_TYPEHASH = 0x647cf02284eea0ecc0dc0b7abae62ba81dc31aa0e7a36bee6accb6705feb1e68;

    mapping(uint256 => bool) private executedCheckNonceRegister;
    mapping(uint256 => bool) private _batch;
    ...
    function checkTokenBalance(bytes memory checkMsg) public view returns (uint256 amount) {
            CheckInfo memory checkInfo = decodeCheckMsg(checkMsg);
            bytes memory data = slice(checkInfo.data,4,64);
            ( ,amount) = abi.decode(data,(address, uint256));
    }
    ...
    function executeCheck(bytes memory checkMsg) public returns (bool success) {
        ...
        require( isLock() == false, "C202");
       
        // decode checkMsg to get check info
        CheckInfo memory checkInfo = decodeCheckMsg(checkMsg);

        require(_batch[checkInfo.batch],"C203");
        
        require(block.timestamp >= checkInfo.startTime,"C204");

         require(block.timestamp <= checkInfo.expirationTime,"C205");
    ...
    function encodeCheckExecutionData(CheckInfo memory checkInfo) public view returns (bytes memory) {
        ...
        bytes32 safeCheckExecutionHash = keccak256(
                ...
                checkInfo.operation,
                checkInfo.startTime,
                checkInfo.expirationTime,
                checkInfo.batch), 
                abi.encode(checkInfo.safeTxGas,
                checkInfo.baseGas,
                checkInfo.gasPrice,
                checkInfo.gasToken,
                checkInfo.refundReceiver,
    ...
    function isLock() public view virtual returns (bool)
    {
        return _lock;
    }

    function enableLock () onlyOwner external returns (bool)
    {
        require(!_lock, "Lock already Enabled!");
        _lock = true;
        emit EnableCheckLock(true, block.timestamp);
        return true;
    }

    function disableLock() onlyOwner external returns (bool)
    {
        require(_lock, "Lock already Disabled!");
        _lock = false;
        emit DisableCheckLock(false, block.timestamp);
        return true;
    }

    function enableBatch(uint256  batch) onlyOwner external returns (bool)
    {
        
        require(!_batch[batch], "batch already Enabled!");
        _batch[batch] = true;
        emit EnableBatch(batch,true, block.timestamp);
        return true;
    }

    function disableBatch(uint256  batch) onlyOwner external returns (bool)
    {
        require(_batch[batch], "batch already Disabled!");
        _batch[batch] = false;
        emit DisableBatch(batch,true, block.timestamp);
        return true;
    }
}
