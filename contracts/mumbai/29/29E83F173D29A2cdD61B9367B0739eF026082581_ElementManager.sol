...
import {IElementManager} from "./IElementManager_v02.sol";
...
//contract ElementManager
{
    ...
    uint256 public processLastBlk;

    uint256 public cursorGlobal;
    uint256 immutable public BlkDeploy;
...
//    constructor(uint256 _elementsToOpen,uint256 _sizeLimit,uint256 _elementsRange) 
    ...
    function getClosableElements(uint256 _cursor, uint256 _count)public view override returns(uint256 newCursor,bool closureNeeded,uint256[] memory ids )
    {
        uint256 count;
    ...
    function needProcess()public view returns(bool)
    {
	// в Etherum блоктайм ~5sec, поэтому делаем усвловие, что не нужно чаще выполнять closeElements
	if((processLastBlk + 6*5) > block.timestamp)return false;
	return true;
    }
//    function closeElements(uint256[] memory ids) public override 
    function closeElements() public override returns(bool flag)
    {
	emit eLog(nn++,"func: closeElements",0);
	if(needProcess() == false)
	{
	    emit eLog(nn++,"Exit by needProces false",0);
	    return false;
	}

	processLastBlk = block.timestamp;
	emit eLog(nn++,"processLastBlk",processLastBlk);

	bool closureNeeded;
	uint256[] memory ids;
	(cursorGlobal,closureNeeded,ids) = getClosableElements(cursorGlobal, _elementsToOpen);
//	uint256 l = ids.lenght;
//	if(l < sizeLimit)
//	(cursorGlobal,bool closureNeeded,uint256[] memory ids) = getClosableElements(0, _elementsToOpen);

	emit eLog(nn++,"ids length",ids.length);
