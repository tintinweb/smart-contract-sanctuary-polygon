...
contract ProtoVerseGenesisCollection is
    Initializable,
    ERC721Upgradeable,
    ...
    UUPSUpgradeable
{
    ...
    function initialize() public initializer {
        __ERC721_init("ProtoVerse Genesis Collection", "PVR");
        ...
        __VRFConsumerBase_init(
            state.rand.linkVRFAddress, // VRF Coordinator
            state.rand.linkTokenAddress // LINK Token
        );
    }
    ...
    function safeMint(
        ...
        _setTokenURI(newTokenId, nft.uri);
        _setTokenRoyalty(newTokenId, msg.sender, nft.royaltyPercent);
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (state.shouldRefreshRand()) {
            bytes32 requestId = getRandomNumber();
            state.requestRand(requestId);
        }
    ...
    function getRandomNumber() internal returns (bytes32 requestId) {
        require(
            LINK.balanceOf(address(this)) >
                state.config.uints[AppType.UintConfig.RAND_LINK_FEE],
            "E0001"
        );

        return
            requestRandomness(
                state.rand.linkVRFKeyHash,
                state.config.uints[AppType.UintConfig.RAND_LINK_FEE]
            );
    }
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness)
        ...
        override
    {
        state.receivedRand(requestId, randomness);
    }
    ...
    function createBatch(
        ...
        state.createBatch(kind, isOpenAt, disabled, root);
    }
...
abstract contract VRFConsumerBase is Initializable, VRFRequestIDBase {
    ...
    function __VRFConsumerBase_init(address _vrfCoordinator, address _link) public initializer {
...
library AppType {
    ...
    struct Randomness {
        uint256 latest;
        uint256 lastRefreshBlock;
        address linkTokenAddress;
        address linkVRFAddress;
        bytes32 linkVRFKeyHash;
        mapping(bytes32 => uint256) requests; // requestId => randomNumber
    }
...
library App {
    ...
    function initialize(AppType.State storage state) public {
        state.config.addresses[AppType.AddressConfig.DAO] = msg.sender; // TODO: Change to DAO address
        state.config.addresses[AppType.AddressConfig.FEE_WALLET] = msg.sender; // TODO: Change to Fee Wallet address
        state.config.uints[AppType.UintConfig.CHAIN_ID] = 0x13881; // TODO: change to real chain id
        state.config.strings[
            AppType.StringConfig.APP_NAME
        ] = "Protoverse Genesis Collection";

        if (state.config.uints[AppType.UintConfig.CHAIN_ID] == 0x89) {
            state.tierSwapAmounts[1][address(0)] = 448572335400000028672; // 0.2ETH
            state.tierSwapAmounts[2][address(0)] = 336429251549999988736; // 0.15ETH
            state.tierSwapAmounts[3][address(0)] = 179428934159999991808; // 0.08ETH
            state.config.uints[AppType.UintConfig.RAND_LINK_FEE] =
                0.0001 *
                10**18; // 0.0001 LINK
        } else if (state.config.uints[AppType.UintConfig.CHAIN_ID] == 0x38) {
            state.tierSwapAmounts[1][address(0)] = 1509527000000000000; // 0.2ETH
            state.tierSwapAmounts[2][address(0)] = 1132145250000000000; // 0.15ETH
            state.tierSwapAmounts[3][address(0)] = 603810800000000000; // 0.08ETH
        } else if (state.config.uints[AppType.UintConfig.CHAIN_ID] == 0x1) {
            state.tierSwapAmounts[1][address(0)] = 200000000000000000; // 0.2ETH
            state.tierSwapAmounts[2][address(0)] = 150000000000000000; // 0.15ETH
            state.tierSwapAmounts[3][address(0)] = 80000000000000000; // 0.08ETH
        } else if (
            state.config.uints[AppType.UintConfig.CHAIN_ID] == 0x13881 ||
            state.config.uints[AppType.UintConfig.CHAIN_ID] == 0x4 ||
            state.config.uints[AppType.UintConfig.CHAIN_ID] == 0x61 // testnets
        ) {
            state
                .rand
            ...
            state
                .rand
                .linkVRFAddress = 0x3d2341ADb2D31f1c5530cDC622016af293177AE0;
            state
                .rand
                .linkVRFKeyHash = 0xf86195cf7690c55907b2b611ebb7343a6f649bff128701cc542f0569e2c549da;

            state.config.uints[AppType.UintConfig.RAND_LINK_FEE] =
                0.0001 *
                10**18; // 0.0001 LINK
            state.tierSwapAmounts[1][address(0)] = 20000000000000000; // 0.02ETH
            state.tierSwapAmounts[2][address(0)] = 15000000000000000; // 0.015ETH
            state.tierSwapAmounts[3][address(0)] = 8000000000000000; // 0.008ETH
        }
...
library BatchFactory {
    ...
    function createBatch(
        ...
        emit BatchCreated(newBatchId, kind, isOpenAt, disabled, root);
    }
...
library Rand {
    ...
    event ReceivedRandomness(bytes32 _requestId, uint256 _randomness);

    function requestRand(AppType.State storage state, bytes32 _requestId)
        public
    {
        state.rand.lastRefreshBlock = block.number;
        state.rand.requests[_requestId] = uint256(
            keccak256(abi.encodePacked(_requestId))
        );
        emit RequestedRandomness(_requestId);
    }

    function receivedRand(
        AppType.State storage state,
        bytes32 requestId,
        uint256 randomness
    ) public {
        uint256 requestHash = uint256(keccak256(abi.encodePacked(requestId)));
        require(
            state.rand.requests[requestId] == requestHash,
            "Invalid requestId"
        );
        state.rand.latest = randomness;
        state.rand.requests[requestId] = randomness;

        emit ReceivedRandomness(requestId, randomness);
    }

    function shouldRefreshRand(AppType.State storage state)
        public
        view
        returns (bool)
    {
        return
            (state.rand.lastRefreshBlock +
                state.config.uints[AppType.UintConfig.RAND_REFRESH_PERIOD]) <
            block.number;
    }
}
