  ...
  MathFunctions
{
  ...
  address internal stableTokenAddress;
    ...
    nonReentrant
  {
    ...
    uint256 price = tierPrice[_tier] * _quantity * (10 ** decimals);

    if(stableToken.balanceOf(_msgSender()) < price) {
      revert InsufficientFee();
    }

    bool paid = stableToken.transferFrom(
      _msgSender(),
      paymentAddress,
      price
    );
  ...
  function mint(
    ...
    payable
  {
    mintTo(_msgSender(), _tier, _quantity);
  }

  function mintContender(
    address _to,
    uint256 _quantity
  )
    external
  ...
  */
  ...
  function setStableTokenAddress(
    address _stableTokenAddress
  )
    external
    onlyRole(DEFAULT_ADMIN_ROLE)
