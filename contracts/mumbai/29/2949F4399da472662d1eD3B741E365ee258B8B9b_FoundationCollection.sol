...
function upgradeCardCost(uint160[] calldata _tokenIds, uint8 _targetCardType, uint8 _collectionId, Coupon memory _coupon, uint _discount) public view validCardType(_targetCardType) returns (uint256 upgradeFeeCostWithDiscount, uint256 partnerFeeCost, uint256 cardsTypeValueDiffCost, uint256 payValue) {
    ...
    if(upgradecost.upgradeFeeCostWithDiscount > cardsTypeValueDiffCost){
      ...
      payValue = cardsTypeValueDiffCost - upgradecost.upgradeFeeCostWithDiscount;
      upgradecost.upgradeFeeCostWithDiscount = 0;
    }

    return (upgradeFeeCostWithDiscount, partnerFeeCost, cardsTypeValueDiffCost, payValue);
}
...
/**
  ...
  function upgradeCards(uint160[] calldata _tokenIds, uint8 _targetCardType, uint8 _collectionId,  Coupon memory _coupon, uint _discount, uint256 _value) public  validCardType(_targetCardType) {
    ...
    (uint256 upgradeFeeCost, uint256 partnerFeeCost, uint256 cardsTypeValueDiffCost, uint256 payValue) = upgradeCardCost(_tokenIds, _targetCardType, _collectionId, _coupon, _discount);
    ...
    
    {
      ...
      // If user sent more funds than required, refund excess
      if(_value > payValue) {
        bool success =  eft.transferFrom(gameTreasury, msg.sender, _value - payValue);
...
function claimUpgradeCommission(uint256 cardsTypeValueDiffCost, uint256 upgradeFeeWithDiscount) private {

      CommissionInfo memory commissioninfo;

      commissioninfo.upgradeCommission = cardsTypeValueDiffCost + upgradeFeeWithDiscount;
