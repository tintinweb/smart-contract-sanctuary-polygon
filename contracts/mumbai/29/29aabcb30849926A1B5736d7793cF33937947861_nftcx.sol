...
contract nftcx is Ownable {
    ...
    struct userInfoItem {
        ...
        uint256 allReward; //所有已发放奖励
        uint256 allPlatformReward; //所有已领取的平台奖励
        uint256 allGoldNum; //所有金卡数量
        uint256 allSilverNum; //所有银卡数量
        uint256 allBronzeNum; //所有铜卡数量
        uint256 idoAmount; //铜卡购买次数
    ...
    function setShareToken(IERC20 _token) external onlyOwner {
        shareToken = _token;
    }
    ...
    function setShare() external {
        ...
        uint256 _shareAmount = getLeftToken(shareToken);
        uint256 _shareNumber = BronzeSet.length();
        if (_shareAmount > 0 && _shareNumber > 0) {
    ...
    function claimById(uint256 _shareId) public returns (uint256){
        ...
        if ((toSilverTimeList[msg.sender] == 0 || toSilverTimeList[msg.sender] > shareList[_shareId]._createTime) && !claimStatusList[msg.sender][_shareId]) {
            shareToken.transfer(msg.sender, _claimAmount);
    ...
    function getUserInfoForShare(address _user, uint256 _shareId) public view returns (userInfoItem2 memory _userInfo) {
        ...
        _userInfo._inWhiteList = toSilverTimeList[msg.sender] == 0 || toSilverTimeList[msg.sender] > shareList[_shareId]._createTime;
