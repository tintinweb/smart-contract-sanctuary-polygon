...
library FractionalAmount {
    ...
    function split(uint128 numerator, uint128 denominator, uint256 inputAmount) internal pure returns (uint256 numeratorAmount, uint256 denominatorAmount) {
        if (numerator == 0) {
            return (0, inputAmount);
        }
        unchecked {
            numeratorAmount = (inputAmount * numerator) / denominator;
