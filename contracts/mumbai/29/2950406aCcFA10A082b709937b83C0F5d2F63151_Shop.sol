...
contract Shop is ReentrancyGuard, AccessControl {
  struct ProductOrder {
    uint256 productId;
    uint256 amount;
  }
  ...
  modifier hasAccess(bytes32 access) {
    require(hasRole(access, msg.sender), "UNAUTHORIZED_ACCESS");
    _;
  }

  event Purchase(address indexed buyer, ProductOrder[] orders);
  event ExternalPurchase(string indexed hash, ProductOrder[] orders);
  event OwnershipManagementTransferred(
    address indexed previousManager,
    address indexed newManager
  );
  event Withdrawal(address indexed owner, uint256 balance);

  IOwnershipManagement public ownershipManagement;

  uint256 public immutable GENESIS_BLOCK;

  constructor(address ownershipManagementAddress) {
    GENESIS_BLOCK = block.number;
    _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
    _setupRole(SHOP_MANAGER, msg.sender);
    setOwnershipManagement(ownershipManagementAddress);
  }

  function setOwnershipManagement(address addr) public hasAccess(SHOP_MANAGER) {
    address previous = address(ownershipManagement);
    ownershipManagement = IOwnershipManagement(addr);
    emit OwnershipManagementTransferred(previous, addr);
  }
  ...
  function purchase(
    ...
    emit Purchase(msg.sender, productOrders);
  }
  ...
  function externallyPaidOrder(
  ...
  ) external hasAccess(PAYMENT_PROCESSOR) nonReentrant {
    ICollection collection = ICollection(collectionAddress);
    ...
    emit ExternalPurchase(ownerHash, productOrders);
  }
  ...
  function withdraw() external hasAccess(FUNDS_MANAGER) {
    uint256 balance = address(this).balance;
    payable(msg.sender).transfer(balance);
    emit Withdrawal(msg.sender, balance);
  }

  function withdrawToken(address token) external hasAccess(FUNDS_MANAGER) {
...
interface IOwnershipManagement {
  ...
  function sendAll(address to, string memory hash) external;
...
interface ICollection {
  ...
  function getProducts(
    uint256 startProductId,
    uint256 endProductId
