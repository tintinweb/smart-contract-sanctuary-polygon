...
contract Escrow is ERC721Holder, ERC1155Receiver, ERC1155Holder {
    ...
    function lend(LendData memory lendData) external {
        ...
        lendData.lendId = generateId(lendData.lender, lendData.tokenId, lendData.lendingQuantity, lendData.nftAddress);
    ...
    function rent(bytes32 lendId, uint256 qty, uint256 duration) external {
        ...
        bytes32 rentId = (generateId(msg.sender, lendingDetails[lendId].tokenId, qty, lendingDetails[lendId].nftAddress));
    ...
    function claim(bytes32 rentalId) external {
        isExpired(rentalId);
    ...
    function regain(bytes32 lendId) external {
    ...
    function generateId(address account, uint256 tokenId, uint256 qty, address nftAddress) internal view returns(bytes32 meomory){
        return keccak256(abi.encodePacked(account, tokenId, qty, nftAddress, block.timestamp));
    }
