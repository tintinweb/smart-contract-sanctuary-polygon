...
contract L2Portal is IPortal, ILayerZeroReceiver, ILayerZeroUserApplicationConfig, Auth {

    // IERC20 constant POW = IERC20(); // L2 mainnet address
    // ILayerZeroEndpoint constant ENDPOINT = ILayerZeroEndpoint(); // mainnet address
    IERC20 POW;
    ...
    event SourcePortalSet(address portal);
    event SourceChainIdSet(uint256 chainId);
    event FundsWithdrawn(address to, uint256 amount);

    // Temporarily taking POW address as parameter so local tests always pass
    constructor(address _pow, address _endpoint) {
        POW = IERC20(_pow);
        ENDPOINT = ILayerZeroEndpoint(_endpoint);
    }

    function setSourcePortal(address _portal) external onlyAuth {
        sourcePortal = _portal;
        emit SourcePortalSet(_portal);
    }

    function setSourceChainId(uint16 _id) external onlyAuth {
        sourceChainId = _id;
        emit SourceChainIdSet(_id);
    }

    function withdrawFunds(address to) external onlyAuth {
        uint256 amount = address(this).balance;
        (bool sent, ) = to.call{value: amount}("");
        if (!sent) revert FailedToSendFunds();
    ...
    function bridge(
        ...
        // Burn L2 POW from `msg.sender`
        POW.burn(msg.sender, _amountToBridge);
        ...
        (uint messageFee, ) = ENDPOINT.estimateFees(sourceChainId, address(this), payload, false, adapterParams);
        ...
        ENDPOINT.send{value: messageFee}( // {value: messageFee} will be paid out of this contract!
            ...
            abi.encodePacked(sourcePortal), // destination address of Portal
        ...
        emit Bridged(sourceChainId, msg.sender, _amountToBridge);
    }
