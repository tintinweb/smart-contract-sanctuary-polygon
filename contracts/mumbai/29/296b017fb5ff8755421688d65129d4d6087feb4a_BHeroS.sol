...
contract BHeroS is Initializable, AccessControlUpgradeable, UUPSUpgradeable {
  ...
  event Fusion(
    ...
    uint256[] mainListIdHero,
    uint256[] secondListIdHero,
    uint256 rarityMain,
  ...
  function fusion(uint256[] calldata mainListIdHero, uint256[] calldata secondListIdHero) external {
    ...
    (rarityMain, rarityTarget, dropRateOption) = _calculatorFusion(mainListIdHero, secondListIdHero);
    uint256 requestCategory = 0;
    uint256 isHeroS = 1;
    uint256 heroCount = 1;
    ...
    //add data fusion
    bool isFusion = true;
    bHeroToken.addFusionData(to, rarityTarget, isFusion);

    //burn list hero
    bHeroToken.burn(mainListIdHero);
    if (secondListIdHero.length > 0) {
      bHeroToken.burn(secondListIdHero);
    }

    emit Fusion(to, mainListIdHero, secondListIdHero, rarityMain, rarityTarget);
  }

  function _calculatorFusion(uint256[] calldata mainListIdHero, uint256[] calldata secondListIdHero)
    internal
    view
    returns (
      uint256,
...
contract BHeroToken is ERC721Upgradeable, AccessControlUpgradeable, PausableUpgradeable, UUPSUpgradeable {
  ...
  function processTokenRequests() external {
    ...
    for (uint256 i = requests.length; i > 0; --i) {
      ...
      if (fusionData[to][i - 1].isFusion) {
        uint256 rarityNewHero = BHeroDetails.decodeRarity(details[0]);
        if (fusionData[to][i - 1].rarityTarget == rarityNewHero) {
