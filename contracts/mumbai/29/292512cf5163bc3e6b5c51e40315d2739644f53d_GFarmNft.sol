...
contract GFarmNft is ERC721, IChildToken, AccessControlMixin, NativeMetaTransaction, ContextMixin {
    ...
    constructor(string memory name_, string memory symbol_, address childChainManager) public ERC721(name_, symbol_) {
    ...
    function withdrawBatch(uint256[] calldata tokenIds) external {
        ...
        for (uint256 i; i < length; i++) {
            ...
            require(
                _msgSender() == ownerOf(tokenId), string(abi.encodePacked("ChildERC721: INVALID_TOKEN_OWNER ", tokenId))
            );
            _burn(tokenId);
        }
