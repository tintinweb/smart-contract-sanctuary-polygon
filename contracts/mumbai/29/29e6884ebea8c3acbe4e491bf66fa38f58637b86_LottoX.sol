...
contract LottoX is Ownable{
  ...
  mapping (address => Winner) private _winnerRecords;
  ...
  function _loopBet(uint256 id, uint256 counter) internal{
    if(counter < _lastBetID){
      ...
      _winnerRecords[winnerAddr].bal = _winnerRecords[winnerAddr].bal.add(betWin);
  ...
  function checkMyWonBalance() public view returns (uint256) {
    return _winnerRecords[msg.sender].bal;
  }
  ...
  function withdrawMyToken() public returns(bool){
    uint256 winAmt = _winnerRecords[msg.sender].bal;
    require(winAmt > 0, "Insuffient amount");
    ERC20(_token).transfer(msg.sender, winAmt);
    _winnerRecords[msg.sender].bal = _winnerRecords[msg.sender].bal.sub(winAmt);
    return true;
  }
