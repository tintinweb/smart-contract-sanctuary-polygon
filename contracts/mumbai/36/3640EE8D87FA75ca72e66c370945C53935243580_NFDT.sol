...
import "./AgentStore.sol";

using Address for address;
using Strings for uint256;

contract NFDT is AgentStore, IERC721, IERC721Metadata, IERC721Enumerable, ERC165 {
    ...
    function handleTransfer(address from, address to, uint256 tokenId) internal {
        emit Transfer(owner(), customer(tokenId), tokenId);

        if (to.isContract()) {
            bytes memory data = abi.encodePacked(fileNumber(tokenId));
    ...
    function afterBeginHook(uint256 _tokenId) internal override {
        super.afterBeginHook(_tokenId);
        handleTransfer(owner(), customer(_tokenId), _tokenId);
    }

    function afterEndHook(uint256 _tokenId) internal override {
        super.afterEndHook(_tokenId);
        handleTransfer(customer(_tokenId), owner(), _tokenId);
    }
    ...
    function ownerOf(uint256 _tokenId) public view returns (address) {
        return isCovenantLegallyEnded(_tokenId) ? owner() : customer(_tokenId);
    }
    ...
    function safeTransferFrom(address _from, address _to, uint256 _tokenId, bytes calldata data) external {
        if (data.length == 0) return safeTransferFrom(_from, _to, _tokenId);
        ...
        require(isCovenantAdministrativelyEnded(_tokenId), "Only completed convenants can sent with file number");

        if (isCovenantAdministrativelyEnded(_tokenId)) {
            require(_from == owner(), "Only the owner can send this token");
            begin(_tokenId, File(abi.decode(data, (uint)), _to));
        } else {
            // never
        }
    }
    ...
    function safeTransferFrom(address _from, address _to, uint256 _tokenId) public {
        ...
        require(!isCovenantAdministrativelyEnded(_tokenId), "Only convenants at the customer can sent without file number");

        if (isCovenantAdministrativelyEnded(_tokenId)) {
            // never
        } else {
            require(_to == owner(), "This token can be sent only to the owner");
            end(_tokenId);
        }
    }
    ...
    function transferFrom(address _from, address _to, uint256 _tokenId) external {
        if (isCovenantAdministrativelyEnded(_tokenId)) {
            revert("Without data only sending to the owner is supported");
        } else {
            require(_from == customer(_tokenId), "Convenant not ended yet, can be sent only by the customer");
            require(_to == owner(), "Convenant not ended yet, can be sent only back to the owner");
            end(_tokenId);
        }
    }
    ...
    function approve(address _approved, uint256 _tokenId) external pure {
        revert("Approvals are not supported");
    }
    ...
    function tokenURI(uint256 _tokenId) external view returns (string memory) {
        return string.concat(
            "https://nft-metadata-hosting.web.app/metadata.json",
            "?name=Non%20Fungible%20DeFi%20Trust%20%23", _tokenId.toString(),
            "&image=https%3A%2F%2Fimg.shields.io%2Fbadge%2FNFDT%2523", _tokenId.toString(), "-", versionNumber(_tokenId).toString(), "-", isCovenantAdministrativelyEnded(_tokenId) ? "red" : "green"
        );
    }
...
struct AgentStoreDump {
    uint256[] expirationQueue;
    AgentDump[COUNT] agents;
}
...
contract AgentStore is Ownable {
    ...
    function begin(uint256 _tokenId, File memory _file) public {
    ...
    function afterEnd(uint256 _tokenId) internal {
        ...
        afterEndHook(_tokenId);
    }

    function afterEndHook(uint256 _tokenId) internal virtual {} 

    function afterBegin(uint256 _tokenId) internal {
        expirationQueue.push(_tokenId);
        afterBeginHook(_tokenId);
    }

    function afterBeginHook(uint256 _tokenId) internal virtual {}

    function isCovenantLegallyEnded(uint256 _tokenId) public view returns (bool) {
        return agents[_tokenId].isCovenantLegallyEnded();
    }

    function isCovenantAdministrativelyEnded(uint256 _tokenId) public view returns (bool) {
        return agents[_tokenId].isCovenantAdministrativelyEnded();
    }

    function versionNumber(uint256 _tokenId) public view returns (uint256) {
        return agents[_tokenId].versionNumber();
    }

    function fileNumber(uint256 _tokenId) public view returns (uint) {
        return agents[_tokenId].fileNumber();
    }

    function customer(uint256 _tokenId) public view returns (address) {
        return agents[_tokenId].customer();
    }

    function expiration(uint256 _tokenId) public view returns (uint) {
        return agents[_tokenId].expiration();
    }

    function dump() public view returns (AgentStoreDump memory) {
        AgentDump[COUNT] memory result;
        for (uint256 i = 0; i < COUNT; i++) {
            result[i] = agents[i].dump();
        }
        return AgentStoreDump(expirationQueue.dump(), result);
    }
...
string constant NAME = "Non Fungible DeFi Trust";
string constant SYMBOL = "NFDT";
string constant URI_BASE = "https://nft-metadata-hosting.web.app/metadata.json?name=My%20NFT&image=https%3A%2F%2Fsource.boringavatars.com%2Fpixel%2F120%2FMy%2520NFT";
