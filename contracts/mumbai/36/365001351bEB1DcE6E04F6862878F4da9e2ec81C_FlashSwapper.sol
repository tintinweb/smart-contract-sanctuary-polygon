...
// File: flashliquidity-swapper/libraries/SafeMathCopy.sol
...
// File: flashliquidity-swapper/@openzeppelin/contracts/utils/math/SafeMath.sol
...
// File: flashliquidity-swapper/@openzeppelin/contracts/utils/Address.sol
...
// File: flashliquidity-swapper/@openzeppelin/contracts/token/ERC20/IERC20.sol
...
// File: flashliquidity-swapper/@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol
...
contract FlashSwapper is KeeperCompatibleInterface {
    ...
    AggregatorV3Interface internal wethPriceFeed; // in USD
    ...
    function withdraw() internal {
        require(flashSwapFarm != address(0), 'revenue distribution contract not set');
        uint256 balance;
        balance = IERC20(rewardToken).balanceOf(address(this));
        if (balance > 0) {
            uint256 fee = balance.div(50); // 2%
            balance = balance.sub(fee);
            IERC20(rewardToken).transfer(feeTo, fee);
            IERC20(rewardToken).transfer(flashSwapFarm, balance);
            emit Withdrawn(flashSwapFarm, balance);
        }
    }
