...
contract NFTBoxWhitelist is INFTBox, AccessControl {
    ...
    constructor(address _admin, address _manager) {
        ...
        boxConfig.usingERC721WhiteList = 1;
        boxConfig.usingERC1155WhiteList = 1;
        boxConfig.NFTTokenCount = 5;
        boxConfig.ERC20TokenCount = 5;
    }

    /**
        SetboxConfig(usingERC721WhiteList, usingERC1155WhiteList, NFTTokenCount, ERC20TokenCount)
     */

    function setUsingERC721Whitelist(uint256 usingList) external  onlyRole(ADMIN) {
        boxConfig.usingERC721WhiteList = uint8(usingList);
    }

    function setUsingERC1155Whitelist(uint256 usingList) external  onlyRole(ADMIN) {
        boxConfig.usingERC1155WhiteList = uint8(usingList);
    }

    function setNFTTokenCount(uint256 limitTokenCount) external  onlyRole(ADMIN) {
        boxConfig.NFTTokenCount = uint8(limitTokenCount);
    }

    function setERC20TokenCount(uint256 limitERC20Count) external  onlyRole(ADMIN) {
        boxConfig.ERC20TokenCount = uint8(limitERC20Count);
    }
    /**
    Getting boxConfig((usingERC721WhiteList, usingERC1155WhiteList, NFTTokenCount, ERC20TokenCount))
    */
    function getBoxConfig() external view returns(BoxConfig memory) {
        return boxConfig;
    }
    /**
    ...
    function _checkAssets(
        ...
        require(
            erc20Details.length <= boxConfig.ERC20TokenCount,
    ...
    */
    ...
    function validateWhiteListERC721Token(address erc721Token)
        ...
        returns (bool)
    {
        if (boxConfig.usingERC721WhiteList == 0) return true;
    ...
    function validateWhiteListERC1155Token(address erc1155Token)
        ...
        returns (bool)
    {
        if (boxConfig.usingERC1155WhiteList == 0) return true;
