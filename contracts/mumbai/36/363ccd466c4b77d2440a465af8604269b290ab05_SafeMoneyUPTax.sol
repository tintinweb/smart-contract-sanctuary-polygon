...
contract SafeMoneyUPTax is ERC20 {
    ...
    constructor() ERC20("SafeMoney UPTax", "SMUT") {
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        _tokenTransfer(_msgSender(), recipient, amount, !_isExcludedFromFee[_msgSender()]);
        return true;
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
      ...
      function calculateTaxFee(uint256 _amount) private view returns (uint256) {
        return (_amount * _taxFee) / 100; // 2% fee
    }

    function excludeFromFee(address account) public {
        require(_msgSender() == admin, "Only admin can exclude from fee");
        _isExcludedFromFee[account] = true;
    }

    function includeInFee(address account) public {
        require(_msgSender() == admin, "Only admin can include in fee");
        _isExcludedFromFee[account] = false;
    }

    function excludeFromReward(address account) public {
        require(_msgSender() == admin, "Only admin can exclude from reward");
        _isExcludedFromRewards[account] = true;
    }

    function includeInReward(address account) public {
        require(_msgSender() == admin, "Only admin can include in reward");
        _isExcludedFromRewards[account] = false;
    }

    function setAdminAddress(address _adminAddress) public {
        require(_msgSender() == admin, "Only admin can set the admin address");
