...
contract RealEstate {
    ...
    event PropertySold(uint256 indexed id , address oldOwner, address indexed newOwner , uint256 price);
    ...
    if(properties[i + 1].owner == user){
        uint256 currentId = i + 1;
        Property storage cuurentItem = properties[currentId];

        items[currentIndex] = cuurentItem;
