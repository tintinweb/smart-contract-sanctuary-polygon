...
contract Marketplace is BasicMetaTransaction, Ownable, ReentrancyGuard {
    ...
    event DoTransfer(uint256 _tokenId);
    ...
    // Status of an offer, calculated using the start date, the duration and the current timeblock
    enum Status {
        pending,
        active,
        finished
    ...
    function createOffer(
        ...
        require(asset.ownerOf(_tokenId) == _msgSender(), "Not the owner");
        require(
            asset.getApproved(_tokenId) == address(this),
            "NFT not approved"
        );
        // Could not be used to update an existing offer (#02)
        Offer memory previous = offers[
            keccak256(abi.encodePacked(_assetAddress, _tokenId))
        ];
        require(previous.isOnSale == false, "An active offer already exists");
        ...
        offers[keccak256(abi.encodePacked(_assetAddress, _tokenId))] = offer;
        emit OfferCreated(_tokenId, _msgSender(), _assetAddress, _price);
    ...
    function removeFromSale(address _assetAddress, uint256 _tokenId) public {
        Offer memory offer = offers[
            keccak256(abi.encodePacked(_assetAddress, _tokenId))
        ];
        require(_msgSender() == offer.creator, "You are not the owner");
    ...
    function getOffer(address _assetAddress, uint256 _tokenId)
        public
        view
        returns (Offer memory)
    {
        return offers[keccak256(abi.encodePacked(_assetAddress, _tokenId))];
    }
    ...
    function buy(address _assetAddress, uint256 _tokenId)
        external
        payable
        nonReentrant
    {
        ...
        require(getStatus(_assetAddress, _tokenId) == Status.active, "Offer expired");

        emit Claim(_tokenId, buyer);
        ERC721 asset = ERC721(offer.assetAddress);
        asset.safeTransferFrom(offer.creator, buyer, _tokenId);

        uint256 commissionToPay = (paidPrice.mul(commission)) / 10000;
        uint256 amountToPay = paidPrice.sub(commissionToPay);

        (bool success, ) = offer.creator.call{value: amountToPay}("");
    ...
    function isActive(address _assetAddress, uint256 _tokenId)
        public
        view
        returns (bool)
    {
        return getStatus(_assetAddress, _tokenId) == Status.active;
    }

    function isFinished(address _assetAddress, uint256 _tokenId)
        public
        view
        returns (bool)
    {
        return getStatus(_assetAddress, _tokenId) == Status.finished;
    }
    ...
    function getStatus(address _assetAddress, uint256 _tokenId)
        public
        view
        returns (Status)
    {
        Offer storage offer = offers[
            keccak256(abi.encodePacked(_assetAddress, _tokenId))
        ];
    ...
    function endDate(address _assetAddress, uint256 _tokenId)
        public
        view
        returns (uint256)
    {
    ...
    function extractBalance() public nonReentrant onlyOwner {
        address payable me = payable(_msgSender());
        (bool success, ) = me.call{value: accumulatedCommission}("");
        require(success, "Transfer failed.");
        accumulatedCommission = 0;
    }

     function transferNFT(address _assetAddress, uint256 _tokenId, address _receiver) public  {
         Offer storage offer = offers[
            keccak256(abi.encodePacked(_assetAddress, _tokenId))
        ];
        ERC721 asset = ERC721(offer.assetAddress);
        asset.safeTransferFrom(offer.creator, _receiver, _tokenId);
        emit DoTransfer(_tokenId);
    }
}
