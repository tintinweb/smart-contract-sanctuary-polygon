...
contract Cafe is Ownable, ERC721URIStorage, ReentrancyGuard {
    ...
    function mintNft(string memory tokenURI) public onlyOwner returns (uint) {
        require(bytes(tokenURI).length > 0, "The TokenURI cannot be empty");
        _tokenIds.increment();
    ...
    function purchaseNft(uint tokenId) public {
        require(
            IERC20(cafeToken).balanceOf(msg.sender) >= 50000000000000000000,
            "you should have more then 50 tokens..."
        );
        IERC20(cafeToken).transferFrom(
            msg.sender,
            address(this),
