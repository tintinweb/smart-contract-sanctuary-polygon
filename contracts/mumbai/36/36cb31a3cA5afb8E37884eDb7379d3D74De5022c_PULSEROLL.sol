...
contract PULSEROLL is VRFConsumerBase {
    ...
    mapping(bytes32 => playersVars)public playerValues;
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal payoutsAreActive override 
    {
        ...
        if ( newPlayerVar.playerDieResult == 0 || bytes32(randomness).length == 0 ) 
        {   
            /**
    ...
    function CheckRange(bytes32 requestId,playersVars storage newPlayerVar)internal view  returns(bool){
    ...
    function TransferProfit(bytes32 requestId,playersVars storage newPlayerVar)internal{
    ...
    function SetRange(bytes32 requestId,playersVars storage newPlayerVar)internal 
    {
        ...
        {
            bool check=CheckRange(requestId,newPlayerVar );
            if(check)
            {
             TransferProfit(requestId,newPlayerVar);
            }
    ...
    function DistributeLoss(bytes32 requestId,playersVars storage newPlayerVar )internal 
    {
        emit LogResult(
            requestId,
            newPlayerVar.playerAddress,
