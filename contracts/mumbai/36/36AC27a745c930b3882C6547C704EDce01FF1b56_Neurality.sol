...
contract Neurality is ERC721Enumerable, Ownable {
    ...
    function setWhitelistMerkleRoot(string memory merkleRoot) public onlyOwner {
        bytes32 _whitelistMerkleRoot;
        bytes memory tempEmptyStringTest = bytes(merkleRoot);

        require(
            tempEmptyStringTest.length == 0,
            "The argument shouldn't be empty string"
        );

        assembly {
            _whitelistMerkleRoot := mload(add(merkleRoot, 32))
        }
    ...
    function setMintType(string memory _mintType) external onlyOwner {
        if (
            ...
            setPricePerNft(0.07 ether);
        } else if (
            keccak256(abi.encodePacked((_mintType))) ==
            keccak256(abi.encodePacked(("wl2")))
        ) {
            mintType = _mintType;
            setMintAmountPerMintType(6);
    ...
    function privateMint(bytes32[] calldata _merkleProof) public payable {
        ...
        if (
            ...
            //  2nd private sale
            require(totalSupply() <= 11, "2nd private sale is finished.");
        }
