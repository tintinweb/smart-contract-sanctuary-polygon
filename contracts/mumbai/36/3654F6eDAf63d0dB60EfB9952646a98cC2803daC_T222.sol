...
contract T222 is Ownable, TimeForDay, SortList {
    ...
    mapping(uint256 => EnumerableSet.UintSet) private Day_Top20InviteNum; //记录每天的推荐数前20名
    //用于处理日大单排行榜
    mapping(uint256 => mapping(uint256 => EnumerableSet.AddressSet)) private Day_InVestNum_AddressList; //记录每天每个推荐数的前20个地址
    mapping(uint256 => EnumerableSet.UintSet) private Day_Top10InVestNum; //记录每天的推荐数前20名
    ...
    function updateInvestBangList(uint256 _day, address _user, uint256 _amount) private {
        ...
        if (Day_Top10InVestNum[_day].length() < _takeNum) {
            Day_Top10InVestNum[_day].add(_inviteNum1);
            //Day_Top10InVestNum[_day] = sortUint256List(Day_Top10InVestNum[_day]);
            if (_num < _takeNum) {
                Day_InVestNum_AddressList[_day][_inviteNum1].add(_user);
            }
        } else {
            uint256[] memory x = Day_Top10InVestNum[_day].values();
            x = sortUint256List(x);
            uint256 _min = x[_takeNum - 1];
            //排行榜满20人,如果推荐数大于最小数,则上榜，并更新拥有同一推荐数的地址
            if (_inviteNum1 > _min) {
                Day_Top10InVestNum[_day].remove(_min);
                Day_Top10InVestNum[_day].add(_inviteNum1);
    ...
    function getDaySets(uint256 _type, uint256 _timestamp, uint256 _amount) public view returns (address[] memory _setList, uint256 _day) {
        if (_timestamp == 0) {
    ...
    function getDayTop(uint256 _timestamp, uint256 _type) public view returns (uint256[] memory _List, uint256 _day) {
        if (_timestamp == 0) {
            _timestamp = block.timestamp;
        }
        _day = getYearMonthDay(_timestamp);
