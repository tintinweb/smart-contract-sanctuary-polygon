...
contract CampaignFactory {
    ...
    event GetLengthDeployedCampaign(uint256 length);
    ...
    function getLengthDeployedCampaign() public returns(uint256) {
        uint256 length = deployedCampaign.length;
        emit GetLengthDeployedCampaign(length);
        return length;
    }
}
