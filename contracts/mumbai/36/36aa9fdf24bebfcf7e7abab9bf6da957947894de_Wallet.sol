...
// File: contracts/WalletTemplate.sol
...
contract WalletTemplate {
    ...
    function setNftCreator(address creator)
        external
        virtual
    {
        nftCreator = creator;
    ...
    function deposit(
        ...
        return _assetPrincipalBalance[assetToken].add(assetAmount);
    }
    ...
    function withdraw(
        ...
        // Track Principal
        _assetPrincipalBalance[assetToken] = _assetPrincipalBalance[assetToken]
            .sub(receiverAmount);
...
contract Wallet {
    ...
    event NewSmartWallet(address, uint256, address, address);

    constructor() {
        _walletTemplate = address(new WalletTemplate());
    }

    // get wallet by id otherwise create a new one, return wallet address
    function getWalletAddressById(
        ...
        if (wallet == address(0x0)) {
            ...
            if (creator != address(0x0)) {
                WalletTemplate(wallet).setNftCreator(creator);
            }

            emit NewSmartWallet(contractAddress, tokenId, wallet, creator);
        }
    ...
    function _createWallet() internal returns (address) {
        return _createClone(_walletTemplate);
    }
