...
contract DutchAuction is Ownable, IDutchAuction {
  ...
  uint256 mintStartPrice;
  uint256 mintEndPrice;
  uint256 mintSliceDuration;
  uint256 mintSliceAmount;

  function earlyMint() payable public {
    ...
    require(earlyAccess[msg.sender] == true, 'You need to have early access to use early mint');
    ...
    require(mintCount <= mintMaximum, 'Early mint has ended');
    require(earlyMintPrice == msg.value, 'The message value is not the same as early mint pricing');

    IIncarnateEcho(_echo).mint(msg.sender, 1);
  ...
  function mint(uint256 quantity) payable public {
    (,,uint256 price,,,uint256 remaining) = getMintInformation(block.timestamp);
    ...
    require(msg.value >= price, 'The message value is not enough');
  ...
  function mintFor(address to, uint256 quantity) public onlyOwner {
    (,,,,,uint256 remaining) = getMintInformation(block.timestamp);
  ...
  function getMintInformation(uint time) view public returns (uint start, uint end, uint256 currentPrice, uint256 nextPrice, uint256 validUntil, uint256 remaining) {
    require(_echo != address(0), 'Mint is not enabled');
    require(mintStart > 0, 'Mint is not enabled');
    require(mintStartPrice > 0, 'Mint is not enabled');
    require(mintEndPrice > 0, 'Mint is not enabled');
    require(mintSliceDuration > 0, 'Mint is not enabled');
    require(mintSliceAmount > 0, 'Mint is not enabled');
    ...
    if (time < mintStart) {
      return (mintStart, mintEnd, mintStartPrice, mintStartPrice - mintSliceAmount, mintStart + mintSliceDuration, remaining);
    }

    if (mintEnd < time) {
      return (mintStart, mintEnd, mintEndPrice, mintEndPrice, mintEnd, 0);
    }
    
    uint256 slicedSteps = (time - mintStart) / mintSliceDuration;
    uint until = mintStart + (slicedSteps * mintSliceDuration) + mintSliceDuration;
    if (until > mintEnd) until = mintEnd;

    uint256 slicedAmount = slicedSteps * mintSliceAmount;
    if (slicedAmount > (mintStartPrice - mintEndPrice)) 
    { 
      slicedAmount = mintStartPrice - mintEndPrice;
      until = mintEnd;
    }

    uint256 nextSlicedAmount = (slicedSteps + 1) * mintSliceAmount;
    if (nextSlicedAmount > (mintStartPrice - mintEndPrice)) 
    { 
      nextSlicedAmount = mintStartPrice - mintEndPrice;
    }
    return (mintStart, mintEnd, mintStartPrice - slicedAmount, mintStartPrice - nextSlicedAmount, until, remaining);
  }

  function enableMint(uint start, uint end, uint256 startPrice, uint256 endPrice, uint sliceDuration, uint256 sliceAmount) onlyOwner public {
    mintStart = start;
    mintEnd = end;
    mintStartPrice = startPrice;
    mintEndPrice = endPrice;
    mintSliceDuration = sliceDuration;
    mintSliceAmount = sliceAmount;
  }
...
interface IDutchAuction {
  ...
  function earlyMint() payable external;
  ...
  function enableMint(uint start, uint end, uint256 startPrice, uint256 endPrice, uint sliceDuration, uint256 sliceAmount) external;
  ...
  function getMintInformation(uint time) view external returns (uint start, uint end, uint256 currentPrice, uint256 nextPrice, uint256 validUntil, uint256 remaining);
