...
contract PostCommentFacet is Modifiers {
    ...
    function addPost(
        ...
        ReputationProof memory unirepProof
    ) external {
        require(groupId >= 0 && groupId < s.groupCount, "Invalid groupId");
        ...
        s.semaphore.verifyProof(groupId, merkleTreeRoot, contentCID, nullifierHash, externalNullifierHash, semaphoreProof);

        //Qualifying process for posting(checking if it has enough reputation for posting)
        if(s.repRequirement.post > 0) {
            Unirep.ReputationSignals memory signals = s.unirep.decodeReputationSignals(unirepProof.publicSignalsQ);
            require(signals.attesterId == uint256(uint160(s.attester)));
            require(signals.minRep == s.repRequirement.post, "Not enough reputations");
            s.unirep.verifyReputationProof(unirepProof.publicSignalsQ, unirepProof.proofQ);
        }

        //
        s.unirep.verifyEpochKeyProof(unirepProof.publicSignals, unirepProof.proof);
        Unirep.EpochKeySignals memory signals2 = s.unirep.decodeEpochKeySignals(unirepProof.publicSignals);

        uint256[] memory childIds;
        s.items[s.itemCount] = Item(ItemKind.POST, s.itemCount, 0, groupId, block.number, childIds, 0, 0, note, unirepProof.ownerEpoch, unirepProof.ownerEpochKey, contentCID, false);
        ...
        require(signals2.epoch == s.unirep.attesterCurrentEpoch(uint160(s.attester)), "Outdated Epoch");
        require(signals2.attesterId == uint256(uint160(s.attester)), "Wrong attester");
        IAttester(s.attester).submitAttestation(
            signals2.epoch,
            signals2.epochKey,
            s.repWeight.post,
            0,
    ...
    function addComment(
        ...
        //Checking if it has enough reputation for commenting
        if(s.repRequirement.comment > 0) {
            Unirep.ReputationSignals memory signals = s.unirep.decodeReputationSignals(unirepProof.publicSignalsQ);
            require(signals.attesterId == uint256(uint160(s.attester)));
            require(signals.minRep == s.repRequirement.comment, "Not enough reputations");
            s.unirep.verifyReputationProof(unirepProof.publicSignalsQ, unirepProof.proofQ);
        }
        //
        ...
        //
        uint256[] memory childIds;
        s.items[s.itemCount] = Item(ItemKind.COMMENT, s.itemCount, parentId, groupId, block.number, childIds, 0, 0, note, unirepProof.ownerEpoch, unirepProof.ownerEpochKey, contentCID, false);
        ...
        //Give reputation as reply
        if(s.items[parentId].ownerEpoch == s.unirep.attesterCurrentEpoch(uint160(s.attester))) {
            IAttester(s.attester).submitAttestation(
                s.items[parentId].ownerEpoch,
                s.items[parentId].ownerEpochKey,
                s.repWeight.reply,
                0,
                0
            );
    ...
    function vote(
        ...
        s.semaphore.verifyProof(groupId, merkleTreeRoot, bytes32(signal), nullifierHash, signal, semaphoreProof);

        //Qualifying Process(checking if it has enough reputation for voting)
        uint256 reqRep = voteType == VoteKind.UP ? s.repRequirement.upvote : s.repRequirement.downvote;
        if(reqRep > 0) {
            Unirep.ReputationSignals memory signals = s.unirep.decodeReputationSignals(unirepProof.publicSignalsQ);
            require(signals.attesterId == uint256(uint160(s.attester)));
            require(signals.minRep == reqRep, "Not enough reputations");
            s.unirep.verifyReputationProof(unirepProof.publicSignalsQ, unirepProof.proofQ);
        }
        ...
        //Give reputation as voted
        if(s.items[itemId].ownerEpoch == s.unirep.attesterCurrentEpoch(uint160(s.attester))) {
            if(voteType == VoteKind.UP) {
                IAttester(s.attester).submitAttestation(
                    s.items[itemId].ownerEpoch,
                    s.items[itemId].ownerEpochKey,
                    s.repWeight.upvoted,
                    0,
                    0
                );
        ...
        IAttester(s.attester).submitAttestation(
            signals2.epoch,
            signals2.epochKey,
            voteType == VoteKind.UP ? s.repWeight.upvote : s.repWeight.downvote,
            0,
            0
        );
...
interface IAttester {
    function attesterSignUp(uint256 epochLength) external; 
    function userSignUp(uint256[] memory publicSignals, uint256[8] memory proof) external;
    function submitAttestation(
        uint targetEpoch,
        uint epochKey,
        uint posRep,
