...
contract Contract is ERC721A, Ownable {
    ...
    uint256 public tokenPricePresale = 0.01 ether; //400.45
    uint256 public tokenPricePublicSale = 0.01 ether; //400.45
    ...
    constructor() ERC721A("TGR TEST", "TGRTEST") {
    ...
    function payout(uint256 payoutAmount) external onlyOwner {
        require(payoutAmount * soldAmount == address(this).balance, "Not enough balance in contract");
 
        for(uint256 i = _startTokenId(); i <= soldAmount; i++){
            if(!_exists(i)){
                continue;
            }
            address owner = ownerOf(i);
            if(ownedNFTS[owner] == 0){
                owners.push(owner);
            }
            ownedNFTS[owner]++;
        }
        for(uint256 i = 0; i < owners.length; i++){
            require(payable(owners[i]).send(ownedNFTS[owners[i]] * payoutAmount));
            ownedNFTS[owners[i]] = 0;
        }
        delete owners;
    }
