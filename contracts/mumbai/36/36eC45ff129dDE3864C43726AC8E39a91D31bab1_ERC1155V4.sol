...
contract ERC1155V4 is ERC1155V3 {
    ...
    function updateTokenURI() external virtual {
        for(uint256 i = 0; i < getCurrentTokenId(); i++) {
            if((block.timestamp - tokenInfo[i].initTimeStamp) > tokenInfo[i].interval) {
    ...
    function setTokenInfo(uint256 tokenId, string memory baseURI, uint interval_) internal virtual {
        tokenInfo[tokenId].baseURI = baseURI;
        tokenInfo[tokenId].initTimeStamp = block.timestamp;
        tokenInfo[tokenId].interval = interval_;
    }
    
    function mintForDynamic(address to, uint256 amount, string memory tokenURI, uint interval_) public virtual onlyOwner {
        uint256 id = getCurrentTokenId();
        _mint(to, id, amount, "");
        _setURI(id, string(abi.encodePacked(tokenURI, "/0.json")));  
        setTokenInfo(id, tokenURI, interval_);
        incrementTokenId();
    }
...
contract BaseERC1155 is Initializable, ERC1155URIStorageUpgradeable, OwnableUpgradeable {
    ...
    function incrementTokenId() internal onlyOwner {
        _tokenTracker.increment();
    }
