...
contract ELSCargoXPool is ReentrancyGuard, Ownable, DSMath {
    ...
    enum AddressStatus{ DELETED, ACTIVE }
    ...
    // get address status
    function getAddressStatus(address _userPublicAddress) external view returns (AddressStatus) {
    ...
    function distributeRewards() external nonReentrant {
        ...
        require(totalRewardsIncludingFee >= 5*10**18, "There is not enough rewards to distribute.");
    ...
    function stake(uint256 _amount) external nonReentrant {
        ...
        require(DSMath.add(_amount, totalStakedBalance) <= 250000*10**18, "Maximum staking amount for this pool is 250000 CXO.");

        // check if user have enough CXO tokens in wallet
        uint256 userWalletBalance = token.balanceOf(msg.sender);
        require(_amount <= userWalletBalance, "You don't have enough CXO in your wallet?");
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        userRewardTally[msg.sender] = DSMath.sub(userRewardTally[msg.sender], DSMath.wmul(rewardPerToken, _amount));
    ...
    function withdrawRewards() public nonReentrant {
        ...
        uint256 rewardsLeft = DSMath.sub(token.balanceOf(address(this)), totalStakedBalance);
