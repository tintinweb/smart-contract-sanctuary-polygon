...
contract ConflictedGary is ERC721Enum, Ownable, ReentrancyGuard {
    using Strings for uint256;

    uint256 public status;
    address private company;
    uint256 public maxMint = 5;
    uint256 public reserve = 60;
    bytes32 public whitelistRoot;
    uint256 public freeMints = 1940;
    uint256 public maxSupply = 3880;
    uint256 public cost = 0.02 ether;
    uint256 public preSaleCost = 0.01 ether;
    string private _name = "XXX";
    string private _symbol = "XXX";
    string public baseURI = "ipfs://QmUF1w7ZTbMvQuF8EiA6ejyWPStX6DfxcTegB1kgz1Wkx8/";
    ...
    function mintPreSale(uint256 _mintAmount, bytes32[] calldata proof)
        external
        payable
        nonReentrant
    {
        ...
        require(balanceOf(msg.sender) + _mintAmount <= maxMint, ">5");
    ...
    function mint(uint256 _mintAmount) external payable nonReentrant {
        ...
        delete s;
        if (freeMints > 0) {
            uint256 pricedAmount = _mintAmount;
            if(freeMints < _mintAmount) pricedAmount = _mintAmount - freeMints;
            uint256 refund = isFreeMint() ? pricedAmount * cost : 0;
            if (refund > 0) freeMints -= pricedAmount;
