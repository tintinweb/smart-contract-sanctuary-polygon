...
contract ProtocolStats is OwnableUpgradeable {
    ...
    address public pairDollarWcro;
    address public pairShareWcro;
    ...
    function getDollarPriceToUsd() external view returns (uint256) {
        return IPriceChecker(priceChecker).getTokenPriceToUsd(dollar);
    }

    function getSharePriceToUsd() external view returns (uint256) {
        return IPriceChecker(priceChecker).getTokenPriceToUsd(share);
    }

    function getWbnbPriceToUsd() external view returns (uint256) {
        return IPriceChecker(priceChecker).getTokenPriceToUsd(mainCollateral);
    }

    function getVvsPriceToUsd() external view returns (uint256) {
        return IPriceChecker(priceChecker).getTokenPriceToUsd(secondCollateral);
    }

    function _reserveTotalDollarValue() internal view returns (uint256 _totalDollarValue) {
        IPriceChecker _priceChecker = IPriceChecker(priceChecker);
        ICollateralReserve _reserve = ICollateralReserve(reserve);

        uint256 _bal = _reserve.fundBalance(mainCollateral);
        if (_bal > 0) {
            _totalDollarValue = _priceChecker.getTokenPriceToUsd(mainCollateral) * _bal / 1e18;
        }

        _bal = _reserve.fundBalance(secondCollateral);
        if (_bal > 0) {
            _totalDollarValue += _priceChecker.getTokenPriceToUsd(secondCollateral) * _bal / 1e18;
        }
    }

    function addresses() external view returns (
        address dollarAddress, address shareAddress, address vvsAddress, address wbnbAddress,
        address pairDollarWcroAddress, address pairShareWcroAddress,
        address sharePoolAddress,
        address poolAddress, address treasuryAddress, address reserveAddress) {

        dollarAddress = dollar;
        shareAddress = share;
        vvsAddress = mainCollateral;
        wbnbAddress = secondCollateral;

        pairDollarWcroAddress = pairDollarWcro;
        pairShareWcroAddress = pairShareWcro;

        sharePoolAddress = sharePool;
    ...
    function tokenStats() external view returns (
        ...
        uint256 dollarUsdcLpPrice, uint256 shareWbnbLpPrice,
        uint256[] memory tokenSupplies
    ) {
        // prices, twaps: DARK, SKY, WCRO, VVS
        {
            prices = new uint256[](4);
            IPriceChecker _priceChecker = IPriceChecker(priceChecker);
            prices[0] = _priceChecker.getTokenPriceToUsd(dollar);
            prices[1] = _priceChecker.getTokenPriceToUsd(share);
            prices[2] = _priceChecker.getTokenPriceToUsd(mainCollateral);
            prices[3] = _priceChecker.getTokenPriceToUsd(secondCollateral);
            dollarUsdcLpPrice = _priceChecker.getTokenPriceToUsd(pairDollarWcro);
            shareWbnbLpPrice = _priceChecker.getTokenPriceToUsd(pairShareWcro);
        }
        {
            twaps = new uint256[](4);
            IPool _pool = IPool(pool);
            twaps[0] = _pool.getDollarPrice();
            twaps[1] = _pool.getSharePrice();
            twaps[2] = _pool.getMainCollateralPrice();
            twaps[3] = _pool.getSecondCollateralPrice();
        }
        {
            tokenSupplies = new uint256[](4);
            tokenSupplies[0] = IERC20(dollar).totalSupply();
            tokenSupplies[1] = IERC20(share).totalSupply();
            tokenSupplies[2] = IERC20(mainCollateral).totalSupply();
            tokenSupplies[3] = IERC20(secondCollateral).totalSupply();
        }
    }

    function treasuryStats() external view returns (
        uint256 minting_fee, uint256 redemption_fee,
        uint256 mintableLimit, uint256 redeemableLimit,
        uint256 mintingDailyLimit, uint256 mintingHourlyLimit,
        uint256 mintableDailyLimitLeft, uint256 mintableHourlyLimitLeft, uint256 redeemableDailyLimitLeft, uint256 redeemableHourlyLimitLeft,
        uint256 targetCollateralRatio, uint256 effectiveCollateralRatio
    ) {
        {
            ITreasury _treasury = ITreasury(treasury);
            minting_fee = _treasury.minting_fee();
            redemption_fee = _treasury.redemption_fee();
            effectiveCollateralRatio = _treasury.getEffectiveCollateralRatio();
        }
        {
            IPool _pool = IPool(pool);
            mintableLimit = _pool.calcMintableDollar();
            redeemableLimit = _pool.calcRedeemableDollar();
            mintingDailyLimit = _pool.mintingLimitDaily();
            mintingHourlyLimit = _pool.mintingLimitHourly();
            mintableDailyLimitLeft = _pool.calcMintableDollarDaily();
            mintableHourlyLimitLeft = _pool.calcMintableDollarHourly();
            redeemableDailyLimitLeft = _pool.calcRedeemableDollarDaily();
            redeemableHourlyLimitLeft = _pool.calcRedeemableDollarHourly();

            targetCollateralRatio = _pool.targetCollateralRatio();
        }
    }

    function reserveStats() external view returns (
        uint256 mainCollateralBalance, uint256 secondCollateralBalance,
        uint256 mainCollateralValue, uint256 secondCollateralValue,
        uint256 totalReserveValue, uint256[] memory reserveAssetPercents
    ) {
        ITreasury _treasury = ITreasury(treasury);

        mainCollateralBalance = _treasury.globalMainCollateralBalance();
        secondCollateralBalance = _treasury.globalSecondCollateralBalance();

        mainCollateralValue = _treasury.globalMainCollateralValue();
        secondCollateralValue = _treasury.globalSecondCollateralValue();

        totalReserveValue = mainCollateralValue + secondCollateralValue;

        reserveAssetPercents = new uint256[](2);
        reserveAssetPercents[0] = mainCollateralValue * 10000 / totalReserveValue;
        reserveAssetPercents[1] = secondCollateralValue * 10000 / totalReserveValue;
    }
    ...
    function _calculateTotalEarnedPerYear(address _pool, address _share) internal view returns (uint256 _totalEarned, uint256 _sharePrice) {
        ...
        _totalEarned = IRewardPool(_pool).getRewardPerSecond() * 365 days * _sharePrice / 1e18;
    }

    function sharePoolStats() public view returns (uint256 poolLength, uint256 totalTvl, address reward, uint256 rewardPrice,
        address[] memory lps, uint256[] memory allocPoints, uint256[] memory _tvls, uint256[] memory aprs, uint256[] memory lpPrices) {
        address _pool = sharePool;
        poolLength = IRewardPool(_pool).poolLength();
        lps = new address[](poolLength);
        allocPoints = new uint256[](poolLength);
        _tvls = new uint256[](poolLength);
        ...
        for (uint i = 0; i < poolLength; i++) {
            (lps[i], allocPoints[i]) = IRewardPool(_pool).getPoolInfo(i);
            {
                ...
                _tvls[i] = _poolBal * lpPrices[i] / 1e18;
                totalTvl += _tvls[i];
            }
            if (_pool == sharePool) {
                reward = share;
                ...
                if (_totalAllocPoint > 0) {
                    uint256 _poolEarnedPerYear = _totalEarnedPerYear * allocPoints[i] / _totalAllocPoint;
                    if (_tvls[i] > 0) aprs[i] = _poolEarnedPerYear * 1e18 / _tvls[i];
                }
            }
        }
    }

    function tvls() external view returns (uint256 sharePoolTvl, uint256 reserveTvl, uint256 totalTvl) {
        if (sharePool != address(0)) {
            (, sharePoolTvl, , , , , , ,) = sharePoolStats();
        }

        reserveTvl = _reserveTotalDollarValue();

        totalTvl = sharePoolTvl + reserveTvl;
    }
    ...
    function initialize(
        ...
        address _pairDollarWcro,
        address _pairShareWcro,
        address _sharePool,
        address _pool,
        address _treasury,
        address _reserve,
        ...
        secondCollateral = _secondCollateral;

        pairDollarWcro = _pairDollarWcro;
        pairShareWcro = _pairShareWcro;
    ...
    function setSharePool(address _sharePool) external onlyOwner {
        sharePool = _sharePool;
    }
