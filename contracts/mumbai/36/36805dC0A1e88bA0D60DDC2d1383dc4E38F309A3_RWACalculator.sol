...
contract RWACalculator is AddressAccessor, IRWACalculator {
        ...
        )
    {
        ...
        (usdValueInTreasury, valueNotLatest) = ITreasuryTracker(tracker)
            .getRwaUsdValue(IERC20Metadata(address(treasuryToken)));

        if (vaultTracker != address(0)) {
            //this is actually vaultTracker
            (usdValueInVaults, valueNotLatest) = ITreasuryTracker(vaultTracker)
                .getRwaUsdValue(IERC20Metadata(address(treasuryToken)));
        }
...
interface ITreasuryTracker {
    ...
    function addValueAfterPurchase(string calldata currency, uint256 value)
        external;
