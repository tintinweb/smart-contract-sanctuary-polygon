...
contract StakingAlgo3 is ReentrancyGuard {
    ...
    function allTokenWithdraw() external onlyAdmin {
        ...
        OMH.transferFrom(address(this), admin, _amount);
    }

    /** external funtions */
    function addRewardNotRefer(uint256 _amount) external {
        // OMH tokens have a burning feature when transferring
        // from the wallet is not ExcludedFromBurn.
        uint256 _tokenBalanceBeforeTransfer = OMH.balanceOf(address(this));
        OMH.transferFrom(msg.sender, address(this), _amount);
        uint256 _tokenBalanceAfterTransfer = OMH.balanceOf(address(this));
        uint256 _newAmount = _tokenBalanceAfterTransfer - _tokenBalanceBeforeTransfer;
    ...
    function addRewardForRefer(uint256 _amount) external {
        ...
        uint256 _tokenBalanceAfterTransfer = OMH.balanceOf(address(this));
        uint256 _newAmount = _tokenBalanceAfterTransfer - _tokenBalanceBeforeTransfer;
        totalRewardForRefer += _newAmount;
    }
