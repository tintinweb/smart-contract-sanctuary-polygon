...
contract BacanoGoComparsasV1 is BacanoGoCollection {
    ...
    function configureCharacters(
        ...
        cardSpec.description = _description;

        require(_images.length > 0, "Must have at least one image");

        // ImageSpec[] storage images = cardSpec.images;
        require(
            cardSpec.images.length == 0 || cardSpec.images.length == _images.length,
            "Can't change images size"
        );
    ...
    function getImages(CardType _cardType)
        external
        view
        returns (ImageSpec[] memory)
    {
        return cardSpecs[_cardType].images;
    }

    function getTotalOdds() internal view returns (uint32) {
        return
            cardSpecs[CardType.COMPARSA].odds +
    ...
    function getCardType(uint256 number) internal view returns (CardType) {
        if (number <= cardSpecs[CardType.COMPARSA].odds) {
            return CardType.COMPARSA;
        } else if (
            number <=
            (cardSpecs[CardType.BOXED].odds +
                cardSpecs[CardType.GOLDEN_STATUE].odds)
        ) {
            return CardType.BOXED;
        } else {
            return CardType.GOLDEN_STATUE;
        }
    }
    ...
    function transferTokensOnClaim(
        ...
        require(odds > 0, "No images configured");
    ...
    function formatTokenURI(
        ...
        ImageSpec memory imageSpec = cardSpec.images[token.imageIndex];
