...
contract CrazyBearsStaking is IERC721Receiver, Ownable, ReentrancyGuard {
    ...
    function checkReward(uint256 _tokenID) public view returns (uint){
        ...
        uint passedDays = passedSec.div(10);
    ...
    function unstake(uint256 _tokenID) public {
        ...
        uint256[] storage stakedNFTs = tokenByUser[msg.sender];
        uint index;
    ...
    function emergencyReturn(address _owner) public onlyOwner {      
        ...
        for (index = 0; index < stakedNFTs.length; index ++){
            ...
            stakedToken storage staking = tokenById[tID];
            stakedNFTs[index] = stakedNFTs[(stakedNFTs.length - 1)];
            stakedNFTs.pop();
            staking.user = address(0);
            stakeableContract.safeTransferFrom(address(this), _owner, tID);
        }
