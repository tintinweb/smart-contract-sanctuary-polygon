 ...
 * @author gotbit
 * @dev Implementation of the mintable ERC1155 collection for creating
 * proxy contracts through CollectionFactory contract
 */
contract ERC1155Mintable is IERC1155Mintable, ERC1155Upgradeable {
    address public immutable market;
    address public immutable factory;

    address public creator;

    constructor(address market_, address factory_) {
        if (market_ == address(0) || factory_ == address(0)) revert ZeroAddress();
     ...
     * @param creator_ ERC1155 collection creator
     */
     ...
     * Can only be called by the NeonomadMarket contract.
    ...
    function mint(
        ...
        if (msg.sender != market) revert RestrictedToMarket();
     ...
     * Doesn't require any token id to be minted.
     * @param tokenId ERC1155 token id
     * @return Token URI
     */
    function uri(uint256 tokenId)
        public
        view
        override(IERC1155Mintable, ERC1155Upgradeable)
        returns (string memory)
    {
        string memory baseURI = ICollectionFactory(factory).baseURI();
        return
 ...
 * the entire price of the deal, the order listing time, the order signature expiration
