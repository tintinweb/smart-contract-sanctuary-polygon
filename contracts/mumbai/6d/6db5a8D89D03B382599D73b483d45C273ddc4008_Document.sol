...
//shareholders
    ...
    //create shares with a limit of 10000
    uint16 shareKey = 0;
    function createShares(uint nr) public{
        require(shareKey < 10000, "limit reached");
        uint stop = nr+shareKey;
        if(stop>9999) stop = 9999;
        for (uint16 i=shareKey; i<stop; i++)
        {
...
//admin
    ...
    function testshareKey() public view returns (uint16){
        return shareKey;
    }

    function test2length() public view returns (uint){
        return shareholders[admin].shares.length;
    }

    function testkey5() public view returns (uint){
        return shareholders[admin].shares[5];
    }
}