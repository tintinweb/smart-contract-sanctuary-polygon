...
contract CarbonEatMonster is Ownable, ERC721Enumerable {
    ...
    // TODO:
    address public OffsetHelper = 0x30dC279166DCFB69F52C91d6A3380dCa75D0fCa7;

    constructor() ERC721("Carbon Eating Monster", "CEM") {}
    ...
    function feed(uint256 tokenId, address feedToken, uint amount) external {
        require(_exists(tokenId), "no token");
        require(_isFeedable(feedToken), "Token not feedable");

        Monster storage m = monsters[tokenId];
        // require(c.isAlive(), "already dead");
        m.lastFeedTime = block.timestamp;
