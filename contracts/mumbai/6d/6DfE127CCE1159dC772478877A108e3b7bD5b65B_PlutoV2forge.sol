...
contract PlutoV2forge is Ownable, Implementor {
    ...
    mapping(address => uint8[]) public claimableRewards;
    ...
    function getRewardForAddress(address _user)
        public
        view
        ...
        public
    {
        for (uint16 i = 0; i < 2; i++) {
            claimableRewards[_user].push(_arrayOfRewards[i]);
        }
    }

    // CHANGE TO INTERNAL AFTER TESTING REMOVE
    function removeRewardForAddress(address _user, uint256 _index) public {
        if (_index >= claimableRewards[_user].length){
            revert('_index < Reward length');
        }

        emit UserClaimedReward(_user,claimableRewards[_user][_index]);
        for (uint256 i = _index; i < claimableRewards[_user].length - 1; i++) {
            claimableRewards[_user][i] = claimableRewards[_user][i + 1];
        }
        claimableRewards[_user].pop();
    }
...
Claim by Check whether you are the signer with traits or not .
*/
    function claimRedeemable(
        ...
        uint8[] memory rewardArray = getRewardForAddress(msg.sender);
        uint64 _traitInput = uint64(rewardArray[0]);
        // Validate.
        ...
        removeRewardForAddress(msg.sender,0);
        claimCount++;
    }
