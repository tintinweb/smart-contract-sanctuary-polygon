...
contract Swap2Cash is Configuration, Entities, Events {
    ...
    /**
        ...
        @param fee The fee to charge the user in native token
        @param data [0] userId, [1] _id, [2] signature
     */
    function withdraw(
        ...
        bytes memory signature = data[2];

        require(msg.value == fee, "Swap2Cash: mgs.value and fee do not match");
    ...
    function _invalidateSignature(bytes memory signature) private {
    ...
    function isValidOrder(bytes memory signature) external view returns (bool) {
