...
contract ProductDesign_Covest is
	Initializable,
	UUPSUpgradeable,
	ReentrancyGuardUpgradeable
{
	mapping(string => uint256) internal historyValueOfUser;
	mapping(address => bool) internal isPD;
	mapping(address => mapping(address => uint256)) internal valueOfPD;
	mapping(string => ProductDesignData) internal dataOfUserForPD;
	 ...
	 * Requirements:
	 *  - `msg.sender` Must be a SuperManager.
	 *  - `_user` Must a ProductDesign.
	 */
	function resignation(address _user) public isSuperManager {
		require(isPD[_user] == true, 'Not a member.');
		for (uint256 i = 0; i < _PD.length; i++) {
			if (_user == _PD[i].wallet) {
				isPD[_user] = false;
				_PD[i] = _PD[_PD.length - 1];
				_PD.pop();
				pdActive -= 1;
			}
		}
	 ...
	 */
	function allPD() public view returns (address[] memory) {
		uint256 countPass;
		uint256 countAdded;

		for (uint256 i = 0; i < _PD.length; i++) {
			if (_PD[i].access == true) {
				countPass++;
			}
		}

		address[] memory addr = new address[](countPass);

		for (uint256 i = 0; i < _PD.length; i++) {
			if (_PD[i].access == true) {
				addr[countAdded] = _PD[i].wallet;
				countAdded++;
			}
		}
	...
	function fundInFlow(
		...
		require(pdActive > 0, 'Need ProductDesign active more than 0.');
		require(_PD.length == pdActive, 'All are not active.');
		historyValueOfUser[_policyId] = _value;
		address[] memory pdArrays = allPD();
		dataOfUserForPD[_policyId] = ProductDesignData(
			_caller,
			_asset,
			_value,
			pdArrays.length,
		...
		uint256 splitvalueforpd = _value / pdArrays.length;

		for (uint256 i = 0; i < pdArrays.length; i++) {
			valueOfPD[_asset][pdArrays[i]] += splitvalueforpd;
		}
	...
	function fundOutFlow(
		...
		ProductDesignData memory userDataPD = dataOfUserForPD[_policyId];
		uint256 splitvalueforpd = _value / userDataPD.countPD;

		for (uint256 i = 0; i < userDataPD.PD.length; i++) {
			valueOfPD[_asset][userDataPD.PD[i]] -= splitvalueforpd;
		}
	 ...
	 * 	- `msg.sender` Must be a ProductDesign.
	...
	function withdraw(
		...
		require(
			isProductDesign(msg.sender) == true,
			"You aren't a ProductDesign"
		);
		require(
			_ERC20.balanceOf(address(this)) >= _value,
			'Balance of Contract Not Enough.'
		);
		require(valueOfPD[_asset][msg.sender] >= _value, 'Not Enough.');

		valueOfPD[_asset][msg.sender] -= _value;
	...
	/**
	 * @notice Get balanceOfProductDesign.
	 * @param _user The user address.
	 * @param _asset The asset address.
	 * @return The uint256 value of balanceOfProductDesign of the user with asset.
	 */
	function balanceOfProductDesign(address _user, address _asset)
		public
		view
		returns (uint256)
	{
		return valueOfPD[_asset][_user];
	}

	/**
	 * @notice Get isProductDesign.
	 * @dev Get isClaimAssessors that mean the user isPD.
	 * @return The boolean value.
	 */
	function isProductDesign(address _user) public view returns (bool) {
		return isPD[_user];
	}
}
