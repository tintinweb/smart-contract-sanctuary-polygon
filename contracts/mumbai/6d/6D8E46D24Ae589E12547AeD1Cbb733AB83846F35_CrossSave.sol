...
contract CrossSave is IAxelarExecutable {
    ...
    constructor(
        ...
        wFTMTokenAddress = _wFtm;
        wMaticTokenAddress = _wMatic;
    }
    ...
    function _handlePOLYGONSaving(
        ...
        IERC20(wMaticTokenAddress).approve(
            address(gasService),
            _estimateGasCostFtm
        );
    ...
    function _handleFTMSavings(
        ...
        IERC20(wFTMTokenAddress).approve(
            address(gasService),
            _estimateGasCostPolygon
        );
        gasService.payGasForContractCall(
    ...
    function withdrawWrappedFtm() public {
        IERC20(wFTMTokenAddress).transfer(
            msg.sender,
            IERC20(wFTMTokenAddress).balanceOf(address(this))
        );
    }

    function withdrawWrappedMatic() public {
        IERC20(wMaticTokenAddress).transfer(
            msg.sender,
            IERC20(wMaticTokenAddress).balanceOf(address(this))
        );
    }
}
