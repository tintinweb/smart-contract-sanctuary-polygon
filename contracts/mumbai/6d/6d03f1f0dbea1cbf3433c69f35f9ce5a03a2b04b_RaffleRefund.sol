...
contract RaffleRefund is Context, Pausable {
    ...
    event RaffleTicketAddressUpdated(address);
    ...
    modifier whenRefundAllowed() {
        require(
            refundstatus,
            "RaffleRefund#whenRefundAllowed: Refund period not open"
        );
        _;
    }
    ...
    function setRaffleTicketAddress(address raffleaddr) public onlyOwner {
        require(
            raffleaddr != address(0),
            "RaffleRefund#setRaffleTicketAddress: RaffleTicket address should not be zero"
        );
        RAFFLE_TICKET_ADDRESS = raffleaddr;
        emit RaffleTicketAddressUpdated(raffleaddr);
    }

    function setVaultAddress(address vaultaddr) public onlyOwner {
        require(
            vaultaddr != address(0),
            "RaffleRefund#setVaultAddress: Vault address should not be zero"
        );
        VAULT = vaultaddr;
        emit VaultAddressUpdated(vaultaddr);
    }
    ...
    function claimRefund() public whenRefundAllowed returns (bool) {
        uint256 balance = IRaffle(RAFFLE_TICKET_ADDRESS).balanceOf(_msgSender());
        require(
            IRaffle(RAFFLE_TICKET_ADDRESS).burnFrom(_msgSender(), balance),
            "RaffleRefund#claimRefund: Burning Raffle Ticket Failed"
        );
        require(
            IVault(VAULT).transferFromVault(
                _msgSender(),
                balance * MINT_PUBLIC_PRICE
            ),
            "RaffleRefund#claimRefund: Payment from Vault Failed"
        );
