...
contract CurdRedemption is Ownable, Pausable, ReentrancyGuard {
	...
	event CoinReedemed(uint256 reedemAmount, uint256 curdToken);
	...
	constructor(uint256 tokenLimit, address[] memory payTokens, address curdToken, address treasuryWallet) {
	...
	function redeemCurdToken(uint256 _amount, address payToken) external whenNotPaused nonReentrant {
		if (_amount < _tokenLimit) revert InsufficientBalance(_tokenLimit, _amount);
		if (_amount > IERC20(_token).balanceOf(_msgSender()))
			revert InsufficientBalance(_amount, IERC20(_token).balanceOf(_msgSender()));

		uint256 redemptionAmount = (_basePrice.mul(_amount)).mul(10 ** ERC20(payToken).decimals());
		if (IERC20(payToken).balanceOf(address(this)) < redemptionAmount)
			revert InsufficientBalance(redemptionAmount, IERC20(payToken).balanceOf(address(this)));

		IERC20(_token).safeTransferFrom(_msgSender(), _treasuryWallet, _amount);
		IERC20(payToken).safeTransfer(_msgSender(), _amount);

		emit CoinReedemed(redemptionAmount, _amount);
	}

	function addToken(address token, uint256 _amount) external whenPaused onlyOwner {
	...
	function removeToken(address token, uint256 _amount, address _to) external whenPaused onlyOwner {
	...
	function setPayTokenStatus(address payToken, bool _status) external whenPaused onlyOwner {
	...
	function setTreasuryWallet(address treasuryWallet) external whenPaused onlyOwner {
		if (treasuryWallet == address(0x0)) revert ZeroAddress();
		if (treasuryWallet == _treasuryWallet) revert SameStatus();
	...
	function withdrawMatic(address payable _to) external payable onlyOwner {
		...
		(bool status, bytes memory data) = _to.call{value: address(this).balance}("");
		if (!status) revert();
	...
	function getTokenBalance(address token) external view {
