...
contract MonsterAssetData is Ownable
{
  ...
  struct DecodedImage {
    ...
    function constructTokenURI(uint8[] memory _decodeGene, uint256 _id) public view returns (string memory)
    {
        ...
        attribute = string(abi.encodePacked(attribute,'{"trait_type":"HEAD",'));
        attribute = string(abi.encodePacked(attribute,'"value":"head', uint256(_decodeGene[4]).toString(), '"}',","));
        attribute = string(abi.encodePacked(attribute,'{"trait_type":"GLASSES",'));
        attribute = string(abi.encodePacked(attribute,'"value":"glasses', uint256(_decodeGene[8]).toString(), '"}'));
