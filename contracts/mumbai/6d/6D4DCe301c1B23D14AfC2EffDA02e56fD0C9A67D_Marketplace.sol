...
contract Marketplace is ReentrancyGuard {
    ...
    event BidAccepted(address indexed seller, address indexed bidder, uint256 indexed tokenId, uint256 amount);
    ...
    function createMarketSale(
        ...
        emit MarketSaleCreated(tokenId, msg.sender, address(this), price);
    ...
    function placeBid(uint256 _tokenId) public payable {
        ...
        require(item.isListed == true, "This NFT is not listed for sale");
        require(item.biddingEnabled == true, "This NFT is not listed for bidding");
        require(msg.sender != item.seller, "You already own this NFT");
        require(msg.value > item.highestBid, "The bid value is not higher than the highest bid");

        if (item.highestBidder != address(0)) {
            // Refund the previous bidder
            address payable previousBidder = payable(item.highestBidder);
            previousBidder.transfer(item.highestBid);
        }

        item.highestBid = msg.value;
        item.highestBidder = msg.sender;

        emit NewBid(msg.sender, _tokenId, msg.value);
    }


    function acceptBid(uint256 tokenId, address nftContract) public payable nonReentrant {
        MarketItem storage item = idToMarketItem[tokenId];
        require(item.biddingEnabled == true, "Bidding not enabled for this item");
        require(item.highestBidder != address(0), "No bids have been placed for this item");
        require(item.seller == msg.sender, "Only owner can accept bids");

        uint256 price = item.highestBid;
        uint256 fees = (marketplaceFees / 100) * price;
        // uint256 amountReceived = price - fees;

        address payable seller = payable(item.seller);
        address payable newOwner = payable(item.highestBidder);

        // require(address(this).balance >= amountReceived, "Insufficeint balance");
        require(address(this).balance >= price, "Insufficient balance to transfer");

        seller.transfer(price - fees);
        IERC721(nftContract).transferFrom(address(this), newOwner, tokenId);

        item.owner = newOwner;
        item.isListed = false;
        item.highestBid = 0;
        item.biddingEnabled = false;

        itemsListed--;

        volume += price;

        emit BidAccepted(seller, newOwner, tokenId, price);
        emit MarketSaleCreated(tokenId, msg.sender, newOwner, price);
