...
contract BullitFile is BullitFileAdmin {
    ...
    function _deleteStorageForUser(address _userAddress) internal {
        for (uint256 i = 0; i < userOwnedFiles[_userAddress].length; i++) {
            ...
            users[_userAddress].numberOfFilesOwend -= files[userOwnedFiles[_userAddress][i]].hashFile.length;
            users[_userAddress].storageSpaceUsed -= files[userOwnedFiles[_userAddress][i]].fileSize;
