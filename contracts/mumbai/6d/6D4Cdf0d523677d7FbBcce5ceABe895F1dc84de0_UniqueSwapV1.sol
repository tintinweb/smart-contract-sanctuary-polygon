...
contract UniqueSwapV1 {
    ...
    function mint(address to) internal lock returns (uint256 liquidity) {
        ...
        if (_totalSupply == 0) {
            ...
            IERC20(ULPT).relayMint(WM, MINIMUM_LIQUIDITY); // here WM is temporary address instead of zero address
        } else {
            liquidity = Math.min(
    ...
    function readFees(uint256 myK) public view returns (uint256) {
        address _token0 = WM; // gas savings
        address _token1 = WW; // gas savings
    ...
    function _liquidityLock(
        ...
        amountOut20Percent = (amountIn1 * 20) / 100;
    ...
    function claimFees(
        ...
        bytes32 message = prefixed(
            keccak256(abi.encodePacked(myK, deadline, lID))
        );
        // console.log("admin: ", admin);
        // console.log("recoverSigner: ", recoverSigner(message, signature));
        require(recoverSigner(message, signature) == admin, "wrong signature");
