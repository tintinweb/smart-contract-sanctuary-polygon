...
contract ChosunGalleryCollection is
    ERC1155,
    Ownable,
    ERC1155Burnable,
    ERC1155Supply
{
    ...
    constructor()
        ERC1155("https://valores-mint-api.herokuapp.com/minted/{id}")
    {
        state.initialize();
    }
...
library BatchFactory {
    ...
    function authorizeMint(
    ...
    ) public returns (uint256 newTokenId) {
        {
            ...
            require(proof.verify(nftBatch.root, nftLeaf), "E008");
...
// E012 - Only ADMIN can perform this operation
...
library App {
    ...
    function initialize(AppType.State storage state) public {
        state.config.addresses[AppType.AddressConfig.ADMIN] = msg.sender; // TODO: Change to Admin address
        ...
        ] = "Chosun Gallery Collection";
        if (state.config.uints[AppType.UintConfig.CHAIN_ID] == 0x89) {
            state.tierSwapAmounts[1][address(0)] = 5e17;
