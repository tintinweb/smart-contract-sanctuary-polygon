...
contract squidGame is Ownable {
    ...
    //86400;
    uint256 private constant TURN_PERIOD = 50; // 24 HOURS
    uint256 private constant LEVEL = 5; // 13 stages
    ...
    */
    function partitionInGame(bool _chooes_side) public after24Hours {
        GameMember memory _member = Player[msg.sender];
        require(LEVEL >= _member.level, "Reached maximum");
        if (randomNumber <= 0) {
            randomNumber = random() * 1e9;
            start_period = block.timestamp;
            _member.chooes_side = _chooes_side;
        }
        uint256 day_ = (block.timestamp - start_period) / 50; //86400;
        uint256 dayAt = 0;
        if (day_ > 0) {
            dayAt = day_;
        }
    ...
    function checkProgres() public view returns (string memory, uint256) {
        GameMember memory _member = Player[msg.sender];
        uint256 period_differance = block.timestamp - _member.startAt;
    ...
    modifier after24Hours() {
        ...
        //86400
        require(calTimer >= 50, "Jump after 24 hours");
        _;
    }
}