...
//   mapping(address => uint256[]) public userPosts; // user address => post IDs
  ...
  function getPost(string memory _communityName, uint256 _postId) public view returns (string memory, string memory, uint256, uint256, uint256, uint256, address) {
        require(community.checkCommunityExists(_communityName), "Community doesn't exist");
        CommunityDetails storage communityDetail = communityDetails[_communityName];
        require(_postId < communityDetail.postCount, "Invalid post ID");
        Post storage post = communityDetail.posts[_postId];
        return (post.content, post.imageUrl, post.likes, post.dislikes, post.commands, post.timestamp, post.creator);
    }

     function getPostCount(string memory _communityName) public view returns (uint256) {
        require(community.checkCommunityExists(_communityName), "Community doesn't exist");
