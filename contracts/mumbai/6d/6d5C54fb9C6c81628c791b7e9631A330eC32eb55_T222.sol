...
contract T222 is Ownable, TimeForDay, SortList {
    ...
    uint256 public totalAccshare; //累积分红额度
    mapping(uint256 => uint256) public accshareList; //记录每次入金时的累积分红额度
    ...
    function Invest(uint256 _amount) external {
        ...
        */
        if (tatalInvestNumber == 0) {
            ...
            totalAccshare += (_amount * 25 / 100) * (10 ** 18) / tatalInvestAmount;
        }
        accshareList[tatalInvestNumber] = totalAccshare;
        userInvestList[_user].push(UserInvestItem(tatalInvestNumber, _amount));
        ...
        //        //uint256 _teamFee = _amount * 45 / 100;
    ...
    function getStaticReward(address _user) public view returns (uint256) {
        ...
        uint256 _num = x.length;
        uint256 _totalReward = 0;
        for (uint256 i = 0; i < _num; i++) {
            uint256 _investId = x[i]._investId;
            uint256 _investAmount = x[i]._investAmount;
            _totalReward += _investAmount * (accshareList[tatalInvestNumber - 1] - accshareList[_investId]) / (10 ** 18);
        }
        return _totalReward;
    }
