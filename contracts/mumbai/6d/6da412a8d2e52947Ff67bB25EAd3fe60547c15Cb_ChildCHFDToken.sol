...
import "../../contract/childchain/DeCashChildToken.sol";

contract ChildCHFDToken is DeCashChildToken {
    constructor(address _storage) DeCashChildToken(_storage) {}
}
...
/// @title DeCashChild Token implementation based on the DeCash perpetual storage
/// @author Shadowy Coders

contract DeCashChildToken is
    DeCashBase,
    IChildERC20,
    IChildToken,
    ...
    ContextMixin
{
    ...
    function initialize(
        ...
        //childChainManagerProxy = _childChainManagerProxy; // ADDED

        uint256 currentVersion = _getUint(
            keccak256(abi.encodePacked("token.version", _tokenName))
        );
    ...
    function deposit(address user, bytes calldata depositData)
        ...
        override
        onlyLastest
        onlyRole("childChainManager")
    {
        uint256 amount = abi.decode(depositData, (uint256));
        _mint(user, amount);
    }

    function withdraw(uint256 amount) external onlyLastest whenNotPaused {
    ...
    /**
     * This function distincts transaction signer from transaction executor. It allows anyone to transfer tokens
     * from the `from` multi senders to a recipient address by providing valid signatures, which can only be obtained from the `from` account owners.
     * Note that passed parameter sigId is unique and cannot be passed twice (prevents replay attacks). When there's
     * a need to make signature once again (because the first on is lost or whatever), user should sign the message
     * with the same sigId, thus ensuring that the previous signature won't be used if the new one passes.
     * Also note that the 1st recipient must be a valid fee receiver
     * Use case: some user wants to send some tokens to a unique user or smart contract, but don't have ether to do so.
     *
     * @param _senders       - array of accounts giving their signature to transfer `value` tokens to `to` address
     * @param _receiver      - account recipient
     * @param _values[]      - array of amount
     * @param _fee           - a fee to pay to `feeRecipient` by each sender
     * @param _feeRecipient  - account which will receive fees
     * @param _deadline      - until when the signature is valid
     * @param _sigId         - array of signature unique IDs. Signatures made with the same signature ID cannot be submitted twice
     * @param _sig           - array of signatures made by `from`, which is the proof of `from`'s agreement with the above parameters
     * @param _sigStd        - array of chosen standards for signature validation. The signer must explicitly tell which standard they use
     */
    function transferManySenderWithSignature(
        ...
        address _feeRecipient,
        uint256 _deadline,
        uint256[] memory _sigId,
        bytes[] memory _sig,
        Signature.Std[] memory _sigStd
    ) external onlyLastest {
        internalStruct memory _internalStruct;
        _internalStruct.sendersLen = _senders.length;
        _internalStruct.i = 0;

        require(
            _internalStruct.sendersLen == _values.length,
            "Wrong array parameter"
        );
        require(_internalStruct.sendersLen <= 100, "Too many senders");

        _validateManyViaSignatureParams(
            msg.sender,
            _senders,
            _feeRecipient,
            _deadline,
        ...
        for (
            _internalStruct.i = 0;
            _internalStruct.i < _internalStruct.sendersLen;
            _internalStruct.i++
        ) {
            Signature.requireSignature(
                keccak256(
                    abi.encodePacked(
                        address(this),
                        _senders[_internalStruct.i],
                        _receiver,
                        _values[_internalStruct.i],
                        _fee,
                        _feeRecipient,
                        _deadline,
                        _sigId[_internalStruct.i]
                    )
                ),
                _senders[_internalStruct.i],
                _sig[_internalStruct.i],
                _sigStd[_internalStruct.i],
                Signature.Dest.transfer
            );

            _subBalance(
                _senders[_internalStruct.i],
                _values[_internalStruct.i].add(_fee)
            );
            ...
            if (_fee > 0) {
                emit Transfer(_senders[_internalStruct.i], _feeRecipient, _fee);
            }

            _burnSigId(_senders[_internalStruct.i], _sigId[_internalStruct.i]);
        }

        _addBalance(_receiver, _calculateTotal(_values));
        if (_fee > 0) {
            _addBalance(_feeRecipient, _fee.mul(_internalStruct.sendersLen));
        }
    }
    ...
    function _validateManyViaSignatureParams(
        address _delegator,
        ...
        internalStruct memory _internalStruct;
        _internalStruct.sendersLen = _from.length;
        _internalStruct.i = 0;
