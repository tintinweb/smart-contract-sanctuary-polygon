...
contract xBegoToken is ERC20('xBego Farming', 'xBEGO') {
    ...
    uint16 public transferTaxRate = 5000; // 50%

    event SetMasterchef(address indexed newMasterchef);

    event transferType(address sender, address recipient, address msgSender, address txSender);

    constructor() {
        _mint(_msgSender(), uint256(1000000000000000000000)); // 1 token for initial liquidity
    }
    ...
    function setContracts(address _masterchef) external onlyOwner {
        require(masterchef == address(0), "Already Set");
    ...
    function setBond(address _bond, bool _isBond) external onlyOwner {
        isBond[_bond] = _isBond;
    }

    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        emit transferType(sender, recipient, msg.sender, tx.origin);
        ...
        if (isExcludedTransferTax) {
            ...
            // default tax is 50% of every transfer
            uint256 taxAmount = amount*transferTaxRate/10000;

            // 50% of transfer sent to recipient
...
contract Masterchef is Ownable, ReentrancyGuard {
    ...
    uint256 public xbegoPerSec = 0.009 ether;
