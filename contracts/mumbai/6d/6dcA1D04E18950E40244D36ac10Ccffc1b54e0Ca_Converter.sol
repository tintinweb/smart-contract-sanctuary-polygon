    ...
    OwnableUpgradeable
{
    ...
    function _burnAVYC(uint256[] calldata avycTokenIds) internal {
        for (uint256 i = 0; i < avycTokenIds.length; ) {
            uint256 avycTokenId = avycTokenIds[i];
            avycContract.transferFrom(
                msg.sender,
    ...
    function convertToGF(
        ...
        _burnAVYC(avycTokenIds);
        gfContract.converterMint(msg.sender, avycTokenIds);
        emit Convert(msg.sender, avycTokenIds.length);
    }

    // TODO: vault approve all OrdKubz to Converter's address
    function convertToOrdKubz(
        uint256[] calldata avycTokenIds
    ) external nonReentrant {
        require(avycRequiredPerOrdKubz > 0, "Conversion not open");
        require(
            avycTokenIds.length == avycRequiredPerOrdKubz,
            "Incorrect count of AVYC provided"
        );
        uint256 l = availOrdKubzIds.length();
        require(l > 0, "No availOrdKubzIds left");
        _burnAVYC(avycTokenIds);
        uint256 randOrdKubzId = availOrdKubzIds.at(rng(avycTokenIds) % l);
        availOrdKubzIds.remove(randOrdKubzId);
        ordKubzContract.transferFrom(vaultAddress, msg.sender, randOrdKubzId);
    }

    function convertToKubz(
        uint256[] calldata avycTokenIds
    ) external nonReentrant {
        require(avycRequiredPerKubz > 0, "Conversion not open");
        require(
            avycTokenIds.length == avycRequiredPerKubz,
            "Incorrect count of AVYC provided"
        );
        uint256 l = availKubzIds.length();
        require(l > 0, "No availKubzIds left");
        _burnAVYC(avycTokenIds);
        uint256 randKubzId = availKubzIds.at(rng(avycTokenIds) % l);
        availKubzIds.remove(randKubzId);
        kubzContract.transferFrom(vaultAddress, msg.sender, randKubzId);
    }
