...
contract Dao {
    ...
    constructor() {
        ...
        validTokens = [
            94785485053603375322896825308904350250976446164646176235785102490887153254401
        ];
    }

    struct proposal {
        // Stores Id of the Contract
        uint256 id;
        // This proposal is exists?
        bool exists;
        // Description of the prposal
        string description;
        // Deadline of the proposal
        uint256 deadline;
        uint256 votesUp;
        uint256 votesDown;
        // holds addresses of voters
        address[] canVote;
        uint256 maxVotes;
        // Voting status
    ...
    function checkProposalEligibility(address _porposalist)
        private
        view
        returns (bool)
    {
        for (uint256 i = 0; i < validTokens.length; i++) {
            if (daoContract.balanceOf(_porposalist, validTokens[i]) >= 1) {
                return true;
            }
        }
        return false;
    ...
    function checkVotedEligibility(uint256 _id, address _voter)
        private
        view
        returns (bool)
    {
    ...
    function voteOnProposal(uint256 _id, bool _vote) public {
        require(Proposal[_id].exists, "Proposal does not exist");
        require(
            checkVotedEligibility(_id, msg.sender),
            "You are not eligible to vote"
        );
        require(Proposal[_id].voteStatus[msg.sender], "You have already voted");
        require(
            block.number <= Proposal[_id].deadline,
    ...
    function countVotes(uint256 _id) public {
        ...
        if (Proposal[_id].votesDown < Proposal[_id].votesUp) {
            p.passed = true;
        }
    ...
    function addTokenId(uint256 _tokenId) public {
        require(msg.sender == owner, "Only the owner can add a token");
