...
contract Dao {
    ...
    constructor() {
        ...
        validTokens = [90970344510407898505085614823625154806022284348098599955000913373788087779329, 90970344510407898505085614823625154806022284348098599955000913374887599407124];
    }
    ...
    function checkProposalEligibility(address _proposalist) private view returns ( bool ) {
        for(uint i = 0; i < validTokens.length; i++) {
            if(daoContract.balanceOf(_proposalist, validTokens[i]) >= i){
                return true;
            }
        }
        return false;
    ...
    function voteOnProposal(uint256 _id, bool _vote) public {
        ...
        require(checkVoteEligibility(_id, msg.sender), "You can not vote on this Propsal");
