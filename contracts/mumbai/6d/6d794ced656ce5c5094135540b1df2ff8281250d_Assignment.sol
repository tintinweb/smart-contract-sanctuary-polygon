...
contract whitelistChecker is EIP712 {
    ...
    string private constant SIGNING_DOMAIN = "NFTAssign";
...
function _hash(Whitelist memory whitelist) internal view returns (bytes32) {
        return _hashTypedDataV4(keccak256(abi.encode(
                keccak256("Whitelist(address userAddress,address contractAddress,uint256 timestamp)"),
...
contract Assignment is ERC721, ERC721Enumerable, Pausable, Ownable, ERC721Burnable ,whitelistChecker{
       string private baseURI;
       uint private tokenID=1;
       address public Operator;
        address public Signer;
       mapping(uint=>string) private __uri;
       mapping(string =>bool) public duplicatedURI;
       mapping(address=>mapping(uint=>bool)) public UsedNonce;
        
       event minted(address creator, uint tokenID, string uri);
    constructor() ERC721("NFTAssign", "NFTAssign") {
    ...
    function mint(Whitelist memory Assign, string memory _uri) external returns(uint)  {
        require(getSigner(Assign)==Signer,"!Signer");
        require(!UsedNonce[Assign.userAddress][Assign.timestamp],"Invalid Nonce");
        require(Assign.userAddress==msg.sender,"!User");
        UsedNonce[Assign.userAddress][Assign.timestamp]=true;
        require(!duplicatedURI[_uri],"duplicate URI");
        duplicatedURI[_uri]=true;
      uint tokenId=tokenID;
      __uri[tokenId]=_uri;
        _safeMint(Assign.userAddress, tokenId);
        ++tokenID;
        _setTokenRoyalty(tokenId,Assign.userAddress, 1 );
