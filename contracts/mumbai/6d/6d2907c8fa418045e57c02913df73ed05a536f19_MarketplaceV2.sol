...
contract MarketplaceV2 is Initializable, Pausable {
    ...
    function initialize(address _nft) public initializer {
        ...
        foundationFee = 200;
        // 2% foundation on only initial sale
        adminFee = 800;
        // 8% admin  =>. owner() on initial sale

        adminFeeOnResell = 250;
        // 2.5% admin  =>. owner() on resell
        creatorFee = 750;
        // 7.5% creator (init artist) on resell

        foundationAddress = 0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2;
    ...
    function _setupAuction(uint256 _tokenId, uint256 _minPrice, uint256 _buyNowPrice, bool _resell) internal {
        ...
        _auctionState[_tokenId] = AuctionState(true, _minPrice, msg.sender, _resell);
    ...
    function _separateFees(uint256 _tokenId, address _seller, uint256 _price, bool _resell) internal returns (bool)
    {
        ...
        if (_seller == owner() && !_resell) {
            ...
            adminPart = _price - foundationPart;
