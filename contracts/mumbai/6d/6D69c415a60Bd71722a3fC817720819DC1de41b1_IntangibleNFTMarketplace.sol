...
interface IIntangibleNFTFactory {
    ...
    function isIntangibleNFT(address _nft) external view returns (bool);
}

interface IIntangibleNFT {
...
contract IntangibleNFTMarketplace is Ownable, ReentrancyGuard {
    IIntangibleNFTFactory private immutable intangibleNFTFactory;
    ...
    constructor(
        ...
        IIntangibleNFTFactory _intangibleNFTFactory
    ) {
        require(_platformFee <= 10000, "Can't more than 10 percent!");
        platformFee = _platformFee;
        feeRecipient = _feeRecipient;
        intangibleNFTFactory = _intangibleNFTFactory;
    }

    modifier isIntangibleNFT(address _nft) {
        require(intangibleNFTFactory.isIntangibleNFT(_nft), "Not Intangible NFT!");
        _;
    }
    ...
    function listNft(
        ...
        address _payToken,
        uint256 _price
    ) external isIntangibleNFT(_nft) isPayableToken(_payToken) {
    ...
    function buyNFT(
        ...
        IIntangibleNFT nft = IIntangibleNFT(listedNft.nft);
        ...
        isListedNFT(_nft, _tokenId)
    {
        ...
        IIntangibleNFT nft = IIntangibleNFT(offer.nft);
        address royaltyRecipient = nft.getRoyaltyRecipient();
        uint256 royaltyFee = nft.getRoyaltyFee();
    ...
    function resultAuction(address _nft, uint256 _tokenId) external {
        ...
        IIntangibleNFT intangibleNft = IIntangibleNFT(_nft);
        address royaltyRecipient = intangibleNft.getRoyaltyRecipient();
        uint256 royaltyFee = intangibleNft.getRoyaltyFee();
    ...
    function addPayableToken(address _token) external onlyOwner {
        require(_token != address(0), "Invalid payable token!");
