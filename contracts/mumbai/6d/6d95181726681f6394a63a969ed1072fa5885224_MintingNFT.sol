...
contract MintingNFT is ERC721Enumerable, Ownable {
    ...
    uint256[] prices;
    /* Contain the max counts of 3 kinds of NFTs */
    uint256[] maxCounts;
    ...
    function getPrice(uint256 category) public view returns (uint256 price){
    ...
    function openProposalPublic(string memory title, string memory content, string[] memory options, bool opened) public{
    ...
    */
    ...
    function openProposal(ProposalData memory proposal) public {
        proposal.index = _proposals.length;
    ...
    /* Get the length of proposals array */
    function getProposalsLength() public view returns (uint256 len) {
        len = _proposals.length;
    }

    /* Get [index] th proposal */
    function viewIthProposal(uint256 index) public view returns (string memory title, string memory content, string[] memory options, bool opened) {
        title = _proposals[index].title;
        content = _proposals[index].content;
        options = _proposals[index].options;
        opened = _proposals[index].opened;
    }
    ...
    function calc_result(uint256 proposal_index) public {
        uint256 nProposals = _proposals.length;
        uint256 i;
