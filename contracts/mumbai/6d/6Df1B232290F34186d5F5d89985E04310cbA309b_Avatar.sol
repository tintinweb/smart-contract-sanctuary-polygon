    ...
    UUPSUpgradeable
{
    ...
    function mint(address _toAddress, string[] memory reference_ids, string memory mintingType) external
        mintingValidation(reference_ids.length)
        onlyRole(MINTER_ROLE){
         uint256 _count = reference_ids.length;
        require(compareStrings(mintingType,"public") || compareStrings(mintingType,"reserved"), 
        ...
        if(compareStrings(mintingType,"reserved")){
         ...
         for (uint8 i = 0; i < _count ; i++) {
            _mintAnElement(address(_toAddress), address(msg.sender), mintingType, reference_ids[i]);
