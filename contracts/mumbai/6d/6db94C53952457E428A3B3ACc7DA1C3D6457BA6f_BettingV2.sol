 ...
 * Has functions for claim tournamnet fees and service fees.
...
contract BettingV2 is Ownable, AccessControl, ReentrancyGuard, Pausable {
    ...
    modifier afterTournamentBettingClosed(string memory _tournamentId) {
        require(
            tournamentByTournamentId[_tournamentId].isClose,
            "[BettingV2.afterTournamentBettingClosedd] Tournament betting period not yet closed"
        );
        _;
    }
    ...
    function createBettingTournament(
        ...
        require(
            _teamIds.length > 1,
            "[BettingV2.createBettingTournament] Invalid entiryIds length"
        );
    ...
    function tournamentFeesWithdraw(string memory _tournamentId)
        ...
        nonReentrant
        tournamentCreated(_tournamentId)
        afterTournamentBettingClosed(_tournamentId)
    {
        Tournament storage tournament = tournamentByTournamentId[_tournamentId];
        require(
        ...
        safeUpoTransfer(msg.sender, tournamentFees);
        emit TournamentFeesWithdrawed(_tournamentId, msg.sender, tournamentFees);

        delete tournamentByTournamentId[_tournamentId].tournamentFees;
    }
