    ...
    OwnableUpgradeable
{
    ...
    function getResources(address account) external view returns (uint256[] memory sym, uint256[] memory qt) {
        for (uint256 i = 0; i < resources[account].length(); i++) {
            (uint256 res, uint256 quantity) = resources[account].at(i + 1);
            sym[i] = res;
            qt[i] = quantity;
        }
    }
    ...
    function getEnriches(address account) external view returns (uint256[] memory sym, uint256[] memory qt) {
        for (uint256 i = 0; i < enriches[account].length(); i++) {
            (uint256 res, uint256 quantity) = enriches[account].at(i + 1);
            sym[i] = res;
            qt[i] = quantity;
        }
    }
