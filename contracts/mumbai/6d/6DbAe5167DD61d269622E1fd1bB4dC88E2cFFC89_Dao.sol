...
contract Dao {
  ...
  event proposalCount(
    ...
    function voteOnProposal(uint256 _id, uint256 _votesUp, uint256 _votesDown) public {
        ...
        require( (proposalToId[_id].individualVotes[msg.sender] += (_votesUp+_votesDown)) <= maxVotesAvailable );
    ...
    function _vote(uint256 _id, uint256 _votesUp, uint256 _votesDown, address _voter, uint256 maxVotesAvailable) internal {
        proposalToId[_id].individualVotes[_voter] += (_votesUp+_votesDown);    
        proposalToId[_id].votesUp += _votesUp;
        proposalToId[_id].votesDown += _votesDown;
        proposalToId[_id].individualChoices[_voter][true] += _votesUp;
        proposalToId[_id].individualChoices[_voter][false] += _votesDown;
        if(proposalToId[_id].individualVotes[_voter] == maxVotesAvailable){
            proposalToId[_id].voteStatus[_voter] == true;
        }
        bool votedFor = (_votesUp > _votesDown);
        if ((proposalToId[_id].votesUp + proposalToId[_id].votesDown) == proposalToId[_id].maxVotes){
            _countVotes(_id);
        }
    ...
    function _countVotes(uint256 _id) internal {
        ...
        } else if(proposalToId[_id].votesUp < proposalToId[_id].votesDown){
