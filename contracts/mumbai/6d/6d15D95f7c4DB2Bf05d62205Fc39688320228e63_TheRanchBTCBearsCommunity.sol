...
contract TheRanchBTCBearsCommunity is 
    ...
    */
    ...
    struct BTCBullOwner {
        ...
        uint256 hostingFeeBalance;     // totaly hosting fees due from BTC Bull owner address  
    ...
    */
    ...
    function mintingGiveaway(
        ...
        for( uint256 i; i < _winningAddresses.length ; i++ ) {
            ...
            btcBullOwners[_winningAddresses[i]].USDC_Balance += _winningAmount;
        ...
        for( uint256 i = startingIndex; i <= endingIndex; i++) {
            ...
            if (bullOwnerAddress != address(0)){
                ...
                if (btcBullOwners[bullOwnerAddress].lastRewardDate != currentRewardingDate) {
                    ...
                    _bullOwner.hostingFeeBalance += _maint_fee_to_add;
         

                    // update the hostingClock for the _bullOwner
                    if (_maint_fee_to_add > 0) {
                        _bullOwner.hostingClock += 1;
                    } else if (_bullOwner.hostingFeeBalance > 0){
                        _bullOwner.hostingClock += 1;
                    } 


                    // Check if _bullOwner is 3 rewards cycles behind on the account 
                    if (_bullOwner.hostingClock == 3){
    ...
    function updateMaintenanceStandingOfThoseNotRewardedThisMonth() external ADMIN_APPROVED {
        for( uint256 i; i < rewardedAddresses.length; i++) {
            ...
            if (btcBullOwners[_address].WBTC_Balance > 0){
                if (btcBullOwners[_address].hostingFeeBalance != 0){
                    if (btcBullOwners[_address].lastRewardDate != currentRewardingDate) {
                        
                        // take action and add one to hostingClock
                        btcBullOwners[_address].hostingClock += 1;
                        
                        if (btcBullOwners[_address].hostingClock == 3){
    ...
    **/
    function liquidateOutstandingAccounts() external ADMIN_APPROVED {
        ...
        for( uint256 i; i < upForLiquidation.length; i++) {
            ...
            uint256 _neededAmountToAvoidLiquidation = _bullOwner.hostingFeeBalance;

            if (_bullOwner.hostingFeeReserve >= _neededAmountToAvoidLiquidation ) {
                _bullOwner.hostingFeeReserve -= _neededAmountToAvoidLiquidation;
                hostingSafeBalance += _neededAmountToAvoidLiquidation;
                USDCReserveBalance -= _neededAmountToAvoidLiquidation; 
                _bullOwner.hostingFeeBalance = 0;
                _bullOwner.hostingClock = 0;
                continue;
            } else {
    ...
    **/
    function payHostingFees() external nonReentrant {
        ...
        uint256 _feesDue = btcBullOwners[msg.sender].hostingFeeBalance;
        uint256 _balance = btcBullOwners[msg.sender].hostingFeeReserve;
        
        if (_feesDue == 0) { revert Maintenance_NoMaintenanceFeesRequired();}

        if (_balance >= _feesDue){

            btcBullOwners[msg.sender].hostingFeeReserve -= _feesDue;
            ...
            if(_balance == 0){
                ...
                btcBullOwners[msg.sender].hostingFeeReserve -= _balance; 
        ...
        btcBullOwners[msg.sender].hostingFeeBalance = 0;
        btcBullOwners[msg.sender].hostingClock = 0;
    }
    ...
    function depositIntoHostingReserve (uint256 _amountToDeposit) public payable nonReentrant {
        ...
        btcBullOwners[msg.sender].hostingFeeReserve += _amountToDeposit;
    ...
    function withdrawHostingReserve() external nonReentrant {
        ...
        uint256 myBalance = btcBullOwners[msg.sender].hostingFeeReserve;
        ...
        btcBullOwners[msg.sender].hostingFeeReserve = 0  ;
   ...
   function withdrawSafe(uint _safeToTarget) external ADMIN_APPROVED {
    ...
    //     if (btcBullOwners[msg.sender].liquidationCount == 0){revert Redemption_NotAllowedWhenZero();}
    //     IERC20Upgradeable tokenContract = IERC20Upgradeable(usdcTokenContract);
    //     uint256 _amount = 125 * 10 ** usdcTokenDecimals;
    //     tokenContract.safeTransferFrom(msg.sender, address(this),_amount);
    //     btcBullOwners[msg.sender].liquidationCount -= 1;
    ...
    function withdrawWbtcBalance() external nonReentrant {
        ...
        require(btcBullOwners[msg.sender].hostingFeeBalance == 0, "You must pay maintenance fee balance before WBTC withdrawal is allowed");
...
contract TheRanchBTCBearsChainLinkVRF is VRFConsumerBaseV2, Ownable {
    ...
    function getGiveawayWinner() public TRBC_contract_only {
        ...
        //emit RequestedGiveawayWinner(requestId);
    }
    ...
    //     uint dailyGiveawayPlayersCount = getNumberOfGiveawayPlayers();
    //     uint256 indexOfWinner = randomWords[0] % dailyGiveawayPlayersCount;

    //     // send indexOfWinner to the proxy contract 
    //     TheRanchBTCBearsCommunity TRBC = TheRanchBTCBearsCommunity(TheRanchBTCBearsProxy);
    //     TRBC.mintingGiveaway(indexOfWinner);
...
//     address[] _winningAddresses;        // 3 winner will populate this 

//     uint256[] _winningIndexes;        // winning Indexes


//     uint256[] _couldHaveWonIndexes;        // indexes that count have won the Giveaway but liquidity count failed them.

    // uint256[] randomWords;

    // function fakeGiveaway() public {

    //     randomWords =  [
    //         12434343567452345234,
    //         22345234234565234523452345,
    //         24524554546523452452,
    //         32442534534542352345,
    //         23452342345234523452354,
    //         14524324235234523455,
    //         25343576674657466,
    //         25343576674657466,
    //         25343576674657466,
    //         2456354673573735739,
    //         532576347232624532345234523,
    //         4321523523452345234554310,
    //         23452345234523452345431252435
    //     ];
    ...
    // uint256[] winnersIPicked = [291,292,293,294,301,302];
    ...
    //     // FIND X amount of winners 
    //     for( uint256 i; i < winnersIPicked.length ; i++ ) {
    //         // find index of a potential winner 
    //         uint256 _winnerIndex = winnersIPicked[i] ;
