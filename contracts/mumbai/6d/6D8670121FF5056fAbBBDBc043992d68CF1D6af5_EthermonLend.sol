...
// File: contracts/EthermonLend.sol
...
contract EthermonLend is EthermonEnum, BasicAccessControl, SafeMathEthermon {
    ...
    uint256 public RentEnableHoldingEMON = 100 * 10**18;
    uint16 public RentEnableHoldingEMONA = 30;
    ...
    function setConfig(uint256 _emonRatio) public onlyModerators {
    ...
    /**
        Minimum Emons available to place mon on rent.
    */
    function updateRentHoldingAmount(uint256 _fee, uint16 _EMONA_Limit)
        public
        onlyModerators
    {
        if (_fee < 0 || _EMONA_Limit < 0) revert();

        RentEnableHoldingEMON = _fee;
        RentEnableHoldingEMONA = _EMONA_Limit;
    }

    function offerBorrowingItem(
        uint64 _objId,
        uint256 _price,
        uint256 _releaseTime
    ) external isActive {
        EthermonTradeData monTradeData = EthermonTradeData(
            tradingMonDataContract
        );
        if (monTradeData.isOnSell(_objId) || monTradeData.isOnLent(_objId))
            revert();
        ...
        if (
            obj.trainer != msgSender() ||
            !monTradeData.isUserWhiteListed(obj.trainer)
        ) revert();

        //Checking if required emon is there and user have certain amount of emon.
        uint256 dexSize = data.getMonsterDexSize(obj.trainer);
        if (
            emon.balanceOf(msgSender()) < RentEnableHoldingEMON ||
            dexSize < RentEnableHoldingEMONA
        ) revert();
        ...
        //Check if timestamp passed is lesser than current time
        if (battle.isOnBattle(obj.monsterId) || _releaseTime < block.timestamp)
            revert();

        monTradeData.addBorrowingItem(
            msgSender(),
            _objId,
            _price,
            address(0),
        ...
        emit EventOfferBorrowingItem(msgSender(), _objId, _price, _releaseTime);
    }

    function removeBorrowingOfferItem(uint64 _objId) external isActive {
        EthermonTradeData monTradeData = EthermonTradeData(
            tradingMonDataContract
        );
        address owner;
        ...
        if (owner != msgSender() || lent == true) revert();

        monTradeData.removeBorrowingItem(_objId);
        emit EventRemoveOfferBorrowingItem(msgSender(), _objId);
    }

    function borrowItemMatic(uint64 _objId) external payable isActive {
        EthermonTradeData monTradeData = EthermonTradeData(
            tradingMonDataContract
        );
        ...
        (
            ...
            borrowItem.createTime,
            borrowItem.releaseTime
        ) = monTradeData.getBorrowInfo(_objId);
        if (borrowItem.index == 0 || borrowItem.lent == true) revert();
        if (borrowItem.owner == msgSender()) revert(); // can not borrow from yourself
        if (borrowItem.price != msg.value) revert();
        ...
        monTradeData.addBorrowingItem(
            ...
            msgSender(),
            true,
            (borrowItem.releaseTime + block.timestamp),
            borrowItem.createTime
        );

        data.removeMonsterIdMapping(trainerAdress, obj.monsterId);
        data.addMonsterIdMapping(msgSender(), obj.monsterId);
        EtheremonMonsterNFTInterface monsterNFT = EtheremonMonsterNFTInterface(
            monsterNFTContract
        );
        monsterNFT.triggerTransferEvent(trainerAdress, msgSender(), _objId);

        trainerAdress.transfer(
            safeSubtract(
                borrowItem.price,
                (borrowItem.price * tradingFeePercentage) / 100
            )
        );
        monTradeData.addItemLendingList(trainerAdress, _objId);
        EventAcceptBorrowItem(
            trainerAdress,
            msgSender(),
            _objId,
            borrowItem.price
    ...
    function borrowItemEmon(uint64 _objId) external isActive {
        EthermonTradeData monTradeData = EthermonTradeData(
            tradingMonDataContract
        );
        BorrowItem memory borrowItem;
        ...
        if (borrowItem.index == 0 || borrowItem.lent == true) revert();
        if (borrowItem.owner == msgSender()) revert(); // can not borrow from yourself
        if (borrowItem.price > emon.balanceOf(msgSender())) revert();
        ...
        uint256 fee = (borrowItem.price * tradingFeePercentage) / 100;
        uint256 emonPrice = safeSubtract(borrowItem.price, fee) * emonRatio;

        emon.safeTransferFrom(msgSender(), obj.trainer, emonPrice);

        //Fee to contract
        emon.safeTransferFrom(msgSender(), address(this), fee);
        //TODO: Need to transfer ((borrowItem.price * tradingFeePercentage) / 100)(fee) to this contract

        // obj.trainer.transfer(
        //     safeSubtract(
        //         borrowItem.price,
        //         (borrowItem.price * tradingFeePercentage) / 100
        //     )
    ...
    function withdrawEmon(address _sendTo, uint256 _amount)
        public
        onlyModerators
    {
    ...
    function getBorrowingItem(uint256 _index)
        external
        view
        returns (
            uint64 objId,
        ...
        )
    {
        ...
        (
            ...
            createTime,
            releaseTime
        ) = monTradeData.getBorrowInfoByIndex(_index);
    }

    function getBackLendingItem(uint64 _objId) external isActive {
        EthermonTradeData monTradeData = EthermonTradeData(
            tradingMonDataContract
        );
        BorrowItem memory borrowItem;
        ...
        if (borrowItem.index == 0) revert();
        if (borrowItem.lent == false) revert();
        if (borrowItem.releaseTime > block.timestamp) revert();

        if (msgSender() != borrowItem.owner) revert();

        monTradeData.removeBorrowingItem(_objId);

        EtheremonDataBase data = EtheremonDataBase(dataContract);
        data.removeMonsterIdMapping(borrowItem.borrower, _objId);
        data.addMonsterIdMapping(msgSender(), _objId);
        EtheremonMonsterNFTInterface monsterNFT = EtheremonMonsterNFTInterface(
            monsterNFTContract
        );
        monsterNFT.triggerTransferEvent(
            borrowItem.borrower,
            msgSender(),
            _objId
        );

        monTradeData.removeItemLendingList(msgSender(), _objId);
        emit EventGetBackItem(msgSender(), borrowItem.borrower, _objId);
    }

    function getBorrowingItemByObjId(uint64 _objId)
        external
        view
        returns (
            uint256 index,
    ...
    function getLendingItemLength(address _trainer)
        external
        view
        returns (uint256)
    {
    ...
    function getLendingItemInfo(address _trainer, uint256 _index)
        external
        view
        returns (
            uint256 objId,
        ...
        )
    {
        ...
        (
            ...
            createTime,
            releaseTime
        ) = monTradeData.getLendingInfo(_trainer, _index);
    }
}