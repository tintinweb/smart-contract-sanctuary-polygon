...
contract Marketplace is Ownable{
    ...
    event RevenueDistributed(uint pdrId, uint totalAmount);
    ...
    function buyRental(Order memory sellOrder, 
        ...
        doRentalTransfer(sellOrder.maker, sellOrder.rentalId, numberOfRentals, start, end);
    }

    function renewRental(Order memory sellOrder, uint rentalId,
     uint128 start, uint128 end) external payable{
         //get pdr from renter id
         uint pdrId = IProperty(collectionAddress).getPdrFromRental(rentalId);

        // pdrOwner 
        uint rwoId = IProperty(collectionAddress).getRwoFromPdr(pdrId);
        // require(IProperty(collectionAddress).getRwoOwner(rwoId) == msg.sender, "Only owner");
        address from = IProperty(collectionAddress).getRwoOwner(rwoId);

        // get rental details
        address ownerRental = IProperty(collectionAddress).getRentalOwner(rentalId);
        uint numberOfRentals = IERC1155(collectionAddress).balanceOf(ownerRental,rentalId); 

        updateOrderFill(sellOrder, numberOfRentals);
        (uint256 totalPayment, uint256 platformPayment) = calculateTotalPayment((sellOrder.payment.amount * numberOfRentals) / sellOrder.numberOfTokens);
        
        //TODO: should be deducted from ownerRental
        doPaymentTransfer(msg.sender, totalPayment, sellOrder.maker, sellOrder.payment, platformPayment, true);
        IProperty(collectionAddress).renewRental(pdrId, from, msg.sender, rentalId, numberOfRentals, start, end);
    }
    ...
    function calculateTotalPayment(uint256 amount) internal view returns(uint256, uint256){
        // adding buyer-side platform fee to total payment
        return (amount + ((amount * platformFee) / 20000), ((amount * platformFee) / 10000) );
    }

    function searchIfRentalOwnerMatches(address[] calldata accounts, address ownerRental) private pure returns(bool){
    ...
    function payPlatformFee(uint256 totalPayment, uint256 platformPayment, Payment memory payment) internal returns(uint){
    ...
    function distributeRevenue(uint pdrId, uint amountPerNFT, address[] calldata accounts, Payment memory payment) public payable {
        //get rental ids
        IProperty.Rental[] memory renters = IProperty(collectionAddress).getRentalFromPDR(pdrId);
        ...
        for(uint i=0; i<renters.length; i++){
            IProperty.Rental memory currentRenter = renters[i];
    ...
    function doPaymentTransferForRevenue( uint currentPayment, address renter, Payment memory payment) internal{
        if (payment.paymentClass == MATIC_PAYMENT_CLASS) {            
    ...
    function doPaymentTransfer(address from, uint256 totalPayment, address seller, Payment memory payment, uint256 platformPayment, bool withRoyaltyFee) internal {
        // Transfer platform fees
    ...
    function doRentalTransfer(address from, uint256 rentalId, uint256 numberOfTokens, uint128 start, uint128 end) internal {
        IProperty(collectionAddress).safeTransferRental(from, msg.sender, rentalId, numberOfTokens, start, end);
    }
    ...
    function setPlatformFee(uint256 _newPlatformFee) external  onlyAdminOrOwner {
...
interface IProperty{
    ...
    function getRentalDetails(
        uint rentalId
    ) external view returns(Rental memory _rental);
    ...
    function getRwoFromPdr(uint pdrId) external view returns(uint);

    function getRwoOwner(uint rwoId) external view returns(address);

    function getPdrFromRental(uint rentalId) external view returns(uint);
}
