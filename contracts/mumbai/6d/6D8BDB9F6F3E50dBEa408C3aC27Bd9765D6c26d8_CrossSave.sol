...
contract CrossSave is IAxelarExecutable {
    ...
    function save(
        ...
        if (!savings[msg.sender].exist) {
            ...
            // IF THE OPTION FOR SAVING IS FLEXIBLE(FALSE) OR LOCKED(TRUE)
            if (!_option) {
    ...
    function updateFtmDestinationAddress(string memory _addr) public {
        ftmDestinationAddress = _addr;
    }

    function updateBnbDestinationAddress(string memory _addr) public {
        bnbDestinationAddress = _addr;
    }

    function updatePolygonDestinationAddress(string memory _addr) public {
        polygonDestinationAddress = _addr;
    }
}
