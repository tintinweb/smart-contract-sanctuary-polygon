...
contract PerpetualSettlement is PerpetualRebalanceFunctions, IFunctionList, IPerpetualSettlement {
    ...
    function settleNextTraderInPool(uint8 _id) external override nonReentrant returns (bool) {
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            if (perpetual.state == PerpetualState.SETTLE) {
                ...
                int128 fAMMCash = _getMarginViewLogic().getAvailableCash(idx, address(this));
        ...
        if (isAllCleared && iEmergencyCount > 0) {
            // we simply pour the broker fund into the default fund
            liquidityPool.fDefaultFundCashCC = liquidityPool.fDefaultFundCashCC.add(
                liquidityPool.fBrokerFundCashCC
            );
            liquidityPool.fBrokerFundCashCC = 0;
            ...
            fTotalCapitalCC = fTotalCapitalCC.add(
                fTotalTraderMarginBalance.add(fTotalEmrgAMMMarginBalance)
            );
     ...
     * @param   _fTotalTraderMarginBalance     Total traders' margin balance for all emergency-state pools
     * @param   _fTotalEmrgAMMMarginBalance      Total AMM margin balance for all emergency-state perpetuals
     */
    function _prepareRedemption(
        ...
        /*
        Traders should be paid _fTotalTraderMarginBalance. This includes their cash (post funding payments) and PnL
            1) The funds used to pay for this are: traders' own funds (margin cash) + AMM margin cash + pool cash
            1) If the funds available cannot cover the entire balance due, 
                then the balance due is down-scaled by the redemption rate
            2) Since the AMM is the counterpary to all traders, we make use of:
                 AMM margin cash + Traders' margin cash == AMM margin balance + Traders' margin balance
            3) The amount owed is then reduced by adding to it all the available margin cash (AMM and traders):
                a) If these funds are more than enough to cover the debt, the excess is sent to the default fund 
                b) If these funds do not cover the amount owed, then:
                    i) PnL fund pays for the remainder
                    ii) If PnL funds were not enough, the default fund pays the rest
            3) If redemption rate == 1, then because of (i) and (ii) the default fund should usually ~not~ be empty after settlement
        */
        int128 fMgnBalanceToPay = _fTotalTraderMarginBalance.mul(_liquidityPool.fRedemptionRate);
        int128 withdrawFromDF = fMgnBalanceToPay.sub(
            _fTotalTraderMarginBalance.add(_fTotalEmrgAMMMarginBalance)
        );
     ...
     *                               need settlement
     * @param   _fTotalCapital       total capital (default fund, amm pools, participation fund, amm margin, traders' margin)
     */
        ...
        returns (int128)
    {
        ...
        if (fPos != 0) {
            ...
            int128 fRate = liquidityPools[poolId].fRedemptionRate;
