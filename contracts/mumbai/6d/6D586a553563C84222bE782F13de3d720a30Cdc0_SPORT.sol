...
contract ESkillzStraightBet is Ownable {
    ...
    function SetSPGameResult(uint256 GameID, uint256 result) external {
        ...
        (uint256 amountToWinner,uint256 amountToESkillz) = getAmountsSPGameToDistribute(GameID,result);
        ...
        sport.mintMore(gamebetting[GameID][0].amount.mul(result).div(100));
    ...
    function  CancelMPGame(uint256 gameID) external {
      ...
      require(gamebetting[gameID][0].gameType == 1, "MP game can cancel only");
    ...
    function getAmountsSPGameToDistribute(uint256 game, uint256 result) private view returns (uint256, uint256) {
        uint256 amountToWinner = gamebetting[game][0].amount+gamebetting[game][0].amount.mul(result).div(100)-(gamebetting[game][0].amount).mul(eskillz_fee).div(100);
        uint256 amountToESkillz = (gamebetting[game][0].amount).mul(eskillz_fee).div(100);
    ...
    function getAmountsMPGameToDistribute(uint256 game) private view returns (uint256, uint256) {
        uint256 amountToWinner = (gamebetting[game][0].amount+gamebetting[game][1].amount).mul(100-eskillz_fee.div(2)).div(100);
        uint256 amountToESkillz = (gamebetting[game][0].amount+gamebetting[game][1].amount).mul(eskillz_fee.div(2)).div(100);
        return(amountToWinner, amountToESkillz);
    }
