...
contract OmniseaDropsManager is IOmniseaDropsManager, IOmniseaReceiver, ReentrancyGuard {
    ...
    function mint(MintParams memory _params) public payable { // TODO (Must) Removed nonReentrant for testing
        require(_params.coll != address(0), "!collAddr");
        require(_params.quantity > 0, "!quantity");
        ...
        if (quantityPrice > 0) {
            ...
            IOmniseaPaymentsManager(paymentsManager).onPayment(minter, quantityPrice, address(collection));

            (bool p,) = payable(paymentsManager).call{value : quantityPrice}("");
            require(p, "!p1");
        }

        uint excessPayment = msg.value - quantityPrice;
        if (excessPayment > 0) {
            (bool p,) = payable(minter).call{value : excessPayment}("");
            require(p, "!p2");
        }
