...
/**
    ...
    Bit Finance Token
    website : https://bitfinancetoken.com
...
contract Molly is Context, IERC20, Ownable {
    ...
    //50B
    uint256 private supply = 50 * 10**9 * 10**_decimals;
    ...
    address public devWallet = 0x04AbFa976cDc0B9245CAD162F1D75BA0e3Af3c6f;
    ...
    constructor() {
        IRouter _router = IRouter(0xbdd4e5660839a088573191A9889A262c0Efc0983);
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSale, bool isBuy ) private view returns (valuesFromGetValues memory s) {
        ...
        if( isSale ) {
            ...
            s.tFee = tAmount * sellFee / 1000;
            s.tTransferAmount = tAmount - s.tFee;

        } else if ( isBuy ) {

            s.tFee = tAmount * buyFee / 1000;
            s.tTransferAmount = tAmount - s.tFee;

        } else {

            s.tFee = tAmount * transferFee / 1000;
            s.tTransferAmount = tAmount - s.tFee;
        }
    ...
    function _tokenTransfer( address sender, address recipient, uint256 tAmount, bool takeFee, bool isSale, bool isBuy ) private {
        ...
        balance[recipient] = balance[recipient] + s.tTransferAmount;

        //Dev wallet
        if( s.tFee > 0 ) {
            balance[devWallet] = balance[devWallet] + s.tFee;
        }
      
        emit Transfer(sender, devWallet, s.tFee);
    ...
    /// @dev Update router address in case of swap migration
