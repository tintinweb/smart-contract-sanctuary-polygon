...
contract Expedition is Ownable, ReentrancyGuard {
    ...
    address public witdrawFeeWallet;
    ...
    constructor(
        ...
        bool _doMint,
        address _lockWallet,
        address _witdrawFeeWallet
    ) public {
        stakingToken = _stakingToken;
        doMint = _doMint;
        lockWallet = _lockWallet;
        witdrawFeeWallet = _witdrawFeeWallet;
    }
    ...
    function changeWithdrawFeeWalletAddress(address _witdrawFeeWallet) external onlyOwner {
        witdrawFeeWallet = _witdrawFeeWallet;
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if(pool.withdrawFeePercentage > 0){
                uint256 principalFee = _amount.div(100).mul(pool.withdrawFeePercentage); 
                uint256 principalLeft = _amount.sub(principalFee);
                pool.lpToken.safeTransfer(witdrawFeeWallet, principalFee);// lockWallet update
    ...
    function safeStakingTokensTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > stakingTokenBalanceOnChef) {
            (bool success1) = stakingToken.transfer(_to, stakingTokenBalanceOnChef);
