...
contract QuestMvp is Pausable, AccessControl, ChainlinkClient {
    ...
    function proposeNewGig(
        ...
        {
            require(suMultisigs.exist(gigSuAddress), "The SU address is not valid.");
            ...
            for (uint256 i = 0; i < _participants.length; i++) {
                require(allRMS.exist(_participants[i]), "A participant is not an RM.");
            }
        }
    ...
    function reproposeGig(
...

        {
            ...
            require(suMultisigs.exist(newGig.suAddress), "SU address is not valid");
        }
        ...
        require(block.timestamp < gigList[_gigIndex].responseDeadline, "The acceptance deadline has been exceeded.");
        ...
        validIndex(_gigIndex)
    {
        ...
        require(outcome == GigResolution.InDispute, "The Gig must be disputed.");
    ...
    function updateGigTime(
        ...
        require(hasRole(ACCOUNT_EXEC_ROLE, msg.sender), "You must hold ACCOUNT_EXEC role.");
    ...
    function _contractSpendFrom(
        ...
        IERC20 rainToken = IERC20(rainTokenAddress);
        require(rainToken.balanceOf(from) >= value, "The sender has insufficient balance.");
        if (value > 0) {
