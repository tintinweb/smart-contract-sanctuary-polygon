    ...
    EIP712Upgradeable
{
    ...
    //     _price = _numPurchases * s_ERC1155tokenPaymentPrices[_saleId][_paymentTokenId];
    ...
    function getOwnershipDiscounts(uint256 _saleId) external view returns(OwnershipDiscount[] memory) { 
        return s_ownershipDiscounts[_saleId];
    }

    function getOracle() external view returns(IOracleManager) {
        return s_oracle;
    }
...
contract ERC1155StoreGenericDataProvider is IERC1155StoreGenericDataProvider {
  ...
  function checkPrice(
      ...
      uint256 discountPrice;
      if (_tokenAddress != address(0)) {
          IERC1155StoreGeneric.OwnershipDiscount[] memory ownershipDiscounts = storeContract.getOwnershipDiscounts(_saleId);
          uint256 discount;
          for (uint256 i; i < ownershipDiscounts.length; ++i) {
              if (
                  ownershipDiscounts[i].tokenAddress == _tokenAddress &&
      ...
      if (currency != defaultCurrency) {
          salePrice = storeContract.getOracle().getAmountOut(defaultCurrency, currency, salePrice);
      }
      _price = salePrice;
  }
