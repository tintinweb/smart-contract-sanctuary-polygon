...
contract Rover is ERC721EnumerableUpgradeable, OwnableUpgradeable, ERC1155HolderUpgradeable {
    ...
    IVRFManager private s_randNumOracle;
    ...
    /// @notice sets default uris for rovers
    function setUris(string[] calldata _uris) public {
        uint256 length = _uris.length;
        for (uint256 i = 0; i < length; ) {
            uris[i] = _uris[i];
            unchecked {
     ...
     */
    function setVRFOracle(address _vrfOracle) public onlyOwner {
        if (_vrfOracle == address(0)) revert Rover__ZeroAddress();
        s_randNumOracle = IVRFManager(_vrfOracle);
    }
