...
// File: contracts/testomni-poly.sol

pragma solidity ^0.8.7;

contract testomni is Ownable, ERC721, NonblockingReceiver {
    address public _owner;
    string private baseURI;
    uint256 nextTokenId = 11;
    uint256 MAX_MINT_POLYGON = 33;
    bool paused = true;
    uint256 maxMint = 2;
    ...
    function freemint(uint8 numTokens) external {
        require(!paused, "Mint is on pause");
        require(numTokens <= maxMint, "test omni: Max 2 NFTs per transaction");
        require(
            numTokens + balanceOf(msg.sender) <= maxMint,
            "test omni: Max wallet limit reached (2)"
        );
        require(
            nextTokenId + numTokens <= MAX_MINT_POLYGON,
            "test omni: Mint exceeds supply"
        );
    ...
    function traverseChains(uint16 _chainId, uint256 tokenId) public payable {
        ...
        require(
            ...
            "test omni: msg.value not enough to cover messageFee. Send gas for message fees"
        );
    ...
    function withdraw(uint256 amt) external onlyOwner {
        ...
        require(sent, "test omni: Failed to withdraw Ether");
    }
