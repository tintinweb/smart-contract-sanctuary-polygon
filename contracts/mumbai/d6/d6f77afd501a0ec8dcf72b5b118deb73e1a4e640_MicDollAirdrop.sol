...
contract MicDollAirdrop is Pausable {
  ...
  int public micDollClaimedCount;
  ...
  function getMicDollRemainCount() external view returns (uint){
    if(micDollContractAdress == address(0))
      ...
      return IERC721(micDollContractAdress).balanceOf(address(this));
  }
  ...
  function claim() external whenNotPaused{
    ...
    require(IERC721(micDollContractAdress).balanceOf(address(this)) != 0, 'There are no airdrop micdoll is available now');
    require(recipients[msg.sender] == true, 'recipient not registered');
    recipients[msg.sender] = false;
    uint tokenId = IERC721Enumerable(micDollContractAdress).tokenOfOwnerByIndex(address(this),0);
    IERC721(micDollContractAdress).transferFrom(address(this), msg.sender, tokenId);
    micDollClaimedCount++;
