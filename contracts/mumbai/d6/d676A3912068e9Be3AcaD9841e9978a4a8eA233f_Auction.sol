...
contract Auction is EIP712MetaTransaction, ReentrancyGuard {
  ...
  // Receive user-transferred gas fee
    ...
    verifyPasscode(_passcode)
  {
    ...
    emit AuctionClaimed(_id, auction.tokenId, msgSender(), block.timestamp);
  }
    ...
    verifyPasscode(_passcode)
  {
    ...
    if (auction.status == AuctionStatus.Bid) {
      _send(auction.highestBid, auction.bidder);
    }
  ...
  function _transferBidAndFee(
    ...
    weth.safeTransferFrom(_bidder, beneficiary, _fee);
  }
