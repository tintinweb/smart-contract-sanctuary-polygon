...
contract Cyber9mumbai is ERC721Enumerable, Ownable, RNG, VRFConsumerBase {
    ...
    modifier status(uint256 _tokenId) {
        // require(isStaked[_from][_tokenId] == false, "Staked");
    ...
    function spawnEnemy() external {
        ...
        int256 hp = int(avg * 200) - walletOfOwner(msg.sender).c7Bonus();
        uint256 enemyAttack = avg + 10;
    ...
    // function tokenStatus(uint256 _tokenId) external view returns (bool) {
...
library ClanLib {
    ...
    function c9Bonus(uint256[] memory _ownerTokens) public pure returns(uint256 bonusExp) {
        uint256 _c9Count; 
        for(uint256 i; i<_ownerTokens.length;i++) {
            if((_ownerTokens[i] > 8000 && _ownerTokens[i] < 90001) || _ownerTokens[i] == 9) {
               _c9Count++; 
           }
        }
