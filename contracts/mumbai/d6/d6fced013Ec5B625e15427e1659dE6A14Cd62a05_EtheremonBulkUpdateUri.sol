...
contract EtheremonBulkUpdateUri is BasicAccessControl {
    mapping(uint32 => bool) neglectedClasses;

    address public monsterNFTContract;
    address public dataContract;
    string public metaUriPrefix = 'https://ethermon.io/api/matic/monster/get_data?monster_ids=';
    ...
    function setContracts(address _dataContract, address _monsterNFTContract) public onlyModerators {
    ...
    function checkNeglectedToken(uint32 _classId)
        ...
        returns (bool found)
    {
        found = neglectedClasses[_classId];
    }

       function getClassId(uint64 _tokenId)
        external
        view
        returns (uint32 _classId)
    {
    ...
    function setBulkTokenURI(
        ...
        for (; _startMonId <= _endMonID; _startMonId++) {
            ...
            if (neglectedClasses[obj.classId] == false) {
    ...
    function getTokenURI(uint64 _tokenId)
        external
        view
