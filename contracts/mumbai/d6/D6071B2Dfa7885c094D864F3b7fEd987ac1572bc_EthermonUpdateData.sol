...
contract EthermonUpdateData is EthermonEnum, BasicAccessControl {
    ...
    uint8 public constant STAT_MAX = 32;
    uint8 public constant GEN0_NO = 24;
    uint public burnPriceEMON = 1 * 10**12;
    IERC20 public emon;
    ...
    function setPrice(uint _price) external onlyModerators {
        if (_price <= 0) revert();
        burnPriceEMON = _price * 10**12;
    }
    ...
    */
    function reRollMonsterStatsEMON(uint32 _tokenId) external isActive {
        ...
        uint256 finalPrice = uint256((uint256(class.price) / uint256(burnPriceEMON)) * 10**18);
