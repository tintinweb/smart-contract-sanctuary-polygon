...
contract SynthrBridgeLightChain is BaseBridge {
    ...
    function sendDepositCollateral(
        ...
        _broadcast(account, lzPayload, PT_DEPOSIT_COLLATERAL, _mainChainId, msg.value);
    }
    ...
    function sendBurn(
        ...
        _broadcast(account, lzPayload, PT_BURN_SYNTH, _mainChainId, msg.value);
    }
    ...
    function sendExchange(
        ...
        if (dstChainId != _selfLZChainId) {
            uint consumed = _broadcast(account, lzPayload, PT_EXCHANGE, dstChainId, value);
            value -= consumed;
        }

        if (dstChainId != _mainChainId) {
            _broadcast(account, lzPayload, PT_EXCHANGE, _mainChainId, value);
        }
    }
    ...
    function sendBridgeSyToken(
        ...
        require(dstChainId != 0, "Can not bridge tokens on the same chain");

        bytes memory lzPayload = abi.encode(PT_BRIDGE_SYNTH, abi.encodePacked(account), synthKey, amount, dstChainId);

        uint value = msg.value;
        uint consumed = _broadcast(account, lzPayload, PT_BRIDGE_SYNTH, dstChainId, value);
        value -= consumed;

        if (dstChainId != _mainChainId) {
            _broadcast(account, lzPayload, PT_BRIDGE_SYNTH, _mainChainId, value);
        }
    }

    /**
    * This function is on only the light chain
    * as we don't need to bridge `transfer margin` message to the other chain 
    */
    function sendTransferMargin(address account, uint256 marginDelta) external payable onlySynthr {
        require(marginDelta > 0, "Margindelta should be greater than 0");
        bytes memory lzPayload = abi.encode(PT_TRANSFER_MARGIN, abi.encodePacked(account), marginDelta);
        _broadcast(account, lzPayload, PT_TRANSFER_MARGIN, _mainChainId, msg.value);
    }
}