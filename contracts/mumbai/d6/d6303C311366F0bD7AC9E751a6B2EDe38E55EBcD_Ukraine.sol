...
contract Ukraine is ERC721Enumerable, Ownable, ReentrancyGuard {
    using Strings for uint256;
    using ECDSA for bytes32;

    // constants
    uint256 public publicSaleNFTPrice = 0.08 ether;
    uint256 public maxNFTPurchase = 10000; 
    ...
    event SetPublicSaleNFTPrice(uint256 publicSaleNFTPrice);
    event SetMaxNFTPurchase(uint256 maxNFTPurchase);
    event SetPublicSaleStartTime(uint256 publicSaleStartTime);
    event SetSignerAddress(address signerAddress);
    event MintToken(address indexed operator, uint256[] ids, uint256 price, uint256 nonce);
    ...
    function mint(
        ...
        bytes32 hash = hashTransaction(msg.sender, tokenId, nonce);
        require(_signerAddress != address(0), "Signer address not set");
        ...
        require(
            tokenId.length <= maxNFTPurchase,
            "Can only mint up to 5 per purchase"
        );
        require(
            publicSaleNFTPrice * tokenId.length == msg.value,
        ...
        emit MintToken(msg.sender, tokenId, publicSaleNFTPrice * tokenId.length, nonce);
    }
    ...
    function setDisabled(bool _disabled) external onlyOwner {
     ...
     // @notice This function sets the public sale nft price
    function setPublicSaleNFTPrice(uint256 _publicSaleNFTPrice) external onlyOwner {
        publicSaleNFTPrice = _publicSaleNFTPrice;
        emit SetPublicSaleNFTPrice(_publicSaleNFTPrice);
    }

    // @notice This function sets the max nft 
    function setMaxNFTPurchase(uint256 _maxNFTPurchase) external onlyOwner {
        maxNFTPurchase = _maxNFTPurchase;
        emit SetMaxNFTPurchase(_maxNFTPurchase);
    }
    ...
    function hashTransaction(
        ...
        string memory idString = id2String(ti);
        bytes32 hash = keccak256(
            abi.encodePacked(
                "\x19Ethereum Signed Message:\n32",
                keccak256(abi.encodePacked(sender, idString, nonce))
            )
        );
    ...
    function id2String(
        ...
        bytes memory idString = "";
        for(uint i = 0; i < ti.length; i++){
            idString = abi.encodePacked(idString, ti[i].toString(),",");
        }
