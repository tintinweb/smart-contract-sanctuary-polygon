...
contract CentenarioCelta is ERC721, ERC721Enumerable, ERC721Burnable, AccessControl {
    ...
    event debug(uint256 sentValue, uint256 expectedValue);
    ...
    function getPriceInWei() public view returns (uint256 weiPrice){
        return priceInDolar * 1e24 / uint256(getLatestPrice());
    }

    function crossmint(address _to) external payable {
        uint256 variance = getPriceInWei() * slippage/100;
        // require(getPriceInWei() - variance <= msg.value, "Value send to high");
        // require(getPriceInWei() + variance >= msg.value, "Value send to low");
