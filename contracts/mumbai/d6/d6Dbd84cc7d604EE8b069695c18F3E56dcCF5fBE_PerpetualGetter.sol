...
contract PerpetualGetter is PerpetualUpdateFunctions, IFunctionList, IPerpetualGetter {
    ...
    function getPerpetualCountInPool(uint8 _poolId) external view override returns (uint8) {
        return uint8(perpetualIds[_poolId].length);
    }

    function getPerpetualId(uint8 _poolId, uint8 _perpetualIndex)
        external
        view
        override
        returns (uint24)
    ...
    function getPoolIdByPerpetualId(uint24 _perpetualId) external view override returns (uint8) {
    ...
    function getLiquidityPool(uint8 _id)
        ...
        view
        override
        returns (LiquidityPoolData memory)
    {
    ...
    function isActiveAccount(uint24 _perpetualId, address _account)
        external
        view
        override
        returns (bool)
    ...
    function countActivePerpAccounts(uint24 _perpId)
        external
        view
        override
    ...
    function getAMMState(uint24 _iPerpetualId) external view override returns (int128[13] memory) {
        int128[13] memory ammState;
        PerpetualData storage perpetual = _getPerpetual(_iPerpetualId);
        MarginAccount storage ma = marginAccounts[_iPerpetualId][address(this)];
        ...
        ammState[6] = _getOraclePrice([perpetual.S2BaseCCY, perpetual.S2QuoteCCY]);
        if (perpetual.S3BaseCCY[0] != bytes4(0)) {
            ammState[7] = _getOraclePrice([perpetual.S3BaseCCY, perpetual.S3QuoteCCY]);
        }
        ...
        if (lp.fTargetDFSize > 0) {
            ammState[11] = lp.fDefaultFundCashCC.div(lp.fTargetDFSize);
        }
        ...
        returns (int128[12] memory)
    {
        /*
            ...
            10 S2
            11 maintenance margin rate
        */

        int128[12] memory traderState;
        PerpetualData storage perpetual = _getPerpetual(_iPerpetualId);
        int128 b0 = _getMarginViewLogic().getMarginBalance(perpetual.id, _traderAddr);
        traderState[0] = b0;
        ...
        MarginAccount storage ma = marginAccounts[_iPerpetualId][_traderAddr];
        ...
        int128 fMarkPrice = _getPerpetualMarkPrice(perpetual);
        int128 fLvg;
        if (ma.fPositionBC != 0) {
            fLvg = ma.fPositionBC.abs().mul(fMarkPrice).div(fS3).div(b0);
        }
        traderState[7] = fLvg;
        traderState[8] = fMarkPrice;
        traderState[9] = fS3;
        traderState[10] = _getSafeOraclePriceS2(perpetual);
        traderState[11] = int128(perpetual.fMaintenanceMarginRate) << 35;
        return traderState;
    }

    function getOraclePrice(bytes4[2] memory _baseQuote)
        external
        view
        override
        returns (int128 fPrice)
    {
        return PerpetualBaseFunctions._getOraclePrice(_baseQuote);
    }
    ...
    function getBrokerDesignation(uint8 _poolId, address brokerAddr)
        ...
        returns (uint32)
    {
        return brokerMap[_poolId][brokerAddr];
    }
    ...
    function getFunctionList() external pure virtual override returns (bytes4[] memory, bytes32) {
        ...
        functionList[7] = this.getPerpetualCountInPool.selector;
        functionList[8] = this.getAMMPerpLogic.selector;
        functionList[9] = this.getOraclePrice.selector;
        functionList[10] = this.getUpdatedTargetAMMFundSize.selector;
        ...
        functionList[19] = this.getBrokerDesignation.selector;
        functionList[20] = this.getOracleFactory.selector;
