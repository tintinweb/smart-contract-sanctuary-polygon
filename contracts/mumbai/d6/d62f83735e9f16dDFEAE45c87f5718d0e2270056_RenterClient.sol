...
//        uint private constant rentingRenewalPeriodGap = 1 minutes;
    ...
    function rentBattleSet(uint256 landId) nonReentrant whenNotPaused external {
        ...
        if (li.rentingType == RentingType.FIXED_PRICE && !paymentContract.transferFrom(_msgSender(), li.owner, li.price)) {
    ...
    function rentFromCollection(uint256 id, uint256 landId, uint256[] memory botsIds) nonReentrant whenNotPaused external returns (bool) {
        ...
        if (collection.rentingType == RentingType.FIXED_PRICE &&
            !IERC20(paymentContracts[collection.chargeCoin]).transferFrom(_msgSender(), collection.owner, collection.price)) {
            return false;
        }
    ...
    function renewRental(uint256 landId) nonReentrant whenNotPaused external {
        ...
        if (ri.rentingType == RentingType.FIXED_PRICE && !IERC20(paymentContracts[ri.chargeCoin]).transferFrom(_msgSender(), ri.owner, ri.price)) {
