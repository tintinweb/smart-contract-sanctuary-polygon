...
contract WowTQuiz is PausableUpgradeable, AccessControlUpgradeable {
  ...
  function createQuiz(
    ...
    quizmap[_quizName].creatorAddress = _userAddress;
    if (!(hasRole(ADMIN_ROLE, _userAddress))) {
  ...
  function setCommunityContract(address _communityContract) external adminOnly {
    community = WowTCommunity(_communityContract);
  }

  function setPointsContract(address _pointsContract) external adminOnly {
    points = WowTPoints(_pointsContract);
  }
