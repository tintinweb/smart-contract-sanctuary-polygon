...
error LiquidityWars__NotEnoughResources(
    uint256 actualResources,
    uint256 resourcesRequired
);
error LiquidityWars__NotEnoughTroops();
error LiquidityWars__CannotAttackYourself();
...
contract LiquidityWars is AutomationCompatibleInterface, Ownable {
    ...
    event ResourcesUpdated(uint256 indexed gameId);
    event RakingUpdated(
    ...
    function _ranking() private {
        ...
        for (uint256 i = 0; i < players.length; i++) {
            uint256 index = _findArrayIndex(
                playerResources,
                sortedPlayerResources[i]
            );
    ...
    function _sort(uint256[] memory arr)
        private
        pure
        returns (uint256[] memory)
    {
        uint256[] memory copyArr = new uint256[](arr.length);
        for (uint256 i = 0; i < arr.length; i++) {
            copyArr[i] = arr[i];
        }
        _quickSort(copyArr, int256(0), int256(copyArr.length - 1));
        return copyArr;
    }
    ...
    function _findArrayIndex(uint256[] memory array, uint256 value)
        private
        pure
        returns (uint256)
    {
    ...
    function trainTroops(uint256 _numberOfTroops) external onlyRunning {
        ...
        if (_player.resources <= totalCosts) {
            revert LiquidityWars__NotEnoughResources(
                _player.resources,
                totalCosts
            );
        }
    ...
    function attackPlayer(address _playerToAttack) external onlyRunning {
        ...
        if (attacker.troops.number == 0) {
            revert LiquidityWars__NotEnoughTroops();
        }
        if (msg.sender == _playerToAttack) {
            revert LiquidityWars__CannotAttackYourself();
        }
        ...
        uint256 robbedResources;
        console.log("Deductiong resources...");
        _deductResources(costOfAttack, playerParams[msg.sender]);
        console.log("Requesting for battle factors...");
        liquidityVrfContract.requestForBattleFactors();
        (attackerFactor, defenderFactor, numberOfCombats) = liquidityVrfContract
            .getRandomFactorsForBattle();
        console.log(
            "For loop with following battle factors:\nattacker: %s\tdefender: %s\tnumberOfCombats: %s\t",
            attackerFactor,
            defenderFactor,
            numberOfCombats
        );
        for (uint256 idx = numberOfCombats; idx > 0; idx--) {
            console.log(
                "1.Attacker total health: %s",
                attackerTotalTroopHealth
            );
            console.log(
                "1.Defender total health: %s",
                defenderTotalTroopHealth
            );
            if (
                attackerTotalTroopHealth <
                ((defenderTroopDefense *
                    defenderTroopsSurvived *
                    defenderFactor) / 10)
            ) {
                attackerTotalTroopHealth = 0;
                attackerTroopsSurvived =
                    ((attacker.troops.health - 1) + attackerTotalTroopHealth) /
                    attacker.troops.health; // round up
            } else {
            ...
            console.log(
                "2.Attacker total health: %s",
                attackerTotalTroopHealth
            );
            console.log(
                "2.Defender total health: %s",
                defenderTotalTroopHealth
            );
            if (
                defenderTotalTroopHealth <
                ...
                defenderTotalTroopHealth = 0;
                defenderTroopsSurvived =
                    ((defender.troops.health - 1) + defenderTotalTroopHealth) /
                    defender.troops.health; // round up
            } else {
            ...
            console.log(
                "3.Attacker total health: %s",
                attackerTotalTroopHealth
            );
            console.log(
                "3.Defender total health: %s",
                defenderTotalTroopHealth
            );
        }
    ...
    function getTroopAttributes(address _playerAddress)
        external
        view
        returns (Troop memory)
    {
        return playerParams[_playerAddress].troops;
    }
...
error LiquidityVrf__NotLiquidityWarsCall();
...
contract LiquidityWarsVrf is VRFConsumerBaseV2, Ownable {
    ...
    modifier onlyLiquidityWars() {
        address liquidityWarsAddress = _assignLiquidityWarsAddress();
        if (msg.sender != liquidityWarsAddress) {
            revert LiquidityVrf__NotLiquidityWarsCall();
        }
        _;
    }
     ...
     * @param linkAddress - the subscription ID that this contract uses for funding requests
    ...
    constructor(
        ...
        requestRandomWords();
        configContract = LiquidityWarsConfig(configAddress);
    }

    function _assignLiquidityWarsAddress() private view returns (address) {
        return configContract.getLiquidityWarsAddress();
    }

    /**
     * @notice Requests randomness
     * Assumes the subscription is funded sufficiently; "Words" refers to unit of data in Computer Science
     * Teves: in order for the chainlink tests to pass it is necessary to change the visibility to public
     */
    ...
    function requestForBattleFactors() external onlyLiquidityWars {
        if (!s_requestPending) {
            requestRandomWords();
        }
    }
    ...
    {
        attackerFactor = ((s_randomWord & 0xFF) % 6) + 7;
        defenderFactor = (((s_randomWord >> 2) & 0xFF) % 6) + 8;
        numberOfCombats = (((s_randomWord >> 4) & 0xFF) % 4) + 2;
    }
...
contract LiquidityVault is Ownable, AutomationCompatibleInterface {
     ...
     *
     */
    function startTheGame() private onlyCalibrated onlyReady {
     ...
     */
    function endTheGame() private onlyRunning {
        address[] memory allowedTokens = configContract.getAllowedTokens();
    ...
    function depositLpToken(address _tokenAddress, uint256 _amount)
        ...
        onlyReady
    {
        console.log("Getting Amount of LPs");
    ...
    function claimRewardTokensFromProtocol() public returns (uint256) {
