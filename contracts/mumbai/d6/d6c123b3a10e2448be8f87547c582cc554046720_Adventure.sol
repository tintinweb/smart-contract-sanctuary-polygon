    ...
    IAdventure
{
    ...
    uint256 public constant REVIVE_PRODUCT = 31;
    uint256 public constant FINISH_IMMEDIATELY_PRODUCT = 32;
    uint256 public constant ONE_HOUR_POTION = 16711718;
    uint256 public constant THREE_HOUR_POTION = 16711782;
    uint256 public constant SIX_HOUR_POTION = 16711816;

    struct Pass {
        uint256 locId;
        uint256 ottoId;
        uint256 departuredAt;
        uint256 canFinishedAt;
        uint256 finishedAt;
        uint256 seed;
        bool success;
        bool revived;
    ...
    modifier validPercentage(uint256 percentage_) {
        require(percentage_ < ONE_HUNDRED_PERCENT, 'invalid percentage');
        _;
    }

    modifier onlyPotionEffectManager() {
        require(
            hasRole(POTION_EFFECT_MANAGER_ROLE, msg.sender),
            'not potion effect manager'
        );
        _;
    }

    modifier isOttoReady(uint256 ottoId_) {
        require(OTTO.ownerOf(ottoId_) == msg.sender, 'not otto owner');
        require(
    ...
    modifier validPassForOtto(uint256 ottoId_) {
        require(
    ...
    modifier canOttoRevive(uint256 ottoId_) {
        require(
            OTTO.ownerOf(ottoId_) == msg.sender,
            'caller is not otto owner'
        );
        require(
    ...
    function departure(
    ...
    ) external onlyEOA isOttoReady(ottoId_) {
        ...
        p.locId = locId_;
        p.ottoId = ottoId_;
        p.departuredAt = block.timestamp;
        p.canFinishedAt = block.timestamp + duration_;
        p.seed = uint256(bytes32(bytes(sig_.nonce)));
        emit Departure(passId, ottoId_);
    }

    function _handleImmediatelyFee(uint256 passId_) private {
        require(
            STORE.discountPriceOf(FINISH_IMMEDIATELY_PRODUCT) ==
                10**CLAM.decimals(),
            'fee decimals mismatch'
        );
        Pass memory p = _passes[passId_];
        uint256 amount = 15;
        uint256 remain = p.canFinishedAt - block.timestamp;
        if (remain <= 1 hours) {
            amount = 1;
        } else if (remain <= 2 hours) {
            amount = 2;
        } else if (remain <= 3 hours) {
            amount = 3;
        } else if (remain <= 4 hours) {
            amount = 4;
        } else if (remain <= 5 hours) {
            amount = 5;
        } else if (remain <= 6 hours) {
            amount = 6;
        } else if (remain <= 7 hours) {
            amount = 7;
        } else if (remain <= 8 hours) {
            amount = 8;
        } else if (remain <= 9 hours) {
            amount = 9;
        ...
        uint256 fee = amount * 10**CLAM.decimals();
        CLAM.transferFrom(msg.sender, address(this), fee);
        CLAM.approve(address(STORE), fee);
        STORE.buy(msg.sender, FINISH_IMMEDIATELY_PRODUCT, amount);
    }

    function usePotions(uint256 ottoId_, uint256[] calldata potions_)
        public
        validPassForOtto(ottoId_)
    {
        ...
        for (uint256 i = 0; i < potions_.length; i++) {
            require(
                potions_[i] == ONE_HOUR_POTION ||
                    potions_[i] == THREE_HOUR_POTION ||
                    potions_[i] == SIX_HOUR_POTION,
                'not allowed potion'
            );
            ITEM.safeTransferFrom(
                msg.sender,
    ...
    function finish(
        ...
        Signature calldata sig_
    ) external onlyEOA validPercentage(sr_) validPassForOtto(ottoId_) {
        bytes32 hash_ = keccak256(
            abi.encodePacked(
                msg.sender,
                address(this),
        ...
        if (immediately_) {
            _handleImmediatelyFee(passId);
            p.canFinishedAt = block.timestamp;
        } else if (potions_.length > 0) {
            usePotions(ottoId_, potions_);
        }

        require(block.timestamp >= p.canFinishedAt, 'not finished');
    ...
    function revive(uint256 ottoId_) external onlyEOA canOttoRevive(ottoId_) {
        uint256 passId = _ottoPass[ottoId_];
        Pass storage p = _passes[passId];
        uint256 price = STORE.discountPriceOf(REVIVE_PRODUCT);
        CLAM.transferFrom(msg.sender, address(this), price);
        CLAM.approve(address(STORE), price);
        STORE.buy(msg.sender, REVIVE_PRODUCT, 1);
        p.revived = true;
        _award(p.ottoId);
    ...
    function _award(uint256 ottoId_) private {
        uint256 passId = _ottoPass[ottoId_];
        Pass storage p = _passes[passId];
        uint256[] memory values = new uint256[](p.rewards.items.length);
        for (uint256 i = 0; i < p.rewards.items.length; i++) {
            values[i] = 1;
        }
        ITEM.mintBatch(msg.sender, p.rewards.items, values, '');
    ...
    function canFinishAt(uint256 ottoId_) public view returns (uint256) {
        Pass memory p = latestPassOf(ottoId_);
        return p.canFinishedAt;
    }

    function shortenDuration(uint256 ottoId, uint256 sec)
        external
        onlyPotionEffectManager
    {
        Pass storage p = _passes[_ottoPass[ottoId]];
        p.canFinishedAt = p.canFinishedAt - sec;
    }

    function shortenCooldown(uint256 ottoId, uint256 sec)
        external
        onlyPotionEffectManager
    {
        require(block.timestamp < restingUntil[ottoId], 'not resting');
