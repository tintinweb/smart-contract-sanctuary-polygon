...
contract TestNFT is ERC721Psi, Ownable, ReentrancyGuard, WhiteList {
    ...
    uint256 public constant MAX_PRESALE_TOKENS = 400;
    ...
    function mint(uint256 amount, bytes32[] memory proof) external payable {
        require(preSaleIsActive || publicSaleIsActive, 'No active sale');
        ...
        require(publicSaleIsActive || totalSupply() + amount <= MAX_PRESALE_TOKENS, 'Would reach max presale supply');
        ...
        require(balanceOf(msg.sender) + amount <= perAddressLimit, 'Address would exceed balance limit');
        ...
        // Whitelist related
        require(!whitelistIsActive || verifyMerkleProof(proof), 'Address not whitelisted');

        // The actual mint
        _safeMint(msg.sender, amount);

        emit Mint(amount, proof);
    }
    ...
    function flipPreSaleState() external onlyOwner {
        ...
        emit FlipPreSaleState(preSaleIsActive);
    }

    function flipPublicSaleState() external onlyOwner {
        publicSaleIsActive = !publicSaleIsActive;
        emit FlipPublicSaleState(publicSaleIsActive);
    }

    function flipWhitelistIsActive() external override onlyOwner {
