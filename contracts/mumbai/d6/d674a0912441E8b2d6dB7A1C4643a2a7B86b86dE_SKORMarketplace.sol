...
contract SKORMarketplace is Ownable, ReentrancyGuard {
    uint256 public skorPercentageCut;
    ...
    function setInitData(
        ...
        uint256 _skorPercentageCut
    ) public {
        platformLazyNFT = _platformLazyNFT;
        royaltyStorage = _royaltyStorage;
        skorPercentageCut = _skorPercentageCut;
    }
    ...
    function createBatchNftAuction(
        ...
        string[] memory _uri
    ) external onlyOwner batchWithinLimits(_batchTokenIds.length) {
        require(
    ...
    function _payout(
        ...
        _amount = _amount - (_amount * skorPercentageCut) / 10000;
        if(auctionERC20Token == address(0)){
    ...
    /// @dev When skor token is transferred then there is an inherrent tax that
    /// is sent to other wallet
    function setSkorPercentageCut(uint256 _skorPercentageCut)
        external
        onlyOwner
    {
        skorPercentageCut = _skorPercentageCut;
