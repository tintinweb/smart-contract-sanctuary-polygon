...
contract SoulFlake is ERC721, ERC721Enumerable, AccessControl, Ownable {
  ...
  uint256 counter = 100000;

  //This stores the total number of themes  / occassions
  uint256 theme_counter = 0;
  
  //This stores the total number of flake designs 
  uint256 design_counter = 0;
  ...
  // This maps the design id to a flakedesign 
  mapping(uint256 => FlakeDesign) public flakedesigns;

  // This maps the theme id to a theme name 
  mapping(uint256 => string) public flakethemes;
  ...
  function mint(
    ...
    require(theme < theme_counter, "The chosen theme is unavailable."); // the theme has to be one of the available themes
    require(msg.value - price <= 100*10**18, "Gift value cannot exceed 100 MATIC."); // capping gift to 100 MATIC + fee
    require(design_id < design_counter, "The chosen design is unavailable."); //the design has to be one of the available designs
    ...
    soulflakes[counter].color = "None"; //getColor(theme);
    soulflakes[counter].theme = theme;
    soulflakes[counter].themename = getTheme(theme);
    soulflakes[counter].design = design_id;
    soulflakes[counter].title = title;
    soulflakes[counter].gift = msg.value - price;

    _mint(to, counter);
    (bool result, ) = payable(to).call{ value: msg.value }("");
    require(result, "Mint failed and token counter will not increment." );

    counter++;
  }

  function addDesign(string calldata name, string calldata artist, string calldata design) public onlyRole(DEFAULT_ADMIN_ROLE){
    flakedesigns[design_counter].artname = name;
    flakedesigns[design_counter].artist = artist;
    flakedesigns[design_counter].artwork = design;
    design_counter++;
  }

  function getDesign(uint256 design_id) public view returns (string memory){
    return flakedesigns[design_id].artwork;
  }
  ...
  function addTheme(string calldata theme) public onlyRole(DEFAULT_ADMIN_ROLE){
    flakethemes[theme_counter]= theme;
    theme_counter++;
  }
  ...
  /* function getTheme(uint _themeid) public pure returns (string memory) {
    string[6] memory themes = ["LOVE","GRATITUDE","CHEERS","RESPECT","SUCCESS","HAPPY VALENTINES DAY"];
    return themes[_themeid];
  }*/
  function getTheme(uint256 theme_id) public view returns (string memory){
    return flakethemes[theme_id];
  }
  ...
  function getSvg(uint256 tokenId) public view returns (string memory) {
    ...
    parts[0] = "<foreignObject x='50' y='80' width='412' height='382'><div xmlns= 'http://www.w3.org/1999/xhtml' > <p id='text' class='bodyText'> soulflake </p></div><script>var input = '";
    parts[1] = Base64.encode(abi.encodePacked(soulflakes[tokenId].message));
    parts[2] = "'; var test = atob(input); document.querySelector('.bodyText').innerText = decodeURIComponent(escape(test));</script></foreignObject></svg>"; //<foreignObject x='50' y='50' width='412' height='30'><div xmlns= 'http://www.w3.org/1999/xhtml' > <p id='themetext' class='themeText'> soulflake </p></div><script>var input = '";
    //parts[5] = Base64.encode(abi.encodePacked(getTheme(soulflakes[tokenId].theme)));
    //parts[6] = "'; var test = atob(input); document.querySelector('.themeText').innerText = test;</script></foreignObject></svg>";
    ...
    return
      Base64.encode(
        abi.encodePacked(
          ...
          //parts[1],
          //style,
          getDesign(soulflakes[tokenId].design),
          parts[0],
          parts[1],
          parts[2]
          //parts[5],
  ...
  function burn(uint256 tokenId) public {
    ...
    require(msg.sender == ownerOf(tokenId),"Cannot burn since sender is not token owner." );
    ...
    returns (string memory)
  {
    ...
    string memory json = Base64.encode(
      abi.encodePacked(
        ...
        gift > 0 ? "Yes":"No",
        '"},{"trait_type": "Theme","value":"', soulflakes[tokenId].themename,
