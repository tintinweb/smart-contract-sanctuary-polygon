  ...
  ReentrancyGuardUpgradeable
{
  ...
  function createSale(
    ...
    if (msg.sender != address(_artfiFixedPrice)) revert GeneralError("AF:109");
  ...
  function mintNft(
    ...
    ArtfiICollectionV2.MintData memory mintDataCollection_ = ArtfiICollectionV2
      .MintData(
        mintData_.uri,
        minter,
        mintData_.creators,
    ...
    emit eMint(tokenId_, mintData_.tokenAddress, minter, mintData_.quantity);
  }
  ...
  function lazyMintSellNft(
    ...
    if (lazyMintSellData_.seller == msg.sender) revert GeneralError("AF:305");
    if ((_artfiManager.isBlocked(msg.sender) == true))
      revert GeneralError("AF:126");
    // if (
    ...
    address signerV2 = _artfiManager.verifyFixedPriceLazyMintV2(
      lazyMintSellData_
    );
    if (lazyMintSellData_.seller != signerV2) revert GeneralError("AF:306");
  ...
  function _lazyMint(
    ...
    );
    {
      ...
      paymentAmount[i] = (lazyMintSellData_.minPrice.mul(purchaseQuantity)).sub(
        stakeRoyaltySum
      );
      i = i + 1;
    }
    ...
    if (
      ...
      if (
        payment <
        (serviceFee.add((lazyMintSellData_.minPrice).mul(purchaseQuantity)))
      ...
      if (!_artfiFixedPrice.isSaleSupportedTokens(lazyMintSellData_.currency))
  ...
  function _transferNFT(
    ...
    (ContractType contractType, bool isOwner, uint256 quantity) = _artfiManager
      .isOwnerOfNFT(from_, tokenId_, tokenAddress_);
