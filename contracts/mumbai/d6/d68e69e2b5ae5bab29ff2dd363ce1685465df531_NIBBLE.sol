...
//Questions to ask:
//1. Stages of token sale (4 stages) - how to implement?
//2. Max wallet balance - is for for all wallets? add exclude list?
//3. do users buy tokens directly from the contract?
//4. are there whitelisted addresses that can buy tokens?
//5. how to implement cooldowns? after every transaction? or after every buy/sell?
//6. how to implement tax? after every transaction? or after every buy/sell?
//7. how to implement max total supply for stages? is each stage hard capped? or is it a soft cap?
//8. core team tokens - vesting contract that locks tokens for 1 year
//9. Tax fee can be changed manually change it by the owner || have iit change automatically based on the stage of the token sale
//10. how to implement the multisig wallet? is it a multisig wallet that can be changed by the owner?
//11. how to implement the voting power? is it a voting power that can be changed by the owner?
//12. ask about liquidity lock or where he wants it
  ...
  ReentrancyGuard
{
  ...
  //byte/ merkle proof for tiered WL
  // bytes32 public tier1WL;
  // bytes32 public tier2WL;
  // bytes32 public tier3WL;
  ...
  mapping(address => uint8) public votingPower;
  ...
  function mint() external onlyOwner {
    _mint(multiSigWallet, MAX_STAGE_SUPPLY);
  }
  ...
  // TODO: functions to set merkle proofs for tiered WL if needed
  // TODO: Clarify if setMaxWalletBalance is needed ie is it a contract that can be changed by the owner?
  ...
  function _beforeTokenTransfer(
    ...
    //set the cooldown
    cooldowns[from] = block.timestamp + cooldownTime;
  ...
  function _transfer(
    ...
    if (addrsExcludedFromTax[sender] || addrsExcludedFromTax[recipient]) {
      super._transfer(sender, recipient, amount);
    } else {
      //calculate the tax fee
      uint256 taxFeeAmount = amount * taxFee / 10000;
      //calculate the amount to be transferred
      uint256 amountToTransfer = amount - taxFeeAmount;
      //transfer the amount to the recipient
      super._transfer(sender, recipient, amountToTransfer);
      //transfer the tax fee to the owner
      super._transfer(sender, multiSigWallet, taxFeeAmount);
    }
  }
}