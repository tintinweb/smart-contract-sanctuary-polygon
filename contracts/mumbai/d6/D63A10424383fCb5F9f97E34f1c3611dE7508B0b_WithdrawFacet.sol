...
/******************************************************************************\
/// @title Sipher Portal - Withdraw Facet
/// @author Ather Labs
/// @dev This contract provides functions for user to withdraw their pending balance from the portal.
/******************************************************************************/
...
contract WithdrawFacet is ReentrancyGuard {
    /* ------ EVENTS ------ */
    /// @dev Event emitted when a withdrawal is successful.
    event Withdrawn(address recipient, uint8 listingId, uint88 tokenId, uint128 amount);

    /// @dev Event emitted when an account is quarantined.
    event QuarantineAsset(address recipient, uint8 listingId, uint88 tokenId, uint128 balance);

    /* ------ EXTERNAL FUNTIONS ------ */
    /// @notice Withdraw all pending balance in user's withdrawal key.
    /// @dev Balance in user's withdrawal key must bigger than 0.
    /// @dev User is not blacklisted.
    ...
    function withdrawPendingBalance(
        ...
        LibStatusControl._requireWithdrawEnabled();
        LibTokenManagement._requireUnpaused(_listingId);
        require(!s.isBlacklisted[_recipient].status, "WF: The recipient is blacklisted");
        ...
        require(balance > 0, "WF: No pending balance");
        ...
        emit Withdrawn(_recipient, _listingId, _tokenId, releasedAmount);
    }

    /// @notice Quarantine all pending balance in blacklisted user's withdrawal key.
    /// @dev Balance in user's withdrawal key must bigger than 0.
    /// @dev User is blacklisted and the deadline is passed.
    /// @param _recipient Recipient's address.
    /// @param _tokenId Id of the token on its contract.
    /// @param _listingId Token's listingId.
    function quarantineBlacklistedAccount(
        address _recipient,
        uint88 _tokenId,
        uint8 _listingId
    ) external nonReentrant {
        LibAccessControl._requireGovOrOperator();
        require(s.isBlacklisted[_recipient].status, "WF: The recipient is not blacklisted");
        require(s.isBlacklisted[_recipient].deadline < block.timestamp, "WF: The deadline has not passed yet");

        bytes32 withdrawKey = bytes32(abi.encodePacked(_recipient, _listingId, _tokenId));

        uint128 balance = s.pendingWithdrawBalance[withdrawKey];
        delete s.pendingWithdrawBalance[withdrawKey];

        emit QuarantineAsset(_recipient, _listingId, _tokenId, balance);
    }
}
...
/******************************************************************************\
/// @title Sipher Portal - Library Access Control
/// @author Ather Labs
...
/******************************************************************************\
/// @title Sipher Portal - Library Math
/// @author Ather Labs
...
/******************************************************************************\
/// @title Sipher Portal - Library Safe Cast
/// @author Ather Labs
...
/******************************************************************************\
/// @title Sipher Portal - Library Status Control
/// @author Ather Labs
...
/******************************************************************************\
/// @title Sipher Portal - Library Strings
/// @author Ather Labs
...
/******************************************************************************\
/// @title Sipher Portal - Library Token Management
/// @author Ather Labs
...
/******************************************************************************\
/// @title Sipher Portal - Library Withdraw Balance
/// @author Ather Labs
/// @dev This library provides internal functions to support Withdraw & Exit transactions.
/******************************************************************************/
...
library LibWithdrawBalance {
    /// @notice Event emitted when the balance of a user's withdrawl key is updated.
    event PendingWithdraw(address recipient, uint8 listingId, uint88 tokenId, uint128 amount);

    /// @notice Increase balance for a user's withdrawl key.
    ...
    /// @notice Send ERC20 token from the portal to recipient.
    /// @param _tokenAddress Layer I address of the token.
    /// @param _to Address of recipient.
    /// @param _amount Amount to transfer.
    /// @param _minLiquidity Minumum liquidity limit, balance of this token on the portal can not go below this limit.
    ...
    /// @notice Send ERC721 token from the portal to recipient.
    /// @param _tokenAddress Layer I address of the token.
    /// @param _to Address of recipient.
    /// @param _tokenId Id of the token on its contract.
    ...
    /// @notice Send ERC1155 token from the portal to recipient.
    /// @param _tokenAddress Layer I address of the token.
    /// @param _to Address of recipient.
    /// @param _tokenId Id of the token on its contract.
    /// @param _amount Amount to transfer.
    ...
    /// @notice Sends native token from the portal to recipient.
    /// @param _to Address of recipient.
    /// @param _amount Amount to transfer.
    ...
    /// @notice Release asset from the portal to recipient.
    /// @param _recipient Address of recipient.
    /// @param _listingId Token's listingId.
    /// @param _tokenId Id of the token on its contract.
    /// @param _amount Amount to transfer.
    /// @param _enableLimit `true` if apply minimum liquidity limit, `false` otherwise.
    function _releaseAsset(
        ...
        uint256 limit;
        if (_enableLimit) {
            limit = s.config.minLiquidity[_listingId];
        }
