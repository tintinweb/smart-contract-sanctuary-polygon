...
/**
 * @dev An L2Loop for Polygon - https://docs.matic.network/docs
 */

contract L2_PolygonBridge is L2Loop {
...
abstract contract L2Loop is Bridge {
    ...
    function commitTransfers(uint256 destinationChainId) external {
        ...
        require(minForceCommitTime < block.timestamp || getIsExecutor(msg.sender), "L2_BRG: Only Executor can commit before min delay");
    ...
    /**
     * @dev Mints new loopTokens for the recipient and optionally swaps them in the AMM market.
    ...
    function setHyperLoopTokenOwner(address newOwner) external onlyGovernance {
        loopToken.transferOwnership(newOwner);
    }

    function setMinimumExecutorFeeRequirements(uint256 _minExecutorBps, uint256 _minExecutorFeeAbsolute) external onlyGovernance {
