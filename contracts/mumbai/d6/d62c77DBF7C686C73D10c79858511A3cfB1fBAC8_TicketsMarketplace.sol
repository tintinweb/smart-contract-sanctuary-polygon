...
interface IAdmin {
    ...
    // @dev set transfer allowance from user and
    ...
    // @dev for retriving the fixedFee associated with and erc20 address
    function fixedFees(address addr) external view returns (uint256);
}
...
contract TicketsMarketplace is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    modifier onlyEventCreatorOrCollaborator(uint256 ticketId) {
        require(
    ...
    modifier isValidErc20Address(address erc20Address) {
        bool isValid = erc20Address == address(0) || erc20Address == address(1);
     ...
     *  @param transferAllowances addreses that can transfer and book the published tickets
     */
    function publishTickets(
        ...
        return publishTicketsForOrganizer(eventId, msg.sender, tickets, memberships, transferAllowances);
    }
    ...
    ) external payable whenNotPaused nonReentrant onlyEventCreatorOrCollaborator(ticketId) {
     ...
     *  @param ticketOwner Owner of the tickets for
     */
    function setAsk(
        uint256 ticketId,
        uint256 ticketPrice,
        ...
        _setAsk(ticketId, ticketPrice, amount, erc20address, ticketOwner, transferAllowances);
    }
     ...
     */
    function modifyCreatorRoyaltyOnTicket(uint256 ticketId, uint256 newCreatorRoyalty)
        external
        onlyEventCreatorOrCollaborator(ticketId)
        whenNotPaused
    {
    ...
    /**
     *  @dev Modifies Marketplace royalties for a given ticket.
     *  @param ticketId The id of the ticket whose royalty will be modified
     *  @param newPrimaryMarketplaceRoyalty The new primary royalty to be setted
     *  @param newSecondaryMarketplaceRoyalty The new secondary royalty to be setted
     */
    function modifyMarketplaceRoyaltyOnTicket(
        uint256 ticketId,
        uint256 newPrimaryMarketplaceRoyalty,
        uint256 newSecondaryMarketplaceRoyalty
    ) external onlyOwner whenNotPaused {
        require(newSecondaryMarketplaceRoyalty <= (HUNDRED_PERCENT - ticketsProperties[ticketId].creatorRoyalty), 'Above 100%.');

        ticketsProperties[ticketId].primaryMarketRoyalty = newPrimaryMarketplaceRoyalty;
        ticketsProperties[ticketId].secondaryMarketRoyalty = newSecondaryMarketplaceRoyalty;

        emit PrimaryMarketRoyaltyModifiedOnTicket(ticketId, newPrimaryMarketplaceRoyalty);
        emit SecondaryMarketRoyaltyModifiedOnTicket(ticketId, newSecondaryMarketplaceRoyalty);
    }
     ...
     */
    function addAllowance(uint256 ticketId, Models.AllowanceInput calldata allowance)
        public
        onlyEventCreatorOrCollaborator(ticketId)
        returns (uint256)
    {
     ...
     */
    function removeAllowance(uint256 ticketId, uint256 allowanceId) public onlyEventCreatorOrCollaborator(ticketId) {
    ...
    function publishTicketsForOrganizer(
        ...
        for (uint256 i = 0; i < ticketsLength; i++) {
            IAdmin(adminAddress).setTransferAllowances(bulkInfo.newTicketIds[i], organizer, transferAllowances);
    ...
    function _setAsk(
        ...
        require(
            msg.sender == ticketOwner ||
                IAdmin(adminAddress).userHasTransferAllowance(ticketId, ticketOwner, msg.sender) == true ||
                msg.sender == adminAddress,
        ...
        IAdmin(adminAddress).setTransferAllowances(ticketId, ticketOwner, transferAllowances);
    ...
    function _buyMarketTicket(
        ...
        require(tokenPaymentType[seller][ticketId] != address(1), 'Only off chain purchases');
        ...
        uint256 marketplaceFixedShare = IAdmin(adminAddress).fixedFees(tokenPaymentType[seller][ticketId]);

        uint256 creatorShare = ticketPrice;
        ...
        if (seller != creator && ticketPrice != 0) {
            ...
            sellerShare = ticketPrice;
        }

        if (tokenPaymentType[seller][ticketId] == address(0)) {
            require(
                msg.value == (amount * (marketplaceShare + creatorShare + sellerShare + marketplaceFixedShare)),
                'Value does not match price'
            );
        } else {
        ...
        if (tokenPaymentType[seller][ticketId] == address(0)) {
            if (ticketPrice == 0) {
                ...
                MarketplaceHelper.transferFundsSupportingGnosisSafe(owner(), (marketplaceShare + marketplaceFixedShare) * amount); // Trusted transfer, Gnosis Safe Wallet
            }
        } else {
            require(
                IERC20(tokenPaymentType[seller][ticketId]).allowance(msg.sender, address(this)) >=
                    amount * (marketplaceShare + marketplaceFixedShare + creatorShare + sellerShare),
                'Not enough erc20 allowed'
            );

            require(
                IERC20(tokenPaymentType[seller][ticketId]).balanceOf(msg.sender) >=
                    amount * (marketplaceShare + marketplaceFixedShare + creatorShare + sellerShare),
                'Not enough erc20'
            );
            if (sellerShare != 0) {
            ...
            MarketplaceHelper.transferERC20ToAccount(
                tokenPaymentType[seller][ticketId],
                msg.sender,
                owner(),
                (marketplaceShare + marketplaceFixedShare) * amount
            );
        }
