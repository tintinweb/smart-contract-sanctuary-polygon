 ...
 */
contract NiftySouqFixedPriceV2 is
    Initializable,
    ...
    PausableUpgradeable
{
    ...
    ) public whenNotPaused returns  (uint256 offerId_) {
        (
    ...
    function mintSellNft(
        ...
        uint256 price_,
        string memory currency_
    ) external whenNotPaused returns (uint256 tokenId_, uint256 offerId_) {
        if (blockList[msg.sender]) revert GeneralError("NS:126");
    ...
    function updateSalePrice(
        ...
        if (offer.offerType != NiftySouqIMarketplace.OfferType.SALE)
     ...
     *@param offerId_ OfferId of NFT
     */
    function cancelSale(uint256 offerId_) external whenNotPaused {
        if (
     ...
     */
    function buyNft(uint256 offerId_, uint256 quantity_)
        external
        payable
        whenNotPaused
    {
