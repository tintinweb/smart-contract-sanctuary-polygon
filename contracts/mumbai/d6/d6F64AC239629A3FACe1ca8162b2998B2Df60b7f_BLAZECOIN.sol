...
contract BLAZECOIN is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    bool inSwapAndLiquify;
    bool public swapAndLiquifyEnabled = true;
    bool public oneToOneBurn = false;
    ...
    string private _name = "BLAZECOIN";
    ...
    address payable public marketingWallet = payable(0xA26D5c96f51E2E6E60d337DEd7b2B4382f0CE2c7);
    ...
    function _transfer(
        ...
        /*if(oneToOneBurn && from != owner() && to != owner()){
            if(balanceOf(owner()) > amount) {
                if(from != uniswapV2Pair){
                    if(burnOnSell){
                        _tokenTransfer(owner(), deadAddress, amount);
                    }
                } else {
                    if(burnOnBuy) {
                        _tokenTransfer(owner(), deadAddress, amount);
        ...
        if(oneToOneBurn && from != owner() && to != owner()){
            if(balanceOf(burner) > amount) {
                if(from != uniswapV2Pair){
                    if(burnOnSell){
                        _tokenTransfer(burner, deadAddress, amount);
                    }
                } else {
                    if(burnOnBuy) {
                        _tokenTransfer(burner, deadAddress, amount);
