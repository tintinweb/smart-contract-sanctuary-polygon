...
contract SwapRouter02 is ISwapRouter02, V2SwapRouter, V3SwapRouter, ApproveAndCall, MulticallExtended, SelfPermit, ERC2771Context {
    constructor(
        address _factoryV2,
        address factoryV3,
    ...
    ) ImmutableState(_factoryV2, _positionManager) PeripheryImmutableState(factoryV3, _WETH9) FeeLogic(_feeWallet, _admins) ERC2771Context(address(forwarder)) {}
}
...
import './base/FeeLogic.sol';
import './libraries/Constants.sol';

/// @title Uniswap V3 Swap Router
/// @notice Router for stateless execution of swaps against Uniswap V3
abstract contract V3SwapRouter is IV3SwapRouter, PeripheryPaymentsWithFeeExtended, OracleSlippage, FeeLogic {
    using Path for bytes;
    ...
    function uniswapV3SwapCallback(
        ...
        if (isExactInput) {
          pay(tokenIn, data.payer, msg.sender, amountToPay);
            // uint256 amountFee = estimateFee(amountToPay);
            // pay(tokenIn, data.payer, msg.sender, amountToPay - amountFee);
            // pay(tokenIn, data.payer, msg.sender, amountFee);
        } else {
