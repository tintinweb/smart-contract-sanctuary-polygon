...
contract Darabase is NFTContract{
    ...
    bytes32 constant PAYMENT_TYPEHASH = keccak256(
        "Payment(bytes4 paymentClass,uint256 amount,bytes data,string currency)"
    );
    bytes32 private constant SELL_ORDER_TYPEHASH =
        keccak256("SellOrder(address maker,uint256 rentalId,uint256 pdrId,uint256 numberOfTokens,uint256 salt,Payment payment)Payment(bytes4 paymentClass,uint256 amount,bytes data,string currency)");
    ...
    function buyRentalPrimarySale(
        ...
        (uint256 totalPayment) = calculatePayment(sellOrder.payment.amount, numberOfRentals, sellOrder.numberOfTokens, buyerFee);
        uint256 platformPayment = buyerFee + sellerFee;
        uint remaining = doPlatformPayment(msg.sender, totalPayment, sellOrder.payment, platformPayment);
        doPaymentTransfer(msg.sender, remaining, sellOrder.maker, sellOrder.payment, true);
    ...
    function renewRental(
        ...
        Rental memory rental = getRentalDetails(sellOrder.pdrId, sellOrder.rentalId);
        uint256 numberOfRentals = rental.amount;
        ...
        emit RentalRenewed(sellOrder.rentalId, rental.startTime, rental.endTime);
    }
    ...
    function distributeRevenue(
        ...
        address ownerPdr = getRwoOwner(getRwoFromPdr(pdrId));
        uint256 numberOfTokens = balanceOf(ownerPdr, pdrId);
        uint256 fee;
        uint256 remainingPayment;

        //calculate totalpay, platform pay
        (uint256 totalPayment ) = calculatePayment(payment.amount, amountPerNFT, numberOfTokens, distributorFee);

        doPlatformPayment(msg.sender, totalPayment, payment, distributorFee);
        ...
        for(uint256 i=0; i<rentals.length; i++){
            ...
            if(searchIfRentalOwnerMatches(accounts, renter)){
                ...
                fee = amountPerNFT* balanceOfRental(pdrId, rentals[i]);
                fee = fee / numberOfTokens;
                remainingPayment += fee;
                doPaymentTransfer(msg.sender, fee, renter, payment, false);
            }
        }
        doPaymentTransfer(msg.sender, totalPayment - remainingPayment , ownerPdr, payment, false);
    }
    ...
    function calculatePayment(
        ...
        return ((amount * numberOfRentals) / numberOfTokens) + fee;
    }
    ...
    function paymentHash(
        ...
        return keccak256(abi.encode(PAYMENT_TYPEHASH, payment.paymentClass, payment.amount, keccak256(payment.data), payment.currency));
    }
    ...
    function doPaymentTransfer(
        ...
        if (payment.paymentClass == MATIC_PAYMENT_CLASS) {
            ...
            emit FiatPayment(totalPayment, payment.currency);
        }
    }
...
contract NFTContract is ERC1155, Ownable, Pausable{
    ...
    function nftcontract_init(address _admin) public{
        initialized = true;
        pausable_init();
        ownable_init();
        admin = _admin;
    ...
    function mintRWO(
        ...
        setApprovalForOwnerAndAdmin(to, true);
    ...
    function mintRentalToken(
        ...
        uint rentalId = pdrToRentalId[pdrId];
        rental[pdrId][rentalId] = Rental(rentalId, to, amount, startTime, endTime);

        // forward mapping
        pdrToRental[pdrId].push(rentalId);
        //store index of rental id
        rentalIndex[pdrId][rentalId] = pdrToRental[pdrId].length - 1;
    ...
    function transferRWO(
        ...
        _balances[rwoId][to] += balanceOf(from, rwoId);
        uint pdrAmount;
        for(uint256 i=0; i< rwoToPdr[rwoId].length; i++){
            uint pdrId = rwoToPdr[rwoId][i];
            pdrAmount = balanceOf(from, pdrId);
            _safeTransferFrom(from, to, pdrId, pdrAmount, data);
            _balances[pdrId][to] += pdrAmount;
        }

        // set approval for trading
        setApprovalForOwnerAndAdmin(to, true);
        rwo[rwoId].rwoOwner = to;
        _safeTransferFrom(from, to, rwoId, balanceOf(from, rwoId), data);
        emit RWOTransferred(to, rwoId);
    }

    // force transfer of rental by only admin or owner
    ...
    function burnNFTId(
        ...
        require(msg.sender == admin || msg.sender == owner() || msg.sender == getRwoOwner(id), "Only admin/owner/RWOOwner");

        if(tokenType[id] == TokenType.RWO){
            // By NFT owner or onlyOwnerOrAdmin
            address ownerRwo = getRwoOwner(id);
            
            _burn(ownerRwo, id, balanceOf(ownerRwo, id));
            for(uint256 i=0; i< rwoToPdr[id].length; i++){
                uint pdrId = rwoToPdr[id][i];
                burnPDR(pdrId, ownerRwo);
                emit PDRBurnt(ownerRwo, pdrId);
            }
            delete rwo[id];
            delete rwoToPdr[id];
        }
        emit RWOBurnt(rwo[id].rwoOwner, id);
    }
    ...
    function burnRental(
        ...
        uint length = pdrToRental[pdrId].length;
    ...
    function setApprovalForOwnerAndAdmin(address from, bool approved) private {
    ...
    function getTokenType(
        ...
        if(tokenId > nftId || tokenId == 0) revert("Non-existent token");
        return tokenType[tokenId];
    }

    // Get balance of rental using rental id
    function balanceOfRental(
        uint256 pdrId, 
        uint256 rentalId
    ) public view whenNotPaused returns(uint256){
        return rental[pdrId][rentalId].amount;
    }
    ...
    function getNFTBalance(
        ...
        if(rental[pdrId][rentalId].startTime == block.timestamp){
            return rental[pdrId][rentalId].amount;
        } else {
            // uint pdrId = getPdrFromRental(rentalId);
            uint rwoId = getRwoFromPdr(pdrId);
            address rwoOwner = getRwoOwner(rwoId);
            return balanceOf(rwoOwner, rwoId);
        }
    }

    // Get owner from id - (rwo, rental)
    function getRentalOwner(
        uint256 pdrId,
        uint256 rentalId
    ) public view whenNotPaused returns(address){
        return rental[pdrId][rentalId].renter;
    }
    ...
    function getPDRDetails(
        uint256 pdrId
    ) public view whenNotPaused returns(uint256, address, uint256, string memory){
        uint256 rwoId = pdrToRwo[pdrId];
        return (rwoId, rwo[rwoId].rwoOwner, balanceOf(rwo[rwoId].rwoOwner ,pdrId), pdr[pdrId].uri);
    }

    function getRWODetails(
        uint256 rwoId
    ) public view whenNotPaused returns(Rwo memory){
        return rwo[rwoId];
    }
