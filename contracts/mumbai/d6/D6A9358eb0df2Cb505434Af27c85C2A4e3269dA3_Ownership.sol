...
contract Ownership is ERC721, Ownable {
    mapping(uint256 => address payable) private tokenOwners;

    constructor() ERC721("Ownership", "MTK") {}

    function mint(address payable to, uint256 tokenId) public payable {
        require(tokenOwners[tokenId] == address(0), "Token already owned");
        require(msg.value >= 10 ether, "Insufficient payment");

        address payable admin = payable(owner());
        uint256 adminFee = msg.value;
        
        // Transfer funds from userA to admin
    ...
    function transferownership(address payable newOwner, uint256 tokenId) public payable {
        require(tokenOwners[tokenId] != address(0), "Token not owned");
        ...
        uint256 transferAmount = msg.value - adminFee;

        admin.transfer(adminFee);
        currentOwner.transfer(transferAmount);
