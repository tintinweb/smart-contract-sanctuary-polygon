...
pragma solidity ^0.8.9;
contract Campaign is NFT721Metadata {
    using AddressUpgradeable for address;
    address private adInfo;
    ...
    struct CampignInfo {
        ...
        uint256 dailyBudget;
        uint256 CPI;
        uint256 estimatedImpression;
    }

    mapping(uint256 => CampignInfo) public campaignInfos;

    mapping(uint256 => uint256[]) public campaignListofAds;

    event CampaignCreated(address advertiser, string ipfsHash, uint256 adId, uint256 campaignId);
     ...
     */
    function _initializeAdInfo(address _adInfo) internal {
        //onlyInitializing
        require(
            _adInfo.isContract(),
     ...
     */
    function getAdInfo() public view returns (address) {
        return adInfo;
    }

    // campaignStrings[0] = title;
    // campaignStrings[1] = summary;
    // campaignStrings[2] = campaignType;

    // campaignValues[0] = adID;
    // campaignValues[1] = startdate
    // campaignValues[2] = endDate
    // campaignValues[3] = baseCPI
    // campaignValues[4] = maxBudget
    // campaignValues[5] = dailyBudget
    // campaignValues[6] = CPI
    // campaignValues[7] = estimatedImpression
    function createCampaign(
        string memory tokenIPFSPath_,
        uint256[] memory campaignValues,
        string[] memory campaignStrings
    ) external {
        IAdCreation(getAdInfo()).isValidAd(campaignValues[0], msg.sender);

        uint256 mintedTokenID = _mintInternal(tokenIPFSPath_);

        campaignInfos[mintedTokenID] = CampignInfo(
            campaignValues[0],
            campaignStrings[0],
            campaignStrings[1],
            campaignStrings[2],
        ...
        campaignListofAds[campaignValues[0]].push(mintedTokenID);
        emit CampaignCreated(msg.sender, tokenIPFSPath_, campaignValues[0], mintedTokenID);
    }
}