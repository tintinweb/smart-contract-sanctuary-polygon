...
contract YexleAppian is ERC721Burnable, Ownable {
    ...
    */
    ...
    /**
        ...
        * @param _l1Address - address of L1Approver
        ...
        *
    */
    function mint(address _l1Address, address _to, uint256 _tokenId, string memory _tokenUri) external onlyAdmin{
        UserContract useC = UserContract(userContract);
        (bool status) = useC.verifyUser(_to);
        if(!status){ revert userContractError();}
        if(_l1Address == L1Approver){
    ...
    */
    ...
    /** 
        *landDocumentViewRequestApprove - only admin can call this function and allow a requesting user to view another user's 
           land documents
          @param _l1Address - address of L1 approver
          @param _requester - address of the user requesting to view someone else's land document with intention of buying it
          @param _tokenId - tokenID of the land documents which the requester is wishing to see
          @param _status - true or false. true if the admin grants access, false if admin doesn't grant access to requester.
        *
    */
    function landDocumentViewRequestApprove(address _l1Address, address _requester, uint _tokenId, bool _status) external onlyAdmin{
        UserContract useC = UserContract(userContract);
        (bool status) = useC.verifyUser(_requester);
        if(!status){ revert userContractError();}
        if(_l1Address != L1Approver){ revert notL1Approver();}
        if(_status){
            viewAccessGranted[_requester][_tokenId] = true;
            noOfRequestsToViewLandDoc[_tokenId] += 1;
            allViewRequesterAddressToView[_tokenId].push(_requester);
            emit AccessGrantedToView(_requester, _tokenId);
        }else{
            viewAccessGranted[_requester][_tokenId] = false;
        }
    }
    ...
    */
    ...
    /** 
        ...
        *@param _ownerOfLand - address of the seller/owner of the land
    ...
    */
    function ownerDecisionforRaisedRequest(address _ownerOfLand, address _requester, uint _tokenId, bool _status) external onlyAdmin{
        UserContract useC = UserContract(userContract);
        (bool status) = useC.verifyUser(_requester);
        if(!status){ revert userContractError();}
        require(_ownerOfLand == _owners[_tokenId],"you are not the owner of nft");
    ...
    /**
        ...
        *@param _requester - buyer's address
        *@param _tokenId - the tokenId of the land documents that buyer wishes to purchase
        *@param _DocumentUri - IPFS URI of the land documents that buyer wishes to purchase
    */
    function registrationForLandByBuyer(address _requester, uint _tokenId, string memory _DocumentUri) external onlyAdmin{
        require(ownerAcceptLandSales[_requester] == true, "registration is not possible");
        require(saleStatus[_tokenId] == true, "sale status is false");
        registrationDocument[_tokenId] = _DocumentUri;
        registrationDocumentStatus[_tokenId] = true;
    }


    /**
        *approveByL1 - L1 approver approves the sale first
        *@param _l1Approver - L1 Approver's address
    ...
    */
    function approveByL1(address _l1Approver, approverData memory _data) external onlyAdmin{
        UserContract useC = UserContract(userContract);
        (bool status) = useC.verifyUser(_data._sellingTo);
        if(!status){ revert userContractError();}
        if(_l1Approver != L1Approver){ revert notL1Approver();}
    ...
    /**
        ...
        *@param _l2Approver - L2 Approver's address
    ...
    */ 
    function approveByL2(address _l2Approver, approverDataForL2 memory _data) external onlyAdmin{
        UserContract useC = UserContract(userContract);
        (bool status) = useC.verifyUser(_data._sellingTo);
        if(!status){ revert userContractError();}
        if(_l2Approver != L2Approver){ revert notL2Approver();}
    ...
    */
    ...
    /**
        ...
        * @param _tokenId - pass the unique ID which represents the land. created during minting the land NFT
    */
    function noOfRequestersInfoToViewDoc(uint _tokenId) external view returns(uint allRequesterCount){
        return noOfRequestsToViewLandDoc[_tokenId]; 
    }

    /**
        * returnAllUriForLandOwner
        * @param _landOwnerAddress - Enter the land owner address 
    */
    function returnAllUriForLandOwner(address _landOwnerAddress) external view returns(string[] memory returnallUris){
        return allURI[_landOwnerAddress];
    }
    ...
    */
    ...
    /**
        * supportsInterface
        * @param _interfaceId Pass interfaceId, to let users know whether the ERC standard is used in this contract or not
    */
    function supportsInterface(bytes4 _interfaceId) public view override returns (bool){
        return _interfaceId == IID_IERC721 || super.supportsInterface(_interfaceId);
    }
