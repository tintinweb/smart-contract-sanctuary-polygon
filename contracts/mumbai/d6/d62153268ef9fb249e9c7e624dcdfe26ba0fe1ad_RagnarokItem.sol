    ...
    UUPSUpgradeable
{
    ...
    function mintRandom(
        ...
        uint256 index = _getRandomNumber(randomSeed, _tokenIds + 1, _itemsTypesByRarity[rarity].length);
        _mintItem(to, _itemsTypesByRarity[rarity][index]);
    }

    function mint(address to, uint16 itemTypeId) external {
        require(_auth[msg.sender], "not allowed");
        _mintItem(to, itemTypeId);
    }

    function burnFrom(uint256 id) external {
        require(_auth[msg.sender], "not allowed");
        _burn(id);
    }

    function _mintItem(address to, uint16 itemTypeId) internal {
        require(_itemTypes[itemTypeId] != 0, "item does not exist");
        _tokenIds++;
        uint256 tokenId = _tokenIds;
        _items[tokenId] = itemTypeId;
    ...
    function getItemTypeIdOfItem(uint256 itemId) external view returns (uint16) {
        return uint16(_items[itemId]);
    }

    function getItemTypeOfItem(uint256 itemId) external view returns (uint256) {
        return _itemTypes[uint16(_items[itemId])];
    }
    ...
    function initialize() public initializer {
        __ERC721_init("Ragnarok Item", "ROItem");
        ...
        returns (string memory)
    {
        return
            bytes(baseURI).length > 0
                ? string(abi.encodePacked(baseURI, StringsUpgradeable.toString(_items[tokenId])))
                : "";
    }
