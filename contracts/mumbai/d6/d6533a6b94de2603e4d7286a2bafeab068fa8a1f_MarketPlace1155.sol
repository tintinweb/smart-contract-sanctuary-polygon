    ...
    IERC1155Receiver
{
    ...
    /// @notice this event is emitted when a listing is created or updated the amounts if the partial was bought
    event ItemListed(
    ...
    /// @notice this event is emitted when the listing was gone since all amount were bought
    event ItemUnlisted(
        address indexed nftAddress,
        uint256 indexed tokenId,
        address indexed seller
    ...
    /// @dev Determine if NFT is listed fixed price on the market
    ...
    modifier isListed(
        ...
        require(
            getListing(_sellerAddress, _nftAddress, _nftId).price > 0,
            "Not Listed"
        );
        _;
    }
    ...
    modifier isOwner(
        ...
        require(
            IERC1155(_nftAddress).balanceOf(_lister, _nftId) >= _nftAmount,
            "Not enough token balance"
        );
        _;
    }

    /// @dev Determine if listed NFT seller is canceller
    /// @param _sellerAddress Seller address
    /// @param _nftAddress Nft address
    /// @param _nftId token ID
    /// @param _canceller canceller address
    modifier isListSeller(
        address _sellerAddress,
        address _nftAddress,
        uint256 _nftId,
        address _canceller
        ...
        require(
            _canceller ==
                getListing(_sellerAddress, _nftAddress, _nftId).seller,
            "Not List Seller"
        );
        _;
    }
    ...
    /// @notice list nft as fixed price
    /// @param _nftAddress nft address
    /// @param _nftId nft ID
    /// @param _nftAmount nft amount
    ...
    function listItem(
        ...
        isOwner(_nftAddress, _nftId, _nftAmount, msg.sender)
    {
        bytes32 hash = _getListingHash(msg.sender, _nftAddress, _nftId);
        require(_listings[hash].price <= 0, "Already Listed");
        ...
        tokenReceiptHandler.createReceipts(
            ...
            _asDefaultValueArray(_nftAmount, 1)
        );
    ...
    /// @notice cancel listing fixed price nft
    /// @param _nftAddress nft address
    /// @param _nftId nft ID
    function cancelListing(address _nftAddress, uint256 _nftId)
        external
        isListSeller(msg.sender, _nftAddress, _nftId, msg.sender)
        isListed(msg.sender, _nftAddress, _nftId)
    {
        bytes32 hash = _getListingHash(msg.sender, _nftAddress, _nftId);
        uint256 listedAmount = _listings[hash].amount;
        delete (_listings[hash]);
        ...
        tokenReceiptHandler.burnReceipts(
            ...
            _nftAddress,
            _asDefaultValueArray(_nftId, 1),
            _asDefaultValueArray(listedAmount, 1)
        );

        emit ItemCanceled(_nftAddress, _nftId, msg.sender);
    }

    /// @notice buy listed fix price nft
    /// @param _sellerAddress seller address
    /// @param _nftAddress nft address
    /// @param _nftId nft ID
    /// @param _nftAmount nft amount
    function buyItem(
        ...
        uint256 _nftId,
        uint256 _nftAmount
    ) external isListed(_sellerAddress, _nftAddress, _nftId) nonReentrant {
        bytes32 hash = _getListingHash(_sellerAddress, _nftAddress, _nftId);
        Listing memory listedItem = _listings[hash];

        require(
            listedItem.amount >= _nftAmount,
            "Buy amount is more than listed"
        );

        // send token to seller and payout wallet
        uint256 sellerReceive = (listedItem.price *
            _nftAmount *
            (percentMaxValue - platformFeePercent)) / percentMaxValue;
        uint256 payoutWalletReceive = (listedItem.price * _nftAmount) -
            sellerReceive;
        ...
        // update listing data
        if ((listedItem.amount - _nftAmount) > 0) {
            _listings[hash].amount -= _nftAmount;

            // emit event
            emit ItemListed(
                _nftAddress,
                _nftId,
                _listings[hash].seller,
                _listings[hash].amount,
                _listings[hash].purchaseTokenAddress,
                _listings[hash].price
            );
        } else {
            delete (_listings[hash]);
        ...
        emit ItemBought(
            ...
            listedItem.price * _nftAmount
        );
    }

    /// @notice get listed fix price nft info
    /// @param _sellerAddress seller address
    /// @param _nftAddress nft address
    /// @param _nftId nft ID
    function getListing(
        ...
        return _listings[_getListingHash(_sellerAddress, _nftAddress, _nftId)];
    }

    /// @notice get listing nft hash
    /// @param _sellerAddress seller address
    /// @param _nftAddress nft address
    /// @param _nftId nft ID
    function _getListingHash(
        address _sellerAddress,
        address _nftAddress,
        uint256 _nftId
    ) internal view returns (bytes32) {
        return keccak256(abi.encodePacked(_sellerAddress, _nftAddress, _nftId));
    }
