...
contract Anonim is ReentrancyGuard, KeeperCompatibleInterface, Ownable {
    ...
    event TokenSupplied(address tokenAddress, address userAddress, uint256 amount);
    event TokenWithdrawn(address tokenAddress, address userAddress, uint256 amount);
    event TokenBorrowed(address tokenAddress, address userAddress, uint256 amount);
    event TokenRepaid(address tokenAddress, address userAddress, uint256 amount);
    ...
    function revertIfHighBorrowing(
        ...
        if (getTotalBorrowValue(userAddress) > 0) {
            uint256 availableWithdrawBalance = getTotalSupplyValue(userAddress) -
                ((uint256(100) * getTotalBorrowValue(userAddress)) / uint256(80));

            (uint256 price, uint256 decimals) = getLatestPrice(tokenAddress);
            uint256 totalAmountValue = amount * (price / 10**decimals);
    ...
    function getMaxBorrow(address userAddress) public view returns (uint256) {
        return (uint256(80) * getTotalSupplyValue(userAddress)) / uint256(100);
    }
    ...
    {
        (uint256 price, uint256 decimals) = getLatestPrice(tokenAddress);
        return getMaxBorrow(userAddress) / (price / 10**decimals);
    }
