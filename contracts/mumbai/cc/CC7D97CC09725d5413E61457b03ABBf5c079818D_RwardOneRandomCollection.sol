...
contract RwardOneRandomCollection is ERC721r, EIP2981PerTokenRoyalties, Ownable {
  ...
  function mintToken(address nftOwner) external payable onlyOwner returns (uint256) {
    ...
    uint256[] memory newTokenIDs = _mintRandom(nftOwner, 1);
  ...
  function mintTokenAtIndex(address nftOwner, uint index) external payable onlyOwner returns (uint256) {
    require(isMintEnabled, "Minting is not open.");
    require(!(limitPerWallet >= 1) || balanceOf(nftOwner) < limitPerWallet, "Limit per wallet reached");


    _mintAtIndex(nftOwner, index);
    _setTokenRoyalty(index, owner());

    emit MintEvent(index, collectionID, "An NFT was minted successfully");
