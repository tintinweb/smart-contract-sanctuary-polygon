 ...
 *              _
 *             |  0 (i = 0)
 *      ΔTi := |  the ith duration (in seconds) between Initialize, Stake, Unstake, NotifyReward events(i = 1 .. n-1)
 ...
 *               ... + x(Tkm) * SUM(ΔTi / S(Ti) | i = km+1..n ) * R / D
...
contract AnimeTokenStakingPool is Ownable, ReentrancyGuard {
  ...
  event RewardPaidForStudio(address indexed animeStudio, uint256 reward);
  ...
  // Studio claims the half of the notified amount according to the following _vestingSchedule function
  function claimStudioReward() external nonReentrant {
    require(
      msg.sender == animeStudio,
      "AnimeBankStakingPool: sender is not anime studio"
    );
    require(initialize == true, "AnimeBankStakingPool: not initialized");
    require(
      _vestingSchedule() > studioRewardPayed,
      "AnimeBankStakingPool: Studio reward has not vested yet"
    );
    uint256 claimable = _vestingSchedule() - studioRewardPayed;
    studioRewardPayed += claimable;
    IERC20(stakingToken).safeTransfer(animeStudio, claimable);
    emit RewardPaidForStudio(animeStudio, claimable);
  }
    ...
    whenNotPaused
  {
    ...
    require(
      minReward <= reward,
      "AnimeBankStakingPool: Reward has not reached to buy nft"
    );
    ...
    if (price > 0) {
      ...
      IERC20(stakingToken).safeTransfer(owner(), price);
...
interface IAnimeTokenStakingPool {
  ...
  function estimateDuration(uint256 _type, address account) external;
  ...
  function buyNft(uint256 _type) external;

  function stake(uint256 amount) external;

  function setPaused(bool _paused) external;

  function setPriceOfType(uint256 _type, uint256 _price) external;

  function setNonceOfType(uint256 _type, uint256 _nonce) external;

  function setMaxSerialsOfType(uint256 _type, uint256 _maxSerial) external;

  function setMinRewardOfType(uint256 _type, uint256 _minReward) external;
