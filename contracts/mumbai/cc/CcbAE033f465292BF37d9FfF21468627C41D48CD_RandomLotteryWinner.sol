...
contract RandomLotteryWinner is VRFConsumerBase, Ownable {
    //Chainlink variables
    // The amount of LINK to send with the request
    uint256 public fee;
    // ID of the public key against which randomness is generated
    bytes32 public keyHash;

    // address of the lottery players
    address[] public players;
    // max number of players in one lottery game
    uint8 maxPlayers;
    // variable to indicate if the lottery has game has started or not
    bool public gameStarted;
    // the fees for entering the lottery
    uint256 entryFee;
    // current lottery game id
    uint256 public gameId;

    // emitted when the lottery starts
    event GameStarted(uint256 gameId, uint8 maxPlayers, uint256 entryFee);
    // emitted when the lottery ends
    ...
    /**
     * startGame starts the lottery game by setting appropriate values for all variables
     */
    function startGame(uint8 _maxPlayers, uint256 _entryFee) public onlyOwner {
        // check if there is a lottery already running
        require(!gameStarted, "Lottery is currently running");
        // empty the players array
        delete players;
        // set the max players for this game
        maxPlayers = _maxPlayers;
        // set the same started to true
        gameStarted = true;
        // setup the entryFee for the lottery
        entryFee = _entryFee;
        gameId += 1;
    ...
    /**
    joinGame is called when a player wants to enter the lottery
    */
    function joinGame() public payable {
        // check if the lottery is already running
        require(gameStarted, "Lottery has not started yet");
