 ...
 * @dev This contract allows users to participate in reveals where they can buy tickets
 * for a chance to win a WETH prize. The contract uses Chainlink VRF v2 for generating random
 * numbers to determine the winner.
 */
contract WEManager is
    AccessControl,
    ReentrancyGuard,
    VRFConsumerBaseV2,
    ConfirmedOwner
{
     ...
     * @param fundsToCollect - The total amount of funds to be collected for the reveal.
     * @param totalTickets - The total number of tickets available for the reveal.
     *
    ...
    event RevealStarted(
        ...
        uint256 fundsToCollect,
        uint256 totalTickets
    );

    /**
     * @dev This event is emitted when a reveal is completed, and the winner is determined using the
     * `sendWETHAndMATIC` function. It contains the following information:
     * @param revealId - The unique identifier of the completed reveal.
     * @param winner - The address of the winner who will receive the WETH prize.
     * @param WETHPrize - The amount of WETH the winner will receive.
     ...
     * When a reveal is cancelled, its status is changed to `Refunding`, allowing participants
     * to claim refunds.
     */
    event RevealCancelled(uint256 indexed revealId);

    /**
     * @dev This event is emitted when a user successfully buys tickets for a reveal using the
     * `buyTickets` function.
     * @param revealId - The unique identifier of the reveal for which the tickets are sold.
     * @param sender - The address of the user who purchased the tickets.
     * @param ticketsSold - The number of tickets sold in the transaction.
     * @param ticketsValue - The total value of the tickets sold, in Matic (equal to the ticket price
     * multiplied by the number of tickets).
     *
     * This event allows users and external applications to track ticket sales for reveals, and
     * provides information about the ticket buyers, the number of tickets purchased, and the
    ...
    /**
     * @dev This event is emitted when a player is refunded from a canceled reveal using the `refundPlayer`
     * function.
     * @param revealId - The unique identifier of the reveal from which the player is being refunded.
     * @param playerRefunded - The address of the player who is refunded.
     * @param amountRefundedInWeis - The amount refunded to the player, in wei (the smallest unit of Matic).
     *
     * This event allows users and external applications to track refunds from canceled reveals, and
     * provides information about the refunded players and the amounts refunded.
     */
    event Refund(
        uint256 indexed revealId,
        address indexed playerRefunded,
        uint256 amountRefundedInWeis
    );
     ...
     * - `Started`: The reveal is active and can receive funds from players buying tickets.
     * - `Completed`: The reveal has ended, a winner has been selected, and the prize has been distributed.
     * - `Refunding`: The reveal has been canceled, and players can now refund their funds.
     *
     ...
     *
     * @param fundsToCollect - The total amount of funds the reveal aims to collect.
     * @param fundsCollected - The current amount of funds collected for the reveal.
     * @param ticketPrice - The price of a single ticket in the reveal.
     * @param totalTickets - The total number of tickets available for the reveal.
     * @param ticketsSold - The number of tickets that have been sold so far.
     * @param firstAvailableTicketNumber - The first ticket number that is available for purchase.
     ...
     * @param WETHPrize - The prize in WETH tokens for the winner.
     * @param cancellationDate - The date after which the reveal is canceled if not all tickets have been bought.
    ...
    /**
     * @dev `tickets` is a nested mapping that keeps track of the buyer's address for each ticket number
    ...
    /**
     * @dev RefundInfo struct
     * @param amountToRefund - The amount spent by a player for a specific reveal
     * @param refunded - A boolean flag indicating whether the player has already been refunded
     *
     * This struct is used to store information related to refunds for players.
     * It keeps track of the amount a player has spent in a specific reveal and whether
     * they have already been refunded or not.
     */
    struct RefundInfo {
    ...
    /**
     * @dev `refundsInfo` is a mapping that stores the refund information for each player and reveal.
     * The key is generated by hashing the reveal ID and the player's address, ensuring a unique
     * identifier for each player-reveal combination. The value is an instance of the `RefundInfo` struct.
     */
    mapping(bytes32 => RefundInfo) public refundsInfo;

    /**
     * @dev The address of the Wrapped Ether (WETH) token contract.
     *
     * This is a public variable that stores the address of the WETH token contract.
     * WETH is an ERC20 token that represents Ether, allowing it to be used in smart contracts
     * that require tokens, rather than native Ether. This address is used in the contract
     * to interact with the WETH token for various operations, such as transferring the prize
     * to the winner of a reveal.
     */
    address public WETH_ADDRESS;

    /**
     * @dev The total amount of Wrapped Ether (WETH) locked for active reveals.
     *
     * This is a public variable that keeps track of the total amount of WETH locked in the contract
     * for active reveals. When a new reveal is created, its WETH prize is locked in the contract
     * by calling the `lockWETHPrize` function, which increases the `WETHLocked` value by the
     * amount of the WETH prize. When a reveal is completed, and the prize is sent to the winner,
     * the `WETHLocked` value is decreased by the amount of the WETH prize.
     */
    uint256 public WETHLocked = 0;

    /**
     * @dev The address of the supervisor.
     *
     * This public variable stores the address of the supervisor who has specific
     * permissions within the contract, such as creating reveals. The supervisor
     * address can also receive the collected funds from completed reveals.
     */
     ...
     * setting the VRF subscription ID, VRF Coordinator address, key hash, WETH token
    ...
    constructor(
        ...
        require(_WETHAddress != address(0), "Invalid WETH token address");
     ...
     * @param _ticketPrice - Price of each ticket for the reveal
     * @param _fundsToCollect - Total funds to collect for the reveal
     * @param _totalTickets - Total number of tickets for the reveal
     * @param _WETHPrize - WETH prize for the winner
     * @param _daysUntilCancellation - Number of days until the reveal can be cancelled
     * @return revealId - Id of the newly created reveal
     *
     * Requirements:
     * - The `_ticketPrice` must be greater than 0.
     * - The `_fundsToCollect` must be greater than 0.
     * - The `_totalTickets` must be greater than 0.
     * - The `_WETHPrize` must be greater than 0.
     * - The `_daysUntilCancellation` must be greater than 0.
     *
     * This function creates a new reveal and initializes its properties.
     * It locks the WETH prize for the reveal.
     * The reveal is then added to the `reveals` array and a `RevealCreated` event
     * is emitted. The reveal can be cancelled after `_daysUntilCancellation` days
     * if it has not reached its goals.
     */
    function createReveal(
        uint256 _ticketPrice,
        uint256 _fundsToCollect,
        uint256 _totalTickets,
        uint256 _WETHPrize,
        uint256 _daysUntilCancellation
    ) external onlyRole(SUPERVISOR_ROLE) returns (uint256) {
        require(_ticketPrice > 0, "Invalid _ticketPrice");
        require(_fundsToCollect > 0, "Invalid _fundsToCollect");
        require(_totalTickets > 0, "Invalid _totalTickets");
        require(_WETHPrize > 0, "Invalid _WETHPrize");
        require(_daysUntilCancellation > 0, "Invalid _daysUntilCancellation");

        lockWETHPrize(_WETHPrize);

        uint256 revealId = reveals.length;

        RevealStruct memory reveal = RevealStruct({
            fundsToCollect: _fundsToCollect,
            fundsCollected: 0,
            ...
            winner: address(0),
            randomNumber: 0,
            WETHPrize: _WETHPrize,
            cancellationDate: block.timestamp + _daysUntilCancellation * 1 days,
            status: RevealStatus.Started
        });

        reveals.push(reveal);

        emit RevealStarted(revealId, _fundsToCollect, _totalTickets);

        return revealId;
    }

    /**
     * @dev Locks the WETH prize for the reveal
     * @param WETHPrize - WETH prize to be locked
     *
     * This function ensures that the WETH prize for the reveal is locked in the
     * contract by checking if the contract's WETH balance is sufficient to cover
     * the new prize. If the condition is met, the `WETHLocked` variable is
     * increased by the value of `WETHPrize`.
     *
     * Requirements:
     * - The contract's WETH balance must be greater than or equal to the sum of
     *   the current `WETHLocked` and the `WETHPrize`.
     */
    function lockWETHPrize(uint256 WETHPrize) internal {
        /* Get WETH contract reference */
        IERC20 weth = IERC20(WETH_ADDRESS);

        /* Check WETH balance */
        require(
            weth.balanceOf(address(this)) >= WETHLocked + WETHPrize,
            "Insufficient WETH balance to lock"
        );

        WETHLocked += WETHPrize;
    }

    /**
     * @dev Buys tickets for a reveal
     * @param _revealId - The reveal ID
     * @param _numberOfTickets - The number of tickets the player wants to buy
     *
     * This function allows a user to buy a specified number of tickets for a given
     * reveal. It checks for various conditions such as the reveal's status, the
     * number of tickets requested, the sender's address, the availability of
     * the requested tickets and the cancellationDate. If all the conditions are met,
     * the function assigns the tickets to the buyer and updates the reveal's funds
     * collected, the number of tickets sold, and the first available ticket number.
     *
     * Requirements:
     * - `_revealId` must be valid.
     * - The reveal must be in the `Started` status.
     * - `_numberOfTickets` must be greater than 0.
     * - `msg.sender` must be a valid address.
     * - The requested number of tickets must be available.
     * - The sent Matic (`msg.value`) must be equal to the total cost of the requested tickets.
     * - The reveal is not cancelled.
     *
     * Emits a `TicketsSold` event.
     */
    function buyTickets(
        uint256 _revealId,
        ...
        require(_numberOfTickets > 0, "Insufficient tickets");
        require(msg.sender != address(0), "msg.sender is not a valid address");
        require(
            _numberOfTickets <= reveal.totalTickets - reveal.ticketsSold,
            "Not enough tickets available"
        );
        require(
            msg.value == reveal.ticketPrice * _numberOfTickets,
            "msg.value not equal to the value of the tickets"
        );
        require(
            block.timestamp <= reveal.cancellationDate,
            "the reveal was cancelled"
        );

        uint256 startTicketNumber = reveal.firstAvailableTicketNumber;
        for (
            uint256 ticketNumber = startTicketNumber;
            ticketNumber < startTicketNumber + _numberOfTickets;
            ticketNumber++
        ) {
            if (tickets[_revealId][ticketNumber] == address(0)) {
                tickets[_revealId][ticketNumber] = msg.sender;
            } else {
                revert("This ticket is already bought");
            }
        }

        reveal.fundsCollected += msg.value;
        reveal.ticketsSold += _numberOfTickets;
        reveal.firstAvailableTicketNumber += _numberOfTickets;

        bytes32 hash = keccak256(abi.encode(_revealId, msg.sender));
        refundsInfo[hash].amountToRefund += msg.value;
        refundsInfo[hash].refunded = false;

        emit TicketsSold(_revealId, msg.sender, _numberOfTickets, msg.value);
    }
     ...
     * - The reveal must have at least one ticket sold.
     * - All tickets for the reveal must be sold.
     * - The reveal status must be "Started".
     * - The random number for the reveal must not have been generated yet.
     *
     * This function requests a random number from the random number generator
     * and uses it to determine the winner of the reveal.
     */
    function setWinner(
        uint256 _revealId
        ...
        require(reveal.ticketsSold != 0, "No tickets sold for the reveal");
        require(
            reveal.ticketsSold == reveal.totalTickets,
            "Not all tickets were sold"
        );
        require(reveal.status == RevealStatus.Started, "Reveal is not started");
        require(reveal.randomNumber == 0, "Random number already generated");

        requestRandomWords(_revealId, reveal.ticketsSold);
    }
     ...
     *
     * Requirements:
     * - The Chainlink VRF subscription must be set and funded.
     *
     * This function sends a random number request to the Chainlink VRF
     ...
     * @param _randomWords - Array of random words received from the Chainlink VRF
     *
     * Requirements:
     * - The random number request must exist.
     *
     * This function is called by the Chainlink VRF when the random number
     ...
     * Finally, it calls the `sendWETHAndMATIC` function to send prizes
     * to the winner and the collected funds to the supervisor address.
     */
    function fulfillRandomWords(
        ...
        sendWETHAndMATIC(revealId, winnerTicketNumber);

        emit RequestFulfilled(_requestId, _randomWords);
    }

    /**
     * @dev Sends WETH to the winner and MATIC to the supervisor and closes the reveal
     * @param _revealId - Id of the reveal
     ...
     * 2. Sends the WETH prize to the winner.
     * 3. Updates the total WETHLocked by subtracting the prize.
     * 4. Sends the collected funds to the supervisor address.
     * 5. Emits a `RevealClosed` event.
     */
    function sendWETHAndMATIC(
        uint256 _revealId,
        ...
        require(
            _winnerTicketNumber > 0 &&
                _winnerTicketNumber <= reveal.ticketsSold,
        ...
        IERC20 weth = IERC20(WETH_ADDRESS);
        weth.transfer(reveal.winner, reveal.WETHPrize);
        WETHLocked -= reveal.WETHPrize;

        (bool sentFundsCollected, ) = supervisorAddress.call{
            value: reveal.fundsCollected
        }("");
        require(
            sentFundsCollected,
            "Failed to send fundsCollected to Supervisor"
        );
     ...
     * Requirements:
     * - The reveal must be in the Started status
     * - The current timestamp must be greater than the reveal's cancellation date
     *
     * Emits a RevealCancelled event with the reveal ID
     */
    function cancelReveal(uint256 _revealId) external nonReentrant {
        RevealStruct storage reveal = reveals[_revealId];
        require(
        ...
        reveal.status = RevealStatus.Refunding;

        emit RevealCancelled(_revealId);
    }

    /**
     * @dev Refund a player in a cancelled reveal
     * @param _revealId - The ID of the reveal to refund from
     * @param _playerAddress - The payable address of the player to be refunded
     *
     * Requirements:
     * - The player address must not be zero.
     * - The reveal must be in the Refunding status
     * - The player must have valid refund information
     * - The player has not been refunded yet
     * - The contract must have sufficient funds to complete the refund.
     *
     * Emits a Refund event with the reveal ID, player address, and refunded amount
     */
    function refundPlayer(
        ...
        require(
            reveal.status == RevealStatus.Refunding,
            "The reveal is not in Refunding status"
        );

        bytes32 hash = keccak256(abi.encode(_revealId, _playerAddress));
        RefundInfo storage refundInfo = refundsInfo[hash];

        require(
            refundInfo.amountToRefund > 0,
            "Refund information not found for the player"
        );
        require(
            refundInfo.refunded == false,
            "The player has already been refunded"
        );
        require(
            address(this).balance >= refundInfo.amountToRefund,
            "Insufficient contract balance to complete refund"
        );

        refundInfo.refunded = true;

        (bool sentPlayerFunds, ) = _playerAddress.call{
            value: refundInfo.amountToRefund
        }("");
        require(sentPlayerFunds, "Failed to refund the player");

        emit Refund(_revealId, _playerAddress, refundInfo.amountToRefund);
    }

    /**
     * @dev Returns the address that bought the ticket with the specified ticket number in the given reveal
     * @param _revealId - The reveal ID
     * @param _ticketNumber - The ticket number
     * @return - The address that bought the specified ticket in the given reveal
     *
     * This function retrieves the address of the user who purchased a specific ticket
     * in a given reveal. It takes the reveal ID and the ticket number as input parameters
     * and returns the buyer's address.
     *
     * Requirements:
     * - `_revealId` must be valid.
     ...
     * which receives funds collected from reveals. The caller must have the
     * DEFAULT_ADMIN_ROLE to execute this function.
     */
    ...
    function setCancellationDate(
        ...
        reveal.cancellationDate = block.timestamp + _minutes * 1 minutes;
    }
}