...
contract BettingGame {
    ...
    function createGame(uint256 betAmount, uint256 _maxHandAmount) public payable {
        ...
        game.maxProduct = 9801; // set maximum product
    ...
    function _randomNumber(address _userAddress, uint256 _gameId, uint256 _handIndex) internal {
        ...
        selectedHand.allHands.push(randomNumber);

        game.endTime = block.timestamp + 12 hours;
        game.pool += game.betAmount;

        //gameId, uint256 pool, address indexed player, uint256 handIndex, uint256[] hands, uint256 product, uint256 endTime
    ...
    function getHand(uint256 _gameId, address _userAddress, uint256 handIndex) public view returns (uint256[] memory, uint256) {
        require(handIndex < games[_gameId].players[_userAddress].numberOfHand, "Hand index out of range");
        Hand memory hand = games[_gameId].players[_userAddress].hands[handIndex];
