...
// File: contracts/subscription/SubscriptionApp.sol
...
contract SubscriptionApp is Initializable {
    mapping(address => uint) public vipLevel;
    SubscriptionManager subscriptionManager;
    SubscriptionDetail public trialDetail =
    SubscriptionDetail(PeriodUnit.DAY, 1, 0);
    SubscriptionDetail public cycleDetail =
    SubscriptionDetail(PeriodUnit.DAY, 10, 100);
    address weth9Address = 0x9c3C9283D3e44854697Cd22D3Faa240Cfb032889;

    event ReceivedSettleToken(uint value);
    ...
    function renewVIP() public {
        uint balanceBefore = IERC20(weth9Address).balanceOf(address(this));
        SimpleWallet(payable(msg.sender)).transferSettleToken();
        uint balanceAfter = IERC20(weth9Address).balanceOf(address(this));
        uint transferAmount = balanceAfter - balanceBefore;
        emit ReceivedSettleToken(transferAmount);
        if (transferAmount >= cycleDetail.value) {
            vipLevel[msg.sender] = 2;
            return;
        }
        if (transferAmount >= trialDetail.value) {
