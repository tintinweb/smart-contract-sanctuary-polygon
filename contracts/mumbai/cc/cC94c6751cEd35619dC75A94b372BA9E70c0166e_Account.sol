...
contract Account is Ownable {
    ...
    constructor(address _holdWallet, address _utilityTokenAddress) {
        require(
            _holdWallet != address(0x0),
            "Account: hold wallet address cannot be zero"
        );
        require(
    ...
    /**
     * @dev Increases Utility Token balance
     * @param _amount uint256 amount of tokens
     */
    function increaseUTBalance(uint256 _amount) external {
        require(!isAddressUnique(msg.sender), "Account: user doesn't exist");
        require(_amount > 0, "Account: amount should be greater than 0");
        require(
            utilityToken.allowance(msg.sender, address(this)) >= _amount,
            "Account: not enough allowance"
        );
        require(
            utilityToken.balanceOf(msg.sender) >= _amount,
            "Account: not enough founds"
        );
        utilityToken.transferFrom(msg.sender, holdWallet, _amount);
        balances[msg.sender][address(utilityToken)] += _amount;
    }

    /**
     * @dev Decreases Utility Token balance
     * @param _address address of user's wallet
     * @param _amount uint256 amount of tokens
     */
    function decreaseUTBalance(address _address, uint256 _amount)
        external
        onlyOwner
    {
        require(!isAddressUnique(_address), "Account: user doesn't exist");
        require(_amount > 0, "Account: amount should be greater than 0");
        require(
            _amount <= balances[_address][address(utilityToken)],
            "Account: amount cannot be greater than balance"
        );
        balances[_address][address(utilityToken)] -= _amount;
    }

    function getBalance(address _address, address _token)
        external
        view
        returns (uint256)
    {
        require(!isAddressUnique(_address), "Account: user doesn't exist");
        require(
            _token == address(utilityToken),
            "Account: token doesn't exist"
        );
        require(balances[_address][_token] != 0, "Account: balance is zero");
        return balances[_address][_token];
    }
