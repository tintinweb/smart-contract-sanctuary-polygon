...
library UserLib {
    ...
    function checkPayment(
        ...
        checkTrueType(info.paymentType);

        uint256 currentTime = block.timestamp;
        require(info.startTime <= currentTime && currentTime <= info.endTime, "UserLib: wrong payment time");
    }
    ...
    function checkTrueType(DataTypes.PaymentType _type) private pure {
        ...
        require(isTruePaymentType, "UserLib: wrong payment type");
    }
}
...
contract SingleWrite is IExecutePlugin, BasePluginWithRules {
    using UserLib for IRegistry;

    constructor(address _registry) {
        PLUGIN_VERSION = 1;
        PLUGIN_NAME = PluginsList.SINGLE_WRITE_COMMENT;
    ...
    function execute(
        ...
        (
            uint256 _subscriptionId,
            uint256 _postId,
            address _owner, string memory _ipfsHash, bool _up, bool _down, bool _isEncrypted, bool _isView
        ) = abi.decode(_data,(uint256, uint256, address, string, bool, bool, bool, bool));

        checkPlugin(_version, address(0));

        ISinglePostData singlePost = ISinglePostData(registry.singlePostData());

        if(singlePost.isUserVerification()) {
            checkBaseRule(RulesList.USER_VERIFICATION_RULES, address(0), _sender);
        }

        DataTypes.MinSimpleVars memory readPostVars;
        readPostVars.pluginName = PLUGIN_NAME;
        readPostVars.version = PLUGIN_VERSION;
        readPostVars.data = abi.encode(_postId);

        DataTypes.SinglePostInfo memory postInfo = ISinglePostData(registry.singlePostData()).readPost(readPostVars);

        if (postInfo.paymentType != 0) {
            registry.checkPayment(_sender, postInfo.paymentType, _subscriptionId);
        }
        ...
        vars.user = _sender;
        vars.data = abi.encode(address(0), _postId, _owner, _ipfsHash, _up, _down, _isEncrypted, _isView);

        uint256 commentId = ICommentData(registry.commentData()).writeComment(vars);
        require(commentId > 0, "Write: wrong create comment");

        require(singlePost.updatePostWhenNewComment(vars),
            "SingleWrite: wrong added commentId for user"
        );

        vars.data = abi.encode(address(0), _postId, commentId);
