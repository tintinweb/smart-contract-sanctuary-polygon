...
contract Trade {
    ...
    function isVerifiedOrder(Order memory order, Sign calldata sign) internal view {
        bytes32 hash = keccak256(abi.encodePacked(this, order.seller, order.buyer, order.erc20Address, order.nftAddress, order.amount, order.tokenId, order.qty, sign.nonce));
    ...
    function transferAsset(Order memory order, Fee memory fee) internal virtual {
        ...
        if(fee.platformFee > 0) {
            IERC20(order.erc20Address).transferFrom(order.buyer, owner, fee.platformFee);
        }
        if(fee.royaltyFee > 0) {
            IERC20(order.erc20Address).transferFrom( order.buyer, fee.tokenCreator, fee.royaltyFee);
        }
