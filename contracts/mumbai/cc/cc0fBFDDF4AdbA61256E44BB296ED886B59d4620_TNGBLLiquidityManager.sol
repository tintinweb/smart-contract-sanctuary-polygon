...
contract TNGBLLiquidityManager is AddressAccessor, Pausable {
    ...
    function checkMissingLiquidity() external view returns (uint256 minSize, uint256 liquidity, uint256 underlyingBalance, uint256 tngblBalance, uint256 tngblPrice) {
        (
        address uniswapV3Pool,
        address underlying,
        address usdr,
        ...
        {
            underlyingBalance = IERC20(underlying).balanceOf(
                uniswapV3Pool
            );
            uint8 underlyingDecimals = IERC20Metadata(underlying).decimals();
            ...
            liquidity =
            underlyingBalance +
            (tngblBalance * (tngblPrice = IPriceOracle(oracle).quote(1e18))) /
            (10**(36 - underlyingDecimals));
        }
    }
