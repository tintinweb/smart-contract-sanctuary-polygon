...
// Note that this pool has no minter key of ABI (rewards).
// Instead, the governance will call ABI distributeReward method and send reward to this pool at the beginning.
contract AbiRewardPool is IRewardPool, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. ABIs to distribute in the pool.
        uint256 lastRewardTime; // Last time that ABIs distribution occurred.
        uint256 accAbiPerShare; // Accumulated ABIs per share, times 1e18. See below.
    ...
    // The time when ABI mining starts.
    uint256 public poolStartTime;
    uint256 public poolEndTime;

    uint256 public rewardPerSecond;
    uint256 public constant RUNNING_TIME = 14 days;
    ...
    constructor(address _abi, uint256 _poolStartTime) public {
        require(block.timestamp < _poolStartTime, "late");
        if (_abi != address(0)) abi = IERC20(_abi);

        poolStartTime = _poolStartTime;
        poolEndTime = _poolStartTime.add(RUNNING_TIME);
        rewardPerSecond = TOTAL_REWARDS.div(RUNNING_TIME); // 0.41335978835978837 ABI/sec
    ...
    // Update the given pool's ABI allocation point. Can only be called by the owner.
    ...
    // View function to see pending ABIs on frontend.
    function pendingReward(uint256 _pid, address _user)
    public
    view
    override
    returns (uint256)
    {
        ...
        uint256 accAbiPerShare = pool.accAbiPerShare;
        ...
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            ...
            uint256 _abiReward = _generatedReward.mul(pool.allocPoint).div(
                totalAllocPoint_
            );
            accAbiPerShare = accAbiPerShare.add(
                _abiReward.mul(1e18).div(tokenSupply)
            );
        }
        return user.amount.mul(accAbiPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint_ > 0) {
            ...
            pool.accAbiPerShare = pool.accAbiPerShare.add(
                _abiReward.mul(1e18).div(tokenSupply)
            );
        }
        pool.lastRewardTime = block.timestamp;
    ...
    nonReentrant
    {
        ...
        if (user.amount > 0) {
            ...
            .amount
            .mul(pool.accAbiPerShare)
            .div(1e18)
            .sub(user.rewardDebt);
            if (_pending > 0) {
                safeAbiTransfer(msg.sender, _pending);
        ...
        user.rewardDebt = user.amount.mul(pool.accAbiPerShare).div(1e18);
    ...
    function _withdraw(
        ...
        updatePool(_pid);
        uint256 _pending = user.amount.mul(pool.accAbiPerShare).div(1e18).sub(
            user.rewardDebt
        );
        if (_pending > 0) {
            safeAbiTransfer(_account, _pending);
    ...
    // Safe abi transfer function, just in case if rounding error causes pool to not have enough ABIs.
    function safeAbiTransfer(address _to, uint256 _amount) internal {
        uint256 _abiBal = abi.balanceOf(address(this));
        if (_abiBal > 0) {
            if (_amount > _abiBal) {
                abi.safeTransfer(_to, _abiBal);
            } else {
                abi.safeTransfer(_to, _amount);
            }
        }
    }
