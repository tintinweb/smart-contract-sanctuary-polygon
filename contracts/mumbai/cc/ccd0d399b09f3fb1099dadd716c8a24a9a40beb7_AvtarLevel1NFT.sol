...
contract AvtarLevel1NFT is ERC721, ERC721URIStorage, Pausable, Ownable {
  ...
  |__________________________________*/
   ...
   struct NFT {
    ...
    constructor(uint8 _nftType, uint256 _totalSupply, uint256 _nftPrice, uint256 _nftFees, uint _totalMint, address _previousTypeContractAddress) ERC721("Avtar NFT", "APNFT") {
        nftType = _nftType;
        ...
        stages[1].stageId = 1;
        stages[1].totalSupply = 5000;
        stages[2].stageId = 2;
        stages[2].totalSupply = 8500;
        stages[3].stageId = 3;
        stages[3].totalSupply = 10000;
        }
    ...
    modifier onlyVerifiedUser(){
        require (verifiedUser(VerifiedUserContractAddress).isUserVerified(msg.sender) == true,"Please Verified from dapp");
        _;
    }

     modifier onlyVerifiedToAddressUser(address _to){
        require (verifiedUser(VerifiedUserContractAddress).isUserVerified(_to) == true,"Please Verified from dapp");
        _;
    }
    
    modifier onlyServer(){
        require (msg.sender == serverAddress,"This Method only for Server Use");
        _;
    }
    ...
    function safeMintForServer(address to) public virtual onlyServer{

        require(stages[currentStage].totalSupply >= _tokenIdCounter.current()+1,"Can not mint more NFT on this Stage");     
        if(nftType > 1) {           
           require(previousTypeContractAddress != address(0),"Previous contract address not added");           
           require (previousNFTContract(previousTypeContractAddress).isNFTPurchased(msg.sender) != false ,"You can not purchase this NFT, Please purchase previous nft first.");
        } 
        require(nftHolder[to].isPurchased == false,"Can not purchase this nft right now");        
        require(totalSupply > totalMint,"Nft Can not Create, total supply reached");
        totalMint += 1;
    ...
    function safeMint(address to) public virtual onlyVerifiedUser{
        ...
        require (ContractToken(ContractTokenContractAddress).balanceOf(msg.sender)  >= nftPrice ,"Amount is not sufficient for mint.");

        ContractToken(ContractTokenContractAddress).transferFrom(to,TreasuryAddress,nftPrice);
     ...
     function safeMintUSDC(address to) public virtual onlyVerifiedToAddressUser(to) {
        ...
        // ContractToken(ContractTokenContractAddress).transferFrom(to,TreasuryAddress,nftPrice);
        ContractToken(ContractTokenContractAddress).transferFrom(msg.sender, TreasuryAddress, nftPrice );
