...
contract HashUpOffer is HashUpBase {
    using Counters for Counters.Counter;

    enum HashUpOfferState {
        ACTIVE,
        CANCELED,
        ACCEPTED,
        DECLINED
        ...
        isTokenOwnerOrApproved(_nftAddress, _tokenId, _amount, _seller)
    {
        ...
        newOffer.state = HashUpOfferState.ACTIVE;
    ...
    function updateOffer(
        uint256 _offerId,
        uint256 _newPrice
    ) external payable nonReentrant isValidOffer(_offerId) {
        Offer memory offer = offers[_offerId];
        offers[_offerId].price = _newPrice;

        if (offer.buyer != _msgSender()) {
            revert PermissionDenied();
        } else if (offer.state != HashUpOfferState.ACTIVE) {
            ...
            uint amount = offer.price - _newPrice;
            (bool success, ) = offer.buyer.call{value: amount}("");
            if (!success) revert TransferFailed(offer.buyer, amount);
        }
        ...
        )
    {
        ...
        offers[_offerId].state = HashUpOfferState.ACCEPTED;
        ...
        (bool success, ) = offer.seller.call{value: rest}("");
        if (!success) revert TransferFailed(offer.seller, rest);
    ...
    function declineOffer(
        uint256 _offerId
    ) external nonReentrant isValidOffer(_offerId) {
        Offer memory offer = offers[_offerId];
        ...
        offers[_offerId].state = HashUpOfferState.DECLINED;

        (bool success, ) = offer.buyer.call{value: offer.price}("");
        if (!success) revert TransferFailed(offer.buyer, offer.price);
    ...
    function cancelOffer(
        ...
        offers[_offerId].state = HashUpOfferState.CANCELED;
