...
import {ACCURACY_TRAIT_ID, DAMAGE_TRAIT_ID, ELEMENTAL_AFFINITY_TRAIT_ID, EVASION_TRAIT_ID, SPEED_TRAIT_ID, GAME_LOGIC_CONTRACT_ROLE, HEALTH_TRAIT_ID} from "../Constants.sol";
...
contract MobCombatable is GameRegistryConsumerUpgradeable, ICombatable {
    ...
    function decreaseHealth(
        ...
        returns (uint256)
    {
        require(false, "MobCombatable: decreaseHealth not implemented");
        return 0;
    }
    ...
    function canBeAttacked(
        ...
        address nftContract = EntityLibrary.entityToAddress(entityId);

        // For now use the TokenTemplateSystem to check if this is a valid mob
        if (nftContract != _getSystem(TOKEN_TEMPLATE_SYSTEM_ID)) {
    ...
    function getCurrentHealth(
        ...
        // Extract contract address and token ID from mob entity
        (address nftContract, uint256 nftTokenId) = EntityLibrary.entityToToken(
            mobEntity
        );
        ...
        return
            uint256(
                ITokenTemplateSystem(_getSystem(TOKEN_TEMPLATE_SYSTEM_ID))
                    .getTraitInt256(nftContract, nftTokenId, HEALTH_TRAIT_ID)
            );
    }
}
