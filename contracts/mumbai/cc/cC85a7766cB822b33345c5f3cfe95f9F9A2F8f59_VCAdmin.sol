...
import "../marketplace/IFeeBeneficiary.sol";
import "../pool/IVCPool.sol";
import "../starter/IVCStarter.sol";
import "../tokens/IVCPoCNft.sol";
import "../tokens/ArtNft721/IVCArtNftERC721.sol";
import "../tokens/ArtNft1155/IVCArtNftERC1155.sol";
import "../marketplace/IVCMarketManager.sol";

/**
 * @dev The function of this contract is to be the 'admin contract' for all other VC contracts. All VC contracts
 * have an admin (address variable) and only admin functions. On the other hand, this contract has itself an
 * admin, whose role is to be the admin of the VC ecosystem.
 */
...
interface IVCAdmin {
    struct ParamsSetup {
        ...
        address marketplaceFixedPricePocNft;
        address marketplaceAuctionPocNft;
        address marketManager;
    }

    event GovProtocolSetup(ParamsSetup params);
    event GovCurrencySet(IERC20 indexed currency);
    event GovPoCNftSet(address indexed poCNft);
    event GovStarterSet(address indexed starter);
    event GovWithdrawToPool(IERC20 indexed currency, uint256 amount);

    error GovOnlyAdminError();
    error GovSetupProtocolError();
    error GovInvalidVCAdminError();
    error GovInvalidAdminError();
    error GovCallContractFailedError();
    error GovERC20TransferError();

    /**
     * @dev Allows the admin of the VC ecosystem to setup the protocol. This function must be called right after
     * all VC cotracts are deployed and it can be called at most once. Its main purpose is to set the protocol
     * ERC20 currency, PoCNFT address, VCPool address, and VCStarter address for all VC smart contracts.
     *
     * @notice Only the protocol admin can invoke this function.
     *
     * @param params This struct contains the addresses all deployed VC smart contracts and the protocol ERC20
     * {currency}.
     */
    function setupProtocol(ParamsSetup calldata params) external;
     ...
     * @notice Only the protocol admin can invoke this function.
     *
     * @notice Initially, the Core-team is supposed to be the admin of this contract.
     *
     * @param _admin The address of the protocol new admin.
     */
    function setAdmin(address _admin) external;

    /**
     * @dev Allows to set/change the admin for all other VC contracts. This function should be called whenever
     * one wants to change the implemantation of the VCAdmin contract.
     *
     * @notice Only the protocol admin can invoke this function.
     *
     * @param _vcAdmin The address of the newly deployed VCAdmin contract.
     */
    function changeVCAdmin(address _vcAdmin) external;

    /**
     * @dev Allows to set/change the PoCNFT contract of the VC ecosystem.
     *
     * @notice Only the protocol admin can invoke this function.
     *
     * @param _pocNft The address of the newly deployed PoCNFT contract.
     */
    function setPoCNft(address _pocNft) external;

    /**
     * @dev Allows to set/change the VCStarter contract of the VC ecosystem.
     *
     * @notice Only the protocol admin can invoke this function.
     *
     * @notice Already deployed VCProject contracts will retain the former VCStarter implementation.
     *
     * @param _starter The address of the newly deployed VCStarter contract.
     */
    function setStarter(address _starter) external;

    /**
     * @dev Allows any user to transfer to VCPool any ERC20 balance accumulated in this contract.
     *
     * @param _currency The ERC20 contract address.
     */
    function withdrawToPool(IERC20 _currency) external;

    /**
     * @dev Allows to set/change the maximum royalty basis points for both ERC721 an ERC1155 ArtNft contracts.
     *
     * @notice Only the protocol admin can invoke this function.
     *
     * @param _maxRoyaltyBps The maximum royalty in basis points.
     */
    function setMaxRoyalty(uint256 _maxRoyaltyBps) external;

    /**
     * @dev Allows to set/change the maximum batch size for both ERC721 an ERC1155 ArtNft contracts.
     *
     * @notice Only the protocol admin can invoke this function.
     *
    ...
    /**
     * @dev Allows to call any contract with any data using the call method. This allows to call any onlyAdmin
     * function of any VC contract.
     *
     * @notice Only the protocol admin can invoke this function.
     *
     * @param _contract The address of the contract to call.
     * @param _data The call data, which must be encoded according to the ABI of the function to call.
     *
     * For example, to call the VCPool function 'withdrawToProject(address _project, uint256 _amount)' using
     * ethers JSON do the following:
     *
     * const iface = new ethers.utils.Interface(abiPool);
     * const data = iface.encodeFunctionData("withdrawToProject", [projectAddress, amount]);
     * await vcAdmin.connect(admin).callContract(Pool.address, data);
     *
     * where 'abiPool' is the JSON ABI of VCPool.
     */
...
contract VCAdmin is IVCAdmin {
    ...
    IVCArtNftERC1155 public artNft1155;
    IVCArtNftERC721 public artNft721;
    ...
    function _onlyAdmin() private view {
        if (msg.sender != admin) {
            revert GovOnlyAdminError();
        }
    }

    function setupProtocol(ParamsSetup calldata _params) external {
        _onlyAdmin();
        if (_protocolSetup) {
            revert GovSetupProtocolError();
        }

        pool = IVCPool(_params.vcPool);
        starter = IVCStarter(_params.vcStarter);
        artNft1155 = IVCArtNftERC1155(_params.artNft1155);
        artNft721 = IVCArtNftERC721(_params.artNft721);
        marketManager = IVCMarketManager(_params.marketManager);

        IFeeBeneficiary(_params.marketplaceFixedPrice1155).setAdmin(_params.marketManager);
        IFeeBeneficiary(_params.marketplaceFixedPrice721).setAdmin(_params.marketManager);
        IFeeBeneficiary(_params.marketplaceAuction721).setAdmin(_params.marketManager);
        IFeeBeneficiary(_params.marketplaceFixedPricePocNft).setAdmin(_params.marketManager);
        IFeeBeneficiary(_params.marketplaceAuctionPocNft).setAdmin(_params.marketManager);

        _setPoCNft(_params.pocNft);
        _setCurrency(_params.currency);
        _setStarter(_params.vcStarter);
        _setPool(_params.vcPool);

        artNft721.setApproverRole(_params.marketManager, true);
        artNft1155.setApproverRole(_params.marketManager, true);

        artNft1155.setMinterRole(_params.marketplaceFixedPrice1155, true);
        artNft721.setMinterRole(_params.marketplaceFixedPrice721, true);
        artNft721.setMinterRole(_params.marketplaceAuction721, true);
        _protocolSetup = true;

        emit GovProtocolSetup(_params);
    }

    // changes the admin for this VC Admin smart contract
    function setAdmin(address _admin) external {
        _onlyAdmin();
        _setAdmin(_admin);
    }

    // changes the VC Admin for all the protocol smart contracts
    function changeVCAdmin(address _vcAdmin) external {
        _onlyAdmin();
        if (_vcAdmin == address(0) || _vcAdmin == address(this)) {
            revert GovInvalidVCAdminError();
        }
        artNft721.setAdmin(_vcAdmin);
        artNft1155.setAdmin(_vcAdmin);
        starter.setAdmin(_vcAdmin);
        pool.setAdmin(_vcAdmin);
        pocNft.setAdmin(_vcAdmin);
        marketManager.setAdmin(_vcAdmin);
    }
    ...
    function setPoCNft(address _pocNft) external {
        ...
        _setPoCNft(_pocNft);
        emit GovPoCNftSet(_pocNft);
    }

    function setStarter(address _starter) external {
        _onlyAdmin();

        starter = IVCStarter(_starter);
        _setStarter(_starter);
        emit GovStarterSet(_starter);
    }

    function withdrawToPool(IERC20 _currency) external {
        uint256 amount = _currency.balanceOf(address(this));
        if (amount == 0 || !_currency.transfer(address(pool), amount)) {
            revert GovERC20TransferError();
        }
        emit GovWithdrawToPool(_currency, amount);
    }
    ...
    //////////////////////////////////////////////////////
    //    Allows to call any contract with any data     //
    //////////////////////////////////////////////////////

    function callContract(address _contract, bytes calldata _data) external {
        _onlyAdmin();
        (bool status, ) = _contract.call(_data);
        if (!status) {
            revert GovCallContractFailedError();
        }
    }
    ...
    function _setAdmin(address _admin) private {
        if (_admin == address(0) || _admin == admin) {
            revert GovInvalidVCAdminError();
        }
        admin = _admin;
    }
    ...
    function _setCurrency(IERC20 _currency) private {
        ...
        emit GovCurrencySet(_currency);
    }

    function _setPoCNft(address _pocNft) private {
        pocNft = IVCPoCNft(_pocNft);

        pocNft.grantMinterRole(address(pool));
        pocNft.grantMinterRole(address(starter));
        pocNft.grantMinterRole(address(marketManager));

        address[5] memory marketplaces = marketManager.getMarketplaces();
        for (uint256 i = 0; i < 5; i++) {
            pocNft.grantMinterRole(marketplaces[i]);
        }
    ...
    function _setPool(address _pool) private {
        ...
        pocNft.grantMinterRole(_pool);

        starter.setPool(_pool);
        pocNft.setPool(_pool);
        marketManager.setPool(_pool);

        address[] memory noFeeAccounts = new address[](1);
        noFeeAccounts[0] = _pool;
        starter.addNoFeeAccounts(noFeeAccounts);
    }

    function _setStarter(address _starter) private {
        pocNft.grantMinterRole(_starter);
        pool.setStarter(_starter);
        marketManager.setStarter(_starter);

        address[] memory noFeeAccounts = new address[](6);
        address[5] memory marketplaces = marketManager.getMarketplaces();
        for (uint256 i = 0; i < 5; i++) {
            noFeeAccounts[i] = marketplaces[i];
        }

        noFeeAccounts[5] = address(pool);
        starter.addNoFeeAccounts(noFeeAccounts);
    }
}
...
interface IVCArtNftERC1155 is IERC1155 {
...
interface IVCArtNftERC721 is IERC721 {
