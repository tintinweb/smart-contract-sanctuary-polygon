...
contract SmartContract is ERC721A, Ownable, ReentrancyGuard {
  ...
  *****************************************************************************************************************************/
  modifier mintCompliance(uint256 _mintAmount) {
    ...
    if (_mintAmount <= 0)                                                  { revert Errors.InvalidMintAmount(); }
    if (_mintAmount > maxMintAmountPerTx)                                  { revert Errors.InvalidMintAmount(); }
    if (totalSupply() + _mintAmount > maxSupply)                           { revert Errors.MaxSupplyExceeded(); }
    if (_numberMinted(msg.sender) + _mintAmount >= maxMintAmountPerWallet) { revert Errors.MaxSupplyExceeded(); }
    _;
  }
  ...
  *****************************************************************************************************************************/
  modifier mintPriceCompliance(uint256 _mintAmount) {
    if (msg.value < mintPrice * _mintAmount) { revert Errors.InsufficientFunds(); }
    _;
  }
