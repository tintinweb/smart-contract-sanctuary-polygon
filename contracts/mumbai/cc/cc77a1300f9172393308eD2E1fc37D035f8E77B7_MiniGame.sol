...
contract MiniGame is VRFConsumerBase {
    ...
    uint256 public constant KEY_1_PRICE = 1000 ether;

    // max purchase per transation
    uint256 public constant MAX_PURCHASES_PER_TRANSACTION = 100;
    ...
    function activateGame() external returns (bool) {
        require(_hasEnoughRewards(), "PEPPER::INSUFFICIENT_REWARD");
    ...
    function mint(uint256 _amount) external returns (uint256) {
        // validate
        uint256 amountToPay = _calculateAmountToPay(msg.sender, _amount);

        // transfer myte
        require(
            myteContract.transferFrom(msg.sender, address(this), amountToPay),
            "PEPPER::MYTE_CONTRACT_TRANSFER_FAILED"
        );
        if (isGameActive == false && _hasEnoughRewards()) {
            isGameActive = true;
        }
        // mint token
        pepperKeyContract.mint(msg.sender, KEY_1, _amount);
    ...
    // ------------ PRIVATE FUNCTIONS ------------ //
    function _hasEnoughRewards() private view returns (bool) {
        return getMyteBalance() >= FIRST_PRIZE;
    }

    function _calculateAmountToPay(address _buyer, uint256 _amount)
        private
        view
        returns (uint256)
    {
        require(_amount > 0, "PEPPER::INVALID_AMOUNT");
        require(pepperKeyContract.exists(KEY_1), "PEPPER::NON_EXIST_TOKEN");

        uint256 amountToPay = KEY_1_PRICE * _amount;

        require(
            myteContract.balanceOf(msg.sender) >= amountToPay,
            "PEPPER::NOT_ENOUGH_MYTE_BALANCE"
        );
        ...
        require(
            allowanceForContract >= amountToPay,
