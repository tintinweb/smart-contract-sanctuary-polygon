...
contract VCStarter {
    ...
    event SttrProjectCreationRejected(address indexed lab, address indexed project);
    ...
    address _admin;
    address public _coreTeam; // multisig of the VC CORE team
    address _pool;
    ...
    //mapping(address => bool) _isWhitelistedResearcher;
    ...
    uint256 _backersTimeout = 15 * 24 * 60 * 60; // 15 days
    ...
    constructor(
        ...
        _maxCampaignOffset = maxCampaignOffset;
    ...
    // function whitelistResearcher(address _researcher) external {
    //     _onlyAdmin();
    //     if (_isWhitelistedResearcher[_researcher]) {
    //         revert Errors.SttrResearcherAlreadyWhitelisted();
    //     }
    //     _isWhitelistedResearcher[_researcher] == true;
    //     emit SttrWhitelistedResearcher(_researcher);
    // }

    // THIS FUNCTION IS DEPRECATED
    // function blacklistResearcher(address _researcher) external {
    //     _onlyAdmin();
    //     if (!_isWhitelistedResearcher[_researcher]) {
    //         revert Errors.SttrResearcherAlreadyBlacklisted();
    //     }
    //     _isWhitelistedResearcher[_researcher] == false;
    //     emit SttrBlacklistedResearcher(_researcher);
    // }
    ...
    function createProject(address _lab, bool _accepted) external returns (address newProject) {
        ...
        _pendingProjectRequest[msg.sender] = false;
        ...
        if (_accepted) {
            ...
            emit SttrProjectCreationRejected(_lab, newProject);
        }
    }

    function cloneProject() external {
        address newProject = Clones.clone(_projectTemplate);
        VCProject(newProject).init(address(this), msg.sender);
    }

    function existsProjectRequest(address _lab) external view returns (bool) {
        return _pendingProjectRequest[_lab];
    }
    ...
    function closeProject(address _project, bytes memory _sig) external {
        ...
        bool projectStatus = VCProject(_project).getProjectStatus();
        if (!projectStatus) {
            revert Errors.SttrProjectAlreadyClosed();
        }
    ...
    function publishCampaignResults(address _project, bytes memory _sig) external {
        _onlyLabOwner(_project);
        uint256 currentCampaignId = VCProject(_project).getNumberOfCampaigns() - 1;
    ...
    function _checkBeforeFund(
        ...
        if (_amount == 0) {
            revert Errors.SttrFundingAmountIsZero(); // add custom error
        }
        if (lab == address(0)) {
            revert Errors.SttrUnexistentProject();
        }
        if (lab == msg.sender) {
...
contract VCProject is Initializable {
    ...
    function closeProject() external {
        ...
        bool canBeClosed = _projectStatus &&
    ...
    function startCampaign(
        ...
        CampaignStatus lastCampaignStatus = getCampaignStatus(_numberOfCampaigns - 1);
        bool lastResultsPublished = _campaigns[_numberOfCampaigns - 1].resultsPublished;
...
library Errors {
    ...
    error SttrProjectAlreadyClosed();
