...
/**
  @notice 
  This contract manages the Daas ecosystem, serves as a payment terminal, and custodies all funds.
...
contract TerminalV1_1 is Operatable, ITerminalV1_1, ITerminal, ReentrancyGuard, Ownable {
  ...
  address payable public anti_rug_address;

  address payable public platformFee_address;

  bool public hardTargetlimit = true;
  ...
  /// @notice The total amount of BNB that each project paid.
  mapping(uint256 => uint256) public totalBalanceOf;

  /// @notice The percent fee the Daas project takes from tapped amounts. Out of 200.
  uint256 public override fee = 10;

  /// @notice The percent fee the Daas project takes from tapped amounts. Out of 10000.
  uint256 public anti_rug_percent = 1000;
  ...
  //fund Raise limit
  mapping(uint256 => uint256) public fundRaiseLimit;

  //whitelist prefer
  mapping(uint256 => bool) public whitelistPrefer;
  ...
  /** 
      ...
      @param _terminalDirectory A directory of a project's current Daas terminal to receive payments in.
    */
  constructor(
    IProjects _projects,
    ...
    address _owner,
    address _platformFee_address,
    address _anti_rug_address
    ...
    platformFee_address = payable(_platformFee_address);

    anti_rug_address = payable(_anti_rug_address);
  }
  ...
  function deploy(
    ...
    if (hardTargetlimit) require(_properties.target < 1E21, 'hardTargetlimit is 1000BNB');
    ...
    require(
      _payoutMods[0].beneficiary == anti_rug_address && _payoutMods[0].percent == anti_rug_percent,
      'anti-rug is necessary'
    );
    ...
    //set defaultTargetLimit
    fundRaiseLimit[_projectId] = 1E21;

    //set whitelistPrefer default state
    whitelistPrefer[_projectId] = true;
  }
  ...
  */
  ...
  function pay(
    ...
    //under Target Limit
    if (fundRaiseLimit[_projectId] > 0) {
      require(
        totalBalanceOf[_projectId] + msg.value <= fundRaiseLimit[_projectId],
        'can not raise fund more than fundRaiseLimit'
      );
    }
  ...
  function setAntiRugPercent(uint256 _anti_rug_percent) external onlyOwner {
    // anti_rug_percent must be under 10%.out of 10000
    require(anti_rug_percent <= 1000, 'TV1_1::setAntiRugPercent: BAD_Percent');

    // Set the anti_rug_percent.
    anti_rug_percent = _anti_rug_percent;
  }
  ...
  function setProjectPayAmount(uint256 _projectId, uint256 _projectPayAmount) external {
    require(projects.ownerOf(_projectId) == msg.sender, 'TV1::setProPayAmt: NotProOwner');
    projectPayAmount[_projectId] = _projectPayAmount;
  }

  function setMultisigAddress(address _anti_rug_address, address _platformFee_address)
    external
    onlyOwner
  {
    anti_rug_address = payable(_anti_rug_address);
  ...
  function setFundRaiseLimit(uint256 _projectId, uint256 _fundRaiseLimit) external onlyOwner {
    fundRaiseLimit[_projectId] = _fundRaiseLimit;
  }

  function setWhitelistPrefer(uint256 _projectId, bool _newstate) external onlyOwner {
    whitelistPrefer[_projectId] = _newstate;
  }

  function setHardTargetlimit(bool _state) external onlyOwner {
    hardTargetlimit = _state;
  }
    ...
    for (uint256 _i = 0; _i < _mods.length; _i++) {
      ...
      if (_modCut > 0) {
        ...
        if (_mod.allocator != IModAllocator(address(0))) {
          ...
          // Get a reference to the Daas terminal being used.
  ...
  function _pay(
    ...
    //Add totalBalance
    totalBalanceOf[_projectId] = totalBalanceOf[_projectId] + _amount;
  ...
  function _takeFee(
    ...
    // feeAmount = _from - PRBMath.mulDiv(_from, 200, _percent + 200);that means 10/210
    feeAmount = PRBMath.mulDiv(_from, _percent, 200);

    // Nothing to do if there's no fee to take.
    if (feeAmount == 0) return 0;

    Address.sendValue(platformFee_address, feeAmount);

    //we send the fee directly to platform Fee multisig address
  }
}
