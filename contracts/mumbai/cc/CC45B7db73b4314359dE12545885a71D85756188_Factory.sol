...
contract Factory {
  Tifi[] private tokens; //an array that contains different ERC1155 tokens deployed
  mapping(uint256 => address) private indexToContract; //index to contract address mapping
  mapping(address => uint256) private contractToIndex; //contract address to index mapping
  mapping(uint256 => address) private indexToOwner; //index to ERC1155 owner address
  ...
  function deployERC1155(
    ...
    Tifi token = new Tifi(_contractName, _uri, _tierNames, _ids);
    address _tokenAddress = address(token);
    tokens.push(token);

    indexToContract[tokens.length - 1] = _tokenAddress;
    contractToIndex[_tokenAddress] = tokens.length - 1;
    indexToOwner[tokens.length - 1] = tx.origin;

    emit ERC1155Created(msg.sender, _tokenAddress);
  ...
  function mintERC1155(
    ...
    tokens[_index].mint(id, amount);
    emit ERC1155Minted(tokens[_index].owner(), address(tokens[_index]), amount);
  }

  function getERC1155Items(uint _index) public view returns (uint[] memory) {
    return tokens[_index].getIds();
  }

  function getERC1155ItemName(
    uint _index,
    uint _id
  ...
  function getIndexByAddress(address _address) public view returns (uint256) {
    return contractToIndex[_address];
  }
  ...
  function getERC1155byIndexAndId(
    ...
    public
    view
    returns (address _contract, address _owner, string memory _uri, uint supply)
  {
    Tifi token = tokens[_index];
    return (
      address(token),
      token.owner(),
      token.uri(_id),
...
contract Tifi is ERC1155, ERC1155Supply, Ownable {
  string[] public tierNames; // tier of the ticket
  uint[] public ids;
  string public baseMetadataURI;
  string public name; //the contract mame
  ...
  function createMapping() private {
    for (uint id = 0; id < ids.length; id++) {
      nameToId[tierNames[id]] = ids[id];
      idToName[ids[id]] = tierNames[id];
    }
  }
  ...
  /*
    ...
    */
  function mint(uint _id, uint256 amount) public onlyOwner returns (uint) {
