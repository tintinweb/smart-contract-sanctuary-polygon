  ...
  DefaultOperatorFilterer
{
  ...
  mapping(address => uint256) public addressBurned;
  ...
  constructor(string memory _uri, address _helper) ERC1155("") {
    name = "Aether Games Packs";
    symbol = "AGPKS";
    helper = _helper;
    ...
    _createToken(1, 175600, _uri, 3000);
    setPaused(1, true);
  }
  ...
  ) public onlyRole(MINTER) nonReentrant {
    require(tokens[_id].exists, "Paused/nonexistent.");

    for (uint256 i = 0; i < _addresses.length; i++) {
      _mint(_addresses[i], _id, _amount, "");
      addressMinted[_addresses[i]] += _amount;
    }
    tokens[_id].mintCount += _amount * _addresses.length;
  }
  ...
  function mintBatch(
    ...
    for (uint256 i = 0; i < _ids.length; i++) {
      require(
        !tokens[_ids[i]].paused && tokens[_ids[i]].exists,
        "Paused/nonexistent."
      );
      require(
        tokens[_ids[i]].mintCount + _amounts[i] <= tokens[_ids[i]].maxSupply,
        "Max supply reached."
      );
    }
    ...
    FunctionInterface(helper).purchaseWithToken(
      totalCost,
  ...
  function minterMint(
    ...
    require(!tokens[_id].paused && tokens[_id].exists, "Paused/nonexistent.");
    require(
      tokens[_id].mintCount + _amount <= tokens[_id].maxSupply,
      "Max supply reached."
    );
  ...
  function setHelper(address _helper) public onlyRole(OWNER) {
    helper = _helper;
  }
  ...
  function _createToken(
    ...
    if (!tokens[_id].exists) {
      tokens[_id].maxSupply = _maxSupply;
      tokens[_id].exists = true;
    }

    tokens[_id].price = _price;
    tokens[_id].uri = _uri;
    tokens[_id].paused = true;
  }
  ...
  modifier onlyRole(bytes32 role) {
    require(roles[role][msg.sender], "Not authorized to fp.");
    _;
  }
