...
contract MECHANFTIDLE {
	...
	constructor(address payable wallet, address payable _developer) public {
        ...
        plans.push(Plan(20, 90, 200)); // SL Mecha 
        plans.push(Plan(20, 80, 200)); // L Mecha
        plans.push(Plan(20, 75, 200)); // SR Mecha
		plans.push(Plan(20, 70, 200)); // R Mecha (Compounding)
        plans.push(Plan(20, 70, 200)); // COMM Mecha (Compounding)
        plans.push(Plan(20, 65, 200)); // UNC Mecha (Compounding)
	}
	...
	function getPercent(uint8 plan) public view returns (uint256) {
	 ...
	 function getReinvest(uint8 plan) public view returns (uint256){
        ...
        if(plan == 0){
            return plans[plan].reinvest.sub(time.mul(0)); //reinvest decreases every day by 2%
        }
        
        if(plan == 1){
            return plans[plan].reinvest.sub(time.mul(0)); // reinvest decreases every day by 1%
        }
        
        if(plan == 2){
            return plans[plan].reinvest.sub(time.mul(0)); // reinvest decreases every day by 0.5%
        }
		if(plan == 3){
            return plans[plan].reinvest.sub(time.mul(0)); // reinvest decreases every day by 0.5%
        }
