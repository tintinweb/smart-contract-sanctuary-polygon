...
// File: contracts/BigMonkee2023.sol
...
contract BigMonkeez2023 is ERC721, Pausable, Ownable, ERC721URIStorage, ERC721Enumerable {

    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;


    uint256 public constant monkeePrice = 25000000000000000000; // 25 Matic

    uint256 public constant originalHolderMonkeePrice = 0; // 0.00 ETH

    uint public constant maxMonkeePurchase = 12;

    uint public constant maxOriginalHolderPurchase = 48;

    uint256 public constant MAX_MONKEEZ = 10420;

    bool public saleIsActive = false;

    bool public originalSaleActive = false;

    uint256 public constant originalCount = 0;
    ...
    uint public monkeeReserve = 2000;
    ...
    constructor() ERC721("Big Monkeez 2.5", "BMon") {
        tokenCounter = 0;
    }
    ...
    function originalHolderMint(bytes32[] calldata _merkleProof) public payable {
        require(originalSaleActive, "OG Mint not active yet");
        require(!whitelistClaimed[msg.sender], "Address has already claimed.");
        uint originalHolderSupply = totalSupply();
        ...
        for (uint i = 0; i < maxOriginalHolderPurchase; i++) {
            _safeMint(msg.sender, originalHolderSupply + i);
        }
    ...
    function ogAllowlistMint(address _to, uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable {
        require(originalSaleActive, "OG Mint not active yet");
        uint256 alreadyMinted = _mintAmount & whitelistMinted[msg.sender];
        require(_mintAmount > 0 && _mintAmount <= maxOriginalHolderPurchase, "You are past max OG mint amount");
        require(alreadyMinted < 48, "Address is allowed only 48 total og mints");
        uint supply = totalSupply();
        bytes32 leaf = keccak256(abi.encodePacked((msg.sender)));
        require(
            MerkleProof.verify(_merkleProof, merkleRoot, leaf),
  ...
  function setBaseURI(string memory baseURI) public onlyOwner {
    ...
    function makeOriginalSaleActive() public onlyOwner {
        originalSaleActive = !originalSaleActive;
    }
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId, uint amount)
        internal
        whenNotPaused
        virtual
