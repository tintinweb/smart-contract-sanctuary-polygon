...
contract FoundersContract is Ownable, ReentrancyGuard, AccessControl {
    ...
    function leaderboardMint(
        ...
        require(
            categories[_category].leaderboardMintedSupply.add(1) <= categories[_category].leaderboardSupply,
            "Founder Contract: Max supply of leaders in this category minted!"
        );

        bytes32 msgHash = getMessageHash(msg.sender,_timestamp,_category,_nftContractAddress);
    ...
    function reservedMint(
        ...
        require(
            categories[_category].reservedMintedSupply.add(1) <= categories[_category].reservedSupply,
        ...
        emit ReservedMint(msg.sender,categories[_category].reservedSupplyPoint,_category);
    ...
    function publicMint(
        ...
        require(
            categories[_category].publicMintedSupply.add(_tokenIdsLength) <= categories[_category].publicSupply,
        ...
        for (uint256 index = 0; index < _tokenIdsLength; index++) {
            ...
            emit PublicMint(msg.sender,categories[_category].publicSupplyPoint,_category,msg.value);

            categories[_category].publicSupplyPoint++;
        }

        categories[_category].publicMintedSupply = categories[_category].publicMintedSupply.add(_tokenIdsLength);
    }
    ...
    function _addCategory(
        ...
        require(
            _categoryValue.reservedSupplyPoint == categories[_category + 1].maxSupplyPoint.add(1),
            "Founder Contract: Next cateogry should start from last category ending!"
        );
        require(
            _categoryValue.maxSupplyPoint == _categoryValue.reservedSupplyPoint.add(_categoryValue.totalSupply.sub(1)),
            "Founder Contract: Invalid supply range!"
        );

        require(
            _categoryValue.totalSupply == _categoryValue.publicSupply.add(_categoryValue.reservedSupply.add(_categoryValue.leaderboardSupply)),
    ...
    function updatePublicCounter(uint8[] memory _category) public {
        require(
            hasRole(ADMIN_ROLE, _msgSender()),
        ...
        for (uint8 index = 0; index < _category.length; index++) {
            require(
                categories[_category[index]].publicSupply == categories[_category[index]].publicMintedSupply,
                "Founder Contract: Public supply of this category not minted completely!"
            );
            require(
                categories[_category[index]].leaderboardSupply.sub(categories[_category[index]].leaderboardMintedSupply) != 0,
            ...
            categories[_category[index]].publicSupply = categories[_category[index]].publicSupply.add(categories[_category[index]].leaderboardSupply.sub(categories[_category[index]].leaderboardMintedSupply));

            // leaderboard supply = leaderboard minted supply
            categories[_category[index]].leaderboardSupply = categories[_category[index]].leaderboardMintedSupply;

            emit UpdatePublicCounter(categories[_category[index]].publicSupplyPoint,_category[index],msg.sender);
        }
    }
    ...
    function updateleaderboardPerWalletCap(
        ...
        emit UpdateleaderboardPerWalletCap(_leaderboardPerWalletCap,msg.sender);
    }
    ...
    function updateLeaderboardSignerAddress(
        ...
        emit UpdateLeaderboardSignerAddress(_leaderboardSignerAddress,msg.sender);
    }
    ...
    function getMessageHash(
        ...
        return keccak256(abi.encodePacked(_to,_timestamp,_category,_nftContractAddress));
    }
    ...
    function getTotalSupply() public view returns (uint256) {
        ...
        for (uint8 index = 0; index < totalCategories; index++) {
