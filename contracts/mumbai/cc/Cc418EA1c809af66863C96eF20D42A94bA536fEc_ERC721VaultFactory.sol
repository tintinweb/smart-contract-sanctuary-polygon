...
// File contracts/ERC721TokenVaultV2.sol
...
contract TokenVault is ERC721Upgradeable, ERC721HolderUpgradeable {
    ...
    function mintToken(address _receiver, uint256 _supply) private {
        uint256 _tokenId = totalSupply();
        for (uint256 i = _tokenId; i < _supply; i++) {
            _mint(_receiver, i);
            tokenId.increment();
        }
    }

    function burnToken(uint256 _balance) private {
    ...
    function _claimFees() internal {
        ...
        uint256 curatorRoyalty = totalSupply() + curatorMint;
        if (curator != address(0)) {
            mintToken(curator, curatorRoyalty);
            // _mint(curator, curatorMint);
            emit FeeClaimed(curatorMint);
        }
        uint256 govRoyalty = totalSupply() + govMint;
        if (govAddress != address(0)) {
            mintToken(govAddress, govRoyalty);
