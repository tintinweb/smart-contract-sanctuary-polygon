...
contract AdventurersGuild is HSystemChecker, Multicall {
    ...
    event LogSetPetInteractionHandlerContractAddressEvent(
    ...
    /// @param tokenIds - Array of pet ids to stake
    function stake(address owner, uint256[] calldata tokenIds)
        public
        onlyRole(GAME_ROLE)
        isUser(owner)
    {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            require(
                pi.getCurrentPetStage(tokenIds[i]) == 3,
                'AG 103 - Pet must be final form to be staked'
            );
    ...
    function calculateClaim(uint256[] calldata ids, uint256 timestamp)
        public
        view
        returns (uint256)
    {
        ...
        for (uint256 i; i < ids.length; i++) {
            if (_activityOracle.canDoActivity(UNSTAKING, PET_TOKEN, ids[i])) {
                require(
                    currentTime >= _lastClaim[ids[i]],
                    'AG 400 - Cannot calculate claim for negative time difference'
                );
    ...
    function _processClaim(
        ...
        for (uint256 i; i < ids.length; i++) {
            require(
                _activityOracle.canDoActivity(UNSTAKING, PET_TOKEN, ids[i]),
                'AG 102 - Pet is not staked'
            );
    ...
    function setMilkModifier(uint16 milkModifier) external onlyRole(ADMIN_ROLE) {
        require(
            ...
            'AG 105 - Milk modifier is out of bounds'
        );
