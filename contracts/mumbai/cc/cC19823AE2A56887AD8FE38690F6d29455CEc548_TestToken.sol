...
contract ERC20 is Context, IERC20 {
    ...
    function blockedBalanceOf(address account) public view returns (uint256) {
    ...
    ERC20Destroyable
{
    ...
    event NameUpdated(address indexed account, string name);
    event SymbolUpdated(address indexed account, string symbol);
    event ClassUpdated(address indexed account, string class);
    event IssuerUpdated(address indexed account, address indexed issuer);
    event CategoryUpdated(address indexed account, bytes4 category);
    event TokenURIUpdated(address indexed account, string tokenURI);
    ...
    function confiscate(
        ...
        emit Confiscated(confiscatee, amount, receiver);
    }
    ...
    function updateClass(string memory newClass) public {
        _class = newClass;
        emit ClassUpdated(msg.sender, newClass);
    }
    ...
    function updateIssuer(address newIssuer) public {
        _issuer = newIssuer;
        emit IssuerUpdated(msg.sender, newIssuer);
    }
    ...
    function updateCategory(bytes4 newCategory) public {
        ...
        emit CategoryUpdated(msg.sender, newCategory);
    }
    ...
    function updateTokenURI(string memory newTokenURI) public {
        ...
        emit TokenURIUpdated(msg.sender, newTokenURI);
    }
