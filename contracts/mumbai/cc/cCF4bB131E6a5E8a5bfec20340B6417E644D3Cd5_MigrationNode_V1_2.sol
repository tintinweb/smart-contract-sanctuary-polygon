...
import "../utils/interfaces/INODERewardManagement.sol";
...
import "../utils/libraries/SafeMath.sol";

import "../utils/types/Ownable.sol";

contract MigrationNode_V1_2 is Ownable {
    using SafeMath for uint256;

    INODERewardManagement public oldManagement;
    INewNODERewardManagement public newManagement;

    bool public enableMigration = true;

    mapping(address => bool) public migratedUser;
    bool isMigrateMigratedUser;

    uint256 rewardPerMinuteLesser = 8680550000000;
    uint256 rewardPerMinuteCommon = 49861110000000;
    uint256 rewardPerMinuteLegendary = 114652770000000;
    uint256 rewardPerMinuteOMEGA = 1318472220000000;

    uint256 claimInterval = 60;
    ...
    constructor(
        ...
        newManagement = INewNODERewardManagement(_newManagement);
    }

    function migrateMigratedUser(address[] memory _migratedUser) external {
        require(!isMigrateMigratedUser, "migratedUser migration done");
        isMigrateMigratedUser = true;

        // migrate migratedUser
        for (uint256 i; i < _migratedUser.length; i++) {
            migratedUser[_migratedUser[i]] = true;
        }
    }
    ...
    function changeRewardsPerMinute(uint256 _rewardPerMinuteLesser, uint256 _rewardPerMinuteCommon, uint256 _rewardPerMinuteLegendary, uint256 _rewardPerMinuteOMEGA) public onlyOwner {
        rewardPerMinuteLesser = _rewardPerMinuteLesser;
        rewardPerMinuteCommon = _rewardPerMinuteCommon;
        rewardPerMinuteLegendary = _rewardPerMinuteLegendary;
        rewardPerMinuteOMEGA = _rewardPerMinuteOMEGA;
    }

    function migration(NodeEntity[] memory _nodesArray) public {
        // address account = msg.sender;
        require(enableMigration, "Not Allowed Migration");
        require(migratedUser[msg.sender] == false, "Already Migrated");
        migratedUser[msg.sender] = true;

        uint256 oldLesserNodeCount;
        uint256 oldCommonNodeCount;
        uint256 oldLegendaryNodeCount;
        uint256 oldOMEGANodeCount;

        uint256 checkLesserNodeCount;
        uint256 checkCommonNodeCount;
        uint256 checkLegendaryNodeCount;
        uint256 checkOMEGANodeCount;

        uint256 lastClaimTime;
        uint256 rewardedAmount;

        (oldLesserNodeCount, oldCommonNodeCount, oldLegendaryNodeCount, oldOMEGANodeCount ) = oldManagement._getNodeCounts(msg.sender);

        uint256 totalNodeCount = oldManagement._getNodeNumberOf(msg.sender);

        NodeEntity[] memory nodesArray = _nodesArray;
        NodeEntity memory _node;

        require(totalNodeCount == nodesArray.length, "Invalid Node Count");

        for (uint256 i = 0; i < nodesArray.length; i ++) {
            _node = nodesArray[i];

            if (_node.nodeType == 1 && _node.isStake == 0) checkLesserNodeCount = checkLesserNodeCount + 1;
            if (_node.nodeType == 2 && _node.isStake == 0) checkCommonNodeCount = checkCommonNodeCount + 1;
            if (_node.nodeType == 3 && _node.isStake == 0) checkLegendaryNodeCount = checkLegendaryNodeCount + 1;
            if (_node.nodeType == 4) checkOMEGANodeCount = checkOMEGANodeCount + 1;
        }

        require(checkLesserNodeCount == oldLesserNodeCount && checkCommonNodeCount == oldCommonNodeCount && checkLegendaryNodeCount == oldLegendaryNodeCount && checkOMEGANodeCount == oldOMEGANodeCount, "Incorrect");
        ...
        for (uint256 i = 0; i < nodesArray.length; i ++) {
            ...
            if (_node.nodeType == 1) {
                rewardPerMinute = rewardPerMinuteLesser;
            } else if (_node.nodeType == 2) {
                rewardPerMinute = rewardPerMinuteCommon;
            } else if (_node.nodeType == 3) {
                rewardPerMinute = rewardPerMinuteLegendary;
            } else if (_node.nodeType == 4) {
                rewardPerMinute = rewardPerMinuteOMEGA;
            }

            lastClaimTime = _node.lastClaimTime;

            if (_node.lastClaimTime >= _node.expireTime && _node.expireTime > 0) {
                lastClaimTime = _node.expireTime;
            }

            uint256 minutesPassed = (lastClaimTime).sub(_node.creationTime).div(claimInterval);
            rewardedAmount = minutesPassed.mul(rewardPerMinute);

            newManagement.migrateNode(msg.sender, _node.name, _node.creationTime, _node.lastClaimTime, _node.expireTime, _node.nodeType, _node.isStake, rewardedAmount);
        }
    }
}
