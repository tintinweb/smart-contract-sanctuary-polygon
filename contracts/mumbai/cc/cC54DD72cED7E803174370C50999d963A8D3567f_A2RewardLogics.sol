...
contract A2RewardLogics is Ownable {
  ...
  // STRAGE VARIABLES
    ...
    bool public electionsResult = true;
  ...
  function setToken(A1UtilityNFTs _a1UtilityNFTs, A0TheStupidestKidsNFTs _a0TheStupidestKidsNFTs, TSKToken _tskToken) public onlyOwner {
    require(electionsResult, "Is not possible to set token");
  ...
  function activateSecondPresale () external onlyOwner {
  ...
  function votePerFP(uint256 _vote) external {
    require(waitToVote[msg.sender] < block.timestamp, "You can't claims reward yet");
  ...
  function closeElections()external onlyOwner{
    if(votesYes > votesNo){
