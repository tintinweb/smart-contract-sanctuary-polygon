...
library VaultLifecycle {
        ...
        returns (uint256)
    {
        ...
        if (currentOption == address(0)) {
            return getAlternateDay(block.timestamp);
        }
        ...
        if (block.timestamp > currentExpiry + 2 days) {
            return getAlternateDay(block.timestamp);
        }
        return getAlternateDay(currentExpiry);
    }
    ...
    /*
     */
    function getNextNextFriday(uint256 timestamp) internal pure returns (uint256) {
        return getNextFriday(timestamp) + 7 days;
    }
    ...
    function getNextDay(uint256 timestamp) internal pure returns (uint256) {
        ...
        uint256 day8am = nextDay - (nextDay % (24 hours)) + (8 hours);
    ...
    /**
    @notice Daily expiries are not possible because of 24 hour limitations, this is for 2 day expiries
    @dev This is not for production, only for testing!

     */
    function getAlternateDay(uint256 timestamp) internal pure returns (uint256) {
        // dayOfDual = 0 (day 1) - 1 (day 2)
        uint256 dayOfDual = ((timestamp / 1 days) + 4) % 2;
        uint256 nextSecondDay = timestamp + ((2 + 5 - dayOfDual) % 2) * 1 days;
        uint256 secondDay8am = nextSecondDay - (nextSecondDay % (24 hours)) + (8 hours);

        // If the passed timestamp is day=Second Day hour>8am, we simply increment it by a week to next SecondDay
        if (timestamp >= secondDay8am) {
            secondDay8am += 2 days;
        }
        return secondDay8am;
    }
        ...
        /**
    @notice for 14 day expiries 


     */

    function getNextBiweeklyFriday(uint256 timestamp) internal pure returns (uint256) {
        // dayOfWeek = 0 (sunday) - 13 (next week saturday)
        uint256 dayOfWeek = ((timestamp / 1 days) + 4) % 14;
        uint256 nextFriday = timestamp + ((14 + 12 - dayOfWeek) % 14) * 1 days;
        uint256 friday8am = nextFriday - (nextFriday % (24 hours)) + (8 hours);

        // If the passed timestamp is day=Friday hour>8am, we simply increment it by a week to nexttoNext Friday
        if (timestamp >= friday8am) {
            friday8am += 14 days;
        }
        return friday8am;
    ...
    function getAnyExpiry(uint256 timestamp, uint256 expiryPeriod, uint256 expiryDay) internal pure returns (uint256){
        uint256 dayOfExpiryPeriod = ((timestamp / 1 days) + 4) % expiryPeriod;
        uint256 nextExpiryDay = timestamp + ((expiryPeriod + expiryDay- dayOfExpiryPeriod) % expiryPeriod) * 1 days;
        uint256 nextExpiry8am = nextExpiryDay - (nextExpiryDay % (24 hours)) + (8 hours);


        if (timestamp >= nextExpiry8am) {
            nextExpiry8am += expiryPeriod;
        }
    ...
    ERC20Upgradeable
{
    ...
    function baseInitialize(
        ...
        require(_wmatic != address(0), "!_wmatic");
...
.
    ...
    function depositMATIC() external payable nonReentrant {
        require(vaultParams.asset == WMATIC, "!WMATIC");
...
interface ITrufinVault {
    ...
    /**
     * @notice Deposits MATIC into the contract and mint vault shares. Reverts if the asset is not WMATIC.
     */
