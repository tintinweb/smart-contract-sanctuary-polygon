...
contract EnergyToken is ERC20, AccessControl, NonblockingLzApp {
    uint16 srcChainId;
    ...
    constructor(address _endpoint, uint16 _srcChainId)
        ERC20("SmashEnergyToken", "SET")
    ...
    function _nonblockingLzReceive(
        ...
        if (srcChainId != _srcChainId) revert InvalidSrcChain();
        (address staker, uint256 totalNfts, ) = abi.decode(
            _payload,
            (address, uint256, uint256)
        );
        _mint(staker, calculateResidue(staker));

        totalStaked[staker] += totalNfts;
        lastStaked[staker] = block.timestamp;
    }

    function calculateResidue(address account) public view returns (uint256) {
        uint256 totalDaysPassed = (block.timestamp - lastStaked[account]) /
            1 days;
        return totalDaysPassed * totalStaked[account] * 3;
    }
    ...
    function balanceOf(address account)
        ...
        returns (uint256)
    {
        return ERC20.balanceOf(account) + calculateResidue(account);
    }
