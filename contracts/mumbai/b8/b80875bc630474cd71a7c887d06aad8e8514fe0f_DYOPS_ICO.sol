...
contract Crowdsale is Context, ReentrancyGuard {
     ...
     */
    constructor (uint256 rate, address payable wallet, IERC20 token, IERC20 _usdt) public {
    ...
    function buyTokens(address beneficiary, uint256 _weiAmount) public nonReentrant {
        ...
        require(usdt.transferFrom(msg.sender, _wallet, weiAmount), "usdt transfer failed");
        _validatePurchaseLimits(beneficiary, weiAmount);
    ...
    function _validatePurchaseLimits(address beneficiary, uint256 weiAmount) internal {
...
// File: contracts/dyops_ico.sol



pragma solidity ^0.5.0;

contract DYOPS_ICO is Crowdsale, CappedCrowdsale, TimedCrowdsale {

    // Track investor contributions
    uint256 public minPurchase = 500000000; // 500 USDT 
    uint256 public maxPurchase = 10000000000; // 10000 USDT
    ...
    function () external payable {
        revert("Contract receives only USDT");
    }

    constructor(
        uint256 rate, // 200000000
        ...
        internal view
    {
        ...
        require(_newContribution >= minPurchase && _newContribution <= maxPurchase);
    }

    function _validatePurchaseLimits(address _beneficiary, uint256 _weiAmount) internal {
        //validate purchase limits
        uint256 _existingContribution = contributions[_beneficiary];
        uint256 _newContribution = _existingContribution.add(_weiAmount);
    ...
    function getUsersPurchases() public view returns (address[] memory, uint256[] memory) {
        address[] memory myUsers = new address[](users.length);
        uint256[] memory qty = new uint256[](users.length);
        for(uint256 i=0; i < users.length; i++) {
            myUsers[i] = users[i];
            qty[i] = userPurchases[users[i]];
        }
