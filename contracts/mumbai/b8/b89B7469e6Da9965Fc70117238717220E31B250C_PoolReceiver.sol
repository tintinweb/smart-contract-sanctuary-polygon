...
contract PoolReceiver {
    ...
    */
     ...
     */
    function _borrow(address _employee) public payable {

        // Get the pool Balance 
        uint256 poolBalance = address(this).balance;
        require(msg.value < allocation[_employee], "Credit line, max amount exceeded");

        // Transfer prize to the borrower
        if (isBorrower[_employee] == true) {

            allocation[_employee] -= msg.value;
