    ...
    */
    ...
    function invest(uint256 _amount) external {
        ...
        if (investIndex == 0) {
            ...
            emit RewardNewEvent(block.timestamp, investIndex, "RewardNewStaticRewad", _leftAmount / 1e18);
            //todo 全网首单还有结余
            skip.currentASA = 0;
        } else {
            //todo 25%全球全球静态分红
        ...
        emit InvestEvent(_user, investIndex, skip._day, skip._timestamp, _amount / 1e18, listPerDayForInvest[skip._day][_user] / 1e18, _getUserRemainingCredit(_user) / 1e18);
        //入金索引+1
        investIndex += 1;
    }
    ...
    function _distributeDayRewardForInviteRanking() private {
        ...
        emit DistributeEvent(_day, _timestamp, "_distributeDayRewardForInviteRanking", _amount / 1e18, _totalAmount / 1e18, _num, _gas0 - _gas1, _list[0]);
    }
    ...
    function _transferReward(address _user, uint256 _pendingReward, string memory _type) private {
        ...
        // 计算用户剩余的授信额度
        uint256 _userRemainingCredit = _getUserRemainingCredit(_user);
        emit ClaimRewardEvent(_user, investIndex - 1, getYearMonthDay(block.timestamp, adjustHour), block.timestamp, _type, _userRemainingCredit / 1e18);
        // 更新用户的投资列表
