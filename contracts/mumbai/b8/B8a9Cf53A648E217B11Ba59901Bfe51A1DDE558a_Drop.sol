    ...
    IERC2981
{
        ...
        returns (Phase memory phase, uint256 phaseIndex)
    {
        ...
        for (uint256 i = 0; i < phaseStartTimestamps.length; i++) {
            if (
                current > phaseStartTimestamps[i] &&
                phaseStates[i].totalMinted < phases[i].maxPerPhase
            ) return (phases[i], i);
        }
    }
        ...
        whenNotPaused
    {
        ...
        if (
            ...
            phases[phaseIndex].phaseEnd = block.timestamp + 1 days;
        }
    }
...
struct Phase {
    ...
    uint256 phaseEnd; // user for public mint phases with min amount
}
