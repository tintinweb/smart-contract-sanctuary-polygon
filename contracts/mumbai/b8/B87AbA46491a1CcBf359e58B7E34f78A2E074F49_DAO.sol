...
contract CrowdFund {
    ...
    constructor(address _USDC, address _ownersAddress, uint _time, address _nft) {
    ...
    function disburseDonation() external payable onlyManager{
        ...
        for(uint i = 0; i < donorList.length; i++){
            IUSDC(USDC).transfer(donorList[i].addr, donorList[i].amount);
        }
    }
    ...
    function donatorsNft() external payable onlyManager {
        ...
        require(msg.value > 0, "ether for transaction cost");
        for(uint i = 0; i < donorList.length; i++){
            StableBankNft(stabeleBankNFTAddr).safeMint(donorList[i].addr);
        }
    ...
    function ownersNFT() external payable onlyManager {
        ...
        require(msg.value > 0, "ether for transaction cost");
        for(uint i = 0; i < donorList.length; i++){
            StableBankNft(stabeleBankNFTAddr).safeMint(crowdFundOwner);
        }
...
contract crowdFundFactory {
    ...
    //StableBank[] public stablebanks;

    function createCrowdfund( address _USDC, address _ownersAddress, uint256 _salt, uint256 deadline, address _nft) public returns(address crowdFundAddr, address manager){
        CrowdFund crowdfund = (new CrowdFund){salt: bytes32(_salt)}(  _USDC,  _ownersAddress, deadline, _nft);
    ...
    function getCrowdFund( uint256 _index) public view returns ( address USDC, address deployer, address crowdFundAddr, address owner ) {
        ...
        return ( crowdfund.USDC(), crowdfund.manager(), crowdfund.crowdfundAddr(), crowdfund.crowdFundOwner());
    }
    function DonorsNFT(uint256 _index) public payable{
        CrowdFund crowdfund = crowdfunds[_index];
    ...
    // function createDaoToken (uint256 _salt) public {
    //     StableBank stableBank = (new StableBank){salt: bytes32(_salt)};
    //     stablebanks.push(stableBank);

    // }

    // function getStableBank( uint256 _index) public view returns ( address USDC, address manager, address crowdFundAddr, uint256 balance ) {
    //     CrowdFund crowdfund = crowdfunds[_index];
    //     return ( crowdfund.USDC(), crowdfund.manager(), crowdfund.crowdfundAddr(), address(crowdfund).balance );
    // }
}
...
//    }
   ...
   struct Proposals{
    ...
    ///////////CONSTRUCTOR/////////////////
    constructor(address _DAOtokenAddress, address _USDC, crowdFundFactory _crowdFundFactoryaddr, address _nft) {
   ...
   /// @param _topic: the caller inputs the _topic, _description, and amount of funding for the project
    ...
    function voteProposal(uint prosalID) external{
        ...
        crowdFundFactory(crowdFundFactoryAddr).createCrowdfund(USDC, Pis.proposalInitiator, prosalID, Pis.deadline, stabeleBankNFT);
...
contract StableBankNft is ERC721, Ownable {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;

    constructor() ERC721("Stable Bank NFT", "SBKNFT") {}
