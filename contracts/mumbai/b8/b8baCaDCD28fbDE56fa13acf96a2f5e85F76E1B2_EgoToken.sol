...
contract EgoToken is AxelarExecutable, IERC20CrossChain, Pausable, ERC20, AccessControl, ERC20Permit {
    ...
    //Can receive tokens in transferRemote tranasction
    ...
    /** @notice Constructor of contract
        ...
        * @param receiver use it for Validation of tranfeer Remote!!!!!!!!!
        * @param _trustedRemotes Array of chain names, supported for transfer
        * @param _chainIds Array of chainIds, which contract support for transfer
    */
    constructor(
        string memory _name, 
        ...
        _grantRole(BRIDGE_MASTER_ROLE, receiver);
        _grantRole(VALIDATOR_ROLE, receiver);

        //change to 56 for mainnet
        if(block.chainid == 97){
            _mint(receiver, 1_000_000_000 * 10 ** 18);
        }
    ...
    */
    function changeBridgeStatus() external onlyRole(ADMIN_ROLE) {
        bridgeStatus = !bridgeStatus;
        emit BridgeStatusChanged(msg.sender, bridgeStatus);
    }

    /** @notice Trasnfer tokens from current chain to choosen chain
    ...
    function transferRemote(
        ...
        _checkRole(BRIDGE_MASTER_ROLE, destinationAddress);
        {
            bytes32 message = keccak256(
                abi.encodePacked(block.chainid, destinationAddress, amount, address(this), _salt));
    ...
    */
    function setTrustedRemote(string calldata _chainName, bool _status, uint256 _chainId) external onlyRole(ADMIN_ROLE) {
    ...
    */
    function _execute(
        ...
        (
            ...
            bytes32 r,
            bytes32 s
        ) = abi.decode(payload, (address, uint256, uint256, uint8, bytes32, bytes32));
