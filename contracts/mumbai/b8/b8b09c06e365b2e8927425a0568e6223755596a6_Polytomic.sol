...
contract Polytomic {
    ...
    function contractInfo() public view returns(uint256 matic, uint256 totalDeposits, uint256 totalPayouts, uint256 totalInvestors, uint256 tokenHolding){
        matic = address(this).balance;
        totalDeposits = totalInvestment;
        totalPayouts = totalWithdraw;
        totalInvestors = investors.length;
        tokenHolding = tokenHoldings;
        return(matic,totalDeposits,totalPayouts,totalInvestors,tokenHolding);
    }

    function getCurrentPrice() public view returns(uint256 price){
        price = (tokenHoldings>=1e18)?basePrice.mul(1e18).div(tokenHoldings):basePrice;
        return price;
    }
    ...
    function deposit(address _referer) public payable{
        ...
        user.token+=(msg.value.mul(65).div(100)).mul(price);
        tokenHoldings+=(msg.value.mul(65).div(100)).mul(price);
        ...
        deposits[msg.sender].push(Deposit(
            ...
            (msg.value.mul(65).div(100)).mul(price),
            block.timestamp
        ));
    ...
    function _distributePSI(address depositor, uint256 _psi) internal{
        ...
        for(uint256 i = 0; i < investors.length; i++){
            ...
            tokenShare = user.token.div(tokenHoldings).mul(100);
            user.PSI+=_psi.mul(tokenShare).div(100);
        }
        emit PSIDistribution(depositor,_psi);
    }

    function distributeWeeklyReward() external onlyInitiator{
        uint256 rewardShare = _calculateWeeklyReward().mul(getCurrentPrice());
        for(uint256 i = 0; i < investors.length; i++){
            User storage user = users[investors[i]];
    ...
    function workingWithdraw() public{
        ...
        uint256 withdrawable = user.totalIncome-user.workingWithdraw;
    ...
    function nonWorkingWithdraw() public{
        ...
        uint256 withdrawable = user.token.mul(perc).div(1000).div(getCurrentPrice());
        msg.sender.transfer(withdrawable);
