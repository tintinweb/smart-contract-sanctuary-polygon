...
contract Marketplace is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function createEvent(
        ...
        if (ticketTypesCount > 0) {
            publishTicket(eventId, ticketTypesCount, tickets);
        }
    ...
    function buyMarketTicket(
        ...
        require(sellingInfo[seller][ticketId].price >= MINIMUM_ASK, 'Ticket is not for sale.');
     ...
     *  @param eventId The id of the ticket to be deleted
     */
    function deleteTicket(
        uint256 eventId,
        uint256[] memory ids,
        uint256[] memory amounts
    ) external whenNotPaused onlyTicketCreatorOrOwner(eventId) {
        require(ids.length == amounts.length, 'Ids and amounts count mismatch.');

        ITicket(ticketAddress).burnBatch(msg.sender, ids, amounts);

        emit TicketDeleted(eventId, ids, amounts);
    }
    ...
    function publishTicket(
        ...
        for (uint8 i = 0; i < ticketTypesCount; i++) {
            ...
            require(tickets[i].price >= MINIMUM_ASK, 'Asking price below minimum.');
