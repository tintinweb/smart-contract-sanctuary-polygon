...
contract EtheremonWorldNFT is BasicAccessControl, NativeMetaTransaction {
    ...
    // TODO: Create mapping for > 1299 to stat = 42.
    mapping(uint32 => uint256) public STAT_MAX_CLASS;
    ...
    function setStatMaxClass(
        uint32 _classId,
        uint256 _statMax
    ) external onlyModerators {
        STAT_MAX_CLASS[_classId] = _statMax;
    }
    ...
    function mintMonster(
        ...
        uint256 maxStat = (STAT_MAX_CLASS[_classId] > 0)
            ? STAT_MAX_CLASS[_classId]
            : STAT_MAX;

        // generate base stat for the previous one
        for (uint256 i = 0; i < STAT_COUNT; i += 1) {
            uint256 seed = getRandom(msgSender(), block.number - 1, i, objId);
    ...
    // Can we use this one as single brun?
    ...
    function mod32(
        ...
        uint256 mod = getRandom(_player, _block, _seed, _count);
        return mod % 32;
    }
    ...
    function getPriceWeth(
        uint32 _classId
    ) external view returns (bool catchable, uint256 price) {
        EtheremonDataBase data = EtheremonDataBase(dataContract);
        MonsterClassAcc memory class;
        (
            class.classId,
    ...
    function getPriceEmon(
        ...
        price = emonOracleData.getEmonRatesFromEth(class.price);

        if (class.total > gapFactor && priceIncreasingRatio < 1000) {
            price += (price * (class.total - gapFactor)) / priceIncreasingRatio;
        }
        if (class.catchable == false) {
            return (classWhitelist[_classId], price);
        } else {
    ...
    function catchMonsterNFTemon(
        ...
        uint256 price = emonOracleData.getEmonRatesFromEth(class.price);
        ...
        uint8 value;
        uint8 mod = 0;
        uint64 classId64 = uint64(_classId);
