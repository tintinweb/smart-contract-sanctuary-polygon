...
contract DopeGoats is ERC721URIStorage {
    ...
    function makeDopeGoat(string memory background, string memory goatColour, bool isMale) public payable {
        ...
        //Save Details
        goatAttributes[newItemId] = DopeGoatAttributes(isMale, maxBirths, newItemId, 0, 0, background, goatColour);
    ...
    function generateURI(string memory background, string memory goatColour, bool isMale, uint256 id, uint256 remainingBirths) internal pure returns (string memory) {
            ...
            bytes(
                string(
                    abi.encodePacked(
                        ...
                        '" },{"trait_type": "Remaining Births", "value": "',Strings.toString(remainingBirths),'"}]}'
                    )
                )
            )
        );
    ...
    function giveBirth(uint256 id1, uint256 id2, bool isMale) public {
        ...
        require(goatAttributes[id1].remainingBirths>0, "Needs To be able to Birth");
        require(goatAttributes[id2].remainingBirths>0, "Needs To be able to Birth");
        ...
        //Save Details
        goatAttributes[newItemId] = DopeGoatAttributes(isMale, maxBirths,newItemId, motherId, fatherId, goatAttributes[motherId].background, goatAttributes[fatherId].colour);

        //emit
        emit DopeGoatsMinted(msg.sender, newItemId);

        goatAttributes[motherId].remainingBirths -= 1;
        emit DopeGoatUpdated(motherId);
        
        goatAttributes[fatherId].remainingBirths -= 1;
        emit DopeGoatUpdated(fatherId);
    }

    function tokenURI(uint256 _tokenId) public view override returns (string memory) { 
        return generateURI(goatAttributes[_tokenId].background, goatAttributes[_tokenId].colour, goatAttributes[_tokenId].isMale, _tokenId, goatAttributes[_tokenId].remainingBirths);
    }
