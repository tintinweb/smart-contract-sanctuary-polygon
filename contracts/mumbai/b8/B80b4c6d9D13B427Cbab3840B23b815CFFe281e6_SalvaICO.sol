...
contract SalvaICO {
    ...
    function changeMinInvest(uint256 _minInvest) external {
    ...
    function halt() external {
        require(msg.sender == admin, "ICO: only admin!");
        require(icoState != State.halted, "ICO: already halted!");
        icoState = State.halted;
    }

    /// @dev start function callable only by the admin

    function start() external {
        require(msg.sender == admin, "ICO: only admin!");
        require(icoState != State.running, "ICO: already running!");
    ...
    function invest() external payable returns (bool) {
        ...
        uint256 _amount = (_valueSent * 10**18) / tokenPrice;
        ...
        emit Invest(_to, msg.value, _amount);
    ...
    function _sendTokens(address _to, uint256 _amount) private {
        salvaContract.safeTransfer(_to, _amount);
    }
    ...
    function withdrawUnsoldTokens() external returns (bool success) {
        require(msg.sender == admin, "ICO: only admin!");
        ...
        emit UnsoldTokenWithdrawn(_to, salvaContract, _amount);
    ...
    function withdrawICOAmount() external returns (bool) {
        require(msg.sender == admin, "ICO: only admin!");
