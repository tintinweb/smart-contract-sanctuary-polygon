...
contract LifeIsAnIllusion is ERC1155, IERC2981, Ownable, ContextMixin {
    ...
    function mintBatch(
        ...
        for (uint i = 1; i <= ids.length; i++) {
            require(!pieces[i].available, "ERC1155Mint: piece already minted");
            require(
                i >= 1 && i <= _totalSupply,
                "ERC1155Mint: URI query for nonexistent token"
            );
            pieces[i].image = image[i - 1];
            pieces[i].description = description[i - 1];
            pieces[i].available = true;
        }
    ...
    function contractURI() public pure returns (string memory) {
        bytes memory dataURI = abi.encodePacked(
            ...
            '"name": "Life is an illusion",',
            '"symbol": "ILLUSION",',
            '"description": "Life is an illusion: this is a unique collection of impressionist photos dedicated to Anna`s father. \n\nLife is an illusion \nA strange game of half-slumber \nIt seems to me that \nEverything is about to collapse \nI love you \nThe sun",',
