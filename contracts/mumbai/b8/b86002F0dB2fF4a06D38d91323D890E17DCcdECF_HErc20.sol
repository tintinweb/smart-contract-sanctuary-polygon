...
contract HErc20 is HToken {
  ...
  string public constant version = "v0.1.1";
...
contract HToken is ERC1155, IERC721Receiver, ErrorReporter, AccessControlEnumerable {
  ...
  event AccrueInterest(uint256 interestAccumulated, uint256 interestPerToken, uint256 totalBorrows);
  event AccrueCouponInterest(
    Coupon activeCoupon,
    uint256 debtPrior,
    uint256 interestAccumulated,
    uint256 interestPerTokenPrior,
    uint256 interestPerTokenCurrent
  );
  ...
  event NewAdminFeeReceiver(address oldReceuver, address newReceiver);
  event NewHiveFeeReceiver(address oldReceuver, address newReceiver);
  ...
  event NewHiveFee(uint256 oldHiveFeeMantissa, uint256 newHiveFeeMantissa);
  event NewAdminFee(uint256 oldAdminFeeMantissa, uint256 newAdminFeeMantissa);
  event AdminFeesWithdrew(uint256 _amount);
  event HiveFeesWithdrew(uint256 _amount);
  event NewProtocolFeeMantissa(uint256 _oldFee, uint256 _newFee);
  ...
  uint8 constant COUPON_UNINITIALIZED = 0;
  uint8 constant COUPON_INACTIVE = 1;
  uint8 constant COUPON_ACTIVE = 2;
  ...
  constructor(
    ...
    _setHiveFeeFresh(_hiveFeeMantissa);

    // Check reserveFactorMantissa + newAdminFeeMantissa + newHiveFeeMantissa ≤ reserveFactorPlusFeesMaxMantissa
    if (adminFeeMantissa + hiveFeeMantissa + reserveFactorMantissa > reserveFactorPlusFeesMaxMantissa) revert AdminError(Error.BAD_INPUT);
  }
  ...
  function redeemInternal(uint256 _amount) internal {
    ...
    /* Fail if redeem not allowed but not in version 1*/
  ...
  function borrowInternal(uint256 _borrowAmount, uint256 _collateralId) internal {
    ...
    _borrowAmount += (_borrowAmount * protocolFeeMantissa) / 1e18;
   ...
   * @param _user owner of the coupon
  ...
  /**
   * @dev check if liquidation is allowed for this coupon, meaning it's active
  ...
  /**
   * @dev check if liquidation is allowed for this nft, meaning it's active
  ...
  function liquidationAllowedByCollateralId(uint256 _collateralId) internal view returns (bool allowed) {
    unchecked {
      ...
      for (; i < length; i++) {
        ...
        if (!collateral.active) continue;
        Coupon memory coupon = borrowCoupons[collateral.collateralId];
        if (coupon.collateralId == _collateralId) return coupon.active == COUPON_ACTIVE;
      }
    }
  }
  ...
  function getAvailableUnderlying() external view returns (uint256) {
    return getCashPrior() - totalReserves - totalAdminFees - totalHiveFees;
  }
  ...
  function getAvailableUnderlyingForUser(address _account) external view returns (uint256) {
    return (balanceOf(_account, 0) * exchangeRateStored()) / 1e18;
  }
  ...
  function getBorrowFromCoupon(uint256 _collateralId) public view returns (uint256) {
    ...
    // console.log("interestPerToken() ", (principal * (interestPerToken() - borrowCoupons[_collateralId].interestPerToken)) / 10**decimals);
    return principal + (principal * (interestPerToken() - borrowCoupons[_collateralId].interestPerToken)) / 10**decimals;
  }
  ...
  function interestPerToken() public view returns (uint256) {
    ...
    // console.log("interestPerToken: ", interestPerTokenStored + (simpleInterestFactor * 10**decimals) / totalBorrows);
  ...
  function accrueInterest() public {
    ...
    totalHiveFees += (hiveFeeMantissa * interestAccumulated) / 10**decimals;
    totalAdminFees += (adminFeeMantissa * interestAccumulated) / 10**decimals;

    emit AccrueInterest(interestAccumulated, interestPerTokenStored, totalBorrows);
  }
  ...
  function accrueInterestToCoupon(uint256 _couponId) public returns (uint256) {
    ...
    totalReserves += (reserveFactorMantissa * earned) / 10**decimals;
    totalHiveFees += (hiveFeeMantissa * earned) / 10**decimals;
    totalAdminFees += (adminFeeMantissa * earned) / 10**decimals;
  ...
  function _accrueInterestToCoupon(uint256 _couponId) internal returns (uint256) {
    ...
    uint256 earned = ((principalPrior * (interestPerTokenStored - userCoupon.interestPerToken)) / 10**decimals);
  ...
  function getDebt() external view returns (uint256) {
    return totalBorrows + ((totalBorrows * (interestPerToken() - interestPerTokenStored)) / 10**decimals);
  }
  ...
  /**
   * @notice accrues interest and sets a new hive fee for the protocol using _setHiveFeeFresh
   * @dev Admin function to accrue interest and set a new hive fee
   * @param _newHiveFeeMantissa new fee hive mantisa
   */
  function _setHiveFee(uint256 _newHiveFeeMantissa) external {
    if (!hasRole(DEFAULT_ADMIN_ROLE, msg.sender)) revert Unauthorized();

    accrueInterest();

    // _setAdminFeeFresh emits reserve-factor-specific logs on errors, so we don't need to.
    _setHiveFeeFresh(_newHiveFeeMantissa);
  }
   ...
   */
  function _setHivemind(HivemindI _newHivemind) external {
    if (!hasRole(DEFAULT_ADMIN_ROLE, msg.sender)) revert Unauthorized();
   ...
   * @param _newReceiver new fee receiver
   */
  function _setAdminFeeReceiver(address _newReceiver) external {
    if (!hasRole(DEFAULT_ADMIN_ROLE, msg.sender)) revert Unauthorized();
    if (_newReceiver == address(0)) revert WrongParams();
    address old = adminFeeReceiver;
    adminFeeReceiver = _newReceiver;
    emit NewAdminFeeReceiver(old, _newReceiver);
  }

  /**
   * @notice sets a new hive fee receiver
   * @param _newReceiver new fee receiver
   */
  function _setHiveFeeReceiver(address _newReceiver) external {
    if (!hasRole(DEFAULT_ADMIN_ROLE, msg.sender)) revert Unauthorized();
    if (_newReceiver == address(0)) revert WrongParams();
    address old = hiveFeeReceiver;
    hiveFeeReceiver = _newReceiver;
    emit NewHiveFeeReceiver(old, _newReceiver);
  }

  /**
   * @notice sets a new protocol fee
   * @dev can not be more than 1%
   * @param _newFee new fee mantissa
   */
  function _setProtocolFeeMantissa(uint256 _newFee) external {
    if (!hasRole(DEFAULT_ADMIN_ROLE, msg.sender)) revert Unauthorized();
    if (_newFee > 1e16) revert WrongParams();
    uint256 old = protocolFeeMantissa;
    protocolFeeMantissa = _newFee;
    emit NewProtocolFeeMantissa(old, _newFee);
  }

  /**
   * @notice Sets a new hive fee for the protocol (*requires fresh interest accrual)
   * @param _newHiveFeeMantissa new hive fee mantisa
   */
  function _setHiveFeeFresh(uint256 _newHiveFeeMantissa) internal {
    // Verify market's block number equals current block number
    if (accrualBlockNumber != getBlockNumber()) {
      revert AdminError(Error.MARKET_NOT_FRESH);
    }

    // Sanitize newHiveFeeMantissa
    if (_newHiveFeeMantissa == type(uint256).max) _newHiveFeeMantissa = hiveFeeMantissa;

    // Check reserveFactorMantissa + newAdminFeeMantissa + newHiveFeeMantissa ≤ reserveFactorPlusFeesMaxMantissa
    if (reserveFactorMantissa + adminFeeMantissa + _newHiveFeeMantissa > reserveFactorPlusFeesMaxMantissa) {
      revert AdminError(Error.BAD_INPUT);
    }

    // If setting hive fee
    if (hiveFeeMantissa != _newHiveFeeMantissa) {
      // Set hive fee
      uint256 oldHiveFeeMantissa = hiveFeeMantissa;
      hiveFeeMantissa = _newHiveFeeMantissa;

      // Emit event
      emit NewHiveFee(oldHiveFeeMantissa, _newHiveFeeMantissa);
  ...
  function _setAdminFeeFresh(uint256 _newAdminFeeMantissa) internal {
    ...
    // Check reserveFactorMantissa + newAdminFeeMantissa + newHiveFeeMantissa ≤ reserveFactorPlusFeesMaxMantissa
    if (reserveFactorMantissa + _newAdminFeeMantissa + hiveFeeMantissa > reserveFactorPlusFeesMaxMantissa) {
      revert AdminError(Error.BAD_INPUT);
    }
  ...
  function _setReserveFactorFresh(uint256 _newReserveFactorMantissa) internal {
    ...
    if (_newReserveFactorMantissa + adminFeeMantissa + hiveFeeMantissa > reserveFactorPlusFeesMaxMantissa) {
      revert AdminError(Error.BAD_INPUT);
    }
