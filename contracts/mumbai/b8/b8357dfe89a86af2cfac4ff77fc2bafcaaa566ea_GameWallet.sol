...
contract GameWallet {
    ...
    //address
    address private gomokuContract;  //ownerAddr

    //mapping
    mapping (uint256=>uint256)public rate; //gameRate
    ...
    modifier onlyGomoku ()
    {
        require(gomokuContract == msg.sender,"you are not Owner");
        _;
    }

    constructor(address gomoku)
    {
        sptoken = new SPToken();
        bptoken = new BPToken();
        gomokuContract = gomoku;
        //rateの初期設定
        //追加可能想定で作成
        rate[0] = 1000 gwei;
        rate[1] = 5000 gwei;
    ...
    function getBpSeparater() external view returns (bytes32)
    {
        return bptoken.DOMAIN_SEPARATOR();
    }

    //払い出し処理
    function withDraw(address winner, address loser, uint256 rateId) public onlyGomoku
    {
        if(winner == address(0x0))
        {
            //引き分けの場合は両方負け
            lockBP -= rate[rateId]*2;
        }
        else
        {
            //ロックしているBPトークンを減らす
            ...
            //BPを勝者に払い出す
            //勝者はプレイヤー１と2のトークン総取り
            bptoken.withdraw(winner, rate[rateId]*2);
            //SPを敗者にBETしたトークンと同じ量払い出す
            sptoken.withdraw(loser, rate[rateId]);
        }
    }

    //プレイヤーの所持しているBPがrate以上かどうかのチェックを行う
    function chkHasBpToken(address player, uint256 rateId) public view  returns (bool) 
    {
        if( bptoken.balanceOf(player) >= rate[rateId])
        {
            //レート以上所有しているためtryeを返す
            return true;
        }
        return false;
    }
    ...
    //rateに応じたBPをBET
    function payBP(address player, uint256 pay,GameWalletStruct.PermitArgs memory permitStruct,bool isPlayer2) public onlyGomoku returns (bool) 
    {
        //rateに応じたBPをこのGameWalletContractにtransfer
        bptoken.permit(permitStruct.owner,permitStruct.spender,pay,permitStruct.deadline,permitStruct.v,permitStruct.r,permitStruct.s);
        bptoken.transferFrom(player,permitStruct.spender,pay);
        //預かったBPに関しては使用してしまうと終了時の報酬がなくなる可能性があるため使用不可に
        lockBP += pay;
        return true;
    }

    //BPを購入
    function buyBP() payable public returns(uint256)
    {
        //1000 gweiからゲームに参加可能のため1000 gweiを最低購入数とする。
        require(msg.value % 1000 gwei == 0,"Must be in 1000gwei units");
        //一度の最大購入数を0.1etherとする
        require(msg.value <= 1e17 ,"Must be 0.1 ether or less");
        //購入しようとしたBPがGameWallet所持のBP-参加費で預かったBPを超えていないかチェック
        require(msg.value <= (bptoken.balanceOf(address(this)) - lockBP),"short BP");
        //指定された数量購入
        bptoken.transfer(msg.sender,msg.value);

        //購入可能な残BP数を返却
        return bptoken.balanceOf(msg.sender) - lockBP;
    }

    //現在プレイヤーが所持しているBP数を返す
    function hasBp(address player) public view returns(uint256)
    {
        return bptoken.balanceOf(player);
    }


    //現在プレイヤーが所持しているBP数を返す
    function getBpNonce(address player) public view returns(uint256)
    {
        return bptoken.nonces(player);
    }

    //現在プレイヤーが所持しているSP数を返す
    function hasSp(address player) public view returns(uint256)
    {
        return sptoken.balanceOf(player);
    }
...
contract BPToken is ERC20Permit
{
    address private ownerAdr;
    ...
    constructor()ERC20("BPToken","BP")ERC20Permit("BPToken")
    {
...
contract Gomoku {
    ...
    {
        gamewallet = GameWallet(_gamewallet);
    }

    function createGame(uint8 availableStatus, uint256 rateId) public returns(uint256)
    {
        ...
        //rateが存在しているか
        require(gamewallet.rate(rateId) != 0,"not exist rate");
        ...
        //rateIdをセット
        wholeGame[gameKey].rateId = rateId;
        //ゲーム作成のイベント送信
    ...
    function entryGame(uint256 gamekey ,GameWalletStruct.PermitArgs memory permitStruct) public {
        //ゲームがOPENでなければrevert
        ...
        //所持BPがレートのBPに満たない
        require(gamewallet.chkHasBpToken(msg.sender,wholeGame[gamekey].rateId),"out of BP");

        if(wholeGame[gamekey].player1Addr == address(0x0))
        {
            //支払いをしてゲームに参加
            gamewallet.payBP(msg.sender,gamewallet.rate(wholeGame[gamekey].rateId),permitStruct,false);
        ...
        {
            //支払いをしてゲームに参加
            gamewallet.payBP(msg.sender,gamewallet.rate(wholeGame[gamekey].rateId),permitStruct,true);
    ...
    function gameEnd(uint256 gamekey,uint8 endFlg) internal
    {
        ...
        if(wholeGame[gamekey].endFlg == 1)
        {
            ...
            if(wholeGame[gamekey].player1Addr == wholeGame[gamekey].winnerAddr)
            {
                gamewallet.withDraw(wholeGame[gamekey].player1Addr, wholeGame[gamekey].player2Addr, wholeGame[gamekey].rateId);
            }
            else
            {
                gamewallet.withDraw(wholeGame[gamekey].player2Addr, wholeGame[gamekey].player1Addr, wholeGame[gamekey].rateId);
            }
        }
        else if(wholeGame[gamekey].endFlg == 2)
        {
            gamewallet.withDraw(address(0x0), address(0x0), wholeGame[gamekey].rateId);
        }
        
       emit gameEndEvent(gamekey,wholeGame[gamekey].endFlg,wholeGame[gamekey].winnerAddr);
    }

    function getRateValue(uint256 gamekey)public view returns (uint256)
    {
        return gamewallet.rate(wholeGame[gamekey].rateId);
    }
