...
import "./WantedStorageV2.sol";

contract WantedTokenV2 is ERC20Upgradeable, WantedStorageV2{


    function _feeCalculation(uint256 _percent, uint256 _amount) private pure returns(uint256 fees){
    ...
    function setToknomics( uint256[] calldata _burnPercent, uint256[] calldata _liqPercent, uint256[] calldata _marketPercent, uint256[] calldata _nftLotteryPercent, uint256[] calldata _sustainPercent, uint256[] calldata _tokenLotteryPercent) onlyOwner external returns(bool){
        uint256 i;
        for(i = 0; i < 3; i++){
            ...
            data.nftLotteryPercent = _nftLotteryPercent[i];
            data.sustainPercent = _sustainPercent[i];
            data.tokenLotteryPercent = _tokenLotteryPercent[i];
        }
        return true;
    }

    function updateSaleStatusAndPrice(uint256 _price, bool _status) external onlyOwner {
        tokenPrice = _price;
        saleStatus = _status;
    }

    function updateWallets(address _bWallet, address _lsmWallet, address _nlWallet, address _tlWallet) external onlyOwner {
        burnWallet = _bWallet;
        liqMarkSustWallet = _lsmWallet;
        nftLotteryWallet = _nlWallet;
        tokenLotteryWallet = _tlWallet;
    }
    ...
    function buyToken() payable external returns(bool){
        ...
        uint256 percent = TokenomicDetails[1].liqPercent + TokenomicDetails[1].marketPercent + TokenomicDetails[1].sustainPercent;

        uint256 _amount = _feeCalculation(percent, tempAmount);
        _transfer(owner(), liqMarkSustWallet, _amount);
        amount -= _amount;

        _amount = _feeCalculation(TokenomicDetails[1].burnPercent, tempAmount);
        _transfer(owner(), burnWallet, _amount);
        amount -= _amount;

        _amount = _feeCalculation(TokenomicDetails[1].nftLotteryPercent, tempAmount);
        _transfer(owner(), nftLotteryWallet, _amount);
        amount -= _amount;
    ...
    function userTotalAmount(address _user) public view returns(uint256 TotalLockAmount, uint256 TotalClaimedBalance, uint256 TotalUnclaimedBalance){
        ...
        for(uint i = 0; i < userInvestingIds[_user].length; i++){
            ...
            if(releaseAmount != 0){
                releaseAmt += releaseAmount;
            }
            if((remainingBalance - releaseAmt) != 0){
                remainAmt += (remainingBalance - releaseAmt);
            }
            if((invests[Id].releaseCounter * ratePerSection) != 0){
                claimAmt += (invests[Id].releaseCounter * ratePerSection);
            }  
        }
    ...
    function transferFrom(
        ...
        if(pairAddress[to] == true){
            uint256 percent = TokenomicDetails[3].liqPercent + TokenomicDetails[3].marketPercent + TokenomicDetails[3].sustainPercent;

            uint256 _amount = _feeCalculation(percent, tempAmount);
            _transfer(owner(), liqMarkSustWallet, _amount);
            amount -= _amount;

            _amount = _feeCalculation(TokenomicDetails[3].burnPercent, tempAmount);
            _transfer(owner(), burnWallet, _amount);
            amount -= _amount;

            _amount = _feeCalculation(TokenomicDetails[3].tokenLotteryPercent, tempAmount);
            _transfer(owner(), tokenLotteryWallet, _amount);
            amount -= _amount;

            _amount = _feeCalculation(TokenomicDetails[3].nftLotteryPercent, tempAmount);
            _transfer(owner(), nftLotteryWallet, _amount);
            amount -= _amount;

        }else{
            uint256 percent = TokenomicDetails[2].liqPercent + TokenomicDetails[2].marketPercent + TokenomicDetails[2].sustainPercent;

            uint256 _amount = _feeCalculation(percent, tempAmount);
            _transfer(owner(), liqMarkSustWallet, _amount);
            amount -= _amount;

            _amount = _feeCalculation(TokenomicDetails[2].burnPercent, tempAmount);
            _transfer(owner(), burnWallet, _amount);
            amount -= _amount;

            _amount = _feeCalculation(TokenomicDetails[2].tokenLotteryPercent, tempAmount);
            _transfer(owner(), tokenLotteryWallet, _amount);
            amount -= _amount;
        }   
    ...
    function randomnumber(uint256 numOfHolders) public view returns (uint256[] memory Numbers){
        uint256[] memory numbers = new uint256[](200);
        uint seed;
        uint256 start = uint(keccak256(abi.encode(block.timestamp,  seed))) % numOfHolders;
        seed = uint(keccak256(abi.encode(block.timestamp, seed)))%20;
        uint ptr = start;
        uint i = 0;
        uint limit = 200;
        while(i<limit && ptr<numOfHolders){
            numbers[i] = ptr; 
            i++;
            ptr = ptr + seed;
            if(ptr == start) {
        ...
        if(ptr >= numOfHolders && i<limit) ptr =(ptr) % numOfHolders;

        Numbers = numbers;
    }

    function rewardsDistribution(RewardOrder[] calldata order) onlyOwner external returns(bool){
        for(uint256 i = 0; i < order.length; i++){
            if(rewardData[order[i].user].rewardReceived == false){
                RewardData storage data = rewardData[order[i].user];
                data.user = order[i].user;
                data.amount = order[i].amount;
    ...
    function claim() external returns(bool){
        require(rewardData[msg.sender].rewardReceived && !rewardData[msg.sender].claimed, "WantedTokenV1: Already claimed or not eligible");
        _transfer(tokenLotteryWallet, msg.sender, rewardData[msg.sender].amount);
        return true;
    }
}
...
abstract contract WantedStorageV2 is OwnableUpgradeable {
    ...
    address burnWallet;
    address liqMarkSustWallet;
    address nftLotteryWallet;
    address tokenLotteryWallet;
    ...
    struct Tokenomics {
        ...
        uint256 nftLotteryPercent;
        uint256 tokenLotteryPercent;
    }
    ...
    mapping (address => bool) public rewardRecived;
    ...
    mapping(address => bool) nftLotteryWinner;
    ...
    mapping(address => TokenHolder) tokenHolderMap;
