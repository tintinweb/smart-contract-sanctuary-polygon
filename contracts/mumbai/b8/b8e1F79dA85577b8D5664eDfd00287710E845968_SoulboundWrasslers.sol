...
contract SoulboundWrasslers is ERC721, Ownable, ERC2771Context {
    ...
    modifier onlyForwarder() {
        require(isTrustedForwarder(msg.sender), "SBW:429");
        _;
    }
    ...
    function mint(string calldata dna) external onlyForwarder {
        require(balanceOf(_msgSender()) == 0, "Sender already minted");
        _tokenIds.increment();
        dnas[_tokenIds.current()] = dna;

        nFight.registerPolygonNFT(address(this), _tokenIds.current(), _msgSender());
