...
// ref: https://github.com/gelatodigital/ops/blob/master/contracts/integrations/OpsReady.sol
...
import {OpsTaskCreator} from "./gelato/OpsTaskCreator.sol";
import "./gelato/Types.sol";

contract Test is OpsTaskCreator {
    ...
    function eoaFn(uint256 _duration) external {
        ...
        moduleData.args[0] = _timeModuleArg(block.timestamp, _duration);
        moduleData.args[1] = _singleExecModuleArg();

        bytes memory execData = abi.encodeWithSelector(this.execFn.selector, 5);

        bytes32 taskId = _createTask(address(this), execData, moduleData, ETH);
    }

    function execFn(uint256 _amount) external onlyOps {
        testNumber += _amount;
