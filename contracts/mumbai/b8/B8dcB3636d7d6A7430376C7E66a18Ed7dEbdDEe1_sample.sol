...
import "./IGLNFT1155.sol";
import "./IGLNFT721.sol";
    ...
    UUPSUpgradeable
{
    ...
    function mintNFTAndExecuteOrder(
        address celebrityAddress,
        string memory tokenURI,
        Order calldata order,
        Sign calldata sign
    ) external whenNotPaused {
        bytes32 hash = hashMintAndExecuteOrder(
            celebrityAddress,
            tokenURI,
            order
        );
        ...
        if (order.assetType == AssetType.ERC721) {
            require(
                IGLNFT721(order.asset).safeMint(
                    celebrityAddress,
                    order.tokenId,
                    tokenURI),
                "ERC721 token transfer failed");
        }
        if (order.assetType == AssetType.ERC1155) {
            require(
                IGLNFT1155(order.asset).safeMint(
                    celebrityAddress,
                    order.tokenId,
                    order.supply,
                    tokenURI),
    ...
    function _executeOrder(Order calldata order, bytes32 hash) internal {
        ...
        if (order.assetType == AssetType.ERC721) {
            IGLNFT721(order.asset).safeTransferFrom(
                order.seller,
                order.buyer,
                order.tokenId
            );
        ...
        if (order.assetType == AssetType.ERC1155) {
            IGLNFT1155(order.asset).safeTransferFrom(
                order.seller,
                order.buyer,
                order.tokenId,
                order.supply,
...
interface IGLNFT1155 is IERC1155Upgradeable {
    function safeMint(
        address receiver,
        uint256 tokenId,
        uint256 supply,
...
interface IGLNFT721 is IERC721Upgradeable {
    function safeMint(
        address receiver,
        uint256 tokenId,
        string calldata tokenURI
