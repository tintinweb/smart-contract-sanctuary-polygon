...
import "hardhat/console.sol";
 ...
 /**
   * @notice PartnerDao interface for getting collection data
   * Collection Dao should have collectionHouseEdge value
   *
   * EXAMPLE: 
   * Dao house edge
   * uint public collectionHouseEdge = 20_000;
   *
   */
interface PartnerDao {
  function collectionHouseEdge() external view returns(uint);
}
...
contract Game is Ownable {
   ...
   * @param _collectionDaoAddress card collection DAO address
   * @return cost the total cost for the mint
   * @return costHouseEdge amount of total cost that goes to DAO
   * @return collectionDaoCost amount of total cost that goes to collection DAO
   */
  function mintRandomCardsCost(uint8 _typeFloor, uint256 _amount, uint16 _collection, address payable _collectionDaoAddress) public view returns (uint256 cost, uint256 costHouseEdge, uint256 collectionDaoCost) {
    ...
    // Get collectionDaoHouseEdge using smart contract address
    uint256 collectionDaoHouseEdge = PartnerDao(_collectionDaoAddress).collectionHouseEdge();

    // cost that should be paid to partner Dao
    uint256 collectionDaoCost = cost/100_000 * collectionDaoHouseEdge;
    
    cost +=  collectionDaoCost;
    console.log(cost);
    return (cost, costHouseEdge, collectionDaoCost);
  }
   ...
   */
  function mintRandomCards(uint8 _typeFloor, uint256 _amount, uint16 _collection, address payable _collectionDaoAddress) public payable {
    ...
    uint256 collectionDaoCost;
    
    (cost, costHouseEdge, collectionDaoCost) = mintRandomCardsCost(_typeFloor, _amount, _collection, _collectionDaoAddress);
    ...
    if (collectionDaoCost > 0) {
      //Send % to collection partner
      _collectionDaoAddress.sendValue(collectionDaoCost);
    }
