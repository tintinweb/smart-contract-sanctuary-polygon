...
contract IronFairiesNFT is ERC721A, Ownable, ReentrancyGuard, AccessControl {
    ...
    uint256 MAX_PRIVATESALE_SUPPLY = 300;
    uint256 MAX_PUBLICSALE_SUPPLY = 9200;
    ...
    modifier whenPublicSaleNotPaused() {
        require(
            !_pausedPublicSale,
            "[IronFairiesNFT.whenPublicSaleNotPaused] Public Sale Paused"
        );
        _;
    }
    ...
    modifier whenPublicSalePaused() {
        require(
            _pausedPublicSale,
            "[IronFairiesNFT.whenPublicSalePaused] Public Sale Not Paused"
        );
        _;
    }
    ...
    modifier whenPrivateSaleNotPaused() {
        require(
            !_pausedPrivateSale,
            "[IronFairiesNFT.whenPrivateSaleNotPaused] Private Sale Paused"
        );
        _;
    }
    ...
    modifier whenPrivateSalePaused() {
        require(
            _pausedPrivateSale,
            "[IronFairiesNFT.whenPrivateSalePaused] Private Sale Not Paused"
        );
        _;
    }

    function mintPrivateSale(uint256 quantity)
        external
        payable
        nonReentrant
