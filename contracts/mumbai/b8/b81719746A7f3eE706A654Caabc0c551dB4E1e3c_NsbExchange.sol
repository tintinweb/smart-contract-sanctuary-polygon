...
// File: utils/TermNft.sol



pragma solidity 0.8.17;

abstract contract TermNft {
    /**
    @dev Enum representing the types of collaterals that can be used.
    */
    enum CollateralType {
        ERC721,
        ERC721A,
    ...
    /**
    @dev Enum representing the rarity levels of the NFTs.
    */
    enum RarityLevel {
        COMMON,
        RARE,
    ...
    /**
    @dev Enum representing the types of incentives for NFTs.
    */
    enum IncentiveNft {
        FIRST_INCENTIVE,
        SECOND_INCENTIVE,
    ...
    /**
    @dev Struct representing information about an NFT.
    */
    struct NftInfo {
        address copyrightHoldersNFT;
        uint256 royaltyPercentage;
        uint256 chanceOfWinning;
        CollateralType collateralType;
        RarityLevel rarityLevel;
        IncentiveNft incentiveNft;
        bool isTransferableInMP;
    }
}
// File: interfaces/IERC721Infos.sol
...
interface IERC721Infos {
    function getChanceOfWinning(uint256 tokenId) external view returns(uint256);
}
...
contract NsbExchange is Ownable {
    ...
    // Address is owner of list contract
    ...
    //ERC1155
    // addrs: from, to, token
    event BuyERC1155Normal(address[3] addrs, uint256 tokenId, uint256 amount, uint256 value);
    event BuyERC1155ETH(address[3] addrs, uint256 tokenId, uint256 amount, uint256 value);
    event AcceptOfferERC1155(address[3] addrs, uint256 tokenId, uint256 amount, uint256 value);
    event AuctionERC1155(address[3] addrs, uint256 tokenId, uint256 amount, uint256 value);
    
    function initialize(address _erc721, address _erc1155) external {
    ...
    modifier verifySignatureRandomNft(
        uint256 nonce,
        address[5] memory _tradeAddress,
    ...
    function checkFeeProductExits(
        address[5] memory _tradeAddress,
        uint256[4] memory _attributes
    ) private pure returns (uint256 amount, uint256 feeOwner, uint256 feeAdmin) {
    ...
    function buyRandomNftNormal(
        address[5] memory _tradeAddress,
        uint256[3] memory _attributes,
        uint256[] memory _listTokenId,
        uint256 nonce,
        bytes memory signature
    ) external verifySignatureRandomNft(nonce, _tradeAddress, _attributes, _listTokenId, signature) {
        require(_listTokenId.length > 0, "List tokenId is empty");

        uint256 randomNumber = getRandomReward(_listTokenId);
        
        _buyNFTNormal(_tradeAddress, 
            [_attributes[0], randomNumber, _attributes[1], _attributes[2]]
        );
    }
    ...
    function buyNFTNormal(
        ...
        uint256 nonce,
        bytes memory signature
    ) public verifySignature(nonce, _tradeAddress, _attributes, signature) {
        _buyNFTNormal(_tradeAddress, _attributes);
    }
    ...
    function auctionNFT(
        ...
        if (feeOwner != 0) {
            ...
            IERC20(tradeInfo.tradeAddress[2]).safeTransferFrom(
                tradeInfo.tradeAddress[0],
    ...
    //Handle for ERC1155
    // address[5]: buyer, seller, token, fee, feeAdmin
    // uint256[4]: amount, tokenId, feePercent, feePercentAdmin, value
    modifier verifySignatureERC1155(
        uint256 nonce,
        address[5] memory _tradeAddress,
    ...
    function checkFeeProductExitsERC1155(
        address[5] memory _tradeAddress,
        uint256[5] memory _attributes
    ) private pure returns (uint256 amount, uint256 feeOwner, uint256 feeAdmin) {
        uint256 totalFeeTrade;
    ...
    // Buy NFT normal by token ERC-20
    // address[5]: buyer, seller, token, fee, feeAdmin
    // uint256[5]: amount, tokenId, feePercent, feePercentAdmin
    function buyERC1155Normal(
        address[5] memory _tradeAddress,
        uint256[5] memory _attributes,
        uint256 nonce,
        bytes memory signature
    ) external verifySignatureERC1155(nonce, _tradeAddress, _attributes, signature) {
        DataERC1155 memory tradeInfo = DataERC1155({
            tradeAddress: _tradeAddress,
            attributes: _attributes
        });
        // check allowance of buyer
        ...
        (uint256 amount, uint256 feeOwner, uint256 feeAdmin) = checkFeeProductExitsERC1155(
            tradeInfo.tradeAddress,
            tradeInfo.attributes
        );
        ...
        IERC1155(ERC1155).safeTransferFrom(
            ...
            msg.sender,
            tradeInfo.attributes[1],
            tradeInfo.attributes[4],
            ""
        );
        emit BuyERC1155Normal(
            [msg.sender, tradeInfo.tradeAddress[1], tradeInfo.tradeAddress[2]],
            tradeInfo.attributes[1],
            tradeInfo.attributes[0],
            tradeInfo.attributes[4]
    ...
    // Buy NFT normal by token ERC-20
    // address[5]: buyer, seller, token, fee, feeAdmin
    // uint256[5]: amount, tokenId, feePercent, feePercentAdmin, value
    function buyERC1155ETH(
        address[5] memory _tradeAddress,
        uint256[5] memory _attributes,
        uint256 nonce,
        bytes memory signature
        ...
        external
        payable
        verifySignatureERC1155(nonce, _tradeAddress, _attributes, signature)
    {
        DataERC1155 memory tradeInfo = DataERC1155({
            tradeAddress: _tradeAddress,
            attributes: _attributes
    ...
    function acceptOfferERC1155(
        ...
        require(
            IERC1155(ERC1155).isApprovedForAll(tradeInfo.tradeAddress[1], address(this)),
            "tokenId do not approve for contract"
        );
        // check allowance of buyer
        require(
    ...
    /**
    * @dev Calculates a random reward based on the total chances of winning from a list of ERC721 tokens.
    * @param _listTokenId An array of ERC721 token IDs to calculate chances of winning from.
    * @return The ID of the randomly selected token.
    */
    function getRandomReward(uint256[] memory _listTokenId) private view returns(uint256){
        require(_listTokenId.length > 0, "List tokenId not invalid");
        uint256 totalUnlock;
        for(uint256 i = 0; i < _listTokenId.length; i++) {
            totalUnlock = totalUnlock + IERC721Infos(ERC721).getChanceOfWinning(_listTokenId[i]);
        }
        
        uint256 ramdomNumber = generateRamdomNumber(totalUnlock);

        uint256 currentUnlock;
        uint256 idReward;
        for(uint256 i = 0; i < _listTokenId.length; i++) {
            uint256 currentChanceOfWinning = IERC721Infos(ERC721).getChanceOfWinning(_listTokenId[i]);
            if(ramdomNumber <= currentChanceOfWinning + currentUnlock) {
                idReward = _listTokenId[i];
                break;
            }
            currentUnlock = currentUnlock + currentChanceOfWinning;
        }
        return idReward;
    }   
 
    /**
    * @dev Generates a random number within a specified limit
    * @param _ramdomWithLimitValue The upper limit of the random number
    * @return The generated random number
    */
    function generateRamdomNumber(uint256 _ramdomWithLimitValue) private view returns(uint256){
        ...
        ) % _ramdomWithLimitValue + 1;
        return firstRamdomValue;
    }
}