...
{
    // ONLY ALLOW BUSD
    uint256 constant public secondInDay = 86400;
    ...
    mapping(address => HistoryPack[]) public poolFen;
    ...
    function registryGuildUser(
        ...
        require(isUserGuild[_user] == false, "user already in guild list");
        require(
        ...
        returns (Reward[] memory)
    {
        ...
        rewards = new Reward[](poolFen[user].length);
        // uint256 sum;
        for (uint256 i = 0; i < poolFen[user].length; i++) {
            if (!poolFen[user][i].isEnd) {
                if (
                    toTimeStamp >
                    poolFen[user][i].startTime + lockDays * secondInDay
                ) {
                    uint256 totalMonthPassed = (toTimeStamp -
                        poolFen[user][i].startTime -
                        lockDays *
                        secondInDay) / 30;
                    totalMonthPassed = totalMonthPassed > (vestingDays) / 30
                        ? (vestingDays) / 30
                        : totalMonthPassed;
                    uint256 sum = (poolFen[user][i].amount *
                        mothlyUnlockRate *
                        totalMonthPassed) /
                        10000 +
                        (poolFen[user][i].amount * tge) /
                        10000;
                    rewards[index] = Reward(
                        i,
                        sum - poolFen[user][i].receivedAmount
                    );
                    index++;
                } else if (toTimeStamp > poolFen[user][i].startTime) {
                    rewards[index] = Reward(
                        i,
                        (poolFen[user][i].amount * tge) /
                            10000 -
                            poolFen[user][i].receivedAmount
                    );
                    index++;
                }
            }
