...
contract EthermonStaking is EthermonStakingBasic {
    ...
    function depositeTokens(DepositeToken calldata depositeData)
        external
        returns (uint256)
    {
        require(
        ...
        // TokenLocked memory lockedData = data.lockedData;
        // TokenMisc memory miscData = data.miscData;
        ...
        data.teamPower =
            ...
            data.level *
            weight.getClassWeight(data.classId) *
        ...
        uint256 hourlyEmon = (((teamPower / sumTeamPower) *
            emonPerPeriod *
            (currentTime - data.lastCalled)) / (1 minutes)) *
            data.validTeam;
        data.balance += hourlyEmon;

        bytes memory output = abi.encode(data);
        uint256 t = stakingData.addTokenData(output);
    ...
    function updateTokens(
        ...
        uint256 timeElapsed = (currentTime - data.lastCalled) /
            (1 minutes);
        data.lastCalled = currentTime;
        ...
        data.level = _level;
        uint256 rarity = weightData.getClassWeight(data.classId);
    ...
    function withdrawRewards(uint64 _lockId) external {
        EthermonStakingInterface stakingData = EthermonStakingInterface(
            stakingDataContract
        );
        uint256 currentTime = now;
