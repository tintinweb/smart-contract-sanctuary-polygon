...
contract WhitelistCreateHook {
    ...
    function startup(bytes memory initializeData) external onlyHookManager {
        (signer) = abi.decode(initializeData, (address));
        require(signer != address(0), "invalid account");
    }

    function onAction(bytes memory data) external payable onlyHookManager {
        (address sender, , ) = abi.decode(data, (address, address, uint256));
        require(signer == sender, "only signer");
    }
}