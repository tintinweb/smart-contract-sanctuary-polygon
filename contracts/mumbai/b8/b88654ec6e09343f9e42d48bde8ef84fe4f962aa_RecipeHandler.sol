...
contract RecipeHandler is Ownable, StudioSignature {
    ...
    function openChest(RecipeSignature calldata packet) external {
        ...
        require(
            outIds.length == outAmts.length,
            "Ids must be same length as amounts"
        );
    ...
    function _xorBytes(bytes memory target, address caller)
        internal
        pure
    ...
    function xorBytes(bytes memory target, address caller)
        external
        pure
        returns (bytes memory out)
    {
