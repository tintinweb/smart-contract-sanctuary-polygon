...
contract Metaverse_Global is Ownable {
    ...
    function registration(address userAddress, address referrerAddress,uint8 level) private {
        ...
        if((users[referrerAddress].entry_time+1 days)>=block.timestamp && !users[referrerAddress].islevelActive)
        {
        ...
        for(uint256 i = 0; i < 7; i++){
            if(upline != address(0)){
                ...
                if(users[upline].partnersCount >= levelConditions[i] && users[upline].islevelActive){
                    ...
                    emit Transaction(upline,_user,reward,1,2);
                } 
                else 
                {
                    royaltyInfo[id1].levelincome += reward;                       
                    royaltyInfo[id1].totalincome +=reward;
                    emit Transaction(id1,_user,reward,1,2);
                }               
