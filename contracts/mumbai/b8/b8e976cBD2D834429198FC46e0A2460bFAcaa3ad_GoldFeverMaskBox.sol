    ...
    UUPSUpgradeable
{
    ...
    struct CreateBoxData {
        uint256[] maskShapes;
        uint256[] maskMaterials;
        uint256[] otherMaskElements;
    }
    ...
    function initialize(address admin,
        ...
        bytes32 keyHash_) public initializer {
        __GoldFeverMaskBox_init(admin, gfiContract_, vrfCoordinator_, subscriptionId_, keyHash_);
    }

    function __GoldFeverMaskBox_init(address admin,
        address gfiContract_,
        address vrfCoordinator_,
        uint64 subscriptionId_,
        bytes32 keyHash_) internal onlyInitializing {
        __ReentrancyGuard_init();
        __ERC721_init("GFMaskBox", "GFMB");
        __VRFConsumerBaseV2_init(vrfCoordinator_);
        __GoldFeverMaskBox_init_unchained(admin, gfiContract_, vrfCoordinator_, subscriptionId_, keyHash_);
        __UUPSUpgradeable_init();
    }

    function __GoldFeverMaskBox_init_unchained(address admin,
        address gfiContract_,
        address vrfCoordinator_,
        uint64 subscriptionId_,
        bytes32 keyHash_) internal onlyInitializing {
    ...
    function createBoxesBatch(CreateBoxData[] calldata boxs)
        external
        only(DEFAULT_ADMIN_ROLE)
    {
        for (uint256 i = 0; i < boxs.length; i++) {
            _createBoxes(
                boxs[i].maskShapes,
                boxs[i].maskMaterials,
                boxs[i].otherMaskElements
            );
        }
    }
