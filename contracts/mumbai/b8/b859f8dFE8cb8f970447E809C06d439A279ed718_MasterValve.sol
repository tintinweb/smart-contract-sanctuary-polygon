...
import './interfaces/IVePvl.sol';
import './interfaces/IPvl.sol';
import './interfaces/IMasterValve.sol';
import './interfaces/IRewarder.sol';

/// This contract rewards users in function of their amount of lp staked (dialuting pool) factor (non-dialuting pool)
/// Factor and sumOfFactors are updated by contract VePvl.sol after any vePvl minting/burning (veERC20Upgradeable hook).
/// Note that it's ownable and the owner wields tremendous power. The ownership
/// will be transferred to a governance smart contract once Valve is sufficiently
/// distributed and the community can show to govern itself.
contract MasterValve is
    Initializable,
    ...
    IMasterValve
{
    ...
    struct UserInfo {
        ...
        uint256 factor; // non-dialuting factor = sqrt (lpAmount * vePvl.balanceOf())
        //
        // We do some fancy math here. Basically, any point in time, the amount of PVLs
        // entitled to a user but is pending to be distributed is:
        //
        //   ((user.amount * pool.accPvlPerShare + user.factor * pool.accPvlPerFactorShare) / 1e12) -
        //        user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accPvlPerShare`, `accPvlPerFactorShare` (and `lastRewardTimestamp`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardTimestamp; // Last timestamp that PVLs distribution occurs.
        uint256 accPvlPerShare; // Accumulated PVLs per share, times 1e12.
        IRewarder rewarder;
        uint256 sumOfFactors; // the sum of all non dialuting factors by all of the users in the pool
        uint256 accPvlPerFactorShare; // accumulated pvl per factor share
        // Note : beware storage collision with old MasterValve
        uint256 adjustedAllocPoint; // Adjusted allocation points for this pool. PVLs to distribute per second.
    }

    // The strongest valve out there (pvl token).
    IERC20 public pvl;
    // Venom does not seem to hurt the Valve, it only makes it stronger.
    IVePvl public vePvl;
    // New Master Valve address for future migrations
    IMasterValve public newMasterValve;
    // PVL tokens created per second.
    uint256 public pvlPerSec;
    ...
    // The timestamp when PVL mining starts.
    ...
    // Amount of claimable pvl the user has
    mapping(uint256 => mapping(address => uint256)) public claimablePvl;
    ...
    event UpdatePool(uint256 indexed pid, uint256 lastRewardTimestamp, uint256 lpSupply, uint256 accPvlPerShare);
    event Harvest(address indexed user, uint256 indexed pid, uint256 amount);
    event EmergencyWithdraw(address indexed user, uint256 indexed pid, uint256 amount);
    event UpdateEmissionRate(address indexed user, uint256 pvlPerSec);
    event UpdateEmissionRepartition(
        address indexed user,
        uint256 dialutingRepartition,
        uint256 nonDialutingRepartition
    );
    event UpdateVePVL(address indexed user, address oldVePVL, address newVePVL);

    /// @dev Modifier ensuring that certain function can only be called by VePvl
    modifier onlyVePvl() {
        require(address(vePvl) == msg.sender, 'notVePvl: wut?');
        _;
    }
    ...
    function initialize(
        ...
        require(address(_pvl) != address(0), 'pvl address cannot be zero');
        require(address(_vePvl) != address(0), 'vePvl address cannot be zero');
        require(_pvlPerSec != 0, 'pvl per sec cannot be zero');
    ...
    function setNewMasterValve(IMasterValve _newMasterValve) external onlyOwner {
        newMasterValve = _newMasterValve;
    }
    ...
    /// @notice Update the given pool's PVL allocation point. Can only be called by the owner.
    ...
    /// @notice View function to see pending PVLs on frontend.
    /// @param _pid the pool id
        ...
        )
    {
        ...
        uint256 accPvlPerShare = pool.accPvlPerShare;
        uint256 accPvlPerFactorShare = pool.accPvlPerFactorShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.timestamp > pool.lastRewardTimestamp && lpSupply != 0) {
            uint256 secondsElapsed = block.timestamp - pool.lastRewardTimestamp;
            uint256 pvlReward = (secondsElapsed * pvlPerSec * pool.adjustedAllocPoint) / totalAdjustedAllocPoint;
            accPvlPerShare += (pvlReward * 1e12 * dialutingRepartition) / (lpSupply * 1000);
            if (pool.sumOfFactors != 0) {
                accPvlPerFactorShare += (pvlReward * 1e12 * nonDialutingRepartition) / (pool.sumOfFactors * 1000);
            }
        }
        pendingPvl =
            ((user.amount * accPvlPerShare + user.factor * accPvlPerFactorShare) / 1e12) +
            claimablePvl[_pid][_user] -
            user.rewardDebt;
    ...
    function _updatePool(uint256 _pid) private {
        ...
        if (block.timestamp > pool.lastRewardTimestamp) {
            ...
            // calculate pvl reward
            uint256 pvlReward = (secondsElapsed * pvlPerSec * pool.adjustedAllocPoint) / totalAdjustedAllocPoint;
            // update accPvlPerShare to reflect dialuting rewards
            pool.accPvlPerShare += (pvlReward * 1e12 * dialutingRepartition) / (lpSupply * 1000);

            // update accPvlPerFactorShare to reflect non-dialuting rewards
            if (pool.sumOfFactors == 0) {
                pool.accPvlPerFactorShare = 0;
            } else {
                pool.accPvlPerFactorShare += (pvlReward * 1e12 * nonDialutingRepartition) / (pool.sumOfFactors * 1000);
            }
            ...
            pool.lastRewardTimestamp = block.timestamp;
            emit UpdatePool(_pid, pool.lastRewardTimestamp, lpSupply, pool.accPvlPerShare);
        }
    }

    /// @notice Helper function to migrate fund from multiple pools to the new MasterValve.
    /// @notice user must initiate transaction from masterchef
    /// @dev Assume the orginal MasterValve has stopped emisions
    /// hence we can skip updatePool() to save gas cost
    function migrate(uint256[] calldata _pids) external override nonReentrant {
        require(address(newMasterValve) != (address(0)), 'to where?');
        ...
        for (uint256 i; i < _pids.length; ++i) {
            ...
            if (user.amount > 0) {
                ...
                pool.lpToken.approve(address(newMasterValve), user.amount);
                newMasterValve.depositFor(pid, user.amount, msg.sender);
    ...
    /// @notice Deposit LP tokens to MasterChef for PVL allocation on behalf of user
    ...
    function depositFor(
        ...
        if (user.amount > 0) {
            // Harvest PVL
            uint256 pending = ((user.amount * pool.accPvlPerShare + user.factor * pool.accPvlPerFactorShare) / 1e12) +
                claimablePvl[_pid][_user] -
                user.rewardDebt;
            claimablePvl[_pid][_user] = 0;

            pending = safePvlTransfer(payable(_user), pending);
        ...
        user.factor = Math.sqrt(user.amount * vePvl.balanceOf(_user));
        pool.sumOfFactors = pool.sumOfFactors + user.factor - oldFactor;

        // update reward debt
        user.rewardDebt = (user.amount * pool.accPvlPerShare + user.factor * pool.accPvlPerFactorShare) / 1e12;

        IRewarder rewarder = poolInfo[_pid].rewarder;
        if (address(rewarder) != address(0)) {
            rewarder.onPvlReward(_user, user.amount);
        }
    ...
    /// @notice Deposit LP tokens to MasterChef for PVL allocation.
        ...
        returns (uint256, uint256)
    {
        ...
        if (user.amount > 0) {
            ...
            pending =
                ((user.amount * pool.accPvlPerShare + user.factor * pool.accPvlPerFactorShare) / 1e12) +
                claimablePvl[_pid][msg.sender] -
                user.rewardDebt;
            claimablePvl[_pid][msg.sender] = 0;

            pending = safePvlTransfer(payable(msg.sender), pending);
        ...
        // update non-dialuting factor
        uint256 oldFactor = user.factor;
        user.factor = Math.sqrt(user.amount * vePvl.balanceOf(msg.sender));
        pool.sumOfFactors = pool.sumOfFactors + user.factor - oldFactor;
        ...
        if (address(rewarder) != address(0)) {
            additionalRewards = rewarder.onPvlReward(msg.sender, user.amount);
        }
        ...
        )
    {
        ...
        for (uint256 i; i < _pids.length; ++i) {
            ...
            if (user.amount > 0) {
                ...
                uint256 poolRewards = ((user.amount * pool.accPvlPerShare + user.factor * pool.accPvlPerFactorShare) /
                    1e12) +
                    claimablePvl[_pids[i]][msg.sender] -
                    user.rewardDebt;

                claimablePvl[_pids[i]][msg.sender] = 0;
                ...
                if (address(rewarder) != address(0)) {
                    additionalRewards[i] = rewarder.onPvlReward(msg.sender, user.amount);
                }
            }
        }
        // transfer all remaining rewards
        uint256 transfered = safePvlTransfer(payable(msg.sender), pending);
    ...
    /// @notice Withdraw LP tokens from MasterValve.
    ...
    /// @notice Safe pvl transfer function, just in case if rounding error causes pool to not have enough PVLs.
    /// @param _to beneficiary
    /// @param _amount the amount to transfer
    function safePvlTransfer(address payable _to, uint256 _amount) private returns (uint256) {
        uint256 pvlBal = pvl.balanceOf(address(this));

        // perform additional check in case there are no more pvl tokens to distribute.
        // emergency withdraw would be necessary
        require(pvlBal > 0, 'No tokens to distribute');

        if (_amount > pvlBal) {
            pvl.transfer(_to, pvlBal);
            return pvlBal;
        } else {
            pvl.transfer(_to, _amount);
            return _amount;
        }
    }

    /// @notice updates emission rate
    /// @param _pvlPerSec pvl amount to be updated
    /// @dev Pancake has to add hidden dummy pools inorder to alter the emission,
    /// @dev here we make it simple and transparent to all.
    function updateEmissionRate(uint256 _pvlPerSec) external onlyOwner {
        massUpdatePools();
        pvlPerSec = _pvlPerSec;
        emit UpdateEmissionRate(msg.sender, _pvlPerSec);
    }
    ...
    /// @notice updates vePvl address
    /// @param _newVePvl the new VePvl address
    function setVePvl(IVePvl _newVePvl) external onlyOwner {
        require(address(_newVePvl) != address(0));
        massUpdatePools();
        IVePvl oldVePvl = vePvl;
        vePvl = _newVePvl;
        emit UpdateVePVL(msg.sender, address(oldVePvl), address(_newVePvl));
    }

    /// @notice updates factor after any vePvl token operation (minting/burning)
    /// @param _user the user to update
    /// @param _newVePvlBalance the amount of vePVL
    /// @dev can only be called by vePvl
    function updateFactor(address _user, uint256 _newVePvlBalance) external override onlyVePvl {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            ...
            uint256 pending = ((user.amount * pool.accPvlPerShare + user.factor * pool.accPvlPerFactorShare) / 1e12) -
                user.rewardDebt;
            // increase claimablePvl
            claimablePvl[pid][_user] += pending;
            // get oldFactor
            uint256 oldFactor = user.factor; // get old factor
            // calculate newFactor using
            uint256 newFactor = Math.sqrt(_newVePvlBalance * user.amount);
            // update user factor
            user.factor = newFactor;
            // update reward debt, take into account newFactor
            user.rewardDebt = (user.amount * pool.accPvlPerShare + newFactor * pool.accPvlPerFactorShare) / 1e12;
            // also, update sumOfFactors
            pool.sumOfFactors = pool.sumOfFactors + newFactor - oldFactor;
        }
    }

    /// @notice In case we need to manually migrate PVL funds from MasterChef
    /// Sends all remaining pvl from the contract to the owner
    function emergencyPvlWithdraw() external onlyOwner {
        pvl.safeTransfer(address(msg.sender), pvl.balanceOf(address(this)));
    }
}
...
/**
 * @dev Interface of the MasterValve
 */
interface IMasterValve {
    ...
    function updateFactor(address _user, uint256 _newVePvlBalance) external;
}
...
interface IRewarder {
    function onPvlReward(address user, uint256 newLpAmount) external returns (uint256);
...
/**
 * @dev Interface of the VePvl
 */
interface IVePvl is IVeERC20 {
    ...
    // function vePvlBurnedOnWithdraw(address _addr, uint256 _amount) external view returns (uint256);
    ...
    function getStakedPvl(address _addr) external view returns (uint256);

    // function levelUp(uint256[] memory valveBurned) external;
