...
contract OctoHeadzPOAP is ERC1155, Ownable, Whitelistable {
    POAP_EVENT[] public events;
    ...
    //ERC1155 constructor should have URI
    constructor(address _whitelistAdmin) 
        ERC1155("DEMO")
        Whitelistable(_whitelistAdmin) {
            events.push(POAP_EVENT("Octohedz", "20210820", "The Original Octohedz Drop"));
            publicMinting = false;
            wlMinting = true;
            nonce = 0;
    }
    ...
    function addTokenURI(string memory _tokenURI) public onlyOwner {
...
///////////////////////////////////////////////// Events /////////////////////////////////////////////////
    ...
    function generateHash(string memory _name, string memory _date, string memory _description) public pure returns (bytes32 hash) {
        return keccak256(abi.encodePacked(_name, _date, _description));
    }

    function addEvent(string memory _name, string memory _date, string memory _description, string memory _tokenURI) public onlyOwner {
        require(events.length == tokenURIs.length, "Token URI length and Event length don't match");
        bytes32 index = generateHash(_name, _date, _description);
        require(!eventExists[index], "Event already exists");
        eventExists[index] = true;
        events.push(POAP_EVENT(_name, _date, _description));
        addTokenURI(_tokenURI);
    }

    function updateExistingEvent(uint256 _eventID, string memory _name, string memory _date, string memory _description) public onlyOwner {
        bytes32 index = generateHash(_name, _date, _description);
        require(eventExists[index], "eventID doesn't exist");
        events[_eventID] = POAP_EVENT(_name, _date, _description);
    }
...
///////////////////////////////////////////////// Minting /////////////////////////////////////////////////
    function mintPOAP(uint _eventID, uint256 _numNFTz) public {
        require(publicMinting, "POAP isn't minting");
        _mint(msg.sender, _eventID, _numNFTz, "");
    }

    function mintSignedPOAP(uint _eventID, uint256 _numNFTz, uint256 _nonce, bytes memory _proof) public isWhitelisted(keccak256(
            abi.encodePacked(
                msg.sender,
                _numNFTz,
                _nonce
            ...
            require(wlMinting, "POAP isn't minting");
            nonce++;
            _mint(msg.sender, _eventID, _numNFTz, "");
    }
