...
contract FIZZYFY_V2 
{
    ...
    uint256 private constant directPercents = 5000;
    ...
    constructor() public {
        ...
        feeReceivers = 0x6e2B9e4190F5978f5fFCe27797b5E33fAcAC1880;
	...
	function _deposit(address _user, uint256 _amount) private {
        ...
        if(user.maxDeposit != 0)
        {
            _updateDirectBusiness(msg.sender, _amount);
        }
    ...
    function _updateDirectBusiness(address _user, uint256 _amount) private {
        UserInfo storage user = userInfo[_user];
        ...
        for(uint256 i = 0; i < 1; i++)
		{
            if(upline != address(0))
            {
                ...
                {
                    userInfo[upline].directBusiness = userInfo[upline].directBusiness.add(_amount.sub(user.maxDeposit));
                }
            }
            else
			{
            ...
            if(upline != address(0))
			{
                if(userInfo[upline].maxDeposit == userInfo[upline].totalDeposit)
                {
                    if(userInfo[upline].directnum >= 1 && userInfo[upline].directnum < 5 )
                    {
                        ...
                        if(userInfo[upline].directnum == 1)
                        {
	...
	function _updateReferInfo(address _user, uint256 _amount) private {
        ...
        for(uint256 i = 0; i < referDepth; i++)
		{
            if(upline != address(0))
			{
                ...
                if(_amount > user.maxDeposit && user.maxDeposit != 0)
                {
                    userInfo[upline].teamTotalDeposit = userInfo[upline].teamTotalDeposit.add(_amount.sub(user.maxDeposit));
                }
                else
                {
