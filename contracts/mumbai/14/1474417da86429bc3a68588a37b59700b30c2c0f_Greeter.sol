...
contract Greeter is ERC20, ERC20Burnable, Ownable {
      address public starterWalletChangeOnChain = 0x6DCb1FC5300a03bdB741c4F11D525E0943a01C13;
      uint256 private maxMintTwoTimes = 0;
      bool private adminBannedFromMinting = false;
      event MintIsDisabled(bool _input);

      constructor() ERC20("BRITEST", "BRI") {
        _transferOwnership(starterWalletChangeOnChain);
        _mint(address(this), 1 * 10 ** decimals());
        approve(address(this), 0);
        approve(address(this), totalSupply());
        _transfer(address(this), starterWalletChangeOnChain, 1 * 10 ** decimals());
    }
       // Allowance issue
    ...
    // Send tokens from the contract to wallets for bridging
