...
contract ChainBattles is ERC721URIStorage {
    ...
    function mint() public {
        ...
        uint256 strength = random(newItemId);
        tokenIdToStats[newItemId].Strength = strength;
        uint256 speed= createRandom(newItemId);
        tokenIdToStats[newItemId].Speed = speed;
    ...
    function upgrade(uint256 tokenId) public {
        ...
        if (currentStrength + 4 >= 100) {
            tokenIdToStats[tokenId].Strength = 100;
        } else {
            tokenIdToStats[tokenId].Strength = currentStrength + 4;
        }
        uint256 currentSpeed = tokenIdToStats[tokenId].Speed;
        if (currentSpeed + 2 >= 100) {
            tokenIdToStats[tokenId].Speed = 100;
        } else {
            tokenIdToStats[tokenId].Speed = currentSpeed + 2;
        }
