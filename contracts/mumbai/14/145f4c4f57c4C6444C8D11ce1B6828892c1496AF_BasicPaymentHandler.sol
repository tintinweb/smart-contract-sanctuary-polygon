...
contract BasicPaymentHandler is IPaymentHandler, IStorage {
    ...
    function checkForPayment(
        string memory unitId /*, address _bconContract*/
    ) public override {
        ...
        console.log(
            '[PaymentHandler] completion rates (total | claimed)',
            unit.completionRateTotal,
            unit.completionRateClaimed
        );
        ...
        if (unit.completionRateTotal == unit.completionRateClaimed) {
            ...
            uint256 _securityDeposit = mulScale(unit.paymentClaimed, config.securityDeposit);

            // TODO store cumulative security deposit as separate variable in unit?
            //unit.paymentTotal -= _securityDeposit;

            // throw event in BconContract
            IBconContract(_bconContract).throwBillingUnitPaymentTriggered(
                IBconContract.BillingUnitEventPayload({
                    billingUnit: unit,
                    timestamp: block.timestamp,
                    sender: md.client,
                    reportType: SharedStates.MessageType.PaymentConfirmation
                }),
                (block.timestamp + config.paymentInterval),
                (unit.paymentClaimed - _securityDeposit),
                _securityDeposit
            );
            ...
            if (config.partialPayment == true) {
                uint256 currentStageOfCompletion = (unit.completionRateClaimed * 100) /
                    unit.completionRateTotal;
                uint256 minStageOfCompletion = config.minStageOfCompletion;

                console.log(
                    '[PaymentHandler] stage of completion',
                    minStageOfCompletion,
                    currentStageOfCompletion
                );
                if (currentStageOfCompletion >= minStageOfCompletion) {
                    ...
                    console.log(
                        '[PaymentHandler] pre deposit',
                        unit.paymentClaimed,
                        config.securityDeposit
                    );
                    uint256 _securityDeposit = 0;
                    if (config.securityDeposit > 0) {
                        _securityDeposit = mulScale(unit.paymentClaimed, config.securityDeposit);
                    ...
                    console.log('[PaymentHandler] not enough reported to get a payment');
                }
            } else {
                console.log(
                    '[PaymentHandler] partial payment is not allowed; not enough work completed'
                );
            }
        }
    }
