...
contract stakeELF is Pausable {
    ...
    uint constant midTermThreshould=30 minutes;//180 days;
    uint constant shortTermThreshould=15 minutes;//90 days;
    ...
    event Withdrawing(address indexed staker, uint amount, uint t);
    ...
    function viewOps() external view returns (address[] memory) {
        return ops;
    }
    ...
    function viewPrices(uint i) external view returns(uint[2][9] memory) {
        return prices[i];
    }

    function userStakeEggs(address addr) external view returns(uint){
    ...
    function stake(uint[] calldata ids) external needPricesSet{
        ...
        for (uint i=0;i<ids.length;i++){
            ...
            if (altar[msg.sender] <= 10) altar[msg.sender] = 0;
            else altar[msg.sender] -= 10;
    ...
    function withdraw() external needPricesSet{
        ...
        for (uint i=0; i<_eggs.length; i++) { //iterate through all eggs.
            ...
            if (!withdrawn[msg.sender][i]){
                ...
                if (totalTime>=unit) {
                    ...
                    if (maxStakingPeriod <= totalTime) amount=amount+_eggs[i].price*11;
                    else if (midTermThreshould <= totalTime) amount=amount+_eggs[i].price*4;
                    else if (shortTermThreshould <= totalTime) amount=amount+_eggs[i].price;
                }
            }
            ...
            (uint eggAmount, uint eggIndex)=stakingReward(totalTime, _eggs[i], i);
            amount+=eggAmount;
        ...
        if (amount > 0) {
            ...
            altarTemp = altarTemp + ((block.timestamp % 7) + 1);
            if (altarTemp > 90) altarTemp = 90;
            altar[msg.sender] = altarTemp;
            ROE.transfer(msg.sender, amount);
            emit Withdrawing(msg.sender, amount, t);
        }
    }

    function stakingReward(uint totalTime, EGG memory egg, uint i) internal returns(uint amount, uint indexOfPrices) {
        ...
        while (totalTime-accumulatedTime>=unit){ //iterate through terms of prices
            uint unitDiff;
            uint tempLastWithdrawTime=lastWithdrawTime[msg.sender][i];
            ...
            if (tempLastWithdrawTime==0) lastWithdrawTime[msg.sender][i]=rewardStartingPoint+unitDiff*unit;
            else lastWithdrawTime[msg.sender][i]=tempLastWithdrawTime+unitDiff*unit;
        }
    }
}