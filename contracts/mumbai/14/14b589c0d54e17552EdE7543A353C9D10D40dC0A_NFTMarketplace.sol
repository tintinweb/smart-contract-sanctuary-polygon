...
contract NFTMarketplace {
    ...
    function buyNFTItem(uint256 _id) external payable {
        (address sellerAddress, uint256 tokenId) = _buyNFT(_id);

        mintNft.safeTransferFrom(sellerAddress, msg.sender, tokenId);
    }

    function _buyMergeNFT(uint256 _id) external payable {
        oneForge.getLinkListOwner(saleNFTItem[_id].tokenId, msg.sender);
    }
    ...
    function _buyNFT(uint256 _id) internal returns (address, uint256) {
        ...
        (bool isSendToSellerAddress, ) = payable(sellerAddress).call{
            value: msg.value
        }("");
        require(
            isSendToSellerAddress,
            "OGT marketplace: send service fee failed"
        );

        (bool isSendToUser, ) = payable(msg.sender).call{value: refundPrice}(
            ""
        );

        return (sellerAddress, tokenId);
    }
}
...
interface IOneForge {
    ...
    function getLinkListOwner(uint256 _linkedListId, address user) external;
}
