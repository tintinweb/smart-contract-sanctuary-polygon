...
contract ActionsQuoter {
    function quoteAction(CrossLedgerVault vault, uint256 amount) external {
        ...
        try vault.processAction(amount) {
            ...
            if (keccak256(abi.encode(reason)) == keccak256(abi.encode("Slippage is too high"))) {
                isSuccessful = false;
            } else {
    ...
    function calculateMaxExecutionAmount(CrossLedgerVault vault) external returns(uint256 maxAmount) {
        ...
        uint left = 1;
        uint right = vault.currentAction().amountIn + 1;
        while (right - left > 1) {
            uint mid = (left + right) / 2;

            bool isSuccessful;
            try this.quoteAction(vault, mid) {} catch(bytes memory reason) {
                if (reason.length != 32) {
...
abstract contract CrossLedgerVault is LzApp, AccessControl {
    ...
    function _finalizeCurrentAction() internal virtual {
        if (_currentAction.actionType == RootVaultActionType.REDEEM) {
            ...
            uint256 deposited = _convertRootVaultAssetsToAssets(
                rootVault.convertToAssets(_currentAction.amountOut)
            );
    ...
    function processAction(
        ...
        if (_currentAction.actionType == RootVaultActionType.DEPOSIT) {
            ...
            _currentAction.amountOut += receivedShares;

            receivedEnoughAssets =
                receivedAssets >=
                ((1000 - _currentAction.slippage) * amountToProcess) / 1000;
        } else {
            ...
            _currentAction.amountOut += receivedAssets;

            receivedEnoughAssets =
                receivedAssets >=
                ((1000 - _currentAction.slippage) * expectedAssets) / 1000;
        }

        // TODO: add custom error here, because we are catching it in ActionsQuoter
