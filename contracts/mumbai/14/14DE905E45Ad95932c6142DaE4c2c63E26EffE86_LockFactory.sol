...
contract LockFactory is AccessControlEnumerable, DeployFee {
    ...
    function setMintrFactory(address _mintrContract) external onlyRole(FACTORY_MANAGER) {
    ...
    function prepareTokens(
        ...
        require(IERC20(token).balanceOf(address(this)) == amount, "ERROR: Can't Lock Tokens With TX Fee");
        if(mintrContract != address(0)) {
            (bool success, bytes memory data) = mintrContract.call{gas: 10000}(abi.encodeWithSignature("createdByMinted(address)", token));
    ...
    function manageRoles(address lockProxy, LockModules memory modules) internal {
...
abstract contract DeployFee is Context {
    ...
    function updateDeployFeeBeneficiaryInternal(address _newDeployFeeBeneficiary) internal {
        require(_newDeployFeeBeneficiary != address(0), "ERROR: Can't set address 0");
    ...
    function updateCryptoFeedInternal(address _newCryptoPriceFeed) internal {
        require(_newCryptoPriceFeed != address(0), "ERROR: Can't set Address 0 as feed");
