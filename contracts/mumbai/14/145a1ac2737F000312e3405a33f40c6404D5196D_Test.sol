...
contract Test{
    ...
    constructor() public {
        ...
        levelPrice[3]  = 200*1e18;
        levelPrice[4]  = 400*1e18;
        levelPrice[5]  = 1600*1e18;
        levelPrice[6]  = 3200*1e18;
        levelPrice[7]  = 6400*1e18;
        levelPrice[8]  = 6400*1e18;
        levelPrice[9]  = 12800*1e18;
    ...
    function updateX6Referrer(address userAddress, address referrerAddress, uint8 level) private{
        ...
        {          
          users[referrerAddress].holdAmount[level]=users[referrerAddress].holdAmount[level]+mlevelPrice[level];
       ...
       //         address(uint160(owner)).transferToken((mlevelPrice[level]*5)/100,tokenId);
            ...
            if(users[referrerAddress].x6Matrix[level].referrals.length < level+3) 
            {
                users[referrerAddress].x6Matrix[level].referrals.push(userAddress);
                emit NewUserPlace(userAddress, referrerAddress, 2, level, uint8(users[referrerAddress].x6Matrix[level].referrals.length));
                return address(uint160(referrerAddress)).transfer((mlevelPrice[level]/maticRate)*1e18);   
            }
            users[referrerAddress].x6Matrix[level].referrals.push(userAddress);
            emit NewUserPlace(userAddress, referrerAddress, 2, level, uint8(users[referrerAddress].x6Matrix[level].referrals.length));           
            address(uint160(referrerAddress)).transfer((mlevelPrice[level]/maticRate)*1e18);
    ...
    function autoUpgrade(address _user, uint8 level) private{
            if((users[_user].holdAmount[level-1]-mlevelPrice[level])>0)
            {
             address(uint160(_user)).transfer(users[_user].holdAmount[level-1]-mlevelPrice[level]);
            }
    ...
    function withdrawLevelIncome() public{
        ...
        for(i=0;i<9;i++)
        {
           ...
           emit CommunityBonus(idToAddress2[payUpId], msg.sender, rest/100, i,1);
        }
        // for downline
        for(i=0;i<9;i++)
        {
           ...
           emit CommunityBonus(idToAddress2[payDownId], msg.sender, rest/100, i,2);
        }
