...
contract VRFv2SubscriptionManager is VRFConsumerBaseV2 {
  ...
  function requestRandomWords(uint32 _numWords, bytes32 _keyHash, uint32 _callbackGasLimit, uint16 _requestConfirmations ) external{
  ...
  function generateRandomness() public view returns(uint256[] memory) {

    uint[] memory random = new uint[](500) ;
    for(uint i = 0; i < 490; i += 1){
      random[i] = uint(keccak256(abi.encodePacked(s_randomWords[0],msg.sender,i))) % 500;
    }
