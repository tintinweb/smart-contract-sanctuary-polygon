...
contract ClaimNFT is Initializable, ERC2771ContextUpgradeable, UUPSUpgradeable {
    ...
    event SessionCancel(string sessionID);
    ...
    mapping(string => RegisterRootERC1155) public registrationRootRecord;
    ...
    function registerRootERC1155(string memory _sessionId, address _contractAddress, uint256 _tokenId, address _owner, bytes32 _merkleRoot) external {
        require(!isCancelled[_sessionId], "Session is cancelled!");
        RegisterRootERC1155 memory data = RegisterRootERC1155({
            contractAddress: _contractAddress,
            owner: _owner,
            tokenId: _tokenId,
        ...
        registrationRootRecord[_sessionId] = data;
    }

    function registerListERC1155(string memory _sessionId, address _contractAddress, uint256 _tokenId, address _owner) external {
        require(!isCancelled[_sessionId], "Session is cancelled!");
    ...
    function claimRootERC1155(string memory _sessionId, bytes32[] calldata _merkleProof) public {
        require(!isCancelled[_sessionId], "Session is cancelled!");
        RegisterRootERC1155 memory data = registrationRootRecord[_sessionId];
        require(!isClaimed[_sessionId][_msgSender()], "Already claimed!");
        bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
        require(MerkleProof.verify(_merkleProof, data.merkleRoot, leaf), "Invalid Proof");
        isClaimed[_sessionId][_msgSender()] = true;
        IERC1155 contract1155 = IERC1155(data.contractAddress);
        contract1155.safeTransferFrom(data.owner, _msgSender(), data.tokenId, 1, "0x");
    }
    ...
    function cancel(string memory _sessioinId) public onlyOperator {
        require(registrationRecordERC721[_sessioinId].contractAddress != address(0) || registrationRecordERC1155[_sessioinId].contractAddress != address(0), "Session not exist!");
        require(!isCancelled[_sessioinId], "Session is already canceled!");
        isCancelled[_sessioinId] = true;
        emit SessionCancel(_sessioinId);
    }
