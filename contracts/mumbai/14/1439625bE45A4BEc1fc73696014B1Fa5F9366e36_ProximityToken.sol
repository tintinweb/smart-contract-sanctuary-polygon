    ...
    AccessControlUpgradeable
{
    ...
    // Using uint16 to represent minutes as the smallest unit of size.
    uint16 private _latStep;
    uint16 private _lngStep;
    uint16 private _maxLat;
    uint16 private _maxLng;
    uint8 internal maxSizeOfCoordinate = 16; // uint
    uint16 public multiplicator = uint16(100);

    mapping(address => uint256) public userCoordinates;
    ...
    function decodeLocatioId(uint256 locationId)
        external
        view
    ...
    function _move(
        ...
        require(userCoordinates[account] == 0, "ACCOUNT_ALREADY_HAS_LOCATION");
        require(msg.sender == account, "MOVING_ACCOUNT_NOT_SENDER");
    ...
    {
        return Coordinate(uint16(lat / 100), uint16(lng / 100));
    }

    // Locatins ordered: Bottom-Left, Top-Left, Top-Right, Bottom-Right
    ...
    function getTravelPrice(uint256 a, uint256 b)
        internal
        pure
        returns (uint256)
    {
        uint256 distance = PRBMathUD60x18.sqrt(
            PRBMathUD60x18.pow(a, 2e18) + PRBMathUD60x18.pow(b, 2e18)
        );
        return distance;
    }
    ...
    function safeTransferFrom(
        ...
        revert("LOCATION_BOUND_TO_ACCOUNT");
    }
