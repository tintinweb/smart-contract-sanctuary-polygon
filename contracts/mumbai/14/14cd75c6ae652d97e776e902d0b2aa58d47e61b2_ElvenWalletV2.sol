...
contract ElvenWalletV2 {
    ...
    event GameLootTransfer(address indexed from, uint256 timestamp, uint256 amount, uint256 lootId, bool subtract, bytes indexed authCode);   
   ...
   function initialize() public {
    ...
    function transferGameAssets(address _owner, uint256 _tokenAmounts, uint256 tokenIndex, bytes memory authCode, bool transferToWallet) external {
        onlyOperator();
        checkWalletStatus();
        ...
        if (transferToWallet) {
            ...
            lootContract.burn(_owner, _tokenAmounts, tokenIndex);
        }

        emit GameLootTransfer(_owner, block.timestamp,_tokenAmounts, tokenIndex, transferToWallet, authCode);  
