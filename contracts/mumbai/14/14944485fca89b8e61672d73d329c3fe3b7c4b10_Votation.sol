...
contract Votation {
     ...
     * @param timestamp Start time of the votation
    ...
    /**
     * @notice Requires the msg.sender to be the superadmin
     */
    modifier onlySuperAdmin {
        require(rolesContract.isSuperAdmin(msg.sender), 'Voting:Only superadmin');
        _;
    }
    ...
    function createProposal(bytes memory action_, uint8 quorum_, address to_) public {
        ...
        proposals[CURRENT_ID] = Proposal(0, 1, 0, 0, uint144(block.timestamp), to_, action_) :
        proposals[CURRENT_ID] = Proposal(0, quorum_, 0, 0, uint144(block.timestamp), to_, action_);
    ...
    /**
     * @notice Returns a list of proposals
     * @return proposalList A list of all proposals
     */
    function getProposals() public view returns (Proposal[] memory proposalList) {
        proposalList = new Proposal[](CURRENT_ID);
        for (uint i = 0; i < CURRENT_ID; i++) {
