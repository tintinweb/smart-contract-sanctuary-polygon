...
// Death Count & Death
  ...
  function feed() public {
    require(hunger() < defHunger, "Cannot eat, already full");
    addToXP(xpToAdd());
    defHunger += defStatToAddValue;

    // Reset values
    if(hunger() == maxStatValue){
        defHunger = maxStatValue;
        foodDepletionTimerStart = block.timestamp;
    }
  }

  // Play action
  // _score is in percent of maximum achievable score in any game
  function play(uint256 _scorePercentage) public {
       require(fun() < maxStatValue, "Cannot add more xp, already maxed for now");
           addToXpFun(_scorePercentage);
        // reset values
        defFun = maxStatValue;
        funDepletionTimerStart = block.timestamp;
  }
  ...
  // Learn action
  function teach() public {
    require(intellect() < defIntellect, "Cannot learn more, too much studying");
    require(isInBareCondition(), "Cannot learn, not feeling good");
    addToXP(xpToAdd());
    defIntellect += defStatToAddValue;

    // minStatValue <-- OBSOLETE if intellectToSub doesn't return more than maxStatValue (was used to prevent negative values)
    if(intellect() >= maxStatValue){
        // reset values
        defIntellect = maxStatValue;
        intellectDepletionTimerStart = block.timestamp;
    }
  ...
  function wash() public {
    require(cleanliness() < maxStatValue, "Cannot wash, already clean");
    addToXP(xpToAdd());
    cleanlinessDepletionTimerStart = block.timestamp; //max it out
  }
  ...
  function heal() public {
      require(health() < maxStatValue, "Cannot heal, already at full health");
      addToXP(xpToAdd());
      defHealth += 5;
      // reset stats?
      if(health() >= maxStatValue){// || health() == minStatValue){ -- No idea why this was here
          defHealth = maxStatValue;
          _healthToSubRandom = getHealthToSubRandomValue(); //randomize value for next sickness
          randomizedSickTimerEnd = block.timestamp + getRandomInterval() * 2;
      }
...
// Free mint users
  ...
  function feedPet(uint petID) public ownerOfTokenId(petID) returns (bool){
    petAtID[petID].feed();
    return true;
  }

  function playPet(uint petID, uint8 _scorePercent) public ownerOfTokenId(petID) returns (bool){
    uint8 scorePercent = _scorePercent;
    if(scorePercent > 100){
        scorePercent = 100;
    }
    petAtID[petID].play(scorePercent);
    return true;
  }

  function sleepPet(uint petID) public ownerOfTokenId(petID) returns (bool){
    petAtID[petID].sleep();
    return true;
  }

  function teachPet(uint petID) public ownerOfTokenId(petID) returns (bool){
    petAtID[petID].teach();
    return true;
  }

  function healPet(uint petID) public ownerOfTokenId(petID) returns (bool){
    petAtID[petID].heal();
    return true;
  }

  function washPet(uint petID) public ownerOfTokenId(petID) returns (bool){
    petAtID[petID].wash();
    return true;
  }
