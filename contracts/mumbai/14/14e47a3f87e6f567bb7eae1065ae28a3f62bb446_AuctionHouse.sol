...
contract AuctionHouse is ERC1155Receiver, ERC2771Context, Ownable {
  ...
  event AuctionCreated(uint256 indexed auctionId, address indexed seller, uint256 indexed tokenId, uint256 tokenAmount, uint256 endDate);
  ...
  /// @param reservePrice Optional minimal sale price set by the seller
  ...
  function createAuction(
    ...
    emit AuctionCreated(auctionId, sender, tokenId, tokenAmount, endDate);
  ...
  function bidStable(uint256 auctionId, uint256 stableAmount) external {
    ...
    // get funds from bidder
    stableCurrency.transferFrom(sender, address(this), stableAmount);
  ...
  function emergencyStableExcessHandout(address to) external onlyOwner {
    ...
    stableCurrency.transfer(to, stableCurrency.balanceOf(address(this)) - _activeBiddingStableTotal);
  }
  ...
  function _transferCurrency(address to, CurrencyType currencyType, uint256 amount) internal {
    if (to != address(0)) {
      if (currencyType == CurrencyType.Main) {
        ...
        stableCurrency.transfer(to, amount);
      }
    }
  }
