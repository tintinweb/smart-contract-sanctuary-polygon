...
interface IMessageData {
    ...
    function removeMsg(uint256 msgID, address requester, uint256 isCommentOf) external;

    // Remove a post from a user mapping
    function saveMsg(MsgData memory msgData) external;

    // Add Stats to a message
    function addStat(uint8 statType, uint256 msgID, int amount) external;
...
contract Likes {
    ...
    mapping (string => address[]) public likesMap;
    mapping (string => mapping (address => bool)) public likesMapMap;
    ...
    */
    ...
    event logAddLike(uint256 indexed msgID, address indexed requester, uint256 indexed isCommentOf);
    event logRemoveLike(uint256 indexed msgID, address indexed requester, uint256 indexed isCommentOf);
    ...
    */
    ...
    function updateContracts(address _kutils, address _messageData, address _comments) public onlyAdmins {
    ...
    /**
    * Returns a list of messages that are tagged to a given user
    * @param msgID : the message ID to get liked for
    * @param startFrom : the number to start getting records from
    */
    function getLikesFromMsgID(uint256 msgID, uint256 startFrom) public view returns(address[] memory) {
        ...
        if (startFrom == 0){
            startFrom = ((likesMap[KUtils.append(msgIDStr,'-',KUtils.toString(bucketKeyID),'','')].length) + (bucketKeyID * maxItemsPerBucket));
            if (startFrom != 0){
                startFrom -= 1;
            } else {
                // It's empty, so end
    ...
    function removeLike(uint256 msgID, address likedBy, uint256 isCommentOf) public onlyAdmins {
        // Stringify the ID
        string memory msgIDStr = KUtils.toString(msgID);
        ...
        for (uint i=0; i <= bucketKeyID; i++) {
            ...
            likesMapMap[likeBucketKey][likedBy] = false;
        }

        // Remove the like from the message
        if (isCommentOf > 0){
            Comments.addStat(1, msgID, -1, isCommentOf);
        } else {
            messageData.addStat(1, msgID, -1);
        }

        // Emit to the logs for external reference
        emit logRemoveLike(msgID, likedBy, isCommentOf);
    ...
    // If you are liking a comment, you MUST have supply the isCommentOf as the msgID
    // of the message the comment is for. Otherwise, it will go towards the regular
    // message with that ID instead. In which case, you need to pass 0 for isCommentOf
    function addLike(uint256 msgID, address likedBy, uint256 isCommentOf) public onlyAdmins {
        // Stringify the ID
        string memory msgIDStr = KUtils.toString(msgID);
        ...
        likesMap[likeBucketKey].push(likedBy);

        // Update the Likes Map Map for quick removal
        likesMapMap[likeBucketKey][likedBy] = true;
        ...
        // Emit to the logs for external reference
        emit logAddLike(msgID, likedBy, isCommentOf);

        delete msgIDStr;
        delete likeBucketKey;
        delete bucketKeyID;
    }
