...
contract EtheremonTradeData is BasicAccessControl {
    ...
    mapping(uint256 => SellingItem) public sellingDict; // monster id => item
    uint256[] public sellingList; // monster id

    mapping(uint256 => BorrowItem) public borrowingDict;
    uint256[] public borrowingList;

    mapping(address => uint256[]) public lendingList;

    function removeSellingItem(uint256 _itemId) external onlyModerators {
    ...
    function removeBorrowingItem(uint256 _itemId) external onlyModerators {
    ...
    function addItemLendingList(address _trainer, uint256 _objId)
        external
        onlyModerators
    {
    ...
    function removeItemLendingList(address _trainer, uint256 _objId)
        external
        onlyModerators
    {
        uint256 foundIndex = 0;
        uint256[] storage objList = lendingList[_trainer];
    ...
    // read access
    function isOnBorrow(uint256 _objId) external view returns (bool) {
        return (borrowingDict[_objId].index > 0);
    }

    function isOnSell(uint256 _objId) external view returns (bool) {
        return (sellingDict[_objId].index > 0);
    }

    function isOnLent(uint256 _objId) external view returns (bool) {
        return borrowingDict[_objId].lent;
    }

    function getSellPrice(uint256 _objId) external view returns (uint256) {
        return sellingDict[_objId].price;
    }

    function isOnTrade(uint256 _objId) external view returns (bool) {
        return ((borrowingDict[_objId].index > 0) ||
            (sellingDict[_objId].index > 0));
    }

    function getBorrowBasicInfo(uint256 _objId)
        external
        view
    ...
    function getBorrowInfo(uint256 _objId)
        external
        view
        returns (
            uint256 index,
    ...
    function getSellInfo(uint256 _objId)
        external
        view
        returns (
            uint256 index,
    ...
    function getTradingInfo(uint256 _objId)
        external
        view
        returns (
...
contract EtheremonTrade is EthermonEnum, BasicAccessControl, SafeMathEthermon {
    ...
    event EventRemoveSellOrder(address indexed seller, uint256 objId);
    ...
    event EventRemoveOfferBorrowingItem(address indexed lender, uint256 objId);
        ...
        onlyModerators
    {
        ...
        weth.transfer(_sendTo, _amount);
    }

    // function _triggerNFTEvent(
    //     address _from,
    //     address _to,
    //     uint256 _objId
    // ) internal {
    ...
    // public
    function placeSellOrder(uint256 _objId, uint256 _price) external isActive {
        if (_price == 0) revert();
        ...
        emit EventPlaceSellOrder(msg.sender, _objId, _price);
    }

    function removeSellOrder(uint256 _objId) external isActive {
        // check ownership
        EtheremonDataBase data = EtheremonDataBase(dataContract);
        MonsterObjAcc memory obj;
        (
        ...
        emit EventRemoveSellOrder(msg.sender, _objId);
    }

    function buyItem(uint256 _objId) external payable isActive {
        ...
        // transfer money
        weth.transfer(obj.trainer, safeSubtract(requestPrice, fee));
    ...
    //         battleContract
    //     );
    //     if (battle.isOnBattle(obj.monsterId)) revert();
    ...
    // function removeBorrowingOfferItem(uint256 _objId) external isActive {
    //     EtheremonTradeData monTradeData = EtheremonTradeData(
    //         tradingMonDataContract
    //     );
    //     address owner;
    ...
    //     if (owner != msg.sender || lent == true) revert();

    //     monTradeData.removeBorrowingItem(_objId);
    //     EventRemoveOfferBorrowingItem(msg.sender, _objId);
    // }

    // function borrowItem(uint256 _objId) external payable isActive {
    //     EtheremonTradeData monTradeData = EtheremonTradeData(
    //         tradingMonDataContract
    //     );
    ...
    //     if (msg.sender != borrowItem.owner) revert();
        ...
        )
    {
        ...
        bp = 0;//monsterNFT.getMonsterCP(_objId);
        owner = obj.trainer;
        classId = obj.classId;
        exp = obj.exp;
        createIndex = obj.createIndex;
        //return (obj.trainer, obj.classId, obj.exp, obj.createIndex, bp);
    }
    ...
    function isOnTrading(uint256 _objId) external view returns (bool) {
        EtheremonTradeData monTradeData = EtheremonTradeData(
            tradingMonDataContract
        );
