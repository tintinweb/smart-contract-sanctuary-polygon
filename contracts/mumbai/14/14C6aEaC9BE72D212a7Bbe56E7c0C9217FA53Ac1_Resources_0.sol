...
contract Resources_0 is Initializable, OwnableUpgradeable, IResource_0 {
    using Resources for Resources.ID;
    using Items for Items.ID;
    IBaseLogic public baseLogic;
    ...
    modifier onlyGameContract() {
        ...
        if (msg.sender == baseLogic.gameController()) {
            valid = true;
        }
        require(valid, "e");
        _;
    ...
    function initialize(address _base) public initializer {
        __Ownable_init();
        baseLogic = IBaseLogic(_base);
    }
    ...
    function s4_grow(
        ...
        IInventory(baseLogic.inventory()).removeResource(
            ...
            IGameController(baseLogic.gameController()).getIndex(id, position)
        );
        uint256 rand;
        uint256 resourceId;
        // Grow to Rare or better res
        if (baseLogic.isItemExist(id, uint256(Items.ID.IF1), false)) {
            uint256 length_rare = IGameResource(baseLogic.gameResource())
                .categoryLength(Constants.RARE_FRUIT);
            uint256 length_very_rare = IGameResource(baseLogic.gameResource())
                .categoryLength(Constants.VERY_RARE_FRUIT);
            rand =
                ((random / 100 ** position) % 400) %
                (length_rare + length_very_rare);

            resourceId = rand < length_rare
                ? IGameResource(baseLogic.gameResource()).at("RARE_FRUTT", rand)
                : IGameResource(baseLogic.gameResource()).at(
                    "VERY_RARE_FRUIT",
                    rand
                );
        } else if (baseLogic.isItemExist(id, uint256(Items.ID.IC11), false)) {
            uint256 length_uncommon = IGameResource(baseLogic.gameResource())
                .categoryLength(Constants.UNCOMMON_FRUIT);
            uint256 length_rare = IGameResource(baseLogic.gameResource())
                .categoryLength(Constants.RARE_FRUIT);
            uint256 length_very_rare = IGameResource(baseLogic.gameResource())
                .categoryLength(Constants.VERY_RARE_FRUIT);
            ...
            resourceId = rand < length_uncommon
                ? IGameResource(baseLogic.gameResource()).at(
                    Constants.UNCOMMON_FRUIT,
                    rand
                )
                : rand > length_uncommon && rand < length_uncommon + length_rare
                ? resourceId = IGameResource(baseLogic.gameResource()).at(
                    Constants.RARE_FRUIT,
                    rand
                )
                : IGameResource(baseLogic.gameResource()).at(
                    Constants.VERY_RARE_FRUIT,
                    rand
                );
        } else {
            rand =
    ...
    function arrow(
        ...
        uint8 sort,
        uint256 randomNumber
    ) external override onlyGameContract returns (uint16[] memory res) {
        uint256 id = _id;
        uint16 position = _position;
        uint8 countDestroy;
        uint16[] memory destroyPos = new uint16[](20);
        uint8 time = 1;
        if (baseLogic.isItemExist(id, uint256(Items.ID.IQ3), false)) {
            time = 2;
        }
        ...
        while (time > 0) {
            ...
            while (prevDirection == direction) {
                direction = uint8((direction + (randomNumber / 1e40)) % 8);
            }
            ...
            IEventContract(baseLogic.eventContract()).emitArrow(
                id,
                position,
                direction
            );
            for (uint8 i = 0; i < listPositionPointedTo.length; ++i) {
                uint8 resourcePointed = uint8(
                    IGameController(baseLogic.gameController()).atPosition(
                        id,
                        listPositionPointedTo[i]
                    )
                );
                if (resourcePointed == uint8(Resources.ID.T1)) {
                    destroyPos[countDestroy] = listPositionPointedTo[i];
                    ++countDestroy;
                    IEventContract(baseLogic.eventContract()).emitEffect(
                        id,
                        1,
                ...
                } else if (resourcePointed != uint8(Resources.ID.E4)) {
                    int256 mulValue = int256(uint(sort)) + 1;
                    IGameController(baseLogic.gameController()).updatePayout(
                        id,
                        listPositionPointedTo[i],
                        mulValue,
                ...
                if (
                    resourcePointed == uint8(Resources.ID.A1) ||
                    resourcePointed == uint8(Resources.ID.P2)
                ) {
                    baseLogic.boostPayout(
                        id,
        ...
        return
            countDestroy > 0
                ? Helper.getArray(countDestroy, destroyPos)
                : new uint16[](0);
    }
        ...
        if (
            ...
            if (value == 3) {
                IGameController(baseLogic.gameController()).updateMap(
                    id,
                    position,
                    uint256(Resources.ID.M5)
                );
            } else if (value == 5) {
                IGameController(baseLogic.gameController()).updateMap(
                    id,
                    position,
                    uint256(Resources.ID.M6)
                );
            } else if (value == 7) {
                IGameController(baseLogic.gameController()).updateMap(
                    id,
                    position,
                    uint256(Resources.ID.M7)
                );
            } else if (value == 9) {
                IGameController(baseLogic.gameController()).updateMap(
                    id,
                    position,
                    uint256(Resources.ID.M8)
                );
            }
            return destroyPos;
        }
    ...
    function rollDice(
        ...
        if (baseLogic.isItemExist(id, uint256(Items.ID.IL9), false)) {
            random = typeDice;
        } else
            random =
                ...
                1;
        if (random == 1) {
            uint16[] memory posGambler = IGameController(
                baseLogic.gameController()
            ).findPosition(id, uint256(Resources.ID.G1));
            countDestroy = posGambler.length;
            uint16[] memory destroyPos = new uint16[](countDestroy);
            ...
            if (countDestroy > 0) {
                res = Helper.getArray(countDestroy, destroyPos);
            }
        }
        IGameController(baseLogic.gameController()).updatePayout(
            id,
        ...
        IEventContract(baseLogic.eventContract()).emitDice(
            id,
            position,
            random
        );
    ...
    function suits(
        ...
        int256 coin;
        bool isItemA1Exist = baseLogic.isItemExist(
            id,
            uint256(Items.ID.IA1),
            false
        );
        ...
        uint256 count = IGameController(baseLogic.gameController())
            .countResource(id, uint256(Resources.ID.C10));
        count += IGameController(baseLogic.gameController()).countResource(
            id,
            uint256(Resources.ID.S9)
        );
        count += IGameController(baseLogic.gameController()).countResource(
            id,
            uint256(Resources.ID.H1)
        );
        count += IGameController(baseLogic.gameController()).countResource(
            id,
            uint256(Resources.ID.D3)
        );
