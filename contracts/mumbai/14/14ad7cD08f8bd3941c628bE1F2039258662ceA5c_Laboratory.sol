...
contract Laboratory is ERC721Enumerable, Ownable {
    ...
    constructor(uint256 _preMint) ERC721("LABO_NFT", "LAB") {
        _preMintNumber = _preMint;
        for(uint256 i = 0; i < _preMint ; ){
            _mintLaboratory(msg.sender);
            unchecked{ ++i; }
        }
    }
    ...
    modifier onlyAuth() {
        require(
            ...
            msg.sender == gameAddresses.getLaboratoryAddress()
            , "!Auth");
        _;
    }
    ...
    function mintLaboratory(address _to)
        external
        onlyAuth
        returns (uint256)
    {
        return (_mintLaboratory(_to));
    }
    ...
    function updateNumberOfWorkingSpots(uint256 _tokenId) external onlyAuth returns(bool){
        _numberOfWorkingSpots[_tokenId] += 1;
        return true;
    }
