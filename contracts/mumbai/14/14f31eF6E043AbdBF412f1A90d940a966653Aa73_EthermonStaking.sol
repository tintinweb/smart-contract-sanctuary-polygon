...
contract EthermonStaking is EthermonStakingBasic {
        ...
        returns (uint256)
    {
        ...
        require(lockedData.monId == 0, "Token already exists");

        EthermonWeightInterface weight = EthermonWeightInterface(
            ethermonWeightContract
        );

        lockedData.owner = owner;
        lockedData.emons = depositeData._amount;
        lockedData.monId = depositeData._monId;
        lockedData.pfpId = depositeData._pfpId;
        lockedData.classId = depositeData._classId;

        miscData.lastCalled = currentTime;
        miscData.duration = depositeData._day;
        miscData.endTime =
            currentTime +
            (daysToStake[uint8(depositeData._day)] * 1 minutes);
        miscData.badge = depositeData._createdIndex;
        miscData.level = depositeData._level;
        miscData.validTeam = 1;

        miscData.teamPower =
            (lockedData.emons / 10**decimal) *
            miscData.level *
            weight.getClassWeight(lockedData.classId) *
            daysAdvantage[uint8(miscData.duration)] *
            badgeAdvantage;

        uint256 teamPower = miscData.teamPower * 10**decimal;

        uint256 sumTeamPower = stakingData.SumTeamPower();
        uint256 hourlyEmon = (((teamPower / sumTeamPower) *
            emonPerPeriod *
            (currentTime - miscData.lastCalled)) / (1 minutes)) *
            miscData.validTeam;
        miscData.balance += hourlyEmon;

        data.lockedData = lockedData;
        data.miscData = miscData;

        bytes memory output = abi.encode(data);
        uint256 t = stakingData.addTokenData(output);

        emon.safeTransferFrom(msgSender(), address(this), lockedData.emons);
        return t;
    }
    ...
    function updateTokens(
        ...
        require(
            data.lockedData.owner != address(0) &&
                data.lockedData.monId > 0 &&
                _level > 0,
            "Data is not valid"
        );

        uint256 timeElapsed = (currentTime - data.miscData.lastCalled) /
            (1 minutes);
        data.miscData.lastCalled = currentTime;

        uint256 teamPower = data.miscData.teamPower * 10**decimal;
        data.miscData.validTeam = validTeam;
        data.miscData.level = _level;
        uint256 rarity = weightData.getClassWeight(data.lockedData.classId);
        uint16 badgeAdv = data.miscData.badge > 2
            ? 10
            : badgeAdvantageValues[data.miscData.badge];

        uint256 sumTeamPower = stakingData.SumTeamPower();

        if (timeElapsed > 0 && currentTime > data.miscData.endTime) {
            require(rewardsCap > 0, "Reward cap reached max capacity");
            rewardsCap--;

            timeElapsed =
                timeElapsed -
                ((currentTime - data.miscData.endTime) / 1 minutes);
            ...
            data.miscData.balance +=
                ...
                _level *
                rarity *
                data.miscData.validTeam *
                badgeAdv;

            data.lockedData.emons += data.miscData.balance;

            if (emon.balanceOf(address(this)) >= data.lockedData.emons) {
                emon.safeTransfer(data.lockedData.owner, data.lockedData.emons);
        ...
        data.miscData.balance += hourlyEmon;
    ...
    function withdrawRewards(uint64 _lockId) external {
        ...
        TokenData memory data = stakingData.getTokenData(_lockId);
        require(
            currentTime > data.miscData.endTime,
            "Time remaining to unstake"
        );
        require(data.lockedData.monId != 0, "Data is not present");
        require(data.lockedData.owner == msgSender(), "Wrong lockId");
    ...
    function updateStakingData(
        ...
        TokenData memory data = stakingData.getTokenData(_lockId);
        require(data.lockedData.owner == owner, "PFP do not belongs to you.");
        require(data.lockedData.monId > 0, "Staking data does not exists");

        data.lockedData.monId = _monId;
        data.lockedData.pfpId = _pfpId;
        data.lockedData.classId = _classId;
        data.miscData.level = _level;
        data.miscData.badge = _createdIndex;
