...
interface iQuacksQQ_OG is IERC721 {
    ...
    function tokenIdLevels(uint256)  external  returns(uint256);
}
...
contract QuacksForThePeople is ERC1155, IERC721Receiver, Ownable, DefaultOperatorFilterer, ReentrancyGuard, ERC2981 {
    ...
    string public baseURI = "https://gateway.pinata.cloud/ipfs/Qmbw6JvsHxhMYTug72zheDfYoH8udJV6ZVnSBiC5DJ9tMd/";
 ...
 /**
    ...
    function updateTokenIdsMintStatus (uint8[] memory _tokenIds, bool[] memory _statuses) public onlyOwner {
        for (uint256 i = 0; i<_tokenIds.length;i++){
            tokenIdsMintStatus[_tokenIds[i]] = _statuses[i];
        }        
    }

    function updateTokenIdsBurnToRedeemStatus (uint8[] memory _tokenIds, bool[] memory _statuses) public onlyOwner {
        for (uint256 i = 0; i<_tokenIds.length;i++){
            tokenIdsBurnToRedeemStatus[_tokenIds[i]] = _statuses[i];
        }        
    }
    ...
    function wrap(uint256[] calldata tokenIds_) external nonReentrant {
        ...
        for (uint256 i = 0; i < tokenIds_.length; i++) {
            ...
            amt += (2**OgQuacksNFT.tokenIdLevels(i));
        }
