...
contract NFTAuction is ReentrancyGuard, Ownable  {
    ...
    function _createNewNftAuction(
        ...
        // addded -> auction end will be set at the creation and should remain the same until the end
    ...
    function _reverseAndResetPreviousBid(
        ...
        _payout(_nftContractAddress, _tokenId, nftHighestBidder, nftHighestBid, false);
    }
    ...
    function _payout(
        ...
        if (_isERC20Auction(auctionERC20Token)) {
            ...
            uint256 netSaleValue = _amount;
            // only perform royalties calculation if last step of operation
            if (_isFinalOperation) {
                setTokenContractAddress(_nftContractAddress);
                if (_checkRoyalties(_tokenContractAddress)) {
                    (netSaleValue) = _deduceRoyalties(_tokenId, _amount);   
                }
            }
        ...
        isAuctionOver(_nftContractAddress, _tokenId)
    {
        ...
        nftContractAuctions[_nftContractAddress][_tokenId].nftHighestBid >= nftContractAuctions[_nftContractAddress][_tokenId].minPrice) {
            // if there is a bid above minumin then proceed with the transfer    
            _transferNftAndPaySeller(_nftContractAddress, _tokenId);
            emit AuctionSettled(_nftContractAddress, _tokenId, msg.sender, true);
        } else {
            // else reset bid. No transfer has been made since the minPrice has not been met.
            _resetAuction(_nftContractAddress, _tokenId);
            _resetBids(_nftContractAddress, _tokenId);
            emit AuctionSettled(_nftContractAddress, _tokenId, msg.sender, false);
        }
   ...
   bytes4 private constant _INTERFACE_ID_ERC2981 = 0x2a55205a;
    ...
    function setTokenContractAddress(address _address) internal {
    ...
    /// @param tokenId - the NFT asset queried for royalties
    ...
    internal returns (uint256) {
        // address payable royalRec;
        // Get amount of royalties to pays and recipient
        (address royaltiesReceiver, uint256 royaltiesAmount) = IERC2981Royalties(_tokenContractAddress).royaltyInfo(tokenId, grossSaleValue);
        // Deduce royalties from sale value
        uint256 netSaleValue = grossSaleValue - royaltiesAmount;
        // Transfer royalties to rightholder if not zero
        if (royaltiesAmount > 0) {
            // royalRec = payable(royaltiesReceiver);
            // royalRec.transfer(royaltiesAmount);

            // attempt to send the funds to the royalties receiver
            ...
            // if it failed, update their credit balance so they can pull it later
            if (!success) {
                failedTransferCredits[royaltiesReceiver] =
                    failedTransferCredits[royaltiesReceiver] +
                    royaltiesAmount;
            }            
        }
