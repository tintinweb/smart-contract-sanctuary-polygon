...
contract BCTShortcut is IShortcut{
    ...
    function checkEligible(address erc20Addr)
        ...
        override
        returns (bool){
            return(IBCT(BCT).checkEligible(erc20Addr));
        }
    ...
    returns(uint){
        if(_checkTokenTCO2(_toToken)){
            ...
            // buy BCT
            _buyNCT(_fromToken, _toToken, _amIn, _amOut);
            // exchange the BCT tokens for TCO2s and send to user
            _exchangeNCTForTCO2(_fromToken, _toToken, _amIn, _amOut);
            // emit complete event
            return _amOut;
        }else{
            // if from token is TCO2 and eligible for BCT try and sell
        ...
        revert("Tokens are not valid for BCT shortcut");
    ...
    * @dev function to check if a sell shortcut will be valid, only returns true if
    ...
    function _checkSellShortcutValid(        
        ...
        try bct.checkEligible(_fromToken) returns(bool eligible){
            address[] memory inTokens = new address[](1);
            uint[] memory inAmount = new uint[](1);
            inTokens[0] = _fromToken;
            inAmount[0] = _amIn;

            uint amNCTBack = _amIn - bct.calculateRedeemFees(inTokens,inAmount);
    ...
    * @dev function to check if a buy shortcut will be valid, only returns true if
    ...
    function _checkBuyShortcutValid(        
        ...
        try dex.getAmountsOut(_amIn, path) returns(uint[] memory amounts){
            ...
            try IBCT(BCT).calculateRedeemFees(tco2s, amountsIn) returns(uint256 out){
                return(out >= _amOut);
            }catch(bytes memory _err){
                return false;
            }
    ...
    function _exchangeTCO2ForOutToken(
        ...
        IBCT nctContract = IBCT(BCT);

        IERC20(_fromToken).safeTransferFrom(msg.sender, address(this), _amIn);
        // we approve each time since BCT does not interpret MAXUINT as infinite
    ...
    /**
    * @dev helper function to exchange the contracts BCT tokens for TCO2s
     */
    function _exchangeNCTForTCO2(        
        ...
        IBCT nctContract = IBCT(BCT);
        require(nctContract.balanceOf(address(this)) != 0, "trying to redeem with no BCT tokens");
        ...
        IERC20(_toToken).safeTransfer(msg.sender, _amOut);
    ...
    /**
    * @dev function to buy an Order with BCT
     */
    function _buyNCT(
        address _fromToken,
        address _toToken,
