...
contract CrazyBearsMarketplaceV1 is ERC1155Holder, IERC721Receiver, Ownable, ReentrancyGuard {
    ...
    function add721TokenItem(string memory _name, string memory _imageUrl, address _contractAddress, uint256 _tokenId, uint256[] memory _costIds, uint256[] memory _costAmounts, bool _canMultiplePurchases, bool _active) public onlyOwner {
        require(_costIds.length == _costAmounts.length, "Array length does not match.");
        ...
        Item memory newItem = Item(itemId, ITEM_TYPE_ERC721, _name, _imageUrl, _contractAddress, _tokenId, 1, 1, 0, _costIds ,_costAmounts, _canMultiplePurchases, _active, block.timestamp);
