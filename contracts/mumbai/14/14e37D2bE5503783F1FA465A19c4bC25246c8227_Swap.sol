...
contract Swap is ZetaInteractor, ZetaReceiver, SwapErrors, Pausable {
    ...
    function swapZeroEx(XcSwapZeroEx calldata args) external payable whenNotPaused {
        ...
        /* (bool successWeth, ) = WETH.call{value: msg.value}("");
        if (!successWeth) revert ErrorSwappingTokens(); */

        // Sell token validation
        /* if (args.sellTokenAddress == address(0) || args.sellTokenAddress == iZetaToken)
            revert InvalidTokenAddress(); */

        // If sellToken is not ETH, transfer token to this contract
            ...
            ZetaInterfaces.SendInput({
                ...
                message: abi.encode(
                    ...
                    XcMessageZeroEx({
                        //messageType: SWAP_MESSAGE,
                        sourceTxOrigin: msg.sender,
                        buyTokenAddress: args.buyTokenAddress,
        ...
        isValidMessageCall(rawZetaMessage)
    {
        ...
        if (messageType == SWAP_MESSAGE) {
            XcMessage memory xcMessage = abi.decode(message, (XcMessage));

            //XcMessage memory xcMessage = abi.decode(rawMessage, (XcMessage));
            ...
            XcMessageZeroEx memory xcMessage = abi.decode(message, (XcMessageZeroEx));
