...
contract Ticket is ERC721URIStorage, Governable, ReentrancyGuard {
    /// @notice Ticket ids.
    uint256 private _ticketId;

    /// @notice The max ticket count.
    uint256 private _maxTicketCount;

    /// @notice The total sold ticket count.
    uint256 public soldTicketCount;

    /// @notice holds the minted tickets for users.
    mapping(address => bool) _hasTicket;

    /// @notice Ticket event name
    string private _eventName;

    /// @notice Emitted when a ticket is sold.
    event TicketSold(address owner, uint256 ticketId);

    /// @notice Emitted when a ticket count is set.
    event TicketCountSet(uint256 ticketCount);

    string private baseSvg =
        "<svg xmlns='http://www.w3.org/2000/svg' preserveAspectRatio='xMinYMin meet' viewBox='0 0 350 350'><style>.base { fill: white; font-family: serif;font-size: 24px; }</style><rect width='100%' height='100%' fill='black'/><text x='50%' y='30%' class='base' dominant-baseline='middle' text-anchor='middle'>IS SANAT NFT BILET</text> <text x='50%' y='40%' fill='white' dominant-baseline='middle' text-anchor='middle' font-size='smaller'>";

    constructor(address governance, uint256 maxTicketCount, string memory eventName) Governable(governance) ERC721("Is Sanat Ticket", "ISSANAT") {
        require(maxTicketCount > 0, "invalid ticket count");
    ...
    /**
     * @notice Creates a ticket.
     * @param user The user to mint ticket.
     * @return ticketId The ticket id.
    */
    function createTicket(address user) external nonReentrant returns (uint256 ticketId) {
        require(soldTicketCount <= _maxTicketCount, "tickets sold out");
        require(!_hasTicket[user], "user has ticket");
        require(user != address(0x0), "zero address user");

        soldTicketCount++;
        _ticketId++;
        _safeMint(user, _ticketId);
        _setTokenURI(_ticketId, createTokenURI());
        _hasTicket[user] = true;
        emit TicketSold(user, _ticketId);
        return _ticketId;
    }

    /**
     * @notice Returns ticket event name.
     * @return eventName The event name in utf-8.
     */
    ...
    /**
     * @notice Creates token URI.
     * @return tokenURI The token URI.
    */
    function createTokenURI() private view returns (string memory) {
        string memory eName = getEventName();
        string memory svg = createSVG();
        string memory json = Base64.encode(
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "IS SANAT NFT TICKET",',
                        '"description": "IS SANAT ticket for ',
                        eName,
                        '",',
    ...
    function createSVG() private view returns (string memory) {
        string memory eName = getEventName();
        return
            string(
                abi.encodePacked(
    ...
    function setMaxTicketCount(uint256 ticketCount) external onlyGovernance {
        _maxTicketCount = ticketCount;
        emit TicketCountSet(ticketCount);
    }
