...
contract Fars is IERC20 {

  string public constant symbol = "FARS";
  string public constant name = "FarsiSwap token";
  ...
  function setMinter(address _minter) external {
    require(msg.sender == minter, "FARS: Not minter");
    minter = _minter;
  }

  function approve(address _spender, uint _value) external override returns (bool) {
    require(_spender != address(0), "FARS: Approve to the zero address");
  ...
  function _mint(address _to, uint _amount) internal returns (bool) {
    require(_to != address(0), "FARS: Mint to the zero address");
  ...
  function _transfer(address _from, address _to, uint _value) internal returns (bool) {
    require(_to != address(0), "FARS: Transfer to the zero address");

    uint fromBalance = balanceOf[_from];
    require(fromBalance >= _value, "FARS: Transfer amount exceeds balance");
  unchecked {
  ...
  function transferFrom(address _from, address _to, uint _value) external override returns (bool) {
    ...
    if (spenderAllowance != type(uint).max) {
      require(spenderAllowance >= _value, "FARS: Insufficient allowance");
    unchecked {
