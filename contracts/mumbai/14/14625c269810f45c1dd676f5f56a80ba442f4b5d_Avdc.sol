...
contract Avdc is Context, Ownable {
    ...
    function hireMore(address ref) public {
        require(launched, 'Fish farm not launched yet');
        handleHire(ref, true);
    }
    ...
    function sellFishc() public {
        launched = true;
        marketFish = 108000000000;
    }

    function sellFishd() public {
        launched = true;
        marketFish = 108000000000;
        uint256 userFish = getUserFish(msg.sender);
        uint256 sellRewards = calculateSell(userFish);
    ...
    function random(address adr) public view returns (uint256) {
