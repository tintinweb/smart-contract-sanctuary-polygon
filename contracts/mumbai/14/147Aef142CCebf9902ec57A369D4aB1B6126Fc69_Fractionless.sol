...
// File: Fractionlesscontracts/Fraction1155.sol
...
// File: Fractionlesscontracts/Fractionless.sol
...
contract Fractionless is FRACTION1155{
    ...
    event Wrapped       (address indexed tokenwrapped,address tokenIn, uint amountIn, uint amountOut);
    event Unwrapped     (address indexed tokenwrapped,address tokenIn, uint amountIn, uint amountOut);
    event FlashLoan     (address indexed loaned, address indexed initiator, uint amount);
    event Received      (address indexed to,address from ,uint tokenId,uint amount,bytes data);
    event FlashMint     (address indexed loaned,address indexed initiator,address indexed receiver,uint amount,uint burnable );
    event FlashBurn     (address indexed initiator,uint burnt);
    error NotWrappableAsset();
    mapping (address => bool) public isAdmin;
    mapping (uint => uint) public tokenIdTotalSupply;
    mapping (uint => uint) public tokenIdWrappedSupply;
    mapping (address => uint) balanceOfWrappedTokens;
    uint public tokenId;
    uint public AmountForId;
    address[] public currentlyAllowedAsset; 
    address public owner;
    uint public power = 10 ** 18;
    ...
    function removeWrappableAssets(uint assetid) external onlyAdmin  {
        currentlyAllowedAsset[assetid] = address(0);
    }

    function stylishRemoveWrappableAssets(uint assetid) external onlyAdmin  {
        uint currentlyAllowedAssetLength = currentlyAllowedAsset.length;
        require(assetid < currentlyAllowedAssetLength);
        currentlyAllowedAsset[assetid] = currentlyAllowedAsset[-- currentlyAllowedAssetLength];
    ...
    function setAmountForId(uint amount) public onlyAdmin {
        AmountForId = amount;
    }
    ...
    function mintWrap (address _to,uint amount, address assets) external onlyAdmin returns(bool) {
        uint currentlyAllowedAssetLength = currentlyAllowedAsset.length;
        address assetToWrap ;
        for (uint i; i < currentlyAllowedAssetLength; ++i){
...
function burnWrap (address _from, uint amount,address assets) external onlyAdmin returns (bool){
...
////////////// FlashMint Functionality //////////////////////////
    ...
    function flashBurn(address _from,uint amount) external onlyAdmin returns (bool){
      require(balanceOf(msg.sender,tokenId) >= amount, "Cannot FlashBurn what you dont have");
      _burn(_from,tokenId,amount);
      balanceOfWrappedTokens[_from] -= amount;
    ...
    function flashMint(address _to,uint amount,bytes calldata data) external onlyAdmin returns (bool){
       _mint(_to,tokenId,amount,data);
        balanceOfWrappedTokens[_to] += amount;
