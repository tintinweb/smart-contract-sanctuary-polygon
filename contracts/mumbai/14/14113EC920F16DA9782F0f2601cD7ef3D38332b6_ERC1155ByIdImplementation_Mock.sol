...
import "../extensions/IPreviousContract.sol";
    ...
    IERC1155ById
{
    ...
    // Mapping tokens of token type `id` to total supply.
    mapping (uint256 => uint256) private _totalSupply;

    // Initialize interface reference for previous contract.
    IPreviousContract public previousContract;
    ...
    {
        return
            interfaceId == type(IERC1155ById).interfaceId || // 0x4a87a5c2
            super.supportsInterface(interfaceId);
    }
     ...
     * @dev
     */
    function getBaseURI(uint256 id) public view virtual returns (string memory) {
        if (bytes(_baseURIByIndex[id]).length == 0) revert IdDoesNotExist();

        return BaseURIStorage._baseURIByIndex[id];
    }

    /**
     * @dev
     */
    function getTotalBaseURI() public view virtual returns (uint256) {
        return BaseURIStorage._totalBaseURI;
    }

    /**
     * @dev 
     */
    function getMaxSupply(uint256 id) public view virtual returns (uint256) {
        if (bytes(_baseURIByIndex[id]).length == 0) revert IdDoesNotExist();
        if (!_isMaxSupplyByIndexDefined[id]) revert MaximumSupplyByIdIsNotDefined();

        return NFTSupplyStorage._maxSupplyByIndex[id];
    }

    /**
     * @dev Returns NFT life cycle tokens of token type `id`.
     * 
     * See {NFTLifeCycle-getNFTLifeCycle}.
     */
    ...
    function uri(uint256 id) public view virtual override returns (string memory) {
        // Get base URI by `id`.
        string memory _baseURIById = _baseURIByIndex[id];

        return bytes(_baseURIById).length > 0 ? string(abi.encodePacked(_baseURIById)) : "";
    }

    /// =============================================================
    ///                 INTERNAL AIRDROP LOGICS
    /// =============================================================
    ...
    function _airdropBatch(
        ...
        if (receiversLength != ids.length || receiversLength != amounts.length || ids.length != amounts.length) revert InvalidLength();
    ...
    function _mintBatch(
        ...
        for (uint256 i = 0; i < idsLength; i = _uncheckedIteration(i)) {
            uint256 _ids = ids[i];
            uint256 _amounts = amounts[i];
    ...
    function _validateMintInput(uint256 id, uint256 amount) internal virtual {
        ...
        if (bytes(getBaseURI(id)).length == 0) revert IdDoesNotExist();
        ...
        if (_isMaxSupplyByIndexDefined[id]) {
            if (amount + totalSupply(id) > getMaxSupply(id)) revert ExceedMaximumSupplyById();
        }
    }

    /// =============================================================
    ///             INTERNAL CHECK {balanceOf} LOGICS
    /// =============================================================

    /**
     * @dev Check {IPreviousContract-balanceOf} of token type `id` from `owners` from `previous contract`.
     */
    function _balanceOfBatchPreviousContract(address[] calldata accounts, uint256 id) internal view {
        uint256 accountsLength = accounts.length;

            for (uint256 i = 0; i < accountsLength; i = _uncheckedIteration(i)) {
                address _owners = accounts[i];

                _balanceOfPreviousContract(_owners, id);
            }
    }

    /**
     * @dev Check {IPreviousContract-balanceOf} of token type `id` from `owner` from `previous contract`.
     */
    function _balanceOfPreviousContract(address account, uint id) internal view {
        if (IPreviousContract(previousContract).balanceOf(account, id) == 0) revert BalanceOfIdIsZero();
    }

    /**
     * @dev Check {ERC1155-balanceOf} of token type `id` from `owners`.
     */
    function _balanceOfBatch(address[] calldata accounts, uint256 id) internal view {
        uint256 accountsLength = accounts.length;

            for (uint256 i = 0; i < accountsLength; i = _uncheckedIteration(i)) {
                address _accounts = accounts[i];

                _balanceOf(_accounts, id);
            }
    }
     ...
     */
    function _balanceOf(address account, uint256 id) internal view {
        if (super.balanceOf(account, id) == 0) revert BalanceOfIdIsZero();
    }

    /// =============================================================
    ///                 INTERNAL BURN LOGICS
    /// =============================================================
    ...
    function _setBaseURIById(uint256[] calldata ids, string[] memory baseURI) internal virtual {
        ...
        for (uint256 i = 0; i < idsLength; i = _uncheckedIteration(i)) {
            ...
            // If total supply of `ids` exist, `baseURI` by `ids` can not be reset.
            if (bytes(_baseURI).length == 0 && totalSupply(_ids) > 0) revert TotalSupplyByIdExist();

            super._setBaseURIByIndex(_ids, _baseURI);
        }

        emit BaseURIByIdUpdated(_msgSender(), ids, baseURI);
    }

    /**
     * @dev Sets maximum amount tokens of token type `id` can be minted in one transaction.
     */
    function _setMaxAmount(uint256 maxAmount) internal virtual {
        if (maxAmount == 0 || maxAmount == _maxQuantity) revert InvalidMaximumAmount();

        super._setMaxQuantity(maxAmount);

        emit MaxAmountUpdated(_msgSender(), maxAmount);
    }

    /**
     * @dev Sets maximum supply by mapping `maxSupply` to `ids` and `defined` as `true` or `false`.
     * 
     * See {NFTSupplyStorage-_setMaxSupplyByIndex}.
     */
    function _setMaxSupplyById(
        ...
        for (uint256 i = 0; i < idsLength; i = _uncheckedIteration(i)) {
            ...
            // `maxSupply` can not be the same with current max supply value.
            if ((_maxSupply == _maxSupplyByIndex[_ids]) && _isMaxSupplyByIndexDefined[_ids]) revert InvalidMaximumSupply();

            // `maxSupply` can not be zero and defined as `true`.
            // If `maxSupply` more than zero can not be defined as `false`.
            ...
            // If total supply of `ids` exist, `maxSupply` can not be reset.
            if ((_maxSupply == 0 && !defined) && (totalSupply(_ids) > 0)) revert TotalSupplyByIdExist();

            super._setMaxSupplyByIndex(_ids, _maxSupply, defined);
        }

         emit MaxSupplyByIdUpdated(_msgSender(), ids, maxSupply, defined);
    }
    ...
    function _setNFTLifeCycleById(uint256[] calldata ids, uint8[] calldata stateValue) internal virtual {
        ...
        for (uint256 i = 0; i < idsLength; i = _uncheckedIteration(i)) {
            ...
            // `stateValue` can not be greater than maximum value represents NFT life cycle state.
            if (_stateValue > uint8(type(NFTLifeCycleState).max)) revert InvalidNFTLifeCycleStateValue();

            super._setNFTLifeCycleState(_ids, _stateValue);
        }
    }
    ...
    function _setRoyaltyById(
        ...
        for (uint256 i = 0; i < idsLength; i = _uncheckedIteration(i)) {
            uint256 _ids = ids[i];
            uint96 _basisPoints = basisPoints[i];

            super._setTokenRoyalty(_ids, recipient, _basisPoints);
        }

        emit RoyaltyByIdUpdated(_msgSender(), ids, recipient, basisPoints);
    }

    /**
     * @dev Sets previous contract to be referenced.
     */
    function _setPreviousContract(address _previousContract) internal virtual {
        // Check if `previousContract` is a contract.
        if (_previousContract.code.length == 0) revert AddressIsNotAContract();

        previousContract = IPreviousContract(_previousContract);
    }

    /// =============================================================
    ///             INTERNAL OVERRIDDEN FUNCTION LOGICS
    /// =============================================================
    ...
    /**
     * @dev Helpfer function for unchecked iteration.
     */
...
interface IERC1155ById is IERC165 {
    ...
    /**
     * @dev Update maximum amount is invalid.
     */
    error InvalidMaximumAmount();

    /**
     * @dev Updated maximum supply is invalid.
     */
    error InvalidMaximumSupply();

    /**
     * @dev NFT life cycle State value is invalid.
     */
    error InvalidNFTLifeCycleStateValue();

    /**
     * @dev Maximum supply of `id` is not defined.
     */
    error MaximumSupplyByIdIsNotDefined();
    ...
    /**
     * @dev Emitted when `caller` update base URI by `ids` of the collection.
     */
    event BaseURIByIdUpdated(
        address indexed caller,
        uint256[] ids,
        string[] updatedBaseURI
    );

    /**
     * @dev Emitted when `caller` update maximum amount of tokens of token type `id` can be minted in one transaction.
     */
    event MaxAmountUpdated(address indexed caller, uint256 updatedMaxAmount);

    /**
     * @dev Emitted when `caller` update maximum supply by index.
     */
    event MaxSupplyByIdUpdated(
        address indexed caller,
        uint256[] ids,
    ...
    /**
     * @dev Emitted when the `caller` update royalty information that all ids in this contract will default to.
     */
    ...
    /**
     * @dev Emitted when the `caller` update royalty by `ids` information.
     */
    event RoyaltyByIdUpdated(
        address indexed caller,
        uint256[] ids,
        address indexed royaltyRecipient,
        uint96[] updatedFeeBasisPoints
...
abstract contract BaseURIStorage {
    // Initialize total base URI.
    uint256 internal _totalBaseURI;
     ...
     * @param baseURI must refer to where the resource base token URI per index is located (e.g "ipfs://..." or "https://...").
     */
    function _setBaseURIByIndex(uint256 index, string memory baseURI) internal virtual {
        if (bytes(baseURI).length == 0) {
            _resetBaseURIByIndex(index);
        }
    ...
    /**
     * @dev Base URI by `index` to default value.
     */
    function _resetBaseURIByIndex(uint256 index) internal virtual {
...
abstract contract NFTLifeCycle {
    ...
    /**
     * @dev Returns NFT life cycle by `index`.
     */
...
abstract contract NFTSupplyStorage {
    ...
    /**
     * @dev Sets maximum quantity of maximum quantity of `tokenId` (ERC721) or
    ...
    /**
     * @dev Reset maximum supply by `index` to default value.
     */
...
contract ERC1155ByIdImplementation_Mock is 
    ...
    constructor(
        ...
        // Init collection.
        ERC1155ById._setMaxAmount(_initMaxAmount);
        ERC2981._setDefaultRoyalty(_initRoyaltyRecipient, _initFeeBasisPoints);
    }
    ...
    function airdropByBalance(
        ...
        _balanceOfBatch(receivers, ownedId);
        _airdrop(receivers, id, amounts);
    }
    ...
    /**
     * @dev See {ERC1155ById-_setDefaultRoyalty}.
     */
    function setDefaultRoyalty(address updateRecipient, uint16 feeBasisPoints) external onlyRole(ADMIN_ROLE) whenNotPaused {
        _setDefaultRoyalty(updateRecipient, feeBasisPoints);
    }
    ...
    /**
     * @dev See {ERC1155ById-_setRoyaltyById}.
     */
    function setRoyaltyById(
        uint256[] calldata ids,
        address updateRecipient,
        uint96[] calldata feeBasisPoints
    ) external onlyRole(ADMIN_ROLE) whenNotPaused {
        _setRoyaltyById(ids, updateRecipient,feeBasisPoints);
    }
