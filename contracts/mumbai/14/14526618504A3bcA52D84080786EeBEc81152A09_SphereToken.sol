...
contract SphereToken is ERC20Detailed, Ownable {
    ...
    function getTokensInLPCirculation() public view returns (uint256) {
        ...
        for (uint256 i = 0; i < _markerPairs.length; i++) {
            ...
            if (token0 == address(this)) {
                LPTotal.add(reserve0);
                //first one
            } else if (token1 == address(this)) {
                LPTotal.add(reserve1);
            }
        }
        ...
        onlyOwner
    {
        ...
        if (_value) {
            ...
            require(subContracts.length > 1, 'Required 1 pair');
            for (uint256 i = 0; i < subContracts.length; i++) {
    ...
    function addPartyAddies(address _partyAddy, bool _value) public onlyOwner {
        ...
        if (_value) {
            ...
            require(partyArray.length > 1, 'Required 1 pair');
    ...
    function PartyIsOver() external onlyOwner {
        isPartyOver = true;
    }
    ...
    //if not a swap in the Sphere LP, then its outside of our LPs and therefore taxed additionally
    //prevents whales from exploiting by swapping from one wallet to another and sell
    function setNonMarketMakerFee(uint256 _nonMarketMakerFee)
        external
        onlyOwner
    {
        require(
            _nonMarketMakerFee <= NON_MARKET_MAKER_FEE_RATE,
            'max non Market Maker fee exceeded'
        );
        nonMarketMakerFee = _nonMarketMakerFee;
    }
