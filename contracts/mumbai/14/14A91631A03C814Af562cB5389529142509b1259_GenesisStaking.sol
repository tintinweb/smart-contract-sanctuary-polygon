...
contract GenesisStaking is Pausable {
    ...
    modifier basicCheck(uint256 _amount) {
        ...
        require(stakes[msg.sender].length == 0, "Already stake");
        //need in the period
        require(
        ...
        returns (bool)
    {
        ...
        stakes[msg.sender].push(Stake(_amount, block.timestamp, 3, false));
        ...
        returns (bool)
    {
        ...
        stakes[msg.sender].push(Stake(_amount, block.timestamp, 6, false));
        uint256 index = stakes[msg.sender].length - 1;
        ...
        returns (bool)
    {
        ...
        stakes[msg.sender].push(Stake(_amount, block.timestamp, 12, false));
        uint256 index = stakes[msg.sender].length - 1;
    ...
    function claimReward_ROE() external {
        require(stakes[msg.sender].length > 0, "No staking");
        require(!stakes[msg.sender][0].executed, "Already claim");
        require(
            block.timestamp >=
                startStaking + stakes[msg.sender][0].month * 30 * 1 days,
        ...
        uint256 bonusAmount;
        if (bonusReward == 0) {
            originAmount = calOriginAmount(msg.sender);
            rewardAmount = calReward(msg.sender);
            totalAmount = originAmount + rewardAmount;
        } else {
            originAmount = calOriginAmount(msg.sender);
            rewardAmount = calReward(msg.sender);
            bonusAmount = calBonus(msg.sender);
        ...
        stakes[msg.sender][0].executed = true;
        token.transfer(msg.sender, totalAmount);
    }

    function estimate_Reward(address _address) external view returns (uint256) {
        uint256 totalAmount;
        uint256 originAmount;
        uint256 rewardAmount;
        uint256 bonusAmount;
        if (bonusReward == 0) {
            originAmount = calOriginAmount(_address);
            rewardAmount = calReward(_address);
            totalAmount = originAmount + rewardAmount;
        } else {
            originAmount = calOriginAmount(_address);
            rewardAmount = calReward(_address);
            bonusAmount = calBonus(_address);
            totalAmount = originAmount + rewardAmount + bonusAmount;
        }
        return totalAmount;
    }

    function calOriginAmount(address _address) internal view returns (uint256) {
        uint256 totalAmount = 0;
        for (uint256 i = 0; i < stakes[_address].length; i++) {
            totalAmount += stakes[_address][i].amount;
        }
        return totalAmount;
    }
    ...
    function calReward(address _address) internal view returns (uint256) {
        uint256 reward;
        for (uint256 i = 0; i < stakes[_address].length; i++) {
            uint256 stakePeriod = stakes[_address][i].month;
            if (stakePeriod == 3) {
                reward +=
                    (stakes[_address][i].amount * three_month_interest) /
                    100;
            } else if (stakePeriod == 6) {
                reward +=
                    (stakes[_address][i].amount * six_month_interest) /
                    100;
            } else if (stakePeriod == 12) {
                reward +=
                    (stakes[_address][i].amount * twelve_month_interest) /
                    100;
            }
        }
        return reward;
    ...
    function calBonus(address _address) public view returns (uint256) {
        uint256 allWeight = getAllWeight();
        uint256 userWeight = getUserWeight(_address);
    ...
    function getUserWeight(address _address) internal view returns (uint256) {
        uint256 userWeight = 0;
        for (uint256 i = 0; i < stakes[_address].length; i++) {
            userWeight +=
                stakes[_address][i].amount *
                (block.timestamp - stakes[_address][i].since);
        }
        return userWeight;
    }
