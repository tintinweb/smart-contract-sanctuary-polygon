...
import "../libraries/math/OptionMath.sol";


contract BinaryOptions is
    IBinaryOptions,
    ReentrancyGuard,
    ERC721,
    AccessControl
{
    ...
    ILiquidityPool public poolManager;
    ...
    function approvePoolToTransferTokenX() public {
        tokenX.approve(address(poolManager), ~uint256(0));
    }
    ...
    function createFromRouter(
        ...
        poolManager.lock(optionID, option.lockedAmount, option.premium);
        emit Create(
            optionParams.user,
            optionID,
            settlementFee,
    ...
    {
        uint256 settlementFeePercentage= _getbaseSettlementFeePercentage(isAbove);
    ...
    {
        // Calculate the max option size due to asset wise poolManager utilization limit
        uint256 totalPoolBalance = poolManager.totalTokenXBalance();
        ...
        availableBalance = poolManager.availableBalance();
        ...
        )
    {
        ...
        uint256 maxPerTxnFee = ((poolManager.availableBalance() *
        ...
        uint256 settlementFeePercentage = _getbaseSettlementFeePercentage(optionParams.isAbove);
    ...
    function ownerOf(uint256 tokenId)
        ...
        view
        virtual
        override(ERC721, IBinaryOptions)
        returns (address)
    {
    ...
    function _exercise(
        ...
        poolManager.send(optionID, user, profit);
...
interface IBinaryOptionsForReader is IBinaryOptions {
...
interface IWhitelist {
    ...
    function addWhitlisted(address user) external  view;
    function removeWhitlisted(address user) external  view;
}
...
library OptionMath {
     ...
     */
    /**
     *注意使用Black-Scholes模型计算一个期权的价格
     * @param impliedVol uint256 表示年化隐含价值，系数为1e4
     *@param strike uint256 表示执行价格，系数为1e8
     * @param spot uint256 代表现货价格，系数为1e8
     * @param period uint256表示期权合约的期限（以秒为单位）。
     * @param isYes 是否为 "看涨 "或 "看跌 "期权定价
     * @param isAbove 用户是否打赌价格会保持在这个行权价之上？
     *@return uint256表示Black-Scholes期权价格，系数为1e8
     */
