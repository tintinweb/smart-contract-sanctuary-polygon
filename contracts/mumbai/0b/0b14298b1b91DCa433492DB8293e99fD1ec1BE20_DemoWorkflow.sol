...
contract DemoWorkflow {
    ...
    function createAddLiquidityWorkflow(
        ...
        AddLiquidityWorkflowDetail memory details = AddLiquidityWorkflowDetail({
            ...
            amountB: _amountB,
            tokenAReferencePrice: _tokenAReferencePrice,
            tokenBReferencePrice: _tokenBReferencePrice,
            isRunning: true
        });
    ...
    function _lookupLimitOrder() internal {
        for(uint256 i = 0; i < limitOrderWorkflows.length;) {
            ...
            if (priceOfTokenA > details.tokenAReferencePrice) {
                swap(
                    owner, 
                    details.tokenA,
                    details.tokenB,
    ...
    function _lookupAddLiquidity() internal {
        for(uint256 i = 0; i < addLiquidityWorkflows.length;) {
            ...
            uint256 priceOfTokenA = vwapOracle.getStoredTokenPrices(details.tokenA);
            uint256 priceOfTokenB = vwapOracle.getStoredTokenPrices(details.tokenB);

            if (priceOfTokenA > details.tokenAReferencePrice || priceOfTokenB > details.tokenBReferencePrice) {
                addLiquidity(
                    owner,
                    details.tokenA,
                    details.tokenB,
