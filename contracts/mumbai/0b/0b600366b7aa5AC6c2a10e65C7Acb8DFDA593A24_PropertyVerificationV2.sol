...
contract PropertyVerificationV2 is OwnableUpgradeable {
    ...
    function makePropertyNotPublic(uint256 propertyId)
        external
        propertyOwnerOnly(propertyId)
    {
        require(
            properties[propertyId].isPublic,
            "Property is already not public"
        );
    ...
    function unlistProperty(uint256 propertyId) external {
        require(
            msg.sender == EscrowContractAddress,
            "User is not the escrow contract"
        );
    ...
    function setEscrowAddress(address newEscrowAddress) external onlyOwner {
        require(
            newEscrowAddress != address(0),
            "Escrow address cannot be 0 address"
        );
