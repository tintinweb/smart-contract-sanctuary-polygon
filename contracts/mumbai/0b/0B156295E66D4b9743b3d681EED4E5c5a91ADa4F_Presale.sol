    ...
    function _saveRoyalties(uint256 _id, LibPart.Part[] memory _royalties) internal {
        for (uint i = 0; i < _royalties.length; i++) {
            ...
            if(royalties[_id].length > 0) {
                royalties[0][0].account = _royalties[i].account;
                royalties[0][0].value = _royalties[i].value;
            }
            else {
...
contract Presale is ERC1155, AccessControl, Pausable, ERC1155Burnable, ERC1155Supply, RoyaltiesV2Impl, ERC2981Base {
    ...
    constructor(address _weth) ERC1155(baseUri) {       
        ...
        _mint(address(this), 0, INITIAL_SUPPLY, "");
        // Mint team and studio
        _mint(msg.sender, 0, 520, "");

        // Rarible Royalties
        setRoyalties(0, payable(address(royaltyAddress)), uint96(royaltyBase));

        // set weth address from constructor
        wethToken = _weth;
    }
    
    /// @dev pause contract
    function pause() external onlyRole(CLEVEL) {
        _pause();
    }

    /// @dev unpause contract
    function unpause() external onlyRole(CEO) {
        _unpause();
    }

    /// @param account send to this
    /// @param id token id
    /// @param amount amount
    ...
    /// @param _interfaceId the interface
    ...
    /// @param _royaltiesReceipientAddress royalty address
    /// @param _percentageBasisPoints royalty amount
    /// @dev update sale price of nft
    function setRoyalties(uint _tokenId, address payable _royaltiesReceipientAddress, uint96 _percentageBasisPoints) internal onlyRole(CEO) {
        ...
        returns (address receiver, uint256 royaltyAmount)
    {
        ...
        royaltyAmount = (_salePrice * royaltyBase) / 10000;
    }
    ...
    /// @param _amount of token to purchase
    /// @dev users cna buy tokens from contract
    ...
    function updatePrefix(string memory _prefix) external onlyRole(CLEVEL) {
        baseUri = _prefix;
    }

    /// @param _suffix the new url suffix for metadata
    /// @dev update metadata urls suffix, like baseuri
    function updateSuffix(string memory _suffix) external onlyRole(CLEVEL) {
        suffix = _suffix;
    }
    ...
    /// @dev allows for setting a royalty address
    function updateRoyaltyAddress(address _address) external onlyRole(CEO) {
        royaltyAddress = _address;
        setRoyalties(0, payable(royaltyAddress), uint96(royaltyBase));
    }

    /// @param _amount the new royalty amount
    /// @dev allows for setting a royalty amount
    function updateRoyaltyAmount(uint256 _amount) external onlyRole(CEO) {
