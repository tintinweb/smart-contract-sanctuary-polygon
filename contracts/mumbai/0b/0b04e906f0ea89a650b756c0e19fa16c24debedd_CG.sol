...
// File: contracts/CG.sol

// SPDX-License-Identifier: MIT

pragma solidity ^0.8.0;

contract CG is ERC721, Ownable, RandomlyAssigned {
  using Strings for uint256;
  
  string public baseURI = "https://dev.tuwaga.io/clickgroup-angpao/";
  ...
  mapping (address => uint256) public mappedTokenIds;
  address[] public airdropped;
  ...
  function mintAirdrop() public onlyOwner payable {
    for(uint256 i = 0; i < maxSupply; i++) {
        uint256 id = nextToken();
        mappedTokenIds[airdropped[i]] = id;
        _safeMint(airdropped[i], id);
    }
  }
  
  function airdropUsers() public view returns(uint256){
