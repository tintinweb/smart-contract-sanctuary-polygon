...
contract ELSSPACE is ERC1155, Ownable {
    ...
    constructor(address _beneficiaryAddress) ERC1155("ipfs://QmYMK5962vMoM2BSko16tHg2cXi6e8x1xX23vANvfsjvHN/{id}.json") {
        ...
        USDT = IERC20(address(0x9c3C9283D3e44854697Cd22D3Faa240Cfb032889)); // Real: 0xdAC17F958D2ee523a2206206994597C13D831ec7
    }
    ...
    function changePrice(uint _price, uint _priceUSDT) external onlyOwner {
        price = _price;
        priceUSDT = _priceUSDT;
    }

    function mint(uint _tokenId) public payable returns (uint) {
        require(minted[_tokenId] != true, "Mint: Token is already minted!");
    ...
    function mintUSDT(uint _tokenId) public returns (uint) {
        require(priceUSDT > 0, "MintUSDT: Mint for USDT is not allowed!");
        ...
        require(USDT.transferFrom(msg.sender, beneficiaryAddress, priceUSDT), "Mint: Transfer of USDT is not allowed / not enough USDT");
    ...
    function mintTo(uint _tokenId, address to) external onlyOwner {
        require(minted[_tokenId] != true, "Mint: Token is already minted!");
