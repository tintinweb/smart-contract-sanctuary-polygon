...
contract AuctionFactoryStreak is Ownable {
  ...
  constructor(address _erc20StreakAddress, address _erc721StreakContract, address _erc1155StreakContract, address _controller) {
    require(_controller != address(0), "AuctionFactoryStreak: controller address can't be empty");
  ...
  external
  {
    ...
    require(msg.sender == parameters.owner, "AuctionFactoryStreak: Only creator can cancel auction");
    require(!auction.cancelled());
    require(block.timestamp < parameters.endTimestamp);
  ...
  public
  {
    require(feeRate <= 100, "AuctionFactoryStreak: feeRate mustn't be greater than 100");
    feeRate = _feeRate;
  }
