...
contract UserClientMulti is SxTClient {
    ...
    uint256 public currentIntResponse;
    ...
    function executeViewString2D(string memory viewName, string memory biscuitId)
        external
    {
        currentRequestId = _executeView(
            viewName,
    ...
    function executeViewMultiWord(string memory viewName, string memory biscuitId)
        external
    {
        currentRequestId = _executeView(
            viewName,
    ...
    function saveIntResponse(bytes32 requestId, uint256 response)
        external
        onlySxTValidator
     ...
     */
    function saveString2DResponse(bytes32 requestId, string[][] calldata response)
        external
        onlySxTValidator
        recordSXTFulfillment(requestId)
    {
    ...
    function saveMultiWordResponse(bytes32 requestId, uint256 response1, uint256 response2)
        external
        onlySxTValidator
        recordSXTFulfillment(requestId)
    {
