...
contract Dao is Ownable, Pausable {
    struct ProposalVote {
        ...
        mapping(address => uint256) amountVote;
        mapping(address => bool) unlockToken;
    ...
    IERC20 public token;
    uint256 public VOTING_DURATION = 60;
    ...
    function unlockVote(bytes32 proposalId) external {
        ...
        require(proposalState == ProposalState.Succeeded || 
                ...
                proposalState == ProposalState.Executed, "invalid state");

        ProposalVote storage proposalVote = proposalVotes[proposalId];
        require(proposalVote.unlockToken[msg.sender] == false, "Unlocked already");

        token.transferFrom(address(this), msg.sender, proposalVote.amountVote[msg.sender]);
    ...
    //    ProposalVote storage proposalVote = proposalVotes[proposalId];
    //    return proposalVote.againstVote;
    //}
