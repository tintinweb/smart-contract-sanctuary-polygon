...
// | $$      | $$\  $$$| $$  \ $$      | $$  | $$| $$  \ $$| $$  | $$| $$
// | $$$$$$$$| $$ \  $$|  $$$$$$/      | $$$$$$$/| $$  | $$|  $$$$$$/| $$
// |________/|__/  \__/ \______/       |_______/ |__/  |__/ \______/ |__/

pragma solidity ^0.8.0;

import "@thirdweb-dev/contracts/base/ERC721Drop.sol";

contract EngNFTDrop is ERC721Drop {
    address public externalContractAddress;
    uint256 selectedHolderClaimPrice = 0.001 ether;
    ...
    /**
     * @dev allow NFT holder of the external contract to claim.
     * @param _receiver The address receiving the NFTs.
     * @param _quantity The number of NFTs to mint. Maximum of 1 NFT can be minted per transaction.
     */
    function selectedHolderClaim(
        address _receiver,
        uint256 _quantity
    ) public payable {
        require(_quantity != 0, "quantity value cannot be null or zero");
        require(_quantity == 1, "allow to claim 1 at a time");
        require(balanceOf(_receiver) == 0, "user has claimed before");
        require(
            ERC721A(externalContractAddress).balanceOf(_receiver) > 0,
            "User does not own a NFT"
        );
        require(
            msg.value != selectedHolderClaimPrice,
            "provided amount is not correct"
        );
