...
// File contracts/ETCBaycComic.sol
...
contract ETCBaycComic is
    ...
    ERC721Burnable
{
    event Mint(address indexed mintedBy, address indexed mintedTo, uint indexed tokenId);
    event MarketTrade(
    ...
    Counters.Counter private _airdroppedCounter;

    uint256 public constant MINTING_LIMIT = 10000;

    uint256[3] public mintingPrices = [0.001 ether, 0.0015 ether, 0.002 ether];

    uint256 internal devComissionNom = 5;
    uint256 internal devComissionDenom = 100;

    bool private _publicSaleEnabled = false;
    bool public isMarketEnabled = false;

    bool private _isCommonURI = true;
    ...
    constructor(string memory _baseURI) ERC721("MyTestNFT", "MTNFT") {
    ...
    function getMintPrice() public view returns (uint256 price) {
        ...
        uint256 mintingNum = totalSupply() + 1;
        if (mintingNum <= 2000) {
            price = mintingPrices[0];
        } else if (mintingNum >= 2001 && mintingNum <= 3000) {
            price = mintingPrices[1];
        } else {
            price = mintingPrices[2];
        }
    }

    function getNFTPrice(uint256 amount)
        public
        view
        ...
        activePublicSale
    {
        ...
        if (msg.value < nftPrice) {
            revert NOT_ENOUGH_ETC();
        }

        if (amount < 1) revert MINTING_MIN_MINT_AMOUNT();

        if (amount > 20) revert MINTING_MAX_MINT_AMOUNT();

        if ((totalSupply() + amount) > MINTING_LIMIT) {
    ...
    function _beforeTokenTransfer(
        ...
        if (from != to) {
            if (from != address(0)) {
                _removeTokenFromOwnerSet(from, tokenId);
            }
            if (to != address(0)) {
                _addTokenToOwnerSet(to, tokenId);
            }
        }
    }
    ...
    {
        return ownerToTokensSet[user].values();
    }

    function airdrop(address[] memory holders, uint256[] memory counts)
        external
        onlyOwner
    {
        require(holders.length == counts.length, "incorrect length");

        for (uint256 i = 0; i < holders.length; i++) {
            for (uint256 j = 0; j < counts[i]; j++) {
                _airdroppedCounter.increment();
                safeMint(holders[i]);
            }
        }
    }

    function getAirdroppedCount() external view returns (uint256) {
        return _airdroppedCounter.current();
    }
    ...
    function _addTokenToOwnerSet(address _to, uint256 _tokenId)
        private
        notZeroAddress(_to)
    {
    ...
    function _removeTokenFromOwnerSet(address _from, uint256 _tokenId) private {
    ...
    function setIsCommonURI(bool isCommonURI) onlyOwner external returns (bool) {
        _isCommonURI = isCommonURI;
        return _isCommonURI;
    }

    function changeBaseURI(string memory _baseURI) onlyOwner external {
       _setBaseURI(_baseURI);
    }
    ...
    /*************************************************************************** */
    //                             Market:
    function buyFromMarket(uint256 _tokenId, address _to)
        external
        payable
        notZeroAddress(_to)
        marketEnabled
        nonReentrant
    {
        ...
        payable(owner).transfer(_price - devComission);
        emit MarketTrade(_tokenId, owner, _to, msg.sender, _price);

        _transfer(owner, _to, _tokenId);
    }

    function putOnMarket(uint256 _tokenId, uint256 price)
        external
        canOperate(_tokenId)
        marketEnabled
    {
        ...
        marketLots[_tokenId] = MarketLot(_tokenId, true, owner, price);

        tokensOnSale.add(_tokenId);

        emit TokenOnSale(_tokenId, owner, price);
    }

    function changeLotPrice(uint256 _tokenId, uint256 newPrice)
        external
        canOperate(_tokenId)
        marketEnabled
    {
        ...
        emit TokenMarketPriceChange(
            _tokenId,
            ownerOf(_tokenId),
            marketLots[_tokenId].price,
            newPrice
        );
    ...
    function withdrawFromMarket(uint256 _tokenId)
        external
        canOperate(_tokenId)
    {
        _removeFromMarket(_tokenId);

        emit TokenNotOnSale(_tokenId, ownerOf(_tokenId));
    }
    ...
    function getMarketLotInfo(uint256 _tokenId)
        external
        view
        returns (MarketLot memory)
    {
        if (!marketLots[_tokenId].isForSale) {
            revert MARKET_NOT_FOR_SALE();
        }
