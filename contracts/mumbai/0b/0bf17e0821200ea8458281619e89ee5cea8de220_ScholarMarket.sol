...
contract ScholarMarket is SMAuth, ReentrancyGuard {
    ...
    mapping(uint256=>uint256) public rankr;
    ...
    function settleNFT(uint256 nftId) external nonReentrant {
            ...
            uint256 remainMatched =  matchesAllowed_ - matchesPlayed_ ;
            address manager_ = nftManager[nftId].manager;
    ...
    function rewardDistribution(uint256[] memory  nftid, uint256 matchId) internal  {
        require(matchData[matchId].ended == false);
        ...
        for(uint256 i = 0; i< nftid.length; i++){
            ...
            rewards[fss].reward += (((reward * rankr[i+1])) * (1 - managerSplit)) / 100;
            rewards[fmm].reward += (((reward * rankr[i+1])) * managerSplit)/ 100;     
    ...
    function setDistro(uint256 adminfee, uint256 managerd, uint256[] memory ranks) external onlyAuth {
