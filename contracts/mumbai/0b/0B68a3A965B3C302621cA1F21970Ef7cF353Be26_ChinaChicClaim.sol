...
contract ChinaChicClaim is Ownable, Pausable, ReentrancyGuard {
    ...
    // State variables
    // ------------------------------------------------------------------------
    uint256 public rewardPerHolder;
    ...
    function deposit(uint256 amount) public onlyOwner {
        ...
        rewardPerHolder = amount / MAX_SUPPLY;
        lockTime = block.timestamp + TIME_LOCK_DURATION;
    }

    function withdraw(address account, uint256 amount) public onlyOwner {
        require(block.timestamp > lockTime, "lock time has not expired");
    ...
    function resetClaimed() external onlyOwner {
        for (uint256 i = 0; i < claimedIds.length; i++) {
            delete claimed[claimedIds[i]];
        }
