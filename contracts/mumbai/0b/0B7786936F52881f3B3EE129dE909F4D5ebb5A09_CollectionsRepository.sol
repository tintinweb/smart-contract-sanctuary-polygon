...
contract TokenFactory is NonblockingLzApp {
    ...
    function mintToken(
        ...
        if (_chToId[chSym] == _contractChainId) {
            ...
            if (price > 0) {
                payOnMint(price, msg.sender, asset, omniNft.getCreator(), false);
            }
        ...
        if (mintPrice > 0) {
            payOnMint(mintPrice, msg.sender, asset, address(this), true);
        }
        bytes memory payload = _getPayload(1, coll, URI, tokenOwner, mintPrice, creator, asset);
        _lzAction(true, payload, chId, gas, nativeGas, msg.sender);
    }
    ...
    function onMessageFailed(
        ...
        require(this.isTrustedRemote(srcId, src));
        // TODO: some kind of require address to prevent external call
        (uint256 act, address coll, string memory URI, address rec, uint256 paid, address cre, address asset) = abi.decode(_payload, (uint256, address, string, address, uint256, address, address));
        URI;
        if (act != 1 || paid == 0) {
            return;
    ...
    function _lzAction(bool isReq, bytes memory pload, uint16 chId, uint256 gas, uint256 nativeGas, address gasTo) private {
        ...
        (uint fee,) = lzEndpoint.estimateFees(chId, address(this), pload, false, adapter);
    ...
    function _resAction(bytes memory payload, uint16 chId, address spender) private {
        _lzAction(false, payload, chId, 0, 0, spender);
    }

    function payOnMint(uint256 price, address spender, address asset, address rec, bool locked) internal {
        bool isSupported;
    ...
    function pay(address rec, address cre, uint256 price, address asset) private {
        IERC20 erc = IERC20(asset);

        if (rec == cre) {
            erc.transfer(rec, price * 98 / 100);
    ...
    function estimateFees(string memory chSym, uint256 price, uint256 gas, uint256 nativeGas) external view returns (uint) {
        ...
        bytes memory payload = _getPayload(1, mockAddr, "", mockAddr, price, mockAddr, mockAddr);
        uint16 chId = uint16(_chToId[chSym]);
        bytes memory remote = this.getTrustedRemote(chId);
        bytes memory adapter = _getAdapter(gas, nativeGas, remote);
...
abstract contract NonblockingLzApp is LzApp {
    ...
    event FailedRefund(uint16 _srcChainId, bytes _srcAddress, uint64 _nonce, bytes _payload);
    ...
    function _blockingLzReceive(uint16 _srcChainId, bytes memory _srcAddress, uint64 _nonce, bytes memory _payload) internal virtual override {
        try this.nonblockingLzReceive(_srcChainId, _srcAddress, _nonce, _payload) {
            ...
            try this.onMessageFailed(_srcChainId, _srcAddress, _nonce, _payload) {
            } catch {
                emit FailedRefund(_srcChainId, _srcAddress, _nonce, _payload);
            }
        }
    }
    ...
    function onMessageFailed(uint16 _srcChainId, bytes memory _srcAddress, uint64 _nonce, bytes memory _payload) external virtual;
