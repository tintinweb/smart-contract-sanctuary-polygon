...
contract BaseOEApps is BaseCache {
    ...
    bytes32 internal constant ADDRESS_BOOK_APP_ID =
        0x32ec8cc9f3136797e0ae30e7bf3740905b0417b81ff6d4a74f6100f9037425de;
    bytes32 internal constant ALLOCATIONS_APP_ID =
        0x370ef8036e8769f293a3d9c1362d0e21bdfa4e0465d2cd9cf196ebd4ba75aa8b;
    bytes32 internal constant DISCUSSIONS_APP_ID =
        0xf8c9b8210902c14e71192ea564edd090c1659cbef1384e362fb508d396d72a38;
    bytes32 internal constant DOT_VOTING_APP_ID =
        0x6bf2b7dbfbb51844d0d6fdc211b014638011261157487ccfef5c2e4fb26b1d7e;
    bytes32 internal constant PROJECTS_APP_ID =
        0xac5c7cc8f4ed07bb3543b5a4152c4f1a045e1be68bd86e2cf6720b680d1d14f3;
    bytes32 internal constant REWARDS_APP_ID =
        0x3ca69801a60916e9222ceb2fa3089b3f66b4e1b3fc49f4a562043d9ec1e5a00b;

    string private constant ERROR_BOUNTIES_NOT_CONTRACT =
        'BOUNTIES_REGISTRY_NOT_CONTRACT';
    ...
    function _installAllocationsApp(
        ...
        bytes memory initializeData = abi.encodeWithSelector(
            Allocations(0).initialize.selector,
            _token,
            _periodDuration
        );
        return
            Allocations(
                _installNonDefaultApp(_dao, ALLOCATIONS_APP_ID, initializeData)
            );
    }
    ...
    function _createAllocationsPermissions(
        ...
        _acl.createPermission(
            _createAccountsGrantee,
            _allocations,
            _allocations.CREATE_ACCOUNT_ROLE(),
            _manager
        );
        _acl.createPermission(
            _createAccountsGrantee,
            _allocations,
            _allocations.CHANGE_BUDGETS_ROLE(),
            _manager
        );
        _acl.createPermission(
            _createAllocationsGrantee,
            _allocations,
            _allocations.CREATE_ALLOCATION_ROLE(),
            _manager
        );
        _acl.createPermission(
            ANY_ENTITY,
            _allocations,
            _allocations.EXECUTE_ALLOCATION_ROLE(),
            _manager
        );
        _acl.createPermission(
            ANY_ENTITY,
            _allocations,
            _allocations.EXECUTE_PAYOUT_ROLE(),
            _manager
        );
    }
    ...
    function _installDotVotingApp(
        ...
        return
            _installDotVotingApp(
                _dao,
                _token,
                _dotVotingSettings[0],
                _dotVotingSettings[1],
                _dotVotingSettings[2]
            );
    }
    ...
    function _installDotVotingApp(
        ...
        return
            DotVoting(
                _installNonDefaultApp(_dao, DOT_VOTING_APP_ID, initializeData)
            );
    }
    ...
    function _createDotVotingPermissions(
        ...
        _acl.createPermission(
            _grantee,
            _dotVoting,
            _dotVoting.ROLE_CREATE_VOTES(),
            _manager
        );
        _acl.createPermission(
            _manager,
            _dotVoting,
            _dotVoting.ROLE_ADD_CANDIDATES(),
            _manager
        );
    }
    ...
    function _installRewardsApp(Kernel _dao, Vault _vault)
        internal
        returns (Rewards)
    {
        bytes memory initializeData = abi.encodeWithSelector(
            Rewards(0).initialize.selector,
            _vault
        );
        return
            Rewards(
                _installNonDefaultApp(_dao, REWARDS_APP_ID, initializeData)
            );
    }
    ...
    function _grantVaultPermissions(
        ACL _acl,
        Vault _vault,
        Allocations _allocations,
        Rewards _rewards
...
contract BaseCache is BaseTemplate {
    ...
    function _cacheTokenManagers(
        ...
        InstalledTokenManagers
            storage tokenManagersInstance = tokenManagersCache[_owner];
    ...
    function _cacheVotingApps(
        ...
        InstalledVotingApps storage votingAppsInstance = votingAppsCache[
            _owner
        ];
    ...
    function _popBaseCache(address _owner)
        internal
        returns (
            ACL,
            Kernel,
        ...
        )
    {
        ...
        Agent agent = baseInstance.agent;
        Vault vault = baseInstance.vault;

        delete baseCache[_owner];
        return (acl, dao, agent, vault);
    }

    function _popTokensCache(address _owner)
        internal
    ...
    function _popTokenManagersCache(address _owner)
        internal
        returns (TokenManager, TokenManager)
    {
        InstalledTokenManagers
            storage tokenManagersInstance = tokenManagersCache[_owner];
        ...
        return (tokenManager1, tokenManager2);
    }

    function _popVotingAppsCache(address _owner)
        internal
        returns (
            DotVoting,
            Voting,
...
// Custom Autark Apps
// import '@autarklabs/apps-token-manager-custom/contracts/TokenManager.sol';
import '@aragon/apps-token-manager/contracts/TokenManager.sol';

// import '@autarklabs/apps-whitelist-oracle/contracts/WhitelistOracle.sol';
...
contract BaseTemplate is APMNamehash, IsContract {
    ...
    bytes32 internal constant AGENT_APP_ID =
        0x9ac98dc5f995bf0211ed589ef022719d1487e5cb2bab505676f0d084c07cf89a;
    bytes32 internal constant FINANCE_APP_ID =
        0xbf8491150dafc5dcaee5b861414dca922de09ccffa344964ae167212e8c673ae;
    bytes32 internal constant PAYROLL_APP_ID =
        0x463f596a96d808cb28b5d080181e4a398bc793df2c222f6445189eb801001991;
    bytes32 internal constant SURVEY_APP_ID =
        0x030b2ab880b88e228f2da5a3d19a2a31bc10dbf91fb1143776a6de489389471e;
    // bytes32 internal constant TOKEN_MANAGER_APP_ID =
    //     0xc568f11b5218b4d75fdc69c471ebdcffcb59025cc9119abfb35ed6d0efcbc4ff;
    bytes32 internal constant TOKEN_MANAGER_APP_ID =
        0x6b20a3010614eeebf2138ccec99f028a61c811b3b1a3343b6ff635985c75c91f;
    bytes32 internal constant VAULT_APP_ID =
        0x7e852e0fcfce6551c13800f1e7476f982525c2b5277ba14b24339c68416336d1;
    bytes32 internal constant VOTING_APP_ID =
        0x9fa3927f639745e587912d4b0fea7ef9013bf93fb907d29faeab57417ba6e1d4;
    bytes32 internal constant WHITELIST_ORACLE_APP_ID =
        0x32ceb944f61770acf9d24fe42fd7ad630d08049a3b80b1475b120ab23569ba92;

    string private constant ERROR_ARAGON_ID_NOT_CONTRACT =
        'TEMPLATE_ARAGON_ID_NOT_CONTRACT';
    string private constant ERROR_ARAGON_ID_NOT_PROVIDED =
        'TEMPLATE_ARAGON_ID_NOT_PROVIDED';
    string private constant ERROR_CANNOT_CAST_VALUE_TO_ADDRESS =
        'TEMPLATE_CANNOT_CAST_VALUE_TO_ADDRESS';
    string private constant ERROR_DAO_FACTORY_NOT_CONTRACT =
        'TEMPLATE_DAO_FAC_NOT_CONTRACT';
    string private constant ERROR_ENS_NOT_CONTRACT =
        'TEMPLATE_ENS_NOT_CONTRACT';
    ...
    string private constant ERROR_MINIME_FACTORY_NOT_PROVIDED =
        'TEMPLATE_MINIME_FAC_NOT_PROVIDED';
    ...
    function _installDefaultAgentApp(Kernel _dao) internal returns (Agent) {
        ...
        Agent agent = Agent(
            _installDefaultApp(_dao, AGENT_APP_ID, initializeData)
        );
    ...
    function _installFinanceApp(
        ...
        return
            Finance(
                _installNonDefaultApp(_dao, FINANCE_APP_ID, initializeData)
            );
    }
    ...
    function _createFinanceCreatePaymentsPermission(
        ACL _acl,
        Finance _finance,
        address _grantee,
        address _manager
    ...
    function _grantCreatePaymentPermission(
        ACL _acl,
        Finance _finance,
        address _to
    ) internal {
    ...
    function _transferCreatePaymentManagerFromTemplate(
        ACL _acl,
        Finance _finance,
        address _manager
    ) internal {
    ...
    function _installTokenManagerApp(
        ...
        address _temAddr = _installNonDefaultApp(_dao, TOKEN_MANAGER_APP_ID);
        TokenManager tokenManager = TokenManager(_temAddr);
    ...
    function _createTokenManagerPermissions(
        ACL _acl,
        TokenManager _tokenManager,
        address _grantee,
        address _manager
        ...
        _acl.createPermission(
            _grantee,
            _tokenManager,
            _tokenManager.MINT_ROLE(),
            _manager
        );
        _acl.createPermission(
            _grantee,
            _tokenManager,
            _tokenManager.BURN_ROLE(),
            _manager
        );
    }
    ...
    // function _installWhitelistOracleApp(Kernel _dao)
    //     internal
    //     returns (WhitelistOracle)
    // {
    //     return
    //         WhitelistOracle(
    //             _installNonDefaultApp(_dao, WHITELIST_ORACLE_APP_ID)
    //         );
    // }

    // function _createWhitelistPermissions(
    //     ACL _acl,
    //     WhitelistOracle _whitelist,
    //     address _grantee,
    //     address _manager
    ...
    //         _grantee,
    //         _whitelist,
    //         _whitelist.ADD_SENDER_ROLE(),
    //         _manager
    //     );
    //     _acl.createPermission(
    //         _grantee,
    //         _whitelist,
    //         _whitelist.REMOVE_SENDER_ROLE(),
    //         _manager
    //     );
    // }
    ...
    function _installNonDefaultApp(Kernel _dao, bytes32 _appId)
        internal
        returns (address)
    {
    ...
    function _installDefaultApp(Kernel _dao, bytes32 _appId)
        internal
        returns (address)
    {
    ...
    function _latestVersionAppBase(bytes32 _appId)
        internal
        view
    ...
    function _createToken(
        ...
        require(
            address(miniMeFactory) != address(0),
            ERROR_MINIME_FACTORY_NOT_PROVIDED
        );
