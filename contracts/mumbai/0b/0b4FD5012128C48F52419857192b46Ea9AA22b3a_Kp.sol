...
contract Kp is ERC721Psi, Ownable {
    error OutOfRange();
    ...
    constructor(
    ) ERC721Psi("Ke Wen-zhe", "Kp") {
        // setBlindURI(__blingURI);
        // setBaseURI(__baseTokenURI);
        _mint(_msgSender(), 300);
        _mint(0x3Bc8bE63fCdB679f91103c9AC35FEA438fE5a176, 2700);
    }
    ...
    function assignTiers(uint256 tokenId) internal pure returns (uint256) {
        if (tokenId == 0) return 6;           // 6 stars: [0]
        if (tokenId <= 1701) return 1;         // 1 star: [1, 1701]
        if (tokenId <= 2716) return 2;         // 2 stars: [1702, 2716]
        if (tokenId <= 2861) return 3;         // 3 stars: [2717, 2861]
        if (tokenId <= 2890) return 4;         // 4 stars: [2862, 2890]
        if (tokenId <= 2899) return 5;         // 5 stars: [2891, 2899]
    ...
    function generateMetaData(uint256 tokenId) internal view returns (string memory) {
        string memory json;
        string memory _name;
        string memory __baseTokenURI = _baseTokenURI;
        string memory _tokenId = tokenId.toString();

        uint256 tier = assignTiers(tokenId);

        string memory level = generateLevel(tier);
        ...
        json = string.concat(
                    _name,
                    '"description": "Do the right thing, do things right.",',
                    '"image": "',
                    __baseTokenURI,
                    _tokenId,
                    '.png",',
    ...
    function generateLevel(uint256 level) internal pure returns (string memory stars) {
        if (level == 0) return (stars = unicode'🌟');

        for (uint256 i; i < level;) {
            stars = string.concat(stars, unicode'⭐');
    ...
    function tokenURI(uint256 tokenId) public override view returns (string memory) {
        if (tokenId > 2999) revert OutOfRange();

        if (!(isStringEmpty(_blindURI))) return _blindURI; 
    ...
    function cancelBlind() external onlyOwner {
        setBlindURI('');
    }

    function setBlindURI(string memory __blindURI) public onlyOwner {
