...
contract TrashPandazClubSteakingV2 is IERC721Receiver, ReentrancyGuard, Ownable, Pausable {
    ...
    function rewardMath(address account, uint256 stakedAt) internal view returns(uint256){
        if (rewardDividedByPool == true){
            return (( rewardRate * (getMultiplier(account) / 100) * 1 ether * (block.timestamp - stakedAt) / 86400 ) ) / totalStaked; 
        }
        else {
            return ( rewardRate * (getMultiplier(account) / 100) * 1 ether * (block.timestamp - stakedAt) / 86400 ) ; 
        }
    ...
    function _claim(address account, uint256[] calldata tokenIds, bool _unstake) internal nonReentrant{
        ...
        earned = rewardMath(account, stakedAt); 
        vault[tokenId] = Stake({
            owner: account,
            tokenId: uint24(tokenId),
            timestamp: uint48(block.timestamp)
