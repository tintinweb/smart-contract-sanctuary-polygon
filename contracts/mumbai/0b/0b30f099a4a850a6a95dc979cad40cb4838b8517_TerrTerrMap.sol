...
import "./extensions/TerrTerrMapLogic.sol";
...
///    ╚═╝   ╚══════╝╚═╝  ╚═╝╚═╝  ╚═╝       ╚═╝   ╚══════╝╚═╝  ╚═╝╚═╝  ╚═╝
// @author nezzar.eth
// @notice This contract is used to manage holder's position & travel.
contract TerrTerrMap is TerrTerrMapMapLogic, ERC1155 {
    event Move(
    ...
    constructor(
        ...
        uint16 step = (uint16(180) * multiplicator) / numberOfTiles;
...
contract TerrTerrMapState {
    string internal _name;
    mapping(address => uint256) internal userCoordinates;

    uint256 internal constant LOCATION_TOKEN_MASK =
    ...
    uint8 internal maxSizeOfCoordinate = 16; // uint
...
import {TerrTerrMapState} from "../TerrTerrMapState.sol";

import {Coordinate} from "../libs/Structs.sol";

/**
 * @title TerrTerrMapLogic
 * @author nezzar.eth
 * @notice This contract holds the logic to encode/decode coordinates & locationIDs.
 */
contract TerrTerrMapMapLogic is TerrTerrMapState {
    /**
    ...
    /**
     * @dev Check if user is in area
     */
    function isLocationID(uint256 locationId) internal pure returns (bool) {
        return (locationId & LOCATION_TOKEN_MASK) == locationId;
    }
}
...
struct TerrTerrMapFollowConfig {
    address mapAddress;
    address followingAddress;
}

struct TerrTerMapCollectConfig {
    address mapAddress;
    uint256 locationId;
}