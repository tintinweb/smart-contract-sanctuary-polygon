...
import "../packages/@openzeppelin/contracts/token/ERC1155/ERC1155.sol";
...
contract Marketplace is ERC1155 {
    ...
    function createProduct(string memory name, uint256 price) public {
        ...
        uint256 productId = _productIdTracker.current();
    ...
    function buyProduct(uint256 productId) public payable {
        ...
        require(msg.value >= products[productId].price, "Insufficient funds");

        _mint(msg.sender, productId, 1, "");

        // Refund excess payment
        if (msg.value > products[productId].price) {
            payable(msg.sender).transfer(msg.value - products[productId].price);
        }
    }
}
