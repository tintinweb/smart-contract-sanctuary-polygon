...
// File: contracts/WhoopEAClaimTestnet.sol
...
contract WhoopEARoyaltiesEngine is Ownable, Initializable {
    ...
    function claim() public {
        ...
        if (previous != 0) {
            require(previous + 5 minutes < block.timestamp, "Wait for Cooldown");
            claimablePerNFT = ((block.timestamp - previous) / 2 minutes) * 5;
        } else {
            claimablePerNFT = ((block.timestamp - initialTime) / 2 minutes) * 5;
        }
    ...
    function getClaimable(address user) public view returns (uint256) {
        ...
        if (pTime != 0) claimablePerNFT = ((block.timestamp - pTime) / 2 minutes) * 5;
        else claimablePerNFT = ((block.timestamp - initialTime) / 2 minutes) * 5;
