...
/**
 ...
 NFT Contract for Disappointing DAD ($DAD) token
*/
...
contract DisappointingDADNFT is ERC721A, AccessLock {
    ...
    function mintByTopHolders()
        external
    {
        require(topHolders[msg.sender], "Can only be used by promoters!");
    ...
    function mint(uint256 _amount)
        ...
        payable
    {
        require(balanceOf(msg.sender) <= 50, "One wallet can't mint more than 50 NFTs");
        require(_amount <= 10, "Can't mint more than 10 at one go");
        require(msg.value == mintPrice * _amount, "Must pay mint fee!");

        if(whitelistOpen) {
            require(totalSupply() + _amount <= 600, "Limited to 600 max");
        } else {
    ...
    function _toggleWhitelist(bool _status) external onlyOwner {
        whitelistOpen = _status;
    }
