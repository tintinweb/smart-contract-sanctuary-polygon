...
import "./ITeleOrdinalStorage.sol";
import "@teleportdao/btc-evm-bridge/contracts/types/ScriptTypesEnum.sol";

interface ITeleOrdinalLogic is ITeleOrdinalStorage {
	...
	function listOrdinal(
        ...
        TeleOrdinalLib.Tx calldata _tx,
        TeleOrdinalLib.Loc calldata _loc,
        string calldata inscriptionId
	) external returns (bool);

    function delistOrdinal(TeleOrdinalLib.Loc calldata _loc) external returns (bool);
    ...
    function sellOrdinal(
        TeleOrdinalLib.Loc calldata _loc,
        uint _bidIdx,
        TeleOrdinalLib.Tx memory _transferTx,
        TeleOrdinalLib.Loc memory _newLoc,
        TeleOrdinalLib.MerkleProof memory _merkleProof,
        TeleOrdinalLib.Tx[] memory _inputTxs,
        bytes memory _lockerLockingScript
    ) external payable returns (bool);
...
library TeleOrdinalLib {
    ...
    function listOrdinalHelper(
        ...
        require(!ordinals[_loc.txId][_loc.outputIdx][_loc.satoshiIdx].isListed, "TeleOrdinal: already listed");
        ...
        if (_isSignRequired) {
            _checkSignature(_scriptType, _scriptHash, _loc.txId, _signature);
        }
...
import "./interfaces/ITeleOrdinalLogic.sol";
import "./TeleOrdinalStorage.sol";
...
contract TeleOrdinalLogic is ITeleOrdinalLogic, TeleOrdinalStorage,
    ...
    /// @param _loc Location of the listing Ordinal (txId, outputIdx, satoshiIdx)
    ...
    function listOrdinal(
        ...
        // Owner can list any Ordinal
        bool _isSignRequired = _msgSender() == owner() ? false : isSignRequired;
        
        bytes memory scriptHash = TeleOrdinalLib.listOrdinalHelper(
            _scriptType,
            _signature,
            _tx,
            _loc,
    ...
    /// @param _loc Location of the delisting Ordinal (txId, outputIdx, satoshiIdx)
    function delistOrdinal(
        TeleOrdinalLib.Loc calldata _loc
    ) external nonReentrant override returns (bool) {

        // Owner can delist any Ordinal
        address _seller = 
            _msgSender() == owner() ? ordinals[_loc.txId][_loc.outputIdx][_loc.satoshiIdx].seller : _msgSender();

        TeleOrdinalLib.delistOrdinalHelper(
            _loc,
            ordinals,
            _seller
        );
        ...
        // Revokes all bids
        for (uint i = 0; i < bids[_loc.txId][_loc.outputIdx][_loc.satoshiIdx].length; i++) {
            if (bids[_loc.txId][_loc.outputIdx][_loc.satoshiIdx][i].buyerETHAddress != address(0)) { 
                // ^ If the bid is not empty
                emit BidRevoked(
                    _loc.txId, 
                    _loc.outputIdx, 
                    _loc.satoshiIdx,
                    ordinals[_loc.txId][_loc.outputIdx][_loc.satoshiIdx].seller,
                    bids[_loc.txId][_loc.outputIdx][_loc.satoshiIdx][i].buyerETHAddress,
                    i
                );
                _removeBid(
                    _loc,
        ...
        delete ordinals[_loc.txId][_loc.outputIdx][_loc.satoshiIdx];
    ...
    function putBid(
        ...
        TeleOrdinalLib.putBidHelper(_loc, _buyerBTCScript, _scriptType, ordinals);

        // Stores bid
        TeleOrdinalLib.Bid memory _bid;
        ...
        bids[_loc.txId][_loc.outputIdx][_loc.satoshiIdx].push(_bid);
        uint bidIdx = bids[_loc.txId][_loc.outputIdx][_loc.satoshiIdx].length - 1;
    ...
    function increaseBid(
        ...
        TeleOrdinalLib.increaseBidHelper(
            _loc,
            _bidIdx,
            _newAmount,
            ordinals,
        ...
        uint bidDifference = _newAmount - bids[_loc.txId][_loc.outputIdx][_loc.satoshiIdx][_bidIdx].bidAmount;
        address paymentToken = bids[_loc.txId][_loc.outputIdx][_loc.satoshiIdx][_bidIdx].paymentToken;
        ...
        bids[_loc.txId][_loc.outputIdx][_loc.satoshiIdx][_bidIdx].bidAmount = _newAmount;
    ...
    function cancelBid(
        ...
        require(
            bids[_loc.txId][_loc.outputIdx][_loc.satoshiIdx][_bidIdx].buyerETHAddress == _msgSender(),
            "TeleOrdinal: not owner"
        );

        // Handles the case where the seller accepted a bid 
        // but didn't transfer Ordinal to the buyer before the deadline
        if (bids[_loc.txId][_loc.outputIdx][_loc.satoshiIdx][_bidIdx].isAccepted) {
            // check that deadline is passed but ordinal hasn't been transferred
            require(!ordinals[_loc.txId][_loc.outputIdx][_loc.satoshiIdx].isSold, "TeleOrdinal: ordinal sold");
            require(
                IBitcoinRelay(relay).lastSubmittedHeight() 
                    > bids[_loc.txId][_loc.outputIdx][_loc.satoshiIdx][_bidIdx].deadline,
                "TeleOrdinal: deadline not passed"
            );
            // Changes the status of the Ordinal (so seller can accept a new bid)
            ordinals[_loc.txId][_loc.outputIdx][_loc.satoshiIdx].hasAccepted = false;
        }
        ...
        emit BidCanceled(
            ...
            _loc.satoshiIdx,
            ordinals[_loc.txId][_loc.outputIdx][_loc.satoshiIdx].seller,
            bids[_loc.txId][_loc.outputIdx][_loc.satoshiIdx][_bidIdx].buyerETHAddress,
            _bidIdx
        );
        _removeBid(_loc, _msgSender(), _bidIdx);
    ...
    function acceptBid(
        ...
        require(ordinals[_loc.txId][_loc.outputIdx][_loc.satoshiIdx].seller == _msgSender(), "TeleOrdinal: not owner");
        require(!ordinals[_loc.txId][_loc.outputIdx][_loc.satoshiIdx].hasAccepted, "TeleOrdinal: already accepted");
        require(bids[_loc.txId][_loc.outputIdx][_loc.satoshiIdx].length > _bidIdx, "TeleOrdinal: invalid idx");  

        // seller has a limited time to send the Ordinal and provide a proof for it to get it
        ordinals[_loc.txId][_loc.outputIdx][_loc.satoshiIdx].hasAccepted = true;
        bids[_loc.txId][_loc.outputIdx][_loc.satoshiIdx][_bidIdx].isAccepted = true;
        bids[_loc.txId][_loc.outputIdx][_loc.satoshiIdx][_bidIdx].deadline = 
            IBitcoinRelay(relay).lastSubmittedHeight() + transferDeadline;
    ...
    /// @notice Sends funds to seller after checking the proof of transfer
    /// @param _loc Old location of the selling Ordinal (txId, outputIdx, satoshiIdx)
    /// @param _bidIdx Index of the accepted bid in bids list
    ...
    function sellOrdinal(
        ...
        require(!ordinals[_loc.txId][_loc.outputIdx][_loc.satoshiIdx].isSold, "TeleOrdinal: sold ordinal");
        require(bids[_loc.txId][_loc.outputIdx][_loc.satoshiIdx][_bidIdx].isAccepted, "TeleOrdinal: not accepted");

        // Checks inclusion of transfer tx
        require(
            _newLoc.txId == BitcoinHelper.calculateTxId(
                _transferTx.version, 
                _transferTx.vin, 
                _transferTx.vout, 
        ...
        require(
            _isConfirmed(
                _newLoc.txId,
                _merkleProof.blockNumber,
                _merkleProof.intermediateNodes,
                _merkleProof.index
            ),
            "TeleOrdinal: not finalized"
        );
        
        // Finds the index of Ordinal satoshi
        require(
            _newLoc.satoshiIdx == TeleOrdinalLib.ordinalIdx(_loc,  _transferTx.vin, _inputTxs),
            "TeleOrdinal: wrong satoshiIdx"
        );
        
        // Checks that whether the Ordinal is transferred to the buyer or not
        TeleOrdinalLib.checkOrdinalTransfer(
            _loc,
            _bidIdx, 
            _transferTx.vout, 
            _newLoc.outputIdx, 
        ...
        ordinals[_loc.txId][_loc.outputIdx][_loc.satoshiIdx].isSold = true;

        uint fee = _sendTokens(_loc, _bidIdx, _lockerLockingScript);
    ...
    function _removeBid(
        ...
        if (bids[_loc.txId][_loc.outputIdx][_loc.satoshiIdx][_bidIdx].paymentToken == NATIVE_TOKEN) {
            // Sends ETH to buyer
            Address.sendValue(payable(_buyer), bids[_loc.txId][_loc.outputIdx][_loc.satoshiIdx][_bidIdx].bidAmount);
        } else {
            IERC20(bids[_loc.txId][_loc.outputIdx][_loc.satoshiIdx][_bidIdx].paymentToken).transfer(
                _buyer,
                bids[_loc.txId][_loc.outputIdx][_loc.satoshiIdx][_bidIdx].bidAmount
            );
        }

        // Deletes the bid
        delete bids[_loc.txId][_loc.outputIdx][_loc.satoshiIdx][_bidIdx];
    }
    ...
    function _sendTokens(
        ...
        address paymentToken = bids[_loc.txId][_loc.outputIdx][_loc.satoshiIdx][_bidIdx].paymentToken;
        uint bidAmount = bids[_loc.txId][_loc.outputIdx][_loc.satoshiIdx][_bidIdx].bidAmount;
        _fee = protocolFee * bidAmount / MAX_PROTOCOL_FEE;
        
        if (paymentToken == NATIVE_TOKEN) {
            Address.sendValue(payable(ordinals[_loc.txId][_loc.outputIdx][_loc.satoshiIdx].seller), bidAmount - _fee);
            if (_fee > 0) {
            ...
            ICCBurnRouter(ccBurnRouter).ccBurn(
                bidAmount - _fee,
                ordinals[_loc.txId][_loc.outputIdx][_loc.satoshiIdx].sellerScript,
                ordinals[_loc.txId][_loc.outputIdx][_loc.satoshiIdx].scriptType,
...
import "./interfaces/ITeleOrdinalStorage.sol";

contract TeleOrdinalStorage is ITeleOrdinalStorage {
    ...
    mapping(bytes32 => mapping(uint => mapping(uint => TeleOrdinalLib.Ordinal))) public ordinals;
    // ^ Mapping from [txId][outputIdx][satoshiIdx] to a listed Ordinal
    mapping(bytes32 => mapping(uint => mapping(uint => TeleOrdinalLib.Bid[]))) public bids; 
    // ^ Mapping from [txId][outputIdx][satoshiIdx] to bids (note: it wasn't possible to define Bid[] in Ordinal)
