...
import "./TicketNumberGovernor.sol";
import "./Vault.sol";

contract Doremon is VRFConsumerBaseV2 {
    struct Ticket {
        uint16 num1;
        uint16 num2;
        uint16 num3;
    ...
    uint256 public immutable poolCapacity = 5;
    ...
    uint256[][] public gameWinningNumbers;
    IERC20 public immutable stableCoin;
    BobTheBuilder public ticketNumberGovernor;
    RichieRich public vault;
    ...
    constructor(address _stableCoin, uint256 _ticketPrice, uint64 subscriptionId, address vrfContractAddress, bytes32 _keyHash, address _ticketNumberGovernor, address _vaultAddress) VRFConsumerBaseV2(vrfContractAddress) {    
        ticketPrice = _ticketPrice;
        ticketNumberGovernor = BobTheBuilder(_ticketNumberGovernor);
        vault = RichieRich(_vaultAddress);
        keyHash = _keyHash;
        jackpot = ticketPrice * poolCapacity;
        games.push(Game(0, block.timestamp, 0, Status.Open));
        gameCount++;
        gameMembers.push(new address[](0));
        gameWinners.push(new address[](0));
        gameTickets.push(new uint256[](0));
        gameWinningNumbers.push(new uint256[](0));
    ...
    function buyTickets(uint256 _referralCode, uint8 _v, bytes32 _r, bytes32 _s) external returns(Ticket memory) {
        
        bool hasSignedConsent = RichieRich(vault).hasSignedConsent(msg.sender);
        if(!hasSignedConsent){
            RichieRich(vault).verifyMessage(msg.sender, _v, _r, _s);
        }
        uint256 gameIndex = games.length - 1;
        Game storage game = games[games.length - 1];
        require(game.status == Status.Open, "Game is closed");
        require(game.membersFilled < poolCapacity, "Game is full");
        uint16 numOfTicketsOfBuyer = 0;
        for(uint i=0; i<gameMembers[gameIndex].length; i++){
            if(gameMembers[gameIndex][i] == msg.sender){
                numOfTicketsOfBuyer++;
                if(numOfTicketsOfBuyer == 5){
                    revert("You can buy only 5 tickets");
                }
            }
        }
        if(_referralCode != 0){
            RichieRich(vault).addMemberToPool(msg.sender, _referralCode);    
        }
        SafeERC20.safeTransferFrom(stableCoin, msg.sender, address(vault), ticketPrice);
        RichieRich(vault).generateReferralCode(msg.sender);
        
        gameMembers[gameIndex].push(address(msg.sender));
        (uint16 num1, uint16 num2, uint16 num3, uint16 num4, uint16 num5, uint16 num6) = ticketNumberGovernor.getTicket();
        ticketCount++;
        tickets[ticketCount] = Ticket(num1, num2, num3, num4, num5, num6);
        gameTickets[gameIndex].push(ticketCount);
        game.membersFilled++;
    ...
    function batchBuyTickets(uint256 _numberOfTickets, uint256 _referralCode, uint8 _v, bytes32 _r, bytes32 _s) external returns(Ticket[] memory) {
        bool hasSignedConsent = RichieRich(vault).hasSignedConsent(msg.sender);
        if(!hasSignedConsent){
            RichieRich(vault).verifyMessage(msg.sender, _v, _r, _s);
        }
    ...
    function fulfillRandomWords(
        ...
        totalWinnningsOfUser[gameMembers[gameId][winner1]] += winningAmounts[0];
        totalWinnningsOfUser[gameMembers[gameId][winner2]] += winningAmounts[1];
        totalWinnningsOfUser[gameMembers[gameId][winner3]] += winningAmounts[2];
        gameWinners[gameId].push(gameMembers[gameId][winner1]);
        gameWinners[gameId].push(gameMembers[gameId][winner2]);
        gameWinners[gameId].push(gameMembers[gameId][winner3]);
        vault.distributePoolPrize(gameWinners[gameId], winningAmounts);
        gameWinningNumbers[gameId].push(winner1);
        gameWinningNumbers[gameId].push(winner2);
        gameWinningNumbers[gameId].push(winner3);

        emit WinnerAnnounced(gameWinners[gameId], gameId, winningAmounts);
    }

    function getMyTicketIds(address _user, uint256 _gameId) public view returns(uint256[] memory) {
        uint256 myTickets = 0;
        for(uint256 i = 0; i < gameTickets[_gameId].length; i++) {
...
contract RichieRich is VRFConsumerBaseV2 {
    IERC20 public immutable stableCoin;
    ...
    bytes32 public hashedMessage = 0x6fc646d63eca4fe73be6b74dd5ad4fb733a90db35e5b3104bb1e053f49052856;
    ...
    uint256 private referralCodeCounter = 111111;
    uint256 public withdrawFeePercent = 2000000;
    ...
    event ReferralWinnerAnnounced(address[] winners, uint256 batch);
    ...
    constructor(
        ...
        s_subscriptionId = subscriptionId;
        poolMembers.push(new address[](0));
        for(uint256 i=0;i<totalReferralWinners;i++){
            ...
            poolMembers[0].push(randomAddress);            
        }
        poolMembers.push(new address[](0));
        totalBatches++;
        hasSignedConsent[msg.sender] = true;
        hasSignedConsent[0xdCdFae782d8429A75C345ac4C920e8dc605afD80] = true;
    }
    ...
    function distributePoolPrize(
        ...
        uint256 jackpot = Doremon(msg.sender).jackpot();
    ...
    function addMemberToPool(address _address, uint256 _referralCode) public {
        require(isWhitelisted[msg.sender], "Unsupported Contract");
        require(referrer[_address] == address(0), "Referrer already set");
        require(
            referralCodeToAddress[_referralCode] != address(0),
            "Invalid referral code"
        );
        require(
            referralCode[_address] == 0,
            "User's referral code already set"
        );
        referrer[_address] = referralCodeToAddress[_referralCode];
        totalReferred[referralCodeToAddress[_referralCode]] += 1;
        ...
        poolMembers[totalBatches].push(_address);
        if (poolMembers[totalBatches].length == totalReferralWinners) {
            totalBatches++;
            poolMembers.push(new address[](0));
        }
    ...
    function generateReferralCode(address _address) public {
        require(isWhitelisted[msg.sender], "Only for whitelisted contracts");
        if (referralCode[_address] == 0) {
            referralCode[_address] = referralCodeCounter;
            referralCodeToAddress[referralCodeCounter] = _address;
            referralCodeCounter++;
        }
    }

    function verifyMessage(address _user, uint8 _v, bytes32 _r, bytes32 _s) public {
        require(isWhitelisted[msg.sender], "Unsupported Contract");
        bytes memory prefix = "\x19Ethereum Signed Message:\n32";
        bytes32 prefixedHashMessage = keccak256(abi.encodePacked(prefix, hashedMessage));
        address signer = ecrecover(prefixedHashMessage, _v, _r, _s);
        require(signer == _user, "Invalid Messsage");
    ...
    function fulfillRandomWords(
        ...
        gameToWinningBatch[gameId] = poolMembers[randomNumber];
        ...
        emit ReferralWinnerAnnounced(poolMembers[randomNumber], randomNumber);
        poolMembers[randomNumber] = poolMembers[poolMembers.length-2];
        poolMembers[poolMembers.length-2] = poolMembers[poolMembers.length-1];
        poolMembers.pop();
        totalBatches--;
    }
