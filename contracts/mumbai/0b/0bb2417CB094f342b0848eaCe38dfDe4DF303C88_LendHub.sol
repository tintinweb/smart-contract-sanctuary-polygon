...
contract LendHub is ReentrancyGuard, AutomationCompatibleInterface, Ownable {
    ...
    InterestRateInfo public interestRateInfo =
        InterestRateInfo({
            ...
            slope1: 8000, // 0.08 in fixed point format
            slope2: 100000 // 1 in fixed point format
        });
    ...
    function supply(
        ...
        // uint256 tokenValue = getTokenPrice(tokenAddress);
    ...
    // account           -> address of user to be liquidated
    // collateral        -> supply token the user kept before borrowing
    // userBorrowToken   -> token the user borrowed after keeping the collateral
    // amountToLiquidate -> the amount that needs to be lquidated
    function Liquidate(
        address account,
        address collateral,
        ...
        // account is the user whose supplied funds are about to be liquidated
        // collateral is the address of the token he supplied during registration
        // userBorrowToken is the address of the token that he borrowed
        // amountToLiquidate
        if (healthFactor(account) >= MIN_HEALTH_FACTOR) {
            revert BorrowerIsSolvant();
        }

        uint256 collateralShares = LH_token_User_SupplyBalance[collateral][account];
        uint256 totalBorrowAmount = LH_token_User_BorrowBalance[userBorrowToken][account];

        if (collateralShares == 0 || totalBorrowAmount == 0) {
            revert InvalidLiquidation();
        }
        // the next 2 lines just makes sure that more than 50% of the total pool amount is not liquiqated..
        //i.e whichever is minimum of the 2 is to be liquidated
        uint256 maxBorrowAmountToLiquidate = (totalBorrowAmount * LIQUIDATION_CLOSE_FACTOR) / 100;
        amountToLiquidate = amountToLiquidate > maxBorrowAmountToLiquidate
            ? maxBorrowAmountToLiquidate
            : amountToLiquidate;

        uint256 _userCollateralBalance = getUserTotalCollateral(account);

        uint256 collateralPrice = getTokenPrice(collateral);
        uint256 borrowTokenPrice = getTokenPrice(userBorrowToken);

        uint256 collateralAmountToLiquidate = (amountToLiquidate * borrowTokenPrice) /
            collateralPrice;
        uint256 maxLiquidationReward = (collateralAmountToLiquidate * LIQUIDATION_REWARD) / 100;

        uint256 liquidationReward;
        if (collateralAmountToLiquidate > _userCollateralBalance) {
            collateralAmountToLiquidate = _userCollateralBalance;
            amountToLiquidate = (_userCollateralBalance * collateralPrice) / borrowTokenPrice;
        } else {
            uint256 collateralBalanceAfter = _userCollateralBalance - collateralAmountToLiquidate;
            liquidationReward = maxLiquidationReward > collateralBalanceAfter
                ? collateralBalanceAfter
                : maxLiquidationReward;
        }
        // Update borrow vault
        LH_token_User_BorrowBalance[userBorrowToken][account] -= amountToLiquidate;

        // Update collateral vault
        LH_token_User_SupplyBalance[collateral][account] -= (collateralAmountToLiquidate +
            liquidationReward);

        // Repay borrowed amount
        transferTokens(msg.sender, address(this), userBorrowToken, amountToLiquidate);
    ...
    function checkUpkeep(
        ...
        // //todo
        // if(LH_borrowers.length > 0) return (true, "0x0");
    ...
    function performUpkeep(bytes calldata /* performData */) external override {
        ...
        //         continue;
        //     }
        //     totalCollateralAmount = getUserTotalCollateral(LH_borrowers[j]);
        //     for (uint i = 0; i < LH_tokens_allowed.length; i++) {
        //         if (healthFactor(LH_borrowers[j]) >= MIN_HEALTH_FACTOR) {
        //             break;
        //         }
        //         if (LH_token_User_BorrowBalance[LH_tokens_allowed[i]][LH_borrowers[j]] == 0)
        //             continue;
        //         for (uint k = 0; k < LH_tokens_allowed.length; k++) {
        //             if (LH_token_User_SupplyBalance[LH_tokens_allowed[k]][LH_borrowers[j]] > 0) {
        //                 Liquidate(
        //                     LH_borrowers[j],
        //                     LH_tokens_allowed[k],
        //                     LH_tokens_allowed[i],
    ...
    function _calculateInterestRate(address token) internal {
        ...
        // if (LH_SupplyPool[token].totalborrow == 0) {
        ...
        // }
            ...
            uint256 _newRate = InterestRate.calculateInterestRate(
                ...
                LH_SupplyPool[token].amount,
                LH_SupplyPool[token].totalborrow
            );
    ...
    // function reducesupply(address user) external {
    //     for (uint64 i = 0; i < LH_tokens_allowed.length; i++) {
    //         if (LH_token_User_SupplyBalance[LH_tokens_allowed[i]][user] > 0) {
    //             LH_token_User_SupplyBalance[LH_tokens_allowed[i]][user] =
    //                 (LH_token_User_SupplyBalance[LH_tokens_allowed[i]][user] * 3) /
    //                 5;
    //         }
    //     }
    // }
