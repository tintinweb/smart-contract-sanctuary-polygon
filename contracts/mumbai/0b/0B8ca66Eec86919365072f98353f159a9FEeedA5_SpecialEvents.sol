...
contract SpecialEvents {
    ...
    mapping(uint256 => bool) public availableLands;                          //  whitelist of lands that is available to be purchased
    ...
    */
    function setAvailableLands(int256[] calldata _longitudes, int256[] calldata _latitudes, bool _setOpt) external onlyManager {
        uint256 _len = _longitudes.length;
        if (_latitudes.length != _len) revert LengthMismatch();

        uint256[] memory _tokenIds = conversion.composeBatch(_longitudes, _latitudes);
        if (_setOpt){
            for(uint256 i; i < _len; i++)
                availableLands[_tokenIds[i]] = true;
        }
        else {
            for(uint256 i; i < _len; i++)
                delete availableLands[_tokenIds[i]];
        }  
    }
    ...
    */
    function fulfillment(
        ...
        string calldata _receipt
    ) external onlyAuthorizer {
        if (events[_eventId].eventType == 1) revert InvalidRequest();

        uint256 _amount = _longitudes.length;
        _precheck(_eventId, _amount, _latitudes.length, _uris.length);
        ...
        emit Fulfillment(_eventId, msg.sender, _beneficiary, _longitudes, _latitudes, _tokenIds, _receipt);
    }
    ...
    */
    function purchase(
        ...
        //  - `msg.sender` must be in the whitelist
        //      - if `eventId` not existed -> revert
        //      - if `eventId` existed but `msg.sender` not in the whitelist for this event -> revert
        //  - event schedule:
        //      - event not yet started -> revert
        //      - event already ended -> revert
        //  - `paymentToken` must be acceptable (check price != 0)
        //  - length of `longitudes` and `latitudes` must be matched
        uint256 _amount = _longitudes.length;
        _precheck(_eventId, _amount, _latitudes.length, _uris.length);
        ...
        emit Purchased(_eventId, msg.sender, _paymentToken, _amount, _paymentAmt, _longitudes, _latitudes, _tokenIds);
    }
    ...
    */
    function airdrop(
        ...
        if (_longitudes.length != _amount) revert LengthMismatch();
        _precheck(_eventId, _amount, _latitudes.length, _uris.length);
        ...
        emit AirDrop(_eventId, _beneficiaries, _longitudes, _latitudes, _tokenIds);
    }

    function _getTokenIds(int256[] calldata _longitudes, int256[] calldata _latitudes) private returns (uint256[] memory _tokenIds) {
        uint256 _len = _longitudes.length;
        _tokenIds = new uint256[](_len);
        for (uint256 i; i < _len; i++) {
            //  compose `longitude` and `latitude` to acquire `_tokenId`
            //  then check whether `_tokenId` is available for sale
            _tokenIds[i] = conversion.compose(_longitudes[i], _latitudes[i]);
            if (!availableLands[_tokenIds[i]]) revert NotAvailForSale();

            delete availableLands[_tokenIds[i]];
        }
    }

    function _precheck(uint256 _eventId, uint256 _len1, uint256 _len2, uint256 _len3) private view {
        uint256 _currentTime = block.timestamp;
        if (_currentTime < events[_eventId].start || _currentTime > events[_eventId].end) 
            revert NotStartOrEnded();
        if (_len1 != _len2 || _len1 != _len3) revert LengthMismatch();
    }
