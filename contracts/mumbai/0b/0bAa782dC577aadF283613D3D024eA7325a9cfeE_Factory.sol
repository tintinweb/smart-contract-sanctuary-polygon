...
contract NFT is ERC721URIStorage, EIP712 {
    // Roles
    ...
    mapping (address => bool) private mintRole;
    mapping (address => bool) private validatorRole;
    ...
    // modifier
    modifier minterRole() {
        require(mintRole[_msgSender()] == true, "0x00");
        _;
    }

    modifier onlyOwner() {
        require(adminControl == msg.sender, "0x00");
        _;
    }
    ...
    constructor(
        ...
        adminControl = _roleAddress;
        grantMinterRole(_roleAddress);
        grantValidatorRole(_roleAddress);
    }
    ...
    function grantMinterRole(address _account) public onlyOwner {
        if (!mintRole[_account]) {
            mintRole[_account] = true;
            emit MinterRoleGranted(_account);
        }
    }

    function grantValidatorRole(address _account) public onlyOwner {
        if (!validatorRole[_account]) {
            validatorRole[_account] = true;
            emit MinterRoleGranted(_account);
        }
    }
