...
contract IdentityNftFactory is OwnablePausable {
    ...
    function mint(uint256 index, address[] memory to) external {
        uint256 len = to.length;
        require(len <= 30, "size too much");
        require(operator() == msg.sender || _rewardMinter == msg.sender, "mint not allowed");
        require(index == 998 || index == 999, "not identity nft");
        for (uint256 i = 0; i < len; i++) {
            identityErc721Tokens[index].mint(to[i], ++id);
        }
    }

    // function mintBatch(uint256 index, address to, uint256 number) external onlyOperator {
    //     require(index == 998 || index == 999, "not identity nft");
    ...
    function setRewardMinter(address minter) external onlyDeployer {
