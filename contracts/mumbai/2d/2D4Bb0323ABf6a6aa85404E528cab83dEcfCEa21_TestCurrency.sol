...
contract TestCurrency is ERC20Upgradeable, FiboEIP712, OwnableUpgradeable {
    ...
    function initialize() public initializer {
        __Ownable_init();
        __ERC20_init("TestCurrency", "TSTC");
        _updateDomain("TestCurrency", "0.1.0");
    }
    ...
    // largely copied from openzeppelin reference implementation
    function permit(
        address owner,
        address spender,
        uint256 value,
        ...
        bytes32 objectHash = _generateObjectHash(
            _PERMIT_TYPEHASH,
            abi.encode(owner, spender, value, currentNonce, deadline)
        );
        EOASignature memory signature = EOASignature({v: v, r: r, s: s});
        require(_verifyEOASignature(objectHash, signature, owner), "invalid signature");
