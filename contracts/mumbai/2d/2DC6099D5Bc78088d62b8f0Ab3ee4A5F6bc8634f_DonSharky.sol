...
contract DonSharky is ERC721Enumerable, Ownable {
    ...
    // uint256[3] private cost = [70 * 1e18, 110 * 1e18, 150 * 1e18];
    uint256[3] private cost = [1 * 1e14, 2 * 1e14, 3 * 1e14];
    uint256[3] private maxSupply = [1000, 3000, 3000];
    uint256 private maxMintAmount = 20;
    // 0: whitelist 1, 1: whitelist 2, 2: public sale
    uint256 private mintPhase = 0;
    bool private paused = true;
    string private notRevealedUri;
    uint256 private alreadyRevealedTokens = 0;

    mapping(address => uint256) private _freeMintList;
    mapping(address => bool) public isWhitelisted;

    address WITHDRAW_WALLET = 0x97d10440604173a1Cb74486e874166Dd07633eF1;
    address KALADIN_WALLET = 0x97d10440604173a1Cb74486e874166Dd07633eF1;
    ...
    function mint(uint256 _mintAmount) external payable {
        ...
        if (getWhitelistOn()) require(isWhitelisted[msg.sender] || msg.sender == owner());
        ...
        uint256 payAmount = getMintPayAmount(msg.sender, _mintAmount, cost[mintPhase]);
    ...
    function mintERC20(uint256 _mintAmount) external {
        ...
        uint256 payAmount = getMintPayAmount(msg.sender, _mintAmount, cost[mintPhase]);
        if (erc20Interface.balanceOf(msg.sender) < payAmount) revert payAmountErr();

        bool success = transferERC20(msg.sender, payAmount);
        if (!success) payAmountErr;
    ...
    function transferERC20(address _from, uint256 payAmount) private returns (bool) {
        if (erc20Interface.balanceOf(_from) >= payAmount) {
            bool success = erc20Interface.transferFrom(_from, address(this), payAmount);
            if (!success) revert payAmountErr();
            return success;
        }
        return false;
    }
    ...
    function withdraw() external onlyOwner {
        uint256 devPercentage = 5;
        if (erc20Interface.balanceOf(address(this)) > 0) {
            unchecked {
                require(
                    erc20Interface.transfer(
                        KALADIN_WALLET,
                        (erc20Interface.balanceOf(address(this)) * devPercentage) / 100
                    )
                );
                require(
                    erc20Interface.transfer(
        ...
        if (address(this).balance > 0) {
            unchecked {
                (bool successDev, ) = KALADIN_WALLET.call{
                    value: (address(this).balance * devPercentage) / 100
                }("");
    ...
    function setFreeMintList(address[] calldata wallets, uint256 numAllowedToFreeMint)
        external
        onlyOwner
    {
        ...
        unchecked {
            for (uint256 i = 0; i < wallets.length; i++) {
                _freeMintList[wallets[i]] = numAllowedToFreeMint;
            }
        }
    }
    ...
    function getMintPayAmount(
        ...
        uint256 numFreeMints = _freeMintList[_account];
        uint256 totalFree = 0;
        if (numFreeMints > 0) {
            if (_mintAmount <= numFreeMints) {
                _freeMintList[_account] -= _mintAmount;
                totalFree = _mintAmount;
            } else {
                _freeMintList[_account] = 0;
                totalFree = _mintAmount - numFreeMints;
            }
        }

        return _cost * (_mintAmount - totalFree);
    }
    ...
    function getWhitelistOn() public view returns (bool) {
        return mintPhase < 2;
    }
