    ...
    UUPSUpgradeable
{
    ...
    event TokenStaked(address indexed sender, uint256 indexed tokenId);
    ...
    function initialize(NftToken _token, NftTokenBox _tokenBox)
        public
        initializer
    {
    ...
    function _beforeTokenStake(uint256 _tokenId) private {
        if (tokenBox.exists(_tokenId)) {
            require(
                tokenBox.ownerOf(_tokenId) == _msgSender(),
                "You do not own this tokenId (new collection)"
            );

            tokenBox.transferFrom(_msgSender(), address(this), _tokenId);
        } else {
            require(
