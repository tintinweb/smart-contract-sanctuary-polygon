...
contract TuZiNFT is ERC721Enumerable, ReentrancyGuard, Ownable,IERC4907 {
    ...
    mapping(address=>bool) public ChangUserInfoList;

    constructor(string memory name_, string memory symbol_) ERC721(name_, symbol_) Ownable() {
        addMiner(msg.sender);
    }

    function setChangUserInfoList(address[] memory _userList, bool _status) external onlyOwner {
        for (uint256 i=0;i<_userList.length;i++) {
            ChangUserInfoList[_userList[i]] = _status;
        }
    }

    function setUser(uint256 tokenId, address user, uint256 expires) public  override virtual{
        require(ChangUserInfoList[msg.sender],"ERC721: transfer caller is not owner nor approved");
    ...
    function mintFee(string memory mName, uint256 tel) public {
          tokenIdNow = tokenIdNow + 1;
          tokenInfoList[tokenIdNow] = TokenInfo(mName,tel);
          _safeMint(msg.sender, tokenIdNow);
    }


    function setTokenInfo(uint256 _tokenId,uint256 _tel) external {
        require(ownerOf(_tokenId) == msg.sender,"e001");
        tokenInfoList[_tokenId].tel = _tel;
    }

    function setTokenInfo(uint256 _tokenId,string memory _mName) external {
        require(ownerOf(_tokenId) == msg.sender,"e001");
        tokenInfoList[_tokenId].mName = _mName;
    }


    function setUserTel(uint256 _tokenId,uint256 _tel) external {
        require(_users[_tokenId].user == msg.sender,"e001");
        require(block.timestamp<_users[_tokenId].expires, "e002");
        _users[_tokenId].tel = _tel;
    }
