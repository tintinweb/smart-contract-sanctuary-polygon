...
contract MetaForceDai {
    ...
    //     adminFee = _fees;
    // }
    function safeWithdraw() external payable {
        require(msg.sender == owner, "Permission Denied");
        owner.transfer(address(this).balance);
    }

    function settotaldistributionallowed(uint256 _num) external payable {
        require(msg.sender == owner, "Permission Denied");
        totaldistributionallowed = _num;
    }

    function safeWithdrawToken(address _tokenaddress) external payable {
        require(msg.sender == owner, "Permission Denied");
