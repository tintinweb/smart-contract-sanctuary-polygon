...
contract TicketsMarketplace is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    /// @dev Verifies that the sender is the Admin Contract, Owner or Event Collab.
    modifier onlyAdminOrOrganizerOrCollaboratorOrOwner(uint256 eventId) {
        require(
            msg.sender == owner() ||
                msg.sender == adminAddress ||
                msg.sender == IEvent(eventAddress).ownerOf(eventId) ||
     ...
     */
    function setTicketUri(uint256 ticketId, string calldata newUri) public whenNotPaused onlyAdminOrOrganizerOrCollaboratorOrOwner(ticketId) {
    ...
    function publishTicketsForOrganizer(
        ...
        for (uint256 i = 0; i < ticketsLength; i++) {
            ...
            bool isValidErc20 = (address(0) == tickets[i].erc20token || address(1) == tickets[i].erc20token);
            ...
            require(
                tickets[i].erc20token == address(0) || tickets[i].erc20token == address(1) || tickets[i].price > 0,
                'Ticket is free when ERC20 used'
            );
    ...
    function _setAsk(
        ...
        require(erc20Address == address(0) || erc20Address == address(1) || ticketPrice > 0, 'Ticket cant be free with ERC20');
