...
contract NFTFarming_v3 is IERC777Recipient {
    using SafeMath for uint256;
    ...
    */
    function withdraw(
        ...
        nftMintingContract.mintFor(
            msg.sender,
            allNFTs[nftID].uintNFTId,
            1,
            editionNum,
            ""
        );
    ...
    */
    function createNFTPool(
        ...
        depositArtistPayout(maxPayout, nftID);
        uint256 timestamp = block.timestamp;
        nftSetup(artistAddress, maxPayout, contractAddress, nftID, uintNFTId);
        poolSetup(nftID, totalSupply, amountLeft, timestamp, stakingPrice, fee);
        emitNewPoolEvent(nftID);
    }

    function emitNewPoolEvent(
        bytes memory nftID
    ) internal {
        emit NewPoolCreated(nftID);
    }

    function depositArtistPayout(
        uint256 maxPayout,
        bytes memory nftID
    ) internal {
        if (maxPayout > 0)
    ...
    function nftSetup(
        ...
        allNFTs[nftID].uintNFTId = uintNFTId;
