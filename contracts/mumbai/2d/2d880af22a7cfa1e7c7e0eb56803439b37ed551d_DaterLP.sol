...
interface IDaterLP {
    ...
    event LiquidityAdded(uint256 stablecoinsAdded, uint256 daterCoinsAdded, uint256 stableReserve, uint256 dcReserve);
    event LiquidityWithdrawn(uint256 stablecoinsTaken, uint256 daterCoinsTaken, uint256 stableReserve, uint256 dcReserve);
    event CommissionsWithdrawn(uint256 stablecoinsWithdrawn, uint256 datercoinsWithdrawn);
    ...
    function getTokenObject(TokensNames) external view returns(IERC20MetaTxn);
    function tokenReserves(uint256) external view returns (uint256);
    function tokens(uint256) external view returns (IERC20MetaTxn);
    function getAmountOutBeforeSlippage(uint256, SwapDirections) external view returns(uint256);
    function getAmountInBeforeSlippage(uint256, SwapDirections) external view returns(uint256);
}
...
contract DaterLP is Ownable, ERC2771Recipient, IDaterLP {
    ...
    */
    ...
    function addLiquidity(uint256 stableAmount) public {
        ...
        emit LiquidityAdded(stableAmount, stableAmount*USD_TO_DC_MAX_RATIO, tokenReserves[uint(TokensNames.STABLECOIN)], tokenReserves[uint(TokensNames.DATERCOIN)]);
    }

    function addLiquidityStable(uint256 stableAmount) public onlyOwner {
        tokens[uint(TokensNames.STABLECOIN)].safeTransferFrom(_msgSender(), address(this), stableAmount);
        tokenReserves[uint(TokensNames.STABLECOIN)] += stableAmount;
        emit LiquidityAdded(stableAmount, 0, tokenReserves[uint(TokensNames.STABLECOIN)], tokenReserves[uint(TokensNames.DATERCOIN)]);
    }

    function addLiquidityDC(uint256 dcAmount) public onlyOwner {
        tokens[uint(TokensNames.DATERCOIN)].safeTransferFrom(_msgSender(), address(this), dcAmount);
        tokenReserves[uint(TokensNames.DATERCOIN)] += dcAmount;
        emit LiquidityAdded(0, dcAmount, tokenReserves[uint(TokensNames.STABLECOIN)], tokenReserves[uint(TokensNames.DATERCOIN)]);
    }
    ...
    function withdrawLiquidity(uint256 stableAmount, uint256 dcAmount) public onlyOwner {
        if (!withdrawalsAllowed)
            revert WithdrawalsNotAllowed();
        
        if (stableAmount > tokenReserves[uint(TokensNames.STABLECOIN)] || dcAmount > tokenReserves[uint(TokensNames.DATERCOIN)])
            revert NotEnoughFundsToWithdraw();

        tokens[uint(TokensNames.STABLECOIN)].safeTransfer(owner(), stableAmount);
        tokens[uint(TokensNames.DATERCOIN)].safeTransfer(owner(), dcAmount);
        tokenReserves[uint(TokensNames.STABLECOIN)] -= stableAmount;
        tokenReserves[uint(TokensNames.DATERCOIN)] -= dcAmount;
        emit LiquidityWithdrawn(stableAmount, dcAmount, tokenReserves[uint(TokensNames.STABLECOIN)], tokenReserves[uint(TokensNames.DATERCOIN)]);
    }
    ...
    function withdrawCommissions() public onlyOwner {
        ...
        emit CommissionsWithdrawn(usdtCommissions, dcCommissions);
    }
    ...
    function getTokenObject(IDaterLP.TokensNames tokenName) public view returns (IERC20MetaTxn) {
        return tokens[uint(tokenName)];
    }
    ...
    function getAmountOutBeforeSlippage(uint256 amountIn, IDaterLP.SwapDirections direction) public view returns (uint256 amountOut) {
        uint256 amountInAfterCommission = amountIn * (ONE_HUNDRED_PERCENT_TWO_DECIMALS - commission) / (ONE_HUNDRED_PERCENT_TWO_DECIMALS);
    ...
    function getAmountInBeforeSlippage(uint256 amountOut, IDaterLP.SwapDirections direction) public view returns (uint256 amountIn) {
        uint256 reservesIn = tokenReserves[uint(direction)];
        uint256 reservesOut = tokenReserves[1-uint(direction)];
