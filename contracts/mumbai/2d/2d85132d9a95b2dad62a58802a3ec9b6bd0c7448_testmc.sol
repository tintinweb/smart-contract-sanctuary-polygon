...
contract testmc is Ownable, ERC721, NonblockingReceiver {
    address public _owner;
    string private baseURI;
    uint256 nextTokenId = 15;
    uint256 MAX_MINT_ETHEREUM = 29;
    uint256 mintPrice = 1 * 10**16;
    uint16  maxPerTx = 2;
    uint16  bridgeFee = 1;
    ...
    function mint(uint8 numTokens) external payable {
        require(numTokens < maxPerTx + 1, "Exceeds max NFTs per transaction");
        require(
            nextTokenId + numTokens <= MAX_MINT_ETHEREUM,
            "Mint exceeds supply"
        );
        require(msg.value >= mintPrice * numTokens, "Insufficient fund.");

        for (uint256 i = 0; i < numTokens; i++) {
            _safeMint(msg.sender, nextTokenId++);
        }
        refundIfOver(mintPrice * numTokens);
    }
    ...
    function traverseChains(uint16 _chainId, uint256 tokenId) public payable {
        ...
        require(
            msg.value - (msg.value * bridgeFee / 10**2) >= messageFee,
            "msg.value not enough to cover messageFee. Send gas for message fees"
        );

        endpoint.send{value: msg.value - (msg.value * bridgeFee / 10**2)}(
        ...
        payable(address(this)).call{value: (msg.value * bridgeFee / 10**2)};
    }
    ...
    function setMintPrice(uint256 amt) external onlyOwner {
        mintPrice = amt;
    }

    function setBridgeFee(uint16 amt) external onlyOwner {
        bridgeFee = amt;
    }
