...
contract TEST is ERC1155 {
    ...
    function mint(uint256 _tokenId,address _address,uint256 _numberoftokens)
        public
        returns(uint256)
    {
     
        _mint(_address, _tokenId,_numberoftokens,"");
    ...
    function batchBurn(uint256[] memory ids,uint256[] memory amounts) public {
     ...
     function _tokenURI(uint256  _tokenId) public view virtual  returns (string memory) {
            bytes memory b;
            string memory uri = uri(_tokenId);
            b = abi.encodePacked(b,uri);
            b = abi.encodePacked(b, "item/");
