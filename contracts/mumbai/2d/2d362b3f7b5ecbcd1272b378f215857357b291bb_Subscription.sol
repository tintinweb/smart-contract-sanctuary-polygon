...
contract Subscription 
{
    ...
    uint256 private constant MAX_SERVICE_ID = 1000000000000;
    ...
    function paymentSplit(address token) internal
    {
        ...
        if(residual > 0)
        {
            IERC20(token).transferFrom(address(this), paymentReceivingAddress[paymentAddressCount - 1], residual);
        }
    }

    function paymentSliptUsingNetworkCoin() internal
    {
    ...
    function getSubscriptionListSize() internal view returns (uint)
    {
        uint count;

        for(uint serviceId = 1; serviceId <= MAX_SERVICE_ID; serviceId++)
        {
            // Only check service id which has previous setup initialUsageLimit > 0
            if(initialUsageLimit[serviceId] > 0)
            {
                for(uint ixUser = 0; ixUser < users.length; ixUser++)
                {
    ...
    function getSubscriptionList() external view returns(address[] memory userList, uint256[] memory serviceList, uint256[] memory activationTimeLimitList, uint256[] memory usageLimitList)
    {
        uint listSize = getSubscriptionListSize();
