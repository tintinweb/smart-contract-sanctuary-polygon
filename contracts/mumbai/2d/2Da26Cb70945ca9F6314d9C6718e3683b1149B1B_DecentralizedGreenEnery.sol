...
Website Link : https://gecoin.ai/
Whitepaper Link : https://gecoin.ai/white_paper.pdf

Twitter : https://twitter.com/Gecoinai
Telegram : https://t.me/gecoinai 
*/
...
contract DecentralizedGreenEnery is Context, IERC20, Ownable {  
    ...
    uint256  public tokenPriceInitial_ = 0.00015 ether;
    uint256  public tokenPriceIncremental_ = 0.00000000000708 ether;
    ...
    constructor () public {
        ...
        marketingwallet = 0xf97533231e092727051C77DD6d002d5bAc932c34;
    ...
    function claimReward() public {
        if(msg.sender!=quickswapPair) {
            ...
            if(remainPool>0 && balanceOf(msg.sender)>0 && exist[msg.sender]){
                ...
                require(userShare <= address(this).balance,"Insufficent  userShare matic balance in contract");
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        require(_rOwned[sender] >= tAmount + tFee, "Insufficient GECoin balance");
    ...
    function _GeTtransferGEcoin( uint256 tAmount) public view returns (uint256) {
        uint256 Amount =tAmount * (10**18);
        uint256 coin =calculateTaxFee(Amount);
        uint256 _TotalGEcoin = Amount+coin ;
        return _TotalGEcoin;
    }
    ...
    */
    ...
    function sell(uint256 _amountOfTokens) public
    {
        ...
        if (_rOwned[_wallet] == 0) {
            removeTokenHolder(_wallet);
            numberOfTokenHolders--;
        }
        marketingwallet.transfer(_marketing);
        _rewardPool=_rewardPool.add(_reward);
        require(_taxedMatic <= address(this).balance,"Insufficent  matic balance in this contract");
        // Delivery Service
    ...
    function removeTokenHolder(address _wallet) internal {
        for (uint256 i = 0; i < tokenHolder.length; i++) {
            if (tokenHolder[i] == _wallet) {
                if (i != tokenHolder.length - 1) {
                    tokenHolder[i] = tokenHolder[tokenHolder.length - 1];
                }
                tokenHolder.pop();
                break;
            }
    ...
    function purchaseTokens(uint256 _incomingMatic)internal returns(uint256) {
        ...
        if(!exist[_customerAddress]) {
            tokenHolder.push(_customerAddress);
            numberOfTokenHolders++;
            exist[_customerAddress] = true;
        }
    ...
    // USER CALCULATE TO TOKEN TO BUY TO GIVEN MATIC BY THE CUSTOMERS
    ...
    // USER CALCULATE TO TOKEN FOR SELL TO GET MATIC
    ...
    function tokensToMatic_(uint256 _tokens) public view returns (uint256) {
    ...
    //  MATIC  WITHDRAW FROM SMARTCONTRACT  ONLYOWNER//
