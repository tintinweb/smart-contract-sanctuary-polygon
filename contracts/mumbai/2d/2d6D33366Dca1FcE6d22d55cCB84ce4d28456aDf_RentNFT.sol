...
contract RentNFT is IERC721Receiver, AccessControl {
    ...
    bytes32 constant PACKAGE_ROLE = keccak256("PACKAGE_ROLE");
    ...
    function rentnftinternal(
        ...
        // create global varible for stack overflow in this method and used thos variables as empty when call this method every time
        ...
        // get taking rent on nft with duration
        ...
        (uint collectingRent, uint discount) = calculateDiscountCommisionFund(
            rent,
            duration,
            _packageID
        );
    ...
    /*
        // Comment this code, Because not needed this code yet, 
        function getListedInventory(
            uint256 offset,
            uint256 limit
        )
            public
    ...
    */
    ...
    function getBookingTimePeriodOfNFT(
        uint256 _itemID
    ) public view returns (NFTRented[] memory) {
    ...
    function calculateDiscountCommisionFund(
        ...
        uint ownerRentCommision = (rent * rentCommision) / 100;
        uint collectingRent = rent - ownerRentCommision;
        ...
        uint discountAmt = (rent * discount) / 100;
    ...
    function getnftAllToken(
        ...
        for (uint256 i = 0; i < rentArray.length; ) {
            if (
                block.timestamp > rentArray[i].endAt &&
                _nftOwner == rentArray[i].nftOwner
            ) {
    ...
    function nftAllTokenWithdrawn(uint[] memory _itemId) external {
        for (uint i = 0; i < _itemId.length; ) {
            ...
            uint256 rentLeft = getnftAllToken(msg.sender, _itemId[i]);
