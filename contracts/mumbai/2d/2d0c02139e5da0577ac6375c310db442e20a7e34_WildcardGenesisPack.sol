...
/// @title Wildcard Genesis Pack
/// @notice Represents an unopened pack of WildcardCollectibles from the Genesis collection
contract WildcardGenesisPack is ERC721PartnerSeaDrop {
    /*//////////////////////////////////////////////////////////////
                                ERRORS
    //////////////////////////////////////////////////////////////*/

    error OnlyOwnerOrWildcardCollectibles();

    /*//////////////////////////////////////////////////////////////
                            STATE VARIABLES
    //////////////////////////////////////////////////////////////*/

    /// @notice Address of the WildcardCollectibles contract
    address public wildcardCollectibles;
    ...
    constructor(
        ...
        address _wildcardCollectibles
    ) ERC721PartnerSeaDrop(_name, _symbol, _administrator, _allowedSeaDrop) {
        wildcardCollectibles = _wildcardCollectibles;
    }

    function burn(uint256 _tokenId) external {
        // only the token owner or WildcardCollectibles contract is allowed to burn
        if (msg.sender != ownerOf(_tokenId) && msg.sender != wildcardCollectibles) {
            revert OnlyOwnerOrWildcardCollectibles();
        }
    ...
    function getPacksByAddress(address _addr) external view returns (uint256[] memory) {
        uint256 tokensRemaining = totalSupply();
        uint256 numPacks = balanceOf(_addr);
        uint256[] memory packsByAddress = new uint256[](numPacks);
        if (numPacks == 0) {
            return packsByAddress;
        }

        uint256 idx = 0;
        for(uint256 tokenId = 1; tokensRemaining > 0; ++tokenId) {
            if (!_exists(tokenId)) {
                continue;
            }

            --tokensRemaining;
            if (ownerOf(tokenId) == _addr) {
                packsByAddress[idx] = tokenId;
                ++idx;
            }
        }
    ...
    function setWildcardCollectibles(address _wildcardCollectibles) external onlyOwner {
        wildcardCollectibles = _wildcardCollectibles;
    }
}