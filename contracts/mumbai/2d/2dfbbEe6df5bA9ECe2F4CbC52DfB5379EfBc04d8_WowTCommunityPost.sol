...
//   mapping(address => uint256[]) public userPosts; // user address => post IDs
  mapping(string => CommunityDetails) public communities;
  ...
  function createPost(
    ...
    communityDetails.postCount++;
    Post storage newPost = communityDetails.posts[communityDetails.postCount];
    // posts[_communityName][postCount] = Post(postCount, _content, 0, 0, 0, creator);
    newPost.id = communityDetails.postCount;
    newPost.content = _content;
    // newPost.likes = 0;
    // newPost.dislikes = 0;
    // newPost.commands = 0;
    newPost.creator = creator;
    newPost.commandName = _communityName;

    // userPosts[msg.sender].push(community.postCount); // Add post ID to userPosts mapping
  }

  function likePost(string memory _communityName, uint256 _postId) public {
    ...
    CommunityDetails storage communityDetails = communities[_communityName];
    require(_postId <= communityDetails.postCount, 'Invalid post ID');
    Post storage post = communityDetails.posts[_postId];
    post.likes++;
  }

  function dislikePost(string memory _communityName, uint256 _postId) public {
    //require(_postId > 0 && _postId <= postCount, "Invalid post ID");
    require(
            community.checkCommunityExists(_communityName),
            "Community doesn't exist"
  ...
  function commandPost(string memory _communityName, uint256 _postId) public {
    // require(_postId > 0 && _postId <= postCount, "Invalid post ID");
    require(
            community.checkCommunityExists(_communityName),
            "Community doesn't exist"
