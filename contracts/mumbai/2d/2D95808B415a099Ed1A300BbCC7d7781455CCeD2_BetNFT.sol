...
/**
 * @title BetNFT contract
 * @notice ERC721 token contract
 */
contract BetNFT is IBetNFT, ERC721, AccessControl, Pausable {
    mapping(uint256 => BetNFTItem) private betNFTItems;
    mapping(uint256 => BetNFTState) private betNFTStates;

    constructor() ERC721("Bet Equity Token", "BET") {
        ...
        returns (bool)
    {
        ...
        betNFTItems[tokenId] = item;
        betNFTStates[tokenId] = state;
        _safeMint(item.minter, tokenId);
        _transfer(item.minter, state.owner, tokenId);

        emit LogBetNFTCreate(
            tokenId,
            item.matchKey,
            item.mintValue,
            item.faceValue,
        ...
        betNFTStates[tokenId].actionType = EnumActionType.NON;
        return true;
    }
     ...
     */
    function update(BetNFTState memory state)
        external
        override
        onlyRole(CONTRACTOR_ROLE)
        returns (bool)
    {
        ...
        betNFTStates[tokenId].actionType = state.actionType;
        if (state.actionType == EnumActionType.LIST) {
            address owner = state.owner;
            state.owner = address(this);
            state.seller = owner;
            state.itemStatus = EnumItemStatus.LISTED;
            ...
            state.actionType = EnumActionType.NON;
            betNFTStates[tokenId] = state;
        } else if (state.actionType == EnumActionType.UNLIST) {
            address owner = state.seller;
            state.owner = owner;
            state.seller = address(0);
            state.itemStatus = EnumItemStatus.UNLISTED;
            _transfer(address(this), owner, tokenId);
            emit LogBetNFTUnlist(
                tokenId,
                state.price,
                state.ticker,
                state.owner,
            ...
            state.actionType = EnumActionType.NON;
            betNFTStates[tokenId] = state;
        } else if (state.actionType == EnumActionType.SALE) {
            address seller = state.seller;
            state.seller = address(0);
            state.itemStatus = EnumItemStatus.UNLISTED;
            _transfer(address(this), state.owner, tokenId);
            emit LogBetNFTSale(
                tokenId,
                state.price,
                state.ticker,
            ...
            state.actionType = EnumActionType.NON;
            betNFTStates[tokenId] = state;
        } else if (state.actionType == EnumActionType.UPDATE) {
            state.itemStatus = EnumItemStatus.LISTED;
            emit LogBetNFTUpdate(
                tokenId,
                state.price,
                state.ticker,
                state.owner,
            ...
            state.actionType = EnumActionType.NON;
            betNFTStates[tokenId] = state;
        } else if (state.actionType == EnumActionType.CLAIM) {
            emit LogBetNFTClaim(
                tokenId,
                state.price,
                state.owner,
            ...
            burn(tokenId);
        } else if (state.actionType == EnumActionType.BUYBACK) {
            emit LogBetNFTBuyback(
                tokenId,
                state.price,
                state.owner,
                state.seller
    ...
    function _afterTokenTransfer(
        ...
        if (
            ...
            betNFTStates[tokenId].owner = to;
            emit LogBetNFTTransfer(tokenId, from, to);
        }
    }
     ...
     */
    function calculate(BetNFTItem memory item)
        external
        override
        onlyRole(CONTRACTOR_ROLE)
        returns (bool)
    {
        betNFTItems[item.tokenId] = item;
        emit LogBetNFTCalculate(item.tokenId, item.reward, item.rewardPhase);
        return true;
    }
     ...
     * @return result whether NFT burned successfully
     */
    function burn(uint256 tokenId)
        ...
        returns (bool)
    {
        delete betNFTItems[tokenId];
        delete betNFTStates[tokenId];
        _burn(tokenId);
        return true;
    }

    /**
     * @notice Get the NFT tokenURI on ipfs
     * @param tokenId BetNFT token Id
     * @return tokenURI NFT tokenURI
     */
    ...
    {
        require(_exists(tokenId), "BetNFT not exist");
        return betNFTItems[tokenId].tokenURI;
    }
    ...
    function exists(uint256 tokenId) external view override returns (bool) {
        return _exists(tokenId) || betNFTItems[tokenId].tokenId > 0;
    }
