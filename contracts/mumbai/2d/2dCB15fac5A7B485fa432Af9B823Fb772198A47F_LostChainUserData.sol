...
contract LostChainUserData is CustomERC2771ContextUpgradeable, UUPSUpgradeable {
    ...
    modifier onlyRole(string memory name, address account) {
        require(LCRBALinker.checkAccountRole(name, account));
        _;
    }
    ...
    function getRole(address playerAddress) public view returns (Privilege memory) {}
    ...
    function TESTING30() external pure returns (bool) {
        return true;
    }
}
...
contract LostChainRBALinker is Initializable, UUPSUpgradeable {
    ...
    modifier onlyRole(string memory name, address account) {
        require(currentLCRBA.checkAccountRole(name, account));
        _;
    }
