    ...
    UUPSUpgradeable
{
    ...
    // Fragments Fade
    mapping(address => uint256) public availableFragmentSwaps;
    ...
    function swapFragments(uint256[] memory ids, uint256[] memory amounts) external {
        require(ids.length < 4, "invalid arguments");
        require(ids.length == amounts.length, "invalid arguments");

        uint256 totalAmount;
        for (uint256 index = 0; index < ids.length; index++) {
            require(ids[index] < 4, "only fragments");
            totalAmount += amounts[index];
        }
        require(totalAmount > 10, "not enough fragments");
        require(totalAmount % 10 == 0, "invalid number of fragments");
        uint256 swaps = totalAmount / 10;
        require(availableFragmentSwaps[msg.sender] >= swaps, "not enough swaps");

        availableFragmentSwaps[msg.sender] -= swaps;
        _burnBatch(msg.sender, ids, amounts);
        _unsafeMint(EYE_OF_SELF_REFLECTION, swaps);
    }

    function convertFragments(uint256[] memory ids, uint256[] memory amounts) external {
        require(ids.length < 4, "invalid arguments");
        require(ids.length == amounts.length, "invalid arguments");
        ...
        _burnBatch(msg.sender, ids, amounts);
        _freyjaTear.mint(msg.sender, totalAmount * 1 ether);
    }
    ...
    function setAvailableFragmentSwaps(address[] memory wallets, uint256 value) external onlyOwner {
        for (uint256 index = 0; index < wallets.length; index++) {
            availableFragmentSwaps[wallets[index]] = value;
        }
    }
