...
contract JumbleMachineMembership is Ownable, PriceLists {
    ...
    uint256 public acceptingMembership = TRUE;
    ...
    //強制入会
    function forcedAdmission(address address_, uint8 course)
        external
        onlyOwner
    {
...
abstract contract PriceLists is Ownable {
    ...
    constructor() {
        ...
        priceLists[1] = PriceList({cost: 0.1 ether, period: 7 days}); //Weekly
        priceLists[2] = PriceList({cost: 0.3 ether, period: 30 days}); //Monthly
        priceLists[3] = PriceList({cost: 1 ether, period: 365 days}); //Annual
    }
