...
contract MetaForceContract is Initializable, Proxied, Governed, ReentrancyGuard, IMetaForceContract {
    ...
    function renewalPackForMFS(
        ...
        if (core.getWorkflowStage() == Workflow.SaleOpen) {
            ...
            holding.holdOnBehalf(senderId, 0, priceRenewalPackInMFS);
        } else {
...
error MetaForceSpaceHoldingLevelNotAllowedToHold(uint256 levelNumber);
...
interface IHoldingContract {
    event Hold(uint256 indexed depositId, uint256 createdAt, uint256 amount);
    event Unhold(uint256 indexed depositId, uint256 amount);
    event Withdraw(uint256 indexed depositId, uint256 indexed level, uint256 amount);
    event BackMFSInMetaPool(uint256 amount);

    function setHoldingOpened(bool _opened) external;

    function hold(uint256 entryLevel, uint256 amount) external returns (uint256);
    ...
    function hMFSToMFS(uint256 entryLevel, uint256 amount) external;

    function getDeposit(uint256 depositId) external view returns (Deposit memory);

    function getDepositIds(uint256 holderId) external view returns (uint256[] memory);

    function getOutHMFSLevelFromDeposit(uint256 depositId) external view returns (uint256 levelHMFS);

    function getHoldingOpened() external view returns (bool);

    function lockupPeriods(uint256 level) external view returns (uint256);
}
