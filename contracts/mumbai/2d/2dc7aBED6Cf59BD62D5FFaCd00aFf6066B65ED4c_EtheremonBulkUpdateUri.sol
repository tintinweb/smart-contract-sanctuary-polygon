...
contract EtheremonBulkUpdateUri is BasicAccessControl {
    ...
    string public metaUriPrefix =
        "https://ethermon.io/api/matic/monster/get_data?monster_ids=";
    ...
    function setContracts(address _dataContract, address _monsterNFTContract)
        public
        onlyModerators
    {
    ...
    function setNeglectedClass(uint32 _classId) public onlyModerators {
        neglectedClasses[_classId] = true;
    }

    function setMetaUriPrefix(string memory _uri) public onlyModerators {
        metaUriPrefix = _uri;
    }
    ...
    function setBulkTokenURI(
        ...
        for (; _startMonId <= _endMonID; _startMonId++) {
            ...
            if (neglectedClasses[obj.classId] == false) {
                string memory _token_id_str = Strings.toString(
                    uint256(_startMonId)
                );
                string memory _uri = string(
                    abi.encodePacked(metaUriPrefix, _token_id_str)
                );
