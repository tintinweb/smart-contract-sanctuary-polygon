...
contract AppNFTUpgradeable is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, 
    ...
    */
    function bulkMintAndURIupdate(address[] calldata to, string[] calldata appNames, string[] calldata uris) external onlyOwner {
        ...
        for (uint256 i = 0; i < appNames.length; i++) {
            string memory validatedAppName = _validateName(appNames[i]); //TODO: check if this is required
            mint(to[i], uris[i], validatedAppName);
        }
    }
    ...
    */
    function bulkTransfer(address from, address to, uint256[] calldata tokenIds) external onlyOwner {
