...
contract MagicMondrian is Ownable, Pausable, ReentrancyGuard, ERC721A, EIP712 {
    ...
    string private contractMetadata = "ipfs://QmbvT9L47MVum7icnoCb7sPPY17mwBQxzYiGHQbhyJvPH6/";
    address payable private adminAddress = payable(0x5D972D27b3D89bC9a9df692670034224Eb6De4d3);
    ...
    // Print price
    uint256 public printPrice = 0.0001 ether;
    ...
    // Mapping to limit amount of prints to 1 print per address
    mapping(address => bool) public hasPrintedOnce;

    constructor(address _paymentSplitter, address _signerAddress, string memory _baseURI) ERC721A("Magic Mondrian", "MAMO") EIP712(_signerAddress, "Magic Mondrian") {
        baseURI = _baseURI;
    ...
    modifier canPrint() {
        require(hasPrintedOnce[msg.sender] == false, "Already printed");
        _;
    }
    ...
    /**
     * @dev Receives specified amount for printing and saves info in mapping.
     */
    function print() external payable callerIsUser canPrint nonReentrant {
        require(msg.value >= printPrice, "Not enough funds");
        require(balanceOf(msg.sender) > 0, "No balance");
        (bool success, ) = payable(adminAddress).call{ value: address(this).balance }(""); // solhint-disable-line avoid-low-level-calls
        require(success, "Withdraw failed");
        hasPrintedOnce[msg.sender] = true;
    }
    ...
    /**
     * @dev Possibility to set a new price for printing in case of fluctuating prices.
     *
     * @param _price uint256 new print price
     */
    function setPrintPrice(uint256 _price) external onlyOwner {
        printPrice = _price;
    }
