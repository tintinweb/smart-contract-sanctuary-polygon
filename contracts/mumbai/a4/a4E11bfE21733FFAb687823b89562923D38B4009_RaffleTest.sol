...
contract RaffleTest is Ownable{
   ...
   IERC721 public token;
    ...
    mapping(uint => mapping(address => bool)) private isRegistered;
...
//    mapping(uint256 => mapping(address => bool)) public isRegistered;
    ...
    modifier checkRaffle(uint Raffle_id){
        ...
        require((raffle[Raffle_id].currentRound + 1) <= raffle[Raffle_id].totalRounds, RI_notExist);
        _;
    }
    ...
    function setPrizesAndWeights(uint Raffle_id, bytes memory _prizes, bytes memory _weights, uint256 sumOfSeats) external onlyOwner checkRaffle(Raffle_id) {
        ...
        require(sumOfSeats >= getQtyOfWinners(Raffle_id, raffle[Raffle_id].currentRound), "wrong amount of parameters.");
    ...
    function pickAwards(uint256 raffleID, uint256 roundNum) external onlyOwner checkRaffle(raffleID) returns(bool){
        ...
        address[] memory winners = getWinners(raffleID, roundNum);
        uint256[] memory awards = new uint256[](totalAwards);
        (bytes32[] memory prizes,) = getPrizesAndWeights(raffleID);


        for(uint256 i = 0; i < totalAwards; i++){ 
            uint randomAward = randomAccordingToWeight(raffleID, roundNum);
            awards[i] = awards[randomAward];
    ...
    function randomWinners(uint256 raffleID, uint256 roundNum) internal{
        ...
        for(uint i = 0; i < totalWinners; i++){
            uint256 randomnumber = uint(keccak256(abi.encodePacked(block.timestamp, msg.sender, nonce)));
            uint256 winnerIndex = (randomnumber % participants.length) + 1;
    ...
    //     if(weights[i] > 0){     //checking if the weight is greater than 0
    //         // console.log("weights[i]",weights[i]);
            randomnumber = uint(keccak256(abi.encodePacked(block.timestamp, msg.sender, nonce)));
            randomnumber = (randomnumber % awards.length) + 1;   
            weights[i] -= 1;
            ...
            if(weights[i] == 0){
                ...
                // delete weights[weights.length - 1];
                // delete awards[awards.length - 1];
                // weights.pop();
            }
         }
            ...
            // raffle[raffleID].prizes.pop();
            // raffle[raffleID].weights.pop();
            // delete weights[weights.length - 1];
            // if(currentElement < awards.length){
            //     currentElement = i + 1;
            // }   else if(currentElement >= awards.length){
