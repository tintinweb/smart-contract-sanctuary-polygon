...
contract AetMasterChef is Ownable, InterestHelper, IAetMasterChef {
   ...
   */
    ...
    receive() external payable {
        revert("AetCoin deposit not supported");
    }
    ...
    function updateFeeValues(uint8 _feePercent, address _feeAddress) external onlyOwner {
    ...
    function add(
        ...
        _stake(poolLength() - 1, compounderContract, 0, false);
    }
    ...
    function stake(uint256 _pid, uint256 _amount) external override returns (bool) {
        ...
        require(
            token.allowance(msg.sender, address(this)) >= _amount, 'Token: Set allowance first'
        );
    ...
    function payout(uint256 _pid, address _address) public view override returns (uint256 value) {
        User storage user = users[_pid][_address];
        Pool memory pool = poolInfo[_pid];
        ...
        uint256 multiplier = calcMultiplier(_pid, _address);

        if (from < to) {
            uint256 rayValue = yearlyRateToRay((pool.apy * 10**18) / 1000);
            value = (accrueInterest(user.totalInvested, rayValue,
                to
                .sub(from)
                .sub(user.totalInvested)));
        }
    ...
    function canClaim(uint256 _pid, address _address) public view override returns (bool) {
        User storage user = users[_pid][_address];
    ...
    function calcMultiplier(uint256 _pid, address _address) public view override returns (uint16 multi) {
        NFTMultiplier memory nft = nftInfo[_pid];

        if (nft.active && ownsCorrectNFT(_address, _pid) && _address != compounderContract) {
    ...
    function ownsCorrectNFT(address _senderAddress, uint256 _pid) public view returns (bool) {
        NFTMultiplier memory nft = nftInfo[_pid];

        uint256[] memory ids = walletOfOwner(nft.contractAdd, _senderAddress);
    ...
    function walletOfOwner(address _contractAddress, address _ownerAddress) internal view returns (uint256[] memory) {
        IERC721Enumerable nft = IERC721Enumerable(_contractAddress);
        uint256 tokenCount = nft.balanceOf(_ownerAddress);

        uint256[] memory tokensId = new uint256[](tokenCount);
        for(uint256 i; i < tokenCount; i++) {
            tokensId[i] = nft.tokenOfOwnerByIndex(_ownerAddress, i);
        }
        return tokensId;
    }
    ...
    function _stake(uint256 _pid, address _sender, uint256 _amount, bool _isReinvest) internal {
        ...
        uint256 stopDeposit = pool.endDate.sub(pool.lockPeriodInDays.mul(1 days));

        require(block.timestamp <= stopDeposit, 'Staking is disabled for this pool');
    ...
    function _claim(uint256 _pid, address _address) internal {
        User storage user = users[_pid][_address];
        Pool memory pool = poolInfo[_pid];

        uint256 amount = payout(_pid, _address);
        ...
        if (amount > 0) {
            ...
            safeTOKENTransfer(pool.token, _address, feeAmount);

            amount = amount.sub(feeAmount);

            safeTOKENTransfer(pool.token, _address, amount);

            user.lastPayout = block.timestamp;

            user.totalClaimed = user.totalClaimed.add(amount);
        }
        emit Claim(_address, amount, block.timestamp);
    }
    ...
    function safeTOKENTransfer(address _token, address _to, uint256 _amount) internal {
        ...
        uint256 maxTransfer = maxTransferAmount[_token];
        uint256 amount = _amount;

        while (amount > maxTransfer) {
            token.transfer(_to, maxTransfer);
            amount = amount.sub(maxTransfer);
        }
