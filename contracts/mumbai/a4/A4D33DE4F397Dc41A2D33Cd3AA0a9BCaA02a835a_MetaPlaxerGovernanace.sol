...
contract MetaPlaxerGovernanace is ReentrancyGuardUpgradeable,OwnableUpgradeable {
        ...
        uint public unstakeWaitTime;
        address[4] public treasuryPoolsAddresses;
        address[4] public nftTokenAddress;
        ...
        function logic (uint poolId,uint tokenAmounts,bool status, uint actualOne, string memory _uri) internal nonReentrant {
                ...
                uint pool=poolId;
                poolMapper[currentIndexOfPool][nftTokenAddress[poolId]] = poolId;
                ...
                if ((poolId== 1) || (actualOne==1)) {
                        ...
                        token.transferFrom(msg.sender, insuranceFundAddress, insuranceAmount);
                          ...
                          IPool(treasuryPoolsAddresses[actualOne]).DistributePLXR(insuranceFundAddress,insuranceAmount);  
                        }
                } 
                ...
                if(status){
               token.transferFrom(msg.sender, treasuryPoolsAddresses[pool], total);}
               else{
    
                    IPool(treasuryPoolsAddresses[actualOne]).DistributePLXR(treasuryPoolsAddresses[pool], total);
               }
                string memory uri=_uri;
               IGovernanceNFT(nftTokenAddress[pool]).mintTokens(msg.sender, nftIndexPerPool[pool], uri);
               emit Deposit(msg.sender, treasuryPoolsAddresses[pool],total, pool ,currentIndexOfPool);
               TVLInAllPools+=total;
        ...
        function claimFunds (uint[] memory poolIds, uint[] memory _nftIds, uint[] memory _amount) external nonReentrant {
                ...
                for (uint i=0; i< poolIds.length; i++) {
                        ...
                        require (IGovernanceNFT(nftTokenAddress[poolIds[i]]).ownerOf(_nftIds[i]) == msg.sender,'Error: Caller Not Owner');
                        ...
                        if(poolIds[i]==1){
                            ...
                            amount-=insure;
                             IPool(treasuryPoolsAddresses[poolIds[i]]).DistributePLXR(msg.sender, insure);
                        }else{
                        ...
                        IPool(treasuryPoolsAddresses[poolIds[i]]).DistributePLXR(msg.sender, amount);
        ...
        function unstakeTokens (uint[] memory poolIds, uint[] memory _nftIds) external nonReentrant {
                ...
                for (uint i=0; i< poolIds.length; i++) {
                        ...
                        require(block.timestamp>unStakeConfirmedTime[msg.sender][_nftIds[i]][poolIds[i]]+unstakeWaitTime,"Funds In Processing");
                        require (IGovernanceNFT(nftTokenAddress[poolIds[i]]).ownerOf(_nftIds[i])==msg.sender,'Error: Caller Not Owner');
                        ...
                        if(poolIds[i]==1){
                           ...
                           IPool(treasuryPoolsAddresses[poolId]).DistributePLXR(insuranceFundAddress,insurance); 
                        }
                        uint finalAmount=amountToReturn+__amount;
                        delete currentTermIdForUserPerPool[msg.sender][poolId];
                        delete poolMapper[_nftIds[i]][nftTokenAddress[poolId]];
                        totalInvestedInEachPool[poolIds[i]]-=__amount;
                        delete detailsPerNftId[_nftIds[i]][poolId];
                        IGovernanceNFT(nftTokenAddress[poolId]).burnTokens(_nftIds[i]);
                        IPool(treasuryPoolsAddresses[poolId]).DistributePLXR(msg.sender,finalAmount);
                        TVLInAllPools-=__amount;
        ...
        function getRewardDetails(uint poolId, uint _nftId) public view returns(uint) {
                ...
                uint finalAmount;
                uint time;
                if (block.timestamp > lastClaimTime + rebaseTime)
                {
                        finalAmount = ((amount*aprPercent[poolId])/1000)/365;
                        time = (block.timestamp - lastClaimTime + rebaseTime)/rebaseTime;
        ...
        function setTreasuryAddresses(address _alphaTreasury, address _betaTreasury, address _metaTreasury) external onlyOwner {
                treasuryPoolsAddresses = [address(0),_alphaTreasury,_betaTreasury,_metaTreasury];
        }

        function  setNFTAddresses(address _alphaNFT, address  _betaNFT, address _metaNFT) external onlyOwner {
                nftTokenAddress = [address(0),_alphaNFT,_betaNFT,_metaNFT];
        }

        function setInsuranceFundAddress(address _insuranceFundAddress) external onlyOwner {
                insuranceFundAddress = _insuranceFundAddress;
        }
        ...
        function depositRewardTokens(uint poolId,uint amount) external onlyOwner{
            token.transferFrom(msg.sender,treasuryPoolsAddresses[poolId], amount);
            totalRewards+=amount;
