...
/**
 * @title An NFT contract for Indi NFTs
 * @author Linum Labs
...
import "./interfaces/IIndiNFT.sol";

contract IndiNFT is ERC1155, ERC165Storage, Ownable, IIndiNFT {
    ...
    function uri(uint256 tokenId) public view override returns(string memory) {
        return bytes(_uri).length > 0
            ? string(abi.encodePacked(_uri, "/", uris[tokenId]))
            : uris[tokenId];
    }
    ...
    function setRoyaltyExemption(uint256 tokenId, address toModify, bool isExempt) external override {
    ...
    function setRedeem(
        ...
        require(msg.sender == owner() || msg.sender == properties[tokenId].creator,
            "setRedeem:only creator or admin");
        ...
        if (bytes(_description).length > 0)
            properties[tokenId].redeemDescrip = _description;
...
interface IIndiNFT is IERC1155, IERC2981 {
    struct Properties {
        address creator;
        bool isRedeemable;
        string redeemDescrip; // description of what you can redeem with the NFT
    ...
    function setRoyaltyExemption(uint256 tokenId, address toModify, bool isExempt) external;
    function setRedeem(uint256 tokenId, string memory description, bool _redeemable) external;
    function mint(address account, uint256 amount, string memory _uri) external;
    function mintBatch(address to, uint256[] memory amounts, string[] memory _uris) external;
}
