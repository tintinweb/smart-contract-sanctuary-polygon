 ...
 *  | $$  \ $| $$_____| $$  | $| $$  | $| $$_____| $$      \____  $$      | $$  | $| $$              \  $$$/ /$$__  $| $| $$  | $| $$
 *  | $$$$$$$|  $$$$$$| $$  | $|  $$$$$$|  $$$$$$| $$      /$$$$$$$/      |  $$$$$$| $$               \  $/ |  $$$$$$| $|  $$$$$$| $$
 *  |_______/ \_______|__/  |__/\_______/\_______|__/     |_______/        \______/|__/                \_/   \_______|__/\______/|__/
 */

contract BoVToken is AccessControl, ERC20Burnable {
    ...
    uint256 public constant maxSupply = 1300000000 ether;
    ...
    constructor(
        ...
        // Mint initial supply to admins account
        _mint(admin, maxSupply);
    }

    function setAccountLimit(uint256 amount) external onlyRole(ADMIN_ROLE) {
        accountLimit = amount;
    }

    function setTransferLimit(uint256 amount) external onlyRole(ADMIN_ROLE) {
        transferLimit = amount;
    }

    function setAccountLimitExclusion(address[] calldata accounts, bool isExcluded) external onlyRole(ADMIN_ROLE) {
    ...
    function setTransferLimitExclusion(address[] calldata accounts, bool isExcluded) external onlyRole(ADMIN_ROLE) {
        for (uint256 i = 0; i < accounts.length; i++) {
    ...
    function setBlacklisting(address[] calldata accounts, bool isBlacklisted) external onlyRole(ADMIN_ROLE) {
        for (uint256 i = 0; i < accounts.length; i++) {
    ...
    function withdrawAsset(
        ...
        if (token == address(0)) {
            require(amount <= address(this).balance, 'Requested amount exceeds balance');
            to.transfer(amount);
        } else {
            require(amount <= IERC20(token).balanceOf(address(this)), 'Requested amount exceeds balance');
