...
/**
    ...
    @notice This contract represents a recall tracing token standard that allows systems, distributors, and customers
...
contract RecallToken is ERC1155, Ownable {
    ...
    */
    ...
    /** @dev 
        The `_announcer` argument MUST be the address of an account/contract approved to manage the token
        The `_tokenId` argument MUST be the token that the systems are merged into
        The `_mergedTokenId` MUST be the token that the systems are sourced from
        The `_systems` MUST all the distinct array combination of systems of _tokenId and _mergedTokenId
    */
    event TokenMerged(
        address indexed _announcer,
        uint256 _tokenId,
    ...
    mapping(uint256 => address[]) systems;

    function getSystemsOfToken(
        uint256 _tokenId
    ) public view returns (address[] memory) {
        return systems[_tokenId];
    }

    mapping(address => mapping(uint256 => TokenCheckingState)) systemTokenCheckingStates;

    function getSystemTokenCheckingStateValue(
        address _address,
        uint256 _tokenId
    ) public view returns (TokenCheckingState) {
        return systemTokenCheckingStates[_address][_tokenId];
    }
    ...
    modifier _isSystem(uint256 _tokenId) {
        bool tempIsSystem = false;
        for (uint i = 0; i < systems[_tokenId].length; i++) {
            if (_msgSender() == systems[_tokenId][i]) {
                tempIsSystem = true;
                break;
            }
        }

        require(tempIsSystem == true, "Must be System");
        _;
    }
    ...
    */
    function announceDefect(uint256 _tokenId) public {
        ...
        for (uint i = 0; i < systems[_tokenId].length; i++) {
            systemTokenCheckingStates[systems[_tokenId][i]][
                _tokenId
    ...
    */
    function checkToken(
        ...
        require(
            systemTokenCheckingStates[_msgSender()][_tokenId] ==
                TokenCheckingState.PLEASE_CHECK,
            "Token can not be checked"
        );
        systemTokenCheckingStates[_msgSender()][
            _tokenId
        ] = _tokenCheckingState;
    }
    ...
    */
    function forwardRecall(uint256[] calldata _tokenIds) public {
        for (uint i = 0; i < _tokenIds.length; i++) {
            bool tempIsSystem = false;
            for (uint x = 0; x < systems[_tokenIds[i]].length; x++) {
                if (_msgSender() == systems[_tokenIds[i]][x]) {
                    tempIsSystem = true;
                    break;
                }
            }
    ...
    /**
     * @dev See {IERC1155-safeTransferFrom}.
     * if _internal is true, receiver will be added as system, otherwise not
     * after _internal is set false for one transfer, receivers will no longer be added as systems
     */
    function transferRecallToken(
        ...
        if (inProduction[_tokenId] && _internal) {
            appendUniqueSystemOfToken(_receiver, _tokenId);
        }
    ...
    function batchTransferRecallToken(
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            if (inProduction[_tokenIds[i]] && _internal) {
                appendUniqueSystemOfToken(_receiver, _tokenIds[i]);
            }
    ...
    function mintBatch(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            systems[ids[i]].push(_msgSender());
        }
    }

    /** @dev 
        The `_tokenIdMergeTo` argument MUST be the token that the systems are merged into
        The `_tokenIdMergeSource` MUST be the token that the systems are sourced from
    */
    function mergeToken(
        uint256 _tokenIdMergeTo,
        uint256 _tokenIdMergeSource
    ) public _isSystem(_tokenIdMergeTo) {
        _authorizedForToken(_msgSender(), _tokenIdMergeTo);
        //_mergeInto does not need to be owned
        for (
            uint256 i = 0;
            i < systems[_tokenIdMergeSource].length;
            i++
        ) {
            appendUniqueSystemOfToken(
                systems[_tokenIdMergeSource][i],
                _tokenIdMergeTo
            );
        }
        emit TokenMerged(
    ...
    */
    function mergeExternalToken(
        ...
        for (
            uint256 i = 0;
            i <
            tokenContract.getSystemsOfToken(_tokenIdMergeSource).length;
            i++
        ) {
            appendUniqueSystemOfToken(
                tokenContract.getSystemsOfToken(_tokenIdMergeSource)[i],
                _tokenIdMergeTo
            );
        }
    ...
    function appendUniqueSystemOfToken(
        address _system,
        uint256 _tokenId
    ) private {
        bool found = false;
        for (uint256 i = 0; i < systems[_tokenId].length; i++) {
            if (systems[_tokenId][i] == _system) {
                found = true;
                break;
            }
        }
        if (!found) {
            systems[_tokenId].push(_system);
        }
    }
}