 ...
 */
contract EventOnlyPrimarySale is ERC721, Pausable, Ownable {
    ...
    function setOrganizer(address _organizer) public EventNotStarted onlyOwner {
        organizer = _organizer;
    }
     ...
     */
    function buyTicket() external payable EventNotStarted whenNotPaused {
        require((msg.value == initialTicketPrice), "not enough money");
        _tokenIdCounter.increment();
        uint256 _ticketId = _tokenIdCounter.current();
        uint256 royalty = (transferFee * (msg.value)) / 100;
        uint256 val = msg.value - royalty;
        (bool success, ) = payable(organizer).call{value: val}("");
