...
contract wav3shack {
    ...
    mapping(uint256 => mapping(address => bool)) s_publicationToFollowerHasMirrored;

    constructor(uint256 fee, address wav3s /*, address wav3sHub*/) {
        i_wav3s = wav3s;
        i_fee = fee;
        owner = msg.sender;
    }
    ...
    function processMirror(uint256 pubId, address mirrorerAddress) external {
    ...
    function fundMySuperReach(
        ...
        IERC20(currencyAddress).safeTransferFrom(address(this), i_wav3s, (fees / 2));
    ...
    function getBudget(uint256 pubId) public view returns (uint256) {
