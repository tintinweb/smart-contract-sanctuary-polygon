...
contract Margin is ReentrancyGuard, MarginStorage, Ownable {
    ...
    modifier onlyPosition() {
        require(msg.sender == positionContract, "Margin: Not admin");
    _;
    }
    function addDelegatorAddress(address assetAddress, address delegatorAddress)
        external
        onlyOwner
    {
        delegatorAddresses[assetAddress] = delegatorAddress;
    }

    function updateMarginFunds(address _marginFundsContract) external onlyOwner {
        require(
            _marginFundsContract.isContract(),
            "Margin: Address is not a contract"
        );
        marginFundsContract = _marginFundsContract;
    }
    
    function updatePosition(address _positionContract) external onlyOwner {
        require(
            _positionContract.isContract(),
            "Margin: Address is not a contract"
        );
    ...
    function deposit(
        ...
        if (assetAddress == address(0)) {
            ...
            (bool successTransfer, ) = marginFundsContract.call{
                    value: marginAmount
                }("");
                require(
                    successTransfer,
        ...
        uint256 marginReserve = (marginAmount * 500) / 10000; //marginPercent should be from admin contract
        ...
        address delegator = delegatorAddresses[assetAddress];
        ...
        uint256 success = ICErc20Delegator(delegator).borrow(
            msg.sender,
            payable(marginFundsContract),
            loanAmount
        );
        // //Loan(Liquidity) check
        ...
        nonReentrant
    {
        ...
        // uint256 loanAllocated = (depositInfo.loanAmount * amountToWithdraw) /
        //     totalMargin;
        ...
        // require(
        //     amountToWithdraw > interestAmount,
        //     "Margin: Insufficient amount"
        // );
        ...
        if (assetAddress == address(0)) {
            ...
            IMarginFunds(marginFundsContract).withdrawFunds(payable(address(this)), amountToWithdraw);
            payable(msg.sender).transfer(amountToWithdraw);
        }
        else {
            //internal call to marginFundsContract
            IMarginFunds(marginFundsContract).withdrawERC20Tokens(assetAddress, address(this), amountToWithdraw);
        ...
        returns (uint256)
    {
        ...
        uint256 maxWithdrawAmount = (4 * totalMargin - loanAmount) / 4;
        ...
        nonReentrant
    {
        ...
        address delegator = delegatorAddresses[assetAddress];
        require(delegator != address(0), "Margin: Not a delegator address");
        //here the amount is getting transferred to the margin contract address(not to user address),
        uint256 success = ICErc20Delegator(delegator).borrow(
            msg.sender,
            payable(marginFundsContract),
            borrowAmount
        ...
        returns (uint256)
    {
        ...
        uint256 totalMargin = depositInfo.marginAmount + depositInfo.marginReserve -
            depositInfo.lossValue +
            depositInfo.gainValue;
        uint256 loanAmount = depositInfo.loanAmount + depositInfo.usedLoan;

        uint256 maxBorrowAmount = 4 * totalMargin - loanAmount;

        return maxBorrowAmount;
    }

    function ltmRatio(address user, address assetAddress)
        public
        view
        returns (uint256)
    {
    ...
    function repay(
        ...
        Deposit storage depositInfo = depositDetails[msg.sender][assetAddress];
        // require(
        //     depositInfo.loanAmount >= repayAmount,
        //     "Margin: Excess repaid amount"
        // );
        address delegator = delegatorAddresses[assetAddress];
        require(delegator != address(0), "Margin: Not a delegator address");
        ...
        if(repayInterestAmount !=0) {
            uint256 ltiRatio = depositInfo.loanAmount * precision/depositInfo.interestAccumulated;
            (loanAmount, interestAmount) = getAmount(repayAmount, ltiRatio);
        }
        else {
            loanAmount = repayLoanAmount;
        }
        depositInfo.loanAmount -= loanAmount;
        ...
        //using the margin account
            ...
            if (assetAddress == address(0)) {
                //internal call to marginFundsContract
                IMarginFunds(marginFundsContract).withdrawFunds(payable(address(this)), repayAmount);
                ICEther(delegator).repayBorrowBehalf{
                    value: repayAmount
                }(msg.sender);
            } else {
                //internal call to marginFunds
                IMarginFunds(marginFundsContract).withdrawERC20Tokens(assetAddress, address(this), repayAmount);
                IERC20(assetAddress).approve(delegator, repayAmount);
                uint256 success = ICErc20Delegator(delegator).repayBorrowBehalf(
                    msg.sender,
                    repayAmount
        ...
        returns (uint256)
    {
        ...
        uint256 totalMargin = depositInfo.marginAmount -
            ...
            depositInfo.gainValue;
        uint256 loanAmount = depositInfo.loanAmount + depositInfo.usedLoan;
        uint256 maxRepayAmount = (4 * totalMargin - loanAmount) / 3;
    ...
    function setPositionAmount(address user, address assetAddress, uint256 marginUsed, uint256 loanUsed) public onlyPosition {
        Deposit storage depositInfo = depositDetails[user][assetAddress];
    ...
    function loanToInterest(address user, address assetAddress) public returns(uint256, uint256) {
        ...
        depositInfo.interestAccumulated = interestAccumulated;
        return (depositInfo.loanAmount, depositInfo.interestAccumulated);
        //return (depositInfo.loanAmount * precision/depositInfo.interestAccumulated);
    }
    ...
    function getAmount(uint256 repayAmount, uint256 ltiRatio) public view returns(uint256, uint256) {
        ...
        uint256 interestAmount = ((precision * repayAmount) / (ltiRatio + precision));
        return (loanAmount/precision, interestAmount/precision);
    }
}
...
contract MarginStorage {
    ...
    address internal marginFundsContract;

    address internal positionContract;
