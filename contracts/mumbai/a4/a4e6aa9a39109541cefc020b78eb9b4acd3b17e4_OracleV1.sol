...
library BetType {
    ...
    uint16 constant MatchHandicap = 701;
    uint16 constant MatchPointHandicap = 704;
    uint16 constant MatchPointOverUnder = 705;
    uint16 constant MatchPointOddEven = 706;
    uint16 constant MapXMoneyline = 9001;
    uint16 constant CorrectMapScore = 9088;
}
...
import "contracts/lib/oracle/BytesResult.sol";
...
import "contracts/lib/oracle/EventResultV1.sol";
import "contracts/lib/Status.sol";
import "contracts/lib/BetType.sol";
...
import "@openzeppelin/contracts/utils/Strings.sol";
import "contracts/lib/oracle/EventResultV2.sol";
import "contracts/lib/Status.sol";
import "contracts/lib/BetType.sol";
import "contracts/lib/SportType.sol";
...
import "@openzeppelin/contracts/utils/Strings.sol";
import "contracts/lib/oracle/EventResultV3.sol";
import "contracts/lib/Status.sol";
import "contracts/lib/BetType.sol";
import "contracts/lib/SportType.sol";
...
library EventHandlerV3 {
    function getScore(
        ...
        EventResultV3.Event memory result = abi.decode(
            resultBytes,
            (EventResultV3.Event)
        );
    ...
    function getEventStatus(
        ...
        if (betType == BetType.MapXMoneyline) {
            Status.MapStatus mapStatus = result.mapScores[resource1 - 1].status;
            if (mapStatus == Status.MapStatus.Completed) {
                return Status.ResultStatus.Completed;
            }
            if (mapStatus == Status.MapStatus.Refund) {
                return Status.ResultStatus.Refund;
            }
        }
        return result.status;
    }
...
import "contracts/lib/oracle/OutrightResultV1.sol";
...
library SportType {
    uint256 constant Soccer = 1;
    uint256 constant Basketball = 2;
    uint256 constant Tennis = 5;
    uint256 constant Baseball = 8;
    uint256 constant Badminton = 9;
    uint256 constant TableTennis = 18;
    uint256 constant Esports = 43;
}
...
import "contracts/interface/IOracleV1.sol";
import "contracts/lib/oracle/BytesResult.sol";
import "contracts/lib/oracle/BytesEventMap.sol";
import "contracts/lib/oracle/BytesOutrightMap.sol";
import "contracts/lib/oracle/EventHandlerV1.sol";
import "contracts/lib/oracle/EventHandlerV2.sol";
import "contracts/lib/oracle/EventHandlerV3.sol";
import "contracts/lib/oracle/OutrightHandlerV1.sol";
import "contracts/utils/BaseOwnable.sol";
