...
contract Resources_1 is Initializable, IResourceLogic {
    IBaseLogic public baseLogic;
    using Resources for Resources.ID;

    modifier onlyGameContract() {
        require(msg.sender == baseLogic.gameSupport(), "l");
        _;
    }
    ...
    function setBaseLogic(address _base) public {
        baseLogic = IBaseLogic(_base);
    }
    ...
    function checkResource(
        ...
        override
        onlyGameContract
        returns (uint16[] memory destroyPos, uint16[] memory removePos)
    {
        if (resourceId == uint16(Resources.ID.A1)) {
            a1(id, position);
        } else if (resourceId == uint16(Resources.ID.A2)) {
            a2(id, position);
        } else if (resourceId == uint16(Resources.ID.B2)) {
            destroyPos = b2(id, position);
        } else if (resourceId == uint16(Resources.ID.B3)) {
            destroyPos = b3(id, position);
        } else if (resourceId == uint16(Resources.ID.B4)) {
            b4(id, position, randomNumber);
        } else if (resourceId == uint16(Resources.ID.B5)) {
            destroyPos = b5(id, position);
        } else if (resourceId == uint16(Resources.ID.B6)) {
            b6(id, position);
        } else if (resourceId == uint16(Resources.ID.B7)) {
            b7(id, position);
        } else if (resourceId == uint16(Resources.ID.B8)) {
            b8(id, position, randomNumber);
        } else if (resourceId == uint16(Resources.ID.B12)) {
            b12(id, position);
        } else if (resourceId == uint16(Resources.ID.B13)) {
            destroyPos = b13(id, position);
        } else if (resourceId == uint16(Resources.ID.B14)) {
            b14(id, position, randomNumber);
        } else if (resourceId == uint16(Resources.ID.B15)) {
            destroyPos = b15(id, position);
        } else if (resourceId == uint16(Resources.ID.B16)) {
            destroyPos = b16(id, position);
        } else if (resourceId == uint16(Resources.ID.C3)) {
            destroyPos = c3(id, position);
        } else if (resourceId == uint16(Resources.ID.C5)) {
            c5(id, position);
        } else if (resourceId == uint16(Resources.ID.C6)) {
            destroyPos = c6(id, position);
        } else if (resourceId == uint16(Resources.ID.C8)) {
            c8(id, position, randomNumber);
        } else if (resourceId == uint16(Resources.ID.C9)) {
            c9(id, position, randomNumber);
        } else if (resourceId == uint16(Resources.ID.G3)) {
            destroyPos = g3(id, position);
        }
    }

    function a1(uint256 id, uint16 position) internal {
        // check adj position and interaction
        ...
        for (uint256 i = 0; i < adjacentPosition.length; ++i) {
            ...
            if (
                adjResource == Resources.ID.L1 ||
                adjResource == Resources.ID.D1 ||
                adjResource == Resources.ID.B16 ||
                adjResource == Resources.ID.M10
            ) {
                baseLogic.boostPayout(id, uint16(Resources.ID.A1), position, 1);
            }
        }
    }

    function a2(uint256 id, uint16 position) internal {
        if (
            position == 0 || position == 3 || position == 16 || position == 19
        ) {
            IGameController(baseLogic.gameController()).updatePayout(
                id,
                position,
    ...
    function b2(
        ...
        for (uint8 i = 0; i < adjacentPosition.length; ++i) {
            ...
            if (adjResource == Resources.ID.T3) {
    ...
    function b3(
        uint256 id,
        uint16 position
    ) internal returns (uint16[] memory res) {
        uint256 countSpin = IGameController(baseLogic.gameController()).getProp(
            id,
            position
        );
        --countSpin;
        if (countSpin == 0) {
            uint16[] memory destroyPos = new uint16[](1);
        ...
        baseLogic.addNewResource(id, uint256(Resources.ID.B15), 1);
    ...
    function b4(uint256 id, uint16 position, uint256 random) internal {
        uint256 percent = ((random / 10 ** position) % 100000) % 100;
        if (
            (baseLogic.isItemExist(id, uint256(Items.ID.IC12), false) &&
                percent < 20) || percent < 10
        ) {
            // uint256 rand = ((random / 100 ** position) % 100000) % 4;
            uint256 rand = Helper.calRandomHelper(position, random, 100000, 4);

            if (rand == 0 || rand == 1 || rand == 2 || rand == 3) {
                baseLogic.addNewResource(
                    id,
                    uint256(
                        rand == 0 ? Resources.ID.B9 : rand == 1
                            ? Resources.ID.W4
                            : rand == 2
                            ? Resources.ID.M3
                            : Resources.ID.C6
    ...
    function b5(
        ...
        for (uint8 i = 0; i < adjacentPosition.length; ++i) {
            ...
            if (adjResource == Resources.ID.H10) {
                destroyPos[adjPosition] = adjPosition;
                ++countDestroy;
                IEventContract(baseLogic.eventContract()).emitEffect(
                    id,
                    1,
    ...
    function b6(uint256 id, uint16 position) internal {
        uint16[] memory adjacentPosition = Helper.getAdjacentPositions(
            position,
            baseLogic.checkAllAdjacent(id, position)
        );
    ...
    function b7(uint256 id, uint16 position) internal {
        uint16[] memory adjacentPosition = Helper.getAdjacentPositions(
            position,
            baseLogic.checkAllAdjacent(id, position)
        );
        for (uint8 i = 0; i < adjacentPosition.length; ++i) {
            ...
            if (
                adjResource == Resources.ID.F3 ||
                adjResource == Resources.ID.B8 ||
                adjResource == Resources.ID.H10
            ) {
                IGameController(baseLogic.gameController()).updatePayout(
                    id,
                    adjPosition,
    ...
    function b8(uint256 id, uint16 position, uint256 random) internal {
        uint256 percent = ((random / 10 ** position) % 100000) % 100;
        if (
            (baseLogic.isItemExist(id, uint256(Items.ID.IC12), false) &&
                percent < 20) || percent < 10
        ) {
            baseLogic.addNewResource(id, uint256(Resources.ID.H10), 1);
        }
    }

    function b12(uint256 id, uint16 position) internal {
        uint16[] memory adjacentPosition = Helper.getAdjacentPositions(
            position,
            baseLogic.checkAllAdjacent(id, position)
        );
        for (uint8 i = 0; i < adjacentPosition.length; ++i) {
            ...
            if (
                adjResource == Resources.ID.C4 || adjResource == Resources.ID.W4
            ) {
                IGameController(baseLogic.gameController()).updatePayout(
                    id,
                    position,
    ...
    function b13(
        ...
        for (uint8 i = 0; i < adjacentPosition.length; ++i) {
            ...
            if (
                adjResource == Resources.ID.T3 ||
                (baseLogic.isItemExist(id, uint256(Items.ID.IZ1), false) &&
                    IGameResource(baseLogic.gameResource()).contains(
                        "human",
                        uint256(adjResource)
                    ))
                ...
                coin += 20;
                if (adjResource == Resources.ID.T3) coin += 20;
            }
        }
        if (coin != 0) {
            IGameController(baseLogic.gameController()).updatePayout(
    ...
    function b14(
        ...
        uint16[] memory res = IResource_0(baseLogic.resourceLogic(0)).arrow(
            id,
            position,
            1,
            random
    ...
    function c3(
        ...
        for (uint8 i = 0; i < adjacentPosition.length; ++i) {
            ...
            if (adjResource == Resources.ID.M11) {
                destroyPos[countDestroy] = adjPosition;
                ++countDestroy;
                IEventContract(baseLogic.eventContract()).emitEffect(
                    id,
    ...
    function c5(uint256 id, uint16 position) internal {
        uint16[] memory adjacentPosition = Helper.getAdjacentPositions(
            position,
            baseLogic.checkAllAdjacent(id, position)
        );
        for (uint256 i = 0; i < adjacentPosition.length; ++i) {
            ...
            if (
                ...
                if (adjResource == uint256(Resources.ID.P2)) {
                    baseLogic.boostPayout(
                        id,
                        uint16(Resources.ID.P2),
                        adjPosition,
                        1
                    );
                }
    ...
    function c6(
        ...
        IInventory(baseLogic.inventory()).addItem(
            id,
            uint256(Items.ID.IL10) + 1000
        );
    }

    function c8(uint256 id, uint16 position, uint256 random) internal {
        uint256 percent = ((random / 10 ** position) % 100000) % 100;
        if (percent < 10) {
            // TODO Why index not used
            // uint256 index = IGameController(baseLogic.gameController()) getIndex(id, position);
            IGameController(baseLogic.gameController()).updateMap(
                id,
                position,
                uint256(Resources.ID.C9)
            );
        }
        if (baseLogic.isItemExist(id, uint256(Items.ID.IC4), false)) {
            IGameController(baseLogic.gameController()).updatePayout(
                id,
                position,
                0,
    ...
    function c9(uint256 id, uint16 position, uint256 random) internal {
        uint256 percent = ((random / 10 ** position) % 100000) % 100;
        uint256 chance = 1;
        bool isIC12Exist = baseLogic.isItemExist(
            id,
            uint256(Items.ID.IC12),
            false
        );
        bool isIC4Exist = baseLogic.isItemExist(
            id,
            uint256(Items.ID.IC4),
            false
        );
        if (isIC12Exist) {
            chance *= 2;
        ...
        if (percent < chance) {
            baseLogic.addNewResource(id, uint256(Resources.ID.G5), 1);
        }
        percent = ((random / 10 ** (position + 5)) % 100000) % 100;
        if (percent < 5 * chance) {
            baseLogic.addNewResource(id, uint256(Resources.ID.E1), 1);
        }
    ...
    function g3(
        ...
        for (uint256 i = 0; i < adjacentPosition.length; ++i) {
            ...
            if (
                ...
                adjResource == Resources.ID.P3 ||
                adjResource == Resources.ID.S5 ||
                adjResource == Resources.ID.S3 ||
        ...
        if (countDestroy > 0) {
            baseLogic.boostPayout(
                id,
                uint16(Resources.ID.G3),
                position,
                int256(countDestroy - 1)
            );
        }
    }
}
