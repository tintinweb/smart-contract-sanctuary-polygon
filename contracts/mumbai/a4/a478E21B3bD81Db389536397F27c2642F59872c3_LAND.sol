...
contract LAND is ERC721, Ownable {
    ...
    function mintNewParcel(
        int256 x,
        int256 y,
        address to
    ) external onlyOwner {
        _safeMint(to, getParcelId(x, y));
        _setTokenURI(getParcelId(x, y), Strings.toString(getParcelId(x, y)));
    }

    function mintMultipleParcels(
        int256[] calldata x,
        int256[] calldata y,
        address to
    ) external onlyOwner {
        for (uint256 i = 0; i < x.length; i++) {
            _safeMint(to, getParcelId(x[i], y[i]));
            _setTokenURI(getParcelId(x[i], y[i]), Strings.toString(getParcelId(x[i], y[i])));
        }
    }
    ...
    function getParcelId(int256 x, int256 y) public pure returns (uint256) {
    ...
    function _exists(int256 x, int256 y) internal view returns (bool) {
        return _exists(getParcelId(x, y));
    }
    ...
    function transferLand(
        ...
        uint256 tokenId = getParcelId(x, y);
    ...
    function transferManyLand(
        ...
        for (uint256 i = 0; i < x.length; i++) {
            uint256 tokenId = getParcelId(x[i], y[i]);
            _transfer(ownerOf(tokenId), to, tokenId);
        }
    }
    ...
    function transferLandToEstate(
        ...
        require(
            ownerOf(estateId) == msg.sender,
            "You must own the Estate you want to transfer to"
        );

        uint256 tokenId = getParcelId(x, y);
        _transfer(ownerOf(tokenId), address(estateRegistry), tokenId);
    }
