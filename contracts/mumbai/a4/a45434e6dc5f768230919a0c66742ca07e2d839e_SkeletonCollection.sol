 ...
 */
contract SkeletonCollection is Initializable, ICollection, AccessControlUpgradeable, OwnableUpgradeable {
  /**
...
/**
  ...
  *
  */
uint8 public marketplaceSellerFeePercent;
/**
  * @notice Stores the Marketplace Fee.
  *
  */
uint8 public marketplaceBuyerFeePercent;

/**
  * @notice Stores the partner Fee for Marketplace.
  *
  */
uint8 public partnerMarketplaceFeePercent;

/**
  * @notice Stores the partner Fee.
  *
  */
uint public partnerFeePercent;

/**
  * @notice Stores the partner Fee.
  *
  */
uint public partnerCommissionPercent;
...
/**
  ...
  modifier validCardType(uint8 _cardType) {
    require(_cardType >= 1 && _cardType <= 53, "must be in [1, 53]");
    _;
  }
 /**
   * @dev Fired in commit()
...
/**
 ...
 /**
   ...
   */
   function initialize (address _card, address payable _daoAddress,  address payable _gameTreasury, address payable _treasuryReserve, address payable _partnerDAOAddress, address _couponSigner) public initializer{
      ...
      collectionId = 3;
      buyBackValue = 10 * 10**18; // 100.000 Matic 
      ...
      cardValue = [
        ...
        mintBasePrice = 97088534040000;
        mintFeePercent = 2_000;
        upgradeFee = 2_000;
        marketplaceSellerFeePercent = 5;
        marketplaceBuyerFeePercent = 5;
...
/**
 ...
 function depositForFreeMint(uint32 _mintCount) public payable{
    require(_mintCount >= 1 && _mintCount <= 1000, "invalid mintCount");
...
function mintCardsCost(uint _amount, Coupon memory _coupon, uint _availableAmount, bool _freeMint) public view returns (uint256 cost, uint256 collectionMintFee, uint256 partnerFee) {
  ...
  if(_freeMint == false){
    ...
    collectionMintFeeWithDiscount = checkDiscount(collectionMintFee, _coupon, _availableAmount);
  }
...
function mintJoker(uint8 _amount, Coupon memory _coupon, uint8 _availableAmount) public {
  require(_amount >= 1 && _amount <= 100, "invalid amount");
  require (_amount <= jokersCount, "not enough jokers");
...
function commit(bytes32 dataHash, uint8 _amount, Coupon memory _coupon, uint8 _availableAmount, bool _freeMint) public payable{
    require(mintingPaused == false, "paused");
    require(commits[msg.sender].amount == 0, "C already exists");
    require(_amount >= 1 && _amount <= 100, "invalid amount");

    if(_freeMint == true){
      require(_amount >= 1 && _amount <= freeMintCount, "invalid amount");
    }
...
function reveal(bytes32 revealHash) public {
    ...
    require(getHash(revealHash)==commits[msg.sender].commit, "invalid C");
    //require that the block number is greater than the original block
    require(uint64(block.number)>commits[msg.sender].block, "invalid block");
...
function upgradeCardCost(uint160[] calldata _tokenIds, uint8 _targetCardType, uint8 _collectionId, Coupon memory _coupon, uint _discount) public view validCardType(_targetCardType) returns (UpgradeCost memory) {
    // Ensure at least 2 cards was sent
    require(_tokenIds.length > 1, "not enough cards");
    ...
    for(uint32 i; i < _tokenIds.length;) {
      ...
      if(upgradecost.totalValueGivenCards > upgradecost.targetCardTypeValue ){
        upgradecost.cardsTypeValueDiff = upgradecost.totalValueGivenCards - upgradecost.targetCardTypeValue;
      }
        ...
        if(_collectionId == collectionId){
          ...
          upgradecost.partnerFeeCost = upgradecost.upgradeFeeCostWithDiscount * partnerFeePercent / 100_000;

          upgradecost.upgradeFeeCostWithDiscount -= upgradecost.partnerFeeCost;
        } else{
    ...
    upgradecost.cardsTypeValueDiffCost = (upgradecost.cardsTypeValueDiff * buyBackValue / 2) / 10**8;
    //check if user can pay less fee (fee-change)
    if(upgradecost.upgradeFeeCostWithDiscount > upgradecost.cardsTypeValueDiffCost){
     upgradecost.payValue = upgradecost.upgradeFeeCostWithDiscount - upgradecost.cardsTypeValueDiffCost;
     upgradecost.refundValue = 0;
    }else{
     upgradecost.payValue = 0;
     upgradecost.refundValue = upgradecost.cardsTypeValueDiffCost - upgradecost.upgradeFeeCostWithDiscount;
    }
...
/**
  ...
  function upgradeCards(uint160[] calldata _tokenIds, uint8 _targetCardType, uint8 _collectionId,  Coupon memory _coupon, uint _discount) public payable validCardType(_targetCardType) {
    // Ensure at least 2 cards was sent
    require(_tokenIds.length > 1, "not enough cards");
    ...
    require(upgradeinfo.collectionBuyBack >= buyBackValue, "invalid to collection");
    
    
    //calculate card upgrade cost
    upgradecost  = upgradeCardCost(_tokenIds, _targetCardType, _collectionId, _coupon, _discount);
    
    require(msg.value >= upgradecost.payValue, "not enough funds");
      
    upgradeinfo.collectionsRatio = upgradeinfo.collectionBuyBack  * 10**8 / buyBackValue ;
     
    card.burnBatch(msg.sender, _tokenIds, 0);
       
    upgradeinfo.totalValue = upgradecost.totalValueGivenCards * 10**8 / upgradeinfo.collectionsRatio;
    // Value of all cards provided (after subtracting attribution)
    // should be at least equal to the value of the new card to be
    // minted
    require(upgradeinfo.totalValue >= cardValue[_targetCardType] * upgradeinfo.collectionsRatio / 10**8, "not enough");
    require(upgradecost.highestCardValue <= cardValue[_targetCardType], "invalid target card");
    
    {
      
      if(upgradecost.partnerFeeCost > 0){
        // Send upgrade fee to  partner DAO
        sendUpgradeFeeToPartner(upgradecost.upgradeFeeCost, upgradecost.partnerFeeCost, msg.value);
      }
      ...
      if(upgradecost.payValue > 0){
        ...
        // Send upgrade fee to DAO From Treasury
         gametreasury.upgradeCommission(daoAddress, upgradecost.upgradeFeeCostWithDiscount);
      }
      
      //claim commitions
      claimUpgradeCommission(upgradecost.cardsTypeValueDiffCost, upgradecost.refundValue);
    }
...
function claimUpgradeCommission(uint256 cardsTypeValueDiffCost, uint256 refundValue) private {
      ...
      commissioninfo.upgradeCommission = cardsTypeValueDiffCost;
...
function calculateBuyback(uint160[] calldata _tokenIds) public view returns (uint256 floorValue, uint256 bonusEarned, uint256 buyback, bool allAces) {
    ...
    for(uint8 i = 0; i < _tokenIds.length;) {
        ...
        if(cardType == 53) {
          ...
          jokers++;
          require(jokers <= 4, "invalid Jokers");
        } else {
          // If not a joker, check if this card type
          // has already been used
          require(!payoutDeck[cardType], "type used twice");
  ...
  for(uint8 i = 1; i < payoutDeck.length;) {
    ...
    for(uint8 j=0; j<4;){
     for(uint8 i=1; i<13;){ // no aces for bonus
        if(slotMatrix[i][j]){
  ...
  function claimBuyBack (uint160[] calldata _tokenIds) public {
    ...
    if(buybackMandatoryAces == true){
      require(allAces == true, "A not provided");
    }
    commissioninfo.claimCommission = (floorValue * 2) - buyback;
    commissioninfo.partnerClaimCommission = commissioninfo.claimCommission * partnerCommissionPercent / 100_000;
    if(buyback > 0){
      ...
      claimBuyBackCommission(commissioninfo.claimCommission,  commissioninfo.partnerClaimCommission);
...
/**
	...
	*/
	function setPartnerFee(uint _newFee) external onlyOwner {
		// Update partnerFee
    require(_newFee < 100, "should be < 100");
  ...
  /**
	...
	*/
	function setPartnerCommission(uint _newCommission) external onlyOwner {
		// Update partnerFee
    require(_newCommission < 100, "should be < 100");
...
/**
	...
	*/
	function setPartnerMarketplaceFee(uint8 _newPartnerMarketplaceFee) external onlyOwner {
		// Update partnerMarketplaceFeePercent
    require(_newPartnerMarketplaceFee < 100, "should be < 100");
...
/**
	...
	*/
  function setMarketplaceSellerFeePercent(uint8 _marketplaceSellerFeePercent) external onlyOwner {
     require(_marketplaceSellerFeePercent >=0 && _marketplaceSellerFeePercent <=100, "wrong %");
  ...
  /**
	...
	*/
  function setMarketplaceBuyerFeePercent(uint8 _marketplaceBuyerFeePercent) external onlyOwner {
        require(_marketplaceBuyerFeePercent >=0 && _marketplaceBuyerFeePercent <=100, "wrong %");
 ...
 */
contract CardNFTV1 is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, AccessControlUpgradeable, OwnableUpgradeable {
  using Address for address;
  ...
  * @dev Fired in addCollection()
  ...
  function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
    ...
    string memory imageUrl = string(abi.encodePacked('"image_url": "', collectionUri[cardInfo[_tokenId].cardCollection], cardInfo[_tokenId].cardType.toString(), '.png",'));
 ...
 /**
   ...
   * @dev Access restricted by `URI_MANAGER_ROLE` AccessControl role
	 ...
	 */
  function setBaseURIForCollection(uint8 collectionId, string memory _newBaseURI) external onlyRole(URI_MANAGER_ROLE) {
     // Update base uri of the collection
    collectionUri[collectionId] = _newBaseURI;
  }
  ...
  function burnBatch(address _to, uint160[] memory _tokenIds, uint8 _type) external onlyRole(BURNER_ROLE) {
   ...
   unchecked{ totalBurned += _tokenIds.length; }
  ...
  function mintBatch(address _to, uint160 _tokenId, uint128 _n, uint8 _cardType, uint8 _cardCollection) public onlyRole(MINTER_ROLE) validCardType(_cardType) {
    ...
    unchecked{ cardPopulation[_cardType] += _n; }
    unchecked{ cardCollectionPopulation[_cardCollection][_cardType] += _n; }
    unchecked{ cardCollectionInfo[_cardCollection].colMinted += _n; }
   ...
   * @param _data additional data with no specified format, sent in call to `_to`
	 */
  function safeMint(address _to, uint160 _tokenId, uint8 _cardType, uint8 _cardCollection, bytes memory _data) external {
    ...
    if(_to.isContract()) {
      // Try calling the onERC721Received function on the to address
		  try IERC721ReceiverUpgradeable(_to).onERC721Received(msg.sender, address(0), _tokenId, _data) returns (bytes4 retval) {
        require(retval == IERC721ReceiverUpgradeable.onERC721Received.selector, "invalid onERC721Received response");
  ...
  function safeMintBatch(address _to, uint160 _tokenId, uint128 _n, uint8 _cardType, uint8 _cardCollection, bytes memory _data) public {
    ...
    if(_to.isContract()) {
      ...
      for(uint256 i = 0; i < _n; i++) {
        // Try calling the onERC721Received function on the to address
        try IERC721ReceiverUpgradeable(_to).onERC721Received(msg.sender, address(0), _tokenId + i, _data) returns (bytes4 retval) {
          require(retval == IERC721ReceiverUpgradeable.onERC721Received.selector, "invalid onERC721Received response");
        // If onERC721Received function reverts
        } catch (bytes memory reason) {
          // If there is no revert reason, assume function
   ...
   */
  function _beforeTokenTransfer(address _from, address _to, uint256 _tokenId, uint256 _batchSize) internal virtual override(ERC721Upgradeable, ERC721EnumerableUpgradeable) {
  ...
  function increaseCollectionBuyBacks(uint8 _collection) external onlyRole(MINTER_ROLE){
...
contract GameTreasury is Initializable, AccessControlUpgradeable, OwnableUpgradeable{
...
contract TreasuryReserve is AccessControl, Ownable, ReentrancyGuard{
  ...
  function buyBackPayout (address _to, uint256 _buyBack) external onlyRole(TREASURER_ROLE){
    require(_to != address(0), "zero address not accepted");
    // Give prize to user
    (bool success, ) = payable(_to).call{value: _buyBack}("");
    require(success, "Transfer failed.");
  }
   ...
   function giveUpgradeCacheBack (address _to, uint256 _cacheBackValue) external onlyRole(TREASURER_ROLE){
    ...
    (bool success, ) = payable(_to).call{value: _cacheBackValue}("");
    require(success, "Transfer failed.");
...
/**
  * @notice Collection interface for getting collection fees data
   ...
   * EXAMPLE:
   * 
   * uint8 public marketplaceFeePercent = 5;
   * 
   * uint8 public marketplaceBuyerFeePercent = 5;
   * 
   * uint8 public partnerMarketplaceFeePercent = 10;
   *
   */
...
interface ICollection {
    ...
    function marketplaceSellerFeePercent() external view returns(uint8);
    function marketplaceBuyerFeePercent() external view returns(uint8);
    function partnerMarketplaceFeePercent() external view returns(uint8);
}
