...
contract VRFBeacon is VRFBeaconOCR, VRFBeaconDKGClient {
    struct EnrollRequest {
        address addressToEnroll;
        bytes publicKeyBytes;
    }

    EnrollRequest private s_mostRecentEnrollRequest;

    mapping(address => uint256) private s_balances;
    mapping(address => bytes[]) private s_enrollmentCiphers;
    ...
    function requestAccountRecoveryEnrollment(bytes calldata publicKeyBytes)
        external
    {
        s_mostRecentEnrollRequest = EnrollRequest({
            addressToEnroll: msg.sender,
            publicKeyBytes: publicKeyBytes
        });
    }

    function getMostRecentEnrollRequest()
        external
        view
        returns (EnrollRequest memory)
    {
        return s_mostRecentEnrollRequest;
    }

    function postCipher(
        address user,
        bytes calldata cipher1,
        bytes calldata cipher2
    ) external {
        s_enrollmentCiphers[user].push(cipher1);
        s_enrollmentCiphers[user].push(cipher2);
    }

    function getEnrollmentCiphers(address user)
        external
        view
        returns (bytes[] memory)
    {
        return s_enrollmentCiphers[user];
    }
...
contract VRFBeaconReport is VRFBeaconTypes, OwnerIsCreator {
    ...
    function _report(
        ...
        OutputServed[] memory outputsServed = COORDINATOR.processVRFOutputs(
            report.outputs,
