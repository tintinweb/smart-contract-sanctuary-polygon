 ...
 * account.
 */
abstract contract ERC721Enumerable is ERC721Upgradeable, IERC721Enumerable {
...
contract MyToken is Initializable, ERC721Enumerable, OwnableUpgradeable, UUPSUpgradeable {
    /// @custom:oz-upgrades-unsafe-allow constructor
  ...
  //50000000000000000000 Wei 10^-18 = 50 MATIC
  //50000000000000000 Wei 10^-18 = 0.05 MATIC
  ...
  function _baseURI() internal view virtual override returns (string memory) {
    ...
    function mintPresaleLand(uint256 _mintAmount, uint256[] memory index) public payable {
    ...
    if (msg.sender != owner()) {
      if(onlyWhitelisted == true) {
        ...
        if(_mintAmount == 1){
          totalPrice = (_mintAmount * cost) * 1;
        }
        else if(_mintAmount == 9){
          totalPrice = (_mintAmount * cost) * 95 / 100;
          // totalPrice = 
        }
        else if(_mintAmount == 36){
          totalPrice = (_mintAmount * cost) * 90 / 100; 
          totalPrice = totalPrice * 95;
        }
        else if(_mintAmount == 144){
          totalPrice = (_mintAmount * cost) * 85 / 100; 
          totalPrice = totalPrice * 90 / 100;
        }
        else if(_mintAmount == 576){
          totalPrice = (_mintAmount * cost) * 80 / 100;
    ...
    for (uint256 i = 1; i <= _mintAmount; i++) {
        ...
        _safeMint(msg.sender, index[i-1]);
