...
interface ISquishilandSaleDelegator {
    ...
    /**
     * @notice Reserve and mint
     * @param tokenIds Any token id that is 0 will be assumed as a mint
     */
    function reserveMintMany(
     ...
     */
    function mintMany(uint256[] calldata internalIds) external payable;
     ...
     */
    function getAvailableInternalIds() external view returns (uint256[] memory);

    /**
     * @notice Get available land plots
     */
    function getUnavailableInternalIds() external view returns (uint256[] memory);
...
contract SquishilandSaleDelegator is
    ISquishilandSaleDelegator,
    Initializable,
    OwnableUpgradeable,
    UUPSUpgradeable
{
    ...
    function initialize(ISquishiland land) external initializer {
        _land = land;
    ...
    function reserveMintMany(
        ...
        for (uint256 i; i < internalIds.length; i++) {
            ...
            uint256 internalId = internalIds[i];
            if (tokenId == 0) {
                _mint(internalId);
            } else {
                require(
                    _land.ownerOf(tokenId) == msg.sender,
                    "!land_non-owner"
                );
     ...
     * @notice Mint specific coordinates
     */
    function mintMany(uint256[] calldata internalIds) public payable {
     ...
     */
    function _mint(uint256 internalId) private {
        ISquishiland.LandSize size = getLandSize(internalId);
    ...
    function _reserve(uint256 tokenId, uint256 internalId) internal {
        ...
        require(
            getLandSize(tokenId) == getLandSize(internalId),
            "!size_mismatch"
        );
    ...
    /**
     * @notice Get the land size from the internal id
     */
    function getLandSize(uint256 internalId)
        private
        pure
        returns (ISquishiland.LandSize)
    {
        if (internalId < 50) {
            return ISquishiland.LandSize.Mythic;
        } else if (internalId < 450) {
            return ISquishiland.LandSize.Legendary;
        } else if (internalId < 1950) {
            return ISquishiland.LandSize.Epic;
        }
        return ISquishiland.LandSize.Rare;
    }
