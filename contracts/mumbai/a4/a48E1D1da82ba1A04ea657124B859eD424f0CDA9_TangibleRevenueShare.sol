...
contract TangibleRevenueShare is RevenueShare, AccessControl {
    ...
    mapping(address => uint256) private _isRegisteredNFT;
    ...
    function updateShare(
        ...
        totals[cycle] += amount;
        if (_isRegisteredNFT[contractAddress] == 0) {
            _contracts.push(contractAddress);
            _isRegisteredNFT[contractAddress] = _contracts.length;
        }
    }
    ...
    {
        uint256 index = _isRegisteredNFT[contractAddress];
        if (index != 0) {
            uint256 numContracts = _contracts.length;
