...
contract Test{
    ...
    event _buyGameSlot(address  _user, uint256 amount);
    ...
    function buyGameSlot(address userAddress) payable public 
    {
        require(isUserExists(userAddress), "User not exists!");        
        require(((msg.value*maticRate)/1e18)>=1, "Minimum 10 USD!");
        uint32 size;
        assembly {
            size := extcodesize(userAddress)
        }
        ...
        emit _buyGameSlot(userAddress, msg.value);
    }
