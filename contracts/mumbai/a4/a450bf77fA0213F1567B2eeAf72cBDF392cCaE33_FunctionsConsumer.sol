...
// some view functions are not needed

contract Campaign is Ownable {
  error IncorrectAmount(uint256);
  error IncorrectToken(address);
  error CampaignEnded();
  ...
  constructor(uint256 campaignDuration, string memory _exchangeName, address _tokenA, address _tokenB) {
    startBlock = block.number;
    endBlock = block.number + campaignDuration;
    exchangeName = _exchangeName;
    tokenA = _tokenA;
    tokenB = _tokenB;
  }

  function getCampaignInfo() public view returns (string memory, address, address, uint, uint) {
    return (exchangeName, tokenA, tokenB, startBlock, endBlock);
  }

  function getPendingWithdrawals(address _beneficiary) public view returns (uint256) {
    return pendingWithdrawals[_beneficiary];
  }

  function getClaimedAmount(address _beneficiary) public view returns (uint256) {
    return claimedAmount[_beneficiary];
  }
  ...
  function updatePendingWithdrawal(address _beneficiary, uint256 _amount) external onlyOwner {
    pendingWithdrawals[_beneficiary] += _amount;
  }

  function updateClaimedAmount(address _beneficiary, uint256 _amount) external onlyOwner {
    claimedAmount[_beneficiary] += _amount;
  }

  // beneficiary can call this directly
  function withdrawTokens(address _token) external {
    uint256 amount = pendingWithdrawals[msg.sender];
    if (amount == 0) {
      revert IncorrectAmount(0);
    }
    if (_token != tokenA && _token != tokenB) {
      revert IncorrectToken(_token);
    }
...
//TODO : Add upgradeable inheritance that doesnt clash with ConfirmedOwnerImplementation
//TODO : Save gas with campaign implementation
//TODO : add contract to track structs, events and errors

// function deposit() is not needed

contract FunctionsConsumer is FunctionsClient, ConfirmedOwner {
  using Functions for Functions.Request;
  event OCRResponse(bytes32 indexed requestId, bytes result, bytes err);
  event NewCampaign(address campaign);
  event LiquidityScoreUpdated(uint256 indexed id, uint256 score);
  ...
  uint256 public campaignsCount = 1;
  mapping(uint256 => address) public campaigns;
  mapping(uint256 => uint256) public latestLiquidityScore;
  address campaignImplementation; // will use this later to save gas
  bool initalized;

  constructor(address oracle) FunctionsClient(oracle) ConfirmedOwner(msg.sender) {}

  modifier whenInitilized(bool isInitialized) {
    if (!initalized == isInitialized) revert initialized(isInitialized);
    _;
  }

  function initialize(address _implementation) external onlyOwner whenInitilized(false) {
  ...
  function createCampaign(
    ...
    address _tokenA,
    address _tokenB
  ) external whenInitilized(true) returns (address) {
    Campaign campaign = new Campaign(_campaignDuration, _exchangeName, _tokenA, _tokenB);
    campaigns[campaignsCount] = address(campaign);
    campaignsCount += 1;
    emit NewCampaign(address(campaign));
    return address(campaign);
  }

  function getDeployedCampaigns() public view returns (address[] memory campaigns_) {
    campaigns_ = new address[](campaignsCount - 1);
    for (uint256 i = 1; i < campaignsCount; i++) {
      campaigns_[i - 1] = campaigns[i];
    }
  }

  // Claim rewards
  function updatePendingBalance(address _beneficiary, uint256 _campaignId) external onlyOwner {
    Campaign campaign = Campaign(campaigns[_campaignId]);

    string memory exchangeName = campaign.exchangeName();
    uint256 claimableAmount = 0;
    // --------- TODO -------------------------
    // TODO : add logic for oracle or use LiquidityScore
    // LiquidityOracle oracle = LiquidityOracle(oracleContractAddress);
    // uint256 totalRewardAmount = (oracle.getProviderVolume(exchangeName, beneficiary) *
    //   getCampaignBalance(_campaignAddress, _token)) / oracle.getPoolTotalVolume(exchangeName);
    // uint256 alreadyClaimedAmount = campaign.claimedAmount(beneficiary);
    // uint256 claimableAmount = totalRewardAmount - alreadyClaimedAmount;
    if (claimableAmount == 0) revert IncorrectAmount(claimableAmount);

    campaign.updatePendingWithdrawal(_beneficiary, claimableAmount);
    campaign.updateClaimedAmount(_beneficiary, claimableAmount);
  }

  function getCampaignLiquidityScore(uint256 _id) external view returns (uint256) {
    return latestLiquidityScore[_id];
  }

  ///--------------------------CHAINLINK FUNCTIONS-----------------------------------
  ///--------------------------CHAINLINK FUNCTIONS-----------------------------------
  ///--------------------------CHAINLINK FUNCTIONS-----------------------------------
  // previously RequestTradingInfo
  ...
  function fulfillRequest(bytes32 requestId, bytes memory response, bytes memory err) internal override {
    (uint256 id, uint256 liquityScore) = abi.decode(response, (uint256, uint256));
    latestLiquidityScore[id] = liquityScore;
    emit LiquidityScoreUpdated(id, liquityScore);
