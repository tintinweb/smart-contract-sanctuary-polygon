...
contract protoKol is iProtoKol {
    ...
    // In local
    //IN Matic
    // address private constant UNISWAP_ROUTER_ADDRESS = 0xa89cbe6F2Bcd58Bbbb577CC269238F13C1a0a8c0;
    // address private factory = 0x9E6Fa3FB6e1E2AA49B709f7dd91fdC722e96bAD7;

    // address private _USDT = 0x9E6Fa3FB6e1E2AA49B709f7dd91fdC722e96bAD7;
    // address private _stakingContract = 0xbE324F045CE03C2D640c0c435da289413a725b67;
   ...
   */
    function createCampaign(
        ...
        recSig(
            ...
            s,
            v,
            keccak256(abi.encodePacked(_newCampaignInput.campaignOwner, _newCampaignInput.startDate, _tgeDate))
        );
        //add TGE Per
   ...
   */
    function _swapRouter(
        ...
        (bool success, bytes memory data) = path[0].call(
            abi.encodeWithSelector(0x095ea7b3, UNISWAP_ROUTER_ADDRESS, _investment)
        );
        require(
        ...
        // console.log("amount output", amount[1]);
    }
    ...
    function investInCampaign(
        ...
        if (campaigns[_campaignId].campaignData.preSaleToken != _USDT) {
            ...
            if (tokenAddress == kolToken) {
                campaigns[_campaignId].campaignData.isKolInvestment = true;
                ...
                require(
                    // (
                    _investment <= campaigns[_campaignId].campaignData.remainingInvestment, //* (10000 - _transactionPer)) / 10000
                ...
                // IKOLT(kolToken).transferFrom(msg.sender, address(this), _totalInvestment);
                // IKOLT(kolToken).transfer(_vault, _totalInvestment - _investment);
                ...
                _investment = (amountInUsdt * 1e18) / 10**IERC20(_USDT).decimals();
            } else {
                campaigns[_campaignId].campaignData.isKolInvestment = false;
                _investment = (_investment * (10000 - _transactionPer)) / 10000;
                require(
                    _investment <= campaigns[_campaignId].campaignData.remainingInvestment,
                    "Protokol:Amount>Investment"
                );
                uint256 amountInUsdt = (_investment * 10**IERC20(_USDT).decimals()) / 1e18;
                ...
                _safeTransferFrom(
                    ...
                    msg.sender,
                    _vault,
                    ((_totalInvestment * 10**IERC20(_USDT).decimals()) / 1e18) - amountInUsdt
                );
            }
        }
        ...
        campaigns[_campaignId].campaignData.remainingInvestment =
            campaigns[_campaignId].campaignData.remainingInvestment -
            _investment;
        // console.log("Investment here->", campaigns[_campaignId].campaignData.remainingInvestment);
   ...
   */
    function generateTGE(uint256 _campaignId, address token) external {
        ...
        // IPST(campaigns[_campaignId].campaignData.preSaleToken).transferFrom(
        //     msg.sender,
        //     _vault,
        ...
        console.log("tgeAmount", campaigns[_campaignId].tgeDetails.tgeAmount);

        campaigns[_campaignId].campaignData.presaleAmount += campaigns[_campaignId].tgeDetails.tgeAmount;

        // campaigns[_campaignId].campaignData.preSaleBalance += campaigns[_campaignId].tgeDetails.tgeAmount;
        VestingDetails memory _vestingInfo;
    ...
    function claimKOLInvestment(uint256 _campaignId) external {
        ...
        if (_investment > campaigns[_campaignId].campaignData.enteredInvestmentAgainstMarketingBudget)
            _investment =
                campaigns[_campaignId].campaignData.enteredInvestmentAgainstMarketingBudget -
                campaigns[_campaignId].campaignData.investmentClaimed;
        // console.log("534 _investment", _investment);
        // _investment = (_investment * 1 * 10**IERC20(_USDT).decimals()) / 1e18;
        // uint256[] memory amount = UniswapV2Library.getAmountsOut(factory, _investment, path);

        // amount = _swapRouter(_investment, 1, path, msg.sender);
        // console.log("538 amount[1]", amount[1]);
   ...
   */
    function depositPreSaleTokens(uint256 _campaignId, uint256 _amount) external {
        ...
        // _amount = (_amount * 10**IPST(token).decimals()) / 10**18;

        // uint256 markBudge = (campaigns[_campaignId].campaignData.marketingBudget * 10**IPST(token).decimals()) / 10**18;
        uint256 _investment = campaigns[_campaignId].campaignData.enteredInvestmentAgainstMarketingBudget +
            ...
            (campaigns[_campaignId].campaignData.marketingBudget * 10**IPST(token).decimals()) /
            10**18;
        ...
        // IPST(token).transferFrom(msg.sender, address(this), (_amount * 10**IPST(token).decimals()) / 10**18);
        _safeTransferFrom(token, msg.sender, address(this), (_amount * 10**IPST(token).decimals()) / 10**18);
   ...
   */
    function claimPreSaleTokens(
        ...
        if (campaigns[_campaignId].vestingData.isVestingInEnabled) {
            // console.log("isVestingInEnabled", campaigns[_campaignId].vestingData.isVestingInEnabled);
            require(
            ...
            _vesting =
                ...
                (100 * 10**18) +
                (_vesting * (investmentDetails.vestingRewardPerCycle * 10**IPST(preSale).decimals())) /
                10**18;
            _vesting = (_vesting * 10**IERC20(preSale).decimals()) / 10**18;
            console.log("652 Amount of vestings", _vesting);
            ...
            // IPST(preSale).transfer(msg.sender, _reward);
            console.log("703  _reward", _reward);
            ...
            // console.log("KOL ELigible Reward:", eligibleRwd);
            if (blackListedKOL[msg.sender][_campaignId] == true) {
            ...
            console.log("726 eligibleRwd", (eligibleRwd * 10**IERC20(preSale).decimals()) / 10**18);
   ...
   */
    function claimBackInvestmentByKOL(
        ...
        require(campaigns[_campaignId].campaignData.preSaleToken != _USDT, "Protokol:Can't claim,PST=USDT");
        // Add signature
        require(campaigns[_campaignId].tgeDetails.tgeDate >= block.timestamp, "Protokol:Can't claim,TGE done");
        ...
        // console.log("return invest", _invest);
        ...
        // console.log("_invest", (_invest * 1 * 10**IERC20(_USDT).decimals()) / 1e18);

        _safeTransfer(_USDT, msg.sender, (_invest * 1 * 10**IERC20(_USDT).decimals()) / 1e18);
   ...
   */
    function _blackListedKOLFundsHandling(
        ...
        uint256 balance = (campaigns[_campaignId].campaignData.presaleAmount * 10**IERC20(preSale).decimals()) / 10**18; //IPST(preSale).balanceOf(address(this)); //change here1
        ...
        if (campaigns[_campaignId].campaignData.preSaleToken != _USDT) {
            ...
            if (campaigns[_campaignId].tgeDetails.tgeAmount > 0 || campaigns[_campaignId].tgeDetails.isTGE) {
                ...
                if (balance < investedCampaignDetails[msg.sender][_campaignId].leftOverInvestment) {
                    ...
                    // balance = (balance * 10**IERC20(_USDT).decimals()) / 1e18;
                    ...
                    // console.log("balance", investedCampaignDetails[msg.sender][_campaignId].leftOverInvestment);
                    balance = investedCampaignDetails[msg.sender][_campaignId].leftOverInvestment;
                }
            }
        }
        campaigns[_campaignId].campaignData.remainingInvestment += investedCampaignDetails[msg.sender][_campaignId]
            .leftOverInvestment;
   ...
   */
    ...
    function withdawPresaleByCampaignOwner(uint256 _campaignId) external {
        ...
        // console.log("shareOfKolsInTermsToken", shareOfKolsInTermsToken);
        ...
        // console.log("transferPresale", transferPresale);
