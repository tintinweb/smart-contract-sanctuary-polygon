...
contract SampleFeeChain is OnFeeChain {
    ...
    constructor(address gateway_, address gasReceiver_) OnFeeChain(gateway_, gasReceiver_) {
        owner = msg.sender;
    }
...
abstract contract OnFeeChain is AxelarExecutable {
    ...
    function mintCrossChain(
        ...
        require(_refundAddress != address(0), 'FeeChain: RefundAddress cannot be 0');

        IERC20(feeTokenForNFT).safeTransferFrom(msg.sender, address(this), feeInTokenForNFT);
        //Create the payload.
        bytes memory payload = abi.encode(_recipient, _refundAddress);
        ...
        gateway.callContract(destinationChain, destinationContractAddress.toString(), payload);
    }
