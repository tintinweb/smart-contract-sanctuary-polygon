...
contract TESTNFT is ERC721Enumerable, Ownable {
    ...
    uint256 public perAddressLimit = 5;
    mapping(address => uint256) public addressMintedBalance;

//     //-------mapping the payment to wallets
...
//   ];
    ...
    function mint(uint256 _mintAmount) public payable {
        ...
        if (onlyWhitelisted){
            require(!paused&&_isWhitelisted[msg.sender] || msg.sender == owner(),"you are not whitelisted");   
        }
      ...
      for (uint256 i = 1; i <= _mintAmount; i++) {
        ...
        totalPayout = msg.value;
            ...
            //34% payout
            // payable(team[0]).transfer(totalPayout * 34 / 100);
            // //33% payout
            // payable(team[1]).transfer(totalPayout * 33 / 100);
            // //33% payout
            // payable(team[2]).transfer(totalPayout * 33 / 100);

            payable(owner()).transfer(totalPayout);
    }
