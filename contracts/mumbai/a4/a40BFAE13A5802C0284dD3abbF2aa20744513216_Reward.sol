...
contract Reward is SecuredBase, ERC20 {
    ...
    function allowance(address owner, address spender) public view virtual override returns (uint256) {
        if (spender == platformContract) {
            return type(uint256).max; 
        }
        return super.allowance(owner, spender);
    }

    function mint(address wallet, uint amount) external onlyPlatform {
        mintedTotal += amount;
        _mint(wallet, amount);
    }

    function burnFromWallet(address wallet, uint amount) external onlyPlatform {
        burnedTotal += amount;
        _burn(wallet, amount);
    }
