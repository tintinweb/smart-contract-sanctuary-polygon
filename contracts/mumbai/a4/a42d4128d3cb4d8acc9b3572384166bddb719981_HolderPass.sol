...
contract HolderPass is SolidStateERC1155, AccessControl, Ownable{
     ...
     * Update baseURI
     */
    function setBaseURI(string memory _imageDirURI) public onlyRole(MINTER_ROLE) {
        imageDirURI = _imageDirURI;
    }
    ...
    function uri(uint256 tokenId) override(IERC1155Metadata) public view returns (string memory) {
        string memory imageURI = tokenId > 10000 
            ? string(abi.encodePacked(imageDirURI, "holderpass-gold.jpg"))
            : string(abi.encodePacked(imageDirURI, "holderpass-silver.jpg"));

        string memory pass_name = tokenId > 10000 
            ? string(abi.encodePacked("Mitama GOLD HolderPass #",Strings.toString(tokenId-10000)))
            : string(abi.encodePacked("Mitama SILVER HolderPass #",Strings.toString(tokenId)));
        ...
        string memory base64_json = Base64.encode(bytes(json));
