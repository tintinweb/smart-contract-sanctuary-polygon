...
contract GoldMintNFT is ERC1155Upgradeable, OwnableUpgradeable {

  event Deposit(address who, uint256 amount);
  event GoldMint(address to, uint id);
  ...
  // initialize
  function init(string memory uri, address _devWallet) public {
    CHEST = 1;
    BAR = 2;
    SACK = 3;
    COIN = 4;
  ...
  function initialize(string memory uri, address _devWallet) public initializer {
    __Ownable_init();
    init(uri, _devWallet);
  }
  ...
  function setPaymentToken(address _tokenaddr) public onlyOwner {
    ...
    tiers[CHEST].price = 25*unit;  // 2.5 ether
    tiers[BAR].price = 10*unit;    // 1 ether
    tiers[SACK].price = 2*unit;    // 0.2 ether
    tiers[COIN].price = 1*unit;    // 0.1 ether
    inited = true;
  }
  ...
  function deposit() public {
    ...
    uint256 amount = 
      (tiers[CHEST].limit * tiers[CHEST].price) +
      (tiers[BAR].limit * tiers[BAR].price) +
      (tiers[SACK].limit * tiers[SACK].price) +
      (tiers[COIN].limit * tiers[COIN].price);
  ...
  function mint() public returns(uint){
    ...
    tokenForPayment.transfer(msg.sender, tiers[id].price);
    emit GoldMint(msg.sender, id);
    return id;
  }
  ...
  function _afterTokenTransfer(
    ...
    if (from != address(0)) // check if minting
      return;
    // if minting, then increment minted count
    for (uint i = 0; i < ids.length; i ++) {
      tiers[ids[i]].minted += amounts[i];
    }
  }
  ...
  function totalSupply() public view returns(uint) {
    ...
    for(uint i = CHEST; i <= COIN; i ++)
      totalSup += tiers[i].minted;
