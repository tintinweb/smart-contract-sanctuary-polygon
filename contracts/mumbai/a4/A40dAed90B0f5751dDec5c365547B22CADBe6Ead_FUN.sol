  ...
  IERC2981
{
  ...
  mapping(uint256 => uint96) public royaltyPerNFT;

  uint96 public royaltyPercentage;

  event BindNFT(
    uint256 indexed polygonTokenId,
    string terraTokenId,
    address indexed to,
    string tokenURI
  );
  ...
  bool public metadataIsFreezed = false;
  ...
  function setCollectionCreator(address _collectionCreator) public onlyOwner {
    collectionCreator = _collectionCreator;
  }
  ...
  function setTokenURI(uint256 _tokenId, string memory _tokenURI) external onlyOwner {
    require(metadataIsFreezed == false, "Metadata is freezed");
  ...
  function freezeMetadata() external onlyOwner {
    require(metadataIsFreezed == false, "Metadata is already freezed");
    metadataIsFreezed = true;
  }

  function safeMint(
    address to,
    string memory _tokenURI,
    string memory terraTokenId
  ) public {
    ...
    setRoyalty(tokenId, collectionCreator, royaltyPercentage);

    emit BindNFT(tokenId, terraTokenId, to, _tokenURI);
  }
  ...
  function setRoyalty(
    ...
    royaltyPerNFT[tokenId] = _royaltyPercentage;
    royaltyPercentage = _royaltyPercentage;
  }
  ...
  {
    uint96 percentage = royaltyPerNFT[tokenId];
    uint256 _royalties = (_salePrice * percentage) / 10000;
