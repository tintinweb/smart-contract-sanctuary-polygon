...
contract RentNFT {
  ...
  function stopLending(uint256 _lendingId) external onlyLender(_lendingId) {
    ...
    IERC721(nftMapping[_lendingId].nftAddress).setApprovalForAll(msg.sender, true);

    IERC721(nftMapping[_lendingId].nftAddress).transferFrom(address(this), msg.sender, nftMapping[_lendingId].tokenId);

    uint diff = (block.timestamp - borrowerDetails[_lendingId].deadline)/3600;
    uint amountToPay = borrowerDetails[_lendingId].totalAmountPaid - diff*nftMapping[_lendingId].dailyRate;
