...
//Initiatilse the smart contract
contract ChainBattles is ERC721URIStorage {
    using Strings for uint256;
    using Counters for Counters.Counter;
    //declare a new tokenIds function that we'll need to store our NFT IDs: 
    Counters.Counter private _tokenIds; 

    // store the level of an NFT associated with its tokenId:
    mapping(uint256 => uint256) public tokenIdToLevels;
    // store speed, strength and life associated with tokenId:
    ...
    // declare the constructor function of our smart contract:
...
// train: to train an NFT and raise its level
    ...
    bytes memory svg = abi.encodePacked(
        ...
        '<text x="50%" y="80%" class="base" dominant-baseline="middle" text-anchor="middle">', "Weapons: ",getLife(tokenId),'</text>',
        '</svg>'
    );
    return string(
        abi.encodePacked(
    ...
    uint256 speed = tokenIdToSpeed[tokenId];
    return speed.toString();
}

    function getStrength(uint256 tokenId) public view returns (string memory) {
    uint256 strength = tokenIdToStrength[tokenId];
    return strength.toString();
}
    function getLife(uint256 tokenId) public view returns (string memory) {
    uint256 life = tokenIdToLife[tokenId];
    ...
    _tokenIds.increment(); //starts at 0, so increment at start, so starts from 1
    uint256 newItemId = _tokenIds.current();
    _safeMint(msg.sender, newItemId);
    tokenIdToLevels[newItemId] = 0;
    tokenIdToSpeed[newItemId] = 10;
    tokenIdToStrength[newItemId] = 10;
    tokenIdToLife[newItemId] = 10;
   ...
   tokenIdToStrength[tokenId] = currentStrength + 2;
   uint256 currentLife = tokenIdToLife[tokenId];
   tokenIdToLife[tokenId] = currentLife + 5;
   ...
   //deployed contract 0xeCAAfa6cdBcaF80f8811D383C620CaD18e107C7C
   // v2 deployed = 0xA4289E5D7bd347da5aE38836c0999E8C0994cd6E

// add speed, strength, life
// mapping tokenIdto - done
// update svg - done
// function get - done
// update mint function - done
// function train update - done
