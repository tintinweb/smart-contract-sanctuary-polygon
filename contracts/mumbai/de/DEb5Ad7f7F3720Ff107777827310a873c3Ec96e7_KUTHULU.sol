...
interface ITips {
    ...
    function addTaggedTips(address[] memory taggedAccounts, uint256 tipPerTag) external payable;
}
...
contract KUTHULU is Initializable, PausableUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize(IERC20Upgradeable _paymentToken, address _userProfiles, address _kutils, uint256 _costToPost, uint256 _maxMessageLength, uint256 _maxMsgReturnCount, uint256 _maxTaggedAccounts, uint256 _maxHashtags, uint256 _cutToPoster) initializer public {
    ...
    */
    ...
    event logMsgPostMsg2(uint256 indexed msgID, address indexed proxy, string uri, uint256[4] attribs, uint256[] inGroups);
    ...
    */
    ...
    function postMsg(string calldata message, string[] memory _hashtags, address[] calldata taggedAccounts, string calldata uri, uint256[4] calldata attribs, uint256[] memory inGroups) public payable nonReentrant whenNotPaused {
        ...
        require(paymentToken.approve(msg.sender, costToPost), "Need payment approval");

        // Transfer the Payment Token to the contract
        require(paymentToken.transferFrom(msg.sender, owner(), costToPost), "No Payment Token");
        ...
        if (msg.value > 0){
            ...
            Tips.addTaggedTips{value: msg.value}(taggedAccounts, tipPerTag);
        }
        ...
        userProfiles.recordPost(posterAddress, tipPerTag, taggedAccounts, attribs[1]);

        // Log it
        emit logMsgPostMsg1(msgCounter, posterAddress, message, _hashtags, taggedAccounts);
        emit logMsgPostMsg2(msgCounter, postProxy, uri, attribs, inGroups);
    ...
    function toggleLike(uint256 msgID) public whenNotPaused {
        ...
        if (Likes.checkUserLikeMsg(msg.sender, msgID)) {
            ...
            // require the approval of the sending of the token to the contract
            require(paymentToken.approve(msg.sender, cutToPoster), "Need payment approval");
