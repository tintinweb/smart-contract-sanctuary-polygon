...
abstract contract MasterEvents {
    ...
    event TransferApproved(address pToken, address spender, address src, address dst, uint256 amount);

    event TransferRejection(uint256 shortfall);
    ...
    function transferAllowed(
        ...
        if (shortfall == 0) {
            ...
            middleLayer.msend{value: msg.value}(
                chainId,
                payload, // bytes payload
                payable(params.spender), // refund address
                fallbackAddress
            );

            emit TransferApproved(params.pToken, params.spender, params.user, params.dst, params.amount);
        } else {
            emit TransferRejection(shortfall);
        }
    }
}
