...
contract NFTManager is Ownable {
    ...
    function create(Item memory item) public onlyOwner returns (address) {
        ...
        ItemToken newItem = new ItemToken(item.name_en);
        _itemIds.increment();
        items[newItemId] = address(newItem);
    ...
    function mint(
        ...
        require(itemId <= _itemIds.current(), "ID Invaild");
        uint256 tokenId = ItemToken(items[itemId]).mint(tag);
        return tokenId;
    }
