...
contract EditionCreator {
    ...
    address private immutable askModule;
    address private immutable erc721TransferHelper;

    // Mapping to store contract address by contract ID
    mapping(uint256 => address) public contractAddresses;

    /// @notice Emitted when a user creates contract
    /// @param contractAddress The address of the created contract
    /// @param contractId The id of the created contract
    /// @param userId The database id of the contract creator
    event ContractCreated(address contractAddress, uint256 indexed contractId, string indexed userId);

    /// @notice Emitted when a user mints token
    /// @param contractAddress The address of the contract that have minted token
    /// @param tokenId The id of the created token
    event TokenMinted(address contractAddress, uint256 tokenId);

    /// @param _moduleManager The address of the moduleManager contract used for the deploy of nftFabric
    /// @param _askModule The address of the askModule contract used for the deploy of nftFabric
    /// @param _erc721TransferHelper The address of the erc721TransferHelper contract used for the deploy of nftFabric
    constructor(address _moduleManager, address _askModule, address _erc721TransferHelper) {
        moduleManager = _moduleManager;
        askModule = _askModule;
        erc721TransferHelper = _erc721TransferHelper;
    }

    /// @param _tokenName The name of the created ERC721
    /// @param _tokenSymbol The symbol of the created ERC721
    /// @param _userId The symbol of the created ERC721
    function createEdition(
        ...
        uint256 currentContractId = atContract.current();

        // Create a new nftFabric contract
        nftFabric newContract = new nftFabric(
            _tokenName,
            _tokenSymbol,
            askModule,
        ...
        IModuleManager(moduleManager).setApprovalForModule(address(askModule), true);
        ...
        contractAddresses[currentContractId] = newContractAddress;

        // Emit an event to track the contract creation
        emit ContractCreated(newContractAddress, currentContractId, _userId);
        // Increment the contractId
        atContract.increment();
        return newContractAddress;
    }

    // Function to get the address of a contract given its ID
    function getEditionAddress(uint256 _editionId) external view returns (address) {
        return contractAddresses[_editionId];
    }

    // Function to be called by the ERC721 contract when a token is minted
    function notifyTokenMinted(uint256 contractId, uint256 _tokenId) external {
        require(contractAddresses[contractId] == msg.sender, "Invalid contract address");
...
import "./EditionCreator.sol";
import "./Asks.sol";

contract nftFabric is ERC721URIStorage, Ownable {
    Asks immutable askModule;
    EditionCreator immutable editionCreator;
    uint256 public immutable contractId;
    ...
    constructor(
        ...
        askModule = Asks(_askModule);
        contractId = _contractId;
        _setApprovalForAll(address(this), _erc721TransferHelper, true);
        editionCreator = EditionCreator(_editionCreator);
    }

//    function mint_by_owner(
//        string memory NFTJson,
//        uint256 NFTAmount,
//        uint256 NFTPrice,
//        uint16 findersFeeBps
//    )
//    public onlyOwner returns (uint256 firsNfTId, uint256 lastNFTId)
//    {
//        firsNfTId = _tokenIds.current() + 1;
//        for (uint i = 0; i < NFTAmount; i++) {
//            _tokenIds.increment();
//            uint256 newItemId = _tokenIds.current();
//            _mint(address(this), newItemId);
//            _setTokenURI(newItemId, NFTJson);
//            askModule.createAsk(address(this), newItemId, NFTPrice, msg.sender, findersFeeBps);
//            editionCreator.notifyTokenMinted(contractId, _tokenIds.current());
//        }
//        lastNFTId = _tokenIds.current();
//    }

    function mintAndSellTokens(
        string memory jsonLink,
        uint256 NFTAmount,
        uint256 NFTPrice,
        uint16 findersFeeBps
        ...
        for (uint i = 0; i < NFTAmount; i++) {
            ...
            putOnSale(tokenIds[i], NFTPrice, findersFeeBps);
        }

        return tokenIds;
    }

    function bulkMint(string memory jsonLink, uint256 NFTAmount) public onlyOwner returns (uint256[] memory){
        uint256[] memory tokenIds = new uint256[](NFTAmount);
    ...
    function mintToken(string memory jsonLink) internal onlyOwner {
        _tokenIds.increment();
        _mint(address(this), _tokenIds.current());
        _setTokenURI(_tokenIds.current(), jsonLink);
        editionCreator.notifyTokenMinted(contractId, _tokenIds.current());
    }

    function putOnSale(uint256 tokenId, uint256 NFTPrice, uint16 findersFeeBps) public onlyOwner{
        askModule.createAsk(address(this), tokenId, NFTPrice, msg.sender, findersFeeBps);
    }
}