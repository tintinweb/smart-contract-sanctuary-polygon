...
contract FacultyElection {
    ...
    address public votingOrganizer = 0xFc5e8b1c8531B1b62A31CFFbB3624ddaFEDB666D;
    ...
    function giveVoterRight(address _address) external {
        require(
            msg.sender == votingOrganizer,
            "Only organizer can give the right to vote"
        );
        _voterId.increment();
        uint256 idNumber = _voterId.current();

        // Check matriculation number eligibility
        uint256 matriculationNumber = parseMatriculationNumber(_address);
        require(
            (matriculationNumber >= 1404144001 && matriculationNumber <= 1404144150) ||
            (matriculationNumber >= 1404244001 && matriculationNumber <= 1404244150),
            "Voter's matriculation number is not within the eligible range"
        );
...
function parseMatriculationNumber(address _address) internal pure returns (uint256) {
    bytes memory addressBytes = abi.encodePacked(_address);
    bytes memory matriculationNumberBytes = new bytes(8);

    // Extract the last 8 bytes of the address
    for (uint256 i = 0; i < 8; i++) {
        matriculationNumberBytes[i] = addressBytes[addressBytes.length - 8 + i];
    }

    // Convert bytes to uint256
    uint256 matriculationNumber = abi.decode(matriculationNumberBytes, (uint256));
