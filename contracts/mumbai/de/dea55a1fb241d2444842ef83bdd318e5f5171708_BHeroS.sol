...
contract BHeroS is Initializable, AccessControlUpgradeable, UUPSUpgradeable {
  ...
  //change ratio num reset shield when burn hero type
  function getNumResetShield(uint256[] memory listIdHero) internal view returns (uint256) {
    uint256 totalResetShield;
    for (uint256 i = 0; i < listIdHero.length; ++i) {
      uint256 rarity = BHeroDetails.decodeRarity(getTokenDetailByID(listIdHero[i]));
      totalResetShield += numResetShield[rarity];
    }
    return totalResetShield;
  }
  ...
  /**
  /* polygon: [1, 2, 4, 7, 11, 16]
  /* BSC: [1,2,3,4,5,6]
   */

  function setNumResetShield(uint8[6] memory value) external onlyRole(DESIGNER_ROLE) {
    numResetShield = value;
  }
    ...
    isOwner(listIdHero)
  {
    ...
    uint256 totalResetShield = getNumResetShield(listIdHero);
    
    //add data
    uint256 detail = getTokenDetailByID(idHeroS);
    uint256 newDetails = BHeroDetails.increaseNumResetShield(detail, totalResetShield);
...
library BHeroDetails {
  ...
  function decode(uint256 details) internal pure returns (Details memory result) {
    ...
    result.numResetShield = (details >> 240) & (2**15 - 1);
  }
  ...
  function decodeNumResetShield(uint256 details) internal pure returns (uint256) {
    return (details >> 240) & (2**15 - 1);
  }

  function increaseNumResetShield(uint256 details, uint256 value) internal pure returns (uint256) {
    uint256 numResetShield = decodeNumResetShield(details);
    details &= ~(uint256(2**15 - 1) << 240);
    details |= (numResetShield + value) << 240;
    return details;
  }
