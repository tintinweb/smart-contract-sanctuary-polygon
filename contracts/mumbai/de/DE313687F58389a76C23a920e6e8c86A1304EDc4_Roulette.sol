...
contract Roulette is  VRFConsumerBaseV2, Ownable {
 ...
 VRFCoordinatorV2Interface COORDINATOR;
  ...
  uint256 public currentSingleBetId;
  ...
  mapping(uint256 => UniBet) public uniBetId;
  mapping(uint256 => bool) public isSinglePlayerBet;   //requestId to bool
  mapping(uint256 => uint256) public requestIdToBetId;
  mapping(uint256 => mapping(address => UserBet)) public players;
  ...
  modifier checkDeadline() {
    if(participants[currentMultiBetId].length >= 1) {
        require(block.timestamp < betIdToBets[currentMultiBetId].time + deadlineOfGame, 'deadline for this bet is passed');
        _;
    }
  }

  modifier maxBetAllowed() {
    require(msg.value <= maxBet, "Maxium allowed bet is 500");
    _;
  }

  modifier maxSpots() {
    require(participants[currentSingleBetId].length < 6, "Maxium spots filled");
    _;
  }

  modifier betFulfilled() {
    Bet storage b = betIdToBets[currentMultiBetId];
    require(b.fulfilled == true, "Wait for random number request to be fulfilled");
    _;
  }
  ...
  */
  function startGame() external onlyOwner {
       currentMultiBetId = _inc(currentMultiBetId);
       BetId = _inc(BetId);
   ...
   */
      function multiPlayerBet(BetTypes[] memory _betType, uint8[] memory number, bool _isMatic, address ERC20Address, uint256[] memory amount) public checkDeadline maxBetAllowed maxSpots payable {
        require(participated[currentMultiBetId][msg.sender] == false, "Already participated");
        ...
        require(b.wheelSpun == false, "Spinning of wheel is completed"); 
            if(participants[currentMultiBetId].length == 0){
        ...
        UserBet storage bet =  players[currentMultiBetId][msg.sender];
        ...
        emit BetPlacedInToken(currentMultiBetId, msg.sender, ERC20Address, amount, _betType, number );
        }
    else {
        ...
        emit BetPlacedInMatic(currentMultiBetId, msg.sender, amount, _betType, number );
      }
        participants[currentMultiBetId].push(msg.sender);
        participated[currentMultiBetId][msg.sender] = true;
    }
    ...
    function singlePlayerBet(BetTypes[] memory _betType, uint8[] memory number, bool _isMatic, address ERC20Address, uint256[] memory amount) public maxBetAllowed payable {
        require(_betType.length == number.length && number.length == amount.length);
        currentSingleBetId = _inc(currentSingleBetId); 
        BetId = _inc(BetId);  
        uint betValue ;  
        for(uint i = 0; i < _betType.length; i++) {
              uint8 temp = uint8(_betType[i]);
            ...
            UniBet storage u = uniBetId[currentSingleBetId];
        ...
        u.tokenAddress = ERC20Address;
        u.isMatic = false;

          emit BetPlacedInToken(currentSingleBetId, msg.sender, ERC20Address, amount, _betType, number );
        }
    else {       
        require(msg.value == betValue, 'Bet value should be same as the bet amount');
        u.isMatic = true;       

      emit BetPlacedInMatic(currentSingleBetId, msg.sender, amount, _betType, number );
    }
    ...
    requestIdToBetId[u.requestId] = BetId;
    isSinglePlayerBet[requestId] = true;   
    }
  ...
  */
  function spinWheel() external onlyOwner {
      ...
      require(participants[currentMultiBetId].length > 0, 'No player has joined');
      requestId = COORDINATOR.requestRandomWords(
      keyHash,
      vrfSubscriptionId,
      requestConfirmations,
    ...
    isSinglePlayerBet[requestId] = false;
  }

  //Internal function called by VRFCoordinator when it receives a rancheckWinnerdom number from valid VRFproof.
  function fulfillRandomWords(uint256 _requestId, uint256[] memory _randomWords)
    internal
    override
  {
     uint256 _betId = requestIdToBetId[_requestId];
    if(isSinglePlayerBet[requestId] == true) {     
      UniBet storage u = uniBetId[_betId];
      u.fulfilled = true;
      uint256 randomValue = (_randomWords[0] % 36);
      u.randomNumber = randomValue;
      emit wheelSpun(currentSingleBetId, _requestId, randomValue);
    }
    else {
    Bet storage b = betIdToBets[_betId];
    b.fulfilled = true;
    uint256 randomValueBtwRange = (_randomWords[0] % 36);
    b.randomNumber = randomValueBtwRange;
    emit wheelSpun(currentMultiBetId, _requestId, randomValueBtwRange);
    }
  }
  ...
  */
    function multiPlayerPayOut(uint256 _requestId) public betFulfilled onlyOwner {
     uint256 _betId = requestIdToBetId[_requestId]; 
    uint256 num = betIdToBets[_betId].randomNumber;
    for(uint j = 0; j < participants[currentMultiBetId].length; j++) {
      address playerAddress = participants[currentMultiBetId][j];   
      UserBet storage b = players[currentMultiBetId][playerAddress];
      ...
      if (won && b.isMatic) {
        winningsInMatic[b.bettorAddress] += b.betAmount[i] * payouts[typeOfBet];
     ...
     Bet storage bet = betIdToBets[currentMultiBetId];
     bet.betCompleted = true;
     emit WinAmount(currentMultiBetId, b.winningAmount);
    }  
  }

    /*
  Checks if the player has won the bet.
  Calculate the payouts for the player of all bet types.
  Adds the winning amount to the user winnings.
  */
    function singlePlayerPayOut(uint256 _requestId) public onlyOwner {
      uint256 _betId = requestIdToBetId[_requestId]; 
      UniBet storage u = uniBetId[_betId];
      require(u.fulfilled == true, "Wait for random number request to be fulfilled");
      uint256 num = u.randomNumber;  
      ...
      else if (won == true && u.isMatic == false) {
        winningsInToken[u.bettorAddress][u.tokenAddress] += u.betAmount[i] * payouts[typeOfBet];
        u.winningAmount += u.betAmount[i] * payouts[typeOfBet];
      }
     emit WinAmount(currentSingleBetId, u.winningAmount);
    } 
      
  }

  function playersLength(uint betId) public view returns(uint256) {
    return participants[betId].length;
  }
