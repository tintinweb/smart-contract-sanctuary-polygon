...
contract VCMarketplaceAuction is VCMarketplaceBase {
    ...
    /**
     * @dev Lists a token in the Marketplace for an auction, which requires
     ...
     * @param _projectIds: Array of project IDs to support on purchases
     * @param _feesBps: Array of fees to support each project ID
     */
    function listAuction(
        ...
        require(auctionListings[tokenAddress][_tokenId].seller == address(0), "Already listed");

        bool minted = _token.exists(_tokenId);
        uint256 totalSupply = minted ? _token.totalSupply(_tokenId) : _token.lazyTotalSupply(_tokenId);
        require(totalSupply == 1, "Can auction NFTs only");

        _setFees(tokenAddress, _tokenId, msg.sender, _poolFeeBps, _projectIds, _feesBps);

        if (!minted) {
            _token.requireCanRequestMint(msg.sender, _tokenId, 1);
        } else {
            _token.safeTransferFrom(msg.sender, address(this), _tokenId, 1, "");
        }

        uint256 maturity = block.timestamp + _biddingDuration;
        auctionListings[tokenAddress][_tokenId] = AuctionListing(
            minted,
            msg.sender,
            _initialPrice,
            maturity,
        ...
        emit ListedAuction(tokenAddress, _tokenId, msg.sender, _initialPrice, maturity);
    }
     ...
     * @param _tokenId the token identifier
     */
    function unlistAuction(IArtistNft _token, uint256 _tokenId) public onlyWhitelistedTokens(address(_token)) {
        address seller = msg.sender;
        address tokenAddress = address(_token);
        AuctionListing memory listing = auctionListings[tokenAddress][_tokenId];
        ...
        delete auctionListings[tokenAddress][_tokenId];

        if (listing.minted) {
            _token.safeTransferFrom(address(this), listing.seller, _tokenId, 1, "");
        }

        emit UnlistedAuction(seller, tokenAddress, _tokenId);
    }

    /**
     * @dev Places a bid for a token listed in the Marketplace, if its placed correctly sends
     * back the last bid to the last highest bidder
     *
     * @param _token the non fungible token address
     * @param _tokenId the token identifier
    ...
    function bid(
        ...
        AuctionListing memory listing = auctionListings[_token][_tokenId];

        _checkBeforeBid(listing, _amount);

        if (listing.highestBid != 0) {
            currency.transfer(listing.highestBidder, listing.highestBid);
            // bidToCollect[listing.highestBidder] += listing.highestBid;
        }

        currency.transferFrom(bidder, address(this), _amount);

        listing.highestBid = _amount;
        listing.highestBidder = bidder;
        auctionListings[_token][_tokenId] = listing;

        emit Bid(bidder, _token, _tokenId, _amount);
    }
     ...
     * @param _tokenId the token identifier
     */
    function settle(IArtistNft _token, uint256 _tokenId) public whenNotPaused {
        address tokenAddress = address(_token);
        AuctionListing memory listing = auctionListings[tokenAddress][_tokenId];
        ...
        if (price != 0) {
            uint256 resultingAmount = _transferFee(tokenAddress, _tokenId, listing.seller, currency, price);
            resultingAmount -= _transferRoyalty(tokenAddress, _tokenId, price);
            if (!currency.transfer(listing.seller, resultingAmount)) {
                revert MktSettleFailed();
            }

            if (!listing.minted) {
                _token.mint(_tokenId, 1);
            }

            _token.safeTransferFrom(address(this), listing.highestBidder, _tokenId, 1, "");
        } else {
            if (listing.minted) {
                _token.safeTransferFrom(address(this), listing.seller, _tokenId, 1, "");
            }
        ...
        emit Settled(listing.highestBidder, listing.seller, tokenAddress, _tokenId, price);
    }
    ...
    // function claimBid() external {
    //     uint256 bidAmount = bidToCollect[msg.sender];
    //     bidToCollect[msg.sender] = 0;
    //     require(bidAmount > 0, "No bid to collect");
    //     require(IERC20(currency).transferFrom(address(this), msg.sender, bidAmount), "Lost bid collection failed");
    // }
}