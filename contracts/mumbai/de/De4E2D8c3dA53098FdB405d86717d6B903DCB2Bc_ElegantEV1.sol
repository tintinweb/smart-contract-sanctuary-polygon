...
contract ElegantEV1 is Ownable, AccessControl {
    ...
    function returnRace(uint _num) public view returns(uint[] memory){
        ...
        for (uint i=0; i<_size; i++){
            uint _rc = _racers[i];
            //_rem = fHundo.mod(perfByRacer[_rc].firstLeg);
            //if (_rem > 0){_rem = 1;}
                
            _steps[i] = perfByRacer[_rc].firstLeg;//fHundo.div(perfByRacer[_rc].firstLeg) + _rem;
            //_rem = fHundo.mod(perfByRacer[_rc].secondLeg);
            //if (_rem > 0){_rem = 1;}
                
            //_steps[i] = _steps[i] + fHundo.div(perfByRacer[_rc].secondLeg) + _rem;
        }
        /*
