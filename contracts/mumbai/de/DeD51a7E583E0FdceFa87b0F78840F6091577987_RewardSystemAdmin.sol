...
contract RewardSystemAdmin {
    event NewInstanceCreated(string _orgName, address _orgAdmin);
    event FundsReceived(uint256 amount);

    uint256 baseFee = 1000000000000; //fee to be charged by us
    address ADMIN;
    uint256 Balance = 0;
    mapping(address => Driver) public m_instances;
    mapping(string => uint256) m_registeredOrganizations;
    address[] addressKeys;

    function allowAdmin() internal view {
        //helper
    ...
    function requestNewInstance(address _orgAdmin, string memory _orgName)
        external
        isADMIN
    {
        if (m_registeredOrganizations[_orgName] != 0) {
            revert("Choose a diferrent name!");
        }
        m_instances[_orgAdmin] = new Driver(_orgAdmin, _orgName, address(this));
        addressKeys.push(_orgAdmin);
        m_registeredOrganizations[_orgName] = 1;

        emit NewInstanceCreated(_orgName, _orgAdmin);
    }

    function getBaseFee() external view isADMIN returns (uint256) {
        return baseFee;
    }

    function setFee(uint256 _fee) external isADMIN {
        //admin
        uint256 count = addressKeys.length;
        baseFee = _fee;
        for (uint256 i; i < count; ++i) {
            m_instances[addressKeys[i]].updateBaseFee(_fee);
        }
    }

    function banOrg(address _orgAdmin) external view isADMIN {
        m_instances[_orgAdmin].pauseAccess;
    }
