...
library PlayerStorageLib {
        ...
        returns (address recipient, uint256 nftId, bytes memory chainId)
    {
        ...
        _mintCrossChainPlayer(transferParams.playerData, recipient, nftId);
        t.test++;
        chainId = abi.encode(srcChainId);
    }

    function _mintCrossChainPlayer(PlayerSlotLib.Player memory _player, address _recipient, uint256 _playerId) internal {
        PlayerStorage storage s = diamondStorage();
