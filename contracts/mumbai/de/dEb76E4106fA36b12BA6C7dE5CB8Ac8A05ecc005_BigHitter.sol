 ...
 * {ERC721Enumerable}.
 */
contract BigHitter is Ownable, UriChanger, ERC721Enumerable, VerifyAttestation, ERC5169 {
    // using Address for address;
    using Strings for uint256;
    using ECDSA for bytes32;
        
    string constant _metadataURI = "https://resources.smarttokenlabs.com";
    string constant _contractName = "Big Hitter";
    string constant _contractSymbol = "BIG_HITTER";
     ...
     * @dev Initializes the contract by setting a `name` and a `symbol` to the token collection.
     */
    constructor () ERC721(_contractName, _contractSymbol) UriChanger(_msgSender()) {}

    function contractURI() public view returns(string memory) {
        return string(abi.encodePacked(_metadataURI, "/contract/", symbol(), JSON_FILE));
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return string(abi.encodePacked(_metadataURI, "/token/" , block.chainid.toString(), "/", contractAddress(), "/", tokenId.toString(), JSON_FILE));
    }
    ...
    function mintDirect(uint price, uint tokenId, uint ttl, bytes calldata signature) public payable {

        bytes memory toSign = abi.encodePacked(address(this), msg.sender, block.chainid, price, tokenId, ttl );
