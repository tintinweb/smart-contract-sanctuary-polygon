...
contract DropyNft is ERC1155, Ownable {
  ...
  function batchMint(address[] memory addresses, uint256[][] calldata _ids, uint256[][] calldata _amounts) public onlyOwner{
   for (uint i = 0; i < addresses.length; i++) {
    _mintBatch(addresses[i], _ids[i], _amounts[i], '0x');
   }
}
