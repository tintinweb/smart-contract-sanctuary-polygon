...
contract FlowerFairies is ERC721Drop {
    ...
    mapping(uint => bool) private isMinted;
    mapping(uint256 => string) private _tokenURIs;

 function mintNFT(string memory _tokenURI) public returns (uint256) {
  require(!isMinted[_tokenIds.current()], "NFT has already been minted"); // Check if NFT has already been minted
  _tokenIds.increment();
  uint256 newItemId = _tokenIds.current();

  _safeMint(_msgSender(), newItemId);
  _setTokenURI(newItemId, _tokenURI);
  isMinted[newItemId] = true; // Set the minting status to true
  return newItemId;
}
