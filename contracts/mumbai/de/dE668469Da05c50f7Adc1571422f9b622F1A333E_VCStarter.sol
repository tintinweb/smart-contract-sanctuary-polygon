...
contract VCMarketplaceFixedPrice is VCMarketplaceBase {
    ...
    function listFixedPrice(
        ...
        _setFees(address(artNft), _tokenId, msg.sender, _poolFeeBps, _projectIds, _projectFeesBps);
        if (!listed(address(artNft), _tokenId, msg.sender)) {
    ...
    function _newList(
        ...
        fixedPriceListings[address(artNft)][_tokenId][msg.sender] = FixedPriceListing(
            minted,
            msg.sender,
            _amount,
            _listPrice,
    ...
    function _updateList(
        ...
        FixedPriceListing memory listing = fixedPriceListings[address(artNft)][_tokenId][msg.sender];
        ...
        fixedPriceListings[address(artNft)][_tokenId][msg.sender] = listing;
    }
    ...
    function purchase(uint256 _tokenId, address _seller) public whenNotPaused {
        FixedPriceListing memory listing = fixedPriceListings[address(artNft)][_tokenId][_seller];
        ...
        if (!listing.minted) {
            ...
            fixedPriceListings[address(artNft)][_tokenId][_seller].minted = true;
        }
    ...
    function _updateFixedPriceListing(
        ...
        if (listing.amount == _amount) {
            delete fixedPriceListings[address(artNft)][_tokenId][_seller];
        } else if (listing.amount > _amount) {
            fixedPriceListings[address(artNft)][_tokenId][_seller].amount -= _amount;
        } else {
...
contract VCMarketplaceAuction is VCMarketplaceBase {
    ...
    function listAuction(
        ...
        if (auctionListings[address(artNft)][_tokenId].seller != address(0)) {
    ...
    function _checkSupply(bool _minted, uint256 _tokenId) internal {
        uint256 totalSupply = _minted ? artNft.totalSupply(_tokenId) : artNft.lazyTotalSupply(_tokenId);
    ...
    function _newList(
        ...
        if (!minted) {
            artNft.requireCanRequestMint(msg.sender, _tokenId, 1);
        } else {
            artNft.safeTransferFrom(msg.sender, address(this), _tokenId, 1, "");
        }

        auctionListings[address(artNft)][_tokenId] = AuctionListing(
            minted,
            msg.sender,
            _initialPrice,
            _maturity,
    ...
    function unlistAuction(uint256 _tokenId) public {
        AuctionListing memory listing = auctionListings[address(artNft)][_tokenId];
        ...
        delete auctionListings[address(artNft)][_tokenId];

        if (listing.minted) {
            artNft.safeTransferFrom(address(this), listing.seller, _tokenId, 1, "");
        }
    ...
    function bid(uint256 _tokenId, uint256 _amount) public whenNotPaused {
        ...
        listing.marketFee = marketFee;
        auctionListings[address(artNft)][_tokenId] = listing;
    ...
    function settle(uint256 _tokenId) public whenNotPaused {
        ...
        if (listing.highestBid != 0) {
            ...
            if (!listing.minted) {
                artNft.mint(_tokenId, 1);
            }
            artNft.safeTransferFrom(address(this), listing.highestBidder, _tokenId, 1, "");
