...
contract CardsFacet {
    ...
    function createCard(CreateCardArgsStruct calldata _args) external {
        ...
        require(bytes(_args.name).length != 0 && bytes(_args.name).length <= 50);

        // ImageCID must be at least 1 and no more than 64 characters.
        require(bytes(_args.imageCID).length != 0 && bytes(_args.imageCID).length <= 64);

        // Description must be at least 1 and no more than 1000 characters.
        require(bytes(_args.description).length != 0 && bytes(_args.description).length <= 1000);

        // Role must be at least 1 and no more than 25 characters.
        require(bytes(_args.role).length != 0 && bytes(_args.role).length <= 25);
