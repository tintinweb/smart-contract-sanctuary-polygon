...
contract MysticGalacticMission1 is ERC721, Ownable, Pausable, AccessControlEnumerable, ReentrancyGuard {
    ...
    uint256 public immutable MAX_TOKENS_PER_DAY;

    uint256 private tokenCounter;
    uint256 private lastMintedDay;
    uint256 private tokensMintedToday;
    uint256 private totalRefunds;

    uint256 private launchEventDate;
    uint256 private recoveryEventDate;
    ...
    modifier onlyOwnerOrAdmin() {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender) || hasRole(ADMIN_ROLE, msg.sender), "Caller is not owner or admin");
        _;
    }

    // Currency converter feed
    address private priceFeedAddress;

    // Events
    event PriceUpdated(uint256 newPrice);
    event LaunchEventDateSet(uint256 launchEventDate);
    event RecoveryEventDateSet(uint256 recoveryEventDate);
    event AdminRoleGranted(address account);
    event AdminRoleRevoked(address account);
    event TokenMinted(uint256 tokenId, address minter, string tokenURI);
     ...
     * @param _initialPriceInUSD The initial price for minting a token in USD.
     * @param _maxTokens The maximum number of tokens that can be minted.
     * @param _maxTokensPerDay The maximum number of tokens that can be minted per day.
     */
    constructor(
    ...
    ) ERC721("Mystic Galactic Mission 1", "MGM1") {
        require(bytes(_baseURI).length > 1, "Base URI cannot be empty");
        require(_maxTokens > 0, "Maximum Tokens provided must be greater than 0: ");
        require((_maxTokens - _maxTokensPerDay) > 0, "Maximum Tokens Per Day must be less then Maximum Tokens");

        MAX_TOKENS = _maxTokens;
        MAX_TOKENS_PER_DAY= _maxTokensPerDay;
        tokenCounter = 0;
        priceInUSD = _initialPriceInUSD;
        baseURI = _baseURI;
        ...
        //maticUsdPriceFeed = AggregatorV3Interface(0x82dB9Fc4956Ddb5b6d5fe1D91b1c9f7f18b6e5b7);
    ...
    /**
     * @dev Sets a new price for minting a token in USD. Can only be called by the owner or an admin.
     * @param _priceInUSD The new price in priceInUSD.
     */
    function setPriceInUSD(uint256 _priceInUSD) public onlyOwnerOrAdmin {
        priceInUSD = _priceInUSD;
        emit PriceUpdated(_priceInUSD);
    }
    ...
    function getPrice() public view returns (uint256) {
        ...
        (, rate, , , ) = priceFeed.latestRoundData();

        require(rate > 0, "Invalid rate");

        uint256 rateUint = uint256(rate);

        // Calculate the price in Wei, assuming the rate is given with 8 decimal places
        uint256 price = (priceInUSD * rateUint) / 1e18;
    ...
    /**
     * @dev Mints a new NFT token for the sender and increments the token counter.
     * Ensures that the maximum number of total tokens and daily tokens is not exceeded.
     * The sent value must be greater than or equal to the current token price.
     * Transfers the payment to the contract owner and emits a TokenMinted event.
     * Can only be called when the contract is not paused.
     * @return uri The token URI of the newly minted token.
     */
    function mint() public payable nonReentrant whenNotPaused returns (string memory) {
        require(tokenCounter < MAX_TOKENS, "Maximum number of pre-sales reached");
        ...
        require(tokensMintedToday < MAX_TOKENS_PER_DAY, "Maximum number of daily pre-sales reached");

        require(msg.value >= getPrice(), "Funds sent is less than the price of token");
        ...
        // Ensure that the tokenCounter doesn't overflow
        uint256 newTokenCounter = tokenCounter + 1;
        tokenCounter = newTokenCounter > MAX_TOKENS ? MAX_TOKENS : newTokenCounter;

        // Ensure that the tokensMintedToday doesn't overflow
        uint256 newTokensMintedToday = tokensMintedToday + 1;
        tokensMintedToday = newTokensMintedToday > MAX_TOKENS_PER_DAY ? MAX_TOKENS_PER_DAY : newTokensMintedToday;

        // Emit that the token is minted
        string memory uri = tokenURI(tokenId);

        // Transfer the payment to the owner's account
    ...
    /**
     * @dev Prepares refunds for all token minters and burns their NFTs. Passing msg.value funds the contract
     */
    function prepareRefunds() external payable onlyOwnerOrAdmin {
        ...
        uint256 refundAmountPerToken = address(this).balance / tokenCounter;

        for (uint256 i = 0; i < tokenCounter; i++) {
            address minter = ownerOf(i);
            uint256 minterTokenCount = balanceOf(minter);
            minterRefundAmounts[minter] += refundAmountPerToken * minterTokenCount;

            // Burn the NFT
            _burn(i);
            emit TokenBurned(i, msg.sender);
        }
    }
     ...
     */
    function sendRefund(uint256 numRefunds) external onlyOwnerOrAdmin {
        require(numRefunds > 0, "Incorrect refund amount");

        for (uint256 i = 0; i < numRefunds; i++) {
            require(totalRefunds < tokenCounter, "All refunds have been completed");

            address minter = _tokenMinters[totalRefunds];
     ...
     * @param _launchEventDate The new launch event date as a Unix timestamp.
     */
    function setLaunchEventDate(uint256 _launchEventDate) public onlyOwnerOrAdmin {
        require(_launchEventDate > block.timestamp, "Launch event date must be in the future");
        if (recoveryEventDate != 0) {
            require(_launchEventDate < recoveryEventDate, "Launch event date must be less than the recovery event date");
        }
        launchEventDate = _launchEventDate;
        emit LaunchEventDateSet(_launchEventDate);
    }
     ...
     * @param _recoveryEventDate The new recovery event date as a Unix timestamp.
     */
    function setRecoveryEventDate(uint256 _recoveryEventDate) public onlyOwnerOrAdmin {
     ...
     */
    function getLaunchEventDate() public view returns (uint256) {
        return launchEventDate;
    }
     ...
     */
    function getRecoveryEventDate() public view returns (uint256) {
        return recoveryEventDate;
    }
    ...
    // For testing
    function getTokensMintedToday() external view onlyOwner returns (uint256) {
        return tokensMintedToday;
    }

    function getTokenCount() external view onlyOwner returns (uint256) {
        return tokenCounter;
    }

    function getMinterRefundAmount(address minter) external view onlyOwner returns (uint256) {
        return minterRefundAmounts[minter];
    }
