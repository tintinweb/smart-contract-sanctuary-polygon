 ...
 * account.
 */
abstract contract ERC721Enumerable is ERC721, IERC721Enumerable, Ownable {
...
contract Malena is ERC721, ERC721Enumerable, ERC2981PerTokenRoyalties, ERC721Pausable {
    ...
    string public MisteryBox;
    string public MisteryBoxVideo;
    string public SharedDinner;
    string public OneToOneDinner;
    string[4] public Uris = [MisteryBox, MisteryBoxVideo, SharedDinner, OneToOneDinner];

    mapping(uint256 => uint256) public _id_URI;

    event NonFungibleTokenRecovery(address indexed token, uint256 tokenId);
    event TokenRecovery(address indexed token, uint256 amount);	
    event CreateMalenaNFT(uint256 indexed id);
	
    constructor() ERC721("Malena", "MALENA") {
        pause(true);
    }
    ...
    function mint(uint256 _count, uint8 _URI) external onlyOwner {
        for (uint256 i = 0; i < _count; i++) {
            _mintAnElement(msg.sender, _URI);
        }
    }
	
    function _mintAnElement(address _to, uint8 _URI) private {
        uint id = _totalSupply() + 1;
        _tokenIdTracker.increment();
        _safeMint(_to, id);
        _setTokenRoyalty(id, royaltiesReceiver, contractRoyalties);
        _id_URI[id] = _URI;
        emit CreateMalenaNFT(id);
    }
    ...
    function setTokenURI(string memory _MisteryBox, string memory _MisteryBoxVideo, string memory _SharedDinner, string memory _OneToOneDinner) external onlyOwner {
        MisteryBox = _MisteryBox;
        MisteryBoxVideo = _MisteryBoxVideo;
        SharedDinner = _SharedDinner;
        OneToOneDinner = _OneToOneDinner;
        Uris = [MisteryBox, MisteryBoxVideo, SharedDinner, OneToOneDinner];
    }
    ...
    return string(Uris[_id_URI[_tokenId]]);
    }
}