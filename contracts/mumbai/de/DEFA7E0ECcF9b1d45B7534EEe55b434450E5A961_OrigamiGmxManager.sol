...
contract OrigamiGmxManager is IOrigamiGmxManager, Ownable, Operators, Pausable {
    ...
    function harvestableRewards(bool forStakedGlpRewards) external override view returns (uint256[] memory amounts) {
        ...
        (uint256 nativeAmount, uint256 esGmxAmount) = primaryEarnAccount.harvestableRewards(forStakedGlpRewards);
    ...
    function projectedRewardRates(bool forStakedGlpRewards) external override view returns (uint256[] memory amounts) {
        amounts = new uint256[](2);

        // Pull the reward rates from Origami's staked positions at GMX.
        (uint256 primaryNativeRewardRate, uint256 primaryEsGmxRewardRate) = primaryEarnAccount.rewardRates(forStakedGlpRewards);
        ...
        (uint256 secondaryNativeRewardRate,) = (address(secondaryEarnAccount) == address(0))
            ? (0, 0)
            : secondaryEarnAccount.rewardRates(forStakedGlpRewards);
    ...
    /// @param forStakedGlpRewards If true, get the reward rates for just staked GLP rewards. Else the reward rates for combined GMX/esGMX/mult points
    /// ie the net amount after Origami has deducted it's fees.
    function harvestableSecondaryRewards(bool forStakedGlpRewards) external view returns (uint256[] memory amounts) {
        amounts = new uint256[](2);
        ...
        (amounts[0],) = secondaryEarnAccount.harvestableRewards(forStakedGlpRewards);
    }
