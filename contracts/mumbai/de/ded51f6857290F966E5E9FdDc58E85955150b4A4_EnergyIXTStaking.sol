...
interface IEnergyIXTStaking {
     ...
     */
    function stake(uint256 _amount, bool _ameliaCommitment) external;
     ...
     */
    function unstake(uint256 _amount, bool _ameliaCommitment) external;
...
contract EnergyIXTStaking is IEnergyIXTStaking, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping(address => uint256) private userAmeliaCommitment;
    ...
    function stake(uint256 _amount, bool _ameliaCommitment) external nonReentrant {
    ...
    /// @inheritdoc IEnergyIXTStaking
    function unstake(uint256 _amount, bool _ameliaCommitment) external nonReentrant {
        ...
        if (!_ameliaCommitment) require(userStakedAmount[msg.sender] >= _amount, "ENERGY STAKING: INSUFFICIENT AMOUNT STAKED");
    ...
    function claim(bool _ameliaCommitment) external nonReentrant {
        if (_ameliaCommitment) require(userAmeliaCommitment[msg.sender] > 0, "ENERGY STAKING: INSUFFICIENT AMOUNT COMMITTED");
        if (!_ameliaCommitment) require(userStakedAmount[msg.sender] > 0, "ENERGY STAKING: INSUFFICIENT AMOUNT STAKED");
    ...
    function earnedAmelia(address _walletAddress) public view returns (uint256) {
        return (userAmeliaCommitment[_walletAddress] * (s_rewardPool.rewardPerTokenStored - s_rewardPool.userRewardPerTokenPaid[msg.sender])) + s_rewardPool.ameliaRewards[msg.sender];
    }
    ...
    function getUserAmeliaRewards(address _walletAddress) external view returns (uint256) {
        return s_rewardPool.ameliaRewards[_walletAddress];
    }
