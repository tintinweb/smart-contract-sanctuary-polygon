...
contract A4AMarket is Ownable, IERC721Receiver {
    ...
    modifier onlyOwnerOrA4A(uint256 itemID) {
        require( a4A_items.ownerOf(itemID) == _msgSender() || a4A_items.owner() == _msgSender(), "Sender is not the owner." );
        _;
    }
    ...
    function listItem(uint256 itemID, uint256 price) external onlyOwnerOrA4A(itemID) {
