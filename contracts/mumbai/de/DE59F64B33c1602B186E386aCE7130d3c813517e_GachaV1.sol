    ...
    PausableUpgradeable
{
    ...
    uint256 public minimalMel;
    uint256 public referralItemId;
    uint256 public referralAmount;
    uint256 constant gachaVoucherGameItemId = 7;
     ...
     */
    function addBulkAddressToWhitelist(
    ...
    function gacha(
    ...
    ) public override whenNotPaused whitelistMiddleware {
        require(
            !userHasPendingRequest[_msgSender()],
            "User have pending request"
        );
        require(
        ...
        require(
            IMelXNFT(melxNftAddress).getUserGarageLimit(_msgSender()) >
                IERC721Upgradeable(melxNftAddress).balanceOf(_msgSender()),
            "Not enough garage space"
        );
        _fee(msg.sender, address(0));
        _gacha(_city, address(0));
    }

    function gachaWithVoucher(
        string calldata _city
    ) public whenNotPaused whitelistMiddleware {
        require(
            gameItem.itemBalance(msg.sender, gachaVoucherGameItemId) >= 1,
            "You don't have enough voucher"
        );
        require(
            IMelXNFT(melxNftAddress).getUserGarageLimit(_msgSender()) >
                IERC721Upgradeable(melxNftAddress).balanceOf(_msgSender()),
        ...
        gameItem.burn(msg.sender, gachaVoucherGameItemId, 1);
        _gacha(_city, address(0));
    }
    ...
    function gachaWithReferral(
        ...
        _fee(msg.sender, _referralAddress);
    ...
    function setMinimalMelToken(uint256 _minimalMel) public onlyAdmin {
        minimalMel = _minimalMel;
    }

    function setFeeAmount(uint256 _feeAmount) public onlyAdmin {
        feeAmount = _feeAmount;
    }

    function setFeeReceiver(address _feeReceiver) public onlyAdmin {
        feeReceiver = _feeReceiver;
    }

    function setReferralItemId(uint256 _referralItemId) public onlyAdmin {
        referralItemId = _referralItemId;
    }

    function setReferralAmount(uint256 _referralAmount) public onlyAdmin {
        referralAmount = _referralAmount;
    }

    /**
     * @dev Update user pending status, in case vrf fail, so certain address not stuck
     */
    function updateUserPendingStatus(
        bool _status,
        address _user
    ) public onlyAdmin {
        userHasPendingRequest[_user] = _status;
    }

    /**
     * @dev Gacha function to do Gacha
     * Only when contract is unpause user can call this function
     * NOTES CAN BE REVERTED IF NFT CONTRACT NOT WELL CONFIGURED
     * FOR EXAMPLE FEE CONFIGURE AND ALREADY MINT LIMIT
     */
    function _gacha(string calldata _city, address _referralAddress) internal {
        userHasPendingRequest[_msgSender()] = true;
    ...
    function fulfillRandomWords(
        ...
        userHasPendingRequest[requestIdToRequestData[_requestId].user] = false;
        ...
        for (
            ...
            if (cumulativeWeight[itemIndex] >= randomness) {
                ...
                uint256 tokenId = IMelXNFT(melxNftAddress).safeMintWithGacha(
                    ...
                    requestIdToRequestData[_requestId].referralAddress,
                    nftData
                );
                emit MintWithGacha(
                    nftData.name,
    ...
    function _fee(address _payer, address _referral) internal {
        ...
        if (_referral != address(0)) {
            ...
            gameItem.mint(_payer, referralItemId, referralAmount, "");
        }

        IERC20Upgradeable(melTokenAddress).transferFrom(
            _payer,
            feeReceiver,
            feeAmount
        );
...
interface IMelXNFT {
    ...
    function increaseUserGarageLimit(
    ...
    function getMintingLimit(
        string memory _city
    ) external view returns (uint64);

    function getDefaultGarageLimit() external view returns (uint8);

    function getUserGarageLimit(
        address _userAddress
    ) external view returns (uint32);

    function getCityToMinted(
        string memory _city
    ) external view returns (uint64);
...
interface IGameItem {
    ...
    function burn(address userAddress, uint256 itemId, uint256 amount) external;
    ...
    function getItemInformation(
        uint256 _itemId
    ) external view returns (ItemInformation memory);
...
contract SinglePlayerV1 is ISinglePlayer, Initializable, UUPSUpgradeable {
    ...
    function setMelPrize(uint256 _melPrize) public onlyAdmin {
        melPrize = _melPrize;
    }
    ...
    function setPoolContractAddress(
        address _poolContractAddress
    ) public onlyAdmin {
        poolContractAddress = _poolContractAddress;
    }

    function setStoreContractAddress(
        address _storeContractAddress
    ) public onlyAdmin {
        storeContractAddress = _storeContractAddress;
    }
    ...
    function getTrackInformation(
        uint8 _trackId
    ) public view returns (TrackInformation memory) {
...
interface IEnergy {
    ...
    function addUserEnergy(address _user, uint256 _energy) external;
    ...
    AccessControlUpgradeable
{
    ...
    bytes32 constant PACKAGE_CONTRACT = keccak256("PACKAGE_CONTRACT");
    ...
    function addUserEnergy(
        address _user,
        uint256 _energy
    ) public override onlyRole(PACKAGE_CONTRACT) {
        addressToEnergy[_user] += _energy;
    }
    ...
    function buyPart(uint256 _partId) public override whenNotPaused {
        IERC20Upgradeable melToken = IERC20Upgradeable(melTokenAddress);
        uint256 userBalance = melToken.balanceOf(_msgSender());
    ...
    function buyGameItem(uint256 _itemId, uint256 _amount) public {
        ...
        uint256 userBalance = melToken.balanceOf(_msgSender());
        require(
            IGameItem(gameItemAddress).getItemInformation(_itemId).price > 0,
            "Item unavailable"
        );
        require(
            userBalance >=
    ...
    function buyItemBundle(uint256 _bundleId) public {
        IERC20Upgradeable melToken = IERC20Upgradeable(melTokenAddress);
        uint256 userBalance = melToken.balanceOf(_msgSender());
        require(bundleIdToItemBundle[_bundleId].price > 0, "Item unavailable");
        require(
    ...
    function useEnergy(
        address _address,
        uint256 _energy
    ) public override onlyRole(SPEND_ENERGY_ROLE) {
    ...
    function useItem(uint256 _nftId, uint256 _itemId, uint8 _amount) public {
        require(
        ...
        emit UseItem(_msgSender(), _nftId, _itemId, _amount);
    }
    ...
    function buyGarageLimit() public override {
        ...
        melToken.transferFrom(_msgSender(), feeReceiver, garagePrice);
...
import {IEnergy} from "../game/IEnergy.sol";
import {IGameItem} from "../game/IGameItem.sol";

contract Package is AccessControl {
    struct PackageInfo {
        string packageName;
        uint256[] items;
        uint256[] quantity;
    ...
    mapping(uint256 => PackageInfo) public packageIdToPackageInfo;

    event BuyItemPackage(address indexed buyer, uint256 packageId);
    event Withdrawn(address indexed to, uint256 amount);

    constructor(address _storeAddress, address _gameItemAddress) public {
    ...
    function setPackageInfo(
        ...
        packageIdToPackageInfo[_packageId].packageName = _packageName;
        packageIdToPackageInfo[_packageId].items = _items;
        packageIdToPackageInfo[_packageId].quantity = _quantity;
        packageIdToPackageInfo[_packageId].energy = _energy;
        packageIdToPackageInfo[_packageId].price = _price;
    }
    ...
    function buyPackage(uint256 _packageId) public payable {
        require(isPresaleActive, "Presale package unavailable");
        require(
            packageIdToPackageInfo[_packageId].price <=
                address(msg.sender).balance,
            "Not enough balance"
        );
        require(
            msg.value == packageIdToPackageInfo[_packageId].price,
        ...
        (bool sent, bytes memory data) = _to.call{
            value: packageIdToPackageInfo[_packageId].price
        }("");
        require(sent, "Failed to send Ether");

        IGameItem(gameItemAddress).mintBatch(
            msg.sender,
            packageIdToPackageInfo[_packageId].items,
            packageIdToPackageInfo[_packageId].quantity,
            ""
        );

        IEnergy(storeAddress).addUserEnergy(
            msg.sender,
            packageIdToPackageInfo[_packageId].energy
        );

        emit BuyItemPackage(msg.sender, _packageId);
    }

    function withdraw(address _to) public onlyRole(DEFAULT_ADMIN_ROLE) {
        uint256 balance = address(this).balance;
        (bool sent, bytes memory data) = payable(_to).call{value: balance}("");
        require(sent, "Failed to send Ether");
...
import {IMelXNFT} from "../melX/IMelXNFT.sol";
import "../upgradeable/VRFConsumerBaseV2Upgradeable.sol";
import {DummyMel} from "../dummy-token/DummyMel.sol";
import {AccessControlUpgradeable} from "@openzeppelin/contracts-upgradeable/access/AccessControlUpgradeable.sol";
import {IERC721Upgradeable} from "@openzeppelin/contracts-upgradeable/token/ERC721/IERC721Upgradeable.sol";
import "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
import "@openzeppelin/contracts-upgradeable/proxy/utils/UUPSUpgradeable.sol";
    ...
    AccessControlUpgradeable
{
    ...
    // Variable tok keep mel Token contract address
    address melTokenAddress;
    ...
    /**
     * @dev Re-request VRF function
     */
    function recallVRF() external onlyRole(DEFAULT_ADMIN_ROLE) {
        _requestRandomness(
            callbackGasLimit,
            requestConfirmation,
            uint32(raceId.current())
        );
    }
     ...
     */
    function setMaxPlayerPerRound(
        uint8 _maxPlayerPerRound
    ) public onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function joinRace(
        uint256 _nftId
    ) public override onlyNftOwner(_nftId, msg.sender) {
        RoundInformation storage roundInformation = roundIdToRoundInformation[
            raceId.current()
        ];

        require(!isRaceStarted, "Race has been started!");
        require(
        ...
        require(
            DummyMel(melTokenAddress).balanceOf(msg.sender) >= racePoolFee,
            "Not enough Mel token"
        );

        DummyMel(melTokenAddress).transferFrom(
            msg.sender,
            address(this),
            racePoolFee
        );
    ...
    function startRace() public override {
        _startRace();
        isRaceStarted = true;
        DummyMel(melTokenAddress).transfer(msg.sender, startRaceIncentive);
    ...
    function claimPrize(
        uint256 _nftId,
        uint256 _roundId
    ) public onlyNftOwner(_nftId, msg.sender) {
        RoundInformation storage roundInformation = roundIdToRoundInformation[
            _roundId
        ];
    ...
    function claimCollectableCommission() public onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        DummyMel(melTokenAddress).transfer(msg.sender, claimable);
    }

    /**
     * @dev Function to let admin update number of winner
     */
    function setNumberOfWinner(
        uint8 _numberOfWinner
    ) public onlyRole(DEFAULT_ADMIN_ROLE) {
     ...
     */
    function setRacePoolFee(
        uint256 _racePoolFee
    ) public onlyRole(DEFAULT_ADMIN_ROLE) {
     ...
     */
    function setRaceCommission(
        uint256 _raceCommission
    ) public onlyRole(DEFAULT_ADMIN_ROLE) {
     ...
     */
    function setRaceCommissionDivider(
        uint256 _raceCommissionDivider
    ) public onlyRole(DEFAULT_ADMIN_ROLE) {
     ...
     */
    function setMinPlayer(
        uint8 _minPlayerPerRound
    ) public onlyRole(DEFAULT_ADMIN_ROLE) {
     ...
     */
    function setMinimumTimeToStartRound(
        uint256 _minimumTimeToStartRace
    ) public onlyRole(DEFAULT_ADMIN_ROLE) {
        minimumTimeToStartRace = _minimumTimeToStartRace;
    }

    function setStartRaceIncentive(
        uint256 _startRaceIncentive
    ) public onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function _claimPrize(
        ...
        DummyMel(melTokenAddress).transfer(_claimer, userPrize);
    ...
    IMelXNFT
{
    ...
    event BikeStatusUpdated(uint256 indexed nftId);

    function initialize(uint8 _garageLimit) public initializer {
        __ERC721Pausable_init_unchained();
        __ERC721_init_unchained("MelXNFT", "MELXNFT");
        __AccessControl_init_unchained();
        garageLimit = _garageLimit;
    ...
    function levelUp(
        uint16 _exp,
        uint256 _nftId
    ) external override onlyRole(GAME_ROLE) returns (uint8, bool) {
        ...
        if (currentExp >= levelToExp[nftIdToNftInformation[_nftId].level]) {
            ...
            emit BikeStatusUpdated(_nftId);
        }

        return (nftIdToNftInformation[_nftId].level, isLevelUp);
    }

    function getNftBaseStat(
        uint256 _nftId
    ...
    function safeMintWithGacha(
        ...
        _tokenIdCounter.increment();
        cityToMinted[_city]++;
        emit BikeStatusUpdated(tokenId);
        return tokenId;
    }
    ...
    function increaseUserGarageLimit(
        address _userAddress,
        uint32 _garageLimit
    ) public override onlyRole(GAME_ROLE) {
     ...
     */
    function addNewCity(
        string calldata _newCity,
        uint64 _limitMinting
    ) public onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    UUPSUpgradeable
{
    ...
    function getItemInformation(
        uint256 _itemId
    ) public view virtual override returns (ItemInformation memory) {
...
import {GameItemV1} from "../game/GameItemV1.sol";

contract GameItemDummyImplementation is GameItemV1 {}
...
contract Faucet {
    address admin;
    address public melxAddress;
    uint256 public melxBalance;
    uint256 public melxFaucetAmount;
    uint256 public bbFaucetAmount;

    constructor(address _melxAddress) {
        admin = msg.sender;
        melxAddress = _melxAddress;
        melxFaucetAmount = 30000 ether;
    }

    function provideMelx() external {
        IERC20Upgradeable(melxAddress).transfer(msg.sender, melxFaucetAmount);
    }

    function setMelxAddress(address _melxAddress) external {
        require(msg.sender == admin, "Only Admin");
        melxAddress = _melxAddress;
    }

    function setMelxFaucetAmount(uint256 _melxFaucetAmount) external {
        require(msg.sender == admin, "Only Admin");
        melxFaucetAmount = _melxFaucetAmount;
    }
}
...
import "../race/RaceV1.sol";

contract RaceDummyImplementation is RaceV1 {
    string public dummyUpgrade;
}
