...
// File: OneAIDistributor.sol

pragma solidity ^0.8.15;

contract OneAIDistributor is Ownable {
    ...
    uint256 public constant MAX_TOKEN = 336_000_000_000 * 1e6;
    uint256 public constant INIT_CLAIM = 3_780_000 * 1e6;
    uint256 public constant MAX_REFER_TOKEN = 16_800_000_000 * 1e6;
    ...
    function claim(
        ...
        require(_usedNonce[nonce] == false, "OneAI: nonce already used");
        require(_claimedUser[_msgSender()] == false, "OneAI: already claimed");

        _claimedUser[_msgSender()] = true;
        require(isValidSignature(nonce, signature), "OneAI: only auth claims");

        _usedNonce[nonce] = true;

        uint256 supplyPerAddress = canClaimAmount();
        require(supplyPerAddress >= 1e6, "OneAI: airdrop has ended");
    ...
    function setSigner(address val) public onlyOwner {
        require(val != address(0), "OneAI: val is the zero address");
        signer = val;
    }
    ...
    function isValidSignature(
        ...
        bytes32 msgHash = keccak256(abi.encodePacked(_msgSender(), nonce));
        bytes32 signedHash = keccak256(
            abi.encodePacked("\x19Ethereum Signed Message:\n32", msgHash)
        );

        return signer == signedHash.recover(signature);
    }
}