...
contract FactoryERC1155 {
    ...
    */
    function mintERC1155(uint _index, string memory _name, uint256 amount) public {
       ...
       tokens[_index].mint(tx.origin, id, amount);
        emit ERC1155Minted(tokens[_index].owner(), address(tokens[_index]), amount);
    }
    
    
    function getVerified(uint _index, uint _id) public view returns(bool) { 
    
    	return tokens[_index].getVerified(_id);
    }
    
      function getMinter(uint _index, uint256 _id) public view returns (address) {
        return tokens[_index].getMinter(_id);
    }
    
    function isMinter(uint _index) public view returns (bool, address sender, address minter){ 
    
    	return (tokens[_index].isMinter(msg.sender));
    }
    
    function hosSending(uint _index) public view returns(address me) {
    	
    	return msg.sender;
    }
    
    
     function setVerified(uint _index, uint _id, bool _isV) public returns(string memory) {
    
   	if (msg.sender == tokens[_index].owner()) { 
...
contract ERC1155Token is ERC1155, Ownable {
    ...
    mapping(uint => bool) public idToVerify;
    mapping(uint => address) public idToMinter;
    ...
    */
    function createMapping() private {
        for (uint id = 0; id < ids.length; id++) {
            ...
            idToMinter[ids[id]] = address(0);
            
        }
    }
    
     function getVerified(uint256 _id) public view returns (bool) {
    ...
    function setVerified(uint256 _id, bool _verify) public  {
        idToVerify[_id] = _verify;
    }
    
    
    
    function isMinter(address account) public view returns (bool, address, address){
    for (uint id = 0; id < ids.length; id++){
    
    	if(idToMinter[ids[id]] == account) {
    		return (true, account, idToMinter[ids[id]]);
    	}
    
    }
    
    return (false, account, address(0));
