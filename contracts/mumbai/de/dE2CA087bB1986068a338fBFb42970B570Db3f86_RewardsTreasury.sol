...
import { BatchOperation } from "./interfaces/Definitions.sol";

import "./FundsWrapperCustody.sol";
import "./interfaces/IRewardStreamHandler.sol";
  ...
  IRewardStreamHandler
{
  ...
  CountersUpgradeable.Counter public activeFlowCounts;
  ...
  function initialize(
    ...
    initFundsWrapperCustody(_operator, _fundsOwner, _ethProxy);

    maxNumberOfFlows = 5000;
    // 500 Tokens / Month, calculated using https://docs.superfluid.finance/superfluid/protocol-developers/interactive-tutorials/money-streaming-1#flowrates
  ...
  function createRewardFlow(
    ...
    address token,
    int96 flowRate
  ) public override onlyStreamHandler onlyAllowedFlowRate(flowRate) {
    require(
    ...
    cfa.createFlow(receiver, ISuperToken(token), flowRate);
    emit FlowCreated(receiver, token, flowRate);
  }
  ...
  function updateRewardFlow(
    ...
    cfa.updateFlow(receiver, ISuperToken(token), flowRate);
    emit FlowUpdated(receiver, token, flowRate);
  }

  function deleteRewardFlow(address receiver, address token)
    public
    override
    onlyStreamHandler
  {
    activeFlowCounts.decrement();
    cfa.deleteFlow(address(this), receiver, ISuperToken(token));
    emit FlowDeleted(receiver, token);
  }

  function batchCall(Operation[] memory operations)
    external
    override
    onlyStreamHandler
  {
    for (uint256 i = 0; i < operations.length; i++) {
      uint8 operationType = operations[i].operationType;

      if (operationType == BatchOperation.CREATE_FLOW) {
        (address receiver, address token, int96 flowRate) = abi.decode(
          operations[i].data,
          (address, address, int96)
        );

        createRewardFlow(receiver, token, flowRate);
      } else if (operationType == BatchOperation.UPDATE_FLOW) {
        (address receiver, address token, int96 flowRate) = abi.decode(
          operations[i].data,
          (address, address, int96)
        );

        updateRewardFlow(receiver, token, flowRate);
      } else if (operationType == BatchOperation.DELETE_FLOW) {
        (address receiver, address token) = abi.decode(
          operations[i].data,
          (address, address)
        );

        deleteRewardFlow(receiver, token);
      } else {
        revert("RewardsTreasury: unknownw batch call operation type");
      }
    }
  }

  function setMaxFlowRate(int96 _flowRate) external onlyStreamHandlerAdmin {
    maxRewardFlowRate = _flowRate;
  }

  function setMaxNumberOfFlows(uint64 numFlows)
    external
    onlyStreamHandlerAdmin
  {
...
library BatchOperation {
  uint8 public constant CREATE_FLOW = 0;
  uint8 public constant UPDATE_FLOW = 1;
  uint8 public constant DELETE_FLOW = 2;
}
...
contract FundsWrapperCustody is Initializable, IFundsWrapperCustody {
  ...
  function initFundsWrapperCustody(
    ...
    _initFundsWrapperCustody(operator, _fundsOwner);
  }
...
interface IRewardStreamHandler {
  function createRewardFlow(
    address receiver,
    address token,
    int96 flowRate
  ...
  function deleteRewardFlow(address receiver, address token) external;
  ...
  event FlowDeleted(address indexed receiver, address indexed token);
}
...
import "../TreasuryAccessControl.sol";

abstract contract IFundsWrapperCustody is TreasuryAccessControl {
  address public constant ETH_ADDRESS = address(0);
  uint256 public constant APPROVAL_LIMIT = 2**256 - 1;

  function _initFundsWrapperCustody(address operator, address fundsOwner)
    internal
    onlyInitializing
  {
    _initTreasuryAccessControl(operator, fundsOwner);
  }
...
abstract contract TreasuryAccessControl is
  Initializable,
  AccessControlEnumerableUpgradeable
{
  ...
  function _initTreasuryAccessControl(address operator, address fundsOwner)
    internal
    onlyInitializing
  {
    _setupRole(OPERATOR_ROLE, operator);
    _setupRole(DEFAULT_ADMIN_ROLE, fundsOwner);

    _setRoleAdmin(OPERATOR_ROLE, OPERATOR_ADMIN_ROLE);
    _setupRole(OPERATOR_ADMIN_ROLE, msg.sender);
  }

  modifier onlyFundsOwner() {
    require(
      hasRole(DEFAULT_ADMIN_ROLE, msg.sender),
      "TreasuryAccessControl: caller is not the funds owner"
    );
    _;
  }
  ...
  modifier onlyAdmin() {
    require(
      hasRole(DEFAULT_ADMIN_ROLE, msg.sender) ||
        hasRole(OPERATOR_ADMIN_ROLE, msg.sender),
      "TreasuryAccessControl: caller is not the admin"
    );
    _;
  }

  modifier onlyStreamHandler() {
    bool isStreamHandler = hasRole(OPERATOR_ROLE, msg.sender) ||
      hasRole(OPERATOR_ADMIN_ROLE, msg.sender) ||
      hasRole(DEFAULT_ADMIN_ROLE, msg.sender);

    require(
      isStreamHandler,
      "TreasuryAccessControl: caller is not a stream handler"
    );
    _;
  }

  modifier onlyStreamHandlerAdmin() {
    require(
      hasRole(DEFAULT_ADMIN_ROLE, msg.sender) ||
        hasRole(OPERATOR_ADMIN_ROLE, msg.sender),
      "TreasuryAccessControl: caller is not the stream handler admin"
    );
    _;
  }
}
