...
contract Faucet1 { 
     ...
     */
    function initialize (address nftAddress) onlyRole(ds.ADMIN) external {   
        ls.LibStorage storage lib = ls.libStorage();
        require(!lib.initialized, "already initialized");
        lib.initialized = true;
        lib._lockId = 1;
        lib.SIGNER = 0xb114E604972e7D2e510730768226DEB6B0F07363; //temp Remove
        lib.minLockDate = 16768238;
        ...
        lib.QUOTER_ADDRESS = 0xb27308f9F90D607463bb33eA1BeBb41C27CE5AB6;
        lib.UNISWAP_V3_ROUTER = 0xE592427A0AEce92De3Edee1F18E0157C05861564; 
        // lib.WETH = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270; // miannet
        lib.WETH = 0x9c3C9283D3e44854697Cd22D3Faa240Cfb032889; // testnet pilygon
    }
    ...
    /**
     * @dev Allows users to deposit tokens into the contract, locking them for a specific period of time and on the expected percentage of growth:
    ...
    {
        ls.LibStorage storage lib = ls.libStorage();
        ls.Token storage token = lib._tokenVsIndex[_addr[0]];
        require(token.status == ls.Status.OPEN, "invalid _token");
        require(_endDate > block.timestamp, "invalid _endDate");
        require(_amount >= token.minAmount,"incorect _amount");
        require(!(!_features[0] && !_features[1]), "Features's values are not correct"); //Check
        uint newAmount = ls._calculateFixedFee(_addr[0],_amount,true);
        uint totalAmount = newAmount + _otherFees;
        require(lib.ETH == _addr[0] ? msg.value >= totalAmount :IERC20(_addr[0]).transferFrom(msg.sender, address(this), totalAmount),'tx.failed');
        uint affiliateFee = (newAmount - _amount) * lib.affiliateRate / 100;
        console.log("Faucet1_deposit()_affiliateFee", affiliateFee);
        token.balance += (totalAmount - _amount - affiliateFee);
        uint tokenId = lib._lockId++;
        int priceInUSD = ls.getLatestPrice(token.priceFeedAddress);
        lib._idVsLockedAsset[tokenId] = ls.LockedAsset({ token : _addr[0], beneficiary : _addr[1],
        creator : payable(msg.sender), amount : _amount, feeRate : lib.EndFee, endDate : _endDate, target :_target, 
        claimedAmount : 0, priceInUSD : priceInUSD, features : _features, status : ls.Status.OPEN });
        (!_features[0]) ? gemChest.safeMint(address(this),tokenId) : gemChest.safeMint(_addr[1], tokenId);
        _addr[2] == address(0) ? () : ls.transferFromContract(_addr[0], _addr[2], affiliateFee);
        emit Deposit(_uuid, tokenId, _amount, priceInUSD, msg.sender, "Success");
    }
     ...
     */
   function claim(uint256 _id, address _swapToken) public payable {
        uint giftreward;
        uint amountOutMinimum;
        uint amountOraclePrice;
        ...
        uint newAmount = ls._calculateFee(asset.amount, false, asset.feeRate);  // Locked amount - fee
        token.balance += (asset.amount - newAmount);                            // Add upmentioned fee to token balance      
        ...
        if (asset.features[1] && eventIs && asset.creator != asset.beneficiary){
            ...
            giftreward = (newAmount * 10) / 100;                                //should be dynamic
            newAmount -= giftreward;
            console.log("Faucet1_claim()_features[1]: ",newAmount, giftreward, asset.creator);
            if(!asset.features[2]){
        ...
        if (asset.features[2] && asset.token != _swapToken) {
            ...
            console.log("Faucet1_claim()_priceInUSD",amountOutMinimum);
            uint oraclePriceLenght = uint256(bytes(Strings.toString(uint(ls.getLatestPrice(token.priceFeedAddress)))).length);
            
            oraclePriceLenght = (oraclePriceLenght >= 8 ) ? 8 : 8 + (9 - oraclePriceLenght);
            amountOraclePrice = (((uint(ls.getLatestPrice(token.priceFeedAddress)) * newAmount ) / 10 ** token.decimal ) * 10**lib._tokenVsIndex[_swapToken].decimal)  / 10 ** oraclePriceLenght;
            amountOraclePrice -= amountOraclePrice / 20;   // 5% tollerable slippage rate 
            // console.log("Faucet1_claim()_amountOutMinimum:amountOraclePrice",amountOutMinimum,amountOraclePrice);
            if (amountOutMinimum >= amountOraclePrice){ 
                if(GiftRewardReady){
                console.log("Faucet1_claim()_block2_newAmount", giftreward,  asset.creator, amountOutMinimum);
                    require(swap(asset.token, _swapToken, giftreward, asset.creator));
                }
                amountOutMinimum = ls.getAmountOutMin(asset.token, _swapToken, newAmount) ;
                require(_swapped = swap(asset.token, _swapToken, newAmount,receiver), "Swap error from claim");
            } else {
                if (asset.features[0]) {
        ...
        asset.claimedAmount = (!asset.features[0]) ? ((asset.features[2] && _swapped) ? amountOutMinimum : newAmount) : 0 ;
        gemChest.burn(_id);
     ...
     */
    function swap (address _tokenIn, address _tokenOut, uint _amountIn,address _to) public payable returns (bool){
        ls.LibStorage storage lib = ls.libStorage();
        ...
        if (_tokenIn != lib.ETH){
            ...
            ISwapRouter(lib.UNISWAP_V3_ROUTER).exactInputSingle(params);
        } else {
            ISwapRouter.ExactInputSingleParams memory params = ISwapRouter.ExactInputSingleParams({
                tokenIn: lib.WETH,
                tokenOut: path[1],
            ...
            ISwapRouter(lib.UNISWAP_V3_ROUTER).exactInputSingle{value:_amountIn}(params);
    ...
    /**
     * @dev swapBeforeDeposit function allows swapping of tokens before depositing them into the contract.
     * @param _token The token to be swapped.
     * @param _addr The array of addresses to receive the swapped tokens.
     * @param _amount The amount of tokens to be swapped.
     * @param _otherFees The amount of fees to be charged.
     * @param _endDate The date of the asset's vesting period to end.
     * @param _target The target rate of locked amount.
     * @param _features The array of features of the asset.
     * @param _uuid The UUID of the asset.
     * 
     * @notice Inactive.
     */
    ...
    //     uint _target,
    //     bool [] memory _features,
    //     string memory _uuid)  public {

    //     ls.LibStorage storage lib = ls.libStorage();
    //     ls.Token storage token = lib._tokenVsIndex[_token];
    //     int priceInUSD = ls.getLatestPrice(token.priceFeedAddress);
    //     require(_token != _addr[0], "incorrect token");
    //     uint oraclePriceLenght = uint256(bytes(Strings.toString(uint(priceInUSD))).length);
    //     oraclePriceLenght = (oraclePriceLenght > 8 ) ? 8 : 8 + (9 - oraclePriceLenght);
    //     uint amountOraclePrice = (((uint(priceInUSD) * _amount ) / 10**token.decimal ) * 10**lib._tokenVsIndex[_addr[0]].decimal)  / 10 ** oraclePriceLenght;
    //     amountOraclePrice -= amountOraclePrice / 20;
    //     uint _amountforDeposit = ls.getAmountOutMin(_token, _addr[0], _amount);
    //     require(_amountforDeposit >= amountOraclePrice);
    //     swap(_token, _addr[0], _amount, msg.sender);
    //     deposit(_addr, _amountforDeposit, _otherFees, _endDate, _target, _features, _uuid);
    //     uint tokenId = ls.libStorage()._lockId;
    //     emit Deposit(_uuid, tokenId, _amountforDeposit, priceInUSD, msg.sender, "Success");
    // }
     ...
     */
    function submitBeneficiary(uint _id, string memory _message, bytes memory _signature, address _swapToken, address payable _newBeneficiary) public {
        ls.LibStorage storage lib = ls.libStorage();
        ls.LockedAsset storage asset = lib._idVsLockedAsset[_id];
        ...
        if (asset.status == ls.Status.OPEN) {
            // if (!ds.hasRole(ds.ADMIN, msg.sender)){
            //     gemChest.approve(_newBeneficiary, _id);
            // }
            asset.beneficiary = _newBeneficiary;
            ...
            // require(IERC20(_swapToken).transfer(_newBeneficiary, _newAmount), "Transfer failed!");
        }
     }
    ...
    //     (bool sent,) = (_token==ls.libStorage().ETH) ?  _receiver.call{value: _value} ("") : (IERC20(_token).transfer(_receiver,_value), bytes(""));
...
library LibraryStorage {
    ...
    function transferFromContract (address _token, address _receiver, uint _value) internal {
        ...
        require(sent, "tx failed");
        console.log("LibStorage_transferFromContract():msg.sender", _token, _receiver, _value);
    } 
}
