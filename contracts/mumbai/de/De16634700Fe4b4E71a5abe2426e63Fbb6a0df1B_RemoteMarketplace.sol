...
contract RemoteMarketplace is Ownable {
    ...
    function handleQueryProject(
        ...
        // console.log('handleQueryProject => beneficiary: %s, pricingTokenAddress: %s', beneficiary, pricingTokenAddress);
        // console.log('                      price: %s, fee: %s, purchaseId: %s', price, fee, purchaseId);
    ...
    function buyFor(bytes32 projectId, uint256 subscriptionSeconds, address subscriber) public {
        // console.log('buyFor => subscriptionSeconds: %s, subscriber: %s, msg.sender: %s', subscriptionSeconds, subscriber, msg.sender);
        uint256 purchaseId = purchaseCount + 1;
        purchaseCount = purchaseId;
        queryRouter.query(
            destinationDomain,
            recipientAddress,
            abi.encodeCall(IMarketplace.getPurchaseInfo, (projectId, subscriptionSeconds, purchaseId)),
    ...
    function _handleProjectPurchase(address buyer, address beneficiary, address pricingTokenAddress, uint256 price, uint256 fee) private {
        // console.log('_handleProjectPurchase => buyer: %s, beneficiary: %s, pricingTokenAddress: %s', buyer, beneficiary, pricingTokenAddress);
        // console.log('                          price: %s, fee: %s', price, fee);
        ...
        // require(pricingToken.transfer(beneficiary, price - fee), "error_projectPaymentFailed");
        // require(pricingToken.transfer(owner(), fee), "error_feePaymentFailed");
    }

    function _subscribeToProject(bytes32 projectId, uint256 subscriptionSeconds, address subscriber) private {
        // console.log('_subscribeToProject => subscriptionSeconds: %s, subscriber: %s', subscriptionSeconds, subscriber);
        outbox.dispatch(
            destinationDomain,
