...
contract FollowNFT is BondNFTBase, IFollowNFT {
    ...
    constructor(address hub) {
        emit Events.OutputLogString('IFollowNFT constructor ',hub,0,_initialized);
    ...
    function initialize(uint256 profileId, uint256 followType, bool burnSign) external override {
        emit Events.OutputLogString('IFollowNFT initialize ',HUB,profileId,_initialized);
    ...
    function tokenOfOwner(address owner) external view returns (uint256) {
        if ( 0 == ERC721Time.balanceOf(owner) )
             return 0;
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal override {
        ...
        //  console.log("_profileId %s %s", _profileId, HUB );
        // console.log("beforeTokenTransfer %s %s %s",  tokenId, from, to);
        //IBondHub(HUB).emitFollowNFTTransferEvent(_profileId, tokenId, from, to);
