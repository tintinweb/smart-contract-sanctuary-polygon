 ...
 */
  ...
  enum TokenType { STANDARD , TRAIT , MASTER }
 ...
 */
  ...
  mapping (uint256 => uint256[]) public masterToTraits;
 ...
 */
  ...
  modifier onlyOpenCollection(uint256 _collectionId) {
    ...
    require(!collections[_collectionId].paused || msg.sender == projects[collections[_collectionId].projectId].manager , "Purchases must not be paused");
    require(!collections[_collectionId].locked, "Only unlocked collections");
    _;
  }
  ...
  modifier onlyTokenOwner(uint256 _tokenId) {
      require(balanceOf(msg.sender, _tokenId) > 0 , "Must own token");
    _;
  }
 ...
 */
  ...
  returns (uint256[] memory)
  {
    require(tokens[_mNftId].tokenType == TokenType.MASTER ,"_mNFTId needs to be Master");
    
    for (uint256 i=0;i < masterToTraits[_mNftId].length; i++){
      addressToTokenIdToUsed[msg.sender][masterToTraits[_mNftId][i]]--;
      delete masterToTraits[_mNftId][i];}

    for (uint256 k=0;k < _traitIds.length; k++){
      require(balanceOf(msg.sender, _traitIds[k]) > addressToTokenIdToUsed[msg.sender][_traitIds[k]], "Trait must not be used");
      
      addressToTokenIdToUsed[msg.sender][_traitIds[k]]++;
    ...
    onlyOwner
  {
    for (uint256 i= 0; i < _name.length; i++) {
      ...
      token.tokenType = _tokenType[i];
      token.maxSupply = _maxSupply[i]; 
      token.tokenURI = _tokenURI[i];
      token.paused = true;
      token.locked = false;
      tokens[nextTokenId] = token;
      
      collectionIdToTokenIds[token.collectionId].push(nextTokenId);
      emit TokenCreated(token.name, nextTokenId,token.projectId ,token.collectionId , token.tokenPriceInWei, token.maxSupply);
      nextTokenId++;   
    }
  }
  ...
  function viewTokensOfAddress(address _address)
  public
  view
 ...
 */
  ...
  function createCollection(
    ...
    uint256 _id = nextCollectionId;
    collections[_id].name = _name;
    collections[_id].maxInvocations = _maxInvocations;
    collections[_id].paused = true;
    collections[_id].locked = false;
    projectToCollectionIds[_projectId].push(nextCollectionId);
    nextCollectionId++;
    emit CollectionCreated(_name, _id, _projectId, _maxInvocations);
...
function createProject(
    ...
    emit ProjectCreated(nextProjectId, _name);
    nextProjectId++;
    
  }

  function editProject( uint256 _id, string memory _name, address _manager) 
  public
  {
    require(msg.sender == projects[_id].manager || msg.sender == owner(), "Must be manager");
    require(address(0) != projects[_id].manager, "Project must exist");
    projects[_id].name = _name;
    projects[_id].manager = _manager;
