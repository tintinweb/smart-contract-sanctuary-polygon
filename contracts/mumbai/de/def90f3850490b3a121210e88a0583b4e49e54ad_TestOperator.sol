...
contract TestOperator is Client, Ownable {
  event ResponseData(uint256 requestId, bytes[] data);

  mapping(uint256 => uint256[]) public _publicInputs;
  mapping(uint256 => bytes) public _zkProofs;
  ...
  function fulfill(uint256 requestId, bytes[] memory data) external recordFulfillment(requestId) {
    uint256[] memory publicInputs = abi.decode(data[0], (uint256[]));
    bytes memory proof = abi.decode(data[1], (bytes));
    _publicInputs[requestId] = publicInputs;
    _zkProofs[requestId] = proof;
    emit ResponseData(requestId, data);
  }
}
