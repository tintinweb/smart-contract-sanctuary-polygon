...
contract OSBFactory is ReentrancyGuardUpgradeable, OwnableUpgradeable {
    struct Pay {
        address owner;
        address token;
    ...
    uint256 public opFundLimit;
    address public opFundReceiver;
    address public serviceFundReceiver;
    ...
    event Deployed(uint256 indexed payId, Pay payment);
    event WithdrawnFund(address indexed serviceFundReceiver, uint256 indexed value);
    event SetDeployFee(uint256 indexed oldFee, uint256 indexed newFee);
    ...
    event SetLibraryAddress(uint256 indexed oldLimit, uint256 indexed newLimit);

    function initialize(address _owner, uint256 _deployFee, uint256 _opFundLimit, address _opFundReceiver, address _library721Address, address _library1155Address) external initializer {
        __Ownable_init();

        require(_deployFee > 0, "Invalid fee");
        require(_opFundLimit > 0, "Invalid deployFund");
        require(_owner != address(0), "Invalid owner address");

        deployFee = _deployFee;
        opFundLimit = _opFundLimit;
		opFundReceiver = _opFundReceiver;

        serviceFundReceiver = _owner;
    ...
    function setLibraryAddress(address _library721Address, address _library1155Address) external onlyOwner {
        library721Address = _library721Address;
        library1155Address = _library1155Address;
    }
  
    function create(bool _isSingle, string memory _name, string memory _symbol, address _royaltyReceiver, uint96 _royaltyFeeNumerator) external payable {
        require(msg.value == deployFee, "Invalid deploy fee");

        lastPaymentId++;
        bytes32 salt = keccak256(abi.encodePacked(lastPaymentId));
        address deployedContract;

        if(_isSingle) {
            OSB721 _osb721 = OSB721(Clones.cloneDeterministic(library721Address, salt));
            _osb721.initialize(_msgSender(), _name, _symbol, _royaltyReceiver, _royaltyFeeNumerator); 
            deployedContract = address(_osb721);
        } else {
            OSB1155 _osb1155 = OSB1155(Clones.cloneDeterministic(library1155Address, salt));
            _osb1155.initialize(_msgSender(), _name, _symbol, _royaltyReceiver, _royaltyFeeNumerator); 
            deployedContract = address(_osb1155);
        }

        payments[lastPaymentId] = Pay({
            token: deployedContract,
            owner: _msgSender(),
            receiverRoyaltyFee: _royaltyReceiver,
            percentageRoyaltyFee: _royaltyFeeNumerator,
            fee: deployFee,
            name: _name,
            symbol: _symbol,
            isSingle: _isSingle
        ...
        uint256 operationBalance = address(opFundReceiver).balance;
        if (operationBalance < opFundLimit) {
            Helper.safeTransferNative(opFundReceiver, opFundLimit - operationBalance);
        }

        emit Deployed(lastPaymentId, payments[lastPaymentId]);
    }
    ...
    function setServiceFundReceiver(address _account) external onlyOwner {
        require(_account != address(0), "Receiver is the zero address");
    ...
    function setDeployFee(uint256 _fee) external onlyOwnerOrController {
        require(_fee > 0, "Invalid deploy fee");
        uint256 oldFee = deployFee;
        deployFee = _fee;
        emit SetDeployFee(oldFee, _fee);
    }

    function setOpFundLimit(uint256 _limit) external onlyOwner {
        require(_limit > 0, "Invalid operation fund limit");
...
contract OSB721 is ERC721Upgradeable, ERC721EnumerableUpgradeable, ERC2981Upgradeable, OwnableUpgradeable {
    ...
    uint256     public constant maxBatch = 50;
    ...
    function initialize(address _owner, string memory _name, string memory _symbol, address _receiverRoyaltyFee, uint96 _percentageRoyaltyFee) public initializer {
