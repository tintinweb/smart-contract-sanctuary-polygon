    ...
    ErrorCodes
{
    ...
    function createListing(ListingParameters calldata _params) public override {
        ...
        require(saleType == SaleType.Primary, throwError(SECONDARY_SALE_NOT_SUPPORTED));
        ...
        _tokenListings[_params.assetContract][_params.tokenId].push(listingId);
        ...
        public
    {
        ...
        require(
            listing.quantity > 0,
            throwError(CANNOT_MODIFY_INACTIVE_SALE)
        );
    ...
    function validateApproval(
        ...
        if (_tokenType == TokenType.ERC721) {
            if(IERC721(_assetContract).isApprovedForAll(_tokenOwner, operator) ||
            ...
            if(IERC1155(_assetContract).isApprovedForAll(_tokenOwner, operator)){
                return true;
            }
            else{
                return false;
    ...
    function setRoyaltyTreasury(address _treasury) public onlyAdmin {
    ...
    function removeListing(uint256 listingId) external {
        ...
        require(
            hasRole(DEFAULT_ADMIN_ROLE, msg.sender) || listing.tokenOwner == msg.sender ||
                validateApproval(
                    listing.tokenOwner,
        ...
        uint256 listedIdsIndex = _listingIdsIndex(assetContract, tokenId, listingId);
        _tokenListings[assetContract][tokenId][listedIdsIndex] = _tokenListings[assetContract][tokenId][_tokenListings[assetContract][tokenId].length-1];
        _tokenListings[assetContract][tokenId].pop();
        delete listings[listingId];
    }

    function _listingIdsIndex(address assetContract, uint256 tokenId, uint256 listingId) internal virtual returns (uint256 index) {
        
        uint256 length = _tokenListings[assetContract][tokenId].length;
        bool found;

        for(index = 0; index < length; index ++) {
            if(_tokenListings[assetContract][tokenId][index] == listingId) { found = true; return index; }
        }
        require(
            found,
        ...
        returns (Listing[] memory activeListings)
    {
        ...
        for (uint256 i = 0; i < _tokenListings[_assetContract][_tokenId].length; i++) {

            Listing memory currentListing = listings[_tokenListings[_assetContract][_tokenId][i]];
            if (currentListing.quantity > 0) {
    ...
    function bootstrap() external onlyAdmin {
        uint256 arrLength =0;
        Listing memory currentListing;
        uint256 insertionFlag = 0;
        for (uint256 i=0; i < listingIdTracker; i++){
            currentListing = listings[i];
            if(currentListing.assetContract != address(0x0)){
            arrLength = _tokenListings[currentListing.assetContract][currentListing.tokenId].length;
            
                if(arrLength == 0)
                    _tokenListings[currentListing.assetContract][currentListing.tokenId].push(currentListing.listingId);
                else{
                    for (uint256 arrayIndex = 0; arrayIndex < arrLength; arrayIndex++){
                        if(_tokenListings[currentListing.assetContract][currentListing.tokenId][arrayIndex] == currentListing.listingId)
                            insertionFlag = 1;
                    }
    ...
    function getTokenListings(address _assetcontract, uint256 _tokenId) public view onlyAdmin returns (uint256[] memory) {
        return _tokenListings[_assetcontract][_tokenId];
    }

    function clearTokenListings() public onlyAdmin {
        uint arrLength;
        Listing memory currentListing;
        for (uint256 i=0; i < listingIdTracker; i++){
            currentListing = listings[i];
            ...
            arrLength = _tokenListings[currentListing.assetContract][currentListing.tokenId].length;
                ...
                if(arrLength > 0){
                    for(uint256 j = 0; j < arrLength; j++) {
                        _tokenListings[currentListing.assetContract][currentListing.tokenId].pop();
                    }
                }
            }
