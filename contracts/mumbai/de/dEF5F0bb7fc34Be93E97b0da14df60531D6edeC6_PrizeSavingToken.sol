    ...
    BatchMintMetadata
{
    ...
    // function conductRaffle(uint256 _seed) external onlyAdmin {
    //     require(payoutTokenAddress != address(0), "Payout token address is not set");
    //     require(balanceOf(address(this), 1) > 0, "No tickets issued for this draw");

    //     uint256 numberOfTickets = balanceOf(address(this), 1);
    //     uint256 winningConsolationPrizeTickets = numberOfTickets / 5; // 20% of tickets will win consolation prize
    //     numberOfWinningTickets = numberOfTickets - winningConsolationPrizeTickets;

    //     // Send consolation prize to winners
    //     for (uint256 i = 0; i < winningConsolationPrizeTickets; i++) {
    //         uint256 tokenId = _random(numberOfTickets);
    //         address winner = address(uint160(uint256(ownerOf(tokenId))));
    //         IERC20(payoutTokenAddress).safeTransfer(winner, 1000);
    //     }

    //     // Send main prize to winner
    //     if (numberOfWinningTickets > 0) {
    //         uint256 tokenId = _random(numberOfWinningTickets);
    //         address winner = address(uint160(uint256(ownerOf(tokenId))));
    //         uint256 mainPrizeAmount = IERC20(payoutTokenAddress).balanceOf(address(this)) - (winningConsolationPrizeTickets * 1000);
    //         IERC20(payoutTokenAddress).safeTransfer(winner, mainPrizeAmount);
    //     }
    // }
