...
contract CoinsRebase is SysCtrl {
    ...
    event NewBonus(uint256 indexed coinID, address indexed leader, uint256 bonus);
...
contract core is CoinsRebase{
    ...
    constructor() {
        minTimeRebase = 60*60;    // 30 min  25 hours (25*60*60);
        minTimeLock = 10*60;      // 5 min         
        minTimeFee = 10*60;       // 5 min
        exfee24h = 500;           // Fee after 24 hours
        sellfee  = 25000;         // fee for withdrawal
        rebasePerc1 = 1000;       // 1 Rebase 
        rebasePerc2 = 500;        // 2 Rebase
        rebasePerc3 = 250;        // 3 Rebase
        maxleader_bonus = 1000;   // Max bonus
    }

    function buyRebase(uint256 _coin) external payable notPaused waitRebase returns (bool){
    ...
    function swap(uint256 _coinIN, uint256 _coinOUT, uint256 _amount) external reentrancyGuard notPaused waitTimeLock returns (bool){
        ...
        uint256 exfee = 0;
        if(lastRebase+(minTimeRebase-(minTimeLock+minTimeFee)) < block.timestamp){
        ...
        emit Exchange(_msgSender(), _coinIN, _coinOUT, _amount);
    ...
    function election(uint256 _coinID, uint256 _amount) external notPaused waitTimeLock returns (bool){
         // 888 before 24 hours
        ...
        if(coins[_coinID].leader_stake > 0){
            IERC20(coins[_coinID].token).transfer(_msgSender(), coins[_coinID].leader_stake);
        }
    ...
    function bonusLeader(uint _coinID, uint256 _bonus) external notPaused waitTimeLock returns (bool){
        // 888 only before 24 hours
        ...
        emit NewBonus(_coinID, coins[_coinID].leader, _bonus);
    ...
    // Mint new coin
    function mintCoin(uint256 _coinID, address _to, uint256 _value) external onlyDAO returns (bool) {
      // Controles adicionais aqui
      //
      return IERC20Rebase(coins[_coinID].token).mint(_to,_value);  
    }
    
    
    function rescue(address payable to_, uint256 amount_) external onlyDAO  {
