    ...
    AccessControlUpgradeable
{
    ...
    bytes32 public merkleRootSpotThree;
    ...
    event setmerkleRootSpotThreeEvent(bytes32 indexed merkleRoot);
    ...
    function initialize(
        ...
        require(address(_cultPunksNFT) != address(0), "Invalid address");
    ...
    function validatePhaseSpecificPurchase(
        ...
        if (phase == 1) {
            ...
            require(merkleRootSpotOne != "" && merkleRootSpotTwo != "" && merkleRootSpotThree != "", "PERMISSIONED SALE CLOSED");
            
            if(MerkleProofUpgradeable.verify(proofWL1, merkleRootSpotOne, leaf) == true && MerkleProofUpgradeable.verify(proofWL2, merkleRootSpotTwo, leaf) == true && MerkleProofUpgradeable.verify(proofWL3, merkleRootSpotThree, leaf) == true)
            {   
                if(addressToMintsWLNFT[msg.sender] > 0) {
                    ...
                    require((addressToMintsWLSpot[msg.sender] + amount) <= (wlPerSpotMint * 3), "BUY AMOUNT EXCEEDS MAX FOR USER");
                }
                addressToMintsWLSpot[msg.sender] = addressToMintsWLSpot[msg.sender] + amount;
            } else if(MerkleProofUpgradeable.verify(proofWL1, merkleRootSpotOne, leaf) == true && MerkleProofUpgradeable.verify(proofWL2, merkleRootSpotTwo, leaf) == true)
            {   
                if(addressToMintsWLNFT[msg.sender] > 0) {
                    require((addressToMintsWLSpot[msg.sender] + amount) <= addressToMintsWLNFT[msg.sender], "BUY AMOUNT EXCEEDS MAX FOR USER WL");
                } else {
    ...
    function buy(
        ...
        validatePhaseSpecificPurchase(amount, phase, proofWL1, proofWL2, proofWL3);
        bought += amount;
    ...
    /**
     * @notice Change the public sale max mint limit.
     *
     * @param _newMintLimit. The new limit.
     */
    ...
    /**
     * @notice Change white list mint limit per spot.
     *
     * @param _newMintSpotLimit. The new spot limit.
     */
    ...
    /**
     * @notice Change white list mint free.
     *
    ...
    /**
     * @notice Change public mint free.
     *
     * @param _status. The new status.
     */
    ...
    /**
     * @notice Change address to mint white list NFTs.
     *
     * @param _user. The user address.
     * @param _amount. The amount of user.
     */
    ...
    /**
     * @notice Change address to mint public NFTs.
     *
     * @param _user. The user address.
     * @param _amount. The amount of user.
     */
     ...
     * @notice Change the phase two merkle root.
     *execute
     * @param newRoot. The new merkleRootSpotThree.
     */
    function setmerkleRootSpotThree(bytes32 newRoot) external onlyOwner {
        merkleRootSpotThree = newRoot;
        emit setmerkleRootSpotThreeEvent(newRoot);
    }
