...
contract USDRExchange is AddressAccessor, IExchange, Pausable {
    ...
    function resetMintingStats() external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    {
        uint256 threshold = (usdrMarketCapE18 * lowerTNGBLMintingThreshold) / 100;
    ...
    function afterBurnTNGBL(uint256 amount) external {
        ...
        if (amount == 0) {
            ...
            if (int256(amount) >= tngbl) {
                ...
                if (usdr > 0 && tngbl > 0) {
                    int256 a = (tngbl * 1e36) / usdr;
                    int256 p = (int256(amount) * 1e36) / tngbl;
                    int256 uap = usdr * (a * 1e18 + a * p);
                    int256 x = ((tngbl * 1e36 - tngbl * p * 1e18 - uap)) / uap;
                    _updateTNGBLMintingStats(-int256(amount), x * usdr);
                }
            }
        }
    }
