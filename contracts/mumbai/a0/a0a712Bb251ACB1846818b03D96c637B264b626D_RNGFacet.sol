...
import {LibEvolution} from "LibEvolution.sol";

contract RNGFacet {

    function setVRFV2RequestConfirmationsByMechanicId(uint256 mechanicId, uint16 confirmations) external {
		LibDiamond.enforceIsContractOwner();
		require(confirmations >= 3, "RNGFacet: confirmations should be greater than or equal to 3");
		LibRNG.setVRFV2RequestConfirmationsByMechanicId(mechanicId, confirmations);
    }

	function setVRFV2NumWords(uint32 words) external {
		LibDiamond.enforceIsContractOwner();
		require(words > 0, "RNGFacet: word count should be greater than 0");
		LibRNG.setVRFV2NumWords(words);
    }

	function setVRFV2CallbackGasLimitByMechanicId(uint256 mechanicId, uint32 limit) external {
		LibDiamond.enforceIsContractOwner();
		LibRNG.setVRFV2CallbackGasLimitByMechanicId(mechanicId, limit);
    }

    function setVRFV2KeyHash(bytes32 keyHash) external {
		LibDiamond.enforceIsContractOwner();
		LibRNG.setVRFV2KeyHash(keyHash);
    }

    function setVRFV2VrfCoordinatorAddress(address coordinator) external {
		LibDiamond.enforceIsContractOwner();
		LibRNG.setVRFV2VrfCoordinatorAddress(coordinator);
    }

    function setVRFV2SubscriptionId(uint64 subscriptionId) external {
		LibDiamond.enforceIsContractOwner();
		LibRNG.setVRFV2SubscriptionId(subscriptionId);
    }

	function getVRFV2RequestConfirmationsByMechanicId(uint256 mechanicId) external view returns(uint16){
		return LibRNG.getVRFV2RequestConfirmationsByMechanicId(mechanicId);
	}

	function getVRFV2NumWords() external view returns(uint32){
		return LibRNG.getVRFV2NumWords();
	}

	function getVRFV2CallbackGasLimitByMechanicId(uint256 mechanicId) external view returns(uint32){
		return LibRNG.getVRFV2CallbackGasLimitByMechanicId(mechanicId);
	}

	function getVRFV2KeyHash() external view returns(bytes32){
		return LibRNG.getVRFV2KeyHash();
	}

	function getVRFV2VrfCoordinatorAddress() external view returns(address) {
		return LibRNG.getVRFV2VrfCoordinatorAddress();
	}

	function getVRFV2SubscriptionId() external view returns(uint64) {
		return LibRNG.getVRFV2SubscriptionId();
	}
	
	function setChainlinkVRFKeyhash(bytes32 _keyhash) external {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
		LibDiamond.enforceIsContractOwner();
		ds.rng_chainlinkVRFKeyhash = _keyhash;
    }

    function getChainlinkVRFKeyhash() external view returns (bytes32) {
		return LibDiamond.diamondStorage().rng_chainlinkVRFKeyhash;
    }

    function setChainlinkVRFFee(uint256 _fee) external {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
		LibDiamond.enforceIsContractOwner();
		ds.rng_chainlinkVRFFee = _fee;
    }

    function getChainlinkVRFFee() external view returns (uint256) {
        return LibDiamond.diamondStorage().rng_chainlinkVRFFee;
    }

	function setChainlinkVRFCoordinator(address coordinator) external {
		LibDiamond.enforceIsContractOwner();
		LibDiamond.diamondStorage().vrfCoordinator = coordinator;
    }

	function getChainlinkVRFCoordinator() external view returns(address) {
		return LibDiamond.diamondStorage().vrfCoordinator;
    }

	//OLD
	function rawFulfillRandomness(bytes32 vrfRequestId, uint256 randomness) external {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        require(msg.sender == ds.vrfCoordinator, "RNGFacet: Only VRFCoordinator can fulfill");
		ds.rng_randomness[vrfRequestId] = randomness;
		uint256 mechanicId = ds.rng_mechanicIdByVRFRequestId[vrfRequestId];
		if(mechanicId == LibRNG.RNG_BREEDING) {
			LibBreeding.breedingFulfillRandomness(vrfRequestId);
		} else if(mechanicId == LibRNG.RNG_HATCHING) {
			LibHatching.hatchingFulfillRandomness(vrfRequestId, randomness);
		} else if(mechanicId == LibRNG.RNG_EVOLUTION) {
			LibEvolution.evolutionFulfillRandomness(vrfRequestId, randomness);
		}

		delete ds.rng_mechanicIdByVRFRequestId[vrfRequestId];
    }

	function rawFulfillRandomWords(uint256 requestId, uint256[] memory randomWords) external {
		LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
		require(msg.sender == LibRNG.getVRFV2VrfCoordinatorAddress(), "RNGFacet: Only VRFCoordinator can fulfill");
		uint256 randomWord = randomWords[0];
		ds.rng_randomness[bytes32(requestId)] = randomWord;
		uint256 mechanicId = ds.rng_mechanicIdByVRFRequestId[bytes32(requestId)];
		if(mechanicId == LibRNG.RNG_BREEDING) {
			LibBreeding.breedingFulfillRandomness(bytes32(requestId));
		} else if(mechanicId == LibRNG.RNG_HATCHING) {
			LibHatching.hatchingFulfillRandomness(bytes32(requestId), randomWord);
		} else if(mechanicId == LibRNG.RNG_EVOLUTION) {
			LibEvolution.evolutionFulfillRandomness(bytes32(requestId), randomWord);
		}

		delete ds.rng_mechanicIdByVRFRequestId[bytes32(requestId)];
  	}

	function setVRFBlocksToRespond(uint256 vrfBlocksToRespond) external {
		LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
		LibDiamond.enforceIsContractOwner();
		ds.vrfBlocksToRespond = vrfBlocksToRespond;
	}

	function getVRFBlocksToRespond() public view returns(uint256) {
		return LibDiamond.diamondStorage().vrfBlocksToRespond;
	}
}
...
library LibEvolution {
    event EvolutionRNGRequested(uint256 indexed roundTripId, bytes32 indexed vrfRequestId, address indexed playerWallet);
    event EvolutionRNGRequestedV2(uint256 indexed roundTripId, bytes32 indexed vrfRequestId, address indexed owner, address playerWallet);
    ...
    bytes32 private constant EVOLUTION_STORAGE_POSITION = keccak256("diamond.libEvolution.storage");
    ...
    struct LibEvolutionStorage {
        ...
        mapping(uint256 => bytes32) vrfRequestIdByRoundTripId;
        mapping(bytes32 => uint256) tokenIdByVRFRequestId;
        mapping(bytes32 => uint256) upgradeBoosterByVRFRequestId;
        mapping(bytes32 => uint256) rngByVRFRequestId;
        mapping(bytes32 => uint256) rngBlockNumberByVRFRequestId;
        mapping(uint256 => uint256) roundTripIdByTokenId;
    }

    function beginEvolution(uint256 roundTripId, uint256 tokenId, uint256 ttlBlock, uint256 upgradeBooster) internal {
        validateBeginEvolution(tokenId, ttlBlock);
        uint256 vrfRequestId = LibRNG.requestRandomWordsFor(LibRNG.RNG_EVOLUTION);
        completeBeginEvolution(roundTripId, tokenId, ttlBlock, upgradeBooster, bytes32(vrfRequestId));
    }

    function completeBeginEvolution(uint256 roundTripId, uint256 tokenId, uint256 ttlBlock, uint256 upgradeBooster, bytes32 vrfRequestId) private {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        saveDataOnEvolutionStruct(vrfRequestId, roundTripId, ttlBlock, tokenId, upgradeBooster);
        setIdempotenceForEvolutionStarted(tokenId);
        emit EvolutionRNGRequested(roundTripId, bytes32(vrfRequestId), ds.erc721_owners[tokenId]);
        emit EvolutionRNGRequestedV2(roundTripId, bytes32(vrfRequestId), ds.erc721_owners[tokenId], msg.sender);
    }

    function validateBeginEvolution(uint256 tokenId, uint256 ttlBlock) private view {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();

        LibPermissions.enforceCallerOwnsNFTOrHasPermission(tokenId, IPermissionProvider.Permission.UNICORN_EVOLVING_ALLOWED);
        uint256 dna = LibUnicornDNA._getDNA(tokenId);
        require(dna > 0, "Evolution: Cannot evolve a unicorn that doesnt exist or with invalid DNA");
        require(ttlBlock >= block.number + ds.vrfBlocksToRespond, "Evolution: TTL has expired.");
        LibUnicornDNA.enforceDNAVersionMatch(dna);

        require(LibUnicornDNA._getLifecycleStage(dna) == LibUnicornDNA.LIFECYCLE_BABY, "Evolution: Unicorn must be baby to evolve");
        require(!LibIdempotence._getHatchingRandomnessFulfilled(tokenId), "Evolution: Cannot begin evolution if hatching hasnt been completed");
        require(!LibIdempotence._getEvolutionStarted(tokenId) && !LibIdempotence._getEvolutionRandomnessFulfilled(tokenId), "Evolution: Baby cannot be evolving already to begin evolution");
        require( LibUnicornDNA._getGameLocked(dna) == true, "Evolution: Unicorn must be locked to start evolving");
    }

    function evolutionStorage() internal pure returns (LibEvolutionStorage storage les) {
    ...
    function getUpgradeBooster(bytes32 vrfRequestId) internal view returns (uint256) {
    ...
    function setRandomness(bytes32 vrfRequestId, uint256 randomness) internal {
        ...
        les.rngBlockNumberByVRFRequestId[vrfRequestId] = block.number;
    }
    ...
    function setIdempotenceForEvolutionStarted(uint256 tokenId) internal {
        uint256 babyIdempotence = LibIdempotence._setEvolutionStarted(tokenId, true);
        LibIdempotence._setIdempotenceState(tokenId, babyIdempotence);
    }

    function setIdempotenceForRandomnessFulfilled(uint256 tokenId) internal {
        uint256 idmp = LibIdempotence._setEvolutionStarted(tokenId, false);
        LibIdempotence._setIdempotenceState(tokenId, idmp);
        idmp = LibIdempotence._setEvolutionRandomnessFulfilled(tokenId, true);
        LibIdempotence._setIdempotenceState(tokenId, idmp);
    }

    function setIdempotenceForEvolutionFinished(uint256 tokenId) internal {
        uint256 idmp = LibIdempotence._setEvolutionRandomnessFulfilled(tokenId, false);
        LibIdempotence._setIdempotenceState(tokenId, idmp);
    }

    function evolutionFulfillRandomness(bytes32 vrfRequestId, uint256 randomness) internal {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        LibDiamond.enforceBlockDeadlineIsValid(getBlockDeadline(vrfRequestId));
        uint256 tokenId = getTokenId(vrfRequestId);
        require(LibIdempotence._getEvolutionStarted(tokenId), "Evolution: Evolution has to be in STARTED state to fulfillRandomness");

        ds.rngNonce = ds.rng_randomness[vrfRequestId];
        ...
        emit EvolutionReadyForTokenURI(getRoundTripId(vrfRequestId), ds.erc721_owners[tokenId]);
        emit EvolutionReadyForTokenURIV2(getRoundTripId(vrfRequestId), ds.erc721_owners[tokenId], msg.sender);
    }
    ...
    function setDNAForAdult(uint256 tokenId, uint256 upgradeBooster) internal {
        ...
        dna = LibUnicornDNA._setLifecycleStage(dna, LibUnicornDNA.LIFECYCLE_ADULT);
        dna = LibUnicornDNA._setBreedingPoints(dna, LibUnicornDNA.DEFAULT_BREEDING_POINTS);
        LibUnicornDNA._setDNA(tokenId, dna);
    }

    function attemptUpgradeOfEveryGene(uint256 tokenId, uint256 upgradeBooster) internal returns(uint256 dna) {
        uint256 randomness = evolutionStorage().rngByVRFRequestId[getVRFRequestId(getRoundTripIdForToken(tokenId))];
        dna = LibUnicornDNA._getDNA(tokenId);
        uint256 newGene;

        // major genes
        newGene = attemptGeneUpgrade(LibUnicornDNA._getBodyMajorGene(dna), 1, upgradeBooster, LibRNG.expand(10000, randomness, SALT_11));
        dna = LibUnicornDNA._setBodyMajorGene(dna, newGene);

        newGene = attemptGeneUpgrade(LibUnicornDNA._getFaceMajorGene(dna), 1, upgradeBooster, LibRNG.expand(10000, randomness, SALT_12));
        dna = LibUnicornDNA._setFaceMajorGene(dna, newGene);

        newGene = attemptGeneUpgrade(LibUnicornDNA._getHornMajorGene(dna), 1, upgradeBooster, LibRNG.expand(10000, randomness, SALT_13));
        dna = LibUnicornDNA._setHornMajorGene(dna, newGene);

        newGene = attemptGeneUpgrade(LibUnicornDNA._getHoovesMajorGene(dna), 1, upgradeBooster, LibRNG.expand(10000, randomness, SALT_14));
        dna = LibUnicornDNA._setHoovesMajorGene(dna, newGene);

        newGene = attemptGeneUpgrade(LibUnicornDNA._getManeMajorGene(dna), 1, upgradeBooster, LibRNG.expand(10000, randomness, SALT_15));
        dna = LibUnicornDNA._setManeMajorGene(dna, newGene);

        newGene = attemptGeneUpgrade(LibUnicornDNA._getTailMajorGene(dna), 1, upgradeBooster, LibRNG.expand(10000, randomness, SALT_16));
        dna = LibUnicornDNA._setTailMajorGene(dna, newGene);

        // mid genes
        newGene = attemptGeneUpgrade(LibUnicornDNA._getBodyMidGene(dna), 2, upgradeBooster, LibRNG.expand(10000, randomness, SALT_21));
        dna = LibUnicornDNA._setBodyMidGene(dna, newGene);

        newGene = attemptGeneUpgrade(LibUnicornDNA._getFaceMidGene(dna), 2, upgradeBooster, LibRNG.expand(10000, randomness, SALT_22));
        dna = LibUnicornDNA._setFaceMidGene(dna, newGene);

        newGene = attemptGeneUpgrade(LibUnicornDNA._getHornMidGene(dna), 2, upgradeBooster, LibRNG.expand(10000, randomness, SALT_23));
        dna = LibUnicornDNA._setHornMidGene(dna, newGene);

        newGene = attemptGeneUpgrade(LibUnicornDNA._getHoovesMidGene(dna), 2, upgradeBooster, LibRNG.expand(10000, randomness, SALT_24));
        dna = LibUnicornDNA._setHoovesMidGene(dna, newGene);

        newGene = attemptGeneUpgrade(LibUnicornDNA._getManeMidGene(dna), 2, upgradeBooster, LibRNG.expand(10000, randomness, SALT_25));
        dna = LibUnicornDNA._setManeMidGene(dna, newGene);

        newGene = attemptGeneUpgrade(LibUnicornDNA._getTailMidGene(dna), 2, upgradeBooster, LibRNG.expand(10000, randomness, SALT_26));
        dna = LibUnicornDNA._setTailMidGene(dna, newGene);

        // minor genes
        newGene = attemptGeneUpgrade(LibUnicornDNA._getBodyMinorGene(dna), 3, upgradeBooster, LibRNG.expand(10000, randomness, SALT_31));
        dna = LibUnicornDNA._setBodyMinorGene(dna, newGene);

        newGene = attemptGeneUpgrade(LibUnicornDNA._getFaceMinorGene(dna), 3, upgradeBooster, LibRNG.expand(10000, randomness, SALT_32));
        dna = LibUnicornDNA._setFaceMinorGene(dna, newGene);

        newGene = attemptGeneUpgrade(LibUnicornDNA._getHornMinorGene(dna), 3, upgradeBooster, LibRNG.expand(10000, randomness, SALT_33));
        dna = LibUnicornDNA._setHornMinorGene(dna, newGene);

        newGene = attemptGeneUpgrade(LibUnicornDNA._getHoovesMinorGene(dna), 3, upgradeBooster, LibRNG.expand(10000, randomness, SALT_34));
        dna = LibUnicornDNA._setHoovesMinorGene(dna, newGene);

        newGene = attemptGeneUpgrade(LibUnicornDNA._getManeMinorGene(dna), 3, upgradeBooster, LibRNG.expand(10000, randomness, SALT_35));
        dna = LibUnicornDNA._setManeMinorGene(dna, newGene);

        newGene = attemptGeneUpgrade(LibUnicornDNA._getTailMinorGene(dna), 3, upgradeBooster, LibRNG.expand(10000, randomness, SALT_36));
        dna = LibUnicornDNA._setTailMinorGene(dna, newGene);

        return dna;
    }

    function attemptGeneUpgrade(uint256 geneId, uint256 geneDominance, uint256 upgradeBooster, uint256 rand) private returns (uint256) {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        if(ds.geneTierById[geneId] >= 6) return geneId;

        // geneUpgradeChances are percentages, while upgradeBooster is on a 10000 = 100% scale.
        uint256 baseUpgradeChance = (ds.geneUpgradeChances[ds.geneTierById[geneId]][geneDominance]) * 100;

        if(baseUpgradeChance == 0 || (rand > baseUpgradeChance + upgradeBooster)) {
            return geneId;
        }

        return ds.geneTierUpgradeById[geneId];
    }
    ...
    function retryEvolution(uint256 roundTripId) internal {
        ...
        require(block.number > getBlockDeadline(requestId), "Evolution: Cannot retry evolution if TTL has not expired yet");

        ds.rngNonce = LibRNG.getRuntimeRNG(LibUnicornDNA.MAX);
        ...
        emit EvolutionReadyForTokenURI(roundTripId, ds.erc721_owners[tokenId]);
        emit EvolutionReadyForTokenURIV2(roundTripId, ds.erc721_owners[tokenId], msg.sender);
    }

    function finishEvolution(uint256 roundTripId, string memory tokenURI, bytes32 vrfRequestId, uint256 tokenId) internal {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        ...
        emit EvolutionComplete(roundTripId, ds.erc721_owners[tokenId]);
        emit EvolutionCompleteV2(roundTripId, ds.erc721_owners[tokenId], msg.sender);
    }
}
