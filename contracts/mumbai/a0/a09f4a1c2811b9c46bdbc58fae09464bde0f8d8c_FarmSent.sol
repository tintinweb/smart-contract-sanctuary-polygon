...
contract FarmSent is Crowdsale, FinalizableCrowdsale, UUPSUpgradeable{
    ...
    event DecimalForUSDT(uint256 decimalsForUSDT_);

     uint256 public decimalsForUSDT;
    ...
    function initialize(
      ...
      public initializer
    {      
        _USDT = IERC20Upgradeable(0x4987D9DDe3b2e059dB568fa26D7Eb38F40956013); // USDT Token
    ...
    */
    ...
    function buyToken(uint256 _USDTTokenAmount) external onlyWhileOpen whenNotPaused{
      ...
      uint256 USDTTokenAmount_  = _USDTTokenAmount * 10 ** decimalsForUSDT;

      require(_USDT.balanceOf(msg.sender) >= USDTTokenAmount_, "USDT Token balance limit exceeded");

      uint256 tokenForUSDT_ = _USDTTokenAmount * 10 ** 18;

      require((_purchasedAmount[msg.sender][round] + tokenForUSDT_) <= purchaseLimitInWei, "Maximum purchase Limit exceeded for this round");

      buyTokens(msg.sender, USDTTokenAmount_);

      // calculate token amount to be created
      uint256 token_amount = _getTokenAmount(tokenForUSDT_);
      token.safeTransfer(msg.sender, token_amount);
      _purchasedAmount[msg.sender][round] += tokenForUSDT_;
    }
  ...
  */
    ...
    /* *
     * @dev Owner can update the usdt token decimals
     * @param _decimalsForUSDT
     */

    function updateUSDTDecimals(uint256 _decimalsForUSDT) external {
      decimalsForUSDT = _decimalsForUSDT;
      emit DecimalForUSDT(_decimalsForUSDT);
    }
}
