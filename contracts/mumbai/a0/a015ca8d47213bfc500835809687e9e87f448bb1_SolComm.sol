...
contract SolComm {
    ...
    constructor(address _token) public {
        ...
        map_ranks[2] = Rank({Id:2,Business:1500e6,Income:300e6,Daily:3e6,Period:100}); 
    ...
    function updateRank(address _user) internal
    {
        ...
        if(otherBusiness>=map_ranks[nextRank].Business*(1 ether) && maxBusness>=map_ranks[nextRank].Business*(1 ether) && currentRank<15){
    ...
    function maxPayoutOf(address _user) view external returns(uint256) {
        ...
        return (totaldeposit*2+rank*40)-totalincome;
     }
