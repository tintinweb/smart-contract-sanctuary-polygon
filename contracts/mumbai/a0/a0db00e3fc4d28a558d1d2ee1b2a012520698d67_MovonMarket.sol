...
// File contracts/RouterInterface.sol
...
interface RouterInterface {
    ...
    function movonToken() external view returns (address);

    function movonNFT() external view returns (address);
}
...
// File contracts/MovonMarket.sol
...
contract MovonMarket is Initializable, AccessControlUpgradeable {
    ...
    function initialize(address _router) public initializer {
        ...
        movonToken = IERC20(router.movonToken());
        movonNFT = IMovonNFT(router.movonNFT());
    }
        ...
        preventUpdateInBlock(_tokenId)
    {
        ...
        movonNFT.transferFrom(_msgSender(), address(this), _tokenId);
        ...
        preventUpdateInBlock(_tokenId)
    {
        ...
        movonNFT.transferFrom(address(this), _msgSender(), _tokenId);
    ...
    function fillOrder(uint256 _tokenId) public preventUpdateInBlock(_tokenId) {
        require(tokenSales.contains(_tokenId), "not sale");
        ItemSale storage itemSale = markets[_tokenId];
        require(itemSale.owner != _msgSender(), "you owned this");
        uint256 feeMarket = itemSale.price.mul(router.feeMarketRate()).div(
            router.divPercent()
        );

        movonToken.transferFrom(_msgSender(), router.feeAddress(), feeMarket);
