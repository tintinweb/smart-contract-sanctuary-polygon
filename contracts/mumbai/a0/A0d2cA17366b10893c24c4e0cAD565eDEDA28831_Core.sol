...
contract Core is Ownable, AccessControl, ICoreContract {
    ...
    constructor(address _root, IRegistryContract _registry) {
        registry = _registry;
        _setupRole(META_FORCE_ROLE, _msgSender());
        _setupRole(META_FORCE_ROLE, registry.getMetaForceContract());
    ...
    function burnMFSPool() external onlyOwner {
        if (getWorkflowStage == Workflow.SaleOpen) {
            ...
            _decreaseFrozenMFS(registry.getMetaPool(), users[registry.getMetaPool()].mfsFrozenAmount);
        }
        emit PoolMFSBurned();
    }
    ...
    function getRegistrationDate(address user) external view override returns (uint256) {
        ...
        //TODO after added time in Classic
