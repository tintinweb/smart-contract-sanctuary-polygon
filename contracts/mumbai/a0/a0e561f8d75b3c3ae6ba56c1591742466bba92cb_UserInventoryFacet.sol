...
import {IProjectFacet} from "./Project/IProjectFacet.sol";
import {IUserFacet} from "../interfaces/IUserFacet.sol";

contract UserInventoryFacet is Base {

    function getAllowlists(address user_)
        external
        view
        returns (uint256[] memory projectIds, uint256[] memory balances, uint256[] memory prices)
    {
        if (_usersFacet().isUserFrozen(user_)) {
            revert UserIsFrozen();
        }
        UserInventoryStorage storage userInventory = s.userInventory[user_];
        projectIds = userInventory.projectIds;
        balances = new uint256[](projectIds.length);
        prices = new uint256[](projectIds.length);
        for (uint256 i = 0; i < projectIds.length; i++) {
            balances[i] = userInventory.allowlistsBalance[projectIds[i]];
            prices[i] = userInventory.allowlistsPrice[projectIds[i]];
        }
    }

    function getRaffles(address user_) 
        external 
        view 
        returns (uint256[] memory raffleIds, uint256[] memory balances, uint256[] memory prices) 
    {
         if (_usersFacet().isUserFrozen(user_)) {
            revert UserIsFrozen();
        }
        UserInventoryStorage storage userInventory = s.userInventory[user_];
        raffleIds = userInventory.raffleIds;
        balances = new uint256[](raffleIds.length);
        prices = new uint256[](raffleIds.length);
        for (uint256 i = 0; i < raffleIds.length; i++) {
            balances[i] = userInventory.raffleBalance[raffleIds[i]];
            prices[i] = userInventory.rafflePrice[raffleIds[i]];
        }
    }

    function _usersFacet() private view returns (IUserFacet usersFacet) {
        usersFacet = IUserFacet(address(this));
    }
}
