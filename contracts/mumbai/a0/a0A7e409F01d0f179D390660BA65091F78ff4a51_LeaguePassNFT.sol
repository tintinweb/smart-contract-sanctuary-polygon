...
contract LeaguePassNFT is ERC721Upgradeable, OwnableUpgradeable {
    ...
    event NewPassCreated(uint256 indexed _tokenId, address indexed _player, uint256 _price, address indexed _token);
    event NewPlayerJoinedTheLeague(address indexed _player, uint256 indexed _nftPass);
    event PaymentTokenIsSet(address indexed _token, uint256 _NFTprice);
    event DleagueTokenIsSet(address indexed _addr);
    event DleagueTokenAmountIsSet(uint256 _amount);
    event WhitelistChanged(address indexed _addr, bool indexed _val);
