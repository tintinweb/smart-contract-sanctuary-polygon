 ...
 */
        ...
        onlyRole(SWAP_ROLE)
    {
        ...
        uint256 bYSLPrice;
        if (byslAmount > (IERC20(Admin.bYSL()).balanceOf(user) / 100)) {
            bYSLPrice = IBYSL(Admin.bYSL()).backedPrice();
            uint256 BUSDForSwap = (bYSLPrice * byslAmount) / 10 **18;
            IBYSL(Admin.bYSL()).burn(address(this), byslAmount);
            withdrawBUSD(BUSDForSwap, msg.sender);
        } else {
            bYSLPrice = IBYSL(Admin.bYSL()).protocolPrice();
            uint256 slipageRate = (((bYSLPrice * byslAmount) /
                (IERC20(Admin.BUSD()).balanceOf(Admin.Treasury()) +
                    IERC20(Admin.BUSD()).balanceOf(Admin.POL()))) * 100000); //1000 is cofficient for slipage %
            uint256 deductSlipage = (100 - (slipageRate / 1000)) * 100; //100 is cofficient
            uint256 BUSDForSwap = (((bYSLPrice * byslAmount) *
                (deductSlipage / 100)) / 100);
            if (
                IBYSL(Admin.bYSL()).backedPrice() < (BUSDForSwap / byslAmount)
            ) {
                IBYSL(Admin.bYSL()).burn(address(this), byslAmount);
                withdrawBUSD(BUSDForSwap, msg.sender);
            } else {
                require(false, "effective price is less than backed price");
            }
        }
    }
...
/**
     @dev Setter functoin for BYSlExitRate
     @param _byslExitRate Exit rate for bysl as uint
*/
    function setByslExitRate(uint256 _byslExitRate)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        bYSLExitRate = _byslExitRate;
    }
/**
     ...
     */
    function UnPaused() public onlyRole(DEFAULT_ADMIN_ROLE) {
        _unpause();
    }
/**
...
*/
    function withdrawBUSD(uint256 amount, address user) internal {
        ...
        IERC20(Admin.BUSD()).transfer(user, amount * 9/10);
        ...
        if(leftAmount != 0){
            IERC20(Admin.BUSD()).transfer(Admin.Treasury(),leftAmount);
        }
