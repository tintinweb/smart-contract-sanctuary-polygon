...
error PagePrinter_FunctionLocked();
error PagePrinter_IncorrectOwner();
error PagePrinter_IncorrectValue();
error PagePrinter_InvalidValue();
error PagePrinter_InvalidMintState();
error PagePrinter_MintNotClosed();
error PagePrinter_SenderNotAllowed();
error PagePrinter_TokenAlreadyExists();
error PagePrinter_WithdrawFailed();

contract PagePrinter is ONFT721Core, ERC721Enumerable, IPagePrinter {
    INFT public immutable NFT;
    IPage public immutable PAGE;
    uint256 public immutable NFT_ID;
    uint256 public immutable BASE_PRICE;
    uint256 public immutable PREMIUM_PRICE;
    uint256 public immutable ROOT_CHAIN_ID;

    string public baseURI;
    uint256 public printerId;
    mapping(uint256 => uint) public pagesPrinted;
    mapping(bytes4 => bool) public functionLocked;

    uint16 public constant FUNCTION_TYPE_PRINT = uint16(uint(keccak256("PRINT")));
     ...
     */
    modifier lockable() {
        if (functionLocked[msg.sig]) revert PagePrinter_FunctionLocked();
        _;
    }

    /**
     * @notice Modifier requiring page minting to be closed
     */
    modifier onlyAfterMintClosed() {
        if (!PAGE.mintClosed()) revert PagePrinter_InvalidMintState();
        _;
    }

    /**
     * @notice Modifier validating the print transaction
     * @param _printerId Printer to print the next pages in the series from
     * @param _numOfPages Amount of the next pages in the series to print
     */
    modifier validatePrint(uint256 _printerId, uint256 _numOfPages) {
        if (msg.sender != ownerOf(_printerId)) revert PagePrinter_SenderNotAllowed();
        if (
            pagesPrinted[_printerId] + _numOfPages <= PAGE.currentMaxPage()
            || _numOfPages == 0
        ) revert PagePrinter_InvalidValue();
        _;
    }
    ...
    /**
     * @notice Returns the token IDs owned by a specified address
     * @param _owner Address to return the owned tokens for
     */
    ...
    /**
     * @notice Set the base metadata URI for the contract
     * @param baseURI_ Base metadata URI for the contract
     */
    function setBaseURI(string calldata baseURI_) public payable lockable onlyOwner {
        baseURI = baseURI_;
    }

    /**
     * @notice Mint a specified number of page printers
     * @param _amount Amount of page printers to mint
     */
    function mint(uint256 _amount) public payable {
        if (PAGE.mintClosed()) revert PagePrinter_InvalidMintState();

        bool mintNft = msg.value == _amount * PREMIUM_PRICE
            && block.chainid == ROOT_CHAIN_ID;

        if (!mintNft && msg.value != _amount * BASE_PRICE)
            revert PagePrinter_IncorrectValue();

        for (uint256 i; i < _amount;) {
            _safeMint(msg.sender, printerId);
            unchecked {
                ++printerId;
                ++i;
            }
        ...
        if (mintNft) NFT.mint(msg.sender, NFT_ID, _amount);
    }

    /**
     * @notice Internal function to print a specified amount of the next pages in the series
     * @param _receiver Address to receive the printed pages
     * @param _startPage Starting page number to print from
     * @param _numOfPages Amount of the next pages in the series to print
     */
    function _print(address _receiver, uint256 _startPage, uint256 _numOfPages) internal {
        for (uint256 i; i < _numOfPages;) {
            PAGE.print(_receiver, _startPage + i, 1);
            unchecked { ++i; }
        }
    }

    /**
     * @notice Print a specified amount of the next pages in the series
     * @param _printerId Printer to print the next pages in the series from
     * @param _numOfPages Amount of the next pages in the series to print
     */
    function print(
        ...
        public
        payable
        validatePrint(_printerId, _numOfPages)
    {
        uint256 nextPage = pagesPrinted[_printerId];
        pagesPrinted[_printerId] += _numOfPages;

        _print(msg.sender, nextPage, _numOfPages);
    }

    /**
     * @notice Print a specified amount of the next pages in the series to a destination chain
        ...
        validatePrint(_printerId, _numOfPages)
    {
        ...
        bytes memory payload = abi.encode(FUNCTION_TYPE_PRINT, _toAddress, [nextPage], [_numOfPages]);

        _checkGasLimit(_dstChainId, FUNCTION_TYPE_PRINT, _adapterParams, dstChainIdToTransferGas[_dstChainId]);
        _lzSend(_dstChainId, payload, _refundAddress, _zroPaymentAddress, _adapterParams, msg.value);
    }
    ...
    function withdraw(address _receiver) public onlyOwner {
        ...
        if (!success) revert PagePrinter_WithdrawFailed();
    }
        ...
        returns (uint256 nativeFee, uint256 zroFee)
    {
        return estimateFee(
            ...
            _toSingletonArray(pagesPrinted[_tokenId]),
            _useZro,
            _adapterParams
        );
    }
    ...
    function estimateSendBatchFee(
        ...
        returns (uint256 nativeFee, uint256 zroFee)
    {
        uint256[] memory _pagesPrinted = new uint256[](_tokenIds.length);
        for (uint256 i; i < _tokenIds.length;) {
            _pagesPrinted[i] = pagesPrinted[_tokenIds[i]];
            unchecked { ++i; }
        }
     ...
     * @param _pagesPrinted Pages printed by each token
     * @param _useZro Flag indicating whether to use $ZRO for payment
     * @param _adapterParams Parameters for custom functionality
     */
    function estimateFee(
        ...
        bytes memory _toAddress,
        uint256[] memory _tokenIds,
        uint256[] memory _pagesPrinted,
        bool _useZro,
        bytes memory _adapterParams
    ) public view returns (uint256 nativeFee, uint256 zroFee) {
        bytes memory payload = abi.encode(_functionType, _toAddress, _tokenIds, _pagesPrinted);
        return lzEndpoint.estimateFees(_dstChainId, address(this), payload, _useZro, _adapterParams);
    }

    /**
     * @notice Override `ONFT721Core` function to debit a token from the current chain
     * @dev For more details see https://layerzero.gitbook.io/docs/evm-guides/master
     * @param _from Address to debit the token from
     * @param _tokenId Token to debit
     */
    function _debitFrom(
        address _from,
        uint16, // _dstChainId
        ...
        override
    {
        if (!_isApprovedOrOwner(msg.sender, _tokenId)) revert PagePrinter_SenderNotAllowed();
        if (ERC721.ownerOf(_tokenId) != _from) revert PagePrinter_IncorrectOwner();

        _transfer(_from, address(this), _tokenId);
    }

    /**
     * @notice Override `ONFT721Core` function to credit a token on the current chain
     * @dev For more details see https://layerzero.gitbook.io/docs/evm-guides/master
     * @param _toAddress Address to credit the token to
     * @param _tokenId Token to credit
     */
    function _creditTo(
        uint16, // _srcChainId
        address _toAddress,
        ...
        override
    {
        if (_exists(_tokenId) && ERC721.ownerOf(_tokenId) != address(this)) revert PagePrinter_TokenAlreadyExists();
    ...
    function _send(
        ...
        override
    {
        if (!PAGE.mintClosed() && msg.sender != address(bytes20((_toAddress))))
            revert PagePrinter_InvalidMintState();
        ...
        bytes memory payload = abi.encode(FUNCTION_TYPE_SEND, _toAddress, _tokenIds, _pagesPrinted);
    ...
    /**
     * @notice Override `ONFT721Core` function that processes a payload from a source chain
     * @dev For more details see https://layerzero.gitbook.io/docs/evm-guides/master
        ...
        override
    {
        (
            ...
            uint256[] memory pagesPrintedArr
        ...
        if (functionType == FUNCTION_TYPE_PRINT) {
            // @dev To reduce contract size this non-intuitive pattern is used
            // tokenIds[0] is an alias for _startPage
            // pagesPrintedArr[0] is an alias for _numOfPages
            _print(toAddress, tokenIds[0], pagesPrintedArr[0]);
        } else {
            for (uint256 i; i < tokenIds.length;) {
                pagesPrinted[tokenIds[i]] = pagesPrintedArr[i];
                unchecked { ++i; }
            }
    ...
    /**
     * @dev Printers are soulbound until after the mint is closed
     * @inheritdoc ERC721
     */
