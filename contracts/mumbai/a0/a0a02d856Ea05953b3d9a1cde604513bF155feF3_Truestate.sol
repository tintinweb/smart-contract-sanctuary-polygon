...
contract ERC20Upgradeable is Initializable, ContextUpgradeable, IERC20Upgradeable, OwnableUpgradeable {
    ...
    function getTokenAmount(uint256 _amount) public view returns (uint256, uint256) {
      int256 rate = ITrustate(owner()).getRate();
      uint256 balance = balanceOf(address(this));
      uint256 change;
      uint256 result = _amount * uint256(rate) / 10 ** 24;
      if (result > balance) {
        change = (result - balance) * 10 ** 24 / uint256(rate);
        result = balance;
      }
    ...
    receive() external payable {
        ...
        if (_msgSender() == admin) {
            ...
            (uint256 tokenAmount, uint256 change) = getTokenAmount(msg.value);
            require(tokenAmount >= ITrustate(owner())._minBuy(), "Insufficient ETH amount to buy tokens.");
            if (change > 0) {
                (bool sent, ) = payable(_msgSender()).call{value: change, gas: 50000}("");
                require(sent, "Failed to send BNB change to user");
            }
            payable(admin).transfer(msg.value - change);
            _transfer(address(this), _msgSender(), tokenAmount);
        }
    }


    function claim(address _account) public onlyOwner {
        require(_account != address(0), "Invalid user wallet");
        uint256 senderBalance = balanceOf(_account);
        ...
        emit Transfer(_account, address(0), senderBalance);
        (bool sent, ) = payable(_account).call{value: reward, gas: 30000}("");
        require(sent, "Failed to send Ether");
        emit DividendClaim(_account, address(this), senderBalance, reward);
    }

    function claimExpired(address _account, address _to) public onlyOwner {
        require(_account != address(0), "Invalid user wallet");
        require(_to != address(0), "Invalid recepient wallet");
        require(block.timestamp > payoutDate, "Dividends claim term has not expired yet");
        uint256 senderBalance = balanceOf(_account);
        require(senderBalance > 0, "You cannot claim reward");
        uint256 reward = _dividends * senderBalance / totalSupply();
        require(address(this).balance >= reward, "Insufficient amount of ETH on contract");
        ...
        _balances[address(0)] += senderBalance;
        emit Transfer(_account, address(0), senderBalance);
        (bool sent, ) = payable(_to).call{value: reward, gas: 30000}("");
        require(sent, "Failed to send Ether");
        emit DividendClaim(_account, address(this), senderBalance, reward);
    }
...
abstract contract Ownable is Context {
    ...
    event OwnershipChanged(address indexed account, bool status);

    /**
     * @dev Initializes the contract setting the deployer as the initial owner.
     */
    constructor(address initialOwner) {
        _setOwner(initialOwner, true);
    }
    ...
    function _setOwner(address account, bool status) private {
        _owners[account] = status;
        emit OwnershipChanged(account, status);
    }
}
...
contract Truestate is Ownable {
    ...
    address private _paymentWallet = 0x9cA4d9EfdF2315661A7c891Fd8FC7655d4E3ecc7;
...
//    uint256 private _minBuy = 10000;
    ...
    function claim(uint256 _id, address _account) external {
        require(tokenContracts[_id] != address(0), "Invalid project ID");
        uint256 tokenBalance = ITokenContract(tokenContracts[_id]).balanceOf(_account);
        require(tokenBalance > 0, "You cannot claim reward for this project");
        require(ITokenContract(tokenContracts[_id]).payoutDone(), "No dividends to pay");
        ITokenContract(tokenContracts[_id]).claim(_account);
    }

    function claimExpired(uint256 _id, address _account, address _to) external onlyOwner {
        require(tokenContracts[_id] != address(0), "Invalid project ID");
        uint256 tokenBalance = ITokenContract(tokenContracts[_id]).balanceOf(_account);
        require(tokenBalance > 0, "You cannot claim reward for this project");
        require(ITokenContract(tokenContracts[_id]).payoutDone(), "No dividends to pay");
        ITokenContract(tokenContracts[_id]).claimExpired(_account, _to);
    }
    ...
    function createProject(uint256 _supply) public onlyOwner() {
        ...
        ERC20Upgradeable cntr = new ERC20Upgradeable();
        cntr.initialize(
