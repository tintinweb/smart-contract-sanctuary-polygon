...
contract SBT is ERC721, ERC721URIStorage, Ownable, EIP712{
    ...
    bool public mustBeWhitelisted = false;
    string private constant SIGNING_DOMAIN = "LOMADS-SBT";
    string private constant SIGNATURE_VERSION = "1";
    address private signerAddress = 0x5Af6E42E155137907f1dEcbC957730Da2Dbf9741;
    ...
    function _check(uint256 id, bytes memory signature) public view returns (address) {
    ...
    function _hash(uint256 id) internal view returns (bytes32) {
        return _hashTypedDataV4(
            keccak256(abi.encode(
                keccak256("Web3Struct(uint256 id)"),
                id
            ))
        );
    }

    function safeMint(string memory uri, uint256 id, bytes memory signature) public payable{
        require(balanceOf(msg.sender) == 0,"Only 1 Token per wallet is allowed !");
        if (mustBeWhitelisted) {
            require(MINTED == id, "Signature already used");
            require(_check(id, signature) == signerAddress, "Signature validation failed");
        }
        ...
        if (mintToken == address(0)){
            ...
            require(IERC20(mintToken).balanceOf(msg.sender) >= mintPrice, "Not enough funds");
            (bool success) = IERC20(mintToken).transferFrom(msg.sender, address(this), mintPrice);
            require(success);
        }
    ...
    return IERC20(mintToken).balanceOf(address(this));
    }
    ...
    function withdraw() external onlyOwner {
        if (mintToken == address(0)){
            ...
            require(IERC20(mintToken).balanceOf(address(this))> 0, "Nothing to withdraw !");
            IERC20(mintToken).transfer(msg.sender, IERC20(mintToken).balanceOf(address(this)));
        }   
    }
    ...
    function setMintPrice(uint256 _priceInWei) external onlyOwner {
        require(_priceInWei > 0, "_price should be > than 0 !");
...
contract SBTDeployer {
    
    address[] private contracts;
    uint256 public counter;

    function deployNewSBT(
        string memory name, 
        ...
        SBT sbtContract = new SBT(name, symbol,msg.sender,_mintPriceInWei,mintToken_,_treasury,_mustBeWhitelisted);
