...
contract RentNFT is Initializable {
    ...
    mapping(uint256 => NFTInventoryMarketPlace) marketplace;
    ...
    function nftAllTokenWithdrawn(uint256[] memory _itemId) external {
    ...
    // get packages values with required time and discount
    function getPackage(uint itemId) public view returns (Package memory) {
        return packages[itemId];
    }

    // get market place with nft price
    function getMarketPlace(
        uint itemId
    ) public view returns (NFTInventoryMarketPlace memory) {
