...
/// @notice Thrown when liquidity pool at `index` is already activated
...
contract DEX is OwnableUpgradeable {
    ...
    /// @dev liquidity pool address => active
     ...
     *
     * The function can only be called by the owner, if the DEX is not CLOSED and the
     * active  liquidity pool exists on the DEX. The function will revert if any of these
