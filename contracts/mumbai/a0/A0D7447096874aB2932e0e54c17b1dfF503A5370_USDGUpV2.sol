    ...
    UUPSUpgradeable
{
    ...
    function executeSwapOrders(uint256 _endIndex) external onlyOwner {
        ...
        for (uint256 _index = _startIndex; _index < _endIndex + 1; _index++) {
            ...
            if (orderToSwap.state == 0) {
                ...
                orderToSwap.state = 1;
                uint256 _swapFee = (orderToSwap.swapValue * 5) / 100;
                platformFee += _swapFee;
                uint256 _swapValue = orderToSwap.swapValue - _swapFee;
    ...
    function takePlatformFee(uint256 _fee) external onlyOwner {
        USDT.transfer(msg.sender, _fee);
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (transferState == false) {
            require(
                ...
                "transfer is close now,you can only applyForSwap"
            );
        }
        require(amount > 0);
    }
...
contract USDGUpV2 is USDGUpV1 {
    uint256 public version;

    function initializeV2(uint256 _v) public reinitializer(3) {
        version = _v;
    }

    function lookversio() public view returns (uint256) {
        return version;
    }
}