...
contract MyNFTCollection is ERC721, Ownable, ReentrancyGuard {
  using Strings for uint256;
  ...
  uint256 public WHITELIST_PRICE = 0.08 ether;

  uint256 public WHITELIST_LIMIT = 500;

  mapping(address => uint) public _walletMaxMints;
  ...
  function mint(address _to, uint256 _mintAmount) public payable nonReentrant {
    ...
    require(_walletMaxMints[_to] + _mintAmount <= MAX_PER_WALLET, "Exceeds max per wallet");
    if (msg.sender != owner()) {
      if (!whitelistPaused) 
      {
    ...
    if (msg.sender != owner()) {
      _walletMaxMints[_to] += _mintAmount;
