...
contract PartnersOW {
    ...
    for (uint256 i = 0; i < _totalSupply(); i++) {
        ...
        if (investment > 0) {
            ...
            // Deduct the distributed funds from the contract's balance
            _investments[investor] -= totalPayout;
        }
    }

    // Transfer the distributed funds and profits to the investors
    for (uint256 i = 0; i < _totalSupply(); i++) {
        uint256 tokenId = _nftContract.tokenByIndex(i);
        address investor = _nftContract.ownerOf(tokenId);

        uint256 payout = _investments[investor];
        if (payout > 0) {
