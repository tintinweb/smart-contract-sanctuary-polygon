...
contract TraceFaucetAdmin is Pausable, Escapable {
    ...
    function setWhitelistedTokens(
        address[] memory newWhitelistedTokens,
        bool[] memory status
    )
		external
		onlyOwner
        whenNotPaused
	{
        for (uint256 i = 0; i < newWhitelistedTokens.length; i++) {
            whitelistedTokens[newWhitelistedTokens[i]] = status[i];
        }
	}

    function isWhitelistedToken(address tokenAddress) public view returns (bool) {
        return whitelistedTokens[tokenAddress];
    }
