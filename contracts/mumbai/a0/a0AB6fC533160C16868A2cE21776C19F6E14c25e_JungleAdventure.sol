...
contract JungleAdventure is VRFConsumerBaseV2 {
	...
	function _generateSeed(uint256 _seed) internal view returns(uint256 rand) {
		if (_seed == 0)
			rand = uint256(keccak256(abi.encodePacked(msg.sender, block.difficulty, block.timestamp)));
		else
