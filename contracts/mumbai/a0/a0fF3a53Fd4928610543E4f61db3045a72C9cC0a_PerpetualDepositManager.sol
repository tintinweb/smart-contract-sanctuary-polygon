    ...
    IPerpetualDepositManager
{
     ...
     * @param   _fAmount     The amount of collateral in 'collateral currency' to deposit.
     */
    function depositToDefaultFund(uint8 _poolId, int128 _fAmount)
        external
        override
        onlyMaintainer
    {
     ...
     *                    (not ABDK)
     */
    function brokerDepositToDefaultFund(uint8 _poolId, uint32 _iLots)
        external
        override
        whenNotPaused
    {
     ...
     */
    function withdrawFromDefaultFund(uint8 _poolId, int128 _fAmount)
        external
        override
        onlyMaintainer
    {
        ...
        PerpStorage.LiquidityPoolData storage pool = liquidityPools[_poolId];
        require(!pool.isRunning, "running is running");
        ...
        _transferFromVaultToUser(pool.marginTokenAddress, treasuryAddress, _fAmount);
    ...
    function _validateInputData(PerpetualData storage _perpetual, int128 _fAmount) internal view {
        require(_perpetual.state == PerpetualState.NORMAL, "state should be NORMAL");
        require(_fAmount > 0, "invalid amount");
    }
