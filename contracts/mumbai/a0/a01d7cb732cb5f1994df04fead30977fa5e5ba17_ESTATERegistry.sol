    ...
    IERC721ReceiverUpgradeable
{
    ...
    function setUpdateManager(
        ...
        require(_owner == msg.sender || operatorApprovals[_owner][msg.sender], "Unauthorized user");
    ...
    function setOperator(
        ...
        require(_owner == ownerOf(estateId), "Unauthorized user");
        require(updateManager[_owner][_operator], "The operator is not authorized by the owner.");

        // uint256[] storage landIds = estateLandIds[estateId];
        items[_operator].push(estateId);
        console.log("items[_operator] length: %d", items[_operator].length);
    }

    function getOperatorLands(address _operator) public view returns (uint256[] memory) {
        console.log("test %d", items[_operator][0]);
        return items[_operator];
    }
    ...
    function removeOperator(
        ...
        for (uint256 i = 0; i < items[_operator].length; i++) {
        
            if (keccak256(abi.encode(items[_operator][i])) == keccak256(abi.encode(estateId))) {
                delete items[_operator][i];
            } else {
    ...
    function _transferLand(
        ...
        // _updateEstateLandBalance(owner, address(landRegistry), 1);

        //landRegistry.approveToken(address(this), landId);
     ...
     */
    function transferManyLands(uint256 estateId, uint256[] calldata landIds)
        external
        // address destinatary
    ...
    function dissolveEstate(uint256 estateId) external {
        ...
        //safeTransferFrom(ownerOf(estateId), address(this), estateId);
    }
    ...
    OwnableUpgradeable
{
    ...
    function initialize() public initializer {
        __ERC721_init("CDBLand", "LAND");
        __Ownable_init();
        __UUPSUpgradeable_init();
    }

    modifier isCurrentOwner(address from, uint256 tokenId) {
    ...
    /**
     * @dev See {IERC721-ownerOf}. ---- chiedere
     */
    function getTokenOwner(uint256 tokenId) public view virtual returns (address) {
        address owner = ownerOf(tokenId);
        return owner;
    ...
    function getTokenOfOperator(address _operator)
        public
        view
        returns (uint256[] memory operatorTokens)
    {
        require(_operator != address(0), "Address zero is not a valid operator");
        return _operatorsTokenIds[_operator];
    }
    ...
    // ) external {
    //     require(_operator != msg.sender, "The operator should be different from owner");
    //     require(msg.sender == ownerOf(tokenId)|| _isApprovedForAll(_owner, msg.sender), "Unauthorized user");
    ...
    /**
     * @dev Copied from {IERC721Enumerable-totalSupply} with _allTokens related array.
     */
    ...
    function canMint(uint256 _tokenId) public view returns (bool) {
        console.log("canMint for token: %s", _tokenId);
        return
            hasEnoughSupply() &&
    ...
    function setUpdateManager(
        ...
        require(_operator != msg.sender, "The operator should be different from owner");
        require(_owner == msg.sender || _isApprovedForAll(_owner, msg.sender), "Unauthorized user");
    ...
    function mintToken(
        ...
        require(saleIsActive, "Sale is not active for this contract");
        require(canMint(getTokenId(x, y)), "Minting is not allowed, already minted");
        uint256 tokenId = getTokenId(x, y);
        _safeMint(to, tokenId);
        _setTokenURI(tokenId, StringsUpgradeable.toString(tokenId));

        _allTokens.push(tokenId);
    }

    //
    // function mintMultipleTokens -0.524kB
    //
    ...
    function mintMultipleTokens(
        ...
        for (uint256 i = 0; i < x.length; i++) {
            ...
            require(canMint(tokenId), "Minting is not allowed, already minted");
            ...
            console.log("Lands minted with tokenId: %d", tokenId);
        }
    }
    ...
    function getTokenId(uint256 x, uint256 y) public pure isValidCoords(x, y) returns (uint256) {
    ...
    /**
     * @dev Transfer a parcel to another address using the parcel's token id.
     */
    ...
    function landAirDrop(
        ...
        console.log("tokenId %d was sent to the address %s", tokenId, _recipient);
    }

    //
    // function transferManyLand -0.312kB//
    //
    ...
    /**
     * @dev Add a parcel to an Estate using the parcel's coordinates.
     */

    //
    // function transferLandToEstate -1.287kB
    //
    ...
    function transferLandToEstate(
        ...
        require(ownerOf(tokenId) == msg.sender, "You must own the Land you want to transfer to");
        require(
            estateRegistry.getTokenOwner(estateId) == msg.sender,
            "You must own the Estate you want to transfer to"
        );

        console.log("TransferLandToEstate of token: %d to %d", tokenId, estateId);

        safeTransferFrom(msg.sender, address(estateRegistry), tokenId, toBytes(estateId));
        approveToken(msg.sender, tokenId);
        transferFrom(address(estateRegistry), msg.sender, tokenId);
    }
    ...
    function transferManyLandToEstate(
        ...
        require(x.length > 0, "You should supply at least one coordinate");
        require(x.length == y.length, "The coordinates should have the same length");
        require(ownerOf(estateId) == msg.sender, "You must own the Estate you want to transfer to");

        for (uint256 i = 0; i < x.length; i++) {
            uint256 tokenId = getTokenId(x[i], y[i]);
            safeTransferFrom(ownerOf(tokenId), address(this), tokenId);

            safeTransferFrom(address(this), address(estateRegistry), tokenId);
        }
    }
    ...
    function approveToken(address to, uint256 tokenId) public virtual {
        ...
        console.log(
            "approveToken, owner is %s and token approval is %s",
            owner,
            ERC721Upgradeable.getApproved(tokenId)
        );
    }
    ...
    function _createEstate(uint256[] calldata x, uint256[] calldata y) internal returns (uint256) {
        ...
        //require(isCoordsAdjacent(x, y) == true, "Coordinates are not adjacent");
        ...
        console.log("Created estateTokenId: %d", estateTokenId);

        for (uint256 i = 0; i < x.length; i++) {
            uint256 tokenId = getTokenId(x[i], y[i]);

            safeTransferFrom(msg.sender, address(estateRegistry), tokenId);
        }

        emit AddEstate(estateTokenId);

        return estateTokenId;
    }

    function setApproval(address to) public {
...
interface ILANDRegistry {
    // function ownerOf(uint256 tokenId) external returns (address); // from ERC721
    ...
    event AddEstate(uint256 indexed _estateId);
...
contract Storage is LANDStorage {
...
contract LANDStorage {
    ...
    mapping(address => uint256[]) public _operatorsTokenIds;
    ...
    uint256[49] public reservedStorage;
