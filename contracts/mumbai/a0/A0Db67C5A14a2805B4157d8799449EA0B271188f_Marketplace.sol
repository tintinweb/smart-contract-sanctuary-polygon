  ...
  only_owner(NftcontractAddress, tokenID)
{
    ...
    IERC721Extended nft = IERC721Extended(NftcontractAddress);
    if (nft.getApproved(tokenID) != address(this))
      revert Marketplace__Address_notApproved();

    string memory tokenURI = nft.tokenURI(tokenID);
    s_listings[NftcontractAddress][tokenID] = Listing(Price, msg.sender, tokenURI);

    emit Itemlisted(msg.sender, NftcontractAddress, tokenID, Price, tokenURI);
}
