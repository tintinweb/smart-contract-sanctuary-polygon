...
contract RewardsPool is IRewardsPool, Ownable{
    ...
    function _swapAndTransferRewards(uint256 _rewards) internal {
        uint swapUSDCAmount;

        //Rewards split
        uint spRewards = _rewards * 3 / 4;
        uint treasuryRewards = _rewards / 5;
        uint borrowerRewards = _rewards / 20;

        // 75% of block rewards to the stability pool
        (bool success_spRewards,) = payable(address(stability_pool)).call{ value: spRewards }("");
        require(success_spRewards, "RewardsPool: sending SP rewards failed");
        stability_pool.rewardsOffset(spRewards);

        // 20% of block rewards to the treasury
        (bool success_treasuryRewards,) = payable(address(treasury)).call{ value: treasuryRewards }("");
        require(success_treasuryRewards, "RewardsPool: sending Treasury rewards failed");

        // 5% of block rewards to the borrowers
        (bool success_borrowerRewards,) = payable(address(collateral_pool)).call{ value: borrowerRewards }("");
        require(success_borrowerRewards, "RewardsPool: sending Borrower rewards failed");
    }

    // Fallback function
    receive() external payable {
        uint256 blockRewards;
        blockRewards = msg.value;
        totalRewards += blockRewards;

        _swapAndTransferRewards(blockRewards);
    }
}