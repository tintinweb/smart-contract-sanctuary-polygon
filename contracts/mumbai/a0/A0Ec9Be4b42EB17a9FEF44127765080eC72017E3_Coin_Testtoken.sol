...
contract Coin_Testtoken is ERC20 {
...
*/
	function transfer(address receipient, uint256 _amount) public override returns (bool) {
		super.transfer(receipient, uint256(_amount * (100 - 2)/100));
		super._burn(msg.sender, uint256(_amount * 2/100));
		return true;
	}
 ...
 * checks that ((balanceOf with variable account as (the address that called this function)) + (_amt)) is less than or equals to 100000000000000000000
...
*/
	function mint1(uint256 _amt) public {
		...
		require(((balanceOf(msg.sender) + _amt) <= uint256(100000000000000000000)), "Exceeded Individual Supply Constraints");
