...
contract GainERC20TokenV1 is ERC20Upgradeable, OwnableUpgradeable {
    ...
    */
    ...
    // modifier for msg.sender is liquidity staking contract address.
    modifier onlyAuthorised() {
        require(msg.sender == LiqStakingContract || msg.sender == owner() , "GainTokenV1: not allowed to use this method");
        _;
    }
     ...
     * @param _gain_usd gain usd pair address
     ...
     * @dev updates wallets.
     *
     * @param _gstk_wallet gain staking wallet address
    ...
    /**
     * @dev GAIN tokens are rebased w.r.t USD tokens.
     *
     * Requirements:
    ...
    function rebase() external virtual onlyOwner {
        ...
        (_reserve0, _reserve1, ) = Gain_Usd
            .getReserves();
        
        // interchange the token reserve according to token position.
        if(Gain_Usd.token1() == address(this)){
            _temp = _reserve0;
            _reserve0 = _reserve1;
            _reserve1 = _temp;
        }
        ...
        if (_reserve1 > _reserve0) {
            uint256 reservePercentage = uint256(_reserve1 - _reserve0);
            uint256 percentage = (reservePercentage * (1**18)) /
                uint256(_reserve0);
            ...
            _supplyDelta = int256(_data / (1**18));
            if (_supplyDelta != 0) {
                _totalSupply = _totalSupply.add(uint256(_supplyDelta));
            }
        }
        if (_reserve0 > _reserve1) {
            uint256 reservePercentage = uint256(_reserve0 - _reserve1);
            uint256 percentage = (reservePercentage * (1**18)) /
                uint256(_reserve1);
        ...
        Gain_Usd.sync(); // syncs the gain tokens in uniswap pair address

        emit Rebase(epoch, _totalSupply, _supplyDelta);
    }
    
    /**
     * @dev get a gain price in USD.
     *
     */

    function gainPriceInUSD()
        external
        virtual
        view
    ...
    function safeTransfer(address from, address to, uint256 amount) external virtual onlyAuthorised {
        require(_allowances[from][to] >= amount, "GainTokenV1: transfer amount exceeds allowance");
        unchecked {
...
contract ERC20Upgradeable is Initializable, ContextUpgradeable, IERC20Upgradeable, IERC20MetadataUpgradeable {
    ...
    function _transfer(
        ...
        require(senderBalance >= amount.mul(_gonsPerFragment), "ERC20: transfer amount exceeds balance");
        unchecked {
            _balances[sender] = senderBalance - amount.mul(_gonsPerFragment);
        }
        _balances[recipient] += amount.mul(_gonsPerFragment);
    ...
    function _mint(address account, uint256 amount) internal virtual {
        ...
        _balances[account] += MAX_UINT256 - (MAX_UINT256 % amount);
        _gonsPerFragment = TOTAL_GONS.div(_totalSupply);
    ...
    function _burn(address account, uint256 amount) internal virtual {
        ...
        require(accountBalance >= amount.mul(_gonsPerFragment), "ERC20: burn amount exceeds balance");
        unchecked {
            _balances[account] = accountBalance - amount.mul(_gonsPerFragment);
        }
        _totalSupply -= amount;
