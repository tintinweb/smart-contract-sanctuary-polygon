...
import "./interfaces/Artfi-IRoyalty.sol";
  ...
  ERC721Upgradeable
{
  ...
  modifier validatePayouts(
    ...
    if (percentage_.length != receivers_.length) revert GeneralError("AF:201");
   ...
   */
  function whiteListUpdate(address _address, bool _value) public onlyRole(DEFAULT_ADMIN_ROLE){
  ...
  function setBaseURI(string memory baseUri_) external {
    ...
    if (
      keccak256(abi.encodePacked(baseUri_)) == keccak256(abi.encodePacked(""))
    ) revert GeneralError("AF:204");
    _baseTokenURI = baseUri_;
  }
  ...
  function updateTokenURI(uint256 tokenId_, string memory uri_) external {
    ...
    if (keccak256(abi.encodePacked(uri_)) == keccak256(abi.encodePacked("")))
      revert GeneralError("AF:204");
    ...
    )
  {
    ...
    coverImageURI_ = string(abi.encodePacked(_baseTokenURI, _coverImageUri));
  }
   ...
   *@param mintData_ contains uri, creater addresses, royalties percentage, investors addresses, stakeRoyalty percentage,minter address.
  ...
  function mint(
    ...
    external
    // validatePayouts(mintData_.investors, mintData_.stakeRoyalty)
   ...
   *@param tokenId_ tokenId of NFT.
   *@param investors_ address of creaters of nfts.
   *@param stakeRoyalty_ Royaltyfee given to the creators.
   */
  function updateRoyalties(
    uint256 tokenId_,
    address[] calldata investors_,
    uint256[] calldata stakeRoyalty_
  ) external validatePayouts(investors_, stakeRoyalty_) {
    if (
      !hasRole(DEFAULT_ADMIN_ROLE, msg.sender) &&
      _nftInfos[tokenId_].investors[0] != msg.sender
    ) revert GeneralError("AF:107");
    _nftInfos[tokenId_].investors = investors_;
    _nftInfos[tokenId_].stakeRoyalty = stakeRoyalty_;
  }
 ...
 */
contract ArtfiProxy is ERC1967Proxy {
    error GeneralError(string errorCode);
    ...
    //     if (msg.sender == _getAdmin()) revert GeneralError("AF:601");
