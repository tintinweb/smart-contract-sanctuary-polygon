...
contract TreasureHuntRewardPool is Initializable, OwnableUpgradeable {
    ...
    function initialize(address _treasureHuntFleet, address _treasureHuntPicker) public initializer {
        ...
        // tokenUSDC = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;  //mainnet
        tokenUSDC = 0xe11A86849d99F524cAC3E7A0Ec1241828e332C62; //testnet
    ...
    function raidIsland(uint256 tokenID, uint8 islandNumber) public enoughFund(tokenID, islandNumber) onSeaport(msg.sender) {
        ...
        uint256 random = ITreasureHunt(treasureHuntPicker).random();
    ...
    function repairFleet(uint256 tokenID, bool isDirect) public {
        ...
        if (isDirect) {
            IERC20(tokenUSDC).transferFrom(msg.sender, address(this), repairCost[tokenID] * (10 ** decimal));
        } else {
    ...
    function transferBurnReward(address acc, uint256 amt) public onlyReleaser(acc){
        uint256 realAmt = amt * (10 ** decimal);
        IERC20(tokenUSDC).transfer(acc, realAmt);
    }

    function transferCost(uint256 amt, bool isMint) public {
        uint256 realAmt = amt * (10 ** decimal);
        require(IERC20(tokenUSDC).balanceOf(tx.origin) >= realAmt, "Insuffient USDC!");

        IERC20(tokenUSDC).transferFrom(tx.origin, address(this), realAmt);
        if (isMint) {
    ...
    function withdrawReward() public onSeaport(msg.sender) {
        ...
        IERC20(tokenUSDC).transfer(msg.sender, reward[msg.sender]);
    }
    ...
    function releaseReward() public onlyOwner {
        ...
        IERC20(tokenUSDC).transfer(owner(), IERC20(tokenUSDC).balanceOf(address(this)));
    }
    ...
    function releaseFee() public onlyOwner {
        IERC20(tokenUSDC).transfer(devWallet, devAmt * 3 / 4);
        IERC20(tokenUSDC).transfer(devWallet1, devAmt / 4);
    }

    function updateAcc(address acc) public onlyOwner {
        tokenUSDC = acc;
    }
}
