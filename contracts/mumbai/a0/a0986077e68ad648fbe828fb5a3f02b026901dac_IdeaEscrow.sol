...
contract IdeaEscrow {
    ...
    uint256 public ideaBalanceLimit;
    address public ideaTokenAddress;
    ...
    event MinBalanceLimitSet(uint256 ideaBalanceLimit);
    constructor(uint256 _fee, uint256 _ideaBalanceLimit,address _ideaTokenAddress,uint256 _disputeFee) {
        fee = _fee;
        admin = msg.sender;
        ideaBalanceLimit = _ideaBalanceLimit;
        ideaTokenAddress=_ideaTokenAddress;
    ...
    function IdeaPay(bytes32 _transactionId) public {
        ...
        uint256 balance = IERC20(ideaTokenAddress).balanceOf(msg.sender);
        //uint256 amountToTransfer = transaction.amount;
        if (balance < ideaBalanceLimit) {
            
            //amountToTransfer += fee;
            //require(amountToTransfer <= balance, "Not enough balance in escrow contract to cover the transaction and fee");
            require(IERC20(transaction.token).transfer(admin, fee), "Fee transfer failed");
   ...
   function IdearesolveDispute(bytes32 _transactionId, bool _buyerWins) public {
        ...
        uint256 balance = IERC20(ideaTokenAddress).balanceOf(msg.sender);
         if (balance < ideaBalanceLimit) {
            require(IERC20(transaction.token).transfer(admin, disputeFee), " dispute Fee transfer failed");
            //amountToTransfer += fee;
            //require(amountToTransfer <= balance, "Not enough balance in escrow contract to cover the transaction and fee");
            require(IERC20(transaction.token).transfer(admin, fee), "Fee transfer failed");
            transaction.amount-=fee;
            transaction.amount-=disputeFee;
    ...
    function setDisputeFee(uint256 _disputeFee) public {
        require(msg.sender == admin, "Only the admin can set the Dispute fee");
        disputeFee = _disputeFee;
    }
