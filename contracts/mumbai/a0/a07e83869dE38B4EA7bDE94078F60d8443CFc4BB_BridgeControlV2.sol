...
contract BridgeControlV2 is IBridgeControlV2, Ownable {
    using SafeERC20 for IERC20;

    event Widthdraw(uint256 indexed chainid, address indexed token, uint256 amount, address to);
    event Deposit(uint256 indexed chainid, bytes32 transactionId, address token, uint256 amount, address sender, address to);
    event BridgeFeeLog(address indexed fromUserProxy, address token, uint256 fee);

    mapping(uint256 => mapping(bytes32 => bool)) chainid_transactions;
    address public proxyFactory;
    address public lendingPool;
    address public tokenFactory;
    address public bridgeFeeController;

    constructor(address _proxyFactory, address _lendingPool, address _tokenFactory, address _bridgeFeeController) {
    ...
    function setBridgeFeeController(address _bridgeFeeController) external onlyOwner {
        require(_bridgeFeeController != address(0));
        bridgeFeeController = _bridgeFeeController;
    }

    function deposit(uint256 chainid, bytes32 transactionId, address token, uint256 amount, address sender, address to) external onlyOwner override {
        require(!chainid_transactions[chainid][transactionId], "transactionId already exec");
        chainid_transactions[chainid][transactionId] = true;

        address vToken = ITokenFactory(tokenFactory).validateTokenAddress(chainid, token);
        (address proxyAddr,) = IUserProxyFactoryV2(proxyFactory).getProxyV2(to);
        require(proxyAddr != address(0), "address not have proxy");

        (, , , , , , , address aToken, , , , ) = ILendingPool(lendingPool).getReserveData(vToken);

        if (aToken == address(0)){
            IVToken(vToken).mint(proxyAddr, amount);
            emit Deposit(chainid, transactionId, token, amount, sender, to);
        } else {
            IVToken(vToken).mint(address(this), amount);
            IERC20(vToken).safeApprove(lendingPool, amount);
            ILendingPool(lendingPool).deposit(vToken, amount, proxyAddr, 0);
            emit Deposit(chainid, transactionId, token, amount, sender, to);
        }
    }

    function withdraw(address vToken, address to, uint256 amount) external override {
        (uint256 chainid, address token) = ITokenFactory(tokenFactory).validateVTokenAddress(vToken);
        require(token != address(0), "Widthdraw: token not support");
        (uint256 fee, address bridgeFeeVault) = IBridgeFeeController(bridgeFeeController).getBridgeFee(vToken, amount);
        if (fee > 0) {
            IERC20(vToken).safeTransferFrom(msg.sender, bridgeFeeVault, fee);
            emit BridgeFeeLog(msg.sender, vToken, fee);
        }
        uint256 targetAmount = amount - fee;
        IVToken(vToken).burn(msg.sender, targetAmount);
        emit Widthdraw(chainid, token, targetAmount, to);
    }
    ...
    function transferFromEthereumForDeposit(
        ...
        require(!chainid_transactions[1][transactionId], "transactionId already exec");
        chainid_transactions[1][transactionId] = true;

        address vToken = ITokenFactory(tokenFactory).validateTokenAddress(1, token);
