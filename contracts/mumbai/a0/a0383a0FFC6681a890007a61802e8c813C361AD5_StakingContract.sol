...
contract StakingContract {
  ...
  function updateDirectRefPercent(uint256 poolIndex, uint256[15] memory rewardPercentages) external onlyAdmin{
    ...
    function togglePoolStatus(uint256 poolIndex) external onlyAdmin{
       ...
       function updateMaxPoolSize(uint256 poolIndex, uint256 maxPoolSize) external onlyAdmin{
        require(poolIndex<poolsCount,"Invalid Pool");
    ...
    function stake(
        ...
        require(stakingAmount>=pools[poolIndex].minAmount, "Stake Amount Is very Less");
    ...
    function unstake(uint256 poolIndex) external {
        ...
        token.transfer(msg.sender, users[msg.sender].deposits[poolIndex].stakedAmount);
