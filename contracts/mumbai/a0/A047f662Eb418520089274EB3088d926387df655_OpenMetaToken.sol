 ...
 *      Besides the addition of voting capabilities, we make a couple of custOMETAisations:
 ...
 *         airdrop claims are made frOMETA this balance.
 *       - Support for the owner (the DAO) to mint new tokens, at up to 2% PA.
 */
contract OpenMetaToken is ERC20, ERC20Permit, Ownable {
    using BitMaps for BitMaps.BitMap;

    bool private initialMintEvent = false;
    ...
    mapping(uint128 => BitMaps.BitMap) private claimed;
    uint128 private whiteListIndex = 0;
     ...
     */
    constructor(uint256 _claimPeriodEnds) ERC20("Test OMetaV3", "TOMetaV3") ERC20Permit("TOMetaV3") {
        claimPeriodEnds = _claimPeriodEnds;
    }

    /**
     * @dev Constructor.
     * @param amount The number of tokens to issue to the contract deployer.
     */
    function initialMint(uint256 amount) external onlyOwner {
        require(initialMintEvent == false, "The initial supply is minted");
        _mint(msg.sender, amount);
        initialMintEvent = true;
    }
    ...
    /**
     * @dev Returns true if the claim by a given address has already been made.
     * @param index The index into the merkle tree.
     */
    function isClaimed(uint256 index) public view returns (bool) {
        return claimed[whiteListIndex].get(index);
    }

    /**
     * @dev Sets the merkle root. Can be called multiple times.
