...
abstract contract Auctionable is Ownable, CreatorsHubOpenBase {
    ...
    function payout(uint256 _tokenId, address _seller) private {
        ...
        if (_seller == _owners[_tokenId]) {
            Fee[] memory fees = getCreatorFees();
            ...
            Fee[] memory fees = getResellerFees();
            for (uint256 i = 0; i < fees.length; ++i) {
                if (fees[i].percentage) {
                    uint256 fee = (fees[i].value * totalPrice) / 10000;
                    allFees += fee;
        ...
        notPaused(_tokenId)
    {
        ...
        clearBid(_tokenId, seller);
        transferFrom(seller, receiver, _tokenId, 1);
    ...
    ERC2981
{
    ...
    // Can fees be changed? (true/false)
    bool private _feesLock;
    ...
    function addCreatorFees(Fee[] calldata fees) public {
        ...
        for (uint256 i = 0; i < fees.length; i++) {
            ...
            _creatorFees.push(fees[i]);
        }
    }

    function addResellerFees(Fee[] calldata fees) public {
        require(_feesLock == false, "Fees locked");
        require(
            _rootOperators[_msgSender()] == true,
            "Must be owner or operator"
        );
        delete _resellerFees;

        for (uint256 i = 0; i < fees.length; i++) {
            _resellerFees.push(fees[i]);
        }
    }
    ...
    function getResellerFees()
        public
        view
        returns (Fee[] memory)
    {
        return _resellerFees;
    }
...
/**
   ...
   */
    ...
    function safeTransferFrom(
        ...
        require(operator == from || isApprovedForAll(from, operator), "not approved");
    ...
    function burn(
        ...
        uint256 id,
        uint256 amount
    ) public onlyAllowedOperator(account) {
        require(account != address(0), "ERC1155: burn from the zero address");

        address operator = _msgSender();

        require(operator == account || isApprovedForAll(account, operator), "not approved");
