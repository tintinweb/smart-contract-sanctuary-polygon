...
contract ERC1155Redeemer is ERC1155Holder {
    uint48 lastHashNumebr;
    ...
    /*  WARNING: The current implementation of this allows anyone to call the function 
        ...
        This MUST be fixed before production (probably by checking its own balance with
        the ERC1155's contract).
        WARNING: The code for redemtion of the discount/reward is contained in the event
        emitted by this contract. Before production, this must be changed so it is 
        generated off-blockchain via the same method.
    */
    function onERC1155Received(
        address /*operator*/,
        address from,
        ...
        uint48 hashNumber = uint48(bytes6(keccak256(abi.encodePacked(
            block.timestamp, from, nftId, address(this)
        ))));
        if (hashNumber < 100000000000000) { //ensure hashNumber is 15 digits
            hashNumber += 100000000000000;
        }
        lastHashNumebr = hashNumber;
