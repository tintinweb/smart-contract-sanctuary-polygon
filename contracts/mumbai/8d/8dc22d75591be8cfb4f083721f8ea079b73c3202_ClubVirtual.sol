...
contract ClubVirtual is ERC721URIStorage, EIP712, AccessControl{
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
  string private constant SIGNING_DOMAIN = "ClubVirtual";
  string private constant SIGNATURE_VERSION = "1";
 
  mapping (address => uint256) pendingWithdrawals;
 
  constructor(address payable minter)
    ERC721("ClubVirtual", "ClubVirtual") 
  ...
  function redeem(address redeemer, NFTVoucher calldata voucher,address creator,address admin,uint256 amount,uint256 adminpersent,uint256 platformFee) public payable returns (uint256) {
   ...
   // require(msg.value >= voucher.minPrice, "Insufficient funds to redeem");
    ...
    pendingWithdrawals[signer] += msg.value;
      ...
      uint256 creatoramount=amount*(100-adminpersent)/100;
      uint256 adminamount=amount*adminpersent/100;

      adminamount = adminamount + platformFee;
   ...
   function transferamount(address creator,address admin,address owner,uint256 amount,uint256 adminpersent, uint256 royaltypersent,uint256 platformFee) public payable
  {   address payable _creator = payable(creator);
      address payable _admin = payable(admin);
