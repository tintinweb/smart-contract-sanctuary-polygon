...
import "./CNPRwanshouShodaiAdmin.sol";

contract CNPRwanshouShodai is CNPRwanshouShodaiAdmin, ERC721A("CNP Rookies Wanshou Shodai", "CNPRWS") {
    constructor() {
     ...
     */
    function airdropMint(address[] calldata _to, uint256[] memory _quantity) external onlyAdmin {
     ...
     */
    function isDefaultAdmin(address _account) external view returns (bool) {
    ...
    function setApprovalForAll(address, bool) public virtual override {
        require(false, "this is the soulbound");
    }
    ...
    function _beforeTokenTransfers(
        ...
        require(to == address(0) || from == address(0), "this is the soulbound");
    }
...
import "./interface/ICNPRwanshouShodaiDescriptor.sol";
import "./CNPRwanshouShodaiConfig.sol";
import "@openzeppelin/contracts/access/AccessControl.sol";

abstract contract CNPRwanshouShodaiAdmin is CNPRwanshouShodaiConfig, AccessControl {
    /**
    ...
    /**
     *  @notice Grant the role of admin to the address.
     *  @dev Only callable by the owner.
     *  @param _admins The array of addresses of the admin.
     */
    function setAdminRole(address[] memory _admins) external onlyOwner {
    ...
    /**
     *  @notice Revoke the role of admin to the address.
     *  @dev Only callable by the owner.
     *  @param _admins The array of addresses of the admin.
     */
    function revokeAdminRole(address[] memory _admins) external onlyOwner {
        for (uint256 i = 0; i < _admins.length; i++) {
     ...
     */
    function setDescriptor(ICNPRwanshouShodaiDescriptor _descriptor) external onlyAdmin {
        descriptor = _descriptor;
    }

    /**
     *  @notice Set the max amount of tokens.
     *  @dev Only callable by the admin.
     *  @param _value The number of the max amount.
     */
...
abstract contract CNPRwanshouShodaiConfig is Ownable {
    // The Project token URI descriptor
    ICNPRwanshouShodaiDescriptor public descriptor;
    ...
    // Maximum number of Project tokens can be minted
    uint256 public maxSupply = 1017;
}

// SPDX-License-Identifier: UNLICENSED
pragma solidity >=0.8.10 <0.9.0;

interface ICNPRwanshouShodaiDescriptor {
