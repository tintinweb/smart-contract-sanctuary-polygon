...
contract Creator is ERC1155, Ownable, ERC1155Supply {
    ...
    modifier VerifyTime(uint startTime, uint endTime) {
        require(startTime > block.timestamp && endTime > startTime);
        _;
    }
    ...
    function launchpad_submit(uint _id, uint _initial, uint _price, address _token, uint _totalSupply, uint _startTime, uint _endTime, string memory _url, bytes memory data) external IsMint(_id) VerifyTime(_startTime, _endTime){
        require(_price > 0 && _totalSupply > 0 && _endTime > block.timestamp);
        ...
        _uri[_id] = _url;
        _mint(msg.sender, _id, _initial, data);
    ...
    function unlock_vote(uint option, uint _count, uint _id) external {
        // require(block.timestamp > votemint[_id][_count].endTime);
        require(option == 1 || option == 2);
    ...
    function vote_uri_token(uint _count, uint _id, uint amount) external {
        require(block.timestamp < voteuri[_id][_count].endTime);
    ...
    function _safeTransafer(address _token, address _to , uint amount) internal {
        ...
        IERC20(_token).transferFrom(msg.sender, _to, amountReceive); // owner 
        IERC20(_token).transferFrom(msg.sender, receiver, amountFee); // protocol
        IERC20(_token).transferFrom(msg.sender, IReferral(referral).getReceiver(msg.sender), amountFeeReferral); // referral
    }
    ...
    function _lockDemask() internal {
        IERC20(demask).transferFrom(msg.sender, address(this), feeLockDemask);
    }
    function _unlockDemask(address creator) internal {
        IERC20(demask).transfer(creator, feeLockDemask);
    }
