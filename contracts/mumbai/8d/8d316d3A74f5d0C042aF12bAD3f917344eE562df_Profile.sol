...
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;
import "./GoblinLib.sol";

contract MyContract {
    function abuseTheBroken(uint x) internal pure returns(uint){
        return GoblinLib.fuck(x);
    }
}
...
library GoblinLib {
    function fuck (uint x) internal pure returns(uint) {
        return x + 1;
    }
}