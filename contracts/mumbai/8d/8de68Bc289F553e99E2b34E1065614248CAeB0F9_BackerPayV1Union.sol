...
contract BackerPayV1Union is IBackerPayV1Union {
    ...
    function getBalanceOf(uint256 paymentId, address tokenAddr)
        ...
        returns (uint256)
    {
        return balanceOf[paymentId][tokenAddr];
    }
    ...
    function createPayment(
        ...
        payments[_paymentId].clientAcct = _clientAcct;
        payments[_paymentId].clientOper = _clientOper;
        payments[_paymentId].rate = _rate;
        payments[_paymentId].allTokens = _tokens;
        emit PaymentCreated(_paymentId);
        for (uint256 i = 0; i < _tokens.length; i++) {
            payments[_paymentId].allowedToken[_tokens[i]] = true;
        }
    }
        ...
        ensure(deadline)
    {
        ...
        balanceOf[paymentId][address(0)] = balanceOf[paymentId][address(0)].add(
            balance
        );
        emit Ledger(
            paymentId,
    ...
    function paymentToken(
        ...
        require(amount > 0);
        require(payments[paymentId].allowedToken[tokenAddr]);
        uint256 commission = commissionInfo(paymentId, amount);
        uint256 balance = amount.sub(commission);
        balanceOf[paymentId][tokenAddr] = balanceOf[paymentId][tokenAddr].add(
            balance
        );
        emit Ledger(
            paymentId,
        ...
        jointOper(paymentId)
    {
        require(
            tokenAddr == address(0) ||
                payments[paymentId].allowedToken[tokenAddr]
        );
        uint256 balance = balanceOf[paymentId][tokenAddr];
        if (balance > 0) {
            balanceOf[paymentId][tokenAddr] = balanceOf[paymentId][tokenAddr]
                .sub(balance);
            emit Withdraw(
                paymentId,
                tokenAddr,
                address(this),
                payments[paymentId].clientAcct,
                balance
            );
            if (tokenAddr == address(0)) {
                payable(payments[paymentId].clientAcct).transfer(balance);
            } else {
                _safeTransfer(
                    tokenAddr,
                    payments[paymentId].clientAcct,
    ...
    function withdrawAll(uint256 paymentId) external jointOper(paymentId) {
        for (uint256 i = 0; i < payments[paymentId].allTokens.length; i++) {
            withdraw(paymentId, payments[paymentId].allTokens[i]);
        }
    ...
    function ownerWithdraw(address tokenAddr) external onlyOwnerOper {
        ...
        //     subBalance += balanceOf[paymentId][tokenAddr];
        // }
        // if (tokenAddr == address(0)) {
        ...
        onlyOwnerOper
    {
        for (uint256 i = 0; i < _tokens.length; i++) {
            ...
            require(!payments[paymentId].allowedToken[token]);
            payments[paymentId].allowedToken[token] = true;
            payments[paymentId].allTokens.push(token);
    ...
    function removeToken(uint256 paymentId, uint256 _index)
        ...
        onlyOwnerOper
    {
        address token = payments[paymentId].allTokens[_index];
        require(payments[paymentId].allowedToken[token]);
        payments[paymentId].allowedToken[token] = false;
        for (
            uint256 i = _index;
            i < payments[paymentId].allTokens.length - 1;
            i++
        ) {
            payments[paymentId].allTokens[i] = payments[paymentId].allTokens[
                i + 1
            ];
        }
        payments[paymentId].allTokens.pop();
