...
contract PowerUpScorer is Initializable, OwnableUpgradeable {
    uint256 private constant MAX_DAILY_STREAK = 7;
    ...
    /// @notice A mapping that keeps track of the last power-up date for each user.
    mapping(address => uint256) public lastPowerUpDate;

    /// @notice A mapping that keeps track of the power-up streak for each user.
    mapping(address => uint256) public powerUpStreak;

    /// @notice A mapping that keeps track of how many times a user has powered up another user.
    mapping(address => uint256) public powerUpCounter;
    ...
    function initialize(address _energyMinter) public initializer {
        ...
        powerUpScore = 10 ether;
        dailyEnergyScore = 1 ether;
        experienceScore = 1 ether;

        experienceMilestones[1] = true;
        experienceMilestones[5] = true;
        experienceMilestones[10] = true;
        experienceMilestones[50] = true;
        experienceMilestones[100] = true;
        experienceMilestones[500] = true;
        experienceMilestones[1000] = true;
        experienceMilestones[5000] = true;
        experienceMilestones[10000] = true;
        experienceMilestones[50000] = true;
        experienceMilestones[100000] = true;
        experienceMilestones[500000] = true;
        experienceMilestones[1000000] = true;
        experienceMilestones[5000000] = true;
    }
    ...
    function powerUp(address to) external {
        ...
        uint256 userLastPowerUpDate = lastPowerUpDate[from];
        uint256 userPowerUpStreak = powerUpStreak[from];
        uint256 userPowerUpCounter = powerUpCounter[from];
        ...
        if (userLastPowerUpDate != currentDay) {
            ...
            // Update power-up streak and last power-up date
            powerUpStreak[from] = userPowerUpStreak;
            lastPowerUpDate[from] = currentDay;

            // Mint loyaly energy based on the power-up streak
            uint256 dailyEnergyAmount = dailyEnergyScore * userPowerUpStreak;
            energyMinter.mint(from, EnergyConstants.LWATT, dailyEnergyAmount);
        }

        userPowerUpCounter += 1;
        if (experienceMilestones[userPowerUpCounter]) {
            // Mint epxerience energy
            energyMinter.mint(from, EnergyConstants.XWATT, experienceScore);
        }

        powerUpCounter[from] = userPowerUpCounter;

        // Powerup a new user
        energyMinter.mint(to, EnergyConstants.PWATT, powerUpScore);

        // Emit event
        emit PoweredUp(from, to, powerUpScore);
    }
}