...
contract eSignDocsCollection  is ERC1155, SimpleAccessControl 
{
   ...
   //mapping(address => uint256[]) private  _signersToIds;
    ...
    function mintDocument( address[] memory signers, bytes32 envelopeId, bytes memory docHash   ) public returns (bool)
    {
        ...
        _idsToDocument[newId].signers = signers;
        _idsToDocument[newId].envelopeId = envelopeId;
        _idsToDocument[newId].documentHash = docHash;
        _idsToDocument[newId].nftId = newId;
        // hash document to id
        _hashDocToNftId[docHash] = newId;
     

        for (uint i = 0; i<signers.length; i++) 
        {
            _mint(signers[i], newId, 1, "");
        }
