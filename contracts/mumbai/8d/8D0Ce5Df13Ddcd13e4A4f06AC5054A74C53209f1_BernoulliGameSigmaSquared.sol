...
/**
 * An interface implemented by contracts that give rewards to Sigma Squared game users based on their activity.
...
abstract contract BernoulliGameBase is RandomnessConsumer, Ownable {
    ...
    function receiveRandomInt(bytes32 requestId, uint256 randomInt) external onlyRandomnessProvider {
        ...
        if (currentRandomInt < uint128(probability)) {
            ...
            if (address(rewardsContract) != address(0)) {
                rewardsContract.recordWin(currentBet.bettor, currentBet.amount, winAmount, requestId);
            }
            ...
            if (address(rewardsContract) != address(0)) {
                rewardsContract.recordLoss(currentBet.bettor, currentBet.amount, requestId);
            }
            // Keep track of total contract profit.
     ...
     * @param rewardsIn The address of the rewards contracts (or 0x0 if nothing should be reported).
     */
    ...
    function _applyRiskChecks(uint256 amount, uint256 multiplier) internal view {
        ...
        require(((amount * multiplier) / 1e8) < ((getContractBalance() * maxLossMantissa) / 1e8), "Max possible win is too high.");
