...
contract GNSPriceAggregatorV6_3 is ChainlinkClient, TWAPPriceGetter {
    ...
    constructor(
        ...
        require(
            address(_storageT) != address(0) && address(_pairsStorage) != address(0)
                && address(_linkPriceFeed) != address(0) /*&& _minAnswers >= MIN_ANSWERS **/ && _minAnswers % 2 == 1
                && _nodes.length > 0 && _linkToken != address(0),
            "WRONG_PARAMS"
        );
    ...
    function getPrice(uint256 pairIndex, OrderType orderType, uint256 leveragedPosDai)
        external
        onlyTrading
        returns (uint256)
    {
        (string memory from, string memory to, bytes32 job, uint256 orderId) = pairsStorage.pairJob(pairIndex);
        ...
        uint256 linkFeePerNode = linkFee(pairIndex, leveragedPosDai) / nodes.length;
        ...
        emit PriceRequested(orderId, job, pairIndex, orderType, nodes.length, linkFeePerNode);

        return orderId;
    }

    // Fulfill on-demand price requests
    function fulfill(bytes32 requestId, uint256 price) external recordChainlinkFulfillment(requestId) {
    ...
    function linkFee(uint256 pairIndex, uint256 leveragedPosDai) public view returns (uint256) {
        ...
        return pairsStorage.pairOracleFeeP(pairIndex) * leveragedPosDai * 1e8 / uint256(linkPriceUsd) / PRECISION / 100;
    }

    // Manage pending SL orders
    function storePendingSlOrder(uint256 orderId, PendingSl calldata p) external onlyTrading {
...
abstract contract TWAPPriceGetter {
    ...
    constructor(IUniswapV3Pool _uniV3Pool, address _token, uint32 _twapInterval, uint256 _precision) {
        require(
            address(_uniV3Pool) != address(0) && _twapInterval >= MIN_TWAP_PERIOD && _twapInterval <= MAX_TWAP_PERIOD
                && _precision > 0,
