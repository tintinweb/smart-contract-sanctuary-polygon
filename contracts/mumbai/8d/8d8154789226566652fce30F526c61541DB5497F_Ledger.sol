...
contract Ledger is UUPSUpgradeable, OwnableUpgradeable {
    Donation[] donations;

    event Claim(uint amount, uint when);

    function donate(string calldata _repository) external payable {
        Donation donation = new Donation{value: msg.value}(_repository);
        donations.push(donation);
    }

    function claim(address payable _to) external {
        emit Claim(address(this).balance, block.timestamp);
