...
contract NFTBoxFeeDiscount is AccessControl {
    ...
    NFTDiscount[] private nftDiscount;
    ...
    function setUserDiscount(address userAddress, uint256 percentage) external {
        ...
        require(percentage >= 0 && percentage <= 10000, "percentage must be between 1 and 100");
        userDiscount[userAddress] = percentage;
    }

    function setNFTDiscount(address nftAddress, uint32 percentage) external {
        require(hasRole(ADMIN, msg.sender), "Not Admin");
        require(percentage >= 0 && percentage <= 10000, "percentage must be between 1 and 100");
        uint256 findIndex;
        for(uint256 i ; i < nftDiscount.length; ++i) {
            if(nftDiscount[i].nftAddress == nftAddress && percentage !=0) {
                nftDiscount[i].percentage = percentage;
                ++findIndex;
            } else if(nftDiscount[i].nftAddress == nftAddress && percentage == 0) {
                for(uint256 j = i ; j < nftDiscount.length - 1; ++j) {
                    nftDiscount[j] = nftDiscount[j+1];
                }
                nftDiscount.pop();
            }
        }
        if(findIndex == 0) {
            NFTDiscount memory newDiscount;
            newDiscount.nftAddress = nftAddress;
            newDiscount.percentage = percentage;
            nftDiscount.push(newDiscount);
        }
    }
    ...
    function getNFTDiscount(address nftAddress) external view returns(uint256) {
        for(uint256 i ; i < nftDiscount.length; ++i) {
            if(nftDiscount[i].nftAddress == nftAddress)
                return nftDiscount[i].percentage;
        }
        return 0;
    }

    function getTotalDiscount(address owner) external view returns(uint256) {
        uint256 totalDiscount;
       for(uint256 i ; i < nftDiscount.length; ++i) {
            if(IERC721(nftDiscount[i].nftAddress).balanceOf(owner) > 0 && nftDiscount[i].percentage > totalDiscount)
                totalDiscount = nftDiscount[i].percentage;
       }
        if(totalDiscount > userDiscount[owner])
            return totalDiscount;
        else
            return userDiscount[owner];
    }
