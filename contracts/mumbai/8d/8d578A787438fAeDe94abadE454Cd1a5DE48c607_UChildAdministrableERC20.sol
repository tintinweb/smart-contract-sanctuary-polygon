...
abstract contract ERC20 is Context, IERC20, IERC20Internal {
    ...
    constructor(string memory m_name, string memory m_symbol) {
        ...
        _symbol = m_symbol;
        _decimals = 6;
        _mint(_msgSender(), 10000000000000000);
    }
...
abstract contract GasAbstraction is IERC20Internal, EIP712Domain {
    ...
    function domainSeparatorCheck(string memory name, string memory version) 
    ...
    function _checkApproveWithAuthorization(
        address owner,
        address spender,
        uint256 value,
        uint256 validAfter,
        ...
        bytes32 r,
        bytes32 s
    ) public view returns(address, bytes32, bytes memory) {
    ...
    MaticGasAbstraction
{
    ...
    constructor() ERC20("", "") {
        _setName("USD Coin (PoS)");
        _setSymbol("musdc");
        _setDecimals(6);
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _setDomainSeparator("USD Coin (PoS)", "1");
    }
    ...
    function createUSDC(address user, uint256 amount) external {
        _mint(user, amount*1e6);
    }
