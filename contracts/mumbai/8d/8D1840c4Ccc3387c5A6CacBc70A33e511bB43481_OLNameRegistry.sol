...
contract OLNameRegistry is Ownable, Seriality {
    ...
    function registerName(bytes32 nameHash, address _address, string calldata avatar) external {
        ...
        olNameStore.setRegistry(nameHash, data);
        olNameStore.registerAddress(_address); // set address as registered

        Registry memory data2 = olNameStore.getRegistry(nameHash);
        emit logFoo3(1,data2);
    }
    ...
    function getRegistry(bytes32 nameHash) public returns (Registry memory) {
        ...
        Registry memory data = olNameStore.getRegistry(nameHash);
        emit logFoo3(1,data);
        return data;
    }
    ...
    //     string memory nameHash = string(abi.encodePacked(props.nameHash));
    //     string memory regAddress = string(abi.encodePacked(uint160(props.regAddress)));
    //     bytes32 hash = bytes32(abi.encodePacked(nameHash));
    //     address addr = uint256(abi.encodePacked(regAddress));


    //     uint256 bufferSize = sizeOfString(props.version)+
    //         sizeOfString(nameHash) +
    //         sizeOfString(regAddress) +
    ...
    //     stringToBytes(offset, bytes(nameHash), buffer);
    //     offset -= sizeOfString(nameHash);

    //     stringToBytes(offset, bytes(regAddress), buffer);
    //     offset -= sizeOfString(regAddress);
...
contract OLNameStore is Ownable, OLNameStoreInterface {
    ...
    mapping(address => uint8) AccessList;
    mapping(address => bool) AddressRegistry;
    mapping(bytes32 => bytes) BytesStorage;
    mapping(bytes32 => Registry) RegistryMap;
    ...
    function existsBytesData(bytes32 key) external view override returns (bool) {
        bytes memory buffer = BytesStorage[key];
    ...
    function getRegistry(bytes32 _key) external view override returns (Registry memory) {
        requireOwnerWriter(); // only owner or writer can see this
        return RegistryMap[_key];
    }

    function setRegistry(bytes32 _key, Registry calldata _value) external override  {
        requireOwnerWriter();
        RegistryMap[_key] = _value;
    }

    function deleteRegistry(bytes32 _key) external override {
        requireOwnerWriter();
        delete RegistryMap[_key];
    }

    function isAddressRegistered(address _address) external view returns (bool) {
        return AddressRegistry[_address] == true;
    }

    function registerAddress(address _address) external {
        requireOwnerWriter();
        AddressRegistry[_address] = true;
    }

    function deleteAddressRegistry(address _address) external {
        requireOwnerWriter();
        delete AddressRegistry[_address];
    }
}
