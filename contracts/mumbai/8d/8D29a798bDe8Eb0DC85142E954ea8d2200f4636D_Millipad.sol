 ...
 */
contract Millipad is Context, IERC20, IERC20Metadata {
    ...
    event gameLost(address indexed user,uint256 indexed lostTokens);
    event userStaked(address indexed user,uint256 indexed AmountOfTokens);
    ...
    function hashUSERTransaction(
        string memory userId,
        string memory nonce
    ) public pure returns (bytes32) {
        bytes32 hash = keccak256(abi.encodePacked(userId , nonce));
        return hash;
    }
    ...
    function StakeToken(address user,uint256 amount) external{
     ...
     function FinishGame(address user,bool gameResult,uint256 stakedTokens,uint256 rewardTokens,bytes memory signature, string memory nonce) external {                 
        ...
        if(gameResult){
        _mint(user,rewardTokens);
        _transfer(address(this), user, stakedTokens);
        }else{
            emit gameLost(user,stakedTokens);
        }
