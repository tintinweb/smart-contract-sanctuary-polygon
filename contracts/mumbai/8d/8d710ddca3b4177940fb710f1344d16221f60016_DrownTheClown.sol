...
contract DrownTheClown is ReentrancyGuard {
    ...
    */
    ...
    // Write out the round number, winners x3
    event RoundWinners(uint roundNumber, address winner1, address winner2, address winner3);

    // MATIC Won (round number, purse x3)
    event RoundWinnings(uint roundNumber, uint purse1, uint purse2, uint purse3);
    ...
    function pickWinner(uint newCostPerPlayer,uint newTotalPlayerLimit, uint256[3] calldata purses, uint newSideBetCost, uint seed) public onlyOwner nonReentrant {
        ...
        emit RoundWinners(gameNumber, winnerAddresses[0], winnerAddresses[1], winnerAddresses[2]);
        emit RoundWinnings(gameNumber, winnerPurses[0], winnerPurses[1], winnerPurses[2]);
