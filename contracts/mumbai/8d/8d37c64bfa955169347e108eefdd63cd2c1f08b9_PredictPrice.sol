...
contract PredictPrice is Ownable {
    ...
    event Predicted(uint256 orderId, address user, address token, uint256 triggerTime, uint256 predictTime, uint256 amount, uint256 totalFee, string tokenName, string detail, uint256 odds, uint256 timestamp);
    ...
    function predictPrice(uint256 orderId, address token, uint256 triggerTime, uint256 predictTime, uint256 amount, uint256 totalFee, string memory tokenName, string memory detail, uint256 odds) public {
        ...
        _orderList.push(Order(orderId, msg.sender, token, triggerTime, predictTime, amount, totalFee, tokenName, detail, odds, block.timestamp));
        uint256 id = _orderList.length;
        _orderIndexes[orderId] = id;
        _allCount ++;
        emit Predicted(orderId, msg.sender, token, triggerTime, predictTime, amount, totalFee, tokenName, detail, odds, block.timestamp);
    }
