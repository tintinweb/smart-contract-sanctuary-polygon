...
contract Kyc is AccessControl {
   ...
   */
  /**
   * @notice Country numeric code from the ISO 3166
   */
  mapping(address => uint16) public countryCode;
  mapping(address => uint) public nonce;
  mapping(uint => bool) private cannotBuy;
  ...
  function changeCountryCode(
    ...
    require(countryCode[toKyc] > 0, "Not a Kyced address");
    require(countryCode[toKyc] != BLACKLIST, "BlackListed");
    countryCode[toKyc] = countryCode_;
  }
  ...
  function revokeKyc(address toUnKyc) external onlyRole(DEV) {
    countryCode[toUnKyc] = 0;
  }

  function revokeKyc(address[] calldata toUnKyc) external onlyRole(SIGNER) {
    uint kycLen = toUnKyc.length;
    unchecked {
      for (uint i = 0; i < kycLen; ) {
        countryCode[toUnKyc[i]] = 0;
        ++i;
      }
    }
  }
  ...
  function addKyc(
    ...
    require(
      hasRole(
        SIGNER,
        ECDSA.recover(
          getHashKyc(toKyc, countryCode_, deadline, nonce_),
          v,
          r,
          s
        )
  ...
  function canBuy(address sender) public view returns (bool) {
    return !cannotBuy[countryCode[sender]];
  }

  function isKyced(address sender) public view returns (bool) {
    return countryCode[sender] > 0 ? true : false;
  }
