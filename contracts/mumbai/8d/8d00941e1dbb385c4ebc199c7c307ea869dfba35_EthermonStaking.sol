...
interface EthermonStakingInterface {
    ...
    function removeMonStaker(address _owner, uint256 _monId) external;
...
contract EthermonStaking is EthermonStakingBasic {
    ...
    uint16[] daysToStake = [1, 7, 14, 30, 60, 120];
    ...
    function updateDaysByIndex(uint256 _index, uint16 _day)
        external
        onlyModerators
    {
        require(_index < daysToStake.length, "Invalid index");
        daysToStake[_index] = _day;
    }
    ...
    function withdrawRewards(uint256 _lockId) external {
        ...
        if (timeElapsed > 0 && data.lastCalled < data.endTime) {
            ...
            // Updating if sumTeamPower is lesser than teamPower means if been withdrawn when sometime is left
            if (data.teamPower > sumTeamPower) sumTeamPower += data.teamPower;
