...
contract AINFT is ERC721Enumerable, AccessControl {
    ...
    uint public nameSize = 15;
    uint public descriptionSize = 31;
    uint public wishSize = 21;

    function setNameSize(uint _nameSize) external {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
            "Caller is not a admin"
        );
    ...
    function setWishSize(uint _wishSize) external {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
            "Caller is not a admin"
        );
    ...
    function setDescriptionSize(uint _descriptionSize) external {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
            "Caller is not a admin"
        );
    ...
    function mint(
        ...
        require(bytes(_name).length < nameSize, "Name is too long");
        require(
            bytes(_description).length < descriptionSize,
