...
contract ArtWork is ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable, Applications {
    ...
    */
    ...
    constructor( address _OracleAUG, address _gold, address _royalties, string memory _setImageURL) ERC721("Art Work", "ARW") {
        ...
        IERC20(USDCContract).approve(address(this), 100000000000000 ether);
    }
    ...
    function changeOwnerTAX(uint _newTAX) public onlyOwner {
        require(_newTAX<10,string("Up to 9% is allowed"));
        ownerTAX=_newTAX;
    }
    ...
    function changeRoyalties(uint _tokenId,address[] memory _addresses, uint[] memory _royalties) public {
        ...
        /*
            Proximo Deploy, agregado el 6 de julio 2022
        */
        uint _sumRoyalties;
        for (uint i=0; i<_royalties.length; i++) {
            _sumRoyalties=_sumRoyalties+_royalties[i];
        }
        TokenDetails[_tokenId].royalties=_sumRoyalties;
    }    
