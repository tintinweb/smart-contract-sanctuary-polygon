...
contract MetaData is IMetaData, VRFConsumerBaseV2, Ownable {
    ...
    function fulfillRandomWords(
        ...
        if (
            ...
            if (legendary_roll < i_legendaries - legendaryId) {
        ...
        if (
            ...
            s_tokenMetaData[tokenId].scratchedNum = s_scratchedOffCount;

            (bool shouldOpenPool, uint256 id) = s_secondChance.needsToOpenPool(
                s_scratchedOffCount
            );

            if (shouldOpenPool) {
                s_secondChance.openPrizePool(id, block.timestamp + 4 hours);
            }
        ...
        if (
            ...
            s_tokenMetaData[tokenId].prize = prize;
            s_tokenMetaData[tokenId].status = NFTStatus.DONE;
            s_tokenMetaData[tokenId].inProgress = false;
    ...
    function setClaimed(uint256 tokenId) external onlyOwner {
        s_tokenMetaData[tokenId].claimed = true;
    }
