...
contract GroupFacet is Modifiers {
    ...
    function attesterSignUp(address attester, uint256 epochLength) public {
        IAttester(attester).attesterSignUp(epochLength);
        s.attester = attester;
    }

    function userUnirepSignUp(
        address attester,
        uint256[] memory publicSignals,
        uint256[8] memory proof
    ) public {
        if(attester == address(this)) {
            s.unirep.userSignUp(publicSignals, proof);
        } else {
            IAttester(attester).userSignUp(publicSignals, proof);
        }
    }
}
