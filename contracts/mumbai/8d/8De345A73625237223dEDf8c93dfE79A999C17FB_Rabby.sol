...
import "./interface/IRabbyDescriptor.sol";

contract Rabby is ERC721, AccessControl {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;

    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    IRabbyDescriptor public descriptor;
    ...
    constructor(IRabbyDescriptor _descriptor) ERC721("Rabby", "RABBY") {
    ...
    function setDescriptor(IRabbyDescriptor newDescriptor) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function updateSeed(uint256 tokenId, uint256 seed) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(canUpdateSeed, "Cannot set the seed");
        seeds[tokenId] = seed;
        emit SeedUpdated(tokenId, seed);
    }

    function disableSeedUpdate() external onlyRole(DEFAULT_ADMIN_ROLE) {
        canUpdateSeed = false;
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        require(_exists(tokenId), "Rabby does not exist.");
        uint256 seed = seeds[tokenId];
        return descriptor.tokenURI(tokenId, seed);
    }

    // 13 11 13 14 12 19999
    function generateSeed(uint256 tokenId) private view returns (uint256) {
        uint256 r = random(tokenId);
        uint256 characterSeed = r % 4 + 10;             // 10 -> 13
        uint256 aspectSeed = (r >> 24) % 2 + 10;        // 10 -> 11
        uint256 bloodtypeSeed = (r >> 48) % 4 + 10;     // 10 -> 13 
        uint256 elementSeed = (r >> 72) % 5 + 10;       // 10 -> 14
        uint256 backgroundSeed = (r >> 120) % 3 + 10;   // 10 -> 12
        uint256 birthdaySeed = (r >> 96) % 8999 + 1000; // 1000 -> 9999
        return 10000 * (100 * (100 * (100 * (100 * characterSeed + aspectSeed) + bloodtypeSeed) + elementSeed) + backgroundSeed) + birthdaySeed;
    }
