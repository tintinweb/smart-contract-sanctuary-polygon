...
contract SalePhases is Context, Ownable, Initializable {
    ...
    IERC20 USDT = IERC20(0x6c07518DCDe3512E2B26526dF24eDCEF9Cd4b6e2); // ERC20
    IERC20 USDC = IERC20(0x6c07518DCDe3512E2B26526dF24eDCEF9Cd4b6e2); // ERC20
    IERC20 BUSD = IERC20(0x6c07518DCDe3512E2B26526dF24eDCEF9Cd4b6e2); // ERC20
    IERC20 ElChai = IERC20(0x8cc3c7109BCb53936B85a35EEC179856cf1BF998); // ERC20
    ...
    function whatUnlockPhaseWeAre() public view returns (uint256) {
        uint256 _phase;
        if(block.timestamp >= First) { _phase = 2; }
        if(block.timestamp >= Second) { _phase = 3; }
        if(block.timestamp >= Third) { _phase = 4; }
        if(block.timestamp >= Fourth) { _phase = 5; }
        if(block.timestamp >= Fifth) { _phase = 6; }
        if(block.timestamp >= Sixth) { _phase = 7; }
        if(block.timestamp >= Seventh) { _phase = 8; }
        if(block.timestamp >= Eight) { _phase = 9; }
        if(block.timestamp >= Nine) { _phase = 10; }
    ...
    function testItN1() external { // remove before LIVE
