...
contract DCAStrategy is Ownable, AccessControl {
    ...
    uint256 public maxPoolFee; // percentage amount divided by DENOMINATOR
    ...
    function updatePool(
        ...
        poolData.feeManager = feeManager;
        ...
        for (uint256 i = 0; i < length; i++) {
            DCATypes.GetPoolsInfoResponse memory item = DCATypes
                .GetPoolsInfoResponse({
                    ...
                    accessManager: pool[poolIds[i]].accessManager,
                    feeManager: pool[poolIds[i]].feeManager,
    ...
    function removeFromPool(uint256 id, address user) public onlyOwner {
        DCATypes.PoolData memory poolData = pool[id];
        if (poolData.executionData.isExecuting) {
            revert StrategyExecutionInProgress();
        }
        ...
        emit PoolParticipantRemoved(id, user);
    }
        ...
        returns (uint32)
    {
        ...
        while (participantsIndex >= 0 && maxLoopIterations > 0) {
                ...
                )
            {
                ...
                uint256 fee = IFeeManager(poolData.feeManager).calculateFee(
                    poolId,
                    msg.sender,
                    toInvest
                );
                if ((fee * DENOMINATOR) / toInvest > maxPoolFee) {
                    revert MaxPoolFeeExceeded();
                }
...
interface IFeeManager {
    ...
    function getFeePercentage(uint256 poolId, address user)
        external
        view
        returns (uint256);
