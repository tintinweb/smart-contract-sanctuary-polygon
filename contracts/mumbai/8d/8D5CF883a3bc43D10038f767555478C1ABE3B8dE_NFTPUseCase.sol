...
contract NFTPUseCase is Ownable, ReentrancyGuard {
    ...
    constructor(address nftp_, address token_, address priceInfo_) {
        if (nftp_ == address(0)) revert ZeroAddress();
        if (token_ == address(0)) revert ZeroAddress();
        if (priceInfo_ == address(0)) revert ZeroAddress();

        nftp = nftp_;
        paidWith = IERC20(token_);
        priceInfo = IPrices(priceInfo_);
        admin = msg.sender;
    }
    ...
    * @notice  */
    ...
    function recharge(uint256 tokenId) external payable {
        uint256 cost = getRechargeCostBNB(tokenId);

        if (msg.value < cost) revert ValueTooLow();
    ...
    function authorizedRecharge(uint256 _tokenId, address _address) external onlyAdmin {
    ...
    function getRechargeCost(uint256 tokenId) public view returns(uint256) {
        ...
        //(uint256 paidWithPrice,) = priceInfo.getPrices();
        uint256 paidWithPrice = 1;
        return paidWithPrice == 0 ? 0 : price * 10**18 / paidWithPrice;
    }

    function getRechargeCostBNB(uint256 tokenId) public view returns(uint256) {
        uint256 price = getRechargePrice(tokenId);
        //(uint256 priceBNB) = priceInfo.getPrices();
        uint256 priceBNB = 1;
        return priceBNB == 0 ? 0 : price * 10**18 / priceBNB;
    }
}