...
contract Loan is AccessProtected, ReentrancyGuard, BaseRelayRecipient, IERC721Receiver, EIP712Base, Pausable {
    ...
    function loanItem(uint256 _loanId) external nonReentrant whenNotPaused {
        ...
        checkLoanRewardPercentageLimit(sender, loanItems[_loanId].percentageRewards);
    ...
    function cancelLoan(uint256 _loanId) external nonReentrant whenNotPaused {
        ...
        require(loanItems[_loanId].canceled, "Loan already canceld");
        if (loanItems[_loanId].startingTime > 0) {
            require(block.timestamp >= loanItems[_loanId].endTime, "Loan period is still active ");
        }
    ...
    function withdrawTokenToOwners(uint256[] memory loanIds) external onlyAdmin {
        for (uint256 _loanIdIndex = 0; _loanIdIndex < loanIds.length; _loanIdIndex++) {
            ...
            for (uint256 i = 0; i < loanItems[_loanId].tokenIds.length; i++) {
                ...
                address nftAddress = loanItems[_loanId].nftAddresses[i];
                _nftToBundleId[nftAddress][id] = 0;
                console.log("transfering nft to owner", loanItems[_loanId].owner, id);
                IERC721(nftAddress).transferFrom(address(this), loanItems[_loanId].owner, id);
            }
        }
    }
