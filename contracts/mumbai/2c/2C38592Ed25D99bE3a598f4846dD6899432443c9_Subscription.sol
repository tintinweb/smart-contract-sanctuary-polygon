...
contract Subscription is OwnableUpgradeable, ERC721EnumerableUpgradeable, ISubscription {
    ...
    //nftId -> SinglePostPaymentInfo
    mapping(uint256 => SinglePostMetadata) private singlePostPaymentInfo;
    ...
    function mint(
        ...
        ( , address owner, , , ) =
        abi.decode(vars.data,(uint256, address, uint256, uint256, uint256));
    ...
    function getSinglePostPaymentInfo(uint256 _nftId) external override view returns(DataTypes.SinglePostPaymentInfo memory info) {
        address currentOwner = super.ownerOf(_nftId);
        return convertSinglePostMetadata(singlePostPaymentInfo[_nftId], currentOwner);
    }
    ...
    function writeInfo(uint256 tokenId, bytes calldata data) private {
        (uint256 postId, , uint256 paymentType, uint256 startTime, uint256 endTime) =
        abi.decode(data,(uint256, address, uint256, uint256, uint256));
        singlePostPaymentInfo[tokenId] = SinglePostMetadata(startTime, endTime, postId, DataTypes.PaymentType(paymentType));
    }
    ...
    function convertSinglePostMetadata(SinglePostMetadata storage _inData, address _tokenOwner) private view returns(
        DataTypes.SinglePostPaymentInfo memory outData
    ) {
        outData.startTime = _inData.startTime;
        outData.endTime = _inData.endTime;
        outData.postId = _inData.postId;
