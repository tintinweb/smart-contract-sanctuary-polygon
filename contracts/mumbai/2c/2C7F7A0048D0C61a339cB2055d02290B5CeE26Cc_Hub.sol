...
contract Hub is Initializable {
    ...
    mapping(address => address) coinToSuMapping;
    ...
    modifier onlySU() {
        require(suToCoinMapping[msg.sender] != address(0), "You must be an SU address.");
        _;
    }
    ...
    function initialize(address _rainTokenAddress) external initializer {
        rainTokenAddress = _rainTokenAddress;
    }

    /// @notice This function is used to update the mappings (SU -> COIN$) and (COIN$->SU) of the Hub contract.
    /// @param _su The startup address
    /// @param _coin The COIN$ address associated with the startup
    function addSuToCoin(address _su, address _coin) external onlyAdmin {
        if (suToCoinMapping[_su] != address(0)) {
            delete suToCoinMapping[_su];
        }
        if (coinToSuMapping[_coin] != address(0)) {
            delete coinToSuMapping[_coin];
        }
        suToCoinMapping[_su] = _coin;
        coinToSuMapping[_coin] = _su;
        emit NewPairAdded(_su, _coin);
    }

    /// @notice This function is used to add a hash in the mapping from startups to hash values.
    /// @param _su The startup address
    /// @param _new_hash The hash value
    ...
    /// @notice This function is used to update the address of a SU and make all mappings consistent with this update.
    /// @param _oldSuAddress The old startup address
    /// @param _newSuAddress The new startup address
        ...
        onlyAdmin
    {
        ...
        coinToSuMapping[suCoin] = _newSuAddress;
    ...
    /// @notice This function is used to update the RAIN$ address referenced by the Hub contract.
    /// @param newRainTokenAddress The new RAIN$ address
    ...
    /// @notice This function is used to view the address of the COIN$ associated with a given startup address.
    /// @param _su The startup address
    function getCoinFromSu(address _su) external view returns (address) {
        return suToCoinMapping[_su];
    }

    /// @notice This function is used to view the address of the startup associated with a given COIN$ address.
    /// @param _coin The COIN$ address
    function getSuFromCoin(address _coin) external view returns (address) {
        return coinToSuMapping[_coin];
    }

    /// @notice This function is used to view a hash value associated with a startup.
    /// @param _su The startup address
    /// @param _index The index of the hash value
