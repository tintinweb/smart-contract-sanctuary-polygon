...
contract OracleV2RealtimeUpdatedPrice is IEpoch, OwnableUpgradeSafe {
    using FixedPoint for *;
    ...
    function checkEpoch() public view returns (bool) {
        ...
        return epochPegPrice[_epoch] == 0 || block.timestamp >= nextEpochPoint();
    }
    ...
    function getPegPriceUpdated() external override view returns (int256) {
        int256 _twap = int256(twap(mainToken, 1e18));
        int256 _realTimePrice = int256(_getRatio(sideToken, mainToken, pair));
        return (_twap < _realTimePrice) ? _twap : _realTimePrice;
    }
    ...
    function consult(address _token, uint256 _amountIn) public view returns (uint144 _amountOut) {
        ...
        if (_token == _token0) {
            ...
            require(_token == _token1, "INVALID_TOKEN");
    ...
    function _getRatio(address _tokenA, address _tokenB, IUniswapV2Pair _pair) internal view returns (uint256 _ratioAoB) {
        ...
        if (_reserve0 > 0 && _reserve1 > 0) {
            if (_token0 == _tokenA) {
                if (_token1 == _tokenB) {
                    _ratioAoB = uint256(_reserve0).mul(1e18).div(uint256(_reserve1));
                }
            } else if (_token0 == _tokenB) {
                if (_token1 == _tokenA) {
                    _ratioAoB = uint256(_reserve1).mul(1e18).div(uint256(_reserve0));
                }
            }
        }
    }
