...
function mintfromCall(
    address _tokenAddress,
    address _to,
    string memory _tokenURI
) public returns(bool) {

   (bool success,) =_tokenAddress.call(abi.encodeWithSignature("safeMint(address,string memory)", _to, _tokenURI));
   if(success){
    return(true);
   }else{
    return(false);
...
function BidAccept(
        ...
        returns (bytes4)
    {
        ...
        Bid memory bid = _getBid(tokenAddress, _tokenId, bidIndex);
        ...
        _requireComposableERC721(tokenAddress, _tokenId, bid.fingerprint);

        // Check if bidder has funds
        _requireBidderBalance(bidder, price);

        // Delete bid references from contract storage
        // delete bidIndexByBidId[bid.bidId];
        delete bidsByToken[tokenAddress][_tokenId][bidIndex];
        delete bidIdByTokenAndBidder[tokenAddress][_tokenId][bidder];

        // Reset bid counter to invalidate other bids placed for the token
        delete bidCounterByToken[tokenAddress][_tokenId];

        // Transfer token to bidder
        ERC721Interface(tokenAddress).transferFrom(_from, bidder, _tokenId);
