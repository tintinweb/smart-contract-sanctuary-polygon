    ...
    function claimStakeNFT(uint256[] memory tokenIds) external {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            require(
                listing.buyer == msg.sender,
                "You are not the owner of the NFT"
            ); // Only the owner can claim and unstake
            ...
            uint256 numNFTs = 1; // Assuming 1 NFT is unstaked
            uint256 rewardAmount = numNFTs * (i + 1) * (1 ether); // Reward amount increases with the index
