...
import "./KnotToken.sol";
import "./NftCarpet.sol";
import "./Lottery.sol";

/// @title Pmkn Farm
/// @author Farhad-GH
/// @notice This contract creates a simple yield farming dApp that rewards users for
///         locking up their Medallion stablecoin with a new ERC20 token KnotToken
/// @dev The inherited KnotToken contract automatically mints PMKN when the user invokes the
///      withdrawYield function. The calculateYieldTime and calculateYieldTotal function 
///      takes care of all yield calculations. Ownership of the KnotToken contract should 
///      be transferred to the KnotFarm contract after deployment
contract KnotFarm {
    ...
    // userAddress => knotBalance
    mapping(address => uint256) public knotBalance;
    // tokenURI => nftCount
    mapping(string => uint256) public nftCount;    

    string public name = "knot Farm";

    IERC20 public medallionToken;
    KnotToken public knotToken;
    NftCarpet public nftCarpet;
    ...
    constructor(
        IERC20 _medallionToken,
        KnotToken _knotToken,
        NftCarpet _nftCarpet,
        Lottery _lottery,
        uint256 _nftPrice
        ) {
            medallionToken = _medallionToken;
            knotToken = _knotToken;
            nftCarpet = _nftCarpet;
            lottery = _lottery;
            nftPrice = _nftPrice;
    ...
    /// @notice Locks the user's Medallion within the contract
    /// @dev If the user already staked Medallion, the 
    /// @param amount Quantity of Medallion the user wishes to lock in the contract
    function stake(uint256 amount) public {
        require(
            amount > 0 &&
            medallionToken.balanceOf(msg.sender) >= amount, 
            "You cannot stake zero tokens");

        if(isStaking[msg.sender] == true){
            uint256 toTransfer = calculateYieldTotal(msg.sender);
            knotBalance[msg.sender] += toTransfer;
        }

        medallionToken.transferFrom(msg.sender, address(this), amount);
    ...
    /// @dev The yieldTransfer variable transfers the calculatedYieldTotal result to knotBalance
    ///      in order to save the user's unrealized yield
    /// @param amount The quantity of Medallion the user wishes to receive
    function unstake(uint256 amount) public {
        ...
        medallionToken.transfer(msg.sender, balTransfer);
        knotBalance[msg.sender] += yieldTransfer;
    ...
    function withdrawYield() public {
        ...
        require(
            toTransfer > 0 ||
            knotBalance[msg.sender] > 0,
            "Nothing to withdraw"
            );
            
        if(knotBalance[msg.sender] != 0){
            uint256 oldBalance = knotBalance[msg.sender];
            knotBalance[msg.sender] = 0;
            toTransfer += oldBalance;
        }

        startTime[msg.sender] = block.timestamp;
        knotToken.mint(msg.sender, toTransfer);
        emit YieldWithdraw(msg.sender, toTransfer);
    } 

    /// @notice Calls the mintItem fuction from the NftCarpet contract which safeMints
    ///         an NFT for the user
    ...
    ///      for dynamically pricing NFTs). Calls the mintItem function from NftCarpet's
    ...
    function mintNFT(address user, string memory tokenURI) public {
        require(
            knotToken.balanceOf(msg.sender) >= nftPrice, 
            "Not enough KNOT"
        );
        lottery.addToLotteryPool(msg.sender, nftPrice);
        uint256 tokenId = nftCarpet.mintItem(user, tokenURI);
        nftCount[tokenURI]++;
...
/// @notice g
/// @dev g
contract KnotToken is ERC20, AccessControl {
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    constructor() ERC20("KnotToken", "KNOT") {
...
/// @title NftCarpet
/// @author Farhad-GH
/// @notice ERC721 contract for minting NftCarpet NFTs
...
///      import used to assign the KnotFarm as NFT minter
contract NftCarpet is ERC721, ERC721Enumerable, ERC721URIStorage, AccessControl {
...
/// @title Lottery
/// @author Farhad-GH
/// @notice This contract adds a lottery feature to the KnotFarm contract that uses
///         the NFT tokenId as the "ticket." This contract uses Chainlink's VRF to 
///         achieve verifiable randomness for the winning number
/// @dev Basic iteration of a lottery feature in the KnotFarm dApp. KnotFarm's mintNFT
...
contract Lottery is Ownable, VRFConsumerBase {
    ...
    ///         number
    /// @dev Takes the totalSupply of NftCarpet NFTs as the divisor and uses Chainlink's 
    ...
    function fulfillRandomness(bytes32 _requestId, uint256 _randomness) internal override {
        uint256 totalIds = nftCarpet.getTotalSupply();
    ...
    /// @dev The KnotFarm contract primarily calls this function within its mintNFT function
    /// @param from The sender of the tx
    /// @param knot The total amount of Knot sent
    function addToLotteryPool(address from, uint256 knot) public {
        require(knot > 0, "Cannot add zero");
        lotteryPool += knot;
        knotToken.transferFrom(from, address(this), knot);
    }

    /// @notice An internal function that verifies the user's NFT tokenId matches the 
    ///         winning lottery number
    /// @dev This function fetches the amount of NFTCARPET NFTs held by the user. Then, the
    ...
    function validateWinner(
        ...
        returns(bool)
        {
        uint256 totalNfts = nftCarpet.balanceOf(user);
        uint256 winNum = winningNumber[lotteryCount - 1];
        unchecked {
            for(uint256 i; i < totalNfts; i++){
                if(nftCarpet.tokenOfOwnerByIndex(user, i) == winNum){
                    return true;
                }
            }
        }
    ...
    function claimLottoWinnings() public {
        ...
        lotteryPool = 0;
        knotToken.transfer(msg.sender, toTransfer);
