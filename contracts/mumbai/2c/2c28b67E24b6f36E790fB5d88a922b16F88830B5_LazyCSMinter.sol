...
contract LazyCSMinter is EIP712, Ownable, ReentrancyGuard {
    ...
    string private constant EIP712_DOMAIN =
        "EIP712Domain(string name,string version,uint256 chainId,address verifyingContract)";
    string private constant CSVoucher_TYPE =
        "CSVoucher(uint256 id,uint256 csAmount,uint256 maticAmount,address minter)";

    bytes32 private constant EIP712_DOMAIN_TYPEHASH =
        keccak256(abi.encodePacked(EIP712_DOMAIN));
    bytes32 private constant BID_TYPEHASH =
        keccak256(abi.encodePacked(CSVoucher_TYPE));
    ...
    bytes32 private DOMAIN_SEPARATOR =
        keccak256(
            abi.encode(
                ...
                keccak256("1"),
                80001, // replace this with your chain id
                address(this)
            )
        );
    ...
    function redeem(
        ...
        // make sure the user is paying the right amount of MATIC
        require(msg.value == voucher.maticAmount, "voucher had been used");
        ...
        csToken.claim(voucher.minter, voucher.csAmount);
        // set id used as true
        idUsed[voucher.id] = true;

        return voucher.csAmount;
    }
    ...
    function _hash(CSVoucher memory voucher) private view returns (bytes32) {
        return
            keccak256(
                abi.encodePacked(
                    "\x19\x01",
