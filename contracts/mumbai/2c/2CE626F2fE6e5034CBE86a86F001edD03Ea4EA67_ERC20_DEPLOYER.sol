...
contract ERC20_DEPLOYER is Ownable {
    ...
    function getPaginatedDeployed(uint256 _offset, uint256 _nbElements, address _valid) 
        view 
        external 
        returns (
        ...
        if (_offset >= deployed.length) {
            ...
            return (addresses, timestamps, valids);
        }
        ...
        for (uint256 i = 0; i < _nbElements; i++) {
            ...
            valids[i] = IERC20(addresses[i]).balanceOf(_valid) >= 1 ether;
        }
    }
}