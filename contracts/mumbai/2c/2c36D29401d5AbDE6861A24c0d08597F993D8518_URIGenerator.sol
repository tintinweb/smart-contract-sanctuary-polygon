...
/**
 * @author  0xFirekeeper
 * @title   ParagonsDAO Player ID Token URI Generator.
 * @notice  Constructs OpenSea-standard metadata json including Base64 SVG image.
 */

contract URIGenerator is Ownable, IURIGenerator {
    using Strings for uint256;
    using Strings for uint64;

    /// @notice Address of the AddressRegistry contract.
    ...
    /**
     * @notice  URIGenerator constructor.
     * @param   addressRegistry_  Address of the AddressRegistry contract.
     */
    constructor(IAddressRegistry addressRegistry_) {
    ...
    function constructTokenURI(
        ...
        address owner,
        IPlayerManagement.PlayerData memory data
    ) public view returns (string memory tokenUri_) {
        string memory image = Base64.encode(
            bytes(ISVGGenerator(addressRegistry.svgGenerator()).generateSVG(tokenId, owner, data))
        );
        string memory name = string.concat("ParagonsDAO Player ID #", tokenId.toString());
        uint256 maxRank = IPlayerManagement(addressRegistry.playerMgmt()).getMaxRank() + 1;
        ...
        string memory json = string.concat(
            '{',
                ...
                '"attributes": [', 
                    ...
                    ',',
                    '{"trait_type":"Level","value":', (data.playerRank+1).toString(),',"max_value":', maxRank.toString(), '}',
                    ',',
     ...
     * @param   addressRegistry_  AddressRegistry contract address.
     */
    function setAddressRegistry(IAddressRegistry addressRegistry_) external onlyOwner {
        addressRegistry = addressRegistry_;
    }
}