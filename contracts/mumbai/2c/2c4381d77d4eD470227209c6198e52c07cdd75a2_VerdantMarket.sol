...
library Pricing {
    ...
    {
        if (isPercent) {
            uint256 _commissionAmount = (amount * commission) / 10000;
            uint256 _buyerAmount = (amount * (10000 - promoCodeNft)) / 10000;
            commissionFromBuyer =
                (((_commissionAmount * (10000 - promoCodeNft)) / 10000) *
                    (10000 - promoCodeServiceFee)) /
                10000;
            ...
            uint256 _buyerAmount = amount > promoCodeNft
                ? amount - promoCodeNft
                : 0;
            uint256 _commissionAmount = (_buyerAmount * commission) / 10000;
        ...
        if (tax > 0) {
            taxAmount = (commissionFromBuyer * tax) / 10000;
            amountToBuyer += taxAmount;
        }
    }
...
library Signature {
    ...
    function encodeUpdateOrderData(
        ...
        return
            keccak256(
                abi.encode(id, _sender, _orderId, _token, _price, _expTime)
            );
    }
}
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function getOrderFloatingPrice(
        ...
        Pricing.AggregatorV3Path[]
            memory _aggregatorV3Paths = aggregatorV3Paths[order.assetName][
                order.paymentToken
            ];
    ...
    function _getOrderFloatingPrice(
        ...
        (price, paths) = Pricing.convertFloatingPrie(
            order.price,
            IERC20MetadataUpgradeable(order.paymentToken).decimals(),
            _aggregatorV3Paths,
            _RATE_DECIMALS
        );
    }
