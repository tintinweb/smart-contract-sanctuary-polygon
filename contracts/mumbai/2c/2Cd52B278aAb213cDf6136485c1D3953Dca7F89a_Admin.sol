...
contract Admin is Initializable, AccessControl, BasicMetaTransaction, ISafe {
    ...
    event BuyForwardContract(address instance, uint256 amount, address buyer);
    ...
    function setFactory(address _address)
        external
        validation(_address)
        onlyAdmin
    {
    ...
    function setRoobaWallet(address _roobaWallet)
        external
        validation(_roobaWallet)
        onlyAdmin
    {
        roobaWallet = _roobaWallet;
    }
    ...
    function tradingBuy(
        ...
        uint256 usdcAmount = ((totalPriceInGrams* 10 ** usdc.decimals()) / (tokenDecimal * 10 ** 15));
        if (usdcAmount > 0) {
    ...
    function forwardBuy(
        ...
        IForward(_forwardAddress).buyContract(_tokenAmount, _buyer);
        emit BuyForwardContract(_forwardAddress, _tokenAmount, _buyer);
    }

    function forwardClaimTokens(address _buyer, address _forwardAddress)
        external
    {
        uint256 amount = IForward(_forwardAddress).userTokenBuyAmount(_buyer);
        IForward(_forwardAddress).getTokens(_buyer);
        emit ForwardClaimTokens(_forwardAddress, _buyer, amount);
    }

    function forwardClaimUSDC(address _seller, address _forwardAddress)
        external
    {
        uint256 amount = usdc.balanceOf(_forwardAddress);
        IForward(_forwardAddress).getRaisedUSDC(_seller);
        emit ForwardClaimUSDC(_forwardAddress, _seller, amount);
    }
