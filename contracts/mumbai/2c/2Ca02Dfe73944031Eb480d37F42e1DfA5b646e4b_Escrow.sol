...
contract Escrow is ERC721Holder, ERC1155Receiver, ERC1155Holder {
    ...
    function lend(LendData memory lendData, Sign calldata sign) external {
        ...
        verifySign(generateId(lendData.lender, lendData.tokenId, lendData.lendingQuantity, lendData.nftAddress, lendData.paymentAddress, sign.nonce), msg.sender, sign);
        lendData.lendTime = block.timestamp;
        lendData.lender = msg.sender;
        lendData.lendId = generateId(lendData.lender, lendData.tokenId, lendData.lendingQuantity, lendData.nftAddress, lendData.paymentAddress, sign.nonce);
    ...
    function rent(bytes32 lendId, uint256 qty, uint256 duration, Sign calldata sign) external {
        ...
        verifySign(generateId(msg.sender, lendingDetails[lendId].tokenId, qty, lendingDetails[lendId].nftAddress, lendingDetails[lendId].lender, sign.nonce), msg.sender, sign);
        bytes32 rentId = (generateId(msg.sender, lendingDetails[lendId].tokenId, qty, lendingDetails[lendId].nftAddress, lendingDetails[lendId].lender, sign.nonce));
    ...
    function claim(bytes32 rentalId, Sign calldata sign) external {
        ...
        require(msg.sender == rentalDetails[rentalId].lender, "Invalid User");
    ...
    function generateId(address account, uint256 tokenId, uint256 qty, address nftAddress, address keyAddress, uint256 nonce) internal pure returns(bytes32 meomory){
        return keccak256(abi.encodePacked(account, tokenId, qty, nftAddress, keyAddress, nonce));
    }
    ...
    function safeTransfer(address nftAddress, address paymentAddress, address caller, address callee, uint256 tokenId, uint256 lendingAmount, uint8 nftType, uint256 amount, bool isClaim) internal {
        ...
        if(amount > 0) {
            if(isClaim) {
                ...
                require(amount >= fee + royalty, "subtraction overflow");
