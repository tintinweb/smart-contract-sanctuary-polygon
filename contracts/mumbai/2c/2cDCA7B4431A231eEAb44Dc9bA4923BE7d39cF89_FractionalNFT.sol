...
contract FractionalNFT is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable, ERC721Burnable {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;

    struct fTokenAddress{
        address fractionalToken;
    }

    mapping(uint256 => fTokenAddress) private FNFT;

    constructor() ERC721("Ruptok", "RPK") {}
    ...
    function transferFNFToken( address _to, uint256 _tokenURI, uint256 _amount) 
        onlyOwner()
        private
    ...
    function getTokenAddress(uint256 tokenId) 
        ...
        view 
        returns(address) {
        return FNFT[tokenId].fractionalToken;
    }
    ...
    function transferFrom( address from, address to, uint256 tokenId) 
        ...
        require(ERC20(tokenAddress).totalSupply() == 0, "Existing Tokens for this NFT!");
        super.transferFrom(from, to, tokenId);
    }
    
    function mintFNFT(address _to, string memory tokenURI_, uint256 _totalFractionalTokens) 
        external 
        onlyOwner() {
        ...
        // Creating an ERC20 Token Contract for this newly minted NFT.
        ...
        fTokenAddress memory tokenData;                                                          
        // tokenData.tokenId = tokenId;                           
        tokenData.fractionalToken = address(_fnftoken);

        // Bind the fractional token address to the newly minted NFT's tokenId.
        FNFT[tokenId]  = tokenData;                                    
    }
