...
contract Reserve {
    ...
    function buyPriceWithTax() public view returns (uint256) {
        ...
        return (usdcInReserve / shqOutsideReserve) - ((usdcInReserve / shqOutsideReserve) * 7) / 100; // Price in USDC (6 decimals)
    }
...
contract SheqelToken is Context, IERC20, IERC20Metadata {
    ...
    function _transfer(
        ...
        if(recipient != uniswapV2Pair/*!liqTransaction*/ && recipient != reserveAddress && sender != reserveAddress && recipient != address(uniswapV2Router) && recipient != address(0)){
            ...
            if(recipient == reserveAddress || sender == reserveAddress){
