...
contract Treasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SXDAO }
    ...
    address public sXDAO;
    uint public sXDAOQueue; // Delays change to sXDAO address
    ...
    /**
        @notice allow approved address to deposit an asset for XDAO
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint XDAO needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( XDAO ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn XDAO for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IXDAOERC20( XDAO ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sXDAO ).balanceOf( msg.sender ); // Can only borrow against sXDAO held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with XDAO
        @param _amount uint
     */
    function repayDebtWithXDAO( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IXDAOERC20( XDAO ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, XDAO, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( XDAO ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( XDAO ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns XDAO valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match XDAO decimals
            value_ = _amount.mul( 10 ** IERC20( XDAO ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SXDAO ) { // 9
            sXDAOQueue = block.number.add( blocksNeededForQueue );
        } else return false;
