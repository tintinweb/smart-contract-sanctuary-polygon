...
contract WrongRecoveryQuestionPresentation is IPresentation {
        ...
        returns (string memory)
    {
        bytes memory svg = abi.encodePacked(
            ...
            _getFontSource(),
            "format('woff2');font-weight:bold;font-style:normal;}tspan{fill:#BC3F3C;font-family:'j',monospace;font-size:20px;}</style><title>invalid</title><rect width='100%' height='100%' fill='black'/><text><tspan x='58' y='410'>FUNCTION throwWrongRecoveryQuestionError</tspan><!-- not a real stack --><tspan x='82' dy='1em'>PRINT \""
        );

        svg = abi.encodePacked(
            svg,
            _addHyphenToDate(day),
            " 00:02:28 error stack dump\"</tspan><tspan x='82' dy='1em'>PRINT \"\u00A0\u00A0xyz.lifeofajacky.program EXCEPTION: metadata\"</tspan><tspan x='82' dy='1em'>PRINT \"\u00A0\u00A0InvalidDataException: same password recovery question as last </tspan><tspan x='82' dy='1em'>\u00A0\u00A0month\"</tspan><tspan x='82' dy='1em'>PRINT \"\u00A0\u00A0\u00A0\u00A0at xyz.lifeofajacky.program.extractPasswordRecoveryQuestion:</tspan><tspan x='82' dy='1em'>\u00A0\u00A03\"</tspan><tspan x='82' dy='1em'>PRINT \"\u00A0\u00A0\u00A0\u00A0at xyz.lifeofajacky.program.getTargetDayEntry:99\"</tspan><tspan x='82' dy='1em'>PRINT \"\u00A0\u00A0\u00A0\u00A0at xyz.lifeofajacky.program.constructMetadata:151\"</tspan><tspan x='82' dy='1em'>PRINT \"\u00A0\u00A0\u00A0\u00A0at xyz.lifeofajacky.program.mintADay:642\"</tspan><tspan x='82' dy='1em'>PRINT \"\u00A0\u00A0\u00A0\u00A0at xyz.lifeofajacky.program.mintDay20220301:2\"</tspan></text></svg>"
        );
    ...
    function _metadataName() internal pure returns (string memory) {
        return "LifeOfAJacky: invalid tokenId";
    }

    function _addHyphenToDate(string memory day)
        internal
        pure
        returns (string memory)
    {
        bytes memory dayStrBytes = bytes(day);

        if (dayStrBytes.length != 8) {
            return day;
        }

        bytes memory resultDay = new bytes(10);
        resultDay[0] = dayStrBytes[0];
        resultDay[1] = dayStrBytes[1];
        resultDay[2] = dayStrBytes[2];
        resultDay[3] = dayStrBytes[3];
        resultDay[4] = "-";
        resultDay[5] = dayStrBytes[4];
        resultDay[6] = dayStrBytes[5];
        resultDay[7] = "-";
        resultDay[8] = dayStrBytes[0];
        resultDay[9] = dayStrBytes[0];

        return string(resultDay);
    }
}
