...
contract WooTemplate is ERC721, ERC721URIStorage, Ownable {
    ...
    event StatusChanged(uint256 tokenId, Status status);

    constructor() ERC721("WooTemplate", "WOO") {

    }

    // string memory uri 后续改为 bytes calldata , 
    ...
    function update(uint256 tokenId, string memory uri) public onlyOwner {
        require(_exists(tokenId), "WooSBT: nonexist tokenId");
        _setTokenURI(tokenId, uri);        
    ...
    function activate(uint256 tokenId) external onlyOwner {
        require(
            tokenStatus[tokenId] == Status.SUSPENDED ||
                tokenStatus[tokenId] == Status.ARCHIVED,
            "WooSBT: The SBT is not suspended or archived now."
        );
    ...
    function suspend(uint256 tokenId) external onlyOwner {
        require(
            tokenStatus[tokenId] == Status.ACTIVE,
            "WooSBT: The SBT is not activating now."
        );

        tokenStatus[tokenId] = Status.SUSPENDED;
        emit StatusChanged(tokenId, Status.SUSPENDED);
    }
    ...
    function safeTransferFrom(
        ...
        address to,
        uint256 tokenId
    ) public override(ERC721) onlyOwner {
    ...
    function _transferToken(
        ...
        require(
            ...
            "WooSBT: `from` address has no token."
        );
 
        require(
            tokenStatus[tokenId] == Status.SUSPENDED,
            "WooSBT: The Active or archived token cannot be transfer."
        );
