...
contract AaveV2V3Adapter is CustodianStorage, ERC165, IAdapter {
    ...
    constructor() {
        // @dev Dummy initialisation of storage as this contract is only used as implementation by `PoolCustodian` contract
        // Change is made in order to silence Slither high level warning
        yieldProvider = address(0);
    ...
    function deposit(uint256 amount) external returns (bool success) {
        ...
        token.safeApprove(yieldProvider, amount);
    ...
    function withdraw(uint256 amount) external returns (bool success) {
        uint256 availableAmount = (yieldProviderBalance * lastYieldFactor) / RAY;

        bool imprecisionDetected = amount > availableAmount;
        if (imprecisionDetected) {
            /**
             * @dev Situtation where imprecision has been detected
             * It has been identified to happen when no rewards have been generated between deposits and withdraw
             * The amount to withdraw may in that case be larger than the computed available balance or what is available on Aave
             * The following patch solution is currently set in place but should be discussed during the audit
             */
            return false;
        }
    ...
    function _empty() private returns (bool success, uint256) {
        yieldProviderBalance = 0;
        uint256 withdrawnAmount = IAaveLendingPool(yieldProvider).withdraw(
            address(token),
            type(uint256).max,
            address(this)
        );
...
contract NeutralAdapter is CustodianStorage, ERC165, IAdapter {
     ...
     * @inheritdoc IAdapter
     */
    function empty() external view returns (bool success, uint256 withdrawnAmount) {
        return (true, depositedBalance);
    }
...
contract PoolCustodian is ERC165, IPoolCustodian, CustodianStorage {
    ...
    constructor(
        ...
        if (!ERC165(address(_rolesManager)).supportsInterface(type(IRolesManager).interfaceId)) revert WRONG_INPUT();
        if (address(_token) == address(0)) revert ZERO_ADDRESS_NOT_ALLOWED();
    ...
    function withdraw(uint256 amount, address to) public onlyPool {
        ...
        emit Withdrawn(amount, to, adapter, yieldProvider);
    }
    ...
    function updateRolesManager(address _rolesManager) public onlyGovernance {
        if (!ERC165(_rolesManager).supportsInterface(type(IRolesManager).interfaceId)) revert WRONG_INPUT();
    ...
    function initializePool(address _pool) public onlyGovernance {
        ...
        if (_pool == address(0)) revert ZERO_ADDRESS_NOT_ALLOWED();
    ...
    function updateRewardsOperator(address _rewardsOperator) public onlyGovernance {
        if (_rewardsOperator == address(0)) revert ZERO_ADDRESS_NOT_ALLOWED();
    ...
    function _deposit(uint256 amount, address from) private {
        ...
        emit Deposited(amount, from, adapter, yieldProvider);
    }
}
...
contract PoolTokenFeesController is IFeesController, Managed {
    ...
    uint256 public ONE; // precision of the fees rates
    ...
    /**
     * @inheritdoc IFeesController
     * @dev Management fees are rounded up to prevent underflows when withdrawing
     */
    function registerManagementFees(uint256 amount) external onlyPool returns (uint256 fees) {
        fees = amount.mulUp(MANAGEMENT_FEES_RATE);
        dueFees += fees;
        totalFees += fees;
    ...
    /**
     * @inheritdoc IFeesController
     * @dev Exit fees are rounded up to prevent underflows when withdrawing
     */
    ...
    /**
     * @inheritdoc IFeesController
     * @dev Borrowing fees are rounded down to maximize amounts within the protocol and prevent underflows when withdrawing
     */
    function registerBorrowingFees(uint256 amount) external onlyPool returns (uint256 fees) {
        fees = amount.mulDown(BORROWING_FEES_RATE);
        dueFees += fees;
        totalFees += fees;
    ...
    /**
     * @inheritdoc IFeesController
     * @dev Repayment fees are rounded down to maximize amounts within the protocol and prevent underflows when withdrawing
     */
    function registerRepaymentFees(uint256 amount) external onlyPool returns (uint256 fees) {
        fees = amount.mulDown(REPAYMENT_FEES_RATE);
        dueFees += fees;
        totalFees += fees;
...
abstract contract BaseNonStandardRepaymentModule is INonStandardRepaymentModule, Managed, ERC721 {
    ...
    ModuleStatus public status = ModuleStatus.INACTIVE;
    bytes32 public root;
    ...
    constructor(
        ...
        address _pool
    ) ERC721(name, symbol) Managed(IPool(_pool).getRolesManager()) {
        POOL = IPool(_pool);
        IPoolCustodian custodian = IPoolCustodian(POOL.CUSTODIAN());
        (address token, ) = custodian.getTokenConfiguration();
        TOKEN = ERC20(token);
    }
    ...
    /**
     * @inheritdoc INonStandardRepaymentModule
     */
    function initialize(uint256 amount) public {
        if (status != ModuleStatus.INACTIVE) revert NSR_WRONG_PHASE();
        if (msg.sender != address(POOL)) revert NSR_UNAUTHORISED_INITIALIZATION();
    ...
    function initiateEarlyRepay(uint256 amount) public onlyBorrower {
        ...
        if (status != ModuleStatus.INITIALIZED) revert NSR_WRONG_PHASE();
        if (amount < POOL.totalBorrowed() + POOL.getCurrentAccruals()) revert NSR_AMOUNT_TOO_LOW_EARLY_REPAY();
    ...
    function initiateRepay(uint256 amount) public onlyBorrower {
        if (status != ModuleStatus.INITIALIZED) revert NSR_WRONG_PHASE();
        if (amount == 0) revert NSR_REPAY_ZERO();
        ...
        if (amount < POOL.totalBorrowed() + POOL.getCurrentAccruals()) {
...
interface INonStandardRepaymentModule {
    ...
    /**
     * @notice Emitted when the root of the compensation merkle tree is set
    ...
    error NSR_UNAUTHORISED_INITIALIZATION(); // Initialization must be done by the pool the NSR was deployed for
    ...
    error NSR_ROOT_NOT_SET(); // The root of the compensation merkle tree is not set
    error NSR_PROOF_VERIFICATION_FAIL(); // The verification of the withdraw compensation merkle proof failed
    error NSR_NOT_IMPLEMENTED(); // The method is not implemented
    error NSR_REPAY_ZERO(); // Cannot repay zero amount
    ...
    /**
     * @notice Withdraw compensation in exchange for position token
     * @param positionId The id of the position to withdraw
     */
    function withdraw(uint256 positionId) external;

    /**
     * @notice Withdraw off chain calculated compensation in exchange for position token
     * @dev The withdrawal process being finalized with the transfer of the position, it will fail if the caller
     * does not hold the target position
     * @param positionId The id of the position to withdraw
     * @param compensationAmount Amount of the compensation to be withdrawn
     * @param proof Merkle proof that the target position is associated with the right compensation amount
     */
    function withdraw(
        uint256 positionId,
    ...
    /**
     * @notice Set the root of the compensation merkle tree
...
/**
 * @title ManagedNonStandardRepaymentModule
 * @author Atlendis Labs
 * Contract handling the repayment cases not supported by the pool
 *
 * Extension of {BaseNonStandardRepaymentModule} implementing the {withdraw} function
 * Withdraw distributes compensation according to a merkle tree root set by governance
 * The tree leaf should be set as keccak256(abi.encode(positionId, compensationAmount)) and calculated offchain by governance
 */
contract ManagedNonStandardRepaymentModule is BaseNonStandardRepaymentModule {
    using FixedPointMathLib for uint256;
    using SafeERC20 for ERC20;
    ...
    constructor(
        ...
        string memory symbol,
        address _pool
    ) BaseNonStandardRepaymentModule(name, symbol, _pool) {}

    /*//////////////////////////////////////////////////////////////
                          ROOT SETTING
    //////////////////////////////////////////////////////////////*/

    function setRoot(bytes32 _root) public override onlyGovernance {
        if (status != ModuleStatus.ACTIVE) revert NSR_WRONG_PHASE();
    ...
    function withdraw(
        ...
        (uint256 withdrawable, ) = POOL.getPositionRepartition(positionId);
        uint256 totalWithdrawnAmount = withdrawable;

        if (repaidAmount != 0) {
            if (root == 0) revert NSR_ROOT_NOT_SET();
            bytes32 leaf = keccak256(abi.encode(positionId, compensationAmount));
            if (!MerkleProof.verify(proof, root, leaf)) revert NSR_PROOF_VERIFICATION_FAIL();
            totalWithdrawnAmount += compensationAmount;
        }
        ...
        if (positionCurrentValue > totalWithdrawnAmount) {
            debts[positionId] = positionCurrentValue - totalWithdrawnAmount;
            _safeMint(msg.sender, positionId);
        }

        // exchange pool positions for token compensation
        POOL.transferFrom(msg.sender, address(this), positionId);
        TOKEN.safeTransfer(msg.sender, totalWithdrawnAmount);

        emit Withdrawn(positionId, positionCurrentValue, totalWithdrawnAmount);
    }
}
 ...
 *         The way to retrieve the rewards is specific for each module type.
 */
interface IRewardsModule is IERC165 {
    /**
     * @notice Thrown when the sender is not the expected one
     * @param actualAddress Address of the sender
     * @param expectedAddress Expected address
...
abstract contract RewardsModule is IRewardsModule, Managed {
    ...
    function supportsInterface(bytes4 interfaceId) public pure override returns (bool) {
        return interfaceId == type(IRewardsModule).interfaceId;
    }
...
abstract contract RewardsManager is IRewardsManager, ERC721, Managed {
    ...
    function addModule(address module, bool asContinuous) public onlyGovernance onlyEnabled {
        if (!IERC165(module).supportsInterface(type(IRewardsModule).interfaceId)) revert WRONG_INPUT();
...
interface IManaged {
    /**
     * @notice Thrown when the input contract does not support the right interface id
     */
    error WRONG_INPUT();

    /**
     * @notice Thrown when a zero address is set as input
     */
...
abstract contract Managed is IManaged {
    ...
    constructor(address _rolesManager) {
        if (!IERC165(_rolesManager).supportsInterface(type(IRolesManager).interfaceId)) revert WRONG_INPUT();
        rolesManager = IRolesManager(_rolesManager);
    }
...
library FixedPointMathLib {
    ...
    /**
     * @notice Access the library internal storage
     */
    ...
    /**
     * @notice Set the denominator in storage
     */
    ...
    /**
     * @notice Multiply with the desired denominator
     * @dev Rounds the result down by default
     */
    function mul(
        uint256 x,
        uint256 y,
        ...
        return mulDown(x, y, denominator);
    }

    /**
     * @notice Multiply with the library denominator
     * @dev Rounds the result down by default
     */
    ...
    /**
     * @notice Multiply with the desired denominator and round the result down
     */
    function mulDown(
        uint256 x,
        uint256 y,
    ...
    /**
     * @notice Multiply with the default denominator and round the result down
     */
    function mulDown(uint256 x, uint256 y) internal view returns (uint256) {
        return x.mulDivDown(y, libStorage().denominator);
    }

    /**
     * @notice Multiply with the desired denominator and round the result up
     */
    function mulUp(
        uint256 x,
        uint256 y,
    ...
    /**
     * @notice Multiply with the default denominator and round the result up
     */
    function mulUp(uint256 x, uint256 y) internal view returns (uint256) {
        return x.mulDivUp(y, libStorage().denominator);
    }

    /**
     * @notice Divide with the desired denominator
     * @dev Rounds the result down by default
     */
    function div(
        uint256 x,
        ...
        return divDown(x, y, denominator);
    }

    /**
     * @notice Divide with the default denominator
     * @dev Rounds the result down by default
     */
    ...
    /**
     * @notice Divide with the desired denominator and round the result down
     */
    function divDown(
        uint256 x,
        uint256 y,
    ...
    /**
     * @notice Divide with the default denominator and round the result down
     */
    function divDown(uint256 x, uint256 y) internal view returns (uint256) {
        return x.mulDivDown(libStorage().denominator, y);
    }

    /**
     * @notice Divide with the desired denominator and round the result up
     */
    function divUp(
        uint256 x,
        uint256 y,
        ...
        return x.mulDivUp(denominator, y);
    }

    /**
     * @notice Divide with the default denominator and round the result up
     */
    function divUp(uint256 x, uint256 y) internal view returns (uint256) {
        return x.mulDivUp(libStorage().denominator, y);
    }
}
...
contract Bullet is LoanBase {
    ...
    function getPositionCurrentValue(uint256 positionId) external view override returns (uint256 positionCurrentValue) {
        /// @dev checks if position exists
        ownerOf(positionId);
        (uint256 withdrawable, uint256 borrowedAmount) = getPositionRepartition(positionId);
        ...
        positionCurrentValue = withdrawable + currentBorrowedAmount;
    }
    ...
    function getPositionRepartition(uint256 positionId)
        ...
        view
        override
        returns (uint256 withdrawable, uint256 borrowedAmount)
    {
        /// @dev checks if position exists
        ownerOf(positionId);
        (withdrawable, borrowedAmount) = LoanLogic.getPositionRepartition(
            ticks[positions[positionId].rate],
            poolPhase,
            positions[positionId].depositedAmount,
            positions[positionId].hasPartiallyWithdrawn
        );
    }
    ...
    function getCurrentAccruals() external view override returns (uint256 currentAccruals) {
        if (poolPhase == LoanTypes.PoolPhase.NON_STANDARD || poolPhase == LoanTypes.PoolPhase.ISSUED) {
            uint256 loanMaturity = borrowTimestamp + LOAN_DURATION;
            uint256 referenceTimestamp = timelock.executedAt > 0 ? timelock.executedAt : block.timestamp;
            uint256 timeUntilMaturity;
            if (loanMaturity > referenceTimestamp) {
                timeUntilMaturity = loanMaturity - referenceTimestamp;
            }
            ...
            while (currentInterestRate <= MAX_RATE) {
                ...
                currentAccruals += (tick.borrowedAmount.div(
                    TimeValue.getDiscountFactor(currentInterestRate, LOAN_DURATION - timeUntilMaturity, ONE)
                ) - tick.borrowedAmount);
                if (referenceTimestamp > loanMaturity) {
                    currentAccruals += _getLateRepaymentAccruals(
                        tick.borrowedAmount,
    ...
    ) internal override returns (uint256 amountToRepayForTick, uint256 tickFees) {
    ...
    function _getLateRepaymentAccruals(uint256 borrowedAmount, uint256 timeDeltaLateRepay)
        internal
        view
        override
        returns (uint256 lateRepaymentAccruals)
    {
        lateRepaymentAccruals =
            borrowedAmount.div(TimeValue.getDiscountFactor(LATE_REPAYMENT_FEE_RATE, timeDeltaLateRepay, ONE)) -
            borrowedAmount;
    }
}
 ...
 * @author Atlendis Labs
 */
contract CouponBullet is LoanBase, IPayableLoan {
    using FixedPointMathLib for uint256;
     ...
     */
    function getPositionCurrentValue(uint256 positionId) external view override returns (uint256 positionCurrentValue) {
        return _getPayableLoanPositionCurrentValue(positionId);
    }
        ...
        returns (uint256 withdrawable, uint256 borrowedAmount)
    {
        /// @dev checks if position exists
        ownerOf(positionId);
        (withdrawable, borrowedAmount) = LoanLogic.getPayableLoanPositionRepartition(
            ticks[positions[positionId].rate],
     ...
     * @inheritdoc ILoanBorrower
     */
    function repay() external onlyBorrower onlyInPhase(LoanTypes.PoolPhase.ISSUED) {
        _repay();
        _makePaymentsDue({finalPayment: true});
    }
    ...
    function calculateRepayAmount(
        ...
        uint256 timeDeltaIntoLateRepay,
        uint256 lateRepaymentRate
    ) internal view override returns (uint256 amountToRepayForTick, uint256 tickFees) {
        if (timeDeltaIntoLateRepay > 0) {
    ...
    function _getLateRepaymentAccruals(uint256 borrowedAmount, uint256 timeDeltaLateRepay)
        ...
        returns (uint256 lateRepaymentAccruals)
    {
        lateRepaymentAccruals = (borrowedAmount * timeDeltaLateRepay).mul(LATE_REPAYMENT_FEE_RATE).div(365 days * ONE);
    }

    /**
     * @inheritdoc IPayableLoan
     */
    function makePaymentsDue() external onlyBorrower onlyInPhase(LoanTypes.PoolPhase.ISSUED) {
        _makePaymentsDue({finalPayment: false});
    }
 ...
 * @author Atlendis Labs
 * @notice Library created in order to isolate Coupon Bullet deployment for contract size reason
 */
...
contract InstallmentLoan is LoanBase, IPayableLoan {
     ...
     * @inheritdoc ILoan
     */
    function totalBorrowed() external view override returns (uint256 totalCurrentBorrowed) {
        return totalBorrowedStatic - principalRepaid;
    }
     ...
     * @inheritdoc IPayableLoan
     */
    /// @notice Can only be used to make due installment payments if the last installment payment is not due
    function makePaymentsDue() external onlyBorrower onlyInPhase(LoanTypes.PoolPhase.ISSUED) {
        uint256 paymentsToMake = _getNumberOfPaymentsExpired() - paymentsDoneCount;
        principalRepaid += (totalBorrowedStatic * paymentsToMake).div(totalPaymentsCount * ONE);
        _makePaymentsDue({finalPayment: false});
    }
     ...
     * @inheritdoc ILoanBorrower
     */
    /// @notice Is the final repay and can only be used to make due installments if the final installment payment is due
    function repay() external onlyBorrower onlyInPhase(LoanTypes.PoolPhase.ISSUED) {
        /// @dev repay needs to happen before installment payment so that paymentsDone does not get distorted
        _repay();
        _makePaymentsDue({finalPayment: true});
        principalRepaid = totalBorrowedStatic;
    }
    ...
    function calculateRepayAmount(
        ...
        if (timeDeltaIntoLateRepay == 0) return (0, 0);
        uint256 paymentsToMake = _getNumberOfPaymentsExpired() - paymentsDoneCount;

        amountToRepayForTick = ((ticks[rate].singlePaymentAmount - ticks[rate].singlePaymentEarnings) *
            paymentsToMake *
            timeDeltaIntoLateRepay).mul(lateRepaymentRate).div(365 days * ONE);
        tickFees = 0;
    }
    ...
    function _getLateRepaymentAccruals(uint256 borrowedAmount, uint256 timeDeltaLateRepay)
        ...
        returns (uint256 lateRepaymentAccruals)
    {
        lateRepaymentAccruals = ((((totalPaymentsCount - paymentsDoneCount) * borrowedAmount) / totalPaymentsCount) *
            timeDeltaLateRepay).mul(LATE_REPAYMENT_FEE_RATE).div(365 days * ONE);
    }
...
interface ILoan {
    ...
    /**
     * @notice Retrieve the repartition between borrowed amount and withdrawable amount of the position
     * @param positionId ID of the position
     * @return withdrawable Amount that is not currently borrowed, and can be withdrawn
    ...
    /**
     * @notice Retrieve the current overall value of the position, including both borrowed and withdrawable amounts
     * @param positionId ID of the position
    ...
    /**
     * @notice Returns the total amount of principal borrowed
     * @return totalCurrentBorrowed Total amount borrowed
     */
    function totalBorrowed() external returns (uint256 totalCurrentBorrowed);
}
...
interface ILoanGovernance is ITimelock {
     ...
     * @param recipient Recipient address of the withdrawable funds
     * @param delay Timelock delay
     */
     ...
     * - Sends the withdrawable funds to the non standard repayment procedure contract
     ...
     * - Stops all native actions possible in the pool
     * - Sends the withdrawable funds to a recipient address
...
interface ILoanState {
    ...
    /**
     * @notice Retrieve the total payment count
     * @return totalPaymentsCount The total number of payments
     */
    function totalPaymentsCount() external view returns (uint256 totalPaymentsCount);
}
...
library LoanErrors {
    ...
    error LOAN_WITHDRAWAL_NOT_AVAILABLE(); // "Withdrawal not possible"
    error LOAN_NO_LIQUIDITY(); // "Borrowing from an empty pool is not allowed"
    ...
    error LOAN_NO_PAYMENTS_DUE(); // "No payment is due"
    ...
    error LOAN_NO_FINAL_PAYMENT(); // "Cannot make final payment"
    error LOAN_ONLY_FINAL_PAYMENT(); // "Can only make final payment/payments"
    error LOAN_NO_CANCELLATION_FEE_TO_WITHDRAW(); // "No cancellation fee can be withdrawn"
...
library LoanLogic {
    ...
    /**
     * @notice Gets the number of payments that are due and not yet paid
    ...
    /**
     * @notice Gets the payment rate of a single payment
    ...
    /**
     * @notice Distributes escrowed cancellation fee to tick
     * @param tick Target tick
     ...
     * @param redistributeCancelFee Distribution of cancellation fee to LP activation
     * @return cancelFeeForTick Cancel fee amount to be paid for a given tick
     */
    ...
    /**
     * @notice Deposits amount to tick
     * @param tick Target tick
    ...
    /**
     * @notice Transfer an amount from one tick to another
    ...
    /**
     * @notice Register borrowed amount in tick
     * @param tick Target tick
    ...
    /**
     * @notice Register amounts for a single payment in the target tick
     * @param tick Target tick
    ...
    /**
     * @notice Derive the allowed payments amount to be withdrawn
     * @param tick Target tick
     * @param position Position conducting the withdrawal
     * @param paymentsDoneCount Amount of payments that have been made
     * @return totalWithdrawablePayments Full amount of payments made that is withdrawable
     * @return earningsSubjectToFees The portion of the withdrawable payments made that represents interest rate paid
     */
    function getWithdrawablePayments(
        LoanTypes.Tick storage tick,
        LoanTypes.Position storage position,
        uint256 paymentsDoneCount
    ) internal view returns (uint256 totalWithdrawablePayments, uint256 earningsSubjectToFees) {
        uint256 paymentsAvailableToWithdraw = paymentsDoneCount - position.numberOfPaymentsWithdrawn;
        if (paymentsAvailableToWithdraw > 0) {
            totalWithdrawablePayments = (paymentsAvailableToWithdraw * position.depositedAmount)
                .mul(tick.singlePaymentAmount)
                .div(tick.depositedAmount);
            earningsSubjectToFees = (paymentsAvailableToWithdraw * position.depositedAmount)
                .mul(tick.singlePaymentEarnings)
                .div(tick.depositedAmount);
        }
    }

    /**
     * @notice Get the position repartition without payments
     * @param tick Target tick
     * @param poolPhase Current pool phase
     * @param depositedAmount Amount deposited in the position
     * @param hasPartiallyWithdrawn Determines if a withdrawal of unborrowed amounts already occured
     * @return withdrawable Amount that is available to withdraw
     * @return borrowedAmount Amount that is currently borrowed
     */
    function getPositionRepartition(
        ...
        uint256 depositedAmount,
        bool hasPartiallyWithdrawn
    ) internal view returns (uint256, uint256) {
        uint256 borrowedAmount;
        uint256 withdrawable;
        if (poolPhase == LoanTypes.PoolPhase.BOOK_BUILDING) {
            withdrawable = getPositionRepartitionBookBuildingPhase(depositedAmount);
        }
        if (poolPhase == LoanTypes.PoolPhase.ISSUED) {
            (withdrawable, borrowedAmount) = getPositionRepartitionNonStandardPhaseOrIssuedPhase(
                tick,
                depositedAmount,
                hasPartiallyWithdrawn
            );
        }
        if (poolPhase == LoanTypes.PoolPhase.CANCELLED) {
            withdrawable = getPositionRepartitionCancelledPhase(tick, depositedAmount);
        }
        if (poolPhase == LoanTypes.PoolPhase.REPAID) {
            withdrawable = getPositionRepartitionRepaidPhase(tick, depositedAmount, hasPartiallyWithdrawn);
        }

        if (poolPhase == LoanTypes.PoolPhase.NON_STANDARD) {
            (withdrawable, borrowedAmount) = getPositionRepartitionNonStandardPhaseOrIssuedPhase(
                tick,
                depositedAmount,
                hasPartiallyWithdrawn
        ...
        return (withdrawable, borrowedAmount);
    }

    /**
     * @notice Gets the position reparition during the book building phase
     * @param depositedAmount Amount that was deposited in the position
     * @return withdrawable Amount that is available to withdraw
     */
    function getPositionRepartitionBookBuildingPhase(uint256 depositedAmount)
        internal
        pure
        returns (uint256 withdrawable)
    {
        withdrawable = depositedAmount;
    }

    /**
     * @notice Gets the position reparition during the cancelled phase
     * @param tick Target tick
     * @param depositedAmount Amount deposited in the position
     * @return withdrawable Amount that is available to withdraw
     */
    function getPositionRepartitionCancelledPhase(LoanTypes.Tick storage tick, uint256 depositedAmount)
        internal
        view
        returns (uint256 withdrawable)
    {
        withdrawable = depositedAmount.mul(tick.repaidAmount).div(tick.depositedAmount);
    }

    /**
     * @notice Gets the position reparition during the issued and non standard phase
     * @param tick Target tick
     * @param depositedAmount Amount deposited in the position
     * @param hasPartiallyWithdrawn Determines if a withdrawal of unborrowed amounts already occured
     * @return withdrawable Amount that is available to withdraw
     * @return borrowedAmount Amount that is currently borrowed
     */
    function getPositionRepartitionNonStandardPhaseOrIssuedPhase(
        LoanTypes.Tick storage tick,
        uint256 depositedAmount,
        bool hasPartiallyWithdrawn
    ) internal view returns (uint256 withdrawable, uint256 borrowedAmount) {
        if (tick.borrowedAmount == 0) {
            withdrawable = depositedAmount;
        }

        // partially matched tick
        if (tick.borrowedAmount > 0 && tick.borrowedAmount < tick.depositedAmount) {
            uint256 notMatched = depositedAmount.mul(tick.depositedAmount - tick.borrowedAmount).div(
                tick.depositedAmount
            );
            withdrawable = hasPartiallyWithdrawn ? 0 : notMatched;
            borrowedAmount = depositedAmount - notMatched;
        }
        // fully matched tick
    ...
    /**
     * @notice Gets the position reparition during the repaid phase
     * @param tick Target tick
     * @param depositedAmount Amount deposited in the position
     * @param hasPartiallyWithdrawn Determines if a withdrawal of unborrowed amounts already occured
     * @return withdrawable Amount that is available to withdraw
     */
    function getPositionRepartitionRepaidPhase(
        LoanTypes.Tick storage tick,
        uint256 depositedAmount,
        bool hasPartiallyWithdrawn
        ...
        if (tick.borrowedAmount > 0) {
            withdrawable = depositedAmount.mul(tick.repaidAmount).div(tick.depositedAmount);
            if (!hasPartiallyWithdrawn) {
                withdrawable += depositedAmount.mul(tick.depositedAmount - tick.borrowedAmount).div(
                    tick.depositedAmount
                );
            }
        }
    ...
    /**
     * @notice Getter to retrieve the current available amount to withdraw (payments made and unborrowed amounts) and the borrowed amount
     * @param tick Target tick
     * @param position Relevant position
     * @param poolPhase Current pool phase of the pool
     * @param paymentsDoneCount Amount of payments that have been made
     */
    function getPayableLoanPositionRepartition(
        LoanTypes.Tick storage tick,
        LoanTypes.Position storage position,
        LoanTypes.PoolPhase poolPhase,
        uint256 paymentsDoneCount
    ) internal view returns (uint256 withdrawable, uint256 borrowedAmount) {
        (withdrawable, borrowedAmount) = getPositionRepartition(
            tick,
            poolPhase,
            position.depositedAmount,
            position.hasPartiallyWithdrawn
        );

        uint256 paymentsAvailableToWithdraw = paymentsDoneCount - position.numberOfPaymentsWithdrawn;
        withdrawable += position.depositedAmount.mul(paymentsAvailableToWithdraw * tick.singlePaymentAmount).div(
            tick.depositedAmount
        );

        ///@dev earnings and payment are not equal for installment loans. In case of installment loans we deduct principal from borrowed amount
        if (tick.singlePaymentAmount != tick.singlePaymentEarnings && poolPhase != LoanTypes.PoolPhase.REPAID) {
            borrowedAmount -= position
                .depositedAmount
                .mul((tick.singlePaymentAmount - tick.singlePaymentEarnings) * paymentsDoneCount)
                .div(tick.depositedAmount);
        }
    }
}
...
library LoanValidation {
    ...
    /**
     * @notice Thrown when min deposit amount is set as zero
     */
    error INVALID_ZERO_MIN_DEPOSIT_AMOUNT();
    ...
    /**
     * @notice Validation of the parameters used for the pool deployment
     * @dev Parameters are packed in bytes to improve readability and prevent stack too deep errors
     * @param ratesAmountsConfig Parameters related to rates and amounts configuration for the pool
     * @param durationsConfig Parameters related to durations configuration for the pool
     * @param decimals Precision of the underlying token
     */
    function validateDeployment(
        ...
        if (ratesAmountsConfig.length != 256) revert INVALID_PRODUCT_PARAMS();
        if (durationsConfig.length != 160) revert INVALID_PRODUCT_PARAMS();
        ...
        if (minDepositAmount == 0) revert INVALID_ZERO_MIN_DEPOSIT_AMOUNT();

        if ((paymentPeriod > 0 && (loanDuration % paymentPeriod != 0)) || paymentPeriod > loanDuration)
            revert PAYMENT_PERIOD();
...
abstract contract LoanBase is ILoan, LoanState, LoanGovernance, LoanLender, LoanBorrower {
     ...
     * @inheritdoc ILoan
     */
    function totalBorrowed() external view virtual returns (uint256 totalCurrentBorrowed) {
        return totalBorrowedStatic;
    }
        ...
        if (poolPhase == LoanTypes.PoolPhase.NON_STANDARD || poolPhase == LoanTypes.PoolPhase.ISSUED) {
            ...
            while (currentInterestRate <= MAX_RATE) {
                ...
                if (tick.borrowedAmount > 0) {
                    ...
                    if (referenceTimestamp > (borrowTimestamp + LOAN_DURATION)) {
                        lateRepayEarnings += _getLateRepaymentAccruals(
                            tick.borrowedAmount,
                            referenceTimestamp - (borrowTimestamp + LOAN_DURATION)
                        );
                    }
                }
            ...
            currentAccruals = totalMissedEarnings + totalRunningEarnings + lateRepayEarnings;
        }
    }
     ...
     * @return positionCurrentValue Current value of the position
     */
    function _getPayableLoanPositionCurrentValue(uint256 positionId)
        internal
        view
        returns (uint256 positionCurrentValue)
    {
        ...
        uint256 timeIntoCurrentPayment = paymentsDoneCount == totalPaymentsCount ||
            ...
            ? 0
            : (referenceTimestamp - borrowTimestamp) - numberOfPaymentsExpired * PAYMENT_PERIOD;
        uint256 singlePaymentEarnings = positions[positionId]
            .depositedAmount
            .mul(ticks[positions[positionId].rate].singlePaymentEarnings)
            .div(ticks[positions[positionId].rate].depositedAmount);
        uint256 paymentsAmountDue = singlePaymentEarnings * (numberOfPaymentsExpired - paymentsDoneCount);
        uint256 runningPaymentsDue = (singlePaymentEarnings * timeIntoCurrentPayment * ONE) / PAYMENT_PERIOD / ONE;
        positionCurrentValue = withdrawable + borrowedAmount + paymentsAmountDue + runningPaymentsDue;
    }
}
...
abstract contract LoanFactoryBase is IProductFactory {
    ...
    constructor(address factoryRegistry) {
        if (factoryRegistry == address(0)) revert ZERO_ADDRESS_NOT_ALLOWED();
    ...
    function deploy(
        ...
        instance = deployPool(address(rolesManager), custodian, feesController);
...
abstract contract LoanBorrower is LoanState, ILoanBorrower {
    ...
    function enableBookBuildingPhase() external onlyBorrower onlyInPhase(LoanTypes.PoolPhase.INACTIVE) {
        ...
        BOOK_BUILDING_PHASE_START_TIMESTAMP = block.timestamp;
    ...
    function withdrawRemainingEscrow(address to) external onlyBorrower onlyInPhase(LoanTypes.PoolPhase.CANCELLED) {
        if (cancellationFeeEscrow == 0) revert LoanErrors.LOAN_NO_CANCELLATION_FEE_TO_WITHDRAW();

        uint256 toBeWithdrawn = cancellationFeeEscrow;
        cancellationFeeEscrow = 0;

        CUSTODIAN.withdraw(toBeWithdrawn, to);

        emit EscrowWithdrawn(address(this), toBeWithdrawn);
    }
        ...
        onlyInPhase(LoanTypes.PoolPhase.ORIGINATION)
    {
        ...
        if (deposits == 0) revert LoanErrors.LOAN_NO_LIQUIDITY();

        if (deposits < MIN_ORIGINATION_AMOUNT) {
     ...
     * @param finalPayment True if this is the final payment
     */
    function _makePaymentsDue(bool finalPayment) internal {
        uint256 paymentsToMake = _getNumberOfPaymentsExpired() - paymentsDoneCount;
        if (paymentsToMake == 0) revert LoanErrors.LOAN_NO_PAYMENTS_DUE();

        if (finalPayment) {
            if (totalPaymentsCount != paymentsDoneCount + paymentsToMake) revert LoanErrors.LOAN_ONLY_FINAL_PAYMENT();
        } else {
            if (totalPaymentsCount == paymentsDoneCount + paymentsToMake) revert LoanErrors.LOAN_NO_FINAL_PAYMENT();
        }
    ...
    function calculateRepayAmount(
        ...
        uint256 timeDeltaIntoLateRepay,
        uint256 lateRepaymentRate
    ) internal virtual returns (uint256 amountToRepayForTick, uint256 tickFees);

    /**
     * @notice Repay the current loan
     */
    function _repay() internal onlyInPhase(LoanTypes.PoolPhase.ISSUED) {
...
abstract contract LoanGovernance is ILoanGovernance, LoanState {
    ...
    function enableOriginationPhase() external onlyGovernance onlyInPhase(LoanTypes.PoolPhase.BOOK_BUILDING) {
        if (block.timestamp <= BOOK_BUILDING_PHASE_START_TIMESTAMP + BOOK_BUILDING_PERIOD_DURATION) {
...
abstract contract LoanLender is LoanState, SolmateERC721, ILoanLender {
    ...
    function deposit(
        ...
        if (
            poolPhase != LoanTypes.PoolPhase.BOOK_BUILDING ||
            block.timestamp > BOOK_BUILDING_PHASE_START_TIMESTAMP + BOOK_BUILDING_PERIOD_DURATION
    ...
    function withdraw(uint256 positionId) external {
        ...
        uint256 earnings;
        if (poolPhase == LoanTypes.PoolPhase.BOOK_BUILDING) {
            withdrawable = LoanLogic.getPositionRepartitionBookBuildingPhase(position.depositedAmount);
            tick.depositedAmount -= withdrawable;
            deposits -= withdrawable;
        }
        if (poolPhase == LoanTypes.PoolPhase.ISSUED) {
            if (!position.hasPartiallyWithdrawn) {
                (withdrawable, ) = tick.getPositionRepartitionNonStandardPhaseOrIssuedPhase(
                    position.depositedAmount,
                    position.hasPartiallyWithdrawn
                );
                if (tick.borrowedAmount > 0 && tick.borrowedAmount < tick.depositedAmount) {
                    position.hasPartiallyWithdrawn = true;
                }
            }

            (uint256 totalWithdrawablePayments, uint256 earningsSubjectToFees) = tick.getWithdrawablePayments(
                position,
                paymentsDoneCount
            );
            earnings = earningsSubjectToFees;
            withdrawable += totalWithdrawablePayments;
            position.numberOfPaymentsWithdrawn = paymentsDoneCount;
        }
        if (poolPhase == LoanTypes.PoolPhase.CANCELLED) {
            withdrawable = tick.getPositionRepartitionCancelledPhase(position.depositedAmount);
        }
        if (poolPhase == LoanTypes.PoolPhase.REPAID) {
            withdrawable = tick.getPositionRepartitionRepaidPhase(
                position.depositedAmount,
                position.hasPartiallyWithdrawn
            );
            (uint256 totalWithdrawablePayments, uint256 earningsSubjectToFees) = tick.getWithdrawablePayments(
        ...
        if (withdrawable == 0) revert LoanErrors.LOAN_WITHDRAWAL_NOT_AVAILABLE();
        ...
        uint256 fees = earnings.mul(FEES_CONTROLLER.getRepaymentFeesRate());
        CUSTODIAN.withdraw(withdrawable - fees, msg.sender);

        emit Withdrawn(positionId, msg.sender, withdrawable - fees);
    }
    ...
    function validateWithdraw(uint256 positionId) internal view {
        ...
        if (poolPhase == LoanTypes.PoolPhase.ORIGINATION || poolPhase == LoanTypes.PoolPhase.NON_STANDARD)
            revert LoanErrors.LOAN_WITHDRAWAL_NOT_AVAILABLE();
    }
...
import '../libraries/LoanTypes.sol';
import '../libraries/LoanErrors.sol';
import '../libraries/LoanValidation.sol';
...
abstract contract LoanState is Managed, ILoanState {
    ...
    uint256 public BOOK_BUILDING_PHASE_START_TIMESTAMP;
    ...
    constructor(
        ...
        LoanValidation.validateDeployment(ratesAmountsConfig, durationsConfig, decimals);
     ...
     */
    function getBookBuildingPhaseEndTimestamp() external view returns (uint256 bookBuildingPhaseEndTimestamp) {
        uint256 bookBuildingPhaseStartTimestamp = BOOK_BUILDING_PHASE_START_TIMESTAMP > 0
            ? BOOK_BUILDING_PHASE_START_TIMESTAMP
            : block.timestamp;
        return bookBuildingPhaseStartTimestamp + BOOK_BUILDING_PERIOD_DURATION;
    }
}
...
import '../../../interfaces/IPool.sol';
import './../modules/interfaces/IRCLState.sol';
 ...
 * @author Atlendis Labs
 */
interface IRevolvingCreditLine is IRCLLenders, IRCLBorrowers, IRCLGovernance, IRCLState {
    /**
     * @notice Retrieve the general high level information of a position
     * @param positionId ID of the position
     * @return owner Owner of the position
...
/**
 * @title AccrualsHelpers
 * @notice Helper functions to compute how interests are accrued in the protocol
 * @author Atlendis Labs
 */
library AccrualsHelpers {
    /*//////////////////////////////////////////////////////////////
                                LIBRARIES
    //////////////////////////////////////////////////////////////*/
    using FixedPointMathLib for uint256;
     ...
     * @dev The result is rounded down given that yield factor is mainly used to compute withdraw values
     * @param tick Target tick
    ...
    function calculateYieldFactorIncrease(
        ...
        yieldFactorIncrease = accrualsForDown(tick.baseEpochsAmounts.borrowed, timeDelta, rate).divDown(
     ...
     */
    function accrualsForDown(
        uint256 amount,
        uint256 timeDelta,
        uint256 rate
    ) public view returns (uint256 accruals) {
        accruals = ((amount * timeDelta * rate) / 365 days).mulDown(1);
    }

    /**
     * @notice Calculates the accruals accumulated by an amount during a duration and at a target rate
     * @dev Rounds the result down
     * @param amount Amount to calculate accruals for
     * @param timeDelta Duration during which to calculate the accruals
     * @param rate Accrual rate
     */
    function accrualsForUp(
        uint256 amount,
        uint256 timeDelta,
        uint256 rate
    ) public view returns (uint256 accruals) {
        accruals = ((amount * timeDelta * rate) / 365 days).mulUp(1);
    }
}
 ...
 * @author Atlendis Labs
 */
library BorrowerLogic {
    /*//////////////////////////////////////////////////////////////
                                EVENTS
    //////////////////////////////////////////////////////////////*/

    /**
     * @notice Emitted when amount in a tick has been borrowed
     * @param rate Rate of the tick
     * @param borrowedAmount Amount of funds borrowed against the tick
     */
    event TickBorrowed(uint256 rate, uint256 borrowedAmount);

    /**
     * @notice Emitted when amount in a tick has been repaid
     * @param rate Rate of the tick
     * @param availableAmount Amount of funds available in the tick
     */
    event TickRepaid(uint256 rate, uint256 availableAmount);
    ...
    /**
     * @notice Borrow logic, loop over order book to take a loan from available positions
     * @param ticks Ticks mapping of the pool
     * @param currentLoan Current loan information
     * @param amount Amount to be borrowed
     * @param minRate Minimum interest rate of a tick
     * @param maxRate Maximum interest rate of a tick
     * @param spacing Interest rate difference between two ticks
     * @return toBeRepaid Total amount to be repaid in consquence of the borrow
     */
    function borrow(
        mapping(uint256 => DataTypes.Tick) storage ticks,
        DataTypes.Loan storage currentLoan,
        uint256 amount,
        uint256 minRate,
        uint256 maxRate,
        uint256 spacing
    ) public returns (uint256 toBeRepaid) {
        uint256 remainingAmount = amount;
        uint256 rate = minRate;
        while (remainingAmount > 0 && rate <= maxRate) {
            (uint256 tickBorrowedAmount, , ) = borrowForTick(
                ticks[rate],
                currentLoan,
                DataTypes.BorrowInput({totalAmountToBorrow: remainingAmount, totalAccrualsToAllocate: 0, rate: rate})
            );
            if (tickBorrowedAmount > 0) {
                remainingAmount -= tickBorrowedAmount;

                // accruals are rounded up to prevent overflows during future withdrawals or exits
                toBeRepaid +=
                    tickBorrowedAmount +
                    AccrualsHelpers.accrualsForUp(tickBorrowedAmount, currentLoan.maturity - block.timestamp, rate);

                emit TickBorrowed(rate, tickBorrowedAmount);
            }
     ...
     * @dev Tick accruals expected earnings are generally rounded down to prevent underflows when substracting
     * @param tick Target tick
     * @param currentLoan Current loan information
     ...
     * @return tickAccrualsExpectedEarnings Amount of expected accruals accumulated until the end of the loan by the accruals allocated to the tick
     */
    function borrowForTick(
        DataTypes.Tick storage tick,
        DataTypes.Loan storage currentLoan,
        ...
        )
    {
        ...
        if (tick.baseEpochsAmounts.available + tick.newEpochsAmounts.available > 0) {
            ...
            // available amount and adjusted deposits can be desynchronised with zero value hence the double check
            ...
            tickAccrualsExpectedEarnings += AccrualsHelpers.accrualsForDown(
                epochAccruals,
                timeUntilMaturity,
                args.rate
            );
     ...
     * @dev Yield factor is rounded down to prevent underflows in substractions when withdrawing in the future
     * @param tick Target tick
    ...
    function borrowFromBase(
        ...
        tick.yieldFactor += AccrualsHelpers
            .accrualsForDown(amountBorrowed + accrualsAllocated, currentLoan.maturity - block.timestamp, rate)
            .divDown(tick.baseEpochsAmounts.adjustedDeposits, RAY);
    }
    ...
    function borrowFromNew(
        ...
        if (epoch.loanId < currentLoan.id) {
            epoch.loanId = currentLoan.id;
            tick.currentEpochId += 1;
        }
        ...
        uint256 earnings = AccrualsHelpers.accrualsForDown(
    ...
    /**
     * @notice Repay logic, loop over order book to finalize the current loan
     * @param ticks Ticks mapping of the pool
     * @param currentLoan Current loan information
     * @param totalBorrowed Total amount borrowed to be repaid
     * @param lateRepaymentFeesRate Later repayment fees rate
     * @param minRate Minimum interest rate of a tick
     * @param maxRate Maximum interest rate of a tick
     * @param spacing Interest rate difference between two ticks
     * @param feesController Fees controller contract, handles fees calculation and distribution
     * @return fees Amount of fees paid because of the loan conditions
     * @return lateRepayFees Amount of fees paid in case of repayment after maturity
     */
    function repay(
        mapping(uint256 => DataTypes.Tick) storage ticks,
        DataTypes.Loan storage currentLoan,
        uint256 totalBorrowed,
        uint256 lateRepaymentFeesRate,
        uint256 minRate,
        uint256 maxRate,
        uint256 spacing,
        IFeesController feesController
    ) public returns (uint256 fees, uint256 lateRepayFees) {
        fees = 0;
        uint256 currentInterestRate = minRate;
        while (currentInterestRate <= maxRate) {
            DataTypes.Tick storage tick = ticks[currentInterestRate];
            if (tick.latestLoanId == currentLoan.id) {
                if (block.timestamp > currentLoan.maturity) {
                    registerLateRepaymentAccruals(tick, block.timestamp - currentLoan.maturity, lateRepaymentFeesRate);
                }

                fees += registerRepaymentFees(tick, feesController);

                uint256 availableAmount = prepareTickForNextLoan(tick, currentLoan);
                emit TickRepaid(currentInterestRate, availableAmount);
            }
            currentInterestRate += spacing;
        }
        // accruals are rounded up to prevent overflows during future withdrawals or exits
        lateRepayFees = block.timestamp > currentLoan.maturity
            ? AccrualsHelpers.accrualsForUp(
                totalBorrowed,
                block.timestamp - currentLoan.maturity,
                lateRepaymentFeesRate
            )
            : 0;
    }
     ...
     * @dev The amounts are rounded up since we are computing additions
     * @param tick Target tick
    ...
    function registerLateRepaymentAccruals(
        ...
        if (tick.baseEpochsAmounts.borrowed != 0) {
            tick.yieldFactor += AccrualsHelpers.calculateYieldFactorIncrease(tick, timeDelta, rate);
        }
        if (tick.newEpochsAmounts.borrowed != 0) {
            uint256 newEpochsAccruals = AccrualsHelpers.accrualsForUp(tick.newEpochsAmounts.borrowed, timeDelta, rate);
            tick.newEpochsAmounts.toBeAdjusted += newEpochsAccruals;
        }
        if (tick.detachedAmounts.borrowed != 0) {
            uint256 detachedAccruals = AccrualsHelpers.accrualsForUp(tick.detachedAmounts.borrowed, timeDelta, rate);
            tick.detachedAmounts.toBeAdjusted += detachedAccruals;
        }
    }

    /**
     * @notice Prepares all the tick data structures for the next loan cycle
     * @dev Quantities that are substracted are rounded down while quantities added are rounded up
     * @param tick Target tick
     * @param currentLoan Current loan information
     * @return availableAmount Available amount in tick after repayment
     */
    function prepareTickForNextLoan(DataTypes.Tick storage tick, DataTypes.Loan storage currentLoan)
        public
        returns (uint256 availableAmount)
    {
        // if last epoch was not borrowed at all, it turns directly into the next loan base epoch
        ...
        if (tick.newEpochsAmounts.available + tick.newEpochsAmounts.borrowed > 0) {
            tick.newEpochsAmounts.toBeAdjusted -= tick
                .newEpochsAmounts
                .toBeAdjusted
                .mulDown(tick.newEpochsAmounts.optedOut)
                .divDown(tick.newEpochsAmounts.available + tick.newEpochsAmounts.borrowed);
        }
        ...
        uint256 tickAdjusted = tick.baseEpochsAmounts.adjustedDeposits +
            (tick.newEpochsAmounts.toBeAdjusted + tick.detachedAmounts.toBeAdjusted).divUp(tick.yieldFactor, RAY) -
            tick.baseEpochsAmounts.adjustedOptedOut;
        uint256 tickAvailable = tickAdjusted.mulUp(tick.yieldFactor, RAY);

        // recording end of loan yield factor for further use
        tick.endOfLoanYieldFactors[currentLoan.id] = tick.yieldFactor;

        uint256 newBaseEpochsPositionsCount;
        if (
            tick.optedOutPositionsCount <=
            tick.baseEpochsAmounts.positionsCount +
                tick.newEpochsAmounts.positionsCount +
                tick.detachedAmounts.positionsCount
        ) {
            newBaseEpochsPositionsCount =
                tick.baseEpochsAmounts.positionsCount +
                tick.newEpochsAmounts.positionsCount +
                tick.detachedAmounts.positionsCount -
                tick.optedOutPositionsCount;
        }
        tick.optedOutPositionsCount = 0;
        // resetting data structures
        delete tick.baseEpochsAmounts;
        delete tick.newEpochsAmounts;
        delete tick.detachedAmounts;
        tick.baseEpochsAmounts.adjustedDeposits = tickAdjusted;
        tick.baseEpochsAmounts.available = tickAvailable;
        tick.baseEpochsAmounts.positionsCount = newBaseEpochsPositionsCount;
    ...
    {
        uint256 baseEpochsFees = registerBaseEpochRepaymentFees(tick, feesController);
        uint256 newEpochsFees = registerNewEpochsRepaymentFees(tick, feesController);
        uint256 detachedFees = registerDetachedRepaymentFees(tick, feesController);
        fees = baseEpochsFees + newEpochsFees + detachedFees;
    }

    /**
     * @notice Logic to persist the repayment fees for base epoch
     * @dev Fees are generally rounded up to reduce the yield factor as much as possible to prevent
     * underflows in future withdrawals
     * @param tick Target tick
     * @param feesController Address of the fees controller
     * @return baseEpochsFees Amount of fees accrued for the tick base epoch
     */
    function registerBaseEpochRepaymentFees(DataTypes.Tick storage tick, IFeesController feesController)
        public
        returns (uint256 baseEpochsFees)
    {
        if (tick.baseEpochsAmounts.positionsCount == 0) return 0;
        uint256 depositsCurrentValue = tick.baseEpochsAmounts.adjustedDeposits.mulUp(tick.yieldFactor, RAY);
        uint256 depositsStartOfLoanValue = tick.baseEpochsAmounts.available + tick.baseEpochsAmounts.borrowed;
        uint256 baseEpochsAccruals = depositsCurrentValue - depositsStartOfLoanValue;
        baseEpochsFees = feesController.registerRepaymentFees(baseEpochsAccruals);
        tick.yieldFactor -= baseEpochsFees.divUp(tick.baseEpochsAmounts.adjustedDeposits, RAY);
    }
     ...
     * @param tick Target tick
     * @param feesController Address of the fees controller
     * @return fees Amount of fees accrued for the tick new epochs
     */
    function registerNewEpochsRepaymentFees(DataTypes.Tick storage tick, IFeesController feesController)
        public
        returns (uint256 fees)
    {
        if (tick.newEpochsAmounts.positionsCount == 0) return 0;
        ...
        fees = feesController.registerRepaymentFees(newEpochsAccruals);
        tick.newEpochsAmounts.toBeAdjusted -= fees;
    }
     ...
     * @return fees Amount of fees accrued for the tick new epochs
     */
    function registerDetachedRepaymentFees(DataTypes.Tick storage tick, IFeesController feesController)
        public
        returns (uint256 fees)
    {
        if (tick.detachedAmounts.positionsCount == 0) return 0;
        uint256 detachedAccruals = tick.detachedAmounts.toBeAdjusted - tick.detachedAmounts.borrowed;
        fees = feesController.registerRepaymentFees(detachedAccruals);
        tick.detachedAmounts.toBeAdjusted -= fees;
    }
    ...
    function allocateBorrowAmounts(
        ...
        if (amountToBorrow + accrualsToAllocate >= epochAvailable) {
            accrualsAllocated = accrualsToAllocate.mulDown(epochAvailable).divDown(amountToBorrow + accrualsToAllocate);
...
library DataTypes {
    ...
    struct Position {
        ...
        uint256 detachLoanId;
        PositionDetachedAmounts detached;
    }

    struct PositionDetachedAmounts {
        uint256 borrowed;
...
import '../../../libraries/FixedPointMathLib.sol';
import './AccrualsHelpers.sol';
import './BorrowerLogic.sol';
import './DataTypes.sol';
import './Errors.sol';
import './PositionHelpers.sol';
...
library LenderLogic {
    ...
    //////////////////////////////////////////////////////////////*/
    /**
     * @notice Emitted when a position is exited and a part or the full borrowed amount is redistributed to the tick
     * @param rate Rate of the tick
     * @param borrowedAmount Amount of borrowed funds in the recipient tick
     */
    event ExitedAmountRedistributedInTick(uint256 indexed rate, uint256 borrowedAmount);
     ...
     * @dev Deposits are rounded up to favorize the protocol and prevent underflows when withdrawing
     * @param tick Target tick
     * @param currentLoan Current loan information
    ...
    function deposit(
        ...
        // deposit in new epoch because tick is borrowed
        if ((currentLoan.maturity > 0) && (tick.latestLoanId == currentLoan.id)) {
            ...
            tick.epochs[tick.currentEpochId].positionsCount++;
        }
        // deposit in base epoch when tick is not borrowed
        else {
            tick.baseEpochsAmounts.available += amount;
            tick.baseEpochsAmounts.adjustedDeposits += amount.divUp(tick.yieldFactor, RAY);
            tick.baseEpochsAmounts.positionsCount++;
        }
    }
    ...
    function updateRate(
        ...
        position.detachLoanId = 0;
        delete position.detached;
    }
     ...
     * @param positionOwner Minimum amount of funds that must be held in a position
     */
    function validateWithdraw(
        ...
        if (positionOwner != msg.sender) revert RevolvingCreditLineErrors.RCL_ONLY_OWNER();

        if (position.creationTimestamp == block.timestamp) revert RevolvingCreditLineErrors.RCL_TIMELOCK();

        (, uint256 borrowedAmount) = PositionHelpers.getPositionRepartition(tick, position, referenceLoan, currentLoan);
        if (borrowedAmount > 0) revert RevolvingCreditLineErrors.RCL_POSITION_BORROWED();
        uint256 positionCurrentValue = PositionHelpers.getPositionCurrentValue(
            tick,
            position,
            referenceLoan,
            currentLoan
     ...
     * @dev Withdrawing are rounded down to prevent underflows when substracting amounts
     * @param tick Target tick
     * @param position Target position
    ...
    function withdraw(
        ...
        bool isFullWithdraw = expectedWithdrawnAmount == type(uint256).max;
        withdrawnAmount = isFullWithdraw ? positionCurrentValue : expectedWithdrawnAmount;

        // if the position was opted out, its value has already been removed from the tick data
        // @dev an opted out position that was withdrawn is necessary from an older loan
        ...
        if (currentLoan.maturity == 0 || tick.latestLoanId < currentLoan.id) {
            uint256 adjustedAmountToWithdraw = withdrawnAmount.divDown(tick.yieldFactor, RAY);
            safeSubstractBaseEpochAmounts({
                tick: tick,
                shouldDecreasePositionCount: isFullWithdraw,
                borrowedAmountToWithdraw: 0,
                availableAmountToWithdraw: withdrawnAmount,
                adjustedDepositsAmountToWithdraw: adjustedAmountToWithdraw
            });
        }
        // when a loan is ongoing, only the current epoch is not borrowed
            ...
            safeSubstractNewEpochsAmounts({
                tick: tick,
                shouldDecreasePositionCount: isFullWithdraw,
                borrowedAmountToWithdraw: 0,
                availableAmountToWithdraw: withdrawnAmount,
                toBeAdjustedAmountToWithdraw: withdrawnAmount
            });

            safeSubstractEpochStruct({
                tick: tick,
                epochId: tick.currentEpochId,
                shouldDecreasePositionCount: isFullWithdraw,
                borrowedAmountToWithdraw: 0,
                depositedAmountToWithdraw: withdrawnAmount,
                accrualsToWithdraw: 0
            });
        }
        ...
        if (!isFullWithdraw) {
            ...
            position.detachLoanId = 0;
            position.detached = DataTypes.PositionDetachedAmounts({borrowed: 0, expectedAccruals: 0});
        }
    }
    ...
    function optOut(
        ...
        if (position.detachLoanId == currentLoan.id) {
            safeSubstractDetachedAmounts({
                tick: tick,
                shouldDecreasePositionCount: true,
                borrowedAmountToWithdraw: position.detached.borrowed,
                toBeAdjustedAmountToWithdraw: position.detached.borrowed + position.detached.expectedAccruals
            });
        }
        // if the position is from the base epoch, the adjusted amount to remove at the end of the loan is known in advance
            ...
            tick.optedOutPositionsCount++;
            tick.baseEpochsAmounts.adjustedOptedOut += PositionHelpers.getAdjustedAmount(tick, position, referenceLoan);
        }
    ...
    /**
     * @notice Validation logic for position detaching, which is withdrawing the unmatched part of the position when borrowed
     * @param tick Target tick
     * @param position Target position
     * @param referenceLoan Either first loan or detach loan of the position
     * @param currentLoan Current loan information
     * @param minDepositAmount Minimum amount a position can hold in the protocol
     * @param owner Current owner of the position
     */
    function validateDetach(
        DataTypes.Tick storage tick,
        DataTypes.Position storage position,
        DataTypes.Loan storage referenceLoan,
        DataTypes.Loan storage currentLoan,
        ...
        if (owner != msg.sender) revert RevolvingCreditLineErrors.RCL_ONLY_OWNER();
        if (currentLoan.maturity == 0) revert RevolvingCreditLineErrors.RCL_NO_LOAN_RUNNING();

        (uint256 unborrowedAmount, uint256 borrowedAmount) = PositionHelpers.getPositionRepartition(
            tick,
            position,
            referenceLoan,
            currentLoan
        ...
        uint256 endOfLoanPositionValue = PositionHelpers.getPositionEndOfLoanValue(
            tick,
            position,
            referenceLoan,
            currentLoan
    ...
    function detach(
        ...
        uint256 borrowedAmount;
        (unborrowedAmount, borrowedAmount) = PositionHelpers.getPositionRepartition(
            tick,
            position,
            referenceLoan,
            currentLoan
        ...
        if (position.epochId <= tick.loanStartEpochId && position.optOutLoanId == currentLoan.id) {
            detachBaseEpochOptedOutPosition(tick, position, referenceLoan, unborrowedAmount, borrowedAmount);
        } else if (position.epochId <= tick.loanStartEpochId && position.optOutLoanId < currentLoan.id) {
            detachBaseEpochPosition(tick, position, referenceLoan, unborrowedAmount, borrowedAmount, accruals);
        } else if (position.optOutLoanId == currentLoan.id) {
            detachNewEpochOptedOutPosition(
                tick,
                position,
                unborrowedAmount,
                borrowedAmount,
            ...
            detachNewEpochPosition(tick, position, unborrowedAmount, borrowedAmount, accruals, endOfLoanPositionValue);
        }

        position.detached = DataTypes.PositionDetachedAmounts({borrowed: borrowedAmount, expectedAccruals: accruals});
        position.detachLoanId = currentLoan.id;
    }

    /**
     * @notice Detach a base epoch position that has been opted out before
     * @param tick Target tick
     * @param position Target position
     * @param referenceLoan Either first loan or detach loan of the position
     * @param unborrowedAmount Unborrowed part of the target position
     * @param borrowedAmount Borrowed part of the target position
     */
    function detachBaseEpochOptedOutPosition(
        DataTypes.Tick storage tick,
        DataTypes.Position storage position,
        DataTypes.Loan storage referenceLoan,
        ...
        uint256 adjustedAmount = PositionHelpers.getAdjustedAmount(tick, position, referenceLoan);
        safeSubstractBaseEpochAmounts({
            tick: tick,
            shouldDecreasePositionCount: true,
            borrowedAmountToWithdraw: borrowedAmount,
            availableAmountToWithdraw: unborrowedAmount,
            adjustedDepositsAmountToWithdraw: adjustedAmount
        });
    }

    /**
     * @notice Detach a base epoch position that is active
     * @param tick Target tick
     * @param position Target position
     * @param referenceLoan Either first loan or detach loan of the position
     * @param unborrowedAmount Unborrowed part of the target position
     * @param borrowedAmount Borrowed part of the target position
     * @param accruals Amount of accruals the position will accrue for the current loan
     */
    function detachBaseEpochPosition(
        DataTypes.Tick storage tick,
        DataTypes.Position storage position,
        DataTypes.Loan storage referenceLoan,
        uint256 unborrowedAmount,
        ...
        tick.detachedAmounts.positionsCount++;
        tick.detachedAmounts.borrowed += borrowedAmount;
        tick.detachedAmounts.toBeAdjusted += borrowedAmount + accruals;
    ...
    /**
     * @notice Detach a new epoch position that has been opted out
     * @param tick Target tick
     * @param position Target position
     * @param unborrowedAmount Unborrowed part of the target position
     * @param borrowedAmount Borrowed part of the target position
     * @param accruals Amount of accruals the position will accrue for the current loan
     * @param endOfLoanPositionValue Value of the target position and the end of the current loan
     */
    function detachNewEpochOptedOutPosition(
        DataTypes.Tick storage tick,
        DataTypes.Position storage position,
        uint256 unborrowedAmount,
        uint256 borrowedAmount,
        ...
        safeSubstractNewEpochsAmounts({
            ...
            borrowedAmountToWithdraw: borrowedAmount,
            availableAmountToWithdraw: unborrowedAmount,
            toBeAdjustedAmountToWithdraw: tick.newEpochsAmounts.toBeAdjusted - endOfLoanPositionValue
        });
        ...
        safeSubstractEpochStruct({
            ...
            shouldDecreasePositionCount: true,
            borrowedAmountToWithdraw: borrowedAmount,
            depositedAmountToWithdraw: position.baseDeposit,
            accrualsToWithdraw: accruals
        });
    }

    /**
     * @notice Detach a new epoch position that is active
     * @param tick Target tick
     * @param position Target position
     * @param unborrowedAmount Unborrowed part of the target position
     * @param borrowedAmount Borrowed part of the target position
     ...
     * @param endOfLoanPositionValue Value of the target position and the end of the current loan
     */
    function detachNewEpochPosition(
        DataTypes.Tick storage tick,
        DataTypes.Position storage position,
        uint256 unborrowedAmount,
        uint256 borrowedAmount,
     ...
     * @dev exits resulting in substracting withdrawn amounts, amounts are rounded down to prevent underflows
     * @param tick Target tick
     * @param position Target position
     ...
     * @return borrowedAmount Borrowed part of the position to be exited
        ...
        )
    {
        ...
        bool isFullExit = borrowedAmountToExit == type(uint256).max;
        if (isFullExit) borrowedAmountToExit = borrowedAmount;
        uint256 exitProportion = borrowedAmountToExit.divDown(borrowedAmount);
        borrowedAmount = borrowedAmount.mulDown(exitProportion);
        // get position values
        (
            uint256 currentPositionValue,
            uint256 startOfLoanPositionValue,
            uint256 endOfLoanPositionValue
        ) = PositionHelpers.getPositionValues(tick, position, referenceLoan, currentLoan, exitProportion);
        ...
        // register the exit for detached position
        if (position.detached.borrowed > 0 && (position.detachLoanId == currentLoan.id)) {
            prepareDetachedPositionExit(tick, position, endOfLoanPositionValue, exitProportion, isFullExit);
        }
        // register the exit for base epoch
        else if (position.epochId <= tick.loanStartEpochId) {
            prepareBaseEpochPositionExit(
                tick,
                position,
                referenceLoan,
                borrowedAmount,
     ...
     * @dev exits resulting in substracting withdrawn amounts, amounts are rounded down to prevent
     * @param tick Target tick
     * @param position Target position
     * @param exitProportion Proportion of the position to be withdrawn, used for partial exits
     * @param isFullExit Is the current action a full or a partial exit
     */
    function prepareDetachedPositionExit(
        DataTypes.Tick storage tick,
        DataTypes.Position storage position,
        uint256 endOfLoanPositionValue,
        ...
        uint256 scaledWithdrawnAmount = position.detached.borrowed.mulDown(exitProportion);
        safeSubstractDetachedAmounts({
            tick: tick,
            shouldDecreasePositionCount: isFullExit,
            borrowedAmountToWithdraw: scaledWithdrawnAmount,
            toBeAdjustedAmountToWithdraw: endOfLoanPositionValue
        });

        if (!isFullExit) {
            position.detached.borrowed -= scaledWithdrawnAmount;
            position.detached.expectedAccruals -= position.detached.expectedAccruals.mulDown(exitProportion);
        }
    }
     ...
     * @param isFullExit Is the current action a full or a partial exit
     */
    function prepareBaseEpochPositionExit(
        DataTypes.Tick storage tick,
        DataTypes.Position storage position,
        DataTypes.Loan storage referenceLoan,
        uint256 borrowedAmount,
        ...
        uint256 adjustedAmountToExit = PositionHelpers.getAdjustedAmount(tick, position, referenceLoan).mulDown(
            exitProportion
        );
        safeSubstractBaseEpochAmounts({
            tick: tick,
            ...
            borrowedAmountToWithdraw: borrowedAmount,
            availableAmountToWithdraw: unborrowedAmount,
            adjustedDepositsAmountToWithdraw: adjustedAmountToExit
        });

        if (!isFullExit) {
            uint256 equivalentYieldFactor = PositionHelpers.getEquivalentYieldFactor(
                tick,
                tick.epochs[position.epochId],
                referenceLoan
            );
            uint256 precedingLoanYieldFactor = tick.endOfLoanYieldFactors[
                tick.epochs[tick.loanStartEpochId].precedingLoanId
            ];
            position.baseDeposit -= borrowedAmount.mulDown(equivalentYieldFactor, RAY).divDown(
                precedingLoanYieldFactor,
                RAY
            );
        }
    }
     ...
     * @param isFullExit Is the current action a full or a partial exit
     */
    function prepareNewEpochPositionExit(
        DataTypes.Tick storage tick,
        DataTypes.Position storage position,
        DataTypes.Loan storage currentLoan,
        uint256 startOfLoanPositionValue,
        ...
        uint256 toBeAdjustedAmountToWithdraw = currentPositionValue +
            AccrualsHelpers.accrualsForDown(borrowedAmount, currentLoan.maturity - block.timestamp, position.rate);
        safeSubstractNewEpochsAmounts({
            tick: tick,
            shouldDecreasePositionCount: isFullExit,
            borrowedAmountToWithdraw: borrowedAmount,
            availableAmountToWithdraw: unborrowedAmount,
            toBeAdjustedAmountToWithdraw: toBeAdjustedAmountToWithdraw
        });
        ...
        safeSubstractEpochStruct({
            ...
            shouldDecreasePositionCount: isFullExit,
            borrowedAmountToWithdraw: borrowedAmount,
            depositedAmountToWithdraw: startOfLoanPositionValue,
            accrualsToWithdraw: endOfLoanPositionValue - startOfLoanPositionValue
        });

        if (!isFullExit) position.baseDeposit -= borrowedAmount;
    }
    ...
    /**
     * @notice Exit logic, loop over order book to swap the debt with remaining positions
     * @param ticks Ticks mapping of the pool
     * @param currentLoan Current loan information
     * @param endOfLoanBorrowEquivalent End of loan value to borrow against the current order book
     * @param remainingAccrualsToAllocate Value of exiting position accruals to advance during the exit
     * @param minRate Minimum interest rate of a tick
     * @param maxRate Maximum interest rate of a tick
     * @param spacing Interest rate difference between two ticks
     * @return totalAmountReborrowed Actual equivalent amount borrowed against the order book as a consequence of the exit
     * @return totalAccrualsInterests Total amount of interests the advanced accruals will accumulate during the remaining loan time
     */
    function exit(
        mapping(uint256 => DataTypes.Tick) storage ticks,
        DataTypes.Loan storage currentLoan,
        ...
        uint256 maxRate,
        uint256 spacing
    ) external returns (uint256 totalAmountReborrowed, uint256 totalAccrualsInterests) {
        // swap the debt to exit with other ticks' available liquidity
        uint256 rate = minRate;
        totalAmountReborrowed = remainingAccrualsToAllocate;
        while (endOfLoanBorrowEquivalent > 0 && rate <= maxRate) {
            if (ticks[rate].baseEpochsAmounts.available > 0 || ticks[rate].newEpochsAmounts.available > 0) {
                ...
                ) = exitForTick(ticks[rate], currentLoan, endOfLoanBorrowEquivalent, remainingAccrualsToAllocate, rate);
                ...
                if (tickBorrowedEndOfLoanBorrowEquivalent > endOfLoanBorrowEquivalent) {
                    endOfLoanBorrowEquivalent = 0;
                } else {
                    endOfLoanBorrowEquivalent -= tickBorrowedEndOfLoanBorrowEquivalent;
                }
                remainingAccrualsToAllocate -= tickAllocatedAccruals;

                emit ExitedAmountRedistributedInTick(rate, tickBorrowed + tickAllocatedAccruals);
            }
            rate += spacing;
        }

        if (endOfLoanBorrowEquivalent > 0) revert RevolvingCreditLineErrors.RCL_NO_LIQUIDITY();
    }
     ...
     * @dev To prevent remaining amount from accumulating, output tick borrowed end of loan value, which will be substracted from remaining amount
     * For further iterations, is rounded up in toEndOfLoanValue, which is different from the rounding logic throughout the rest of the codebase. For the same
     * reasons, tick to borrow equivalent amount is rounded down in toTickCurrentValue
     * @param tick Target tick
     * @param currentLoan Current loan information
        ...
        )
    {
        ...
        (tickBorrowed, tickAllocatedAccruals, tickAllocatedAccrualsInterests) = BorrowerLogic.borrowForTick(
            tick,
            currentLoan,
     ...
     * @dev Result in rounded up to prevent NO_LIQUIDITY error, see exit function for more context
    ...
    function toTickCurrentValue(
        ...
        uint256 currentValueToEndOfLoanMultiplier = RAY +
            AccrualsHelpers.accrualsForDown(RAY, currentMaturity - block.timestamp, rate);
        tickCurrentValue = endOfLoanValue.divUp(currentValueToEndOfLoanMultiplier, RAY);
    }

    /**
     * @notice Util to calculate the end loan value of an amount for a tick depending on its current value
     * @dev Result in rounded up, see exit function for more context
    ...
    function toEndOfLoanValue(
        ...
        uint256 currentValueToEndOfLoanMultiplier = RAY +
            AccrualsHelpers.accrualsForUp(RAY, currentMaturity - block.timestamp, rate);
        endOfLoanValue = tickCurrentValue.mulUp(currentValueToEndOfLoanMultiplier, RAY);
    }

    /**
     * @notice Substracts amounts from base epoch amounts structs, zero it if there are no positions left
     * @param tick Target tick
     * @param shouldDecreasePositionCount Boolean indicating if the number of positions in base epoch should be decreased
     * @param borrowedAmountToWithdraw Borrowed amount to be withdrawn in the operation
     * @param availableAmountToWithdraw Available amount to be withdrawn in the operation
     * @param adjustedDepositsAmountToWithdraw Adjusted deposits amount to be withdrawn in the operation
     */
    function safeSubstractBaseEpochAmounts(
        DataTypes.Tick storage tick,
        bool shouldDecreasePositionCount,
        uint256 borrowedAmountToWithdraw,
        uint256 availableAmountToWithdraw,
        uint256 adjustedDepositsAmountToWithdraw
    ) private {
        if (shouldDecreasePositionCount) tick.baseEpochsAmounts.positionsCount--;
        if (tick.baseEpochsAmounts.positionsCount == 0) {
            delete tick.baseEpochsAmounts;
        } else {
            tick.baseEpochsAmounts.borrowed -= borrowedAmountToWithdraw;
            tick.baseEpochsAmounts.available -= availableAmountToWithdraw;
            tick.baseEpochsAmounts.adjustedDeposits -= adjustedDepositsAmountToWithdraw;
        }
    }

    /**
     * @notice Substracts amounts from new epochs amounts structs, zero it if there are no positions left
     * @param tick Target tick
     * @param shouldDecreasePositionCount Boolean indicating if the number of positions in base epoch should be decreased
     * @param borrowedAmountToWithdraw Borrowed amount to be withdrawn in the operation
     * @param availableAmountToWithdraw Available amount to be withdrawn in the operation
     * @param toBeAdjustedAmountToWithdraw Amount to be adjusted to be withdrawn in the operation
     */
    function safeSubstractNewEpochsAmounts(
        DataTypes.Tick storage tick,
        bool shouldDecreasePositionCount,
        uint256 borrowedAmountToWithdraw,
        uint256 availableAmountToWithdraw,
        uint256 toBeAdjustedAmountToWithdraw
    ) private {
        if (shouldDecreasePositionCount) tick.newEpochsAmounts.positionsCount--;
        if (tick.newEpochsAmounts.positionsCount == 0) {
            delete tick.newEpochsAmounts;
        } else {
            tick.newEpochsAmounts.borrowed -= borrowedAmountToWithdraw;
            tick.newEpochsAmounts.available -= availableAmountToWithdraw;
            tick.newEpochsAmounts.toBeAdjusted -= toBeAdjustedAmountToWithdraw;
        }
    }

    /**
     * @notice Substracts amounts from epoch structs, zero it if there are no positions left
     * @param tick Target tick
     * @param epochId Id of the target epoch
     * @param shouldDecreasePositionCount Boolean indicating if the number of positions in base epoch should be decreased
     * @param borrowedAmountToWithdraw Borrowed amount to be withdrawn in the operation
     * @param depositedAmountToWithdraw Deposited amount to be withdrawn in the operation
     * @param accrualsToWithdraw Accruals to be withdrawn in the operation
     */
    function safeSubstractEpochStruct(
        DataTypes.Tick storage tick,
        uint256 epochId,
        bool shouldDecreasePositionCount,
        uint256 borrowedAmountToWithdraw,
        uint256 depositedAmountToWithdraw,
        uint256 accrualsToWithdraw
    ) private {
        DataTypes.Epoch storage epoch = tick.epochs[epochId];
        if (shouldDecreasePositionCount) epoch.positionsCount--;
        if (epoch.positionsCount == 0) {
            epoch.borrowed = 0;
            epoch.deposited = 0;
            epoch.accruals = 0;
        } else {
            epoch.borrowed -= borrowedAmountToWithdraw;
            epoch.deposited -= depositedAmountToWithdraw;
            epoch.accruals -= accrualsToWithdraw;
        }
    }

    /**
     * @notice Substracts amounts from detached amounts structs, zero it if there are no positions left
     * @param tick Target tick
     * @param shouldDecreasePositionCount Boolean indicating if the number of positions in base epoch should be decreased
     * @param borrowedAmountToWithdraw Borrowed amount to be withdrawn in the operation
     * @param toBeAdjustedAmountToWithdraw Amount to be adjusted to be withdrawn in the operation
     */
    function safeSubstractDetachedAmounts(
        DataTypes.Tick storage tick,
        bool shouldDecreasePositionCount,
        uint256 borrowedAmountToWithdraw,
        uint256 toBeAdjustedAmountToWithdraw
    ) private {
        if (shouldDecreasePositionCount) tick.detachedAmounts.positionsCount--;
        if (tick.detachedAmounts.positionsCount == 0) {
            delete tick.detachedAmounts;
        } else {
            tick.detachedAmounts.borrowed -= borrowedAmountToWithdraw;
            tick.detachedAmounts.toBeAdjusted -= toBeAdjustedAmountToWithdraw;
        }
    }
}
...
/**
 * @title PositionHelpers
 * @notice Helper functions to compute diverse position values
 * @author Atlendis Labs
 */
library PositionHelpers {
    /*//////////////////////////////////////////////////////////////
                                LIBRARIES
    //////////////////////////////////////////////////////////////*/
    using FixedPointMathLib for uint256;
    ...
    /*//////////////////////////////////////////////////////////////
                                ADJUSTED AMOUNT GETTERS
    //////////////////////////////////////////////////////////////*/

    /**
     * @notice Gets the adjusted amount corresponding to the position depending on its history
     * @dev A position adjusted amount being mainly used to compute withdraw and exit amounts
     * This method tries to minimize it, hence the rounding down of added amounts and up of substracted amounts
     * @param tick Target tick
     * @param position Target position
     * @param referenceLoan Either first loan or detach loan of the position
    ...
    function getAdjustedAmount(
        ...
        if (position.detachLoanId > 0) return getDetachedPositionAdjustedAmount(tick, position, referenceLoan);
        return position.baseDeposit.divDown(getEquivalentYieldFactor(tick, epoch, referenceLoan), RAY);
    }

    /**
     * @notice Get the adjusted amount of a detached position
     * @param tick Target tick
     * @param position Target position
     * @param detachLoan Detach loan of the position
     * @return adjustedAmount Adjusted amount of the position
     */
    function getDetachedPositionAdjustedAmount(
        DataTypes.Tick storage tick,
        DataTypes.Position storage position,
        DataTypes.Loan storage detachLoan
    ) public view returns (uint256 adjustedAmount) {
        uint256 detachLoanlateRepayFees = AccrualsHelpers.accrualsForDown(
            position.detached.borrowed,
            detachLoan.lateRepayTimeDelta,
            detachLoan.lateRepayFeeRate
        );
        uint256 detachLoanProtocolFees = (position.detached.expectedAccruals + detachLoanlateRepayFees).mulUp(
            detachLoan.repaymentFeesRate
        );
        adjustedAmount = (position.detached.borrowed +
            position.detached.expectedAccruals +
            detachLoanlateRepayFees -
            detachLoanProtocolFees).divDown(tick.endOfLoanYieldFactors[position.detachLoanId], RAY);
    }

    /**
     * @notice Gets the equivalent yield factor for the target epoch depending on its borrow history
     * @dev A position equivalent yield factor being mainly used as a denominator to compute an adjusted amount
     * The goal is to minimize the adjusted amount, and thus to maximize the equivalent yield factor, hence the rounding up
     * @param tick Target tick
     * @param epoch Target epoch
     * @param referenceLoan Either first loan or detach loan of the position
    ...
    function getEquivalentYieldFactor(
        ...
        if (epoch.isBaseEpoch) {
            ...
            uint256 accrualsDown = epoch.accruals +
                AccrualsHelpers.accrualsForDown(
                    epoch.borrowed,
                    referenceLoan.lateRepayTimeDelta,
                    referenceLoan.lateRepayFeeRate
                );
            uint256 accrualsUp = epoch.accruals +
                AccrualsHelpers.accrualsForUp(
                    epoch.borrowed,
                    referenceLoan.lateRepayTimeDelta,
                    referenceLoan.lateRepayFeeRate
                );
            uint256 protocolFees = accrualsUp.mulUp(referenceLoan.repaymentFeesRate);
            uint256 endOfLoanValue = epoch.deposited + accrualsDown - protocolFees;

            equivalentYieldFactor = tick.endOfLoanYieldFactors[epoch.loanId].mulUp(epoch.deposited).divUp(
                endOfLoanValue
            );
        }
    }
     ...
     * @dev A position current value being mainly used to compute withdraw amounts, results are rounded down
     * @param tick Target tick
     * @param position Target position
     * @param referenceLoan Either first loan or detach loan of the position
     * @param currentLoan Current loan information
    ...
    function getPositionCurrentValue(
        ...
        if (position.epochId == tick.currentEpochId) return position.baseDeposit;

        // position opted out during a preceding loan
        if (position.optOutLoanId > 0) {
            bool optOutLoanRepaid = (position.optOutLoanId < currentLoan.id) || currentLoan.maturity == 0;
            if (optOutLoanRepaid)
                return
                    getAdjustedAmount(tick, position, referenceLoan).mulDown(
                        tick.endOfLoanYieldFactors[position.optOutLoanId],
                        RAY
                    );
        }

        // position detached during the current loan
        if (position.detachLoanId > 0) {
            bool detachLoanRepaid = (position.detachLoanId < currentLoan.id) || currentLoan.maturity == 0;
            if (!detachLoanRepaid) return getCurrentLoanDetachedPositionValue(position, currentLoan);
        }

        // new epochs positions
        if (
            (currentLoan.maturity > 0) &&
            (tick.latestLoanId == currentLoan.id) &&
            (position.epochId > tick.loanStartEpochId)
        ) return getNewEpochsPositionCurrentValue(tick, position, currentLoan);

        // base epoch positions
        return getBaseEpochPositionCurrentValue(tick, position, referenceLoan, currentLoan);
    }

    /**
     * @notice Get the current value of a position that was detached during the current loan
     * @param position Target position
     * @param currentLoan Current loan information
     * @return currentValue Current value of the position
     */
    function getCurrentLoanDetachedPositionValue(
        DataTypes.Position storage position,
        DataTypes.Loan storage currentLoan
    ) private view returns (uint256) {
        // if current timestamp is after maturity, the position is entitled to late repayment fees
        if (block.timestamp > currentLoan.maturity) {
            uint256 lateRepayTimeDelta = block.timestamp - currentLoan.maturity;
            uint256 lateRepayFees = AccrualsHelpers.accrualsForDown(
                position.detached.borrowed,
                lateRepayTimeDelta,
                currentLoan.lateRepayFeeRate
            );
            return position.detached.borrowed + position.detached.expectedAccruals + lateRepayFees;
        }
        // if current timestamp is before maturity, the position is only entitled to part of its accruals
        else {
            uint256 timeUntilMaturity = currentLoan.maturity - block.timestamp;
            // while we are generally trying to minimise the global position value, which would mean maximising
            // the unrealised interests here, that does not work in this case since we are already minimising the accruals
            // minimising the accruals while maximising the unrealized interests would create overflows
            uint256 unrealizedInterests = AccrualsHelpers.accrualsForDown(
                position.detached.borrowed,
                timeUntilMaturity,
                position.rate
            );
            return position.detached.borrowed + position.detached.expectedAccruals - unrealizedInterests;
        }
    }

    /**
     * @notice Get the current value of a new epoch position
     * @param tick Target tick
     * @param position Target position
     * @param currentLoan Current loan information
     * @return currentValue Current value of the position
     */
    function getNewEpochsPositionCurrentValue(
        DataTypes.Tick storage tick,
        DataTypes.Position storage position,
        DataTypes.Loan storage currentLoan
    ) private view returns (uint256) {
        ...
        if (block.timestamp > currentLoan.maturity) {
            ...
            return
                ...
                (epoch.accruals + lateRepayFees).mulDown(position.baseDeposit).divDown(epoch.deposited);
        }
        // if current timestamp is before maturity, the position is only entitled to part of its accruals
        else {
            uint256 timeUntilMaturity = currentLoan.maturity - block.timestamp;
            // while we are generally trying to minimise the global position value, which would mean maximising
            // the unrealised interests here, that does not work in this case since we are already minimising the accruals
            // minimising the accruals while maximising the unrealized interests would create underflows
            uint256 unrealizedInterests = AccrualsHelpers.accrualsForDown(
                epoch.borrowed,
                timeUntilMaturity,
                position.rate
            ...
            return
                position.baseDeposit +
                (epoch.accruals - unrealizedInterests).mulDown(position.baseDeposit).divDown(epoch.deposited);
        }
    }

    /**
     * @notice Get the current value of a base epoch position
     * @param tick Target tick
     * @param position Target position
     * @param currentLoan Current loan information
     * @return currentValue Current value of the position
     */
    function getBaseEpochPositionCurrentValue(
        DataTypes.Tick storage tick,
        DataTypes.Position storage position,
        DataTypes.Loan storage referenceLoan,
        DataTypes.Loan storage currentLoan
        ...
        if (referenceTimestamp > currentLoan.maturity) {
            uint256 lateRepayFeesYieldFactorDelta = AccrualsHelpers.calculateYieldFactorIncrease(
                tick,
        ...
        // if current timestamp is before maturity, the position is only entitled to part of its accruals
            ...
            // even though we generally want to minise yield factor, and here that would bring us to
            // maximise its decrease, this does not work here because it creates underflows when exiting
            // since we generally minimise the yield factor's increase, we also need to minimise its
            // decrease here not to create any potential problems
            uint256 unrealizedYieldFactorIncrease = AccrualsHelpers.calculateYieldFactorIncrease(
                tick,
        ...
        return getAdjustedAmount(tick, position, referenceLoan).mulDown(newYieldFactor, RAY);
    }

    /*//////////////////////////////////////////////////////////////
                                POSITION START AND END OF LOAN VALUES
    //////////////////////////////////////////////////////////////*/
     ...
     * @dev A position start of loan value being mainly used to compute accruals by being substracted, results are rounded up
     * @param tick Target tick
     * @param position Target position
     * @param referenceLoan Either first loan or detach loan of the position
     * @param currentLoan Current loan information
    ...
    function getPositionStartOfLoanValue(
        ...
        if (position.detachLoanId > 0) {
            bool detachLoanRepaid = (position.detachLoanId < currentLoan.id) || currentLoan.maturity == 0;
            if (!detachLoanRepaid) return position.detached.borrowed;
        }
        if ((position.epochId < tick.loanStartEpochId) || position.detached.borrowed > 0) {
            uint256 precedingLoanId = tick.epochs[tick.loanStartEpochId].precedingLoanId;
            positionValue = getAdjustedAmount(tick, position, referenceLoan).mulUp(
     ...
     * @dev A position end of loan value being mainly used to compute accruals, results are rounded down
     * @param tick Target tick
     * @param position Target position
     * @param referenceLoan Either first loan or detach loan of the position
     * @param currentLoan Current loan information
    ...
    function getPositionEndOfLoanValue(
        ...
        if (position.detachLoanId > 0) {
            bool detachLoanRepaid = (position.detachLoanId < currentLoan.id) || currentLoan.maturity == 0;
            if (!detachLoanRepaid) return position.detached.borrowed + position.detached.expectedAccruals;
        }

        if ((position.epochId <= tick.loanStartEpochId) || position.detached.borrowed > 0) {
            return getAdjustedAmount(tick, position, referenceLoan).mulDown(tick.yieldFactor, RAY);
        }

        DataTypes.Epoch storage epoch = tick.epochs[position.epochId];
        uint256 endOfLoanInterest = epoch.accruals.mulDown(position.baseDeposit).divDown(epoch.deposited);
        return position.baseDeposit + endOfLoanInterest;
    }
     ...
     * @dev A position unborrowed amounts being mainly used to compute withdrawals, results are rounded down
     * @param tick Target tick
     * @param position Target position
     * @param referenceLoan Either first loan or detach loan of the position
     * @param currentLoan Current loan information
    ...
    function getPositionRepartition(
        ...
        DataTypes.Loan storage referenceLoan,
        DataTypes.Loan storage currentLoan
    ) public view returns (uint256 unborrowedAmount, uint256 borrowedAmount) {
        // current epoch position
        if (tick.currentEpochId == 0) return (position.baseDeposit, 0);

        // position opted out during a preceding loan
        if (position.optOutLoanId > 0) {
            bool optOutLoanRepaid = (position.optOutLoanId < currentLoan.id) || currentLoan.maturity == 0;
            if (optOutLoanRepaid) {
                unborrowedAmount = getAdjustedAmount(tick, position, referenceLoan).mulDown(
                    tick.endOfLoanYieldFactors[position.optOutLoanId],
                    RAY
                );
        ...
        // position detached during current loan
        if (position.detachLoanId > 0) {
            bool detachLoanRepaid = (position.detachLoanId < currentLoan.id) || currentLoan.maturity == 0;
            if (!detachLoanRepaid) return (0, position.detached.borrowed);
        }

        // unborrowed positions
        if (currentLoan.maturity == 0 || tick.latestLoanId < currentLoan.id) {
            uint256 adjustedAmount = getAdjustedAmount(tick, position, referenceLoan);
            unborrowedAmount = adjustedAmount.mulDown(tick.yieldFactor, RAY);
            return (unborrowedAmount, 0);
        }

        // borrowed base epoch positions
        if (position.epochId <= tick.loanStartEpochId) {
            uint256 adjustedAmount = getAdjustedAmount(tick, position, referenceLoan);
            unborrowedAmount = tick.baseEpochsAmounts.available.mulDown(adjustedAmount).divDown(
                tick.baseEpochsAmounts.adjustedDeposits
            );
            borrowedAmount = tick.baseEpochsAmounts.borrowed.mulUp(adjustedAmount).divUp(
                tick.baseEpochsAmounts.adjustedDeposits
            );
            return (unborrowedAmount, borrowedAmount);
        }

        // borrowed and unborrowed new epoch positions
        DataTypes.Epoch storage epoch = tick.epochs[position.epochId];
        if (epoch.deposited == 0) return (0, 0);
        uint256 epochAvailable = epoch.borrowed > epoch.deposited ? 0 : epoch.deposited - epoch.borrowed;
        unborrowedAmount = epochAvailable.mulDown(position.baseDeposit).divDown(epoch.deposited);
        borrowedAmount = epoch.borrowed.mulUp(position.baseDeposit).divUp(epoch.deposited);
        return (unborrowedAmount, borrowedAmount);
    }
    ...
    /**
     * @notice Get the position start of loan, current and end of loan values in the context of an exit
     * @param tick Target tick
     * @param position Target position
     * @param referenceLoan Either first loan or detach loan of the position
     * @param currentLoan Current loan information
     * @param exitProportion Percentage of the position to exit
     * @return currentPositionValue Current value of the position
     * @return startOfLoanPositionValue Value of the position at the start of the current loan
     * @return endOfLoanPositionValue Value of the position at the end of the current loan
     */
    ...
    {
        currentPositionValue = getPositionCurrentValue(tick, position, referenceLoan, currentLoan).mulDown(
            exitProportion
        );
        startOfLoanPositionValue = getPositionStartOfLoanValue(tick, position, referenceLoan, currentLoan).mulUp(
            exitProportion
        );
        endOfLoanPositionValue = getPositionEndOfLoanValue(tick, position, referenceLoan, currentLoan).mulDown(
            exitProportion
        );
    }
}
...
library RCLValidation {
     ...
     * @notice Validation of the parameters used for the pool deployment
     * @dev Parameters are packed in bytes to improve readability and prevent stack too deep errors
     * @param parametersConfig Parameters related to the pool configuration
     */
    function validateDeployment(bytes memory parametersConfig) public pure {
        if (parametersConfig.length != 256) revert INVALID_PRODUCT_PARAMS();

        (, uint256 minRate, uint256 maxRate, uint256 rateSpacing, , uint256 minDepositAmount, ) = abi.decode(
            parametersConfig,
...
interface IRCLBorrowers {
     ...
     * @param borrowedAmount Amount of funds borrowed against the order book
     ...
     *
     * Emits a {Borrowed} event and one {TickBorrowed} event per borrowed tick
     * @dev TickBorrowed event is declared in BorrowerLogic library
     */
     ...
     *
     * Emits a {Repaid} event and one {TickRepaid} event per repayed tick
     * @dev TickRepaid event is declared in BorrowerLogic library
     */
    function repay() external;
}
...
interface IRCLLenders {
    ...
    /**
     * @notice Emitted when the unborrowed part of a borrowed position is detached
     * @param positionId ID of the position
     * @param receivedAmount Amount of funds received by the position owner
     * @param managementFees Amount of fees taken
     */
     ...
     * @param movedAmount Amount moved to the new rate
     */
    ...
    /**
     * @notice Emitted when a position is fully exited
     * @dev Partial exits are only possible for fully matched positions
     * @param positionId ID of the position
     * @param isFullyExited True if the position is fully exited, false if it is partially exited
     * @param unborrowedAmount Withdrawn unborrowed amount
     ...
     *
     * Emits a {Exited} event and one {ExitedAmountRedistributedInTick} event per recipient tick
     * @dev ExitedAmountRedistributedInTick event is declared in LenderLogic library
     */
...
interface IRCLState {
     ...
     * @return precedingLoanId Id of the loan before that epoch loan
     * @return positionsCount Number of positions in the epoch
     */
     ...
     * @return positionsCount Count of positions in base epochs
     */
     ...
     * @return positionsCount Count of positions in new epochs
     */
...
import '../../../libraries/FixedPointMathLib.sol';
import '../../../libraries/TimeValue.sol';
import '../libraries/BorrowerLogic.sol';
 ...
 */
abstract contract RCLBorrowers is IRCLBorrowers, RCLState {
    /*//////////////////////////////////////////////////////////////
                                LIBRARIES
    //////////////////////////////////////////////////////////////*/
     ...
     */
    function borrow(address to, uint256 toBeBorrowed) external onlyBorrower onlyInPhase(DataTypes.PoolPhase.OPEN) {
        validateBorrow(toBeBorrowed, to);
        ...
        uint256 toBeRepaid = BorrowerLogic.borrow(ticks, currentLoan, toBeBorrowed, MIN_RATE, MAX_RATE, RATE_SPACING);

        amendGlobalAmountsOnBorrow(toBeBorrowed, toBeRepaid);

        uint256 fees = FEES_CONTROLLER.registerBorrowingFees(toBeBorrowed);
        FundsTransfer.chargedWithdraw({
            token: TOKEN,
            custodian: CUSTODIAN,
            recipient: to,
            amount: toBeBorrowed - fees,
            fees: fees
        });

        emit Borrowed(toBeBorrowed - fees, fees, to);
    }

    /**
     * @inheritdoc IRCLBorrowers
     */
    function repay() external onlyBorrower notInPhase(DataTypes.PoolPhase.NON_STANDARD) {
        validateRepay();

        (uint256 fees, uint256 lateRepayFees) = BorrowerLogic.repay(
            ticks,
            currentLoan,
            totalBorrowed,
            LATE_REPAYMENT_FEE_RATE,
        ...
        emit Repaid(toBeRepaid - fees, fees);
    }
    ...
    function validateBorrow(uint256 amount, address to) internal view {
        ...
        if (amount < minDepositAmount) revert RevolvingCreditLineErrors.RCL_BORROW_AMOUNT_TOO_LOW();
        if (!rolesManager.isBorrower(to)) revert RevolvingCreditLineErrors.RCL_ONLY_BORROWER();
    }

    function validateRepay() internal view {
        if (currentLoan.maturity == 0) revert RevolvingCreditLineErrors.RCL_NO_LOAN_RUNNING();
    ...
    function amendGlobalsOnRepay() internal {
        ...
        loans[currentLoan.id].repaymentFeesRate = FEES_CONTROLLER.getRepaymentFeesRate();
        currentLoan.maturity = 0;
        totalBorrowed = 0;
        totalToBeRepaid = 0;
    }
     ...
     * @param toBeRepaid Amount to be repaid
     */
    function amendGlobalAmountsOnBorrow(uint256 borrowed, uint256 toBeRepaid) internal {
        totalToBeRepaid += toBeRepaid;
 ...
 */
abstract contract RCLGovernance is IRCLGovernance, RCLState {
    /*//////////////////////////////////////////////////////////////
                              LIBRARIES
    //////////////////////////////////////////////////////////////*/
     ...
     */
    function close() external onlyGovernance onlyInPhase(DataTypes.PoolPhase.OPEN) {
        poolPhase = DataTypes.PoolPhase.CLOSED;
        emit Closed();
    }
     ...
     * @inheritdoc IRCLGovernance
     */
    function open() external onlyGovernance onlyInPhase(DataTypes.PoolPhase.CLOSED) {
        poolPhase = DataTypes.PoolPhase.OPEN;
        emit Opened();
    }
    ...
    function setFeesController(IFeesController feesController)
        external
        onlyGovernance
        onlyInPhase(DataTypes.PoolPhase.OPEN)
    {
        address managedPool = feesController.getManagedPool();
    ...
    function startNonStandardRepaymentProcedure(INonStandardRepaymentModule nonStandardRepaymentModule, uint256 delay)
        external
        onlyGovernance
        notInPhase(DataTypes.PoolPhase.NON_STANDARD)
    {
        if (delay < NON_STANDARD_REPAY_MIN_TIMELOCK_DELAY) revert TIMELOCK_DELAY_TOO_SMALL();
        if (
            !IERC165(address(nonStandardRepaymentModule)).supportsInterface(
    ...
    function executeTimelock() external onlyGovernance {
        ...
        poolPhase = DataTypes.PoolPhase.NON_STANDARD;
...
import '../libraries/Errors.sol';
import '../libraries/LenderLogic.sol';
 ...
 *         Positions are created according to associated ERC721 token
 */
abstract contract RCLLenders is IRCLLenders, RCLState, SolmateERC721 {
    /*//////////////////////////////////////////////////////////////
                                LIBRARIES
    //////////////////////////////////////////////////////////////*/
    ...
    function deposit(
        ...
        uint256 amount,
        address to
    ) external onlyLender onlyInPhase(DataTypes.PoolPhase.OPEN) returns (uint256 positionId) {
        validateDeposit(amount, to, rate);

        LenderLogic.deposit(ticks[rate], currentLoan, amount);
        ...
        positions[positionId] = DataTypes.Position({
            ...
            optOutLoanId: 0,
            detachLoanId: 0,
            detached: DataTypes.PositionDetachedAmounts({borrowed: 0, expectedAccruals: 0})
        });
     ...
     * @inheritdoc IRCLLenders
     */
    function updateRate(uint256 positionId, uint256 newRate) external onlyLender onlyInPhase(DataTypes.PoolPhase.OPEN) {
        ...
        validateUpdateRate(tick, positionId, referenceLoan, currentLoan, newRate);

        uint256 movedAmount = LenderLogic.updateRate({
            position: position,
            tick: tick,
        ...
        emit RateUpdated(positionId, newRate, movedAmount);
    }
        ...
        notInPhase(DataTypes.PoolPhase.NON_STANDARD)
    {
        ...
        uint256 withdrawableAmount = LenderLogic.withdraw(tick, position, amountToWithdraw, referenceLoan, currentLoan);
     ...
     * @inheritdoc IRCLLenders
     */
    function detach(uint256 positionId) public onlyLender notInPhase(DataTypes.PoolPhase.NON_STANDARD) {
        DataTypes.Position storage position = positions[positionId];
        DataTypes.Tick storage tick = ticks[position.rate];
        DataTypes.Loan storage referenceLoan = getPositionLoan(position, tick);

        address positionOwner = ownerOf(positionId);
        LenderLogic.validateDetach(tick, position, referenceLoan, currentLoan, minDepositAmount, positionOwner);

        uint256 unborrowedAmount = LenderLogic.detach(tick, position, referenceLoan, currentLoan);
     ...
     * @inheritdoc IRCLLenders
     */
    function optOut(uint256 positionId) external onlyLender onlyInPhase(DataTypes.PoolPhase.OPEN) {
        ...
        LenderLogic.validateOptOut(tick, position, referenceLoan, currentLoan);

        LenderLogic.optOut({tick: tick, position: position, referenceLoan: referenceLoan, currentLoan: currentLoan});
        ...
        onlyInPhase(DataTypes.PoolPhase.OPEN)
    {
        ...
        (
            ...
            uint256 unborrowedAmount
        ) = LenderLogic.prepareExit(tick, position, borrowedAmountToExit, referenceLoan, currentLoan);

        uint256 totalAccrualsInterests;
        (totalAmountReborrowed, totalAccrualsInterests) = LenderLogic.exit(
            ticks,
            currentLoan,
            endOfLoanBorrowEquivalent,
            totalAmountReborrowed,
            MIN_RATE,
            MAX_RATE,
            RATE_SPACING
        );
        ...
        if (borrowedAmountToExit == type(uint256).max) {
            burn(positionId);
            emit Exited(positionId, true, unborrowedAmount, actualBorrowedAmountToExit, toBeWithdrawn - fees, fees);
        } else {
            emit Exited(positionId, false, unborrowedAmount, actualBorrowedAmountToExit, toBeWithdrawn - fees, fees);
        }
    }
    ...
    {
        uint256 loanId = position.detachLoanId > 0 ? position.detachLoanId : tick.epochs[position.epochId].loanId;
        loan = loans[loanId];
    }
...
import './../libraries/DataTypes.sol';
import './../libraries/Errors.sol';
import './../libraries/RCLValidation.sol';
import './interfaces/IRCLState.sol';

/**
 * @title RCLState
 * @author Atlendis Labs
 * @notice Implementation of the IRCLState
 *         Contains the core storage of the pool and shared methods accross the modules
 */
abstract contract RCLState is IRCLState, Managed {
    /*//////////////////////////////////////////////////////////////
                                STORAGE
    //////////////////////////////////////////////////////////////*/
    ...
    DataTypes.PoolPhase public poolPhase;
    ...
    constructor(
        ...
        RCLValidation.validateDeployment(parametersConfig);
        ...
        )
    {
        ...
        precedingLoanId = tick.epochs[epochId].precedingLoanId;
        positionsCount = tick.epochs[epochId].positionsCount;
    }
    ...
    function getTickDetachedAmounts(uint256 rate)
        public
        view
        returns (
            uint256 toBeAdjusted,
        ...
        )
    {
        ...
        return (tick.detachedAmounts.toBeAdjusted, tick.detachedAmounts.borrowed, tick.detachedAmounts.positionsCount);
    }
     ...
     * @param expectedPhase Expected phase
     */
    modifier onlyInPhase(DataTypes.PoolPhase expectedPhase) {
        if (poolPhase != expectedPhase) revert RevolvingCreditLineErrors.RCL_INVALID_PHASE();
        _;
    }
     ...
     */
    modifier notInPhase(DataTypes.PoolPhase excludedPhase) {
        if (poolPhase == excludedPhase) revert RevolvingCreditLineErrors.RCL_INVALID_PHASE();
        _;
    }
}
 ...
 */
contract RevolvingCreditLine is IRevolvingCreditLine, RCLState, RCLGovernance, RCLBorrowers, RCLLenders {
    /*//////////////////////////////////////////////////////////////
                                LIBRARIES
    //////////////////////////////////////////////////////////////*/
    ...
    constructor(
        ...
        string memory name,
        string memory symbol
    ) RCLLenders(name, symbol) RCLState(rolesManager, custodian, feesController, parametersConfig) {}
        ...
        returns (uint256 unborrowedAmount, uint256 borrowedAmount)
    {
        ...
        DataTypes.Tick storage tick = ticks[position.rate];
        DataTypes.Loan storage referenceLoan = getPositionLoan(position, tick);
        return PositionHelpers.getPositionRepartition(tick, position, referenceLoan, currentLoan);
    }
    ...
    function getPositionCurrentValue(uint256 positionId) public view returns (uint256 positionCurrentValue) {
        ...
        positionCurrentValue = PositionHelpers.getPositionCurrentValue(tick, position, referenceLoan, currentLoan);
    }
    ...
    function getCurrentAccruals() external view returns (uint256 currentAccruals) {
        ...
        if (currentLoan.maturity > block.timestamp) {
            timeUntilMaturity = currentLoan.maturity - block.timestamp;
        }
