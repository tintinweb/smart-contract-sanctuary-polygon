...
import "./PolyArtifacts.sol";
import "./Corn.sol";

contract HenRaids is Ownable, VRFConsumerBase {
  bytes32 internal keyHash;
  uint256 internal fee;
  FarmStaker staker;
  FarmCrops crops;
  Corn corn;
  PolyArtifacts artifacts;
  AggregatorInterface ethFeed;
  AggregatorInterface maticFeed;
  IERC20 weth;

  mapping(address=>mapping(address=>bool)) public isApprovedForAll;
  uint256 public baseHeistPrice = 2 * 10 ** 8;
  uint256 public increment = 2 * 10 ** 8;
  uint256 public maxAttempts = 5;
  uint256 public minStealAmount = 20;
  uint256 public maxStealAmount = 40;

  struct RaidCount {
    uint128 lastRaidTimestamp;
    uint128 amount;
  }
  //      account=>        landId=>         farmId=> amount of dailyRaids
  mapping(address=>mapping(uint256=>mapping(uint256=>RaidCount))) public farmRaids;
  ...
  mapping(bytes32=>Raid) public raids;

  event RaidEvent(address indexed raider, uint64 indexed landId, uint8 indexed farmId, bool success, address raided, uint64 raidedLandId, uint8 raidedFarmId, uint256 amount);

  constructor(address _crops, address _staker, address _artifacts, address _corn, address _weth, address _vrf, address _link, bytes32 _keyHash, uint256 _fee, address _ethFeed, address _maticFeed) VRFConsumerBase(_vrf, _link) {
    crops = FarmCrops(_crops);
    staker = FarmStaker(_staker);
    artifacts = PolyArtifacts(_artifacts);
    corn = Corn(_corn);
    keyHash = _keyHash;
    fee = _fee;
    weth = IERC20(_weth);
  ...
  function raidCost(address account, uint256 landId, uint256 farmId) public view returns (uint256, uint256, uint256, uint256) {
    RaidCount storage r = farmRaids[account][landId][farmId];
    uint256 amount = (block.timestamp - r.lastRaidTimestamp > 24 hours) ? 0 : r.amount;
    uint256 usdCost = baseHeistPrice + increment * amount;
    return (usdCost * 10 ** 10, ETHPrice(usdCost), MATICPrice(usdCost), amount);
  }
  ...
  function raid(address account, uint256 landId, uint256 farmId) public payable {
    require(account == msg.sender || isApprovedForAll[account][msg.sender], "Unauthorized");
    uint256 hensAmount = staker.henCount(account, landId, farmId);
    require(hensAmount > 1, "You must have enough hens to raid");
    (, uint256 eth, uint256 matic, uint256 amount) = raidCost(account, landId, farmId);

    if (msg.value > 0) {
      require(msg.value >= matic, "You must pay the correct amount of MATIC");
    } else {
      uint256 allowance = weth.allowance(msg.sender, address(this));
      uint256 balance = weth.balanceOf(msg.sender);
      require(allowance >= eth && balance >= eth, "You must pay the correct amount of ETH");
      weth.transferFrom(msg.sender, address(this), eth);
    }
    farmRaids[account][landId][farmId] = RaidCount(uint128(block.timestamp), uint8(amount + 1));

    bytes32 requestId = requestRandomness(keyHash, fee);
    raids[requestId] = Raid(account, uint64(landId), uint8(farmId));
  }

  function calculateFarmDefense(address account, uint256 landId, uint256 farmId) public view returns (uint256) {
    address[] memory copiesOfArtifacts = new address[](4);
    copiesOfArtifacts[0] = address(artifacts);
    copiesOfArtifacts[1] = address(artifacts);
    copiesOfArtifacts[2] = address(artifacts);
    copiesOfArtifacts[3] = address(artifacts);
    ...
    uint256[] memory balances = staker.balanceOfBatch(account, landId, farmId, copiesOfArtifacts, tokenIds);
    uint256 scarecrowLevel = 0;

    uint256[5] memory scarecrowProtection = [uint256(0), 3, 5, 7, 10];

    for (uint256 i = balances.length - 1; i >= 0; i--) {
      if (balances[i] > 0) {
        scarecrowLevel = i + 1;
        break;
      }
    ...
    uint256 foxdenLevel = staker.getBuildingLevel(account, landId, farmId, uint256(BuildingTypes.FOXDEN));
    return foxdenLevel * 5 + (scarecrowProtection[scarecrowLevel]);
  }

  function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
    Raid memory r = raids[requestId];
    uint256 cropsLength = crops.cropsLength();
    uint256 hensAmount = staker.henCount(r.account, uint256(r.landId), uint256(r.farmId));

    for (uint256 i = 0; i < maxAttempts + hensAmount; i++) {
      uint256 index = uint256(keccak256(abi.encode(randomness, i))) % cropsLength;
      (FarmCrops.Crop memory a, FarmCrops.PlotStatus memory b) = crops.getCrop(index);
      if (b.stolen > 0) {
        continue;
      }
      uint256 henhouseLevel = staker.getBuildingLevel(r.account, uint256(r.landId), uint256(r.farmId), uint256(BuildingTypes.HENHOUSE));

      uint256 defense = calculateFarmDefense(a.account, uint256(a.landId), uint256(a.farmId));
      uint256 attack = uint256(keccak256(abi.encode(randomness, 100 + i))) % 100;

      if (defense > attack) {
        emit RaidEvent(r.account, r.landId, r.farmId, false, a.account, a.landId, a.farmId, 0);
        return;
      }

      uint256 stealPercentage = minStealAmount + (maxStealAmount - minStealAmount) * henhouseLevel / 10;
      uint256 totalCrop = crops.seedsPerLine() * crops.multiplier();
      uint256 stealAmount = stealPercentage * totalCrop / 100;

      crops.getRaided(a.account, a.landId, a.farmId, a.plot, stealPercentage);
      corn.mint(a.account, stealAmount);
      emit RaidEvent(r.account, r.landId, r.farmId, true, a.account, a.landId, a.farmId, stealAmount);
    }
  }
  ...
  function setBaseHeistPrice(uint256 _p) public onlyOwner {
    baseHeistPrice = _p;
  }

  function setIncrement(uint256 _i) public onlyOwner {
    increment = _i;
  }
   
  function setMaxAttempts(uint256 _m) public onlyOwner {
    maxAttempts = _m;
  }

  function setStealAmounts(uint256 min, uint256 max) public onlyOwner {
    minStealAmount = min;
    maxStealAmount = max;
  }
}
...
contract Realtor is Handlers {
  ...
  mapping(uint256=>mapping(uint256=>uint256)) rentPerFarm;
  ...
  function leaveFarm(uint256 landId, uint256 farmId) public {
    ...
    eggs.transferFrom(address(this), msg.sender, deposits[landId][farmId]);
...
contract PolyArtifacts is Ownable, ERC1155, Handlers {
  ...
  function mint(address account, uint256 tokenId, uint16 amount) public isHandler {
...
contract HenHouseS2 is Ownable {
  ...
  function claimEggsBatch(uint64 landId, uint8 farmId, bytes32[] calldata keys, bytes32 henHouseKey) public returns (uint256) {
    ...
    eggs.transferFrom(address(this), msg.sender, total);
