...
/// @title Utility Marketplace
pragma solidity ^0.8.4;
import "@openzeppelin/contracts/token/ERC20/ERC20.sol";
import "@openzeppelin/contracts/utils/Counters.sol";
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/token/ERC721/utils/ERC721Holder.sol";
import "./UtilityArtNFT.sol";
import "./UtilityArtFactory.sol";

contract UtilityArtMarketplace is Ownable, ERC721Holder {
    uint256 private platformFee;
    address private feeRecipient;
    UtilityArtFactory utilityArtFactory;
    struct ListNFT {
        address nft;
        uint256 tokenId;
        address seller;
    ...
    constructor(
        ...
        address _utilityArtFactoryAddr
    ) {
        platformFee = _platformFee;
        feeRecipient = _feeRecipient;
        utilityArtFactory = UtilityArtFactory(_utilityArtFactoryAddr);
    }
    ...
    function list(
        ...
        require(
            ...
            "Caller needs to be owner of NFT"
        );
     ...
     */
    function buy(address _nft, uint256 _tokenId)
        external
        payable
        isListedNFT(_nft, _tokenId)
    {
        ...
        require(
            ...
            "Submitted value less than item price"
        );
        listedNft.sold = true;
        UtilityArtNFT nft = UtilityArtNFT(listedNft.nft);
        address royaltyRecipient = nft.getRoyaltyRecipient();
        uint256 royaltyFee = nft.getRoyaltyFee();
        uint256 remaningAmount = msg.value;
        if (royaltyFee > 0) {
            uint256 royaltyTotal = calculateRoyalty(royaltyFee, msg.value);
            // Transfer royalty fee to collection owner / royalty Recipient
            payable(royaltyRecipient).transfer(royaltyTotal);
            remaningAmount -= royaltyTotal;
        }
        ...
        remaningAmount -= platformFeeTotal;
        // Transfer to nft owner
        payable(listedNft.seller).transfer(remaningAmount);
     ...
     * @param _tokenId NFT id
     */
    function placeBid(address _nft, uint256 _tokenId)
        external
        payable
        isAuction(_nft, _tokenId)
    {
        ...
        if (auction.lastBidder != address(0)) {
            ...
            (bool os, ) = lastBidder.call{value: lastBidPrice}(""); // balance transfer to lastBidder
            if (os) {
                emit AmountReleased(_nft, _tokenId, lastBidPrice, lastBidder);
            }
        }
        // Set new heighest bid price
        auction.lastBidder = msg.sender;
        auction.heighestBid = msg.value;
        emit PlacedBid(_nft, _tokenId, msg.value, msg.sender);
    }
    ...
    function completeBid(address _nft, uint256 _tokenId) external {
        ...
        UtilityArtNFT utilityArt = UtilityArtNFT(_nft);
        address royaltyRecipient = utilityArt.getRoyaltyRecipient();
        uint256 royaltyFee = utilityArt.getRoyaltyFee();
        uint256 remaningAmount = auction.heighestBid;
        if (royaltyFee > 0) {
        ...
        payable(auction.creator).transfer(remaningAmount);
...
contract UtilityArtNFT is ERC721Enumerable, Ownable {
...
contract UtilityArtFactory {
    // owner address => nft list
    mapping(address => address[]) private nfts;

    mapping(address => bool) private utilityNFT;
    ...
    function createNFTCollection(
        ...
        UtilityArtNFT nft = new UtilityArtNFT(
            _name,
            _symbol,
            _initBaseURI,
            _maxSupply,
    ...
    function isUtilityNFT(address _nft) external view returns (bool) {
        return utilityNFT[_nft];
    }
}

interface IUtilityArtFactory {
    function createNFTCollection(
        string memory _name,
        string memory _symbol,
        string memory _initBaseURI,
