    ...
    IFiboTransferAgent
{
    ...
    event AssetReconfigured(address indexed asset);
    ...
    function registerAsset(address asset, AssetConfig calldata config)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        if (assets[asset].registered) {
            emit AssetReconfigured(asset);
        } else {
            emit AssetRegistered(asset);
        }

        assets[asset].registered = true;
    }
    ...
    function validateGuards(AssetGuardConfig[] memory guards) private {
        for (uint256 i = 0; i < guards.length; ++i) {
            ITransferGuard guard = ITransferGuard(guards[i].guardContract);
            (bool success, bytes memory returnData) = guards[i].guardContract.call(
            ...
            require(
                ...
                string(
                    abi.encodePacked(
                        StringsUpgradeable.toHexString(uint160(guards[i].guardContract), 20),
