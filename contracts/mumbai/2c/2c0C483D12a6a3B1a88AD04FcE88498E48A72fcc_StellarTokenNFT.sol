...
contract StellarTokenNFT is ERC721URIStorage {
    ...
    constructor(string memory name, string memory symbol, address payable _marketingWallet) ERC721(name, symbol) {
        levelImages.push("https://stellartokennft.s3.amazonaws.com/level_1.png");
        levelImages.push("https://stellartokennft.s3.amazonaws.com/level_2.png");
        levelImages.push("https://stellartokennft.s3.amazonaws.com/level_3.png");
        levelImages.push("https://stellartokennft.s3.amazonaws.com/level_4.png");
        levelImages.push("https://stellartokennft.s3.amazonaws.com/level_5.png");
        levelImages.push("https://stellartokennft.s3.amazonaws.com/level_6.png");
    ...
    function setStellarTokenContract(address _stellarTokenContract) external onlyOwner {
    ...
    function mintNFT(address recipient, uint256 quantity) external payable {
        ...
        for (uint256 i = 0; i < quantity; i++) {
            ...
            string memory backgroundColor = "#FFFFFF";
            ...
            tokenIdToMetadata[nextTokenId] = NFTMetadata(level, rarity, backgroundColor);
    ...
    function findHighestLevel(address owner) internal view returns (uint8) {
        EnumerableSet.UintSet storage tokens = ownedTokens[owner];
        uint8 highestLevel;
    ...
    function getOwnedTokens(address owner) external view returns (uint256[] memory) {
        uint256[] memory tokens = new uint256[](ownedTokens[owner].length());
        for (uint256 i = 0; i < ownedTokens[owner].length(); i++) {
            tokens[i] = ownedTokens[owner].at(i);
        }
        return tokens;
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        parts[1] = "<defs><style>@font-face { font-family: 'Press Start 2P'; src: url('https://stellartokennft.s3.amazonaws.com/PressStart2P-Regular.ttf'); }";
        parts[2] = ".base { font-family: 'Press Start 2P', serif; font-size: 12px; color: #fff} </style></defs>";

        parts[3] = string(abi.encodePacked('<image href="', levelImages[tokenIdToMetadata[tokenId].level-1], '" height="200" width="200" />'));

        parts[4] = '<text x="10" y="20" class="base">#: ';
        parts[5] = toString(tokenId);
        parts[6] = '</text><text x="10" y="35" class="base">Level: ';
        parts[7] = toString(tokenIdToMetadata[tokenId].level);
        parts[8] = '</text><text x="10" y="50" class="base">Rarity: ';
        parts[9] = tokenIdToMetadata[tokenId].rarity;
        parts[10] = '</text></svg>';
