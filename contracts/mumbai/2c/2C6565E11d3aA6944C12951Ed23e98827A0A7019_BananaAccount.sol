...
contract BananaAccount is Safe {
    ...
    event TokenReceivedEvent(address indexed token, address indexed from, uint256 value, string tokenName);
    ...
    function received(address token, address from, uint256 value, string memory tokenName) external {
        emit TokenReceivedEvent(token, from, value, tokenName);
    }
