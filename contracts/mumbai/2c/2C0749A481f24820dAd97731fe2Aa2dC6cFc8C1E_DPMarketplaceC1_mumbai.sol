...
import "./PriceConverter_mumbai.sol";
...
contract DPMarketplaceC1_mumbai is ERC721URIStorage, ReentrancyGuard {
  ...
  using PriceConverter_mumbai for uint256;
  ...
  constructor() ERC721("GoyaTB", "GOYATB") {
  ...
  function resellToken(
    ...
    if (_unlist){
      ...
      idToMarketItem[tokenId].referral = _referral;
  ...
  function createMarketSale(
    ...
    uint price2 = idToMarketItem[tokenId].sellpriceUSD.getLowUsdMatic();
    require(msg.value > price2, "asking price is lower than selling price");
...
library PriceConverter_mumbai {
    ...
    function getUsdMatic(uint256  _amount) internal view returns (uint256) {
        ...
        uint256  usd = _amount * 1e18;
        uint256  rate = (usd * 1e18) / adjust_price;
        return rate * 102 / 100; //include 2% markup
    }

    function getLowUsdMatic(uint256  _amount) internal view returns (uint256) {
        uint256 ethPrice = getPrice() ;
        uint256  adjust_price = uint256 (ethPrice) * 1e18;
        uint256  usd = _amount * 1e18;
        uint256  rate = (usd * 1e18) / adjust_price;
