...
contract RandomWinnerGame is VRFConsumerBase ,Ownable{
    ...
    function startGame() external payable {
        require(msg.value == entryFee, "Please send the correct entry fee");
        require(players.length < maxPlayers, "The maximum number of players has been reached");

        // Add the player to the list of players
        players.push(payable(msg.sender));
        emit PlayerJoined(gameId, msg.sender);

        // If the maximum number of players has been reached, choose a random winner
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        require(requestIdUsed[requestId], "This request ID was not used for the game");
