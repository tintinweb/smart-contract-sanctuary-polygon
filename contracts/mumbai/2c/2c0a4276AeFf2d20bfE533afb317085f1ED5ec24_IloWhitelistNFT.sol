...
/* @dev: Check if contract owner */
       ...
       require(totalNftsMinted > 0, "No NFTs have been minted");
        ...
        for (uint256 i = 0; i < totalNftsMinted; i++) {
            ...
            if (details[user].userWhitelistedIlos > 0) {
                ...
                if (numUsersToAddToWhiteList == batchSize) {
    ...
    if (numUsersToAddToWhiteList > 0) {
        address[] memory remainingUsers = new address[](numUsersToAddToWhiteList);
        for (uint256 i = 0; i < numUsersToAddToWhiteList; i++) {
            remainingUsers[i] = usersToAddToWhiteList[i];
        }
        iloContract.addToWhiteListBulk(remainingUsers);
    }
}
