...
contract Domains is ERC721URIStorage {
    ...
    // mapping domain names to a user address, to see who owns what names
    mapping(string => address) public domains;

    // mapping domain name to record values
    mapping(string => string) public records;

    mapping(uint => string) public names;

    constructor(string memory _tld) ERC721("Mango Name Service", "MNS") payable {
    ...
    // A register function that adds names to our domains mapping
    function register(string calldata _name) public payable {
        // check that the name the user is trying to register is unregistered
        /* require(domains[_name] == address(0), "Domain Name is already taken!"); */
        if (domains[_name] != address(0)) revert AlreadyRegistered();
        if (!valid(_name)) revert InvalidName(_name);

        uint domainPrice = price(_name);

        // check to see if enough Matic for the tx
        require(msg.value >= domainPrice, "Not enough MATIC for tx");


        // combine the name passed into the function w/ the tld
        string memory name = string(abi.encodePacked(_name, ".", tld));
        // create the svg image for the NFT w/ the name
        string memory finalSvg = string(abi.encodePacked(svgStart, name, svgEnd));

        uint256 newItemId = _tokenIds.current();
        uint256 length = StringUtils.strlen(_name);
        string memory strLen = Strings.toString(length);

        console.log("Registering %s.%s with tokenId %d", _name, tld, newItemId);
    ...
    // Get the address of a particular domain owner
    ...
    // require domain names to be between 3 and 10 characters
    ...
    function setRecord(string calldata _name, string calldata _record) public {
        ...
        /* require(domains[_name] == msg.sender, "Must own the domain to set the record!"); */
