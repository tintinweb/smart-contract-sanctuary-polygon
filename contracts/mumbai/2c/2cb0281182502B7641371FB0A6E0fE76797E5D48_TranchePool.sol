...
contract TranchePool is ITranchePool, TrancheStorage, OwnableUpgradeable, ERC721Upgradeable, ERC1155HolderUpgradeable {
  ...
  function _invest(uint256 amount, address account, bool isSeniorTranche) private {
    ...
    if (pool.totalAmountOfInvestment >= globalTrancheDetails.totalBalance + amount) {
      ...
      require(IERC20Upgradeable(pool.asset).transferFrom(account, address(this), amount - amountToBePutInQueue), 'ERC20 transfer fail');
      _mint(account, ++globalTrancheDetails.tokenId);
  ...
  function _claim(uint256 amount, bool isSeniorTranche, uint256 counter) private {
    ...
    if (isSeniorPool) {
      ...
      while (true) {
        ...
        if (srTranche.amountToRedeem > queuedAccount.pendingSwap) {
          IERC20Upgradeable(pool.asset).transferFrom(queuedAccount.account, account, queuedAccount.pendingSwap);
          ...
          IERC20Upgradeable(pool.asset).transferFrom(queuedAccount.account, account, srTranche.amountToRedeem);
      ...
      while (true) {
        ...
        if (jrTranche.amountToRedeem > queuedAccount.pendingSwap) {
          ...
          IERC20Upgradeable(pool.asset).transferFrom(queuedAccount.account, account, jrTranche.amountToRedeem);
