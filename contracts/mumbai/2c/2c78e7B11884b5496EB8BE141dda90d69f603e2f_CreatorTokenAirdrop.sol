...
contract CreatorTokenAirdrop is Ownable, ReentrancyGuard {
    ...
    function addAirdrop(
        ...
        if (airdrops[_token][_to].amount == 0) {
            ...
            airdrops[_token][_to].token = _token;
            airdrops[_token][_to].beneficiary = _to;
            airdrops[_token][_to].dateAdded = block.timestamp;
    ...
    function getAirdrops() external view returns (Airdrop[] memory) {
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            for (uint256 j = 0; j < beneficiaries.length; j++) {
                token = airdrops[tokens[i]][beneficiaries[j]].token;
                beneficiary = airdrops[tokens[i]][beneficiaries[j]].beneficiary;
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            for (uint256 j = 0; j < beneficiaries.length; j++) {
                ...
                if (token != address(0) && beneficiary != address(0)) {
                    airdropArray[idx] = airdrops[tokens[i]][beneficiaries[j]];
                    idx++;
                }
            }
        }
    ...
    function getAirdropsByBeneficiary(address _beneficiary)
        external
        view
        returns (Airdrop[] memory)
    {
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            token = airdrops[tokens[i]][_beneficiary].token;
            beneficiary = airdrops[tokens[i]][_beneficiary].beneficiary;
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            if (token != address(0) && beneficiary != address(0)) {
                airdropArray[i] = airdrops[tokens[i]][_beneficiary];
            }
        }
        return airdropArray;
    }
