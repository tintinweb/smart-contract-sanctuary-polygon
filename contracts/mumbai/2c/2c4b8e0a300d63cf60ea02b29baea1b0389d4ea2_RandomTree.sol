...
library rarityChart{
    ...
    function getRarityChances(uint256 value)internal pure returns(uint256[4] memory){
        if(value <= 100){
            return getUnder100(value);       
        }
        if(100< value && value <= 1000){
            return getRarityMiddle(value);
        }else{
            return getRarityOver1000(value);
        }
...
contract RandomTree is VRFConsumerBaseV2, Ownable{
    ...
    uint8 rarity = pickRarity(randomWords[0], requestId);

    s_requestIdToNftAddress[requestId].setTree(s_requestIdToToken[requestId], rarity, randomWords[1]);

    }

    function pickRarity(uint256 random, uint256 requestId)internal view returns(uint8){
        uint256[4] memory a = s_requestIdToRarity[requestId];
        uint256 pick = random% a[3];
        uint8 b;
        for(uint8 i =0; i<3; i++){
            if(a[i] > pick){
                pick -= a[i];
