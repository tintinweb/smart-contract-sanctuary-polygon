...
contract DepositUSDPool is Ownable {
    ...
    constructor(address _token,address _treasuryAddress,uint256 _maxQuota,address _funAddress){
        token = _token;
    ...
    function recharge(uint256 amount) public {
        ...
        if(availableQuota>0){
            if(availableQuota>=amount){
                ...
                IERC20(token).transferFrom(msg.sender,treasury,availableQuota);
            }
        }else{
    ...
    function withdraw(address account,uint256 amount) public onlyController {
        IWallet(treasury).withdraw(token,account,amount);
