...
contract Crowdban4 {
    struct Item {
        string id;
        string stage;
        address taskCreator; // Added taskCreator field
        address assignedTo;
        uint256 reward;
        bool isMoved;
        bool isPaid;
    ...
    event ItemCreated(string id, string stage, address taskCreator, address assignedTo, uint256 reward);
    event ItemMoved(string id, string oldStage, string newStage, address taskCreator, address assignedTo, uint256 reward);
    ...
    function createItem(string memory id, string memory stage, address assignedTo) public payable {
        ...
        items.push(Item(id, stage, msg.sender, assignedTo, reward, false, false)); // Set taskCreator as the message sender
        assignedItems[assignedTo].push(items.length - 1);
        emit ItemCreated(id, stage, msg.sender, assignedTo, reward);
    ...
    function moveItem(string memory itemId, string memory newStage) public {
        ...
        require(msg.sender == item.taskCreator, "Only task creator can move item"); // Only task creator can move the item
