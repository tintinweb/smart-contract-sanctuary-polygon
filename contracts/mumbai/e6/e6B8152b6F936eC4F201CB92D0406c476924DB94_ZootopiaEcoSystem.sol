    ...
    ResourceImplementationPointer
{
    ...
    function compoundReward(uint256 _nftId, string memory _tokenURI)
        external
        nonReentrant
        onlyNFTOwner
        checkPermissions(_nftId)
        ...
        resourceSet
    {
        (
            ...
            uint256 feeAmount
        ) = _getNFTCompoundRewards(_nftId, _tokenURI);
        require(
            amountToCompound > 0,
    ...
    function compoundAll(string[] memory tokenURIs)
        external
        nonReentrant
        onlyNFTOwner
        whenNotPaused
        resourceSet
    {
        ...
        require(
            tokenURIs.length == nftsOwned.length,
            "NFTs: URI count is wrong."
        );
        ...
        for (uint256 i = 0; i < nftsOwned.length; i++) {
            (
                uint256 amountToCompound,
                uint256 feeAmount
            ) = _getNFTCompoundRewards(nftsOwned[i], tokenURIs[i]);
    ...
    function _getNFTCompoundRewards(uint256 _nftId, string memory _tokenURI)
        private
    ...
    function changeProcessingFee(uint256 _processingFee) public onlyOwner {
        require(_processingFee <= 30, "NFTs: Cashout fee can never exceed 30%");
