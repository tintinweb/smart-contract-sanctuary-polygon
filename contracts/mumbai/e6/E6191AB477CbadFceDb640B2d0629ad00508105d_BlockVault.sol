...
contract BlockVault is CoreCell, Vault {
    constructor(address core_) CoreCell("BlockAPI_BlockVault_V1", core_) Vault() {}
}
...
contract Vault {
    ...
    mapping(address => Depositor) _depositors;
    mapping(address => uint256) private _depositNative;
    mapping(address => mapping(address => uint256)) private _deposit;

    error ErrDepositAmount(address who, address depositContract, uint256 depositAmount);

    event NewDepositNativeToken(address who, uint256 depositAmount, uint256 depositTotal);
    event NewDeposit(address who, address depositContract, uint256 depositAmount, uint256 depositTotal);

    function deposit(address depositContract_, uint256 depositAmount_) external payable {
        if (depositAmount_ <= 0) {
            revert ErrDepositAmount(msg.sender, depositContract_, depositAmount_);  
        }

        _depositNativeToken(msg.value);
        _deposit[msg.sender][depositContract_] += depositAmount_;

        if (msg.value > 0) {
            emit NewDepositNativeToken(msg.sender, msg.value, _depositNative[msg.sender]);
        }

        emit NewDeposit(msg.sender, depositContract_, depositAmount_, _deposit[msg.sender][depositContract_]);
    }
