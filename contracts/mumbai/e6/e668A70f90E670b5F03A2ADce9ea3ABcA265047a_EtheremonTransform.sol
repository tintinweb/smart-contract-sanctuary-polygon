...
function debugTrade( uint64 _objId) public view returns (bool) {

         if (tradeContract != address(0)) {
            EtheremonTradeInterface trade = EtheremonTradeInterface(
                tradeContract
            );
            return trade.isOnTrading(_objId);
        }
...
function debugSender() public view returns ( address ) {
...
function debugLevelDet(uint64 _objId) public view returns (uint32 transformClass, uint8 transformLevel, uint8 level ) {
