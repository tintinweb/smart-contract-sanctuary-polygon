...
contract NODERewardManagement {
    ...
    function createNode(address account, string memory nodeName) external onlySentry {
        ...
        rewardAvailable: rewardPerNode  //they made this 0
        })
        );
...
contract ThorV2 is ERC20, Ownable, PaymentSplitter {
    ...
    uint256 maxSupply = 170000000 * (10**18);

    mapping(address => bool) public _isBlacklisted;
    mapping(address => bool) public automatedMarketMakerPairs;
    mapping(uint => NODERewardManagement ) public nodeRewardMangers;
    mapping(uint => bool) private nodeTypesUsed;

    event NodeManagerChanged(
        uint indexed nodeType,
        address  newNodeManager
    );
    ...
    constructor(
        ...
        uint256 swapAmount,
        address uniV2Router
    ) ERC20("THUTY", "TH31") PaymentSplitter(payees, shares) {
        ...
        // require(totalSupply() == 170000000000000000000000000, "CONSTR: totalSupply must equal 17 million");
        require(totalSupply() == 500000*(10**18), "CONSTR: Initial supply must equal  500000");

        require(swapAmount > 0, "CONSTR: Swap amount incorrect");
        swapTokensAmount = swapAmount * (10**18);
    }

    function getNodeManger(uint _nodetype) public view returns (NODERewardManagement){
        require(_nodetype!=0 && nodeTypesUsed[_nodetype] == true, "Valid node type required");
        return nodeRewardMangers[_nodetype];
    }

    function setNodeManager(address nodeManager, uint _nodetype) external onlyOwner{
        require(_nodetype!=0, "Node type can not be zero");
        nodeRewardMangers[_nodetype] = NODERewardManagement(nodeManager);
        nodeTypesUsed[_nodetype]  = true;
        emit NodeManagerChanged(_nodetype, nodeManager);
    ...
    function createNodeWithTokens(string memory name, uint256 _nodeType) public {
        require(
        ...
        NODERewardManagement nodeMangerToUse = getNodeManger(_nodeType);
        uint256 nodePrice = nodeMangerToUse.nodePrice();
        require(
        ...
        nodeMangerToUse.createNode(sender, name);
    }
    function _deliverRewardToNodeOwner(address sender, address recipient, uint256 amount) internal{
        //mint only if not enough
        uint256 senderBalance = super.balanceOf(sender);
        if (amount>senderBalance){
            uint256 diff = amount-senderBalance;
            //ensure it is not above max
            require(totalSupply()+diff <= maxSupply, "Max supply reached");
            super._mint(recipient, diff );
            super._transfer(sender,recipient, senderBalance);
    ...
    function cashoutReward(uint256 blocktime, uint _nodeType) public {
        address sender = _msgSender();
        ...
        uint256 rewardAmount = nodeMangerToUse._getRewardAmountOf(
            sender,
            blocktime
        );
        require(
        ...
        _deliverRewardToNodeOwner(distributionPool, sender, rewardAmount);
        nodeMangerToUse._cashoutNodeReward(sender, blocktime);
    }

    function cashoutAll(uint _nodeType) public {
        address sender = _msgSender();
        require(
            sender != address(0),
        ...
        uint256 rewardAmount = nodeMangerToUse._getRewardAmountOf(sender);
        require(
            rewardAmount > 0,
        ...
        _deliverRewardToNodeOwner(distributionPool, sender, rewardAmount);
        nodeMangerToUse._cashoutAllNodesReward(sender);
    }
    ...
    function getNodeNumberOf(address account, uint _nodeType) public view returns (uint256) {
        //todo all node managers operation
        NODERewardManagement nodeMangerToUse = getNodeManger(_nodeType);

        return nodeMangerToUse._getNodeNumberOf(account);
    }

    function getRewardAmountOf(address account, uint _nodeType)
    public
    view
    onlyOwner
    returns (uint256)
    {
        //todo all nodeManagers
        NODERewardManagement nodeMangerToUse = getNodeManger(_nodeType);

        return nodeMangerToUse._getRewardAmountOf(account);
    }

    function getRewardAmount(uint _nodeType) public view returns (uint256) {
        require(_msgSender() != address(0), "SENDER CAN'T BE ZERO");
        NODERewardManagement nodeMangerToUse = getNodeManger(_nodeType);

        require(
            nodeMangerToUse._isNodeOwner(_msgSender()),
            "NO NODE OWNER"
        );
        return nodeMangerToUse._getRewardAmountOf(_msgSender());
    }

    function changeNodePrice(uint256 newNodePrice, uint _nodeType) public onlyOwner {
        NODERewardManagement nodeMangerToUse = getNodeManger(_nodeType);

        nodeMangerToUse._changeNodePrice(newNodePrice);
    }

    function getNodePrice(uint _nodeType) public view returns (uint256) {
        NODERewardManagement nodeMangerToUse = getNodeManger(_nodeType);

        return nodeMangerToUse.nodePrice();
    }

    function changeRewardPerNode(uint256 newPrice, uint _nodeType) public onlyOwner {
        NODERewardManagement nodeMangerToUse = getNodeManger(_nodeType);

        nodeMangerToUse._changeRewardPerNode(newPrice);
    }

    function getRewardPerNode(uint _nodeType) public view returns (uint256) {
        NODERewardManagement nodeMangerToUse = getNodeManger(_nodeType);

        return nodeMangerToUse.rewardPerNode();
    }

    function changeClaimTime(uint256 newTime, uint _nodeType) public onlyOwner {
        NODERewardManagement nodeMangerToUse = getNodeManger(_nodeType);

        nodeMangerToUse._changeClaimTime(newTime);
    }

    function getClaimTime(uint _nodeType) public view returns (uint256) {
        NODERewardManagement nodeMangerToUse = getNodeManger(_nodeType);

        return nodeMangerToUse.claimTime();
    }

    function changeAutoDistri(bool newMode, uint _nodeType) public onlyOwner {
        NODERewardManagement nodeMangerToUse = getNodeManger(_nodeType);

        nodeMangerToUse._changeAutoDistri(newMode);
    }

    function getAutoDistri(uint _nodeType) public view returns (bool) {
        NODERewardManagement nodeMangerToUse = getNodeManger(_nodeType);

        return nodeMangerToUse.autoDistri();
    }

    function changeGasDistri(uint256 newGasDistri, uint _nodeType) public onlyOwner {
        NODERewardManagement nodeMangerToUse = getNodeManger(_nodeType);

        nodeMangerToUse._changeGasDistri(newGasDistri);
    }

    function getGasDistri(uint _nodeType) public view returns (uint256) {
        NODERewardManagement nodeMangerToUse = getNodeManger(_nodeType);

        return nodeMangerToUse.gasForDistribution();
    }

    function getDistriCount(uint _nodeType) public view returns (uint256) {
        NODERewardManagement nodeMangerToUse = getNodeManger(_nodeType);

        return nodeMangerToUse.lastDistributionCount();
    }

    function getNodesNames(uint _nodeType) public view returns (string memory) {
        //todo all nodes operation
        require(_msgSender() != address(0), "SENDER CAN'T BE ZERO");
        NODERewardManagement nodeMangerToUse = getNodeManger(_nodeType);
        ...
        return nodeMangerToUse._getNodesNames(_msgSender());
    }

    function getNodesCreatime(uint _nodeType) public view returns (string memory) {
        //todo all nodes operation
        require(_msgSender() != address(0), "SENDER CAN'T BE ZERO");
        NODERewardManagement nodeMangerToUse = getNodeManger(_nodeType);
        ...
        return nodeMangerToUse._getNodesCreationTime(_msgSender());
    }

    function getNodesRewards(uint  _nodeType) public view returns (string memory) {
        //todo all nodes operation
        require(_msgSender() != address(0), "SENDER CAN'T BE ZERO");
        NODERewardManagement nodeMangerToUse = getNodeManger(_nodeType);
        ...
        return nodeMangerToUse._getNodesRewardAvailable(_msgSender());
    }

    function getNodesLastClaims(uint _nodeType) public view returns (string memory) {
        require(_msgSender() != address(0), "SENDER CAN'T BE ZERO");
        NODERewardManagement nodeMangerToUse = getNodeManger(_nodeType);
        ...
        return nodeMangerToUse._getNodesLastClaimTime(_msgSender());
    }

    function distributeRewards(uint _nodeType)
    public
    onlyOwner
    returns (
        uint256,
    ...
    )
    {
        ...
        return nodeMangerToUse._distributeRewards();
    }

    function publiDistriRewards(uint _nodeType) public {
        NODERewardManagement nodeMangerToUse = getNodeManger(_nodeType);

        nodeMangerToUse._distributeRewards();
    }

    function getTotalStakedReward(uint _nodeType) public view returns (uint256) {
        NODERewardManagement nodeMangerToUse = getNodeManger(_nodeType);

        return nodeMangerToUse.totalRewardStaked();
    }

    function getTotalCreatedNodes(uint _nodeType) public view returns (uint256) {
        NODERewardManagement nodeMangerToUse = getNodeManger(_nodeType);

        return nodeMangerToUse.totalNodesCreated();
    }
}