...
contract PlushGetTree is Initializable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable, IPlushGetTree {
     ...
     */
    function getTreesTypes() external view returns(string[] memory, bytes32[] memory)
    {
        string[] memory treeTypesString = new string[](treesTypes.length);

        for(uint256 i = 0; i < treesTypes.length; i++){
            treeTypesString[i] = string(abi.encodePacked(treesTypes[i]));
        }

        return (treeTypesString, treesTypes);
    }
