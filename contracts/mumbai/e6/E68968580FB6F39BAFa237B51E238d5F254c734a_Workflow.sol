...
contract Workflow {
    ...
    uint256 public priceOfTokenA;
    uint256 public referecePrice;
    ...
    function lookupLimitOrderWorkflows() public {
        // Iterate through workflows prices
        // 1) Lookup on Stored workflow price references -> LimitOrderWorkflow[] limitOrderWorkflows
        // 2) compare with updated prices from Oracle fetch
        // 3) make Decision (Execute workflow or not)
        ...
        for(uint256 i = 0; i < limitOrderWorkflows.length;) {
            ...
            priceOfTokenA = vwapOracle.getStoredTokenPrices(details.tokenA);
            referecePrice = details.tokenAReferencePrice;
            if (priceOfTokenA > referecePrice) {
                swap(details.tokenA, details.tokenB, details.limitOrderAmount, 100, owner);
