...
contract EnglishAuction is AccessControl {
    bytes32 public constant CURATED_ROLE = keccak256("CURATED_ROLE");
    ...
    event ListingCancelled(uint256 auctionId);
    ...
    function setupAuction(
        ...
        uint256 _duration
    ) external onlyRole(CURATED_ROLE) {
        ...
        emit AddAuction(_auctionId, _nftAddress, _tokenId, _duration, _startingBid, _seller);
    }

    function bid(uint256 _auctionId) external payable {
        Auction storage auction = _auctionListings[_auctionId];

        require(_msgSender() != auction.seller, "seller cannot be bidder");
    ...
    function cancelAuction(uint256 _auctionId) external {
        ...
        require(auction.seller == _msgSender() || hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "not the seller");
        require(auction.started, "not started");
        ...
        emit ListingCancelled(auction.auctionId);
    }
