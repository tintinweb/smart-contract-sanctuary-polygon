...
contract VToken is IVToken,Context,IERC20{
    ...
    address public  PToken;//polygen 映射的token
    ...
    function initialize(address token,address _PToken,string memory tokenName,string memory tokenSymbol,uint8 tokenDecimals) external override{
    ...
    function flashSwap(address to, bytes calldata data) external override lock  {
        uint balance0 = IERC20(PToken).balanceOf(address(this));//清算合约转账ptoken给到vtoken合约
        _mint(to, balance0);//mint对应的vtoken，给到to
        IFlashSwapCallee(to).flashSwapCall(msg.sender, balance0, data);//调用to
        uint balance1 = IERC20(address(this)).balanceOf(address(this));//检查vtoken余额
        _burn(address(this), balance1);//burn vtoken
        _safeTransfer(PToken, msg.sender, balance0);//转账ptoken给到msg.sender，原路返回
        require(balance1 >= balance0, 'error');//如果还回来的vtoken小于转入的ptoken，本次tx失败
    }

 
    
    function mint(address spender, uint256 amount) external override{
        require(IVTokenFactory(factory).bridgeControl() == msg.sender, "Ownable: caller is not the owner");
