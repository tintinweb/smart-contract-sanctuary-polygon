...
contract ChainBattles is ERC721URIStorage {
    ...
    function generateCharacter(uint256 tokenId) public returns (string memory) {
        bytes memory svg = abi.encodePacked(
            ...
            '<rect width="100%" height="100%" fill="#E66B50" />',
    ...
    function mint() public {
        ...
        uint _randLevel = genRandom(newItemId);
        uint _randSpeed = genRandom(_randLevel);
        uint _randStrength = genRandom(_randSpeed);
        uint _randLife = genRandom(_randStrength);
    ...
    function genRandom(uint256 number) public view returns (uint256) {
        return
