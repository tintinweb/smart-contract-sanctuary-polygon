...
contract MarketContractMPX is MarketContract, ShambaGeoConsumer {
    ...
    constructor(
        ...
        geostatsFloor,
        geostatsCap,
        geostatsDecimalPlaces,
        expiration
    ) ShambaGeoConsumer(chainId)
    {
    ...
    */
     ...
     */
    function sendRequestToShambaOracle(
        string memory agg_x,
        string memory dataset_code,
        string memory selected_band,
        string memory image_scale,
    ...
    function getLatestGeostatsDataAndCallback() public returns (int256) {
        int256 latestGeostats = ShambaGeoConsumer.getGeostatsData();
        oracleCallBack(latestGeostats);
        return latestGeostats;
    }

    /// @dev called only by our oracle hub when a new price is available provided by our oracle.
    /// @param geostats provided by the oracle.
    function oracleCallBack(int256 geostats) private {
        require(!isSettled);
        lastGeostats = geostats;
        emit UpdatedLastGeostats(geostats);
    ...
    /// @param geostats settlement price
    function arbitrateSettlement(int256 geostats) public onlyOwner {
        require(geostats >= GEOSTATS_FLOOR && geostats <= GEOSTATS_CAP, "arbitration price must be within contract bounds");
        lastGeostats = geostats;
        emit UpdatedLastGeostats(geostats);
        settleContract(geostats);
        isSettled = true;
    }
