...
contract EventNFTNew is Ownable, ERC721URIStorage {
    ...
    function buyTicket(uint dayIndex, string[] memory ticketNameToBuy, uint[] memory ticketAmountToBuy, uint saleIndex) public {
        ...
        require(block.timestamp < dailyEventData.dateEvent, "Event is Already Started");
				require(block.timestamp > dailyEventData.sales[saleIndex].date, "Ticket Sale is Not Opened");
				if (saleIndex + 1 < dailyEventData.sales.length) {
						require(block.timestamp < dailyEventData.sales[saleIndex + 1].date, "This Type of Sale is Closed");
				}
