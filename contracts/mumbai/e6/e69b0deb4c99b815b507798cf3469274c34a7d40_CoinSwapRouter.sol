...
// File: coinswap_router/interfaces/IWMATIC.sol
...
// File: coinswap_router/interfaces/IERC20.sol
...
// File: coinswap_router/libraries/SafeMath.sol
...
// File: coinswap_router/interfaces/ICoinSwapPair.sol
...
// File: coinswap_router/libraries/CoinSwapLibrary.sol
...
// File: coinswap_router/interfaces/ICoinSwapRouter01.sol
...
// File: coinswap_router/interfaces/ICoinSwapRouter02.sol
...
// File: coinswap_router/libraries/TransferHelper.sol
...
// File: coinswap_router/interfaces/ICoinSwapFactory.sol
...
// File: coinswap_router/CoinSwapRouter.sol
...
contract CoinSwapRouter is ICoinSwapRouter02 {
    ...
    function addLiquidityBNB(
        ...
        address pair = CoinSwapLibrary.pairFor(factory, token, WMATIC);
        TransferHelper.safeTransferFrom(token, msg.sender, pair, amountToken);
        IWMATIC(WMATIC).deposit{value: amountBNB}();
        assert(IWMATIC(WMATIC).transfer(pair, amountBNB));
        liquidity = ICoinSwapPair(pair).mint(to);
    ...
    function removeLiquidityBNB(
        ...
        IWMATIC(WMATIC).withdraw(amountBNB);
    ...
    function swapExactBNBForTokens(uint amountOutMin, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WMATIC, 'CoinSwapRouter: INVALID_PATH');
        amounts = CoinSwapLibrary.getAmountsOut(factory, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'CoinSwapRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        IWMATIC(WMATIC).deposit{value: amounts[0]}();
        assert(IWMATIC(WMATIC).transfer(CoinSwapLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    function swapTokensForExactBNB(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        external
        ...
        returns (uint[] memory amounts)
    {
        require(path[path.length - 1] == WMATIC, 'CoinSwapRouter: INVALID_PATH');
        amounts = CoinSwapLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, 'CoinSwapRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, CoinSwapLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        ...
        ensure(deadline)
    {
        ...
        assert(IWMATIC(WMATIC).transfer(CoinSwapLibrary.pairFor(factory, path[0], path[1]), amountIn));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
