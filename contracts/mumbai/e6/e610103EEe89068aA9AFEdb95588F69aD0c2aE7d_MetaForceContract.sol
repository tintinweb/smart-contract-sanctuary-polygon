...
contract MetaForceContract is Initializable, Proxied, Governed, ReentrancyGuard, IMetaForceContract {
    using SafeERC20 for IERC20;
    using SafeERC20 for IMFS;
    using FixedPointMath for uint256;

    mapping(uint256 => mapping(uint256 => uint256)) public countRenewal;

    mapping(uint256 => DatesBuyingMFS[]) public datesForBuying;
    mapping(uint256 => uint256) public lastIndexBuying;
    mapping(uint256 => AmountSwapSFCR) public amountForSwapSFCR;

    IRegistryContract internal registry;

    constructor() {}

    function swapSFCRtoEnergy(uint256 amount, bool v2) external override {
        ...
        uint256 rate;
        if (v2) {
            sfcr = IERC20(tempRegistry.getSFCR2());
            left = sfcr.balanceOf(msg.sender) - amountForSwapSFCR[senderId].swapedSFCR2;
            rate = RATE_SFCR2_TO_ENERGY;
            amountForSwapSFCR[senderId].swapedSFCR2 += amount;
        } else {
            sfcr = IERC20(tempRegistry.getSFCR());
            left = sfcr.balanceOf(msg.sender) - amountForSwapSFCR[senderId].swapedSFCR;
            rate = RATE_SFCR_TO_ENERGY;
            amountForSwapSFCR[senderId].swapedSFCR += amount;
        }
        if (left < amount) {
            revert MFCNotEnoughSFCR();
        }
        if (amountForSwapSFCR[senderId].amountForSwapSFCR < amount) {
            revert MFCNotEnoughAvailableSFCRForSwap();
        }
        amountForSwapSFCR[senderId].amountForSwapSFCR -= amount;
        _giveEnergy(senderId, amount.mul(rate));
    }

    function firstActivationPack(
        uint256 estimatedMarketinReferrerId,
        uint256 amount,
        uint256 amountBuyingMFS
    ) external override {
        ICoreContract core = ICoreContract(registry.getCoreContract());
        uint256 senderId = getUserId(msg.sender);
        if (core.getMarketingReferrer(senderId) != 0) {
            revert MFCNotFirstActivationPack();
        }
        uint256[] memory referrer = new uint256[](1);
        referrer[0] = estimatedMarketinReferrerId;
        uint256 marketinReferrerId = core.getFreePlace(referrer);
        core.setMarketingReferrer(senderId, marketinReferrerId);
        activationPack(1, amount, amountBuyingMFS);
    }
    ...
    function firstActivationPackWithReplace(
        ...
        core.replaceUserInMarketingTree(replaceId, senderId);
        activationPack(1, amount, amountBuyingMFS);
    }
    ...
    function renewalPackForMFS(
        ...
        uint256 senderId = getUserId(msg.sender);
        renewalConditions(level, amount, senderId);
        IRegistryContract tempRegistry = registry;
        ...
        uint256 priceRenewalPackInMFS = core
            .nowPriceFirstPackInMFS()
            .mul(uint256(COEFF_INCREASE_COST_PACK_FOR_NEXT_LEVEL).pow((level - 1).convertIntToFixPoint()))
            .div(ACTIVATION_COST_RATIO_TO_RENEWAL) * amount;
        if (priceRenewalPackInMFS < amountBuyingMFS) {
            revert MFCNotEqualMFSForBuyingAndPricePacks();
        }
        if (core.getWorkflowStage() == Workflow.SaleOpen) {
            IHoldingContract holding = IHoldingContract(tempRegistry.getHoldingContract());
            uint256 startSaleOpenDate = core.getDateStartSaleOpen();
            if (
                core.getRegistrationDate(senderId) > startSaleOpenDate ||
                block.timestamp > startSaleOpenDate + TRANSITION_PHASE_PERIOD
            ) {
                revert MFCRenewalPaymentIsOnlyPossibleInHMFS();
            }
            holding.holdOnBehalf(senderId, 1, priceRenewalPackInMFS);
        } else {
            DatesBuyingMFS memory purchase;
            unchecked {
                purchase.date = block.timestamp + PURCHASE_TIME_LIMIT_PERIOD;
            }
            purchase.amount = priceRenewalPackInMFS;
            datesForBuying[senderId].push(purchase);
            if (amountBuyingMFS > 0) {
                buyMFS(amountBuyingMFS);
            }
        }
        IMetaPayment metaPayment = IMetaPayment(IMetaCore(tempRegistry.getMetaCore()).getPaymentChannelAddress());
        metaPayment.decreaseBalance(tempRegistry.getMFS(), senderId, tempRegistry.getMetaPool(), priceRenewalPackInMFS);
        distributionRewardReferal(senderId, level, priceRenewalPackInMFS, priceRenewalPackInUSD, false);
        renewalPack(level, amount, priceRenewalPackInMFS, senderId);
    }

    function getDatesForBuyingLength(uint256 userId) external view returns (uint256) {
        return datesForBuying[userId].length;
    }

    function getDatesForBuying(uint256 userId, uint256 index) external view returns (DatesBuyingMFS memory) {
        return datesForBuying[userId][index];
    }
    ...
    function buyMFS(uint256 amountMFS) public override nonReentrant {
        IRegistryContract tempRegistry = registry;
        ...
        address royaltyPool = tempRegistry.getRoyaltyNFTGiftsPool();
        uint256 amountToPay = core.calcUSDAmountForMFS(amountMFS);
        if (core.getWorkflowStage() == Workflow.SaleOpen) {
            revert MFCLateForBuyMFS();
        }
        uint256 tempAmount = amountMFS;
        for (uint256 i = lastIndexBuying[senderId]; i < datesForBuying[senderId].length; ) {
            if (datesForBuying[senderId][i].date < block.timestamp) {
                datesForBuying[senderId][i].amount = 0;
            } else if (datesForBuying[senderId][i].amount < tempAmount) {
                tempAmount -= datesForBuying[senderId][i].amount;
                datesForBuying[senderId][i].amount = 0;
            } else {
                datesForBuying[senderId][i].amount = datesForBuying[senderId][i].amount - tempAmount;
                tempAmount = 0;
            }

            if (tempAmount == 0) {
                lastIndexBuying[senderId] = i;
                break;
            }
            unchecked {
                ++i;
        ...
        uint256 shareRoyalty = amountToPay.mul(SHARE_ROYLTY_GIFT_POOL_FROM_BUY_MFS);
        metaPayment.decreaseBalance(address(stableCoin), senderId, royaltyPool, shareRoyalty);
        metaPayment.decreaseBalance(address(stableCoin), senderId, metaPool, amountToPay - shareRoyalty);
        amountForSwapSFCR[senderId].amountForSwapSFCR += amountToPay;
        core.giveMFSFromPool(senderId, amountMFS);
        core.increaseTotalEmission(amountMFS);
        _giveEnergy(senderId, amountToPay.mul(core.getEnergyConversionFactor()));
        emit MFCTokensMFSIsBuyingInOffer(senderId, amountMFS);
    }
    ...
    function activationPack(
        ...
        uint256 senderId = getUserId(msg.sender);
        uint256 tempDate;
        if (core.isPackActive(senderId, level)) {
            tempDate = core.getTimestampEndPack(senderId, level);
        } else {
            tempDate = block.timestamp;
        }
        uint256 pricePacksInUSD = PRICE_PACK_LEVEL1_IN_USD.mul(
            uint256(COEFF_INCREASE_COST_PACK_FOR_NEXT_LEVEL).pow((level - 1).convertIntToFixPoint())
        ) * amount;
        uint256 pricePacksInMFS = core.nowPriceFirstPackInMFS().mul(
            uint256(COEFF_INCREASE_COST_PACK_FOR_NEXT_LEVEL).pow((level - 1).convertIntToFixPoint())
        ) * amount;
        if (pricePacksInMFS < amountBuyingMFS) {
            revert MFCNotEqualMFSForBuyingAndPricePacks();
        }
        if (core.getMarketingReferrer(senderId) == 0) {
        ...
        for (uint256 i = 1; i < level; ) {
            if (!core.isPackActive(senderId, i)) {
        ...
        spendTokensForActivation(senderId, pricePacksInUSD, pricePacksInMFS);
        core.setTimestampEndPack(senderId, level, tempDate + PACK_ACTIVITY_PERIOD * amount);
        distributionRewardReferal(senderId, level, pricePacksInMFS, pricePacksInUSD, true);
        if (core.getWorkflowStage() != Workflow.SaleOpen) {
            if (amountBuyingMFS > 0) {
                buyMFS(amountBuyingMFS);
            }
            emit MFCPackIsActivated(senderId, level, pricePacksInUSD, tempDate + PACK_ACTIVITY_PERIOD * amount);
        } else {
            emit MFCPackIsActivated(senderId, level, pricePacksInMFS, tempDate + PACK_ACTIVITY_PERIOD * amount);
        }
    }
    ...
    function renewalPack(
        ...
        while (tempAmount != 0) {
            ...
            if (countRenewal[senderId][level] < HMFS_COUNT - 1) {
                for (uint256 i = countRenewal[senderId][level]; i < amountsCurrency.length; ) {
                    if (amountsCurrency[i] < tempPrice) {
                        _burnHMFS(senderId, i + 1, amountsCurrency[i]);
                        tempPrice = tempPrice - amountsCurrency[i];
                        amountsCurrency[i] = 0;
                    } else {
                        _burnHMFS(senderId, i + 1, tempPrice);
                        amountsCurrency[i] = amountsCurrency[i] - tempPrice;
                        tempPrice = 0;
                        break;
                    }
                    unchecked {
                ...
                renewalPack(level, 1, priceRenewalPackInMFS, senderId);
                countRenewal[senderId][level] = countRenewal[senderId][level] + 1;
                tempAmount--;
            } else {
                if (amountsCurrency[HMFS_COUNT - 1] < tempPrice * tempAmount) {
                    revert MFCNotEnoughHMFSNeedLevel();
                }
                _burnHMFS(senderId, HMFS_COUNT, amountsCurrency[HMFS_COUNT - 1]);
                renewalPack(level, tempAmount, priceRenewalPackInMFS * amount, senderId);
                tempAmount = 0;
            }
        }
    ...
    function spendTokensForActivation(
        uint256 userId,
        uint256 purshaseValueInUSD,
        uint256 purshaseValueInMFS
    ) internal {
        IRegistryContract tempRegistry = registry;
        ICoreContract core = ICoreContract(tempRegistry.getCoreContract());
        IERC20 stableCoin = IERC20(tempRegistry.getStableCoin());
        ...
        address metaPool = tempRegistry.getMetaPool();
        if (core.getWorkflowStage() != Workflow.SaleOpen) {
            metaPayment.decreaseBalance(address(stableCoin), userId, metaPool, purshaseValueInUSD);
            unchecked {
                purchase.date = block.timestamp + PURCHASE_TIME_LIMIT_PERIOD;
            }
            purchase.amount = purshaseValueInMFS;
            datesForBuying[userId].push(purchase);
            core.increaseTotalEmission(purshaseValueInMFS);
        } else {
            metaPayment.decreaseBalance(address(mfs), userId, metaPool, purshaseValueInMFS);
        }
    }
    ...
    function renewalPack(
        ...
        ICoreContract core = ICoreContract(registry.getCoreContract());
        core.increaseTimestampEndPack(senderId, level, PACK_ACTIVITY_PERIOD * amount);
        emit MFCPackIsRenewed(senderId, level, amountTokens, core.getTimestampEndPack(senderId, level));
    }
    ...
    function distributionRewardReferal(
        ...
        uint256 tempAmountMFS = amountMFS;
        uint256 tempAmountUSD = amountUSD;
        {
            uint256 _level = level;
            uint256[] memory rewardsRefers = core.getRewardsDirectReferrers();
            uint256 refererId = userId;
            for (uint256 i; i < rewardsRefers.length; ) {
                uint256 dRewardMFS = amountMFS.mul(rewardsRefers[i]);
                uint256 dRewardUSD = amountUSD.mul(rewardsRefers[i]);
                while (true) {
                    refererId = core.getReferrer(refererId);
                    if (core.isPackActive(refererId, _level)) {
                        break;
                    }
                    emit LostMoney(refererId, userId, dRewardUSD, false);
                }
                tempAmountMFS -= dRewardMFS;
                tempAmountUSD -= dRewardUSD;
                if (activate) {
                    TypeReward typeReward = core.getTypeReward(refererId);
                    if (typeReward == TypeReward.ONLY_MFS || core.getWorkflowStage() == Workflow.SaleOpen) {
                        core.giveMFSFromPool(refererId, dRewardMFS);
                        emit RevenueMFS(refererId, userId, dRewardMFS, _level, true, false);
                    } else if (typeReward == TypeReward.ONLY_USD) {
                        core.giveStableFromPool(refererId, dRewardUSD);
                        amountMFSForRealization += dRewardMFS;
                        emit RevenueStable(refererId, userId, dRewardUSD, _level, true, false);
                    } else {
                        core.giveMFSFromPool(refererId, dRewardMFS / 2);
                        core.giveStableFromPool(refererId, dRewardUSD / 2);
                        amountMFSForRealization += dRewardMFS / 2;
                        emit RevenueMFS(refererId, userId, dRewardMFS / 2, _level, true, false);
                        emit RevenueStable(refererId, userId, dRewardUSD / 2, _level, true, false);
                    }
                } else {
                    core.giveMFSFromPool(refererId, dRewardMFS);
                    emit RevenueMFS(refererId, userId, dRewardMFS, _level, false, false);
                }
                unchecked {
                    ++i;
                }
        ...
        }
        {
            ...
            uint256 marketingRefererId = userId;
            for (uint256 i; i < rewardsMarketingRefers.length; ) {
                uint256 mRewardMFS = amountMFS.mul(rewardsMarketingRefers[i]);
                uint256 mRewardUSD = amountUSD.mul(rewardsMarketingRefers[i]);
                while (true) {
                    marketingRefererId = core.getMarketingReferrer(marketingRefererId);
                    if (core.isPackActive(marketingRefererId, _level)) {
                        break;
                    }
                    emit LostMoney(marketingRefererId, userId, mRewardUSD, true);
                }

                if (i < rewardsMarketingRefers.length - 1) {
                    tempAmountMFS -= mRewardMFS;
                    tempAmountUSD -= mRewardUSD;
                } else {
                    mRewardMFS = tempAmountMFS;
                    mRewardUSD = tempAmountUSD;
                }
                if (activate) {
                    TypeReward typeReward = core.getTypeReward(marketingRefererId);
                    if (typeReward == TypeReward.ONLY_MFS || core.getWorkflowStage() == Workflow.SaleOpen) {
                        core.giveMFSFromPool(marketingRefererId, mRewardMFS);
                        emit RevenueMFS(marketingRefererId, userId, mRewardMFS, _level, true, true);
                    } else if (typeReward == TypeReward.ONLY_USD) {
                        core.giveStableFromPool(marketingRefererId, mRewardUSD);
                        amountMFSForRealization += mRewardMFS;
                        emit RevenueStable(marketingRefererId, userId, mRewardUSD, _level, true, true);
                    } else {
                        core.giveMFSFromPool(marketingRefererId, mRewardMFS / 2);
                        core.giveStableFromPool(marketingRefererId, mRewardUSD / 2);
                        amountMFSForRealization += mRewardMFS / 2;
                        emit RevenueMFS(marketingRefererId, userId, mRewardMFS / 2, _level, true, true);
                        emit RevenueStable(marketingRefererId, userId, mRewardUSD / 2, _level, true, true);
                    }
                } else {
                    core.giveMFSFromPool(marketingRefererId, mRewardMFS);
                    emit RevenueMFS(marketingRefererId, userId, mRewardMFS, _level, false, true);
                }
                unchecked {
                    ++i;
                }
    ...
    function realizeMFS(uint256 amount) internal {
        IRegistryContract tempRegistry = registry;
        IRequestMFSContract requestContract = IRequestMFSContract(tempRegistry.getRequestMFSContract());
        requestContract.realizeMFS(amount);
    }

    function _giveEnergy(uint256 userId, uint256 amount) internal {
        IRegistryContract tempRegistry = registry;
        IMetaPayment metaPayment = IMetaPayment(IMetaCore(tempRegistry.getMetaCore()).getPaymentChannelAddress());
        IEnergy energy = IEnergy(tempRegistry.getEnergyCoin());
        energy.mint(address(metaPayment), amount);
        metaPayment.increaseBalance(address(energy), userId, amount);
    }
    ...
    function _burnHMFS(
        ...
        IRegistryContract tempRegistry = registry;
        IMetaPayment metaPayment = IMetaPayment(IMetaCore(tempRegistry.getMetaCore()).getPaymentChannelAddress());
        IHMFS hFMS = IHMFS(tempRegistry.getHMFS(level));
        metaPayment.decreaseBalance(address(hFMS), userId, address(this), amount);
        hFMS.burn(address(this), amount);
    }

    function renewalConditions(
        uint256 level,
        uint256 amount,
        uint256 senderId
    ) internal view {
...
error MetaForceSpaceHoldingZeroAddress();
...
interface IHoldingContract {
    ...
    function getDepositIds(uint256 holderId) external view returns (uint256[] memory);
}
