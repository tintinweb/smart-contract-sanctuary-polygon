    ...
    UUPSUpgradeable
{
 ...
 event finishedProcessingPayment(
    ...
    function getApproversdReward(bytes32  reviewHarsh)
        public
        view
    ...
    function processPayments(int256 size)
        public
        ...
        while( (size > 0) && (DoubleEndedQueue.empty(pendingPayoutsCID) == false) ) {
 ...
 ///@dev this method is used to send rewards of rvz erc20 token to users who sumbit reviews that have been approve
    function payoutApproverReward(bytes32 _reviewCID)
        public
        //internal changing to public , so it could be called externally
        onlyRole(APPROVER_ADMIN)
    ...
    {
        require(
            approversRewards[_reviewCID].exists == true,
        ...
        require(approversRewards[_reviewCID].rewardBalance < maxRewardAmount);

        RevewzToken token = RevewzToken(rvzTokenAddress);
        uint256 balance = token.balanceOf(address(this));
        uint256 rewardAmt = approversRewards[_reviewCID].rewardBalance;
        require(
            balance > rewardAmt,
        ...
        approversRewards[_reviewCID].approvedPayout = true;
        token.transfer(receipient, rewardAmt);
        emit RewardedApprover(
            receipient,
            approversRewards[_reviewCID].ipfsReviewCID,
            rewardAmt,
