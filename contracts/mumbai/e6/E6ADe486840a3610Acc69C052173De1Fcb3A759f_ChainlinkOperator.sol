...
abstract contract ChainlinkTokenReceiver {
  /**
   * @notice Called when Chainlink is sent to the contract via `transferAndCall`
   * @dev The data payload's first 2 words will be overwritten by the `sender` and `amount`
   * values to ensure correctness. Calls oracleRequest.
   * @param sender Address of the sender
   * @param amount Amount of Chainlink sent (specified in wei)
  ...
  function onTokenTransfer(
    ...
    uint256 amount,
    bytes memory data
  ) public validateFromChainlink permittedFunctionsForChainlink(data) {
    assembly {
  ...
  /**
   * @dev Reverts if not sent from the Chainlink token
   */
  modifier validateFromChainlink() {
    require(msg.sender == getChainlinkToken(), "Must use Chainlink token");
    _;
  }
   ...
   */
  modifier permittedFunctionsForChainlink(bytes memory data) {
    bytes4 funcSelector;
    assembly {
      // solhint-disable-next-line avoid-low-level-calls
 ...
 * @notice The contract will be able to register the request query and provide response to that request
 */
contract ChainlinkOperator is Operator {
    constructor(address _chainlink, address _owner) Operator(_chainlink, _owner) {}
}
...
interface WithdrawalInterface {
  /**
   * @notice transfer Chainlink held by the contract belonging to msg.sender to
   * another address
   * @param recipient is the address to send the Chainlink to
   * @param amount is the amount of Chainlink to send
   */
  function withdraw(address recipient, uint256 amount) external;

  /**
   * @notice query the available amount of Chainlink to withdraw by msg.sender
   */
  function withdrawable() external view returns (uint256);
}
...
import "./abstract/ChainlinkTokenReceiver.sol";
...
import "./libraries/SafeMathChainlink.sol";

/**
 * @title The Operator contract
 * @notice Node operators can use this contract to fulfill requests sent to them
 */
contract Operator is AuthorizedReceiver, ConfirmedOwner, ChainlinkTokenReceiver, OperatorInterface, WithdrawalInterface {
  ...
  ChainlinkTokenInterface internal immutable chainlinkToken;
  ...
  /**
   * @notice Deploy with the address of the Chainlink token
   * @dev Sets the ChainlinkToken address for the imported ChainlinkTokenInterface
   * @param chainlink The address of the Chainlink token
   * @param owner The address of the owner
   */
  constructor(address chainlink, address owner) ConfirmedOwner(owner) {
    chainlinkToken = ChainlinkTokenInterface(chainlink); // external but already deployed and unalterable
  }
  ...
  function oracleRequest(
    ...
    uint256 dataVersion,
    bytes calldata data
  ) external override validateFromChainlink {
  ...
  /**
   * @notice Allows the node operator to withdraw earned Chainlink to a given address
   * @dev The owner of the contract can be another wallet and does not have to be a Chainlink node
   * @param recipient The address to send the Chainlink token to
  ...
  {
    assert(chainlinkToken.transfer(recipient, amount));
  }

  /**
   * @notice Displays the amount of Chainlink that is available for the node operator to withdraw
   * @dev We use `ONE_FOR_CONSISTENT_GAS_COST` in place of 0 in storage
   * @return The amount of withdrawable Chainlink on the contract
   */
   ...
   */
  function ownerForward(address to, bytes calldata data) external onlyOwner validateNotToChainlink(to) {
  ...
  /**
   * @notice Interact with other ChainlinkTokenReceiver contracts by calling transferAndCall
  ...
  function ownerTransferAndCall(
    ...
    return chainlinkToken.transferAndCall(to, value, data);
  }
   ...
   * Will transfer the Chainlink sent for the request back to the recipient address.
  ...
  function cancelOracleRequest(
    ...
    chainlinkToken.transfer(msg.sender, payment);
  }
  ...
  /**
   * @notice Returns the address of the Chainlink token
   * @dev This is the public implementation for ChainlinkTokenAddress, which is
   * an internal method of the ChainlinkClient contract
   */
  function getChainlinkToken() public view override returns (address) {
    return address(chainlinkToken);
  }
  ...
  function _verifyAndProcessOracleRequest(
    ...
    uint256 nonce,
    uint256 dataVersion
  ) private validateNotToChainlink(callbackAddress) returns (bytes32 requestId, uint256 expiration) {
  ...
  /**
   * @notice Returns the Chainlink available in this contract, not locked in escrow
   * @return uint256 Chainlink tokens available
   */
  function _fundsAvailable() private view returns (uint256) {
    uint256 inEscrow = s_tokensInEscrow.sub(ONE_FOR_CONSISTENT_GAS_COST);
    return chainlinkToken.balanceOf(address(this)).sub(inEscrow);
  }
  ...
  /**
   * @dev Reverts if the callback address is the Chainlink token
   * @param to The callback address
   */
  modifier validateNotToChainlink(address to) {
    require(to != address(chainlinkToken), "Cannot call to Chainlink");
    _;
  }
