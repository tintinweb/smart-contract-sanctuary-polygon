    ...
    CommonModifiers
{
    ...
    function msend(
        ...
        bytes32 metadata;
        assembly {
            metadata := mload(add(_params, 0x20))
        }
        if (metadata == bytes32(0)) {
            IHelper.Selector _selector;
            assembly {
                _selector := mload(add(_params, 0x40))
            }

            metadata = ecc.preRegMsg(
                _params,
                msg.sender,
                _dstChainId,
                _selector
...
abstract contract MiddleLayerModifiers is MiddleLayerStorage, CommonErrors {
    ...
    modifier notPaused() {
        // NOTE: This returns and doesnt revert for ECC Resend tests
        // This also makes tests mimic closer to how they would in real world situations
        if (paused) return;
