...
contract FlowMi is VRFConsumerBaseV2 {
    ...
    constructor(
        ...
        iWmatic = IWETH(wmaticTokenAddress);
    ...
    function flowmiFollow(address _profileid) public payable {
        ...
        if (
            ...
            // requestRandomWords();
            //iamatic.approve(address(this), fraction * i_goal); // aprueba aMatic tmb,
            ...
            iaWmatic.approve(address(POOL), fraction * (i_goal - 1)); // aprueba aMatic tmb,

            iWmatic.approve(address(this), fraction * (i_goal - 1)); // aprueba aMatic tmb,
            iWmatic.approve(address(iWmatic), fraction * (i_goal - 1)); // aprueba aMatic tmb,
            ...
            // Intento sacar wraped y deswrapearlos
            POOL.withdraw(
                wmaticTokenAddress,
                fraction * (i_goal - 1),
                address(this)
            ...
            // deswrapeo
            // iWmatic.withdraw(fraction * (i_goal - 1));
            // Recupera de la Pool
            ...
            // apprueva sacar matics de la cuenta
            //iWmatic.approve(address(this), fraction);
            //imatic.approve(address(this), fraction * i_flowmiCost);
            ...
            // pay(maticAddress, ((msg.value) * 9) / 10);
            // deposit(wmaticTokenAddress, fraction * i_flowmiCost);
        }
    }
        ...
        checkAmount(amount)
    {
        ...
        // check that user give enough allowance to the contract for supplying the token
        if (
        ...
        //POOL.depositETH(address(POOL), address(this), AAVE_REF_CODE, {value: amount});
    ...
    function withdraw(address tokenAddr, uint256 amount) private {
        IERC20 token = IERC20(tokenAddr);
        inwithdraw = 1;
        ...
        // After withdrawal, we can calculate the random word and transfer to the winner
        requestRandomWords();
        inwithdraw = 4;
    }
