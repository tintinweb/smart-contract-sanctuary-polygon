...
contract PozBonding is Initializable, OwnableUpgradeable {
    ...
    function getMarketPrice(address _addr, uint256 _amount)
        public
        view
        returns (uint256)
    {
        return market.getMarketPriceUsingSushi(factory, _addr, POZ, _amount);
    }

    function getBondingPrice(address _addr, uint256 _amount)
        public
        view
        returns (uint256)
    {
        uint256 mp = getMarketPrice(_addr, _amount);
        return (mp * (BOND_DISCOUNT + 10000)) / 10000;
    }
    ...
    function setMarketPriceContract(address _mp) external onlyOwner {
        market = IMarketPrice(_mp);
    }
    ...
    function claim(address _addr) external {
        ...
        ERC20(POZ).transferFrom(TREASURY_WALLET, _user, _claim);
        lastIndex[_user][_addr] = _index;
    }

    function bond(address _addr, uint256 _amount) external {
        address _user = msg.sender;
        require(
        ...
        uint256 _bond = getBondingPrice(_addr, _amount);
        ERC20(_addr).transferFrom(_user, TREASURY_WALLET, _amount);
