...
contract SpawnPoolFacet is WithStorage, ReentrancyGuard, Modifiers {
    ...
    function populatePool(uint8 species, uint256 amount) public onlyOwner {
        _populateSpawnPool(species, amount);
    }

    function _populateSpawnPool(uint8 species, uint256 amount) internal {
        uint256 fishSpawnId = gs().fishSpawnId;
        uint256 i = uint8(species);
    ...
    modifier fishOwner(uint256[] memory fishIds) {
        for (uint256 i = 0; i < fishIds.length; i++) {
            require(msg.sender == GameItemsFacet(address(this)).ownerOfItem(fishIds[i]), "Sender does not own one of the passed fish.");
        }
        _;
    }
    ...
    function checkForFish(uint256 x, uint256 y) public returns (uint256 _fishId, FishSpecies _species) {
        ...
        if (gs().spawnPoolMap[y].fishIds[x] > 0) {
            ...
            // If we get a fish egg back, we need to pass the expected species down to the person handling the egg
            if (species == FishSpecies.Unknown) {
                species = FishSpecies(y);
            }
    ...
    function depositFish(uint256[] memory fishIds) public nonReentrant {
        ...
        for (uint256 i = 0; i < fishIds.length; i++) {
            FishSpecies species = gs().fishMetadataLookup[fishIds[i]].species;
    ...
    function withdrawFish(uint256[] memory fishIds) public nonReentrant {
        ...
        for (uint256 i = 0; i < fishIds.length; i++) {
            ...
            FishSpecies species = gs().fishMetadataLookup[fishId].species;
    ...
    function _rewardsForFishId(uint256 fishId) private view returns (uint256) {
        uint8 species = uint8(gs().fishMetadataLookup[fishId].species);
        require(species != uint8(FishSpecies.Unknown), "Fish is not a valid fish type.");
        if (gs().spawnPoolMap[species].snapshots.length == 0) {
            return 0;
        }
...
contract GameItemsFacet is WithStorage, ERC1155, Modifiers {
    ...
    function convertFishToBait(uint256 fishId) onlyFishItems(fishId) public {
        require(gs().fishIdToWallet[fishId] == msg.sender, "You do not own this fish.");
        FishMetadata storage metadata = gs().fishMetadataLookup[fishId];

        _burn(msg.sender, fishId, 1);

        uint8 speciesId = uint8(metadata.species);
    ...
    function mock_catchFish(address to, uint256 fishId) public onlyOwner onlyFishItems(fishId) {
        ...
        gs().players[to].fishIds.push(fishId);
        gs().players[to].fishIdIndexLookup[fishId] = gs().players[to].fishIds.length - 1;
    }
    ...
    function transferFish(
        ...
        require(gs().gameItemLookup[fishId] == GameItemType.FISH, "You can't transfer an item that is not a fish");
        require(from != to, "Can't transfer a fish to yourself.");
        _transfer(address(this), from, to, fishId, 1, "");
    }

    function spawnFishEggs(uint256 totalEggs) external returns (uint256) {
        if (gs().fishSpawnId == 0) {
            gs().gameItemsCounter.increment();
    ...
    function hatchFishEgg(FishSpecies _species, uint256 randomSeed) external returns (FishMetadata memory) {
        ...
        FishMetadata storage metadata = gs().fishMetadataLookup[hatchedFishId];
    ...
    function baitIdForFishSpecies(FishSpecies species) public view returns (uint256) {
        return gs().fishSpeciesToBaitTokenId[uint8(species)];
    }
    ...
    function fishOwnedByWallet(address player) public view returns (uint256[] memory) {
        return gs().players[player].fishIds;
    }
}
