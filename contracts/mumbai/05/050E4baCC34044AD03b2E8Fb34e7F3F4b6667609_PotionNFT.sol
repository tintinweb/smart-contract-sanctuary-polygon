...
contract PotionNFT is ERC721Enumerable, Ownable {
    ...
    function setBasePower(uint256 _power) external onlyOwner {
        require(_power > 0 && _power < 100, "Not a good value");
        power = _power;
    }
    ...
    function offerPotion(
        ...
        require(_type <= 6 && _power > 0, "Not valid potion");
        if(_type == 5 || _type == 6){
            _power = 5;
        }
        _tokenIds.increment();
    ...
    function mintPotionForSale(
        ...
        require(_type <= 4 && _power > 0, "Not valid potion");
        _tokenIds.increment();
        uint256 _newPotionId = _tokenIds.current();
    ...
    function mintMultiplePotion(uint256[7] memory _powers, address _owner) external onlyAuth returns(uint256){
        _tokenIds.increment();
        uint256 _newPotionId = _tokenIds.current();
        ...
        i.powers.rest = _powers[5]; 
        i.powers.xp = _powers[6]; 
