...
abstract contract Ownable is Context {
    ...
    function _transferOwnership(address newOwner) internal virtual {
        ...
        modifier _onlyOwner() {
        require(_ownner == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
...
contract AeroPups is ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable {
    using SafeMath for uint256;
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdTracker;

    uint256 private constant MAX_SUPPLY = 10000; // 3353 AeroPups

    uint256 private _price = 0.8 ether; // Public Sale Price;
    ...
    constructor (string memory baseURI) ERC721("The Real AeroPups", "RealAeroPups") {
        setBaseURI(baseURI);
    }

    function mint(address payable _to, uint256[] memory _ids) public payable {
        ...
        require(total <= maxNFTSupply, "MINT: Please go to the Opensea to buy AeroPups.");
        require(_ids.length <= maxMintCount, "MINT: Current count exceeds maximum mint count.");
        require(msg.value >= price * _ids.length, "MINT: Current value is below the sales price of AeroPups");
    ...
    function withdrawAll() public payable _onlyOwner {
        uint256 totalBalance = address(this).balance;
        require(totalBalance > 0, "WITHDRAW: No balance in contract");

        _widthdraw(msg.sender, totalBalance);
    }
