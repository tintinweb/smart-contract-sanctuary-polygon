...
contract ZenftMarketplace is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    modifier onlyBundleMarketplace() {
        if (address(addressRegistry.bundleMarketplace()) != _msgSender())
            revert MustBeBundleMarketplace();
        _;
    }
    ...
    modifier isListed(
        ...
        if (listing.quantity < 1) revert ItemNotListedOrNotOwner();
        _;
    }
    ...
    modifier notListed(
        ...
        Listing memory listing = listings[_nftAddress][_tokenId][_owner];
        if (listing.quantity > 0) revert ItemAlreadyListed();
        _;
    }
    ...
    modifier validListing(
        ...
        if (_getNow() < listedItem.startTime) revert SaleNotStarted();
        if (_getNow() > listedItem.endTime) revert SaleExpired();
        _;
    }
    ...
    modifier offerExists(
        ...
        if (offer.quantity < 0 && offer.expirationTime < _getNow())
            revert OfferExpiredOrInvalid();
        _;
    }
    ...
    function initialize(
        ...
        addressRegistry = IZenftAddressRegistry(_addressRegistry);

        __Ownable_init();
        __ReentrancyGuard_init();
    }

    /// @notice Public Method for Listing NFT
    /// @param _nftAddress Address of NFT contract
    /// @param _tokenId Token ID of NFT
    /// @param _quantity token amount to list (required for ERC-1155's)
    ...
    function listItem(
        ...
        if (IERC165(_nftAddress).supportsInterface(type(IERC721).interfaceId)) {
            ...
            if (nft.ownerOf(_tokenId) != _msgSender()) revert NotNftOwner();
    ...
    /// @notice Canceling an NFT Listing
    ...
    /// @notice Updating NFT Listing
    /// @param _nftAddress Address of NFT contract
    /// @param _tokenId Token ID of NFT
    /// @param _newPrice New sale price for each NFT
    ...
    {
        Listing memory listedItem = listings[_nftAddress][_tokenId][_owner];
            if (msg.value < listedItem.pricePerItem) revert AmountSentTooLow();
    ...
    function _buyItem(
        ...
        ) = getRoyalties(_nftAddress, _tokenId, price - feeAmount);
        ...
        if (IERC165(_nftAddress).supportsInterface(type(IERC721).interfaceId)) {
            if (hasRoyalties) {
                //pay royalties
                (bool royaltyTransferSuccess, ) = minter.call{
                    value: royaltyTotal
                }("");
            ...
            (bool ownerTransferSuccess, ) = _owner.call{
                value: price - feeAmount - royaltyTotal
            }("");
        ...
        if (addressRegistry.bundleMarketplace() != address(0)) {
    ...
    //     IZenftAuction auction = IZenftAuction(addressRegistry.auction());

    //     (, , , uint256 startTime, , bool resulted) = auction.auctions(
    //         _nftAddress,
    //         _tokenId
    //     );
    //     if (startTime != 0 || resulted != true) revert ActiveAuction();
    //     if (_expirationTime > _getNow()) revert InvalidExpirationTime();
    ...
    //     offer.paymentToken.safeTransferFrom(
    //         _creator,
    //         treasuryAddress,
    //         feeAmount
    //     );
    ...
    //         offer.paymentToken.safeTransferFrom(_creator, minter, royaltyFee);
    //         feeAmount = feeAmount + royaltyFee;
    //     } else {
    //         minter = collectionRoyalties[_nftAddress].treasuryAddress;
    ...
    function _isZenftNFT(address _nftAddress) internal view returns (bool) {
        return
            addressRegistry.zenft() == _nftAddress ||
            IZingArt_NFT721_Factory(addressRegistry.factory()).exists(
                _nftAddress
            ) ||
            IZingArt_NFT721_Factory(addressRegistry.artFactory()).exists(
                _nftAddress
            );
    }
        ...
        returns (bool hasRoyalties, address minter, uint256 royaltyTotal)
    {
        ...
        if (supportsRoyalties) {
            ...
            bool _hasRoyalties = _royaltyAmount > 0 && _receiver != address(0);
    ...
    function _validOwner(
        ...
        if (IERC165(_nftAddress).supportsInterface(type(IERC721).interfaceId)) {
            IERC721 nft = IERC721(_nftAddress);
            if (nft.ownerOf(_tokenId) != _owner) revert NotNftOwner();
        } else if (
            IERC165(_nftAddress).supportsInterface(type(IERC1155).interfaceId)
        ) {
            IERC1155 nft = IERC1155(_nftAddress);
            if (nft.balanceOf(_owner, _tokenId) < quantity)
                revert InvalidQuantity();
        } else {
            revert InvalidAddress();
        }
