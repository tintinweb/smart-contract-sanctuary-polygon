...
contract MeowTokenContract is ERC20, Ownable {
    ...
    address public StakeableTokensContract = address(0);
    bool StakeableTokensContractIsErc1155;
    address public StakeableTokensCreator = address(0);
  
    event TokenStaked(uint256 indexed tokenid, address user);
    event RewardPaid(address indexed user, uint256 reward);

    constructor() ERC20("MeowToken", "MEOW") {
        //StakeableTokensContract = address(0x2953399124F0cBB46d2CbACD8A89cF0599974963);
        StakeableTokensContract = address(0xfEC923CF605bBd43A806C78439d41c411bE0ACa6);
        StakeableTokensContractIsErc1155 = true;
        //StakeableTokensCreator = address(0xE8aF6d7e77f5D9953d99822812DCe227551df1D7);
        StakeableTokensCreator = address(0);
      }
    ...
    function isValidStakeableToken(uint256 id) view internal returns(bool) {
        if (StakeableTokensCreator == address(0))
            return true;
        // 0xe8af6d7e77f5d9953d99822812dce227551df1d7000000000013ad0000000001
    ...
    // called when staking new NFTs
    // prevStaked = walletStakedBalance(user)
    ...
    /*
    // 'spend' computed Rewards from the wallet
    function collect(address from, uint256 amount) internal {
        require(msg.sender == from);
