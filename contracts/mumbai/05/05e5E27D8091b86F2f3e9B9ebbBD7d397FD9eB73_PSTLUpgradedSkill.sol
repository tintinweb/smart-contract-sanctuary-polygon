...
contract PSTLUpgradedSkill is ERC721LazyMint {
    // Store constant values for the 2 collections:
    // 1. PSTLBaseSkill collection
    ERC721LazyMint public immutable pstlBaseSkill;
    // 2. PSTLSkillpoint collection
    ERC1155Base public immutable pstlSkillpoint;
    ...
    constructor(
        ...
        address _royaltyRecipient,
        uint128 _royaltyBps,
        address _pstlBaseSkillAddress,
        address _pstlSkillpointAddress
    ) ERC721LazyMint(_name, _symbol, _royaltyRecipient, _royaltyBps) {
        pstlBaseSkill = ERC721LazyMint(_pstlBaseSkillAddress);
        pstlSkillpoint = ERC1155Base(_pstlSkillpointAddress);
    }

    /**
     *  @dev Checks a request to claim SPs against a custom condition.
     *
     *  @param _claimer   Caller of the claim function.
     *  @param _quantity  The number of SKILLS being claimed.
     */
    function verifyClaim(address _claimer, uint256 _quantity, uint256 _quantitySkillpoint) 
        public 
        view 
        virtual 
        {
            // 1.. override the claim fn to ensure:
            // require user has balanceOf > 1 of the pstlBaseSkill collection
            // i.e user has the prequisite skill necessary to upgrade
            require(pstlBaseSkill.balanceOf(_claimer) > _quantity, "You don't have the necessary base skill!");
            // require user has balanceOf > 1 of the pstlSkillpoint
            // i.e user has the necessary skillpoints to pay upgrade cost
            require(pstlSkillpoint.balanceOf(_claimer, 0) > _quantitySkillpoint, "You don't have the necessary skill point(s)!");   
        }

    // 2. Within claim fn, we need to burn 1 qty of the upgraded skill point
    /**
     *
     *  @dev             The logic in `verifyClaim` determines whether the caller is authorized to mint NFTs.
     *                   The logic in `transferTokensOnClaim` does actual minting of tokens,
     *                   can also be used to apply other state changes.
     *
     *  @param _quantity  The number of SPs to mint.
     */
    function claim(uint256 _quantity, uint256 _quantitySkillpoint) public payable nonReentrant {
        verifyClaim(msg.sender, _quantity, _quantitySkillpoint);
        pstlSkillpoint.burn(msg.sender, 0, _quantitySkillpoint);
        super.claim(msg.sender, _quantity);
    }
}