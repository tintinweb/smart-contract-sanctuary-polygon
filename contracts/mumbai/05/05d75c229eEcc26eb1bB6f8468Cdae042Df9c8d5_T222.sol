...
contract T222 is Ownable, TimeForDay, SortList {
    ...
    event DistributeEvent(uint256 _day, uint256 _time, string _type, uint256 _toShareAmount, uint256 _totalAmount);
    ...
    */
    ...
    function _distributeDayRewardForInviteRanking() private {
        ...
        emit DistributeEvent(_day, _timestamp, "_distributeDayRewardForInviteRanking", _amount, _totalAmount);
    }
    ...
    function _distributeDayRewardForInvestRanking() private {
        ...
        emit DistributeEvent(_day, _timestamp, "_distributeDayRewardForInvestRanking", _amount, _totalAmount);
    }
    ...
    function _distributeCompensation() private {
        ...
        emit DistributeEvent(_day, _timestamp, "_distributeCompensation", _amount, _amount);
    }
    ...
    function invest(uint256 _amount) external {
        ...
        if (userInvestList[_user]._remainingCredit > 0) {
            //userInfoList[_user]._updatedRewardItem._updatedStaticReward += (userInfoList[_user]._depositAmount * 2 - userInfoList[_user]._totalClaiRewardAmount) * skip.currentASA / 10 ** 18;
    ...
    // 更新用户的 Dao 等级和奖励
    function updateDao(address _user, uint256 _investStartIndex) private {
        // 获取用户的推荐人列表和数量
    ...
    */
    ...
    //测试专用
    function addLast100(address[] memory _addressList) public onlyOwner {
    ...
    function addRankingPerDayForInvite20(address[] memory _addressList) public onlyOwner {
        uint256 _num = _addressList.length;
        uint256 _day = getYearMonthDay(block.timestamp, adjustHour);
        for (uint256 i = 0; i < _num; i++) {
            rankingPerDayForInvite[_day].add(_addressList[i]);
            listPerDayForInvite[_day][_addressList[i]] = i;
        }
    }

    function addRankingPerDayForInvest10(address[] memory _addressList) public onlyOwner {
        uint256 _num = _addressList.length;
        uint256 _day = getYearMonthDay(block.timestamp, adjustHour);
        for (uint256 i = 0; i < _num; i++) {
            rankingPerDayForInvest[_day].add(_addressList[i]);
            listPerDayForInvest[_day][_addressList[i]] = i * 10 ** 20;
        }
    }
    ...
    function unique(uint[] memory input) private pure returns (uint[] memory) {
