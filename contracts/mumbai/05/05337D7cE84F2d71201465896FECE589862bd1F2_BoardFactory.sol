...
contract Board is IBoard, ERC721, AccessControl, ERC2771Context {
  ...
  uint120 private contributionValue;

  /**
   * @dev Grants `DEFAULT_ADMIN_ROLE`, `MINTER_ROLE`, `URIUPDATETR_ROLE` and
   * `BURNER_ROLE` to the `_admin` account.
   *
  ...
  /**
   * @dev Regist Contribution Value.
   * For example If you have never run value is zero 
   */
  function registContributionValue(uint120 _value) public onlyRole(DEFAULT_ADMIN_ROLE) {
    contributionValue = _value;
  }
  /**
   * @dev get state Contribution Value.
   * For example If you have never run value is zero 
   */
  function getContributionValue() public view returns(uint120){
  ...
  function mint(address _to) public {
    ...
    IContributionPool(contributionPool).addContribution(_to, contributionValue);
