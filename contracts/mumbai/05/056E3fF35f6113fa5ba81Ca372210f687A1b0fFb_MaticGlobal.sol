...
contract MaticGlobal {
  ...
  function invest(address referrer) public payable {
   ...
   function withdrawal() external{
    ...
    (uint256 UplineBonus, uint256 TotalUplineBonus) = _userUplineIncome(msg.sender);
    _user.singleUplineBonus =  _user.singleUplineBonus.add(TotalUplineBonus);

    if(UplineBonus>=_user.singleUplineBonusTaken)
    {
      uint256 TotalUplineTaking = UplineBonus.sub(_user.singleUplineBonusTaken);
      usersDetails[msg.sender].nonworkingIncomeCappingReceived = usersDetails[msg.sender].nonworkingIncomeCappingReceived.add(TotalUplineTaking);
      if(usersDetails[msg.sender].nonworkingIncomeCappingRemain>=TotalUplineTaking)
      {
        usersDetails[msg.sender].nonworkingIncomeCappingRemain = usersDetails[msg.sender].nonworkingIncomeCappingRemain.sub(TotalUplineTaking);
      }
      else
      {
        usersDetails[msg.sender].nonworkingIncomeCappingRemain =usersDetails[msg.sender].nonworkingIncomeCappingRemain.sub(usersDetails[msg.sender].nonworkingIncomeCappingRemain);
      }
      _user.singleUplineBonusTaken = _user.singleUplineBonusTaken.add(TotalUplineTaking);
    }
  ...
  function _refPayout(address _addr, uint256 _amount) internal {
   ...
   function _usersDownlineIncomeDistribution(address _user, uint256 _Amount) internal {
      uint256 DistributionPayment = _Amount.mul(maxupline).div(perDistribution);
  ...
  function _userUplineIncome(address _user) public view returns(uint256,uint256) { 
      address upline = users[_user].singleUpline;
      uint256 UplineBonus=0;
      uint256 TotalUplineBonus=0;
      for (uint i = 0; i < maxdownline; i++) {
            if (upline != address(0)) {
                if(upline_Business_eligible[upline]){                    
                    ...
                    uint256 poolPayableAmount = principleAmount.mul(maxdownline).div(perDistribution);
                    uint256 individualPayableAmount = poolPayableAmount.div(maxdownline); 
                    if(_CheckIncomeEligibility(_user,individualPayableAmount) && usersDetails[_user].nonworkingIncomeCappingRemain>=(UplineBonus.add(individualPayableAmount)))                  
                    {
                        UplineBonus = UplineBonus.add(individualPayableAmount);                  
                    } 
                    TotalUplineBonus=TotalUplineBonus.add(individualPayableAmount);                  
       ...
       uint256 CurentUplineBonus = UplineBonus;
       uint256 ActualUplineAmount=0;

       // If Total Upline Bonus Equals To Taken Upline+Current Upline
       if(TotalUplineBonus==(CurentUplineBonus.add(users[_user].singleUplineBonusTaken)))
       {
         ActualUplineAmount=CurentUplineBonus.add(users[_user].singleUplineBonusTaken);
       }
       // If Total Upline Bonus Equals To Taken Upline
       else if(TotalUplineBonus==users[_user].singleUplineBonusTaken)
       {
         ActualUplineAmount=users[_user].singleUplineBonusTaken;
       }
       // If Current Upline Equals To Zero
       else if(CurentUplineBonus==0)
       {
         ActualUplineAmount=users[_user].singleUplineBonusTaken;
       }
       // If Current Upline Greater Than Zero AND Taken Upline Is Zero
       else if(CurentUplineBonus>0 && users[_user].singleUplineBonusTaken==0)
       {
         ActualUplineAmount=CurentUplineBonus;
       }
       // If Current Upline Greater Than Zero AND Taken Upline Is Also Greater Than Zero
       else if(CurentUplineBonus>0 && users[_user].singleUplineBonusTaken>0)
       {
         if(CurentUplineBonus.add(users[_user].singleUplineBonusTaken)<TotalUplineBonus)
         {
            ActualUplineAmount=CurentUplineBonus.add(users[_user].singleUplineBonusTaken);
         }
         else
       ...
       return (ActualUplineAmount, TotalUplineBonus);
  }
  ...
  function getEligibleWithdrawal(address _user) public view returns(uint8 reivest, uint8 withdrwal){  
   ...
   function TotalBonus(address _user) public view returns(uint256){
     (uint256 UplineBonus, uint256 TotalUplineBonus) = _userUplineIncome(msg.sender);
     TotalUplineBonus=0;
     uint256 TotalEarn = users[_user].referrerBonus.add(UplineBonus).add(users[_user].singleDownlineBonus);
     uint256 TotalTakenFromUplineDownline = users[_user].singleDownlineBonusTaken.add(users[_user].singleUplineBonusTaken);
     return TotalEarn.sub(TotalTakenFromUplineDownline);
   }
