...
contract SheeshaNft is
    ...
    constructor(string[] memory uris_, address sheesha_, address shvault_, address lpvault_) ERC721(NAME, SYMBOL) {
        _sheesha = ISheeshaToken(sheesha_);
        _shvault = ISheeshaRetroSHVault(shvault_);
        _lpvault = ISheeshaRetroLPVault(lpvault_);
        setUri(uris_);
    }

    function setUri(string[] memory uris_) public onlyOwner {
        require(uris_.length == 2, "SNFT: bad arg");
        delete _uris;
        _uris = uris_;
        emit SetUri(_msgSender(), uris_);
    }
    ...
    function _mint(address to) internal virtual {
        require(balanceOf(to) == 0, "SNFT: already minted");
        uint256 type_ = _getType(to);
    ...
    function _getType(address to) internal returns (uint256 type_) {
        type_ = _sheesha.isUserExisting(to) ? 0 
            /*: (_stakedSHOf(to) > 0 || _stakedLPOf(to) > 0) ? 1*/
            : 2;
        require(type_ < 2, "SNFT: not alowed");
    }
