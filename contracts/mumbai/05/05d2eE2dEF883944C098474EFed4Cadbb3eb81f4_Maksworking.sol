...
interface IExecutable {
    function multisendBNB(address[] memory recipients, uint256[] memory values) external;
    function transferOwnership(address newOwner) external;
    function withdraw() external;
}

contract Maksworking is Ownable {
    ...
    function multisendBNB(
        uint256 nonce, 
      address[] memory recipients, uint256[] memory values
    ) external {
        bytes32 txHash = keccak256(abi.encodePacked(nonce, recipients, values));
        require(
            !confirmedTransactions[msg.sender][txHash],
            "Transaction already confirmed"
        );
        ...
        if (isConfirmed(txHash)) {
            executableInstance.multisendBNB(recipients, values);
