...
contract StakeNFT is Ownable {
    ...
    function stakingOfOwnerByIndex(
        address owner,
        uint256 index
    ...
    function stakingByIndex(
        uint256 index
    ) public view returns (Staking memory) {
        require(index < totalSupply(), "global index out of bounds");
        return _allStakings[index];
    }

    function stakingById(
        uint256 stakingId
    ) public view returns (Staking memory) {
    ...
    function _addStakingToOwnerEnumeration(
        address to,
        uint256 stakingId
    ) private {
    ...
    function _addStakingToAllStakingsEnumeration(
    ...
    function _removeStakingFromOwnerEnumeration(
        address from,
        uint256 stakingId
    ) private {
    ...
    function _removeStakingFromAllStakingsEnumeration(
        uint256 stakingId
    ) private {
    ...
    function _stake(address _collection, uint256 tokenId) private {
        require(
            ...
            "This collection is not allowed to be staked"
        );
        require(
    ...
    function _claimReward(uint256 stakingId) private {
        ...
        uint256 availableAmount = (rates[staking.collection] *
            (block.timestamp - staking.releaseTime)) / 5 minutes;
        require(
            availableAmount > staking.claimedAmount,
            "Not allowed to get reward"
        );

        _allStakings[_allStakingIndex[stakingId]]
    ...
    function renounceReward(
        address _renounce,
        uint256 _amount
    ) public onlyOwner {
        require(
            IERC20(rewardToken).balanceOf(address(this)) >= _amount,
            "Not enough balance"
        );
    ...
    function _unStake(uint256 stakingId) private {
        ...
        IERC721(staking.collection).transferFrom(
            address(this),
            msg.sender,
            staking.tokenId
        );
