...
contract VRFv2Consumer is VRFConsumerBaseV2, ConfirmedOwner {
    ...
    event OnRandomnessShuffled(uint256 reuse_count,uint256 nounce,uint256[] values);
    ...
    function random(uint256 range) external returns (uint256){
        ...
        uint256 _rand_value = (words[last_fullfilled_id][index] +  words_reuse_count[last_fullfilled_id]) % range;
        ...
        if(_new_reuse_count > words_reuse_count[last_fullfilled_id]){
            ...
            emit OnRandomnessShuffled(words_reuse_count[last_fullfilled_id],nounces[last_fullfilled_id],words[last_fullfilled_id]);
        }
        emit OnRandomed(msg.sender,_rand_value);
        return _rand_value;
    }
    ...
    function get_randomness(uint256 _request_id) public view returns (bool fulfilled,uint256,uint256, uint256[] memory randomWords) {
        return (fullfiled[_request_id],nounces[_request_id],words_reuse_count[last_fullfilled_id], words[_request_id]);
    }
