    ...
    AmberfiKeyStorageUpgradeable
{
    ...
    function initialize(address marketAddress_, address amberfiStorageAddress_)
        public
        initializer
    {
        __ERC721_init("Amberfi", "AMBR");
    ...
    function mint(
        ...
        setContentAsEncrypted(newItemId, isEncrypted_);
        setContentURI(newItemId, contentUri_);
        if (to_ == address(0x0)) _safeMint(_msgSender(), newItemId, "");
        else _safeMint(to_, newItemId, "");
        if (bytes(tokenUri_).length > 0) _setTokenURI(newItemId, tokenUri_);
        setApprovalForAll(marketAddress, true);
        AmberfiStorage astorage = AmberfiStorage(
            address(amberfiStorageAddress)
        );
        astorage.registerStorage(newItemId, tokenUri_, storageLocationId_);
        if (royaltyValue_ > 0) {
            address recipient = royaltyRecipient_;
            if (recipient == address(0)) {
                recipient = _msgSender();
            }
            _setTokenRoyalty(newItemId, recipient, royaltyValue_);
        }
    ...
    function stringVar(Status status) internal pure returns (string memory) {
        if (status == Status.Cancelled) return "1";
        if (status == Status.InProgress) return "2";
        if (status == Status.Failed) return "3";
        if (status == Status.Finalized) return "4";
        return "0";
    }
...
abstract contract MinterAccessControl {
     ...
     *
     * @param account_ an address which is granted minter role
     */
    function _grantMinterRole(address account_) internal virtual {
        require(account_ != address(0), "invalid address");
        minters[account_] = true;
        emit MinterRoleGranted(msg.sender, account_);
    }
     ...
     * @dev this function can only be called by owner
     *
     * @param account_ an address which is revorked minter role
     */
    function _revokeMinterRole(address account_) internal virtual {
        require(account_ != address(0), "invalid address");
        minters[account_] = false;
        emit MinterRoleRevoked(msg.sender, account_);
    }
...
interface IERC721TokenURIAmberfiStorageContentFactory {
    struct Content {
        uint256 tokenId;
        address owner;
...
// Interfaces
import "../interfaces/IERC721TokenURIAmberfiStorageContentFactory.sol";
...
contract ERC721TokenURIAmberfiStorageContentUpgradeable is
    ...
    OwnableUpgradeable,
    ERC165Upgradeable,
    IERC721TokenURIAmberfiStorageContentFactory
{
    ...
    function supportsInterface(bytes4 interfaceId_)
        ...
        view
        virtual
        override(ERC165Upgradeable, IERC721TokenURIAmberfiStorageContentFactory)
        returns (bool)
    {
        return
            interfaceId_ ==
            type(IERC721TokenURIAmberfiStorageContentFactory).interfaceId ||
            super.supportsInterface(interfaceId_);
    }
}
