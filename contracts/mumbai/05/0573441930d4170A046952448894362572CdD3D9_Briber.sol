...
contract Briber is AutomateReady, Pausable {
    ...
    function execBribe(
        ...
        bool executed = plan.style == PlanStyle.FIXED
            ? _execBribe(key, plan, proposal) : plan.style == PlanStyle.PERCENT
            ? _execBribePct(key, plan, proposal)
            : _execBribeAll(key, plan, proposal);

        if (!executed) {
            // plan was skipped due to insufficient tokens
            if (plan.canSkip)
        ...
        // remove plan if it is completed (no more epochs remaining)
    ...
    function _createPlan(
        ...
        require(address(token) != address(0), "Briber._createPlan: invalid token");
        require(
    ...
    function _execBribe(
        ...
        _bribe(key, plan, plan.amount, proposal);
        return true;
    }

    function _execBribePct(
        bytes32 key,
        Plan storage plan,
        bytes32 proposal
    ) internal returns (bool) {
        uint256 amount = _getAvailable(plan.token) * 1000 / plan.amount;

        if (amount == 0)
            return false;

        _bribe(key, plan, amount, proposal);
        return true;
    }
    ...
    function _bribe(
        bytes32 key,
        Plan storage plan, // IS IT BETTER TO REMOVE THIS?
        uint256 amount,
        bytes32 proposal
    ) internal {
        ...
        emit ExecutedBribe(key, proposal, plan);
    }
...
library Mapping {
    ...
    function set(Map storage map, bytes32 key, Plan memory value) internal { // change back to storage
    ...
    function get(Map storage map, bytes32 key)internal view returns (Plan storage) {
...
address constant BRIBE_VAULT = 0x9DDb2da7Dd76612e0df237B89AF2CF4413733212;
