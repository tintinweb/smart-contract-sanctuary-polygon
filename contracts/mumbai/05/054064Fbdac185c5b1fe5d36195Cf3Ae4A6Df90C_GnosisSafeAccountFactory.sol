...
contract EIP4337Manager is IAccount, GnosisSafeStorage, Executor {
    ...
    function validateUserOp(
        ...
        // bytes _calldata = userOp.callData;
        ...
        if (missingAccountFunds > 0) {
            ...
            (bool success, ) = payable(msgSender).call{
    ...
    function executeAndRevert(
        ...
        require(msgSender == entryPoint, "account: not from entrypoint");
        require(
            msg.sender == eip4337Fallback,
            "account: not from EIP4337Fallback"
        );
     ...
     */
    function getCurrentEIP4337Manager(GnosisSafe safe)
        public
        view
        returns (address prev, address manager)
    {
        ...
        for (uint i = 0; i < modules.length; i++) {
            ...
            try EIP4337Fallback(module).eip4337manager() returns (
                address _manager
            ) {
...
contract GnosisSafeAccountFactory {
    ...
    function getInitializer(address owner)
        internal
        view
