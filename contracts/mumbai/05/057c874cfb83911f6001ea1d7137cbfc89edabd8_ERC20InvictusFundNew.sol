  ...
  ERC20PausableUpgradeable
{
  ...
  function __ERC20PresetMinterPauser_init_unchained(string memory name, string memory symbol)
    internal
    onlyInitializing
  {
  ...
  function _handleRedemptionsDealing() private returns (bool) {
    ...
    for (uint256 i = redemptionsLength(); i > endingIndex; i--) {
      ...
      if (!InvictusWhitelist(whitelistContract).isWhitelisted(redemption.investor)) {
        redemptions.pop();
        transferFrom(DEAL_ADDRESS, redemption.investor, redemption.amount);
