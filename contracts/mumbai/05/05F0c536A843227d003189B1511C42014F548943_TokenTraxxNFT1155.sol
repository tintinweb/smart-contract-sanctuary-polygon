...
// Extensions
import "./utils/ERC1155EnumerableUpgradeable.sol";
import "./utils/ERC1155URIStorageUpgradeable.sol";
    ...
    DefaultOperatorFiltererUpgradeable
{
   ...
   /**
     ...
     */
     // TODO : validation for ids, amounts, uris, maxPurchaseLimit in a for loop 
    function batchMintTraxx(
        ...
        require(
            (ids.length == amounts.length) && (amounts.length == uris.length) && (uris.length == maxPurchaseLimit.length),
            throwError(INVALID_FUNCTION_PARAMS)
        );
    ...
    function setContractURI(string memory contractUri) external onlyMaintainer {
         require(bytes(contractUri).length > 0, throwError(INVALID_FUNCTION_PARAMS));
    ...
    function setPrimarySalePurchaseLimitPerToken(
        ...
        require(_tokenId >= 0 && _tokenId <= _tokenIdTracker.current(), throwError(TOKEN_ID_NOT_FOUND));
   ...
   */
    ...
    /**
     * @dev See {ERC1155Upgradeable-setApprovalForAll}.
    ...
    /**
     * @dev See {ERC1155Upgradeable-safeTransferFrom}.
    ...
    /**
     * @dev See {ERC1155Upgradeable-safeBatchTransferFrom}.
     *      The added modifier ensures that the operator is allowed by the OperatorFilterRegistry.
     */
    function safeBatchTransferFrom(
        address from,
