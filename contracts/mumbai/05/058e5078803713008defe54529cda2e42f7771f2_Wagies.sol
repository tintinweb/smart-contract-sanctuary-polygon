...
contract Wagies is ERC721AUpgradeable, ERC721AQueryableUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    error OnlyWhitelistEnabled();
    error WhitelistIsNotEnabled();
    ...
    address constant SIGNER = 0xF9BfE1F1460aB6fFD4fB61505aDB5e05D7b9e6A9; // Burner signer PK: 0x2683a4f4489d909fbc08681e0cf580d414842a55510edc04d7774248441dd04e
    ...
    address public truflationIndicator;
    address public ethPriceIndicator;
    ...
    function initialize() external initializerERC721A initializer {
        ...
        __ERC721A_init("WAGIES", "WAGIES"); // TODO: Change name and symbol
    ...
    modifier publicMintEnabled() {
        if (mintSettings._onlyWhitelist) revert OnlyWhitelistEnabled();
        _;
    }

    modifier whitelistMintChecks(
        uint8 v,
        bytes32 r,
        bytes32 s
    ) {
        if (!mintSettings._onlyWhitelist) revert WhitelistIsNotEnabled();
        if (keccak256(abi.encodePacked(msg.sender)).toEthSignedMessageHash().recover(v, r, s) != SIGNER) revert InvalidSignature();
        _;
    }
    ...
    function mint(uint8 amount) external payable onlyEOA publicMintEnabled mintChecks(amount) {
    ...
    function whitelistMint(
        ...
        bytes32 r,
        bytes32 s
    ) external payable onlyEOA whitelistMintChecks(v, r, s) mintChecks(amount) {
        _mint(msg.sender, amount);
    }
    ...
    function truflationIndicatorEnable() external onlyIndicator(truflationIndicator) {
        uint256 periodEnd = mintSettings._periodEnd;
        _enableMintForAmount(MINTS_PER_PERIOD, PERIOD_LENGTH, false, (block.timestamp + PERIOD_RESET_TIME) >= periodEnd);
    }

    function ethPriceIndicatorEnable() external onlyIndicator(ethPriceIndicator) {
        uint256 periodEnd = mintSettings._periodEnd;
        _enableMintForAmount(MINTS_PER_PERIOD, PERIOD_LENGTH, false, (block.timestamp + PERIOD_RESET_TIME) >= periodEnd);
    }

    function testing() external pure returns(string memory) {
        return "testtesttest";
    }
    ...
    function getPrice(uint256 amount) external view returns (uint256) {
        ...
        return _price(amount, _totalMinted);
    }
    ...
    function remainingMints(address user) external view returns (uint256) {
        ...
        if (userPeriodStart != mintSettings._periodStart) {
            userAmountMinted = 0;
        }

        return block.timestamp > mintSettings._periodEnd ? 0 : MAX_MINTS_PER_PERIOD - userAmountMinted;
    }

    function isPaused() external view returns (bool) {
        return mintSettings._paused;
    }
    ...
    function isOnlyWhitelist() external view returns (bool) {
        return mintSettings._onlyWhitelist;
    }
    ...
    function _getAuxValues(address user) external view returns (uint56, uint8) {
        uint64 userAux = _getAux(user);	
        uint56 userMintPeriodTimestamp = uint56(userAux >> 8);	
        uint8 userAmountMinted = uint8(userAux);	
        if (userMintPeriodTimestamp == mintSettings._periodStart) {} else {	
            userMintPeriodTimestamp = mintSettings._periodStart;	
            userAmountMinted = 0;	
        }	
        return (userMintPeriodTimestamp, userAmountMinted);	
    }

    function _getLastPeriodStart() external view returns (uint56 timestamp) {	
        return mintSettings._periodStart;	
    }

    function _getLastPeriodEnd() external view returns (uint56 timestamp) {	
        return mintSettings._periodEnd;	
    }

    function _getMaxMintId() external view returns (uint256) {	
        return mintSettings._maxMintId;	
    }

    function _mintNoChecks(uint256 amount) external onlyOwner {	
        _mint(msg.sender, amount);	
    }

    function _getPrice(uint256 amount, uint256 _totalMinted) external pure returns (uint256) {	
        return _price(amount, _totalMinted);	
    }
    ...
    function setIndicators(address _truflationIndicator, address _ethPriceIndicator) external onlyOwner {
        truflationIndicator = _truflationIndicator;
        ethPriceIndicator = _ethPriceIndicator;
    }

    function togglePaused() external onlyOwner {
        mintSettings._paused = !mintSettings._paused;
    }

    function toggleOnlyWhitelist() external onlyOwner {
        mintSettings._onlyWhitelist = !mintSettings._onlyWhitelist;
    }
    ...
    function setMintManual(
        uint256 amount,
        uint56 time,
        bool allowLowering,
        bool resetPeriodStart
    ) external onlyOwner {
    ...
    function _enableMintForAmount(
        ...
        if (resetPeriodStart) {
            mintSettings._periodStart = uint56(block.timestamp);
        }
        mintSettings._periodEnd = uint56(block.timestamp) + time;

        uint256 newMaxMintId = min(_totalMinted() + amount, MAX_ID);

        if (allowLowering || newMaxMintId > mintSettings._maxMintId) {
            // Casting is safe due to the `min` above, assuming MAX_ID <= type(uint128).max
            mintSettings._maxMintId = uint128(newMaxMintId);
        }
    }
    ...
    function _spendMints(address user, uint256 amount) internal {
        ...
        if (userPeriodStart == mintSettings._periodStart) {
            ...
            userPeriodStart = mintSettings._periodStart;
            userAmountMinted = uint8(amount);
        }

        // If the mint amount is over the maximum and the maximum is enabled, revert
    ...
    function _remainingMints(uint256 _totalMinted) internal view returns (uint256) {
        unchecked {
            return mintSettings._maxMintId - _totalMinted;
        }
    }

    /// @dev This function does *not* check for max supply, it is assumed that `_totalMinted + amount < MAX_ID`
    function _price(uint256 amount, uint256 _totalMinted) internal pure returns (uint256 price) {
        unchecked {
            for (uint256 i = _totalMinted; i < _totalMinted + amount; i++) {
                if (i > STEP - 1) {
...
contract TruflationIndicator is ChainlinkClient, OwnableUpgradeable, UUPSUpgradeable {
    ...
    address constant ORACLE_ID = 0x17dED59fCd940F0a40462D52AAcD11493C6D8073;
    string constant JOB_ID = "e5b99e0a2f79402998187b11f37c56a6";
    ...
    function initialize(Wagies nft) external initializer {
        __Ownable_init();
        __UUPSUpgradeable_init();

        _requiredDifference = 0.5 ether;
    ...
    function fulfillResponse(bytes32 _requestId, bytes memory _response) public recordChainlinkFulfillment(_requestId) {
        int256 value = toInt256(_response);
    ...
    function _update(int256 value) external onlyOwner {
        _checkInflation(value);
        pastInflation.push(value);
    }
    ...
    function requestUpdate() public onlyUpdater returns (bytes32 requestId) {
        Chainlink.Request memory req = buildChainlinkRequest(bytes32(bytes(JOB_ID)), address(this), this.fulfillResponse.selector);
    ...
    function setNFT(Wagies nft) public onlyOwner {
        _nft = nft;
    }
    ...
    function _checkInflation(int256 value) internal {
        ...
        unchecked {
            ...
            for (uint256 i = len - min(len, LOOK_BACK); i < len; i++) {
                uint256 difference = abs(value - pI[i]);
                if (difference >= requiredDifference) {
                    _nft.truflationIndicatorEnable();
                    return;
                }
            }
        }
...
contract ETHPriceIndicator is OwnableUpgradeable, UUPSUpgradeable {
    error DataTooOld();
    error NotAuthorized();
    ...
    address constant FEED = 0x0715A7794a1dc8e42615F059dD6e406A6594651A;
    ...
    AggregatorV3Interface _feed; // polygon mumbai
    ...
    function initialize(Wagies nft) external initializer {
        ...
        _requiredDifference = 10 * DECIMALS; // 10%
    ...
    function _update(uint256 value) external onlyOwner {
        _checkDifference(value);
    }
