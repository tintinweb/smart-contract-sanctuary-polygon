...
contract MarketPlace is ERC2771Context, IERC721Receiver, ReentrancyGuard, IMarketPlace {
    ...
    constructor(RroseForwarder forwarder, PriceConverter _priceConverter) // Initialize trusted forwarder
    ...
    function getMarketItem(uint256 itemId) public view returns (MarketItem memory) {
        require(_exists(itemId), "bad itemId");
        return idToMarketItem[itemId];
    }

    function createMarketItem(address nftContract, uint256 tokenId, uint256 price) public payable nonReentrant {
        require(IERC721(nftContract).ownerOf(tokenId) == _msgSender(), "onlyOwner");
        require(price > 0, "bad price");
        if(floorPriceMode) {
            require(price >= INFTMasterPiece(nftContract).getFloorPriceForToken(tokenId), "minimal floor price");
        }
        // require(msg.value == listingPrice, "Price must be equal to listing price fees");
...
/*        console.log("=======MARKETPLACE===createMarketItem=====");
        console.log("ADDRESS MSG SENDER: ", _msgSender());
        console.log("OWNER WHO IS DEPOSITING TOKEN : ", IERC721(nftContract).ownerOf(tokenId));
        console.log("ADDRESS OF MARKETPLACE DEPOSIT : ", address(this));
        console.log("IS MARKETPLACE IS APPROVED : ", IERC721(nftContract).isApprovedForAll(IERC721(nftContract).ownerOf(tokenId), address(this)));
        console.log("======================");*/
        ...
        INFTMasterPiece(nftContract).setStatus(INFTMasterPiece.Status.LISTED, tokenId);
    }
    ...
    function createMarketSale(address nftContract, uint256 itemId, address transferTo) public payable nonReentrant {
        ...
        require(msg.value >= priceInWei, "bad price");

        // Compute the fees
        uint256 fees = priceInWei * INFTMasterPiece(nftContract).getFeesBasisPointsForToken(tokenId) / 10000;
        require(fees >= 0 && fees < priceInWei, "bad fees");
        ...
        INFTMasterPiece(nftContract).setStatus(INFTMasterPiece.Status.UNLISTED, tokenId);
    ...
    function removeMarketItem(address nftContract, uint256 itemId) public {
        require(_exists(itemId), "bad itemId");
        // console.log("owner in remove : ", IERC721(nftContract).ownerOf(idToMarketItem[itemId].tokenId));
        require(idToMarketItem[itemId].seller == _msgSender(), "onlySeller");
