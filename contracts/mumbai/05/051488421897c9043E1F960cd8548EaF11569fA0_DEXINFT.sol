...
contract DEXINFT is ERC721URIStorage, ERC721Enumerable, IERC721TokenCreator, Ownable {
    using Address for address;

    // Index of the current token for minting
    uint256 private mintIndex = 0;

    // address of the contract for token selling
    address public marketContractAddress;

    // Mapping from token to token creator
     ...
     */
    event Minted(address indexed minter, uint256 nftID, string uri, uint256 royalty);
     ...
     */
    constructor() ERC721("DEXI NFT", "DEXINFT") {}

    /**
     * @notice Sets the market contract address
     *
     * @param _marketContractAddress The payment token address.
     */
    ...
    /**
     * @notice Safely mints `_numberOfToken` tokens and sets their `_tokenURI`.
     *
     * Emits a {Transfer} event.
     */
    function bulkMint(address _to, string memory _tokenURI, uint256 _numberOfToken, uint256 _royalty) external {
        require(marketContractAddress != address(0), "Market contract is not set");

        for(uint i = 0; i < _numberOfToken; i++) {
            mint(_to, _tokenURI, _royalty);
        }
    }
     ...
     * Emits a {Transfer} event.
     */
    function mint(address to, string memory _tokenURI, uint256 _royalty) public returns (uint256) {
        require(marketContractAddress != address(0), "Market contract is not set");
        ...
        _approve(marketContractAddress, mintIndex);

        _creators[mintIndex] = to;
        _royalties[mintIndex] = _royalty;

        emit Minted(to, mintIndex, _tokenURI, _royalty);

        return mintIndex;
    }

    /**
     * @notice Returns the creator data of `tokenId`.
     */
    function creator(uint256 tokenId) external view returns(address) {
    ...
    /**
     * @notice Returns the royalty of `tokenId`.
     */
    function royalty(uint256 tokenId) external view returns(uint256) {
        require(_exists(tokenId), "Token does not exist");
