...
contract PhiDaily is AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping(address sender => mapping(uint256 eventid => uint256 _status)) public DailyClaimedStatus;
    /* --------------------------------- ****** --------------------------------- */
    ...
    /*                                   ERRORS                                   */
    /* -------------------------------------------------------------------------- */
    error AllreadyClaimed(address sender, uint256 eventid);
    ...
    /*                                  MODIFIERS                                 */
    /* -------------------------------------------------------------------------- */
    modifier onlyIfAllreadyClaimed(uint256 eventid) {
        if (DailyClaimedStatus[msg.sender][eventid] == _CLAIMED) {
            revert AllreadyClaimed({ sender: msg.sender, eventid: eventid });
        }
        _;
    }
    ...
    function claimMaterialObject(
    ...
    ) external onlyIfAllreadyClaimed(eventid) nonReentrant {
        ...
        ++ClaimedEachCount[msg.sender][tokenid];
        DailyClaimedStatus[msg.sender][eventid] = _CLAIMED;
        emit LogClaimMaterialObject(msg.sender, eventid, logicid, tokenid);
    }

    function checkClaimedCount(address sender) external view returns (uint256 count) {
        return ClaimedCount[sender];
    }

    function checkClaimedEachCount(address sender, uint256 tokenid) external view returns (uint256 count) {
        return ClaimedEachCount[sender][tokenid];
    }

    function checkDailyClaimedStatus(address sender, uint256 eventid) external view returns (uint256 status) {
        return DailyClaimedStatus[sender][eventid];
    }
}