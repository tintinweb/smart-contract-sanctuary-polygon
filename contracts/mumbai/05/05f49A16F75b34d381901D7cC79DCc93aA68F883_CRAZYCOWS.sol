...
contract CRAZYCOWS is ERC1155, Ownable, Pausable {
    ...
    bool public pubsaleStarted = false;
    ...
    function startPresale() public onlyOwner {
        presaleStarted = true;
        presaleEnded = block.timestamp + presalePeriod;
    }

    function startPubsale() internal {
    ...
    function presaleMint(uint256 amount) external payable whenNotPaused {
        require(
            presaleStarted &&
                block.timestamp < presaleEnded &&
                totalSupply < maxPreSale,
            "Presale is not running"
        );
        require(whitelist[msg.sender], "You are not on the Allow List");
        require(totalSupply + amount <= maxSupply, "Exceed maximum NFT supply");
        ...
        totalSupply += amount;
        if (totalSupply >= maxPreSale || block.timestamp >= presaleEnded) {
            startPubsale();
        }
    }
