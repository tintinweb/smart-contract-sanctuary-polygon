...
abstract contract Gate is Slot {
    ...
    function clearGate(
        ...
        if (_gateSignatures.length != stonePerGate + 1)
        ...
        uint256 seasonPackCollectionId = seasonContract
            .getSeasonPackCollectionId(gate.seasonId);

        ISLSeasonPack seasonPack = ISLSeasonPack(
            projectContract.getTokenContractByCollectionId(
                seasonPackCollectionId
            )
        );
        uint256[] memory packTokens = seasonPack.getOpenTokens();

        (uint256[] memory stoneIds, uint256 seasonPackId) = _getGateResult(
            hunter,
            gate.gateRank,
            _gateSignatures,
            packTokens
        ...
        essenceStone.mintBatch(
            hunter,
            stoneIds,
            _asUintArray(1, stoneIds.length)
        );

        if (seasonPackId != 0) {
            seasonPack.mint(hunter, seasonPackId, 1);
        }
    ...
    function _getGateResult(
        address _hunter,
        RankType _gateRank,
        bytes[] calldata _gateSignatures,
        uint256[] memory _packTokens
    ) private returns (uint256[] memory stoneIds, uint256 seasonPackId) {
        uint256[7] memory percentages = percentageForGate[_gateRank];

        stoneIds = new uint256[](stonePerGate);

        for (uint256 i = 0; i < stonePerGate; i = i.increment()) {
        ...
        seasonPackId;
        _verifyRandomSignature(_hunter, _gateSignatures[stonePerGate]);
        bytes memory packSignature = _gateSignatures[stonePerGate];

        if (_packTokens.length > 0) {
            uint256 numberForPack = uint256(keccak256(packSignature)) %
                _packTokens.length;
            seasonPackId = _packTokens[numberForPack];
        }
    }
...
contract SLDungeonGate is Gate, UUPSUpgradeable {
    ...
    function __gateBlockPerRank_init() private {
        gateBlockPerRank[RankType.E] = 720;
        gateBlockPerRank[RankType.D] = 2160;
        gateBlockPerRank[RankType.C] = 4320;
        gateBlockPerRank[RankType.B] = 8640;
        gateBlockPerRank[RankType.A] = 17280;
        gateBlockPerRank[RankType.S] = 34560;
    }
