...
contract DecentralizedAI {
    ...
    function getModelTypes() public view returns (ModelType[] memory) {
        uint256 numModelTypes = 0;
        for (uint256 i = 0; i < 2**256-1; i++) {
            if (bytes(modelTypes[i].name).length > 0) {
                numModelTypes++;
            } else {
                break;
            }
        ...
        ModelType[] memory modelTypeList = new ModelType[](numModelTypes);
        for (uint256 i = 0; i < numModelTypes; i++) {
            modelTypeList[i] = modelTypes[i];
        }
