...
contract DemandBook is IDemandBook, Initializable, AbstractDependant {
    ...
    modifier onlyCapitalPool() {
        require(address(capitalPool) == msg.sender, "DemandBook: Caller is not CP contract");
        _;
    }
    ...
    modifier onlyClaimingRegistry() {
        require(address(claimingRegistry) == msg.sender, "DemandBook: Caller is not CR contract");
        _;
    }
    ...
    function isDemandActive(uint256 demandIndex) public view override returns (bool) {
        return
    ...
    function _getRewardAmount(uint256 demandIndex) internal view returns (uint256 rewardAmount) {
        if (demandInfo[demandIndex].depositedLiquidity > 0) {
            ...
            if (isDemandActive(demandIndex) || _isDemandEnded(demandIndex)) {
                rewardAmount = demandInfo[demandIndex]
                    .premiumAmount
    ...
    function getListDemands(
        ...
        demandPublicInfo = new DemandInfo[](to.uncheckedSub(offset));
        ...
        for (uint256 i = offset; i < to; i = uncheckedInc(i)) {
            ...
            demandPublicInfo[newIndex] = demandInfo[demandIndex];
            demandPublicInfo[newIndex].publicDemandInfo.status = getDemandStatus(demandIndex);
            if (demandPublicInfo[newIndex].publicDemandInfo.status == DemandStatus.OPEN) {
                demandPublicInfo[newIndex].publicDemandInfo.time = demandInfo[demandIndex]
                    .creationTime
                    .tryAdd(OPEN_PERIOD)
                    .trySub(block.timestamp);
            } else if (
                demandPublicInfo[newIndex].publicDemandInfo.status == DemandStatus.PENDING
            ) {
                demandPublicInfo[newIndex].publicDemandInfo.time = demandInfo[demandIndex]
                    .creationTime
                    .tryAdd(OPEN_PERIOD)
                    ...
                    .trySub(block.timestamp);
            } else if (demandPublicInfo[newIndex].publicDemandInfo.status == DemandStatus.ACTIVE) {
                demandPublicInfo[newIndex].publicDemandInfo.time = demandInfo[demandIndex]
                    .endTime
                    .tryAdd(STILL_CLAIMABLE_FOR)
                    .trySub(block.timestamp);
            }
        }
    }
    ...
    function _validateDemand(uint256 demandIndex) internal {
        ...
        demandInfo[demandIndex].endTime = block.timestamp.tryAdd(
            (epochAmount).tryMul(EPOCH_DURATION)
        );

        uint256 premiumAmount = demandInfo[demandIndex].premiumAmount;
        uint256 reinsurancePoolAmount =
            premiumAmount.mul(PROTOCOL_PERCENTAGE).uncheckedDiv(PERCENTAGE_100);
        ...
        returns (uint256)
    {
        ...
        return
            coverageAmount.div(100).mul(DECIMALS18).div(
    ...
    function submitClaimAndInitializeVoting(
        ...
        _submitClaimAndInitializeVoting(demandIndex, _evidenceURI, bmiPriceInUSDT, false);
    }
    ...
    function submitAppealAndInitializeVoting(
        ...
        _submitClaimAndInitializeVoting(demandIndex, _evidenceURI, bmiPriceInUSDT, true);
    }
    ...
    function _submitClaimAndInitializeVoting(
        ...
        require(isDemandActive(demandIndex), "DB: Demand is not available to claim");
        address demander = demandInfo[demandIndex].demander;
        require(msg.sender == demander, "DB: Cannot claim this demand");
        _excludeDefiLostLiquidity(demandIndex, _countLostInDefi());
        uint256 coverage =
        ...
        claimVotingFacade.initializeVoting(
            claimProvenance,
            _evidenceURI,
            coverage,
            bmiPriceInUSDT,
    ...
    function commitClaim(
        ...
        if (_status == IClaimingRegistry.ClaimStatus.REJECTED) {
            _endDemand(demandIndex, _endTime);
        }
    }
