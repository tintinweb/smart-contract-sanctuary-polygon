...
contract TokenStaking is ReentrancyGuard, Ownable {
    ...
    // Reward rate (block)
    uint public currentRewardPerBlock;
    ...
    function deposit(uint256 amount) external nonReentrant updateReward(msg.sender) {
        require(amount >= PRECISION_FACTOR, "Deposit: Deposit amount must be >= 1");
    ...
    function _calculatePendingRewards(address user) internal view returns (uint256) {
        return
        ((userInfo[user].shares * (_rewardPerToken() - (userInfo[user].userRewardPerTokenPaid))) / PRECISION_FACTOR) +
 ...
 * @notice Withdraw staked tokens (and collect reward tokens if requested)
    ...
    function getUserPendingRewards(address account) external view returns (uint256) {
...
contract TokenStakingFactory is ITokenStakingFactory {

    function createTokenStaking(address WETH, address fanToken) external override returns (address) {
        TokenStaking staking = new TokenStaking(WETH, fanToken);
