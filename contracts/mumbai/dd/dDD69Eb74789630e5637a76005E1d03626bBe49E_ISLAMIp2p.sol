...
*/  
    ...
    mapping(uint => uint) public userOrder;
...
*/
    ...
    function createOrder(
        ...
        if(_type == 1){ //sell ISLAMI
            p2p[msg.sender].remainAmount = _amount.mul(ISLAMIdcml);
    ...
    function cancleOrder() external nonReentrant{
        ...
        uint256 _orderIndex = userOrder[p2p[msg.sender].orderNumber];
        if(p2p[msg.sender].orderType == 1){
            uint256 amount = p2p[msg.sender].remainAmount;
            ISLAMI.transfer(msg.sender, amount);
            sellOrders--;
        }
        else if(p2p[msg.sender].orderType == 2){
            uint256 amount = p2p[msg.sender].remainCurrency;
            IERC20 currency = p2p[msg.sender].orderCurrency;
            currency.transfer(msg.sender, amount);
            buyOrders--;
        }
        delete userOrder[_orderIndex];
        delete p2p[msg.sender];
    ...
    function fixOrders(uint256 _orderIndex) internal{
        for(uint i = _orderIndex; i < OrderDetails.length-1 ; i++){
            ...
            userOrder[_orderIndex++] -= 1;
            //p2p[_orderIndex + 2].orderIndex -= 1;
            //OrderDetails[_orderIndex + 1].orderIndex -=1;
        }
    ...
    function takeOrder(address _orN, uint256 _amount) external nonReentrant{
