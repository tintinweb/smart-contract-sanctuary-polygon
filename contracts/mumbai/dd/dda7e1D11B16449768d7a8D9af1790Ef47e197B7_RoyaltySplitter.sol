  ...
  IRoyaltySplitter 
{
  ...
  uint256 public ethTotalReleased;
  ...
  mapping(IERC20 => mapping(address => uint256)) public erc20Vault;
  ...
  function releasable(address account) public view returns (uint256) {
    ...
    return payment + ethVault[account];
  }
    ...
    public view returns (uint256) 
  {
    ...
    return payment + erc20Vault[token][account];
  }
   ...
   */
  function accept(IERC20 token) public virtual {
    for (uint256 i = 0; i < erc20Accepted.length; i++) {
      require(erc20Accepted[i] != token, "Token already accepted");
    }

    erc20Accepted.push(token);
  }

  /**
   * @dev Moves the given eth funds to a payees vault
   */
  function disburse(address[] memory _payees, uint256[] memory _shares) 
    public virtual payable
  {
    require(
      _payees.length == _shares.length, 
      "RoyaltySplitter: payees and shares length mismatch"
    );
    require(_payees.length > 0, "RoyaltySplitter: no payees");
    ...
    for (uint256 i = 0; i < _payees.length; i++) {
      ...
      account = _payees[i];
      //store eth
      ethVault[account] += _pendingPayment(msg.value, _shares[i], sharesTotal, 0);
    }
  }

  /**
   * @dev Moves the given erc20 funds to a payees vault
   */
  function disburse(
    IERC20 token, 
    uint256 amount,
    ...
    require(_payees.length > 0, "RoyaltySplitter: no payees");
    //transer token from sender to here
    ...
    for (uint256 i = 0; i < _payees.length; i++) {
      ...
      account = _payees[i];
      //store eth
      erc20Vault[token][account] += _pendingPayment(amount, _shares[i], sharesTotal, 0);
    }
  }
  ...
  /**
   * @dev Vaults the amounts in order to update the `payees` and `shares`
   */
  function update(address[] memory _payees, uint256[] memory _shares) 
    public virtual onlyRole(DEFAULT_ADMIN_ROLE)
  {
    require(_payees.length == _shares.length, "RoyaltySplitter: payees and shares length mismatch");
    require(_payees.length > 0, "RoyaltySplitter: no payees");
    _purge();
    //now add payees
    for (uint256 i = 0; i < _payees.length; i++) {
  ...
  /**
   * @dev Safely removes all payees and shares by migrating account values to a vault
   */
  function _purge() internal virtual {
    IERC20 token;
    address account;
    //loop through payees
    for (uint256 i = 0; i < payees.length; i++) {
      ...
      account = payees[i];
      //store eth
      ethVault[account] += releasable(account) - ethVault[account];
      //loop through the accepted erc20 tokens
      for (uint256 j = 0; j < erc20Accepted.length; j++) {
        //get the erc20
        token = erc20Accepted[j];
        //store the token
        erc20Vault[token][account] += releasable(token, account) - erc20Vault[token][account];
      }
  ...
  function _pendingPayment(
    ...
    return (totalReceived * accountShares) / sharesTotal - alreadyReleased;
  }
}
...
interface IRoyaltySplitter {
   ...
   * @dev Considers tokens that can be vaulted
   */
  function accept(IERC20 token) external;
