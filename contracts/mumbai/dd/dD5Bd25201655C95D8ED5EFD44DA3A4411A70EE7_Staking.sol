...
contract Staking is IERC721Receiver, Context {
    ...
    *@dev lastClaim is in epoch time.
    ...
    function setERC20(address _contractAddress) external onlyOwner {
        require(_contractAddress.isContract(), "not a contract");
    ...
    function _stake(uint _tokenId, address _from) private {
        require(msg.sender == whiteListedContract, "only base contract is allowed to stake");
        StakingInfo memory copy = stakingInfo[_from]; //copy to store data

        //Update user staking informations
        stakingInfo[_from].multiplier = 1 + (copy.stakedNftNum + copy.stakedNftNum /10); // add 1.1 every time
        (copy.stakedNftNum !=0) ? stakingInfo[_from].claimed += _toClaim(_from) : stakingInfo[_from].claimed = 0; //when new NFT is staked, already generated amount is automatically claimed 
        
        if(copy.stakedNftNum !=0) _claim(_from); //claim all generated gains
        
        stakingInfo[_from].stakedNftNum = copy.stakedNftNum +1;
        stakingInfo[_from].lastClaim = block.timestamp; // reset last claim to now
    ...
    function unstake(uint _tokenId) external {
        ...
        uint toClaim = _toClaim(user);

        //Update user staking informations
        stakingInfo[user].multiplier = copy.multiplier - (1 + copy.stakedNftNum/10);
        stakingInfo[user].claimed += toClaim;
        stakingInfo[user].stakedNftNum = copy.stakedNftNum - 1;
        (stakingInfo[user].stakedNftNum !=0) ? stakingInfo[user].lastClaim = block.timestamp : stakingInfo[user].lastClaim = 0; // reset last claim to now or 0. Ensure if no NFT is staking user won't generate $AMMO
        ...
        ammo.mint(_msgSender(), toClaim ); // creates ERC20 tokens into the user's address
        emit Unstake(user, _tokenId);
    }
    ...
    function _toClaim(address _user) internal view returns(uint){
        ...
        uint toClaim = 10 ** 18 * (block.timestamp - SI.lastClaim) * SI.multiplier / rewardSpeed;
        return toClaim;
    }
    ...
    *@notice Returns the amount of ERC20 tokens generated that a user can claim
     */
    function logToClaim() public view returns(uint) {
