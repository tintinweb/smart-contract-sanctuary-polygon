...
contract Sherwood is ERC721, ERC721URIStorage, Ownable {
	...
	uint256 public MAX_TOKENS=8218;
	uint256 public MAX_TOKENS_PRESALE=8218;
	uint256 public FEE;
	...
	address constant ReservedAddress = 0x218fcc82A5cA0672bc07De63E32d03230e5E5e76;
	
	
	

    constructor(string memory mybaseuri, string memory myrevealuri) ERC721("Sherwood", "SHRWD") {
		_tokenIdCounter.increment();
		setBaseURI(mybaseuri);
		setprerevealURI(myrevealuri);
		FEE=73000000000000000000;
		FEEpresales=73000000000000000000;
		MAXNFTperTX = 10;
		reveal=false;
		MAXNFTperADDR = 5;
	...
	function whitelistUserBulk(address[] memory _users)  external onlyOwner {
        uint i;
	...
	function settokenidreserved(uint256 _newtokenidreserved) external onlyOwner {
		require(_newtokenidreserved>8218,"value must be above 5000");
		require(_newtokenidreserved<8333,"value must be below 5500");
		_tokenidreserved = _newtokenidreserved;
	}
	
	function setReveal(bool _newreveal)  external onlyOwner {
		reveal = _newreveal;
	}
	
	function setMaxTokensPresale(uint256 _newMax)  external onlyOwner {
	...
	function startpresale(bool _state)  external onlyOwner {
		presale = _state;
	}

	function startpublicsale(bool _state) external onlyOwner {
		publicsale = _state;
	}
	
	function setMaxNFTperBuyer(uint256 newMax)  external onlyOwner {
		MAXNFTperADDR = newMax;
	}
	
	function setMaxNFTperTX(uint256 newMax)  external onlyOwner {
		MAXNFTperTX = newMax;
	}
	...
	function increaseCounter()  external onlyOwner {
		_tokenIdCounter.increment();
	}
	
	function decreaseCounter()  external onlyOwner {
	...
	//Airdrop function
	function safeMintAirdrop(uint minttimes, address toAddress) external payable onlyOwner {
	...
	function safeMintReserved(uint minttimes) external payable {
		
		require(!paused);
		
		require(minttimes <= MAXNFTperTX,"Max 10 NFTs per tx!");
		require(_tokenidreserved + minttimes < 8333,"Cannot go above NFT 5500");
				...
				_tokenidreserved = _tokenidreserved + minttimes;
