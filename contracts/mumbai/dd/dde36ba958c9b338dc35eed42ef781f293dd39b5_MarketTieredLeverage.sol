...
contract OracleManagerFixedEpoch {
  // Global state.
  AggregatorProxy public immutable chainlinkOracle;
  ...
  constructor(
    ...
    chainlinkOracle = AggregatorProxy(_chainlinkOracle);
    oracleDecimals = AggregatorV3Interface(AggregatorProxy(chainlinkOracle).aggregator()).decimals();
  ...
  {
    (, , uint256 startedAtPrev, , ) = AggregatorV3Interface(
      AggregatorProxy(chainlinkOracle).aggregator()
    ).getRoundData(oracleUpdateIndex - 1);
    (, int256 oraclePrice, uint256 timstampPriceUpdated, , ) = AggregatorV3Interface(
      AggregatorProxy(chainlinkOracle).aggregator()
    ).getRoundData(oracleUpdateIndex);
  ...
  {
    return AggregatorV3Interface(AggregatorProxy(chainlinkOracle).aggregator()).latestRoundData();
  }
  ...
  function shouldUpdatePrice(uint32 epochTimestamp) public view returns (bool) {
    (, , uint256 timestampPriceUpdated, , ) = AggregatorV3Interface(
      AggregatorProxy(chainlinkOracle).aggregator()
    ).latestRoundData();
  ...
  function priceAtIndex(uint80 index) public view returns (int256 price) {
    (, int256 oraclePricePrev, , , ) = AggregatorV3Interface(
      AggregatorProxy(chainlinkOracle).aggregator()
    ).getRoundData(index);
    ...
    )
  {
    ...
    ) = AggregatorV3Interface(AggregatorProxy(chainlinkOracle).aggregator()).latestRoundData();
    currentEpochStartTime = getEpochStartTimestamp();
    ...
    if (timstampPriceUpdated < currentEpochStartTime + MINIMUM_EXECUTION_WAIT_THRESHOLD) {
      ...
      while (true) {
        ...
        ) = AggregatorV3Interface(AggregatorProxy(chainlinkOracle).aggregator()).getRoundData(
            oracleUpdateIndex
          );
    ...
    returns (uint256)
  {
    ...
    (, , uint256 _timstampPriceUpdated, , ) = AggregatorV3Interface(
      AggregatorProxy(chainlinkOracle).aggregator()
    ).latestRoundData();
  ...
  function getMissedEpochPriceUpdates(
    ...
    (uint80 latestRoundId, , , , ) = AggregatorV3Interface(
      AggregatorProxy(chainlinkOracle).aggregator()
    ).latestRoundData(); // TODO: this function call should be integrated into the oracle more efficiently.
    ...
    (, , uint256 _previousOracleUpdateTimestamp, , ) = AggregatorV3Interface(
      AggregatorProxy(chainlinkOracle).aggregator()
    ).getRoundData(_currentOracleIndex - 1);
    ...
    while (_isSearchingForMissedEpochExecutionInfo && latestRoundId >= _currentOracleIndex) {
      (
        ...
        uint256 _currentOracleUpdateTimestamp,
        ,
