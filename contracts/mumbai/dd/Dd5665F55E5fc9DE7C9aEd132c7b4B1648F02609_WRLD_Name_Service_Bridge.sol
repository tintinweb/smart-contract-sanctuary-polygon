...
// File: @maticnetwork/fx-portal/contracts/tunnel/FxBaseChildTunnel.sol
...
// File: contracts/WRLD_Name_Service_Bridge.sol
...
contract WRLD_Name_Service_Bridge is Ownable, FxBaseChildTunnel, IWRLD_Name_Service_Bridge {
  ...
  function _processMessageFromRoot(uint256 , address sender, bytes memory data) internal override validateSender(sender) {
    ...
    // uint256 tokenId = uint256(abi.decode(data[0:31], (bytes32)));
    // uint96 expiresAt = uint96(uint256(abi.decode(data[32:43], (bytes32))));
    // address registerer = address(uint160(uint256(abi.decode(data[44:63], (bytes32)))));
