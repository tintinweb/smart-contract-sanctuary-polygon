...
contract ArtNftERC1155 is ERC2981, AccessControl, ERC1155Supply {
    ...
    mapping(uint256 => Batch) public tokenBatch;
    ...
    event BatchCreated(address indexed creator, Batch batch);
    ...
    function setMaxRoyalty(uint256 _maxRoyaltyBps) public onlyRole(DEFAULT_ADMIN_ROLE) {
        maxRoyaltyBps = _maxRoyaltyBps;
    }

    function setMaxBatchSize(uint256 _maxBatchSize) public onlyRole(DEFAULT_ADMIN_ROLE) {
        maxBatchSize = _maxBatchSize;
    }
    ...
    function lazyMintBatch(
        ...
        Batch memory batch = Batch(_currentBatchId, _cid, _name, creator, _currentTokenId, _totalSupplies);

        for (uint256 i = 0; i < batchSize; i++) {
            uint256 tokenId = _currentTokenId + i;
            tokenBatch[tokenId] = batch;
            _setTokenRoyalty(tokenId, _receiver, _royaltyFeeBps);
        }

        _currentBatchId += 1;
        _currentTokenId += batchSize;

        emit BatchCreated(creator, batch);
    }
    ...
    function creatorOf(uint256 _tokenId) public view virtual returns (address) {
        address creator = tokenBatch[_tokenId].creator;
    ...
    function setTokenRoyaltyInfo(
        ...
        address _receiver,
        uint96 _royaltyFeeBps
    ) external onlyTokenCreator(_tokenId) {
        _setTokenRoyalty(_tokenId, _receiver, _royaltyFeeBps);
    }

    function setRoyaltyInfo(address _receiver, uint96 _royaltyFeeBps) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _setDefaultRoyalty(_receiver, _royaltyFeeBps);
    }
