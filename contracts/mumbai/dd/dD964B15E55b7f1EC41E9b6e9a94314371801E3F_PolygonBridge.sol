...
contract PolygonBridge is Ownable, ERC1155Receiver {
    ...
    ERC1155Tradable private wrappedNftContract;
    ...
    event TokensMigrated(address indexed requester, bytes32 indexed sideDepositHash, uint256 amount, uint256 timestamp);
    ...
    function initializeBridge (IERC20Minter _tokenContract, ERC1155Tradable _wrappedNftContract) onlyOwner external {       
    ...
    function migrateTokens (address _requester, uint256 _bridgedAmount, bytes32 _sideDepositHash) verifyInitialization onlyGateway external {
        tokenContract.mint(address(this), _bridgedAmount);
        emit TokensMigrated(_requester, _sideDepositHash, _bridgedAmount, block.timestamp);
    }
    ...
    function setNftContract(ERC1155Tradable _wrappedNftContract) public onlyOwner {
        wrappedNftContract =_wrappedNftContract;
    }
...
//        isSwapping = false;
    ...
    event MintTokens(address user, uint256 amount);
    function _mint(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");
    ...
    event BurnTokens(address user, uint256 amount);
    function _burn(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: burn from the zero address");
