...
contract RagersCityMetadata is IRagersCityMetadata, UnrevealedURI, Ownable {
    ...
    constructor(string memory _unrevealedMetadata) {
        setUriPrefix(_unrevealedMetadata);
    }
    ...
    function setUriPrefix(string memory _uriPrefix) public onlyOwner isUnlocked {
        uriPrefix = _uriPrefix;
    }

    function setEncryptedPrefix(bytes memory _encryptedPrefix) external onlyOwner isUnlocked {
        _setEncryptedBaseURI(_encryptedPrefix);
    }

    function setUriSuffix(string memory _uriSuffix) external onlyOwner isUnlocked {
...
abstract contract UnrevealedURI is IUnrevealedURI {
    bytes public encryptedBaseURI;

    // Sets the encrypted baseURI
    function _setEncryptedBaseURI(bytes memory _encryptedBaseURI) internal {
        encryptedBaseURI = _encryptedBaseURI;
    }

    /// Get the decrypted revealed URI
    function getRevealURI(bytes calldata _key) public view returns (string memory revealedURI) {
        bytes memory encryptedURI = encryptedBaseURI;
    ...
    function isEncryptedURI() public view returns (bool) {
        return encryptedBaseURI.length != 0;
    }
}
...
interface IUnrevealedURI {
    ...
    /**
     *  @notice Reveals an unrevealed URI.
     *   
     *  @param key        The key with which the base URI was encrypted.
     */
     ...
     *
     *  @param key  The key used to encrypt/decrypt the data.
     */
