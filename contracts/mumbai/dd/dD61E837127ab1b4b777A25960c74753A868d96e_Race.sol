...
contract Race {
    ...
    event CreateGame(address gameAddre, string title, address bundleAddre);
    ...
    event CloseGame(address gameAddr);

    event PublishGame(address gameAddr, address nftContract);

    event RegisterServer(address serverAddre, string endpoint);

    event ServeGame(address serverAddre, address gameAddre);
    ...
    function createGameState(
        ...
        require(!gameState.isInitialized, "already initialized");
    ...
    modifier checkGame(address _gameAddr) {
        GameState storage gameState = games[_gameAddr];
        require(gameState.isInitialized, "uninitialized");
        _;
    }
    ...
    modifier checkGameOwner(address _gameAddr) {
        GameState storage gameState = games[_gameAddr];
        require(gameState.isInitialized, "uninitialized");
        require(msg.sender == gameState.owner, "invalid game owner");
        _;
    }
    ...
    function publishGame(
        ...
        string memory _symbol,
        string memory _url
    ) external checkGameOwner(_gameAddr) returns (address) {
        require(gameNFTContracts[_gameAddr] == address(0), "already publish");

        NFTTemplate nft = new NFTTemplate(_name, _symbol, _url);
        address nftAddr = address(nft);
        gameNFTContracts[_gameAddr] = nftAddr;

        emit PublishGame(_gameAddr, nftAddr);
        return nftAddr;
    }
    ...
    modifier checkServer(address serverAddr) {
        ...
        require(state.isInitialized, "Server Uninitialized");
        _;
    }
    ...
    function _registerServer(
        ...
        require(!state.isInitialized, "already initialized");
