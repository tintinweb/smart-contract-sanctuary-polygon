...
contract StellarToken is IERC20 {
    ...
    function addLiquidity(uint256 amountStellarToken, uint256 amountMaticDesired) external onlyDev {
        ...
        uint256 remainingTokenStellar = IERC20(address(this)).balanceOf(address(this));
        uint256 remainingMATIC_TOKEN = IERC20(MATIC_TOKEN).balanceOf(address(this));

        if (remainingTokenStellar > 0) {
            IERC20(address(this)).transfer(msg.sender, remainingTokenStellar);
        }
        if (remainingMATIC_TOKEN > 0) {
            IERC20(MATIC_TOKEN).transfer(msg.sender, remainingMATIC_TOKEN);
        }
    }
