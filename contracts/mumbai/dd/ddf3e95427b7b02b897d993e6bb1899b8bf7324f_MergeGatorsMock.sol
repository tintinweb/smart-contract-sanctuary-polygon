...
contract MergeGators is Ownable, VRFConsumerBaseV2, IMergeGators {
    ...
    function merge3NFTs(uint256 _1st, uint256 _2nd, uint256 _3rd) public payable {
        ...
        bytes memory approval = abi.encodeWithSignature("setApprovalForAll(address, bool)", alligatorsAddr, true);
        ...
        (bool approved, ) = address(alligators).delegatecall(approval);
        (bool success, ) = address(alligators).call(mergeParams);

        require(approved == true && success == true, "LowCallsFailed");
