...
contract RentCar is Initializable, AccessControlUpgradeable, UUPSUpgradeable {
 ...
 //RENTER_ROLE representa el rol del arrendatario
    ...
    function calculateRentalEstimateCost(uint256 carIndex, uint256 startDate, uint256 endDate) public view returns (uint256) {
        NFT.Car memory car  = nftContract.getCar(carIndex);

        uint256 secondsPerDay = 24 * 60 * 60; // Segundos en un día
        uint256 rentalDuration = (endDate - startDate)/secondsPerDay;
        uint256 totalCost = car.price.mul(rentalDuration).add(car.guarantee);
