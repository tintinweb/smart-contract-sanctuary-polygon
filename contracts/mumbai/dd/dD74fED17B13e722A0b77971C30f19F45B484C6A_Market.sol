...
import "../interfaces/IVoucherMarketBase.sol";
    ...
    AccessControl
{
    ...
    string private constant SIGNING_DOMAIN_NAME =
        "BlockVareNFT LazyNFT-Voucher";
    string private constant SIGNING_DOMAIN_VERSION = "1";

    string public constant name = "BlockVare Lazy-Mint Market";
    ...
    // /// @dev unique voucher index to Bid

    mapping(uint256 => VoucherBids) public voucherIdToBids;
    ...
    /// @dev no sooner than does an nft voucher is created and only if listed for bid, this fuction is invoked with BID END TIME.

    function voucherCreateERCMarket(
        ...
        whenNotPaused
    {
        require(msg.sender == _voucher.creator, "Market: only voucher creator");

        uint256 _tokenId = _voucher.tokenId;
        require(!voucherIdToBids[_tokenId].listed, "Market: already listed!");
        require(msg.value >= listingFee, "listing fee require!");
        require(
            _bidEnds * 1 seconds >= block.timestamp + minBidDays,
            "Market: bid period must be >= 3 days!"
        ); // can be set to (3days * 24hrs * 3600)seconds

        totalmarketFeeInEth += msg.value;

        voucherIdToBids[_tokenId].endsAt = _bidEnds;
        voucherIdToBids[_tokenId].listed = true;
        voucherIdToBids[_tokenId].paymentToken = _erc20Token;
        drawableSeller[_voucher.creator].isSeller = true;
        /**======================================================== */

        emit VoucherERCMarketCreated(
            _voucher.creator,
            _erc20Token,
            _voucher.minPrice,
            _bidEnds
    ...
    function voucherCreateEthMarket(
        NFTVoucher calldata _voucher,
        uint256 _bidEndTime
    ) external payable whenNotPaused {
        VoucherBids memory vBids = voucherIdToBids[_voucher.tokenId];

        require(!vBids.listed, "Market: already listed!");
        require(msg.sender == _voucher.creator, "Market: only voucher creator");
        require(msg.value >= listingFee, "listing fee required!");
        require(
            _bidEndTime * 1 seconds >= block.timestamp + minBidDays,
            "Market: bid period must be >= 3 days!"
        ); // can be set to (2days * 24hrs * 3600)seconds

        totalmarketFeeInEth += msg.value;
        vBids.endsAt = _bidEndTime;
        vBids.listed = true;
        // drawableSeller[_voucher.creator][_voucher.tokenId].isSeller = true;

        emit VoucherETHMarketCreated(
            _voucher.creator,
            _voucher.minPrice,
            _bidEndTime
        );

        voucherIdToBids[_voucher.tokenId] = vBids;
    }
    ...
    function buyNFTVoucherNowWithEth(
        ...
        uint256 _tokenId = voucher.tokenId;

        require(voucherIdToBids[_tokenId].listed, "Market: not for sale!");
        // to ensure value sent is ether higher than max bid and min price set by seller/creator.
        require(
            msg.value > voucherIdToBids[_tokenId].highestBid &&
                msg.value > voucher.minPrice,
            "Market: send more eth than highestBid"
        );

        // // make sure signature is valid and get the address of the signer
        // address signer = _verify(voucher);
        address signer = voucher.creator;

        // // owner can not bid on it own listed nft.

        require(
            msg.sender != signer && signer != redeemer,
            "Market: Seller/Creator not allowed!"
        );

        drawableSeller[signer].eth += msg.value;

        uint256 tokenIdAfterMint = nftContract.redeem(
            redeemer,
            signer,
            voucher
        );
    ...
    function buyNFTVoucherNowWithERC(
        address redeemer,
        NFTVoucher calldata voucher,
        uint256 ercValue
    ) public payable whenNotPaused returns (uint256) {
        ...
        // to ensure value sent is ether higher than max bid and min price set by seller/creator.
        require(
            ercValue > voucherIdToBids[_tokenId].highestBid &&
                ercValue > voucher.minPrice,
            "Market: send more ERC!"
        );
        ...
        drawableSeller[signer].erc += ercValue;
    ...
    function makeAbidOnNFTVoucherWithETH(NFTVoucher calldata voucher)
        public
        payable
        whenNotPaused
    {
        ...
        //     require(
        //         msg.value > voucherIdToBids[voucher.tokenId].highestBid,
        //         "Market: Bid Must be > the last bid"
        //     );
        //     address signer = _verify(voucher);

        //     require(msg.sender != signer, "creator not allowed!");

        //     emit BidMade(msg.sender, msg.value, voucher.tokenId);

        //     voucherIdToBids[voucher.tokenId].highestBid = msg.value;
        //     voucherIdToBids[voucher.tokenId].bidders.push(msg.sender);
        ...
        VoucherBids memory vBids = voucherIdToBids[voucher.tokenId];

        require(block.timestamp < vBids.endsAt, "Market: already ended!");
        ...
        if (
            drawablesBidder[msg.sender][address(0)][voucher.tokenId]
                .isBidder = true
        ) {
            uint256 previousBidAmount = isBidderOf[msg.sender][voucher.tokenId];

            uint256 differenceBidAmount = vBids.highestBid - previousBidAmount;
            ...
            vBids.started = true;
            vBids.highestBid = _newHeighestBid;
            vBids.highestBidder = msg.sender;

            /** @dev updating the values of vBids to storage VocherBids */
            voucherIdToBids[voucher.tokenId] = vBids;

            // drawablesBidder[msg.sender][voucher.tokenId].isBidder = true;
            drawablesBidder[msg.sender][
                0x0000000000000000000000000000000000000000
            ][voucher.tokenId].eth = _newHeighestBid;

            drawablesBidder[msg.sender][
                0x0000000000000000000000000000000000000000
            ][voucher.tokenId].isBidder = true;

            isBidderOf[msg.sender][voucher.tokenId] += _newHeighestBid;
        } else {
            require(
                msg.value > voucher.minPrice,
                "Market: Bid should be > minPrice"
            ...
            require(
                msg.value > vBids.highestBid,
                "Market: Bid Must be > the last bid"
            );

            vBids.started = true;
            vBids.highestBid = msg.value;
            vBids.highestBidder = msg.sender;
    ...
    function makeAbidOnNFTVoucherWithERC20(
        ...
        emit BidMade(msg.sender, _erc20Bid, _tokenId);

        VoucherBids memory vBids = voucherIdToBids[_tokenId];

        address _paymentToken = vBids.paymentToken;
        ...
        if (drawablesBidder[msg.sender][_paymentToken][_tokenId].isBidder) {
            ...
            uint256 _differenceBidAmount = vBids.highestBid -
                _previousBidAmount;

            /// below line of code assures a repeat bidder sending the right difference amount

            require(
                _erc20Bid > _differenceBidAmount,
                "Market: must be > the last bid!"
            );

            uint256 _newHeighestBid = _previousBidAmount + _erc20Bid;
            ...
            // drawablesBidder[msg.sender][voucher.tokenId].isBidder = true;
            drawablesBidder[msg.sender][_paymentToken][voucher.tokenId]
                .erc = _newHeighestBid;
            ...
            require(
                _erc20Bid > voucherIdToBids[_tokenId].highestBid &&
                    _erc20Bid > voucher.minPrice,
                "Market: Bid more ERC20"
            );
            vBids.started = true;
            vBids.highestBid = _erc20Bid;
            vBids.highestBidder = msg.sender;

            // for authenticating erc20 withdrawer and revert if not legit withdrawer
            // drawablesBidder[msg.sender][_tokenId].isBidder = true;
            drawablesBidder[msg.sender][_paymentToken][_tokenId]
                .erc = _erc20Bid;
            drawablesBidder[msg.sender][_paymentToken][_tokenId]
                .isBidder = true;
            pendingERC20Withdrawal[msg.sender][_tokenId] += _erc20Bid;

            console.log(
                "drawablesBidder ERC balance is %s",
                drawablesBidder[msg.sender][_paymentToken][_tokenId].erc
            );

            /// @dev updating the new vBids value to storage

            voucherIdToBids[_tokenId] = vBids;
        }

        /// @dev transfer of ERC20 token to market for custody
    ...
    function acceptBidNFTVoucher(address redeemer, NFTVoucher calldata voucher)
        public
        payable
        whenNotPaused
        nonReentrant
    ...
    {
        require(
            voucherIdToBids[voucher.tokenId].started,
            "Market: no bids yet!"
        );
        require(
            block.timestamp > voucherIdToBids[voucher.tokenId].endsAt,
            "Market: after bid end time!"
        );

        //make sure signature is valid and get the address of the signer
        address signer = _verify(voucher);
        console.log("Signer from Market:", signer);
        // require(
        //     hasRole(MINTER_ROLE, signer),
        //     "Market: Signature invalid or unauthorized"
        // );

        require(
            msg.sender == voucherIdToBids[voucher.tokenId].highestBidder ||
                msg.sender == voucher.creator,
            "Market: only bid winner or creator!"
        );
        require(redeemer != signer, "Market: creator can't be redeemer/buyer!");

        uint256 bidAmount = voucherIdToBids[voucher.tokenId].highestBid;
        address highestBidder = voucherIdToBids[voucher.tokenId].highestBidder;

        //saving value to be withdrawn later by the nft creator/signer
        drawableSeller[signer].eth += bidAmount;

        /** @dev deleting DrawableBidder struct info as bid has been accepted from the bidder */

        delete drawablesBidder[highestBidder][address(0)][voucher.tokenId];

        delete voucherIdToBids[voucher.tokenId];

        uint256 _tokenId = nftContract.redeem(redeemer, signer, voucher);

        // CALLING THE REDEEM FUNCTION OF BLOCKVARE NFT CONTRACT
    ...
    function acceptERC20BidNFTVoucher(
        address redeemer,
        NFTVoucher calldata voucher
    ) external returns (uint256) {
        uint256 _voucherId = voucher.tokenId;
        require(voucherIdToBids[_voucherId].started, "Market: no bids yet!");

        if (msg.sender == voucherIdToBids[_voucherId].highestBidder) {
            require(
                block.timestamp > voucherIdToBids[_voucherId].endsAt,
                "Market: after bid end time!"
            );
        }
        ...
        require(
            ...
            "Market: signer and creator must be same!"
        );
        require(redeemer != signer, "Market: creator can't be buyer!");
        // require(msg.sender == redeemer, "Market: caller must be the redeemer!");

        require(
            msg.sender == voucherIdToBids[_voucherId].highestBidder ||
                msg.sender == signer,
            "Market: only bid winner or signer!"
        );

        /** @dev copying voucher index to VoucherBids data and  then deleting the storage for the same immediately after it */
        /** @dev because bid has been concluded here and accounts settled */
        VoucherBids memory vBids = voucherIdToBids[_voucherId];

        delete voucherIdToBids[_voucherId];

        /// @dev calling the redeem function of BlockVare nft contract to redeem/mint

        uint256 _tokenId = nftContract.redeem(redeemer, signer, voucher);

        uint256 _highestBid = vBids.highestBid;
        address highestBidder = vBids.highestBidder;
        address _paymentToken = vBids.paymentToken;

        require(
            redeemer == highestBidder,
            "Market: redeemer must be highest bidder!"
        );
        ...
        drawableSeller[signer].erc += _highestBid;
        pendingERC20Withdrawal[signer][_voucherId] += _highestBid;

        console.log("drawableSeller contract %s", _highestBid);

        delete pendingERC20Withdrawal[highestBidder][_voucherId];
        delete drawablesBidder[highestBidder][_paymentToken][_voucherId];

        // console.log("seller erc balance is %s", drawableSeller[signer].erc);
    ...
    function revokeVoucherListing(NFTVoucher calldata _voucher) public {
        uint256 _bidEndsAt = voucherIdToBids[_voucher.tokenId].endsAt;
        console.log("block_time %s", block.timestamp);
        console.log("end time %s", _bidEndsAt);
        console.log("end time - 43200 %s", (_bidEndsAt - 43200));
        require(
            block.timestamp < (_bidEndsAt * 1 seconds) - 43200 seconds, //1668 594600//(_bidEndsAt - 43200), /// 12hrs * 60 mins * 60 secs 43200
            "Market: allowed only 12hrs before bid ends!"
        );
        require(
            msg.sender == _voucher.creator,
            "Market: only voucher creator!"
        );
        delete voucherIdToBids[_voucher.tokenId];

        emit VoucherUnlisted(_voucher.tokenId);
    }
    ...
    /** Functions to withdraw Ether or ERC20 to cretor/seller account */
        ...
        nonReentrant
    {
        ...
        if (CheckIf.isBidder(drawablesBidder, address(0), _tokenId)) {
            require(
                msg.sender != voucherIdToBids[_tokenId].highestBidder,
            ...
            amount = drawablesBidder[withdrawer][
                0x0000000000000000000000000000000000000000
            ][_tokenId].eth;

            delete drawablesBidder[withdrawer][
                0x0000000000000000000000000000000000000000
            ][_tokenId].eth;

            require(amount > 0, "Bidder: Zero Eth Bal.");

            /// deleting the stored balance from the struct after complete withdrawal
            delete isBidderOf[msg.sender][_tokenId];
        } else {
            amount = drawableSeller[withdrawer].eth;

            require(amount > 0, "Zero Eth Bal.");

            // zero account before transfer to prevent re-entrancy attack
            drawableSeller[withdrawer].eth -= amount;
        }

        /// @dev Emitting Etherwithdrawal event before state change to  Ether balance of caller
        emit Etherwithdrawal(withdrawer, amount);

        // USING LIBTRANSFER TO  ACCOMPLISH THE BELOW ETH TRANSFER

        bool sent = LibMarket.transferEth(withdrawer, amount);
    ...
    function availableToWithdrawBidder(
        ...
        uint256 _voucherId
    ) public view returns (uint256 ethAmount, uint256 erc20Amount) {
        // address _paymentToken = voucherIdToBids[_voucherId].paymentToken;

        /** @dev to fetch total Eth amount */

        ethAmount = drawablesBidder[msg.sender][
            0x0000000000000000000000000000000000000000
        ][_voucherId].eth;
        ...
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "NFTVoucher(address creator,uint256 tokenId,uint256 minPrice,uint256 royaltyBPS,string uri)"
                        ),
                        voucher.creator,
                        voucher.tokenId,
                        voucher.minPrice,
...
import {NFTVoucher} from "./ILazyNFT.sol";
import {ILazyMintMarket} from "../interfaces/ILazyMintMarket.sol";
...
interface IVoucherMarketBase is ILazyMintMarket {
    function voucherCreateERCMarket(
        NFTVoucher calldata _voucher,
        address _erc20Token,
        uint256 _bidEnds
    ...
    function revokeVoucherListing(NFTVoucher calldata _voucher) external;

    function setListingFee(uint96 _feePercent) external;

    function withdrawEther(uint256 _tokenId) external;

    function withdrawERC20(address _ERC20token, uint256 _tokenId) external;
    ...
    function availableToWithdrawBidder(address _paymentToken, uint256 _voucherId)
        external
        view
