...
contract MATICDAPP {
    ...
    function addDownlineRef(address senderAddress, address refUserAddress, uint refLevel) internal {
	 ...
	 function distributeRef(address _referredBy,address _sender, bool _newReferral) internal {
		...
		chkLv1                          = referralLevel1Address[_referredBy];
        chkLv2                          = referralLevel2Address[_referredBy];
        chkLv3                          = referralLevel3Address[_referredBy];
        chkLv4                          = referralLevel4Address[_referredBy];
        chkLv5                          = referralLevel5Address[_referredBy];
        chkLv6                          = referralLevel6Address[_referredBy];
        chkLv7                          = referralLevel7Address[_referredBy];
        chkLv8                          = referralLevel8Address[_referredBy];
        chkLv9                          = referralLevel9Address[_referredBy];
        chkLv10                         = referralLevel10Address[_referredBy];
		
 
		
		
		        // Level 1
        if(chkLv1 != 0x0000000000000000000000000000000000000000) {
            referralLevel2Address[_customerAddress]                     = referralLevel1Address[_referredBy];
            if(_newReferral == true) {
                addDownlineRef(referralLevel1Address[_referredBy], _customerAddress, 1);
            }
        }
  ...
  function toggleStarted() public {
    require(msg.sender == commissionWallet, "Caller Is Not the Owner");
    started = !started;
  }
