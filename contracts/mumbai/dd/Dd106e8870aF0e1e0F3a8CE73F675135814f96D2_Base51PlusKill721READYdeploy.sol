...
// File: contracts/721_Base/Base721 PlusKill 50.sol
/*
...
contract Base51PlusKill721READYdeploy is ERC721, Ownable, ERC2981, DefaultOperatorFilterer, RoyaltiesV2Impl { 
    ...
    uint256 public CostForMinter = 0.0000 ether;                          
    

    constructor(string memory _name, string memory _symbol, string memory _uri, string memory _notRevealedURI, address payable _receiverRoyalty, uint96 _royaltyFeesInBips, address payable _AddressWithdrawBalanceOne,  uint256 _FaktorWithdrawBalanceOne, address payable _AddressWithdrawBalanceTwo, uint256 _FaktorWithdrawBalanceTwo, uint256 _MaxTokenInCollection) ERC721(_name, _symbol) {     
        ...
        require(_FaktorWithdrawBalanceOne + _FaktorWithdrawBalanceTwo == FakCheck, "constructor - missconfiguration of both FaktorWithdrawBalance"); 
        baseURI = _uri; 
        setRevealed(true);
    ...
    function _requireMinted(uint256 tokenId) internal view virtual override {  
        require(_exists(tokenId), "_requireMinted - the token ID is NOT mined now OR the token was previously burned");
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {   
        ...
        return bytes(baseURI).length > 0 ? string.concat(currentBaseURI, tokenId.toString(), ".json" ): "";  
    }
    ...
    function mintAllAtOnce() public onlyOwner  { 
    ...
    function mintArea(uint256 _areaStart, uint256 _areaEnd) public onlyOwner  { 
        require(!paused, "The contract is paused.");
    ...
    function mintSingle(uint256 _tokenId) public onlyOwner {
        require(!paused, "The contract is paused.");

        require(_tokenId <= maxTokenInCollection, "The areaEnd value can not be bigger then the max. token in collection. Decrease the value areaEnd and try again");
        require(exist[_tokenId]  == false, "You cannot mint a token that already exists.");

        if(exist[_tokenId]  == false) {
    ...
    function walletOfOwner(address _owner) public view onlyOwner returns (uint256[] memory) {
        ...
        while (ownedTokenIndex < ownerTokenCount && currentTokenId <= maxTokenInCollection) {
    ...
    function setRoyaltyRariableMintable(uint _tokenId, address payable _receiverRoyalty, uint96 _royaltyFeesInBips) public onlyOwner {  
        LibPart.Part[] memory _royalties = new LibPart.Part[](1);
    ...
    function _royaltyMintable(uint256 _tokenId, uint256 _salePrice) public view onlyOwner returns(address receiver, uint256 royaltyAmount) {   
    ...
    function withdrawAll() public payable onlyOwner {  
        ...
        require(balance > 0, "withdrawAll - no balance left to withdraw");
        ...
        require(transferOne && transferTwo, "withdrawAll - transfer failed.");
    }


    function burnSingle(uint256 tokenId) public onlyOwner { 
    ...
    function burnAreaTokenAtOnce(uint256 areaStart, uint256 areaEnd) public onlyOwner {
        require(!paused, "The contract is paused!");
        
        for(uint256 i= areaStart; i<= areaEnd; i++){
            uint256 startTokenId = 0;
            require(areaEnd <= maxTokenInCollection, "The areaEnd value can not be bigger then the max. token in collection. Decrease the value areaEnd and try again");
    ...
    function burnSingleForMint(uint256 burnTokenId, uint256 mintTokenId) public onlyOwner { 
    ...
    function killThisContract() public payable onlyOwner { 
        uint256 balance = address(this).balance;
        require(balance <= maxKillBalance, "killThisContract - contract cannot kill. You have too much money in the contract. Pay out first or increase the maxKillBalance.");
