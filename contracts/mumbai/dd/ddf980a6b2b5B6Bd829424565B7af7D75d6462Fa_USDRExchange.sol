...
contract USDRExchange is AddressAccessor, IExchange, Pausable {
        ...
        returns (uint256 multiplier)
    {
        assembly {
            if iszero(iszero(usdrMarketCapE18)) {
                let threshold := div(
                    mul(usdrMarketCapE18, lowerTNGBLMintingThreshold.slot),
                    100
                )
                if gt(stableBacking, threshold) {
                    multiplier := sload(tngblMultiplier.slot)
                }
            }
        }
    }
    ...
    function afterBurnTNGBL(uint256 amount) external {
        ...
        if (amount == 0) {
            _tngblMintingStats.usdrMinted = 0;
            _tngblMintingStats.tngblValue = 0;
        } else {
            _updateTNGBLMintingStats(-int256(amount), 0);
        }
    }
