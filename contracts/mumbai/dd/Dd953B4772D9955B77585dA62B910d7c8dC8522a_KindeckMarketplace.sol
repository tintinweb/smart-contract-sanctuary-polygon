    ...
    Ownable
{
    ...
    mapping(uint256 => address) internal productManagers; //productId => contract address (this if not created buy another profile)
    ...
    // address private immutable _treasuryBackup;
     ...
     * @dev _manager is the sellers proxy contract that should receive funds. Should we fetch from Inception instead and only use msg.sender here?
     */
    ...
    function mintProduct(
        ...
        if (_receiver == address(this)) {
            address _parent = kindeckInception.getParent(msg.sender);
            uint256 _kickback = _price / 10;
            uint256 _rest = _price - _kickback;
            send(_parent, _kickback, _userData);
            send(_receiver, _rest, _userData);
        } else {
    ...
    function tokensReceived(
        ...
        mintProduct(1, from, userData);
    }

    function getBytes(string memory input) external pure returns (bytes32) {
