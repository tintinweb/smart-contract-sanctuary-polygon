...
contract AdventurePass is
    BaseContractV2,
    ERC721EnumerableUpgradeable,
    IAdventurePass
{
    uint256 public passIdTracker;
    string public baseURI;

    function initialize(string memory baseURI_) public initializer {
        __BaseContract_init();
        __ERC721_init_unchained('Otto Adventure Pass', 'OAP');
    ...
    function issuePass(address to_) external onlyManager returns (uint256) {
        uint256 id = passIdTracker;
        passIdTracker++;
        _safeMint(to_, id, '');
        return id;
    }

    function revokePass(uint256 id) external onlyManager {
        _burn(id);
    }
    ...
    {
        return
            interfaceId == type(IAdventurePass).interfaceId ||
