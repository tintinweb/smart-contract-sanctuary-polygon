...
contract VeArtProxyUpgradeable is IVeArtProxy, Initializable {
    ...
    function _tokenURI(uint _tokenId, uint _balanceOf, uint _locked_end, uint _value) external pure returns (string memory output) {
        ...
        output = string(abi.encodePacked(output, "balanceOf ", formatUintToString(_balanceOf, 18), '</text><text x="10" y="60" class="base">'));
        output = string(abi.encodePacked(output, "locked_end ", toString(_locked_end), '</text><text x="10" y="80" class="base">'));
        output = string(abi.encodePacked(output, "value ", formatUintToString(_value, 18), '</text></svg>'));
    ...
    function toString(uint256 value) private pure returns (string memory) {
        ...
        if (digits <= 32) {
            ...
            while (value != 0) {
                unchecked {
                    digits -= 1;
                    buffer |= bytes32(uint(uint8(48 + uint256(value % 10))) << (248 - index));
                    value /= 10;
                    index += 8;
                }
            }
    ...
    function substring(bytes memory strBytes, uint startIndex, uint endIndex) private pure returns (string memory) {
