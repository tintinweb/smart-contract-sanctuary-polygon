...
import {AbstractBridgeAdapter} from '../abstracts/AbstractBridgeAdapter.sol';
import {Router} from '@abacus-network/app/contracts/Router.sol';
import {IBridge} from '../interfaces/IBridge.sol';
import {IBridgeReceiver} from '../interfaces/IBridgeReceiver.sol';

/// @author FlowCarbon LLC
/// @title Abacus Bridge Adapter Contract
contract AbacusBridgeAdapter is AbstractBridgeAdapter, Router, IBridge {

    /// Adapter name and version
    bytes32 constant public IDENTIFIER = keccak256('ABACUS_v1.0.0');

    /// Maps chain IDs to abacus domain identifiers
    mapping(uint32 => uint32) public chainToDomain;
    /// Maps abacus domain identifiers to chain IDs
    ...
    function _handle(uint32 source_, bytes32, bytes memory message_) internal virtual override {
        ...
        require(chainId != 0, 'AbacusBridgeAdapter: unknown source domain');
        receiver.receiveMessage(uint256(chainId), message_);
    }
    ...
    /// @dev See IBridgeInterface
    function registerRemoteContract(uint256 destination_, address contract_) external onlyReceiver {
        uint32 domain = chainToDomain[uint32(destination_)];
        require(domain != 0, 'AbacusBridgeAdapter: unknown destination chain');
...
/// @author FlowCarbon LLC
/// @title Bridge Adapter Base-Contract
abstract contract AbstractBridgeAdapter {

    /// Address of our local station that connects to the cross-chain network
    ...
    modifier onlyReceiver() {
        require(msg.sender == address(receiver),
            'AbstractBridgeAdapter: caller is not the bridge receiver');
        _;
    }
}
