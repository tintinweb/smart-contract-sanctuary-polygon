...
contract Tractor is ERC721{
    ...
    mapping(uint256 => string) private tokURI;

    constructor(address _controller) ERC721("Tractor", "TRC") {
        controller = IController(_controller);
    }

    function setController(address _newAddress) external {
        require(controller.isSuperAdmin(msg.sender), "Tractor: only Super admin can set new contract address.");
        controller = IController(_newAddress);
    }

    function mint(address to) external {
        require(controller.isNFTManager(msg.sender) || controller.getSellerContract() == msg.sender, "Tractor: sender must have NFTManager role.");
        _mint(to, counter++);
    }
    ...
    function burn(uint256 tokenId) external {
        require(controller.isNFTManager(msg.sender), "Tractor: sender must have NFTManager role.");
        _burn(tokenId);
    }
    ...
    function setTokenUri(uint256 tokenId, string memory _newURL) external {
        require(controller.isNFTManager(msg.sender), "");
        tokURI[tokenId] = _newURL;
    }
