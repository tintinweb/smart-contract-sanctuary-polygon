...
contract TreasuryTracker is AddressAccessor, ITreasuryTracker {
    ...
    function setCurrencyData(
        ...
        currencyInfo[currency].latestOraclePrices = cData.latestOraclePrices;
        currencyInfo[currency].currencyOracle = cData.currencyOracle;
    }

    function setRwaNativeCurrencyValue(string memory currency, uint256 rwaValue)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        currencyInfo[currency].valueInNativeCurrency = rwaValue;
    }
    ...
    function getRwaUsdValue(IERC20Metadata token)
        external
        view
        returns (uint256 usdValue, bool valueNotLatest)
    {
        ...
        if (usdValue != 0) {
            // hardcoding to 3 since all our oracles have 3 decimals in prices
            usdValue = convertPriceToUSDCustom(token, usdValue, 3);
        }
    }
...
interface ITreasuryTracker {
    ...
    function getRwaUsdValue(IERC20Metadata token)
        external
        view
        returns (uint256 usdValue, bool priceUpToDate);
}
