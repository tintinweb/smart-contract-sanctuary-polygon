...
contract Trident is ERC20, Ownable, ReentrancyGuard{    
  ...
  function _tokenAmount(uint256 amount_) private pure returns (uint256) {
     uint256 _amount;
     uint256 _toTest = 1000;
      if(amount_ * _toTest <= (1 * 10**17)){
          _amount = amount_;
      } else if((amount_ * _toTest > (1*10**17)) && (amount_ * _toTest <= (5*10**17))){
          _amount = amount_ * 2;
      } else if((amount_ * _toTest > (5*10**17)) && (amount_ * _toTest <= (1*10**18))){
          _amount = amount_ * 3;
      } else if(amount_ * _toTest > (1*10**18)){	
