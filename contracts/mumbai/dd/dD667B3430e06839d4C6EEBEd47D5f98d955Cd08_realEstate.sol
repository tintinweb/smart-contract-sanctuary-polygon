...
contract realEstate {
     ...
     struct Property{
      ...
      uint256 public reveiwsCounter;

       event ReviewAdded (uint256 indexed productId, address indexed reviewer, uint256 rating , string coment);
       event ReviewLiked (uint256 indexed productId, uint256 indexed reviewIndex, address indexed liker, uint256 likes); 

     //FUNCTION IN CONTRACT 

     function listproperty(address owner, uint256 price, string memory _property, string memory _category , string memory _images , 
     string memory _propertyTitle  ,string memory _propertyAddress,string memory _description ) external returns (uint256){
     require (price > 0, "price is must be greater then 0.");
     ...
     function updateproperty (address owner, uint256 productId, string memory _propertyAddress, string memory _category, string memory 
     _images, string memory _propertyTitle, string memory _description) external returns (uint256){
        Property storage property = properties [productId];
     ...
     function updatePrice(address owner, uint256 productId, uint256 price) external returns (string memory){
        ...
        require (property.owner == owner, " you are not a owner");

        property.price = price;

        return "your propert price is updated";
     }
     ...
     function getProperty(uint256 id ) external view returns(uint256,address,
     uint256,string memory,string memory,string memory,string memory,string memory){
     ...
     function getHighestratedProduct() external view returns(uint256){
        ...
        for (uint256 i = 0; i< reveiwsCounter; i++){
