...
contract Pay is IPay, PayConfig {
    ...
    function pay(
        ...
        (uint256 usdcFee, uint256 tokenFee) = getFee(_merchant, _orderAmount, _paiAmount);
        ...
        if (address(0) != settleToken) {
            ...
            emit Order(_orderId, _paiAmount, _currency, _orderAmount, settleToken, fee, _merchant, msg.sender);
            ...
            emit Order(_orderId, _paiAmount, _currency, _orderAmount, _currency, fee, _merchant, msg.sender);
    ...
    function payWithETH(
        ...
        (uint256 usdcFee, uint256 tokenFee) = getFee(_merchant, _orderAmount, msg.value);

        address settleToken = iMerchant.getSettleCurrency(_merchant);

        if (address(0) != settleToken) {

            if (WETH9 != settleToken) {
            ...
            emit Order(_orderId, _paiAmount, WETH9, _orderAmount, settleToken, fee, _merchant, msg.sender);
            ...
            if (iMerchant.getAutoSettle(_merchant)) {
                ...
                merchantFunds[_merchant][WETH9] += (_orderAmount - fee);

            }

            tradeFeeOf[WETH9] += fee;

            emit Order(_orderId, _paiAmount, WETH9, _orderAmount, WETH9, fee, _merchant, msg.sender);
    ...
    function getFee(address _merchant, uint256 _orderAmount, uint256 _paidAmount) internal view returns (uint256 usdcFee,uint256 tokenFee) {

        if(iMerchant.getFixedRate(_merchant)) {

            uint256 usdcRate = SafeMath.div(_orderAmount, fixedRate);

            uint256 tmp = SafeMath.mul(_paidAmount, usdcRate);

            tokenFee = SafeMath.div(tmp, 100);
