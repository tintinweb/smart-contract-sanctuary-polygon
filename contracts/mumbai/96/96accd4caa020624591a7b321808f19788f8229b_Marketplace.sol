...
contract Marketplace is Context, Ownable {
    ...
    address public admin = 0xc57a243254707D8df53733FE14f675A9F3537B49;
    constructor(){
    }
    ...
    function buy(uint256 sellerid,uint256 _amount) public payable returns(bool){
        ...
        if(details[sellerid].isnativate){
            ...
            IBEP20(details[sellerid]._taddress).transferFrom(msg.sender,address(this),t);
    ...
    function removeSell(uint256 sellerid) public returns(bool){
        require(details[sellerid].benifit != address(0x0),"is not for sell");
        require(details[sellerid].benifit == msg.sender || admin == msg.sender,"caller not the seller address");

        IBEP20(details[sellerid].FractionalAddress).transfer(details[sellerid].benifit,details[sellerid].token);

        emit Removesell(sellerid,details[sellerid].benifit,details[sellerid].token);
