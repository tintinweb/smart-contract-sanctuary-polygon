    ...
    OwnableUpgradeable
{
    ...
    function mintJoker(
        ...
        require(_isVerifiedCoupon(digest, _coupon), "wrong coupon");
        jokersCount -= _amount;
    ...
    function upgradeCardCost(
        ...
        require(_tokenIds.length > 1, "not enough c");
        ...
        if (_collectionId == collectionId) {
            ...
            upgradecost.targetCardTypeValue =
                (upgradecost.targetCardTypeValue * collectionsRatio) /
                10 ** 8;
    ...
    function upgradeCards(
        ...
        require(msg.value >= upgradecost.payValue, "not enough f");

        upgradeinfo.collectionsRatio =
            (upgradeinfo.collectionBuyBack * 10 ** 8) /
            buyBackValue;

        card.burnBatch(msg.sender, _tokenIds, 0);

        upgradeinfo.totalValue =
            (upgradecost.totalValueGivenCards * 10 ** 8) /
            upgradeinfo.collectionsRatio;
        ...
        require(
            upgradeinfo.totalValue >=
                (cardValue[_targetCardType] * upgradeinfo.collectionsRatio) /
                    10 ** 8,
            "not enough"
        );
        require(
            upgradecost.highestCardValue <= cardValue[_targetCardType],
            "wrong t card"
        );
