...
contract DigicapMarket is Manager, ERC721Holder, ReentrancyGuard {
    ...
    struct BuyerCommission {
        address token;
        address paymentToken;
        uint256 tokenId;
        uint256 amount;
        address minter;
        TaxPromoCodeInfo taxPromoCodeInfo;
    }
    ...
    function callBuyerCommission(
        BuyerCommission memory _buyerCommission,
        bytes calldata _signature
    ) public returns (uint256, uint256) {
        require(
            _buyerCommission.taxPromoCodeInfo.tax <= ZOOM_FEE,
            "Invalid tax"
        );
        if (_buyerCommission.taxPromoCodeInfo.isPercent) {
            require(
                _buyerCommission.taxPromoCodeInfo.promoCodeNft <= ZOOM_FEE,
                "Invalid promoCodeNft"
            );
            require(
                _buyerCommission.taxPromoCodeInfo.promoCodeServiceFee <=
                    ZOOM_FEE,
        ...
        uint256 _commissionFromBuyer = 0;
        ...
        if (
            verifyMessage(
                ...
                _buyerCommission.taxPromoCodeInfo,
                _signature
            )
        ) {
            if (_buyerCommission.taxPromoCodeInfo.isPercent) {
                ...
                uint256 _buyerAmount = _buyerCommission.amount -
                    (_buyerCommission.amount *
                        _buyerCommission.taxPromoCodeInfo.promoCodeNft) /
                    ZOOM_FEE;
                _commissionFromBuyer = (_buyerAmount * _commission) / ZOOM_FEE;

                amountToBuyer = _buyerAmount + _commissionFromBuyer;
            } else {
                uint256 _buyerAmount = _buyerCommission.amount >
                    _buyerCommission.taxPromoCodeInfo.promoCodeNft
                    ? _buyerCommission.amount -
                ...
                _commissionFromBuyer =
                    (_buyerAmount * _commission) /
                    ZOOM_FEE;
                _commissionFromBuyer = _commissionFromBuyer >
                    _buyerCommission.taxPromoCodeInfo.promoCodeServiceFee
                    ? _commissionFromBuyer -
                        _buyerCommission.taxPromoCodeInfo.promoCodeServiceFee
                    : 0;
            ...
            if (_buyerCommission.taxPromoCodeInfo.tax > 0) {
                amountToBuyer +=
                    (amountToBuyer * _buyerCommission.taxPromoCodeInfo.tax) /
                    ZOOM_FEE;
            }
        ...
        _commissionFromBuyer =
            ...
            ZOOM_FEE;
        amountToBuyer = _buyerCommission.amount + _commissionFromBuyer;
        return (amountToBuyer, _commissionFromBuyer);
    }
