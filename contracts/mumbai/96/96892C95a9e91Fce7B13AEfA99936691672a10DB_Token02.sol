...
contract Token02 is ERC20, AccessControl {
	...
	function rebase() public initialized {
		...
		} else if (lastRebasePrice > currentPrice && currentPrice < (lastRebasePrice * 995) / 1000) {
			uint256 currentSupply = (lastRebasePrice * _tTotal) / currentPrice;
			uint256 supplyDelta = currentSupply - _tTotal;
	...
	function _getRate() private view returns (uint256) {
		...
		return rSupply / (tSupply + _totalBurn + _totalRebase);
	}
