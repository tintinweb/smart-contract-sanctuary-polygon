...
contract BridgeSwap is Ownable {
    ...
    function updateMinmumamount(uint256 _amount) public onlyOwner {
        minmumlock = _amount;
    }
    ...
    function withdrawOwner(address _token) public onlyOwner {
        uint256 totalAmount = IERC20(_token).balanceOf(address(this));
        uint256 balanceAmount = totalAmount - minmumlock;
        require(
            balanceAmount > 0,
            "Insufficient balance after subtraction minimum amount"
        );
        IERC20(_token).approve(msg.sender, balanceAmount);
        IERC20(_token).transfer(msg.sender, balanceAmount);
    }
