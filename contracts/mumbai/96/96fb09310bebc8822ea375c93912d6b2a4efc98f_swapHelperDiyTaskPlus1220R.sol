...
contract swapHelperDiyTaskPlus1220R is OpsTaskCreator,Ownable {
    using SafeMath for uint256;
    ...
    event swapEvent(bytes32 _taskID, uint256 _timestamp, address _caller, uint256 _usdtAmount, uint256 _fee, uint256 _spendUsdtAmount, uint256 _poolFee);
       ...
       constructor(
        ...
        //    rD.delegatecall(abi.encodeWithSignature("setDefaultSwapInfo(uint256)",_approveAmount));
        //    rD.delegatecall(abi.encodeWithSignature("setUSDTPoolAddress(USDTPool)",_USDTPoolAddress));
        emit constructorE(_approveAmount,_ops, _fundsOwner, _USDTPoolAddress);
    }


    function setDefaultSwapInfo(uint256 _approveAmount) public  {
         rD.delegatecall(abi.encodeWithSignature("setDefaultSwapInfo(uint256)",_approveAmount));
        // approveAmount = _approveAmount;
    }

    function setUSDTPoolAddress(USDTPool _USDTPoolAddress) public {
         rD.delegatecall(abi.encodeWithSignature("setUSDTPoolAddress(USDTPool)",_USDTPoolAddress));
