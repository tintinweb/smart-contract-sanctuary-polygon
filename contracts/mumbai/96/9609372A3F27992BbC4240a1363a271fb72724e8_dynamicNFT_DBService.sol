    ...
    AccessControlEnumerable
{
    ...
    constructor(address admin, address[] memory contracts) {
        ...
        if (contract_admin != address(0)) {
            _grantRole(DEFAULT_ADMIN_ROLE, contract_admin);
            _grantRole(ADMIN_ROLE, contract_admin);
        }

        INFT_DB_CONTRACTS = contracts;
        for (uint256 i = 0; i < INFT_DB_CONTRACTS.length; i++) {
            _grantRole(ADMIN_ROLE, contracts[i]);
        }
    }
    ...
    function performUpkeep(bytes calldata performData) external override {
        ...
        string[] memory input = abi.decode(performData, (string[]));
    ...
    //PRIVILEGES MANAGEMENT FUNCTIONS AREA

    function updateINFT_DBPrivilegedAccounts(address[] memory contracts) public onlyRole(DEFAULT_ADMIN_ROLE) //DA RIVEDERE
    {
        require(contracts.length > 0, "INFT_DB: Array parameter length equals 0!");

        for (
            //old array cleaning process
            uint256 i = 0;
            i < INFT_DB_CONTRACTS.length;
            i++
        ) {
            (bool tx_success, ) = INFT_DB_CONTRACTS[i].call(abi.encodeWithSignature("_revokeRole(bytes32,address)", ADMIN_ROLE, INFT_DB_CONTRACTS[i]));
            require(tx_success, "INFT_DB: Error, privileged accounts upgrade transaction failed!");

            delete INFT_DB_CONTRACTS[i];
        }

        INFT_DB_CONTRACTS = contracts; //reassignment
        ...
        for (
            ...
            (bool tx_success, ) = INFT_DB_CONTRACTS[i].call(abi.encodeWithSignature("_grantRole(bytes32,address)", ADMIN_ROLE, INFT_DB_CONTRACTS[i]));
            require(tx_success, "INFT_DB: Error, privileged accounts upgrade transaction failed!");
        }
    }

    function updateCurrentPrivilegedAccounts(address[] memory contracts) //DA RIVEDERE
        public
        ...
        nonReentrant
    {
        ...
        for (
            ...
            _revokeRole(ADMIN_ROLE, INFT_DB_CONTRACTS[i]); //i ruoli devono essere assegnati?
            delete INFT_DB_CONTRACTS[i];
        }
