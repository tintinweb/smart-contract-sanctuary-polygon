...
contract BasketballHead is IERC2981, ERC165Storage, Ownable, IERC1155MetadataURI, ERC1155, MinterRole {
    ...
    */
     ...
     * @param _beneficiary address where mint to
     */
    function mint(address _beneficiary, uint256 _count) public payable {
        ...
        safeMint(_beneficiary, 1, _count);
        totalsupply+=_count;
     ...
     * @param _beneficiary address where mint to
     */
    function mintByAdmin(address _beneficiary, uint256 _count) public onlyAdmin {
        safeMint(_beneficiary, 1, _count);
        totalsupply += _count;
    }

    function setTotalSupply(uint256 _totalsupply) external onlyAdmin {
     ...
     */
    function reserve(uint256 _count) public payable {
        require(_count <= maxMintCount, "too many count for one transaction");
        require(msg.value >= (mintprice * _count), "mintprice is not enough");
        reserveCount[msg.sender] += _count;
    }
