...
// File: contracts/DaddyAvatar.sol
...
contract DaddyAvatars is Ownable, ERC1155 {
    ...
    constructor(IERC20 _token, uint256 _price)
        ERC1155(
            'https://defydefi.mypinata.cloud/ipfs/QmTmgCPP3sbzG6fadPUGhHaxJacgWq8FmTnX6pDuXbeu5V/{id}.json'
        )
    {
        setName('Daddy Profile Avatar');
        setSymbol('DPA');
        token = _token;
        price = _price;
    }
    ...
    function mintBatch(uint256[] memory ids, uint256[] memory amounts)
        external
    {
        uint256 numID = ids.length;
        
        for(uint256 i = 0; i < numID; i++){
         require(amounts[numID] >= price, "Not enough token!");
         require(ids[numID] <= 10000, "No ID value for NFT!");   
         token.safeTransferFrom(msg.sender, address(this), amounts[numID]);   
        _mintBatch(msg.sender, ids, amounts, '');
        }
    }

    function mint(uint256 id, uint256 amount) external {
        require(amount >= price, "Not enough token!");
        require(id <= 10000, "No ID value for NFT!");
