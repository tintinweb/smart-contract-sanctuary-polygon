...
contract BCChars is ERC721URIStorage, Ownable, BCUtils {
    ...
    //                                  // Or should that live in the game entity?
    ...
    // needing an oracle.  Note that players may be able to determine properties
    // without decanting
    // TODO: I don't think this actually helps.  Attacker can pick a block
    // N in the future and precalculate when to do a block
    // I should add some sort of recycling mechanism
    struct CloneTank {
        bytes32 hashBase;
    ...
    function installCloneInTank() public payable {
        ...
        cloneTanks.push(CloneTank(
            randKeccak(), // TODO: Just block.now?  Does it even add value to have more than the timestamp here?
            block.number + offset,
            false
        ));
    ...
    function decantNewClone(uint _id) public {
        require(cloneTankToOwner[_id] == msg.sender, "you don't own this tank");
        require(cloneTanks[_id].decanted == false, "clone already decanted");

        // TODO: Owner override
        require(msg.sender == owner() || cloneTanks[_id].decantBlockNumber <= block.number, "can't decant yet");

        cloneTanks[_id].decanted = true;

        bytes32 finalHash = randKeccak();

        // Consume the hash to create pseudo-random character attributes
        // +1 to cap values to accomodate normalization function and have a 1/16 changes
        ...
        // TODO: I don't think having the two step process adds entropy or security
        characters.push(Character(
            finalHash,
            Traits(
                normalizeToSmallRange(2, healthCap+1, sliceHashToSmallInt(finalHash, 4, getUIOffset(12, 1)), 15),
                normalizeToSmallRange(3, carryCap+1, sliceHashToSmallInt(finalHash, 4, getUIOffset(10, 2)), 15),
                normalizeToSmallRange(2, defenseCap+1, sliceHashToSmallInt(finalHash, 4, getUIOffset(12, 0)), 15),
                normalizeToSmallRange(2, hackCap+1, sliceHashToSmallInt(finalHash, 4, getUIOffset(13, 0)), 15),
                normalizeToSmallRange(2, breachCap+1, sliceHashToSmallInt(finalHash, 4, getUIOffset(10, 1)), 15),
                normalizeToSmallRange(2, shootCap+1, sliceHashToSmallInt(finalHash, 4, getUIOffset(11, 0)), 15),
                normalizeToSmallRange(2, meleeCap+1, sliceHashToSmallInt(finalHash, 4, getUIOffset(10, 0)), 15)
            ),
            0,
    ...
    // TODO implement this
    function setInGameStatus(uint _id, bool _status) public {
        // TODO: Require only game can do this
        // Or maybe make it only owner of nft, which will be game currently
        console.log("Messege sender setting game", msg.sender);
        characters[_id].inGame = _status;
    }

    function setGameContract(address _contract) public onlyOwner {
        gamesContract = _contract;
    }
    ...
    // TODO: Return tanks by owner
    // TODO: Put name back into contract???
...
contract BCEvents is Ownable {
    ...
    enum MovePlayer { NONE, HOME, LAST_ROOM, RANDOM, PORTAL }

    // ENEMY Types: 0 - turret, 1 - robot, 2 - scav, 3 - bug

    struct BCEffect {
        bool permanant; // TODO: May be able to infer from having trait changes
        bool fullHealth;
        bool instantDeath;
        bool placeHazard;
        bool grantEgg;
        ...
        uint8 placeEnemyOfType;
        uint8 numEnemyToPlace;  // TODO: Limits to only placing one type per event
        EnemyPlacement wheretoPlace;
        ...
        MovePlayer moveType;
        uint8 trapPlayerEscapeRoll;  // >0 to trap player
        uint8 grantAbility;  // TODO

        BCChars.Traits traitIncreases; // TODO:  Should this split from one off effects to avoid waste
        BCChars.Traits traitDecreases;
    }
    ...
    struct BCEvent {
        ...
        string name;
        string text;
        bool permanant; // Draw card before use allowed.  As an action - both can be derived from this
        uint8 rollForLow;  // On or below (unused if zero)
        uint8 rollForHigh; // On or above (nothing happens for rest of numbers)

        BCEffect defaultEffect;
        BCEffect lowEffect;
        BCEffect highEffect;
    }
    ...
    function _initializeDefaultRoomEvents() internal {
        BCChars.Traits memory noTraitsChanged;
        BCEffect memory emptyEffect;
...
contract BCUtils {
    ...
    function smallIntBetweenVals(uint8 _min, uint8 _max) internal returns(uint8) {
        TODO_randomSeed++;
    ...
    function randKeccak() internal returns (bytes32) {
        TODO_randomSeed++;
    ...
    // Design accordingly
    function normalizeToSmallRange(uint _min, uint _max, uint _val, uint _valMax) internal pure returns (uint8) {
    ...
    function sliceHashToSmallInt(bytes32 _hash, uint256 _size, uint256 _offset) internal pure returns(uint8) {
    ...
    function getUIOffset(uint _traitNum, uint _subTraitNum) internal pure returns(uint) {
