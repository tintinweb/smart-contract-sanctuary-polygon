...
contract AssetRouter is IAssetRouter, AccessControl, ReentrancyGuard {
    ...
    function swap(SwapParams memory swapParams) external payable nonReentrant returns (SwapObj memory) {
        ...
        cp.bandwidth = cp.bandwidth - s.kbpRemove;
        {
            if (swapParams.newLiquidity) {
                pool.undistributedVouchers += s.amount + s.fee;
                pool.totalLiquidity += s.amount + s.fee;
            }
        }
        if (pool.undistributedVouchers >= (pool.totalLiquidity * syncDeviation) / BP_DENOMINATOR) {
            _sync(pool);
        }
        CreditObj memory c = _sendVouchers(swapParams.srcPoolId, swapParams.dstChainId, swapParams.dstPoolId);
    ...
    function sendVouchers(
        ...
        c = _sendVouchers(_srcPoolId, _dstChainId, _dstPoolId);
    ...
    function receiveVouchers(
        ...
        cp.bandwidth = cp.bandwidth + _c.vouchers;
    ...
    function swapRemote(
        ...
        cp.kbp -= _s.kbpRemove;
        IAssetV2(pool.poolAddress).withdraw(_to, _s.amount);
    ...
    function createChainPath(
        ...
        chainPaths[_srcPoolId].push(
            ChainPath(false, _srcPoolId, _dstChainId, _dstPoolId, _weight, 0, 0, 0, 0, _poolAddress)
        );
    ...
    function deposit(address _to, uint16 _poolId, uint256 _amount) external {
        ...
        console.log("pool.poolAddress ", IAssetV2(pool.poolAddress).token());
