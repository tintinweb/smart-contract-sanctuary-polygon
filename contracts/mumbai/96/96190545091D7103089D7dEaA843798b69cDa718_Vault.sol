...
contract LottoGame {
    ...
    constructor(address _stableCoin, uint256 _ticketPrice, address _vaultAddress) {    
        owner = msg.sender;
    ...
    function buyTickets(string calldata _referrerUsername, string calldata _username, uint8 _v, bytes32 _r, bytes32 _s) external returns(Ticket memory) {
        ...
        if(game.membersFilled == poolCapacity){
            RNG(RNGManager).requestRandomNumber(game.id, address(this), RNG.GameType.Lotto);
    ...
    function setRNGManager(address _rngManager) external {
        ...
        require(RNGManager == address(0), "RNG Manager already set");
...
contract RNG is VRFConsumerBaseV2 {
    ...
    function fulfillRandomWords(
        ...
        if (requestIdToGame[_requestId].gameType == GameType.Lotto) {
            LottoGame(requestIdToGame[_requestId].gameContract).declareResults(requestIdToGame[_requestId].gameId, _randomWords);
        } else {
            Vault(payable(requestIdToGame[_requestId].gameContract)).decalreReferralWinners(requestIdToGame[_requestId].gameId, _randomWords);
        }
    }
}
...
contract Vault {
    ...
    uint256 public referralJackpot = 60000000;
    ...
    function addMemberToPool(address _address, string calldata _referralCode) public {
        ...
        if (totalBatches >= 1 && specialWalletAmount >= referralJackpot) {
            RNG(RNGManager).requestRandomNumber(gameCount, address(this), RNG.GameType.Vault);
            gameCount++;
        }
    }
