...
// ... 0xA7DB2f3F319598c66DDD78f2a9bc6de9aEa3e71E
// ... 0x9640cEf46C1d9dBAFA184646fcF082E9acc8CE79
...
contract BoomerangCross is ERC721, ERC721URIStorage, ERC721Burnable, ERC2981, Ownable, ReentrancyGuard {
    ...
    // ==============================================================================
    function verifyer(bytes32 messageHash, bytes memory signature) public pure returns (uint8 v, bytes32 r, bytes32 s) {
        ...
        assembly {
            ...
            // final byte (should be either 0x00 or 0x01)
            v := byte(0, mload(add(signature, 96)))
        }

        // transform v from 0 or 1 to 27 or 28
        if (v < 2) {
            v += 27;
        }
    ...
    function claimCheck(bytes32 messageHash, bytes memory signature) public view returns (bool) {
        (uint8 v, bytes32 r, bytes32 s) = verifyer(messageHash, signature);
        require(ecrecover(messageHash, v, r, s) == msg.sender, "Only user");
        return true;
    }
    // ==============================================================================
    ...
    public /*theUser(messageHash, signature)*/ returns (uint256 _tokenId, uint256 _destroy, address _sender) {
        require(claimCheck(messageHash,signature));
