...
import "./AmberfiCallbackUpgradeable.sol";
import "./AmberfiContentUpgradeable.sol";
import "./AmberfiKeyStorageUpgradeable.sol";
import "./AmberfiStorage.sol";
// Interfaces
...
contract NFT is
    ...
    ERC2981Upgradeable,
    I721MintToFullAmberfiFactory,
    AmberfiCallbackUpgradeable,
    AmberfiContentUpgradeable,
    AmberfiKeyStorageUpgradeable
{
    ...
    function initialize(address marketAddress_, address amberfiStorageAddress_)
        ...
        initializer
    {
        __ERC721_init("Amberfi", "AMEF");
        // __ERC721Burnable_init();
        // __ERC721URIStorage_init();
        // __ERC2981_init();
        // __AmberfiCallback_init();
    ...
    function mint(
        ...
        string memory encryptKey_,
        StorageLocation calldata storageLocation_
        ...
        astorage.registerStorage(newItemId, tokenUri_, storageLocation_);
        if (royaltyValue > 0) {
            address recipient = royaltyRecipient;
            if (recipient == address(0)) {
                recipient = _msgSender();
            }
            _setTokenRoyalty(newItemId, recipient, royaltyValue);
        }
        return newItemId;
    }
    ...
    function setContent(
        uint256 _itemId,
        // string calldata _tempCid,
        string calldata _cid
    )
        public
        override(
            // string calldata _cidProof,
            // Status _status
            AmberfiCallbackUpgradeable
        )
    {
        // AmberfiStorage astorage = AmberfiStorage(address(amberfiAddress));
        // AmberfiCallbackUpgradeable.Status store = astorage.getStorageStatus(
        //     _itemId,
        //     _tempCid
        // );
        //string memory errorBase = "NFT: Amberfi failed to store content on Arweave old status=";
        //string memory errorStatus = " new status= ";
        //string memory errorMessage = append(errorBase,stringVar(store), errorStatus , stringVar(_status));
        // This isn't properly compariing, is the error message the issue?
        //require(store == Status.Pending && _status == Status.Finalized, errorMessage);
        _setTokenURI(_itemId, _cid);
    }
        ...
        returns (bool)
    {
        return
            ...
            interfaceId == type(I721MintToFullAmberfiFactory).interfaceId ||
            interfaceId == type(AmberfiCallbackUpgradeable).interfaceId ||
            interfaceId == type(AmberfiContentUpgradeable).interfaceId ||
            interfaceId == type(AmberfiKeyStorageUpgradeable).interfaceId ||
 ...
 * Platform.
 */
abstract contract AmberfiContentUpgradeable is
    Initializable,
    OwnableUpgradeable,
    ERC165Upgradeable,
    IAmberfiContentUpgradeable, 
    IStorageLocation
{
    struct Content {
        uint256 tokenId;
    ...
    /// @param tokenId_ the token id fir which we register the royalties
    /// @param storageLocation_ TODO
    /// @param isEncrypted_ TODO
    /// @param hashVerify_ TODO
    /// @param content_ TODO
    function setContent(
        uint256 tokenId_,
        StorageLocation calldata storageLocation_,
        ...
        contentObj.isEncrypted = isEncrypted_;
        contentObj.storageLocation = storageLocation_;
        contentObj.content = content_;
    ...
    function supportsInterface(bytes4 interfaceId_)
        ...
        view
        virtual
        override(ERC165Upgradeable, IAmberfiContentUpgradeable)
        returns (bool)
    {
        return
            interfaceId_ == type(IAmberfiContentUpgradeable).interfaceId ||
 ...
 * Platform.
 */
contract AmberfiStorage is Initializable, OwnableUpgradeable, IStorageLocation {
    enum Status {
        Pending,
        InProgress,
        Cancelled,
    ...
    struct StorageRecord {
        ...
        StorageLocation location; // This will be the final location of  content (arweave, nfts for nft.storage, infura)
    }

    StorageLocation[] private _locations;
    ...
    function __AmberfiStorage_init() internal onlyInitializing {
        __AmberfiStorage_init_unchained();
    }

    function __AmberfiStorage_init_unchained() internal onlyInitializing {
        __Ownable_init();
    }
    ...
    function registerStorage(
        ...
        srecord.location = storageLocation_;
        emit Store(
            _msgSender(),
            itemId_,
