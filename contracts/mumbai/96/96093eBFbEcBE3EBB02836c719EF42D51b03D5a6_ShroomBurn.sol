...
// File: contracts/ShroomBurnAccessControl.sol
...
contract ShroomBurn is ShroomBurnAccessControl {
    IMushroom shroom;
    uint256 raffleMultiplier;
    mapping(address => uint256) public burned;
    IRaffleMinter raffleMinter;

    event ShroomBurned(uint256 amount, address user);

    constructor(IMushroom _shroom, uint256 _rMul,IRaffleMinter _minter) {
        shroom = _shroom;
        raffleMultiplier = _rMul;
        raffleMinter=_minter;
    }
    ...
    function buyRaffle(uint256 _amount) external {
        address user = msg.sender;
        uint256 shroomToBurn = getEqvShroom(_amount);
        shroom.burnFrom(user, shroomToBurn);
        burned[user] += shroomToBurn;
        emit ShroomBurned(shroomToBurn, user);
        raffleMinter.claimRaffle(user, _amount, "ShroomBurner");
    }

    function getEqvShroom(uint256 _amount) internal view returns (uint256) {
        uint256 shroom = (raffleMultiplier * _amount) * (10 ** 18);
        return shroom;
    }
}