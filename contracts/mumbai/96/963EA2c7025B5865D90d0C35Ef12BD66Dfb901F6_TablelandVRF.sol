    ...
    VRFConsumerBaseV2Upgradeable
{
    ...
    function shuffle() public {
        ...
        require(s_requests[vrf.requestIds[0]].fulfilled == true, "Still awaiting random number from oracle.");

        uint256 index = 0;
        bool available = false;
        for (uint256 i = vrf.requestIds.length - 1; i >= 0; i -=1) {
            if (s_requests[vrf.requestIds[i]].exists == true && s_requests[vrf.requestIds[i]].used == false && s_requests[vrf.requestIds[i]].fulfilled == true) {
                index = i;
                available = true;
                break;
            }
