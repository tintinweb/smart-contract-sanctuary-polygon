...
import "./interfaces/Artfi-IPassVoucher.sol";
    ...
    ReentrancyGuardUpgradeable
{
     ...
     *@return tokenId_ Id of NFT.
     */
    function mintPassVoucher(
        address buyer_,
        PassVoucherData memory mintData_,
        uint256 price,
        string memory currency
    ) public payable returns (uint256 tokenId_) {
        if (mintData_.unclaimed <= 0) revert GeneralError("AF:303");
        if (_artfiManager.isBlocked(msg.sender)) revert GeneralError("AF:126");
        if (_artfiManager.isPaused()) revert GeneralError("AF:128");
        if (mintData_.unclaimed > 50) revert GeneralError("AF:309");
        ...
        emit eMintPassVoucher(tokenId_, mintData_.tokenAddress, buyer_, mintData_.unclaimed);
    }

    function claimVoucherService(address tokenAddress, uint256 tokenId) public returns (string memory uri){
    ...
    function lazyMintSellNft(
        ...
        if (
            ArtfiICollectionV2(lazyMintSellData_.tokenAddress)
                .isQuantityAllowed(purchaseQuantity) == false
     ...
     *@param purchaseQuantity always 1.
    ...
    function lazyMintSellBatch(
        ...
        for (uint256 i = 0; i < purchaseQuantity; i++) {
            lazyMintSellNft(buyer, purchaseQuantity, lazyMintBatchData_[i]);
        }
    }
    ...
    function _lazyMint(
        ...
        _offers[offerId_] = Offer(tokenId_, OfferType.SALE, OfferState.ENDED);
        ...
        for (uint256 i = 0; i < payoutData_.refundAddresses.length; i++) {
            if (payoutData_.refundAddresses[i] != address(0)) {
                if (address(0) == payoutData_.currency) {
                    (bool success, ) = payable(payoutData_.refundAddresses[i])
                        .call{value: payoutData_.refundAmounts[i], gas: 2300}(
                        ""
                    );
