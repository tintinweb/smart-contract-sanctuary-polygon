...
interface ISLLegendaryScene {
    ...
    function setSeasonSettlementContract(
        address _seasonSettlementContract
    ) external;
    ...
    function getSeasonSettlementContract() external view returns (address);
    ...
    LegendarySceneError
{
    ...
    address internal seasonSettlementContract;

    mapping(uint256 => uint256) internal mintedSupplyOfToken;
    mapping(uint256 => string) internal tokenURIs;

    event SetTokenURI(uint256 tokenId, string tokenURI, uint256 timestamp);
}
...
import {ISLLegendaryScene} from "./ISLLegendaryScene.sol";
import {LegendarySceneBase} from "./LegendarySceneBase.sol";
import {SLControllerUpgradeable} from "../../core/SLControllerUpgradeable.sol";
import {ISLProject} from "../../project/ISLProject.sol";
import {ISLSeason} from "../../season/ISLSeason.sol";

contract SLLegendaryScene is
    ISLLegendaryScene,
    LegendarySceneBase,
    ERC1155Upgradeable,
    ...
    UUPSUpgradeable
{
    ...
    function initialize(
        ...
        seasonSettlementContract = _seasonSettlementContract;
    }
    ...
    function supportsInterface(
        ...
        return
            ...
            _interfaceId == type(ISLLegendaryScene).interfaceId;
    }
     ...
     *  Mint
     */
    modifier onlySeasonSettlementContract() {
        _onlySeasonSettlementContract();
        _;
    }
    ...
    function mint(
        ...
        uint256 _tokenId,
        uint256 _amount
    ) external onlySeasonSettlementContract {
        if (!exists(_tokenId)) {
            revert DoesNotExistTokenId();
        }
    ...
    function _onlySeasonSettlementContract() private view {
        if (_msgSender() != seasonSettlementContract) {
            revert OnlySeasonSettlementContract();
        }
    }
    ...
    function setTokenURI(
        ...
        emit SetTokenURI(_tokenId, _tokenURI, block.timestamp);
    }
    ...
    function getSeasonSettlementContract() external view returns (address) {
        return seasonSettlementContract;
    }
