...
/// @dev Mintable. Burnable. Snapshot. Access control. Taxable. PaymentSplitter added for stuck tokens.
...
contract Gaia is ReentrancyGuard, ERC20, ERC20Snapshot, AccessControl, Pausable, Taxable, PaymentSplitter {
    ...
    constructor(address[] memory _payees, uint256[] memory _shares, address _0xBossyBabe, bool taxed_, uint default_tax_, uint maxtax_, uint mintax_)
        ERC20("Gaia", "GAIA")
        Taxable(taxed_, default_tax_, maxtax_, mintax_, _0xBossyBabe)
        PaymentSplitter(_payees, _shares)
        payable
    {
        _grantRole(DEFAULT_ADMIN_ROLE, _0xBossyBabe);
        _grantRole(SNAPSHOT_ROLE, msg.sender);
        _grantRole(SNAPSHOT_ROLE, _0xBossyBabe);
        _grantRole(EXCLUDED_ROLE, msg.sender);
        _grantRole(EXCLUDED_ROLE, _0xBossyBabe);
        _mint(_0xBossyBabe, 5000 * 10 ** decimals()); // Mint 5,000 GAIA to 0xBossyBabe wallet.
    }
...
/// @dev Add the public functions for the DEFAULT_ADMIN_ROLE to add functions:
    ...
    function addExcludedAddress(address newexcluded) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _grantRole(EXCLUDED_ROLE, newexcluded);
    }

    function removeExcludedAddress(address oldexcluded) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _revokeRole(EXCLUDED_ROLE, oldexcluded);
    }
