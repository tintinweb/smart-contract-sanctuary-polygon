...
contract Item is ERC1155, Ownable, ReentrancyGuard {
    ...
    /**
     * @dev Creates a new type of items with `totalSupply_` total supply and `priceUsd` USD price in Wei.
     * ID has the value of `itemTypes.length` before pushing the new item type elements.
     *
     * @notice `priceUsd` must be in Wei!
     */
    function addItems(
        uint256[] calldata totalSupply_,
        uint256[] calldata priceUsd
    ) external onlyOwner {
        require(
            totalSupply_.length == priceUsd.length,
            "totalSupply and priceUsd must have same length"
        );
        uint256 firstId = itemTypes.length;
        for (uint256 i = firstId; i < totalSupply_.length; ++i) {
            itemTypes.push(i);
            items[i].minted = 0;
            items[i].totalSupply = totalSupply_[i];
            items[i].priceUsd = priceUsd[i];
            items[i].paused = false;
        }
    }
