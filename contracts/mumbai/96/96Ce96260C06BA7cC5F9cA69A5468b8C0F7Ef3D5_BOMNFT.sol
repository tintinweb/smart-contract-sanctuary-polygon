...
interface LPPrice {
    function getPriceFromLP(address _lppairaddress) external view returns(uint256);
}
...
contract BOMNFT is ERC721URIStorage, Ownable, BlackList {
    ...
    function mint(string memory _tokenURI, uint id) public payable  {
        ...
        if (id == 0) {
            uint maticCost = usdCost  * 10 ** matic.decimals().mul(2 ** 192).div(lpinfo.getPriceFromLP(lppairaddress) ** 2);
    ...
    function mintNFTs(uint256 _count, string memory _tokenURI, uint id) public payable {
        ...
        if (id == 0) {
            uint nMaticCost = usdCost  * 10 ** matic.decimals().mul(2 ** 192).div(lpinfo.getPriceFromLP(lppairaddress) ** 2).mul(_count);
