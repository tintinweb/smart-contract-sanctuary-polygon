...
contract RWACalculator is AddressAccessor, IRWACalculator {
        ...
        )
    {
        ...
        uint256[] memory tokenIds = new uint256[](1);
        tokenIds[0] = ITangibleFractionsNFT(ftnft).tnftTokenId();
        ITNFTPriceManager priceManager = _getPriceManager();
        (weSellAt, , , , lockedAmount) = priceManager.itemPriceBatchTokenIds(
            tnft,
            paymentToken,
            tokenIds
        );
        ...
        uint256 topPrice = ((((weSellAt[0] + lockedAmount[0]) * share) /
            10000000) * priceThreshold[address(tnft)]) / fullPercent;
        if (amount <= topPrice) {
            inRange = true;
        } else {
            inRange = false;
