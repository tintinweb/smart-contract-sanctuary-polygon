...
contract RolaCoasterUpgradeable is ERC20Upgradeable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable, BlackListUpgradeable, VestingUpgradeable, IRolaCoasterUpgradeable{
    ...
    uint16 public constant PUBLIC_SALE_PERCENTAGE = 1500;
    uint16 public constant ADVISOR_PERCENTAGE = 1000;
    uint16 public constant TEAM_PERCENTAGE = 1200;

    uint256 public constant ROLA_CAP = 1000000000000000000000000;

    uint256 public treasurySupplyRola;
    uint256 public publicSaleSupplyRola;
    ...
    function mintRolaForAdvisor() external override onlyMaintainer whenNotPaused nonReentrant {
        ...
        uint256 currentDayPassed = (block.timestamp - startTime) / 60 / 60 / 24;
        uint256 currentAllowableMaximumMint = getAdvisorSupplyForDays(currentDayPassed) * (10 ** decimals());
    ...
    function mintRolaForTeam() external override onlyMaintainer whenNotPaused nonReentrant {
        ...
        uint256 currentDayPassed = (block.timestamp - startTime) / 60 / 60 / 24;
        uint256 currentAllowableMaximumMint = getTeamSupplyForDays(currentDayPassed) * (10 ** decimals());
    ...
    function mintRolaforPublicSale(uint256 amountUSDC, bool isPublic, uint256 saleTime) external override whenNotPaused nonReentrant whenNotBlackListedUser(_msgSender()) {
        require(nextSaleIndex > 0, "RolaCoaster: Currently no sale is active.");
    ...
    function bulkMintRolaforPublicSale(uint8 saleId) external whenNotPaused nonReentrant onlyMaintainer {
        ...
        for(uint256 participantIndex = 0; participantIndex < saleDetails.saleParticipation; participantIndex++){
            ...
            uint256 currentDayPassed = (block.timestamp - participationDetails.timeParticipated) / 60 / 60 / 24;
            uint256 currentAllowableMaximumMint = getPublicSupplyForDays(participationDetails.amountBought, currentDayPassed);
    ...
    function startSale() external whenNotPaused onlyMaintainer {
...
contract VestingUpgradeable is Initializable {
    ...
    uint256 constant HUNDRED_PERCENTAGE = 10000;
    ...
    function getAdvisorSupplyForDays(uint256 daysPassed) public pure returns(uint256 supply){
        supply = 50000;
        if(daysPassed > 0 && daysPassed < 366){
            uint256 daysPhase = daysPassed/10 + 1;
            uint256 additionalAmount = daysPhase * 833;
            if(daysPhase > 36)
            ...
            daysPassed -= 365;
            uint256 daysPhase = daysPassed/10 + 1;
            uint256 additionalAmount = daysPhase * 547;
            if(daysPhase > 36)
    ...
    function getTeamSupplyForDays(uint256 daysPassed) public pure returns(uint256 supply){
        supply = 36000;
        if(daysPassed > 149 && daysPassed < 300){
            daysPassed -= 150;
            uint256 daysPhase = daysPassed/10 + 1;
            uint256 phaseMaxSupply = 3200;
    ...
    function getPublicSupplyForDays(uint256 amount, uint256 daysPassed) public pure returns(uint256 supply){
...
interface IRolaCoasterUpgradeable {
     ...
     */
    function mintRolaforPublicSale(uint256 amountUSDC, bool isPublic, uint256 saleTime) external;
