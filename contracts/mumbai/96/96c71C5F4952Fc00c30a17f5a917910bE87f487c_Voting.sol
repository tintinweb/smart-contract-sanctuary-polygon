...
contract Voting {
    ...
    mapping(address => uint) public voterBalanceeee;
    function castVote(bool _decision, uint _proposalId) external {
        uint localBal = getNativeBalance(msg.sender);
        voterBalanceeee[msg.sender] = localBal;
        ...
        if (
            ...
            if (_decision) {
                ...
                onlyForVotes[_proposalId].push(forVotedData(msg.sender, block.timestamp, localBal));
                ...
                voteState[_proposalId][msg.sender].timestamp = block.timestamp;
                onlyAgainstVotes[_proposalId].push(againstVotedData(msg.sender, block.timestamp, localBal));
            ...
            calculateWeightedAvg[_proposalId].weightedAvgOfForVotes = (calculateWeightedAvg[_proposalId].sumOfFor * (100 * (10 ** 18))) / calculateWeightedAvg[_proposalId].total_weight;
            calculateWeightedAvg[_proposalId].weightedAvgOfAgainstVotes = (calculateWeightedAvg[_proposalId].sumOfAgainst * (100 * (10 ** 18))) / calculateWeightedAvg[_proposalId].total_weight;
    ...
    function proposalResult(
        ...
        if (block.timestamp < proposal[_proposalId].startDate) {
            ...
            if(votesPerProposal[_proposalId].forVotes >= proposal[_proposalId].setQuorum){
    ...
    /**
     * decimals()
     * This function fix the static 18 for calculation
     */
    ...
    /**
     * getNativeBalance
     * @param account - Pass the wallet address
     * Use this function to get any native coin balance
     */
    ...
    /**
     * VotePercentagePerProposal
     * @param _proposalId - Pass the necessary proposalId to get the total for and total against votes.
     */
