...
/**
 * @title Eternity Deck Partner(Collection) DAO
 *
 * @notice DAO used to distributes funds from the
 *         Eternity Deck game
 */
contract EDCollectionDao is Ownable {
  /**
  ...
  * Dao house edge
  */
  uint public collectionHouseEdge = 0;

    /**
  * Dao house edge
  */
  uint public collectionAttribution = 0;
  
   /**
  * Dao house edge
  */
  uint public marketplaceRoyalty = 0;
  ...
  function changeShareholders(address[] memory _shareholders, uint[] memory _shareholderOwnership) public onlyOwner {
    ...
    for(uint i = 0; i < _shareholderOwnership.length; i++) {
   ...
   */
  function withdraw(uint _amount) external onlyIfShareholderHasBalance(msg.sender, _amount) {
   ...
   */
  function forceWithdraw(address _addr, uint _amount) external onlyOwner onlyIfShareholderHasBalance(_addr, _amount) {
   ...
   */
  function dangerousForceWithdraw(uint _amount) external onlyOwner {
    payable(msg.sender).transfer(_amount);
