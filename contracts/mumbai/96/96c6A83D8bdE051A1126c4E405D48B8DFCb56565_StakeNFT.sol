...
contract NFT is ERC721URIStorage {
    ...
    constructor() ERC721("CelebsNFT", "CelNFT") {
        ...
        returns (uint256)
    {
        ...
        nft.push(NFT(nftTokenURl, msg.sender, id));

        return id;
    }

    function getNFTById(uint256 nftId) public view returns (NFT memory) {
        require(nftId > 0 && nftId <= nft.length, "Invalid NFT ID");
        return nft[nftId - 1];
    }
}
...
contract StakeNFT {
    ...
    constructor(address payable _rewardtoken) {
        rewardtoken = stakingcoin(_rewardtoken);
    ...
    function stakeNFTs(
        ...
        payable(feesAddress).transfer(listingfees);
        ...
        stakerDetails[msg.sender] = Stakers;
    ...
    function unStake(uint256 _tokenId) public payable {
        staker storage newunStaker = stakerDetails[msg.sender];
    ...
    function claimReward(uint256 _tokenId) public payable {
        staker storage newstakeReward = stakerDetails[msg.sender];
