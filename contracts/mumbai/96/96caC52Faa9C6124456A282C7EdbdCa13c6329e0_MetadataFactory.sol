...
interface IMetadataFactory{
    ...
    function createRandomZombie(uint8 level,uint nonce) external view returns(uint8[] memory, uint8, bool, uint, uint);
    function createRandomSurvivor(uint8 level,uint nonce) external view returns(uint8[] memory, uint8, bool, uint, uint);
    function constructNft(uint8 nftType, uint8[] memory traits, uint8 level, bool canClaim, uint stakedTime, uint lastClaimTime) external view returns(nftMetadata memory);
...
contract MetadataFactory is IMetadataFactory, ProxyTarget {
    ...
    function constructNft(uint8 nftType, uint8[] memory traits, uint8 level, bool canClaim, uint stakedTime, uint lastClaimTime) public pure override returns(nftMetadata memory) {
    ...
    function levelUpSurvivor(nftMetadata memory nft,uint nonce) public view returns (nftMetadata memory) {
        ...
        {
            //re roll this
            uint8 chestArmorTrait = uint8(randomNumberGenerator.getRange(1, survivorFactory.survivorChestArmorTraitCount(nft.level),nonce));
            traits[7] = chestArmorTrait;
            //re roll this
            uint8 shoulderArmorTrait = uint8(randomNumberGenerator.getRange(1, survivorFactory.survivorShoulderArmorTraitCount(nft.level),nonce)); 
            traits[8] = shoulderArmorTrait;
            
            //persist - if it's already set
            if(nft.traits[9] == 0) {
                uint8 legArmorTrait = uint8(randomNumberGenerator.getRange(1, survivorFactory.survivorLegArmorTraitCount(nft.level),nonce)); 
                traits[9] = legArmorTrait > 0 ? legArmorTrait : nft.traits[9];
            } else traits[9] = nft.traits[9];
    
            //persist - if level is > 2
            if(nft.level <= 2) {
                uint8 rightWeaponTrait = uint8(randomNumberGenerator.getRange(1, survivorFactory.survivorRightWeaponTraitCount(nft.level),nonce));
                traits[10] = rightWeaponTrait;
            } else traits[10] = nft.traits[10];

            //re roll this
            uint8 leftWeaponTrait = uint8(randomNumberGenerator.getRange(1, survivorFactory.survivorLeftWeaponTraitCount(nft.level),nonce));
    ...
    function createRandomZombie(uint8 level,uint nonce) public view override returns(uint8[] memory, uint8, bool, uint, uint) {
        return (
    ...
    function randomZombieTraits(uint8 level,uint nonce) public view returns(uint8[] memory) {
        ...
        uint8 torsoTrait = uint8(randomNumberGenerator.getRange(1, zombieMetadata.zombieTorsoTraitCount(level),nonce));
        traits[0] = torsoTrait;
        uint8 leftArmTrait = uint8(randomNumberGenerator.getRange(1, zombieMetadata.zombieLeftArmTraitCount(level),nonce)); 
        traits[1] = leftArmTrait;
        uint8 rightArmTrait = uint8(randomNumberGenerator.getRange(1, zombieMetadata.zombieRightArmTraitCount(level),nonce));
        traits[2] = rightArmTrait;
        uint8 legsTrait = uint8(randomNumberGenerator.getRange(1, zombieMetadata.zombieLegsTraitCount(level),nonce)); 
        traits[3] = legsTrait;
        uint8 headTrait = uint8(randomNumberGenerator.getRange(1, zombieMetadata.zombieHeadTraitCount(level),nonce)); 
        traits[4] = headTrait;

        return traits;
    }

    function createRandomSurvivor(uint8 level,uint nonce) public view override returns(uint8[] memory, uint8, bool, uint, uint) {
        return (
    ...
    function randomSurvivorTraits(uint8 level,uint nonce) public view returns(uint8[] memory) {
        uint8[] memory traits = new uint8[](12);
            ...
            {
                uint8 shoesTrait = uint8(randomNumberGenerator.getRange(1, survivorFactory.survivorShoesTraitCount(),nonce)); 
                traits[0] = shoesTrait;
                uint8 pantsTrait = uint8(randomNumberGenerator.getRange(1, survivorFactory.survivorPantsTraitCount(),nonce));
                traits[1] = pantsTrait;
                uint8 bodyTrait = uint8(randomNumberGenerator.getRange(1, survivorFactory.survivorBodyTraitCount(),nonce)); 
                traits[2] = bodyTrait;
                uint8 beardTrait = uint8(randomNumberGenerator.getRange(1, survivorFactory.survivorBeardTraitCount(),nonce)); 
                traits[3] = beardTrait;
                uint8 hairTrait = uint8(randomNumberGenerator.getRange(1, survivorFactory.survivorHairTraitCount(),nonce)); 
                traits[4] = hairTrait;
                uint8 headTrait = uint8(randomNumberGenerator.getRange(1, survivorFactory.survivorHeadTraitCount(),nonce)); 
                traits[5] = headTrait;
                uint8 shirtTrait = uint8(randomNumberGenerator.getRange(1, survivorFactory.survivorShirtTraitCount(),nonce)); 
                traits[6] = shirtTrait;
            }

            {
                uint8 chestArmorTrait = uint8(randomNumberGenerator.getRange(1, survivorFactory.survivorChestArmorTraitCount(level),nonce));
                traits[7] = chestArmorTrait;
                uint8 shoulderArmorTrait = uint8(randomNumberGenerator.getRange(1, survivorFactory.survivorShoulderArmorTraitCount(level),nonce)); 
                traits[8] = shoulderArmorTrait;
                uint8 legArmorTrait = uint8(randomNumberGenerator.getRange(1, survivorFactory.survivorLegArmorTraitCount(level),nonce)); 
                traits[9] = legArmorTrait;
                uint8 rightWeaponTrait = uint8(randomNumberGenerator.getRange(1, survivorFactory.survivorRightWeaponTraitCount(level),nonce));
                traits[10] = rightWeaponTrait;
                uint8 leftWeaponTrait = uint8(randomNumberGenerator.getRange(1, survivorFactory.survivorLeftWeaponTraitCount(level),nonce));
                traits[11] = leftWeaponTrait;
            }
            return traits;
    }
