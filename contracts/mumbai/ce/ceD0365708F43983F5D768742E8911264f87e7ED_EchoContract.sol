...
/// @title Simple Contract for testing
/// @notice This is only used for testing and experimenting with etherjs and solidity.
contract EchoContract is Initializable, UUPSUpgradeable, OwnableUpgradeable, EIP712Upgradeable, ERC2771ContextUpgradeable {
    using ECDSAUpgradeable for bytes32;

    mapping(address => uint) counters;
    ...
    function getRawMsgSender() public view returns (address) {
        // console.log("@@getRawMsgSender: ", RgLib.hexlify(msg.data));
        return msg.sender;
    }

    function getMsgSender() public view returns (address) {
        // console.log("@@getMsgSender: ", RgLib.hexlify(msg.data));
        return _msgSender();
    }
    ...
    function getParams(bool, uint256, string memory) public view returns(string memory, address) {
        // console.log("@@getParams: ", RgLib.hexlify(msg.data));
        return ("bye bye", _msgSender());
    }

    function increaseCounter() public {
        counters[_msgSender()] += 1;
        // console.log("@@increaseCounter: %s => %s", _msgSender(), counters[_msgSender()]);
    }

    function increaseCounterBy(uint value) public {
        // uint oldValue = counters[_msgSender()];
        counters[_msgSender()] += value;
        // console.log("@@increaseCounter: %s - %s => %s", _msgSender(), oldValue, counters[_msgSender()]);
    }

    function getCounter(address _address) public view returns (uint) {
