    ...
    Metadata
{
    ...
    function init(bytes memory _encodedData) external {
        ...
        (rewardsToken, stakingToken, rewardsDistribution, stakingTokenUrl) = abi
            .decode(_encodedData, (IERC20, IERC20, address, string));
        ...
        (
            ...
            _encodedData,
            (IERC20, IERC20, address, string, string, address, uint256, uint256)
        );

        notifyRewardAmountInternal(rewardAmount, rewardsDuration);

        updateMeta(address(stakingToken), routerAddress, stakingTokenUrl);
        updateMeta(address(rewardsToken), address(0), rewardTokenUrl);
