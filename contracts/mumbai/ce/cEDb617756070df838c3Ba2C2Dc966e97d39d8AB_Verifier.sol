...
error SignerNotRecovered(bytes32 hash, bytes signature);
error SignedHashAlreadyApplied(bytes32 hash);

contract Verifier is IVerifier, Initializable, AccessControlUpgradeable {
    ...
    function signedHashApplied(bytes32 hash) external view returns (bool) {
        return signedHashes.contains(hash);
    }

    function recoverSigner(bytes32 hash, bytes memory signature) public returns (address) {
        address signer = hash.toEthSignedMessageHash().recover(signature);
        if (signer == address(0)) {
            revert SignerNotRecovered(hash, signature);
        }

        bool hashApplied = signedHashes.add(hash);
        if (!hashApplied) {
            revert SignedHashAlreadyApplied(hash);
        }
