...
import "@thirdweb-dev/contracts/extension/interface/ISignatureMintERC1155.sol";
...
import "./academy/SignatureMintERC1155Upgradeable.sol";
    ...
    SignatureMintERC1155Upgradeable
{
    ...
    function setContractURI(string memory _uri) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Not approved");
        contractURI = _uri;
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (!hasRole(transferRole, address(0)) && from != address(0) && to != address(0)) {
            require(hasRole(transferRole, from) || hasRole(transferRole, to), "only TRANSFER_ROLE");
        }
    ...
    /// @dev Mints an NFT using a signed payload
    function mintWithSignature(
        ...
        require(
            balanceOf(receiver,tokenIdToMint) == 0,
            "NFT claimed"
        );
