...
contract EthermonStaking is EthermonStakingBasic {
    ...
    function depositeTokens(DepositeToken calldata depositeData) external {
        require(
        ...
        require(
            keccak256(abi.encodePacked(depositeData._appSecret)) == appHash,
        ...
        uint16 badgeAdvantage = (depositeData._createdIndex > 2)
            ? 10
            : badgeAdvantageValues[depositeData._createdIndex];
        ...
        data.badge = depositeData._createdIndex;
    ...
    function updateTokens(
        ...
        if (timeElapsed > 0 && currentTime > data.endTime) {
            ...
            // rewardsCap--;
            emit LOG(timeElapsed, ((currentTime - data.endTime) / 1 minutes));
            uint256 time = timeElapsed /
                ((currentTime - data.endTime) / 1 minutes);

            //uint256 _t = ((currentTime - data.endTime) / 1 minutes);
            //if (_t > timeElapsed) {
            timeElapsed = (data.endTime - data.lastCalled) / 1 minutes;
            //}
            //timeElapsed = (timeElapsed - _t) * time;

            emit LOG(timeElapsed, time);
    ...
    event LOG(uint256 _rarity, uint256 _timeElapsed);
    ...
    function updateStakingData(
        uint256 _lockId,
        uint64[] calldata _monId,
        uint32[] calldata _classId,
        uint256 _pfpId,
        uint16 _level,
        uint16 _createdIndex,
        bytes32 _appSecret
        ...
        require(
            _monId.length <= maxMonCap && _classId.length <= maxMonCap,
            "Mons limit exceed"
        );

        require(
            _monId.length == _classId.length,
            "Mon ID and Class ID length should match"
        );
        ...
        data.level = _level;
        data.validTeam = (_monId.length > 0 && _classId.length > 0) ? 1 : 0;
        data.badge = _createdIndex;

        bytes memory encoded = abi.encode(data);
        stakingData.updateTokenData(encoded);
        emit UpdateData(owner, _lockId, _monId, _pfpId, _level, _createdIndex);
    }
