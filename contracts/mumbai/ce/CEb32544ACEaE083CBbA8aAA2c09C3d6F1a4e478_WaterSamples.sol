...
contract WaterSamples {
    ...
    function addSample(string memory _sampleAddress, uint256 _lead, string memory _latitude, string memory _longitude) public {
        ...
        emit sampleCreated(msg.sender, _sampleAddress, _lead, _latitude, _longitude);
    ...
    function getCoordinates(string memory _sampleAddress) public view returns (string memory latitude, string memory longitude) {
        require(Samples[_sampleAddress].exists, "Sample does not exist");
        sample storage s = Samples[_sampleAddress];
