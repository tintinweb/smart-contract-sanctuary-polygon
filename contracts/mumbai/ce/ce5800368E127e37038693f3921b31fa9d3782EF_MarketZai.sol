...
contract MarketZai is ERC721Enumerable, Ownable {
    ...
    function _buyZai(uint256 _state, uint256 _maxPrice,string memory _name, address _user) internal returns(uint256){
        ...
        return _generateRandomZai(_name, _user, _state);
    }
    ...
    function _getZaiPrice(uint256 _state) internal view returns(uint256){
        ...
        for(uint256 i = 1 ; i <= _nurseriesPreMintedNumber ;){
            ...
            if(_state == 0 ){
                _price += _eggPrices.bronzePrice * 2;
            }
            if(_state == 1 ){
                _price += _eggPrices.silverPrice * 4;
            }
            if(_state == 2 ){
                _price += _eggPrices.goldPrice * 8;
            }
            if(_state == 3 ){
                _price += _eggPrices.platinumPrice * 1;//100;
            }
    ...
    function _calculateDutchPrice(uint256 _state, uint256 _price) internal view returns(uint256){
        ...
        uint256 _nbOfDaysPast = _pastBlock / 43200;
        if(_nbOfDaysPast > 0){
        ...
        // calculate value to reduce for each block past
        uint256 _toReducePerBlock = _price / 43200 / 2; 
        // calculette number of block in the current day
        uint256 _pastBlockInCurrentDay = _pastBlock - (_nbOfDaysPast * 43200);
        
        return (_price - (_pastBlockInCurrentDay * _toReducePerBlock));
