...
contract PerpetualGetter is PerpetualUpdateFunctions, IFunctionList, IPerpetualGetter {
    ...
    function getAMMState(uint24 _perpetualId) external view override returns (int128[13] memory) {
    ...
    function getTraderState(uint24 _perpetualId, address _traderAddr)
        external
        view
        override
        returns (int128[12] memory)
    {
        ...
        */
        ...
        PerpetualData storage perpetual = _getPerpetual(_perpetualId);
        int128 b0 = _getMarginViewLogic().getMarginBalance(_perpetualId, _traderAddr);
        traderState[0] = b0;
        traderState[1] = _getMarginViewLogic().getAvailableMargin(_perpetualId, _traderAddr, true);
    ...
    function getFunctionList() external pure virtual override returns (bytes4[] memory, bytes32) {
        ...
        bytes4[] memory functionList = new bytes4[](22);
        ...
        functionList[12] = this.getActivePerpAccounts.selector;
        functionList[13] = this.getActivePerpAccountsByChunks.selector;
        functionList[14] = this.isTraderMaintenanceMarginSafe.selector;
        functionList[15] = this.countActivePerpAccounts.selector;
        functionList[16] = this.getAMMState.selector;
        functionList[17] = this.getTraderState.selector;
        functionList[18] = this.getOracleFactory.selector;
        functionList[19] = this.getTreasuryAddress.selector;
        functionList[20] = this.getOrderBookFactoryAddress.selector;
        functionList[21] = this.getOrderBookAddress.selector;
...
contract LimitOrderBookFactory is Ownable {
    ...
    uint8 CANCEL_BLOCK_DELAY = 5; // number of blocks before order can be cancelled
...
contract LimitOrderBook is IPerpetualOrder, PerpetualHashFunctions {
    ...
    uint8 internal immutable iCancelBlockDelay;
     ...
     * @param _perpetualManagerAddr the address of perpetual proxy manager.
     * @param _perpetualId The id of perpetual.
     * @param _iCancelBlockDelay How many blocks do we need to wait for canceling to be allowed
     * */
    constructor(
        address _perpetualManagerAddr,
        uint24 _perpetualId,
        uint8 _iCancelBlockDelay,
        ...
        iCancelBlockDelay = _iCancelBlockDelay;
     ...
     * */
    function postOrder(Order memory _order, bytes memory signature) external {
        // Validations
        ...
        perpManager.chargePostingFee(_order, postingFeeTbps);
        // set order submission block
        _order.submittedBlock = uint64(block.number);
        // register
     ...
     * */
    function executeOrder(bytes32 _digest, address _referrerAddr) external {
     ...
     * */
    function cancelOrder(bytes32 _digest, bytes calldata _signature) public {
        Order memory order = orderOfDigest[_digest];
        require(perpetualId == order.iPerpetualId, "order not found");
        require(block.number >= order.submittedBlock + iCancelBlockDelay, "delay required");
    ...
    function _executeOrder(bytes32 _digest, address _referrerAddr) internal {
        ...
        if (block.timestamp <= order.iDeadline) {
            try perpManager.tradeViaOrderBook(order) {
                ...
                /*
                    order should be removed in the following cases
                        ...
                        "order was cancelled" <- no rebate ??probably never reached (order does not exist anymore)
                        "order already executed" <- no rebate
                        ...
                        market order: "price exceeds limit" <- rebate
     ...
     */
    function _wipeOrder(bytes32 _digest) internal {
...
interface IPerpetualTreasury {
    ...
    function removeLiquidity(uint8 _iPoolIndex, uint256 _shareAmount) external;
...
interface IPerpetualLimitTradeManager is IPerpetualOrder {
    function tradeViaOrderBook(Order calldata _order) external;
