...
contract StoryChain is ERC721, ERC721URIStorage, FunctionsClient, ConfirmedOwner {
  ...
  mapping(bytes32 => uint256) public requestToStoryNonce;
  ...
  function createStory(string calldata prompt, string calldata title, uint256 category, uint256 pageLimit) public {
    ...
    if (category == CATEGORY_ALL) {
      ...
      bytes32 reqId = executeRequest(prompt, category);
      requestToData[reqId].nonce = nonce;
      requestToData[reqId].prompt = prompt;
      requestToData[reqId].author = msg.sender;
    }
  ...
  function updateStoryIPFS(uint256 storyNonce, string memory ipfsHash, address author, string memory nftURI) public {
    Story storage s = stories[storyNonce];
    s.ipfs = ipfsHash;
    s.pages++;
    s.updating = false;
