...
contract DepositPool is Ownable {
    ...
    constructor(address _funAddress,address _radTokenAddress,address _gameRewardAddress,address _treasuryAddress){
        funAddress = _funAddress;
        gameReward = _gameRewardAddress;
    ...
    function updateBalance(Record [] memory records) public onlyController {
        uint256 length = records.length;
        for(uint256 i=0;i<length;i++) {
            accountBalance[records[i].account] = records[i].amount;
        }
    }

    function newAccountWeal(address account,uint256 amount) public onlyController {
        accountBalance[account] += amount;
        IWallet(gameReward).withdraw(radToken,address(this),amount);
        emit NewAccount(account,amount);
    }
