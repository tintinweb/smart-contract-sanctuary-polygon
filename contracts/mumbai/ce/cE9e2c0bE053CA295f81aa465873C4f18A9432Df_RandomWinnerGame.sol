...
contract RandomWinnerGame is VRFConsumerBase, Ownable {
    ...
    //amount of link to be sent with request
    uint256 public fee; 
    //id of public key against which randomness is generated
    bytes32 public keyHash; 
    //array of addresses that represent the players
    address[] public players; 
    //maximum number of players that may enter
    uint8 maxPlayers; 
    //determines if the lottery has started
    ...
    //emits when lottery game is started
    event GameStarted(uint256 gameId, uint8 maxPlayers, uint256 entryFee);
    //emits when player enters the lottery
    event PlayerJoined(uint256 gameId, address player);
    //emits when lottey ends
   ...
   */
    ...
    //join game is called when a player wants to join the lottery
    function joinGame() public payable {
        require(gameStarted, "game not started");
        require(msg.value == entryFee, "more MATIC to enter");
   ...
   */
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal virtual override {
        //winner index should be length from 0 to players.length - 1
        //use the modulo operator with players.length value
        uint256 winnerIndex = randomness % players.length;
        //gets the address of winner from players array
        address winner = players[winnerIndex];
        //send winner the funds
