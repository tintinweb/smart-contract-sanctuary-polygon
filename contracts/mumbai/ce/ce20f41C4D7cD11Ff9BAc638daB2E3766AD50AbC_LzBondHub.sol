...
contract LzBondHub is NonblockingLzAppUpgradeable {
    ...
    function _nonblockingLzReceive(
        ...
        // ) = abi.decode(_payload, (uint256, address, string, DataTypes.TaxableTokenData));

        // deployer.lzDeploy(pid, owner, metadataURI, args);

        (address hub, address ownerAddress, string memory id) = abi.decode(
            _payload,
            (address, address, string)
        );

        _proxy = address(deployer.lzDeployApplicationFollowModule(hub, ownerAddress, id));
    }
    ...
    function estimateFees(
     ...
     function estimateFeesEx(
        ...
        bytes memory payload = abi.encode(hub, ownerAddress, id);
        return lzEndpoint.estimateFees(dstChainId, address(this), payload, false, _adapterParams);
    }
    ...
    function deploy(
        ...
        _lzSend(dstChainId, payload, payable(msg.sender), address(0), _adapterParams, msg.value);
    }
...
contract TokenDeployer {
    ...
    function lzDeployApplicationFollowModule(
        ...
        address ownerAddress,
        string memory id
    ) external returns (ApplicationFollowModule) {
