...
contract ProjectElementoMemento is ERC1155, ERC1155Holder {
    ...
    bytes32 public name = "Project Elemento (Memento)";
    bytes32 public symbol = "MEMENTO";
    ...
    function _AdminAddSupply(uint256 tokenId, uint256 supply) public onlyAllowedMinter hasSupply {
        _AdminAddSupplyTo(msg.sender, tokenId, supply);
    }

    function _AdminAddSupplyTo(address wallet, uint256 tokenId, uint256 supply) public onlyAllowedMinter hasSupply {
        require(tokenId < nextTokenId, "Not existing token");
        require(!isTokenPoap[tokenId], "Token is not POAP");
        __mint(wallet, supply, tokenId);
    }

    function _AdminBatchTransfer(address[] memory destinations, uint256 tokenId) public onlyAllowedMinter {
    ...
    function _claimMemento(uint16 tokenId, string memory code) internal {
        require(isTokenPoap[tokenId], "Token is not POAP");
