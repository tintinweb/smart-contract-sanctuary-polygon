...
contract Hivemind is HivemindStorage, HivemindI, AccessControlEnumerable, ReentrancyGuard {
  ...
  function addToMarketInternal(HTokenI _hToken, address _account) internal {
    ...
    accountMarkets[_account].push(_hToken);
  ...
  function underlyingExistsInMarkets(
    ...
    unchecked {
      ...
      for (uint256 i; i < len; i++) {
        if (allMarkets[i].underlyingToken() == _underlying) return true;
      }
    }
    return false;
  }
 ...
 - Loan to value shortfall
  ...
  function _removeMarket(HTokenI _hToken) external {
    ...
    uint256 len = allMarkets.length;
    for (uint256 i; i < len; ) {
      if (allMarkets[i] == _hToken) {
        allMarkets[i] = allMarkets[len - 1];
        break;
      }
      unchecked {
        ++i;
...
contract HivemindStorage {
   ...
   */
  uint256 internal updateThreshold = 1 days;

  /**
   * @notice Per-account mapping of "markets you are in"
   */
