...
// File contracts/Nouns.sol


pragma solidity ^0.8.4;
contract Nouns is ERC721, ERC721Enumerable, ERC721Burnable, Ownable, ILayerZeroReceiver, ILayerZeroUserApplicationConfig {
    using Counters for Counters.Counter;
    using Strings for uint256;
    string public baseURI;
    Counters.Counter private _tokenIdCounter;
    Counters.Counter private _nounIdCounter;
    ...
    event NOUN_SENT (uint noun_id, address destination);
    event NOUN_RECIEVED (uint noun_id);
    event BALANCE_MESSAGEFEE(uint message_fee);

    constructor(string memory _initBaseURI, address _endpoint) ERC721("Nouns", "Noun") {
  ...
  /*   function createNoun(
        uint256 background_id,
        uint256 body_id,
        uint256 accessories_id,
        uint256 head_id,
        uint256 glasses_id
    ) public {
        //must be the owner of the tokens
        require(
            msg.sender == ownerOf(background_id),
            "You don't own this background"
        );
        require(
            msg.sender == ownerOf(body_id),
            "You don't own this Body"
        );
        require(
            msg.sender == ownerOf(accessories_id),
            "You don't own this accessory"
        );
        require(
            msg.sender == ownerOf(head_id),
            "You don't own this head"
        );
        require(
            msg.sender == ownerOf(glasses_id),
            "You don't own these glasses"
        );

        uint256 noun_id = background_id * body_id * accessories_id * head_id * glasses_id;
        burn(background_id);
        burn(body_id);
        burn(accessories_id);
        burn(head_id);
        ...
        _nounIdCounter.increment();
        _safeMint(msg.sender, noun_id);
    } */
    ...
    returns (string memory)
  {
    ...
    return bytes(currentBaseURI).length > 0
        ? string(abi.encodePacked(currentBaseURI, tokenId.toString(), ".png"))
        : "";
  }
    ...
    //send the noun ID to destination address
    function sendNoun(
        ...
        uint noun_id,                  // the uint to start at. use 0 as a default
        address nft_owner
    ) public {
        emit NOUN_SENT(noun_id, _dstPingPongAddr);
        burn(noun_id);
        // abi.encode() the payload with the number of pings sent
        bytes memory payload = abi.encode(noun_id, nft_owner);

        // encode adapterParams to specify more gas for the destination
        uint16 version = 1;
        uint gasForDestinationLzReceive = 900000;
        ...
        emit BALANCE_MESSAGEFEE(messageFee);
    ...
    function lzReceive(
        ...
        (uint noun_id, address nft_owner) = abi.decode(_payload, (uint, address));

        _safeMint(nft_owner, noun_id);
        emit NOUN_RECIEVED(noun_id);
    }
