...
contract MintAndRevealViaBurnFightMe is ERC721Base {
    ...
    function mintViaLootbox(uint256 _tokenId, address _burnerAddress) external onlyLootboxContract {
        _setTokenURI(nextTokenIdToMint(), string(abi.encodePacked(placeholderURIPrefix, Strings.toString(_tokenId))));
        _safeMint(_burnerAddress, 1, "");
        emit MintViaLootbox(_tokenId, _burnerAddress, msg.sender);
    }

    function setRevealedURI(uint256 _tokenId, string memory _uri) external onlyOwner {
        revealedURIs[_tokenId] = _uri;
    }

    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        string memory revealedUriForToken = revealedURIs[_tokenId];
        if (bytes(revealedUriForToken).length > 0) {
            return revealedUriForToken;
        }
