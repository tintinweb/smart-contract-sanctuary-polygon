...
contract ZaiNFT is ERC721, Ownable {
    ...
    modifier onlyAuth() {
        require(gameAddresses.isAuthToManagedNFTs(msg.sender), "Not Authorized to manage Zai NFT");
        _;
    }
    ...
    function tokenURIipfs(uint256 tokenId)
        public
        view
    ...
    function burnZai(uint256 _tokenId)
        external
        returns(bool)
    {
        require(msg.sender == gameAddresses.getAddressOf(AddressesInit.Addresses.PAYMENTS),"only Payment can burn Zai for getting piggyBank");
        _burn(_tokenId);
        return true;
    }
