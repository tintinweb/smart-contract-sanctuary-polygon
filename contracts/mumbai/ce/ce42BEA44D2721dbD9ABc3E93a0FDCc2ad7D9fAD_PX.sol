...
contract PX is ERC721URIStorage, Ownable  {
    ...
    function freeMing() public{
        ...
        pxFreePackage[msg.sender].mintTimestamp = block.timestamp;
        pxFreePackage[msg.sender].expirationTimestamp = 0;
        pxFreePackage[msg.sender].lastUpdateTimestamp = block.timestamp;
    ...
    function updateStorageUsage(uint256 tokenId, uint256 usageInBytes) external onlyOwner() {
        ...
        require(newUsage <= pxStorage[tokenId].sizeInBytes, "Your storage is full. Please mint the new PollinationX NFT or upgrade existing");
