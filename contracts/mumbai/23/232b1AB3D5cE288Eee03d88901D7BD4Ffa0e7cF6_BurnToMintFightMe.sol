...
interface FighterContract {
    function mintViaLootbox(uint256 _tokenId, address _burnerAddress) external;
}

contract BurnToMintFightMe is ERC721Base {
    // event BurnedToMint(
    //     uint256 tokenId,
    //     address claimer
    // );    

    FighterContract public fighterContract;
    ...
    {
        fighterContract = FighterContract(_fighterContract);
    }
    ...
    function burnToMint(uint256 _tokenId) external {
        ...
        // 1. Check if the burner is the owner of the token
        // 2. Burn the token
        // 3. Call a mint function on another contract to mint a new token to the burner        

        _burn(_tokenId, true);
        fighterContract.mintViaLootbox(_tokenId, msg.sender);
    }
}