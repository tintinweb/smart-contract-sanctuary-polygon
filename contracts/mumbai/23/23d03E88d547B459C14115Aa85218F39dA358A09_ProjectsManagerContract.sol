...
contract ProjectsManagerContract is Initializable, Common {
    ...
    function initialize(IERC20Upgradeable _wozxToken) public initializer { // 0x2aac70d576489F519880Ab5314687C79e3755632 fake wozx token
...
return projectProxyToken;
    ...
    function setProjectProxyToken(address _address) external {
        ...
        console.log(projectProxyToken);

        console.log("okkokokoko");
    }

// @todo deploy new contract project
    function addNewProject(
        ...
        emit NewProject(project.id, project.projectValue, project.totalNFT);
    }
    ...
    modifier canMintNft(uint256 _id, uint256 _totalNftToMint) {
        require(
            projects[_id].totalNFT + _totalNftToMint >=
                projects[_id].totalMintedNFT,
            "Not enough NFT to mint"
        );
        _;
    }
    ...
    function contribute(
        uint256 _id,
        uint256 _totalNftToMint,
        uint256 _amount
    ) external  canMintNft(_id, _totalNftToMint) existingProject(_id) 
    {
        // @todo user's can allocate more than the NFT value
        require(
            _amount == (projects[_id].nftValue * _totalNftToMint) * 10**18,
            "Not enough woxz"
        );

        Project project = Project(projects[_id].addr);
        project.contribute(msg.sender, _totalNftToMint, _amount);
        wozxToken.transferFrom(msg.sender, projects[_id].addr, _amount);
    }

    function isWhitelisted(uint256 _projectId)
        external
        view
        returns (uint256)
    {
        Project project = Project(projects[_projectId].addr);
        Project.Locked memory lockedAmount = project.getLockedAmounts(
            msg.sender
        );

        if (lockedAmount.amount > 0) {
            return lockedAmount.nbOfNftToMint;
        }
        return 0;
    }

    //@todo call project's function to distribute rewards (amount);
...
contract Project is Initializable, Common {
    ...
    function contribute(
        ...
        if (contributions[_address].amount > 0) {
            contributions[_address].nbOfNftToMint.add(_totalNftToMint);
            contributions[_address].amount.add(_amount);
            emit UpdatedLocking(
                _address,
                id,
                contributions[_address].amount,
                contributions[_address].nbOfNftToMint
            );
        } else {
            ...
            emit NewLocking(_address, id, _amount, _totalNftToMint);
        }
    }
    ...
    function getLockedAmounts(address _address)
        external
        view
        returns (Locked memory)
    {
        return contributions[_address];
    }

    //@todo only manager caller function to distribute rewards (amount);
    // rewards are on MWOXZ

    // @todo add setter to whitelist user's without them having to call contribute
}
