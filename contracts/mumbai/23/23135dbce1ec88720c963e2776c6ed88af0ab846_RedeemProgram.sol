...
abstract contract NFT {
    ...
    function mintNFT(address owner, string memory tokenURI) public virtual returns (uint256);
}


contract RedeemProgram is Ownable, ReentrancyGuard  {
    ...
    mapping(uint256 => mapping(uint256 => AddProduct)) public company_products;
    mapping(uint256 => uint256[]) public c_products;
    ...
    function addProduct(
        ...
        AddProduct storage productCom = company_products[product_count][_company_code];
        bool status = true;
        ...
        productAdd.company_code = _company_code;

        productCom.product_id = product_count;
        productCom.product_price = _product_price;
        productCom.product_stocks = _product_stocks;
        productCom.product_name = _product_name;
        productCom.product_image = _product_image;
        productCom.token_name = token_name;
        productCom.token_URI = _tokenURI;
        productCom.token_address = token_need;
        productCom.availability = status;
        productCom.company_code = _company_code;


        product_counter.push(product_count);
        c_products[_company_code].push(product_count);
        counter += 1;
        ...
        )
    {
        ...
        if (length > c_products[_company_code].length - start) {
            length = c_products[_company_code].length - start;
        }
        ...
        for (uint256 i = 0; i < length; i++) {
            values[i] = c_products[_company_code][start + i];
            betInfo[i] = company_products[values[i]][_company_code];
        }

        return (values, betInfo, start + length);
    }
