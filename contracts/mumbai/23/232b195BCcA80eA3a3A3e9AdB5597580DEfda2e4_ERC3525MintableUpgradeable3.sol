...
contract ERC3525MintableUpgradeable3 is Initializable, ContextUpgradeable, ERC3525Upgradeable {
    ...
    function changefromNormalToSpecial(
        ...
        require(slotOf(fromTokenId_) == NORMAL, "slot ID is different");
        require(balanceOf(fromTokenId_) >= value_, "too short");
        require(ownerOf(fromTokenId_) == msg.sender, "this is not yours");
        uint modValue = value_ % UNIT;
        uint changeValue = value_ - modValue;
        _burnValue(fromTokenId_, changeValue);
    ...
    function changefromSpecialToNormal(
        ...
        require(ownerOf(fromTokenId_) == msg.sender, "this is not yours");
        _burn(fromTokenId_);
        mintValue(toTokenId_, balanceOf(fromTokenId_)*UNIT);
    }
