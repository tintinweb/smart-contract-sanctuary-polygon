...
error InavlidFallbackFunction();

/**
 * @title rhinestone - a rhinestone account
 * @author Konrad Kopp - @konradkopp
 * @author Ollie Turnbull - @oliverturnbull1
 * @notice version 0.0.1 - do not use in production
 */
contract Rhinestone is BaseAccount {
    constructor(IEntryPoint _entryPoint, IDiamondCut.FacetCut[] memory _initialFacets, address _pluginRegistry) payable {
        // set the entry point
    ...
    function _execute(address dest, uint256 value, bytes calldata func) internal {
        ...
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        bytes4[] memory preExecutePlugins = ds.preExecuteFunctionSelectors;

        if (!_callFacets(preExecutePlugins, true, dest, value, func)) {
            revert PreExecutePluginsFailed();
        }
     ...
     */
    function _callFacets(bytes4[] memory plugins, bool returnFalse, address dest, uint256 value, bytes calldata func)
        internal
        returns (bool)
    {
        ...
        unchecked {
            for (uint256 i = 0; i < pluginsLen; ++i) {
                ...
                bool success = _delegatecall(facet, abi.encodeWithSelector(plugins[i], dest, value, func));
                if (!success) {
                    if (returnFalse) {
                        return false;
                    }
    ...
    function _callFacets(bytes4[] memory plugins, bool returnFalse, UserOperation calldata userOp, bytes32 userOpHash)
        internal
        returns (bool)
    {
        uint256 pluginsLen = plugins.length;
        ...
        unchecked {
            for (uint256 i = 0; i < pluginsLen; ++i) {
                address facet = ds.selectorToFacetAndPosition[msg.sig].facetAddress;
                bool success = _delegatecall(facet, abi.encodeWithSelector(plugins[i], userOp, userOpHash));
                if (!success) {
                    if (returnFalse) {
                        return false;
                    }
    ...
    function _callFacets(bytes4[] memory plugins, bool returnFalse) internal returns (bool) {
        uint256 pluginsLen = plugins.length;
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        unchecked {
            for (uint256 i = 0; i < pluginsLen; ++i) {
                address facet = ds.selectorToFacetAndPosition[msg.sig].facetAddress;
                bool success = _delegatecall(facet, abi.encodeWithSelector(plugins[i]));
                if (!success) {
                    if (returnFalse) {
                        return false;
                    }
        ...
        returns (uint256 validationData)
    {
        ...
        bool validationResult = _callFacets(validatePlugins, true, userOp, userOpHash);
        if (!validationResult) {
            return 1;
        }
        return 0;
    ...
    function _delegatecall(address facet, bytes memory callData) internal returns (bool result) {
        assembly {
            result := delegatecall(gas(), facet, add(callData, 0x20), mload(callData), 0, 0)
        }
    }
    ...
    receive() external payable {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        bytes4[] memory preReceiveFunctionSelectors = ds.preReceiveFunctionSelectors;
        if (!_callFacets(preReceiveFunctionSelectors, true)) {
            revert PreReceivePluginsFailed();
        }
        bytes4[] memory postReceiveFunctionSelectors = ds.postReceiveFunctionSelectors;
    ...
    fallback() external payable {
        ...
        //     revert InavlidFallbackFunction();
        // }
        address facet = ds.selectorToFacetAndPosition[msg.sig].facetAddress;
        if (facet == address(0)) revert InavlidFallbackFunction();
...
contract RhinestoneFactory is Ownable {
    ...
    function createAccount(
        bytes32 salt,
        IDiamondCut.FacetCut[] calldata initialFacets
        ...
        addr = Create2.deploy(
            ...
            abi.encodePacked(
                ...
                abi.encode(entryPoint, initialFacets, pluginRegistry)
            )
        );
    }
...
interface IPluginRegistry {
    function claimNamespace(bytes32 _namespace) external returns (uint256 tokenId);
    function burnNamespace(bytes32 _namespace) external;
    function submitPlugin(PluginDetails calldata _details, bytes calldata _signature) external;
    function validateCut(IDiamondCut.FacetCut[] memory _diamondCut) external view returns (bool);
    ...
    function getImplementation(bytes32 _namespace, bytes4 _functionSelector) external view returns (address);
...
library LibDiamond {
    ...
    struct DiamondStorage {
        ...
        bytes4[] preExecuteFunctionSelectors;
        bytes4[] postExecuteFunctionSelectors;
        bytes4[] validateFunctionSelectors;
        bytes4[] preReceiveFunctionSelectors;
        bytes4[] postReceiveFunctionSelectors;
