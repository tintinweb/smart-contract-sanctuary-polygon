...
abstract contract IERC721 is IERC165 {
    ...
    event Reveal(uint256 indexed round);
    event whiteList1Time(uint256 indexed round, uint256 indexed startTimeStamp, uint256 indexed endTimeStamp);
    event whiteList2Time(uint256 indexed round, uint256 indexed startTimeStamp, uint256 indexed endTimeStamp);
...
contract WalkDony is ERC721, ERC721Metadata, Ownable {
    string nft_name = "Test";
    string nft_symbol = "Test";
    string private _buri;
    string private _notBuri;
    address public WETH;
    ...
    constructor(uint256 _tokenPrice) public ERC721() {
    ...
    function checkMint(uint256 _tokenId) public view returns (bool ) {
        return mintCheck[_tokenId];
    }
    ...
    function mint(address _user, uint256 amount) public {
        ...
        require(mintingTotalSupply >= mintCount + amount);
        for (uint256 i = 1; i <= amount; i++) {
            _safeMint(_user, mintCount + i);
            mintCheck[mintCount +i] = true;
            mintCount += 1;
        }
    ...
    function selectMint(address _user, uint256 _tokenId) public payable {
        require(_tokenId > mintingTotalSupply && !mintCheck[_tokenId]);
        require(selectTotalSupply >= selectSupplyCount);
        if (msg.sender != owner) {
            // TransferHelper.safeTransferFrom(WETH, msg.sender, address(this), tokenPrice);
    ...
    function EmergencyMintPause() public onlyOwner {
