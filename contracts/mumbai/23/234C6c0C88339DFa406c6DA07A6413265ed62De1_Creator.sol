...
contract Creator is ERC1155, Ownable, ERC1155Supply {
    ...
    function launchpad_submit(LaunchPad.launchpad_data memory launchpad_information , string memory _url, bytes memory data) external IsMint(launchpad_information.id) VerifyTime(launchpad_information.startTime, launchpad_information.endTime){
        ...
        LaunchPad launchPad = new LaunchPad(launchpad_information, maxNFTLock, address(this), referral, WETH);
...
contract LaunchPad is ERC1155Holder {
    ...
    constructor(launchpad_data memory launchpad_information, uint _maxNFTLock, address _nft, address _referral, address _weth) {
    ...
    function listing(address _router) external verifyTimeClaim(){
        ...
        dml = IRouter(_router).launchpadAddLiquidity(LaunchPadInfo.tokenPayment, NFT, LaunchPadInfo.id, tokenAddLiquidity, totalNFTAddLiquidity, 0, address(this), block.timestamp + 20*60);
        if((totalBurn + remaining) > 0){
            ICreator(NFT).burn(LaunchPadInfo.id, (totalBurn + remaining));
        }
        if(totalNFTReceive > 0){
            TransferHelper.safeTransferFromNFT(NFT, address(this), msg.sender, LaunchPadInfo.id, totalNFTReceive, bytes(''));
        }
        if((totalTokenReceive - tokenAddLiquidity - totalRewardReferral) > 0){
            TransferHelper.safeTransfer(LaunchPadInfo.tokenPayment, msg.sender, (totalTokenReceive - tokenAddLiquidity - totalRewardReferral));
        }
