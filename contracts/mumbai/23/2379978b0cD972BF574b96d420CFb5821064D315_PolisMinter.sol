...
contract Polis is IPolis, IAccessControlHolder, ERC721AQueryable {
    ...
    mapping(uint256 => address) internal _lockers;
    ...
    function lock(uint256 _tokenId) external override isNotLocked(_tokenId) {
        address locker = msg.sender;
        address owner = ownerOf(_tokenId);

        if (!isApprovedToLock(_tokenId, locker)) {
            revert CannotLock();
        }
        _lockers[_tokenId] = locker;

        emit Lock(_tokenId, owner, locker);
    }
    ...
    function isLocked(uint256 _tokenId) public view returns (bool) {
        return _lockers[_tokenId] != address(0);
    }
    ...
    function _upgrade(uint256 tokenId) internal {
        ...
        emit Upgrade(tokenId, currentLevel);
    }

    function mintAsMinter(address to) external onlyMinterRoleAccess {
        _safeMint(to, 1);
    }
    ...
    function senateLevel(uint256 tokenId) public view returns (uint8) {
    ...
    function boost(
        ...
        uint256 level = senateLevel(tokenId);
    ...
    function lockerOf(uint256 tokenId) public view virtual returns (address) {
        if (!_exists(tokenId)) {
            _revert(OwnerQueryForNonexistentToken.selector);
        }
    ...
    function _afterTokenTransfers(
        ...
        uint256 stopTokenId = startTokenId + quantity;
        for (uint256 tokenId = startTokenId; tokenId < stopTokenId; ++tokenId) {
    ...
    function isApprovedToLock(
        uint256 _tokenId,
        address _wallet
    ) public view returns (bool) {
        address owner = ownerOf(_tokenId);
        return
            _lockApprovals[_tokenId] == _wallet ||
            _lockOperatorApprovals[owner][_wallet];
    }
...
contract PolisMinter is IAccessControlHolder {
    error OnlyPolisMinterRole();
    error PaymentFiled();
    error AlreadyLocked();

    bytes32 constant POLIS_MINTER_WITH_PAYMENT_ADMIN =
        keccak256("POLIS_MINTER_WITH_PAYMENT_ADMIN");

    event MintedWithPayment(address indexed to);

    Polis public immutable polis;
    IAccessControl public immutable override acl;
    IERC20 public paymentToken;
    address public paymentReceiver;
    uint256 public paymentValue;

    constructor(
        IAccessControl _acl,
        Polis _polis,
        IERC20 _payemntToken,
        address _paymentReceiver,
        uint256 _paymentValue
    ) {
        acl = _acl;
        polis = _polis;
        paymentToken = _payemntToken;
    ...
    function setPayment(
        ...
        uint256 value,
        address wallet
    ) external onlyPolisMinterWithPaymentAdminRole {
        paymentToken = token;
        paymentValue = value;
