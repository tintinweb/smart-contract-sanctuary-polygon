...
{
  uint256 public constant MUTANT_BATCH = 1;

  uint256 public constant MAX_SUPPLY = 1400;
   ...
   */
  function __MutantBatch_init(string memory uri_) internal onlyInitializing {
    __Ownable_init();
    __ERC1155_init_unchained(uri_);
  ...
  function mint(address to, uint256 quantity) external payable onlyOperator {
    if (quantity + totalSupply(MUTANT_BATCH) > MAX_SUPPLY) {
      revert MaxSupplyReached();
    }
    _mint(to, MUTANT_BATCH, quantity, '');
  }
