...
abstract contract DropSinglePhase is IDropSinglePhase {
    using TTBitMaps for TTBitMaps.BitMap;
     ...
     */
    mapping(bytes32 => TTBitMaps.BitMap) private usedAllowlistSpot;
        ...
        if (!_hasRole[role][account]) {
            revert(
                string(
                    abi.encodePacked(
                        "Permissions: account ",
                        TTStrings.toHexString(uint160(account), 20),
                        " is missing role ",
                        TTStrings.toHexString(uint256(role), 32)
                    )
                )
            );
        }
 ...
 *  'delayed-reveal' NFTs. You can learn more about the usage of delayed reveal NFTs here - https://blog.triton.com/delayed-reveal-nfts
 */
...
/**
 *  Triton's `PlatformFee` is a contract extension to be used with any base contract. It exposes functions for setting and reading
...
contract TritonSignatureDrop is
    Initializable,
    ContractMetadata,
    PlatformFee,
    Royalty,
