...
abstract contract ItemDistributeBaseContract is CooperatableBase {
    ...
    function _consume_distributed(uint32 rand_value,bool ordered) internal returns (bool,uint32){
...
contract StringItemDistribute is ItemDistributeBaseContract,IItemDistributeString {
    ...
    event OnConsumeDistributed(address indexed addr,uint32 value,string data,uint256 amount,uint32 as_item_index);
    ...
    function _do_consume_distributed(address addr,uint32 rand_value,bool ordered) internal returns (string memory){
        (bool found,uint32 item_index) = _consume_distributed(rand_value,ordered);
        require(found,"FAILED_ON_DISTRIBUTE");
        emit OnConsumeDistributed(addr,item_index,items[item_index].item,items[item_index].amount,item_index);
