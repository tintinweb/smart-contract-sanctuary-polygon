...
contract WooTemplate is ERC721, ERC721URIStorage, Ownable {
    ...
    function activate(uint256 tokenId) external onlyOwner {
        require(
            tokenStatus[tokenId] == Status.SUSPENDED ||
                tokenStatus[tokenId] == Status.DESTORYED,
    ...
    function archive(uint256 tokenId) private {
        require(
            tokenStatus[tokenId] == Status.DESTORYED,
            "WooSBT: The SBT is not activating now."
        );
        ...
        tokenStatus[tokenId] = Status.DESTORYED;
        emit StatusChanged(tokenId, Status.DESTORYED);
    }
    ...
    function _transferToken(
        ...
        require(
            balanceOf(to) == 0,
            "WooSBT: `to` address already has token."
        );
        require(
            tokenStatus[tokenId] == Status.SUSPENDED,
