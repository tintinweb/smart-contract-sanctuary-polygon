...
import {IOpsProxy} from "./gelato/IOpsProxy.sol";

// solhint-disable no-empty-blocks
// solhint-disable not-rely-on-time
contract W3fContractTask is AutomateTaskCreator {
    uint256 public count;

    event LogIncreaseCount(uint256 indexed timestamp, uint256 indexed newCount);
    event LogIncreaseCountSyncFee(
    ...
    function createSyncFeeTask(
        string memory _web3FunctionHash,
        bytes calldata _web3FunctionArgsHex
    ) external {
        ModuleData memory moduleData = _getModuleArgs(
            _web3FunctionHash,
        ...
        _createTask(
            dedicatedMsgSender,
            abi.encode(IOpsProxy.batchExecuteCall.selector),
            moduleData,
            ETH
        );
    }

    function create1BalanceTask(
        string memory _web3FunctionHash,
        bytes calldata _web3FunctionArgsHex
    ) external {
        ModuleData memory moduleData = _getModuleArgs(
    ...
    /**
     * @dev
     * (intentional) will not work with v2.0.0 w3f if
     * - w3f returns an execAddress that is not address(this)
     * - w3f returns more than 1 callData
     * will work with v1.0.0 w3f
     */
    function createInvalidTask(
        string memory _web3FunctionHash,
        bytes calldata _web3FunctionArgsHex
    ) external {
        ModuleData memory moduleData = _getModuleArgs(
    ...
    function increaseCount() external {
        ...
        emit LogIncreaseCount(block.timestamp, newCount);
    }

    function increaseCountSyncFee() external {
        uint256 newCount = count + 1;
        count = newCount;
        ...
        emit LogIncreaseCountSyncFee(block.timestamp, newCount);
    }

    function _getModuleArgs(
        string memory _web3FunctionHash,
        bytes calldata _web3FunctionArgsHex
