...
contract Alkhemist is ERC721, ERC721Enumerable, Ownable, EIP712, ERC721Votes, ERC2981 {
    using Strings for uint256;
...
contract Elixir is ERC20, ERC20Snapshot, Ownable, Pausable {
    ...
    constructor() ERC20("Elixir", "ELXR") {
        maxSupply = 36900000 * 10 ** decimals();
        _mint(msg.sender, 400000 * 10 ** decimals());
    }

    function mint(address to, uint256 amount) external {
        require(controllers[msg.sender], "Only controllers can mint");
        require(ERC20.totalSupply() + amount <= getMaxSupply(), "ERC20Capped: cap exceeded");
...
contract MagnumOpus is Ownable, IERC721Receiver {

  uint256 public totalStaked;
  address public CeremonyOfFate = 0x9bc698bB813c2e39A55Aea86a4fBbaFCb739b5D7;
  ...
  mapping(uint256 => Stake) public vault; 
   ...
   constructor(Alkhemist _nft, Elixir _token) { 
    nft = _nft;
    token = _token;
  }
  ...
  function _claim(address account, uint256[] calldata tokenIds, bool _unstake) internal {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      ...
      earnedAfterFee = earned * 90 / 100;
      fee = earned - earnedAfterFee;
      vault[tokenId] = Stake({
        owner: account,
        tokenId: uint24(tokenId),
        timestamp: uint48(block.timestamp)
    ...
    if (earned > 0) {
      token.mint(account, earnedAfterFee);
      token.mint(CeremonyOfFate, fee);
    }
    if (_unstake) {
      _unstakeMany(account, tokenIds);
    }
    emit Claimed(account, earnedAfterFee);
    emit Claimed(CeremonyOfFate, fee);
  }
  ...
  function onERC721Received(
    ...
    function updateLotteryAddress (address _newCeremonyOfFate) external onlyOwner {
        CeremonyOfFate = _newCeremonyOfFate;
    }
