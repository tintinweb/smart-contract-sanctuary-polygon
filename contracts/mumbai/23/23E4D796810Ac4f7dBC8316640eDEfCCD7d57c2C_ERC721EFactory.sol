...
import '../SuperOwner.sol';

/// @notice Deploys most simple ERC721 Event contract
contract ERC721EFactory is ERC721FactoryWide, IERC721EFactory, SuperOwner {

    using EnumerableSet for EnumerableSet.AddressSet;

    bytes32 private constant forwarderVersion = keccak256(bytes('1.0.0-beta.0+fob.rsv.iForwarder'));
    
    constructor(address forwarder, address superOwner, address owner) SuperOwner(superOwner, owner) {
    ...
    function deploy(
        ...
        ERC721E erc721E = new ERC721E(name, symbol, venue, data);
...
interface InterfaceVenueRegistar {
    function getOwner() external view returns (address);
    function superOwner() external view returns (address);
    function isTokenCreator(address sender) external view returns (bool);
}

contract ERC721E is IERC721E, ERC721AMarketplace, SuperOwner, AccessControl {
    ...
    InterfaceVenueRegistar private _venueRegistar;
    ...
    constructor(
        ...
        address venue_,
        bytes memory data
    ) ERC721A(name, symbol) SuperOwner(_msgSender(), _msgSender()) {
        venue = venue_;
        (
        ...
        _venueRegistar = InterfaceVenueRegistar(payable(venueRegistar));
        transferOwnership(_venueRegistar.getOwner());
        transferSuperOwnership(_venueRegistar.superOwner());
    ...
    /// @inheritdoc IERC721Wide
    function setEventDate(uint256 _eventDate) external virtual override onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function buyTickets(
        ...
        bool isTokenCreator = _venueRegistar.isTokenCreator(_msgSender());

        //Implement royalties
        if (price > 0 && !isTokenCreator) { 
...
contract SuperOwner is Ownable {
    ...
    error SenderIsNotOwnerOrSuperOwner();
    error SenderIsNotSuperOwner();

    address public superOwner;

    constructor(address superOwner_, address owner) {
        superOwner = superOwner_;
        _transferOwnership(owner);
    }

    function transferOwnership(address newOwner) public virtual override {
        if(_msgSender() != superOwner && _msgSender() != owner()) revert SenderIsNotOwnerOrSuperOwner();
        _transferOwnership(newOwner); 
    }

    function transferSuperOwnership(address newSuperOwner) public virtual {
        if(_msgSender() != superOwner) revert SenderIsNotSuperOwner();
