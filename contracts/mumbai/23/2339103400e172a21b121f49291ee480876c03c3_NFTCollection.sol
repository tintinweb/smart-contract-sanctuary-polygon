...
contract NFTCollection is ERC2981, Ownable, ERC721Enumerable {
...
//Structs
    ...
    //Define bytes for eip712
    bytes32 constant EIP712DOMAIN_TYPEHASH =
        keccak256("EIP712Domain(string name,string version,uint256 chainId,address verifyingContract)");

    bytes32 constant WHITELIST_TYPEHASH =
        keccak256(
            "WhiteListe(address whitelistUser,uint256 nonce,bool _isWhiteListed)");
    ...
    constructor(
        ...
        Domainhash(
            EIP712Domain({
                ...
                version: '1',
                chainId: 80001,
                // verifyingContract: this for testing 0xe7f1725E7734CE288F8367e1Bb143E90bb3F0512
        ...
        launchpad = INFTLaunchPad(launchpadAddress);
    ...
    function Domainhash(WhiteListe memory whiteListeds) internal pure returns (bytes32) {
        return
            keccak256(
                abi.encode(
                    WHITELIST_TYPEHASH,
                    whiteListeds.whitelistUser,
                    whiteListeds.nonce,
                    whiteListeds._isWhiteListed  
                )
            );
    }

    function verifyWhiteListUser(WhiteListe memory whiteLists, uint8 v, bytes32 r, bytes32 s) public view returns(bool){
     
         bytes32 digest = keccak256(abi.encodePacked(
            "\x19\x01",
            DOMAIN_SEPARATOR,
            Domainhash(whiteLists)
        ));
        return (ecrecover(digest, v, r, s) == _getPublicKey());
    }
     ...
     *@param _currency: NFT currency to be minted
     */
    function mint(
        uint256 _quantity,
        address _currency,
        WhiteListe memory whitelists,
        uint8 v,
        bytes32 r,
        bytes32 s
    ) external payable {
        ...
        require(
            balanceOf(msg.sender) + _quantity <= maxNFTPerUser,
            "NFTCollection: Max limit reached"
        );

        require(!proceedNonce[whitelists.nonce], "NFTCollection: Nonce already proceed!");
        require(
            _quantity > 0 && _quantity <= maxQuantity,
            "NFTCollection: Max quantity reached"
        );

        bool WhiteListed = verifyWhiteListUser(whitelists,v,r,s);

        if (WhiteListed && block.timestamp >= WhiteListStartTime && block.timestamp <= WhiteListEndTime) {
            require(
        ...
        proceedNonce[whitelists.nonce] = true;
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return
            bytes(baseURI).length != 0
                ? string(abi.encodePacked(baseURI, (id), baseURISuffix))
                : "";
    }
}