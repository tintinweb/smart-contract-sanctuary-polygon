...
contract Test {
    ...
    event Buy(address indexed _user, string _from, uint256 tokenQty, uint256 amt);
    event Sell(address indexed _user, string _from, uint256 tokenQty, uint256 amt);
    ...
    function callFunctionB(string memory _name,uint256 buyAmt) public payable
	{
        if(keccak256(abi.encodePacked((_name))) == keccak256(abi.encodePacked(("MATIC")))){
            require(msg.value==buyAmt,"Insufficient Amount.");
            uint256 buyFee=((buyAmt).mul(pairs[_name].buyFee)).div(1e20); 
            buyAmt=buyAmt-buyFee;
            uint256 tokenAmt=((buyAmt).div(pairs[_name].buyPrice));            
            TestToken.transfer(msg.sender,tokenAmt);
        }
        else
        {
     ...
     function calBuy(string memory _name,uint256 buyAmt) public view returns(uint256 _buyFee,uint256 _buyAmt,uint256 _tokenAmt){
            uint256 buyFee=((buyAmt).mul(pairs[_name].buyFee)).div(1e20); 
            buyAmt=buyAmt-buyFee;
            uint256 tokenAmt=((buyAmt).div(pairs[_name].buyPrice)); 
            return(buyFee,buyAmt,tokenAmt); 
     }
