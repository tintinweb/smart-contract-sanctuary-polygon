...
contract RealEstate is ERC721 {
    ...
    function createProperty(
        string memory imageHash, string memory title,
         bytes memory description, string memory location, uint256 price, uint256 rooms, uint256 bathrooms, bool sellOrRent) public returns (uint256) {
        _tokenIds.increment();

        uint256 newItemId = _tokenIds.current();
        _safeMint(msg.sender, newItemId);

        Property memory newProperty = Property(newItemId, msg.sender, title, description, imageHash, location, price, rooms, bathrooms, sellOrRent, block.timestamp, 0);
    ...
    function buyOrRent(uint256 id) public payable {
        ...
        if (_properties[id].sellOrRent) { // If the property is for sale
            ...
            require(_properties[id].soldOn == 0, "Property has already been rented");

            _properties[id].soldOn = block.timestamp + 30 days; // Set the rental period to 30 days
            _transfer(_properties[id].owner, msg.sender, id); // Transfer the ownership of the property to the renter
        }
    }
}