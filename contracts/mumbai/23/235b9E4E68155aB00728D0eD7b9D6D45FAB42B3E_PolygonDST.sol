...
import "./MagpieHelper.sol";

 contract PolygonDST is MagpieHelper {
    using Strings for uint256;

    constructor(uint256 _decimals) MagpieHelper(_decimals) {}
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    suppliedLiquidity,
                    ' DST</text><text transform="rotate(-90 213.61 143.092)" class="prefix__st2 prefix__st19 prefix__st20">',
                    sharePercent,
...
abstract contract MagpieHelper is Ownable {
    using Strings for uint256;

    uint256 public tokenDecimals;

    event GetBackgroundUrlUpdated(string BackgroundUrl);
    ...
    function getDescription(uint256, uint256) public view virtual returns (string memory) {
        return
            string(
                abi.encodePacked(
                    "This NFT represents your position as Liquidity Provider on MagpieCore on ",
                    getChainName(),
                    ". To visit the bridge, visit [Magpie](www.example.com)."
                )
            );
    }
