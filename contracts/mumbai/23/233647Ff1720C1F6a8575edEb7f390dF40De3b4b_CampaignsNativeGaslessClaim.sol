  ...
  EIP712MetaTransactionUpgradeable
{
  ...
  function createCampaign(
    ...
    if (_tokenAddress != ETHAddress) {
      IERC20Upgradeable(_tokenAddress).safeTransferFrom(_sender, address(this), _totalAmount);
    } else {
      assembly {
  ...
  function claim(
    ...
    // Check claimFee paid if not isGasless | no fee paid if isGasless
    /// @dev checking latter since function is payable
    if ((_campaign.isGasless == 0 && msg.value != claimFee) || (_campaign.isGasless != 0 && msg.value != 0)) {
      assembly {
