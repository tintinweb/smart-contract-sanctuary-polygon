...
contract Deal is Initializable, PausableUpgradeable {
        ...
        returns (bytes32 hashA, bytes32 hashB, uint256 timeA, uint256 timeB)
    {
        ...
        return (
            milestones[_index].hashA,
            milestones[_index].hashB,
            milestones[_index].timeA,
            milestones[_index].timeB
        );
    }
    ...
    function pause() external whenNotPaused onlyRainCompliance {
        _pause();
    }

    /// @notice Unpause function, only for Rain compliance 
    function unpause() external whenPaused onlyRainCompliance {
        _unpause();
    }
}