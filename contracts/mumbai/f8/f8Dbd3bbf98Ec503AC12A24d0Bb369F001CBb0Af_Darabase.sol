...
contract Darabase is NFTContract{
    ...
    function buyRentalSecondarySale(
        ...
        // require(block.timestamp >= rental[sellOrder.pdrId][sellOrder.rentalId].startTime && block.timestamp < rental[sellOrder.pdrId][sellOrder.rentalId].endTime, "Expired");
        require(startTime<endTime, "end time should be greater than start time");
    ...
    function distributeRevenue(
        ...
        //get rental ids
        uint256[] memory renters = getRentalFromPDR(pdrId);
        ...
        uint256 platformPayment = distributorFee;

        doPlatformPayment(msg.sender, totalPayment, payment, platformPayment);

        for(uint i=0; i<renters.length; i++){
            uint rentalId = renters[i];
        ...
        doPaymentTransfer(msg.sender, totalPayment - fee, ownerPdr, payment, false);
    }
    ...
    function renewRental(
        ...
        require(startTime<endTime, "end time should be greater than start time");
        uint256 numberOfRentals = rental[pdrId][rentalId].amount;
        ...
        uint remaining = doPlatformPayment(msg.sender, totalPayment, sellOrder.payment, platformPayment);
        doPaymentTransfer(rental[pdrId][rentalId].renter, remaining, sellOrder.maker, sellOrder.payment, true);
    
        rental[pdrId][rentalId].startTime = startTime;
        rental[pdrId][rentalId].endTime = endTime;

        emit RentalRenewed(rentalId, rental[pdrId][rentalId].startTime, rental[pdrId][rentalId].endTime);
    }
...
contract NFTContract is ERC1155, Ownable, Pausable{
    ...
    function mintPDR(
        ...
        _mint(ownerRwo, nftId, amount, data);
    ...
    function burnRental(
        ...
        delete pdrToRental[pdrId][length-1];
