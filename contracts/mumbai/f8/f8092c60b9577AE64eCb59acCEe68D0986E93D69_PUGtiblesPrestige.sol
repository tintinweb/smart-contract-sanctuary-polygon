...
contract PUGtiblesPrestige is ERC721AQueryable, Ownable, ReentrancyGuard {
    ...
    uint256 public wlPugCost = 0.001 ether;
    ...
    // Public functions
    function pugMint(uint256 mintAmount) external payable {
    ...
    function pugMintWL(uint8 mintAmount) external payable {
        uint256 nextTokenId = _nextTokenId();
        ...
        require(mintAmount > 0, "Provide mint amount major to 0");
        require(mintAmount <= maxDalisPerMint, "Max Dalis per mint reached"); 
        require(mintAmount <= whiteList[msg.sender], "Exceeded max available to purchase");

        require(nextTokenId + mintAmount < seasonSupply, "Max season supply reached");
        require(nextTokenId + mintAmount < TOTAL_SUPPLY, "Max supply reached"); 

        require(msg.value >= wlPugCost * mintAmount, "Insufficient funds"); 

        whiteList[msg.sender] -= mintAmount;
        _safeMint(msg.sender, mintAmount);
    }

    function pugMint2(uint8 mintAmount) external payable {
        uint256 nextTokenId = _nextTokenId();
        ...
        if(isWhiteListActive && (whiteListAmount > 0)){
            if(mintAmount <= whiteListAmount){
                ...
                require(msg.value >= (wlPugCost * whiteListAmount) + (pugCost * (mintAmount - whiteListAmount)), "Insufficient funds"); 
    ...
    function setWlPugCost(uint256 _newWlPugCost) external onlyOwner {
        wlPugCost = _newWlPugCost;
    }
