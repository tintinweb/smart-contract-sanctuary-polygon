...
interface ILottery {
    ...
    function startLottery(uint256 _lotteryDuration) external;

    function drawWinner() external;

    function claimLottery(uint64 _lotterId, uint256 _riderId) external;
...
interface IRouterRider {
    ...
    function getRiderMetadata(
        uint256 riderId
    ) external view returns (RiderMetadata memory);

    function redeemRider() external;
...
import "./interface/IRouterRider.sol";
import "./interface/ILottery.sol";

/**
 * @title Ticket
 * @author  Router Protocol
 * @notice this contract will be use for submitting ticket to lottery contract on dst chain by using their rider NFT
 * this will be deployed on each supported chain
 */
...
{
    /** 
        it will be emitted when user submitted ticket from src chain to dst chain
    ...
    */
    ...
    /** 
        it will be emitted when user submitted ticket on same chain
        * @param sender sender who submitted address
        * @param riderId this it tokenId of nft from which user participated in lottery
    */
    event SubmittingTicketOnSameChain(
        address sender,
        uint256 riderId,
        uint64 lotteryId
    );
    ...
    /** 
        it will be emitted when ticket got submitted on dst chain

        * @param eventIdentifier event Id on src gateway
    ...
    */
    ...
    /**
     * @notice gateway contract address on this chain
     */
    address public gatewayContract;
    /**
     * @notice router rider contract on this chain
     */
    address public routerRiderAddress;
    /**
     * @notice gas limit for handleRequestFromSource on dst chain
     */
    uint64 public destGasLimit;
    /**
     * @notice gas limit for handleCrossTalkAck on this chain
     */
    uint64 public ackGasLimit;
    /**
     * @notice chain type for chain on which lottery contract deployed
     */
    uint64 public lotteryChainType;
    /**
     * @notice upper time limit for which handleCrossTalkAck on dst chain should be executed
     */
    ...
    /**
     * @notice chain type of this chain
     */
    uint64 public chainType;
    /**
     * @notice lottery contract address on lottery chain
     */
    bytes public lotteryContract;
    /**
     * @notice chain id for chain on which lottery contract deployed
     */
    string public lotteryChainId;

    /**
     * initializer for this contract
     * @param gatewayAddress gateway contract address on this chain
     * @param _routerRiderAddress router rider contract on this chain
    ...
    function initialize(
        ...
        routerRiderAddress = _routerRiderAddress;
        expiryTimestamp = _expiryTimestamp;
    }

    /**
     * this will be used to submit ticket on lottery contract
     * @param riderId token id of rider nft
     * @param ackGasPrice gas price for handleCrossTalkAck for this contract which will be executed from gateway contract on this chain
     * @param destGasPrice gas price for handleCrossTalkAck for this contract which will be executed from gateway contract on dst chain
     */
    function submitTicket(
        ...
        IRouterRider.RiderMetadata memory riderMetadata = IRouterRider(
            routerRiderAddress
        ).getRiderMetadata(riderId);
        require(riderMetadata.owner == msg.sender, "caller not owner of Rider");
        ...
        // lottery contract exist on this chain itself
        if (
            keccak256(abi.encodePacked(lotteryChainId)) ==
            keccak256(abi.encodePacked(chainId)) &&
            lotteryChainType == chainType
        ) {
            require(msg.value == 0, "fee not required");
            uint64 lotteryId = ILottery(toAddress(lotteryContract))
                .processTicket(riderId, msg.sender, riderMetadata.score);
            emit SubmittingTicketOnSameChain(msg.sender, riderId, lotteryId);
            return;
        }

        uint64 eventIdentifier = CrossTalkUtils
            .singleRequestWithAcknowledgement(
                gatewayContract,
        ...
        emit SubmittingTicket(eventIdentifier, msg.sender, riderId);
    }

    /**
     * @notice it will be use to map lottery contract on each chain to lottery contract present on lottery chain
     * @param _contractAddress contract address of lottery contract
     * @param _chainType chaintype of chain on which lottery contract deployed
     * @param _chainId chainid of chain on which lottery contract deployed
     */
    function enrollLotteryContract(
        ...
        // require(lotteryContract.length == 0, "already enrolled");
        lotteryContract = toBytes(_contractAddress);
        lotteryChainId = _chainId;
        lotteryChainType = _chainType;
    }
    ...
    /**
     * this function will be executed only if some contract send payload to this contract since ticket contract only for submitting ticket to lottery contract therefore this is not be called
     */
    ...
    /**
     * this function will be called from gateway contract of this chain only when ticket got submitted on dst chain successfully
     * @param eventIdentifier event id of ticket for which this ack is
     * @param execData this is array of abi encoded payload that got returned from handleRequestFromSource on dst chain since we are sending single request therefore length of this array will be one only
     */
    function handleCrossTalkAck(
        ...
        (
            ,
            ,
            /*string memory srcChainId*/ /*uint64 srcChainType*/ uint256 lowerBound,
            uint256 upperBound,
            uint256 lotteryId
        ) = abi.decode(_execData, (string, uint64, uint256, uint256, uint256));
    ...
    /**
     * util function to bytes to address
     */
    function toAddress(
        bytes memory _bytes
