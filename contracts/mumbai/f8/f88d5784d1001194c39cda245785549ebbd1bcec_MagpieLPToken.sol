    ...
    Pausable
{
    ...
    mapping(uint256 => MagpieLPTokenInfo) public magpieTokenMetadata;
    ...
    function updateTokenMetadata(uint256 _tokenId, MagpieLPTokenInfo calldata _lpTokenMetadata)
        external
        onlyMagpiePools
        whenNotPaused
    {
        require(_exists(_tokenId), "ERR__TOKEN_DOES_NOT_EXIST");
        magpieTokenMetadata[_tokenId] = _lpTokenMetadata;
    }
    ...
    {
        address tokenAddress = magpieTokenMetadata[tokenId].token;
        ...
        string memory svgData = magpieHelper.getTokenDetails(
            tokenId,
            magpieTokenMetadata[tokenId].suppliedLiquidity,
    ...
    function _beforeTokenTransfer(
        ...
        if (from != address(0) && to != address(0)) {
            magpieListManager.beforeTransferringLiquidity(
                from,
                to,
                magpieTokenMetadata[tokenId].token,
                magpieTokenMetadata[tokenId].suppliedLiquidity
            );
        }
    }
