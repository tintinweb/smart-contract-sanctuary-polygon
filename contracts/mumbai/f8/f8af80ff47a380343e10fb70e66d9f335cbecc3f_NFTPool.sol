...
import './NFTPoolBase.sol';

contract NFTPool is Initializable, NFTPoolBase, ERC1155HolderUpgradeable {

    bytes4 private constant InterfaceId_ERC721 = 0x80ac58cd;
    bytes4 private constant InterfaceId_ERC1155 = 0xd9b67a26;

    function initialize(IGameMaster _gameMaster, address _poolAdmin) public initializer {
        __NFTPoolBase_init(_gameMaster, _poolAdmin);
        __ERC1155Holder_init();
        _registerInterface(InterfaceId_ERC721);
        _registerInterface(InterfaceId_ERC1155);
    }

    function setRegisterInterfaces() public {
...
contract Weapons is Initializable, ERC721Upgradeable, AccessControlUpgradeable {
    ...
    uint256 public constant MAX_DURABILITY = 200;
    uint256 public constant SECONDS_PER_DURABILITY = 300; // 50 * 60
    ...
    function getFireRate(uint256 id) public view noFreshLookup(id) returns (uint16) {
    ...
    // RETURN STATS FROM NEW WEAPON STRUCT
    ...
    {
        (_power, _fireRate, _accuracy, _gunType, _element, _quality, holographic) = _getStats(id);
        _durability = getDurabilityPoints(id);
    }

    function _getStats(uint256 id)
        internal
        view
        returns (
            uint16 _power,
        ...
        )
    {
        ...
        return (w.power, w.fireRate, w.accuracy, w.gunType, w.element, w.quality, w.holographic);
    }
    ...
    function mint(
        ...
        uint8 element;
        uint256 selectedURI;
        uint8 holographicRoll;
        uint256 qualityRoll = 4001;

        if (!freeWeapon) {
            qualityRoll = RandomUtil.randomSeededMinMax(1, 10000, seed);
            holographicRoll = uint8((seed) % 1000);
        }

        uint256 gunTypeRoll = seed % 100;

        if (qualityRoll == 1) {
            quality = 4;
        } else if (qualityRoll < 100) {
            quality = 3;
        } else if (qualityRoll < 1000) {
            quality = 2;
        } else if (qualityRoll < 4000) {
            quality = 1;
        } else {
            quality = 0;
        }
        ...
        element = uint8(RandomUtil.randomSeededMinMax(0, 3, seed.add(now)));
        selectedURI = RandomUtil.randomSeededMinMax(firstIndex[gunType], lastIndex[gunType], seed);

        return mintWeaponWithQualityAndType(minter, quality, gunType, element, selectedURI, holographicRoll, seed);
    }
    ...
    {
        uint16 minPower = uint16(getPowerMin(quality, gunType));
        uint16 maxPower = uint16(getPowerMax(quality, gunType));
    ...
    function mintWeaponWithQualityAndType(
        ...
        uint8 holographicRoll,
        uint256 seed
    ) internal restricted returns (uint256) {
        require(quality < 5, 'Quality parameter too high! (max 4)');
        return performMintWeapon(minter, uint8(gunType), element, uint8(quality), selectedURI, holographicRoll, seed);
    }
    ...
    function performMintWeapon(
        ...
        (uint16 power, uint16 fireRate, uint8 accuracy) = getStatRolls(quality, gunType, seed);

        if (block.number != lastMintedBlock) firstMintedOfLastBlock = tokenID;
        lastMintedBlock = block.number;

        tokens.push(Weapon(power, fireRate, accuracy, gunType, element, quality, holographicRoll == 3));
        _mint(minter, tokenID);
        durabilityTimestamp[tokenID] = uint64(now.sub(getDurabilityMaxWait()));
        _setTokenURI(tokenID, string(abi.encodePacked(gunTypeURIs[gunType], selectedURI.toString())));
    ...
    function getPowerMin(uint256 quality, uint256 gunType) public pure returns (uint256) {
        uint256 commonMinDamageForSniper = 8000;
        uint256 commonMinDamageForSMG = 6000;
        uint256 commonMinDamageForShotgun = 8000;
        uint256 commonMinDamageForAssault = 7000;
        uint256 commonMinDamageForPistol = 5000;
        ...
        if (quality == 1 && gunType == 0) return commonMinDamageForSniper.mul(2).add(2000);
        if (quality == 1 && gunType == 1) return commonMinDamageForSMG.mul(2).add(4000);
        if (quality == 1 && gunType == 2) return commonMinDamageForShotgun.mul(2).add(2000);
        if (quality == 1 && gunType == 3) return commonMinDamageForAssault.mul(2).add(3000);
        if (quality == 1 && gunType == 4) return commonMinDamageForPistol.mul(2).add(1000);

        // Rare Guns Min Damage
        if (quality == 2 && gunType == 0) return commonMinDamageForSniper.mul(3).add(4000);
        if (quality == 2 && gunType == 1) return commonMinDamageForSMG.mul(4).add(2000);
        if (quality == 2 && gunType == 2) return commonMinDamageForShotgun.mul(3).add(4000);
        if (quality == 2 && gunType == 3) return commonMinDamageForAssault.mul(3).add(6000);
        if (quality == 2 && gunType == 4) return commonMinDamageForPistol.mul(4).add(1000);

        // Epic Guns Min Damage
        if (quality == 3 && gunType == 0) return commonMinDamageForSniper.mul(4).add(6000);
        if (quality == 3 && gunType == 1) return commonMinDamageForSMG.mul(6);
        if (quality == 3 && gunType == 2) return commonMinDamageForShotgun.mul(4).add(6000);
        if (quality == 3 && gunType == 3) return commonMinDamageForAssault.mul(5).add(2000);
        if (quality == 3 && gunType == 4) return commonMinDamageForPistol.mul(6).add(1000);

        // Legendary Guns Min Damage
        if (quality == 4 && gunType == 0) return commonMinDamageForSniper.mul(6);
        if (quality == 4 && gunType == 1) return commonMinDamageForSMG.mul(7).add(4000);
        if (quality == 4 && gunType == 2) return commonMinDamageForShotgun.mul(6);
        if (quality == 4 && gunType == 3) return commonMinDamageForAssault.mul(6).add(5000);
        if (quality == 4 && gunType == 4) return commonMinDamageForPistol.mul(8).add(1000);
    }

    function getPowerMax(uint256 quality, uint256 gunType) public pure returns (uint256) {
        uint256 commonMaxDamageForSniper = 10000;
        uint256 commonMaxDamageForSMG = 9000;
        uint256 commonMaxDamageForShotgun = 9000;
        uint256 commonMaxDamageForAssault = 9000;
        uint256 commonMaxDamageForPistol = 10000;

        // Common Guns Max Damage
        ...
        // Normal Guns Max Damage
        if (quality == 1 && gunType == 0) return commonMaxDamageForSniper.mul(2);
        if (quality == 1 && gunType == 1) return commonMaxDamageForSMG.mul(2).add(1000);
        if (quality == 1 && gunType == 2) return commonMaxDamageForShotgun.mul(2).add(1000);
        if (quality == 1 && gunType == 3) return commonMaxDamageForAssault.mul(2).add(1000);
        if (quality == 1 && gunType == 4) return commonMaxDamageForPistol.mul(2);

        // Rare Guns Max Damage
        if (quality == 2 && gunType == 0) return commonMaxDamageForSniper.mul(3);
        if (quality == 2 && gunType == 1) return commonMaxDamageForSMG.mul(3).add(2000);
        if (quality == 2 && gunType == 2) return commonMaxDamageForShotgun.mul(3).add(2000);
        if (quality == 2 && gunType == 3) return commonMaxDamageForAssault.mul(3).add(2000);
        if (quality == 2 && gunType == 4) return commonMaxDamageForPistol.mul(3);

        // Epic Guns Max Damage
        if (quality == 3 && gunType == 0) return commonMaxDamageForSniper.mul(4);
        if (quality == 3 && gunType == 1) return commonMaxDamageForSMG.mul(4).add(3000);
        if (quality == 3 && gunType == 2) return commonMaxDamageForShotgun.mul(4).add(3000);
        if (quality == 3 && gunType == 3) return commonMaxDamageForAssault.mul(4).add(3000);
        if (quality == 3 && gunType == 4) return commonMaxDamageForPistol.mul(4);

        // Legendary Guns Max Damage
        if (quality == 4 && gunType == 0) return commonMaxDamageForSniper.mul(5);
        if (quality == 4 && gunType == 1) return commonMaxDamageForSMG.mul(5).add(4000);
        if (quality == 4 && gunType == 2) return commonMaxDamageForShotgun.mul(5).add(4000);
        if (quality == 4 && gunType == 3) return commonMaxDamageForAssault.mul(5).add(4000);
        if (quality == 4 && gunType == 4) return commonMaxDamageForPistol.mul(5);
    }

    // Minimum shots per second
    function getFireRateMin(uint256 gunType, uint256 quality) public pure returns (uint16) {
        // SNIPER FIRE RATE
    ...
    function getFightData(
        ...
        return (getBonusPowerForFight(id, playerPower, wep.gunType, wep.element, wep.quality, charTrait, charClass));
    }
    ...
    function getBonusPowerForFight(
        ...
        // wepType: [Sniper, SMG, Shotgun, Assault, Pistol]
        // charClass: 0-4 [0(Sentry) 1(Spy) 2(Commando) 3(Vanguard) 4(Hunter)]

        uint24 totalPower = 0;
        if (wepType == 0 && charClass == 0) totalPower = basePower;
        if (wepType == 0 && charClass == 1)
            totalPower = uint24(basePower.add(uint24(ABDKMath64x64.divu(1, 25).mulu(basePower))));
        if (wepType == 0 && charClass == 2)
            totalPower = uint24(basePower.sub(uint24(ABDKMath64x64.divu(9,100).mulu(basePower))));
        if (wepType == 0 && charClass == 3)
            totalPower = uint24(basePower.sub(uint24(ABDKMath64x64.divu(1, 25).mulu(basePower))));
        if (wepType == 0 && charClass == 4)
            totalPower = uint24(basePower.add(uint24(ABDKMath64x64.divu(9,100).mulu(basePower))));

        if (wepType == 1 && charClass == 0)
            totalPower = uint24(basePower.sub(uint24(ABDKMath64x64.divu(1, 25).mulu(basePower))));
        if (wepType == 1 && charClass == 1)
            totalPower = uint24(basePower.sub(uint24(ABDKMath64x64.divu(9,100).mulu(basePower))));
        if (wepType == 1 && charClass == 2) totalPower = basePower;
        ...
        if (wepType == 2 && charClass == 0)
            totalPower = uint24(basePower.add(uint24(ABDKMath64x64.divu(9,100).mulu(basePower))));
        if (wepType == 2 && charClass == 1) totalPower = basePower;
        if (wepType == 2 && charClass == 2)
            totalPower = uint24(basePower.add(uint24(ABDKMath64x64.divu(1, 25).mulu(basePower))));
        if (wepType == 2 && charClass == 3)
            totalPower = uint24(basePower.sub(uint24(ABDKMath64x64.divu(9,100).mulu(basePower))));
        ...
        if (wepType == 3 && charClass == 1)
            totalPower = uint24(basePower.sub(uint24(ABDKMath64x64.divu(1, 25).mulu(basePower))));
        if (wepType == 3 && charClass == 2)
            totalPower = uint24(basePower.add(uint24(ABDKMath64x64.divu(9,100).mulu(basePower))));
        if (wepType == 3 && charClass == 3)
            totalPower = uint24(basePower.add(uint24(ABDKMath64x64.divu(1, 25).mulu(basePower))));
        if (wepType == 3 && charClass == 4) totalPower = basePower;

        if (wepType == 4 && charClass == 0)
            totalPower = uint24(basePower.add(uint24(ABDKMath64x64.divu(1, 25).mulu(basePower))));
        if (wepType == 4 && charClass == 1)
            totalPower = uint24(basePower.add(uint24(ABDKMath64x64.divu(9,100).mulu(basePower))));
        if (wepType == 4 && charClass == 2)
            totalPower = uint24(basePower.sub(uint24(ABDKMath64x64.divu(1, 25).mulu(basePower))));
        if (wepType == 4 && charClass == 3) totalPower = basePower;
        if (wepType == 4 && charClass == 4)
            totalPower = uint24(basePower.sub(uint24(ABDKMath64x64.divu(9,100).mulu(basePower))));

        if (charTrait == wepTrait && wepQuality == 0) totalPower.add((uint24(ABDKMath64x64.divu(11, 100).mulu(basePower))));
        if (charTrait == wepTrait && wepQuality == 1) totalPower.add((uint24(ABDKMath64x64.divu(3, 25).mulu(basePower))));
        if (charTrait == wepTrait && wepQuality == 2) totalPower.add((uint24(ABDKMath64x64.divu(13, 100).mulu(basePower))));
        if (charTrait == wepTrait && wepQuality == 3) totalPower.add((uint24(ABDKMath64x64.divu(7, 50).mulu(basePower))));
        if (charTrait == wepTrait && wepQuality == 4) totalPower.add((uint24(ABDKMath64x64.divu(3, 20).mulu(basePower))));

        return totalPower;
    }

    function getFightDataAndDrainDurability(uint256 id, uint8 drainAmount)
        public
        restricted
        noFreshLookup(id)
        returns (uint8, uint16)
    {
        ...
        uint64 drainTime = uint64(drainAmount * SECONDS_PER_DURABILITY);
        if (durabilityPoints >= MAX_DURABILITY) {
        ...
        return (wep.element, wep.fireRate);
    }
    ...
    function getDurabilityPointsFromTimestamp(uint64 timestamp) public view returns (uint8) {
        ...
        uint256 points = (now - timestamp) / SECONDS_PER_DURABILITY;
        if (points > MAX_DURABILITY) {
            points = MAX_DURABILITY;
        }
        return uint8(points);
    }

    function isDurabilityFull(uint256 id) public view returns (bool) {
        return getDurabilityPoints(id) >= MAX_DURABILITY;
    }

    function getDurabilityMaxWait() public pure returns (uint64) {
        return uint64(MAX_DURABILITY * SECONDS_PER_DURABILITY);
    }
    ...
    function setURIStartIndex(uint8 gunType, uint256 first) public restricted {
        firstIndex[gunType] = first;
    }

    function setURILastIndex(uint8 gunType, uint256 last) public restricted {
        lastIndex[gunType] = last;
    }
}
...
interface IGameMaster {
    ...
    function characters() external returns (Characters);
    function weapons() external returns (Weapons);
    function tagToken() external returns (IERC20Upgradeable);
    function rewardsManager() external returns (IRewardsManager);

    function fight(uint256 char, uint256 wep, Target calldata target, uint8 fightMultiplier) external;
...
contract Characters is Initializable, ERC721Upgradeable, AccessControlUpgradeable, ITransferCooldownable, ICharacter {
    ...
    */
    ...
    uint8 public holographicChance;
    ...
    function transferCooldownEnd(uint256 tokenId) public view override returns (uint256) {
        return lastTransferTimestamp[tokenId].add(TRANSFER_COOLDOWN);
    }

    function transferCooldownLeft(uint256 tokenId) public view override returns (uint256) {
        (bool success, uint256 secondsLeft) = lastTransferTimestamp[tokenId].trySub(
    ...
    function mint(
        ...
        uint8 class = uint8(RandomUtil.randomSeededMinMax(0, 4, seed));
        ...
        tokens.push(Character(ctype, xp, accuracy, level, element, class, pp, eliteId, holographicRoll == holographicChance));
    ...
    function customizeCharacter(
        ...
        if (customType == 0) {
            ...
            uint8 newClass = uint8(RandomUtil.randomSeededMinMax(0, 4, seed));
            char.class = newClass != currrentClass ? newClass : newClass == 4 ? 0 : uint8(newClass.add(1));

            uint256 selectedURI = RandomUtil.randomSeededMinMax(
                firstIndex[char.class],
                lastIndex[char.class],
                seed.add(now)
            );
        ...
        // Holographic chance on roll
        if (!char.holographic) {
            char.holographic = uint8((seed.add(now)) % 1000) == holographicChance;
        }
    ...
    function setCharacterName(
        ...
        characterNames[id] = CharacterName({firstName: firstN, familyName: lastN});
    ...
    function getPowerAtLevel(uint8 level) public pure returns (uint24) {
        ...
        return uint24(uint256(1000).add(level.mul(10)).mul(level.div(10).add(1)));
    }
    ...
    function gainXp(uint256 id, uint16 xp) public restricted {
        ...
        if (char.level < 255) {
            ...
            while (newXp >= requiredToLevel) {
                ...
                if (char.level < 255) requiredToLevel = experienceTable[char.level];
                else newXp = 0;
            }
    ...
    function getOperatorsCountPerRole(CharacterType ctype) public view returns (uint256 count) {
        require(
            ctype == CharacterType.Elite || ctype == CharacterType.Trainee || ctype == CharacterType.Citizen,
            'Invalid operator type!'
        );
        for (uint256 i = 0; i < tokens.length; i++) {
            if (tokens[i].ctype == ctype) {
                count = count.add(1);
            }
        }
    }
    ...
    function getFightDataAndDrainStamina(uint256 id, uint8 amount) public restricted returns (uint112) {
        ...
        if (staminaPoints >= maxStamina) {
            // if stamina full, we reset timestamp and drain from that
    ...
    function setHolographicChance(uint8 chance) public restricted {
        holographicChance = chance;
    }
    ...
    function setXp(uint256 id, uint256 xpNew) external restricted {
        tokens[id].xp = uint16(tokens[id].xp.add(uint256(xpNew)));
    }
...
interface IRewardsManager {
    ...
    function transfer(uint256 pid, uint256 amount, address to) external;
...
contract CraftingItems is Initializable, ERC1155Upgradeable, AccessControlUpgradeable {
    using ABDKMath64x64 for int128;
    using SafeMath for uint256;

    uint256 public constant PART = 0;
    uint256 public constant FIRE_CRYSTAL = 1;
    uint256 public constant LIGHTNING_CRYSTAL = 2;
    uint256 public constant WATER_CRYSTAL = 3;
    uint256 public constant EARTH_CRYSTAL = 4;

    bytes32 public constant CRAFTING_ITEMS_ADMIN = keccak256('CRAFTING_ITEMS_ADMIN');
    ...
    function _beforeTokenTransfer(
        ...
        require(hasRole(CRAFTING_ITEMS_ADMIN, operator), 'Crafting items admin role needed.');
    }

    function mintParts(address _address, uint256 amount) public restricted {
        _mint(_address, PART, amount, '');
    }

    function burnParts(address _address, uint256 amount) public restricted {
        _burn(_address, PART, amount);
    }

    function mintFireCrystals(address _address, uint256 amount) public restricted {
        _mint(_address, FIRE_CRYSTAL, amount, '');
    }

    function burnFireCrystals(address _address, uint256 amount) public restricted {
        _burn(_address, FIRE_CRYSTAL, amount);
    }

    function mintWaterCrystals(address _address, uint256 amount) public restricted {
        _mint(_address, WATER_CRYSTAL, amount, '');
    }

    function burnWaterCrystals(address _address, uint256 amount) public restricted {
        _burn(_address, WATER_CRYSTAL, amount);
    }

    function mintEarthCrystal(address _address, uint256 amount) public restricted {
        _mint(_address, EARTH_CRYSTAL, amount, '');
    }

    function burnEarthCrystals(address _address, uint256 amount) public restricted {
        _burn(_address, EARTH_CRYSTAL, amount);
    }

    function mintLightningCrystals(address _address, uint256 amount) public restricted {
        _mint(_address, LIGHTNING_CRYSTAL, amount, '');
    }

    function burnLightningCrystals(address _address, uint256 amount) public restricted {
        _burn(_address, LIGHTNING_CRYSTAL, amount);
    }

    function updateUri(string calldata newuri) external onlyAdmin {
        _setURI(newuri);
    }
    ...
    function _restricted() internal view {
        require(hasRole(CRAFTING_ITEMS_ADMIN, msg.sender), 'Crafting items admin role needed.');
    }

    function _onlyAdmin() internal view {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), 'Default admin role needed.');
    }
}
