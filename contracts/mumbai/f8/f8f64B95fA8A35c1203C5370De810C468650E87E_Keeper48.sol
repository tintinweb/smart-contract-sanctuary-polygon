...
contract Keeper48 is Ownable {
    ...
    function checkUpkeep4(
        ...
        return (k > 0, abi.encode(token0, token1, block.timestamp));
    }

    function checkUpkeep3(
        bytes calldata checkData
    ) external view returns (bool upkeepNeeded, bytes memory performData) {
        AggregatorInterface oracle;
    ...
    function checkUpkeep1(
        ...
        token0 = pairs[0][0];
        token1 = pairs[1][0];
        address oracleAddress = _factory.getPairsOracleAddress(token0, token1);
