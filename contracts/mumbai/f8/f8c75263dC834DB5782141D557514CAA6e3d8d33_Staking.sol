...
contract Staking is
    ...
    uint256 monthlyInterestRateBP;
    uint256 lockedDuration;
    uint256 unstakePenaltyDuration;
    uint256 unstakePenaltyRateBP;
    ...
    function depositClosed() external onlyDeposit {
        emit DepositClosed(msg.sender);
    }

    function depositHarvest(address _beneficiary, uint256 _amount) external onlyDeposit {
        token.transferFrom(harvestWallet, _beneficiary, _amount);
        emit DepositHarvest(msg.sender);
    }    
...
contract Deposit is ReentrancyGuard {
    ...
    function getCurrentTime() public view returns(uint256 _currentTime) {
        if (currentTime == 0) {
    ...
    constructor(
        ...
        amount = _amount;
        depositDate = getCurrentTime();
        unstakePenaltyDuration = _unstakePenaltyDuration;
        unstakePenaltyRateBP = _unstakePenaltyRateBP;
        lockedDuration = _lockedDuration;


        addInterestAllocation((_amount.mul(_monthlyInterestRateBP).div(BASIS_POINTS_10000).mul(3)), depositDate.add(MONTH.mul(3)));
        addInterestAllocation((_amount.mul(_monthlyInterestRateBP).div(BASIS_POINTS_10000)), depositDate.add(MONTH.mul(4)));
        addInterestAllocation((_amount.mul(_monthlyInterestRateBP).div(BASIS_POINTS_10000)), depositDate.add(MONTH.mul(5)));
        addInterestAllocation((_amount.mul(_monthlyInterestRateBP).div(BASIS_POINTS_10000)), depositDate.add(MONTH.mul(6)));
    }
    ...
    function closeDeposit() external nonReentrant {        
        require(
            lockedUntilDate() <= getCurrentTime(),
            "Staking deposit is still locked"
        );
        if (unstakePenaltyUntilDate() < getCurrentTime()) {
        ...
        stakingContract.depositClosed();
        closed = true;
    }

    function calculateHarvest() public view returns(uint256 _interest) {
        for(uint i = 0; i<interestPayDates.length; i++) {
            if (interestPayDates[i] <= getCurrentTime() && interestPayDates[i] > interestLastPaymentTime) {
    ...
    function harvest() external nonReentrant {
        ...
        if (interest > 0) {
            stakingContract.depositHarvest(beneficiary, interest);
            interestLastPaymentTime = getCurrentTime();            
        }
    }

    function canCloseDeposit() public view returns(bool _canClose) {
        return (lockedUntilDate() <= getCurrentTime());
    }
...
interface IStaking {
    ...
    function depositClosed() external;  
