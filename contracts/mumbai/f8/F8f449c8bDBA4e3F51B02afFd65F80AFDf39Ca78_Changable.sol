 ...
 */
abstract contract SimpleAbstract is AccessControl, ERC721B {
     ...
     * be updated)
     */
    function updateTreasury(address newTreasury) external onlyRole(_DAO_ROLE) {
...
contract Changable is ReentrancyGuard, SimpleAbstract {
    ...
    address public lamboAddress;
    // the sale price per token
    // uint256 public lamboCost = 500000000000000000; // 50 LAMBOs
    uint256 public lamboCost = 20000000000000000; // 2 LAMBOs
    ...
    constructor(
        INounsDescriptorMinimal _descriptor,
        INounsSeeder _seeder,
        address _lamboAddress
    ) SimpleAbstract(msg.sender, _descriptor, _seeder) {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(CURATOR_ROLE, msg.sender);
        lamboAddress = _lamboAddress;
    }

    function name() external pure returns (string memory) {
        return "Changable NFTs";
    }
