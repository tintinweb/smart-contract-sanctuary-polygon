...
contract TreasureChest is ERC721URIStorage, Ownable {
    ...
    function levelUp(uint256 tokenId) external payable {
        ...
        uint256 nextLevelUpPrice = levelUpPrice[nextLevel - 2];
        require(levelCount[nextLevel] < levelCap[nextLevel], "Next level is fully minted");
        require(msg.value == nextLevelUpPrice, "Payment must be exact");

        require(nextLevel <= MAX_LEVEL, "Max level is already reached");

        chest.level = nextLevel;

        levelCount[currentLevel]--;
        levelCount[nextLevel]++;

        emit ChestLeveledUp(tokenId, msg.sender, nextLevel, msg.value);
    }
