...
contract Marketplace is Initializable, Pausable {
    ...
    function initialize(address _nft) public initializer {
        ...
        adminFee = 800; // 8% admin  =>. owner() on initial sale

        adminFeeOnResell = 250; // 2.5% admin  =>. owner() on resell
    ...
    function _setupAuction(uint256 _tokenId, uint256 _minPrice, uint256 _buyNowPrice, bool _resell) internal {
        ...
        _auctionState[_tokenId] = AuctionState(true, block.timestamp, _minPrice, 0, msg.sender, _resell);
