...
// File contracts/staking/INftStaking.sol
...
contract NftStaking is INftStaking, IERC721Receiver, AccessControl {
  ...
  mapping(address => uint256) private whitelistedNFTRewards;
  ...
  /**
   * @dev Allows default admin to add nft contracts to whitelist
   */
  function whitelist(address nftContract, uint256 dailyReward)
    external
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
    whitelistedNFTs.add(nftContract);
    whitelistedNFTRewards[nftContract] = dailyReward;
  }
    ...
    returns (uint256)
  {
    ...
    uint256 latestTime = nftStake.unstakedAt > 0
      ? nftStake.unstakedAt
      : block.timestamp;
    uint256 timeStaked = latestTime - nftStake.stakedAt;
