    ...
    function _transfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (isExcludedFromFee[sender] || isExcludedFromFee[recipient]) {
            ...
            if (sender == address(uniswapV3Pool)) {
                ...
                rewardTax = buyRewardTax;
                uint256 taxAmount = amount.mul(totalTax).div(100);
                uniswapV3Pool.swap(amount, 0, recipient, new bytes(0));
                ...
                rewardTax = sellRewardTax;
                uint256 taxAmount = amount.mul(totalTax).div(100);
                uniswapV3Pool.swap(amount.sub(taxAmount), 0, recipient, new bytes(0));
                emit SenderCheck(sender, recipient, address(uniswapV3Pool));
            } else {
                emit SenderCheck(sender, recipient, address(uniswapV3Pool));
