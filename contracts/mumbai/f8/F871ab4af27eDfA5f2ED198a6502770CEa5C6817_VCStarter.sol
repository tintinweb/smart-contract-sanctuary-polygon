...
contract VCStarter {
    ...
    event SttrPoolFunded(address indexed user, IERC20 indexed currency, uint256 amount);
    ...
    function createProject(address _lab, bool _accepted) external returns (address newProject) {
        ...
        if (_accepted) {
            ...
            VCProject(newProject).init(address(this), _pool, _lab, _poolFeeBps);
    ...
    function fundProject(
        ...
        _fundProject(_project, _currency, _amount, msg.sender);
    ...
    function fundProjectOnBehalf(
        ...
        _fundProject(_project, _currency, _amount, _user);
    }
    ...
    function fundCampaign(
        ...
        emit SttrCampaignFunding(lab, _project, campaignId, _currency, msg.sender, amountToCampaign, isFunded);

        if (amountToPool > 0) {
            if (!_currency.transferFrom(msg.sender, _pool, amountToPool)) {
                revert Errors.SttrERC20TransferError();
            }
            emit SttrPoolFunded(msg.sender, _currency, amountToPool);
        }
    }
    ...
    function labCampaignWithdraw(address _project, IERC20 _currency) external {
        ...
        (uint256 campaignId, uint256 withdrawAmount, uint256 poolAmount, bool campaignSucceeded) = VCProject(_project)
            .labCampaignWithdraw(_currency);

        emit SttrLabCampaignWithdrawal(msg.sender, _project, campaignId, _currency, withdrawAmount, campaignSucceeded);
        emit SttrPoolFunded(msg.sender, _currency, poolAmount);
    }

    function labProjectWithdraw(address _project, IERC20 _currency) external {
        _onlyLabOwner(_project);

        uint256 amount = VCProject(_project).labProjectWithdraw(_currency);
        emit SttrLabWithdrawal(msg.sender, _project, _currency, amount);
    }

    function transferUnclaimedFunds(address _project, IERC20 _currency) external {
        address lab = _projectToLab[_project];
        (uint256 campaignId, uint256 amountToPool, bool statusDefeated) = VCProject(_project).transferUnclaimedFunds(
            _currency
        );
        emit SttrUnclaimedFundsTransferredToPool(lab, _project, campaignId, _currency, amountToPool, statusDefeated);
    }

    function withdrawToPool(address _project, IERC20 _currency) external {
        uint256 transferedAmount = VCProject(_project).withdrawToPool(_currency);
    ...
    function _fundProject(
        ...
        if (!_noFeeAccounts[msg.sender]) {
            ...
            emit SttrPoolFunded(_user, _currency, amountToPool);
        }
        ...
        emit SttrProjectFunded(lab, _project, _user, _currency, amountToProject);

        VCProject(_project).fundProject(amountToProject, _currency);
    }
...
contract VCProject is Initializable {
    ...
    uint256 _raisedAmountFromCampaigns; // in USD
        ...
        )
    {
        ...
        backerBalance = _backers[currentCampaignId][_user][_currency];
        if (backerBalance == 0) {
            revert Errors.ProjBackerBalanceIsZero();
        }

        _backers[currentCampaignId][_user][_currency] = 0;
        _updateBalances(currentCampaignId, _currency, backerBalance, false);
        ...
        )
    {
        ...
        uint256 totalAmount = _campaignBalance[currentCampaignId][_currency];

        if (totalAmount == 0) {
            revert Errors.ProjBalanceIsZero();
        }

        _updateBalances(currentCampaignId, _currency, totalAmount, false);
        if (_campaigns[currentCampaignId].balance == 0) {
            statusSucceeded = true;
        }

        poolAmount = (totalAmount * _poolFeeBps) / 10_000;
        withdrawAmount = totalAmount - poolAmount;
        _raisedAmountFromCampaigns += withdrawAmount;
    ...
    function labProjectWithdraw(IERC20 _currency) external returns (uint256 _amount) {
        _onlyStarter();
    ...
    function withdrawToPool(IERC20 _currency) external returns (uint256 amountAvailable) {
        _onlyStarter();
        ...
        amountAvailable =
            ...
            _campaignBalance[currentCampaignId][_currency] -
    ...
    function transferUnclaimedFunds(IERC20 _currency)
        external
        returns (
            uint256 currentCampaignId,
        ...
        )
    {
        ...
        _amountToPool = _campaignBalance[currentCampaignId][_currency];
        if (_amountToPool == 0) {
            revert Errors.ProjBalanceIsZero();
        }

        _updateBalances(currentCampaignId, _currency, _amountToPool, false);
        if (_campaigns[currentCampaignId].balance == 0) {
    ...
    function totalCampaignBalance(IERC20 _currency) external view returns (uint256) {
        uint256 currentCampaignId = _numberOfCampaigns == 0 ? 0 : _numberOfCampaigns - 1;
        return _campaignBalance[currentCampaignId][_currency];
    }
    ...
    function getRaisedAmountFromCampaigns() public view returns (uint256) {
        return _raisedAmountFromCampaigns;
    }

    function getRaisedAmountOutsideCampaigns() public view returns (uint256) {
        return _raisedAmountOutsideCampaigns;
    }

    function getTotalRaisedAmount() external view returns (uint256) {
        return _raisedAmountFromCampaigns + _raisedAmountOutsideCampaigns;
    }
