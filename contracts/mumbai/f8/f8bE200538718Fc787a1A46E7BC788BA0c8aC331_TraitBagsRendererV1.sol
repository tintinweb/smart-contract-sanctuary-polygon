...
contract TraitBagsRendererV1 is ITraitBags {
    ...
    function _splitLine(bytes memory line, uint256 lineLength)
        internal
        pure
        returns (string memory, string memory)
    {
        // starting at the 70th position (half of max 140 chars per trait)
        // move backwards to find the last space char, with its position stored in lineBreakChar
        uint256 lineBreakChar;
        for (uint256 i = 70; i > 0; i--) {
            if (line[i] == " ") {
                lineBreakChar = i;
                break;
            }
        ...
        returns (string memory)
    {
        ...
        for (uint256 i = 0; i < traits.length; i++) {
            ...
            if (traitLength > 70) {
                (lineOne, lineTwo) = _splitLine(traitBytes, traitLength);
                traitsStack = string(
                    abi.encodePacked(
                        traitsStack,
    ...
    function tokenURI(
        ...
        string memory imageSVG = string(
            abi.encodePacked(
                ...
                category,
                "</text><text x='90%' y='10%' font-size='13px' text-anchor='end'>",
                world,
                traitsStack,
                "</text>"
            )
