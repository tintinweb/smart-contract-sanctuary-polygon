...
contract Bridge is AccessControl, IBridge {
    ...
    function swap(
        ...
        _call(_params.dstChainId, TYPE_SWAP_REMOTE, _params.refundAddress, payload);
    }
    ...
    function sendVouchers(
        ...
        _call(_chainId, TYPE_ADD_LIQUIDITY, _refundAddress, payload);
    }
    ...
    function quoteLayerZeroFee(
        ...
        uint16 version = 1;
        bytes memory adapterParams = abi.encodePacked(version, gasLookup[_chainId][_functionType]);
    ...
    function _call(uint16 _chainId, uint8 _type, address payable _refundAddress, bytes memory _payload) internal {
        uint16 version = 1;
        bytes memory adapterParams = abi.encodePacked(_type, version, gasLookup[_chainId][_type]);
...
interface IAssetRouter {
    ...
    function receiveVouchers(uint16 _srcPoolId, uint16 _dstChainId, uint16 _dstPoolId, CreditObj memory _c) external;
