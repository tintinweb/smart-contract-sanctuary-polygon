...
contract MerkleDistributor is Ownable {
    ...
    event Claimed(address claimant, uint256 tranche, uint256 claimAmount, uint256 vaultAmount);
    ...
    // Mapping of week number to addresses that have claimed for that week
    ...
    function claimWeek(address _claimant, uint256 _tranche, uint256 _balance, uint256 _claimAmount, bytes32[] memory _merkleProof) public {
        require(_claimAmount <= _balance, "Claim amount exceeds balance");

        _claimWeek(_claimant, _tranche, _balance, _merkleProof);

        uint256 vaultAmount = _balance - _claimAmount;
        _disburse(_claimant, _claimAmount, vaultAmount);
    }

    function _claimWeek(address _claimant, uint256 _tranche, uint256 _balance, bytes32[] memory _merkleProof) private {
        require(_tranche < numTranches, "Cannot claim for future week.");
        require(!claimed[_tranche][_claimant], "Claimant has already claimed for this week.");
        require(_verifyClaim(_claimant, _tranche, _balance, _merkleProof), "Incorrect merkle proof.");

        claimed[_tranche][_claimant] = true;

        emit Claimed(_claimant, _tranche, _balance, _balance - _balance);
    }

    function _verifyClaim(address _claimant, uint256 _tranche, uint256 _balance, bytes32[] memory _merkleProof) private view returns (bool valid) {
    ...
    function _disburse(address _claimant, uint256 _claimAmount, uint256 _vaultAmount) private {
        if (_claimAmount > 0) {
    ...
    function depositToVault(uint256 _tranche, uint256 _balance, bytes32[] memory _merkleProof) public {
        _claimWeek(msg.sender, _tranche, _balance, _merkleProof);
        token.safeApprove(address(vault), _balance);
        vault.depositToVault(msg.sender, _balance);

        emit Claimed(msg.sender, _tranche, 0, _balance);
    }

    function checkUnclaimed(address _claimant) public view returns (uint256[] memory unclaimedTranches) {
        uint256 count = 0;
        for (uint256 i = 0; i < numTranches; i++) {
            if (!claimed[i][_claimant]) {
                count++;
            }
        }

        unclaimedTranches = new uint256[](count);
        uint256 index = 0;
        for (uint256 i = 0; i < numTranches; i++) {
            if (!claimed[i][_claimant]) {
                unclaimedTranches[index] = i;
                index++;
            }
        }
    }
...
contract Vault is Ownable {
    ...
    function startNewSeason(uint256 _end) external onlyOwner {
        require(distributed[seasons.length - 1], "Distribute first!");
