...
contract LinearVestingFarm is OwnableUpgradeable, ReentrancyGuardUpgradeable{
    ...
    {
        uint256 _earned = earned(user);
        uint256 rewardAmount = _earned.sub(totalUserPayouts[user]);
        ...
        return(
            rewardAmount + totalUserPayouts[user],
            totalUserRewards[user].sub(rewardAmount + totalUserPayouts[user]),
            rewardAmount + claimAmountFromLocked,
            burnAmount
        );
    }
        ...
        nonReentrant
    {
        ...
        uint256 rewardAmount = _withdrawCalculation();
        ...
        if (rewardAmount > 0) {
            ...
            uint256 claimAmountFromLocked = totalLeftLockedForUser
                .mul(earlyClaimAvailablePercent).div(100);
            uint256 burnAmount = totalLeftLockedForUser.mul(burnPercent).div(100);
            rewardAmount = rewardAmount.add(claimAmountFromLocked);
            totalUserPayouts[address(msg.sender)] = totalUserRewards[address(msg.sender)];

            totalRewardsPendingBalance = totalRewardsPendingBalance
                .sub(rewardAmount + burnAmount);
            ...
            // Should totalWithdrawn follow how much was taken from farm or,
            // what was transferred to users wallet
