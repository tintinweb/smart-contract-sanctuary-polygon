...
contract AuctionPureV2 is Initializable {
    ...
    modifier checkStateOfAution(bytes memory _auctionId) {
        ...
        if (_auctionInstance.state != AuctionStructure.StateOfAution.ACTIVE)
            revert InvalidState();
        _;
    }
    ...
    function listingNFTAuction(
        ...
        if (
            stanFund.get(_paramListing.maker) < _feeSystem.stanFee &&
            !_isWhiteList
    ...
    function cancelListingAuction(
        ...
        if (
            stanFund.get(listingInstance.Owner) < _feeSystem.stanFee &&
            !_isWhiteList
        ) revert InvalidBalance();
    ...
    function _reOffer(
        ...
        if ((infoInstance.amount >= _amount)) revert ReOfferFailed();

        return _amount - infoInstance.amount;
    }
    ...
    {
        if (
            stanFund.get(_maker) < (_amount + _feeSystem.stanFee) &&
            !_isWhiteList
        ) revert InvalidBalance();
        AuctionStructure.Listing memory listingInstance = listing
            .getInforListing(_auctionId);

        if (listingInstance.ExpirationTime < block.timestamp)
            revert InvalidTimestamp();
        if (listingInstance.Amount > _amount && !_isWhiteList)
            revert InvalidOffer();
        if (!_isWhiteList)
            stanFund.set(
                (_reOffer(_subOfferId, _auctionId, _amount) +
                    _feeSystem.stanFee),
                AuctionStructure.Operator.MINUS,
                _maker
            );
    ...
    function acceptOfferAuction(
        ...
        if (block.timestamp >= listingInstance.ExpirationTime)
            revert InvalidTimestamp();
    ...
    function cancelOfferAuction(
        ...
        if (!_isWhiteList && _sender != maker) revert InvalidOwner();
        if (!_isWhiteList)
    ...
    function _processFinishAuction(
        ...
        if (infoInstance.state != AuctionStructure.StateOfOffer.ACTIVE)
            revert InvalidState();
        ...
        returns (AuctionStructure.Currency, uint256)
    {
        ...
        if (listingInstance.state != AuctionStructure.StateOfListing.ACTIVE)
            revert InvalidState();
