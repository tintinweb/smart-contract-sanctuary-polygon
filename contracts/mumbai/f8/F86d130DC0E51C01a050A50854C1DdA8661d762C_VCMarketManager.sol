...
contract VCMarketManager is FeeManager, AccessControl {
    ...
    function claimRoyalties(address _to, uint256 _poolAmount) external {
        ...
        claimAmount -= _poolAmount;
        if (claimAmount > 0 && !currency.transfer(_to, claimAmount)) {
        ...
        emit RoyaltiesClaimed(msg.sender, _to, claimAmount, _poolAmount);
    }
    ...
    function setMinPoolFeeBps(uint96 _minPoolFeeBps) external onlyRole(DEFAULT_ADMIN_ROLE) {
        VCMarketplaceBase(marketplaceFixedPriceERC1155).setMinPoolFeeBps(_minPoolFeeBps);
        VCMarketplaceBase(marketplaceFixedPriceERC721).setMinPoolFeeBps(_minPoolFeeBps);
        VCMarketplaceBase(marketplaceAuctionERC721).setMinPoolFeeBps(_minPoolFeeBps);
    }
...
abstract contract VCMarketplaceBase is FeeBeneficiary, Pausable {
    ...
    function _settle(
        ...
        _transferFee(_tokenId, _seller, _highestBidder, _highestBid, _starterFee, _poolFee, _marketFee);
    ...
    function _purchase(
        ...
        _transferFee(_tokenId, _seller, msg.sender, _listPrice, _starterFee, _poolFee, _marketFee);
...
contract FeeBeneficiary is FeeManager, AccessControl {
    ...
    function setMinPoolFeeBps(uint96 _minPoolFeeBps) external onlyManager {
        _setMinPoolFeeBps(_minPoolFeeBps);
    }
    ...
    function _transferFee(
        ...
        if (_starterFee > 0) {
            ...
            _poolFee += _fundProjects(_seller, feesData, _price);
        }
        if (_poolFee > 0) {
            ...
            // IVCPool(pool).fundOnBehalf(_buyer, currency, _mktFee, false);
            // IVCPool(pool).fundOnBehalf(_seller, currency, _poolFee, false);
        }
    }
