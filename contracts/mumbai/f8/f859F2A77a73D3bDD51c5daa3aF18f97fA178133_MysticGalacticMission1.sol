...
contract MysticGalacticMission1 is ERC721, Ownable, Pausable, AccessControl , ReentrancyGuard  {
    ...
    uint16  public  constant priceDecimals = 8;
    ...
    // Sub Contracts
    IPriceHandler private priceHandler;
    address private priceHandlerAddr;
    ISupplyManager private supplyManager;
    address private supplyManagerAddr;
     ...
     */
    constructor( uint16 _initialSupply, uint16 _initialReservedSupply, uint16 _supplyPerDay, bytes32 _callerKey ) ERC721("Mystic Galactic Mission 1", "MYGM1") {
    ...
    /**
     * @dev Set the logic for pricing
     * @param _priceHandlerAddr The price handler
     */
    function setPriceHandler (address _priceHandlerAddr) external onlyAdmin {
        // Set the handler
        priceHandlerAddr = _priceHandlerAddr;
        priceHandler  = IPriceHandler(_priceHandlerAddr);

    }

    /**
     * @dev Set the additional logic handlers
     * @param _supplyManagerAddr The price handler
     */
    function setSupplyManager (address _supplyManagerAddr) external onlyAdmin {
        // Set the handler
        supplyManagerAddr = _supplyManagerAddr;
        supplyManager = ISupplyManager(_supplyManagerAddr);
        supplyManager.initializeSupply();
    }
    ...
    function mintNFT(uint16 bottles) public payable nonReentrant whenNotPaused {
        (uint256 totalAmount, int256 refundAmount ) = supplyManager.mintFromPublicSupply(msg.sender, bottles, msg.value, priceHandler.price());
    ...
    /**
     * @dev mint from the owner's reserved supply
     */
    function mintNFTFromReservedSupply(uint256 bottles) public onlyOwner nonReentrant whenNotPaused {
        supplyManager.mintFromReservedSupply(msg.sender, bottles);
    }


    function _safeMinter(address minter, uint256 tokenId, uint256 tokenPrice, bytes32 _callerKey) external  {
        require ((msg.sender == supplyManagerAddr) && (_callerKey == callerKey), "Unauthorized");
    ...
    /**
     *  @dev Update Metadata -  should be replaced by ENS but we probably still need to emit the event
     *  @param _baseURI new base URI for metadata change
     */
    function updateMetadata( string memory _baseURI ) external onlyAdmin {
        baseURI = _baseURI;
        uint16 _supplyUsed = supplyManager.supplyUsed();
        if ( _supplyUsed != 0 ) {
    ...
    /**
     * @dev Refunds the specified minters of the specified tokens.
     * This function transfers the refund amount to each minter, burns their NFT, and emits a RefundProcessed event.
     * @param _tokenIds The number of refunds to send.
     */
    function refundMinters(uint256[] calldata _tokenIds, bool forever) external payable onlyAdmin {
        for ( uint16 i = 0; i < _tokenIds.length; i++ ) {
            ...
            emit RefundProcessed(minter, msg.value);
        }
    }
    ...
    function burn(uint256 tokenId) public onlyAdmin {
        _burn(tokenId);
        supplyManager.tokenBurnt(tokenId);
    }
    ...
    function validateKey(bytes32 _callerKey) external view {
       require((msg.sender == priceHandlerAddr || msg.sender == supplyManagerAddr) && callerKey == _callerKey, "Contract caller is not authorized");
    }
    ...
    function setPrice(uint256 _usdPrice) external onlyAdmin {
        priceHandler.setPrice(_usdPrice);
    ...
    function setPriceFeed( address _priceFeedAddress ) external onlyAdmin {
        priceHandler.setPriceFeed(_priceFeedAddress);
    }

    /**
     * @dev Returns the current price for minting a token in MATIC.
     * @return The current price in Matic with 8 decimals.
     */
...
interface ISupplyManager {
    ...
    function mintFromPublicSupply( address minter, uint16 volume, uint256 amountSent, uint256 currentPrice ) external returns ( uint256 totalPrice, int256 refundAmount);
