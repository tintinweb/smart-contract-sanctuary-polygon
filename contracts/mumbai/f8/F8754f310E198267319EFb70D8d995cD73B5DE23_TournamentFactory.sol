...
function removeLosers(uint256[] memory teamIds) public onlyOwner {
  ...
  function modiftyTeamAfterInscription(uint256 teamIndex, uint256 gotchiLeader, uint256[10] memory gotchiFormation) external {
    ...
    require(teamIdToTeam[teamIndex].owner == msg.sender, "Msg.sender not owner of the team");
    require(participants[(msg.sender)] == true, "Participants didn't enter or is eliminated");
    require(checkGotchiFormationValidity(gotchiFormation) == true, "Gotchi formation not valid");
    require(isGotchiLeaderInTeam(teamIdToTeam[teamIndex].gotchiTeam, gotchiLeader) == true, "Gotchi leader is not in the team");
    ...
    Team[] storage teams = participantToTeams[teamIdToTeam[teamIndex].owner];
    for (uint256 i = 0; i < teams.length; i++) {
...
function getAllTeamIdStillInCompetition() public view returns (uint256[] memory) {
    uint256 count = 0;
    // Compter le nombre d'équipes encore en compétition
    for (uint256 i = 0; i < participantTeams.length; i++) {
    ...
    // Créer un tableau temporaire pour stocker les teamIds des équipes encore en compétition
    uint256[] memory teamIdsStillInCompetition = new uint256[](count);
    uint256 index = 0;

    // Ajouter les teamIds des équipes encore en compétition au tableau temporaire
    for (uint256 i = 0; i < participantTeams.length; i++) {
        if (teamIdStillInCompetition[participantTeams[i].teamId]) {
    ...
    function getNumberOfTeamsStillInCompetition() public view returns (uint256) {
