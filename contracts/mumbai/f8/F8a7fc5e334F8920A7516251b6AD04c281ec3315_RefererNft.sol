    ...
    function rewardDistribution(address user, uint _reward) public nonReentrant onlyRole(REWARD_ROLE) returns(uint amount, uint leftAmount) {
        ...
        for(uint i = 1; i <= 3; i++){
            if(userRefferer != address(0)){
                users[userRefferer].totalReferralReward = (_reward * refferrerPercentage[1])/totalRewardPercentage;       
                leftAmount -= (_reward * refferrerPercentage[1])/totalRewardPercentage;
 ...
 */
    function claimReward() public nonReentrant{
        require(users[msg.sender].lockPeriod > block.timestamp, "ReferralNft: lockPeriod time of user is less than the current time");
...
@param user user that refers
 */
    function setRefferer(address referrer, address user) public _isAdmin {
        require(referrer != user, "ReferralNft: User adress not equals to referrer address");
        require(users[user].refferer ==  address(0), "ReferralNft: address user is null");
        emit setterForRefferer("ReferralNft",address(this),users[user].refferer ,referrer, user,block.number,block.timestamp);
        users[user].refferer = referrer;
    }
