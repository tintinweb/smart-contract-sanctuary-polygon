...
contract PolygonBSG {
        ...
        for (uint256 i = 0; i < orderInfos[_user].length; i++) {
            ...
            if (block.timestamp > order.unfreeze && order.isUnfreezed == false && _amount >= order.amount) {
                ...
                if (isStopLoss30ofATH || isStopLoss50ofATH) {
                    ...
                    emit Debug(_user, 6, user.totalRevenue);
        ...
        if(day1kDepositCount > 0){
            for(uint256 i = day1kDepositCount; i > 0; i--){
                ...
                if(userAddr != address(0)){
                    ...
                    emit Debug(userAddr, 8, userInfo[userAddr].totalRevenue);
                }
            }
            more1kIncomePool = 0;
        }
    ...
    function _updateReward(address _user, uint256 _amount) private {
        ...
        if (upline != address(0) && user.teamNum >= 2) {
            ...
            emit Debug(_user, 11, user.directBonusCount);
            upRewards.directs = upRewards.directs.add(reward);
            emit Debug(_user, 12, upRewards.directs);
            userInfo[upline].totalRevenue = userInfo[upline].totalRevenue.add(reward);
            emit Debug(_user, 13, userInfo[upline].totalRevenue);
        }
    }
    ...
    function _releaseUpRewards(address _user, uint256 _amount) private {
        ...
        if (upline != address(0)) {
            ...
            emit Debug(_user, 14, upRewards.levelReleased);
            emit Debug(_user, 15, userInfo[upline].totalRevenue);
