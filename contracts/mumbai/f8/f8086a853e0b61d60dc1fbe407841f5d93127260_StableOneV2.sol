...
// File: contracts/StableOneV2.sol
...
contract StableOneV2 is ReentrancyGuard, Ownable {
    ...
    function _invest(
        ...
        _investor.totalDeposit = investors[_addr].totalDeposit.add(
            _amount
        );
        totalInvested = totalInvested.add(_amount);
        activeInvested = activeInvested.add(_amount);

        uint256 planCount = _investor.investmentCount;

        _investor.investments[planCount].investmentDate =  block.timestamp;
        _investor.investments[planCount].investment = _amount;
        _investor.investments[planCount].dividendCalculatedDate = block.timestamp;
        _investor.investments[planCount].lastWithdrawalDate = block.timestamp;
        _investor.investments[planCount].isExpired = false;
        _investor.investments[planCount].dividends = 0;
        _investor.investments[planCount].withdrawn = 0;
    ...
    function _sendRewardOnInvestment(uint256 _amount) private {
        ...
        if(primaryBenificiary!=secondaryBenificiary){
    ...
    function calculateDividendsAndautoReinvest(address addr) public {
        Investor storage investor = investors[addr];
        uint256 totalAmountToReinvest=0;
        uint256 startingPoint = investor.expiredCount;
        for (uint256 i = startingPoint; i < investor.investmentCount; i++) {
            ...
            totalAmountToReinvest = totalAmountToReinvest.add(amountToReinvest);
        ...
        )
    {
        ...
        bool[] memory isExpireds = new bool[](investor.investmentCount);
        uint256 startingPoint = investor.expiredCount;
        for (uint256 i = startingPoint; i < investor.investmentCount; i++) {
            require(
    ...
    function withdraw() public nonReentrant {
        ...
        uint256 dividendAmount = 0;
        // Investment[] memory updatedInvestments;
        uint256 startingPoint = investor.expiredCount;
        for (uint256 i = startingPoint; i < investor.investmentCount; i++) {
            console.log("i is",i);
            if (investor.investments[i].isExpired) {
            ...
            if (calculatedDate >= endTime) {
                ...
                investor.expiredCount = investor.expiredCount.add(1);
            }
            ...
            console.log("Dividends is",amount);
            investor.investments[i].dividends = amount;
            dividendAmount = dividendAmount.add(amount); 
            console.log("dividendAmount is",dividendAmount);
            uint256 numberOfDays = calculatedDate.subz(
                investor.investments[i].dividendCalculatedDate
            ) / TIME_STEP_DRAW;
            ...
            _amountToReinvest = _amountToReinvest.add(amountToReinvest);
            console.log("_amountToReinvest is",i,dividendAmount);
            investor.investments[i].dividends = investor
                .investments[i]
                .dividends
                .subz(amountToReinvest);
        ...
        // if (_reinvest(addr, _amountToReinvest)) {
        //     emit OnInvest(addr, _amountToReinvest);
        // }

        // calculateDividendsAndautoReinvest(msg.sender);   END
        ...
        uint256 reinvestAmount = dividendAmount
            .mul(ON_WITHDRAW_AUTO_REINTEREST_RATE)
            .div(1000);

        uint256 totalToReinvest = _amountToReinvest.add(reinvestAmount);
        console.log("totalToReinvest is",totalToReinvest);
        
        _reinvest(msg.sender, totalToReinvest);
        
        uint256 remainingAmount = dividendAmount.subz(reinvestAmount);
        console.log("remainingAmount is",remainingAmount);
        // Withdrawal date save

        totalWithdrawal = totalWithdrawal.add(remainingAmount);
        console.log("totalWithdrawal is",totalWithdrawal);
        investors[msg.sender].totalWithdraw =investors[msg.sender].totalWithdraw.add(remainingAmount);
        for (uint256 i = 0; i < investors[msg.sender].investmentCount; i++) {
