...
interface QuestLike {
    ...
    function doQuestByAdmin(uint256[] memory doges, address[] memory owners, uint256[] memory scores, uint8 groupIndex, uint256 combatId) external;
}
...
contract BattlePoly {
    ...
    function combatSingle(uint256 dogeId) external {
        ...
        if(lbPoints_ > 0) {
            ...
            quest.doQuestByAdmin(doges_, owners_, scores_, _getGroupIndex(doge_.level), _currentCombatId);
        }
