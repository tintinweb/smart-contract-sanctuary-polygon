...
contract BridgeBank is Ownable, ReentrancyGuardUpgradeable {
    ...
    uint256 private lockBurnNonce = 0;
    ...
    function refund(
        ...
        // Check if it is ETH
        require(
            IERC20(stableToken).balanceOf(address(this)) >= lockCompleted[_nonce].amountStableToken,
            "Insufficient ERC20 balance."
        );
        ...
        if (_tempTokenSource == address(0)) {
            ...
            payable(lockCompleted[_nonce].sender).transfer(amountStableToRefund);
        } else {
            IERC20(_tempTokenSource).safeTransfer(lockCompleted[_nonce].sender, amountStableToRefund);
        }
