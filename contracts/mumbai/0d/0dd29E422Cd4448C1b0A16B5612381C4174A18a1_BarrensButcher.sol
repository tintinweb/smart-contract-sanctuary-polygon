...
// T1 - T4: OK
contract BarrensButcher is Ownable {
    ...
    // V1 - V5: OK
    address public immutable meatlocker;
    // V1 - V5: OK
    ...
    modifier onlyEOA() {
        ...
        require(msg.sender == tx.origin, "Butcher: must use EOA");
        _;
    }

    // F1 - F10: OK
    // F3: _convert is separate to save gas by only checking the 'onlyEOA' modifier once in case of convertMultiple
    // F6: There is an exploit to add lots of GovernanceTokens to the meatlocker, run convert, then remove the GovernanceTokens again.
    //     As the size of the meatlocker has grown, this requires large amounts of funds and isn't super profitable anymore
    ...
    function _convert(address token0, address token1) internal {
        ...
        require(address(pair) != address(0), "Butcher: Invalid pair");
        // balanceOf: S1 - S4: OK
        // transfer: X1 - X5: OK
    ...
    function _convertStep(
        ...
        if (token0 == token1) {
            ...
            if (token0 == govToken) {
                IERC20(govToken).safeTransfer(meatlocker, amount);
                govTokenOut = amount;
            } else if (token0 == weth) {
            ...
            // eg. GovToken - ETH
            IERC20(govToken).safeTransfer(meatlocker, amount0);
            govTokenOut = _toGovToken(token1, amount1).add(amount0);
        } else if (token1 == govToken) {
            // eg. USDT - GovToken
            IERC20(govToken).safeTransfer(meatlocker, amount1);
    ...
    function _swap(
        ...
        require(address(pair) != address(0), "Butcher: Cannot convert");
    ...
    {
        // X1 - X5: OK
        amountOut = _swap(token, govToken, amountIn, meatlocker);
    }
}
