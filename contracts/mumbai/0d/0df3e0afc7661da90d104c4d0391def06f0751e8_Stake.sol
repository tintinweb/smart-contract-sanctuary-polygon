...
contract Stake is Ownable {
    ...
    struct RewardOffPeriod {
        uint256 start;
        uint256 end;
    }
     
    uint256 private lockperiod = 180 seconds;  // days
    uint256 private claimTime  = 24 seconds;   // hours
    
    mapping (uint256 => StakeStruct) private stakeDetails;
    mapping (address => UserStruct)  private userDetails;

    RewardOffPeriod[] public rewardOffPeriods;

    event Staked(uint256 _stakingId, address _staker, uint256 _amount, uint256 _time);
    event UnStaked(uint256 _stakingId, address _staker, uint256 _amount, uint256 _time);
    ...
    function _harvest(address _user) internal {
        ...
        for(uint256 i = 0; i < userDetails[_user].stakingIds.length; i++){
            uint256 amount = getCurrentReward(userDetails[_user].stakingIds[i]);
            harvestAmount += amount;
            stakeDetails[userDetails[_user].stakingIds[i]].harvested += amount;          
        }
        userDetails[_user].totalHarvestAmount += harvestAmount;
        token.transfer(_user, harvestAmount);
        userDetails[msg.sender].lastClaim = block.timestamp + claimTime;
    ...
    //Change mintues to days when deploying on mainnet
    function getTotalReward(uint256 _stakingId) public view returns (uint256) {  
        if(isRewardOff && rewardOffPeriods.length != 0 && rewardOffPeriods[rewardOffPeriods.length-1].start < stakeDetails[_stakingId].stakeTime){
            return 0;
        }else if(!isRewardOff && rewardOffPeriods.length == 0){
            uint256 stakingDuration = 0;
            if (block.timestamp > stakeDetails[_stakingId].unStakeTime){
                stakingDuration = stakeDetails[_stakingId].unStakeTime - stakeDetails[_stakingId].stakeTime;     
            } else {
                stakingDuration = block.timestamp - stakeDetails[_stakingId].stakeTime;     
            }
            uint256 totalReward = ((stakingDuration / 1 minutes) * stakeDetails[_stakingId].amount * rewardPer / 100) / (7 * 26); // Assuming 6 months as 26 weeks
            return totalReward;
        } else {   
            uint256 timeDiff = 0;
            if (block.timestamp > stakeDetails[_stakingId].unStakeTime){
                timeDiff = stakeDetails[_stakingId].unStakeTime - stakeDetails[_stakingId].stakeTime;     
            }else{
                timeDiff = block.timestamp - stakeDetails[_stakingId].stakeTime;     
            }
            for (uint256 i = 0; i < rewardOffPeriods.length; i++) {
                if(stakeDetails[_stakingId].unStakeTime < rewardOffPeriods[i].start){
                    break;
                }else if(rewardOffPeriods[i].end != 0 && (stakeDetails[_stakingId].unStakeTime < rewardOffPeriods[i].end) && (stakeDetails[_stakingId].stakeTime < rewardOffPeriods[i].start)){
                    timeDiff = timeDiff - (stakeDetails[_stakingId].unStakeTime - rewardOffPeriods[i].start);
                    break;
                }else if(rewardOffPeriods[i].end == 0 && (stakeDetails[_stakingId].unStakeTime < block.timestamp) && (stakeDetails[_stakingId].stakeTime < rewardOffPeriods[i].start)){
                    timeDiff = timeDiff - (stakeDetails[_stakingId].unStakeTime - rewardOffPeriods[i].start);
                    break;
                }
                else if(rewardOffPeriods[i].end != 0 && (stakeDetails[_stakingId].unStakeTime < rewardOffPeriods[i].end) && (stakeDetails[_stakingId].stakeTime > rewardOffPeriods[i].start)){
                    ...
                    if(rewardOffPeriods[i].start > stakeDetails[_stakingId].stakeTime) {                 
                        if(rewardOffPeriods[i].end != 0) {                      
                            timeDiff = timeDiff - (rewardOffPeriods[i].end - rewardOffPeriods[i].start);
                        } else {
                            timeDiff = timeDiff - (block.timestamp - rewardOffPeriods[i].start);
                        }
                    }else if(rewardOffPeriods[i].end < stakeDetails[_stakingId].stakeTime){
                        timeDiff = timeDiff - 0;
                    }else{
                        timeDiff = timeDiff - (rewardOffPeriods[i].end - stakeDetails[_stakingId].stakeTime);
                    }
                }
            }
            uint256 totalReward = ((timeDiff / 1 minutes) * stakeDetails[_stakingId].amount * rewardPer / 100) / (7  * 26); // Assuming 6 months as 26 weeks
            return (totalReward); 
        } 
    }
    ...
    function getCurrentStakingId() public view returns (uint256){
        return stakingId;
    }
    ...
    function getStakeDetails(uint256 _stakingId) public view returns (StakeStruct memory){
        return stakeDetails[_stakingId];
    }

    function getIsRewardOFF() public view returns (bool){
        return isRewardOff;
    }
    ...
    function getRewardOFFPeriodLength() public view returns (uint256){
        return rewardOffPeriods.length;
    }

    function isRewardONOFF(bool _isRewardOFF) public onlyOwner returns (bool) {
        require(isRewardOff != _isRewardOFF,"Already off/on");
        if (!isRewardOff) {
            rewardOffPeriods.push(RewardOffPeriod(block.timestamp, 0));
        } else {
            rewardOffPeriods[rewardOffPeriods.length - 1].end = block.timestamp;      
        }
        isRewardOff = _isRewardOFF;
        return true;
    }
