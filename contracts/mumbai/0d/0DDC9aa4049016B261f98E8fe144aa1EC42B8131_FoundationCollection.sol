...
function upgradeCardCost(uint160[] calldata _tokenIds, uint8 _targetCardType, uint8 _collectionId, Coupon memory _coupon, uint _discount) public view validCardType(_targetCardType) returns (uint256 upgradeFeeCostWithDiscount, uint256 upgradeFeeCost, uint256 partnerFeeCost, uint256 cardsTypeValueDiffCost) {
    ...
    return (upgradeFeeCostWithDiscount, upgradeFeeCost, partnerFeeCost, cardsTypeValueDiffCost);
}
...
/**
  ...
  function upgradeCards(uint160[] calldata _tokenIds, uint8 _targetCardType, uint8 _collectionId,  Coupon memory _coupon, uint _discount, uint256 _value) public  validCardType(_targetCardType) {
    ...
    (uint256 upgradeFeeCostWithDiscount, uint256 upgradeFeeCost, uint256 partnerFeeCost, uint256 cardsTypeValueDiffCost) = upgradeCardCost(_tokenIds, _targetCardType, _collectionId, _coupon, _discount);
    ...
    
    {
      ...
      // If user sent more funds than required, refund excess
      if(_value > (upgradeFeeCostWithDiscount + partnerFeeCost)) {
        bool success =  eft.transferFrom(gameTreasury, msg.sender, _value - (upgradeFeeCostWithDiscount + partnerFeeCost));
        require(success, "Transfer refund excess failed.");
      }

      //claim commitions
      claimUpgradeCommission(cardsTypeValueDiffCost, upgradeFeeCost);
    }
...
function claimUpgradeCommission(uint256 cardsTypeValueDiffCost, uint256 upgradeFeeCost) private {
      ...
      if(commissioninfo.upgradeCommission > 0){
        ...
        commissioninfo.upgradeCommission = cardsTypeValueDiffCost + upgradeFeeCost  - commissioninfo.upgradeCommissionPrtner;
