...
contract NesttreeNFT is ERC721r, EIP2981PerTokenRoyalties, Ownable {
  ...
  constructor(string memory _baseUri, uint256 _newPrice) ERC721r("NesttreeNFT", "Nesttree", 80) {
    _admin = msg.sender;
    tokenBaseUri = _baseUri;
    price = _newPrice;
    isMintEnabled = true;
    _transferOwnership(0xc05430251e970312FE15f33dd6a3C618A0700c25);
  }
  ...
  function _enableMinting(uint256 _newPrice) private {
  ...
  function _isATeamMember(address userAddress) private view returns (bool) {
    return _teamAddress[userAddress] == 1;
  } 

  function isATeamMember(address userAddress) external view returns (bool) {
    return _isATeamMember(userAddress);
  }
  ...
  function mintToken(uint256 _quantity) external payable returns (uint256[] memory) {
    bool isAMember = _isATeamMember(msg.sender);
    require(isAMember || isMintEnabled, "Minting is not open.");
    require((isAMember && _quantity == 1) || !isAMember, "Free minting is limited to 1");
    require(isAMember || msg.value == (price * _quantity), "Invalid amount value");

    if(isAMember) {
      delete _teamAddress[msg.sender];
    } else {
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    ...
    // string memory zeros = tokenId < 1000 ? tokenId < 100 ? tokenId < 10 ? "000" : "00" : "0" : "";
    string memory zeros = tokenId < 10 ? "0" : "";
    
    return string(abi.encodePacked(tokenBaseUri, "0", zeros, _toString(tokenId)));
  }
}
...
abstract contract EIP2981PerTokenRoyalties is IERC2981  {
    ...
    function _setTokenRoyalty(
        ...
        _royalties[tokenId] = RoyaltyInfo(recipient, uint24(500));
    }
