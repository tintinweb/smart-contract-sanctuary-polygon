...
// File: contracts/interfaces/ITransferCooldownable.sol
// SPDX-License-Identifier: MIT
...
// File: contracts/interfaces/ICharacter.sol
// SPDX-License-Identifier: MIT
pragma solidity >=0.4.22 <0.9.0;
interface ICharacter {
    enum CharacterType {Elite, Trainee, Citizen}
}
// File: contracts/characters.sol
// SPDX-License-Identifier: MIT
...
// File: contracts/RewardsManager/interfaces/IRewardsManager.sol
// SPDX-License-Identifier: MIT
...
// File: contracts/CommandCenter/CommandCenter.sol
// SPDX-License-Identifier: MIT
/*
    Original proposal: https://snapshot.org/#/ciphershooters.eth/proposal/0xd3db6a3bbbf53b317bdfeb21ab62b361c02b2d619992903b02fc7d52b4119add
    ownerOf(0) - current "master commander"
    operator got transferred to CC when he conquer it
    Needed roles:
    - characters.RECEIVE_DOES_NOT_SET_TRANSFER_TIMESTAMP()
    - characters.GAME_ADMIN()
    - weapons.GAME_ADMIN()
*/
// TODO: add access control
// TODO: add crafting rewards (parts)
contract CommandCenter is IERC721ReceiverUpgradeable, Initializable, AccessControlUpgradeable, ERC721Upgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    using SafeMath for uint8;
    using SafeMath for uint16;
    using SafeMath for uint24;
    using ABDKMath64x64 for int128;
    Characters public operators;
    Weapons public weapons;
    IERC20Upgradeable public rewardToken;
    IRewardsManager public rewardsManager;
    IRandoms public randoms;
    uint256 public defenderOperatorId;
    uint256 public defenderWeaponId;
    uint256 public erc721tokenId;
    uint256 public baseReward;
    uint256 public baseXpGain;
    uint8 public baseStaminaCost;
    uint8 public baseDurabilityCost;
    uint16 public combatDuration;
    struct FightData {
        uint24 power;
        uint8 operatorAccuracy;
        uint16 fireRate;
        uint8 weaponAccuracy;
    }
    event FightOutcome(
        address indexed attacker,
        uint256 attackerOperatorId,
        uint256 attackerWeaponId,
        uint256 attackerReward,
        uint256 attackerXpGain,
        address indexed defender,
        uint256 defenderOperatorId,
        uint256 defenderWeaponId,
        uint256 defenderReward,
        uint256 defenderXpGain,
        bool conquestSuccessful
    );
    event ShotOutcome(uint256 shooterOperatorId, uint256 recipientOperatorId, uint256 totalDamage, uint8 bodyPartHit);
    ...
    function initialize(
        ...
        randoms = _randoms;
        operators.transferFrom(msg.sender, address(this), operatorId);
        weapons.transferFrom(msg.sender, address(this), weaponId);
        defenderOperatorId = operatorId;
        defenderWeaponId = weaponId;
        erc721tokenId = 1;
        _mint(msg.sender, erc721tokenId);
    }
    function conquer(uint256 operatorId, uint256 weaponId) external {
        // 0. Precalculate rewards
        // During the conquer attempt new owner receives a one-time reward that is determined by the level of the fallen owner.
        // If the fight is lost by the attacker, the attacker pays “compensation” to the owner.
        uint256 attackerReward = getReward(defenderOperatorId);
        uint256 defenderReward = getReward(operatorId);
        require(
            rewardToken.balanceOf(msg.sender) >= defenderReward,
            'Insufficient balance to cover compensation of conquer attempt'
        );
        require(operators.ownerOf(operatorId) == msg.sender, "operatorId is not owned by the sender");
        require(weapons.ownerOf(weaponId) == msg.sender, "weaponId is not owned by the sender");
        require(operators.isApprovedForAll(msg.sender, address(this)), "operators are not approved for transfers");
        require(weapons.isApprovedForAll(msg.sender, address(this)), "weapons are not approved for transfers");
        // TODO: only elites
        // 1. Calculate fight outcome
        uint256 rand = randoms.getRandomSeed(msg.sender);
        (bool conqured, uint16 attackerXpGain, uint16 defenderXpGain) = fightOutcome(
            operatorId,
            weaponId,
            defenderOperatorId,
            defenderWeaponId,
        ...
        // 2. Update stamina and durability
        operators.getFightDataAndDrainStamina(operatorId, baseStaminaCost);
        operators.getFightDataAndDrainStamina(defenderOperatorId, baseStaminaCost);
        weapons.getFightDataAndDrainDurability(weaponId, baseDurabilityCost);
        weapons.getFightDataAndDrainDurability(defenderWeaponId, baseDurabilityCost);
        // 3. Update xp
        if (attackerXpGain > 0) {
            operators.gainXp(operatorId, attackerXpGain < 65535 ? attackerXpGain : 65535);
        }
        if (defenderXpGain > 0) {
            operators.gainXp(defenderOperatorId, defenderXpGain < 65535 ? defenderXpGain : 65535);
        }
        // 4. Distribute reward or compensation to the winner
        if (conqured) {
            rewardToken.safeApprove(address(rewardsManager), attackerReward);
            rewardsManager.deposit(0, attackerReward, msg.sender);
        } else {
            rewardToken.safeApprove(address(rewardsManager), defenderReward);
            rewardsManager.deposit(0, defenderReward, ownerOf(erc721tokenId));
        }
        emit FightOutcome(
            msg.sender,
            operatorId,
            weaponId,
            conqured ? attackerReward : 0,
            conqured ? attackerXpGain : 0,
            ownerOf(erc721tokenId),
            defenderOperatorId,
            defenderWeaponId,
            conqured ? 0 : defenderReward,
            conqured ? 0 : defenderXpGain,
            conqured
        );
        // 5. Transfer ownership if conqured
        if (conqured) {
            operators.transferFrom(msg.sender, address(this), operatorId);
            weapons.transferFrom(msg.sender, address(this), weaponId);
            operators.transferFrom(address(this), ownerOf(erc721tokenId), defenderOperatorId);
            weapons.transferFrom(address(this), ownerOf(erc721tokenId), defenderWeaponId);
            defenderOperatorId = operatorId;
            defenderWeaponId = weaponId;
            _transfer(ownerOf(erc721tokenId), msg.sender, erc721tokenId);
        }
    }
    function fightOutcome(
        uint256 _attackerOperatorId,
        uint256 _attackerWeaponId,
        uint256 _defenderOperatorId,
        uint256 _defenderWeaponId,
        uint256 _rand
    )
        internal
        returns (
            bool conquered,
            uint16 attackerXpGain,
            uint16 defenderXpGain
        )
    {
        FightData memory attackerFightData = getFightData(_attackerOperatorId, _attackerWeaponId);
        FightData memory defenderFightData = getFightData(_defenderOperatorId, _defenderWeaponId);
        uint24 attackerRoll = getDamangeDealt(
            attackerFightData,
            RandomUtil.combineSeeds(_rand, 1),
            _attackerOperatorId,
            _defenderOperatorId
        );
        uint24 defenderRoll = getDamangeDealt(
            attackerFightData,
            RandomUtil.combineSeeds(_rand, 2),
            _defenderOperatorId,
            _attackerOperatorId
        );
        conquered = attackerRoll > defenderRoll;
        attackerXpGain = uint16(ABDKMath64x64.divu(defenderFightData.power, attackerFightData.power).mulu(baseXpGain));
        defenderXpGain = uint16(ABDKMath64x64.divu(attackerFightData.power, defenderFightData.power).mulu(baseXpGain));
    }
    function getReward(uint256 opponentOperatorId) public view returns (uint256) {
        uint8 level = operators.getLevel(opponentOperatorId);
        return baseReward.mul(level);
    }
    function getFightData(uint256 operatorId, uint256 weaponId) public view returns (FightData memory) {
        uint24 operatorPower = operators.getPower(operatorId);
        (, uint8 _operatorAccuracy, , uint8 _operatorElement, uint8 _operatorClass, , , , ) = operators.get(operatorId);
        (uint16 _weaponPower, uint16 _fireRate, uint8 _weaponAccuracy, , , , , ) = weapons.get(weaponId);
        uint24 totalPower = uint24(_weaponPower.add(operatorPower));
        totalPower = weapons.getFightData(weaponId, totalPower, _operatorElement, _operatorClass);
        return FightData(totalPower, _operatorAccuracy, _fireRate, _weaponAccuracy);
    }
    function setBaseReward(uint256 _baseReward) public onlyAdmin {
        baseReward = _baseReward;
    }
    function setBaseXpGain(uint8 _baseXpGain) public onlyAdmin {
        baseXpGain = _baseXpGain;
    }
    function setBaseStaminaCost(uint8 _baseStaminaCost) public onlyAdmin {
        baseStaminaCost = _baseStaminaCost;
    }
    function setBaseDurabilityCost(uint8 _baseDurabilityCost) public onlyAdmin {
        baseDurabilityCost = _baseDurabilityCost;
    }
    function setCombatDuration(uint16 _combatDuration) public onlyAdmin {
        combatDuration = _combatDuration;
    }
    function setRewardToken(IERC20Upgradeable _rewardToken) public onlyAdmin {
        rewardToken = _rewardToken;
    }
    function debug_setIds(uint256 operatorId, uint256 weaponId) public onlyAdmin {
        defenderOperatorId = operatorId;
        defenderWeaponId = weaponId;
    }
    function getDamangeDealt(
        FightData memory data,
        uint256 seed,
        uint256 shooterOperatorId,
        uint256 recipientOperatorId
    ) internal returns (uint24) {
        uint8 finalAccuracy = uint8(ABDKMath64x64.divu(data.weaponAccuracy, 100).mulu(data.operatorAccuracy));
        uint256 shotsFired = uint256(ABDKMath64x64.divu(data.fireRate, 100).mulu(combatDuration)); // floor value
        ...
        for (uint256 i = 0; i < shotsFired; i++) {
            ...
            uint24 perShotDamage = calculateBodyPartShot(data.power, bodyPartHit);
            totalDamage = totalDamage.add(perShotDamage);
            emit ShotOutcome(shooterOperatorId, recipientOperatorId, perShotDamage, bodyPartHit);
        }
    ...
    modifier onlyAdmin() {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), 'DEFAULT_ADMIN_ROLE role needed');
        _;
    }
}