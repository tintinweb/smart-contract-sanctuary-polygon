...
contract WhiteListMinterV2 is Ownable{
    ...
    function getEventWhitelistedContract(string memory eventId) external view returns (address){
            return address(dropEvents[eventId].whiteListedContract);
    }

    function getEventWDropContract(string memory eventId) external view returns (address){
        return address(dropEvents[eventId].dropContract);
    }

    function getEventAllowedEIDs(string memory eventId, uint eID) external view returns (uint[] memory){
        return dropEvents[eventId].eIdToAllowedEIDs[eID];
    }

    function getEventMaxMints(string memory eventId) external view returns (uint){
        return dropEvents[eventId].maxMints;
    }

    function getEventCurrentMints(string memory eventId) external view returns (uint){
        return dropEvents[eventId].mints;
    }
