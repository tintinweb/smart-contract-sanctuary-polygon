...
contract SwaprFee is BaseGovernanceWithUserUpgradable {
    ...
    uint constant internal _EXP = 1e18;
    ...
    function setLKRToken(address _lkrToken, address _lkrOracle, uint256 __discountAmount) external onlyAdmin {
    ...
    function isTokenSupported(address _paymentToken) public view returns(bool exists) {
        if(_paymentToken == address(0)) {
            address(_cryptoPriceFeed) != address(0) ? exists = true : exists = false;
    ...
    function _calculateFinalFeeCap(FeePricing memory feePricing, address token) internal view returns (uint priceCapTokens) {
    ...
    function _getLkrPrice(IDIAOracleV2 priceFeed) internal view returns (uint) {
        (
            uint128 price, uint128 timeStamp
        ) = priceFeed.getValue("LKR/USD");
        require(timeStamp > 0 && timeStamp <= block.timestamp && timeStamp >= block.timestamp - 60 , "Round not complete");
        require(price > 0, "Price is 0");
        return uint(price);
    }
