...
import './token/Dealmintr.sol';


contract DealmintrFactory {
    ...
    struct DealmintrNFT{
      ...
      DealmintrNFT[] private dealmintrNfts;
      mapping(address => uint256[]) private dealmintrIndexes;

      address private operator;

  
    event CreateCollection(address indexed to,address indexed from,uint256 index);

    constructor(string memory _baseURI,address _operator){
        baseURI = _baseURI;
        operator =_operator;
    }
     ...
     uint256 price
        ...
        Dealmintr pair = new Dealmintr(name,symbol,baseURI,maxBatchSize,operator,price);
        DealmintrNFT memory _dealmintrNft = DealmintrNFT(c_name,name,symbol,address(pair),controller,companyAddress,price,block.timestamp);
        dealmintrNfts.push(_dealmintrNft);
        dealmintrIndexes[msg.sender].push(dealmintrNfts.length);
        emit CreateCollection(address(this),address(pair),dealmintrNfts.length);
        return true;
        }
        return false;
     }
    ...
    function getAllCollections() external view returns (address[] memory) {
        address[] memory _dealmintrs = new address[](dealmintrNfts.length);
        for (uint256 i = 0; i < dealmintrNfts.length; i++) {
            _dealmintrs[i] = dealmintrNfts[i].c_collection;
        }
        return _dealmintrs;
    }

    function getCollectionAddress(uint256 _id) external view returns (address, uint256) {
        return (dealmintrNfts[_id].c_collection, dealmintrNfts[_id].timestamp);
    }
 
    function getCollections(address _cAddress) external view returns (DealmintrNFT[] memory) {
        uint256[] memory _indexes = dealmintrIndexes[_cAddress];
        DealmintrNFT[] memory _dealmintrInfos = new DealmintrNFT[](_indexes.length);
        for (uint256 i = 0; i < _indexes.length; i++) {
            _dealmintrInfos[i] = dealmintrNfts[_indexes[i] - 1];
        }
        return _dealmintrInfos;
    }

     
      function getCollectionId(address _cAddress) external view returns (uint256[] memory) {
        return dealmintrIndexes[_cAddress];
    }
  ...
  address private owner;
    ...
    function mint(
        ...
        uint256 _price
         ...
         if(msg.sender != owner && _price != price) revert Unauthorized();
          _tokenIds.increment();
