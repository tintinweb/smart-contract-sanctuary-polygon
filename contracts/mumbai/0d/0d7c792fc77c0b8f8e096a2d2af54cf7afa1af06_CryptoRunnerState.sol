...
// - E008: RUN HAS ALREADY ENDED
// - E009: RUNNER IS ALREADY RUNNING
// - E010: RUNNER MUST BE RUNNING
// - E011: RUN ID DOES NOT MATCH
// - E012: INVALID REGISTRY ADDRESS
...
contract CryptoRunnerState is Initializable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    event RunStarted(address indexed sender, uint256 indexed tokenId, string runId);
    event RunEnded(address indexed to, uint256 indexed tokenId, string runId);
    event InitialClockChanged(uint256 previousAmount, uint256 newAmount, address indexed sender);
    event ClockRegenChanged(uint256 previousAmount, uint256 newAmount, address indexed sender);
    event MaxClockChanged(uint256 previousAmount, uint256 newAmount, address indexed sender);
    event DataChanged(address indexed sender, address indexed previous, address indexed data);

    // TEMPORARY KEYS TO RESET DATA
    uint256[] private nonces;
    string[] private runIds;
    ...
    function initialize(
        ...
        _grantRole(Constants.UPGRADER_ROLE, upgrader_);
        _grantRole(DEFAULT_ADMIN_ROLE, admin_);
        _grantRole(Constants.SIGNER_ROLE, signer_);
        initialClock = initialClock_;
        clockRegen = clockRegen_;
        maxClock = maxClock_;
        registry = IRegistry(registry_);
        registry.register(Constants.CRYPTO_RUNNER_STATE, msg.sender);
    }
    ...
    function setInitialClock(uint256 initialClockAmount_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        emit InitialClockChanged(previousAmount, initialClockAmount_, msg.sender);
    }

    function setClockRegen(uint256 clockRegen_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        uint256 previousAmount = clockRegen;
        clockRegen = clockRegen_;
        emit ClockRegenChanged(previousAmount, clockRegen_, msg.sender);
    }

    function setMaxClock(uint256 maxClock_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        uint256 previousAmount = maxClock;
        maxClock = maxClock_;
        emit MaxClockChanged(previousAmount, maxClock_, msg.sender);
    }

    /// @notice Function used to start a run
    /** @dev Types: ['bytes','uint256[]', 'string']
             uint256[] order: tokenId, requiredClock, nonce, deadline */
    ...
    function startRun(bytes calldata data_) external returns (CryptoRunner memory) {
        ...
        nonces.push(runArgs[2]);
        runIds.push(runId);
        usedNonces[runArgs[2]] = true;
    ...
    function _startRunArgs(bytes memory data_) internal view returns (uint256[] memory, string memory) {
        (bytes memory signature, uint256[] memory args, string memory runId) = abi.decode(
            data_,
            (bytes, uint256[], string)
        );
        _isValidSignature(signature, abi.encodePacked(args, runId));
        require(block.timestamp < args[3], "E007");
        require(!usedNonces[args[2]], "E002");
        require(runsById[runId].startTime == 0, "E005");
        ...
        )
    {
        ...
        _isValidSignature(signature, abi.encodePacked(args, runId, to));
    ...
    /** @dev Types: ['bytes','uint256[]', 'string', 'address']
             uint256[] order: tokenId, notorietyPoints, dataReward, nonce, deadline */
    /// @param data_ abi encoded data
    function endRun(bytes calldata data_) external {
        ...
        require(run.endTime == 0, "E008");
        require(cryptoRunner.isRunning, "E010");
        require(keccak256(bytes(cryptoRunner.currentRunId)) == keccak256(bytes(runId)), "E011");
        nonces.push(runArgs[3]);
        usedNonces[runArgs[3]] = true;
        ...
        cryptoRunners[runArgs[0]] = cryptoRunner;
        cryptoRunners[runArgs[0]].currentRunId = "";
    ...
    function getRun(string calldata runId_) external view returns (Run memory) {
    ...
    function getRunsByRunner(uint256 tokenId_) public view returns (string[] memory) {
    ...
    function getRunners(uint256[] memory tokenIds_) public view returns (CryptoRunner[] memory) {
    ...
    function _transferData(address to_, uint256 amount_) internal {
        ITreasury treasury = ITreasury(registry.getEntry(Constants.TREASURY));
        IERC20(registry.getEntry(Constants.DATA)).approve(address(treasury), Constants.UINT256_MAX);
    ...
    function _isValidSignature(bytes memory signature_, bytes memory encodedArgs_) internal view {
        ...
        require(hasRole(Constants.SIGNER_ROLE, (signedHash.recover(signature_))), "E001");
    }

    function resetData() external onlyRole(Constants.SIGNER_ROLE) {
        for (uint256 i = 0; i < nonces.length; i++) {
            delete usedNonces[nonces[i]];
        }
        for (uint256 i = 0; i < runIds.length; i++) {
            delete runsById[runIds[i]];
        }
...
*/
library Constants {
    address internal constant MULTI_SIG           = 0xD1B058A2D7E3689fcBe11d798f61b420cD39DCC6;
