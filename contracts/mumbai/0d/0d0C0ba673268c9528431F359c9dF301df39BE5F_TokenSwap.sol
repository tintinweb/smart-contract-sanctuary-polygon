...
contract TokenSwap is ITokenSwap, AccessControl {
    ...
    mapping(bytes => TokenSwapData) public zappers;
    ...
    function addSwapRoute(
        ...
        zappers[tokenized] = TokenSwapData(router, routerPath);
        zappers[tokenizedReverse] = TokenSwapData(router, routerPathReverse);
    }
        ...
        onlyRole(ROUTER_POLICY_ROLE)
    {
        ...
        delete zappers[tokenized];
        delete zappers[tokenizedReverse];
    }
    ...
    function exchange(
        ...
        address[] memory path = zappers[tokenized].path;
        //take the token
        IERC20(tokenIn).safeTransferFrom(msg.sender, address(this), amountIn);
        //approve the router
        IERC20(tokenIn).approve(zappers[tokenized].router, amountIn);

        uint256[] memory amounts;
        if (exchangeType == EXCHANGE_TYPE.EXACT_INPUT) {
            amounts = IUniswapV2Router01(zappers[tokenized].router)
    ...
    function quoteOut(
        ...
        uint256[] memory amounts = IUniswapV2Router01(zappers[tokenized].router)
