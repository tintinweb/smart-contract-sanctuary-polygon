...
contract PNounsProvider3 is IAssetProviderEx, Ownable, IERC165 {
  ...
  INounsSeeder.Seed public seedFor553 =
    INounsSeeder.Seed({ background: 1, body: 28, head: 47, accessory: 48, glasses: 16 });
  INounsSeeder.Seed public seedFor556 =
    INounsSeeder.Seed({ background: 1, body: 7, head: 37, accessory: 238, glasses: 12 });
  ...
  function generateSVGPart(uint256 _assetId) public view override returns (string memory svgPart, string memory tag) {
    ...
    // stack.width = SVG.textWidth(font, ' ');
    // stack.pnouns = SVG.text(font, ' ').fill('#224455').transform(TX.scale1000((1000 * 512) / stack.width));
    ...
    stack.series = SVG.text(font, stack.seriesText).fill('#224455').transform(
      TX.translate(1024 - int(stack.width / 50), 1024 - 52).scale('0.02')
    );

    // main Noun
    (stack.svg, stack.idNouns) = nounsProvider.getNounsSVGPart(_assetId);
    stack.svgNouns = SVG.element(bytes(stack.svg));
    stack.nouns = SVG.use(stack.idNouns).transform('translate(102,150) scale(0.9)');

    // pNoun (553 or 556)
    stack.idNouns2 = 'pNouns553or556';
    stack.svg2 = nounsProvider.svgForSeed(_assetId % 2 == 0 ? seedFor553 : seedFor556, stack.idNouns2);
    stack.svgNouns2 = SVG.element(bytes(stack.svg2));
    stack.nouns2 = SVG.use(stack.idNouns2).transform('translate(52,44) scale(0.2)');
