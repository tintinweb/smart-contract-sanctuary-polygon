...
contract NgageNFactory is AccessControl {
    ...
    constructor() {
        ...
        _setupRole(CONTRACT_CREATOR, msg.sender);
    }
    ...
    function create(
        ...
        address ngageN = Clones.clone(address(new NgageN()));
        NgageN(ngageN).initialize(_uri, _contractUri);

        factoryCreated[_factoryName] = _msgSender();
        factoryContracts[_factoryName] = FactoryContract(
            ngageN
        );
        emit FactoryCreated(_msgSender(), _factoryName, ngageN);
    }
}
