...
contract AdminManagedSBT is Initializable, OwnableUpgradeable, AdminManaged, SoulBound, ContractMetadata {
    ...
    function burn(uint256 tokenId) external {
        require(_isAllowedToBurn[tokenId], 'AdminManagedSBT: not allowed to burn');
        _unlock(tokenId);
        super._burn(tokenId, true); // with Approval checking
    }

    function forceBurn(uint256 tokenId) external onlyRole(BURN_ROLE) {
        _unlock(tokenId);
    ...
    function allowToBurn(uint256 tokenId) external onlyOwner {
        _isAllowedToBurn[tokenId] = true;
    }

    function denyToBurn(uint256 tokenId) external onlyOwner {
        _isAllowedToBurn[tokenId] = false;
    }

    function supportsInterface(
        bytes4 interfaceId
    ) public view override(SoulBound, AccessControlUpgradeable) returns (bool) {
    ...
    function _canSetContractURI() internal view virtual override onlyOwner returns (bool) {
        return true;
    }
}
...
abstract contract AdminManaged is Initializable, AccessControlUpgradeable {
    bytes32 public constant TRANSFER_ROLE = keccak256('TRANSFER_ROLE');

    function __AdminManaged_init(address owner) public initializer {
...
abstract contract SoulBound is Initializable, IERC5192, ERC721AUpgradeable {
    ...
    function __SoulBound_init(string memory name, string memory symbol) internal initializerERC721A onlyInitializing {
