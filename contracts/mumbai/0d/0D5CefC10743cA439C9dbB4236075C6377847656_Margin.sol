...
interface IPosition {
    ...
    function checkUserOpenPosition(address user, address assetAddress) external view returns(bool);
...
contract Margin is ReentrancyGuard, MarginStorage, Ownable {
    ...
    function repay(
        ...
        //using the margin account
            ...
            if(repayAmount >= ICErc20Delegator(delegator).borrowBalanceCurrent(msg.sender)) {
                ...
                require(repayAmount <= depositInfo.loanAmount + depositInfo.interestAccumulated, "Margin: Excess loan repaid");
                if(repayInterestAmount !=0) {
                    // require(15 < 10 , "error-365");
                    uint256 ltiRatio = (repayLoanAmount * precision)/depositInfo.interestAccumulated;
                    (loanAmount, interestAmount) = getAmount(repayAmount, ltiRatio);
                }
                else {
                    loanAmount = repayAmount; //1usdc
                }
                if(loanAmount > depositInfo.loanAmount) {
                    // require(15 < 10 , "error-372");
                    depositInfo.interestAccumulated = depositInfo.interestAccumulated - interestAmount - (loanAmount - depositInfo.loanAmount);
                    depositInfo.marginAmount -= interestAmount;
                    depositInfo.loanAmount = 0;
                }
                else {
                    // require(15 < 10 , "error-377");
                    depositInfo.loanAmount -= loanAmount; //0
                    depositInfo.marginAmount -= interestAmount;
                    depositInfo.interestAccumulated -= interestAmount;
                }
                depositInfo.repaidLoan += loanAmount;
    ...
    function liquidate(address user, address assetAddress) internal {
        ...
        bool isPosition = IPosition(IAdminFunctions(adminContract).getPositionContract()).checkUserOpenPosition(user, assetAddress);
        if(isPosition == true) {
            //call repay
