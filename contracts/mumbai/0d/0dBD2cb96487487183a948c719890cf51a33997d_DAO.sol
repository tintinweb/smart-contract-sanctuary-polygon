...
contract DAO is OwnableUpgradeable, UUPSUpgradeable {
    ...
    ///@dev Time delay (in seconds) between any proposal creation and start of voting for it.
    uint256 public votingDelay;
    ///@dev Time (in seconds) for voting duration of any proposal
    ...
    ///@param _votingDelay, in seconds, between the proposal is created and the vote starts. This can be increased to leave time for users to buy voting power, before the voting of a proposal starts.
    ///@param _votingPeriod in seconds of blocks, between the vote start and vote ends.The {votingDelay} can delay the start of the vote. This must be considered when setting the voting duration compared to the voting delay.
        ...
        returns (ProposalState state)
    {
        ...
        uint256 start = proposal.voteStart;
        uint256 deadline = proposal.voteEnd;
        uint256 currentBlockTimeStamp = block.timestamp;

        //before the voting period starts it's in pending state
        if (currentBlockTimeStamp < start) {
            return ProposalState.Pending;
        }
        //in the voting state
        else if (currentBlockTimeStamp <= deadline) {
    ...
    function propose(
        ...
        uint256 deadline = start + votingPeriod;
        proposals[_tokenId][proposalCounter[_tokenId]].voteStart = start;
        proposals[_tokenId][proposalCounter[_tokenId]].voteEnd = deadline;
        uint256 quorumVotes = (quorumPercentage * tokenTotalSupply) / 100_00;
    ...
    function editProposal(
        ...
        uint256 tokenTotalSupply = token.totalSupply(_tokenId);
        proposal.totalTokenSupply = tokenTotalSupply;
        ...
        returns (bool result)
    {
        ...
        return ((voting.forVote) >=
            ((proposal.totalTokenSupply * quorumPercentage) / 100_00));
