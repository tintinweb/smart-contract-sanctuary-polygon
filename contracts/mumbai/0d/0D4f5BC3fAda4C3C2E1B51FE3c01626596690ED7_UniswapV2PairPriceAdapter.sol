...
import { UniswapV2Library } from "../../../external/uniswap-v2/lib/UniswapV2Library.sol";
...
contract UniswapV2PairPriceAdapter is AccessControl, IOracleAdapter {
     ...
     * @param uniswapFactory    Address of Uniswap factory
     * @param uniswapPools      Array of allowed Uniswap pools
     */
    constructor(
        IPriceOracle priceOracle,
        address uniswapFactory,
        IUniswapV2Pair[] memory uniswapPools
    ) {
        ...
        for (uint256 i = 0; i < uniswapPools.length; i++) {
            _addPool(address(uniswapPools[i]));
        }
    }
    ...
    function getPoolSetting(address pool) external view returns (PoolSetting memory) {
    ...
    function addPool(address poolAddress) external onlyAdmin {
        _addPool(poolAddress);
    }

    function removePool(address poolAddress) external onlyAdmin {
        require(_poolSettings[poolAddress].isValid, "UPPA0"); // "Uniswap pool address does not exist"

        _allowedUniswapPools.quickRemoveItem(poolAddress);
        delete _poolSettings[poolAddress];
    }

    function addQuoteAsset(address quoteAsset) external onlyAdmin {
        require(!_quoteAssets.contain(quoteAsset), "UPPA1"); // "add already exist quote asset"

        _quoteAssets.push(quoteAsset);
    }

    function removeQuoteAsset(address quoteAsset) external onlyAdmin {
        require(_quoteAssets.contain(quoteAsset), "UPPA2"); // "remove nonexist quote asset"
    ...
    function _getUniswapPairPrice(
        ...
        PoolSetting memory poolInfo = _poolSettings[poolAddress];
        IUniswapV2Pair poolToken = IUniswapV2Pair(poolAddress);
        ...
        // Get reserve amounts
        (uint256 token1Reserves, uint256 token2Reserves) = UniswapV2Library.getReserves(_uniswapFactory, poolInfo.token1, poolInfo.token2);
        ...
        return totalNotionalToMaster.preciseDiv(poolToken.totalSupply());
    }

    // ==================== Private functions ====================

    function _addPool(address poolAddress) private {
        require(!_poolSettings[poolAddress].isValid, "UPPA3"); // "Uniswap pool address already added"

        PoolSetting memory poolSetting;

        poolSetting.token1 = IUniswapV2Pair(poolAddress).token0();
        uint256 token1Decimals = ERC20(poolSetting.token1).decimals();
        poolSetting.token1BaseUnit = 10**token1Decimals;

        poolSetting.token2 = IUniswapV2Pair(poolAddress).token1();
        ...
        _allowedUniswapPools.push(poolAddress);
        _poolSettings[poolAddress] = poolSetting;
    }
...
// Copy from https://github.com/Uniswap/v2-periphery/blob/master/contracts/libraries/UniswapV2Library.sol under terms of GPL-3.0 with slight modifications
...
library UniswapV2Library {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint160(uint256(keccak256(abi.encodePacked(
                ...
                hex'c42f4f4e70ef1902e4512d6ff9464fb6a46d6c6821f1ffefefb634afd9802389' // init code hash
            )))));
    }
