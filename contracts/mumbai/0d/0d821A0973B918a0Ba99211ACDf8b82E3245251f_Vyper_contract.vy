...
s_usdc_address: address

# State variables
s_addresses_set: bool
s_admin_address: address
s_withdraw_address: address
...
s_meeting_race_status: HashMap[String[18], DynArray[int256, 20]]
...
s_win_place_bets: HashMap[address, HashMap[String[40], uint256]]
...
@external
@view
def raceStatus(_meeting_key: String[18]) -> (DynArray[int256, 20]):
...
@external
def setRaceStatus(_meeting_key: String[18], _race: uint256, _status: int256):
    assert self.s_admin_address == msg.sender, "You are not the admin address"
...
@external
@payable
def createWinBet(_wl_operator: String[2], _meeting_key: String[18], _race: String[2], _runner: String[2], _amount: uint256):
    # assert self.s_meeting_race_status[_meeting_key][convert(_race, uint256)] == 1, "Race not active"
    # assert self.s_meeting_race_runner_status[_meeting_key][convert(_race, uint256)][convert(_runner, uint256)] == 1, "Runner not active"

    self._receiveUSDC(_amount)
    _bet_key: String[40] = concat(_wl_operator, "-win-", _meeting_key, "-", _race, "-", _runner)
...
@external
def changeWinBet(_wl_operator: String[2], _meeting_key: String[18], _race: String[2], _from_runner: String[2], _to_runner: String[2]):
    # assert self.s_meeting_race_status[_meeting_key][convert(_race, uint256)] == 1, "Race not active"
    # assert self.s_meeting_race_runner_status[_meeting_key][convert(_race, uint256)][convert(_from_runner, uint256)] == 1, "Runner not active"
    # assert self.s_meeting_race_runner_status[_meeting_key][convert(_race, uint256)][convert(_to_runner, uint256)] == 1, "Runner not active"

    _old_bet_key: String[40] = concat(_wl_operator, "-win-", _meeting_key, "-", _race, "-", _from_runner)
    _new_bet_key: String[40] = concat(_wl_operator, "-win-", _meeting_key, "-", _race, "-", _to_runner)

    bet_amount: uint256 = self.s_win_place_bets[msg.sender][_old_bet_key]
    ...
    if bet_amount > 0:
        ...
        self.s_win_place_bets[msg.sender][_new_bet_key] += bet_amount_new
        self.s_win_place_bets[msg.sender][_old_bet_key] = 0

        log WinOrPlaceBet(
            _old_bet_key, _wl_operator, 0, msg.sender, "win", "change", _meeting_key,
            _race, _from_runner, self.s_win_place_bets[msg.sender][_old_bet_key]
        )
        log WinOrPlaceBet(
            _new_bet_key, _wl_operator, 0, msg.sender, "win", "change", _meeting_key,
            _race, _to_runner, self.s_win_place_bets[msg.sender][_new_bet_key]
        )
...
@external
def cancelWinBet(_wl_operator: String[2], _meeting_key: String[18], _race: String[2], _runner: String[2]):
    # assert self.s_meeting_race_status[_meeting_key][convert(_race, uint256)] == 1, "Race not active"
    # assert self.s_meeting_race_runner_status[_meeting_key][convert(_race, uint256)][convert(_runner, uint256)] == 1, "Runner not active"

    bet_key: String[40] = concat(_wl_operator, "-win-", _meeting_key, "-", _race, "-", _runner)
