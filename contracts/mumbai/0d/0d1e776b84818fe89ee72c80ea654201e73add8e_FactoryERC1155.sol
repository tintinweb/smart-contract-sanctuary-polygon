...
contract ERC1155Token is ERC1155, Ownable, ReentrancyGuard {
    ...
    */
    constructor(string memory _contractName, string memory _uri ,address payable _feeAccount, uint _feePercent) ERC1155(_uri) {
    ...
    */
    ...
    function purchaseItem(uint _tokenId) external payable nonReentrant {
        ...
        require(balanceOf(msg.sender, _tokenId) == 0, "Can only buy 1 nft");

        //pay feeAccount and seller
        uint256 fees = getFees(item.price);
        feeAccount.transfer(fees);

        address payable _owner = payable(owner());
        _owner.transfer(item.price - fees);
    ...
    function getFees(uint256 _total) view public returns(uint256 _fees) {
        return (_total/100)*feePercent;
    }
...
contract FactoryERC1155 is Ownable {
    ...
    // mapping(address => uint256) public contractToIndex; // address to index 
    ...
    event ERC1155Minted(address owner, address tokenContract, uint amount, uint256 price); //emmited when ERC1155 token is minted
    ...
    */
    function deployERC1155(string memory _contractName, string memory _uri) public returns (address) {
        ...
        ERC1155Token t = new ERC1155Token(_contractName, _uri, feeAccount, feePercent);
        ownerToContract[msg.sender] = address(t);
        // contractToIndex[address(t)] = tokens.length - 1;
        // ownerToIndex[tx.origin] = tokens.length - 1;
    ...
    */
    // function mintERC1155(uint256 id, uint256 amount, uint256 price) public {
    //     tokens[ownerToIndex[msg.sender]].mint(id, amount,price);
    //     emit ERC1155Minted(tokens[ownerToIndex[msg.sender]].owner(), address(tokens[ownerToIndex[msg.sender]]), amount, price);
    // }

    /*
    Helper functions below retrieve contract data given an ID or name and index in the tokens array.
    */
    // function getCount(uint256 _index, uint256 _id) public view returns (uint amount) {
