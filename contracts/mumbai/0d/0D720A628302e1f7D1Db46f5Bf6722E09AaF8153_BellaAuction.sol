...
contract BellaAuction is Initializable, UUPSUpgradeable, ReentrancyGuardUpgradeable, AccessControlUpgradeable, PausableUpgradeable {

    // ERR-1-BA         NFT must not be zero-address
    // ERR-2-BA         NFT contract not valid
    // ERR-3-BA         Price cannot be zero
    // ERR-4-BA         Invalid supply
    // ERR-5-BA         Invalid address for nft contract
    // ERR-6-BA         newImplementation cannot be zero-address
    // ERR-7-BA         The supply is greater than balance
    // ERR-8-BA         Store must be approved as operator
    // ERR-9-BA         You must own the token
    // ERR-10-BA        Sender address not valid
    // ERR-11-BA        Seller address not valid
    // ERR-12-BA        Please submit required price of 
    // ERR-13-BA        Invalid itemId
    // ERR-14-BA        The auction is closed
    // ERR-15-BA        Invalid offer address
    // ERR-16-BA        Invalid offer value
    // ERR-17-BA        endTime time not valid
    // ERR-18-BA        Auction out from active range
    // ERR-19-BA        Start time not valid
    ...
    // Modifier to check that auction is valid
    modifier isAuctionValid(uint auctionId, bool checkInterval) {
        require(auctionId < _auctionId.current(), "ERR-13-BA");
        require(idToAuction[auctionId].status == Status.AVAILABLE, "ERR-14-BA");
        if(checkInterval && idToAuction[auctionId].hasExpirationTime) {
            require(idToAuction[auctionId].startTime <= block.timestamp 
                && idToAuction[auctionId].endTime >= block.timestamp, "ERR-19-BA");
        }
        _;
    }
    ...
    /// Set Extend Price 
    /// @param _extendPrice new feePercentage
    /// @notice update the extend price value
    function setExtendPrice(uint _extendPrice) 
        external 
        onlyRole(DEFAULT_ADMIN_ROLE) 
    {
        extendPrice = _extendPrice;
    }
    ...
    function putUpForAuction(
        ...
        whenNotPaused
    {
        (bool __isERC721, string memory tokenType) = Utils(utilsAddress).validatePutUpForAuction(
            tokenId, 
            nftContract, 
            supply, 
            price, 
        ...
        uint auctionId = _auctionId.current();
    ...
    function makeAnOffer(uint auctionId) 
        external 
        payable 
        nonReentrant
        whenNotPaused
        isAuctionValid(auctionId, true)
    {
        Auction memory auction = idToAuction[auctionId];
        Utils(utilsAddress).validateMakeAnOffer(
            msg.sender, 
            auction.seller, 
            auction.price, 
            auction.offerValue, 
        ...
        idToAuction[auctionId].offer = msg.sender;
        idToAuction[auctionId].offerValue = msg.value;
        ...
        //Update withdraw for old offer
        if(auction.offer != address(0)
            && auction.offerValue != 0) {
                Withdraw(withdrawAddress).registerWithdraw{value: auction.offerValue}(auction.offer);
        }
    }

    function acceptOffer(uint auctionId)
        external
        nonReentrant
        whenNotPaused
        isAuctionValid(auctionId, false)
    {
        Auction memory auction = idToAuction[auctionId];
        Utils(utilsAddress).validateAcceptOffer(
            msg.sender,
            auction.seller,
            auction.offer,
            auction.offerValue
        ...
        uint buyerFee = Utils(utilsAddress).calculateFee(auction.offerValue, feePercentage);

        //close auction
        idToAuction[auctionId].status = Status.CLOSED;
        ...
        if(receiptReceiver != address(0) 
            && receiptReceiver != auction.seller 
            && royaltyAmount > 0) 
        {
            Withdraw(withdrawAddress).registerRoyaltyWithdraw{value: royaltyAmount}(receiptReceiver);
        }

        //Update pending withdraw
        if(receiptReceiver == auction.seller) {
            Withdraw(withdrawAddress).registerWithdraw{value: auction.offerValue - buyerFee}(auction.seller);
        } else {
            Withdraw(withdrawAddress).registerWithdraw{value: auction.offerValue - buyerFee - royaltyAmount}(auction.seller);
        }
        Withdraw(withdrawAddress).registerWithdraw{value: buyerFee}(bellaAddress);

        //Transfer token
        if(auction.tokenType == TokenType.ERC721) {
            IERC721Upgradeable(auction.nftContract).safeTransferFrom(auction.seller, auction.offer, auction.tokenId);
        } else if(auction.tokenType == TokenType.ERC1155) {
            IERC1155Upgradeable(auction.nftContract).safeTransferFrom(auction.seller, auction.offer, auction.tokenId, auction.supply, "");
        }
    }
    ...
    function extendAuction(
        ...
        isAuctionValid(auctionId, false)
    {
        require(msg.value == extendPrice, string(abi.encodePacked("ERR-12-BA", ":", Strings.toString(extendPrice))));
        
        Auction memory auction = idToAuction[auctionId];
        Utils(utilsAddress).validateExtendAuction(
            msg.sender,
            auction.seller,
            auction.startTime,
            startTime,
        ...
        idToAuction[auctionId].startTime = startTime;
        idToAuction[auctionId].endTime = endTime;
        idToAuction[auctionId].hasExpirationTime = hasExpirationTime;

        emit AuctionTimeExtended(
            auctionId, 
            startTime, 
            endTime);

        Withdraw(withdrawAddress).registerWithdraw{value: msg.value}(bellaAddress);
    }

    function closeAuction(uint auctionId)
        external
        ...
        isAuctionValid(auctionId, false)
    {
        ...
        if(auction.offer != address(0) && auction.offerValue > 0) {
            Withdraw(withdrawAddress).registerWithdraw{value: auction.offerValue}(auction.offer);
        }
    }
...
contract Utils is IERC165, AccessControl {
    ...
    address private bellaAuctionAddress;
    ...
    constructor(address _bellaAuctionAddress) {
        bellaAuctionAddress = _bellaAuctionAddress;
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
    }
    ...
    function validatePutUpForAuction(
        uint tokenId, 
        address nftContract, 
        uint supply, 
        uint price, 
    ...
    {
        require(nftContract != address(0), "ERR-1-BA");
        require(contractAddressWhitelist[nftContract], "ERR-2-BA");

        if(hasExpirationTime) {
            require(startTime < endTime, "ERR-20-BA");
            require(block.timestamp < endTime, "ERR-18-BA");
        }

        require(price > 0, "ERR-3-BA");
    ...
    function validateMakeAnOffer(
        ...
        pure 
    {
        require(sender != address(0), "ERR-10-BA");
        require(seller != address(0), "ERR-11-BA");
        require(sender != seller, "ERR-10-BA");
        require(value >= price, string(abi.encodePacked("ERR-12-BA", ":", Strings.toString(price))));
        require(value > lastOfferValue, string(abi.encodePacked("ERR-12-BA", ":", Strings.toString(lastOfferValue))));
    }

    /// Validate Accept Offer.
    /// @param sender for buy transaction
    /// @param seller address
    /// @param offer address
    /// @param lastOfferValue value from offer
    /// @notice validate the data to buy an Item
    ...
    {
        require(sender != address(0), "ERR-10-BA");
        require(sender == seller, "ERR-11-BA");
        require(offer != address(0), "ERR-15-BA");
        require(seller != offer, "ERR-11-BA");
        require(lastOfferValue > 0, "ERR-16-BA");
    }

    /// Validate Extend Auction.
    /// @param sender for buy transaction
    /// @param seller address
    /// @param oldStartTime address
    /// @param newStartTime address
    /// @param oldEndTime address
    /// @param newEndTime address
    /// @notice validate the data to buy an Item
    function validateExtendAuction(
        address sender, 
        address seller, 
        uint oldStartTime,
        uint newStartTime,
        ...
        view 
    {
        require(sender != address(0), "ERR-10-BA");
        require(sender == seller, "ERR-11-BA");
        require(oldStartTime < newStartTime, "ERR-20-BA");
        require(oldEndTime < newEndTime, "ERR-18-BA");
        require(newStartTime < newEndTime, "ERR-20-BA");
        require(block.timestamp < newEndTime, "ERR-18-BA");
    }
    ...
    /// @param _bellaAuctionAddress smart contract address
    /// @notice set the bella resell address
    function setbellaAuctionAddress(address _bellaAuctionAddress) 
        external 
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        bellaAuctionAddress = _bellaAuctionAddress;
    ...
    function _validateERC1155(address nftContract, uint tokenId, uint supply, address seller) private view {
        ...
        if(!IERC1155(nftContract).isApprovedForAll(seller, bellaAuctionAddress)) {
            revert("ERR-8-BA");
        }
    }
    ...
    function _validateERC721(address nftContract, uint tokenId, address seller) private view {
        ...
        if(!IERC721(nftContract).isApprovedForAll(seller, bellaAuctionAddress)) {
            revert("ERR-8-BA");
        }
    }
