...
contract Stake is LicenseBase, Ownable, ReentrancyGuard {
  /* ========== STATE VARIABLES ========== */
  mapping(address => uint256) public stakedBalanceOf;
  uint256 public minStakeAmount;
  uint256 public dimoTotalAmountStaked;
  string baseUri;
  uint256 public dimoMintCost;
  uint256 tokenId;
  Dimo dimoToken;
  address public AFTERMARKET_CONTRACT;
  ...
  // needs the already deployed DIMO token address when finished
  constructor(
    address tokenAddress,
    address aftermarketContract,
    address foundationAddress
  ) LicenseBase("Dimo License", "DIMO", "0.1") {
    dimoToken = Dimo(tokenAddress);
    minStakeAmount = 100000 * 10**18; // 100,000
    baseUri = "https://dimo.zone/";
    dimoMintCost = 50 * 10**18;
    AFTERMARKET_CONTRACT = aftermarketContract;
  ...
  modifier onlyAftermarketContract() {
    require(msg.sender == AFTERMARKET_CONTRACT, "wrong contract");
    _;
  }
  ...
  /// The approval tx needs to be executed in advance
  ...
  function stake(uint256 _amount) external nonReentrant {
    ...
    // check that the token approval amount is at least the minStakeAmount.
    require(dimoToken.allowance(msg.sender, address(this)) >= minStakeAmount, "Token allowance too low");

    // token has to send  before continuing the function
    bool tokenSuccess = dimoToken.transferFrom(msg.sender, address(this), _amount);
  ...
  function setNewMinStakeAmount(uint256 _amount) external onlyOwner {
  ...
  function unstake(address user, uint256 _amount) external onlyOwner {
    // update the balance of the user's staked tokens
    require(stakedBalanceOf[user] >= _amount, "User does not have a staked balance");
    // stakedBalanceOf[user] -= _amount;
    // dimoTotalAmountStaked -= _amount;

    dimoToken.transfer(user, _amount);
  ...
  function mint(address to) external onlyOwner {
    ...
    // wip
    super._mint(to, newTokenId, baseUri);
...
import {ILicenseBase} from "./interfaces/ILicenseBase.sol";

bytes32 constant AGREEMENT_HASH = keccak256(
    "Agreement(address active,address passive,string tokenURI)"
);

abstract contract LicenseBase is EIP712, ERC165, IERC721Metadata, ILicenseBase {
        ...
        returns (bool)
    {
        return
            ...
            interfaceId == type(ILicenseBase).interfaceId ||
...
interface ILicenseBase {
    /// @dev This emits when ownership of any License changes by any mechanism.
    ///  This event emits when Licenses are given or equipped (`from` == 0) and
    ///  unequipped (`to` == 0).
    event Transfer(
    ...
    /// @notice Count all Licenses assigned to an owner
    /// @dev Licenses assigned to the zero address are considered invalid, and this
    ///  function throws for queries about the zero address.
    /// @param owner An address for whom to query the balance
    /// @return The number of Licenses owned by `address owner`, possibly zero
    function balanceOf(address owner) external view returns (uint256);

    /// @notice Find the address bound to a License
    /// @dev Licenses assigned to zero address are considered invalid, and queries
    ///  about them do throw.
    /// @param tokenId The identifier for a License.
    /// @return The address of the owner bound to the License.
