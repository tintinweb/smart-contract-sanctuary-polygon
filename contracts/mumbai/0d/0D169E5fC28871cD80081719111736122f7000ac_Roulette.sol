...
contract Roulette is  VRFConsumerBaseV2, Ownable {
  ...
  */
  ...
  modifier betFulfilled() {
    Bet memory b = betIdToBets[currentMultiBetId];
  ...
  */
   ...
   */
    function multiPlayerBet(BetTypes[] memory _betType, uint8[] memory number, bool _isMatic, address ERC20Address, uint256[] memory amount) public payable {
      require(currentMultiBetId > 0, "Multi Player bets are yet to be started");
      require(msg.value <= maxBet, "Maximum allowed bet is 500");
  ...
  */
    function multiPlayerPayOut(uint256 _requestId) public betFulfilled  {
     uint256 _betId = requestIdToBetId[_requestId]; 
  ...
  Adds the winning amount to the user winnings.
  */
    function singlePlayerPayOut(uint256 _requestId) public  {
      uint256 _betId = requestIdToBetId[_requestId]; 
      UniBet storage u = uniBetId[_betId];
