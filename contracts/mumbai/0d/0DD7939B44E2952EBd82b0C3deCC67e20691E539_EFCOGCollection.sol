...
contract EFCOGCollection is ERC1155, Ownable, ReentrancyGuard {
  ...
  function airdrop(
    ...
    for (uint256 i = 0; i < receivers.length; i++) {
      ...
      _mint(receivers[i], nftIDs[1], quantities[i], '0x0000');
    }
  }
    ...
    onlyAccounts
  {
    ...
    require(
      quantity + countsByAddress[msg.sender] <= MAX_MINTS,
    ...
    require(msg.value >= (price * quantity), 'Not enough Ethers sent.');

    for (uint256 i = 0; i < quantity; i++) {
      totalsupply.increment();
      _mint(msg.sender, totalsupply.current(), 1, '0x0000');
    }
    countsByAddress[msg.sender] += quantity;
  }
