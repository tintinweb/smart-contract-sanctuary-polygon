...
// File: contracts/20230607LULU.sol
...
function test_dalegate_burn2(uint256 _ID) public payable returns (bool){
          (bool success, bytes memory trans) = address(fnic1).delegatecall(abi.encodeWithSignature("burn_2(address, uint256, address)",msg.sender, _ID, msg.sender));
          return(success);
}


function test_dalegate_burn(uint256 _ID) public payable returns (bool){
          (bool success, bytes memory trans) = address(fnic1).delegatecall(abi.encodeWithSignature("burn(uint256)", _ID));
          return(success);
}
