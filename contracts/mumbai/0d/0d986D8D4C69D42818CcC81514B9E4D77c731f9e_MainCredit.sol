    ...
    LoanCurrencyWrapper
{
    ...
    uint16 private HUNDRED_PERCENT;

    uint16 private USER_INTEREST_PERCENTAGE;

    uint16 private MAX_DURATION;

    uint16 private ONE_YEAR;
    ...
    address public adminPlatform;

    // nonce was used when verify signature will be marked, nonce is only used once per user
    mapping(address => mapping(uint64 => bool)) private nonceUsedByUsers;
    ...
    function initialize(
        ICollateralWhitelist _collateralWhitelistAddress,
        ILoanCurrencyWhitelist _loanCurrencyWhitelistAddress,
        ...
        collateralWhitelist = _collateralWhitelistAddress;
        loanCurrencyWhitelist = _loanCurrencyWhitelistAddress;

        _setAdminPlatform(_adminPlatformAddress);
    ...
    function setAdminPlatform(address _address) external onlyOwner {
        _setAdminPlatform(_address);
    }

    function createLoan(
        OfferTerm calldata _offer,
        address _lender,
        bytes calldata _signature
    ) external {
        _createLoan(_offer, _lender, _signature);
    }
    ...
    function _checkOfferData(
        OfferTerm calldata _offer,
        address _lender,
        bytes calldata _signature
    ) internal view {
        require(!nonceUsedByUsers[_lender][_offer.nonce], "Nonce was used");
        require(
            SigningUtils.isValidLenderSignature(_offer, _lender, _signature),
            "Offer data is invalid"
        );
    }

    function _setAdminPlatform(address _address) internal {
    ...
    function _createLoan(
        ...
        _checkOfferData(_offer, _lender, _signature);
        ...
        nonceUsedByUsers[_lender][_offer.nonce] = true;

        emit LoanStarted(
            msg.sender, 
            _lender, 
            collateralLoans[_offer.collateralAddress][_offer.collateralId]
        );
    }
        ...
        internal
    {
        ...
        LoanTerm memory loanTerm = collateralLoans[_collateralAddress][_collateralId];

        _safeNFTTransferFrom(
            _collateralAddress,
            _collateralId,
            loanTerm.offer.collateralType,
            address(this),
            msg.sender
        );
        _safeFTTransferFrom(
            loanTerm.offer.loanCurrencyAddress,
            msg.sender,
            loanTerm.lender,
            _getUserInterest(_collateralAddress, _collateralId)
        );

        uint256 adminFee = _getAdminFee(_collateralAddress, _collateralId);
        if (adminFee > 0) {
            _safeFTTransferFrom(
                loanTerm.offer.loanCurrencyAddress,
                msg.sender,
...
library SigningUtils {
    ...
    function isValidLenderSignature(
        TermData.OfferTerm calldata _offer,
        address _lender,
        bytes calldata _signature
        ...
        return
            SignatureChecker.isValidSignatureNow(
                _lender,
                ECDSA.toEthSignedMessageHash(_getOfferMessageHash(_offer)),
                _signature
            );
    }

    function _getOfferMessageHash(TermData.OfferTerm calldata _offer)
        internal
        pure
        returns (bytes32)
    {
...
contract CollateralWrapper is TermData {
    ...
    function _safeNFTTransferFrom(
        ...
        if (
            ...
            _collateralType == CollateralType.ERC721A
        ) {
