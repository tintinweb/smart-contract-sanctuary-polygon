...
contract Gixx is Ownable  {
    ...
    function depositUsdt (uint256 numberOfUsdt) public returns (uint256 id) {
        ...
        uint256 final_amt = amt * 10**18;

        id = ++depositId;
        deposithistory[id].user_address = msg.sender;
        deposithistory[id].usdt_amt = final_amt;
        emit DepositRecord(msg.sender,  final_amt, depositId);
               
        IERC20(usdt_address).transferFrom(msg.sender, address(this), final_amt);
