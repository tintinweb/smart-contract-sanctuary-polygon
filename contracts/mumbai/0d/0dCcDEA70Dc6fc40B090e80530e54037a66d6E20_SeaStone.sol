...
contract SeaStone is ERC721A, Ownable, PaymentSplitter, ReentrancyGuard {
    ...
    function mint(uint8 v, bytes32 r, bytes32 s, uint256 amount, uint256 allowedAmount) public payable {
        ...
        require(!Address.isContract(msg.sender), 'CONTRACT ERROR: contracts are not allowed to mint.');
        require(claimedTokens[msg.sender] + amount <= allowedAmount, "CONTRACT ERROR: Address has already claimed max amount");
        require(amount > 0, "CONTRACT ERROR: must specify at least 1 token");
        require(amount * cost == msg.value, "CONTRACT ERROR: incorrect amount of ether sent");
        require(totalSupply() + amount <= maxSupply, "CONTRACT ERROR: not enough remaining in supply to support desired mint amount");
        require(_verify(_hash(msg.sender, amount, allowedAmount), v, r, s), 'CONTRACT ERROR: Invalid signature');
