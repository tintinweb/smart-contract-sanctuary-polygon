 ...
 */
contract TombStaking is ERC721Holder, Ownable {
   ...
   *
   * @param _goldAddress address of  rewards token
   * @param _tombAddress address of  token to be staken for rewards
  ...
  constructor(
    ...
    address _tombAddress,
    uint128 _dailyRewards
  ) {
    gold = IMintableERC20(_goldAddress);
    tomb = IERC721(_tombAddress);
...
//   }
  ...
  function stake(uint256[] memory _tokenIds) public {
    ...
    for (uint256 i = 0; i < _tokenIds.length; i++) {
      ...
      // behalf
      tomb.safeTransferFrom(msg.sender, address(this), tokenId, "");
  ...
  function unstake(uint256[] memory _tokenIds) public {
    ...
    for (uint256 i = 0; i < _tokenIds.length; i++) {
      ...
      tomb.transferFrom(address(this), msg.sender, tokenId);
    }

    // Mint total rewards for all sent NFTs
    // to user
    gold.mint(msg.sender, totalRewards);
  }
}