...
contract RewardsRankingFound is Ownable{
    ...
    event AddresseUpdated(address ranking, address pvpAddress, address pveAddress);
    event BalancerToPvp(uint256 amount);
    event BalancerToWinPve(uint256 amount);
    event RewardRankingPoolIncreased(uint256 amount);
    event PercentageRewardsUpdated(uint256 dayOldMetric,uint256 dayNewMetric,uint256 weekOldMetric,uint256 weekNewMetric);


    constructor(address _BZAI, address _reserve, uint256 _startingTimestamp) {
        BZAI = IERC20(_BZAI);
    ...
    function updateInterfaces() external {
        rankingContract = gameAddresses.getAddressOf(AddressesInit.Addresses.RANKING);
        pvpRewardAddress = gameAddresses.getAddressOf(AddressesInit.Addresses.REWARDS_PVP);
        pveRewardAddress = gameAddresses.getAddressOf(AddressesInit.Addresses.REWARDS_WINNING_PVE);
        emit AddresseUpdated(rankingContract, pvpRewardAddress, pveRewardAddress);
    }
    ...
    function balancerToPvpReward(uint256 _amount) external onlyOwner {
        ...
        require(_amount <= BZAI.balanceOf(address(this)) / 10, "not much than 10%");
        require(block.timestamp >= lastBalancerCalled + 1 days, "too soon to balance poolz");
        ...
        onlyOwner
    {
        ...
        emit PercentageRewardsUpdated(dayOldMetric, _daily, weekOldMetric, _weekly);
    }
        ...
        returns (uint256)
    {
        ...
        require(dayBeginingTimestamp != _dayBeganAt,"Reward already sent today");
        ...
        returns (uint256)
    {
        ...
        require(weekBeginingTimestamp != _weekBeganAt,"Reward already sent this week");
