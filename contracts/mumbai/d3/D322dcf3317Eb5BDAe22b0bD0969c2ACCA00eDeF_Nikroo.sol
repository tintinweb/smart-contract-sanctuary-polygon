...
contract Nikroo is Ownable, ReentrancyGuard, IERC721, IERC721Metadata, IERC721Enumerable {
    using SafeMath for uint256;
    using Strings for uint256;
    using EnumerableSet for EnumerableSet.UintSet;

    string private constant _name = "Nikroo";
    string private constant _symbol = "NIK";
    ...
    address payable internal immutable _wallet1;

    uint256 private mintingLevelPrices = 250 ether;
    ...
    uint public priceToken = 0.02 ether;
    ...
    function getMintPrice() public view returns (uint256 price) {
        ...
        // require(currentSupply + amount <= MAX_SUPPLY, "sale already ended");

        price += priceToken;

    }

    function mint(uint tokenId) external payable nonReentrant mintingEnabled {
        require(tokenId >= 1 && tokenId <= MAX_SUPPLY , "amountLimit");
        require((totalMinted + 1) <= MAX_SUPPLY, "exceedsMaxSupply");

        uint256 nftPrice = getMintPrice();

        require(msg.value >= nftPrice, "notEnough");

        if (msg.value > nftPrice) {
            (bool send, ) = msg.sender.call{ value: msg.value - nftPrice }("");
            require(send, "refund");
        }

        uint256 accountFee = nftPrice ;
       
        (bool sentOne, ) = _wallet1.call{ value: accountFee }("");
        require(sentOne, "fee failed");
    ...
    function _mint(address _to , uint tokenId) internal notZeroAddress(_to) returns (uint256 _mintedTokenId) {
        require(totalMinted < MAX_SUPPLY, "allMinted");

        // uint randomId = generateRandom();
    ...
    function generateRandom() internal returns (uint256) {
        uint256 maxIndex = MAX_SUPPLY + startFrom - totalMinted;
    ...
    function addNft(uint256 count) public onlyOwner {
        // tokenToOwner[_tokenId] = _to;
        // ownerToTokensSet[_to].add(_tokenId);
    ...
    function secondLaunch(uint256 max_supply) external onlyOwner {
        require(max_supply > MAX_SUPPLY && max_supply > totalSupply(), "invalid");
        startFrom = MAX_SUPPLY;
    ...
    function _setMintingPrice(uint256[] calldata newPrices) external onlyOwner {
        require(newPrices.length == 2, "length");
        require(newPrices[0] > 0 && newPrices[1] > 0, "invalid");
        // mintingLevelPrices = [newPrices[0], newPrices[1]];
    }

    function _claimETC(uint256 amount) external onlyOwner {
    ...
    modifier mintingEnabled() {
        require(isMintingEnabled, "mintDisabled");
        _;
    }
