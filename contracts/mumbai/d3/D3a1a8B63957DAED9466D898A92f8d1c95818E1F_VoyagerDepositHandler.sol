...
contract VoyagerDepositHandler is AccessControl, ReentrancyGuard, Pausable {
    ...
    address public immutable _reserve;
    ...
    constructor(bytes memory depositArgs) {
        (_WETH, _usdc, _reserve) = abi.decode(depositArgs, (address, address, address));
    ...
    function setLiquidityPool(address contractAddress, address lpAddress) public onlyRole(RESOURCE_SETTER) {
        ...
        // _contractWhitelist[lpAddress] = true;
        // _burnList[lpAddress] = true;
        Setter.setLiquidityPool(IHandlerReserve(_reserve), _contractWhitelist, _burnList, contractAddress, lpAddress);
    }
    ...
    function setLiquidityPoolOwner(
        ...
        IHandlerReserve(_reserve)._setLiquidityPoolOwner(oldOwner, newOwner, tokenAddress, lpAddress);
    }
    ...
    function checkBurnableAndLock(address token, address user, uint256 amount) internal {
        if (!_burnList[token]) {
            IERC20(token).safeTransferFrom(user, _reserve, amount);
        } else {
            IHandlerReserve(_reserve).burnERC20(token, user, amount);
        }
    }
