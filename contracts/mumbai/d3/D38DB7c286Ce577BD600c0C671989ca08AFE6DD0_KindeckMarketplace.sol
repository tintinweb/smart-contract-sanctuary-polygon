...
error FluidManagerNotFound(address sellerAddress);
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize(
        ...
        kindeckInception = IKindeckInception(_kindeckInception);
        ...
        isOwner(nftAddress, tokenId, msg.sender)
    {
        ...
        //Used as check to see that manager address exists
        getManagerAddress(msg.sender);
    ...
    function buyItem(
    ...
    ) internal isListed(nftAddress, tokenId) nonReentrant {
        address sellerFluidManager = getManagerAddress(listedItem.seller);
        delete (s_listings[nftAddress][tokenId]);
        token.send(sellerFluidManager, listedItem.price, userData);
        ...
        emit ItemBought(to, nftAddress, tokenId, listedItem.price);
    }

    function getManagerAddress(address _seller)
        internal
        view
        returns (address)
    {
        address manager = kindeckInception.getTokenManagerAddress(_seller);
        if (manager == address(0)) {
            revert FluidManagerNotFound(msg.sender);
        }
        return manager;
    }
    ...
    function tokensReceived(
        ...
        if (listedItem.price != amount) {
            revert PriceNotMet(nftAddress, tokenId, listedItem.price);
        }
        buyItem(nftAddress, tokenId, listedItem, from, userData);
    }
}
