...
contract RandomWinnerGame is VRFConsumerBase, Ownable {
    ...
    //emitted when some join a game
    event playerJoined(uint256, address player);
    //emitted when the games ends
    event GameEnded(uint256 gameId, address winner, bytes32 requestId);

    /**
    * constructor inherits a VRFConsumerBase and initiates the values for keyHash, fee and gameStarted
    * @param vrfCoordinator address of VRDCoordinator contract
    * @param linkToken address of LINK token contract
    * @param vrfFee the amount of LINK to send with the request
    * @param vrfKeyHash ID of public key against which randmoness is generated
     */
     ...
     /**
     * startGame: starts the game by setting appropriate values for all the variabes
      */
      function startGame(uint8 _maxPlayers, uint256 _entryFee) public onlyOwner {
          //Check if there is a game already running 
          require(!gameStarted, "Game currently running");
      ...
      * joinGame is called when a player wants to enter the game
       */
       function joinGame() public payable {
           ...
           // Check if the value sent by the user matches the entreeFee
           require(msg.value == entryFee, "Value sent is not equal to entryFee");
           // Check if there is still space left in the game to add anothe player
       ...
       /**
       * fulfillRandomness is called by VRFCoordinator when it recevies a valid VRF proof.
       * This function is overrided to act upon the random number generated bt Chainlink VRF.
       ...
       * @param randomness this is a random uint256 generated and returned to us by the VRF Coordinator.
        */
        function fulfillRandomness(bytes32 requestId, uint256 randomness) internal virtual override {
            ...
            // get the adress of the winner from the players array
            address winner = players[winnerIndex];
            // send the ether in the contract to the winner
            (bool sent,) = winner.call{value: address(this).balance}("");
            require(sent, "Faild to send the ether");
    ...
    * getRandomWinner is called to start the process of selecting a random winner
     */
     function getRandomWinner() private returns (bytes32 requestId) {
         ...
         // Here use the balanceOF method from that interface to make sure that our
         // contract has enough link so tht we can request VRFCoordinator from randomness
         require(LINK.balanceOf(address(this)) >= fee, "Not enough LINK");
         // Make a request to the VRF coordinator 
         // requestRandomness is a function wwithin the VRFConsumerBae
