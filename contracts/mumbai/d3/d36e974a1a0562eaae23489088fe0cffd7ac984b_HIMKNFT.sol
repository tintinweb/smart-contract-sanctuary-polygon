...
contract HIMKNFT is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    uint256 public A = 1*10**18;
    uint256 public B = 3*10**18;
    ...
    function setType(uint256 _tokenId, cardType _type) public {
        ...
        //设置卡的类型,直接加
        if (_type == cardType.Bronze) {
            _burn(_tokenId);
            _safeMint(owner, A+_tokenId);
        }
        if (_type == cardType.Silver) {
            _burn(_tokenId);
            _safeMint(owner, A+_tokenId);
        }
        if (_type == cardType.Gold) {
            _burn(_tokenId);
            if (_tokenId<A)
            _safeMint(owner, B+_tokenId); 
            else
            _safeMint(owner, A+_tokenId); 
        }
