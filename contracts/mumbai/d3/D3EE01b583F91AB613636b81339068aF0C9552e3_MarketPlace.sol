...
contract MarketPlace is ERC2771Context,IAuctionHouse, ReentrancyGuard,Whitelist,ERC721Holder{
    ...
    modifier onlyValidUser() {
        require(owner() == _msgSender() || !isAuctionRestricted,"Only Valid user can create auction");
        _;
    }
    ...
    function createAuction(
        ...
        uint256 duration,
        uint256 reservePrice
    ) public nonReentrant isWhitelisted(tokenContract) onlyValidUser() returns (uint256) {
        require(
            IERC165(tokenContract).supportsInterface(interfaceId),
            "tokenContract does not support ERC721 interface"
        );
    ...
    */
    ...
    function changeAuctionCreatorState(bool _isAuctionRestricted) external onlyOwner {
        require(isAuctionRestricted != isAuctionRestricted,"state is already same");
