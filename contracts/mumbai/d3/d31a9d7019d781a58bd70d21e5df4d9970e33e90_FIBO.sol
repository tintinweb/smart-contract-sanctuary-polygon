...
contract FIBO is ERC20, Ownable {
    ...
    uint256 public MAX_TAX = 5000; // 50%
    ...
    event TransferWithSupportingFee(
        address indexed sender, address indexed receiver, uint256 amount, uint256 feeAmount, address treasury
    );
     ...
     * we need to take the corresonding fee amount and transfer them to the treasury
     *
     * - MUST emit the TransferWithSupportingFee event
     */
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        emit TransferWithSupportingFee(sender, recipient, amount, feeAmount, treasury);
    }

    /**
     * @dev Hooks each transfer to get the fee amount of FIBO token
     *
     ...
     */
    function _hookTransfer(address sender, address recipient, uint256 amount)
        internal
        returns (uint256)
    {
        if (excluded[sender] || excluded[recipient]) return amount;
        TransferType transferType =
            exchanges[sender] ? TransferType.BUY : exchanges[recipient] ? TransferType.SELL : TransferType.TRANSFER;
        if (transferType == TransferType.TRANSFER) return amount;
