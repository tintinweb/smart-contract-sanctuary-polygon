...
contract OLNameStore is Ownable, OLNameStoreInterface {
    ...
    mapping(address => bool) private addressRegistry;
    ...
    function getPermissions(address user) external view override returns (uint8) {
        return accessList[user];
    }

    function existsRegistry(bytes32 key) external view override returns (bool) {
        return registryMap[key].regAddress != address(0);
    }

    function isAddressRegistered(address regAddress) external view returns (bool) {
        return addressRegistry[regAddress] == true;
    }

    function clearAddressRegistry(address regAddress) external {
        if (addressRegistry[regAddress]) {
            delete addressRegistry[regAddress];
        }
    }

    function getRegistry(bytes32 key) external view override returns (Registry memory) {
    ...
    function setRegistry(bytes32 key, Registry calldata registry) external override {
        requireOwnerWriter();
        registryMap[key] = registry;
        addressRegistry[registry.regAddress] = true;
        emit SetRegistry(key, registry.regAddress);
    }

    event DeleteRegistry(bytes32 key, address addr);

    function deleteRegistry(bytes32 key) external override {
        requireOwnerWriter();
