...
contract Swapient {
    ...
    function refundNativeDeposit(
        uint256 _depositId
    ) external correctDepositor(_depositId) isNativeDeposit(_depositId) {
        Deposit storage deposit = deposits[_depositId];
    ...
    function refundERC20Deposit(
        uint256 _depositId
    ) external correctDepositor(_depositId) isERC20Deposit(_depositId) {
        Deposit storage deposit = deposits[_depositId];
        require(deposit.amount > 0, "ERR__ZERO_AMOUNT");
    ...
    function refundAddressedNativeDeposit(
        ...
        external
        correctDepositor(_addressedDepositId)
