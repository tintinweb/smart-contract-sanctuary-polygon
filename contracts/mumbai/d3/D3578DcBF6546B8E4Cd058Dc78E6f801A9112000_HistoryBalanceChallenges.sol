...
contract ChallengeDetail is IERC721Receiver, IChallenge{
    ...
    */
     ...
     */
    function getChallengeInfo() external view returns(
    ...
    function updateBalance() private {
        balanceToken.push(address(this).balance);
        for(uint256 i = 0; i < erc20ListAddress.length; i++) {
            balanceToken.push(getBalanceTokenOfContract(erc20ListAddress[i], address(this)));   
        }
    }
    ...
    function getHistoryTokenAndCoinSendToContract(address payable _contractChallengeAddress) public view returns(
        ...
        if(isCoinChallenges) {
            ...
            if(ChallengeDetail(_contractChallengeAddress).isChallengeFinish()) {
                uint256[] memory balanceCoinAndToken = ChallengeDetail(_contractChallengeAddress).getBalanceToken();
                uint256[] memory balanceTokenTemp = new uint256[](balanceCoinAndToken.length.sub(1));
                balanceTokenTemp[0] = balanceCoinAndToken[1];
                balanceTokenTemp[1] = balanceCoinAndToken[2];
                return(totalReward, balanceCoinAndToken[0].sub(totalReward), tokenBalanceBefor, balanceTokenTemp, listTokenSymbol);
            } else {
            ...
            if(ChallengeDetail(_contractChallengeAddress).isChallengeFinish()) {
                uint256[] memory balanceCoinAndToken = ChallengeDetail(_contractChallengeAddress).getBalanceToken();
                uint256[] memory balanceTokenTemp = new uint256[](balanceCoinAndToken.length.sub(1));
                if(balanceCoinAndToken[1] > totalReward) {
                    balanceTokenTemp[0] = balanceCoinAndToken[1].sub(totalReward);
                }
                balanceTokenTemp[1] = balanceCoinAndToken[2];
                return(0, balanceCoinAndToken[0], tokenBalanceBefor, balanceTokenTemp, listTokenSymbol);
            } else {
