...
contract Dao {
    ...
    constructor(address _NFTContractAddress){
        owner = msg.sender;
        nextProposal = 1;
        daoContract = IdaoContract(_NFTContractAddress);    //to be changed
        validTokens = [0];
    }
    ...
    function createProposal(string memory _description, address[] memory _canVote) public onlyOwner{
        ...
        newProposal.deadline = block.timestamp + 1 days;                 //to be changed
    ...
    function countVotes(uint256 _id) public onlyOwner {
        require(Proposals[_id].exists, "This Proposal does not exist.");
        require(block.timestamp > Proposals[_id].deadline, "Voting has not yet completed for this proposal.");
    ...
    function setNFTAddress(address _nftAddress) public onlyOwner{
        daoContract = IdaoContract(_nftAddress);
    }
