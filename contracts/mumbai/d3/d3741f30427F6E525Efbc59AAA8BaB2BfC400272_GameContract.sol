...
contract GameContract is Ownable{
    ...
    constructor(IERC20Charity _Token) {
        TokenP2E = _Token;
        level memory L = level(
            28*10**18,
            5*10**18,
            33000,
            5000,
    ...
    function ChangeLevel(level calldata L) onlyOwner public{
        Level =L;
    }
    ...
    function gameOver(string calldata _time, uint64 _milliseconds, uint64 _score, address _from, string calldata _email) onlyOwner public{
        ...
        _score = _score > Level.score_max ? Level.score_max:_score;
        uint Reward = Level.reward_score*_score/ Level.score_max;
        if(_milliseconds <= Level.time_min){
            Reward += Level.reward_time;
        }else{
            uint64 number_penalty = 1+(_milliseconds - Level.time_min)/Level.palier;
            if(number_penalty<(Level.reward_time/Level.penality)){
                Reward += (Level.reward_time - number_penalty*Level.penality);
            }else{
                Reward += Level.reward_base;
            }
        }
        Reward = TokenP2E.balanceOf(GameAddress) > Reward ?Reward:TokenP2E.balanceOf(GameAddress);
