...
/**
 Writers create Publication NFT-s. These are owned by writers
 Publications to store publication metadata.
 */
contract Publication is ERC721Enumerable, AccessControl, IPublication {
    event PublicationAdded(uint256 tokenId);

    bytes32 public constant WRITER_ROLE = keccak256("WRITER_ROLE");

    uint256 s_tokenCounter;

    mapping(uint256 => PublicationInfo) s_publications;
    ...
    /**
     * Add a new writer that can add new publications.
     */
    ...
    function setMintFee(uint256 publicationId, uint256 mintFee) external {
        // check if sender is owner of token
        require(ownerOf(publicationId) == msg.sender, "Only owner can set mint fee");
        s_publications[publicationId].mintFee = mintFee;
    }
    ...
    /**
     * Add a publication.
     * @param publication IPFS uri with metadata and image url.
     */
    function addPublication(PublicationInfo memory publication) public onlyRole(WRITER_ROLE) {
        uint256 tokenId = ++s_tokenCounter;
        _mint(msg.sender, s_tokenCounter);
        s_publications[tokenId] = publication;
        emit PublicationAdded(s_tokenCounter);
    }

    /**
     * IPublication
     */
    function getPublicationInfo(uint256 tokenId)
        public
        view
        override
        returns (PublicationInfo memory)
    {
        if (!_exists(tokenId)) revert Errors.TokenDoesNotExist();
        return s_publications[tokenId];
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        IPublication.PublicationInfo memory publication = getPublicationInfo(tokenId);
    ...
    function _beforeTokenTransfer(
        ...
        if (from != address(0x0) && to != address(0x0)) revert Errors.NonTransferrable();
...
interface IPublication is IERC721Time {
    struct PublicationInfo {
        string uri;
        uint256 mintFee;
    }

    function getPublicationInfo(uint256 tokenId) external view returns (PublicationInfo calldata);
}
