...
contract Exchange is ERC20 {
    ...
    constructor(address _address) ERC20("CryptoDev LP Token", "CDLP") {
        require(_address != address(0), "Address could not be empty");
        cryptoDevsTokenAddress = _address;
    }

    // create a function to get the reserved amount cryptodevs token held in this
    // smart contract
    ...
    function addLiquidity(uint256 _amount) public payable returns (uint256) {
        ...
        if (cryptoDevsTokenReserve == 0) {
            ...
            uint cryptoDevTokenAmount = (msg.value * cryptoDevsTokenReserve) /
                (ethReserve);
            require(
                _amount >= cryptoDevTokenAmount,
                "Amount of token is less than the minimum required"
            );
