...
contract Game is IGame, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    modifier nonZero(uint256[12] memory positions) {
        ...
        require(valuePosition > 0 && !invalid, "missiles");
        _;
    }
    ...
    function arrangeMissile(uint256[12] memory positions) external override nonZero(positions) nonReentrant whenNotPaused {
        ...
        if (blacklistIds[msg.sender] != 0) {
            ...
            require(blacklistInfos[blacklistIds[msg.sender]].spent + uint128(price) <= blacklistInfos[blacklistIds[msg.sender]].limitSpend, "limit");
       ...
       for (uint256 i = 0; i < countTypeMissile + 1; i++) {
            if (amountAff[i] > 0) {
    ...
    function shoot() external override nonReentrant whenNotPaused {
        if (blacklistIds[msg.sender] != 0) {
            ...
            require(blacklistInfos[blacklistIds[msg.sender]].numberUsed < blacklistInfos[blacklistIds[msg.sender]].limitNumber, "limit");
    ...
    function clickAffiliate(address endUser, address publisher) external onlyOperator {
        ...
        freeAffiliate[publisher][typeFreeAffiliate] += numberFreeAffilate;
    }
