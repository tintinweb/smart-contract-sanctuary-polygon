...
contract MyNFT is ERC721Drop, PlatformFee {
...
function tokenMint(uint256 tokenId, Dragon[] memory dragons) public onlyOwner {
    require(!_exists(tokenId), "Token already exists");
    require(dragons.length == 3, "Provide Dragon Details for Dragon, Matron, and Sire");

    // Store the dragon details
    _parentals[tokenId] = dragons[0];

    // Check if the dragon is not generation 0
    if (dragons[0].generation != 0) {
        require(dragons[1].matronId != 0 && dragons[2].sireId != 0, "Provide valid Dragon Details for Matron and Sire");

        // Store the parent details
        _parentals[dragons[0].matronId] = dragons[1];
        _parentals[dragons[0].sireId] = dragons[2];
    }

    // Continue with the minting process
    _bridgeMint(msg.sender, tokenId);
}
function prepBridge(uint256 tokenId) public view returns (Dragon[] memory) {
        require(_exists(tokenId), "Dragon does not exist");
        require(block.timestamp > isBreeding[tokenId], "Dragon is still in Breeding cooldown");
        Dragon[] memory dragons = new Dragon[](3);

        // Check if the dragon is not generation 0
        if (dragons[0].generation != 0) {
            // Fill in the parent details
            dragons[0] = _parentals[tokenId];  // Main dragon
            dragons[1] = _parentals[_parentals[tokenId].matronId];  // Matron dragon
            dragons[2] = _parentals[_parentals[tokenId].sireId];    // Sire dragon
        }
        return dragons;
    }
...
contract ERC721C is Context, ERC165, IERC721A {
    ...
    // BaseURI
    string internal _baseuri = "http://dijt.io/api/dragons/";
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return bytes(baseURI).length != 0 ? string(abi.encodePacked(baseURI, TWStrings.toString(tokenId))) : "";
    }
