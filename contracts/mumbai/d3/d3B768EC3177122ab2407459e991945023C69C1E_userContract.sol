...
contract userContract is Initializable{
    ...
    function initialize(address _userOwner, address _whitelistedAddress) external initializer{
    ...
    function addOrder(address erc20, int256 amt) external onlyOwner{
        ...
        bool isTrue = whitelistedTokens(whitelistedAddress).isWhitelisted(erc20); 
    ...
    function executeOrder(uint256 orderId,address addressOfexchange, uint amountOutMin,
        address[] calldata path,
        address to,
        ...
        uint[] memory amount = uniswapInterface.swapExactTokensForTokens(amountIn,amountOutMin,path,to,deadline);
