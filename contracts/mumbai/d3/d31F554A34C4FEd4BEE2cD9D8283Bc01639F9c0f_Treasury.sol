...
import "./interfaces/IISAERC20.sol";
...
contract Treasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SISA }
    ...
    address public sISA;   // staking ISA
    uint public sISAQueue; // Delays change to sISA address
    ...
    function setISA(address _isa) external onlyOwner {
        require(_isa != address(0));
        ISA = _isa;
    }

    /**
        @notice allow approved address to deposit an asset for ISA
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint ISA needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( ISA ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn ISA for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IISAERC20( ISA ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sISA ).balanceOf( msg.sender ); // Can only borrow against sISA held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with ISA
        @param _amount uint
     */
    function repayDebtWithISA( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IISAERC20( ISA ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, ISA, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( ISA ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( ISA ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns ISA valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match ISA decimals
            value_ = _amount.mul( 10 ** IERC20( ISA ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyOwner() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SISA ) { // 9
            sISAQueue = block.number.add( blocksNeededForQueue );
        } else return false;
