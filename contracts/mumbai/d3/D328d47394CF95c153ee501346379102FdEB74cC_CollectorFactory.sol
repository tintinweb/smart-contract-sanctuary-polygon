...
contract CollectorFactory is Ownable, ReentrancyGuard, AccessControlEnumerable, IFactory, Pausable {
    ...
    uint internal portable_rand;        //便携采集器随机数
    uint internal efficient_rand;       //高效采集器随机数
    uint internal portable_count = 0;   //便携采集器总数
    uint internal efficient_count = 0;  //高效采集器总数
...
/*
        if (_type_id == 0) {
            duration = 195 + randomResult % 5;
            oec_rate = 45 + randomResult % 5;
            scan = 45 + randomResult % 5;
            uru_rate = collector.prop_sum - duration - oec_rate - scan;
            if (uru_rate>=100) {
                uint diff = uru_rate - 99;
                uru_rate = 99; 
                duration += diff;
            }

        } else if (_type_id == 1) {
            duration = 95 + randomResult % 5;
            oec_rate = 145 + randomResult % 5;
            scan = 45 + randomResult % 5;
            uru_rate = collector.prop_sum - duration - oec_rate - scan;
            if (uru_rate>=100) {
                uint diff = uru_rate - 99;
            ...
            duration = 95 + randomResult % 5;
            uru_rate = 145 + randomResult % 5;
            scan = 45 + randomResult % 5;
            oec_rate = collector.prop_sum - duration - uru_rate - scan;
            if (oec_rate>=100) {
                uint diff = oec_rate - 99;
                oec_rate = 99; 
                uru_rate += diff;
            }
        } 
        uint256 potential = 1 + uint256(keccak256(abi.encode(randomResult, 5))) % 5; //潜力1-5
*/
        ...
        if (settings[_type_id]["oec_rate"].section > 0) {
            props["oec_rate"]  = settings[_type_id]["oec_rate"].base + randomResult % settings[_type_id]["oec_rate"].section;
        }
        
        if (settings[_type_id]["uru_rate"].section > 0) {
            props["uru_rate"]  = settings[_type_id]["uru_rate"].base + randomResult % settings[_type_id]["uru_rate"].section;
        }

        if (settings[_type_id]["scan"].section > 0) {
            props["scan"]  = settings[_type_id]["scan"].base + randomResult % settings[_type_id]["scan"].section;
        }
