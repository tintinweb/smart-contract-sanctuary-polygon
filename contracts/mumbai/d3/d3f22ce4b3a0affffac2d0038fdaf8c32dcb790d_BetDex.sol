...
contract BetDex is Initializable, AccessControl, ReentrancyGuardUpgradeable {
    ...
    mapping(bytes32 => mapping(address => uint256)) private userBalances;
    mapping(bytes32 => mapping(address => bool)) private userBalanceMutexes;
    ...
    function lockRoomReverse_Vtq2I(bytes32 roomId) internal {
        require(!rooms[roomId].roomReverseMutex, "FAILED_TO_LOCK_ROOM_REVERSE");
        rooms[roomId].roomReverseMutex = true;
    }

    function unlockRoomReverse_IyMI(bytes32 roomId) internal {
        require(
            rooms[roomId].roomReverseMutex,
            "FAILED_TO_UNLOCK_ROOM_REVERSE"
        );
        rooms[roomId].roomReverseMutex = false;
    }
    ...
    function createRoomWithToken(
        ...
        rooms[roomId].contractAddress = contractAddress;
    }
    ...
    function withdrawReserves(
        ...
        uint256 roomBalance = roomAvailableReserve_cga(roomId);
        require(roomBalance <= _amount, "INSUFFICIENT_BALANCE");
        require(
            rooms[roomId].reverse > rooms[roomId].reverse - _amount,
            "INVALID_AMOUNT"
        );
        require(
    ...
    function userWithdrawal(
        ...
        lockUserBalance_0Hao(roomId, msg.sender);
        userBalances[roomId][msg.sender] -= _amount;
        Token.withdrawal(contractAddress, msg.sender, _amount);
        unlockUserBalance_Wwha(roomId, msg.sender);
    }

    /// @notice Get current amount of available reserves.
    function roomAvailableReserve_cga(bytes32 roomId)
        public
        view
        returns (uint256)
    {
        return rooms[roomId].reverse - rooms[roomId].lockedReverse;
    }
    ...
    function lockUserBalance_0Hao(bytes32 roomId, address userAddress)
        internal
    {
        require(
            !userBalanceMutexes[roomId][userAddress],
            "FAILED_TO_LOCK_USER_BALANCE"
        );
        userBalanceMutexes[roomId][userAddress] = true;
    }

    function unlockUserBalance_Wwha(bytes32 roomId, address userAddress)
        internal
    {
        require(
            userBalanceMutexes[roomId][userAddress],
        ...
        userBalanceMutexes[roomId][userAddress] = false;
    }
    ...
    function batchSignedBet(SigningBet[] calldata signingBets) external {
        require(signingBets.length > 0, "INVALID_INPUT_PARAMETERS");
        for (uint256 i = 0; i < signingBets.length; i = unchecked_inc(i)) {
    ...
    function batchSignedBetV2(
        SigningBetV2[] calldata signingBets,
        bytes32 roomId,
        uint32 timestamp,
        bytes calldata signature
    ) external {
        require(signingBets.length > 0, "INVALID_INPUT_PARAMETERS");
        //		bytes memory str = abi.encodePacked(roomId);
        //		bytes32 hash;
        require(
        ...
        //		require(!usedNonce[timestamp], "NONCE_ALREADY_USED");
        //		usedNonce[timestamp] = true;
        uint256 totalPotentialWinningAmount = 0;
        uint256 totalBetAmount = 0;
        for (uint256 i = 0; i < signingBets.length; i = unchecked_inc(i)) {
            //			str = abi.encodePacked(
            //				str,
            //				signingBets[i].gameId,
            //				signingBets[i].matchId,
            //				signingBets[i].gameType,
            //				signingBets[i].target,
            //				signingBets[i].odds,
            //				signingBets[i].amount
            //			);
            require(signingBets[i].amount > 0, "Amount must be nonzero");
            uint256 potentialWinningAmount = (signingBets[i].amount *
                signingBets[i].odds) / (10**ODDS_DECIMAL);
            totalPotentialWinningAmount += potentialWinningAmount;
            totalBetAmount += signingBets[i].amount;
            ...
            myBet.bettor = msg.sender;
            myBet.roomId = roomId;
            myBet.target = signingBets[i].target;
            myBet.potentialWinningAmount = potentialWinningAmount;
            gameBets[signingBets[i].matchId][signingBets[i].gameType].add(
                betSlipId
            );

            MatchInfo memory matchInfo;
            matchInfo.roomId = roomId;
            matchInfo.gameId = signingBets[i].gameId;
            matchInfo.matchId = signingBets[i].matchId;
        ...
        //		hash = keccak256(str);
        //		require(
        //			hash.recover(signature) == signerAddress,
        //			"INVALID_SIGNATURE"
        //		);
        address contractAddress = rooms[roomId].contractAddress;
        uint256 transferAmount;
        uint256 userBalance = userBalances[roomId][msg.sender];
        if (userBalance == 0) {
            Token.deposit(contractAddress, msg.sender, totalBetAmount);
            emit TransferToRoom(
                roomId,
                contractAddress,
                msg.sender,
        ...
        } else if (userBalance < totalBetAmount) {
            lockUserBalance_0Hao(roomId, msg.sender);
            transferAmount = totalBetAmount - userBalance;
            userBalances[roomId][msg.sender] = 0;
            unlockUserBalance_Wwha(roomId, msg.sender);
            Token.deposit(contractAddress, msg.sender, transferAmount);
            emit TransferToRoom(
                roomId,
                contractAddress,
                msg.sender,
            ...
            lockUserBalance_0Hao(roomId, msg.sender);
            userBalances[roomId][msg.sender] = userBalance - totalBetAmount;
            unlockUserBalance_Wwha(roomId, msg.sender);
        }
        lockRoomReverse_Vtq2I(roomId);
        rooms[roomId].reverse += totalBetAmount;
        require(
            roomAvailableReserve_cga(roomId) >= totalPotentialWinningAmount,
            "FAILED_TO_BET"
        );
        rooms[roomId].lockedReverse += totalPotentialWinningAmount;
        unlockRoomReverse_IyMI(roomId);
    }
    ...
    function bet_wWe1d(
        ...
        require(
            roomAvailableReserve_cga(matchInfo.roomId) >=
                potentialWinningAmount,
            "FAILED_TO_BET"
        );
        //        require(potentialWinningAmount > _amount, "FAILED_TO_BET_OVERFLOW"); v8 no need check overflow
        bytes32 betSlipId = keccak256(
            abi.encodePacked(
                blockhash(block.number - 1),
                bettorAddress,
        ...
        lockUserBalance_0Hao(matchInfo.roomId, bettorAddress);
        uint256 transferAmount;
        uint256 userBalance = userBalances[matchInfo.roomId][bettorAddress];
        if (userBalance == 0) {
        ...
        } else if (userBalance < _amount) {
            transferAmount = _amount - userBalance;
            userBalances[matchInfo.roomId][bettorAddress] = 0;
            Token.deposit(contractAddress, bettorAddress, transferAmount);
            emit TransferToRoom(
                matchInfo.roomId,
                contractAddress,
            ...
            userBalances[matchInfo.roomId][bettorAddress] =
                userBalance -
                transferAmount;
            transferAmount = 0;
        }
        unlockUserBalance_Wwha(matchInfo.roomId, bettorAddress);
        ...
        lockRoomReverse_Vtq2I(matchInfo.roomId);
        rooms[matchInfo.roomId].lockedReverse += potentialWinningAmount;
        rooms[matchInfo.roomId].reverse += _amount;
    ...
    function resolveBet_X3EB(
        ...
        for (uint256 i = 0; i < targetLength; i = unchecked_inc(i)) {
            targets[_targets[i]] = true;
        }
        for (uint256 i = 0; i < length; i = unchecked_inc(i)) {
            bytes32 betSlipId = gameBets[matchId][gameType].at(0);
            ...
            if (result) {
                lockUserBalance_0Hao(myBet.roomId, myBet.bettor);
                userBalances[myBet.roomId][
                    myBet.bettor
                ] += potentialWinningAmount;
                unlockUserBalance_Wwha(myBet.roomId, myBet.bettor);
                lockRoomReverse_Vtq2I(myBet.roomId);
                rooms[myBet.roomId].reverse -= potentialWinningAmount;
                rooms[myBet.roomId].lockedReverse -= potentialWinningAmount;
                unlockRoomReverse_IyMI(myBet.roomId);
            } else {
                lockRoomReverse_Vtq2I(myBet.roomId);
                rooms[myBet.roomId].lockedReverse -= potentialWinningAmount;
                unlockRoomReverse_IyMI(myBet.roomId);
