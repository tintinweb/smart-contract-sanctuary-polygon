...
contract PoolImplementation is PoolStorage, NameVersion {
    ...
    function addMargin(address underlying, string memory symbolName, uint256 amount, OracleSignature[] memory oracleSignatures) external payable _reentryLock_
    {
        _updateOracles(oracleSignatures);
    ...
    function removeMargin(address underlying, string memory symbolName, uint256 amount, OracleSignature[] memory oracleSignatures) external _reentryLock_
    {
        _updateOracles(oracleSignatures);

        Data memory data = _initializeData(underlying, msg.sender);
        bytes32 symbolId = keccak256(abi.encodePacked(symbolName));
