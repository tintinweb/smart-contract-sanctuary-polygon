...
contract StormTrooper is ERC1155, ERC2981, AccessControl, VRFConsumerBaseV2, ConfirmedOwner {
    ...
    // function.
    uint32 public callbackGasLimit = 140000;
    ...
    mapping(uint256 => StormTrooperItem) private infanities;
    // brandId => Brand
    ...
    function fulfillRandomWords(uint256 _requestId, uint256[] memory _randomWords) internal override {
        ...
        infanities[_collectionId].minted =  infanities[_collectionId].minted + 1;
    ...
    function getRequestStatus(uint256 _requestId) external view returns (
        ...
        return (request.to, request.brand, request.collectionId, request.fulfilled, request.randomWords);
    }
    ...
    function getMintableCollectionIds(uint256 _brandId) public view returns (uint256[] memory) {
        ...
        for (uint256 id = 0; id < ids.length; id++) {
            ...
            if (infanities[ids[id]].minted >= infanities[ids[id]].cap || !infanities[ids[id]].enabled) {
                unmintableCount += 1;
            }
        }
    ...
    function getCollectionInfoById(uint256 _tokenId) public view returns (StormTrooperItem memory) {
        require(infanities[_tokenId].exist, "Token does not exist.");
        return infanities[_tokenId];
    }
    ...
    function createBrand(uint256 brandId, uint256 mintPriceInWei) public onlyHasRole(ADMIN_ROLE) {
    ...
    function createCollection(uint256 cap, uint256 brand) external onlyHasRole(ADMIN_ROLE) {
        require(brands[brand].exist, "Brand doesnt exist.");
        require(cap > 0, "Collection supply cannot be zero");
        uint256 tokenId = nextTokenId();
        infanities[tokenId] = StormTrooperItem(
    ...
    function enableCollection(uint256 _id) public onlyHasRole(ADMIN_ROLE) {
        require(infanities[_id].exist, "Collection ID doesnt exist.");
    ...
    function updateCapCollection(uint256 tokenId, uint256 cap) external onlyHasRole(ADMIN_ROLE) {
        require(cap > 0, "Collection supply cannot be zero");
        require(infanities[tokenId].enabled, "Collection ID doesnt exist.");
        require(infanities[tokenId].cap >= infanities[tokenId].minted, "Cap cannot less than minted token.");
        StormTrooperItem storage item =  infanities[tokenId];
        item.cap = cap;
    }
