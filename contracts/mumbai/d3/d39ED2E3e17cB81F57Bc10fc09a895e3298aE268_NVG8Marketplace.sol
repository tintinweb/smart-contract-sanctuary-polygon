...
*/
    ...
    function enlistDataToken(address _erc721Token, address _erc20Token, address _owner, string memory _name, string memory _symbol, uint256 _usagePrice) public onlyFactory returns (bool _success) {
        // check if the data token is already enlisted
        uint256 _dataTokenId = _dataTokenIdCounter.current();
        _dataTokenIdCounter.increment();
        // enlist data token
        DataToken memory dataToken = DataToken(_erc721Token, _erc20Token, _owner, _name, _symbol, _usagePrice, true);
    ...
    //SETTING FIXED PRICE FOR TOKENS
    mapping (address => uint256) public fixedPrice;
    function setTokenFixedPrice(uint256 _dataTokenId, uint256 _price) public returns(bool) {
        require(dataTokens[_dataTokenId].erc20Token != address(0) && dataTokens[_dataTokenId].isActive, "Data token not enlisted");
        require(dataTokens[_dataTokenId].owner == msg.sender, "Only Owner can set Price of the Token");
        fixedPrice[dataTokens[_dataTokenId].erc20Token] = _price;
        return true;
    }

    //SET NVG8 TOKEN ADDRESS
    IERC20Template public nvg8token;
    address _nvg8tokenaddress;
    function setNVG8Token(address _token) public onlyOwner returns(bool){
        nvg8token = IERC20Template(_token);
        _nvg8tokenaddress = _token;
        return true;
    }

    //Set Address of Buy Token
    address _contractAddressBuyToken;
    function setBuyTokenContract(address _contractAddress) public onlyOwner returns(bool){
        _contractAddressBuyToken = _contractAddress;
        return true;
    }

    //BUY DATA TOKEN USING FIXED PRICE NVG8 TOKENS
    function buyTokenFixPrice(uint256 _dataTokenId, uint256 _amount) public returns(bool){
        require(dataTokens[_dataTokenId].erc20Token != address(0), "Data Token doesn't Exist");
        require(dataTokens[_dataTokenId].isActive, "Data token not enlisted");
        require(IERC20Template(dataTokens[_dataTokenId].erc20Token).balanceOf(dataTokens[_dataTokenId].owner) > 0, "Not Enough Data Tokens");
        require(nvg8token.balanceOf(msg.sender) >= fixedPrice[dataTokens[_dataTokenId].erc20Token], "Not Enough NVG8 Tokens");
        require(fixedPrice[dataTokens[_dataTokenId].erc20Token] > 0, "Price not Set");

        IBuyToken(_contractAddressBuyToken)._buyTokenFixPrice(dataTokens[_dataTokenId].erc20Token, _nvg8tokenaddress, fixedPrice[dataTokens[_dataTokenId].erc20Token], msg.sender, dataTokens[_dataTokenId].owner, _amount);
        return true;
    }
...
interface IBuyToken{
    function _buyTokenFixPrice (address _erc20address, address _nvg8token, uint256 _fixedPrice, address _user, address _owner, uint256 _amount) external returns(bool);
}
