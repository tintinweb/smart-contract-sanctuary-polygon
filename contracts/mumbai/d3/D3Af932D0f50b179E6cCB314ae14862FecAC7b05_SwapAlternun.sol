...
contract SwapAlternun is Swap, SwapNative, Withdraw {
    constructor() {}
...
import "./factory/FactoryBusinessPartners.sol";

contract Swap is 
    ReEntrancyGuard, 
    FactorySwap, 
    FactoryBusinessPartners, 
    Oracle 
{
    ...
    function BuyTokensFor(
        ...
        /// @dev calculate vault fee percentage and send it to the vault
        uint256 vualtFee = calculatePorcent(_amountTokens, _vaultPercentage);
        if (vualtFee > 0) {
            IERC20(pair.tokenA).transferFrom(address(this), _vault, vualtFee);
        }
        ...
        /// @dev calculate the amount of token to buy in USD
        uint256 valueInUsd = amountTo18.mul(latestPrice);

        /// @dev amount of token to buy
        uint256 amountTokenBuy = valueInUsd.div(pair.price);
        ...
        /// @dev transfer the token to the vendor contract
        require(
    ...
    Oracle
{
    ...
    function buyToken(
        ...
        require(msg.value > 0, "Buy Token: Send BNB to buy some tokens");

        /// @dev get the pair
        Pair storage pair = _PairstoSwap[_pairId];
        require(pair.active, "Buy Token: Pair is not active");
        ...
        /// @dev calculate vault fee percentage and send it to the vault
        uint256 vualtFee = calculatePorcent(_amountOfTokens, _vaultPercentage);
        if (vualtFee > 0) {
            (bool successVualtFeeTransfer, ) = payable(_vault).call{value: vualtFee}("");
            require(
                successVualtFeeTransfer,
                "Buy With Native Token: Failed to transfer vault FEE"
            );
        }
        ...
        require(
            ...
            "Buy Token: Vendor contract has not enough tokens in its balance"
        );

        /// @dev Transfer token to the msg.sender
        require(
            IERC20(pair.tokenB).transfer(_msgSender(), tokenAmountToBuy),
            "BuyT oken: Failed to transfer token to user"
        );

        /// @dev emit the event
        emit BuyTokensNative(_msgSender(), msg.value, tokenAmountToBuy);
    }
}
...
contract Withdraw is Administered {
    ...
    /// @dev struct
    function WithdrawOwnerNative(uint256 amount) external payable onlyAdmin {
        require(
    ...
    /// @dev Withdrawal  tokens to the owner of the contract
...
contract Oracle {
    ...
    /// @dev calcule porcentage of the commission of the advisor
    function calculatePorcent(uint256 amount, uint256 fee_porcent)
        public
        pure
        returns (uint256)
    {
        // return (amount * fee_porcent) / 10000;
        return amount.mul(fee_porcent).div(10000);
    }
...
contract FactoryBusinessPartners is Administered {

    address public _vault = address(0);
    
    uint256 public _vaultPercentage = 0;

    /**
     * @dev Update vault address
     * @param _addr The address of the vault
     */
    function updateVault(address _addr) external onlyAdmin {
        _vault = _addr;
    }

    /**
     * @dev Update vault percentage
     * @param _val The percentage of the vault
     */
    function updateVaultPercentage(uint256 _val) external onlyAdmin {
        _vaultPercentage = _val;
    }
}
