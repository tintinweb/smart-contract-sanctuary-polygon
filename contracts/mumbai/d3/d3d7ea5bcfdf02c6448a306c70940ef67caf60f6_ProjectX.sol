...
contract ProjectX is ERC20, Ownable, Pausable {
    using SafeMath for uint256;
    RewardContract public myRewardContract;
    address public _stakingAddress = 0x1111111111111111111111111111111111111111; // replace with actual address
    address private _charityAddress =
        0x2222222222222222222222222222222222222222; // replace with actual address
    address private _managementAddress =
        0x3333333333333333333333333333333333333333; // replace with actual address
    address private _liquidityAddress =
        0x4444444444444444444444444444444444444444; // replace with actual address
    address private _burnAddress = 0x0000000000000000000000000000000000000000; // default burn address
    ...
    constructor() ERC20("Project", "X") {
        _managementAddress = address(owner());
    }
    ...
    function burn(uint256 amount) public {
        ...
        transferWithBurn(address(0), amount);
    ...
    function transferWithBurn(address recipient, uint256 amount) internal {
        require(amount > 0, "transferWithBurn: Zero transfer not allowed.");
        require(
            balanceOf(_msgSender()) >= amount,
            "transferWithBurn:Insufficient balance."
        );
        uint256 totalTaxAmount = amount
            .mul(
                _stakingPoolFee.add(_charityFundFee).add(_managementFee).add(
                    _liquidityPoolFee
                )
            )
            .div(10000); //default is div.100 for 100% rate
        ...
        require(
            balanceOf(_msgSender()) >= amount,
            "transferWithBurn: Not enough tokens to transfer"
        );

        // Make sure the calle is not the zero address
        require(
            msg.sender != address(0),
            "transferWithBurn: Transfer from the zero address"
        );

        // Make sure the transfer amount is not zero
        require(
            amountAfterTax > 0,
            "transferWithBurn: Transfer amount must be greater than zero"
        );
        if (recipient == address(0)) {
            _burn(msg.sender, amountAfterTax);
            balance = balanceOf(_burnAddress).add(amountAfterTax);
            tokenBurned += amountAfterTax; // Increment tokenBurned by the amount being burned
            emit TokenBurned(_msgSender(), amountAfterTax);
        } else {
    ...
    // Controller to transfer from the token owner
    function transferFrom(
        address sender,
        address recipient,
        uint256 amount
        ...
        require(
            balanceOf(_msgSender()) >= amount,
            "transferFrom: Not enough tokens to transfer"
        );

        // Make sure the calle is not the zero address
        require(
            msg.sender != address(0),
            "transferFrom: transfer from the zero address"
        );
