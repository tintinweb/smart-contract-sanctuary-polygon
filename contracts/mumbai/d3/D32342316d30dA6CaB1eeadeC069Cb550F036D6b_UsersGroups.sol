...
/// @title UsersGroups.
/// @notice Contract stored iser and group membership.
/// @dev Contract stored iser and group membership.
contract UsersGroups is Initializable {
  ...
  /// ```
  /// userID - User Address.
  /// current_balance - For future use.
  /// blocked_balance - For future use.
  /// timestamp_status - Timestamp Creation.
  /// bool timestamp_status - For future use.
  /// ```
  ...
  /// @return user_lisr : List of user, when one lement is:
  /// ```
  /// userID - User Address.
  /// current_balance - For future use.
  /// blocked_balance - For future use.
  ...
  /// @dev Get All Group.
  /// @return group_list : List of groups, whe one element is:
  /// ```
  /// group_name - Group name.
  /// current_balance - For future use.
  /// blocked_balance - For future use.
  /// timestamp_created - Timestamp Creation.
  /// timestamp_last_integration - Timestamp for last integration.
  /// ```
  function getGroups()
  public
  view
  ...
  /// @notice Reference to GovernanceToken contract.
    /// @dev Reference to GovernanceToken contract.
    ...
    returns (bool)
    {
        if(!isMember(msg.sender) && !ifRequestedAccounts(_address)){
    ...
    public
    returns (bool) {
        if(!isMember(_address) && ifRequestedAccounts(_address)){
...
/// @notice Contract managed tokens used for voting on proposals.
/// @dev Contract managed tokens used for voting on proposals.
...
contract GovernanceToken is Initializable, ERC20Upgradeable, ERC20PermitUpgradeable, ERC20VotesUpgradeable{
    ...
    /// @notice Transfer token to account and delegate vote for him.
    ...
    /// @notice Get All Tokens number.
    /// @dev Get All Tokens number.
