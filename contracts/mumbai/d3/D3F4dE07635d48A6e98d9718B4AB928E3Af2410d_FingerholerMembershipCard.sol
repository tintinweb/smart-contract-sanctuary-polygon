...
contract FingerholerMembershipCard is RoyaltySplits, PassCreator {
	constructor(
		string memory _name,
...
abstract contract PassCreator is
	ReentrancyGuard,
	ERC1155Supply,
	Ownable,
	Pausable,
	...
	LockedPaymentSplitter
{
	...
	uint256 public constant MAX_PER_WALLET_LIMIT = 6;
	...
	/// @dev Enables timed minting of tokens.
	uint256 public saleStartTimestamp;
	uint256 public saleEndTimestamp;

	uint256 public mintCost = 0.069 ether;
	uint256 public immutable mintPassId;
	...
	event TokenMinted(address indexed mintedBy, uint256 indexed quantity);
	...
	function _isLessThanMaxSupply(uint256 quantity) internal view {
		if (totalSupply(mintPassId) + quantity > maxSupply) {
			revert MaxSupplyExceeded(
				totalSupply(mintPassId) + quantity,
				maxSupply
			);
		}
	}
		...
		nonReentrant
	{
		...
		_mint(_msgSender(), mintPassId, quantity, "");
		emit TokenMinted(_msgSender(), quantity);
	}
	...
	function saleIsActive() public view returns (bool) {
		if (
			...
			totalSupply(mintPassId) == maxSupply ||
			isSalePaused
		) {
			return false;
		}
