...
contract MaterialObject is BaseObject, ERC1155Supply {
    /* -------------------------------------------------------------------------- */
    /*                                   CONFIG                                   */
    /* -------------------------------------------------------------------------- */
    ...
    function getObject(address to, uint256 tokenid) external onlyOwner nonReentrant {
        ...
        emit LogGetMaterialObject(msg.sender, tokenid);
    }

    function burn(address from, uint256 id, uint256 amount) external nonReentrant {
        // check if the function caller is not an zero account address
        require(from != address(0), "from(0) is invalid");
        // check the token id exists
        isValid(id);
        // burn the token
    ...
    function burnBatch(address from, uint256[] memory ids, uint256[] memory amounts) external nonReentrant {
        // check if the function caller is not an zero account address
        require(from != address(0), "from(0) is invalid");
        // burn the token
