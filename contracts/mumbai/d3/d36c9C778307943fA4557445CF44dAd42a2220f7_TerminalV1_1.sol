...
contract TerminalV1_1 is Operatable, ITerminalV1_1, ITerminal, ReentrancyGuard, Ownable {
  ...
  //target limit
  mapping(uint256 => uint256) public targetLimit;
  ...
  */
  ...
  function pay(
    ...
    //under Target Limit
    if (targetLimit[_projectId] > 0) {
      require(
        totalBalanceOf[_projectId] + msg.value <= targetLimit[_projectId],
        'can not raise fund more than target Limit'
      );
    }
  ...
  function setTargetLimit(uint256 _projectId, uint256 _targetLimit) external onlyOwner {
    targetLimit[_projectId] = _targetLimit;
  }
