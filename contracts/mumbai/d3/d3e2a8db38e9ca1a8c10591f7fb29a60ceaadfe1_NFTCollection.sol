    ...
    ReentrancyGuardUpgradeable
{
    ...
    mapping(address => address[]) public userCreatedCollections;
    ...
    event RoyaltyFeeUpdated(address indexed owner, uint256 newRoyaltyFee);
    event AdminWalletAddressUpdated(address indexed newAdminWallet);
    ...
    function setRoyaltyFee(uint256 _newFee) external onlyOwner whenNotPaused {
        require(_newFee > 0, "Royalty Fee must be greater than zero.");
        royaltyFee = _newFee;
        emit RoyaltyFeeUpdated(msg.sender, _newFee);
    }

    function updateAdminWalletAddress(address _newAdminWallet)
        public
        whenNotPaused
        onlyOwner
    {
        require(
            _newAdminWallet != address(0),
            "Admin wallet address should not be zero address."
        );
        admin = _newAdminWallet;
        emit AdminWalletAddressUpdated(_newAdminWallet);
    }
    ...
    ERC2981Upgradeable
{
    ...
    function setRoyaltyReceiver(address _receiver)
        ...
        whenNotPaused
    {
        super._setDefaultRoyalty(_receiver, royaltyFee);
    }
    ...
    function setPlatformFee(uint256 _newPlatformFee)
        ...
        whenNotPaused
    {
        require(_newPlatformFee > 0, "Fee must be greater than zero");
