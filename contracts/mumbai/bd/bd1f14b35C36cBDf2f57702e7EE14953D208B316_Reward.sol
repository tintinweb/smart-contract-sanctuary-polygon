...
// File: all-code/hive/Reward-multisign.sol
...
contract Reward {
    ...
    modifier onlyMultiSign() {
        require(msg.sender == multiSign, "Not authorized to edit team members!");
        _;
    }

    constructor(address[] memory _owners, uint _numConfirmationsRequired, address _token, address _multiSign) {
    ...
    function addTeamMember(address _addr) external onlyMultiSign {
        owners.push(_addr);
        isOwner[_addr] = true;
        numConfirmationsRequired += 1;
    }

    function removeTeamMember(address _addr) external onlyMultiSign {
