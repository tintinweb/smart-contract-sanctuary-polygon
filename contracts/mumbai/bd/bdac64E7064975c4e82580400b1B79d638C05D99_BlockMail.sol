...
contract BlockMail is Ownable {
    ...
    event MailSent(
        ...
        bool nativeAsset,
        uint256 indexed nativeAssetAmount,
        address token,
        uint256 tokenAmount,
        address nftContract,
        uint256 tokenId
     ...
     */
    constructor(uint256 _blockMailFeeAmount, address _priceFeedAddress) {
        priceFeed = AggregatorV3Interface(_priceFeedAddress);
        blockMailFeeAmount = _blockMailFeeAmount;
    }
    ...
    /**
        ...
        @param _nativeAsset The amount of Native Asset for the blockchain e.g amount in MATIC or ETH. 
    ...
    function sendBlockMail(
        ...
        if (getConversionRate(msg.value) < blockMailFeeAmount) {
            revert BlockMail__InsufficientFeeAmount();
        }
        //pay fee
        (bool ok, ) = owner().call{value: blockMailFeeAmount}("");
        require(ok, "Failed to pay fee");

        //send native asset if there is any
        if (_nativeAsset) {
            uint256 amountToSend = msg.value - blockMailFeeAmount;
    ...
    /**
        @notice sets a new fee for sending mails
        @dev Incase a user decides to no longer see mails from a user.
        @param _newFee The new fee in usd (18 decimals)
     */
    function modifyFee(uint256 _newFee) public onlyOwner {
        blockMailFeeAmount = _newFee;
    }

    /**
        @notice converts the nativeAsset to its usd equivalent in USD(18 decimals)
        @param _maticValue The msg.value that it converts it from
        @return maticAmountInUsd returns the value of the native asset in usd wei value(18 decimals)
     */
    function getConversionRate(uint256 _maticValue)
        public
        view
        returns (uint256)
    {
        uint256 maticPrice = getPrice();
        uint256 maticAmountInUsd = (maticPrice * _maticValue) / 10**18;
        return maticAmountInUsd;
    }

    /**
        @notice cget the current price of the asset in 10 decimals
        @dev we multiply by 10**10 to match the wei units for proper calculations.
        @return answer returns the price of the asset in 18 decimals.
     */
    ...
    /**
        @notice gets the current fee amount for sending a mail
        @return blockMailFeeAmount returns the current fee for sending a mail
     */
    function getFeeAmount() public view returns (uint256) {
        return blockMailFeeAmount;
    }
     ...
     */
    function allowedToSendMail(address receiver) public view returns (bool) {
