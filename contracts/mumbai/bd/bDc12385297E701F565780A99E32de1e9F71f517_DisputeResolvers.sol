...
contract DisputeResolvers is Mappings, IDisputeResolution {
    ...
    function withdraw() external override returns (bool) {
        ...
        if(!isAdminWithdrawed[tx.origin]) IERC20(dbethAddress).transfer(admin,(usersStake[tx.origin] - lastWithdrawal[tx.origin]));
        usersStake[tx.origin] = 0;
        isActiveStaker[tx.origin] = false;
        if(isAdminWithdrawed[tx.origin] == true) isAdminWithdrawed[tx.origin] = false;
    ...
    function adminWithdrawal(address user_) external override returns (bool status_) {
        require(!isAdminWithdrawed[user_] || usersStake[user_] > 0,"Admin Has Already Withdrawed Or User Stake Is 0");
        uint256 amount_ = calculateStrikeAmount(user_);
        uint temp = userInitialStake[tx.origin] - amount_;
