...
contract DemoWorkflow {
    ...
    function approveRemoveLiquidityToProtocol(address _pair) external {
        liq = IERC20(_pair).balanceOf(msg.sender);
        require(liq != 0, "Workflow: has no balance");

        IERC20(_pair).approve(QUICKSWAP_ROUTER, liq);

        emit LiquidityRemoveApproved(QUICKSWAP_ROUTER, _pair, liq);
    }
    ...
    function _removeLiquidity(
        ...
        amountPair = IERC20(_pair).balanceOf(_owner);
        require(amountPair != 0, "Workflow: has no balance");
        IERC20(_pair).transferFrom(_owner, address(this), amountPair);
