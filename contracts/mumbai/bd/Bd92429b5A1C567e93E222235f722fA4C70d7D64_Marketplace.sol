...
contract Marketplace is ERC721Holder, AccessControl, ReentrancyGuard {
    ...
    returns (uint256)
  {
    ...
    require(
      propertyVerificationContract.isPropertyOwnedBy(propertyId, propertyOwner),
  ...
  function approveBooking(uint256 requestId, uint256 ownerFee) external {
    ...
    require(!listingRequest.approved, 'This request has already been approved');
    // owner fee cannot be greater than 100%
    require(ownerFee < highestCommissionBasisPoint);

    uint256 disputeTimeLimit = propertyVerificationContract.disputeTimeLimit();
  ...
  function createBnftAuction(
    ...
    require(checkInTime > endTime, 'The auction ends after the check in time');
    ...
    nonReentrant
  {
    ...
    if (auction.currencyAddress == address(0)) {
      ...
      token.safeTransfer(auction.highestBid.bidder, auction.highestBid.amount);
    }
  ...
  function claimAuction(uint256 auctionId) public payable {
    ...
    (address propertyOwnerAddress, , , , , , uint256 ownerFee) = BnftContract
      .bookingDetails(auction.tokenId);
    ...
    if (auction.currencyAddress == address(0)) {
      ...
      require(treasurySuccess, 'Transfer of tokens to treasury wallet failed');
      // send the commission to the property owner
      (bool propertyOwnerSuccess, ) = propertyOwnerAddress.call{value: fee}('');
      require(
      ...
      token.safeTransferFrom(msg.sender, propertyOwnerAddress, commission);
    }
