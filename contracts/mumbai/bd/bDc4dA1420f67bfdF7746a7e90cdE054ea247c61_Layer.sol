...
contract Layer is AccessControl, ERC721Enumerable, DateUtil,ReentrancyGuard{
    ...
    function getClass1List() public view returns(address[] memory c1 ){
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Caller is not admin");
        string memory date = DateUtil.daysToDateString(int(block.timestamp - 1 days),8);
        c1 = class1[date];
        return c1;
    ...
    function getqClass1List() public view returns(address[] memory  ){
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Caller is not admin");
        string memory date = DateUtil.daysToDateString(int(block.timestamp - 1 days),8);
        address[] memory c1;
    ...
    function class1Length() public view returns(uint){
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Caller is not admin");
        string memory date = DateUtil.daysToDateString(int(block.timestamp - 1 days),8);
        return class1[date].length;
    }
