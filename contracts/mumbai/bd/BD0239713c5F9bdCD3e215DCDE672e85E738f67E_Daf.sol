...
contract Daf is ERC20, Ownable, Pausable {
    ...
    */
    ...
    function buy(uint256 _batchNumber, uint256 _amount, Coin coinType) external payable {
        ...
        if (coinType == Coin.ETH) {
            uint256 EHTAmount = (_amount * batch.priceUSD) / getLatestETHPrice();
            require(msg.value >= EHTAmount, "Daf: insufficient balance of ethereum");
        }
        else if (coinType == Coin.WBTC) {
            uint256 BTCAmount = (_amount * batch.priceUSD) / getLatestWBTCPrice();
            ERC20(WBTCAddress).transferFrom(msg.sender, address(this), BTCAmount);
        }
        else if (coinType == Coin.USDC) {
            uint256 USDCAmount = (_amount * batch.priceUSD) / getLatestUSDCPrice();
            ERC20(USDCAddress).transferFrom(msg.sender, address(this), USDCAmount);
        }
    ...
    */
    ...
    function hedge(uint256 _batchNumber, uint256 _amount, HedgeCoin coinType) external payable {
        ...
        if (coinType == HedgeCoin.WBTC) {
            amount = (_amount * batch.priceUSD) / getLatestWBTCPrice();
            uint256 hedgingFee = amount * BTCHedgingPercent / 10 ** percentageDecimals;

            // fee should be transferred into Daf contract wallet
            ERC20(WBTCAddress).transferFrom(msg.sender, dafWalletAddress, hedgingFee);
            ERC20(WBTCAddress).transferFrom(msg.sender, address(dafNft), amount);
        } else {
            amount = (_amount * batch.priceUSD) / getLatestETHPrice();
            uint256 hedgingFee = amount * ETHHedgingPercent / 10 ** percentageDecimals;

            require(msg.value >= hedgingFee + amount, "Daf: insufficient eth balance");
            // fee should be transferred into Daf contract wallet
    ...
    */
    ...
    function takeBatchCoins(uint256 _batchNumber) external onlyOwner {
        ...
        _transfer(address(this), dafWalletAddress, amount);
    ...
    */
    ...
    override
    {
        if (from != address(0) && balanceOf(from) < dafNft.getHedgeBalance(from) + amount) {
            dafNft.stopUserHedges(from);
        }
...
interface IDafNFT {
    ...
    */
    ...
    function stopUserHedges(address account) external;

    // @dev gets users whole hedge balance
    function getHedgeBalance(address account) external view returns(uint256);
}