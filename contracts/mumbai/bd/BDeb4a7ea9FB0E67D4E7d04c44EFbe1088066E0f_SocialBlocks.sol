...
contract SocialBlocks is AccountCreation("Social Blocks", "$SB"), Ownable, Verify {
    ...
    function changePostStatus(uint256 _postId, uint8 _status) external ValidToChange(_postId) isValidUser {
        ...
        emit PostDetailsChanged(_postId, _status, postInfo[_postId].sellValue);
    }
    ...
    function changePostPrice(uint256 _postId, uint256 _price) external ValidToChange(_postId) isValidUser {
        ...
        postInfo[_postId].sellValue = _price;
        emit PostDetailsChanged(_postId, postInfo[_postId].buyStatus, _price);
    }
    ...
    function bid(uint256 _postId, uint256 _price) external isValidUser {
        ...
        if(lastBidder != address(0)) (sent2) = rewardToken.transfer(lastBidder, userBid[lastBidder][_postId]);
        require(sent1 && sent2, "transfer fail");
    ...
    function claimBid(uint256 _postId) external isValidUser{
        ...
        emit BiddableTokenPurchased(owner, lastBidder, _postId);
    }

    function buyPost(uint256 _postId) external isValidUser{
        PostInfo memory _postInfo = postInfo[_postId];
        address user = msg.sender;
        require(
        ...
        require(rewardToken.balanceOf(user) >= _postInfo.sellValue, "insufficient balance");
        ...
        postInfo[_postId].buyStatus = 2;
        postInfo[_postId].sellValue = 0;
        _transfer(getOwnerById(_postId), user, _postId);
    }
...
contract AccountCreation is ERC721Enumerable {
    ...
    event BiddableTokenPurchased(address oldOwner, address newOwner, uint256 tokenId);
