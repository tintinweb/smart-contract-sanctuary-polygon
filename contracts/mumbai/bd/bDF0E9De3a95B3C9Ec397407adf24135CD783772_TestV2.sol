...
contract TestV2 is Test{
    ...
    mapping(uint256 => string) uriDecById;

    function setUriDecById(uint256 tokenId, string memory decUri) public onlyRole(DEFAULT_ADMIN_ROLE){
        uriDecById[tokenId] = decUri;
    }

    function getUriDecById(uint256 tokenId) public view returns(string memory){
        return uriDecById[tokenId];
    }
...
contract Test is Initializable, ERC721Upgradeable, 
