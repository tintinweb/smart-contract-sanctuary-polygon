...
// File: moa/airdrop/RedemptionCoupon.sol
...
/// @dev a RedemptionCoupon for pre-(random) disturibtion another ERC721 before it launches
contract RedemptionCoupon is ERC721Enumerable, Ownable, IERC721Receiver {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;

    address public nftContractAddress = address(0);
    bytes4 public constant _INTERFACE_ID_IERC721ENUMERABLE = 0x780e9d63;

    string public imageURI;

    constructor(address _contractAddress, string memory _imageURI)
        ERC721("Redemption Coupon", "RC")
    {
        _setNftContractAddress(_contractAddress);
        imageURI = _imageURI;
    }
    ...
    function _setNftContractAddress(address contractAddress) internal {
        require(
            IERC721Enumerable(contractAddress).supportsInterface(
                _INTERFACE_ID_IERC721ENUMERABLE
            )
        ); // the NFT Contract must support IERC721Enumerable
        nftContractAddress = contractAddress;
    }

    /// @notice update the nftAddress to be distributed to the owner
    function setNftContractAddress(address contractAddress) public virtual onlyOwner {
        _setNftContractAddress(contractAddress);
    }

    /// @notice burn the token to send 1 of the contracts owned NFT to the owner
    /// @dev anyone can call burn as the actual NFT is transfer always transfer to the original token owner
    /// @return moaId the moa tokenId transfered
    function burn(uint256 tokenId) public returns (uint256) {
        ...
        require(balance > 0); // there is still nfts left for us to be distributed

        address to = ownerOf(tokenId);
        super._burn(tokenId);

        uint256 nftId = nftContract.tokenOfOwnerByIndex(
            address(this),
            _pseudoRandom(tokenId, balance, balance)
        );
        // transfer one of nft of this contract to the burnt token's owner
        nftContract.safeTransferFrom(address(this), to, nftId);
        return nftId;
    }

    /// @dev simple pseudoRandom from 0 to max-1
    ...
    /// @notice batch mint coupon to the target Address
    /// @param toAddresses target addresses
    /// @param quantity how many token to mint to each target address, max 3 for each address
    function safeBatchMint(address[] calldata toAddresses, uint256 quantity)
        public
        onlyOwner
    {
        require(toAddresses.length > 0); //
        require(quantity > 0 && quantity <= 3);
    ...
    /// @notice handle ERC721 receive, only accept the NFT we wanted to be distributed
    function onERC721Received(
        address,
        address,
        uint256,
        ...
        require(_msgSender() == nftContractAddress); // only accept token from nftContract
...
// File: moa/airdrop/RedemptionGachaCoupon.sol
...
/// @dev a RedemptionCoupon that support draw from Gen1GachaMacine
contract RedemptionCouponWithGacha is RedemptionCoupon, Gen1GachaMachine {
    constructor(address _contractAddress, string memory _imageURI)
        RedemptionCoupon(_contractAddress, _imageURI)
        Gen1GachaMachine(_contractAddress)
    {}
    ...
    function setNftContractAddress(address _nftAddress) public override onlyOwner {
        _setNftContractAddress(_nftAddress);
        _setNftAddress(_nftAddress);
    }

    /// @notice burn the token and draw a moa from the Gacha Machine, require autoBirthFee
    /// @dev anyone can call burn as the actual NFT is transfer always transfer to the original token owner
    /// @dev the autoBirthFee will be the rewards stored on this contract
    /// @param tokenId the token to be burnt
    /// @return moaId the moa tokenId transfered
    function burnAndDraw(uint256 tokenId) public returns (uint256) {
        require(_exists(tokenId));
