...
//1155s/ @dev This contract uses token = address(0) to
//refer to ETH.
contract LS1155 is LiquidSplit, ContractMetadata, ERC1155Drop {
    /// -----------------------------------------------------------------------
    /// errors
    /// -----------------------------------------------------------------------
    ...
    function scaledPercentBalanceOf(
        ...
        uint256 currentTotalSupply = totalSupply[TOKEN_ID];
        if (currentTotalSupply == 0) {
            return 0;
        } else {
            uint256 scale = 1e6 / currentTotalSupply;
            return uint32(balanceOf[account][TOKEN_ID] * scale);
        }
    }

    // -----------------------------------------------------------------------
    // ERC1155Drop Overrides (Custom minting logic) Only TokenId 0
    // -----------------------------------------------------------------------
    ...
    function lazyMint(
        ...
        require(nextTokenIdToMint() == 0, "Only token 0 can be minted.");

        if (nextTokenIdToMint() == 0) {
            _setTokenURI(0, _baseURIForTokens);

            nextTokenIdToLazyMint = 1;
        }
        uint256 result = LazyMint.lazyMint(_amount, _baseURIForTokens, _data);
