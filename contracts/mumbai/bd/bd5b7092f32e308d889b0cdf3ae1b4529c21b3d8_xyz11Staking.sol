...
contract xyz11Staking is Ownable {
    ...
    function approveTransaction(address _user, uint256 _pos, bool _status) public onlyOwner {
        if(user_info[_user].transactionStatus[_pos] != 1){
            if (_status){
                ...
                distributeReferralReward(_tokenAmount);
    ...
    function withdrawTokens(address _user, uint256 _pos) public {
        require(!paused, "The contract is paused");
        require(_user == msg.sender, "Unauthorised");
        if(user_info[_user].transactionStatus[_pos] == 1 && user_info[_user].stakingStatus[_pos]){
            uint256 _amount = calculateYieldTotal(_user,_pos);
            user_info[_user].withdrawal[_pos] = _amount;
            totalWithdraw += _amount;
            user_info[_user].stakingStatus[_pos] = false;
            _xyzToken.transferFrom(address(this),_user, _amount);
        }
    }
    ...
    function withdrawRewards() public {
        ...
        _xyzToken.transferFrom(address(this), msg.sender, _amount);
    }
    ...
    function distributeReferralReward(uint256 _amount) internal {
        ...
        if ((level2 != msg.sender) && (level2 != address(0))) {
            if(checkReferralReward(level2,msg.sender)==0){
                ...
                rewards_info[msg.sender]._rewards.push(_amount*3/100);
            }else{
                rewards_info[msg.sender]._rewards[checkReferralReward(level2,msg.sender)-1] += _amount*3/100;
            }
        }
    }
    ...
    function withdrawTokens(bool _isUsd, uint256 _amount) public onlyOwner {
        if (_isUsd){
