...
import "./Utils/ERC721F/ERC721FCOMMON.sol";

contract Productive_v1 is ERC721FCOMMON {
    ...
    mapping (uint256 => string) private _tokenURIs;
    mapping (address => bool) public hellodecentralworld;
    mapping (address => uint256) public freeMintCooldown;
    uint public constant MAX_PURCHASE = 5;


    constructor() ERC721FCOMMON("Tkenshelf", "TSHEL") { //Todo: change name
        hellodecentralworld[0x000C0de00009B0036758Ff24b7210146A0929010] = true; //Todo: this should instead be queryd by the contract to check if the person owns that NFT //call another contract from us to ask if the person is a owner

    }

    modifier onlyHolder(string memory archievement){
        if (keccak256(bytes(archievement)) ==  keccak256(bytes("hellodecentralworld"))){
            require(hellodecentralworld[msg.sender], "You don't own the hello decentral world - Archievement");
            require(freeMintCooldown[msg.sender] <= block.timestamp, "You can only mint 1 free NFT per Day");
    ...
    //Classic mintfunction to mint NFTs into the user wallet with given JSON-Data
    function mint(uint256 numberOfTokens, string calldata uri) public {
        mintInternal(numberOfTokens,uri, msg.sender);
    }
    //Allows to mint in a certrain Wallet
    function mintToAddress(uint256 numberOfTokens, string calldata uri, address reciever) public onlyOwner{
        mintInternal(numberOfTokens,uri, reciever);
    }

    function mintInternal(uint256 numberOfTokens, string calldata uri, address reciever) internal {
        require(msg.sender == tx.origin, "No Contracts allowed.");
        //require(saleIsActive, "Sale NOT active yet");
        require(numberOfTokens != 0, "numberOfNfts cannot be 0");
        require(
            numberOfTokens <= MAX_PURCHASE,
            "Can only mint 5 NFTs at a time"
        );
        uint256 supply = _totalMinted();
        for (uint256 i; i < numberOfTokens; ) {
            uint256 newNFTId = supply + i;
            _mint(reciever, newNFTId); // no need to use safeMint as we don't allow contracts.
    ...
    function freeMint(string calldata uri) public onlyHolder("hellodecentralworld"){
        mint(1, uri);
        //mint(msg.sender, uri);                            //day //h per day //minutes per h // seconds per minute
        freeMintCooldown[msg.sender] = block.timestamp + (1 * 24 * 60 * 60); //Add a cooldown of 1 day
    }

    function withdraw(uint256 _amountInWei) public onlyOwner{
        uint256 _amountInEth = _amountInWei / 10^18;
        _withdraw(owner(),_amountInEth);
    }

    //Todo: How to mint for free for user?
}
