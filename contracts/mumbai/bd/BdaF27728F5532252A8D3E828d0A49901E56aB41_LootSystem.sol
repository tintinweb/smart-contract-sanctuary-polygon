...
contract LootSystem is ILootSystem, GameRegistryConsumerUpgradeable {
    ...
    function setLootTable(
        ...
        if (lootTable.mints.length == 0) {
            lootTable.mints = new uint32[](lootTable.loots.length);
        }
     ...
     */
    function grantLoot(address to, Loot[] calldata loots)
        external
        override
        nonReentrant
        onlyRole(GAME_LOGIC_CONTRACT_ROLE)
    {
        ...
        if (needVRF) {
            ...
            for (uint8 idx; idx < loots.length; ++idx) {
                ...
                _finishGrantLoot(to, loots, requestId);
            }
        } else {
     ...
     */
    function getLootTable(uint256 lootTableId)
        external
        view
        returns (LootTable memory)
    {
    ...
    function fulfillRandomWordsCallback(
        ...
        Loot[] storage loots = request.loots;

        if (account != address(0)) {
            _finishGrantLoot(account, loots, randomWords[0]);
    ...
    function _validateLoot(Loot memory loot)
        internal
        view
        returns (bool needsVRF)
    {
    ...
    function _pickLoot(LootTable storage lootTable, uint256 randomWord)
        private
        returns (Loot[] memory)
    {
