...
interface INFTTemplate is IERC721
{
    ...
    function bulkMint(address to, uint256[] memory _tokenIDs,string[] memory _tokenURIs)external;
    function bulkTransfer(address[] memory _to, uint[] memory _tokenIds) external;
}
...
contract Template is Initializable,customERC721, AccessControl, ERC2981, BasicMetaTransaction {
    ...
    function _bulkMint(address to, uint256[] memory _tokenIDs,string[] memory _tokenURIs)internal {
        require(to!=address(0), "zero address");
        require(_tokenIDs.length== _tokenURIs.length,"Uneven length");
        uint256 len= _tokenIDs.length;
        for(uint256 i=0;i<len;i++){
            uint256 tId= _tokenIDs[i];
             _safeMint(to,tId);
             _setTokenURI(tId,_tokenURIs[i]);
        }
        _totalSupply+=len;
    }
    ...
    function bulkTransfer(address[] memory _to, uint[] memory _tokenIDs) external{
        ...
        for(uint i=0;i<len;i++){
            transferFrom(msg.sender,_to[i],_tokenIDs[i]);
        }
...
function initialize(address _templateAddress)external initializer{
    ...
    function create721Token(string memory name, string memory symbol)external returns(address token)
    {
