    ...
    IVRFSubscriber
{
    ...
    event RaffleRewardMinted(
        uint256 _weekNumber,
        address _user,
        uint256[] _tokenIds
    );
    event WinnerAddressesSet(uint256 _weekNumber, address[] _users);
    ...
    mapping(uint256 => WeeklyRaffleConfig) weeklyConfigs;
    ...
    constructor(
        ...
        newWeeklyConfig.endTimestamp = startTimestamp + 10 * 60;
        winnerCount = 40;
    }

    function getWeeklyConfig(uint256 _weekNumber)
        public
        view
        returns (WeeklyConfigResponse memory)
    {
    ...
    function getRemainingCount(uint256 _weekNumber, address _user)
        public
        view
        returns (uint256)
    {
    ...
    function getWinners(uint256 _weekNumber)
        public
        view
        returns (User[] memory)
    {
    ...
    function getPendingCount(uint256 _weekNumber, address _user)
        public
        view
        returns (uint256)
    {
    ...
    function drawWeeklyRaffle(
        ...
        //TODO
        //require(config.hash == bytes32(0), "weekly draw already executed");

        requestId = vrfGenerator.requestRandomWords(1, _callbackGasLimit);
        ...
        requestIds[requestId] = _weekNumber;

        uint256 newWeek = _weekNumber + 1;

        WeeklyRaffleConfig storage newWeeklyConfig = weeklyConfigs[newWeek];
        newWeeklyConfig.startTimestamp = config.startTimestamp + 10 * 60;
        newWeeklyConfig.endTimestamp = config.endTimestamp + 10 * 60;

        return requestId;
    }

    function processRandomness(uint256 _requestId, uint256[] memory _numWords)
        external
        override
    {
        require(msg.sender == address(vrfGenerator), "Unauthorized access");

        uint256 weekNumber = requestIds[_requestId];
        require(
            weeklyConfigs[weekNumber].vrfRequest.status ==
                VRFRequestStatus.PENDING,
        ...
        uint256 value;
        uint256 count = 0;
        for (uint256 i = 0; count < winnerCount || i < raffleCount; i++) {
            value =
                uint256(keccak256(abi.encodePacked(randomNumber, i))) %
                raffleCount;
    ...
    function setWinnerAddresses(
        ...
        for (uint256 i; i < _indices.length; i++) {
            ...
            require(
                config.indexToUser[index] == address(this),
                "Index not found"
            );
        ...
        emit WinnerAddressesSet(_weekNumber, _users);
    }
    ...
    function claim(uint256 _weekNumber, uint256 _count) external {
        ...
        WeeklyRaffleConfig storage config = weeklyConfigs[_weekNumber];
        uint256 nftCounts = config.remainingClaimCount[caller];

        require(_count <= 5 && _count <= nftCounts, "Invalid claim count");

        config.remainingClaimCount[caller] -= _count;
        config.inProgressClaimCount[caller] += _count;

        uint256 requestId = nftMinter.mint(caller, _count);
        requestIds[requestId] = _weekNumber;
        emit RaffleRewardMintRequested(caller, requestId);
    }
    ...
    function acknowledgeMint(
        ...
        require(
            tokenCount <= config.inProgressClaimCount[_user],
        ...
        emit RaffleRewardMinted(weekNumber, _user, _tokenIds);
    }
}
...
interface IWonderVRFGenerator {
    function requestRandomWords(uint32 _numWords, uint256 _callbackGasLimit)
        external
