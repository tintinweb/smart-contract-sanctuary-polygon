...
contract Pool is ReentrancyGuard, Ownable {
    ...
    function transferReward(
        ...
        allowedAssets[_assetId].highestPrice = getTokenPlusBuyPrice(_assetId);
    }
    ...
    ///@param _assetId id of stablecoin asset
    ///@return sell price
    function getTokenPlusSellPrice(
        uint8 _assetId
    ) public view returns (uint256) {
        return getTokenPlusBuyPrice(_assetId);
        /*uint256 x = allowedAssets[_assetId].stablecoin.balanceOf(address(this));
        uint256 y = allowedAssets[_assetId].tokenPlus.totalSupply();
        if (y == 0) {
            return MIN_USDT_PRICE;
        }
...
*/
    ...
    function makeDeposit(uint8 _assetId, uint256 _amount) external {
        uint256 buyPrice = getTokenPlusBuyPrice(_assetId);
