...
contract Traits is Ownable, ITraits {
   ...
   * @return a valid SVG of the Cow / Farmer
   */
  function drawSVG(uint256 tokenId) public view returns (string memory) {
    IFarmer.CowFarmer memory s = farmer.getTokenTraits(tokenId);
    uint8 shift = s.isCow ? 0 : 9;

    string memory svgString = string(abi.encodePacked(
      drawTrait(traitData[0 + shift][s.fur]),
      s.isCow ? drawTrait(traitData[1 + shift][s.head]) : drawTrait(traitData[1 + shift][s.alphaIndex]),
      s.isCow ? drawTrait(traitData[2 + shift][s.ears]) : '',
      drawTrait(traitData[3 + shift][s.eyes]),
      s.isCow ? drawTrait(traitData[4 + shift][s.nose]) : '',
      drawTrait(traitData[5 + shift][s.mouth]),
      s.isCow ? '' : drawTrait(traitData[6 + shift][s.neck]),
      s.isCow ? drawTrait(traitData[7 + shift][s.feet]) : ''
    ));

    return string(abi.encodePacked(
      '<svg id="farmer" width="100%" height="100%" version="1.1" viewBox="0 0 40 40" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">',
      svgString,
      "</svg>"
    ));
  }
  ...
  function compileAttributes(uint256 tokenId) public view returns (string memory) {
    ...
    return string(abi.encodePacked(
      ...
      tokenId <= farmer.getPaidTokens() ? '"Gen 0"' : '"Gen 1"',
  ...
  function tokenURI(uint256 tokenId) public view override returns (string memory) {
    ...
    string memory metadata = string(abi.encodePacked(
      ...
      s.isCow ? 'Cow #' : 'Farmer #',
      tokenId.toString(),
      '", "description": "Thousands of Cows and Farmers compete on a farm in the metaverse. A tempting prize of $MEAT awaits, with deadly high stakes. All the metadata and images are generated and stored 100% on-chain. No IPFS. NO API. Just the Ethereum blockchain.", "image": "data:image/svg+xml;base64,',
...
interface IFarmer {
  ...
  function getTokenTraits(uint256 tokenId) external view returns (CowFarmer memory);
}
