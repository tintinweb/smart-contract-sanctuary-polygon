...
contract CharacterEquipment is ICharacterEquipment, GameBase {
    ...
    function validateEquip(uint256 worldId, uint256 tokenId, uint256[] calldata itemDefinitionIds) external view override returns(EquipValidationResult memory) {
        ...
        for (uint256 i; i < itemDefinitionIds.length; i++) {
            ...
            uint256 oldItemId = equipments[i];

            validSlots[i] = characterDefinition.isValidEquipmentSlot(characterDefinitionId, i);
            equipableItems[i] = characterDefinition.canEquip(characterDefinitionId, itemId, i);
            (, bool enabled,,,) = itemDefinition.getDefinition(itemId);
            validItemDefinitionIds[i] = enabled;
            validItemCounts[i] = (itemCount > 0 && oldItemId != itemId) || (oldItemId == itemId);
        }
    ...
    function equip(uint256 worldId, uint256 tokenId, uint256[] calldata itemDefinitionIds) external override {
        ...
        Character character = getCharacter(worldId, tokenId);
        character.setEquipments(itemDefinitionIds);
