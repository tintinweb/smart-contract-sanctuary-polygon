...
contract DynamicMetadataAndClaiming is ERC721Drop, PermissionsEnumerable {
    constructor(
        string memory _name,
    ...
    /**
     *  @notice                  Lets an authorized address lazy mint a given amount of NFTs, and updates the base URI for
     *                           all NFTs.
     *
     ...
     *  @return batchId          A unique integer identifier for the batch of NFTs lazy minted together.
     */
    function lazyMintAndUpdateBaseURIForAll(
        uint256 _amount,
        ...
        // Reset baseURI and batchIds
        _resetBatchMetadata();

        // Start at the beginning and update the baseURI for all NFTs
    ...
    /**
     * @notice         Lets an authorized address set the URIs for a given batch of NFTs.
     * @dev            Caller should be authorized to set token URIs.
     *                 See {_canSetTokenURIs}.
     *                 Emits {TokensURIsUpdated Event}.
     *
     * @param _baseURI The base URI for the 'n' number of NFTs being set, where the metadata 
     *                 for each of those NFTs is `${baseURI}/${tokenId}`.
     * @param _data    Additional bytes data to be used at the discretion of the consumer of the contract.
     *
     * @return batchId The batch ID for the batch of NFTs being set.
     */
    function setTokenURIs(
        ...
        string memory prevURI = _getBaseURI(0);
        // Loop through batchIds and set each index of baseURI to the new _baseURI
        for (uint256 i = 0; i < getBaseURICount(); i++) {
            batchId = getBatchIdAtIndex(i);
            _setBaseURI(batchId, _baseURI);
        }

        emit TokensURIsUpdated(_baseURI, prevURI, batchId, _data);

        return batchId;
    }

    /// @dev Emitted when the contract URI is updated.
    event TokensURIsUpdated(string newURI, string prevURI, uint256 batchId, bytes data);

    /// @dev Returns whether token metadata can be set in the given execution context.
    function _canSetTokenURIs() internal view returns (bool) {
        return hasRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /// @dev Returns whether lazy minting can be performed in the given execution context.
    function _canLazyMint() internal view override returns (bool) {
        return (msg.sender == owner() || hasRole(keccak256("MINTER_ROLE"), msg.sender));
    }
 ...
 */
abstract contract ERC721Drop is
    ERC721A,
    ContractMetadata,
    Multicall,
    Ownable,
...
contract BatchMintMetadata {
    ...
    /// @dev Resets baseURI and batchIds
    function _resetBatchMetadata() internal {
        uint256 numOfTokenBatches = getBaseURICount();
        for (uint256 i = 0; i < numOfTokenBatches; i += 1) {
            baseURI[i] = "";
        }
...
contract ClaimsLogic is IClaimsLogic {
     ...
     *  @return tokenID    the next token to claim.
     */
    ...
    function _getTotalPrice(
        ...
        if (_isActiveOverride) {
            ...
            uint256 paidQuantity = _quantityToClaim > freeQuantity
                ...
                : 0;
            totalPrice = paidQuantity * _pricePerToken;
...
/**
 *  Based on Igor + André's `ClaimsLogic, IClaimsLogic is a contract extension for any base NFT contract. 
 *  It lets you 'Claim' out of order any number of NFTs at once. This contract handles the logic for 
 *  finding the next token to claim if the claimers token has already been claimed.
 */
