...
contract billionMoney is owned {
    ...
    bool public goLive;
    uint public autopoolMultiplier=3; // multiplier 3x
...
//------------------------FOR TESTING----------------------
    ...
    function payForLevel(uint8 _level, address _user, uint _runtime) internal returns(bool) {
        ...
        if (userGainList[0]==address(0) && _runtime==5){
        ...
        if (userLostList[0]!=address(0)){
            
            loosePerPerson= lvlAmount/(userGainList.length+userLostList.length);
        }

        if (userGainList[0]!=address(0)){

            gainPerPerson = lvlAmount/gainId;            
        }
       
        for(uint i=0; i<looseId;i++){
