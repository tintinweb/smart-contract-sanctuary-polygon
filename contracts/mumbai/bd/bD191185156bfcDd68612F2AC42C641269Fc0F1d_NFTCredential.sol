...
contract NFTCredential is ERC721A, Ownable {
  ...
  function mintAndTransfer(
    ...
    DetailCert[] memory _detailCert
  ) public onlyOwner {
    uint256 requestNum = _detailCert.length;
    require(requestNum > 0, 'The _detailCert is empty.');
    require(
      requestNum <= getMaxBatchSize(),
      'The length of _detailCert must be less than or equal to _maxBatchSize.'
    );

    for (uint256 i = 0; i < requestNum; i++) {
      address _to = _detailCert[i].toAddress;
      ...
      require(
        _to != owner(),
        'toAddress of _detailCert must NOT be included OWNER.'
      );
    }
    ...
    for (uint256 i = 0; i < requestNum; i++) {
      ...
      safeTransferFrom(owner(), _detailCert[i].toAddress, tokenId);
      // update the token URI
      _setTokenURI(tokenId, generateTokenURI(_credentialId, _detailCert[i]));
      tokenId += 1;
    }
  }
  ...
  function updateNFT(
    ...
    // update the tokenURI
    _setTokenURI(tokenId, generateTokenURI(_credentialId, _detailCert));
  }
  ...
  function generateTokenURI(
    ...
    bytes memory _attributes = abi.encodePacked(
      '"attributes": [{ "trait_type": "tags", "value": ',
      _detailCert.skillList,
      ' }, { "trait_type": "issuer_name", "value": "',
      _detailCert.issuerName,
      '" }, { "trait_type": "issue_date", "value": "',
      _detailCert.issuerDate,
      '" }, { "trait_type": "skill_detail", "value": ',
      _detailCert.skillDetail,
      ' }, { "trait_type": "work_place", "value": "',
      _detailCert.workPlace,
    ...
    string memory json = Base64.encode(
      abi.encodePacked(
        ...
        '",'
        '"description": "',
        _detailCert.description,
        '",'
        '"image": "',
        _detailCert.imageURI,
        '",',
        '"external_url": "',
        _detailCert.externalURI,
        '",',
        string(_attributes),
        '}'
      )
