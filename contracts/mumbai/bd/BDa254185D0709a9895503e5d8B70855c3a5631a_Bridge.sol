...
contract Bridge is AccessControl, IBridge {
    ...
    /*//////////////////////////////////////////////////////////////
                           STORAGE
    //////////////////////////////////////////////////////////////*/
    ILayerZeroEndpoint private immutable _layerZeroEndpoint;

    ICrossRouter private _router;

    mapping(uint16 chainId => bytes bridgeAddress) public _bridgeLookup;
     ...
     * @param layerZeroEndpoint Address of the Layer Zero endpoint contract.
     * @param router Address of the Cross Router contract.
     */
    constructor(address layerZeroEndpoint, address router) {
        _layerZeroEndpoint = ILayerZeroEndpoint(layerZeroEndpoint);
    ...
    {
        uint16 version = 1;
        bytes memory adapterParams = abi.encodePacked(version, _gasLookup[chainId][messageType]);

        (uint256 estimatedFee,) = _layerZeroEndpoint.estimateFees(chainId, address(this), payload, false, adapterParams);
        if (estimatedFee > msg.value) {
            revert InsuficientFee(estimatedFee);
        }

        _layerZeroEndpoint.send{ value: msg.value }(
            chainId, _bridgeLookup[chainId], payload, refundAddress, address(this), adapterParams
        );
    ...
    // function lzReceive(
    //     uint16 srcChainId_,
    //     bytes calldata srcAddressBytes_,
    //     uint64 nonce_,
    ...
    //         _failedMessages[srcChainId_][srcAddressBytes_][nonce_] = keccak256(payload_);
    ...
    function lzReceive(
        uint16 srcChainId,
        bytes calldata srcAddressBytes,
        uint64,
        bytes calldata payload
    )
        external
        override
    {
        nonBlockingReceive(srcChainId, srcAddressBytes, payload);
    }
     ...
     * @inheritdoc IBridge
     */
    function setBridge(uint16 chainId, bytes calldata bridgeAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _bridgeLookup[chainId] = abi.encodePacked(bridgeAddress, address(this));
    }
    /**
     * @inheritdoc IBridge
     */
    ...
    function forceResumeReceive(
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        _layerZeroEndpoint.forceResumeReceive(srcChainId, srcAddress);
    }
    ...
    function setConfig(
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        _layerZeroEndpoint.setConfig(version, chainId, configType, config);
    }
    ...
    function setSendVersion(uint16 version) external override onlyRole(DEFAULT_ADMIN_ROLE) {
        _layerZeroEndpoint.setSendVersion(version);
    }
    ...
    function setReceiveVersion(uint16 version) external override onlyRole(DEFAULT_ADMIN_ROLE) {
        _layerZeroEndpoint.setReceiveVersion(version);
    }
    ...
    function nonBlockingReceive(uint16 srcChainId, bytes calldata srcAddressBytes, bytes calldata payload) public {
        // TODO remove this for testing
        // if (msg.sender != address(this) && !hasRole(DEFAULT_ADMIN_ROLE, msg.sender)) revert InsufficientAccess();
        // if (
        //     srcAddressBytes_.length != _bridgeLookup[srcChainId_].length ||
        //     keccak256(srcAddressBytes_) != keccak256(_bridgeLookup[srcChainId_])
        // ) {
    ...
    function _handleReceive(uint16 srcChainId, bytes calldata payload) internal {
        ...
        if (functionType == 1) {
            ...
            ICrossRouter.SwapMessage memory swapMessage = _decodeSwapMessage(payload);
            ...
            ICrossRouter.LiquidityMessage memory liquidityMessage = _decodeLiquidityMessage(payload);
        ...
        returns (uint256, uint256)
    {
        ...
        if (messageType == MESSAGE_TYPE.SWAP) {
            encodedMessage = abi.encodePacked(
                ...
                type(uint256).max,
                type(uint256).max,
                keccak256(abi.encode(address(this), 1, 1, MESSAGE_TYPE.SWAP)),
                payload
            );
        } else if (messageType == MESSAGE_TYPE.ADD_LIQUIDITY) {
            encodedMessage = abi.encodePacked(
        ...
        uint16 version = 1;
        bytes memory adapterParams = abi.encodePacked(version, _gasLookup[chainId][messageType]);
        return _layerZeroEndpoint.estimateFees(chainId, address(this), encodedMessage, false, adapterParams);
    }

    /**
     * @inheritdoc IBridge
     */
    function nextNonce(uint16 dstChain) public view override returns (uint256) {
        return _layerZeroEndpoint.getOutboundNonce(dstChain, address(this)) + 1;
    }
    ...
    function _decodeSwapMessage(bytes calldata payload)
        private
        pure
        returns (ICrossRouter.SwapMessage memory swapMessage)
    {
        assembly ("memory-safe") {
            ...
            // receiver
            mstore(add(swapMessage, 0x60), and(shr(56, firstPayloadChunk), 0xffffffffffffffffffff))
            ...
            // Starting at amount slot (0x80), then Fee's, Vouchers, optimalDstBandwidth, id, so 5 bytes32 slot to copy,
            // so 256
            calldatacopy(add(swapMessage, 0x80), add(payload.offset, 0x19), 0x100)
        }

        swapMessage.payload = payload[185:];
    }

    function _decodeLiquidityMessage(bytes calldata callDataPayload)
        private
        pure
        returns (ICrossRouter.LiquidityMessage memory liquidityMessage)
    {
        assembly ("memory-safe") {
