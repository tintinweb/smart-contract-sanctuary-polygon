...
contract Escrow {
    ...
    uint64 public super_admin_claim_period = 60 * 60;
    ...
    constructor(
        ...
        withdrawal_period = times[0]*60;   
        delivery_period = times[1]*60;     
        inspection_period = times[2]*60;   
        extension_period = times[3]*60;    
        request_reply_period = times[4]*60;
    ...
    function claim(uint64[] memory _order_nums) external isPause onlyAdmin reentrencyGuard{
        for(uint64 i; i<_order_nums.length; i++) {
            ...
            require( ((orders[_order_num].time
                        + withdrawal_period + delivery_period +inspection_period 
                        + extension_time[_order_num]
                        +  super_admin_claim_period) > block.timestamp) && (block.timestamp < (orders[_order_num].time
                        + withdrawal_period + delivery_period +inspection_period 
                        + extension_time[_order_num]))
                     ,"CTP");
            orders[_order_num].status = Status.CO;
...
contract Factory {
    ...
    modifier ValidToken(address _token) {
        require (token[_token].status, "Invalid Token");
    _;}
    modifier isPause() {
    ...
    function deployEscrowContract(
        ...
        uint64[6] memory _times) external isPause {
            require(token[_tokens[0]].status 
                && token[_tokens[1]].status 
                && token[_tokens[2]].status 
                && token[_tokens[3]].status 
                && token[_tokens[4]].status,
                 "Invalid token");
