...
contract RafflePanel {
    ...
    function purchaseTicketsWithTokens(uint256 raffleId, uint256 numTickets, address tokenAddress) external {
        ...
        require(totalPrice > 0, "Invalid ticket price.");

        IERC20 tokenContract = IERC20(tokenAddress);
        require(tokenContract.balanceOf(msg.sender) >= totalPrice, "Insufficient token balance.");
    ...
    function withdrawServiceFee() external onlyAdmin {
        ...
        uint256 serviceFee = balance.mul(serviceFeePercentage).div(100);
        require(serviceFee > 0, "No service fee to withdraw.");
    ...
    function setServiceFeePercentage(uint256 percentage) external onlyAdmin {
        require(percentage <= 100, "Invalid service fee percentage.");
