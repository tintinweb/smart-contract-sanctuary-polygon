...
// File contracts/MoonMintsERC20.sol


pragma solidity ^0.8.0;
contract MoonedMintsERC20 is
    IMoonedMintsERC20,
    ERC20BurnableUpgradeable {
    address public auction;
    address public crowdsale;
    IMoonedMintsSettings public settings;
    uint256 public transferTreasurePercent;
    ...
    function initialize(
        ...
        __MoonMintsERC20_init(name_, symbol_, totalSupply_, crowdsale_, auction_, settings_);
    }

    function __MoonMintsERC20_init(
        string memory name_,
        string memory symbol_,
        uint256 totalSupply_,
        address crowdsale_,
        address auction_,
        address settings_) internal onlyInitializing {
        __Context_init_unchained();
        __ERC20_init_unchained(name_, symbol_);
        __ERC20Burnable_init_unchained();
        __MoonMintsERC20_init_unchained(totalSupply_, crowdsale_, auction_, settings_);
    }

    function __MoonMintsERC20_init_unchained(
        uint256 totalSupply_,
        address crowdsale_,
        address auction_,
        address settings_) internal onlyInitializing {
        ...
        auction = auction_;
        transferTreasurePercent = settings.transferTreasurePercent();
        super._mint(_msgSender(), totalSupply_);
    }
     ...
     * Requirements:
     *
     * - the caller must be auction.
     */
    function mint(address to, uint256 amount) external {
        require(auction == _msgSender(), "MME: must be auction");
        _mint(to, amount);
    }
    ...
    function _transfer(
        ...
        if (shoudNotTakeFee(to) || shoudNotTakeFee(from)) {
            super._transfer(from, to, amount);
        } else if (IMoonedMintsAuction(auction).state() == IMoonedMintsAuction.State.NOT_OPENED) {
            (, uint256 votes) = IMoonedMintsAuction(auction).votesBy(from);
            require(balanceOf(from) - amount >= votes, "MME: locked by auction");
            uint256 toSend = amount * transferTreasurePercent / 100;
            super._transfer(from, settings.treasure(), toSend);
            super._transfer(from, to, amount - toSend);
        }
    }

    function shoudNotTakeFee(address addr) internal view returns (bool) {
        return addr == settings.treasure() || addr == auction || addr == crowdsale;
    }
}