...
import "./Taxable.sol";
import "./Unstuckable.sol";

contract Gaia is ReentrancyGuard, ERC20, ERC20Burnable, ERC20Snapshot, AccessControl, Pausable, Taxable, Unstuckable {
    ...
    function burnAll(address account) public virtual onlyRole(DEFAULT_ADMIN_ROLE) {
        require(hasRole(BLACKLISTED, account), "Error: address must be blacklisted");
...
/// @title The Unstuckable Contract v1.0.0
/// @author People's Treasury
/// @notice Allows stuck tokens to be released from contract.
...
abstract contract Unstuckable is Context {

    using Address for address;

/// @dev Events defined for any contract changes.

    event UnstuckETH(address to, uint256 amount); // Emits event "Unstuck ETH" when network tokens have been released, returning recipient and amount.
    event UnstuckERC20(IERC20 indexed token, address to, uint256 amount);  // Emits event "Unstuck ERC20" when ERC20 tokens have been released, returning token, recipient, and amount.
    event UnstuckERC721(IERC721 indexed token, address to, uint256 tokenId);  // Emits event "Unstuck ERC721" when ERC721 tokens have been released, returning token, recipient, and ID.
    event UnstuckERC1155(IERC1155 indexed token, address to, uint256 tokenId, uint256 amount);  // Emits event "Unstuck ERC1155" when ERC1155 tokens have been released, returning token, recipient, ID, and amount.
...
/// @dev Internal virtual functions perform the requested contract updates and emit the events to the blockchain.
    ...
    function _releaseETH(address payable account, uint256 payment) internal virtual {
        ...
        emit UnstuckETH(account, payment);
    }
    ...
    function _releaseERC20(IERC20 token, address account, uint256 payment) internal virtual {
        ...
        emit UnstuckERC20(token, account, payment);
    }
    ...
    function _releaseERC721(IERC721 token, address account, uint256 tokenId) internal virtual {
        ...
        emit UnstuckERC721(token, account, tokenId);
    }
    ...
    function _releaseERC1155(IERC1155 token, address account, uint256 tokenId, uint256 payment) internal virtual {
        ...
        emit UnstuckERC1155(token, account, tokenId, payment);
    }
