    ...
    function mint(uint256 tokenId) private returns (uint256 amount) {
        ...
        if (
            params.nftAddress != address(_nft) || params.bondingCurveAddress != bondingCurve
                || !validator.validate(_pool, curveParams, fee, tokenIDFilterRoot)
        ) revert PoolMismatch();
...
interface IValidator {
    ...
    function validate(ICollectionPool pool, ICurve.Params calldata params, uint96 fee, bytes32 tokenIDFilterRoot)
        external
        returns (bool);
}
