    ...
    Oracle
{
        ...
        external onlyUser noReentrant  
    {
        ...
        IERC721(pair.nftAddress).transferFrom(address(this), _wallet, _tokenId);
    }
        ...
        external noReentrant
    {
        ...
        /// @dev get the token from the white list
        ERC20List storage erc20List = whitelistTokensPay[_token];
        require(erc20List.active, "Buy With Custom Token: Token is not available");
        

        /// @dev check pair is native
        require(!erc20List.isNative, "Buy With Custom Token: not custom token native sended");

        /// @dev Validate user approve amount
        require(
            IERC20(erc20List.tokenAddress).allowance(_msgSender(), address(this)) >=
                _amountTokens,
        ...
        require(
            IERC20(erc20List.tokenAddress).balanceOf(_msgSender()) >= _amountTokens,
        ...
        require(
            IERC20(erc20List.tokenAddress).transferFrom(
                _msgSender(),
                address(this),
                _amountTokens
            ),
        ...
        uint256 latestPrice = getLatestPrice(
            erc20List.addressOracle,
            erc20List.addressDecimalOracle
        );

        /// @dev Parse amount to 18 decimals
        uint256 amountTo18 = transformAmountTo18Decimal(
            _amountTokens,
            uint8(ERC20(erc20List.tokenAddress).decimals())
        );
        ...
        /// @dev Check Contract is the owner of the NFT
        require(
            address(this) == IERC721(pair.nftAddress).ownerOf(_tokenId),
        ...
        noReentrant
    {
        ...
        /// @dev get the token from the white list
        ERC20List storage erc20List = whitelistTokensPay[_token];
        require(erc20List.active, "Buy With Native Token: Token is not available");

        /// @dev check pair is native
        require(erc20List.isNative, "Buy With Native Token: The token sended is not native");
        ...
        require(
            valueInUsd >= pair.price,
            "Buy With Native Token: The price is lower than the minimum price"
        );

        /// @dev Check Contract is the owner of the NFT
        require(
            address(this) == IERC721(pair.nftAddress).ownerOf(_tokenId),
            "Buy With Native Token: Sender must be owner"
        );
...
contract FactoryTokens is Administered {
    ...
    /// @dev struct
    struct Pair {
        address nftAddress;                         // NFT to sell
        ...
        onlyUser
    {
        ...
        /// @dev save the pair
        _PairstoSwap[pairCount] = Pair(_nftAddress, _price, _active);
        ...
        onlyUser 
    {
        ...
        /// @dev Update NFT contract address
            ...
            _PairstoSwap[_id].nftAddress = _address;
...
contract WhiteListTokens is Administered {
    ...
    mapping(address => ERC20List) public whitelistTokensPay;
        ...
        onlyUser
    {
        ...
        require(
            isPayTokenExist(_tokenAddress) == false,
            "Add Token Collection already exist"
        );

        /// @dev add token to whitelist
        whitelistTokensPay[_tokenAddress] = ERC20List(
            _tokenAddress,
            _addressOracle,
        ...
        onlyUser 
    {
        ...
        require(
            isPayTokenExist(_tokenAddress) == true,
            "Remove Token: Collection does not exist"
        );

        /// @dev remove token from whitelist
        whitelistTokensPay[_tokenAddress].active = false;
    }
    ...
    /// @notice Get Token
    /// @dev Get token from the whitelist
    /// @param _tokenAddress                            Address of the token contract
    ...
    {
        return whitelistTokensPay[_tokenAddress];
    }
    ...
    /// @param _tokenAddress                            Address of the token contract
    /// @return bool                                    Status of the pair
    function isPayTokenExist(address _tokenAddress) public view returns (bool) {
        return whitelistTokensPay[_tokenAddress].active;
    }
}
