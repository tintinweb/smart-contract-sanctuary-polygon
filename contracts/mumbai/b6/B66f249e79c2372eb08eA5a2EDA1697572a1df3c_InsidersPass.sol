...
//    uint256 public checkFreePromo = 0;      


    // //Total Copies minted wrt a ticket
    mapping(uint256 => uint256) totalCopiesMinted;
    mapping(uint256 => string) tokenURI;
    //PromoCode => eventID => valid/invalid
    mapping(string => mapping(uint256 => bool)) public promoIsValid; //promo for EventID old map
    
    mapping(string => mapping(uint256 => bool)) public promoIsValidForTicket; //promo for TicketID new map for promo to event with ticket

    event URI(string value, bytes indexed id);
    event PromoCodeUsed(string value); //to get promocode when use to get ticket
    ...
    struct PromoCodes {
        ...
        uint256 maxUsers;           //maxusers tht can avail a promocode 
        uint256 claimedUsers;       //users that have availed the promocode 
        uint256 maxDiscountAmount;
        bool discountIsPercentage;
        bool isFree; //here add for check ticket is free or not
        uint256 tiketIDs;  //here we are add tiket id with promo
    }
    ...
    function mintTicketsWithFixPrice(
        ...
        if (_msgSender() != owner()) {
            if (
                ...
                ////////////////////////////////////////////// here i add set required
    //    if(promoCodeDetails[_promoCode].isFree){
    //     ///here we add logic of free ticket
    //      require(promoIsValid[_promoCode][ticketTypeDetails[ticketID].eventIDforTicket],"InvalidPromo");
        
        //////////////// here i add to set isFree True
    //    }
    //        else 
    // 
    if(promoCodeDetails[_promoCode].isFree){

require(promoIsValidForTicket[_promoCode][ticketID],"Here is ticketEROR");
// checkFreePromo++; // add here to check the function is working by calling is on free

        // check ticket type id with required statemnt
//    require(
//                     promoIsValid[_promoCode][
//                         ticketTypeDetails[ticketID].tktId
//                     ], //here i addd tkID
//                     "Invalid Promo ON this Ticket Of ID"
//                 );
        //here im addding to check free tikcet id

//   require(promoIsValid[_promoCode][ticketID] ==], //here i addd tkID
//   "Invalid Promo ON this Ticket Of ID");

                       uint256 newDiscountedAmount = 0;
// ticketTypeDetails[ticketID].ticketFixPrice -((promoCodeDetails[_promoCode].discountPercentage /100) *(ticketTypeDetails[ticketID].ticketFixPrice));
                        if (
    ...
    //here is normal running on percentage and fixed price
           if (promoCodeDetails[_promoCode].discountIsPercentage) {
                        ...
                        // here i add && and secound check TO  validate the promoCode is Not for free , this is for fixed amount
        ...
        emit PromoCodeUsed(_promoCode); // i add to emit (see in log promo T/F)
    }
    ...
    function addPromoCodes(
        uint64[] memory _eventIDs, /// 1,2,3,4
        ...
        for (uint256 i = 0; i < _eventIDs.length; i++) {
            for (uint256 j = 0; j < _promoCodes.length; j++) 
            {
                ...
                promoCodeDetails[promoCode[j]] = _promoCodes[j];
                  if(promoCodeDetails[promoCode[j]].isFree)
                  { // here i will check to addpromo
               promoIsValidForTicket[promoCode[j]][_promoCodes[j].tiketIDs] = true;
       }
            }
        }
        ///////////////////////////////////////////////////////////////// here i add to store a single promo with tikect id
        //     for (uint256 j = 0; j < _promoCodes.length; j++) {
        //         promoIsValids[promoCode[j]][_promoCodes[j].tiketIDs] = true;
        //         promoCodeDetails[promoCode[j]] = _promoCodes[j];
            
        // }
    //////////////////////////////////////////////////////////////////////////           here i check old contract
    //         if(promoCodeDetails[promoCode[0]].isFree){
            
    //             // promoIsValids[promoCode[0]][_eventIDs[0]] = true;

    //            promoIsValid[promoCode[0]][_promoCodes[0].tiketIDs] = true;
    //     ///here we add logic of free ticket
    //     //  require(promoIsValid[_promoCode][ticketTypeDetails[ticketID].eventIDforTicket],"InvalidPromo");
    //    }
    }
    ...
    InsidersPassRoyalties //is SpaceERC20
{
    ...
    struct TicketType {
        ...
        uint256[] validDates;
        // uint256 tktId; // here may i pass ticket id to check event with id
        uint256 totalCopies;
        string ticketURI;
        uint256 copiesSold;
...
// here said by PM to For rent
    ...
    //     uint256 packageFixPrice
    // ) external {
    //     require(_endTime > block.timestamp && eventDetails[_eventID].eventEndDate > _endTime, "EndTime>CurrentTime");
    //     checkPackageConditions(_eventID, _ticketIDs, ticketCopiesCount);
    //     packagesCount++;
    //     uint256 newPackageID = packagesCount;
    ...
    //     emit availableForRent(newPackageID, "AvailableForRent");
    // }
//2
    //PENDING YET
   
   // here said by PM for reducing size
    ...
    //         eventDetails[_eventID].servicePercentage
    //     );
    //     emit packageSold(packageID, "PackageBorrowed");
    // }
