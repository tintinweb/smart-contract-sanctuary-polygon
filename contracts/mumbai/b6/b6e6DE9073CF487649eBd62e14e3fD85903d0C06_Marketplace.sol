...
contract Marketplace is Initializable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function listItem(
    ...
    ) external whenNotPaused nonReentrant notListed(_nftAddress, _tokenId, msg.sender) {
        ...
        listings[_nftAddress][_tokenId][msg.sender] = Listing(_quantity, _payToken, _pricePerItem, _startingTime);
        emit ItemListed(
            msg.sender,
            _nftAddress,
            _tokenId,
    ...
    function updateListing(
        ...
        address _payToken,
        uint256 _newPrice
    ) external whenNotPaused nonReentrant isListed(_nftAddress, _tokenId, msg.sender) {
    ...
    /// @notice A Method for buying assets from lazy minted vouchers
    /// @dev Pass the voucher from the frontend in order to mint it here
    /// @param _voucher Refer RequestVoucher struct in interfaces/IMintRequests.sol
    /// @param _payToken Token the buyer wishes to pay with
        ...
        nonReentrant
    {
        ...
        if (_voucher.deployAndMint) {
            ...
            if (!_voucher.isMultiToken) {
                ...
                (_nftAddress, _tokenId) = IMultiTokenFactory(addressRegistry.MultiTokenFactory()).createNFTContractFromRequest(
                    _requestID,
                    _payToken,
                    msg.sender
                );
    ...
    function _buyItem(
        ...
        require(total <= price, "Royalty splitup shouldn't be greater than 99.99% of actual price");
        IERC20Upgradeable(_payToken).safeTransferFrom(msg.sender, _owner, price - total);
    ...
    function createOffer(
        ...
        uint256 _pricePerItem,
        uint256 _deadline
    ) external whenNotPaused nonReentrant offerNotExists(_nftAddress, _tokenId, msg.sender) {
        require(
        ...
        require(
            IERC20Upgradeable(_payToken).allowance(msg.sender, address(this)) >= _pricePerItem * _quantity,
    ...
    function acceptOffer(
        ...
        require(total <= price, "Royalty splitup shouldn't be greater than 99.99% of actual price");
        offer.payToken.safeTransferFrom(_creator, msg.sender, price - total);
        // offer.payToken.safeTransferFrom(_creator, msg.sender, price);
