...
contract VMarket is AccessControl {
    ...
    function addYesBet(uint256 _marketId, uint256 _value) public payable {
        require(
            _value <= ERC20(vToken).allowance(msg.sender, address(this)),
            "Not allowed to spend this amount."
        );
        Markets storage market = markets[_marketId];
        ERC20(vToken).transferFrom(msg.sender, address(this), _value);
    ...
    function distributeWinningAmount(
        uint256 _marketId,
        bool eventOutcome
    ) public payable onlyRole(RESOLVER_ROLE) {
        Markets storage market = markets[_marketId];
        if (eventOutcome) {
            ...
            for (uint256 i = 0; i < winningAddresses.length; i++) {
                ...
                ERC20(vToken).transfer(_address, winningAmount[_address]);
