...
contract Stake is ReentrancyGuard, AccessControl {
    ...
    mapping(address => uint256) public stakerToAmount;
    mapping(address => uint256[]) public stakerToNFTs;
    mapping(uint256 => address) public tokenToStaker;
    address[] public beneficiaries;
    mapping(address => bool) public isBeneficiary;

    WaitingForApproval[] public waitingForStake;
    WaitingForApproval[] public waitingForUnstake;

    uint lastTimeRewarded;

    function setNftCollection(IERC721 _nftCollection)
        external
    ...
    function setRewardsToken(IERC20 _rewardsToken)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        rewardsToken = _rewardsToken;
    ...
    function setCalculator(Calculator _calculator)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        calculator = _calculator;
    ...
    function getWaitingForStake()
        public
        view
        returns (WaitingForApproval[] memory)
    {
        return waitingForStake;
    }

    function getWaitingForWithdraw()
        public
        view
        returns (WaitingForApproval[] memory)
    {
        return waitingForUnstake;
    }

    function getStakedNFTs(address _staker)
        public
        view
        returns (uint256[] memory)
    {
        return stakerToNFTs[_staker];
    }
    ...
    function stake(uint256 _tokenId) external nonReentrant {
        ...
        WaitingForApproval memory waitingForApproval = WaitingForApproval({
            tokenId: _tokenId,
            beneficiary: msg.sender
        });
        waitingForStake.push(waitingForApproval);
        tokenToStaker[_tokenId] = msg.sender;
        stakerToNFTs[msg.sender].push(_tokenId);

        if (isBeneficiary[msg.sender] == false) {
            isBeneficiary[msg.sender] = true;
            beneficiaries.push(msg.sender);
        }
    }

    function stakes(uint256[] calldata _tokenIds) external nonReentrant {
       for(uint256 i = 0; i < _tokenIds.length; i++){
        ...
        stakerToNFTs[msg.sender].push(_tokenIds[i]);
    ...
    function approveStake() external onlyRole(APPROVER_ROLE) nonReentrant {
        for (uint256 i = 0; i < waitingForStake.length; i++) {
            WaitingForApproval memory waitingForApproval = waitingForStake[i];
            stakerToAmount[waitingForApproval.beneficiary]++;
            stakedCount.increment();
        }

        delete waitingForStake;
    }

    function unstake(uint256 _tokenId) external nonReentrant {
        require(
            tokenToStaker[_tokenId] == msg.sender,
            "staker don't own this token"
        );
        tokenToStaker[_tokenId] = address(0);

        uint index = 0;
        for (uint i = 0; i < stakerToNFTs[msg.sender].length; i++) {
            if (stakerToNFTs[msg.sender][i] == _tokenId) {
                index = i;
                break;
            }
        }
        require(index != stakerToNFTs[msg.sender].length, "token not found");
        _removeArrayElement(stakerToNFTs[msg.sender], index);
        WaitingForApproval memory waitingForApproval = WaitingForApproval({
            tokenId: _tokenId,
            beneficiary: msg.sender
        });
        waitingForUnstake.push(waitingForApproval);
    }

    function approveUnstake() external onlyRole(APPROVER_ROLE) nonReentrant {
        for (uint256 i = 0; i < waitingForUnstake.length; i++) {
            WaitingForApproval memory waitingForApproval = waitingForUnstake[i];
            stakerToAmount[waitingForApproval.beneficiary]--;
            stakedCount.decrement();
            nftCollection.transferFrom(
                address(this),
                waitingForApproval.beneficiary,
                waitingForApproval.tokenId
            );
        }

        delete waitingForUnstake;
    }

    function reward(uint8 fundraiseID)
        external
        onlyRole(APPROVER_ROLE)
        nonReentrant
    {
        require(
            block.timestamp - lastTimeRewarded >= 1 days,
            "Already rewarded today"
        );

        (uint toTreasury, uint toStakers) = calculator
            .calculateTokensProportion(fundraiseID, stakedCount.current());
        uint256 tokenPerNFT = toStakers / stakedCount.current();
        for (uint256 i = 0; i < beneficiaries.length; i++) {
            address staker = beneficiaries[i];
            uint256 amount = stakerToAmount[staker];
            if (amount > 0) {
                _transfer(staker, tokenPerNFT * amount);
            }
        }

        if (toTreasury > 0) {
            _transfer(treasury, toTreasury);
        }
        lastTimeRewarded = block.timestamp;
        emit Reward(
            toStakers,
            toTreasury,
            stakedCount.current(),
    ...
    // ====================== Internal =========================================
    function _removeArrayElement(uint[] storage arr, uint _index) internal {
    ...
    function _transfer(address to_, uint value_) private {
        rewardsToken.transfer(to_, value_);
    }
}
