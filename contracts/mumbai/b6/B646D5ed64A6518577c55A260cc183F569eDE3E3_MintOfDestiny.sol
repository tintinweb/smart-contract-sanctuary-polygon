...
// File: contracts/MintOfDestinyHeroes.sol
...
contract MintOfDestiny is Pausable, AccessControl, Ownable, ReentrancyGuard {
    ...
    */
    ...
    bytes32 public constant _trustedContracts = keccak256("_trustedContracts");
    ...
    constructor(){
        ...
        _grantRole(_admins, msg.sender);
        _grantRole(_trustedContracts, msg.sender);
        _setRoleAdmin(_admins,_admins);
        _setRoleAdmin(_trustedContracts,_admins);
    }
    ...
    */
    ...
    modifier onlyAdmins() {
        require(hasRole(_admins, msg.sender), "Only admins can call this function.");
        _;
    }

    modifier onlyTrustedContracts() {
        require(hasRole(_trustedContracts, msg.sender), "Only trusted contracts can call this function.");
        _;
    }
    ...
    */
    ...
    /**
    * Adds an address to the _admins role and the _trustedContracts role
    * @param newAdmin : the address of the admin address to add
    */
    function addAdmin(address newAdmin) public onlyAdmins{
        grantRole(_admins, newAdmin);
        grantRole(_trustedContracts, newAdmin);
    }

    /**
    * Remove an address from the _admins role and the _trustedContracts role
    * @param oldAdmin : the address of the admin address to remove
    */
    function removeAdmin(address oldAdmin) public onlyAdmins{
        revokeRole(_admins, oldAdmin);
        revokeRole(_trustedContracts, oldAdmin);
    }

    /**
    * Add a contract address to the _trustedContracts role
    * @param newContract : the address of the contract to add to the role
    */
    function addTrustedContract(address newContract) public onlyAdmins{
        grantRole(_trustedContracts, newContract);
    }

    /**
    * Remove a contract address from the _trustedContracts role
    * @param oldContract : the address of the contract to remove from the role
    */
    function removeTrustedContract(address oldContract) public onlyAdmins{
        revokeRole(_trustedContracts, oldContract);
    }
    ...
    function mint(address sendTo, string calldata roleName) private whenNotPaused {
        ...
        // Make sure they haven't already been awarded this role (1 mint per address)
    ...
    function privateMint(address sendTo, string calldata roleName) public onlyAdmins whenNotPaused {
        // Call to mint
        mint(sendTo, roleName);
    }

    function publicMint(string calldata roleName) public payable whenNotPaused nonReentrant {

        // Require the minters to be human
        ...
        // Call to mint
        mint(msg.sender, roleName);

    }

    function addRole(string calldata roleName, uint256 tokenLimit, bool allowPublicMint, uint256 price, string calldata mandatoryRole) public onlyAdmins whenNotPaused {
    ...
    function getCostOfTokenByRole(string memory roleName) public view returns(uint256){
        return roles[roleName].price;
    }
    ...
    function updateRoleOwnership(uint256 tokenId, address addr, bool flag) external onlyTrustedContracts {
    ...
    function updateMoDContract(address _mod) public onlyAdmins {
        mod = IMoD(_mod);
    }
