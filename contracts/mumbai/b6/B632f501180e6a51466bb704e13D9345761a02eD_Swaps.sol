...
contract Swaps is ERC721Enumerable, Ownable, SuperAppBase{
    ...
    constructor(ISuperfluid host, IConstantFlowAgreementV1 cfa, address _token)
    Ownable()
    ERC721("Total Return Swap", "TRS")
    {
        _host = host;
        _cfa = cfa;
    ...
    function newSwap(address _receiver, address _payer, int96 _requiredFlowRate, uint _amountUnderlying) external{
        cfaV1.createFlowByOperator(_payer, address(this), token, _requiredFlowRate, new bytes(0));
        ...
        _newFlow(_receiver, _requiredFlowRate);
        // mint NFTs
        _mintReceiver(_receiver,_amountUnderlying, _requiredFlowRate, msg.sender);    // note receiver will always have an even ID 0,2,4,ect.
