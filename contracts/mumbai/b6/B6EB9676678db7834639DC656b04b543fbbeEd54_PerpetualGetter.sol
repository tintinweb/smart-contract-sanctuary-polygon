...
contract PerpetualGetter is PerpetualUpdateFunctions, IFunctionList, IPerpetualGetter {
     ...
     * @param _perpetualIds array of perpetual IDs
     * @return array with perpetual data for given IDs
     */
    function getPerpetuals(uint24[] calldata _perpetualIds)
        external
        view
        override
        returns (PerpetualData[] memory)
    {
        PerpetualData[] memory perpData = new PerpetualData[](_perpetualIds.length);
        for (uint256 k = 0; k < _perpetualIds.length; k++) {
            perpData[k] = _getPerpetual(_perpetualIds[k]);
        }
        return perpData;
    }
     ...
     * @param _perpetualIds array of perpetual ids
     * @param _idxPriceDataPairs array of index prices, in same order as perpetual ids
     *  index prices for the k-th perpetual are at positions [2*k, 2*k+1]
     * @return array of mid-prices in ABDK format
     */
    function queryMidPrices(uint24[] calldata _perpetualIds, int128[] calldata _idxPriceDataPairs)
        external
        view
        override
        returns (int128[] memory)
    {
        int128[] memory prices = new int128[](_perpetualIds.length);
        require(_perpetualIds.length * 2 == _idxPriceDataPairs.length, "input len mismatch");
        for (uint256 i = 0; i < _perpetualIds.length; i++) {
            prices[i] = _getTradeLogic().queryPerpetualPrice(
                _perpetualIds[i],
                0,
                [_idxPriceDataPairs[i * 2], _idxPriceDataPairs[i * 2 + 1]]
            );
        }
        return prices;
    }
     ...
     *  0: L1
     *  1: K2
     *  M = pricing cash = pool cash + margin cash
     *  2: M1 or 0
     *  3: M2 or 0
     *  4: M3 or 0
     ...
     *  11:open interest
     *  12:pool liquidity used (%, decimals)
     *  13:perpetual state
     *  14:funding rate
        ...
        returns (int128[15] memory)
    {
        ...
        int128 fPricingPnLCash = _getPerpetualAllocatedFunds(perpetual);
        ammState[idx] = fPricingPnLCash.add(ma.fCashCC); // M1, M2, or M3
        ...
        if (lp.fTargetDFSize > 0) {
            ...
            ammState[12] = fPricingPnLCash.div(lp.fPnLparticipantsCashCC);
        }
     ...
     */
    function getPerpetualStaticInfo(uint24[] calldata _perpetualIds)
        external
        view
        virtual
        override
    ...
    {
        PerpetualStaticInfo[] memory info = new PerpetualStaticInfo[](_perpetualIds.length);
        for (uint256 k = 0; k < _perpetualIds.length; k++) {
            PerpetualData storage perpetual = _getPerpetual(_perpetualIds[k]);
            address lo = LimitOrderBookFactory(orderBookFactory).orderBooks(_perpetualIds[k]);
            (bytes32[] memory priceIds, bool[] memory isPyth) = _getPriceInfo(_perpetualIds[k]);
        ...
        returns (address[] memory unsafeAccounts)
    {
        ...
        {
            PerpetualData storage perpetual = _getPerpetual(_perpetualId);
            fS3 = _getCollToQuoteFromIndex(_fIndexPrice, perpetual.eCollateralCurrency);
            fB2CAtMark = _fIndexPrice[0]
                .mul(perpetual.currentMarkPremiumRate.fPrice.add(ONE_64x64))
                .div(fS3);
            fMaintenanceMarginRate = int128(perpetual.fMaintenanceMarginRate) << 35;
        }
        address[] memory accounts = activeAccounts[_perpetualId].enumerateAll();
        uint256 liquidatable;
        for (uint256 i = 0; i < accounts.length; i++) {
            MarginAccount memory ma = marginAccounts[_perpetualId][accounts[i]];
            if (ma.fPositionBC != 0) {
                int128 fCashCC = _getMarginViewLogic().getAvailableCash(_perpetualId, accounts[i]);
                // bal - required margin = cash + (pos * Sm - L) / S3 - m_r * |pos| * Sm / S3
                //     = cash + (pos - mr * |pos|) * B2Cfx - L / C2Qfx
                int128 fAmountAbove = fCashCC
                    .add(
                        ma.fPositionBC.sub(ma.fPositionBC.mul(fMaintenanceMarginRate).abs()).mul(
                            fB2CAtMark
                        )
                    )
                    .sub(ma.fLockedInValueQC.div(fS3));
                if (fAmountAbove < 0) {
                    accounts[liquidatable] = accounts[i];
                    liquidatable++;
                }
            }
        }
        unsafeAccounts = new address[](liquidatable);
        for (uint256 i = 0; i < liquidatable; i++) {
            unsafeAccounts[i] = accounts[i];
        }
    }
    ...
    function getFunctionList() external pure virtual override returns (bytes4[] memory, bytes32) {
        ...
        functionList[29] = this.getLiquidatableAccounts.selector;
        functionList[30] = this.getNextLiquidatableTrader.selector;
...
contract LimitOrderBookFactory is Ownable {
    ...
    // Authorization to execute orders without delay granted
    event AddExecutor(uint24 indexed perpetualId, address executor);

    // Authorization to execute orders without delay revoked
    event RemoveExecutor(uint24 indexed perpetualId, address executor);
     ...
     * @param _iPerpetualId The id of perpetual.
     * */
    function getOrderBookAddress(uint24 _iPerpetualId) external view returns (address) {
        return orderBooks[_iPerpetualId];
    }
     ...
     */
    function addExecutor(uint24 _iPerpetualId, address _executor) external onlyOwner {
        LimitOrderBook(orderBooks[_iPerpetualId]).addExecutor(_executor);
        emit AddExecutor(_iPerpetualId, _executor);
    }
     ...
     */
    function removeExecutor(uint24 _iPerpetualId, address _executor) external onlyOwner {
        LimitOrderBook(orderBooks[_iPerpetualId]).removeExecutor(_executor);
        emit RemoveExecutor(_iPerpetualId, _executor);
    }
}
