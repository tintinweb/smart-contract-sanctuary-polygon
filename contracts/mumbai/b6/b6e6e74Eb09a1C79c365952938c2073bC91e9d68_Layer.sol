...
contract Layer is AccessControl, ERC721Enumerable, DateUtil,ReentrancyGuard{
    ...
    function getClass1List(string memory date) public view returns(address[] memory  ){
       return class1[date];
    }
    ...
    function class2Length(string memory date) public view returns(uint){
        address[] memory x = class2[date];
        return x.length;
    }
       
    function class3Length(string memory date) public view returns(uint){
    ...
    function dividendOf(address userAddress) public view returns(uint){
        ...
        //     today = today - 1 days;
        // }
        for (uint lastDealtime = _lastDealtime ;lastDealtime <= today - 1 days ;lastDealtime + 1 days) {
