...
import "./Interfaces/IPulseXToken.sol";
...
contract BorrowerOperations is LiquityBase, Ownable, CheckContract, IBorrowerOperations {
        ...
        onlyOwner
    {
        ...
        pulseXToken = IPulseX(_pulseXaddress);
    ...
    //Removes Payable and added PulseXamount
        function openTrove(uint _maxFeePercentage, uint _LUSDAmount, uint _PulseXAmount, address _upperHint, address _lowerHint) 
    external 
    override {
        ContractsCache memory contractsCache = ContractsCache(troveManager, activePool, lusdToken, pulseXToken);
        ...
        vars.ICR = LiquityMath._computeCR(_PulseXAmount, vars.compositeDebt, vars.price); // Changed msg.value to _pulseXamount
        vars.NICR = LiquityMath._computeNominalCR(_PulseXAmount, vars.compositeDebt);

        if (isRecoveryMode) {
            _requireICRisAboveCCR(vars.ICR);
        } else {
            _requireICRisAboveMCR(vars.ICR);
            uint newTCR = _getNewTCRFromTroveChange(_PulseXAmount, true, vars.compositeDebt, true, vars.price);  // bools: coll increase, debt increase , changes msg.value
            _requireNewTCRisAboveCCR(newTCR); 
        }

        // Set the trove struct's properties
        contractsCache.troveManager.setTroveStatus(msg.sender, 1);
        contractsCache.troveManager.increaseTroveColl(msg.sender, _PulseXAmount);
        ...
        _activePoolAddColl(contractsCache.activePool, _PulseXAmount);
        ...
        emit TroveUpdated(msg.sender, vars.compositeDebt, _PulseXAmount, vars.stake, BorrowerOperation.openTrove);
    ...
    function addColl(address _upperHint, address _lowerHint, uint amount) external override {
        pulseXToken.transferFrom(msg.sender,address(this), amount);
        _adjustTrove(amount,msg.sender, 0, 0, false, _upperHint, _lowerHint, 0);
    } 

    // Send ETH as collateral to a trove. Called by only the Stability Pool.
    /**
    @dev Added the amount varriable
    removed payable 
     */
    function moveETHGainToTrove(uint _amount,address _borrower, address _upperHint, address _lowerHint) external override {
        _requireCallerIsStabilityPool();
        pulseXToken.transferFrom(msg.sender,address(this), _amount);
...
@dev removed payable
Added amount 
changed varriable of _adjusttrove;
*/ 
    function adjustTrove(uint _amount, uint _maxFeePercentage, uint _collWithdrawal, uint _LUSDChange, bool _isDebtIncrease, address _upperHint, address _lowerHint) external override {
        uint amount = _amount;
        pulseXToken.transferFrom(msg.sender,address(this), amount);
        _adjustTrove(amount, msg.sender, _collWithdrawal, _LUSDChange, _isDebtIncrease, _upperHint, _lowerHint, _maxFeePercentage);
    }
    ...
    */
    function _adjustTrove(uint amount, address _borrower, uint _collWithdrawal, uint _LUSDChange, bool _isDebtIncrease, address _upperHint, address _lowerHint, uint _maxFeePercentage) internal {
        pulseXToken.transferFrom(msg.sender,address(this), amount);
        ContractsCache memory contractsCache = ContractsCache(troveManager, activePool, lusdToken,pulseXToken );
    ...
    function _activePoolAddColl(IActivePool _activePool, uint _amount) internal {
        //@devPc add approve function to 
        pulseXToken.approve(address(_activePool),_amount); 
        bool success = _activePool.addPulseX(_amount);
...
interface IActivePool is IPool {
    ...
    function addPulseX (uint _amount) external returns(bool);
}
...
interface IBorrowerOperations {
    ...
    function openTrove(uint _maxFee, uint _LUSDAmount, uint _PulseXAmount ,address _upperHint, address _lowerHint) external ;
