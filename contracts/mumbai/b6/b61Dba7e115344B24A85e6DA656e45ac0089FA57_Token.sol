...
contract Token is ERC20 {
    ...
    */
    ...
    function transferFrom(address _from, address _to, uint256 _value) public whenNotPaused returns (bool) {
        ...
        releaseLock(_from);
        require(_value <= super.balanceOf(_from), "Not enough balance");
    ...
    function detailBalance(address _holder) public view returns (uint256 totalBalance, uint256 lockedBalance, uint256 availableBalance) {
        uint256 lockedValue;
        uint256 length = lockInfo[_holder].length;
        for(uint256 i = 0; i < length; i++ ) {
            LockInfo memory acc = lockInfo[_holder][i];
        ...
        return (balanceOf(_holder), lockedValue, balanceOf(_holder).sub(lockedValue));
    }
