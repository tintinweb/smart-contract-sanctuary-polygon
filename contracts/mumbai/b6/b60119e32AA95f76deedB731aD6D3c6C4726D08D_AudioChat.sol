...
function getAudioChatsByAddress(address creator) public returns(CreateAudioChat[] memory audio_chats){
        ...
        if (creator == msg.sender){
            ...
            for (uint256 i; length_of_array > i; i++ ){
                ...
                if (id_to_audio_chat[address_to_audio_chat[creator][i]].is_indexed == true){
                    our_audio_chats[counter] = id_to_audio_chat[address_to_audio_chat[creator][i]];
                    counter++;
                }
            }
            return our_audio_chats;
