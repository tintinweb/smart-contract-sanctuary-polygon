...
/*
  ...
  　　　　SPDX-License-Identifier: MIT
      ...
      　　　　shimadamasafumi ArtWork On-chain Datebase v1 {
            ...
            SEQUENCE {
            [ InventoryNumber, Title_en, Title_ja, Material, Edition, Term&Year, Size,ImageURL, Details, Exhibition, Status, ArtistMemo ]
            }
...
contract shimadamasafumi_Artwork_Onchain_Datebase_v1 {
    ...
    event StatusAdded(uint256 indexed workId);
    event DetailAdded(uint256 indexed workId);
    event ExhibitionAdded(uint256 indexed workId);
    event MemoAdded(uint256 indexed workId);
    ...
    if (bytes(_newMemo).length > 0) {
        ...
        emit MemoAdded(workId);
    }

    // Update latest update title
    latestUpdateTitle = _inventoryNumber;
    }
