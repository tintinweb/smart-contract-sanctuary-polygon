...
contract RafflePanel {
    ...
    event TokenAdded(address indexed tokenAddress, uint256 decimals);
    ...
    function addSupportedToken(address tokenAddress) external onlyAdmin {
        ...
        require(supportedTokens[tokenAddress] == 0, "Token already supported.");

        supportedTokens[tokenAddress] = 18; // Replace with actual implementation to fetch decimals from the token contract

        emit TokenAdded(tokenAddress, 18);
    }
    ...
    function purchaseTicketsWithTokens(uint256 raffleId, uint256 numTickets, address tokenAddress) external {
        ...
        require(numTickets > 0, "Invalid number of tickets to purchase.");
        require(raffle.ticketsSold.add(numTickets) <= raffle.maxTickets, "Exceeded maximum tickets.");
        require(supportedTokens[tokenAddress] > 0, "Token not supported.");

        uint256 decimals = supportedTokens[tokenAddress];
        uint256 totalPrice = raffle.ticketPrice.mul(numTickets).mul(10**decimals);
        require(totalPrice > 0, "Invalid total price.");

        IERC20 tokenContract = IERC20(tokenAddress);
        require(tokenContract.allowance(msg.sender, address(this)) >= totalPrice, "Insufficient token allowance.");

        bool success = tokenContract.transferFrom(msg.sender, address(this), totalPrice);
    ...
    function withdrawServiceFee() external onlyAdmin {
        ...
        require(serviceFee > 0, "No service fee available for withdrawal.");
