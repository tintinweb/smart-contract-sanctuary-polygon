...
contract SkyToken is ERC20, FxBaseChildTunnel, Ownable {
    ...
    {
        rarityRate[0] = 70 ether;
        rarityRate[1] = 100 ether;
        rarityRate[2] = 120 ether;
        rarityRate[3] = 150 ether;
    }
    ...
    function pendingRewardsByNFT(address user, uint256 tokenId) external view returns (uint256) {
        return _getPendingRewardsByNFT(user, tokenId);
    }
    ...
    function getStakeRecord(address user, uint256 index) external view returns (StakeRecord memory){
        StakeRecord memory _record = stakeRecord[user][index];
        uint256 deltaDifference = block.timestamp - _record.stakedOn;
    ...
    function _processMessageFromRoot(
        ...
        if(stake){
            ...
            _userInfo.stakedBalance--;
            _userInfo.stakedIds[tokenIndex[tokenId]] = _userInfo.stakedIds[_userInfo.stakedIds.length - 1];
    ...
    function _pendingByNFTAndMarkClaim(address user, uint256 tokenId) internal returns (uint256){
        ...
        StakeRecord storage _record = stakeRecord[user][tokenId];
        uint256 deltaStakedDifference = block.timestamp - _record.stakedOn;
        uint256 deltaClaimedDifference = _record.lastClaimed > 0 ? block.timestamp - _record.lastClaimed : deltaStakedDifference;
        
        uint256 bonusPercent = _bonusThreshold(deltaStakedDifference);
        uint256 rarityBasedRate = rarityRate[tokenRarity[_record.tokenId]];

        if(bonusPercent > 0) rewardsBonus = (rarityBasedRate / 100 * bonusPercent);
