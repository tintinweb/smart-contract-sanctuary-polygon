...
contract ThreeRoulettes is Ownable{
    ...
    // function setWinnerAmount(uint8 _wheel, uint256 _rouletteOption) internal 
    // {
    //     if(_wheel == 1)
    //     {
    //          uint256 payoutAmount = msg.value*(Wheel1[_rouletteOption])/(100);
    //         balances[msg.sender] += payoutAmount;
    //         bets[1].profit = payoutAmount;
    //         setProfit(payoutAmount,1);
    //     //event BetResolved(address indexed player, uint256 indexed betId,uint8 wheel, uint256 amount,uint result,uint256 playersChoice,uint256 winnersChoice);   
    //     emit BetResolved(msg.sender, betId,1,payoutAmount,1,_rouletteOption,_rouletteOption);
    //     }
    //     else if(_wheel == 2)

    //     {
    //         uint256 payoutAmount = msg.value*(Wheel2[_rouletteOption])/(100);
    //         balances[msg.sender] += payoutAmount;
    //         bets[1].profit = payoutAmount;
    //         setProfit(payoutAmount,1);
    //     //event BetResolved(address indexed player, uint256 indexed betId,uint8 wheel, uint256 amount,uint result,uint256 playersChoice,uint256 winnersChoice);   
    //     emit BetResolved(msg.sender, betId,1,payoutAmount,2,_rouletteOption,_rouletteOption);
    //     }   
    //     else if(_wheel == 3)
    //     {
    //          uint256 payoutAmount = msg.value*(Wheel3[_rouletteOption])/(100);
    //         balances[msg.sender] += payoutAmount;
    //         bets[1].profit = payoutAmount;
    //         setProfit(payoutAmount,1);
    //     //event BetResolved(address indexed player, uint256 indexed betId,uint8 wheel, uint256 amount,uint result,uint256 playersChoice,uint256 winnersChoice);   
    //     emit BetResolved(msg.sender, betId,1,payoutAmount,3,_rouletteOption,_rouletteOption);
    //     }     
    // }
