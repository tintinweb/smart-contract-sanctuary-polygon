...
contract IndexAdmin is AccessControl, Initializable, IIndex {
    using EnumerableSet for EnumerableSet.AddressSet;
    using SafeERC20 for IERC20;

    EnumerableSet.AddressSet _newAssets;
    ...
    function rebalance(Asset[] memory newAssets) external onlyRole(ADMIN_ROLE) {
        //TODO: обменять все на доллары
        _exchangeToUSD(); // добавть долю
        _rebalance(newAssets);

        uint256 indexPrice = _calcCost(1e18);

        emit Rebalance(newAssets, indexPrice);
    }

    function _exchangeToUSD() internal {
        uint256 len = _activeAssets.length;
        for (uint256 i; i < len; i++) {
            address[] memory reversPath = _reversArray(_activeAssets[i].path);
            _activeAssets[i].totalAmount += _exchangeDEX(
                reversPath,
        ...
        returns (address[] memory reversArray)
    {
        ...
        for (uint256 i; i <= len; i++) {
            reversArray[len - i] = array[i];
        }
    }

    function addAsset(address asset) external onlyRole(DAO_ADMIN_ROLE) {
        _newAssets.add(asset);
        emit AddAsset(asset);
    }

    function removeAsset(address asset) external onlyRole(DAO_ADMIN_ROLE) {
        _newAssets.remove(asset);
        emit RemoveAsset(asset);
    }

    function stake(uint256 amountLP) external isZeroAmount(amountLP) {
        uint256 amountUSD = _calcCost(amountLP);

        IERC20(_USD).safeTransferFrom(msg.sender, address(this), amountUSD);
        ...
        IndexLP(_indexLP).mint(address(this), tax);

        emit Stake(msg.sender, amountUSD, amountLP);
    }
    ...
    function getAddressLP() external view returns (address) {
        return _indexLP;
    }

    function getAddressUSD() external view returns (address) {
        return _USD;
    }
    ...
    function _approveDEX(address asset) public {
        IERC20(asset).safeApprove(
            _adapter,
            11111111 /* type(uint256).max */
        );
    }

    function _approveDEX2(address asset, address adapter) public {
        IERC20(asset).safeApprove(adapter, type(uint256).max);
    }
    ...
    function _rebalance(Asset[] memory assets) internal {
        delete (_activeAssets);
        uint256 len = assets.length;
        for (uint256 i; i < len; i++) {
            _activeAssets.push(assets[i]);
            _approveDEX(assets[i].asset);
        }
    }
    ...
    function _deployIndexLP() internal returns (address) {
        return address(new IndexLP());
    }
}
...
interface IIndex {
    ...
    event Rebalance(Asset[] assets, uint256 indexPrice);
