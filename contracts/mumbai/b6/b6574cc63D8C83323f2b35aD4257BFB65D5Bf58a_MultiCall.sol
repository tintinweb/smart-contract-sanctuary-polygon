...
contract MultiCall is KeyEnumBase {
    ...
    function search(address asker, address registrar, string memory targetName)
        external
        view
        returns (
            bytes32 askerNode,
        ...
        )
    {
        ...
        (price, likes, reserver, owner, targetNode) = searchTarget(registrar, targetName);
        if (owner != address(0)) {
            uint64 expire;
        ...
        )
    {
        ...
        if (length > 0) {
            ...
            for (uint256 i = 0; i < length; i++) {
                (likees[i], timestamps[i]) = abi.decode(itemArray[i], (bytes32, uint256));
                fullNames[i] = IResolver(RESOLVER).fullName(likees[i]);
            }
        }
    }
}