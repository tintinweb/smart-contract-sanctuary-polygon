...
contract BackstageOnlineTicket is ERC721, ERC721Enumerable, Ownable {
    ...
    address payable[] internal _payees;
    ...
    function mint(address to) public payable {
        require(msg.value >= minPrice, "Insufficient funds to mint");
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        _safeMint(to, tokenId);

        //Split payment to shareholders
        for (uint256 i = 0; i < _payees.length; i++) {
            _payees[i].transfer(
                (msg.value * _shares[_payees[i]]) / _totalShares
            );
        }
    }
    ...
    function _addPayee(address payable account, uint256 shares_) internal {
        require(
            account != address(0),
