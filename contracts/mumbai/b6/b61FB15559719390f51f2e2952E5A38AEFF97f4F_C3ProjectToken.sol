    ...
    OwnableUpgradeable
{
    ...
    /**
    **  With this method you can offset credits.
    **/
    function offset(uint256 amount, string memory transferee, string memory reason) public virtual whenNotPaused {
        burn(_msgSender(),_msgSender(), amount, transferee, reason);
    }

    /**
    **  With this method you can offset credits and transfer to another benificiary.
    **/
    function offsetFor(uint256 amount, address beneficiary, string memory transferee, string memory reason) public virtual whenNotPaused {
        burn(_msgSender(),beneficiary, amount, transferee, reason);
    }

    function burn(address account,address beneficiary, uint256 amount, string memory _transferee, string memory _reason) internal virtual {
        _burn(account, amount);
        // call a function to create a offset nft
        address nftOffsetAddress = IContractAddresses(orchestratorAddress).nftOffset();
        IContractNFTOffset(nftOffsetAddress).mintOffset(beneficiary, projectKey, _transferee, _reason, vintage, amount);
    }
