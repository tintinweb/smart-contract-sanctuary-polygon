...
contract TestContract is BaseContract {
    ...
    function addWeekAddress(address[] memory _addressList) external onlyOwner {
        ...
        weekDayList["Invest"] = new uint256[](7);
        weekDayList["Invite"] = new uint256[](7);
        for (uint256 j = 0; j < 7; j++) {
            uint256 k = 7 - j;
            uint256 _day = getYearMonthDay(_timestamp - k * 3600 * 24, adjustHour);
            weekDayList["Invest"][j] = _day;
            weekDayList["Invite"][j] = _day;
        }
    }
    ...
    function addRankingPerDayForInvite20(address[] memory _addressList) external onlyOwner {
        ...
        for (uint256 i = 0; i < _num; i++) {
            if (dayRewardAddressList["Invite"][_day].length() < 10) {
                dayRewardAddressList["Invite"][_day].add(_addressList[i]);
                dayRewardUserDataList["Invite"][_day][_addressList[i]] = (i + 1) * 10 ** 20;
            }
        }
    }
    ...
    function addRankingPerDayForInvest10(address[] memory _addressList) external onlyOwner {
        ...
        uint256 _day = getYearMonthDay(block.timestamp, adjustHour);
        for (uint256 i = 0; i < _num; i++) {
            if (dayRewardAddressList["Invest"][_day].length() < 10) {
                dayRewardAddressList["Invest"][_day].add(_addressList[i]);
                dayRewardUserDataList["Invest"][_day][_addressList[i]] = (i + 1) * 10 ** 20;
            }
        }
    }
}
