    ...
    DefaultOperatorFiltererUpgradeable
{
    ...
    function joinStrings(
        ...
        uint256 stringsCount = stringsArray.length;

        // Calculate the total length of all strings in the array
        for (uint256 i = 0; i < stringsCount; i++) {
            totalLength += bytes(stringsArray[i]).length;
        }

        // Allocate a new string with the total length
        bytes memory resultBytes = new bytes(totalLength);
        uint256 resultIndex = 0;

        // Copy each string into the new string
        for (uint256 i = 0; i < stringsCount; i++) {
            bytes memory stringBytes = bytes(stringsArray[i]);
            uint256 stringLength = stringBytes.length;

            for (uint256 j = 0; j < stringLength; j++) {
                resultBytes[resultIndex] = stringBytes[j];
                resultIndex++;
            }
        }
    ...
    function _encodeRequest(
        ...
        for (uint i = 0; i < _req.uris.length; i++) {
            stringHashes[i] = keccak256(bytes(_req.uris[i]));
        }
        return
            abi.encode(
                TYPEHASH,
