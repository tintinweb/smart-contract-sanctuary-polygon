...
import '../../gameplay/coordinators/LootCoordinator/ILootCoordinator.sol';
import '../../gameplay/models/Inventory.sol';
import './LootItem.sol';

contract MysteryItem is LootItem, InventoryTypesUser {
  uint256 fake_token_count = 0;
  ILootCoordinator lootCoordinator;

  constructor() ERC721A('MysteryItem', 'MYSTERYITEM') {}

  function setLootCoordinator(address _lootCoordinator)
    external
    onlyController
  {
    lootCoordinator = ILootCoordinator(_lootCoordinator);
  }

  function reveal(uint256 tokenId, Entity memory entity) external payable {
    require(
      _msgSender() == ownerOf(tokenId),
      'MysteryItem: must own token to reveal'
    );
    burn(tokenId);
    lootCoordinator.reserve(entity);
  }
}
...
import './ILootItem.sol';

abstract contract LootItem is
  ERC721AQueryable,
  ERC721ABurnable,
  IInventoryItemContract,
  Controllable,
  ILootItem
{
    ...
    returns (uint256[] memory)
  {
    ...
    _safeMint(to, amount);
    uint256 total = totalSupply() - start;
  ...
  function setMaxItems(uint256 max) external payable onlyOwner {
    max_items = max;
  }

  function irlBalance(uint256, address)
    external
    view
    override
    returns (uint256)
