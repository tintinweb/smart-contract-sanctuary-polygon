...
contract Voting is GSNRecipient {
    ...
    uint[] public responses = new uint[](180);
    ...
    function createSurvey(uint id, string memory question, bool isActive) external onlyOwner {
        require(surveys[id].id == 0, "Voting: Survey exists!");
        
        surveys[id].id = id;
        surveys[id].isActive = isActive;
        surveys[id].question = question;
        surveys[id].responses = new uint[](180);
    }
    ...
    function castVote(uint surveyId, bool answer) external onlyWhitelisted {
        ...
        surveys[surveyId].responses[index]++;
    }
