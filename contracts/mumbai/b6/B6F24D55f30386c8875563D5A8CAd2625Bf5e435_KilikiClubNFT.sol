...
interface NinfyDropsInterface {
    ...
    function toggleFreeMintActive() external;
    ...
    function reserve(uint256 _amount , address _wallet , string memory _memo) external;
    ...
    function ownerpresaleBuy(uint256 _tokenId, uint256 _amount) external payable;
    ...
    function freeMintActive() view external returns(bool);
    ...
    function usedTokens(uint256 _tokenId) view external returns(bool);



    // EVENTS

    event Reserve(uint256 _amount, address _wallet , string _memo);
    ...
    event ToggleFreeMintState(bool _freeMintState);

    event OwnerPresaleBuy(address _user, uint256 _ticketId, uint256 _amount);

    event WhitelistBuy(address _user, uint256 _amount);

    event WhitelistFreeMint(address _user, uint256 _amount);
...
contract KilikiClubNFT is NinfyDropsInterface, ERC721, Ownable {
    ...
    bool public override freeMintActive;
    ...
    mapping(uint256 => bool) public override usedTokens;
    ...
    function toggleFreeMintActive() external onlyOwner override {

        freeMintActive = !freeMintActive;

        emit ToggleFreeMintState(freeMintActive);
   ...
   function setDevWallet(address payable _devWallet) external onlyOwner override {
    ...
    function reserve(uint256 _amount , address _wallet , string calldata _memo) external onlyOwner override {

        _reservemint(_amount , _wallet);

        emit Reserve(_amount, _wallet , _memo);
 ...
 function _reservemint(uint256 _amount , address _wallet) internal {
        ...
        for(uint i = 0; i < _amount; i++) {
            ...
            _safeMint(_wallet, newId);

            emit Mint(_wallet, newId, tokenURI(newId));
...
super._beforeTokenTransfer(from, to, tokenId);  
    ...
    function ownerpresaleBuy(uint256 _tokenId, uint _amount) external payable override {

        require(presaleActive, "NiftyDropPlatform::Owners presaleBuy: Owners Presale is not Active.");

        require (ownerOf(_tokenId) == _msgSender(), "NiftyDropPlatform::Owners PresaleBuy: you are not owning any kiliki NFT.");

        require(_amount <= mintLimit, "NiftyDropPlatform::Owners presaleBuy: Too many tokens for one transaction.");

        require(msg.value >= mintPrice.mul(_amount), "NiftyDropPlatform::Owners presaleBuy: Insufficient payment.");
        ...
        emit OwnerPresaleBuy(_msgSender(), _tokenId, _amount);
    ...
    function freeMint(uint256 _amount) external payable override {

        require(freeMintActive, "NiftyDropPlatformSale::FreeMint: Free Minting is not Active.");

        require(_amount <= 1, "NiftyDropPlatform::FreeMint: Max 1 token.");

        require(whitelist[_msgSender()], "NiftyDropPlatform::whitelistFreeMint: You are not whitelisted."); 

        

        emit WhitelistFreeMint(_msgSender(), _amount);
