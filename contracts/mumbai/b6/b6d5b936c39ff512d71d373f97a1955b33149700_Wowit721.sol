...
/**
 * @dev Implementation of https://eips.BEPereum.org/EIPS/eip-721[BEP721] Non-Fungible Token Standard, including
 * the Metadata extension, but not including the Enumerable extension, which is available separately as
 * {BEP721Enumerable}.
 */
contract BEP721Upgradeable is
    Initializable,
    ContextUpgradeable,
    BEP165Upgradeable,
    IBEP721Upgradeable,
    IBEP721MetadataUpgradeable
{
     ...
     */
    function __BEP721_init(string memory name_, string memory symbol_)
        internal
        initializer
    {
        __Context_init_unchained();
    ...
    function __BEP721_init_unchained(string memory name_, string memory symbol_)
        internal
        initializer
    {
        _name = name_;
    ...
    function _transfer(
        ...
        require(
            BEP721Upgradeable.ownerOf(tokenId) == from,
    ...
    function _checkOnBEP721Received(
        ...
        if (to.isContract()) {
            try
                IBEP721ReceiverUpgradeable(to).onBEP721Received(
                    _msgSender(),
                    from,
                    tokenId,
                    _data
            ...
            returns (bytes4 retval) {
                return
                    retval ==
                    IBEP721ReceiverUpgradeable.onBEP721Received.selector;
 ...
 * account.
 */
abstract contract BEP721EnumerableUpgradeable is
    Initializable,
    BEP721Upgradeable,
    IBEP721EnumerableUpgradeable
{
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(IBEP165Upgradeable, BEP721Upgradeable)
        returns (bool)
    {
        return
            interfaceId == type(IBEP721EnumerableUpgradeable).interfaceId ||
            super.supportsInterface(interfaceId);
    }
    ...
    {
        require(
            index < BEP721Upgradeable.balanceOf(owner),
    ...
    {
        require(
            index < BEP721EnumerableUpgradeable.totalSupply(),
 ...
 */
abstract contract BEP721URIStorageUpgradeable is
    Initializable,
    BEP721Upgradeable
{
 ...
 */
abstract contract BEP721BurnableUpgradeable is
    Initializable,
    ContextUpgradeable,
    BEP721Upgradeable,
    OwnableUpgradeable
    ...
    BEP721BurnableUpgradeable
{
    ...
    function initialize() public initializer {
        __Ownable_init();
        BEP721Upgradeable.__BEP721_init("WOW", "IT");
        _tid = 1;
    ...
    function getRentalOwner(address tokenOwner, uint256 tokenId)
        public
        view
        returns (address)
    {
        if (
            order_place[tokenOwner][tokenId].isRent == true &&
            order_place[tokenOwner][tokenId].endTime >= block.timestamp
        ) {
            return order_place[tokenOwner][tokenId].rentOwner;
        } else {
            return tokenOwner;
        }
    }
    ...
    function mint(
        ...
        if (value != 0) {
            _orderPlace(msg.sender, id_, value, timePeriod, isRent);
        }
    ...
    function orderPlace(
        ...
        _orderPlace(msg.sender, tokenId, _price, timePeriod, isRent);
    }
    ...
    function acceptRent(
        ...
        require(msg.value >= amount, "msg.value Not equal to current amount");
        require(
        ...
        require(
            order_place[from][tokenId].isRent == false ||
                order_place[from][tokenId].endTime <= block.timestamp,
            "NFT is in Rental"
        );

        order_place[from][tokenId].rentOwner = msg.sender;
        order_place[from][tokenId].endTime = endTime;
        ...
        require(
            ...
            "msg.value Not equal to Calculation amount"
        );
    ...
    function _saleToken(
        ...
        require(order_place[from][tokenId].isRent == false, "NFT is in Rental");
