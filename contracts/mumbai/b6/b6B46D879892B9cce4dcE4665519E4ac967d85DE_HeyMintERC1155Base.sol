...
contract HeyMintERC1155Base is HeyMintERC1155Upgradeable, IERC2981Upgradeable {
    ...
    returns (bool)
    {
        return HeyMintERC1155Upgradeable.supportsInterface(interfaceId);
    }

    /**
    * @notice Lock a token id so that it can never be minted again
     */
    function permanentlyDisableTokenMinting(uint16 _tokenId) external onlyOwner {
        HeyMintStorage.state().data.tokenMintingPermanentlyDisabled[_tokenId] = true;
    }
...
* @notice Override ERC1155 such that zero amount token transfers are disallowed.
    ...
    function safeTransferFrom(
        ...
        uint256 amount,
        bytes memory data
    ) public override(ERC1155UDS) {
        HeyMintStorage.State storage state = HeyMintStorage.state();
        require(amount > 0, "AMOUNT_CANNOT_BE_ZERO");
        require(!state.tokens[uint16(id)].soulbindingActive, "TOKEN_SOULBOUND");
...
* @notice Override ERC1155 such that zero amount token transfers are disallowed.
    ...
    function safeBatchTransferFrom(
        ...
        for (uint256 i; i < ids.length; i++) {
            ...
            require(!state.tokens[uint16(ids[i])].soulbindingActive, "TOKEN_SOULBOUND");
        }
