    ...
    Pausable,Initializable,UUPSUpgradeable
{
    ...
    uint256 public paymentReceiverFee; //marketplace fee
    ...
    function initialize(address _owner,address _paymentReceiver,uint256 _paymentReceiverFee) external initializer {
    ...
    function cancel(
        ...
        require(orderSigner == order.user, 'Input signature error');
     ...
     * @param orderHash - order struct hash
     */
    function englishAuctionERC1155Bid(Order memory order, uint tokenAmount, uint _price, bytes32 orderHash) external payable {
        //check for seller's signature
        _verifyOrderSignature(order);
        //check for conditions
        require(order.status == Status.AUCTION && orderStatus[orderHash] == Status.NEW,'Auction:order already exist');
     ...
     * * @param input - input struct
     */
    function DutchAuctionBuy(Input memory input) public payable{

        //get order
        Order memory order = input.orders[0];

        //check for order party signatures
        _verifyOrderSignature(order);
    ...
    function withdrawPayment() external returns (bool) {
        uint256 amount = wallet[msg.sender];
        if (amount > 0) {
        wallet[msg.sender] = 0;
    ...
    /**
     * get a biding struct for 721 token
     * @param _tokenAddress - tokena address
     * @param _tokenId - tokenid
     * @param _owner - token owner before auction complete
     */
    function getEnglishBid721(address _tokenAddress, uint _tokenId, address _owner) external view returns(BidListing721 memory){
        return _bidlisting721s[_tokenAddress][_tokenId][_owner];
    }
     ...
     * @param _owner - token owner before auction complete
     */
    function getEnglishBid1155(address _tokenAddress, uint _tokenId, address _owner) external view returns(BidListing1155 memory){
        return _bidlisting1155s[_tokenAddress][_tokenId][_owner];
    }
    //internal
     /**
    ...
    function _distributeFeeAndProfit(
        ...
        require(price >= 0,'Total amount of fees are more than the price');
