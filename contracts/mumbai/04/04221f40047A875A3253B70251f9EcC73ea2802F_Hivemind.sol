 ...
 * @author mario
 */
contract Hivemind is HivemindV1Storage, HivemindI, ErrorReporter, AccessControlEnumerable {
  ...
  function _setTransferPaused(HTokenI _hToken, bool _state) public returns (bool) {
    if (!hasRole(DEFAULT_ADMIN_ROLE, msg.sender) && !hasRole(PAUSABLE, msg.sender)) revert Unauthorized();
  ...
  function _setRedeemPaused(HTokenI _hToken, bool _state) public returns (bool) {
    if (!hasRole(DEFAULT_ADMIN_ROLE, msg.sender) && !hasRole(PAUSABLE, msg.sender)) revert Unauthorized();
