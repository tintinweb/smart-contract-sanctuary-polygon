...
contract Flex2Earn is OperatorAccessControlUpgradeable {
    ...
    function awardFlexer(uint256 _flexedNfcId) external onlyClerk {
        ...
        _awardFlexer(eventId, msg.sender, flexer);
    }

    function _awardFlexer(uint256 eventId, address clerk, address flexer) private {
        address partner = partnerRegistry.partnerOfClerk(clerk);
        uint256 balance = partnerBalances[partner];
        uint256 reward = partnerRegistry.getRewardPerFlexer(eventId);

        require(!partnerRegistry.partnerLocked(partner), "Flex2Earn::_awardFlexer: partner is locked");
        require(!partnerRegistry.isEventCancelled(eventId), "Flex2Earn::_awardFlexer: event is cancelled");

        require(
            balance >= reward,
            "Flex2Earn::_awardFlexer: deposited reward balance insufficient"
        );
        ...
        require(
            ...
            "Flex2Earn::_awardFlexer: rewards exceeded token pool"
        );
        ...
        require(
            ...
            "Flex2Earn::_awardFlexer: cannot award flexer berfore startTime or after endTime"
        );
    ...
    function awardWhitelistedFlexer(bytes32[] calldata _merkleProof, address flexer) external onlyClerk {
        uint256 eventId = partnerRegistry.getClerkEventId(msg.sender);
        bytes32 node = keccak256(abi.encodePacked(flexer));
        require(
            MerkleProofUpgradeable.verify(
                _merkleProof,
                partnerRegistry.merkleRootByEventId(eventId),
                node
            ),
            "Flex2Earn::awardWhitelistedFlexer: Invalid merkle proof for flexer address"
        );
        _awardFlexer(eventId, msg.sender, flexer);
    }
