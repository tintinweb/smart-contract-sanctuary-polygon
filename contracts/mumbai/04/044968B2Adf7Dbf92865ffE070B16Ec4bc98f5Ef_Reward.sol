...
contract Reward is Ownable {
    ...
    uint256 MAX_AMOUNT_NATIVE_TOKEN = 0.001 * 5 ether;

    event RewardEvent(address indexed, uint256 amountSVC, uint256 amountNativeToken);
    ...
    function reward(
        ...
        // payable(recipient).transfer(amountNativeToken);
        require(amountSVC <= MAX_AMOUNT_SVC, "Reward Quiz: amountSVC too much");
        require(amountNativeToken <= MAX_AMOUNT_NATIVE_TOKEN, "Reward Quiz: amountNativeToken too much");
        TransferHelper.safeTransfer(rewardToken, recipient, amountSVC);
        TransferHelper.safeTransferETH(recipient, amountNativeToken);
        RewardEvent(recipient, amountSVC, amountNativeToken);
        return true;
    }
