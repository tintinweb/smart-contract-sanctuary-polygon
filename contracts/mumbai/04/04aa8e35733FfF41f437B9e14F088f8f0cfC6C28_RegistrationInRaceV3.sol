...
contract RegistrationInRaceV3 is
    Initializable,
    ...
    OwnableUpgradeable
{
    ...
    mapping(string => RaceParticipant[]) public raceParticipant;
    ...
    modifier checkPayState(uint256 amount, address _msgSender) {
    ...
    ) external view returns (RaceParticipant[] memory) {
        return raceParticipant[raceId];
    }
    ...
    function register(
        ...
        external
        checkPayState(amount, _msgSender())
        whenNotPaused
        returns (bool)
    {
        update(raceId, amount, horseId, _msgSender());
        ...
        bool success = TokenInterface.transferFrom(
            _msgSender(),
            operatorAddress,
            amount
        );
    ...
    function update(
        ...
        raceParticipant[raceId].push(
            RaceParticipant(_user, horseId, amount, block.timestamp)
        );
    }

    function setOperatorAddress(address _operatorAddres) external onlyOwner {
        operatorAddress = _operatorAddres;
    }
