...
contract EstiPriceCalculation {
    /** Constants */
    uint256 constant EXPONENT = 21;
    uint256 constant TIME_EXPONENT = 50;

    using PRBMathUD60x18Typed for PRBMath.UD60x18;

    /**
     * @dev calculates the risk value depending on the distance of the strike to the current price of the underlyings and using the given maxDelta
     * @param tradingType - the value can be 1 or 0: 1 means bearish strike and 0 means bullish trade
     */
    function calculateRiskValue(
        ...
        uint256 _delta;
        bool geq; // strike price greater equal the price
        if (strikePrice > actualPrice) {
            _delta = strikePrice - actualPrice;
            if (tradingType == 0) {
                geq = true;
            }
        } else {
            _delta = actualPrice - strikePrice;
            if (tradingType == 1) {
                geq = true;
            }
        }

        // (delta / maxDelta) = _a / _b
        PRBMath.UD60x18 memory _a = PRBMath.UD60x18({value: _delta});
        PRBMath.UD60x18 memory _b = PRBMath.UD60x18({value: maxDelta});

        uint256 f = _a.div(_b).value * 10;
        ...
        PRBMath.UD60x18 memory _r = PRBMath.UD60x18({value: (x * 10**18)}).pow(
            PRBMath.UD60x18({value: (EXPONENT * 10**18)})
        );

        // uint256 _riskValue = _r.pow(_e).value;

        uint256 _riskValue = _r.toUint() / 10**24;

        return _riskValue;
    }

    /** @notice - calculate the time passed between 0 and 100
     *  0 <= _b / _a <= 100
     * - start: 0
     * - expired: 100
     */
    function calculateTimeValue(
        ...
        uint256 _timePassed = timeStamp - startTimeStamp;

        PRBMath.UD60x18 memory _a = PRBMath.UD60x18({value: duration});
        PRBMath.UD60x18 memory _b = PRBMath.UD60x18({value: _timePassed});

        uint256 timePassed = _b.div(_a).value;

        uint256 _x = timePassed + 1 * 10**18;

        PRBMath.UD60x18 memory _c = PRBMath.UD60x18({value: _x});
        PRBMath.UD60x18 memory _e = PRBMath.UD60x18({
            value: TIME_EXPONENT * 10**17
        });

        PRBMath.UD60x18 memory divisor = _c.pow(_e);
        PRBMath.UD60x18 memory _one = PRBMath.UD60x18({value: 1 * 10**18});

        uint256 timeValue = _one.div(divisor).value;

        return timeValue;
    }

    /** @dev return the the price in uint256 with 18 decimals */
    function calulatePrice(uint256 riskValue, uint256 timeValue)
        external
        pure
        returns (uint256)
    {
        PRBMath.UD60x18 memory _riskValueUD = PRBMath.UD60x18({
            value: riskValue
        });
        PRBMath.UD60x18 memory _timeValueUD = PRBMath.UD60x18({
            value: timeValue
        });

        //multiply the riskValue and timeValue
        //the result is the amount of EstiTokens u gett for one ETH
        PRBMath.UD60x18 memory _tokenPriceUD = _riskValueUD.mul(_timeValueUD);
        PRBMath.UD60x18 memory _oneUd = PRBMath.UD60x18({value: 1 * 10**18});

        _tokenPriceUD = _oneUd.div(_tokenPriceUD);
        return _tokenPriceUD.value;
    }

    /**
     * @dev - calculate the total purchase price for an amount of tokens
     * @param tokenPrice - the cost of 1**18 tokens
     * @param amountOfTokens - the amount of tokens want to buy
     */
    function calculatePurchasePrice(uint256 tokenPrice, uint256 amountOfTokens)
        external
        pure
        returns (uint256)
    {
        PRBMath.UD60x18 memory _tokenPriceUD = PRBMath.UD60x18({
            value: tokenPrice
        });
        PRBMath.UD60x18 memory _amountOfTokensUD = PRBMath.UD60x18({
            value: amountOfTokens
        });

        uint256 _purchasePrice = _tokenPriceUD.mul(_amountOfTokensUD).value;
