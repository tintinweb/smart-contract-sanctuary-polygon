...
import "./NiftyBridge721UpgradeableV1.sol";

contract NiftyBridge721UpgradeableV2 is NiftyBridge721UpgradeableV1 {
...
contract NiftyBridge721UpgradeableV1 is
    IERC5643,
    ERC721Upgradeable,
    ERC2981Upgradeable,
    OwnableUpgradeable,
    UUPSUpgradeable
{
    ...
    function initialize(
        ...
        __ERC721_init("NiftyBridge721NftCollection", "NB");
        __Ownable_init();
    ...
    //TODO: owner or manager
    function _authorizeUpgrade(
        address newImplementation
    ) internal override onlyOwner {}

    modifier ownerOrManager() {
        require(isOwner() || isManager());
        _;
    }
    ...
    function isManager() public view returns (bool) {
        return _msgSender() == _manager && _manager != address(0);
    }
    ...
    function mint(
        ...
        uint64 duration
    ) external virtual ownerOrManager returns (uint256 _id) {
        _tokenIds.increment();
        ...
        if (bytes(_uri).length > 0) {
            setTokenURI(_uri, _id);
        }

        if (duration > 0) {
            _setSubscription(_id, duration);
        }
    }
    ...
    ) external virtual ownerOrManager returns (uint256 firstTokenId) {
        firstTokenId = _tokenIds.current() + 1;
    ...
    function renewSubscription(
        uint256 tokenId,
        uint64 duration
    ) public payable virtual ownerOrManager {
        require(
            isManager() || isOwner() || _isApprovedOrOwner(msg.sender, tokenId),
            "Caller is not owner nor approved nor manager"
        );
        ...
        if (currentExpiration == 0) {
            ...
            if (!isRenewable(tokenId)) {
                revert("Subscription not renewable");
            }
    ...
    function cancelSubscription(
        uint256 tokenId
    ) external payable virtual ownerOrManager {
        require(
            isManager() || isOwner() || _isApprovedOrOwner(msg.sender, tokenId),
            "Caller is not owner nor approved nor manager"
        );
...
interface IERC5643 {
    ...
    /// should be access from NB master address
