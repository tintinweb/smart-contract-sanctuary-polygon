...
interface Sepatickets{
 ...
 contract SepaticketsMarket is IERC1155Receiver, Ownable, ReentrancyGuard{
  ...
  event ItemBought(
    ...
    require(Sepatickets(mintingContract).isTicket(tokenId),"Nft not a ticket");
   ...
   function hashSaleTransaction(
    ...
    (address minterAdd, uint256 royaltyPercentage) = Sepatickets(mintingContract).idToOrganizerRoyalty(tokenId);
  ...
  function EndSale(uint256 itemId) external nonReentrant {
    ...
    function gethfghg(uint256 ticket) public view returns(address , uint256 ){
        (address minterAdd, uint256 royaltyPercentage) = Sepatickets(mintingContract).idToOrganizerRoyalty(ticket); 
