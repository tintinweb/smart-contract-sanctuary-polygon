	...
	Withdrawable
{
	...
	uint256 public constant PRE_SALES_MAX_SUPPLY = 2000;

	uint256 public constant PRE_SALES_PRICE = 0.05 ether;

	uint256 public constant PUBLIC_SALES_START_PRICE = 0.1 ether;

	uint256 public constant PRICE_DROP_DURATION = 600;

	uint256 public constant PRICE_DROP_AMOUNT = 0.025 ether;

	uint256 public constant PRICE_DROP_FLOOR = 0.08 ether;

	uint public constant MAX_PURCHASE = 20;

	// merkle root generated by MerkleTree
	bytes32 public constant WHITELIST_MERKLE_ROOT = 0xecf96566abbdff297e7d40d7d0e7d48ac529c01e5ec2e4efee7bc4a089ea0c0d;
	...
	constructor(string memory _baseTokenURI) ERC721("Sabda Alam NFT", "BIRD") {
	...
	/// @dev get available birds to mint.
	function getAvailableBirds() public view returns (uint256) {
	...
	function getPrice() public view returns (uint256) {
        ...
        if (isPublicSalesActivated()) {
            ...
            return
                dropCount < 12
                    ? PUBLIC_SALES_START_PRICE - dropCount * PRICE_DROP_AMOUNT
                    : PRICE_DROP_FLOOR;
        }
	...
	/// @dev  verify whitelist
	// function verifyWhitelist(bytes32[] calldata _merkleProof) public view returns(bool) {


	// 	bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
	// 	require(MerkleProof.verify(_merkleProof, WHITELIST_MERKLE_ROOT, leaf), "Not on whitelist");
	...
	function preSaleMint(bytes32[] calldata _proof, uint numberOfTokens)
		public
		payable
		isPreSalesActive
	{
		bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
		require(MerkleProof.verify(_proof, WHITELIST_MERKLE_ROOT, leaf), "Not on whitelist");
		...
		require(preSalesMintedQty.add(numberOfTokens) <= PRE_SALES_MAX_SUPPLY, "Purchase would exceed pre-sale limit");
		...
		isPublicSalesActive
	{
		uint256 tokenId = currentTokenId.current();
		require(numberOfTokens <= MAX_PURCHASE, "Can only mint 20 tokens at a time");
		// require(saleIsActive, "Sale must be active to mint");  //TODO: on live use pre-sales activation
		require(tokenId.add(numberOfTokens) <= MAX_SUPPLY, "Purchase would exceed max supply");
		require(getPrice() * numberOfTokens >= msg.value, "Ether value sent is not correct");
