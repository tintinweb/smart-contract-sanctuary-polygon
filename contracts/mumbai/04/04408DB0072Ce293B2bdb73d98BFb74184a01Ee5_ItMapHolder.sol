...
interface IUserRegistry {
  ...
  function getInvestorById(uint256 i) external view returns (bytes32);
...
contract UserRegistry {
  ...
  function getInvestorById(uint256 i) public view returns (bytes32) {
  ...
  function getContribLimitFor(bytes32 id) public view returns (uint) {
    return contributionLimits[userMap.getKycLevelFor(id)] * 10 ** 18;
  }
  ...
  function getRemainingInvestAmtOf(bytes32 id) public view returns (uint256) {
    ...
    uint256 limit = contributionLimits[level] * 10 ** 18;
  ...
  function getRemainingInvestAmtConvert(address tokenOut, bytes32 id) public view returns (uint256) {
    ...
    // do it for usdt so =/= decimal todo
...
contract PairFactory {
  ...
  constructor(address addressRegistry_, uint256 fee_) {
    _addressRegistry = addressRegistry_;
    _implementation = address(new PairSwap());
    _baseFee = fee_;
  }
...
contract PairSwap is Initializable, Pausable, ReentrancyGuard {
  ...
  function __PairSwap_init(
    ...
    require(_isFactory(msg.sender), "initializer is not factory");
    token0 = token0_;
    token1 = token1_;
    fee = fee_;
  }
  ...
  function swapAmountOut(
    ...
    require(_amountOut <= liquidity, "Insufficient liquidity");
    ...
    require(_amountInMax <= requiredAmt, "PairSwap: required amount is more than amountInMax");
    ...
    require(newBalance - initialBalance == requiredAmt, "Transfer failed");
  ...
  // ---- Add liquidity ---- //
  // bool istoken => address? better comphrension?
...
contract CHF is ERC20, ERC20Permit {
  constructor(
    ...
    address _account6,
    address _account7
  ) ERC20("CHF", "CHF") ERC20Permit("CHF") {
    _mint(_account1, 10000000 * 10 ** decimals());
    _mint(_account2, 10000000 * 10 ** decimals());
    _mint(_account3, 10000000 * 10 ** decimals());
    _mint(_account4, 10000000 * 10 ** decimals());
    _mint(_account5, 10000000 * 10 ** decimals());
    _mint(_account6, 10000000 * 10 ** decimals());
    _mint(_account7, 5000 * 10 ** decimals());
  }
}
...
contract EUR is ERC20, ERC20Permit {
  constructor(address _account1, address _account2) ERC20("EUR", "EUR") ERC20Permit("EUR") {
    _mint(_account1, 10000000 * 10 ** decimals());
    _mint(_account2, 10000000 * 10 ** decimals());
  }
}
...
contract USD is ERC20, ERC20Permit {
  constructor(address _account1, address _account2) ERC20("USD", "USD") ERC20Permit("USD") {
    _mint(_account1, 10000000 * 10 ** decimals());
    _mint(_account2, 10000000 * 10 ** decimals());
  }
}
...
contract USDT is ERC20, ERC20Permit {
  constructor(address _account1, address _account2) ERC20("USDT", "USDT") ERC20Permit("USDT") {
    _mint(_account1, 10000000 * 10 ** decimals());
    _mint(_account2, 10000 * 10 ** decimals());
  }
...
contract SecurityFactory {
  ...
  constructor(address addressRegistry_) {
    _addressRegistry = addressRegistry_;
    _implementation = address(new SecurityToken());
  }
  ...
  function createSecurityToken(
    ...
    cloneAddr = Clones.clone(_implementation);
    SecurityToken(cloneAddr).__SecurityToken_init(_addressRegistry, _name, _symbol, _docUUID);
...
contract CrowdsaleFactory is ICrowdsaleConfig {
  ...
  address public _addressRegistry;
  ...
  constructor(address addressRegistry_, address beneficiary_) {
    _cappedImpl = address(new CappedCrowdsale());
    _uncappedImpl = address(new UncappedCrowdsale());
    _batchImpl = address(new BatchDrivenCrowdsale());
    require(addressRegistry_ != address(0), "CRF01");
    _addressRegistry = addressRegistry_;
  ...
  function createCappedCrowdsale(
    ...
    require(_executionValueMin < _maxTokenPerOrder && _startTime < _endTime && _softcap < _hardcap, "CRF02");
  ...
  function setNewBeneficiary(address newAddress) public onlyRole(CROWDSALE_ADMINISTRATOR_ROLE) {
    require(newAddress != _beneficiary, "CRF03");
...
/**
 * CRW01: Sender is missing role
 * CRW02: Value can't be null nor equal to zero
 * CRW03: Sender or Investor is not whitelisted
 * CRW04: Invested amount too low
 * CRW05: Invested amount too big
 * CRW06: New value equal to old value
 * CRW07: Value => remainingInvestment
 */
contract Crowdsale is Pausable, Initializable, ReentrancyGuard {
  ...
  function buyTokensPermit(
    ...
    (uint256 decimalPurchase, bytes32 id) = (purchasedAmount * 10 ** 18, _getIdByAddress(investor));
    _preValidatePurchase(id, decimalPurchase); // Check inputs
    uint256 requiredAmt = _getFinalAmt(paymentToken, decimalPurchase); // convert in sale currency
    uint256 purchaseCost = decimalPurchase * tokenPrice;
    _remInvest(id, paymentToken, requiredAmt); // check rem' invest in KCHF
    ...
    _updateSaleState(decimalPurchase, purchaseCost); // update sale state
    _updateContribState(id, investor, decimalPurchase, purchaseCost); // update contrib
    _emitEvent(id, investor, paymentToken, decimalPurchase, requiredAmt, purchaseCost);
  }
  ...
  function addInvestmentManually(
    ...
    _preValidatePurchase(id, decimalPurchase); // Check inputs
    uint256 purchaseCost = decimalPurchase * tokenPrice;
    _remInvest(id, baseCurrency, purchaseCost); // check rem' invest in KCHF
    _processPurchase(investor, decimalPurchase); // process purchase
    _updateSaleState(decimalPurchase, 0); // update sale state
    _updateContribState(id, investor, decimalPurchase, 0); // update contrib
    _emitEvent(id, investor, paymentToken, decimalPurchase, 0, purchaseCost);
  }

  function removeInvestment(address investor) external onlyRole(CROWDSALE_ADMINISTRATOR_ROLE) {
  ...
  function setNewBeneficiary(address newAddress) external onlyRole(CROWDSALE_ADMINISTRATOR_ROLE) {
    require(newAddress != address(0), "CRW02");
    require(newAddress != tokenAddress, "CRW06");
  ...
  function setNewPrice(uint256 newVal) external onlyRole(CROWDSALE_ADMINISTRATOR_ROLE) {
    require(newVal > 0, "CRW02");
    require(newVal != tokenPrice, "CRW06");
    tokenPrice = newVal;
    emit TokenPriceUpdated(address(this), tokenAddress, newVal);
  }
  ...
  function getFinalAmt(address pay_tok, uint256 amt) public view returns (uint256 pay_amt) {
    uint256 purchasedAmt = amt * 10 ** 18;
    return _getFinalAmt(pay_tok, purchasedAmt);
  }
  ...
  function __Crowdsale_init(
    ...
    saleType = saleType_;
    (beneficiary, tokenAddress, baseCurrency, tokenPrice) = (beneficiary_, token_, crowdsaleCurrency_, tokenPrice_);
  ...
  function _getIdByAddress(address investor) internal view returns (bytes32) {
    address userRegistry = getUserRegAddr();
    return IUserRegistry(userRegistry).getIdByAddress(investor);
  }

  function _remInvest(bytes32 id, address paymentToken, uint256 paidAmt) internal view {
    uint256 val;
    ...
    bool isKCHF = baseCurrency == ITokenRegistry(tokenReg).getStableAddress("CHF");
    uint256 rem = IUserRegistry(userRegistry).getRemainingInvestAmtOf(id);
    if (isKCHF) {
      val = paidAmt;
    } else {
      uint256 price = IPriceFeed(priceFeed).getLatestPrice(paymentToken, baseCurrency);
      val = paidAmt.mul(price).div(10 ** 8);
    }
    require(val <= rem, "Wrong Purchase Amt");
  }

  function _preValidatePurchase(bytes32 id, uint256 amount) internal virtual {
    require(id != bytes32(0), "_preValidatePurchase: issue");
    require(amount != 0 && amount >= executionValueMin && amount <= maxTokenPerOrder, "_preValidatePurchase: issue");
  }

  function _deliverTokens(address investor, uint256 purchasedAmt) internal virtual {
    require(IERC20(tokenAddress).balanceOf(address(this)) >= purchasedAmt, "Not enough token in SC");
    IERC20(tokenAddress).safeTransfer(investor, purchasedAmt);
  }
  ...
  function _forwardFundsPermit(
    ...
    require(pay_tok != address(0), "Invalid pay_tok address");
    ...
    if (pay_tok == baseCurrency) {
      return _transferFunds(investor, receiver, pay_tok, purchasedAmt);
    } else {
      uint256 finalAmount = _getFinalAmt(pay_tok, purchasedAmt);
      return _swapIntoCrowdsaleCurrency(investor, receiver, pay_tok, purchasedAmt, finalAmount);
    }
  }

  function _forwardFunds(address investor, address pay_tok, uint256 purchasedAmt) internal returns (uint256) {
    require(investor != address(0), "Invalid investor address");
    require(pay_tok != address(0), "Invalid pay_tok address");
  ...
  function _getFinalAmt(address pay_tok, uint256 amt) public view returns (uint256 pay_amt) {
    require(pay_tok != address(0), "Invalid pay_tok address");
  ...
  function _updateSaleState(uint256 buy, uint256 paid) internal {
    totalTokensSold += buy;
    totalRaised += paid;
  }

  function _updateContribState(bytes32 id, address investor, uint256 purchasedAmt, uint256 paidAmt) internal {
    address userRegistry = getUserRegAddr();
  ...
  function _swapConvertEstimationOut(
    ...
    address base,
    uint256 amountOut
  ) internal view returns (uint256 pay_amt) {
    address factory = getPairFactoryAddr();
...
contract BatchDrivenCrowdsale is Crowdsale, TimedValidation, BatchedValidation, ICrowdsaleConfig {
  ...
  function _preValidatePurchase(
    bytes32 contributor,
    uint256 amount
  ) internal virtual override(Crowdsale, BatchedValidation, TimedValidation) {
    require(_state == State.Active, "Not opened");
    super._preValidatePurchase(contributor, amount);
  }
...
contract CappedCrowdsale is Initializable, Crowdsale, FinalizableCrowdsale, CappedValidation, ICrowdsaleConfig {
  ...
  function distributeToken(uint256 startIndex, uint256 endIndex) public onlyRole(CROWDSALE_ADMINISTRATOR_ROLE) {
    ...
    for (uint i = startIndex; i <= endIndex; i++) {
      ...
      if (buyAmt > 0) {
        ...
        IERC20(tokenAddress).safeTransfer(beneficiary, buyAmt);
        ids[i - startIndex] = id;
    ...
    address _event = getEventAddr();
    ICrowdsaleEvent(_event).distributeEvent(tokenAddress, ids, beneficiaries, purchasedAmt);
  }
  ...
  function _preValidatePurchase(
    bytes32 contributor,
    uint256 amount
  ) internal virtual override(Crowdsale, TimedValidation, CappedValidation) {
    super._preValidatePurchase(contributor, amount);
  }
...
contract UncappedCrowdsale is Crowdsale, TimedValidation, ICrowdsaleConfig {
  ...
  function _preValidatePurchase(
    bytes32 beneficiary,
    uint256 amount
  ) internal virtual override(Crowdsale, TimedValidation) {
...
contract BatchedValidation is Crowdsale {
  ...
  function _preValidatePurchase(bytes32 beneficiary, uint256 amount) internal virtual override(Crowdsale) {
...
contract CappedValidation is Crowdsale {
  ...
  function _preValidatePurchase(bytes32 beneficiary, uint256 amount) internal virtual override(Crowdsale) {
    require((totalRaised + amount) <= hardcap, "CappedCrowdsale: cap exceeded");
    super._preValidatePurchase(beneficiary, amount);
  }
}
...
contract TimedValidation is Crowdsale {
  ...
  function _preValidatePurchase(
    bytes32 beneficiary,
    uint256 amount
  ) internal virtual override(Crowdsale) onlyWhileOpen {
    super._preValidatePurchase(beneficiary, amount);
  }
}
