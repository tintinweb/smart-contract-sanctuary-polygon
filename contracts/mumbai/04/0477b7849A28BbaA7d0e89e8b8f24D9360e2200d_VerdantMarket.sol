...
library Pricing {
    ...
    {
        commissionFromBuyer = (amount * commission) / (10 ** 4);
        amountToBuyer = amount + commissionFromBuyer;

        if (taxForBuyerPercent > 0) {
            taxAmount = (commissionFromBuyer * taxForBuyerPercent) / (10 ** 4);
            amountToBuyer += taxAmount;
        }
    }
    ...
    function convertFloatingPrie(
        ...
        AggregatorV3Path[] memory _aggregatorV3Paths,
        uint256 RATE_DECIMALS
    ) public view returns (uint256 price, PricePath[] memory paths) {
        uint256 rate = 10 ** RATE_DECIMALS;

        uint256 length = _aggregatorV3Paths.length;
        paths = new PricePath[](length);
...
contract Manager is Initializable, OwnableUpgradeable, PausableUpgradeable {
    ...
    mapping(string => mapping(address => Pricing.AggregatorV3Path[]))
    ...
    event FloatingPaymentMethodSet(
        ...
        Pricing.AggregatorV3Path[] aggregatorV3Paths
    );
    ...
    function setFloatingPaymentMethod(
        ...
        address _paymentToken,
        Pricing.AggregatorV3Path[] calldata _aggregatorV3Paths
    ) external onlyOwner {
        require(paymentMethod[_paymentToken], "Payment-token-not-support");
        delete aggregatorV3Paths[_assetName][_paymentToken];
        ...
        for (uint256 i = 0; i < _aggregatorV3Paths.length; i++) {
            ...
            aggregatorV3Paths[_assetName][_paymentToken].push(
    ...
    function getAggregatorV3Paths(
        string memory _assetName,
        address _paymentToken
    ) public view returns (Pricing.AggregatorV3Path[] memory) {
    ...
    function withdrawFunds(
        ...
        if (_tokenAddress == address(0)) {
            ...
            IERC20MetadataUpgradeable(_tokenAddress).transfer(
                _beneficiary,
                IERC20MetadataUpgradeable(_tokenAddress).balanceOf(
                    address(this)
                )
            );
        }
    ...
    ReentrancyGuardUpgradeable
{
    ...
    struct Order {
        ...
        string assetName;
        Pricing.PricePath[] pricePathConfirmed;
        uint256 premiumPercent;
    ...
    function getOrderFloatingPrice(
        uint256 _orderId
    ) external view returns (uint256 price, Pricing.PricePath[] memory paths) {
        Order memory order = orders[_orderId];
        Pricing.AggregatorV3Path[]
            memory _aggregatorV3Paths = getAggregatorV3Paths(
                order.assetName,
                order.paymentToken
            );
        (price, paths) = _getOrderFloatingPrice(order, _aggregatorV3Paths);
    }

    function _getOrderFloatingPrice(
        Order memory order,
        Pricing.AggregatorV3Path[] memory _aggregatorV3Paths
    ) private view returns (uint256 price, Pricing.PricePath[] memory paths) {
        uint256 tokenDecimals = IERC20MetadataUpgradeable(order.paymentToken)
            .decimals();

        (price, paths) = Pricing.convertFloatingPrie(
            order.price,
            tokenDecimals,
    ...
    function _match(
        ...
        uint256 taxForSeller = (order.commissionToSeller *
            _taxForSellerPercent) / ZOOM_FEE;
        uint256 amountToFeeTo = _commissionFromBuyer +
            order.commissionToSeller +
            _taxForBuyerAmount +
            taxForSeller;

        // send payment to feeTo
        _paid(_paymentToken, feeTo, amountToFeeTo);
    ...
    function calBuyerCommissionHasSign(
        BuyerCommissionInput memory _buyerCommission,
        PromoCodeInfo memory _promoCodeInfo,
        uint256 taxForBuyerPercent
    ) private returns (BuyerCommissionOutput memory) {
        require(taxForBuyerPercent <= ZOOM_FEE, "Invalid tax");
        if (_promoCodeInfo.isPercent) {
            require(
                _promoCodeInfo.promoCodeNft <= ZOOM_FEE,
                "Invalid Nft code"
            );
            require(
                _promoCodeInfo.promoCodeServiceFee <= ZOOM_FEE,
        ...
        (
            ...
            _buyerCommissionOutput.taxAmount
        ) = Pricing.calBuyerCommissionHasSign(
    ...
    function buy(
        ...
        require(
            Signature.verifyBuyMessage(
                ...
                _promoCodeInfo.promoCodeNft,
                _promoCodeInfo.promoCodeServiceFee,
                _taxInfo.taxForBuyerPercent,
                _taxInfo.taxForSellerPercent,
                _signatureExpTime,
                nFTAddress,
                verifier
            ),
        ...
        if (isUserBid) {
            ...
            if (bid.paymentToken == _paymentToken) {
                ...
                if (buyerPaidAmount > _buyerCommissionOutput.amountToBuyer) {
                    ...
                    IERC20MetadataUpgradeable(_paymentToken).safeTransferFrom(
                        msg.sender,
                        address(this),
    ...
    function createBid(
        ...
        uint256 _signatureExpTime,
        bytes calldata _signature
    ) external whenNotPaused returns (uint256 _bidId) {
        bytes32 _id = keccak256(
            abi.encodePacked(nFTAddress, _tokenId, msg.sender)
        );
        ...
        IERC20MetadataUpgradeable(newBid.paymentToken).safeTransferFrom(
            msg.sender,
            address(this),
            _buyerCommissionOutput.amountToBuyer
        );
    ...
    function cancelBid(uint256 _bidId) external whenNotPaused nonReentrant {
        ...
        uint256 payBackAmount = bid.bidPrice +
            ...
            bid.taxAmount;
        if (payBackAmount > 0) {
            IERC20MetadataUpgradeable(bid.paymentToken).safeTransfer(
                bid.bidder,
                payBackAmount
            );
        }
        ...
        isBid[
            keccak256(abi.encodePacked(nFTAddress, bid.tokenId, bid.bidder))
        ] = false;
    ...
    function updateBid(
        ...
        if (bid.bidPrice != _bidPrice) {
            ...
            if (isExcess) {
                ...
                IERC20MetadataUpgradeable(bid.paymentToken).safeTransferFrom(
                    bid.bidder,
                    address(this),
                    amount
                );
