...
contract Treasury is AccessControl {
    ...
    */
    modifier onlyFactory(address addr) {
        require(addr == factory_contract, "Only Factory contract has the Access Permission");
        _;
    }
    ...
    */
    function setCurrency(address _currency) public onlyRole(ADMIN_ROLE) {
        currency = _currency;
    }
    ...
    */
    function withdrawUSDCFund(uint256 _total, address _account) public onlyRole(ADMIN_ROLE) {
    ...
    * @notice This function can only be called by an address with `ADMIN_ROLE`
    */
    function withdrawFund(uint256 _total, address _currency, address _account) public onlyRole(ADMIN_ROLE) {
    ...
    */
    function cancelUSDCRefund(uint256 _total, address _account) public onlyFactory(_msgSender()) {
        IERC20(currency).transfer(_account, _total);
    }
    ...
    * @notice This function can only be called by factory contract
    */
    function cancelRefund(uint256 _total, address _account, address _currency) public onlyFactory(_msgSender()) {
        IERC20(_currency).transfer(_account, _total);
    }
}