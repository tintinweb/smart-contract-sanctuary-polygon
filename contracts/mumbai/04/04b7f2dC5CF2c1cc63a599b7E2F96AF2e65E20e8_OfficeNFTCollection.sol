...
contract OfficeNFTCollection is 
    ...
    {
        address[] memory owners = new address[](getMintCount());

        for (uint256 i = 0; i < getMintCount(); i++) {
                owners[i] = ownerOf(i);
            }
...
//Implements the functionality for TokenURIERC721, and adds to it by explicity stating a role,
//through OpenZeppelin's AccessControl standard, that only allows certain addresses to be able
//to set tokens' tokenURIs.
abstract contract TokenURIERC721StorageOnlyByDesignatedRole is TokenURIERC721Storage, AccessControl {

    //Initializes the TOKEN_SETTER_ROLE role property. Hashes the name.
    bytes32 public constant TOKEN_SETTER_ROLE = keccak256("TOKEN_SETTER_ROLE");

    //Gets passed in an address during deployment which will be given the TOKEN_SETTER_ROLE role.
    constructor(address tokenSetterAddress){
        _setupRole(TOKEN_SETTER_ROLE, tokenSetterAddress);
    }

    //Overrides MintableERC721's mint function. Adds a check to make sure
    //that the address calling this function has the role: TOKEN_SETTER_ROLE.
    ...
    //AccessControl and ERC721 both implement supportsInterface. Thus, it needs to be overrided here so that
    //it can be explicity stated how the smart contract wants to handle their implementations
    //In this case, it simply calls their base implementations.
    function supportsInterface(bytes4 interfaceId)
    public
    view
    virtual
...
//Adds the public facing Mintable functionality and also attaches the AccessControl feature set
//to allow a MINTER role to be created. Which will limit the mint function to only succesfully
//be called by the address passed into the constructor. 
abstract contract MintableERC721OnlyByDesignatedRole is MintableERC721, AccessControl {

    //Initializes the name of the minter role
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    //Gives the passed in address the minter role  
    constructor(address minterRoleAddress){
        _setupRole(MINTER_ROLE, minterRoleAddress);
    }
    
    //Overrides MintableERC721's mint function. Adds a check to make sure
    //that the address calling this function has the role: MINTER_ROLE.
...
//Adds public facing functionality to Web3 technologies which allows them to set ERC721 tokens' tokenURIs.
abstract contract TokenURIERC721Storage is ERC721URIStorage {
    
    //Initializes the event to be called when a token's tokenURI has been set.
    event OnSetTokenURI(uint256, string);

    //Function called by Web3 technologies to set the selected token's tokenURI to a custom URI
    //@params:
    //  @param 1: The tokenId of the token of which will have its tokenURI changed.
    function setTokenURI(uint256 tokenId, string memory _tokenURI) public virtual {
        
        //calls the base implementation of _setTokenURI.
        super._setTokenURI(tokenId, _tokenURI);
        //Invokes the event passing in the tokenId, and new URI designated to the token.
...
//Adds public facing functionality to Web3 technologies which allows them to mint ERC721 tokens.
abstract contract MintableERC721 is ERC721 {
    //Gets and Sets a reference to OpenZeppelin's Counters utility smart contract. 
    using Counters for Counters.Counter;

    //Initializes a property which holds the number of minted tokens.
    //TokenId's are assumed to start at 0.
    Counters.Counter private _mintCount;

    //Initializes an event that will run when the mint function has finished running.
    event Minted(uint256 tokenId);

    //The public facing mint function to be called from Web3 technologies.
    function mint() public virtual {
        //Assigns a local variable to the current value of the number of minted tokens
        uint256 newTokenId = _mintCount.current();
        //Calls ERC721's _safeMint function passing in the address which called this function, and the tokenId that we want to set it to. 
