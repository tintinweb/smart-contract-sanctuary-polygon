...
abstract contract MysteryBoxStorage {
    uint256 internal constant VERSION = 10;
...
import "../storage/MysteryBoxStorage.sol";
...
contract MysteryBox is ERC721ABase, MysteryBoxStorage, IMysteryBox,  IUpgradeable {

    function initialize(string memory _name, string memory _symbol, string memory boxType_, string memory _metadataUrl, string memory _contractUrl, address _owner) initializerERC721A initializer public {
        super._initialize(_name, _symbol, _metadataUrl, _contractUrl, _owner);
        boxType = boxType_;
    }
    ...
    function balanceOf(address owner) public view override(IERC721AUpgradeable, ERC721AUpgradeable, IMysteryBox) returns (uint256 balance) {
    ...
    function mint(address to, uint256 amount, bool freeReward) public payable {
        ...
        if ((owner() != msg.sender) && !freeReward && msg.value < price * amount) {
