...
contract swapHelperDiyTask is OpsTaskCreator, Ownable {
    ...
    event CounterTaskCreated(uint256 _taskAmount, bytes32 _taskId);
    event CounterTaskCreated2(taskConfig _taskConfig);
    event swapCotE(address _tx_origin, address _msg_sender, uint256 _gasUsed, uint256 _spendSwapInToken, uint256 _timestamp, address _user);
    event swapCotE2(uint256 _index, bytes32 _md5, taskConfig _taskConfig);
    ...
    function createTask(IAocoRouter02 _routerAddress, address[] memory _swapRouter, address[] memory _swapRouter2, uint256 _swapAmount, uint256 _interval, uint256 _salt) external payable {
        ...
        taskConfigList[id] = k;
        emit CounterTaskCreated(taskAmount, id);
        emit CounterTaskCreated2(k);
    ...
    function swapToken(address _user, bytes32 _md5) external onlyDedicatedMsgSender {
        ...
        if (x._swapIn == 0) {
            ...
            emit swapCotE(tx.origin, msg.sender, x.gasUsed, x.spendSwapInToken, block.timestamp, _user);
            ...
            emit swapCotE2(y._index, y._md5, y);
        }
    }
