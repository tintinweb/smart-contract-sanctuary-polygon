...
contract USDRTreasury is AddressAccessor, ITreasury, IERC721Receiver {
    ...
    function triggerRebase(uint256 depositAmount, uint256 amount)
        external
        onlyRole(CONTROLLER_ROLE)
    {
        ...
        returns (ITreasury.TreasuryValue memory value)
    {
            ...
            );
        {
            ...
            value.usdr = IERC20(usdr).balanceOf(address(this)) * 1e9;
            value.rwa =
        ...
        {
            uint256 tngblPrice = IPriceOracle(oracle).quote(TNGBL);
            value.tngbl =
                (IERC20(tngbl).balanceOf(address(this)) * tngblPrice) /
                TNGBL;
        ...
        value.liquidity = ILiquidityManager(liquidityManager).liquidity();
        value.debt =
