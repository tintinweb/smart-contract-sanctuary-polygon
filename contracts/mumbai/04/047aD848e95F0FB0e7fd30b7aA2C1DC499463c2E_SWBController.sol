...
contract SWBController is SWBControllerHelpers, SWBControllerEvents, KeeperCompatibleInterface, VRFConsumerBaseV2, ReentrancyGuard, Ownable {
    ...
    uint public s_houseSharePerc = 7;
    uint public s_winnersSharePerc = 50;
    uint public s_winnersSWBSharePerc = 18;
    uint public s_losersSharePerc = 25;
    ...
    // SweetBlocks state
    uint constant s_minNum = 1;
    uint constant s_maxNum = 36;
    ...
    mapping(uint => uint) public s_rand_dir;
    // Players state
    ...
    function performUpkeep(bytes calldata performData) external override { 
        ...
        if(keccak256(performData) == keccak256(hex'04')) {
            ...
            if(s_MegaDrawers.length>0) {
                windex = (s_last_random_word - block.timestamp - block.number) % s_MegaDrawers.length;
    ...
    */
    ...
    function getRandomness(uint dir) private  {
        ...
        s_rand_dir[s_requestId] = dir;
        emit RequestRandomnessEvent(dir, block.number);
    }
    ...
    */
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        ...
        if(s_rand_dir[requestId]==1) {
            ...
            if(s_swb_prize[s_swb_block_last]>0) {
                // store sb losers count
                    ...
                    //}
                    emit FulfillRandomnessEvent(s_swb_block_last, randomWords[0], s_rand_dir[requestId], block.number);
        ...
        //megadraw
        } else if(s_rand_dir[requestId]==2) {
            rotateMegaDraw();
            uint _HouseShare = s_HouseShare;
            uint windex = 0;
            uint winnerHouseShare = 0;
            ...
            emit FulfillRandomnessEvent(s_mega_draw_block_last, randomWords[0], s_rand_dir[requestId], block.number);
            emit CloseMegaDrawEvent(windex, s_MegaDrawLastWinner, winnerHouseShare, (_HouseShare-winnerHouseShare), _HouseShare, 0, block.number);
        }
    }
