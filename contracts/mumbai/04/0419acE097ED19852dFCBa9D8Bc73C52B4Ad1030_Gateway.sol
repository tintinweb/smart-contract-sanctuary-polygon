...
contract Gateway is Context, ReentrancyGuard {
    // states
    // chain id
    ...
    function validateValset(Utils.ValsetArgs calldata _valset, Utils.Signature[] calldata _sigs) private pure {
      ...
      modifier isSelf() {
        require(
            ...
            "ExecuteHandleCalls : Can only be called by Current Contract"
        );
        _;
    }
    ...
    function makeCheckpoint(Utils.ValsetArgs memory _valsetArgs, string memory _chainId) private pure returns (bytes32) {
    ...
    function updateValset(
        ...
        if (_newValset.valsetNonce <= _currentValset.valsetNonce) {
            revert Utils.InvalidValsetNonce({ newNonce: _newValset.valsetNonce, currentNonce: _currentValset.valsetNonce });
        }
        ...
        if (cumulativePower <= Utils.constantPowerThreshold) {
            revert Utils.InsufficientPower({ cumulativePower: cumulativePower, powerThreshold: Utils.constantPowerThreshold });
        }
    ...
    function executeHandlerCalls(
        ...
        for (uint8 i = 0; i < handlers.length; i++) {
            ...
            (bool success, ) = address(handler).call(
                abi.encodeWithSelector(
                    IUser.handleRequestFromRouter.selector,
                    sender,
                    payloads[i]
                )
            );
