...
/**
 * @dev This contract is written only for mumbai testnet
 */
contract SendMeDemoLps {
    address private immutable demoLpTokenAddress;
    LiquidityWarsConfig liquidityWarsConfigContract;

    constructor(address _liquidityWarsConfigAddress) {
        liquidityWarsConfigContract = LiquidityWarsConfig(
            _liquidityWarsConfigAddress
        );
        demoLpTokenAddress = liquidityWarsConfigContract.getAllowedTokens()[0];
    }

    //Setters
    function sendMeDemoLps() external returns (uint256) {
        uint256 demoLpsToSend = (liquidityWarsConfigContract
            .getAmountOfLpTokensRequired(demoLpTokenAddress) * 11) / 10; //10% more than required
        require(
            demoLpsToSend >= getAvailableLps(),
            "Not enough lps in the contract"
        );
        console.log(
            "%s >= %s",
            IUniswapV2Pair(demoLpTokenAddress).balanceOf(address(this)),
            demoLpsToSend
        );
        IUniswapV2Pair(demoLpTokenAddress).transfer(msg.sender, demoLpsToSend);
        return demoLpsToSend;
    }

    function depositDemoLps(uint256 _amount) external {
        IUniswapV2Pair demoLp = IUniswapV2Pair(demoLpTokenAddress);
        require(
            (demoLp.allowance(msg.sender, address(this)) >= _amount),
            "Not have allowance"
        );
        demoLp.transferFrom(msg.sender, address(this), _amount);
    }

    // Getters
    function getAvailableLps() public view returns (uint256) {
        return IERC20(demoLpTokenAddress).balanceOf(address(this));
    }

    function getAddressOfLpContract() external view returns (address) {
        return demoLpTokenAddress;
    }

    function getAmountThatWillBeSent() external view returns (uint256) {
        uint256 demoLpsToSend = (liquidityWarsConfigContract
            .getAmountOfLpTokensRequired(demoLpTokenAddress) * 11) / 10; //10% more than required
        return demoLpsToSend;
    }
