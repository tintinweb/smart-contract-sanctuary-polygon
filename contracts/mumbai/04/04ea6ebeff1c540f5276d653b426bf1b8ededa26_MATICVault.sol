...
interface IConfiguration {
    ...
    error Configuration__InvalidCapTarget();
    ...
    function availableCap() external view returns (uint256);

    function spendCap(uint256 shares) external ;
 ...
 * @author Pods Finance
 */
abstract contract BaseVault is IVault, ERC20, ERC20Permit {
    ...
    constructor(IConfiguration _configuration, IERC20Metadata _asset)
    ...
    function controller() public view returns (address) {
        uint256 _address = configuration.getParameter(address(this), "VAULT_CONTROLLER");
        bytes memory data = new bytes(32);
        assembly {
            mstore(add(data, 32), _address)
        }
 ...
 * @author Pods Finance
 */
contract MATICVault is BaseVault {
    using TransferUtils for IERC20Metadata;
    using FixedPointMath for uint256;
    ...
    function name() public view override returns (string memory) {
        return string(abi.encodePacked(asset.symbol(), " BHAVISH GAME TOKEN"));
    }
    ...
    function symbol() public view override returns (string memory) {
        return string(abi.encodePacked(asset.symbol(), "BGT"));
    }
    ...
    function deposit(uint256 assets, address receiver) external virtual override returns (uint256 shares) {
        ...
        assets = _tryTransferMATIC(msg.sender, assets);
        depositQueue.push(DepositQueueLib.DepositEntry(receiver, assets));
     ...
     */
    function _tryTransferMATIC(address from, uint256 amount) internal returns (uint256 effectiveAmount) {
