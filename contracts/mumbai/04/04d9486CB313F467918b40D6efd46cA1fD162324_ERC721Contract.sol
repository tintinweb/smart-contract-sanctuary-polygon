...
interface IRoyalty is IERC2981Upgradeable {
    struct RoyaltyInfo {
        address recipient;
        uint256 bps;
    }
...
interface ITokenERC721 is IERC721Upgradeable {
    ...
    event TokenMinted(address indexed mintedTo, uint256 indexed tokenIdMinted);
    ...
    ITokenERC721
{
    ...
    function initialize(
        ...
        // ! initialize opensea enforcement!!
        ...
        maxSupply = _maxSupply; // ! When set this should be satisified
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        if (bytes(updatedTokens[tokenId]).length > 0) return updatedTokens[tokenId];
        else {
    ...
    function mint(
        ...
        // ! calculate token id to mint here
        // ! Validate price or token here
        // ! validate minted token or not
        _mintTo(msg.sender, _tokenId);
    }

    function addNewToken(string calldata _uri) external returns (uint256) {
        require(nextTokenIdToMint + addedTokens > maxSupply, "max supply reached.");
        require(bytes(_uri).length <= 0, "empty uri.");

        /// @dev increament new token counter.
        addedTokens += 1;
        /// @dev add newly added token URI to mapping.
    ...
    function _mintTo(address _to, uint256 _tokenId) internal returns (uint256) {
        ...
        if(bytes(updatedTokens[_tokenId]).length > 0) {
            ...
            uri[_tokenId] = tokenURI(_tokenId);
        }
