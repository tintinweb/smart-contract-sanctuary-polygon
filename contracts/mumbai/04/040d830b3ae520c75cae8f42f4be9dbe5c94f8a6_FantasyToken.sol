...
abstract contract ReducingTokenFees is ERC20Burnable, Ownable {

    uint256 private burnRate;
    uint256 private promoRate;
    uint256 private opsRate;
    
    uint256 private _tBurnTotal;
    
    address public PromotionalAccount;
    address public OperationsAccount;
    address public burnAccount = 0x000000000000000000000000000000000000dEaD;

    mapping (address => bool) private isExemptFromFee;

    constructor(uint256 _initialBurnRate, uint256 _initialPromoRate, uint256 _initialOpsRate, address _PromotionalAccount, address _OperationsAccount) {
	...
	opsRate = _initialOpsRate;
        ...
        isExemptFromFee[PromotionalAccount] = true;
        isExemptFromFee[OperationsAccount] = true;
    }
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        if(!isExemptFromFee[_msgSender()]) {
            ...
            opsFee = calculateOperationsFee(amount);
            _tBurnTotal = _tBurnTotal + burnFee;
            _transfer(_msgSender(), burnAccount, burnFee);
            _transfer(_msgSender(), PromotionalAccount, promoFee);
            _transfer(_msgSender(), OperationsAccount, opsFee);
        }
        _transfer(_msgSender(), recipient, amount - burnFee - promoFee - opsFee);
        return true;
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        _transfer(sender, recipient, amount - burnFee - promoFee - opsFee);
    ...
    function setburnRate(uint256 _newburnRate) external onlyOwner {
        require(_newburnRate <= 100, "RTT: Burn rate too high");
        burnRate = _newburnRate;
    }

    function setPromoRate(uint256 _newPromoRate) external onlyOwner {
        promoRate = _newPromoRate;
    }

    function setOpsRate(uint256 _newOpsRate) external onlyOwner {
        opsRate = _newOpsRate;
    }

   
    function addFeeExemption(address user) external onlyOwner {
       require(isExemptFromFee[user], "Account is already included");
        isExemptFromFee[user] = true;
    }

    function removeFeeExemption(address user) external onlyOwner {
      require(!isExemptFromFee[user], "Account is already excluded");
        isExemptFromFee[user] = false;
    }

 function ExemptFromFee(address user) public view returns (bool) {
        return isExemptFromFee[user];
    }

    function updatePromoAccount(address _newPromoAccount) external onlyOwner {
        PromotionalAccount = _newPromoAccount;
    }
    function updateOpsAccount(address _newOpsAccount) external onlyOwner {
        OperationsAccount = _newOpsAccount;
    }   
}

abstract contract AntiWhaleTx is ReducingTokenFees {

    uint256 private maxTxAmount;
    uint256 private hotlistTxAmount;
    uint256 private cooldownPeriod;

    mapping (address => bool) private isExemptFromTxLimit;
    mapping (address => uint) private lastHotTxTime;
    ...
    function _checkAndUpdateLimits(address from, uint256 amount) internal {
        require (isExemptFromTxLimit[from]
                || amount <= maxTxAmount,
                "AWT: Exceeds max transaction amount");
    ...
    function updateMaxTxAmount(uint256 percentTx) external onlyOwner {
        require (percentTx >= 20 && percentTx <= 100, "AWT: MaxTxAmount out of range");
        maxTxAmount = (totalSupply() * percentTx) / 10000;
    }

    function updateHotlistTxAmount(uint256 percentTx) external onlyOwner {
        require (percentTx >= 10 && percentTx <= 50, "AWT: HotlistTxAmount out of range");
        hotlistTxAmount = (totalSupply() * percentTx) / 10000;
    }
...
contract FantasyToken is AntiWhaleTx {

    uint256 initialSupply = 500000000 * 10**18;

    constructor(address _promoWallet, address _opsWallet)
        ERC20("Fantasy Token", "FTXXX") 
        ReducingTokenFees(100, 200, 200, _promoWallet, _opsWallet)
        AntiWhaleTx (
