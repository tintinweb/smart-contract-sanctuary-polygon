...
contract DealmintrFactory is Ownable {
    ...
    constructor(string memory _baseURI){
     ...
     uint256 maxBatchSize) external returns(uint256){
        Dealmintr _dealmintr = new Dealmintr(name,symbol,baseURI,maxBatchSize);
        dealmintrs.push(address(_dealmintr));
        index ++;
        DealmintrNFT memory _dealmintrNft = DealmintrNFT(c_name,name,symbol,address(_dealmintr));
    ...
    function getAllCollection(uint256 id) external view returns(string memory,string memory,string memory,address){

       return (dealmintrNfts[id].c_name,dealmintrNfts[id].name,dealmintrNfts[id].symbol,dealmintrNfts[id].c_collection);
    }
    ...
    Ownable
{
    ...
    function batchmint(
        ...
        for (uint256 i = 0; i < quantity; i++) {
            uint256 tokenId = i+1;
            _mint(to, tokenId, royaltyRecipient, royaltyValue);
