// SPDX-License-Identifier: MITadmin
 ...
 */
contract OddzPrediction is AccessControl, Pausable, ReentrancyGuard {
    ...
    function pause() external whenNotPaused onlyAdmin(msg.sender) {
        ...
        // refund current round, round +1, round+2  and increase current round id
     ...
     */
    function _createRound(uint256 roundId, uint256 startTimestamp) internal {
        require(rounds[roundId].roundId == 0, "Round already exists");
        require(startTimestamp - roundTime > block.timestamp, "Round time is too short");
    ...
    function _refundable(uint256 roundId) internal view returns (bool) {
        return
            rounds[roundId].rewardBaseCalAmount == 0 &&
     ...
     * @param roundId: Round Id
     */
    function predictUp(uint256 roundId) external payable whenNotPaused nonReentrant notContract(msg.sender) {
        ...
        round.upPredictAmount = round.upPredictAmount + amount;

        // Update user data
        if (ledger[roundId][msg.sender].upPredictAmount == 0 && ledger[roundId][msg.sender].downPredictAmount == 0) {
            userRounds[msg.sender].push(roundId);
            usersInRounds[roundId].push(msg.sender);
        }

        betInfo.upPredictAmount = betInfo.upPredictAmount + amount;

        emit PredictUp(msg.sender, roundId, msg.value);
    }
     ...
     * @param roundId: Round Id
     */
    function predictDown(uint256 roundId) external payable whenNotPaused nonReentrant notContract(msg.sender) {
        require(rounds[roundId].roundState == RoundState.CREATED, "Bet is too early/late");
        require(msg.value >= minPredictAmount, "Bet amount must be greater than minBetAmount");
        ...
        Round storage round = rounds[roundId];
        round.totalAmount = round.totalAmount + amount;
        round.downPredictAmount = round.downPredictAmount + amount;
        ...
        betInfo.downPredictAmount = betInfo.downPredictAmount + amount;

        emit PredictDown(msg.sender, roundId, msg.value);
    }
     ...
     * @dev callable by Operator
     */
    function startPredictionMarket() external whenNotPaused onlyOperator(msg.sender) notContract(msg.sender) {
        ...
        int256 price = oddzPriceManager._getPriceByTimestamp(roundzeroStartTimestamp);
     ...
     * @dev Callable by Operator
     */
    function executeRound(uint256 referenceId) external whenNotPaused onlyOperator(msg.sender) notContract(msg.sender) {
        ...
        int256 price = oddzPriceManager._getPriceByTimestamp(rounds[currentRoundId].endTimestamp);
        ...
        _createRound(currentRoundId + 3, rounds[currentRoundId + 2].endTimestamp);
...
interface IPriceManager {
    function _getPriceByTimestamp(uint256 timestamp) external returns (int256);
}
