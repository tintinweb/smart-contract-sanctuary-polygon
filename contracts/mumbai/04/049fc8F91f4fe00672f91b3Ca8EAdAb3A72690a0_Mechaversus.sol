...
contract Mechaversus is Initializable, OwnableUpgradeable {
    ...
    // @dev Lobby of active Match
    uint256[] public matchActiveLobby;
    ...
    struct Match {
        ...
        uint256 _block_to_respond;
        uint256 _arena_id;
        address _creator;
        address[] _active_players;
        address[] _players;
        uint256[] _decks;
        uint16 _stake_amount;
        uint256[] _performers_order;
        uint16 _last_action_id;
        bool _halted;
        address _winner;
    }
    ...
    mapping( uint256 => mapping( uint256 => uint16 ) ) _entities;
    ...
    modifier callerIsPlayer( uint256 match_id ) {
        bool founded = false;
        for( uint256 i = 0; i < PLAYERS_COUNT; i++) founded = founded || _matches[ match_id ]._active_players[i] == msg.sender;
    ...
    function getMatchActiveLobby() public view returns ( uint256[] memory ) {
        return matchActiveLobby;
    }
    ...
    function getMatchDecks( uint256 match_id ) public view returns ( uint256[] memory ) {
        ...
        return _matches[ match_id ]._performers_order.length == PLAYERS_COUNT * DECK_SIZE
        ? _matches[ match_id ]._decks : emptyUint256s;
    }

    function getMatchStakeAmount( uint256 match_id ) public view returns ( uint16 ) {
        return _matches[ match_id ]._stake_amount;
    }

    function getMatchPerformersOrder( uint256 match_id ) public view returns ( uint256[] memory ) {
        return _matches[ match_id ]._performers_order;
    }

    function getMatchActionCount( uint256 match_id ) public view returns ( uint16 ) {
        return _matches[ match_id ]._last_action_id;
    }
    ...
    function getMatchActions( uint256 match_id ) public view returns ( Action[] memory ) {
        Action[] memory actions = new Action[]( _matches[ match_id ]._last_action_id );
        for ( uint256 i = 0; i < _matches[ match_id ]._last_action_id; i++ ){
    ...
    function getEntityEP( uint256 match_id, uint256 entity_id ) public view returns ( uint16 ) {
    ...
    function generateMatch( uint16 stake_amount, uint256 block_to_respond ) public virtual {
        ...
        _matches[ matchId ] = Match({
            ...
            _block_to_respond: block_to_respond,
            _arena_id: arenaId,
            _creator: msg.sender,
            _active_players: emptyAddresses,
            _players: emptyAddresses,
            _decks: emptyUint256s,
            _stake_amount: stake_amount,
            _performers_order: emptyUint256s,
            _last_action_id: uint16(0),
            _halted: false,
            _winner: address(0x0)
        });

        matchActiveLobby.push( matchId );
    ...
    function destroyMatch( uint256 match_id ) public virtual {
        ...
        require( _matches[ match_id ]._start_block + MATCH_LIFE_BLOCKS <= block.number ||
        ...
        matchActiveLobby = Arrays.popUint256( matchActiveLobby, match_id );
    ...
    function joinMatch( uint256 match_id, uint256 deck_id ) public virtual {
        ...
        _Decks.deckAssetsOwnershipChecks( deck_id, msg.sender, DECK_SIZE );

        // @dev Player send Mechadium to the reserve
        require( _Mechadium.allowance( msg.sender, mechadium_reserve ) >= _matches[ match_id ]._stake_amount,
            'Mechaversus should be approved on MECHA from all players');
        _Mechadium.transferFrom( msg.sender, mechadium_reserve, _matches[ match_id ]._stake_amount );

        _matches[ match_id ]._active_players.push( msg.sender );
        _matches[ match_id ]._players.push( msg.sender );
        _matches[ match_id ]._decks.push( deck_id );
        _usedDeck[ deck_id ] = true;

        if( _matches[ match_id ]._players.length == PLAYERS_COUNT - 1 ) matchActiveLobby = Arrays.popUint256( matchActiveLobby, match_id );

        emit JoinMatch( match_id, _matches[ match_id ]._arena_id, msg.sender, _matches[ match_id ]._stake_amount );
    }
    ...
    function leaveMatch( uint256 match_id ) public virtual callerIsPlayer(match_id){
        ...
        require( _matches[ match_id ]._performers_order.length == 0, "Players cannot Leave a started Match" );

        // @dev Reserve resend Mechadium to the player
        _Mechadium.transfer( msg.sender, _matches[ match_id ]._stake_amount );

        if( _matches[ match_id ]._players.length == PLAYERS_COUNT ) matchActiveLobby.push( match_id );

        for( uint256 i = 0; i < _matches[ match_id ]._players.length; i++ ){
            if( _matches[ match_id ]._players[i] == msg.sender ) {
                _matches[ match_id ]._active_players = Arrays.popAddress( _matches[ match_id ]._active_players, _matches[ match_id ]._active_players[i] );
    ...
    // @dev The Player with fastest Mecha should set the actions performers order before Match starting
    function setMatchPerformersOrder( uint256 match_id, uint256[] memory performers_order ) public virtual callerIsPlayer(match_id){

        require( _matches[ match_id ]._performers_order.length == 0, "Performers order already set in this Match");
        ...
        _matches[ match_id ]._performers_order = performers_order;

        emit StartMatch( match_id, performers_order, _matches[ match_id ]._decks );
    }

    // @dev Next player turn the Match is updated with the action performed
    function setMatchNextTurn( uint256 match_id, uint256 performer_id, uint8[] memory action_type, bytes2[] memory coordinates,
        uint256[] memory target_ids, uint16[] memory target_eps ) public virtual callerIsPlayer(match_id){

        require( !_matches[ match_id ]._halted, "This Match is Halted for suspicious activities" );
        require( _matches[ match_id ]._performers_order.length == PLAYERS_COUNT * DECK_SIZE , "Players cannot update Match before it has started");
        require(
        ...
        for( uint256 i = 0; i < action_type.length; i++){
            ...
            uint16 actionId = _matches[ match_id ]._last_action_id + 1;
            _matches[ match_id ]._last_action_id = actionId;
            _entities[ match_id ][ target_ids[i] ] = target_eps[i];

            _matchActions[ match_id ][ actionId ] = Action({
                _player: msg.sender,
                _block: block.number,
                _performer_id: performer_id,
                _type: action_type[i],
                _coordinates: coordinates[i],
                _target_id: target_ids[i],
                _target_ep: target_eps[i]
            });
        }
    ...
    function haltMatch( uint256 match_id ) public virtual callerIsPlayer(match_id){
        ...
        require( _matches[ match_id ]._performers_order.length == PLAYERS_COUNT * DECK_SIZE , "Players cannot Halt the Match before starting" );

        uint256 lastActionId = _matches[ match_id ]._last_action_id;
    ...
    function surrendMatch( uint256 match_id ) public virtual callerIsPlayer(match_id){
        ...
        require( _matches[ match_id ]._performers_order.length == PLAYERS_COUNT * DECK_SIZE , "Players cannot surrender Match before it has started");
    ...
    function endMatchConditionsChecks( uint256 match_id ) public virtual callerIsPlayer(match_id) {
        ...
        require( _matches[ match_id ]._performers_order.length == PLAYERS_COUNT * DECK_SIZE , "Players cannot end Match before it has started");
        require( _matchActions[ match_id ][ lastActionId ]._player != msg.sender
            || _matchActions[ match_id ][ lastActionId ]._block + _matches[ match_id ]._block_to_respond >= block.number,
            "Cannot call this checks before blocks to respond are passed");

        for( uint256 i = 0; i < _matches[ match_id ]._active_players.length; i++){
            uint256 idx = 0;
            for( uint256 j = 0; j < _matches[ match_id ]._players.length; j++) {
                if( _matches[ match_id ]._players[j] == _matches[ match_id ]._active_players[i] ) idx = j;
            }

            uint256 nexus_id = _Decks.getNexusId( _matches[ match_id ]._decks[ idx ] );
            if( _entities[ match_id ][ nexus_id ] == uint16(0) ) _loseMatch( match_id, _matches[ match_id ]._players[ idx ] );
            ...
            for( uint256 j = 0; j < DECK_SIZE; j++){
                allMechaBroken = allMechaBroken && _entities[ match_id ][ mechas_ids[j] ] == uint16(0);
            }
    ...
    function _loseMatch( uint256 match_id, address player ) internal virtual {
        ...
        _matches[ match_id ]._active_players = Arrays.popAddress( _matches[ match_id ]._active_players, player );
        if( _matches[ match_id ]._active_players.length == 1 ) _endMatch( match_id, _matches[ match_id ]._active_players[0] );
    }
    ...
    function _endMatch( uint256 match_id, address winner ) internal virtual {
        ...
        _rewardsDistribution( match_id, _matches[ match_id ]._arena_id, winner );
    ...
    function _getNextPerformerId( uint256 match_id ) internal view virtual returns ( uint256 ){
        ...
        uint256 lastActionId = _matches[ match_id ]._last_action_id;
        if ( lastActionId == 0 ) nextPerformerId = _matches[ match_id ]._performers_order[0];

        for (uint256 i = 0; i < _matches[ match_id ]._performers_order.length; i++) {
            if ( _matches[ match_id ]._performers_order[i] == _matchActions[ match_id ][ lastActionId ]._performer_id ){
                if ( i == _matches[ match_id ]._performers_order.length - 1 ) nextPerformerId = _matches[ match_id ]._performers_order[ 0 ];
                else nextPerformerId = _matches[ match_id ]._performers_order[ i + 1 ];
                break;
            }
        }
    ...
    function _rewardsDistribution( uint256 match_id, uint256 arena_id, address winner ) internal {
        ...
        uint256 amount = _matches[ match_id ]._stake_amount * PLAYERS_COUNT;
    ...
    function _updateBreakingLoads( uint256 match_id ) internal {
        ...
        for( uint256 i = 0; i < PLAYERS_COUNT; i++){
            ...
            uint256 mechaErosion = ( MECHA_DEGRADATION + _matches[ match_id ]._last_action_id ) / degradationReduction;
            ...
            for( uint256 j = 0; j < DECK_SIZE; j++) {
                ...
                if( _entities[ match_id ][ mechas_ids[j] ] == uint16(0) )
