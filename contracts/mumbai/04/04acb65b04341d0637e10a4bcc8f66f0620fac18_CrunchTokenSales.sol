...
contract CrunchTokenSales {
    ...
    constructor( address _crunchtoken, address _usdt, address _usdc, uint256 _price, uint256 _fee, uint256 mFee, address multiSig) {
        owner =  _msgSender();
    ...
    function buy(uint256 _tokenAmount, address purchaseWith) public {
        ...
        require(IERC20(purchaseWith).transfer(multiSigContractAddress, fees));
    ...
    function buyWithMatic() external payable{
        uint256 p = Price;
        uint256 reward = msg.value / p;
        uint256 addressBalance = IERC20(crunchToken).balanceOf(address(this));
        require(addressBalance >= reward, "CRUNCH NETWORK: Contract Balance too low for amount provided");
        require(checkSaleStatus == true, "CRUNCH NETWORK: SALE HAS ENDED.");
        totalAmountSoldOut += msg.value;
        payable(multiSigContractAddress).transfer(maticFee);
        IERC20(crunchToken).transfer(_msgSender(), reward);
