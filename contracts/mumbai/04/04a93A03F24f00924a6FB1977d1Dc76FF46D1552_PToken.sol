...
contract PToken is VersionedInitializable, IPToken {
  ...
  function withdraw(address user, uint256 amount) external virtual override onlyPool {
    ...
    if(_userState[user].supplyBalance - withdrawAmount < WadRayMath.RAY) withdrawAmount = _userState[user].supplyBalance;
  ...
  function liquidate(address user) external virtual override onlyPool {
    ...
    IERC20(_underlyingAsset).safeTransfer(_addressesProvider.getTradingWallet(), _userState[user].supplyBalance / WadRayMath.RAY);
  ...
  function _updateInterestRate(address user) internal {
    if(_userState[user].supplyBalance > 0) {
      if(_userState[user].referralCode == 0) {
        ...
        _supplyTotalInterestAmount += _userState[user].supplyBalance.rayMul(_userState[user].supplyInterestRate);
      }
      else {
        _userState[user].prxyInterestRate = POOL.getPrxyInterestRate(_underlyingAsset);
        _prxyTotalInterestAmount += _userState[user].supplyBalance.rayMul(_userState[user].prxyInterestRate);
      }
  ...
  function _updateState(address user) internal {
    ...
    if(_userState[user].supplyBalance > 0) {
      ...
      if(_userState[user].referralCode == 0) {
        uint256 supplyInterestAmount = _userState[user].supplyBalance.rayMul(_userState[user].supplyInterestRate);
        _supplyTotalInterestAmount -= supplyInterestAmount;
        _userState[user].supplyBalance += supplyInterestAmount * dt;
      } else {
        uint256 prxyInterestAmount = _userState[user].supplyBalance.rayMul(_userState[user].prxyInterestRate);
