...
// ISkulls <- IERC721Enumerable <- IERC721 <- IERC165 ( ERC165 has implementation )
// IERC721Metadata <- IERC721 <- IERC165
contract HashSkulls is Context, Ownable, ERC165, ISkulls, IERC721Metadata {
    ...
    // ~14 days after SALE_START_TIMESTAMP
    ...
    // Mapping from token ID to whether the HashSkull's was minted before reveal
    mapping(uint256 => bool) private _mintedBeforeReveal;

    // Mapping from owner to operator approvals
    mapping(address => mapping(address => bool)) private _operatorApprovals;

    // Token name : set via constructor
    string private _name;

    // Token symbol : set via constructor
    string private _symbol;

    // Name change token address : set via constructor
    address public COIN_CONTRACT_ADDRESS;
    ...
    constructor(
        ...
        uint256 coinEmissionStartTimestamp
    ) {
        _name = Name;
        _symbol = Symbol;
        COIN_CONTRACT_ADDRESS = coinAddress;
        _hiddenMetadataUri = hiddenMetadataUri;
        SALE_START_TIMESTAMP = coinEmissionStartTimestamp;
    ...
    modifier mintCompliance(uint256 _mintAmount) {
        require(
            _mintAmount > 0 && _mintAmount <= getMaxMintAmountPerTx(),
            "Invalid mint amount!"
        );
        require(
            totalSupply() + _mintAmount <= MAX_NFT_SUPPLY,
            "Max supply exceeded!"
        );
        _;
    }
    ...
    modifier mintPriceCompliance(uint256 _mintAmount) {
        require(
            msg.value >= getPublicPrice() * _mintAmount,
            "Insufficient funds!"
        );
        _;
    }

    function setMaxMintAmountPerTx(uint256 max) public onlyOwner {
        _maxMintAmountPerTx = max;
    }

    function getMaxMintAmountPerTx() public view returns (uint256) {
        return _maxMintAmountPerTx;
    }
    ...
    /**
     * Functions related to whitelist
     */
        ...
        mintCompliance(_mintAmount)
    {
        ...
        require(
            whitelistClaimed[_msgSender()] + _mintAmount < getMaxMintAmountPerTx(),
            "NFT count exceeds per wallet limit!"
        );
        require(msg.value >=getWhitelistPrice().mul(_mintAmount) , "Insufficient funds!");
    ...
    function setWhitelistMintEnabled(bool _state) public onlyOwner {
        _whitelistMintEnabled = _state;
    }

    function isWhitelistMintEnabled() public view returns (bool) {
        return _whitelistMintEnabled;
    }
    function getWhitelistClaimedCount(address _address) public view returns (uint256) {
    ...
    function mintNFT(uint256 numberOfNfts) public payable {
        ...
        require(
            getPublicPrice().mul(numberOfNfts) == msg.value,
    ...
    function giveaways(uint256 numberOfNfts, address winnerAddress)
        public
        onlyOwner
    {
        require(
    ...
    /**
     * @dev Changes the name for HashSkull's tokenId
     */
    function changeName(uint256 tokenId, string memory newName) public {
        ...
        ERC20Burnable(COIN_CONTRACT_ADDRESS).transferFrom(
            msg.sender,
            address(this),
            NAME_CHANGE_PRICE
        );
        ...
        ERC20Burnable(COIN_CONTRACT_ADDRESS).burn(NAME_CHANGE_PRICE);
    ...
    /**
     * @dev See {ISkull-tokensOfOwner}.
     * used for claiming the ECR20 coins for renaming NFT
     */
    ...
    {
        uint256[] memory tokensOwnedByOwner = new uint256[](
            _holderTokens[owner].length()
        );
        for (uint256 i = 0; i < _holderTokens[owner].length(); i++) {
            tokensOwnedByOwner[i] = _holderTokens[owner].at(i);
        }
        return tokensOwnedByOwner;
    }
    ...
    function getPublicPrice() public view returns (uint256) {
        ...
        if (_publicPrice == 0) {
            ...
            if (currentSupply >= 16381) {
                return 1000; // 16381 - 16383 1000 Matic
            } else if (currentSupply >= 16000) {
                return 300; // 16000 - 16380 300 Matic
            } else if (currentSupply >= 15000) {
                return 170; // 15000  - 15999 170 Matic
            } else if (currentSupply >= 11000) {
                return 90; // 11000 - 14999 90 Matic
            } else if (currentSupply >= 7000) {
                return 50; // 7000 - 10999 50 Matic
            } else if (currentSupply >= 3000) {
                return 30; // 3000 - 6999 30 Matic
            } else {
                return 20; // 0 - 2999 20 Matic
            }
        } else {
            return _publicPrice;
        }
...
interface ISkulls is IERC721Enumerable {
