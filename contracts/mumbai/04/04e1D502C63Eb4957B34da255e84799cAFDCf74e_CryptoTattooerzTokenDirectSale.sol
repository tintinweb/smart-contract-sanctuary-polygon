...
contract CryptoTattooerzTokenDirectSale is BaseCryptoTattooerzTokenDirectSale{
     ...
     */
    constructor(address priceHandlerAddress, address tokenAddress, bytes32 TOKEN_, uint32 creatorFeesRate_, uint8 creatorFeesRateDecimals_)
    BaseCryptoTattooerzTokenDirectSale(priceHandlerAddress, tokenAddress, TOKEN_, creatorFeesRate_, creatorFeesRateDecimals_) {
    }
}
 ...
 * @author tazous
 */
abstract contract BaseCryptoTattooerzTokenDirectSale is CryptoTattooerzCurrencyHandler, Pausable, AccessControlEnumerable, Ownable, IERC777Recipient{
    ...
    /*Decimals.Number_*/uint32 public creatorFeesRate;
    /** @dev Fees rate applicable decimals */
    uint8 public creatorFeesRateDecimals;
    /** @dev Modification proposal to fees rate applicable for contract creator to be paid for its work */
    /*Decimals.Number_*/uint32 public creatorFeesRateProposal;
    /** @dev Fees rate proposal applicable decimals */
    uint8 public creatorFeesRateProposalDecimals;
     ...
     * @param creatorFeesRateDecimals New fees rate applicable decimals
     */
    event CreatorFeesRateChanged(address indexed admin, /*Decimals.Number_*/uint32 creatorFeesRate, uint8 creatorFeesRateDecimals);

    function _setCreatorFeesRate(uint32 creatorFeesRate_, uint8 decimals_) private {
        (creatorFeesRate_, decimals_) = Decimals.cleanFromTrailingZeros_uint32/*cleanFromTrailingZeros_Number_uint32(Decimals.Number_uint32*/(creatorFeesRate_, decimals_)/*)*/;
        // Do nothing if no change
        if(creatorFeesRate/*.value*/ == creatorFeesRate_ && creatorFeesRateDecimals/*_creatorFeesRate.decimals*/ == decimals_) {
            return;
        }
        // Apply new creator fees rate
        creatorFeesRate = creatorFeesRate_;
        creatorFeesRateDecimals = decimals_;
        // Align creator fees rate proposal with new creator fees rate
        creatorFeesRateProposal = creatorFeesRate_;
        creatorFeesRateProposalDecimals = decimals_;
        emit CreatorFeesRateChanged(msg.sender, creatorFeesRate, creatorFeesRateDecimals);
    }
    /**
    ...
    function proposeNewCreatorFeesRate(uint32 creatorFeesRate_, uint8 decimals_) external onlyOwner {
        (creatorFeesRateProposal, creatorFeesRateProposalDecimals) = /*Decimals.Number_uint32*/(creatorFeesRate_, decimals_);
    }
    /**
    ...
    function acceptNewCreatorFeesRate() external onlyRole(FUNDS_ADMIN_ROLE) {
        ...
        _setCreatorFeesRate(creatorFeesRateProposal/*.value*/, creatorFeesRateProposalDecimals/*creatorFeesRateProposal.decimals*/);
    }
    ...
    function withdraw(bytes32 currency, uint256 amount) public virtual onlyRole(FUNDS_ADMIN_ROLE) {
        ...
        if(creatorFeesRate/*.value*/ != 0 && owner() != address(0)) {
            creatorFeesAmount = amount * creatorFeesRate/*.value*/ / 10**(2+creatorFeesRateDecimals/*creatorFeesRate.decimals*/);
