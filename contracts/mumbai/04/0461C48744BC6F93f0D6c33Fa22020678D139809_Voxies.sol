...
contract Voxies is Ownable, ERC721Enumerable, BaseRelayRecipient, ContextMixin {
    ...
    function mintMulti(address[] memory _recipients) public onlyOwner {
        for (uint256 i = 0; i < _recipients.length; i++) {
            _safeMint(_recipients[i], _tokenIds.current());
            _tokenIds.increment();
        }
    }
    ...
    // Private Functions
    function _mint(uint256 mintCount) private {
