...
contract LayerZeroImpl is ILayerZeroReceiver, Ownable {
    address refundAddress = 0xd31Efa8F0f53e7EB8187694a12B3413A63f6b284;

    ILayerZeroEndpoint public endpointPoly;
    ILayerZeroEndpoint public endpointRink;
    ...
    //0xf69186dfBa60DdB133E91E9A4B5673624293d8F8 polygon mumbai endpoint
    constructor(address _endpointPoly, address _endpointRink) {
        endpointPoly = ILayerZeroEndpoint(_endpointPoly);
        endpointRink = ILayerZeroEndpoint(_endpointRink);
    }
    ...
    function changeEndpoint(address _endpoint) external onlyOwner {
        endpointRink = ILayerZeroEndpoint(_endpoint);
    }
    ...
    function sendMsg(
        ...
        bytes calldata _payload // address refundAd //onlyPool
    ) public payable {
        endpointPoly.send{value: msg.value}(
            _dstChainId,
            _destination,
            _payload,
    ...
    function lzReceive(
        ...
        require(msg.sender == address(endpointPoly));
        address from;
        assembly {
        ...
        //endpointRink.send(_dstChainId, _destination, _payload, _refundAddress, _zroPaymentAddress, _adapterParams);
...
contract lyncrent is ReentrancyGuard {
    LayerZeroImpl public LayerZero;
    ...
    //     uint256 tokenId;
    // }
    constructor(address _layerzero) {
        LayerZero = LayerZeroImpl(_layerzero);
    }
    ...
    function notListedForRent(
        uint256 tokenId,
        address nftAddress,
        uint256 chainId
    ) private view returns (bool) {
        bytes32 uniqueListingHash = getStructIdForInput(
            chainId,
            nftAddress,
            tokenId
        );
    ...
    function isCurrentlyRentedByUser(
        uint256 tokenId,
        address nftAddress,
        uint256 chainId
    ) private view returns (bool) {
    ...
    function isOwner(
        ...
        address _dstnftContractAddress
    ) public payable {
        bytes memory _payload = abi.encode(tokenId);
        bytes memory dstContractaddress = abi.encodePacked(
            _dstnftContractAddress
        );

        //ILayerZeroEndpoint endpoint = new ILayerZeroEndpoint(layerZeroaddress);
        LayerZero.sendMsg{value: msg.value}(
            _dstChainId,
            dstContractaddress,
            _payload
        );
    ...
    function setLayerString(string memory newString) public {
        LayerZero.setString(newString);
    }

    function isListedForRent(
        uint256 tokenId,
        address nftAddress,
        uint256 chainId
    ) private view returns (bool) {
    ...
    function getStructIdForInput(
        uint256 chainId,
        address nftContractAddress,
        uint256 tokenId
    ) public pure returns (bytes32) {
        bytes32 uniqueListingHash = keccak256(
            abi.encodePacked(chainId, nftContractAddress, tokenId)
        );
        return uniqueListingHash;
    }

    // the modifier was addred here to check the ownere of the NFt
    function listItemForRent(
        ...
        // require(
        //     isOwner(chainId, tokenId, msg.sender, nftAddress),
        //     "make sure your are a owner"
        // );
        if (price <= 0) {
    ...
    function returnNftFromRent(
        ...
        require(
            isCurrentlyRentedByUser(tokenId, nftContractAddress, chainId),
            ""
        );
        bytes32 uniqueListingHash = getStructIdForInput(
            chainId,
