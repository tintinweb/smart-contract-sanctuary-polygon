...
contract PolygonHeartbreakBearsNFTStaking is Initializable, PausableUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function initialize(address _nftAddress, address _tokenAddress) initializer public {
        __Ownable_init();
    ...
    function stakeBatch(uint256[] memory _tokenIds) public whenNotPaused {
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            require(NFT.ownerOf(_tokenIds[i]) == msg.sender , "Token must be owned by staker");
            _values[i] = 1;
