 ...
 */
	...
	struct NFTOffer {
		...
		address currency;
		address requestor;
		uint256 status; // 0-active, 1-canceled, 2-rejected, 3-claimed
		uint256 expiryTime;
	}
 ...
 */
	 ...
	 */
	function cancelListingAndClaimToken(
		uint256 _listingId
	) external virtual onlyValidListingId(_listingId) onlyActiveListing(_listingId) nonReentrant {
	...
	) external virtual onlyValidBidId(_bidId) {
		...
		returns (uint256 offerId)
	{
		...
		require(_expiryTime > 0, 'Market: INVALID_EXPIRY');
	...
	function acceptOffer(uint256 _offerId) external onlyValidOfferId(_offerId) {
		...
		require(block.timestamp <= offer.expiryTime, 'Market: OFFER_EXPIRED');
 ...
 */
	...
	/**
	 * @notice Returns the offer ids.
	 */
	function getNftOffersIdList(
		address _nftAddress,
		uint256 _tokenType,
		address _userAddress,
		uint256 nftId
	) external view virtual returns (uint256[] memory nftOfferIds) {
		uint256 noOfOffers = nftOfferCount[_nftAddress][_tokenType][_userAddress][nftId];

		if (noOfOffers > 0) {
			nftOfferIds = nftOffersList[_nftAddress][_tokenType][_userAddress][nftId];
		}
	}
