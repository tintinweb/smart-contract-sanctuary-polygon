...
*/
contract UniFlip is VRFConsumerBaseV2 {
    ...
    */
    ...
    @param _entryUnivrs The entry fee cost that all entries will pay when they join the game. The creator pays this when they create the game.
    ...
    */
    function createGame(
        ...
        newGame.entryUnivrs = _entryUnivrs;
        ...
        _proceedPayment(address(this), msg.sender, _entryUnivrs);
        totalEntryFees[msg.sender] = totalEntryFees[msg.sender] + _entryUnivrs;
    }
    ...
    */
    function joinGame(uint256 gameID) external {
        ...
        _proceedPayment(address(this), msg.sender, gameToJoin.entryUnivrs);
        totalEntryFees[msg.sender] =
            totalEntryFees[msg.sender] +
            gameToJoin.entryUnivrs;
    ...
    function cancelGame(uint256 gameID) external {
        ...
        for (uint256 i = 0; i < gamesMapping[gameID].addresses.length; i++) {
            paymentToken.safeTransfer(
                ...
                gamesMapping[gameID].entryUnivrs
            );
    ...
    function _completeGame(uint256 _gameIndex, uint256 _randomWord) private {
        ...
        // take
        uint256 gameFeePerEntrant = 0;
        if (game.entryUnivrs < 201) {
            // 25-100 UNIVRS takes 10 from each entrant
            gameFeePerEntrant = 10;
        } else if (game.entryUnivrs < 501) {
            // 201-500 UNIVRS takes 20 from each entrant
            gameFeePerEntrant = 20;
        } else if (game.entryUnivrs < 1001) {
            // 501-1000 UNIVRS takes 50 from each entrant
            gameFeePerEntrant = 50;
        } else if (game.entryUnivrs < 5001) {
            // 1001-5000 UNIVRS takes 100 from each entrant
            gameFeePerEntrant = 100;
        } else {
            // anything beyond a 5000 univrs entry fee just takes 200 from each person
            gameFeePerEntrant = 200;
        }

        game.prize =
            (game.entryUnivrs - gameFeePerEntrant) *
