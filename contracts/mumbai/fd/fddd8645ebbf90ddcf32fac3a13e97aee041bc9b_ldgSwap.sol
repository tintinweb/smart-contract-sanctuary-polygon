...
contract ldgSwap is Ownable {
    ...
    constructor(address _USDAddress, address _CAPSAddress, address _CAPSATokenAddress, address _LDGTokenAddress, address _fundWallet, address _feeWallet) {
    ...
    function takeFee() external onlyOwner { // isTokenSet
        ...
        if (lastTimeFee == 0)
        {
            feeTake = (balances_capsa * fee / 100) / 360;
            balances_capsa -= feeTake;
            token_capsa.transfer(feeWallet, feeTake); // TRANSFER FEE AMOUNT TO FEE WALLET
    ...
    // function setCapsaContract(address _contract) external onlyOwner { // SET CAPSA CONTRACT
    //     capsa = contractCapsa(_contract);
    //     CAPSAddress = address(_contract);
    // }

    // function setCapsAddress(address _token) external onlyOwner { // SET CAPSA ADDRESS
    //     token_capsa = IERC20(_token);
    //     tokenCapsAddress = address(_token);
    // }
    ...
    function setFee(uint8 _fee) external onlyOwner { // SET FEE ---> AMELIORER LA FONCTION FEE QUI GERE QUE LES NOMBRES SANS FLOAT
        require(_fee < 100, "You can't have more than 100% fee");
        fee = _fee;
    }
    ...
    function transferTokenToFundWallet(uint256 amount) external onlyOwner isTokenSet { // SEND AMOUNT TOKEN ON FUND WALLET
        token_capsa.transferFrom(address(this), fundWallet, amount); // TRANSFER CAPSA TOKEN IN FUND WALLET
    }

    // FUNCTION A FAIRE POUR RECUPERER LES FONDS ET LES ENREGISTRER
    ...
    function deposit(uint256 amount) external isTokenSet { // DEPOSIT X TOKENS USD
        ...
        require(token_usd.allowance(address(this), CAPSAddress) >= amount, "The contract was incorrectly approved");
        uint256 actualPrice = myPrice(); // Price of LDG

        bool res = token_usd.transferFrom(msg.sender, address(this), amount); // TRANSFER USD VS TOKEN LDG
        ...
        balances_capsa += capsa.getAmountCAPSA(amount); // AMOUNT OF CAPSA ADDED TO THE BALANCES OF CAPSA
    }
    ...
    function withdraw(uint256 amount) external isTokenSet {
        ...
        require(token_capsa.allowance(address(this), CAPSAddress) >= amountCapsa, "The contract was incorrectly approved");
