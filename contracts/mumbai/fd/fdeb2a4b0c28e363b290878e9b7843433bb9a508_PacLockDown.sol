...
// File: contracts/PacLockDown.sol
...
contract PacLockDown is Ownable, AccessControl {
  ...
  function isRole(bytes32 role, address _address) public view returns (bool)
  {
  ...
  function addTeamMember(address _address) public onlyRole(TEAM_MEMBER)
  {
    grantRole(TEAM_MEMBER, _address);
    emit TeamMemberAdded(msg.sender, _address);
  }

  function addTeamMembers(address[] memory _addresses) public onlyRole(TEAM_MEMBER)
  {
  ...
  function removeTeamMember(address _address) public onlyRole(TEAM_MEMBER)
  {
  ...
  function blacklistWalletAddress(address _address) public onlyRole(TEAM_MEMBER)
  {
    grantRole(BLACKLISTED, _address);
    emit BlacklistedWalletAdded(msg.sender, _address);
  }

  function blacklistAddresses(address[] memory _addresses) public onlyRole(TEAM_MEMBER)
  {
    for (uint256 i; i < _addresses.length; i++) {
  ...
  function removeBlacklistedWallet(address _address) public onlyRole(TEAM_MEMBER)
  {
