...
contract GangNFTCreator is ERC721, Ownable, ReentrancyGuard {
    ...
    /**
     @notice Get an array with a boolean flag indicating if the given wallet holds any token from each club.
     @param wallet Address of the wallet to check holding status.
     @return tokenHolders A boolean array where each element represents if the wallet holds any token from the respective club.
     */
    function getAllTokenHolders(
        address wallet
    ) public view returns (bool[] memory) {
        bool[] memory tokenHolders = new bool[](totalSupply());
        for (uint256 i = 0; i < totalSupply(); i++) {
            GangNFT gangNFT = GangNFT(allClubs[i]);
            uint256 balance = gangNFT.balanceOf(wallet);
            tokenHolders[i] = balance > 0;
        }
        return tokenHolders;
    }
