...
contract PrivatePlacementMarket is OwnablePausable, ReentrancyGuardUpgradeable{
    ...
    function initialize(
        ...
        address tdex) public initializer {
            ...
            _stockNftIds.add(99*usdtPrecision/10);
            _stockNftIds.add(100*usdtPrecision);
            _stockNftIds.add(2200*usdtPrecision);
            _stockNftIds.add(12000*usdtPrecision);
            _stockNftIds.add(39000*usdtPrecision);
    ...
    function buy(uint256 index) external whenNotPaused {
        ...
        if (_bags[index].ttValue > 0) {
            ...
            ttAmount = _bags[index].ttValue*10**_ttDecimals*TT_PRICE_PRECISION/ttPrice/10**_usdtDecimals;
    ...
    function getTokens() external view returns (
        ...
        address ttAddress,
        uint256 ttDecimals,
        uint256 ttPricePrecision
    ) {
        return (_usdtAddress, _usdtDecimals, _ttAddress, _ttDecimals, TT_PRICE_PRECISION);
    }

    function getTTPrice() external view returns(uint256) {
        return _tdex.getPrice(_ttAddress);
    }
