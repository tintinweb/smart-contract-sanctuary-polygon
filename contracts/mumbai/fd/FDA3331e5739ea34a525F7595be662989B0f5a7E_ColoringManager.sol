...
contract ColoringManager is Ownable {
    ...
    function setColorableRegistry(address colorableRegistry_) public onlyOwner {
        require(
    ...
    function colorInCanvas(
        ...
        require(
            ...
            "ColoringManager#colorInCanvas: PARAM_LENGTH_MISMATCH"
        );

        require(
            address(colorableRegistry) != address(0x0), "ColoringManger#colorInCanvas: COLORABLE_REGISTRY_NOT_SET"
        );
        ...
        uint256 _numCrayonIds = crayonStorage.currentCrayonId() + 1; // crayons are indexed from 0
        ...
        for (uint256 i = 0; i < _loopThrough; i++) {
            ...
            require(_colorId < _numCrayonIds, "ColoringManager#colorInCanvas: INVALID COLOR");
            numColorsNeeded[_colorId]++;
        }

        uint256[] memory _crayonIds = new uint256[](_numCrayonIds);

        for (uint256 c = 0; c < _numCrayonIds; c++) { // c is colorId
        ...
        console.logBytes(abi.encode(_crayonIds));
        console.logBytes(abi.encode(numColorsNeeded));

        crayonStorage.burnBatch(msg.sender, _crayonIds, numColorsNeeded);
