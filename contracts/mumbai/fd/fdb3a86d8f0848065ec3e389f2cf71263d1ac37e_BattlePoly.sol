...
contract BattlePoly {
    ...
    ERC1155Like items;
    uint256 public constant LOOTBOX_COMMON_ID = 1;
    uint256 public constant LOOTBOX_UNCOMMON_ID = 2;
    uint256 public constant LOOTBOX_RARE_ID = 3;
    uint256 public constant LOOTBOX_EPIC_ID = 4;
    uint256 public constant LOOTBOX_LEGENDARY_ID = 5;
    ...
    function _rollForLoot(IDogewood.Doge2 memory doge_) internal returns (uint256) {
         ...
         *      12.00 * 0.01 + 2    vs  100%
         *      1200 * 0.01 + 200   vs 10_000
         *      1200 * 1 + 20_000   vs 1_000_000
         */
        uint discoveryRange_ = uint(statInfo[doge_.level][doge_.class][doge_.breed][0].nose) + 20_000;
        uint relevantDiscovery_ = vrf.getRandom("discovery loot") % 1_000_000;
        if(relevantDiscovery_ <= discoveryRange_) { // loot found
            ...
            else if(relevantBox_ <= legendPerc_+ epicPerc_ + 75_000) itemId_ = LOOTBOX_RARE_ID; // 7.5%
            else if(relevantBox_ <= legendPerc_+ epicPerc_ + 75_000 + 200_000) itemId_ = LOOTBOX_UNCOMMON_ID; // 20%
