...
contract StakingReward is OwnablePausable, ReentrancyGuardUpgradeable {
    ...
    function initialize(
        ...
        BASE_REWARD_PER_BLOCK = SECONDS_PER_BLOCK*PRECISION_FACTOR/365 days/100;
    ...
    function calculatePendingRewards(address staker) public view returns(uint256 rewardDebt, uint256 pendingRewards) {
        ...
        for (uint256 i; i < userOrders.length; i++) {
            pendingRewards += _calculatePendingReward(userOrders[i], levels, levelSize);
        }
    ...
    function _calculatePendingReward(
        ...
        if (lastRewardBlock >= levels[levelSize-1].startBlock) {
            pendingReward += __calculatePendingReward(lastRewardBlock, 
            ...
            for (uint256 j; j < levelSize-1; j++) {
                ...
                if (matches >= 1) {
                    pendingReward += __calculatePendingReward(matches == 1 ? lastRewardBlock : levels[j].startBlock, 
                                    userOrder.remainedAmount, levels[j].level, levels[j+1].startBlock);                       
                    matches = 2;
                }
            }
            pendingReward += __calculatePendingReward(levels[levelSize-1].startBlock, 
                                    userOrder.remainedAmount, levels[levelSize-1].level, endBlock);    
        }
    }
    ...
    function __calculatePendingReward(
        ...
        if (rewardFromBlock >= apyLevels[apySize-1].startBlock) {
            uint256 multiplier = _getMultiplier(rewardFromBlock, block.number, endBlock);
            ...
            for (uint256 i; i < apySize-1; i++) {
                ...
                if (matches >= 1) {
                    uint256 multiplier = _getMultiplier(matches == 1 ? rewardFromBlock : apyLevels[i].startBlock, apyLevels[i+1].startBlock, endBlock);
