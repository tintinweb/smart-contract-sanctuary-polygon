...
contract Chiky is ERC1155, Ownable, ReentrancyGuard, IERC1155Receiver{
  ...
  event ItemBought(
   ...
   // returns the total amount of NFTs minted
        ...
        function airdropNFT(address[] memory users, uint256[] memory amounts, string[] memory uris)
        external {
            require(isAdmin[msg.sender],"Access Denied");
            require(users.length == amounts.length &&amounts.length == uris.length,"Length mismatch");
            for(uint256 i =0; i< users.length; i++){
                uint256 nftId = tokenCount.current();
               _uri[nftId] = uris[i];
               _mint(users[i],nftId,amounts[i],"");
               tokenCount.increment();
               emit Minted(nftId, amounts[i], users[i]);
            }
        ...
        function updateAdmin(address user, bool admin) external onlyOwner{
