...
contract SocialBlocks is AccountCreation("Social Blocks", "$SB"), Ownable, Verify {
    ...
    constructor(IMintable _rewardToken, address _admin) {
    ...
    function changePostStatus(uint256 _postId, uint8 _status) external {
        ...
        require(isPostIdValid(_postId), "postId is not valid");
    ...
    external {
        ...
        require(_postId > 0 && isPostIdValid(_postId), "Invalid postId");
    ...
    function isPostIdValid(uint256 _postId) public view returns (bool) {
        address user = msg.sender;
        uint256 length = userPostIds[user].length;
        for(uint256 pid = 0; pid < length; pid++) {
            if(userPostIds[user][pid] == _postId) {
                return true;
            }
        }
        return false;
