...
contract PolicyManager_Covest is Initializable, UUPSUpgradeable, ReentrancyGuardUpgradeable {

    address public nameRegistry;

    mapping(uint256 => bool) private _timestampRepeaterBuyer;
    mapping(uint256 => bool) private _timestampRepeaterRedeem;
    ...
    function isBlacklistOfUser(address _user) public view returns (bool) {
        return PDS.isBlacklistOfUser(_user);
    }
    ...
    function isBlacklistAssetsOfUser(address _user, address _asset)
        ...
        returns (bool)
    {
        return PDS.isBlacklistAssetsOfUser(_user, _asset);
    }

    function exist(string memory _policyId) public view returns (bool) {
        return PDS.exist(_policyId);
    }
    ...
    function buyPolicyInternal(
        ...
        for (uint256 i = 0; i < decodedData.pricing.length; i++) {
            require(
                _pricing[i] == decodedData.pricing[i],
                "pricing Equal."
            );
        }
        require(decodedData.expiresAt > block.timestamp, "Time has expired");
        require(
            timestampBuyer(decodedData.generatedAt) == false,
            "This time already used."
        );
        _timestampRepeaterBuyer[decodedData.generatedAt] = true;
    ...
    function redeemPolicyInternal(
        ...
        bytes memory _data
    ) private returns (bool isSuccess) {
        ...
        require(decodedData.expiresAt > block.timestamp, "Time has expired");
        require(
            timestampRedeem(decodedData.generatedAt) == false,
            "This time already used."
        );

        _timestampRepeaterRedeem[decodedData.generatedAt] = true;
    ...
    function buyPolicyWillIssue(string memory _policyId,address _asset,uint256[] memory _pricing,address _userIsRefer) private returns(bool) {
        (uint _valueSub,,uint _percentSub,string memory _orgSub) = SUB.checkSubsidized(msg.sender,_asset,_pricing[0]*10**_pricing[2],poolId);
        bytes memory _data = abi.encode(_policyId, msg.sender,_asset,_pricing,_userIsRefer,_valueSub,_percentSub,_orgSub);
        PDS.issuePolicy(_data);
        return true;
    }
    ...
    function buyPolicy(
        ...
        // under backend ทำ ca ยังไม่้ตองเพราะยังไม่รู้ใครอนุมัติขอเงินหรือเคลม
        IERC20 _ERC20 = IERC20(_asset);
        INameRegistry.InitStructerParams memory _DMR = dataNameRegistry;
        require(
            RM.riskCoefficientOfUser(msg.sender) > 0 && RM.riskCoefficientOfUser(msg.sender) <= 10,
            "RiskLevel not Equal."
        );
        require(isBlacklistOfUser(msg.sender) != true, "Blacklist User.");
        require(
            isBlacklistAssetsOfUser(msg.sender, _asset) != true,
            "Blacklist Assets of User."
        );
        uint256[] memory _weightRequested = percentWeightIs();
        require(
            _weightRequested.length == 7,
            "Invaild Length of PercentWeight"
        );
        require(exist(_policyId) != true, "This _policyId exist.");

        {
            bool isDistributor = AR.isPolicyDistributor(poolId,msg.sender);

            if(isDistributor == false) {
                require(PDS.activePoliciesUser(msg.sender) == 0,"You have active policy.");
            }
        ...
        require(
            _ERC20.balanceOf(msg.sender) >= _value,
...

        {
            ...
            _batchTransfer(msg.sender,_asset,_paymentAddress,_value,_weightRequested);
            
            (bool isSuccess) = buyPolicyWillIssue(_policyId,_asset,_pricing,_referrer);
            require(isSuccess == true,"BuyPolicy Error");

            // percentall = [RF,UW,CA,PD,CP,PF,Investment Profit + Profit(CSM) + Claim Reserve]
            
            _callFundInFlow(CallFundInFlowParams(_policyId,msg.sender,_asset,_referrer,_paymentAddress,_value,_weightRequested));
    ...
    function _batchTransfer(address _caller,address _asset, address[7] memory _addr, uint256 _value, uint[] memory _weightRequested) internal {
        IERC20 _ERC20 = IERC20(_asset);

        require(_addr.length == _weightRequested.length,"Error Length BATCHTRANSFER");

        for(uint i = 0;i<_addr.length;i++){
            bool isMoreThanZero =  (_value * _weightRequested[i]) / 100 > 0 ? true : false;
            if(isMoreThanZero){
                _ERC20.transferFrom(
                    _caller,
                    _addr[i],
                    (_value * _weightRequested[i]) / 100
                );
            }
        }
    ...
    function _callFundInFlow(CallFundInFlowParams memory _params) private {
        require(_params.addr.length == _params.weightRequested.length,"Error Length CALLDATA");
      
        for(uint i = 0;i<_params.addr.length;i++) {
            if(i == 0) {
    ...
    function redeemPolicy(
        ...
        require(isBlacklistOfUser(msg.sender) != true, "Blacklist User.");
        require(
            isBlacklistAssetsOfUser(msg.sender, PolicyIdUserData.asset) != true,
            "Blacklist Assets of User."
        );
        require(
            (PolicyIdUserData.percent).length == 7,
        ...
        (,,uint256 redeemId) = PDS.redeemPolicy(_policyId,_percentRedeem);
 
        require(redeemId != 0, "ERROR caseId");
        require(PDS.whoIsRequest(PolicyIdUserData.asset,redeemId) == msg.sender,"ERROR THIS REDEEMID NOT YOUR OWN.");

        uint256 amountRequested = (PDS.amountRequest(PolicyIdUserData.asset, redeemId) /
            10**(decimals() - IERC20(PolicyIdUserData.asset).decimals()));
        uint256[] memory weightRequested = PDS.weightRequest(PolicyIdUserData.asset, redeemId);


        _callFundOutFlow(CallFundOutFlowParams(_policyId,msg.sender,PolicyIdUserData.asset,[
            _DMR.RF,
            _DMR.UW,
            _DMR.CA,
            _DMR.PD,
            ...
            _DMR.PF,
            _DMR.R
        ],amountRequested,weightRequested));
    ...
    function timestampBuyer(uint256 _timer) private view returns (bool) {
        return _timestampRepeaterBuyer[_timer];
    }

    function timestampRedeem(uint256 _timer) private view returns (bool) {
        return _timestampRepeaterRedeem[_timer];
    }

    function percentWeightIs() public view returns (uint256[] memory) {
        return PDS.percentWeightIs();
    }
}
...
interface IReserve {
    ...
    function caseIdCancel(uint256 _claimId) external;

    function caseIdStatusIs(uint256 _claimId) external view returns (uint8);
    ...
    function claimIdMetadata(uint256 _claimId)
        external
        view
        returns (CaseIdRequestData memory);

    function countCaseForRequest() external view returns (uint256);

    function existIpfs(string memory _ipfsHash) external view returns (bool);
    ...
    function getAllClaimStatus(uint8 _status)
        external
        view
        returns (CaseIdRequestData[] memory, uint256[] memory);
...
interface IRiskManager {
    ...
    function riskProfileUser(address _user)
        external
        view
        returns (bool approved, uint8 riskCoefficient);
