    ...
    ITrustedMintable
{
    ...
    // @inheritdoc IPIXLandmark
    function setCategories(uint256[] calldata _tokenIds, PIXCategory[] calldata _categories)
        external
        override
        onlyOwner
    {
        ...
        for (uint256 i; i < categoriesMem.length; i++)
            s_landCategories[tokenIdsMem[i]] = categoriesMem[i];
    }

    // @inheritdoc IPIXLandmark
    function setMaxSupplies(uint256[] calldata _tokenIds, uint256[] calldata _supplies)
        external
        override
        onlyOwner
    {
    ...
    function safeMint(
        ...
        require(
            totalSupply(_tokenId) + _amount <= s_maxSupplies[_tokenId],
            "Landmark: AMOUNT_TOO_BIG"
        );
    ...
    function batchMint(
        ...
        require(
            _walletAddresses.length == _tokenIds.length &&
        ...
        for (uint256 i; i < amountsMem.length; i++) {
            require(
                totalSupply(tokenIdsMem[i]) + amountsMem[i] <= s_maxSupplies[tokenIdsMem[i]],
                "Landmark: AMOUNT_TOO_BIG"
            );
    ...
    function mint(uint256 _amount, address _currency) external payable override {
        ...
        if (!IERC20(_currency).transferFrom(msg.sender, address(this), totalPrice))
            revert("LANDMARK: TRANSDER_FAILED");
    ...
    function trustedMint(
        ...
        if (totalSupply(_tokenId) + _amount > s_maxSupplies[_tokenId]) {
            return;
        }
    ...
    function trustedBatchMint(
        ...
        for (uint256 i; i < amountsMem.length; i++) {
            ...
            _safeMint(_to, tokenIdsMem[i], amountsMem[i]);
        }
    }
