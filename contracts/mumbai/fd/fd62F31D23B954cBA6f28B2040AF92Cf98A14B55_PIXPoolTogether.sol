...
import "../interfaces/IPIXPoolTogether.sol";

contract PIXPoolTogether is IPIXPoolTogether, VRFConsumerBaseV2, Ownable, Pausable {
    ...
    uint8[5] private POOL_PERCENT = [40, 27, 19, 10, 4];
    ...
    function addTickets(uint256 _tickets) external whenNotPaused {
        ...
        uint256 week = block.timestamp / SECONDS_PER_WEEK;
    ...
    function addOldTickets(uint256 _week, uint256 _tickets) external whenNotPaused {
        require(_tickets > 0, "tickets must be > 0");
        ...
        require(oldInfo.playerTickets[msg.sender] >= _tickets, "invalid tickets");
        oldInfo.playerTickets[msg.sender] -= _tickets;
        ...
        emit TicketsWithdrawn(_week, msg.sender, _tickets);
        emit TicketsAdded(week, msg.sender, _tickets, block.timestamp);
    }

    function withdrawOldTickets(uint256 _week, uint256 _tickets) external whenNotPaused {
        require(_tickets > 0, "tickets must be > 0");
        require(_drawn[_week], "not drawn");
        ...
        require(info.playerTickets[msg.sender] >= _tickets, "invalid tickets");
        info.playerTickets[msg.sender] -= _tickets;

        pixToken.safeTransfer(msg.sender, _tickets * ticketPrice);

        emit TicketsWithdrawn(_week, msg.sender, _tickets);
    }

    function claimPrize(uint256 _week, uint8 _place) external whenNotPaused {
        require(!prizeClaimed[_week][_place], "already claimed");
        require(_place < weeklyWinners[_week].length, "invalid place");

        address winner = weeklyWinners[_week][_place];
        require(msg.sender == winner, "can't claim other's prize");

        prizeClaimed[_week][_place] = true;

        WeeklyTicketInfo storage info = weeklyTickets[_week];
        uint256 prize = ((info.prize + weeklyPrize) * POOL_PERCENT[_place]) / 100;

        pixToken.safeTransferFrom(treasury, msg.sender, prize);

        emit PrizeClaimed(_week, _place, msg.sender, prize);
    }
    ...
    function draw(uint256 _week) external override whenNotPaused {
        ...
        require((_week + 1) * SECONDS_PER_WEEK < block.timestamp, "invalid week");
    ...
    function winnings(uint256 _week) external view returns (address[] memory, uint256[] memory) {
        WeeklyTicketInfo storage info = weeklyTickets[_week];
        uint256[] memory prizes = new uint256[](weeklyWinners[_week].length);
        for (uint256 i; i < weeklyWinners[_week].length; i++) {
            prizes[i] = ((info.prize + weeklyPrize) * POOL_PERCENT[i]) / 100;
        }
        return (weeklyWinners[_week], prizes);
    }
    ...
    function drawable(uint256 _week) external view override returns (bool) {
        ...
        if ((_week + 1) * SECONDS_PER_WEEK >= block.timestamp) {
            return false;
        }
