...
contract Organiser is AccessControl {
    enum Status {
        None,
        Pending,
        Approved,
    ...
    modifier onlyAdmin() {
        require(isAdminRole(msg.sender), "Restricted to admins");
        _;
    }
    ...
    modifier onlyAdminOrOganiserApprover() {
        require(isAdminRole(msg.sender) || isOrganiserApproverRole(msg.sender), "Restricted to admins or organisers approver");
        _;
    }

    // Check that the organiser does not exist in the `organisers` mapping
    modifier checkOrganiserNotExist(address organiser) {
        require(isOrganiserNotExist(organiser), "Organiser exist");
        _;
    }

    event ApplyForApprove(address indexed organiser);

    event ChangeOrganiserStatus(address indexed organiser, address indexed organiserApprover, Status oldStatus, Status newStatus);
    ...
    function isAdminRole(address account) public view returns(bool) {
        return hasRole(DEFAULT_ADMIN_ROLE, account);
    }

    // Create a bool check to see if a account address has the role organiser approver
    function isOrganiserApproverRole(address account) public view returns(bool) {
        return hasRole(ORGANISER_APPROVER_ROLE, account);
    }

    // Check if the organiser does not exist
    function isOrganiserNotExist(address organiser) public view returns(bool) {
    ...
    function addOrganiserApprover(address account) public onlyAdmin {
        grantRole(ORGANISER_APPROVER_ROLE, account);
    }

    // Apply for approve
    function applyForApprove(address organiser) checkOrganiserNotExist(organiser) public {
        organisers[organiser] = Status.Pending;

        emit ApplyForApprove(organiser);
    }

    // Approve organiser that could create events
    function approveOrganiser(address organiser) onlyAdminOrOganiserApprover public {
    ...
    function blockOrganiser(address organiser) onlyAdminOrOganiserApprover public {
    ...
    // Unblock organiser
    function unblockOrganiser(address organiser) onlyAdminOrOganiserApprover public {
