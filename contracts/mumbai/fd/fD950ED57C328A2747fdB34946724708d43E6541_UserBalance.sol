...
contract UserBalance is Ownable{
    ...
    event UserDeposit(address indexed _from, uint _value);
    ...
    receive() external payable{
        ...
        emit UserDeposit(_msgSender(), msg.value);
    }
    ...
    function returnBalanceToUser(address user, uint amount) external payable onlyOwner {
        require(amount <= balance[user], "Not enough balance");
        if (checkBalance()){
            balance[user] -= amount;
    ...
    function spendUsersBalance(address user, uint amount) external onlyOwner {
        require(amount <= balance[user], "Not enough balance");
        balance[user] -= amount;
    }
