...
// This was implemented by Chainlink but not available in their builds (afaik). 
// This just allows us to use VRF from an upgradeable contract.
// So all running examples do it this way, just copy the below file into your project.
import "./VRFConsumerBaseV2Upgradeable.sol";

// StoredData is all tableland info stored by the contract
struct StoredData {
    uint256 _tableId;
    string _table;
...
// This is used to track each random word that comes back from the VRF
struct Request {
    bool fulfilled; // whether the request has been successfully fulfilled
    bool exists; // whether a requestId exists
    bool used; // whether a requestId was used for a shuffle
    uint256[] results; // called "randomWords" in all docs
}

// VRFInfo are all the info needed to run the VRF methods
struct VRFInfo {
    ...
    // The smart contract conneciton for the VRF coordinator
    VRFCoordinatorV2Interface coordinator;

    mapping(uint256 => Request) requests; /* requestId --> requestStatus */
}
    ...
    VRFConsumerBaseV2Upgradeable
{
    ...
    // TokenIDs will be counted off for each new one minted
    ...
    function initialize(address vrfCoordinator, uint64 subscriptionId) public initializer {
        ...
        // Follow the ChainLink instructions for setting up a subscription
        vrf.subscriptionId = subscriptionId;
        // Alternatively, you can add programatic subscriptions here too. 
    }

    /**
     * @dev Will request a new random number from Chainlink.
     */
    function requestRandomWords()
        private
        returns (uint256 requestId)
    {
        ...
        vrf.requests[requestId] = Request({
    ...
    /**
     * @dev Will be called by Chainlink when the random number has been generated
     */
    function fulfillRandomWords(
        uint256 _requestId,
        uint256[] memory _randomWords
    ) internal override {
        require(vrf.requests[_requestId].exists, "request not found");
        vrf.requests[_requestId].fulfilled = true;
        vrf.requests[_requestId].results = _randomWords;
    }

    /**
     * @dev Public function to return info for any requestId.
     */
    function getRequestStatus(
        uint256 _requestId
    ) external view returns (Request memory request) {
        require(vrf.requests[_requestId].exists, "request not found");
        return vrf.requests[_requestId];
    }

    /**
     * @dev Public function to see how many times random numbers have been requested.
     */
    ...
    /**
     * @dev Show just one response by its index.
     */
    function showAtIndex(uint256 index) public view returns (Request memory result) {
        return vrf.requests[vrf.requestIds[index]];
    }

    /**
     * @dev Show just one response by its ID.
     */
    function show(uint256 requestId) public view returns (Request memory result) {
        return vrf.requests[requestId];
    }

    /**
     * @dev Show just the most recent result status.
     */
    function showLatest() public view returns (Request memory result) {
        return vrf.requests[vrf.requestIds[vrf.requestIds.length-1]];
    }
    ...
    function _insertNFT(
        ...
        /*
         * insert a single row of metadata
         */
    ...
    /**
     * @dev Called by anyone after reveal to just, reshuffle the tokenID to
     * metadata pairing. Your token ownership stays the same, but the metadata
     * it maps to will be randomly new.
     * If you just wanted a randomized reveal, you wouldn't call this function
     * ever again, and you certainly wouldn't make it public :D. 
     */
    function shuffle() public {
        require(vrf.requestIds.length > 0, "No random numbers requested.");
        require(vrf.requests[vrf.requestIds[0]].fulfilled == true, "Still awaiting random number from oracle.");

        uint256 index = 0;
        bool available = false;
        for (uint256 i = vrf.requestIds.length - 1; i >= 0; i -=1) {
            if (vrf.requests[vrf.requestIds[i]].exists == true && vrf.requests[vrf.requestIds[i]].used == false && vrf.requests[vrf.requestIds[i]].fulfilled == true) {
                index = i;
                available = true;
                break;
            }
        }
        require(available, "No random numbers left, request a new one.");

        uint256 rowCount = _tokenIdCounter.current();
        // 3 is just our collection size
        string memory offset = StringsUpgradeable.toString(
            SafeMathUpgradeable.mod(
                vrf.requests[vrf.requestIds[index]].results[0],
                rowCount // in demo, 3
            )
        );
        // example, if the offset = 10, we'll just shift the ids by that offset
        // we stored a "mint" order column to use for the shifting
        // UPDATE tablename SET id = CASE WHEN mint - offset >= 0 THEN mint - offset ELSE rowCount + mint - offset END;
        string memory sRowCount = StringsUpgradeable.toString(rowCount);
        stored._tableland.runSQL(
            address(this),
            stored._tableId,
            string.concat(
        ...
        // Queue the next random word for the next person's fun shuffling
        requestRandomWords();
    }

    /**
     * @dev Called by the owner to request the first random number used to issue 
     * tokenIds randomly across the collection. The random number is public, so 
     * you don't want the contract to know the result until you are ready to reveal. 
     */
    ...
    /**
     * @dev Called by the owner to set the metadata to tokenIDs for the first time
     * and reveal the artwork for each token. 
     */
    ...
    /**
     * @dev This token is worthless, so anyone can "buy" it from anyone else for free :D
     */
    ...
    /**
     * @dev Called by the owner to premint a bunch of tokens on init.
     */
    ...
    /**
     * @dev Called by the owner to premint one token on init.
     */
    ...
    /**
    * @dev Dynamically generate the metadata payload for any token depending on if
    * it's pre-reveal or revealed.
    */
    function tokenURI(
        uint256 tokenId
        ...
        if (stored.reveal) {
            return string.concat(
                "https://testnet.tableland.network/query?unwrap=true&s=SELECT%20*%20FROM%20",stored._table,"%20WHERE%20id=",
                strTokenId
            );
        } 
    ...
    /**
     * @dev just a helpful function to know the metadata table created by the contract
     */
