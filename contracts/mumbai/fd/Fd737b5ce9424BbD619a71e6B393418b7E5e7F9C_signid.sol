...
contract signid is Ownable, ERC721, ERC721URIStorage, ERC721Enumerable {
      using strings for *; 
   string private info;
    struct Metadata {
        uint256 id;
    ...
    function mint(uint256 _id, string memory title) internal {
        uint256 listingPrice = 1 ether;

        if (bytes(title).length >= 1 && bytes(title).length <= 3) {
            listingPrice = 100 ether;
        } else if (bytes(title).length == 4) {
            listingPrice = 50 ether;
        } else if (bytes(title).length == 5) {
            listingPrice = 10 ether;
        } else if (bytes(title).length >= 6 && bytes(title).length <= 10) {
            listingPrice = 1 ether;
        } else if (bytes(title).length >= 11 && bytes(title).length <= 12) {
            listingPrice = 0.5 ether;
        } else {
            listingPrice = 0.10 ether;
        }
        ...
        require(!_exists(_id), "Domain ID already assigned to others.");
        ...
        uint256 tokenId = _id;
        id_to_date[tokenId] = Metadata(_id, _toLowerCase(title));
        id_to_title[_toLowerCase(title)] = Metadata(_id, _toLowerCase(title));
        _safeMint(msg.sender, tokenId);
        _setTokenURI(tokenId, "https://signid.net/info.php");
    }
    ...
    function getID(string memory title) public view returns (string memory) {
     ...
     function getiid(string memory title) internal view returns (uint256) {
        Metadata memory tt = id_to_title[_toLowerCase(title)];
        require(_exists(tt.id), "Domain not minted");
        return tt.id;
    }
    ...
    function claim(uint256 tokenId, string calldata title) external payable {
        mint(tokenId, _toLowerCase(title));
       ...
       string memory primary=subdomain(_toLowerCase(title),2);
            ...
            {
               uint256 iid=getiid(primary);
               payable(getOwner(iid)).transfer(msg.value/2);
    ...
    constructor() ERC721("signid", "SIGNID") {
        // mint(0, "");
    }

  
 function subdomain(string memory title,uint pos) internal
        pure
        returns  (string memory) {
