...
import "./BetVerseToken.sol";
import '@openzeppelin/contracts/access/Ownable.sol';
import "@openzeppelin/contracts/token/ERC20/ERC20.sol";
import "@openzeppelin/contracts/utils/math/SafeMath.sol";

contract BetVerseTokenATMICO is ERC20, ReentrancyGuard, Ownable {
    ...
    constructor(
        ...
        token = BetVerseToken(_tokenAddress);
    ...
    function buy(address to, uint256 daiAmount)
        external
        icoActive
        nonReentrant
        hasTokenBalance
    ...
    {
        uint256 tokenAmount = getTokenAmountToSell(daiAmount);

        require(tokenAmount > 0, "amount cannot be 0");

        require(
            tokenBalance() > 0 && tokenBalance() >= tokenAmount,
            "The ICO has no tokens available to sell anymore."
        );

        transferTokenToLockedAddresses(to, tokenAmount);

        return (tokenBalance(), tokenAmount, price);
    }

    function getTokenAmountToSell(uint256 daiAmount)
        public
        view
        returns (uint256)
    {
        return daiAmount.div(price) * 10**18;
    }

    function hasEnoughTokensToSell(uint256 daiAmount)
        public
        view
        returns (
            bool,
    ...
    {
        return (
            tokenBalance() >= getTokenAmountToSell(daiAmount),
            tokenBalance(),
            getTokenAmountToSell(daiAmount)
        );
    }

    function getLockedAddresses(address investor)
        public
        view
    ...
    function transferTokenToLockedAddresses(address to, uint256 amount) public {
        address[] memory lockedAddresses;
        sales[to] = Sale(to, amount.div(term), lockedAddresses);
        uint256 currentDate = block.timestamp;
        for (uint256 i = 1; i <= term; i++) {
            currentDate = currentDate.add(365 days); // It will start releasing it in one year
            BetVerseTokenTimelock timeLockContract = new BetVerseTokenTimelock();
            timeLockContract.initialize(token, to, currentDate);
    ...
    modifier icoActive() {
        require(end > 0 && (end >= block.timestamp), "ICO must be active");
        _;
    }
    ...
    modifier notInvesting() {
        require(
            isAlreadyInvesting() == false,
            "You can't participate of the ICO more than one time."
        );
        _;
    }
}
...
contract BetVerseTokenTimelock is TokenTimelockUpgradeable, OwnableUpgradeable {
    mapping(address => bool) public AllowedToClaim;

    function setAllowedToClaim(address _beneficiary, bool value) external onlyOwner {
        AllowedToClaim[_beneficiary] = value;
    }
    ...
    function release() public override {
        ...
        require(AllowedToClaim[beneficiary()], 'The beneficiary must be allowed to claim');
...
import './BetVerseTokenTimelock.sol';
import './Interest.sol';
import '@openzeppelin/contracts/token/ERC20/IERC20.sol';
import '@openzeppelin/contracts/utils/math/SafeMath.sol';

contract BetVerseToken is ERC20Upgradeable, OwnableUpgradeable, Interest, ReentrancyGuardUpgradeable {
    using SafeMath for uint256;
    uint256 public ratePerYearInWei;
    uint256 public calculateRatePerYearInRAY;
    uint256 public startDate;
    uint256 public stakingDuration;
    uint256 public stakingEndDate;
    uint256 public maxManualMintable;
    uint256 public cap;
    address[] public forbiddenToStake;
    address[] public burnableAddresses;
    ...
    function initialize(
        ...
        uint256 calculateRatePerYearInRAY_,
        uint256 maxManualMintable_,
        ...
        calculateRatePerYearInRAY = calculateRatePerYearInRAY_;
        cap = cap_;
        maxManualMintable = maxManualMintable_;
    ...
    function mint(address _account, uint256 _amount) external onlyOwner maxManualMintableNotReached nonReentrant {
        _mint(_account, _amount);
        emit tokenMintedSuccess(_account, _amount, block.timestamp);
    }

    function burn(address _account, uint256 _amount) external onlyOwner isAllowedToBurn {
        _burn(_account, _amount);
        emit tokenBurnedSuccess(_account, _amount, block.timestamp);
    }

    function stake(uint256 amount_) external payable isAllowedToStake isStakingActive isNotEther nonReentrant {
        require(amount_ > 0, 'The deposit amount must be greather than zero');

        _transfer(msg.sender, address(this), amount_);

        createDeposit(amount_, block.timestamp);
    ...
    function unstake() external nonReentrant {
        ...
        for (uint256 i = 0; i < deposits.length; i++) {
            if (isStaking(deposits[i])) {
                uint256 profit = getProfits(deposits[i]);

                if (profit > 0) _mint(msg.sender, profit);

                _transfer(address(this), msg.sender, deposits[i].amount);

                totalStaked = totalStaked.sub(deposits[i].amount);

                deleteDeposit(deposits[i], i);

                found = true;

                emit unstakeSuccess(msg.sender, deposits[i].amount, block.timestamp);

                break;
            }
        }

        if (!found) revert("You don't have anything to unstake.");
    }

    // This returns the balance in ETH not in token
    ...
    function getDeposit() external view returns (Deposit memory) {
        ...
        if (found) return deposits[index];
        return Deposit(0x0000000000000000000000000000000000000000, 0, 0);
    }

    function createDeposit(uint256 amount, uint256 startdate) internal {
        bool found = false;
        for (uint256 i = 0; i < deposits.length; i++) {
            if (deposits[i].user == msg.sender) {
                require(!isStaking(deposits[i]), 'You are already staking.');
                found = true;
                deposits[i].amount = amount;
                deposits[i].startdate = startdate;
                break;
            }
        }
    ...
    function deleteDeposit(Deposit memory deposit, uint256 index) internal {
        if (isStaking(deposit)) {
            deposits[index].amount = 0;
            deposits[index].startdate = 0;
        }
    }

    function listDeposits() external view returns (Deposit[] memory) {
        Deposit[] memory deposits_ = new Deposit[](deposits.length);
        for (uint256 i = 0; i < deposits.length; i++) {
            deposits_[i] = deposits[i];
        }
        return deposits_;
    }
    ...
    function calculateInterest(Deposit memory deposit) public view returns (uint256) {
        ...
        //return accrueInterest(stakedAmount, calculateRatePerYearInRAY, stakingAge);
    ...
    function getForbiddenToStake() external view returns (address[] memory) {
        address[] memory forbiddenAddresses = new address[](forbiddenToStake.length);

        for (uint256 i = 0; i < forbiddenToStake.length; i++) {
            forbiddenAddresses[i] = forbiddenToStake[i];
        }
    ...
    function stakingStatus() public view returns (bool) {
        return (stakingEndDate > 0 && (stakingEndDate >= block.timestamp));
    }

    function isForbiddenToStake(address address_) public view returns (bool) {
        bool isForbidden = false;
        for (uint16 i = 0; i < forbiddenToStake.length; i++) {
            if (forbiddenToStake[i] == address_) {
                isForbidden = true;
                break;
            }
        }
    ...
    function isBurnable(address address_) public view returns (bool) {
        bool _isBurnable = false;
        for (uint16 i = 0; i < burnableAddresses.length; i++) {
            if (burnableAddresses[i] == address_) {
                _isBurnable = true;
                break;
            }
        }
    ...
    function isStaking(Deposit memory deposit) public view returns (bool) {
        return (deposit.user == msg.sender && deposit.amount > 0 && deposit.startdate > 0);
    }

    function setForbiddenToStake(address _account) external onlyOwner {
        forbiddenToStake.push(_account);
        emit setForbiddenToStakeSuccess();
    }

    function setBurnableAddresses(address _account) external onlyOwner {
        burnableAddresses.push(_account);
        emit setBurnableAddressesSuccess();
    }

    modifier isAllowedToStake() {
        require(!isForbiddenToStake(msg.sender), 'You are forbidden to stake');
        _;
    }

    modifier isAllowedToBurn() {
        require(isBurnable(msg.sender), 'This address is not allowed to burn.');
        _;
    }

    modifier isNotEther() {
        require(msg.value == 0, "You can't deposit Ether directly to this contract using this method");
        _;
    }

    modifier maxManualMintableNotReached() {
        require(this.totalSupply() <= maxManualMintable, 'You can only mint manually till the half of the maximum supply.');
        _;
    }

    modifier isStakingActive() {
        require(stakingStatus(), 'Staking must be active');
        _;
    }
    ...
    event setForbiddenToStakeSuccess();

    event setBurnableAddressesSuccess();
}
