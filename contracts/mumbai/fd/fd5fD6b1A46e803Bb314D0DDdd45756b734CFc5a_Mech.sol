...
contract Mech is ERC721Enumerable, ERC2981, AccessControl, Ownable, IERC721Receiver, ReentrancyGuard {
    bytes32 public constant OPERATOR_ROLE = keccak256("OPERATOR_ROLE");
    IERC721A public partContract;
    ...
    // external
    function assemble(uint256 blockNumberLimit, uint256[] calldata parts, bytes calldata signature) external payable nonReentrant {
    ...
    function disassemble(uint256 blockNumberLimit, uint256 tokenId, bytes calldata signature) external payable nonReentrant {
    ...
    function checkPayment(uint256 price) private {
      require(msg.value >= price, "Not enough amount is paid");
      if (msg.value > price) {
