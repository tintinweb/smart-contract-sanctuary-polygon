...
contract Staking is Ownable, IERC721Receiver, ReentrancyGuard {
    ...
    uint256 MAX_STAKE_LIMIT_IN_TXN = 10;

    // Structure to store staked NFT information
    // TODO: add tokenAddress in the struct
    struct StakedNFT {
        uint256 tokenId;
        uint256 stakedAt;
        address owner;
    ...
    uint256 MAX_RELEASE_PER_TXN = 50;
    uint256 nftHoldDuration = 1209600; // 14 Days in seconds
    mapping(address => ReleaseQueueLib.ReleaseQueue) releaseQueue;

    // Event emitted when an NFT is staked
    event NFTStaked(uint256 tokenId, uint256 stakedAt, address owner);

    // Event emitted when an NFT is unstaked
    event NFTUnstaked(uint256 tokenId, uint256 unstakedAt, address owner);

    // Event emitted when an NFT is released to owner
    event NFTReleased(uint256 tokenId, address owner);
    ...
    function stake(
        ...
        if (address(nftContracts[tokenAddress]) == address(0)) {
            revert ContractNotRegistered();
        }

        if (len > MAX_STAKE_LIMIT_IN_TXN) {
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            if (stakedNfts[tokenId].owner != address(0)) {
                revert NFTAlreadyStaked();
            }

            if (nftContracts[tokenAddress].ownerOf(tokenId) != _msgSender()) {
                revert NotTokenOwner();
            }

            nftContracts[tokenAddress].safeTransferFrom(
                _msgSender(),
                address(this),
                tokenId,
                "0x00"
    ...
    function unstake(
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            if (stakedNfts[tokenId].owner != _msgSender()) {
                revert NotTokenOwner();
            }
            ...
            ReleaseQueueLib.ReleaseQueueItem memory qItem = ReleaseQueueLib
                .ReleaseQueueItem(
                    tokenId,
                    _msgSender(),
                    block.timestamp + nftHoldDuration
                );
            releaseQueue[tokenAddress].push(qItem);

            delete stakedNfts[tokenId];

            emit NFTUnstaked(tokenId, block.timestamp, _msgSender());
        }
    }

    function releaseToOwner(address tokenAddress) external {
        for (uint256 i = 0; i < MAX_RELEASE_PER_TXN; i++) {
            if (!isReadyForRelease(tokenAddress)) {
                break;
            }
            ReleaseQueueLib.ReleaseQueueItem memory qItem = releaseQueue[
                tokenAddress
            ].pop();
            ...
            emit NFTReleased(qItem.tokenId, qItem.tokenOwner);
        }
    }

    function setCoolingOffPeriod(uint256 copInSeconds) external onlyOwner {
        nftHoldDuration = copInSeconds;
    ...
    function isReadyForRelease(
        ...
        return releaseQueue[tokenAddress].isReadyForRelease();
    }
