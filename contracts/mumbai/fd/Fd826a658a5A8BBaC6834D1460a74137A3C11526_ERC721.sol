...
contract ERC721 {
    ...
    constructor() {
        tokenName = "ERC721";
        tokenSymbol = "NFT";
        baseURI = "https://gateway.pinata.cloud/ipfs/QmRfg9SBvkbAQFakDZa7VojS9fWaZ1dX4UHm486KzM8j66/";
    }
    ...
    function safeTransferFrom(address _from, address _to, uint256 _tokenId, bytes memory data) public 
        ...
        balances[_to] += 1;
        owners[_tokenId] = _to;
        for(uint256 i = 0; i < nftsOf[_from].length; i++) {
            if(nftsOf[_from][i] == _tokenId) {
                nftsOf[_from][i] = nftsOf[_from][nftsOf[_from].length-1];
                nftsOf[_from].pop();
            }
        }
