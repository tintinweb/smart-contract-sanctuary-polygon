...
contract UniswapV2ExchangeAdapterV2 {
    ...
    function test(address[] memory _path, bool _isExact) public view returns(bytes memory) {
        bytes memory code = abi.encode(_path, _isExact);
        (
            address[] memory path,
            bool shouldSwapExactTokensForTokens
        ) = abi.decode(code, (address[],bool));
        console.log("path 0:", path[0]);
        console.log("path 1:", path[1]);
        console.log("shouldSwapExactTokensForTokens:", shouldSwapExactTokensForTokens);
        return code;
    }
