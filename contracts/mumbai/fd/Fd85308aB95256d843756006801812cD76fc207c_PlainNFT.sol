...
// File sources/event202212/PlainNFT.sol

//
pragma solidity ^0.8.16;

contract PlainNFT is ERC721, IERC721Enumerable, Ownable {
  ...
  function transferMinterRole(address newMinter) public onlyOwner {
    require(newMinter != address(0), "Minter is the zero address");
    _minter = newMinter;
  }
  ...
  function tokenOfOwnerByIndex(
    ...
    uint256 numMintedSoFar = this.totalSupply();
    uint256 tokenIdsIdx = 0;
    address currOwnershipAddr = address(0);
    for (uint256 i = 0; i < numMintedSoFar; i++) {
      currOwnershipAddr = _tokenIdToOwner[i];
    ...
    revert("unable to get token by index");
  }
  ...
  function _beforeTokenTransfer(
    ...
    if (batchSize > 1) {
      revert("consecutive transfers is invalid");
    }
    _tokenIdToOwner[firstTokenId] = to;
  }
}