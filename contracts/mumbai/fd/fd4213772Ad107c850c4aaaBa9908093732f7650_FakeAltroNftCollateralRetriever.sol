...
import "./FakeAltroNftCollectionFactory.sol";
import "./interfaces/INftCollectionVaultService.sol";
import "./interfaces/IFeeManager.sol";

contract FakeAltroNftCollateralRetriever is ERC721HolderUpgradeable, OwnableUpgradeable {
	...
	//TODO: set collection factory + platform fee manager + EVENT
	...
	function initialize(address collectionFactory, address platformFeeManager) public initializer {
		__FakeAltroNftCollateralRetriever_init(collectionFactory, platformFeeManager);
	}

	function __FakeAltroNftCollateralRetriever_init(address collectionFactory, address platformFeeManager) internal initializer {
		__Context_init_unchained();
		__ERC721Holder_init_unchained();
		__FakeAltroNftCollateralRetriever_init_unchained(collectionFactory, platformFeeManager);
	}

	function __FakeAltroNftCollateralRetriever_init_unchained(address collectionFactory, address platformFeeManager) internal initializer {
	...
	function burnNft(address _nftContract, uint256 _tokenId) external onlyMinter(_nftContract) {
		require(FakeAltroNftCollectionFactory(_collectionFactory).isAKnownCollection(_nftContract), "Unknown collection");
	...
	function onERC721Received(
		...
		require(FakeAltroNftCollectionFactory(_collectionFactory).isAKnownCollection(msg.sender), "Unknown collection");
...
import "./FakeAltroNftCollection.sol";
...
contract FakeAltroNftCollectionFactory is Initializable, OwnableUpgradeable {
	using EnumerableSetUpgradeable for EnumerableSetUpgradeable.AddressSet;

	// FIXME: should be IFakeAltroNftCollection so that when we upgrade we can keep the interface?
	struct CreatedContract {
		FakeAltroNftCollection collection;
		string symbol;
		string name;
		address oracle;
		bytes1 tokenVersion;
	...
	function initialize(address _licenseManager, address _nftReserveAddress) public initializer {
		...
		__FakeAltroNftCollectionFactory_init(_licenseManager, _nftReserveAddress);
	}

	function __FakeAltroNftCollectionFactory_init(address _licenseManager, address _nftReserveAddress) internal onlyInitializing {
		__Ownable_init_unchained();
		__FakeAltroNftCollectionFactory_init_unchained(_licenseManager, _nftReserveAddress);
	}

	function __FakeAltroNftCollectionFactory_init_unchained(address _licenseManager, address _nftReserveAddress) internal onlyInitializing {
	...
	function createCollection(
		...
		FakeAltroNftCollection collection = new FakeAltroNftCollection(
			_name,
			_symbol,
			_oracle,
			msg.sender,
...
contract FakeAltroNftCollection is ERC721URIStorage, ERC721Enumerable, ERC721Burnable, AccessControl, INftCollectionVaultService {
