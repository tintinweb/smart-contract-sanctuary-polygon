...
contract PortfolioDAO is Ownable{
    ...
    event NewProject(address indexed creator, uint256 indexed projectIndex, uint256 CapitalNeeded, uint256 deadline);
    // Emits when a project is funded
    event Funded(address indexed investors, uint256 indexed projectIndex, uint256 amount);
    // Emits when a project is executed
    event Executed(address indexed creator, uint256 indexed projectIndex, uint256 amount);
    // Emits when a project is cancelled
    event Cancelled(address indexed creator, uint256 indexed projectIndex, uint256 amount);
    ...
    // Execute project
    function executeProject(uint256 projectIndex) internal {
        // Make sure the proposal has not been executed yet
        require(projects[projectIndex].executed == false, "Proposal has already been executed");
        // Make sure the proposal is still active
