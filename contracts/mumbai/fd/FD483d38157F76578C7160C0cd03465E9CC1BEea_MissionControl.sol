...
contract MissionControl is IMissionControl, OwnableUpgradeable, EIP712Upgradeable {
    ...
    mapping(address => CollectOrder[]) private userRentedTiles;
    mapping(address => mapping(int256 => mapping(int256 => uint256))) private userRentedTilesIndices;
    ...
    function createRentTiles(
        ...
        if (_supertoken != superToken) return;
        int96 tilesRented;
        uint256 i;
        ...
        for (; i < length; i++) {
            ...
            TileElement[2] storage tileElements = tiles[_renter][placeOrderItem.order.x][placeOrderItem.order.y];
            TileElement storage tile = tileElements[1].nonce != 0 ? tileElements[1] : tileElements[0];

            if (tile.nonce != 0 && tile.pausedAt != 0) {
                tile.pausedAt = 0;
                IMissionControlStakeable(placeOrderItem.tokenAddress).onResume(_renter, tile.nonce);
                if (__zeroOrOne(placeOrderItem.order.x, placeOrderItem.order.y, _renter) == 1) {
                    TileElement storage bottom = tileElements[0];
                    bottom.pausedAt = 0;
                }
                unchecked {
                    ++tilesRented;
                ...
                require(
                    (abs(placeOrderItem.order.x) + abs(placeOrderItem.order.y) + abs(placeOrderItem.order.z)) / 2 == 2,
                    "MISSION_CONTROL: RADIUS 2 ONLY"
                );
                __placeNFT(placeOrderItem.order, placeOrderItem.tokenId, placeOrderItem.tokenAddress, _renter);
                if (__zeroOrOne(placeOrderItem.order.x, placeOrderItem.order.y, _renter) == 0) {
                    userRentedTiles[_renter].push(placeOrderItem.order);
                    userRentedTilesIndices[_renter][placeOrderItem.order.x][placeOrderItem.order.y] =
                        userRentedTiles[_renter].length -
                        1;
    ...
    function updateRentTiles(
        ...
        int96 tilesRented;
        uint256 i;
        uint256 placeOrderLength = _placeOrders.length;
        uint256 removeTileLength = _removeTiles.length;

        for (; i < placeOrderLength; i++) {
            PlaceOrder memory placeOrderItem = _placeOrders[i];
            TileElement[2] storage tileElements = tiles[_renter][placeOrderItem.order.x][placeOrderItem.order.y];
        ...
        i = 0;
        for (i; i < removeTileLength; i++) {
            CollectOrder memory removeTileItem = _removeTiles[i];
            require(
                (abs(removeTileItem.x) + abs(removeTileItem.y) + abs(removeTileItem.z)) / 2 == 2,
                "MISSION_CONTROL: RADIUS 2 ONLY"
            );
            TileElement storage tile = tiles[_renter][removeTileItem.x][removeTileItem.y][
                __zeroOrOne(removeTileItem.x, removeTileItem.y, _renter)
            ];

            if (__zeroOrOne(removeTileItem.x, removeTileItem.y, _renter) == 0) {
                uint256 index = userRentedTilesIndices[_renter][removeTileItem.x][removeTileItem.y];
                uint256 size = userRentedTiles[_renter].length;
                userRentedTiles[_renter][index] = userRentedTiles[_renter][size - 1];
                userRentedTiles[_renter].pop();

                if (tile.pausedAt == 0) --tilesRented;
            }
            __removeNFT(removeTileItem, _renter);
        }

        require(_flowRate == _oldFlowRate + tilesRented * RENT_TILE_FLOW_RATE);
    }
    ...
    function deleteRentTiles(address _supertoken, address _renter) external onlyStreamer {
        if (_supertoken != superToken) return;
        uint256 i;
        uint256 length = userRentedTiles[_renter].length;
        for (i; i < length; i++) {
            CollectOrder memory rental = userRentedTiles[_renter][i];
            TileElement storage tile = tiles[_renter][rental.x][rental.y][__zeroOrOne(rental.x, rental.y, _renter)];
            tile.pausedAt = block.timestamp;

            if (__zeroOrOne(rental.x, rental.y, _renter) == 1) {
                TileElement storage bottom = tiles[_renter][rental.x][rental.y][0];
                bottom.pausedAt = block.timestamp;
            }
        }
    }
    ...
    function checkTile(
        ...
        return IMissionControlStakeable(tile.stakeable).checkTile(_user, tile.nonce, tile.pausedAt, _x, _y);
    }
    ...
    function _handle_raid_collect(
        ...
        if (!iStakeable.isRaidable(tileElement.tokenId, _x, _y)) {
            return (0, WASTE_ID);
        }
        return iStakeable.onCollect(msg.sender, tileElement.nonce, tileElement.pausedAt, _x, _y);
    }
    ...
    function setSuperToken(address _superToken) external onlyOwner {
        require(_superToken != address(0), "MISSION_CONTROL: ADDRESS ZERO");
    ...
    function getUserRentals(address _renter) external view returns (CollectOrder[] memory _orders) {
        _orders = userRentedTiles[_renter];
    }

    function getAllTileInfo(address _renter) external view returns (ContractTileInfo[] memory _contractTileInfo) {
        ...
        _contractTileInfo = new ContractTileInfo[](_totalTiles);
            ...
            for (int256 y = -radius; y <= radius; y++) {
                for (int256 z = -radius; z <= radius; z++) {
                    ...
                    if (rad <= allowedRadius && rad > 0 && sum == 0) {
                        ...
                        _contractTileInfo[_count].order = CollectOrder(x, y, z);
                        _contractTileInfo[_count].timeLeft = _timeLeft;
                        _contractTileInfo[_count].base = _base;
                        _contractTileInfo[_count].top = _top;
                        _timeLeft = 0;
                        _count++;
                    }
                }
