...
contract NFTMarketPlace {
    ...
    function listNFT(
        ...
        market[_tokenId] = item;
        marketMappingKeys.push(_tokenId);
    ...
    function buyNFT(uint _tokenId) external payable {
        require(
            msg.value >= market[_tokenId].price,
            "Insufficient balance Transfered!"
        );
        require(market[_tokenId].status, "Already sold");
        IERC4907(market[_tokenId].tokenContract).transferFrom(
            market[_tokenId].seller,
            msg.sender,
            market[_tokenId].tokenId
        );

        market[_tokenId].status = false;
        (bool success, ) = market[_tokenId].seller.call{value: msg.value}("");
    ...
    function deleteNFT(uint _tokenId) public {
        require(msg.sender == market[_tokenId].seller, "not auhorized");
        delete market[_tokenId];
    }

    function updateNFT(uint _tokenId) public {
        require(
            msg.sender == market[_tokenId].seller || msg.sender == owner,
            "not auhorized"
        );
        market[_tokenId].status = false;
    }
    ...
    function getAllItemsForSale() public view returns (Item[] memory) {
        Item[] memory result = new Item[](marketMappingKeys.length);
        // populate the array with structs

        for (uint i = 0; i < marketMappingKeys.length; i++) {
            if(market[marketMappingKeys[i]].status){
                result[i] = market[marketMappingKeys[i]];
                console.log("hree");
            }
        }
        return result;
