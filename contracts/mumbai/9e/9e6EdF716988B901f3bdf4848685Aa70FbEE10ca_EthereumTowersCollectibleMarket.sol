...
import "../interfaces/IEthereumTowersCollectible.sol";
    ...
    ReentrancyGuardUpgradeable
{
    ...
    modifier supportsERC1155(address contractAddress) {
        require(
            IERC165Upgradeable(contractAddress).supportsInterface(ERC1155_INTERFACE_ID),
    ...
    function authorHasRole(
        ...
        return IEthereumTowersCollectible(nftContractAddress).hasRole(authorRole, author);
    }

    function _validateMintConditions(address author) internal view returns (bool) {
        require(
    ...
    function _matchSignatures(bytes memory left, bytes memory right) internal pure returns (bool) {
    ...
    function updateServiceAddress(address _serviceAddress) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "ETT: missing required role");

        require(!_isContract(_serviceAddress), "ETT: contract cannot be signer");
    ...
    function _nftTransferHelper(
        ...
        require(
            IERC1155Upgradeable(nftContractAddress).balanceOf(from, tokenId) >= amount,
            "ETT: insufficient tokens to transfer"
        );

        require(
            IERC1155Upgradeable(nftContractAddress).isApprovedForAll(from, address(this)),
            "ETT: contract does not have access to this token."
        );

        IERC1155Upgradeable(nftContractAddress).safeTransferFrom(
            from,
            address(this),
            tokenId,
            amount,
    ...
    function mintByVoucher(InstantMintVoucher calldata voucher)
        ...
        returns (uint256)
    {
        require(_validateMintConditions(voucher.author), "ETT: invalid mint conditions");
        (address authorAddress, address serviceSignerAddress) = _verify(voucher);

        require(
            serviceSignerAddress == serviceAddress && authorAddress == voucher.author,
            "ETT: invalid signatures"
        );

        require(!signatureUsed[voucher.serviceSignature], "ETT: this voucher already used");
        ...
        IEthereumTowersCollectible(nftCollectibleAddress).mint(
            voucher.author,
            voucher.tokenId,
            voucher.amount,
            ""
    ...
    function vaultRedeem(
        ...
        uint256 quantity
    ) public payable whenNotPaused supportsERC1155(voucher.target) nonReentrant returns (uint256) {
        require(_validateTakerOrderParams(takerOrder), "ETT: taker order not valid");

        require(_verify(voucher) == voucher.tokenOwner, "ETT: invalid maker order signer");

        require(
            _verify(takerOrder) == serviceAddress && voucher.serviceAddress == serviceAddress,
        ...
        require(quantity <= voucher.amount, "ETT: quantity should not exceed amount");

        uint256 totalPrice = voucher.fixPrice * quantity;

        require(
            IERC20Upgradeable(paymentToken).allowance(takerOrder.taker, address(this)) >=
                totalPrice,
        ...
        if (voucher.isFirstSale) {
            ...
            require(
                IEthereumTowersCollectible(nftCollectibleAddress).totalSupply(voucher.tokenId) +
                    quantity <=
                    voucher.amount,
        ...
        uint256 projectFee = percentage(totalPrice, serviceFeePercent);
        ...
        IERC20Upgradeable(paymentToken).safeTransfer(serviceFeeAddress, projectFee);
        IERC20Upgradeable(paymentToken).safeTransfer(voucher.tokenOwner, sellerValue);

        _markSignatureUsed(takerOrder.serviceSignature);

        emit VaultRedeem(voucher.tokenOwner, takerOrder.taker, voucher.tokenId, quantity);
    ...
    function redeem(
        ...
        emit FixPriceRedeem(voucher.tokenOwner, takerOrder.taker, voucher.tokenId, quantity);
    ...
    function _hash(TakerOrderVoucher calldata voucher) internal view returns (bytes32) {
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
    ...
    function _hash(InstantMintVoucher calldata voucher) internal view returns (bytes32) {
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
    ...
    function _hash(AirdropVoucher calldata voucher) internal view returns (bytes32) {
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
    ...
    function _verify(TakerOrderVoucher calldata voucher) internal view returns (address) {
    ...
    function _verify(InstantMintVoucher calldata voucher) internal view returns (address, address) {
        bytes32 digest = _hash(voucher);

        address artAuthorAddress = ECDSAUpgradeable.recover(digest, voucher.signature);
        address serviceSignerAddress = ECDSAUpgradeable.recover(digest, voucher.serviceSignature);

        return (artAuthorAddress, serviceSignerAddress);
    }

    function _verify(AirdropVoucher calldata voucher) internal view returns (address, address) {
        bytes32 digest = _hash(voucher);
...
interface IEthereumTowersCollectible is IAccessControlEnumerable {
    function mint(
        address,
        uint256,
        uint256,
