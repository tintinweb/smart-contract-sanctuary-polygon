...
*/
    ...
    function addToWhiteList(address[] memory _allowed) external onlyAdmin {
        for (uint256 i = 0; i < _allowed.length; i++) {
            require(_allowed[i] != address(0x0), "Zero address!");
    ...
    function distributeTokens(uint256 start, uint256 end) public nonReentrant {
    ...
    function emergencyRefund(uint256 _confirm) external onlyAdmin {
        ...
        require(_confirm == 369, "Wrong confirmation");
        success = false;
        fundsReturn = true;
        sellerVault = 0;
        duration = 0;
