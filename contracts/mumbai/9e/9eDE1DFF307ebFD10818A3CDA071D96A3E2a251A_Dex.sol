...
contract Dex {
    ...
    mapping(bytes32 => mapping(uint => Order[])) public orderBook;   //if uint is 0 it is buy order, if 1 it is sell order
    ...
    function addToken(bytes32 ticker, address tokenAddress) onlyAdmin() tokenAlreadyExists(ticker) external {
    ...
    modifier tokenAlreadyExists(bytes32 ticker) {
        for(uint i=0; i<tokenList.length; i++){
