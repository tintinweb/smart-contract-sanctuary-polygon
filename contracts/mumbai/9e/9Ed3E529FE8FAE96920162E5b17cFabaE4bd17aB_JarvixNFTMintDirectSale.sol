 ...
 * @author tazous
 */
contract Allowance4MintProxy is AllowanceProxy {
    ...
    ProxyDiamond(proxyHubAddress_) AllowanceProxy(allowanceProxyAddress_, false, false, false){
    ...
    returns (uint256 allowance, Bucket memory bucket) {
        (uint256 _allowance, Bucket memory _bucket) = super._addOrUseAllowance(address_, bucketName, amount, add);
        // Every payable NFT have been minted, the user gain a free mint
        if(!add && bucketName == mintBucketName && amount != 0 && _allowance == 0) {
            _addOrUseAllowance(address_, freeMintBucketName, 1, true);
        }
...
abstract contract BaseJarvixTokenDirectSale is CurrencyHandler, PriceHandlerProxy, SecuredProcessProxy,
    ...
    function _buy(address buyer, uint256 amount, bytes32 currency, uint256 price, bool discount,
        ...
        doProcess(getSecuredProcessName(), buyer, amount);
