...
contract ExchangeV1 is IExchange, IxBaseV1, FeeWorkerV1 {
    ...
    {
        try this.innerWithdrawToken(user, token, amount) {
            // success
            _log(user, token, amount, memo, WithdrawStatus.OK, "", "");
        } catch Error(string memory reason) {
            // catch failing revert() and require()
            _log(user, token, amount, memo, WithdrawStatus.ERR, reason, "");
        } catch (bytes memory reason) {
            // any other errors
            _log(user, token, amount, memo, WithdrawStatus.LOW_ERR, "", reason);
        }

        tryRechargeGasFee(msg.sender);
    }
...
abstract contract FeeWorkerV1 {
    ...
    function tryRechargeGasFee(address target) internal {
