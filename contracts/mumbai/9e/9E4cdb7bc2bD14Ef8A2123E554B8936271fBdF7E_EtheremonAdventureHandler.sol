...
// File: contracts/EtheremonAdventureHandler.sol
...
contract EtheremonAdventureHandler is BasicAccessControl, EthermonEnum {
    uint8 public constant STAT_MAX_VALUE = 32;
    ...
    uint256 public rebornItemClass = 500;
    // Removing reborn
    uint256[] public rebornMonsterIds = [
        25252,
        25264,
        25267,
        25372,
    ...
    function handleSingleItem(
        ...
        //     _param =
        //         getRandom(_sender, block.number - 1) %
        //         rebornMonsterIds.length;
        //     _target = rebornMonsterIds[_param];

        //     // remove monsterId
        //     rebornMonsterIds[_param] = rebornMonsterIds[
        //         rebornMonsterIds.length - 1
        //     ];
        //     delete rebornMonsterIds[rebornMonsterIds.length - 1];
        //     //rebornMonsterIds.length--;
        //     rebornMonsterIds.pop();
        ...
        if (_classId == expItemClass) {
            // TODO: It would be % of EXP to next leve
            ...
            uint256 expValue = exp + ((exp * _value) / exp);
            //uint256 expValue = levelExp + ((levelExp * _value) / levelExp);

            data.increaseMonsterExp(obj.monsterId, uint32(expValue));
            ...
            data.increaseMonsterExp(
                obj.monsterId,
                levelExps[currentLevel - 1] - obj.exp
            );
        } else if (_classId == transformItemClass) {
            //TODO: Need to verify from Idon once if the ancestors check required here means should we use EthermonTransform transform().
            ...
            uint256 newObjId = monsterTokenContract.mintMonster(
                transformClass,
                msg.sender,
                "..name me..."
            );
            monsterTokenContract.burnMonster(obj.monsterId);
            // uint256 remainingExp = getLevel(obj.exp);
        } else {
            revert();
        }
    }
    ...
    function handleMultipleItems(
        ...
        if (currentValue + 1 >= LEVEL_MAX_VALUE) revert();
