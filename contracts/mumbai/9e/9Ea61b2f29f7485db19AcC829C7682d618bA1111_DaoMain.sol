...
import "./interfaces/tokens/ILPToken.sol"; // Staked DAO Tokens are LPToken
 ...
 *         holder tokens back.
 * @notice DaoMain is LockBox, LockBox is Accesshandler.
 */
contract DaoMain is LockBox, IDaoMain {
    ...
    IRewardHandler private feeRewards1; // Bet rewards for SDT holders
    IRewardHandler private feeRewards2; // LP rewards for SDT holders
     ...
     * @param inFeeRewards1 Is 1st contract that handles SDT rewards.
     * @param inFeeRewards2 Is 2nd contract that handles SDT rewards.
     */
    function init(
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        if (stakedToken == address(0) || daoToken == address(0)) {
            revert BadTokenZero();
        }
     ...
     * @param inTokenAdd The address of the token type to unstake.
    ...
    function unstakeDaoToken(
        ...
        external
        onlyAllowedDistToken(inTokenAdd)
    {
        address owner = msg.sender;
     ...
     * @param inTokenAdd The staked token type to return for the unstake.
     */
    function unstakeAllTokens(address inTokenAdd)
        external
        override
        onlyAllowedDistToken(inTokenAdd)
    {
        address owner = msg.sender;
        uint256 stakedTokenBalance = IERC20(inTokenAdd).balanceOf(owner);
        unstakeTokens(owner, stakedTokenBalance);
    }

    /**
     * @notice Let the admin of this contract unstake all tokens of a user.
     * @param owner The user to update.
    *  @param inTokenAdd The address of the staked token type to return.
     */
    function returnAllStakedTokensAsAdmin(address owner, address inTokenAdd)
        external
        override
        onlyAllowedDistToken(inTokenAdd)
        onlyRole(DEFAULT_ADMIN_ROLE)
    ...
    function _stake(
        ...
        console.log("Will mint:", inAmount / 1 ether);
