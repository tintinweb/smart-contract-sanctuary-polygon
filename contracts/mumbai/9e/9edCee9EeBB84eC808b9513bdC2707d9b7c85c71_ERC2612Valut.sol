...
contract ERC2612Valut {
    ...
    /*
    ERC2612 新增permit方法，在链下用私钥签署產生一個签名(用户签名操作里面已经包含了approve信息),将签名作为参数
    在合约验证成功后，该合约就可以执行transferFrom操作
    并且这个智能合约的服务商，可以代替用户支付主网交易的gas，并且从交易的代币里面扣除对应的费用，代币持有者账户不需要发送交易
    */
    ...
    function withdrawAll() external {
        SafeERC20.safeTransfer(IERC20(_token), msg.sender, depositAmount[msg.sender]);
        delete depositAmount[msg.sender];
    }
}