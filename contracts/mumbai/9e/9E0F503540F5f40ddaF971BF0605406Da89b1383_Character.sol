...
contract BattleSkills is ERC1155Base {
    ...
    mapping(uint256 => uint256) public tokenIdToSkillId;
    ...
    function createSkill(
        ...
        uint256 tokenId = type(uint256).max; // pass type(uint256).max as the tokenId argument
        mintTo(msg.sender, tokenId, _tokenURI, 1);
        tokenIdToSkillId[nextTokenIdToMint() - 1] = numSkills + 1; // update tokenIdToSkillId mapping
        numSkills++;
        skills[numSkills] = Skill(numSkills, _name, _damage, _manaCost);
    }

    function mintSkill(uint256 _skillId) public {
        require(_skillId <= numSkills, "Skill does not exist");
        uint256 tokenId = _skillId;
        _mint(msg.sender, tokenId, 1, "");
        tokenIdToSkillId[tokenId] = _skillId;
    }
...
contract Character is ERC721Base, ERC1155Holder {
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(ERC721Base, ERC1155Receiver)
        returns (bool)
    {
        return
    ...
    CharacterStats[] public initialCharacter;
    ...
    function getCharacterDefense(uint256 tokenId)
        public
        view
        returns (uint256)
    {
    ...
    function newCharacter() public {
        CharacterStats memory newChar = initialCharacter[0];
        ...
        characterStats[tokenId] = newChar;
        numCharacters++;
    }

    function equipItem(uint256 characterTokenId, uint256 tokenId) external {
    ...
    function unequipItem(uint256 characterTokenId) external {
        ...
        uint256 equippedTokenId = characterEquips[characterTokenId]
            .equippedItem;
        require(equippedTokenId != 0, "No item equipped");

        // Unequip the item
        characterEquips[characterTokenId].equippedItem = 0;
        battleItems.safeTransferFrom(
            address(this),
            msg.sender,
            equippedTokenId,
    ...
    function unequipSkill(uint256 characterTokenId) external {
        ...
        uint256 equippedSkillId = characterEquips[characterTokenId]
            .equippedSkill;
        require(equippedSkillId != 0, "No skill equipped");

        // Unequip the skill
        characterEquips[characterTokenId].equippedSkill = 0;
        battleSkills.safeTransferFrom(
            address(this),
            msg.sender,
            equippedSkillId,
    ...
    function getEquippedItem(uint256 characterTokenId)
        external
        view
        returns (uint256)
    {
    ...
    function getEquippedSkill(uint256 characterTokenId)
        external
        view
        returns (uint256)
    {
