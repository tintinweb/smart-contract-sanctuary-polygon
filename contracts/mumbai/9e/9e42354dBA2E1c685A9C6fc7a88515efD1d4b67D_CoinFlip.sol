...
contract CoinFlip is Ownable {
  ...
  uint256[] public betAmounts = [0.05 ether, 0.1 ether, 0.5 ether, 1 ether];
  ...
  function flip(uint256 guess, uint256 betIndex) public payable {
    ...
    if (randomFlip == guess) {
      ...
      require(sent, 'Failed to send Ether');
      emit Flipped(msg.sender, true, msg.value);
    } else {
      emit Flipped(msg.sender, false, msg.value);
    }
  }
  ...
  function setContractPaused(bool state) external onlyOwner {
    paused = state;
  }
