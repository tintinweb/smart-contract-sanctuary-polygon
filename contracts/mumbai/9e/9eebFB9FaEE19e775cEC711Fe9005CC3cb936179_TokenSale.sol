...
/**
    TODO: 
        1. totalSupplyInValue should be usdc to be raised
 */
...
contract TokenSale is Initializable, ITokenSale {
    ...
    function initialize(
        ...
        usdc = IERC20D(0x0068Fd2436C613962dbFeBce8Dc255972F1bcADe); //TODO change this for mainnet
    }

    function giftTier(address[] calldata users, uint256[] calldata allocation)
        public
    {
        require(admin.hasRole(OPERATOR, msg.sender), "TokenSale: OnlyOperator");
        require(users.length == allocation.length, "TokenSale: Invalid length");
        for (uint256 i = 0; i < users.length; i++) {
            if (stakingContract.getTierOf(users[i]) < allocation[i]) {
                tokensaleTiers[users[i]] = allocation[i];
            }
        }
    }
    ...
    function destroy() external override {
        _onlyAdmin();
        uint256 amountBUSD = usdc.balanceOf(address(this));
        if (amountBUSD > 0) {
            usdc.safeTransfer(admin.wallet(), amountBUSD);
        }
    ...
    function _processPrivate(address _sender, uint256 _amount) internal {
        ...
        uint256 max = maxInFiat * (10 ** usdc.decimals());
