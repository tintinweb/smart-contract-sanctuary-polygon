...
contract GameRoulette {
    address owner;
    uint256 constant minBet = 0.5 ether;
    uint8 public constant maxDrumSpin = 6;
    struct Game {
        uint256 id;
        address player1;
        address player2;
        uint256 bet;
        bool player1Turn;
        bool player1ShotOpponent;
        bool player2ShotOpponent;
    ...
    function createGame() public payable {
        require(msg.value >= minBet, "You must bet at more 0.5 ether");
    ...
    function turn(uint256 gameId, bool shotOpponent) public {
        ...
        require(
            game.player1Turn == (msg.sender == game.player1),
            "It is not your turn"
        );

        // Calculate the probability of the shot being fired
        uint256 probability = game.numberDrumSpin;

        // Generate a random number between 1 and 100 (inclusive)
        uint256 randomNumber = random(maxDrumSpin);

        if (shotOpponent) {
            require(
                (game.player1Turn && !game.player1ShotOpponent) ||
                    (!game.player1Turn && !game.player2ShotOpponent),
                "You have already shot your opponent"
            );

            if (game.player1Turn) game.player1ShotOpponent = true;
            else game.player2ShotOpponent = true;
        }
        ...
        if (randomNumber < probability) {
            if (shotOpponent) {
                game.winner = game.player1Turn ? game.player1 : game.player2;
            } else {
                game.winner = game.player1Turn ? game.player2 : game.player1;
            }
            ...
            emit GameTurn(gameId, msg.sender, game.timeLastMove, shotOpponent);
        }
    }
