    ...
    NiftyJourneyErrors
{
    ...
    function disableMarketplaceBlockList() public {
        ...
        deployedHostileRegistryAddress = address(0);
    }
    ...
    function requireAddressIsNotBlocked(address addressToCheck) public {
        if (deployedHostileRegistryAddress != address(0)) {
    ...
    modifier notFinalized() {
        if (collectionStatus.isContractFinalized) {
            revert ContractIsNotFinalized({
                isContractFinalized: collectionStatus.isContractFinalized
            });
        }
        _;
    }
    ...
    function mintMore(uint88 amount) external notFinalized {
        _requireOnlyValidSender();
        if (amount == 0) {
            revert CannotMintZeroNifties({amountToMint: amount});
        }
    ...
    function burnUnusedTokens(uint88 startTokenId) public notFinalized {
        ...
        require(
            startTokenId >= niftyTypeDetail.idFirst,
            "Can only burn tokens in current nifty type"
        );
...
interface IHostileMarketplaceRegistryImplementer {
    ...
    /**
     * @dev removes the deployed Hostile Marketplace Registry contract address therefore disallowing it's behavior
     */
...
contract NiftyJourneyErrors {
    error ContractIsNotFinalized(bool isContractFinalized);
    error CannotMintZeroNifties(uint88 amountToMint);
