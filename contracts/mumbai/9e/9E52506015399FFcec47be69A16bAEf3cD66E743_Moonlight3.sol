...
contract Moonlight3 is BaseRelayRecipient {
    address public owner;

    ISuperfluid public host;
    IConstantFlowAgreementV1 public cfaV1;
    address public cfaV1Address;

    constructor(
        address _host,
        address _cfaV1, // if Moonlight2 works with Biconomy, update this to get address from host
        ...
        cfaV1 = IConstantFlowAgreementV1(_cfaV1);
        cfaV1Address = _cfaV1;
    }

    function grantOperatorPermissionToDelete(
        ISuperfluidToken _token,
        address _flowOperator
    ) external {
        ISuperfluid.Operation[] memory array = new ISuperfluid.Operation[](1);

        array[0] = ISuperfluid.Operation(
            201,
            cfaV1Address,
            abi.encode(
                abi.encodeWithSelector(
                    cfaV1.updateFlowOperatorPermissions.selector,
                    _token,
                    _flowOperator,
                    4,
        ...
        host.forwardBatchCall(array);
    }

    function revokeAllOperatorPermissions(
        ISuperfluidToken _token,
        address _flowOperator
    ) external {
        ISuperfluid.Operation[] memory array = new ISuperfluid.Operation[](1);
        ...
        array[0] = ISuperfluid.Operation(
            ...
            abi.encode(
                abi.encodeWithSelector(
                    cfaV1.revokeFlowOperatorWithFullControl.selector,
                    _token,
                    _flowOperator,
                    new bytes(0) // placeholder - always pass in bytes(0)
                ),
