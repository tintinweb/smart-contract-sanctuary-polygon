...
contract stakeStorage is stakeEvents {
    ...
    function _getSetsBadgeForMerchant(bytes memory _badge) internal view returns(
        ...
        return (
            ...
            merchantsBadge[_badge].requireURI,
    ...
    function earn1155(bytes calldata _rank, uint256 tokenID) internal {
        ...
        MerchantStore memory store = 
            MerchantStore(msg.sender, tokenAddr, _rank, s_rank, tokenID, merchantsBadge[_rank].maxAmountOnDisputeToJoin,0, block.timestamp);
    ...
    function isQualifiedWithNFTs(bytes calldata _rank, address account, uint256 tokenID) public view returns(bool whatIhave, address iHave) {
        ...
        for(uint256 j; j < len; ) {
            ...
            if (bal != 0) {
                string memory set = IERC1155(merchantsBadge[_rank].tokenAddresses[j]).uri(tokenID);
                string memory setURI = IERC1155(merchantsBadge[_rank].tokenAddresses[j]).setUri(merchantsBadge[_rank].requireURI[j]);
