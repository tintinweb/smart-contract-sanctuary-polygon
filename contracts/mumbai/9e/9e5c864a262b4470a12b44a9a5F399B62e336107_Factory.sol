...
contract Factory is AccessControl, Initializable, BasicMetaTransaction {
    ...
    event ForwardCreated(address instance, string forwardId, uint256 amount,uint256 batchAmount);
    ...
    event NFTMinted(address _to, uint256 nftID, string uri);
    ...
    event MintTokens(address to, uint256 amount, address _token);
    event batchmintTokens(address[] to, uint256[] amount, address _token);

    event BurnTokens(address _userAddress, uint256 _amount, address _token);
    event batchBurn(address[] _userAddress, uint256[] amount, address _token);
    ...
    function init(
        ...
        // INFT(nftContract).grantRole(NFT_MINTER_ROLE,address(this));
    }
    ...
    function _clone(
        address impl,
        bytes32 _salt
    ) internal returns (address _cloned) {
        assembly {
            let ptr := mload(0x40)
            mstore(
                ptr,
    ...
    function _initProxy(
        ...
        (success, ) = proxy.call(
            abi.encodeWithSelector(0xb422d830, adminContract)
        );
        require(success, " Admin added as Agent");
    ...
    function createForward(
        ...
        forwardInstance = _clone(IAdmin(adminContract).masterForward(), _salt);
        IForward(forwardInstance).initialize(
            adminContract,
            IAdmin(adminContract).usdc(),
            _token,
            _participationTime,
            _settlementTime,
            _settlementPrice,
        ...
        address userIdentity = deployIdentity(forwardInstance, false);
        whitelistUser(_token, forwardInstance, _CountryCode, userIdentity);
        // deployAndWhitelist(_token, forwardInstance, _CountryCode);
        IToken(_token).transferFrom(_fund, forwardInstance, _amount);
        emit ForwardCreated(forwardInstance, _forwardId, _amount,batchAmount);
    }
    ...
    function tokenMint(address _to, uint256 _amount, address _token) external {
        require(tokenAgent[_token][_msgSender()] == true, "Not Agent");
    ...
    function tokenBurn(
        ...
        require(tokenAgent[_token][_msgSender()] == true, "Not Agent");
        IToken(_token).burn(_userAddress, _amount);
        emit BurnTokens(_userAddress, _amount, _token);
    }
    ...
    function batchBurnTokens(
        ...
        require(tokenAgent[_token][_msgSender()] == true, "Not Agent");
        IToken(_token).batchBurn(_userAddresses, _amounts);
        emit batchBurn(_userAddresses, _amounts, _token);
    }

    //---------------------------ROLES------------------------//

    function mintNFT(address to, string memory uri) external {
        uint256 nftID = INFT(nftContract).currentID();
        INFT(nftContract).safeMint(to, uri);
        emit NFTMinted(to, nftID, uri);
    }

    function addAgentOnToken(
        address _agent,
        address _token
    ) external onlyRole(Admin_Role) {
    ...
    function addAgentOnIdentityRegistry(
        address _agent,
        address _token
    ...
    function approvalForToken(
        ...
        address _spender,
        uint256 _amount,
        string memory _tnxID
    ) external returns (bool) {
        IToken(_token).approvalFromAdmin(_msgSender(), _spender, _amount);
        emit ApprovalForToken(_token, _msgSender(), _spender, _amount, _tnxID);
        return true;
    }

    function setNFTContract(
        address _address
    ) external validation(_address) onlyOwner {
...
interface IForward {
    ...
    function claimLeftToken(address _seller) external;
    function tokenAvailable() external returns(uint);
}
...
interface INFT is IAccessControl {
    ...
    function tokenURI(uint256 tokenId) external returns (uint256);
