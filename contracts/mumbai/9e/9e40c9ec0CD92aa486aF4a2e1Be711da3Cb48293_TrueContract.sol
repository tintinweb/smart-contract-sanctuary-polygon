...
// 60* m*h* d
    function createTicket(address _senderAddr,uint256 _senderBalance, address _recieverAddr, uint256 _recieverBalance, uint256 _expireDate ) public payable{
        ...
        tk.senderAddr = _senderAddr;
        tk.senderBalance = _senderBalance;
        tk.recieverAddr = _recieverAddr;
        tk.recieverBalance = _recieverBalance;
        tk.expireDate = _expireDate + block.timestamp;

        // token deposite
        IERC20(_senderAddr).transferFrom(msg.sender, address(this), _senderBalance);
        tk.exist = true;
        Ids.push(rnum);
    }
    ...
    function TransactionProccess(uint256 _id) public payable{
        ...
        require(!tk.expired, "IdNumber is expired");
         // token deposite
        IERC20(tk.recieverAddr).transferFrom(msg.sender, address(this), tk.recieverBalance);
        IERC20(tk.senderAddr).transfer(tk.sender, tk.senderBalance);
        IERC20(tk.recieverAddr).transfer(msg.sender, tk.recieverBalance);
        tk.retrived = true;
        tk.expired = true;
        tk.reciever = msg.sender;
    }
    ...
    function getMyTickets() view public returns(ticket[] memory){
        ticket[] memory tks;
        uint256 k = 0;
        for(uint i=0;i<Ids.length;i++){
            if(msg.sender == Tickets[Ids[i]].sender || msg.sender == Tickets[Ids[i]].reciever){
