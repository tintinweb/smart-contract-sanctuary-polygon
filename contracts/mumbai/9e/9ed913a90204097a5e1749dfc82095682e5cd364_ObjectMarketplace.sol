...
import "./modules/MarketplaceManager.sol";
...
contract ObjectMarketplace is IObjectMarketplace, Ownable {
    ...
    function createListing(
        ...
        //We check if the seller is the owner
    ...
    function updateListing(
        ...
        emit ListingUpdated(msg.sender, currency, _listing.nft, _listing.tokenId, price);
    }
    ...
    ///@notice buys nft from existing listing. Separates fee payment, royalty payment and amount for seller.
    function buyToken(uint256 listingId) external payable {
        Listing memory _listing =  listingById[listingId];
        uint256 marketplaceFeePoints = _marketplaceManager.getMarketplaceFee();
        address feeReceiver = _marketplaceManager.getFeeReceiver();
        ...
        emit ItemSold(_listing.nft,  _listing.seller, msg.sender, _listing.currency, _listing.price);
    }
...
interface IObjectMarketplace {
    ...
    event ItemSold(address nft, address seller, address buyer, address currency, uint256 price);
    ...
    event ListingUpdated(address seller, address currency, address nft, uint256 tokenId, uint256 price);
...
contract MarketplaceManager is IObjectMarketplaceManager, Ownable {
    ...
    mapping(address => bool) public authorizedObject;
    ...
    function deleteAuthorizedERC20(address token) external noZeroAddress(token) onlyOwner{
        ...
        ///@notice Adds ERC20 token to the list of authorized tokens to be used in the marketplace.
    function addAuthorizedObject(address newToken) public noZeroAddress(newToken) onlyOwner{

        if(authorizedObject[newToken] == true){
            revert AlreadyAuthorized();
        }
        authorizedObject[newToken] = true;

        emit AddedPermission(newToken, block.timestamp);
    }
    
    ///@notice delete previously authorized ERC20 token from the list of tokens allowed for trading.
    function deleteAuthorizedObject(address token) external noZeroAddress(token) onlyOwner{

        if(authorizedObject[token] == false){
            revert UnauthorizedObject();
        }

        authorizedObject[token] = false;
    ...
    function checkAuthorizedObject(address token) external view returns (bool){
        return authorizedObject[token];
    }
...
interface IObjectMarketplaceManager {
    ...
    function addAuthorizedObject(address newToken) external;
    function checkAuthorizedERC20(address token) external view returns (bool);
    function checkAuthorizedObject(address token) external view returns (bool);
    function deleteAuthorizedERC20(address token) external;
    function deleteAuthorizedObject(address token) external;
