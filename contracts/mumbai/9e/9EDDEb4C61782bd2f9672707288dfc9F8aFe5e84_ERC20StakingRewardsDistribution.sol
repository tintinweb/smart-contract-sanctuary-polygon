...
contract ERC20StakingRewardsDistribution is IERC20StakingRewardsDistribution {
    ...
    Reward[] public rewards;//奖励数组
    mapping(address => Staker) public stakers;//这是一个表，地址：质押者

    uint64 public startingTimestamp;//活动开始事件戳
    uint64 public endingTimestamp;//活动结束事件戳
    uint64 public lastConsolidationTimestamp;//？？？？？
    IERC20 public stakableToken;//质押代币的合约地址
    address public owner;//质押合约的owner，拥有者，拥有最高权限
    address public factory;
    bool public locked;//活动锁定标志
    bool public canceled;//活动取消标志
    bool public initialized;//活动初始化标志
    uint256 public totalStakedTokensAmount;//一共质押了代币数量
    uint256 public stakingCap;//质押容量的上限数量

    event OwnershipTransferred(//owner转移事件
        address indexed previousOwner,
        address indexed newOwner
    );
    event Initialized(//初始化事情
    ...
    event Canceled();//活动取消事件
    event Staked(address indexed staker, uint256 amount);//质押事件
    event Withdrawn(address indexed withdrawer, uint256 amount);//提现事件
    ...
    event UpdatedRewards(uint256[] amounts);//更新奖励事件

    function initialize(
        address[] calldata _rewardTokenAddresses,//奖励代币的合约地址
        address _stakableTokenAddress,//质押代币的合约地址
        uint256[] calldata _rewardAmounts,//奖励数量
        uint64 _startingTimestamp,//活动开始时间，时间戳
        uint64 _endingTimestamp,//活动结束时间，时间戳
        bool _locked,//锁定状态
        uint256 _stakingCap//质押容量
    ) external override onlyUninitialized {
        require(_startingTimestamp > block.timestamp, "SRD01");//开始时间戳一定要大于当前的时间戳
        require(_endingTimestamp > _startingTimestamp, "SRD02");//结束时间戳一定要大于开始时间戳
        require(_rewardTokenAddresses.length == _rewardAmounts.length, "SRD03");//奖励代币合约地址数组和奖励数量数组长度要保持一致
        require(_rewardTokenAddresses.length <= 5, "SRD27");//奖励代币的数量不能超过5个。
        ...
        for (uint8 _i = 0; _i < _rewardTokenAddresses.length; _i++) {
            ...
            for (uint8 _j = _i + 1; _j < _rewardTokenAddresses.length; _j++)//检查奖励的代币合约是否输入重复，有点儿多余。
                require(
            ...
            require(_rewardTokenAddress != address(0), "SRD04");//奖励代币的合约地址不能是0地址
            require(_rewardAmount > 0, "SRD05");//奖励代币的数量一定得大于0
            IERC20 _rewardToken = IERC20(_rewardTokenAddress);//将奖励代币的合约地址转换为，奖励代币erc20 object
            require(
                _rewardToken.balanceOf(address(this)) >= _rewardAmount,//本合约地址下的奖励代币数量一定要大于要奖励的数量
                "SRD06"
            );
            rewards.push(//将奖励详情推送到reawds数组
                Reward({
                    token: _rewardTokenAddress,//奖励代币的合约地址
                    amount: _rewardAmount,//奖励的数量
                    toBeRewarded: _rewardAmount * MULTIPLIER,//奖励的数量乘以一个系数，不知道为什么。
                    perStakedToken: 0,//因为是初始化，所以是0
                    recoverableAmount: 0,//因为是初始化，所以是0
                    claimed: 0//因为是初始化所以是0
                })
            );
        }

        require(_stakableTokenAddress != address(0), "SRD07");//质押代币合约地址也不能是0地址
        stakableToken = IERC20(_stakableTokenAddress);//将质押合约地址转换为，质押代币erc20 object

        owner = msg.sender;//将消息发送者设置为owner
        factory = msg.sender;//将消息发送者设置为factory
        startingTimestamp = _startingTimestamp;//将活动开始时间参数设置为状态变量：活动开始时间。
        endingTimestamp = _endingTimestamp;//将活动结束时间参数设置为状态变量：活动结束时间。
        lastConsolidationTimestamp = _startingTimestamp;//将开始时间参数设置为Consolidation状态变量。？？？？？？不太明白什么意思。
        locked = _locked;//将锁定状态参数设置为状态变量：锁定状态。
        stakingCap = _stakingCap;//将质押上限参数设置为状态变量：质押上限。
        initialized = true;//初始化状态设置为真，因为这个时候已经要初始化完毕了。
        canceled = false;//取消状态设置为false，因为现在只是在初始化。

        emit Initialized(//发送初始化事件
    ...
    function cancel() external override onlyOwner {//只有owner才可以调用这个活动取消函数
        require(initialized && !canceled, "SRD19");//必须已经初始化过了，并且没有活动处于没有取消的状态，如果没有初始化，
        require(block.timestamp < startingTimestamp, "SRD08");//并且现在活动还没有开始，才可以取消。
        canceled = true;//将取消状态设置为真。
        emit Canceled();//发送取消活动事件
    }

    function recoverRewardsAfterCancel() external override onlyOwner {//只有owner才可以调用，这个活动取消后，归还不止一种奖励的函数
        require(canceled, "SRD29");//只有活动取消以后，才可以归还奖励
        for (uint256 _i; _i < rewards.length; _i++) {
            Reward storage _reward = rewards[_i];
            IERC20(_reward.token).safeTransfer(//把本合约地址下的奖励代币，归还给owner
                owner,
    ...
    function recoverRewardAfterCancel(address _token)//只有owner才可以调用，这个活动取消侯，归还一种奖励的函数
        external
        override
        onlyOwner
    {
    ...
    function recoverUnassignedRewards()//收回未分配的奖励代币，只有owner才可以调用，只有活动开始以后才可以调用
        external
        override
        onlyOwner
        onlyStarted
    ...
    function exit(address _recipient) external override {//解质押并领取奖励，退出活动
        claimAll(_recipient);
    ...
    function stakedTokensOf(address _staker)//用户质押数量
        external
        view
        override
        returns (uint256)
    ...
    function renounceOwnership() external override onlyOwner {//放弃owner
        owner = address(0);
        emit OwnershipTransferred(owner, address(0));
    }

    function transferOwnership(address _newOwner) external override onlyOwner {//转移owner
