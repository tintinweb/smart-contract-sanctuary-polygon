...
interface IPair {
    ...
    function addLiquidity(
        uint _erc20TokenAmount,
        uint _resourceAmount
    ) external;

    function removeLiquidity(uint _percent) external;

    function getPairBalance() external view returns (uint, uint);
...
interface ISingleApproveTransfer {
    function takeERC20Token(address _sender, uint _darAmount) external;

    function takeERC1155Token(
        address _sender,
        uint _resourceId,
        uint _resourceAmount
    ) external;
...
import "src/roflSwapCore/interfaces/ISingleApproveTransfer.sol";
import "src/roflSwapCore/interfaces/IPair.sol";

import "./roflSwapPair.sol";
...
contract Factory is AccessControl {
    ...
    constructor(
        ...
        address _singleApproveTransfer
    ) {
    ...
    function createPair(uint _resourceId) public returns (address pair) {
        ...
        IPair(pair).init(
            ...
            erc1155Token,
            _resourceId,
            singleApproveTransfer,
            msg.sender
        );

        ISingleApproveTransfer(singleApproveTransfer).setPair(pair);
        ISingleApproveTransfer(erc1155Token).setPair(pair);
...
import "src/roflSwapCore/interfaces/IRewards.sol";
import "src/roflSwapCore/interfaces/ISingleApproveTransfer.sol";
...
contract Pair is ERC1155Holder, AccessControl, Pausable {
    ...
    event log_uint(string log, uint log2);
    event SwapResourceIn(
    ...
    modifier isLiquidityProvider() {
        require(
            hasRole(LIQUIDITY_PROVIDER, msg.sender),
            "Not LIQUIDITY_PROVIDER"
        );
        _;
    }
    ...
    function addLiquidity(
        ...
        IRewards(erc1155Token).mintBatch(
            address(this),
            resource,
            amount,
            "0x0"
    ...
    function removeLiquidity(uint _percent) public isLiquidityProvider {
        uint amounterc20TokenToRemove;
        uint[] memory amount = new uint[](1);

        if (_percent < 100) {
            amounterc20TokenToRemove =
                (IERC20(erc20Token).balanceOf(address(this)) * _percent) /
                100;
            amount[0] =
                (IERC1155(erc1155Token).balanceOf(address(this), tokenId) *
                    _percent) /
            ...
            amounterc20TokenToRemove = IERC20(erc20Token).balanceOf(
                address(this)
            );
            amount[0] = IERC1155(erc1155Token).balanceOf(
                address(this),
                tokenId
            );
        }

        IERC20(erc20Token).transfer(msg.sender, amounterc20TokenToRemove);

        uint[] memory resource = new uint[](1);
        resource[0] = tokenId;

        IRewards(erc1155Token).burnBatch(address(this), resource, amount);
    ...
    function swap(
        ...
        if (_resourceOut > 0) {
            uint erc20TokenToPay = (kMultiplier /
                (IERC1155(erc1155Token).balanceOf(address(this), tokenId) -
                    _resourceOut)) -
                IERC20(erc20Token).balanceOf(address(this));
            emit log_uint("erc20TokenToPay", erc20TokenToPay);
            require(erc20TokenToPay > 0, "Not enough Liquidity");
            require(erc20TokenToPay <= _erc20TokenAmount, "Slippage Failure");

            ISingleApproveTransfer(singleApproveTransfer).takeERC20Token(
                msg.sender,
                erc20TokenToPay
            );
            ...
            uint erc20TokenToGet = IERC20(erc20Token).balanceOf(address(this)) -
                (kMultiplier /
                    (IERC1155(erc1155Token).balanceOf(address(this), tokenId) +
                        _resourceIn));

            require(erc20TokenToGet > 0, "Not enough Liquidity");
            require(erc20TokenToGet >= _erc20TokenAmount, "Slippage Failure");

            ISingleApproveTransfer(singleApproveTransfer).takeERC1155Token(
                msg.sender,
                tokenId,
                _resourceIn
            );

            IERC20(erc20Token).transfer(msg.sender, erc20TokenToGet);
