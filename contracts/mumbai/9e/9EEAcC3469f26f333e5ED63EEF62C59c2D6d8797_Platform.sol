    ...
    IPlatform
{
    ...
    function pay(
        ...
        address token_,
        uint256 amount_
    ) external payable override onlyUserRegistered(user_) {
    ...
    function withdraw(
        ...
        (address[] memory users, uint256[] memory amounts) = extension.withdraw(
            _msgSender(),
            id_,
            users_,
            token_,
        ...
        for (uint256 i = 0; i < users.length; i++)
            _increaseReward(users[i], token_, amounts[i]);
    }
