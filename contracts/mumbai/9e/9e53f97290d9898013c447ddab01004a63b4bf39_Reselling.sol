...
contract Reselling is Ownable, Auctions, Pausable{

    using EnumerableSet for EnumerableSet.UintSet;

    enum ReSaleType {fixedPrice, auction}

    event EventPkgCreatedWithFixedPrice(uint eventPkgId);
    event EventPkgCreatedWithAuction(uint eventPkgId, uint auctionId);
    
    event EventPkgDeleted(EventPkg pkg);
    ...
    //mapping(seller => tokenId => amount)
    mapping(address => mapping(uint => uint)) public _lockedCopies;

    mapping(uint => EventPkg) public _eventPkgDetails;
    mapping(address => EnumerableSet.UintSet) _sellersCreatedEventPkgs;

    //mapping(pkgId => auctionId)
    mapping(uint => uint) public _eventPkgWithAuction;


    constructor(address mainContractAddress, address ticketsERC1155Contract){    
        require(mainContractAddress.code.length > 0, "Reselling: Invalid main contract address");
        require(ticketsERC1155Contract.code.length > 0, "Reselling: Invalid ticket1155 contract address");
        
        _mainContract = Main(mainContractAddress);
        _ticketsERC1155Contract = TicketsERC1155(ticketsERC1155Contract);

        require(msg.sender == _mainContract.owner(), "Reselling: Main and Reselling contract have different owners");
        require(msg.sender == _ticketsERC1155Contract.owner(), "Reselling: Tickets and Reselling contract have different owners");
    }

    modifier onlyAllowed(address caller){
        require(caller == address(_mainContract) || caller == owner(), "Reselling: Caller is not authorized");
        _;
    }

    modifier isValidMainContract(){
        require(address(_mainContract) != address(0), "Reselling: Main contract is null");
        _;
    }

    function setMainContract(address mainContractAddress) public whenNotPaused onlyOwner{
        require(mainContractAddress.code.length > 0, "Reselling: Invalid contract address");
        _mainContract = Main(mainContractAddress);
    ...
    function setTicketContract(address ticketContractAddress) public whenNotPaused onlyOwner{
        require(ticketContractAddress.code.length > 0, "Reselling: Invalid contract address");
        _ticketsERC1155Contract = TicketsERC1155(ticketContractAddress);

        require(msg.sender == _ticketsERC1155Contract.owner(), "Reselling: Ticket1155 and Reselling contract have different owners");
    }

    function createEventPackageWithFixedPrice(EventPkg calldata pkg) public whenNotPaused {
        isValidEventPkg(pkg);
        isEligibleForPkgCreation(pkg);

        require(pkg.reSaleType == ReSaleType.fixedPrice, "Reselling: Re-sale type should be fixed-price");

        addNewEventPkgDetails(pkg);

        emit EventPkgCreatedWithFixedPrice(_eventPkgId);
    }

    function createEventPackageWithAuction(EventPkg calldata pkg, Auction calldata auction) public whenNotPaused {
        isValidEventPkg(pkg);
        isEligibleForPkgCreation(pkg);

        require(pkg.seller == auction.owner, "Reselling: Different owners in package and auction details");
        require(pkg.reSaleType == ReSaleType.auction, "Reselling: Re-sale type should be auction");

        uint auctionId = createAuction(auction);

        addNewEventPkgDetails(pkg);
        _eventPkgWithAuction[_eventPkgId] = auctionId;
        
        emit EventPkgCreatedWithAuction(_eventPkgId, auctionId);
    }

    function addNewEventPkgDetails(EventPkg calldata pkg) private{
        _eventPkgId++;
        _eventPkgDetails[_eventPkgId] = pkg;
        _sellersCreatedEventPkgs[pkg.seller].add(_eventPkgId);
        lockUserCopies(pkg);
    }
    ...
    function updateEventPkg(uint eventPkgId, EventPkg calldata updatedPkg) public whenNotPaused{
        ...
        require(msg.sender == _eventPkgDetails[eventPkgId].seller, "Reselling: Not an owner of package");
        require(_eventPkgDetails[eventPkgId].reSaleType == updatedPkg.reSaleType, "Reselling: Re-sale type cannot be updated");
    ...
    function deleteEventPkg(uint eventPkgId) public whenNotPaused{
        ...
        if(_eventPkgDetails[eventPkgId].reSaleType == ReSaleType.auction){
            deleteAuction(_eventPkgWithAuction[eventPkgId]);
            _eventPkgWithAuction[eventPkgId] = 0;
        }

        unlockUserCopies(_eventPkgDetails[eventPkgId]);
            
        EventPkg memory pkg = _eventPkgDetails[eventPkgId];
        _sellersCreatedEventPkgs[pkg.seller].remove(eventPkgId);

        delete _eventPkgDetails[eventPkgId];
        emit EventPkgDeleted(pkg);
    }
    
    function buyEventPkgWithFixedPrice(uint eventPkgId) public whenNotPaused payable{
        checkEventPkgExists(eventPkgId);
        require(msg.sender != _eventPkgDetails[eventPkgId].seller, "Reselling: Cannot buy your own package");
        checkUserBalance(_eventPkgDetails[eventPkgId].seller, _eventPkgDetails[eventPkgId].ticketTypeIds, _eventPkgDetails[eventPkgId].copies);
        require(_eventPkgDetails[eventPkgId].reSaleType == ReSaleType.fixedPrice, "Reselling: Package resale type is not fixed price");
        ...
        require(msg.value == serviceFee + royaltyFee + pkg.price, "Reselling: Insufficient amount to buy package");
        ...
        payable(_eventPkgDetails[eventPkgId].seller).transfer(pkg.price);
    ...
    function endAuctionForEventPkg(uint eventPkgId) public whenNotPaused{
        checkEventPkgExists(eventPkgId);
        require(msg.sender == _eventPkgDetails[eventPkgId].seller, "Reselling: Not an owner of package");
        checkUserBalance(_eventPkgDetails[eventPkgId].seller, _eventPkgDetails[eventPkgId].ticketTypeIds, _eventPkgDetails[eventPkgId].copies);
        require(_eventPkgDetails[eventPkgId].reSaleType == ReSaleType.auction, "Reselling: Package is not on auction");

        EventPkg memory pkg = _eventPkgDetails[eventPkgId];
        Event memory eventDetails = _mainContract.getEventDetails(pkg.eventId);
        Auction memory auction = _auctionDetails[_eventPkgWithAuction[eventPkgId]];

        require(auction.lastBidder != address(0), "Reselling: Can't end auction having no bid");
        ...
        _sellersCreatedEventPkgs[pkg.seller].remove(eventPkgId);
        payable(_eventPkgDetails[eventPkgId].seller).transfer(auction.maxBid - (serviceFee + royaltyFee));

        endAuction(_eventPkgWithAuction[eventPkgId]);
        delete _eventPkgDetails[eventPkgId];
        emit EventPkgDeleted(pkg);
    ...
    function isValidEventPkg(EventPkg calldata pkg) private view{
        ...
        if (pkg.seller == address(0) || pkg.seller != msg.sender || pkg.price == 0 || pkg.ticketTypeIds.length == 0 || pkg.copies.length == 0){
            revert("Reselling: Got invalid event package");
        }

        checkTicketTypeAndCopiesArrayLength(pkg);
        // require(pkg.copies.length == pkg.ticketTypeIds.length, "Reselling: Ticket type ids and copies arrays have different length");
        ...
        // checkUserBalance(pkg.seller, pkg.ticketTypeIds, pkg.copies);
    }

    function lockUserCopies(EventPkg calldata pkg) private{
        for (uint i; i < pkg.ticketTypeIds.length; i++){
            _lockedCopies[pkg.seller][pkg.ticketTypeIds[i]] += pkg.copies[i];
        }
    }
    ...
    function getSellersCreatedEventPkgs(address seller) public view returns(uint[] memory){
        return _sellersCreatedEventPkgs[seller].values();
    }

    function checkUserBalance(address user, uint[] memory ticketTypeIds, uint[] memory copies) private view{

        for (uint i; i < ticketTypeIds.length; i++){
            require(copies[i] > 0, "Reselling: Got invalid number of copies");
            require(_ticketsERC1155Contract.balanceOf(user, ticketTypeIds[i]) >= copies[i], "Reselling: Seller has insufficient token balance");
        }
    }
    ...
    function isEligibleForPkgCreation(EventPkg calldata pkg) private view {
        ...
        for (uint i; i < pkg.ticketTypeIds.length; i++){
            ...
            uint lockedCopies = _lockedCopies[pkg.seller][pkg.ticketTypeIds[i]];

            require(pkg.copies[i] > 0, "Reselling: Got invalid number of copies");
            require(_ticketsERC1155Contract.balanceOf(pkg.seller, pkg.ticketTypeIds[i]) >= (lockedCopies + pkg.copies[i]), "Reselling: Not eligible for creating package");
        }

    }

    function checkEventPkgExists(uint eventPkgId) private view{
        require(_eventPkgDetails[eventPkgId].seller != address(0), "Reselling: Event package dont exists");
    }
