...
contract MainController is   Ownable  , AccessControl , Pausable{
  ...
  /// @dev Add an account to the admin role. Restricted to admins.
    ...
    function createProposal (  ProposalDB.Proposal memory proposal ) public payable   whenNotPaused    returns (  uint256 )
    {
        
        require( erc20db.checkERC20Info(proposal.token) == true , "E1" );
        require( proposal.fundsRequested >= minFundrequested , "E2" );
        require ( proposal.fundsRequested.mod(minFundBase) == 0 , "E3"   );
        require ( msg.value == registrationFee , "E6") ;
        if ( proposal.premium == true )
        {
        ...
        //IERC20 token_contract = IERC20(token) ;
        uint256 fundsAllocatedAfterTax = proposal.fundsRequested.sub( proposal.fundsRequested.mul(fee).div(100) )  ; 
        proposal.fundsAllocatedAfterTax = fundsAllocatedAfterTax ; 
        proposal.debtBased = true ; 
        uint256 index = proposaldb.createProposal( proposal );
    ...
    function createInvestment (   address token  , uint256 fundsDeposit , string memory details , uint256 proposalID   ) public   whenNotPaused    returns (  uint256 )
    {
        ...
        if ( p.Locked == 2 )
        {
            require ( block.timestamp >= p.lockL2StartDate  &&  block.timestamp  <= p.lockL2EndDate  , "E12"   ) ; 
        }
     ...
     function createDAOInvestment (   address token  , uint256 fundsDeposit , string memory details , uint256 proposalID   ) public  onlyContractOrAdmin whenNotPaused      returns (  uint256 )
    {
        ...
        require ( p.token == token , "E3" );
        require ( p.enabled == true , "E4"  );
        require ( p.Locked == 3 , "E5"   ) ; 
        require ( p.didPass == true , "E6"   ) ;
        require ( p.aborted == false , "E7"   ) ; 


        require ( (  bankdb.getProposalFundInfo(proposalID).propsoalFund.add(fundsDeposit) ) == p.fundsRequested   , "E10"   ) ; 


        if ( p.Locked == 3 )
        {
            require ( block.timestamp >= p.lockL3StartDate  &&  block.timestamp  <= p.lockL3EndDate  , "E12"   ) ; 
        }
    ...
    function withdrawProfitAndBalance (  uint256 bankID   ) public   whenNotPaused    returns (  uint256  , uint256)
    {
        uint256 proposalID = bankdb.getBankAccount(bankID).proposalID ; 
        //require ( erc20db.checkERC20Info(token) == true , "E1" );
        ...
        require ( p.enabled == true , "E1" ) ;

        //require ( proposaldb.getProposal(  proposalID ).token == token , "E3" );
        require ( p.Locked > 1 , "E2"   ) ; 
        require ( p.didPass == true , "E3"   ) ;
        //require ( proposaldb.getProposal(  proposalID ).aborted == false , "E5"   ) ; 
        require ( bankdb.getBankAccount( bankID ).investor == msg.sender , "E4"   ) ;
        require ( p.processed == true , "E5"   ) ;
        require ( p.paid == true , "E12"   ) ;
        require ( p.fundReturned == true , "E13"   ) ;
        ...
        //return (bankdb.getBankAccount( bankID ).fundsDeposit) ;
        return ( profit , bankdb.getBankAccount( bankID ).fundsDeposit ) ;
    ...
    function checkInvestmentFund (   uint256 proposalID   ) public        returns (  bool )
    {
        ...
        //require ( proposaldb.getProposal(  proposalID ).token == token , "E3" );
        //require ( investordb.getInvestor(msg.sender).enabled == true , "E4"  );
        require ( p.processed == true , "E2"   ) ;
        require ( p.Locked == 2 ||  p.Locked == 3 , "E3"   ) ; 
        require ( p.didPass == true , "E4"   ) ;
        require ( p.aborted == false , "E5"   ) ; 
