...
interface IERC20TestnetMintable is IAccessControl {
    function mintTo(address to_, uint256 amount_) external;
}

contract ERC20Mintable is ERC20, AccessControl, IERC20TestnetMintable {

    constructor (string memory name_, string memory symbol_) ERC20(name_, symbol_) {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
    }

    function mintTo(address to_, uint256 amount_) external onlyRole(DEFAULT_ADMIN_ROLE) override {
        _mint(to_, amount_);
    }
}
