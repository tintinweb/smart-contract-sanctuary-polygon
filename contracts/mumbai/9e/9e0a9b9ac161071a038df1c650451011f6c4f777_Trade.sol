...
contract Trade {
    ...
    function verifySellerSign(address seller, bytes memory _orderItems, address paymentAssetAddress, Sign memory sign) internal pure {
    ...
    function getFees( Order memory order) public view returns(Fee memory){
        ...
        require(price >= (royaltyFee + sellerFee), "subtraction overflow");
    ...
    function buyAsset(Order memory order, uint256 assetlength, Sign memory sign) public returns(bool) {
        ...
        verifySellerSign(order.seller, order._orderItems, order.erc20Address, sign);
    ...
    function updatingNonce(Order memory order, uint256 assetlength, uint256 nonce) internal returns(bool) {
        ...
        if(!(nftListStatus[order.seller][order._orderItems])) {
            nftlist[order.seller][order._orderItems] = assetlength;
