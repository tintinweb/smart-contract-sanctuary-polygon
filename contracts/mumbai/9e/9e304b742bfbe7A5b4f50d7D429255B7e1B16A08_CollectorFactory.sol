...
contract CollectorFactory is Ownable, ReentrancyGuard, AccessControlEnumerable, IFactory, Pausable {
    ...
    bytes32 hash_oec_rate = keccak256(abi.encodePacked("oec_rate"));
    bytes32 hash_uru_rate = keccak256(abi.encodePacked("uru_rate"));
    ...
    struct Collector {
        ...
        uint256 count;               //实际数量
        bytes32 margin_name;         //余量属性名称哈徐, 总和减去其它
    }
  ...
  // MANAGING Authorization Contract Address for Call 
   ...
   // 合约部署者增加操作员
        ...
        returns(uint[] memory)
    {
        ...
        props[hash_duration]  = 0;       
        props[hash_oec_rate]  = 0;      
        props[hash_uru_rate]  = 0;    
        props[hash_scan]      = 0;    
        uint256 potential; 
        ...
        require(
            collectors[_type_id].margin_name == hash_duration ||  
            collectors[_type_id].margin_name == hash_oec_rate ||  
            collectors[_type_id].margin_name == hash_uru_rate ||  
            collectors[_type_id].margin_name == hash_scan, 
            "wrong margin_name"
        );
        ...
        if (settings[_type_id][hash_oec_rate].section > 0) {
            new_rand = randomResult.sub(randomResult%100).div(100);
            props[hash_oec_rate]  = settings[_type_id][hash_oec_rate].base + new_rand % settings[_type_id][hash_oec_rate].section;
        }
        
        if (settings[_type_id][hash_uru_rate].section > 0) {
            new_rand = randomResult.sub(randomResult%1000).div(1000);
            props[hash_uru_rate]  = settings[_type_id][hash_uru_rate].base + new_rand % settings[_type_id][hash_uru_rate].section;
        }

        if (settings[_type_id][hash_scan].section > 0) {
            new_rand = randomResult.sub(randomResult%10000).div(10000);
            props[hash_scan]  = settings[_type_id][hash_scan].base + new_rand % settings[_type_id][hash_scan].section;
        }
        
        //余量
        //不做溢出检测
        unchecked {
            uint margin = collectors[_type_id].prop_sum - (props[hash_duration] + props[hash_oec_rate] + props[hash_uru_rate] +props[hash_scan]);
        ...
        characters[_id] = Character(
                ...
                props[hash_oec_rate],    //源晶采集效率
                props[hash_uru_rate],    //钨矿采集效率
                props[hash_scan] ,       //扫描 
                potential                //潜力
        );
        ...
        ch[0] =  props[hash_duration];
        ch[1] =  props[hash_oec_rate];
        ch[2] =  props[hash_uru_rate];
        ch[3] = props[hash_scan];
        ch[4] =  potential;
        ...
        returns(uint[] memory)
    {
        ...
        if ( randomResult % 4 == 0) {
           ch_new.duration = ch.duration.add(ch.duration.mul(newPercent).div(100));
        } else if ( randomResult % 4 == 1) {
            ch_new.oec_rate = ch.oec_rate.add(ch.oec_rate.mul(newPercent).div(100));
        } else if ( randomResult % 4 == 2) {
            ch_new.uru_rate = ch.uru_rate.add(ch.uru_rate.mul(newPercent).div(100));
        } else {
            ch_new.scan =ch.scan.add(ch.scan.mul(newPercent).div(100));
        }

        uint[] memory charactor = new uint[](4);
        charactor[0] =  ch_new.duration;
        charactor[1] =  ch_new.oec_rate;
        charactor[2] =  ch_new.uru_rate;
        charactor[3] =  ch_new.scan;
