...
contract DirtyPix is ERC721, ERC2981, DefaultOperatorFilterer, Ownable {
    ...
    uint256 public constant PUBLIC_MINT_PRICE = 3.9 ether;

    bool public isPublicMintOpen = false;
    bool public isWhiteListMintOpen = false;
    mapping(address => bool) public whiteListMinted;

    constructor(string memory _baseTokenURI) ERC721("DirtyPix", "DP") {
        setBaseTokenURI(_baseTokenURI);
        _setDefaultRoyalty(msg.sender, 900);
    }

    function setRoyaltyFee(uint96 _fee) public onlyOwner {
    ...
    function publicMint(uint256 _quantity) external payable {
        require(isPublicMintOpen, "Public mint closed");
        require(
            msg.value >= (PUBLIC_MINT_PRICE * _quantity),
            "Not enough funds"
        );
        require((totalSupply() + _quantity) <= MAX_SUPPLY, "Not enough supply");
    ...
    function whiteListMint(bytes32[] calldata _proof) external payable {
        require(isWhiteListMintOpen, "Whitelist mint closed");
        require(totalSupply() < MAX_SUPPLY, "Not enough supply");
        require(whiteListMinted[msg.sender] == false, "Already minted");
        require(verifyWhiteListProof(_proof, msg.sender), "Not whitelisted");
        whiteListMinted[msg.sender] = true;
        _mint(msg.sender);
    }

    function freeMint(uint256 _quantity) external onlyOwner {
        require((totalSupply() + _quantity) <= MAX_SUPPLY, "Not enough supply");
        for (uint256 i = 1; i <= _quantity; i++) {
            _mint(msg.sender);
        }
    ...
    function _mint(address _to) private {
        _tokenIdCounter.increment(); // tokenId starts at 1
        _safeMint(_to, _tokenIdCounter.current());
    }

    function setPublicMintOpen(bool _publicOpen) external onlyOwner {
        isPublicMintOpen = _publicOpen;
    }

    function setWhiteListMintOpen(bool _whiteListOpen) external onlyOwner {
        isWhiteListMintOpen = _whiteListOpen;
    }
