...
contract YexleAppian is ERC721Burnable, Ownable {
    ...
    */
    ...
    /** 
        ...
        *@param _requester - address of the seller/owner of the land
        *@param oldOwner - address of the buyer
    ...
    */
    function ownerDecisionforRaisedRequest(address _requester, address oldOwner, uint _tokenId, bool _status) external onlyAdmin{
        UserContract useC = UserContract(userContract);
        (bool status) = useC.verifyUser(_requester);
        if(!status){ revert userContractError();}
