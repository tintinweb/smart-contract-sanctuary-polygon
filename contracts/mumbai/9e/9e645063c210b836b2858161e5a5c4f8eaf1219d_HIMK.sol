...
contract HIMK is Ownable {
    ...
    function setWhiteListSet(address[] calldata _buyerList, bool _status) external onlyOwner {
        for (uint256 i = 0; i < _buyerList.length; i++) {
            whiteListSet[_buyerList[i]] = _status;
        }
    }
    ...
    function takeSellFee(address sender, address recipient, uint256 amount,bool _takeFee) private {
        ...
        if (marketFee>0) {
            ...
            emit Transfer(sender, address(this), marketFee);
        }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (
            ...
            emit Transfer(sender, shareAddress, minAddPoolAmount);
        }
