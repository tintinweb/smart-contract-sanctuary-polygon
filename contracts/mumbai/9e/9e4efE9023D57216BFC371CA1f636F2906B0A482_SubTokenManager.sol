...
import "./ISubTokenManager.sol";
import "./ISubTokenDescriptor.sol";

contract SubTokenManager is ISubTokenManager, ERC721Enumerable, Ownable {

    address public manager;
    address public merchantToken;
    address public planManager;
    address public subInfoManager;
    address public subTokenDescriptor;
    ...
    constructor() ERC721("S10N Sub Token", "SSB") {
    }
    ...
    function setSubTokenDescriptor(address descriptor) external onlyOwner override {
        require(descriptor != address(0), "descriptor address invalid");
        subTokenDescriptor = descriptor;
    }
    ...
    function mintSubToken(address tokenOwner) external onlyManager override returns (uint) {
        uint tokenId = nextTokenId;
        _mint(tokenOwner, tokenId);
        return tokenId;
    }

    function tokenURI(uint tokenId) public view override returns (string memory uri) {
        uri = ISubTokenDescriptor(subTokenDescriptor).tokenURI(merchantToken, planManager, subInfoManager, tokenId);
    }
