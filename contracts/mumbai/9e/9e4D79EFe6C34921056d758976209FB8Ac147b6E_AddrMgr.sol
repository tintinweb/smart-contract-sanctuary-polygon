...
contract AddrMgr {
    ...
    modifier modcanDo(bytes32 FuncName) {
        require(_canDo(FuncName, msg.sender), "Not Authorized.");
        _;
    }
    function get(bytes32 ContractName) public view returns (address) {
        // If the value was never set, it will return the default value.
        address PANICAuthMgr = 0xe34924bb6F62d92F2f8432204d739b9c8f5cf6c3;  //ToDo: Impostare l'indirizzo di un AuthMgr con Address e Permessi su indirizzi Base Speciali
    ...
    function Remove(bytes32 ContractName) external virtual modcanDo("Remove") {
        if(DBKeys.length > 1) {
            ...
            if(Ix == DBKeys.length) { return; } DBKeys[Ix] = DBKeys[DBKeys.length - 1];  DBKeys.pop();
        }
    ...
    function Clear() external virtual modcanDo("Clear") {
    ...
    function canDo(bytes32 FuncName, address Sender) external view returns (bool) { return _canDo(FuncName, Sender); }
    function _canDo(bytes32 FuncName, address Sender) internal view returns (bool) {
        try IAuthLib(get("AuthMgr")).canDo(SMName, FuncName, Sender) returns (bool res) { return res; }
        catch {
