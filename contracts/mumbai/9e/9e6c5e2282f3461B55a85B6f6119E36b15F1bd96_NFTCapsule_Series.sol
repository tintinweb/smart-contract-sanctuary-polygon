...
contract NFTCapsule_Series is Pausable {
    NFTCore NFTcore;

    uint256 seriesId = 101; // 套圖編號
    uint256 picAmount = 200; // 圖片數量
    uint256 fragmentAmount = 50; // 碎片數量
    uint256 maxFragmentAmount = 10; // 每個碎片的發行量
    ...
    uint256 price = 20000000; // 售價，divide 1e6
    uint256 startTime = 0;
    uint256 endTime = 0;
    ...
    function getTokenId() external returns (uint256 tokenId) {
        ...
        // 取得隨機編號
        uint256 picId = ((uint160(msg.sender) + block.timestamp + counter) % picAmount) + 1;
        uint256 fragmentId = ((uint160(msg.sender) + block.timestamp + totalSupply) % fragmentAmount) + 1;
    ...
    function checkFragment(
        ...
        uint256 _seriesId,
        uint256 _picId
    ) external returns (uint256 _tokenId, bool _verified) {
        require(_seriesId == seriesId, 'Wrong Series');
        require(_picId <= picAmount, 'Exceed pic amount');

        _tokenId = _buildTokenId(_picId, 0);
        for (uint256 tokenId = _tokenId + 1; tokenId <= _tokenId + 50; tokenId++) {
            require(fragmentMintAmount[tokenId] > 0, 'Wrong picture ID');
            require(NFTcore.balanceOf(_address, tokenId) >= 1, "Don't have enough fragments");
        }
        return (_tokenId, true);
    }

    //NOTE:上線前拿掉
    function getTestTokenId(uint256 picId, uint256 fragmentId) external returns (uint256 tokenId) {
        tokenId = _buildTokenId(picId, fragmentId);
        picMintAmount[picId]++;
        fragmentMintAmount[tokenId]++;
        totalSupply++;
