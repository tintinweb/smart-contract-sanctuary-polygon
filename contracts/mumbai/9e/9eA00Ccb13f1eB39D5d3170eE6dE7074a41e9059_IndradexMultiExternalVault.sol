...
abstract contract IndradexBaseVault is Ownable, ERC20 {
  ...
  constructor(address newOwner, string memory name_, string memory symbol_, address payable inputToken_,
                address[] memory outputTokens_, address[] memory aggregators_, uint256 withdrawalTime) ERC20(name_, symbol_) {
    ...
    _setWithdrawalTime(withdrawalTime);
    _transferOwnership(newOwner);
  }
  ...
  function tokensBalances() external view returns(TokenBalances[] memory){
    ...
    for (uint i=0; i<tokens.length(); i++) {
      address currentToken = tokens.at(i);
      uint256 _internalBalance = tokenInternalBalance(currentToken);
      uint256 _externalBalance = tokenExternalBalance(currentToken);
      uint256 _pendingBalance = tokenPendingBalance(currentToken);
      int256 _totalBalance = _internalBalance.toInt256() + _externalBalance.toInt256() - _pendingBalance.toInt256();
  ...
  function calculateMintAmount(uint256 amount, uint256 totalSupply) internal view returns(uint256){
    ...
    return totalSupply.mul(amount).div(scaledTotalTokensPrice);
  }
  ...
  function deposit(uint256 amount) external {
    ...
    uint256 mintAmount;
    uint256 scaledAmount = amount * 10 ** (18 - tokenInfo[address(inputToken)].tokenDecimals);
    uint256 totalSupply = totalSupply();
    if(totalSupply==0){
      mintAmount = scaledAmount;
    }
    else{
      mintAmount = calculateMintAmount(scaledAmount, totalSupply);
    }
  ...
  */
  function requestWithdrawal(uint256 amount) external{
    // TODO: add require that user has enough tokens
    address sender = msg.sender;
    uint256 tokensLength = tokens.length();
    uint256 percentage = amount.div(totalSupply()); 
    _burn(sender, amount);
    ...
    for(uint i=0; i<tokensLength; i++){
      ...
      tokenAmount = percentage.mul(tokenTotalBalance(currentToken));
  ...
  */
  function releaseAdmin(address userAddress, uint256[] calldata withdrawals) external onlyOwner {
    _release(userAddress, withdrawals);
  }
  ...
  */
  function releaseUser(uint256[] calldata withdrawals) external {
    ...
    for(uint i=0; i<withdrawals.length; i++){
      require(pendingUsersWithdrawal[sender][withdrawals[i]].endingTime <= block.timestamp, "Vault: withdrawal time has not passed since withdrawal request");
    }
    _release(sender, withdrawals);
  }
   ...
   * Internal function without access restriction.
   */
  function _release(address userAddress, uint256[] calldata withdrawals) internal {
    PendingUsersWithdrawal memory withdrawal;
    address currentToken;
    ...
    for(uint i=0; i<withdrawals.length; i++){
      ...
      for(uint j=0; j<tokens.length(); j++){
        ...
        currentTokenAmount = withdrawal.tokenAmounts[j];
        // TODO: vale la pena controllare se current token amount > 0 per risparmiare gas?
...
contract IndradexMultiExternalVault is IndradexBaseVault {
  ...
  /**
    ...
    */
  constructor(address newOwner, string memory name_, string memory symbol_, address payable inputToken_,
                address[] memory outputTokens_, address[] memory aggregators_, uint256 withdrawalTime) 
      IndradexBaseVault(newOwner, name_, symbol_, inputToken_, outputTokens_, aggregators_, withdrawalTime) {}

  /**
    * @dev Returns the external balance for a token.
    */
  function tokenExternalBalance(address token) override internal view returns(uint256){
  ...
  /**
    ...
    */
  function setExternalBalances(uint256[] calldata tokensExternalBalances) external onlyOwner {
    require(tokensExternalBalances.length == tokens.length(), "Vault: tokensExternalBalances has wrong length");
    for (uint i=0; i<tokens.length(); i++) {
      tokenInfo[tokens.at(i)].externalBalance = tokensExternalBalances[i];
    }
  }
  ...
  /**
    ...
    */
  function setTokenExternalBalance(address token, uint256 newExternalBalance) external onlyOwner onlyPresentToken(token) {
