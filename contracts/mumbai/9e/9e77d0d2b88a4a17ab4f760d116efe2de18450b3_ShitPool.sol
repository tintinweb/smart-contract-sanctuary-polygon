...
// File contracts/DYPRToken.sol

pragma solidity 0.8.13;

contract DYPRToken is ERC20, Ownable {
    uint256 public devFee = 2;
    uint256 public ownerFee = 1;
    uint256 public sellFee = 40;
    address public shitPoolAddress;
    ...
    modifier onlyShitPool() {
        require(
            shitPoolAddress == _msgSender(),
            "Only ShitPool can call this function"
        );
        _;
    }

    constructor() ERC20("Diapers Token", "DYPR") {
    ...
    function _transfer(
        ...
        } else if (from == pairAddress || to == pairAddress) {
            //BUYING OR SELLING
            if (from == pairAddress) {
                //BUYING
                (
                ...
                if ((to != ownerWallet) && (to != devWallet)) {
                    //Dev and Owner can't create stakes
                    addStake(to, _restAmount, block.timestamp);
                }
                super._transfer(from, ownerWallet, _ownerAmount);
                super._transfer(from, devWallet, _devAmount);
                super._transfer(from, to, _restAmount);
            } else if (to == pairAddress) {
                //SELLING
                (
                    uint256 _ownerAmount,
                    uint256 _devAmount,
                    uint256 _toShitPoolAmount,
                    uint256 _restAmount
                ) = getSellTax(amount);
                if ((from != ownerWallet) && (from != devWallet)) {
                    //Dev and Owner can't create stakes
                    require(balanceOf(from) - amount >= 0, "Negative amount");
                    resetStakes(
                        from,
                ...
                super._transfer(from, ownerWallet, _ownerAmount);
                super._transfer(from, devWallet, _devAmount);
                super._transfer(from, shitPoolAddress, _toShitPoolAmount);
                super._transfer(from, to, _restAmount);
            }
        } else if (from == shitPoolAddress) {
            //CLAIMING
            resetStakes(to, balanceOf(to) + amount, block.timestamp);
            super._transfer(from, to, amount);
        } else {
            require(balanceOf(from) - amount >= 0, "Negative amount");
            resetStakes(from, balanceOf(from) - amount, block.timestamp);
            addStake(to, amount, block.timestamp);
            super._transfer(from, to, amount);
        }
    }
    ...
    function addStake(
        ...
        users[_userAddress].push(Stake(_amount, _timestamp));
    }
    ...
    {
        count = users[_userAddress].length;
    }

    function getUserStake(address _userAddress, uint256 _index)
        external
        view
        returns (uint256 amount, uint256 timestamp)
    {
        amount = users[_userAddress][_index].amount;
        timestamp = users[_userAddress][_index].timestamp;
    }
    ...
    {
        stakes = users[_userAddress];
    }

    function getUserMatureStakes(address _userAddress)
        external
        view
        returns (uint256 matureStakes)
    {
        for (uint256 i = 0; i < users[_userAddress].length; i++) {
            if (block.timestamp - users[_userAddress][i].timestamp > 1 days) {
                matureStakes += users[_userAddress][i].amount;
            }
        }
    }

    function getBuyTax(uint256 _amount)
        public
        view
        returns (
            uint256 ownerAmount,
    ...
    {
        (, ownerAmount) = calculateTax(_amount, ownerFee);
        (, devAmount) = calculateTax(_amount, devFee);
        restAmount = _amount - ownerAmount - devAmount;
    }

    function getSellTax(uint256 _amount)
        public
        view
        returns (
            uint256 ownerAmount,
            uint256 devAmount,
            uint256 toShitPoolAmount,
            uint256 restAmount
        )
    {
        (, ownerAmount) = calculateTax(_amount, ownerFee);
        (, devAmount) = calculateTax(_amount, devFee);
        (restAmount, toShitPoolAmount) = calculateTax(_amount, sellFee);
        toShitPoolAmount -= ownerAmount + devAmount;
    }

    function calculateTax(uint256 _amount, uint256 _tax)
        public
        pure
        returns (uint256 adjustedAmount, uint256 taxAmount)
    {
        taxAmount = (_amount * _tax) / 100;
        adjustedAmount = _amount - taxAmount;
    }
    ...
    function setShitPoolAddress(address _shitPoolAddress) external onlyOwner {
        shitPoolAddress = _shitPoolAddress;
    }
...
// File contracts/ShitPool.sol

pragma solidity 0.8.13;

contract ShitPool is Ownable {
    DYPRToken dyprToken;
    ...
    function _claim(uint256 _rewards) private {
        require(_rewards > 0, "Nothing to claim");
        dyprToken.transfer(msg.sender, _rewards);
    }

    function getRewards(address _userAddress)
        public
        view
        returns (uint256 rewards)
    {
        uint256 _matureStakes = dyprToken.getUserMatureStakes(_userAddress);
        rewards = calculateRewards(_matureStakes);
    }

    function calculateRewards(uint256 _matureStakes)
        public
        view
        returns (uint256 rewards)
    {
        rewards =
            (_matureStakes * (getShitPoolBalance()**2) * 30) /
            ((100 * 1e18)**2 * 100);
    }

    function getShitPoolBalance()
        public
        view
        returns (uint256 shitPoolBalance)
    {
        shitPoolBalance = dyprToken.balanceOf(address(this));
    }

    function setDYPRToken(address _dyprToken) external onlyOwner {
        dyprToken = DYPRToken(_dyprToken);
    }
}