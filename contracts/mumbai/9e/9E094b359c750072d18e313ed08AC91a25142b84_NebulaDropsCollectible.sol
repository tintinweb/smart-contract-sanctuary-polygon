...
contract NebulaDropsCollectible is ERC1155, Ownable {
    ...
    mapping(address => ApprovedArtists4Minting) public allowArtistToMintTokens;
    struct ApprovedArtists4Minting {
    //   string dropId;
      address allowedArtist;
      uint256 totalTokens;
    //   string dropName;
    //   string dropIPFSUri;
    }
    ...
    event DropApproved( 
        // string dropId,
     address allowedArtist
     // , string dropName, string dropIPFSUri
     );
    ...
    /* Approve owners to ..*/
    function approveArtistToMint(
        //string memory _dropid,
     address _allowedartist,uint256 _totaltokens
    // ,string memory _dropname, string memory _dropipfsuri
    ) 
    public onlyOwner
    {
        allowArtistToMintTokens[_allowedartist] =  ApprovedArtists4Minting (
            // _dropid,
            _allowedartist,
            _totaltokens
            // ,
        ...
        emit DropApproved(
            //_dropid,
        _allowedartist
        //,_dropname,_dropipfsuri
        );
    }


    /* Reject approved drops..*/
    function rejectApprovedDrops(address  _allowedartist) public onlyOwner{
        delete allowArtistToMintTokens[_allowedartist];  
    }

    /* this will return a list of all whitelisted Marketplaces..*/
    function fetchApprovedDropsList(address _allowedartist) public view returns(ApprovedArtists4Minting memory) {
      return allowArtistToMintTokens[_allowedartist];  
    }
    ...
    function mint(uint256 _id, uint256 _amount) 
        public  payable  
     {
        ...
        ApprovedArtists4Minting memory _approvedArtist =   allowArtistToMintTokens[msg.sender];

        require(address(_approvedArtist.allowedArtist) == msg.sender, "You are not allowed to mint the tokens for this drop.");
        
        /* Check whether it's first time minting..*/
       // if(mintedTokensByArtists[_dropid].tokensMinted > 0){
            require(mintedTokensByArtists[msg.sender].tokensMinted < _approvedArtist.totalTokens, "Your total number of allowed Token(s) are already minted.");
        //}
        ...
        uint256 _totalMintedTokens = mintedTokensByArtists[msg.sender].tokensMinted + 1;
        ...
        public payable
    {
        // string memory _name, string memory _newuri, 
        ApprovedArtists4Minting memory _approvedArtist =   allowArtistToMintTokens[msg.sender];
        // ApprovedArtists4Minting memory _approvedArtist = dropToAllowedDrop[_dropid];


        require(_approvedArtist.allowedArtist == msg.sender, "You are not allowed to mint the tokens for this drop.");

        uint256 _totalMintedTokens = 0;
         if(mintedTokensByArtists[msg.sender].tokensMinted > 0){
             _totalMintedTokens= mintedTokensByArtists[msg.sender].tokensMinted;
           }
        _totalMintedTokens += _ids.length;
        require(_totalMintedTokens < _approvedArtist.totalTokens, "Your total number of allowed Token(s) are already minted.");
