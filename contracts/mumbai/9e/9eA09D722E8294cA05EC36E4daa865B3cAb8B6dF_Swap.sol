...
contract Swap is ZetaInteractor, ZetaReceiver, SwapErrors, Pausable {
    ...
    function swapExactInputSingle(XcSwapArgs calldata args) external payable {
        if (!_isValidChainId(args.destinationChainId)) revert InvalidDestinationChainId();

        if (msg.value == 0) revert ValueShouldBeGreaterThanZero();

        // Wrap ETH and transfer to this contract
        ...
        zetaValueAndGas -= protocolFee; // TODO: takes fee of value and gas - should remove gas from fee collection

        connector.send(
            ZetaInterfaces.SendInput({
                destinationChainId: args.destinationChainId,
                destinationAddress: interactorsByChainId[args.destinationChainId],
                destinationGasLimit: args.crossChaindestinationGasLimit,
                message: abi.encode(
                    ZetaMessage({
                        messageType: SWAP_MESSAGE,
                        sourceTxOrigin: msg.sender,
                        sourceInputToken: iZetaToken,
                        receiverAddressEncoded: args.receiverAddress,
                        outTokenMinAmount: args.outTokenMinAmount,
                        amountIn: args.amountIn,
                        destinationOutToken: args.destinationOutToken
                    })
                ),
        ...
        isValidMessageCall(zetaMessage)
    {
        /* (
            ...
            address destinationOutToken
                ...
                zetaMessage.message,
                (bytes32, address, address, bytes, uint256, uint256, address)
            ); */
        ZetaMessage memory message = abi.decode(zetaMessage.message, (ZetaMessage));

        address receiverAddress = address(uint160(bytes20(message.receiverAddressEncoded)));

        if (message.messageType != SWAP_MESSAGE) revert InvalidMessageType();

        uint256 outTokenFinalAmount;
        if (message.destinationOutToken == iZetaToken) {
            if (zetaMessage.zetaValue < message.outTokenMinAmount) revert InsufficientOutToken();
            ...
            // Approve the router to spend sourceInputToken
            TransferHelper.safeApprove(iZetaToken, address(swapRouter), message.amountIn);

            uint256 amountOut;
            ISwapRouter.ExactInputSingleParams memory params = ISwapRouter.ExactInputSingleParams({
                tokenIn: iZetaToken,
                tokenOut: message.destinationOutToken,
                fee: poolFee,
            ...
            if (amountOut < message.outTokenMinAmount) revert InsufficientOutToken();
        }

        emit SecondLegSuccess(
            message.sourceTxOrigin,
            message.sourceInputToken,
            message.amountIn,
            message.destinationOutToken,
            outTokenFinalAmount,
            receiverAddress
        );
    }
