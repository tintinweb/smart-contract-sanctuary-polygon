...
contract MustasheRPSGame {
    ...
    function joinGame(uint256 _gameId) external {
        ...
        require(game.betAmount > 0, "Invalid game");
    ...
    function play(uint256 _gameId, string memory _choice) external {
        ...
        require(game.player1 == msg.sender || game.player2 == msg.sender, "You are not part of the game");
        ...
        if (bytes(game.choices[game.player1]).length > 0 && bytes(game.choices[game.player2]).length > 0) {
    ...
    function revealChoices(uint256 _gameId) private {
        ...
        string memory choice1 = game.choices[player1];
        string memory choice2 = game.choices[player2];
    ...
    function endGame(uint256 _gameId) private {
        ...
        if (game.wins[game.player1] > game.wins[game.player2]) {
            winner = game.player1;
            loser = game.player2;
        } else {
            winner = game.player2;
        ...
        token.transfer(loser, fee / 10**18);
    }
    ...
    function getGameStatus(uint256 _gameId) external view returns (address, address, uint256, uint256, uint256, uint256) {
        ...
        return (
            ...
            game.wins[game.player1],
            game.wins[game.player2],
            game.round
        );
    }
    ...
    function getActiveGames() external view returns (uint256[] memory, address[] memory, address[] memory, uint256[] memory, uint256[] memory, uint256[] memory) {
        ...
        address[] memory players1 = new address[](gameId);
        address[] memory players2 = new address[](gameId);
        ...
        for (uint256 i = 1; i <= gameId; i++) {
            ...
            if (game.player2 == address(0) && game.betAmount > 0 && !game.gameEnded) {
                gameIds[activeGamesCount] = i;
                players1[activeGamesCount] = game.player1;
                players2[activeGamesCount] = game.player2;
                betAmounts[activeGamesCount] = game.betAmount / 10**18;
                wins1[activeGamesCount] = game.wins[game.player1];
                wins2[activeGamesCount] = game.wins[game.player2];
                activeGamesCount++;
            }
        }

        return (gameIds, players1, players2, betAmounts, wins1, wins2);
    }
}