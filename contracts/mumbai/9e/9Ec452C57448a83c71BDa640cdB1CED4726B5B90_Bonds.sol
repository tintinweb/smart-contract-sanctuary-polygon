...
interface IERC20PaymentStandard {
    ...
    function addressBook() external returns(IAddressBook);
...
library Loan{
    ...
    function onPayment(loan storage _ln, globalInfo storage _user, globalInfo storage _global, uint _erc20Amount) internal{
        require(
            _erc20Amount >= _ln.minPayment || //Payment must be more than min payment
                (
                getOustanding(_ln) < _ln.minPayment  //Exception for the last payment (remainder)
                &&
                _erc20Amount >= getOustanding(_ln)  // Exception is only valid if user is paying the loan off in full on this transaction
                ),
    ...
    * @param _global global loan for the whole contract
    ...
    function getOustanding(loan memory _ln) internal pure returns(uint){
        if(_ln.paymentComplete > _ln.totalPaymentsValue){
            return 0;
        }
        return(_ln.totalPaymentsValue - _ln.paymentComplete);
    }
    function getOustanding(globalInfo memory _global) internal pure returns(uint){
