...
import {CollectionERC721} from "./CollectionERC721.sol";
import {CollectionERC1155} from "./CollectionERC1155.sol";
...
contract Factory {
     ...
     */
    function CreateNewERC721Collection(string memory _name, string memory _symbol,bool _isPublic) 
        external
        returns (address)
    {
        require(bytes(_name).length != 0,"Factory: Use valid _name");
        require(bytes(_symbol).length != 0,"Factory: Use valid _symbol");
        CollectionERC721 collection = new CollectionERC721(_name, _symbol,_isPublic);
    ...
    function CreateNewERC1155Collection(string memory _name, string memory _symbol,bool _isPublic)
        external
        returns (address)
    {
        require(bytes(_name).length != 0,"Factory: Use valid _name");
        require(bytes(_symbol).length != 0,"Factory: Use valid _symbol");
        CollectionERC1155 collection = new CollectionERC1155(_name, _symbol,_isPublic);
        collection.transferOwnership(msg.sender);
        collectionAddresses.push(address(collection));
        emit CollectionTokenCreated(address(collection));
        return address(collection);
...
abstract contract ERC2981 is IERC2981, ERC165 {
    ...
    modifier checkRoyaltyInfo(address receiver, uint feeNumerator) {
        require(feeNumerator <= MAX_LIMIT,"ERC2981: royalty fee will exceed MAx limit");
    ...
    function _setDefaultRoyalty(address receiver, uint96 feeNumerator) internal virtual checkRoyaltyInfo(receiver, feeNumerator) {
    ...
    Ownable
{
    ...
    modifier onlyValidAddress() {
        require(
          _msgSender() == owner() || isPublic,
            "Collection: only owner can call this function"
        );
        _;
    }
    ...
    function setDefaultRoyalty(address _receiver, uint96 _feeNumerator) public onlyOwner() {
        require(!isPublic,"Collection: collection is public");
    ...
    function mint(
        ...
        require(
            _to != address(0),
            "Collection: _to address not valid"
        );
        require(
 ...
 */
contract CollectionERC1155 is ERC1155, Ownable,ERC2981 {
    using Strings for uint256;
    ...
    ) external ownersOnly returns (uint256) {
