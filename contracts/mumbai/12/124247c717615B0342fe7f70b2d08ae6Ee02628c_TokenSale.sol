...
contract TokenSale is Initializable, ITokenSale {
    ...
    uint256 public whitelistTxFreeAllc; // in dollar with decimals
    ...
    function initialize(
        ...
        marketingWallet = 0x0B4d9ba9634D5782a4682ec5c8919A490A863E79;
        maxAllocation = _maxAllocation;
        globalTaxRate = _globalTaxRate;
        whitelistTxFreeAllc = _whiteListTxFreeAllc;
        whitelistTxRate = _whitelistTxRate;
    }

    // allocation is amount in dollar without decimals
    function giftTier(address[] calldata users, uint256[] calldata allocations)
        public
    {
    ...
    function setAllocationAndTax(uint256[4] calldata _allocations) external {
        require(block.timestamp <= params.privateStart, "Time lapsed");
        require(admin.hasRole(OPERATOR, msg.sender), "TokenSale: OnlyOperator");
        maxAllocation = _allocations[0];
        globalTaxRate = _allocations[1];
        whitelistTxFreeAllc = _allocations[2];
        whitelistTxRate = _allocations[3];
    }
    ...
    function _processPrivate(address _sender, uint256 _amount) internal {
        ...
        uint256 taxFreeAllcOfUser = (calculateTaxFreeAllocation(_sender)) *
            PCT_BASE;
    ...
    function calculateTaxFreeAllocation(address _sender)
        public
        returns (uint256)
    {
        uint256 userMaxAllc = _maxTierAllc(_sender);
        ...
        if (userMaxAllc > 0) {
            if (isWhitelisted[_sender]) {
                if (whitelistTxFreeAllc > userMaxAllc) {
                    return whitelistTxFreeAllc;
                } else {
                    return userMaxAllc;
                }
            } else {
    ...
    function userTaxRate(uint256 _amount, address _sender)
        ...
        returns (uint256)
    {
        uint256 userTaxFreeAllc = calculateTaxFreeAllocation(_sender);
    ...
    function claim() external {
        ...
        if (s.taxAmount > 0) {
            ...
            uint256 taxFreeAllc = calculateTaxFreeAllocation(msg.sender) *
                PCT_BASE;
...
interface IAdmin is IAccessControl {
    ...
    function createPool(ITokenSale.Params calldata _params, uint256,uint256,uint256,uint256) external;
