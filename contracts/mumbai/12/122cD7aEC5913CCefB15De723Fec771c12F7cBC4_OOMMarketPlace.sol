...
abstract contract OOMContractBase is Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    function _placeBid(
        ...
        if (lst.biddingId != 0) {
            ...
            bool _isStripe = biddingRegistry[lst.biddingId].isGL;
            if (_isGl == false) {
                if (_isStripe == true) {
                ...
                if (biddingRegistry[lst.biddingId].isGL == false) {
                    _transferToken(
                        olderBidder,
                        olderBidder,
                        biddingRegistry[lst.biddingId].price
        ...
        biddingRegistry[lst.biddingId].isGL = _isGl;
        operationNonce++;
        lst.biddingId = biddingId;
        addrBiddingIds[_fromAddress].push(biddingId);
        _depositToken(_fromAddress, price, _isGl);
        emit BiddingPlaced(_fromAddress, biddingId, listingId, price);
    }
    ...
    function _buyNow(
        ...
        if (_fromAddress == msg.sender) {
            _depositToken(_fromAddress, price, false);
        } else {
            _depositToken(msg.sender, price, true);
        }
    ...
    function getRoyaltiesInfo(address _contract, bytes32 listingId)
        external
        view
        returns (
            uint256,
        ...
        )
    {
        ...
        return (tokenId, royaltiesReceiver, royaltiesAmount);
    }

    function getBidInfo(bytes32 listingId)
        external
        view
        returns (
            address,
        ...
        )
    {
        ...
        listing storage lst = listingRegistry[listingId];
        if (lst.biddingId != 0) {
            bidding storage bid = biddingRegistry[lst.biddingId];
            bidder = bid.bidder;
            isGassLess = bid.isGL;
            price = bid.price;
            return (bidder, isGassLess, price);
        } else {
            return (bidder, isGassLess, price);
        }
    }
...
contract OOMMarketPlace is OOMContractBase {
    ...
    function placeBid(bytes32 listingId) external payable nonReentrant {
        _placeBid(msg.sender, listingId, msg.value, false);
    }
    ...
    function placeBid(
        ...
        _placeBid(_fromAddress, listingId, _price, true);
    }
    ...
    function buyNowGL(
        ...
        // For BNB, the value need to larger then the price + fee
        // uint256 fee = (price * feeRate) / FEE_RATE_BASE;
        require(_price >= price, "Not enough funds to buy");
    ...
    function _depositToken(
        ...
        if (isGl == true) {
            addrTokens[_fromAddress] += _amount;
        } else {
            addrTokens[_fromAddress] += _amount;
    ...
    function claimNftGassLess(address _fromAddress, bytes32 biddingId)
        external
        payable
        nonReentrant
        onlyOwner
    ...
    function withdraw(address payable _to) external onlyOwner {
        ...
        uint256 balance = address(this).balance - totalEscrowAmount;
