...
contract Chat {
    Structures structures;
     constructor() {
    structures = Structures(0x9b5DDf4a62387B1A1AA0b8feaf8FF386910B4Ec4);
}
    enum DeletionStatus {
        NotDeleted,
        DeletedBySender,
    ...
    if (replies[messageId].responses.length == 0) {
        ...
        addReply(messageId, response, messageOriginal, shareable, true);
    } else {
        addReply(messageId, response, messageOriginal, shareable, false);
    }
}

function addReply(uint256 messageId, string memory response, Message memory messageOriginal, bool shareable, bool setRep) private {
...
function viewMessage(uint256 messageId) public {
 ...
 function MessageSent(string memory email) public view returns (Message[] memory) {
    ...
    Message[] memory messagesReceived = new Message[](count);
    uint index = 0;
