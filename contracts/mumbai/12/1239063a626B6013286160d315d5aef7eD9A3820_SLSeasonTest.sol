...
abstract contract SeasonTest is ISLSeason, SeasonBase {
    using Unsafe for uint256;
    using CountersUpgradeable for CountersUpgradeable.Counter;
    ...
    function setSeasonEndBlock(
        uint256 _seasonId,
        uint256 _endBlock
    ) external onlyOperator {
        if (!isExistSeasonById(_seasonId)) revert InvalidSeasonId();
...
import {RankUpTest} from "./parts/RankUpTest.sol";
import {ISLProject} from "../project/ISLProject.sol";
import {ISLMonsterFactory} from "../monsterFactory/ISLMonsterFactory.sol";

contract SLSeasonTest is RankUpTest, UUPSUpgradeable {
    function initialize(
        ISLProject _projectContract,
        ISLMonsterFactory _monsterFactoryContract,
