...
contract Marketplaces {
    ...
    // @abstract create collection
    // @param name collection's name
    // @param symbol collection's symbol
    // @param uint256 NFT's max supply
    // @param content collection's content
    // @return address collection's address
    function createCollection(
        ...
        NFTCollection collection = new NFTCollection(name, symbol, maxSupply); // create collection
        allowedCollections[address(collection)] = true; // usable
    ...
    // @param content NFT's ocntent
    // @param price NFT's price
    // @return address collection's address
    function createNFT(
        address collection,
        ...
        nftInfoForSale[collection][tokenID] = SaleInfo({
            seller: msg.sender,
            price: price
        });
...
contract NFTCollection is ERC721URIStorage {
    ...
    modifier onlyMarketplacesContract() {
    ...
    // @param _maxSupply nft's max supply
    constructor(
        string memory _name,
    ...
    // @abstract contractor
    // @param _tokenURI NFT's uri
    // @param _to NFT's creator
    // @param return uint256 mint NFT token's id
    function MintNFT(
        string memory _tokenURI,
        address _to
    ) external onlyMarketplacesContract returns (uint256) {
        require(
