    ...
    ReentrancyGuardUpgradeable
{
    ...
    event tokenMintedSuccess(
        address indexed _who,
        uint256 _amount,
        uint256 timestamp
    );

    event tokenBurnedSuccess(
        address indexed _who,
        uint256 _amount,
        uint256 timestamp
    );
    ...
    // The initial max cap is 1Bi
    uint256 public cap;

    function setCapInWei(uint256 cap_) public onlyOwner {
        cap = cap_;
    }

    function setRatePerYearInWei(uint256 ratePerYearInWei_) public onlyOwner {
    ...
    //enricash,ess,1000000000000000000000000000,500000000000000000,["0x72ce142b8fcd8cf0bae46dc26d28408247c6deb2","0x2a7340c19b021957fa83f37959f4b77fc5cfc65c","0x0707bf36710f3f49c18ea9e02eaf0e63e17de3db","0xe61d458bac7dcf6e3099d3eac0866ff1c0ace2cf"],["0xa55C8eD7C6198F57cB69D8dC9CE41301E93B24B5"],["0x3e0d4b1fde25ff9eB051E987Ebf4D3AB3D5E1bb3"]
    function initialize(
        string memory name_,
        ...
        setRatePerYearInWei(ratePerYearInWei_);
        cap = cap_;
        emit tokenInitialed(address(this), block.timestamp);
    }

    //0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2,1000000000000000000000000000
    ...
    //1000000000000000000 000 000
    function stake(uint256 amount_) public payable {
        require(
            msg.value == 0,
            "You can't deposit Ether directly to this contract using this method"
        );
        require(amount_ > 0, "The deposit amount must be greather than zero");

        _transfer(msg.sender, address(this), amount_);

        deposits[msg.sender] = Stake(
            getStakedAmount() + amount_,
            block.timestamp
        );
    ...
    function getSendersBalance() public view returns (uint256) {
    ...
    function getSenderDeposits() public view returns (Stake memory) {
        return (deposits[msg.sender]);
    }

    function calculateInterest() public view returns (uint256) {
        uint256 stakeAge = block.timestamp - (deposits[msg.sender]).startdate;

        uint256 stakedAmount = getStakedAmount();

        return
            accrueYearlyRateInterest(stakedAmount, ratePerYearInWei, stakeAge);
    }

    function getProfits() public view returns (uint256) {
        return calculateInterest() - getStakedAmount();
    }

    // Now I must calculate the amount he should earn
    // Function to withdraw all Ether from this contract.
    function unstake() public {
        uint256 _amount = getStakedAmount();
        require(_amount > 0, "You don't have any stake balance to unstake");
        ...
        emit unstakeSuccess(msg.sender, _amount, block.timestamp);
    }
...
contract BetVerseTokenTimelock is TokenTimelockUpgradeable {
    function initialize(
