...
contract Catcontract is ERC721Enumerable, Ownable {
    ...
    uint256 public breedFee = 0.05 ether; 
...
function breed(uint256 _dadId, uint256 _mumId) external payable returns (uint256) {
    ...
    require(msg.value >= breedFee, "Catcontract: Breed fee not met");

    uint256 excessAmount = msg.value - breedFee; // 
    if (excessAmount > 0) {
        payable(_msgSender()).transfer(excessAmount); // 
    }
...
function setBreedFee(uint256 _fee) external onlyOwner { //  breed
    breedFee = _fee;
}
