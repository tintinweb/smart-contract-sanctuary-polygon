...
contract NftpayMinter is AccessControl{

    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    bytes32 public constant ADMIN_ROLE = keccak256("ADMIN_ROLE");
    address private constant USDC_mumbai = 0xFEca406dA9727A25E71e732F9961F680059eF1F9;


    event PaymentSuccessful(uint256 nftId, address receiver, uint256 priceReceived);
    ...
    function mint(address _to, uint256 totalPrice, uint256 parameter_option_list_0) payable external {
//        address winery = ERC721(nftAddress).ownerOf(nftId);
//        ERC721(nftAddress).transferFrom(winery, _to, nftId);
//        IERC20(USDC_mumbai).transferFrom(msg.sender, winery, totalPrice * 1e6);
        emit PaymentSuccessful(totalPrice, _to, parameter_option_list_0);
    }

    function withdrawTokens(address erc20Token, address payable _to, uint256 amount) external onlyRole(ADMIN_ROLE){
        IERC20(erc20Token).transfer(_to, amount);
    }

    // Function to transfer Ether from this contract to address from input
    function transferMatic(address payable _to, uint _amount) public onlyRole(ADMIN_ROLE){
