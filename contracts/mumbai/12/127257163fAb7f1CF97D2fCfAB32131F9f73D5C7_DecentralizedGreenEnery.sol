...
contract DecentralizedGreenEnery is Context, IERC20, Ownable {  
    ...
    function updateprice(uint BuyPrice_,uint SellPrice_,uint PriceIncremental_) onlyOwner public {
        BuyPrice =BuyPrice_;
    ...
    */
    ...
    function sell(uint256 amount) public {
        ...
        uint256 _getmatic=SellPrice-=(amount*priceIncrement);
        ...
        transfer (address(this),_marketing);
        transfer(address(this),_reward);
        _wallet.transfer(_getmatic);

        //Fire event
        emit onTokenSell(_wallet, netAmount, _getmatic);
    }
    
    function buy(uint amount)public payable returns (uint) {
        require(msg.value > BuyPrice, "Insufficient payment");
        require(!paused, "Function is paused don't sell token");
        uint256 matic = BuyPrice * amount;
        uint256 _marketing = amount.mul(_marketingPer).div(100);
        uint256 _autoBurn = amount.mul(_autoBurnPer).div(100);
        uint256 _reward = amount.mul(_RewardPer).div(100);
    ...
    function AutoUpdateBuyPrice( uint amount)  internal  {
       BuyPrice+=(amount*priceIncrement);
    }

    function AutoUpdateSellPrice( uint amount)  internal {
        SellPrice-=(amount*priceIncrement);
    }
