    ...
    UUPSUpgradeable
{
    ...
    uint256 public fragmentsFadeTimestamp;

    function initialize(address tearsAddress, uint256 treasury) public initializer {
    ...
    function _safeMint(uint256 id) private onlyExistingRelic(id) {
        require(_relicMintCounts[id] < _relicMintLimits[id], "E02: Supply limit exceeded");
    ...
    function stakeRelic(uint256 id, uint256 amount) external {
        ...
        if (block.timestamp > fragmentsFadeTimestamp) {
            require(id > 3, "fragments fade");
        }
    ...
    function unstakeRelic(uint256 id, uint256 amount) external onlyRelicOwner(id, amount) {
    ...
    function getAllAvailableRewards() external view returns (uint256) {
        uint256 totalReward;
        for (uint256 id = 1; id <= _relicIds; id++) {
            totalReward += _tearsReward(msg.sender, id, _relicOwners[msg.sender][id]);
        }
        return totalReward;
    }
    ...
    function _tearsReward(
        ...
        if (id < 4 && timestamp > fragmentsFadeTimestamp) {
            timestamp = fragmentsFadeTimestamp;
        }
        uint256 stakeTime = _relicStakeTimes[owner][id];
        if (stakeTime == 0 || stakeTime > timestamp) return 0;
        uint256 elapsedTime = timestamp - stakeTime;
    ...
    function setFragmentsFadeTimestamp(uint256 timestamp) external onlyOwner {
        fragmentsFadeTimestamp = timestamp;
    }

    function forceFragmentsFadeTimestamp() external onlyOwner {
        fragmentsFadeTimestamp = block.timestamp;
    }
    ...
    function setRelicMerkleRoot(uint256 id, bytes32 merkleRoot) external onlyOwner {
    ...
    function airdropRelics(uint256 id, address[] memory wallets) external onlyOwner {
        require(_relicMintCounts[id] + wallets.length <= _relicMintLimits[id], "E02: Supply limit exceeded");
        _relicMintCounts[id] += wallets.length;
        for (uint256 index = 0; index < wallets.length; index++) {
            emit Mint(wallets[index], id, 1);
            _mint(wallets[index], id, 1, "");
        }
    }
...
contract FreyjaTear is ERC20Burnable, Ownable {
    mapping(address => bool) private _minters;

    uint256 public constant HALF_CAP = 50_000_000 * 1 ether;

    constructor() ERC20("Freyja Tear", "TEAR") {}

    function mint(address to, uint256 amount) external onlyMinters {
        if (totalSupply() < HALF_CAP) {
            _mint(to, amount);
        } else {
            _mint(to, amount / 2);
        }
    ...
    modifier onlyMinters() {
        require(_minters[msg.sender], "E01: Only minters can mint");
        _;
    }
}
