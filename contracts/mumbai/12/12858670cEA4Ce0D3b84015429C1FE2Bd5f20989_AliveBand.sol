...
contract AliveBand is Initializable, AccessControlUpgradeable {
    ...
    function addMember(address _addr) public onlyAdmin returns (bool) {
        ...
        uint256 id = membersOfBand.length;
...
contract AliveRegistry is OwnableUpgradeable {
    ...
    function updateSplitsAddress(address _newSplit) external onlyOwner {
...
contract AliveBondedSong is AliveCommonSong {
    ...
    function getPrice(uint256 _quantity) internal returns (uint256 price) {
        uint256 eachPrice = currentPrice;
        unchecked {
            for (uint256 i = 1; i < _quantity; i++) {
                eachPrice = eachPrice + upcountRate;
            }
        }
        currentPrice = eachPrice;
        return eachPrice * _quantity;
    }
...
contract AliveDutchSong is AliveCommonSong {
    ...
    function initialize(
        ...
        currentPrice = ceilPrice;
        floorPrice = _songInfos[2];
        discountRate = _songInfos[3];
        startedAt = block.timestamp;
    }

    function getPrice(uint256 _quantity) public returns (uint256 price) {
        ...
        if (discount > ceilPrice || floorPrice > ceilPrice - discount) {
            ...
            currentPrice = ceilPrice - discount;
    ...
    ReentrancyGuardUpgradeable
{
    ...
    uint256 public offchainRoyaltyPercentageInBips;
    ...
    // Events
    event songMinted(address minter, uint256 quantity);
    ...
    {
        require(ownerOf(tokenID) == msg.sender, "Only artist can claim drop!");
