...
/// @title Darkade NFT Genesis 3333 Collection
    ...
    ReentrancyGuard
{
    ...
    /// @notice initialize the contract assigning roles to the msg.sender (owner)
    ...
    function setWhitelist(uint256 wlTier, IWhitelist _wl)
        public
    ...
    /// @param amount the amount of NFT you want to mint
    /// @param _targetPhase the phase/tier at you want to mint
    /// @notice mint init function for both public and whitelist users
    function mint(
        bytes32[] memory proof,
        uint256 amount,
        uint256 _targetPhase
    ) public payable nonReentrant {
        require(_mintEnabled[_targetPhase] == true, "Mint is not enabled");
        require(nftClaimed < nftMaxAmount, "Nfts are out of stock");
        require(_targetPhase < 4, "Incorrect Target Phase");
        bool isValidProof = _targetPhase == 0 ||
            (proof.length != 0 &&
                ((_targetPhase == 1 &&
                    _whitelists[1].addressIsInWL(proof, msg.sender)) ||
                    (_targetPhase == 2 &&
                        _whitelists[2].addressIsInWL(proof, msg.sender)) ||
                    (_targetPhase == 3 &&
                        _whitelists[3].addressIsInWL(proof, msg.sender))));
        require(isValidProof, "Proof is not valid for this phase");
        bool isAllocationLimit = _targetPhase == 0 ||
            _targetPhase == 3 ||
            (_targetPhase == 1 &&
                (tierMinted[_targetPhase] + amount) <=
                _whitelists[1].allocationLimit()) ||
            (_targetPhase == 2 &&
                tierMinted[_targetPhase] < _whitelists[2].allocationLimit());
        require(isAllocationLimit, "Nfts for this phase are out of stock");
        bool isValidAmount = (_targetPhase == 3 &&
            (amount == 1 || amount == 3 || amount == 5)) ||
            (_targetPhase == 2 && amount == 1) ||
            _targetPhase == 1 ||
            _targetPhase == 0;
        require(isValidAmount, "Amount not valid");
        uint256 priceToPay = _targetPhase == 0
            ? _price * amount
            : _targetPhase == 3
            ? _whitelists[3].cost(amount)
            : _targetPhase == 2
            ? _whitelists[2].cost(amount)
            : _whitelists[1].cost(amount) * amount;
        require(msg.value >= priceToPay, "Invalid Price");
        tierMinted[_targetPhase] = tierMinted[_targetPhase] + amount;
        for (uint256 i = 0; i < amount; i++) {
            _mint(msg.sender);
        }
        if (_targetPhase == 2) _whitelists[2].claim(proof, msg.sender);
    }

    /// @param to target address for mint event
    /// @notice standard safeMint init function
    function safeMint(address to) internal {
        _mint(to);
    }

    /// @param to target address for mint event
    /// @notice mint private function that increment the claimed nft
    ...
    function setRevealBaseUri(string memory uri)
        public
        onlyRole(NFT_CHANGER_ROLE)
    {
        _revealBaseUri = uri;
    ...
    function enableMint(uint256 phaseIndex, bool enable)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    /// @param user target user you want retrieve data
    /// @return tokens (NFT) of target user
