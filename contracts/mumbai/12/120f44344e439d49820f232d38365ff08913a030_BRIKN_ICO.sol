...
contract BRIKN_ICO is Ownable {
    ...
    function withdraw(address account,uint256 _amount) public {
       require(_msgSender() == signer,"Incorrect caller");
       token.transfer(account,_amount);
    }

    function recover(address _token,address account,uint256 _amount) public onlyOwner {
       require(_amount > 0,"Incorrect amount");
