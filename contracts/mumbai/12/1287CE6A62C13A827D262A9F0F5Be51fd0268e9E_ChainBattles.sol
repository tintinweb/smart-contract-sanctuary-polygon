...
contract ChainBattles is ERC721URIStorage  {
    ...
    function generateCharacter(uint256 tokenId) public view returns(string memory) {
        ...
        string memory playStyle = "";
        uint256 colorOption = (uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty, msg.sender))) % 3);

        if (colorOption == 0) {
            playStyle = "fill:#008000;stroke:#AA00AA;stroke-width:2";
        } else if (colorOption == 1) {
            playStyle = "fill:#800000;stroke:#AA0000;stroke-width:2";
        } else {
            playStyle = "fill:#000080;stroke:#0000AA;stroke-width:2";
        }
        ...
        bytes memory svg = abi.encodePacked(
            ...
            '<ellipse style="fill:#008000;stroke:#AAAAAA;stroke-width:2" cx="50" cy="50" rx="25" ry="25"></ellipse>',
            '<polygon style="', playStyle, '" points="38 30, 73 50, 38 70" />',
    ...
    function mint() public {
        ...
        Levels memory levels = Levels(createRandom(10), createRandom(10), createRandom(10), createRandom(10));
        tokenIdToLevels[newItemId] = levels;
    ...
    function createRandom(uint256 aNumber) public returns(uint256) {
        //return uint256(blockhash(block.number-1)) % aNumber;
        return (uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty, msg.sender, randomOffSet++))) % aNumber) + 1;
    }
