...
contract MetalArmors is Ownable, ERC1155, ArmorTokensMetadata {
    ...
    // Track declared Chubby Metal Armor components
    ...
    //紀錄用戶有沒有用 ChubbyApe 的 token id免費領過
    mapping(uint256 => uint256) public freeMintedRecord;

    //用戶可以免費兌換的次數
    uint256 public freeAcquireNumberLimit = 2;
    ...
    */
    ...
    // uint256 public declareEnd = 20000;
    ...
    //     ids[0] = itemId(basicEquipmentTokenId, maskComponents, MASK);
    //     ids[1] = itemId(basicEquipmentTokenId, leftArmComponents, LEFTARM);
    //     ids[2] = itemId(basicEquipmentTokenId, rightArmComponents, RIGHTARM);
    //     ids[3] = itemId(basicEquipmentTokenId, armorComponents, ARMOR);
    //     ids[4] = itemId(basicEquipmentTokenId, leftLegComponents, LEFTLEG);
    //     ids[5] = itemId(basicEquipmentTokenId, rightLegComponents, RIGHTLEG);
    ...
    /**
     自帶裝備 ; Free 
    1. 把自己的裝備轉給合約
    2.一定要用 chubbyApe 來產生出一隻MetalApe
     */
    // function mintMetalApeThenEquipment(
    //     uint256 _chubbyApeTokenId,
    //     uint256[] memory _itemIds,
    //     address _recipient
    // ) public checkEquipmentType(_itemIds) onlyMetalArmorLogicAddress {
    //     require(saleIsActive, "Mint is not active");
    //     require(
    ...
    //     //產生出一個 MetalApe 然後記錄裝備
    //     metalApe.mintMetalApe(
    //         _chubbyApeTokenId,
    //         _recipient,
    //         address(this),
    //         _itemIds
    ...
    function mintMetalApeThroughPairingThenPutOn(
        ...
        require(saleIsActive, "Mint is not active");
        require(
            msg.sender == metalApe.ownerOf(_leftPairId) &&
                msg.sender == metalApe.ownerOf(_rightPairId),
            "Must own Chubby Ape token"
        );

        //產生出一個MetalApe然後記錄裝備
        metalApe.mintMetalApeThroughPairing(
            _leftPairId,
            _rightPairId,
            msg.sender,
            address(this),
        ...
        returns (uint16)
    {
        ...
        //const rankRand = Math.round(Math.random() * 100);
        ...
        uint8 rankIndex = rankIndexMap[materialIndex][rankCheckPlus];
        //const ranktitle = rankAllTitle[rankIndex];

        // 取得setIndex
        //setCkeck = getSetCkeck(partIndex, materialIndex, rankIndex, rankCheckPlus);

        uint8 setIndex = setTitle_Index[rankCheckPlus][setCkeck];

        // const setTitle = setTitle_CN[rankCheckPlus][setCkeck];

        // if (rankIndex === 8) {
        //   rankIndexCheckData.push(rankCheckPlus + '_' + setCkeck);
        // }
        // 檢查資料
     ...
     *----------------------/  
     */
    function SetInterfaceAddress(address _cAddress, address _mAddress)
        public
        onlyOwner
    {
        chubbyApe = IERC721Enumerable(_cAddress);
        metalApe = IMetalApe(_mAddress);
    }

    function setMetalArmorLogicAddress(address _MetalArmorLogicAddress)
        public
        onlyOwner
    {
    ...
    function setFreeAcquireNumberLimit(uint256 _freeAcquireNumberLimit)
        public
        onlyOwner
    {
        freeAcquireNumberLimit = _freeAcquireNumberLimit;
    }
