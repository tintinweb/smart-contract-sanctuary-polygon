...
contract FrameItNFT is ERC721, IERC2981 {
    ...
    function initialize(string memory _newname, string memory _newsymbol, string memory _newuri, address _owner, uint256 _royaltyFeeInBips, address _salesWallet, address _lootBox, address _factory) external {
        ...
        require(_salesWallet != address(0), "ZeroAddress");
        require(_owner != address(0), "ZeroAddress");
        require(_lootBox != address(0), "ZeroAddress");
    ...
    function mint(address _to, uint256[] calldata _ids) external {
        require(msg.sender == lootBox, "OnlyFromLootBox");
    ...
    function isApprovedForAll(address account, address operator) public view virtual override returns (bool) {
        if (operator == IFrameItFactory(factory).marketplace() || operator == IFrameItFactory(factory).auctionMarketplace() || operator == lootBox) return true;
