    ...
    IHedgeManager
{
    ...
    function liquidateHedge(
        ...
        emit HedgeLiquidated(latestHedgeId);
    }
    ...
    function addCollateral(
        ...
        emit CollateralAdded(_hedgeId, _amount);
    }
    ...
    function addCopcLiquidity(uint256 _amount) external onlyRole(LP) {
        ...
        emit LiquidityDeposited(address(copcContract), _amount);
    }

    function addUsdcLiquidity(uint256 _amount) external onlyRole(LP) {
        usdcContract.safeTransferFrom(msg.sender, address(this), _amount);

        emit LiquidityDeposited(address(usdcContract), _amount);
    }
    ...
    function withdrawCopcLiquiditySAFE(uint256 _amount) external onlyRole(LP) {
        ...
        emit LiquidityWithdrawn(address(copcContract), _amount);
    }
    ...
    function withdrawUsdLiquiditySAFE(uint256 _amount) external onlyRole(LP) {
        ...
        emit LiquidityWithdrawn(address(usdcContract), _amount);
    }
    ...
    function withdrawFeeReservesCopc(uint256 _amount) external onlyRole(LP) {
        ...
        emit WithdrawFees(address(copcContract), _amount);
    }
    ...
    function withdrawFeeReservesUsdc(uint256 _amount) external onlyRole(LP) {
        ...
        emit WithdrawFees(address(usdcContract), _amount);
    }
    ...
    function setExchangeRate(
        ...
        emit ExchangeRateChanged(oldRate, _newRate);
    }
    ...
    function setVarToken(
        ...
        emit VarSet(what, abi.encode(value));
    }
...
interface IHedgeManager {
    ...
    event VarSet(bytes32 what, bytes value);

    event NewHedge(uint256 hedgeId);

    event HedgeClosed(uint256 hedgeId);

    event HedgeLiquidated(uint256 hedgeId);

    event CollateralAdded(uint256 hedgeId, uint256 amount);

    event ExchangeRateChanged(uint256 oldRate, uint256 newRate);

    event ChangeWithdrawTarget(address oldTarget, address newTarget);

    event LiquidityDeposited(address token, uint256 amount);

    event LiquidityWithdrawn(address token, uint256 amount);

    event WithdrawFees(address token, uint256 amount);
