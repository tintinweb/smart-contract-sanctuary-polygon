...
contract BatchBet is AdminExecutorAccess, MarketErrors, BatchBetEvents {
    ...
    bytes4 private immutable marketMakerV1 = 0x19a298e7;
    ...
    function batchBuyAffiliate(IERC20 token, BuyOrder[] calldata buys, address affiliate) public whenNotPaused {
        ...
        for (uint256 i = 0; i < buys.length; i++) {
            ...
            if (!market.supportsInterface(marketMakerV1)) {
                revert NotAMarket(market);
            }
            IMarketMakerV1 amm = IMarketMakerV1(market);
            ...
            amm.buyFor(buyer, buys[i].investmentAmount, buys[i].outcomeIndex, buys[i].minOutcomeTokensToBuy);
        }

        if (affiliate != address(0x0) && totalInvestment > 0) {
            emit AffiliateBuy(buyer, affiliate, address(token), totalInvestment);
        }
    }
}
