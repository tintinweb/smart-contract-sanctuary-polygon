...
library LibInitializable {
    ...
    function init(InitData memory initData) internal initializer {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
...
import {DiamondCutFacet} from "../external/diamond/facets/DiamondCutFacet.sol";
...
contract MutualOwnership is Diamond, DiamondCutFacet, DiamondLoupeFacet {
    constructor(
        address contractOwner,
        LibInitializable.InitData memory initData
    ) payable Diamond(contractOwner) {
        LibInitializable.init(initData);
    }
}