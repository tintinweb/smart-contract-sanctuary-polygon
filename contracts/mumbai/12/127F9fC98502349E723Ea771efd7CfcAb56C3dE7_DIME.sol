...
import "./interfaces/ICommentPosts.sol";
...
contract DIME is Pausable, ReentrancyGuard {
    ...
    // Link to the Comment Posts
    ICommentPosts public CommentPosts;
    ...
    */
    ...
    function updateContracts(IERC20 _payments, address _userProfiles, address _hashtags, address _tagged, address _posts, address _likes, address _messageData, address _doom, address _comments, address _groups, address _commentPosts) public onlyAdmins {
        ...
        // Update the Posts addresses
        CommentPosts = ICommentPosts(_commentPosts);
...
//                if (inGroups[g] > 0){
//                    require(Groups.getOwnerOfGroup(inGroups[g]) != address(0), "That group doesn't exist");
//                }
//            }
//        }
        ...
        uint256 postedByContract = tx.origin == msg.sender ? 1 : 0;


        // Build the new message struct
        if (attribs[1] > 0){
            ...
            if (_hashtags.length > 0){
                Hashtags.addHashtags(attribs[1], _hashtags);
            }

            // TODO: Tags need comments buckets to allow this
            // Update the tagged accounts with this message ID and tagged user address
//            Tagged.addTags(msgCounter, taggedAccounts);

            IComments.MsgData memory newMsg;

            newMsg.msgID = commentCounter;
            newMsg.postedBy = [posterAddress, msg.sender];
            ...
            Comments.saveComment(newMsg);

            // Update that message commented on total comments stats
            MessageData.addStat(2, newMsg.isCommentOf, 1);

            // Log the message post
            userProfiles.logPost(posterAddress, tipPerTag, taggedAccounts, true);
        } else {
            ...
            // Log the message post
            userProfiles.logPost(posterAddress, tipPerTag, taggedAccounts, false);
        }
    }

    function eraseMsg(uint256 msgID, uint256 isCommentOf) public whenNotPaused nonReentrant {

        // Erase the message
        MessageData.removeMsg(msgID, msg.sender, isCommentOf);
    ...
    function addLike(uint256 msgID, address likedBy, uint256 isCommentOf) public whenNotPaused nonReentrant {
        // require the approval of the sending of the token to the contract
        require(paymentToken.approve(msg.sender, costToLike), "Must approve payment tokens to be transferred");

        // Get the poster of the message / comment
        address poster = isCommentOf > 0 ? Comments.getPoster(msgID, isCommentOf) : MessageData.getPoster(msgID);

        // Transfer the Payment Token to the contract
        require(paymentToken.transferFrom(msg.sender, poster, costToLike), "Didn't receive the Payment Token");

        // Like a post
        Likes.addLike(msgID, likedBy, isCommentOf);
    }

    function removeLike(uint256 msgID, address likedBy, uint256 isCommentOf) public whenNotPaused nonReentrant {
        // Unlike a post
        Likes.removeLike(msgID, likedBy, isCommentOf);
    }
    ...
    function getMsgIDsByAddress(address usrAddress, uint256 startFrom) public view whenNotPaused returns (uint256[] memory) {
        return Posts.getMsgIDs(usrAddress, startFrom);
    }

    function getCommentIDsByAddress(address usrAddress, uint256 startFrom) public view whenNotPaused returns (uint256[2][] memory) {
        return CommentPosts.getCommentIDs(usrAddress, startFrom);
    }
...
interface ICommentPosts {

    // Get a list of a users posts (returns [commentID , isCommentOf])
    function getCommentIDs(address usrAddress, uint256 startFrom) external view returns(uint256[2][] memory);

    // Remove a post from a user mapping
    function addCommentPost(uint256 msgID, address addressPoster, uint256 isCommentOf) external;

    // Add a post to a users mapping
    function removeCommentPost(uint256 msgID, address addressPoster) external;
}
...
interface ILikes {
    ...
    function removeLike(uint256 msgID, address likedBy, uint256 isCommentOf) external;

    // Remove a like from a users post
    function addLike(uint256 msgID, address likedBy, uint256 isCommentOf) external;
}
