...
contract UpgradeCelestial is Initializable, UUPSUpgradeable, OwnableUpgradeable, PausableUpgradeable {
  ...
  IFBX public fbx;
  // maps cLevel to price
  mapping(uint256 => uint256) public cLevelPrice;
  // maps cPP to price
  mapping(uint256 => uint256) public cPPPrice;
  // maps user address to cppUpgradeAllowance amount
  mapping(address => uint256) public cppUpgradeAllowances;
  // maps user address to levelUpgradeAllowance amount
  mapping(address => uint256) public levelUpgradeAllowances;
  ...
  function initialize(address _fng, address _fbx) public initializer {
    ...
    fbx = IFBX(_fbx);
    _pause();
    cLevelPrice[1] = 2000 ether;
    cLevelPrice[2] = 4000 ether;
    cPPPrice[1] = 2000 ether;
    cPPPrice[2] = 4000 ether;
    cPPPrice[3] = 8000 ether;
  }
  ...
  function getPPUpgradePrice(uint256 celestialId) public view returns(uint256) {
    uint256 cPP = fngNFT.getCelestialAttributes(celestialId).cPP;
    require(cPP <= 3, "Celestial already max Pilfer Power");
    return cPPPrice[cPP];
  }

  function getLevelUpgradePrice(uint256 celestialId) public view returns(uint256) {
    uint256 cLevel = fngNFT.getCelestialAttributes(celestialId).cLevel;
    require(cLevel <= 2, "Celestial already max Level");
    return cLevelPrice[cLevel];
  }
  ...
  function upgradeLevel(uint256 celestialId) external whenNotPaused {
    require(fngNFT.ownerOf(celestialId) == msg.sender, "You do not own this token");
    CelestialV2 memory attributes = fngNFT.getCelestialAttributes(celestialId);
    require(attributes.cLevel <= 2, "Celestial already max Level");
    if(levelUpgradeAllowances[msg.sender] > 0 && attributes.cLevel == 1){
      levelUpgradeAllowances[msg.sender] -= 1;
    }else{
      fbx.burn(msg.sender, cLevelPrice[attributes.cLevel]);
    }
    attributes.cLevel += 1;
    fngNFT.updateCelestialAttributes(celestialId, attributes);
  }

  function upgradePP(uint256 celestialId) external whenNotPaused {
    require(fngNFT.ownerOf(celestialId) == msg.sender, "You do not own this token");
    CelestialV2 memory attributes = fngNFT.getCelestialAttributes(celestialId);
    require(attributes.cPP <= 3, "Celestial already max Pilfer Power");
    if(cppUpgradeAllowances[msg.sender] > 0 && attributes.cPP == 1){
      cppUpgradeAllowances[msg.sender] -= 1;
    }else{
      fbx.burn(msg.sender, cPPPrice[attributes.cPP]);
    }
    attributes.cPP += 1;
    fngNFT.updateCelestialAttributes(celestialId, attributes);
  ...
  function setCLevelPrice(uint256 level, uint256 price) external onlyOwner {
    cLevelPrice[level] = price;
  }

  function setCPPPrice(uint256 level, uint256 price) external onlyOwner {
    cPPPrice[level] = price;
  }
  ...
  function setCppUpgradeAllowances(address[] calldata accounts, uint256[] calldata allowances) external onlyOwner {
    require(accounts.length == allowances.length, "Number of accounts should match the number of allowances");
    for(uint256 i = 0; i < accounts.length; i++){
      cppUpgradeAllowances[accounts[i]] = allowances[i];
    }
  }

    function setLevelUpgradeAllowances(address[] calldata accounts, uint256[] calldata allowances) external onlyOwner {
    require(accounts.length == allowances.length, "Number of accounts should match the number of allowances");
    for(uint256 i = 0; i < accounts.length; i++){
      levelUpgradeAllowances[accounts[i]] = allowances[i];
    }
  }
