...
contract Swap is AccessControl, ReentrancyGuard {
    ...
    function mint(address acceptedToken, uint256 amount, uint256 minAmount) external {
        ...
        IERC20(USDC).safeTransfer(_masterWallet, amountTransfer);
        ...
        IERC20MintBurn(CRUS).mint(msg.sender, amountCRUS);
    ...
    function mintETH(uint256 minAmountCRUS) external payable nonReentrant {
        ...
        uint amountUSDC = _uniswapRouter.exactInputSingle(
            ISwapRouter.ExactInputSingleParams({
                tokenIn: WETH,
                tokenOut: USDC,
                fee: mapUniFee[WETH],
                ...
                deadline: block.timestamp,
                amountIn: msg.value,
                amountOutMinimum: minAmountCRUS / 10**factor - 1, // In case of different rounding -1
                sqrtPriceLimitX96: 0
            })
        );
        uint256 amountTransfer = amountUSDC;
