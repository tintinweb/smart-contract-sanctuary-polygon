...
contract Crowdfunding is Ownable, EIP712 {
    ...
    function getSignedAddress(
        ...
        bytes32 VALIDATE_BUY = keccak256(
            "Payment(uint256 amount,address sender,uint256 nonce)"
        );
        bytes32 structHash = keccak256(
            abi.encode(VALIDATE_BUY, amount, msg.sender, nonce)
        );
    ...
    function contribute(
        ...
        address recoveredAddress = getSignedAddress(amount, nonce, signature);
        ...
        (bool sent, bytes memory data) = collectorAddress.call{
            value: msg.value
        }("");
