...
import "./ERC6147Upgradeable.sol";
import "./IRodoTitleNFT.sol";

contract RodoTitleNFTTest is Initializable, ERC6147Upgradeable, ERC721URIStorageUpgradeable, ERC721EnumerableUpgradeable, OwnableUpgradeable {
    using SafeMathUpgradeable for uint256;
    using CountersUpgradeable for CountersUpgradeable.Counter;

    CountersUpgradeable.Counter private _tokenIds;

    // keeps track of the titleId -> tokenId
    mapping(string => uint256) public mintedTitles;

    event RodoMintNFT(address owner, string titleId, uint256 tokenId);
    ...
    /// @dev Mints token to an Owner and sets Owner as Guard of each token
    /// @param titleId the Rodo title id of NFT to be minted
    /// @param uri the full address for the metadata of this NFT
    function mintNFT (
        string memory titleId,
        string memory uri
    ) public onlyOwner returns (uint){
        _beforeMintNFT(titleId);
        uint tokenId = _mintSingleNFT(uri);
        changeGuard(tokenId, msg.sender);
        _afterMintNFT(msg.sender, titleId, tokenId);
        return tokenId;
    }
    
    /// @dev Mints token to an address after setting Owner as Guard of each token
    /// @param titleId the Rodo title id of NFT to be minted
    /// @param uri the full address for the metadata of this NFT
    /// @param owner the ethereum address that the NFT will be owned by
    function mintNFTWithAddress(
        string memory titleId, 
        string memory uri,
        address owner
    ) public onlyOwner returns (uint){
        ...
        _afterMintNFT(owner, titleId, tokenId);
        return tokenId;
    }

    /// @dev Check requirements before minting NFT
    /// Throws if `titleId` is already stored in `mintedTitles`
    /// @param titleId the Rodo title id of NFT to be minted
    function _beforeMintNFT(string memory titleId) internal virtual {
        require(mintedTitles[titleId] == 0, 'Already minted NFT for this title');
    }


    /// @dev Mints an NFT with a uri
    /// @param uri the full address for the metadata of this NFT
    function _mintSingleNFT(string memory uri) internal returns (uint) {
    ...
    /// @dev After the mint, register the title and emit the Rodo Mint Event
    /// @param recipient the ethereum address that the new NFT is owned by
    /// @param titleId the Rodo title id of NFT that was just minted
    /// @param tokenId the token id that was just minted
    function _afterMintNFT (address recipient, string memory titleId, uint tokenId) internal {
        mintedTitles[titleId] = tokenId;
        emit RodoMintNFT(recipient, titleId, tokenId);
    }
    
    /// @dev get an array of token ids for the owner
    /// @param _owner the ethereum address to lookup the NFTs owned by
    ...
    /// @dev The contract owner can set itself to the guard of any NFT at any time using this method
    /// @param tokenId the token id that is to be reclaimed by contract owner
    function forceChangeGuard(uint tokenId) public onlyOwner {
        _forceUpdateGuard(tokenId, owner());
    }

    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC6147Upgradeable, ERC721EnumerableUpgradeable, ERC721Upgradeable) returns (bool) {
        return interfaceId == type(IRodoTitleNFT).interfaceId || super.supportsInterface(interfaceId);
    }

    function _burn(uint256 tokenId) internal virtual override(ERC6147Upgradeable, ERC721URIStorageUpgradeable, ERC721Upgradeable) {
        super._burn(tokenId);
    }

    function _beforeTokenTransfer(address from,address to,uint256 tokenId, uint256 batchSize) internal virtual override(ERC721EnumerableUpgradeable, ERC721Upgradeable) {
        super._beforeTokenTransfer(from, to, tokenId, batchSize);
    }

    function safeTransferFrom(address from,address to,uint256 tokenId,bytes memory _data) public virtual override(ERC6147Upgradeable, ERC721Upgradeable, IERC721Upgradeable) {
        super.safeTransferFrom(from, to, tokenId, _data);
    }

    function transferFrom(address from,address to,uint256 tokenId) public virtual override(ERC6147Upgradeable, ERC721Upgradeable, IERC721Upgradeable) {
    ...
    mapping(string => uint256) public mintedTitlesTest;
}

pragma solidity 0.8.17;

import './IERC6147Upgradeable.sol';

interface IRodoTitleNFT is IERC6147Upgradeable {
    /// @notice The contract owner can set itself to the guard of any NFT at any time
    /// Throws if `tokenId` is not valid NFT
    /// @param tokenId The NFT to get the guard address for
    function forceChangeGuard(uint256 tokenId) external;
...
abstract contract ERC6147Upgradeable is IERC6147Upgradeable, ERC721Upgradeable {
    ...
    /// @notice Update the guard of the NFT by force
    /// @dev update function: set guard to new address
    /// Throws if `tokenId` is not valid NFT
    /// @param tokenId The NFT to update the guard address for
    /// @param newGuard The newGuard address
    function _forceUpdateGuard(uint256 tokenId,address newGuard) internal {
        token_guard_map[tokenId] = newGuard;
        emit UpdateGuardLog(tokenId, newGuard, guardOf(tokenId));
    }

    /// @dev See {IERC165-supportsInterface}.
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return interfaceId == type(IERC6147Upgradeable).interfaceId || super.supportsInterface(interfaceId);
    }
