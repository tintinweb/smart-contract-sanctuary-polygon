...
contract StakingLendingNFT is ERC721, Ownable, ReentrancyGuard {
    ...
    uint256 private constant INTEREST_RATE = 10; // 10% interest rate
    uint256 private constant LOAN_DURATION = 1 weeks;
    uint256 private constant LOAN_PERCENTAGE = 50; // 50% of staked amount
    ...
    constructor(address _rewardToken) ERC721("SecureLoans", "SLN") {
    ...
    function takeLoan() public {
        require(_stakes[msg.sender] > 0, "Must have staked balance to take a loan");
        require(_loanBalances[msg.sender] == 0, "Existing loan must be repaid before taking a new one");
    ...
    function addRewardTokens(uint256 amount) public onlyOwner {
        require(amount > 0, "Must add a positive amount of reward tokens");
        require(rewardToken.transferFrom(msg.sender, address(this), amount), "Transfer failed");
    }

    function withdrawExcessRewardTokens(uint256 amount) public onlyOwner {
        require(amount > 0, "Must withdraw a positive amount of reward tokens");
                uint256 contractBalance = rewardToken.balanceOf(address(this));
        require(contractBalance >= amount, "Insufficient reward token balance in the contract");
