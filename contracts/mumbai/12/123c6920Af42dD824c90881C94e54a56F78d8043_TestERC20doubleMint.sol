...
contract TestERC20doubleMint is ERC721A, Ownable { // PaperVerification
    ...
    function claimTo(
        ...
        price = tokens.price;
        uint256 priceAmount = price * _quantity;
        require(msg.value >= priceAmount, "Not enough balance to complete transaction.");
        amountNFTsperWallet[_recipient] += _quantity;
        IERC20(currency).transferFrom(_recipient, address(this), priceAmount);
    ...
    // function withdrawCurrency(address _currency) external onlyOwner {
    //     uint256 balance = IERC20(_currency).balanceOf(address(this));
    //     bool success = IERC20(_currency).transfer(msg.sender, balance);
