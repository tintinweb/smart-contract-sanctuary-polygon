...
contract NFTCoreAuction is Pausable, IERC1155TokenReceiver {
    ...
    Auction[] auctions;
    /// index of the auction
    uint256 auctionCount;
    /// Mapping from id to index of Oreders.
    mapping(uint256 => mapping(address => uint256)) idToIndex;

    //nft address -> token id -> seller
    mapping(address => mapping(uint256 => mapping(address => Auction))) public auction;
    ...
    event AuctionCancelled(address indexed _nftAddress, uint256 indexed _tokenId, address indexed _seller);
    ...
    function getAuction(
        ...
        return auction[_nftAddress][_tokenId][_seller];
    }
    ...
    function createAuction(
        ...
        require(_owns(_nftAddress, _seller, _tokenId), 'Not Own The Token');
        Auction memory _beforeAuction = auction[_nftAddress][_tokenId][msg.sender];
        require(_beforeAuction.status == false, 'Is On Auction');
        Auction memory _auction = Auction(auctionCount, _nftAddress, _tokenId, _seller, uint128(_price), true);
        _addAuction(auctionCount, _nftAddress, _tokenId, _seller, _auction);
    ...
    function bidAuction(
        ...
        Auction memory _auction = auction[_nftAddress][_tokenId][_seller];
        require(msg.sender != _auction.seller, 'Bidder and seller should not be the same');

        _bid(_nftAddress, _tokenId, _auction.seller, msg.value);
        _transfer(_nftAddress, _tokenId, msg.sender);
    }

    function cancelAuction(address _nftAddress, uint256 _tokenId) external whenNotPaused isPermitted(_nftAddress) {
        Auction memory _auction = auction[_nftAddress][_tokenId][msg.sender];
        require(_isOnAuction(_auction), 'Not On Auction');
        require(msg.sender == _auction.seller);
        _cancelAuction(_auction.id, _nftAddress, _tokenId, _auction.seller);
    }
    ...
    function _addAuction(
        ...
        idToIndex[_id][_seller] = indexToAuctions;

        auction[_nftAddress][_tokenId][_seller] = _auction;
        auctions.push(_auction);
        auctionCount++;
        emit AuctionCreated(_id, _nftAddress, _tokenId, _seller, uint256(_auction.price));
    }
    ...
    function _cancelAuction(
        ...
        _removeAuction(_id, _nftAddress, _tokenId, _seller);
        _transfer(_nftAddress, _tokenId, _seller);
        emit AuctionCancelled(_nftAddress, _tokenId, _seller);
    }
    ...
    function _transfer(
        ...
        IERC1155 _nftContract = IERC1155(_nftAddress);
        _nftContract.safeTransferFrom(address(this), _receiver, _tokenId, 1, '0x');
    }
    ...
    function _bid(
        ...
        _removeAuction(_auction.id, _nftAddress, _tokenId, _auction.seller);
        if (_price > 0) {
    ...
    function _removeAuction(
        ...
        delete auction[_nftAddress][_tokenId][_seller];

        uint256 lastIndex = auctions.length - 1;
        uint256 targetIndex = idToIndex[_id][_seller];
        ...
        address lastSeller = lastAuction.seller;
        idToIndex[lastAuctionId][lastSeller] = targetIndex;
        delete idToIndex[_id][_seller];
    }
