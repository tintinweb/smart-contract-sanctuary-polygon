...
contract PublicGovSale is Initializable, UUPSUpgradeable, PausableUpgradeable, OwnableUpgradeable, AdminProxyManager {
    ...
    // function buyToken(uint128 _amountIn) external whenNotPaused {
        // (bool success, bytes memory data) = address(this).call(abi.encodeWithSelector(PublicGovSale._buyToken.selector, _amountIn, _msgSender()));
        // require(success, "failed");
        // uint128 amountDebt = abi.decode(data, (uint128));
    ...
    function _buyToken(
        ...
        if(sender != factory.crossChainManager()) _buyer = sender;

        uint32 running = boosterProgress();
        require(
            // (_msgSender() == factory.crossChainManager() || _msgSender() == address(this)) &&
            running > 0 &&
        ...
        // uint128 amountDebt = amountInFinal + feeCharged;

        if(sender == factory.crossChainManager()){
