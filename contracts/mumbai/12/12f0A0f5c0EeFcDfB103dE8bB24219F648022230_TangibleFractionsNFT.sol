    ...
    IERC721Receiver
{
    ...
    uint256 public override fullShare; //number which represents 100% sets via constructor. 100 000 for example that supports 0.001
    ...
    function claimAll() external returns (uint256 amount) {
        ...
        uint256 numFractions = balanceOf(sender);
        for (uint256 i = 0; i < numFractions; i++) {
            uint256 fractionId = tokenOfOwnerByIndex(sender, i);
            uint256 free = claimableIncome(fractionId);
            if (free > 0) {
                amount += free;
                claimed[fractionId] += free;
    ...
    function totalClaimableIncome() external view returns (uint256 amount) {
        address sender = msg.sender;
        uint256 numFractions = balanceOf(sender);
        for (uint256 i = 0; i < numFractions; i++) {
            uint256 fractionId = tokenOfOwnerByIndex(sender, i);
            amount += claimableIncome(fractionId);
        }
    }
    ...
    function _transferShare(
        ...
        return _transferShare(contractAddress, from, contractAddress, to);
    }
    ...
    function _transferShare(
        ...
        RevenueShare revenueShare = IFactory(factory).revenueShare();
        share = uint256(
            revenueShare.share(
    ...
    function _transferShare(
        ...
        RevenueShare revenueShare = IFactory(factory).revenueShare();
        RevenueShare rentShare = IFactory(factory).rentShare();
        bool paysRent = tnft.paysRent();
    ...
    function _mintFtnft(
        ...
        _transferShare(address(0), 0, address(this), tokenId, share);
    }
        ...
        returns (uint256 tokenKeep, uint256 tokenSell)
    {
        ...
        tokenKeep = ++_tokenIds;
        _mintFtnft(owner, tokenKeep, (share * keepShare) / fullShare);
        //set shares
        ...
        tokenSell = ++_tokenIds;
        _mintFtnft(owner, tokenSell, (share * sellShare) / fullShare);
        //set shares for sale
        ...
        returns (uint256[] memory fractTokenIds)
    {
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            uint256 newShare = (uint256(share) * shares[i]) / fullShare;
            if (i != 0) {
                //mint others
                _mintFtnft(ownerOfFract, ++_tokenIds, newShare);
    ...
    function defractionalize(uint256[] calldata tokenIds) external override {
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            if (i != 0) {
                ...
                _transferShare(address(this), tokenId, firstFract);
