...
contract CryptoBeastsNFT is ERC721Enumerable, Ownable, Pausable {
    ...
    // Probabilidad de comÃºn = 100 - LEGENDARY_PROBABILITY - RARE_PROBABILITY

    // Variables de estado para llevar la cuenta de cuÃ¡ntas cartas de cada tipo se han creado
    ...
    struct Card {
        // cardType: 0: Criatura, 1: Habilidad, 2: Objeto, es el mÃ³dulo 3 del cardId
        uint256 cardId;
        // string imageURI;
        uint8 rarity;
    }
    ...
    // Limitar la cantidad de cartas que se pueden quemar por dÃ­a
    uint256 public burnLimit = 10; // LÃ­mite de cartas quemadas por usuario en un perÃ­odo de tiempo
    uint256 public burnTimeFrame = 86400; // PerÃ­odo de tiempo en segundos (86400 segundos = 24 horas)
    ...
    // FunciÃ³n para agregar nuevos sets de cartas
    function _agregarSetCartas(
        ...
        // AÃ±adir criaturas al cardStock
        for (
        ...
        // AÃ±adir habilidades al cardStock
        for (
    ...
    function burn(uint256 tokenId) external {
        ...
        if (currentTime - userLastBurnTime > burnTimeFrame) {
            // Reinicia el conteo de cartas quemadas si ha pasado el perÃ­odo de tiempo
    ...
    // FunciÃ³n para retirar tokens del contrato (usada por el propietario del contrato)
    function withdraw() external onlyOwner {
        uint256 balance = paymentToken.balanceOf(address(this));
        paymentToken.transfer(msg.sender, balance);
    }

    // FunciÃ³n pÃºblica para agregar nuevos sets de cartas, restringida al propietario del contrato
