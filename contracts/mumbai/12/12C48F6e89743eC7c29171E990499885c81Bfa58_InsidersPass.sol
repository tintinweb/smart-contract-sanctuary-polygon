...
contract InsidersPass is InsidersPassAuction {
    ...
    mapping(uint256 => bool) public promoValid;

   error PromoCodeOnEvent(string);
  
   enum promoCodeType{ fixedDiscount, discountPercentage, freeTicket }

    mapping(uint256 => PromoCodes) public promoCodeDetails;

    event URI(string value, bytes indexed id);
    event PromoCodeUsed(PromoCodes value);
    ...
    function mintTicketsWithFixPrice(
        ...
        uint256 ticketID,
        uint256 noOfCopies,
        uint256 _promoId // for checking promoCode
    ) public payable {
        ...
        if (_msgSender() != owner()) {
            if ( _promoId !=0  ) {
                require(promoValid[_promoId],"InvalidPromoCode");
                for(uint256 i = 0; i < promoCodeDetails[_promoId].eventId.length; i++){
                   if( ticketTypeDetails[ticketID].eventIDforTicket == promoCodeDetails[_promoId].eventId[i] ){
                     response = true;
                   }
                }
                if(response != false){
                    require(noOfCopies > 0, "Enter Atleast One Copy Of Ticket");
             
                require(
                    promoCodeDetails[_promoId].maxUsers > 0, "UsersLimitReached"
                );
                require(
                    promoCodeDetails[_promoId].promoStartDate <=
                        block.timestamp &&
                        promoCodeDetails[_promoId].promoEndDate >= // 24 hours up =
                        block.timestamp,
                    "NotValidPromoTime"
                );
                if(promoCodeDetails[_promoId].choice == promoCodeType.fixedDiscount )
               {
                   fixedDiscount(_promoId,ticketID, noOfCopies);
               }
               else if(promoCodeDetails[_promoId].choice == promoCodeType.discountPercentage)
               {
                   discountPercentage(_promoId,ticketID, noOfCopies);
               }
               else  if(promoCodeDetails[_promoId].choice == promoCodeType.freeTicket )
               {
                   if(promoCodeDetails[_promoId].ticketId == ticketID && promoCodeDetails[_promoId].ticketId != 0)
                   {
                    checkPrice(
                        0 
                    );
                        promoCodeDetails[_promoId].maxUsers--;
                   }
                   else {
                             revert PromoCodeOnEvent("PromoCodeInvalidForFree");
         ...
         revert PromoCodeOnEvent("PromoCodeIsNotValidOnEvent");
            }
             //neew else
            } else {
                checkPrice(
        ...
        emit PromoCodeUsed(promoCodeDetails[_promoId]);
    }
     function promoId(uint256 _promoId) internal view {
        require(promoValid[_promoId] == false, "PromoId is Already Exist");
    }
    function checkPrice(uint256 _price) internal view {
        require(msg.value == _price, "Insufficient Amount");
    }
    function fixedDiscount(uint256 _promoId, uint256 ticketID, uint256 noOfCopies) internal   {
         {
                    require(
                        (ticketTypeDetails[ticketID].ticketFixPrice *
                            noOfCopies) -
                            (promoCodeDetails[_promoId].fixedDiscount) >=
                            0,
                        "InvalidDiscountPriceOnFixedDiscount"
                    );
                    checkPrice(
                        ((ticketTypeDetails[ticketID].ticketFixPrice *
                            noOfCopies) -
    ...
    function discountPercentage(uint256 _promoId, uint256 ticketID, uint256 noOfCopies) internal {
                if (
                        promoCodeDetails[_promoId].maxDiscountAmount >=
                        (((ticketTypeDetails[ticketID].ticketFixPrice *
                            noOfCopies) *
                            (promoCodeDetails[_promoId].discountPercentage)) /
                            100)
                    ) {
                        checkPrice(
                            (
                                ((ticketTypeDetails[ticketID].ticketFixPrice *
                                    ...
                                    (((ticketTypeDetails[ticketID]
                                        .ticketFixPrice * noOfCopies) *
                                        (
                                            promoCodeDetails[_promoId]
                                                .discountPercentage
                                        )) / 100))
                            )
                        );
...
//AddPromoCode
    function addPromoCodes(
        uint64[] memory _promoId,
        PromoCodes[] memory _promoCodes
    ) external  whenNotPaused onlyOwner {
                for(uint256 i = 0; i < _promoId.length; i++){
                     promoId(_promoId[i]);
                for(uint256 j = 0; j < _promoCodes[i].eventId.length; j++){
                            eventIsRegistered(_promoCodes[i].eventId[j]);
                }
                 if(_promoCodes[i].choice == promoCodeType.freeTicket){
                            TicketExists(_promoCodes[i].ticketId);
                          }
                promoValid[_promoId[i]] = true;
                promoCodeDetails[_promoId[i]] = _promoCodes[i];
                }
    }
    // updatePromoCode
    function updatePromoCode(
        ...
        PromoCodes memory _promoCodes
                    ...
                    require(promoValid[_promoId],"PromoIsInValid");
                     for(uint256 j = 0; j < _promoCodes.eventId.length; j++){
                            eventIsRegistered(_promoCodes.eventId[j]);
                }
                 if(_promoCodes.choice == promoCodeType.freeTicket){
                            TicketExists(_promoCodes.ticketId);
                          }
                    promoCodeDetails[_promoId] = _promoCodes;
   }
    ...
    function registerEvent(
        ...
        eventDetails[newEventID].eventEndDate = _eventEndDate + 24 hours; // 24 hours add to check
