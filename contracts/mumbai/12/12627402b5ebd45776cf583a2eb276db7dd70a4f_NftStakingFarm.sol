    ...
    ERC721Holder
{
  ...
  //  using Calculator for uint256;
   ...
   // event BaseApySet(uint256 value, address sender);
    ...
    mapping(uint256 => uint8) private nftApys;
    ...
    function _unstakeNft(
        ...
        if (nftBalancesStacked[msg.sender] == 0) {
        ...
        //withdraw reward too
        require(
            umiToken.transfer(msg.sender, balances[msg.sender]),
    ...
    */
    function claimRewardById(uint256 _id) external whenNotPaused nonReentrant {
        ...
        require(
            umiToken.transfer(msg.sender, balance),
            "claim: transfer failed"
        );
        ...
        returns (uint256, uint256)
    {
       // uint256 currentBalance = balances[_user];
        ...
        uint256 totalWithInterest = _days.mul(BASE_APY).mul(nftBalancesStacked[_user]);
