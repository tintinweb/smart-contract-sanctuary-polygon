...
contract RoyaltyVaultForPassHolder is AccessControl, ReentrancyGuard{
  using SafeMath for uint256;
  uint256 public requestId;
  uint256 public receiveETHId;
  address public WETH;
  address public TEAM_WALLET = 0xB48495B000e82bF4bCbDEe17997a70146964d601;
  ...
  mapping(uint256=>uint256[]) public tokenIdToRequestIds;
  struct Request{
      uint256 amount;
      bool isWETH;
      uint256 tokenId;
  ...
  mapping(uint256=>MembersAmount[]) public tokenIdToMembersAmounts;
  ...
  event Sweeped(address indexed account, uint256 indexed amountETH, uint256 indexed amountWETH);
  ...
  function setTeamWallet(address wallet) external onlyRole(DEFAULT_ADMIN_ROLE) {
  ...
  function addRequest(uint256 amount, bool isWETH, uint tokenId, address[] memory members) external onlyRole(REQUESTER_ROLE) {
    ...
    tokenIdToRequestIds[tokenId].push(requestId);
    ...
    MembersAmount memory ma = MembersAmount(members, revenuePerMember);
    tokenIdToMembersAmounts[tokenId].push(ma);
    emit RequestAdded(requestId, isWETH, amount, tokenId, members);
  }
  ...
  function removeRequest(uint256 _requestId)external onlyRole(REQUESTER_ROLE){
    ...
    emit RequestRemoved(requestId, isWETH, amount, tokenId, members);
  }
  ...
  function withdraw() external payable nonReentrant{
    ...
    if(claimableETH> 0 && _isETHClaimable(claimableETH)) {
      ...
      _transfer(msg.sender, claimableETH);
    }
  ...
  function sweep(address[] memory addressList) external payable onlyRole(WITHDRAWER_ROLE) nonReentrant{
    ...
    for(uint256 i; i < addressList.length; i++){
      ...
      emit Sweeped(addressList[i], claimableETH, claimableWETH);
    }
  }
  ...
  function _isWETHClaimable(uint256 claimed) internal view returns (bool){
    if(WETH == address(0)) revert NoWETHAddress();
