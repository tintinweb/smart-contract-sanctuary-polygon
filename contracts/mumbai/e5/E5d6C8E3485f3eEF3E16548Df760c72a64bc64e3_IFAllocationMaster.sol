...
contract IFAllocationMaster is Ownable, ReentrancyGuard {
    ...
    // winner mapping -- (track, user address) => is winner of track?
    mapping(uint24 => mapping(address => bool)) public winners;

    //PaybNftMarketplace
    address PaybNftMarketplace;
    ...
    // ) external onlyOwner {
        ...
        tracks.push(
            TrackInfo({
                ...
                startTime: _startTime, // time start track
                endTime: _endTime, // time end track
    ...
    function addUserCheckpoint(
        ...
        if (nCheckpointsUser == 0) {
            ...
            require(amount >= track.minTotalStake, 'exceeds staking cap');
    ...
    function stake(uint24 trackId, uint104 amount) external nonReentrant {
        ...
        require(!_msgSender().isContract(), "Token address is a contract address");

        // get track info
        TrackInfo storage track = tracks[trackId];

        //check expired stake
        require(track.startTime < block.timestamp && block.timestamp < track.endTime,"staking time has expired");
    ...
    // unstake
    function unstake(uint24 trackId) external nonReentrant {
        // require msg.sender is wallet
        require(!_msgSender().isContract(), "Token address is a contract address");
        ...
        // require not winners
        require(!winners[trackId][_msgSender()], 'the winner is not received');

        // get track info
        TrackInfo storage track = tracks[trackId];

        //check expired stake
        require(block.timestamp > track.endTime, "staking is not over yet");
        ...
        // check staked
        require(checkpoint.staked > 0 , 'staked is 0');
        ...
        // emit
        emit Unstake(trackId, _msgSender(), checkpoint.staked);
    }

    //set Winers
    function setWinners(uint24 trackId, address[] memory _winners) public onlyOwner(){
        // require track is disabled
        require(trackDisabled[trackId], 'track !disabled');

        TrackInfo storage track = tracks[trackId];
        for(uint256 i=0; i< _winners.length; ++i){
            winners[trackId][_winners[i]] = true;
        }
        IPaybNftMarketplace(PaybNftMarketplace).mintFragment(_winners, track.rootIdToken);
    }
