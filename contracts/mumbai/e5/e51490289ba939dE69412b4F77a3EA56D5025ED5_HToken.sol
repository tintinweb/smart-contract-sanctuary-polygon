...
contract HToken is HTokenInternal, ReentrancyGuard {
  ...
  function depositUnderlying(uint256 _amount) external nonReentrant {
    depositUnderlyingInternal(_amount, msg.sender);
  }
   ...
   * @dev Pull pattern, user must approve the contract before calling.
   * @param _amount Quantity of underlying ERC-20 to transfer in
   * @param _to Target address to deposit to
   */
  function depositUnderlyingTo(uint256 _amount, address _to) external nonReentrant {
...
contract HTokenInternal is ERC1155, IERC721Receiver, AccessControl {
  ...
  event SetDAO(address _oldDao, address _newDao);
  ...
  function depositUnderlyingInternal(uint256 _amount, address _to) internal {
    ...
    uint256 tokensToMint = (doUnderlyingTransferIn(_to, _amount) * 1e18) / exchangeRateCurrent();
    totalHTokenSupply += tokensToMint;

    _mint(_to, 0, tokensToMint, "");

    emit UnderlyingDeposited(_to, _amount, tokensToMint, totalHTokenSupply);
  }
  ...
  function getCashPrior() public view returns (uint256) {
    uint256 totalFees = totalAdminFees + totalHiveFees + totalProtocolFees;
  ...
  function _withdrawProtocolFees(uint256 _amount) external {
    ...
    if (getCashPrior() + totalProtocolFees < cachedTotalProtocolFees) {
      revert AdminError(Error.TOKEN_INSUFFICIENT_CASH);
    }
  ...
  function _setDao(address _newAddress) external {
    if (_newAddress == address(0)) revert WrongParams();
    if (!hasRole(DEFAULT_ADMIN_ROLE, msg.sender)) revert Unauthorized();
    emit SetDAO(dao, _newAddress);
    dao = _newAddress;
  }
