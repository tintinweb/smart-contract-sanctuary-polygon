...
contract KraksV3 is Ownable {
    ...
    mapping (string => mapping (string => bool)) public userIsOwnerOfChallenge;
    mapping (string => mapping (string => uint)) public userAmountAddedInChallenge;
    mapping (string => mapping (string => bool)) public userIsInvitedToChallenge;
    ...
    function createChallenge(
        ...
        require (userIsOwnerOfChallenge[ownerId_][challengeId_], "User is not owner of the challenge.");
        require (userAmountAddedInChallenge[ownerId_][challengeId_] == amount_, "User is not paying what it was set in the app.");
        ...
        _payAndAddUserToChallenge(challengeId_, ownerId_, amount_);
    }
    ...
    function acceptInvitation(
        ...
        require(userIsInvitedToChallenge[userId_][challengeId_], "User is not invited to the challenge.");
        require (userAmountAddedInChallenge[userId_][challengeId_] == amount_, "User is not paying what it was set in the app.");
        require(!userInChallenge[challengeId_][userId_], "User is already in challenge.");
        require(!challengeInfo[challengeId_].isClosed, "Challenge is closed.");

        _payAndAddUserToChallenge(challengeId_, userId_, amount_);
    }
    ...
    function addOwnerToChallenge(
        string calldata ownerId_,
        string calldata challengeId_,
        uint amount_
    ) external onlyOwner {
        userIsOwnerOfChallenge[ownerId_][challengeId_] = true;
        userAmountAddedInChallenge[ownerId_][challengeId_] = amount_ * 1 ether;
    }

    function inviteUserToChallenge(
        string calldata userId_,
        string calldata challengeId_,
        uint amount_
    ) external onlyOwner {
        userIsInvitedToChallenge[userId_][challengeId_] = true;
        userAmountAddedInChallenge[userId_][challengeId_] = amount_ * 1 ether;
    }
