...
contract RewardsDistributor is IRewardsDistributor {
    ...
    function _claim(uint _tokenId, address ve, uint _last_token_time) internal returns (uint) {
        ...
        IVotingEscrow.Point memory user_point = IVotingEscrow(ve).user_point_history(
            _tokenId,
            user_epoch
        );
        ...
        for (uint i = 0; i < 50; i++) {
            ...
            if (week_cursor >= user_point.ts && user_epoch <= max_user_epoch) {
                ...
                uint balance_of = Math.max(
                    uint(int256(old_user_point.bias - dt * old_user_point.slope)),
                    0
                );
