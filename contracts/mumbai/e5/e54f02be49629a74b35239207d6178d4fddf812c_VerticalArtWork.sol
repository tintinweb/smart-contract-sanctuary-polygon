...
contract VerticalArtWork is ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable, Applications {
    ...
    constructor( address _OracleAUG, address _Gold, string memory _setImageURL, string memory _setCertificateUrl, address _galleryArtAddress) ERC721("ArtWork Vertical", "AWV") {
    ...
    /*
        Funciones:
        1-Crear Art Work
        2-Cambiar Certificado del Art Work
        3-Cambiar el nombre del dueño
        4-Transferir Art Work
        5-Add Gold
        6-Remove Gold
        7-Cambiar el Precio
        
    */
    function createArtWork (string memory _ownerName, uint _price, uint _royaltyAmount, address _royaltyAddress, string memory _artWorkName, string memory _certificateHash) public {
        ...
        TokenDetails[nextTokenId] = TokenDetail({
            ...
            certificateHash: string(_certificateHash),
            creator: address(msg.sender),
            owner: address(msg.sender),
            royalties: uint(_royaltyAmount),
            price: uint(_price),
            goldTokens: uint(0),
            priceVisibility: bool(true)
        });
    ...
    function changeCertificate (uint _tokenId, string memory _newCertificate) public {
        //Solo el dueño de la galeria (que es quien emite) puede cambiarlo
        require(admin[msg.sender],string("changeCertificate: You are not a Gallery Admin"));
        TokenDetails[_tokenId].certificateHash=_newCertificate;
    }
    ...
    function changeOwnerAndTransfer (uint _tokenId, string memory _newOwner, bool _transfer, address _newOwnerAddress) public {
    ...
    function changePrice(uint _tokenId, uint _newPrice) public {
    ...
    function publicPriceOnArtWork(uint _tokenId, bool _status) public {
        require(ownerOf(_tokenId)==msg.sender,string("publicPriceOnArtWork: Not the Token Owner"));
        TokenDetails[_tokenId].priceVisibility=_status;
    }
    ...
    function compiledAttributes (uint256 _tokenId) internal view returns (string memory) {
        ...
        _certificateAddress = string.concat(_certificateUrl,"/",TokenDetails[_tokenId].certificateHash);
        _certificateAddress = string.concat(_certificateAddress,".",".pdf");
        ...
        traits =  string(abi.encodePacked(
            ...
            attributeForTypeAndValue("Certificate URL Hash", _certificateAddress)
        ));
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        */
        string memory metadata = string(abi.encodePacked(
            '{"name":"Vertical Certificate and Art Work NFT",',
