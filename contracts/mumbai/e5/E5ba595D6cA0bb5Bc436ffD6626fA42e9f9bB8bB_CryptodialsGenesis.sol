...
contract CryptodialsGenesis is ERC721A, Ownable {
    ...
    uint256 public publicSalePrice = 20000000000000;
    uint256 public privateSalePrice = 10000000000000;
    ...
    // Private Mint
    function privateMint(uint256 quantity, bytes calldata _allowlistSignature)
        external
        payable
    {
        if (!privateSaleActive) revert SaleNotActive();
        if (msg.value < quantity * privateSalePrice) revert NotEnoughETH();
        if (
            !verifySignature(
