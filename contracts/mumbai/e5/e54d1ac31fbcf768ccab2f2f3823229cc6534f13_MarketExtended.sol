...
interface IMarketExtended is IMarketCommon {
  ...
  function longShort() external view returns (ILongShort);
  ...
  MarketTieredLeverageGetters
{
  ...
  modifier systemStateUpdated() {
    require(
      !marketDeprecated,
      "Market is deprecated, please exit using function: exitDeprecatedMarket"
    );
  ...
  modifier checkMintingNotPaused() {
    require(!mintingPaused, "Minting actions cannot be executed because the market is paused");
    _;
  }
  ...
  constructor(address paymentToken, ILongShort longShort) {
  ...
  // This will only be called if upkeep has missed, its a more effcient function to catch up missed epochs.
  function _rebalanceMarketPools(
    ...
    uint256 actualLiquidityBefore = calculateActualLiquidity();
    ...
    uint256 actualLiquidityAfter = calculateActualLiquidity();
  ...
  /// @dev We have to check market not deprecated to avoid changing state once deprecated
  function updateSystemStateSpecific(uint256 updateNEpochs) external checkMarketNotDeprecated {
  ...
  function _updateSystemStateSpecific(uint256 updateNEpochs) internal virtual {
    uint256 currentEpochIndex = _oracleManager.getCurrentEpochIndex();
    require(
      currentEpochIndex - _epochInfo.latestExecutedEpochIndex > 2,
    ...
    (
      ...
      int256 previousPrice,
      IOracleManagerFixedEpoch.MissedEpochExecutionInfo[] memory epochsToExecute
    ) = _oracleManager.getOracleInfoForSystemStateUpdate(
        _epochInfo.latestExecutedEpochIndex,
        _epochInfo.previousExecutionPriceOracleIdentifier,
        _epochInfo.previousEpochEndTimestamp,
        updateNEpochs
  ...
  function _exitDeprecatedMarket(address user, IMarketCommon.PoolType poolType) internal {
    ...
    IMarketCommon.UserAction storage userAction;
    ...
    for (uint8 poolIndex = 0; poolIndex < maxPoolIndex; poolIndex++) {
      ...
      if (userAction.amount > 0 || userAction.nextEpochAmount > 0) {
        IYieldManager(_yieldManager).transferPaymentTokensToUser(
          user,
