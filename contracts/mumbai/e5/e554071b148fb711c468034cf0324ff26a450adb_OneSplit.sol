...
contract OneSplitRoot {
    ...
    IWETH internal constant wnative = IWETH(0xc86c7C0eFbd6A49B35E8714C5f59D99De09A225b);
...
contract OneSplit is Initializable, IOneSplit, OneSplitRoot, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    function getExpectedReturnETH(
        ...
        (returnAmount, ) = getExpectedReturn(srcStableFromToken, wnativeAddress, srcStableFromTokenAmount, parts, flags);
        return returnAmount;
    }
    ...
    function setFlagToFactoryAddress(uint256 _flagCode, address _factoryAddress) public onlyRole(DEFAULT_ADMIN_ROLE) returns (bool) {
        require(_flagCode != 0, "Flag can't be 0");
        flagToAddress[_flagCode] = address(_factoryAddress);
        return true;
    }

    function setWNativeAddresses(address _native, address _wrappedNative) public onlyRole(DEFAULT_ADMIN_ROLE) returns (bool) {
    ...
    {
        if ((flag.sub(0x3000).mul(flag.sub(0x2000)) <= 0)) {
            return _swapOnOneInch;
        } else if ((flag.sub(0x1000).mul(flag.sub(0x01)) <= 0)) {
    ...
    function _swap(
        ...
        function(IERC20Upgradeable, IERC20Upgradeable, uint256, bytes memory, uint256) reserve = _getReserveExchange(flags);
    ...
    function _swapOnExchangeInternal(
        ...
        IUniswapV2Factory exchangeInstance = IUniswapV2Factory(dexAddress);
