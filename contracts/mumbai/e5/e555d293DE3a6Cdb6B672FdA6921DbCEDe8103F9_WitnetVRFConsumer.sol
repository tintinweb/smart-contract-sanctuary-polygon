...
contract WitnetVRFConsumer is Ownable {
    ...
    function get_nounce() public view returns(uint256){
        return address(witnet) != address(0) ? nounces[witnet.latestRandomizeBlock()] : 0;
    }
    function get_witnet() public view returns (address){return address(witnet);}
    function should_refresh_randomness() public view returns (bool) {
        if(address(witnet) == address(0))
            return false;
        uint256 fullfilled_block = witnet.latestRandomizeBlock();
        return (fullfilled_block > max_nounce_to_refresh);
    }
}
