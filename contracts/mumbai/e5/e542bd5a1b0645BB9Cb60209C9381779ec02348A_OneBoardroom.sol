...
interface ITreasury is IEpoch {
    ...
    function getDarkUpdatedPrice() external view returns (uint256);
    ...
    function getDarkLockedBalance() external view returns (uint256);

    function getDarkCirculatingSupply() external view returns (uint256);
    ...
    function previousEpochDarkPrice() external view returns (uint256);
...
contract OneBoardroom is ShareWrapper, ContractGuard {
    ...
    IERC20 public dark;
    IERC20 public wcro;
    IERC20 public light; // second collateral
    ...
    uint256 public lightCollateralDiscountPercent;
    uint256 public darkTokenSupplyTarget;
    ...
    function initialize(
        ...
        minCollateralPercent = 2500; // 25%
        maxCollateralPercent = 7500; // 75%

        lightCollateralDiscountPercent = 2000; // 20% discount if pay by DARK
        darkTokenSupplyTarget = 500000000 ether;
    ...
    function setDark(IERC20 _light) external onlyOperator {
        light = _light;
    }
    ...
    function setPegTokenConfig(address _token, address _oracle) external onlyOperator {
    ...
    function setMinCollateralPercent(uint256 _minCollateralPercent) external onlyOperator {
        require(_minCollateralPercent <= 3500, "too high"); // <= 35%
        minCollateralPercent = _minCollateralPercent;
    }

    function setMaxCollateralPercent(uint256 _maxCollateralPercent) external onlyOperator {
        require(_maxCollateralPercent >= 6500, "too low"); // >= 65%
        maxCollateralPercent = _maxCollateralPercent;
    }

    function setLightCollateralDiscountPercent(uint256 _lightCollateralDiscountPercent) external onlyOperator {
        require(_lightCollateralDiscountPercent <= 5000, "too high"); // <= 50%
        lightCollateralDiscountPercent = _lightCollateralDiscountPercent;
    }

    function setDarkTokenSupplyTarget(uint256 _darkTokenSupplyTarget) external onlyOperator {
        require(_darkTokenSupplyTarget >= 100000000 ether, "too low"); // >= 100 million DARKO
        darkTokenSupplyTarget = _darkTokenSupplyTarget;
    }
    ...
    function canClaimReward() public pure returns (bool) {
        ...
        // return _treasury.previousEpochDarkPrice() >= 1e18 && _treasury.getNextExpansionRate() > 0; // current epoch and next epoch are both expansion
    }

    function getCollateralPercent() public view returns (uint256 _collateralPercent) {
        uint256 _darkSupply = dark.totalSupply();
        uint256 _darkTokenSupplyTarget = darkTokenSupplyTarget;
        if (_darkSupply >= _darkTokenSupplyTarget) {
            _collateralPercent = maxCollateralPercent;
        } else {
            _collateralPercent = _darkSupply.mul(10000).div(_darkTokenSupplyTarget);
    ...
    function getDarkOverWftmPrice() public view returns (uint256) {
        return (lightOracle == address(0)) ? 1e18 : uint256(IEpoch(lightOracle).getPegPriceUpdated());
    }

    function getPegOriginalTokenOverWftmPrice(address _token) public view returns (uint256) {
        address _oracle = pegOriginalTokenOracle[_token];
        return (_oracle == address(0)) ? 1e18 : uint256(IEpoch(_oracle).getPegPriceUpdated());
    }
    ...
    function getCollateralForClaimAllPegTokens(address _token, address _member) public view returns (uint256) {
        uint256 _earned = earned(_token, _member);
        uint256 _price = getPegOriginalTokenOverWftmPrice(_token);
        uint256 _collateralPercent = getCollateralPercent();
        return _earned.mul(_price).mul(_collateralPercent).div(1e22); // 1e18 * 10000
    }

    function getCollateralForClaimAllPegTokens(address _member) public view returns (uint256 _numOfPegTokens, address[] memory _pegTokenAddresses, uint256[] memory _earnedPegTokens, uint256[] memory _collateralForPegTokens, uint256 _totalCollateral, uint256 _totalDarkCollateral) {
        _numOfPegTokens = numOfPegTokens();
        _pegTokenAddresses = new address[](_numOfPegTokens);
        _earnedPegTokens = new uint256[](_numOfPegTokens);
        ...
        for (uint256 i = 0; i < _numOfPegTokens; i++) {
            ...
            uint256 _price = getPegOriginalTokenOverWftmPrice(_token);
            uint256 _collateral = _earned.mul(_price).mul(_collateralPercent).div(1e22);
            _collateralForPegTokens[i] = _collateral;
            _totalCollateral = _totalCollateral.add(_collateral);
        }
        _totalDarkCollateral = _totalCollateral.mul(1e18).div(getDarkOverWftmPrice());
        _totalDarkCollateral = _totalDarkCollateral.sub(_totalDarkCollateral.mul(lightCollateralDiscountPercent).div(10000)); // sub discount
    }

    function getTotalCollateralForClaimAllPegTokens(address _member) public view returns (uint256 _totalCollateral) {
    ...
    function getTotalDarkCollateralForClaimAllPegTokens(address _member) public view returns (uint256 _totalDarkCollateral) {
        uint256 _totalCollateral = getTotalCollateralForClaimAllPegTokens(_member);
        _totalDarkCollateral = _totalCollateral.mul(1e18).div(getDarkOverWftmPrice());
        _totalDarkCollateral = _totalDarkCollateral.sub(_totalDarkCollateral.mul(lightCollateralDiscountPercent).div(10000)); // sub discount
    }
    ...
    function claimReward(bool _useDarkCollateral) external onlyOneBlock {
//        require(canClaimReward(), "contraction");
        _claimReward(_useDarkCollateral);
    }

    function _claimReward(bool _useDarkCollateral) internal updateReward(msg.sender) {
        uint256 _reward = members[address(dark)][msg.sender].rewardEarned;
        if (_reward > 0) {
            if (_useDarkCollateral) {
                light.safeTransferFrom(msg.sender, insuranceFund, getTotalDarkCollateralForClaimAllPegTokens(msg.sender));
            } else {
                wcro.safeTransferFrom(msg.sender, insuranceFund, getTotalCollateralForClaimAllPegTokens(msg.sender));
            }
        }
