...
// File: contracts/DiscordNFT/AlterAccessPass.sol
  ...
  BaseRelayRecipient
{
  ...
  function initialize() public initializer {
    ...
    __ERC1155_init_unchained(
      "https://firebasestorage.googleapis.com/v0/b/pokerx-6c898.appspot.com/o/proto-planet-lands%2F10.json?alt=media&token=79852c19-bf97-4b3b-8a5e-965b44021456"
    );
  ...
  mapping(uint256 => AlterPass) NFTLogs;

  mapping(address => bool) public claimedWallets;

  event Mint(uint256 tokenId, address tokenOwner, AlterPass newNFT);

  event MintBatch(uint256 tokenId, address tokenOwner, AlterPass newNFT);
  ...
  // Mint Methods
  function mintPass() public payable virtual mintCompliance(_msgSender()) {
    _mint(_msgSender(), nextTokenId, 1, "");

    AlterPass memory newNft = AlterPass({
      id: nextTokenId,
      owner: _msgSender(),
    ...
    emit Mint(nextTokenId, _msgSender(), newNft);
    nextTokenId++;
    tokensInCirculation++;
  }
    ...
    virtual
  {
    ...
    for (uint256 i = 0; i < ids.length; i++) {
      ...
      emit MintBatch(ids[i], msg.sender, newNft);
      nextTokenId++;
      tokensInCirculation++;
    }
  }
  ...
  function getNFTDetails(uint256 id) external view returns (AlterPass memory) {
    return NFTLogs[id];
  }
