...
//        IUniswapV2Router02 _router = router; // gas savings
        tokenA.uniApprove(address(router), _addLiqDescriptor.amountADesired);
        tokenB.uniApprove(address(router), _addLiqDescriptor.amountBDesired);
        (uint256 amountCrowd, uint256 amountUsdt, uint256 liquidity) =
            router.addLiquidity(
                address(tokenA),
                address(tokenB),
        ...
        return (amountCrowd, amountUsdt, liquidity);
    }
    ...
    function removeLiquidity(
        ...
        IUniswapV2Router02 _router = router; // gas savings
        pair.uniApprove(address(_router), _removeLiqDescriptor.amountLP);
        (uint256 amountCrowd, uint256 amountUsdt) = _router.removeLiquidity(
            address(tokenA),
            address(tokenB),
            _removeLiqDescriptor.amountLP,
    ...
    function stake(uint256 _amount) internal override {
        ...
        _stakingLP.stakeLP(_amount, msg.sender);
    }

    function unstake(uint256 _amount) internal override returns (uint256, uint256) {
        return stakingLP.withdraw(_amount, msg.sender);
    }
...
abstract contract Opportunity is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    * @member amountLP The amount of LP token to be unstaked and removed
    ...
    */
    ...
    event Swapped(address indexed user, uint256 amountOut);
    event AddedLiquidity(address indexed user, uint256 amountA, uint256 amountB, uint256 liquidity);
    event Staked(address indexed user, uint256 amount);
    event InvestedByTokenATokenB(address indexed user, uint256 amountA, uint256 amountB);
     ...
     * @param _token The address of the token that is quoted based on it
     ...
     * @param _token The address of fromToken in swap transaction
    ...
    function investByTokenAOrTokenB(
        IERC20Upgradeable _token,
        uint256 _amount, //a hacker can send 1, however amountIn in swap tx is more than 1 to use contract's balance
        AddLiqDescriptor memory _addLiqDescriptor,
        ...
        emit InvestedByTokenAOrTokenB(msg.sender, address(_token), _amount);
        ...
        _amount -= (_token == _tokenA ? _addLiqDescriptor.amountADesired : _addLiqDescriptor.amountBDesired);
        uint256 _amountOut = _token == _tokenA ? _swap(_token, _tokenB, _amount, _swapData) : _swap(_token, _tokenA, _amount, _swapData);
        require(_amountOut >= (_token == _tokenA ? _addLiqDescriptor.amountBDesired : _addLiqDescriptor.amountADesired), "oe01");
//        if (_token == _tokenA) {
//            _amount -= _addLiqDescriptor.amountADesired;
//            _amountOut = _swap(_token, _tokenB, _amount, _swapData);
//            require(_amountOut >= _addLiqDescriptor.amountBDesired, "oe01");
//
//        } else if (_token == _tokenB) {
//            _amount -= _addLiqDescriptor.amountBDesired;
//            _amountOut = _swap(_token, _tokenA, _amount, _swapData);
//            require(_amountOut >= _addLiqDescriptor.amountADesired, "oe01");
//        }

        uint256 _balance = _token.uniBalanceOf(address(this));
        require(_balance >= (_token == _tokenA ? _addLiqDescriptor.amountADesired : _addLiqDescriptor.amountBDesired), "oeX");
     ...
     * @param _token The address of fromToken in swap transactions
     * @param _amount The amount of _token to be swapped
     * @param _addLiqDescriptor parameters needed to add liquidity
     * @param _swapDataToA The transaction to swap _token to tokenA
    ...
    function investByToken( //
        ...
        emit InvestedByToken(msg.sender, address(_token), _amount);
        ...
        require(_amountOut > _addLiqDescriptor.amountBDesired, "oe02");
        _amountOut -= _addLiqDescriptor.amountBDesired;
        _amountOut = _swap(tokenB, tokenA, _amountOut, _swapDataToA);
        require(_amountOut >= _addLiqDescriptor.amountADesired, "oe01");
    ...
    function investByLP(
        ...
        emit InvestedByLP(msg.sender, _amountLP);
    ...
    */
    function leave(
        ...
        (uint256 _amountLP, uint256 _rewards) = _unstake(_removeLiqDescriptor.amountLP);
        if (_rewards > 0) {
        ...
        uint256 _totalFee = _deductFee(_fees, pair, _amountLP);
        _removeLiqDescriptor.amountLP = _amountLP - _totalFee;
    ...
    function setAddLiquidityFee(int256 _feePercentage) external onlyOwner {
        require(_feePercentage >= 0, "oe11");
        addLiquidityFee = uint256(_feePercentage);
    }

    function setRemoveLiquidityFee(int256 _feePercentage) external onlyOwner {
        require(_feePercentage >= 0, "oe11");
        removeLiquidityFee = uint256(_feePercentage);
    }

    function setStakeFee(int256 _feePercentage) external onlyOwner {
        require(_feePercentage >= 0, "oe11");
        stakeFee = uint256(_feePercentage);
    }

    function setUnstakeFee(int256 _feePercentage) external onlyOwner {
        require(_feePercentage >= 0, "oe11");
        unstakeFee = uint256(_feePercentage);
    }
    ...
    function unstake(uint256 _amount) internal virtual returns (uint256, uint256) {
        return (0, 0);
    }
    ...
    function _swap(
        ...
        require(_afterBalance - _beforeBalance == _amountOut, "oe01");
        return _amountOut;
    }
    ...
    function _addLiquidity(
        ...
        //todo add two require to check balance of this in tokenA and tokenB to be equal or greater than desired amounts
        (uint256 amountA, uint256 amountB, uint256 liquidity) = addLiquidity(_addLiqDescriptor);
        require(liquidity > 0, "oe06");
    ...
    function _removeLiquidity(
        ...
        emit RemovedLiquidity(msg.sender, _removeLiqDescriptor.amountLP, amountA, amountB);
    }
    ...
    function _unstake(uint256 _amount) private returns (uint256, uint256) {
        ...
        (uint256 _amountLP, uint256 _rewards) = unstake(_amount);
        uint256 _afterBalance = _pair.uniBalanceOf(address(this));
        require(_afterBalance - _beforeBalance == _amountLP, "oeX");
        uint256 _afterBalanceReward = _tokenA.uniBalanceOf(address(this));
        require(_afterBalanceReward - _beforeBalanceReward == _rewards, "oeX");
    ...
    function _deductFee(
        ...
        emit FeeDeducted(msg.sender, _totalFee);
    }
    ...
    function _transferFrom(
        ...
        require(_afterBalance - _beforeBalance == _amount, "oe09");
    }
...
interface IStakingLP {
    ...
    function withdrawRewards(uint256 _lpRewards, address _receiverAccount) external;
