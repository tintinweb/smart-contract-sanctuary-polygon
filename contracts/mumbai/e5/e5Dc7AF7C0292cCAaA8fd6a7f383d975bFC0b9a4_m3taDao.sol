...
contract m3taDao is 
    ...
    IValist private _valistRegistry;   
     ...
     */
    function initialize(Im3taQuery initQueryContract , Im3taUser intitM3taUserContract )
        public
        initializer
    {
        ...
        _valistRegistry = IValist(0xD504d012D78B81fA27288628f3fC89B0e2f56e24);
        __Ownable_init();
        ...
        _projectTableId = _tableland.createTable(
            address(this),
            queryContract.getCreateValistProjectTableStatement(_projectTablePrefix,_chainID)
        );
        ...
        _subProjectTableId = _tableland.createTable(
            address(this),
            queryContract.getCreateValistSubProjectTableStatement(_subProjectTablePrefix,_chainID)
        );
        ...
        _releaseTableId = _tableland.createTable(
            address(this),
            queryContract.getCreateValistReleaseTableStatement(_releaseTablePrefix,_chainID)
        );
        ...
        _postTableId = _tableland.createTable(
            address(this),
            queryContract.getCreateValistProjectTableStatement(_postTablePrefix,_chainID)
        );
    ...
    function createProjectAccount(DataTypes.AccountStruct memory vars) public payable  {

        require(m3taUserContract.getProfIdByAddress(vars.founderAddress) > 0, "only m3taDao users can create an Account" );
        // accountID & accountHex null values

        vars.metadataTable = _projectTable;

        vars.accountID = _valistRegistry.generateID(_chainId, vars.accountName);

        vars.accountHex = Strings.toHexString(vars.accountID);

        _valistRegistry.createAccount(vars.accountName,vars.metaURI, vars.members);
    ...
    function createSubProject(DataTypes.ProjectStruct memory vars) public payable  {

        require(m3taUserContract.getProfIdByAddress(vars.sender) > 0, "only m3taDao users can create a Project" );
        // projectID & projectHex null values
        vars.metadataTable = _subProjectTable;

        vars.projectID = _valistRegistry.generateID(_chainId, vars.projectName);

        vars.projectHex = Strings.toHexString(vars.projectID);

        _valistRegistry.createProject(vars.accountID, vars.projectName, vars.metaURI, vars.members);
    ...
    function createRelease(DataTypes.ReleaseStruct memory vars) public payable  {
        ...
        require(m3taUserContract.getProfIdByAddress(vars.sender) > 0, "only m3taDao users can create a Release" );
        // releaseID & releaseHex null values
        vars.metadataTable = _releaseTable;

        vars.releaseID = _valistRegistry.generateID(_chainId, vars.releaseName);

        vars.releaseHex = Strings.toHexString(vars.releaseID);

        _valistRegistry.createRelease(vars.projectID, vars.releaseName, vars.metaURI);
    ...
    function createPost(DataTypes.PostStruct memory vars) public{
        require(_valistRegistry.isAccountMember(vars.accountID, vars.posterAddress), "only accountMembers can Post");
    ...
    function updateAccountMetadata(uint _accountID, string calldata _metaURI) public {

        _valistRegistry.setAccountMetaURI(_accountID, _metaURI);
    }

    function updateProjectMetadata(uint _projectID, string calldata _metaURI) public {

        _valistRegistry.setProjectMetaURI(_projectID, _metaURI);
    }
