...
contract ALP is StToken, IAlp {
    ...
    uint256 public _totalReserve;
    uint256 public _reserve; // available reserve
    uint256 public _feesBalance;
    ...
    constructor() {
        (factory, _token, _name) = IAlpDeployer(msg.sender).parameters();
    }
    ...
    function withdraw(uint256 amount) external {
        (, uint256 redeem) = _burnShares(msg.sender, amount);

        require(
            _getTotalPooled() >= redeem,
            "ALP: Insufficient reserve for token"
        );

        _reserve -= amount;
        _totalReserve -= amount;
        _feesBalance -= (redeem - amount);

        TransferHelper.safeTransfer(_token, msg.sender, redeem);

        emit Withdraw(msg.sender, redeem);
        // emit Sync(_reserve);
    }
    ...
    function _getTotalPooled() internal view override returns (uint256) {
        return _totalReserve + _feesBalance;
    }
...
contract Deforex is IDeforex, Ownable {
    ...
    function setQuoter(IQuoterV2 quoter) external {
        _quoter = quoter;
    }
    ...
    function liquidation(uint256 id, bytes calldata path) external {
        ...
        // rate = 1.001: 10000->10010
        uint256 marketRate = (position.amount * position.leverage * 1e18) /
            position.amountOut; // 0.999000999 * 1e18

        // marginCallRate = marketRate * (leverage - 1 + liquidationDeltaPercent) / leverage
        uint256 marginCallRate = (marketRate *
            (position.leverage - 1) +
            (marketRate * liquidationDeltaPercent) /
            1e18) / position.leverage;

        uint256 rate = (1e18 * amountOut) / position.amountOut; // 0.9910802775

        console.log(rate, marginCallRate);
    ...
    bytes4 public constant QUOTE_EXACT_INPUT_SINGLE =
        bytes4(
            keccak256(
                bytes(
        ...
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < positionsArr.length; i++) {
            ...
            leverageAmoutOut = position.amount * (position.leverage - 1);
