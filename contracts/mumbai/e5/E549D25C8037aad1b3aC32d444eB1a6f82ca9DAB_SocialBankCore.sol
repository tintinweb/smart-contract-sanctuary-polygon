...
contract DAOVault is IDAOVault, AccessControlEnumerable {
    ...
    function autoPayout() external onlyRole(DAO_CORE) override(IDAOVault) {
        ...
        require(currentBalance > _amount, "Current pool balance is less than principal"); //-may return this

        uint256 yield = currentBalance - totalStake;
        //uint256 finalAmount = yield - _amount;
        uint256 finalAmount = yield; //* 10**6; //non prefaced by amount
    ...
    function harvestYieldTest(address _account, bool _true) external {
        ...
        //With Current Stake It Works
        uint256 currentStake = customers[_account];

        if(_true == false){

            //for some reason it fails with total stake
    ...
    //PROXY FUNCTIONS
    function setBenificiary(address _receiver, address _proposer, uint256 _amount, bool _altToken, uint _tokenId) external onlyRole(DAO_CORE) override(IDAOVault) {
    ...
    function claimable() public view returns (uint256) {
        ...
        uint256 yield = currentBalance - totalStake;
        uint256 finalAmount = yield; //* 10**6;
        return finalAmount;
    }
