...
// flattened :  Wednesday, 07-Jun-23 06:51:33 UTC
interface IERC20 {
    /**
...
contract MeteorNFTMarket is Context, Ownable {
    ...
    uint256 constant public PriceDenominator = 100;
    ...
    function buy(uint8 nftType, bool payMTO) external {
        Sale storage sale = sales[nftType];
        require(sale.total > 0, "MeteorNFTMarket: sold out");
        address sender = _msgSender();
        if (sale.price > 0) {
            if (payMTO) {
                uint256 price = sale.price / (MTOPriceByUSDT / PriceDenominator);
                IERC20(MTO).transferFrom(sender, treasury, price);
            }else{
