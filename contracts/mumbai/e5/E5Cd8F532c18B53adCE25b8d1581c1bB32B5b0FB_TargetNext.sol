...
contract TargetNext is IXReceiver, Ownable {
    ...
    function deposit(
        ...
        emit Deposit(address(underlyingToken), user, depositAmount, chainID);
    }
    ...
    function withdraw(
        ...
        emit Withdraw(address(underlyingToken), user, withdrawAmount, chainID);
    }
    ...
    function borrow(
        ...
        emit Borrow(address(underlyingToken), user, borrowAmount, chainID);
    }
    ...
    function repay(
        ...
        emit Repay(address(underlyingToken), user, repayAmount, chainID);
    }
    ...
    function xReceive(
        ...
        (
            ...
            uint256 chainID
                ...
                _callData,
                (address, address, uint256, address, uint8, uint256)
            );

        // flag=0 is for deposit
        if (flag == 0) {
            deposit(underlyingToken, token, amount, user, chainID);
            ...
            withdraw(underlyingToken, token, amount, user, chainID);
            ...
            borrow(underlyingToken, token, amount, user, chainID);
            ...
            repay(underlyingToken, token, amount, user, chainID);
