...
import "./token/contracts/interfaces/IApeironPlanet.sol";
    ...
    IERC721ReceiverUpgradeable
{
    ...
    IStarOrbitalTrackLogic internal starOrbitalTrackLogicContract;
    ...
    uint256 internal percentMaxValue; // 10000
    ...
    // open rental upgrade version
    // user address => admin? mapping
    mapping(address => bool) internal admins;
    address internal starReceiptAddress;

    // support treasury version
    address internal treasuryAddress;
    uint256 internal treasuryFeePercent; // % * 100, e.g. 5% = 500
    ...
    // update tokenReceipt contract addresss
    // update platform fee
    // update detachCooldown
    ...
    event PlanetDetachedOrbitalTrack(
        ...
        uint256 indexed _planetId,
        uint256 indexed _slotIndex,
        uint256 _originalRentalStartTime,
        uint256 _planetCooldown,
        uint256 _obCooldown,
        address _starOwnerAddress,
    ...
    function _checkNFTOwner(
        ...
        _requireArgument(
            _nftContract.ownerOf(_tokenId) == _walletAddress,
            "Not owner"
        );
    }
    ...
    modifier isNFTOwner(
        ...
        _checkNFTOwner(_nftContract, _tokenId, _walletAddress);
        _;
    }
    ...
    function updateContractSetting(
        ...
        // );
        _checkAddressValid(_starAddress);
        _checkAddressValid(_starDataAddress);
        _checkAddressValid(_starOrbitalTrackDataAddress);
        _checkAddressValid(_starOrbitalTrackLogicAddress);
        _checkAddressValid(_planetAddress);
        _checkAddressValid(_blacklistAddress);
    ...
    function updateOrbitalSetting(
        ...
        _requireArgument(
            _platformAddress != address(0) &&
                ...
                _treasuryFeePercent <= _percentMaxValue &&
                _earlyDetachFeePercent <= _percentMaxValue,
            "invalid argument"
        );
        _checkAddressValid(_tokenReceipt);
        _checkAddressValid(_starReceiptAddress);

        // Setup TokenReceipt contracts
        tokenReceiptHandler = TokenReceiptHandler(_tokenReceipt);

        // Set up Platform Fee Percent
     ...
     */
    function setApprovedTokenAddresses(
        address _token,
        bool _isApproved
    ) external onlyOwner {
        // check token address is erc20 or not
        // _requireArgument(_token.isContract(), "addresses must be contract");
        _checkAddressValid(_token);
     ...
     * @param _enabled - Enable/Disable Admin Access
     */
     ...
     */
    function starOwnerAttachOwnPlanet(
        address _attacherAddress,
        uint256 _starId,
        uint256 _planetId
    )
        external
        isNFTOwner(starContract, _starId, _attacherAddress)
        isNFTOwner(planetContract, _planetId, _attacherAddress)
        onlyAdmin
    {
        ...
        _attachingPlanet(
            ...
            _attacherAddress,
            _planetId,
            starOrbitalTrackLogicContract.getStarOwnerAttachSlotIndex(
                _starId,
                _planetId
            ),
            0
    ...
    function updateRentalSetting(
        ...
        _checkNFTOwner(starContract, _starId, msg.sender);

        // require token address is approved
        _requireArgument(
            approvedTokenAddresses[_tokenAddress],
            "invalid token"
        );
        ...
        if (count > 0) {
            _requireArgument(_duration >= minimumDuration, "invalid argument");
        }
     ...
     *
     * @param _attacherAddress attacher address
     * @param _starId star ID
    ...
    {
        // get slot index
        uint256 _slotIndex = starOrbitalTrackLogicContract
            .getRentalSlotIndexWithInviteGroupID(
                _starId,
                _planetId,
                _inviteGroupId
            );
        ...
        StarOrbitalTrackData
            memory orbitalTrackData = _getStarOrbitalTrackDataWithIndex(
                _starId,
                _slotIndex
            );
        ...
        uint256 treasuryFee = _getTreasuryFee(orbitalTrackData.rentalFee);
        // send token to contract
        _tokenTransferFromTo(
            orbitalTrackData.tokenAddress,
            _attacherAddress,
            address(this),
            orbitalTrackData.rentalFee - platformFee - treasuryFee
        );
    ...
    function renewRentingOrbitalTrack(
        ...
        address attacherAddress = _attacherAddress;

        // get orbital track data
        uint256 _slotIndex = _getPlanetAttachedIndex(_starId, _planetId);
        ...
        starOrbitalTrackLogicContract.checkCanRenewRentingOrbitalTrack(
            attacherAddress,
            _starId,
            _planetId,
            orbitalTrackData
        ...
        // transfer old rental fee to star owner
        address starOwnerAddress = _getTargetStarOwnerAddress(_starId);
        _tokenTransfer(
            orbitalTrackData.tokenAddress,
            starOwnerAddress,
        ...
        _updateStarOrbitalTrackData(_starId, _slotIndex, orbitalTrackData);
        ...
        emit PlanetRenewOrbitalTrack(
            ...
            _planetId,
            _slotIndex,
            orbitalTrackData.originalRentalStartTime,
            orbitalTrackData.rentalEndTime,
            orbitalTrackData.tokenAddress,
            orbitalTrackData.rentalFee,
            starOwnerAddress,
    ...
    function starOwnerDetachPlanet(
        address _detacherAddress,
        uint256 _starId,
        uint256 _planetId
    ) external isNFTOwner(starContract, _starId, _detacherAddress) onlyAdmin {
        uint256 _slotIndex = _getPlanetAttachedIndex(_starId, _planetId);
        ...
        if (orbitalTrackData.planetOwnerAddress != _detacherAddress) {
            (
        ...
        _detachingPlanet(
            ...
            // true,
            // orbitalTrackData.planetOwnerAddress == _detacherAddress
            _detacherAddress
        );
    }
    ...
    function planetOwnerDetachPlanet(
        ...
        _tokenTransfer(
            orbitalTrackData.tokenAddress,
            _getTargetStarOwnerAddress(_starId),
            starOwnerReceive
        );
        _tokenTransfer(
            orbitalTrackData.tokenAddress,
    ...
    function _attachingPlanet(
        ...
        // check is planet and star is blacklisted
        starOrbitalTrackLogicContract.checkIsBlacklistedForAttachingPlanet(
            blacklistContract,
            _starId,
            starContract,
            _planetOwner,
        ...
        // check planet is bornt
        // primeval planet id <= 4585 dont have born time
        (IApeironPlanet.PlanetData memory planetData, ) = planetContract
            .getPlanetData(_planetId);
        _requireArgument(
            planetData.bornTime != 0 || _planetId <= 4585,
            "Planet not yet born"
        );
        ...
        orbitalTrackData.originalRentalStartTime = block.timestamp;
        orbitalTrackData.rentalStartTime = block.timestamp;
        orbitalTrackData.attachedPlanetId = _planetId;
        ...
        orbitalTrackData.rentalFeeAfterPlatform =
            ...
            _getTreasuryFee(orbitalTrackData.rentalFee);
        ...
        tokenReceiptHandler.createReceipts(
            ...
            _asDefaultValueArray(_planetId),
            _asDefaultValueArray(1)
        );
    ...
    function _detachingPlanet(
        ...
        // bool _isStarOwnerDetach,
        // bool _detacherIsPlanetOwner
        address _detacherAddress
    ) internal {
        address starOwnerAddress = _getTargetStarOwnerAddress(_starId);

        IApeironStarOrbitalTrackData _starOrbitalTrackDataContract = starOrbitalTrackDataContract;
        uint256 originalRentalStartTime = _getStarOrbitalTrackDataWithIndex(
            _starId,
            _slotIndex
        ).originalRentalStartTime;
        ...
        uint256 _planetCooldown;
        uint256 _obCooldown;
        bool _isStarOwnerDetachOwnPlanet = (starOwnerAddress == _planetOwner);
        // starOwnerAddress == _detacherAddress => isStarOwnerDetach

        // if planet owner detach and _haveDetachCooldown, or _isStarOwnerDetachOwnPlanet
        if (
            (_haveDetachCooldown && !(starOwnerAddress == _detacherAddress)) ||
            _isStarOwnerDetachOwnPlanet
        ) {
            _planetCooldown = block.timestamp + detachCooldown;
            _starOrbitalTrackDataContract.setPlanetOrbitalTrackCooldownMap(
                _planetId,
                _planetCooldown
            );
        }
        ...
        _starOrbitalTrackDataContract.updateDetachOrbitalTrackData(
            _starId,
            _slotIndex,
            _obCooldown,
            _isStarOwnerDetachOwnPlanet
        ...
        // emit event
        // planetOwnerAddress & isStarOwnerDetach are used for solving stack too deep
        address planetOwnerAddress = _planetOwner;
        bool isStarOwnerDetach = (starOwnerAddress == _detacherAddress);
    ...
    /**
     * @notice get Fee * treasuryFeePercent
     *
     * @param _fee fee
     */
    function _getTreasuryFee(uint256 _fee) internal view returns (uint256) {
        return (_fee * treasuryFeePercent) / percentMaxValue;
    }

    /**
     * @notice require funciton
     *
    ...
    /**
     * @notice get which star owner should receive rental token,
     * if starReceipt exist, return starReceipt owner, else return star owner
     *
     * @param _starId star ID
     */
    function _getTargetStarOwnerAddress(
        uint256 _starId
    ) internal view returns (address) {
        try IERC721(starReceiptAddress).ownerOf(_starId) returns (
            address owner
        ) {
            // return starReceipt owner
            return owner;
        } catch {
            // if no starReceipt owner, return star owner
            return starContract.ownerOf(_starId);
        }
    }

    /**
     * @notice get StarOrbitalTrackData With Index
     *
     * @param _starId star ID
     * @param _slotIndex slot index
     */
    function _getStarOrbitalTrackDataWithIndex(
        uint256 _starId,
        uint256 _slotIndex
    ) internal view returns (StarOrbitalTrackData memory) {
        return
            starOrbitalTrackDataContract.getStarOrbitalTrackDataWithIndex(
                _starId,
                _slotIndex
            );
    }
    ...
    /**
     * @notice update StarOrbitalTrackData
     *
     * @param _starId star ID
     * @param _slotIndex slot index
     * @param _orbitalTrackData StarOrbitalTrackData
     */
    function _updateStarOrbitalTrackData(
        uint256 _starId,
        uint256 _slotIndex,
    ...
    /**
     * @notice get planet attached index
     *
     * @param _starId star ID
     * @param _planetId planet ID
     */
    function _getPlanetAttachedIndex(
        uint256 _starId,
        uint256 _planetId
    ) internal view returns (uint256) {
        return
            starOrbitalTrackLogicContract.getPlanetAttachedIndex(
                _starId,
                _planetId
            );
    }
    ...
    /**
     * @notice check address is contract
     *
     * @param _address address
     */
    function _checkAddressValid(address _address) internal {
        _requireArgument(_address.isContract(), "address must be contract");
    }
}
...
interface IStarOrbitalTrackLogic {
    ...
    function checkCanRenewRentingOrbitalTrack(
        address _renterAddress,
        uint256 _starId,
        uint256 _planetId,
        StarMeta.StarOrbitalTrackData memory _orbitalTrackData
    ...
    function getStarOwnerAttachSlotIndex(
        uint256 _starId,
        uint256 _planetId
    ) external view returns (uint256);

    function getRentalSlotIndexWithInviteGroupID(
        uint256 _starId,
        uint256 _planetId,
        uint256 _inviteGroupId
    ) external view returns (uint256);

    function getPlanetAttachedIndex(
        uint256 _starId,
        uint256 _planetId
    ) external view returns (uint256);
    ...
    function checkIsBlacklistedForAttachingPlanet(
        IBlacklist _blacklistContract,
        uint256 _starId,
        IApeironStar _starContract,
        address _planetOwner,
        uint256 _planetId,
        IApeironPlanet _planetContract
    ) external;
}
