...
contract JarvixNFTMintDirectSale is BaseJarvixTokenDirectSale {
    ...
    constructor(address proxyHubAddress_, address priceHandlerAddress_, address nftAddress_, bytes32 TOKEN_,
                uint32 creatorFeesRate_, uint8 creatorFeesRateDecimals_, address securedProcessAddress_)
    ...
    function getTokenInterfaceId(bytes32 currency) internal override view returns (bytes4) {
        if(currency == getTOKEN()) {
            return type(IERC721Auto).interfaceId;
        }
        return super.getTokenInterfaceId(currency);
    }
    /**
     * @dev Getter of the contract handling TOKENs in sale treated as a CryptoTattooerzERC721Token
     */
...
abstract contract BaseAllowance4Mint is BaseAllowanceHandler {
     ...
     */
    constructor(bytes32 mintBucketName_, bytes32 freeMintBucketName_) {
...
contract Allowance4MintProxy is BaseAllowance4Mint, AllowanceProxy {
     ...
     * @param freeMintBucketName_ Name of the free mint allowance bucket
     */
    constructor(address proxyHubAddress_, address allowanceProxyAddress_, bytes32 mintBucketName_, bytes32 freeMintBucketName_)
    ...
    function _addOrUseAllowance(address address_, bytes32 bucketName, uint256 amount, bool add) internal override(BaseAllowance4Mint, AllowanceProxy)
    returns (uint256 allowance, Bucket memory bucket) {
...
contract Allowance4MintHandler is BaseAllowance4Mint, AllowanceHandler {
    ...
    function _addOrUseAllowance(address address_, bytes32 bucketName, uint256 amount, bool add) internal override(BaseAllowance4Mint, AllowanceHandler)
    returns (uint256 allowance, Bucket memory bucket) {
        return BaseAllowance4Mint._addOrUseAllowance(address_, bucketName, amount, add);
    }
    function _addOrUseAllowanceSuper(address address_, bytes32 bucketName, uint256 amount, bool add) internal override
...
abstract contract BaseAllowanceHandler is IAllowance, PausableImpl {
    ...
    returns (uint256[] memory allowances, Bucket[] memory buckets) {
        ...
        for(uint256 i = 0 ; i < addresses.length ; i++) {
            (allowances_[i], buckets_[i]) = _addOrUseAllowance(addresses[i], bucketNames[i], amounts[i], true);
        }
