...
contract TokenDetails {
    function getName(address _tokenAddress)
        public
        view
        returns (string memory)
    {
        if (
            IERC721MetadataUpgradeable(_tokenAddress).supportsInterface(
                0x80ac58cd
            )
        ) return IERC721MetadataUpgradeable(_tokenAddress).name();
        else if (_tokenAddress != address(0))
            return IERC20(_tokenAddress).name();
        else return "Matic";
    }

    function getSymbol(address _tokenAddress)
        public
        view
        returns (string memory)
    {
        ...
        ) return IERC721MetadataUpgradeable(_tokenAddress).symbol();
        else if (_tokenAddress != address(0))
            return IERC20(_tokenAddress).symbol();
        else return "Matic";
    }

    function getDecimal(address _tokenAddress) public view returns (uint256) {
        if (
            IERC721MetadataUpgradeable(_tokenAddress).supportsInterface(
                0x80ac58cd
            )
