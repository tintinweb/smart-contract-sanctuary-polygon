...
contract AppianContract is Initializable, ERC721Upgradeable, ERC721BurnableUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    */
    ...
    function initialize(string memory _baseUri) initializer public { // _metadata
        metadataUri = _baseUri; // eg: https://ipfs.io/ipfs/
        totalLimit = 0;
        contracturi = "https://ipfs.io/ipfs/QmUekmQdD9stUSswQ3Y8H14YXkQUtvBU5BKXHGVG7FJcB3"; // eg: https://ipfs.io/ipfs/QmUekmQdD9stUSswQ3Y8H14YXkQUtvBU5BKXHGVG7FJcB3
        __ERC721_init("Land Registration Documents", "APPIAN_LANDS");
    ...
    function whitelistApprovers(address _admin) external onlyAdmin{
        if(_admin == address(0)){ revert zeroAddressNotSupported();}
        if(approverAddress[_admin] == true){revert adminAlreadyExist();}
        approverAddress[_admin] = true;
    ...
    */
    ...
    /**
        ...
        * @param _tokenUri : tokenURI for the token id will be appended to the metadatURI(aka baseURI).
    */
    function mintLand(address _to, uint256 _tokenId, string memory _tokenUri) public {
    ...
    function mintLand1(address _to, uint256 _tokenId, string memory _tokenUri) public {
        holdUri[_tokenId] = bytes(metadataUri).length != 0 ? string(abi.encodePacked(_tokenUri)) : '';
        _mint(_to, _tokenId);
        totalLimit = totalLimit + 1;
        for(uint i = 0; i < adminAddresses.length; i++){
            approve(adminAddresses[i], _tokenId);
        }
        emit OwnershipTransferOfLand(msg.sender, _to, _tokenId);
    }
    ...
    struct approverData{
        address _ownerOfLand;
        address _sellingTo;
        uint _tokenId;
        bool status;
    }

    function approveAndTransferLand(approverData memory _data) external{
    ...
    function transferLand(address _sellingTo, uint _tokenId) external{
        if(msg.sender != _owners[_tokenId]){ revert NotOwnerOfID();}
        if(approvecount[_tokenId] == 2){
            transferFrom(msg.sender, _sellingTo, _tokenId);
            emit OwnershipTransferOfLand(msg.sender, _sellingTo, _tokenId);
        }
    }
    ...
    */
    function setTokenURI(uint256 _id, string memory _tokenUri) external {
        ...
        if(msg.sender != _owners[_id]){ revert NotOwnerOfID();}
