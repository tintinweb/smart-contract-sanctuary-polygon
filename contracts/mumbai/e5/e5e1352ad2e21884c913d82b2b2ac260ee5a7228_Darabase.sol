...
contract Darabase is NFTContract{
    ...
    function buyRentalPrimarySale(
        ...
        doPaymentTransfer(msg.sender, totalPayment, buyPayment, platformPayment, sellOrder.maker, sellOrder.payment, true, currency);
    ...
    function renewRental(
        ...
        (uint256 totalPayment, uint256 buyPayment, uint256 platformPayment) = calculateTotalPaymentWithPlatformFee(renewOrderValue.payment.amount, getRentalDetails(renewOrderValue.pdrId, rentalId).amount, renewOrderValue.numberOfTokens, platformRenewPDRFee);
        doPlatformPayment(msg.sender, renewOrderValue.payment, platformPayment, currency);
        doPaymentTransfer(msg.sender, totalPayment, buyPayment, platformPayment, renewOrderValue.maker, renewOrderValue.payment, true, currency);
    ...
    function calculateTotalPaymentForRevenueDist(
        ...
        uint256 platformFee
    ) internal pure returns(uint256 totalPayment, uint256 buyPayment, uint256 platformPayment){
        amount = amount * numberOfTokens;
        totalPayment = amount;
        buyPayment = amount;
        platformPayment = ((amount * platformFee) / 10000);
    }
    ...
    function distributeRevenue(
        ...
        (uint256 totalPayment, , uint256 platformPayment) = calculateTotalPaymentForRevenueDist(amountPerNFT, numberOfTokens, platformRevenueFee);
        doPlatformPayment(msg.sender, payment, platformPayment, currency);

        (bool sendRemainingToRwoOwner, uint completedPayment) = distribute(pdrId, ownerRwo, accounts, numberOfTokens,  totalPayment - platformPayment, payment, currency);

        if(msg.sender != ownerRwo && sendRemainingToRwoOwner){
            doRevenueTransfer(msg.sender, totalPayment - platformPayment - completedPayment , ownerRwo, payment, currency);
        }
    }
    ...
    function distribute(
        ...
        Signature.Payment memory payment,
        string memory currency
     ) internal returns(bool sendRemainingToRwoOwner, uint completedPayment){
        address renter;
        uint256[] memory rentals = getRentalFromPDR(pdrId);
        uint256 fee;

        for(uint256 i=0; i<rentals.length; i++){
            renter = getRentalOwner(pdrId, rentals[i]);
            if(renter == ownerRwo){
                ...
                if(searchIfRentalOwnerMatches(accounts, renter)){
                        ...
                        fee = (netPayment * balanceOfRental(pdrId, rentals[i]) ) / numberOfTokens;
                        completedPayment += fee;
    ...
    /// @param numberOfRentals In uint256, number of rentals 
    /// @param numberOfTokens  In uint256, number of tokens
    function calculateTotalPaymentWithPlatformFee(
        uint256 amount,
        uint256 numberOfRentals,
        uint256 numberOfTokens,
        uint256 platformFee
    ...
    function doPlatformPayment(
        ...
        } else if( payment.paymentClass == Signature.FIAT_PAYMENT_CLASS){
    ...
    function doPaymentTransfer(
        ...
        if (payment.paymentClass == Signature.MATIC_PAYMENT_CLASS) {
            ...
            if(isRoyalty && (royaltyFee > 0)){
                // Transfer royalties
                currentPayment = (buyPayment*royaltyFee) / 10000;
    ...
    function doRevenueTransfer(
        ...
        if (payment.paymentClass == Signature.MATIC_PAYMENT_CLASS) {
            // msg value is not updated value TODO check
            if(msg.value < netPayment) revert("Insufficient payment");

            // Transfer remaining payment to receiver
            Signature.transferMatic(receiver, netPayment);
        } else if( payment.paymentClass == Signature.ERC20_PAYMENT_CLASS){
            (address token) = abi.decode(payment.data, (address));

            // Transfer remaining payment to seller
            IERC20(token).transferFrom(from, receiver, netPayment);
        } else if( payment.paymentClass == Signature.FIAT_PAYMENT_CLASS){
...
contract NFTContract is ERC1155, Ownable, Pausable{
    ...
    function mintRWO(
        ...
        // rwo mapping
        rwo[nftId]= Rwo(uint256(nftId), address(to));
    ...
    function burnPDR(
        ...
        if(pdrRemainingAmount[pdrId] != 0)  pdrRemainingAmount[pdrId] = 0;
    ...
    // TODO check onlyowner and admin? to owner and admin?
    ...
    function setBaseURI(string memory _baseURI) external whenNotPaused {
        require(msg.sender == admin || msg.sender == admin, "only owner or admin");
        baseURI = _baseURI;
    }
    ...
    function getRentalTokenURI(uint256 pdrId, uint256 rentalId) external view returns(string memory){
        return uri(pdrId);
    }

    function updatePdrRemaining(uint pdrId, uint remaining) external onlyAdminOrOwner {
        pdrRemainingAmount[pdrId] = remaining;
    }
