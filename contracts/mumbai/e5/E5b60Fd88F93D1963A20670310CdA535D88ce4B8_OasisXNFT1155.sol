 ...
 **/
contract OasisXNFT1155 is ERC1155, ERC1155Supply, Pausable, Initializable {
     ...
     * @param tokenSupplies max supply per token id
    ...
    /**
     * @notice Check to see if token exists
     * @param tokenId token id to verify
     */
    function exists(uint256 tokenId) public view override returns (bool) {
        return tokenMaxSupplies[tokenId] > 0;
    }
    ...
    function mint(
        ...
        require(
            received == mintPrice[_phase].mul(amount),
    ...
    function mintBatch(
        ...
        for (uint256 i = 0; i < amounts.length; i++) {
            isLimitExceeded(ids[i], amounts[i]);
            amount += amounts[i];
        }
     ...
     **/
    function isLimitExceeded(uint256 id, uint256 amount)
        public
        view
        returns (bool)
    {
        require(exists(id), "OasisXNFT1155: Token not available");
    ...
    function _forwardFunds(uint256 received) internal {
        ...
        require(success, "OasisXNFT1155: Failed to forward funds");
    }
