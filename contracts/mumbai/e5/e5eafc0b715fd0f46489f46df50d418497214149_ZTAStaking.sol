...
contract ZTAStaking is ReentrancyGuard, Ownable, Initializable {
    ...
    mapping(uint8 => uint256) public endOfEvent;
    ...
    function setEventStaking(uint256 _period, uint256[] memory _intervals, uint256 _openedAt
    ) public onlyOwner{
        require(_period > 60, 'Period must greater than 60 seconds, e.g. 86400 for 1 day');

        period      = _period;
        openedAt    = _openedAt;
        endOfEvent[uint8(Phase.PREPARATION)] = openedAt - 1;

        for(uint8 i = 0; i < phases.length; i++){
            interval[i] = _intervals[i];
            if (i > 0) {
                endOfEvent[i] = endOfEvent[i - 1] + (period * interval[i]);
            }
        }
    }
