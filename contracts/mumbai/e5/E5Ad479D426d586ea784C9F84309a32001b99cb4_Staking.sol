...
contract Staking is APR, ReentrancyGuard {
    ...
    function getStakesInfo(address _addr, uint256 _stakeID)
        external
        view
        returns (Data memory)
    {
        return stakeStatus[_addr].stakeDetails[_stakeID];
    }
    ...
    function getStakesIDOf(address _addr)
        public
        view
        returns (uint256[] memory)
    {
        uint256 activeStakeCount = 0;
        uint256 len = stakeID[_addr].length;
        for (uint256 i = 0; i < len; i++) {
            if (stakeID[_addr][i] != 0) {
                activeStakeCount++;
            }
        }
