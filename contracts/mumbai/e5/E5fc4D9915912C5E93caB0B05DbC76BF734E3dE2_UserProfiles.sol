...
contract UserProfiles {
    ...
    // Set Max URI Length
    uint256 public maxURILength = 255;

    // Set Max Bio Length
    uint256 public maxBioLength = 255;

    // Set Following Limit
    uint256 public maxFollowing = 5000;
    ...
    */
    ...
    event logProfileUpdated1(address indexed requester, string uri, string avatar, string location, string bio);
    ...
    */
    ...
    function addApproved(address newAddress) public onlyAdmins{
        approved[newAddress] = true;
    }

    function removeApproved(address oldAddress) public onlyAdmins{
        approved[oldAddress] = false;
    }
    ...
    */
    function updateProfileLimits(uint256 _bioLen, uint256 _uriLen) public onlyAdmins {
    ...
    */
    function logPost(address posterAddress, uint256 tipPerTag, address[] calldata tipReceivers, uint256 isCommentOf) public onlyAdmins {
    ...
    */
    function updateProfile(string calldata location, string calldata avatar, string calldata _uri, string calldata _bio, uint256 groupID) public {
        ...
        emit logProfileUpdated1(profileAddress, _uri, avatar, location, _bio);
    }
    ...
    function setNFTAsAvatar(address _nftContract, uint256 tokenId) public {
        ...
        usrProfileMap[msg.sender].userAvatar.metadata = NFT.tokenURI(tokenId);

        // Save the token contract
        usrProfileMap[msg.sender].userAvatar.contractAddress = _nftContract;

        // Save the token ID
        usrProfileMap[msg.sender].userAvatar.tokenID = tokenId;

        // Set the network ID to polygon
        usrProfileMap[msg.sender].userAvatar.networkID = 137;
    }

    // Set an NFT as your profile photo (only for users / not groups)
    function setAvatar(string calldata imageURI, address _nftContract, uint256 tokenId, string calldata metadata, uint8 networkID) public onlyApproved {
        // Save the token metadata
        usrProfileMap[msg.sender].userAvatar.metadata = metadata;
        ...
        // Save image URI
        usrProfileMap[msg.sender].userAvatar.avatar = imageURI;

        // Save contract network
        usrProfileMap[msg.sender].userAvatar.networkID = networkID;
    }

    // Set an NFT as your profile photo (only for users / not groups)
    function setupNewGroup(address groupAddress, string memory groupName, uint256 groupID, address _nftContract, string memory _baseURI) public onlyAdmins {
        // Setup link to the NFT contract
        NFT = INFT(_nftContract);

        // Save the token metadata
        usrProfileMap[groupAddress].userAvatar.metadata = _baseURI;
        ...
        usrProfileMap[groupAddress].userAvatar.networkID = 137;
    ...
    // Setup a new user profile
    function updateHandle(address userAddress, string calldata handle) public onlyAdmins {
    ...
    * 18 = Group ID (0 = user)
    */
    function getUserDetails(address usrAddress) public view returns(string[] memory){
