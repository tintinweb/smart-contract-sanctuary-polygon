...
import "./utils/AdvertisementMetadata.sol";
import "./interface/IAdvertiser.sol";
import "@openzeppelin/contracts-upgradeable/utils/AddressUpgradeable.sol";

contract Advertisement is AdvertisementMetadata {
    using AddressUpgradeable for address;
    address private advertiserContract;
    ...
    mapping(uint256 => AdInfo) public getInfo;

    error NotAValidAdvertisement(uint256 tokenID, address user);
    ...
    event AdCreated(
        ...
        string adLayout,
        string adTargetPlatform,
        string adResourcePath
    );
    ...
    /**
     * @dev Called once after the initial deployment to set the Advertiser Register
     */
    function updateAdvertiserContract(address advertiserContract_)
        external
        onlyOwner
    {
        require(
            advertiserContract_.isContract(),
            "Advertisement: Address is not a contract"
        );
        advertiserContract = advertiserContract_;
    }

    /**
     * @notice Returns the address of the Foundation treasury.
     */
    function getAdvertiserContract() public view returns (address) {
        return advertiserContract;
    }

    // adDetails_[0] = Ad Title
    // adDetails_[1] = Ad Target Link
    // adDetails_[2] = Ad category
    // adDetails_[3] = Ad Description
    // adDetails_[4] = Ad layout
    // adDetails_[5] = Ad Target Platform
    // adDetails_[6] = Ad Resource IPFS path
    function create(string memory tokenCID_, string[7] memory details_)
        external
    {
        require(
            IAdvertiser(getAdvertiserContract()).isValidAdvertiser(
                msg.sender
            ),
            "New User / User Not Registered"
        );

        uint256 mintedTokenID = _mintInternal(tokenCID_);

        AdInfo storage adInfo = getInfo[mintedTokenID];
        // adInfo.title = details_[0];
        // adInfo.targetLink = details_[1];
        // adInfo.category = details_[2];
        // adInfo.content = details_[3];
        // adInfo.layout = details_[4];
        // adInfo.targetPlatform = details_[5];
        adInfo.tokenId = mintedTokenID;
        adInfo.advertiser = msg.sender;
    ...
    function isValidAdvertisement(uint256 tokenID_, address advertiser_)
        public
        view
        returns (bool)
    {
        if (_exists(tokenID_) && getInfo[tokenID_].advertiser == advertiser_)
            return true;
        else revert NotAValidAdvertisement(tokenID_, advertiser_);
    }
...
import "../utils/AdvertisementERC721.sol";
import "@openzeppelin/contracts/utils/Strings.sol";

/**
 * @notice A mixin to extend the OpenZeppelin metadata implementation.
 */
abstract contract AdvertisementMetadata is AdvertisementERC721 {
...
interface IAdvertiser {
    function isValidAdvertiser(address user) external view returns (bool);
}
 ...
 * {ERC721Enumerable}.
 */
contract AdvertisementERC721 is
    Initializable,
    Ownable,
    ERC165Upgradeable,
    IERC721Upgradeable,
    IERC721MetadataUpgradeable
{
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        address owner = AdvertisementERC721.ownerOf(tokenId);
    ...
    function _transfer(
        ...
        require(
            AdvertisementERC721.ownerOf(tokenId) == from,
    ...
    function _approve(address to, uint256 tokenId) internal virtual {
        ...
        emit Approval(AdvertisementERC721.ownerOf(tokenId), to, tokenId);
    }
