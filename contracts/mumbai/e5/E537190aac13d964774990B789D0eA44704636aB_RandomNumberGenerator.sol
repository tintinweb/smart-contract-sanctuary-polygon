    ...
    Ownable
{
    ...
    function getRandomNumSeed(uint256 _raffleId)
        public view returns(uint256)
    {
        return uint256(keccak256(abi.encodePacked(_raffleId, uint256(0))));
    }
    /**
    ...
    function setLotteryAddress(address _pancakeSwapLottery) external onlyOwner {
        require(
            _pancakeSwapLottery != address(0),
            "lottery address cannot be set as zero."
        );
