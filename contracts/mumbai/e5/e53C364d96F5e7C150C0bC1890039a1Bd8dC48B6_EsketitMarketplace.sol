...
contract EsketitMarketplace is Ownable, ReentrancyGuard {
    ...
    modifier isListed(
        ...
        for(uint256 itr = 0; itr < _tokenId.length; itr++){
            Listing memory listing = listings[_nftAddress][_tokenId[itr]][_owner];
    ...
    function buyFromEsketit(
        ...
        uint256 _floorPrice = esketitNFT(_nftAddress).floorPrice();
        
        IERC20(_payToken).safeTransferFrom(
            _msgSender(),
            esketitTreasury,
            _floorPrice.mul(_tokenId.length)
        );
    ...
    function esketitBuyBack(
        ...
        for (uint256 itr = 0; itr < _tokenId.length; itr++) {
            ...
            uint256 price = esketitNFT(_nftAddress).getCurrentPrice(_tokenId[itr]);
            _totalPrice += price;
        }
    ...
    function listItem(
        ...
        require(esketitLoanBasket[_nftAddress], "unknown nft address");
        require(payTokens[_payToken], "not verified payToken");
        ...
        require(
            ...
            "all item's are not approved"
        );

        for(uint256 itr = 0; itr < _tokenId.length; itr++){
            require(nft.ownerOf(_tokenId[itr]) == _msgSender(), "not the owner");
        ...
        validListing(_nftAddress, _tokenId, _owner)
    {
        ...
        for(uint256 itr = 0; itr < _tokenId.length; itr++){
            Listing memory listedItem = listings[_nftAddress][_tokenId[itr]][_owner];
            require(listedItem.payToken == _payToken, "not same payToken for all");
    ...
    function _buyItem(
        ...
        for(uint256 itr = 0; itr < _tokenId.length; itr++){
            ...
            esketitNFT(_nftAddress).updateCurrentPrice(_tokenId[itr], listedItem.price);
    ...
    */
    ...
    function updateEsketitTreasury(address payable _esketitTreasury) external onlyOwner {
    ...
    function updatePayToken(address _payToken, bool _status) external onlyOwner {
