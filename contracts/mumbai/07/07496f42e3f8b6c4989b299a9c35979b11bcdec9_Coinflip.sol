...
abstract contract Manager is Ownable {
    ...
    // Events
    event BetPlaced(uint indexed betId, address indexed player, uint amount, uint indexed coins, uint choice, address token);
    event BetSettled(uint indexed betId, address indexed player, uint amount, uint indexed coins, uint choice, uint outcome, uint winAmount, address token);
    ...
    function amountToWinnableAmount(uint _amount, uint coins, address token) internal view returns (uint) {
...
contract Coinflip is ReentrancyGuard, Manager {
    // aggiunge address in params e token giocato, controllo require(tx.origin === addressDelPlayer)
    function placeBet(uint betChoice, uint coins, address player, address token, uint amount) external payable nonReentrant {
        ...
        emit BetPlaced(betId, player, amount, coins, betChoice, token);
        bets.push(Bet({
            coins: uint8(coins),
    ...
    function refundBet(uint betId) external nonReentrant {
        ...
        address token = bet.token;
        uint winnableAmount = amountToWinnableAmount(amount, bet.coins, token);
        ...
        house.refundBet(player, token, bettedAmount, winnableAmount);
        emit BetRefunded(betId, player, bettedAmount, token);
    }
}