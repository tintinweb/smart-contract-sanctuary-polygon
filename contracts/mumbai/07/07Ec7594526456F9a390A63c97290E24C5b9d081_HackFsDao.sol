...
contract HackFsDao {
  ...
  IHackFsDaoContract hackFsDaoContract; 

constructor(){
        owner = msg.sender;
        nextProposal = 1;
        hackFsDaoContract = IHackFsDaoContract (0x2953399124F0cBB46d2CbACD8A89cF0599974963);
        validTokens = [33414823501146813550053172971385822950994162408590262080474068968151195844609];
    }
...
function checkProposalEligibility(address _proposalist) private view returns (
        ...
        for(uint i = 0; i < validTokens.length; i++){
            if(hackFsDaoContract.balanceOf(_proposalist, validTokens[i]) >= 1){
                return true;
            }
        }
        return false;
