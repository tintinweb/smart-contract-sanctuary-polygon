...
/**
 * @title WISCurrency
 * @dev WISCurrency is an ERC20 with blacklist & redemption addresses
 *
 * WISCurrency is a compliant stablecoin with blacklist and redemption
 ...
 */
abstract contract WISCurrency is BurnableTokenWithBounds {
    ...
    function mint(address account, uint256 amount) external onlyOwner {
        require(!isBlacklisted[account], "WISCurrency: account is blacklisted");
        require(!isRedemptionAddress(account), "WISCurrency: account is a redemption address");
    ...
    function setBlacklisted(address account, bool _isBlacklisted) external onlyOwner {
        require(uint256(account) >= REDEMPTION_ADDRESS_COUNT, "WISCurrency: blacklisting of redemption address is not allowed");
    ...
    function _transfer(
        ...
        require(!isBlacklisted[sender], "WISCurrency: sender is blacklisted");
        require(!isBlacklisted[recipient], "WISCurrency: recipient is blacklisted");
    ...
    function _approve(
        ...
        require(!isBlacklisted[owner], "WISCurrency: tokens owner is blacklisted");
        require(!isBlacklisted[spender] || amount == 0, "WISCurrency: tokens spender is blacklisted");
    ...
    function _burn(address account, uint256 amount) internal override {
        require(canBurn[account], "WISCurrency: cannot burn from this address");
        super._burn(account, amount);
    }
...
/**
 * @title DelegateERC20
 * Accept forwarding delegation calls from the old WISTOKEN (V1) contract.
 ...
 * This gives the delegate contract the power to change the state of the WISTOKEN
 * contract. The owner of this contract is the WISTOKEN TokenController
 * at 0x0000000000075efbee23fe2de1bd0b7690883cc9.
 *
 * Our audits for WISCurrency can be found here: github.com/trusttoken/audits
 */
abstract contract DelegateERC20 is WISCurrency {
    // require msg.sender is the delegate smart contract
    modifier onlyDelegateFrom() {
        revert("DelegateERC20: WISTOKEN (V1) is not supported");
        _;
    }
 ...
 */
abstract contract WisCurrencyWithLegacyAutosweep is DelegateERC20 {
    function _transfer(
        address sender,
        address recipient,
        uint256 amount
 ...
 */
contract WisdomToken is WisCurrencyWithLegacyAutosweep {
