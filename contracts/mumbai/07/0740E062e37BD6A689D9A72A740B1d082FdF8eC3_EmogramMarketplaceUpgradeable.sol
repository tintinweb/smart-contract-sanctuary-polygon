    ...
    AccessControlUpgradeable
{
    ...
    modifier isTheHighestBidder(address _bidder, uint256 _auctionId) {
        require(
            emogramsOnAuction[_auctionId].highestBidder == _bidder,
    ...
    modifier auctionNotEnded(uint256 _auctionId) {
        require(
            emogramsOnAuction[_auctionId].endDate > block.timestamp,
    ...
    modifier AuctionActive(uint256 _auctionId) {
        require(
            activeAuctions[emogramsOnAuction[_auctionId].tokenAddress][
                emogramsOnAuction[_auctionId].tokenId
            ] == true,
    ...
    modifier itemExists(uint256 id) {
        require(
            id <= emogramsOnSale.length && emogramsOnSale[id].sellId == id,
            "Could not find item"
        );
        _;
    }
    ...
    modifier itemExistsAuction(uint256 id) {
        require(
            id <= emogramsOnAuction.length &&
                emogramsOnAuction[id].auctionId == id,
            "Could not find item"
        );
        _;
    }
    ...
    function initialize(bool _isTest, address _weth) public initializer {
    ...
    function emogramsOnSaleLength() public view returns (uint256) {
        return emogramsOnSale.length;
    }

    function emogramsOnAuctionLength() public view returns (uint256) {
        return emogramsOnAuction.length;
    }

    function getSaleArray() public view returns (sellItem[] memory) {
        return emogramsOnSale;
    }

    function getAuctionArray() public view returns (auctionItem[] memory) {
    ...
    function sendRoyalty(uint256 _id) private returns (address, uint256) {
        //Calculating royalty
        (bool succes, bytes memory result) = emogramsOnSale[_id]
            .tokenAddress
            .call(
                abi.encodeWithSignature(
                    "royaltyInfo(uint256,uint256)",
                    emogramsOnSale[_id].tokenId,
                    emogramsOnSale[_id].price
                )
            );
        (address receiver, uint256 royAmount) = abi.decode(
            result,
            (address, uint256)
        );
        ...
        weth.transferFrom(msg.sender, receiver, royAmount);

        //return (emogramsOnSale[_id], toSend);
        return (receiver, toSend);
    }
    ...
    {
        //Calculating royalty
        (bool succes, bytes memory result) = emogramsOnAuction[_id]
            .tokenAddress
            .call(
                abi.encodeWithSignature(
                    "royaltyInfo(uint256,uint256)",
                    emogramsOnAuction[_id].tokenId,
                    emogramsOnAuction[_id].highestBid
                )
            );
        (address receiver, uint256 royAmount) = abi.decode(
            result,
    ...
    {
        require(
            activeEmograms[tokenAddress][tokenId] == false,
            "Item is already up for sale"
        );
        require(
            activeAuctions[tokenAddress][tokenId] == false,
            "Item already up for auction"
        );
    ...
    function cancelSell(uint256 _id)
        ...
        isForSale(_id)
        isTheOwner(
            emogramsOnSale[_id].tokenAddress,
            emogramsOnSale[_id].tokenId,
            msg.sender
        )
    {
        ...
        activeEmograms[emogramsOnSale[_id].tokenAddress][
            emogramsOnSale[_id].tokenId
        ] = false;
    ...
    {
        require(
            weth.balanceOf(msg.sender) >= emogramsOnSale[id].price,
        ...
        activeEmograms[emogramsOnSale[id].tokenAddress][
            emogramsOnSale[id].tokenId
        ] = false;

        IERC1155(emogramsOnSale[id].tokenAddress).safeTransferFrom(
            emogramsOnSale[id].seller,
            msg.sender,
            emogramsOnSale[id].tokenId,
            1,
            ""
        ...
        //Sending the payment
        weth.transferFrom(msg.sender, emogramsOnSale[id].seller, toSend);

        emit EmogramSold(
            id,
            emogramsOnSale[id].tokenId,
            msg.sender,
            emogramsOnSale[id].price,
            emogramsOnSale[id].seller
        );
    }
        ...
        returns (uint256)
    {
        require(
            ...
            "Emogram is already up for auction"
        );
        require(
            activeEmograms[_tokenAddress][_tokenId] == false,
            "Item is already up for sale"
        );
        ...
        emogramsOnAuction.push(
            auctionItem(
                emogramsOnAuction.length,
                _tokenAddress,
                _tokenId,
                (msg.sender),
                (msg.sender),
        ...
        assert(
            emogramsOnAuction[emogramsOnAuction.length - 1].auctionId ==
                emogramsOnAuction.length - 1
        );
        emit AuctionCreated(
            emogramsOnAuction[emogramsOnAuction.length - 1].auctionId,
            _tokenId,
        ...
        returns (uint256)
    {
        ...
        //Else we send the weth (highestBid) back to the highestBidder from the marketplace contract
            ...
            weth.safeTransferFrom(
                ...
                address(this),
                emogramsOnAuction[_auctionId].highestBidder,
                emogramsOnAuction[_auctionId].highestBid
            );
    ...
    function PlaceBid(
    ...
    ) public auctionNotEnded(_auctionId) nonReentrant returns (uint256) {
        require(
            activeAuctions[_tokenAddress][_tokenId] == true,
        ...
        if (
            ...
            emogramsOnAuction[_auctionId].highestBidder = (msg.sender);
            //uint256 amount = emogramsOnAuction[_auctionId].highestBid;
            emogramsOnAuction[_auctionId].highestBid = _tokenAmount;
            weth.safeTransferFrom(msg.sender, address(this), _tokenAmount);
            ...
            // so we rew. the _tokenAmount to be bigger than the current bid, and not equal or bigger
    ...
    function endAuctionWithBid(
        address _tokenAddress,
        uint256 _tokenId,
        uint256 _auctionId
    ) private {
        require(
            emogramsOnAuction[_auctionId].highestBid != 0,
            "Highest bid cannot be zero in endAuctionWithBid()"
        );

        //Send royalty
        (address receiver, uint256 toSend) = sendRoyaltyAuction(_auctionId);

        //Send payment to contract to seller
        weth.transfer(emogramsOnAuction[_auctionId].seller, toSend);
    ...
    function endAuctionWithNoBid(
        ...
        require(
            emogramsOnAuction[_auctionId].highestBid ==
                emogramsOnAuction[_auctionId].startPrice &&
                emogramsOnAuction[_auctionId].highestBidder ==
                emogramsOnAuction[_auctionId].seller
        );
        ...
        returns (bool)
    {
        IERC1155 tokenContract = IERC1155(_tokenAddress);
        require(
            tokenContract.balanceOf(msg.sender, _tokenId) != 0 ||
                emogramsOnAuction[_auctionId].highestBidder == msg.sender,
            "Not the owner or highest bidder"
        );
