    ...
    IERC4907
{
    ...
    function getAffiliateAddress(string memory _userId)
        public
        view
        returns (address)
    {
        return affiliateAddresses[_userId];
    }

    function getUserId(address _addr) public view returns (string memory) {
        return isAffiliate[_addr] ? affiliateIds[_addr] : "";
    }
        ...
        returns (string memory)
    {
        ...
        string memory name_ = (
            isSoulToken(tokenId)
                ? string(abi.encodePacked("NIFTILIATE/", [ownerOf(tokenId)]))
                : string(
                    abi.encodePacked(
                        niftyvisual.name(),
                        "#",
