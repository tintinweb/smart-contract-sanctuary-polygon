...
contract VRFBeacon is VRFBeaconOCR, VRFBeaconDKGClient {
    struct EnrollmentRequest {
        address addressToEnroll; // Address to be enrolled.
        bytes publicKeyBytes; // Public key for the public-key encryption pair.
    }

    struct EnrollmentResponse {
        uint8 playerIdx; // Index of the committee member.
        uint8 threshold; // Threshold to recover a commit.
        bytes cipher; // Encrypted answer share.
        bytes ephermeralKeyPubKeyBytes; // Public key for the ephemeral key pair used.
        bytes nonce; // Nonce of the encryption.
        bytes distributedPublicKey; // Public key of the committee.
        bytes accountPointBytes; // Bytes of point representation of account.
    }

    // Currently, the committee only serves one enrollemnt request at a time.
    EnrollmentRequest private s_mostRecentEnrollmentRequest;
    mapping(address => EnrollmentResponse[]) private s_enrollmentResponses;
    mapping(address => bytes) private recoveries;

    // Balances for the recoverable wallet.
    ...
    function requestAccountRecoveryEnrollment(bytes calldata publicKeyBytes)
        external
    {
        s_mostRecentEnrollmentRequest = EnrollmentRequest({
            addressToEnroll: msg.sender,
            publicKeyBytes: publicKeyBytes
        });
    }

    function getMostRecentEnrollmentRequest()
        external
        view
        returns (EnrollmentRequest memory)
    {
        return s_mostRecentEnrollmentRequest;
    }

    function postCipher(
        address user,
        EnrollmentResponse calldata enrollmentResponse
    ) external {
        s_enrollmentResponses[user].push(enrollmentResponse);
    }

    function getEnrollmentResponses(address user)
        external
        view
        returns (EnrollmentResponse[] memory)
    {
        return s_enrollmentResponses[user];
    }

    function setRecovery(bytes calldata answer) external {
        recoveries[msg.sender] = answer;
    }

    function getRecovery(address user) external view returns (bytes memory) {
        return recoveries[user];
    }

    // For testing purposes.
    function reset(address addr) external {
        delete s_enrollmentResponses[addr];
        delete recoveries[msg.sender];
        delete s_mostRecentEnrollmentRequest;
    }

    // Simple safe deposit logic.
