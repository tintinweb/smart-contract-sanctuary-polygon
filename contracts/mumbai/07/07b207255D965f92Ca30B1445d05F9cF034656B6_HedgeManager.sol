...
interface IHedgeManager {
    ...
    event VarSet(bytes32 indexed what, bytes value);

    event NewHedge(uint256 indexed hedgeId);

    event HedgeClosed(uint256 indexed hedgeId);

    event HedgeLiquidated(uint256 indexed hedgeId);

    event CollateralAdded(uint256 indexed hedgeId, uint256 amount);
