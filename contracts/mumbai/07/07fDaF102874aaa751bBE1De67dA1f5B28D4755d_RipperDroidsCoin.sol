...
interface IERC20 {
    ...
    event Burn(address indexed _from,address indexed _to,uint256 _value,uint256 tokenidmemo);
    ...
    function burnFrom(address sender, uint256 amount, uint256 tokenidmemo) external returns (bool);
...
/////////////////////////////////////////// ERC20 ///////////////////////////////////////
    function getCurrState() public returns(bool) {
        return paused;
    }
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        require(paused == false,"ERC20: current state is paused");
...
/////////////////////////////////////////// Internal Functions ///////////////////////////////////////
    ...
    function _burn(address account, uint256 amount, uint256 tokenidmemo) internal virtual {
        require(account != address(0), "ERC20: burn from the zero address");
        _beforeTokenTransfer(account, address(0), amount);
        _balances[account] = _balances[account].sub(amount, "ERC20: burn amount exceeds balance");
        _totalSupply = _totalSupply.sub(amount);
        emit Burn(account, address(0), amount, tokenidmemo);
    }
...
contract RipperDroidsCoin is ERC20WithOperators {
    using SafeMath for uint256;
    using Address for address;
    string public override constant name = "Ripper";
    string public override constant symbol = "RPD";
    uint8 public override constant decimals = 4;
    address public teamAddr;
    constructor (uint256 initialBalance) public {
        _mint(_msgSender(), initialBalance);
    }
    function setState(bool _newState) public onlyOwner returns(bool) {
        paused = _newState;
    }
    ...
    function regulateBurn(uint256 amount) public onlyOwner {
      uint256 tokenidmemo = 1;
      _burn(_msgSender(),amount,tokenidmemo);
    }
    function getTeamaddr() public onlyOwner returns (address) {
      return teamAddr;
    }
    function changeTeamaddr(address _newAddr) public onlyOwner returns (bool) {
      teamAddr = _newAddr;
      return true;
    }
    function burnFrom(address account, uint256 amount,uint256 tokenidmemo) public virtual override returns (bool) {
        require(paused == false,"ERC20: current state is paused");
        ...
        uint256 toBurn = amount.div(20).mul(2); // 10 %
        uint256 toTeam = amount.div(20).mul(3); // 15 %
        _transfer(address(this), teamAddr,toTeam);
        _burn(account, toBurn, tokenidmemo);
        return true;
    }
    ...
    function withDrawFromContract(uint256 amount) public onlyOwner returns (bool) {
