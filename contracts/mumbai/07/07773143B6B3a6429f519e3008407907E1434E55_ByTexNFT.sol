...
contract ByTexNFT is ERC721Upgradeable, OwnableUpgradeable{
    address public exchange;


    error AddressZero();
    error  OnlyExchangeAddressBurn();
    ...
    function mint(address merchant, uint256 tokenId) external onlyOwner {
        if (merchant == address(0)) {
            revert AddressZero();
        }
        _mint(merchant, tokenId);
        _transfer(merchant, exchange, tokenId);
    }

    function burn(uint256 tokenId) external {
        if (msg.sender != address(exchange)) {
            revert OnlyExchangeAddressBurn();
        }
        _burn(tokenId);
    }
