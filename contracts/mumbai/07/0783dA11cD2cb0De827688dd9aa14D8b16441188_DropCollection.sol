...
abstract contract BaseCollection is ERC2981, IBaseCollection, ReentrancyGuard {
    ...
    function computePreMintedFee(
        uint256 tokenId,
        uint256 preMinted
    ) external view override returns (uint256) {
        return _computePreMintFee(tokenId, preMinted);
    }

    function _computePreMintFee(
        uint256 tokenId,
        uint256 preMinted
        ...
        uint256 preMintFee = (totalPrice * item.shareFee) / _feeDenominator();
    ...
    function _mintItemOnPublish(
        ...
        if (preMinted > freebiesAmount) {
            uint256 preMintFee = _computePreMintFee(tokenId, preMinted);
...
interface IAISkinFactory {
    ...
    /**
     * @dev Updates the baseToken address and emits an {IAISkinFactory-UpdateBaseTokenAddress} event.
     *
     * Requirements:
     *  - The caller must have keccak256(MANAGER_ROLE) role
     *
     * @param baseToken The new address for the base token.
     */
...
interface IBaseCollection {
    ...
    /**
     * Calculate the fee for the creator's preminted items based on the tokenId and tokenPayment
     * @param tokenId The unique identifier of the item to be minted
     * @param preMinted The quantity of the item to be minted at the creator's address
     */
    function computePreMintedFee(
        uint256 tokenId,
        uint256 preMinted
