...
contract marketplace is ERC1155Base {

    //initializing a custom type variable for a listing (info about a nft being sold)
    struct Listing {
        uint256 tokenId;
        address seller;
        uint256 price;
    ...
    //storing listings data as key-value pairs
    mapping(uint256 => Listing) public listings;

    //initializing a type variable to keep count of the listing id 
    uint256 private listingIdCounter;

    //initializing a custom type variable for sellerInfo
    struct SellerInfo {
        address seller;
        uint256 goal;
        uint256 amountHeld;
    ...
    //storing seller's info data as key-value pairs
    mapping(address => SellerInfo) public sellerInfo;

    //initializing two type variables
    uint256 public percentageToHold;
    address public holdingContract;

    //Triggering this functions to act as events which will listen for when they are called
    event ListingCreated(uint256 listingId, uint256 tokenId, address seller, uint256 price, uint256 sharesAvailable);
    event FractionalNFTSold(uint256 listingId, address buyer, uint256 sharesSold);
    event ListingUpdated(uint256 listingId, uint256 price, uint256 sharesAvailable);
    event ListingDeleted(uint256 listingId);
    ...
    {
        holdingContract = _holdingContract;
        percentageToHold = _percentageToHold;
    }


    //This function allows sellers to put up nfts for sale 
    function createListing(uint256 tokenId, uint256 price, uint256 sharesAvailable) external {
        require(sharesAvailable > 0, "Shares available must be greater than zero");
        require(price > 0, "Price must be greater than zero");

        listingIdCounter++;
        listings[listingIdCounter] = Listing(tokenId, msg.sender, price, sharesAvailable);

        emit ListingCreated(listingIdCounter, tokenId, msg.sender, price, sharesAvailable);
    }

    //This function allows any user to buy a fractional nft that is up for sale 
    function buyShares(uint256 listingId, uint256 sharesToBuy) external payable {
        require(sharesToBuy > 0, "Shares to buy must be greater than zero");

        Listing storage listing = listings[listingId];
        require(listing.sharesAvailable >= sharesToBuy, "Insufficient shares available");
        require(msg.value >= listing.price * sharesToBuy, "Insufficient funds");

        // Transfer funds to the seller
        uint256 sellerEarnings = listing.price * sharesToBuy;
        payable(listing.seller).transfer(sellerEarnings);
        sellerInfo[listing.seller].totalEarnings += sellerEarnings;

        // Calculate the percentage to hold
        uint256 amountToHold = (sellerEarnings * percentageToHold) / 100;

        // Transfer the percentage to the holding contract
        sellerInfo[listing.seller].amountHeld += amountToHold;
        payable(holdingContract).transfer(amountToHold);

        // Update the listing
        listing.sharesAvailable -= sharesToBuy;

        // Transfer the fractional NFT to the buyer
        _safeTransferFrom(address(this), msg.sender, listing.tokenId, sharesToBuy, "");

        emit FractionalNFTSold(listingId, msg.sender, sharesToBuy);
    }

    //This function allows sellers to update an existing nft that is up for sale
    function updateListing(uint256 listingId, uint256 price, uint256 sharesAvailable) external {
        require(sharesAvailable > 0, "Shares available must be greater than zero");
        require(price > 0, "Price must be greater than zero");

        Listing storage listing = listings[listingId];
        require(msg.sender == listing.seller, "Only the seller can update the listing");

        listing.price = price;
        listing.sharesAvailable = sharesAvailable;

        emit ListingUpdated(listingId, price, sharesAvailable);
    }

    //This function allows a seller to delete a nft they put up for sale
    function deleteListing(uint256 listingId) external {
        Listing storage listing = listings[listingId];
        require(msg.sender == listing.seller, "Only the seller can delete the listing");

        // Transfer remaining shares back to the seller
        if (listing.sharesAvailable > 0) {
            _safeTransferFrom(address(this), listing.seller, listing.tokenId, listing.sharesAvailable, "");
        }
    ...
    //This function sets the percentage of each earning a seller makes from each sale
    function setPercentageToHold(uint256 _percentageToHold) external onlyOwner {
        percentageToHold = _percentageToHold;
    }

    //This function sets the address of the SLE Vault smart contract which holds the percentage of each seller's funds to be use to pay loans off
    function setHoldingContract(address _holdingContract) external onlyOwner {
        holdingContract = _holdingContract;
    }

    //This function sets the loan amount each selller is trying to raise funds for 
    function setSellerGoal(uint256 goal) external {
        sellerInfo[msg.sender].goal = goal;
    }

    //This function allows each seller to withdrawn their total earnings minus their goal earnings(sle vault)
    function withdrawEarnings() external {
        require(sellerInfo[msg.sender].totalEarnings >= sellerInfo[msg.sender].goal, "Goal not reached");

        uint256 amountToWithdraw = sellerInfo[msg.sender].totalEarnings - sellerInfo[msg.sender].goal;
        sellerInfo[msg.sender].totalEarnings -= amountToWithdraw;
