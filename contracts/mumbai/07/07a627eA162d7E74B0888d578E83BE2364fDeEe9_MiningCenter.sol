...
contract MiningCenter is Ownable {
    ...
    // refresh cooldown mine duration. The unit is 1 days.
    ...
    function getAvailableMintedAmount(
        ...
        if (boosterStatus.boosterStyle == 0) {
            if (
                beginStampList[_owner][_nftType][_nftNum] + 1 days <=
                block.timestamp
            ) {
                return 1;
            } else {
    ...
    function getRemainTimeBySeconds(
        ...
        if (boosterStatus.boosterStyle == 0) {
            ...
            if (boosterStatus.endStamp < block.timestamp) {
                if (boosterStatus.endStamp + 1 days < block.timestamp) {
                    return
                        (boosterStatus.endStamp + 1 days - block.timestamp) /
                        1 seconds;
                }
            }
            return 0;
        ...
        for (
            ...
            for (uint256 i = 0; i < nftHistoryLength[_owner][nftType]; i++) {
                ...
                if (
                    ...
                    if (
                        miningBoosterStatus[_owner].endStamp + 1 days >=
                        block.timestamp
                    ) {
                        beginStampList[_owner][_nftType][_nftNum] = block
                            .timestamp;
