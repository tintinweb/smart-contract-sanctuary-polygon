...
contract AdvisorVault is Ownable {
  ...
  function claimableAmount() public view onlyAdvisor returns (uint256) {
    ...
    if(infos.lastClaimAt == 0) {
      periodToClaim = currentEpoch();
    } else {
      periodToClaim = (currentEpochStartAt - infos.lastClaimAt) / unlockPeriod + 1;
    }
  ...
  function currentEpoch() public view returns (uint) {
    ...
    uint256 epoch = ((block.timestamp - (lockPeriodStart + lockPeriod)) / unlockPeriod) + 1;

    if(epoch > unlockAmountPerPeriodDenominator / unlockAmountPerPeriod) {
      return unlockAmountPerPeriodDenominator / unlockAmountPerPeriod;
    }
