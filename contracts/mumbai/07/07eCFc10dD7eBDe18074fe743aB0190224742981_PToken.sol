...
contract PToken is VersionedInitializable, IPToken {
  ...
  uint256 public constant PTOKEN_REVISION = 0x2;
  ...
  /// @inheritdoc IPToken
  function reduxInterestSplit(uint256 split) external virtual override onlyPool  {
    _updateReduxInterest();
    
    uint256 amount = reduxInterest.reduxTotalAmount / WadRayMath.RAY;
    uint256 rewardSplit = amount * split / 1000000;
    uint256 treasurySplit = amount - rewardSplit;
    if(rewardSplit > 0) IERC20(_underlyingAsset).safeTransfer(address(POOL), rewardSplit);
    if(treasurySplit > 0) IERC20(_underlyingAsset).safeTransfer(_addressesProvider.getDaoWallet(), treasurySplit);
