...
contract EverLand is ERC721Enumerable, Ownable {
        ...
        )
    {
        ...
        uint256[] memory _unit;
        uint32[] memory _id;
        address[] memory _creator;
        for (uint256 i = 0; i < m_AuctionsData.length; i++) {
            _price[i] = m_AuctionsData[i].price;
            _unit[i] = m_AuctionsData[i].unit;
            _id[i] = m_AuctionsData[i].id;
            _creator[i] = m_AuctionsData[i].creator;
        }
        return (_price, _unit, _id, _creator);
    }
