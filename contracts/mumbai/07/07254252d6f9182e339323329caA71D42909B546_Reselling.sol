...
//  mapping(eventId => associated promoCodes[])
    ...
    function deleteEvent(uint eventId) public onlyOwner {
        checkEventExists(eventId);

        delete _eventDetails[eventId];
        delete _eventTicketTypes[eventId];
        delete _promoCodeDiscountOnEvent[_eventId];
    }
    ...
    function isValidEvent(Event calldata eventDetails) private view {
        ...
        if (eventDetails.endDate < currentTimestamp || eventDetails.startDate >= eventDetails.endDate || eventDetails.servicePercentage == 0 || eventDetails.royaltyPercentage == 0){
