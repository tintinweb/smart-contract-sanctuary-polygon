...
contract GUTradeMatcher {
    ...
    function acceptOffer(TradeLib.FinalizeOffer calldata data) external
    {
        ...
        emit OfferAccepted(data.sale.nftId, data.offerSignature, data.offerer, data.offer.to, data.offer.listingId, data.sale.amount, data.offer.amount);
    }
    ...
    function finalizeBatchOffers(TradeLib.FinalizeOffer[] calldata data) external
    {
        for(uint i; i < data.length; i++){
            ...
            emit OfferAccepted(data[i].sale.nftId, data[i].offerSignature, data[i].offerer, data[i].offer.to, data[i].offer.listingId, data[i].sale.amount, data[i].offer.amount);
