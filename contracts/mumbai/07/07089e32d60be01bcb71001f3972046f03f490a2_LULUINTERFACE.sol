...
// File: contracts/20230625LULU.sol
...
interface IERC721Burnable {
    ...
    function balanceOf(address _add) external returns (uint256);

}


contract LULUINTERFACE is ERC721Enumerable, Ownable {
  ...
  // trigger number for move the state from 0 to 1
  uint256 trigger;

  address public fnic1;
 ...
 /* struct data{
  ...
  uint256 public n_whitelisted=0; ///must be equal to the length of PR
  //address payable [] public PR;
  uint256 public ref_rew;

  mapping(uint256 => address) public PR;
  event PRiniti(address indexed _PR);
...
function testburnmint2(uint256 _mintAmount,  address whitelisted1, address whitelisted2)public payable {
      require(msg.value >= cost , "you need to pay more for this nft");
      uint256 supply = totalSupply();
      bool success=false;

      IERC721Burnable burn_contract = IERC721Burnable(address(fnic1));
      require(burn_contract.balanceOf(msg.sender)>0 , "not FNIC1");

      uint256 ID_ =  burn_contract.tokenOfOwnerByIndex(msg.sender,0);
      uint256 _ID =  burn_contract.tokenOfOwnerByIndex(msg.sender, 0);
      burn_contract.burn(_ID);
      PR[ n_PR ] = address (msg.sender);
      emit PRiniti(address (msg.sender));
      whitelist[n_PR]= address (msg.sender);
      n_PR =  n_PR  + 1;
      n_whitelisted = n_whitelisted +1;

      //testburnmint(_mintAmount, whitelisted1, whitelisted2);
          ...
          //mintburn(_mintAmount, whitelisted1, whitelisted2);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 function mint(uint256 _mintAmount,  address whitelisted1, address whitelisted2) public payable {
  ...
  /// set state trigger
    ...
    function settrigger(uint256 _trigger) public onlyOwner{
      trigger = _trigger;
    }
  ...
  //// move the trigger
    ...
    function movetrigger() internal {
      uint256 n_supply = totalSupply();
      if (n_supply >= trigger){
        state = 1;
      }
    }
  ...
  function publicmint (uint256 _mintAmount) public payable {
    ...
    require(state == 1, "it's not time for public sale");
    require(!paused, "the contract is paused");
    uint256 supply = totalSupply();
    require(_mintAmount > 0, "need to mint at least 1 NFT");
    require(_mintAmount <= maxMintAmount, "max mint amount per session exceeded");
