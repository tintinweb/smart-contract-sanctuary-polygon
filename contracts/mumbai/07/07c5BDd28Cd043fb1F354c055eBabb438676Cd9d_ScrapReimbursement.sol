...
contract ScrapReimbursement {
  ...
  constructor(address _scrap, address jys, address blueprints, address sprays) {
    owner = msg.sender;
    scrap = Scrap(_scrap);
    ...
    for (uint256 i = 1; i < 6; i++) {
      rewards[jys][10 + i] = 10;
      rewards[jys][20 + i] = 40;
      rewards[jys][30 + i] = 160;
    }

    for (uint256 i = 1; i < 7; i++) {
      rewards[blueprints][i] = 160;
    }
    ...
    rewards[sprays][4] = 60;
    rewards[sprays][5] = 80;
    rewards[sprays][6] = 150;
    rewards[sprays][7] = 300;
  }

  function reinburse(address[] calldata assets, uint256[] calldata tokenIds) public {
    require(!paused, "Contract paused");
    require(tokenIds.length == assets.length, "Incorrect parameters");
    uint256 totalReward = 0;
    for (uint256 i = 0; i < assets.length; i++) {
      address asset = assets[i];
      uint256 tokenId = tokenIds[i];

      JunkyardScraps c = JunkyardScraps(asset);
      uint256 balance = c.balanceOf(msg.sender, tokenId);
      if (balance == 0) {
        continue;
      }
      uint256 reward = rewards[asset][tokenId];
      
      totalReward += reward * balance;
      c.burn(msg.sender, tokenId, balance);
    }
    scrap.mint(msg.sender, totalReward * 1 ether);
  }
