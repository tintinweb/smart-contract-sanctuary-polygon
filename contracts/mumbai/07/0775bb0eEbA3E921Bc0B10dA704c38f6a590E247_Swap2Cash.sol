...
abstract contract Configuration is Upgradeable {
    ...
    UUPSUpgradeable
{
    ...
    function initialize(address _signer) public virtual initializer {
        __Ownable_init();
...
abstract contract CancelOrderEntity {
    ...
    function hash(
        CancelOrder memory cancelOrder
...
abstract contract CreateOrderEntity {
    ...
    */
    ...
    function hash(
        CreateOrder memory createOrder
    ) internal pure returns (bytes32) {
        bytes32 digest = keccak256(
            abi.encodePacked(
                createOrder.from,
...
contract Swap2Cash is Configuration, Entities, Events {
    ...
    function withdraw(
        ...
        for (uint256 i = 0; i < len; i++) {
            transfer(wd.from, wd.to, wd.tokens[i], wd.amounts[i]);
        }
    ...
    function emergencyWithdraw(address[] calldata tokens) external onlyOwner {
        ...
        for (uint i = 0; i < len; i++) {
            amounts[i] = withdraw(tokens[i], msg.sender);
        }
