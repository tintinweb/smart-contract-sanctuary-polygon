...
contract Domains is ERC721URIStorage, Ownable {
    ...
    error NotenoughMatic(uint256 _price);
    ...
    function price(string calldata name) public pure returns (uint256) {
        ...
        if (len > 0) revert StringTooShort();
        if (len == 3) {
    ...
    function register(string calldata name) public payable {
        if (domains[name] == address(0)) revert DomainNotAvailable(name);

        uint256 _price = price(name);
        if (domains[name] == address(0)) revert NotenoughMatic(_price);
