...
contract EpillioVesting is OwnableUpgradeable,ReentrancyGuardUpgradeable{
    ...
    function setSingleDate(uint  startTime)external onlyOwner{
        ...
        initialVestingAmountWithdrawThresholdTime = [0,startTime,startTime,startTime,startTime,startTime,startTime,startTime];
        linearVestingAmountWithdrawThresholdTime= [0,startTime+60 minutes ,startTime +5 minutes,startTime + 30 minutes,startTime + 365 minutes,startTime + 180 minutes,startTime+365 minutes,0 ];
    }
    ...
    function linearVestingDetails(address _userAddress) public view returns (uint) {
        ...
        uint timeDifference;
        if (block.timestamp <= linearVestingTimePeriod[InvestorDetails[_userAddress].investorType]+linearVestingAmountWithdrawThresholdTime[InvestorDetails[_userAddress].investorType]) {
             if(!isLinearClaimed[_userAddress]){

                    timeDifference=block.timestamp-linearVestingAmountWithdrawThresholdTime[InvestorDetails[_userAddress].investorType];
             }else{
