    ...
    UUPSUpgradeable
{
    ...
    function initialize(
        ...
        require(nftAddress != address(0x0), 'LWNFT: NFT is zero address');
        require(rbaLinkerAddress != address(0x0), 'LWNFT: RBA linker is zero address');
        LCRBALinker = ILostChainRBA(rbaLinkerAddress);
    ...
    function editLand(
        ...
        require(
            rewardList.length == rewardAmount.length && rewardAmount.length == maxRewardAmount.length,
            'LWNFT: Array lengths mismatch'
        );
        require(landPrice > 0, 'LWNFT: Land price is 0');

        for (uint256 i = 0; i < rewardAmount.length; i++) {
            require(rewardAmount[i] > 0, 'LWNFT: Reward amount must be minimum of 1');
            require(maxRewardAmount[i] > 0, 'LWNFT: Max reward amount must be minimum of 1');
            require(rewardAmount[i] <= maxRewardAmount[i], 'LWNFT: Max reward over limit');
        }

        for (uint256 i = 0; i < enemyList.length; i++) {
            require(LWD.checkEnemyAvailable(enemyList[i]), 'LWNFT: Enemy is disabled');
        }
        for (uint256 i = 0; i < rewardList.length; i++) {
            require(LCNFT.checkNFTRewardable(rewardList[i]), 'LWNFT: NFT is not rewardable');
        }
