...
contract POT is ReentrancyGuard {
    ...
    bool public delivery_by_passenger = false;
    bool public delivery_by_driver = false;
    bool public delivery_by_app = false;
    ...
    function confirmDeliveryByPassenger() public onlyPassenger {
        require(
            currState == State.AWAITING_DELIVERY && activated_by_driver,
            "Delivery not confirmed by driver yet"
        );
        require(!delivery_by_passenger, "Already confirmed delivery");
        delivery_by_passenger = true;
        contract_settled = true;
        token.transfer(driver, amount_payable);
        token.transfer(arbitrator, arbitration_fee);
    ...
    function confirmDeliveryByApp() public onlyArbitrator {
        require(
            currState == State.AWAITING_DELIVERY && delivery_by_driver == true,
            "Cannot confirm delivery"
        );
        currState = State.COMPLETE;
        delivery_by_app = true;
        contract_settled = true;
        token.transfer(driver, amount_payable);
        token.transfer(arbitrator, arbitration_fee);
    }
    ...
    function withdrawByPassenger() public onlyPassenger {
        require(activated_by_passenger && contract_activated == false);
        activated_by_passenger = false;
        uint256 amount_payable_by_passenger = amount_payable + arbitration_fee;
        token.transfer(passenger, amount_payable_by_passenger);
    }
