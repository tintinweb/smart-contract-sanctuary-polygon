...
contract ElegantEV1 is Ownable, AccessControl {
    ...
    function signUp (uint[] calldata _racers, uint _event) public payable{
        ...
        teamInfo[address(0)].maticEarned = teamInfo[address(0)].maticEarned.add(moolah);
        teamInfo[address(0)].nutsEarned = teamInfo[address(0)].nutsEarned.add(moolah);
    ...
    function nextRace() public {
        ...
        if (currentRace < (raceNum-1)){
            ...
            uint[] memory prizes = RulesInterface(rulesByRace[_ev].conCheck).prizeCheck();
            uint[] memory nutses = RulesInterface(rulesByRace[_ev].conCheck).nutPrizeCheck();
            uint i = 0;
    ...
    function returnTokenAll() public {
        ...
        while (stakedNFTs.length > 0) {
            ...
            currentlyStaked[_stakeID] = currentlyStaked[(currentlyStaked.length-1)];
            currentlyStaked.pop();
