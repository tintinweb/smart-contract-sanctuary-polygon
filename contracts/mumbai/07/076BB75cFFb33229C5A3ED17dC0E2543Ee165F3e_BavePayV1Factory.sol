...
import "./BavePayV1Payment.sol";

contract BavePayV1Factory is IBavePayV1Factory {
    address public baveAcct;
    address public baveOper;

    address[] public allPayments;

    constructor(address _baveAcct, address _baveOper) {
        baveAcct = _baveAcct;
        baveOper = _baveOper;
        emit BaveChanged(0, address(0), baveAcct);
        emit BaveChanged(1, address(0), baveOper);
    }
    ...
    function createPayment(
        ...
        payment = address(
            new BavePayV1Payment(clientAcct, clientOper, tokenArray, rate, allPayments.length)
        );
        emit PaymentCreated(allPayments.length, payment);
        emit ClientChanged(allPayments.length, 0, address(0), clientAcct);
        emit ClientChanged(allPayments.length, 1, address(0), clientOper);
    ...
    function ledger(
        ...
        require(msg.sender == allPayments[paymentId]);
        emit Ledger(paymentId, token, from, to, amount);
    }
    ...
    function clientChanged(
        ...
        require(msg.sender == allPayments[paymentId]);
        emit ClientChanged(paymentId, class, oldClient, newClient);
    }

    function setBaveAcct(address _baveAcct) external {
        require(msg.sender == baveOper);
        require(_baveAcct != address(0));
        emit BaveChanged(0, baveAcct, _baveAcct);
        baveAcct = _baveAcct;
    }

    function setBaveOper(address _baveOper) external {
        require(msg.sender == baveOper);
        require(_baveOper != address(0));
        emit BaveChanged(1, baveOper, _baveOper);
        baveOper = _baveOper;
    }
}
