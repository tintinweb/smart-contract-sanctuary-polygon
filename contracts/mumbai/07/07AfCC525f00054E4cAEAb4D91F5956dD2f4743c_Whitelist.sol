...
contract Whitelist is AccessControl, ERC1155Holder {
    ...
    mapping(address => Discount[]) public discountsUnique;
    mapping(address => uint) public discountsUniqueCount;
    ...
    constructor(address _weth, address _presale) {
      ...
      // add discounts
      discounts.push(Discount(250, 5, true));
      discounts.push(Discount(500, 10, true));
      discounts.push(Discount(1000, 15, true));
      discounts.push(Discount(2000, 20, true));
    ...
    /// @param _addressesToWhitelist array of addresses
    ...
    function removeUsers(address[] calldata _addressesToRemove) external onlyRole(CLEVEL) {
      for(uint i = 0; i < _addressesToRemove.length; i++) {
        whitelistedAddresses[_addressesToRemove[i]] = false;    
      }
    }

    /// @param _whitelistedAddress address to check
    ...
    /// @dev users can buy nfts from contract balance, ceo can buy freely to reallocate tokens
    function buyNft(uint _amount) external isWhitelisted(msg.sender) {  
        ...
        if(!hasRole(CEO, msg.sender)) {
            ...
            uint _discount = discountAmount(_amount);
    ...
    /// @dev ceo can withdraw tokens sent to contract including weth
    ...
    function discountAmount(uint _amount) public view returns(uint) {
      uint _discountPercent = 0;
      uint _discount = 0;
      
      // loop and check if amount exceed the discount levels
      if(discountsUnique[msg.sender].length > 0) {
        for(uint i = 0; i < discountsUnique[msg.sender].length; i++) {
          if(_amount >= discountsUnique[msg.sender][i].amount) {
            _discountPercent = discountsUnique[msg.sender][i].percent;
          }
        }
      }
      else {
         for(uint i = 0; i < discounts.length; i++) {
          if(_amount >= discounts[i].amount) {
            _discountPercent = discounts[i].percent;
          }
        } 
      }
      

      // calculate discount amount and subtract from total
      _discount = ((salePrice*_amount)/100) * _discountPercent;
      return _discount;
    }


    function addUniqueDiscounts(address _user, uint256[] calldata _amounts, uint256[] calldata _percents) public onlyRole(CLEVEL) {
      require(_amounts.length == _percents.length, 'array lengths should match');

      for(uint i = 0; i < _amounts.length; i++) {
        if(discountsUnique[msg.sender].length > i) {
          discountsUnique[_user][i].amount = _amounts[i];
          discountsUnique[_user][i].percent = _percents[i];
        } 
        else {
          Discount memory _discounts = Discount(_amounts[i], _percents[i], true);
          discountsUnique[_user].push(_discounts);
        }
      }

      discountsUniqueCount[_user] = discountsUnique[_user].length;
    }


    function deleteUniqueDiscounts(address _address) public onlyRole(CLEVEL) {
      delete discountsUnique[_address];
      discountsUniqueCount[_address] = discountsUnique[_address].length;
    }

    // @dev override ERC1155 method
    function onERC1155Received(
        address,
        address,
        uint256,
    ...
    // @dev enable support for ERC1155
