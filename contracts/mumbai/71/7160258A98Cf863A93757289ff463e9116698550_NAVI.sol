...
contract NAVI {
    ...
    function getContractInfos() external view returns(address[3] memory infos0, uint256[6] memory infos1, bool freezing) {
        infos0[0] = address(usdt);
        infos0[1] = feeReceiver;
        infos0[2] = defaultRefer;
        infos1[0] = startTime;        
        infos1[1] = totalUsers;
        infos1[2] = totalDeposit;
        infos1[3] = freezedTimes;
        infos1[4] = freezeTime[freezedTimes];
        infos1[5] = unfreezeTime[freezedTimes];
        freezing = isFreezing;
    }   
}