...
contract AuctionWithAdmin is Ownable {
    ...
    mapping(address => mapping(uint256 => BidDetails)) private bid_info;
    // Mapping from address to token ID for bid
    mapping(address => mapping(uint256 => OfferDetails)) private offer_info;
    ...
    /// @param _tokenFee - Token fee.
    function setCancelOfferFee(uint256 _tokenFee) public {
    ...
    function cancelOffer(uint256 _tokenId) public {
        ...
        emit OfferCancelled(msg.sender, _tokenId, pending_claim_offer[msg.sender][_tokenId] - (pending_claim_offer[msg.sender][_tokenId] * (cancel_offer_fee / 100)), block.timestamp);
    ...
    function makeOffer(uint256 _tokenId, uint256 _amount) public {             
        ...
        offer_info[offer_info[msg.sender][_tokenId].prevOfferer][_tokenId].offerer = offer_info[msg.sender][_tokenId].offerer;
        offer_info[offer_info[msg.sender][_tokenId].offerer][_tokenId].offerer = offer_info[msg.sender][_tokenId].offerer;
        delete offer_info[msg.sender][_tokenId];       
    ...
    function auctionClaim(uint256 _tokenId) public {
        ...
        delete bid_info[auction[_tokenId].highestBidder][_tokenId];
    ...
    function getBidInfo(address _user, uint256 _tokenId) public view returns(BidDetails memory user_bid_info){
        return bid_info[_user][_tokenId];
    }

    /// @dev Returns user offer details.
    function getOfferInfo(address _user, uint256 _tokenId) public view returns(OfferDetails memory user_offer_info){
        return offer_info[_user][_tokenId];
    }
