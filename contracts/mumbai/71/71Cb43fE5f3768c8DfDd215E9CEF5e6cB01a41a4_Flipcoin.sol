...
function acceptBet(uint256 lobbyId) external gasLimit(200000) {
    ...
    for (uint256 i = 0; i < _betAddresses.length; i++) {
        if (_bets[_betAddresses[i]] > 0 && _lobbies[lobbyId][_betAddresses[i]] > 0 && _betAddresses[i] != msg.sender) {
            opponent = _betAddresses[i];
            opponentIndex = i;
            break;
        }
    }
    ...
    delete _lobbies[lobbyId][_betAddresses[opponentIndex]];
