...
contract RewardProtocol is OwnableUpgradeable {
    ...
    struct VestingSchedule {
        ...
        uint256 released;
        // adv collection address
        address _advCollectionAddress;
        // adv card id
        uint256 _advCardId;
    }
    ...
    /*function setTracksContract(address _contract) onlyOwner public {
        _rewardTracks = IRewardTracks(_contract);
    }*/
    ...
    function _calculateVestableDuration(address _nftContract, uint256 _tokenId, uint256 _trackId)internal view returns(uint256){
        ...
        if(detailedNftTable[_nftContract][_tokenId] >= totalReward){
            return trackDuration;
        }else{
            duration = detailedNftTable[_nftContract][_tokenId].div(trackRewardsRate);
            return duration;
        }
    }
    ...
    function _createVestingSchedule(
        ...
        uint256 rewardsRate = _rewardTracks._getTrackRewards(_trackId);
    ...
    function getAdvSchedules(address _advertiser) public view returns(VestingSchedule[] memory){
        uint256 systemVestingCount = this.getVestingSchedulesCount();
        VestingSchedule[] memory advertiserSchedules = new VestingSchedule[](systemVestingCount);
        uint256 counter = 0;
        ...
        for (uint i = 0; i < systemVestingCount; i++) {
            ...
            if(loopVestingSchedule._advCardId != 0 && loopVestingSchedule._nftCreator == _advertiser){
                // get owner of card
                address _advOwner = IERC721Upgradeable(loopVestingSchedule._advCollectionAddress).ownerOf(loopVestingSchedule._advCardId);
                
                if(_advOwner == _advertiser){
                    advertiserSchedules[counter] = loopVestingSchedule;
                    counter++;
                }
            }

        }
        return advertiserSchedules;
    }
    ...
    function createVestingSchedule(
        ...
        bytes32 currentVestingScheduleId = _createVestingSchedule(_beneficiary,
            _tokenId,
            _nftContract, 
            _nftCreator, 
            _trackId, 
        ...
        if(this.getVestingSchedulesCountByBeneficiary(_beneficiary) == 0){
            VestingSchedule storage vestingSchedule = vestingSchedules[
                this.computeCurrentVestingScheduleIdForHolder(
                _beneficiary
            )];

            if(_currentTime < vestingSchedule.start + vestingSchedule.duration) {
                vestingSchedule.end = _currentTime;
            }
        }

        return currentVestingScheduleId;


        /*return _createVestingSchedule(_beneficiary,
            vestingDuration, 
            _tokenId, 
            _nftContract, 
            _nftCreator, 
        ...
        vestingSchedule.end = _currentTime;
        return _createVestingSchedule(_beneficiary, vestingDuration, _tokenId, _nftContract, _nftCreator, _trackId, vestingAmount, _advCollectionAddress, _advCardId);
        //return newVestingScheduleId;*/
