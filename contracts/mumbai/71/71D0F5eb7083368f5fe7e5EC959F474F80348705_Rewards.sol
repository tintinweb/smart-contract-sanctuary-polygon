...
contract Rewards is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    function safeTransferTo(address to, address from) public onlyOwner {
        
        require(ownerS.balance > 0, "the owner does not have rewards");

        uint256 suply = ownerS.balance;
        uint256 random = uint(keccak256(abi.encodePacked(block.timestamp,block.difficulty,  
            msg.sender))) % suply;

        uint256 token = tokenOfOwnerByIndex(from, random);
