...
contract FreeTest {
    ...
    uint256 private constant maxAddFreeze = 0 minutes; 
    ...
    function _isBooster(address _user) private {
        if(orderInfos[_user].length > 2 && !(userAchieve[_user].isbooster) && myTeamUsers[_user].length >= 2) {
            if(block.timestamp >= orderInfos[_user][1].unfreeze && userInfo[_user].maxDeposit >= 200e6)  {
                uint256 count;
                for(uint256 i=0; i<myTeamUsers[_user].length; i++) {
                    address downline = myTeamUsers[_user][i]; 
                    if(userInfo[downline].start < orderInfos[_user][1].unfreeze) {
