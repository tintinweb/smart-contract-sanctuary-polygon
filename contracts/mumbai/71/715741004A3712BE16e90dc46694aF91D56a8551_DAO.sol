...
contract DAO is OwnableUpgradeable, UUPSUpgradeable {
    ...
    IMOGTokenUpgradeable public token;
    ...
    event voted(
        uint256 totalForVote,
        uint256 totalAgainstVote,
        uint256 vote,
        address voter
    );
    ///@dev To notify execution of a proposal
    event executed(uint256 tokenId, uint256 proposalId, bool result);
    ...
    function initialize(
        ...
        IMOGTokenUpgradeable _token
    ) public initializer {
        __Ownable_init();
        __UUPSUpgradeable_init();
        setVotingDelayAndPeriod(_votingDelay, _votingPeriod);
        setPropertyManager(_propertyManager);
        token = IMOGTokenUpgradeable(_token);
        quorumPercentage = (_quorumPercentage * 100);
    }
    ...
    ///@dev Call this function to get voting detail.
    ///@param _proposalId Id of the proposal.
    ///@param _tokenId Id of the proposal.
    function getVotingDetail(uint256 _tokenId, uint256 _proposalId)
        public
        view
        returns (uint256, uint256)
    {
        ...
        uint256 forVote = voting.forVote;
        uint256 against = voting.against;
        return (forVote, against);
    }
    ...
    function propose(
        ...
        uint256 tokenTotalSupply = token.totalSupply(_tokenId);
        proposals[_tokenId][proposalCounter[_tokenId]]
            .totalTokenSupply = tokenTotalSupply;
        proposals[_tokenId][proposalCounter[_tokenId]]
        ...
        uint256 quorumVotes = (quorumPercentage * tokenTotalSupply) / 10000;
    ...
    function vote(
        ...
        require(
            ...
            "Invalid proof: Voter is not whitelisted with this amount"
        );
        require(
            getProposalState(_tokenId, _proposalId) == ProposalState.Active,
            "Voting is not active"
        );
        require(_vote == 0 || _vote == 1, "Invalid vote id");
        ...
        returns (bool result)
    {
        ...
        emit executed(_tokenId, _proposalId, result);
    }
        ...
        returns (bool result)
    {
        Voting storage voting = votings[_tokenId][_proposalId];
        Proposal memory proposal = proposals[_tokenId][_proposalId];
        return ((voting.forVote) >=
            ((proposal.totalTokenSupply * quorumPercentage) / 10000));
        // return voting.forVote > voting.against;
    }
