...
interface IPack {
  ...
  function externalURIs(uint16 seriesId, uint8 editionId)
    external
    view
    returns (string memory);

  function imageURIs(uint16 seriesId, uint8 editionId)
    external
    view
    returns (string memory);
...
import "./interfaces/IPackTokenURIGetter.sol";
  ...
  ERC721ABurnable
{
  ...
  mapping(uint16 => mapping(uint8 => string)) public override imageURIs;
  mapping(uint16 => mapping(uint8 => string)) public override externalURIs;
  mapping(uint16 => bool) public isSeriesFrozen;
  mapping(uint16 => mapping(uint8 => IPackTokenURIGetter))
  ...
  modifier seriesIsNotFrozen(uint16 id) {
    require(!isSeriesFrozen[id], "Pack: Series already frozen");
    _;
  }
  ...
  function mint(
    ...
    require(
      ...
      "Pack: Series or edition is not available"
    );
    ...
    returns (string memory)
  {
    ...
    if (address(tokenURIGetters[seriesId][editionId]) != address(0)) {
      return
      ...
      bytes(
        string(
          abi.encodePacked(
            ...
            _series.description,
            '", "image" : "',
            imageURIs[seriesId][editionId],
            '", "external_url" : "',
            externalURIs[seriesId][editionId],
            tokenIdStr,
            '", "attributes": [{"trait_type": "Series", "value": "',
            _series.name,
            '"}, {"trait_type": "Edition", "display_type": "number", "value": ',
            editionId.toString(),
            '}, {"trait_type": "Pack Number", "value": ',
            tokenIdStr,
            "}]}"
          )
        )
  ...
  function setTokenURIGetter(
    uint16 seriesId,
    uint8 editionId,
    IPackTokenURIGetter tokenURIGetter
  ...
  function setExternalURI(
    ...
    externalURIs[seriesId][editionId] = uri;
    emit ExternalURISet(seriesId, editionId, uri);
  }
  ...
  function setImageURI(
    ...
    imageURIs[seriesId][editionId] = imageURI;
    emit ImageURISet(seriesId, editionId, imageURI);
  }
