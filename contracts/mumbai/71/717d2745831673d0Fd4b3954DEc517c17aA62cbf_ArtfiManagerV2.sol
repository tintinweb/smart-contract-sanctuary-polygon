    ...
    PausableUpgradeable
{
    ...
    function verifyFixedPriceLazyMintV2(
        ...
        bytes32 digest = _hashTypedDataV4(
            keccak256(
                abi.encode(
                    keccak256(
                        "NFT(address tokenAddress, string uri,address seller, address buyer, string uid, address[] creators,uint256[] royalties,address[] investors,uint256[] stakeRoyalty,uint256 minPrice,uint256 quantity,string currency)"
                    ),
                    lazyData_.tokenAddress,
                    keccak256(abi.encodePacked(lazyData_.uri)),
                    lazyData_.seller,
                    lazyData_.buyer,
                    keccak256(abi.encodePacked(lazyData_.uid)),
    ...
    function _is721(address tokenContract) internal returns (bool) {
        return
            _checkInterface(
                tokenContract,
                type(ArtfiICollectionV2).interfaceId
            );
    }
}
