...
contract RevenueShare is AccessControl, ReentrancyGuard{
  ...
  function batchWithdraw(uint256 counter) external onlyRole(WITHDRAWER_ROLE) payable nonReentrant{
    if(withdrawnId + counter > receiveId) revert NoWithdrawableRequest();
    ...
    for(uint i=withdrawnId; i<= withdrawnId + counter; i++) {
  ...
  function withdrawETH(address account) external payable nonReentrant{
    if(claimablePerAddress[account].amountETH == 0) revert AcountHasNoClaimableAmount(account);
    uint256 amount = claimablePerAddress[account].amountETH;
  ...
  function withdrawWETH(address account) external payable nonReentrant{
    if(claimablePerAddress[account].amountWETH == 0) revert AcountHasNoClaimableAmount(account);
    uint256 amount = claimablePerAddress[account].amountWETH;
