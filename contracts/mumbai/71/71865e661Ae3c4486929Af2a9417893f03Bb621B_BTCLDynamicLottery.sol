...
contract BTCLDynamicLottery is AutomationCompatible, AutomatedRandomness, ReentrancyGuard {
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomness) internal override {
        ...
        uint256 numWordsPerRound = totalWinners * 3; // 3 numWords per round winner
        uint256 startIndex = 0;

        // Loop through all batches of rounds that need to be drawn
        while (roundsProcessed < maxRounds) {
            // Loop through all rounds in the current batch and set the randomness
            for (uint256 i = 0; i < maxRounds; i++) {
                ...
                if (rounds[roundNr].status.requestId == requestId && rounds[roundNr].status.roundStatus == BTCLCore.Status.Drawing) {
                    ...
                    // Increment the index for the next round's randomness
                    startIndex += numWordsPerRound;
                }
            }

            // Increment the next round if all rounds in the current batch have been drawn
