...
contract NFTWithFee is ERC721URIStorage, Ownable {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIdCounter;

    uint256 public constant MINT_PRICE = 5 * 10**17; // 0.5 ETH
    uint256 public constant DEPLOYER_FEE_PERCENTAGE = 1;

    constructor() ERC721("NFTWithFee", "NWF") {
    }

    function mintNFT(address recipient, string memory tokenURI) public payable {
        require(msg.value == MINT_PRICE, "Invalid mint price");

        uint256 deployerFee = (MINT_PRICE * DEPLOYER_FEE_PERCENTAGE) / 100;
        payable(owner()).transfer(deployerFee);
