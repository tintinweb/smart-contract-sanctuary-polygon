...
contract SecondNft is ERC721, ERC2981, Ownable, MerkleProof, EIP712 {
    ...
    mapping(uint256 => blindBox) blindBoxMap;
    uint256 blindBoxMaxSupply = 100;
    mapping(uint256 => bool) blindBoxMinted;
    ...
    function drawCard()public payable returns(uint256){
        require(msg.value<100,"token is not enough");
        uint256 requestId = VRFv2Consumer(0x99B2D80c8DfeB1C14Bbd23B211C807CDA8d6E6AA).requestRandomWords();
        blindBoxMap[requestId] = blindBox(requestId,msg.sender,false);
        return requestId;
    }

    function openBox(uint256 requestId)public returns(uint256){
        bool fulfilled;
        uint256[] memory randomWords;
        (fulfilled,randomWords) = VRFv2Consumer(0x99B2D80c8DfeB1C14Bbd23B211C807CDA8d6E6AA).getRequestStatus(requestId);
        uint256 initialRandomIndex = randomWords[0] % blindBoxMaxSupply;
        bool minted = false;
        uint256 supply = _tokenIds.current();
        if(!blindBoxMinted[initialRandomIndex]){
            for(uint256 i = initialRandomIndex+1; i < blindBoxMaxSupply; i++){
                if(blindBoxMinted[initialRandomIndex]){
                    continue; 
                }else{
                    tokenCreateAddress[supply] = msg.sender;
                    ...
                    _tokenIds.increment();
                    _setTokenURI(supply, string(abi.encodePacked(baseURI, i.toString())));
                    blindBoxMinted[i] = true;
                    minted = true;
                    break;
                }
            }
            if(minted){
                ...
                for(uint256 i = initialRandomIndex-1; i >= 0; i--){
                    if(blindBoxMinted[initialRandomIndex]){
                        continue; 
                    }else{
                        tokenCreateAddress[supply] = msg.sender;
            ...
            _mint(msg.sender, supply);
            _tokenIds.increment();
            _setTokenURI(supply, string(abi.encodePacked(baseURI, initialRandomIndex.toString())));
            blindBoxMinted[initialRandomIndex] = true;
            return supply;
        }  
    }
