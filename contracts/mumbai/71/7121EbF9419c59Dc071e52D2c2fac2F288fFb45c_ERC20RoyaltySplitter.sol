...
contract ERC20RoyaltySplitter is ERC20Splitter {
  ...
  /**
   * @notice Fallback function to split the incoming tokens
   * @dev This function is called when the contract receives tokens
   */
  fallback() external {
    this.split(IERC20(token).balanceOf(address(this)));
  }

  function mockMint(uint256 qty) external {
    require(
      IERC20(token).approve(address(this), type(uint256).max),
      "ERC20Splitter: approve failed"
    );
    this.split(qty * price);
  }
}
...
abstract contract ERC20Splitter is Ownable {
  ...
  function split(uint256 _splitAmount) external {
    ...
    for (uint i = 0; i < splitAddresses.length; i++) {
      ...
      bool sent = IERC20(token).transfer(splitAddresses[i], amount);
  ...
  function referralSplit(uint256 _splitAmount, address referrer) external {
    ...
    for (uint i = 0; i < referralAddresses.length; i++) {
      ...
      if (referralAddresses[i] == address(0)) {
        bool sent = IERC20(token).transfer(referrer, amount);
        require(sent, "Splitter: Couldn't send tokens to you.");
        emit Referral(referrer, amount);
      } else {
        bool sent = IERC20(token).transfer(referralAddresses[i], amount);
        require(sent, "Splitter: Couldn't send tokens to you.");
