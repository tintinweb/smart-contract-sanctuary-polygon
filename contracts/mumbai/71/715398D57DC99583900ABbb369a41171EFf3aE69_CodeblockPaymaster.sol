 ...
 */
contract CodeblockPaymaster is BasePaymaster, Pausable {
        ...
        returns (bytes memory context, bool revertOnRecipientRevert)
    {
        ...
        require(
            !senderBlacklist[relayRequest.request.from],
            "sender blacklisted"
        );
    ...
    function addBlacklistSender(address sender) external onlyOwner {
        require(sender != address(0), "sender can't be zero address");
        senderBlacklist[sender] = true;
    }

    function removeBlacklistSender(address sender) external onlyOwner {
        require(sender != address(0), "sender can't be zero address");
        delete senderBlacklist[sender];
    }
