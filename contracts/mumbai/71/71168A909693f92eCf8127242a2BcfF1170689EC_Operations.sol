...
function sendMessageToGroup(
    ...
    string[] memory message,
    string[] memory cciMessages,
    bool isShareble,
    string[] memory fileHash,
    string memory emailGroup,
    address[] memory cciReceivers,
    uint256 timestamp
) external {
    require(
        structures.checkUserExists(msg.sender) == true,
    ...
    sendRegularMessages(receiver, subject, message, messageTimestamp, isShareble, fileHash, emailGroup);
    sendCCIMessages(cciReceivers, subject, cciMessages, messageTimestamp, isShareble, fileHash, emailGroup);
}

function sendRegularMessages(
    address[] memory receiver,
    string[] memory subject,
    string[] memory message,
...
function sendCCIMessages(
    ...
    for (uint256 i = 0; i < cciReceivers.length; i++) {
...
contract Operations {
    ...
    constructor() {
        structures = Structures(0x6C330e24A6BDfDf8017994C134E20FE35C38D03A);
        chat = Chat(0xf1B219Ff33077Ad005108fAD6E8173341Bf0452C);
    }
