 ...
 */
contract WeatherStaticNFT is FunctionsClient, ConfirmedOwner {
   ...
   * @param _subscriptionId The Functions billing subscription ID used to pay for Functions requests
   * @param _fulfillGasLimit Maximum amount of gas used to call the client contract's `handleOracleFulfillment` function
   * @param newCBOR Buffer
   */
  function setRequest(
    uint64 _subscriptionId,
    uint32 _fulfillGasLimit,
    bytes memory newCBOR
  ) external onlyOwner {
    subscriptionId = _subscriptionId;
    fulfillGasLimit = _fulfillGasLimit;
    requestCBOR = newCBOR;
  ...
  function fulfillRequest(bytes32 requestId, bytes memory response, bytes memory err) internal override {
    ...
    currWeather = uint256(abi.decode(response, (int256)));

    // todo: Mint NFT for the current weather and get that image
