...
contract SocialBlocks is AccountCreation("Social Blocks", "$SB"), Ownable {
    ...
    // uint8 public constant BUYABLE = 0;
    // uint8 public constant BIDDABLE = 1;
    // uint8 public constant NOT_FOR_SELL = 2;
    ...
    // mapping from user to rewardPaid
    mapping(address => uint256) public rewardPaid;


    constructor(IMintable _rewardToken) {
        rewardToken = _rewardToken;
    }
    
    // onlyowner functions
    function changeRewardToken(IMintable _rewardToken) external onlyOwner {
        rewardToken = _rewardToken;
    }
    ...
    //setter functions
    function changeRewardFactor(uint256 _rewardFactor) external onlyOwner{
        require(_rewardFactor != _rewardFactor, "can not set the previous reward factor");
    ...
    function changePostStatus(uint256 _postId, uint8 _status) external {
        require(
            _status < 3 && postInfo[_postId].buyStatus != _status,
            "can't set the previous or undefined status"
        );
        require(_postId > 0, "postId must be greater than 0");
        uint256 postId = userPostIds[msg.sender][_postId - 1]; //postIds are indexed from 1
        postInfo[postId].buyStatus = _status;
        emit PostDetailsChanged(_postId, _status, postInfo[postId].sellValue);
    }

    function changePostPrice(uint256 _postId, uint256 _price) external {
        require(_postId > 0, "postId must be greater than 0");
        // not directly accessing the postInfo mapping because of security concerns
        uint256 postId = userPostIds[msg.sender][_postId - 1]; //postIds are indexed from 1
        postInfo[postId].sellValue = _price;
        emit PostDetailsChanged(_postId, postInfo[postId].buyStatus, _price);
    }

    function claimPostReward(uint256 _postId, uint256 _likesCount) external {
        //local copy for gas efficiency
        address user = msg.sender;

        // total reward of the post minus rewardPaid = reward to be paid
        uint256 reward = ((_likesCount * ONE) / rewardFactor) - rewardPaid[user];
        
        rewardPaid[user] += reward;
        if(reward > 0) rewardToken.mint(user, reward);
        emit PostRewardClaimed(user, _postId, reward);
    }

    function mint(uint8 _buyStatus, uint256 _sellValue, string memory tokenURI_) external {
        ...
        emit PostCreated(maxTokenId,msg.sender,tokenURI_, _buyStatus, _sellValue);
        postInfo[maxTokenId] = PostInfo(_buyStatus, _sellValue, tokenURI_);
        userPostIds[msg.sender].push(maxTokenId);
        maxTokenId++;
    }
}
...
contract AccountCreation is ERC721Enumerable {
    ...
    event PostDetailsChanged(uint256 postId, uint8 status,  uint256 price);
    ...
    // function changeBio(string memory _bio) public virtual {
    //     bytes memory b = bytes(_bio);
    //     require(b.length <= 50, "Bio can't be longer than 25 characters");
    //     bio[msg.sender] = _bio;
    //     emit BioChange(_bio);
    // }
