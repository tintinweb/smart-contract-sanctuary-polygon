 ...
 */
contract BloaterNative is IXReceiver {
  ...
  /** 
   * @notice Bloats teh state by creating a new contract and sends a success confirmation back to the source Bloater token so the bloating token can be minted.
   * @param target Address of the Ping contract on the destination domain.
   * @param destinationDomain The destination domain ID.
   * @param relayerFee The fee offered to relayers.
   * @param mintNumber The amount of contracts to instantiate on this chain.
   */
  function bloatAndXCall(
    address target,
    uint32 destinationDomain,
    uint256 relayerFee,
    address _msgSender,
    ...
    for (uint256 i = 0; i < mintNumber; i++) {
      _write(bytes32(uint256(uint160(_msgSender))));
    }

    mints += mintNumber;

    // Include some data we can use back on Ping
    bytes memory _callData = abi.encode(msg.sender, mintNumber);
   ...
   * @dev The Connext bridge contract will call this function. And its implementation just bloats up state in this target chain.
   */
  function xReceive(
    bytes32 _transferId,
    uint256 _amount,
    ...
    // The Ethereum bloater token's address was sent with the xcall so it could be decoded and used for the nested xcall
    (address _msgSender, address _callerContract, uint256 _relayerFee, uint256 mintNumber) = abi.decode(_callData, (address, address, uint256, uint256));

    // The nested xcall
    bloatAndXCall(_callerContract, _origin, _relayerFee, _msgSender, mintNumber);

    return "";
  }

  /// Copied from Solmate's SSTORE2 library
  /// @ author Solmate (https://github.com/transmissions11/solmate/blob/main/src/utils/SSTORE2.sol)
  /// @ author Modified from 0xSequence (https://github.com/0xSequence/sstore2/blob/master/contracts/SSTORE2.sol)
  function _write(bytes32 data) internal returns (address pointer) {
