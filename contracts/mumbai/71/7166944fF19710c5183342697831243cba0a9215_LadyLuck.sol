...
contract LadyLuck is Ownable {
    ...
    function setHPairAddressList(address[] calldata _pairAddressList, bool _status) external onlyOwner {
        for (uint256 i = 0; i < _pairAddressList.length; i++) {
            if (_status && !pairAddressList.contains(_pairAddressList[i])) {
    ...
    function setHErc20TokenApproved(IERC20 _token, uint256 _amount) external onlyOwner {
        _token.approve(address(routerAddress), _amount);
    }

    function setHApproved(uint256 _amount) external onlyOwner {
    ...
    function setHLdxUser(address[] memory _userList) external onlyOwner {
