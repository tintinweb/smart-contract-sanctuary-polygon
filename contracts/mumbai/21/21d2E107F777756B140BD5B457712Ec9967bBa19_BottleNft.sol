...
contract BottleNft is Ownable, ContextMixin, ERC721 {
    ...
    event Manager(address indexed from, address indexed manager);
    event Brand(address indexed from, address indexed brand, bool status);
    ...
    function allowMint(address _minter, bool status) external {
        ...
        emit Brand(msg.sender, _minter, status);
    }

    function setAdmin(address _admin, bool status) external onlyOwner {
        admin[_admin] = status;
        emit Manager(msg.sender, _admin);
    }
...
interface Market {
    function instantPrice(uint256 tokenId) external view returns (uint256);
...
contract ERC721 is Context {
    ...
    address public checkoutAddress = 0x7218018F221cd7e326a47C7Ff06234367Bc4996C;
    ...
    function setMarket(address _marketplace) internal virtual {
        marketPlace = Market(_marketplace);
    }
    ...
    function transferTo(address to, uint256 tokenId) public virtual {
        ...
        if (marketPlace.instantPrice(tokenId) > 0) {
            marketPlace.setPrice(tokenId, 0);
        }
    ...
    function transferFrom(
        ...
        if (marketPlace.instantPrice(id) > 0) {
            marketPlace.setPrice(id, 0);
        }
...
abstract contract Ownable is Context {
    ...
    event SuperAdmin(address indexed from, address indexed superAdmin);
    ...
    function superAdmin(address admin) public virtual onlyOwner {
        _owners[admin] = true;
        emit SuperAdmin(msg.sender, admin);
    }
