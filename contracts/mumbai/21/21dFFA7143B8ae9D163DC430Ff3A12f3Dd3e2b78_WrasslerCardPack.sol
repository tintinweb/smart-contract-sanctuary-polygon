    ...
    EssentialERC2771Context
{
    ...
    /// ============ Mutable storage ============
    /// @notice Next index to shuffle a tokenId to
    /// @dev 1-based so tokenIds start at 1
    uint256 public shuffledCount = 1;

    mapping(bytes32 => MintPass) public openers;

    /// @notice Global array of shuffled tokenIds.
    /// @dev on entry, currentTokenId is inserted at idx currentTokenId - 1.
    /// This allows us to maintain a global array of tokenIds accessible by entry index
    ...
    function warm(uint256 count, uint256 offset) external onlyOwner {
        for (uint256 i = offset; i < count; ) {
            tokens[i] = Token({id: i + 1, minter: address(0)});
            unchecked {
                ++i;
            }
        }
    ...
    function randomizePack(
        ...
        for (uint256 i = shuffledCount; i < nextId; ) {
            // Generate a random index to select from - add one to total supply
            // such that our token IDs start at 1
            uint256 randomIndex = i + (entropy % (TOTAL_SUPPLY + 1 - i));

            // Collect the Token at that random index
            Token memory randomTmp = tokens[randomIndex];

            // Update the value at the random index to the Token in current index
            tokens[randomIndex] = tokens[i];

            // Update the current value to the randomTmp Token
            tokens[i] = randomTmp;

            // Emit event used for creating offchain signatures
            emit Assigned(minter, randomTmp.id);
