...
import "../ZK--Circom-Circuit-Generator/verifier.sol";

contract ArrayStorage is PlonkVerifier {
    struct Proof {
        uint256[] proof;
        uint256[] pubSignals;
    }

    mapping(address => Proof) private proofMap;

    function saveProof(uint256[] memory _proof, uint256[] memory _pubSignals) public {
        Proof storage newProof = proofMap[msg.sender];
        newProof.proof = _proof;
        newProof.pubSignals = _pubSignals;
    }

    function getProof(address _address) public view returns (uint256[] memory, uint256[] memory) {
        return (proofMap[_address].proof, proofMap[_address].pubSignals);
    }

    function getVerifiedProof(address _address) public view returns (bool) {
        Proof storage proof = proofMap[_address];
        uint256[] memory proof1 = proof.proof;
        uint256[] memory proof2 = proof.pubSignals;
        return verifyProof(proof1, proof2);
    }
}
...
contract PlonkVerifier {
    ...
    function verifyProof(uint256[] memory _proof, uint256[] memory _pubSignals) public view returns (bool) {
        assembly {
            /////////
