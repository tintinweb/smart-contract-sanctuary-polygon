...
contract Pangea is ERC721,ERC721URIStorage,EIP712 {

    using Counters for Counters.Counter;

    mapping (address => uint256) pendingWithdrawals;

    string private constant SIGNING_DOMAIN = "Pangea";
  ...
  EIP712(SIGNING_DOMAIN, SIGNATURE_VERSION) {}
    ...
    event Minted(uint nftId, address nftOwner,uint time );
    ...
    function redeem(address _minter, address redeemer, NFTVoucher calldata voucher) public payable returns (uint256) {
    ...
    require(_minter == signer, "unauthorized signer"); //minter will be fetched from backedn throug IPFS minter

    require(msg.value >= voucher.minPrice, "Insufficient funds to redeem");

    safeMint(signer, voucher.uri);
