...
contract PerpetualLiquidator is PerpetualRebalanceFunctions, IFunctionList, IPerpetualLiquidator {
    ...
    function liquidateByAMM(
        ...
        virtual
        override
        updateFundingAndPrices(_perpetualIndex, _getPoolIdFromPerpetual(_perpetualIndex))
        returns (int128 fLiquidatedAmount)
    {
        ...
        int128 fPrice;
        int128 fRealizedPnL;
        (fTraderPos, fPrice, fRealizedPnL) = _liquidateAndPenalize(
            perpetual,
            _traderAddr,
            _liquidatorAddr,
            fLiquidatedAmount
        ...
        require(
            _isTraderMarginSafe(perpetual, _traderAddr, false),
            "trader margin unsafe after liquidation"
        );
        {
            int128 newpos = marginAccounts[perpetual.id][_traderAddr].fPositionBC;
            bytes16 posID = marginAccounts[perpetual.id][_traderAddr].positionId;
     ...
     */
    function _liquidateAndPenalize(
        PerpetualData storage _perpetual,
        address _traderAddr,
        address _liquidatorAddr,
        int128 _fLiquidatedAmount
    )
        internal
        returns (
            int128,
        ...
        )
    {
        ...
        fPrice = _getPerpetualMarkPrice(_perpetual);
        {
            ...
            fRealizedPnL = _getTradeLogic().executeTrade(
                _perpetual.id,
                _traderAddr,
                fTraderPos,
                _fLiquidatedAmount,
            ...
            int128 fPenaltyCC;
            (fPenaltyCC, fRemainingMargin) = _payLiquidationPenalty(
                _perpetual,
                _fLiquidatedAmount,
                [_traderAddr, _liquidatorAddr]
            );
            ...
            if (fRemainingMargin.sub(fPenaltyCC) > 0) {
                // regular fees
                fRealizedPnL = fRealizedPnL.sub(
                    _getTradeLogic().distributeFeesLiquidation(
                        _perpetual.id,
                        _traderAddr,
                        _fLiquidatedAmount
                    )
    ...
    /**
     * Pays penalty fees
     * No choice of collateral token for liquidator
     * @param   _perpetual          The reference for the perpetual.
    ...
    function _payLiquidationPenalty(
        ...
        int128 fRemainingMargin = _getMarginViewLogic().getAvailableMargin(
            _perpetual.id,
            _traderAddr,
            false
        );
        ...
        int128 fPenaltyCC;
        {
            ...
            fPenaltyCC = _fLiquidatedAmount.mul(fB2C).abs().mul(
                ConverterDec18.tbpsToABDK(_perpetual.liquidationPenaltyRateTbps)
            );
     ...
     * Derivation see whitepaper. Tau is set to the initial margin rate.
     * No broker fees paid when liquidation occurs.
     ...
     */
    function _getPositionAmountToLiquidate(PerpetualData storage _perpetual, address _traderAddr)
        internal
        view
        returns (int128)
    {
        ...
        int128 m = int128(_perpetual.fInitialMarginRate) << 35;
        if (b0.mul(fS3) > fTraderPos.abs().mul(m).mul(fSm) || fTraderPos == 0) {
            // trader is already initial margin safe or has no position
            return 0;
        }

        int128 fTotalFeeRate = int128(
            uint128(marginAccounts[_perpetual.id][_traderAddr].feeTbps) +
                uint128(_perpetual.liquidationPenaltyRateTbps)
        ) * ConverterDec18.TBPSTOABDK;
        if (b0.mul(fS3) <= fTraderPos.abs().mul(fTotalFeeRate).mul(fS2)) {
            // liquidate total position
            return fTraderPos;
        }
        int128 delta_bc = fTraderPos.abs().mul(m).mul(fSm).sub(b0.mul(fS3)).div(
            m.mul(fSm).sub(fTotalFeeRate.mul(fS2))
        );
        if (fTraderPos < 0) {
     ...
     */
    function _getBaseToQuoteConversionMultiplier(
        PerpetualData storage _perpetual,
