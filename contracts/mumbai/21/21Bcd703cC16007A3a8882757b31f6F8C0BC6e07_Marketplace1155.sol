...
contract Marketplace1155 is Ownable, IMarketplace1155 {
    ...
    mapping(bytes32 => ListedNFT) listedNftsList;
    ...
    {
        ListedNFT memory nft = listedNftsList[listId];
        return (
    ...
    function updateNFT(
        ...
        ListedNFT storage listedNft = listedNftsList[listId];

        require(
            listedNft.status == true,
            "Marketplace: You cannot update an unlisted batch of nfts"
        );
        require(
    ...
    function buyNFT(bytes32 listId) external payable {
        ...
        if (listedNft.currencyContractAddress == address(0)) {
            require(
                ...
                "Marketplace: You dont have enough money to buy these batch of nfts"
            );
...
interface IMarketplace1155 {
    ...
    event UnlistNFT(bytes32 listId);
