...
contract FatChickenContest is Ownable, VRFConsumerBase {
    Corn corn;
    FoxHen foxHen;
    ...
    mapping(bytes32 => Feeding) public feedings;
    ...
    function userChickens(address user) public view returns (Chicken[] memory) {
        uint256[] memory tokenIds = entries[user];
        Chicken[] memory chck = new Chicken[](tokenIds.length);
        for (uint16 i = 0; i < chck.length; i++) {
            chck[i] = chickens[tokenIds[i]];
        }
        return chck;
    }

    function allChickens() public view returns (Chicken[] memory) {
        Chicken[] memory chckns = new Chicken[](participants.length);
        for (uint256 i = 0; i < chckns.length; i++) {
            chckns[i] = chickens[participants[i]];
        }
        return chckns;
    }

    function feedChicken(uint256 id, uint256 amount) public {
        Chicken storage chicken = chickens[id];
        bool isNew = chicken.id == 0;
        require(
            msg.sender == foxHen.ownerOf(id),
            "Only the chicken owner can feed this chicken"
        );
        require(amount > 0, "Amount must be greater than 0");
        corn.burn(msg.sender, amount);

        if (isNew) {
            chicken.id = id;
            chicken.owner = msg.sender;
            chicken.eaten = 0;
            chicken.sick = false;
            entries[msg.sender].push(id);
            participants.push(id);
        }
        if (chicken.owner != msg.sender) {
            chicken.owner = msg.sender;
        }

        bytes32 requestId = requestRandomness(keyHash, fee);
        feedings[requestId] = Feeding(id, amount, block.timestamp);
    }

    function feedingFailed(
        Chicken memory chicken,
        uint256 amount,
        uint256 random
    ) internal pure returns (bool) {
        if (chicken.eaten + amount < 500 ether) {
            return false;
        }
        if (chicken.eaten + amount < 2000 ether) {
            return (random % chicken.eaten) < amount;
        }
        return (random % 4) == 0;
    }
    ...
    function fulfillRandomness(bytes32 _requestId, uint256 _randomness)
        ...
        override
    {
        Feeding storage feeding = feedings[_requestId];
        Chicken storage chicken = chickens[feeding.chickenId];
        require(chicken.id != 0, "Chicken does not exist");
        uint256 newAmount = chicken.eaten + feeding.amount;
        if (records[chicken.id] < newAmount) {
            records[chicken.id] = newAmount;
        }
        if (feedingFailed(chicken, feeding.amount, _randomness)) {
            chicken.sick = true;
            chicken.eaten = 0;
        } else {
            chicken.eaten = newAmount;
        }
    }
}
