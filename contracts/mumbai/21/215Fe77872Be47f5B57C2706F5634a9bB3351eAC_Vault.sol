    ...
    Initializable
{
    ...
    // big dig discount percentage
    uint256 public constant BIG_DIG_DISCOUNT_PCT = 5000;

    // basis odds
    uint16 public constant BASIS_ODDS = 10000;
    // jackpot odds
    uint16 public constant JACKPOT_ODDS = 5;
    // medium win odds
    uint16 public constant MEDIUM_WIN_ODDS = 30;
    // small win odds
    uint16 public constant SMALL_WIN_ODDS = 100;
    // big consolation odds
    uint16 public constant BIG_CONSOLATION_ODDS = 1000;
    // medium consolation odds
    uint16 public constant MEDIUM_CONSOLATION_ODDS = 2000;

    // jackpot percentage (prize pool)
    uint256 public constant JACKPOT_PCT = 5000;
    // medium win percentage (prize pool)
    uint256 public constant MEDIUM_WIN_PCT = 300;
    // small win percentage (prize pool)
    uint256 public constant SMALL_WIN_PCT = 50;
    // medium consolation percentage (price)
    uint256 public constant BIG_CONSOLATION_PCT = 10000;
    // medium consolation percentage (price)
    uint256 public constant MEDIUM_CONSOLATION_PCT = 5000;
    // default: small consolation percentage (price)
    uint256 public constant SMALL_CONSOLATION_PCT = 2500;
    ...
    function calculateRewards(
        ...
        if (random < JACKPOT_ODDS) {
            ...
            oddPct = MEDIUM_WIN_PCT;
            rewards = calculateBpsPercentage(price, oddPct);
        } else if (random < SMALL_WIN_ODDS) {
            oddPct = SMALL_WIN_PCT;
            rewards = calculateBpsPercentage(price, oddPct);
        } else if (random < BIG_CONSOLATION_ODDS) {
            oddPct = BIG_CONSOLATION_PCT;
            rewards = calculateBpsPercentage(price, oddPct);
        } else if (random < MEDIUM_CONSOLATION_ODDS) {
            oddPct = MEDIUM_CONSOLATION_PCT;
            rewards = calculateBpsPercentage(price, oddPct);
        } else {
            oddPct = SMALL_CONSOLATION_PCT;
            rewards = calculateBpsPercentage(price, oddPct);
        }
        return (rewards, oddPct);
    }
    ...
    function dig(
        ...
        (uint256 rewards, uint256 oddPct) = calculateRewards(
            seed.add(id),
            currency,
            price
        );
        ...
        oddsPercentageEvents(id, oddPct, currency, rewards);
