  ...
  ERC1155BurnableUpgradeable
{
  ...
  function closePack(uint256 _packId) external onlyTeamOrOwner {
    require(exists(_packId), "DNE");
    for (uint256 i; i < _packs[_packId].tokens.length; i++) {
      PackItem memory _token = _packs[_packId].tokens[i];
      if (_token.remaining > 0) {
    ...
    _packs[_packId].tokensRemaining = 0;
    _packs[_packId].closed = true;
  }
