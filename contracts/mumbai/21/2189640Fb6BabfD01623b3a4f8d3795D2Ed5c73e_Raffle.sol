    ...
    VRFConsumerBaseV2
{
    ...
    function ticketDiscounts(
        uint32 id
    ) external view returns (TicketDiscount[] memory) {
    ...
    function ticketSale(
        uint32 id,
        uint256 saleId
    ) external view returns (TicketSale memory) {
    ...
    function ticketCounter(
        uint32 id,
        address buyer
    ) external view returns (TicketCounter memory) {
    ...
    function raffleDrawInfo(
        uint32 id
    ) external view returns (RaffleDrawInfo memory) {
    ...
    ) external whenNotPaused onlyRole(ADMIN) returns (uint32) {
        require(
    ...
    function cancel(
        uint32 id
    ) external payable nonReentrant onlySeller(id) ableToEdit(id) {
        require(
    ...
    function requestRefund(
        uint32 id
    ) external onlySeller(id) onlyOnStatus(id, Status.Created) {
        _raffles[id].status = Status.RefundRequested;
    ...
    function approveRefund(
        uint32 id
    ) external onlyRole(ADMIN) onlyOnStatus(id, Status.RefundRequested) {
        _raffles[id].status = Status.RefundApproved;
    ...
    ///@param id the id of the raffle
    ///@return value total number of ticket sales.
    function ticketSaleTotalAmount(
        uint32 id
    ...
    function changeStatusEmergency(
        uint32 id,
        Status status
    ) external onlyRole(ADMIN) {
    ...
    /// @param id The id of the raffle.
    /// @param addresses the address that will receive the free tickets.
    function issueFreeTickets(
        uint32 id,
        address[] calldata addresses
    ) external payable onlyRole(BACKEND) ableToSell(id) {
        require(addresses.length > 0, "Raffle: Empty array");
        uint256 price = _raffles[id].pricePerTicket;
        uint256 totalPrice = price * addresses.length;
        require(msg.value == totalPrice, "Raffle: Invalid value");

        // Verify the tickets is enough for this buyer.
        if (_raffles[id].maxTicketsNum > 0) {
            require(
                _ticketSaleTotalCount(id) + addresses.length <=
                    _raffles[id].maxTicketsNum,
                "Raffle: Insufficient tickets"
            );
        }
        ...
        for (uint32 i = 0; i < addresses.length; i++) {
            _ticketSales[id].push(
                TicketSale({
                    isRefunded: false,
                    buyer: addresses[i],
                    cumulativeSoldCount: _ticketSaleTotalCount(id) + 1,
                    soldAmount: price,
                    cumulativeSoldAmount: _ticketSaleTotalAmount(id) + price
                })
            );

            _ticketCounter[id][addresses[i]].totalSoldCount += 1;
            emit FreeTicketSent(id, addresses[i], 1, price);
        }
    }
    ...
    function recycleRaffleNoClaim(
        uint32 id,
        address payable receiver
    ) external nonReentrant onlyRole(ADMIN) onlyOnStatus(id, Status.Drawn) {
        RaffleInfo storage raffle = _raffles[id];
        require(
