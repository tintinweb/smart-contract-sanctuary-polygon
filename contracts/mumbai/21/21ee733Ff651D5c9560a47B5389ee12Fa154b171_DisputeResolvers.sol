...
contract DisputeResolvers is Mappings, IDisputeResolution {
    ...
    function stake() external override returns (bool) {
        ...
        if (juryVersion[tx.origin] == 0) juryVersion[tx.origin] = IConfig(config).getLatestVersion();
        uint256 amount;
    ...
    function withdraw() external override returns (bool) {
        ...
        IERC20(dbethAddress).transfer(admin,(usersStake[tx.origin] - amount_));
        ...
        returns (uint256)
    {
        uint256 amount;
        amount =
                (IConfig(config).getJuryTokensShare(juryStrike[user_], juryVersion[tx.origin]) *
                    userInitialStake[tx.origin]) /
                1e2;
        return (amount);
    }
    ...
    function adminWithdrawal(address user_) external override returns (bool status_) {
        uint256 amount_ = calculateStrikeAmount(user_);
        IERC20(dbethAddress).transfer(admin,(usersStake[user_] - amount_));
