...
contract UnderWriter_Covest is Initializable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
	...
	function isUnderWriter(address _user) public view returns (bool) {
		IStaking.StakingData memory _StakingData = S.stakingData(_user, IStaking.StakingTypes.UW);
		IStaking.WorkType memory _dataType = S.workData(IStaking.StakingTypes.UW);
	...
	function withdraw(
		...
		IERC20 _ERC20 = IERC20(_asset);
		require(_ERC20.balanceOf(address(this)) >= _value, "Balance of Contract Not Enough.");
		require(PDS.isBlacklistUser(_worker) != true, "Policy: This worker is Blacklist.");
		require(PDS.isBlacklistAssetsUser(_worker, _asset) != true, "Policy: This Assets of worker is Blacklist.");
		require(isUnderWriter(_worker) == true, "This user Doesn't Underwriter.");
