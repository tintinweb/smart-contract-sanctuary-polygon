...
// File: contracts/20230617SEKINTERFACE.sol
...
interface IERC721Burnable {
    ...
    function tokenOfOwnerByIndex(address _ad, uint256 id) external returns(uint256);
}
...
contract SEKINTERFACE is ERC721Enumerable, Ownable {

  
  using Strings for uint256;

  //state 0 = PR/PRIVATE MINT; 1=public sale
  ...
  event PRiniti(address payable indexed _PR);
  event whitelistevent(address whitelisted);
  event welcomePRevent( address  payable indexed indPr,  address indexed welcome1, address indexed welcome2);
  event welcomeUserevent( address payable indexed indUser , address indexed welcome1, address indexed welcome2);
  event welcome_PR_reward(address  payable indexed indPr, uint256 indexed minted, uint256 new_reward);
  event _fnic1(address __fnic1);
  event changestate(uint256 state);
  event message (string message);
  event minted(bool minted); 
  event burned (address _sender, uint256 ID);

 // event welcome_user_reward(address payable indexed indUser, bool friend1mint, bool friend2mint, uint256 new_reward);
  ...
  //set fnic
  function setfinc1(address _fnic) public onlyOwner{
    fnic1 = FNIC1( _fnic);
    emit _fnic1(_fnic);
...
/*function test_dalegate_burn3(uint256 _ID) public returns (bool){   //burn_2(address from, uint256 tokenId, address tokenowner) 
          (bool success, bytes memory trans) = address(fnic1).delegatecall(abi.encodeWithSignature("burn_2(address, uint256, address)", address(msg.sender), _ID, address(msg.sender)));
          return(success);
}*/


/*function test_dalegate_burn(address contract_ , uint256 _ID) public {
         IERC721Burnable burn_contract = IERC721Burnable(contract_);
         burn_contract.burn(_ID);
               
}*/

/*function test_delegate_number(address contract_ ,uint256 num) public returns(bool){
            (bool success, bytes memory trans) = contract_.delegatecall{gas: 1000000}(abi.encodeWithSignature("setnumber(uint256)",num));
            return (success);

}*/

/*function test_burn(address contract_ , uint256 _ID) public payable returns(bool){
         IERC721Burnable burn_contract = IERC721Burnable(contract_);
         //burn_contract.burn(_ID);
   
         (bool burnsuccess, bytes memory burndata) = address(burn_contract).call{value: msg.value, gas: 1000000}(
            abi.encodeWithSignature(" burn(uint256)", _ID));
               
}*/

/*function test_burn2(address contract_ , uint256 _ID) public payable returns(bool){
        IERC721Burnable burn_contract = IERC721Burnable(contract_);
        // burn_contract.burn(_ID);
        bool burn_success = false;
   
        try burn_contract.burn(_ID) {
          
          emit message ("burn did it");
          burn_success = true;
          return(burn_success);
        } catch{

            emit message ("burn doesn't work");
            burn_success = false;
            return(burn_success);
...
function test_burn3() public payable returns(bool){
        IERC721Burnable burn_contract = IERC721Burnable(address(fnic1));
        uint256 _ID =  burn_contract.tokenOfOwnerByIndex(msg.sender, 0);
       // burn_contract.burn(_ID);
        bool burn_success = false;
...
function testburnmint(uint256 _mintAmount,  address whitelisted1, address whitelisted2)public payable returns(bool){
      require(msg.value >= cost , "you need to pay more for this nft");
      uint256 supply = totalSupply();
      bool success=false;



      // IF not PR BUT HOLD FNIC1 -> BURN AND BECOME PR
      if(isPR(payable(msg.sender))==false){
          require(fnic1.balanceOf(msg.sender)>0 , "not FNIC1");
            uint256 ID_ =  fnic1.tokenOfOwnerByIndex(msg.sender,0);
            IERC721Burnable burn_contract = IERC721Burnable(address(fnic1));
            uint256 _ID =  burn_contract.tokenOfOwnerByIndex(msg.sender, 0);
          // burn_contract.burn(_ID);
            bool burn_success = false;
            ...
            try burn_contract.burn(_ID) {
              ...
              emit message("burned good");
            } catch{

                emit message ("burn doesn't work");
                burn_success = false;
                return(burn_success);
                emit message("dint' burn");

            }

      require(burn_success = true, "You cannot burn the FNIC1");

      (bool mintsuccess, bytes memory data) = address(this).call{value: cost, gas: 1000000}(
                      abi.encodeWithSignature(" mint(uint256 , address,  address)", _mintAmount, whitelisted1, whitelisted2));
        
      //mint(_mintAmount,  whitelisted1, whitelisted2);
    
      return (mintsuccess);


    ////////////////////////////////////////// MINT IF YOU ARE A PR /////////////////////////////////////////
  }if(isPR(payable(msg.sender))==true){
...
/*function mint1(uint256 _mintAmount,  address whitelisted1, address whitelisted2) public payable {
   // I don't set require here because also after this step you can always mint
  uint256 supply = totalSupply();
  bool success=false;
   // IF not PR BUT HOLD FNIC1 -> BURN AND BECOME PR
   if(isPR(payable(msg.sender))==false){
      require(fnic1.balanceOf(msg.sender)>0 , "not FNIC1");
        uint256 ID_ =  fnic1.tokenOfOwnerByIndex(msg.sender,0);
       // (bool success, bytes memory trans) = address(fnic1).delegatecall(abi.encodeWithSignature("burn_2(address, uint256, address)",msg.sender, ID_, msg.sender));

        //fnic1.burn_2(msg.sender, ID_, msg.sender)
       /* {
        success = true;
        }catch{
          success = false;
     ...
     // require(success == true, "burn doesn't work");
      uint256 len = PR.length;
      PR[len+1] = payable (msg.sender); ///verifica se usare la funzione mint con PR perchè non so se lo aggiunge effettivamente come PR prima del mint
      require(!paused, "the contract is paused");
   ...
   // IF PR AND MINT<5 mint and whitelist
   // IF you are whitelisted -> mint and give money to PR
 ...
 function mint2(uint256 _mintAmount) public payable {
   uint256 supply = totalSupply();
   require(state==1, "Wait the public the sale");
  // everyone can mint
  ...
  // public
 function mint(uint256 _mintAmount,  address whitelisted1, address whitelisted2) internal {
    require(!paused, "the contract is paused");
    uint256 supply = totalSupply();
    require(_mintAmount > 0, "need to mint at least 1 NFT");
    require(_mintAmount <= maxMintAmount, "max mint amount per session exceeded");
    ...
    if (msg.sender != owner()) {
      if(onlyWhitelisted == true) {
           ...
           if(isPR(payable (msg.sender))){
            ...
            emit welcomePRevent(payable(msg.sender), whitelisted1, whitelisted2);
           }
     ...
     if (msg.sender != owner()) {
            ...
            emit welcomeUserevent(payable(msg.sender), whitelisted1, whitelisted2);*/
           ...
           // inviter[whitelisted1] = msg.sender;
            ...
            if(isPR(payable(inviter[msg.sender])) == false){
                  ...
                  if(welcome[inviter[msg.sender]].friend1 == msg.sender){
                   ...
                   emit  welcome_user_reward(payable (inviter[msg.sender]),  true,false,  temp_money + ref_rew);
                  }
                  if(welcome[inviter[msg.sender]].friend2 == msg.sender){
                    welcome[inviter[msg.sender]].mintfriend2 = true;
                    emit welcome_user_reward(payable (inviter[msg.sender]),  false, true,  temp_money + ref_rew);
                   }
            }*/
            ...
            if(isPR(payable(inviter[msg.sender])) == true){
                  ...
                  emit welcome_PR_reward(payable(inviter[msg.sender]), welcome_PR[inviter[msg.sender]].minted , _temp_money + ref_rew);
...
/*    if(isPR(payable(msg.sender)) == false){
          ...
          emit welcome_user_reward(payable(msg.sender), false, false, 0);*/
   ...
   // } else if (isPR(payable (msg.sender))){
          ...
          emit welcome_PR_reward(payable( msg.sender), welcome_PR[ (msg.sender)].minted , 0);
   // }
