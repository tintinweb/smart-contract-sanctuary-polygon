...
contract AirdropQatar is Ownable, ReentrancyGuard {
    ...
    function claimAllAirdrop(uint256 _cid, uint256[] memory _tokenIds) public nonReentrant { 
        address _sender = msg.sender;
        uint256 _total;
        for (uint i = 0; i < _tokenIds.length; i++) {
            ...
            emit Claimed(_cid, _sender, _total);
    }
...
contract Whitelist is IWhitelist {
    ...
    function _addWhitelisted(address account, uint256 amount, uint256 tokenId) internal {
        ...
        require(qatarWorldCup.ownerOf(tokenId) == account, "Whitelist: Invalid owner token");
        WhitelistInfo memory whitelistInfo = WhitelistInfo(tokenId,amount, true, false);
