...
contract Classifica {
    ...
    function getSquadre() public view returns (Squadra[] memory) {
    Squadra[] memory result = new Squadra[](listaSquadre.length);
    for (uint i = 0; i < listaSquadre.length; i++) {
        Squadra storage s = squadre[listaSquadre[i]];
        result[i] = s;
    }
    return result;
}
