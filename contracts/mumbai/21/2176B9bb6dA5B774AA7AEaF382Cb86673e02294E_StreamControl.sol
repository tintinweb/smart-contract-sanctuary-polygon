...
import {LibStreamControl} from "../../libraries/core/LibStreamControl.sol";
...
contract StreamControl is IStreamControl {
    ...
    function flowCreateAndScheduleDelete(
        ...
        if (!LibStreamControl._storageStreamControl().superTokens[_superToken])
        ...
        LibStreamControl._storageStreamControl()._cfaLib.createFlowByOperator(
            msg.sender,
            _receiver,
            _superToken,
            _flowRateReceiver
        ...
        // LibStreamControl._storageStreamControl().taskIds[msg.sender].push(
        //     taskId
    ...
    function deleteFlow(
        ...
        LibAutomate._requireOnlyGelatoOpsOrParticipants(_sender, _receiver); // TODO: besides gelato ops, add creator of flow (sender) and receiver of flow

        LibStreamControl._storageStreamControl()._cfaLib.deleteFlowByOperator(
            _sender,
            _receiver,
            _superToken
        );
...
library LibStreamControl {
    using CFAv1Library for CFAv1Library.InitData;

    bytes32 constant STORAGE_POSITION_STREAMCONTROL =
        keccak256("ds.streamcontrol");

    struct StorageStreamControl {
        ISuperfluid sfHost;
    ...
    function _storageStreamControl()
        internal
        pure
        returns (StorageStreamControl storage s)
    {
        bytes32 position = STORAGE_POSITION_STREAMCONTROL;
        assembly {
            s.slot := position
        }
    }
    ...
    function _setConstantFlowAgreement(ISuperfluid _sfHost) internal {
        ...
        _storageStreamControl()._cfaLib = CFAv1Library.InitData(_sfHost, cfa);
        _storageStreamControl().sfHost = _sfHost;
    }

    function _getSuperfluidAddresses() internal view returns (address) {
        return address(_storageStreamControl().sfHost);
    }

    function _appendSuperToken(ISuperToken _superToken) internal {
        // TODO: add only access - in facet
        _storageStreamControl().superTokens[_superToken] = true;
    }

    function _removeSuperToken(ISuperToken _superToken) internal {
        // TODO: add only access - in facet
        delete _storageStreamControl().superTokens[_superToken];
    }
    ...
    function _hasSupportedSuperToken(ISuperToken _superToken)
        ...
        returns (bool)
    {
        return _storageStreamControl().superTokens[_superToken];
    }
}
