...
contract ApePayV1 is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    function verifyRecipient(
        uint128 nonce_,
        string memory baseMsg,
        bytes memory signature
    ) public view returns(PaymentDetail memory) {
        // verify payment details
        // verify signature
        string memory fullMsg = getMessageTemplate(baseMsg, nonce_);
        bytes32 ethMessage = ECDSA.toEthSignedMessageHash(bytes(fullMsg));
        address paymentSender = ECDSA.recover(ethMessage, signature);
        PaymentDetail memory paymentDetail = Payments[nonce_];
        // if the signature is correct, we should recover the sender address
        require(paymentSender == paymentDetail.sender, "AP: not sender");
        require(paymentDetail.claimed == false, "AP: payment claimed already");
    ...
    function receivePayment(
        ...
        _receivePayment(nonce_, baseMsg, signature, msg.sender);
    }
    
    // receive payment from gas payer on behalf of recipient
    /// @param nonce_ nonce of payment
    /// @param baseMsg a base message including all payment metadata except nonce
    ...
    function receivePaymentDelegate(
        ...
        _receivePayment(nonce_, baseMsg, senderSignature, recipient);
    }

    // function receivePaymentV2(
    //     uint128 nonce_,
    //     bytes memory message,
    //     bytes memory signature
    // ) external nonReentrant {
    //     PaymentDetail memory paymentDetail = verifyRecipientV2(nonce_, message, signature);
    //     // send fund
    //     sendFund(paymentDetail, msg.sender);

    //     emit PaymentReceived(msg.sender, nonce_);
    // }
    ...
    function _receivePayment(
        ...
        PaymentDetail memory paymentDetail = verifyRecipient(nonce_, baseMsg, signature);
        // send fund
    ...
    function getMessageTemplate(
        string memory baseMessage,
        uint128 nonce_
    ) public pure returns(string memory) {
        string memory message = string.concat(baseMessage, Strings.toString(nonce_));
        return message;
    }
}
