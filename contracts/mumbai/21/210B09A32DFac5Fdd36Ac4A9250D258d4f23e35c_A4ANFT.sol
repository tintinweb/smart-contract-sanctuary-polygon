...
contract A4ANFT is ERC721URIStorage, Ownable {
    ...
    mapping (uint256 => uint256) private _firstA4aFee;
    mapping (uint256 => uint256) private _nextA4aFee;
    mapping (uint256 => uint256) private _firstSaleFee1;
    mapping (uint256 => uint256) private _nextSalesFee1;
    mapping (uint256 => address) private _saleReceiver1;
    mapping (uint256 => uint256) private _firstSaleFee2;
    mapping (uint256 => uint256) private _nextSalesFee2;
    mapping (uint256 => address) private _saleReceiver2;
    mapping (uint256 => string[]) private _pastItemMetadata;

    event ItemCreated(uint256 itemID, address receiver, string tokenURI, uint256 authorFee_);
    event ItemMetadataUpdated(uint256 itemID, uint256 order, string newtokenURI, string lastTokenURI);
    ...
    function createNFT(
      ...
      uint256 authorFee_,
      uint256 firstA4aFee_,
      uint256 nextA4aFee_,
      uint256 firstSaleFee1_,
      uint256 nextSalesFee1_,
      address saleReceiver1_,
      uint256 firstSaleFee2_,
      uint256 nextSalesFee2_,
      address saleReceiver2_
    )
        public
        onlyOwner
        returns (uint256)
    {
        ...
        _firstA4aFee[newItemId]   = firstA4aFee_;
        _nextA4aFee[newItemId]    = nextA4aFee_;
        _firstSaleFee1[newItemId] = firstSaleFee1_;
        _nextSalesFee1[newItemId] = nextSalesFee1_;
        _saleReceiver1[newItemId] = saleReceiver1_;
        _firstSaleFee2[newItemId] = firstSaleFee2_;
        _nextSalesFee2[newItemId] = nextSalesFee2_;
        _saleReceiver2[newItemId] = saleReceiver2_;
    ...
    function pastItemMetadata(uint256 itemID) external view returns (string[] memory) {
        return (_pastItemMetadata[itemID]);
    }

    function updateItemMetadata(uint256 itemID, string memory newTokenURI) external onlyOwner {
      require(_exists(itemID), "ERC721: operator query for nonexistent token");
      string memory lastTokenURI = tokenURI(itemID);
      _pastItemMetadata[itemID].push(lastTokenURI);

      _setTokenURI(itemID, newTokenURI);
      emit ItemMetadataUpdated(itemID, _pastItemMetadata[itemID].length, newTokenURI, lastTokenURI);

    }


    function _isAllowedToTransfer(uint256 itemID, address from) public view returns (bool) {

        return (_allowedToTransfer[itemID][from]);
    }
    ...
    function firstA4aFee(uint256 tokenId) external view returns(uint256) {
      return _firstA4aFee[tokenId];
    }

    function setFirstA4aFee(uint256 tokenId, uint256 fee) external onlyOwner {
      _firstA4aFee[tokenId] = fee;
    }

    function nextA4aFee(uint256 tokenId) external view returns(uint256) {
      return _nextA4aFee[tokenId];
    }

    function setNextA4aFee(uint256 tokenId, uint256 fee) external onlyOwner {
      _nextA4aFee[tokenId] = fee;
    }

    function firstSaleFee1(uint256 tokenId) external view returns(uint256) {
      return _firstSaleFee1[tokenId];
    }

    function setFirstSaleFee1(uint256 tokenId, uint256 fee) external onlyOwner {
      _firstSaleFee1[tokenId] = fee;
    }

    function nextSalesFee1(uint256 tokenId) external view returns(uint256) {
      return _nextSalesFee1[tokenId];
    }

    function setNextSalesFee1(uint256 tokenId, uint256 fee) external onlyOwner {
      _nextSalesFee1[tokenId] = fee;
    }

    function saleReceiver1(uint256 tokenId) external view returns(address) {
      return _saleReceiver1[tokenId];
    }

    function setSaleReceiver1(uint256 tokenId, address receiver) external onlyOwner {
      _saleReceiver1[tokenId] = receiver;
    }

    function firstSaleFee2(uint256 tokenId) external view returns(uint256) {
      return _firstSaleFee2[tokenId];
    }

    function setFirstSaleFee2(uint256 tokenId, uint256 fee) external onlyOwner {
      _firstSaleFee2[tokenId] = fee;
    }

    function nextSalesFee2(uint256 tokenId) external view returns(uint256) {
      return _nextSalesFee2[tokenId];
    }

    function setNextSalesFee2(uint256 tokenId, uint256 fee) external onlyOwner {
      _nextSalesFee2[tokenId] = fee;
    }

    function saleReceiver2(uint256 tokenId) external view returns(address) {
      return _saleReceiver2[tokenId];
    }

    function setSaleReceiver2(uint256 tokenId, address receiver) external onlyOwner {
      _saleReceiver2[tokenId] = receiver;
    }
 ...
 * {ERC721Enumerable}.
 */
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata { //IERC721
