...
contract Prospecting is OwnableUpgradeable, ReentrancyGuardUpgradeable, IProspector {
    ...
    uint256 public s_prospectorMaxOrders;
    ...
    mapping(address => Order[]) new_prospectingOrders;
    ...
    function placeProspectingOrders(uint256 _numOrders) external nonReentrant {
        Order[] memory orders = new_prospectingOrders[msg.sender];
        ...
        if (s_prospectingTime > 0) {
            for (uint256 i; i < _numOrders; ) {
                ...
                nextOrderId++;
                new_prospectingOrders[msg.sender].push(Order({
                    orderId: _orderId,
                    orderAmount: 1,
    ...
    function claimBatchOrder() external override {
        ...
        for(uint i=orders.length; i > 0; i--) {
            if ((orders[i-1].createdAt + orders[i-1].totalCompletionTime) < block.timestamp) {
                _requestBiomod();
                emit ProspectingOrderCompleted(msg.sender, orders[i-1].orderId);
                _removeOrder(i-1);
            }
        }
    }
    ...
    function _removeOrder(uint _index) internal {
        if (_index < new_prospectingOrders[msg.sender].length -1)
            new_prospectingOrders[msg.sender][_index] = new_prospectingOrders[msg.sender][new_prospectingOrders[msg.sender].length-1];
        new_prospectingOrders[msg.sender].pop();
    }
    ...
    /// @inheritdoc ITrader
    function getOrders(address _player) external view returns (Order[] memory){
        return new_prospectingOrders[_player];
    }
}
