...
import {IWebshopNFTRewards} from "./interface/IWebshopNFTRewards.sol";

contract WebshopNFTRewards is
    ERC1155,
    ERC2981,
    IWebshopNFTRewards,
    ...
    AccessControl
{
    ...
    event ShopCreated(uint256 itemId, bool enabled, bool exist);
    ...
    mapping(uint256 => ShopItem) private shopItems;
    ...
    function _mintItem(
        ...
        shopItems[_itemId].minted = item.minted + _quantity;
    ...
    function getCollectionInfo(
        uint256 itemId
    ) external view onlyExistingItem(itemId) returns (ShopItem memory) {
    ...
    function enableCollection(
        ...
        require(!shopItems[_id].enabled, "Collection already enabled");
        shopItems[_id].enabled = true;
    }

    function disableCollection(
        uint256 _id
    ) external onlyExistingItem(_id) onlyHasRole(ADMIN_ROLE) {
        require(shopItems[_id].enabled, "Collection already disabled");
        shopItems[_id].enabled = false;
    }
    ...
    function _create(bool enabled) private {
        ...
        shopItems[itemId] = ShopItem({
            minted: 0,
            enabled: enabled,
            exist: true
        });

        emit ShopCreated({itemId: itemId, enabled: enabled, exist: true});
    }
