...
contract NftContract is ERC721Enumerable,Ownable {
    ...
    function _setURI(uint256 _id, string memory _uri) internal {
        _tokenURI[_id] = string(abi.encodePacked(_uri, _id.toString(),".json"));
    }
    ...
    function mintNft(uint _quantity) external onlyOwner payable{
        ...
        for (uint256 i = 0; i < _quantity; i++) {
            ...
            string memory temp_uri = string(abi.encodePacked(_baseURI(), totalMinted.toString(),".json"));
            _setURI(totalMinted, temp_uri);

            //FROZEN CONTRACT ---------  _setPermanentURI(_tokenIds.current(), temp_uri);
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function _initialMint(address to) internal virtual {
        _balances[to] = 400;
