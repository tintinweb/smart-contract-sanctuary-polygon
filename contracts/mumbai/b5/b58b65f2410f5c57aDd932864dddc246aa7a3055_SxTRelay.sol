...
contract SxTRelay is ConfirmedOwner, String, Initializable, ReentrancyGuard, ChainlinkClient, ISxTRelay {
     ...
     */
    function requestQueryString2D(string memory query, string memory resourceId, address callerContract, bytes4 callbackFunctionId, string memory chainlinkJobId) external override checkAllowance(callerContract, FEE) nonReentrant returns (bytes32) {
        require(LinkTokenInterface(chainlinkTokenAddress()).transferFrom(callerContract, address(this), FEE), "SxTRelay: Insufficient LINK tokens");
     ...
     * @param chainlinkJobId - Chainlink Job ID that SxT team has created on the Chainlink Oracle Node
     */
    function requestQueryUint256(string memory query, string memory resourceId, address callerContract, bytes4 callbackFunctionId, string memory chainlinkJobId) external override checkAllowance(callerContract, FEE) nonReentrant returns (bytes32) {
        require(LinkTokenInterface(chainlinkTokenAddress()).transferFrom(callerContract, address(this), FEE), "SxTRelay: Insufficient LINK tokens");
        Chainlink.Request memory request = buildChainlinkRequest(
            stringToBytes32(chainlinkJobId),
            callerContract,
     ...
     * @param chainlinkJobId - Chainlink Job ID that SxT team has created on the Chainlink Oracle Node
     */
    function requestQueryBytes(string memory query, string memory resourceId, address callerContract, bytes4 callbackFunctionId, string memory chainlinkJobId) external override checkAllowance(callerContract, FEE) nonReentrant returns (bytes32) {
        require(LinkTokenInterface(chainlinkTokenAddress()).transferFrom(callerContract, address(this), FEE), "SxTRelay: Insufficient LINK tokens");
        Chainlink.Request memory request = buildChainlinkRequest(
            stringToBytes32(chainlinkJobId),
            callerContract,
     ...
     * @param chainlinkJobId - Chainlink Job ID that SxT team has created on the Chainlink Oracle Node
     */
    function requestQueryString(string memory query, string memory resourceId, address callerContract, bytes4 callbackFunctionId, string memory chainlinkJobId) external override checkAllowance(callerContract, FEE) nonReentrant returns (bytes32) {
        require(LinkTokenInterface(chainlinkTokenAddress()).transferFrom(callerContract, address(this), FEE), "SxTRelay: Insufficient LINK tokens");
        Chainlink.Request memory request = buildChainlinkRequest(
            stringToBytes32(chainlinkJobId),
            callerContract,
    ...
    /**
     * @dev Execute the request query
     * @param viewName - View ID requested by User contract
     * @param callerContract - User contract address that called this requestQuery function
     * @param callbackFunctionId - User contract callback function selector id
     * @param chainlinkJobId - Chainlink Job ID that SxT team has created on the Chainlink Oracle Node
     */
    function requestViewString2D(string memory viewName, address callerContract, bytes4 callbackFunctionId, string memory chainlinkJobId) external override checkAllowance(callerContract, FEE) nonReentrant returns (bytes32) {
        require(LinkTokenInterface(chainlinkTokenAddress()).transferFrom(callerContract, address(this), FEE), "SxTRelay: Insufficient LINK tokens");
        Chainlink.Request memory request = buildChainlinkRequest(
            stringToBytes32(chainlinkJobId),
            callerContract,
     ...
     * @param chainlinkJobId - Chainlink Job ID that SxT team has created on the Chainlink Oracle Node
     */
    function requestViewUint256(string memory viewName, address callerContract, bytes4 callbackFunctionId, string memory chainlinkJobId) external override checkAllowance(callerContract, FEE) nonReentrant returns (bytes32) {
        require(LinkTokenInterface(chainlinkTokenAddress()).transferFrom(callerContract, address(this), FEE), "SxTRelay: Insufficient LINK tokens");
        Chainlink.Request memory request = buildChainlinkRequest(
            stringToBytes32(chainlinkJobId),
            callerContract,
     ...
     * @param chainlinkJobId - Chainlink Job ID that SxT team has created on the Chainlink Oracle Node
     */
    function requestViewBytes(string memory viewName, address callerContract, bytes4 callbackFunctionId, string memory chainlinkJobId) external override checkAllowance(callerContract, FEE) nonReentrant returns (bytes32) {
        require(LinkTokenInterface(chainlinkTokenAddress()).transferFrom(callerContract, address(this), FEE), "SxTRelay: Insufficient LINK tokens");
        Chainlink.Request memory request = buildChainlinkRequest(
            stringToBytes32(chainlinkJobId),
            callerContract,
     ...
     * @param chainlinkJobId - Chainlink Job ID that SxT team has created on the Chainlink Oracle Node
     */
    function requestViewString(string memory viewName, address callerContract, bytes4 callbackFunctionId, string memory chainlinkJobId) external override checkAllowance(callerContract, FEE) nonReentrant returns (bytes32) {
        require(LinkTokenInterface(chainlinkTokenAddress()).transferFrom(callerContract, address(this), FEE), "SxTRelay: Insufficient LINK tokens");
        Chainlink.Request memory request = buildChainlinkRequest(
            stringToBytes32(chainlinkJobId),
            callerContract,
     ...
     */
    modifier checkAllowance(address callerContract, uint amount) {
