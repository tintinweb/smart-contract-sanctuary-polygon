...
contract BTCLDynamicLottery is AutomationCompatible, AutomatedRandomness, ReentrancyGuard {
    ...
    uint256 public maxPlayers;   // editable if game is paused
    uint256 public totalWinners; // editable if game is paused
     ...
     * @dev If high demand you will buy a ticket in a future round
     */
    function buyTicket() public payable {
        purchaseTickets(selectRound());
    }

    function selectRound() private returns (uint256) {
        ...
        while (rounds[currentRound].status.totalBets >= maxPlayers || rounds[currentRound].contributed[msg.sender] > 0) {
            ...
            rounds[currentRound].status.roundStatus = BTCLCore.Status.Open;
        }
        return currentRound;
    }
    ...
    function purchaseTickets(uint roundNr) private {
        ...
        if(paused == true && rounds[roundNr].status.totalBets == 0) revert BTCLCore.LOTTERY_PAUSED();

        // Check if the round is open, if not, revert the transaction
        if(rounds[roundNr].status.roundStatus != BTCLCore.Status.Open) revert BTCLCore.TRANSFER_FAILED();
    ...
    function checkUpkeep(bytes calldata /* checkData */) external view override returns (bool upkeepNeeded, bytes memory performData) {
        uint256 numberOfRoundsToDraw = 0;
        uint256[] memory roundsToProcess = new uint256[](maxRounds);

        for (uint256 i = nextRound; i < nextRound + maxRounds; i++) {
            if (rounds[i].status.totalBets >= maxPlayers && rounds[i].status.roundStatus == BTCLCore.Status.Open) {
                numberOfRoundsToDraw++;
                roundsToProcess[numberOfRoundsToDraw - 1] = i;
            }
        }

        upkeepNeeded = numberOfRoundsToDraw > 0;
        performData = abi.encode(roundsToProcess);
    }


    mapping(uint256 => uint256) private pendingRequests;
    ...
    function performUpkeep(bytes calldata performData) external nonReentrant override {
        ...
        require(roundsToProcess.length > 0, "No rounds to draw");

        uint256 totalNumWords = totalWinners * maxRounds;
        require(totalNumWords <= 200, "Too many numWords for a single VRF request");

        uint256[] memory requestIds = new uint256[](maxRounds);

        // Request VRF numbers for each batch of rounds
        for (uint256 i = 0; i < maxRounds; i++) {
            uint256 startIndex = i * totalWinners;
            uint256[] memory indices = new uint256[](totalWinners);

            // Calculate the indices of the winners for this batch of rounds
            for (uint256 j = 0; j < totalWinners; j++) {
                indices[j] = startIndex + j;
            }

            // Request VRF numbers for this batch of rounds
            requestIds[i] = requestRandomness(uint32(totalNumWords));
        }

        // Loop through all rounds that need to be drawn and set the requestId
        for (uint256 i = 0; i < roundsToProcess.length; i++) {
            uint256 roundNr = roundsToProcess[i];
            rounds[roundNr].status.roundStatus = BTCLCore.Status.Drawing;
            rounds[roundNr].status.requestId = requestIds[roundNr % maxRounds];
            emit BTCLCore.LotteryClosed(roundNr, rounds[roundNr].status.totalTickets, rounds[roundNr].status.totalBets);
        }
    }

    function fulfillRandomWords(uint256 requestId, uint256[] memory randomness) internal override {
        // Loop through all rounds that need to be drawn
        for (uint256 i = 0; i < maxRounds; i++) {
            uint256 roundNr = nextRound + i;

            if (rounds[roundNr].status.requestId == requestId && rounds[roundNr].status.roundStatus == BTCLCore.Status.Drawing) {
                rounds[roundNr].status.roundStatus = BTCLCore.Status.Completed;

                // Copy 3 elements from the randomness array to the storage array for the current round
                uint256 startIndex = i * totalWinners;
                for (uint256 j = 0; j < totalWinners; j++) {
                    rounds[roundNr].status.randomness[j] = randomness[startIndex + j];
                }

                emit BTCLCore.LotteryOpened(roundNr);
            }
        }

        // Increment the next round if all rounds have been drawn
        if (nextRound + maxRounds == round + 1) {
            nextRound += maxRounds;
            round++;
            rounds[round].status.roundStatus = BTCLCore.Status.Open;
    ...
    //     // Request VRF number for all rounds that need to be drawn
    //     uint256 numWords = totalWinners * maxRounds; // Request 3 numWords per round winner for all rounds
    //     uint256 requestId = requestRandomness(uint32(numWords));

    //     // Loop through all rounds that need to be drawn and set the requestId
    //     for (uint256 i = 0; i < roundsToProcess.length; i++) {
    //         uint256 roundNr = roundsToProcess[i];
    //         rounds[roundNr].status.roundStatus = BTCLCore.Status.Drawing;
    //         rounds[roundNr].status.requestId = requestId;
    //         emit BTCLCore.LotteryClosed(roundNr, rounds[roundNr].status.totalTickets, rounds[roundNr].status.totalBets);
    //     }
    // }
    ...
    //             // Copy 3 elements from the randomness array to the storage array for the current round
    //             uint256 startIndex = i * 3;
    //             rounds[roundNr].status.randomness[0] = randomness[startIndex];
    //             rounds[roundNr].status.randomness[1] = randomness[startIndex + 1];
    //             rounds[roundNr].status.randomness[2] = randomness[startIndex + 2];
    ...
    // function performUpkeep(bytes calldata performData) external nonReentrant override {
    //     uint256 numberOfRoundsToDraw = abi.decode(performData, (uint256));
    //     require(numberOfRoundsToDraw > 0, "No rounds to draw");

    //     // Request VRF number for all rounds that need to be drawn
    //     uint256 maxRoundsToDraw = 10;
    //     for (uint256 i = round; i < round + maxRoundsToDraw && i <= round + 100; i++) { // Limiting to draw a maximum of 10 rounds, but not exceeding 100 rounds in total
    //         if (rounds[i].status.totalBets >= maxPlayers && rounds[i].status.roundStatus == BTCLCore.Status.Open) {
    //             rounds[i].status.roundStatus = BTCLCore.Status.Drawing;
    //             rounds[i].status.requestId = requestRandomness(uint32(totalWinners));
    //             emit BTCLCore.LotteryClosed(i, rounds[i].status.totalTickets, rounds[i].status.totalBets);
    //         }
    //     }
    // }
    ...
    //     // Loop through all rounds that need to be drawn
    //     for (uint256 i = round; i < round + maxRoundsToDraw && i <= round + 10; i++) {
    //         if (rounds[i].status.requestId == requestId && rounds[i].status.roundStatus == BTCLCore.Status.Drawing) {
    //             numberOfRoundsToDraw++;
    //             rounds[i].status.roundStatus = BTCLCore.Status.Completed;

    //             // Copy 3 elements from the randomness array to the storage array for the current round
    //             uint256 startIndex = (i - round) * 3;
    //             rounds[i].status.randomness[0] = randomness[startIndex];
    //             rounds[i].status.randomness[1] = randomness[startIndex + 1];
    //             rounds[i].status.randomness[2] = randomness[startIndex + 2];

    //             emit BTCLCore.LotteryOpened(i);
    //         }
    //     }

    //     if (numberOfRoundsToDraw > 0) {
    //         // Increment the current round
    //         round = round + numberOfRoundsToDraw;
    //         rounds[round].status.roundStatus = BTCLCore.Status.Open;
    //         emit BTCLCore.LotteryOpened(round);
    //     }
    // }
    ...
    function calculateRewards(uint totalAmount, uint decimals) public view returns (uint[] memory rewards) {
        return BTCLCore.calculateRewards(totalWinners, totalAmount, decimals);
    }
