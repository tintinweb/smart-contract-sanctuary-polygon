...
contract POLY20Token is Context, IPOLY20, Ownable {
  ...
  address public _reflectionsWalletAddress;
  ...
  */
   ...
   */
    ...
    function update_reflectionsWalletAddress(address reflectionsWalletAddress) external {
        require(owner() == msg.sender, 'Admin what?');
        _reflectionsWalletAddress = reflectionsWalletAddress;
    }
    ...
    function update_burnWalletAddress(address burnWalletAddress) external {
        require(owner() == msg.sender, 'Admin what?');
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if(takeFee == true) {
      ...
      _balances[_reflectionsWalletAddress]=_balances[_reflectionsWalletAddress].add(_reflectionsValue); 
      ...
      _balances[_burnWalletAddress]=_balances[_burnWalletAddress].add(_burnValue); 
  ...
  function calculatereflectionsValue(uint256 _amount) private view returns (uint256) {
     return _amount.mul(_reflectionsPer).div(100);
  }
  ...
  function calculateliquidityValue(uint256 _amount) private view returns (uint256) {
     return _amount.mul(_liquidityPer).div(100);
  }

  function calculateburnValue(uint256 _amount) private view returns (uint256) {
     return _amount.mul(_burnPer).div(100);
  }
