...
contract ChainBattles is ERC721URIStorage {
    ...
    // {1: 2} = {tokenId: currentLevel} by opensea
    ...
    function generateCharacter(uint256 tokenId) public view returns(string memory) {
        // byte type: a dynamically sized array of up to 32 bytes where you can store strings, and integers
        // global solidity: https://docs.soliditylang.org/en/latest/abi-spec.html#non-standard-packed-mode
        bytes memory svg = abi.encodePacked(
            ...
            '<text x="50%" y="30%" class="base" dominant-baseline="middle" text-anchor="middle">', "Name: ",getName(tokenId),'</text>',
    ...
    function getLevel(uint256 tokenId) public view returns(string memory) {
        ...
        // toString() form openZeppelin Strings library, transfer uint356 to string
    ...
    function getTokenURI(uint256 tokenId) public view returns(string memory) {
        bytes memory dataURI = abi.encodePacked(
            '{',
                ...
                '"worrior": "Name: #', getName(tokenId), ' Level: #', getLevel(tokenId), ' Speed: #', getSpeed(tokenId), ' Strength: #', getStrength(tokenId), ' Life: #', getLife(tokenId), '",',
    ...
    function mint(string memory _name) public {
        ...
        //  _safeMint() function from the OpenZeppelin ERC721 library
        _safeMint(msg.sender, newItemId);
        tokenIdToWarrior[newItemId].name = _name;
        tokenIdToWarrior[newItemId].level = 0;
        tokenIdToWarrior[newItemId].speed = random(100) + 25;
        tokenIdToWarrior[newItemId].strength = random(100) + 20;
        tokenIdToWarrior[newItemId].life = random(100) + 10;
        _setTokenURI(newItemId, getTokenURI(newItemId));
    }

    function train(uint256 tokenId) public {
        //  _exists() function from the ERC721 standard
        require(_exists(tokenId));
        require(ownerOf(tokenId) == msg.sender, "You must own this NFT to train it!");
        // uint256 currentLevel = tokenIdToWarrior[tokenId];
        // tokenIdToWarrior[tokenId] = currentLevel + 1;
        tokenIdToWarrior[tokenId].level = tokenIdToWarrior[tokenId].level + 1;
        tokenIdToWarrior[tokenId].speed = random(100);
        tokenIdToWarrior[tokenId].strength = random(100);
        tokenIdToWarrior[tokenId].life = random(100);
