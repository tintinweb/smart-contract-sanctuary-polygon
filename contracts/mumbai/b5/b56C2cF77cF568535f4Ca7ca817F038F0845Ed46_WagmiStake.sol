...
contract WagmiStake is Ownable, ReentrancyGuard {
    ...
    public view returns (uint256 o_amount, uint256 checkPoint) {
        ...
        (uint256 profit) = _getResult(deposit.planIdx, deposit.amount);
		...
		if (timeA < timeB) {
			...
			if(multiplier < TIME_STEP) {
				o_amount = (profit.mul(multiplier).div(dayCounts)).div(TIME_STEP);
			} else {
				o_amount = profit.mul(multiplier.div(TIME_STEP)).div(dayCounts);
			}

			checkPoint = block.timestamp; ///deposit.checkpoint = timeB;
		}		
	}
