...
contract EthermonStaking is EthermonStakingBasic {
    ...
    function updateTokens(
        ...
        require(
            data.owner != address(0) && data.monId > 0 && _level > 0,
            "Data is not valid"
        );
    ...
    function withdrawRewards(uint64 _lockId) external {
        ...
        if (timeElapsed > 0 && currentTime > data.endTime) {
            ...
            if (emon.balanceOf(address(this)) >= data.emons) {
                emit LOG(
                    data.emons,
                    emon.balanceOf(address(this)),
                    data.balance
                );
                // emon.safeTransfer(data.owner, data.emons);
                // stakingData.removeTokenData(data.lockId);
            }
        }
    }

    event LOG(uint256 _amount, uint256 _amount1, uint256 _amount2);
