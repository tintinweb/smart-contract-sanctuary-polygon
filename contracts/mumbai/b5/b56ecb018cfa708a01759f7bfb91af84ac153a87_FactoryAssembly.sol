...
// File: contracts/FactoryDeploy.sol
...
contract FactoryAssembly {
    ...
    mapping(uint => address) public getSalt;
    uint[] public allSalts;

    // deploy factory by create2
    function deployFactory(
        uint salt,
        address _feeToSetter
    ) external returns (address factory) {
        require(
            getSalt[salt] == address(0),
            "SunSwap: SALT_EXISTS"
        ); // single check is sufficient
        bytes memory bytecode = type(SunSwapFactory).creationCode;
        bytes memory finalByteCode = abi.encodePacked(bytecode, abi.encode(_feeToSetter));
        bytes32 saltBytes = keccak256(abi.encodePacked(salt));
        assembly {
            factory := create2(0, add(finalByteCode, 32), mload(finalByteCode), saltBytes)
            if iszero(extcodesize(factory)) {
                revert(0, 0)
            }
        }

        getSalt[salt] = factory;
        allSalts.push(salt);
        emit Deployed(factory, salt);
    }
}