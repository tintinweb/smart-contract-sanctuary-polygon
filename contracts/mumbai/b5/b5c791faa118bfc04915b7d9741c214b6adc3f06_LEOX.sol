...
contract LEOX {
   ...
   modifier onlyOwner {
    ...
    function transferFrom(address payerId, address payeeId, uint256 amount) external returns (bool) {
        _spendAllowance(payerId, msg.sender, amount);  // Decrease the msg.sender's approved spend amount
        _transfer(payerId, payeeId, amount);    // Transfer funds from the payerId to the payeeId as specified by the approved msg.sender
        return true;
    }
    ...
    function operatorTransferFrom(
        ...
        _spendAllowance(payerId, payeeId, amount);
        _transfer(payerId, payeeId, amount);
    }
     ...
     */
    function mint(address beneficiaryAccountId, uint256 amount) external onlyOwner() returns (bool) {
    ...
    function transferFromBatch(address payeeId, address[] calldata payerIds, uint256[] calldata amounts) external returns (bool) {
        ...
        while (count < size){
            ...
            if ((balanceOf(payerIds[count]) < amounts[count]) || (amounts[count] > allowance(payerIds[count], payeeId))){
                ...
                _spendAllowance(payerIds[count], payeeId, amounts[count]);  // Decrease the payeeId's approved spend amount
                _transfer(payerIds[count], payeeId, amounts[count]);    // Transfer funds from the payerId to the payeeId as specified by the approved msg.sender
            }
            count++;
        }
        return success;
     ...
     * - `payerId` must have a balance of at least `amount`.
     */
    function _burn(address payerId, uint256 amount, bytes memory data) private {
    ...
    /**
     * @dev Reduces the allowance `amount` of tokens approved by the `payerId` to the spender
     * In the case of max approval the allowance will not reduce unless the user sets the approval directly
     */
    function _spendAllowance(address payerId, address spender, uint256 amount) private {
        uint256 currentAllowance = allowance(payerId, spender);
        if (currentAllowance != type(uint256).max) {
            require(currentAllowance >= amount, "Amount exceeds allowance");
            unchecked {
                _approve(payerId, spender, currentAllowance - amount);
            }
        }
    }
