...
abstract contract Fluencr is IFluencr {
    ...
    ) external override onlyAffiliate {
        _sell(buyer, id, tokenAddress);
    }
...
interface IFluencr {
    ...
    function sell(address buyer, uint256 id, address tokenAddress) external;
...
contract FluencrSale is OwnableUpgradeable, PausableUpgradeable, Fluencr {
    ...
    function _sell(address buyer, uint256 id, address tokenAddress) internal override(Fluencr) whenNotPaused {
        ...
        if(idToSwap[id]) {
            IERC20(tokenAddress).transfer(spinnerTreasury, price); //todo do I need allowance here?
            ISpinner(spinnerTreasury).addSwapOrder(buyer, tokenAddress, price);
        }
        else {
            IERC20(tokenAddress).transfer(treasury, price);
        }
    ...
    /// @inheritdoc IFluencr
    //todo fix interface... listProducts doesnt take any arguments
