...
contract NavIssuanceModule is ModuleBase, ReentrancyGuard {
    ...
    /**
     * @dev MANAGER ONLY. Initializes this module to the MatrixToken with hooks, allowed reserve assets,
    ...
    function addReserveAsset(IMatrixToken matrixToken, address reserveAsset) external onlyManagerAndValidMatrix(matrixToken) {
        require(!_isReserveAssets[matrixToken][reserveAsset], "N1"); // "Reserve asset already exists"
    ...
    function removeReserveAsset(IMatrixToken matrixToken, address reserveAsset) external onlyManagerAndValidMatrix(matrixToken) {
        require(_isReserveAssets[matrixToken][reserveAsset], "N2"); // "Reserve asset does not exist"
    ...
    function editPremium(IMatrixToken matrixToken, uint256 premiumPercentage) external onlyManagerAndValidMatrix(matrixToken) {
        require(premiumPercentage <= _issuanceSettings[matrixToken].maxPremiumPercentage, "N3"); // "Premium must be less than maximum allowed"
    ...
    function editManagerFee(
        ...
        require(managerFeePercentage <= _issuanceSettings[matrixToken].maxManagerFee, "N4"); // "Manager fee must be less than maximum allowed"
    ...
    function editFeeRecipient(IMatrixToken matrixToken, address managerFeeRecipient) external onlyManagerAndValidMatrix(matrixToken) {
        require(managerFeeRecipient != address(0), "N5"); // "Fee recipient must not be 0 address"
    ...
    function _validateCommon(
        ...
        require(quantity > 0, "N6a"); // "Quantity must be > 0"
        require(_isReserveAssets[matrixToken][reserveAsset], "N6b"); // "Must be valid reserve asset"
    }
    ...
    function _validateIssuanceInfo(
        ...
        require(issueInfo.previousMatrixTokenSupply >= _issuanceSettings[matrixToken].minMatrixTokenSupply, "N7a"); // "Supply must be greater than minimum to enable issuance"
        require(issueInfo.matrixTokenQuantity >= minMatrixTokenReceiveQuantity, "N7b"); // "Must be greater than min MatrixToken"
    }
    ...
    function _validateRedemptionInfo(
        ...
        require(redeemInfo.newMatrixTokenSupply >= _issuanceSettings[matrixToken].minMatrixTokenSupply, "N8a"); // "Supply must be greater than minimum to enable redemption"
        require(redeemInfo.netFlowQuantity >= minReserveReceiveQuantity, "N8b"); // "Must be greater than min receive reserve quantity"
    }
    ...
    function _getRedeemPositionUnit(
        ...
        require(totalExistingUnits >= outflow, "N9"); // "Must be greater than total available collateral"
