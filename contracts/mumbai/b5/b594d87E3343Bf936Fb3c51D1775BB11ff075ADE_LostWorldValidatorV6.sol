...
contract LostWorldValidatorV6 is CustomERC2771ContextUpgradeable, UUPSUpgradeable {
    // Lost Chain RBA Linker
    ILostChainRBA private LCRBALinker;
    modifier onlyRole(string memory name, address account) {
        require(LCRBALinker.checkAccountRole(name, account), 'LWV: Unauthorized');
        _;
    }

    function setRBALinker(address rbaLinkerAddress) public onlyRole('DEFAULT_ADMIN_ROLE', _msgSender()) {
        require(rbaLinkerAddress != address(0x0), 'LWV: RBA linker is zero address');
    ...
    function initialize(
        ...
        require(address(forwarder) != address(0x0), 'LWV: Forwarder is zero address');
        require(rbaLinkerAddress != address(0x0), 'LWV: RBA linker is zero address');
        require(nftAddress != address(0x0), 'LWV: NFT  is zero address');
        require(worldDataAddress != address(0x0), 'LWV: World data is zero address');
        LCRBALinker = ILostChainRBA(rbaLinkerAddress);
    ...
    ILostChainGameSettings private LCGS;

    function setLCNFT(address lostchainNFTAddress) public onlyRole('DEFAULT_ADMIN_ROLE', msg.sender) {
        require(lostchainNFTAddress != address(0x0), 'LWV: Lost chain NFT is zero address');
        LCNFT = ILostChainNFT(lostchainNFTAddress);
    }

    function setLWD(address lostWorldAddress) public onlyRole('DEFAULT_ADMIN_ROLE', msg.sender) {
        require(lostWorldAddress != address(0x0), 'LWV: Lost chain NFT is zero address');
        LWD = ILostWorldData(lostWorldAddress);
    }
    ...
    function validateLand(
        ...
        validate(rewardAmount, maxRewardAmount, enemyList, rewardList);
    }
    ...
    function validate(
        ...
        require(
            rewardList.length == rewardAmount.length &&
                rewardAmount.length == maxRewardAmount.length &&
                maxRewardAmount.length == rewardList.length,
            'LWV: Array lengths mismatch'
        );

        for (uint256 i = 0; i < rewardAmount.length; i++) {
            require(rewardAmount[i] > 0, 'LWV: Reward amount must be minimum of 1');
            require(maxRewardAmount[i] > 0, 'LWV: Max reward amount must be minimum of 1');
            require(rewardAmount[i] <= maxRewardAmount[i], 'LWV: Max reward over limit');
        }

        for (uint256 i = 0; i < enemyList.length; i++) {
            require(LWD.checkEnemyAvailable(enemyList[i]), 'LWV: Enemy is disabled');
        }
        for (uint256 i = 0; i < rewardList.length; i++) {
            require(LCNFT.checkNFTRewardable(rewardList[i]), 'LWV: NFT is not rewardable');
        }

        (bool validatedRewardRartiy, bool validateRewardAmount) = validateRarity(
            maxRewardAmount,
            enemyList,
            rewardList
        );
        require(validatedRewardRartiy, 'LWV: Reward rarity is too high for that enemy');
        require(validateRewardAmount, 'LWV: Reward amount exceeds the limit');
    }
    ...
    function validateRarity(
        ...
        bool rewardRarityValidated = false;
        bool rewardAmountValidated = false;

        uint8 highestEnemyRarity = 0;
        string memory highestRarity;
        for (uint256 i = 0; i < enemyList.length; i++) {
            (uint8 rarityIndex, string memory rarityString) = LWD.getEnemyRarity(enemyList[i]);

            if (rarityIndex > highestEnemyRarity) {
                highestEnemyRarity = rarityIndex;
                highestRarity = rarityString;
            }
        }

        uint8 highestRewardRarity = 0;
        for (uint256 i = 0; i < rewardList.length; i++) {
            string memory rarity = LCNFT.getRewardableRarity(rewardList[i]);

            uint8 rarityIndex = 0;
            if (compareStrings(rarity, 'common')) rarityIndex = 1;
            else if (compareStrings(rarity, 'rare')) rarityIndex = 2;
            else if (compareStrings(rarity, 'epic')) rarityIndex = 3;
            else if (compareStrings(rarity, 'mythical')) rarityIndex = 4;
            else revert('LCNFT: Reward rarity unknown');

            if (rarityIndex > highestRewardRarity) highestRewardRarity = rarityIndex;
        }

        require(highestEnemyRarity > 0, 'LWV: Enemy rarity is not valid');
        require(highestRewardRarity > 0, 'LWV: Reward rarity is not valid');
        require(bytes(highestRarity).length > 0, 'LWV: Reward string is not valid');
        if (highestEnemyRarity >= highestRewardRarity) rewardRarityValidated = true;

        uint256 totalRewardWeight = 0;
        for (uint256 i = 0; i < rewardList.length; i++) {
            string memory rarity = LCNFT.getRewardableRarity(rewardList[i]);
            // uint8 rewardWeight = LCGS.getRewardWeight(rarity);

            // uint8 currentTotalWeight = rewardWeight * maxRewardAmount[i];
            // totalRewardWeight += currentTotalWeight;
        }

        // uint8 maxRewardWeight = LCGS.getEnemyMaxRewardWeight(highestRarity);
        // if (maxRewardWeight >= totalRewardWeight) rewardAmountValidated = true;

        return (rewardRarityValidated, rewardAmountValidated);
    }
