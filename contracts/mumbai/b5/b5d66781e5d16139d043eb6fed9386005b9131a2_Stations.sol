...
library ContractMetadataGenerator {
    ...
    function contractURI(ContractParams memory _params) internal pure returns (string memory) {
        return _webURIData(_contractMetadata(_params));
    }

    function valid(ContractParams memory _params) internal pure {
        require(bytes(_params.name).length != 0, "ContractParams: empty name");
        require(bytes(_params.description).length != 0, "ContractParams: empty description");
        require(bytes(_params.imageURL).length != 0, "ContractParams: empty image");
        require(_params.sellerFeeBasisPoints <= 2000, "ContractParams: seller fee > 2000");
        if (_params.sellerFeeBasisPoints > 0) {
            require(_params.feeRecipient != address(0), "ContractParams: empty recipient");
        }
    }

    function _webURIData(bytes memory _json) private pure returns (string memory) {
        return string(abi.encodePacked("data:application/json;base64,", Base64.encode(_json)));
    }

    function _contractMetadata(ContractParams memory params) private pure returns (bytes memory) {
...
library StationsMetadataGenerator {
    ...
    function valid(MetadataParams memory params) internal pure returns (bool) {
        if (bytes(params.name).length == 0) {
            return false;
        }
        if (bytes(params.description).length == 0) {
            return false;
        }
        if (bytes(params.imageURL).length == 0) {
            return false;
        }
        if (bytes(params.props.country).length == 0) {
            return false;
        }
        if (bytes(params.props.city).length == 0) {
            return false;
        }
        return true;
    }
    ...
    function _jsonMetadata(MetadataParams memory params) private pure returns (bytes memory) {
        bytes memory _json = abi.encodePacked(
            "{",
            '"name":"',
            params.name,
    ...
    function _propertiesString(MetadataProps memory props) private pure returns (string memory) {
...
contract OpenSeaCollection is Initializable, OwnableUpgradeable, WheelcoinRoles, IERC2981Upgradeable {
    ...
    modifier nonEmptyString(string memory _name, string memory _input) {
        string memory _revertReason = string(abi.encodePacked("OpenSeaCollection: empty ", _name));
        require(bytes(_input).length != 0, _revertReason);
        _;
    }

    // solhint-disable-next-line  func-name-mixedcase, private-vars-leading-underscore
    function __OpenSeaCollection_init(ContractParams memory _params) public onlyInitializing {
        OwnableUpgradeable.__Ownable_init();
        WheelcoinRoles.__WheelcoinRoles_init();

        if (_params.feeRecipient == address(0)) {
    ...
    function setName(string memory _name) public onlyRole(ADMIN_ROLE) nonEmptyString("name", _name) {
        contractMetadata.name = _name;
        _afterMetadataChange();
    }

    function setDescription(string memory _description) public onlyRole(ADMIN_ROLE) nonEmptyString("description", _description) {
        contractMetadata.description = _description;
        _afterMetadataChange();
    }

    function setImage(string memory _image) public onlyRole(ADMIN_ROLE) nonEmptyString("image", _image) {
        contractMetadata.imageURL = _image;
        _afterMetadataChange();
    }

    function setFeeRecipient(address _recipient) public onlyRole(ADMIN_ROLE) {
        contractMetadata.feeRecipient = _recipient;
        _afterMetadataChange();
    }

    function setSellerFeeBasisPoints(uint256 _fee) public onlyRole(ADMIN_ROLE) {
        contractMetadata.sellerFeeBasisPoints = _fee;
        _afterMetadataChange();
    }

    function _afterMetadataChange() internal {
...
contract Stations is Initializable, OpenSeaCollection, ERC1155PausableUpgradeable, WheelcoinPausable {
    ...
    modifier requireValidMetadata(MetadataParams memory _params) {
        require(_params.valid(), "Stations: invalid metadata");
        _;
    }

    modifier requireExist(uint256 _id) {
        require(exist(_id), "Stations: token doesn't exists");
        _;
    }

    // solhint-disable-next-line  func-name-mixedcase, private-vars-leading-underscore
    function __Stations_init(ContractParams memory _contractParams) public initializer onlyInitializing {
    ...
    ) public onlyRole(ADMIN_ROLE) requireValidMetadata(_md) whenNotPaused {
    ...
    function mint(address _to, uint256 _tokenId) public onlyRole(MINTER_ROLE) requireExist(_tokenId) whenNotPaused {
    ...
    ) public view returns (BalanceByCountryEntry[] memory) {
        string[] memory _cities = countryToCities[_country];
        uint256 _citiesCount = _cities.length;
        require(_citiesCount != 0, "Stations: no cities found");
    ...
    function balanceByCity(address _account, string memory _city) public view returns (TokenBalanceEntry[] memory) {
        uint256[] memory _ids = cityToTokens[_city];
        require(_ids.length != 0, "Stations: no tokens found");

        return _balanceByAccountBatch(_account, _ids);
    }

    function uri(uint256 id) public view override requireExist(id) returns (string memory) {
    ...
    // solhint-disable-next-line  no-unused-vars
        bytes4 interfaceId
    ) public view virtual override(OpenSeaCollection, AccessControlUpgradeable, ERC1155Upgradeable) returns (bool) {
        return true;
    }

    function _appendCountry(string memory _country) internal {
        for (uint256 i = 0; i < countries.length; i++) {
            string memory _current = countries[i];
            if (keccak256(bytes(_current)) == keccak256(bytes(_country))) {
                return;
            }
        }
    ...
    function _appendCity(string memory _country, string memory _city) internal {
        string[] memory _cities = countryToCities[_country];

        for (uint256 i = 0; i < _cities.length; i++) {
            string memory _current = _cities[i];
            if (keccak256(bytes(_current)) == keccak256(bytes(_city))) {
                return;
            }
        }
    ...
    function _balanceByAccountBatch(
        address _account,
        uint256[] memory _ids
    ) internal view returns (TokenBalanceEntry[] memory) {
...
contract WheelcoinPausable is Initializable, WheelcoinRoles, PausableUpgradeable {
    ...
    // solhint-disable-next-line  func-name-mixedcase, private-vars-leading-underscore
    function __WheelcoinPausable_init() internal onlyInitializing {
        WheelcoinRoles.__WheelcoinRoles_init();
...
contract WheelcoinRoles is Initializable, AccessControlUpgradeable {
    ...
    function getHighestRole() public view returns (bytes32) {
