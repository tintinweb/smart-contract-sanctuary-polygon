...
contract DEGABaseNFT is ERC721, Ownable, ERC2771Recipient, DEGAEvents, DEGAModifiers, DEGAStructs {
    ...
    function equipToken(address _tokenAddress, uint256 _destinationNFT, uint256 _tokenId, uint256 _amount) external {
        ...
        if (ownerOf(_destinationNFT) != _msgSender()) {
            revert CallerNotTheOwner();
        }

        if (_amount > 0) {
            if (IERC20(_tokenAddress).balanceOf(_msgSender()) < _amount) {
        ...
        if (_tokenId > 0) {
            if (IERC721(_tokenAddress).ownerOf(_tokenId) != _msgSender()) {
...
contract ImmutableNFTFactory {
    ...
    struct NFTData {
        ...
        bool isDynamic;
        bool isMetadataOnChain;
        DEGAStructs.Property[] evolutionaryProperties;
    }

    function createNFTContract(NFTData memory data) external returns (address nftAddr) {
        DEGAImmutableNFT nft = new DEGAImmutableNFT(DEGAStructs.contractData({
            _name: data.name,
            _symbol: data.symbol,
            _contractURI: data.contractURI,
            _baseURI: data.baseURI,
            _maxSupply: data.maxSupply,
            _owner: data.owner,
            _forwarder: data.forwarder,
            _isImmutable: data.isImmutable,
            _isMultisign: data.isMultisign,
            _isComposable: data.isComposable,
            _isEvolutionary: data.isEvolutionary,
            _isDynamic: data.isDynamic,
            _isMetadataOnChain: data.isMetadataOnChain,
            _evolutionaryProperties: data.evolutionaryProperties
        }));        
...
contract DEGAErrors {
    error CallerNotApiError(address caller);
    error ZeroAddressError();
    error InsufficientBalance(uint256 available, uint256 required);
    error ExceedCollectionMaxSupply(address collection, uint256 maxSupply, uint256 counter);
    error CallerNotTheOwner();
    error CallerDoNotHaveAmountToEquip();
    error CallerDoNotOwnTokenToEquip();
    error CannotUnequipAmount();
    error InvalidCallParameters();
    error InvalidSignature();
    error IsImmutableNFT();
}
...
contract DEGAEvents {
    event NFTItemMinted(uint256 tokenId, address owner);
    event NFTCollectionCreated(address nftCollection, address owner);
}
...
import {DEGAErrors} from "./DEGAErrors.sol";

contract DEGAModifiers is DEGAErrors {
    using ECDSA for bytes32;

    modifier checkMultisign(bytes memory _tokenOwnerSign, bool _isMultisign, uint256 _tokenId, address _holder) {
        if (_isMultisign) {
            // Verify approval signature of token owner
            bytes32 _hash = keccak256(abi.encode(address(this), _tokenId, _holder));
            if ((_hash.toEthSignedMessageHash().recover(_tokenOwnerSign) != _holder)) revert InvalidSignature();
        }
        _;
    }
}
...
contract DEGAStructs {
    struct contractData {
        ...
        bool _isMultisign;
        bool _isComposable;
        bool _isEvolutionary;
        bool _isDynamic;
        bool _isMetadataOnChain;
        Property[] _evolutionaryProperties;
    }
    ...
    struct Composable {
        uint256 erc20added;
        uint256 nftsAdded;
        mapping(address => bool) isAddressERC20; // (0x1 -> false -- 0x2 -> true)
        mapping(address => uint256[]) addressItems; // 0x1 -> [1,2,4] ;
        mapping(address => uint256) addressAmount; // 0x2 -> 10_000;
    }
}
