...
contract RandomWinnerGame is VRFConsumerBase, Ownable {
    ...
    // Max no of players in the game
    uint8 maxPlayers;
    // If game started
    bool public gameStarted;
    // the fee for entering the game
    uint256 entryFee;
    // current game id
    ...
    */
    ...
    /**
    * joinGame: called when a player wants to enter the game
    */
    function joinGame() public payable {
        ...
        require(players.length < maxPlayers, "Game if full");
    ...
    */
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal virtual override {
        ...
        // For this we mod `randomness` with `players.length`
        uint256 winnerIndex = randomness % players.length;
        // get the address of the winner from the players array
        address winner = players[winnerIndex];
        // send the ether in the contract to the winner address
        (bool sent, ) = winner.call{value: address(this).balance}("");
        // confirm ether is sent
        require(sent, "Failed to send the ether");
        // Emit the game has ended event
    ...
    /**
    * getRandomWinner: called to start the process of selecting a random winner
    */
    function getRandomWinner() private returns (bytes32 requestId) {
        ...
        // requestRandomness is a function in VRFCoordinatorBase
        // it start the process of randomness generation
