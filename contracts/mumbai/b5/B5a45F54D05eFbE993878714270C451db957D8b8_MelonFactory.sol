...
import { IMelonFactory } from "./interfaces/IMelonFactory.sol";
...
/**
 * @title MelonFactory
 * @notice A factory that deploys minimal proxies of `MelonDropV1.sol`.
 * @dev The proxies are OpenZeppelin's Clones implementation of https://eips.ethereum.org/EIPS/eip-1167
 */
contract MelonFactory is IMelonFactory, OwnableRoles {
    // =============================================================
    //                            STORAGE
    // =============================================================
    ...
    constructor(address _dropImplementation) implementationNotZero(_dropImplementation) {
    ...
    function createDrop(
        ...
        bytes[] calldata data
    ) external returns (address melonDrop, bytes[] memory results) {
        // Create Sound Edition proxy.
        melonDrop = payable(Clones.cloneDeterministic(dropImplementation, _saltedSalt(msg.sender, salt)));
        ...
        assembly {
            ...
            if iszero(
                call(
                    gas(), // Gas remaining.
                    melonDrop, // Address of the edition.
        ...
        OwnableRoles(melonDrop).transferOwnership(msg.sender);

        emit MelonDropCreated(melonDrop, msg.sender, initData, contracts, data, results);
    }

    /**
     * @inheritdoc IMelonFactory
     */
    function setDropImplementation(address newImplementation)
        external
        onlyOwner
        implementationNotZero(newImplementation)
    {
        ...
        emit MelonDropImplementationSet(dropImplementation);
    }
     ...
     * @inheritdoc IMelonFactory
     */
    function dropAddress(address by, bytes32 salt) external view returns (address addr, bool exists) {
        addr = Clones.predictDeterministicAddress(dropImplementation, _saltedSalt(by, salt), address(this));
...
/**
 * @title IMelonFactory
 * @notice The interface for the Melon Drops creation.
 */
interface IMelonFactory {
    // =============================================================
    //                            EVENTS
    // =============================================================

    /**
     * @dev Emitted when a drop is created.
     * @param melonDrop    The address of the drop.
     * @param deployer     The address of the deployer.
     * @param initData     The calldata to initialize MelonDrop via `abi.encodeWithSelector`.
    ...
    /**
     * @dev Emitted when the drop implementation address is set.
     * @param newImplementation The new implementation address to be set.
     */
    event MelonDropImplementationSet(address newImplementation);
    ...
    /**
     * @dev Creates a Melon Drop proxy, initializes it,
     ...
     * @param initData  The calldata to initialize MelonDrop via
     ...
     * @return melonDrop Returns the address of the created contract.
     * @return results      The results of calling the contracts.
    ...
    /**
     * @dev Changes the Melon Drop implementation contract address.
     *
     * Calling conditions:
     ...
     * @param newImplementation The new implementation address to be set.
     */
    function setDropImplementation(address newImplementation) external;
    ...
    /**
     * @dev The address of the Melon drop implementation.
     * @return The configured value.
     */
    function dropImplementation() external returns (address);

    /**
     * @dev Returns the deterministic address for the Melon drop clone.
