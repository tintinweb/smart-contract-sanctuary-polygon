...
contract GalaxisConsumer is Ownable, ChainlinkClient {
    using Chainlink for Chainlink.Request;
    using EnumerableSet for EnumerableSet.AddressSet;

    IECRegistry public registry;
    uint32      public LastDataRecordId;
    bytes32     public LastDate;
    bool        public NeedsMoreProcessing;
    ...
    constructor(address _registry, address _linkToken, address _oracle, bytes32 _specId) {
        registry = IECRegistry(_registry);
        setChainlinkToken(_linkToken);
        oracle = _oracle;
        specId = _specId;
        fee = 0.1 * 10 ** 18; // (Varies by network and job)
        LINK = xERC20(_linkToken);
    }
    ...
    // _date: YYYY-DD-MM
    function requestNBA(string calldata _date) external onlyControllers returns (bytes32 requestId) 
    {
        require(
           LINK.balanceOf(address(this)) >= fee,
           "GalaxisConsumer: Not enough LINK"
        );

        Chainlink.Request memory request = buildChainlinkRequest(specId, address(this), this.fulfillBytes.selector);
    ...
    // 7     - 7        - is there any more data to be processed for today's date ?
    // 32    - 8:40     - lastDate / misc 32 bytes
    // ------ call 1 ------------------------------------
    ...
    function fulfillBytes(
        ...
        returns (bool[] memory)
    {
        return _execute(requestId, bytesData);
    }
