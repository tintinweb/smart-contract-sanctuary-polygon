...
contract Game is OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable{
    ...
    function arrangeMissile(uint256[12] memory positions) external payable nonReentrant whenNotPaused {
        ...
        uint256 valuePosition = positions[0] + positions[1] + positions[2] + positions[3]
                                + positions[4] + positions[5] + positions[6] + positions[7]
                                + positions[8] + positions[9] + positions[10] + positions[11];
        require(valuePosition > 0, "Game: need missiles");
        ...
        if (blacklistIds[msg.sender] != 0) {
            ...
            require(blacklistInfos[blacklistIds[msg.sender]].spent + uint128(price) <= blacklistInfos[blacklistIds[msg.sender]].limitSpend, "Game: exceed limit");
    ...
    function shoot() external nonReentrant whenNotPaused {
        if (blacklistIds[msg.sender] != 0) {
            ...
            require(blacklistInfos[blacklistIds[msg.sender]].numberUsed < blacklistInfos[blacklistIds[msg.sender]].limitNumber, "Game: exceed limit");
        ...
        emit Shot(msg.sender, userReward, positions, boatPosition, firstPosition, secondPosition, price, boatShot);
    }

    function calPrice(uint256[12] memory positions, address account) public view returns (uint256, uint256, uint256) {
        uint256 value;
        bool condition = freeObject == 2 || (freeObject == 0 && whitelistIds[account] != 0) || (freeObject == 1 && (whitelistIds[account] != 0 || userRequestIds[account] == 0));
    ...
    function calPosition(uint256 randomNumber) public view returns (uint256[12] memory map, uint256 firstPosition, uint256 secondPosition) {        
    ...
    function addToBlacklist(address account_, uint256 limitSpend_, uint256 limitNumber_) external onlyOwner {
        require(blacklistIds[account_] == 0 && account_ != address(0), "Game: existed in blacklist");
    ...
    function removeFromBlacklist(address account) external onlyOwner {
        ...
        require(id != 0 && account != address(0), "Game: not in blacklist");
    ...
    function addToWhitelist(address account_) external onlyOwner {
        require(whitelistIds[account_] == 0 && account_ != address(0), "Game: existed in whitelist");
    ...
    function removeFromWhitelist(address account) external onlyOwner {
        ...
        require(id != 0 && account != address(0), "Game: not in whitelist");
