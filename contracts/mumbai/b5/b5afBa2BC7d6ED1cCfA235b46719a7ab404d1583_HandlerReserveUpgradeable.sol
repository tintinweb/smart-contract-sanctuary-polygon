    ...
    ERC20SafeUpgradeable
{
    ...
    function initialize(
        ...
        _setupRole(VOYAGER_DEPOSIT_HANDLER, _voyagerDepositHandler);
        _setupRole(VOYAGER_EXECUTE_HANDLER, _voyagerExecuteHandler);
        _setupRole(VOYAGER_UTILS, _voyagerUtils);
    }
    ...
    function mintWrappedERC20(
        ...
        address recipient,
        uint256 amount
    ) public onlyRole(VOYAGER_EXECUTE_HANDLER) {
    ...
    function stake(
        ...
        address tokenAddress,
        uint256 amount
    ) external virtual onlyRole(VOYAGER_UTILS) {
    ...
    function withdrawWETH(address WETH, uint256 amount) external onlyRole(VOYAGER_EXECUTE_HANDLER) {
        IWETH(WETH).transfer(address(_ethHandler), amount);
        _ethHandler.withdraw(WETH, amount);
    }
    ...
    function _setLiquidityPool(address contractAddress, address lpAddress) external virtual onlyRole(VOYAGER_UTILS) {
    ...
    function swap(
        ...
        uint256 flags,
        bytes memory dataTx
    ) external onlyRole(VOYAGER_DEPOSIT_HANDLER) returns (uint256 returnAmount) {
...
contract ERC20SafeUpgradeable is Initializable, ContextUpgradeable, AccessControlUpgradeable {
    ...
    function fundERC20(
        ...
        address owner,
        uint256 amount
    ) public onlyRole(VOYAGER_DEPOSIT_HANDLER) {
    ...
    /// @param to Address to transfer token to
    /// @param value Amount of token to transfer
    function safeTransferETH(address to, uint256 value) public onlyRole(VOYAGER_EXECUTE_HANDLER) {
