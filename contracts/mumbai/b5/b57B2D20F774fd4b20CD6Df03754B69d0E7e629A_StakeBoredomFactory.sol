...
contract StakeBoredom is ReentrancyGuard {
    ...
    address public TAX_WALLET_ADDRESS;

    constructor() {
        RewardPoolAdmin = msg.sender;
        TAX_WALLET_ADDRESS = 0x1AaFe79f62Faa7dF8286b37B8e82097945BF75B7;
    }
    ...
    // Calculate the withdrawal tax
    uint256 taxAmount = amount / 10;  // 10% of the withdrawal amount

    // Calculate the amount to be sent to the user after deducting the tax
    uint256 withdrawAmount = amount - taxAmount;

    // Deduct the withdrawal amount from the total stake and the user's balance
    _totalStake -= amount;
    _balances[msg.sender] -= amount;
    ...
    // Transfer the tax amount to the specific wallet
    stakingToken.safeTransfer(TAX_WALLET_ADDRESS, taxAmount);
