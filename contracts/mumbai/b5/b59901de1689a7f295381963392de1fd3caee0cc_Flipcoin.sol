...
contract Flipcoin {
    ...
    mapping(uint256 => MiniLobby) private _lobbies; // Маппинг лобби по их номеру

    struct MiniLobby {
        address creator; // Создатель лобби
        uint256 totalPot; // Общая сумма ставок в лобби
        mapping(address => uint256) bets; // Ставки игроков в лобби
        bool closed; // Флаг, указывающий, закрыто ли лобби
    }
    ...
    function createLobby() external {
        require(_lobbies[_lobbyCounter].creator == address(0), "Lobby already exists");
        
        _lobbies[_lobbyCounter].creator = msg.sender;
    ...
    function placeBet(uint256 lobbyId, uint256 betAmount) external {
        ...
        require(!lobby.closed, "Lobby is closed");
        require(lobby.bets[msg.sender] == 0, "You have already placed a bet");
    ...
    function checkLobbyClosure(uint256 lobbyId) internal {
        ...
        if (lobby.bets[msg.sender] > 0 && msg.sender != lobby.creator) {
            // Если создатель лобби сделал ставку, то лобби автоматически закрывается
            closeLobby(lobbyId);
        }
    }
    ...
    function closeLobby(uint256 lobbyId) public {
        ...
        MiniLobby storage lobby = _lobbies[lobbyId];
        require(!lobby.closed, "Lobby is already closed");
        require(msg.sender == lobby.creator, "Only the lobby creator can close the lobby");

        require(lobby.bets[msg.sender] > 0, "No bets placed in this lobby");
        ...
        // Найти игрока с наибольшей ставкой в лобби
        for (uint256 i = 0; i < _lobbyCounter; i++) {
            if (lobby.bets[_lobbies[i].creator] > maxBetAmount) {
                maxBetAmount = lobby.bets[_lobbies[i].creator];
                winner = _lobbies[i].creator;
            }
        }
    ...
    function getLobbyBet(uint256 lobbyId) external view returns (uint256) {
        require(lobbyId < _lobbyCounter, "Invalid lobbyId");
        return _lobbies[lobbyId].bets[msg.sender];
    }
