...
contract TreasureHuntSeaport is
    Initializable,
    ITreasureHuntTrait,
    ...
    OwnableUpgradeable
{
    ...
    uint256 public seaportUpdatePrice;
    ...
    bool private adminSeaportExist;
    ...
    modifier _isMarket() {
        require(
            msg.sender == treasureHuntMarketPlace,
            "You are not MarketPlace!"
        );
        _;
    }

    event ReceiveGift(address sender, address receiver, uint256 tokenID);
    ...
    function initialize() public initializer {
        ...
        seaportUpdatePrice = 20000;

        mintPrice = 20000;
    }

    function readSeaportsByOwner(address acc)
        public
        view
        returns (uint256[] memory)
    {
        uint256[] memory tokens = new uint256[](balanceOf(acc));
        uint8 count;
        for (uint8 i; i < totalSupply(); i++) {
            uint256 tokenID = tokenByIndex(i);
            if (ownerOf(tokenID) == acc) {
    ...
    function createAdminSeaport(string memory name) public onlyOwner {
        require(!adminSeaportExist, "Admin seaport already exists!");
        adminSeaportExist = true;
        _mint(rewardPool, 1);
        seaportInfo[1] = Seaport(name, 1, 1, 0);
        string memory uri = string(
            abi.encodePacked(SeaportBaseURI, StringsUpgradeable.toString(1))
        );
        _setTokenURI(1, uri);
        join2Seaport(1);
        haki4Seaport[1] = 100;
    ...
    function mint() private {
        require(
            onSeaport[msg.sender] == 0 || msg.sender == owner(),
            "You need to leave current seaport!"
        );
    ...
    function giveGift(address receiver, uint256 tokenID) public onlyOwner {
        require(!isOnSeaport(receiver), "Receiver is on seaport now!");
        transferFrom(msg.sender, receiver, tokenID);
        emit ReceiveGift(msg.sender, receiver, tokenID);
    }
    ...
    function join2Seaport(uint256 tokenID) public {
        require(
            ...
            "You can't join into that seaport!"
        );
        ...
        ITreasureHunt(rewardPool).resetEarningOnSeaport();
    }

    function seaportUseability(uint256 tokenID, address account)
        public
        view
        returns (bool)
    {
        if (acceptMod[tokenID]) {
            return
                seaportInfo[tokenID].Current + 1 <=
                seaportSizeByLevel[seaportInfo[tokenID].Level] &&
                WL4Seaport[tokenID].contains(account);
        } else {
            return
                seaportInfo[tokenID].Current + 1 <=
                seaportSizeByLevel[seaportInfo[tokenID].Level] &&
                ITreasureHunt(treasureHuntFleet).getMaxHakiByOwner(account) >=
                haki4Seaport[tokenID];
        }
    }

    function writeWL4Seaport(uint256 tokenID, address[] memory _wl)
        public
        onlySeaportOwner(tokenID)
    {
    ...
    function setPermitOption(bool isWL, uint256 tokenID)
        public
        onlySeaportOwner(tokenID)
    {
    ...
    function getPermitOption(uint256 tokenID) public view returns (bool) {
        return acceptMod[tokenID];
    }

    function kickFromSeaport(
        address user,
        uint256 tokenID,
        bool isDirect
    ) public {
        require(
            playersBySeaport[tokenID].contains(user),
            "He is not on the Seaport"
        );
        require(
            ownerOf(tokenID) == msg.sender || msg.sender == owner(),
            "You are not owner of that seaport!"
        );
        if (tokenID != 1) {
            if (isDirect) {
    ...
    function exitFromSeaport(uint256 tokenID, bool isDirect) public {
        require(
            playersBySeaport[tokenID].contains(msg.sender),
            "You are not on the Seaport"
        );
        if (isDirect) {
            ITreasureHunt(rewardPool).transferCost(kickPrice);
        } else {
    ...
    function getJoinableSeaports() public view returns (Seaport[] memory) {
        Seaport[] memory seaports = new Seaport[](
            totalSupply() - balanceOf(owner())
        );
        uint8 count;
        for (uint8 i; i < totalSupply(); i++) {
            if (ownerOf(i + 1) != owner()) {
                seaports[count++] = seaportInfo[i + 1];
            }
        }
        return seaports;
    }
    ...
    function playersOfSeaport(uint256 tokenID)
        public
        view
    ...
    function setHaki4Seaport(uint256 tokenID, uint256 power)
        public
        onlySeaportOwner(tokenID)
    {
    ...
    function getTotalHakiOfSeaport(uint256 tokenID)
        public
        view
        returns (uint256)
    {
        uint256 haki = 0;
        for (uint256 i; i < playersBySeaport[tokenID].length(); i++) {
            haki += ITreasureHunt(treasureHuntFleet).getTotalHakiByOwner(
                playersBySeaport[tokenID].at(i)
            );
        }
        return haki;
    }
    ...
    function initSeaport(uint256 tokenID) internal {
        seaportInfo[tokenID] = Seaport("", tokenID, 1, 0);
        string memory uri = string(
            abi.encodePacked(
                SeaportBaseURI,
    ...
    function upgradeSeaport(uint256 tokenID) public onlySeaportOwner(tokenID) {
        require(seaportInfo[tokenID].Level < 7, "Max Level!");
        ITreasureHunt(rewardPool).transferCost(seaportUpdatePrice);
        seaportInfo[tokenID].Level += 1;
    }

    function upgradeAdminSeaport() public onlyOwner {
        require(seaportInfo[1].Level < 7, "Max Level!");
        seaportInfo[1].Level += 1;
    }
    ...
    function getSeaportOwnerByPlayer(address acc)
        public
        view
        returns (address)
    {
    ...
    function transferFrom(
        ...
        require(
            isSeaportOwner(from) &&
                (!isOnSeaport(to) || isMarketPlace(to) || to == owner()),
            "Can't transfer Seaport!"
        );
    ...
    function setSeaportUpdatePrice(uint256 price) public onlyOwner {
        seaportUpdatePrice = price;
    }
