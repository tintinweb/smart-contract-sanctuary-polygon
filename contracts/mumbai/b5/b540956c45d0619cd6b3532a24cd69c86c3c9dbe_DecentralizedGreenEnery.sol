...
contract DecentralizedGreenEnery is Context, IERC20, Ownable {  
    ...
    // uint256 private tokenPrice = 500000000000000000; // 0.5 BUSD = 10^18 wei (the smallest unit of ether)
    // uint256 private tokenPriceIncrement =  1 wei;// 1 wei
    uint256  internal tokenPriceInitial_ = 0.000001 ether;
    ...
    */
    ...
    function sell(uint256 _amountOfTokens) public {
        ...
        uint256 _taxedMatic = _matic.sub(_marketing).sub(_autoBurn).sub(_reward);
     ...
     function purchaseTokens(uint256 _incomingMatic)internal returns(uint256)
    {
        ...
        _mint(address(this), _marketingPer);
        _mint(address(this), _RewardPer);

        // update circulating supply & the ledger address for the customer
         _rOwned[_customerAddress] = SafeMath.add( _rOwned[_customerAddress], _amountOfTokens);
    ...
    function sellPrice() public view returns (uint256) {
       if (_tTotal == 60000000 * 10**18) {
        return tokenPriceInitial_.sub(tokenPriceIncremental_);
            ...
            uint256 marketing = _tokens.mul(_marketingPer).div(100);
            uint256 autoBurn = _tokens.mul(_autoBurnPer).div(100);
            uint256 reward = _tokens.mul(_RewardPer).div(100);
            uint256 _taxedMatic = _matic.sub(marketing).sub(autoBurn).sub(reward);
    ...
    function buyPrice() public view returns (uint256) {
       if (_tTotal == 60000000 * 10**18) {
        return tokenPriceInitial_.add(tokenPriceIncremental_);
            ...
            uint256 autoBurn = _tokens.mul(_autoBurnPer).div(100);
            uint256 reward = _tokens.mul(_RewardPer).div(100);
            uint256 _taxedMatic = _matic.add(marketing).add(autoBurn).add(reward);
    ...
    function calculateBuyToken(uint256 _maticToSpend) public view returns (uint256) {
        uint256 marketing = _maticToSpend.mul(_marketingPer).div(100);
        uint256 autoBurn = _maticToSpend.mul(_autoBurnPer).div(100);
        uint256 reward = _maticToSpend.mul(_RewardPer).div(100);
        uint256 _taxedMatic = _maticToSpend.sub(marketing).sub(autoBurn).sub(reward);
    ...
    function calculateSellToken(uint256 _tokensToSell) public view returns (uint256) {
        uint256 marketing = _tokensToSell.mul(_marketingPer).div(100);
        uint256 autoBurn = _tokensToSell.mul(_autoBurnPer).div(100);
        uint256 reward = _tokensToSell.mul(_RewardPer).div(100);
        uint256 _taxedMatic = _tokensToSell.add(marketing).add(autoBurn).add(reward);
        return _taxedMatic;
    }
            ...
            SafeMath.sub(
                (
                    sqrt(
                        ...
                        (2 * tokenPriceIncremental_ * 1e18 * _matic) +
    ...
    function verifyMatic() public onlyOwner {
