...
for (uint256 ii=0; ii < 8; ii++) {
  ...
  if ((workingByte >> (7 - ii)) & 1 == 1) {
          rects.appendSafe(abi.encodePacked('<use xlink:href="#hex" x="',y%2==1 ? toString((x*46) + (ii*46)) : toString((x*46) + (ii*46) + 23) ,'" y="',
              toString(y*40),'" fill="',svgBackgroundColor2[color1]));//'" fill="', svgBackgroundColor2[colorByte]));
  } else {
        if ((colorByte >> (7 - ii)) & 1 == 1) {
          rects.appendSafe(abi.encodePacked('<use xlink:href="#hex" x="',y%2==1 ? toString((x*46) + (ii*46)) : toString((x*46) + (ii*46) + 23),'" y="',
              toString(y*40),'" fill="',svgBackgroundColor2[color2]));//'" fill="', svgBackgroundColor2[colorByte]));
        } else {
            rects.appendSafe(abi.encodePacked('<use xlink:href="#hex" x="',y%2==1 ? toString((x*46) + (ii*46)) : toString((x*46) + (ii*46) + 23),'" y="',
              toString(y*40),'" fill="',svgBackgroundColor2[color3]));//'" fill="', svgBackgroundColor2[colorByte]));
        }
  }
