...
contract WBITSTokenPolygon is ERC20Capped, ERC2771Context, ReentrancyGuard, Ownable {
  uint public feeBp;
  ...
  ) ERC20("Worlds Beyond", "WBITS") ERC20Capped(6e9 ether) ERC2771Context(_forwarder) {
  ...
  function transferWithFee(address recipient, uint256 amount) public nonReentrant returns (bool) {
    uint256 feeEst = amount * feeBp / 10000 + feeFixed;
    uint256 fee = feeEst <= feeCap ? feeEst : feeCap;
  ...
  function setFees(uint _feeBp, uint _feeFixed, uint _feeCap) external onlyOwner {
    feeBp = _feeBp;
    feeFixed = _feeFixed;
    feeCap = _feeCap;
  }
