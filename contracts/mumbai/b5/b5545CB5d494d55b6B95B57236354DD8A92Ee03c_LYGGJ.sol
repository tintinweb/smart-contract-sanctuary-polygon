...
contract LYGGJ is ERC20 {
    ...
    constructor(address yggjAddress) ERC20("Locked YGGJ", "LYGGJ") {
        yggjToken = YGGJ(yggjAddress);
    }
     ...
     * @param yggjAmount Amount of YGGJ tokens to deposit.
     */
    function mint(
        address to,
        uint64 since,
        ...
        if (yggjAmount == 0) revert NoAmount();

        // Perform the transfer of YGGJ tokens
        require(yggjToken.transferFrom(msg.sender, address(this), yggjAmount), "YGGJ transfer failed");

        // Mint the corresponding amount of LYGGJ tokens
        _mint(to, yggjAmount);

        claimInfo[to] = ClaimInfo(yggjAmount, 0, since, until, msg.sender);
        originalClaimer[to] = to;
        emit Mint(to, yggjAmount, since, until);
    }
    ...
    function claim(uint256 amount) external {
        ...
        require(
            yggjToken.transfer(msg.sender, amount),
            "LYGGJ: YGGJ transfer failed"
        );
    ...
    function renounce(uint256 amount) external {
        ...
        _burn(msg.sender, amount);
        require(
            yggjToken.transfer(originalClaimInfo.from, amount),
            "LYGGJ: YGGJ transfer failed"
        );
    ...
    /**
     * Bulk transfer
     * @param tos List of recipient address.
