...
contract IntegrationApprove is Initializable{
    ...
    modifier onlyPost()
    {
        require(roles.isPost(msg.sender), "Restricted to governance contract.");
        _;
    }
    ...
    /// @param amount_integration Amount USD to transfer, when integration is successful proposal.
    /// @param amount_integration_approve Amount USD to transfer, when integration approve is successful confirmed.
    /// @param groups Array of groups, when one user from group must confirm and after that all group is confirmed. 
    /// @param group_for_vote Group for users, who can confirm.
    ...
    function addIntegrationApprove(string memory _hash, address payable _account, uint256 amount_integration, uint256 amount_integration_approve, bytes32[] memory groups,bytes32 group_for_vote,uint256 block_start,uint256 block_end)
    public
    onlyPost
    returns (uint256) {
        ...
        integrationsApprove.push(IntegrationsApprove(_id,false,_hash,_account,amount_integration,groups,group_for_vote,block_start,block_end,Confirmation.Pending));
        totalNumber++;
