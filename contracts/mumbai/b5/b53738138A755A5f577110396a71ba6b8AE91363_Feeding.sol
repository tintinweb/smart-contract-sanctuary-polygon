...
contract Feeding is Ownable {
    ...
    address swapperToken = 0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48; // USDC address
    mapping(address => bool) public feedToken;
    ...
    function calculateFeedReward(
        ...
        unchecked {
            ...
            UD60x18 term3 = term1 * (term2 - ud(1e18)) + ud(1e18);

            return intoUint256(term3);
        }
    }
    ...
    function swapTokens(
        ...
        address[] memory path;
        if (
            _tokenIn != address(swapperToken) &&
            _tokenOut != address(swapperToken)
        ) {
            path = new address[](3);
            path[0] = _tokenIn;
            path[1] = swapperToken;
        ...
        uint256 _reward = calculateFeedReward(
            amounts[amounts.length - 1],
            _vestingTime
        );
        _depositReward(_tokenOut, _taxReceiver, _reward);

        // Store vesting balance
        vestingBalances[msg.sender].push(
            Vesting({
                amount: amounts[amounts.length - 1] + _reward,
    ...
    function setSwapperToken(address _newToken) public onlyOwner {
        ...
        swapperToken = _newToken;
    }

    function getFeedTokensList() public view returns (address[] memory) {
        return feedTokensList;
    }

    function addFeedToken(address _token) public onlyOwner {
        require(_token != address(0), "Invalid address");
        require(!feedToken[_token], "Already exists");
        feedToken[_token] = true;
        feedTokensList.push(_token);

        emit AddFeedToken(_token);
    }

    function removeTokensPair(uint256 _index) public onlyOwner {
        require(_index < feedTokensList.length);
        address _token = feedTokensList[_index];
        feedToken[_token] = false;
    ...
    function _hasEnoughBalance(
        ...
        return IERC20(_tokenAddress).balanceOf(_taxReceiver) >= _amount;
    }
    ...
    function _depositReward(
        ...
        require(_hasEnoughBalance(_tokenAddress, _taxReceiver, _amount));
        IERCMod20(_tokenAddress).approveSpending(_taxReceiver, _amount);
