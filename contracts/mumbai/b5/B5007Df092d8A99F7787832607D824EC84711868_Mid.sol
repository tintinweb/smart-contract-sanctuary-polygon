...
import {StringLib, LibString} from "../utils/StringLib.sol";
import "../interfaces/ILand.sol";
import "../constant/TokenURISetting.sol";
import "./Royalty.sol";
import "../interfaces/IMIC.sol";
import "../utils/Rand.sol";

contract Mid is Royalty, AccessControl, BaseRegistrarImplementation, Pausable, TokenURISetting, Initializable {
    struct Properties{
        uint256 birthblock;
        string rarity;
        uint256 seed;
    ...
    uint256 constant public _totalSupply = 100000;
    address private _landAddress;
    IMIC private _micToken;
    address payable _fundBank;
    ...
    constructor(address usdt_, NNS nns_, address timelockController, address erc20TransferHelper_) BaseRegistrarImplementation("MetaID", "MID", nns_) Royalty(erc20TransferHelper_){
        _usdt = usdt_;
     ...
     */
    function init(address micAddress_, address landAddress_, address payable micBank_, address payable midLandBank_, address payable alchemistBank_, address payable fundBank_) external initializer {
        _landAddress = landAddress_;
        _micToken =  IMIC(micAddress_);
        _fundBank = fundBank_;
        ROYALTY = 1 ether;
        _init(micBank_, midLandBank_, alchemistBank_, micAddress_);
    }
    ...
    function priceOracle(uint256 count) internal pure returns(uint256) {
        uint256 epoch = count / 10;// 10000
        return 200 ether * (15 ** epoch) / (10 ** epoch);
    }
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "', name, '", "description": "meta id is.",', render, '}'))));
    ...
    function mintMid(string memory name) nonReentrant external {
        (, bytes32 rootName) = _nameSplit(name);
        require(rootName == keccak256(bytes("mid")), "name must be mid land");
        require(IERC20(_usdt).allowance(_msgSender(), address(this)) >= currentPrice(), "Not enough approve tokens");
        require(_micBank != address(0) && _midLandBank != address(0) && _alchemistBank != address(0) && _fundBank != address(0), "Address cannot be 0");
        IERC20(_usdt).safeTransferFrom(msg.sender, _fundBank, currentPrice());
    ...
    function mintRoute(string memory name, address owner_) external {
        ...
        (bytes32 namePrefix, bytes32 rootName) = _nameSplit(name);
        uint256 landTokenId = _landTokenId(rootName);
        require(ILand(_landAddress).ownerOf(landTokenId) != address(0x0), "Not found on the land");
        ...
        _nameOfTokenId[tokenId] = name;
        _tokenIdOfName[name] = tokenId;
        ...
        _micToken.mint(owner_, 300 ether);
        _micToken.mint(_micBank, 200 ether);
        _micToken.mint(_midLandBank, 200 ether);
        _micToken.mint(_alchemistBank, 300 ether);

        uint256 seed = Rand.getRand(Strings.toString(block.number), tokenId);
        string memory _rarity = rarity(seed);
        _properties[tokenId] = Properties(block.number, _rarity, seed);
    }
    ...
    function getNameOfTokenId(uint256 _tokenId) public view returns (string memory) {
        return _nameOfTokenId[_tokenId];
    }
     ...
     */
    function getProperty(uint256 tokenId) public view returns (Properties memory) {
     ...
     */
    function  tokenURIByName(string memory name_) public view returns (string memory) {
        uint256 tokenId = getTokenIdOfName(name_);
        return tokenURI(tokenId);
    }

    function _nameSplit(string memory input) internal pure returns (bytes32, bytes32) {
        StringLib.slice  memory s = input.toSlice();
        StringLib.slice  memory delim = string(".").toSlice();
        require(s.count(delim) == 1, "name format error");
        StringLib.slice memory name;
        StringLib.slice memory rootName;
        s.split(delim, name);
        require(name.toString().lenOfChars() > 0 && name.toString().lenOfChars() <= 12, "name is invalid");
        s.split(delim, rootName);
        return (name.keccak(), rootName.keccak());
    }

    function rarity(uint256 seed) internal pure returns (string memory ratiryVal) {
    ...
    function setTokenURIEngine(address tokenURIEngine__) public nonReentrant {
...
contract BaseRegistrarImplementation is Ownable, ERC721Enumerable  {
    ...
    modifier ownerOfname(bytes32 baseNode) {
        require(ownerOf(uint256(baseNode)) == _msgSender(), "BaseRegistrarImplementation: Caller is not the owner");
        _;
    }
    ...
    function setResolver(bytes32 baseNode, address resolver) external ownerOfname(baseNode) {
        nns.setResolver(baseNode, resolver);
    }

    // Set the resolver for the TLD this registrar manages.
    function setRootResolver(bytes32 baseNode, address resolver) external onlyController {
        require(ownerOf(uint256(baseNode)) == address(0), "BaseRegistrarImplementation: Caller is not the controller");
        nns.setResolver(baseNode, resolver);
    }
...
abstract contract Royalty is ReentrancyGuard, Context, IncomingTransferSupport{
    ...
    address payable _micBank;
    address payable _midLandBank;
    address payable _alchemistBank;
    IERC20 _MIDCoin;
    address private _erc20TransferHelper;

    mapping(uint256 => address) private EOAOf;
    ...
    constructor(address erc20TransferHelper_) IncomingTransferSupport(erc20TransferHelper_) {
     ...
     * @dev Initializes the contract with the given parameters.
     */
    function _init(address payable micBank_, address payable midLandBank_, address payable alchemistBank_, address micAddress_) internal {
        _micBank = micBank_;
        _midLandBank = midLandBank_;
        _alchemistBank = alchemistBank_;
        _MIDCoin = IERC20(micAddress_);
    }

    /**
     * @dev Returns the mic bank address.
     */
    function micBank() public view returns (address) {
        return _micBank;
    }

    /**
     * @dev Returns the midland bank address.
     */
    function midLandBank() public view returns (address) {
        return _midLandBank;
    }

    /**
     * @dev Returns the alchemist bank address.
     */
    function alchemistBank() public view returns (address) {
        return _alchemistBank;
    }

    /**
     * @dev Returns the alchemist bank address.
     */
    function micAddress() public view returns (address) {
        return address(_MIDCoin);
    }

    /**
     * @dev Returns the previous owner of the given token.
     */
    function previousOwnerOf(uint256 tokenId) public view returns (address) {
        return EOAOf[tokenId];
    }

    /**
     * @dev Pay royalties for specified id.
     */
    function paymentOfRoyalties(uint256 tokenId) public nonReentrant {
        uint256 allowanceOf = _MIDCoin.allowance(_msgSender(), address(this));
        require(allowanceOf >= ROYALTY, "Payments on behalf of Error");
    ...
    function run(
        ...
        if(from != address(0) && to != address(0) && to != address(0x000000000000000000000000000000000000dEaD) && !to.isContract() && to != EOAOf[tokenId]){
            require(_micBank != address(0) && _midLandBank != address(0) && _alchemistBank != address(0), "Address cannot be 0");
            uint256 allowanceOfErc20TransferHelper = _MIDCoin.allowance(EOAOf[tokenId], _erc20TransferHelper);
            if (IERC20TransferHelper(_erc20TransferHelper).isModuleApproved(EOAOf[tokenId]) && allowanceOfErc20TransferHelper >= ROYALTY) {
                _handleIncomingTransfer(EOAOf[tokenId], ROYALTY, address(_MIDCoin));
            } else {
                uint256 allowanceOf = _MIDCoin.allowance(EOAOf[tokenId], address(this));
                address payer;
                if (allowanceOf >= ROYALTY) {
                    payer = EOAOf[tokenId];
                } else {
                    address payerOf = _paymentOfRoyalties[tokenId];
                    require(payerOf != address(0), "Payer Error");
                    allowanceOf = _MIDCoin.allowance(payerOf, address(this));
                    if(allowanceOf >= ROYALTY){
                        payer = payerOf;
                    }
                }
                require(payer != address(0), "Insufficient balance");
                _MIDCoin.safeTransferFrom(payer, address(this), ROYALTY);
            }
            _MIDCoin.safeTransfer(_alchemistBank, ROYALTY*3/10);
            _MIDCoin.safeTransfer(_micBank, ROYALTY*2/10);
            _MIDCoin.safeTransfer(_midLandBank, ROYALTY*5/10);
        }
        if(!to.isContract()){
            EOAOf[tokenId] = to;
        }
...
import {IERC20TransferHelper} from "../../interfaces/transferHelpers/IERC20TransferHelper.sol";
...
contract IncomingTransferSupport {
    using SafeERC20 for IERC20;

    /// @notice The ZORA ERC-20 Transfer Helper
    IERC20TransferHelper public immutable erc20TransferHelper;

    constructor(address _erc20TransferHelper) {
        erc20TransferHelper = IERC20TransferHelper(_erc20TransferHelper);
    }
    ...
    function _handleIncomingTransfer(address from, uint256 _amount, address _currency) internal {
        ...
        erc20TransferHelper.safeTransferFrom(_currency, from, address(this), _amount);
