...
import "@openzeppelin/contracts/utils/math/SafeMath.sol";
 contract PledgeStakeV2 {
    ...
    function getTotalRoundRewards(address _user) public view returns (uint256) {
      ...
      // Calculate level rewards for the user
    function getLevelReward(address _user) public view returns (uint256) {
        uint256 levelReward = 0;
        UserInfo storage user = userInfo[_user];
        uint256 userLevel = user.level;

        // Iterate through the referrals in the user's referral tree
        for (uint256 i = 0; i < user.referrals.length; i++) {
            address referral = user.referrals[i];

            for (uint256 j = 0; j < stakes[referral].length; j++) {
                StakeInfo storage stake = stakes[referral][j];
                if (!stake.unstaked) {
                    uint256 roundReward = getRoundReward(referral, j);
