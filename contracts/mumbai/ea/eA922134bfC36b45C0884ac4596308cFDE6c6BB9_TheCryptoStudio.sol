...
contract TheCryptoStudio is ERC1155
{
    ...
    string private constant MAIN_SCHEMA = "tokenID text, description text, image text, name text, animation_url text";
    ...
    string private constant SPACE_SCHEMA = "spaceName text, space_owner text, groupID text, image text, description text";
    ...
    function SocialSpaceCreation(string memory spaceName , string memory space_group , string memory imageCID, string memory description) public payable{
        require(msg.value >= spaceMintPrice);
        require(spaceInfoMap[spaceName].spaceAdmin == address(0));
        spaceInfoMap[spaceName].spaceAdmin = msg.sender;
        spaceInfoMap[spaceName].spaceArtists.add(msg.sender);
        string memory insert_statement = SQLHelpers.spaceInsertion( SPACE_TABLE_PREFIX,space_tableID,spaceName,Strings.toHexString(msg.sender),space_group,imageCID,description);
    ...
    function DeclareNFT(string memory name , string memory imageCID , string memory animationCID , string memory description ,string memory spaceName, uint256 mintPrice, bool token_type, uint256 maxSupply) public  returns(uint256) {
        ...
        string memory insert_statement =  SQLHelpers.insertMainStatement(MAIN_TABLE_PREFIX, mainTableID,tokenID.current(),description,imageCID,name,animationCID);
    ...
    function getTokenMintPrice(uint256 tokenid) public view returns(uint256){
    ...
    function getTokenRemainingSuply(uint256 tokenid) public view returns(uint256){
        return tokenInfoMap[tokenid].maxCap;
    }
    ...
    function updateAttribute(uint256 tokenid , string memory attributeName , string memory value) public {
        require(tokenid <= tokenID.current() && tokenInfoMap[tokenid].creator == msg.sender);

        // all attributes except spaceName & Creator can get updated in the attribute table
        require(!(stringsEquals(attributeName,"spaceName")) && !(stringsEquals(attributeName,"Creator")));
    ...
    function assignAnimationURI(uint256 tokenid , string memory animationURL) public {
        require(tokenInfoMap[tokenid].creator == msg.sender && tokenInfoMap[tokenid].tokenType);
    ...
    function addSpaceArtist(string memory spaceName, address artist) public {
        require(spaceInfoMap[spaceName].spaceAdmin == msg.sender);
...
library SQLHelpers {
    ...
    function insertMainStatement(string memory MAIN_TABLE_PREFIX, uint256 mainTableID,uint256 tokenid ,string memory description, string memory imageCID,string memory name, string memory animationCID) internal view returns(string memory){
    return
    toInsert(
            MAIN_TABLE_PREFIX,
            mainTableID,
            "tokenID, description, image, name, animation_url",
            string.concat(
    ...
    function spaceInsertion(string memory SPACE_TABLE_PREFIX, uint256 space_tableID, string memory  spaceName , string memory sender , string memory space_group, string memory imageCID, string memory description) internal view returns(string memory){
        return toInsert(
            SPACE_TABLE_PREFIX,
            space_tableID,
            "spaceName, space_owner, groupID, image, description",
            string.concat(
                quote(spaceName),
                ",",
                quote(sender),
...
// string private constant PROPOSAL_TABLE_PREFIX = "vocdoni_proposals";
    // string private constant PROPOSAL_SCHEMA = "spaceName text, proposalID text, proposalType text";
    function proposalInsertion(string memory PROPOSAL_TABLE_PREFIX, uint256 proposal_tableID, string memory spaceName, string memory proposalID, string memory proposalType, string memory creator) internal view returns(string memory){

        return toInsert(
            PROPOSAL_TABLE_PREFIX,
            proposal_tableID,
            "spaceName, proposalID, proposalType, creator",
            string.concat(
                quote(spaceName),
                ",",
                quote(proposalID),
