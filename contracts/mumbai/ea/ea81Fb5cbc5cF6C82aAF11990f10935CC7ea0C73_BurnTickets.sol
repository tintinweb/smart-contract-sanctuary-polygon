...
contract BurnTickets is ERC721Drop {
    ...
    uint256 public immutable maxBurn = 10;

    using Counters for Counters.Counter;
    Counters.Counter private _fablesBurnCounter;
    ...
    function burnToClaimTicket(uint256 _fablesTokenId) external {
        require(_fablesBurnCounter.current() < maxBurn, "BurnTickets: max burn reached");
        ...
        _fablesBurnCounter.increment();
        _mint(msg.sender,1);
    }

    function burnToClaimTicketArray(uint256[] memory _fablesTokenId) external {
        for (uint i = 0; i < _fablesTokenId.length; i++) {
            require(_fablesBurnCounter.current() < maxBurn, "BurnTickets: max burn reached");
            require(FablesContract.ownerOf(_fablesTokenId[i]) == msg.sender, "BurnTickets: caller is not the owner of the ticket");
            FablesContract.burn(_fablesTokenId[i]);
            _fablesBurnCounter.increment();
        }
        _mint(msg.sender, _fablesTokenId.length);
    }
    ...
    function burnTicketToClaimFables(uint256 _ticketTokenId, address _receiver) external {
        ...
        FablesContract.claim(_receiver, 1);
    }
}