...
contract GalakticGadgets is ERC1155SupplyUpgradeable, OwnableUpgradeable {
    ...
    function addUtility(
        ...
        util.id = uint64(counter.current());
    ...
    function bulkMint(
        ...
        for (uint256 i = 0; i < to.length; i++) {
            require(
                totalSupply(utilityIdx) + amount <
                    utilities[utilityIdx].maxSupply,
                'Mint: Max supply reached'
            );
     ...
     */
    function purchase(uint256 utilId, uint16 amount) external payable {
        require(
        ...
        require(
            msg.value == amount * utilities[utilId].mintPrice,
    ...
    function uri(uint256 _id) public view override returns (string memory) {
        ...
        return
            string(
                abi.encodePacked(
                    super.uri(_id),
                    utilities[_id].ipfsMetadataHash
                )
            );
    }
