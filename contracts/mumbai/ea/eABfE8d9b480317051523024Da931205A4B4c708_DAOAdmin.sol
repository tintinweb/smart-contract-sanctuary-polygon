...
contract DAOAdmin is IDAO, ReentrancyGuard, AccessControl {
    ...
    mapping(uint256 => mapping(address => bool)) private _voteStatus;
    ...
    constructor(
        ...
        for (uint256 i; i < len; i++) {
            _selectors[selectors[i]] = true;
        }
    }

    event AddWhiteList(address[] account, bool[] status);

    modifier isWhiteListM(address account) {
        if (!_isWhiteList(msg.sender)) {
            revert DAONoInWhiteList();
        }
        _;
    }
    ...
    function addProposal(
        ...
        uint256 startTime
    ) external isWhiteListM(msg.sender) {
        if (!_isValidSignature(bytes4(callData))) {
            revert DAOInvalidSignature();
        }
    ...
    function finishVote(uint256 proposalId) external nonReentrant {
        Proposal storage proposal = _proposals[proposalId];
    ...
    function addSelector(bytes4 selector) external onlyRole(DAO_ROLE) {
        _selectors[selector] = true;
    }

    function removeSelector(bytes4 selector) external onlyRole(DAO_ROLE) {
        _selectors[selector] = false;
    }

    function changeWhiteList(address[] calldata account, bool[] calldata status)
        external
        onlyRole(DAO_ROLE)
    {
    ...
    function isValidSignature(bytes4 selector) external view returns (bool) {
        return _isValidSignature(selector);
    }
    ...
    function _isValidSignature(bytes4 selector) internal view returns (bool) {
    ...
    function test(uint256 val) external {
        value = val;
    }
}
...
interface IDAO {
    ...
    error DAOInvalidSignature();
    error DAOVotingEnded();
    error DAOImpossibleVote();
    error DAOArrayLen();
    error DAONoInWhiteList();
