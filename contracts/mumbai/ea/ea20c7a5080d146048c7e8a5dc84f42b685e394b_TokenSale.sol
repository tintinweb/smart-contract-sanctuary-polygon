...
contract TokenSale {
    ...
    constructor(
        ...
        address _tokenAddress, address _marketing
    ) {
    ...
    function buyTokens(address payable referralAddress) external payable {
        ...
        if (!hasUsedReferral[msg.sender] && referralAddress != address(0) && referralAddress != msg.sender) {
    ...
    function withdrawEther() external {
        ...
        if (hasUsedReferral[msg.sender]) {
            ...
            remaining = paymentAmount-paymentAmount/2;
            marketing.transfer(remaining);
    ...
    function withdrawTokens(uint256 amount) external {
        require(msg.sender == contractOwner, "Only contract owner can withdraw tokens");
    ...
    function setTokenPrice(uint256 newTokenPrice) external {
        require(msg.sender == contractOwner, "Only contract owner can set token price");
