...
contract NFTMegaPool {
    AppStorage s;
    uint256 baseStakeValue = 1000000000000000000; // 1000
    IERC721 public nftContract;

    constructor(address _nftCollection) {
        s.nftCollection = IERC721(_nftCollection);
        nftContract = IERC721(_nftCollection);
    ...
    function totalSupply() external view returns (uint256[] memory totalSupply_) {
        totalSupply_ = s.totalSupply;
    }

    function nftCollection() external view returns (address) {
        return address(s.nftCollection);
    }
    ...
    function balanceOf(address _account) external view returns (uint256[] memory) {
    ...
    function getAPY(address _rewardToken) external view returns (uint256 apy_) {
        require(s.totalSupply.length > 0, 'No stake inside the pool, apy is not calculatable');

        RewardToken storage rewardToken = s.rewardTokens[_rewardToken];
        uint256 l_totalSupply;
        for (uint256 i = 0; i < s.totalSupply.length; i++) {
            l_totalSupply += baseStakeValue * obtainMiningPowerFactor(s.totalSupply[i]);
        }
        uint256 anoTime = 31557600; //year in seconds  decimals?  1e18

        apy_ =
            rewardToken.rewardPerTokenStored +
            (anoTime * rewardToken.rewardRate * 1e18) /
            l_totalSupply;
    }
    ...
    function internalEarned(
        ...
        RewardToken storage rewardToken = s.rewardTokens[_rewardToken];
        uint256 nftStakeValue;
        for (uint256 i = 0; i < s.balances[_account].length; i++) {
            nftStakeValue += baseStakeValue * obtainMiningPowerFactor(s.balances[_account][i]);
        }
        return
            (nftStakeValue *
    ...
    //     IERC721 l_nftCollection = s.nftCollection;
    //     s.totalSupply += _amount;
    //     s.balances[msg.sender] += _amount;
    //     emit Staked(msg.sender, _amount);
    //     permit
    //     IERC20Permit(address(l_nftCollection)).permit(msg.sender, address(this), _amount, _deadline, _v, _r, _s);

    //     SafeERC20.safeTransferFrom(l_nftCollection, msg.sender, address(this), _amount);
    ...
    function stake(uint256[] memory tokenIds) external {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            s.totalSupply.push(tokenIds[i]);
            s.balances[msg.sender].push(tokenIds[i]);
    ...
    function remove(uint256 _index, uint256[] storage arr) internal {
    ...
    function withdraw(uint256[] memory tokenIds) public {
        ...
        require(
            tokenIds.length <= s.balances[msg.sender].length,
            "Can't withdraw more than staked"
        );
        if (tokenIds.length == s.balances[msg.sender].length) {
            for (uint256 y = 0; y < tokenIds.length; y += 1) {
                for (uint256 x = 0; x < s.totalSupply.length; x += 1) {
                    if (s.totalSupply[x] == tokenIds[y]) {
                        remove(x, s.totalSupply);
                        // delete s.totalSupply[x];
                    }
                }
            }
            ...
            for (uint256 y = 0; y < tokenIds.length; y += 1) {
                ...
                for (uint256 z = 0; z < s.balances[msg.sender].length; z += 1) {
                    if (s.balances[msg.sender][z] == tokenIds[y]) {
                        // delete s.balances[msg.sender][z];
                        remove(z, s.balances[msg.sender]);
                    }
                }
            }
        }
    ...
    function exit() external {
        getRewards();
        uint256[] memory tokenIds = s.balances[msg.sender];
    ...
    function obtainMiningPowerFactor(uint256 tokenId) internal pure returns (uint256) {
        if (tokenId <= 10) {
            //Brilliant 3 Dimensions 100 % Mining Power
            return 100;
        } else if (tokenId <= 40) {
            //Brilliant 2 Dimensions 95 % Mining Power
            return 95;
        } else if (tokenId <= 90) {
            //Brilliant 1 Dimensions 90 % Mining Power
            return 90;
        } else if (tokenId <= 150) {
            //Diamond 3 Dimensions 85 % Mining Power
            return 85;
        } else if (tokenId <= 300) {
            //Diamond 2 Dimensions 80 % Mining Power
            return 80;
        } else if (tokenId <= 600) {
            //Diamond 1 Dimensions 75 % Mining Power
            return 75;
        } else if (tokenId <= 1000) {
            //Rubin 3 Dimensions 70 % Mining Power
            return 70;
        } else if (tokenId <= 1500) {
            //Rubin 2 Dimensions 65 % Mining Power
            return 65;
        } else if (tokenId <= 2100) {
            //Rubin 1 Dimensions 60 % Mining Power
            return 60;
        } else if (tokenId <= 2700) {
            //Opal 3 Dimensions 55 % Mining Power
            return 55;
        } else if (tokenId <= 3300) {
            //Opal 2 Dimensions 50 % Mining Power
            return 50;
        } else if (tokenId <= 3900) {
            //Opal 1 Dimensions 45 % Mining Power
            return 45;
        } else if (tokenId <= 4500) {
            //Saphir 3 Dimensions 40 % Mining Power
            return 40;
        } else if (tokenId <= 5100) {
            //Saphir 2 Dimensions 35 % Mining Power
            return 35;
        } else if (tokenId <= 5700) {
            //Saphir 1 Dimensions 30 % Mining Power
            return 30;
        } else if (tokenId <= 6300) {
            //Smaragd 3 Dimensions 25 % Mining Power
            return 25;
        } else if (tokenId <= 6900) {
            //Smaragd 2 Dimensions 20 % Mining Power
            return 20;
        } else if (tokenId <= 7500) {
            //Smaragd 1 Dimensions 15 % Mining Power
            return 15;
        } else {
            return 0;
        }
