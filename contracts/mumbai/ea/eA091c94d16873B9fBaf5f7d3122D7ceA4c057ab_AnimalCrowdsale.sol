...
contract AnimalCrowdsale is AccessControl, PullPayment, Ownable {
    ...
    AnimalState public state;

    uint256 public sLimit = 10; //single mint limit
    uint256 public mLimit = 500; //total mining limit
    uint256 public salePrice = 1.5 ether;
    ...
    event StateChange(AnimalState state);
    ...
    modifier isState(AnimalState _state) {
        require(state == _state, "AnimalCrowdsale: Wrong state for this action");
        _;
    }
    function mint(uint256 _potionId, uint256 _halflingId) external isState(AnimalState.ALLOW){
    ...
    function pubMint(uint256 _amount) external payable isState(AnimalState.PUBLIC) {
        require(
            _amount <= sLimit,
            "AnimalCrowdsale:Exceeded the single purchase limit"
        );
        require(
            msg.value == _amount.mul(salePrice),
            "AnimalCrowdsale:Payment declined"
        );
        mAmount = mAmount.add(_amount);
        require(
            mAmount <= mLimit,
            "AnimalCrowdsale:Exceeded the total amount of mining"
        );
    ...
    function setCollector(address _collector) external onlyOwner {
        require(_collector != address(0), "AnimalCrowdsale:Invalid address");
        collector = _collector;
    ...
    function setState(AnimalState _state) external onlyRole(CROWD_ROLE) {
        state = _state;
    ...
    NativeMetaTransaction
{
    ...
    constructor() ERC721A("POTION", "POTION") {
        _initializeEIP712("POTION");
        baseTokenURI = "https://cdn.nftstar.com/potion/metadata/";
        collectionURI = "https://cdn.nftstar.com/potion/meta-potion.json";
        // Grant the contract deployer the default admin role: it will be able to grant and revoke any roles
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSenderERC721A());
        _setupRole(MINER_ROLE, _msgSenderERC721A());
        _setupRole(PAUSER_ROLE, _msgSenderERC721A());
        ...
        onlyRole(MINER_ROLE)
    {
        require(
            _accounts.length == _quantities.length,
            "POTION: The two arrays are not equal in length"
        );
        for (uint256 index = 0; index < _accounts.length; index++) {
            _mint(_accounts[index], _quantities[index]);
        }
    ...
    function pause() public virtual {
        require(
            hasRole(PAUSER_ROLE, _msgSenderERC721A()),
            "POTION: must have pauser role to pause"
        );
        _pause();
    }
    ...
    function unpause() public virtual {
        require(
            hasRole(PAUSER_ROLE, _msgSenderERC721A()),
            "POTION: must have pauser role to unpause"
        );
        _unpause();
    }
