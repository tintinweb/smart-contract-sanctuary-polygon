...
/******************************************************************************\
/// @title Sipher Portal - Exit Facet 
/// @author Ather Labs
...
contract ExitFacet is IOperations, ReentrancyGuard {
    ...
    event ExitAsset(address caller, address defaultReceiver, uint8 listingId, uint88[] _tokenIds, uint128[] _amounts);
    ...
    /// @dev This function is guarded by the nonReentrant modifier.
    /// @dev Only available when MassExit mode is activated.
    /// @dev User's balance will be grouped into array of 100 token Id at max.
    ...
    /// @param _listingId Listing Id of the expired deposit record.
    /// @param _tokenIds Array of tokenId of the expired deposit record.
    /// @param _amounts Amounts of the expired deposit record.
    ...
    function exitAsset(
        ...
        LibStatusControl._requireExitEnabled();
        require(_tokenIds.length == _amounts.length, "ExitFacet: tokenIds and amounts length mismatch");
        require(_tokenIds.length > 0 && _tokenIds.length < 101, "ExitFacet: Invalid data length");
        ...
        // Invalidate the key and release assets.
        ...
        emit ExitAsset(msg.sender, _defaultReceiver, _listingId, _tokenIds, _amounts);
    }
}
