...
interface IBondHub {
    ...
    function createProfile(
     ...
     /**
     * @notice a ApplicationFollowModule clone is deployed.
     * @param ownerAddress The address of application owner.
     * @param id The Id of application.
     */
    function deployApplicationFollowModule(
        address ownerAddress,
        string memory id
    ...
    /**
     * @notice Returns the application follow module implementation address.
     *
     * @return address The application follow module implementation address.
     */
    function getApplicationFollowModuleImpl() external view returns (address);

    /**
     * @notice Returns the hub proxy address in layerzero.
     *
     * @return address The address of the hub proxy  in layerzero.
     */
    function getLzHubProxy() external view returns (address);

    /**
     * @notice the hub proxy address in layerzero to set.
     *
     * @param lzHubProxy  The address of the hub proxy  in layerzero.
     */
    function setLzHubProxy(address lzHubProxy) external;
...
library Errors {
    ...
    error NotGovernance();
    error NotGovernanceOrLzHubProxy();
...
/**
     * @dev Emitted when the LzHubProxy address is changed. We emit the caller even though it should be the previous
     * LzHubProxy address, as we cannot guarantee this will always be the case due to upgradeability.
     *
     * @param caller The caller who set the LzHubProxy address.
     * @param prevLzHubProxy The previous LzHubProxy address.
     * @param newLzHubProxy The new LzHubProxy address set.
     * @param timestamp The current block timestamp.
     */
    event LzHubProxySet(
        address indexed caller,
        address indexed prevLzHubProxy,
        address indexed newLzHubProxy,
        uint256 timestamp
    );
    ...
    /**
     * @dev Emitted when a ApplicationFollowModule clone is deployed using a lazy deployment pattern.
     *
     * @param ownerAddress The address of application owner.
     * @param id The Id of application.
     * @param timestamp The current block timestamp.
     */
    event ApplicationFollowModuleDeployed(
...
import {IFollowModule} from '../../interfaces/IFollowModule.sol';
import {Events} from "../../libraries/Events.sol";
import { ApplicationFollowModule } from "../../core/modules/follow/ApplicationFollowModule.sol";
import { TransparentUpgradeableProxy } from "../../upgradeability/TransparentUpgradeableProxy.sol";
import { TokenDeployer} from './tack.sol';
import "./DataTypes.sol";

contract LzBondHub is NonblockingLzAppUpgradeable {
    ...
    //address  public _appFollowModule;
    TokenDeployer public immutable deployer;
    constructor(address _lzEndpoint, address hub,TokenDeployer _deployer) {
        HUB = hub;
        deployer = _deployer;
    ...
    function _nonblockingLzReceive(
        ...
        counter += 1;
        //  if (HUB == address(0)) revert Errors.NotHub();
        // (address ownerAddress, string memory id) = abi.decode(_payload, (address, string  ));
        //  _ownerAddress = ownerAddress;
        //  _id = id;
        // _user = msg.sender;
        //_appFollowModule = address(new ApplicationFollowModule(HUB,ownerAddress,id));
        //_deployApplicationFollowModule(ownerAddress,id);
        //_followModule = IBondHub(HUB).getFollowNFTImpl();
        //IBondHub(HUB).whitelistFollowModule(followModule, whitelist);
        //_proxy = IBondHub(HUB).deployApplicationFollowModule(_ownerAddress,_id);
        //_proxy = address(new TackToken());

         (
            uint256 pid,
            address owner,
            string memory metadataURI,
            DataTypes.TaxableTokenData memory args
        ) = abi.decode(
                _payload,
                (uint256, address, string, DataTypes.TaxableTokenData)
            );

        deployer.lzDeploy(pid, owner, metadataURI, args);
    }
    ...
    //     bytes memory payload = abi.encode(ownerAddress,id);
    //     return
    ...
    function estimateFees(
        ...
        DataTypes.TaxableTokenData calldata args,
        bytes calldata _adapterParams
    ) external view returns (uint nativeFee, uint zroFee) {
        bytes memory payload = abi.encode(pid, owner, metadataURI, args);
    ...
    function _deployApplicationFollowModule(
        address ownerAddress,
        string memory id
    ) private returns (address) {
       
        address appFollowModule = address(new ApplicationFollowModule(HUB,ownerAddress,id));
        return appFollowModule;

        //  bytes memory functionData = abi.encodeWithSelector(
        //     ApplicationFollowModule.initialize.selector,
        //     ownerAddress,
        //     id
        // );

        // address proxy = address(new TransparentUpgradeableProxy(appFollowModule,ownerAddress,functionData));
        // emit Events.ApplicationFollowModuleDeployed(ownerAddress, id, block.timestamp);
        //return proxy;
    }
     function localDeployApplicationFollowModule(
        address ownerAddress,
        string memory id
    ) external returns (address) {
        return _deployApplicationFollowModule(ownerAddress,id);
    }
}
...
contract TackToken {
    address public immutable _name;
    constructor(address sname) {
        _name = sname;
    }
}
...
contract TokenDeployer {
    ...
    function deploy(
        ...
        string calldata metadataURI,
        DataTypes.TaxableTokenData calldata args
    ) public returns (TaxableToken) {
        TaxableToken token = new TaxableToken(args);

        token.transferOwnership(owner);

        emit Deployed(owner, pid, address(token), metadataURI, block.timestamp);
    ...
    function lzDeploy(
        ...
        string calldata metadataURI,
        DataTypes.TaxableTokenData calldata args
    ) external returns (TaxableToken) {
        return deploy(pid, owner, metadataURI, args);
    }
}
...
abstract contract LzAppUpgradeable is Initializable, OwnableUpgradeable, ILayerZeroReceiverUpgradeable, ILayerZeroUserApplicationConfigUpgradeable {
    ...
    function isTrustedRemote(uint16 _srcChainId, bytes calldata _srcAddress) external view returns (bool) {
        console.log(" %s %s",owner(),_msgSender());
