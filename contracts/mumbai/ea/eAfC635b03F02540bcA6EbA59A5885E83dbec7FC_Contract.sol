...
contract Contract is ERC1155Base, Permissions, KeeperCompatible {

    uint256 private immutable interval = 1800;
    ...
    function performUpkeep(bytes calldata performData) external override {
        (address[] memory addresses, uint256[] memory ids) = abi.decode(
            performData,
            (address[], uint256[])
        );
        
        if ((block.timestamp - lastTimeStamp) > interval ) {
            airdrop(addresses, ids);
            lastTimeStamp = block.timestamp;
        }
    }

    function airdrop(address[] memory addresses, uint256[] memory ids) internal {
        require(addresses.length == ids.length, "Airdrop error - array sizes mismatch.");
        for(uint256 i = 0; i < addresses.length; i++) {
            safeTransferFrom(msg.sender, addresses[i], ids[i], 1, "");
        }
    }
