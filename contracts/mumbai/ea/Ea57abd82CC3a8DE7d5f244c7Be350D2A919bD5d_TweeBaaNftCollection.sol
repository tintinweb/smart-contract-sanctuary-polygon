...
*/
contract TweeBaaNftCollection is Ownable, ERC1155, ERC1155Metadata_URI {
    ...
    function createBatch(uint256[] calldata _initialSupplies, string[] calldata _uriValues) external onlyOwner {

        require(_initialSupplies.length == _uriValues.length, "_ids and _values array length must match.");

        for (uint256 i = 0; i < _initialSupplies.length; ++i) {
            create(_initialSupplies[i], _uriValues[i]);
        }
    }
    ...
    function ownerBurn(address _from, uint256 _id, uint256 _value) external onlyOwner  {
