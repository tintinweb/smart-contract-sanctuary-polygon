...
// MetaAnchorFactory, MetaAnchor, AppHub and AppContract is UNLICENSED, visit metaanchor.io for licensing.
// OpenZeppelin is licensed under MIT
// Meta Anchor (TM) and Digital Soul (TM) are Registered Trademarks and will be denoted as MetaAnchor and DigitalSoul subsequently.
...
abstract contract DeployerContract is IDeployedContract {
    ...
    function _deployBinary(bytes memory bytecode, bytes32 salt) internal onlyDeployer() returns (address) {
        address addr;
...
pragma solidity ^0.8.18;
/**
 * @title AppHub for access-control management in linked contracts
 * @author [email protected] / lm
 * @notice Used to manage roles and call functions across MetaAnchorFactory and the factory-deployed Meta Anchor (TM) contracts.
 */
contract AppHub is AccessControl, DeployerContract {
  ...
  /**
   * @notice MINTER_ROLE has permissions to safeMint, dropAnchor,... in MetaAnchor-contract. 
   * @return Role hash, as should be passed to hasRole(), grantRole()
   */
  bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
  /**
   * @notice BURNER_ROLE has permission to burn tokens from MetaAnchor-contracts
   * @dev see EIP-5484, only has effect for BurnAuth.Both or BurnAuth.IssuerOnly
   * @return Role hash, as should be passed to hasRole(), grantRole()
   */
  ...
  /**
   * @notice MAINTAINER_ROLE can execute maintainance-tasks in a MetaAnchor-Contract and Factory, e.g. updateValidAnchors(), setBurnAuth(), setClaimsPerAnchor() etc.
   * @return Role hash, as should be passed to hasRole(), grantRole()
   */
