...
contract ArtTokenUpgradable is ERC721AUpgradeable, OwnableUpgradeable {
    struct AllowList {
        bool enabled;
        uint256 totalMinted;
        uint256 maxToMint;
    ...
    mapping(address => AllowList) private _allowlist;
    ...
    {
        isOpen = false;
        closedBaseURI = "ipfs://QmXsWKyr4DnBCY8n6VohJ9Ev1WREH11sMSCwTSm4qCzwQg";
        maxPerMint = 200;
    ...
    function addAllowlist(address _account, uint256 _maxToMint)
        public
        onlyOwner
    {
        _allowlist[_account].enabled = true;
        _allowlist[_account].maxToMint = _maxToMint;
    }

    function removeAllowlist(address _account) public onlyOwner {
        _allowlist[_account].enabled = false;
    }
    ...
    function mint(uint256 _quantity) external payable {
        AllowList memory allowList = _allowlist[_msgSender()];

        if (isAllowlistEnabled) {
            require(allowList.enabled, "not in allowlist");
            require(
                _quantity <= (allowList.maxToMint - allowList.totalMinted),
        ...
        require(_quantity <= maxPerMint, "max per mint exceeded");

        _safeMint(msg.sender, _quantity);

        _allowlist[_msgSender()].totalMinted += _quantity;
    }
        ...
        returns (string memory)
    {
        ...
        if (isOpen) {
            return
                bytes(openBaseURI).length != 0
                    ? string(abi.encodePacked(openBaseURI, _toString(tokenId)))
                    : "";
        }

        return bytes(closedBaseURI).length != 0 ? closedBaseURI : "";
    }
