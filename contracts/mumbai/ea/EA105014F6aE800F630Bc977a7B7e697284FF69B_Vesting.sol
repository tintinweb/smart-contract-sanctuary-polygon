...
contract Vesting {
    ...
    uint256 constant TGE_UNLOCK = 10; // 10% : released percent at TGE stage
    uint256 constant UNLOCK_UNIT = 30; // 30% of the total allocation will be unlocked
    uint256 constant CLIFF_PERIOD = 1 hours; // cliff period
    uint256 constant vestingPeriod = 3 hours; // rest vesting period
    ...
    function getVested(address beneficiary) public view virtual returns (uint256 _amountVested) {
        ...
        if (block.timestamp > firstVestingPoint && block.timestamp <= secondVestingPoint) {
            vestedPercent = TGE_UNLOCK.add(UNLOCK_UNIT) + (block.timestamp - firstVestingPoint).mul(90-UNLOCK_UNIT).div(vestingPeriod);
