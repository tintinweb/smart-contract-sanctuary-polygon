...
contract NFT is ERC721URIStorage {
    ...
    constructor(address _marketplace) ERC721("LN Token", "LNW") {
    ...
    event Mint(address indexed creator, uint256 tokenId);
    ...
    function setTokenRoyalty(
        ...
        require(
            totalRoyalty <= 10,
            "setTokenRoyalty: royalty must be between 0 an 10"
        );
        royalties[_id] = RoyaltyInfo(recipient, value);

        emit RoyaltyUpdated(_id, value, recipient);
    }
    ...
    function mint(string memory tokenURI) public returns (uint256) {
        ...
        _beforeTokenMint(newTokenId, tokenURI);
    ...
    //! we can change creator after setting creator
    function setCreator(uint256 id, address _creator) external {
        creator[id] = _creator;
    }

    function _beforeTokenMint(uint256 _id, string memory _tokenURI)
        internal
        view
    {
        require(creator[_id] == address(0), "Token is already minted");
