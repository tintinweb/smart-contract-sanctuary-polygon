...
contract nftPreSale is Ownable {
    ...
    function buyNft(uint256 _amount) external {
        require(config.nftToken.MinerList(address(this)), "e001");
        //不能超过总量
        require(config.idoAmount.add(_amount) <= config.totalAmount, "e002");
        //每个人不能超过10个
        require(userIdoAmountList[msg.sender].add(_amount) <= config.maxAmountPerUser, "e003");
