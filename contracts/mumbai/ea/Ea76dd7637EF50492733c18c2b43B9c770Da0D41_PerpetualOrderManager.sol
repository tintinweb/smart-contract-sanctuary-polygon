...
contract PerpetualOrderManager is PerpetualTradeFunctions, IFunctionList, IPerpetualOrderManager {
    function isOrderExecuted(bytes32 digest) external view override returns (bool) {
        return executedOrders[digest];
    }

    function isOrderCanceled(bytes32 digest) external view override returns (bool) {
        return canceledOrders[digest];
    }
    ...
    function getFunctionList() external pure virtual override returns (bytes4[] memory, bytes32) {
        ...
        functionList[0] = this.isOrderExecuted.selector;
        functionList[1] = this.isOrderCanceled.selector;
...
contract PerpetualTradeFunctions is PerpetualRebalanceFunctions, PerpetualHashFunctions {
    ...
    using ConverterDec18 for uint16;
    ...
    function _checkTradePreCond(IPerpetualOrder.Order memory _order) internal view returns (bool) {
        ...
        require(perpetual.state == PerpetualState.NORMAL, "state should be NORMAL");
        return true;
    }
     ...
     */
    function _trade(IPerpetualOrder.Order memory _order, bytes32 _digest) internal returns (bool) {
        // console.log("contract: _trade");
        ...
        PerpetualData storage perpetual = _getPerpetual(_order.iPerpetualId);
        {
            ...
            _doMarginCollateralActions(perpetual, fTraderPos, fPrice, _order);
            ...
            fPnLCC = _getTradeLogic().executeTrade(
                perpetual.id,
                _order.traderAddr,
                fTraderPos,
                _order.fAmount,
        ...
        // distribute fees
        int128 fFee = _getTradeLogic().distributeFees(_order, !isClose);
        fPnLCC = fPnLCC.sub(fFee);
        require(
            _isTraderMarginSafe(perpetual, _order.traderAddr, !isClose),
            "trader margin unsafe"
        );
        {
            bytes16 positionId = marginAccounts[perpetual.id][_order.traderAddr].positionId;
            emit Trade(
                perpetual.id,
                _order.traderAddr,
                positionId,
    ...
    function _doMarginCollateralActions(
        ...
        if (
            (_fTraderPos.add(_order.fAmount).abs() < _perpetual.fLotSizeBC) ||
        ...
        uint16 totalFeeRateTbps = _setExchangeFee(_order);
        // determine and set fee for broker + update the volume EMA relevant for fees
        totalFeeRateTbps = totalFeeRateTbps + _setBrokerFeeAndUpdateVolumeEMA(_order);
        ...
        int128 fTargetLeverage = isFlip || isOpen ? (_order.leverageTDR).TDRToABDK() : int128(0);
        ...
        int128 fTotalFee;
        {
            ...
            int128 fTradeAmountCC = _order.fAmount.abs().mul(fx);
            fTotalFee = fTradeAmountCC.mul(ConverterDec18.tbpsToABDK(totalFeeRateTbps));
            if (_order.referrerAddr != address(0x0)) {
                fTotalFee = fTotalFee.add(_perpetual.fReferralRebateCC);
            }
        }
        fDeposit = fDeposit.add(fTotalFee);
        if (fDeposit > 0) {
            // can revert:
            _getMarginLogic().depositMarginForOpeningTrade(_perpetual.id, fDeposit, _order);
        } else if (fDeposit < 0) {
            _getMarginLogic().reduceMarginCollateral(
                _perpetual.id,
                _order.traderAddr,
                fDeposit.abs()
            );
    ...
    function _doOpeningTradeActions(
        ...
        int128 maxTradeDelta = _getTradeLogic().getMaxSignedOpenTradeSizeForPos(
            _perpetual.id,
            _fTraderPos,
            _order.fAmount > 0
        );

        require(_order.fAmount.abs() <= maxTradeDelta.abs(), "Trade amt>max amt for trader/AMM");
        marginAccounts[_perpetual.id][_order.traderAddr].iLastOpenTimestamp = uint64(
            block.timestamp
        );
        if (_fTraderPos == 0) {
            // trader opens a new position, generate position id
            marginAccounts[_perpetual.id][_order.traderAddr].positionId = bytes16(
                keccak256(abi.encodePacked(block.timestamp, _fPrice))
            );
        }
    }
     ...
     */
    function _executeCancelOrder(uint24 _perpetualId, bytes32 _digest) internal {
        require(!canceledOrders[_digest], "order was canceled");
        canceledOrders[_digest] = true;
        emit PerpetualLimitOrderCancelled(_perpetualId, _digest);
    }

    /**
     * Set exchange fee in margin account based on broker and trader credentials
     * Reverts if signature of broker present but invalid.
     * @param   _order            order struct
     * @return exchange fee in tenth of a basis points
     */
    function _setExchangeFee(IPerpetualOrder.Order memory _order) internal returns (uint16) {
        uint16 fee = _getBrokerFeeLogic().determineExchangeFee(_order);
        marginAccounts[_order.iPerpetualId][_order.traderAddr].feeTbps = fee;
        return fee;
    }

    /**
     * Broker fee is updated in the margin struct of the trader.
     * We update the volume of trader and broker
     * Pre-condition: _setExchangeFee is called (check of broker signature & revert if invalid)
     * @param _order            order struct
     * @return broker fee in tenth of a basis points
     */
    function _setBrokerFeeAndUpdateVolumeEMA(IPerpetualOrder.Order memory _order)
        internal
        returns (uint16)
    {
        // console.log("_setBrokerFeeAndUpdateVolumeEMA");
