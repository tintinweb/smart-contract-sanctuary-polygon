...
contract UserProfiles {
    ...
    */
    function logPost(address posterAddress, uint256 tipPerTag, address[] calldata tipReceivers, bool isComment) public onlyAdmins {
    ...
    */
    function getUserDetails(address usrAddress) public view returns(string[] memory){
        ...
        string[] memory userDetails = new string[](16);
        ...
        userDetails[6] = usrProfileMap[usrAddress].userAvatar.avatarMetadata;
        userDetails[7] = KUtils.addressToString(usrProfileMap[usrAddress].userAvatar.avatarContract);
        userDetails[8] = usrProfileMap[usrAddress].uri;
        userDetails[9] = usrProfileMap[usrAddress].bio;
        userDetails[10] = usrProfileMap[usrAddress].location;
        userDetails[11] = KUtils.toString(usrProfileMap[usrAddress].joinBlock);
        userDetails[12] = KUtils.toString(usrProfileMap[usrAddress].joinTime);
        userDetails[13] = KUtils.toString(usrProfileMap[usrAddress].followLimit);
        userDetails[14] = KUtils.toString(usrProfileMap[usrAddress].userStats.tipsReceived);
        userDetails[15] = KUtils.toString(usrProfileMap[usrAddress].userStats.tipsSent);
    ...
    */
    ...
    function unfollowUser(address addressRequester, address addressToUnfollow) public onlyAdmins {
        ...
        string memory addressString = KUtils.addressToString(addressToUnfollow);

        // Make sure it's not themselves
        require(!KUtils.stringsEqual(addressString, KUtils.addressToString(addressRequester)), "not a thing");
        ...
        uint256 place = 0;
        string memory addressToUnfollowStr = KUtils.addressToString(addressToUnfollow);
        for (uint i=0; i < usrProfileMap[addressRequester].following.length; i++) {
            if (KUtils.stringsEqual(KUtils.addressToString(usrProfileMap[addressRequester].following[i]), addressToUnfollowStr)){
                place = i;
                break;
            }
        }
