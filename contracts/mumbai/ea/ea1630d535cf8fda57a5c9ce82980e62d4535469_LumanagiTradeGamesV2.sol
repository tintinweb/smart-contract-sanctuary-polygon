 ...
 */
contract LumanagiTradeGamesV2 is Ownable, Pausable, ReentrancyGuard, AutomationCompatibleInterface  {
    ...
    bool public needGenesisLock = false;
    
    uint256 public _nextGenesisLockOnceExecuteTimeStamp;
    ...
    event CheckUpKeepChainLink(uint256 indexed epoch);
    ...
    /// @dev this internal method is called to check whether to call the executeRound function is needed
    function checkExecuteRoundNeeded() internal view returns (bool status) {
        Round storage round = rounds[currentEpoch - 1];
        if (block.timestamp >= round.closeTimestamp && (block.timestamp < (round.closeTimestamp + bufferSeconds))){
            status = true;
        }
        else status = false;
    ...
    {
        if (checkExecuteRoundNeeded() == true){
            upkeepNeeded = true;
        }
    ...
    function automateRounds() external whenNotPaused onlyOperator {
        while(_paused == false){
            Round storage round = rounds[currentEpoch - 1];
            if(block.timestamp >= round.closeTimestamp && (block.timestamp < (round.closeTimestamp + bufferSeconds))){
                executeRound();
     ...
     */
    function executeRound() internal {  //whenNotPaused onlyOperator
        require(
     ...
     * @notice Start the next round n, lock price for round n-1, end round n-2
     * @dev Callable by operator
     * Demo Function to test temporary - Just in case Chainlink Upkeep fails to execute we can try this function 
     to execute it manually and find the error
     */
    function executeRoundManual() external whenNotPaused onlyOperator {
        require(
            genesisStartOnce && genesisLockOnce,
            "Can only run after genesisStartRound and genesisLockRound is triggered"
        );
     ...
     * @dev Callable by operator
     */
    function genesisLockRound() external whenNotPaused onlyOperator {   //whenNotPaused onlyOperator
    ...
    function genesisStartRound() external whenNotPaused onlyOperator {
        ...
        _nextGenesisLockOnceExecuteTimeStamp = block.timestamp + intervalSeconds;
        needGenesisLock = true;
    }
