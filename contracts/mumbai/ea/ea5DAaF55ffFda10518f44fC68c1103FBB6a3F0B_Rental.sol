...
contract Rental is Initializable, ReentrancyGuardUpgradeable {
    ...
    //     require(_timeCheck(_rewardId), "not claim time");
    //     _;
    // }

    modifier isCatorgyValid(uint256 _landCatorgy, uint256 _lordCatory) {
        require(
            _landCatorgy <= uint256(3) && _lordCatory <= uint256(3),
            "not valid catory"
        );
        _;
    }
    ...
    function setPool(
        ...
        require(_poolId > availablePoolId, "not valid PoolId");
        availablePoolId = _poolId;
        uint256 poolEndTime = _poolTimeSlot * _poolMonth;
    ...
    function depositLandLords(
        ...
        // isMerkelProofValid(_landId, _lordId, _landCatorgy, _lordCatory)
        ...
        while (!loop) {
            if (_currentPoolId == landLordsInfo[_rewardId].currentPoolId) {
                (
                    ...
                    uint256 time,
                    uint256 claims
                ) = _rewardForCurrentPool(_currentPoolId, _rewardId);
                claimAmount += reward;
    ...
    function _rewardForCurrentPool(uint256 _poolId, uint256 _rewardId)
        public
        view
        returns (
            uint256,
        ...
        )
    {
        ...
        uint256 currentMonth = _currentMonth(_poolId) - 1;
        ...
        for (
            ...
            uint256 monthTime = poolInfo[poolId].poolEndTime -
                (poolInfo[poolId].poolMonth -
                    userClaimPerPool[_rewardId][poolId] +
                    1) *
                poolInfo[poolId].poolTimeSlot;
    ...
    function _rewardsForPreviousPool(uint256 poolId, uint256 _rewardId)
        public
        view
    ...
    function _calcuatePoolId() internal view returns (uint256 poolId) {
        for (uint256 i = 0; i < availablePoolId; i++) {
            if (poolInfo[i].poolEndTime > block.timestamp) {
                return i + 1;
            } else {
        ...
        returns (uint256)
    {
        ...
        uint256 weight;
        while (loop) {
            weight = poolInfo[_poolId].poolTotalWeight[_month - 1];
            if (weight > 0) {
                loop = true;
            } else {
                month -= 1;
