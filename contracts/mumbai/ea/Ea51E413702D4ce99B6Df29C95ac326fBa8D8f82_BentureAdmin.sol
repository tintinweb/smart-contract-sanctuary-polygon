...
contract BentureAdmin is IBentureAdmin, ERC721, Ownable {
    ...
    modifier onlyFactory() {
        require(msg.sender == _factoryAddress, "BentureAdmin: caller is not a factory!");
        _;
    }
    ...
    constructor(address factoryAddress_) ERC721("", "") {
        require(factoryAddress_ != address(0), "BentureAdmin: factory address can not be zero address!");
    ...
    function checkOwner(address user) external view {
        require(user != address(0), "BentureAdmin: zero address is an invalid user!");
        require(_holderToId[user] != 0, "BentureAdmin: user does not have an admin token!");
    }
    ...
    function verifyAdminToken(address user, address ERC20Address) external view {
        require(user != address(0), "BentureAdmin: user can not have a zero address!");
        require(ERC20Address != address(0), "BentureAdmin: token can not have a zero address!");
        ...
        require(_idToHolder[id] == user, "BentureAdmin: user does not have an admin token!");
    }
    ...
    function getControlledAddressById(uint256 tokenId) external view returns (address) {
        require(_adminToControlled[tokenId] != address(0), "BentureAdmin: no controlled token exists for this admin token!");
    ...
    function setControlledAddress(uint256 tokenId, address ERC20Address) internal onlyFactory {
        require(ERC20Address != address(0), "BentureAdmin: controlled token can not have a zero address!");
        _requireMinted(tokenId);
    ...
    function mintWithERC20Address(address to, address ERC20Address) external onlyFactory {
        require(to != address(0), "BentureAdmin: admin token mint to zero address is not allowed!");
        require(ERC20Address != address(0), "BentureAdmin: controlled token can not have a zero address!");
        require(!_usedControlled[ERC20Address], "BentureAdmin: only a single admin token is allowed for a single controlled token!");
        _tokenIds.increment();
    ...
    function burn(uint256 tokenId) external {
        require(ownerOf(tokenId) == msg.sender, "BentureAdmin: only owner of the token is allowed to burn it!");
        _requireMinted(tokenId);
