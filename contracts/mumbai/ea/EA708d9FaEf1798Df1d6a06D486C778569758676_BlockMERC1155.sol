...
contract BlockMERC1155 is ERC1155, Ownable {
    ...
    function mint (string memory tokenName,uint256 cost, uint256 amountToCreator,uint256 amountToPool, string memory uri)
        public
        onlyCreator
    {
        ...
        _mint(msg.sender, id, amountToCreator, "");
         _mint(pool, id, amountToPool, "");
         tokensAvailable[id]=amountToPool;
         emit onMint(msg.sender,tokenName, id, cost, amountToPool+amountToCreator );
    ...
    function buyTokens(uint _tokenId, uint _amount) payable public returns(bool) {
        ...
        payable(tokenCreator).transfer(amountToTransferToCreator);
            ...
            _safeTransferFrom(pool,msg.sender,_tokenId,_amount,"");
