...
contract Booster {
    ...
    uint256 public stakerIncentive = 450; //incentive to psdn stakers
    ...
    address public stakerRewards; //psdn rewards
    address public lockRewards; //psdnOcean rewards(ocean)
    address public lockFees; //psdnOcean veocean fees
        ...
        external
    {
        ...
        //has a means to redeploy and mint ocean via rewardClaimed()
    ...
    function setFeeInfo(address _feeDistro, address _lockFees) external {
        require(msg.sender == feeManager, "!auth");
    ...
    function setFees(
        ...
        uint256 total = _stakerFees.add(_callerFees).add(_platform);
    ...
    function _distributeToken(IERC20 _token, uint256 _amount) internal {
        if (_amount > 0) {
            uint256 _stakerIncentive = _amount.mul(stakerIncentive).div(
                FEE_DENOMINATOR
            );
            ...
            _amount = _amount.sub(_callIncentive).sub(_stakerIncentive);

            //send incentives for calling
            _token.safeTransfer(msg.sender, _callIncentive);

            //send lockers' share of ocean to reward contract
            _token.safeTransfer(lockRewards, _amount);
            IRewards(lockRewards).queueNewRewards(_amount);

            //send stakers's share of ocean to reward contract
            _token.safeTransfer(stakerRewards, _stakerIncentive);
            IRewards(stakerRewards).queueNewRewards(_stakerIncentive);
        }
    }

    //claim ocean and extra rewards and disperse to reward contracts
