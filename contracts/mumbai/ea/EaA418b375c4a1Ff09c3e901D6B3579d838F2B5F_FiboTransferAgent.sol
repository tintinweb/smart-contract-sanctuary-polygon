...
contract FiboTransferAgent is UUPSUpgradeable, AccessControlUpgradeable, PausableUpgradeable, FiboTransferAgentState {
    ...
    modifier onlyAdminOrAsset(address asset) {
        ...
        require((_msgSender() == asset) || hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "unauthorized");
        _;
    }
    ...
    function updateAssetConfig(address asset, AssetConfig calldata config) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(assets[asset].exists, "unknown asset");
        validateGuards(config.guards);
    ...
    function transfer(address asset, address initiator, address from, address to, uint256 quantity) public onlyAdminOrAsset(asset) whenNotPaused {
    ...
    function mint(address asset, address initiator, address user, uint256 amount) public onlyAdminOrAsset(asset) whenNotPaused {
        bool valid;
        uint256 newBalance;
        uint256 newSupply;
    ...
    function burn(address asset, address user, uint256 amount) public onlyAdminOrAsset(asset) whenNotPaused {
