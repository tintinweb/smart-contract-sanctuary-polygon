...
contract ChallengeDetail is IERC721Receiver {
    ...
    */
     ...
     * @param challengeAddress The address of the newly created challenge.
     * @param caller The address of the caller who created the challenge.
     * @param timestamp The timestamp when the challenge was created.
     */
    event CreateNewChallenge(
    ...
    constructor(
        ...
        emit CreateNewChallenge(address(this), msg.sender, block.timestamp);
    }
    ...
    function giveUp(
        ...
        uint256 remainningAmountFee = uint256(100).sub(amountFailFee);
    ...
    function transferNFTForSenderWhenFailed(
        ...
        for (uint256 i = 0; i < _listNFTAddress.length; i++) {
            if (_statusTypeNft[i]) {
                ...
                uint256 lengthListIndexNFT = _listIndexNFT[i].length.div(2);
    ...
    function updateRewardSuccessAndfail() private {
        ...
        if (coinNativeBalance > 0) {
            serverSuccessFee = coinNativeBalance.mul(amountSuccessFee).div(100);
            serverFailureFee = coinNativeBalance.mul(amountFailFee).div(100);
    ...
    function tranferCoinNative(address payable from, uint256 value) private {
        if (getContractBalance() >= value) {
            ...
            // Activate event FundTransfer
            emit FundTransfer(from, value);
        }
    }
