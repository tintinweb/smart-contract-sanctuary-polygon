...
{
    // attributes
    uint256 public immutable collectionSize = 880;
    ...
    function setWhiteListByArray(address[] calldata addrArray)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < addrArray.length; i++) {
            whiteList[addrArray[i]] = 1;
        }
    }

    function setWhiteListByNum(address whiteListaddr, uint256 num)
        external
        onlyOwner
    {
        whiteList[whiteListaddr] = num;
    }
    ...
    function setWhiteListSaleActive() public onlyOwner {
    ...
    {
        if (publicSaleActive) {
            _publicSaleMint(quantity);
        } else {
            _whiteListSaleMint(quantity);
        }
    }

    // Private
    function _whiteListSaleMint(uint256 quantity)
        private
        whenWhiteListSaleActive
    {
        require(whiteList[msg.sender] - quantity >= 0, "Not enough whitelists");
        _internalMint(msg.sender, quantity);
        _refundIfOver(publicPrice * quantity);
        whiteList[msg.sender] -= quantity;
    }

    function _publicSaleMint(uint256 quantity) 
        private 
    ...
    function _internalMint(address to, uint256 quantity) private {
        require(currentId + quantity <= collectionSize, "reached max supply");
