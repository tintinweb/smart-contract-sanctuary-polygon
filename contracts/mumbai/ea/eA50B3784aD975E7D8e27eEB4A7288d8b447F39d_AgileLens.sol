...
contract Ownable is Context {
     ...
     */
    function authorizeOwnershipTransfer(address authorizedAddress)
        external
        onlyOwner
    {
    ...
    function assumeOwnership() external {
        require(
            _msgSender() == _authorizedNewOwner,
            "Ownable: only the authorized new owner can accept ownership"
        );
...
contract AGL is Ownable {
    ...
    /// @notice Total number of tokens in circulation // 10000000
    uint public constant totalSupply = 10000000e18; // 1 billion AGL
    ...
    event EpochConfigChanged(
        uint32 indexed previousEpoch,
        uint32 previousBlocks,
        uint32 previousROI,
    ...
    constructor(address account) public {
        ...
        emit EpochConfigChanged(
            0,
            0,
            0,
            newEpochConfig.epoch,
            newEpochConfig.blocks,
            newEpochConfig.roi
        );
    ...
    function transfer(address dst, uint rawAmount) external returns (bool) {
        uint96 amount = safe96(
            rawAmount,
            "AGL::transfer: amount exceeds 96 bits"
        );
    ...
    function transferFrom(
        ...
        uint96 amount = safe96(
            rawAmount,
            "AGL::approve: amount exceeds 96 bits"
        );
        ...
        if (spender != src && spenderAllowance != uint96(-1)) {
            uint96 newAllowance = sub96(
                spenderAllowance,
                amount,
                "AGL::transferFrom: transfer amount exceeds spender allowance"
            );
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "AGL::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "AGL::delegateBySig: invalid nonce"
        );
        ...
        returns (uint96)
    {
        require(
            ...
            "AGL::getPriorVotes: not yet determined"
        );
    ...
    function setEpochConfig(uint32 blocks, uint32 roi) public onlyOwner {
        ...
        EpochConfig memory newEC = EpochConfig(
            getEpochs(block.number),
            blocks,
            roi
        );
        require(
            prevEC.blocks != newEC.blocks || prevEC.roi != newEC.roi,
            "AGL::setEpochConfig: blocks and roi same as before"
        );
     ...
     */
    function getCurrentEpochConfig()
        public
        view
        returns (
            uint32 epoch,
     ...
     * @return (index of config,
                config at epoch)
     */
    function getEpochConfig(uint32 forEpoch)
        public
        view
        returns (
            uint32 index,
    ...
    function getEpochs(uint blockNumber) public view returns (uint32) {
        ...
        for (uint32 i = 0; i < epochConfigs.length; i++) {
            uint96 deltaBlocks = (uint96(epochConfigs[i].epoch) - epoch) *
                blocks;
    ...
    function getHoldingReward(address account) public view returns (uint96) {
        ...
        if (lastEpoch < eligibleEpochs) {
            ...
            // First check most recent balance
            if (
                transferPoints[account][nTransferPoint - 1].epoch <=
                lastEligibleEpoch
            ) {
                ...
                reward =
                    ...
                    (uint256(tp.balance) *
                        ...
                        sub32(
                            epoch,
                            startEpoch,
                            "AGL::getHoldingReward: invalid epochs"
                        ));
                if (tookNextTP) {
                    break;
                }
        ...
        uint96 amount = safe96(
            reward / 10000,
            "AGL::getHoldingReward: reward exceeds 96 bits"
        );
        ...
        if (claimedAmounts[account] > 0) {
            amount = sub96(
                amount,
                claimedAmounts[account],
                "AGL::getHoldingReward: invalid claimed amount"
            );
        }
    ...
    function claimReward() public {
        ...
        claimedAmounts[msg.sender] = add96(
            claimedAmounts[msg.sender],
            holdingReward,
            "AGL::claimReward: invalid claimed amount"
        );
    ...
    function _transferTokens(
        ...
        require(
            src != address(0),
            "AGL::_transferTokens: caglot transfer from the zero address"
        );
        require(
            dst != address(0),
            "AGL::_transferTokens: caglot transfer to the zero address"
        );

        balances[src] = sub96(
            balances[src],
            amount,
            "AGL::_transferTokens: transfer amount exceeds balance"
        );
        balances[dst] = add96(
            balances[dst],
            amount,
            "AGL::_transferTokens: transfer amount overflows"
        );
        ...
        if (srcRep != dstRep && amount > 0) {
            if (srcRep != address(0)) {
                ...
                uint96 srcRepNew = sub96(
                    srcRepOld,
                    amount,
                    "AGL::_moveVotes: vote amount underflows"
                );
            ...
            if (dstRep != address(0)) {
                ...
                uint96 dstRepNew = add96(
                    dstRepOld,
                    amount,
                    "AGL::_moveVotes: vote amount overflows"
                );
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "AGL::_writeCheckpoint: block number exceeds 32 bits"
        );
    ...
    function _writeTransferPoint(
        ...
        if (dst != address(this)) {
            ...
            if (
                nDstPoint > 0 &&
                transferPoints[dst][nDstPoint - 1].epoch >= epoch
            ) {
                transferPoints[dst][nDstPoint - 1].balance = dstBalance;
            } else {
                transferPoints[dst][nDstPoint] = TransferPoint(
                    epoch,
                    dstBalance
                );
