...
// File: contracts/utils/GoTokensRoles.sol
...
function createAsset(uint32 _tokenizedPercentage, Asset memory asset, uint16 _correspondingFlorest, uint256[] memory ids, uint256[] memory amount, string calldata _tokenURI ) external {
    ...
    uint8 fromAsset = uint8(asset.woodTypeForAsset);
    uint8 fromFlorest = uint8(florest.woodTypeForFlorest); 
    console.log(fromAsset, "150");
    console.log(fromFlorest,"151");
    ...
    assetsQuantityInFlorest[_correspondingFlorest] = assetIdInCurrentFlorest++;
