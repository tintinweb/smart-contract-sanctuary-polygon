...
contract MetaTraceNFTCar is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, PausableUpgradeable, OwnableUpgradeable, ERC721BurnableUpgradeable, UUPSUpgradeable {
    ...
    function safeMintUser(bytes memory _signature, address to) external virtual {
        require(SignatureProcessor._isSigned(_signature, secret, msg.sender), "Not allowed");
        _safeMintInternal(to);
    }
    ...
    function safeMintWithTokenIdUser(bytes memory _signature, address to, uint256 tokenId) external virtual {
        require(SignatureProcessor._isSigned(_signature, secret, msg.sender), "Not allowed");
        _safeMint(to, tokenId);
    }
    ...
    function changeSecret(bytes32 _secret) public virtual onlyOwner {
        secret = _secret;
    }
}