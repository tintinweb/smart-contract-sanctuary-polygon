...
import "./RandomIdGenerator.sol";
import "./IMorphable.sol";
import "./IPass.sol";
	...
	ReentrancyGuard
{
	...
	// Total number of commons minted
	Counters.Counter private _commonTokenIds;
	// Total number of rares minted
	Counters.Counter private _rareTokenIds;

	// Starting index for commons to be minted from
	uint256 public immutable COMMON_STARTING_INDEX;
	// Max number of commons to be minted
	uint256 public immutable COMMON_MAX_SUPPLY;
	// Max number of rares to be minted
	uint256 public immutable RARE_MAX_SUPPLY;
	// Max number of commons to be minted via Diamond/Gold passes
	uint256 public commonMaxEarlyAccessSupply;

	// Price per token mint
	uint256 public price = 55 ether;
	// Price per common morph
	uint256 public morphPrice = 25 ether;
	// Token price used to morph commons
	uint256 public morphTokenValue;
	...
	// Morphing will occur after initial minting phase
	MorphCurrency public morphCurrency = MorphCurrency.NONE;

	// Diamond pass NFT
	IPass public diamondPass;
	// Gold pass NFT
	IPass public goldPass;
	// Token used to morph commons
	IERC20 public morphToken;
	// Aradena team
	...
	constructor(
		uint256 _commonMaxSupply,
		uint256 _commonMaxEarlyAccessSupply,
		uint256 _rareMaxSupply,
	...
	) ERC721("Gates of Aradena", "GA") RandomIdGenerator(_rareMaxSupply, _startingIndex) {
		COMMON_MAX_SUPPLY = _commonMaxSupply;
		commonMaxEarlyAccessSupply = _commonMaxEarlyAccessSupply;
		RARE_MAX_SUPPLY = _rareMaxSupply;
		...
		goldPass = _goldPass;
		COMMON_STARTING_INDEX = _rareMaxSupply;
	}

	/**
	 * @notice Purchase Gates of Aradena during sale
	...
	function purchase(uint256 _amount) external payable nonReentrant {
		...
		_validatePurchaseWindows(_commonTokenIds.current(), _amount);
		_purchase(_amount);
	...
	/**
	 * @notice Morph two token ids to generate a rare breed
	 * @notice Only enabled when morphCurrency is set to ether or ERC20
	 *
	 * @param _tokenIdOne First token id to morph
	 * @param _tokenIdTwo Second token id to morph
	 */
	function morph(uint256 _tokenIdOne, uint256 _tokenIdTwo)
		external
		payable
		override
		nonReentrant
	{
		require(morphCurrency != MorphCurrency.NONE, "Morphing not active");

		if (morphCurrency == MorphCurrency.ETHER) {
			require(msg.value >= morphPrice, "Insufficient value");
		} else {
			uint256 allowance = morphToken.allowance(msg.sender, address(this));
			require(allowance >= morphTokenValue, "Must approve contract to spend tokens");

			_safeTransferFrom(msg.sender, address(this));
		}

		_morph(_tokenIdOne, _tokenIdTwo);
	}

	function commonTokenIds() external view returns (uint256) {
		return _commonTokenIds.current();
	}

	function rareTokenIds() external view returns (uint256) {
		return _rareTokenIds.current();
	}
	 ...
	 */
	function setCommonMaxEarlyAccessSupply(uint256 _amount) external onlyOwner {
		commonMaxEarlyAccessSupply = _amount;
	}

	/**
	 * @param _morphCurrency The currency used to morph commons
	 */
	function setMorphCurrency(MorphCurrency _morphCurrency) external onlyOwner {
		morphCurrency = _morphCurrency;
	}

	/**
	 * @param _morphPrice The ether price to used to morph commons
	 */
	function setMorphPrice(uint256 _morphPrice) external onlyOwner {
		morphPrice = _morphPrice;
	}

	/**
	 * @param _token The address of the morphing ERC20 token
	 */
	function setMorphToken(IERC20 _token) external onlyOwner {
		morphToken = _token;
	}

	function setMorphTokenValue(uint256 _value) external onlyOwner {
		morphTokenValue = _value;
	}
	 ...
	 */
	function _validatePurchaseWindows(uint256 _commonTokenId, uint256 _amount) internal {
	...
	function _purchase(uint256 _amount) internal {
		require(
			_commonTokenIds.current() + _amount <= COMMON_MAX_SUPPLY,
			"Amount exceeds common NFT max supply"
		);
		...
		for (uint256 i = 0; i < _amount; ) {
			...
			unchecked {
				++i;
				uint256 commonTokenId = _commonTokenIds.current() + COMMON_STARTING_INDEX;
				_safeMint(msg.sender, commonTokenId);
			}
		}
	}
	...
	function _morph(uint256 _tokenIdOne, uint256 _tokenIdTwo) internal {
		require(
			_tokenIdOne > RARE_MAX_SUPPLY && _tokenIdTwo > RARE_MAX_SUPPLY,
			"Both tokens must be a common NFT"
		);
		require(
			_isApprovedOrOwner(msg.sender, _tokenIdOne) &&
				_isApprovedOrOwner(msg.sender, _tokenIdTwo),
			"Caller is not approved or owner"
		);

		_safeMorph(_tokenIdOne, _tokenIdTwo);
	}

	function _safeMorph(uint256 _tokenIdOne, uint256 _tokenIdTwo) internal {
		_burn(_tokenIdOne);
		_burn(_tokenIdTwo);
		// Get a random token id for rares
		uint256 rareTokenId = _getRandomId(_tokenIdOne, _tokenIdTwo);
		_rareTokenIds.increment();
		_safeMint(msg.sender, rareTokenId);

		emit Morph(msg.sender, _tokenIdOne, _tokenIdTwo, rareTokenId);
	}

	function _safeTransferFrom(address _sender, address _recipient) internal {
		bool success = morphToken.transferFrom(_sender, _recipient, morphTokenValue);
	...
	modifier validateEarlyAccessPurchaseAmount(uint256 _tokenId, uint256 _amount) {
		require(
			_tokenId + _amount <= commonMaxEarlyAccessSupply,
			"Amount exceeds common max early supply"
		);
		_;
	}
	...
	function withdrawTokens() public onlyOwner {
		uint256 balance = morphToken.balanceOf(address(this));
		morphToken.transfer(msg.sender, balance);
	}
}
...
/// @author 0xJohn / @JCHerts
abstract contract RandomIdGenerator is WithLimitedSupply {
	/**
	 * Matrix mapping used for random index assignment
	 */
	mapping(uint256 => uint256) private _tokenMatrix;

	uint256 private _startingIndex;

	constructor(uint256 totalSupply, uint256 startingIndex) WithLimitedSupply(totalSupply) {
		_startingIndex = startingIndex;
	}

	/**
	 * @notice Randomly gets a new token ID and keeps track of the ones that are still available.
	 *
	 * @param _tokenIdOne first token id used as a random input
	 * @param _tokenIdTwo second token id used as a random input
	 * @return random id generated
	 */
	function _getRandomId(uint256 _tokenIdOne, uint256 _tokenIdTwo)
		internal
		ensureAvailability
		returns (uint256)
	{
		uint256 maxIndex = getTotalSupply() - tokenCount();
		...
		uint256 value = 0;
		// If the mapping is empty for the key random, assign random to value
		if (_tokenMatrix[random] == 0) {
			value = random;
		} else {
			// Else assign previously stored number from the mapping to value
			value = _tokenMatrix[random];
		}

		// If the last available token ID is still unused
		unchecked {
			if (_tokenMatrix[maxIndex - 1] == 0) {
				// store that token ID in the current matrix position.
				_tokenMatrix[random] = maxIndex - 1;
			} else {
				// else copy over the stored number to the current matrix position.
				_tokenMatrix[random] = _tokenMatrix[maxIndex - 1];
			}

			// Increment counter
			incrementAndGetTokenId();

			return value + _startingIndex;
		}
	}
}
...
interface IMorphable {
	...
	event Morph(address indexed from, uint256 tokenIdOne, uint256 tokenIdTwo, uint256 newTokenId);
	 ...
	 * @param _tokenIdTwo Second token id to morph
	 */
	function morph(uint256 _tokenIdOne, uint256 _tokenIdTwo) external payable;
}
...
abstract contract WithLimitedSupply {
	...
	function availableTokenCount() public view returns (uint256) {
		return getTotalSupply() - tokenCount();
	}

	function incrementAndGetTokenId() internal virtual returns (uint256) {
