...
interface __AQLToken{
        function mintTokens(address _userAddress,uint256 amount)external;
...
pragma solidity 0.8.19;
contract AQLVesting is OwnableUpgradeable,ReentrancyGuardUpgradeable{
    
    using SafeERC20Upgradeable for IERC20Upgradeable;

    // AQL token
       IERC20Upgradeable public AQLToken;
    ...
    // user struct 
        struct Investor {
            ...
            uint256 _initialVestingStartTime;
        }
    ...
    event AddNewInvestorType(uint256 initialAmount, uint256 linearAmount, uint256 initialTime, uint256 linearstartTime, uint256 linearTimePeriod);

    function initialize(address __AQLToken_)external initializer{
        AQLToken=IERC20Upgradeable(__AQLToken_);
        __Ownable_init();
        __ReentrancyGuard_init();
        vestingNames=["0","Pre-Seed Sale","Seed Sale","Whitelist Sale","Public Sale","Core Team","Founders","Advisors"];
        initialAmountReleased=[0,100,0,1000,1000,0,100,0];
        linearVestingAmountReleased=[0,900,1000,0,0,1000,900,1000]; // stores percentage
        // linearVestingTimePeriod=[0,364 minutes, 208 minutes,0, 0, 364 minutes ,364 minutes,364 minutes];
        linearVestingTimePeriod=[0,20 minutes, 20 minutes,0, 0, 20 minutes ,20 minutes,20 minutes];
        lastSaleType=7;
        // 1= 3Months >> 2= 12months >> 3=0,4=0,5=3months >>6=3months>> 7= 3 months
        linearVestingStartTime= [0,5 minutes ,10 minutes,0,5 minutes,5 minutes,5 minutes,0 ];
    }

    function getSaleTypeName()external view returns(string[] memory){
        return vestingNames;
    }
     ...
     function addInvestors(Investor[] memory vest) external onlyOwner {
        for (uint i = 0;i < vest.length;i++) {
            ...
            vestingDetails storage vesting= InvestorDetails[vest[i].account];
            vesting.investorType = vest[i].investorType;
            vesting.totalBalance = vest[i].amount * 1 ether;
            vesting.vestingStart = vest[i]._initialVestingStartTime;
    ...
    function claimFunds() external nonReentrant{
        ...
        if (returnType == 1) {
            ...
            AQLToken.safeTransfer(msg.sender, amount);
            // AQLToken.mintTokens(msg.sender, amount);
    ...
    function setAQLToken (address _tokenAddress) external onlyOwner {
        require(!isRewardTokenSet,"Already Reward Token Set");
        isRewardTokenSet=true;
        AQLToken = IERC20Upgradeable(_tokenAddress);
        emit SetRewardToken(msg.sender, _tokenAddress);
    }

    function getContractTokenBalance() external view returns(uint){
        return AQLToken.balanceOf(address(this));
    }

    function depositAQL(uint amount)external onlyOwner{
         totalDepositedTokens+=amount;
         AvailableTokens+=amount;
         AQLToken.safeTransferFrom(msg.sender,address(this), amount);
      ...
      function withdrawExcessTokens(address ERC20Address, uint256 value) external onlyOwner {
        require(value <= IERC20Upgradeable(ERC20Address).balanceOf(address(this)), 'Insufficient balance to withdraw');
           if(AQLToken==IERC20Upgradeable(ERC20Address))
    ...
    function getVestingBalance(address _userAddress) public view returns (uint, uint) {
        if (!InvestorDetails[_userAddress].hasInitialClaimed &&
            block.timestamp >=  InvestorDetails[_userAddress].vestingStart &&
            InvestorDetails[_userAddress].initialToBeClaimed > 0 && block.timestamp <= InvestorDetails[_userAddress].vestingStart+linearVestingStartTime[InvestorDetails[_userAddress].investorType] ) {return (InvestorDetails[_userAddress].initialToBeClaimed, 1);
            }
        else if (!InvestorDetails[_userAddress].hasLinearClaimed && block.timestamp >=InvestorDetails[_userAddress].vestingStart+ linearVestingStartTime[InvestorDetails[_userAddress].investorType]) return (linearVestingDetails(_userAddress),3);
        else return (0,4);
    }
    ...
    function linearVestingDetails(address _userAddress) public view returns (uint) {
        ...
        if (block.timestamp <= linearVestingTimePeriod[InvestorDetails[_userAddress].investorType]+InvestorDetails[_userAddress].vestingStart+linearVestingStartTime[InvestorDetails[_userAddress].investorType]) {
             if(!isLinearClaimed[_userAddress])
                    timeDifference=block.timestamp-(InvestorDetails[_userAddress].vestingStart+linearVestingStartTime[InvestorDetails[_userAddress].investorType]);
             else
    ...
    function getLinearVestEndTime(uint256 saletype,address _userAddress) external view returns(uint){
        uint256 endminutes=InvestorDetails[_userAddress].vestingStart+linearVestingStartTime[saletype]+linearVestingTimePeriod[saletype];
        return endminutes;  
    }

     function getLinearVestStartTime(uint256 saletype,address _userAddress) external view returns(uint){
        uint256 startminutes=InvestorDetails[_userAddress].vestingStart+linearVestingStartTime[saletype];
        return startminutes;  
    }
