...
contract Payment {
    ...
    event UserPayment(
        uint256 amount, string message, address creator, address payer, bool status, uint256 id, bytes32 uid
    );
    ...
    event requestUserEvent(string user, uint256 amount);
    event updateUserEvent(uint256 id, uint256 status);
    event requestWUserEvent(address user, uint256 amount);
    ...
    mapping(uint256 => RequestW) public idToRequestW;
    ...
    function getLatestPrice(uint256 amount) public view returns (uint256) {
        ...
        uint256 result = (((amount * 10 ** 18) / uint256(price * 10000000000))) * 10 ** 18;
    ...
    */
    function createPay(string memory message, uint256 amount) public {
        bytes32 uniqueID = keccak256(abi.encodePacked(message, block.timestamp));
        uint256 maticAmount = uint256(getLatestPrice(amount));
        PayLink memory payl = PayLink(maticAmount, message, msg.sender, address(0), false, payCounter, uniqueID);
        idToPayLink[payCounter] = payl;
        payCounter++;
        emit UserPayment(amount, message, msg.sender, address(0), false, payCounter, uniqueID);
    }
    ...
    */
    ...
    function createRaisePay(string memory message, uint256 amount, uint256 deadline) public payable {
        bytes32 uniqueID = keccak256(abi.encodePacked(message, block.timestamp));
        uint256 maticAmount = uint256(getLatestPrice(amount));
        address[] memory payerList;
        RaisePay memory raise =
            RaisePay(maticAmount, message, msg.sender, payerList, false, raiseCounter, deadline, uniqueID, 0);
        idToRaiseLink[raiseCounter] = raise;
        raiseCounter++;
        emit RaisePayment(amount, message, msg.sender, payerList, false, raiseCounter, deadline, uniqueID, 0);
    }
    ...
    */
    function acceptRequestUser(uint256 id, uint256 paystatus) public {
        string memory user = idToRequest[id].user;
        require(UserToAddr[_toLower(user)] != address(0), "User does not exist");
        if (paystatus == 1) {
            ...
            balances[UserToAddr[_toLower(user)]] = balances[UserToAddr[_toLower(user)]] + amount;
        } else {
    ...
    */
    function requestAddress(address user, uint256 amount) public {
        uint256 maticAmount = uint256(getLatestPrice(amount));
        RequestW memory req = RequestW(maticAmount, user, msg.sender, 0, requestWId);

        idToRequestW[requestWId] = req;

        requestWId++;
        emit requestWUserEvent(user, amount);
    }
    ...
    */
    function acceptRequestAddr(uint256 id, uint256 paystatus) public {
        if (paystatus == 1) {
            idToRequestW[id].status = 1;
            address user = idToRequestW[id].user;
            uint256 amount = idToRequestW[id].amount;
        ...
        emit updateWUserEvent(id, paystatus);
    }
