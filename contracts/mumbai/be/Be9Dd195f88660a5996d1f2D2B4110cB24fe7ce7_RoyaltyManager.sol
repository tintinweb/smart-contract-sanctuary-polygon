...
contract RoyaltyManager is Ownable {
    ...
    event SetFees(address indexed creator, uint256 pertenthousand, uint256 indexed id);
    event MaxFees(uint256 maxFees);

    constructor(address _harfang, uint256 _maxFees){
        harfang = _harfang;
        maxFees = _maxFees;
    }

    function setMaxFees(uint256 _maxFees) external onlyOwner{
        maxFees = _maxFees;
        emit MaxFees(maxFees);
    }
    ...
    function calculateRoyaltyFeeAndGetRecipient(
        ...
        return (Harfang(harfang).creator(id), amount*fees[id]/10000);
    }

    function setFees(uint256 id, uint256 _fees) external{
        require(Harfang(harfang).creator(id) == msg.sender, "You are not the creator");
        require(_fees <= maxFees, "Fees are too high");
        fees[id] = _fees;
        emit SetFees(msg.sender, _fees, id);
    }
}
