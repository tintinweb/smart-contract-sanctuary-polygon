...
import "./IArtNftERC721.sol";

contract ArtNftERC721 is IArtNftERC721, ERC2981, AccessControl, ERC721 {
    using Strings for uint256;
    ...
    function lazyMintBatchOnBehalf(LazyMintBatchArgs memory _args) public onlyRole(LAZY_MINTER_ROLE) {
        _lazyMintBatchOnBehalf(_args);
    }
    ...
    function tokenURI(uint256 _tokenId) public view virtual override(IArtNftERC721, ERC721) returns (string memory) {
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(IArtNftERC721, ERC2981, AccessControl, ERC721)
        returns (bool)
    {
        return
    ...
    function withdrawTo(IERC20 _currency, address _to) external {
        _onlyAdmin();
        uint256 amount = _currency.balanceOf(address(this));
        if (amount == 0 || !_currency.transfer(_to, amount)) {
            revert ArtERC20TransferError();
        }
        emit ArtWithdrawal(_currency, _to, amount);
    }
    ...
    function _lazyMintBatchOnBehalf(LazyMintBatchArgs memory args) internal {
        if (0 == args.batchSize || args.batchSize > maxBatchSize) {
        ...
        Batch memory newBatch = Batch(args.cid, args.batchName, args.creator, firstTokenId, args.batchSize);

        tokenBatch.push(newBatch);

        for (uint256 tokenId = firstTokenId; tokenId < firstTokenId + args.batchSize; tokenId++) {
