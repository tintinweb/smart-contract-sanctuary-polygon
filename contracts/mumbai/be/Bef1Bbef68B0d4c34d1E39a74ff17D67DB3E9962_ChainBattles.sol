...
contract ChainBattles is ERC721URIStorage {
    ...
    function getId(uint256 tokenId) public view returns (string memory) {
        PlayerData memory IDs = tokenIdToLevels[tokenId];
        return IDs.playerid.toString();
    }

    function getLevels(uint256 tokenId) public view returns (string memory) {
        PlayerData memory levels = tokenIdToLevels[tokenId];
        return levels.level.toString();
    }

    function getHP(uint256 tokenId) public view returns (string memory) {
        PlayerData memory playerHP = tokenIdToLevels[tokenId];
        return playerHP.hp.toString();
    }

    function getStrength(uint256 tokenId) public view returns (string memory) {
        PlayerData memory playerStrength = tokenIdToLevels[tokenId];
        return playerStrength.strength.toString();
    }

    function getSpeed(uint256 tokenId) public view returns (string memory) {
        PlayerData memory playerSpeed = tokenIdToLevels[tokenId];
        return playerSpeed.speed.toString();
    }

    function createRand() internal returns (uint256) {
        // increase nonce
        randomNonce++;
        return
            uint256(
    ...
    function mint() public {
        ...
        uint256 randId = createRand();
        tokenIdToLevels[newItemId] = PlayerData(randId, 0, 0, 0, 0);
    ...
    function train(uint256 tokenId) public {
        ...
        PlayerData memory current = tokenIdToLevels[tokenId];
        data = PlayerData(
            current.playerid,
            current.level + 1,
            current.hp + 5,
