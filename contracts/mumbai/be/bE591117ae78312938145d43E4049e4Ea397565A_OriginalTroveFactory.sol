...
interface IFeeRecipient {
  ...
  function getRedemptionFeeRatio(uint256 _collateralRatio, uint256 _mcr) external view returns (uint256);
...
abstract contract TroveFactory is UUPSUpgradeable, OwnableUpgradeable, PausableUpgradeable, Constants, ITroveFactory {
  ...
  function getRedemptionFeeRatio(address _trove) public view override returns (uint256) {
    return feeRecipient.getRedemptionFeeRatio(ITrove(_trove).collateralization(), ITrove(_trove).mcr());
  }
  ...
  function createTrove(address _token) public override returns (ITrove trove) {
    ...
    require(tokenToPriceFeed.tokenPriceFeeds(_token) != address(0x0), "66c10 the token price feed must be set");
