...
contract IERC20Basic is IERC20 {
...
contract ButterFly {
    ...
    uint256 private constant maxDeposit = 200e18;
    ...
    constructor(IERC20 _daiAddr, address _defaultRefer, address[3] memory _feeReceivers) public {
        dai = _daiAddr;
    ...
    function _reOrderTop(uint256 _dayNow) private {
        for(uint256 i = 5; i > 1; i--){
            ...
            if(amount1 > amount2){
                dayTopUsers[_dayNow][i - 1] = topUser4;
                dayTopUsers[_dayNow][i - 2] = topUser3;
                dayTopUsers[_dayNow][i - 3] = topUser2;
                dayTopUsers[_dayNow][i - 4] = topUser1;
            }
        }
    }
    ...
    function _calLevelNow(address _user) private view returns(uint256) {
        ...
        if(total >= 100e18)
        {
            ...
            if(total >= 200e18 && user.teamNum >= 2 && maxTeam >= 200e18 && otherTeam >= 200e18)
            {
                levelNow = 5;
            }
            else if(user.teamNum >= 1 && maxTeam >= 100e18 && otherTeam >= 100e18)
            {
                levelNow = 4;
            }
        }
    ...
    function _distributeTopPool(uint256 _dayNow) private {
        uint16[5] memory rates = [3000, 2500, 2000, 1500, 1000];
        uint72[5] memory maxReward = [2000e18, 1500e18, 1000e18, 500e18, 200e18];
        uint256 totalReward;
        for(uint256 i = 0; i < 5; i++){
