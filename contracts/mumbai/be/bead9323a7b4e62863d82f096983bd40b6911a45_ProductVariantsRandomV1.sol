...
contract ProductVariantsRandomV1 is AbstractProductV1 {
    ...
    function freezeSupply() public onlyRole(DELEGATE_ADMIN_ROLE) {
    ...
    function freezeSupply(bytes32 variantId) public onlyRole(DELEGATE_ADMIN_ROLE) {
    ...
    function variantByIndex(uint256 index) public view returns (Variant memory) {
    ...
    function productInventory() public view returns (Inventory memory) {
        Inventory memory productInventory_;
        for (uint256 i = 0; i < _variants.length; i++) {
            Inventory memory variantInventory_ = variantInventory(_variants[i].id);
    ...
    function variantInventory(bytes32 variantId) public view returns (Inventory memory) {
        uint256 index = _variantIndex(variantId);
        Variant memory variant_ = _variants[index];

        uint256 total = totalSupply(variant_.id);
        uint256 available = variant_.initialInventory - total;

        return Inventory({availableInventory: available, totalSupply: total});
    }
    ...
    /// @dev Not intended to be securely random, just fair
    /// @param tokenId The token id
    function _assignRandomVariant(uint256 tokenId) internal {
        bytes32 variantId;
        uint256 numVariants = totalVariants();

        // must have inventory available
        Inventory memory inventory_ = productInventory();
        require(inventory_.availableInventory > 0, 'Product: unavailable');
        ...
        require(variantId != 0x0, 'Product: cannot assign variant');

        _assignVariant(tokenId, variantId);
    }

    /// @notice Assigns variant to newly issued token id
    /// @dev Does not check if the variant exists
    /// @param tokenId The token id
    /// @param variantId The variant id
