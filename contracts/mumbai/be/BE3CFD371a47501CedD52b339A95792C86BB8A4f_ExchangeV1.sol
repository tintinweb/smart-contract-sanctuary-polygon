...
contract ExchangeV1 is IExchange, IxBaseV1, FeeWorkerV1 {
    ...
    // withdrawer whitelist management
    ...
    function depositToken(
        ...
        if (feeAmount > 0) {
            ...
            IERC20(token).safeTransferFrom(user, collector, feeAmount);
        }

        emit Deposited(token, user, pool, amount, collector, feeAmount, memo);
    }
...
interface IExchange {
    event WithdrawerRegistered(address indexed recharger, address indexed owner);
    event WithdrawerUnregistered(address indexed recharger, address indexed owner);
