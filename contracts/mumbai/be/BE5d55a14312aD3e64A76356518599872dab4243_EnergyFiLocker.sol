...
contract EnergyFiLocker is IEnergyFiLocker, Ownable, ReentrancyGuard {
    ...
    struct FeeStruct {
        uint256 glmrFee; // glmr fee charged on non whitelisted locking
        IERC20Burn secondaryFeeToken; // fee token optional to glmr
    ...
    IEnergyFiFactory public immutable ENERGYFI_FACTORY; // factory of the LP token
    ...
    function lockLPToken(
        ...
        IEnergyFiPair lpair = IEnergyFiPair(address(_lpToken));
        address factoryPairAddress = ENERGYFI_FACTORY.getPair(
            lpair.token0(),
            lpair.token1()
        );
        ...
        if (!feeWhitelist.contains(msg.sender)) {
            // charge fee with GLMR token
            if (_feeInGlmr) {
                uint256 glmrFee = fees.glmrFee;
                // calculate discounted fee for providing valid referrer
                if (_referral != address(0)) {
                    glmrFee = glmrFee.mul(1000 - fees.referralDiscount).div(1000);
                }
                require(msg.value == glmrFee, "FEE NOT MET");

                // calculate referral fee
                uint256 devFee = glmrFee;
                if (glmrFee != 0 && _referral != address(0)) {
            ...
            // send back GLMR for whitelisted callers
     ...
     * @dev a fee in GLMR (glmrFee) is required to be sent by calling to create a new lock
    ...
    function splitLock(
        ...
        // trasnfer GLMR as fee to developer address
        require(msg.value == fees.glmrFee, "FEE NOT MET");
        devaddr.transfer(fees.glmrFee);
     ...
     * @param _glmrFee GLMR fee charged on lock creation for GLMR base tokens
     * @param _secondaryTokenFee fee charged on locking non GLMR base token
    ...
    /**
     * @notice set secondary fee token as option to glmr as fee by owner
...
interface IMigrator {
     ...
     * @dev is called from EnergyFiLocker contract
