...
contract ERC1155 is Context, ERC165, IERC1155, IERC1155MetadataURI {
    ...
    function totalBalance(uint256 tokenId) public view returns (uint256) {
        require(
            _exists(tokenId),
            "ERC1155Metadata: balance query for nonexistent token"
        );
...
contract RDLAND1155 is ERC1155 {
    ...
    function mint(
        ...
        _mint(to, newItemId, supply, uri, fee);
        uint256 tokenId = newItemId;
        newItemId = newItemId + 1;

        if (approve == true) {
            setApprovalForAll(approveAddress, approve);
        }
        return tokenId;
    }
    ...
    function setCreatorRoyalty(uint256 tokenId, uint256 fee) external {
        ...
        require(
            balanceOf(msg.sender, tokenId) == totalBalance(tokenId),
            "ERC721: not enough token balance"
        );
