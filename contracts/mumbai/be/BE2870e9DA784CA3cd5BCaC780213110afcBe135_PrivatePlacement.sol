...
contract PrivatePlacement is OwnerPausable {
    ...
    function buy(uint256 index) public whenNotPaused {
        ...
        uint256 ttPrice = _tdex.getPrice();
        uint256 ttAmount = bags[index].ttAmountInUsdt*10**18*10**_ttDecimals/ttPrice;
        IERC20(_ttAddress).safeTransferFrom(_ttPayOutAddress, address(_stackingReward), ttAmount);
        _stackingReward.deposit(trader, index, ttAmount);
...
contract StockMargin is OwnerPausable {
    ...
    function addStockMargin(address account, uint256 margin) external {
        require(msg.sender == _privatePlacementAddress, "not PrivatePlacement");
        _margins[account] += margin;
        _totalMargin += margin;
        IERC20(_usdtAddress).safeTransfer(_marginDepositAddress, margin);
    }
    ...
    function accountMargin(address account) external view returns(uint256) {
        return _margins[account];
    }
...
contract StakingReward is OwnerPausable {
    ...
    //index => apy
    mapping(uint256 => LevelApy[]) private _apys;
    ...
    constructor(address owner, uint256 secondsPerBlock, address ttAddress, address ttPayOutAddress) OwnerPausable(owner) {
        ...
        _apys[0].push(LevelApy(number, 18));
        _apys[1].push(LevelApy(number, 20));
        _apys[2].push(LevelApy(number, 28));
        _apys[3].push(LevelApy(number, 30));
        _apys[4].push(LevelApy(number, 32));
    ...
    function deposit(address staker, uint256 index, uint256 amount) external onlyPP {
    ...
    function updateUserLevel(address userAddress, uint256 level) external onlyOperator {
        require(level>0 && level < 5, "lever error");
        (, uint256 pendingRewards) = calculatePendingRewards(userAddress);
        UserInfo storage user = _userInfo[userAddress];
        user.rewardDebt += pendingRewards;
        user.level = level;
        
        OrderInfo[] storage userOrders = _orders[userAddress];
        for (uint256 i; i < userOrders.length; i++) {
            userOrders[i].lastRewardBlock = block.number;
        }
    }

    function addApy(uint256[] memory levels, uint256[] memory apys_) external onlyOwnerOrOperator{
        require(levels.length == apys_.length, "size error");
        for (uint i; i<levels.length; i++) {
            _apys[levels[i]].push(LevelApy(block.number, apys_[i]));
        }
    }

    function depositMock(uint256 index, uint256 amount, uint256 loopCounter) external {
    ...
    function calculatePendingRewards(address staker) public view returns(uint256, uint256) {
        ...
        for (uint256 i; i < userOrders.length; i++) {
            ...
            LevelApy[] memory levelApys = _apys[user.level];
            ...
            if (lastRewardBlock >= levelApys[apySize-1].startBlock) {
                ...
                for (uint256 j; j<apySize-1; j++) {
                    if (matches == 0 && levelApys[j].startBlock <= lastRewardBlock && lastRewardBlock < levelApys[j+1].startBlock) {
                        matches = 1;
                    }
    ...
    function withdraw(uint256 amount) external {
        ...
        IERC20(_ttAddress).safeTransfer(msg.sender, withdrawn);
    }    
    ...
    function _withdraw(address withdrawer, uint256 amount) internal returns(uint256 withdrawn){
        ...
        (, uint256 pendingRewards) = calculatePendingRewards(withdrawer);
