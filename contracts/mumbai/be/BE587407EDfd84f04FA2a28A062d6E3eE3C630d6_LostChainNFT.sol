    ...
    UUPSUpgradeable
{
    ...
    mapping(uint256 => Hero) hero;

    struct Recharger {
        bool isRecharger;
        uint8 rechargeAmount;
        uint256 nftId;
    }
    mapping(uint256 => Recharger) recharger;

    struct Rewardable {
        bool isRewardable;
        string rarity;
    }
    mapping(uint256 => Rewardable) private rewardable;
    ...
    function mint(
        ...
        _mintNFT(to, rechargeAmount, id, amount, tokenURI, rarity, isHero, isRewardable, isRecharger);
    }
    ...
    function _mintNFT(
        ...
        require(id == getLatestId(), 'LCNFT: ID mistmach');
        if (isHero) require(amount == 1, 'LCNFT: Hero must be unique, no multiple hero on the same ID allowed');
        ...
        if (isRecharger) {
            recharger[id].isRecharger = true;
            recharger[id].nftId = id;
            recharger[id].rechargeAmount = rechargeAmount;
        }

        if (isRewardable) {
            rewardable[id].isRewardable = true;
            rewardable[id].rarity = rarity;
        }
    }
    ...
    function setRewardable(uint256 nftId, bool state) public onlyRole('DEFAULT_ADMIN_ROLE', _msgSender()) {
        ...
        rewardable[nftId].isRewardable = state;
        if (!state) rewardable[nftId].rarity = '';
    }

    function getRewardableRarity(uint256 nftId) external view returns (string memory) {
        require(rewardable[nftId].isRewardable, 'LCNFT: NFT is not rewardable');
        return rewardable[nftId].rarity;
    }

    function checkNFTRewardable(uint256 nftId) external view returns (bool) {
        uint256 maxNftId = _nftId.current();
        require(maxNftId > nftId, 'LCNFT: NFT id is out of range');
        return rewardable[nftId].isRewardable;
    }

    function checkNFTHero(uint256 nftId) external view returns (bool) {
        uint256 maxNftId = _nftId.current();
        require(maxNftId > nftId, 'LCNFT: NFT id is out of range');
        return hero[nftId].isHero;
    }

    function checkNFTRecharger(uint256 nftId) external view returns (bool) {
        uint256 maxNftId = _nftId.current();
        require(maxNftId > nftId, 'LCNFT: NFT id is out of range');
        return recharger[nftId].isRecharger;
    }

    function isHeroes(uint256[] memory ids) public view returns (bool[] memory) {
        bool[] memory ret = new bool[](ids.length);
        for (uint256 i = 0; i < ret.length; i++) {
            ret[i] = hero[ids[i]].isHero;
        }
        return ret;
    }

    function isRechargers(uint256[] memory ids) public view returns (bool[] memory) {
        bool[] memory ret = new bool[](ids.length);
        for (uint256 i = 0; i < ret.length; i++) {
            ret[i] = recharger[ids[i]].isRecharger;
        }
        return ret;
    }

    function getHeroExperience(uint256 heroId) public view returns (Hero memory) {
        require(hero[heroId].isHero, 'LCNFT: NFT is not hero');
        return hero[heroId];
    }

    function getRechargeAmount(uint256 nftId) external view returns (uint8) {
        require(recharger[nftId].isRecharger, 'LCNFT: NFT not a recharger');
        return recharger[nftId].rechargeAmount;
    }
    ...
    function addHeroExperience(
        ...
        require(ids.length == xpAmount.length, 'LCNFT: Hero id and xp mismatch');

        for (uint256 i = 0; i < ids.length; i++) {
            require(hero[ids[i]].isHero, 'LCNFT: NFT is not hero');
            hero[ids[i]].xp += xpAmount[i];
        }
    }

    function getAllHeroExperiences(uint256[] memory ids) public view returns (Hero[] memory) {
        uint256 maxNftId = _nftId.current();
        require(maxNftId > 0, 'LCNFT: No NFT available');

        Hero[] memory tempRet = new Hero[](maxNftId);
        uint256 totalIndex = 0;
        for (uint256 i = 0; i < ids.length; i++) {
            if (hero[ids[i]].isHero) {
                tempRet[totalIndex] = hero[ids[i]];
                totalIndex++;
            }
        }
    ...
    function getBulkNFTBalance(address account, uint256[] memory ids) public view returns (uint256[] memory) {
        uint256[] memory ret = new uint256[](ids.length);
        for (uint256 i = 0; i < ret.length; i++) {
            ret[i] = balanceOf(account, ids[i]);
        }
        return ret;
    }
    ...
    function gameDataSendNFT(
        ...
        require(_msgSender() == LCGD, 'LCNFT: caller is not LCGD');
        require(isApprovedForAll(from, _msgSender()), 'LCNFT: Not approved');
        safeBatchTransferFrom(from, to, id, amount, '');
    }
    ...
    function gameDataMint(
        ...
        require(_msgSender() == LCGD, 'LCNFT: caller is not LCGD');
        _mintNFT(to, 0, id, 1, tokenURI, '', isHero, false, false);
    }
