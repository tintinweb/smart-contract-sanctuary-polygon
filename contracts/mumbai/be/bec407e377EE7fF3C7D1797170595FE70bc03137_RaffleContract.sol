...
contract RaffleContract is ERC721Holder {
    ...
    function createRaffle(
        ...
        emit RaffleCreated(raffleCount, ticketPrice, ticketSupply, endTime, msg.sender, nftContract, nftId);
    }
    ...
    function endRaffle(uint256 raffleId) external {
        ...
        address winner = tickets[raffleId][raffles[raffleId].winnerTicket];

        IERC721 nft = IERC721(raffles[raffleId].nftContract);
        nft.safeTransferFrom(address(this), winner, raffles[raffleId].nftId);

        IERC20 token = IERC20(tokenAddress);
        uint256 totalTokens = raffles[raffleId].ticketPrice.mul(raffles[raffleId].ticketSupply).mul(TOKEN_DECIMALS);
        token.transfer(raffles[raffleId].creator, totalTokens);

        emit RaffleEnded(raffleId, raffles[raffleId].winnerTicket, winner);
    }
