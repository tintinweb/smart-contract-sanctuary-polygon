    ...
    ERC1155BurnableUpgradeable
{
    ...
    mapping(uint256 => uint256) public maximumpurchase;
    ...
    function initialize(address _sig) public initializer {
    ...
    function mint(
        ...
        maximumpurchase[id] = _maximumpurchase;
    ...
    function saleWithToken(
        ...
        if (maximumpurchase[tokenId] > 0) {
            require(
                nooftoken <= maximumpurchase[tokenId],
                "Maximum NFT limit is reached"
            );
        }
    ...
    function saleToken(
        ...
        if (isChange == 1) {
            changePrice(tokenPrice, tokenId);
        } else if (isChange == 2) {
            orderPlace(from, tokenId, tokenPrice);
        }
        require(
            amount >= order_place[from][tokenId].price.mul(nooftoken),
            "Invalid Balance"
    ...
    function _saleToken(
        ...
        if (keccak256(bytes(bidtoken)) == keccak256(bytes("ALW"))) {
    ...
    function _acceptBId(
        ...
        uint256 serviceVal = serviceValue;
        if (keccak256(bytes(tokenAss)) == keccak256(bytes("ALW"))) {
            serviceVal = serviceValueEsc;
        }
    ...
    function orderPlace(
        ...
        require(_balances[tokenId][from] > 0 && _price > 0, "Is Not a Owner");
        Order memory order;
