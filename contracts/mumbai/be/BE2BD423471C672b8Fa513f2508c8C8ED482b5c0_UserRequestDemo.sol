...
contract SxTRelay is Admin, String, Initializer, ReentrancyGuard, ChainlinkClient {
     ...
     */
    function withdrawChainlink(address to, uint256 amount) external onlyAdmin nonReentrant {
...
abstract contract UserRequest is ReentrancyGuard{
    ...
    address public sxtRelayContract;
    ...
    modifier onlySxTRelay() {
        require(msg.sender == sxtRelayContract, "UserRequest: Only callable by SxT Request Contract");
        _;
    }
     ...
     */
    function runRequestQuery(string memory query, string memory resourceId) external nonReentrant returns(bytes32 requestId){
        SxTRelay sxtRelayInstance = SxTRelay(sxtRelayContract);
        require(chainlinkToken.approve(sxtRelayContract, sxtRelayInstance.FEE()), "UserRequest: Insufficient allowance");
        return bytes32(abi.encodePacked(sxtRelayInstance.requestQuery(query, resourceId, address(this), this.saveQueryResponse.selector)));
    }
     ...
     */
    function saveQueryResponse(bytes32 requestId, string[][] calldata data) external nonReentrant virtual onlySxTRelay {}
