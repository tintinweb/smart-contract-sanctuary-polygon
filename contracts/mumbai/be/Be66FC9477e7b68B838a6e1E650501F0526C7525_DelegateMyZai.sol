...
contract DelegateMyZai is Ownable {
    ...
    modifier onlyGame(){
        require(gameAddresses.getGameAddress() == msg.sender, "Not your zai");
        _;
    }
    ...
    function updateLastScholarPlayed(uint256 _zaiId) external onlyGame returns(bool){
    ...
    function gotDelegationForZai(uint256 _zaiId, address _user) external view returns(bool){
        if(!EnumerableSet.contains(scholarDelegated[_user],_zaiId)){
            return false;
        }else {
            ZaiStruct.DelegateData storage m = _delegateDatas[_zaiId];
            if(
