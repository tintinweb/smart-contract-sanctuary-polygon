...
contract NFTStakingV1 is Ownable {
    ...
    uint256 public rewardsPeriod = 3 minutes; // 3 day
    uint256 private dayUnit = 1 minutes; // 1 day =  86400s
    ...
    constructor() {
        rewardsTokenContract = IERC20(0xC34d8CF73f3db5e7AF2c381AD5410e4F16370fA4);
        //$shibanon token contract address here
        nftContract = IERC721(0x23dd36F65E809bb21049f5CF9B382E43c0d1c08C);
        // ShibAnon Collection contract address here
    }
    ...
    function stakeNFT(uint256 _tokenId) public {
        ...
        require(nftContract.getApproved(_tokenId) == address(this), "You need to approve this contract to transfer your NFT");
    ...
    function claimTokenReward() public {
        require(!paused, "Staking is paused");
        require(stakedNFTs[msg.sender].length > 0, "You are not staking any NFT");
        ...
        for (uint256 i = 0; i < stakedNFTs[msg.sender].length; i++) {
            if (stakers[msg.sender][stakedNFTs[msg.sender][i]].claimedAt.add(rewardsPeriod) < block.timestamp) {
                ...
                uint256 claimableAmount = claimableDays.mul(rewardsPerDay).div(rewardsPeriod);

                stakers[msg.sender][stakedNFTs[msg.sender][i]].claimedAt = block.timestamp;
                totalClaimableAmount = totalClaimableAmount.add(claimableAmount);
            } else {
                revert("You are not eligible to claim rewards");
            }
        }
    ...
    function getClaimableAmount(address _staker) public view returns (uint256) {
        ...
        if (stakedNFTs[_staker].length == 0) return totalClaimableAmount;
    ...
    function changeTokenContract(address _newTokenContract) public onlyOwner {
        rewardsTokenContract = IERC20(_newTokenContract);
    }

    function changeRewardsPeriod(uint256 _newRewardsPeriod) public onlyOwner {
        rewardsPeriod = _newRewardsPeriod;
    }

    function withdrawToken() public onlyOwner {
        require(paused, "Staking is not paused");
        rewardsTokenContract.transfer(msg.sender, rewardsTokenContract.balanceOf(address(this)));
    }
