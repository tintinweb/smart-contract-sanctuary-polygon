...
contract ClaimNFTs is Ownable, ERC721Holder{
    ...
    event OwnerSetted(string indexed NftType, address indexed futurOwner, uint256 indexed tokenId);
    event NftSetted(address nursery, address trainingCenter, address laboratory, address ticket);
    ...
    function setNFTs(address _nursery, address _training, address _labo, address _ticket)external onlyOwner{
        ...
        require(_training != address(0x0),"training center address can't be 0x0");
    ...
    function _getWhatCanUserClaim(address _user) internal view returns(uint256[] memory nurseries,uint256[] memory laboratories, uint256[] memory trainings, uint256[] memory tickets ){
        uint256[] memory _nurseries = new uint256[](myNurseries[_user].length());
        uint256[] memory _laboratories = new uint256[](myLaboratories[_user].length());
        uint256[] memory _trainings = new uint256[](myTrainings[_user].length());
    ...
    function claimAllNFTs() external {
        ...
        if(nurseries.length != 0){
            for(uint256 i ; i < (nurseries.length > 2 ? 2:nurseries.length) ;){
        ...
        if(laboratories.length != 0){
            for(uint256 i ; i < (laboratories.length > 2 ? 2:laboratories.length) ;){
        ...
        if(trainings.length != 0){
            for(uint256 i ; i < (trainings.length > 2 ? 2:trainings.length) ;){
