...
contract MetaData is IMetaData, VRFConsumerBaseV2, Ownable {
    ...
    function fulfillRandomWords(
        ...
        if (msg.sender != address(i_vrfCoordinator)) {
            revert MetaData_OnlyAllowedForCoordinator();
        }

        uint256 tokenId = s_requestIdToTokenId[requestId];

        if (s_tokenMetaData[tokenId].status == NFTStatus.MINTED) {
            uint256 offset = 0;
        ...
        if (s_tokenMetaData[tokenId].status == NFTStatus.REVEALED) {
        ...
        if (s_tokenMetaData[tokenId].status == NFTStatus.WINNER) {
