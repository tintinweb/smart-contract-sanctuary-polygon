...
contract Finde {
    ...
    modifier ValidPageNumber(uint _page){
    ...
    function getPagenatedURIs(uint256 _page, uint256 _limit, uint rating , string calldata _country) external view ValidPageNumber(_page) ValidLimit(_limit) returns(User[] memory){
        
        User[] memory data = new User[](_limit);

        uint dataLength = 0;
        uint from = ((_page - 1) * _limit ) + 1;

        for(uint i=0; i< _limit; i++){
            User storage usr = indexToUser[from + i];
            ...
            if (rating == 0 &&
                ...
                if ( (usr.addr != address(0)) &&
                 (usr.votes.upVote >= usr.votes.downVote) &&
