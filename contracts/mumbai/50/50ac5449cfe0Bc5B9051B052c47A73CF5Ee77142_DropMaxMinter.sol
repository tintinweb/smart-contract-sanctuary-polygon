...
import { IMelonFeeRegistry } from "../interfaces/IMelonFeeRegistry.sol";
import { IDropMaxMinter, DropMintData, MintInfo } from "../interfaces/IDropMaxMinter.sol";
import { BaseMinter } from "./BaseMinter.sol";
import { IMinterModule } from "../interfaces/IMinterModule.sol";
import { IMelonDropV1, DropInfo } from "../interfaces/IMelonDropV1.sol";

/*
 * @title DropMaxMinter
 * @notice Module for unpermissioned mints of Melon drops.
 */
contract DropMaxMinter is IDropMaxMinter, BaseMinter {
    // =============================================================
    //                            STORAGE
    // =============================================================
    ...
    /**
     * @dev Drop mint data
     * drop => mintId => DropMintData
     */
    mapping(address => mapping(uint128 => DropMintData)) internal _dropMintData;

    // =============================================================
    //                          CONSTRUCTOR
    // =============================================================

    constructor(IMelonFeeRegistry feeRegistry_) BaseMinter(feeRegistry_) {}
    ...
    /**
     * @inheritdoc IDropMaxMinter
     */
    function createDropMint(
        address drop,
        uint96 price,
        uint32 startTime,
        uint32 endTime,
        ...
        mintId = _createDropMint(drop, startTime, endTime, affiliateFeeBPS);

        DropMintData storage data = _dropMintData[drop][mintId];
        data.price = price;
        data.maxMintablePerAccount = maxMintablePerAccount;

        // prettier-ignore
        emit DropMaxMintCreated(
            drop,
            mintId,
            price,
            startTime,
    ...
    function mint(
        ...
        unchecked {
            ...
            uint256 numberMinted = IMelonDropV1(drop).numberMinted(msg.sender);
            // Won't overflow. The total number of tokens minted in `drop` won't exceed `type(uint32).max`,
            // and `quantity` has 32 bits.
            if (numberMinted + quantity > data.maxMintablePerAccount) revert ExceedsMaxPerAccount();
        }

        _mint(drop, mintId, quantity, affiliate);
    }
    ...
    function setPrice(
        ...
        uint128 mintId,
        uint96 price
    ) public onlyDropOwnerOrAdmin(drop) {
        _dropMintData[drop][mintId].price = price;
        emit PriceSet(drop, mintId, price);
    }
    ...
    function setMaxMintablePerAccount(
        ...
        if (maxMintablePerAccount == 0) revert MaxMintablePerAccountIsZero();
        _dropMintData[drop][mintId].maxMintablePerAccount = maxMintablePerAccount;
        emit MaxMintablePerAccountSet(drop, mintId, maxMintablePerAccount);
    }
    ...
    function totalPrice(
        ...
        unchecked {
            ...
            return uint128(uint256(_dropMintData[drop][mintId].price) * uint256(quantity));
        }
    }
     ...
     * @inheritdoc IDropMaxMinter
     */
    function mintInfo(address drop, uint128 mintId) external view returns (MintInfo memory info) {
        BaseData memory baseData = _baseData[drop][mintId];
        DropMintData storage mintData = _dropMintData[drop][mintId];

        DropInfo memory dropInfo = IMelonDropV1(drop).dropInfo();
        ...
        info.maxMintableLower = dropInfo.dropMaxMintableLower;
        info.maxMintableUpper = dropInfo.dropMaxMintableUpper;
        info.cutoffTime = dropInfo.dropCutoffTime;
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view override(IERC165, BaseMinter) returns (bool) {
        return BaseMinter.supportsInterface(interfaceId) || interfaceId == type(IDropMaxMinter).interfaceId;
    }
    ...
    function moduleInterfaceId() public pure returns (bytes4) {
        return type(IDropMaxMinter).interfaceId;
    }
}
...
/**
 * @title IMelonFeeRegistry
 */
interface IMelonFeeRegistry {
    // =============================================================
    //                            EVENTS
    // =============================================================

    /**
     * @dev Emitted when the `melonFeeAddress` is changed.
     */
    event MelonFeeAddressSet(address melonFeeAddress);
    ...
    /**
     * @dev The new `melonFeeAddress` must not be address(0).
     */
    error InvalidMelonFeeAddress();
    ...
    /**
     * @dev Sets the `melonFeeAddress`.
     *
     * Calling conditions:
     * - The caller must be the owner of the contract.
     *
     * @param melonFeeAddress_ The melon fee address.
     */
    function setMelonFeeAddress(address melonFeeAddress_) external;
    ...
    /**
     * @dev The melon protocol's address that receives platform fees.
     * @return The configured value.
     */
    function melonFeeAddress() external view returns (address);
...
/**
 * @dev Data unique to a drop max mint.
 */
struct DropMintData {
...
/**
 * @dev All the information about a drop max mint (combines DropMintData with BaseData).
 */
struct MintInfo {
    uint32 startTime;
    uint32 endTime;
...
/**
 * @title IDropMaxMinter
 * @dev Interface for the `DropMaxMinter` module.
 * @author Sound.xyz
 */
interface IDropMaxMinter is IMinterModule {
    // =============================================================
    //                            EVENTS
    // =============================================================

    /**
     * @dev Emitted when a drop max is created.
     * @param drop               Address of the song drop contract we are minting for.
     * @param mintId                The mint ID.
     * @param price                 Sale price in ETH for minting a single token in `drop`.
    ...
    /**
     * @dev Emitted when the `price` is changed for (`drop`, `mintId`).
     * @param drop Address of the song drop contract we are minting for.
     * @param mintId  The mint ID.
     * @param price   Sale price in ETH for minting a single token in `drop`.
     */
    event PriceSet(address indexed drop, uint128 indexed mintId, uint96 price);

    /**
     * @dev Emitted when the `maxMintablePerAccount` is changed for (`drop`, `mintId`).
     * @param drop               Address of the song drop contract we are minting for.
     * @param mintId                The mint ID.
     * @param maxMintablePerAccount The maximum number of tokens that can be minted per account.
     */
    event MaxMintablePerAccountSet(address indexed drop, uint128 indexed mintId, uint32 maxMintablePerAccount);
    ...
    /*
     * @dev Mints tokens for a given drop.
     * @param drop   Address of the song drop contract we are minting for.
     * @param mintId    The mint ID.
     * @param quantity  Token quantity to mint in song `drop`.
    ...
    /*
     * @dev Sets the `price` for (`drop`, `mintId`).
     * @param drop Address of the song drop contract we are minting for.
     * @param mintId  The mint ID.
     * @param price   Sale price in ETH for minting a single token in `drop`.
     */
    ...
    /*
     * @dev Sets the `maxMintablePerAccount` for (`drop`, `mintId`).
     * @param drop               Address of the song drop contract we are minting for.
     * @param mintId                The mint ID.
     * @param maxMintablePerAccount The maximum number of tokens that can be minted by an account.
     */
    ...
    /**
     * @dev Returns {IDropMaxMinter.MintInfo} instance containing the full minter parameter set.
     * @param drop The drop to get the mint instance for.
     * @param mintId  The ID of the mint instance.
     * @return mintInfo Information about this mint.
     */
    function mintInfo(address drop, uint128 mintId) external view returns (MintInfo memory);
}

// SPDX-License-Identifier: MIT
pragma solidity ^0.8.16;

import { OwnableRoles } from "../helpers/OwnableRoles.sol";
import { IMelonDropV1 } from "../interfaces/IMelonDropV1.sol";
import { IMinterModule } from "../interfaces/IMinterModule.sol";
import { IMelonFeeRegistry } from "../interfaces/IMelonFeeRegistry.sol";
import { IERC165 } from "@openzeppelin/contracts/utils/introspection/IERC165.sol";
import { SafeTransferLib } from "../helpers/SafeTransferLib.sol";
import { FixedPointMathLib } from "../helpers/FixedPointMathLib.sol";

/**
 * @title Minter Base
 * @dev The `BaseMinter` class maintains a central storage record of drop mint instances.
 */
abstract contract BaseMinter is IMinterModule {
    ...
    /**
     * @dev The next mint ID. Shared amongst all drops connected.
     */
    ...
    /**
     * @dev Maps an drop and the mint ID to a mint instance.
     */
     ...
     */
    IMelonFeeRegistry public immutable feeRegistry;

    // =============================================================
    //                          CONSTRUCTOR
    // =============================================================

    constructor(IMelonFeeRegistry feeRegistry_) {
     ...
     * @inheritdoc IMinterModule
     */
    function setDropMintPaused(
        address drop,
        uint128 mintId,
        bool paused
    ) public virtual onlyDropOwnerOrAdmin(drop) {
        _baseData[drop][mintId].mintPaused = paused;
        emit MintPausedSet(drop, mintId, paused);
    }
    ...
    function setTimeRange(
        ...
        uint32 startTime,
        uint32 endTime
    ) public virtual onlyDropOwnerOrAdmin(drop) onlyValidTimeRange(startTime, endTime) {
        _baseData[drop][mintId].startTime = startTime;
        _baseData[drop][mintId].endTime = endTime;

        emit TimeRangeSet(drop, mintId, startTime, endTime);
    }
    ...
    function setAffiliateFee(
        ...
        uint128 mintId,
        uint16 feeBPS
    ) public virtual override onlyDropOwnerOrAdmin(drop) onlyValidAffiliateFeeBPS(feeBPS) {
        _baseData[drop][mintId].affiliateFeeBPS = feeBPS;
        emit AffiliateFeeSet(drop, mintId, feeBPS);
    }
    ...
    function withdrawForPlatform() public override {
        ...
        if (accrued != 0) {
            ...
            SafeTransferLib.safeTransferETH(feeRegistry.melonFeeAddress(), accrued);
        }
    }
    ...
    /**
     * @dev Restricts the function to be only callable by the owner or admin of `drop`.
     * @param drop The drop address.
     */
    modifier onlyDropOwnerOrAdmin(address drop) virtual {
        if (
            msg.sender != OwnableRoles(drop).owner() &&
            !OwnableRoles(drop).hasAnyRole(msg.sender, IMelonDropV1(drop).ADMIN_ROLE())
        ) revert Unauthorized();
    ...
    /**
     * @dev Creates an drop mint instance.
     * @param drop The drop address.
     * @param startTime The start time of the mint.
     * @param endTime The end time of the mint.
     * @param affiliateFeeBPS The affiliate fee in basis points.
     ...
     * Calling conditions:
     * - Must be owner or admin of the drop.
     */
    function _createDropMint(
        address drop,
        uint32 startTime,
        uint32 endTime,
        uint16 affiliateFeeBPS
        ...
        returns (uint128 mintId)
    {
        ...
        BaseData storage data = _baseData[drop][mintId];
        ...
        emit MintConfigCreated(drop, msg.sender, mintId, startTime, endTime, affiliateFeeBPS);
    }

    /**
     * @dev Mints `quantity` of `drop` to `to` with a required payment of `requiredEtherValue`.
    ...
    function _mint(
        ...
        BaseData storage baseData = _baseData[drop][mintId];
        ...
        uint128 requiredEtherValue = totalPrice(drop, mintId, msg.sender, quantity);
        ...
        bool affiliated = isAffiliated(drop, mintId, affiliate);
        ...
        emit Minted(
            ...
            uint32(IMelonDropV1(drop).mint{ value: remainingPayment }(msg.sender, quantity)),
            quantity,
            requiredEtherValue,
            platformFee,
            affiliateFee,
...
import { IERC165 } from "@openzeppelin/contracts/utils/introspection/IERC165.sol";
import { IMelonFeeRegistry } from "./IMelonFeeRegistry.sol";
...
interface IMinterModule is IERC165 {
    ...
    /**
     * @dev Emitted when the mint instance for an `drop` is created.
     * @param drop The drop address.
    ...
    /**
     * @dev Emitted when the `paused` status of `drop` is updated.
     * @param drop The drop address.
     * @param mintId  The mint ID, to distinguish between multiple mints for the same drop.
     * @param paused  The new paused status.
     */
    event MintPausedSet(address indexed drop, uint128 mintId, bool paused);
     ...
     * @param endTime   The end time of the mint.
     */
    event TimeRangeSet(address indexed drop, uint128 indexed mintId, uint32 startTime, uint32 endTime);
     ...
     */
    event AffiliateFeeSet(address indexed drop, uint128 indexed mintId, uint16 bps);
    ...
    /**
     * @dev Sets the paused status for (`drop`, `mintId`).
     *
     * Calling conditions:
     * - The caller must be the drop's owner or admin.
     */
    function setDropMintPaused(
        address drop,
        uint128 mintId,
    ...
    /**
     * @dev Sets the time range for an drop mint.
     *
     * Calling conditions:
     * - The caller must be the drop's owner or admin.
     *
    ...
    /**
     * @dev Sets the affiliate fee for (`drop`, `mintId`).
     *
     * Calling conditions:
     * - The caller must be the drop's owner or admin.
     */
    ...
    /**
     * @dev Whether `affiliate` is affiliated for (`drop`, `mintId`).
     * @param drop   The drop's address.
     * @param mintId    The mint ID.
     * @param affiliate The affiliate's address.
    ...
    /**
     * @dev The total price for `quantity` tokens for (`drop`, `mintId`).
     * @param drop   The drop's address.
     * @param mintId    The mint ID.
     * @param to        The address to mint to.
     ...
     *      and is shared amongst all drops connected to the minter contract.
     * @return The latest value.
     */
     ...
     */
    function feeRegistry() external view returns (IMelonFeeRegistry);
}
