...
pragma solidity ^0.8.13;
contract EpilloICO is Ownable{
    using SafeERC20 for  IERC20;
    // Epillo Token
    IERC20 public token;
    //Epillo token per matic
    uint public tokenRateMatic; //1 Matic = 10000 Epillo Tokens 
    //start Date
    ...
    event EpilloUsers(address _userAddress, uint amount);
    event VestingAmount(address account, uint _amout);


    constructor(address _EpilloToken){
             token=IERC20(_EpilloToken);
    }
    ...
    function buyEpillo() external payable{
        require(block.timestamp>= IcoStartDate && block.timestamp<=IcoEndDate,"Check ICO Time");
        require(!isUserAdded[msg.sender],"User exist in Private");
        uint _tokens;
         ...
         uint tokenAmount=totalUserFunds[msg.sender];
           userDetails memory __user;
              ...
              emit EpilloUsers(msg.sender, _tokens);
    }
    ...
    function addInvestors( user[] memory _user) external onlyOwner{
        for(uint i=0;i<_user.length;i++){
            ...
            isUserAdded[_user[i]._userAddress]=true;
              ...
              emit  EpilloUsers(_user[i]._userAddress,amount);
        }
      ...
      function setVestingStartTime(uint _time) external onlyOwner{
          ...
          LinearEndTime=_time+70 minutes;
      }
    ...
    function getLinearBalance(address _userAddress)public view returns(uint){
        ...
        if(block.timestamp>=linearStarts[getUserDetails[_userAddress].saleType]){
            ...
            if(block.timestamp>=linearStarts[getUserDetails[_userAddress].saleType]+70 minutes){
    ...
    if(block.timestamp<startDate) return 0;
        ...
        if(isclaimed[_address]){
          ...
          if(!isVestingStarted|| block.timestamp<=startDate){
              funds=0;
          }
     return(funds);          
    }
  
    // set the Epillo  Token
    function setEpilloToken(address _EpilloAddress) external onlyOwner{
        token=IERC20(_EpilloAddress);
    }
    ...
    function removeSingleInvestor(address _userAddress) public onlyOwner{
        ...
        require(getUserDetails[_userAddress].saleType==2,"its Sale Type 1");
    ...
    function getExcessToken() external onlyOwner{
            payable(msg.sender).transfer(address(this).balance);
            totalMaticReceived=0;
    }
    function recoverEpillo(uint amount) external onlyOwner{
