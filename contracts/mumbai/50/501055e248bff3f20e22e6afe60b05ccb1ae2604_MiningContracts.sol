...
// File: @thirdweb-dev/contracts/interfaces/ITWFee.sol
...
// File: @thirdweb-dev/contracts/interfaces/drop/IDropClaimCondition.sol
...
// File: @thirdweb-dev/contracts/interfaces/drop/IDropERC1155.sol
...
// File: contracts/MiningContracts.sol
...
contract MiningContracts is ReentrancyGuard {
    DropERC1155 public immutable unknownNftCollection;
    ...
    constructor(DropERC1155 dropContractAddress, TokenERC20 tokenContractAddress) {
        unknownNftCollection = dropContractAddress;
    ...
    mapping (address => MapValue) public playerUnknown;

    mapping (address => MapValue) public playerLastUpdate;

    function stake(uint256 _tokenId) external nonReentrant {
        require(unknownNftCollection.balanceOf(msg.sender, _tokenId) >= 1, "Anda harus memiliki setidaknya 1 NFT untuk coba anda stake");

        if (playerUnknown[msg.sender].isData) {
            unknownNftCollection.safeTransferFrom(address(this), msg.sender, playerUnknown[msg.sender].value, 1, "Mengembalikan NFT lama kamu");
        }

        uint256 reward = calculateRewards(msg.sender);
        rewardsToken.transfer(msg.sender, reward);

        unknownNftCollection.safeTransferFrom(msg.sender, address(this), _tokenId, 1, "Staking NFT");

        playerUnknown[msg.sender].value = _tokenId;
        playerUnknown[msg.sender].isData = true;
    ...
    function withdraw() external nonReentrant {
        require(playerUnknown[msg.sender].isData, "Kamu tidak memiliki NFT untuk di withdraw.");
        ...
        playerUnknown[msg.sender].isData = false;
        playerLastUpdate[msg.sender].isData = true;
        playerLastUpdate[msg.sender].value = block.timestamp;
    }
    ...
    {
        if (!playerLastUpdate[_player].isData || !playerUnknown[_player].isData) {
            return 0;
        }

        uint256 timeDifference = block.timestamp - playerLastUpdate[_player].value;
        uint256 rewards = timeDifference * 10_000_000_000_000 * (playerUnknown[_player].value + 1);
    ...
    function withdraw(uint amount, address payable destAddr) public {
        require(msg.sender == owner, "Hanya si Boss yang bisa withdraw"); 
        require(amount <= balance, "Saldo Kosong, Boss...");
    ...
    function transferERC20(IERC20 token, address to, uint256 amount) public {
        ...
        require(amount <= erc20balance, "Saldo Kosong, boss...");
