...
* @dev Fired in cancelOfferOnBuyOrder()
          ...
          uint256 partnerMarketplaceFeePercentValue = 0;
              if(_collectionPartnerDaoAddress.isContract()){
                ...
                if (partnerMarketplaceFeePercent > 0) {
                  ...
                  bool success = eft.transfer(_collectionPartnerDaoAddress, partnerMarketplaceFeePercentValue);
                  require(success, "Send fees to partner failed.");
                  marketplaceFeeValueWithDiscount -= partnerMarketplaceFeePercentValue;
                }
            }   
          //Send % to Dao 
          bool success = eft.transferFrom(address(this), daoAddress, marketplaceFeeValueWithDiscount);
  ...
  */
  function cancelSellOrder(bytes32 _orderId) external sellOrderExists(_orderId) senderOwnsSellOrder(_orderId) {
    ...
    if(sellOrder.highestOfferor != address(0) && !sellOrder.highestOfferor.isContract()) {
      // Refund offeror
        bool success = eft.transferFrom(address(this), sellOrder.highestOfferor, sellOrder.highestOffer);
  ...
  */
  function placeOfferOnSellOrder(
    ...
    require(_value > 0, "offer value must grater than 0");

    // If the sell order is not open to offers without an ask price
    if(sellOrder.ask != 0) {
      // Ensure the offer value is less than the order ask
      require(_value < sellOrder.ask, "offer value must be less than order ask");
    }
    if(sellOrder.ask == 0 && sellOrder.offerMinPrice > 0){
      require(_value >= sellOrder.offerMinPrice, "offer value must be higher than offer min price");
    }

    // Ensure the value of the new offer is higher than the current
    require(_value > sellOrder.highestOffer, "offer value must be higher than current");
  ...
  */
  function acceptSellOrderHighestOffer(bytes32 _orderId) external sellOrderExists(_orderId) senderOwnsSellOrder(_orderId) {
    ...
    if(!sellOrder.owner.isContract() && sellOrder.highestOffer > 0) {
     ...
     bool success = eft.transferFrom(address(this), sellOrder.owner, _takeFeesFrom(sellOrder.highestOffer, collectionAddress, partnerDaoAddress));
   ...
   */
  function fulfillSellOrder(bytes32 _orderId, uint256 _value) external onlyIfSenderNotContract sellOrderExists(_orderId) {
    // Copy sell order in memory
    SellOrder memory sellOrder = sellOrders[_orderId];
    ...
    require(sellOrder.ask == _value, "incorrect amount of ether provided");
   ...
   *      in that case it is open to offers
   */
function buySellOrder(bytes32 _orderId, uint256 _value) external onlyIfSenderNotContract sellOrderExists(_orderId) {
    ...
    if(!sellOrder.owner.isContract()) {
      // Transfer Ether amount without fees to sell order owner
      // (fees are paid to DAO and Partner in the _takeFeesFrom function)
      bool success = eft.transferFrom(address(this), sellOrder.owner, _takeFeesFrom(_value, collectionAddress, partnerDaoAddress));
      require(success, "Transfer to sell order owner failed.");
    }
  ...
  function createBuyOrder(
    ...
    uint8 _bid
  ) external  validCardType(_cardType) returns (bytes32) {
 ...
 * @notice Cancelles an offer on a buy order
  ...
  function acceptBuyOrderLowestOffer(bytes32 _orderId) external buyOrderExists(_orderId) senderOwnsBuyOrder(_orderId) {
    ...
    if(!buyOrder.lowestOfferor.isContract()) {
      ...
      // (fees are paid to DAO and Partner in the _takeFeesFrom function)
      bool success = eft.transferFrom(address(this), buyOrder.lowestOfferor, _takeFeesFrom(buyOrder.lowestOffer, collectionAddress, partnerDaoAddress));
  ...
  function fulfilBuyOrder(bytes32 _orderId, uint256 _cardId) external onlyIfSenderNotContract buyOrderExists(_orderId) {
    ...
    // (fees are paid to DAO and reserve in the _takeFeesFrom function)
    bool success = eft.transferFrom(address(this), msg.sender, _takeFeesFrom(buyOrder.bid, collectionAddress, partnerDaoAddress));
    require(success, "Transfer to sell order owner failed.");
    ...
    if(buyOrder.lowestOfferor != address(0) && !buyOrder.lowestOfferor.isContract()) {
      // Refund previous offeror
      bool success = eft.transferFrom(address(this), buyOrder.lowestOfferor, buyOrder.lowestOffer);
      require(success, "Refund previous offeror failed.");
    }
