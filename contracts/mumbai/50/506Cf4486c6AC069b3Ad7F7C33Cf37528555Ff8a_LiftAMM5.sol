...
contract LiftAMM5 is ERC20 { 
    ...
    constructor(address _tokenA, address _tokenB, uint _feeRate) ERC20("LIFTAMM5", "LAMM5") {  
        tokenA = _tokenA;
        tokenB = _tokenB;   
        feeRate = _feeRate;
    }
    ...
    modifier checkDeadline(uint deadline) {
        require(block.timestamp <= deadline, "Transaction took too long to complete");
        _;
    }
    ...
    function swap(address tokenIn, uint256 amountIn, uint256 minAmountOut, uint deadline) external 
            checkDeadline(deadline) returns (uint256 amountOut, uint256 fee) {
        uint256 newBalance;
        ...
        if (tokenIn == tokenA) {
            ...
            require(amountOut >= minAmountOut, "Cannot swap for less than required minimum");
    ...
    function modifyFeeRate(uint newRate) external onlyOwner {
        feeRate = newRate;
    }
}
