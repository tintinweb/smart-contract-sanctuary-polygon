...
contract Marketplace is Ownable, ReentrancyGuard {
    ...
    uint256 public minStakingPercentage = 1000; // 10%
    ...
    mapping(address => Collection) private _itemsByOwner;
    ...
    function listMarketItem(
        ...
        _itemsByOwner[seller].safeRemove(tokenId);
    ...
    function purchaseMarketItem(uint256 tokenId) external nonReentrant {
        ...
        if (price > 0) {
            ...
            if (fee > 0 && path.length > 0) {
                feeAmount = (item.price * fee) / 10000;
                payout = item.price - feeAmount;
