...
contract DarkEarn is OwnableUpgradeSafe, IERC721Receiver, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // The block number when VVS mining ends.
    ...
    mapping(address => NFTSlot) private _depositedNFT; // user => nft slot;
    ...
    event NewStartAndEndTimes(uint256 startTime, uint256 endBlock);
    event NewRewardPerSecond(uint256 rewardPerSecond);
    event NewPoolLimit(uint256 poolLimitPerUser);
    event RewardsStop(uint256 blockNumber);
    event Withdraw(address indexed user, uint256 amount);
    event RewardPaid(address indexed user, uint256 amount, uint256 boost);
    event UpdateNFTController(address indexed user, address controller);
    event UpdateNFTBoostRate(address indexed user, uint256 boostRate);
    ...
    function getSlots(address _account) public view returns (address, address, address) {
        NFTSlot memory slot = _depositedNFT[_account];
        return (slot.slot1, slot.slot2, slot.slot3);
    }

    function getTokenIds(address _account) public view returns (uint256, uint256, uint256) {
        NFTSlot memory slot = _depositedNFT[_account];
    ...
    function unfrozenStakeTime(address _account) public view returns (uint256) {
        if (userInfo[_account].lastStakeTime + unstakingFrozenTime > bonusEndTime)
            return bonusEndTime;
    ...
    // Depositing of NFTs
    function depositNFT(address _nft, uint256 _tokenId, uint256 _slot) external nonReentrant {
        ...
        if (pending > 0) {
            uint256 _boost = pending.mul(getBoost(address(msg.sender))).div(10000);
            rewardToken.safeTransfer(address(msg.sender), pending.add(_boost));
            emit RewardPaid(address(msg.sender), pending, _boost);
        }
    ...
    // Withdrawing of NFTs
    function withdrawNFT(uint256 _slot) external nonReentrant {
        _updatePool();
        UserInfo storage user = userInfo[msg.sender];
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(accTokenPerShare).div(PRECISION_FACTOR).sub(user.rewardDebt);
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        require(block.timestamp >= unfrozenStakeTime(msg.sender), "locked time");
    ...
    function setUnstakingFrozenTime(uint256 _unstakingFrozenTime) external onlyOwner {
        require(_unstakingFrozenTime <= 30 days, "DarkEarn: !safe - dont lock for too long");
     ...
     */
    function pendingReward(address _user) external view returns (uint256 _rewards) {
        ...
        if (block.timestamp > lastRewardTime && stakedTokenSupply != 0) {
            ...
            _rewards = user.amount.mul(adjustedTokenPerShare).div(PRECISION_FACTOR).sub(user.rewardDebt);
        } else {
            _rewards = user.amount.mul(accTokenPerShare).div(PRECISION_FACTOR).sub(user.rewardDebt);
        }
        uint256 _boost = getBoost(address(msg.sender));
        if (_boost > 0) {
            _rewards = _rewards.add(_rewards.mul(_boost).div(10000));
        }
    }
     ...
     */
    function _getMultiplier(uint256 _fromTime, uint256 _toTime) internal view returns (uint256) {
        if (_toTime <= bonusEndTime) {
            return _toTime.sub(_fromTime);
        } else if (_fromTime >= bonusEndTime) {
            return 0;
        } else {
            return bonusEndTime.sub(_fromTime);
        }
    }
}
