...
contract CampaignFactory {
    ...
    function createCampaign(
        ...
        RewardNFT[] memory _nftRewardsURIs
    ) public {
        ...
        for (uint256 i = 0; i < _nftRewardsURIs.length; i++) {
            newCampaign.createRewardNFT(
                _nftRewardsURIs[i].contribution,
                _nftRewardsURIs[i].maxNumberNFTs,
                _nftRewardsURIs[i].URI
            );
        }
        ...
        CreateNewCampaign storage c = deployedCampaigns.push();

        c.name = _name;
        c.addressOfNewCampaign = address(newCampaign);
        c.goal = _goal;
        c.minimumContribution = minimum;
        c.shortDescription = _shortDescription;
        c.startAt = _startAt;
        c.endAt = _endAt;
...
contract Campaign {
    ...
    function createRewardNFT(
        uint256 contribution,
        uint256 maxNumberNFTs,
        string memory URI
    ) public restricted {
        RewardNFT memory newReward = RewardNFT({
            contribution: contribution,
            maxNumberNFTs: maxNumberNFTs,
            URI: URI
        });
    ...
    function mintERC1155(uint256 _id, uint256 amount) public {
