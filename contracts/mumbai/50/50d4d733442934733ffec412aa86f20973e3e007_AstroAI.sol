...
contract AstroAI is ERC20 {
    ...
    uint256 private _claimFee = 600000000000000;
    bool private _claimAllowed = true;

    mapping(address => bool) private _claimed;

    constructor() ERC20("AstroAI", "Astro") {
        _owner = msg.sender;
        _mintedSupply = 0;
    ...
    function claimTokens() external payable {
        require(!_claimed[msg.sender], "AstroAIToken: Already claimed");
        require(msg.value >= _claimFee, "AstroAIToken: Insufficient fee");
        require(_claimAllowed, "AstroAIToken: Claiming is not allowed anymore");
    ...
    function setClaimFee(uint256 fee) external {
        require(msg.sender == _owner, "AstroAIToken: Only owner can set claim fee");
        _claimFee = fee;
    }

    function allowClaiming() external {
        require(msg.sender == _owner, "AstroAIToken: Only owner can allow claiming");
        _claimAllowed = true;
    }

    function exchangeTokens(uint256 tokenAmount) external {
        require(tokenAmount > 0, "AstroAIToken: Invalid token amount");
        require(balanceOf(msg.sender) >= tokenAmount, "AstroAIToken: Insufficient balance");
    ...
    function burnTokens(uint256 tokenAmount) external {
        require(msg.sender == _owner, "AstroAIToken: Only owner can burn tokens");
        require(tokenAmount > 0, "AstroAIToken: Invalid token amount");
        require(balanceOf(msg.sender) >= tokenAmount, "AstroAIToken: Insufficient balance");
