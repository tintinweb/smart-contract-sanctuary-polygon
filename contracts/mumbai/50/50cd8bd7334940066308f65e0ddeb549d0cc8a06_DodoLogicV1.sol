...
library DodoConfig {
  ...
  function getIncomeUpgradeConfig() internal pure returns (uint256[] memory, uint) {
  ...
  function getBonusUpgradeConfig() internal pure returns (uint256[] memory, uint) {
...
contract DodoLogicV1 is Initializable {
  ...
  event makeMoneyEvent(address player, uint256 times, uint256 reward); 
  event buyPropEvent(address player, uint256 propId); 
  event betEvent(address player, uint256 amount, uint256 result, uint256 odds); 
  ...
  function getPlayerData(address player) public view returns (uint256, uint256, uint256, uint256, uint256) {
    ...
    return (
      ...
      playerData.casinoTimes,  
      playerData.incomeLevel,  
      playerData.bonusLevel,  
      playerData.bonus 
    );
  }
  ...
  function makeMoney() public payable {
    ...
    require(msg.value == DodoConfig.getFee(), "DodoLogic: fee error");
    ...
    emit makeMoneyEvent(player, 1, reward);
  ...
  function getPlayerUpgradeConfig(address player) public view returns (uint256, uint256, uint256, uint256) {
    ...
    (uint256[] memory incomeUpgradeConfig, uint incomeMaxLevel) = DodoConfig.getIncomeUpgradeConfig();
    (uint256[] memory bonusUpgradeConfig, uint bonusMaxLevel) = DodoConfig.getBonusUpgradeConfig();
  ...
  function generateRandomNumber() private view returns (uint256) {
    uint256 randomNumber =  uint(keccak256(abi.encodePacked(
  ...
  function checkWin(uint256 result) private pure returns (uint256) {
    uint256 odds = 0;
    if (result==0) {
      return odds;
    }
  ...
  function casinoBet(uint256 amount) public {
    ...
    uint256 result = generateRandomNumber();
    uint256 odds = checkWin(result);
    if(odds == 0) {
      
      IDodoCoin(tokenContract).deductTokens(player, dataContract, amount);
    } else if (odds > 1) {
      
      DodoStorageInterface(dataContract).transferCoin(player, amount * (odds-1));
    }
    ...
    DodoStorageInterface(dataContract).setPlayerExtraData(player, DodoConfig.getLastBetResultKey(), result);
    DodoStorageInterface(dataContract).setPlayerExtraData(player, DodoConfig.getLastBetAmountKey(), amount);
    
    emit betEvent(player, amount, result, odds);
  }

  
  function getLastBetInfo(address player) public view returns (uint256 result, uint256 amount, uint256 odds) {
    result = DodoStorageInterface(dataContract).getPlayerExtraData(player, DodoConfig.getLastBetResultKey());
    amount = DodoStorageInterface(dataContract).getPlayerExtraData(player, DodoConfig.getLastBetAmountKey());
    odds = checkWin(result);
  }
