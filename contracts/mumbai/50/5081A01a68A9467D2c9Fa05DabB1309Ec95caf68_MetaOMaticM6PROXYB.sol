...
contract MetaOMaticM6PROXYB is  IPOLYCONTRACTM6PROXYB {
    ...
    function generate_report(uint package,address user,uint cycle) view public returns(uint currentrecycle,address parent,address left,address right,uint noofidinfirstlevel,uint noofidinsecondlevel){
      if(package==2){
          ...
          return(_M6120Details[user].currentrecycle,_M6120Details[user].parent[cycle],_M6120Details[user].left[cycle],_M6120Details[user].right[cycle],_M6120Details[user].noofidinfirstlevel[cycle],_M660Details[user].noofidinsecondlevel[cycle]);
      }
    }
