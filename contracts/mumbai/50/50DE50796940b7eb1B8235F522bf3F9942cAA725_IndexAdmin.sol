...
abstract contract Index is AccessControl, IIndex, ReentrancyGuard {
    ...
    function _initialize(
        ...
        _adapter = adapter;
        wETH = IUniswapV2Router01(adapter).WETH();
    ...
    function _unstake(uint256 amountLP) internal {
        ...
        console.log(
            "balanceOf ==",
            IERC20(_actualAcceptToken).balanceOf(address(this))
        );
        for (uint256 i; i < len; i++) {
            console.log("-----*****1");
            address[] memory reversPath = _reversArray(_activeAssets[i].path);
            ...
            console.log("-----*****1", reversPath[0], reversPath[1]);
            console.log("-----*****1", amountForSwap);
