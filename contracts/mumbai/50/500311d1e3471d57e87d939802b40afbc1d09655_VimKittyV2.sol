...
contract VimKittyV2 is ERC721Enumerable, Ownable, RandomlyAssigned {
  using Strings for uint256;

  string baseURI = "ipfs://QmXkLjSX9dmLC18QnrVxGMBBTtsfiaMJo9r3WPUhHDxVrB/";
  string baseExtension = ".json";
  string notRevealedUri;
  uint256 public cost;
  uint256 public phaseOneCost = 0.5 ether;
  uint256 public phaseTwoCost = 1 ether;
  uint256 public phaseThreeCost = 2 ether;
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    require(!paused, "Smart Contract is not allowed to mint yet.");
    require(_mintAmount > 0);
    require(_mintAmount <= nftPerAddressLimit);
    require(supply + _mintAmount <= _maxSupply, "YOU CAN'T MINT MORE THAN MAXIMUM SUPPLY");
    if (supply >=0 && supply < 15){
        delete cost;
        cost = phaseOneCost;
    }
    else if (supply >= 15 && supply < 30){
        delete cost;
        cost = phaseTwoCost;
    }
    else{
    ...
    if (msg.sender != owner()) {
      require(msg.value == cost * _mintAmount, "Insufficient funds this point1");
    ...
    for (uint256 i = 1; i <= _mintAmount; i++) {
        require(!paused, "Phase Limited");
        uint256 id = nextToken();
      _safeMint(msg.sender, id);
      addressMintedBalance[msg.sender]++;
      supply++;
      if(supply == 15 || supply == 30){
          paused = true;
      }
    }
  }
  ...
  function setNftPerAddressLimit(uint256 _newmaxMintAmount) public onlyOwner {
  ...
  function setWhitelistAddress(address[] calldata _wUsers) public onlyOwner{
  ...
  function withdraw() public payable onlyOwner {
    ...
    // =============================================================================
    //(bool hs, ) = payable(ownerOf(1)).call{value: address(this).balance * 5 / 100}("");
    //require(hs);
