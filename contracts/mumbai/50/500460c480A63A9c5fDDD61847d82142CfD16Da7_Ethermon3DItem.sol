...
interface Ethermon3DItemData {
    function ItemUpdate(uint32 _itemId, uint256 _qty) external;
    ...
    function UpdateEvent(Ethermon3DItemBasic.Items calldata _items) external;
}
...
contract Ethermon3DItem is Ethermon3DItemBasic {
    ...
    function getPriceEmon(uint32 _itemId, uint256 _qty)
        external
        view
        returns (uint256 price)
    {
        ...
        ItemData memory item = itemDataContract.getItems(_itemId);

        if (item.price < 1) return 0;

        uint256 priceInEmon = emonOracleData.getEmonRatesFromEth(item.price) *
            _qty;
        return priceInEmon;
    }
        ...
        external
    {
        ...
        uint32[] memory itemIds = new uint32[](_itemIds.length);
        uint256[] memory qtys = new uint256[](_qtys.length);
        for (uint256 i = 0; i < _itemIds.length; i++) {
            ItemData memory item = itemDataContract.getItems(_itemIds[i]);
            require(item.pause, "Item cannot be purchase at the moment");
            require(
                item.qty > _qtys[i] && item.qty > 0 && _qtys[i] > 0,
            ...
            require(
                emon.balanceOf(boughtItems.owner) >= amount,
                "Insufficitn balance"
            );
            item.qty -= _qtys[i];
            ...
            itemDataContract.ItemUpdate(item.id, item.qty);
        }
        boughtItems.itemId = itemIds;
        boughtItems.qty = qtys;
        emon.safeTransferFrom(boughtItems.owner, address(this), amount);
        itemDataContract.UpdateEvent(boughtItems);
    }
}