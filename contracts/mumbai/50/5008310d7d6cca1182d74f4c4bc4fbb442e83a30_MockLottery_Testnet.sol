...
import {WhitelistStorage} from "../../src/lottery/whitelist/WhitelistStorage.sol";
...
contract MockLottery_Testnet is Mock_Lottery, RandomsUtils {
    ...
    event MockRequests(string igoId, uint256[] requestIds, uint256 nonce);
    ...
    function produceFakeRandoms_ByBatchesOf500(
        ...
        emit MockRequests(igoId, requestIds, nonce);
    }

    function getFulfilledReqLength() public view returns (uint256) {
        return
            RandomnessStorage
                .layout()
                .requestsOf[IGO_ID]
    ...
    function resetLastIndexProcessed() public {
        WhitelistStorage.layout().lastIndexProcessed[IGO_ID] = 0;
    }

    // ----------------------------------------- DUMMY -----------------------------------------
    uint256[] public tranformedRandoms;

    function getTranformedRandoms() public view returns(uint256[] memory) {
        return tranformedRandoms;
    }

    function getTranformedRandomsLength() public view returns(uint256) {
        return tranformedRandoms.length;
    }

    function dummy_transformRandomsAndPickWinners(
        string memory igoId,
        uint256 participantsAmount,
        uint256 batchesBy_500
    ) public {
        uint256 amountWinners = batchesBy_500 * 500; // better with or without var ?
        uint256[] memory indexes = new uint256[](amountWinners);
        ...
        for (uint256 batchIndex; batchIndex < batchesBy_500; ++batchIndex) {
            randoms_ = rdmStr
            ...
            assembly {
                ...
                function positionInMemory(index) -> pos {
                    // memory array:
                    //  - index at 0 is array.length
                    //  - index of data stored starts at 1
                    pos := shl(5, add(index, 1))
                }

                let curentBatchIndex := mul(batchIndex, 500)
                ...
                for {
                    ...
                    // indexes[i+currentBatchIndex] = randoms_[i] % participantsAmount
                    mstore(
                        add(
                            indexes,
                            positionInMemory(add(i, curentBatchIndex))
                        ),
                        mod(
                            mload(add(randoms_, positionInMemory(i))), // randoms_[i]
                            participantsAmount
                        )
                    )
                }
        ...
        for (uint256 batchIndex; batchIndex < batchesBy_500; ++batchIndex) {
            ...
            assembly {
                ...
                for {
                    ...
                    // newWinnerPos = randoms_[i] % participants
                    newWinnerPos := positionInMemory(
                        mod(
                            mload(add(randoms_, positionInMemory(i))), // randoms_[i]
                            participants
                        )
                    ...
                    // winners[i + curentBatchIndex] = addresses[newWinnerPos]
                    mstore(
                        add(
                            winners,
                            positionInMemory(add(i, curentBatchIndex))
