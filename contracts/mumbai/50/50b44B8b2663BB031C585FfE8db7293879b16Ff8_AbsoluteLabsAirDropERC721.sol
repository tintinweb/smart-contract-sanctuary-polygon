    ...
    IERC721ReceiverUpgradeable
{
    ...
    // Call custom mint function directly on ERC721 contract
    function customMintERC721(
        ...
        for (uint256 i = 0; i < recipients.length; ) {
            (bool success, ) = token.call(
                abi.encodeWithSignature(functionToCall, recipients[i], amount)
            );

            if (!success) revert Errors.CallHasFailed();
