 ...
 * @title MELOBLOCK
 */
contract DEV721 is ERC721Enumerable, Ownable, ERC2981 {
    using Strings for uint256;

    /**
     * @dev Minimum token ID of MELOBLOCK.
     */
    uint256 private constant START_TOKEN_ID = 1;

    /**
     * @dev Maxium number of MELOBLOCK tokens can be minted.
     */
    ...
    /**
     * @dev Address of sales contract owner
     */
    address private primarySaleContractOwner;
    ...
    // modifier isTokenMint(uint256 tokenId) {
    ...
    constructor(
        ...
        address _primarySaleContractOwner
        ...
        primarySaleContractOwner = _primarySaleContractOwner;
     ...
     * @param newPrimarySaleContract Address of the contract selling MEGAMI tokens.
     */
    function setPrimarySaleContract(address newPrimarySaleContract)
        external
        onlyOwner
    {
        primarySaleContractOwner = newPrimarySaleContract;
    }

    /**
     * @dev Returns the address of the sales contract.
     */
    function getPrimarySaleContract() external view returns (address) {
        return primarySaleContractOwner;
    }
    ...
    /**
     * @dev set isMint of the sales contract.
     */
    function setIsMint(bool _isMint) external onlyOwner {
        isMint = _isMint;
    }

    /**
     * @dev Mint the specified MELOBLOCK token and transfer it to the specified address.
     ...
     * @param _endTokenId The token ID being minted.
     */
    ...
    function mint(uint256 _tokenId, address _address) external payable {
        ...
        require(isMint && salePrice > 0, "It's not the time to mint yet");
        require(
            msg.value == salePrice,
            "must submit the asking price in order to complete the purchase"
        );
        require(
            SALE_START_ID < _tokenId && _tokenId < SALE_END_ID,
            "The sale has not started yet"
        );

        uint256 marketOwnerRoyaltyAmount = (salePrice * 2) / 100; // 2%

        payable(primarySaleContractOwner).transfer(marketOwnerRoyaltyAmount);
        payable(owner()).transfer(msg.value - marketOwnerRoyaltyAmount);
