...
interface ISnookGame is IDescriptorUser {
  ...
  function setLivesPerSnook(uint livesPerSnook) external;
  function getLivesPerSnook() view external returns(uint);
...
contract SnookGame is ISnookGame, AccessControlEnumerableUpgradeable, PausableUpgradeable {
    ...
    function setLivesPerSnook(uint livesPerSnook) external override onlyRole(PAUSER_ROLE) {
      _livesPerSnook = livesPerSnook;
    }

    function getLivesPerSnook() view external override returns(uint) {
      return _livesPerSnook;
    }
    ...
    function mint2(uint count) external override whenNotPaused() {
      ...
      uint amountPaid = count * price * _livesPerSnook;
      require(
        _skill.transferFrom(
          msg.sender, // from 
          address(this),  // to 
      ...
      for (uint i=0; i<count; i++) {
        ...
        descriptors[i] = Descriptor({
            ...
            resurrectionCount: 0,
            gameAllowed: false,
            lives: _livesPerSnook
        });
      }

      uint[] memory tokenIds = _snook.multimint(msg.sender, tokenURIs);
      _state.setDescriptors(tokenIds, descriptors); // this can be probably change'd to a single call
      
      _spc += count * _livesPerSnook;
