...
contract GamingToken is ChainlinkClient, ConfirmedOwner, ERC20 {
    ...
    mapping(uint256 => StakeStruct) nftStake;
    ...
    mapping(address => uint256) requestTime;

    uint256[] _stakedNfts;

    uint256 tokensPerMin;
    ...
    function stakeUnstakeStatus(uint256 _nftId, bool _stakeStatus) external {
        address user = _msgSender();
        require(user != address(0), "Invalid address");
    ...
    function NftBalances(uint256[] memory _nftIds) public view returns (uint256) {
        ...
        require(valid, "one or more Nfts not staked");
    ...
    function stakedNfts() external onlyOwner view returns(uint256[] memory, address[] memory) {
        address[] memory holders = new address[](_stakedNfts.length); 

        for(uint256 i = 0; i < _stakedNfts.length; i++) {
            //holders[i] = nftStake[_stakedNfts[i]].holderAddrs;
        }

        return (_stakedNfts,holders);
    }
