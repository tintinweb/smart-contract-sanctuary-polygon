...
interface IDatabase {
  ...
  function deactivateKyc() external;
...
interface IKonkreteVault {
  ...
  event InterestUpdated(address from, uint256 amount);
  event Paused(address account);
  ...
  function refundCapital(int256 capital_) external;
  ...
  function setCaps(uint soft, uint hard) external;

  function setDatabase(address database_) external;

  function setMaxDepositPerUser(uint256 amount) external;
  ...
  function updateInterest(uint256 interest) external;
  ...
  PausableUpgradeable
{
  ...
  //In this case , can be used as tokenMantissa
  ...
  uint public depositsStart;
  uint public depositsStop;

  uint public maxDepositPerUser;
  uint public softCap;
  uint public hardCap;

  uint public assetPrice;
  uint public collectedCapital;
   ...
   *****************************Events******************************************
   */
  event TotalRefunded(address from, uint amount);
  event InterestUpdated(address from, uint amount);
  event UnclaimedFundsCollected(uint amount);
  event CapitalCollected(uint amount);
  event CapitalLoss(uint originalCapital, uint remainingCapital, uint loss);
  event InterestRefunded(uint expected, uint refunded);
  event CapitalRefunded(uint amount, uint collected);

  event TimesUpdated(uint[2]);
  event TreasuryUpdated(address oldTreasury, address newTreasury);
  /**
   *****************************Errors******************************************
   */
  error WrongDecimalNumber(uint expected1, uint expected2, uint current);
  error DecimalOverFlow();
  ...
  error WrongDatabase();
  error WrongTreasury(address treasury);
  error WrongMaxDepositPerUser(uint max, uint amountWanted);
  error NegativeCapital(int capital_);
  ...
  modifier canDepositOrMint(address receiver) {
    address msgSender = _msgSender();

    if (msgSender != receiver && !dataBase.canBuy(receiver))
      revert ReceiverUnauthorized(receiver);

    if (!dataBase.canBuy(msgSender)) revert MsgSenderUnauthorized(msgSender);
  ...
  modifier withdrawalsEnabled() {
    SaleStep step = getStep();
    if (uint(step) == uint(SaleStep.SALE_COMPLETE)) revert WrongStep(step);
    _;
  }
  ...
  function initialize(
    ...
    uint assetDecimals = abi.decode(encodedDecimals, (uint));
    ...
    softCap = softCap_;
    hardCap = type(uint).max;
    depositsStart = depositsStart_;
    depositsStop = depositsStop_;
    treasury = multisig;
    originalPrice = assetPrice = 10 ** assetDecimals;
  ...
  function refundCapital(int capital_) external onlyRole(TREASURY) {
    if (capital_ < 0) revert NegativeCapital(capital_);

    int collectedCapital_ = int(collectedCapital);
    ...
    emit CapitalRefunded(uint(capital_), uint(collectedCapital_));

    int interest = capital_ - collectedCapital_;
    uint newPrice = assetPrice;

    if (interest < 0) {
      uint loss = uint(interest * -1);

      emit CapitalLoss(
        uint(collectedCapital_),
        uint(collectedCapital_) - loss,
        loss
      );

      newPrice = originalPrice - priceImpact(loss);
    } else {
      uint expectedInterest = (newPrice - originalPrice).mulDiv(
        totalSupply(),
        originalPrice
      );

      newPrice =
        uint(interest).mulDiv(originalPrice, totalSupply()) +
        originalPrice;

      emit InterestRefunded(expectedInterest, uint(interest));
    }
    assetPrice = newPrice;
    refunded = true;
  }

  // TIMELOCK functions {Check if if it's interesting}

  function collectUnclaimedFunds() external onlyRole(TIMELOCK) returns (uint) {
    SaleStep step = getStep();
    if (step != SaleStep.CAPITAL_REFUNDED)
      revert NotExpectedStep(SaleStep.CAPITAL_REFUNDED, step);
    IERC20Upgradeable stable = IERC20Upgradeable(asset());
    uint pendingFunds = stable.balanceOf(address(this));
  ...
  function collectCapital() external onlyRole(DEV) returns (uint) {
    SaleStep step = getStep();
    ...
    IERC20Upgradeable stable = IERC20Upgradeable(asset());
    uint collectedCapital_ = stable.balanceOf(address(this));
  ...
  function updateInterest(uint interest) external onlyRole(DEV) {
    uint increment = priceImpact(interest);
    assetPrice += increment;
  ...
  function setTimes(uint start, uint stop) external onlyRole(DEV) {
    if (start == 0 || start > stop) revert WrongSaleTimeStamps();
    depositsStart = start;
    depositsStop = stop;
  }

  function setCaps(uint soft, uint hard) external onlyRole(DEV) {
    if (soft == 0 || soft > hard) revert WrongCaps();
    softCap = soft;
    hardCap = hard;
  }

  function setMaxDepositPerUser(uint amount) external onlyRole(DEV) {
    uint softCap_ = softCap;
    if (amount > softCap_) revert WrongMaxDepositPerUser(softCap_, amount);
   ...
   * - The contract must be paused.
   */
  function unpause() external onlyRole(DEV) {
    _unpause();
  }
    ...
    returns (uint shares)
  {
    ...
    _withdraw(_msgSender(), receiver, owner, assets, shares);
    if (!refunded) collectedCapital -= assets;
  }
    ...
    returns (uint assets)
  {
    ...
    _withdraw(_msgSender(), receiver, owner, assets, shares);
    if (block.timestamp < depositsStop) collectedCapital -= assets;
  }

  //Public View functions

  /** @dev See {IERC4626-maxDeposit}.
  wee keep converting to asset  if we have another deposit phase */
  function maxDeposit(address sender) public view override returns (uint) {
    return getStep() != SaleStep.SALE ? 0 : _maxDeposit(sender);
  }

  /** @dev See {IERC4626-maxMint}. */
  function maxMint(address sender) public view override returns (uint) {
    return getStep() != SaleStep.SALE ? 0 : _maxMint(sender);
  }
  ...
  function priceImpact(uint amountOfInterest) public view returns (uint) {
    return amountOfInterest.mulDiv(10 ** decimals(), totalSupply());
  }
  ...
  /**
   * @notice Overrides the ERC4626 function check the amount of asset you can have qwith a certain amount of share
   *  ,after the rounds are finished ,
   *  use the price var (which raise with the interests)
  ...
  function _convertToAssets(
    ...
    assets = shares > 0
      ? shares.mulDiv(assetPrice, 10 ** super.decimals(), rounding)
      : 0;
  }

  /**
   * @notice Same but the input and output are reversed
   */
  ...
  function _convertToShares(
    ...
    shares = assets > 0
      ? assets.mulDiv(10 ** super.decimals(), assetPrice, rounding)
      : 0;
  }

  function _maxDeposit(address sender) internal view returns (uint) {
    uint hardCap_ = hardCap;

    uint tsInAssets = _convertToAssets(
      totalSupply(),
      MathUpgradeable.Rounding.Down
    );
    if (hardCap_ <= tsInAssets || maxDepositPerUser <= paid[sender]) return 0;

    uint theoricMax = maxDepositPerUser - paid[sender];
    uint suppliableMax = hardCap_ - tsInAssets;

    return suppliableMax > theoricMax ? theoricMax : suppliableMax;
  }

  function _maxMint(address sender) internal view returns (uint) {
    uint ts = totalSupply();

    uint hardCapInShares = _convertToShares(
      hardCap,
      MathUpgradeable.Rounding.Down
    );

    if (hardCapInShares <= ts || maxDepositPerUser <= paid[sender]) return 0;

    uint theoricMax = maxDepositPerUser - paid[sender];
    uint suppliableMax = hardCapInShares - ts;
