...
interface IBigBoyTable {
  event RoomCreatedWithKnownParticipants(uint roomId);
  event RoomCreated(uint roomId);
  event RoomFinished(uint roomId, address winner);
  ...
  function reportWinner(uint, address payable) external;
  function deleteAgedActiveRoom(uint) external;
  ...
  function getNative2USDC() external view returns(uint, uint);
  function setNative2USDC(uint, uint) external;
...
contract BigBoyTable is IBigBoyTable, AccessControlEnumerableUpgradeable, PausableUpgradeable {
  ...
  address private _foundersAddress;
  ...
  Room[] private _activeRooms;
  // The mapping is needed to manage _activeRooms unordered.
  // Unordering appears due to slicing _activeRooms array without using a loop.
  // https://blog.finxter.com/how-to-delete-an-element-from-an-array-in-solidity/
  mapping (uint=>uint) _activeRoomIdIndex;  
  Room[] private _finishedRooms;

  modifier onlyActiveRoom(uint roomId) {
    require(_activeRoomIdIndex[roomId] > 0, 'BBT: room is not active');
    _;
  }

  modifier whenStakePossible(uint roomId) {
    require(
      _activeRoomIdIndex[roomId] > 0, 
      'BBT: room is not active'
    );
    Room memory room = _activeRooms[_activeRoomIdIndex[roomId]];
    require(
      room.lockedAt > 0, 'BBT: room is not locked'
    );
    require(
  ...
  function _getEmptyRoom() internal pure returns(Room memory) {
    return Room({
      id: 0,
      participantCount: 0,
      participants: new address[](0),
      requiredStakeAmount: 0,
      isStakeInNativeCurrency: false,
      owner: address(0),
  ...
  function initialize(
    ...
    _setupRole(EXTRACTOR_ROLE, adminAccount);
    ...
    _native2usdcNumerator = native2usdcNumerator;
    _native2usdcDenominator = native2usdcDenominator;
    // Making index 0 in active rooms invalid.
    // Active rooms are located at indexes > 0.
    _activeRooms.push(_getEmptyRoom());
    // For consistency of room getters: indexes start from 1 both in active and finished rooms.
    _finishedRooms.push(_getEmptyRoom());
  }
  ...
  function _convertUSDC2Native(uint usdc) internal view returns(uint native) {
    native = usdc * _native2usdcNumerator / _native2usdcDenominator;
  }

  function getNative2USDC() external override view returns(uint, uint) {
    return (_native2usdcNumerator, _native2usdcDenominator);
  }

  function setNative2USDC(uint numerator, uint denominator) 
    external override onlyRole(PAUSER_ROLE)
  {
  ...
  // Room owner makes stake for all participants and locks the room 
  function createRoomWithKnownParticipants(
    ...
    Room storage room = _activeRooms[_activeRoomIdIndex[roomId]]; // pays creation fee in SNK
    room.totalStake = totalStake;
    room.participantCount = participants.length;
    room.participants = participants;
    room.lockedAt = block.timestamp;
    emit RoomCreatedWithKnownParticipants(roomId);
  }
  ...
  function _createRoom(
    ...
    // creation fee is taken ONLY in SNK
    require(
      _skill.transferFrom(msg.sender, _foundersAddress, _getRoomCreationPriceInSkill()),
    ...
    Room memory room = Room({
      id: roomId,
      participantCount: 0,
      participants: new address[](maxParticipantCount),
    ...
    _activeRoomIdIndex[roomId] = _activeRooms.length;
    _activeRooms.push(room);
    return roomId;
  }
  ...
  function lockRoom(uint roomId) external override onlyActiveRoom(roomId) {
    uint roomIndex = _activeRoomIdIndex[roomId];
    Room storage room = _activeRooms[roomIndex];
    ...
    external override whenStakePossible(roomId) 
  {
    ...
    room.participants[room.participantCount] = msg.sender;
  ...
  {
    Room storage room = _activeRooms[_activeRoomIdIndex[roomId]];
    require(
  ...
  function _deleteActiveRoom(uint roomId) internal returns(Room memory) {
    uint roomIndex = _activeRoomIdIndex[roomId];
    Room memory lastRoom = _activeRooms[_activeRooms.length-1];
    Room memory room = _activeRooms[roomIndex];
    _activeRooms[roomIndex] = lastRoom;
    _activeRoomIdIndex[lastRoom.id] = roomIndex; 
    delete _activeRoomIdIndex[roomId];
    _activeRooms.pop();
    return room;
  }

  function _moveRoomFromActiveToFinished(uint roomId) internal returns(Room memory) {
    Room memory room = _deleteActiveRoom(roomId);
    _finishedRooms.push(room);
    return room;
  }

  function _isParticipant(Room memory room, address winner) internal pure returns(bool) {
  ...
  ) external override onlyActiveRoom(roomId) onlyRole(EXTRACTOR_ROLE) 
  {
    Room memory room = _moveRoomFromActiveToFinished(roomId);
    require(
      block.timestamp > room.createdAt + _stakingWindowInSeconds,
      "BBT: cannot report winner before staking window is closed"
    );
    // This requirement ensures a stake was done.
    require(
      _isParticipant(room, winner) == true, 
      'BBT: reported winner is not participant'
    );
    uint winningFeeAmount = room.totalStake * _winningFeeInPercents / 100;
    uint prizeAmount = room.totalStake - winningFeeAmount;
    if (room.isStakeInNativeCurrency == false) {
      _skill.transfer(winner, prizeAmount);
    } else {
      winner.transfer(prizeAmount);
    }
    emit RoomFinished(roomId, winner);
  }

  function _getRoomCount(bool isActiveRooms) internal view returns (uint) {
    // length-1 as 0th room is unused.
    return isActiveRooms == true ? _activeRooms.length-1 : _finishedRooms.length-1 ;
  }

  function getActiveRoomCount() external override view returns(uint) {
    return _getRoomCount(true);
  }

  function getFinishedRoomCount() external override view returns(uint) {
    return _getRoomCount(false);
  }
  
  // endIdx is included
  function _getRooms(Room[] memory rooms, uint startIdx, uint endIdx) internal pure
    returns(Room[] memory)
  {
    require(
      startIdx >= 1 &&
    ...
    Room[] memory slice = new Room[](endIdx-startIdx+1);
    for (uint i=startIdx; i<=endIdx; i++) {
      slice[i-startIdx] = rooms[i];
    } 
    return slice;
  }
  ...
  function getActiveRooms(uint startIdx, uint endIdx) external override view 
    returns(Room[] memory)
  {
    return _getRooms(_activeRooms, startIdx, endIdx);
  }

  function getFinishedRooms(uint startIdx, uint endIdx) external override view 
    returns(Room[] memory)
  {
    return _getRooms(_finishedRooms, startIdx, endIdx);
  }
  ...
  function deleteAgedActiveRoom(uint roomId) external override onlyActiveRoom(roomId) {
    ...
    Room memory room = _activeRooms[roomIndex];
    require(
      block.timestamp > room.createdAt + _activeRoomDeletionDelayInSeconds,
      "BBT: room is not aged enough"
    );
    _deleteActiveRoom(roomId);
