...
contract OPTGovernorOverwrite is Governor, GovernorSettings, GovernorCountingSimple, GovernorVotes, GovernorVotesQuorumFraction {
    ...
    modifier onlyOwner() {
        if(_owner != address(0))
            require(msg.sender == _owner, "OPTGovernorOverwrite: caller is not the owner");
        
        _;
    }

    address[] private excludedAddressesArray;

    constructor(IVotes _token)
        Governor("OPTGovernor")
        GovernorSettings(0,300,0)
    ...
    function execute(
        ...
        bytes[] memory calldatas,
        bytes32 descriptionHash
    ) public payable virtual override onlyOwner returns (uint256) {
