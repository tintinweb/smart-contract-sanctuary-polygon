...
contract MaykNFTFactory is Ownable {
    mapping (uint256 => address) private maykNFTs;
    ...
    function mint(
        ...
        MaykNFT maykNFT = new MaykNFT(title, tokenURI, contractURI, ownerAddress, royaltyAddress);
        maykNFT.transferOwnership(msg.sender);
        address maykNFTAddress = address(maykNFT);
        maykNFTs[_tokenIdCounter.current()] = maykNFTAddress;
        return maykNFTAddress;
    }
    ...
    function contractAddress(uint256 tokenId) public view returns (address) {
        return maykNFTs[tokenId];
    }
}
