 ...
 *
 */
abstract contract ERC721Roles is AccessControlUpgradeable

{
  ...
  function __ERC721Roles_init() internal {
    ...
    ERC2981Upgradeable
{
    ...
    function initialize(string memory _name, string memory _symbol) external initializer {
        ...
        __ERC721Roles_init();
        __EIP712_init("ERC721Virtualness", "1.0");
    ...
    * @param data          Mint721Data
    * @param to          Token ID to mint
    */
    function lazyMint(
        ...
        for (uint i = 0; i < data.creators.length; ++i) {
            ...
            if (creator != sender) {
                ...
                require(recover(hash, data.signatures[i]) == creator, "Creator signature is invalid");
            }
        }

        _setTokenURI(data.tokenId, data.tokenURI);
        _setTokenRoyalty(data.tokenId, data.royalties[0].account, data.royalties[0].value);
        creators[data.tokenId] = data.creators[0].account;
    ...
    /**
     *  @dev Returns the creator for given tokenid.
     *  @param  _id tokenId
     */
    function getCreators(uint256 _id) external view returns (address) {
        return creators[_id];
    }
...
import {OrderParameters, Fee, Collection } from "../protocol-v1/lib/OrderStructs.sol";
