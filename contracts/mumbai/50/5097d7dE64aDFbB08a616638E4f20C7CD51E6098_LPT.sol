...
// File: contracts/adr-liquidity-pool.sol

//SPDX-License-Identifier: Unlicense
pragma solidity 0.8.17;

contract LPT is Ownable {

    function addLiquidity(address ADRContractAddr, uint256 amount) external onlyOwner {
        Token(ADRContractAddr).mint(amount);
    }

    function removeLiquidity(address ADRContractAddr, uint256 amount) external onlyOwner {
        Token(ADRContractAddr).burn(amount);
    }

       function transfer(address ADRContractAddr,address to, uint amount) external onlyOwner {
        Token(ADRContractAddr).transfer(to, amount);
    } 
