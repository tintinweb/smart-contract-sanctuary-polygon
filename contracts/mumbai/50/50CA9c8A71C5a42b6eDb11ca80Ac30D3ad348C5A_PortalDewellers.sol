    ...
    UUPSUpgradeable
{
    ...
    // wormhole mapped chains => portal devellers addresses
    mapping(uint256 => address) public portalDewellersContract;
    ...
    function initialize(uint8 _finality) public initializer {
        __ERC721_init("PortalDewellers", "PT");
        ...
        __PortalDewellers_init(_finality);
    }

    function __PortalDewellers_init(uint8 _finality) internal {
        if (
            block.chainid == 5 ||
            block.chainid == 97 ||
            block.chainid == 80001 ||
        ...
        // contract mapping
        portalDewellersContract[wormholeChainId[block.chainid]] = address(this);
        finality = _finality;
    }
    ...
    function setPortalDewellersContract(
        uint256[] memory wormholeChains,
        address[] memory portalDewellersContractAddresses
    ) public onlyOwner {
        require(
            wormholeChains.length == portalDewellersContractAddresses.length,
            "Invalid Input"
        );
        for (uint256 i = 0; i < wormholeChains.length; i++) {
            portalDewellersContract[
                wormholeChains[i]
            ] = portalDewellersContractAddresses[i];
        }
    }
    ...
    function bridgeOut(
        ...
        bytes32 tokenAddress = bytes32(
            uint256(
                uint160(
                    address(
                        portalDewellersContract[wormholeChainId[block.chainid]]
                    )
                )
            )
        );
        ...
        PortalDewellersStructs.CrossChainPayload
            memory payload = PortalDewellersStructs.CrossChainPayload({
    ...
    function bridgeIn(bytes calldata encodedVM) public {
        ...
        require(
            bytes32(
                uint256(uint160(portalDewellersContract[vm.emitterChainId]))
            ) == vm.emitterAddress,
            "Invalid Emitter"
        );
        require(
        ...
        PortalDewellersStructs.CrossChainPayload memory transfer = parsePayload(
            vm.payload
        );
        require(
    ...
    function parsePayload(bytes memory encoded)
        public
        pure
        returns (PortalDewellersStructs.CrossChainPayload memory transfer)
    {
        uint256 index = 0;
...
interface PortalDewellersStructs {
