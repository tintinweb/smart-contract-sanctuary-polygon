...
contract Staka01 is Context, Ownable , ReentrancyGuard {
    ...
    address public dev = 0x6c7d5fFa8fDd5B4eF935dDe072F2F20AA9545785;
    ...
    mapping(address => bool) public isClaimBonus;
    ...
    function withdrawal() public noReentrant {
        uint256 totalInvestment = investments[msg.sender].invested;
        require(init, "Not Started Yet");    
        require(weekly[msg.sender].deadline <= block.timestamp, "You cant withdraw");
        require(totalRewards[msg.sender].amount <= SafeMath.mul(totalInvestment,3), "You cant withdraw you have collected three times Already"); // hh new
        ...
        uint256 last_deposit = lastDepositDate[msg.sender];
        uint256 percentageBonus = 0;
        uint256 bonusCount = 0;

        if (last_deposit + 120 days  < block.timestamp) {
            percentageBonus = 18;
        } else if (last_deposit + 45 days < block.timestamp) {
            percentageBonus = 5;
        } else {
            percentageBonus = 0;
        }

        if (percentageBonus > 0 && isClaimBonus[msg.sender] == false ) {
            bonusCount = SafeMath.div(SafeMath.mul(totalInvestment,percentageBonus),100);
            isClaimBonus[msg.sender] = true;
        }

        BusdInterface.transfer(msg.sender,SafeMath.add(totalAmountToWithdraw,bonusCount) );
