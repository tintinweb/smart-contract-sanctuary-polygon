...
contract Vesting is Ownable {
    ...
    function init(
        ...
        for (uint256 roundId = 0; roundId < roundLength; ++roundId) {
            ...
            rounds[roundId].schedule = rounds_[roundId].schedule;
        }
    ...
    /// @dev function to change round parameters (only owner)
    /// @param roundId id of round
    /// @param name name of round
    /// @param startTimestamp_ timestamp of start
    /// @param unlockPeriod_ period of unlock
    /// @param unlockSpeed_ speed of unlock
    function changeRound(
        uint256 roundId,
        string memory name,
        uint256 startTimestamp_,
        uint256 unlockPeriod_,
        uint256 unlockSpeed_
    ) external onlyOwner {
        if (roundId >= roundLength) revert Zero();
        if (block.timestamp >= rounds[roundId].schedule.startTimestamp)
            revert AlreadyStarted();
        if (rounds[roundId].schedule.startTimestamp > startTimestamp_)
            revert AlreadyStarted();

        rounds[roundId].name = name;
        rounds[roundId].schedule.startTimestamp = startTimestamp_;
        rounds[roundId].schedule.unlockPeriod = unlockPeriod_;
        rounds[roundId].schedule.unlockSpeed = unlockSpeed_;
    }
    ...
    function unlocked(
        ...
        if (block.timestamp <= round.schedule.startTimestamp) return 0;

        uint256 period = block.timestamp - round.schedule.startTimestamp;
        uint256 totalAmount = (period < round.schedule.unlockPeriod)
            ? (((allocation * round.schedule.unlockSpeed) / 1 ether) * period)
            : allocation;
    ...
    function _updateClaimed(address user, uint256 id, uint256 amount) internal {
