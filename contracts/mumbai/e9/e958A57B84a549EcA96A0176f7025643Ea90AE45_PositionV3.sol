...
pragma solidity ^0.8.0;
//getAmountsOut need to be worked
pragma abicoder v2;
...
contract PositionV3 is ReentrancyGuard, Ownable, PositionV2Storage {
    ...
    function createPositionAndTradeV3(
        address tokenAddress,
        address targetToken,
        uint256 amount,
        address userAddress
    ) public returns(uint256,uint256, uint256,uint256, address[] memory) {
        if (tokenAddress == WETH9) {
            tokenAddress = address(0);
        }
        (uint256 availableMargin, uint256 loanAmount, , 
        ...
        // pair should exist in quickswap check
        ...
        // IMargin(IAdminFunctions(adminContract).getMarginContract())
        //     .setPositionAmount(user, tokenAddress, marginUsed, loanUsed, 2);
        ...
        //     closedPrice: 0,
        //     marginPrice: getMarginPrice(amount, marginUsed, result, path),
    ...
    function trade(
        ...
        uint price = getAmountsOut(1 * 10 ** decimals, path);

        uint256 protocolFees;
        if (source == 1) {
            uint256 expectedUnits = getAmountsOut(tradeAmount, path);
            //0.2% of expected units as protocol fees
            uint256 protocolFeePercent = IAdminFunctions(adminContract)
                .getProtocolFees();
            protocolFees = (expectedUnits * protocolFeePercent) / 10000;
        }
        ...
        if (source == 1) {
            ...
            ).withdrawFunds(payable(address(this)), tradeAmount);
        }
        ISwapRouter.ExactInputSingleParams memory params = ISwapRouter
            .ExactInputSingleParams(
                ...
                path[1],
                3000,
                msg.sender, //replace it with address(this)
                deadline,
                tradeAmount,
                1,
                0
        ...
        if (path[0] == WETH9) {
            result = uniswapV3Router.exactInputSingle{value: msg.value}(params);
        }
        else {
            result = uniswapV3Router.exactInputSingle(params);
        }
        price =
            (getConvertedAmount(decimals, tradeAmount) * (10 ** 18)) /
            (getConvertedAmount(IERC20Metadata(path[1]).decimals(), result));
        if (source == 1) {
            //deducting the protocol fees
            result = result - protocolFees;
            //this protocol fees should be transferred to ProtocolReserves contract
            if (path[1] == address(0) || path[1] == WETH9) {
                (bool successTransfer, ) = IAdminFunctions(adminContract)
                    .getProtocolReserves()
                    .call{value: protocolFees}("");
                require(
                    successTransfer,
                ...
                IERC20(path[1]).transfer(
                    IAdminFunctions(adminContract).getProtocolReserves(),
                    protocolFees
                );
            }
        }
    ...
    function closePositionV3(address asset,  address targetAsset, uint256 targetAmount) public
    returns(uint256, uint256) {
        ...
        path_[1] = path[0];
        //Get price
        uint256 price = getAmountsOut(1 * 10 ** decimals, path_);
        // positionInfo.reverseAmount = _result;
        // positionInfo.closedPrice = price;
        // positionInfo.status = 1;
        //checkfor profit or loss
        //(uint256 marginAmount, uint256 temp) = closeAmount(positionId, _result);

        // if (positionInfo.asset == WETH9) {
        //     IMargin(IAdminFunctions(adminContract).getMarginContract())
        //         .closePositionAmount(
        ...
        if (asset != address(0) && asset != WETH9) {
            require(
                IERC20(asset).balanceOf(address(this)) >= _result,
                "Position: No balance"
            );
            ...
            require(
                address(this).balance >= _result,
                "Position: Less matic balance"
            );
            (bool successTransfer, ) = IAdminFunctions(adminContract)
                .getMarginFundsContract()
                .call{value: _result}("");
            require(
                successTransfer,
                "Position:Transfer to margin funds contract failed"
    ...
    function liquidateV3(address asset, uint256 targetAmount, address[] memory path) public 
    returns(uint256, uint256)  {
        ...
        if (path[1] == address(0) || path[1] == WETH9) {
            (bool successTransfer, ) = IAdminFunctions(adminContract)
                .getLiquidationReserves()
                .call{value: liquidationPenalty}("");
            require(
                successTransfer,
                "Position: Transfer to margin funds contract failed"
            );
    ...
    function getCurrentPriceV3(address[] memory path) public returns (uint256) {
        // Since price should be in same asset
        // Calculate path again and get price
        uint256 decimals = IERC20Metadata(path[0]).decimals();
        // Get Price
        uint256 price = getAmountsOut(1 * 10 ** decimals, path);
        if (IERC20Metadata(path[1]).decimals() == 18) {
            return ((1 * 10 ** 18) / price);
        } else {
