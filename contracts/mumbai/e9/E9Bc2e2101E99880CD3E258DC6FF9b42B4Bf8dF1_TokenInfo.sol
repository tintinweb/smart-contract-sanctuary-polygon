...
contract TokenInfo is AccessControl {
    ...
    //token address => vaults for CrowdFunding Phase.
    mapping(address => CrowdVaults) internal vaultsCrowdFunding;
    ...
    function addVaultRegistry(address _token, address _lender,address _lenderRewards) public onlydev  {
        vaultsInfo[_token].lender = _lender;
        vaultsInfo[_token].lenderRewards = _lenderRewards;
    }

    /// @dev Function to add the Vaults used for crowfunding.
    function addCrowdVaults(address _token, address _Crowdfunding,address _FoundersRewards) public onlydev  {
        vaultsCrowdFunding[_token].vCrowdfunding = _Crowdfunding;
        vaultsCrowdFunding[_token].vFounderRewards = _FoundersRewards;
    }
    ...
    function getVaultInfo(address _token) public view returns(address _lender, address _lenderRewards){
        return ( vaultsInfo[_token].lender,
        vaultsInfo[_token].lenderRewards);
    }

   function getCrowdVaults(address _token) public view returns(address , address){
        return (
            vaultsCrowdFunding[_token].vCrowdfunding,
            vaultsCrowdFunding[_token].vFounderRewards
        );
    }
