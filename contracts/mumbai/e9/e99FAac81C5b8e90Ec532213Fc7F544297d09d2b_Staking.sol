...
contract Staking {
    ...
    // Parameters
    uint128 public constant VALIDATOR_THRESHOLD = 10000000000000000; // 1 ether
    ...
    mapping(address => address) public _addressToWithdrawalAddress; // from signer/validator -> staker
    ...
    // proposer delegation
    uint128 public constant DELEGATION_THRESHOLD = 1000000000000000; // 0.1 ether
    ...
    function _stake() private {
        require(
            ...
            "accounts that called staked(signer) must not call this stake() function"
        );
    ...
    function _unstake() private {
        ...
        // if address staked with function stake(signer)
        if (_isStaker(msg.sender)) {
            signer = _addressToWithdrawalAddress[msg.sender];
        } else if (_isSigner(msg.sender)) {
            staker = _addressToWithdrawalAddress[msg.sender];
        }
        _addressToIsStaker[staker] = false;
        _addressToIsSigner[signer] = false;
        _addressToWithdrawalAddress[signer] = address(0);
    ...
    function _stake(address signer) private {
        ...
        if (_isStaker(msg.sender) && _isSigner(signer)) {
            require(
                _addressToWithdrawalAddress[signer] == msg.sender,
            ...
            !_isSigner(signer) &&
            _addressToWithdrawalAddress[signer] == address(0),
                "both addresses are not valid to call func. Reset them by calling unstake for both accounts manually"
            );
        }
