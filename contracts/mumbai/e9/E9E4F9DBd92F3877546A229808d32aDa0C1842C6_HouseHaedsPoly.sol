...
contract HouseHaedsPoly is ERC721EnumerableUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, ERC2771ContextUpgradeable {
  ...
  mapping(uint256 => TokenRarity) public tokenRarity;
  ...
  function mintForGame(address gameAddress, uint256 startTokenId, uint256 amount) external onlyOwner {
    require(!paused, "The contract is paused!");
    for (uint256 i = 0; i < amount; i++) {
      _safeMint(gameAddress, startTokenId + i);
    }
  }
  ...
  function setTokenRarities(uint256[] memory _tokenIds, TokenRarity[] memory _rarities) public onlyOwner {
    require(_tokenIds.length == _rarities.length, "Invalid input!");
    for (uint256 i = 0; i < _tokenIds.length; i++) {
      tokenRarity[_tokenIds[i]] = _rarities[i];
    }
  }
