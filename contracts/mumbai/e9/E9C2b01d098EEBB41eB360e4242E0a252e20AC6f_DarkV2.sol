...
contract DarkV2 is IDollar, ERC20Burnable, Ownable, ReentrancyGuard {
    ...
    event MinterCapUpdated(address indexed account, uint256 newCap);
    ...
    constructor(address _treasury, uint256 _genesis_supply) ERC20("DarkCrypto 2.0", "DARK") {
        treasury = _treasury;
        if (_genesis_supply > 0) {
            __mint(msg.sender, _genesis_supply); // will be minted at genesis for liq pool seeding
        }
    }
    ...
    function setMinterCap(address _account, uint256 _cap) external onlyOwner {
        minterCap[_account] = _cap;
        emit MinterCapUpdated(_account, _cap);
    }
    ...
    function poolMint(address _address, uint256 _amount) external override onlyPool {
        __mint(_address, _amount);
    }

    function mint(address _address, uint256 _amount) external override {
        require(minterCap[msg.sender] >= _amount, "!minter");
        minterCap[msg.sender] -= _amount;
        __mint(_address, _amount);
    }

    function __mint(address _address, uint256 _amount) internal {
