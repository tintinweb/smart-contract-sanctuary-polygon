...
import {PixelynxOwnership} from './PixelynxOwnership.sol';
import {SharedStructs} from '../shared-structs/SharedStructs.sol';
import {PixelynxNft} from './PixelynxNft.sol';

contract PixelynxNftDeployer is IPixelynxNftDeployer, Context, Ownable {
    mapping(address => bool) private _allAirdroppers;
    mapping(address => bool) private _admins;
    mapping(address => bool) private _allowedToGift;

    address[] private _allDeployedContracts;

    address private _pixelynxOwnershipAddress;

    constructor(address knownOwnership) {
        if (knownOwnership != address(0)) {
            _pixelynxOwnershipAddress = knownOwnership;
        } else {
            PixelynxOwnership ownership = new PixelynxOwnership(address(this));
            _pixelynxOwnershipAddress = address(ownership);
        }
    }
    ...
    modifier isAirdropper() {
        require(_allAirdroppers[_msgSender()], 'AIR_O');
        _;
    }
     ...
     */
    modifier isAdmin() {
        require(_admins[_msgSender()], 'ADMIN_O');
        _;
    }
    ...
    modifier isAdminOrSuperAdmin() {
        require(_admins[_msgSender()] || this.owner() == _msgSender(), 'SA_O or ADMIN_O');
        _;
    }

    /// @inheritdoc IPixelynxNftDeployer
    ...
    function deployNewCollection(SharedStructs.NewCollectionInitialize memory collectionInitialize)
        ...
        returns (address)
    {
        for (uint256 i = 0; i < collectionInitialize.internalTokenGatedContracts.length; i++) {
            require(
                IPixelynxOwnership(_pixelynxOwnershipAddress).isDeployedContract(
                    collectionInitialize.internalTokenGatedContracts[i]
                ),
                'Invalid tokens'
            );
        }

        PixelynxNft collect = new PixelynxNft(address(this), _pixelynxOwnershipAddress, collectionInitialize);
        address collectionNft = address(collect);

        _allDeployedContracts.push(collectionNft);

        IPixelynxOwnership(_pixelynxOwnershipAddress).setDeployedContract(collectionNft);
    ...
    /// @inheritdoc IPixelynxNftDeployer
    function setAllowedToGift(address user, bool allowed) external isAdminOrSuperAdmin {
        _allowedToGift[user] = allowed;
    }

    /// @inheritdoc IPixelynxNftDeployer
    function isAllowedToGift(address user) public view returns (bool) {
        return _allowedToGift[user] || _admins[user] || this.owner() == user || _allAirdroppers[user];
    }

    /// @inheritdoc IPixelynxNftDeployer
    function getPixelynxOwnership() external view returns (address) {
        return _pixelynxOwnershipAddress;
    }

    /// @inheritdoc IPixelynxNftDeployer
    function setPixelynxOwnership(address ownershipContract) external isAdminOrSuperAdmin {
        _pixelynxOwnershipAddress = ownershipContract;
    }
    ...
    function getRoles(address[] memory addresses) external view returns (SharedStructs.Roles[] memory) {
        ...
        for (uint256 i = 0; i < addresses.length; i++) {
            ...
            SharedStructs.Roles memory hasRole = SharedStructs.Roles({
                ...
                isAllowedToGift: isAllowedToGift(_address)
            });
            roles[i] = hasRole;
        }
...
/**
 * @title Pixelynx Nft Deployer Contract
...
interface IPixelynxNftDeployer {
    ...
    /**
     * Used to set the allowed to gift state contracts or wallets
     */
    function setAllowedToGift(address user, bool allowed) external;

    /**
     * Get the state if the address is allowed to gift
     */
    function isAllowedToGift(address user) external view returns (bool);

    /**
     * Get the state for the pixelynx ownership contract
     */
    function getPixelynxOwnership() external view returns (address);

    /**
     * Set the pixelynx ownership contract
     */
    ...
    IERC4906
{
    ...
    address private _pixelynxNftDeployer;
    ...
    uint256 _maxSupply;
    bool _random;
    address[] _internalTokenGatedContracts;
    // beneficiaries
    ...
    // mapping to see if tokens have been minted
    ...
    /**
     * @dev Constructor function for the PixelynxNft contract.
     * Initializes various contract state variables such as name, symbol, price, maxSupply, beneficiaries, royalties, etc.
     * @param __pixelynxNftDeployer The address of the contract deployer.
    ...
    constructor(
        ...
        _pixelynxNftDeployer = __pixelynxNftDeployer;
        ...
        _random = collectionInitialize.mintRandom;
        ...
        if (collectionInitialize.maxSupply > 0) {
            ...
            // unlimited supply
            _maxSupply = 0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff;
             _unusedTokensLeft = _maxSupply;
        }
        ...
        _transferOwnership(IPixelynxNftDeployer(_pixelynxNftDeployer).getSuperAdmin());
    }
    ...
    modifier isPixelynSuperAdmin() {
        require(IPixelynxNftDeployer(_pixelynxNftDeployer).getSuperAdmin() == _msgSender(), 'SA_O');
        _;
    }
    ...
    modifier isPixelynAdmin() {
        require(IPixelynxNftDeployer(_pixelynxNftDeployer).getAdminState(_msgSender()), 'A_O');
        _;
    }
     ...
     */
    modifier isAirdropper() {
        require(IPixelynxNftDeployer(_pixelynxNftDeployer).getAirdropperState(_msgSender()), 'AIR_O');
        _;
    }
    ...
    /**
     * @dev Get the pixelynx nft deployer contract address
     */
    function pixelynxNftDeployer() public view returns (address) {
        return _pixelynxNftDeployer;
    }

    /**
     * @dev Set the pixelync nft deployer contract address
     */
    function setPixelynNftDeployer(address newPixelynNftDeployer) external isPixelynSuperAdmin {
        _pixelynxNftDeployer = newPixelynNftDeployer;
    }
    ...
    function setAllowlistStartTimestamp(uint256 _timestamp) external isPixelynAdmin {
        if (_timestamp > _mintStartTimestamp) revert TimestampHigherThanMintTimestamp();
    ...
    /**
     * @dev Set internal token gated contracts
     */
    ...
    /**
     * @dev Get internal token gated contracts
     */
    ...
    /**
     * @dev Get if the mint is random
     */
    function isRandomMint() external view returns (bool) {
        return _random;
    }
    ...
    function _mint(address to, uint256 tokenId) private {
        ...
        // Update the ownership state in the PixelynxOwnership contract
    ...
    /**
     * @dev Used for paper wallet integration
     */
    function checkClaimEligibility(address to, uint256 numToMint) external view returns (string memory) {
        if (
            !inMintablePeriod() || _killSwitch || to == address(0) || numToMint <= 0 || _unusedTokensLeft < numToMint
                ...
                || !IPixelynxOwnership(_pixelynxOwnershipContract).userOwnershipCheck(_internalTokenGatedContracts, to)
        ) return 'Can not mint';
    ...
    function mintFor(address to, uint256 numToMint) external nonReentrant {
        if (!IPixelynxNftDeployer(_pixelynxNftDeployer).isAllowedToGift(_msgSender())) revert NotAllowedToGift();
        if (numToMint <= 0) revert MintAtLeastOneToken();
        if (_unusedTokensLeft < numToMint) revert MintMoreTokensThanSupply();

        // Mint the tokens
        if (_random) _mintRandom(to, numToMint);
        else _mintSequential(to, numToMint);
    }

    /**
     * @dev mint token to `to`.
     *
     * Requirements:
     *
     * - `to` must exist.
    ...
    function mint(address to, uint256 numToMint) external payable nonReentrant {
        if (!inMintablePeriod()) revert MintNotStarted();
        if (_killSwitch) revert MintIsPaused();
        if (to == address(0)) revert MintToZeroAddress();
        if (numToMint <= 0) revert MintAtLeastOneToken();
        if (_unusedTokensLeft < numToMint) revert MintMoreTokensThanSupply();
        if (msg.value != _price * numToMint) revert MintNotEnoughFunds();

        if (inAllowlistPeriod()) {
            if (_allowlist[to] < numToMint) revert MintNotEnoughAllowance();
        ...
        // free mint not need to split payments!
        if (msg.value > 0) {
            // Split the payments
            _splitPayments();
        }
    ...
    /**
     * @dev Mint `numToMint` sequential tokens and assign them to `to`.
     * @param to The address to assign the minted tokens to.
     * @param numToMint The number of tokens to mint.
     */
    function _mintSequential(address to, uint256 numToMint) private {
        ...
        _unusedTokensLeft -= numToMint;
        _balances[to] += numToMint;
    }

    /**
     * @dev airdrops NFTs to addresses.
     * This mints an NFT to the wallet
     *
     * Requirements:
     *
    ...
    function airdrop(address[] calldata recipients) external isAirdropper {
        if (recipients.length == 0 || _unusedTokensLeft < recipients.length) {
            revert AirdropEmptyReceipientListOrMintingMoreThenSupply();
        }

        for (uint256 i; i < recipients.length; ++i) {
            // Mint the tokens
            if (_random) _mintRandom(recipients[i], 1);
