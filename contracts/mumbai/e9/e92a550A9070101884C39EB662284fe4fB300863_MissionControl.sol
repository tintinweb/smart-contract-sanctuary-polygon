...
contract MissionControl is IMissionControl, OwnableUpgradeable, EIP712Upgradeable {
        ...
        for (; i < length; i++) {
            ...
            if (tile.nonce != 0 && tile.pausedAt != 0) {
                ...
                if (__zeroOrOne(placeOrderItem.order.x, placeOrderItem.order.y, _renter) == 1) {
                    //TODO: rhs can be replaced with tileElements[0]
                    TileElement storage bottom = tiles[_renter][placeOrderItem.order.x][placeOrderItem.order.y][0];
                    bottom.pausedAt = 0;
                }
                unchecked {
                    ++tilesRented;
    ...
    function _handleBasePlacement(
        ...
        IMissionControlStakeable(_stakeable).onStaked(_staker, _tokenId, _nonce, address(0), 0);
    ...
    function __placeNFT(
        ...
        if (IMissionControlStakeable(tokenAddress).isBase(_tokenId)) {
            _handleBasePlacement(order.x, order.y, order.z, _tokenId, tokenAddress, ++stakeNonces[_staker], _staker);
        } else {
            _handleTopPlacement(order.x, order.y, order.z, _tokenId, tokenAddress, ++stakeNonces[_staker], _staker);
        }
