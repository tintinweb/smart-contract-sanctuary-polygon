...
contract Hashtags {
    ...
    */
    ...
    event logAddHashtags(string[] hashtags, uint256 indexed msgID);
    event logRemoveHashtags(string[] hashtags, uint256 indexed msgID);
    ...
    */
    function getMsgIDsFromHashtag(string memory hashtag, uint256 startFrom) public view returns(uint256[] memory) {
    ...
    function addHashtags(uint256 msgID, string[] memory hashtagsToToggle) public onlyAdmins {
        ...
        for (uint h=0; h < hashtagsToToggle.length; h++) {
            ...
            // Lowercase the hashtag first
            hashtagsToToggle[h] = KUtils._toLower(hashtagsToToggle[h]);

            // Disable the hashtag to the message
            uint256 bucketKeyID = getBucketKey(hashtagsToToggle[h], true);
            string memory hashtagBucketKey = KUtils.append(hashtagsToToggle[h],'-',KUtils.toString(bucketKeyID),'','');
            ...
            if (!hashtagMapMap[hashtagBucketKey][msgID]){
                ...
                delete hashtagBucketKey;
                delete bucketKeyID;
            }
        }

        // Emit to the logs for external reference
        emit logAddHashtags(hashtagsToToggle, msgID);
    }
    ...
    function removeHashtags(uint256 msgID, string[] calldata hashtagsToToggle) public onlyAdmins {
        ...
        delete hashtagToToggle;

        // Emit to the logs for external reference
        emit logRemoveHashtags(hashtagsToToggle, msgID);
    }
