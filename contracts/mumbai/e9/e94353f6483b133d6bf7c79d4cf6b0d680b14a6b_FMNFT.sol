...
contract FMNFT is ERC721, Ownable {
    
    using SafeMath for uint256;
    using Strings for uint256;
    bool public enableReward;
            ...
            if (enableReward) {
                ...
                if (pool.rewardLastStored > 0) {
                    ...
                    if (account != address(0)) {
                        pool.newReward[account] = available(account);
    ...
    function claim() external updateDispatch(msg.sender) payable {
        PoolInfo storage pool = poolInfos[deadAddress];
