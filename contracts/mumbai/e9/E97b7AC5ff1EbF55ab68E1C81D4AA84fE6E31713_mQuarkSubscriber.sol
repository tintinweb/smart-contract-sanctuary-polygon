...
contract mQuarkSubscriber is AccessControl, ImQuarkSubscriber, ReentrancyGuard, NoDelegateCall {
  ...
  event SubscribedBatch(uint256 tokenId, address tokenContract, uint64[] subscriptionIds, address to,string[] defaultUris, uint256 amount);
  event Subscribed(uint256 tokenId, address tokenContract, uint256 subscriptionId, address to, string defaultUri, uint256 amount);
  ...
  function subscribe(
    ...
    emit Subscribed(_tokenId, _tokenContract, _subscriptionId, msg.sender,m_projectConfig.defaultURI, msg.value);
  }
