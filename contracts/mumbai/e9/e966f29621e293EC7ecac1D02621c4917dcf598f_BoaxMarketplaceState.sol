...
contract BoaxMarketplaceState is Ownable {
     ...
     *
     */
    function setTokenArtist(uint256 _tokenId, address _artist)
        external
        onlyContracts
    {
    ...
    function setMarketplaceFee(uint24 _fee) external onlyOwner {
        // solhint-disable-next-line
        require(_fee > 0 && _feeLimit(_fee, _artistFee)); // new fee should not exceed the limit i.e it should be be greater than 0 and less than 90% including artist fee and marketplace fee
    ...
    function setSecondarySaleFeeArtist(uint24 _fee) external onlyOwner {
        // solhint-disable-next-line
        require(_fee > 0 && _feeLimit(_fee, _marketplaceFee)); // new fee should not exceed the limit i.e it should be be greater than 0 and less than 90% including artist fee and marketplace fee
        _artistFee = _fee;
    }
    ...
    constructor(address feeAccount) {
        ...
        _artistFee = 500; // 5% fee (profits) on all secondary sales paid to the artist (seller receives remainder after paying 5% secondary fees to artist and 5% to marketplace, 90%)
        //structs
    }
}
