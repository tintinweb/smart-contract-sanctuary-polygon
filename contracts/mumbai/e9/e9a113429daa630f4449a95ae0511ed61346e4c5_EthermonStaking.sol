...
interface EthermonStakingInterface {
    ...
    function NewSumTeamPower() external returns (uint256);

    function Counter() external returns (uint256);

    function EmonPerPeriod() external returns (uint64);

    function updateNewSumTeamPower(uint256 _sumTeamPower) external;

    function updateSumTeamPower(uint256 _newSumTeamPower) external;
    ...
    function increaseSumTeamPower(uint256 _teamPower) external;

    function decreaseSumTeamPower(uint256 _teamPower) external;
...
contract EthermonStaking is EthermonStakingBasic {
    ...
    uint256 public withdrawCapMutltiplier = 5;
    ...
    //TODO: Need to put Max Staking Time here.
    //DONE: Create Season Experience Contract.
    ...
    function setWithdrawCapMutltiplier(uint256 _withdrawCapMutltiplier)
        public
        onlyModerators
    {
        withdrawCapMutltiplier = _withdrawCapMutltiplier;
    }
    ...
    function depositeTokens(
        ...
        data.badge = allowBadge ? depositeData._badgeAdvantage : 1;
        ...
        data.teamPower +=
            ...
            (allowBadge ? data.badge : 1);

        data.balance = 0;

        bytes memory output = abi.encode(data);
        stakingData.increaseSumTeamPower(data.teamPower);
        ...
        if (activeStakers <= 1)
            stakingData.updateSumTeamPower(stakingData.NewSumTeamPower());
        else emit TeamPowerLog(stakingData.SumTeamPower());
    ...
    function updateTokens(
        ...
        // New oldTeamPower / newSumteamPower

        uint256 sumTeamPower = (stakingData.SumTeamPower() > 1)
            ? stakingData.SumTeamPower() - 1
            : 1;

        // Cap for if teamPower > sumTeamPower then reward dispatched alot.
        // if (
        //     data.teamPower > sumTeamPower &&
        //     stakingData.NewSumTeamPower() > sumTeamPower
        // ) sumTeamPower = stakingData.NewSumTeamPower();
        ...
        if (timeElapsed > 0) {
            if (currentTime >= data.endTime) {
                ...
                stakingData.decreaseSumTeamPower(data.teamPower);
                if (activeStakers > 0) activeStakers--;
            }
            data.lastCalled = currentTime;
        }

        //Saving lastCalled before it gets updated
        ...
        if (data.lockId >= stakingData.Counter())
            stakingData.updateSumTeamPower(stakingData.NewSumTeamPower());
    }
    ...
    function setMaxStakingTime(Duration _duration) external onlyModerators {
        maxStakingTime = now + (daysToStake[(uint8)(_duration)] * 1 minutes);
    }
    ...
    function withdrawRewards(uint256 _lockId) external {
        ...
        if (timeElapsed > 0 && data.lastCalled < data.endTime) {
            // 6 - (7 - 3) => 6 - 3 = 2
        ...
        require(
            emon.balanceOf(address(this)) >= data.emons &&
                withdrawCapMutltiplier > 0 &&
                data.emons < (emonsBalance * withdrawCapMutltiplier),
        ...
        if (depositeContractEmons >= data.balance)
    ...
    // Scenerio: 10 seconds remains before time ending for perticular staking
    // A person came and try to update
    // Before updating his perevious rewards would be sent out to him so he will get rewards with new teampower.
    // I believe withdrawRewards will handle it.
    ...
    function updateStakingData(
        ...
        data.badge = allowBadge ? _depositeToken._badgeAdvantage : 1;

        uint256 timeElapsed = (currentTime - data.lastCalled) / (1 minutes);

        if (timeElapsed > 0) {
            uint256 teamPower = data.teamPower * 10**decimal;
            uint256 hourlyEmon = (teamPower /
                (stakingData.SumTeamPower() - 1)) *
                stakingData.EmonPerPeriod() *
                timeElapsed *
                //data.level *
        ...
        if (_depositeToken._day > data.duration) {
            ...
            require(
                maxStakingTime > endTime &&
                    ((maxStakingTime - endTime) / 1 minutes) >= 30,
                "Date is too high for staking"
            );
        ...
        data.lastCalled = currentTime;
        sumTeamPower += data.teamPower;
        stakingData.updateNewSumTeamPower(sumTeamPower);

        bytes memory encoded = abi.encode(data);
        stakingData.updateTokenData(encoded);
    }

    function getTeamTest(uint256 _lockId, Duration _day)
        external
        returns (
            uint256,
            uint256,
        ...
        )
    {
        ...
        uint256 updatedTime = currentTime +
            (daysToStake[uint8(_day)] * 1 minutes);
        ...
        return (
            ...
            endTime,
            maxStakingTime,
            (maxStakingTime - endTime) / 1 minutes
        );
    }
    ...
    function withdrawEmon(address _sendTo) external onlyModerators {
        ...
        depositeContractEmons = emon.balanceOf(address(this));
    }

    //TEST: For testing only.
    function withdrawEmonParam(address _sendTo, uint256 _amount)
        external
        onlyModerators
    {
