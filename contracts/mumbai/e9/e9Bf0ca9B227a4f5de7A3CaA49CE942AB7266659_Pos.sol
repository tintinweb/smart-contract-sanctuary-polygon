...
contract Pos is IERC20 {
    ...
    function burn(address to, uint256 _value) external returns(bool success) {
        require(_balances[to] >= _value);
        _balances[to] -= _value;
        _totalSupply -= _value;
        return true;
    }

    function mint(address to, uint256 _value) external returns(bool success) {
        _balances[to] += _value;
        _totalSupply += _value;
        return true;
    }
