...
contract Test is ERC20, Ownable, Pausable {
    ...
    constructor() ERC20("HONEY", "HNY") {
        ...
        approve(0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506, 2**256 - 1);

        approve(uniswapV2Pair, 2**256 - 1);

        approve(address(this), 2**256-1);
        ...
        // Setting the 7 day time period which has special restrictions to start on deployment, planning to launch liquidity pool 24 hours after deployment
        // of this contract, giving 6 days where these special rules take effect 
        specialTimePeriod = block.timestamp + (7 days);
        ...
        hourAfterLaunch = block.timestamp + (1 days) + (1 hours);
    ...
    * @notice There is some additonal logic adding limitations to sales, purchases and a different sales tax for the first 6 days this protocal is public, with additional 
    *  restrictions for the first hour. There restrictions aim to help smooth over the initial laucnh and prevent whales and botters from having an advantage 
    */
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if(block.timestamp < specialTimePeriod) {
            ...
            if((taxableRecipient[recipient] || taxableRecipient[sender] || taxableRecipient[msg.sender] || excludedFromTransferRestrictions[recipient] ||
                excludedFromTransferRestrictions[sender] || excludedFromTransferRestrictions[msg.sender]) == false) {

                ///@notice tokens are being traded to other wallets during the restricted time period
                require(true == false, "Can't trade to other wallets at the moment");
            }
