...
contract ADM {
    ...
    struct RewardInfo{
        ...
        uint256 level3Released; //this contains level 2 and 3, to prevent stack to deep error
        uint256 level5Left;
    ...
    function _calCurDynamicRewards(address _user) private view returns(uint256) {
        ...
        uint256 totalRewards = userRewards.directs.add(userRewards.level2Released).add(userRewards.level3Released).add(userRewards.level4Released).add(userRewards.level5Released);
    ...
    function _calLevelNow(address _user) private view returns(uint256) {
        ...
        if(total >= 2000e6 && user.teamNum >= 200 && maxTeam >= 70000e6 && otherTeam >= 70000e6){
             levelNow = 5;
        }else if(total >= 2000e6 && user.teamNum >= 50 && maxTeam >= 20000e6 && otherTeam >= 20000e6){
             levelNow = 4;
        }else if(total >= 1500e6 && user.teamNum >= 20 && maxTeam >= 10000e6 && otherTeam >= 10000e6){
             levelNow = 3;
        }
    ...
    function getRewardInfos(address _address, uint256 _type) public view returns (uint256, uint256, uint256, uint256, uint256, uint256){
     ...
     if(_type == 1){
         ...
         _release = _release.add(rewardInfo[_address].level3Released);
    ...
    function _releaseUpRewards(address _user, uint256 _amount) private {
        ...
        for(uint256 i = 0; i < referDepth; i++){
            if(upline != address(0)){
                ...
                RewardInfo storage upRewards = rewardInfo[upline];
                if(i > 0 && i < 2 && userInfo[upline].level > 1){
                ...
                if(i > 0 && i < 3 && userInfo[upline].level > 2){
                    if(upRewards.level4Freezed > 0){
                        uint256 level3Reward = _amount.mul(level4Percents[i - 1]).div(baseDivider);
