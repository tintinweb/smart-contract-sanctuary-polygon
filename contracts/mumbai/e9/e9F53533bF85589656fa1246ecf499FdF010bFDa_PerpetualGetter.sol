...
contract PerpetualGetter is PerpetualUpdateFunctions, IFunctionList, IPerpetualGetter {
     ...
     */
    function getLiquidityPools(uint8 _poolFromIdx, uint8 _poolToIdx)
        external
        view
        override
        returns (LiquidityPoolData[] memory)
    {
        // pools count starts at 1
        require(_poolFromIdx > 0, "poolIdx>0");
        if (_poolToIdx > iPoolCount) {
            _poolToIdx = iPoolCount;
        }
        LiquidityPoolData[] memory result = new LiquidityPoolData[](_poolToIdx - _poolFromIdx + 1);
        uint8 idx = 0;
        for (uint8 i = _poolFromIdx; i <= _poolToIdx; i++) {
            result[idx] = liquidityPools[i];
            idx++;
        }
        return result;
    }
    ...
    /**
     * Query data for multiple perpetuals at once
     * @param perpetualIds array of perpetual IDs
     * @return array with perpetual data for given IDs
     */
    function getPerpetuals(uint24[] calldata perpetualIds)
        ...
        returns (PerpetualData[] memory)
    {
        PerpetualData[] memory perpData = new PerpetualData[](perpetualIds.length);
        for (uint256 k = 0; k < perpetualIds.length; k++) {
            perpData[k] = _getPerpetual(perpetualIds[k]);
        }
        return perpData;
    }

    /**
     * Query mid-prices for the perpetuals defined by its id
     * @param perpetualIds array of perpetual ids
     * @param idxPriceDataPairs array of index prices, in same order as perpetual ids
     *  index prices for the k-th perpetual are at positions [2*k, 2*k+1]
     * @return array of mid-prices in ABDK format
     */
    ...
    {
        int128[] memory prices = new int128[](perpetualIds.length);
        require(perpetualIds.length * 2 == idxPriceDataPairs.length, "input len mismatch");
        for (uint256 i = 0; i < perpetualIds.length; i++) {
            prices[i] = _getTradeLogic().queryPerpetualPrice(
                perpetualIds[i],
                0,
                [idxPriceDataPairs[i * 2], idxPriceDataPairs[i * 2 + 1]]
            );
        }
        return prices;
    }
    ...
    {
        return marginAccounts[_perpetualId][_traderAddress];
    }
    ...
    function isActiveAccount(uint24 _perpetualId, address _traderAddress)
        ...
        returns (bool)
    {
        return activeAccounts[_perpetualId].contains(_traderAddress);
    }
    ...
    function isTraderMaintenanceMarginSafe(uint24 _perpetualId, address _traderAddress)
        ...
        returns (bool)
    {
        return _getMarginViewLogic().isMaintenanceMarginSafe(_perpetualId, _traderAddress);
    }

    function getActivePerpAccounts(uint24 _perpetualId)
        external
        view
        override
        returns (address[] memory perpActiveAccounts)
    {
        perpActiveAccounts = activeAccounts[_perpetualId].enumerateAll();
    }

    function countActivePerpAccounts(uint24 _perpetualId)
        external
        view
        override
        returns (uint256 count)
    {
        count = activeAccounts[_perpetualId].length();
    }
    ...
    function getActivePerpAccountsByChunks(
        ...
        chunkPerpActiveAccounts = activeAccounts[_perpetualId].enumerate(_from, _to);
    }
     ...
     *  3: M2 or 0
     *  4: M3 or 0
     *  5: fCurrentTraderExposureEMA
     *  6: on-chain price of S2
     *  7: on-chain price of S3 or 0
     *  8: mark premium rate
     *  9: default fund cash/target rate
     *  10:perpetual Mid-Price
     *  11:open interest
     *  12:maxPositionBC
     *  13:perpetual state
     *  14:funding rate
     * @param _perpetualId perpetual id
     * @param _fIndexPrice spot prices of index S2 and S3 (S3 can be zero if not quanto)
     * @return array of 128-bit numbers
     */
    ...
    {
        int128[15] memory ammState;
        PerpetualData storage perpetual = _getPerpetual(_perpetualId);
        MarginAccount storage ma = marginAccounts[_perpetualId][address(this)];
        ...
        LiquidityPoolData storage lp = _getLiquidityPoolFromPerpetual(_perpetualId);
        if (lp.fTargetDFSize > 0) {
            ammState[9] = lp.fDefaultFundCashCC.div(lp.fTargetDFSize);
        }
        ammState[10] = _getTradeLogic().queryPerpetualPrice(_perpetualId, 0, _fIndexPrice);
        ammState[11] = perpetual.fOpenInterest;
        ammState[13] = int128(uint128(perpetual.state));
        ammState[14] = perpetual.fCurrentFundingRate;
        return ammState;
    }

    /**
     * Calculate base to collateral currency conversion
     * @param _fIndexPrice index price pair S2,S3
     * @param ccy perpetual-type
     * @return fx conversion
     */
    function _getBaseToCollFromIndex(
        int128[2] calldata _fIndexPrice,
        AMMPerpLogic.CollateralCurrency ccy
        ...
        if (ccy == AMMPerpLogic.CollateralCurrency.QUANTO) {
            ...
            return _fIndexPrice[0].div(_fIndexPrice[1]);
        } else {
            // Example: 0.5 contracts of ETHUSD paid in USD
            //  the rate is ETHUSD
            //  ETHUSD = 2000 => 0.5 * 2000 = 1000
            return _fIndexPrice[0];
        }
    }

    /**
     * Calculate collateral to quote currency conversion
     * @param _fIndexPrice index price pair S2,S3
     * @param ccy perpetual-type
     * @return fx conversion
     */
    function _getCollToQuoteFromIndex(
        int128[2] calldata _fIndexPrice,
        AMMPerpLogic.CollateralCurrency ccy
    ) internal pure returns (int128) {
        if (ccy == AMMPerpLogic.CollateralCurrency.BASE) {
     ...
     * @param _perpetualId perpetual id
     * @param _traderAddr  address of the trader for which we want to query the state
     * @param _fIndexPrice spot prices of index S2 and S3 (S3 can be zero if not quanto)
     */
    function getTraderState(
        ...
        address _traderAddr,
        int128[2] calldata _fIndexPrice
    ) external view override returns (int128[11] memory) {
        int128[11] memory traderState;
        PerpetualData storage perpetual = _getPerpetual(_perpetualId);

        traderState[2] = _getMarginViewLogic().getAvailableCash(_perpetualId, _traderAddr);

        MarginAccount storage ma = marginAccounts[_perpetualId][_traderAddr];
        ...
        // mark-price
        int128 fS3 = _getCollToQuoteFromIndex(_fIndexPrice, perpetual.eCollateralCurrency);
        traderState[8] = _fIndexPrice[0].mul(
            perpetual.currentMarkPremiumRate.fPrice.add(ONE_64x64)
        );
        int128 fB2CAtMark = traderState[8].div(fS3);

        // margin balance
        int128 b0;
        {
            // bal = cash + (pos * Sm - L) / S3
            //     = cash + pos * B2Cfx - L / C2Qfx
            b0 = traderState[2].add(ma.fPositionBC.mul(fB2CAtMark)).sub(
                ma.fLockedInValueQC.div(fS3)
            );
            traderState[0] = b0;
            // initial margin = pos * fx_atMark * marginrate
            int128 fInitialMarginRate = int128(perpetual.fInitialMarginRate) << 35;
            int128 fInitialMgn = ma.fPositionBC.mul(fB2CAtMark).mul(fInitialMarginRate).abs();
            // available margin = margin balance - initial margin
            traderState[1] = b0.sub(fInitialMgn);
        }

        int128 fLvg;
        if (b0 != 0) {
            // b0 shold be positive:
            // trader should be liquidated before b0 hits 0
            fLvg = ma.fPositionBC.abs().mul(fB2CAtMark).div(b0);
        }
        traderState[7] = fLvg;
        traderState[9] = fS3;
        traderState[10] = int128(perpetual.fMaintenanceMarginRate) << 35;
        return traderState;
    }
    ...
    /**
     * True if market is closed for currencies
        ...
        returns (bool)
    {
        ...
        (int128 fPrice, ) = factory.getSpotPrice(_baseCurrency, _quoteCurrency);
        return fPrice == 0;
    }
        ...
        returns (bool isClosed)
    {
        OracleFactory factory = OracleFactory(oracleFactoryAddress);
        PerpetualData storage perpetual = _getPerpetual(_perpetualId);
        (int128 fPrice, ) = factory.getSpotPrice(perpetual.S2BaseCCY, perpetual.S2QuoteCCY);
        isClosed = fPrice == 0;
        if (perpetual.S3BaseCCY != bytes4(0)) {
            (fPrice, ) = factory.getSpotPrice(perpetual.S3BaseCCY, perpetual.S3QuoteCCY);
    ...
    {
        return _getPriceInfo(_perpetualId);
    }

    /**
     * 1) Get an array of arrays with all perpetual ids, that is,
     * poolPerpetualIds[k][j] contains the perpetual Id of the j-th
     * perpetual in pool k+1 (pool numbering starts at 1).
     * 2) Array of pool share token addresses order with pool ids (from 1 to n)
     * 3) Array of pool margin token addresses order with pool ids (from 1 to n)
     * 4) address of Oracle Factory
     * @param _poolFromIdx start from (>=1)
     * @param _poolToIdx up to (can be larger than number of pools)
     * @return array of array poolPerpetualIds[k][j]
     * @return array of poolShareTokenAddr, zero if pool not running
     * @return array of poolMarginTokenAddr, zero if pool not running
     * @return address of Oracle Factory
     */
        ...
        )
    {
        ...
        uint8 numPools = 1 + _poolToIdx - _poolFromIdx;
        uint24[][] memory poolPerpetualIds = new uint24[][](numPools);
        address[] memory poolShareTokenAddr = new address[](numPools);
        address[] memory poolMarginTokenAddr = new address[](numPools);
        uint256 idx = 0;
        for (uint8 k = _poolFromIdx; k <= _poolToIdx; k++) {
            LiquidityPoolData storage pool = liquidityPools[k];
            //pool id => perpetual id list
            uint24[] memory perpetualIdsOfPool = perpetualIds[k];
            poolPerpetualIds[idx] = perpetualIdsOfPool;
            if (pool.isRunning) {
                poolShareTokenAddr[idx] = pool.shareTokenAddress;
                poolMarginTokenAddr[idx] = pool.marginTokenAddress;
            }
            idx = idx + 1;
        }
        return (poolPerpetualIds, poolShareTokenAddr, poolMarginTokenAddr, oracleFactoryAddress);
    }

    /**
     * Get an array of "PerpetualStaticInfo" for all the perpetual IDs provided
     * @param perpetualIds array of perpetual ids
     * @return PerpetualStaticInfo-array
     */
    ...
    {
        PerpetualStaticInfo[] memory info = new PerpetualStaticInfo[](perpetualIds.length);
        for (uint256 k = 0; k < perpetualIds.length; k++) {
            PerpetualData storage perpetual = _getPerpetual(perpetualIds[k]);
            address lo = LimitOrderBookFactory(orderBookFactory).orderBooks(perpetualIds[k]);
            (bytes32[] memory priceIds, bool[] memory isPyth) = _getPriceInfo(perpetualIds[k]);
            PerpetualStaticInfo memory perpInfo = PerpetualStaticInfo(
                perpetual.id,
                lo,
                perpetual.fInitialMarginRate,
                perpetual.fMaintenanceMarginRate,
                uint8(perpetual.state),
                perpetual.eCollateralCurrency,
                perpetual.S2BaseCCY,
                perpetual.S2QuoteCCY,
                perpetual.S3BaseCCY,
                perpetual.S3QuoteCCY,
                perpetual.fLotSizeBC,
    ...
    function getFunctionList() external pure virtual override returns (bytes4[] memory, bytes32) {
        ...
        bytes4[] memory functionList = new bytes4[](29);
        ...
        functionList[10] = this.getShareTokenFactory.selector;
        functionList[11] = this.getActivePerpAccounts.selector;
        functionList[12] = this.getActivePerpAccountsByChunks.selector;
        functionList[13] = this.isTraderMaintenanceMarginSafe.selector;
        functionList[14] = this.countActivePerpAccounts.selector;
        functionList[15] = this.getAMMState.selector;
        functionList[16] = this.getTraderState.selector;
        functionList[17] = this.getOracleFactory.selector;
        functionList[18] = this.getTreasuryAddress.selector;
        functionList[19] = this.getOrderBookFactoryAddress.selector;
        functionList[20] = this.getOrderBookAddress.selector;
        functionList[21] = this.isMarketClosed.selector;
        functionList[22] = this.isPerpMarketClosed.selector;
        functionList[23] = this.getPriceInfo.selector;
        functionList[24] = this.getPoolStaticInfo.selector;
        functionList[25] = this.getPerpetualStaticInfo.selector;
        functionList[26] = this.getLiquidityPools.selector;
        functionList[27] = this.getPerpetuals.selector;
        functionList[28] = this.queryMidPrices.selector;
...
contract LimitOrderBookFactory is Ownable {
    ...
    uint8 CANCEL_DELAY_SEC = 10; // number of seconds before order can be cancelled
    uint16 POSTING_FEE_TBPS = 5; // 0.5bps posting fee that can be rebated (not for cancel)
...
contract LimitOrderBook is IPerpetualOrder, IClientOrder, PerpetualHashFunctions {
    ...
    uint256 private constant MAX_ORDERS_PER_TRADER = 20;
    // Events
    ...
    int64 marketCloseSwitchTimestamp;
        ...
        returns (bytes32 digest)
    {
        ...
        perpOrder.submittedTimestamp = uint64(block.timestamp);
        ...
        returns (Order memory perpOrder)
    {
        ...
        perpOrder.referrerAddr = _order.referrerAddr;
        ...
        perpOrder.fLeverage = _order.fLeverage;
        perpOrder.iDeadline = _order.iDeadline;
        perpOrder.createdTimestamp = _order.createdTimestamp;
        ...
        returns (ClientOrder memory clientOrder)
    {
        ...
        clientOrder.referrerAddr = _order.referrerAddr;
        ...
        clientOrder.fLeverage = _order.fLeverage;
        clientOrder.iDeadline = _order.iDeadline;
        clientOrder.createdTimestamp = _order.createdTimestamp;
     ...
     *  at block 345
     */
    function handleMarketOpening() internal {
            ...
            try perpManager.tradeViaOrderBook(order) returns (bool orderSuccess) {
                ...
                /*
                    order should be removed in the following cases
                        "cannot be closing if already closed" <- REMOVE THIS IN CONTRACT AND TESTS
                        "trade amount too small for lot-size" <- no rebate
                        ...
                        "perpetual should be in NORMAL state" <- no rebate (rare)
