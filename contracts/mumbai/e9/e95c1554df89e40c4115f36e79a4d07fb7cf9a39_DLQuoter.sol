...
import {DLQuoter_InvalidLength} from "../DLErrors.sol";
import {BinHelper} from "../libraries/BinHelper.sol";
import {Constants} from "../libraries/Constants.sol";
import {Math512Bits} from "../libraries/Math512Bits.sol";
import {IDLFactory} from "../interfaces/IDLFactory.sol";
import {IDLRouter} from "../interfaces/IDLRouter.sol";

/// @title Discretized Liquidity Quoter
/// @author Bentoswap
...
contract DLQuoter {
        ...
        returns (Quote memory quote)
    {
        ...
        for (uint256 i; i < swapLength; i++) {
            IDLFactory.DLPairInformation[] memory DLPairsAvailable = IDLFactory(factory).getAllDLPairs(
                IERC20(_route[i]),
                IERC20(_route[i + 1])
            );
            if (DLPairsAvailable.length == 0 || quote.amounts[i] == 0) {
                continue;
            }
            for (uint256 j; j < DLPairsAvailable.length; j++) {
                if (DLPairsAvailable[j].ignoredForRouting) {
                    continue;
                }
                bool swapForY = address(DLPairsAvailable[j].dlPair.tokenY()) == _route[i + 1];

                try IDLRouter(router).getSwapOut(DLPairsAvailable[j].dlPair, quote.amounts[i], swapForY)
                    returns (uint256 swapAmountOut, uint256 fees)
                {
                    if (swapAmountOut > quote.amounts[i + 1]) {
                        quote.amounts[i + 1] = swapAmountOut;
                        quote.pairs[i] = address(DLPairsAvailable[j].dlPair);
                        quote.binSteps[i] = DLPairsAvailable[j].binStep;

                        // Getting current price
                        (, , uint256 activeId) = DLPairsAvailable[j].dlPair.getReservesAndId();
                        quote.virtualAmountsWithoutSlippage[i + 1] = _getQuote(
        ...
        returns (Quote memory quote)
    {
        ...
        for (uint256 i = swapLength; i > 0; i--) {
            ...
            for (uint256 j; j < DLPairsAvailable.length; j++) {
                ...
                bool swapForY = address(DLPairsAvailable[j].dlPair.tokenY()) == _route[i];
                try IDLRouter(router).getSwapIn(DLPairsAvailable[j].dlPair, quote.amounts[i], swapForY)
                    returns (uint256 swapAmountIn, uint256 fees)
                {
                    if (swapAmountIn != 0 && (swapAmountIn < quote.amounts[i - 1] || quote.amounts[i - 1] == 0)) {
                        ...
                        quote.pairs[i - 1] = address(DLPairsAvailable[j].dlPair);
                        quote.binSteps[i - 1] = DLPairsAvailable[j].binStep;

                        // Getting current price
                        (, , uint256 activeId) = DLPairsAvailable[j].dlPair.getReservesAndId();
                        quote.virtualAmountsWithoutSlippage[i - 1] = fees + _getQuote(
    ...
    /// @dev Calculates a quote for a pair
