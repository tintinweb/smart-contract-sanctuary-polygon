...
contract PlugInPlay_2022 is Ownable, RoyaltiesV2Impl, IERC1155MetadataURI, ERC1155  {

    //Interface for royalties
    bytes4 private constant _INTERFACE_ID_ERC2981 = 0x2a55205a;

    address public ROYALTIESADDRESS = 0x58645a99D8B49f748d3eC9bb8413Eb7Af21806dc;
    uint96 public ROYALTIESPOINTS = 100;
    ...
    constructor() ERC1155("RBM Non-Fungible Matic V1") {}
    ...
    function contractURI() public pure returns (string memory) {
        return "https://gateway.pinata.cloud/ipfs/QmZnEKQhnU1pV9WfCmhteKLPZCyhHQJHfSe61v4ETDbToE";
    }
    ...
    bytes memory data, string memory givenURL) public onlyOwner returns (uint256) {
        ...
        //royalties set at 50% for the test. Always go to the same address, that we set up above. Only need to set them once per token ID
        setRoyalties(id, payable(ROYALTIESADDRESS), ROYALTIESPOINTS);
    ...
    function mintToUser(uint256 id, uint256 amount, bytes memory data) 
        public payable callerIsUser returns (uint256) {

        uint256 _price = tokenPrice[id];
        require(msg.value == amount * _price, "Not enough/too much ether sent");
        require(getTotalSupplyByTokenId(id) + amount <= maxTokensPerId[id], "Not enough tokens left in the collection");
        require(balanceOf(msg.sender, id) + amount <= maxTokensPerAddress[id], "Cannot mint more tokens with the same address");
    ...
    function supportsInterface(bytes4 interfaceId) public view override(IERC165, ERC1155) 
    returns (bool){
