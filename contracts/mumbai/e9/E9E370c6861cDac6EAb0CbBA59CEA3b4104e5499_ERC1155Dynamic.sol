...
contract ERC1155Dynamic is BaseERC1155 {
    ...
    function uri(uint256 tokenId) public view override returns(string memory) {
        uint256 idx = getDynamicIdx(tokenId);
        return dynamicURIs[tokenId][idx];
    }

    function getDynamicIdx(uint256 tokenId) internal view returns(uint256) {
        return dynamicInfo[tokenId].idx;
    }

    function setDynamicInfo(uint256 tokenId) internal onlyOwner {
        dynamicInfo[tokenId].idx = dynamicInfo[tokenId].idx > 0 ? dynamicInfo[tokenId].idx + 1 : 0;
        dynamicInfo[tokenId].interval = dynamicInfo[tokenId].interval > 0 ? dynamicInfo[tokenId].interval * 2 : 120;
