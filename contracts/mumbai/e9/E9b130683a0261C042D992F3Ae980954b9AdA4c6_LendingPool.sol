...
contract LendingPool is Ownable, ReentrancyGuard {
    ...
    event PoolEndDateUpdated(uint256 poolId, uint256 newEndDate);
    ...
    function setPoolEndDate(
        ...
        require(
            newEndDate > pools[poolId].endDate,
            "Lending-pool: new end date must fall behind previous end date"
        );
        pools[poolId].endDate = newEndDate;
        emit PoolEndDateUpdated(poolId, newEndDate);
    }

    function setPoolDepositToken(
        uint256 poolId,
        IERC20 depositToken
    ) public poolExist(poolId) onlyOwner {
        require(
    ...
    function _getWithdrawableAmount(
        ...
        uint256 _payoutTimes = (block.timestamp - depositTime) /
            (isDailyPool ? ONE_DAY_IN_SECONDS : ONE_YEAR_IN_SECONDS);
        uint256 _denominator = isDailyPool ? 36500 : 100;
        if (depositToken == rewardToken) {
            if (
                (depositAmount * APY * _payoutTimes) / _denominator <
                claimedAmount
            ) {
                return 0;
            }
        ...
        require(
            payoutTimes == _payoutTimes,
            "Lending-pool: invalid number of payout times from price feed"
        );
        uint256 _rewardDecimals = IERC20Metadata(address(depositToken))
            .decimals();
        if (
            (depositAmount * APY * rateSum) /
                (10 ** _rewardDecimals * _denominator) <
            claimedAmount
        ) {
            return 0;
        }
        return
            ((depositAmount * APY * rateSum) /
                (10 ** _rewardDecimals * _denominator)) - claimedAmount;
    }
    ...
    function getPools(
        uint256 offset,
        uint256 limit
    ) public view returns (PoolInfo[] memory _pools) {
        if (offset > poolCount) {
    ...
    function getUserRecordsRaw(
        address userAddr,
        uint256 offset,
        uint256 limit
    ) public view returns (RecordInfo[] memory _records) {
        ...
        uint256 _iterLimit = offset + limit > _recordCount
            ? _recordCount
            : offset + limit;
        _records = new RecordInfo[](_length);
        for (uint256 i = offset; i < _iterLimit; i++) {
    ...
    function getUserRecords(
        ...
        view
        returns (
            RecordInfo[] memory _records,
            uint256[] memory _withdrawableAmount
        )
    {
        uint256 _recordCount = userRecords[userAddr].count;
        if (offset > _recordCount) {
        ...
        uint256 _iterLimit = offset + limit > _recordCount
            ...
            : offset + limit;
        _records = new RecordInfo[](_length);
        _withdrawableAmount = new uint256[](_length);
        for (uint256 i = offset; i < _iterLimit; i++) {
            RecordInfo memory _recordInfo = userRecords[userAddr].recordMap[i];
            _records[i - offset] = _recordInfo;
