...
/// @author Amit Molek
/// @notice Shared code for access control for drop contracts
/// @dev Uses OpenZeppelin roles to enforce access control
contract DropRole is AccessControl {
    /// Dropper role identifier
    bytes32 public constant DROPPER_ROLE = keccak256("DROPPER_ROLE");

    constructor(address owner) {
        // Grant the deployer the dropper role
        _setupRole(DROPPER_ROLE, owner);
    }
}
...
import "../ipfs/ERC1155IPFS.sol";
import "../access/DropRole.sol";
import "../access/MarketplaceAccess1155IPFS.sol";

/// @title Drop (Mint) content to recipients
/// @author Amit Molek
/// @notice This contract provides the ability to drop (mint) content and
/// send it to given recipients
contract RecipientsDrop is
    Ownable,
    ReentrancyGuard,
    DropRole,
    MarketplaceAccess1155IPFS
{
    ...
    error InvalidArgument(string name, string value);
    /// Only the token's original owner can access this
    error OnlyDropCreator();
    ...
    constructor(
        ...
        _grantRole(DROPPER_ROLE, antic);
        _grantRole(MARKETPLACE_ROLE, marketplaces);
    }

    modifier onlyDropCreator(uint256 tokenId) {
        if (_drops[tokenId].from != msg.sender) {
            revert OnlyDropCreator();
        }
        _;
    }
    ...
    function _mintDrop(
        ...
        emit Dropped(tokenId, msg.sender, recipients, amount);
    }

    /// @notice Create a drop, mint new tokens and send them to the given recipients
    /// @param recipients Addresses that will receive the token
    /// @param amount Amount to mint to each recipient
    /// @param uriHex Hex URI of the IPFS content, such as:
    /// 0x0155171c7a5638f61fc6cb5fe79746800653ba213355b9569ab13daf64efb25e
    function drop(
        address[] memory recipients,
        uint256 amount,
        uint256 uriHex
    ) external virtual nonReentrant onlyRole(DROPPER_ROLE) {
        if (recipients.length == 0) {
            revert InvalidArgument("recipients", "0");
        }
        if (amount == 0) {
            revert InvalidArgument("amount", "0");
        }

        // Iterate over the recipients and mint the token to them
        uint256 tokenId = _tokenIdCounter.current();
        _mintDrop(tokenId, recipients, amount);
        _tokenIdCounter.increment();

        // Store drop
        _drops[tokenId].from = msg.sender;

        // Store token's uri
        setIpfsUri(tokenId, uriHex);
    }

    /// @notice Drop an existing token again
    /// @param tokenId The token you want to redrop
    /// @param recipients Addresses that will receive the token
    /// @param amount Amount to mint to each recipient
    function redrop(
        uint256 tokenId,
        ...
        onlyDropCreator(tokenId)
    {
        // Token not minted
        if (tokenId >= _tokenIdCounter.current()) {
            revert InvalidArgument("tokenId", tokenId.toString());
        }
    ...
    /// @notice Fetch saved drop information
    /// @param tokenId Id of the drop
    /// @return Drop Returns drop information
    function fetch(uint256 tokenId)
        external
        view
        virtual
        returns (Drop memory)
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(MarketplaceAccess1155IPFS, AccessControl)
        returns (bool)
    {
        return super.supportsInterface(interfaceId);
    }
...
import "../interfaces/factory/IContractFactory.sol";
import "@openzeppelin/contracts/utils/introspection/ERC165.sol";

abstract contract BaseContractFactory is IContractFactory, ERC165 {
    function supportsInterface(bytes4 interfaceId)
        public
        view
        virtual
    ...
    {
        return
            interfaceId == type(IContractFactory).interfaceId ||
            super.supportsInterface(interfaceId);
    }
}
...
import "../drop/RecipientsDrop.sol";
import "./BaseContractFactory.sol";
import "@openzeppelin/contracts/access/Ownable.sol";

contract RecipientsDropFactory is BaseContractFactory, Ownable {
    address private _dropAntic;
    address[] private _dropMarketplaces;

    struct RecipientsDropArguments {
        address owner;
    }

    event RecipientsDropDeployed(address contractAddress);

    constructor(
        address factoryOwner,
        address dropAntic,
        address[] memory dropMarketplaces
    ) {
        transferOwnership(factoryOwner);

        _dropAntic = dropAntic;
        _dropMarketplaces = dropMarketplaces;
    }
    ...
    {
        // Decode params
        RecipientsDropArguments memory args = abi.decode(
            data,
            (RecipientsDropArguments)
        );

        RecipientsDrop recipientsDrop = new RecipientsDrop(
            args.owner,
            _dropAntic,
            _dropMarketplaces
        );

        emit RecipientsDropDeployed(address(recipientsDrop));

        return address(recipientsDrop);
    }

    function setAnticParam(address antic) external onlyOwner {
        _dropAntic = antic;
    }

    function setMarketplacesParam(address[] memory marketplaces)
        external
        onlyOwner
    {
        _dropMarketplaces = marketplaces;
    }

    function encode(RecipientsDropArguments memory args)
        external
        pure
