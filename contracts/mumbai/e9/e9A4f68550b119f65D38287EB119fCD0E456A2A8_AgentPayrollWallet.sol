...
contract AgentPayrollWallet is IAgentPayrollWallet,YousovRoles {
    ...
    function canUserClaim(address user) public view returns(bool) {
        if (block.timestamp >= lastClaimFreeTokens[user] + IManagerController(managerController).getValueInt("timeLimitFaucet") && YousovAccessControl(yousovAccessControl).hasRole(TEMPORARY_YOUSOV_USER_ROLE, user)) {
            return true;
        } 
        return false;
    }
    ...
    function claimFreeEZR() external override {
        require(canUserClaim(msg.sender), "YOUSOV : Not authorized to claim");
