...
contract YPredictToken is BEP20 {
    ...
    function setDevFee(uint256 _onBuy, uint256 _onSell) external onlyOwner {
    ...
    function setCtoFee(uint256 _onBuy, uint256 _onSell)
        external
        onlyOwner
    {
        require(_onBuy <= 10 && _onSell <= 10, "Invalid CTO Fee");
        ctoFeeOnBuy = _onBuy;
        ctoFeeOnSell = _onSell;
        totalFeesOnBuy = ctoFeeOnBuy
            .add(devFeeOnBuy)
            .add(marketingFeeOnBuy)
            .add(liquidityFeeOnBuy)
            .add(researchFeeOnBuy);
        totalFeesOnSell = ctoFeeOnSell
            .add(devFeeOnSell)
            .add(marketingFeeOnSell)
            .add(liquidityFeeOnSell)
            .add(researchFeeOnSell);
        ctoShare = ctoFeeOnBuy + ctoFeeOnSell;
        totalShares = totalFeesOnBuy + totalFeesOnSell;
        emit SetCtoFee(_onBuy, _onSell);
    }

    function setResearchFee(uint256 _onBuy, uint256 _onSell)
        external
        onlyOwner
    {
        require(_onBuy <= 10 && _onSell <= 10, "Invalid Research Fee");
        researchFeeOnBuy = _onBuy;
        researchFeeOnSell = _onSell;
        totalFeesOnBuy = researchFeeOnBuy
            .add(devFeeOnBuy)
            .add(marketingFeeOnBuy)
            .add(liquidityFeeOnBuy)
            .add(ctoFeeOnBuy);
        totalFeesOnSell = researchFeeOnSell
            .add(devFeeOnSell)
            .add(marketingFeeOnSell)
            .add(liquidityFeeOnSell)
            .add(ctoFeeOnSell);
        researchShare = researchFeeOnBuy + researchFeeOnSell;
        totalShares = totalFeesOnBuy + totalFeesOnSell;
        emit SetResearchFee(_onBuy, _onSell);
    }
    ...
    function updateCtoWallet(address _ctoWallet) internal onlyOwner {
        require(_ctoWallet != ctoWallet, "The CTO wallet is already this address");
        excludeFromFees(_ctoWallet, true);
        emit CtoWalletUpdated(_ctoWallet, ctoWallet);
        ctoWallet = payable(_ctoWallet);
    }

    function updateResearchWallet(address _researchWallet) internal onlyOwner {
        require(_researchWallet != researchWallet, "The research wallet is already this address");
    ...
    function swapTokens(uint256 tokenAmount) private {
        ...
        uint256 nativeForCto = swappedNative.mul(ctoShare).div(
            totalShares
        );
