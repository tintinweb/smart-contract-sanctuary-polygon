    ...
    IPerpetualLimitTradeManager
{
    ...
    function tradeViaOrderBook(Order calldata _order) external override {
        // ensure trade was sent via order-book
        require(
            LimitOrderBookFactory(orderBookFactory).orderBooks(_order.iPerpetualId) == msg.sender,
            "only order-bk"
        );
        require(block.number >= _order.submittedBlock + iBlockDelay, "delay required");
...
contract PerpetualTradeFunctions is PerpetualRebalanceFunctions, PerpetualHashFunctions {
    ...
    function _doOpeningTradeActions(
        ...
        marginAccounts[_perpetual.id][_order.traderAddr].iLastOpenBlock = uint64(block.number);
        if (_fTraderPos == 0) {
