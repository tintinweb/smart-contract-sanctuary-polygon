...
contract SexPartners is Ownable {
    ...
    function onERC721Received(
        ...
        if (tokenID == 0) {
            ...
            SEX.mint(_operator, advance * 99/100);
            SEX.mint(owner,advance * 1/100);
            u.claimed = advance;
    ...
    function claim() external returns (uint256) {
        ...
        if (mintable > t.minted) {
            ...
            SEX.mint(address(this), amount * 99/100);
            SEX.mint(owner,amount * 1/100);
            t.minted = mintable;
            totalMinted += amount;
        }
    ...
    function earlyPartnerClaim() external returns (uint256) {
        ...
        t.minted += amount;
        totalMinted += amount;
        SEX.mint(msg.sender, amount * 99/100);
        SEX.mint(owner,amount * 1/100);
