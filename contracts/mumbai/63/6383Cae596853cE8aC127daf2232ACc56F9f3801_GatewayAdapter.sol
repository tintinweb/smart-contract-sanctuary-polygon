...
contract GatewayAdapter is Context {
    ...
    function burnWithPayload(
        ...
        IMintGateway gateway = gatewayRegistry.getMintGatewayBySymbol(asset);
        IERC20 token = gatewayRegistry.getRenAssetBySymbol(asset);
        uint256 amount = IERC20(token).allowance(_msgSender(), address(this));
        token.transferFrom(_msgSender(), address(this), amount);
        gateway.burnWithPayload(recipientAddress, recipientChain, recipientPayload, amount);
        return amount;
    }

    function lockWithPayload(
        string calldata asset,
        string calldata recipientAddress,
        string calldata recipientChain,
        bytes calldata recipientPayload
    ) public payable returns (uint256) {
        ILockGateway gateway = gatewayRegistry.getLockGatewayBySymbol(asset);
        IERC20 token = gatewayRegistry.getLockAssetBySymbol(asset);
        uint256 amount = IERC20(token).allowance(_msgSender(), address(this));
        token.transferFrom(_msgSender(), address(this), amount);
