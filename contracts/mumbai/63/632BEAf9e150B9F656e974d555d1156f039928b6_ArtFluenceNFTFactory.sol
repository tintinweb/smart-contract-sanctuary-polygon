...
contract ArtFluenceNFT is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable {
    ...
    function buyPunk(address to,uint numberofNTS,string[] memory uri) public payable reentrancyGuard{
        require(startTime < block.timestamp && startTime > 0,"ArtFluenceNFT:Time is not Started");
        require(stopTime > block.timestamp && stopTime > 0,"ArtFluenceNFT:Time is End");
        require(to != address(0),"ArtFluenceNFT:Invalid Address");
        require(numberofNTS <= 10 , "ArtFluenceNFT:Min 10 Tokens");
        require(msg.value == price*numberofNTS,"ArtFluenceNFT:Invalid Amount");
        require(uri.length == numberofNTS,"ArtFluenceNFT:Invalid Length");
    ...
    function retrive(address to,uint amount)public onlyOwner{
        require(to != address(0),"ArtFluenceNFT:Invalid Address");
        require(address(this).balance >=amount, "ArtFluenceNFT:Invalid Amount");
...
contract ArtFluenceNFTFactory {
    ...
    event create(uint indexed _id, address indexed _owner , address indexed _nftAddress);
    ...
    function createCollection(string memory _name ,string memory _symbol,string memory _uri,uint expiry,address payable _royaltyAddress,uint _royalFee,Sig memory sig)public{
        ...
        _ArtFluenceNFT = new ArtFluenceNFT(_name,_symbol,_uri,msg.sender,_royaltyAddress,_royalFee);
        contractList[msg.sender].push(address(_ArtFluenceNFT));
        emit create(contractList[msg.sender].length - 1,msg.sender,address(_ArtFluenceNFT));
    }
