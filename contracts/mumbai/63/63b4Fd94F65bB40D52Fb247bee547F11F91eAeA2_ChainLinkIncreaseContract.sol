...
contract ChainLinkIncreaseContract is
  ...
  IncreaseCounter
{
  address public REGISTRY_ADDRESS = 0x6179B349067af80D0c171f43E6d767E4A00775Cd; //mumbai testnet
  ...
  bytes4 private constant CANCEL_FUNC_SELECTOR =
    bytes4(keccak256("cancel(bytes32)"));

  uint256 public minFundingAmount = 500000000000000000; //0.5 LINK
  ...
  function createUpkeep() public {
    ...
    bytes32 hash = keccak256(
      abi.encode(address(this), 100000, msg.sender, hex"")
    );
    taskIdByUser[msg.sender] = hash;
    ERC677Link.transferAndCall(registarAddress, minFundingAmount, data);
  }

  function cancelUpkeep() external {
    require(taskIdByUser[msg.sender] != bytes32(0), "Upkeep Notregistered");
    address registarAddress = KeepersRegistry(REGISTRY_ADDRESS).getRegistrar();
    bytes32 hash = taskIdByUser[msg.sender];
    bytes memory data = abi.encodeWithSelector(CANCEL_FUNC_SELECTOR, hash);
    taskIdByUser[msg.sender] = bytes32(0);
