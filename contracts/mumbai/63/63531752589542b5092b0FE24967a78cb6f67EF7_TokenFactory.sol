...
interface ICompliance {
    ...
    function revokeSubOwnerOnCompliance(address _SubOwner) external;
...
import 'contracts/core/IdentityProxy.sol';
import 'contracts/core/TokenProxy.sol';
...
contract TokenFactory is AccessControl, Initializable{
    ...
    // bytes32 public constant Tokenization_Role = keccak256("Tokenization_Role");
    ...
    modifier onlyAdmin() {
        require(msg.sender == superAdmin,"Only owner can call");
        _;
    }

    function addSubOwner(address _userAddress) external onlyAdmin {
        require(!isSubOwner(_userAddress), "Already has SubAdmin");
        _setupRole(Owner_Role, _userAddress);
    }

    function revokeSubOwner(address _SubOwner) external onlyAdmin {
        require(isSubOwner(_SubOwner), "Is not SubAdmin");
    ...
    function setImplementation (
        address _identityRegistry, address _claimTopicsRegistry,
         ...
         address _implAuthorithyIdentity) public onlyRole(Owner_Role){
    ...
    function deployIdentity(address _initialManagementKey, bool _isLibrary) internal returns (address) {
    ...
    function createToken(
        ...
        bytes32 _salt, uint256 _holdReleaseTimestamp, uint256 _tokenLimitPerUser, uint16[] calldata _countries, 
        address _issuer, address _transferAgent) external onlyRole(Owner_Role){
        ...
        _initCompliance(identityRegistry, _name, _symbol, _decimals, _cap, mappingValue, _holdReleaseTimestamp, _tokenLimitPerUser,_countries, _issuer, _transferAgent);
        IIdentityRegistry(identityRegistry).addAgentOnIdentityRegistryContract(_issuer);
    }
    function _initCompliance(address identityRegistry,string memory _name,string memory _symbol,uint8 _decimals,uint _cap, string memory mappingValue, 
                uint256 _holdReleaseTimestamp, uint256 _tokenLimitPerUser, uint16[] calldata _countries, address _issuer, address _transferAgent) internal{
    ...
    function _initProxy(address identityRegistry,string memory _name,string memory _symbol,uint8 _decimals,uint _cap, string memory mappingValue, 
        ...
        success = false;
        (success,) = proxy.call(abi.encodeWithSelector(0xf2fde38b, address(this)));
        require(success, "token ownership Failed");
        success = false;
        (success,) = proxy.call(abi.encodeWithSelector(0x70480275, superAdmin));
        require(success, "Add Admin Role Failed");
        success = false;
        (success,) = proxy.call(abi.encodeWithSelector(0x70480275, address(this)));
        require(success, "Add Admin Role Failed");
        success = false;
        ...
        emit tokenCreated(proxy, implAuthorithyToken, identityRegistry, identity, mappingValue, block.timestamp);
    }
    ...
    function _initProxyCompliance(address proxy, address compliance, uint256 _holdReleaseTimestamp, uint256 _tokenLimitPerUser) internal {
        ...
        require(success, "Compliance Intiatialization Failed");
        success = false;
        (success,) = compliance.call(abi.encodeWithSelector(0x32119257, superAdmin));
    ...
    function _initIdentityRegistry(address identityRegistry) internal{
        ...
        require(success, "identity Intiatialization Failed");
        success = false;
        (success,) = identityRegistry.call(abi.encodeWithSelector(0xf2fde38b, address(this)));
        require(success, "identityRegistry ownership Failed");
        success = false;
        (success,) = identityRegistryStorage.call(abi.encodeWithSelector(0x19ab453c, address(this)));
        ...
        success = false;
        (success,) = identityRegistryStorage.call(abi.encodeWithSelector(0xf2fde38b, address(this)));
        require(success, "identityRegistryStorage ownership Failed");
    }

    function addAgents(address _transferAgent, address _tokenizationAgent, address _token) external onlyRole(Owner_Role){
    ...
    function setRolesOnToken(address _userAddress, bytes32[] memory roles, address _token) external onlyRole(Owner_Role){
        IToken token = IToken(_token);
        token.setRolesToTokenAgent(_userAddress, roles);
    }

    function revokeRolesOnToken(address _userAddress, bytes32[] memory roles, address _token) external onlyRole(Owner_Role){
        IToken token = IToken(_token);
        token.revokeRolesOnTokenAgent(_userAddress, roles);
    }

    function addSubAdminOnToken(address _userAddress, address _token) external onlyRole(Owner_Role){
        IToken token = IToken(_token);
        token.addAdmin(_userAddress);
    }

    function revokeSubAdminOnToken(address _userAddress, address _token) external onlyRole(Owner_Role){
        IToken token = IToken(_token);
        token.revokeAdmin(_userAddress);
    }

    function addSubAdminOnIdentityRegistry(address _userAddress, address _identityRegistry) external onlyRole(Owner_Role){
        IIdentityRegistry iR = IIdentityRegistry(_identityRegistry);
        iR.addSubOwnerOnIR(_userAddress);
    }

    function revokeSubAdminOnIdentityRegistry(address _SubAdminAddress, address _identityRegistry) external onlyRole(Owner_Role){
        IIdentityRegistry iR = IIdentityRegistry(_identityRegistry);
        iR.revokeSubAdminOnIR(_SubAdminAddress);
    }

    function addSubOwnerOnCompliance(address _userAddress, address _compliance) external onlyRole(Owner_Role){
        ICompliance compliance = ICompliance(_compliance);
        compliance.addSubOwnerOnCompliance(_userAddress);
    }

    function revokeSubOwnerOnCompliance(address _SubOwner, address _compliance) external onlyRole(Owner_Role){
        ICompliance compliance = ICompliance(_compliance);
        compliance.revokeSubOwnerOnCompliance(_SubOwner);
    }

    function addTokenizationAgentOnToken(address _agent, address _token) public onlyRole(Owner_Role){
        IToken token = IToken(_token);
    ...
    function revokeTokenizationAgentOnToken(address _agent, address _token) external onlyRole(Owner_Role){
        IToken token = IToken(_token);
    ...
    function addTransferAgentOnToken(address _agent, address _token) public onlyRole(Owner_Role){
        IToken token = IToken(_token);
        token.addTokenizationAgentOnToken(_agent);
    }

    function removeTransferAgentOnToken(address _agent, address _token) external onlyRole(Owner_Role){
        IToken token = IToken(_token);
        token.removeTransferAgentOnToken(_agent);
    }

    function updateReleaseTimeStampForToken(address _token, uint256 _timestamp) external onlyRole(Owner_Role){
        IToken token = IToken(_token);
        ICompliance compliance = token.compliance();
        compliance.updateReleaseTimestamp(_timestamp);
    }

    function updateHolderLimitForToken(address _token, uint256 _newHolderLimit) external onlyRole(Owner_Role){
        IToken token = IToken(_token);
        ICompliance compliance = token.compliance();
