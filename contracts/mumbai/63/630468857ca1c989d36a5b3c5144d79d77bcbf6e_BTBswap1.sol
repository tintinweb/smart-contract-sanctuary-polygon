...
contract BTBswap1 is ReentrancyGuard, Ownable {
 ...
 function buy(uint256 tokenAmount, address spendingToken) external whenNotPaused nonReentrant {
    uint256 totalTokensInContract = tokenBalances[spendingToken];
    uint256 price = getPrice(totalTokensInContract, totalTokensSupply());
    uint256 tokensToBuy = (tokenAmount * 1e18) / price;

    uint256 fee = excludedFromFee[msg.sender] ? 0 : (tokensToBuy * feePercentage) / 100;
    uint256 feeForOwner = (fee * 20) / 100; // 20% of the fee

    tokensToBuy -= fee; // deduct the fee from tokens to buy
    ...
    tokenBalances[spendingToken] += tokenAmount - feeForOwner;
