    ...
    ReentrancyGuard
{
    ...
    uint16 public max_eggs_available=1779;
    ...
    uint16 private time_shift_seconds = 3600 * 3; //3 hours earlier
    ...
    function getFusionCounter(address to) public view returns(uint16){
        uint currentDayPart = (block.timestamp+time_shift_seconds) / 86400;
    ...
    function setEvolutionCounters(uint16 evo, uint8 types,uint16 counter) external onlyOwner {
        if(types==0){
            indexEvolutionBlue[evo]=counter;
        }else{
            indexEvolutionPink[evo]=counter;
        }
    }
    ...
    function setTimeShiftInSeconds(uint16 secs) external onlyOwner {
        time_shift_seconds=secs;
    }
    ...
    function getBuyCounter(address to) public view returns(uint16){
        uint currentDayPart = (block.timestamp+time_shift_seconds)  / 86400;
        bytes memory key = abi.encodePacked(to, currentDayPart);
        return max_eggs_by_day - throttling_buy[key];
    }
    ...
    function setMaxEggs(uint16 maxEggs) external onlyOwner(){
        max_eggs_available=maxEggs;
    }
    ...
    payable
    {
        if(max_eggs_available<quantity) revert("not enought eggs to sale");
        ...
        throttling_buy[key]+=quantity;
        unchecked {
            index += quantity;
            max_eggs_available-=quantity;
        }
    }
    ...
    function receiveForGame() external payable onlyOwner{
        uint256 value = msg.value;
        gameFunds += value;
    }
    function receiveForClaim() external payable onlyOwner{
        uint256 value = msg.value;
        claimFunds += value;
    } 
    function receiveForFusions() external payable onlyOwner{
        uint256 value = msg.value;
        evolutionFunds += value;
    } 
    ...
    function withdrawClaimFundsForDispatch(address to, uint256 amount) external onlyOwner {
        (bool success, ) = payable(to).call{value: amount}("");
        if (!success) revert FailToWithdraw();
        claimFunds=claimFunds-amount;
        emit WithdrawClaimFunds(claimFunds);
    }
    ...
    function airdrop(address to, uint16 quantityToMint) external onlyOwner {
        if (Address.isContract(to)) revert SenderIsContract();
    ...
    function _afterTokenTransfers(
        ...
        uint256 timestampToSet=0;
        if (from!=0x0000000000000000000000000000000000000000) {
            timestampToSet=block.timestamp;
        } else {
            timestampToSet=0;
        }
        for(uint256 i=startTokenId;i<startTokenId+quantity;i++){
            transferts[i] = timestampToSet;
        }
    }
