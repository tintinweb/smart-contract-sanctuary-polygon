...
contract BrickStaking{
    ...
    uint256 constant UPGRADE_REWARD_PERIOD = 1 minutes;
    ...
    function getNewPixelsAmount(uint256 stakeValue, address staker) public view returns (uint256 amount){
        stakeValue -= bricksToFinishPixel[staker];

        return (stakeValue + PIXEL_VALUE / 2) / PIXEL_VALUE;
    }
    ...
    function setBrick(address brickToken_, address brickNft_, address brickManager_) external onlyAdmin{
    ...
    modifier onlyRegistered{
        bytes32 REGISTERED_ROLE = brickManager.REGISTERED_ROLE();
        require(brickManager.hasRole(REGISTERED_ROLE, msg.sender), "This account is not registered");
        _;
    }

    modifier onlyAdmin{
        bytes32 DEFAULT_ADMIN_ROLE = brickManager.DEFAULT_ADMIN_ROLE();
