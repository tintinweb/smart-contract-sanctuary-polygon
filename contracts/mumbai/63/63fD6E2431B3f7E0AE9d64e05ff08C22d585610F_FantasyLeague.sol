...
abstract contract Playoff is H2HCompetition {
    ...
    // //         divisionWinners[i - _from] = refDivisionsWinners[i];
    // //     }
    ...
    // //         divisionWinnersStats[i - _from] = DivisionWinnerStats(
...
abstract contract H2HCompetition is UsersNDivisions {
     ...
     * User => season ID => week => user week stats.
     */
     ...
     */
    function calculateRewardPerPoint()
        external
        onlyGameStage(GameStage.H2HRewardPerPointCalculation)
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
     ...
     */
    function updateRewardsForUsers(uint256 _numberOfUsers)
        external
        onlyGameStage(GameStage.H2HRewardsUpdate)
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    // ____ Temporary setters for fixing ____

    event UserSeasonRankingSet(address _user);

    function setUserSeasonRanking(
        address _user,
        uint32[3] calldata _ranking
    )
        public
        addedUser(seasonId.current(), _user)
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        UserSeasonStats storage refUserSeasonStats = userSeasonStats[_user][seasonId.current()];
        refUserSeasonStats.wins = _ranking[0];
        refUserSeasonStats.losses = _ranking[1];
        refUserSeasonStats.ties = _ranking[2];

        emit UserSeasonRankingSet(_user);
    }

    // prettier-ignore
    function setUserSeasonRankingBatch(
        address[] calldata _users,
        uint32[3][] calldata _rankings
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_users.length == _rankings.length, "Length mismatch");

        for (uint256 i = 0; i < _users.length; ++i)
            setUserSeasonRanking(_users[i], _rankings[i]);
    }

    event UserWeekResultSet(uint8 indexed _week, address indexed _user, uint256 _undoubledPoints,bool indexed _isWinner);

    function setUserWeekResult(
        uint8 _week,
        address _user,
        uint256 _undoubledPoints,
        bool _isWinner
    )
        public
        addedUser(seasonId.current(), _user)
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(_week != 0, "Zero week number");

        UserWeekStats storage refUserWeeklyStats = userWeeklyStats[_user][seasonId.current()][_week];
        refUserWeeklyStats.isWinner = _isWinner;
        refUserWeeklyStats.points = _isWinner ? _undoubledPoints * 2 : _undoubledPoints;

        emit UserWeekResultSet(_week, _user, _undoubledPoints, _isWinner);
    }

    function emitH2HCompetitionResultBatch(
        address[] calldata _firstUsers,
        address[] calldata _secondUsers,
        CompetitionResult[] calldata _results,
        uint256 _week
        ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_firstUsers.length == _secondUsers.length && _firstUsers.length == _results.length, "Length mismatch");
        for (uint256 i = 0; i < _firstUsers.length; ++i)
            emit H2HCompetitionResult(_firstUsers[i], _secondUsers[i], _results[i], _week);
    }

    // prettier-ignore
    function setUserWeekResultBatch(
        uint8 _week,
        address[] calldata _users,
        uint256[] calldata _undoubledPoints,
        bool[] calldata _isWinners
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_week != 0, "Zero week number");
        require(
            _users.length == _undoubledPoints.length &&
                _users.length == _isWinners.length,
            "Length mismatch"
        );

        for (uint256 i = 0; i < _users.length; ++i)
            setUserWeekResult(_week, _users[i], _undoubledPoints[i], _isWinners[i]);
    }

    event UserSeasonTotalPointsUpdated(address _user, uint256 _totalPoints);

    // prettier-ignore
    function updateUserSeasonTotalPoints(address _user)
        public
        addedUser(seasonId.current(), _user)
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        uint256 season = seasonId.current();
        mapping(uint256 => UserWeekStats) storage refUserWeeklyStats = userWeeklyStats[_user][season];
        for (uint256 i = 0; i <= week; ++i)
            totalPoints += refUserWeeklyStats[i].points;

        userSeasonStats[_user][season].totalPoints = totalPoints;

        emit UserSeasonTotalPointsUpdated(_user, totalPoints);
    }

    // prettier-ignore
    function updateUserSeasonTotalPointsBatch(address[] calldata _users) external onlyRole(DEFAULT_ADMIN_ROLE) {
        for (uint256 i = 0; i < _users.length; ++i)
            updateUserSeasonTotalPoints(_users[i]);
    }
