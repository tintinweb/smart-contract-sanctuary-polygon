...
pragma solidity ^0.8.0;
 ...
 contract GamePlayToken is ERC20{
     ...
     mapping(address=>uint) public remainingTokenBalance;
     ...
     mapping(address=>bool) private listCheckForPublic;
     mapping(address=>bool) private listCheckForStrategic;
     ...
     mapping(address=>uint) public publicWalletUnlockTime;
     mapping(address=>uint) public strategicWalletUnlockTime;

     mapping(address=>uint) public unlockedStrategicBalance;
     mapping(address=>uint) public remainingStrategicBalance;
     bool private publicSaleControl = true;
     bool private strategicSaleControl = true;
     ...
     function BuyStrategicSale() public payable{
         ...
         require(strategicSaleControl==true,"Strategic sale has ended");
         ...
         remainingStrategicBalance[msg.sender] += tokenAmount;
         remainingTokenBalance[msg.sender] += tokenAmount;
          if(!listCheckForStrategic[msg.sender]){
            strategicAdresses.push(msg.sender);
            tokenBalance[msg.sender] += tokenAmount;
            listCheckForStrategic[msg.sender] = true;
          }
          else{
            tokenBalance[msg.sender] += tokenAmount;
          }
     ...
     function BuyPublicSale() public payable{
         require(msg.value<=msg.sender.balance,"You do not have enough funds");
         require(publicSaleControl==true,"Public sale has ended");
         ...
         remainingPublicBalance[msg.sender] += tokenAmount;
         remainingTokenBalance[msg.sender] += tokenAmount;
         if(!listCheckForPublic[msg.sender]){
              publicAddresses.push(msg.sender);
              publicSaleRemainingAmount[msg.sender] = (publicSaleBalance[msg.sender]-(publicSaleBalance[msg.sender]*5)/100)/12;
              tokenBalance[msg.sender] += tokenAmount;
              listCheckForPublic[msg.sender]= true;
         }
         else{
             publicSaleRemainingAmount[msg.sender] = (publicSaleBalance[msg.sender]-(publicSaleBalance[msg.sender]*5)/100)/12;
             tokenBalance[msg.sender] += tokenAmount;
     ...
     function UnlockBeforeDex() public OnlyOwner{
         publicWalletUnlockTime[ownerAddress] = block.timestamp + 30 days;
         strategicWalletUnlockTime[ownerAddress] = block.timestamp + 90 days;
         for(uint i=0; i<publicAddresses.length; i++){
             uint unlockedAmount = (publicSaleBalance[publicAddresses[i]]*5)/100;
             unlockedBalance[publicAddresses[i]] += unlockedAmount;
             remainingPublicBalance[publicAddresses[i]] -= unlockedAmount;
             unlockedPublicBalance[publicAddresses[i]] += unlockedAmount;
             remainingTokenBalance[publicAddresses[i]] -= unlockedAmount;
         }
         for(uint i=0; i<strategicAdresses.length; i++){
             ...
             remainingTokenBalance[strategicAdresses[i]] -= unlockedAmount;
         }
     }
     function WithdrawTokens() public{
         ...
         if(unlockedPublicBalance[msg.sender]!=0){
             remainingPublicBalance[msg.sender] -= unlockedPublicBalance[msg.sender];
             unlockedPublicBalance[msg.sender] = 0;
         }
         if(unlockedStrategicBalance[msg.sender]!=0){
             remainingStrategicBalance[msg.sender] -= unlockedStrategicBalance[msg.sender];
             unlockedStrategicBalance[msg.sender] = 0;
         }
         unlockedBalance[msg.sender] = 0;
         
     }
     function WithdrawStrategicTokens(uint tokenAmount) public{
         if(tokenAmount>unlockedStrategicBalance[msg.sender]){
             revert("You don't have enough funds");
         }
         else{
            unlockedStrategicBalance[msg.sender] -= tokenAmount;
            this.transfer(msg.sender,tokenAmount);
            remainingTokenBalance[msg.sender] = remainingPublicBalance[msg.sender]+remainingStrategicBalance[msg.sender];
            unlockedBalance[msg.sender] -= tokenAmount;

         }

     }
     function WithdrawPublicTokens(uint tokenAmount) public{
         if(tokenAmount>unlockedPublicBalance[msg.sender]){
             revert("You don't have enough funds");
         }
         else{
            unlockedPublicBalance[msg.sender] -= tokenAmount;
            this.transfer(msg.sender,tokenAmount);
            remainingTokenBalance[msg.sender] = remainingPublicBalance[msg.sender]+remainingStrategicBalance[msg.sender];
            unlockedBalance[msg.sender] -= tokenAmount;
     ...
     function UnlockStrategicTokens() public OnlyOwner{
         require(block.timestamp>strategicUnlockTimeLock[ownerAddress],"The time has not passed yet");
         strategicUnlockTimeLock[ownerAddress] = block.timestamp + 90 days;
         for(uint i=0; i<strategicAdresses.length; i++){
             uint unlockedAmount = (remainingStrategicBalance[strategicAdresses[i]]*4)/100;
             unlockedBalance[strategicAdresses[i]] += unlockedAmount;
             remainingStrategicBalance[strategicAdresses[i]] -= unlockedAmount;
     ...
     function UnlockPublicTokens() public OnlyOwner{
         require(block.timestamp>publicWalletUnlockTime[ownerAddress],"The time has not passed yet");
         publicWalletUnlockTime[ownerAddress] = block.timestamp+30 days + 3 minutes;
         for(uint i=0; i<publicAddresses.length; i++){
            ...
            remainingTokenBalance[publicAddresses[i]] -= publicSaleRemainingAmount[publicAddresses[i]];
     ...
     function SetPublicFunction() public OnlyOwner{
         require(publicSaleControl==true);
         publicSaleControl = false;
     }
     function SetStrategicSale() public OnlyOwner{
         require(strategicSaleControl==true);
         strategicSaleControl = false;
     }
