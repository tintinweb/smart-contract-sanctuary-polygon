...
interface IGroup {
    ...
    /// @dev Join `member` to the group
    /// Emits `Joined` event
    function join(address member) external payable;
...
contract DominiumProxyFactory is Manageable {
    ...
    function clone(bytes memory data) external returns (DominiumProxy) {
        return _replicate(data);
    }

    /// @dev The caller also joins the group
    /// @return instance an initialized `DominiumProxy` with the caller as the first member
    function cloneAndJoin(bytes memory data)
        external
        payable
        returns (DominiumProxy instance)
    {
        instance = _replicate(data);

        // Member joins the group
        IGroup(address(instance)).join{value: msg.value}(msg.sender);
    }

    /// @dev Emits `Cloned`
    function _replicate(bytes memory data)
        internal
        returns (DominiumProxy instance)
    {
