...
contract Marimo is Ownable, ERC721A {
  ...
  function getLastSize(uint256 tokenId) internal view returns (uint256) {
    require(_exists(tokenId), "nonexistent token");
    return _lastSize[tokenId] > 0 ? _lastSize[tokenId] : 250;
  }

  function getCurrentSize(uint256 tokenId) public view returns (uint256) {
    require(_exists(tokenId), "nonexistent token");
    uint256 elapsedMinutes = getElapsedTimeFromLastWaterChanged(tokenId) / 60;
    uint256 unitTime = 900;
    if (elapsedMinutes < 24 * 60 * 6) {
      return (100 * elapsedMinutes) / unitTime / 100  + getLastSize(tokenId);
    } else if (elapsedMinutes <= 24 * 60 * 15) {
      return (95 * elapsedMinutes + 432 * 100) / unitTime / 100 + getLastSize(tokenId);
    } else if (elapsedMinutes <= 24 * 60 * 24) {
      return (50 * elapsedMinutes + 10152 * 100) / unitTime / 100 + getLastSize(tokenId);
    } else if (elapsedMinutes <= 24 * 60 * 30) {
      return (10 * elapsedMinutes + 23976 * 100) / unitTime / 100 + getLastSize(tokenId);
    } else {
      return getLastSize(tokenId) + (28296 / unitTime);
    }
  }
