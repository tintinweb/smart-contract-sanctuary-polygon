...
contract EggHatcher is RandomConsumerBase, Pausable {

    using Address for address;

    uint constant THRESHOLD_DENOMINATOR = 1e8;
    
    string public constant BAD_CALLER_ERROR = "EggHatcher: caller cannot be a contract";
    string public constant HATCH_CALLER_ERROR = "EggHatcher: hatch caller is not owner";
    string public constant HATCH_ERROR = "EggHatcher: cannot be hatched";
    string public constant HATCH_IN_PROGRESS_ERROR = "EggHatcher: hatching in progress";

    address private _eggTokenAddress;

    mapping(bytes32 => uint) private _requestData;
    mapping(uint => uint) private _pTokens;
    ...
    function eggTokenAddress() public view returns (address) {
        return _eggTokenAddress;
    }

    function setEggTokenAddress(address newAddress) external onlyRole(COO_ROLE) {
        address previousAddress = _eggTokenAddress;
        _eggTokenAddress = newAddress;
        emit AddressChanged("eggToken", previousAddress, newAddress);
    }
    ...
    function hatch(uint tokenId) external whenNotPaused {
        require(!_msgSender().isContract(), BAD_CALLER_ERROR);
        require(_pTokens[tokenId] == 0, HATCH_IN_PROGRESS_ERROR);
        EggToken et = EggToken(eggTokenAddress());
        require(et.canHatch(tokenId), HATCH_ERROR);
        require(et.ownerOf(tokenId) == _msgSender(), HATCH_CALLER_ERROR);

        bytes32 requestId = requestRandomness();
        _requestData[requestId] = tokenId;
        _pTokens[tokenId] = uint(requestId);
    }

    function fulfillRandomness(bytes32 requestId, uint randomness) internal override {
        uint tokenId = _requestData[requestId];
        EggToken et = EggToken(eggTokenAddress());
        
        EggInfo.Details memory info = et.eggInfo(tokenId);
        (DragonInfo.Types dragonType, uint genes) = _randomGenes(info, randomness);
        et.hatch(tokenId, dragonType, genes, et.ownerOf(tokenId));

        delete _requestData[requestId];
        delete _pTokens[tokenId];
    }

    function _randomGenes(EggInfo.Details memory info, uint randomness) 
    internal view 
    returns (DragonInfo.Types, uint) {
        DragonInfo.Types t = (info.dragonType == DragonInfo.Types.Unknown) 
            ? _randomDragonType(randomness) : info.dragonType;
        
        uint genes = GenesLib.randomSetGenesToPositions(
            0, GenesLib.createOrderedRangeArray(COMMON_RANGE.from, COMMON_RANGE.to), 
            Random.rand(randomness), true);
    ...
    function _randomDragonType(uint salt) internal view returns (DragonInfo.Types) {
        ...
        uint remainingLegendarySupply = et.randomDragonSupply(DragonInfo.Types.Legendary);
        uint remainingEpic20Supply = et.randomDragonSupply(DragonInfo.Types.Epic20);
        uint remainingCommonSupply = et.randomDragonSupply(DragonInfo.Types.Common);

        uint remainingTotalSupply = 
            remainingLegendarySupply + remainingEpic20Supply + remainingCommonSupply;
        
        uint r = Random.rand(salt) % THRESHOLD_DENOMINATOR;
    ...
    function _calcDragonThreshold(uint remainingDragonSupply, uint remainingTotalSupply) 
    pure internal 
    returns (uint) {
        return remainingDragonSupply * THRESHOLD_DENOMINATOR / remainingTotalSupply;
    }
}
...
contract EggToken is ERC721Burnable, BaseAccessControl {
