...
contract Phase4 is ERC721, ERC721Burnable,DefaultOperatorFilterer, ERC721Enumerable, Ownable{
    ...
    constructor(string memory _name, string memory _symbol, address _phase3Address) ERC721(_name,_symbol) {
        phase3Address = _phase3Address;
    ...
    */
    function claim(uint256 phase3NFTId) external {
        require(toggle == true, "Phase4: Toggle is off");
        require(revealTime !=0, "Phase4: Set revel Timer");
        require(block.timestamp > revealTime, "Phase4: Redeem has not started yet");
        require(IERC721(phase3Address).ownerOf(phase3NFTId) == msg.sender, "Phase4: You are not the owner of this NFT");
        require(alreadyClaimed[phase3NFTId] == false, "Phase4: NFT already claimed for this token");
        require(_randomNumbers.length != 0, "Phase4: All NFT has been already minted");
        uint256 randomId = randomTokenId();
        alreadyClaimed[phase3NFTId] = true;
    ...
    function updatePhase3Address(address phase3Address_) external onlyOwner{
        require(phase3Address_ != address(0), "Phase4: Phase3 address cannot be zero Address");
        phase3Address = phase3Address_;
    }
    ...
    function transferFrom(address from, address to, uint256 tokenId) public override(ERC721, IERC721) onlyAllowedOperator(from) {
        require(_isApprovedOrOwner(_msgSender(), tokenId),"Phase4: transfer caller is not owner nor approved");
        uint256 temp;
    ...
    function _burn(uint256 _tokenId) internal override(ERC721) {
        require(_isApprovedOrOwner(_msgSender(), _tokenId), "Phase4: caller is not token owner nor approved");
