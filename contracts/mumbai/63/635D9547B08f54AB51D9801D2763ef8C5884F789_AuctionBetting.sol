...
contract AuctionBetting is Ownable, ReentrancyGuard {
    ...
    struct Auction {
        uint256 minPrice; // minimum aution price
    ...
    address public constant DEFAULT_BETTING_TOKEN_ADDRESS =
        0xdAC17F958D2ee523a2206206994597C13D831ec7; //USDT address
    uint256 public constant DEFAULT_PRICE_INCREMENT = 10000; // 1 cent,  how much the current price of the auction will be increased,

    //state variables
    address public companyAddress; // Company wallet Adress
    address public bettingToken; //usdt token
    uint256 public auctionIndex; //auction id
    ...
    function setIncrementPrice(uint256 _incrementPrice) external onlyOwner {
        priceIncrement = _incrementPrice > 0
            ? (_incrementPrice * (10 ** decimals)) / 100
            : DEFAULT_PRICE_INCREMENT;
    }

    function setFeePercentage(uint256 _feePercentage) external onlyOwner {
        companyFeePercentage = _feePercentage > 0
            ? _feePercentage
            : DEFAULT_COMPANY_FEE_PERCENTAGE;
    }

    function setTicketPrice(uint256 _ticketPrice) external onlyOwner {
        ticketPrice = _ticketPrice > 0
            ? _ticketPrice * (10 ** decimals)
            : DEFAULT_TICKET_PRICE;
    }

    function buyBettingPackages() external {
        tickets[msg.sender] += ticketCountsPerPackage;
        uint256 companyFee = (ticketPrice *
            ticketCountsPerPackage *
            companyFeePercentage) / DEFAULT_FEE_PRECISION;
    ...
    function createAuction(uint256 _rewardAmount) external onlyOwner {
        ...
        if (remainedPrize > (_rewardAmount * (10 ** decimals) * 1352) / 1000) {
            _beginningPrice =
                (_rewardAmount * (10 ** decimals) * 1352) /
                100000;
        } else {
        ...
        Auction memory newAuction = Auction({
            minPrice: (_rewardAmount * (10 ** decimals) * 1352) / 100000,
    ...
    function closeAuction(uint256 _auctionId) external onlyOwner {
        ...
        require(
            auctions[_auctionId].currentPrice > auctions[_auctionId].minPrice,
            "Auction is not finished"
        );
        require(
            block.timestamp > highestBid.lastBidTime + 30 seconds,
            "Auction is not finished yet"
        );
        auctions[_auctionId].ended = true;
    }

    function determinWiners(
        uint256 _auctionId
    ) external onlyOwner nonReentrant {
        require(auctions[_auctionId].ended == true, "Auction is not ended yet");
        uint256 winerCount;
        if (
            auctions[_auctionId].currentPrice / auctions[_auctionId].minPrice ==
            1
            ...
            winerCount =
                auctions[_auctionId].currentPrice /
                auctions[_auctionId].minPrice /
                2;
        ...
        uint256 companyReward = (auctions[_auctionId].prizeAmount +
            remainedPrize -
            auctions[_auctionId].rewardAmount *
            winerCount) / 2;
        IERC20(bettingToken).safeTransfer(companyAddress, companyReward);
        remainedPrize = companyReward + auctions[_auctionId].currentPrice;
    }
    ...
    function bet(uint256 _auctionId) external nonReentrant {
        ...
        if (auction.currentPrice <= auction.minPrice) {
            ...
            auction.prizeAmount +=
                (ticketPrice * (100 - companyFeePercentage)) /
                DEFAULT_FEE_PRECISION;
        } else {
            require(
                block.timestamp < highestBid.lastBidTime + 30 seconds,
                "Time up"
            ...
            require(
                freeTickets[msg.sender] > 0 || tickets[msg.sender] > 0,
    ...
    function claimReward(uint256 _auctionId) external nonReentrant {
        ...
        uint256 winerReward = auctions[_auctionId].rewardAmount -
            auctions[_auctionId].currentPrice;
        IERC20(bettingToken).safeTransfer(msg.sender, winerReward);
    }

    function findBiderPosition(
        uint256 _auctionId
    ) public view returns (uint256[] memory bidderPositions) {
    ...
    function getAuctions() public view returns (Auction[] memory) {
        Auction[] memory auctionArray = new Auction[](auctionIndex);
        for (uint256 i = 0; i < auctionIndex; ++i) {
            Auction storage auction = auctions[i];
            auctionArray[i] = auction;
        }
        return auctionArray;
    }
}