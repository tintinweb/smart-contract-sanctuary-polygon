...
contract NftPacks is Ownable, Pausable, AccessControl, ReentrancyGuard, VRFConsumerBase, IERC1155Receiver {
  ...
  function _mint(
    ...
    uint256 _amount,
    bytes memory /* _data */
  ) internal whenNotPaused onlyRole(MINTER_ROLE) nonReentrant {
    ...
    require(devRandom || LINK.balanceOf(address(this)) > linkFee, "Not enough LINK - fill contract with faucet");

    isOpening[_toAddress] = _optionId;

    // check if we're past the seed life and don't have a request ID
    if(!devRandom && seedData.requestId == 0 && block.number >= seedData.lastSeed.add(seedData.seedLife)){
      // send off for a new request 
      seedData.requestId = requestRandomness(keyHash, linkFee);
    } 

    doMint(_toAddress, _optionId, _amount);
  }
  ...
  */
  function fulfillRandomness(bytes32, uint256 randomness) internal override onlyVRFCoordinator {
    // sets the seed
    seedData.randomSeed = randomness;
    // set the last block
    seedData.lastSeed = block.number;
    // clear the request
    seedData.requestId = 0;
  }

  function doMint(address _userAddress, uint256 _optionId, uint256 _amount) private {
    ...
    for (uint256 i = 0; i < _amount; i++) {
      ...
      if (settings.hasGuaranteedClasses) {
        ...
        for (uint256 classId = 1; classId < settings.guarantees.length; classId++) {
            ...
            if(quantityOfGaranteed > 0) {
              tokenId = _sendTokenWithClass(classId, _userAddress, quantityOfGaranteed);
              lastOpen[_userAddress].push();
      ...
      while (quantitySent < settings.maxQuantityPerOpen) {
        ...
        tokenId =  _sendTokenWithClass(classId, _userAddress, quantityOfRandomized);
        lastOpen[_userAddress].push(tokenId);
...
/*
      if(tokenId == 0){
        ...
        for(uint256 i=0; i<tries; ++i){
          tokenId = _pickRandomAvailableTokenIdForClass(_classId);
           if(tokenId > 0){
            break;
          }
        }
...
*/      
  ...
  function _random() internal returns (uint256) {
    uint256 randomNumber = uint256(keccak256(abi.encodePacked(block.timestamp, msg.sender, seedData.randomSeed)));
    seedData.randomSeed++;
    return randomNumber;
  }
...
//    ERC1155Tradable nftContract = ERC1155Tradable(nftAddress);
  ...
  function setSeedLife(uint256 _seedLife) public onlyOwner {
        seedData.seedLife = _seedLife;
    }

  function setDevRandom(bool _devRandom) public onlyOwner {
      devRandom = _devRandom;
  }
