...
import './lib/PBTSimple.sol';

/// @title CoolPBT
/// @author Adam Goodman
/// @notice This contract allows tokens to be linked to a physical item
contract CoolPBT is PBTSimple, Ownable {
  ...
  event MintStatusChanged(bool mintOpen);
  ...
  /// @notice Seeds the chip addresses for tokens
  /// @dev Only the owner can call this function, should only be called for tokenIds that have not yet been minted
  ///      For chips that have already been minted, use updateChips
  /// @param chipAddresses The chip addresses
  /// @param tokenIds The token IDs
  /// @param throwIfTokenAlreadyMinted Whether to throw if the token has already been minted
  function seedChipsToTokens(
  ...
  /// @notice Updates the chip addresses for a token
  /// @dev Only the owner can call this function, should only be called for tokenIds that have already been minted
  /// @param oldChipAddresses The old chip addresses
  /// @param newChipAddresses The new chip addresses
  function updateChips(
    address[] calldata oldChipAddresses,
    address[] calldata newChipAddresses
  ) external onlyOwner {
    _updateChips(oldChipAddresses, newChipAddresses);
  }

  /// @notice Mint a token with a chip signature
  /// @param signatureFromChip The signature from the chip
  ...
  /// @notice Opens or closes minting
  /// @dev Only the owner can call this function
  /// @param mintOpen Whether minting should be open or closed
  function setMintState(bool mintOpen) external onlyOwner {
    _mintOpen = mintOpen;

    emit MintStatusChanged(mintOpen);
  }
...
contract PBTSimple is ERC721ReadOnly, IPBT {
  ...
  function _transferTokenWithChip(
    ...
    uint256 tokenId = _getTokenDataForChipSignature(signatureFromChip, blockNumberUsedInSig)
      .tokenId;
