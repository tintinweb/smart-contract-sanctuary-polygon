...
contract OlympusStaking is OlympusAccessControlled {
    ...
    uint256 public stakingTax;  // non wei
    uint256 public unstakingTaxEarly; // non wei
    uint256 public unstakingTax; // non wei
    ...
    function claim(address _to, bool _rebasing) public returns (uint256) {
        ...
        if (epoch.number >= info.expiry && info.expiry != 0) {
            ...
            uint256 tax = 0;
            if(block.timestamp > info.expiry && block.timestamp < info.expiry + 7 days){
                tax = (info.gons).mul(unstakingTaxEarly).div(100);
            }
            else if(block.timestamp >= info.expiry + 7 days){
                tax = (info.gons).mul(unstakingTax).div(100);
            }

            info.gons = info.gons.sub(tax);
