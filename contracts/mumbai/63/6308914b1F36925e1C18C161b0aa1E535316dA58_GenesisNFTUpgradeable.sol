    ...
    UUPSUpgradeable
{
    ...
    {
        require(msg.value == salesInfo().mintPrice.mul(requestedCount), "Sales: not enough ETH");
    ...
    function setDiscountRate(uint256 discountRate_) public onlyOwner {
        uint256 h = 100;
        discountRate = (h.sub(discountRate_)).div(h);
    }

    function mintPriceByAccount(address account) public view returns (uint256) {
        return isWhiteList(account) ? salesInfo().mintPrice.mul(discountRate) : salesInfo().mintPrice;
    }
