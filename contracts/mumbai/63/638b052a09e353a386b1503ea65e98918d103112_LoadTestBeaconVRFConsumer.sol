...
contract LoadTestBeaconVRFConsumer is BeaconVRFConsumer {
    ...
    mapping(VRFBeaconTypes.RequestID => uint256)
    ...
    function fulfillRandomWords(
        ...
        s_totalFulfilled++;
        s_fulfillmentDurationInBlocks[requestID] = requestDelay;
    }
        ...
        returns (VRFBeaconTypes.RequestID[] memory)
    {
        ...
        for (uint256 i = 0; i < s_requestIDs.length; i++) {
            ...
            if (s_fulfillmentDurationInBlocks[reqID] == 0) {
    ...
    function reset() external {
        ...
        for (uint256 i = 0; i < s_requestIDs.length; i++) {
            delete s_requestOutputHeights[s_requestIDs[i]];
            delete s_fulfillmentDurationInBlocks[s_requestIDs[i]];
        }
        delete s_requestIDs;
    }
}
