...
/**
 * @notice Interface to access methods from Kindeck Inception ERC721 contract
 * @dev getTokenManagerAddress can be used to fetch managers proxy contract addresses
 * @dev getParent can be used to fetch parent for calling user
 */
interface IKindeckInception {
    function getTokenManagerAddress(address _manager)
        external
        view
        returns (address);
...
interface IKindeckFluidManager {
...
/**
 * @title Contract for managing product and sales inside Kindeck marketplace
 */
contract KindeckMarketplace is
    ERC721Enumerable,
    IERC777Sender,
    IERC777Recipient,
    Ownable
{
    ...
    mapping(uint256 => address) internal productManagers; //productId => contract
    mapping(uint256 => uint256) internal productPrices; //productId => price for product
    mapping(uint256 => uint256) internal products; //tokenId => productId

    bool private _shouldRevertSend;
    address private immutable _adminSigner;
    uint256 internal tokenCounter = 1;
    string internal baseURI;
    address private kindeckVendor;

    IKindeckInception kindeckInception;

    /**
     * @notice Register ERC777 for fdai that is used as currency in marketplace
     * @dev IERC1820 registry for receiving? Implementer for sending?
     */
    ...
    /**
     * @param adminSigner - used for ecrecover
     * @param _baseURI - baseURI for token metadata
     * @param _kindeckInceptionAddr - address for Kindeck Inception ERC721 contract
     */
    constructor(
        address adminSigner,
        address _operator,
        address _kindeckVendor,
        string memory _baseURI,
        address _kindeckInceptionAddr,
        ISuperToken _superToken
    ) ERC721("Kindeck Product Manager", "KINP") {
        _adminSigner = adminSigner;
        kindeckVendor = _kindeckVendor;
        baseURI = _baseURI;
        ...
        kindeckInception = IKindeckInception(_kindeckInceptionAddr);

        token = _superToken;
        setAdmin(_operator, true);
    }

    modifier onlyAdmin() {
        require(
            admins[msg.sender],
            "Only an `admin` address can call this method"
        );
        _;
    }

    /**
     * @notice function to validate generated and signed tickets. Used for permission checks.
     * @param _digest - keccak256 encoded input from ticket.
     * @param _ticket - the ticket to be validated from the signed admin.
     */
    ...
    function setAdmin(address _operator, bool _allowed) public onlyOwner {
        require(_operator != address(0), "Invalid address");
        require(
            admins[_operator] != _allowed,
    ...
    /**
     * @notice fetches tokenURI for product nfts. Mapped on productIds.
     */
        ...
        returns (string memory)
    {
        ...
        uint256 _product = products[tokenId];
        return string(abi.encodePacked(baseURI, "/", _product.toString()));
    }
    ...
    /**
     * @notice function for creating new products. Only users who have been granted access through a signed ticket
     * can create new products. They will then be set as manager and receieve sale fees for that product.
     * @param _productId - identifier for product
     * @param _manager - manager to be set for this product.
     * @param _price - sale price of the product.
     * @param _ticket - signed ticket for permission confirmation.
     * @dev _manager is the sellers proxy contract that should receive funds.
     */
    function createProduct(
        ...
        require(
            _manager != address(0),
            "You can nog use a zero address for managers"
        );

        productManagers[_productId] = _manager;
        productPrices[_productId] = _price;
    }

    /**
     * @notice function to transfer funds to receiver
     */
    function send(
        address to,
        uint256 amount,
    ...
    /**
     * @notice function to mint the product that buyer is purchasing. All products are represented through a NFT.
     * @param _productId - identifier for product.
     * @param _to - buyer of the product.
     */
    function mintProduct(
        ...
        address _receiver = productManagers[_productId];
        require(_receiver != address(0), "Could not find manager for product");

        uint256 _price = productPrices[_productId];
        require(_price != 0, "Could not find price for product");

        if (_receiver == kindeckVendor) {
            address _parent = kindeckInception.getParent(_to);
            address _parentContractaddress = kindeckInception
                .getTokenManagerAddress(_parent);
            address _buyerContractAddress = kindeckInception
                .getTokenManagerAddress(_to);
            if (
                _parent != address(0) &&
                _parentContractaddress != address(0) &&
                _buyerContractAddress != address(0)
            ) {
                (
                    bool exist,
                    bool approved,
                    uint128 units,
                    uint256 pendingDistribution
                ) = IKindeckFluidManager(_buyerContractAddress)
                        .getSubscriptionBySubscriber(_parentContractaddress);
                if (exist) {
                    uint256 _kickback = (_price / 100) *
                        convertUnitsToPercent(units);
                    uint256 _vendorPrice = _price - _kickback;
                    send(_parentContractaddress, _kickback, _userData);
                    send(_receiver, _vendorPrice, _userData);
                } else {
                    send(_receiver, _price, _userData);
                }
            } else {
                send(_receiver, _price, _userData);
            }
        ...
        products[tokenCounter] = _productId;
        tokenCounter++;
    }

    /**
     * @notice fallback function if we want to stop send calls
     * @param shouldRevert - bool to set if we should revert or not
     */
    function setShouldRevertSend(bool shouldRevert) external onlyAdmin {
        _shouldRevertSend = shouldRevert;
    }

    /**
     * @notice overriden function that gets triggered before tokens are sent. Checks if sending is allowed and that
     * sender has correct balance.
     */
    ...
    function tokensReceived(
        ...
        uint256 _productId = abi.decode(userData, (uint256));
        if (productPrices[_productId] > amount) {
            revert();
        }
        mintProduct(_productId, from, userData);
    }

    /**
     * @dev to account for units being on a 10e4 scale
     */
