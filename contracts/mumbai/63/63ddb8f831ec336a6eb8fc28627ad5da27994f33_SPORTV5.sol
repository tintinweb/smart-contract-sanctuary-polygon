...
import "./sport_v4.sol";
import "./bet_v4.sol";
contract SPORTV5 is SPORTV4 {

    ESkillzStraightBetV4 public ESkillzBetV4;
     function setBetV5Address(ESkillzStraightBetV4 _bet) external onlyOwner {
        ESkillzBetV4 = _bet;
    }  

    function approveAndCreateSPGameV5(address spender, uint256 amount, string memory _uID) external {
        require(amount>0, "Amount should be bigger than zero");
        approve(spender, amount);
        ESkillzBetV4.CreateSPGameV4(msg.sender, amount, _uID);
    }

    function approveAndCreateMPGameV5(address spender, uint256 amount, string memory _uID) external {
        require(amount>0, "Amount should be bigger than zero");
        approve(spender, amount);
        ESkillzBetV4.CreateMPGameV4(msg.sender, amount, _uID);
    }

    function approveAndJoinMPGameV5(address spender, uint256 amount,  uint256 gameID, string memory _uID) external {
        require(amount>0, "Amount should be bigger than zero");
        approve(spender, amount);
        ESkillzBetV4.JoinMPGameV4(msg.sender, gameID, amount, _uID);
    }
...
import "./bet_v3.sol";
import "./sport_v5.sol";
contract ESkillzStraightBetV4 is ESkillzStraightBetV3 {

    SPORTV5 public sportV5;
    mapping(string => address) public whiteList;

    function setSportV4Address(SPORTV5 _sport) external onlyOwner {
        sportV5 = _sport;
    }

    function addWhiteList(string memory _UserID, address _walletAddress) external onlyOwner {
        require(whiteList[_UserID] == address(0), "WhiteList setted before.");
        whiteList[_UserID] = _walletAddress;
    }

    function  CreateSPGameV4(address _sender, uint256 amount, string memory _uID) external onlySportContract{
    
      require(whiteList[_uID] == _sender, "You were not added whiteList. Register to the whiteList now.");
      ...
      GameIDs++;
      sportV5.transferFrom(_sender, address(this), amount);
      delete(gamebetting[GameIDs]);
      gamebetting[GameIDs].push(Bet(_sender, amount, 0));
      emit BetEvent(GameIDs, amount);
    }

    function  CreateMPGameV4(address _sender, uint256 amount, string memory _uID) external onlySportContract{
         require(whiteList[_uID] == _sender, "You were not added whiteList. Register to the whiteList now.");
      require(minBetAmounts<= amount, "Bet amounts must be bigger than minBetAmounts");
      totalAmountsOfDay[(block.timestamp- startTimeStamp)/ 1 days] += amount;
      GameIDs++;
    ...
    function  JoinMPGameV4(address _sender, uint256 gameID, uint256 amount, string memory _uID) external onlySportContract{
        require(whiteList[_uID] == _sender, "You were not added whiteList. Register to the whiteList now.");
