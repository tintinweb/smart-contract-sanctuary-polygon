...
contract FunctionsConsumer is FunctionsClient, ConfirmedOwner {
  ...
  function fulfillRequest(
    ...
    latestError = err;
    uint256 temp = uint256(bytes32(response));
    nftDog.updateMetaData(temp);
...
contract NFTDogAdvanced is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    constructor(address vrfAddr) ERC721("NFTDog", "ND") {
    ...
    function safeMint() public {
        ...
        reqIdToTokenId[reqId] = tokenId;
    }

    function randomMint(uint256 randomNumber, uint256 reqId) external {
        uint256 tokenId = reqIdToTokenId[reqId];
        if(randomNumber % 3 == 0) {
...
contract VRFv2Consumer is VRFConsumerBaseV2, ConfirmedOwner {
    ...
    function setNFTAddr(address addr) external {
        nftDog = NFTDogAdvanced(addr);
    } 
    ...
    function fulfillRandomWords(
        ...
        nftDog.randomMint(_randomWords[0], _requestId);
