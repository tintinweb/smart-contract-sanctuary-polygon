...
contract Lambo is ERC20Burnable, Ownable {
    /*
    .____                  ___.           
...
*/
    ...
    uint256 public CLAIM_END_TIME = 1704067200;
    ...
    constructor() ERC20("Lambo Staker", "LAMBO") {}
    ...
    function stakeByIds(uint256[] memory tokenIds) public {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                ERC721B(collectionAddress).ownerOf(tokenIds[i]) == msg.sender &&
                    tokenIdToStaker[tokenIds[i]] == nullAddress,
                "Token must be stakable by you!"
            );

            ERC721B(collectionAddress).transferFrom(
                msg.sender,
                address(this),
                tokenIds[i]
            );
