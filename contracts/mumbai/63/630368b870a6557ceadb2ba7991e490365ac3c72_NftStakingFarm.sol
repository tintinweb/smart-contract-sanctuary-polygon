    ...
    ERC721Holder
{
  ...
  //  using Calculator for uint256;
   ...
   // event BaseApySet(uint256 value, address sender);
    ...
    event NftApySet(uint256 value, address sender);
   ...
   //  mapping(address => uint256) private funding;
    ...
    // Nft balance of users(user address->amount))
    mapping(address => uint256) public nftBalancesStacked;
     // Nft times per users(user address->(nftId->time))
    mapping(address => mapping (uint256=> uint256)) public nftTimes;
    ...
    function setBaseApy(uint256 _APY) public onlyOwner {
        BASE_APY = _APY;
        emit NftApySet(_APY, msg.sender);
    }
     ...
     */
    function stakeNft(uint256 id
    ) external whenNotPaused nonReentrant {
       _stakeNft(msg.sender, address(this), id);
    }
    ...
    function _stakeNft(
        ...
        // modify nftBalancesStacked of user
        nftBalancesStacked[_from] = nftBalancesStacked[_from].add(1);
        // modify user's nft id array
        setUserNftIds(_from, _id);
        totalNftStaked = totalNftStaked.add(1);

        nftTimes[_from][_id] =  _now();
    ...
    function _unstakeNft(
        ...
        // reduce nft balance
        nftBalancesStacked[msg.sender] = nftBalance.sub(1);
    ...
    */
    function claimRewardById(uint256 _id) external whenNotPaused nonReentrant {
       require(nftBalancesStacked[msg.sender]>0 , "No Nts Stocked");
        ...
        // update stake date
        nftTimes[msg.sender][_id] = claimTimestamp;
    ...
    function recalculateBalance(address _from, uint256 _id) internal {
        ...
        require(
            timePassed >= 0,
            "NFT and reward unlocked after 1 day"
        );
    ...
    {
        uint256 stakeDate = nftTimes[_user][_id];
        // one day seconds
        uint256 oneDay = 5 minutes; //days
        ...
        uint256 totalWithInterest = _days.mul(BASE_APY);
