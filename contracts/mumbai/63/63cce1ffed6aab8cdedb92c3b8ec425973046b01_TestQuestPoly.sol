...
contract QuestPoly is KeeperCompatibleInterface { // proxy implementation
    ...
    event NewQuest(uint256 combatId,uint8 gameType,uint256 indexed performId,uint256 indexed actionId,uint8 indexed groupIndex,QuestLike.Action action,uint256[] scores,uint256[] chances);
    ...
    struct LbMappingInfo {
        ...
        // uint256[][WINNER_COUNT] chances;
    }
    struct LbWinnerInfo {
        QuestLike.Action winner;
        uint256[] score;
        // uint256[] chance; // TODO -- leaderboard winners chance will be shown later
    }
     ...
     */
    function doQuestByUser(uint256[] memory doges, uint256[] memory scores, uint8 groupIndex) external {
        ...
        for (uint256 i = 0; i < doges.length; i++) {
            if(i == 0) params_ = keccak256(abi.encodePacked(doges[i], scores[i]));
            else params_ = keccak256(abi.encodePacked(params_, doges[i], scores[i]));
        }
        ...
        _doQuest(doges, owners, scores, groupIndex, 0, 0);
    }
    ...
    function doQuestByAdmin(uint256[] memory doges, address[] memory owners, uint256[] memory scores, uint8 groupIndex, uint256 combatId) external {
        ...
        // for (uint256 i = 0; i < doges.length; i++) {
        //     owners[i] = msg.sender;
        //     require(dogewood.validateOwnerOfDoge(doges[i], owners[i]), "invalid owner");
        // }

        _doQuest(doges, owners, scores, groupIndex, combatId, 1);
    }

    // gameType - (0: unity game, 1: web game)
    function _doQuest(uint256[] memory doges, address[] memory owners, uint256[] memory scores, uint8 groupIndex, uint256 combatId, uint8 gameType) internal {
        ...
        // entry check
        uint256[] memory finalScores_ = new uint256[](doges.length);
        uint256[] memory chances_ = new uint256[](doges.length);
        for (uint256 i = 0; i < doges.length; i++) {
            ...
            finalScores_[i] = ownerScores[owners[i]].score;
            chances_[i] = dogeEntryLog[doges[i]].entryCount; // TODO -- should add earned entry (winnerEntryConfig[k])
        }
        // cooldown update
        ...
        emit NewQuest(combatId, gameType,  _currentPerformId, _currentActionId, groupIndex, action, finalScores_, chances_);
    }
    ...
    function _performLeaderboard() internal {
        ...
        for (uint8 gid = 0; gid < GROUP_COUNT; gid++) {
            ...
            for (uint256 rank_ = 0; rank_ < WINNER_COUNT; rank_++) {
                emit LeaderboardPerformWinnersInfo(_currentPerformId,gid,rank_,LbWinnerInfo(leaderboard[gid].winners[rank_], leaderboard[gid].scores[rank_]));
            }
        }
