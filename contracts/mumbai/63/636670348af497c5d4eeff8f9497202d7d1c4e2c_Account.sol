...
import "../Store/ERC721StoreChild.sol";
  ...
  IProduct 
{
   ...
   */
  function initialize(uint256 id, IStorage store, address admin) 
    public initializer 
  {
   ...
   */
  function product(string memory productId) public view returns(
    //max supply size
    uint256 maxSupply,
    ...
    returns(address receiver, uint256 royaltyAmount) 
  {
    ...
    (,,uint256 royaltyPercent, address royaltyRecipient,,) = product(productId);
  ...
  function supportsInterface(bytes4 interfaceId) 
    ...
    view 
    virtual 
    override(AccessControlEnumerableUpgradeable, ERC721StoreChild) 
    returns(bool) 
  {
  ...
  IERC721MetadataUpgradeable
{
  ...
  function __ERC721StoreChild_init(uint256 id, IStorage store) internal onlyInitializing {
    _childId = id;
    _store = store;
  }
   ...
   */
  function childId() public view returns(uint256) {
    return _childId;
  }
  ...
  function approve(address to, uint256 tokenId) public virtual override {
    address owner = ERC721StoreChild.ownerOf(tokenId);
   ...
   */
  function _storage() internal view returns(IStorage) {
    return _store;
  }
}
...
interface IStorage is IAccount, IProduct, IToken {
