...
contract TreasureHuntShip is Initializable, ITreasureHuntTrait, ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable, OwnableUpgradeable {
    ...
    function getShips(uint256[] memory tokenIDs) view public returns(Ship[] memory) {
        Ship[] memory ships = new Ship[](tokenIDs.length);
    ...
    function getAcitveShips() public view returns(Ship[] memory) {
        return getShips(activeShipsByOwner[msg.sender]);
    }
    ...
    function getInAcitveShips() public view returns(Ship[] memory) {
        return getShips(inActiveShipsByOwner[msg.sender]);
    }
    ...
    function giveGift(address receiver, uint16 amount, bool isDirect) public {
        ...
        if (isDirect) {
            ...
            ITreasureHunt(rewardPool).payUsingReward(msg.sender, giftFee * amount, false);
        }
    ...
    function burnAllShips() public {
        uint amount = inActiveShipsByOwner[msg.sender].length;
        for (uint i; i < amount; i++) {
            uint256 tokenID = inActiveShipsByOwner[msg.sender][i];
            _burn(tokenID);
            delete shipInfo[tokenID];
            delete tokenIndex[inActiveShipsByOwner[msg.sender][i]];
        }
        delete inActiveShipsByOwner[msg.sender];
        ITreasureHunt(rewardPool).transferBurnReward(msg.sender, burnPrice * amount);
    }
