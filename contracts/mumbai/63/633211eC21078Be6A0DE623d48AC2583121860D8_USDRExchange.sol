...
contract USDRExchange is AddressAccessor, IExchange, Pausable {
    ...
    function swapToTNGBL(uint256 amountIn, address to)
        external
        whenNotPaused
        returns (uint256)
    {
    ...
    function swapFromTNGBL(
        ...
        uint256 multiplier;
        {
            ...
            uint256 usdrMarketCap = IStaking(staking).usdrMarketCap();
        ...
        IERC20(tngbl).safeTransferFrom(msg.sender, address(this), amountIn);
        IERC20(tngbl).approve(treasury, amountIn);
        ITreasury(treasury).deposit(tngbl, amountIn);
        {
        ...
        )
    {
        ...
        (treasury, usdr, tngbl, oracle) = abi.decode(
            addressProvider.getAddresses(
                abi.encode(TREASURY_ADDRESS, USDR_ADDRESS, TNGBL_ADDRESS, TNGBL_ORACLE_ADDRESS)
            ),
        ...
        amountOut = (amountOut * 1e18) / tngblPrice;
