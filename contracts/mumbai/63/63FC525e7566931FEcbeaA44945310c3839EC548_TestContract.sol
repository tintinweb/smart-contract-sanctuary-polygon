...
contract TestContract is BaseContract {
    ...
    //添加多个周地址，以满足触发条件
    function addWeekAddress(address _address) external onlyOwner {
        for (uint256 i = 0; i < 7; i++) {
            weekAddressList["Invest"].push(_address);
            weekAddressList["Invite"].push(_address);
        }
        weekDataList["Invest"] = [1 * 1e18, 2 * 1e18, 3 * 1e18, 4 * 1e18, 5 * 1e18, 6 * 1e18, 7 * 1e18];
        weekDataList["Invite"] = [7, 6, 5, 4, 3, 2, 1];
        weekDayList["Invest"] = [1, 2, 3, 4, 5, 6, 7];
        weekDayList["Invite"] = [1, 2, 3, 4, 5, 6, 7];
    }
