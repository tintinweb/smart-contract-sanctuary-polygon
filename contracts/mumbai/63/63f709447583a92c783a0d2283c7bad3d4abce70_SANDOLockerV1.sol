    ...
    ReentrancyGuardUpgradeable
{
    ...
    mapping(string => LockerType) public lockerTypeList;
    ...
    function unlock(uint256 _lockerId, uint256 _expiration)
        external
        nonReentrant
        whenNotPaused
    {
        uint256 _expirationList = userTokenLock[msg.sender][_lockerId]
            .expiration;

        if (_expiration == _expirationList) {
            uint256 _amount = userTokenLock[msg.sender][_lockerId].amount;

            token.safeTransfer(msg.sender, _amount);
            userTokenLock[msg.sender][_lockerId].status = true;
        }
    }
