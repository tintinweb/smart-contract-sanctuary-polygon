...
interface IRecovery is IYousovStructs{
    ...
    event AgentRoleForRecovery(address agent, address recovery, RecoveryRole recoveryRole, string initialAnswer);
    event SendAnswersToAnswerAgents(address agent, address recovery, string actualAnswer);
    ...
    function isUserIsAnAnswerAgent(address userAgent) external view returns (bool);
}
...
interface IRecoveryFactory is IYousovStructs{
    ...
    function getLegalSelectableAgents(address _currentRecovery) external ;
    function getCurrentUserRecovery(address user) external returns (address);
}
