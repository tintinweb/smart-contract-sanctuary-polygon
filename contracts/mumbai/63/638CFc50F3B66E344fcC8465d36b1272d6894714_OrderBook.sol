...
contract OrderBook is ReentrancyGuard, IOrderBook {
    ...
    function initialize(
        ...
        emit Initialize(_router, _vault, _weth, _usda, _minExecutionFee, _minPurchaseTokenAmountUsd);
    }
    ...
    function getUsdaMinPrice(address _otherToken) public view returns (uint256) {
        // USDA_PRECISION is the same as 1 USDA
        uint256 redemptionAmount = IVault(vault).getRedemptionAmount(_otherToken, USDA_PRECISION);
    ...
    function validateSwapOrderPriceWithTriggerAboveThreshold(
        ...
        // 1. USDA doesn't have a price feed so we need to calculate it based on redepmtion amount of a specific token
        // That's why USDA price in USD can cvary depending on the redepmtion token
        // 2. In complex scenarios with path=[USDA, BNB, BTC] we need to know how much BNB we'll get for provided USDA
        // to know how much BTC will be received
        // That's why in such scenario BNB should be used to determine price of USDA
        if (tokenA == usda) {
            // with both _path.length == 2 or 3 we need usda price against _path[1]
            tokenAPrice = getUsdaMinPrice(_path[1]);
        } else {
