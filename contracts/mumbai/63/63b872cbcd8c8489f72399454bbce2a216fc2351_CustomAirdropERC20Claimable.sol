...
import "@thirdweb-dev/contracts/interfaces/airdrop/IAirdropERC20Claimable.sol";
...
contract CustomAirdropERC20Claimable is
    Initializable,
    Ownable,
    ...
    IAirdropERC20Claimable
{
    ...
    function claim(
        address _receiver,
        uint256 _quantity, // availiable = 10000, maxQuantity = 200 -> quantity = 200,
                            // availiable = 300, maxQuantity = 200, claimed = 100 -> quantity = 100
                            // availiable = 0,
                            // maxQuantity = 0,
                            // quantity = availiable > maxQuantity - claimed ? maxQuantity - claimed : availiable
    ...
    function verifyClaim(
        ...
        if (merkleRoot != bytes32(0)) {
            (isOverride, ) = MerkleProof.verify(
                _proofs,
                merkleRoot,
                keccak256(bytes.concat(keccak256(abi.encode(_claimer, _proofMaxQuantityForWallet))))
            );
        }
...
import "./CustomAirdropERC20Claimable.sol";

contract PagerAirdrop {
