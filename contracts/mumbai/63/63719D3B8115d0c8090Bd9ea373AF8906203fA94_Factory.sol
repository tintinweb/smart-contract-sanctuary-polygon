...
import "./ERC1155/ERC1155Wakame.sol";
...
contract Factory is Ownable {
    ...
    event CollectionCreated(address proxy);
    ...
    function createToken(
        ...
        uint256 salt) 
        external {
        address beaconProxy = deployProxy(getData(_name, _symbol, _baseURI, _contractURI), salt);

        ERC1155Wakame token = ERC1155Wakame(beaconProxy);
        token.transferOwnership(_msgSender());

        emit CollectionCreated(beaconProxy);
    }

    //deploying BeaconProxy contract with create2
    function deployProxy(bytes memory data, uint256 salt) internal returns(address proxy){
    ...
    function getAddress(
        ...
        string memory _baseURI,
        string memory _contractURI, 
        uint256 _salt) public view returns (address)
    {   
        bytes memory bytecode = getCreationBytecode(getData(_name, _symbol, _baseURI, _contractURI));
    ...
    function getData(string memory _name, string memory _symbol, string memory _baseURI, string memory _contractURI) 
        pure 
        internal returns(bytes memory){
        return abi.encodeWithSelector(ERC1155Wakame.__ERC1155Wakame_init.selector, _name, _symbol, _baseURI, _contractURI);
    }
}
...
contract ERC1155Wakame is ERC1155Base, MinterAccessControl {
    event Created(address owner, string name, string symbol, string contractURI);

    function __ERC1155Wakame_init(
        string memory _name, 
        string memory _symbol,
        string memory _baseURI, 
        string memory _contractURI) external virtual {
        __ERC1155Wakame_init_unchained(_name, _symbol, _baseURI, _contractURI);

        emit Created(_msgSender(), _name, _symbol, _contractURI);
    }

    function __ERC1155Wakame_init_unchained(string memory _name, string memory _symbol, string memory _baseURI, string memory _contractURI) internal initializer {
...
contract ERC1155Base is OwnableUpgradeable, ERC1155Lazy {
    string public name;
    string public symbol;
    string public contractURI;

    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC1155Lazy) returns (bool) {
...
import "./MintValidator.sol";

abstract contract ERC1155Lazy is IERC1155LazyMint, ERC1155BaseURI, MintValidator {
...
contract MintValidator is ERC1271Validator {
...
library LibERC1155LazyMint {
    ...
    bytes32 public constant MINT_AND_TRANSFER_TYPEHASH = keccak256("Mint1155(uint256 tokenId,uint256 supply,string tokenURI,Part[] creators)Part(address account,uint96 value)");
