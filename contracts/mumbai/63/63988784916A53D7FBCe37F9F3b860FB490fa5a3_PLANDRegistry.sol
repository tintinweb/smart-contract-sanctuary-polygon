...
contract PLANDRegistry is Storage, Ownable, FullAssetRegistry, ILANDRegistry {
  bytes4 constant public GET_METADATA = bytes4(keccak256("getMetadata(uint256)"));

  function initialize(bytes) external {
    _name = "Poliversity LAND";
    _symbol = "PLAND";
    _description = "Contract that stores the Poliversity LAND registry";
  }
  ...
  /**
   * @notice Set PLAND updateOperator
   * @param assetId - PLAND id
  ...
  /**
   * @notice Set many PLAND updateOperator
   * @param _assetIds - PLAND ids
