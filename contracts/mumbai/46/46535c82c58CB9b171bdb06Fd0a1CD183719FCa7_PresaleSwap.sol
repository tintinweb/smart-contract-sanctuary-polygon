...
contract PresaleSwap is PresaleSwapStorage, Ownable, ReentrancyGuard {
    ...
    function swap(
        ...
        claims[msg.sender] += (_amount * rate) * (10 ** 9);
...
/** notes JaWsome
    ...
    1-claims[msg.sender] += (_amount * rate) / 1000 
      Changed To
      claims[msg.sender] += (_amount * rate) * (10 ** 9);

      - It can cause wrong calculation of claims when iDecimals are not same as wDecimals. 

      Notes Rufi: 
      Use a dynamic routine to counter this issue in future, I have hardcoded it for now.
