...
// File: contracts/ITransferProxy.sol
...
contract Trade is AccessControl, EIP712 {
    ...
    function verifySellerSign(
        ...
        bytes32 digest = _hashTypedDataV4(
            keccak256(
                abi.encode(
                    keccak256(
                        "signData(address assetAddress,uint256 tokenId, address paymentAssetAddress, uint256 amount, uint256 nonce)"
                    ),
                    assetAddress,
                    tokenId,
                    paymentAssetAddress,
    ...
    function verifyBuyerSign(
        ...
        bytes32 digest = _hashTypedDataV4(
            keccak256(
                abi.encode(
                    keccak256(
                        "signData(address assetAddress,uint256 tokenId, address paymentAssetAddress, uint256 amount, uint256 qty, uint256 nonce)"
                    ),
                    assetAddress,
                    tokenId,
                    paymentAssetAddress,
        ...
        address signer = ecrecover(digest, v, r, s);
        require(signer != address(0), "ECDSA: invalid signature");
        require(signer == buyer, "buyer sign verification failed");
    }
