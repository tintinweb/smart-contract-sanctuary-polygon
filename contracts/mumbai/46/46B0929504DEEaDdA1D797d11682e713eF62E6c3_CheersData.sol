...
// データ保存のためのコントラクト
contract CheersData is ICheersData, AccessControl {
  // DAOのデータ
  ...
  mapping(address => uint256) public projectIndexByProjectAddress;
	...
	constructor(address _cheers) {
		_grantRole(DEFAULT_ADMIN_ROLE, _cheers);
	}

  // Cheers.solのaddNewOwner関数からのみ呼ばれる
  // Owner（userやDAO）を追加
  ...
  // @note Cheers.solのchallenge関数からのみ呼ばれる
  // Projectを追加
  function addProject(
  ...
  // Cheers.solのcheer関数からのみ呼ばれる
  // Cheer済みフラグをオンにする
  function addCheerProject(address _cheererAddress, address _projectAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
    require(!isCheer[_cheererAddress][_projectAddress], "already cheer");
    isCheer[_cheererAddress][_projectAddress] = true;
  }

  // Cheers.solから呼ばれる予定 未実装
  // Cheer済みフラグをオフにする
  function removeCheerProject(address _cheererAddress, address _projectAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
