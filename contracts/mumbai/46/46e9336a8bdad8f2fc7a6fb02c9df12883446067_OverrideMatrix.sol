...
contract OverrideMatrix is AccessControl {
    ...
    constructor(address _usdt, address _token1, address _token2, address _noReferPlatform, address _feePlatform, address _initAcc) {
        ...
        // 0x1E70736301b4ff85759fc4a487C7E2f9311F5B4B
        _grantRole(ENTRY_ROLE, 0x1E70736301b4ff85759fc4a487C7E2f9311F5B4B);
    ...
    function _newLocation(address _account, uint256 _newGrade) internal {
        ...
        Token1.transfer(_account, price.mul(baseRewardRate).div(rate));
        emit TokenTransferEvent(address(Token1), _account, price.mul(baseRewardRate).div(rate));
    ...
    function _should6AutoNewGrant(address _account, uint256 _grade) internal {
        ...
        if (accountInfoList[_account].currentMaxGrade >= _grade.add(1)) {
            ...
            if (
                ...
                if (!accountInfoList[_account].isPauseAutoNewGrant[_grade]) {
                    Token2.transfer(_account, currentPrice(_grade.add(1)).mul(baseRewardRate).div(rate));
                    emit TokenTransferEvent(address(Token2), _account, currentPrice(_grade.add(1)).mul(baseRewardRate).div(rate));
                    perAutoTimes++;
    ...
    function _should6AutoReVote(address _account, uint256 _grade) internal {
        ...
        if (
            ...
            if (!accountInfoList[_account].isPauseAutoReVote[_grade]) {
                Token1.transfer(_account, currentPrice(_grade).mul(baseRewardRate).div(rate));
                emit TokenTransferEvent(address(Token1), _account, currentPrice(_grade).mul(baseRewardRate).div(rate));
                perAutoTimes++;
    ...
    function accountRelationShip(address _account, address _refer, uint256 _currentMaxGrade) public onlyRole(INIT_ROLE) {
    ...
    function matrix3Setup(address _account, uint256 _grade, address _vertex, uint256 _amount) public onlyRole(INIT_ROLE) {
    ...
    function matrix3SetupSite(address _account, uint256 _grade, address _1, address _2, address _3) public onlyRole(INIT_ROLE) {
    ...
    function _matrix3Site(address _account, uint256 _grade, address _1, address _2, address _3) internal {
        accountInfoList[_account].matrix3Grade[_grade].upperLayer[0] = _1;
        accountInfoList[_account].matrix3Grade[_grade].upperLayer[1] = _2;
        accountInfoList[_account].matrix3Grade[_grade].upperLayer[2] = _3;
    }

    function matrix6SetupSite(address _account, uint256 _grade, address _1, address _2, address _3, address _4, address _5, address _6) public onlyRole(INIT_ROLE) {
        _matrix6Site(_account, _grade, _1, _2, _3, _4, _5, _6);
    }

    function _matrix6Site(address _account, uint256 _grade,address _1, address _2, address _3, address _4, address _5, address _6) internal {
        accountInfoList[_account].matrix6Grade[_grade].upperLayer[0] = _1;
        accountInfoList[_account].matrix6Grade[_grade].upperLayer[1] = _2;
        accountInfoList[_account].matrix6Grade[_grade].lowerLayer[0] = _3;
        accountInfoList[_account].matrix6Grade[_grade].lowerLayer[1] = _4;
        accountInfoList[_account].matrix6Grade[_grade].lowerLayer[2] = _5;
        accountInfoList[_account].matrix6Grade[_grade].lowerLayer[3] = _6;
    }

    function matrix6Setup(address _account, uint256 _grade, address _vertex, address _upper, uint256 _amount) public onlyRole(INIT_ROLE) {
