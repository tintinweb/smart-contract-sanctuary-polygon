...
abstract contract BaseRouter is EIP712, EndPoint, AccessControlEnumerable {
    ...
    function _start(
        ...
        emit ComlexOpProcessed(uint64(block.chainid), 0, chainIdTo, nextRequestId, result, lastOp);
    }
    ...
    function _resume(
        ...
        emit ComlexOpProcessed(uint64(block.chainid), requestId, chainIdTo, nextRequestId, result, lastOp);
    }
    ...
    function _checkSignature(
        ...
        bytes32 accountantHash = keccak256(
            abi.encodePacked(
                keccak256(
                    "AccountantPermit(address from,uint256 nonce,bytes32 operationHash,bytes data,uint256 executionPrice,uint256 aggregationFee,uint256 deadline)"
                ),
                from,
                nonce,
                operationHash,
...
contract RouterV2 is BaseRouter, IRouter {
    ...
    function _lock(SynthParams memory p) internal returns (uint64 chainIdTo) {
        require(p.spType == SynthParamsType.LockMint, "Router: wrong sp type");
    ...
    function _unlock(SynthParams memory p) internal returns (uint256 amountOut) {
        require(p.spType == SynthParamsType.BurnUnlock, "Router: wrong sp type");
        address portal = IAddressBook(addressBook).portal(uint64(block.chainid));
        amountOut = IPortalV2(portal).unlock(currentRequestId, p.tokenIn, p.amountIn, p.from, p.to);
    }
    ...
    function _mint(SynthParams memory p) internal returns (uint256 amountOut) {
        ...
        amountOut = ISynthesisV2(synthesis).mint(currentRequestId, p.tokenIn, p.amountIn, p.to);
    }
...
interface IRouterParams {
    ...
    enum SynthParamsType { LockMint, BurnUnlock, BurnMint }
