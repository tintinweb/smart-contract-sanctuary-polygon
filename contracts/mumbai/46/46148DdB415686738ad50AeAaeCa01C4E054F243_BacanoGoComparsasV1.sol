...
contract BacanoGoComparsasV1 is BacanoGoCollection {
    ...
    struct ImageSpec {
        ...
        string image;
        string video;
        uint256 startRange; // 0 Means no range
        uint256 endRange; // 0 Means no range
        string[2][] attributes;
    }
    ...
    mapping(CardType => uint256) private mintCounters;
    ...
    function configureCharacters(
        CardType _cardType,
        uint32 _odds,
        ImageSpec[] memory _images
    ) external onlyOwnerOrAdmin {
        CardSpec storage cardSpec = cardSpecs[_cardType];
        cardSpec.odds = _odds;

        require(_images.length > 0, "Images req");
        require(
            cardSpec.images.length == 0 ||
                cardSpec.images.length == _images.length,
            "Img size"
        );

        // Reset the images
        delete cardSpec.images;

        // Add the new images
        for (uint256 i = 0; i < _images.length; i++) {
            cardSpec.images.push(_images[i]);
        }
    }
    ...
    {
        uint256 odds = cardSpecs[CardType.COMPARSA].odds +
            cardSpecs[CardType.BOXED].odds +
            cardSpecs[CardType.GOLDEN_STATUE].odds;
        ...
        for (uint32 i = 0; i < _quantity; i++) {
            ...
            CardType cardType;
            if (target <= cardSpecs[CardType.COMPARSA].odds) {
                cardType = CardType.COMPARSA;
            } else if (
                target <=
                (cardSpecs[CardType.BOXED].odds +
                    cardSpecs[CardType.GOLDEN_STATUE].odds)
            ) {
                cardType = CardType.BOXED;
            } else {
                cardType = CardType.GOLDEN_STATUE;
            }

            CardSpec storage cardSpec = cardSpecs[cardType];

            //Calculate mint number
            uint256 mintNumber = mintCounters[cardType] + 1;

            // Get the index of the character to mint
            uint256 imageIndex;
            for (uint256 j = 0; j < cardSpec.images.length; j++) {
                ImageSpec memory imageSpec = cardSpec.images[j];
                if (
                    (imageSpec.startRange == 0 && imageSpec.endRange == 0) ||
                    (mintNumber >= imageSpec.startRange &&
                        (mintNumber <= imageSpec.endRange ||
                            imageSpec.endRange == 0))
                ) {
                    imageIndex = j;
                    break;
                }
    ...
    function mint(address _recipient, uint32 _quantity) public onlyOwnerOrAdmin {
        ...
        returns (string memory)
    {
        ...
        CardSpec memory cardSpec = cardSpecs[tokens[tokenId].cardType];
        ImageSpec memory imageSpec = cardSpec.images[
            tokens[tokenId].imageIndex
        ];

        // Leading zeros for mint numbers below 100
        string memory leadingZeros = "";
        if (tokens[tokenId].mintNumber < 10) {
            leadingZeros = "00";
        } else if (tokens[tokenId].mintNumber < 100) {
            leadingZeros = "0";
        }

        string memory nameSuffix = "";
        if (tokens[tokenId].cardType == CardType.BOXED) {
            nameSuffix = "Box ";
        } else if (tokens[tokenId].cardType == CardType.GOLDEN_STATUE) {
            nameSuffix = "Gold ";
        }
        ...
        string memory name = string(
            abi.encodePacked(
                ...
                "#",
                leadingZeros,
                Strings.toString(tokens[tokenId].mintNumber)
            )
        );
        return
