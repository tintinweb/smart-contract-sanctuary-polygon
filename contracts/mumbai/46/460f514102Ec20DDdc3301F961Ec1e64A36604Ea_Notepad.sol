...
contract Notepad is Ownable {
    ...
    // uint256 public SUBSCRIPTION_TIME = 30 days;
    ...
    mapping(address => mapping(address => bool)) public myUsers;

    constructor(uint256 _SUBSCRIPTION_FEE, uint256 _SUBSCRIPTION_TIME) {
        SUBSCRIPTION_FEE = _SUBSCRIPTION_FEE;
        SUBSCRIPTION_TIME = _SUBSCRIPTION_TIME;
    }

    function addMyUsersList(address[] memory _user) public returns (bool) {
        for (uint i = 0; i < _user.length; i++) {
            if (myUsers[msg.sender][_user[i]] == false) {
                myUsers[msg.sender][_user[i]] = true;
            }
        }
        return true;
    }

    function userAllNotes(
        address _user
    ) public view returns (string[] memory, uint256) {
        if ((msg.sender == _user) || (myUsers[msg.sender][_user] == true)) {
            return (notepad[_user].note, notepad[_user].count);
        } else {
            require(false, "Not allowed");
            return (notepad[address(0)].note, notepad[address(0)].count);
        }
    }
    ...
    function write(string memory _text) public returns (bool) {
    ...
    function update(string memory _text, uint256 _index) public returns (bool) {
        require(user[msg.sender].exist, "User not registred");
        require(userSubsription(msg.sender), "Substription expired");
    ...
    function read(uint256 _index) public view returns (string memory) {
