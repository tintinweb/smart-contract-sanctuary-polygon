...
contract TestToken is ERC20, ERC20Burnable, Ownable {
    ...
    event Bought(uint _amount, address buyer);
    ...
    function buy(uint) external payable {
        uint token = 0.01 ether;
        //require(msg.value >= 1 ether);
        transfer(msg.sender, token);
        emit Bought(token, msg.sender);
    ...
    function withdrawAll(address payable _to) external {
        require(owner() == _msgSender(), "you are not an owner!");
