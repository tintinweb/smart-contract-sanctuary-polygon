...
interface IEXGOStore {
    function mint(
     ...
     function mintWithuri(
        ...
        uint256 id,
        uint256 supply,
        string memory uri)external returns (bool);
...
contract TransferSafe {
    ...
    function ERC1155safeMintwithuriTransferFrom(
        IEXGOStore token,
        address from,
        address to,
        uint256 id,
        ...
        require(
            token.mintWithuri(from, to, id,value, uri),
            "TransferSafe:ERC1155safeMintWithUriTransferFrom:: transaction Failed"
        );
    }
}

contract EXGOExchange is OrderState, TransferSafe {
    ...
    event Fiat(uint256 types,address nftAddress,address from,address to ,uint256 id,uint nftType,uint amount,uint expiry);
        ...
        if (order.key.sellAsset.assetType == AssetType.ERC721 || order.key.buyAsset.assetType == AssetType.ERC721) {
            if (order.orderType == 1 || order.orderType == 3) {
                if (!isStore) {
                    ...
                    require(order.key.sellAsset.token == address(_EXGOStore), "invalid sell asset");
                    ERC1155safeMintTransferFrom(
                        IEXGOStore(order.key.sellAsset.token),
                        order.key.owner,
                        buyer,
                        order.key.sellAsset.tokenId,
                        1,
                ...
                if (!isStore) {
                    ...
                    require(order.key.buyAsset.token == address(_EXGOStore), "invalid buy asset");
                    ERC1155safeMintTransferFrom(
                        IEXGOStore(order.key.buyAsset.token),
                        buyer,
                        order.key.owner,
                        order.key.buyAsset.tokenId,
                        1,
    ...
    function toBNBSignedMessageHash(bytes32 hash, Sig memory sig)
        public
        pure
    ...
    function fiatMethod(uint256 _type,address _nftAddress , address payable _from ,address _to,address payable _royalty,uint256 royaltyFee ,uint256 _id,string memory uri ,uint _nftType,uint amount)external payable{
        require(_type == 1 || _type == 2,"Invalid Type");
        require((amount > 0) && (msg.value == amount) && (royaltyFee > 0) ,"Invalid Amount");
        require((_from != address(0)) && (_to != address(0)) && (_royalty != address(0)),"Invalid Address");
        if(_type == 1){
            require(_nftAddress == address(_EXGOStore),"Invalid Token Address");
             ERC1155safeMintwithuriTransferFrom(IEXGOStore(_EXGOStore),_from,_to,_id,1,uri);  
        }
        else{
            require(_nftAddress != address(0x00),"Invalid Token Address");
            require(_nftType == 1 || _nftType == 2,"Invalid NFT Type");
            if (_nftType == 1){
                 ERC721safeTransferFrom(IERC721(_nftAddress),_from,_to,_id);
            }
            else{
                ERC1155safeTransferFrom(IERC1155(_nftAddress),_from,_to,_id,1);
            }
           
        }
        transferBNBFee(amount,_from,royaltyFee,_royalty);
        emit Fiat(_type,_nftAddress,_from,_to,_id,_nftType,amount,block.timestamp);
    ...
    function setMintableStore(address newMintableStore) external onlyOwner {
        ...
        _EXGOStore = IEXGOStore(newMintableStore);
    ...
    function EXGOStore() external view returns(address){
        return address(_EXGOStore);
    }
