...
contract PolygonNFT721Batch is ERC721URIStorage, ERC2771Context, Ownable {
    ...
    function mint(address user, uint256 from, uint256 to, string memory _uri, uint256 batchId) external {
        ...
        require( ((batchId * 100) - 99) <= from && from <= (batchId * 100), "Incorrect range or batchId!");
        require(totalMintedUnderBatchId[batchId] < from, "Range already minted");

        if(totalMintedUnderBatchId[batchId] == 0) {
            require(from == ((batchId * 100) - 99), "Incorrect starting range");
        } else {
            require(from == (((batchId * 100) - 100) + totalMintedUnderBatchId[batchId] + 1), "Incorrect starting range");
        }
