...
contract LOTTOGame is VRFConsumerBaseV2, ConfirmedOwner {
    ...
    uint256 public availableTimeToClaimJackpot = 5 minutes;
     uint256 public gameSpan = 5 minutes;
    ...
    event ClaimDataEvent(
        uint256 nftId,
        uint256 gameId,
        uint256 matchedNumber,
        uint256 reward,
    ...
    function getCurrentGameId() public view returns (uint256) {
        return (block.timestamp / gameSpan - gameStartTime / gameSpan) + 1;
    }
    ...
    function settleBet(uint256 gameId,uint256 _nftId) public {
        require(bets[gameId][_nftId].gameId > 0, "Invalid game");
        ...
        if(counter<6){
            ...
            IERC20(busd).transfer(msg.sender, hitsAndRewards[counter]);

            emit EarningEvent(hitsAndRewards[counter], msg.sender, address(0),0, block.timestamp);
            }
            emit ClaimDataEvent(
                _nftId,
                gameId,
    ...
    function claimJackpot(uint256 gameId,uint256 _nftId) external
    {
        ...
        emit EarningEvent(hitsAndRewards[6], msg.sender, address(0),0, block.timestamp);
             emit JackpotClaimed(
        msg.sender,
        amount,
        gameId,
    ...
    function declareResult() internal {
        ...
        if (gameResult[_gameId].status == 1) {
            if (
                ...
                (block.timestamp /
                    gameSpan -
                    (gameStartTime + (_gameId * gameSpan) + 1))
            ) {
    ...
    function getResult(uint256 _gameId) external {
        ...
        require(gameResult[_gameId].status == 1 || gameResult[_gameId].status==4 || gameResult[_gameId].status == 3,"In process");
        if (gameResult[_gameId].status == 1 || gameResult[_gameId].status==4) {
            require(
    ...
    function fulfillRandomWords(
        ...
        for (uint256 _index = 0; _index < 6; _index++) {
            gameResult[gameId].randomWords.push((_randomWords[_index] % 70) + 1);
        }
         bool checkDuplicate;
    ...
    function getResultDetails() external view returns(uint256 gameId,uint8 status)
    {
        return (lastActiveGameId,gameResult[lastActiveGameId].status);
    }
}
