...
contract CLLUserManagement is CLStorage {
    /// @notice Add a new user to CLH
    /// @param _walletAddr Wallet of the new user
    function AddUser(
        address _walletAddr,
        ...
        public
    {
        ...
        uint256 uid = arrUsers.length;
        arrUsers.push( _walletAddr );
        numUsers++;

        if( _isManager ){
            numManagers++;
            try pxyNFTManager.safeMint( _walletAddr ) {}
            catch Error(string memory reason) {
                // revert( reason );
            }
        }
        else {
            pxyNFTMember.safeMint(
                _walletAddr
            );
        }
        
        mapUsers[ _walletAddr ] =  strUser(
            {
                userID: arrUsers.length-1,
                nickname: _nickname,
                isManager: _isManager
            }
        );
                
        emit evtUser( userEvent.addUser, _walletAddr, _nickname );
    }


    /// @notice Remove a user from CLH
        ...
        external
    {
        ...
        if( mapUsers[ _walletAddr ].isManager ){
            pxyNFTManager.burn(
                _walletAddr
            );
            numManagers--;
        ...
        string memory nickname = mapUsers[ _walletAddr ].nickname;

        delete arrUsers[ mapUsers[ _walletAddr ].userID ];
        delete mapUsers[ _walletAddr ];
        numUsers--;

        emit evtUser( userEvent.delUser, _walletAddr, nickname );
    }


    /// @notice Generate a invitation
    ...
    function InviteUser(
        ...
        external
    {
        pxyNFTInvitation.safeMint( _walletAddr );
        ...
        external
    {
        ...
        if( _signature.length == 65 ) {
            ...
            realSender = pxyApiCLH.SignerOCInvit(
                _acceptance,
                _nickname,
                address(this),
                _signature
        ...
        uint256 nid = pxyNFTInvitation.mapCLHUSRNFTID( address(this), realSender );

        // check Internal Invitation OR whiteListNFT Invitation
        if( 
            0 != nid ||
            (
                address(0) != whiteListNFT &&
                0 != IERC721( whiteListNFT ).balanceOf( realSender )
            )
        ) {
            if ( 0 != nid )
                pxyNFTInvitation.burn( realSender );
            ...
            revert( "You Don't have a NFT invitation" );
    }
}
...
abstract contract CLStorage {
    ...
    event evtChangeHouseName( string newHouseName );
    ...
    function CheckECDSA( address _walletAddr ) internal view {
