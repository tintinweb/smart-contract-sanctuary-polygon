...
contract SoularSearching is ERC721Enumerable, Ownable {
    using SafeMath for uint256;
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdTracker;

    uint256 public constant MAX_ELEMENTS = 2;
    uint256 public PRICE = 3705 * 10**18; // 3705 MATIC
    uint256 public SALE_START = 1652572800; //15 May 2022 00:00:00 UTC

    bool public isPaused = false;

    string public baseTokenURI;

    event CreateSoularSearchingNFT(uint256 indexed id);

    constructor(string memory baseURI) ERC721("Soular Searching Poetry Series", "SSP") {
        setBaseURI(baseURI);
    ...
    function mint(uint256 _count) public payable saleIsOpen {
        ...
        require(total <= MAX_ELEMENTS, "All Soular Searching NFT's are sold out");
    ...
    function _mintAnElement(address _to) private {
        ...
        _tokenIdTracker.increment();
        _safeMint(_to, id);
        emit CreateSoularSearchingNFT(id);
    }
