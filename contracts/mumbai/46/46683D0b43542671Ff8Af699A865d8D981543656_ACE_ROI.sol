...
contract ACE_ROI{
    ...
    function calcReward(address _staker, uint256 _plan) public view returns(uint256) {
        ...
        if (investers[msg.sender].plan>0) {
    ...
    function calNoOfDays() public view returns(uint256) {
        uint256 noOfDays;
        if (investers[msg.sender].time > 0)
        {
        uint256 noOfSeconds = block.timestamp.sub(investers[msg.sender].time);
    ...
    function holdBonus() public view returns(uint256) {
        uint256 noOfDays = calNoOfDays();
        uint256 bonus = noOfDays.mul(investers[msg.sender].bonusChk.div(1000).mul(1));
