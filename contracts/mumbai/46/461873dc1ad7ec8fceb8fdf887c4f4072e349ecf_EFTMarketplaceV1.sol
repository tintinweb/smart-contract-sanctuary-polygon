...
contract EFTMarketplaceV1 is  Initializable, ERC721Holder, OwnableUpgradeable {
 ...
 address public couponSigner;
   ...
   */
  function initialize (address _card, address payable _daoAddress, uint8 _minOfferGuaranteeHours) public initializer{
...
/**
  ...
  function _transferFees(uint256 _feeValue, address _collectionAddress, address _collectionPartnerDaoAddress) internal {
    ...
    if (partnerMarketplaceFeePercent > 0) {
        ...
        feeValue -= partnerMarketplaceFeePercentValue;
      }
  ...
  function _getSellerFeeWithDiscount(uint256 _amount, uint8 _collectionId, Coupon memory _coupon, uint _discount) internal view returns (uint marketplaceFee, uint256 marketplaceFeeValue, uint discount){
    ...
    if(_discount > 0){
      marketplaceFeeValue = marketplaceFeeValue - (marketplaceFeeValue * _discount / 100);
    }
  ...
  */
  function cancelSellOrder(bytes32 _orderId) external sellOrderExists(_orderId) senderOwnsSellOrder(_orderId) {
    ...
    if(sellOrder.highestOfferor != address(0) && !sellOrder.highestOfferor.isContract()) {
      // Refund offeror
        bool success = eft.transfer(sellOrder.highestOfferor, sellOrder.highestOffer);
 ...
 * @notice Cancelles an offer on a sell order
  ...
  */
  function acceptSellOrderHighestOffer(bytes32 _orderId) external sellOrderExists(_orderId) senderOwnsSellOrder(_orderId) {
    ...
    uint256 marketplaceFeeValue = sellOrder.highestOffer * sellOrder.marketPlaceFee / 100_000;
...
function fulFillSellOrder(bytes32 _orderId, uint256 _value, Coupon memory _coupon, uint _discount) external onlyIfSenderNotContract sellOrderExists(_orderId) {
    ...
    // Ensure Ether sent is equal to sell order ask value with fee
 ...
 * @notice Cancelles an offer on a buy order
   ...
   */
  function fulfilBuyOrder(bytes32 _orderId, uint256 _cardId, Coupon memory _coupon, uint _discount) external onlyIfSenderNotContract buyOrderExists(_orderId) {
    // Copy buy order in memory
    BuyOrder memory buyOrder = buyOrders[_orderId];
