...
contract Token is Context, IERC20, IERC20Metadata, Ownable {
    ...
    function transferFrom(address from, address to, uint256 amount) public virtual override returns (bool) {
        address spender = _msgSender();
        _approve(from, spender, _allowances[from][spender].sub(amount, "ERC20: transfer amount exceeds allowance"));
        _transfer(from, to, amount);
        return true;
    }
    ...
    function _mint(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");
        require(_totalSupply.add(amount) <= MAXSupply, "ERC20: total supply exceeds maximum supply");
