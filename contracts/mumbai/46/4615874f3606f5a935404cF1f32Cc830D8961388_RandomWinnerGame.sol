...
contract RandomWinnerGame is VRFConsumerBase, Ownable {
    ...
    // Each game should have a max no.of players and entry fee

    // chainlink vars
    // fee to be paid to chainlink oracle
    uint256 public fee;
    // ID of the public key against which the random number is generated
    bytes32 public keyHash;
    // to store the player's addresses
    address[] public players;
    // max no.of players allowed per game
    ...
    // events
    // to emit when a new game is started
    event GameStarted(uint256 gameId, uint256 maxPlayers, uint256 entryFee);
    // to emit when a new player joins the game
    event PlayerJoined(uint256 gameId, address player);
    // to emit when the game ends - i.e., the winner is chosen
    ...
    /**
     * @dev to start a new game
     * @param _maxPlayers max no.of players allowed per game
     * @param _entryFee entry fee for each player
     */
    ...
    /**
     * fulfillRandomness is an internal function of VRFConsumerBase
     * @dev to choose a random winner
     * @param requestId ID of the request
     * @param randomness random number generated by chainlink oracle
     */
