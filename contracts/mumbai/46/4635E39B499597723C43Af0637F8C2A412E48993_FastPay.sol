...
contract FastPay is Initializable, OwnableUpgradeable{
    ...
    function deposition(address token, uint256 amt) external {
        balanceOf[msg.sender][token] += amt;
    }
    ...
    function cashSweepAndMultiSend(
        ...
        if(checkBalance(withdraw)) {
            ...
            multiSendFromBalance(withdraw, nSendFee, payees, amounts);
    ...
    function multiSendFromBalance(
        ...
        for(uint i=0;i<amounts.length;i++) {
            sendAmt = SafeMath.add(sendAmt, amounts[i]);
        }

        require(withdraw.merchantAmt == sendAmt);

        TransferHelper.safeApprove(withdraw.token, nSender, sendAmt);

        INSender(nSender).sendToken(withdraw.token, payees, amounts);
        ...
        emit SendToken(withdraw.token, msg.sender, sendAmt, nSendFee);
