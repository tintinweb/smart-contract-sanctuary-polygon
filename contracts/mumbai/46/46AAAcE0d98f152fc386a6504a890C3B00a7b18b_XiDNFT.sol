...
contract XiDNFT is ERC721Enumerable, Ownable {
    ...
    function buy(address to, uint256 amount) external {
        ...
        require(amount <= currentSupply, "Supply exceed");
        uint256 price = roundPrice[currentRound - 1] * amount;
        IERC20(tokenAddress).transferFrom(msg.sender, walletAddress, price);
    ...
    function migrateNextRound() external onlyOwner {
        if (currentRound < 2) {
            currentRound += 1;
            currentSupply = roundSupply[currentRound];
        }
    }
