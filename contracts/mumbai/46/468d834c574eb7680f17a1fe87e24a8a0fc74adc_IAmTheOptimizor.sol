...
contract IAmTheOptimizor is ERC721, Ownable {
    ...
    constructor() ERC721("I Am The Optimizor", "IAMTHEOPTIMIZOR") {
        ...
        lowestPoints = LowestPoints(address(0), type(uint64).max, type(uint64).max, type(uint64).max);
    ...
    function becomeTheOptimizor(address player) external {
        ...
        unchecked {
            ...
            uint256 seed = uint256(keccak256(abi.encodePacked(block.coinbase, block.number, block.timestamp, size)));
            ...
            uint256[3] memory indexesFor3Sum = constructIndexesFor3Sum(seed >> (BIT_MASK_LENGTH * 10));
            ...
            uint256[3] memory playerAnswerIndexes = IPlayer(player).solve(inputArr, desiredSum);
            ...
            require(gasUsed + size < currLowestPoints.totalScore, "not optimized enough");
            ...
            lowestPoints = LowestPoints(msg.sender, uint64(size), uint64(gasUsed), uint64(size + gasUsed));
        }
    }
    ...
    function constructProblemArray(uint256 seed) public pure returns (uint256[10] memory arr) {
        unchecked {
    ...
    function constructIndexesFor3Sum(uint256 seed) public pure returns (uint256[3] memory arr) {
        unchecked {
...
interface IPlayer {
    ...
    function solve(uint256[10] calldata inputArr, uint256 n) external returns (uint256[3] calldata);
}
