...
contract RuleManagerFacet is IdentityVerifier {
    ...
    event RuleAdded(uint256 indexed mnft, uint256 ruleNum);
    event RuleDeleted(uint256 indexed mnft, uint256 ruleNum);
    event RuleAssigned(uint256 indexed mnft, uint256 operation, uint256 ruleNum);

    function addRule(
        uint256 mnft,
        VerifierCredentials[] calldata credentials,
        AccessRule calldata rule
    ) external returns (uint256 ruleNum) {
        verifyAccess(mnft, IRules.Operation.MANAGE_RULES, credentials, msg.sender);
        ruleNum = _addRule(mnft, rule);
        emit RuleAdded(mnft, ruleNum);
    }
    ...
    function deleteRule(
        ...
        verifyAccess(mnft, IRules.Operation.MANAGE_RULES, credentials, msg.sender);
        _deleteRule(mnft, ruleNum);
        emit RuleDeleted(mnft, ruleNum);
    }
    ...
    function assignRule(
        ...
        require(ruleNum < ud.rules.length, "wrong ruleNum");
        ud.operations[op] = ruleNum;
        emit RuleAssigned(mnft, uint256(op), ruleNum);
    }

    function _addRule(uint256 mnft, AccessRule calldata rule) private returns (uint256 ruleNum) {
        IdentityStorage.RuleData storage ud = IdentityStorage.layout().ruleData[mnft];
        if (ud.rules.length == 0) ud.rules.push(); //initialize zero element
        ruleNum = ud.rules.length;
        ud.rules.push(rule);
    }

    function _deleteRule(uint256 mnft, uint256 ruleNum) internal {
        require(ruleNum > 0, "rule 0 can  not be deleted");
        IdentityStorage.RuleData storage ud = IdentityStorage.layout().ruleData[mnft];
        require(ruleNum < ud.rules.length, "rule does not exist");

        //TODO Verify this rule is not required for MANAGE_RULES

        ud.rules[ruleNum].required = 0;
        delete ud.rules[ruleNum].identities;
    }
}
