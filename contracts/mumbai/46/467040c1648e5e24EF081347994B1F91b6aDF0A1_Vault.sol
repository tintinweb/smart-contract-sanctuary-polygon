...
contract Vault is Ownable, Initializable {
  ...
  event FundAdded(uint256 value, uint256 newFund);
  event FundRemoved(uint256 value, uint256 newFund);
  event ProfitClaimed(address to, uint256 value);
  ...
  function initialize(address payable _operator, address payable _developer) public onlyInitializing  {
    operator = _operator;
  ...
  function addFund() external payable onlyOwner {
    fund = fund + msg.value;
    emit FundAdded(msg.value, fund);
  }

  function removeFund(uint256 value) external onlyOwner {
      fund = fund - value;
      payable(msg.sender).transfer(value);
      emit FundRemoved(value, fund);
  }

  receive() external payable {}

  function sendWinner(address payable _to, uint256 _amount) public {
  ...
  function claim() public onlyOwner {
    ...
    emit ProfitClaimed(developer, devShare);
    emit ProfitClaimed(operator, opShare);
  }
