...
contract BlackAlpha is DataStorage {
    ...
    function register(string memory _username, string memory _referral) public {
        address userAddr = msg.sender;
        address refAddr = nameToAddr[_referral];

        uint256 _entrance = checkCanRegister(_username, userAddr, refAddr);
        _newNode(userAddr, refAddr, _entrance);
        _newUsername(_username, userAddr);

        uint256 remaining = _payDirect(userAddr, refAddr, _entrance);
        DAI.transferFrom(userAddr, address(this), remaining);
        _payBinary(userAddr, refAddr, _entrance);
    }
    ...
    function checkCanRegister(
        ...
        address userAddr, 
        address upAddr
    ) internal view returns(uint256 entrance) {
    ...
    function checkCanUpgrade(address userAddr) internal view returns(uint256 entrance) {
        ...
        return deficitToUpgrade(userAddr);
    }

    function _newUsername(string memory username, address userAddr) internal {
    ...
    function changeUsername(string memory newUsername) public {
        address userAddr = msg.sender;
        string memory oldUsername = addrToName[userAddr];
        require(
            bytes(addrToName[userAddr]).length != 0, 
            "BlackAlpha: userAddr not existing"
        );
        require(
            nameToAddr[newUsername] == address(0), 
            "BlackAlpha: username is taken by another user"
        );
        delete nameToAddr[oldUsername];
        nameToAddr[newUsername] = userAddr;
        addrToName[userAddr] = newUsername;
    }
    ...
    function withdraw(uint256 amount) public {
        ...
        require(
            amount <= pendingWithdrawal(userAddr),
            "BlackAlpha: insufficient amount"
        );
        user.paidOut += amount;
        DAI.transfer(userAddr, amount);
    }
}
...
abstract contract DataStorage {
    ...
    mapping(address => mapping(uint256 => uint256)) userTodayEarning;
    mapping(address => mapping(uint256 => uint256)) userTodayFlash;

    function dayCounter() internal view returns(uint256) {
    ...
    function userDashboard(string memory username) public view returns(
        uint16 depth,
        uint256 entrance,
        uint256 paidOut,
        uint256 pending,
        uint256 upgradeDeficit,
        uint256 todayEarning,
        uint256 todayFlash,
        uint256 leftPay,
        uint256 rightPay,
        uint256 leftUsers,
        ...
        upgradeDeficit = deficitToUpgrade(userAddr);
        todayEarning = userTodayEarning[userAddr][dayCount];
        todayFlash = userTodayFlash[userAddr][dayCount];
        leftPay = user.leftPay;
        rightPay = user.rightPay;
        leftUsers = user.leftUsers;
        rightUsers = user.rightUsers;
        status = userStatus(userAddr);
    }
    ...
    function deficitToUpgrade(address userAddr) internal view returns(uint256 amount) {
        uint256 upgradeAmount = users[userAddr].entrance * 2;
        uint256 pendingAmount = pendingWithdrawal(userAddr);
        amount = pendingAmount > upgradeAmount ? 0 : (upgradeAmount - pendingAmount);
    }

    function userStatus(address userAddr) internal view returns(string memory ans) {
        User storage user = users[userAddr];
        ...
        if(user.entrance == 0) {
            ...
            ans = string.concat("Upgrade Respite:", (block.timestamp - user.upgradeTime).toString());
        } else {
            ans = "Terminated User";
        }
    }
    ...
    function userTree(string calldata username, uint256 len) public view returns(string[] memory temp) {
        ...
        uint256 j = 1;
        while(j < len) {
            addrs[j] = users[addrs[i]].leftAddr;
            addrs[j + 1] = users[addrs[i]].rightAddr;
            i++;
            j += 2;
        }
    ...
    function autoReferral(string memory username) public view returns(string memory _referral) {
        address userAddr = nameToAddr[username];
        if(users[userAddr].childsCount < 2) {
            return username;
        } else {
            address refAddr;
            if(users[userAddr].leftPay > users[userAddr].rightPay) {
                refAddr = users[userAddr].rightAddr;
            } else {
                refAddr = users[userAddr].leftAddr;
            }
            while(users[refAddr].childsCount != 0) {
                if(users[refAddr].leftPay >= users[refAddr].rightPay) {
                    refAddr = users[refAddr].leftAddr;
                } else {
                    refAddr = users[refAddr].rightAddr;
                }
            }
            return addrToName[refAddr];
        }
    }
