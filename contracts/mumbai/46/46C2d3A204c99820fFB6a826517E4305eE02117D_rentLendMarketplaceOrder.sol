...
interface IRentLendMarketplace {
    ...
    function delistItemsFromRent(uint256 _listingID, uint256 _tokenQuantity)
        external;
...
contract rentLendMarketplaceOrder is
    Ownable,
    ReentrancyGuard,
    IRentLendMarketplace
{
    //mapping(address => RentersArrayStruct) renterTempMapping;
    ...
    modifier notListedForRent(
        ...
        if (_nftStandard == NFTStandard.E721) {
            ...
            uint256 totalListedQty = addressToTokenIdToListedQty[_nftAddress][
                _tokenId
            ][msg.sender];

            require(totalListedQty == 0, "Item already listed ");
            ...
            require(
                ownerAmount >= totalListedQty + _tokenQuantity,
    ...
    // function notAlreadyRentedSeller(
    ...
    {
        console.log("item listed started");
        if (_price <= 0) {
        ...
        tokenSellerStorage.orderSts = OrderStatus.ACTIVE;

        listingIDtoTokenSeller[orderCtr] = tokenSellerStorage;
        activelistingKeys.push(orderCtr);

        addressToTokenIdToListedQty[_nftAddress][_tokenId][
            msg.sender
        ] += _tokenQuantity;
    ...
    function updateListedItemForRent(
        ...
        // isListedForRent(
        //     allNftContractListings[tokenSellerStorage._nftAddress][
        //         tokenSellerStorage._tokenId
        //     ],
        //     tokenSellerStorage._tokenQuantity
        // );
        ...
        } else if (tokenSellerStorage.nftStandard == NFTStandard.E1155) {
            IERC1155 nft = IERC1155(tokenSellerStorage.nftAddress);
            uint256 ownerAmount = nft.balanceOf(
                msg.sender,
        ...
        external
    {
        ...
        if (tokenSellerStorage.tokenQuantity - _tokenQuantity == 0) {
            ...
            tokenSellerStorage.orderSts = OrderStatus.INACTIVE;
        } else {
            tokenSellerStorage.tokenQuantity =
                tokenSellerStorage.tokenQuantity -
                _tokenQuantity;
    ...
    function rentItem(
        ...
        if (_duration > tokenSellerStorage.maxRentDuration) {
            console.log("duration failed", _duration);
            console.log(
                "duration by listing",
                tokenSellerStorage.maxRentDuration
            );
        ...
        console.log("value of ", msg.value);
        console.log("price per day", tokenSellerStorage.pricePerDay);
        console.log("max duration ", tokenSellerStorage.maxRentDuration);
        ...
        //we should add duration and start timeStamp
        emit ItemRented(
            _listingID,
            renterCtr,
            msg.sender,
    ...
    function updateRentItemStorage(
        ...
        renterDetails.rentStatus = OrderStatus.RENTED;
    ...
    function returnNftFromRent(
        ...
        if (renterDetails.tokenQuantityRented == 0) {
            renterDetails.rentStatus = OrderStatus.RETURNED;
        }
        ...
        console.log("length before returning", renterStructKeys.length);
        removeRenterListing(tokenSellerStorage.renterKeyArray, _rentingID);
        removeRenterFromArray(_rentingID);

        console.log("length after returning", renterStructKeys.length);
    }
        ...
        returns (uint256[] memory, uint256)
    {
        ...
        console.log("renters struct length", renterStructKeys.length);
        uint256 k = 0;
        for (uint256 i = 0; i < renterStructKeys.length; i++) {
            if (
                block.timestamp >=
    ...
    function updateRentersDetails(uint256[] memory _rentingIDs) internal {
        ...
        console.log("start", renterStructKeys.length);
        ...
        for (uint256 i = 0; i < _rentingIDs.length; i++) {
            ...
            console.log("renting ID to be removed", _rentingIDs[i]);
            removeRenterListing(
        ...
        console.log("later", renterStructKeys.length);
    }
