...
contract SmashAttack is ERC1155LazyMint {
    struct PowerUp {
        uint256 id;
        uint256 quantity;
    ...
    event Defended(address indexed attacker, address indexed victim);
    event UsedDefensePowerUp(address indexed account, uint256 remainingUses);

    bool public isGamePaused = false;
    uint256 public constant DEFENSE_USES = 2; // Number of attacks defense power-up can withstand

    mapping(address => PowerUp[]) public powerUps;
    mapping(address => uint256) public defenses;

    // Token IDs
    uint256 private constant POWER_UP_ID = 4;
    uint256 private constant DEFENSE_ID = 5;
    ...
    /**
     * @notice Claim a stick to start playing, but only if you don't already own a weapon
     */
    function claimStick() external {
        claim(msg.sender, 0, 1);
        // claiming a Stick enters the game at level 1
    ...
    function verifyClaim(
        ...
        require(_tokenId == 0, "Only Wooden Sticks can be claimed");
        require(_quantity == 1, "Only one Wooden Stick at a time!");
        require(balanceOf[_claimer][0] == 0, "Already got a Wooden Stick");
        require(balanceOf[_claimer][1] == 0, "Already got a Stone Hammer ");
        require(balanceOf[_claimer][2] == 0, "Already got a Iron Sword");
    }

    /**
     * @notice Transfer weapons to level up
     */
    function safeTransferFrom(
        address from,
        address to,
        ...
        // can only transfer sticks
        require(id == 0, "This weapon is not transferable!");
        super.safeTransferFrom(from, to, id, amount, data);
        if (from != to && id == 0) {
            // transferring level 1 NFT gives you a level 2 NFT
            _mint(from, 1, 1, "");
    ...
    /**
     * @notice Burn a weapon to either level up or attack another Aetherial
     */
    function burn(
        address account,
        uint256 id,
        ...
        // the owner can burn their NFT, but any Iron Sword owner can also burn anyone else's NFT
        require(
            msg.sender == account || balanceOf[msg.sender][2] > 0,
            "NOT_TOKEN_OWNER or Iron Sword"
        );
    ...
    function _attack(address attacker, address victim) internal {
        ...
        // Check if the victim has an active defense power-up
        if (defenses[victim] > 0) {
            // Defense power-up is active, reduce the remaining uses
            defenses[victim]--;

            if (balanceOf[attacker][POWER_UP_ID] > 0) {
                // Attacked by an attack power-up, defense power-up withstands one attack
                emit Smashed(attacker, victim, 0);
                emit Defended(attacker, victim);
                // Burn the defense and attack power-ups
                _burn(victim, DEFENSE_ID, 1);
                _burn(attacker, POWER_UP_ID, 1);
            } else {
                // Regular attack, defense power-up withstands two attacks
                uint256 highestLevelWeapon;
                if (defenses[victim] == 0) {
                    // Defense power-up has exhausted its uses, burn the highest level weapon of the victim
                    _burn(victim, DEFENSE_ID, 1);
                    _mint(attacker, 3, 1, "");

                    // Check if the attacker has enough NFTs with tokenId 3 to claim a power-up
                    uint256 numNFTs = balanceOf[attacker][3];
                    if (numNFTs % 5 == 0) {
                        ...
                        if (randomNumber == 0) {
                            // Claim an attack power-up
                            _mint(attacker, POWER_UP_ID, 1, "");
                        } else {
                            // Claim a defense power-up
                ...
                if (defenses[victim] > 0) {
                    // Defense power-up successfully defended the attack
                    emit Defended(attacker, victim);
            ...
            // No defense power-up, regular attack behavior
            uint256 highestLevelWeapon;
    ...
    function attack(address attacker, address victim) external gameNotPaused {
        require(balanceOf[attacker][2] > 0, "You need a Iron Sword to attack!");
        require(victim != address(0), "Invalid victim address");

        _attack(attacker, victim);
    }

    /**
     * @notice Use an attack power-up to attack multiple players
     * @dev The power-up is burned after all attacks are completed
     * @param victims An array of victim addresses
     */
    function useAttackPowerUp(address[] calldata victims)
        external
        gameNotPaused
    {
        require(
            balanceOf[msg.sender][POWER_UP_ID] > 0,
            "You need an Attack power-up to perform this action"
        );
        require(victims.length > 0, "No victims specified");

        for (uint256 i = 0; i < victims.length; i++) {
            address victim = victims[i];
            require(victim != address(0), "Invalid victim address");

            _attack(msg.sender, victim);
        }

        // Burn the attack power-up after all attacks are completed
        _burn(msg.sender, POWER_UP_ID, 1);
    }

    function useDefensePowerUp() external gameNotPaused {
        require(
            balanceOf[msg.sender][DEFENSE_ID] > 0,
            "You need a Defense power-up to perform this action"
        );

        if (defenses[msg.sender] == 0) {
            // Set the initial remaining uses for the defense power-up
            defenses[msg.sender] = DEFENSE_USES;
        }
        ...
        if (defenses[msg.sender] == 0) {
            // Burn the defense power-up if all uses are exhausted
            _burn(msg.sender, DEFENSE_ID, 1);
        }
    }

    function getScore(address player) public view returns (uint256) {
        // Wooden Stick: 1 point
        // Stone Hammer: 2 points
        // Iron Sword: 3 points
