...
contract ClaimTeamAndMarketingBzai is Ownable, ReentrancyGuard {
    ...
    uint256[6] _teamVestingTimestamp;
    uint256[6] _marketingVestingTimestamp;
    ...
    function setTgeTimestamp(uint256 _timestamp) external onlyOwner {
        ...
        _marketingVestingTimestamp[0] = _timestamp; 
        _teamVestingTimestamp[0] = _timestamp + (toAdd * 6); 


        for(uint256 i = 1 ; i < 6 ;){
            _marketingVestingTimestamp[i] = _marketingVestingTimestamp[0] + (i * toAdd);
            _teamVestingTimestamp[i] = _teamVestingTimestamp[0] + (i * toAdd);
            unchecked {++i ;}
        }
    ...
    function setTeamVesting(address _user, uint256 _amount) external onlyOwner {
        require(BZAI.balanceOf(address(this)) - attributed - _amount > 0, "To much attributed");

        for(uint256 i = 0 ; i < 6 ; ){
            if(_teamVestingAmount[_user][i] > 0){
                attributed -= _teamVestingAmount[_user][i];
    ...
    function setMarketingVesting(address _user, uint256 _amount) external onlyOwner {
        ...
        for(uint256 i = 0 ; i < 6 ; ){
            if(_marketingVestingAmount[_user][i] > 0){
                attributed -= _marketingVestingAmount[_user][i];
    ...
    function _getMarketingClaimable(address _user) internal view returns(uint256){
        uint256 _claimable;
        for (uint256 i = 0 ; i < 6 ; ){
            if(block.timestamp >= _marketingVestingTimestamp[i]){
    ...
    function _getTeamClaimable(address _user) internal view returns(uint256){
        uint256 _claimable;
        for (uint256 i = 0 ; i < 6 ; ){
            if(block.timestamp >= _teamVestingTimestamp[i]){
    ...
    function claimMarketingBZAIs() external nonReentrant returns(bool){
    ...
    function claimTeamBZAIs() external nonReentrant returns(bool){
        require(tgeUnlockedTimestamp != 0,"tge not set");
        require(block.timestamp >= tgeUnlockedTimestamp,"Too soon !");
