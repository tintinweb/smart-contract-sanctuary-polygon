// Same Image
// Each event will be a new image
// ERC721 - Physicall attended an event. Ticket for something
...
contract OctoHeadzPOAP721 is Ownable, ERC721A, Whitelistable {
    string public baseIPFS;
    string public baseHashURI;
    uint256 currentEvent = 0;
    ...
    constructor(string memory _firstEvent, address _whitelistAdmin) 
        ERC721A("TestPAO","testPAO")  
        Whitelistable(_whitelistAdmin) {
        baseIPFS = "https://ipfs.io/ipfs/";
        events.push(_firstEvent);
    }
    ...
    function tokenURI(uint256 _tokenID) override public view returns (string memory) {
        return string(abi.encodePacked(baseIPFS, baseHashURI, "/", Strings.toString(_tokenID), ".json"));
    }

    function addEvent(string memory _newEvent) public onlyOwner {
        require(!eventExists(_newEvent), "Event already exists");
        events.push(_newEvent);
    }

    function eventExists(string memory _newEvent) public view returns (bool) {
        for (uint i = 0; i < events.length; i++) {
            if (keccak256(abi.encodePacked(events[i])) == keccak256(abi.encodePacked(_newEvent))) {
                return true;
            }
        }
        return false;
    ...
    function getEventIndex(string memory _newEvent) public view returns (int) {
        for (uint i = 0; i < events.length; i++) {
            if (keccak256(abi.encodePacked(events[i])) == keccak256(abi.encodePacked(_newEvent))) {
                return int(i);
            }
    ...
    function mintPOAP(uint256 _numNFTz) public {
        _safeMint(msg.sender, _numNFTz);
    }

    function airdropPOAP(address[] memory addresses, uint256 _numNFTz) public onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            _safeMint(addresses[i], _numNFTz);
        }
    }
}
