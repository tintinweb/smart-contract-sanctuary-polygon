...
contract NFTBulkMinter is Ownable {
    TangibleTokenSwap private immutable tokenSwap;
    PassiveIncomeNFTSwap private immutable nftSwap;
    ...
    constructor(
        ...
        tokenSwap = TangibleTokenSwap(tokenSwapAddress);
        nftSwap = PassiveIncomeNFTSwap(nftSwapAddress);
        ...
        onlyOwner
    {
        ...
        guru.approve(address(nftSwap), guru.balanceOf(address(this)));
        uint256 len = addresses.length;
        for (uint256 i = 0; i < len; i++) {
            uint256 tokenId = nftSwap.swapGURU(amounts[i], 48, false, true);
            nft.safeTransferFrom(address(this), addresses[i], tokenId);
        }
    }

    function withdrawGURU() external onlyOwner {
        IERC20 guru = IERC20(tokenSwap.GURU());
        guru.transfer(msg.sender, guru.balanceOf(address(this)));
    }
}
