...
contract NurseryManagement is Nursery {
    ...
    constructor(address _BZAI, uint256 _preMint) Nursery(_preMint)  {
        BZAI = IERC20(_BZAI);
    }
    ...
    function _updateNextMint(uint256 _tokenId, uint256 state) internal {
        ...
        if(temp.bronzeMinted < 5){
            temp.bronzeMinted = temp.bronzeMinted + 1;
            total.bronzeMinted = total.bronzeMinted + 1;
            if(temp.bronzeMinted == 5){
                _next = 1;
            ...
            temp.silverMinted = temp.silverMinted + 1;
            total.silverMinted = total.silverMinted + 1;
            temp.bronzeMinted = 0;

        }else if(temp.goldMinted < 5){
            temp.goldMinted = temp.goldMinted + 1;
            total.goldMinted = total.goldMinted + 1;
            temp.bronzeMinted = 0;
            temp.silverMinted = 0;
        }else {
            temp.platinumMinted = temp.platinumMinted + 1;
            total.platinumMinted = total.platinumMinted + 1;
            temp.bronzeMinted = 0;
            temp.silverMinted = 0;
            temp.goldMinted = 0;
        }
    ...
    function setPrice(
        ...
        if(_state == 0){
           (uint256 _ticketPrice, , ,) = ILottery(gameAddresses.getLotteryAddress()).getLotteryPrices();
           require(price >= _ticketPrice / 2,"Bronze egg can't be lower than 1/2 lottery ticket price");
        }
    ...
    function offerNextEgg(uint256 _tokenId, address _to) external returns(uint256) {
        require(msg.sender == ownerOf(_tokenId), "only nursery owner can reserve next egg");
        require(lastTimeReserveEgg[_tokenId] > block.timestamp + 1 days,"You can offer only one egg by day");
        lastTimeReserveEgg[_tokenId] = block.timestamp;
        uint256 state = _nextStateToMint[_tokenId];
        _updateNextMint(_tokenId,state);
        return IEggs(gameAddresses.getEggsAddress()).mintEgg(_to, state, block.timestamp + maturities[state] + 1 days);
    }
...
contract Nursery is ERC721Enumerable, Ownable {
    ...
    constructor(uint256 _preMint) ERC721("Nursery_NFT", "NURS") {
        ...
        returns (uint256)
    {
        ...
        (uint256 _ticketPrice, , ,) = ILottery(gameAddresses.getLotteryAddress()).getLotteryPrices();
        require(_bronzePrice >= _ticketPrice / 2,"Bronze egg can't be lower than 1/2 lottery ticket price");
