 ...
 */
contract CryptoDadsStakeChild is FxBaseChildTunnel, Ownable {
    uint256 public yieldPeriod = 1 hours;
    IERC20Rewards public rewardsToken;

    uint256 public firstStakeBonus = 50000000000000000000;
        ...
        FxBaseChildTunnel(fxChild)
    {
        ...
        uint256[] memory newRewards = new uint256[](3);
        ...
        uint256[] memory momAmounts = new uint256[](3);
        uint256[] memory newMomRewards = new uint256[](3);

        // 1-2: 5 tokens per day
        momAmounts[0] = 1;
        newMomRewards[0] = 5000000000000000000;

        // 3-9: 10 tokens per day
        momAmounts[1] = 3;
        newMomRewards[1] = 10000000000000000000;

        // 10+: 13 tokens per day
        momAmounts[2] = 10;
        newMomRewards[2] = 13000000000000000000;

        setMomRewards(amounts, newMomRewards);
    }
     ...
     */
    function setRewards(uint256[] memory amounts, uint256[] memory newRewards)
        public
        onlyOwner
    {
    ...
    /**
     * Resets the mom reward calculation schema.
     * @param amounts - a list of held amounts in increasing order.
     * @param newRewards - a parallel list to amounts containing the per period for the respective amount.
     */
    function setMomRewards(
    ...
    {
        uint256 numTiers = _countRewardsTiers();
     ...
     */
    function _countRewardsTiers() internal view returns (uint256) {
        uint256 count = 0;
        uint256 nextTier = 1;
        ...
        returns (uint256)
    {
        ...
        uint256 baseReward = _calculateReward(stake.amount);

        uint256 momBaseReward = _calculateMomReward(stake.momAmount);

        uint256 reward = (baseReward * periodsPassed) *
            stake.amount +
            (momBaseReward * periodsPassed) *
            stake.momAmount;
    ...
    /**
     * Evaluates the current Mom reward for having staked the given amount of tokens.
     * @param amount - the amount of tokens staked.
     * @return reward - the dividend per day.
     */
    function _calculateMomReward(uint256 amount)
        internal
        view
        returns (uint256)
    {
        ...
        do {
            Reward memory currentReward = momRewards[next];
            reward += currentReward.amount;
            next = currentReward.nextTier;
        } while (next != 0 && next <= amount);
