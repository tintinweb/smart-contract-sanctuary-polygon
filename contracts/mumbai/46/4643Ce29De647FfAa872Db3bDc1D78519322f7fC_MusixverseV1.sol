    ...
    MusixverseEternalStorage
{
    ...
    function mintTrackNFT(
        ...
        for (uint256 i = 0; i < amount; i++) {
            ...
            trackNFTs[mxvLatestTokenId.current()] = TrackNFT(
                price,
                msg.sender,
                resaleRoyaltyPercentage,
                onSale,
...
contract MusixverseEternalStorage {
    ...
    CountersUpgradeable.Counter public totalTracks;
    ...
    mapping(uint256 => TrackNFT) public trackNFTs;
...
library MusixverseLib {
    ...
    function isPastUnlockTimestamp(
        MusixverseEternalStorage.TrackNFT memory trackNFT
    ) public view {
        // Require that current time is past unlockTimestamp
        require(
            block.timestamp > trackNFT.unlockTimestamp,
    ...
    function checkForSale(
        ...
        require(trackNFT.onSale, "Song is not available for sale");
    ...
    function purchaseToken(
        uint256 tokenId,
        mapping(uint256 => MusixverseEternalStorage.TrackNFT) storage trackNFTs,
        ...
        // Fetch the songNFT
        MusixverseEternalStorage.TrackNFT memory _trackNFT = trackNFTs[tokenId];
    ...
    function _distributeFunds(
        ...
        if (trackNFT.soldOnce) {
            ...
            uint256 _royaltyAmount = (
                (msg.value).mul(trackNFT.resaleRoyaltyPercentage)
            ).div(100);
            ...
            uint256 _value = (
                (msg.value).mul(
                    100 -
                        (PLATFORM_FEE_PERCENTAGE +
                            trackNFT.resaleRoyaltyPercentage)
                )
            ).div(100);
    ...
    function updateTokenPrice(
        ...
        // Fetch the song
        MusixverseEternalStorage.TrackNFT storage _trackNFT = trackNFTs[
            tokenId
        ];
        // Old price
