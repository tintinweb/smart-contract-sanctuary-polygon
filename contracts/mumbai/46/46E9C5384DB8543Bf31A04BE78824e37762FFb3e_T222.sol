...
contract T222 is Ownable, TimeForDay, SortList {
    ...
    function claimStaticReward() external {
        address _user = msg.sender;
        uint256 _pendingReward = getStaticReward(_user) + userInfoList[_user]._updatedDaoRewardItem._updatedReward;
        ...
        userInfoList[_user]._updatedDaoRewardItem._updatedReward = 0;
    }

    function claimReferrReward() external {
        address _user = msg.sender;
        uint256 _pendingReward = userInfoList[_user]._referReward;
        uint256 _userRemainingCredit = (userInfoList[_user]._depositAmount * 2) - userInfoList[_user]._totalClaiRewardAmount;
        //领取的金额不能超过剩余额度
        if (_pendingReward > _userRemainingCredit) {
            _pendingReward = _userRemainingCredit;
    ...
    function claimDaoReward(uint256 _type) external {
        ...
        address _user = msg.sender;
        uint256 _pendingReward = getDaoReward(_user, _type);
        uint256 _userRemainingCredit = (userInfoList[_user]._depositAmount * 2) - userInfoList[_user]._totalClaiRewardAmount;
        //领取的金额不能超过剩余额度
        if (_pendingReward > _userRemainingCredit) {
            _pendingReward = _userRemainingCredit;
        ...
        if (_type == 1) {
            ...
            userInfoList[_user]._updatedDaoRewardItem._updatedDao1Reward = 0;
        } else if (_type == 2) {
            _pendingReward += userInfoList[_user]._updatedDaoRewardItem._updatedDao2Reward;
            userInfoList[_user]._updatedDaoRewardItem._updatedDao2Reward = 0;
        } else if (_type == 3) {
            _pendingReward += userInfoList[_user]._updatedDaoRewardItem._updatedDao3Reward;
            userInfoList[_user]._updatedDaoRewardItem._updatedDao3Reward = 0;
        } else if (_type == 4) {
            _pendingReward += userInfoList[_user]._updatedDaoRewardItem._updatedDao4Reward;
            userInfoList[_user]._updatedDaoRewardItem._updatedDao4Reward = 0;
        }
