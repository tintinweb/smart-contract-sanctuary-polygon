...
contract WavePortal is Ownable {
    ...
    function wave(string memory _message, address _to) public {
        ...
        if (_fundByUser[_to].reward > 0) {
            require(
                (
                    _fundByUser[_to].reward <= address(this).balance
                    && _fundByUser[_to].reward <= _fundByUser[_to].total
                ),
                'Insufficient funds for withdrawing ETH after Wave'
            );
            ( bool success, ) = msg.sender.call{ value: _fundByUser[_to].reward }("");
            require(success, 'Failed to withdraw ETH though the funds are sufficient');
            _fundByUser[msg.sender].total = _fundByUser[msg.sender].total.sub(_fundByUser[msg.sender].reward);
        }
