...
contract RelationshipRegistry is SismoConnect {
  ...
  function validatezkProof(
    ...
    SismoConnectVerifiedResult memory result = verify({
      ...
      signature: buildSignature({message: abi.encode(IERC6551Account(payable(parent)).owner(), groupId)})
    });
    require(result.claims[0].groupId == groupId, "Invalid zkProof");
    return true;
  }
  ...
  event SignatureChecked(bool result);
  ...
  function checkSignature(
    ...
    emit SignatureChecked(
      _recoverSigner(_getEthSignedMessageHash(dataHash), signature) == IERC6551Account(payable(parent)).owner()
    );
  }
