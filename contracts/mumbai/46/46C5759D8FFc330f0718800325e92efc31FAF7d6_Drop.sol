    ...
    IERC2981
{
    ...
    uint256[] public phaseStartTimestamps;
     ...
     * Collection settings
     */
    string private _contractBaseURI = "ipfs://xxx/";
    string private _contractURI = "ipfs://yyy";
    ...
    function getCurrentPhase()
        public
        view
        returns (Phase memory phase, uint256 phaseIndex)
    {
        uint256 current = block.timestamp;
        require(phaseStartTimestamps.length != 0, "Timestamps are not set");
        require(current >= phaseStartTimestamps[0], "No phase is started");

        for (uint256 i = 0; i < phaseStartTimestamps.length; i++) {
            if (current > phaseStartTimestamps[i]) return (phases[i], i);
        }
    }
    ...
    function whitelistedMint(
        ...
        (Phase memory phase, uint256 phaseIndex) = getCurrentPhase();
        require(phase.phaseType == PhaseType.Whitelist, "Incorrect phase type");
        require(msg.value == quantity * phase.price, "Wrong price");

        validateWhitelistMint(phaseIndex, to, proof, quantity);
    ...
    {
        (Phase memory phase, uint256 phaseIndex) = getCurrentPhase();
        require(phase.phaseType == PhaseType.Public, "Incorrect phase type");
        require(msg.value == quantity * phase.price, "Wrong price");

        validatePublicMint(phaseIndex, to, quantity);
    ...
    function setPhaseTimestamps(uint256[] calldata timestamps)
        external
        onlyOwner
    {
        if (phaseStartTimestamps.length != 0) delete phaseStartTimestamps;
        for (uint256 i = 0; i < timestamps.length; i++) {
            phaseStartTimestamps.push(timestamps[i]);
        }
    }
    ...
    function supportsInterface(bytes4 interfaceId)
        public
        view
        override(ERC721A, IERC721A, IERC165, CantBeEvil)
        returns (bool)
    {
        return
