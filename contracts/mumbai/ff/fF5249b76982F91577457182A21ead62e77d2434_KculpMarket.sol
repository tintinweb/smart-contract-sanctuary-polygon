...
contract KculpMarket is Ownable {
  ...
  event LogPrice(uint256 price, uint256 value);

  event LogRoyaltyPayment(address account, uint256 value);
  ...
  function buy(
    ...
    emit LogPrice(_price, msg.value);

    require(_price == msg.value, "MarketBuy: Asking price is not met");
    ...
    if (_accounts[0] == _accounts[1]) {
      ...
      emit LogRoyaltyPayment(_accounts[0], _roy1);
    ...
    if (_accounts[1] == royaltyAccount) {
      ...
      emit LogRoyaltyPayment(_accounts[1], _roy2);
      payable(_accounts[1]).transfer(_roy2);
    }
    if (royaltyAccount == _seller) {
      _tot = _tot + _roy3;
    } else {
      emit LogRoyaltyPayment(royaltyAccount, _roy3);
      payable(royaltyAccount).transfer(_roy3);
    }

    emit LogRoyaltyPayment(_seller, _tot);
