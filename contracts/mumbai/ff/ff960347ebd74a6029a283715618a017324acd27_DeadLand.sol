...
contract DeadLand is ERC721Enumerable, Ownable, UsingTellor {
    ...
    struct LandHistory {
        uint256 landType; // starts from 1, 0 means undefined
        uint256 timestamp;
    }
    // landType => LandInfo
    mapping(uint256 => LandInfo) public lands;
    // tokenId => LandHistory
    mapping(uint256 => LandHistory) public landOwners;
    // Coorddinates used => bool
    mapping(bytes32 => bool) coordinatesUsed;

    string private baseURI;

    bytes32 public root;

    constructor(bytes32 _root, address payable _tellorAddress)
        UsingTellor(_tellorAddress)
        ERC721("DEADLAND", "DLD")
    {
        baseURI = "landMetadataURI.com/";
        lands[1] = LandInfo({mintPrice: 1 ether, rentalAmount: 1 ether});
        lands[2] = LandInfo({mintPrice: 2 ether, rentalAmount: 2 ether});
        lands[3] = LandInfo({mintPrice: 3 ether, rentalAmount: 3 ether});
        root = _root;
    }
    ...
    function setBtcPrice() external {
        ...
        ) = getDataBefore(btcQueryId, block.timestamp - 2 hours);
        if (_ifRetrieve) {
            btcPrice = _value;
        }
    }
    ...
    function mintLand(
        ...
        bytes32 coordinateHash = keccak256(
            abi.encodePacked(x, ",", y, ":", _landType.toString())
        );

        require(!coordinatesUsed[coordinateHash], "Coodrinates already minted");
        require(
            isValid(proof, coordinateHash),
            "Invalid Coodrinates or LandType"
        );

        require(_landType > 0 && _landType < 3, "Invalid land");
        require(msg.value >= lands[_landType].mintPrice, "Invalid amount");
        uint256 supply = totalSupply();
        require(supply < 1e4, "Exceeds maximum Lands supply"); // 10,000
        LandHistory memory land = LandHistory({
            landType: _landType,
            timestamp: block.timestamp
        });
        landOwners[supply] = land;
        _mint(msg.sender, supply);
        coordinatesUsed[coordinateHash] = true;
    }
    ...
    function _transfer(
        ...
        landOwners[tokenId].timestamp = block.timestamp;
        super._transfer(from, to, tokenId);
    }

    function getLands(uint256 _landType) public view returns (LandInfo memory) {
        return lands[_landType];
    }

    function getLandOwners(uint256 _tokenId)
        public
        view
        returns (LandHistory memory)
    {
        return landOwners[_tokenId];
    }
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    tokenId.toString(),
                    "/",
                    landOwners[tokenId].landType.toString(),
                    ".json"
                )
            );
    }
