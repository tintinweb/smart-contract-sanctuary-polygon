...
function getStateIndex(bytes32 chat_id,bytes32[] storage arr) private returns (uint256) {
...
function getAddressIndex(bytes32 our_id) private returns (uint creator_index) {
...
function changeState(stateOptions new_changed_state, bytes32 audio_chat_id) public {
    ...
    if (stateOptions.PLANNED == old_audio_chat_state.state){
        state_planned[getStateIndex(audio_chat_id ,state_planned)] = state_planned[state_planned.length - 1];
        state_planned.pop();
    }
    else if (stateOptions.LIVE == old_audio_chat_state.state) {
        state_live[getStateIndex(audio_chat_id ,state_live)] = state_live[state_live.length - 1];
        state_live.pop();
    }
    else if (stateOptions.CANCELED == old_audio_chat_state.state) {
        state_canceled[getStateIndex(audio_chat_id ,state_canceled)] = state_canceled[state_canceled.length - 1];
        state_canceled.pop();
    }
    else if (stateOptions.READY == old_audio_chat_state.state) {
        state_ready[getStateIndex(audio_chat_id ,state_ready) ] = state_ready[state_ready.length - 1];
        state_ready.pop();
    }
...
function deleteAudioChat(bytes32 audio_chat_id) public {
    ...
    address_to_audio_chat[old_audio_chat_state.creator][getAddressIndex(audio_chat_id)] = 
