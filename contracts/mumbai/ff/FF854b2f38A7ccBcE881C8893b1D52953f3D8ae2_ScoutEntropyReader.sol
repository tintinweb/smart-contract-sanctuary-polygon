...
import "./Redeemers.sol";
import "./Scouts.sol";
import "./TicketsV2.sol";

contract ScoutEntropyReader {

  YouthScouts public scouts;
  TicketsV2 public ticketsV2;

  constructor (YouthScouts _scouts, TicketsV2 _ticketsV2) {
    scouts = _scouts;
    ticketsV2 = _ticketsV2;
  }
  ...
  function getAttributeFromSeed(string memory _attr, uint256 _tokenId) public view returns(string memory) {
    if (keccak256(abi.encodePacked((_attr))) == keccak256(abi.encodePacked(("StartingLevel")))) {
      uint256[2] memory scoutGenerator = scouts.getTokenOrigin(_tokenId);
      if (scoutGenerator[0] == uint(RedeemersTypes.Tickets)) {
        uint256 entropy = scouts.getEntropy(_tokenId, 0);
      ...
      if (scoutGenerator[0] == uint(RedeemersTypes.TicketsV2)) {
        string memory lootbox = ticketsV2.voucherTypes(_tokenId);
        bytes32 lootboxHash = keccak256(abi.encodePacked((lootbox)));
        if (lootboxHash == keccak256(abi.encodePacked(('Starter')))) {
          return "1";
        }
        if (lootboxHash == keccak256(abi.encodePacked(('Bronze')))) {
          return "2";
        }
        if (lootboxHash == keccak256(abi.encodePacked(('Silver')))) {
          return "3";
        }
        if (lootboxHash == keccak256(abi.encodePacked(('Gold')))) {
          return "4";
        }
      }
    }
 ...
 * AccessControl is used for actual permissioning
 *
 */
contract YouthScouts is ERC721Enumerable, EntropyStorage, TokenWithdraw, ReentrancyGuard, Ownable {
  using Strings for uint256;
  ...
  string public metadata_uri = "https://api.metasoccer.com/scouts/collection/meta";
...
import "@openzeppelin/contracts/utils/cryptography/MerkleProof.sol";
import "@openzeppelin/contracts/security/Pausable.sol";
import {Base64} from "@metasoccer/vouchers/contracts/libraries/Base64.sol";
import "./TokenWithdraw.sol";


/**
 * @dev TicketsV2 implementation
 *
 * Tickets can be redeemed against actual Youth Scouts.
 * Ownable is used just so we can have access to OpenSea collection editor,
 * AccessControl is used for actual permissioning
 *
 */
contract TicketsV2 is ERC721Enumerable, TokenWithdraw, Ownable, Pausable, ReentrancyGuard {
    using Strings for uint256;
    ...
    string private constant STARTER = "Starter";
    ...
    constructor(address _mintingToken, address _beneficiary) ERC721("MetaSoccer Youth Scout Tickets 2", "MSYST2") {
        require(_mintingToken != address(0), "Wrong minting token");
    ...
    function setDrop(bytes32 drop, Drop memory _drop) external onlyRole(DEFAULT_ADMIN_ROLE) {
        drops[drop] = _drop;
        emit NewDropCreated(drop, _drop.root);
    }
    ...
    function buyVouchers(
        ...
        // Vouchers should be available at drop level
        require(
            drops[drop].starters.soldNum + qtyStarters <=
                drops[drop].starters.totalNumber,
            ERROR_WRONG_QTY
        );
        require(
            drops[drop].bronzes.soldNum + qtyBronzes <=
                drops[drop].bronzes.totalNumber,
            ERROR_WRONG_QTY
        );
        require(
            drops[drop].silvers.soldNum + qtySilvers <=
                drops[drop].silvers.totalNumber,
            ERROR_WRONG_QTY
        );
        require(
            drops[drop].golds.soldNum + qtyGolds <=
                drops[drop].golds.totalNumber,
            ERROR_WRONG_QTY
        );

        // Should buy more than minimum required by drop and less than total allowed per type
        uint16 totalQty = qtyStarters + qtyBronzes + qtySilvers + qtyGolds;
        uint16 previousQty = buysPerDrop[msg.sender][drop][STARTER] +
            buysPerDrop[msg.sender][drop][BRONZE] +
            buysPerDrop[msg.sender][drop][SILVER] +
            buysPerDrop[msg.sender][drop][GOLD];
        require(
            totalQty + previousQty >= drops[drop].minPurchase,
            ERROR_WRONG_QTY
        );
        ...
        // Should buy less than total allowed per type per drop
        require(
            qtyStarters + buysPerDrop[msg.sender][drop][STARTER] <=
                drops[drop].starters.maxAllowed,
            ERROR_WRONG_QTY
        );
        require(
            qtyBronzes + buysPerDrop[msg.sender][drop][BRONZE] <=
                drops[drop].bronzes.maxAllowed,
            ERROR_WRONG_QTY
        );
        require(
            qtySilvers + buysPerDrop[msg.sender][drop][SILVER] <=
                drops[drop].silvers.maxAllowed,
            ERROR_WRONG_QTY
        );
        require(
            qtyGolds + buysPerDrop[msg.sender][drop][GOLD] <=
                drops[drop].golds.maxAllowed,
            ERROR_WRONG_QTY
        );

        uint256 amount = qtyStarters * drops[drop].starters.price + qtyBronzes * drops[drop].bronzes.price + qtySilvers * drops[drop].silvers.price + qtyGolds * drops[drop].golds.price;
        require(
            IERC20(mintingToken).transferFrom(msg.sender, beneficiary, amount),
            "Unable to receive payment"
        );

        drops[drop].starters.soldNum =
            drops[drop].starters.soldNum +
            qtyStarters;
        drops[drop].bronzes.soldNum = drops[drop].bronzes.soldNum + qtyBronzes;
        drops[drop].silvers.soldNum =
            drops[drop].silvers.soldNum +
            qtySilvers;
        drops[drop].golds.soldNum = drops[drop].golds.soldNum + qtyGolds;

        _mintForDrop(drop, STARTER, qtyStarters);
        _mintForDrop(drop, BRONZE, qtyBronzes);
        _mintForDrop(drop, SILVER, qtySilvers);
        _mintForDrop(drop, GOLD, qtyGolds);
    }
            ...
            bytes(
                string(
                    abi.encodePacked(
                        ...
                        voucherType, ' Ticket',
                        '","description": "',
                        'MetaSoccer Youth Scouts tickets, redeemable for actual Youth Scouts.',
                        '","external_url": "',
                        externalUrl,
                        '","image": "',
                        imageURLs[voucherType],
    ...
    function toggleDropStarted(bytes32 _drop) external onlyRole(DEFAULT_ADMIN_ROLE) {
        drops[_drop].started = !drops[_drop].started;
    }

    function setDropWhitelist(bytes32 _drop, bytes32 _root) external onlyRole(DEFAULT_ADMIN_ROLE) {
        drops[_drop].root = _root;
    }
    ...
    function setMintingToken(address _new_minting_token) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_new_minting_token != address(0), "Wrong minting token");
        mintingToken = _new_minting_token;
    }

    function setExternalUrl(string memory _url) external onlyRole(DEFAULT_ADMIN_ROLE) {
        externalUrl = _url;
    }

    function setImageUrl(string memory _covering, string memory _url) external onlyRole(DEFAULT_ADMIN_ROLE) {
        imageURLs[_covering] = _url;
    }

    function setAnimationUrl(string memory _covering, string memory _url) external onlyRole(DEFAULT_ADMIN_ROLE) {
        animationURLs[_covering] = _url;
    }

    function mintForEcosystem(address _to, uint256 _num_to_mint, string memory _type) external onlyRole(DEFAULT_ADMIN_ROLE) {
        for (uint256 i = 0; i < _num_to_mint; ++i) {
            voucherTypes[totalSupply() + burnedCount] = _type;
            _safeMint(_to, totalSupply() + burnedCount);
        }
    }
    ...
    function getAllOwned(address _owner) external view returns (string[] memory) {
        string[] memory ret = new string[](balanceOf(_owner));
        for (uint256 i = 0; i < balanceOf(_owner); i++) {
            ret[i] = voucherTypes[tokenOfOwnerByIndex(_owner, i)];
        }
        return ret;
    }
