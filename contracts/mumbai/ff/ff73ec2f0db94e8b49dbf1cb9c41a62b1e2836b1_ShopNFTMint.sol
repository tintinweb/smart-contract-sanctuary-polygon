...
// File: contracts/Onecape-chainlink/ShopNFTMint.sol
  ...
  BaseRelayRecipient
{
  ...
  function initialize(address _trustForwarderAddress) public initializer {
    ...
    __ERC1155_init_unchained("https://bafybeibgvrdprufrzkovp4vsegkss5vi4ctjf6bz47ylbwtp5odlkt5y2m.ipfs.nftstorage.link/{id}.json");
  ...
  mapping(uint256 => NFTDetails) nftLogs;
    ...
    mintCompliance(mintAmount)
  {
    for (uint256 i = 0; i < mintAmount; i++) {
      NFTDetails memory newNft = NFTDetails({
        id: nextTokenId,
        tokenURI: string(abi.encodePacked(metaDataURI,Strings.toString(nextTokenId),".json")),
        creator: creator,
        owner: msg.sender,
      ...
      emit Mint(nextTokenId, string(abi.encodePacked(metaDataURI,Strings.toString(nextTokenId),".json")), msg.sender, block.timestamp);
      nextTokenId++;
      nftsInCirculation++;
    }
  }
    ...
    mintCompliance(ids.length)
  {
    ...
    for (uint256 i = 0; i < ids.length; i++) {
      NFTDetails memory newNft = NFTDetails({
        id: ids[i],
        tokenURI: string(abi.encodePacked(metaDataURI,Strings.toString(ids[i]),".json")),
        creator: creator,
        owner: msg.sender,
        timeOfGenerate: block.timestamp
      });
      nftLogs[ids[i]] = newNft;
      emit Mint(ids[i], string(abi.encodePacked(metaDataURI,Strings.toString(ids[i]),".json")), msg.sender, block.timestamp);
      nextTokenId++;
      nftsInCirculation++;
    }
  }
  ...
  function setTokenURIByID(uint256 id, string memory tokenUri) public virtual {
    require(
      hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
