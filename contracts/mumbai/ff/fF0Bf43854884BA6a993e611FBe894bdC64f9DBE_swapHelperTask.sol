...
contract swapHelperTask is OpsTaskCreator, Ownable {
    ...
    function createTask(uint256 _INTERVAL) external payable {
        ...
        bytes memory execData = abi.encodeCall(this.swapCot, (msg.sender));
    ...
    function swapCot(address _user) external onlyDedicatedMsgSender {
        ...
        if (x._swapIn == 0) {
            ...
            emit swapCotE(tx.origin, msg.sender, x.gasUsed, x.spendSwapInToken, block.timestamp);
