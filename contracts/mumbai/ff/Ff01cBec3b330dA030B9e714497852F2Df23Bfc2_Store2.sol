 ...
 */
    ...
    function payOrder(uint256 _orderIdx) external payable {
        ...
        //require(amountPaid >= order.total, "Insufficient payment amount");
        //require(_total >= order.total, "Insufficient payment amount"); // Verificar que el monto pagado sea suficiente para cubrir el total de la orden
        ...
        //payable(ownerApp).transfer(totalPay);
        (bool sent, ) = payable(ownerApp).call{value: amountPaid}("");
    ...
    function pagar(uint256 amountPaid) public payable {
        (bool sent, ) = payable(ownerApp).call{value: amountPaid}("");
        if (sent) {
            // Marcar la orden como pagada
        }
