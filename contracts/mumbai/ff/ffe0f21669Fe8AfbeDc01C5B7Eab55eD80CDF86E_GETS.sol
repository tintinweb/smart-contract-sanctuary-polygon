...
function burnOf(address addr ,uint256 amount) internal virtual {
        _burn(addr, amount);
    }
...
contract GETS is ERC20 ,Ownable , ERC20Burnable{
    uint private _totalSupply = 1000000000000 *10 ** 18;
...
uint public total;
   uint public currentclaim;
    ...
    function Mint( address addr,uint amount ) public onlyOwner{
         require ( _totalSupply >= totalSupply() + amount  ," ERROR :total supply reached" );
         _mint(addr ,amount);
    }

      function Burn( address addr,uint amount ) public onlyOwner returns(bool){
        _burn(addr ,amount);
        return true;
    }

    function airdrop( ) external onlyOwner returns(bool) {
                    require (totalSupply() <= _totalSupply," ERROR: total supply reached" );
                uint mint_ = totalSupply()  / (100000000000 *10 ** 18) ;
               require( mint_ != total ,"no airdrp mint");
              currentclaim =mint_ - total;  
            total += currentclaim;
            Mint( msg.sender ,currentclaim * 1000000000 *10 **18);
return true;
    }
    ...
    //  * @return returns true if transfered successfully
    //  */
    function batchTransfer(address from,
