...
// But this does not affect the users in execution
    ...
    ISwapVault
{
    ...
    modifier onlyDuringPoolState(bytes32 poolId, PoolStatus _status) {
        require(pools[poolId].status == _status, "Pool Status Does Not Match");
        _;
    }
    ...
    // ====================================================================
    function _scaleSwapRate(bytes32 poolId, uint256 _swapRate)
        internal
        view
        returns (uint256)
    {
        ...
        uint8 assetFromDecimals = IERC20Detailed(assetFrom).decimals();
        uint8 assetToDecimals = IERC20Detailed(assetTo).decimals();
        uint256 scaledRate = (_swapRate * (10**assetToDecimals)) /
            (10**assetFromDecimals);
        return scaledRate;
    }

    function _unscaleSwapRate(bytes32 poolId, uint256 _scaledRate)
        internal
        view
        returns (uint256)
    {
        ...
        uint8 assetFromDecimals = IERC20Detailed(assetFrom).decimals();
        uint8 assetToDecimals = IERC20Detailed(assetTo).decimals();
        //todo: what if assetFromDecimals < assetToDecimals?
        //answer: removed this concept now,
        // now I am scaling all asset values to 18 decimals and only converting at the end
        uint256 rate = (_scaledRate * (10**assetFromDecimals)) /
            (10**assetToDecimals);
        return rate;
    }
    ...
    function _changePoolStatus(bytes32 poolId, PoolStatus _status)
        internal
        returns (bool)
    {
        PoolStatus currentStatus = pools[poolId].status;
        // If pool is Locked
        if (_status == PoolStatus.LOCKED) {
            if (currentStatus == PoolStatus.UNLOCKED) {
                pools[poolId].status = _status;
        ...
        // If Pool is Unlocked
        else if (_status == PoolStatus.UNLOCKED) {
        ...
        else if (_status == PoolStatus.WAITING_FOR_RATE) {
            require(
        ...
        else if (_status == PoolStatus.EMERGENCY) {
            if (
        ...
        // If Pool is INACTIVE
        else if (_status == PoolStatus.INACTIVE) {
            if (currentStatus == PoolStatus.WAITING_FOR_RATE) {
                pools[poolId].status = _status;
                return true;
            } else {
    ...
    function _setAggregateSwapRate(bytes32 poolId) internal {
        ...
        //todo: looks dangerous, try to div after all mul
        //SPOT_RATE_MULTIPLIER * assetFromDecimals (and div on assetFromDecimals) doens't make sense,
        //you will have 10^8 * 18 / 6, for instance. Maybe you want 10**(assetFromDecimals) / 10 ** (assetToDecimals)
        //and don't forget, thah in theory, assetToDecimals can be > assetFromDecimals (for instance, usdc/shib),
        //more over, even > assetFromDecimals + 8 (like SPOT_RATE_MULTIPLIER has)

        // answer: done, removed concept of scaled rate
    ...
    //todo: do we have garantee that pools[poolId].length will be always small?
    //max length = blockGasLimitForOneContract / gasSpent of one iteration of for
    //if yes, for gas saving (but it is not neccessary in our project) can be done:
    //for (uint256 i = 0; i < tempPool.orderIds.length;) {
    //...
    //unchecked{
    //i = i + 1;
    //}
    ...
    // answer: Answered below the commit,
    // but yes we have a guarantee as this is set manually by jacob and ovidiu for each vault
    function _deleteSwapOrder(bytes32 poolId, uint256 orderId) internal {
        SwapPool memory tempPool = pools[poolId];
        for (uint256 i = 0; i < tempPool.orderIds.length; i++) {
            if (tempPool.orderIds[i] == orderId) {
                uint256 tempVar = pools[poolId].orderIds[
                    tempPool.orderIds.length - 1
                ];
                pools[poolId].orderIds[tempPool.orderIds.length - 1] = tempVar;
    ...
    //todo: same as above
    // answer: swap requests are O(Number of Trufin Vaults) which is guaranteed to be small
    function _deleteSwapRequest(bytes32 poolId, bytes32 requestId) internal {
        SwapPool memory tempPool = pools[poolId];
        for (uint256 i = 0; i < tempPool.requestIds.length; i++) {
            if (tempPool.requestIds[i] == requestId) {
                bytes32 tempVar = pools[poolId].requestIds[
                    tempPool.requestIds.length - 1
                ];
                pools[poolId].requestIds[
                    tempPool.requestIds.length - 1
                ] = tempVar;
    ...
    /// @dev add require statements
    // Check if oppPool needs to exist or not
    // Remember to check math
    function _rebalanceVault(bytes32 poolId) internal {
        //OppVault refers to the vault that is giving the reverse swap of the currentPoolId
        ...
        uint256 oppVaultFunds = pools[oppPoolId].originalAmount;
        ...
        if (oppVaultFunds > currVaultFunds) {
            ...
            pools[poolId].toLockedAmount =
                (pools[poolId].originalAmount *
                    pools[poolId].internalSwapRate) /
                SPOT_RATE_MULTIPLIER;
        } else {
            //todo: do we have garantee that
            //oppVaultFunds * pools[oppPoolId].internalSwapRate < pools[poolId].originalAmount?
            // answer: Answered in comment below the commit
        ...
        pools[poolId].totalAmountSwappedinFrom =
            (pools[poolId].toLockedAmount * pools[poolId].internalSwapRate) /
            SPOT_RATE_MULTIPLIER;
    ...
    function _withdraw(bytes32 id) internal {
        SwapPool memory tempPool = pools[swapRequests[id].poolId];
        address user = swapRequests[id].userAddress;
        uint256 shares = swapRequests[id].amount;
        ...
        IERC20(tempPool.assetTo).approve(
            user,
            _unscaleAssetAmountToOriginal(tempPool.assetTo, userSwappedAmount)
        );
        IERC20(tempPool.assetFrom).approve(
            user,
            _unscaleAssetAmountToOriginal(
                tempPool.assetFrom,
                userLeftoverAmount
            )
        );
    ...
    function _callReceiveFunds(bytes32 id) internal {
        SwapPool memory tempPool = pools[swapRequests[id].poolId];
        address user = swapRequests[id].userAddress;
        uint256 shares = swapRequests[id].amount;
        uint256 userSwappedAmount = ((tempPool.toLiquidAmount +
    ...
    /// @dev remember to check that both assets should be whitelisted
    // at the end of every epoch, pool should become unlocked
    // Do we want to keep a minimum allowed deposit value? Ask jacob
    // Remember to delete requestids during withdrawals
    // Remember to check if swapRequest already exists then we have to add amount to it
        ...
        returns (bytes32)
    {
        ...
        IERC20(tempPool.assetFrom).transferFrom(
            msg.sender,
            address(this),
            _amount
        );
        uint256 scaledAmount = _scaleAssetAmountTo18(
            tempPool.assetFrom,
            _amount
        );
    ...
    function fillSwapOrder(uint256 orderId) external nonReentrant {
        ...
        swapOrders[orderId].isFilled = true;
        ...
        IERC20(transferAsset).transferFrom(
            msg.sender,
            address(this),
        ...
        nonReentrant
    {
        //todo: do you mean ">="?
        //answer: Yes, fixed
        //todo: what about pools[poolId].originalAmount?
        //answer: the value of originalAmount is subtracted below
        ...
        require(pools[poolId].status == PoolStatus.UNLOCKED, "Pool is Locked");
        swapRequests[requestId].amount -= scaledAmount;
        ...
        IERC20(pools[poolId].assetFrom).approve(
            msg.sender,
            swapRequests[requestId].amount
        );
    }
    ...
    // receiveSwappedFunds(uint256 spotRate, uint256 midSpotRate, uint256 amountSwapped, uint256 amountLeftover)
    // amountLeftover is in assetFrom

    //todo: what max requestArr.length? (see some functions above)
    //answer: answered above, but max requestArr.length is O(Number of Trufin vaults)
    // add midSwapRate check
        ...
        onlyDuringPoolState(poolId, PoolStatus.LOCKED)
    {
        require(
            pools[poolId].internalSwapRate != 0,
            "Internal Swap Rate is Zero"
        );
        bytes32[] memory requestArr = pools[poolId].requestIds;
        for (uint256 i = 0; i < requestArr.length; i++) {
            bytes32 id = requestArr[i];
            _withdraw(id);
            _callReceiveFunds(id);
        }
        pools[poolId].requestIds = new bytes32[](0);
        _changePoolStatus(poolId, PoolStatus.WAITING_FOR_RATE);
    }

    /// @dev remember to check that both assets should be whitelisted
    // Remember to test if opp Pool doesn't exist, code should still work
    ...
    /// @param  _amount should have decimals in assetFrom for
    // change the address to MM id
    function createSwapOrder(
        ...
        uint256 scaledAmount;
        if (_isReverseOrder) {
            scaledAmount = _scaleAssetAmountTo18(
    ...
    /// @param poolId is the Id of the pool for which rate is being set, if rate is X,X amount of assetTo per AssetFrom
    /// @param rate should be in terms of assetTo with 8 decimal places allowed
    // Let rate = s * multiplier
    // remember to use internalSwapRate/ multiplier every time you want final value of anything
    // Components to Swap Rate - 1. Spot Value, 2. Multiplier, 3. Balancer
    // Remember to test Math behind swap rate
    // Expose universal swap rate
        ...
        onlyDuringPoolState(poolId, PoolStatus.LOCKED)
    {
        ...
        //todo: if swap rate has >= ~14-16 decimals, midSwapRate can be set to zero
        //answer: I don't think it will give an error still, because the math seems to work out fine,
    ...
    function closeAllPoolOrders(bytes32 poolId)
        ...
        onlyDuringPoolState(poolId, PoolStatus.WAITING_FOR_RATE)
    {
        for (uint256 i = 0; i < pools[poolId].orderIds.length; i++) {
    ...
    {
        require(
            msg.sender == swapRequests[requestId].userAddress,
            "Access to Request Denied"
        );

        _withdraw(requestId);
        _deleteSwapRequest(swapRequests[requestId].poolId, requestId);
    }

    // Need to take care of the pool state while doing this, currently it should be locked
...
// Jacob sets a spot rate for a given pool, epoch begins
// Vaults are allowed to deposit until jacob calls lock pool
// Amount of external swap is decided by the spot rate S
// Jacob creates as many swap orders as he wants with the amount left for external swap
// Jacob manually unlocks pools by redistributing the swap, pool can only be unlocked if there are no pending swapOrder
...
// Changes needed -
// 1. Partial Settlement of offer
// 2. Internal Rebalancing should be at spot rate S
// 3. Should be able to reverse offer ( If possible, not mandatory )

interface ISwapVault {
    /************************************************
     *  IMMUTABLES & CONSTANTS
     ***********************************************/
    // Remove swap Failed and swap successful, just keep swap complete
    enum PoolStatus {
        INACTIVE,
        WAITING_FOR_RATE,
        UNLOCKED,
 ...
 */
contract MasterWhitelist is Ownable {
    /**
