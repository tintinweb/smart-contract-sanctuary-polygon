...
contract StakeInTheTown is Staking721Base {
    ...
    function getMultiplierSecretWord() external view onlyOwner returns (string memory) {
        return (
            _secretWordMultiplier
        );
    }
    ...
    function getFixedSecretWord() external view onlyOwner returns (string memory) {
        return (
            _secretWordFixed
        );
    }
    ...
    function _claimRewards() internal override {
        ...
        if (multiplierToggle && multiplierEndTime[_stakeMsgSender()] > 0 && block.timestamp < multiplierEndTime[_stakeMsgSender()]) {
            if (rewardMultiplier < 100) {
                rewardMultiplier += 100;
            }
            rewards = rewards * (rewardMultiplier / 100);
        } else {
            rewardMultiplier = 0;
        }
    ...
    function claimSecretWordRewardFixed(string memory inputSecretWordFixed) external {
    ...
    function claimSecretWordRewardMultiplier(string memory inputSecretWordMultiplier) external {
