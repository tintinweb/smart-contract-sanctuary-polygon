...
contract Digger is Random {
    using SafeMath for uint256;
    uint256 public minDigAmount;
    uint public minDigCount = 1;
    uint public maxDigCount = 5;
    uint public digDiscountPct = 10;
    string[2] public digKeys = ["nft", "balance"];
    struct DigReward {
        uint id;
        string key;
        uint256 value;
    ...
    constructor(uint256 _minDigAmount) {
        minDigAmount = _minDigAmount;
    }

    event VaultWasDigged(address by, uint digCount, DigReward[] digRewards);
    event MinDigAmountUpdated(address by, uint256 digAmount);
    event MinDigCountUpdated(address by, uint digCount);
    event MaxDigCountUpdated(address by, uint digCount);
    event DigDiscountPctUpdated(address by, uint discountPct);

    function digVault(uint digCount) public payable {
        require(
            digCount == minDigCount || digCount == maxDigCount,
            "digCount not supported"
        );
        if (digCount == minDigCount) {
            require(msg.value == getMinDigAmount(), "minDigAmount not enough");
        } else if (digCount == maxDigCount) {
            require(msg.value == getMaxDigAmount(), "maxDigAmount not enough");
        }
        DigReward[] memory digRewards = new DigReward[](digCount);
        for (uint i = minDigCount; i <= digCount; i++) {
            DigReward memory digReward;
            uint256 index = random(digKeys.length) % digKeys.length;
            digReward.id = i;
            digReward.key = digKeys[index];
            if (
                keccak256(bytes(digReward.key)) == keccak256(bytes(digKeys[0]))
            ) {
                // _mint(msg.sender, 1);
                digReward.value = 1;
                console.log(
                    "digReward id: %s key: %s, value: %s",
                    i,
                    digReward.key,
                    digReward.value
                );
            } else if (
                keccak256(bytes(digReward.key)) == keccak256(bytes(digKeys[1]))
            ) {
                // set percentage
                // 1 (80% odds), 5 (15% odds), 50 (5% odds)
                // transfer to address
                digReward.value = 2;
                console.log(
                    "digReward id: %s key: %s, value: %s",
            ...
            digRewards[i - 1] = digReward;
        }
        emit VaultWasDigged(msg.sender, digCount, digRewards);
    }

    function setMinDigAmount(uint256 _minDigAmount) public {
        minDigAmount = _minDigAmount;
        emit MinDigAmountUpdated(msg.sender, _minDigAmount);
    }

    function getMinDigAmount() public view returns (uint256) {
        return minDigAmount;
    }

    function getMaxDigAmount() public view returns (uint256) {
        uint256 discountPct = digDiscountPct.div(100);
        uint256 maxDigAmount = minDigAmount.mul(maxDigCount);
        uint256 digDiscount = maxDigAmount.mul(discountPct);
        return maxDigAmount.sub(digDiscount);
    }

    function setMinDigCount(uint _minDigCount) public {
        minDigCount = _minDigCount;
        emit MinDigCountUpdated(msg.sender, _minDigCount);
    }

    function getMinDigCount() public view returns (uint) {
        return minDigCount;
    }

    function setMaxDigCount(uint _maxDigCount) public {
        maxDigCount = _maxDigCount;
        emit MaxDigCountUpdated(msg.sender, _maxDigCount);
    }

    function getMaxDigCount() public view returns (uint) {
        return maxDigCount;
    }

    function setDigDiscountPct(uint _digDiscountPct) public {
        digDiscountPct = _digDiscountPct;
        emit DigDiscountPctUpdated(msg.sender, _digDiscountPct);
    }

    function getDigDiscountPct() public view returns (uint) {
        return digDiscountPct;
    }
}
...
abstract contract Game is Digger, PermissionsEnumerable, ERC721Drop {
    using SafeMath for uint256;
    uint256 public minParticipants = 2;

    constructor(uint256 _minDigAmount) Digger(_minDigAmount) {}

    event MinParticipantsUpdated(address by, uint256 count);
    ...
    function setMinParticipants(uint256 _minParticipants) public {
        minParticipants = _minParticipants;
        emit MinParticipantsUpdated(msg.sender, _minParticipants);
    }
    ...
    function getWinner() public {
        ...
        require(
            participants >= getMinParticipants(),
            "participants must be >= minParticipants"
        );
        uint256 tokenId = random(participants).mod(participants);
        address winner = ownerOf(tokenId);
        emit WinnerSelected(winner, tokenId);
    }
}
...
contract MinutoNFT is PlatformFee, Game {
...
contract Random {
    ...
    function random(
        ...
        return
            uint256(
                keccak256(
                    abi.encodePacked(block.difficulty, block.timestamp, length)
                )
            );
    }
}
