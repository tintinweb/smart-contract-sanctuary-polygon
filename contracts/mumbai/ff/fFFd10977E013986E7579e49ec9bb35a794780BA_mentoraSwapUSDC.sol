...
contract mentoraSwapUSDC is AccessControl, PriceConsumerMaticDollar, ReentrancyGuard{
    ...
    address public constant USDCAddress =
        0x921D0E46B20F10c9BB9DcC38C7ff222DbcB97083;
    address public constant MeCoinAddress =
        0xA1ac913125C744D90010563617C18AAC779a934B;
    address public constant swapRouterAddress =
        0xE592427A0AEce92De3Edee1F18E0157C05861564;
    uint256 public bipsTxFeeAmountA = 200;
    uint256 public bipsTxFeeAmountB = 150;
    uint256 public bipsTxFeeAmountC = 100;
    uint256 public priceRangeA = 50 * 10**18;
    uint256 public priceRangeB = 200 * 10**18;
    uint24 public maticUSDCPoolFee = 500;
    ...
    event MaticToMeCoinTransaction(
        address indexed _caller,
        uint indexed _bipsFeeAmount,
        uint  grossAmountIn,
        uint  netAmountIn,
        uint  _amountOut
    );
    event MeCoinToMaticTransaction(
        address indexed _caller,
        uint indexed _bipsFeeAmount,
        uint  _amountIn,
        uint grossAmountOut,
    ...
    function setTxFeeRangeC(uint256 _bipsnewFeeC) onlyRole(CHANGE_FEE_ORDER_AMOUNT_ROLE)  public {
        ...
        function setMATICUSDCPoolFee(uint24 _newFee) onlyRole(CHANGE_FEE_ORDER_AMOUNT_ROLE) public {
        maticUSDCPoolFee = _newFee;
    }
    ...
    /** @notice This function swaps the Polygon native token(MATIC) to  Mentora Well Played token(MeCoin)
     *  @dev This payable function will use the amount defined on msg.value to swap to MeCoin.
     ...
     */
    function MaticToMeCoin(
        uint256 _deadline,
        uint256 _minAmountOut,
        uint24 _fee
    ) external payable {
        bipsFeeAmount = returnFee(msg.value);
        uint256 amountSwap = msg.value - ((msg.value * bipsFeeAmount) / 10000);
        ...
        emit MaticToMeCoinTransaction(msg.sender,bipsFeeAmount, msg.value, amountSwap, amountOut);
    }
    /** @notice This function swaps the Mentora Well Played token(MeCoin) to Polygon native token(MATIC).
     *  @dev First of all, it needed aprove this contract to spend the MeCoin balance.
     ...
     *  @param _amountSwap the amount in MeCoin to swap to Matic
     *  @param _deadline the unix time after which a swap will fail, to protect against long-pending transactions and wild swings in prices.
     *  @param _minAmountOut the mininum amount acceptable.
     *  @param _fee The fee tier of the pool, used to determine the correct pool contract in which to execute the swap.
     */
    function MeCoinToMatic (
        uint256 _amountSwap,
        uint256 _deadline,
        uint256 _minAmountOut,
        uint24 _fee
        ...
        bipsFeeAmount = returnFee(amountOut);
        
        uint256 netAmountOut = amountOut - ((amountOut * bipsFeeAmount) / 10000);
        (bool success, ) = payable(msg.sender).call{value: netAmountOut}("");
        if (!success) {
            revert("Failure on withdraw!");
        }
        emit MeCoinToMaticTransaction(msg.sender, bipsFeeAmount, _amountSwap, amountOut, netAmountOut);
    }
    function swap(
        address _tokenIn,
        address _tokenOut,
        ...
        ISwapRouter.ExactInputParams({
                path: abi.encodePacked(_tokenIn, _fee1, USDCAddress, _fee2, _tokenOut),
...
contract PriceConsumerMaticDollar {
    ...
    function getPriceMaticperDolar() public view returns (uint){
        ...
        int256 dec = 10**8; //for wei
        //int256 dec = 10**26 // for ether 
    ...
    function getPriceMATIC(uint usd) public view returns(uint){
        uint matic = usd * getPriceDolarperMatic() / 10**18; // for wei
        // uint matic = usd * getPriceDolarperMatic(); // for ether
        return matic;
    }
