...
// File: contracts/MetagetXNFTMarketplace.sol


// Test: 0x97f3CEA6A653c360b6d0272DF940447399d87B97
...
interface IMetagetXNFTFactory {
    ...
    function isMetagetXNFT(address _nft) external view returns (bool);
}

interface IMetagetXNFT {
    function getRoyaltyFee() external view returns (uint256);

    function getRoyaltyRecipient() external view returns (address);
}

/* MetagetX NFT Marketplace
    List NFT, 
    Buy NFT, 
    Offer NFT, 
    Accept offer, 
    ...
    & support Royalty
*/
contract MetagetXNFTMarketplace is Initializable, UUPSUpgradeable, OwnableUpgradeable  {
    uint256 private platformFee;
    address private feeRecipient;
    uint public totalListed;
    ...
    // nft => list all nft was listed
    mapping (uint => ListNFT) private nftListed;
    ...
    IMetagetXNFTFactory private MetagetXNFTFactory;
    
    function initialize(
        uint256 _platformFee,
        address _feeRecipient,
        IMetagetXNFTFactory  _MetagetXNFTFactory

    ) public initializer {
        require(_platformFee <= 10000, "Can't more than 100 percent");
        platformFee = _platformFee;
        feeRecipient = _feeRecipient;
        MetagetXNFTFactory = _MetagetXNFTFactory;
    ...
    modifier isMetagetXNFT(address _nft) {
        require(MetagetXNFTFactory.isMetagetXNFT(_nft), "Not MetagetX NFT");
        _;
    }
    ...
    function listNft(
        ...
        address _payToken,
        uint256 _price
    ) external isMetagetXNFT(_nft) isPayableToken(_payToken) {
        IERC721 nft = IERC721(_nft);
        require(nft.ownerOf(_tokenId) == msg.sender, "Not nft owner");
        nft.transferFrom(msg.sender, address(this), _tokenId);

        // list nft with single tokenId
        ...
        isListedNFT(_nft, _tokenId)
    {
        ...
        // delete nft was listed from struct
        delete listNfts[_nft][_tokenId];
        deleteListedNFT(_nft, _tokenId);
        totalListed--;
    }

    function deleteListedNFT(address _nft, uint256 _tokenId) 
        internal 
    {
        for (uint i = 0; i < totalListed; i++) {
            if (nftListed[i].nft == _nft && nftListed[i].tokenId == _tokenId) {  
                delete nftListed[i];
            }
        }
    }
    ...
    function buyNFT(
        ...
        IMetagetXNFT nft = IMetagetXNFT(listedNft.nft);
    ...
    function offerNFT(
        ...
        require(_offerPrice > 0, "Price can not zero");
        require(_offerPrice >= listNfts[_nft][_tokenId].price, "Price offer must higher than listed price"); //For auction
        //require(_offerPrice <= listNfts[_nft][_tokenId].price, "Price offer must lower than listed price"); //For simple offer
        ...
        isListedNFT(_nft, _tokenId)
    {
        ...
        IMetagetXNFT nft = IMetagetXNFT(offer.nft);
        address royaltyRecipient = nft.getRoyaltyRecipient();
        uint256 royaltyFee = nft.getRoyaltyFee();
    ...
    function resultAuction(address _nft, uint256 _tokenId) external {
        ...
        IMetagetXNFT MetagetXNft = IMetagetXNFT(_nft);
        address royaltyRecipient = MetagetXNft.getRoyaltyRecipient();
        uint256 royaltyFee = MetagetXNft.getRoyaltyFee();
    ...
    function getListedAllNFT() 
        public 
        view 
        returns (ListNFT[] memory)
    {
        ListNFT[] memory listNftListed = new ListNFT[](totalListed);
        for (uint i = 0; i < totalListed; i++) {
            ListNFT storage listAllnft = nftListed[i];
            listNftListed[i] = listAllnft;
        }
        return listNftListed;
    }

    function getListedAByUser() 
        public 
        view 
        returns (ListNFT[] memory)
    {
