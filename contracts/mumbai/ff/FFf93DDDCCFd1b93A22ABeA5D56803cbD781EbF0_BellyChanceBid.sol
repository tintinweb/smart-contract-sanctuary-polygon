...
contract BellyChanceBid is VRFConsumerBase {
    ...
    mapping(address => PlayerBidItem) public playerBidItems;
        ...
        returns (uint256)
    {
        ...
        for (uint256 index = 0; index < _bids.length; index++) {
            BidFor memory _bid = _bids[index];
            if (_bid.player == player) {
                res = uint256(_bid.amount);
            }
        }
        return res;
    }

    function getMyBidIndex(uint256 _itemId, address _player)
        private
        view
        returns (uint256)
    {
    ...
    function enterChanceBid(
        ...
        require(
            bellyToken.balanceOf(msg.sender) >= _amount,
            "The account value is lower than send tx!"
        );

        uint256 _prevAmount = getPrevAmount(itemId, msg.sender);

        uint256 totalAmount = _prevAmount + _amount;

        require(
            totalAmount <= _maxPerPlayer,
            "You cant bid more than 25% of the total value"
        );

        if (_prevAmount != 0) {
            uint256 index = getMyBidIndex(itemId, msg.sender);
            bidsForItems[itemId][index] = BidFor(totalAmount, msg.sender);
        } else {
            bidsForItems[itemId].push(BidFor(_amount, msg.sender));
        }

        bellyToken.increaseAllowance(msg.sender, _amount);
        bellyToken.transferFrom(msg.sender, adminWallet, _amount);
