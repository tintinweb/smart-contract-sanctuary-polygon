    ...
    SwixRoles
{
    ...
    ISwixEcosystem ecosystem;

    /// Toggle when during Ecosystem initialization
    bool initialized = false;
    /// Marks when this Ecosystem was updated last
    uint256 lastUpdated;
    ...
    /// @param setGovernance - governance of the current SwixEcosystem contract
    constructor(address setGovernance) {
        /// Grant governance role
        _setupRole(GOVERNANCE_ROLE, setGovernance);
        // Grant Governance power to set and revoke different roles
        _setupRole(DEFAULT_ADMIN_ROLE, setGovernance);

        /// Setup Contract Manager as admin of contract roles
        _setRoleAdmin(SWIX_TOKEN_CONTRACT, CONTRACT_MANAGER_ROLE);
        _setRoleAdmin(LEASE_AGREEMENT_CONTRACT, CONTRACT_MANAGER_ROLE);
        _setRoleAdmin(CITY_CONTRACT, CONTRACT_MANAGER_ROLE);
        _setRoleAdmin(BOOKING_MANAGER_CONTRACT, CONTRACT_MANAGER_ROLE);
        _setRoleAdmin(CANCEL_POLICY_CONTRACT, CONTRACT_MANAGER_ROLE);
        _setRoleAdmin(REVENUE_SPLIT_CONTRACT, CONTRACT_MANAGER_ROLE);
        _setRoleAdmin(TOKENBACK_CONTRACT, CONTRACT_MANAGER_ROLE);
        _setRoleAdmin(STABLECOIN_TOKEN_CONTRACT, CONTRACT_MANAGER_ROLE);

        /// Grant Contract Manager role to this contract's deployer
        _setupRole(CONTRACT_MANAGER_ROLE, msg.sender);

        ecosystem = ISwixEcosystem(address(this));
    }
        ...
        onlyRole(CONTRACT_MANAGER_ROLE)    
    {
        ...
        require(getRoleMemberCount(SWIX_TOKEN_CONTRACT) > 0,        "SWIX");
        require(getRoleMemberCount(STABLECOIN_TOKEN_CONTRACT) > 0,         "DOLLAR");
        require(getRoleMemberCount(BOOKING_MANAGER_CONTRACT) > 0,   "BM");
        require(getRoleMemberCount(CITY_CONTRACT) > 0,              "CITY");
        require(getRoleMemberCount(LEASE_AGREEMENT_CONTRACT) > 0,   "LEASE_AGREEMENT");
        require(getRoleMemberCount(CANCEL_POLICY_CONTRACT) > 0,     "CANCEL");
        require(getRoleMemberCount(REVENUE_SPLIT_CONTRACT) > 0,     "REV_SPLIT");
        require(getRoleMemberCount(TOKENBACK_CONTRACT) > 0,         "TOKENBACK");
        require(getRoleMemberCount(LEASE_MANAGER_ROLE) > 0,         "LEASE_MANAGER");
        require(getRoleMemberCount(LEASE_POLICY_ROLE) > 0,          "LEASE_POLICY");
        require(getRoleMemberCount(COST_MANAGER_ROLE) > 0,          "COST_MANAGER");
        require(getRoleMemberCount(CANCEL_POLICY_MANAGER_ROLE) > 0, "CANCEL_MANAGER");
        require(getRoleMemberCount(CONTRACT_MANAGER_ROLE) > 0,      "CONTRACT_MANAGER");
        require(getRoleMemberCount(DAO_ROLE) > 0,                   "DAO");
        require(getRoleMemberCount(EXPENSE_WALLET_ROLE) > 0,        "EXPENSE_WALLET");
        require(getRoleMemberCount(REFUND_WALLET_ROLE) > 0,         "REFUND_WALLET");
        
        initialized = true;
    }

    /// update a specific role in the SwixEcocystem
    /// @param role - specify the role to update
    /// @param newContract - specify the new address of the role
    function updateContract(bytes32 role, address newContract)
        external
        onlyRole(CONTRACT_MANAGER_ROLE)
    {
        require(
            role == SWIX_TOKEN_CONTRACT
            || role == BOOKING_MANAGER_CONTRACT
            || role == CANCEL_POLICY_CONTRACT
            || role == REVENUE_SPLIT_CONTRACT
            || role == TOKENBACK_CONTRACT
            || role == STABLECOIN_TOKEN_CONTRACT,
            "NOT_CONTRACT"
        );

        // Revoke role from previous contract
        _revokeRole(role, getRoleMember(role, 0));
        
        // Grant role to a new contract
        _grantRole(role, newContract);

        // Mark update
        lastUpdated = block.timestamp;
    }

    /// Function used to grant roles
    /// @param role - specify the role to update
    /// @param account - specify the new address of the role
    function grantRole(bytes32 role, address account)
        public
        virtual override
        onlyRole(getRoleAdmin(role))
    {
        require(role != GOVERNANCE_ROLE, "GOVERNANCE");
        require(role != DEFAULT_ADMIN_ROLE, "ADMIN");
    ...
    /// Updates both Governance and Default Admin roles at once
    /// @param newGovernance address - specify the new address of the governance
    function updateGovernance(address newGovernance)
        external
        onlyRole(GOVERNANCE_ROLE)
    {
        // Grant Admin role to newGovernance
        grantRole(DEFAULT_ADMIN_ROLE, newGovernance);
        // Grant Governance role to newGovernance
        grantRole(GOVERNANCE_ROLE, newGovernance);

        // Renounce Governance role
   ...
   /// update the newEcosystem address
   /// @param newEcosystem - specify the new address of the SwixEcosystem
    function migrateEcosystem(ISwixEcosystem newEcosystem)
        external
        onlyRole(GOVERNANCE_ROLE)
    {
        ...
        emit UpdateEcosystem(newEcosystem);
    ...
    ///  check whether an address has the specified role
    /// @dev revert if the account does not have the specified role
    ...
    /// currentEcosystem
    /// @return the address for current ecosystem
    function currentEcosystem()
        external
        view
        returns (ISwixEcosystem)
    {
        return ecosystem;
    }

    /// currentEcosystemInitialized
    /// @return the bool whether the current ecosystem is initialized   
    function ecosystemInitialized()
        external
        view
        returns (bool)
    {
    ...
    event UpdateEcosystem(ISwixEcosystem indexed swixEcosystem);
}
