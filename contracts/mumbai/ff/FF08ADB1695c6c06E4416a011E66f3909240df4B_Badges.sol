...
import "./BadgesAdmin.sol";
import "../permissions/interfaces/IPermissionsRegistry.sol";
    ...
    ERC1155SupplyUpgradeable
{
    ...
    // @notice Mapping from TokenId to TokenData
    ...
    function initialize(
        ...
        __BadgesAdmin_init(platform, creator);
        __ERC1155_init("");
    ...
    function createToken(
        uint256 supply,
        string memory newuri
    ) public onlyRole(PLATFORM_ROLE) returns (uint256) {
    ...
    function setURI(
        ...
        require(isActive(tokenId), "Token is not active");
    ...
    function maxSupply(
        ...
        return tokens[tokenId].maxSupply;
    }
    ...
    function setMaxSupply(
        ...
        tokens[tokenId].maxSupply = supply;
    ...
    function mint(
        ...
        require(isActive(tokenId), "Token is not active");
        require(maxSupply(tokenId) > totalSupply(tokenId), "Max supply reached");
    ...
    function isPlatformExecutor(address account) public view returns (bool) {
        return IPermissionsRegistry(_permissionsRegistry).isPlatformExecutor(account);
    }

    function hasPlatformRole(address account) public view override virtual returns (bool) {
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view override(ERC1155Upgradeable, BadgesAdmin) returns (bool) {
        return 
            super.supportsInterface(interfaceId) ||
            BadgesAdmin.supportsInterface(interfaceId);
    }
    ...
    ReentrancyGuardUpgradeable
{
    ...
    /**
     * @notice Holders of this role are the brand's admin, granted by platform role holders
     */
    bytes32 public constant BRAND_ADMIN_ROLE = keccak256("BRAND_ADMIN_ROLE");

    function __BadgesAdmin_init(address platform, address creator) internal onlyInitializing {
        __Ownable_init();
        ...
        _grantRole(PLATFORM_ROLE, creator);

        _setRoleAdmin(PLATFORM_ROLE, DEFAULT_ADMIN_ROLE);
        _setRoleAdmin(BRAND_ADMIN_ROLE, PLATFORM_ROLE);
    }

    function swapDefaultAdmin(address newDefaultAdmin) external virtual onlyRole(DEFAULT_ADMIN_ROLE) {
        _revokeRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _grantRole(DEFAULT_ADMIN_ROLE, newDefaultAdmin);
    }

    function swapPlatform(address account) external virtual onlyRole(PLATFORM_ROLE) {
        AccessControlUpgradeable._revokeRole(PLATFORM_ROLE, _msgSender());
        AccessControlUpgradeable._grantRole(PLATFORM_ROLE, account);
    }

    function hasPlatformRole(address account) external view virtual returns (bool) {
        return hasRole(PLATFORM_ROLE, account);
    }

    function hasBrandAdminRole(address account) external view virtual returns (bool) {
        return hasRole(BRAND_ADMIN_ROLE, account);
    }
...
interface IPermissionsRegistry {
     ...
     */
    function initialize(address _platformVault) external;
