...
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;
import "./RareketCollectionToken.sol";
import "./EventEmitter.sol";

contract NFTFactory is Ownable, EventEmitter {
    RareketCollectionToken[] public collectionAddressByIndex;
    constructor() {}
    function createCollection(
        ...
        RareketCollectionToken newCollection = new RareketCollectionToken(
            _marketplaceAddress,
            _name,
            _symbol
        );
        collectionAddressByIndex.push(newCollection);
        newCollectionCreatedEvent(address(newCollection), _name, msg.sender, _symbol);
    }
    //
}
...
contract  RareketCollectionToken  is ERC721Enumerable, Ownable, EventEmitter
{
    using SafeMath for uint256;
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdTracker;
    // EventEmitter eventContract;
    ...
    constructor(address _mpAddress, string memory _name, string memory _symbol ) ERC721(_name, _symbol)  {
    ...
    //  }
function mint(string memory _uri, uint _royalty_percentage, bool _wantsRoyalty, bool _lock, string memory _lockedURI) public onlyOwner isNotPaused{       
            ...
            emit newMint(address(this),currentTokenID, msg.sender, _wantsRoyalty, _royalty_percentage,  _lock, _lockedURI, _uri);
            newMintEvent(address(this),currentTokenID, msg.sender, _wantsRoyalty, _royalty_percentage,  _lock, _lockedURI, _uri);
    }
