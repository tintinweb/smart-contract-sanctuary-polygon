...
contract RBLSDepositor is IERC1363Receiver, Ownable {
    ...
    address private rblsContractAddress;
    IERC20 private rblsContract;


    mapping(uint256 => DepositData) private depositsDataSet;

    constructor(address _depositWallet, address _rblsContractAddress) {
        depositWallet = _depositWallet;
        rblsContract = IERC20(_rblsContractAddress);
        rblsContractAddress = _rblsContractAddress;
    }
    ...
    function onTransferReceived(address operator, address from, uint256 value, bytes memory data) public virtual override returns (bytes4) {
        require(_msgSender() == rblsContractAddress, "It may be called only by the RBLS contract");
        rblsContract.transfer(depositWallet, value);
