...
import "./token/IERC721GatewayV2.sol";
import "./utils/TransactedEventHash.sol";
import "@openzeppelin/contracts/token/ERC721/utils/ERC721Holder.sol";

contract GatewayV2 is
    IERC721GatewayV2,
    AccessOperatable,
    ERC721Holder,
    TransactedEventHash
{
    ...
    function approve(
        ...
        uint256 _tokenId,
        bytes32 _eventHash
    ) public override onlyRole(OPERATOR) {
        if (isTransactedEventHash(_eventHash)) return;

        IERC721Asset assetContract = IERC721Asset(_assetContract);
        assetContract.approve(_to, _tokenId);
    ...
    function isTransactedEventHash(bytes32 _eventHash)
        public
        view
        override(IERC721GatewayV2, TransactedEventHash)
        returns (bool)
    {
