    ...
    ReentrancyGuardUpgradeable
{
    ...
    uint256 public constant LENGTH = 7 days;
    ...
    function initialize(
        ...
        OmmniToken = IERC20Upgradeable(_ommni);
        settings = msg.sender;
    ...
    function getRreservePrice() public view returns (uint256) {
        uint256 reserve_price = supply.mul(omni_price).add(
            (supply.mul(omni_price)).mul(30).div(100)
        );
        return reserve_price;
    }

    /// @notice buys fractions of the Nft. Must send price in Ommni

    function buyFraction(uint256 _amount) external payable nonReentrant {
        require(
            OmmniToken.balanceOf(msg.sender) >= _amount * omni_price,
            "Not enough balance"
        );
        require(
            OmmniToken.transferFrom(
                msg.sender,
                address(this),
                _amount * omni_price
            ),
            "Ommni Transfer failed"
        );
    ...
    /// @notice kick off an auction. Must send reservePrice in Ommni
    function start(uint256 _amount) external payable nonReentrant {
        require(auctionState == State.inactive, "start:no auction starts");
        require(
            OmmniToken.balanceOf(msg.sender) >= getRreservePrice(),
            "Not enough balance"
        );
        OmmniToken.transferFrom(msg.sender, address(this), _amount);
    ...
    function bid(uint256 _amount) external payable nonReentrant {
        ...
        require(
            OmmniToken.balanceOf(msg.sender) >= _amount,
            "Not enough balance"
        );
        OmmniToken.transferFrom(msg.sender, address(this), _amount);
    ...
    function end() external nonReentrant {
        ...
        if (ISettings(settings).feeReceiver() != address(0)) {
            _sendOmmni(ISettings(settings).feeReceiver(), livePrice / 40);
        }
        emit AuctionEnded(winning, livePrice);
    }

    /// @notice an external function to burn ERC1155 tokens to receive Ommni tokens from ERC721 token purchase
    function cash() external nonReentrant {
        ...
        _sendOmmni(msg.sender, share);
        emit FractionRedeemed(msg.sender, share);
    }

    function _sendOmmni(address _to, uint256 _value) internal {
        OmmniToken.transfer(_to, _value);
    }
