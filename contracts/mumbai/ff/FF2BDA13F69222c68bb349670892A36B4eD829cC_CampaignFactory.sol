...
contract CampaignFactory is OwnableUpgradeable, PausableUpgradeable {
    ...
    IStoreStakingUsers public storeStakingAddress;
    ...
    function initialize(
        ...
        IStoreStakingUsers _storeStakingAddress
        ...
        storeStakingAddress = _storeStakingAddress;
    ...
    /**
     * @notice Owner can set the Snapshot contract
     * @param _storeStaking Address of the Snapshot contract
     */
    function setStoreStakingContract(IStoreStakingUsers _storeStaking) external onlyOwner {
        storeStakingAddress = _storeStaking;
...
contract Campaign is ReentrancyGuard, Pausable, TierSystem, Blacklist {
    ...
    /**
     * @notice Returns the claimed tokens of an address
     * @return claimedTokens Returns amount of tokens the user has claimed
     * @param _address Address to find the claimed tokens
     */
    ...
    {
        claimedTokens = userTokensMapping[_address].tokensClaimed;
    }
        ...
        nonReentrant
    {
        ...
        uint256 amount = userTokensMapping[_msgSender()].tokensBought;
    ...
    function loadCampaign(
        ...
        setTiers(_tier1, _tier2, _tier3, _tier4, _tier0);
...
contract TierSystem is Ownable {
    ...
    event TierSystemSetUp(IStakBank stakBank, IStoreStakingUsers _storeStaking, uint256 openTime);
    ...
    /**
     * @notice Change stakbank tier times
     */
        ...
        returns (uint8 tier, uint256 timestamp)
    {
        ...
        // Phases E = after 240m: non-whitelisted (NO STAK)
         ...
         uint256 senderPolygonBalance = storeStakingAddress.getUsers(_address);
        ...
        if (totalBalance >= tier1 )  {
                ...
                tier = 2;
                timestamp = campaignOpenTime + tier2Time;
        } else if (totalBalance >= tier3) {
                tier = 3;
                timestamp = campaignOpenTime + tier3Time;
        }else if (totalBalance >= tier4 || whitelist[_address]) {
                tier = 4;
                timestamp = campaignOpenTime + tier4Time;
        }else {
                tier = 0;
                timestamp = campaignOpenTime + tier0Time;
        }  
    }
    ...
    function _setupTierSystem(IStakBank _stakBank, IStoreStakingUsers _storeStaking, uint256 _openTime) internal {
        stakBank = _stakBank;
        storeStakingAddress = _storeStaking;
        campaignOpenTime = _openTime;
        emit TierSystemSetUp(stakBank, _storeStaking, campaignOpenTime);
    }
