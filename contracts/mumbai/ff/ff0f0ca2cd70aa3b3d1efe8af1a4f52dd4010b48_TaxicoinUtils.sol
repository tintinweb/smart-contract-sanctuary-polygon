...
contract TaxicoinUtils is OwnableUpgradeable, EIP712MetaTransaction {
    ...
    // Set/Unset if the token address supports Permit
    ...
    // Starts a new ride for user address. If it is a meta tx, the user address is retrieved using signatures,
    // else msg.sender is the user
    function startRide(
    ...
    function completeRide(address _user, uint256 actualRideAmount) public nonReentrant {
        ...
        require(maxAmount != 0, "Invalid ride");
    ...
    // Update ride detail parameters, prior ride must exist for this function to work
    function editRide(
        address _tokenAddress,
        uint256 _maxRideAmount,
        uint256 _rideDeadline
