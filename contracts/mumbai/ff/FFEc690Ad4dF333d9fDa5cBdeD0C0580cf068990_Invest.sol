...
contract Invest is Ownable, ReentrancyGuard {
    ...
    function getFundProjects(string memory _key) public view returns (string[] memory projectNames, uint256[] memory projectWeights) {
        string[] memory _projectNames = funds[_key].projects;
        uint256[] memory _projectWeights = new uint256[](_projectNames.length);
        
        for (uint i = 0; i < _projectNames.length; i++) {
            _projectWeights[i] = weights[_key][_projectNames[i]];
        }
        
        return (_projectNames, _projectWeights);
    }
    ...
    function getInvestors(string memory _key) public view onlyProjectOwner(_key) returns(address[] memory) {
        return projects[_key].investors;
    }

    function setPayableToken(address _token) public onlyOwner {
        payableToken = _token;
    }
    ...
    function setProjectPrice(string memory _key, uint256 _price, uint256 _decimals) public onlyOwnerOrSigner {
        projects[_key].price = _price;
        projects[_key].decimals = _decimals;
    }

    function setReleaseToken(string memory _key, address _token) public onlyProjectOwner(_key) {
    ...
    function removeProjectFromFund(string memory _key, string memory _projectKey) public onlyFundOwner(_key) {
    ...
    function updateProjectWeightInFund(string memory _key, string memory _projectKey, uint256 _weight) public onlyFundOwner(_key) {
        require(!projects[_projectKey].stopped, "Project is already stopped for investments");
    ...
    function stopFundInvestments(string memory _key) public onlyFundOwner(_key) {
        require(!funds[_key].stopped, "Fund is already stopped for investments");

        funds[_key].stopped = true;
    }
