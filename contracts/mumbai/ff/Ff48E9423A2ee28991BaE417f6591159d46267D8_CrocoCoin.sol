...
contract CrocoCoin is ERC20, Ownable {
    using SafeMath for uint256;
    uint256 MaxSupply = 1000000000 ether; // 1 000 000 000
    mapping(address => bool) public controllers;

    constructor() ERC20("Croco Coin", "CC") {
        transferOwnership(msg.sender);
        _mint(msg.sender, 500000000 ether); //500 000 000
        _mint(address(this), 500000000 ether); //500 000 000
    }
    ...
    function Burn(uint256 _amount) external onlyOwner {
        require(balanceOf(msg.sender) >= _amount, "BalanceToLow");
        _burn(msg.sender, _amount);
    }

    function payRewards(address _to, uint256 _amount) external onlyController {
        uint256 contractBalance = balanceOf(address(this));
        require(_amount <= contractBalance, "ContractBalanceToLow");
