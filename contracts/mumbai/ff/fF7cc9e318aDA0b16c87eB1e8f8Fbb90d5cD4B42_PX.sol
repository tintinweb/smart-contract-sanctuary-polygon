...
contract PX is ERC721URIStorage {
    ...
    constructor() ERC721 ("PollinationX", "PXS"){
        pxStoragePackage[_packageIds.current()].name = "100MB";
        pxStoragePackage[_packageIds.current()].price = 0;
        pxStoragePackage[_packageIds.current()].size = freePackageSize;
        pxStoragePackage[_packageIds.current()].sizeInBytes = 104857600;
        pxStoragePackage[_packageIds.current()].storageUnit = "MB";
        pxStoragePackage[_packageIds.current()].active = 1;
        pxStoragePackage[_packageIds.current()].creationTimestamp = block.timestamp;
    }
    ...
    function addNewPackage(uint256 priceInWeiValue, uint256 sizeInGb) external onlyOwner() {
        ...
        pxStoragePackage[_packageIds.current()].name = string(abi.encodePacked(sizeInGb, "GB"));
        pxStoragePackage[_packageIds.current()].price = priceInWeiValue;
        pxStoragePackage[_packageIds.current()].size = sizeInGb;
        pxStoragePackage[_packageIds.current()].sizeInBytes = sizeInGb.mul(_gbToBytes);
        pxStoragePackage[_packageIds.current()].storageUnit = "GB";
        pxStoragePackage[_packageIds.current()].active = 1;
        pxStoragePackage[_packageIds.current()].creationTimestamp = block.timestamp;
    }
    function upgradeTokenPackage(uint256 tokenId, uint256 packageId) public {
        require(_exists(tokenId), "Incorrect tokenId");
        require(_isApprovedOrOwner(_msgSender(), tokenId),"ERC721: not NFT owner");
