...
// File: contracts/NftES1155/IERC165.sol
...
// File: contracts/NftES1155/IERC1155.sol
...
// File: contracts/NftES1155/IERC1155Receiver.sol
...
// File: contracts/NftES1155/Address.sol
...
// File: contracts/NftES1155/Context.sol
...
// File: contracts/NftES1155/ERC165.sol
...
// File: contracts/NftES1155/ERC1155.sol
...
// File: contracts/NftES1155/ERC1155Burnable.sol
...
abstract contract ERC1155Burnable is ERC1155 {
    //AH, changed value to noOfCopies that needs to be burned
    function burn(
        address account,
        uint256 id,
        uint256 noOfCopies
...
// File: contracts/NftES1155/NftES1155Royalties.sol
...
// File: contracts/NftESAuctions/NftES1155Auctions.sol
...
// File: contracts/NftES1155/NftES1155.sol
...
contract NftES1155 is ERC1155, ERC1155Burnable, NftESErc20 ,NftES1155Auction { 
    ...
    */
    ...
    // AH, In function below, amount is replaced by noOfCopies that needs to be minted
    function simpleMint (uint nftId, uint noOfCopies,  bytes memory data, string memory tokenURI, uint RoyaltyValueOfMinter ) contractIsNotPaused TokenNotExist(nftId) public {
        _mint(_msgSender(), nftId, noOfCopies, data);
    ...
    function LocalMintedNfts (address to, uint id, uint amount, bytes memory data, string memory tokenURI, uint NftPrice, address payable MinterAddress, uint RoyaltyValueOfMinter) public payable{
        ...
        // Deduct Royalties
        _royaltyAndNftESFee(NftPrice*amount, RoyaltyValueOfMinter, MinterAddress, MinterAddress );
    }
    ...
    */
    ...
    // AH, In function below, amount is replaced by noOfCopies that needs to be minted
    function mintForOpenBidding (uint nftId, uint noOfCopies,  bytes memory data, string memory tokenURI, uint RoyaltyValueOfMinter ) contractIsNotPaused TokenNotExist(nftId) external{
        simpleMint (nftId,  noOfCopies,  data, tokenURI, RoyaltyValueOfMinter );
        _placeNftForBids(_msgSender(),nftId);
    }
    // AH, In function below, amount is replaced by noOfCopies that needs to be minted
    function mintForFixedPrice (uint nftId, uint noOfCopies,  bytes memory data, string memory tokenURI, uint RoyaltyValueOfMinter, uint fixPriceOfNft ) contractIsNotPaused TokenNotExist(nftId) external{
        simpleMint (nftId,  noOfCopies,  data, tokenURI, RoyaltyValueOfMinter );
    ...
    // AH, In function below, amountOfNFT is replaced by noOfCopiesofNFT
    function purchaseAgainstFixedPrice ( address from, address to, uint nftId, uint noOfCopiesofNFT) external payable{
        if (deposits[_msgSender()] < NFT_Price[from][nftId]*noOfCopiesofNFT){
            depositBidAmmount(_msgSender(), msg.value);
        }
        require(NFT_Price[from][nftId]*noOfCopiesofNFT <= deposits[_msgSender()] && noOfCopiesofNFT > 0, "Error while Purchasing" );
        deductAmount(_msgSender(), NFT_Price[from][nftId]*noOfCopiesofNFT);
        NftESsafeTransferFrom(from,  to,  nftId, noOfCopiesofNFT, "Data");
    }
