        ...
        onlyRole(HOUSE_POOL_DATA_PROVIDER)
    {
        int256 tempPendingStakes;
            for(uint256 i= 0; i <betinformation.length; i++) {
                tempPendingStakes += int256(betinformation[i].amount * 10**precisionDifference);
        ...
        onlyRole(HOUSE_POOL_DATA_PROVIDER)
    {
        ...
        for(uint256 i =0; i < _Id.length; i++) {
            if (_result[i] == 1) {
                ...
                liquidity -= (betInfoWithId[_Id[i]].payout * 10**precisionDifference +
                    betInfoWithId[_Id[i]].amount * 10**precisionDifference);
                bool success = stakedToken.transfer(
                    lionGamingContract,
                    betInfoWithId[_Id[i]].payout
                );
                if (success) {
                    pendingStakes -= int256(betInfoWithId[_Id[i]].amount * 10**precisionDifference);
                ...
                betInfoWithId[_Id[i]].result = _result[i];
                liquidity += betInfoWithId[_Id[i]].amount * 10**precisionDifference;
                pendingStakes -= int256( betInfoWithId[_Id[i]].amount * 10**precisionDifference);
                updateAttributes();
            }
        }
