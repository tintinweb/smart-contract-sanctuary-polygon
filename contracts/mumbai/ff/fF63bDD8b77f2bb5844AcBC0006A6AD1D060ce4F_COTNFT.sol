...
contract COTNFT is ERC721Enumerable, Ownable {
    ...
    function split(uint256 tokenId, string memory imageUrl, NFTInfo[] memory nftArray) public {
        ...
        NFTInfo storage splitNft = tokenInfoMap[tokenId];
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        string memory imageUrl = tokenInfoMap[tokenId].imageUrl;
        return imageUrl;
    }
    ...
    function getNFTList(address owner) public view returns (NFTInfo[] memory nftInfoList) {
        uint256 balance = balanceOf(owner);
        
        nftInfoList = new NFTInfo[](balance);
        for (uint i = 0; i < balance; i++) {
            uint256 tokenId = tokenOfOwnerByIndex(owner, i);
            nftInfoList[i] = getNFTInfo(tokenId);
        }
    }
