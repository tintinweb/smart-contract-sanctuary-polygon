...
contract T222 is Ownable, TimeForDay, SortList {
    ...
    function updateTotalADAList(uint256 _amount, uint256 _type) private {
        ...
        if (num == 0) {
            ...
            if (_type == 0) {
                totalShareAllocationList[tatalInvestNumber]._totalAD1A += currentADA;
            } else if (_type == 1) {
                totalShareAllocationList[tatalInvestNumber]._totalAD2A += currentADA;
            } else if (_type == 2) {
                totalShareAllocationList[tatalInvestNumber]._totalAD3A += currentADA;
            } else if (_type == 3) {
                totalShareAllocationList[tatalInvestNumber]._totalAD4A += currentADA;
            }
        }
    }
    ...
    function getDaoReward2(address _user, uint256 _type) public view returns (uint256 _totalReward, bool isDao, UserDaoItem memory x, uint256 _investId, uint256 _daoType, bool _ok) {
        isDao = (userInfoList[_user]._daoItem.isDao1 || userInfoList[_user]._daoItem.isDao2 || userInfoList[_user]._daoItem.isDao3 || userInfoList[_user]._daoItem.isDao4) ? true : false;
        x = userDaoList[_user];
        _investId = x._investId;
        _daoType = x._daoType;
        _ok = tatalInvestNumber == 0 || _type > 3 || !isDao || _type != _daoType;
        if (_ok) {
            _totalReward = 0;
        }
        if (_type == 0) {
