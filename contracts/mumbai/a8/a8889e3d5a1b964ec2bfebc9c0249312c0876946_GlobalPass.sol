...
contract GlobalPass is ERC1155, Ownable, Pausable, ERC1155Burnable, GlobalPassAuction,GlobalPassRoyalties, ReentrancyGuard {
   ...
   mapping(address=> mapping(uint => uint)) public CopiesOnBidOrSale;
    ...
    function getTicketTypeDetails(uint _ticketID) public view returns(TicketType memory Ticket){
        return   ticketTypeDetails[_ticketID];
    }
    ...
    // tickets will be created on time of event registration
     function registerEvent(
        ...
        addTicketType(  _eventID, _TicketTypes /*_auctionStartTimes, _auctionEndTimes */) ;
        ...
        for(uint8 i=0; i < _ticketTypes.length ; i++)
         {
            ...
            for(uint j=0; j<_ticketTypes[i].validDates.length; j++){
                require(eventDetails[eventID].eventStartDate <= _ticketTypes[i].validDates[j] && eventDetails[eventID].eventEndDate >= _ticketTypes[i].validDates[j], "Validity dates must be withing Event Dates");
            }
    ...
    function updateTicket(
        uint _eventID,
        uint _ticketID,
        uint _newTicketCopies,
        uint _ticketFixPrice,
        uint32[] memory _validDates,
        ...
        uint _servicePercentage
            ...
            require(ticketExists[_ticketID] == true, "Ticket Does not Exist");
            require(_newTicketCopies >= ticketTypeDetails[_ticketID].copiesSold, "These no of copies have already been sold");
            ticketTypeDetails[_ticketID].totalCopies = _newTicketCopies;
            ticketTypeDetails[_ticketID].ticketFixPrice = _ticketFixPrice;
            
            for(uint j = 0; j<_validDates.length; j++)
                 ticketTypeDetails[_ticketID].validDates = _validDates;

             ticketTypeDetails[_ticketID].royaltyPercentage = _royaltyPercentage;
             ticketTypeDetails[_ticketID].servicePercentage = _servicePercentage;
