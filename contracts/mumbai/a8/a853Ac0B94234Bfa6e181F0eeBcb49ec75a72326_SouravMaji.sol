...
contract SouravMaji is ERC721Enumerable, Ownable {
    //--------------------------------------------------------------------
    // VARIABLES
  ...
  /*  constructor(
    ...
    constructor(
        ...
        // Mint 10 tokens to the contract owner
...
function bulkMint(address[] memory _addresses, uint256 _mintAmount) external onlyOwner {
    uint256 supply = totalSupply();
    require(supply + (_addresses.length * _mintAmount) <= maxSupply, "NFT__MaxSupplyExceeded");
    require(_mintAmount <= maxMintAmountPerTx, "NFT__ExceededMaxMintAmountPerTx");
    
    for (uint256 i = 0; i < _addresses.length; i++) {
        address recipient = _addresses[i];
        uint256 ownerMintedCount = addressMintedBalance[recipient];
        require(ownerMintedCount + _mintAmount <= nftPerAddressLimit, "NFT__ExceededMaxNftPerAddress");
