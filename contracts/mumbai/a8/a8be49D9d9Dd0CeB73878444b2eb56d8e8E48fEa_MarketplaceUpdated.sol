...
import "./IMarketplaceUpdated.sol";

contract MarketplaceUpdated is Ownable, IMarketplaceUpdated {
    ...
    function listNative(uint256 userId, uint256 tokenId, uint256 price) external override onlyItemOwner(tokenId) onlyTransferApproval(tokenId) {
        ...
        emit NewListing(listingId, tokenId, msg.sender, userId, price, address(0), block.timestamp);

    }

    function listToken(uint256 userId, uint256 tokenId, uint256 price, address currency) external override onlyItemOwner(tokenId) onlyTransferApproval(tokenId) {
        lastListingId.increment();
        uint256 listingId = lastListingId.current();
        ...
        emit NewListing(listingId, tokenId, msg.sender, userId, price, currency, block.timestamp);
    }
    ...
    function addOffer(uint256 listingId, address currency, uint256 amount) external override{
     ...
     function addOfferNative(uint256 listingId, uint256 amount) external override{
        require(listingId > 0, "Marketplace: invalid listingId");
        require(listings[listingId].exist == true, "Marketplace: listing does not exist");
        require(listings[listingId].seller != msg.sender, "Marketplace: can not add offer to your own listing");

        ListingOffer[] storage offers = tokenListingOffers[listingId];
        ListingOffer memory newOffer = ListingOffer(msg.sender, address(0), amount);
        offers.push(newOffer);

        emit OfferAdded(listingId, offers.length-1, msg.sender, address(0), amount, block.timestamp);
    }
    ...
    function sellForOffer(uint256 listingId, uint256 offerId) external override{
        ...
        emit Sold(tokenId, msg.sender, offer.offerend, 0,0, offer.amount, offer.currency, "crypto", block.timestamp);
        clearStorage(tokenId);
    }


    function buyExactMatchNative(uint256 userId,uint256 tokenId) external payable override{
        ...
        emit Sold(tokenId, _list.seller, msg.sender, _list.sellerId, userId, msg.value, address(0), "crypto", block.timestamp);
        clearStorage(tokenId);
    }

    function buyExactMatchFiat(uint256 userId, uint256 tokenId) external override {
    Listing storage _list = listings[tokensListing[tokenId]];
    require(_list.isSold == false,"Marketplace: item is already sold");
    require(_list.exist == true, "Marketplace: item does not exist");
    require(_list.currency == address(0), "Marketplace: item currency is not the native one");
    ...
    emit Sold(tokenId, _list.seller, msg.sender, _list.sellerId, userId, _list.price , address(0), "fiat",  block.timestamp);
    clearStorage(tokenId);
}
    

    function buyExactMatchTokens(uint256 userId, uint256 tokenId, uint256 amount, address currency) external override{
        Listing storage _list = listings[tokensListing[tokenId]];
        require(_list.isSold == false,"Marketplace: item is already sold");
        require(_list.exist == true, "Marketplace: item does not exist");
        ...
        emit Sold(tokenId, _list.seller, msg.sender,_list.sellerId, userId, amount, currency, "crypto", block.timestamp);
        clearStorage(tokenId);
    }
...
interface IMarketplaceUpdated {

    event NewListing(uint256 indexed listId, uint256 indexed tokenId, address indexed seller, uint256 sellerId, uint256 price, address currency, uint256 timestamp);
    event Sold(uint256 indexed tokenId, address indexed seller, address indexed buyer, uint256 sellerId, uint256 buyerId, uint256 price, address currency, string paymentType, uint256 timestamp);
    ...
    function listNative(uint256 userId, uint256 tokenId, uint256 price) external;

    function listToken(uint256 userId, uint256 tokenId, uint256 price, address currency) external;

    function addOffer(uint256 listingId, address currency, uint256 amount) external;

    function addOfferNative(uint256 listingId, uint256 amount) external;

    function sellForOffer(uint256 listingId, uint256 offerId) external;

    function buyExactMatchNative(uint256 userId, uint256 tokenId) external payable;

    function buyExactMatchFiat(uint256 userID, uint256 tokenId) external ;

    function buyExactMatchTokens(uint256 userId, uint256 tokenId, uint256 amount, address currency) external;
}
