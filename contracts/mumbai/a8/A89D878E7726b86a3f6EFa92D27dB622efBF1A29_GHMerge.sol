...
/// @custom:security-contact [email protected]
contract GHMerge is Pausable, AccessControl, AdminPayable {
    ...
    function startMerge(string[] calldata tokens_, uint256[] calldata amounts_, uint256[] calldata items_) public payable {
        ...
        for(uint256 i = 0; i < tokens_.length; i++) {
            require(
                currencies[tokens_[i]] != address(0),
                string.concat("Unknown Currency ",tokens_[i]) 
            );
            ERC20 target = ERC20(currencies[tokens_[i]]);
            address[] memory _admins;
            uint256[] memory _values;
            (_admins,_values) = getAdminsShare(amounts_[i]);
            for(uint256 j =0; j<_admins.length; j++){
                target.transferFrom(msg.sender,_admins[j],_values[j]);
            }
        }
        gotchi.merge{value: msg.value}(items_);
        emit MergeStarted(tokens_,amounts_,items_);
    }

    event MergeStarted(string[],uint256[],uint256[]);

    //add or replacy ERC20 currency to contract
    function addCurrency(string calldata name_, address contract_) public onlyRole(DEFAULT_ADMIN_ROLE) {
        currencies[name_] = contract_;
    }
