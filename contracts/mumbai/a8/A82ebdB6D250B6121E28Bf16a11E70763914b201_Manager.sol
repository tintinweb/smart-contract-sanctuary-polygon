...
contract FunctionsUptimeChecker is FunctionsClient, ConfirmedOwner {
    ...
    function fulfillRequest(
        ...
        latestError = err;
        SLA slaContract = SLA(requestIdSlaMap[requestId]);
        slaContract.completeCheckSLACompliance(requestId, err, response);
        emit OCRResponse(requestId, response, err);
    }
...
contract Manager is IManager {
    ...
    function addConsumer(address _consumerAddress, string memory _slaName)
        public
        payable
    {
        require(
    ...
    function getProviderSLAs(address _providerAddress)
        public
        view
        returns (SLAContract[] memory)
    {
        return providerSLAs[_providerAddress];
    }

    // get all SLAs of a consumer
    function getConsumerSLAs(address _consumerAddress)
        public
        view
        returns (SLAContract[] memory)
    {
    ...
    function addAuthorizedWithdrawer(address _withdrawer)
        public
    ...
    function removeAuthorizedWithdrawer(address _withdrawer)
        public
        onlyAuthorizedWithdrawer
    {
...
contract SLA {
    ...
    function completeCheckSLACompliance(
        ...
        uint256 wasApiSuccess = sliceUint(response, 0);
        if (wasApiSuccess != 1) {
