    ...
    PausableUpgradeable
{
    ...
    /// @custom:oz-renamed-from totalSubscriptions
    ...
    function autoClaim() external whenNotPaused onlyOperator {
        ...
        for (uint256 index = 0; index < activeSubscriptions.length; index++) {
            ...
            if (block.timestamp >= subscription.nextTimeClaim) {
    ...
    function _calculateClaimableAmount(uint256 totalAmount, uint256 totalDays)
        ...
        returns (uint256 claimableAmount)
    {
        claimableAmount = (totalAmount / totalDays);
    }

    function _calculateRemainingDays(uint256 endTime, uint256 nowTime)
        internal
        pure
        returns (uint256 currentRemainingDays)
    {
        currentRemainingDays = (endTime - nowTime) / day + 1;
    }
        ...
        returns (uint256 completedSub)
    {
        ...
        if (blockTime >= subscription.endTimeVesting) {
            claimableAmount = subscription.totalAmount;
            subscription.vestedAmount += claimableAmount;
            subscription.totalAmount -= claimableAmount;
            subscription.numberOfclaimed += 1;
            subscription.nextTimeClaim = 0;
            subscription.isActive = false;
        } else {
            uint256 currentRemainingDays = _calculateRemainingDays(
                subscription.endTimeVesting,
                block.timestamp
            );
