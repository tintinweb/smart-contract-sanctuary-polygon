...
contract Webston {
    ...
    function _setb14self(uint256 poolNo,address _referral,address _addr,uint256 _amount) private{
        ...
        if(players[_referral].b14_level[poolNo][0]<2){
            if(players[_referral].b14_level[poolNo][0]==0){
                players[_referral].b14_lr[poolNo][0]=_addr;
            }else if(players[_referral].b14_level[poolNo][0]==1){
                players[_referral].b14_lr[poolNo][1]=_addr;
            }
        }
        else{
    ...
    function b14TeamAddress(address _addr,uint8 pool) view external returns(address[3] memory p) {
        for(uint8 i=0;i<=2;i++){
            p[i]=players[_addr].b14_lr[pool][i];
        }
        return (
           p
        );
