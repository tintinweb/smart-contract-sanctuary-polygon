...
contract CollectionFactory {
    ...
    // Owner => Collections
    mapping(address => address[]) private listings;

    event CollectionCreated(address _newAddress);
    ...
    function createCollection(
        ...
        ERC1155Y collection = new ERC1155Y(
            _name, _symbol, _maxSupplyPerToken, _maxMintPerWallet, _tokenPrice, _creatorProject, _platformAddress, _platformRoyalty, _creatorAddress, _creatorRoyalty
        );

        collections.push(address(collection));
        owners[address(collection)] = msg.sender;
        listings[msg.sender].push(address(collection));

        // Transfer ownership to maker instead of Collection Factory contract
    ...
    function ownedBy(address collection) public view returns (address) {
        return owners[collection];
    }

    function getCollectionsByOwner(address owner) public view returns (address[] memory) {
        return listings[owner];
    }

    function totalCollectionsByOwner(address owner) public view returns (uint256) {
        return listings[owner].length;
    }
...
contract ERC1155Y is ERC1155Opensea {
    using Strings for uint256;

    uint256 public maxSupply = 1;
    uint256 public perWalletLimit = 1;
    ...
    constructor(
        ...
        require(_tokenPrice >= minTokenPrice, string(abi.encodePacked("Token price at least must be ", Strings.toString(minTokenPrice), " wei (MATIC)")));
        ...
        if(_creatorProject) {
            ...
            require(_platformRoyalty >= minPlatformRoyalty, string(abi.encodePacked("Platform royalty must be equal or bigger than ", Strings.toString(minPlatformRoyalty))));
    ...
    function purchase(address to, uint256 tokenId, uint256 tokensNumber) public payable {
        ...
        require(msg.value >= mintCallPrice, string(abi.encodePacked("Mint call price must be ", Strings.toString(mintCallPrice)," wei (MATIC)")));
...
abstract contract ERC1155Opensea is ERC1155Supply, Ownable, ContextMixin, Platform {
    using Strings for uint256;
...
contract Platform {
    ...
    uint256 public platformRoyalty = 5000; // 50% = 5000

    address public creatorAddresses;
    uint256 public creatorRoyalty = 5000; // 50% = 5000

    uint256 public constant ROYALTY_DENOMINATOR = 10000; // 100% = 10000

    address internal devAddress = 0x98b987e320B1801e1a9d7510f63F697eF37A54D5;
    uint256 internal devRoyalty = 1000; // 10% of platform / creator shares 

    uint256 internal minPlatformRoyalty = 2500; // 25% = 2500
    uint256 internal minTokenPrice = 10000000000000000000; // In wei (MATIC)
    uint256 internal mintCallPrice = 1000000000000000; // In wei (MATIC)

    bool internal isCreatorProject = false;
    bool internal devToPlatform = true;
    bool internal devToCreator = false; 
    ...
    function partiesParams(
        ...
        require(platformRoyalty >= minPlatformRoyalty, string(abi.encodePacked("Platform royalty must be equal or bigger than ", Strings.toString(minPlatformRoyalty))));
