...
contract BotMinter is Managable, Pausable, ReentrancyGuard {
    ...
    function breed(uint256 _matronId, uint256 _sireId, address _token) external whenNotPaused nonReentrant returns(uint256) {
        require(_token != ZERO_ADDRESS, "zero address token");
        (uint _tokenId, uint _tokenPrice, uint _oilPrice ) = _breed(_matronId, _sireId, _token);

        _transferPayments(_token, _tokenPrice, _oilPrice);
    ...
    function breedWithMATIC(uint256 _matronId, uint256 _sireId) external whenNotPaused nonReentrant payable returns(uint256) {
        ...
        (uint _tokenId, uint _tokenPrice, uint _oilPrice ) = _breed(_matronId, _sireId, ZERO_ADDRESS);

        require(msg.value == _tokenPrice, "not enough native token");
        (bool success,) = treasuryAddress.call{value: msg.value}(
            abi.encodeWithSignature("split(address,address,uint256)", ZERO_ADDRESS, msg.sender, _tokenPrice)
        );
    ...
    function breedWithEarlyClaim(
        ...
        require(_earlyClaim.parts.length <= 2, "wrong token number");

        (uint _tokenId, uint _tokenPrice, uint _oilPrice ) = _breed(_matronId, _sireId, _token);

        IRewardsSpender.Rewarder memory _rewarder1 = IRewardsSpender(rewardsSpenderAddress).rewarders(_earlyClaim.parts[0].name);
        require(_rewarder1.addr == _token || _rewarder1.addr == oilAddress, "wrong token");
        _checkToken(_rewarder1.addr,_tokenPrice,_oilPrice,_earlyClaim.parts[0].amountUserWallet,_earlyClaim.parts[0].amountClaim, _token);
        ...
        if(_earlyClaim.parts.length == 2){
            ...
            require(_rewarder2.addr == _token || _rewarder2.addr == oilAddress, "wrong token");
            require(_rewarder1.addr != _rewarder2.addr, "same token");
            _checkToken(_rewarder2.addr,_tokenPrice,_oilPrice,_earlyClaim.parts[1].amountUserWallet,_earlyClaim.parts[1].amountClaim, _token);
        }
        {
    ...
    function _breed(uint256 _matronId, uint256 _sireId, address _token) internal returns(uint256, uint256, uint256) {
        ...
        return (tokenId, _tokenPrice, (_matronPrice + _sirePrice));
    }
