...
contract InfiniPack is ERC1155Drop, SignatureAction {
    ...
    function burnWithSignature(GenericRequest calldata _req, bytes calldata _signature)
        ...
        returns (address signer)
    {
        // Make a generic request from the Burn Request
    ...
    function extractBurnData(GenericRequest calldata _req)
        public
        pure
        returns (address from, uint256 id, uint256 amount)
    {
        (from, id, amount) = abi.decode(_req.data, (address, uint256, uint256));

        return (from, id, amount);
    }
