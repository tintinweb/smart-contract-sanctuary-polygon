...
contract BossQuests is Multicall, HSystemChecker {
    ...
    function initiateBossQuest(
        ...
        // convert gwei to wei
        _treasury.burn(quester, uint256(bossTable.milkRequired) * 1 gwei);

        // burn items
        uint16[] memory mandatoryItems = bossTable.mandatoryItems;
        ...
        )
    {
        ...
        mandatoryItems = bossTable.mandatoryItems;
    }
    ...
    function setGildedBP(uint256 number) external onlyRole(ADMIN_ROLE) {
        require(number < 10000, 'BQ 103 - The basis points should be less than 10000');
    ...
    function setGildedAndAnimatedBP(uint256 number) external onlyRole(ADMIN_ROLE) {
        require(number < 10000, 'BQ 104 - The basis points should be less than 10000');
    ...
    function _addBossQuest(
        ...
        require(milkRequired > 0, 'BQ 105 - MILK can not be 0');
        // enforce a max item req
        require(mandatoryItems.length < 9, 'BQ 106 - Max total item req is 8');

        _bossIdToBossTable[bossQuestId] = BossTable(0, milkRequired, mandatoryItems);
    }
}
