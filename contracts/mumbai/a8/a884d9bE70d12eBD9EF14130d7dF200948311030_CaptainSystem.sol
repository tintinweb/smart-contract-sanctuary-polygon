...
contract CaptainSystem is ICaptainSystem, GameRegistryConsumerUpgradeable {
     ...
     */
    function setCaptainNFT(address tokenContract, uint256 tokenId)
        external
        whenNotPaused
        nonReentrant
    {
        ...
        // Remove hold on old captain
        if (captainNFT.reservationId > 0) {
            lockingSystem.removeNFTReservation(
                captainNFT.tokenContract,
                captainNFT.tokenId,
                captainNFT.reservationId
            );
            captainNFT.reservationId = 0;
        }
        ...
        if (tokenContract != address(0)) {
            ...
            if (IGameNFT(tokenContract).ownerOf(tokenId) != account) {
                revert NotOwner();
            }
            ...
            // Non exclusive hold on NFT
            captainNFT.reservationId = lockingSystem.addNFTReservation(
                tokenContract,
                tokenId,
                false,
                0
    ...
    function getLastSetCaptainTime(address account)
        external
        view
        returns (uint256)
    {
