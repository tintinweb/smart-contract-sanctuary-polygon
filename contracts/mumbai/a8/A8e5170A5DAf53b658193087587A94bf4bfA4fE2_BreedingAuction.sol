...
contract BreedingAuction is ClockAuction {
    ...
    function createAuction(
        ...
        require(isOnAuction(_tokenId) == false, "BreedingAuction: Already on Auction");
...
contract ClockAuction is Pausable, Ownable, ClockAuctionBase {
    ...
    ///  ownership of the NFT if enough Ether is supplied.
    /// @param _tokenId - ID of token to bid on.
    function bid(uint64 _tokenId) external payable virtual whenNotPaused {
        // _bid will throw if the bid or funds transfer fails
        _bid(_tokenId, msg.value);
