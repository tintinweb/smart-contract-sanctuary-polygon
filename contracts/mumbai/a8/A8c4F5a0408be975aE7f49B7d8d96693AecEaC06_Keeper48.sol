...
contract Keeper48 is AutomationCompatibleInterface, Ownable {
        ...
        returns (bool upkeepNeeded, bytes memory performData)
    {
        ...
        while (i < _n - 1) {
            points[i] = oracle.getAmountOracle(oracleTimestamp);
            oracleTimestamp += 3600;
            i++;
        }

        points[i] = oracle.getAmountOracle(block.timestamp);
        ...
        return (k > 0, abi.encode(token0, token1, block.timestamp, lm));
    }

    function performUpkeep(bytes calldata performData) external override {
        (address token0, address token1, uint256 lastTimestamp, int256 lm) = abi
            .decode(performData, (address, address, uint256, int256));
...
library OracleUtils {
    function getAmountOracle(
        ...
        amount = oracle.getAnswer(roundIndex);
    }
}
