...
contract DoubleUp is Ownable, ReentrancyGuard {
    ...
    event BalanceUpdate(uint balance, uint bonus);
    event MessageUpdate(address indexed target, string message);

    constructor(){
        entries[msg.sender] = Entry(address(0), maximumEntry, 0, "Crypto Dungeon Team Starting Entry! Best of luck one and all! <3 Keep an eye open for our upcoming token launch: Blood Souls (SOULS).");
        entriesLength = 1;
        head = msg.sender;
        tail = msg.sender;
    }
    ...
    function enterImplementation(uint256 entry, uint256 immunityLevel, string calldata message, address msgSender) private {
        ...
        for(uint max = 0;max < 6 && head != address(0) && ((current.collateral * winMultiple) >= currentActionableBalance);++max){
            uint256 amountWon = current.collateral * winMultiple;
        ...
        emit BalanceUpdate(currentActionableBalance, bonusPrize);
