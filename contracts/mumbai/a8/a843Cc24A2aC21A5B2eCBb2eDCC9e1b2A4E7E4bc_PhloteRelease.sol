...
contract PhloteRelease is ERC721A, Ownable {
    ...
    uint256[] public cosignerPrice = [50,60,70,80,90];
    ...
    function cosignerMint() external payable whenSaleIsActive {
        ...
        require(
                phloteToken.allowance(msg.sender, address(this)) >= cosignerPrice[0],
                "Phlote token allowance is too low"
            );
        require(phloteToken.transferFrom(msg.sender, address(this), cosignerPrice[0]));

        if(totalSupplyLeft == 0 && cosigners.length == 0){
