...
//contract NFTMarketplace is ERC1155 {
    ...
    function createToken(string memory creator, string memory tokenURI, uint256 price, uint _flag, uint tier, uint num, address royalty, uint _time) public returns (uint) {
        ...
        for(uint i = 0; i < actualcount; i++)
        {
          ...
          NftTkn.mint(msg.sender, newTokenId);
              ...
              if(idToMarketItem[tokenId].isAuction == false){
                if(idToMarketItem[tokenId].isMarket == true){
                  ...
                  emit MarketTrx(tokenId,msg.sender,address(0),0,
                                  "Cancled NFT token on market place",
                                  block.timestamp
                  );
                }
              }
