...
import "./CedarERC721DropV1.sol";
import "../api/deploy/ICedarDeployer.sol";
import "../SignatureVerifier.sol";

contract CedarERC721DropV1Factory is Ownable, ICedarDeployerEventsV4, ICedarImplementationVersionedV0 {
    CedarERC721DropV1 public implementation;
    ...
    constructor(address _greenlistManagerAddress) {
        ...
        implementation = new CedarERC721DropV1();

        implementation.initialize(_msgSender(), "default", "default", "", new address[](0), address(0), address(0), 0, "0", address(0), address(0));
        (uint256 major, uint256 minor, uint256 patch) = implementation.implementationVersion();
        emit CedarImplementationDeployed(address(implementation), major, minor, patch, "CedarERC721DropV1");
    }

    function emitEvent(
        EventParams memory params
    ) private {
        emit CedarERC721DropV1Deployment(params.contractAddress, params.majorVersion, params.minorVersion, params.patchVersion, params.defaultAdmin, params.name, params.symbol, params.contractURI, params.trustedForwarders, params.saleRecipient, params.royaltyRecipient, params.royaltyBps, params.userAgreement, params.signatureVerifier, params.greenlistManagerAddress);
    }
    ...
    function deploy(
    ...
    ) external onlyOwner returns (CedarERC721DropV1 newClone) {
        newClone = CedarERC721DropV1(Clones.clone(address(implementation)));
        SignatureVerifier signatureVerifier = new SignatureVerifier(_userAgreement, "_", "_");
        newClone.initialize(_defaultAdmin, _name, _symbol, _contractURI, _trustedForwarders, _saleRecipient, _royaltyRecipient, _royaltyBps, _userAgreement, address(signatureVerifier), greenlistManagerAddress);
        ...
        params.signatureVerifier = address(signatureVerifier);
        params.greenlistManagerAddress = greenlistManagerAddress;
...
import "../generated/impl/BaseCedarERC721DropV1.sol";
    ...
    BaseCedarERC721DropV1
{
    ...
    {
        if(index >= baseURIIndices.length) revert InvalidIndex();

        uint256 _index = baseURIIndices[index];
        bytes memory encryptedURI = encryptedBaseURI[_index];

        if(encryptedURI.length == 0) revert NothingToReveal();
    ...
    function claim(
        ...
        uint[] memory tokens = CedarDropERC721ClaimLogicV0.executeClaim(claimData, _receiver, _quantity, _currency, _pricePerToken, _proofs, _proofMaxQuantityPerTransaction, _msgSender());
...
// Filename: './BaseCedarERC721DropV1.sol'
...
abstract contract BaseCedarERC721DropV1 is ICedarERC721DropV1 {
    function supportedFeatures() override public pure returns (string[] memory features) {
        ...
        features[5] = "lazymint/ICedarNFTLazyMint.sol:ICedarNFTLazyMintV0";
...
library CedarDropERC721ClaimLogicV0 {
    ...
    function executeClaim(
        ...
        */
        ...
        internalData.toVerifyMaxQuantityPerTransaction = _proofMaxQuantityPerTransaction == 0 ||
        ...
        if (internalData.validMerkleProof && _proofMaxQuantityPerTransaction > 0) {
            ...
            */
            claimData.claimCondition.limitMerkleProofClaim[internalData.activeConditionId].set(internalData.merkleProofIndex);
        }
        ...
        uint256[] memory tokens = transferClaimedTokens(claimData, internalData.activeConditionId, _quantity, msgSender);

        emit TokensClaimed(internalData.activeConditionId, msgSender, _receiver, internalData.tokenIdToClaim, _quantity);
    ...
    function verifyClaimMerkleProof(
        ...
        if (currentClaimPhase.merkleRoot != bytes32(0)) {
            ...
            if(!(!claimData.claimCondition.limitMerkleProofClaim[_conditionId].get(merkleProofIndex))) revert InvalidProof();
            if(!(_proofMaxQuantityPerTransaction == 0 || _quantity <= _proofMaxQuantityPerTransaction)) revert InvalidProof();
        }
  }

    function getActiveClaimConditionId(DataTypes.ClaimData storage claimData) public view returns (uint256) {
        for (uint256 i = claimData.claimCondition.currentStartId + claimData.claimCondition.count; i > claimData.claimCondition.currentStartId; i--) {
    ...
    function verifyClaim(
        ...
        if(!(claimData.maxTotalSupply == 0 || claimData.nextTokenIdToClaim + _quantity <= claimData.maxTotalSupply)) revert CrossedLimit();
        if(!(claimData.maxWalletClaimCount == 0 || claimData.walletClaimCount[_claimer] + _quantity <= claimData.maxWalletClaimCount)) revert CrossedLimit();
        (uint256 lastClaimTimestamp, uint256 nextValidClaimTimestamp) = getClaimTimestamp(claimData, _conditionId, _claimer);
    ...
    function collectClaimPrice(
        ...
        if(!(_currency == CurrencyTransferLib.NATIVE_TOKEN)) revert InvalidPrice();

        CurrencyTransferLib.transferCurrency(_currency, msgSender, claimData.primarySaleRecipient, totalPrice);
    }
    ...
    function getActiveClaimConditions(DataTypes.ClaimData storage claimData) public view returns (IDropClaimConditionV0.ClaimCondition memory condition, uint256 conditionId, uint256 walletMaxClaimCount, uint256 remainingSupply) {
    ...
    function getUserClaimConditions(DataTypes.ClaimData storage claimData, address _claimer) public view returns (uint256 conditionId, uint256 walletClaimedCount, uint256 lastClaimTimestamp, uint256 nextValidClaimTimestamp) {
        conditionId = getActiveClaimConditionId(claimData);
