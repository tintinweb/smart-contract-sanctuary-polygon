...
interface Irouter {
    ...
    function getAmountOut(address swapFrom, address swapTo, uint256 _inPutAmount) external view returns(uint256 _outPutAmount);
    function getLengthOfPeriod(address swapFrom, address swapTo) external view returns(uint256  lnght);
    function getUserDistributionDetails(address swapFrom, address swapTo, address _user) external view returns(uint256[] memory batches, uint256[] memory _time);
...
contract routerV1 is Irouter {    
    ...
    function lockFunds(address swapFrom, address swapTo, uint256 amount) external  {        
        ...
        // Transfer swapFrom from _for to the pair contract
        // If it returns false/didn't work, the
        //  msg.sender may not have allowed the contract to spend swapFrom or
        //  msg.sender or the contract may be frozen or
        //  msg.sender may not have enough swapFrom to cover the transfer.
    ...
    function userBalance( address swapFrom, address swapTo,  address _user) public view returns(uint256 _amount) {
        address pair = IRigelLaunchPadFactory(Factory).getPairedPad(swapFrom, swapTo);
    ...
    function getLengthOfPeriod(address swapFrom, address swapTo) public view returns(uint256  lnght) {
        address pair = IRigelLaunchPadFactory(Factory).getPairedPad(swapFrom, swapTo);
        lnght = IrigelProtocolLanchPadPool(pair).lengthOfPeriod();
        return lnght;
    }

    function numbersOfDistributions(address swapFrom, address swapTo, uint256 _i) public view returns(uint256 _prd, uint256 percent) {
        address pair = IRigelLaunchPadFactory(Factory).getPairedPad(swapFrom, swapTo);
        (_prd, percent) = IrigelProtocolLanchPadPool(pair).period(0, _i );
        return (_prd, percent);
    }

      function getUserDistributionDetails(address swapFrom, address swapTo, address _user) external view returns(uint256[] memory, uint256[] memory) {
        uint256 pLength = getLengthOfPeriod(swapFrom, swapTo);
        uint256[] memory batches = new uint256[](pLength);
        uint256[] memory _time = new uint256[](pLength);
        for (uint256 i = 0; i <= batches.length - 1; i++) {
            (uint256 _period, uint256 _percent) = numbersOfDistributions(swapFrom, swapTo, i);
            uint256 _userAmount = userBalance(swapFrom, swapTo, _user);            
            batches[i] =  (_userAmount * _percent) / 100E18;    
            _time[i] = _period;
        }
        return (batches, _time);
    }
