...
contract SynthesisV2 is EndPoint  {
    ...
    event Synthesized(bytes32 indexed requestId, address to, uint256 amount, address token);
    event Move(address from, address to, uint256 amount, address token, uint64 chainIdTo);
    event Burn(address from, address to, uint256 amount, address token);
    ...
    function mint(
        ...
        emit Synthesized(requestId, to, amount, address(synthImpl));
    }
    ...
    function burn(
        ...
        if (impl.chainIdFrom() != chainIdTo) {
            emit Move(from, to, amount, stoken, chainIdTo);
        } else {
            emit Burn(from, to, amount, stoken);
        }
    }
