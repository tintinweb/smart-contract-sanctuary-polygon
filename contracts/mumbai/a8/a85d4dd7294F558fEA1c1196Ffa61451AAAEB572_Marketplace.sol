...
import {IFantasySportFee} from "../interfaces/IFantasySportFee.sol";

import "../libraries/Currency.sol";
import "../libraries/FeeType.sol";
    ...
    IERC1155ReceiverUpgradeable
{
    ...
    IFantasySportFee public immutable fantasySportFee;

    /// @dev The address that receives all market fees from all sales.
    address private marketFeeRecipient;

    /// @dev The max bps of the contract. So, 10_000 == 100 %
    uint64 private constant MAX_BPS = 10_000;

    /// @dev The % of primary sales collected as market fees.
    ...
    //////////////////////////////////////////////////////////////*/
    constructor(address _nativeTokenWrapper, address _fantasySportFee)
        initializer
    {
        fantasySportFee = IFantasySportFee(_fantasySportFee);
        ...
        onlyListerRoleWhenRestricted
    {
        ...
        emit NewListing(
            ...
            newListing.tokenOwner,
            newListing.assetContract,
            newListing.tokenId,
            newListing.startTime,
            newListing.endTime,
            newListing.quantity,
            newListing.currency,
            newListing.reservePricePerToken,
    ...
    function updateListing(
        ...
        require(safeNewQuantity != 0, "Quantiy must > 0");

        // Can only update auction listing before it starts.
        if (isAuction) {
            require(
        ...
        // TODO: validate start time is not in the past
        ...
        onlyListingCreator(_listingId)
    {
        ...
        require(targetListing.listingType == ListingType.Direct, "!DL");
        ...
        onlyExistingListing(_listingId)
    {
        ...
        targetListing.currency = targetListing.currency == targetOffer.currency
            ? targetOffer.currency
            : targetOffer.currency == nativeTokenWrapper &&
                targetListing.currency == Currency.NATIVE_TOKEN
            ? nativeTokenWrapper
            : targetListing.currency;

        require(targetOffer.expirationTimestamp > block.timestamp, "EXPIRED");
        require(
            targetListing.currency == targetOffer.currency,
            "invalid currency"
        );
    ...
    /// @dev Lets an account
    /// 1. make an offer to a direct listing, or
    /// 2. make a bid in an auction.
    function offer(
        uint256 _listingId,
        ...
        if (targetListing.listingType == ListingType.Auction) {
            ...
            newOffer.quantityWanted = _getSafeQuantity(
                targetListing.tokenType,
                targetListing.quantity
            );
            ...
            // must be made in the listing's desired currency.
            newOffer.currency = _currency == Currency.NATIVE_TOKEN
                ? nativeTokenWrapper
                : targetListing.currency;
            newOffer.quantityWanted = _getSafeQuantity(
                targetListing.tokenType,
        ...
        internal
    {
        ...
        require(_newOffer.quantityWanted > 0, "quantityWanted > 0");
    ...
    function handleBid(Listing memory _targetListing, Offer memory _incomingBid)
        internal
    {
        Offer memory _currentWinningBid = winningBid[_targetListing.listingId];
        uint256 currentOfferAmount = _currentWinningBid.pricePerToken *
            _currentWinningBid.quantityWanted;
        uint256 incomingOfferAmount = _incomingBid.pricePerToken *
            _incomingBid.quantityWanted;

        uint256 reservePrice = _targetListing.reservePricePerToken *
        ...
        if (
            ...
            Currency.transferCurrencyWithWrapperAndBalanceCheck(
            ...
            if (
                _currentWinningBid.offeror != address(0) &&
                currentOfferAmount > 0
            ) {
                Currency.transferCurrencyWithWrapperAndBalanceCheck(
                    _targetListing.currency,
                    address(this),
                    _currentWinningBid.offeror,
                    currentOfferAmount,
                    nativeTokenWrapper
                );
            }
    ...
    /*///////////////////////////////////////////////////////////////
                    Auction Lisitings Sales Logic
    //////////////////////////////////////////////////////////////*/

    /// @dev Lets an account close an auction for both the winning bidder && auction creator.
    function closeAuction(uint256 _listingId)
        external
        override
        nonReentrant
        onlyExistingListing(_listingId)
    {
        ...
        require(targetListing.listingType == ListingType.Auction, "!auction.");

        Offer memory _winningBid = winningBid[_listingId];

        // Cancel auction if (1) auction hasn't started, or (2) auction doesn't have any bids.
        bool cancel_ = targetListing.startTime > block.timestamp ||
            _winningBid.offeror == address(0);
        ...
        if (cancel_) {
            ...
            _closeAuctionForBidder(targetListing, _winningBid);
            _closeAuctionForAuctionCreator(targetListing, _winningBid);
        }
    }
    ...
    function payout(
        ...
        // Collect market fee
        uint256 marketFeeCut = (_totalPayoutAmount * marketFeeBps) / MAX_BPS;

        (
            address upfrontFeeRecipient,
            uint256 upfrontBps,
            address royaltySalesRecipient,
            uint256 royaltySalesBps
        ) = fantasySportFee.getFeeInfo(address(this), FeeType.MARKET_SALE);

        /// For pots
        uint256 upfrontSalesCut = (marketFeeCut * upfrontBps) / MAX_BPS;
        uint256 royaltySalesCut = 0;
        ...
        returns (address royaltyFeeRecipient, uint256 royaltyFeeAmount) {
            if (royaltyFeeRecipient != address(0) && royaltyFeeAmount > 0) {
                require(
                    royaltyFeeAmount + marketFeeCut <= _totalPayoutAmount,
                    "fees exceed the price"
                );
                royaltyRecipient = royaltyFeeRecipient;
                royaltyCut = royaltyFeeAmount;

                royaltySalesCut = (royaltyCut * royaltySalesBps) / MAX_BPS;
            }
        } catch {}

        // Distribute upfront sales fee cut
        Currency.transferCurrencyWithWrapperAndBalanceCheck(
            _currencyToUse,
            _payer,
            upfrontFeeRecipient,
        ...
        // Distribute market fee cut
        Currency.transferCurrencyWithWrapperAndBalanceCheck(
            _currencyToUse,
            _payer,
            marketFeeRecipient,
            marketFeeCut - upfrontSalesCut,
            nativeTokenWrapper
        );

        // Distribute royalty fee cut to royalty sales recipient
        Currency.transferCurrencyWithWrapperAndBalanceCheck(
            _currencyToUse,
            _payer,
            royaltySalesRecipient,
            royaltySalesCut,
            nativeTokenWrapper
        );

        // Distribute royalty cut to token creator
        Currency.transferCurrencyWithWrapperAndBalanceCheck(
            _currencyToUse,
            _payer,
            royaltyRecipient,
            royaltyCut - royaltySalesCut,
            nativeTokenWrapper
        );
        // Distribute remainder to token owner
        Currency.transferCurrencyWithWrapperAndBalanceCheck(
            _currencyToUse,
            _payer,
            _payee,
            _totalPayoutAmount - (marketFeeCut + royaltyCut),
            nativeTokenWrapper
        );
    }
    ...
    function _validateDirectListingSale(
        ...
        if (_listing.currency == Currency.NATIVE_TOKEN) {
    ...
    function _getSafeQuantity(TokenType _tokenType, uint256 _quantityToCheck)
        internal
        pure
    ...
    /// @dev Returns the market fee recipient and bps.
    function getMarketFeeInfo() external view returns (address, uint16) {
        return (marketFeeRecipient, uint16(marketFeeBps));
    }
    ...
    /// @dev Lets a contract admin update market fee recipient and bps.
    function setMarketFeeInfo(
        address _marketFeeRecipient,
        uint256 _marketFeeBps
    ) external onlyModuleAdmin {
        require(_marketFeeBps <= MAX_BPS, "bps <= 10000.");

        marketFeeBps = uint64(_marketFeeBps);
        marketFeeRecipient = _marketFeeRecipient;

        emit MarketFeeInfoUpdated(_marketFeeRecipient, _marketFeeBps);
    }
    ...
    function setRestrictedListerRoleOnly(bool restricted)
        external
        onlyModuleAdmin
    {
...
import "../IFantasySportContract.sol";

interface IMarketplace is IFantasySportContract {
    ...
    *  @param _expirationTimestamp The timestamp after which a seller cannot accept this offer.
...
interface IFantasySportFee {
    function getFeeInfo(address _proxy, uint256 _type)
        external
        view
        returns (
            address upfrontSalesRecipient,
            uint256 upfrontSalesbps,
            address royaltySalesRecipient,
            uint256 royaltySalesBps
        );
}
...
library Currency {
    ...
    function safeTransferNativeTokenWithWrapper(
        ...
        if (!success) {
            ...
            require(
                IERC20Upgradeable(_nativeTokenWrapper).transfer(to, value),
                "transfer failed"
            );
        }
    }
...
interface IFantasySportContract {
