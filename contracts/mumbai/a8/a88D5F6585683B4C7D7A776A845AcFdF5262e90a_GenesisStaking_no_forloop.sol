...
contract GenesisStaking_no_forloop is Pausable {
    ...
    //bonus reward
    uint256 bonusReward;
    uint256 bonusTotalStaking;
    uint256 bonusPerTokenReward;
    uint256 decimals = 10 * 1e6;
    //matic swap rate
    // uint256 swapRate = 2;
    ...
    function setDetails(
        ...
        require(
            token.balanceOf(address(this)) >= _totalReward,
    ...
    function addReward(uint256 _amount) external onlyAdmin {
        require(
            token.balanceOf(address(this)) >= restReward + _amount,
            "Add the reward first"
        );
        bonusReward += _amount;
        bonusPerTokenReward = (bonusReward / bonusTotalStaking) * decimals;
    }

    function getBonusReward() external view returns (uint256) {
        return bonusReward;
    }
    ...
    function stake_3month(uint256 _amount)
        public
        whenNotPaused
        basicCheck(_amount, 0)
        returns (bool)
    {
        require(
            _amount <= restStakingDetail.threeMonth,
            "Staking is up to limit"
        );
    ...
    function stake_6month(uint256 _amount)
        public
        whenNotPaused
        basicCheck(_amount, 1)
        returns (bool)
    {
        require(
            _amount <= restStakingDetail.sixMonth,
            "Staking is up to limit"
        );
    ...
    function stake_12month(uint256 _amount)
        public
        whenNotPaused
        basicCheck(_amount, 2)
        returns (bool)
    {
        require(
            _amount <= restStakingDetail.sixMonth,
    ...
    function claimReward_ROE(uint256 _index) external {
        ...
        require(
            block.timestamp >=
                startStaking + stakes[msg.sender][_index].month * 30 * 1 days,
            "Claim time is not achieved"
        );
        ...
        require(
            ...
            "This contract balance is not enough"
        );
        bonusTotalStaking -= originAmount;
        bonusReward -= bonusAmount;
        stakes[msg.sender][_index].executed = true;
        token.transfer(msg.sender, totalAmount);
    }

    function estimate_Reward(address _address, uint256 _index)
        external
        view
        returns (uint256)
    {
    ...
    function calOriginAmount(address _address, uint256 _index)
        internal
        view
        returns (uint256)
    {
    ...
    function calReward(address _address, uint256 _index)
        internal
        view
        returns (uint256)
    {
        ...
        if (stakePeriod == 3) {
            reward +=
                (stakes[_address][_index].amount * three_month_interest) /
                100;
        } else if (stakePeriod == 6) {
            reward +=
                (stakes[_address][_index].amount * six_month_interest) /
                100;
        } else if (stakePeriod == 12) {
            reward +=
                (stakes[_address][_index].amount * twelve_month_interest) /
                100;
        }
    ...
    function calBonus(address _address, uint256 _index)
        public
        view
        returns (uint256)
    {
        return
            (stakes[_address][_index].amount * bonusPerTokenReward) / decimals;
        ...
        for (uint256 i = 0; i < stakers.length; i++) {
            ...
            for (uint256 j = 0; j < stakes[_address].length; j++) {
                if (stakes[_address][j].executed == false) {
                    allWeights +=
                        stakes[_address][j].amount *
                        (block.timestamp - stakes[_address][j].since);
                }
            }
        }
        return allWeights;
    ...
    function getUserWeight(address _address, uint256 _index)
        internal
        view
        returns (uint256)
    {
        ...
        if (stakes[_address][_index].executed == false) {
            userWeight =
                stakes[_address][_index].amount *
                (block.timestamp - stakes[_address][_index].since);
        }

        return userWeight;
    }

    function getStakes(address _address)
        external
        view
        returns (Stake[3] memory)
    {
