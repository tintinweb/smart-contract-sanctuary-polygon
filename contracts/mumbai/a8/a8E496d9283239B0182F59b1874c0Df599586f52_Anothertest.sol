 ...
 * @notice This contract handles minting anothertest tokens.
 */
contract Anothertest is
    ERC721A,
    ERC721AQueryable,
    ERC4907A,
    Ownable,
    ...
    RevokableOperatorFilterer
{
    ...
    address private presaleSignerAddress =
        0xfd5768DA28061BF7a41FFbD896E9E7bF35A3DC02;
    // Address where HeyMint fees are sent
    address public heymintPayoutAddress =
        0xE1FaC470dE8dE91c66778eaa155C64c7ceEFc851;
    address public royaltyAddress = 0x99DcC504fc9E007088c006f04015b34613C375A6;
    address[] public payoutAddresses = [
        0x99DcC504fc9E007088c006f04015b34613C375A6
    ];
    ...
    bool public useOriginalTokenIdForMetadata;
    // Maps a token id to the burn token id that was used to mint it to match metadata
    mapping(uint256 => uint256) public tokenIdToBurnTokenId;
    string public baseTokenURI =
        "ipfs://bafybeidygbe7jbzurujv35wjmr4h6akamqdqmo6z2tv2wjn3s3jrxpka7u/";
        ...
        )
    {
        ...
        burnTokens.push(
            BurnToken(0x75af4d0a370A3FcEc8013c3D4c82Fb91295Aeb34, 1, 1, 0)
        );
    }
        ...
        returns (string memory)
    {
        ...
        uint256 tokenURITokenId = useOriginalTokenIdForMetadata
            ? tokenId
            : tokenIdToBurnTokenId[tokenId];
        if (tokenURITokenId == 0) tokenURITokenId = tokenId;
        return
            bytes(baseURI).length != 0
                ? string(abi.encodePacked(baseURI, _toString(tokenURITokenId)))
                : "";
    }
     ...
     */
    function setUseOriginalTokenIdForMetadata(
        bool _useOriginalTokenIdForMetadata
    ) external onlyOwner {
        useOriginalTokenIdForMetadata = _useOriginalTokenIdForMetadata;
    }
    ...
    /**
     * @notice Update the price required to be paid alongside burning tokens to mint
     * @param _burnPayment The new amount of payment required per burn transaction
     */
    function updatePaymentPerBurn(uint256 _burnPayment) external onlyOwner {
        burnPayment = _burnPayment;
    }
    ...
    function burnToMint(
        ...
        for (uint256 i = 0; i < _tokensToMint; i++) {
            tokenIdToBurnTokenId[firstNewTokenId + i] = _tokenIds[0][i];
        }
