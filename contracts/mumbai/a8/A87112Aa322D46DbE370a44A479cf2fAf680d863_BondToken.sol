...
contract BondToken is IERC1404,Ownable,ERC20 {
    using SafeERC20 for IERC20;
    address public issuer = msg.sender;
    uint public maturityDate = block.timestamp + 1 minutes;
    uint public interestRate = 100;
    ...
    function setmaturityDate(uint256 maturity) external onlyOwner() {
        maturityDate = maturity;
    }

    function setInterestRate(uint256 intrest) external onlyOwner() {
    ...
    //     uint256 value
    // ) external {
    //     require(block.timestamp >= maturityDate, "Bond has not reached maturity yet.");

    //     require(
    //         _checkTransferRestrictions(from, to, value) == SUCCESS_CODE,
    ...
    //     if (from == issuer) {
    //         return SUCCESS_CODE; // Allow issuer to transfer tokens freely
    //     }

    //     if (block.timestamp < maturityDate) {
    //         return ERROR_CODE; // Disallow transfers until bond reaches maturity
    //     }

    //     uint interest = (value * interestRate * (block.timestamp - maturityDate)) / (365 days);

    //     if (balanceOf(from) - value + interest < balanceOf(to)) {
    //         return ERROR_CODE; // Ensure the account maintains enough balance for the interest
    //     }
    ...
    function redeemBond() external {
        ...
        uint interest = (balance * interestRate * (block.timestamp - maturityDate)) / (365 days);
