...
contract MyNewCollection is ERC721, Ownable {
    ...
    uint256 public MAX_SUPPLY = 3650;
    uint256 public MAX_PER_MINT = 50;
    uint256 public MINT_PRICE = 100000000000000;

    IERC20 public maticToken;

    address public receiverAddress = 0xb6ba815DC649b7Db1Ed4dA400da9D76688ea8A54;

    constructor(address _maticTokenAddress) ERC721("MyNewCollection", "TEST") {
    ...
    function mint(uint256 numberOfTokens) public {
        ...
        uint256 mintCost = MINT_PRICE * numberOfTokens;
    ...
    function airdropMint(uint256 numberOfTokens, address recipient) external onlyOwner {
        require(numberOfTokens > 0, "Quantity must be greater than 0.");
        require(numberOfTokens <= MAX_PER_MINT, "Exceeds max per mint.");
        require(totalSupply() + numberOfTokens <= MAX_SUPPLY, "Exceeds max supply.");
    ...
    function withdrawBalance() public onlyOwner {
        uint256 contractBalance = maticToken.balanceOf(address(this));
        maticToken.transfer(receiverAddress, contractBalance);
    }
}