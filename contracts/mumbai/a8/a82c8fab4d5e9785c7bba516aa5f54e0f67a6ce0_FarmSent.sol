...
abstract contract Crowdsale is Initializable {
   ...
   * @param _USDTTokenAmount USDT Token Amount
   */
  function buyTokens(address _beneficiary, uint256 _USDTTokenAmount) internal {
      _preValidatePurchase(_beneficiary, _USDTTokenAmount);
      // update state
      USDTRaised = USDTRaised + _USDTTokenAmount;
      _forwardFunds(_beneficiary, _USDTTokenAmount);
  }
   ...
   * @param _beneficiary Address performing the token purchase
   * @param _USDTTokenAmount Value in wei involved in the purchase
   */
    ...
    virtual
  {
    ...
    require(_USDTTokenAmount != 0, "Amount cant be 0");
  }
   ...
   */
  function _forwardFunds(address _beneficiary, uint256 _USDTTokenAmount) internal {
    _USDT.safeTransferFrom(_beneficiary, wallet, _USDTTokenAmount);
  }
  ...
  /**
    ...
    */
  function _changeSaleToken(IERC20Upgradeable newToken) virtual internal {
    token = newToken;
  }
...
contract FarmSent is Crowdsale, FinalizableCrowdsale, UUPSUpgradeable{
    ...
    function initialize(
      ...
      public initializer
    {      
        _USDT = IERC20Upgradeable(0x15a6f5Ac10120915396E212A1D236ed8d7860Eb3); // USDT Token
    ...
    */
    function getUSDTToken() external view returns (IERC20Upgradeable) {
        return _USDT;
    }
     
    /**
    * @dev Setting token USDT address  
    * @param USDTToken token USDT will be updated
    */
    function setReceiveTokens(address USDTToken) external onlyOwner {
        _USDT = IERC20Upgradeable(USDTToken);
    }
    ...
    /**
     * @param _USDTTokenAmount  performing the token purchase amount in usdt
     */
    function buyToken(uint256 _USDTTokenAmount) external onlyWhileOpen whenNotPaused{
      require(!_blacklist[msg.sender], "blacklist: Your Account has been blacklisted");
      if(whiteListingStatus){
        require(_whitelist[msg.sender], "whitelist: Your Account has not been whitelisted");
      }

      _USDTTokenAmount  = _USDTTokenAmount * 10 ** 18;

      require(_USDT.balanceOf(msg.sender) >= _USDTTokenAmount, "USDT Token balance limit exceeded");

      require((_purchasedAmount[msg.sender][round] + _USDTTokenAmount) <= purchaseLimitInWei, "Maximum purchase Limit exceeded for this round");

      buyTokens(msg.sender, _USDTTokenAmount);

      // calculate token amount to be created
      uint256 token_amount = _getTokenAmount(_USDTTokenAmount);
      token.safeTransfer(msg.sender, token_amount);
      _purchasedAmount[msg.sender][round] += _USDTTokenAmount;
    }
     ...
     */
    function changeRate(uint256 newRate) virtual external onlyOwner whenNotPaused{
     ...
     */
    function changeToken(IERC20Upgradeable newToken) virtual external onlyOwner whenNotPaused{
      require(address(newToken) != address(0), "Token: Address cant be zero address");
      _changeSaleToken(newToken);
    }
     ...
     */
    function changeWallet(address newWallet) virtual external onlyOwner whenNotPaused{
     ...
     */
    function setPurchaseLimitInWei(uint256 amount) external onlyOwner whenNotPaused {
     ...
     */
    function updateWhitelistingStatus(bool enable) external onlyOwner whenNotPaused {
      whiteListingStatus = enable;
      emit UpdateWhitelistingStatus(whiteListingStatus);
    }

    /**
     * @dev check the _beneficiary purchased amount for the given round
     * @param _beneficiary address to check the purchased value for this given round
     * @param _round round number to check how much token has been purchased
     */
    function purchasedTokens(address _beneficiary, uint256 _round) external view returns(uint256){
      return _purchasedAmount[_beneficiary][_round];
    }
  ...
  */
    function withdrawAnyToken(address _tokenAddress, uint256 amount)
        external
        onlyOwner
    {
