...
contract Sale is ERC721Holder, Ownable, ReentrancyGuard {
    ...
    address public web3re;

    uint256 public charityPercentage; 
    uint256 public web3rePercentage;
    ...
    mapping(uint256 => SaleInfo) public resales;
    ...
    event SetRewardAddress(address charity, address web3re);
    event SetRewardPercentage(uint256 charityPercentage, uint256 web3rePercentage, uint256 artistPercentage);
    ...
    constructor(address _collection, address _charity, address _web3re ) Ownable() {
    ...
    /**
       	@notice Update the new operator address
       	@dev  Caller must be Owner
		@param	_operator			Address of a new minter

        NOTE: `_Operator` = 0x00: disable MINTER role
    */
    function setOperator(address _operator) external onlyOwner {
        // require(address(0) != _operator, "Null address");
        operator = _operator;
    }
    ...
    */
    ...
    /**
       	@notice Set reward addresss
       	@dev  Caller must be Owner
        @param _charity address of _charity
        @param _web3re address of _web3re
        
    */
    function setRewardAddress(address _charity, address _web3re ) public onlyOwner {
        require(_charity != address(0), "_charity = ZeroAddress");
        require(_web3re != address(0), "_web3re = ZeroAddress");

        charity = _charity;
        web3re = _web3re;

        emit SetRewardAddress(_charity, _web3re);
    }

    /**
       	@notice Set reward percentage
       	@dev  Caller must be Owner
        @param _charityPercentage percent of _charity
        @param _web3rePercentage percent of _web3re
        @param _artistPercentage percent for the artist
        
    */
    function setRewardPercentage(uint256 _charityPercentage, uint256 _web3rePercentage, uint256 _artistPercentage ) public onlyOwner {
        require(_charityPercentage > 0, "MUST NOT BE 0");
        require(_web3rePercentage > 0, "MUST NOT BE 0");
        require(_artistPercentage > 0, "MUST NOT BE 0");
        require(_charityPercentage + _web3rePercentage + _artistPercentage < 50, "MUST NOT OVER 50");

        charityPercentage = _charityPercentage; 
        web3rePercentage = _web3rePercentage;
        artistPercentage = _artistPercentage;

        emit SetRewardPercentage(_charityPercentage, _web3rePercentage, artistPercentage);
    }
    ...
    */
    ...
    /**
       	@notice Create original art via sale smart contract because sale has the minter role
       	@dev  Caller could be anyone
        @param	_artist				    artist
        @param	_numOfRow				Row
        @param	_numOfCol				Column 
        @param	_baseURI				URI
    */
    function delegateCreateArt(address _artist, uint256 _numOfRow, uint256 _numOfCol, string calldata _baseURI) external {
        ISpecialCollection(COLLECTION).delegateCreate(_artist, _numOfRow, _numOfCol, _baseURI);
    }
    ...
    /**
        ...
        @param	_baseURI				URI
    */
    function createArt(uint256 _numOfRow, uint256 _numOfCol, string calldata _baseURI) external {
        ISpecialCollection(COLLECTION).create(_numOfRow, _numOfCol, _baseURI);
    }
    ...
    */
    function createSale(uint256 _saleId, address _paymentToken, uint256 _price) external {
        require(_paymentToken != address(0), "Pay with native when set paymentToken to usdToken");
        require(_price != 0, "ZeroPrice");
        require(
        ...
        // Transfer to this address for sale
        IERC721(COLLECTION).safeTransferFrom(msg.sender, address(this), _saleId);
    ...
    */
    function terminateSale(uint256 _saleId) external {
        require(sales[_saleId].fixedPrice != 0, "SaleNotExist");
        require(!endSales[_saleId], "Terminated");

        require(sales[_saleId].owner == msg.sender, "Not the owner of this sale");

        // Transfer back to owner
        IERC721(COLLECTION).safeTransferFrom(address(this), msg.sender , _saleId);
    ...
    function getNativePrice(uint256 saleID) public view returns(uint256){
        ...
        return maticPrice * _info.fixedPrice / 10**usdTokenDecimal;
    }
    ...
    */
    function purchase(uint256 _tokenId) external {
        ...
        // Original Artist
        address artist = ISpecialCollection(COLLECTION).getArtistAddress(_tokenId);
        ...
        _makePayment(_info.paymentToken, _buyer, _info.fixedPrice, artist, _info.owner, false);
    ...
    */
    function purchaseWithNative(uint256 _tokenId) external payable {
        ...
        // Convert to native 
        uint256 nativePrice = maticPrice * _info.fixedPrice / 10**usdTokenDecimal;

        require(msg.value >= nativePrice, "Not enough matic");

        _makePayment(address(0), _buyer, msg.value, artist, _info.owner, false);
    ...
    */
    function purchaseResale(uint256 _fragmentId) external {

        // Original Artist
        address artist = ISpecialCollection(COLLECTION).getArtistAddress(_fragmentId);

        address _buyer = msg.sender;

        SaleInfo memory _info = checkPriceResale(_fragmentId);

        require(_info.paymentToken != address(0), "Not payable with native with this function");

        _makePayment(_info.paymentToken, _buyer, _info.fixedPrice, artist, _info.owner, true);
    ...
    */
    function purchaseWithNativeResale(uint256 _fragmentId) external payable {
        ...
        _makePayment(address(0), _buyer, msg.value, artist, _info.owner, true);

        IERC721(COLLECTION).safeTransferFrom(address(this), _buyer, _fragmentId);

        removeFromIndexer(_fragmentId);

        emit ResaleSold(_buyer, _fragmentId, address(0), msg.value);
    }
    ...
    */
    ...
    /**
        ...
        @param	_fragmentId            Token Id of a fragment NFT
    */
    function checkPriceResale(uint256 _fragmentId) public view returns (SaleInfo memory _info) {
        // require in  resale
        require(resales[_fragmentId].owner != address(0));
        _info = resales[_fragmentId];
    }

    function _makePayment(address _token, address _from, uint256 _amount, address _artist, address _owner, bool _isResale) nonReentrant private {
        
        uint256 hasLeft = _amount;

        
        uint256 charityAmount = _amount * charityPercentage / 100;
        uint256 web3reAmount = _amount * web3rePercentage / 100;

        uint256 artistAmount = 0;
        if(_isResale){
            artistAmount = _amount * artistPercentage / 100;
        }

        hasLeft = hasLeft - charityAmount - web3reAmount - artistAmount;
        
        if (_token == address(0)){
            Address.sendValue(payable(charity), charityAmount);
            Address.sendValue(payable(web3re), web3reAmount);
            if(_isResale){
                Address.sendValue(payable(_artist), artistAmount);
            ...
            IERC20(_token).safeTransferFrom(_from, charity, charityAmount);
            IERC20(_token).safeTransferFrom(_from, web3re, charityAmount);

            if(_isResale){
                IERC20(_token).safeTransferFrom(_from, _artist, artistAmount);
...
interface ISpecialCollection {
    ...
    */
    ...
    /**
       	@notice Check the artist address of this original NFT
       	@dev  Caller can be ANY
		@param	_tokenId    TokenId orginal or Fragment NFT
    */
    function getArtistAddress(uint256 _tokenId) external view returns (address);
    ...
    */
    ...
    /**
       	@notice Mint original NFT to message sender aka original artist
       	@dev  Caller must be MINTER
        @param	_numOfRow				Row
        @param	_numOfCol				Column 
        @param	_baseURI				URI
    */
    function create(uint256 _numOfRow, uint256 _numOfCol, string calldata _baseURI) external;
    ...
    /**
        ...
        @param	_baseURI				URI
    */
    function delegateCreate(address _artist, uint256 _numOfRow, uint256 _numOfCol, string calldata _baseURI) external;
