...
import "../view/VerifyChainId.sol";

contract Info is IInfo, InfoBase, VerifyChainId {
...
/**
 @title Contract for verifying if assetIDs belong to correct chainIdx 
 @author Freeverse.io, www.freeverse.io
*/

import "../pure/EncodingAssets.sol";
import "../view/ChainId.sol";
import "../interfaces/IVerifyChainId.sol";

contract VerifyChainId is EncodingAssets, ChainId, IVerifyChainId {

    /// @inheritdoc IVerifyChainId
    function hasCorrectChainIdx(uint256 assetId) public view returns (bool) {
        return isAssetIdChainIdxCorrectForChainId(assetId, block.chainid);
    }

    /// @inheritdoc IVerifyChainId
    function isAssetIdChainIdxCorrectForChainId(uint256 assetId, uint256 chainId) public pure returns (bool) {
        return decodeChainIdx(assetId) == chainId2ChainIdx(chainId);
    }
}
...
/**
 @title Returns the chainIdx given the id of the chain where this contact is deployed
 @author Freeverse.io, www.freeverse.io
*/
...
contract ChainId is IChainId {
    ...
    uint256 private constant HEDERA_MAINNET = 295;
    uint256 private constant HEDERA_TESTNET = 296;
    uint256 private constant HEDERA_PREVIEWNET = 297;
    uint256 private constant HEDERA_DEVNET = 298;
    uint256 private constant TESTING = 1337;

    /// @inheritdoc IChainId
    function getChainIdx() public view returns (uint8 chainIdx) {
        return chainId2ChainIdx(block.chainid);
    }

    /**
    * Converts a given chainId to its corresponding chain index.
    * @param chainId The chainId to convert.
    * @return chainIdx The chain index corresponding to the given chainId.
    */
    function chainId2ChainIdx(uint256 chainId)
        public
        pure
        returns (uint8 chainIdx)
    {
        if (chainId == POLYGON_MAINNET) return 0;
        if (chainId == HEDERA_MAINNET) return 1;
        if (chainId == POLYGON_MUMBAI) return 2;
        if (chainId == HEDERA_TESTNET || chainId == HEDERA_PREVIEWNET || chainId == HEDERA_DEVNET) return 3;

        revert("Deploying to unsupported chainId");
    }
}
...
/**
 ...
 @dev  version(8b) + universeIdx(24) + isImmutable(1b) + isUntradable(1b) + chainIdx(8b) + editionIdx(16b) + assetIdx(38b) + initOwner(160b)
*/
...
contract EncodingAssets {
    function encodeAssetId(
        ...
        require(chainIdx >> 8 == 0, "chainIdx cannot be larger than 8 bit");
    ...
    function decodeChainIdx(uint256 assetId)
        public
        pure
        returns (uint32 chainIdx)
    {
...
/**
 @title Interface to contract for verifying if assetIDs belong to correct chainIdx 
 @author Freeverse.io, www.freeverse.io
*/

interface IVerifyChainId {
    /**
     * @notice Returns true if the provided assetId has the correct encoded chainIdx
     * @param assetId the id of the asset
     * @return true if the provided assetId has the correct encoded chainIdx
     */
    function hasCorrectChainIdx(uint256 assetId) external view returns (bool);

    /**
     * @notice Returns true if the provided assetId has an encoded chainIdx that matches
     *  the chainIdx corresponding to the provided chainId
     * @param assetId the id of the asset
     * @param chainId the id of the chain of deployment
     * @return true if the provided assetId has the correct encoded chainIdx
     */
    function isAssetIdChainIdxCorrectForChainId(uint256 assetId, uint256 chainId) external pure returns (bool);
}
...
/**
 @title Interface to contract for querying chainIdx 
 @author Freeverse.io, www.freeverse.io
*/

interface IChainId {
    /**
     * @notice Returns the chainIdx corresponding to the chain where this
     *  contract is deployed; reverts if chainId is not supported.
     * @return chainIdx the chainIdx
     */
    function getChainIdx() external view returns (uint8 chainIdx);
}