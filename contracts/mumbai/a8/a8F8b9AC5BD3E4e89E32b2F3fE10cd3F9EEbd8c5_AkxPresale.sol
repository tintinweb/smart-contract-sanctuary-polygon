...
contract AkxPresale is  Initializable,PausableUpgradeable, Presale, AccessControlEnumerableUpgradeable {
	...
	address public  _gnosisVault = 0x044e25f98fA9f46BEda68f1469E83dcDBF7C8d4A;
	...
	function setTimeLocked(address _timeLock) public {
		timeLockedWallet = _timeLock;
	}

	function setVestingWallets(address[] memory _vw, uint256[] memory _amounts, uint256 duration) public {
		vestingWallets = _vw;
	...
	function setRefundableEscrow(address _escrowImplementation) public {
		_holdersEscrows[msg.sender] = _escrowImplementation;
	}

	function setPriceForOneToken(uint256 price) public  {
	...
	function setQtyPerETH(uint256 qty) public {
		require(init == false, "already done");
		qtyPerEth = qty;
	}

	function setPhase1(uint startTime, uint duration, uint price, uint _qtyPerEth, uint supply) public  {
		require(init == false, "already done");
	...
	function setPhase2(uint startTime, uint duration, uint price, uint _qtyPerEth, uint supply) public {
