...
contract USDTPool is Ownable {
    ...
    function setDefaultMaxAmount(uint256 _defaultMaxAmount) external onlyOwner {
    ...
    function claimUSDT(address _user, uint256 _amount) external {
        require(callerList[msg.sender], "e001");
        require(!userBlackList[_user], "e002");
        if (userMaxAmountList[_user] == 0) {
            require(_amount <= defaultMaxAmount, "e003");
        } else {
            require(_amount <= userMaxAmountList[_user], "e004");
        }
