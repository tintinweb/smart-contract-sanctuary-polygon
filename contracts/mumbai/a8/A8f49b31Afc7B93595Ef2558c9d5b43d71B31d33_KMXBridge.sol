...
contract KMXBridge is Ownable {
    ...
    function buyCoinBundle(uint256 bundleId)
        external
        payable
        returns (bool)
    {

        if (IERC20(ERC20_ADDRESS).balanceOf(msg.sender) < bundles[bundleId].bundleCost)
            revert("You don't have enough tokens");

        bool sent = IERC20(ERC20_ADDRESS).transferFrom(
            msg.sender,
            address(this),
            bundles[bundleId].bundleCost
        );
        ...
        emit BundleBought(msg.sender, bundleId, bundles[bundleId].bundleCost, bundles[bundleId].bundleReward);
