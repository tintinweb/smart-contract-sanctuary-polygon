...
contract LostWorldData is CustomERC2771ContextUpgradeable, UUPSUpgradeable {
    ...
    function addEnemy(
        ...
        require(bytes(health).length > 0, 'LWD: Health cannot be empty');
        require(bytes(damage).length > 0, 'LWD: Damage cannot be empty');
        require(bytes(defense).length > 0, 'LWD: Defemse cannot be empty');
        require(bytes(criticalChance).length > 0, 'LWD: Critical chance cannot be empty');
        require(bytes(attackSpeed).length > 0, 'LCLCGDED: Attack speed cannot be empty');
        require(bytes(cleaveChance).length > 0, 'LWD: Cleave chance cannot be empty');
