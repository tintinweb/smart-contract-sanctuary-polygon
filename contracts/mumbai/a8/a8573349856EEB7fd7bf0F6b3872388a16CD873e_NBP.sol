...
contract NBP is SBT {
    ...
    function mint(
        ...
        _mint(_to, _tokenId, _metadata.projectId);
    ...
    function mintBatch(
        ...
        _mintBatch(_tos, _tokenId, _metadata.projectId);
        _setTokenMetadata(_tokenId, _metadata);
    }
...
contract Projects {
    ...
    function addUserToProject(uint256 _projectId, address[] memory _addressList) external {
        uint256 count = _addressList.length;
        for(uint256 i; i < count; i++) {
            address walletAddress = _addressList[i];

            if (!existProjectUser(_projectId, walletAddress)) {
            ...
            if (!existUserProject(walletAddress, _projectId)) {
    ...
    function getProjectUsersByName(
        ...
        return (_projects[projectId], getProjectUsers(projectId));
    }
    ...
    function getProjectUsers(uint256 _projectId) public view returns(ProjectUserStruct[] memory) {
    ...
    function updateAdmin(
        ...
        if (_operatorAdmins[_projectId][_walletAddress] != isAdmin) {
            _operatorAdmins[_projectId][_walletAddress] = isAdmin;
        }
    }
