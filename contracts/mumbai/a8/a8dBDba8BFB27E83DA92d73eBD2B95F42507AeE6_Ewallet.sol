...
contract Ewallet {
    ...
    mapping(uint256 => Transaction[]) public miniPoolIndividualToTransactions; // My Transactions (Incoming)
    mapping(uint256 => Transaction[]) public miniPoolTeamToTransactions; // My Team's Transactions (Outgoing)
    ...
    modifier lesserThanMiniPoolProfitTracker(uint256 amount) {
     ...
     function cashPrizeTransfer(uint256 _individualID, uint256 _teamID, string memory _description, uint256 _amount) public {
        IDToIndividual[_individualID].balance = IDToIndividual[_individualID].balance + _amount;
        IDToTeam[_teamID].balance = IDToTeam[_teamID].balance - _amount;
         // record transaction related to every team and individual
        Transaction memory transaction = Transaction(_individualID, _teamID, _amount, _description);
        miniPoolIndividualToTransactions[_individualID].push(transaction);
        miniPoolTeamToTransactions[_teamID].push(transaction);
     }

     function addToMiniPoolProfit(uint256 _profit, uint256 _individualID, uint256 _teamID, string memory _description, uint256 _amount) public {
        transfer(_individualID, _teamID, "Basic Spin The Wheel Entry Ticket", _amount);
        miniPoolProfitTracker = miniPoolProfitTracker + _profit;
     }

     function subtractFromMiniPoolProfit(uint256 _individualID, uint256 _teamID, string memory _description, uint256 _amount) public lesserThanMiniPoolProfitTracker(_amount) {
        cashPrizeTransfer(_individualID, _teamID, "Basic Spin The Wheel Cash Prize", _amount);
