...
contract BlotMap is Ownable, ERC721{
    ...
    // !! must change before deploying !! or after lol
    ...
    function mint(uint256 _ammount) public payable mintable(_ammount) isPaused whitelisted(msg.sender) {
    ...
    /// @notice ONLY OWNER changes the price to mint an nft
    function changePrice(uint256 _price) public onlyOwner {
        cost = _price;
    }

    /// @notice ONLY OWNER add addresses to the whitelist
    ...
    /// @notice ONLY OWNER remove addresses from the whitelist
    function whitelisteRemoveAddress(address _address) public onlyOwner {
        require(whitelist[_address], "Cannot remove unexisting address.");
