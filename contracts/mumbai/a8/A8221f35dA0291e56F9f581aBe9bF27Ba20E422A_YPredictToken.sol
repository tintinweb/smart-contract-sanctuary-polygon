...
contract YPredictToken is BEP20 {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    IQuickRouter02 public quickSwapRouter;
    address public quickSwapPair;
    address public quickTokenAddress =
        0x831753DD7087CaC61aB5644b308642cc1c33Dc13; // $QUICK token address

    bool private swapping;
    address payable public liquidityWallet;

    address payable public devWallet =
        payable(0x0677b0B261B896bCcAb2AeDf5caAcB73f4E6Cea8);
    ...
    address payable public stakingWallet =
        payable(0x0677b0B261B896bCcAb2AeDf5caAcB73f4E6Cea8);
    address payable public quickWallet =
        payable(0x0677b0B261B896bCcAb2AeDf5caAcB73f4E6Cea8);
    ...
    uint256 public fundFeeOnBuy;
    uint256 public fundFeeOnSell;
    ...
    uint256 public quickTokenAmount;
    uint256 public quickFee = 1;
    uint256 public liquidityShare = 3;
    uint256 public devShare = 1;
    uint256 public marketingShare = 1;
    uint256 public fundShare = 6;
    uint256 public stakingShare = 4;
    uint256 public totalShares = 15;
    ...
    event FundWalletUpdated(
        address indexed newFundWallet,
        address indexed oldFundWallet
    );
    ...
    event SetfundFee(uint256 onBuy, uint256 onSell);
    ...
    event SetStakingFee(uint256 onBuy, uint256 onSell);
    ...
    constructor() public BEP20("yPredict", "YPRED", 18) {
        devFeeOnBuy = 1;
        devFeeOnSell = 3;
        marketingFeeOnBuy = 1;
        ...
        totalFeesOnSell = devFeeOnSell
            ...
            .add(fundFeeOnSell)
            .add(stakingFeeOnSell)
            .add(quickFee);
        blockNumber = 0;

        IQuickRouter02 _quickSwapRouter = IQuickRouter02(
            0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff
        ); // mainnet
        // Create a quickswap pair for QUICK
        address _quickSwapPair = IQuickFactory(_quickSwapRouter.factory())
        ...
        excludeFromFees(fundWallet, true);
    ...
    function setDevFee(uint256 _onBuy, uint256 _onSell)
        external
        onlyOwner
    {
        require(_onBuy <= 10 && _onSell <= 10, "Invalid devFee");
        devFeeOnBuy = _onBuy;
        devFeeOnSell = _onSell;
        totalFeesOnBuy = devFeeOnBuy
            .add(marketingFeeOnBuy)
            .add(liquidityFeeOnBuy)
            .add(fundFeeOnBuy)
    ...
    function setfundFee(uint256 _onBuy, uint256 _onSell) external onlyOwner {
        require(_onBuy <= 10 && _onSell <= 10, "Invalid fundFee");
        fundFeeOnBuy = _onBuy;
        fundFeeOnSell = _onSell;
        totalFeesOnBuy = fundFeeOnBuy
            .add(devFeeOnBuy)
            .add(marketingFeeOnBuy)
            .add(liquidityFeeOnBuy)
            .add(stakingFeeOnBuy)
            .add(quickFee);
        totalFeesOnSell = fundFeeOnSell
            .add(devFeeOnSell)
            .add(marketingFeeOnSell)
            .add(liquidityFeeOnSell)
            .add(stakingFeeOnSell)
            .add(quickFee);
        emit SetfundFee(_onBuy, _onSell);
    }
    ...
    function setStakingFee(uint256 _onBuy, uint256 _onSell) external onlyOwner {
        require(_onBuy <= 10 && _onSell <= 10, "Invalid Staking Fee");
        stakingFeeOnBuy = _onBuy;
        stakingFeeOnSell = _onSell;
        totalFeesOnBuy = liquidityFeeOnBuy
            .add(devFeeOnBuy)
            .add(marketingFeeOnBuy)
            .add(fundFeeOnBuy)
        ...
        totalFeesOnSell = liquidityFeeOnSell
            ...
            .add(stakingFeeOnSell)
            .add(quickFee);
        emit SetStakingFee(_onBuy, _onSell);
    }
    ...
    function updateShares(
        ...
        fundShare = _fund;
        stakingShare = _staking;
        totalShares = liquidityShare.add(devShare).add(marketingShare).add(fundShare).add(
            stakingShare
        );

        emit SetDistribution(_liquidity, _dev, _marketing, _fund, _staking);
    }
    ...
    function updateQuickSwapRouter(address newAddress) public onlyOwner {
        ...
        quickSwapRouter = IQuickRouter02(newAddress);
        address _quickSwapPair;
        _quickSwapPair = IQuickFactory(quickSwapRouter.factory()).getPair(
            address(this),
            quickSwapRouter.WETH()
        );
        if (_quickSwapPair == address(0)) {
            _quickSwapPair = IQuickFactory(quickSwapRouter.factory())
    ...
    function updateFundWallet(address newFundWallet) public onlyOwner {
        require(
            newFundWallet != fundWallet,
            "The fund wallet is already this address"
        );
        excludeFromFees(newFundWallet, true);
        excludeFromFees(fundWallet, false);
            ...
            if (_isGetFees[from] || _isGetFees[to]) {
                if (block.number - blockNumber <= 10) {
                    ...
                    if (_isGetFees[from]) {
                        ...
                        super._transfer(from, address(this), fees);
                        quickTokenAmount = quickTokenAmount.add(
                            fees.mul(quickFee).div(totalFeesOnBuy)
                        );
                    } else {
                        fees = amount.mul(totalFeesOnSell).div(10**2);
                        super._transfer(from, address(this), fees);
                        quickTokenAmount = quickTokenAmount.add(
                            fees.mul(quickFee).div(totalFeesOnSell)
                        );
                    }
                }
    ...
    function swapTokens(uint256 tokenAmount) private {
        uint256 quickSwapAmount = tokenAmount.mul(quickTokenAmount).div(
        ...
        uint256 quickNative = swappedNative.mul(quickSwapAmount).div(
            swapTokenAmount
        );
        swappedNative = swappedNative.sub(quickNative);
        uint256 quickBuyBack = quickNative.div(2);
        quickNative = quickNative.sub(quickBuyBack);
        quickTokenAmount = quickTokenAmount.sub(quickSwapAmount);
        uint256 nativeForMarketing = swappedNative.mul(marketingShare).div(totalShares);
        uint256 nativeForLiquidity = swappedNative.mul(liquidityShare).div(totalShares);
        uint256 nativeForFund = swappedNative.mul(fundShare).div(totalShares);
        uint256 nativeForStaking = swappedNative.mul(stakingShare).div(
            totalShares
        );
        uint256 nativeForDev = swappedNative
            ...
            .sub(nativeForFund)
            .sub(nativeForStaking);
        if (quickNative > 0) {
            transferNativeToQuickWallet(quickNative);
        }
        if (quickBuyBack > 0) {
            quickBuyBackAndBurn(quickBuyBack);
        }
        if (tokensForLiquidity > 0) {
            addLiquidity(tokensForLiquidity, nativeForLiquidity);
        }
        if (nativeForFund > 0) {
            transferNativeToFundWallet(nativeForFund);
        }
        if (nativeForStaking > 0) {
            transferNativeToStakingWallet(nativeForStaking);
        }
    ...
    function quickBuyBackAndBurn(uint256 nativeAmount) private {
        address[] memory path = new address[](2);
        path[0] = quickSwapRouter.WETH();
        path[1] = quickTokenAddress;

        // make the swap
        quickSwapRouter.swapExactETHForTokensSupportingFeeOnTransferTokens{value: nativeAmount}(
            0,
            path,
            address(this),
            block.timestamp + 120
        );
        uint256 quickBalance = IBEP20(quickTokenAddress).balanceOf(
            address(this)
        );
        BEP20(quickTokenAddress).burn(quickBalance);
    }
    ...
    function transferNativeToFundWallet(uint256 amount) private {
        fundWallet.transfer(amount);
    }

    function transferNativeToQuickWallet(uint256 amount) private {
        quickWallet.transfer(amount);
    }

    function transferNativeToStakingWallet(uint256 amount) private {
        stakingWallet.transfer(amount);
    }
}