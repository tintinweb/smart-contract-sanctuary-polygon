...
contract BullitFile is BullitFileAdmin {
    ...
    //     subscriptionContract = 0xd9145CCE52D386f254917e481eB44e9943F39138;
    //     masterWallet = 0xdD870fA1b7C4700F2BD7f44238821C26f7392148;
    //     profitRatio = 20;
    //     timeLimitForDeletingFilesAfterEndSubscription = 30 days;
    // }
    ...
    function encrypt(string[] memory _uuid, uint256[] memory _fileId, string[] memory _hashFile, uint256[] memory _fileSize, string calldata _storageName, uint256 _amountToDecrypt, bool isFolder) external{
        ...
        require(_amountToDecrypt > 0, "The amount to decrypt must be greater than zero");
    ...
    function getAllHashForUser() external senderNonZeroAddress view returns (string[] memory files_) {
        return(getHashFiles(fileSharedWithUser[msg.sender], users[msg.sender].numberOfFilesSharedWithYou));
    }

    function getFilesOwned() external senderNonZeroAddress view returns (string[] memory files_) {
    ...
    function getHashFiles (string[] memory _files, uint256 _arrLength) internal view returns (string[] memory files_) {
...
interface IBullitFile {
    ...
    function getAllHashForUser() external view returns (string[] memory);

    function getFilesOwned() external view returns (string[] memory);
