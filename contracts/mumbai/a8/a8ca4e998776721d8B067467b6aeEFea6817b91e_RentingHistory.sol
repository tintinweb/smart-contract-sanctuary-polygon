...
contract RentingHistory is IRentingTypes, MarketplaceAccessControl {

    mapping(uint256 => RentingHistory) private rentingHistory;

    struct RentingHistory {
        uint256 id;
        BattleSet battleSet;
        RentingType rentingType;
        Coin chargeCoin;
    ...
    function saveRentingInfo(RentingInfo memory ri) external onlyManager {
        rentingHistory[ri.id] = RentingHistory(ri.id, ri.battleSet, ri.rentingType, ri.chargeCoin, ri.price,
            ri.revenueShare, ri.owner, ri.renter, ri.collectionId,  ri.rentingTs, ri.rentingEndTs);
    }

    function getRentingInfo(uint256 id) external view returns(RentingHistory memory) {
        return rentingHistory[id];
    }
