...
contract RecruitCoinTest is ERC20, Ownable, ReentrancyGuard {
        ...
        onlyIfVestingScheduleNotRevoked(vestingScheduleId)
    {
        ...
        if (
            ((amount + firstVestingAmount) <= vestingSchedule.amountTotal)
        ) {
        ...
        returns (uint256)
    {
        ...
        if (
            ...
            uint256 reaminAmountTotal = vestingSchedule.amountTotal.div(2);

            if (vestingSchedule.released == 0) {
                vestedAmount = vestingSchedule.amountTotal / 2;
            } else {
                vestedAmount = reaminAmountTotal.mul(vestedSeconds).div(
                    vestingSchedule.duration
                );
                vestedAmount = vestedAmount.sub(vestingSchedule.released);
            }

            require(vestedAmount > 0, "NO TOKENS AVALIBLE");
            vestingSchedule.released = vestingSchedule.released.add(
                reaminAmountTotal.mul(vestedSeconds).div(
                    vestingSchedule.duration
                )
            );*/
            ...
            if (vestingSchedule.firstVestingAmount >= 0) {
                vestedAmount = reainingAmountTotal
                    .mul(vestedSeconds)
                    .div(vestingSchedule.duration);
                vestedAmount = vestedAmount.sub(
                    vestingSchedule.firstVestingAmount.sub(
                        vestingSchedule.released
                    )
                );
            } else {
