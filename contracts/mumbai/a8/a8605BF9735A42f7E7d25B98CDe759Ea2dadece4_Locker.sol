...
interface ILocker {
    ...
    struct VoteLocked {
        ...
        uint256 withdrawMinCycle;
    }

    // Just to prevent 'Stack too deep' error in _checkpoint function
    ...
    // function voteDeposit(address account, uint256 amount) external;
}
...
contract Locker is ILocker, Initializable, Ownable, Pausable, ReentrancyGuard, ERC20 {
    using SafeERC20 for IERC20;

    IERC20 public oliveToken;
    ...
    mapping(address => VoteLocked) public votesLocked;
    ...
    event Deposit(address indexed provider, uint256 value, uint256 indexed locktime, int128 vetype, uint256 ts);
    event Withdraw(address indexed provider, uint256 value, uint256 ts);
    event Supply(uint256 prevSupply, uint256 supply);

    event VoteDeposited(address indexed account, uint256 amount, uint256 cycle);
    event WithdrawalRequested(address indexed account, uint256 amount, uint256 cycle);
    event WithdrawalCompleted(address indexed account, uint256 amount);

    modifier onlyVoting() {
        require(_msgSender() == address(voting), "NOT_ALLOWED");
        _;
    }
    ...
    function initialize(
        ...
        __Ownable_init_unchained();
        __Pausable_init_unchained();
        __ERC20_init_unchained("veOlive", "veOlive");

        require(address(_oliveToken) != address(0), "INVALID_TOKETOKEN");
        require(address(_manager) != address(0), "INVALID_MANAGER");
        require(address(_voting) != address(0), "INVALID_VOTING");

        oliveToken = _oliveToken;
        manager = _manager;
        voting = _voting;
    }


    // function voteDeposit(address account, uint256 amount) external override nonReentrant whenNotPaused onlyVoting {
    //     require(amount > 0, "INVALID_AMOUNT");
    //     require(account != address(0), "INVALID_ADDRESS");

    //     votesLocked[account].amount += amount;
    //     require(votesLocked[account].amount <= balanceOf(account), "Cannot exceed balance");
    //     votesLocked[account].cycle = manager.getCurrentCycleIndex();

    //     emit VoteDeposited(account, amount, cycle);
    // }

    // function requestVoteWithdrawal(uint256 amount) external {
    //     require(amount > 0, "INVALID_AMOUNT");

    //     VoteLocked storage vl = votesLocked[msg.sender];
    //     uint256 amountAvailable = vl.amount - vl.requestedWithdrawal;
    //     require(amountAvailable > amount, "INSUFFICIENT_AVAILABLE");

    //     vl.requestedWithdrawal += amount;
    //     uint256 currentCycle = manager.getCurrentCycleIndex();
    //     vl.requestCycle = currentCycle;
    //     if (manager.getRolloverStatus()) {
    //         vl.withdrawMinCycle = currentCycle + 2;
    //     } else {
    //         vl.withdrawMinCycle = currentCycle + 1;
    //     }

    //     emit WithdrawalRequested(msg.sender, amount, currentCycle);
    // }

    // function voteWithdraw(uint256 amount) external nonReentrant whenNotPaused {
    //     require(amount <= votesLocked[msg.sender].requestedWithdrawal, "WITHDRAW_INSUFFICIENT_BALANCE");
    //     require(amount > 0, "NO_WITHDRAWAL");
    //     uint256 currentCycle = manager.getCurrentCycleIndex();
    //     require(
    //         votesLocked[msg.sender].withdrawMinCycle <= currentCycle,
    //         "INVALID_CYCLE"
    //     );

    //     votesLocked[msg.sender].amount -= amount;
    //     votesLocked[msg.sender].requestedWithdrawal -= amount;

    //     bytes32 eventSig = "Withdraw";
    //     sendBalanceUpdate(msg.sender, amount, false);
    //     sendVotingUpdate(eventSig, msg.sender);

    //     emit WithdrawalCompleted(msg.sender, amount, currentCycle);
    // }
    ...
    function sendBalanceUpdate(address _user, uint256 _exchangeAmount, bool _isPositive) private {
        ...
        // sendVotingUpdate("Update", _user);  // We can remove eventSig as parameter in voteOlive
    }
    ...
    function uint256Toint128(uint256 value) private pure returns (int128) {
        return int128(int256(value));
    }

    function int128Touint256(int128 value) private pure returns (uint256) {
    ...
    function _checkpoint(address addr, LockedBalance memory old_locked, LockedBalance memory new_locked) internal {
        ...
        if (addr != address(0)) {
            ...
            if (old_locked.end > block.timestamp && old_locked.amount > 0) {
                u_old.slope = old_locked.amount / uint256Toint128(MAXTIME);
                u_old.bias = u_old.slope * uint256Toint128(old_locked.end - block.timestamp);
            }
            if (new_locked.end > block.timestamp && new_locked.amount > 0) {
                u_new.slope = new_locked.amount / uint256Toint128(MAXTIME);
                u_new.bias = u_new.slope * uint256Toint128(new_locked.end - block.timestamp);
            }
        ...
        for(uint i=0; i < 255; i++) {
            ...
            last_point.bias -= last_point.slope * uint256Toint128(t_i - last_checkpoint);
       ...
       point_history[vars._epoch] = last_point;
        ...
        if (addr != address(0)) {
            ...
            if (old_locked.end > block.timestamp) {
                // vars.old_dslope was <something> - u_old.slope, so we cancel that
                vars.old_dslope += u_old.slope;
                if (new_locked.end == old_locked.end) {
                    vars.old_dslope -= u_new.slope;   // It was a new deposit, not extension
                }
            ...
            if (new_locked.end > block.timestamp) {
                if (new_locked.end > old_locked.end) {
                    vars.new_dslope -= u_new.slope;  // old slope disappeared at this point
                    slope_changes[new_locked.end] = vars.new_dslope;
                }
                // else: we recorded it already in vars.old_dslope
            }
        }
    ...
    */
    function _deposit_for(address _addr, uint256 _value, uint256 unlock_time, LockedBalance memory locked_balance, int128 vetype) internal {
        ...
        sendBalanceUpdate(_addr, _value, true);
        emit Deposit(_addr, _value, _locked.end, vetype, block.timestamp);
    ...
    */
    function checkpoint() external {
        ...
        _checkpoint(address(0), old_locked, new_locked);
    }
    ...
    */
    function withdraw() external nonReentrant {
        ...
        uint256 value = int128Touint256(_locked.amount);
        ...
        oliveToken.transfer(msg.sender, value);
        
        sendBalanceUpdate(msg.sender, value, false);
    ...
    */
    function find_block_epoch(uint256 _block, uint256 max_epoch) internal view returns (uint256) {
        ...
        for(uint i=0; i < 128; i++) {   // Will be always enough for 128-bit numbers
            if (_min >= _max) {
                break;
            }
    ...
    */
    function balanceOf(address addr) public view override returns (uint256) {
        ...
        // console.log("uuuuuuu",user_point_epoch[addr]);
        if (_epoch == 0) {
            return 0;
        } else {
            Point memory last_point = user_point_history[addr][_epoch];
            last_point.bias -= last_point.slope * uint256Toint128(_t - last_point.ts);
            if (last_point.bias < 0) {
                last_point.bias = 0;
            }
            return int128Touint256(last_point.bias);
        }
    }
    ...
    */
    function balanceOfAt(address addr, uint256 _block) external view returns (uint256) {
        ...
        upoint.bias -= upoint.slope * uint256Toint128(block_time - upoint.ts);
        if (upoint.bias >= 0) {
            return int128Touint256(upoint.bias);
        } else {
            return 0;
        }
    }
    ...
    */
    function supply_at(Point memory point, uint256 t) internal view returns (uint256) {
        ...
        for (uint i=0; i < 255; i++) {
            ...
            last_point.bias -= last_point.slope * uint256Toint128(t_i - last_point.ts);
            if (t_i == t) {
                break;
            }
            last_point.slope += d_slope;
