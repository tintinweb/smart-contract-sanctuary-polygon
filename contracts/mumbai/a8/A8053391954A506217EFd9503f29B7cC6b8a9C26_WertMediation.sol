...
interface IMintingService {
    function listings(uint _id) external view returns (MintListing memory);
    function mintBnb(uint256 _id) external payable;
    function mintMultiBnb(uint256 _id, uint256 _quantity) external payable;
}

interface IMarket {
    function listings(uint _id) external view returns (MarketListing memory);
...
contract WertMediation is Ownable {
    ...
    function mintBnb(address _recipient, uint _listingId) public payable {
        MintListing memory listing = mintingService.listings(_listingId);
        mintingService.mintBnb{ value: msg.value }(_listingId);
        _transferMintedNfts(_recipient, listing.nft);
    }
    ...
    function directBuyBnb(address _recipient, uint _listingId, address[] memory _discountAddresses) public payable {
        MarketListing memory listing = market.listings(_listingId);
        market.directBuyBnb{ value: msg.value }(_listingId, _discountAddresses);
        INft nft = INft(listing.nft);
        nft.transferFrom(address(this), _recipient, listing.tokenId);
    }
