...
contract CouponContract is ERC721, Ownable{
    ...
    */
    ...
    function createCoupon(uint _id, string memory _ipfsUrl, uint _start, uint _end) public onlyAdmin{
        ...
        tokenIds.push(_id);
        idCheck[_id] = true;
        // bytes memory ipfsStringified = abi.encodePacked(_ipfsUrl);
        // newValue = ipfsStringified;
        emit CouponCreated(_id, _ipfsUrl, couponCreated);
    }
    ...
    function verifyDriverAd(address _driverAddress, uint _id) external view returns(bool){
     ...
     function readEvent(bytes memory eventData) public pure returns (string memory) {
        (string memory name) = abi.decode(eventData, (string));
        return (name);
    }
}
