...
contract CalvadosCoquerel is ERC721A, Ownable, EIP712, Pausable {
    ...
    modifier checkMintPerAddress(address account_, uint256 amount_) {
        require(
            ...
            "Calvados Coquerel: mint limit per wallet was exceeded"
        );
        _;
    }
    ...
    modifier checkSupply(uint256 amount_) {
        require(
            ...
            "Calvados Coquerel: Mint supply limit was exceeded"
        );
        _;
    }

    constructor()
        ERC721A("CalvadosCoquerel", "CC")
        EIP712("CalvadosCoquerel", "1")
    {}
    ...
    bytes32 private constant CALVADOS_COQUEREL_SHIPPING_DATA_TYPEHASH =
        keccak256(
            "CalvadosCoquerelShippingData(bytes32 hashdata,uint64 timestamp)"
        );
    ...
    function verifyShippingData(
        ...
        bytes32 digest = _hashTypedDataV4(
            keccak256(
                abi.encode(
                    CALVADOS_COQUEREL_SHIPPING_DATA_TYPEHASH,
                    hashdata,
                    timestamp
                )
            )
    ...
    function redeem(
        ...
        require(ownerOf(tokenId) == msg.sender, "Calvados Coquerel: Not allowed");
        require(
            redeemedTimestamp[tokenId] == 0,
            "Calvados Coquerel: NFT already redeemed"
        );
        require(
