...
contract ERC721Holder is IERC721Receiver {
  ...
  contract NFT is ERC721Holder,Ownable{
      ...
      constructor (IERC721 _token,address _signer) {
         token = IERC721(_token);
         signer = _signer;
      }

    event Deposit(address indexed user,uint amount,uint time);
    event Claim(address indexed user,uint amount,uint time);
    event Emergency(address indexed user,uint amount,uint time);
    ...
    function deposit(uint256 _id)public isLock isContractCheck(msg.sender) {
        require (_id > 0 ,"Incorrect params");
        token.safeTransferFrom(msg.sender,address(this),_id);
        emit Deposit(msg.sender,_id,block.timestamp);
    }

    function claim(address _user,uint256 _id,bytes calldata signature,uint time,uint _random) public {
        require(_user != address(0) && _id > 0,"Invalid params");
        bytes32 messageHash = message(_user,_id,time,_random);
    ...
    */
    function message(address  _receiver ,uint256 amount,uint time,uint _random)
        public pure returns(bytes32 messageHash)
    {
        messageHash = keccak256(abi.encodePacked(_receiver,amount,time,_random));
    }
    ...
    function emergency(address user,uint256 _id)public onlyOwner{
        require(user != address(0) && _id > 0,"Incorrect params");
        token.approve(msg.sender,_id);
        token.safeTransferFrom(msg.sender,user,_id);
        emit Emergency(user,_id,block.timestamp);
    }
