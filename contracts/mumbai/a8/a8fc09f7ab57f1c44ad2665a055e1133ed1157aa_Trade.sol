...
contract Trade is tariNFT{
    ...
    */
    ...
    function mintEvents(address nftAddress, string memory _uri, uint supply, uint96 fee) external{
      (uint256 tokenID, bool mint) =IERC1155(nftAddress).mint(msg.sender, _uri, supply, fee);
        require(mint, "Minting: NFT Minting Failed");
        emit Minted(address(0), msg.sender, tokenID, supply);

    }
    function mintNFTS(address nftAddress, string memory _uri, uint supply, uint96 fee) external {
        IERC1155(nftAddress).mint(msg.sender, _uri, supply, fee);
    }
