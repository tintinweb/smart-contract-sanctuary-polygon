...
contract PriceOracle {
  ...
  IQuickSwap quickSwapInstance;
  constructor(
    address _pairAddress,
    address _priceFeedAddress
  ) 
  {
    quickSwapInstance = IQuickSwap(_pairAddress);
  ...
  function getUsdtToToken(uint usdtQuantity) external view returns(uint exchange) {
      (uint112 reserve0, uint112 reserve1, ) = quickSwapInstance.getReserves(); 
      return (10 ** 16) * usdtQuantity * uint(reserve1) / uint(reserve0);
  }
  ...
  function getTokenToUsdt(uint tokenQuantity) external view returns(uint exchange) {
    (uint112 reserve0, uint112 reserve1, ) = quickSwapInstance.getReserves(); 
    return tokenQuantity * uint(reserve0) / ( uint(reserve1) * (10**16) );
  }
  ...
  function getLatestPrice() external view returns (int) {
    ...
    return price * 10000; //If 1 Matcic = 200,34 it will return 20034000000
  }
