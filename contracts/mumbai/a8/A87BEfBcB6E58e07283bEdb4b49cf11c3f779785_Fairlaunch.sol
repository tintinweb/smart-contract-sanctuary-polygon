...
import "./interfaces/ICryptenToken.sol";
...
contract Fairlaunch is Accessable 
{
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    address public TreasuryToken;
    address public RewardToken;
    address public Crypten;
    modifier onlyCrypten{
        require(msg.sender == Crypten, "Not called by crypten");
        _;

    }
    mapping (string => mapping (address=> uint256)) public RewardShares;
    mapping (string => uint256) public TotalRewardShares;
    mapping (string => uint256) public TotalRewards;
    uint256 PRECISION = 1e8;
    function TaskCreated(string calldata taskHash, address creator, uint256 treasuryAmount) public onlyCrypten
    {
        TotalRewards[taskHash] = treasuryAmount;
        IERC20(TreasuryToken).safeTransferFrom(creator, address(this), treasuryAmount);
    }

    function TaskStageFinished(string calldata taskHash, address executor,
        uint256 addShare) public onlyCrypten
    {
        RewardShares[taskHash][executor] = RewardShares[taskHash][executor].add(addShare);
        TotalRewardShares[taskHash] = TotalRewardShares[taskHash].add(addShare);
        ICryptenToken(RewardToken).mint(executor, addShare);
    }
    function Claim(string calldata taskHash) public{
        
        uint256 rewardShare = RewardShares[taskHash][msg.sender];
        require(rewardShare > 0, "No shares.");
        uint256 reward = rewardShare.mul(PRECISION).div(TotalRewardShares[taskHash])
            .mul(TotalRewards[taskHash]).div(PRECISION);
        RewardShares[taskHash][msg.sender] = 0;
        ICryptenToken(RewardToken).burn(msg.sender, rewardShare);
        IERC20(TreasuryToken).safeTransfer(msg.sender, reward);
    }
    function SetParams(address _treasury, address _reward, address _crypten) public onlyOwner{
        TreasuryToken = _treasury;
        RewardToken = _reward;
        Crypten = _crypten;
    }
...
interface ICryptenToken is IERC20
{
    function mint(address account, uint256 amount) external ;
    function burn(address account, uint256 share) external ;
