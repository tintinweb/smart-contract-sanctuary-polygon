...
*/
    ...
    // remove this on release this is for testing

    function adminMint(uint8 _amount, address _mintTo) external adminOnly{
        // transfer WETH to contract
        uint256 ffsplit = cost * _amount * 1/2;
        ...
        bytes32 _requestId;
        mintsPerWeek[currentWeek][msg.sender] += _amount;

        for(uint8 i=0; i<_amount; i++){
            _requestId = requestRandomness(keyHash, fee);
            responseIdToMint[_requestId] = Minting({tokenId: _tokenIds, minter: _mintTo});
            responseIdToAction[_requestId] = 1;
            _tokenIds++;
        }
    }

    function newAdminMint(uint8 _amount, address _mintTo) external adminOnly{
        // transfer WETH to contract
        uint256 ffsplit = cost * _amount * 1/2;
        weth.transferFrom(msg.sender, address(this), ffsplit);
        weth.transferFrom(msg.sender, ownerAddress, ffsplit);
