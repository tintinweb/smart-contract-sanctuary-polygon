...
abstract contract ERC20Recyclable is ERC20, AccessControl, IERC20Recyclable {
    ...
    //allocation for each recycling contract
    ...
    function setMinter(
        ...
        require(minter != address(0), "ERC20Recyclable: Invalid address");
    ...
    function setBurner(
        ...
        require(burner != address(0), "ERC20Recyclable: Invalid address");
    ...
    function setRecycler(
        ...
        require(recycler != address(0), "ERC20Recyclable: Invalid address");
        grantRole(MINTER_ROLE, recycler);
        grantRole(BURNER_ROLE, recycler);
    }

    function removeRecycler(
        address recycler
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        revokeRole(MINTER_ROLE, recycler);
        revokeRole(BURNER_ROLE, recycler);
    }
    ...
    function allocate(
        ...
        require(to != address(0), "ERC20Recyclable: Invalid address");
        require(amount > 0, "ERC20Recyclable: Amount must be > 0");
        require(totalSupply() + allocated + amount <= supply, "ERC20Recyclable: Total supply allocation exceeded");
    ...
    function release(
        ...
        require(from != address(0), "ERC20Recyclable: Invalid address");
        require(amount > 0, "ERC20Recyclable: Amount must be > 0");
        require(amount <= allocation[from], "ERC20Recyclable: Release exceeds Allocation");
        allocation[from] -= amount;
        allocated -= amount;

        emit Released(from, amount);
    }
    ...
    function mint(
        ...
        require(totalSupply() + amount <= supply, "ERC20Recyclable: Max supply exceeded");
...
contract HOPEToken is ERC20Recyclable {
    constructor() ERC20Recyclable("Hope Token", "HOPE", 1000000000 ether) {}
}