...
contract DPSDocks is ERC721Holder, ERC1155Holder, Ownable {
    ...
    /**
     * @notice used to lock multiple voyages at once
     * @param _voyagesParam parameters for sailing
     */
    function lockVoyagesItems(VoyageData[] calldata _voyagesParam) external {
        ...
        uint256 i;
        for (i; i < _voyagesParam.length; ++i) {
            lockVoyageItems(
                _voyagesParam[i].lockedVoyages,
                _voyagesParam[i].voyage,
                _voyagesParam[i].pirate,
                _voyagesParam[i].flagship
            );
        }
    ...
    function lockVoyageItems(
        ...
        for (uint256 index; index < _lockedVoyages.length; ++index) {
            ...
            for (uint256 i; i < lockedVoyage.supportShips.length; ) {
    ...
    function claimRewards(uint256[] calldata _voyageIds) external {
        ...
        for (uint256 i; i < _voyageIds.length; ++i) {
            ...
            address voyageOwner = ownerOfLockedVoyages[voyageId];
            if (voyageOwner == address(0)) revert AddressZero();

            LockedVoyageV2 memory lockedVoyage = lockedVoyages[voyageOwner][voyageId];
            VoyageConfigV2 memory voyageConfig = lockedVoyage.voyage.getVoyageConfig(voyageId);
            if (!gameEngine.checkIfViableClaimer(msg.sender, lockedVoyage, voyageOwner)) revert NotViableClaimer();
            ...
            lockedVoyages[voyageOwner][voyageId] = lockedVoyage;
            finishedVoyages[voyageOwner][lockedVoyage.voyageId] = lockedVoyage;
            finishedVoyagesIds[voyageOwner].push(lockedVoyage.voyageId);
            ownerOfFinishedVoyages[lockedVoyage.voyageId] = voyageOwner;
            voyageResults[voyageId] = voyageResult;

            cleanLockedVoyage(lockedVoyage.voyageId, voyageOwner);

            awardRewards(voyageResult, voyageConfig.typeOfVoyage, lockedVoyage, voyageOwner);
