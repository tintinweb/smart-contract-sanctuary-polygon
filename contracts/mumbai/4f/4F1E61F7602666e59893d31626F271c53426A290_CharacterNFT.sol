...
contract CharacterNFT is Ownable, ERC721Enumerable, AccessControl {
    ...
    mapping(address => uint256) private whitelistLimitedBuy;
    ...
    function pause() public onlyRole(MANAGERMENT_ROLE) {
        require(isPresale == false, "Not in unpause");
        isPresale = true;
    }

    function unpause() public onlyRole(MANAGERMENT_ROLE) {
        require(isPresale == true, "Not in pause");
        isPresale = false;
    }
    ...
    function setWhitelistBuy(uint256 _max) external onlyRole(MANAGERMENT_ROLE) {
        whitelistBuy = _max;
    }
        ...
        onlyRole(MANAGERMENT_ROLE)
    {
        ...
        emit whitelistEvent(_whitelist, _licensed);
    }
    ...
    function buyCharacter(address selectedToken, uint256 quantity) external isWhitelisted{
        if(isPresale){
            _buyCharacter(selectedToken, whitelistBuy);
        } else {
            _buyCharacter(selectedToken, quantity);
        }
    }
     ...
     */
    function _buyCharacter(address selectedToken, uint256 quantity) internal {
    ...
    function breedNFT(address _receiver, uint256 breedWaitingTime) external isBreeding returns (uint256 tokenID){
        uint256 tokenIdNow = _tokenIdCount.current();
        require(
