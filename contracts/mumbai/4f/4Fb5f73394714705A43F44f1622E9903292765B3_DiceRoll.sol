...
contract DiceRoll is Ownable, ReentrancyGuard {
    ...
    event Claim(uint256 id, address winner, uint256 amount, bool spent);
    ...
    */
    ...
    function gameState(uint256 _gameId) external view returns (
        address _creator,
        address _joiner,
        uint256 _balance,
        bool spent
    ...
    function claim(
        ...
        if (_winnerAddress == games[_gameId].creator) {
            require(_loserAddress == games[_gameId].joiner, "DiceRoll.claim: Game joiner address is not correct");
        } else {
            require(_winnerAddress == games[_gameId].joiner, "DiceRoll.claim: Game joiner address is not correct");
            require(_loserAddress == games[_gameId].creator, "DiceRoll.claim: Game creator address is not correct");
        }
        require(!games[_gameId].spent, "DiceRoll.claim: prize is already claimed");
        // require(
        ...
        uint256 gameAmount = games[_gameId].balance;
        ...
        games[_gameId].spent = true;
        emit Claim(_gameId, _winnerAddress, claimAmount, games[_gameId].spent);
    }
