...
/**
 * @title TokenTraxxNFT Contract
 * @author Tokentraxx Team
 * @dev This contract is used to create the NFT which can be pausable and burnable,
 * also batch minting can be done.
 */
    ...
    IERC721Custom
{
    ...
    /// @dev Emitted when an NFT are minted in batches;
    // Event to be emitted when a traxx is minted in batches
    event BatchTraxxMinted(uint256[] tokenIds);

    /// @dev Emitted when an Royalty is Updated;
    // Event to be emitted when the royalty is updated
    event RoyaltyUpdated(uint256 royaltyBps);

    /// @dev Emitted when the Traxx URI is updated ;
    // Event to be emitted when the traxx token URI is updated.
    ...
    function initialize(
        ...
        owner = msg.sender;
        //allow our marketplace as global approver(to transfer the token on successfull purchase)
        market = _market;
     ...
     * Requirement:
     * _maintainer address cannot be zero address.
     */
     ...
     * @param _uri URI string which is to be set.
     * Requirement:
     * Only maintainer can call the function
     */
    function setContractURI(string calldata _uri) external onlyMaintainer {
        _contractURI = _uri;
        emit TraxxURIUpdated(_uri);
    }

    /**
     * @dev Mints an ERC721 token.
     * @param _to address of which token will be minted.
     * @param _uri uri string for that token
     * Requirement:
     * _collectionId cannot be less than Zero
     */
    function _mintTraxx(
        address _to,
     ...
     * @param _uri uri string for that token
     * Requirement:
     * {_uri} length should not be less than zero
     */
    function mintTraxx(
        address _to,
        string calldata _uri
        ...
        uint256 id = _mintTraxx(_to, _uri); // Sid: Do we even need to be adding tokens to collections from NFT - mint functions
        emit TraxxMinted(msg.sender, _to, id, _uri); // as these collections are not tracked the same as collection contract ones,
    } // and also don't seem to have any use elsewhere, or can we update the Collection-contract collections from NFT contracts?
    ...
    function batchMintTraxx(
        ...
        // note : Need to upgrade this change on mainnet.
        // uint256 _latestId = _tokenIdTracker.current();
        require(
            (_to.length == _uri.length),
            throwError(INVALID_FUNCTION_PARAMS)
        );
        uint256[] memory tokenIds = new uint256[](_to.length);
        uint256 len = _to.length;
        for (uint256 i; i < len; i++) {
            uint256 id = _mintTraxx(_to[i], _uri[i]); // Sid: Should batch mint be adding tokens to the same collection?
            tokenIds[i] = id;
        }
     ...
     * @param _royaltyBps royalty to be set in BPS.
     * Requirement:
     ...
     * @param batchSize size of the batch mint
     */
    function _beforeTokenTransfer(
        address from,
        address to,
    ...
    function royaltyInfo(
        ...
        returns (address receiver, uint256 royaltyAmount)
    {
        address artistAddress = firstOwner[tokenId];
     ...
     * @param tokenId Id of token which is to be updated.
     * @param _URI uri string for that token
     */
