...
contract WftmAggregatorProxy is IAggregatorInterface {
    ...
    address public wftmUsdcPair;
    address public wftmUsdtPair;

    uint256 public minimumReserve = 1e10; // 10000 USDC

    constructor(address _wftm, address _usdc, address _usdt, address _wftmUsdcPair, address _wftmUsdtPair) {
        wftm = _wftm;
        usdc = _usdc;
        usdt = _usdt;

        wftmUsdcPair = _wftmUsdcPair;
        wftmUsdtPair = _wftmUsdtPair;
    ...
    function latestAnswer() external override view returns (int256) {
        (uint256 _wftmRes0, uint256 _usdcRes) = getReserves(wftm, usdc, wftmUsdcPair);
        (uint256 _wftmRes1, uint256 _usdtRes) = getReserves(wftm, usdt, wftmUsdtPair);
        uint256 _minimumReserve = minimumReserve;
        if (_usdcRes < _minimumReserve) {
            if (_usdtRes < _minimumReserve) return 0;
            else return int256(_usdtRes.mul(MULTIPLE).div(_wftmRes1));
        } else {
            if (_usdtRes < _minimumReserve) return int256(_usdcRes.mul(MULTIPLE).div(_wftmRes0));
            else return int256(_usdcRes.add(_usdtRes).mul(MULTIPLE).div(_wftmRes0.add(_wftmRes1)));
        }
    }
