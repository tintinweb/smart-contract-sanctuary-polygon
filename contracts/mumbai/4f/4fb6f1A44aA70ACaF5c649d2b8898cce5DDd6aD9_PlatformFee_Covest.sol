...
contract PlatformFee_Covest is
	Initializable,
	UUPSUpgradeable,
	ReentrancyGuardUpgradeable
{
	mapping(string => uint256) internal historyValueOfUser;
	mapping(address => uint256) internal valueOfPF;
	...
	function withdraw(
		...
		require(valueOfPF[_asset] >= _value, "Not Enough.");
		require(
			PDS.isBlacklistUser(_to) != true,
			"Policy: The destination is Blacklist."
		);
		require(
			PDS.isBlacklistAssetsUser(_to, _asset) != true,
			"Policy: The Assets of destination is Blacklist."
		);
		valueOfPF[_asset] -= _value;
	...
	/**
	 * @notice Get balanceOfPlatformFee.
	 * @param _asset The asset address.
	 * @return The uint256 value of balanceOfPlatformFee.
	 */
	function balanceOfPlatformFee(address _asset)
		public
		view
		returns (uint256)
	{
		return valueOfPF[_asset];
	}
