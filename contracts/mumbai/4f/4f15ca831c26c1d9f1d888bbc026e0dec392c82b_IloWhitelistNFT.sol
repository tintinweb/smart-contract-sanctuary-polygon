...
/* @dev: Check if contract owner */
    ...
    function mint(address _to) external onlyAdmin {
        ...
        if (
            ...
            (NUM_SPECIAL_NFTS + totalNftsMinted) < NUM_SPECIAL_NFTS - specialNftsMinted) 
            {
              ...
              serialNumber = string(abi.encodePacked("S", uint2str(tokenId)));
              details[_to].serialNumber = serialNumber;
        } else {
            tokenId = totalNftsMinted;
            details[_to].tokenId = tokenId;
            normalNftsMinted++;
            ...
            serialNumber = string(abi.encodePacked("N", uint2str(tokenId)));
            details[_to].serialNumber = serialNumber;
        }
    ...
    function mintRare(address _to, uint256 _numWhitelistedIlos) external onlyOwner {
        ...
        serialNumber = string(abi.encodePacked("R", uint2str(tokenId)));
        _safeMint(_to, tokenId);
    ...
    function transfer(address _to, uint256 _tokenId) external {
        ...
        details[_to].serialNumber = details[msg.sender].serialNumber;
