...
contract InvestmentPool is IInitializableInvestmentPool, SuperAppBase, OwnableUpgradeable {
    ...
    function getDedicatedSender() public {
        (gelatoDedicatedMsgSender, ) = IOpsProxyFactory(getGelatoOpsProxyFactory()).getProxyOf(
            address(this)
        );
    }

    function allProxyDetails() public view returns (address gelatoSender, bool success) {
        (gelatoSender, success) = IOpsProxyFactory(getGelatoOpsProxyFactory()).getProxyOf(
            address(this)
        );
    }
