...
contract BSBBarracksUpgradeable is Initializable, BSBBaseGameModeUpgradeable {
    ...
    function _claimRewards(CollectionItems calldata collectionItems_) internal {
        ...
        for (uint i = 0; i < collectionItems_.assets.length; ++i) {
            ...
            uint resultingLevel_ = bearsStats_[i].level.value + _calculateLevelReward(daysLifetime_);
            bearsStats_[i].level.value = resultingLevel_ <= bearsStats_[i].level.max ?
                resultingLevel_ : bearsStats_[i].level.max;

            uint remainingStamina_ = bearsStats_[i].stamina.value - _calculateSpentStamina(daysLifetime_, applyBuff_);
            bearsStats_[i].stamina.value = remainingStamina_ >= bearsStats_[i].stamina.min ?
                remainingStamina_ : bearsStats_[i].stamina.min;

            BSBStorageCaller.setBearStats(collectionItems_.collection, collectionItems_.assets[i], bearsStats_[i]);

            emit LevelClaimed(_msgSender(), collectionItems_.assets[i], bearsStats_[i].level.value);

            stateChangeDates[collectionItems_.collection][collectionItems_.assets[i]] = block.timestamp;

            delete remainingStamina_;
    ...
    function calculateRemainingStamina(
        ...
        uint remainingStamina_ = bearStamina_.value - _calculateSpentStamina(daysLifetime_, applyBuff_);
        return remainingStamina_ >= bearStamina_.min ? remainingStamina_ : bearStamina_.min;
    }
    ...
    function _calculateLevelReward(uint daysLifetime_) internal view returns(uint) {
        return daysLifetime_ / levelDaysRate;
    }

    function _calculateSpentStamina(uint daysLifetime_, bool applyBuff_) internal view returns(uint) {
        return daysLifetime_ * _calculateActualDecreaseRate(applyBuff_);
    }
}
