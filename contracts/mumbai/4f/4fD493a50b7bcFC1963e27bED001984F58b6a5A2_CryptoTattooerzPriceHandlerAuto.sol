...
abstract contract CryptoTattooerzPriceHandler is ICryptoTattooerzPriceHandler, CryptoTattooerzCurrencyHandler, AccessControlEnumerable{
     ...
     */
    event TokenPriceDiscountChanged(address indexed admin, uint256 endingAmountUSD, uint8 maxDiscountRate, uint8 decimals);
     ...
     */
    function transform(bytes32 fromCurrency, bytes32 toCurrency, uint256 amount) public view override returns (uint256, uint256, uint8) {
    ...
    function setTokenPriceDiscount(uint256 endingAmountUSD, uint8 maxDiscountRate, uint8 decimals) external onlyRole(PRICES_ADMIN_ROLE){
        (maxDiscountRate, decimals) = cleanFromTrailingZeros_uint8(maxDiscountRate, decimals);
    ...
    function calculateTokenPriceDiscountRate(uint256 amountUSD, uint8 decimalsUSD) public view returns(uint256, uint8){
        ...
        return cleanFromTrailingZeros(discountRate, decimals);
    }
}
...
interface ICryptoTattooerzPriceHandler {
     ...
     * rate (and its applicable decimals)
     */
    function transform(bytes32 fromCurrency, bytes32 toCurrency, uint256 amount) external view returns (uint256, uint256, uint8);
    /**
     * @dev Getter of the USD price for given currency amount (could be default chain coin "COIN", proprietary token
      "TOKEN", or any other of the handled tokens such as "USDC"...)
...
abstract contract CryptoTattooerzCurrencyHandler{
     ...
     * @dev Utility methods that allows to clean unnecessary trailing zeros to reduce size of values
     */
    function cleanFromTrailingZeros_uint8(uint8 amount, uint8 decimals) public pure returns(uint8, uint8) {
        while(decimals > 0 && amount % 10 == 0) {
            decimals--;
            amount = amount/10;
        }
