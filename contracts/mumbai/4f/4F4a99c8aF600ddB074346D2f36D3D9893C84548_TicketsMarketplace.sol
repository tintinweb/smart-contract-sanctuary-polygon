...
contract TicketsMarketplace is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event AskSetted(uint256 indexed ticketId, address indexed seller, uint256 ticketPrice, uint256 amount);
    ...
    modifier hasAllowedMembership(
        ...
        if (IERC165(contractAddress).supportsInterface(type(IERC721).interfaceId)) {
            ...
            if (!isTokenNeeded) {
                require(IERC721(contractAddress).balanceOf(msg.sender) > 0, 'Membership not owned!');
            } else {
                require(IERC721(contractAddress).ownerOf(tokenId) == msg.sender, 'Membership token not owned!');
            }
        } else {
            require(IERC1155(contractAddress).balanceOf(msg.sender, tokenId) > 0, 'Membership token not owned!');
        }
        _;
    }
     ...
     */
    function claimFreeTicket(uint256 ticketId, address claimer) external whenNotPaused {
        ...
        require(offers[creator][ticketId].price == 0, 'Ticket is not free.');
        ...
        emit TicketBought(ticketId, creator, claimer, 0, 1);
    }
    ...
    function claimTicketByMembership(
        ...
        address contractAddress,
        uint256 tokenId
    ) external hasAllowedMembership(ticketId, contractAddress, tokenId) whenNotPaused {
        address creator = ticketsProperties[ticketId].creator;
        require(ITicket(ticketAddress).balanceOf(creator, ticketId) >= 1, 'Creator hasnt enough tickets.');
        require(ITicket(ticketAddress).balanceOf(msg.sender, ticketId) == 0, 'Claimer has one ticket.');
        require(offers[creator][ticketId].amount >= 1, 'Not enough tickets for claim');

        ITicket(ticketAddress).safeTransferFrom(address(creator), msg.sender, ticketId, 1, '');
        offers[creator][ticketId].amount -= 1;

        emit TicketClaimed(ticketId, creator, msg.sender);
    }
    ...
    function buyMarketTicket(
        ...
        require(msg.sender != seller, 'You cant buy your own ticket');
        ...
        if (ticketPrice == 0) {
            ...
            if (seller == creator) {
                ...
                uint256 marketplaceShare = MarketplaceHelper.calculateFee(ticketPrice, ticketsProperties[ticketId].secondaryMarketRoyalty);
                uint256 creatorShare = MarketplaceHelper.calculateFee(ticketPrice, ticketsProperties[ticketId].creatorRoyalty);
    ...
    function setAsk(
        ...
        emit AskSetted(ticketId, msg.sender, ticketPrice, amount);
    }
    ...
    function publishTicketsForOrganizer(
        ...
        for (uint256 i = 0; i < ticketsLength; i++) {
            emit TicketPublished(
                eventId,
                organizer,
                bulkInfo.newTicketIds[i],
                bulkInfo.amounts[i],
                tickets[i].price,
                tickets[i].royalty,
                tickets[i].amountToSell,
                tickets[i].isResellable,
                bulkInfo.uris[i]
            );
        }
