...
contract Read is BasePluginWithRules {
    ...
    constructor(address _registry) {
        PLUGIN_VERSION = 1;
        PLUGIN_NAME = PluginsList.SAFE_DEAL_READ;
        registry = IRegistry(_registry);
    }

    function read(
        uint256 _dealId
    ) external view returns(DataTypes.SafeDeal memory outData) {
        ...
        vars.data = abi.encode(_dealId);

        outData = ISafeDeal(registry.safeDeal()).readDeal(vars);
    }
}
