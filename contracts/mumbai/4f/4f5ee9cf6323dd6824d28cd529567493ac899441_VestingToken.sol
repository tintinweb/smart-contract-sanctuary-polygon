...
contract VestingToken is Initializable, ERC20Upgradeable {
    ...
    function claim() external {
        ...
        uint256 releasable = _unlockedOf(msg.sender) - s_released[msg.sender];
    ...
    /// @notice Returns the available balance of unlocked tokens for a specific account
    function availableBalanceOf(address account) external view returns (uint256) {
        return _unlockedOf(account) - s_released[account];
    }

    /// @notice Returns the total amount of unlocked tokens for a specific account
    function unlockedOf(address account) external view returns (uint256) {
        return _unlockedOf(account);
    }

    /// @notice Returns the total amount of locked tokens for a specific account
    function lockedOf(address account) external view returns (uint256) {
        return s_initialLocked[account] - _unlockedOf(account);
    }
