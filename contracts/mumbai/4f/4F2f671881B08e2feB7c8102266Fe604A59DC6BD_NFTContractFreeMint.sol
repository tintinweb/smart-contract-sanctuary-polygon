...
// File: contracts/NFTContractFreeMint.sol
...
contract NFTContractFreeMint is ERC721S, ERC2981Base, Ownable {
    ...
    constructor ()
        ERC721S("TEST001", "T001")            
    {        
        initialize(            
            _msgSender(), 
            "https://assets.jokercharlie.com/nft/", 
            "https://assets.jokercharlie.com/nft/default.json", 555, 1, 1000
        );
    }

    function initialize(address owner_, string memory baseURI_, string memory notRevealedURI_, uint256 maxSupply_, uint256 tokenIdStart_, uint256 royaltyFee_) internal {        
        baseURI = baseURI_;
        notRevealedURI = notRevealedURI_;
        baseExtension  = ".json";

        _withdrawAddress = owner_;
        minterAddress  = owner_;
        royaltyAddress = owner_;

        royaltyFee = royaltyFee_;
        maxSupply  = maxSupply_;
        tokenIdStart = uint32(tokenIdStart_);
    }
    ...
    function mintTo(address to, uint256 amount) external {
        ...
        require(supply + amount <= maxSupply, "Max supply limit exceed");
        for (uint256 i; i < amount; i++) {
            _safeMint(to);
        }
    }

    function _startTokenId() internal view virtual override returns (uint32) {
        return tokenIdStart;
    }   
    ...
    ///
    function setMetadataAddress(address metadata) external onlyOwner {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        } else if (metadataAddress != address(0)) {            
            return ITokenURI(metadataAddress).tokenURI(tokenId);
        } else {
    ...
    /// @inheritdoc	ERC165
    ////
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721S,ERC2981Base) returns (bool)
    {
        return
            interfaceId == mintToInterface ||
            ERC721S.supportsInterface(interfaceId) ||
            ERC2981Base.supportsInterface(interfaceId);
    }

    /////////////////////////////////////////////////////////////////////////////////////
    /// EIP2981 - NFT Royalities Standard
    /////    
    function setRoyalties(address recipient, uint256 value) external onlyOwner {
        require(value <= 1000, "ERC2981Royalties: Too high");        
        royaltyAddress = recipient;
        royaltyFee = value;
    }
    ...
    ////////////////////////////////////////////////////////////////////////////////////
    /// beforeTokenTransfer override to locked token 
    ////
    function setLockerAddress(address locker) external onlyOwner {
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal virtual override {
        ...
        if (lockerAddress != address(0)) {
            require(!IIsTokenLocked(lockerAddress).isTokenLocked(tokenId),"Token is Locked");
        }
    ...
    /////
    /***
     * @dev Thros if called by any account other than owner or minter
     */
    modifier onlyOwnerAndMinter {    
        require(_msgSender() == owner() || _msgSender() == minterAddress, "Unauthorised call" );
        _;
    }
