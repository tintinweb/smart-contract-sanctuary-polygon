...
contract PoseToken is ERC20, ERC20Permit, IPoseToken {
    IPoseStaking private poseStaking;

    uint256 public constant INITIAL_SUPPLY = 1_000_000_000 * 10 ** 18;

    constructor(IPoseStaking _poseStaking) ERC20("POSE", "POSE") ERC20Permit("POSE") {
        _mint(address(this), INITIAL_SUPPLY);
        poseStaking = _poseStaking;
    }

    /**
     * @dev Claims `amount` tokens from the callers non-tokenized POSE balance to the caller
     *
     */
    function claim(uint256 amount) external override {
        poseStaking.withdrawPose(_msgSender(), amount);
        _transfer(address(this), _msgSender(), amount);
    }

    /**
     * @dev Claims `amount` tokens from the callers non-tokenized POSE balance to the `recipient`
     *
     */
