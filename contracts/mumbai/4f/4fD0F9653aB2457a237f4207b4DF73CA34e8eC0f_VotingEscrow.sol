...
contract VotingEscrow is IVotingEscrow, ReentrancyGuard {
        ...
        if (_addr != address(0)) {
            ...
            if (_oldLocked.end > block.timestamp && _oldLocked.delegated > 0) {
                userOldPoint.slope =
                    ...
                    int128(int256(_oldLocked.end - _oldLocked.start));
                userOldPoint.bias =
                    userOldPoint.slope *
            ...
            if (_newLocked.end > block.timestamp && _newLocked.delegated > 0) {
                userNewPoint.slope =
                    ...
                    int128(int256(_newLocked.end - _newLocked.start));
                userNewPoint.bias =
                    userNewPoint.slope *
    ...
    // See IVotingEscrow for documentation
    function createLockByPeriod(
        uint256 amount,
        LockPeriods period
    ) external override {
        _createLock(
            amount,
            block.timestamp + _getLockTimeFromPeriod(period),
            _getMultipliedAmount(amount, period)
        );
    }
    ...
    function _getLockTimeFromPeriod(
        LockPeriods period
    ) internal pure returns (uint256 lockTime) {
    ...
    function _getMultipliedAmount(
        uint256 amount,
        LockPeriods period
    ) internal pure returns (uint256 multipliedAmount) {
        if (period == LockPeriods.oneWeek) {
            return amount;
        } else if (period == LockPeriods.threeMonths) {
