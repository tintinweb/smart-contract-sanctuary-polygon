    ...
    BaseRelayRecipient
{
    ...
    mapping(uint256 => ThreeD_NFTDetails) NFTLogs;
    ...
    event Mint(
        uint256 tokenId,
        address creator,
        ThreeD_NFTDetails newNFT,
        string assetId
    );

    event MintBatch(uint256 tokenId, address creator, ThreeD_NFTDetails newNFT);
    ...
    modifier mintComplianceCostCheck() {
        require(msg.value >= mintingfee, "Not enough balance");
        _;
    }
    ...
    function minter(
        ...
        ThreeD_NFTDetails memory newNft = ThreeD_NFTDetails({
            tokenID: nextTokenId,
            tokenURI: newURI,
            creator: _creator,
    ...
    function mint3D(
        ...
        if (noOfFreeMintsPerWallet[_msgSender()] < uint256(10)) {
            minter(tokenURI, _msgSender(), mintAmount, assetId);
    ...
    function setGeneralNoOfFreeMints(uint256 _freeMints) public virtual {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
            "ThreeD_NFT: Must have admin role to change"
        );
    ...
    function CheckfreeMintWallet(address _address) public view returns (bool) {
        if (noOfFreeMintsPerWallet[_address] >= uint256(10)) return false;
        return true;
    }
    ...
    function CheckFreeMintsAvailablePerWallet(address _address)
        ...
        returns (uint256)
    {
        return 10 - noOfFreeMintsPerWallet[_address];
    }

    function getNFTDetails(uint256 id)
        external
        view
        returns (ThreeD_NFTDetails memory)
    {
        return NFTLogs[id];
    }
