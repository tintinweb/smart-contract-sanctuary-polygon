...
contract NameRegistry is Initializable, PausableUpgradeable, UUPSUpgradeable {
	 ...
	 *
	 *  Requirements:
	 *   - `msg.sender` Must be Default admin role when contractName is `AR`.
	 *   - `msg.sender` Must be a SuperManager when contractName not be `AR`.
	 */
	function setContract(string memory _contractName, address _addr) public {
		if (keccak256(bytes(_contractName)) == keccak256(bytes("AR"))) {
			require(
				AR().hasRole(DEFAULT_ADMIN_ROLE, msg.sender),
				"You're not Super Manager."
			);
		} else {
			require(
