...
contract ERC20Upgradeable is Initializable, ContextUpgradeable, IERC20Upgradeable, IERC20MetadataUpgradeable {
    ...
    function _approve( address owner, address spender, uint256 amount) internal virtual {
        require(!_freezingStatus[owner], "frozen owner address");
        require(!_freezingStatus[spender], "frozen spender Address");
    ...
    function lockupPlanSet(address account, uint32 lockupTime, uint128[] calldata lockupPlanTime, uint128[] calldata lockupPlanAmount) external returns (bool) {
        ...
        uint32 lockupPlanListLength = uint32(_initialLock[account][lockupTime].lockupPlanList.length);
        ...
        if (lockupTime < block.timestamp){
            require(lockupPlanListLength != 0,"lockupTime over");
        }
        uint128 addAmount;
        
        for (uint32 i=0; i<lockupPlanTime.length;){
            uint128 i_lockupPlanAmount = lockupPlanAmount[i];
            require(_initialLock[account][lockupTime]._lockupPlan[lockupPlanTime[i]].lockupPlanAmount == 0 && lockupPlanTime[i] >= lockupTime,"Incorrect lockupPlanTime");
            require(i_lockupPlanAmount != 0,"Incorrect lockupPlanAmount");
            unchecked{addAmount += i_lockupPlanAmount;}
            _initialLock[account][lockupTime]._lockupPlan[lockupPlanTime[i]] = lockupPlanInfo(lockupPlanListLength,i_lockupPlanAmount);
