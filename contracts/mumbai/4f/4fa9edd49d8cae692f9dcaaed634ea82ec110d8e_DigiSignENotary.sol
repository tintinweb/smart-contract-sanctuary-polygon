...
contract DigiSignIdentity is ERC721URIStorage  {
    ...
    constructor() ERC721("DigiSign Idenity", "DSI") {
        owner = msg.sender;
    }

    mapping(address => string) public personToIdenity;
    mapping(address => bool) public issuedIdentity;

    function registerIdentity(string memory tokenURI)
        public
        returns (uint256)
    {
        _tokenIds.increment();
        ...
        personToIdenity[msg.sender] = tokenURI;
        issuedIdentity[msg.sender] = true;

        return newItemId;
    }

    function checkIdentityOfPerson(address person) external view returns (string memory) {
        return personToIdenity[person];
    }

    function updateIdentity(string memory _uri) external {
          personToIdenity[msg.sender] = _uri;
    }
