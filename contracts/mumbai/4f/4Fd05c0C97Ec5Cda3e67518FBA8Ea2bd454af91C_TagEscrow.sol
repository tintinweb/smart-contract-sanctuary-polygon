    ...
    Ownable
{
    ...
    mapping(address => IdContainer[]) public walletEscrows;
    ...
    */
    function createEscrow(
        ...
        walletEscrows[msg.sender].push(IdContainer(nextEscrowId));
        walletEscrows[partyB].push(IdContainer(nextEscrowId));
        walletEscrows[partyArbitrator].push(IdContainer(nextEscrowId));
    ...
    */
    ...
    /**
    * @dev Withdraw for partyA only if 24h have passed since bet started and partyB didn't deposit.
    ...
    */
    ...
    onlyOpenCase(escrowId)
    {
        require(msg.sender == escrows[escrowId].partyA, "You are not authorized to withdraw funds from this escrow");
        ...
        if (address(escrowAssetsA[escrowId].currency) != address(0)) {
            escrowAssetsA[escrowId].currency.safeTransfer(msg.sender, escrowAssetsA[escrowId].amount - escrowAssetsA[escrowId].amount * tagFeeBps / maxFeeBps);
        } else { // GAS token
    ...
    **/
    ...
    // -- VIEWS
    function getCallerEscrows(address _address) public view returns (uint256[] memory escr) {
        uint length = walletEscrows[_address].length;
        escr = new uint256[](length);

        for(uint i = 0; i < length; i++) {
            escr[i] = walletEscrows[_address][i].escrowId;
        }
    }
}