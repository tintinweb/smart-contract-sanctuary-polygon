...
contract Margin is ReentrancyGuard, MarginStorage, Ownable {
        ...
        nonReentrant
    {
        ...
        else IERC20(assetAddress).transfer(msg.sender, amountToWithdraw);
    ...
    function repay(
        ...
        if (source == 1) {
            if (assetAddress == address(0)) {
                ...
                uint256 success = ICEther(delegator).repayBorrowBehalf{
                    value: msg.value
                }(msg.sender);
    ...
    function getDepositDetails(address user, address assetAddress)
        public
        view
        returns (
            uint256 marginAmount,
...
contract MarginStorage {
    ...
    mapping(address => mapping(address => Deposit)) internal depositDetails;
