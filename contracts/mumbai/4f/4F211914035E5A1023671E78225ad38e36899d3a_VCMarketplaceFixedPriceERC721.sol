...
interface IVCMarketplaceFixedPriceERC721 {
...
import "./IVCMarketplaceFixedPriceERC721.sol";

contract VCMarketplaceFixedPriceERC721 is IVCMarketplaceFixedPriceERC721, VCMarketplaceBase, ERC721Holder {
    ...
    /**
     * @dev See {IVCMarketplaceFixedPriceERC721-listFixedPrice}.
     */
    function listFixedPrice(
        uint256 _tokenId,
        uint256 _listPrice,
        ...
        if (status == IVCMarketManager.ListStatus.AUCTION) {
            revert MktAlreadyListedOnAuctionError();
        }

        _setFeesAndCheckRoyalty(_tokenId, _poolFeeBps, _projects, _projectFeesBps);

        if (status == IVCMarketManager.ListStatus.NOT_LISTED) {
            _newList(ListingData(_tokenId, _listPrice, _projects, _projectFeesBps));
        } else {
    ...
    /**
     * @dev See {IVCMarketplaceFixedPriceERC721-unlistFixedPrice}.
     */
    function unlistFixedPrice(uint256 _tokenId) external {
        ...
        emit MktUnlistedFixedPrice(artNft, _tokenId, msg.sender);
    }

    /**
     * @dev See {IVCMarketplaceFixedPriceERC721-purchase}.
     */
    ...
    function _newList(ListingData memory data) internal {
        ...
        fixedPriceListings[data.tokenId] = FixedPriceListing(
            minted,
            msg.sender,
            data.price,
            marketFee,
        ...
        address marketManager = admin;
        IVCMarketManager(marketManager).setListStatusERC721(data.tokenId, true);
     ...
     * @param _listPrice new list fixed price
     */
    ...
    function _minting(FixedPriceListing memory listing, uint256 _tokenId) internal {
        ...
        IVCArtNftERC721(artNft).transferFrom(address(this), msg.sender, _tokenId);
