...
struct FulfillmentFunction {
    address fulfillmentContract;
    bytes4 fulfillmentSelector;
}
...
contract Marketplace is BaseViciContract {
    ...
    mapping(uint256 => FulfillmentFunction) internal fulfillments;
    ...
    /**
     * @notice Returns true if this contract can auto-fulfill an order for
     * the `_itemId`.
     *
     * @param _itemId the item id
     */
    function hasFulfillmentFunction(uint256 _itemId)
        public
        view
        returns (bool)
    {
        return fulfillments[_itemId].fulfillmentContract != address(0);
    }

    /**
     * @notice Returns the fulfillment function for the `_itemId`.
     *
     * @param _itemId the item id
     *
     * Requirements:
     * `_itemId` MAY be for an item without a fulfillent function. In
     *    this case, the fulfillmentContract SHALL be the null address.
     */
    function getFulfillmentFunction(uint256 _itemId)
        public
        view
        returns (FulfillmentFunction memory)
    {
        return fulfillments[_itemId];
    }

    /**
     * @notice Set a fulfillment function for the `_itemId`.
     *
     * @param _itemId the item id
     *
     * Requirements:
     * `_itemId` MAY refer to an invalid item. It's legal to call this
     *    function before calling `setItem()`.
     * Caller MUST be the contract owner or have the STOCK_BOY role.
     */
    function setFulfillmentFunction(
        uint256 _itemId,
        address _contract,
        bytes4 _selector
    ) public onlyOwnerOrRole(STOCK_BOY) {
        fulfillments[_itemId] = FulfillmentFunction(_contract, _selector);
    }

    function _fulfill(uint256 _itemId, bytes memory _arguments) internal {
        FulfillmentFunction memory fulfiller = getFulfillmentFunction(_itemId);
        bytes memory encodedFunction = bytes.concat(
            fulfiller.fulfillmentSelector,
            _arguments
        );
        string memory errorMessage = string(
            abi.encodePacked(
                "Could not fulfill item ",
                _itemId,
                " using ",
                fulfiller.fulfillmentContract
            )
        );
        fulfiller.fulfillmentContract.functionCall(
            encodedFunction,
            errorMessage
        );
    }
     ...
     * @notice If the item has a fulfillment function, and if `_arguments` is not empty,
     *      this function will call the fulfillment function on the fulfillemtn contract,
     *      passing `_arguments`.
     ...
     * - If there is a fulfillment function for the item and `_arguments` is set,
     *      all requirements for the fulfillment function MUST be met. For example,
     *      if the fulfillment function is `mint()` with a requirement that the
     *      caller has the `CREATOR` role, then this contract MUST have have been
     *      granted the `CREATOR` role on the fulfillment contract.
     * - `_arguments` MAY be empty, even if a fulfillment function is set for the
     *      item. In this case, the fulfillment function SHALL not be called.
     * - `_arguments` MAY be set, even if there is no fulfillment function for
     *      the item. In this case, the `_arguments` parameter SHALL be ignored.
     */
    function pay(
        IERC20Metadata _currency,
        uint256 _itemId,
        ...
        if (hasFulfillmentFunction(_itemId) && _arguments.length > 0) {
            _fulfill(_itemId, _arguments);
        }
    }
