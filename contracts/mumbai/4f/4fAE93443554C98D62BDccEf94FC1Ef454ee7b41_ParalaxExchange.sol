...
contract ParalaxExchange is ExtensionPercent, EIP712, AccessControl {
    ...
    bytes32 public constant TIME_MULTIPLIER =
        keccak256("TimeMultiplier(uint256 amount,uint256 interval,bytes sign)");
    ...
    mapping(bytes => uint256) private _scaleAmount;
    mapping(bytes => uint256) private _lastLevel;
     ...
     */
    function initOrderDCA(
        bytes calldata sign,
        OrderDCA memory orderData,
        uint256 nonce
    ) public {
        require(_verify(sign, orderData, nonce), "Sign Error");
        require(_validateOrderDCA(orderData), "Order Error");
        require(_validatePriceDCA(orderData), "Price Error");
        require(_validateInitDCA(sign), "Init Error");

        uint256 amountIn = _levelDCA(sign, orderData);
        orderData.done += uint96(amountIn);
        _ordersDCA[sign] = orderData;
        _deadlinesDCA[sign] =
            block.timestamp +
            orderData.levels *
            orderData.period;
    ...
    function dca(bytes calldata sign) public {
        ...
        require(order.done < order.volume, "Order Error");
        require(_validatePriceDCA(order), "Price Error");

        uint256 amountIn = _levelDCA(sign, order);
        _ordersDCA[sign].done += uint96(amountIn);
    }

    function initTimeMultiplier(
        bytes calldata signDCA,
        bytes calldata signTM,
        OrderDCA memory order,
        TimeMultiplier memory tm,
        uint256 nonce
    ) external {
        require(_verify(signDCA, order, nonce), "Sign DCA Error");
        require(_verify(signDCA, signTM, tm, order.account), "Sign TM Error");
        require(_validateOrderDCA(order), "Order Error");
        require(_validatePriceDCA(order), "Price Error");
        require(_validateInitDCA(signTM), "Init Error");
        require(_timeMultipliers[signTM].amount == 0, "TM: Init Error");

        uint256 amountIn = _levelTM(signTM, order, tm);

        order.done += uint96(amountIn);
        tm.done += amountIn;

        _timeMultipliers[signTM] = tm;
        _ordersDCA[signTM] = order;
        _deadlinesDCA[signTM] = block.timestamp + order.levels * order.period;
    }

    function dcaTM(bytes calldata sign) external {
        TimeMultiplier memory tm = _timeMultipliers[sign];

        if (tm.done == tm.amount && tm.amount != 0) {
            dca(sign);
        } else {
            OrderDCA memory order = _ordersDCA[sign];
            ...
            require(tm.done < tm.amount, "TM: Order Error");

            uint256 amountIn = _levelTM(sign, order, tm);

            _ordersDCA[sign].done += uint96(amountIn);
            _timeMultipliers[sign].done += amountIn;
        }
    }
    ...
    function _levelDCA(
        ...
        require(
            block.timestamp >= _lastLevel[sign] + order.period,
            "Period Error"
        );
        ...
        if (
            order.done == 0 ||
            (order.done != 0 && order.scale == 0) ||
            (order.done != 0 &&
                order.scale != 0 &&
                _timeMultipliers[sign].done == order.done)
        ) {
            amount = order.volume / order.levels;
        } else {
            uint256 scalingValue = _scaleAmount[sign];
            (amount, ) = addPercentage(scalingValue, order.scale);
        }

        amountIn = (order.volume - order.done < amount)
            ? order.volume - order.done
            : amount;

        _scaleAmount[sign] = amountIn;
        _lastLevel[sign] = block.timestamp;
    ...
    function _levelTM(
        ...
        require(
            block.timestamp >= _lastLevel[sign] + tm.interval,
            "Interval Error"
        );
        // scale logic
        uint256 amount;

        if (tm.done == 0 || (tm.done != 0 && order.scale == 0)) {
            amount = order.volume / order.levels;
        } else {
            (amount, ) = addPercentage(_scaleAmount[sign], order.scale);
        }

        if (tm.amount - tm.done <= amount) {
            amountIn = tm.amount - tm.done;
            _scaleAmount[sign] = 0;
            _lastLevel[sign] = 0;
        } else {
            amountIn = amount;
    ...
    function verify(
        ...
        return _verify(signature, orderDCA, nonce);
    }
    ...
    function _verify(
        ...
        bytes32 hashStruct = _hashStructOrderDCA(orderDCA, nonce);
        status = EIP712._verify(orderDCA.account, signature, hashStruct);
    }
    ...
    function _verify(
        ...
        bytes32 hashStruct = _hashStructTM(tm, signDCA);
        status = EIP712._verify(account, signTM, hashStruct);
    }
