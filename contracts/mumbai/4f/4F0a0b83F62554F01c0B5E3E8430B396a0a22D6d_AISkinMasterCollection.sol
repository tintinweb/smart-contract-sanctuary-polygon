...
contract AISkinMasterCollection is
    BaseCollection,
    ERC1155,
    AccessControl,
    IAISkinMasterCollection
{
    ...
    // baseUri / collection address / tokenId
    ...
    modifier onlyDropCollection() {
        ...
        if (!IAISkinFactory(_aiskinFactory).isValidCollection(_msgSender())) {
    ...
    function getItemInfo(
        ...
        view
        virtual
        override(BaseCollection, IBaseCollection)
        returns (Item memory)
    {
        if (_dropReference[tokenId] != address(0)) {
            return
                IBaseCollection(_dropReference[tokenId]).getItemInfo(tokenId);
        }
    ...
    function uri(
        ...
        address _uriAddrsPath = _dropReference[_id] != address(0)
            ? _dropReference[_id]
            : address(this);
        return
            string(
                abi.encodePacked(_baseURI, _uriAddrsPath, "/", _id.toString())
            );
    }
    ...
    function supportsInterface(
        ...
        public
        view
        override(BaseCollection, ERC1155, AccessControl)
        returns (bool)
    {
...
abstract contract BaseCollection is ERC2981, IBaseCollection, ReentrancyGuard {
    // Token name
    ...
    function royaltyInfo(
        ...
        (address receiver, uint96 royaltyFraction) = IAISkinFactory(
            aiskinFactory()
        ).getRoyaltyInfo();
    ...
    function _distributeMintAmountReceived(
        ...
        address treasury = IAISkinFactory(aiskinFactory()).getTreasury();
