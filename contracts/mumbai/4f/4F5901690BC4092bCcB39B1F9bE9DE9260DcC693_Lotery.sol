...
contract Lotery is Ownable, Pausable{
  ...
  event Winners(uint loteryNumber, address[] winners, address[] winersNumbers);
  ...
  function finishPlay(uint[] memory randomNumber) public {
    ...
    delete totalPrize;
    emit Winners(loteryCounter-1,LastAddressWiners, viewLastAddressWiners());
  }
  ...
  function viewLastHistoricalTotalPrizes() public view returns(uint[30] memory){
    ...
    for(uint i; i < stop; i++){
      lastPrizes[historicalTotalPrize[loteryCounter-i]];
    }
