...
// File: contracts/erantomoon/Airdrop.sol
...
contract Airdrop is Ownable {
    ...
    function toggleOpen(bool _isopen) external onlyOwner {
        isopen = _isopen;
    }

    function claim(bytes32[] calldata merkleProof,address _reference) external {
        ...
        //发送总数量的万分之一
        uint256 amount=token.balanceOf(msg.sender)/(10000);
        
        //给推荐人发奖励，预算5%
        uint256 referAmt=(amount*5)/100;

        //剩余的发给调取人
        uint256 endamt=amount.sub(referAmt);

        hasClaimed[msg.sender] = true;
        claimedCount++;

        require(token.transfer(msg.sender, endamt), "Airdrop: Token transfer failed");
        require(token.transfer(_reference, referAmt), "Airdrop: Token transfer failed");
