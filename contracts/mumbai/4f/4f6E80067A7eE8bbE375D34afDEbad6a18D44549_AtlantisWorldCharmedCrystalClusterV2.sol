 ...
 *
 * @title AtlantisWorldCharmedCrystalClusterV2
 * @author Carlo Miguel Dy
 */
contract AtlantisWorldCharmedCrystalClusterV2 is
    IERC721AUpgradeable,
    ...
    ERC2981Upgradeable
{
    ...
    /// @notice The treasury address, where all sales get sent to
    address public treasuryAddr;

    /// @notice The URI for the token metadata
    string public baseURI;

    event SetTreasuryAddr(
        address indexed prevTreasuryAddr,
        address indexed newTreasuryAddr
    );
    event SetBaseUri(string indexed prevURI, string indexed newURI);
    ...
    modifier onlyAdmin() {
        require(
            ...
            'AtlantisWorldCharmedCrystalCluster: Only admin.'
        );
        _;
    }

    /**
     * @notice Sets the `treasuryAddr` for receiving the sales.
     * @param _treasuryAddr The new treasury address for receiving sales.
     */
    function setTreasuryAddr(address _treasuryAddr) external onlyAdmin {
    ...
    /**
     * @notice Sets a new `baseURI` for the metadata
     * @param newURI In string format that points to the base URL where metadata is hosted
     */
     ...
     */
    function mintTo(address to) external onlyAdmin {
        _mint(to, 1);
    }

    function tradeWithOres(uint256 tokenId) external pure {
        revert(string(abi.encodePacked('Yea, this is tokenId ', tokenId)));
    }

    /**
     * @notice Initiliazes the upgradeable contract
     */
    function initialize(address _treasuryAddr, string calldata _defBaseURI)
        public
        initializerERC721A
        initializer
    {
        __ERC721A_init('AtlantisWorldCharmedCrystalCluster', 'AWCCC');
        ...
        _setupRole(DEFAULT_ADMIN_ROLE, _treasuryAddr);
        _setDefaultRoyalty(_treasuryAddr, 2_500);
    ...
    function _mint(address to, uint256 quantity) internal virtual override {
        require(
            ...
            'AtlantisWorldCharmedCrystalCluster: Unable to mint more than the max supply.'
        );
