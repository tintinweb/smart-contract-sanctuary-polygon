    ...
    ERC2771Context
{
    ...
    event PlayerMadeDecision(
        uint256 indexed _gameId,
        uint256 indexed _player,
        PlayerDecision _playerDecision
    );

    event GettingVRF(uint256 indexed _gameId);
    ...
    event HouseWins(uint256 indexed _gameId, uint256 _payout);
    ...
    constructor(
        ...
        TreasuryManagerConstructorArgs memory _TreasuryManagerConstructorArgs,
        GameManagerConstructorArgs memory _gameManagerConstructorArgs
    )
        ERC2771Context(_forwarder)
        VRFManager(_vrfManagerConstructorArgs)
        TreasuryManager(_TreasuryManagerConstructorArgs)
        GameManager(_gameManagerConstructorArgs)
    {
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
    }

    function startHolyWar(uint256 _ante) public {
        address sender = _msgSender();

        // add revert with custom error statement including game numebr
        require(
        ...
        require(
            _ante >= minimumAnte && _ante <= maximumAnte,
            "startHolyWar: ante not within the min/max"
        );

        // check has enough balance (max bet including backup bet?)
        require(
            balances[sender] >= _ante,
            "startHolyWar: players balance less than _ante"
        );

        // check house has enough balance (max payout)
        require(
            houseBalance >= (_ante * 2),
            "startHolyWar: house balance less than max payout"
        );
        ...
        gameCounter++;
        initializeGameData(gameCounter, sender, _ante);
        emit GameCreated(gameCounter, sender, _ante);
        deal(gameCounter);
    }
    ...
    function surrender() public {
        ...
        uint256 payout = gameData[gameId].ante / 2;
        gameData[gameId].winner = Winner.SURRENDER;

        holds[gameData[gameId].player] -= gameData[gameId].ante;
        houseHold -= gameData[gameId].ante * 2;

        balances[sender] += payout;
        houseBalance += gameData[gameId].ante * 2 + payout;

        resetPlayerInGame(sender);
        emit PlayerSurrenders(gameId, sender, payout);
    }

    function goToWar(uint256 sideWager) public {
        address sender = _msgSender();
        uint256 gameId = playerInGame[sender];
        ...
        require(
            balances[sender] >= gameData[gameId].ante + sideWager,
            "PlayerDecision: not enough in player balance to go to war and sidebet"
        );

        require(
            houseBalance >= 10 * sideWager,
            "PlayerDecision: the house ballance cannot cover this side bet"
        );

        balances[sender] -= gameData[gameId].ante + sideWager;
        holds[sender] += gameData[gameId].ante + sideWager;

        houseBalance -= sideWager * 10;
        houseHold += sideWager * 10;

        deal(gameId);
    }

    function deal(uint256 _gameId) private {
        emit GettingVRF(_gameId);

        if (gameData[_gameId].state == GameState.INIT) {
    ...
    function fulfillRandomWords(
        ...
        if (gameData[gameId].state == GameState.GETTING_VRF_1) {
            // store raw random numbers
            gameData[gameId].randomNumbers[0] = _randomWords[0];
            gameData[gameId].randomNumbers[1] = _randomWords[1];

            // reduce random number in range
            // swap index in array

            shuffleDeck(0, 2, gameId);

            uint256 playerCard = gameData[gameId].deck[0] % 13;
            uint256 houseCard = gameData[gameId].deck[1] % 13;
            ...
            if (playerCard == houseCard) {
                ...
                balances[gameData[gameId].player] += payout;
                houseBalance += gameData[gameId].ante;

                resetPlayerInGame(gameData[gameId].player);
                emit PlayerWins(gameId, gameData[gameId].player, payout);
            } else if (playerCard < houseCard) {
                gameData[gameId].state = GameState.COMPLETED;
                uint256 payout = gameData[gameId].ante * 3;
            ...
            shuffleDeck(2, 8, gameId);

            uint256 playerCard = gameData[gameId].deck[8] % 13;
            uint256 houseCard = gameData[gameId].deck[9] % 13;
            ...
            if (playerCard == houseCard) {
                ...
                holds[gameData[gameId].player] -= ante * 2 + sidebet;
                houseHold -= ante * 2 + sidebet * 10;
    ...
    function shuffleDeck(
        ...
        uint256 lastDeckIndex = _startingIndex + _numberOfCards - 1;
        for (
            uint256 i = _startingIndex;
            i < _startingIndex + _numberOfCards;

        ) {
            swapCardPositions(
                _gameId,
                _startingIndex,
                reduceRandomNumberInRange(
                    gameData[_gameId].randomNumbers[i],
                    i,
                    lastDeckIndex
                )
            );
    ...
    function capture(uint256 _gameId) public {
        ...
        uint256 payout = gameData[_gameId].ante / 2;
        gameData[_gameId].winner = Winner.SURRENDER;

        holds[gameData[_gameId].player] -= gameData[_gameId].ante;
        houseHold -= gameData[_gameId].ante * 2;

        balances[gameData[_gameId].player] += payout;
        houseBalance += gameData[_gameId].ante * 2 + payout;

        resetPlayerInGame(gameData[_gameId].player);
        emit PlayerSurrenders(_gameId, gameData[_gameId].player, payout);
    }
    ...
    function updateBeneficiary(address _beneficiary)
        ...
        onlyRole(GAME_MANAGER_ROLE)
    {
        super.updateBeneficiary(_beneficiary);
    }

    function updateCallBackGasLimit(uint32 _limit)
        public
        override
        onlyRole(GAME_MANAGER_ROLE)
    {
    ...
    function houseWithdraw(uint256 _amount)
        public
        override
        onlyRole(GAME_MANAGER_ROLE)
    {
        super.houseWithdraw(_amount);
    }
...
contract VRFManager is VRFConsumerBaseV2 {
    ...
    function reduceRandomNumberInRange(
        uint256 _raw,
        uint256 _min,
        uint256 _max
    ) public pure returns (uint256) {
        return (_raw % (_max - _min)) + _min;
    }
...
struct TreasuryManagerConstructorArgs {
    address beneficiary;
    IERC20 anteTokenContract;
}
...
contract TreasuryManager {
    ...
    IERC20 public anteTokenContract;

    event BeneficiaryUpdated(address _beneficiary);
    event AnteTokenContractUpdated(IERC20 _anteTokenContract);
    event Deposit(address indexed _player, uint256 _amount);
    ...
    constructor(
        ...
        beneficiary = _TreasuryManagerConstructorArgs.beneficiary;
        anteTokenContract = _TreasuryManagerConstructorArgs.anteTokenContract;
    }

    function updateBeneficiary(address _beneficiary) public virtual {
        beneficiary = _beneficiary;
        emit BeneficiaryUpdated(_beneficiary);
    }

    function updateAnteTokenContract(IERC20 _anteTokenContract) public virtual {
        anteTokenContract = _anteTokenContract;
        emit AnteTokenContractUpdated(_anteTokenContract);
    }

    function playerDeposit(uint256 _amount) public {
        // fix for meta transactions
        // address sender = _msgSender();
        anteTokenContract.safeTransferFrom(msg.sender, address(this), _amount);
    ...
    function playerWithdraw(uint256 _amount) public {
        ...
        require(
            ...
            "playerWithdraw: amount exceeds players balance"
        );

        anteTokenContract.safeTransfer(msg.sender, _amount);
        balances[msg.sender] -= _amount;
        emit Withdraw(msg.sender, _amount);
    }

    /// @notice This function depostis to the HOUSE BALANCE.
    /// @dev This will not credit your balance for sender.
    function houseDeposit(uint256 _amount) public {
        // fix for meta transactions
        // address sender = _msgSender();
        anteTokenContract.safeTransferFrom(msg.sender, address(this), _amount);
        houseBalance += _amount;

        emit HouseDeposit(_amount);
    }

    function houseWithdraw(uint256 _amount) public virtual {
        require(
            houseBalance >= _amount,
            "houseWithdraw: house balance does not cover withdraw"
        );
        anteTokenContract.safeTransfer(beneficiary, _amount);
        houseBalance -= _amount;
...
contract GameManager {
    ...
    function initializeGameData(
        ...
        gameData[_gameCounter] = GameData({
            ...
            playerDecision: PlayerDecision.PRAY,
            requestId: new uint256[](2)
        });

        playerInGame[_player] = _gameCounter;
    }

    function createDeck() private view returns (uint256[] memory) {
        uint256 numberOfCards = numberOfDecks * 52;
        uint256[] memory initialDeck = new uint256[](numberOfCards);

        for (uint256 i = 0; i < numberOfCards; ) {
            initialDeck[i] = i + 13;
            unchecked {
                i++;
            }
    ...
    function swapCardPositions(
        ...
        uint256 tempValue = gameData[_gameId].deck[_secondIndex];
        gameData[_gameId].deck[_secondIndex] = gameData[_gameId].deck[
            _firstIndex
        ];
        gameData[_gameId].deck[_firstIndex] = tempValue;
    }
    ...
    function updateMinimumAnte(uint256 _minimumAnte) public virtual {
        minimumAnte = _minimumAnte;
        emit AnteMinimumUpdated(_minimumAnte);
    }

    function updateMaximumAnte(uint256 _maximumAnte) public virtual {
