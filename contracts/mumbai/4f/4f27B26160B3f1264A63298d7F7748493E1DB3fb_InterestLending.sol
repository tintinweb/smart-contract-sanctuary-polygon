...
contract InterestLending is ReentrancyGuard, Calcs, ILending {
    ...
    event ClaimRewardsSuccess(address indexed wallet, uint256 indexed idLending, uint256 amount);
    event ReinvestRewardsSuccess(address indexed wallet, uint256 indexed idLending, uint256 amount);
    event ClaimRequestSuccess(address indexed wallet, uint256 indexed idLending, uint256 amountRewards);
    ...
    function reinvestRewards(uint256 _idLending, address _token) public nonReentrant validToken(_token) isPaused{
        ...
        // valida que no se pueda reinvertir si no tienen rewards por un valor representable en 6 digitos.
        totalRewards /= 1e12; // si no es representable en 6 digitos la division dara 0
        if(totalRewards == 0){
            revert("You have not enough rewards yet");
        }
        (, uint96 claimTime,) = LControl.getRewardsClaimed(msg.sender, _idLending);
         uint256 checkLastClaimTime = block.timestamp - uint256(claimTime);
        if(checkLastClaimTime <= waitTimeForRewards){
            revert("You Must Wait At Least 24 Hours To Reinvest");
        }
    ...
    function claimRewards(uint256 _idRequest, uint256 _idLending, address _token) public nonReentrant validToken(_token) isPaused{
        ...
        if(rewards == 0 && state != Status.pending){
        ...
        IVaultLendersRewards(lenderRewards).claimRequest(rewards);
        IERC20(_token).safeTransfer(msg.sender, rewards);
        /* if(!IERC20(_token).transfer(msg.sender, rewards)){
            revert("transfer rewards fail");
        } */
    ...
    function SetClaimRewardsRequest(uint256 _idLending, address _token) public nonReentrant validToken(_token) isPaused{
        ...
        if(amount == 0){
         ...
         /* (,uint256 rewards, Status state, ) = LControl.getRequest(msg.sender, _idLending); // ahora maneja idRequest
        if(rewards != 0 && state == Status.pendrewards){
            revert("You already has an open request");
        }  */
        (uint256 amountRewards, uint256 timeOfCalc) = calcInterestAccumulated(msg.sender, _idLending);
        uint256 totalRewards = amountRewards + LControl.getPendingRewards(msg.sender, _idLending);
        totalRewards /= 1e12;
    ...
    function updateWaitTimeForRewards(uint256 _newWaitTime) public onlyDev{
        waitTimeForRewards = _newWaitTime;
    }

    function getWaitTimeForRewards() public view returns(uint256){
    ...
    ///@dev function to update the TokenInfo contract if needed
    function updateTokenInfo(address _newTokenInfo) public onlyDev checkAddress0(_newTokenInfo) {
         tokenInfo = TokenInfo(_newTokenInfo);
    }
    ///@dev function to update the Lending control contract if needed
    function updateLendingControl(address _LendingControl) public onlyDev checkAddress0(_LendingControl) {
    ...
    function permissions(address _vaulLenders, address _token) public onlyDev validToken(_token) checkAddress0(_vaulLenders){
        ...
        //IERC20(_token).approve(_vaulLenders, MAX_UINT);
    }
