...
contract DAppSocialPool is Ownable {
    ...
    function createRecord(uint256 id, address tokenAddress, uint256 amount) external  {
        require(_supportedTokens[tokenAddress], "Token is not supported");
        _acceptedRecords[id][tokenAddress] = amount;

    }

    // Will be called only after cancel on Source
    function updateRecord(uint256 id, address tokenAddress) external  {
    ...
    function updateAmount(uint256 id, address tokenAddress, address fromAddress, address toAddress, uint256 initialAmount, uint256 amount) external  {
    ...
    function cancelRequest(address tokenAddress, address fromAddress, uint256 initialAmount) external  {
