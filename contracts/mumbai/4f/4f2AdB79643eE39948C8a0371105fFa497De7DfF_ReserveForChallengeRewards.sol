...
contract ReserveForChallengeRewards is Ownable { 
    ...
    IERC20 public BZAI;
    uint256 _nextUpdateTimestamp;
    bool _rewardsFinished = false;
    uint256 public monthlyAddOn = 3000000 * 1E18; //   3M added rewards/month for 50Months

    constructor(address _BZAI){
        BZAI = IERC20(_BZAI);
        _nextUpdateTimestamp = block.timestamp;
    }

    modifier onlyRewards(){
        require(msg.sender == gameAddresses.getChallengeRewardsAddress(),"only Rewards contract auth");
        _;
    }
    ...
    function updateRewards() external onlyRewards returns(bool){
        if(BZAI.balanceOf(address(this)) < monthlyAddOn){
            monthlyAddOn = BZAI.balanceOf(address(this));
        }
        return(BZAI.transfer(msg.sender,monthlyAddOn));
    }
