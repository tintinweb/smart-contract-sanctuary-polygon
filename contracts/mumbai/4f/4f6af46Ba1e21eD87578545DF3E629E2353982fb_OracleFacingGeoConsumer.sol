...
contract OracleFacingGeoConsumer is ShambaGeoConsumer {
    ...
    function sendRequestToOracle(
        ...
        string memory start_date,
        string memory end_date,
        Geometry[] memory geometry) public onlyOwner {
        
        MarketContractMPX mpx;
        require(compareStrings(mpx.ORACLE_STATISTIC(), ''), "ORACLE_STATISTIC is not assigned yet");
        requestGeostatsData(mpx.ORACLE_STATISTIC(), dataset_code, selected_band, image_scale, start_date, end_date, geometry);
    }

    function callbackMarketProtocol(uint price) public onlyOwner {
        MarketContractMPX mpx;
        mpx.oracleCallBack(price, getGeostatsData());
    }
