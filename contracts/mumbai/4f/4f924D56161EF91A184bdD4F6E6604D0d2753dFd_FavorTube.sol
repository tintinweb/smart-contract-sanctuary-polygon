...
import "./UsersConfig.sol";
import "./utils/Address.sol";
import "./LockBalance.sol";
import "./Uniswap.sol";

contract FavorTube is IERC223Recipient, UsersConfig, Ownable, LockBalance, Uniswap, ReentrancyGuard {
    ...
    constructor(
        ...
        Mode memory mode,
        uint32 subscribeBlock_,
        address ERC223Token,
        uint interval,
        address router
    ) UsersConfig(userConfig, mode) LockBalance(interval) Uniswap(router) {
    ...
    function exchange() external override nonReentrant {
        ...
        uint amount = _exchange(_msgSender());

        require(IERC223(_token).transfer(_msgSender(), amount), "Withdrawal Failure");
    }

    // UsersConfig
    function setMode(uint8 modeType, Mode memory mode) external override onlyOwner {
        _setMode(modeType, mode);
    }

    function removeMode(uint8 modeType, uint index) external override onlyOwner {
        _removeMode(modeType, index);
    }

    function setDefaultUserConfig(UserConfig memory userConfig) external override onlyOwner {
        _setDefaultUserConfig(userConfig);
    }

    function setUserConfig(UserConfig memory userConfig) external override {
        _setUserConfig(_msgSender(), userConfig);
    }

    // FavorTube
    function tokenReceived(address _sender, uint _value, bytes memory _data) public override {
        ...
        UserConfig memory config = getUserConfig(channel);

        require(_value >= config.price, "Paid price is too low");

        bool isZeroAddress = sharer == address(0);

        uint8[3] memory computedTaxRate = getComputedTaxRate(isZeroAddress, getMode(config.modeType).taxRate);
    ...
    function withdrawTax(address account, uint value) external onlyOwner returns (bool success) {
    ...
    function swapToken(address tokenIn, uint24 fee, uint256 amountInMaximum, uint256 amountOut, bytes memory data) external payable {
        ...
        require(msg.value == 0 ? exactTransferToken(params) : ethTransferToken(params));
        tokenReceived(_msgSender(), amountOut, data);
    }
}
...
abstract contract UsersConfig {
    ...
    mapping(address => UserConfig) public usersConfig;
    mapping(address => bool) public isSetConfig;
    UserConfig public defaultUserConfig;

    mapping(uint8 => Mode) public modes;
    mapping(uint8 => bool) public modeSet;
    uint8[] modesKey;

    modifier modeTypeExist(uint8 modeType) {
        require(modeSet[modeType], "Mode type does not exist");
        _;
    }

    constructor(UserConfig memory userConfig, Mode memory mode) {
        _setMode(userConfig.modeType, mode);
        _setDefaultUserConfig(userConfig);
    }

    function getMode(uint8 modeType) public view returns (Mode memory) {
        return modes[modeType];
    }

    function getModesKey() external view returns (uint8[] memory) {
        return modesKey;
    }

    function getUserConfig(address addr) public view returns (UserConfig memory) {
        if (!isSetConfig[addr]) return defaultUserConfig;
        UserConfig memory userConfig = usersConfig[addr];
        if (!modeSet[userConfig.modeType]) {
            userConfig.modeType = defaultUserConfig.modeType;
        }
        return userConfig;
    }

    function _setMode(uint8 modeType, Mode memory mode) internal {
        if (!modeSet[modeType]) {
            modesKey.push(modeType);
        }
        modes[modeType] = mode;
        modeSet[modeType] = true;
    }

    function _removeMode(uint8 modeType, uint index) internal modeTypeExist(modeType) {
        require(modesKey[index] == modeType, "Index Error");
        delete modes[modeType];
        delete modeSet[modeType];
        modesKey[index] = modesKey[modesKey.length - 1];
        modesKey.pop();
    }

    function _setUserConfig(address addr, UserConfig memory userConfig) internal modeTypeExist(userConfig.modeType) {
        Mode memory mode = modes[userConfig.modeType];
        require(userConfig.price <= mode.maxPrice && userConfig.price >= mode.minPrice, "Incorrect price setting");
        usersConfig[addr] = userConfig;
        isSetConfig[addr] = true;
        emit $setUserConfig(addr, userConfig.price, userConfig.modeType);
    }

    function _setDefaultUserConfig(UserConfig memory userConfig) internal modeTypeExist(userConfig.modeType) {
        defaultUserConfig = userConfig;
    }

    function setMode(uint8, Mode memory) external virtual;

    function removeMode(uint8, uint) external virtual;

    function setUserConfig(UserConfig memory) external virtual;

    function setDefaultUserConfig(UserConfig memory) external virtual;
}
...
interface IUniswapRouter is ISwapRouter, IPeripheryPaymentsWithFee {}
...
abstract contract Uniswap {
    ...
    function ethTransferToken(ISwapRouter.ExactOutputSingleParams memory params) internal returns (bool) {
        uint256 amountIn = uniswapRouter.exactOutputSingle{value : msg.value}(params);
        uniswapRouter.refundETH();
        TransferHelper.safeTransferETH(msg.sender, params.amountInMaximum - amountIn);
        return true;
    }

    function exactTransferToken(ISwapRouter.ExactOutputSingleParams memory params) internal returns (bool)  {
        TransferHelper.safeTransferFrom(params.tokenIn, msg.sender, address(this), params.amountInMaximum);
        TransferHelper.safeApprove(params.tokenIn, address(uniswapRouter), params.amountInMaximum);
...
abstract contract LockBalance {
    ...
    function _exchange(address addr) internal returns (uint) {
