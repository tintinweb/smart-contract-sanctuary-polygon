...
uint8 constant INVEST_TYPE_CRYPTO = 0;
uint8 constant INVEST_TYPE_FIAT = 1;
uint8 constant INVEST_TYPE_REINVEST_REQUEST = 2;
uint8 constant INVEST_TYPE_REINVEST = 3;
uint8 constant INVEST_TYPE_PAYOUT_CRYPTO = 4;
uint8 constant INVEST_TYPE_PAYOUT_FIAT = 5;
uint8 constant INVEST_TYPE_PAYOUT_RETURN = 6;
uint8 constant INVEST_TYPE_PAYOUT_FIAT_DONE = 7;
uint8 constant INVEST_TYPE_PAYOUT_CRYPTO_FAILED_FUNDING = 8;
...
contract Invest is Ownable {
    ...
    event NewProject(ProjectInvestImplementer.Project project, string uid);
    ...
    mapping (address => bool) public allowedOffChainInvest;
    ...
    function addProject(ProjectInvestImplementer.Project calldata proj, address destWallet, string calldata uid) external onlyOwner returns (uint32) {
        projectInvestImplementer.addProject(proj, destWallet, uid);
        emit NewProject(proj, uid);
        return proj.id;
    }
    ...
    function invest(uint32 projectId, uint16 tokenId, uint256 amount) external {
        ...
        require(isInvestmentAllowed(projectId, tokenId, msg.sender, netInv, block.timestamp), "Investment not possible - check project and limits");
    ...
    function isInvestmentAllowed(uint32 projectId, uint16 tokenId, address user, uint256 netInv, uint256 timestamp)  internal view returns (bool) {       
        ...
        try projectInvestImplementer.getProject(projectId) returns (ProjectInvestImplementer.Project memory _project) {
            ...
            return false; // Error if project does not exist
        }
        return  userHasActiveToken(tokenId, user) &&
                timestamp >= project.fundingStart && 
                timestamp <= project.fundingEnd &&
                netInv >= project.minInvestmentPerUser && 
                userInvestmentImplementer.getUserTotalInvest(tokenId, projectId) + netInv <= project.maxInvestmentPerUser &&
                netInv + projectInvestImplementer.getProjectTotalInvestment(projectId) <= project.maxFundingTarget;
    }
    ...
    function reInvestRequest(uint32 fromProjectId, uint32 toProjectId, uint16 tokenId, bool isCrypto, uint256 amount) external{
        ...
        canScheduleReinvest(fromProject, toProject, tokenId, isCrypto, amount, block.timestamp);
        reinvestRequestImplementer.incrementValue(tokenId, fromProjectId, toProjectId, isCrypto, amount);
    ...
    function canScheduleReinvest(ProjectInvestImplementer.Project memory fromProject, 
        ...
        require(scheduleTimestamp <= fromProject.fundingEnd + fromProject.duration, "Cannot schedule reinvest; Source project has reached maturity"); 
        require(isReinvestmentPossible(fromProject, toProject, tokenId, isCrypto, amount, fromProject.fundingEnd + fromProject.duration), "Cannot schedule reinvest");
    ...
    function isReinvestmentPossible(ProjectInvestImplementer.Project memory fromProject, 
                                ...
                                bool isCrypto, 
                                uint256 amount, 
                                uint256 reinvestTimestamp ) internal view returns (bool) {
        return  fromProject.investmentCurrency == toProject.investmentCurrency &&
                isInvestmentAllowed(fromProject.id, tokenId, msg.sender, amount, reinvestTimestamp) &&
                userInvestmentImplementer.getValue(tokenId, fromProject.id, isCrypto) * (10000 + fromProject.returnRate) / 10000 >= amount;
    }



    function addFiatInvestment(uint32 projectId, uint16 tokenId, address destWallet, uint256 netInv) external allowOffChainInvest { //net investment should be passed. Any fee, if it exsists, should be handled by the caller.
        require(isInvestmentAllowed(projectId, tokenId, destWallet, netInv, block.timestamp), "Investment not allowed");
    ...
    function executeReinvest(uint16 tokenId, ProjectInvestImplementer.Project memory fromProj, ProjectInvestImplementer.Project memory toProj, bool isCrypto) internal {
        uint256 value = reinvestRequestImplementer.getValue(tokenId, fromProj.id, toProj.id, isCrypto);
        if(value > 0 ) {
            if(isReinvestmentPossible(fromProj, toProj, tokenId, isCrypto, value, block.timestamp)){
                reinvestRequestImplementer.decrementValue(tokenId, fromProj.id, toProj.id, true, value);
                userInvestmentImplementer.decrementValue(tokenId, fromProj.id, true, value); 
                projectInvestImplementer.decrementProjectInvestment(fromProj.id, true, value);
                if(isCrypto){
                    ProjectInvestImplementer.Project memory proj = projectInvestImplementer.getProject(toProj.id);
                    userInvestmentImplementer.incrementPayedCrypto(toProj.id, tokenId, toProj.investmentCurrency, exchange(proj.investmentCurrency, CURRENCY_USDC, value) );
                }
                _invest(toProj.id, tokenId, true, value);
                emit InvestEvent(INVEST_TYPE_REINVEST, fromProj.id, toProj.id, tokenId, value, true, fromProj.investmentCurrency);
            }
        }
    }

    function markFiatPaymentAsDone(uint16 tokenId, uint32 projectId, uint8 currency, uint256 value) external onlyOwner {
        require(userInvestmentImplementer.getFiatPayment(tokenId, currency) >= value, "markFiatPaymentAsDone: insufficient fiat payment");
    ...
    function exchange(uint8 sourceCurrency, uint8 destCurrency, uint256 amount) internal view returns (uint256){
        return amount; //TODO call exchange SC
    }

    modifier allowOffChainInvest(){
        require(msg.sender == owner() || allowedOffChainInvest[msg.sender], "Invest: offchain invest not allowed");
        _;
    }

    function setAllowedOffChainInvest(address _address, bool _allowed) external onlyOwner {
        allowedOffChainInvest[_address] = _allowed;
    }
