    ...
    */
    ...
    function packClaimable(
        ...
        if (_tokenAddress != address(0)) {
            ...
            for (uint256 i; i < rebates.length; ++i) {
                if (
                    rebates[i].tokenAddress == _tokenAddress &&
                    (rebates[i].tokenType == TokenType.ERC721 ||
                    (rebates[i].tokenType == TokenType.ERC1155 && rebates[i].tokenId == _tokenId))
                ) {
    ...
    function applyOwnershipRebates(
        ...
        for (uint256 i; i < _rebates.length; ++i) {
            if (
                _rebates[i].tokenAddress == _tokenAddress &&
...
/// @title Paper.xyz extension for Gravity Grade
interface IGravityGrade_Paper {
    /**
