...
contract CapitalProvider_Covest is
	Initializable,
	UUPSUpgradeable,
	ReentrancyGuardUpgradeable
{
	...
	mapping(uint256 => mapping(address => SnaphotCA))
		internal claimIdCASnapshotAddress;
	...
	event ReInsuranceFeeNative(
		address indexed from,
		address indexed to,
		uint256 value
	);
	event ReInsuranceFeeAsset(
		address indexed from,
		address indexed to,
		address indexed asset,
		uint256 value
	...
	modifier ForFinalize() {
		require(
			msg.sender == address(this) || AR.hasRole(MANAGER_ROLE, msg.sender),
			'FF'
		);
		_;
	}
	...
	function initialize(address _nameRegistry, uint256 _percentage)
		public
		initializer
	{
	...
	function fundInFlow(
		...
		_historyOfPolicyId[_policyId] = HistoryOfPolicyIdData(
			_caller,
			_asset,
			0,
			0,
	 ...
	 */
	function decodedClaimPolicy(bytes memory _data)
		internal
		pure
		returns (ClaimPolicyParams memory)
	{
		(
			address user,
			address asset,
	...
	function policyClaimInternal(
		...
		require(
			keccak256(bytes(_ipfsHash)) ==
				keccak256(bytes(decodedData.ipfsHash))
		);
	 ...
	 *  - `msg.sender` Must be a SuperManager.
	 */
	function reInsuranceFeeNative(address _to, uint256 _value)
		public
		nonReentrant
		isSuperManager
	{
	...
	function policyClaimRequest(
		...
		(
			IPolicyDetails.PolicyIdData memory policyData,
			bool isActive,
			bool isRedeemed
		...
		claimIdDataThisId.caSnapshotValue = S
			.workData(IStaking.StakingTypes.CA)
			.powerCount;
	...
	function firstCAVote(
		...
		IStaking.StakingData memory dataCA = S.stakingData(
			msg.sender,
		...
		if (
			claimIdDataThisId.caSnapshotVoteCount ==
			claimIdDataThisId.caSnapshotCount
		) {
			_finalize(_claimId);
		}
	 ...
	 *  - `msg.sender` Must be worker of ClaimAssessors.
	 */
	function caProposalVoting(uint256 _claimId, bool _boolean)
		public
		nonReentrant
		isCA
	{
	...
	function claimIdCancel(uint256 _claimId) public nonReentrant {
		...
		require(
			claimIdDataThisId.user == msg.sender ||
				AR.hasRole(MANAGER_ROLE, msg.sender) == true,
			'NOMR'
		);
		require(
			_claimIdStatus[_claimId] != 5 &&
				(_claimIdStatus[_claimId] == 0 || claimIdStatus(_claimId) == 6),
			'AC-NP'
		);
		address _asset = claimIdDataThisId.asset;
		require(_asset != address(0), 'IA');
		_claimIdStatus[_claimId] = 5;
		_historyOfPolicyId[claimIdDataThisId.policyId]
			.claimPending -= claimIdDataThisId.requestAmount;
	...
	function _finalize(uint256 _claimId) private {
		...
		require(policyIsActive == true, 'PNA');
		uint256 pass = (claimIdDataThisId.caSnapshotValue *
			percentageWillPassWhenRequest) / 100;
		...
		if (claimIdDataThisId.firstCheckerBool == true) {
			if (claimIdDataThisId.votingPass >= pass) {
				...
				_historyOfPolicyId[claimIdDataThisId.policyId]
					.claimAmountPaid += approveAmount;
				totalClaimValuePaid += approveAmount;
				uint256 _amountTransfer = approveAmount /
					10**(18 - decimalAsset);
				IERC20(_asset).transfer(
					claimIdDataThisId.user,
					_amountTransfer
				);
			} else {
	...
	function claimIdStatus(uint256 _claimId) public view returns (uint8) {
		...
		if (
			_claimIdStatus[_claimId] == 0 &&
			block.timestamp > _claimIdData[_claimId].timeout
		) {
			return 6;
		} else if (
			_claimIdStatus[_claimId] == 1 &&
		...
		returns (ClaimIdRequestData[] memory, uint256[] memory)
	{
		...
		uint256[] memory _claimIdArrayBefore = new uint256[](
			countClaimForRequest
		);
		...
		ClaimIdRequestData[] memory arrayStatus = new ClaimIdRequestData[](
			_keepNumberLength
		);
		uint256[] memory _claimIdArray = new uint256[](_keepNumberLength);

		for (uint256 index = 0; index < _keepNumberLength; index++) {
			ClaimIdRequestData memory claimIdMetadataIndex = claimIdData(
				_claimIdArrayBefore[index]
			);
