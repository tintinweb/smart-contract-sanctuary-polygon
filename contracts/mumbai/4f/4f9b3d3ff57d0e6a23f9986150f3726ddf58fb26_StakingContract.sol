...
event StakeRecord(uint256 stakeAmount , uint256 stakingPlan ,uint256 withDrawTime , uint256 rewardCalculations, address stakerAddress);
...
function unstakeBeforeTime(uint256 count) public nonReentrant  {
    ...
    require(block.timestamp < user.stakerecord[count].withdrawTime,"time already completed");
...
function unstakeUponTimeCompletion(uint256 count) public {
