...
import "./interfaces/IDaoBonusDistribution.sol";
...
/**
 * @title DAO Treasury Box
 * @author Deepp Dev Team
 * @notice Contract for storing tokens in a locked state.
 ...
 */
contract DaoBonusDistribution is IDaoBonusDistribution, LockBox {
    ...
    // token => amount
    mapping(address => uint256) public addedBonus;

    // Holds the amount of pending bonus, per token.
    // token => amount
    mapping(address => uint256) public releasedBonus; // Only used for external info
    ...
    mapping(address => mapping(address => uint256)) public availableBonus;

    // portion id => portion data
    mapping(uint8 => BonusPortion) public portions;
    ...
    {
        BonusPortion storage portion =  portions[currentPortion];
    ...
    function releaseBonus(address[] calldata inRecipients) external override whenNotPaused {
        ...
        // Calc the relative bonus assignment using extra 8 digits decimal precission
        uint256 bonusPerProgress = portion.amount * 1e8 / portion.progress;
        ...
        for(uint256 i=0; i<inRecipients.length; i++) {
            ...
            uint256 userBonus = userProg * bonusPerProgress / 1e8; // remove the additional 8 digits precission again
            released += userBonus;
    ...
    function setPortionState(uint8 inId, BonusState inState) external override onlyRole(BONUS_CONTROLLER_ROLE) {
        BonusPortion storage portion =  portions[inId];
        bool success = false;
        // Check that change is valid
        if (portion.state == BonusState.Undefined) {
        ...
        if (portion.state == BonusState.Pending && inState == BonusState.Active) {
            ...
            if (inId == 1 || portions[inId - 1].state == BonusState.Released) {
                success = true;
                releasedProgress = 0;
                currentPortion = inId;
            }
    ...
    function validateNewPortion(BonusPortion memory inPortion) private view returns (string memory) {
        ...
        if (!_isAllowedToken(token)) {return "INVALID_TOKEN_ZERO";}
...
abstract contract TokenValidator is ITokenValidator, AccessHandler {
    ...
    modifier onlyAllowedDistToken(address distTokenAdd) {
        ...
        if (!isERC20(distTokenAdd)) {
            revert InvalidToken({tokenAdd: distTokenAdd});
        }
        ...
        if (!isERC20(tokenAdd)) {
            revert InvalidToken({tokenAdd: tokenAdd});
        }
        if (enabledValidation && !hasRole(TOKEN_ROLE, tokenAdd)) {
            revert TokenNotAllowed({tokenAdd: distTokenAdd});
        }
        _;
    }
    ...
    modifier onlyAllowedToken(address tokenAdd) {
        ...
        if (enabledValidation && !hasRole(TOKEN_ROLE, tokenAdd)) {
            revert TokenNotAllowed({tokenAdd: tokenAdd});
        }
        _;
    }
        ...
        returns (bool)
    {
        ...
        return _isAllowedDistToken(distTokenAdd);
    }
        ...
        returns (bool)
    {
        ...
        return _isAllowedToken(tokenAdd);
    }
     ...
     * @return bool True if the dist token is allowed, false if not.
     */
    function _isAllowedDistToken(address distTokenAdd)
        internal
        view
        returns (bool)
    {
     ...
     * @return bool True if the token is allowed, false if not.
     */
    function _isAllowedToken(address tokenAdd)
        internal
        view
        returns (bool)
    {
        ...
        internal
    {
        ...
        // Check if the token is added already, we dont want overwrites.
     ...
     * @param tokenAdd The token address to check.
     * @return bool True if the address is compliant, false if not.
     */
    function isERC20(address tokenAdd)
        internal
        view
        returns (bool)
    {
        // Since OZ ERC does not implement EIP165, we make a contract check.
        return tokenAdd.isContract();
    }
}