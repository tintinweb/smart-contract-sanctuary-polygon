...
contract NFT is ERC721, AccessControl {
    event CrossChain(address indexed from, string indexed to, uint256 indexed tokenId);
    ...
    mapping(address => uint256) public ogClaimed; // max -> 2
    ...
    uint256 MAX_INDEX = 6000;
    uint256 price_public = 0.08 ether;
    uint256 price_wl = 0.07 ether;
    uint256 price_og= 0.06 ether;
    ...
    function setOgMintEnabled() public onlyOwner {
        ogMintEnabled = _reverse(ogMintEnabled);
    }
    ...
    function _ensureMint(uint256 amount, uint256 price) internal view virtual {
        require(amount > 0, "Invalid mint amount!");
        require(msg.value >= price * amount, "Insufficient funds!");
        require(_totalMinted() + amount <= MAX_SUPPLY, "Max supply exceeded!");
    }

    function _verifyMint(bytes32[] calldata _proof, bytes32 root) internal {
        bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
        require(MerkleProof.verify(_proof, root, leaf), "Not white list account!");
    ...
    function publicMint(uint256 amount) public payable {
        ...
        _ensureMint(amount, price_public);
    ...
    function wlMint(bytes32[] calldata _proof) public payable {
        require(whitelistMintEnabled, "Mint Closed!");
        require(!whitelistClaimed[_msgSender()], "Already Claimed!");

        _ensureMint(1, price_wl);
        _verifyMint(_proof, wl_root);
        whitelistClaimed[_msgSender()] = true;
    }

    function ogMint(bytes32[] calldata _proof) public payable {
        require(ogMintEnabled, "Mint Closed!");
        require(ogClaimed[_msgSender()] < 2, "Already Claimed!");

        _ensureMint(1, price_og);
        _verifyMint(_proof, og_root);
        ogClaimed[_msgSender()] += 1;
    }

    function crossChain(uint256 tokenId, string memory target) public onlyApprovedOrOwner(tokenId) {
        require(crossChainEnabled, "Cross-chain Closed!");

        _burn(tokenId);
        emit CrossChain(_msgSender(), target, tokenId);
    }

    function withdraw(uint256[] calldata tokenIds, address[] calldata accounts) public onlyMinter {
        require(crossChainEnabled, "Cross-chain closed!");
