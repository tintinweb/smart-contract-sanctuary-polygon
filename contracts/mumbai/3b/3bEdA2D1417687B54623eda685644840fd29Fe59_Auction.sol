...
contract Auction is Initializable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    /// @param _platformFee enter in multiples of 10^3 (1% == 0.01 => 0.01*10^3 = 10)
    ...
    function delegatedConstructor(
        ...
        require(_platformFee > 1, "Auction: platformFee should be greater than 1");
        require(_minBidIncrement > 1, "Auction: _minBidIncrement should be greater than 1");
        addressRegistry = IAddressRegistry(_addressRegistry);
        platformFeeRecipient = ITreasuryManagement(addressRegistry.treasuryManagement()).veritoneTreasuryAddress();
    ...
    function resultAuction(
        ...
        if (winningBid > auction.reservePrice) {
            ...
            // we don't get a platform fee if winning bid is =< than reserve price
        ...
        // If not minted for the first time
        if (minter != address(0) && royalty != 0) {
            // for taking decimal percentages
            ...
            // If royalty is zero, or the minter is zero (hence do not pay royalty)
        } else {
            // Get royalty and the address of the creator (the fee recipient)
            (royalty, , minter) = marketplace.collectionRoyalties(_nftAddress);
            if (minter != address(0) && royalty != 0) {
                uint256 royaltyFee = payAmount.mul(royalty).div(10000);
                if (auction.payToken == address(0)) {
                    // in the network native token
                    (bool royaltyTransferSuccess, ) = payable(minter).call{value: royaltyFee}("");
        ...
        // Pay the seller of the NFT the remaining payAmount
        if (payAmount > 0) {
