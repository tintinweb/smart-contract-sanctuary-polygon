...
contract EthermonStakingData is EthermonStakingBasic {
    ...
    function depositeEmons(address _owner, uint256 _amount)
        public
        onlyModerators
    {
        require(_amount > 0);
    ...
    {
        require(_amount > 0);
        emon.safeTransferFrom(address(this), _owner, _amount);
    }
...
contract EthermonStaking is EthermonStakingBasic {
    ...
    function depositeTokens(uint16 _day, uint256 _amount) external {
        ...
        stakingData.depositeEmons(msgSender(), _amount);
    ...
    function withdrawTokens(uint256 _tokenId) external {
        ...
        for (uint256 i = 0; i < data.length; i++) {
            if (data[i].tokenId == _tokenId) {
                ...
                stakingData.withdrawEmons(msgSender(), data[i].amount);
                stakingData.removeTokenData(msgSender(), i);
                // emon.safeTransferFrom(
                //     payable(stakingDataContract),
                //     msgSender(),
                //     data[i].amount
                // );
