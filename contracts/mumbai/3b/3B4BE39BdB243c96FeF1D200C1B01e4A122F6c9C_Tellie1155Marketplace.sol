...
// This architecture requires a two-step deploy per NFT collection per user
// 1. Deploy an ERC1155Creator contract (renamed to token name) for the user's NFT collection (via proxy?)
// 2. Deploy the Tellie1155Marketplace contract for that collection's contract

/// @title Tellied NFT Minting and Listing contract
///
/// This is a non-escrow minting and seller contract
/// that allows users to list NFTs for sale for a fixed price
/// via a configurable list of currencies.
///
/// @dev listings have no expiration date
///
/// Lots of inspiration from manifold.xyz and showtime.io
contract Tellie1155Marketplace is Pausable, Ownable, BaseRelayRecipient {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    using Address for address;

    TieredTakeRate internal _takeRateContract;
    ERC1155Creator internal _erc1155Contract;
    // TODO: How do we use a Gnosis wallet here?
    // TODO: Will need to create another contract to hold our wallet address so we can easily update it
    address payable private constant TELLIE_WALLET_ID = payable(0x89394dF2bC36B2225947f18E0A3EC03D6700A6Cc);
    ...
    mapping(address => bool) internal _acceptedCurrencies;
    ...
    event TakeRatePaid(address indexed receiver, IERC20 currency, uint256 amount);
    error AvailableQuantityInsuficient(uint256 availableQuantity);
    error SellerDoesNotOwnToken(uint256 tokenId, uint256 quantity);
    error TotalFeesTooHigh(uint256 totalFees, uint256 totalPrice);
    ...
    modifier onlySeller(uint256 _id) {
        if (_listings[_id].seller != _msgSender()) revert NotListingSeller(_id);
        _;
    }

    modifier usesCurrency(address currency) {
        if (!_acceptedCurrencies[currency]) revert CurrencyNotAccepted(currency);
        _;
    }
    ...
    constructor(
        ...
        address ercAddress,
        address[] memory initialCurrencies,
        address _trustedForwarder
    ) {
        if (!takeRateAddress.isContract()) revert InvalidContract(takeRateAddress);
        _takeRateContract = TieredTakeRate(takeRateAddress);

        if (!ercAddress.isContract()) revert InvalidContract(ercAddress);
        _erc1155Contract = ERC1155Creator(ercAddress);

        for (uint256 i = 0; i < initialCurrencies.length; i++) {
            address currency = initialCurrencies[i];
            if (!initialCurrencies[i].isContract()) revert NotContractAddress(currency);
            _acceptedCurrencies[currency] = true;
        }
    ...
    function listForSale(
        ...
        uint256 quantity,
        address currency
    ) public whenNotPaused usesCurrency(currency) hasQuantity(quantity) returns (uint256 listingId) {
    ...
    function cancelSale(uint256 listingId) external listingExists(listingId) onlySeller(listingId) {
    ...
    function availableForSale(uint256 _listingId) public view listingExists(_listingId) returns (uint256) {
        ...
        return Math.min(_erc1155Contract.balanceOf(listing.seller, listing.tokenId), listing.quantity);
    }
    ...
    function buy(
        ...
        address currency,
        address buyer
    ) public whenNotPaused usesCurrency(currency) hasQuantity(quantity) listingExists(listingId) {
        if (buyer == address(0)) revert NullAddress();
        ...
        if (listing.tokenId != tokenId) revert TokenIdMismatch(listing.tokenId);
        if (buyer == listing.seller) revert CanNotSellToSelf();

        SalesInfo memory salesInfo;

        salesInfo.availableQuantity = availableForSale(listingId);
        if (quantity > salesInfo.availableQuantity) revert AvailableQuantityInsuficient(salesInfo.availableQuantity);
        if (address(listing.currency) != currency) revert CurrencyMismatch(address(listing.currency));

        salesInfo.totalPrice = SafeMath.mul(listing.price, quantity);
        salesInfo.takeRateAmount = TieredTakeRate(_takeRateContract).getTakeAmount(tier, salesInfo.totalPrice);
        (address payable[] memory addresses, uint256[] memory royaltyBPSs) = _erc1155Contract.getRoyalties(tokenId);
        ...
        for (uint256 x = 0; x < royaltyBPSs.length; x++) {
            ...
            salesInfo.totalFees += salesInfo.royaltyAmounts[x];
        }

        // TODO: Need to make sure the take rate and royalies never exceed 100%
        if (salesInfo.totalFees > salesInfo.totalPrice)
            revert TotalFeesTooHigh(salesInfo.totalFees, salesInfo.totalPrice);

        updateListing(listingId, SafeMath.sub(salesInfo.availableQuantity, quantity));
        emit SaleCompleted(listingId, listing.seller, buyer, quantity, tier);

        // Send take rate fee to Tellie
        if (salesInfo.takeRateAmount > 0) {
            emit TakeRatePaid(TELLIE_WALLET_ID, listing.currency, salesInfo.takeRateAmount);
            listing.currency.safeTransferFrom(buyer, TELLIE_WALLET_ID, salesInfo.takeRateAmount);
        }

        // Send royalties to addresses
        for (uint256 y = 0; y < salesInfo.royaltyAmounts.length; y++) {
            if (salesInfo.royaltyAmounts[y] > 0) {
                emit RoyaltyPaid(addresses[y], listing.currency, salesInfo.royaltyAmounts[y]);
                listing.currency.safeTransferFrom(buyer, addresses[y], salesInfo.royaltyAmounts[y]);
            }
        }

        // The total fees are deducted from the price paid by the buyer and sent to the seller
        listing.currency.safeTransferFrom(
            buyer,
            listing.seller,
            SafeMath.sub(salesInfo.totalPrice, salesInfo.totalFees)
        );

        // Transfer the NFTs from the seller to the buyer
        _erc1155Contract.safeTransferFrom(listing.seller, buyer, listing.tokenId, quantity, "");
    }
    ...
    function setAcceptedCurrency(address currency, bool accepted) external onlyOwner {
        ...
        _acceptedCurrencies[currency] = accepted;
...
contract TieredTakeRate is ITieredTakeRate, Ownable {
    using SafeMath for uint256;
    ...
    constructor(uint8[] memory tiers, uint16[] memory percentages) {
