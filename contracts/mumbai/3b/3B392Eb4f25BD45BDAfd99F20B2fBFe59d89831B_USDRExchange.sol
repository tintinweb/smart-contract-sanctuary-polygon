    ...
    function afterBurnTNGBL(uint256 amount) external {
        ...
        if (amount == 0) {
            ...
            if (int256(amount) >= tngbl) {
                ...
                int256 p = (tngblUpdated * 1e18) / tngbl;
                //avgPrice + avgPrice * p = (usdrUpdated * 1e27) / tngblUpdated;
                //avgPrice * (1 + p) = (usdrUpdated * 1e27) / tngblUpdated;
                //avgPrice * (1 + p) * tngblUpdated = usdrUpdated * 1e27;
                int256 usdrUpdated = (avgPrice * tngblUpdated * (1 + p)) / 1e45;
                _updateTNGBLMintingStats(-int256(amount), usdr - usdrUpdated);
            }
        }
    }
