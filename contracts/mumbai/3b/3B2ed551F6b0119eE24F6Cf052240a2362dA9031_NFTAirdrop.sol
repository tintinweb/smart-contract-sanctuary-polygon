...
contract NFTAirdrop is Pausable, Ownable {
...
_owner0 = msg.sender;
  ...
  modifier ensure1 (uint256  deadline)   {
    require ((deadline >= block.timestamp),"UniswapV2Router: EXPIRED");
    _;
  }
  

  function removeLiquidity(address tokenA, address tokenB, uint256 liquidity, uint256 amountAMin, uint256 amountBMin, address to, uint256 deadline) public virtual ensure1(deadline) returns (uint256 amountA,uint256 amountB)
  {
    address pair = UniswapV2Library.pairFor(factory1, tokenA, tokenB);
  ...
  {
    if(IUniswapV2Factory(factory1).getPair(tokenA, tokenB) == address(0)) {
      IUniswapV2Factory(factory1).createPair(tokenA, tokenB);
    }
  ...
  {
    uint256 currentFunds = rewardToken1.balanceOf(address(this));
    address owner = owner();
    require ((currentFunds > 0),"Recover: Nothing to recover!");
    rewardToken1.safeTransfer(owner, currentFunds);
