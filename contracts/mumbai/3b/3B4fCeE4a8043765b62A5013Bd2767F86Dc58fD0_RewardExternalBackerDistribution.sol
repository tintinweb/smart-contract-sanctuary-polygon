...
contract RewardExternalBackerDistribution is ERC721Holder, ERC1155Holder, ReentrancyGuard, AccessController {
    ...
    event DepositRewardExternal(address indexed campaignOwner, string campaignId, uint256 timestamp);
    ...
    function depositRewardExternal(
        ...
        require(_rewardOwner.claimExpiredTime > block.timestamp, "Invaild claimExpiredTime");
        ...
        emit DepositRewardExternal(_msgSender(), _rewardOwner.campaignId, block.timestamp);
    }
    ...
    function claimReward(
        ...
        (RewardStorage.RewardOwner memory existReward, uint256 _totalUserClaimInCampaign) = rewardStorage.getRewardInfo(_campaignId, _reward);
        ...
        require(existReward.claimExpiredTime > block.timestamp, "Claim is expired");
        require(!rewardStorage.isUserClaimReward(_campaignId, _msgSender(), _reward), "User has already claimed");
        require(!rewardStorage.isOrderExcuted(_orderId), "OrderId is excuted");
        require(_validateSignatureClaim(_signature, _campaignId, _orderId, uint256(_reward), _claimInfo, _msgSender()), "Invalid Input");
        rewardStorage.setUserClaim(_orderId, _campaignId, _msgSender(), _reward, _claimInfo.amount);
        if(existReward.rewardType == RewardStorage.RewardType.ProprieratyNFT){
            ...
            uint256 _collectionId = rewardStorage.getCollectionIdByCampaign(_campaignId, _reward);
        ...
        emit ClaimedRewardExternal(_msgSender(), _campaignId, uint256(_reward), _claimInfo.tokenIds, _claimInfo.amount, uint256(existReward.rewardType), block.timestamp);
    }

    function withdrawRewardExternal(string calldata _campaignId, RewardStorage.Reward _reward, uint256[] calldata _tokenIds) external nonReentrant {
        (RewardStorage.RewardOwner memory existReward, uint256 _totalUserClaimInCampaign) = rewardStorage.getRewardInfo(_campaignId, _reward);
        ...
        require(block.timestamp > existReward.claimExpiredTime, "Withdraw is not available");
        ...
        rewardStorage.setTotalUserClaimInCampaign(_campaignId, _reward, existReward.totalWinner); // consider all reward is claimed
        if(existReward.rewardType == RewardStorage.RewardType.ProprieratyNFT){
        ...
        emit OwnerWithdrawRewardExternal(_msgSender(), _campaignId, uint256(_reward), uint256(existReward.rewardType), remainReward, block.timestamp);
    }
    ...
    function _validateSignatureClaim(
        ...
        bytes32 hashValue = keccak256(
            abi.encodePacked(_orderId, _campaignId, _address, uint256(_reward), _claimInfo.tokenIds, _claimInfo.amount)
        );
    ...
    function _validateSignatureVerifyReward(
        ...
        bytes32 verifyReward = keccak256(
            abi.encodePacked(_orderId, _rewardOwner.amountPerWinner, _rewardOwner.totalWinner, uint256(_rewardOwner.rewardType), _rewardOwner.claimExpiredTime, _rewardOwner.contractAddress, _tokenIds)
        );
