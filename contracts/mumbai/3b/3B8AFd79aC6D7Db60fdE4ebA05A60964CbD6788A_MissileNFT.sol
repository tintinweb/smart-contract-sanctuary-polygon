...
interface IMissileNFT is IERC721EnumerableUpgradeable {
    event NFTMinted (uint256 startId, uint256 amount, address to, uint256 requestId);

    function mintToken(address to, uint256 amount, uint256 requestId) external;
    ...
    IMissileNFT
{
    ...
    function mintToken(address to, uint256 amount, uint256 requestId) external override onlyGenesisMinter nonReentrant whenNotPaused {
        uint256 startId = currentId;
        require(startId + amount <= maxAmount,"exceed max");
        currentId += amount;
        for (uint256 i = 1; i <= amount; i++) {
            _safeMint(to, startId + i);
        }
        emit NFTMinted(startId, amount, to, requestId);
    }
