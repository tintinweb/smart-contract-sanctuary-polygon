...
contract ChainlinkAggregatorFaster is AggregatorV3Interface, AccessControlledAndUpgradeable {
  ...
  string public constant override description =
    "A wrapper around a chainlink oracel to help simulate faster oracle updates";
  ...
  function changePriceUpOrDownByLessThan1Percent(int256 currentPrice, uint80 roundId)
    public
    pure
    returns (int256)
  {
    return
      (currentPrice * (1e18 + int256((uint256(keccak256(abi.encode(roundId))) % 2e16)) - 1e16)) /
      1e18;
  }

  function determinePredictedUpdatedAtTime(uint256 chainlinkOracleSubIndex, uint256 updatedAtSource)
    internal
    view
    returns (uint256 updatedAt)
  {
    updatedAt =
      updatedAtSource +
      Math.min(
        (chainlinkOracleSubIndex * fastOracleUpdateLength),
        (speedupFactor - 1) * fastOracleUpdateLength
      );
  }
    ...
    )
  {
    if (chainlinkOracleSubIndex > speedupFactor || chainlinkOracleSubIndex == 0) {
      return (
        roundIdSource * uint80(speedupFactor),
        answerSource,
        updatedAtSource,
        updatedAtSource,
    ...
    )
  {
    ...
    (
      ...
      uint256 updatedAtSource,
      uint80 _answeredInRoundSource
    ) = baseChainlinkAggregator.getRoundData(chainLinkOriginalId);

    if (chainLinkOriginalId < roundIdImplementation) {
      (, , , uint256 updatedAtNext, ) = baseChainlinkAggregator.getRoundData(
        chainLinkOriginalId + 1
      );
      updatedAt = determinePredictedUpdatedAtTime(
        uint256(_roundId) % (speedupFactor),
        updatedAtSource
      );
      ...
      if (updatedAt >= updatedAtNext) {
        (
          ...
          ,
          uint80 _answeredInRoundNext
        ) = baseChainlinkAggregator.getRoundData(chainLinkOriginalId + 1);
    ...
    )
  {
    (
      ...
      uint256 updatedAtSource,
      uint80 _answeredInRoundSource
    ) = baseChainlinkAggregator.latestRoundData();
