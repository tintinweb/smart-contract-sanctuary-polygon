...
contract SubscriptionManager is ERC721Enumerable, Ownable {
  ...
  // Mapping from author to subscription levels
   ...
   * user => author => [LinkedList]
   */
  mapping(address => mapping(address => LinkedList.List[]))
    private userSubscriptionsByAuthor;

  // Mapping from author to referrer bonus in %
  mapping(address => uint16) public referrerShareByAuthor;

  struct UserRefData {
    address invitedBy;
    uint16 referralShare; // referral share of referrer fee
  }
   ...
   * in the other - a referrer who determines which part of his reward to share with the referral as a discount
   * user => author => UserRefData
   */
  mapping(address => mapping(address => UserRefData))
    public userRefDataByAuthor;

  uint8 private maxSubscriptionLevels = 3;

  uint16 public serviceFee = 500; // 5% fee
  uint256 private constant DENOMINATOR = 10000;
  uint256 public minPrice = 1 ether;
  uint16 private annualSubscriptionDiscount = 3000; // 30% discount

  IERC20 public token;

  constructor() ERC721("Subscriptions Club", "SUBCLUB") {}
  ...
  function subscribe(
    ...
    _subscribe(msg.sender, author, _weeks, level, autoRenewal, referrer);
  }
  ...
  function _subscribe(
    ...
    uint256 price = subscriptionLevel.price;
    bool limited = subscriptionLevel.limited;
    uint256 totalSupply = subscriptionLevel.totalSupply;

    require(price > 0, "Inactive level");
    if (limited) require(totalSupply > 0, "Sold out");

    uint256 cost = price * _weeks;

    distibuteFees(subscriber, author, cost, referrer);

    uint256 expiration = block.timestamp + _weeks * 1 weeks;

    mint(subscriber, author, expiration, level, autoRenewal, _weeks);

    if (limited) subscriptionLevel.totalSupply--;
  }
  ...
  function distibuteFees(
    ...
    uint256 _serviceFee = (cost * serviceFee) / DENOMINATOR;
    uint256 referrerFee = (cost * referrerShareByAuthor[author]) / DENOMINATOR;

    address referrer = userRefDataByAuthor[subscriber][author].invitedBy;

    if (referrer == address(0) && _referrer != address(0)) {
      referrer = _referrer;
      userRefDataByAuthor[subscriber][author].invitedBy = _referrer;
    }

    uint256 subscriberDiscount = 0;
    if (referrer != address(0) && referrerFee > 0) {
      uint16 referralShareOfReferrerFee = userRefDataByAuthor[referrer][author]
        .referralShare;

      subscriberDiscount =
        (referrerFee * referralShareOfReferrerFee) /
        DENOMINATOR;
    }

    token.transferFrom(subscriber, address(this), cost - subscriberDiscount);

    if (referrer != address(0) && referrerFee > 0) {
      token.transfer(referrer, referrerFee - subscriberDiscount);
    }

    token.transfer(author, (cost - _serviceFee - referrerFee));
    token.transfer(owner(), _serviceFee);
  }
  ...
  function renewSubscription(
    ...
    bool autoRenewal = subscription.autoRenewal;
    uint16 period = subscription.period;
    uint256 expiration = subscription.expiration;

    require(autoRenewal, "Not auto-renewable");

    SubscriptionLevel storage subscriptionLevel = subscriptionLevels[author][
      level
    ];
    uint256 cost = period * subscriptionLevel.price;

    uint256 allowance = token.allowance(subscriber, address(this));

    if (allowance < cost && expiration < (block.timestamp + 1 days)) {
      subscription.autoRenewal = false;
      if (subscriptionLevel.limited) subscriptionLevel.totalSupply++;

      return;
    }

    _subscribe(subscriber, author, period, level, autoRenewal, address(0));
  }
  ...
  function setServiceFee(uint16 fee) external onlyOwner {
  ...
  function setSubscriptionLevelPrice(
    uint256 price,
    uint8 level,
    uint256 totalSupply,
    bool limited
  ) external {
    require(price > minPrice, "Too small price");
    require(level < maxSubscriptionLevels, "Too large level");
    ...
    if (_authorSubscriptionLevels.length < maxSubscriptionLevels) {
      require(_authorSubscriptionLevels.length >= level, "Invalid level");
    }
  ...
  function setReferrerShare(uint16 fee) external {
    require(fee < (DENOMINATOR - serviceFee));

    referrerShareByAuthor[msg.sender] = fee;
  }
