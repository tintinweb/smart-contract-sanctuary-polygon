...
contract ManagementContract is
    ERC20,
    Ownable,
    ERC20Permit,
    ERC20Burnable,
    Pausable,
    EIP712MetaTransaction("ManagementContract", "1")
{
    ...
    ) ERC20(TOKEN_NAME, TOKEN_SYMBOL) ERC20Permit(TOKEN_NAME) {
    ...
    function buyNft(
        ...
        bool transferSuccess = transfer(_seller, totalPrice);
        ...
        bool mintSuccess;
        if (transferSuccess) {
            (mintSuccess, ) = _nftContractAddress.call(
    ...
    function sellNft(
        ...
        // address owner = __msgSender();
        uint256 totalPrice = _tokenPrice * _amount;
        _transfer(_buyer, _seller, totalPrice);

        // (bool success, ) = payable(managementContract).call{value: 0.02 ether}(
        //     abi.encodeWithSignature("publicMint()")
        // );

        (bool success, ) = _nftContractAddress.call(
            abi.encodeWithSignature(
