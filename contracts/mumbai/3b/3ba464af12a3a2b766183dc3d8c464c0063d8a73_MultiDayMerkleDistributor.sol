...
interface IMultiDayMerkleDistributor {
    ...
    function claimMultiDay(
        uint256[] calldata day,
...
contract MultiDayMerkleDistributor is
    IMultiDayMerkleDistributor,
    ReentrancyGuard,
    Pausable,
    Ownable
{
    ...
    constructor(address token_, address devFund_) {
        token = token_;
        devFund = devFund_;
        devRate = 2500; // 25%
    ...
    function merkleRoot(uint256 day) external view override returns (bytes32) {
    ...
    function setDevFundConfig(address _devFund, uint256 _devRate)
        external
        onlyOwner
    {
        require(_devFund != address(0) || _devRate == 0, "zero address");
        require(_devRate <= 10000, "too high");
        devFund = _devFund;
        devRate = _devRate;
    }

    function isClaimed(uint256 day, uint256 index)
        public
        view
        override
        returns (bool)
    ...
    function _setClaimed(uint256 day, uint256 index) private {
        uint256 claimedWordIndex = index / 256;
        uint256 claimedBitIndex = index % 256;
        claimedBitMap[day][claimedWordIndex] =
            claimedBitMap[day][claimedWordIndex] |
            (1 << claimedBitIndex);
    }
    ...
    function setMerkleRoot(
        ...
        uint256 amount,
        bytes32 _merkleRoot
    ) external override isAuthorised {
        merkleRoot_[day] = _merkleRoot;
        ICappedMintableBurnableERC20 _token = ICappedMintableBurnableERC20(
            token
        );
        _token.mint(address(this), amount);
        _token.mint(devFund, (amount * devRate) / 10000);
    }
    ...
    function _claim(
        ...
        require(
            !blacklisted[account],
            "MerkleDistributor: Account is blocked."
        );

        // Verify the merkle proof.
        bytes32 node = keccak256(abi.encodePacked(index, account, amount));
        require(
            MerkleProof.verify(merkleProof, merkleRoot_[day], node),
