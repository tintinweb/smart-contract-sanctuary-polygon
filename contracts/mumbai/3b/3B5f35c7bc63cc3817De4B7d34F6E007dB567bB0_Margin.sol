...
contract Margin is ReentrancyGuard, MarginStorage, Ownable {
    ...
    function addDelegatorAddress(address assetAddress, address delegatorAddress) external onlyOwner {
    ...
    function deposit(address assetAddress, uint256 marginAmount, uint256 leverage) external payable {
        ...
        uint success = ICErc20Delegator(delegator).borrow(loanAmount);
    ...
    function borrow(address assetAddress, uint256 borrowAmount) nonReentrant external {
        ...
        uint success = ICErc20Delegator(delegator).borrow(borrowAmount);
    ...
    function maxBorrow(address user, address assetAddress) public view returns(uint256) {
        ...
        uint256 totalMargin = depositInfo.marginAmount - depositInfo.lossValue +
            depositInfo.gainValue;
    ...
    function repay(address assetAddress, uint256 repayAmount, uint256 source) external payable {
        ...
        if(source == 1) {
            if(assetAddress == address(0))  {
                ...
                uint success = ICEther(delegator).repayBorrow{value: msg.value}();
                ...
                uint success = ICErc20Delegator(delegator).repayBorrow(repayAmount);
                require(success == 0, "Margin: Repay failed");
            }
        }
        //using the margin account
        else {
            require(depositInfo.marginAmount >= repayAmount, "Margin: Insufficient Margin");
            if(assetAddress == address(0)) {
                //need to check contract should have repayAmount as balance
                uint success = ICEther(delegator).repayBorrow{value: repayAmount}();
                require(success == 0, "Margin: Repay failed");
            }
            else {
                IERC20(assetAddress).approve(delegator, repayAmount);
