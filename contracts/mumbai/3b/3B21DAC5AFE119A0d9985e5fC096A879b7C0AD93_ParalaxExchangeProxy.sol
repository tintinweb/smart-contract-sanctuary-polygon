...
contract ParalaxExchange is Storage, ReentrancyGuard {
    ...
    /**
     * @dev Verify the signature for limit orders
     */
    // function verifyLimit(
    //     bytes calldata signature,
    //     SignerData memory signerData
    // ) public view returns (bool) {
    //     return _verifySignerData(signature, signerData);
    // }

    receive() external payable {}

    /**
     * @dev Verify the signature for DCA order
     */
    // function verifyDCA(
    //     Order memory order,
    //     bytes calldata signature
    // ) external view returns (bool) {
    //     return _verify(order, signature);
    // }
    ...
    function limitOrderP2P(
        ...
        if (signerData1.baseCurrency != _wETH) {
            ...
            _convertWETHTransfer(newAmountFromSignerOne, signerData2.account);
        }
        ...
        if (signerData2.baseCurrency != _wETH) {
            ...
            _convertWETHTransfer(newAmountFromSignerTwo, signerData2.account);
        }
    ...
    function limitOrderDEX(
        ...
        if (signerData.quoteCurrency != _wETH) {
            ...
            _convertWETHTransfer(transferAmount, signerData.account);
        }
    ...
    function orderDCATM(bytes calldata sign, Order memory order) public {
        ...
        if (_isEmptyDCA(orderDca)) {
            ...
            if (_emptyTM(order.tm)) {
                ...
                _levelTM(sign, order.dca, order.tm);
            }
    ...
    function _dcaTM(bytes calldata sign) internal {
        ...
        if (procDCA.doneTM == tm.amount && tm.amount != 0) {
            ...
            // require(
            //     procDCA.doneTM + procDCA.done < order.volume,
            //     "Order Error"
            // );
            require(_validatePriceDCA(order), "Price Error");

            _levelDCA(sign, order);
        } else {
            // OrderDCA memory order = _ordersDCA[sign];
            require(order.volume != 0, "Init Error");
    ...
    function _getQuotePrice(
        ...
        path[1] = order.quoteCurrency;
        if (order.price == 0) return (0, 0, path);
        if (amountIn == 0) {
        ...
        return (amountOutMin, amounts[1], path);
    }

    function _levelDCA(bytes memory sign, OrderDCA memory order) internal {
        ProcessingDCA memory procDCA = _processingDCA[sign];
        require(
        ...
        _proceedLevel(order, scaleAmount);
    ...
    function _levelTM(
        ...
        _proceedLevel(order, amountIn);

        emit TimeMultiplierDCA(sign, tm, amountIn, order.account);
    }

    function _proceedLevel(OrderDCA memory order, uint256 amountIn) internal {
        (uint256 newAmountIn, uint256 tax) = subtractPercentage(
            amountIn,
            feeDCA
        );
        ...
        IERC20(order.baseCurrency).transferFrom(
            order.account,
            address(this),
            newAmountIn
        );
        (uint256 amountOutMin, , address[] memory path) = _getQuotePrice(
            order,
            amountIn
        );
        ...
        if (order.quoteCurrency != _wETH) {
            ...
            _convertWETHTransfer(amounts[amounts.length - 1], order.account);
        }
    }
    ...
    function _approve(address token) internal {
        if (IERC20(token).allowance(address(this), _adapter) == 0) {
            IERC20(token).safeApprove(_adapter, type(uint256).max);
        }
    }
    ...
    function _transferTaxETH(uint256 tax) internal {
        ...
        require(success == true, "ETH transfer error");
    }
    ...
    function _transferTax(
        ...
        IERC20(asset).transferFrom(from, _treasure, amount);
    }

    function _convertWETHTransfer(uint amount, address account) internal {
    ...
    function _verifyOrder(
        ...
        bytes32 hashOrder = _hashOrderDCA(order.dca, order.tm);
        status = EIP712._verify(order.dca.account, sign, hashOrder);
    }
...
contract ParalaxExchangeProxy is Storage, IParalaxExchange {
    ...
    string constant ORDER_DCA_TM =
        "orderDCATM(bytes,((uint256,uint128,uint64,uint64,uint96,address,uint96,address,address,uint256),(uint256,uint256)))";
    ...
    function limitOrderP2P(OrderLMP2P[] calldata orders) external {
        for (uint256 i = 0; i < orders.length; i++) {
            ...
            if (!success) {
                // console.log("---ERORRRRRRRR---limitOrderP2P");
    ...
    function limitOrderDEX(LimitOrderDEX[] calldata limitOrdersDEX) external {
        for (uint256 i = 0; i < limitOrdersDEX.length; i++) {
            ...
            if (!success) {
                // console.log("---ERORRRRRRRR---limitOrderDEX");
    ...
    function orderDCATM(InitOrderDCA[] calldata dataInitOrderDCA) public {
        for (uint256 i = 0; i < dataInitOrderDCA.length; i++) {
            ...
            if (!success) {
                // console.log("EROROR--------------", string(data));
                emit InitOrderDCAError(dataInitOrderDCA[i].sign, data);
            }
        }
    }

    // function initTimeMultiplier(
    //     InitOrderDCA[] memory dataInitOrderDCA
    // ) external {
    //     for (uint256 i = 0; i < dataInitOrderDCA.length; i++) {
    //         (bool success, bytes memory data) = _implementation.delegatecall(
    //             abi.encodeWithSignature(
    //                 // INIT_TIME_MULTIPLIER,
    //                 ORDER_DCA_TM,
    //                 dataInitOrderDCA[i].sign,
    //                 dataInitOrderDCA[i].order
    //             )
    ...
    // function dcaTM(bytes[] calldata sign) external {
    //     for (uint256 i = 0; i < sign.length; i++) {
    //         (bool success, bytes memory data) = _implementation.delegatecall(
    //             abi.encodeWithSignature(DCA_TM, sign)
    //         );
    //         if (!success) {
    //             emit DCAError(sign[i], data);
    //         }
    //     }
    // }
