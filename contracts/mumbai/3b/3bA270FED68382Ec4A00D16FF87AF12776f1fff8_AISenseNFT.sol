...
contract AISenseNFT is ERC721Enumerable, Ownable, ContractMetadata {
    ...
    event CreateAISenseNFT(uint256 indexed id, uint256 duration, uint256 price);

    // function getUsdtBalance(address _address) public view returns(uint256) {
    //     return IERC20(usdt).balanceOf(_address).mul(10 ** (decimals - usdtDecimals));
    // }
    // function getUsdtAllowance(address _address) public view returns(uint256) {
    //     return IERC20(usdt).allowance(_address, address(this)).mul(10 ** (decimals - usdtDecimals));
    // }
    function claimNFT(
        address recipient,
        uint8 nftType
    ) external returns (uint256) {
        require(nftType < nftTypes.length, "Invalid NFT type");
        require(isPaused == false, "Contract is paused");
        return _mintAnElement(recipient, nftType, nftTypes[nftType].price);
    }

    function mintWithUsdt(uint8 nftType) external returns (uint256) {
        require(nftType < nftTypes.length, "Invalid NFT type");
        require(!isPaused, "Contract is paused");
        require(_reservedCount < 100, "All reserved NFTs are sold out");
        // require(IERC20(usdt).balanceOf(_msgSender()) >=  nftPrice[nftType].mul(10 ** usdtDecimals), "Insufficient USDT balance");
        // require(IERC20(usdt).approve(address(this), nftPrice[nftType].mul(10 ** usdtDecimals)) == true, "USDT approval failed");
        // require(getUsdtAllowance(_msgSender()) >=  nftPrice[nftType], "USDT allowance is not enough");

        require(
            IERC20(usdt).transferFrom(
                _msgSender(),
                address(this),
                nftPrice[nftType].mul(10 ** usdtDecimals)
            ),
    ...
    function _mintAnElement(
        ...
        tokenId.increment();
        uint256 endTimestamp = block.timestamp + nftTypes[_duration].duration;
        nftValidUntil[id] = endTimestamp;
        _safeMint(_to, id);
        NFTTYPE[id] = _duration;
        emit CreateAISenseNFT(id, _duration, _price);
        return id;
    }

    function setNFTPrice(uint256 newPrice, uint256 _duration) public onlyOwner {
        require(newPrice >= 0, "Invalid price");
        nftPrice[_duration] = newPrice;
    }

    function getNFTPrice(uint256 _duration) public view returns (uint256) {
        return nftPrice[_duration].mul(10 ** decimals);
    }
