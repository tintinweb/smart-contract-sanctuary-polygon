...
interface IPirateAvatarMetadata {
    ...
    function addPirateAvatar(uint256 pirateAvatarTokenId, bool isGovernor, string memory uri) external;

    function updatePirateAvatar(uint256 tokenId, PirateAvatar calldata pirateAvatar, TokenTrait[] calldata traits) external;

    function addToNumberTraitType(uint256 tokenId, uint256 amount, bytes32 traitTypeName) external;

    function getTraitTypeValue(uint256 tokenId, bytes32 traitTypeName) external view returns(bytes32);

    function setTraitTypeValue(uint256 tokenId, bytes32 traitValue, bytes32 traitTypeName) external;
...
import "./PirateAvatar.sol";
import "../traits/TokenTrait.sol";
import "../traits/TraitDisplayType.sol";
    ...
    IPirateAvatarMetadata 
{
    ...
    modifier isAvatarManager(address account) {
        require(hasRole(AVATAR_MANAGER_ROLE, account), "Not in role");
        _;
    }
    ...
    function addPirateAvatar(uint256 tokenId, bool isGovernor, string memory uri) 
        override 
        external 
        isAvatarManager(msg.sender)
    {
        require(!pirateAvatarExists(tokenId), "Avatar exists");

        tokenIdToPirateAvatar[tokenId] = PirateAvatar(
            string(abi.encodePacked("Pirate Avatar #", tokenId.toString())), 
            uri, 
            isGovernor);
        ...
        tokenIdToTokenTraits[tokenId].push( 
            TokenTrait(
                PIRATE_TYPE_NAME,
                isGovernor ? bytes32("Governor") : bytes32("Buccaneer"),
                0
            )
        );
    ...
    function updatePirateAvatar(uint256 tokenId, PirateAvatar memory pirateAvatar, TokenTrait[] memory traits) 
        override 
        external 
        isAvatarManager(msg.sender)
    ...
    function getTraitTypeValue(uint256 tokenId, bytes32 traitTypeName) 
        override 
        external 
        view
        returns(bytes32)
        
    {
        for (uint8 i = 0; i < tokenIdToTokenTraits[tokenId].length; i++) {
            if (tokenIdToTokenTraits[tokenId][i].traitType == traitTypeName)
            {
    ...
    function setTraitTypeValue(uint256 tokenId, bytes32 traitValue, bytes32 traitTypeName) 
        override 
        external
        isAvatarManager(msg.sender) 
    {
    ...
    function addToNumberTraitType(uint256 tokenId, uint256 amount, bytes32 traitTypeName) 
        override 
        external 
        isAvatarManager(msg.sender)
        avatarExists(tokenId)
    {
        for (uint8 i = 0; i < tokenIdToTokenTraits[tokenId].length; i++) {
            if (tokenIdToTokenTraits[tokenId][i].traitType == traitTypeName)
            {
                uint _newValue = uint(tokenIdToTokenTraits[tokenId][i].traitValue).add(amount);

                tokenIdToTokenTraits[tokenId][i].traitValue = bytes32(_newValue);
                break;
            }
        }
    }

    function removePirateAvatar(uint256 tokenId) 
        public 
        isAvatarManager(msg.sender)
        avatarExists(tokenId) 
    {
    ...
    function getNotorietyLevel(uint256 tokenId, bytes32 notorietyTraitTypeName) external view returns(uint8 level) {
        for (uint8 i = 0; i < tokenIdToTokenTraits[tokenId].length; i++) {
            if (tokenIdToTokenTraits[tokenId][i].traitType == notorietyTraitTypeName)
            {
                uint notorietyPoints = uint(tokenIdToTokenTraits[tokenId][i].traitValue);
            ...
            attributes = string(
                abi.encodePacked(attributes, 
                    string(
                        abi.encodePacked(
                            '{"value":"',
                            bytes32ToString(tokenTraits[i].traitValue),
                            ...
                            '","trait_type":"',
                            bytes32ToString(tokenTraits[i].traitType),
                            '"}',
    ...
    function getDisplayTypeName(uint256 traitDisplayType) private pure returns (string memory)
    {
        if (TraitDisplayType(traitDisplayType) == TraitDisplayType.Number)
        {
            return "number";
        }
        return "string";
