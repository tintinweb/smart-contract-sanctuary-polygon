...
contract BaseSBT is ERC721Enumerable, IERC4906, Ownable, Pausable {
    ...
    mapping(uint256 metaType => string uri) internal _baseTokenURIByMetaType;
    mapping(uint256 tokenType => string uri) internal _baseTokenURIByTokenType;
    ...
    mapping(uint256 tokenID => address to) internal _address;
    ...
    mapping(address to => mapping(uint256 => uint256)) internal _typeIDs;

    mapping(address minter => bool isMinter) internal _minters;

    mapping(address to => mapping(uint256 => bool isAirdropped))
        internal _isAirdroppeds;
    ...
    function tokenURI(
        ...
        return
            bytes(_baseTokenURIByTokenType[tokenType_]).length > 0
                ? _baseTokenURIByTokenType[tokenType_]
                : "";
    }
    ...
    function tokenIDByType(
        address to_,
        uint256 tokenType_
    ) external view returns (uint256) {
        return _typeIDs[to_][tokenType_];
    }
    ...
    function _mint(address to_, uint256 tokenID_, uint256 tokenType_) internal {
        _isAirdroppeds[to_][tokenType_] = true;
        _tokenTypes[tokenID_] = tokenType_;
        _typeIDs[to_][tokenType_] = tokenID_;
        _address[tokenID_] = to_;
    ...
    function _burn(uint256 tokenID_) internal override {
        ...
        _isAirdroppeds[owner_][tokenType_] = false;

        delete _holdingStartedAts[tokenID_];
        delete _address[tokenID_];
...
contract KUVM is BaseSBT {
    string internal constant NAME = "KINDAI UNIV VOLUNTEER MEMBERSHIP";
    string internal constant SYMBOL = "KUVM";

    string internal constant OPERATION_TYPE_MINT = "1";
    string internal constant OPERATION_TYPE_SETTOKENURI = "2";
    string internal constant OPERATION_TYPE_BURN = "3";

    using Counters for Counters.Counter;
    Counters.Counter private _tokenIDCounter;

    constructor() BaseSBT(NAME, SYMBOL) {}
    ...
    function setMetaTypeURI(
        string calldata uri_,
        uint256 metatype_
    ) external onlyOwner {
        _baseTokenURIByMetaType[metatype_] = uri_;
        _refreshMetadata();
    }

    function setTokenTypeBaseURI(
        string calldata uri_,
        uint256 tokentype_
    ) external onlyOwner {
        _baseTokenURIByTokenType[tokentype_] = uri_;
        _refreshMetadata();
    }

    function isAirdropByType(
        address to_,
        uint256 type_
    ) external view returns (bool) {
        return _isAirdroppeds[to_][type_];
    }
    ...
    function airdropByType(
        ...
        string memory _typeStr = Strings.toString(type_);
        string memory _operationTypeStr = substring(_typeStr, 0, 1);
        string memory _tokenTypeStr = substring(_typeStr, 1, 5);
        string memory _metaTypeStr = substring(_typeStr, 5, 9);
        uint256 _tokenType = st2num(_tokenTypeStr);
        uint256 _metaType = st2num(_metaTypeStr);

        if (
            hashCompareWithLengthCheck(OPERATION_TYPE_MINT, _operationTypeStr)
        ) {
            require(!_isAirdroppeds[to_][_tokenType], "already airdropped");
            _airdropByType(to_, _tokenType);

            if (_metaType != 0) {
                uint256 _tokenID = _typeIDs[to_][_tokenType];
                _tokenURIs[_tokenID] = _baseTokenURIByMetaType[_metaType];
                emit MetadataUpdate(_tokenID);
            }
        }
        if (
            hashCompareWithLengthCheck(
                OPERATION_TYPE_SETTOKENURI,
                _operationTypeStr
            )
        ) {
            if (_metaType != 0) {
                ...
                _requireTokenURINotFrozen(_tokenID);
        ...
        if (
            hashCompareWithLengthCheck(OPERATION_TYPE_BURN, _operationTypeStr)
        ) {
            uint256 _tokenID = _typeIDs[to_][_tokenType];
            _burn(_tokenID);
        }
