    ...
    ERC20Votes
{
    ...
    constructor(
        ...
        ColdStaking(20000 ether)
        ERC20Permit("LumiShare LUMI")
    {
        transferFee = 0;
        srgTokenV1 = IERC20(_srgTokenV1);

        //TBD if we need to do the airdrop too or we just ignore it
        _mint(multiSigWallet, 221600000 ether);

        // Presale balance
        _mint(address(this), 83333333 ether);

        //Staking balance
        _mint(address(this), 20000 ether);
...
contract Illumipresale is Ownable, ReentrancyGuard {
        ...
        nonReentrant
    {
        ...
        if (referralActive) {
            _referralReward(referralId, stableReward, srgTokens);
        }
    ...
    function _referralReward(
        ...
        if (referralId != address(0)) {
            if (stableReward) {
                // 12% of 2% Because stable coin = 0.12 SRG
                IERC20(BUSD).transfer(referralId, (srgTokens * 24) / 10000);
            } else {
                IERC20(address(this)).transfer(
                    referralId,
                    (srgTokens * 5) / 100
