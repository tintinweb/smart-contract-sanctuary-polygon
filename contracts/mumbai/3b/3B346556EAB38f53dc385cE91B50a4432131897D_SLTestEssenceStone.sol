    ...
    UUPSUpgradeable
{
    ...
    function exists(uint256 id) public pure override returns (bool) {
        return id <= uint256(StoneType.Broken);
    }
     ...
     *  Mint
     */
    modifier onlyDungeonGateOrSystemContract() {
        _onlyDungeonGateOrSystemContract();
        _;
    }
    ...
    function mint(
        ...
        uint256 _tokenId,
        uint256 _amount
    ) external onlyDungeonGateOrSystemContract {
        if (!exists(_tokenId)) {
            revert DoesNotExistTokenId();
        }
    ...
    function _onlyDungeonGateOrSystemContract() private view {
        address msgSender = _msgSender();
        if (msgSender != dungeonGateContract && msgSender != systemContract) {
            revert OnlyDungeonGateOrSystemContract();
        }
    }
...
interface EssenceStoneError {
    error OnlyDungeonGateOrSystemContract();
