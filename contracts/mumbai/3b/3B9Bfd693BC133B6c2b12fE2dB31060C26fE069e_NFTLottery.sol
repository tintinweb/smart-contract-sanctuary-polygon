...
contract NFTLottery is INFTLottery, AccessControl, ReentrancyGuard, ProtocolFee {
    ...
    constructor() {
        ...
        nextLotteryId = 1;
        addFeeInfo(0, Fee(FeeType.Bps, 500, 0, msg.sender)); 
    }

    // =============================================================
    //                    External calls
    // =============================================================
    function createLottery(Lottery calldata lotteryData) external virtual override {
        ...
        lotteries[lotteryId] = Lottery({
            ...
            endTimestamp: lotteryData.endTimestamp,
    ...
    function openLottery(uint256 lotteryId) external virtual override onlyLotteryOwner(lotteryId) whenLotteryPending(lotteryId) {
    ...
    function closeLottery(uint256 lotteryId) external virtual override onlyLotteryOwnerOrAdmin(lotteryId) whenLotteryOpenOrPending(lotteryId) {
    ...
    function extendEndtime(uint256 lotteryId, uint256 newEndtime) external onlyLotteryOwnerOrAdmin(lotteryId) whenLotteryOpenOrPending(lotteryId) nonExpired(lotteryId) {
        require(lotteries[lotteryId].endTimestamp < newEndtime, "Invalid new endtime");
        
        lotteries[lotteryId].endTimestamp = newEndtime;
    }
    ...
    function buyTicket(uint256 lotteryId, uint256 numberOfTickets) public virtual override whenLotteryOpen(lotteryId) nonExpired(lotteryId) nonReentrant {
        IERC20(lotteries[lotteryId].ticketCurrency).safeTransferFrom(msg.sender, address(this), numberOfTickets * lotteries[lotteryId].ticketCost);

        players[lotteryId].totalTicketsSold += numberOfTickets;
        players[lotteryId].tickets[msg.sender] += numberOfTickets;

        if (!isPlayerOf(lotteryId, msg.sender)) players[lotteryId].playerWallets.push(msg.sender); 
    ...
    function _pickWinner(uint256 lotteryId) internal virtual {
        ...
        // There is no participant. Send the nft back to owner.
        if (players[lotteryId].totalTicketsSold < 1) {
            nftContract.safeTransferFrom(address(this), lottery.owner, lottery.tokenId);
        }
        // If there are participants, pick random winner.
            ...
            uint256 totalSales = players[lotteryId].totalTicketsSold * lotteries[lotteryId].ticketCost;
            uint256 fee = _chargeFee(token, 0, totalSales);
            token.safeTransfer(lottery.salesRecipient, totalSales.sub(fee));
        }
    ...
    function _randomPlayer(uint256 lotteryId) internal virtual returns (address) {
        ...
        uint256 totalTickets = players[lotteryId].totalTicketsSold;

        // Generate a random number between 0 and totalTickets
        uint256 randomNumber = uint256(keccak256(abi.encodePacked(block.prevrandao, block.timestamp, totalTickets))) % totalTickets;

        // Iterate over the players
        for (uint256 i = 0; i < players[lotteryId].playerWallets.length; i++) {
            address player = players[lotteryId].playerWallets[i];
            uint256 tickets = players[lotteryId].tickets[player];

            // If the random number is less than the number of tickets the player has, return the player
            if (randomNumber < tickets) {
                return player;
            }

            // Subtract the number of tickets the player has from the random number
            randomNumber -= tickets;
        }

        return players[lotteryId].playerWallets[players[lotteryId].playerWallets.length - 1];
    }
...
interface INFTLottery is IERC721Receiver {
    ...
    function createLottery(Lottery calldata lotteryData) external;
