...
contract Rover is ERC721EnumerableUpgradeable, OwnableUpgradeable, ERC1155HolderUpgradeable, VRFConsumerBaseV2Upgradable, ITrustedMintable {
    ...
    function purchaseWithMatic(uint32 amount) public payable returns (uint256 requestId) {
        uint256 amountToPay = s_oracle.getAmountOut(address(USDT), address(0), 10 * amount);
    ...
    function purchaseWithIXT(uint32 amount) public returns (uint256 requestId) {
        uint256 wmaticAmount = s_oracle.getAmountOut(address(USDT), address(0), 10 * amount);
        uint256 amountToPay = s_oracle.getAmountOut(address(0), address(IXT), wmaticAmount);
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        if (rover.status == Status.Pristine) {
            status = "Pristine";
        } else if (rover.status == Status.Worn) {
            status = "Worn";
        } else if (rover.status == Status.Damaged) {
            status = "Damaged";
        } else {
            status = "Wrecked";
        }
        ...
        string memory attributes = string.concat(
            '"attributes":[{"trait_type":"NFT","value":"Rover"},{"trait_type":"ID","value":"',
            tokenIdHex,
