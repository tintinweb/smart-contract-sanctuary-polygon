...
contract STRTLocking is Ownable {
    ...
    uint256 public penalty_d2; // charged when withdraw before due date (decimal 2)
    uint256 public fee_d2; // charged when withdraw after due date (decimal 2)
    ...
    // mapping(address => bool) public everLocked;
     ...
     */
    function lock(uint256 _amount, address _member) isLockPaused public {
        ...
        // emit Locked(_member, _amount, lockReward, matureUntil);
    }
    ...
    function _releaseToken(uint256 _amount, address _member, uint256 _charged_d2, uint256 _start) internal {
        ...
        uint256 chargedAmount = (_amount * _charged_d2) / 10000;
        uint256 finalAmount = _amount - chargedAmount;

        if(chargedAmount > 0) TransferHelper.safeTransfer(address(token), DEAD, chargedAmount);
        TransferHelper.safeTransfer(address(token), _member, finalAmount);
    }

    function claim(uint256 _lockIndex, address _member) public {
        uint256 reward = _claim(_lockIndex, _member);
        _releaseToken(reward, _member, fee_d2, 0);

        if(locked[_member] == 0) deleteMember(_member);

        // emit Claim(msg.sender, unlockableTokens);
    }
    ...
    function _claim(uint256 _lockIndex, address _member) internal returns(uint256 reward) {
        ...
        if(summary.rewardEnded > 0){
            ...
            deleteLockIndex(_lockIndex, _member);
        }

        reward = calcReward(summary.amount, startDate, endDate);
    }

    function deleteLockIndex(uint256 _lockIndexToDelete, address _member) internal {
    ...
    function relock(uint256 _lockIndex, address _member) public {
        uint256 reward = _claim(_lockIndex, _member);
        lock(reward, _member);
    ...
    // take locked token
    function withdraw(uint256 _lockIndex, address _member) public {
        uint256 dateTimeNow = block.timestamp;
        Detail memory summary = summaries[_member][_lockIndex];
        ...
        // emit Withdraw(msg.sender, unlockableTokens);
    }
