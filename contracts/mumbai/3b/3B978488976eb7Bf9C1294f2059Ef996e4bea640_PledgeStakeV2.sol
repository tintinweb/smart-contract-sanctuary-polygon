...
import "@openzeppelin/contracts/utils/math/SafeMath.sol";
 contract PledgeStakeV2 {
    ...
    function _updateRewards(address _user) private {
        ...
        rewardInfo[_user].roundRewards = reward.roundRewards;
        reward.levelRewards = getTotalLevelRewards(_user);
        rewardInfo[_user].levelRewards = reward.levelRewards;
        rewardInfo[_user].totalRewards = reward.roundRewards.add(reward.levelRewards);
        rewardInfo[_user].availableRewards = reward.totalRewards.sub(reward.withdrawnRewards);
    }

     // Function to withdraw rewards
    function withdraw() external {
        uint256 directRewards = rewardInfo[msg.sender].directRewards;
        uint256 roundRewards = rewardInfo[msg.sender].roundRewards;
        uint256 levelRewards = rewardInfo[msg.sender].levelRewards;
        uint256 UnstakeAmount = rewardInfo[msg.sender].UnstakeAmount;
        uint256 totalRewards = directRewards.add(roundRewards).add(levelRewards).add(UnstakeAmount);
        uint256 withdrawnRewards = rewardInfo[msg.sender].withdrawnRewards;
        ...
        rewardInfo[msg.sender].withdrawnRewards = withdrawnRewards.add(withdrawable);
