...
contract KeystoneStashFacet {
    ...
    function UnlockKeystoneFromGameGenerateMessageHash(
        ...
        bytes32 structHash = keccak256(
            abi.encode(
                keccak256(
                    "UnlockKeystoneFromGamePayload(uint256 poolId, uint256 amount, uint256 roundTripId, uint256 requestId, uint256 blockDeadline)"
                ),
                poolId,
                amount,
                roundTripId,
    ...
    function UnlockKeystoneFromGameWithSignature(
        ...
        LibTerminusToken._unlockKeystonesFromGame(poolId, amount, roundTripId);
    }

    function setKeystonePoolId(uint256 pool) external {
        LibDiamond.enforceIsContractOwner();
        LibTerminusToken.setKeystonePoolId(pool);
    }
...
library LibTerminusToken {
    ...
    function setKeystonePoolId(uint256 pool) internal {
        terminusStorage().keystonePoolIds[pool] = true;
    }

    function setLootboxId(uint256 pool) internal {
        terminusStorage().lootboxPoolIds[pool] = true;
    }
    ...
    function stashLootBox(
        ...
        emit Claimed(stasher, tokenAddress, bundleId, poolId, amount);
    }
