...
import "../libraries/LibCategories.sol";
...
contract CardsFacet {
    ...
    function createCard(CreateCardArgsStruct calldata _args) external {
        uint256 tokenId = LibCards.currentTokenId();

        // Number of cards created is below the limit.
        require(s.projectCardIds[_args.projectId].length < LibCards.cardLimit());

        // Not authorized.
        require(LibProjects.getAdminByProjectId(_args.projectId, LibERC2771Context.msgSender()) == 1);

        // Name must be at least 1 and no more than 50 characters.
        require(bytes(_args.name).length != 0);
        require(bytes(_args.name).length <= 50);

        // ImageCID must be at least 1 and no more than 64 characters.
        require(bytes(_args.imageCID).length != 0);
        require(bytes(_args.imageCID).length <= 64);

        // Description must be at least 1 and no more than 1000 characters.
        require(bytes(_args.description).length != 0);
        require(bytes(_args.description).length <= 1000);

        // Role must be at least 1 and no more than 25 characters.
        require(bytes(_args.role).length != 0);
        require(bytes(_args.role).length <= 25);

        string memory category = LibStrings.lower(_args.category);
        // Category must be at least 1 and exist in the categories
        require(LibCategories.validateCategory(category));

        // NOTE: URL validation is GAS-costly, so only limit the maximum number of characters
        // SNS URL must be no more than 100 characters.
        require(
            bytes(_args.twitter).length <= 100
        );
        require(
            bytes(_args.opensea).length <= 100
        );
        require(
            bytes(_args.discord).length <= 100
        );
        ...
        LibCards.incrementCurrentTokenId();
    }
...
library LibCategories {
    ...
    function getCategories() internal view returns (string[] memory) {
        LibAppStorage storage s = appStorage();
        return s.categories;
    }

    function validateCategory(string memory _category) internal view returns (bool) {
        require(bytes(_category).length != 0);

        LibAppStorage storage s = appStorage();

        uint256 count = s.categories.length;
        for (uint256 i; i < count; ) {
            if (!LibStrings.compareToIgnoreCase(_category, s.categories[i])) {
                return false;
            }
            unchecked {
                ++i;
    ...
    // NOTE: Only the owner of the contract can call this method, so validation is light.
    function addCategory(string calldata _category) internal {
        // Category must be at least 0 characters.
        require(bytes(_category).length != 0);

        LibAppStorage storage s = appStorage();
        s.categories.push(_category);
    }

    // NOTE: Only the owner of the contract can call this method, so validation is light.
    function updateCategory(uint256 _index, string calldata _category) internal {
        LibAppStorage storage s = appStorage();

        // _index must be within the array length.
        require(s.categories.length > _index);

        // Category must be at least 0 characters.
        require(bytes(_category).length != 0);

        s.categories[_index] = _category;
    }

    function deleteCategory(uint256 _index) internal {
        LibAppStorage storage s = appStorage();
        ...
        uint256 count;
        uint256 categoryCount = s.categories.length;
        for (uint256 i; i < categoryCount; ) {
            if (bytes(s.categories[i]).length != 0) {
                unchecked {
                    ++count;
                }
            }
