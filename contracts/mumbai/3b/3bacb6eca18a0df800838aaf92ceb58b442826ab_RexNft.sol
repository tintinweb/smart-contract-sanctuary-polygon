...
contract RexNft is Ownable, ERC721A, ReentrancyGuard {
  ...
  
  {
    ...
    if(block.timestamp > whitelistStart && block.timestamp < whitelistEnd){
     ...
     require(whitelistMinted + quantity <= maximumWhitelistMint,"Limit Reached for whitelist");
     require(totalSupply() + quantity <= collectionSize, "reached max supply");
     require(msg.value >= quantity * whitelistPrice ,"Enter correct whitelist Amount"); 
     _safeMint(msg.sender, quantity);
     whitelistMintByUser[msg.sender] += quantity;
  ...
  function updateWhitelist(address[] memory users, bool _isWhitelisted) public {
    for(uint256 i=0; i< users.length; i++){
      isWhitelisted[users[i]] = _isWhitelisted;
      emit WhitelistUpdated(users[i], _isWhitelisted);
    }
  }
