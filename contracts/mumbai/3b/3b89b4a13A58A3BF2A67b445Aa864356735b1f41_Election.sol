...
/// @author Alexandas
contract Election is IElection, ReentrancyGuardUpgradeable {
	...
	/// @dev vote frozen period
	uint256 public voteFrozenPeriod;

	/// @dev return voter inforamtion
	...
	/// @dev return shares for a specific candidate
	mapping(address => uint256) public accPerShares;

	/// @dev voter apply withdrawl
	mapping(address => mapping(uint256 => VoterWithdrawl)) public voterWithdrawals;

	mapping(address => uint256) public voterWithdrawlCurrentNonces;

	modifier onlyStake() {
		require(msg.sender == address(stake), 'Election: caller must be Stake contract');
		_;
	}

	/// @dev proxy initialize function
	/// @param _token ERC20 token address
	/// @param _rewarder `Rewarder` contract
	...
	function initialize(
		...
		_setToken(_token);
		_setRewarder(_rewarder);
		_setStake(_stake);
		_setMajorCandidates(_majorCandidates);
		_setVoteFrozenPeriod(3600);
		ACC_PRECISION = 1e12;
	}
	...
	function vote(
		...
		stake.voterAllocate(candidate);
		voters[candidate][voter].amount = voters[candidate][voter].amount + amount;
		voters[candidate][voter].rewardDebt = voters[candidate][voter].rewardDebt + int256((amount * accPerShares[candidate]) / ACC_PRECISION);
		voteSupply[candidate] = voteSupply[candidate] + amount;
		majorCandidates.upsetCandidateWithAnchor(candidate, voteSupply[candidate], anchor, maxSlippage);
	...
	function claim(address candidate, address to) external override nonReentrant {
		stake.voterAllocate(candidate);
		VoterInfo storage voter = voters[candidate][msg.sender];
		int256 accumulated = int256((voter.amount * accPerShares[candidate]) / ACC_PRECISION);
		uint256 pending = (accumulated - voter.rewardDebt).toUInt256();
		voter.rewardDebt = accumulated;
		rewarder.mint(to, pending);
	...
	function applyWithdraw(
		...
		stake.voterAllocate(candidate);
		VoterInfo storage voter = voters[candidate][msg.sender];
		voter.rewardDebt = voter.rewardDebt - int256((amount * accPerShares[candidate]) / ACC_PRECISION);
		voter.amount = voter.amount - amount;
		voteSupply[candidate] = voteSupply[candidate] - amount;
		if (stake.isCandidateRegistered(candidate)) {
			majorCandidates.upsetCandidateWithAnchor(candidate, voteSupply[candidate], anchor, maxSlippage);
		}
		uint256 nonce = voterWithdrawlCurrentNonces[msg.sender];
		voterWithdrawals[msg.sender][nonce].endTime = block.timestamp + voteFrozenPeriod;
		voterWithdrawals[msg.sender][nonce].amount += amount;
		voterWithdrawlCurrentNonces[msg.sender]++;
		emit ApplyWithdrawn(nonce, candidate, msg.sender, amount);
	}

	function withdraw(uint256 nonce, address to, uint256 amount) external override nonReentrant {
		require(isWithdrawUnfrozen(nonce, msg.sender), 'Election: votes is frozen');
		voterWithdrawals[msg.sender][nonce].amount -= amount;
		token.transfer(to, amount);
		emit Withdrawn(nonce, msg.sender, to, amount);
	}
	...
	function pendingReward(address candidate, address voter) public view override returns (uint256 pending) {
		if (voteSupply[candidate] > 0) {
			uint256 pendingAllocation = stake.pendingVoterAllocation(candidate);
			int256 rewardDebt = voters[candidate][voter].rewardDebt;
			uint256 pendingShares = accPerShares[candidate] + ((pendingAllocation * ACC_PRECISION) / voteSupply[candidate]);
			pending = (int256((voters[candidate][voter].amount * pendingShares) / ACC_PRECISION) - rewardDebt).toUInt256();
		}
	}
	...
	/// @param candidate candidate address
	/// @param amount reward amount
	function onAllocate(address candidate, uint256 amount) external override onlyStake {
		if (voteSupply[candidate] > 0) {
	...
	function isWithdrawUnfrozen(uint256 nonce, address voter) public view returns(bool) {
		if (voterWithdrawals[voter][nonce].endTime == 0) {
			return false;
		}
		return block.timestamp > voterWithdrawals[voter][nonce].endTime;
	}
	...
	function _setVoteFrozenPeriod(uint256 period) internal {
		voteFrozenPeriod = period;
		emit VoteFrozenPeriodUpdated(period);
	}
}
