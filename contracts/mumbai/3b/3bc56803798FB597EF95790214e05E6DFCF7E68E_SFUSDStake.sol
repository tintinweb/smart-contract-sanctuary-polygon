...
contract SFUSDStake is Ownable {
    using SafeERC20 for IERC20;

    address public SFUSDaddress;
    ...
    uint256 public constant WITHDRAW_PERIOD = 7 seconds;
    uint256 public constant LOCKED_PERIOD = 63 seconds;
    ...
    constructor(
        ...
        address _SFUSDaddress,
        address _devAddress,
        uint256 _startDate
    ) {
        require(_SFUSDaddress != address(0), "Invalid tokenAddress");
        require(_devAddress != address(0), "Invalid dev wallet address");
        require(_startDate > block.timestamp, "Invalid start time");
        SFUSDaddress = _SFUSDaddress;
        devAddress = _devAddress;
        startDate = _startDate;

        transferOwnership(_multiSignWallet);
    }
    ...
    function deposit(uint256 _amount) external {
        ...
        if (investors[msg.sender].totalRewardAmount >= _amount) {
            ...
            depositState[_id].pocketInvestment =
                amountToDeposit -
                investors[msg.sender].totalRewardAmount;
            rewardPool = rewardPool + investors[msg.sender].totalRewardAmount;
        ...
        investors[msg.sender].nextClaimDate = block.timestamp + WITHDRAW_PERIOD;
    ...
    function withdrawPocketInvestment() external {
    ...
    function getClaimableReward(
        ...
        uint256 profit;
        uint256 currentTime = (investors[investor].startTime + LOCKED_PERIOD) >
            block.timestamp
            ? block.timestamp
            : (investors[investor].startTime + LOCKED_PERIOD);
        if (
            ...
            profit =
                ...
                (TOTAL_PERCENTAGE * WITHDRAW_PERIOD);
        } else {
            profit =
                ((depositState[_id].depositAmount * (DAILY_ROI * 7)) *
    ...
    function migrateInvestorToV2(
        ...
        investors[_investor].nextClaimDate = _depositAt + WITHDRAW_PERIOD;
        totalInvested = totalInvested + _amount;
    ...
    function removeInvestorOnWithdraw(address _investor) private {
        ...
        totalInvested =
            totalInvested -
            ((totalAmountLocked * TOTAL_PERCENTAGE) /
                (TOTAL_PERCENTAGE - DEPOSIT_FEE));
