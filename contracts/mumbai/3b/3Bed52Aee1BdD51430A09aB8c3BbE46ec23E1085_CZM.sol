...
function userBirthdateFunction(address user) external view returns (string memory) {
    ...
    uint256 age = getDecentralizedDays();

    if (age > birthdate) {
        uint256 decentralizedBirthdaysPassed = age / 360;
        return string(abi.encodePacked("You were born in the TimeVerse ", uint256ToString(decentralizedBirthdaysPassed), " decentralized birthdays ago."));
    } else if (age == birthdate) {
        return "Happy decentralized birthday! You were born in the TimeVerse.";
    } else {
        uint256 daysRemaining = (birthdate - age);
        uint256 decentralizedBirthdaysRemaining = (daysRemaining + 359) / 360;
        uint256 hsRemaining = daysRemaining * HS_DURATION;
        uint256 decentralizedDaysRemaining = daysRemaining * DECENTRALIZED_DAY_DURATION;

        string memory message = string(abi.encodePacked("Your decentralized birthday is coming up! Remaining days before decentralized anniversary: "));
        message = string(abi.encodePacked(message, uint256ToString(daysRemaining), " days ("));
        message = string(abi.encodePacked(message, uint256ToString(hsRemaining), " HS, "));
        message = string(abi.encodePacked(message, uint256ToString(decentralizedDaysRemaining), " decentralized days, "));
        message = string(abi.encodePacked(message, uint256ToString(decentralizedBirthdaysRemaining), " decentralized birthdays)."));
