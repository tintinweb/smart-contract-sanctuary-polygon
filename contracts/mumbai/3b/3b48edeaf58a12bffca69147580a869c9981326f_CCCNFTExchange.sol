...
contract CCCNFTExchange is OrderState, TransferSafe {
    ...
    function sell(
        ...
        if(rewardParam.isBadge && isStore) { sendReward(order.key.owner, rewardParam.rewardAmount); }
    ...
    function buy(
        ...
        setCompleted(order, true);
        transferToken(order, msg.sender, isStore, uri);
        if(rewardParam.isBadge  && isStore) { sendReward(msg.sender, rewardParam.rewardAmount); }
        emitMatchOrder(order, msg.sender);
    }
