...
contract Lottery is Ownable {
	...
	function getLotteryInfo(uint _round) public view returns (uint, uint, uint[] memory, uint[] memory, uint[] memory) {
		LotteryInfo storage _info = lotteryInfos[_round];
		uint _winNum = _info.winQuantity;
		uint _totals = _info.totalQuantity;
		uint _ratesNum = _info.winRateBitValues.length;

		uint[] memory winnedNumbers = new uint[](_winNum);
		uint _tail;
		uint _winnedNumber;
		uint _tailIdx = 0;
		uint _winnedNumIdx = 0;
		for (uint i = 1; i <= _ratesNum; i++) {
			for (uint j = 0; j < _info.winRateBitValues[i-1]; j++) {
				_tail = _info.winningTails[_tailIdx];
				if (i == _ratesNum) {
					winnedNumbers[_winnedNumIdx] = _tail;
					_winnedNumIdx++;
				} else {
					for (uint k = 0; k <= _totals / 10**i; k++) {
						_winnedNumber = k * 10**i + _tail;
						if (_winnedNumber > 0 && _winnedNumber <= _totals) {
							winnedNumbers[_winnedNumIdx] = _winnedNumber;
							_winnedNumIdx++;
						}
					}
				}
		...
		return (_winNum, _totals, _info.winningTails, _info.winRateBitValues, winnedNumbers);
	}
}