...
import { IRoboNouns } from "contracts/interfaces/IRoboNouns.sol";
...
contract RoboNouns is IRoboNouns, PausableUpgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    function settleAuction(
        ...
        require(
            expectedBlockNumber == block.number - 1 ||
                expectedBlockNumber == block.number - 2 ||
                expectedBlockNumber == block.number - 3 ||
    ...
    function unpause() external override onlyOwner {
        ...
        if (nextNounId == 0) {
            ...
            NounsAuctionHouse oldAuctionHouse = NounsAuctionHouse(oldAuctionHouseAddress);
        ...
        returns (uint256 nounId, IRoboNounsSeeder.Seed memory seed, string memory svg, uint256 price, bytes32 hash)
    {
        ...
        INounsDescriptor descriptor = INounsDescriptor(roboNounstoken.descriptor());
...
import { IRoboNounsToken } from "./interfaces/IRoboNounsToken.sol";
import { IWETH } from "./interfaces/IWETH.sol";

contract NounsAuctionHouse is INounsAuctionHouse, PausableUpgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    // The Nouns ERC721 token contract
    IRoboNounsToken public nouns;
    ...
    function _createAuction() internal {
        try nouns.mint(block.number - 1) returns (uint256 nounId) {
...
interface IRoboNouns {
    ...
    function settleAuction(uint256 nounId, uint256 expectedBlockNumber) external payable;
