    ...
    ERC721Pausable
{
    ...
    function setMinter(address _mintAddress) external onlyOwner {
        minter = _mintAddress;
    }
    ...
    function batchMint(uint256[] memory tokenIds, address toAddress)
        external
        onlyMinter
    {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            _mint(toAddress, tokenIds[i]);
        }
    }
    ...
    {
        require(_exists(tokenId), "PFP: URI query for nonexistent token");
    ...
    function _minter() public view virtual returns (address) {
        return minter;
    }
