...
contract Factory is IFactory, IOwnable {
    ...
    function setPassiveNft(address _passiveNft) external onlyOwner {
        require(
            (_passiveNft != address(0x0)) && (_passiveNft != address(passiveNft)),
            "Wrong passive"
        );
        if (address(passiveNft) != _passiveNft) {
            emit PassiveNftSet(address(passiveNft), _passiveNft);
    ...
    function setTNGBL(address _tngbl) external onlyOwner {
        ...
        emit TNGBLAddressSet(address(TNGBL), _tngbl);
        TNGBL = IERC20(_tngbl);
    }
    ...
    function lockTNGBLOnTNFT(
        ...
        tnft.lockTNGBL(tokenId, _years, lockedAmountTNGBL);
    }

    function decreaseInstantLiquidityStock(
        ITangibleNFT nft,
        uint256 fingerprint
    ) external override onlyOwnerOrMarketplace {
