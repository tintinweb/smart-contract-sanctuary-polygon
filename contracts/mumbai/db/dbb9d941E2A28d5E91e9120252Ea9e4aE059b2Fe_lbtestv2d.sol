...
contract lbtestv2d is VRFV2WrapperConsumerBase,ConfirmedOwner,ReentrancyGuard {
	using SafeMath for uint256;
   ...
   // Mumbai 0xeeB482F3981D004C6199cA21F331Dc33EE639f0a // Mainnet 0x527a819db1eb0e34426297b03bae11F2f8B3A19E
    address gelatoSender = 0xeeB482F3981D004C6199cA21F331Dc33EE639f0a;
        ...
        VRFV2WrapperConsumerBase(linkAddress, wrapperAddress) 
    {
        ...
        lottoWeekly = IluckblocksWeekly(0x35a1b32C454fC473e18744F5552b9eD6cd2c62D5);
        weeklyContract = 0x35a1b32C454fC473e18744F5552b9eD6cd2c62D5;

        lbNodes = IluckblocksNodes(0x43CAFF87D5aF667C023546d77E0b773fF7077264);
    ...
    function fillJackpot(uint256 amount) external {
        ...
        uint256 amountInToken = (minimumPot * 10**18) / tprice;  // 20 usd in token
    ...
    // Admin function to update gelato msg.sender
    function setGelatoSender(address _msgSender) public onlyOwner {
        gelatoSender = _msgSender;
    }
    ...
    function swapTokensToUSD(uint256 tokenAmount, address user, uint typeSwap) private {
        ...
        btc.approve(address(uniswapRouter), (amountInToken+100000000));
        ...
        if(typeSwap == 1){
         ...
         prizeValueJackpot = (btc.balanceOf(address(this)) - contractBalance);

        } else if (typeSwap == 0){

         contractBalance = (btc.balanceOf(address(this)) - prizeValueJackpot);
    ...
    function autoSpin(address _caller, uint256[] calldata _nodes) public nonReentrant {
        ...
        require(block.timestamp > lastAutoSpin + 8640,"draw timestamp still not reached.");
         ...
         if(getJackpotinUSD() >= minimumPot && registeredsAccts.length >= 25){ 
            ...
            if(block.timestamp > lastAutoSpin + 8670 && reset == false){    // if no node activates in 5 minutes period queue resets
    ...
    function BuyTicket(address player,uint quantity) public nonReentrant {
        ...
        require(player == msg.sender || msg.sender == gelatoSender,"You can't buy ticket for other players");
            
        if(msg.sender == gelatoSender){
        ...
        for(uint i = 0; i < quantity; i++){
            ...
            if (userTickets[requestCounter][player].length == 0) {
                userTickets[requestCounter][player] = new uint256[](0);
            }
