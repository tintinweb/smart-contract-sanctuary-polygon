...
contract NFTCredential is ERC721A, Ownable {
  ...
  function mintAndTransfer(
    ...
    require(
      requestNum == _description.length,
      'The length of _description, _toAddresses and _imageURIs are NOT same.'
    );
    require(
    ...
    require(
      ...
      'The length of _description, _toAddresses and _externalURIs are NOT same.'
    );
    ...
    for (uint i = 0; i < requestNum; i++) {
      ...
      _setTokenURI(
        tokenId,
        generateTokenURI(_description[i], _imageURIs[i], _externalURIs[i])
      );
      tokenId += 1;
    }
  }
