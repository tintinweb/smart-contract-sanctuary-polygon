...
import './interfaces/ISBCToken.sol';
...
contract Presale {
    ...
    uint256 public constant MIN_ALLOC_USDC = 10 * 1e18; // USDC min allocation for each presale buyer
    uint256 public constant MAX_ALLOC_USDC = 100 * 1e18; // USDC max allocation for each presale buyer
    uint256 public MIN_ALLOC_SBC = priceRate * MIN_ALLOC_USDC; // min SBC allocation for each presale buyer
    uint256 public MAX_ALLOC_SBC = priceRate * MAX_ALLOC_USDC; // max SBC allocation for each presale buyer
    uint256 public TotalPresaleAmnt = 1e5 * 1e18; // Total SBCToken amount for presale : 100,000 SBC
    ...
    address public USDC_Addr = 0xe11A86849d99F524cAC3E7A0Ec1241828e332C62; // USDC on testnet
    ...
    function deposit(uint256 USDC_amount) external payable whileOnGoing returns (uint256) {
        ...
        uint256 newSBCAmount = USDC_amount.div(1e18).mul(priceRate * 1e18);
