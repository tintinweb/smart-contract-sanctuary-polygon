    ...
    ExtensionPause
{
        ...
        onlyRole(ADMIN_ROLE)
    {
        ...
        _rebalance2(newAssets, usdAmount); // обновляет структуры Asset
        // _buyNewAssets(newAssets, usdAmount);
        uint256 newPrice = _calcCost(1e18);

        emit Rebalance(newAssets, oldPrice, newPrice);
    }
    ...
    function _rebalance2(Asset[] memory newAssets, uint256 usdAmount)
        internal
        virtual
    {
        _clearActiveAssets();
...
interface IIndex {
     ...
     */
    function init(Asset[] memory newAssets) external;
