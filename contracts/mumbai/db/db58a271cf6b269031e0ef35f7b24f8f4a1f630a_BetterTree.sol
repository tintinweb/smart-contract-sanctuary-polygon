...
// File: default_workspace/Trees.sol
...
contract BetterTree is  ReentrancyGuard, ERC721Enumerable, Ownable{
    ...
    event roleChanged(address indexed account, bool indexed role);
    ...
    uint256 constant calculator = 6 weeks;


    uint256 _tokenIdCounter;
    uint256 _storedTreesCounter;
    ...
    /**
    * @dev owner function to change pollinator and minting roles
    */
    function changeRoles(bool role, address account)external onlyOwner{
        require(Address.isContract(account), "Must be a contract.");
        if(role){minter = account;}
        else{pollinator = account;}
    ...
    modifier onlyRole(){
        require(msg.sender == minter || msg.sender == pollinator, "Not Authorized");
        _;
    }
     ...
     */
    function mint(uint8 _numberToMint, uint16 _cause, uint256 _treeValue, address _user)external onlyRole nonReentrant returns(uint256[] memory){
        uint256[] memory ret = new uint256[](_numberToMint);
        require(treesInStorage() >= _numberToMint, "Error: Tree shortage");
        for(uint8 i =0; i < _numberToMint; i++){ret[i] = _minting(_user, _cause, _treeValue);}
    ...
    */
    function treesInStorage()public view returns(uint256){
        ...
        if(sum > waiting){return sum - waiting;}
        return 0;
    }
    /**
    ...
    */
    function _minting(address _user,  uint16 _cause, uint256 _treeValue)internal returns(uint256){
    ...
    */
    ...
    function checkAdminTrees()external onlyOwner view returns(uint256[] memory, uint256[] memory,uint256[] memory, uint256[] memory){
        return (_rarityToTreeIndex[0], _rarityToTreeIndex[1], _rarityToTreeIndex[2], _rarityToTreeIndex[3]);
    }
     ...
     */
    function burn(uint256 _tokenId, uint8 _reqGrowth)external nonReentrant returns(bool){
    ...
    function fertilise(uint256 _amount, uint256 _tokenId) public onlyRole{ 
    ...
    function addTree(uint256 batchID, uint256[] memory jsonId)external onlyOwner{
        ...
        for(uint16 i =0; i<a; i++){
            ...
            _rarityToTreeIndex[r].push(_storedTreesCounter);
            _storedTreesCounter++;
        }
    ...
    */
    function addValue(uint256 _amount, uint256 _tokenId, uint16 _causeId)public onlyRole{
        require(_exists(_tokenId), "Token Does Not exist.");
        TokenData storage a = tokenInfo[_tokenId];
        if(a.causeID==0){a.causeID=_causeId;}
        a.treeValue += _amount;
