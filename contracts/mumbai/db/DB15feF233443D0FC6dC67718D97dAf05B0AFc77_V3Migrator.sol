...
import '../interfaces/ISkeletonFactory.sol';
import '../interfaces/ISkeletonPool.sol';
...
abstract contract PoolInitializer is IPoolInitializer, PeripheryImmutableState {
    ...
    function createAndInitializePoolIfNecessary(
        ...
        pool = ISkeletonFactory(factory).getPool(token0, token1, fee);

        if (pool == address(0)) {
            pool = ISkeletonFactory(factory).createPool(token0, token1, fee);
            ISkeletonPool(pool).initialize(sqrtPriceX96);
        } else {
            (uint160 sqrtPriceX96Existing, , , , , , ) = ISkeletonPool(pool).slot0();
...
/// @notice Wraps SkeletonSwap V3 positions in a non-fungible token interface which allows for them to be transferred
/// and authorized.
...
interface IPeripheryImmutableState {
    /// @return Returns the address of the SkeletonSwap V3 deployer
    function deployer() external view returns (address);

    /// @return Returns the address of the SkeletonSwap V3 factory
    function factory() external view returns (address);
...
interface IPeripheryPayments {
    ...
    /// that use ether for the input amount. And in SkeletonSwap Router, this would be called
...
/// @title The interface for the SkeletonSwap V3 Factory
/// @notice The SkeletonSwap V3 Factory facilitates creation of SkeletonSwap V3 pools and control over the protocol fees
interface ISkeletonFactory {
...
import './pool/ISkeletonPoolImmutables.sol';
import './pool/ISkeletonPoolState.sol';
import './pool/ISkeletonPoolDerivedState.sol';
import './pool/ISkeletonPoolActions.sol';
import './pool/ISkeletonPoolOwnerActions.sol';
import './pool/ISkeletonPoolEvents.sol';

/// @title The interface for a SkeletonSwap V3 Pool
/// @notice A SkeletonSwap pool facilitates swapping and automated market making between any two assets that strictly conform
/// to the ERC20 specification
/// @dev The pool interface is broken up into many smaller pieces
interface ISkeletonPool is
    ISkeletonPoolImmutables,
    ISkeletonPoolState,
    ISkeletonPoolDerivedState,
    ISkeletonPoolActions,
    ISkeletonPoolOwnerActions,
    ISkeletonPoolEvents
{
...
interface ISkeletonPoolActions {
    ...
    /// @dev The caller of this method receives a callback in the form of ISkeletonMintCallback#uniswapV3MintCallback
    ...
    /// @dev The caller of this method receives a callback in the form of ISkeletonSwapCallback#uniswapV3SwapCallback
    ...
    /// @dev The caller of this method receives a callback in the form of ISkeletonFlashCallback#uniswapV3FlashCallback
...
interface ISkeletonPoolDerivedState {
...
interface ISkeletonPoolImmutables {
    /// @notice The contract that deployed the pool, which must adhere to the ISkeletonFactory interface
...
interface ISkeletonPoolOwnerActions {
...
library PoolAddress {
    bytes32 internal constant POOL_INIT_CODE_HASH = 0xdf7aabc163e8381786aed31aa1649740a76ef34262766dc5bca3d3aa5fcede29;
    ...
    /// @param deployer The SkeletonSwap V3 deployer contract address
