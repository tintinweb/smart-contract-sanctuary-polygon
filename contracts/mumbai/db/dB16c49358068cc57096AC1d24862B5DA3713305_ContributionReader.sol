...
import {IContributionReader} from "../interface/platform/IContributionReader.sol";
import {IBoard} from "../interface/token/IBoard.sol";
import {LibContributionReader} from "../library/LibContributionReader.sol";

/**
 * @dev Implementation of ContributionReader.
 * Read the Contribution for board member's.
 */
contract ContributionReader is
  IContributionReader,
  Initializable,
  ...
  UUPSUpgradeable
{
  ...
  function initialize(address _admin) public initializer {
    ...
    _setupRole(LibContributionReader.UPGRADER_ROLE, _admin);
  }
  ...
  function _authorizeUpgrade(address _newImplementation)
    internal
    override
    onlyRole(LibContributionReader.UPGRADER_ROLE)
  {}

  /// @inheritdoc IContributionReader
  function calculateBoardMemberContributionWithLinear(
    address[] memory _members,
    LibContributionReader.BoardsList memory _lists
  )
    external
    view
    virtual
    returns (LibContributionReader.BoardCalcResult[] memory results)
  {
    results = new LibContributionReader.BoardCalcResult[](_members.length);
    for (uint256 memberIdx = 0; memberIdx < _members.length; memberIdx++) {
      //Calc Contribution for manatary board
      results[memberIdx].BoardMenber = _members[memberIdx];
      for (uint256 monetaryboardIdx = 0; monetaryboardIdx < _lists.monetaryBoards.length; monetaryboardIdx++) {
        IBoard board = _lists.monetaryBoards[monetaryboardIdx];
        if (board.isBoardingMember(_members[memberIdx])) {
          IContributionPool c = board.getContributionPool();
          uint120 value = c.getContribution(_members[memberIdx]);
          results[memberIdx].ContributionTotal += value;
          results[memberIdx].MonetaryContributionTotal += value;
        }
      }
      //Calc Contribution for Non manatary board
      for (uint256 NonmonetaryboardIdx = 0; NonmonetaryboardIdx < _lists.NonmonetaryBoards.length; NonmonetaryboardIdx++) {
        IBoard board = _lists.NonmonetaryBoards[NonmonetaryboardIdx];
        if (board.isBoardingMember(_members[memberIdx])) {
          IContributionPool c = board.getContributionPool();
          uint120 value = c.getContribution(_members[memberIdx]);
          results[memberIdx].ContributionTotal += value;
          results[memberIdx].NonMonetaryContributionTotal += value;
        }
      }
    }
  }
...
library LibContributionReader {
  ...
  struct BoardsList{
    IBoard[] monetaryBoards;
    IBoard[] NonmonetaryBoards;
  }

  struct BoardCalcResult{
    address BoardMenber;
    uint120 ContributionTotal;
    uint120 MonetaryContributionTotal;
    uint120 NonMonetaryContributionTotal;
  }
