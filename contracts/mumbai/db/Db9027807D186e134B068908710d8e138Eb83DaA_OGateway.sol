...
import "../interfaces/IComposableOA.sol";
import "./OmniERC20.sol";
import "@openzeppelin/contracts/token/ERC20/ERC20.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/security/ReentrancyGuard.sol";
import {SendParams, SendPayload} from "../structs/gateway/OGatewayStructs.sol";

contract OGateway is IOmniApp, Ownable, ReentrancyGuard {
     ...
     *
     * @param params SendParams parameters of sending transaction, including sent token and destination chain name.
     */
    function sendTo(SendParams calldata params) public payable nonReentrant {
        require(bytes(params.dstChainName).length > 0, "!CHAIN");
        address tokenAddress = address(params.token);
        require(tokenAddress != address(0), "!ADDRESS");
        require(params.amount > 0, "!AMOUNT");
        require(_isContract(tokenAddress), "!EXISTS");
        require(params.token.balanceOf(msg.sender) >= params.amount, "!BALANCE");

        if (isCopy[tokenAddress]) {
            IOmniERC20 omniToken = IOmniERC20(tokenAddress);
            omniToken.burn(msg.sender, params.amount);
            _send(_getPayload(params, copyToOriginal[tokenAddress], omniToken.originalChainName()), params.dstChainName, params.gas);

            return;
        }

        params.token.transferFrom(msg.sender, address(this), params.amount);
        locked[tokenAddress][msg.sender] += params.amount;
        emit Locked(tokenAddress, params.amount, msg.sender);
        _send(_getPayload(params, tokenAddress, chainName), params.dstChainName, params.gas);
    }
     ...
     */
    function omReceive(bytes calldata _payload, address srcOA, string memory srcChain) external override nonReentrant {
        emit OmReceived(srcChain, srcOA);
        require(isOA(srcChain, srcOA));
        (SendPayload memory payload) = abi.decode(_payload, (SendPayload));

        // TODO: Each to separate method passing only SendPayload as argument
        if (locked[payload.originalAddress][payload.to] > 0) {
            IERC20 original = IERC20(payload.originalAddress);
            locked[payload.originalAddress][payload.to] -= payload.amount;
            original.transfer(payload.to, payload.amount);
            emit Transferred(payload.originalAddress, payload.amount, payload.to);

            if (payload.message.length > 0) {
                IComposableOA(payload.to).onReceive(payload.message, payload.from);
            }

            return;
        }

        if (originalToCopy[payload.originalAddress] != address(0)) {
            IOmniERC20 omniToken = IOmniERC20(originalToCopy[payload.originalAddress]);
            omniToken.mint(payload.to, payload.amount);
            isCopy[address(omniToken)] = true;
            // Is that needed?
        } else {
            OmniERC20 omniToken = new OmniERC20(payload.name, payload.symbol, payload.decimals, address(this), payload.originalChainName);
            omniToken.mint(payload.to, payload.amount);
            address tokenAddress = address(omniToken);
            isCopy[tokenAddress] = true;
            originalToCopy[payload.originalAddress] = tokenAddress;
            copyToOriginal[tokenAddress] = payload.originalAddress;
        }
        emit Transferred(payload.originalAddress, payload.amount, payload.to);
    ...
    */
    ...
    function requestForceUnlock(address _token) external {
        require(locked[_token][msg.sender] > 0, '!LOCKED');
        forceUnlockRequests[_token][msg.sender] = locked[_token][msg.sender];
    }

    function forceUnlock(address _token, address _account) external onlyOwner nonReentrant {
        require(locked[_token][_account] > 0, '!LOCKED');
        require(forceUnlockRequests[_token][_account] == locked[_token][_account], '!REQUESTED');
        uint256 toUnlock = locked[_token][_account];
        forceUnlockRequests[_token][_account] = 0;
        locked[_token][_account] = 0;
        IERC20 unlocked = IERC20(_token);
        unlocked.transferFrom(address(this), _account, toUnlock);
    }
     ...
     */
    function _send(bytes memory payload, string memory dstChainName, uint256 gas) private {
        omnichainRouter.send{value : msg.value}(dstChainName, remoteChainToOA[dstChainName], payload, gas, msg.sender, 0);
    }

    /**
     * @notice Encodes the data to the bytes payload for the cross-chain message.
     *
     * @param params SendParams struct sendTo function parameters.
     * @param original Address of the original bridged ERC20.
     */
    function _getPayload(SendParams calldata params, address original, string memory _originalChainName) private view returns (bytes memory) {
        address _to = address(params.to) == address(0) ? msg.sender : params.to;

        SendPayload memory payload = SendPayload(
            params.dstChainName,
            _originalChainName,
            original,
            params.amount,
...
interface IComposableOA {
    /**
     * @notice Handles the external contract's custom logic on receive bridged token
     *
     * @param message arbitrary encoded message
     * @param from account that initiated bridging request
     */
    function onReceive(bytes memory message, address from) external;
}
...
import "../interfaces/IOmniERC20.sol";
import "../interfaces/IOGateway.sol";
import "@openzeppelin/contracts/security/ReentrancyGuard.sol";

contract OmniERC20 is IOmniERC20, ERC20, IERC165, ReentrancyGuard {
    address public bridge;
    string public override originalChainName;
    ...
    constructor(
        ...
        _decimals = decimals_;
        bridge = _bridge;
        originalChainName = _originalChain;
    }
    ...
    function burn(address _account, uint256 _amount) override external nonReentrant {
        require(msg.sender == bridge, "!BRIDGE");
        _burn(_account, _amount);
    }

    function mint(address _account, uint256 _amount) override external nonReentrant {
        require(msg.sender == bridge, "!BRIDGE");
        _mint(_account, _amount);
    }

    function redeem(uint256 _amount, address _to, bytes calldata _message, uint256 _gas) public payable nonReentrant {
        SendParams memory params = SendParams(
            originalChainName,
            ERC20(address(this)),
            _amount,
            _to == address(0) ? msg.sender : _to,
            _message,
            _gas
        );

        IOGateway(bridge).sendTo{value : msg.value}(params);
    }
}
...
import { SendParams } from "../structs/gateway/OGatewayStructs.sol";

interface IOGateway {
    function sendTo(SendParams calldata params) external payable;
}
