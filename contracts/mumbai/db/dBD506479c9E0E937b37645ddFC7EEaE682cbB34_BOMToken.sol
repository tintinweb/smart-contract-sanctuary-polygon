...
import "./LPPrice.sol";
    ...
    */
    ...
    abstract contract IBOM716 is Ownable, LPPrice {
        ...
        mapping(uint => uint) public nftBalances;
        ...
        // Public functions
        function nftBalanceOf(uint _nftID) public view onlyOwner returns (uint) {
    ...
    contract BOMToken is Context, IERC20, Ownable, IBOM716, BlackList, Pausable {
        ...
        */
        ...
        function getDecVal() internal override view returns (uint) {
            return decVal;
        }
        ...
        */
        ...
        function transferByNFT(uint _nftID, uint _value, uint _key) 
            ...
            _addTxnWeightCnt(_nftID, txn_fee);
        ...
        function distRewardToNFTHolders() public override whenNotPaused {
            ...
            require(block.timestamp - lastDistTime > distDelayDuration);
            ...
            uint burnRewardDist = totalReward.mul(1).div(100); 
            ...
            for(uint _index = 1; _index <= _nftTotalSupply; _index++) {
                ...
                if (_pastcntsum > 0) nftBalances[_index] = nftBalances[_index].add(pastCountReward.mul(_weight.pastCnt).div(_pastcntsum));
                if (_curweightsum > 0) nftBalances[_index] = nftBalances[_index].add(curWeightReward.mul(_weight.curWeight).div(_curweightsum));
                if (_curcntsum > 0) nftBalances[_index] = nftBalances[_index].add(curCountReward.mul(_weight.curCnt).div(_curcntsum));
    ...
    function transferFromByNFT(address _from, uint _nftID, uint _value, uint _key) 
        ...
        _balances[_from] = _balances[_from].sub(sendAmount);
    ...
    function withdrawReward(address _to, uint _amount, uint _nftID) external override whenNotPaused {
        ...
        require(nftBalances[_nftID] >= _amount, "Reward amount is smaller than withdraw amount");
...
import "./IERC20.sol";
    ...
    abstract contract LPPrice is Ownable{
        ...
        function setLPpairaddress(address _pairAddress, bool _isUSDFirst) public onlyOwner {
            lppairaddress = _pairAddress;
            isUSDFirst = _isUSDFirst;
        }
        ...
        function getDecVal() virtual view internal returns (uint);

        function updatePriceFromLP() public returns(uint) {
            if (isUpdatePriceFromLP == true) {
                currentPrice = getPriceFromLP();
            }
            return currentPrice;
        }

        function getPriceFromLP() public view returns(uint) {
            ...
            (uint Res0, uint Res1,) = pair.getReserves(); // Res0 = BOM, Res1 = USDT

            if (isUSDFirst) {
                return Res0 * getDecVal() / dec0 * dec1 / Res1; // Price = USDT / BOM
            } else {
                return Res1 * getDecVal() / dec1 * dec0 / Res0; // Price = USDT / BOM
            }
        }
    }
