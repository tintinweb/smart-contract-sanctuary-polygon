...
contract Manager is Ownable, Pausable {
    ...
    event SetCommissions(
        uint256[] indexed _categoryIds,
        uint256[] indexed _sellerCommissions,
        uint256[] indexed _buyerCommissions
    );
    ...
    function getCommissionSeller(
        ...
        _config = mapConfigId(_categoryId, _branchId, 0);
        if (isCommissionSellerSets[_config]) {
            return commissionSellers[_config];
        }

        _config = mapConfigId(_categoryId, 0, 0);
        if (isCommissionSellerSets[_config]) {
            return commissionSellers[_config];
        }
...
contract DigicapMarket is Manager, ERC721Holder, ReentrancyGuard {
    ...
    function _match(
        ...
        ) = callGetCategoryToken(order.tokenAddress, order.tokenId);
        uint256 _commission = getCommissionSeller(
            _categoryId,
            _branchId,
            _collectionId
        );
        uint256 _commissionToSeller = (_price * _commission) / ZOOM_FEE;
        uint256 amountToSeller = _price - _commissionToSeller;
        ...
        order.commissionToSeller = _commissionToSeller;
        order.commissionFromBuyer = _commissionFromBuyer;
        orders[_orderId] = order;
        return true;
    }

    function callGetCategoryToken(address _token, uint256 _tokenId)
        private
        returns (
            uint256,
            uint256,
    ...
    function callBuyerCommission(
        ...
        (
            ...
            uint256 _branchId,
            uint256 _collectionId
        ) = callGetCategoryToken(_token, _tokenId);
        uint256 _commission = getCommissionBuyer(
            _categoryId,
            _branchId,
            _collectionId
        );
        uint256 _commissionFromBuyer = (_amount * _commission) / ZOOM_FEE;
        uint256 amountToBuyer = _amount + _commissionFromBuyer;
        return (amountToBuyer, _commissionFromBuyer);
    }
    ...
    function buy(uint256 _orderId, address _paymentToken)
        external
        payable
        whenNotPaused
        returns (bool)
    {
        ...
        (
            ...
            uint256 _commissionFromBuyer
        ) = callBuyerCommission(order.tokenAddress, order.tokenId, orderAmount);

        if (_paymentToken == address(0) && msg.value > 0) {
            require(msg.value >= amountToBuyer, "Not-enough-to-buy");
        } else {
        ...
        emit Buy(_orderId, _paymentToken, amountToBuyer);
        return
            _match(
                msg.sender,
                _paymentToken,
    ...
    function createBid(
        ...
        (
            uint256 _amountToBuyer,
            uint256 _commissionFromBuyer
        ) = callBuyerCommission(_tokenAddress, _tokenId, _price);

        if (msg.value > 0) {
            require(msg.value >= _amountToBuyer, "Invalid-amount");
        ...
        newBid.commissionFromBuyer = _commissionFromBuyer;
        newBid.status = true;
    ...
    function updateBid(uint256 _bidId, uint256 _bidPrice)
        external
        payable
        whenNotPaused
        nonReentrant
    {
        ...
        (
            uint256 _amountToBuyerNew,
            uint256 _commissionFromBuyer
        ) = callBuyerCommission(bid.tokenAddress, bid.tokenId, _bidPrice);
        uint256 _amountToBuyerOld = bid.bidPrice + bid.commissionFromBuyer;

        bool isExcess = _amountToBuyerOld > _amountToBuyerNew;
        uint256 amount = isExcess
            ? _amountToBuyerOld - _amountToBuyerNew
            : _amountToBuyerNew - _amountToBuyerOld;
        ...
        bid.commissionFromBuyer = _commissionFromBuyer;
        bids[_bidId] = bid;
