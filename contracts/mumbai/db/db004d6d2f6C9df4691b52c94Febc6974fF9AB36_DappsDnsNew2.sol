...
contract DappsDnsNew is Initializable, ERC721Upgradeable, OwnableUpgradeable {
    ...
    DappsDnsNew2 public dappsDnsNew2;
    ...
    function updateDappsDnsNew2(
        DappsDnsNew2 _dappsDnsNew2
    ) external onlyOwner {
        require(address(_dappsDnsNew2) != address(0), "ZERO_ADDRESS");
        dappsDnsNew2 = _dappsDnsNew2;
    }
    ...
    function renewDomain(
        ...
        (address token, uint256 price) = fetchTokenAndPrice(_partnerId, _isForLifetime, domainData.tld, true);
            
        uint256 expiryTimestamp;
        if(_isForLifetime) {
            expiryTimestamp = type(uint256).max;
            uint256 creditValue = _updateCreditValue(domainData.owner, price, token);
    ...
    function registerDomainWithProof(
        ...
        _registerDomain(_msgSender(), _tld, _name, _isForLifetime, _referrer, _partnerId);
    }
    ...
    function registerDomainInBatch(
        ...
        uint tldsLen = _tlds.length;
        require(tldsLen == _names.length && tldsLen == _isForLifetime.length, "LEN_NOT_EQ");
        
        for (uint i = 0; i < tldsLen; i++) {
            // check if the tld requires merkle proof
            require(merkleRoots[_tlds[i]] == bytes32(0), "PROOF_REQ");

            _registerDomain(_msgSender(), _tlds[i], _names[i], _isForLifetime[i], _referrer, _partnerId);
        }
    }
    ...
    function _registerDomain(
        ...
        (address token, uint256 price) = fetchTokenAndPrice(_partnerId, _isForLifetime, _tld, false);
            
        uint256 expiryTimestamp;
        if(_isForLifetime) {
            expiryTimestamp = type(uint256).max;
            uint256 creditValue = _updateCreditValue(_user, price, token);
            if(_partnerId != bytes32(0))
    ...
    function fetchTokenAndPrice(
        bytes32 _partnerId,
        bool _isForLifetime,
        string memory _tld,
        bool _isRenew
    ) internal returns (address token, uint256 price) {
        if(_partnerId != bytes32(0)) {
            (token, price) = dappsDnsNew2.getTokenAndPrice(_partnerId, _isForLifetime);
            if(!_isRenew)
                dappsDnsNew2.checkAndUpdateLimit(_partnerId, _tld);
        }
        else
            price = _isForLifetime ? lifetimePrice : annualPrice;
    }
    ...
    function _updateCreditValue(
        ...
        if(_token == address(0))
            ...
            bool status = IERC20(address(0)).transferFrom(_msgSender(), address(this), _domainPrice - creditValue);
            require(status, "TRANSFER_FAILED");
        }
...
import "@openzeppelin/contracts-upgradeable/utils/CountersUpgradeable.sol";
import "./libraries/StringUtils.sol";
import "./DappsDnsNew.sol";
// import "hardhat/console.sol";

contract DappsDnsNew2 is Initializable, OwnableUpgradeable {

    DappsDnsNew public dappsDnsNew;
    ...
    // partnerId => purchased domains count
    mapping(bytes32 => uint256) public domainCount;
    ...
    modifier onlyDappsDnsNew() {
        require(_msgSender() == address(dappsDnsNew));
        _;
    }

    function __DappsDnsNew2_init(
        address _trustedForwarder,
        DappsDnsNew _dappsDnsNew
    ) public initializer {
        __Ownable_init(_trustedForwarder);
        dappsDnsNew = _dappsDnsNew;
    }

    function updateSocialAccountTypes(
        uint256 _socialAccountType,
        bool _status
    ) external onlyOwner {
        require(isAllowedSocialAccount[_socialAccountType] != _status, "UNCHANGED");
        isAllowedSocialAccount[_socialAccountType] = _status;
        emit UpdatedAllowedSocialAccountType(_socialAccountType, _status);
    }

    function updateSocialAccounts(
        string memory _domainName,
        uint256 _socialAccountType,
        string calldata _socialAccount
    ) external {
        address _user = _msgSender();

        _domainName = StringUtils.toLower(_domainName);
        (, address domainOwner, , ) = dappsDnsNew.domains(_domainName);
        require(domainOwner == _user, "NOT_DOMAIN_OWNER");
        require(isAllowedSocialAccount[_socialAccountType], "ACC_TYPE_NOT_SUPPORTED");

        socialAccounts[_domainName][_socialAccountType] = _socialAccount;

        emit UpdatedSocialAccounts(_user, _domainName, _socialAccountType, _socialAccount);
    }
    ...
    function updateAllowedToken(
        ...
        require(isAllowedToken[_token] != _isAllowed, "UNCHANGED");
        ...
        if(_isAllowed) {
            ...
            ( , uint index) = getTokenExistsAndIndex(_token);
            uint len = allowedTokens.length;
            if(index < len) {
                allowedTokens[index] = allowedTokens[len-1];
            }
            allowedTokens.pop();
        }
    ...
    function addPartner(
        ...
        // string calldata _mainText,
        // string calldata _description,
        // string[] memory _supportedTlds,
        // address _token,
        // uint256 _annualPrice,
        // uint256 _lifetimePrice,
        // uint256 _domainLimit,
        // bool _isActive
        Partner memory _partner
    ) external onlyOwner {
        (bool exists, ) = getTokenExistsAndIndex(_partner.token);
        require(exists, "INVALID_TOKEN");
        
        bytes32 id = keccak256(abi.encodePacked(_partner.name, _partner.logoUrl, _partner.domainLimit, block.timestamp));
        Partner memory partner = Partner({
            id: id,
            name: _partner.name,
            logoUrl: _partner.logoUrl, 
            mainText: _partner.mainText,
            description: _partner.description,
            supportedTlds: _partner.supportedTlds,
            payInNativeToken: _partner.payInNativeToken,
            token: _partner.token,
            annualPrice: _partner.annualPrice,
            lifetimePrice: _partner.lifetimePrice, 
            domainLimit: _partner.domainLimit,
            isActive: _partner.isActive
        });
        partners[id] = partner;

        emit PartnerAdded(id, _partner.name, _partner.isActive, ++partnerCount, block.timestamp);
    }

    function getTokenExistsAndIndex(
        address _token
    ...
    function getTokenAndPrice(
        bytes32 _partnerId,
        bool _isForLifetime
    ) public view returns (address token, uint256 price) {
        require(bytes(partners[_partnerId].name).length != 0, "INVALID_PARTNER");

        Partner memory partner = partners[_partnerId];
        if(partners[_partnerId].payInNativeToken)
            token = address(0);
        else
            token = partner.token;

        price = _isForLifetime ? partner.lifetimePrice : partner.annualPrice;
    }

    function checkTldSupport(
        bytes32 _partnerId,
        string calldata _tld
    ) public view returns (bool) {
        Partner memory partner = partners[_partnerId];
        uint len = partner.supportedTlds.length;
        for (uint i = 0; i < len; i++) {
            if(keccak256(abi.encodePacked(_tld)) == keccak256(abi.encodePacked(partner.supportedTlds[i])))
                return true;
        }
        return false;
    }
    ...
    function checkAndUpdateLimit(
        bytes32 _partnerId,
        string calldata _tld
    ) public onlyDappsDnsNew {
        Partner memory partner = partners[_partnerId];
        require(partner.isActive, "PARTNER_DEACTIVATED");
        require(domainCount[_partnerId] < partner.domainLimit, "LIMIT_EXCEEDED");
        require(checkTldSupport(_partnerId, _tld), "TLD_NA");
        
        ++domainCount[_partnerId];
    }

    // function getPartnerLimitAndStatus(
    //     bytes32 _partnerId
    // ) public view returns (uint256, bool) {
    //     return (partners[_partnerId].domainLimit, partners[_partnerId].isActive);
    // }

    // function _msgSender() internal view override(OwnableUpgradeable) returns (address) {
    //     return OwnableUpgradeable._msgSender();
    // }
