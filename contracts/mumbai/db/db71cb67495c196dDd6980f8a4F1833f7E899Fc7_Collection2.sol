...
contract Collection2 is ERC721, Ownable, IERC2981 {
    using Strings for uint256;

    // Counter for token id
    uint256 private _tokenIdCounter;
    uint256 public constant maxSupply = 6500;
    // First collection
    ...
    error NotIsApprovedOrOwnerError();
    error InvalidReceiverError();
    error NotTokenOwnerError();
    error OnlyAllowedCallerError();

    // Modifier to check only allowed caller can call function
    modifier onlyAllowedCaller() {
        if (msg.sender != _allowedCaller) revert OnlyAllowedCallerError();
        _;
    }

    constructor(
        string memory _hiddenMetadataUri,
        address _guCollection,
        uint96 _royaltyFraction
    ) ERC721("Test", "TEST") {
        hiddenMetadataUri = _hiddenMetadataUri;
        _collection = ICollection(_guCollection);
        _royaltyInfos = RoyaltyInfo(msg.sender, _royaltyFraction);
    }
    ...
    /**
     * @notice Mint token for specific token id
     * @param tokenId uint
     */
    function mint(uint256 tokenId) external {
        if (_tokenIdCounter >= maxSupply) revert MaxSupplyReachedError();
        unchecked {
            _tokenIdCounter++;
        }
     ...
     * @param _hiddenMetadataUri string
     */
    ...
    function setRoyaltyInfo(
        ...
        if (_receiver == address(0)) revert InvalidReceiverError();
