...
// Contract Address: 0xDbD0Fad8259d69C9EfeaBC29471B6114b8f78864
...
contract ChainBattles is ERC721URIStorage {
    ...
    function generateCharacter(uint256 tokenId) public returns(string memory){
        bytes memory svg = abi.encodePacked(
            ...
            '<text x="50%" y="60%" class="base" dominant-baseline="middle" text-anchor="middle">', "Strnegth: ",getStrength(tokenId),'</text>',
    ...
    function getLevels(uint256 tokenId) public view returns (string memory) {
        TokenStats memory stats = tokenIdToStats[tokenId];
    ...
    function mint() public {
        ...
        tokenIdToStats[newItemId] = TokenStats(0, random(100, 10), random(100, 40), 100);
    ...
    function random(uint number, uint adjuster) public view returns(uint){
        return uint(keccak256(abi.encodePacked(block.timestamp, block.difficulty,  msg.sender, adjuster))) % number;
    }
}
