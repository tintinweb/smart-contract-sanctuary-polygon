    ...
    ERC721
{
    ...
    // hashedId => originalId(0~9999)
    mapping(uint256 => uint256) public originalIdOf;

    // originalId(0~9999) => hashedId
    mapping(uint256 => uint256) public hashedIdOf;
    ...
    function zap(
        ...
        (
            ...
            uint256 hashedTokenId
        ) = _getOriginalAndHashedId(tokenId);

        require(
            msg.sender == ownerOf(hashedTokenId),
        ...
        returns (address avatarAddress)
    {
        ...
        uint256 hashedId = _hashId(originalId);
        originalIdOf[hashedId] = originalId;
        hashedIdOf[originalId] = hashedId;

        super._mint(to, hashedId);
        return _mintWithProxy(originalId);
    }
    ...
    function isApprovedOrOwner(address spender, uint256 hashedTokenId)
        external
        view
        override
        returns (bool)
    {
        return _isApprovedOrOwner(spender, hashedTokenId);
    }
    ...
    function ownerOf(uint256 tokenId)
        ...
        returns (address)
    {
        (uint256 originalId, uint256 hashedTokenId) = _getOriginalAndHashedId(
            tokenId
        );
        return super.ownerOf(hashedTokenId);
    }
    ...
    function _refreshToken(uint256 originalTokenId)
        private
        returns (uint256 newHashedTokenId)
    {
        uint256 hashedTokenId = hashedIdOf[originalTokenId];

        _burn(hashedTokenId);
        delete originalIdOf[hashedTokenId];

        newHashedTokenId = _hashId(originalTokenId);
        hashedIdOf[originalTokenId] = newHashedTokenId;
        originalIdOf[newHashedTokenId] = originalTokenId;

        _mint(msg.sender, newHashedTokenId);
    }

    function _hashId(uint256 tokenId) private view returns (uint256) {
        return
            uint256(
                keccak256(abi.encodePacked(tokenId, block.number, tx.origin))
            );
    }

    function _getOriginalAndHashedId(uint256 tokenId)
        private
        view
        returns (uint256 originalId, uint256 hashedId)
    {
        //TODO: code issue here
        if (tokenId < uint256(MAX_SUPPLY)) {
            hashedId = hashedIdOf[tokenId];
            originalId = tokenId;
        } else {
            hashedId = tokenId;
