...
contract Pair is IPair {
    ...
    function swap(
        ...
        require(amount0In > 0 || amount1In > 0, "IIA"); // Pair: INSUFFICIENT_INPUT_AMOUNT
        {
            ...
            (address _token0, address _token1) = (token0, token1);
            if (amount0In > 0) _update0(PairFactory(factory).getFeeAmount(stable, amount0In)); // accrue fees for token0 and move them out of pool
            if (amount1In > 0) _update1(PairFactory(factory).getFeeAmount(stable, amount1In)); // accrue fees for token1 and move them out of pool
    ...
    function getAmountOut(uint256 amountIn, address tokenIn) external view returns (uint256) {
        (uint256 _reserve0, uint256 _reserve1) = (reserve0, reserve1);
        amountIn -= PairFactory(factory).getFeeAmount(stable, amountIn); // remove fee from amount received
...
contract PairFactory is IPairFactory {
    ...
    uint256 public stableFee;
    uint256 public volatileFee;
    uint256 public MAX_REFERRAL_FEE = 0.12e18; // 12%
    uint256 public constant MAX_FEE = 0.0025e18; // 0.25%
    ...
    constructor() {
        ...
        isPaused = false;
        feeManager = msg.sender;
        stableFee = 0.0004e18; // 0.04%
        volatileFee = 0.0018e18; // 0.18%
    }
    ...
    function getFeeAmount(bool _stable, uint256 _forAmount) public view returns (uint256) {
        return (_forAmount * getFee(_stable)) / FEE_PRECISION;
    }
