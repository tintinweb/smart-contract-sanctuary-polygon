...
abstract contract ERC2981 is IERC2981, ERC165 {
    ...
    function _setTokenRoyalty(
        ...
        require(
            ...
            "royality must be less than max limit"
        );
        require(
...
contract Authorizable is Ownable {
    ...
    modifier onlyAuthorized() {
        require(
            ...
            "Cow Authorizable: caller is not the SuperAdmin or Admin"
        );
        _;
    }
    ...
    function addAuthorized(address _toAdd) external onlyOwner {
        require(
            _toAdd != address(0),
            "Cow Authorizable: _toAdd isn't vaild address"
        );
        require(
            !authorized[_toAdd],
            "Cow Authorizable: _toAdd is already added"
        );
        ...
        onlyOwner
    {
        require(
            _toRemove != address(0),
            "Cow Authorizable: _toRemove isn't vaild address"
        );
        require(
            adminList[_index] == _toRemove,
            "Cow Authorizable: _index isn't valid index"
        );
...
contract Collection is ERC2771Context, ERC721URIStorage, ERC2981, Whitelist {
    ...
    event TokenOwnerShipTransffered(
        address _from,
        address _to,
        uint256 tokenId
    );
    ...
    modifier onlyInitialOwner(uint256 _tokenId) {
        require(
            ...
            "only initial owner can set the royality"
        );
        _;
    }
    ...
    ) public onlyInitialOwner(_tokenId) {
    ...
    function changeLimitOfRoyality(uint256 _limit) external onlyOwner {
        ...
        require(_limit != maxRoyalty, "limit must be different");
        maxRoyalty = _limit;
    }
    ...
    function batchMint(
        ...
        require(_numberOfToken == _tokenURI.length,"Collection: _tokenURI lenght is not valid");
