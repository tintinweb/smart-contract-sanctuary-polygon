...
contract VoyagerV1 is VRFConsumerBaseV2,RaritySigner,Ownable{
    ...
    mapping(uint=>requestInfo) public requestToVoyage;
    ...
    constructor(address _puff,address _nexus,address _grav,uint64 subscriptionId) 
    ...
    function startVoyage(uint[][] memory tokenIds,uint[] memory price) external {
        ...
        require(NEXUS.transferFrom(msg.sender,address(this),amount),"xGrav transfer failed");
    }
    ...
    function endVoyage(uint[] memory voyageIds) external {
        ...
        for(uint i=0;i<length;i++){
            ...
            requestToVoyage[requestId] = requestInfo(voyageIds[i],_bonus,msg.sender);            
        }
    ...
    uint256[] memory randomWords
        ...
        requestInfo storage info = requestToVoyage[requestId];
        tokenInfo storage currToken = stakeInfo[info.user][info.voyageId];
        uint length = currToken.tokens.length;
        uint Grav;
        uint Nexus;
        if (randomWords[0] % 100 < voyageSuccess + info.rarityBonus) {
                Grav += currToken.amount * length * 1 ether;
                result[info.user][info.voyageId] = resultInfo(currToken.tokens,currToken.amount,true);
                emit Result(info.user, info.voyageId, true, currToken.amount);
            }
        else{
            Nexus += currToken.amount * length * 1 ether;
            result[info.user][info.voyageId] = resultInfo(currToken.tokens,currToken.amount,false);
            emit Result(info.user, info.voyageId, false, currToken.amount);
        }
        popSlot(info.user, info.voyageId);
        userEnded[info.user].push(info.voyageId);
        delete stakeInfo[info.user][info.voyageId];
        GRAV.transfer(info.user,Grav);
        NEXUS.transfer(info.user,Nexus);
    }
}
