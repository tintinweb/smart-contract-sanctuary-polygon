...
contract CRM is ICRM, CRMStorage {
  ...
  function initialize(
    ...
    ICRM[] memory collateralRatioModels = new ICRM[](assets.length);
    for (uint256 i = 0; i < assets.length; i++) {
            collateralRatioModels[i] = ICRM(this);
    }
    _setCollateralRatioModels(assets, collateralRatioModels);
    _setAbsMaxLtvRatios(assets, absMaxLtvRatios);
  ...
  function getCurrentMaxLtvRatios(address[] calldata assets)
    ...
    function _setAbsMaxLtvRatios(
        ...
        require(
            assets.length == maxLtvRatios.length,
            "ERROR: Length mismatch between 'assets' and 'assetLtvRatios'"
        );
        for (uint256 i = 0; i < assets.length; i++) {
            absMaxLtvRatios[assets[i]] = maxLtvRatios[i];
            emit AssetLtvRatioUpdated(assets[i], maxLtvRatios[i]);
        }
    }
    ...
    function _setCollateralRatioModels(
       ...
       ICRM[] memory collateralModels
        ...
        require(
            assets.length == collateralModels.length,
            "ERROR: Length mismatch between 'assets' and 'assetLtvRatios'"
        );
        for (uint256 i = 0; i < assets.length; i++) {
            collateralRatioModels[assets[i]] = collateralModels[i];
            emit CollateralRatioModelUpdated(assets[i], address(collateralModels[i]));
        }
    }
}
