...
contract Marketplace1155_V11 is 
  Initializable, 
  OwnableUpgradeable, 
  ...
  UUPSUpgradeable 
{
    ...
    whenNotPaused
  {
    ...
    require(isNFTOwner(marketItem.nftContract, marketItem.seller) || (marketItem.nftContract == PUBLIRARE_STORE), "Sorry, it looks like you are not the current owner of the NFT contract. Please cancel your offer and try again.");
  ...
  {
    MarketItem storage marketItem = idToMarketItem[itemID];
    require(marketItem.seller == msg.sender || (isNFTOwner(marketItem.nftContract, msg.sender) && (marketItem.nftContract != PUBLIRARE_STORE)), "Only current seller or contract owner can cancel the listing");
    ...
    whenNotPaused
  {
    ...
    if (amount > 0) {
      ...
      IERC1155Upgradeable(marketItem.nftContract).safeTransferFrom(address(this), msg.sender, marketItem.tokenID, returnedAmount, "");
      emit ResellOfferCancelled(itemID, resellID, offer.seller, offer.amountToSell, offer.status);
    }
  ...
  //! =============== FETCH NFT SINGLE RESELL OFFER ============================================

  function getNFTResellOffer(uint256 itemID, uint256 resellID)
    public view
