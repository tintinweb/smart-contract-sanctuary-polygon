...
library ProfileTokenURILogic {
    ...
    function _shouldUseCustomPicture(string memory imageURI) internal pure returns (bool) {
        ...
        if (imageURIBytes.length < 22) {
            // data:image/<image-format>;base64, with minimum <image-format> which is 3
            return false;
        }
        bytes32 beginningHash;
        assembly {
            // keccak256 of first 11 characters on imageURI
            beginningHash := keccak256(imageURIBytes, 11)
        }
        // if (beginningHash == keccak256('data:image/'))
        if (beginningHash != 0x67057e31e8ff5f1a1b8fe0051367bb8fd7d8e96705b608681b4df73983748716) {
            return false;
        }
        for (uint256 i = 12; i < imageURIBytes.length; i++) {
            if (imageURIBytes[i] == '"') {
                // imageURI could contain
                return false;
            }
        }
        return true;
