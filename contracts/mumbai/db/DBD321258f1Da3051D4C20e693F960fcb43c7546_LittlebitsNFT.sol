...
contract LittlebitsNFT is VRFConsumerBaseV2, ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    function ADMIN_registerCharacters2(Character[] memory newCharacters) public onlyOwner {
        for (uint i = 0; i < newCharacters.length; i++) {
            _unresolvedCharacters.push(newCharacters[i]);
        }
    }

    function ADMIN_registerCharacters3(Character[] memory newCharacters) public onlyOwner {
        uint len = newCharacters.length;
