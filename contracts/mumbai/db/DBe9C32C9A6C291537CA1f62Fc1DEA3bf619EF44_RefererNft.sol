...
contract RefererNft is ERC721, AccessControl,ReentrancyGuard,IEvents {
    using Counters for Counters.Counter;
    uint public price= 25 * 10**18;
    IERC20 public token;
    IAdmin public Admin;
    uint256 public expiryDate = 30 days;
    uint[4] public refferrerPercentage = [10,40,30,20];
    ...
    constructor(IERC20 _token, IAdmin _admin) ERC721 ("referral_nft", "nft") {
        token = IERC20(_token);
        Admin = IAdmin(_admin); 
...
/**
@dev this function mints NFT to the user
@param to to address
@param counter token ID
 */
    function mint(address to, uint counter) internal {
        counter = 1;
        counter = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        _mint(to, counter);
        users[to].tokenId = counter;
    }
/**
@dev this function buys refferal NFT in exchange for BUSD
@param user address of caller
@param counter token ID
 */
...
@param amount reward distribution ratio
 */
    function Reward_Percentage( address user,  uint128[4] memory amount) public nonReentrant _isAdmin {
        refferrerPercentage = amount;
 ...
 */
    function rewardDistribution(address user, uint _reward) public nonReentrant onlyRole(REWARD_ROLE) returns(uint amount, uint leftAmount) {   
        uint totalRewardPercentage = refferrerPercentage[0] + refferrerPercentage[1] + refferrerPercentage[2] + refferrerPercentage[3];
        address userRefferer = users[user].refferer;
        // users[user].totalReferralReward = (_reward * refferrerPercentage[0])/totalRewardPercentage;
        IERC20(Admin.BUSD()).transfer(user,(_reward * refferrerPercentage[0])/totalRewardPercentage);
        leftAmount = _reward - users[user].totalReferralReward;
        for(uint i = 1; i <= 3; i++){
            if(userRefferer != address(0)){
                if(users[userRefferer].totalReferralReward == 0){
                    users[userRefferer].lockPeriod = block.timestamp;
                }else if(users[userRefferer].lockPeriod + expiryDate > block.timestamp){
                    token.transfer(Admin.Treasury() , users[userRefferer].totalReferralReward/2); //price as variable
                    token.transfer(Admin.TeamAddress() , users[userRefferer].totalReferralReward/2); //price as variable
                }
                users[userRefferer].totalReferralReward += (_reward * refferrerPercentage[i])/totalRewardPercentage;       
                leftAmount -= (_reward * refferrerPercentage[i])/totalRewardPercentage;
                userRefferer = users[userRefferer].refferer;
            }else{
                break;
            }
        }
        emit RewardDistribute("ReferralNFT", user, _reward, totalRewardPercentage, userRefferer, leftAmount, block.number, block.timestamp);
 ...
 */
    function claimReward() public nonReentrant{
        // require(users[msg.sender].lockPeriod + expiryDate > block.timestamp, "ReferralNft: lockPeriod time of user is less than the current time");
        // if(users[msg.sender].totalReferralReward != 0) {
        //     token.transfer(msg.sender, users[msg.sender].totalReferralReward);
        // }
        ...
        if(users[msg.sender].lockPeriod + expiryDate > block.timestamp){
            token.transfer(Admin.Treasury() , users[msg.sender].totalReferralReward/2); //price as variable
            token.transfer(Admin.TeamAddress() , users[msg.sender].totalReferralReward/2); //price as variable
        }else{
            if(users[msg.sender].totalReferralReward <= price){
                token.transfer(Admin.Treasury() , users[msg.sender].totalReferralReward/2); //price as variable
                token.transfer(Admin.TeamAddress() , users[msg.sender].totalReferralReward/2); //price as variable
            }else{
                token.transfer(Admin.Treasury() , price/2); //price as variable
                token.transfer(Admin.TeamAddress() , price/2); //price as variable
                token.transfer(msg.sender, users[msg.sender].totalReferralReward - price);
            }
        }
    }
/**
@dev this function is used to set price
 */
    function setPrice(uint _price)public _isAdmin {
        require(_price!=0,"ReferralNFT:price cannot be zero");
        emit setterForUint("ReferralNFT",address(this), price, _price,block.number,block.timestamp);
        price=_price;
    }

    function setRewardRole(address rewardSetter) public _isAdmin{
        require (rewardSetter != address(0),"RefferalNFT: Invalid Address");  
        _setupRole(REWARD_ROLE, rewardSetter);
    }

    // function buy(address user, uint counter) public nonReentrant{
    //     require(users[user].tokenId == 0,'ReferralNft: User have already bought NFT');
    //     token.transferFrom(user, Admin.Treasury() , price/2); //price as variable
    //     token.transferFrom(user, Admin.TeamAddress() , price/2); //price as variable
    //     mint(user, counter);
    //     users[user].tokenId = counter;
    //     users[user].lockPeriod = uint128(block.timestamp + expiryDate);
    //     emit Buy("ReferralNft", counter, users[user].lockPeriod, block.number, block.timestamp);
    // }
/**
@dev this function reactivates NFT 
 */
    // function reActivateNFT() public {
    //     // require(ownerOf() == msg.sender); // check token id  is equal to 0 aur not
    //     require(users[msg.sender].lockPeriod > block.timestamp,"The NFT is still Active");
    //     token.transferFrom(msg.sender, Admin.Treasury() , price/2); //price as variable
    //     token.transferFrom(msg.sender, Admin.TeamAddress() , price/2); //price as variable
    //     users[msg.sender].lockPeriod = uint128 (block.timestamp + expiryDate);
    //     emit ReactivateNFT("ReferralNft", msg.sender, users[msg.sender].tokenId, block.number, block.timestamp);
    // }
 ...
 */
    function setRefferer(address referrer) public {
        ...
        require(users[msg.sender].refferer ==  address(0), "ReferralNft: already referred");
        emit setterForRefferer("ReferralNft",address(this),users[msg.sender].refferer ,referrer, msg.sender,block.number,block.timestamp);
        users[msg.sender].refferer = referrer;
    }
 ...
 */
    function getRefferer(address from) public view returns(address) {
        return users[from].refferer;
    }
/**
@dev this function checks if user has an activeNFT or not
@param user address of user
 */
    function isactive(address user) public view returns(bool) {
        if  (users[user].lockPeriod > block.timestamp){
            return true;
        }
        else {
            return false;
