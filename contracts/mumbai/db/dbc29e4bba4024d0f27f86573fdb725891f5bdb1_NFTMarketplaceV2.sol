...
contract NFTMarketplaceV2 is INFTMarketplaceV2, NFTMarketplace, PausableUpgradeable, EIP712 {
    ...
    function matchOrders(
        ...
        if (canceledBids[LibOrder.keyHash(takerOrder)]) {
            revert InvalidOrder("Taker canceled");
        }

        if (canceledBids[LibOrder.keyHash(makerOrder)]) {
            revert InvalidOrder("Maker canceled");
        }
        ...
        if (
            ...
            revert InvalidOrder("Not supported");
        }
        ...
        if (
            ...
            revert InvalidOrder("Deadline passed");
        }
    ...
    function _transferERC20Tokens(
        ...
        if (makerTakeAssetAddress != tokenAddress) {
            revert InvalidOrder("Assets don't match");
        }

        if (makerTakeAssetAmount > takerPrice) {
            revert InvalidOrder("Invalid ERC20 amount");
        }
...
interface INFTMarketplaceV2 is INFTMarketplace {
    ...
    event Match(address indexed nftAddress, uint indexed tokenId, address indexed buyer);
