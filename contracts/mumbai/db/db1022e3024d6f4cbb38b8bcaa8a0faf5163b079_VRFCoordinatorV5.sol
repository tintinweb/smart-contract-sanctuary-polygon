...
// File contracts/interfaces/VRFCoordinatorV4Interface.sol




interface VRFCoordinatorV4Interface {
  /**
  ...
  function getSubscription(
    uint64 subId
  ) external view returns (uint256 balance, uint64 reqCount, address owner, address[] memory consumers);
  ...
  /**
   * @notice Fund a subscription
   * @param subId - ID of the subscription
   * @param amount - Where to send the remaining LINK to
   */
  function fundSubscription(uint64 subId, uint256 amount) external;

  /**
   * @notice Fund a subscription by native
   * @param subId - ID of the subscription
   */
...
// File contracts/interfaces/WrappedNativeInterface.sol
...
// File contracts/VRFCoordinatorV5.sol
...
contract VRFCoordinatorV5 is VRF, ConfirmedOwner, TypeAndVersionInterface, VRFCoordinatorV4Interface {
  ...
  struct Subscription {
    ...
    uint256 balance; // Common link balance used for all consumer requests.
  ...
  mapping(address => uint256) /* oracle */ /* WKCS balance */ private s_withdrawableTokens;
   ...
   * @param fulfillmentFlatFeePPM is the flat fee in millionths of WKCS that VRFCoordinatorV2
   */
  function setConfig(
    uint16 minimumRequestConfirmations,
    uint32 maxGasLimit,
    ...
    s_config = Config({
      ...
      isOn: isOn,
      fulfillmentFlatFeePPM: fulfillmentFlatFeePPM,
      reentrancyLock: false
    });
    emit ConfigSet(minimumRequestConfirmations, maxGasLimit, gasAfterPaymentCalculation, isOn, fulfillmentFlatFeePPM);
  }
  ...
  /**
   * @inheritdoc VRFCoordinatorV4Interface
   */
  ...
  function fulfillRandomWords(Proof memory proof, RequestCommitment memory rc) external nonReentrant returns (uint256) {
    ...
    // Its specified in millionths of link, if s_config.fulfillmentFlatFeePPM = 1
  ...
  function oracleWithdraw(address recipient, uint256 amount) external nonReentrant {
    ...
    //    if (!WRAPPER_TOKEN.transfer(recipient, amount)) {
    //      revert InsufficientBalance();
    //    }
  }
  ...
  function fundSubscriptionInternal(uint64 _subId, uint256 _amount, bool isNative) internal nonReentrant {
    ...
    if (isNative) {
      ...
      // Todo only wrapper native
      // IERC20(address(WRAPPER_TOKEN)).safeTransferFrom(msg.sender, address(this), _amount);
      WRAPPER_TOKEN.transferFrom(msg.sender, address(this), _amount);
      //    bool isSuccess = WRAPPER_TOKEN.transferFrom(msg.sender, address(this), _amount);
      //      if (!isSuccess) {
      //        revert InsufficientBalance();
      //      }
    }
  ...
  function getSubscription(
    uint64 subId
  ) external view override returns (uint256 balance, uint64 reqCount, address owner, address[] memory consumers) {
    if (s_subscriptionConfigs[subId].owner == address(0)) {
      revert InvalidSubscription();
    }
    return (
  ...
  function cancelSubscriptionHelper(uint64 subId, address to) private nonReentrant {
    ...
    //    if (!WRAPPER_TOKEN.transfer(to, balance)) {
    //      revert InsufficientBalance();
    //    }
