...
contract Vesting is Ownable {
    ...
    struct Round {
        string name;
        uint256 lockPeriod;
        uint256 amountOnUnlockX10000;
        uint256 cliffPeriod;
        uint256 linearUnit;
        uint256 amountPerUnitX10000;
    }
    // X10000 => 2_50 -> 2.5%; 250 / 10000 = 0.025 (2.5%)
    ...
    event Started(uint256 indexed timestamp, address indexed user);
    event Claim(
    ...
    function initRounds(Round[] memory rounds_) external onlyOwner {
        require(!isInitRound);

        uint256 length = rounds_.length;
    ...
    /// @dev strats vesting (only owner)
    function start() external onlyOwner {
        require(startTimestamp > block.timestamp, 'Already started');
        startTimestamp = block.timestamp;
        emit Started(block.timestamp, msg.sender);
    }
    ...
    function claim(
        ...
        for (uint256 i; i < length; ++i) {
            uint256 amount = unclaimed(msg.sender, i, allocations[i]) -
                claimed(msg.sender, i, allocations[i]);

            if (totalAmount + amount > targetAmount) amount = targetAmount - totalAmount;

            _updateClaimed(msg.sender, i, amount);
            totalAmount += amount;
            emit Claim(block.timestamp, msg.sender, rounds[i].name, amount);

            if (totalAmount == targetAmount) break;
        }
        if (totalAmount == 0) revert ZeroClaim();
    ...
    ///
    /// @return amount amount of unlclaimed tokens
    function unclaimed(
        address, // user
        uint256 id,
        uint256 allocation
        ...
        // lock period
        if (startTimestamp + round.lockPeriod > block.timestamp) return totalAmount;
        // on unclock
        totalAmount += (allocation * round.amountOnUnlockX10000) / 10000;
        // cliff period
        if (startTimestamp + round.lockPeriod + round.cliffPeriod > block.timestamp)
            return totalAmount;

        uint256 period = block.timestamp -
            (startTimestamp + round.lockPeriod + round.cliffPeriod);
        uint256 units = period / round.linearUnit;

        totalAmount += (allocation * round.amountPerUnitX10000 * units) / 10000;

        return totalAmount <= allocation ? totalAmount : allocation;
    }
