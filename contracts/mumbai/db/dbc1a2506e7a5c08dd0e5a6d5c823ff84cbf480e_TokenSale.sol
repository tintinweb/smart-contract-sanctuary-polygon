...
import "./ERC1155NoTransfer.sol";
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/utils/Strings.sol";
import "@openzeppelin/contracts/security/Pausable.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import "./PolyYetiGame.sol";
import "./YetiL2.sol";
// import "./TestPolyYetiGame.sol";
// import "./TestYeti.sol";


contract TokenSale is ERC1155NoTransfer, Ownable, Pausable {
    ...
    PolyYetiGame game;
    L2Yeti yeti;
    //TestPolyYetiGame testgame;
    //TestYeti testyeti;

    bool public burnActive = false;

    //Mumbai
    address public yetiAdr = address(0xbF438d831a9fE98D92C005a83Ba169b7B5Ca8bea);
    address public frxstAdr = address(0xAd7153e9B8C227ec8311Ba781cAA6Edb623C7475);
    address public proxyAdr = address(0xAe88b6df8dc8da94541953f481Ef7e0AA7863be6);
    address public wethAdr;

    address payable private devAdress;

    mapping(uint => uint) public TotalSupplyPerToken;
    mapping(uint => uint) public CurrentSupplyPerToken;
    mapping(uint => uint) public EthPricePerToken;
    mapping(uint => uint) public FrxstPricePerToken;
    mapping(address => uint) public highestLevel;
    mapping(address => uint) public highestGrade;
    mapping(uint => bool) public HolderOnlyToken;
    mapping(uint => bool) public SaleActive;
    string baseUri = "";
    string public baseExtension = ".json";

    event FrxstPurchase(address, uint, uint, uint, uint);
    event WethPurchase(address, uint, uint, uint, uint);
    event NewRefFrxst(address);
    event NewRefWeth(address);
    event NewRefGame(address);
    event NewYeti(address);
    event BurnActivated(address, uint);
    event SaleActivated(address, uint);
    event Withdraw(address, uint);

    constructor(address payable _dev) ERC1155NoTransfer("") {
        devAdress = _dev;   
        _SetInitialMappings();
        CurrentSupplyPerToken[0] += 10;
        _mint(devAdress, 0, 10, "");
        frxst = IERC20(frxstAdr);
        weth = IERC20(wethAdr);
        game = PolyYetiGame(proxyAdr);
        yeti = L2Yeti(yetiAdr);
        //testgame = TestPolyYetiGame(proxyAdr);
        //testyeti = TestYeti(yetiAdr);
    }

    function _SetInitialMappings() internal {

        TotalSupplyPerToken[0] = 300; //LandS
        TotalSupplyPerToken[1] = 500; //LandA
        TotalSupplyPerToken[2] = 1000; //LandB
        TotalSupplyPerToken[3] = 300; //AssetS
        TotalSupplyPerToken[4] = 500; //AssetA
        TotalSupplyPerToken[5] = 1000; //AssetB
        TotalSupplyPerToken[6] = 4444; //VoxelRedeem
        TotalSupplyPerToken[7] = 111; //VoxelSuperRare
        
        EthPricePerToken[0] = 50 * 1e16; //LandS
        EthPricePerToken[1] = 30 * 1e16; //LandA
        EthPricePerToken[2] = 20 * 1e16; //LandB
        EthPricePerToken[3] = 50 * 1e16; //AssetS
        EthPricePerToken[4] = 30 * 1e16; //AssetA
        EthPricePerToken[5] = 20 * 1e16; //AssetB
        EthPricePerToken[6] = 2 * 1e16; //VoxelRedeem
        EthPricePerToken[7] = 10 * 1e16; //VoxelSuperRare

        FrxstPricePerToken[0] = 500000 * 1e18; //LandS
        FrxstPricePerToken[1] = 300000 * 1e18; //LandA
        FrxstPricePerToken[2] = 200000 * 1e18; //LandB
        FrxstPricePerToken[3] = 500000 * 1e18; //AssetS
        FrxstPricePerToken[4] = 300000 * 1e18; //AssetA
        FrxstPricePerToken[5] = 200000 * 1e18; //AssetB 
        FrxstPricePerToken[6] = 0; //VoxelRedeem
        FrxstPricePerToken[7] = 0; //VoxelSuperRare

        SaleActive[0] = true; //LandS
        SaleActive[1] = true; //LandA
        SaleActive[2] = true; //LandB
        SaleActive[3] = true; //AssetS
        SaleActive[4] = true; //AssetA
        SaleActive[5] = true; //AssetB 
        SaleActive[6] = true; //VoxelRedeem
        SaleActive[7] = false; //VoxelSuperRare
    }

    modifier PreReq(uint _tokenId) {
        require(yeti.viewTotalTokensOfOwner(msg.sender).length > 0, "You need to own a yeti");
        require(SaleActive[_tokenId], "Token sale not active");
        _;
        // require(game.getStakedTokens(msg.sender).length > 0, "You need a staked yeti");
        // _;
    }

    // Purchase
    function BuyTokenForWeth(uint _nftId, uint _quantity) external PreReq(_nftId) whenNotPaused() {
        require(CurrentSupplyPerToken[_nftId] + _quantity <= TotalSupplyPerToken[_nftId], "Token sold out");
        if (_nftId == 6) {
            uint[] memory l = yeti.viewTotalTokensOfOwner(msg.sender);
            require(balanceOf(msg.sender, _nftId) + _quantity <= l.length, "Need to own more yetis");
        }
        CurrentSupplyPerToken[_nftId] += _quantity;
        uint price =  GetFinalPrice(_nftId, false) * _quantity;
        require(weth.balanceOf(msg.sender) > price, "Insufficient Weth");
        bool succ = weth.transferFrom(msg.sender, address(this), price);
        require(succ, 'transfer failed');
        _mint(msg.sender, _nftId, _quantity, "");

        emit WethPurchase(msg.sender, _nftId, block.timestamp, _quantity, price);
    }


    function BuyTokenForFrxst(uint _nftId, uint _quantity) external PreReq(_nftId) whenNotPaused() {
        require(CurrentSupplyPerToken[_nftId] + _quantity <= TotalSupplyPerToken[_nftId], "Token sold out");
        require(FrxstPricePerToken[_nftId] != 0, "Only purchased by Weth");
        
        CurrentSupplyPerToken[_nftId] += _quantity;
        uint price = GetFinalPrice(_nftId, true) * _quantity;
        require(frxst.balanceOf(msg.sender) > price, "Insufficient Frxst");
        bool succ = frxst.transferFrom(msg.sender, address(this), price);
        require(succ, 'transfer failed');
        _mint(msg.sender, _nftId, _quantity, "");

        emit FrxstPurchase(msg.sender, _nftId, block.timestamp, _quantity, price);
    }

    function isSoldOut(uint _nftId) public view returns (bool) {
        return CurrentSupplyPerToken[_nftId] == TotalSupplyPerToken[_nftId];
    }

    function getHighestGrade(uint[] memory _tokenIds) public view returns (uint) {
        uint high = 0;

        for (uint i = 0; i < _tokenIds.length; i++) {
            if (game.tokenRarity(_tokenIds[i]) == 2) {
                return 2;
            }
            if (game.tokenRarity(_tokenIds[i]) > high) {
                high = game.tokenRarity(_tokenIds[i]);
            }    
        }
        return high;
    }

    function _getDiscountedPriceMultiplier(uint _nftID) public view returns (uint) {
        // No discount for normal Voxels
        if (_nftID == 6) {
            return 100;
        } else {
            if (getHighestGrade(yeti.viewTotalTokensOfOwner(msg.sender)) == 2) {
                return 50;
            } else if (getHighestGrade(yeti.viewTotalTokensOfOwner(msg.sender)) == 1) {
                return 70;
            } else {
                return 80;
            }
    ...
    function GetFinalPrice(uint _nftID, bool isFrxst) public view returns(uint) {
        if (isFrxst) {
            return FrxstPricePerToken[_nftID] * _getDiscountedPriceMultiplier(_nftID) / 100;
        } else {
            return EthPricePerToken[_nftID] * _getDiscountedPriceMultiplier(_nftID) / 100;
        }
    }

    function tierURI(uint256 _nftID) public view virtual returns (string memory) {
        return string(abi.encodePacked(baseUri, _nftID.toString(), baseExtension));
    } 
    ...
    function setFrxstContract(address _frxstRef) public onlyOwner() {
        frxstAdr = _frxstRef;
       frxst = IERC20(_frxstRef);
       emit NewRefFrxst(_frxstRef);
    }

    function setWethContract(address _wethRef) public onlyOwner() {
        wethAdr = _wethRef;
       weth = IERC20(_wethRef);
       emit NewRefWeth(_wethRef);
    }

    function setGameContract(address _proxyAdr) public onlyOwner() {
        proxyAdr = _proxyAdr;
       game = PolyYetiGame(_proxyAdr);
       emit NewRefGame(_proxyAdr);
    }

    function setYetiContract(address _yetiAdr) public onlyOwner() {
        _yetiAdr = _yetiAdr;
       yeti = L2Yeti(_yetiAdr);
       emit NewYeti(_yetiAdr);
    }

    // function setTestGameContract(address _proxyAdr) public onlyOwner() {
    //     proxyAdr = _proxyAdr;
    //    testgame = TestPolyYetiGame(_proxyAdr);
    // }

    // This function can be used in future to burn tokens to get the real ERC721s
    function burn(uint256 id, uint256 amount) external {
        require(burnActive, "Burn  not active");
        _burn(msg.sender, id, amount);
    }

    function activateBurn(bool _burnActive) external onlyOwner() {
        burnActive = _burnActive;
        emit BurnActivated(msg.sender, block.timestamp);
    }

    function activateSale(uint _tokenId, bool _state) external onlyOwner() {
        SaleActive[_tokenId] = _state;
        emit SaleActivated(msg.sender, block.timestamp);
    }

    function withdraw() public onlyOwner() {
        bool a =  weth.transfer(devAdress, weth.balanceOf(address(this)) * 5 / 100);
        require(a, "Weth to Dev failed");
        bool b =  frxst.transfer(devAdress, frxst.balanceOf(address(this)) * 5 / 100);
        require(b, "Frxst to Dev failed");
        bool c = weth.transfer(owner(), weth.balanceOf(address(this)));
        require(c, "Weth to Owner failed");
        bool d = frxst.transfer(owner(), frxst.balanceOf(address(this)));
        require(d, "Frxst to Owner failed");
 ...
 */
contract ERC1155NoTransfer is Context, ERC165, IERC1155, IERC1155MetadataURI {
    ...
    function setApprovalForAll(address operator, bool approved) public virtual override {
        require(true == false, "This token is untransferable");
...
contract PolyYetiGame is OwnableUpgradeable, YetiSigner, ReentrancyGuardUpgradeable, signerChecker {
    ...
    public  initializer
    {
        ...
        rates = [
        [50, 60, 4], //25, 30, 4        //12, 15, 2         //6, 7, 1
        [90, 135, 10], //45, 62, 10     //22, 31, 5         //11, 15, 2
        [130, 195, 26] //65, 92, 26     //32, 46, 13        //16, 23, 6
        ];
        exprates = [
        [80,100,120],
        [96,120,144],
...
import "hardhat/console.sol";
contract L2Yeti is Ownable, ERC1155Burnable {
    ...
    modifier Controller(address _caller){
        require(isController[_caller], "Not controller");
        _;
    }
    ...
    function mintNewTokens(uint _tokenId, address _to) external Controller(_msgSender()){
        _mint(_to, _tokenId, 1, "");
    }

    function burnToken(uint _tokenId, address _from) external Controller(_msgSender()){
        _burn(_from, _tokenId, 1);
    }

    function viewOwnerOfToken(uint _tokenId) external view returns(address){
        return ownerOfToken[_tokenId];
    }

    function viewTotalTokensOfOwner(address _user) external view returns (uint[] memory) {
        return tokensOfOwner[_user];
    }

    function popToken(uint tokenId,address _from) private {
        uint lastToken = tokensOfOwner[_from][tokensOfOwner[_from].length-1];
        uint currPosition = tokenToPosition[tokenId];
        tokensOfOwner[_from][currPosition] = lastToken;
        tokenToPosition[lastToken] = currPosition;
        tokensOfOwner[_from].pop();
    }
    ...
    function _beforeTokenTransfer(
        ...
        if(!isController[to]){
            for(uint i = 0; i < length; i++) {
                    ...
                    if (from!=address(0))
                    popToken(ids[i],from);
                    tokenToPosition[ids[i]] = tokensOfOwner[to].length;
                    tokensOfOwner[to].push(ids[i]);
            }
        }
