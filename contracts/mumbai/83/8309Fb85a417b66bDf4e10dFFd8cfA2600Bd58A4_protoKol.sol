...
/**
  ...
  external{
    ...
    require(_kolData.kolWallet != address(0) && !blackListedKOL[msg.sender][_campaignId], "Protokol:KOL !registered");
    ...
    require(block.timestamp <= campaigns[_campaignId].campaignData.endDate, "Protokol:Campaign ended, can't invest");
  ...
  function generateTGE(uint256 _campaignId, address token) external
  {
    ...
    IPST(campaigns[_campaignId].campaignData.preSaleToken).transferFrom(msg.sender ,_vault,(markBudge *  _platformPer)/10000);
    IPST(campaigns[_campaignId].campaignData.preSaleToken).transferFrom(msg.sender ,address(this),campaigns[_campaignId].tgeDetails.tgeAmount);
    VestingDetails memory _vestingInfo;
  ...
  function claimPreSaleTokens(uint256 _campaignId, uint256 progress, bytes32 r,
    ...
    if(campaigns[_campaignId].vestingData.isVestingInEnabled){
      ...
      require(IPST(preSale).balanceOf(address(this)) >= _reward
      ...
      require(IPST(preSale).balanceOf(address(this)) >= 
              eligibleRwd
              , "Protokol:Not enough funds");
      IPST(preSale)
              .transfer(msg.sender
