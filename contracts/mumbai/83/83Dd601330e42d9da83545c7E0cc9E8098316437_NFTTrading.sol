...
contract NFTTrading {
    ...
    function listingFeesforMultiple() public payable {
        ...
        payable(feesAddress).transfer(tradingfees * 1 ether);
    }
    ...
    function tradeNFT(
        ...
        require(_nftContract.ownerOf(_tokenId) == msg.sender, "No NFT Owner");
        require(_tokenId > 0, "TokenId is not a zero");
        require(_price > 0, "price is not a zero");
        require(block.timestamp <= _releaseTime, "Release time is in the past");
        trades.push(
            TradeNFT(
                payable(msg.sender),
                payable(address(0)),
    ...
    function buyNFT(uint256 _tradeId) public payable {
        ...
        uint256 totalPrice = trade.price * 1 ether;
    ...
    function releaseNFT(uint256 _tradeId) public payable {
        require(_tradeId < trades.length, "Invalid Trade ID");
        TradeNFT storage trade = trades[_tradeId];
        require(msg.sender == trade.seller, "Unauthorized to release funds");
        require(!trade.release, "Funds have already been released");
        require(
    ...
    function claimNFTbyUser(uint256 _tradeId) public  {
        TradeNFT storage trade = trades[_tradeId];
        require(msg.sender == trade.buyer, "Unauthorized to claim funds");
        require(!trade.release, "Funds have already been released");
        require(
            block.timestamp >= trade.releaseTime,
            "Release time not reached"
