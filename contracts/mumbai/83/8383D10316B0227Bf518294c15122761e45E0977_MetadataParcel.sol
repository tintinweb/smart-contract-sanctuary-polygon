...
contract MetadataParcel is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        string[9] memory metadata;
        metadata[0] = string.concat('{"name": "', "Reality chain Parcel Metadata", '",');
        metadata[1] = string.concat('{"token ID": "', toString(tokenId), '",');
        metadata[2] = string.concat('"image_data": "', getSvg(), '",');
        metadata[3] = string.concat('"attributes": [{"trait_type": "Name Metaverse", "value": ', metadataMetaverse[tokenId].name, '},');
        metadata[4] = string.concat('{"trait_type": "Blockchain", "value": ', metadataMetaverse[tokenId].blockchainSelection, '},');
        metadata[5] = string.concat('{"trait_type": "Size", "value": ', metadataMetaverse[tokenId].size, '},');
        metadata[6] = string.concat('{"trait_type": "Type", "value": ', metadataMetaverse[tokenId]._type, '},');
        metadata[7] = string.concat('{"trait_type": "Map", "value": "', metadataMetaverse[tokenId].map, '"}');
        metadata[8] = ']}';

        string memory output = string.concat(metadata[0], metadata[1], metadata[2], metadata[3], metadata[4]);
        output = string.concat(output, metadata[5], metadata[6], metadata[7], metadata[8]);

        string memory json = Base64.encode(bytes(string(output)));
        return string(abi.encodePacked('data:application/json;base64,', json));
    }

    function mint(
        uint256 tokenId, 
        string memory nameMetaverse,
        string memory blockchainSelection,
        string memory metaverseSize,
        string memory metaverseType,
        string memory metaverseMap) 
    public onlyOwner nonReentrant {
        _safeMint(msg.sender, tokenId);
        metadataMetaverse[tokenId] = Attr(nameMetaverse, blockchainSelection, metaverseSize, metaverseType, metaverseMap);
    }
}