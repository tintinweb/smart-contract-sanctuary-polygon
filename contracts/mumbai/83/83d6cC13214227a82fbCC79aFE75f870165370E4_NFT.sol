...
contract NFT is ERC721, Ownable, Withdrawable {
	...
	constructor() ERC721("Sabda Alam NFT", "BIRD") {
		baseTokenURI = "https://bafybeibjulli7enq5j3kgk3nduvwq6humz5xpizawbyc3guak2unrso4n4.ipfs.nftstorage.link/metadata/";
	}
	...
	/// @dev reserve some birds
    function reserveBirds() public onlyOwner {
		uint256 tokenId = currentTokenId.current();
		require(tokenId.add(50) <= MAX_SUPPLY, "Reserve would exceed max supply");
		require(!saleIsActive, "Sale must not be active to reserve");
        uint i;
        for (i = 0; i < 50; i++) {
			currentTokenId.increment();
			uint256 newItemId = currentTokenId.current();
	...
	function mintBird(uint numberOfTokens) public payable {
		...
		require(tokenId.add(numberOfTokens) <= MAX_SUPPLY, "Purchase would exceed max supply");
