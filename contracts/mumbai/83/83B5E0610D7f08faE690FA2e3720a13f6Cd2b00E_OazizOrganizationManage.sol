...
contract OazizOrganization is Ownable{
    ...
    function getOrganizationAddress() public view returns (address) {
...
contract OazizOrganizationManage is Ownable {
    ...
    function createOrganization(string memory orgName, address newOwner) external onlyOwner {
        ...
        OrgAddress[newOwner] = org.getOrganizationAddress();
    }
    ...
    function getData(bytes32[] memory dataKeys, address orgAddr) public view returns (string[] memory dataValues) {
        OazizOrganization org = OazizOrganization(orgAddr);
        return org.getData(dataKeys);
    }
}