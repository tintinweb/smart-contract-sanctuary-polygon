...
contract NFToken is Ownable, ERC721, SupportsInterface {
    ...
    function transferFrom(
        ...
        address _to,
        uint256 _tokenId
    ) external virtual override canTransfer(_tokenId) validNFToken(_tokenId) {
        address tokenOwner = idToOwner[_tokenId];
...
contract SkorLandNFT is NFTokenMetadata {
    ERC721 public crest;
    uint256 _tokenId;
    mapping(uint256 => uint256) public LandFromCrest;

    constructor(ERC721 _crest) {
        crest = _crest;
        nftName = "SkorLand";
        nftSymbol = "SLAND";
        _tokenId = 1;
    ...
    modifier isCrestContract() {
        require(address(crest) == msg.sender, "not crest contract");
        _;
    }
    ...
    function mintLand(
        ...
        string memory uri,
        uint256 tokenId
    ) public isCrestContract {
        super._mint(_to, tokenId);
        super._setTokenUri(tokenId, uri);
        LandFromCrest[tokenId] = tokenId;
        tokensowned[_to].push(tokenId);
        _tokenId += 1;
    }
    ...
    function transferFrom(
        ...
        address _to,
        uint256 tokenId
    ) public override isCrestContract {
        address Nftowner = ERC721(address(this)).ownerOf(tokenId);
        require(Nftowner == _from, "You are not the owner of NFT");
    ...
    function safeTransferFrom(
        ...
        address Nftowner = ERC721(address(this)).ownerOf(tokenId);
        require(Nftowner == _from, "You are not the owner of NFT");
        _safeTransferFrom(_from, _to, tokenId, data);
    }

    function setCrestAddress(ERC721 _crest) external onlyOwner {
        crest = _crest;
    }
