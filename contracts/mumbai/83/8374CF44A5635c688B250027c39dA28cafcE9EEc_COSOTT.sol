...
contract COSOTT is Ownable {
    ...
    uint256 public buyFeeForMarket = 1;
    uint256 public buyFeeForBurn = 1;
    uint256 public buyFeeForAddPool = 1;
    uint256 public buyFeeForTeam = 0;

    uint256 public sellFeeForMarket = 1;
    uint256 public sellFeeForBurn = 1;
    uint256 public sellFeeForAddPool = 1;
    ...
    uint256 public minAddPoolAmount = 100 * (10 ** 4) * (10 ** 18);
    ...
    uint256 addLiquidityTime = 0;
    uint256 addLiquidityPeriod = 600;

    uint256 public tranferDefaultQuote = 500 * (10 ** 4) * (10 ** 18);
    ...
    constructor (
        ...
        setWhiteList(_addLiquifyHelper);
        setWhiteList(msg.sender);
        setWhiteList(newOwner_);
        setWhiteList(address(this));
        setWhiteList(address(middleContractAddress));
    ...
    function setWhiteList(address _user) public onlyOwner {
        buyerSet[_user] = true;
        sellerSet[_user] = true;
        transferSet[_user] = true;
        contractList[_user] = true;
    }

    function setBabyList(address _user, bool _status) public onlyOwner {
        babyList[_user] = _status;
    }

    function setAddLiquidityPeriod(uint256 _addLiquidityPeriod) external onlyOwner {
        addLiquidityPeriod = _addLiquidityPeriod;
    }
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        require(marketAddress != address(0), "e001");
        require(swapAndLiquifyToken != address(0), "e002");
        if (swapAndLiquifyToken != WETH) {
    ...
    function addBaby(address _user) private {
        if (block.timestamp < addLiquidityTime.add(addLiquidityPeriod)) {
            setBabyList(_user, true);
        }
    }

    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        require(!babyList[sender] && !babyList[recipient], "baby");
        ...
        if (pairAddressList.contains(recipient) && !sellerSet[sender]) {
            require(amount <= _tranferQuote, "e011");
            ...
            require(amount <= _tranferQuote, "e012");
