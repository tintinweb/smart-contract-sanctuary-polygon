...
import "./IJamonRouter.sol";
import "./KeeperRegistryBaseInterface.sol";
import "./KeeperBase.sol";

/**
 * @title JamonSpotLimit
 ...
 * The accepted token to buy credits is LINK (Chainlink ERC 677), the amount of LINK is used to fund the keeper and execute the order, a percentage of that amount will be used to buy and burn JAMON.
 */
contract JamonSpotLimit is
    ...
    Ownable
{
    ...
    //---------- Contracts ----------//
    KeeperRegistryInterface private  keeperRegistery; //Keeper registry contract.
    IJamonRouter private immutable jamonRouter; // JamonSwap Router contract.
    IERC20 private immutable link; // LINK (Chainlink ERC 677) contract.

    //---------- Variables ----------//
    address public burnContract; // Address for buy and burn JAMON.
    uint256 public baseFee; // Price of credit.
    uint256 public burnFee; // Fee to buy and burn JAMON.
    ...
    //---------- Constructor ----------//
    constructor(address buyAndBurn) {
        keeperRegistery = KeeperRegistryInterface(
            0xE16Df59B887e3Caa439E0b29B42bA2e7976FD8b2
        );
        jamonRouter = IJamonRouter(0xdBe30E8742fBc44499EB31A19814429CECeFFaA0);
        link = IERC20(0x326C977E6efc84E512bB9C30f76E30c160eD06FB);
        burnContract = buyAndBurn;
        baseFee = 0.0020 ether;
        burnFee = 2500;
        creditTime = 7 days;
    }
    ...
    function _forceFail(
        ...
        //Interactions
        if (token == address(jamonRouter.WETH())) {
    ...
    function _performSwap(
        ...
        address WETH = jamonRouter.WETH();
        address tokenIn = path[0];
        address tokenOut = path[path.length - 1];
        if (tokenIn == WETH) {
            try
                jamonRouter.swapExactETHForTokens{value: amountIn}(
                    amountOutMin,
                    path,
                    user,
            ...
            try
                jamonRouter.swapExactTokensForETH(
                    amountIn,
                    amountOutMin,
                    path,
                    payable(user),
            ...
            try
                jamonRouter.swapExactTokensForTokens(
                    amountIn,
                    amountOutMin,
                    path,
                    user,
     ...
     */
    function getPrice(uint256 amountIn, address[] memory path)
        public
        view
        returns (bool, uint256)
    {
        try jamonRouter.getAmountsOut(amountIn, path) returns (
            uint256[] memory result
        ) {
    ...
    function creditPrice() public view returns (uint256) {
     ...
     function testCredits(uint256 amount) public view returns (uint256,uint256) {
         uint256 _creditPrice = creditPrice().mul(amount);
        uint256 toBurn = _creditPrice.mul(burnFee).div(10000);
        return (_creditPrice,toBurn);
    }
    ...
    function approveKeep() external {
        link.approve(address(keeperRegistery), ~uint256(0));
    }

    function addFundstoKeep(uint256 amount) external {
         uint256 _creditPrice = creditPrice().mul(amount);
        uint256 toBurn = _creditPrice.mul(burnFee).div(10000);
    ...
    /**
     * @notice Buy credits and fund keepers, also send a percentage to buy and burn JAMON.
        ...
        onlyOnKeeperActive
    {
        ...
        if (tokenIn == address(jamonRouter.WETH())) {
        ...
        if (token.allowance(address(this), address(jamonRouter)) == 0) {
            token.approve(address(jamonRouter), ~uint256(0));
        }
        if (tokenIn != address(jamonRouter.WETH())) {
    ...
    /**
     * @notice Set the base fee and the percentage to buy and burn JAMON.
