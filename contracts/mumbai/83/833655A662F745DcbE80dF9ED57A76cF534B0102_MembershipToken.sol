...
contract MembershipToken is ERC1155 {
    ...
    event mintedTokens(string retailerName, uint256 tokenId, uint256 amount, string metadata);
    ...
    function mintTokens(string memory _retailerName, uint256 amount, string memory _metadata) public onlyOwner{
        uint256 localTokenId = retailerNameToTokenId[_retailerName];
        _mint(owner, localTokenId, amount, bytes(_metadata));
        emit mintedTokens(_retailerName, localTokenId, amount, _metadata);
    }
}
