...
contract texasHoldEm {
    ...
    function deposit(uint256 _WETH) public {
        ...
        require(WETH(WETHAddress).transferFrom(msg.sender, address(this), _WETH));
    ...
    function withdraw(address _claimer, uint256 _BETCOIN) public onlyOwner {
        ...
        require(WETH(WETHAddress).transfer(owner, processFee));
        require(WETH(WETHAddress).transfer(_claimer, _claim - processFee));
    ...
    function withdrawList(address[] memory _claimers, uint256[] memory _BETCOINs) public onlyOwner {
        ...
        for(uint i = 0; i < _claimers.length; i++) {
            ...
            require(WETH(WETHAddress).transfer(_claimers[i], _claim - processFee));
            userClaimed[_claimers[i]] += _claim;
            _claimFees[i] = _claim - processFee;
            _totalProcessFee += processFee;
        }
        require(WETH(WETHAddress).transfer(owner, _totalProcessFee));
    ...
    function updateWETHAddress(address _newAddress) external onlyOwner {
