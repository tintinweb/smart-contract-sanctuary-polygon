...
contract tickets
{
    ...
    mapping(bytes32 => uint256)public hisByHash;
    function processTicket(address tokenAddress, uint256 tokenAmount,bytes32 hash)public
    {
	uint256 nn;
...
//	b = bytes(hash);
	...
	emit eLog(nn++,"hash",uint256(hash));
    ...
    function hash2(string memory str)public pure returns (uint256) 
    {
	return uint256(keccak256(abi.encode(str)));
    }
    function hash3(string memory str)public pure returns (bytes32) 
    {
	bytes memory b;
	b = bytes(str);
