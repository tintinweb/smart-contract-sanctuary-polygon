...
contract Installment is Ownable, AccessControl, ReentrancyGuard {
    ...
    */ 
    ...
    event OrderCreated(bytes32 orderId, address indexed seller, address nftAddress , uint256 price);
    event OrderPruchase(
    bytes32 billId, 
    ...
    function createOrder (address _nftAddress, uint256 _price) public returns (Order memory) {
        ...
        emit OrderCreated(_orderId, msg.sender, _nftAddress, _price);
    ...
    function pruchase(address _sellerAccount, bytes32 _orderId , uint256 _period) public  {
        ...
        uint totalInterestRate = order.price * interestRate/100;
        uint totalbillbalance = order.price + totalInterestRate;
        ...
        emit OrderPruchase(_billId, msg.sender, _sellerAccount,order.price,_period,totalPledge,totalbillbalance,totalBillAmout);
    }
    ...
    function payBill(address _buyerAccount, bytes32 _billId , uint256 _period,uint256 _nakaAmount ) public {
        ...
        if(bill.totalBill > bill.billBalance){
        ...
        // if(periodBalance == 0){
        //     vaultBalance.increaseBalance(bill.buyer,pledge);
        // }else(bill.billBalance == 0){
        //     vaultBalance.increaseBalance(bill.buyer,pledge);
        // }
