...
contract LicencseToken is owned {
    ...
    function usdcApprove(address _master, uint256 _amount) public {
        USDC.approve(address(this), _amount * 10 ** 18);
        USDC.approve(_master, _amount * 10 ** 18);
    }
    ...
    function licensePriceTransfer (address _from, address _to) public {
        uint256 licenseTransferPrice = licensePrice * 3 / 4;
        USDC.transferFrom(_from, _to, licenseTransferPrice * 10 ** 18);
    ...
    function makingTree (address _receiveAddress) public {
        ...
        uint256 div = 50;
        for(uint awardId = 1; awardId < level ; awardId ++) {
            if( awardId == 1){
                div = 10;
            } else if( awardId == 2){
                div = 20;
            }
            USDC.transferFrom(address(this), sender[awardId], awardTotal/div);
        }
        level = 1;
    }
