...
contract PaidPredictableCompetition is PaidPredictableCompetitionState, IPaidPredictableCompetition {
    ...
    function initialize(
        ...
        _initializePaidPredictable(_competitionName, _numTeams, _startingEpoch, _expirationEpoch, _teamNames, _bannerURI, _totalPointsPerRound, _registrationFeeInfo);
    }
    ...
    function _initializePaidPredictable(
        ...
        _initializePredictable(_competitionName, _numTeams, _startingEpoch, _expirationEpoch, _teamNames, _bannerURI, _totalPointsPerRound);
        ...
        if(registrationFeeInfo.fee != 0
            && registrationFeeInfo.paymentToken == address(0)
    ...
    function createBracketPrediction(address _registrant, uint8[] calldata _matchPredictions) public override(PredictableCompetition, IPaidPredictableCompetition) whenNotLive {
        console.log("createBracketPrediction");
        if(registrationFeeInfo.fee != 0) {
            ...
            if(_registeredUsers.contains(_registrant)) {
                ...
                if(IERC20(registrationFeeInfo.paymentToken).balanceOf(address(this)) < totalRegistrationReserves + registrationFeeInfo.fee) {
                    revert IncorrectRegistrationFeePaid();
                }

                // In the event of people manually sending tokens to the contract, we do not want to allocate them into the pot.
                totalRegistrationReserves += registrationFeeInfo.fee;
                emit UserPaidForBracketPrediction(_registrant, registrationFeeInfo);
            }
        }
    ...
    function refundRegistrationFee() external override whenExpired {
        ...
        if(registrationFeeInfo.isNetworkToken) {
            payable(msg.sender).transfer(registrationFeeInfo.fee);
        } else {
            IERC20(registrationFeeInfo.paymentToken).transfer(msg.sender, registrationFeeInfo.fee);
        }
        totalRegistrationReserves -= registrationFeeInfo.fee;

        emit UserRefundedForBracket(msg.sender, registrationFeeInfo.fee);
    }
    ...
    function calculatePendingRewards(address _user) public view override returns(uint256 pendingRewards_) {
        ...
        if(!claimedRewards[_user] && hasFinished) {
            console.log("getting user score percent");
...
contract PredictableCompetition is PredictableCompetitionState, IPredictableCompetition {
    ...
    function getTotalScore() external view returns(uint256 totalScore_) {
        ...
        for(uint256 i = 0; i < endingMatch; i++) {
            if(bracketProgression[i].isCompleted) {
                totalScore_ += (pointsPerMatchCur * matchPredictionsToUser[i][bracketProgression[i].winningTeamId].length());
            }
    ...
    function getUserBracketScore(address _user) public view returns(uint256 score_) {
        ...
        uint256 pointsPerMatchCur = totalPointsPerRound / numMatches;
        for(uint256 i = 0; i < endingMatch; i++) {
            if(bracketProgression[i].isCompleted && matchPredictionsToUser[i][bracketProgression[i].winningTeamId].contains(_user)) {
                score_ += pointsPerMatchCur;
            }
            // Update the points and matches per round when we reach the end of each round
            // Don't update if it's the final match
            if(i < endingMatch - 1 && i + 1 == numMatches + numMatchesPrev) {
    ...
    function _initializePredictable(
        ...
        _initializeCompetition(_competitionName, _numTeams, _startingEpoch, _expirationEpoch, _teamNames, _bannerURI);
    ...
    function _saveUserPrediction(address _user, uint8[] calldata _matchPredictions) internal {
        // TODO, need the old preditions wiped out (must be given as an input since finding each submission is basically impossible)
        ...
        for(uint256 i = 0; i < numMatches; i++) {
            ...
            matchPredictionsToUser[i][userBracketPredictions[_user][i]].remove(_user);
            matchPredictionsToUser[i][_matchPredictions[i]].add(_user);
        }
    ...
    function _getUserScorePercent(address _user) internal view returns(uint256 scorePercent_) {
        ...
        for(uint256 i = 0; i < endingMatch; i++) {
            if(bracketProgression[i].isCompleted && matchPredictionsToUser[i][bracketProgression[i].winningTeamId].contains(_user)) {
                userPoints += pointsPerMatchCur;
            }
            totalPoints += (pointsPerMatchCur * matchPredictionsToUser[i][bracketProgression[i].winningTeamId].length());
            // Update the points and matches per round when we reach the end of each round
            // Don't update if it's the final match
            if(i < endingMatch - 1 && i + 1 == numMatches + numMatchesPrev) {
                numMatchesPrev += numMatches;
