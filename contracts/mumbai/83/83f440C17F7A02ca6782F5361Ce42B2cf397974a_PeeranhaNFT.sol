...
contract PeeranhaNFT is IPeeranhaNFT, ChildMintableERC721Upgradeable {
  ...
  event ConfigureNewNFT(uint64 indexed NFTId);
  
  NFTLib.AchievementNFTsContainer achievementsNFTContainer;
  NFTLib.AchievementCount achievementCount;
    ...
    override
  {
    ...
    achievementCount.properties[achievementsType]++;
    if (achievementsType == AchievementCommonLib.AchievementsType.Rating) {
      require(achievementId == achievementCount.properties[achievementsType], "Wrong achievement Id");  // tests??
    }
  ...
  function setRatingAchievementCount(
  ) 
    external
    onlyRole(OWNER_MINTER_ROLE)
  {
    achievementCount.properties[AchievementCommonLib.AchievementsType.Rating] = achievementsNFTContainer.achievementsCount;
  }
  ...
  function mint_( // .._ ?
    ...
    override
  {
    require(achievementsNFTContainer.achievementsCount >= achievementId && achievementId != 0, "NFT does not exist");   // todo Tests
...
library NFTLib {
  ...
  struct AchievementCount {   // name
    mapping(AchievementCommonLib.AchievementsType => uint64) properties; // AchievementsType? and uint64 or uint256
  }
  ...
  //   uint64 maxCount;
  //   string NFTURI;
  //   AchievementCommonLib.AchievementsType NFTType;
  // }

  // struct NFTContainer {
  //   mapping(uint64 => NFTsConfigs) NFTConfigs;
  //   uint64 NFTCount;
  // }
}
...
interface IPeeranhaNFT {
  function mint_(address user, uint64 achievementId) external;
  function configureNewAchievementNFT(
    uint64 achievementId,
    uint64 maxCount,
    string memory achievementURI,
