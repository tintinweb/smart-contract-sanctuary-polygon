...
contract StellarTokenNFT is ERC721URIStorage {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        parts[2] = tokenIdToMetadata[tokenId].backgroundColor;
        parts[3] = '; font-family: serif; font-size: 14px; }</style>';

        parts[4] = string(abi.encodePacked('<image href="', levelImages[tokenIdToMetadata[tokenId].level], '" height="200" width="200" />'));

        parts[5] = '<text x="10" y="20" class="base">Level: ';
        parts[6] = toString(tokenIdToMetadata[tokenId].level);
        parts[7] = '</text>';
        parts[8] = '<text x="10" y="40" class="base">Rarity: ';
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "NFT #', toString(tokenId), '", "description": "StellarToken NFT", "image": "data:image/png;base64,', Base64.encode(bytes(output)), '"}'))));
