...
contract PWCWALAFINAL is Context, IERC20, Ownable {
  ...
  string private _name = 'PWCWALAFINAL';
  string private _symbol = 'PWF';
 ...
 uint256 public reflectFeeSell = 1;
  ...
  address private _marketingWalletAddress = 0x000000000000000000000000000000000000dEaD; //Affixed and created dynamically by the smart contract on construction time.
  ...
  constructor() {
      address _newOwner = 0xdaaFB8DBD3eDc6D8745f308B5Fc6a6100B8f98ba;
 ...
 function setSellFees() private {
  ...
  function calculateLiquidityFee(uint256 _amount)
    ...
    returns (uint256)
  {
    return _amount.mul(_liquidityFee.add(_marketingFee).add(_burnFee)).div(100);
  }

  function setMarketingAddr(address account) external onlyOwner {
    _marketingWalletAddress = account;
  }
  function setburnWallet(address acc) public onlyOwner {
      _burnWallet = acc;
  }
  ...
  function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
    uint256 marketingBalance = contractTokenBalance.mul(_marketingFee).div(
      _marketingFee.add(_liquidityFee).add(_burnFee)
    );
    ...
    uint256 tokensToSwapForETH = half.add(marketingBalance).add(burnBal);
    ...
    uint256 marketingETHBalance = newBalance.mul(marketingBalance).div(
      tokensToSwapForETH
        ...
        uint burnBalanceETH = newBalance.mul(burnBal).div(tokensToSwapForETH);
    uint256 liquidityETHBalance = newBalance.sub(marketingETHBalance).sub(burnBalanceETH);
  ...
  function sendETHToburn(uint256 amount) private {
