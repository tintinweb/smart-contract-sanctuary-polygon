...
contract Voting is Ownable, AccessControl, Pausable {
    ...
    */
    ...
    /**
    * @dev Allow when contest has started.
    */
    ...
    /**
    * @dev Allow when contest not ended.
    */
    ...
    * @param _entryFee - Contest entry fee (package price).
    * @param _entryVoteRewardLimit - Limit of user eligible for vote reward.
    * @param _entryCommentRewardLimit - Limit of user eligible for comment reward.
    */
    ...
    * @param _entryFee - New contest entry fee (package price).
    * @param _entryVoteRewardLimit - New limit of user eligible for vote reward.
    * @param _entryCommentRewardLimit - New limit of user eligible for comment reward.
    */
    ...
    */
    ...
    {
        Contest storage contest = contestByContestId[_contestId];
        uint256 voteReward = contest.userVoteReward[_userAddress];
        uint256 commentReward = contest.userCommentReward[_userAddress];
    ...
    */
        ...
        )
    {
        ...
        uint256 setProjectIdLength = 0;
        // Create unique array for projectId
        {
            string[] memory userVotes = contest.userVoteParticipation[_userAddress];
            string[] memory userComments = contest.userCommentParticipation[_userAddress];
            if(userVotes.length > 0) {
                setProjectId = userVotes;
                setProjectIdLength = userVotes.length;    
            }
            for (uint256 i = 0; i < userComments.length; i++) {
                string memory projectId = userComments[i];
                bool isExist = false;
                for (uint256 j = 0; j < userVotes.length; i++) {
                    string memory existProjectId = userVotes[i];
        ...
        // Create user entries reward from unique array
        {
            for (uint256 i = 0; i < setProjectIdLength; i++) {
                ...
                list[listLength] = UserEntryReward(
                    setProjectId[i],
                    (entry.voteByAddress[_userAddress].voteId == 0 || entry.voteByAddress[_userAddress].voteId > contest.entryVoteRewardLimit)?
                        0:
                        contest.rewardPerVote,
                    (entry.commentByAddress[_userAddress].commentId == 0 || entry.commentByAddress[_userAddress].commentId > contest.entryCommentRewardLimit)?
                        0:
                        contest.rewardPerComment
                );
                listLength++;
            }
        }
    ...
    /** 
    * @dev Get infomation in entry level.
    * @param _contestId - Contest id.
    * @param _projectId - Project id of entry.
    */
