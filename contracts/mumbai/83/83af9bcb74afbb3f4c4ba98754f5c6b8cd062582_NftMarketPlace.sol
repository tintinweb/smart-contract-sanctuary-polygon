...
struct CryptoBoy {
  ...
  constructor() ERC721("Reel Crypto", "REELT") {
    collectionName = name();
    collectionNameSymbol = symbol();
  }
function getAllNFTs() public view returns (CryptoBoy[] memory) {
    uint nftCount = tokenCounter;
    CryptoBoy[] memory tokens = new CryptoBoy[](nftCount);
    uint currentIndex = 0;
    uint currentId;
    ...
    {
        currentId = i + 1;
        CryptoBoy storage currentItem = allCryptoBoys[currentId];
  ...
  function mintNft(string memory _name, string memory _tokenURI, uint256 _price) external {
    ...
    allCryptoBoys[tokenCounter] = newCryptoBoy;
  }
