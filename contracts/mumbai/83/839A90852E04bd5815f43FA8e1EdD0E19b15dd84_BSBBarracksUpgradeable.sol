...
contract BSBBarracksUpgradeable is Initializable, BSBBaseGameModeUpgradeable {
    ...
    function _claimRewards(CollectionItems calldata collectionItems_) internal {
        ...
        for (uint i = 0; i < collectionItems_.assets.length; ++i) {
            uint referenceDate_ = _getReferenceDateFromStamina(
                collectionItems_.collection,
            ...
            uint resultingLevel_ = bearsStats_[i].level.value + _calculateLevelReward(
                                                                    collectionItems_.collection,
                                                                    collectionItems_.assets[i],
                                                                    referenceDate_
                                                                );
            resultingLevel_ = resultingLevel_ <= bearsStats_[i].level.max
                ? resultingLevel_
                : bearsStats_[i].level.max;
            ...
            bearsStats_[i].stamina.value =
                bearsStats_[i].stamina.value - bearsStats_[i].stamina.min >= spentStamina_
                ? bearsStats_[i].stamina.value - spentStamina_
                : bearsStats_[i].stamina.value;


            BSBStorageCaller.setBearStats(collectionItems_.collection, collectionItems_.assets[i], bearsStats_[i]);

            emit LevelClaimed(_msgSender(), collectionItems_.assets[i], resultingLevel_);

            stateChangeDates[collectionItems_.collection][collectionItems_.assets[i]] = block.timestamp;

            delete spentStamina_;
            delete resultingLevel_;
            delete referenceDate_;
        }

        delete bearsStats_;
    }

    function claimRewards(CollectionItems[] calldata collectionItems_) public {
        for (uint i = 0; i < collectionItems_.length; ++i) {
    ...
    function calculateReward(address collection_, uint tokenId_) external view returns(uint) {
        return _calculateLevelReward(
            collection_, 
            tokenId_, 
            _getReferenceDateFromStamina(
                collection_, 
                tokenId_, 
                BSBStorageCaller.getBearStats(collection_, tokenId_).stamina,
                BSBMintPassBuffCaller.getBuffBalance(tokenOwners[collection_][tokenId_]) != uint(0)
            )
        );
    }

    function _calculateLevelReward(address collection_, uint tokenId_, uint referenceDate_) internal view returns(uint) {
        return (referenceDate_ - stateChangeDates[collection_][tokenId_]) / (levelDaysRate * DAYS);
    }

    function _getReferenceDateFromStamina(
        address collection_,
        uint tokenId_,
        ...
        uint remainingStaminaEndDate_ = ((stamina_.value - stamina_.min + 1) /
            (staminaDecreaseRate - (applyBuff_ ? staminaBuff : 0))) *
            DAYS +
            stateChangeDates[collection_][tokenId_];

        if (remainingStaminaEndDate_ > block.timestamp) {
            return block.timestamp;
        }
        
        return remainingStaminaEndDate_;
    }
    ...
    function calculateRemainingStamina(
        ...
        return bearStamina_.value - _calculateSpentStamina(
            collection_,
            tokenId_,
            referenceDate_,
            applyBuff_
    ...
    function _calculateSpentStamina(
        address collection_,
        uint tokenId_,
        uint referenceDate_,
        bool applyBuff_
    ) internal view returns(uint) {
        return (referenceDate_ - stateChangeDates[collection_][tokenId_]) /
        (
            (staminaDecreaseRate - (applyBuff_ ? staminaBuff : 0)) *
            DAYS
        );
