...
// File: contracts/IERC1155TokenCreator.sol
...
// File: contracts/ICreatoRoyaltyRegistry.sol
 ...
 */
interface ICreatoRoyaltyRegistry is IERC1155TokenCreator {
    /**
    ...
    /**
     * @dev Sets the royalty percentage set for an Creato token
     * Requirements:
...
// File: contracts/ICreatoTokenCreatorRegistry.sol
 ...
 */
interface ICreatoTokenCreatorRegistry {
    /**
     * @dev Gets the creator of the token
     * @param _tokenId uint256 ID of the token
     * @return address of the creator
...
/**
 * @dev Interface for interacting with the Creato contract that holds Creato beta tokens.
 */
interface ICreato {
...
// File: contracts/CreatoTokenCreatorRegistry.sol
 ...
 * @title IERC1155 Non-Fungible Token Creator basic interface
 */
contract CreatoTokenCreatorRegistry is Ownable, ICreatoTokenCreatorRegistry {
    ...
    /////////////////////////////////////////////////////////////////////////
    /**
     * @dev Set creato contract address
     * @param _creato uint256 ID of the token
     */
    function setCreato(address _creato) external onlyOwner {
        creato = _creato;
    }
    ...
    function setTokenCreator(
        ...
        require(msg.sender == creato || msg.sender == owner(), "setTokenCreator::only creato and owner allowed");
     ...
     * @param _oldCreatoAddress get the token ids from the old creato contract.
     * @param _startIndex start index of array
     * @param _endIndex end index of array
     */
    function restore(address _oldAddress, address _oldCreatoAddress, uint256 _startIndex, uint256 _endIndex) external onlyOwner {
        CreatoTokenCreatorRegistry oldContract = CreatoTokenCreatorRegistry(_oldAddress);
        ICreato oldCreatoContract = ICreato(_oldCreatoAddress);

        uint256 length = oldCreatoContract.getTokenIdsLength();
        require(_startIndex < length, "wrong start index");
        require(_endIndex <= length, "wrong end index");

        for (uint i = _startIndex; i < _endIndex; i++) {
            uint256 tokenId = oldCreatoContract.getTokenId(i);
