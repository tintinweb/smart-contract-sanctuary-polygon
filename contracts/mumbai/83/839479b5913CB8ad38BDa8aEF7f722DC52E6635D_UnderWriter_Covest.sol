...
contract UnderWriter_Covest is Initializable, UUPSUpgradeable, ReentrancyGuardUpgradeable { 

    mapping(address => bool) internal _isUnderWritier;
    mapping(string => uint256) internal historyValueOfUser;
    mapping(address => address) internal underWriterTo;
    mapping(address => mapping(address => uint256)) internal ValueofUnderwriter;

    address[] internal _allUW;
    uint256 public countUW;
    ...
    event Withdraw(address indexed asset,address indexed to, uint256 value,uint256 remain);
    ...
    modifier isPolicy {
        require(AR.isPolicy(NR.poolId(),msg.sender));
        _;
    }
    ...
    function applyWorking(address _user) public isSuperManager {
        require(PDS.isBlacklistOfUser(_user) != true,"Policy: This _user are Blacklist.");
        require(_isUnderWritier[_user] != true);
        _isUnderWritier[_user] = true;
        _allUW.push(_user);
        countUW += 1;
    ...
    function resignation(address _user) public isSuperManager { 
        require(PDS.isBlacklistOfUser(_user) != true,"Policy: This _user are Blacklist.");
        require(_isUnderWritier[_user] != false);
        _isUnderWritier[_user] = false;

        for (uint256 i = 0; i < _allUW.length; i++) {
            if (_user == _allUW[i]) {
                _allUW[i] = _allUW[
                    _allUW.length - 1
                ];
    ...
    function allUW() public view returns(address[] memory) {
        require(_allUW.length > 0,"UW: Not have any UW Active.");
        uint countPass;
        uint countAdded;

        for (uint i = 0;i<_allUW.length;i++){
            bool isThisUserPass = isUnderWriter(_allUW[i]);
            if(isThisUserPass == true){
                countPass++;
            }
        }

        address[] memory allUWPass = new address[](countPass);

        for (uint i = 0;i<_allUW.length;i++){
            bool isThisUserPass = isUnderWriter(_allUW[i]);
            if(isThisUserPass == true){
                allUWPass[countAdded] = _allUW[i];
                countAdded++;
            }
        }
    ...
    function isUnderWriter(address _user) public view returns(bool) {
        (IStaking.StakingData memory _StakingData) = S.stakingData(_user,IStaking.typeUser.UW);
        (IStaking.WorkType memory _dataType) = S.workData(IStaking.typeUser.UW);
        return _isUnderWritier[_user] && (_StakingData.balance >= _dataType.minUserStake);
    }

    function isUnderWriterNonCheckStake(address _user) public view returns(bool){
        return _isUnderWritier[_user];
    }

    function fundInFlow(address _caller, address _asset, string memory _policyId, uint256 _value) public isPolicy { // only Policy can Manage //
        require(isUnderWriter(_caller) == false,"You are UnderWriter.");
        historyValueOfUser[_policyId] = _value;
     
        emit FundInFlow(_caller,_asset,_policyId,_value);
    }

    function fundOutFlow(address _caller, address _asset, string memory _policyId, uint256 _value) public isPolicy { // only Policy can Manage // 
    ...
    function balanceOfUnderWriterPool(address _asset) public view returns(uint256) {
        IERC20 _ERC20 = IERC20(_asset);
        return _ERC20.balanceOf(address(this));
    }

    function withdraw(address _asset,address _user, uint256 _value) public nonReentrant isSuperManager checkAsset(_asset) { 
        IERC20 _ERC20 = IERC20(_asset);
        require(_ERC20.balanceOf(address(this)) >= _value,"Balance of Contract Not Enough.");
        require(isUnderWriter(_user) == true,"This _user Doesn't Underwriter.");
        _ERC20.transfer(_user,_value);

        emit Withdraw(_asset,msg.sender,_value,_ERC20.balanceOf(address(this)));
    }
