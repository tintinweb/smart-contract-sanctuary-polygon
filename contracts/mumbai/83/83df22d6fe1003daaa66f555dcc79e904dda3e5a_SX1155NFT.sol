...
contract SX1155NFTBase is ERC1155 {
    ...
    function _burnToken(uint256 _id, uint256 _quantity) internal {
    ...
    function _burnTokenBatch(uint256[] calldata _ids, uint256[] calldata _quantities) internal {
...
contract SX1155NFT is Pausable, SX1155NFTBase, RoleManager, AuthorizeManager, AccountControl {
    ...
    /// List of greylist transers waiting for confirmation from ISSUER
    mapping(uint256 => GreylistTransferRequest) internal greylistTransferRequests;

    /// emitted whe an issuer does a 'force' transfer
    event ForceTransfer(address indexed issuer, address indexed from, address indexed to, uint256 tokenId);
    event ForceBurn(address indexed issuer, address indexed from, uint256 indexed tokenId);
    ...
    function burnBatch(uint256[] calldata _ids, uint256[] calldata _quantities) external {
        _burnTokenBatch(_ids, _quantities);
    }
    ...
    /// Raise a transfer request that require issuer confirmation. Only Greylist users
    /// are allowed.
    function requestTransfer(
        ...
        // 'from' or 'to' account must not be frozen
    ...
    function _beforeTokenTransfer(
        ...
        // Whitelist accounts can transfer, overrides all rules from authorized contracts
