...
contract AgriCredit6 is ERC721, FinControl, StatusControl {
    ...
    function mintCredits(bytes32[] memory _guid, uint256 _price, uint256 _newExchangeRate, uint256 amount, uint256 status) whenNotPaused allowedToMint external payable returns (uint256[] memory) {
    ...
    function updateForSaleThreshold(uint256 newThreshold) onlyExecutiveOfficer external {
