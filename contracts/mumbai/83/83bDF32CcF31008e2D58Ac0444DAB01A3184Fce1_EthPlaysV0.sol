...
contract EthPlaysV0 is Ownable {
    ...
    onlyRegistered
  {
    ...
    if (bestBannerBid.from != address(0)) {
      poke.gameTransfer(address(this), bestBannerBid.from, bestBannerBid.amount);
    }
  ...
  function rolloverBannerAuction() external onlyActive {
    if (block.timestamp < bannerAuctionTimestamp + bannerAuctionCooldown + bannerAuctionDuration) {
  ...
  function submitControlBid(uint256 amount) external onlyActive onlyRegistered {
    if (block.timestamp < controlAuctionTimestamp + controlAuctionCooldown) {
      revert AuctionNotActive();
    }
    ...
    if (bestControlBid.from != address(0)) {
      poke.gameTransfer(address(this), bestControlBid.from, bestControlBid.amount);
    }
    poke.gameTransfer(msg.sender, address(this), amount);
  ...
  function setBannerAuctionCooldown(uint256 _bannerAuctionCooldown) external onlyOwner {
    bannerAuctionCooldown = _bannerAuctionCooldown;
    emit SetBannerAuctionCooldown(_bannerAuctionCooldown);
  }

  function setBannerAuctionDuration(uint256 _bannerAuctionDuration) external onlyOwner {
    bannerAuctionDuration = _bannerAuctionDuration;
    emit SetBannerAuctionDuration(_bannerAuctionDuration);
  }

  function setControlAuctionCooldown(uint256 _controlAuctionCooldown) external onlyOwner {
