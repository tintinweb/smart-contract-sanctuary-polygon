...
contract CollectionNFT is ERC1155, ReentrancyGuard{
    ...
    modifier onlyStarted() {
        require(
            block.timestamp >= blockStart && block.timestamp <= blockEnd,
            "Modifier: Not started yet"
        );
    ...
    function setMintingTime(uint256 startDate, uint256 endDate)
        public
    ...
    // mint
    function mint(uint256 tokenId, address mintAddress, uint256 amount)
    external
    onlyStarted
    onlyAdminOrOwner(msg.sender)
    {
        ...
        _mint(mintAddress, tokenId, amount, "");
        mintedPerAddress[mintAddress] = mintedPerAddress[mintAddress] + amount;
        mintedAmount[tokenId] = mintedAmount[tokenId] + amount;
    }

    function batchmint(uint256[] memory tokenIds, address[] memory mintAddresses, uint256[] memory amounts)
    external
    onlyAdminOrOwner(msg.sender)
    {
        require(mintAddresses.length == amounts.length, "Length of amounts must match length of mint addresses");
        require(mintAddresses.length == tokenIds.length, "Length of ids must match length of mint addresses");
    ...
    function uri(uint256 typeId) public view override returns (string memory) {
        require(typeId < 6, "TokenUri: ID isn't in the range");
        require(bytes(baseURI).length > 0, "TokenUri: base URI is not set");
        return
            string(abi.encodePacked(baseURI, typeId.toString(), metaDataExt));
    }
}