...
contract NFTRaffle is AccessControl, ReentrancyGuard, VRFConsumerBaseV2, ConfirmedOwner {
    ...
    function createRaffle(address _nftAddress, uint32 _nftId, uint32 _price, uint32 _expirationTime, address[] memory _holderArray) external onlyAdmin nonReentrant returns (uint64) {
        ...
        uint256 holderLength = _holderArray.length;
        if (holderLength > 3) revert("Holder Illegal");
        ...
        holder_1 : holderLength > 0 ? _holderArray[0] : address(0),
        holder_2 : holderLength > 1 ? _holderArray[1] : address(0),
        holder_3 : holderLength > 2 ? _holderArray[2] : address(0)
        });
    ...
    function buyEntry(uint64 _raffleId, uint16 _qty, uint256 _price) external nonReentrant {
        entry(_raffleId, _qty, msg.sender, _price);
    }

    function presentEntry(uint64 _raffleId, uint16 _qty, address _receiptAddress, uint256 _price) external nonReentrant {
        entry(_raffleId, _qty, _receiptAddress, _price);
    }


    //need to token allowance before sending to this contract
    function entry(uint64 _raffleId, uint16 _qty, address _receiptAddress, uint256 _price) internal {
        ...
        if (_qty == 0 || _qty > config.totalSupply - raffle.currentSupply || _qty > config.totalSupply * config.maxPurchaseLimit / 10000) revert("Wrong entries");

        // check holder only
        if (checkHolder(raffle.holder_1, msg.sender, true) || checkHolder(raffle.holder_2, msg.sender, false) || checkHolder(raffle.holder_3, msg.sender, false)) {
            ...
            // fee management
            uint256 fee = _price * config.treasuryFee / 10000;
            IERC20(TokenContract).safeTransferFrom(msg.sender, config.treasuryWallet, fee);
            uint256 burnAmount = _price - fee;
            IERC20(TokenContract).safeTransferFrom(msg.sender, BurnAddress, burnAmount);
    ...
    function draw(uint64 _raffleId) external nonReentrant onlyAdmin {
        RaffleStruct memory raffle = raffleMap[_raffleId];

        // Unsold entries cannot draw
        if (raffle.status != STATUS.NORMAL || raffle.currentSupply == 0) revert("Wrong status");

        if (raffle.currentSupply == config.totalSupply
    ...
    function fulfillRandomWords(uint256 _requestId, uint256[] memory _randomWords) internal override {
        ...
        raffleMap[raffleId].status = STATUS.DREW;

        emit RaffleStatus(raffleId, STATUS.DREW);
    }

    function winnerClaim(uint64 _raffleId) internal {
        RaffleStruct memory raffle = raffleMap[_raffleId];
        //        if (raffle.status != STATUS.DREW || raffle.currentSupply == 0) revert("Wrong status");
    ...
    function setTreasuryFee(uint32 _fee, address payable _treasuryWallet) external onlyRole(DEFAULT_ADMIN_ROLE) {
        config.treasuryFee = _fee;
