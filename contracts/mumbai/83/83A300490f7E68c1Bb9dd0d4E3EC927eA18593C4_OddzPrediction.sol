...
contract OddzPrediction is IOddzPrediction, AccessControl, Pausable, ReentrancyGuard {
    ...
    bool public startPredictionMarketOnce = false;
    bool public createPredictionMarketOnce = false;
    ...
    function pause() external whenNotPaused onlyAdmin(msg.sender) {
        _pause();
        _refundable(currentRoundId);
        _refundable(currentRoundId + 1);
    ...
    function unPause() external whenPaused onlyAdmin(msg.sender) {
        createPredictionMarketOnce = false;
        startPredictionMarketOnce = false;
        _unpause();

        emit UnPaused(currentRoundId);
    }
    ...
    function _createRound(uint256 roundId, uint256 _startTimestamp) internal {
        require(oddzPredictionStorage.getPredictionRound(roundId).roundId == 0, "Round already exists");
        ...
        oddzPredictionStorage.createPrediction(round);
    ...
    function _startRound(uint256 _predictRoundId, uint256 _price) internal {
        ...
        require(round.startTimestamp >= block.timestamp, "Too late to start the round");
    ...
    function _endRound(uint256 _predictRoundId, uint256 _price) internal {
        ...
        require(round.endTimestamp <= block.timestamp, "Too early to end the round");
        round.endPrice = _price;
    ...
    function _calculateRewards(uint256 _predictRoundId) internal {
        ...
        treasuryAmt = (round.totalAmount * treasuryFee * 100) / (10**decimals);
     ...
     * @param _predictRoundId round Id
     */
    function _refundable(uint256 _predictRoundId) internal view returns (bool) {
        Round memory round = oddzPredictionStorage.getPredictionRound(_predictRoundId);
        return round.rewardBaseCalAmount == 0 && round.rewardAmount == 0 && round.startPrice == round.endPrice;
    }

    /**
     * @notice Transfer the funds to oddz admin contract
        ...
        onlyManager(msg.sender)
    {
        ...
        if (betInfo.upPredictAmount == 0 && betInfo.downPredictAmount == 0) {
            oddzPredictionStorage.setUserRounds(msg.sender, _predictRoundId);
            oddzPredictionStorage.setUserInRounds(msg.sender, _predictRoundId);
        }

        oddzPredictionStorage.setBetInfo(amount, _predictRoundId, msg.sender, true);
        ...
        onlyManager(msg.sender)
    {
        ...
        oddzPredictionStorage.setBetInfo(amount, _predictRoundId, msg.sender, false);
    ...
    {
        require(!createPredictionMarketOnce, "Can only run roundzeroCreateRound once");
        currentRoundId = currentRoundId + 1;
        roundzeroStartTimestamp = _roundzeroStartTimestamp;
        _createRound(currentRoundId, _roundzeroStartTimestamp);
        createPredictionMarketOnce = true;
    }

    /**
     * @notice Start Zero round
     * @dev callable by Operator
     * @param _priceRoundId Round Id of the timestamp
     */
    function startPredictionMarket(uint256 _priceRoundId)
        external
        override
        whenNotPaused
        onlyOperator(msg.sender)
        notContract(msg.sender)
    {
        require(createPredictionMarketOnce, "Can only run after roundzeroCreateRound is triggered");
        require(!startPredictionMarketOnce, "Can only run roundzeroStartRound once");
        (uint256 price, ) = oddzPriceManager.getPriceByTimestamp(
            assetPair.underlying,
            assetPair.strike,
            roundzeroStartTimestamp,
            uint80(_priceRoundId)
        );
        ...
        startPredictionMarketOnce = true;
    }
     ...
     * @param _priceRoundId Round Id of the timestamp
     */
    function executeRound(uint256 _priceRoundId)
        external
        override
        whenNotPaused
        onlyOperator(msg.sender)
        notContract(msg.sender)
    {
        require(createPredictionMarketOnce && startPredictionMarketOnce, "Can only run after roundzeroStartRound");
        Round memory curRound = oddzPredictionStorage.getPredictionRound(currentRoundId);
        Round memory roundPlusTwo = oddzPredictionStorage.getPredictionRound(currentRoundId + 2);
        ...
        (uint256 price, ) = oddzPriceManager.getPriceByTimestamp(
            assetPair.underlying,
            assetPair.strike,
            curRound.endTimestamp,
            uint80(_priceRoundId)
        );
        // int256 price = _getPriceByTimestamp(rounds[currentRoundId].endTimestamp, referenceId);
        ...
        _createRound(currentRoundId + 3, roundPlusTwo.endTimestamp);
...
interface IOddzPrediction {
     ...
     * @dev callable by Operator
     */
    function startPredictionMarket(uint256 roundId) external;

    /**
     * @notice Execute round
     * @dev Callable by Operator
     */
    function executeRound(uint256 referenceId) external;
...
contract OddzPredictionStorage is AccessControl {
    ...
    IOddzPrediction.Round[] public rounds;
    ...
    function createPrediction(IOddzPrediction.Round memory round) external onlyManager(msg.sender) {
        rounds.push(round);
    }

    function getPredictionRoundsCount() external view returns (uint256 count) {
        count = rounds.length;
    }
    ...
    function setUserRounds(address userAddress, uint256 roundId) external onlyManager(msg.sender) {
        userRounds[userAddress].push(roundId);
    }

    function setUserInRounds(address userAddress, uint256 roundId) external onlyManager(msg.sender) {
