...
contract TokenTest is ERC20, Ownable {
    ...
    uint16 public constant MAX_FEE = 2000; // max 20% fees for bot
    ...
    function _setFeeConfiguration(FeeConfiguration memory configuration)
        private
    {
    ...
    function setLiquidityOwner(address _owner) external {
        liquidityOwner = _owner;
        emit LiquidityOwnerUpdated(liquidityOwner);
    }
    ...
    function _changeStableCoin(address _stableCoin) internal {
    ...
    function updateSwapRouter(address _newRouter) external onlyOwner {
        ...
        swapPair = factory.getPair(address(this), stableCoin);
        if (swapPair == address(0)) {
            swapPair = factory.createPair(address(this), stableCoin);
        }
    ...
    function _swapTokensForStable(uint256 tokenAmount, uint256 minAmountOut)
        private
    {
        // generate the swap pair path of token -> stableCoin
    ...
    function _addLiquidity(uint256 tokenAmount, uint256 stableAmount) private {
        // approve token transfer to cover all possible scenarios
        _approve(address(this), address(swapRouter), tokenAmount);
        ERC20(stableCoin).approve(address(swapRouter), stableAmount);
    ...
    function _processLiquidity(
        ...
        uint256 taxAmount = (_amount * _taxFee) / FEE_PRECISION;
        uint256 sendAmount = _amount - taxAmount;

        if (taxAmount > 0) {
            _transfer(_from, address(this), taxAmount);
            _swapTokensForStable(taxAmount / 2, 0);
            _addLiquidity(
                balanceOf(address(this)),
                ERC20(stableCoin).balanceOf(address(this))
            );
        }
    ...
    function _executeTransfer(
        ...
        if (fromLP && !toLP && to != address(swapRouter)) {
            // buy fee
        ...
        if (taxFee > 0) {
            require(_processLiquidity(from, to, amount, taxFee));
        } else {
        ...
        returns (bool)
    {
        ...
        return _executeTransfer(owner, to, amount);
    }
