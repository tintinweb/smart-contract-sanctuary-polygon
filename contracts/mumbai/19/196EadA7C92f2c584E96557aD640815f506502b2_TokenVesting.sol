...
contract TokenVesting is Ownable2Step {
    ...
    function getAllVestingSchedules() external view returns (VestingSchedule[] memory) {
        ...
        require(scheduleNamesLength != 0, 'TokenVesting: There are no vesting schedules');
    ...
    function getValidVestingSchedules() external view returns (VestingSchedule[] memory) {
        require(validVestingSchedulesCount != 0, 'TokenVesting: There are no valid vesting schedules');
    ...
    function decreaseVestingScheduleTotalAmount(uint256 _amount, string calldata _name) external onlyOwner {
        ...
        require(_amount > 0, 'TokenVesting: amount must be greater than zero');
        require(
            getScheduleUnallocatedAmount(_name) >= _amount,
            'TokenVesting: amount cannot be greater than vesting schedule unallocated amount'
        );
        vestingSchedules[_name].totalAmount -= _amount;
        vestingSchedulesTotalReservedAmount -= _amount;
    }
    ...
    function addBeneficiaryAllocation(
        ...
        require(_amount > 0, 'TokenVesting: amount must be > 0');
        require(isVestingScheduleValid(_vestingScheduleName), 'TokenVesting: vesting schedule is not valid');
        require(
    ...
    function deleteBeneficiary(address _beneficiary) external onlyOwner {
        ...
        require(scheduleNamesLength != 0, 'TokenVesting: None active vestings allocated to this address');
        for (uint32 i = 0; i < scheduleNamesLength; i++) {
    ...
    function claimAllTokens() public {
        ...
        require(scheduleNamesLength > 0, 'TokenVesting: there are no unclaimed tokens');
        for (uint32 i = 0; i < scheduleNamesLength; i++) {
