...
contract BaseNFT is ERC721Enumerable, ERC721Burnable, Blacklist, Pausable {
  // Base token URI
  string public baseTokenURI;
  // Last token ID starting from 1
  uint256 public tokenID;
  // Metal Core game portal contract
  address public gamePortal;

  event LogMinted(uint256 indexed tokenId);
  event LogBurnt(uint256 indexed tokenId);
  event LogGamePortalSet(address gamePortal);
  ...
  /**
    ...
    * Only `owner` and game portal contract can call
    * `_account` must not be zero address
    * `_account` must not be blacklisted
    */
  function mint(address _account) external whenNotPaused returns (uint256 newTokenId) {
    require(msg.sender == owner() || msg.sender == gamePortal, "BaseNFT: CALLER_NOT_AUTHORIZED");
    require(!blacklist[_account], "BaseNFT: ACCOUNT_BLACKLIST");
    newTokenId = ++tokenID;
  ...
  /**
    ...
    * Only `owner` can call
    * `_accounts` must not have zero address
    * `_accounts` must not have blacklisted address
    */
  function mintBatch(address[] calldata _accounts) external whenNotPaused onlyOwner {
    for (uint256 i = 0; i < _accounts.length; i++) {
      require(!blacklist[_accounts[i]], "BaseNFT: ACCOUNT_BLACKLIST");
      uint256 newTokenId = ++tokenID;
      super._mint(_accounts[i], newTokenId);
  ...
  function isApprovedForAll(
    ...
    if (_operator == gamePortal) return true;
