...
contract AgentRoleUpgradeable is OwnableUpgradeable, AccessControl {
    ...
    event TransferAgentAdded(address indexed _agent);
    event TransferAgentRemoved(address indexed _agent);
    ...
    function removeAgent(address _agent) public onlyAdmins {
     ...
     function addIssuer(address _userAddress) public onlyRole(Owner_Role) {
        require(!isIssuer(_userAddress), "Already have Issuer Role");
        ...
        _setupRole(Issuer_Role, _userAddress);
    }

    function revokeIssuer(address _issuerAddress) public onlyRole(Owner_Role) {
        require(isIssuer(_issuerAddress), "Doesn't have Issuer Role");
        bytes32[] memory array = new bytes32[](5);
        array[0] = Minter_Role;
        array[1] = Burner_Role;
        array[2] = Freezer_Role;
