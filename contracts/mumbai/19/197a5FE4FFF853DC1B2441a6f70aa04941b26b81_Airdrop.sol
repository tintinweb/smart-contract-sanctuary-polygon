...
contract Airdrop is EIP712MetaTransaction, ReentrancyGuard {   
    ...
    bytes32 private constant WHITELIST_TYPEHASH =
            keccak256('WhitelistInfo(address contractAddress,address creator,string productId,uint256 airdropFee)');
    bytes32 private immutable DOMAIN_SEPARATOR;
    
    mapping(string => bytes32) public productIdMerkleRoots;
      ...
      isActive
    {
        ...
        require(bytes(_productId).length > 0, "Airdrop#createWhitelist: _productId is empty");
        require(bytes32(_merkleRoot).length > 0, "Airdrop#createWhitelist: _merkleRoot is empty");
        require(_airdropFee > 0, "Airdrop#createWhitelist: invalid _airdropFee");
        require(
            weth.allowance(creator, address(this)) >= _airdropFee,
            "Airdrop#createWhitelist: weth allowance too low"
        );
        require(
            productIdMerkleRoots[_productId] == 0,
            "Airdrop#createWhitelist: productId has already been created"
        );
        // Check that the signature is valid.
        require(
            getSigner(_signature, _productId, _airdropFee) == dbiliaToken.dbiliaTrust(), 
            'Airdrop#createWhitelist: Invalid signer'
        );
        
        weth.safeTransferFrom(creator, dbiliaToken.dbiliaFee(), _airdropFee);
        productIdMerkleRoots[_productId] = _merkleRoot;
      ...
      nonReentrant 
    {
        ...
        require(productIdMerkleRoots[_productId] != "", "Airdrop#mintWhitelist: whitelist not created");
        require(!whitelistClaimed[_productId][minter], "Airdrop#mintWhitelist: address already claimed");

        bytes32 leaf = keccak256(abi.encodePacked(minter));
        bytes32 merkleRoot = productIdMerkleRoots[_productId];

        require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), "Airdrop#mintWhitelist: invalid Merkle Proof");       
        whitelistClaimed[_productId][minter] = true;
        dbiliaToken.mintWithFiatw3user(_royaltyReceiver, _royaltyPercentage, minter, _productId, _edition, _tokenURI);   
        ...
        bytes32 digest = keccak256(
            abi.encodePacked(
                ...
                keccak256(abi.encode(
                    ...
                    address(this), 
                    msgSender(), 
                    keccak256(abi.encodePacked(_productId)), // convert string to bytes32
                    _airdropFee 
                ))
            )
        );
    ...
    function setDbiliaToken(address _newDbiliaToken) external {
        require(msgSender() == dbiliaToken.owner(), "Airdrop#setDbiliaToken: wrong owner");
