...
contract EmojiNumeralsTraitChecker is Ownable, ITraitChecker {
    string public constant TRAIT_FAMILY = "Emoji Numeral Club";
    ...
    function getTrait(string calldata _username) external view returns (string memory, string memory) {
        ...
        for (uint i = 0; i < b.length;) {
            (uint32 code, ) = UnicodeLib.readUnicodeDecimal(b, i);
            if(code < 48 || code > 57) {
                return ("", "");
            }
            (bool isValid, ) = EmojiUtils(emojiUtils).isValidKeyCap(b, i);
            if(!isValid) {
                return ("", "");
            }
            i += 7;
        ...
        if(l == 1) return (TRAIT_FAMILY,"Emoji 10 club");
        if(l == 2) return (TRAIT_FAMILY,"Emoji 100 club");
        if(l == 3) return (TRAIT_FAMILY,"Emoji 1000 club");
        if(l == 4) return (TRAIT_FAMILY,"Emoji 10k club");
        if(l == 5) return (TRAIT_FAMILY,"Emoji 100k club");
        if(l == 6) return (TRAIT_FAMILY,"Emoji 1m club");
