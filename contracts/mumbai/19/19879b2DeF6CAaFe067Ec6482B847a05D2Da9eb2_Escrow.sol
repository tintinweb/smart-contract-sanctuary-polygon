...
contract Escrow is ERC1155Holder, ERC721Holder, ReentrancyGuard {
    ...
    function unlistAuction(string memory orderID) public {
        require(
            msg.sender == orders[orderID].seller || msg.sender == address(this),
        ...
        require(orders[orderID].stage == Stage.LISTED, "Wrong Token stage");
    ...
    function confirmSale(string memory orderID) public {
        ...
        require(orders[orderID].stage == Stage.LISTED, "Order is closed");
        ...
        if (cost == 0) {
            ...
            if (tokenType[orderID] == nftType.ERC1155) {
                ...
                try
                    token721.royaltyInfo(orders[orderID].tokenID, cost)
    ...
    function unlistSale(string memory orderID) public {
        require(
            orders[orderID].stage != Stage.orders_CLOSED,
        ...
        require(
            msg.sender == orders[orderID].seller,
            "Only seller can unlist sale"
        );
        require(
            orderType.sale == orders[orderID].ordertype,
            "Only seller can unlist sale"
        );
        orders[orderID].stage = Stage.orders_CLOSED;
        emit TokenUnlisted(
    ...
    function fulfillSale(
        ...
        require(
            (buyAmount > 0) &&
                    buyAmount <= orders[orderID].amountListed,
