...
contract MyCollection is ERC721, ERC721Enumerable, ERC721Pausable, ERC721Burnable, ERC721URIStorage, ERC721Royalty, Ownable{
    ...
    constructor(uint96 feeNumerator) ERC721("MY COLLECTION", "MLC"){
    ...
    function safeMint(address to, string memory uri)public onlyOwner returns(uint tokenId){
        _tokenIdCounter.increment();
        tokenId = _tokenIdCounter.current();
        _safeMint(to, tokenId);
        _setTokenURI(tokenId, uri);
    }
    function safeMintRoyalty(address to, string memory uri, address royaltyReceiver, uint96 feeNumerator)external onlyOwner returns(uint tokenId){
        tokenId = safeMint(to, uri);
        _setTokenRoyalty(tokenId, royaltyReceiver, feeNumerator);
    }

    function setTokenURI(uint256 tokenId, string memory _tokenURI)external{
        require(_owners[tokenId] == msg.sender, "only owner can do this");
