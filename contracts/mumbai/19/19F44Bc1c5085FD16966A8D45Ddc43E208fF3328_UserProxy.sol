...
contract UserProxy is AccessControl {
	...
	constructor (address _ethUser,address _lendingPool) public {
	...
	function deposit(address token,uint256 amount,uint16 referralCode) public {
		require(hasRole(OPERATORROLE, msg.sender), "Caller is not a operator");
		ILendingPool(LendingPOOL).deposit(token, amount,address(this),referralCode);
	}
