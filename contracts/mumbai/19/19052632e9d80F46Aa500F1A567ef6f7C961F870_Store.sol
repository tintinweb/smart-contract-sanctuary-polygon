...
contract Bank is Context, Ownable, Pausable, ReentrancyGuard {
     ...
     * @param account The address of the account that made the depoist.
...
/**
 * @title Store
 * @dev This contract represents a store that sells NFTs for deposits.
 */
contract Store is Context, Ownable, Pausable, ERC721URIStorage {
    using Address for address payable;
     ...
     */
    event OfferCreated(string indexed id, Offer offer);

    /**
     * @dev Emitted when an offer is purchased.
     * @param buyer The address of the buyer.
     * @param id The ID of the offer.
     * @param tokenId The ID of the purchased NFT.
     * @param deposits The deposits required to purchase the NFT.
     */
    ...
    /**
     * @dev Initializes the contract with the address of the bank contract, the name of the NFT token, and the symbol of the NFT token.
     * @param bankAddress The address of the bank contract.
     * @param tokenName The name of the NFT token.
     * @param tokenSymbol The symbol of the NFT token.
     */
    constructor(
        address bankAddress,
        ...
        _bank = Bank(bankAddress);
    }

    /**
     * @dev Returns the total amount of deposits for a given offer.
     * @param id The ID of the offer.
     * @return The total amount of deposits for the offer.
     */
    function offerAmount(string calldata id) public view returns (int256) {
        return _getDepositsAmount(offers[id].deposits);
    }

    /**
     * @dev Purchases an NFT with a given ID and deposits.
     * @param id The ID of the offer.
     * @notice The bank contract must be approved to transfer the deposits amount in USDC.
     * @notice This function will revert if the offer does not exist.
     * @notice This function will revert if the contract is paused.
     */
    function purchase(string calldata id) public whenNotPaused {
        require(bytes(offers[id].tokenUri).length > 0, "Offer does not exist");

        _bank.deposit(_msgSender(), offers[id].deposits);

        _mintNft(offers[id].tokenUri, _msgSender());
    ...
    /**
     * @dev Mints an NFT with a given token URI and assigns it to an address.
     * @param tokenUri The URI of the token.
     * @param address_ The address to assign the NFT to.
     * @notice Only the contract owner can call this function.
     */
    function mintNft(
        ...
        _mintNft(tokenUri, address_);
    }

    /**
     * @dev Creates a new offer for an NFT with a given ID and deposits.
     * @param id The ID of the offer.
     * @param offer_ The offer to create.
     * @notice Only the contract owner can call this function.
     * @notice This function will revert unless the following requirements are met:
     * - Offer must have at least one deposit;
     * - Offer token URI must not be empty;
     * - Offer must not exist;
     * - Offer deposits amount sum must be divisible by 1e6 (6 decimals);
     * - Offer deposits amount must all be greater than 0;
     */
    function createOffer(
        string calldata id,
        Offer calldata offer_
    ) public onlyOwner {
        require(offer_.deposits.length > 0, "Deposits is empty");
        require(bytes(offer_.tokenUri).length > 0, "Token URI is empty");
        require(offers[id].deposits.length == 0, "Offer already exists");
        require(
            _getDepositsAmount(offer_.deposits) % 1e6 == 0,
            "Amount is not divisible by 1e6 (6 decimals)"
        );

        offers[id].tokenUri = offer_.tokenUri;

        for (uint256 i = 0; i < offer_.deposits.length; i++) {
            require(offer_.deposits[i].amount > 0, "Deposit amount is 0");

            offers[id].deposits.push(offer_.deposits[i]);
        }

        emit OfferCreated(id, offers[id]);
    }

    /**
     * @dev Removes an offer for a given ID.
     * @param id The ID of the offer to remove.
     * @notice Only the contract owner can call this function.
     */
    function removeOffer(string calldata id) public onlyOwner {
        delete offers[id];
    }

    /**
     * @dev Returns the total amount of deposits for an array of deposits.
     * @param deposits_ The array of deposits.
     * @return The total amount of deposits.
     */
    function _getDepositsAmount(
        Bank.Deposit[] memory deposits_
     ...
     * @param address_ The address to assign the NFT to.
     */
    function _mintNft(string memory tokenUri, address address_) private {
