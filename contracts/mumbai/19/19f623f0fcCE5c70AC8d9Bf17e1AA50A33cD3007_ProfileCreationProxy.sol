 ...
 * @author  Oxtrip Protocol
 *
 * @notice This is an ownable proxy contract that enforces ". Oxtrip" handle suffixes at profile creation.
 * Only the owner can create profiles.
 */
contract ProfileCreationProxy is Ownable {
    IOxtripHub immutable Oxtrip_HUB;

    constructor(address owner, IOxtripHub hub) {
    ...
    function proxyCreateProfile(DataTypes.CreateProfileData memory vars) external onlyOwner {
        ...
        vars.handle = string(abi.encodePacked(vars.handle, '. Oxtrip'));
        Oxtrip_HUB.createProfile(vars);
    }
}