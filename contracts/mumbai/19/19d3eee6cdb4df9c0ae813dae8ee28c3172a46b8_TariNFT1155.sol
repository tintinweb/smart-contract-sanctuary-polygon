...
contract ERC165 is IERC165Upgradeable {
...
contract ERC1155 is Context, ERC165, IERC1155Upgradeable , IERC2981 {
    ...
    */    
    ...
    {
        if (to.isContract()) {
            try IERC1155ReceiverUpgradeable (to).onERC1155Received(_operator, from, tokenId, amount, data) returns (bytes4 response) {
    ...
    {
        if (to.isContract()) {
            try IERC1155ReceiverUpgradeable (to).onERC1155BatchReceived(_operator, from, tokenIds, amounts, data) returns (bytes4 response) {
...
interface IMINTNFT1155 {

    function mint(address creator, string memory uri, uint256 supply) external returns(uint256, bool);
}

contract TariNFT1155 is ERC1155 ,Initializable{
     string private _name ;
     string private _symbol; 
    ...
    function initialize(string memory names, string memory symbols,address _operator) external initializer{
        _name=names;
        _symbol=symbols;
         owner = msg.sender;
