...
contract EnergyIXTStaking is IEnergyIXTStaking, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function _claim(bool _ameliaCommitment, uint256 _epoch) internal {
        // Removed these because if user doesn't have anything staked in one epoch it blocks them from claiming from all other epochs

        // if (_ameliaCommitment) require(userEnergyStakedAmelia[msg.sender][_epoch] > 0, "ENERGY STAKING: INSUFFICIENT AMOUNT COMMITTED");
        // else require(userEnergyStaked[msg.sender][_epoch] > 0, "ENERGY STAKING: INSUFFICIENT AMOUNT STAKED");
