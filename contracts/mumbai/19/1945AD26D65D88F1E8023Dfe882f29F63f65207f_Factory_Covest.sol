...
{
	mapping(uint256 => PairsData) private _dataPairing;
	mapping(string => uint256) private _dataStringToParingId;
	 ...
	 *  @return policy The policyDetails address.
	 *  @return nameRegistry The nameRegistry address.
	 */
	...
	{
		if (_dataPairingBool[_poolId] == true) {
			return (
				_dataPairing[_dataStringToParingId[_poolId]].policy,
				_dataPairing[_dataStringToParingId[_poolId]].nameRegistry
			);
		} else {
	 ...
	 *   - `_policyDetails` and `_nameRegistry` must not be the zero address.
	 */
	function createPair(address _policyDetails, address _nameRegistry)
		public
		isSuperManager
	{
		require(_policyDetails != address(0) && _nameRegistry != address(0));
		uint256 Id = pairsarray.length;
		IPolicyDetails _PDS_ = IPolicyDetails(_policyDetails);
		string memory poolName = _PDS_.poolName();
		string memory poolId = _PDS_.poolId();
		_dataPairing[Id] = PairsData(
			poolName,
			poolId,
			_policyDetails,
			_nameRegistry
		);
		_dataStringToParingId[poolId] = Id;
		pairsarray.push(poolId);
		_dataPairingBool[poolId] = true;

		emit PairCreated(poolName, poolId, _policyDetails, _nameRegistry, Id);
	}
}
