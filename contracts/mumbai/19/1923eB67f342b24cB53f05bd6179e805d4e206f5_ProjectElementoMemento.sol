...
contract ProjectElementoMemento is ERC1155, ERC1155Holder {
    string private _baseMetadataApi = "";
    string private _baseUri = "";
    uint16 nextTokenId = 1;
    uint16 maxSupply = type(uint16).max - 1;
    address public _admin;

    mapping(address => bool) allowedMinters;
    mapping(uint16 => bool) isTokenPoap;
    mapping(uint16 => string) mementoCodes;
    mapping(uint16 => uint256) mementoExpiry;
    ...
    constructor(
        ...
        _admin = msg.sender;
        _baseUri = baseUri;
        allowedMinters[_admin] = true;
        // console.log('DEPLOYED', block.timestamp);
    }
    ...
    function transferFromContract(uint256 tokenId, uint256 amount) public onlyAdmin {
    ...
    /**
     * @dev Mints MEMENTO to sender's wallet
     */
    function _AdminMint(uint256 supply) public onlyAllowedMinter hasSupply {
        _AdminMintTo(msg.sender, supply);
    }

    /**
     * @dev Mints MEMENTO to parameter wallet
     */
    function _AdminMintTo(address wallet, uint256 supply) public onlyAllowedMinter hasSupply {
        __mint(wallet, supply);
    }


    function _AdminMintPOAP(uint256 supply, string memory code, uint256 expiry)
    public onlyAllowedMinter hasSupply {
    ...
    /**
     * @dev INTERNAL: calls ERC1155 _mint
     */
    function __mint(address wallet, uint256 supply) internal {
        _mint(wallet, nextTokenId, supply, "");
        isTokenPoap[nextTokenId] = false;
        nextTokenId++;
    }

    function __compareStrings(string memory a, string memory b) pure internal returns (bool) {
        return keccak256(bytes(a)) == keccak256(bytes(b));
    }

    function claimMemento(uint16 tokenId, string memory code) public {
        require(isTokenPoap[tokenId], "Token is not POAP");
        require(claimedPoaps[msg.sender][tokenId] == false, "Limit to 1 per wallet");
        require(block.timestamp < mementoExpiry[tokenId], "Claiming is expired for this token");
