...
contract Shotgun is IShotgun, ReentrancyGuard, ERC1155Holder, Ownable {
    ...
    constructor(IDibbsERC721Upgradeable _nftAddr, IDibbsERC1155 _fractionAddr) {
        nftAddr = _nftAddr;
    ...
    function getPrice(uint256 _amount, uint256 id)
        public
        view
        returns (uint256)
    {
    ...
    function startAuction(uint256 _tokenId, uint256 _amount)
        external
        payable
        override
        nonReentrant
    ...
    function purchase(uint256 id) external payable override nonReentrant {
        require(
            auctionInfos[id].status == ShotgunStatus.ONGOING,
            "Shotgun: is not started yet."
        );
    ...
    function claimEtherAfterFinishing(uint256 id)
        external
        override
        nonReentrant
    {
        ...
        require(
            msg.sender == auctionInfos[id].starter,
            "Shotgun: only starter can redeem the locked ETH"
        );

        (bool success, ) = payable(msg.sender).call{
            value: auctionInfos[id].restEthers + auctionInfos[id].ethers
        }("");
    ...
    function sendAndRedeemProportion(uint256 id)
        external
        override
        nonReentrant
    {
        require(isAuctionExpired(id), "Shotgun: is not expired yet.");
        ...
        if (
            fractionAddr.balanceOf(address(this), tokenId) ==
            TOTAL_FRACTION_AMOUNT
        ) {
            auctionInfos[id].status = ShotgunStatus.OVER;
    ...
    function withdrawNFT(uint256 id) external override {
        ...
        require(
            msg.sender == auctionInfos[id].starter,
            "Shotgun: only starter can withdraw the NFT"
        );
        require(isAuctionExpired(id), "Shotgun: is not expired yet.");
...
interface IShotgun {
    ...
    function changeAuction_Duration(uint256 newDuration) external;
}
