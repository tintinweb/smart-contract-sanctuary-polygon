...
contract FrameItMarketplace is FrameItContext {
    ...
    event BuyLootBoxEvent(address indexed _seller, address indexed _buyer, uint256 _saleID, uint256 _amount, uint256 _price, address _token);
    ...
    function setPrimarySaleFullLootBox(address _nftAddress, uint24 _numberOfNFTs, uint256 _price, address _token, uint64 _startDate, uint64 _endDate) external {
        ...
        emit PrimarySaleLootBoxEvent(_msgSender(), _nftAddress, _numberOfNFTs, sale._price, _token, primarySaleIDNonceCounter, _startDate, _endDate);
    }
    ...
    // TODO. Configurar el límite que se puede comprar    
    ...
    function buyPrimarySaleLootBox(uint256 _saleID, uint24 _amount, bool _payInRungies) external payable {
        ...
        for (uint256 i=0; i<_amount; i++) {
            sale._sold++;
            require(IERC721(sale._nft).ownerOf(sale._sold) == sale._nft, "NFTIdNotFound");
            IERC721(sale._nft).safeTransferFrom(sale._nft, _msgSender(), sale._sold);
        }

        address salesWallet = IFrameItLootBox(sale._nft).salesWallet();

        if (_payInRungies == false) {
            if (sale._token == address(0)) payable(salesWallet).transfer(msg.value);
        ...
        emit BuyLootBoxEvent(sale._owner, _msgSender(), _saleID, _amount, msg.value, sale._token);
    }
    ...
    function buySecondarySale(uint256[] calldata _saleIDs) external payable {
        ...
        for (uint256 i=0; i<_saleIDs.length; i++) {
            ...
            if (sale._token == address(0)) payable(salesWallet).transfer(sale._price);
            else IERC20(sale._token).safeTransferFrom(_msgSender(), salesWallet, sale._price);
    ...
    function acceptOffer(bytes calldata _message, bytes memory _messageLen, bytes calldata _signature) external payable {
        // NFT owner signature
        ...
        if (_token == address(0)) payable(salesWallet).transfer(_price);
