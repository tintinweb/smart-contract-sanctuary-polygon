...
contract PolygonNFT721Batch is ERC721URIStorage, ERC2771Context, Ownable {
    ...
    mapping(uint256 => uint256) public totalMintedUnderBatchId;

    event BatchMintERC721(uint256 indexed batchId, uint256 indexed from, uint256 indexed to);
    ...
    function mint(address user, uint256 from, uint256 to, string memory _uri, uint256 batchId) external {
        require(_msgSender() == collectionOwner || _msgSender() == collectionMinter, "Not a owner!");
        require(batchId > 0, "Incorrect batchId");
        require( ((batchId * 100) - 100) <= from && from <= (batchId * 100) - 1, "Incorrect range or batchId!");
        require( ((batchId * 100) - 100) <= to && to <= ((batchId * 100) - 1), "Incorrect range or batchId!");

        if(totalMintedUnderBatchId[batchId] == 0) {
            require(from == ((batchId * 100) - 100), "Incorrect starting range");
        } else {
            require(from == (((batchId * 100) - 100) + totalMintedUnderBatchId[batchId]), "Incorrect starting range");
        }

        uint256 totalNFTsBeforeMint = totalNFTsMinted;

        for(uint256 i = from; i <= to; i++) {
            ++totalMintedUnderBatchId[batchId];
            ++totalNFTsMinted;
            _mint(user, i);
            _setTokenURI(i, string(abi.encodePacked(_uri, '/', i.toString(), '.json')));
        }

        if(totalNFTsMinted > totalNFTsBeforeMint) {
            emit BatchMintERC721(batchId, from , to);
        }
...
contract PolygonNFTCollectionBatch is ERC2771Context, Ownable {
    ...
    function mintUnderCollection(address collection, string calldata _session, address user, uint256 from, uint256 to, string memory _uri, uint256 batchId) external {
        if(_msgSender() == operator || collectionRecords[_msgSender()][_session] == address(collection)) {
            PolygonNFT721Batch(collection).mint(user, from, to, _uri, batchId);
        } else {
