...
contract FactoryNFT {
    ...
    event TokenCreated(address owner, address tokenContract,uint tokenId); //emitted when ERC1155 token is deployed
    ...
    function deployContract(string memory _contractName, string memory _symbol ,string memory _uri) public returns (address) {
        NFT token = new NFT(_contractName, _symbol,_uri);
        tokens.push(token);
        emit ChildContractCreated(msg.sender,address(token),_contractName);
        return address(token);
    }

    function createToken(uint _indexOfContract,string memory _name, bool _limited, uint128 _limit,bool _allowed, bool _allowedList) public {
        tokens[_indexOfContract].createToken(_name,_limited,_limit,_allowed,_allowedList);
        emit TokenCreated(tokens[_indexOfContract].owner(), address(tokens[_indexOfContract]),_indexOfContract);
    }


    function mintNFT(uint _indexOfContract,address to) public {

        // mint function
        tokens[_indexOfContract].mintTokenItem(to);
        emit NFTMinted(tokens[_indexOfContract].owner(), address(tokens[_indexOfContract]),_indexOfContract);
    }

    function mintNFTOther(uint _indexOfContract,address to) public {

        // mint function
        tokens[_indexOfContract].mintTokenItemOther(to);
        emit NFTMinted(tokens[_indexOfContract].owner(), address(tokens[_indexOfContract]),_indexOfContract);
    }
   /*
