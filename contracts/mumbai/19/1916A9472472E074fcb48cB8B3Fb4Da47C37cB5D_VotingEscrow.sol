...
import {IERC20 as ERC20Votes} from '@openzeppelin/contracts/token/ERC20/extensions/ERC20Votes.sol';

interface IERC20 is ERC20Votes {
...
contract VotingEscrow is IVotingEscrow, ReentrancyGuard {
    ...
    Point[1000000000000000000] public pointHistory; // 1e9 * user_point_history-length, so sufficient for 1e9 users
    ...
    modifier checkBlocklist() {
        require(blocklist != address(0), 'Blocklist contract not defined');
        require(!IBlocklist(blocklist).isBlocked(msg.sender), 'Blocked contract');
        _;
    }

    function userPointHistory(address user, uint256 timestamp) external view override returns (Point memory) {
        return user_point_history[user][timestamp];
    }
    ...
    function getLastUserPoint(address _addr) external view returns (int128 bias, int128 slope, uint256 ts) {
        ...
        Point memory point = user_point_history[_addr][uepoch];
    ...
    function _checkpoint(address _addr, LockedBalance memory _oldLocked, LockedBalance memory _newLocked) internal {
        ...
        if (_addr != address(0)) {
            ...
            if (_oldLocked.end > block.timestamp && _oldLocked.delegated > 0) {
                userOldPoint.slope = _oldLocked.delegated / int128(int256(_oldLocked.end - _oldLocked.start));
            ...
            if (_newLocked.end > block.timestamp && _newLocked.delegated > 0) {
                userNewPoint.slope = _newLocked.delegated / int128(int256(_newLocked.end - _newLocked.start));
            ...
            uint256 uEpoch = userPointEpoch[_addr];
            if (uEpoch == 0) {
                user_point_history[_addr][uEpoch + 1] = userOldPoint;
            }

            userPointEpoch[_addr] = uEpoch + 1;
            userNewPoint.ts = block.timestamp;
            userNewPoint.blk = block.number;
            user_point_history[_addr][uEpoch + 1] = userNewPoint;
    ...
    function createLock(uint256 _value, uint256 _unlockTime) external override nonReentrant checkBlocklist {
        _createLock(_value, _unlockTime, _value);
    }

    // See IVotingEscrow for documentation
    function _createLock(uint256 _value, uint256 _unlockTime, uint256 _veValue) internal checkBlocklist {
        ...
        locked_.delegated += int128(int256(_veValue));
        locked_.delegatee = msg.sender;
        locked[msg.sender] = locked_;
        stakersCount += 1;
        _checkpoint(msg.sender, LockedBalance(0, 0, 0, 0, address(0)), locked_);
    ...
    // See IVotingEscrow for documentation
    function createLockByPeriod(uint256 amount, LockPeriods period) external override {
        _createLock(amount, block.timestamp + _getLockTimeFromPeriod(period), _getMultipliedAmount(amount, period));
    }
    ...
    function _getLockTimeFromPeriod(LockPeriods period) internal pure returns (uint256 lockTime) {
        if (period == LockPeriods.oneWeek) {
            return 1 weeks;
        } else if (period == LockPeriods.threeMonths) {
            return 3 * 4 weeks;
        } else if (period == LockPeriods.sixMonths) {
            return 6 * 4 weeks;
        } else if (period == LockPeriods.oneYear) {
            return 365 days;
        }
    }

    function _getMultipliedAmount(uint256 amount, LockPeriods period) internal pure returns (uint256 multipliedAmount) {
        if (period == LockPeriods.oneWeek) {
            return amount;
        } else if (period == LockPeriods.threeMonths) {
            return (amount * 1250) / 1000;
        } else if (period == LockPeriods.sixMonths) {
            return (amount * 1500) / 1000;
        } else if (period == LockPeriods.oneYear) {
            return amount * 2;
        }
    }
    ...
    function _copyLock(LockedBalance memory _locked) internal pure returns (LockedBalance memory) {
        return LockedBalance({amount: _locked.amount, start: _locked.start, end: _locked.end, delegatee: _locked.delegatee, delegated: _locked.delegated});
    }
    ...
    function _findTimestampEpoch(uint256 _timestamp, uint256 _maxEpoch) internal view returns (uint256) {
        // Binary search
        uint256 min = 0;
    ...
    function _findUserBlockEpoch(address _addr, uint256 _block) internal view returns (uint256) {
        ...
        for (uint256 i = 0; i < 128; i++) {
            ...
            uint256 mid = (min + max + 1) / 2;
            if (user_point_history[_addr][mid].blk <= _block) {
                min = mid;
            } else {
                max = mid - 1;
            }
    ...
    // @param max_epoch Don't go beyond this epoch
    // @return Epoch which contains _timestamp
    function _findTimestampUserEpoch(address _addr, uint256 _timestamp, uint256 max_epoch) internal view returns (uint256) {
        uint256 min = 0;
        uint256 max = max_epoch;
        for (uint256 i = 0; i < 128; i++) {
            if (min >= max) {
                break;
            }
            uint256 mid = (min + max + 1) / 2;
            if (user_point_history[_addr][mid].ts <= _timestamp) {
                min = mid;
            } else {
                max = mid - 1;
            }
    ...
    function balanceOf(address _owner) public view override returns (uint256) {
        ...
        Point memory lastPoint = user_point_history[_owner][epoch];
    ...
    function balanceOfAtT(address _owner, uint256 _ts) external view returns (uint256) {
        ...
        if (_ts != block.timestamp) {
            epoch = _findTimestampUserEpoch(_owner, _ts, userPointEpoch[_owner]);
        }
        if (epoch == 0) {
            return 0;
        }
        Point memory lastPoint = user_point_history[_owner][epoch];
        lastPoint.bias = lastPoint.bias - (lastPoint.slope * int128(int256(_ts - lastPoint.ts)));
        if (lastPoint.bias < 0) {
            lastPoint.bias = 0;
        }
        return uint256(uint128(lastPoint.bias));
    ...
    function balanceOfAt(address _owner, uint256 _blockNumber) public view override returns (uint256) {
        ...
        Point memory upoint = user_point_history[_owner][userEpoch];
    ...
    // See IVotingEscrow for documentation
    function totalSupplyAtT(uint256 _t) public view override returns (uint256) {
        require(_t <= block.timestamp, 'Only past timestamps');

        uint256 epoch_ = globalEpoch;
        if (_t == block.timestamp) {
            epoch_ = globalEpoch;
        } else {
            epoch_ = _findTimestampEpoch(_t, globalEpoch);
        }
