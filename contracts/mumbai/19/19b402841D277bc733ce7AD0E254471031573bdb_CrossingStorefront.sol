...
contract CrossingStorefront is ICrossingNFT, AccessControl {
    ...
    mapping(bytes32 => ProofOfIntegrityId) private proofOfIntegrityToId;
    ...
    function addCollectible(string memory _uri, uint256 _price)
      public
      onlyRole(CURATOR_ROLE)
      returns (uint256)
    {
      ...
      collectibles[collectibleId] = Collectible(true, _uri, _price);
    ...
    function checkCollectibleId(uint256 _collectibleId)
      internal
      view
      returns (bool)
    {
      return collectibles[_collectibleId].valid;
    }

    function getCollectiblePrice(uint256 _collectibleId)
      public
      view
      returns (uint256)
    {
      require(checkCollectibleId(_collectibleId), "Invalid collectible ID");
      return collectibles[_collectibleId].price;
    }

    function updateCollectiblePrice(uint256 _collectibleId, uint256 newPrice)
      public
      onlyRole(CURATOR_ROLE)
    {
      require(checkCollectibleId(_collectibleId), "Invalid collectible ID");
      collectibles[_collectibleId].price = newPrice;
    }

    function mintCollectible(uint256 _collectibleId)
      public
      payable
      returns (uint256)
    {
      require(checkCollectibleId(_collectibleId), "Invalid collectible ID");
      require(msg.value == collectibles[_collectibleId].price, "Wrong price");

      string memory _uri = collectibles[_collectibleId].collectibleURI;
      delete collectibles[_collectibleId];
      uint256 tokenId = crossingNFTContract.safeMint(msg.sender, _uri);
      ...
      returns (uint256)
    {
      ...
      proofOfIntegrityToId[proofOfIntegrity] = ProofOfIntegrityId(true, tokenId);
    ...
    function revealMysteryBox(uint256 mysteryBoxId, string[] memory newURIs)
      public
      onlyRole(CURATOR_ROLE)
    {
      require(newURIs.length == mysteryBoxCollections[mysteryBoxId].size, "List of new URIs my be equal in size to the mystery box collection");
      uint256 uriCount = newURIs.length;
      for(uint256 i = 0; i < uriCount; i++){
        bytes32 proofOfIntegrity = keccak256(abi.encode(newURIs[i]));
        if(proofOfIntegrityToId[proofOfIntegrity].valid){
          uint256 tokenId = proofOfIntegrityToId[proofOfIntegrity].tokenId;
          crossingNFTContract.revealNFT(tokenId, newURIs[i]);
        }
      }
    }
}
