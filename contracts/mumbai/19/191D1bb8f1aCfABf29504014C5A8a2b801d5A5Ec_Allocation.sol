...
// Original by: Sreepriya Kalarikkal Sreekumar
// Modified by: Ben DeAngelis

pragma solidity >=0.8.2 <0.9.0;

import "./Product.sol";
import "./EmissionsToken.sol";

/*
this file contains pseudocode for emission allocations to products.
ERC3525 or ERC1155 or ERC721 contract  for Product token
ERC3525 contract for emissions (and reference token if using CET reference tokens)
Custom contract for all other functions, e.g., emission allocation to product, fetching emissions_list for parties 
and any other non standard features
*/
contract Allocation {

    address private _owner;
    address private _productAddress;
    address private _emissionsAddress;

    // report/shipment/product
    struct Product_Struct {
        uint256 productId;
        /* Slot of a token can be determined given the tokenId. 
        This approach to emission allocation for a product will help track the entire list of emissions for a specific product 
        irrespective of which party created those emissions. scope of these emissions are relative to the parties involved 
        and that can be determined with the help of the ownership and slot data of these emissions. 
        for example, if party-A creates emissions during manufacturing, they will mint the emission tokens 
        (slot1 tokens for scope1 emissions, slot2 tokens for scope2 emissions and slot3 tokens for scope3 emissions if they 
        also mint their own scope3 emissions, mostly applicable to the very first party in our system)
        this gives the end-to-end carbon footprint for a given product irrespective of who created it or who owns it at any given 
        point in time 
        */
        uint256[] emissionIDs;  
    }

    event Allocate(uint256 prod_id, uint256 emit_id);

    // stores list of all product tokens minted by the system and its corresponding emissions
    mapping(uint256 => Product_Struct) productMapping;

    constructor (address owner_, address prodAddress_, address emitAddress_) {
        _owner = owner_;
        _productAddress = prodAddress_;
        _emissionsAddress = emitAddress_;
    }
    ...
    function getProductAddress() view external returns (address)  {
        return _productAddress;
    }

    function setProductAddress(address prodAddress_) public {
        require(msg.sender == _owner,
            "Allocation: only owner can change the allocation addresses");
        _productAddress = prodAddress_;
    }

    function getEmissionAddress() view external returns (address) {
        return _emissionsAddress;
    }

    function setEmissionAddress(address emitAddress_) public {
        require(msg.sender == _owner,
            "Allocation: only owner can change the allocation addresses");
        _emissionsAddress = emitAddress_;
    }

    function registerProduct( uint256 id_) external {
        productMapping[id_].productId = id_;
    }

    // NOTE: Might need to prevent duplicate allocations
    function mapEmissionToProduct(uint256 product_id, uint256 emission_id) public {
        productMapping[product_id].emissionIDs.push(emission_id);
        emit Allocate(product_id, emission_id);
    }

    function getEmissionsForProduct(uint256 product_id) public view returns (uint256[] memory) {
        return productMapping[product_id].emissionIDs;
    }
}
...
contract EmissionsToken is ERC3525 {
    ...
    */
    function mint( address to_, uint256 slot_, uint256 value_, string memory uri_) public returns (uint256){
        uint256 newID = _mint(to_, slot_, value_);
        updateURI(newID, uri_);
        emit Mint(to_, newID);
        return newID;
    }
...
abstract contract Product is ERC3525  {
    ...
    /* 
        * Mint a new product token
        * @param to_ the recipient address
        * @param id_ ID of the token
        * @param slot_ slot of the token
        * @param value_ value of token
        * @param uri_ URI of token metadata
    */
    function mint( address to_, uint256 id_, uint256 slot_, uint256 value_, string memory uri_) public {
        _mint(to_, id_, slot_, value_);
        updateURI(id_, uri_);
    }

    /*
        * Transfers token between two address
        * @param from address that starts with token
