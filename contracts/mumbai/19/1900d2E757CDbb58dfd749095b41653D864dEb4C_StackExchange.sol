...
contract StackExchange is Ownable {
  ...
  function getUserPoolInfo(address token, address user) external view returns (uint shares, uint rewards) {
    Pool memory pool = poolMap[token];
    PoolUser memory poolUser = poolUserMap[token][user];
  ...
  function swap(address sellToken, address buyToken, address receiver, uint sellAmount) external {
    ...
    if (sellToken != baseToken) {
      ...
      require(sellPool.baseReserve > 0, "SELL_POOL_ZERO");
    ...
    if (buyToken != baseToken) {
      ...
      require(buyPool.baseReserve > 0, "BUY_POOL_ZERO");
