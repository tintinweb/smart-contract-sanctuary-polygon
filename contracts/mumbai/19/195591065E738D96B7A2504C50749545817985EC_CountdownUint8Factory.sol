...
import "../../traits/implementers/CountdownUint8Implementer.sol";


contract CountdownUint8Factory is BlackHolePrevention {
    ...
    uint8                               constant            traitType    = 105;
    uint256                             constant            version      = 20230621;

    event CountdownUint8ImplementerCreated(address b8,address traitRegAddress,uint16 _traitId,uint8 _defaultValue);

    error RegistryNotLoaded();
    error CommunityListNotLoaded(address clAddr);
    error CommunityRegistryNotLoaded(address crAddr);
    error TraitRegistryNotLoaded(uint32 projectID,string traitRegistryName,address crAddr);
    error FactoryIsNotTraitRegistryAdmin();
    ...
    struct inputTraitStruct {
        ...
        uint8   defaultValue;               // default value returned for a tokenId that had not been claimed
    }


    function addTrait(
        inputTraitStruct calldata data
    ) external {
        require(theRegistry.getRegistryAddress("TRAIT_TYPE_105_FACTORY") == address(this),"CountdownUint8Factory: unauthorised");
        createCountdownUint8Implementer(
            data.communityId,
            data.start,
            data.end,
            data.tokenNum,
    ...
    function createCountdownUint8Implementer( 
        uint32 communityId,
        uint16 start,
        uint16 end,
        uint32 registryNumber,
        string memory _name,
        string memory _ipfsHash,
        uint8 _defaultValue
    ) internal  {

        string memory traitRegistryName = string.concat("TRAIT_REGISTRY_",uint256(registryNumber).toString());
        (CommunityRegistry myCommunityRegistry, address traitRegAddress) = findMyTraitRegistry(communityId,traitRegistryName);
        ...
        if (regAddress.code.length == 0) {
            revert TraitRegistryNotLoaded(communityId,traitRegistryName,address(myCommunityRegistry));
        }

        ECRegistryV3c traitRegistry = ECRegistryV3c(traitRegAddress);
        bytes32 TRAIT_REGISTRY_ADMIN = traitRegistry.TRAIT_REGISTRY_ADMIN();
        if (!myCommunityRegistry.hasRole(TRAIT_REGISTRY_ADMIN,address(this))) revert FactoryIsNotTraitRegistryAdmin();


        uint16 _traitId = traitRegistry.traitCount();

        CountdownUint8Implementer b8 = new CountdownUint8Implementer(traitRegAddress,_traitId,start,end,_defaultValue); // <- start end
        ECRegistryV3c.traitStruct[] memory ts = new ECRegistryV3c.traitStruct[](1);
        ...
        ts[0].id = _traitId;
        ts[0].traitType = traitType;
        ts[0].start = start;
        ts[0].end = end;
        ts[0].enabled = true;
        ts[0].storageImplementer = address(b8);
        ts[0].ipfsHash = _ipfsHash;
        ts[0].name = _name;
        traitRegistry.addTrait(ts);

        emit CountdownUint8ImplementerCreated(address(b8),traitRegAddress,_traitId,_defaultValue);
    }
    ...
    function findMyTraitRegistry(uint32 projectID,  string memory traitRegistryName) internal returns (CommunityRegistry myCommunityRegistry,address traitRegAddress) {
        ...
        bytes32 COMMUNITY_REGISTRY_ADMIN = myCommunityRegistry.COMMUNITY_REGISTRY_ADMIN();
        require(myCommunityRegistry.isUserCommunityAdmin(COMMUNITY_REGISTRY_ADMIN,msg.sender),"CountdownUInt8Factory Factory : unauthorised");
 ...
 * 
 * 
 * ref : https://www.youtube.com/watch?v=vNuVifA7DSU
 */

import "./TraitUint8ValueImplementer.sol";

contract CountdownUint8Implementer is TraitUint8ValueImplementer {

    uint16         immutable    start;
    uint16         immutable    end;
    uint8          immutable    defaultValue;
    mapping (uint16 => bool)    used;


    constructor(address _registry, uint16 _traitId, uint16 _start, uint16 _end, uint8 _defaultValue) TraitUint8ValueImplementer(_registry,_traitId) {
        start = _start;
        end = _end;
        defaultValue = _defaultValue;
    }

    function getBadgeValue(uint16 tokenId) external view returns (uint8 val) {
        uint8 def = defaultValue;
        val = data[tokenId];
        if (
            (!used[tokenId]) && 
            (def > 0) &&
    ...
    function HandleBadges(uint16[] calldata _tokenIds, uint16 ) external onlyAllowed {
        uint8 def = defaultValue;
        for(uint16 i = 0; i < _tokenIds.length; i++) {            
            ...
            if (
                ...
                used[tid] = true;
                if (val == 0) // do not handle manually set tokenid badges
                    val = def;
            }
            require (val > 0,"One or more badges are not valid");
            data[_tokenIds[i]] = val - 1;
        }
    }
