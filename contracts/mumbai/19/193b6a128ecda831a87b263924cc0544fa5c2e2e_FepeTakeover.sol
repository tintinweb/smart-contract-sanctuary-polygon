...
contract FepeTakeover is ERC721A, DefaultOperatorFilterer, Ownable {
    uint256 public MAX_SUPPLY = 10000;
    uint256 public MINT_PRICE = 0.005 ether;
    uint256 public MAX_PER_WALLET = 8;

    string public baseURI =
        "ipfs://bafybeibq75c4je4jeu5it4uodkbgty3dti5hixkv7g5rfmwrzvsgmuzegu/";
    ...
    constructor() ERC721A("Fepe Takeover", "FEPE") {}

    function mint(uint256 _amount) external payable {
        require(totalSupply() + _amount <= MAX_SUPPLY, "FepeTakeover has been sold out!");
        ...
        uint256 freeCount = 0;
        uint256 paidCount = 0;
        uint256 previousPurchases = nftMinted[msg.sender];

        for (uint256 i = 0; i < _amount; i++) {
            if (previousPurchases % 2 == 0) {
                freeCount++;
                previousPurchases++;
            } else if (previousPurchases % 1 == 0) {
                paidCount++;
                previousPurchases++;
            }
        }

        // Calculate the total cost of the NFTs
        value = (freeCount * 0) + (paidCount * MINT_PRICE);
