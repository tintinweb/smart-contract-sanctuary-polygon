...
contract Marketplace is AccessControl {
    ...
    event RoyaltyFound(address _buyingAssetAddress, uint _tokenId, uint _price);
    event RoyaltyNotFound(address _buyingAssetAddress);
    // buyer platformFee
    ...
    constructor(
        ...
        require(
            _buyerFee > 0 && _sellerFee > 0,
            "Buyer and Seller fee cannot be 0"
        );
        require(
            address(_transferProxy) != address(0),
            "Zero Address Transfer Proxy"
        );
    ...
    */
    ...
    function executeBid(
        ...
        if (order.nftType == BuyingAssetType.ERC721) {
            require(
                IERC721(order.nftAddress).ownerOf(order.tokenId) == seller,
                "Only Seller can execute bid"
            );
        }
        if (order.nftType == BuyingAssetType.ERC1155) {
            require(
                IERC1155(order.nftAddress).balanceOf(seller, order.tokenId) >
                    order.qty,
                "Only Seller can execute bid"
            );
        }
    ...
    function getFees(
        ...
        (tokenCreator, royaltyFee) = getRoyalty(
            buyingAssetAddress,
            tokenId,
            price
        );
    ...
    function getRoyalty(
        address _buyingAssetAddress,
        uint _tokenId,
        uint _price
    ) internal returns (address, uint) {
        try
            IERC2981(_buyingAssetAddress).royaltyInfo(_tokenId, _price)
        returns (address tokenCreator, uint royaltyFees) {
            emit RoyaltyFound(_buyingAssetAddress, _tokenId, _price);
            return (tokenCreator, royaltyFees);
        } catch {
            emit RoyaltyNotFound(_buyingAssetAddress);
            return (_buyingAssetAddress, 0);
        }
    }
