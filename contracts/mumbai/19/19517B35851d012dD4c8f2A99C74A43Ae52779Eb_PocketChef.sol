	...
	MulticallUpgradeable
{
		...
		nonReentrant
	{
		...
		bool shouldStopLoss = registry.shouldStopLoss(pocketId, amountIn, amountOut);
		bool shouldTakeProfit = registry.shouldTakeProfit(pocketId, amountIn, amountOut);
...
OwnableUpgradeable
{
    ...
    function getCurrentQuote(string calldata pocketId) public returns (uint256, uint256) {
        /// @dev Extract necessary info
        (address ammRouterAddress, address baseTokenAddress, address targetTokenAddress,, uint256 batchVolume,,,,,) = registry.getTradingInfoOf(pocketId);
