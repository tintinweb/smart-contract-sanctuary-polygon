...
import "./MockSoulbound1155.sol";

contract NFTSwap2 is ReentrancyGuard, Ownable, ERC1155Holder {
    ...
    function createSwapPair(
        ...
        uint256 _count,
        uint256[][] memory srcIds,
        uint _tokenId,
        address _minterContract
    ) public onlyAdminOrOwner(msg.sender) {
        require(sources.length == srcIds.length,"array length mismatch error");
	for (uint l = 0; l < sources.length; l++) {
        require(
            whitelistedContracts[sources[l]] == true,
            "contract is not whitelisted"
        );
        }

        swapPair memory pair = swapPair(sources, _count, _tokenId, 0, 0, srcIds,true,_minterContract);
        count++;
        pairs[count] = pair;
        emit swapPairCreated(sources, _count, _tokenId, count);
    }

    function setPairDuration(
        uint256 id,
        uint256 start,
        uint256 end
    ) public onlyAdminOrOwner(msg.sender) {
	...
	address source,
        ...
        require(
            pair.startTime < block.timestamp && pair.endTime > block.timestamp,
            "Not within pair swap times"
        );

	(bool ifSourceExists, uint256 sourceIndex) = ifAddressExists(source,pair.sources);
	require(ifSourceExists,"not one of the source ids");

        uint256[] memory acceptedIds = pair.sourceIds[sourceIndex];
        for (uint i = 0; i < _ids.length; i++) {
            uint256 id = _ids[i];
            require(ifExists(id,acceptedIds),"not one of the accepted sourceIds");
        }
	
	uint256 currentCount = userBalances[pairId][recipient];
	if(total + currentCount >= pair.count) {
	userBalances[pairId][recipient] = 0;
        mock1155 = MockSoulbound1155(source);
        mock1155.burnBatch(address(this), _ids, _amts);
        mock1155 = MockSoulbound1155(pair.minterContract);
        mock1155.swapMint(recipient, pair.targetTokenID, 1);
	}
	else{
	userBalances[pairId][recipient] = currentCount + 1;	
	}
    ...
    function onERC1155BatchReceived(
        ...
        for (uint i = 0; i < _amounts.length; i++) {
            require(_amounts[i]==1,"amount cannot be greater than 1");
            amt = amt + _amounts[i];
        }
        

        swapMultiple(pair, _ids, _amounts,msg.sender,_from, amt);
        return this.onERC1155BatchReceived.selector;
    }

    function getUserPairs(address _user) public view returns (uint256[] memory) {
        uint256[] memory pairids = new uint[](count);
        uint256 counter;
        for(uint i=1; i <= count;i++) {
        swapPair memory pair = pairs[i];
        if(pair.startTime < block.timestamp && pair.endTime > block.timestamp && pair.isActive){
            uint256 total = 0;
            for(uint h=0; h < 20;h++) {
                uint256 cnt = ERC1155(pair.sources[0]).balanceOf(_user, h);
                total = total + cnt;
            }
            if(total>=pair.count){
                pairids[counter] = i;
    ...
    function ifExists(uint id,uint[] memory ids) public view returns (bool) {
    ...
    function ifAddressExists(address addr,address[] memory addresses) public view returns (bool,uint256) {
...
contract MockSoulbound1155 is ForjSoulboundERC1155 {
...
contract ForjSoulboundERC1155 is ForjERC1155 {
    ...
    modifier onlySwapContract() {
        require(msg.sender == swapContract, "Msg Sender Is Not Swap Contract");
        _;
    }
    ...
    function setSwapContractAddress(address _swapContract) public onlyAdminOrOwner(msg.sender){
        swapContract = _swapContract;
    }

    function swapMint(address to, uint256 id, uint256 amount) public onlySwapContract {
        this.mint(to, id, amount, "");
    }
}
