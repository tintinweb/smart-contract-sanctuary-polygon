...
contract OHtestX2 is ERC721URIStorage, Ownable, ERC721Enumerable {
    ...
    uint256 public maxSupply = 1980; //1980
    ...
    constructor() ERC721("OHTest X2", "OHT") {
    ...
    function safeMint() public payable returns (uint256) {
        ...
        _setTokenURI(
            tokenId,
            "https://ipfs.moralis.io:2053/ipfs/QmdXat79QVNY63fR5Jf2ceAXj56FsKskvtXWPeMvrhcfvp/OH/Preview.json"
        );
        return tokenId;
    }
    ...
    function _premiumMint(uint256 NumOfTokens)
        public
        isSuperAdmin(msg.sender)
    ...
    function updateURI(RevealData memory data) public isSuperAdmin(msg.sender) {
        _setTokenURI(uint256(data.tokenid), data.tokenuri);
    }

    function updateURIs(RevealData[] memory data)
        public
        isSuperAdmin(msg.sender)
    {
        if (data.length > 0) {
            for (uint256 i = 0; i < data.length; i++) {
                _setTokenURI(uint256(data[i].tokenid), data[i].tokenuri);
            }
        }
    }
