...
contract ButovskyNFT is ERC721, ERC721URIStorage, ERC721Burnable, Ownable {
    ...
    function generateSVG(string memory _title) internal returns (string memory) {
        bytes memory svg = abi.encodePacked(
            '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350">',
                '<style>.base { fill: white; font-family: serif; font-size: 40px; font-weight: 600; font-family: "Helvetica", "Arial", sans-serif; }</style>',
                '<linearGradient id="gradient" x1="0" x2="0.5" y1="0" y2="1">',
                    '<stop offset="0%" stop-color="black" stop-opacity="0" />',
                    '<stop offset="100%" stop-color="', colors[pseudoRandom(10)], '" />',
                '</linearGradient>',
                '<rect width="100%" height="100%" fill="url(#gradient)"/>',
                '<text x="50%" y="90%" class="base" dominant-baseline="middle" text-anchor="middle">', _title,'</text>',
            '</svg>'
        );
    ...
    function generateJSON(Metadata memory _metadata) internal returns (string memory) {
        string memory titleWithDomain = string(abi.encodePacked(_metadata.title, domain));
        string memory svg = generateSVG(titleWithDomain);
        bytes memory json = abi.encodePacked(
            '{',
                '"name": "', titleWithDomain, '",',
                '"description": "', _metadata.description, '",',
                '"image": "', svg, '",',
                '"attributes": ', _metadata.attributes, // must be [{}] JSON object
            '}' 
        );
    ...
    function safeMint(Metadata memory _metadata)
        public
        onlyOnce
    {
        ...
        string memory json = generateJSON(_metadata);
