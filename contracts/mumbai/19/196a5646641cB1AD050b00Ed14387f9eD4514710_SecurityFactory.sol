...
interface IDividendEvent {
  ...
  function setTokenExists(address token) external;
...
contract SecurityFactory {
  ...
  modifier onlyRole(bytes32 _role) {
      ...
      require(IAccessControl(_accessControlAddress).hasRole(_role, msg.sender), "Caller has no right");
      _;
    }
  ...
  function createSecurityToken(
    ...
    string calldata _docName,
    string calldata _docUUID,
    address _USDT
  ) external onlyRole(MANAGER_ROLE) returns (address cloneAddr) {
    require(
      keccak256(abi.encodePacked(_name)) != keccak256(abi.encodePacked("")) &&
        keccak256(abi.encodePacked(_symbol)) !=
        keccak256(abi.encodePacked("")) &&
        keccak256(abi.encodePacked(_docName)) !=
        keccak256(abi.encodePacked("")) &&
        keccak256(abi.encodePacked(_docUUID)) !=
        keccak256(abi.encodePacked("")) &&
        _USDT != address(0), "SEF02"
    );
    cloneAddr = Clones.clone( implementation);
    SecurityToken(cloneAddr).__SecurityToken_init(
      _name,
      _symbol,
      addressRegistry,
      _docName,
    ...
    IDividendEvent(hub).setTokenExists(cloneAddr);
  ...
  // -----------------------------------------
  // Getters access control addresses
  // -----------------------------------------

  function getEventAddr() public view returns (address) {
    return IAddressRegistry(addressRegistry).getDividendEventAddr();
  }
...
import "../hub-event/IDividendEvent.sol";
  ...
  ERC20PermitUpgradeable
{
  ...
  using IterableMapping for IterableMapping.HolderMap;
  using SafeERC20 for IERC20;

  IterableMapping.HolderMap private holderMap;

  bytes32 public constant MANAGER_ROLE = keccak256("MANAGER_ROLE");
  bytes32 public constant BLOCKLISTED_TRANSFER_ROLE = keccak256("BLOCKLISTED_TRANSFER_ROLE");
  ...
  uint256 public excludeFromDividendValue;

  mapping(address => bool) excludedFromDividend;
  ...
  event SetNewRegistryAddress(address newAddress);
  event SetNewDividendToken(address newAddress);
  ...
  function __SecurityToken_init(
    ...
    string memory docName_,
    string memory docURI_,
    address dividendToken_
  ) external initializer {
    addressRegistry = addressRegistry_;
    require(_isFactory(msg.sender));
    __ERC20_init(name_, symbol_);
    __ERC20Burnable_init();
    __Pausable_init();
    __ERC20Permit_init(name_);
    _setDocument(docName_, docURI_);
  ...
  {
      require(!excludedFromDividend[account], "Account is excludedFromDividend");
      excludedFromDividend[account] = true;
      uint256 bal = this.balanceOf(account);
      excludeFromDividendValue += bal;
      emit ExcludeFromDividend(account);
  }

  function includeInDividends(address account)
        external
        onlyRole(MANAGER_ROLE)
  {
      require(excludedFromDividend[account], "Account is not excluded");
      excludedFromDividend[account] = false;
      uint256 bal = this.balanceOf(account);
      excludeFromDividendValue -= bal;
  ...
  function setNewDividendToken(address token) public onlyRole(MANAGER_ROLE) {
        require(token != dividendToken, "CRF03");
        dividendToken = token;
        emit SetNewDividendToken(token);
  }
  ...
  function emergencyWithdrawTether() external onlyRole(MANAGER_ROLE) {
        uint256 balance = getTetherContractBalance();
        require(balance > 0, "No token in contract");
        IERC20(dividendToken).safeTransfer(msg.sender, balance);
  ...
  // -----------------------------------------
    ...
    function withdrawStuckMATIC() external onlyRole(MANAGER_ROLE) {
        require(address(this).balance > 0, "withdrawStuckMATIC: There are no ETH in the contract");
        payable(msg.sender).transfer(address(this).balance);
    } 

    function withdrawStuckERC20(address token, address to) external onlyRole(MANAGER_ROLE) {
        require(IERC20(token).balanceOf(address(this)) > 0, "withdrawStuckERC20: There are no tokens in the contract");
  ...
  function distributeDividends(uint256 _startIndex, uint256 _endIndex, uint256 totalDividendVal)
        external
        onlyRole(MANAGER_ROLE)
        whenPaused
    {
        require(totalDividendVal > 0, "Dividend value is zero");
        ...
        uint256[] memory dividendValues = new uint256[](_endIndex- _startIndex + 1);

        uint256 totalSupply = getEligibleToDividendSupply();

        uint256 startIndex = _startIndex;

        for (uint256 i = startIndex; i <= _endIndex;  ) {
            
            address investor = holderMap.getAddressAtIndex(i);
            if (!excludedFromDividend[investor]) {
              uint256 balance = holderMap.getBalanceOf(investor);
              uint256 value = (balance * totalDividendVal) / totalSupply;
              IERC20(dividendToken).safeTransfer(investor, value);
              investorsAddress[i - startIndex] = investor;
              dividendValues[i - startIndex] = value;
            }
            unchecked { ++i; }
        }
        address eventAddr = getEventAddr();
        IDividendEvent(eventAddr).dividendDistribution(
            dividendToken,
            investorsAddress,
            dividendValues
            );
  ...
  function getTetherContractBalance() public view returns (uint256) {
  ...
  function getAddressAtIndex(uint256 index) public view returns(address) {
    return holderMap.getAddressAtIndex(index);
  }
  ...
  function getEligibleToDividendSupply() public view returns(uint256) { 
    ...
    return ts - excludeFromDividendValue;
  }
  ...
  function _beforeTokenTransfer(
    ...
    address _accessControlAddress = getRoleAddr();
    bool isFromBlocklisted = IAccessControl(_accessControlAddress).hasRole(
      BLOCKLISTED_TRANSFER_ROLE,
      from
    );
    bool isToAdmin = IAccessControl(_accessControlAddress).hasRole(
      MANAGER_ROLE,
      to
    );

    bool isToWhitelisted = IAccessControl(_accessControlAddress).hasRole(
      WHITELISTED_ROLE,
      to
    );
    require(!isFromBlocklisted || (isFromBlocklisted && isToAdmin) || isToWhitelisted, "_beforeTokenTransfer: not allowed");
    super._beforeTokenTransfer(from, to, amount);
    // update balance
    if (from == address(0)) {
            // mint
            // update to
            if(excludedFromDividend[to]) {
              excludeFromDividendValue += amount;
            }
        } else if (to == address(0)) {
            // burn
            // update from
            if(excludedFromDividend[from]) {
              excludeFromDividendValue -= amount;
            }
        } else {
            // transfer
              if(excludedFromDividend[from] && !excludedFromDividend[to]) {
                excludeFromDividendValue -= amount;
              } if(!excludedFromDividend[from] && excludedFromDividend[to]) {
                excludeFromDividendValue += amount;
              }
  ...
  function _afterTokenTransfer(
        ...
        if (from == address(0)) {
          ...
          balReceiver > 0 ? _updateBalance(to, balReceiver) : _remove(to);
        } else if (to == address(0)) {
          // burn
          // update from
          uint256 balSender = this.balanceOf(from);
          balSender > 0 ? _updateBalance(from, balSender) : _remove(from);
        } else {
          // transfer
          uint256 balSender = this.balanceOf(from);
          uint256 balReceiver = this.balanceOf(to);
  ...
  function _isFactory(address sender) internal view returns (bool) {
    return sender == IAddressRegistry(addressRegistry).getTokenFactAddr();
  }
}
...
library IterableMapping {
    ...
    function getBalanceOf(HolderMap storage map, address key)
        external
        view
        returns (uint256)
    {
    ...
    function getAddressAtIndex(HolderMap storage map, uint256 index)
        external
        view
        returns (address)
    {
