...
contract NFTMarketplace is ReentrancyGuard, ERC721URIStorage, Ownable{
    ...
    function tokenIdOwner(uint256 tokenId) public view returns(address){
        if(MarketItemDatabase[tokenId].nftOwner == address(0)){
            return MarketItemDatabase[tokenId].nftCreator;
        } else {
            return MarketItemDatabase[tokenId].nftOwner;
        }
    }
