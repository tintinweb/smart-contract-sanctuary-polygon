...
contract Marketplace is Ownable, Pausable, MarketplaceStorage {
   ...
   */
  function cancelOrder(bytes32 orderId) public whenNotPaused returns(Order memory) {
   ...
   */
  function executeOrder(bytes32 orderId, uint256 acceptedTokenIndex, bytes calldata signature) public payable whenNotPaused {
  ...
  function _cancelOrder(bytes32 orderId) internal returns(Order memory) {
    ...
    address sender = _msgSender();
    require(sender == owner() || order.seller == sender, "Marketplace: Not owner or seller of the order");
    _closeOrder(orderId);
    return order;
  }
