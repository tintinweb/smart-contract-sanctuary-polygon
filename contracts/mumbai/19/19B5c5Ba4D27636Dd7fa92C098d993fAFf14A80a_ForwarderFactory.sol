...
contract ForwarderFactory is
  ...
  function hasRole(bytes32 role, address account) external view override(IAccessControl, IForwarderFactory) returns (bool) {
  ...
  function emitNativeReceived(
    address _sender,
    address _forwarderAddress,
    uint256 _amount
  ) external {
    require(
      _msgSender() == _forwarderAddress &&
      isForwarderDeployed(_forwarderAddress),
      "!forwarder"
    );
  ...
  function setValidUntil(uint256 _validUntil) external onlyOwner whenPaused {
