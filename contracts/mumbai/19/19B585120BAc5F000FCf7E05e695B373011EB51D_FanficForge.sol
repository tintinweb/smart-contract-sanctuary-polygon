...
contract FanficForge is ERC721, ERC721URIStorage, Ownable {
    ...
    mapping(string => uint256) public usernameToKudos;

    // NFT base metadata
    string public external_url = 'https://localhost:3000/explore/?tokenId=';
    string public image_data_url = 'https://localhost:3000/api/?tokenId=';
    ...
    constructor() ERC721('FanficForge', 'FIC') {}
    ...
    function mint(
        ...
        for (uint i = 0; i < kudos.length; i++) {
            tokenIdToFanfic[tokenId].kudos.push(
                Kudo({username: kudos[i].username, cites: kudos[i].cites})
            );
        }
        ...
        for (uint i = 0; i < kudos.length; i++) {
            usernameToKudos[kudos[i].username] += kudos[i].cites;
        }
    }
