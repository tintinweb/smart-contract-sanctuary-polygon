...
contract BUSDLotto is Ownable {
    ...
    function buyTicket(uint256 numberOfTickets, address referral) external {
    ...
    unchecked {
        if (referral != address(0) && referrals[msg.sender] == address(0)) {
        ...
        if (developerWallet != msg.sender) {
            ...
            bool transferStatusCon = IBEP20(BUSDContract).transferFrom(msg.sender, address(this), numberOfTickets * developerReward);
            require(transferStatusCon, "Lottery: Transfer BUSD error (dev-con)");
            bool transferStatusDev = IBEP20(BUSDContract).transferFrom(address(this), developerWallet, numberOfTickets * developerReward);
            require(transferStatusDev, "Lottery: Transfer BUSD error (con-dev)");
        }
    ...
    function claim() external {
        ...
        require(_reward > 0);
        require(IBEP20(BUSDContract).balanceOf(address(this)) >= _reward);
        rewards[msg.sender] -= _reward;

        bool approveStatus = IBEP20(BUSDContract).approve(msg.sender, _reward);
        require(approveStatus, "Lottery: Approve claim BUSD error");
        bool claimStatus = IBEP20(BUSDContract).transferFrom(address(this), msg.sender, _reward);
    ...
    function withdraw() external onlyOwner {
        ...
        IBEP20(BUSDContract).approve(msg.sender, balance);
        IBEP20(BUSDContract).transferFrom(address(this), msg.sender, balance);
    }
