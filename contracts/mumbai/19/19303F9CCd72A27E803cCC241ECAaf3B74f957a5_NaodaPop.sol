...
contract NaodaAdminManager is AccessControlEnumerableUpgradeable, OwnableUpgradeable {

    bytes32 public constant NAODA_ADMIN_ROLE = keccak256("NAODA_ADMIN_ROLE");

    function __NaodaAdminManager_init() internal onlyInitializing {
        __NaodaAdminManager_init_unchained();
    }

    function __NaodaAdminManager_init_unchained() internal onlyInitializing {
        __Ownable_init_unchained();
        _grantRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _grantRole(NAODA_ADMIN_ROLE, _msgSender());
    }
    ...
    function isContractAdmin(address account) public view returns (bool){
        return hasRole(NAODA_ADMIN_ROLE, account) || hasRole(DEFAULT_ADMIN_ROLE, account) || owner() == account;
    }
...
contract NaodaPop is
    ...
    function initialize(string memory baseURI, string memory _contractURI, address[] memory admins) public virtual initializer {
        __NaodaPop_init(baseURI, _contractURI, admins);
    }

    function __NaodaPop_init(string memory baseURI, string memory _contractURI, address[] memory admins) internal onlyInitializing {
        __NaodaPop_init_unchained(baseURI, _contractURI, admins);
    }

    function __NaodaPop_init_unchained(string memory baseURI, string memory _contractURI, address[] memory admins) internal onlyInitializing {
        contractURI = _contractURI;
        for (uint i = 0; i < admins.length; i++) {
            _grantRole(NAODA_ADMIN_ROLE, admins[i]);
        }
        __Pausable_init_unchained();
        __Ownable_init_unchained();
        __NaodaAdminManager_init_unchained();
        ...
        if (paymentTokenAmount > 0) {
            if (paymentTokenAddress == address(0)) {
                require(
                    ...
                    "Insufficient MATIC amount received"
                );
            } else {
