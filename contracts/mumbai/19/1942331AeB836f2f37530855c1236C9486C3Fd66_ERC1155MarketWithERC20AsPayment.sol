...
// File contracts/exchange/ERC1155MarketWithERC20AsPayment.sol
...
contract ERC1155MarketWithERC20AsPayment is ERC1155Holder, Ownable {
	...
	function createMarketSale(
		...
		uint256 inventory = idToMarketItem[itemId].amount;
		require(msg.value >= _serviceCharge, "Not enough service charge paid");
		require(amount <= inventory, "Not enough inventory");

		// pay by specified coin token
		uint256 price = idToMarketItem[itemId].price;
		address coinContract = idToMarketItem[itemId].coinContract;
		uint256 amountApproved = IERC20(coinContract).allowance(
			msg.sender,
			address(this)
		);
		require(
			amountApproved >= price * amount,
