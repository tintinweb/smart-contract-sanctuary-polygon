...
import {ISmardexFactory} from "./interfaces/ISmardexFactory.sol";
...
contract PairDeployer {
    ...
    IERC20Factory constant ERC20_FACTORY = IERC20Factory(address(0xE7350189633530E5941756024FB5cAbcc8b665F9));

    constructor() {}

    function deployPair(address factory) external returns (address pair, address tokenA, address tokenB) {
        /* ----------------------------- Deploy token A ----------------------------- */
        string memory tokenAName = string.concat("tokenA-", pairCount.toString());
        string memory tokenASymbol = string.concat("TA", pairCount.toString());
        ERC20_FACTORY.deployERC20(tokenAName, tokenASymbol);
        (tokenA,) = ERC20_FACTORY.deployed(ERC20_FACTORY.getLengthDeployed() - 1);

        /* ----------------------------- Deploy token B ----------------------------- */
        string memory tokenBName = string.concat("tokenB-", pairCount.toString());
        string memory tokenBSymbol = string.concat("TB", pairCount.toString());
        ERC20_FACTORY.deployERC20(tokenBName, tokenBSymbol);
        (tokenB,) = ERC20_FACTORY.deployed(ERC20_FACTORY.getLengthDeployed() - 1);

        /* ------------------------------- Create pair ------------------------------ */

        pair = ISmardexFactory(factory).createPair(tokenA, tokenB);
...
interface IERC20Factory {
    function deployed(uint256 index) external returns (address, uint256);
    function getLengthDeployed() external returns (uint256);
