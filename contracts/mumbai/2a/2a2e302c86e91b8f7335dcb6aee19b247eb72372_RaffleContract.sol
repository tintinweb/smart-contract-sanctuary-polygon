...
contract RaffleContract is ERC721Holder {
    ...
    function endRaffle(uint256 raffleId, uint256 winnerTicket) external {
        ...
        address winner = tickets[raffleId][winnerTicket];

        IERC721 nft = IERC721(nftAddress);
        nft.safeTransferFrom(address(this), winner, raffleId);

        IERC20 token = IERC20(tokenAddress);
        uint256 totalTokens = raffles[raffleId].ticketPrice.mul(raffles[raffleId].ticketSold).mul(TOKEN_DECIMALS);
