    ...
    NativeMetaTransaction
{
    ...
    function __ERC721Tradeable_init(
        ...
        // Counter is initialized to 1, since starting at 0 leads to higher gas cost for the first minter
    ...
    function _mintTo(address to, string memory uri) internal {
        require(bytes(uri).length != 0, "Error: Token URI is empty!");
    ...
    /**
     * @dev Batch mint tokens to an address with a tokenURI.
     * @param _recipients addresses of the future owner of the token
     * @param _tokenURIs uris of the token metadata
     */
    function batchMint(
        address[] calldata _recipients,
        string[] calldata _tokenURIs
    ) external onlyRole(MINTER_ROLE) {
        require(_tokenURIs.length != 0, "Error: Token URIs are empty!");
        require(_recipients.length != 0, "Error: Recipients are empty!");
        require(
            _recipients.length == _tokenURIs.length,
            "Error: Recipients are empty!"
        );
        for (uint256 i; i < _recipients.length; i++) {
            _mintTo(_recipients[i], _tokenURIs[i]);
        }
    }
