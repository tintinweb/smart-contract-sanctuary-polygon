...
contract StakingPoolUpdatableFixedAPR is Ownable, ReentrancyGuard, Metadata {
    ...
    struct RewardInfo {
        ...
        uint256 actualAPR; // if actual APR is 20%, then expectedAPR =  ( 20 / 100 ) * 1e18. Percentage APR is scaled up by e18.
        uint256 rewardTokenPrice; //rate in USD scaled up by 1e18 like in crowdsale
    }
    ...
    struct FarmInfo {
        ...
        uint256 endTimestamp;
        uint256 inputTokenPrice; //rate in USD scaled up by 1e18 like in crowdsale
    }
    ...
    event ActualAPRUpdated(uint256 _actualAPR, uint256 _rewardTokenIndex);
    ...
    function init(bytes memory extraData) external {
        ...
        rewardPool.push(
            RewardInfo({
                ...
                actualAPR: (expectedAPR.mul(rewardTokenPrice)).div(
                    farmInfo.inputTokenPrice
                )
            })
        );
    ...
    function updateTokenPrices(
        ...
        farmInfo.inputTokenPrice = _inputTokenPrice;
        RewardInfo storage rewardInfo = rewardPool[_rewardTokenIndex];
        rewardInfo.rewardTokenPrice = _rewardTokenPrice;
    ...
    function updateActualAPR(uint256 _actualAPR, uint256 _rewardTokenIndex)
        external
        onlyOwner
    {
        RewardInfo storage rewardInfo = rewardPool[_rewardTokenIndex];
        rewardInfo.actualAPR = _actualAPR;
        emit ActualAPRUpdated(_actualAPR, _rewardTokenIndex);
    }
