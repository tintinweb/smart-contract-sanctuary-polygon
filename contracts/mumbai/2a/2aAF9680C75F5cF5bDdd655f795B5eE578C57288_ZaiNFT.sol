...
contract ZaiNFT is ERC721Enumerable, Ownable {
    ...
    constructor(address _zaiMeta) ERC721("BandZai_NFT_ZAI", "ZAI") {
    ...
    function _createZaiDatas(uint256 _newItemId,string memory _name, uint256 _state, address _to) internal{
        ...
        uint256 random = _getRandom(_to, _ipfsId, _newItemId);
    ...
    function preMintZais(uint256 _start) external onlyOwner{
        uint256 _final = _start + 5;
        for(uint256 i = _start ; i < _final ;){
            _preMintZai(i);
            unchecked{ ++i; }
        }
    }
    ...
    function _preMintZai(uint256 _level) private {
        _tokenIds.increment();
        uint256 _newItemId = _tokenIds.current();
        address storageAddress = gameAddresses.getLevelStorageAddress();
        _safeMint(storageAddress, _newItemId);
        _createZaiDatas(_newItemId, "challenger", 0, storageAddress);
        ZaiStruct.Zai storage z = _zai[_newItemId];

        z.level = _level;
        z.xp = zaiMeta.getNextLevelUpPoints(_level);

        uint256 random = _getRandom(address(this), z.metadata.ipfsPathId, _newItemId);
        ...
        require(ILevelStorage(storageAddress).addFighter(_level,_newItemId));
    }
    ...
    function updatePowers(
        ...
        require(
            ...
            IDelegate(gameAddresses.getDelegateZaiAddress()).gotDelegationForZai(_id, msg.sender),
            "Not yours nor Delegated");
        z.creditForUpgrade -= (_water + _fire + _metal + _air + _stone);
        z.powers = zaiMeta.updatePowers(z.level, z.powers, ZaiStruct.Powers(_water,_fire,_metal,_air,_stone), _isGod[_id]);
    ...
    function updateAlchemyXp(uint256 _tokenId, uint256 _xpRaised) external onlyAuth{
        _zai[_tokenId].alchemyXp += _xpRaised; 
    }
    ...
    function _getRandom(address _to, uint256 _id, uint256 _nonce) internal returns(uint256){
        return IOracle(gameAddresses.getOracleAddress()).getRandom(keccak256(abi.encodePacked(_to, _id, _nonce, block.timestamp)));
    }
