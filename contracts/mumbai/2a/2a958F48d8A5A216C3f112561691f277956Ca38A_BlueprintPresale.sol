...
import "./interfaces/IBlueprintNFT.sol";

contract BlueprintPresale is
  OwnableUpgradeable,
  EIP712Upgradeable,
  ...
  ReentrancyGuardUpgradeable
{
  ...
  IBlueprintNFT public nft;

  event SetSaleConfig(uint256 _startTime, uint256 _endTime, uint256 _supply, uint256 _price);
  event UpdateBlacklist(address indexed _address, bool _status);
  event WithdrawETH(address indexed _sender, uint256 _balance);
  event BuyPrivate(address indexed _sender, uint256 _numberOfNft, uint256 _timestamp);
  event BuyPublic(address indexed _sender, uint256 _numberOfNft);

  /**
   * @dev Upgradable initializer
   */
  function __BlueprintPresale_init(IBlueprintNFT _nft) external initializer {
    __Ownable_init();
    __AccessControl_init();
    __EIP712_init("BlueprintPresale", "1.0.0");
  ...
  /**
   * @notice Update blacklist status of address
   * @dev Only callable by owner
   * @param _address Address of user
   * @param _status Status of address in blacklist
   */
  function updateBlacklist(address _address, bool _status) external onlyOwner {
    blacklist[_address] = _status;

    emit UpdateBlacklist(_address, _status);
  }
   ...
   * @param _timestamp Timestamp of signatures
   * @param _signature Signature of whitelist
   * @dev Error Details
   * - 0x1: User in black list
   * - 0x2: There is no private sale running
   * - 0x3: Invalid price
   * - 0x4: User can't be muy more than max nft allowed
   * - 0x5: Invalid signature
   * - 0x6: Signature already userd
   * - 0x7: User can't buy more than private supply
   */
  function buyPrivate(
    ...
    require(blacklist[sender_] == false, "0x1");
    ...
    require(_verify(_hash(sender_, _numberOfNft, _timestamp), _signature), "0x5");
    require(usedSignature[_signature] == false, "0x6");
    require(privateCount + _numberOfNft <= privateConfig.supply, "0x7");
    ...
    emit BuyPrivate(sender_, _numberOfNft, _timestamp);
  }
  ...
  function buyPublic(uint256 _numberOfNft) external payable nonReentrant {
    ...
    emit BuyPublic(sender_, _numberOfNft);
  }
  ...
  function _hash(
    ...
    return
      _hashTypedDataV4(
        keccak256(
          abi.encode(
            keccak256("BlueprintPresale(address _user,uint256 _numberOfNft,uint256 _timestamp)"),
            _user,
            _numberOfNft,
            _timestamp
          )
  ...
  /**
   * @dev Verify whitelist signature
  ...
  function _verify(bytes32 _digest, bytes memory _signature) internal view returns (bool) {
    return hasRole(VALIDATOR_ROLE, ECDSAUpgradeable.recover(_digest, _signature));
  }
}
