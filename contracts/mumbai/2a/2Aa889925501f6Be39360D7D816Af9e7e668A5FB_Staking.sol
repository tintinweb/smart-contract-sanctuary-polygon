...
contract Staking is
    ...
    function setToken(address _tokenAddress, address _penaltyWallet)
        external
        onlyOwner
        whenNotPaused
    {
...
contract Deposit is ReentrancyGuard {
    ...
    uint256 public lockedDuration = 60 * 60 * 24 * 90;

    uint256 public unstakePenaltyDuration = 60 * 60 * 24 * 180;

    uint256 public unstakePenaltyBP = 500;

    uint256 public monthlyInterestRateBP = 116;
    ...
    constructor(
        ...
        stakingContract = Staking(msg.sender);
    ...
    function addInterestAllocation(uint256 _amount, uint256 _date) external nonReentrant onlyStakingContract {
        interestPayAmounts[interestPayIndex] = _amount;
        interestPayDates[interestPayIndex] = _date;
    }

    function lockedUntilDate() internal view returns (uint256) {
        return depositDate + lockedDuration;
    }

    function unstakePenaltyUntilDate() internal view returns (uint256) {
        return depositDate + unstakePenaltyDuration;
    }
    ...
    function closeDeposit() external nonReentrant {        
        ...
        if (unstakePenaltyUntilDate() < getCurrentTime()) {
            ...
            uint256 penalty = (amount * unstakePenaltyBP) / BASIS_POINTS_10000;
    ...
    function calculateInterest() public view returns(uint256 _interest) {
        uint256 depositDuration = getCurrentTime().sub(depositDate);
        uint256 months = depositDuration.div(MONTH);
        uint256 interest = months.mul(monthlyInterestRateBP).div(BASIS_POINTS_10000);

        return interest;
    }

    function retrieveInterest() external nonReentrant {
        require(
            lockedUntilDate() <= getCurrentTime(),
            "Staking deposit is still locked"
        );
