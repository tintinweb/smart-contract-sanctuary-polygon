...
contract COECardPacks is ERC1155, Ownable, ReentrancyGuard {
  ...
  constructor(address _helper) ERC1155("COE Card Packs") {
    totalTokens = 0;
    helper = _helper;
  ...
  function mintBatch(
    ...
    for (uint256 i = 0; i < _ids.length; i++) {
      require(
        !tokens[_ids[i]].paused && tokens[_ids[i]].exists && _ids[i] != 0,
        "Token is paused/nonexist."
      );
      totalCost += tokens[_ids[i]].price * _amounts[i];
    }

    // purchaseWithToken(totalCost, _paymentTokenAddress); //in contract
  ...
  function fpMintBasic(address _to) public nonReentrant {
  ...
  //   string memory _uri
  // ) external onlyOwner {
  //   Library.NewTokenData memory data = Library.NewTokenData({
  //     id: _id,
  //     price: _price,
  //     maxSupply: _maxSupply,
  //     cardCount: _cardCount,
  //     rarityAllocations: _rarityAllocations,
  //     races: _races,
  //     elements: _elements,
  //     packType: _packType,
  //     season: _season,
  ...
  //   Library.CardPack memory newToken = Library.addNewPack(
  //     data,
  //     msg.sender,
  //     tokens[data.id].exists
  //   );
  //   tokens[data.id] = newToken;
  //   totalTokens++;
  //   // Library.addNewPack(tokens, totalTokens, data, msg.sender);
  // }
  ...
  function pauseUnpauseTokens(
    bool _paused,
    uint256[] memory _ids
  ) public onlyOwner {
    for (uint256 i = 0; i < _ids.length; i++) {
      tokens[_ids[i]].paused = _paused;
    }
  }
  ...
  //       checkAllowedToken(_tokenAddress, allowedTokens),
  ...
  //     _totalCost,
  //     _tokenAddress,
  //     address(priceFeeds[_tokenAddress])
  //   );

  //   require(
  //     AEGInterface(_tokenAddress).balanceOf(msg.sender) >= tokenAmount,
  ...
  //     if (_allowedTokens[i] == _tokenAddress) {
  //       return true;
  //     }
  //   }
  //   return false;
