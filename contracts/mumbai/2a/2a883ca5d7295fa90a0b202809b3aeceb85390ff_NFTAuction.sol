...
contract NFTAuction is Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    event AuctionStarted(uint256 indexed auctionId, uint256 tokenId, uint256 reservePrice, uint256 startTime, uint256 endTime, uint256 addTime);
    ...
    modifier auctionHasEnded(uint256 auctionId) {
        ...
        require(
            block.timestamp >= auction.endTime, "auction has not ended yet"
        );
        _;
    }
    ...
    function getWinner(uint256 _auctionId) external view hasAuction(_auctionId) auctionHasEnded(_auctionId) returns (address) {
    ...
    /// Bid on the auction.
    function bid(uint256 _auctionId, uint256 _bidPrice) external hasAuction(_auctionId) payable {
        ...
        if (auction.holderOnlyContractList.length > 0) {
            ...
            for (uint256 i = 0; i < auction.holderOnlyContractList.length; i++) {
                ...
                require(isHolder, "This auction is in `Holder Only Mode`, only holders can buy tickets.");
            }
        }

        if (auction.token == IERC20(address(0))) {
            require(msg.value == _bidPrice, "Send wrong amount of bid price.");
        } else {
            auction.token.transferFrom(msg.sender, address(this), _bidPrice);
        }
    ...
    function adminClaimRevenue(uint256 _auctionId) external hasAuction(_auctionId) payable nonReentrant {
        AuctionInfo storage auction = auctions[_auctionId];
