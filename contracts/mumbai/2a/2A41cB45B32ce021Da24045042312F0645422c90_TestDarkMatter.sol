...
// File: contracts\Lite-Dark-Exchange\DarkNFTs\Knights\Dark_Knight.sol
...
contract TestDarkMatter is ERC721 {
    ...
    function awardTitle() public returns (uint256) {
        require(listedAddresses[msg.sender] == true, "!listed");
        _tokenIds.increment();
        ...
        listedAddresses[msg.sender] = false;

        return newItemId;
    }

    function listNewKnight(address[] memory _addresses) public {
        require(msg.sender == gov, "!gov");
        uint256 i = 0;
        for (i; i < _addresses.length; i++) {
            listedAddresses[_addresses[i]] = true;
        }
    }
}