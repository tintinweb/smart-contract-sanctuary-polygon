...
contract Pay is IPay, PayConfig {
    ...
    event Order(string orderId, uint256 paidAmount,address paidToken,uint256 orderAmount,address settleToken,uint256 fee,address merchant, address payer);
    ...
    function pay(
        ...
        address settleCurrency = iMerchant.getSettleCurrency(_merchant);
        if(address(0) != settleCurrency) {

            require(IERC20(settleCurrency).balanceOf(msg.sender) >= _paiAmount, "Balance insufficient");

            if(_currency != settleCurrency) {

                paidAmount = swapExactOutputSingle(_currency, _paiAmount, settleCurrency, _orderAmount);

            } else {

                TransferHelper.safeTransferFrom(_currency, msg.sender, address(this), _orderAmount);
                paidAmount = _orderAmount;
        ...
        if (iMerchant.getAutoSettle(_merchant)) {
            fee = _autoWithdraw(_merchant, settleCurrency, _orderAmount);
        } else {
            fee = getFee(_orderAmount);
            merchantFunds[_merchant][settleCurrency] += _orderAmount - fee;
        }
        ...
        emit Order(_orderId, paidAmount, _currency, _orderAmount, settleCurrency, fee, _merchant, msg.sender);
    ...
    function payWithETH(
        ...
        if(address(0) != settleCurrency) {
            ...
            if(WETH9 != settleCurrency) {
                paidAmount = swapExactOutputSingle(WETH9, msg.value, settleCurrency, _orderAmount);
            }

        } else {
            settleCurrency = WETH9;
        ...
        emit Order(_orderId, paidAmount, address(0), _orderAmount, settleCurrency, fee, _merchant, msg.sender);
    ...
    function claimToken(
        ...
        _claim(settleAccount, _currency, _amount, _withdrawAddress);
    ...
    function claimEth(
        ...
        _claim(settleAccount, address(0), _amount, _withdrawAddress);
    ...
    function claimAllToken() external {
        ...
        require(iMerchant.isMerchant(msg.sender), "Invalid merchant");
        address settleAccount = iMerchant.getSettleAccount(msg.sender);
        if(address(0) == settleAccount) {
            settleAccount = msg.sender;
        }
...
interface IPay {
    ...
    function payWithETH(string memory _orderId, address _merchant, uint256 _settleAmount) external payable returns(bool);

    function claimEth(address _withdrawAddress) external;

    function claimEth(uint256 _amount, address _withdrawAddress) external;

    function claimToken(address _currency, address _withdrawAddress) external;

    function claimToken(address _currency, uint256 _amount, address _withdrawAddress) external;
