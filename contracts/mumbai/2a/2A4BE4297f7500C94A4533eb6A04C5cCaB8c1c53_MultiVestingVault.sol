...
contract MultiVestingVault is Ownable {
    ...
    uint256 private immutable _releaseAfter;
    ...
    constructor(ERC20Mintable token_, uint256 releaseAfter_, uint256 duration_, uint256 periods_) {
        require(address(token_) != address(0), "MultiVestingVault: token has zero address");
        require(releaseAfter_ > 0, "MultiVestingVault: release period must be positive");
    ...
    function releaseAfter() public view returns (uint256) {
    ...
    function remainingTokens(address beneficiary) public view returns (uint256 remaining) {
        Vesting memory balance = _balances[beneficiary];

        remaining = balance.tokensVested - balance.tokensReleased;
    }
    ...
    function addVesting(address beneficiary, uint256 tokens) public onlyOwner {
        ...
        if (_balances[beneficiary].timeOfFirstPurchase == 0) {
            _balances[beneficiary].timeOfFirstPurchase = block.timestamp;
        }
    ...
    function release() public {
        Vesting memory balance = _balances[msg.sender];
        uint256 timeOfFirstPurchase = balance.timeOfFirstPurchase;

        require(balance.tokensVested > 0, "MultiVestingVault: no tokens to release");
        require(block.timestamp >= timeOfFirstPurchase + _releaseAfter, "MultiVestingVault: release time has not come");
    ...
    function availableTokens(address beneficiary) public view returns (uint256) {
        ...
        uint256 releaseTime = info.timeOfFirstPurchase + _releaseAfter;
        if (block.timestamp < releaseTime || info.timeOfFirstPurchase == 0) {
            return 0;
        }

        uint256 periodsElapsed = ((block.timestamp - releaseTime) / _duration) + 1;
