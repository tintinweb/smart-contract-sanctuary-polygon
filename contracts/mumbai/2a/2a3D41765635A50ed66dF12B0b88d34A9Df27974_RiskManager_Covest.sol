...
contract RiskManager_Covest is
	Initializable,
	UUPSUpgradeable,
	ReentrancyGuardUpgradeable
{
	...
	mapping(address => RiskProfiler) private _riskProfile;
	mapping(address => bool) private _isApproved;

	event SignatureApproved(
		address indexed user,
		uint8 indexed riskCoefficient,
	...
	/**
	 * @notice Get maxRiskCoefficient
	 * @dev maxriskCoefficient of this pool.
	 * @return The uint8 value.
	 */
	uint8 public maxRiskCoefficient;
	...
	function initialize(address _nameRegistry, uint8 _maxRiskCoefficient)
		public
		initializer
	{
		require(
			_maxRiskCoefficient > 0,
			'maxRiskCoefficient should be more than 0.'
		);
		...
		maxRiskCoefficient = _maxRiskCoefficient;
	}
	...
	modifier isUnderWriter() {
		require(UW.isUnderWriter(msg.sender), "You aren't Underwriter.");
		_;
	}

	/**
	 * @notice approveUser.
	 * @dev approve user to use this pool with riskCoefficient.
	 * @param _user The user address.
	 * @param _riskCoefficient The uint8 value of riskCoefficient.
	 *
	 * Requirements:
	 *  - `nonReentrant` Can't call multiple when the first tx is pending of a user.
	 *  - `isReady` The pool must not paused.
	 *  - `msg.sender` Must be a UnderWriter.
	 *  - `_riskCoefficient` Must be less than maxRiskCoefficient.
	 *  - `_riskCoefficient` Must be more than 0.
	 *  - `_user` Must be not approved.
	 *  - `_user` Must not be UnderWriter.
	 */
	function approveUser(address _user, uint8 _riskCoefficient)
		public
		nonReentrant
		isReady
		isUnderWriter
	{
		require(
			UW.isUnderWriter(_user) == false,
			"You are UnderWriter You can't approve your self."
		);
		require(
			_riskCoefficient > 0 && _riskCoefficient <= maxRiskCoefficient,
			'RiskManager : Risklevel Error.'
		);
		require(_isApproved[_user] != true, 'RiskManager: Already Approve.');

		S.update(msg.sender, IStaking.StakingTypes.UW);
		_isApproved[_user] = true;
		_riskProfile[_user] = (RiskProfiler(true, _riskCoefficient));

		emit SignatureApproved(_user, _riskCoefficient, true);
	}

	/**
	 * @notice reApproveUser.
	 * @dev change riskCoefficient of user that already approved.
	 * @param _user The user address.
	 * @param _riskCoefficient The uint8 value of riskCoefficient.
	 *
	 * Requirements:
	 ...
	 *  - `_riskCoefficient` Must be less than maxRiskCoefficient.
	 *  - `_riskCoefficient` Must be more than 0.
	 *  - `_user` Must be approved.
	 *  - `_user` Must not be UnderWriter.
	 */
	function reApproveUser(address _user, uint8 _riskCoefficient)
		public
		nonReentrant
		isReady
		isSuperManager
	{
		...
		require(_isApproved[_user] == true, 'RiskManager: Not Approve.');
	...
	/**
	 * @notice Get isApprove.
	 * @param _user The user address.
	 * @return The boolean value that mean this user is approved or not.
	 */
	function isApprove(address _user) public view returns (bool) {
		return _isApproved[_user];
	}

	/**
	 * @notice Get riskCoefficientOfUser.
	 * @param _user The user address.
	 * @return The uint8 value of riskCoefficient.
	 */
	function riskCoefficientOfUser(address _user) public view returns (uint8) {
		return (_riskProfile[_user].riskCoefficient);
	}

	/**
	 * @notice Get riskCoefficientOfUser.
	 * @param _user The user address.
	 * @return approved The boolean value that mean this user is approved or not.
	 * @return riskCoefficient The uint8 value of riskCoefficient.
	 */
	...
	{
		return (
			_riskProfile[_user].approved,
			_riskProfile[_user].riskCoefficient
		);
	}
}
