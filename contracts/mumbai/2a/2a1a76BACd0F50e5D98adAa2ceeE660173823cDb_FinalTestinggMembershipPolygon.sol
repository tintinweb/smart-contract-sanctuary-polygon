 ...
 */
interface IFinalTestinggMembershipPolygon {
    /**
...
contract FinalTestinggMembershipPolygon is
    IFinalTestinggMembershipPolygon,
    ERC721AQueryable,
    ERC2981,
    Ownable,
    ReentrancyGuard
{
    ...
    /// @param _root set root for whiltlist if provided 
    ...
    constructor(
        ...
        bytes32 _root
    ) ERC721A("Final Testingg Membership Polygon", "FTMP") {
    ...
    function allowlistMint(
        ...
        if (numberMinted(to) + (quantity) > maxPerAddressDuringMint)
            revert ExceedsMaxMintLimit();
        if (totalSupply() + (quantity) > collectionSize)
            revert ExceedsSupply();
            // we have checked above quantity must not be greator than maxperAddressDuringMint ther for no need to check for overflow or underflow
        unchecked {
            _setAux(to, uint64(quantity + (_getAux(to))));
        }
            _safeMint(to, quantity);
            unchecked {
    ...
    if (totalSupply() + (quantity) > amountForDevs)  revert ExceedsDevMintLimit();
    if (totalSupply() + (quantity) > collectionSize)  revert ExceedsSupply();
    if (quantity % (maxBatchSize) != 0) revert NotMultipleOfMaxBatchSize();
