...
contract GBotUpgrader is ERC721Receiver, Ownable, Pausable, TokenTimelock, UpgraderProperties {
  ...
  // Mappings tokenId => newMetadata
  mapping(uint256 => uint256) private newGBotMetadata;
  ...
  event GBotEvolutionStarted(address indexed owner, uint256 tokenId, uint256 gmeeCost, uint256 ompCost, uint256 newMetadata, uint256 lockTimestamp);
    ...
    function upgradeGBot(
        ...
        // Upgrade properties
        newGBotMetadata[tokenId] = newMetadata;
    ...
    function evolveGBot(uint256 tokenId, bytes calldata sig) public virtual whenNotPaused {
        ...
        uint256 ompCost = GBotCalculations.determineEvolutionOmpCost(rarity, calculateCop(metadata));
        require(ompCost <= ompToken.balanceOf(_msgSender()), "Not enough OMP");
    ...
    function claimGBot(uint256 tokenId) private {
        require(isRightfulOwner(tokenId) == true, "GBot Upgrader: Not the rightful owner");
        _releaseLock(tokenId, _msgSender());
        
        GBotContract.upgradeGBot(tokenId, newGBotMetadata[tokenId]);
        newGBotMetadata[tokenId] = 0;
    ...
    function isBabyEvolvable(uint256 rarity, uint256 cop) internal pure {
      // Baby Bot cannot upgrade past evolution point
          if (rarity == GBOT_RARITY_COMMON) {
            require(cop >= EVOLUTION_POINTS_COMMON, "Baby GBot cannot be evolved, not enough cop");
          }
          if (rarity == GBOT_RARITY_RARE) {
            require(cop >= EVOLUTION_POINTS_RARE, "Baby GBot cannot be evolved, not enough cop");     
          }
          if (rarity == GBOT_RARITY_EPIC) {
            require(cop >= EVOLUTION_POINTS_EPIC, "Baby GBot cannot be evolved, not enough cop");
          }
          if (rarity == GBOT_RARITY_LEGENDARY) {
            require(cop >= EVOLUTION_POINTS_LEGENDARY, "Baby GBot cannot be evolved, not enough cop");
          }
          if (rarity == GBOT_RARITY_MYTHICAL) {
            require(cop >= EVOLUTION_POINTS_MYTHICAL, "Baby GBot cannot be evolved, not enough cop");
          }
          if (rarity == GBOT_RARITY_ULTIMATE) {
            require(cop >= EVOLUTION_POINTS_ULTIMATE, "Baby GBot cannot be evolved, not enough cop");
          }
    }
