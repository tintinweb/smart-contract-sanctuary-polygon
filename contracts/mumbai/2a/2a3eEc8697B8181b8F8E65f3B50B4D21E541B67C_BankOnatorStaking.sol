...
contract BankOnatorStaking is Ownable, ReentrancyGuard {
    ...
    function stake(uint256 tokens) external nonReentrant {
        ...
        //         tokens + _stakes[msg.sender].stakedAmount,
        ...
        if (_stakes[msg.sender].stakedAmount != 0) {
            ...
            _stakes[msg.sender].totalEarned.add(newRewardAmount);
        }
    ...
    function unstake(uint256 unstakeAmount) external nonReentrant {
        ...
        uint256 resultingUnstakeAmt = _stakes[msg.sender].stakedAmount.sub(
            unstakeAmount
        );
        ...
        _stakes[msg.sender].stakedAmount = resultingUnstakeAmt;
        _stakes[msg.sender].lastStakedTime = block.timestamp;
    ...
    function getTotalRewards(address user) public view returns (uint256) {
        ...
        uint256 lastStakedTime = _stakes[user].lastStakedTime;
        uint256 totalStaked = _stakes[user].stakedAmount;
