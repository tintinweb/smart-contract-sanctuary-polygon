...
/// @title The Floor NFT descriptor

pragma solidity ^0.8.6;

import {Ownable} from "@openzeppelin/contracts/access/Ownable.sol";
import {Strings} from "@openzeppelin/contracts/utils/Strings.sol";
import { IFundraiserCollectionDescriptor } from './lib/IFundraiserCollectionDescriptor.sol';
import { NFTDescriptor } from './lib/NFTDescriptor.sol';

contract FundraiserCollectionDescriptor is IFundraiserCollectionDescriptor, Ownable {
    ...
    function dataURI(uint256 _tokenId) public view returns (string memory) {
        ...
        string memory name = string(abi.encodePacked('Fundraiser Collection #', tokenId));
        string memory image = string(abi.encodePacked('ipfs://', imageURI, '/image-', tokenId, '.', ext));
        string memory description = string(abi.encodePacked('Fundraiser Collection #', tokenId));

        return genericDataURI(name, image, description);
    }
...
interface IFundraiserCollectionDescriptor {
    function tokenURI(uint256 tokenId) external view returns (string memory);
