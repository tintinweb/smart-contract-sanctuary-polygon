...
contract MyNft is ERC721, Ownable, ERC2981 {
    ...
    address[] public withdrawAddresses = [0x11eec0923C7c49796482cdEB4F158b348166a783, 0x5cd819CAa5e1fDd7b9E29dA8aE4036E85330fd21, 0x0f44989866e3894294159F5a0fFB7EB907Df85D9];
    uint256[] public withdrawPercentages = [65, 11, 24];


    bytes32 public merkleRoot = 0x4348eab8dd12130a081cfa0b1887f61e2833456a331a1b9cc3e3af78279b6ca4;

    constructor() ERC721("NFTOOL", "DEMO") {
        _baseTokenURI = "https://test-api.nftool.io/generator-server/api/contract/v1/9Mh8J5zmr22XPnGx1/1607942571613896705/metadata/";
        _setDefaultRoyalty(0x11eec0923C7c49796482cdEB4F158b348166a783, uint96(600));
    }
    ...
    function presale(uint256 qty, bytes32[] calldata merkleProof) external payable {
        ...
        if (_currentIndex + qty > presaleSupply) revert SoldOut();
        if (msg.value != presalePrice * qty) revert InvalidPrice();
        if (bytes32(0) != merkleRoot && !MerkleProof.verify(merkleProof, merkleRoot, keccak256(abi.encodePacked(msg.sender)))) {
            revert InvalidProof();
        }
    ...
    function setBurn(bool _burn) external onlyOwner {
        burnEnabled = _burn;
    }

    function burn(uint256 tokenId) public virtual {
        require(burnEnabled, "ERROR: Non-burnable");
    ...
    function getWithdraws() public view returns(address[] memory, uint256[] memory){
        return (withdrawAddresses, withdrawPercentages);
    }
    ...
    function freeMint(uint256 qty, address recipient) external onlyOwner {
        ...
        for(uint i = 0; i < qty; ++i){
            _currentIndex++;
            _safeMint(recipient, _currentIndex);
        }
    }
    ...
    onlyOwner
    {
        ...
        for (uint256 i = 0; i < numRecipients; ) {
            if (_currentIndex + qtys[i] > supply) revert SoldOut();

            uint qty = qtys[i];
            address recipient = recipients[i];
            for(uint m = 0; m < qty; ++m){
                _currentIndex++;
                _safeMint(recipient, _currentIndex);
            }
