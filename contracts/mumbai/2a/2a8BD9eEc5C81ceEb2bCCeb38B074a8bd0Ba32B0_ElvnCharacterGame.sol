...
contract ElvnCharacterGame is Ownable {
    ...
    */
    ...
    function getPlayersOfOwner(address _owner) public view returns(uint[] memory, Parts[] memory){
        uint256[] memory _characters = walletOfOwner(_owner);
        Parts[] memory _parts = new Parts[](balanceOfOwner[_owner]);
        uint _hoody;
        uint _mask;
        uint _short;
        uint _shoes;
        uint _gloves;
        for(uint256 i=0; i < balanceOfOwner[_owner];i++){
            _hoody = INFTParts(partContract).partSet(tokenParts[_characters[i]].hoodyId);
            _mask = INFTParts(partContract).partSet(tokenParts[_characters[i]].maskId);
            _short = INFTParts(partContract).partSet(tokenParts[_characters[i]].shortId);
            _shoes = INFTParts(partContract).partSet(tokenParts[_characters[i]].shoeId);
            _gloves = INFTParts(partContract).partSet(tokenParts[_characters[i]].gloveId);
            _parts[i] = Parts(_hoody,_mask,_short,_shoes,_gloves);
        }
        return (_characters,_parts);
    }

    function getCharactersLog(address _owner) public view returns(uint[] memory){
