...
contract FitMintToken is ERC20, Ownable {
    ...
    mapping(string => uint256) public tokenDistribution;
    uint256 public maxTokenSupply = 10000000000;
    uint256 public tokenMinted = 0;

    // struct TokenDistribution
    event TransferReceived(address _from, uint _amount);
    event TransferSent(address _from, address _destAddr, uint _amount);
    // todo : create struct which will limit tranfer of tokens {"FundRaise" : 25%, "UserDistribution" : 25 %}
    constructor() public ERC20("FitMintToken", "FITT") {
    ...
    function setGameAddr(address _gameContractAddress) public onlyOwner {
    ...
    function mintTokens(uint256 tokenAmount) external{
        ...
        require(tokenAmount <= maxTokenSupply - tokenMinted);
        _mint(msg.sender, tokenAmount);
        tokenMinted += tokenAmount;
    }
}
