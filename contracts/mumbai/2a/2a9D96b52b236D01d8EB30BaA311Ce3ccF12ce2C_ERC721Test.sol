...
contract ERC721Test is Ownable, ERC721A, NonblockingReceiver {

    address public _owner;
    string private baseURI;
    uint256 startTokenId = 0;
    uint256 chainStartId;
    ...
    constructor(
        ...
        uint256 _price,
        uint256 _chainStartId,
        uint256 _chainMaxSupply,
        uint256 _preAllocation
    ) ERC721A("CBS", "CB", _chainStartId) { 
        _owner = msg.sender;
        ...
        chainMaxSupply = _chainMaxSupply;
        startTokenId = _chainStartId;

        if(_preAllocation > 0) {
            _safeMint(msg.sender, _preAllocation);
            mintedPerAddress[msg.sender] = _preAllocation;
        }
    }
    ...
    function mint(uint8 numTokens) external payable {
        require(mintedPerAddress[msg.sender] + numTokens <= maxMintPerWallet, "User can't mint more");
        require(amountMinted + numTokens <= chainMaxSupply, "Mint exceeds max supply");
        require(numTokens * price == msg.value, "Wrong amount sent");
