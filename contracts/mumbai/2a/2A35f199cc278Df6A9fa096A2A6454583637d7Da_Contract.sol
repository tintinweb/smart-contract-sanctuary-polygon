...
contract Contract is ERC721Base {
    ...
    function pluck(uint256 _tokenId, string[] memory _sourceArray) internal pure returns(string memory) {
        uint256 randomNumber = random(string(abi.encodePacked("letter", Strings.toString(_tokenId))));
    ...
    function tokenURI(uint256 _tokenId) override public view returns (string memory){
        ...
        parts[0] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: white; font-family: serif; font-size: 14px; }</style><rect width="100%" height="100%" fill="black" /><text x="100" y="200" class="base">';
    
        parts[1] = getLetter(_tokenId);
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Web3 Card: ', Strings.toString(_tokenId), '", "description": "TestOnChain NFTs", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
