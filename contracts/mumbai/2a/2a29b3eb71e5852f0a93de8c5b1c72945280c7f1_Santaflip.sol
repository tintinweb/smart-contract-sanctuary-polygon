...
abstract contract Manager is Ownable {
    ...
    // Events
    event BetPlaced(uint indexed betId, address indexed player, uint amount, uint indexed coins, bool isBonus, uint note);
    event BetSettled(uint indexed betId, address indexed player, uint amount, uint indexed coins, uint outcome, uint winAmount);
...
contract Santaflip is ReentrancyGuard, Manager {
    ...
    /*
    * @betChoice (0-x) (00-0x-xx-x0) (000-00x-0xx-xxx-xx0-x00) //vuela
    * @coins cant of coin in flip //vuela
    * bonus : no idea
    * note: no idea
    */
    ...
    function placeBet(uint betChoice, uint bonus, uint note) external payable nonReentrant {
        ...
        emit BetPlaced(betId, msg.sender, amount, coins, isBonus, note);   
        bets.push(Bet({
            coins: uint8(coins),
            outcome: 0,
    ...
    function _settleBet(uint betId, uint256 randomNumber) private nonReentrant {
        ...
        uint outcome = randomNumber % (100 ** coins);
        uint winNum = 50;
        uint winnableAmount = amountToWinnableAmount(amount, coins);
        uint winAmount = winNum > outcome ? winnableAmount : 0;
        ...
        emit BetSettled(betId, player, amount, coins, outcome, winAmount);
    }
