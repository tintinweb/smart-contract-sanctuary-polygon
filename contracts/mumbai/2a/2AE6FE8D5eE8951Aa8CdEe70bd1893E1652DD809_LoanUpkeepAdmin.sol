...
contract LoanContract {
    ...
    function confirmLoan(
        ...
        ILoanUpkeepAdmin(addressLoanUpkeepAdmin).registerAndPredictID(
            addressChainlinkFunctions
        );
...
contract LoanUpkeepAdmin {
    ...
    EnumerableMapExtended.AddressToUintMap loanContractToUpkeepId;

    // add enumberablemap for upkeepIDs mapped to address (address=>upkeepID)
    ...
    function registerAndPredictID(
        ...
        if (upkeepID != 0) {
            ...
            loanContractToUpkeepId.set(msg.sender, upkeepID);
            return upkeepID;
        } else {
    ...
    function cancelUpKeep(address _addressLoanContract) external {
        if (msg.sender != admin) revert Unauthorized();
        if (
            LoanContract(_addressLoanContract).loanStatus() !=
            LoanLibrary.LoanStatus.Repaid
        ) revert LoanNotRepaid();
        uint upkeepId = loanContractToUpkeepId.get(_addressLoanContract);
        chainlinkRegistry.cancelUpkeep(upkeepId);
        loanContractToUpkeepId.remove(_addressLoanContract);
    }

    // can be called only after 20 or more confirmations after cancellation (see chainlink app)
    // amend to that upkeeps are somehow cancelled automatically or, if manually, in batches
    function withdrawFundsFromChainlink(address _addressLoanContract) external {
        if (msg.sender != admin) revert Unauthorized();
        if (
            LoanContract(_addressLoanContract).loanStatus() !=
            LoanLibrary.LoanStatus.Repaid
        ...
        uint upkeepId = loanContractToUpkeepId.get(_addressLoanContract);
        chainlinkRegistry.withdrawFunds(upkeepId, address(this));
    }

    function getUpkeepId(
        address _addressLoanContract
    ) public view returns (uint) {
        return loanContractToUpkeepId.get(_addressLoanContract);
    }
    ...
    function withdrawFundsFromUpkeepAdminContract(uint _amount) external {
        ...
        if (
            loanContractToUpkeepId.length() > 0 &&
            balanceAfterWithdrawal < MINIMUM_LINK_BALANCE
