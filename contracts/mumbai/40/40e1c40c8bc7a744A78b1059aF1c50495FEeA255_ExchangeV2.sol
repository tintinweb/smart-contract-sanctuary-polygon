...
abstract contract MoviecoinTransferManager is OwnableUpgradeable, ITransferManager {
    ...
    function __MoviecoinTransferManager_init_unchained(
        uint newProtocolFee,
        address newDefaultFeeReceiver,
        IRoyaltiesProvider newRoyaltiesProvider
...
abstract contract TransferExecutor is Initializable, OwnableUpgradeable, ITransferExecutor {
    ...
    function __TransferExecutor_init_unchained(INftTransferProxy transferProxy, ITransferProxy _ERC1155lazyTransferProxy, IERC20TransferProxy erc20TransferProxy) internal {
        ...
        proxies[LibERC1155LazyMint.ERC1155_LAZY_ASSET_CLASS] = address(_ERC1155lazyTransferProxy);
    }
    ...
    function transfer(
        ...
        if (asset.assetType.assetClass == LibAsset.ETH_ASSET_CLASS) {
            ...
            ITransferProxy(proxies[LibERC1155LazyMint.ERC1155_LAZY_ASSET_CLASS]).transfer(asset, from, to);
        } else {
...
library LibERC1155LazyMint {
    ...
    struct Mint1155Data {
        ...
        string movieId;
        address producer;
        uint256 nftRevealTime;
        uint256 filmEquityPercentage;
        LibPart.Part[] creators;
        LibPart.Part[] royalties;
        bytes[] signatures;
    }

    bytes32 public constant MINT_AND_TRANSFER_TYPEHASH = keccak256("Mint1155(uint256 tokenId,string movieId,uint256 supply,uint256 nftRevealTime,address producer,uint256 filmEquityPercentage,string tokenURI,Part[] creators,Part[] royalties)Part(address account,uint96 value)");
    ...
    function hash(Mint1155Data memory data) internal pure returns (bytes32) {
        ...
        return keccak256(abi.encode(
                ...
                data.tokenId,
                keccak256(bytes(data.movieId)),
                data.supply,
                data.nftRevealTime,
                data.producer,
                data.filmEquityPercentage,
