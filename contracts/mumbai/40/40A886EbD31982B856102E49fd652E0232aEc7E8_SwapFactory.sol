...
contract SwapFactory is Ownable, CallerPermission, ReEntrancyGuard, Ticker {
   ...
   /*receive() external payable {
    ...
    function getReserve() public view returns (uint, uint) {
        ...
        uint token1Reserve = address(this).balance;

        if (token1Reserve >= 10 ** 12) {
            token1Reserve = token1Reserve.div(10 ** 12);
            token1Reserve = token1Reserve;
        }

        // limited time
        if (isManualy) {
            token1Reserve += token1Balance;
        }
    ...
    function buy(
        ...
        uint256 amountIn6Places = amount.div(10 ** 12);

        // calculation
        uint256 amountOfToken = getAmountOfTokens(
            amountIn6Places,
            reserve1.sub(amountIn6Places),
            reserv0,
            true
        ); // token
        ...
        investers[msg.sender].total_buy += (amountIn6Places).sub(
            partfeeAmount.mul(6)
        );
...
// generate unique ID for invester
    function generateUniqueId() internal returns (uint256) {
        investerId++;
