...
contract StanNFT is ERC721URIStorage, IERC721Receiver {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;
    ICollectionNFT private collectionNFT;
    address public auctionAddress;
    mapping(bytes => AuctionLibrary.userMobile) public ownerOfNFTMobile;
    ...
    modifier onlyCallerAndOwner(uint256 _tokenId) {
    ...
    function setCaller(address[] calldata _caller, bool _allowed)
        external
        onlyOwner
    {
    ...
    function createNFTByStan(
        ...
        if (!_isWeb) {
            ownerOfNFTMobile[_nftID].owner = _to;
            ownerOfNFTMobile[_nftID].tokenId = newTokenId;
        }
        ...
        emit STAN_EVENT(
            _requestId,
            AuctionLibrary.FunctionName.CREATE_NFT_BY_STAN,
            _isWeb,
            newTokenId
        );
        ...
        external
    {
        ...
        ownerOfNFTMobile[_nftId].owner = _owner;
    }
    ...
    function processNFTClaiming(
        ...
        uint256 tokenId = _isClaim
            ? ownerOfNFTMobile[_nftId].tokenId
            : _tokenId;
        if (_isClaim) {
            delete ownerOfNFTMobile[_nftId].owner;
        } else {
            ownerOfNFTMobile[_nftId].owner = _from;
        }

        collectionNFT.updateOwnerNFT(
            collectionNFT.getTokenIdToCollectionId(tokenId),
            _from,
            _to
        );
    ...
    function updateOwnerNFTAndTransferNFT(
        ...
        address _to,
        uint256 _tokenId
    ) external onlyCallerAndOwner(_tokenId) {
        collectionNFT.updateOwnerNFT(
