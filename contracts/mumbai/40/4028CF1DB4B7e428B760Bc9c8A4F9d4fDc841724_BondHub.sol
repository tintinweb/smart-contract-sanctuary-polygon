...
/**
 * @title BondMultiState
 *
 * @notice This is an abstract contract that implements internal BondHub state setting and validation.
 *
 * whenNotPaused: Either publishingPaused or Unpaused.
 * whenPublishingEnabled: When Unpaused only.
 */
abstract contract BondMultiState {
...
import {IBondNFTBase} from "../../interfaces/IBondNFTBase.sol";
import {Errors} from "../../libraries/Errors.sol";
import {DataTypes} from "../../libraries/DataTypes.sol";
import {Events} from "../../libraries/Events.sol";
 ...
 *
 * @notice This is an abstract base contract to be inherited by other Bond Protocol NFTs, it includes
 ...
 */
abstract contract BondNFTBase is ERC721Enumerable, IBondNFTBase {
     ...
     * @param burnSign The sign of the token can be burned.
     */
    ...
    function _validateRecoveredAddress(
        ...
        console.log(
            "recoveredAddress %s expectedAddress %s",
            recoveredAddress,
            expectedAddress
        );
        if (
...
abstract contract ERC721Enumerable is ERC721Time, IERC721Enumerable {
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view virtual override(IERC165, ERC721Time) returns (bool) {
        return
    ...
    function tokenOfOwnerByIndex(
        ...
        require(
            index < ERC721Time.balanceOf(owner),
...
abstract contract ERC721Time is Context, ERC165, IERC721Time, IERC721Metadata {
     ...
     * @param ssymbol The symbol to set.
     */
    function __ERC721_Init(
        string calldata sname,
        string calldata ssymbol
    ) internal {
        _name = sname;
        _symbol = ssymbol;
    }
...
import {BondNFTBase} from "./base/BondNFTBase.sol";
import {BondMultiState} from "./base/BondMultiState.sol";
import {HubStorage} from "./storage/HubStorage.sol";
 ...
 * @author Bond Protocol
 *
 * @notice This is the main entrypoint of the Bond Protocol. It contains governance functionality as well as
 * publishing and profile interaction functionality.
 *
 * NOTE: The Bond Protocol is unique in that frontend operators need to track a potentially overwhelming
 * number of NFT contracts and interactions at once. For that reason, we've made two quirky design decisions:
 *      1. Both Follow & Collect NFTs invoke an BondHub callback on transfer with the sole purpose of emitting an event.
    ...
    IBondHub
{
    ...
    function initialize(
        ...
        console.log("hub initializer %s %s",name,symbol);
        super._initialize(name, symbol, false);
    ...
    function setProfileImageURIWithSig(
        ...
        console.log("setProfileImageURIWithSig %s %s",vars.profileId, vars.imageURI);
        address owner = ownerOf(vars.profileId);
        unchecked {
            _validateRecoveredAddress(
                _calculateDigest(
    ...
    function _validateFollowAuthority(address follower) private returns (bool) {
        uint256 profileId = tokenOfOwnerByIndex(follower,0);
        address followModule = _profileById[profileId].followModule;
        if (followModule == address(0)) {
             revert Errors.FollowInvalid();
        }

        address ownerAddress = IFollowModule(followModule).getOwnerAddress();
        console.log(
            "_validateFollowAuthority %s %s %s",
            ownerAddress,follower,
            msg.sender
        );
        if ( ownerAddress == msg.sender || follower == msg.sender) {
            return true;
        }
        return false;
    }
    ...
    function follow(
        ...
        if (!_validateFollowAuthority(msg.sender)) {
            revert Errors.NotOwnerOrApproved();
        }
        return
    ...
    function followWithSig(
        ...
        console.log(
            "followWithSig %s %s",
            vars.follower,
            vars.profileIds.length
        );

        // if (!_validateFollowAuthority(vars.follower)) {
        //     revert Errors.NotOwnerOrApproved();
        // }
    ...
    function emitFollowNFTTransferEvent(
        ...
        address expectedFollowNFT = InteractionLogic.getFollowNFT(
            followType,
            profileId,
            _profileById
        );
    ...
    function getFollowNFT(
        ...
        return
            InteractionLogic.getFollowNFT(followType, profileId, _profileById);
    }
    ...
    function tokenURI(
        ...
        address followNFT = InteractionLogic.getFollowNFT(
            uint256(DataTypes.FollowType.Follow),
            tokenId,
            _profileById
        );
        return
    ...
    function _setProfileImageURI(
        ...
        console.log("_setProfileImageURI %s %s %s",profileId, imageURI, block.timestamp);
 ...
 * @author Bond Protocol
 *
 * @notice This is an abstract contract that *only* contains storage for the BondHub contract. This
 * *must* be inherited last (bar interfaces) in order to preserve the BondHub storage layout. Adding
 ...
 * @author Bond Protocol
 *
 * @notice This is the interface for the LensNFTBase contract, from which all Bond NFTs inherit.
...
interface IFollowNFT {
     ...
     */
    function initialize(uint256 profileId, uint256 followType, bool burnSign) external;
     ...
     * @param tokenId The followNFT being transferred's token ID.
     * @param to The address the followNFT is being transferred to.
     */
...
library InteractionLogic {
    ...
    function _followSingle(
        ...
        console.log("_followSingle type(%s) %s -> %s", followType, follower, profileId);
        ...
        address followModule = _profileById[profileId].followModule;
        address followNFT = _getFollowNFT(followType, profileId, _profileById);
        console.log("followNFT %s followModule %s", followNFT, followModule);

        if (followNFT == address(0)) {
            followNFT = _deployFollowNFT(profileId,followType);
           // _setFollowNFT(followType, profileId, followNFT, _profileById);
            console.log("new followNFT %s", followNFT);
        } 
    
    
        return 100;

        uint256 tokenId  = IFollowNFT(followNFT).tokenOfOwner(follower);
        if ( 0 == tokenId  ) {
            tokenId = IFollowNFT(followNFT).mint(follower);
            IFollowNFT(followNFT).emitTransferEvent(followType, tokenId, follower);
            if (followModule != address(0)) {
    ...
    function getFollowNFT(
        ...
        return _getFollowNFT(followType, profileId, _profileById);
    }

    function _getFollowNFT(
        uint256 followType,
        uint256 profileId,
        mapping(uint256 => DataTypes.ProfileStruct) storage _profileById
        ...
        for (
            uint256 i = 0;
            i < _profileById[profileId].followType.length;
            i++
        ) {
            if (_profileById[profileId].followType[i] == followType) {
                nftAddress = _profileById[profileId].followNFT[i];
                break;
            }
        }
        return nftAddress;
    ...
    function _setFollowNFT(
        ...
        for (
            ...
            if (_profileById[profileId].followType[i] == followType) {
                _profileById[profileId].followNFT[i] = nftAddress;
                return;
            }
        }
        _profileById[profileId].followType.push(followType);
        _profileById[profileId].followNFT.push(nftAddress);
    }
     ...
     */
    function _deployFollowNFT(uint256 profileId,uint256 followType) private returns (address) {
    
        bool burnSign = true;
        if (followType == uint256(DataTypes.FollowType.Referral)){
            burnSign = false;
        }
       ...
       // emit Events.OutputLogString('FollowNFTImpl',msg.sender,0);

        address followNFT = address(new FollowNFTProxy(functionData));
        console.log("sender %s, followNFT %s", msg.sender, followNFT);
...
library ProfileTokenURILogic {
     ...
     */
    function _getSVGProfilePicture(
        string memory imageURI
    ) internal pure returns (string memory) {
     ...
     */
    function _shouldUseCustomPicture(
        string memory imageURI
...
library PublishingLogic {
    ...
    function _initFollowModule(
        ...
        console.log("followModule %s", followModule);
        console.log(
            "_followModuleWhitelisted %s",
            _followModuleWhitelisted[followModule]
        );
...
contract FollowNFTProxy is Proxy {
    ...
    constructor(bytes memory data) {
        console.log("FollowNFTProxy HUB %s",msg.sender);
        emit Events.OutputLogString('FollowNFTProxy HUB',msg.sender,0);
        HUB = msg.sender;
        IBondHub(msg.sender).getFollowNFTImpl().functionDelegateCall(data);
    }

    function _implementation() internal view override returns (address) {
        return IBondHub(HUB).getFollowNFTImpl();
    }
}
