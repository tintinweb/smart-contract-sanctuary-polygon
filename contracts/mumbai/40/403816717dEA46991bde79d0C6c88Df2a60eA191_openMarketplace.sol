...
contract openMarketplace is  ReentrancyGuard,Ownable,ERC721Holder {
    ...
    //  mapping(address=>Listing) private nftAddressToMyNft;
     ...
     function getMyListedNft(address _nftAddress) external view returns(Listing[] memory){
         return ownerToListingArray[_nftAddress];
      }

      function getListedNft() external view returns(Listing[] memory){
         return ownerToListingArray[msg.sender];
      }
