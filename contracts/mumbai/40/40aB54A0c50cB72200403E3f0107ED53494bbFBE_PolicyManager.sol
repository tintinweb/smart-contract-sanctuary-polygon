	...
	ReentrancyGuardUpgradeable
{
	...
	mapping(bytes32 => bool) private _existHashMessage;
		...
		returns (bool)
	{
		...
		require(
			_existHashMessage[hashMessage] == false,
		...
		if (signer == V) {
			_existHashMessage[hashMessage] = true;
			return true;
		} else {
			return false;
		}
		...
		checkAsset(_buyPolicyParams.asset)
	{
		...
		require(_fundAllocationWeight.length == 7, "ILFAW");
		require(
			PDS.existPolicyId(_buyPolicyParams.policyId) != true,
		...
		uint8 _decimalsForConvert = decimals() - _ERC20.decimals();
		require(
			_ERC20.allowance(msg.sender, address(this)) >=
				(_buyPolicyParams.buyValue / 10**_decimalsForConvert),
		...
		returns (uint256 redeemId)
	{
		...
		(, string memory _errorMessage, uint256 _redeemId) = PDS.redeemPolicy(
			_redeemPolicyParams.policyId,
			_redeemPolicyParams.redeemPercentage
		);
		...
		require(redeemId != 0, _errorMessage);
		require(
	...
	/**
	 * @dev existHashMessage for chcek that is exists hashMessage or not.
	 * @param _hashMessage Hashmessage.
	 * @return The boolean value that means `_hashMessage` exists or not.
	 */
	function existHashMessage(bytes32 _hashMessage)
		private
		view
		returns (bool)
	{
		return _existHashMessage[_hashMessage];
	}
}
