...
contract Asset721 is ERC721, ERC721Enumerable {
    ...
    string public baseURI =
        "https://mint8-api.herokuapp.com/media-batch-items/metadata/";
    ...
    modifier onlyAdmin() {
        if (admin != msg.sender) {
            revert InvalidAuth("Only admin can perform this action");
        }
        _;
    }
    ...
    modifier onlyAuthorized() {
        if (admin != msg.sender && mintManager != msg.sender) {
            revert InvalidAuth("Not authorized to perform this action");
        }
        _;
    }
    ...
    function initialize(
        ...
        if (isInitialized) {
            revert InvalidAuth("Already initialized");
        }

        if (_sign.timestamp + 3600 < block.timestamp) {
            revert InvalidAuth("Expired signature");
        }
        ...
        if (admin != _sign.signer) {
            revert InvalidAuth("Invalid signature");
        }
...
error InvalidAuth(string _message);
