...
pragma solidity ^0.5.4;
    ...
    contract KAIZEN_WORLD {
        ...
        event Registration(string waddress,address investor,uint256 investorId,address referrer,uint256 referrerId,uint256 amount,uint256 amt_usd,uint256 position);
        ...
        function Register(string memory _user,uint256 _referrerCode, uint256 _amt, uint256 _type,uint256 position) public payable
        {
            ...
            require(idToAddress[_referrerCode]!=address(0),"Invalid Referrer ID");
                ...
                if(_type==1)
                {
                    ...
                    emit Registration(_user,msg.sender,latestReferrerCode,idToAddress[_referrerCode],_referrerCode,msg.value,0,position);
                }
                else if(_type==2)
                {   
                    require(_amt>0);
                    busd_token.transferFrom(msg.sender,adminAccount_, _amt);
                    emit Registration(_user,msg.sender,latestReferrerCode,idToAddress[_referrerCode],_referrerCode,0,_amt/1e18,position);
                }
        }
