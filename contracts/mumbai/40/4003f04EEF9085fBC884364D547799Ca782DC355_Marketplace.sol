...
contract Marketplace is ReentrancyGuard, RoleControl {
    ...
    */
    ...
    function addGallery(address _galleryWallet, uint256 _royalty) public onlyOperator {
    ...
    */
    function createMarketSale(
        ...
        uint galleryRoyalty = calculateRoyalty(msg.value, gallery.royalty);
        uint marketRoyalty = calculateRoyalty(msg.value, royalty);
        uint sellerRoyalty = msg.value - (galleryRoyalty + marketRoyalty);
    ...
    /** 
        Disable Gallery
     */
    function disableGallery(uint galleryId) public onlyOperator {
        Gallery storage gallery = galleries[galleryId];
    ...
    /** 
        Enable Gallery
     */
    function enableGallery(uint galleryId) public onlyOperator {
        Gallery storage gallery = galleries[galleryId];
    ...
    */
    ...
    function calculateRoyalty(uint amount, uint percentage) private pure returns(uint){
    ...
    */
    ...
    function getActiveGalleries() public view returns (Gallery[] memory){
        uint total = _galleryCounter.current();
        uint itemCount = 0;
