...
contract ScoreCast is FunctionsClient, ConfirmedOwner {
  ...
  // Test
  function getRequest(string calldata requestId) external view returns(string memory){
    bytes32 req;
    bytes memory tmp = bytes(requestId);
    assembly {
        req := mload(add(tmp, 32))
    }
    
    return requestToFixture[req];
  }
