    ...
    ReentrancyGuardUpgradeable
{
    ...
    function _withdraw(
        ...
        require(
            ...
            "PIT: try to withdraw more than available"
        );
    ...
    function getCollateralAvailableToWithdraw(
        ...
        uint256 depositRemaining = totalPITRemaining(account);
        collateralProjectToWithdraw =
            (depositRemaining *
                lvr.denominator *
                (10 ** ERC20Upgradeable(projectToken).decimals())) /
            getTokenEvaluation(
                projectToken,
                10 ** ERC20Upgradeable(projectToken).decimals()
            ) /
            lvr.numerator;
        if (
            collateralProjectToWithdraw > depositedAmount[account][projectToken]
        ) {
            collateralProjectToWithdraw = depositedAmount[account][
                projectToken
            ];
        }
    }
