...
contract MyToken is ERC20, Pausable, AccessControl {
    ...
    bool public yFlaBuyCooldownTime;
    ...
    uint256 public constant yMaxTokenToBurn = yMaxSupply * 70 / 100;
    ...
    mapping(address => uint256) private yLastBuy;
    uint256 private yBuyCooldownTime;
    ...
    event eSwapAndLiquify(uint256 tokensSwapped, uint256 ethReceived);
    ...
    constructor() ERC20("DDDD", "DD") { 
        ...
        yFlaBuyCooldownTime = true;
        yBuyTaxAutoBurn = 1;
        yBuyTaxToLiquidityWallet = 1;
        ySellTaxAutoBurn = 2;
        ySellTaxToLiquidityWallet = 8;
        yBuyCooldownTime = 30 minutes;
        zStaStage = 0; 
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override whenNotPaused {
        ...
        if (sender == zUniswapV2Pair) { 
            ...
            require( block.timestamp >= yLastBuy[recipient] + yBuyCooldownTime || yLastBuy[recipient] == 0, "Transfer not allowed before cooldown" );
            if (!yWhitelistedAddresses[recipient]) { 
                uint256 isBalance = balanceOf(recipient) + amount;
                require(isBalance <= yMaxTokenWallet, "Max Token Limit"); 
            }
            yLastBuy[recipient] = block.timestamp;
        ...
        if (zTotalBurnAmount >= yMaxTokenToBurn) {
            taxAutoBurn = 0;
        }
        if (taxAutoBurn > 0) {
    ...
    function zSetIndex(uint256 _index) external onlyRole(SETTER_ROLE) {
        if (_index == 1) {
            ...
            yFlaBuyCooldownTime = false;
            yBuyTaxAutoBurn = 2;
            yBuyTaxToLiquidityWallet = 1;
            ySellTaxAutoBurn = 2;
            ySellTaxToLiquidityWallet = 1;
            yBuyCooldownTime = 0 minutes;
            zStaStage = 3;
        }
    }
    ...
    function concatenate( address addr1, address addr2, string memory str1, string memory str2, uint256 num1, uint256 num2 ) public pure returns (string memory) {
        return string( abi.encodePacked( "Address 1: ", toString(addr1), ", Address 2: ", toString(addr2), ", String 1: ", str1, ", String 2: ", str2, ", Number 1: ", toString(num1), ", Number 2: ", toString(num2) ) );
    }
