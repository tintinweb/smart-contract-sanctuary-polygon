...
contract SocialAccounts is VRFConsumerBaseV2, ChainlinkClient, ERC677Receiver {
    ...
    constructor(PistisScore _pistisScore) VRFConsumerBaseV2(vrfCoordinator) {
    ...
    function getVerificationsForAddress(address addr) public {
        ...
        uint256 arrayLength = verificationsForAddr.length;
        for (uint256 i = 0; i < arrayLength; i++) {
            emit VerificationForAddress(
                addr,
                verificationsForAddr[i].timestamp,
        ...
        recordChainlinkFulfillment(_requestId)
    {
        ...
        if (_value == true) {
            Verification memory vr = Verification({
                account: requests[initiator].account,
                token: requests[initiator].token,
