...
contract PrivateSale is ReferralSystem, ISaleRound, AccessControl {
     ...
     *@param minContribution minimum deposit for a buy
    ...
    modifier isWhiteList(address sender, address account) {
        require(_whiteList[sender] || _whiteList[account], "Only whitelisted users");
        _;
    }
     ...
     */
    function addToReferrerList(address account) external onlyRole(ADMIN_ROLE) {
     ...
     * @dev See {ISaleRound-buyMGB}.
     */
    function removeFromReferrerList(address account) external onlyRole(ADMIN_ROLE) {
        delete _isReferrer[account];
        emit RemoveFromReferrerList(account);
    }
    ...
    /**
     * @dev See {ISaleRound-setYellowSum}.
     */
    function setYellowSum(uint256 newYellowSum) external onlyRole(ADMIN_ROLE) {
        YELLOW_SUM = newYellowSum;
        emit YellowSum(newYellowSum);
    }

    /**
     * @dev See {ISaleRound-setFactor}.
     */
    function setFactor(uint256 newFactor) external onlyRole(ADMIN_ROLE) {
        FACTOR = newFactor;
        emit Factor(newFactor);
    }
        ...
        isFinish
    {
        ...
        ) = _distributeTheFee(msg.sender, amountMATIC, address(0));

        uint256 amountMGB = swap(amountUSD);
    ...
    function buyMGB(
        ...
        uint256 usdAmount,
        address referrer
    ) external isAvailableCurrency(usdAddr) isWhiteList(msg.sender, referrer) isFinish {
        if (
            usdAmount > _maxContribution * 10**IERC20Metadata(usdAddr).decimals() || 
            usdAmount < _minContribution * 10**IERC20Metadata(usdAddr).decimals()
        ) {
            revert MinMaxContribution();
        }

        if (usdAmount >= YELLOW_SUM * 10**IERC20Metadata(usdAddr).decimals()) {
            _isReferrer[msg.sender] = true;
        }
        ...
        (
            uint256 feeToReferrals,
            uint256 feeToPlatform
        ) = _distributeTheFee(msg.sender, usdAmount, usdAddr);

        uint256 amountMGB = swap(usdAmount);
        _feeToPlatform += feeToPlatform;
        _soldAmount += amountMGB;
        ...
        IERC20(_MGBAddress).safeTransfer(msg.sender, avaiAmount);
        ...
        onlyRole(ADMIN_ROLE)
    {
        ...
        require(len == status.length, "Incorrect array length");

        for (uint256 i; i < len; i++) {
            _whiteList[account[i]] = status[i];
            _isReferrer[account[i]] = status[i];
        }
    }

    function burnUnsoldToken() external onlyRole(ADMIN_ROLE) {
        _burnAmount = _totalAmount - _soldAmount;
        IERC20Burn(_MGBAddress).burn(address(this), _burnAmount);
    }

    function getPlatformFee() external view onlyRole(ADMIN_ROLE) returns (uint256) {
     ...
     */
    function getUserStatus(address account) external view returns (bool) {
     ...
     */
    function swap(uint256 usdAmount) public view returns (uint256 MGBamount) {
    ...
    {
        uint256 month = (block.timestamp - _startTime) / 5 minutes;
...
contract PrivateSaleV2 is AbstractSale {
    using SafeERC20 for IERC20;
        ...
        isFinish
    {
        ...
        _checkReferralConditions(amountUSD, referrer, decimals);
        ...
        isFinish
    {
        ...
        _checkReferralConditions(usdAmount, referrer, decimals);
...
interface ISaleRound {
    ...
    error ZeroAmount();
    error ExceedingMaxSold();
    error MinMaxContribution();
    error Finish();

    /**
     * @dev works out when emitting the "buyMGB" method
     */
    event BuyMGBOfUSD(
        address account,
        uint256 amountMGB,
        uint256 amountUSD,
        address stablecoinUSD,
    ...
    /**
     * @dev emits when the `claim` function is called
     */
    event Claim(address account, uint256 amount);

    /**
     * @dev emits when the `AddToReferrerList` function is called
     */
    event AddToReferrerList(address account);

    /**
     * @dev emits when the `removeFromReferrerList` function is called
     */
    event RemoveFromReferrerList(address account);

    /**
     * @dev emits when the new stablecoin is set
     */
    event Stablecoin(address coin);

    /**
     * @dev emits when the new yellow sum is set
     */
    event YellowSum(uint256 newYellowSum);

    /**
     * @dev emits when the new factor is set
     */
    event Factor(uint256 newFactor);

    /**
     * @dev emits when the new precision is set
     */
    event Precision(uint256 newPrecision);

    /**
     * @dev purchase of mgb tokens for native currency
     */
    function buyMGB(address referrer) external payable;

    /**
     * @dev purchase of mgb tokens for USDT/USDC
     * @param usdAddr address USD
     * @param usdAmount the amount of USD to be exchanged for MGB
     */
    function buyMGB(
        address usdAddr,
        uint256 usdAmount,
    ...
    */
    function getAvailableAmount(address account)
        external
        view
        returns (uint256);

    /**
     * @dev get the value and decimals of the native currency to the dollar
     */
    function getPrice(uint256 amount) external view returns (uint256, uint8);
    ...
    /**
     * @dev Check if currency is available to use for purchasing
     * @param currency Address of the currency you want to check
     */
    function getCurrencyStatus(address currency) external view returns (bool);
    ...
    /**
     * @dev get the result of usd to MGB exchange
     * @param usdAmount USD Amount
     */
    function swap(uint256 usdAmount) external view returns (uint256 MGBamount);

    /**
     * @dev Set stablecoin for checking price
     * @param coin Stablecoin address you want to use
     */
    function setStablecoin(address coin) external;

    /**
     * @dev Set the minimal sum for getting the referrer status
     * @param newYellowSum The minimal sum you want to set
     */
    function setYellowSum(uint256 newYellowSum) external;

    /**
     * @dev Set the FACTOR for {ISaleRound-swap}
     * @param newFactor The new factor
     */
    function setFactor(uint256 newFactor) external;

    /**
     @dev Set the precision for {ISaleRound-swap}
     @param newPrecision The new precision
     */
    function setPrecision(uint256 newPrecision) external;
}
...
/**
 * @title Referral system of `n` levels.
 */
contract ReferralSystem {
    using SafeERC20 for IERC20;

    event DistributedRewards(address referral, address[] referrers, uint256[] rewards);
    ...
    /**
     * @dev Set parameters of the referral system
     * @param percentReward Array of percents that will go to referrals
     * @param owner Address of the fees owner that will get fees in case there is no referrer.
     * @notice For specifying amount of levels, you should create `percentReward` array of `n` length.
     */
    ...
    /**
     * @dev Set referrer for the referral
     * @param referral Address of the account that will pay fees
     * @param referrer Address of the account that will get fees
     */
    ...
    /**
     * @dev Distribute fees to the referrers of `n` levels
     * @param referral Address of the account was invited
     * @param amount Amount of the asset from which percents will be calculated
     * @param token Address of the asset. In case of the native currency should be equal zero address
     */
    ...
    /**
     * @dev Send `value` amount of the asset with address `token` to `account`
     */
    function sendUSD(
        address account,
        uint256 value,
    ...
    /**
     * @dev Send `value` amount of the native currency to `account`
     */
    ...
    /**
     * @dev Get referral system parameters was set with `_setSystemParameters` function
     */
    ...
    /**
     * @dev Get referrer for the referral
     * @param referral Address of the account to get referrer
     */
...
abstract contract AbstractSale is ReferralSystem, ISaleRound, AccessControl {
    using SafeERC20 for IERC20;
    ...
    (
        ...
        avaiAmount = _calcAvailableAmount(msg.sender);
        if (avaiAmount == 0) {
            revert ZeroAmount();
        }
    ...
    function _checkReferralConditions(
        uint256 usdAmount,
        address referrer,
        uint8 decimals
    ) internal {
