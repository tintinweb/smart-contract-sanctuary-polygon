...
contract LicensTransactionContract is Ownable, ReentrancyGuard{
    ...
    function createListing(
        ...
        require(accountContract.isRegistered(_creator),"You have to create creator profile first !");

        uint256 newID = _idGenerator.getNewID(); // Generate a new unique ID using the ListingIDGenerator

        uint256 tokenId = artListingNFT.safeMint(newID, _creator,_listingCID); // Use the new unique ID as the token ID
    ...
    function updateListing(
        ...
        require(_creator == listing.owner,"You have to be listing's owner to be able to update listing !");
        ListingDetails memory _listingInfo = ListingDetails(
                                                _listingId, 
                                                _listingCID, 
                                                _creativeCID, 
