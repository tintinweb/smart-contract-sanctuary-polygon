...
contract Zap {
	...
	function quoteAddress() internal view returns (address) {
		...
		if (chainID == 1) {
			...
			return 0xB9e9C21F4C1723878C83c5528A4bA34283dD8FCf;
		} else if (chainID == 42161) {
			return 0xFF970A61A04b1cA14834A43f5dE4533eBDDB5CC8;
		} else if (chainID == 137 || chainID == 80001) {
			return 0x149480B179Db4aed78D79fD172211024e35B1bf6;
		} else {
			return address(0);
		}
	}
	...
	function zap(
		...
		require(
			base == Curve(_curve).numeraires(0),
			"!Zap/base-is-not-in-numeraires"
		);
		...
		(uint256 lpAmount, ) = Curve(_curve).deposit(
			depositAmount,
			0,
			0,
			type(uint256).max,
		...
		IERC20(_curve).safeTransfer(
			msg.sender,
			IERC20(_curve).balanceOf(address(this))
		);
	...
	function _calcQuoteSwapAmount(
		...
		for (uint256 i = 0; i < 32; i++) {
			...
			recvAmount = Curve(zapData.curve).viewOriginSwap(
				address(USDC),
				zapData.base,
				swapAmount
			);
	...
	function _calcDepositAmount(
		...
		uint256 curveRatio = IERC20(_base)
			...
			.div(USDC.balanceOf(_curve).mul(1e12));
