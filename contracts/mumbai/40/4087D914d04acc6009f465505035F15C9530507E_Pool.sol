...
contract Pool is VersionedInitializable, PoolStorage, IPool {
  ...
  function initReserve(
    ...
    uint256 borrowRate,
    uint256 prxymode2rate,
    uint256 prxymode3rate,
    uint256 splitRate,
    ...
    if (
      PoolLogic.executeInitReserve(
        ...
        DataTypes.InitReserveParams({
          ...
          prxymode2rate: prxymode2rate,
          prxymode3rate: prxymode3rate,
          splitRate: splitRate,
  ...
  function borrow(
    ...
    if(amount > 0) borrowerList[borrowingAsset].addUser(msg.sender);
  }
  ...
  function repay(
    ...
    if(userBorrowBalance == 0) borrowerList[borrowingAsset].removeUser(msg.sender);
    return repayAmount;
  }
  ...
  /// @inheritdoc IPool
  function getSuppliers(address asset) external view override returns (uint256) {
    return supplierList[asset].list.length;
  }

  /// @inheritdoc IPool
  function getBorrowers(address asset) external view override returns (uint256) {
    return borrowerList[asset].list.length;
  }
  ...
  /// @inheritdoc IPool
  function setReserveInterestRates(address asset, uint256 borrowRate, uint256 prxymode2rate, uint256 prxymode3rate) external virtual override onlyPoolConfigurator {
    DataTypes.ReserveData storage reserve = _reserves[asset];
    reserve.borrowRate = borrowRate;
    reserve.prxymode2rate = prxymode2rate;
    reserve.prxymode3rate = prxymode3rate;
  }
  ...
  /// @inheritdoc IPool
  function getPrxyMode2InterestRate(address supplyingAsset) 
    external view override 
    returns(uint256)
  {
    uint256 rate = _reserves[supplyingAsset].prxymode2rate * WadRayMath.RAY;
  ...
  /// @inheritdoc IPool
  function getPrxyMode3InterestRate(address supplyingAsset) 
    external view override 
    returns(uint256)
  {
    uint256 rate = _reserves[supplyingAsset].prxymode3rate * WadRayMath.RAY;
    rate /= SECONDS_PER_YEAR * GeneralLogic.RATE_PRECISION;
...
interface IPool {
   ...
   * @param prxymode2rate The prxy interest rate for supply mode2
   * @param prxymode3rate The prxy interest rate for supply mode3
   ...
   * @param prxymode2rate The prxy rate
   * @param prxymode3rate The collateral prxy rate
   **/
  function setReserveInterestRates(address asset, uint256 borrowRate, uint256 prxymode2rate, uint256 prxymode3rate) external;
   ...
   **/
  function getPrxyMode2InterestRate(address supplyingAsset) external view returns(uint256);
   ...
   **/
  function getPrxyMode3InterestRate(address supplyingAsset) external view returns(uint256);
  ...
  /**
   * @notice Function to get suppliers
   * @param asset The address of the underlying asset used as collateral
   **/
  ...
  /**
   * @notice Function to get borrowers
   * @param asset The address of the underlying asset used as collateral
   **/
...
library DataTypes {
  struct ReserveData {
    ...
    uint256 prxymode2rate;     // Fixed Rate
    // the collateral prxy rate.
    uint256 prxymode3rate;     // Fixed Rate
    // native interest paid
    uint256 nativeInterestPaid;
    // prxy interest paid
    uint256 prxyInterestPaid;
    // previous prxy balance
    uint256 prevPrxyBalance;
    // the split rate.
...
library PoolLogic {
  ...
  function executeInitReserve(
    ...
    reservesData[asset].init(
      ...
      params.borrowRate,
      params.prxymode2rate,
      params.prxymode3rate,
      params.splitRate,
...
library ParticularLogic {
  ...
  function executeSplitReduxInterest(
    ...
    uint256 currentPrxyBalance = IPToken(reserve.pTokenAddress).totalRewardPrxy();
    reserve.prxyInterestPaid = currentPrxyBalance - reserve.prevPrxyBalance;
    reserve.prevPrxyBalance = currentPrxyBalance;

    if (totalInterestAmount == 0) {
      reserve.nativeInterestPaid = 0;
    ...
    if(totalUncollateralAmount == 0) {
      ...
      reserve.nativeInterestPaid = realTotalReward;
    ...
    if(totalRewardAmount > 0) IERC20(params.asset).safeTransfer(reserve.pTokenAddress, totalRewardAmount);
    uint256 daoAmount = totalInterestAmount - totalRewardAmount;
    if(daoAmount > 0) IERC20(params.asset).safeTransfer(params.daoWallet, daoAmount);
  ...
  function _calcUncollateralAmount (
    ...
    if(IPToken(reserve.pTokenAddress).supplyMode(user) != 1) return 0;
...
interface IPrxyTreasury {
  /**
   * @notice Transfer the ownership of the contract to a new account
...
interface IPToken is IERC20Detailed, IInitializablePToken {
  ...
  /**
   * @notice Returns the total reward amount of the prxy interest of this pToken
   * @return The total reward amount of the prxy interest 
   **/
  function totalRewardPrxy() external view returns (uint256);
