...
contract Elixir is ERC721, ERC721Burnable, AccessControl  {
    ...
    function getSteadyRequired(uint256 tokenId) public view returns(uint256 steadyRequired, uint256 chymeAmount) {
        ...
        (,, uint ratioOfSteady, uint decimals,) = IAcademy(address(academy)).getChymeInfo(elements[tokenId].chyme);
        return ((myElixir.forgePrice *  ratioOfSteady * myElixir.amount) / 100 * 10 ** decimals, myElixir.amount);
    }
    ...
    function generateImage(uint256 tokenId,uint256 timeLeft,string memory elixirCurrentSteadyValue) 
        public view returns (string memory) {
            ...
            string memory treasureChest = ITreasure(treasure).generateTreasureChest(toString(timeLeft));
            return string(
                abi.encodePacked(
                    ITreasure(treasure).generateHeader(),
                    '<text x="14" y="205" font-size="10px" font-family="Arial">Current Value</text>',
                    '<text x="74" y="205" font-size="12px"  font-family="Arial">',elixirCurrentSteadyValue,'</text>',
                    // '<text x="14" y="220" font-size="10px" font-family="Arial">Units</text>',
                    // '<text x="74" y="220" font-size="12px"  font-family="Arial">',elixirCurrentSteadyValue,'</text>',
                    // '<text x="14" y="235" font-size="10px" font-family="Arial">Elixir</text>',
                    // '<text x="74" y="235" font-size="12px"  font-family="Arial">',elixirCurrentSteadyValue,'</text>',
...
interface ITreasure{
function generateTreasureChest(string memory timeLeft) external pure returns (string memory);
