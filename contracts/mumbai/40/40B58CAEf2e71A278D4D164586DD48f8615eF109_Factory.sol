...
contract Factory {
    address[] public generativeNFT;

    function createGenerativeNFT(
        string memory _name,
        string memory _symbol,
        string memory _initBaseURI,
        uint256 _mintAmount
    ) public {
        GenerativeNFT newGenerativeNFT = new GenerativeNFT(
            _name,
            _symbol,
            _initBaseURI,
            _mintAmount
        ...
        generativeNFT.push(address(newGenerativeNFT));
    }

    function getAddress(uint256 _index) external view returns (address) {
        return generativeNFT[_index];
    }

    function getLength() external view returns (uint256) {
        return generativeNFT.length;
    }
}
