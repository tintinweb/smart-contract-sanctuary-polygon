...
contract OnSaleTracker is IOnSaleTracker, AdminAccess {
    ...
    struct FtnftSaleItem {
        ITangibleFractionsNFT ftnft;
        uint256 fractionId;
        uint256 indexInCurrentlySelling;
    }

    ITangibleFractionsNFT[] public currentFtnftContractsOnSaleList;
    ...
    mapping(ITangibleFractionsNFT => mapping(uint256 => FtnftSaleItem))
        public fractionSaleMapper;

    ITangibleNFT[] public currentTnftCategoriesOnSale;
    ...
    //return whole array
    function getFtnftContractsOnSale()
        external
        view
        returns (ITangibleFractionsNFT[] memory)
    {
        return currentFtnftContractsOnSaleList;
    }

    //return size
    function currentFtnftContractsOnSaleListSize()
        external
        view
        returns (uint256)
    {
        return currentFtnftContractsOnSaleList.length;
    }

    //return whole array
    function getTnftContractsOnSale()
        external
        view
        returns (ITangibleNFT[] memory)
    {
        return currentTnftCategoriesOnSale;
    }

    //return size
    function currentTnftCategoriesOnSaleSize() external view returns (uint256) {
        return currentTnftCategoriesOnSale.length;
    }

    //return whole array
    function getTokensForFtnftOnSale(ITangibleFractionsNFT ftnft)
        external
        view
    ...
    function getTokensForFtnftOnSaleBatch(
    ...
    //return whole array
    function getTokensForTnftOnSale(ITangibleNFT tnft)
        external
        view
        returns (uint256[] memory)
    {
    ...
    //return whole batch arrays
    function getTokensForTnftOnSaleBatch(ITangibleNFT[] calldata tnfts)
        external
        view
    ...
    //return size
    function ftnftTokensOnSaleSize(ITangibleNFT tnft)
        external
        view
        returns (uint256)
    {
    ...
    function tnftSalePlaced(
        ...
        if (place) {
            ...
            if (!isTnftOnSale[tnft].selling) {
                ...
                currentTnftCategoriesOnSale.push();
                isTnftOnSale[tnft].selling = true;
                isTnftOnSale[tnft].index =
                    currentTnftCategoriesOnSale.length -
                    1;
            }
    ...
    function ftnftSalePlaced(
        ...
        if (place) {
            ...
            if (!isFtnftOnSale[ftnft].selling) {
                //add category to actively selling list
                currentFtnftContractsOnSaleList.push();
                isFtnftOnSale[ftnft].selling = true;
                isFtnftOnSale[ftnft].index =
                    currentFtnftContractsOnSaleList.length -
                    1;
            }
            //something is added to marketplace
            fractionTokensOnSale[ftnft].push(tokenId);
            FtnftSaleItem memory fsi = FtnftSaleItem(
                ftnft,
                tokenId,
    ...
    function _removeCategory(uint256 index) internal {
        require(index < currentTnftCategoriesOnSale.length, "IndexC");
        //take last token
        ITangibleNFT _tnft = currentTnftCategoriesOnSale[
            currentTnftCategoriesOnSale.length - 1
        ];

        //replace it with the one we are removing
        currentTnftCategoriesOnSale[index] = _tnft;
        //set it's new index in saleData
        isTnftOnSale[_tnft].index = index;
        currentTnftCategoriesOnSale.pop();
    }

    function _removeFraction(uint256 index) internal {
        require(index < currentFtnftContractsOnSaleList.length, "IndexFr");
        //take last token
        ITangibleFractionsNFT _ftnft = currentFtnftContractsOnSaleList[
            currentFtnftContractsOnSaleList.length - 1
        ];

        //replace it with the one we are removing
        currentFtnftContractsOnSaleList[index] = _ftnft;
        //set it's new index in saleData
        isFtnftOnSale[_ftnft].index = index;
        currentFtnftContractsOnSaleList.pop();
    }
}
