...
contract Operations is VRFConsumerBaseV2, ERC2771Recipient {
    ...
    function placeBet(
        ...
        if (s_placeBetKey[betDetails[3]] == true) {
        ...
        s_keyToBetSlip[betDetails[3]] = BetSlip(
            betDetails[3],
            _msgSender(),
            token,
            totalStake,
    ...
    function settleBet(
        ...
        if (settleStatus == 1) {
            handleLoss(betSlip, betOrder, stake, payout, odds);
        } else if (settleStatus == 2) {
            handleWin(betSlip, betOrder, stake, payout, odds);
        } else if (settleStatus == 3) {
            handleSuspension(betSlip, betOrder, stake, payout, odds);
        } else {
