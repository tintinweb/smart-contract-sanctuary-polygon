    ...
    ERC2981
{
    ...
    //mapping(uint256 => NakaInfo) public nakaInfos;
    ...
    constructor (
        ...
        //     TokenInfo storage tokenInfo = tokenInfos[i + 1];
        //     tokenInfo.maxSupply = _maxSupplies;
        //     tokenInfo.price = _prices;
        // }
    ...
    function mint(uint256 quantity) public nonReentrant whenNotPaused payable{
        // uint256 price; 
        TokenInfo storage tokenInfo = tokenInfos[quantity];
        uint256 supply = tokenInfo.currentSupply.current();
        require(quantity > 0);

        require(quantity <= maxMintAmount);

        require(supply <= tokenInfo.maxSupply, "NakaPunk.sol: Max limit.");
        for (uint256 i = 1; i <= quantity; i++) {
            currency.safeTransferFrom(msg.sender, address(this), tokenInfo.price);
