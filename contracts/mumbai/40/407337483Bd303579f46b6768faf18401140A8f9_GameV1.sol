...
contract GameV1 is IGame, Initializable, UUPSUpgradeable, PausableUpgradeable {
    ...
    uint256[] energyAmountList;
    mapping(uint256 => EnergyInfo) public energyAmountToEnergyInfo;

    event TopUpEnergy(uint256 energyAmount, address user);
    ...
    function setEnergyPackage(
        ...
        if (energyAmountToEnergyInfo[_energyAmount].price == 0) {
            energyAmountList.push(_energyAmount);
        }

        energyAmountToEnergyInfo[_energyAmount] = EnergyInfo(
            _energyPrice,
            _isAvailable
        );
    }
     ...
     * @param _energyAmount Energy amount
     */
    function buyEnergy(uint256 _energyAmount) public override whenNotPaused {
        IERC20Upgradeable bbToken = IERC20Upgradeable(bbTokenAddress);
        uint256 userBalance = bbToken.balanceOf(_msgSender());

        require(
            userBalance >= energyAmountToEnergyInfo[_energyAmount].price,
        ...
        bbToken.transferFrom(
            _msgSender(),
            feeReceiver,
            energyAmountToEnergyInfo[_energyAmount].price
        );

        emit TopUpEnergy(_energyAmount, _msgSender());
    }

    /**
     * @dev Get energy amount list
     */
    function getEnergyAmountList()
        public
        view
        override
        returns (uint256[] memory _energyAmountList)
    {
...
interface IGame {
    ...
    function buyEnergy(uint256 _energyAmount) external;
