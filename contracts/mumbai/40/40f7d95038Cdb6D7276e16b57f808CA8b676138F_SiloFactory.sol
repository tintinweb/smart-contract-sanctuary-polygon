...
//TODO add in LP token balances to the strategy balances arrays
contract SiloFactory is Ownable, ERC721Enumerable{
    ...
    constructor(address _silo) ERC721("Test Silo Ownership Token","TSOT") {
    ...
    function adjustActionLogivCheck(bool _state) external{
    ...
    function AdminCall( uint siloID, address target, bytes memory data) external isOwner(siloID){
        ISilo silo = ISilo(siloMap[siloID]);
        silo.adminCall(target, data);
    }
    ...
    function exitStrategyAndWithdraw(uint siloID) external isOwner(siloID){
        ISilo silo = ISilo(siloMap[siloID]);
        silo.Withdraw();
        silo.ExitSilo(msg.sender);
    }
    ...
    function _beforeTokenTransfer(
        ...
        if(enforceBeforeTransfer){
            require(ISiloManagerFactory(managerFactory).managerApproved(to), "Gravity Finance: to address does not have approved manager");
