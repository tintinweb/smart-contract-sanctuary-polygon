...
contract MaxityMarketPlace is Ownable,Pausable ,IMaxityMarketPlace,IERC721Receiver{
    ...
    event TokenIdSaled(address indexed _token,uint256 indexed _tokenid,address  buyer,address  seller,uint256 price,uint sold_count);
    ...
    function sellByFixPrice(address _token,uint256[] memory _tokenids,uint256 _price,address _seller) external  returns (uint) {
            
        uint256 length = _tokenids.length;
        for (uint256 pid = 0; pid < length; ++pid) {
            uint256 _tokenid=_tokenids[pid];
            require(marketUnits[_token][_tokenid].seller==address(0x0),'already saled');
            ...
            saleInfo.fixprice = _price;
            emit TokenIdOnSale(_token,_tokenid,saleInfo.seller,_price);
        }
        return length;
    }
    ...
    function auctionMax721(address _token,uint256[] memory _tokenids,uint base_price,uint incre_price,uint starttime,uint deadline,address _seller) external returns (uint amountU){
        uint256 length = _tokenids.length;
        for (uint256 pid = 0; pid < length; ++pid) {
            uint256 _tokenid=_tokenids[pid];
            require(marketUnits[_token][_tokenid].seller==address(0x0),'already saled');
            ...
            saleInfo.base_price = base_price;
            saleInfo.incre_price = incre_price;
            saleInfo.starttime = starttime;
            saleInfo.deadline = deadline;
            saleInfo.cur_bid_price = base_price;
            emit TokenIdOnAuction(_token,_tokenid,saleInfo.seller,base_price,incre_price,deadline);
        }
        return length;
    }

    function buy(address _token,uint256 _tokenid,address to) public {
        SaleInfo storage saleInfo = marketUnits[_token][_tokenid];
        require(saleInfo.seller!=address(0x0),'not on saled');
        require(IERC721(_token).ownerOf(_tokenid)==address(this),'token not in this market');
        require(!saleInfo.isAuction,'token is on auction');
        
        
        //calc fee.
        TransferHelper.safeTransferFrom(ut, msg.sender, address(this), saleInfo.fixprice);
        
        tradeDone(_token,_tokenid,saleInfo.fixprice,to);
    }


   function delist(address _token,uint256 _tokenid) public override {
        SaleInfo storage saleInfo = marketUnits[_token][_tokenid];
        require(saleInfo.seller!=address(0x0),'not on saled');
        require(IERC721(_token).ownerOf(_tokenid)==address(this),'token not in this market');
        require(!saleInfo.isAuction,'token is on auction');
        require(saleInfo.seller==msg.sender || owner() == _msgSender(),'only seller or can delist');
    ...
    function bid(address _token,uint256 _tokenid,uint256 amount,address to) public returns (bool){
        SaleInfo storage saleInfo = marketUnits[_token][_tokenid];
        require(saleInfo.seller!=address(0x0),'not on saled');
        require(IERC721(_token).ownerOf(_tokenid)==address(this),'token not in this market');
        require(saleInfo.isAuction,'token is not on auction');
        require(block.timestamp >= saleInfo.starttime,'token auction is not start');
    ...
    function tradeDone(address _token,uint256 _tokenid,uint amount,address buyer) internal {
        SaleInfo storage saleInfo = marketUnits[_token][_tokenid];
        ...
        if(soldCount[_token][_tokenid]==0){//first sell
            if(IMaxityMetadata(_token).disigner(_tokenid)==address(0x0)){
                TransferHelper.safeTransfer(ut, IMaxityMetadata(_token).ngowallet(),amount.mul(98).div(100));
            }else{//ngo has no designer ability
                TransferHelper.safeTransfer(ut, IMaxityMetadata(_token).ngowallet(), amount.mul(90).div(100));//to ngo
                TransferHelper.safeTransfer(ut, IMaxityMetadata(_token).disigner(_tokenid), amount.mul(8).div(100));//to designer
            }
        ...
        emit TokenIdSaled(_token,_tokenid, buyer,saleInfo.seller,amount, soldCount[_token][_tokenid]);
        saleInfo.seller = address(0x0);
    }
    ...
    function checkAuctionDone(address _token,uint256 _tokenid) public returns(bool){
        ...
        if(saleInfo.deadline >= block.timestamp){
            tradeDone(_token,_tokenid,saleInfo.cur_bid_price,saleInfo.price_offer);
            return true;
        }else{
            return false;
        }   
...
interface IMaxityMarketPlace{
    ...
    function sellByFixPrice(address _token,uint256[] memory _tokenids,uint256 _price,address seller) external  returns (uint) ;

    function auctionMax721(address _token,uint256[] memory _tokenids,uint base_price,uint incre_price,uint starttime,uint deadline,address seller) external returns (uint amountU);
...
interface IMaxity721Token{
    ...
    function mint(address designer,address to,uint count,uint _futureRoyalty,string memory uri) external returns (uint256[] memory new_tokenids);
