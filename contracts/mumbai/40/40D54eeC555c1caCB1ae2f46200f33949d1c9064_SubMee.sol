...
contract SubMee is ERC721 {
    ...
    constructor(
        ...
        address paperKeyManagerAddress_,
        address payoutWallet_,
        address usdcAddress_,
        uint256 price_,
        uint256 priceInUsdc_
    ) ERC721(name_, symbol_) {
        baseURI = baseURI_;
        paperKeyManager = IPaperKeyManager(paperKeyManagerAddress_);
        payoutWallet = payoutWallet_;
        usdc = IERC20(usdcAddress_);
        price = price_;
        priceInUsdc = priceInUsdc_;
    }
    ...
    function _mintMultiple(address to, uint8 quantity) private {
    ...
    function mint(address to, uint8 quantity) public payable {
        require(
    ...
    function mintUsdc(address to, uint8 quantity) public {
        bool transferSuccess = usdc.transferFrom(
            msg.sender,
            payoutWallet,
            quantity * priceInUsdc
        );
        require(transferSuccess, 'Error during transfer of USDC');
        _mintMultiple(to, quantity);
    }

    function mintPaperXyz(address to, uint8 quantity) public {
        // limit mint to paper.xyz wallets: https://docs.withpaper.com/reference/restrict-calls-to-paper
        require(
            msg.sender == address(0xf3DB642663231887E2Ff3501da6E3247D8634A6D) ||
                msg.sender ==
                address(0x5e01a33C75931aD0A91A12Ee016Be8D61b24ADEB) ||
                msg.sender ==
                address(0x9E733848061e4966c4a920d5b99a123459670aEe) ||
                msg.sender ==
                address(0x7754B94345BCE520f8dd4F6a5642567603e90E10),
            'Sender is not paper.xyz'
        );
        _mintMultiple(to, quantity);
    }
    ...
    function registerPaperKey(address _paperKey) public {
        require(paperKeyManager.register(_paperKey), 'Error registering key');
    }

    function mintPaperXyzSignature(
        address to,
        uint8 quantity,
        bytes32 _nonce,
        bytes calldata _signature
        ...
        public
        payable
        onlyPaper(keccak256(abi.encode(to, quantity)), _nonce, _signature)
    {
        _mintMultiple(to, quantity);
    }
}