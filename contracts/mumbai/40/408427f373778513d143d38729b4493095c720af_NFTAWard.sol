...
contract NFTAWard is ERC721Enumerable, Ownable {
  using Strings for uint256;
    ...
    // internal
    function _baseURI(string memory new_URI) internal view virtual returns (string memory) {
        return new_URI;
    }
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        require(_exists(_tokenId),"ERC721Metadata: URI query for nonexistent token");
            ...
            if(bytes(currentPage.message).length > 0) {
                ...
                return _baseURI(currentPage.winURI); 
            }
            else {
    ...
    function setMessage(uint256 _tokenId, string memory _newMessage, string memory result) public onlyAdmin {
    ...
    function setNewURI(string memory _setNewURI) public onlyOwner {
        newURI = _setNewURI;
    }
     ...
     */
    function setNewAdmin(address _adminAddress, bool state) external onlyOwner {
        Admin[_adminAddress] = state;

        emit NewAdmin(_adminAddress, state);(_adminAddress, state);
    }
