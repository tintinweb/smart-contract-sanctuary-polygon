 ...
 */
contract CertificateToken is NonTransferable {
    ...
    mapping(bytes32 => bool) private _number;
    mapping(bytes32 => bool) private _url;

    modifier onlyAllowedMinter() {
        bool isOwner = _msgSender() == owner();
        bool isTrustedForwarder = isTrustedForwarder(msg.sender);
        require(isOwner || isTrustedForwarder, "Only owner or trusted forwarder allowed");
        _;
    }
    ...
    constructor(
        ...
        address forwarder
    ) NonTransferable(_name, _symbol) {
        setTrustedForwarder(forwarder);
    }

    function setTrustedForwarder(address _forwarder) public onlyOwner {
        if (_forwarder != address(0)) {
            super._setTrustedForwarder(_forwarder);
        }
    }
     ...
     */
    function mintTo(address _to, CertificateData memory certificateData) public onlyAllowedMinter {
        require(
            _number[keccak256(abi.encodePacked(certificateData.Number))] != true &&
            _url[keccak256(abi.encodePacked(certificateData.URL))] != true,
            "certificate number and url already exists"
        );
        uint256 currentTokenId = super.mintTo(_to);
        _setTokenURI(currentTokenId, formatTokenURI(certificateData));
        _number[keccak256(abi.encodePacked(certificateData.Number))] = true;
        _url[keccak256(abi.encodePacked(certificateData.URL))] = true;
    }

    function batchMintTo(address[] memory _to,CertificateData[] memory certificateData, string[] memory imageUrl) public payable onlyAllowedMinter {
        require(_to.length == certificateData.length && imageUrl.length == certificateData.length, "not same length");
        uint arrayLength = _to.length;
        for (uint i=0; i<arrayLength; i++) {
            mintTo(_to[i], certificateData[i]);
        }
    }
    ...
    function formatTokenURI(CertificateData memory _info)
        internal
        pure
        returns (string memory){
        bytes memory temp = abi.encodePacked('"Issuer": "', _info.Issuer,
                        '", "Title": "', _info.Title,
                        '", "Owner":"',_info.Owner,
                        '", "Number":"',_info.Number,
                        '", "DateIssue":"', _info.DateIssue,
                        '", "URL":"', _info.URL);
        return string(
            abi.encodePacked(
                "data:application/json;base64,",
                Base64.encode(string(abi.encodePacked('{', temp,'"}')))
            )
        );
    }

    function tokenURI(uint256 _tokenId) override(NonTransferable) public view returns (string memory) {
        return super.tokenURI(_tokenId);
    }

    function _burn(uint256 tokenId) internal override(NonTransferable) {
        super._burn(tokenId);
    }
...
abstract contract NonTransferable is BaseRelayRecipient, ERC721, ERC721URIStorage, ContextMixin, Ownable {
    ...
    function _msgSender() internal view override(BaseRelayRecipient, Context) returns (address ret) {
        return BaseRelayRecipient._msgSender();
    }

    function _msgData() internal view override(BaseRelayRecipient, Context) returns (bytes calldata ret) {
    ...
    function tokenURI(uint256 _tokenId) override(ERC721, ERC721URIStorage) virtual public view returns (string memory)
    {
        return super.tokenURI(_tokenId);
    }

    function _burn(uint256 tokenId) override(ERC721, ERC721URIStorage) virtual internal {
        super._burn(tokenId);
    }
    ...
    function _transfer(
        ...
        require(msg.sender == super.owner() , "Nontransferable token");
        super._transfer(from, to, tokenId);
    }

    function _isApprovedOrOwner(address spender, uint256 tokenId) internal override(ERC721) view virtual returns (bool) {
        address owner = ERC721.ownerOf(tokenId);
        return (msg.sender == super.owner() || spender == owner || isApprovedForAll(owner, spender) || getApproved(tokenId) == spender);
    }
