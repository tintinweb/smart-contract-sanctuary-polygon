...
contract Game is Ownable {
   ...
   * @param _typeFloor lowest card that can be minted
   * @param _amount of cards to be minted in range [1, 25]
   * @param _cardCollection collection
   */
  function mintRandomCards(uint8 _typeFloor, uint256 _amount, uint16 _cardCollection) public payable {
    ...
    for(uint i = 0; i < _amount; i++) {
      ...
      card.mint(msg.sender, tokenId, j, _cardCollection);
    }
  ...
  function upgradeCards(uint256[] calldata _tokenIds, uint8 _targetCardType) public validCardType(_targetCardType) {
    ...
    for(uint256 i = 0; i < _tokenIds.length; i++) {
      ...
      totalValue += card.cardValue(cardType) * (100_000 - houseEdge) / 100_000;
...
contract CardNFT is ERC721, ERC721Enumerable, AccessControl, Ownable {
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    ...
    string memory name = string(abi.encodePacked('"name": "Eternity deck card #', tokenId.toString(), '",'));
