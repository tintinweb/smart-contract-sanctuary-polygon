...
contract Land is ILand, NFT, AccessControl, ReentrancyGuard, Pausable {
    ...
    function createLand(
        ...
        nonReentrant
    {
        _create(_beneficiary, x, y, _categories);
        emit CreateLand(_beneficiary, x, y, _categories);
    }

    function createMultipleLand(
        address _beneficiary,
        uint256[] memory x,
        uint256[] memory y,
        uint256[] memory _categories
    ) external isNotNull(x.length) isNotNullAddress(_beneficiary) nonReentrant {
        require(
            x.length == y.length && x.length == _categories.length,
            "not same length"
        );

        _isMultipleLandExist(x, y);
        _isMultipleCategoriesExist(_categories);

        for (uint256 i = 0; i < x.length; i++) {
            _create(_beneficiary, x[i], y[i], _categories[i]);
        }

        emit CreateMultipleLand(_beneficiary, x, y, _categories);
    }
    ...
    function encodeLandId(uint256 x, uint256 y) external pure returns (uint256) {
    ...
    function _isMultipleCategoriesExist(uint256[] memory _categories)
        internal
        pure
    {
        for (uint256 i = 0; i < _categories.length; i++) {
            require(
                Type(_categories[i]) == Type.Normal ||
                    Type(_categories[i]) == Type.Platinum ||
                    Type(_categories[i]) == Type.Premium,
                "categories not exist"
            );
        }
    }

    function _isMultipleLandExist(uint256[] memory x, uint256[] memory y)
        internal
        view
    {
        for (uint256 i = 0; i < x.length; i++) {
    ...
    function _ownerOfLand(uint256 x, uint256 y) internal view returns (address) {
