...
contract SimiDAO {
    ...
    event ProcessedProposal(uint256 paymentRequested, string details, bool[6] flags, uint256 proposalId, address indexed senderAddress, address indexed beneficiary, string beneficiaryName, string beneficiarySocial);
    ...
    constructor(address[] memory approvedMembers, address _ydt, uint256 _minFund, uint256 _minVote) {
        ...
        votingPeriodLength = 86400; // 7 days = 604800, 1 hr = 3600
    ...
    function receivedDonation(uint _proposalId, uint256 _amount) external {
        ...
        approvedDonations[_proposalId].amountRaised += _amount;
        approvedDonations[_proposalId].donors += 1;
    }
    ...
    function submitDonationProposal(
        ...
        _submitDonationProposal(_name, _goal, _beneficiary, _beneficiaryName, _beneficiarySocial, _media, _duration, details, flags);
    ...
    function processProposal(uint256 proposalId) public onlyMember returns (bool _status) {
        ...
        if(prop.flags[4] == false && prop.flags[5] == false) {
            ...
            emit ProcessedProposal(prop.paymentRequested, prop.details, prop.flags, proposalId, prop.proposer, ben.beneficiary, ben.beneficiaryName, ben.beneficiarySocial);
            return true;
        }
    }
