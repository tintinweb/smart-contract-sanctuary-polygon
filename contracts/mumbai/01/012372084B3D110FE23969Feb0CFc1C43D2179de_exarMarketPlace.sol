...
contract exarMarketPlace is Initializable, OwnableUpgradeable, UUPSUpgradeable, ERC165{
    ...
    */
    ...
    /**
        * setRoyaltyReceiver
        * @param _newReceiver Enter the royalty receiver for the contract.
        * This can only be called by the contract deployer or owner.
    */
    function setRoyaltyReceiver(address _newReceiver) 
    external 
    onlyOwner
    {
        require(msg.sender != address(0),"connect valid address");
        royaltyReceiver = _newReceiver;
    }

    /**
        * royaltyInfo
        * @param salePrice This gives the fee split and the receiver information.
    */
    ...
    {
        return (royaltyReceiver,calculateRoyaltyFee(salePrice));
    }

    /**
        * calculateRoyaltyFee
        * @param _tokenSalePrice Manually calculate the sale price for the nft.
    */
    function calculateRoyaltyFee(uint _tokenSalePrice) 
    internal 
    view 
    returns(uint)
    {
        return (_tokenSalePrice / 10000) * royaltyFee;
    }

    /**
        * setRoyaltyFee
        * @param _royaltyFee Manually set the royalty fee.
        * pips is expected to be in multiples of 100.
        * Eg: For 1 % = 100, For 10% = 1000
    */
    function setRoyaltyFee(uint _royaltyFee) 
    external 
    onlyOwner 
    returns(uint)
    { 
    ...
    */
    ...
    external
    {
        require(auctionStarted[msg.sender][_nftTokenid] == false, "Auction for the nft is already started");    // This doesnt let the previous owner to re-list the same nft.
        require(IERC721(_nftAddress).ownerOf(_nftTokenid) == msg.sender,"You are not the owner of NFT");
        ...
        auctionStarted[msg.sender][_nftTokenid] = true;
        if(block.timestamp > _endDate){
            auctionStarted[msg.sender][_nftTokenid] == false;
        }
        nftEndDate[_nftTokenid] = block.timestamp + _endDate; // if days enter input as 7 days, if seconds enter input as 60.
        nftListerForAuction[_nftTokenid] = msg.sender;
        highestBid[_nftTokenid] = _startingBid;
        highestBidder[_nftTokenid] = msg.sender;
        listedSeller[_nftTokenid] = msg.sender;
    ...
    */
    ...
    external
    {
        if(msg.sender == nftListerForAuction[_nftTokenid]){
    ...
    */
    ...
    external 
    {
        ...
        if(_biddingAmount > highestBid[_nftTokenid] && alreadyBidded[_nftTokenid][msg.sender] == false){
            highestBidder[_nftTokenid] = msg.sender;
            highestBid[_nftTokenid] = _biddingAmount;
            alreadyBidded[_nftTokenid][msg.sender] = true; 
            allBiddedTokens[msg.sender][_nftTokenid] +=  highestBid[_nftTokenid];
            IERC20(_erc20TokenAddress).transferFrom(msg.sender, address(this), allBiddedTokens[msg.sender][_nftTokenid]);
            emit Bid(msg.sender, _biddingAmount);
        }else{   
            uint deduct = _biddingAmount - allBiddedTokens[msg.sender][_nftTokenid];
            uint add = allBiddedTokens[msg.sender][_nftTokenid] + deduct;
            highestBid[_nftTokenid] = add;
            allBiddedTokens[msg.sender][_nftTokenid] += deduct;
            highestBidder[_nftTokenid] = msg.sender;
            IERC20(_erc20TokenAddress).transferFrom(msg.sender, address(this), deduct);
            emit Bid(msg.sender, _biddingAmount);
        }
    }
    ...
    */
    ...
    external
    {
        ...
        if(addressExist[_erc20TokenAddress] != true){ revert tokenNotSupported();}
        if(msg.sender == highestBidder[_nftTokenid]){
            revert pleaseCollectTheNFT();
        }
        uint bal = allBiddedTokens[msg.sender][_nftTokenid];
        IERC20(_erc20TokenAddress).transfer(msg.sender, bal);
    ...
    */
    ...
    external
    {
        ...
        if(msg.sender == highestBidder[_nftTokenid]){
            ...
            uint afterDeductingRoyalty = highestBid[_nftTokenid] - calculateRoyaltyFee(highestBid[_nftTokenid]);
            IERC20(_erc20TokenAddress).transfer(listedSeller[_nftTokenid], afterDeductingRoyalty);
            IERC20(_erc20TokenAddress).transfer(royaltyReceiver, calculateRoyaltyFee(highestBid[_nftTokenid]));
            emit NftAuctionEnd(msg.sender, _nftTokenid);
        }else{
            IERC721(_nftAddress).safeTransferFrom(address(this), listedSeller[_nftTokenid], _nftTokenid);
    ...
    */
    ...
    returns(bool submitted)
    {
        ...
        if(nftAddressExist[_nftAddress] != true){ revert nftAddressNotSupported();}
        require(IERC721(_nftAddress).ownerOf(_nftTokenid) == msg.sender,"You are not the owner of NFT");    // use if condition
        holdLandByLister[_nftTokenid] = msg.sender;   // nft lister address holder.
    ...
    */
    ...
    external
    {
        if(_nftAddress == address(0)){ revert zeroAddressNotSupported();}
        if(addressExist[_erc20TokenAddress] != true){ revert tokenNotSupported();}
        require(IERC20(_erc20TokenAddress).balanceOf(msg.sender) >= _amountForNFT,"You don't have enough balance to buy the NFT");
        if (_amountForNFT == nftValue[_nftTokenid]){
            IERC721(_nftAddress).safeTransferFrom(address(this), msg.sender, _nftTokenid);
            // Deduct Royalty;
            uint afterDeductingRoyalty = _amountForNFT - calculateRoyaltyFee(_amountForNFT);
            IERC20(_erc20TokenAddress).transferFrom(msg.sender, holdLandByLister[_nftTokenid], afterDeductingRoyalty);
            IERC20(_erc20TokenAddress).transferFrom(msg.sender, royaltyReceiver, calculateRoyaltyFee(_amountForNFT));
        }else{
            revert("The amount is mismatching, transaction reverted");
        }
    }
}
