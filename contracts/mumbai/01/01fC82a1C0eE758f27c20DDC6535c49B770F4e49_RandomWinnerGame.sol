...
contract RandomWinnerGame is VRFConsumerBase, Ownable {
    ...
    // Bool to indicate if the game has started or not 
    ...
    // vrfcoordinator = is the address of vrfcoordinator contract
    // linkToken = address of LINK token contract
    // vrffree = the maount of LINK to send with the request
    // vrfkeyhash id = public key against which randomness is generated 
    ...
    function joinGame() public payable {
        ...
        require(gameStarted, "Game has not yet beens started");
        // Check if the value sent by the user matches the entry fee
        require(msg.value == entryFee, "value sent is not equal to the entryFee");
    ...
    // FulfillRandomness is called by the VRFcoordinator when it receives a valid VRF Proof. This is the random number generated by Chainlink VRF. 
    // RequestId = ID is a unique for the request we sent to the VRF coordinator 
    // Randomness is a random uint256 generated and returned to us by the vrf coordinator 
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal virtual override {
        // We want out winnerIndex to be in the length from 0 to players.length - 1
        // We will mode it with the player.length value 
        ...
        // The gameStarted to variable to false 
        gameStarted = false; 
    }
