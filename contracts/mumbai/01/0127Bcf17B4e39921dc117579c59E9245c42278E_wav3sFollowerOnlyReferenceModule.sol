...
contract wav3sFollowerOnlyReferenceModule is
    ...
    FeeModuleBase
{
    ...
    function initializeReferenceModule(
        ...
        (uint256 _budget, uint256 _reward, address _currency, uint256 _minFollowers) = abi.decode(
            data,
            (uint256, uint256, address, uint256)
        );

        // Check that the follower has the minimun followers required
        if (_minFollowers >= 0) {
            //  revert Errors.MirrorToEarn__MinimumFollowersUnderZero();
        }

        // Check that the budget is not equal to 0
        if (_budget == 0) {
            revert Errors.MirrorToEarn__BudgetEqualZero();
        }
        ...
        if (_budget >= thisGlobalBudget) {
            revert Errors.MirrorToEarn__BudgetBiggerThanGlobalBudget();
        }

        // Check that the reward is greater than 0
        if (_reward < 0) {
            revert Errors.MirrorToEarn__RewardEqualZero();
        }
        ...
        _dataByProfileByPost[profileId][pubId].minFollowers = _minFollowers;
    ...
    function processMirror(
        ...
        // Get the minimum followers required to mirror this post
        uint256 __minFollowers = _dataByProfileByPost[profileIdPointed][pubIdPointed].minFollowers;

        // Decode the data passed to the function into the followers of the account
        uint256 followerFollowers = abi.decode(data, (uint256));
        ...
        // Check if the follower has enough followers
        if (followerFollowers < __minFollowers) {
            revert Errors.MirrorToEarn__NotEnoughFollowers();
            ...
            if (__reward >= __budget) {
                revert Errors.MirrorToEarn__NotEnoughPostBudget();
            }
    ...
    /**
     * @dev Adds budget to a post directly to the Post Budget.
     * @param profileId The ID of the profile that owns the post to be funded.
     * @param pubId The ID of the post to be funded.
     * @param refill The amount to be added to the post's current budget.
     * @param _frontend The address of the frontend who is using this module.
     */
    function refillMyPost(
        uint256 profileId,
        uint256 pubId,
        uint256 refill,
        address _frontend
    ) public {
        // Get the owner of the profile and the currency of the post
        address budgetOwner = IERC721(HUB).ownerOf(profileId);
        address _currency = _dataByProfileByPost[profileId][pubId].currency;
        // Transfer funds from the budget owner to the contract
        IERC20(_currency).safeTransferFrom(budgetOwner, address(this), refill);

        // Calculate the fees to be charged
        uint256 fees = (refill * i_fee) / 100;
        ...
        _dataByProfileByPost[profileId][pubId].budget += refill;

        // Emit the PostBudgetFunded event
        emit Events.PostBudgetFunded(profileId, pubId, refill, _currency);
    }

    /**
     * @dev Adds budget to a post from the Global Budget.
     * @param profileId The ID of the profile that owns the post to be funded.
     * @param pubId The ID of the post to be funded.
     * @param refill The amount to be transfered to the post's current budget from the global budget.
     */
    function refillFromBudget(uint256 profileId, uint256 pubId, uint256 refill) public {
        // Get the owner of the profile and the currency of the post
        address budgetOwner = IERC721(HUB).ownerOf(profileId);
        address _currency = _dataByProfileByPost[profileId][pubId].currency;
        // Bring the global budget
        ...
        // Check if the refiller is the owner od the profile

        if (budgetOwner != msg.sender) revert Errors.MirrorToEarn__NotSenderProfile();

        // Check that there is enough budget to refill the post
        if (refill >= thisGlobalBudget) revert Errors.MirrorToEarn__NotEnoughGlobalBudget();

        // Update the post budget
        _dataByProfileByPost[profileId][pubId].budget += refill;
        ...
        GlobalBudget.setGlobalBudget(
            ...
            profileId,
            _currency,
            thisGlobalBudget - refill
        );

        // Emit the PostBudgetFunded event
        emit Events.PostBudgetFunded(profileId, pubId, refill, _currency);
        emit Events.GlobalBudgetWithdrawn(profileId, refill, _currency);
    }
    ...
    function withdrawGlobalBudget(address _currency, uint256 _profileId, uint256 _amount) public {
        if (IERC721(HUB).ownerOf(_profileId) != msg.sender)
            revert Errors.MirrorToEarn__NotSenderProfile();
        ...
        if (_amount > thisGlobalBudget) revert Errors.MirrorToEarn__NotEnoughGlobalBudget();
    ...
    function withdrawPostBudget(uint256 _profileId, uint256 _pubId, uint256 _amount) public {
        ...
        if (_amount > thisPostBudget) {
            ...
            revert Errors.MirrorToEarn__NotEnoughGlobalBudget();
        } else {
...
library Errors {
    ...
    error MirrorToEarn__NotEnoughPostBudget();
    error MirrorToEarn__NotEnoughGlobalBudget();
    error MirrorToEarn__FollowerAlreadyMirrored();
    error MirrorToEarn__NotSenderProfile();
    error MirrorToEarn__BudgetBiggerThanGlobalBudget();
    error MirrorToEarn__BudgetEqualZero();
    error MirrorToEarn__RewardEqualZero();

    // MirrorToEarn Minimun Followers version
    error MirrorToEarn__NotEnoughFollowers();
}
