...
contract NFTCollections is Ownable {
  ...
  mapping(address => bool) ownerCollectionVerify;
  ...
  event CollectionMarketplaceCreated(
    uint256 collectionId, 
    string[] marketplaceData
  );

  event CollectionTagsCreated(
    uint256 collectionId, 
    string[] tags
  );

  modifier onlyVerifyOwners() {
    require(msg.sender == owner() || ownerCollectionVerify[msg.sender] == true, "Only the owner or the verified owners can verify a collection");
    _;
  }
  ...
  function createCollection(
    ...
    emit CollectionMarketplaceCreated(
      _collectionIds.current(),
      _marketplaceData
    );

    emit CollectionTagsCreated(
      _collectionIds.current(),
      _tags
    );
  }

  function verifyCollection(uint256 _collectionId) public onlyVerifyOwners {
    ...
    ownerCollectionVerify[collectionData.collectionInfo.owner] = true;
  }
}
