...
contract KittieNodeRewards is Ownable {
  ...
  function claimDividend(uint256 _tokenId) public {
    ...
    require(mainToken.balanceOf(address(this)) >= unpaid, "not enough liquidity to distribute dividends");
  ...
  function _getTotalNumberClaims(uint256 _tokenId) internal view returns (uint256) {
    uint256 availableClaims = block.timestamp.sub(node.tokenMintedAt(_tokenId)).div(rewardFrequencySeconds);
