...
contract BSG {
    ...
    function renew() internal {
        //renewal by making a little deposit
        uint256 _amount = _calLevelPrice(userInfo[msg.sender].level);
        usdt.transferFrom(msg.sender, address(this), _amount);
    }
    ...
    function _calLevelPrice(uint256 _level) private view returns(uint256) {
        uint256 _price;
        if(_level == 1){_price = 50e6;}
        else if(_level == 2){_price = 500e6;}
        else if(_level == 3){_price = 1000e6;}
        else if(_level == 4){_price = 10000e6;}
        else if(_level >= 5){_price = 50000e6;}
