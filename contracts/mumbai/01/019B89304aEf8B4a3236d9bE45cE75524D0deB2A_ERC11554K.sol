    ...
    ERC2981Upgradeable
{
    ...
    /*
        If an item's guardian class charges storage fees, then the item should have a minimum to transfer
    */
    function _beforeTokenTransfer(
        ...
        for (uint256 i = 0; i < ids.length; ++i) {
            ...
            if (from != address(0) && to != address(0)) {
                require(
                    guardians.storageFeePaidUntil(
                        from,
                        address(this),
                        ids[i]
                    ) >= block.timestamp + guardians.minStorageTime(),
                    "8" //ERC11554K: Not enough storage time to transfer
                );
                guardians.shiftStorageFeesOnTokenMove(
                    from,
                    to,
                    ids[i],
                    amounts[i]
