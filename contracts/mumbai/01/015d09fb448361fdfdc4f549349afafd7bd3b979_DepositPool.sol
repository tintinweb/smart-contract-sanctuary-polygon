...
contract DepositPool is Ownable {
    ...
    uint256 accountWithdrawTotalSupply;
    uint256 controllerWithdrawTotalSupply;
    ...
    event NewAccount(address to,uint256 amount);

    constructor(address _funAddress,address _radTokenAddress,address _treasuryAddress){
        funAddress = _funAddress;
        radToken = _radTokenAddress;
        treasury = _treasuryAddress;
    }

    function withdrawToken(address to,uint256 amount,uint256 timestamp,bytes32 hash,bytes32 r,bytes32 s,uint8 v) public {
        ...
        string memory signDataStr = 
        string(abi.encodePacked("RAD",string(abi.encodePacked("0x",addressToStr(radToken))), 
        string(abi.encodePacked("0x",addressToStr(to))),
        uint2str(amount),uint2str(timestamp),
        string(abi.encodePacked("0x",addressToStr(funAddress)))));
        ...
        IERC20(radToken).transfer(to,amount);
        withdrawAmount[to] += amount;
        accountWithdrawTotalSupply +=amount;
        hashList[hash] = true;

        emit WithdrawToken(radToken,to,amount,timestamp,hash);
    }
    ...
    function withdraw(address account,uint256 amount) public onlyController {
        ...
        controllerWithdrawTotalSupply += amount;
    }
    ...
    function getTotalWithdraw() public view returns (uint256) {
        return accountWithdrawTotalSupply;
    }

    function getWithdrawTotalSupply1 () public view returns (uint256) {
        return controllerWithdrawTotalSupply;
    }
