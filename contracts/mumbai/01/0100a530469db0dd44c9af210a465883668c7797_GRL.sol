...
contract GRL is ERC1155, Ownable {
    ...
    function checkOut(uint _amount) private{
        if (onlyWhitelisted) {
            ...
            require(whitelisterMintedCount + _amount <= whitelisterLimit, "Exceeded Limit");
            require(msg.value >= (_amount * whitelisterCost(msg.sender)), "Insufficient Funds");
    ...
    function _mintInOrder(uint _numberOfTokensToMint) public payable {
        require(mintInOrder, "mintInOrder is false");
        require(!paused, "Paused");
        require(!exists(collectionTotal), "Minted Out");
        require(_numberOfTokensToMint + tokenNextToMint <= collectionTotal + 1, "Lower Amount");
    ...
    function mint(address _to, uint _id, uint _amount) public payable {
        require(!mintInOrder, "mintInOrder is true");
        require(!paused, "Paused");
        require(canMintChecker(_id, _amount), "CANNOT MINT");

        //This is if the Original Tokens are only 1 of 1s
        if (_id <= ogCollectionTotal){
            require(oneOfOneOnly(_id, _amount), "Amount must be 1");
            require(!createdToken[_id], "Already Minted");
            maxSupply[_id] = 1;
            hasMaxSupply[_id] = true;
        }
    ...
    function mintBatch(address _to, uint[] memory _ids, uint[] memory _amounts) public payable {
        require(!mintInOrder, "mintInOrder is true");
        require(!paused, "Paused");
        require(_ids.length <= maxMintAmount, "Limit Exceeded");
        require(_ids.length == _amounts.length, "Arrays Not Equal");
        require(canMintBatchChecker(_ids, _amounts), "CANNOT MINT");
        ...
        require(_totalBatchAmount <= maxBatchMintAmount, "Limit Exceeded");
    ...
    function adminMint(address _to, uint _id, uint _amount) external onlyAdmins {
        require(_id > ogCollectionTotal, "NOT NEW");
        if (!exists(_id)) {
            createdToken[_id] = true;
            flagged[_id] = false;
        }
    ...
    function adminMintBatch(address _to, uint[] memory _ids, uint[] memory _amounts) external onlyAdmins {
        require(!checkIfOriginal(_ids), "NOT NEW");
        for (uint256 i = 0; i < _ids.length; ++i) {
            uint256 _id = _ids[i];
            if (!exists(_id)) {
                createdToken[_id] = true;
        ...
        if(createdToken[_id]){
            if (_id > 0 && _id <= ogCollectionTotal) {
                ...
                if(!revealed){
                    ...
                    // //has A role
                    // if(keccak256(abi.encodePacked((role[_id]))) == keccak256(abi.encodePacked(("A")))){
                    //     return (
                    //     string(abi.encodePacked(
                    //         "ipfs://",
                    //         ipfsCID,
                    ...
                    // }
                    // //has B role
                    // if(keccak256(abi.encodePacked((role[_id]))) == keccak256(abi.encodePacked(("B")))){
                    //     return (
                    //     string(abi.encodePacked(
                    //         "ipfs://",
                    //         ipfsCID,
                ...
                //no URI
                return "Token URI Does Not Exist Yet";
            }
            else {
    ...
    * Note: If the returned value is 0 Max Supply is infinite.
    */
