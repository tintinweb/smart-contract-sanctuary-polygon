...
contract AuxiliaryDeviceFactory is Ownable, ReentrancyGuard, AccessControlEnumerable, IFactory, Pausable {
    ...
    struct AuxiliaryDevice {
        ...
        uint256 count;               //实际数量
        string margin_name;          //余量属性名称, 总和减去其它
    }
    ...
    //index 与 属性=>配置 的映射
    mapping(uint256 => mapping(string =>Setting)) settings;
 
    mapping(string => uint) props;
    ...
    constructor(address _mgge, address _serverSide, uint256 _salt) ReentrancyGuard() {
        ...
        //初始化
        //能量盾1（防御） 
        uint _type_id = _addItem(110101,100,300,"defense");
        _setPropValues(_type_id, "attack",0,0);
        _setPropValues(_type_id, "defense",150,200);
        _setPropValues(_type_id, "hp",80,100);
        _setPropValues(_type_id, "duration",80,100);
        _setPropValues(_type_id, "potential",1,5);
        _setPropValues(_type_id, "move",0,0);
        //能量盾2（生命） 
        _type_id = _addItem(110102,150,310,"hp");
        _setPropValues(_type_id, "attack",0,0);
        _setPropValues(_type_id, "defense",80,100);
        _setPropValues(_type_id, "hp",150,200);
        _setPropValues(_type_id, "duration",80,100);
        _setPropValues(_type_id, "potential",1,5);
        _setPropValues(_type_id, "move",0,0);
        //能量盾3（均衡） 
        _type_id = _addItem(110103,200,320,"duration");
        _setPropValues(_type_id, "attack",0,0);
        _setPropValues(_type_id, "defense",100,150);
        _setPropValues(_type_id, "hp",100,150);
        _setPropValues(_type_id, "duration",80,100);
        _setPropValues(_type_id, "potential",1,5);
        _setPropValues(_type_id, "move",0,0);
        //浮游1（攻击） 
        _type_id = _addItem(110201,200,301,"attack");
        _setPropValues(_type_id, "attack",270,300);
        _setPropValues(_type_id, "defense",10,50);
        _setPropValues(_type_id, "hp",0,0);
        _setPropValues(_type_id, "duration",10,50);
        _setPropValues(_type_id, "potential",1,5);
        _setPropValues(_type_id, "move",10,20);
        //浮游2（生命） 
        _type_id = _addItem(110202,100,311,"hp");
        _setPropValues(_type_id, "attack",170,200);
        _setPropValues(_type_id, "defense",20,50);
        _setPropValues(_type_id, "hp",70,100);
        _setPropValues(_type_id, "duration",10,50);
        _setPropValues(_type_id, "potential",1,5);
        _setPropValues(_type_id, "move",10,20);
        //浮游3（均衡） 
        _type_id = _addItem(110203,100,321,"duration");
        _setPropValues(_type_id, "attack",150,190);
        _setPropValues(_type_id, "defense",10,30);
        _setPropValues(_type_id, "hp",10,30);
        _setPropValues(_type_id, "duration",10,50);
        _setPropValues(_type_id, "potential",1,5);
        _setPropValues(_type_id, "move",10,20);        
    }
    ...
    function _setPropValues(uint _type_id, string memory _prop_name, uint _min, uint _max) internal {
        settings[_type_id][_prop_name] = Setting (
            _min,
            _max - _min
        );  
    }
  ...
  // MANAGING Authorization Contract Address for Call 
   ...
   // 合约部署者增加操作员
        ...
        returns(uint[] memory)
    {
        ...
        // if (settings[_type_id]["attack"].section > 0) {
        //     new_rand = (randomResult.sub(randomResult%10)).div(10);
        //     props["attack"] = settings[_type_id]["attack"].base + new_rand % settings[_type_id]["attack"].section;
        // }
        props["attack"] = 1; //settings[_type_id]["attack"].base;

        // if (settings[_type_id]["defense"].section > 0 ) {
        //      new_rand = (randomResult.sub(randomResult%100)).div(100);
        //      props["defense"] = settings[_type_id]["defense"].base + new_rand % settings[_type_id]["defense"].section;
        // }

        props["defense"] = 1; //settings[_type_id]["defense"].base;

        // if (settings[_type_id]["hp"].section > 0) {
        //     new_rand = (randomResult.sub(randomResult%1000)).div(1000);
        //     props["hp"]      = settings[_type_id]["hp"].base + new_rand % settings[_type_id]["hp"].section;
        // }

        props["hp"]      = 1; //settings[_type_id]["hp"].base;

        // if (settings[_type_id]["duration"].section > 0) {
        //     new_rand = (randomResult.sub(randomResult%10000)).div(10000);
        //     props["duration"]= settings[_type_id]["duration"].base + new_rand % settings[_type_id]["duration"].section;
        // }
        props["duration"]= 1; //settings[_type_id]["duration"].base;

        uint margin =auxiliaryDevice.prop_sum - (props["attack"] + props["defense"] + props["hp"] + props["duration"]);
        props[auxiliaryDevice.margin_name] += margin;

    //    if (settings[_type_id]["move"].section > 0) {
    //         new_rand = (randomResult.sub(randomResult%100000)).div(100000);
    //         move = settings[_type_id]["move"].base +new_rand % settings[_type_id]["move"].section; 
    //     }

    //     if (settings[_type_id]["potential"].section > 0) {
    //         new_rand = (randomResult.sub(randomResult%1000000)).div(1000000);
    //         potential = settings[_type_id]["potential"].base +new_rand % settings[_type_id]["potential"].section;
    //     }
        ...
        charactor[0] =  props["attack"];
        charactor[1] =  props["defense"];
        charactor[2] =  props["hp"];
        charactor[3] =  props["duration"];
    ...
    // }
        ...
        props["attack"]  = settings[_type_id]["attack"].base + 1; // new_rand % settings[_type_id]["attack"].section;
        props["defense"] = settings[_type_id]["defense"].base + 1;// new_rand % settings[_type_id]["defense"].section;
        props["hp"]      = settings[_type_id]["hp"].base + 1; // new_rand % settings[_type_id]["hp"].section;
        props["duration"]= settings[_type_id]["duration"].base + 1; //new_rand % settings[_type_id]["duration"].section;
    

        uint margin = auxiliaryDevices[_type_id].prop_sum - (props["attack"] + props["defense"] + props["hp"] + props["duration"]);
        props[auxiliaryDevices[_type_id].margin_name] += margin;

        move = settings[_type_id]["move"].base +1; //new_rand % settings[_type_id]["move"].section; 
        potential = settings[_type_id]["potential"].base +1; //new_rand % settings[_type_id]["potential"].section;
        ...
        returns(uint[] memory)
    {
        ...
        if (rand == 0) {
            ...
            ch.duration += newValue; 
        }
        
        uint[] memory charactor = new uint[](6);
        charactor[0] =  ch.attack;
        charactor[1] =  ch.defense;
        charactor[2] =  ch.hp;
        charactor[3] =  ch.duration;
        charactor[4] =  ch.move;
        charactor[5] =  ch.potential;
        return charactor;
    }
    ...
    function getItemSetting(
        uint256 _type_id,
        string memory prop_name
    )
        public
        view
    returns (Setting memory)
    {
        return settings[_type_id][prop_name];
    }    
