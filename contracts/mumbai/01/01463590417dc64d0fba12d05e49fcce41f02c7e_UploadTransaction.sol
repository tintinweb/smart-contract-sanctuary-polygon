...
contract UploadTransaction is Ownable {
    ...
    mapping(address=>Data[]) accountRecords;

    

    function upload(address account,string[] memory records) public onlyController {
       
    //    uint256 length = records.length;
    //    for(uint256 i=0;i<length;i++){
    //         accountRecords[account].push(records[i]);
    //    }
    }

    // function getAccountRecordsLength(address account) public view returns (uint256){
    //     return accountRecords[account].length;
    // }

    // function getAccountRecordsByIndex(address account,uint256 index) public view returns (Data memory){
    //     return accountRecords[account][index];
    // }
}