...
// File: contracts/GetDrawDataContract.sol
...
contract GetDrawDataContract is ChainlinkClient, ConfirmedOwner {
    using Chainlink for Chainlink.Request;
    ...
    event LogDrawData(string hash, string token, string createdAt);

    bytes32 private jobId;
    uint256 private fee;

    //string public hash; // zmienic na private?
   // string public token;
    ...
    constructor() ConfirmedOwner(msg.sender) {
        ...
        jobId = "7a0d4de8efc448309e55d988708a2ca3";
     ...
     */
    function draw(string memory url, string memory lotterySlug, string memory drawDate) payable external{
        ...
        req.add("path1", "hash");
        req.add("path2", "token");
        req.add("path3", "created_at");

        bytes32 queryId = sendChainlinkRequest(req, fee);
    ...
    function __callback(
        ...
        emit LogDrawData(_hash, _token, _createdAt);
      //  hash = _hash;
      //  token = _token;
       // createdAt = _createdAt;

        if (!validIds[requestId]) revert();
