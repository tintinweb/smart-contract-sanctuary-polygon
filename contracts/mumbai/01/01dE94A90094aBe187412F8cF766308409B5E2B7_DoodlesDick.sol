...
contract DoodlesDick is ERC721A, MultisigOwnable, VRFConsumerBaseV2 {
  ...
  Dick public immutable dick;

  // Keys are dick token ids
  mapping(uint256 => bool) public dickCanClaim;
  ...
  constructor(
    address _dickAddress,
    uint256 _maxSupply,
    address _vrfCoordinator,
    address _linkToken,
    string memory initialName,
    string memory initialSymbol
  ) ERC721A('Doodles Dick', 'Doodles Dick') VRFConsumerBaseV2(_vrfCoordinator) {
    dick = Dick(_dickAddress);
  ...
  function airdrop(
    ...
    uint256[] calldata numDickTokens,
    uint256 chunkNum
  ) external onlyRealOwner {
    if (processedChunksForAirdrop[chunkNum] || balanceOf(receivers[0]) > 0) {
      revert ChunkHasBeenAirdropped();
    }
    if (receivers.length != numDickTokens.length) {
      revert InvalidChunk();
    }
    for (uint256 i; i < receivers.length; ++i) {
      _mintWrapper(receivers[i], numDickTokens[i]);
    }
    processedChunksForAirdrop[chunkNum] = true;
  }

  // Used to claim unclaimed tokens after airdrop/claim phase
  function devClaim(uint256 numDickTokens) external onlyRealOwner {
    _mintWrapper(msg.sender, numDickTokens);
  }

  function claim(uint256[] calldata dickTokenIds) external {
    ...
    for (uint256 i; i < dickTokenIds.length; ++i) {
      uint256 dickId = dickTokenIds[i];

      Dick.TokenOwnership memory tokenOwnership = dick.getOwnershipData(
        dickId
      );
      address tokenOwner = tokenOwnership.addr;
      if (tokenOwner != msg.sender) revert MismatchedTokenOwner();
      if (!dickCanClaim[dickId]) revert BeanCannotBeClaimed();
      ...
      if (block.timestamp - ownershipStart < MIN_OWNERSHIP_TIME_FOR_CLAIM) {
        revert DickNotOwnedLongEnough();
      }
      dickCanClaim[dickId] = false;
    }
    _mintWrapper(msg.sender, dickTokenIds.length);
  }

  function _mintWrapper(address to, uint256 numDickTokens) internal {
    uint256 numToMint = numDickTokens * 2;
  ...
  function setCanClaim(uint256[] calldata dickIds) external onlyRealOwner {
    for (uint256 i; i < dickIds.length; ++i) {
      dickCanClaim[dickIds[i]] = true;
    }
  }
