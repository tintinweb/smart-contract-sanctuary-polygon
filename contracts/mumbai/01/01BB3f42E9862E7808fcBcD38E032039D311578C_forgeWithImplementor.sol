...
import "./Implementor/ITraitUint8ValueImplementer.sol";
import "./TraitRegistry/IECRegistry.sol";

pragma solidity ^0.8.0;

contract forgeWithImplementor is Ownable {
    // Card related
    uint256 public claimCount = 0;
    IERC721 public dr_contract_address; //= IERC721(0x922A6ac0F4438Bf84B816987a6bBfee82Aa02073);
    IECRegistry public traitRegistry; // 0x4E6ed01C6d4C906aD5428223855865EF75261338
    ITraitUint8ValueImplementer public Implementer; // 0x4E6ed01C6d4C906aD5428223855865EF75261338

    // Events
    event setClaimStatus(uint256 __tokenIDFirst, uint256 __tokenIDMid, uint256 __tokenIDLast);
    event setMultipleClaimStatus(uint16[] __tokenID, bool _status);

    constructor(
        address _dr_contract_address,
        address _tr,
        address _implementer
    ) {
        dr_contract_address = IERC721(_dr_contract_address);
        traitRegistry = IECRegistry(_tr);
        Implementer = ITraitUint8ValueImplementer(_implementer);
    }
    ...
    function isInitialized() external view returns (bool) {
        ...
        return traitRegistry.addressCanModifyTrait(address(this), 0);
    }
...
*/
    function claim(
        ...
        // Implementer
        require(getRedeemStatus(__tokenIDFirst, __tokenIDMid, __tokenIDLast), "fail combination");
        ...
        uint16[] memory indexes = new uint16[](3);
        indexes[0] = __tokenIDFirst;
        indexes[1] = __tokenIDMid;
        indexes[2] = __tokenIDLast;
        ...
        Implementer.setDataNoEvent(indexes, zeroIndexes);
        // Event
        emit setClaimStatus(__tokenIDFirst, __tokenIDMid, __tokenIDLast);
    }
    ...
    function getRedeemStatus(
        ...
        uint256 midValue = Implementer.getValue(__tokenIDMid);
        if (Implementer.getValue(__tokenIDFirst) + 1 == midValue) {
            if (Implementer.getValue(__tokenIDLast) - 1 == midValue) {
                return true;
            }
        }
        return false;
    ...
    function getValue(uint16 __tokenID) public view returns (uint8) {
        return Implementer.getValue(__tokenID);
    }

    function getTraitsStatus(uint16 _tokenId, uint16 _traitValue) public view returns (bool) {
        return (Implementer.getValue(_tokenId) == _traitValue);
    }
...
interface ITraitUint8ValueImplementer {
    function setDataNoEvent(uint16[] memory _tokenIds, uint8[] memory _value) external;

    // update multiple token values at once
    function setData(uint16[] memory _tokenIds, uint8[] memory _value) external;

    function setSingleData(uint16 _tokenIds, uint8 _value) external;
