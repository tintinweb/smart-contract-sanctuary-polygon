...
contract ESTATERegistryTest is ERC721, IERC2981, Ownable, IEstateRegistry, IERC721Receiver, EstateStorage {
    ...
    address public LandContract = 0x57E3179Ca015085aa777C7E8516681A2C7fa10a7;

    ILANDRegistry landRegistry = ILANDRegistry(LandContract);
    ...
    */
    ...
    constructor() ERC721("CDBEstate", "CDBE") {
        setBaseURI("https://62cd7d9b066bd2b69926fd66.mockapi.io/api/v1/estate/");
    }
    ...
    modifier onlyLandRegistry() {
        require(msg.sender == address(landRegistry), "Only the land registry can make this operation");
        _;
    }
    ...
    function _mintEstate(address to, string memory metadata) internal returns (uint256) {
        ...
        _setTokenURI(estateId, Strings.toString(estateId));
    ...
    function _bytesToBytes32(bytes memory b) internal pure returns (bytes32) {
        ...
        out |= bytes32(b[i] & 0xFF) >> (SafeMath.mul(i, 8));
        }
    ...
    */
    ...
    public /**onlyOwner*/ returns (bytes4)
    {
        ...
        console.log("EstateRegistry, onERC721Received, sender: %s, estateId: %s", msg.sender, estateId);
    ...
    function _pushLandId(uint256 estateId, uint256 landId) internal {
        ...
        // require(
        //     landRegistry.ownerOf(landId) == address(this),
     ...
     * @param metadata string metadata
     */
    function updateMetadata(uint256 estateId, string memory metadata) external {
        _updateMetadata(estateId, metadata);
    }
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function transferFrom(
        ...
        console.log("Transfer From, sender: %s, tokenId: %s", _msgSender(), tokenId);
    ...
    function safeTransferFrom(
        ...
        console.log("safeTransferFrom 1, From: %s, To: %s", from, to);
        console.log("safeTransferFrom 2, Message sender in ERC721: %s, owner is: %s", _msgSender(), getApproved(tokenId));
        require(_isApprovedOrOwner(_msgSender(), tokenId), "ERC721: caller is not token owner nor approved");
    ...
    function _transfer(
        ...
        console.log("_transfer, from: %s, to: %s, ownerOf: %s", from, to, ERC721.ownerOf(tokenId));
...
contract LANDRegistryTest is ILANDRegistry, ERC721, IERC2981, ReentrancyGuard, Ownable, LANDStorage {
    ...
    address public EstateInterface = 0xBb37AE12E922c84A4708c5629880B5c5ba9C55A2;

    bool public saleIsActive = false;    

    constructor() ERC721("CDBLand", "CDBL") {
        setBaseURI("/");

        console.log("Contract deployed from %s", msg.sender);
    }
    ...
    modifier isValidCoords(int256 x, int256 y) {
        require(SignedMath.abs(x) <= getGridWidth(), "X coordinate is out of boundaries");
        require(SignedMath.abs(y) <= getGridHeight(), "Y coordinate is out of boundaries");
        _;
    }
    ...
    function isLandCoordsAvailable(int256 x, int256 y) public view returns (bool) {
    ...
    function canMint(uint256 _tokenId) public view returns (bool) {
        ...
        return hasEnoughSupply() && isValidTokenId(_tokenId) && isLandAvailable(_tokenId) && isNotMinted(_tokenId);
    }

    function canMintByCoords(int256 x, int256 y) public view returns (bool) {
        return hasEnoughSupply() && isValidTokenId(getTokenId(x, y)) && isLandCoordsAvailable(x, y) && isNotMinted(getTokenId(x, y));
    }
    ...
    function getTokenId(int256 x, int256 y) public pure isValidCoords(x, y) returns (uint256) {
    ...
    function encodeTokenId(int256 x, int256 y) external pure isValidCoords(x, y) returns (uint256) {
        return _encodeTokenId(x, y);
    }

    function _encodeTokenId(int256 x, int256 y) internal pure returns (uint256 result) {
        return getTokenId(x, y);
    }

    function decodeTokenId(uint256 tokenId) external pure returns (int256, int256) {
        return _decodeTokenId(tokenId);
    }

    function _decodeTokenId(uint256 tokenId) internal pure returns (int256 x, int256 y) {
        return (getX(tokenId), getY(tokenId));
    }    
    ...
    function transferLandToEstate(
        ...
        uint256 tokenId = getTokenId(x, y);
        safeTransferFrom(ownerOf(tokenId), address(estateRegistry), tokenId);
    }
    ...
    function createEstate(
        ...
        return _createEstate(x, y, to, "");
    }

    function createEstateTest(
        int256[] calldata x,
        int256[] calldata y,
        address to
    ) external onlyOwner returns (uint256) {
        // solium-disable-next-line arg-overflow
        return _createEstateTest(x, y, to, "");
    }
    ...
    // function _clearApproval(address holder, uint256 tokenId) internal {
    //     if (ownerOf(tokenId) == holder && ERC721._tokenApprovals[tokenId] != 0) {
    //     ERC721._tokenApprovals[tokenId] = 0;
    //     emit Approval(holder, 0, tokenId);
    //     }
    // }    
    ...
    function approveToken(address to, uint256 tokenId) public virtual {
        ...
        console.log("approveToken, owner is %s and token approval is %s", owner, ERC721.getApproved(tokenId));
    }        
    ...
    function _createEstate(
        ...
        uint256 estateTokenId = estateRegistry.mint(to, metadata);

        console.log("Created estateTokenId: %d", estateTokenId);

        for (uint256 i = 0; i < x.length; i++) {
            uint256 tokenId = getTokenId(x[i], y[i]);
            // estateRegistry.requestTransfer(ownerOf(tokenId), msg.sender, tokenId);
            
            // _clearApproval(ownerOf(tokenId), tokenId);
            // estateRegistry._approve(ownerOf(tokenId), tokenId);
            approveToken(msg.sender, tokenId);
            // approveToken(address(estateRegistry), tokenId);

            safeTransferFrom(ownerOf(tokenId), msg.sender, tokenId, toBytes(estateTokenId));
            safeTransferFrom(msg.sender, address(estateRegistry), tokenId, toBytes(estateTokenId));
    ...
    // Test per provare a chiamare solo la funzione mint tramite interfaccia

    function _createEstateTest(
        int256[] calldata x,
        int256[] calldata y,
        address to,
        string memory metadata
...
interface ILANDRegistry {
    ...
    function mintToken(address to, int256 x, int256 y) external;

    function mintMultipleTokens(address to, int256[] calldata x, int256[] calldata y) external;

    function setEstateRegistry(address registry) external;

    function createEstate(int256[] calldata x, int256[] calldata y, address to) external returns (uint256);
...
contract LANDStorage {
    ...
    uint256 internal constant GRID_SIZE_X = 200;
    uint256 internal constant GRID_SIZE_Y = 200;
