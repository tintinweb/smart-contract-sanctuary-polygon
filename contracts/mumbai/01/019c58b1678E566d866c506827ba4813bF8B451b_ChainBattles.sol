...
//Contract deployed to:  0xbfa37bc42ecF221b232477F48eb2B22F4bde771C


contract ChainBattles is ERC721URIStorage  { // my contract inherits from the oz ERC721URIStorage contract
    // pull in other oz contracts here
    using Strings for uint256; 
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    // create structure for player data
    struct PlayerData {
      uint256 level;
      uint256 speed;
      uint256 strength;
    ...
    function mint() public {
        ...
        PlayerData memory playerData = PlayerData(0, createRandom(2), createRandom(3), createRandom(4)); // add level, speed, strength, life values to playerData structure

        tokenIdToPlayerData[newItemId] = playerData; // adds new playerData struct to the master tokenIdToPlayerData struct
    ...
    function train(uint256 tokenId) public {
      ...
      PlayerData memory currentPlayerData = tokenIdToPlayerData[tokenId]; // sets up currentPlayerData struct
      uint256 currentLevel = currentPlayerData.level; // determine the current level
      currentPlayerData.level = currentLevel + 1; // +1 to current level
      tokenIdToPlayerData[tokenId] = currentPlayerData; // set tokenIdToPlayerData for tokenId based on the current player's data
      _setTokenURI(tokenId, getTokenURI(tokenId)); // set the token URI base on tokenId
    }

    // create a random number (presumably not completely random)
