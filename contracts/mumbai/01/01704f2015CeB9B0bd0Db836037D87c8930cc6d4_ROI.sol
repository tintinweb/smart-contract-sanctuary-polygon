...
contract ROI {
    ...
    function _setUpdaddy(address _addr, address _upline) private {
        if (
            ...
            investorsMap[_upline].deposits.length > 0
        ) {
    ...
    function setDepositeFlag() internal
    {
        if(restartTime>0){
            investorsMap[msg.sender].isInvestedRestart = true;
        }
    }

    function checkContractIsRestarted() internal view{
        if(restartTime>0)
        require(block.timestamp > restartTime.add(24 hours),"Contract not started yet after restart");
    }
    ...
    function deposit(uint256 _amount, address _upline) public payable {
        ...
        collect(msg.sender);
        checkContractIsRestarted();
        if (investorsMap[msg.sender].deposits.length == 0 && restartTime==0) {
            require(
