...
contract ChestConfig is ICHESTCONFIG, AccessControl {
    bytes32 public constant ADMIN_ROLE = keccak256("ADMIN_ROLE");
    bytes32 public constant WRITER_ROLE = keccak256("WRITER_ROLE");
    bytes32 public constant READER_ROLE = keccak256("READER_ROLE");

    constructor() {
        // Asignar el primer administrador durante la creación del contrato
    ...
    // Modificador que requiere el rol de administrador
    modifier onlyAdmin() {
        require(hasRole(ADMIN_ROLE, msg.sender), "Restricted to admins");
        _;
    }

    // Modificador que requiere el rol de escritor
    modifier onlyWriter() {
        require(hasRole(WRITER_ROLE, msg.sender), "Restricted to writers");
        _;
    }

    // Modificador que requiere el rol de lector
    modifier onlyReader() {
        require(hasRole(READER_ROLE, msg.sender), "Restricted to readers");
        _;
    }

    // Función para asignar el rol de administrador a una cuenta
    function grantAdminRole(address account) public onlyAdmin {
        grantRole(ADMIN_ROLE, account);
    }

    // Función para revocar el rol de administrador de una cuenta
    function revokeAdminRole(address account) public onlyAdmin {
        revokeRole(ADMIN_ROLE, account);
    }

    // Función para asignar el rol de escritor a una cuenta
    function grantWriterRole(address account) public onlyAdmin {
        grantRole(WRITER_ROLE, account);
    }

    // Función para revocar el rol de escritor de una cuenta
    function revokeWriterRole(address account) public onlyAdmin {
        revokeRole(WRITER_ROLE, account);
    }

    // Función para asignar el rol de lector a una cuenta
    function grantReaderRole(address account) public onlyAdmin {
        grantRole(READER_ROLE, account);
    }

    // Función para revocar el rol de lector de una cuenta
    function revokeReaderRole(address account) public onlyAdmin {
    ...
    ChestConfiguration[] private chestConfigurations; // Array que almacena las configuraciones de los cofres
    ...
    mapping(address => Token) private validPaymentTokens;
    ...
    address[] private validAddressPayments;
    ...
    {
        require(
            _id >= 0 && _id < chestConfigurations.length,
    ...
    function isValidPaymentToken(
        address _addressTokenContract
    ) public view onlyReader returns (bool status) {
    ...
    function getValidPaymentData(
        address _addressTokenContract
    ) public view onlyReader returns (uint decimals_, string memory symbol_) {
     ...
     * @return tokens Array de estructuras ListToken que contiene la información de los tokens de pago válidos (Public)
     */
