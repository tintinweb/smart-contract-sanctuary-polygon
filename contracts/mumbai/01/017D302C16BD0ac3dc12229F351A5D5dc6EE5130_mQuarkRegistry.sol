...
interface ImQuarkRegistry {
   ...
   * @param subscriptionPrice              Slot price for the project
   */
  function registerProject(
    ...
    string calldata projectSlotDefaultURI,
    uint256 subscriptionPrice // ) external onlyRole(AUTHORIZED_REGISTERER_ROLE) {
  ) external;
...
interface ImQuarkSubscriber {
  ...
  function setSigner(uint64 projectId, address signer) external;
  ...
  function getProjectConfig(
    ...
    external
    view
    returns (uint64 projectId_, uint256 subscriptionPrice, string memory defaultURI, bool uriSet, address signer);
  ...
  function getProjectBalance(uint64 projectId) external view returns (uint256);

  function getIsAddressRegisteredAsProject(address address_) external view returns (bool);

  function calculateBatchSubscriptionPrice(uint64[] calldata subscriptionIds) external view returns (uint256);

  function withdraw(uint64 projectId, uint256 amount) external;
...
contract mQuarkController is AccessControl, ImQuarkController, NoDelegateCall {
  ...
  function setRegistryContract(address _addr) external onlyRole(CONTROL_ROLE) noDelegateCall {
    ...
    emit RegistryContractAddressSet(_addr);
  }
...
contract mQuarkProject is ImQuarkProject, Ownable, NoDelegateCall {
  ...
  {
    return s_registry.getRegisteredProject(s_ID);
  }
}
 ...
 */
contract mQuarkRegistry is ImQuarkRegistry, mQuarkProjectDeployer, AccessControl, NoDelegateCall {
   ...
   */
  mapping(uint256 => uint256) private s_projectSlotPrices;

  /**
   * Mapping from 'project id' to 'project struct'
   */
  mapping(uint256 => Project) private s_registeredProjects;
  ...
  /// @notice The last registered project ID
  uint64 private s_lastProjectId;
   ...
   * @param _thumbnail              Thumbnail url
   * @param _projectSlotDefaultURI  The uri that will be assigned to project slot initially
   * @param _slotPrice              Slot price for the project
   */
  function registerProject(
    string calldata _projectName,
    string calldata _description,
    string calldata _thumbnail,
    string calldata _projectSlotDefaultURI,
    uint256 _slotPrice
  ) external noDelegateCall {
    uint64 m_projectId = ++s_lastProjectId;
    address m_project = deploy(ImQuarkRegistry(address(this)), s_subscriber, msg.sender, m_projectId);
    address m_contractAddress = address(m_project);

    s_registeredProjects[m_projectId] = Project(
      msg.sender,
      m_contractAddress,
      m_projectId,
      s_registeredProjects[m_projectId].balance,
      _projectName,
      _description,
      _thumbnail,
      _projectSlotDefaultURI
    );

    s_projectIds[m_contractAddress] = m_projectId;
    s_projectSlotPrices[m_projectId] = _slotPrice;
    ImQuarkSubscriber(s_subscriber).initializeProject(
      m_contractAddress,
      m_projectId,
      msg.sender,
      _projectSlotDefaultURI,
  ...
  /// @notice Getter function to retrieve the project id
  function getProjectId(address _contractAddress) external view returns (uint256) {
    return s_projectIds[_contractAddress];
  }

  function getProjectAddress(uint64 _projectId) external view returns (address) {
    return s_registeredProjects[_projectId].contractAddress;
  }
  ...
  /**
   * @notice Returns registered project
   *
   * @return contractAddress         Contract address
   * @return creator                 Creator address
   * @return id                      ID
  ...
  {
    Project memory m_registeredProjects = s_registeredProjects[_projectId];
    return (
      m_registeredProjects.contractAddress,
      m_registeredProjects.creator,
      m_registeredProjects.id,
      m_registeredProjects.balance,
      m_registeredProjects.name,
      m_registeredProjects.description,
      m_registeredProjects.thumbnail,
      m_registeredProjects.projectSlotDefaultURI
    );
  }

  function getProjectSlotPrice(uint256 _projectId) external view returns (uint256) {
    return s_projectSlotPrices[_projectId];
  }
