...
contract BloomThis is ERC721, Ownable, ERC2981 {
    ...
    function mint(address receiver, string memory tokenURL, uint8 kind, uint256 newItemId) validAdmin external returns (uint256) {
        return _mintTo(receiver, tokenURL, kind, newItemId);
    }

    //Used to mint NFT for a receiver. he has to provice token url and kind
    function _mintTo(address receiver, string memory tokenURL, uint8 kind, uint256 newItemId) private returns (uint256) {
        if(_maxTokens > 0) {
            require(_maxTokens > _tokenCounter.current(), "Tokens exhausted");
        }
        require(_tokenCounter.current() + 1 == newItemId, "Invalid Id provided");
    ...
    function doFusion(uint8 ruleNo, uint256[] memory ids) external {
        ...
        _mintTo(msg.sender, uri, _rules[ruleNo][0], _tokenCounter.current() + 1);
    }
