...
import './IMissionCoordinator.sol';
...
import '../GameplayCoordinator/IGameplayCoordinator.sol';
import '../../../minting/ScionCoordinator/IScionCoordinator.sol';

contract MissionCoordinator is IMissionCoordinator, Controllable, Payable {
  event MissionBegun(uint256 indexed botId, uint256 missionId);
  event MissionComplete(uint256 indexed botId, Result missionResult);
  event MissionCanceled(uint256 indexed botId, Result missionResult);
  event MissionAdded(uint256 indexed missionId, IMission missionContract);
  event AirdropMissionAdded(uint256 indexed missionId);

  IInventoryCoordinator public inventoryCoordinator;
  IGameplayCoordinator public gameplayCoordinator;
  ILocationCoordinator public locationCoordinator;
  IScionCoordinator public scionCoordinator;

  mapping(uint256 => IMission) public availableMissions;
  //to avoid duplication of missions
  mapping(IMission => uint256) public setMissions;
  mapping(uint256 => uint256) public override botMissions;
  mapping(uint256 => Result[]) botMissionHistory;
  mapping(uint256 => RewardHistoryItem[]) botRewardHistory;
  uint256 private missionIdCounter = 0;

  //"Airdropped" location-based missions //
  mapping(uint256 => string) public airdrop_bot_origins;
  mapping(uint256 => string) public airdrop_bot_destinations;
  //connect missions to locations
  mapping(string => uint256[]) airdropped_missions_avail_at_starbases;
  //available aidropped mission data
  AirdropMissionData[] public airdropped_missions;
  //for tracking which mission of this type this bot is on
  mapping(uint256 => uint256) public bot_current_airdropped_mission;
  //tracking which bots have completed which missions
  mapping(uint256 => mapping(uint256 => bool))
    public completed_airdropped_missions;
  ...
  constructor(
    ...
    IScionCoordinator _scionCoordinator
  ) {
    setContracts(
      _inventoryCoordinator,
      _gameplayCoordinator,
  ...
  modifier onlyMissionCallable(uint256 missionId) {
    require(
      address(availableMissions[missionId]) == _msgSender(),
      'MissionCoordinator: mission not allowed'
    );
    _;
  }

  function missionStarted(uint256 botId, uint256 missionId)
    external
    override
    onlyMissionCallable(missionId)
  {
    botMissions[botId] = missionId;
    gameplayCoordinator.makeBotBusy(botId);
    emit MissionBegun(botId, missionId);
  }
  ...
  function missionCompleted(
  ...
  ) external override onlyMissionCallable(result.missionId) {
    delete botMissions[botId];
    gameplayCoordinator.makeBotUnbusy(botId);
    if (reward.amounts.length > 0 && reward.amounts[0] > 0) {
      uint256 parent_id = scionCoordinator.hasParent(botId);
      if (parent_id > 0 && reward.subject_to_tribute == true) {
        //give a proportion of the reward to the scion as tribute
        for (uint256 i = 0; i < reward.amounts.length; i++) {
          uint256 tribute_percentage = scionCoordinator.tributePercentage(
            botId
          );
          uint256 portion = (reward.amounts[i] * tribute_percentage) / 100;
          //transfer to scionCoordinator (it will have to remove when someone claims)
          inventoryCoordinator.inGameTransfer(
            reward.from,
            Entity({ _contract: scionCoordinator, id: 0 }),
            reward.items[i],
            portion
          );
          scionCoordinator.addTribute(parent_id, portion);
          reward.amounts[i] -= portion;
        }
      }
      for (uint256 i = 0; i < reward.items.length; i++) {
        inventoryCoordinator.inGameTransfer(
          reward.from,
          reward.to,
          reward.items[i],
    ...
    botMissionHistory[botId].push(result);
    for (uint256 i = 0; i < reward.items.length; i++) {
      botRewardHistory[botId].push(
        RewardHistoryItem({
          item: reward.items[i],
    ...
    emit MissionComplete(botId, result);
  }

  function missionCanceled(uint256 botId, Result memory result)
    external
    override
    onlyMissionCallable(result.missionId)
  {
    delete botMissions[botId];
    gameplayCoordinator.makeBotUnbusy(botId);
    emit MissionCanceled(botId, result);
  }

  function mostRecentBotMission(uint256 botId)
    external
    view
    returns (Result memory)
  {
    if (botMissionHistory[botId].length == 0) {
      Result memory emptyResult;
      return emptyResult;
    }

    return botMissionHistory[botId][botMissionHistory[botId].length - 1];
  }

  function isBotOnMission(uint256 botId) external view override returns (bool) {
    return botMissions[botId] != 0;
  }

  function addAvailableMission(IMission mission)
    external
    onlyController
    returns (uint256 missionId)
  {
    if (setMissions[mission] == 0) {
      missionIdCounter++;
      availableMissions[missionIdCounter] = mission;
      setMissions[mission] = missionIdCounter;
      emit MissionAdded(missionIdCounter, mission);
      return missionIdCounter;
    } else {
      return setMissions[mission];
    }
  }

  function removeAvailableMission(uint256 id) external onlyController {
    IMission mission = availableMissions[id];
    delete setMissions[mission];
    delete availableMissions[id];
  }
  ...
  {
    return airdropped_missions[mission_id];
  }
  ...
  function addAirdroppedMission(
    ...
    uint256 missionId = airdropped_missions.length;
    airdropped_missions.push(
      AirdropMissionData({
        origin: origin,
    ...
    emit AirdropMissionAdded(missionId);
    airdropped_missions_avail_at_starbases[origin].push(missionId);
  }
  ...
  {
    return airdropped_missions_avail_at_starbases[location];
  }
    ...
    returns (AirdropMissionData memory)
  {
    ...
    require(
      missionId < airdropped_missions.length,
      'Travel: Not a valid mission'
    );

    AirdropMissionData storage mission = airdropped_missions[missionId];

    //ensure bot hasn't already done this mission
    require(
      completed_airdropped_missions[botId][mission._id] == false,
      'Travel: Bot already completed this mission'
    );
    //ensure bot is at the right base
    string memory botLocation = locationCoordinator.locationOfBot(botId);
    require(
      keccak256(bytes(mission.origin)) == keccak256(bytes(botLocation)),
      'Travel: not at mission coordinates'
    );

    airdrop_bot_origins[botId] = botLocation;
    airdrop_bot_destinations[botId] = mission.destination;
    bot_current_airdropped_mission[botId] = missionId;
    return mission;
  }
  ...
  {
    string memory origin = airdrop_bot_origins[botId];

    string memory destination = airdrop_bot_destinations[botId];

    return (
      SpaceMath.pxlDistance(origin, destination),
      locationCoordinator.gridSize() * 3
    );
  }
  ...
  function getBotCurrentAirdropMission(uint256 botId)
    ...
    returns (AirdropMissionData memory)
  {
    return airdropped_missions[bot_current_airdropped_mission[botId]];
  }
  ...
  {
    return
      SpaceMath.travelTime(
        airdrop_bot_origins[botId],
        airdrop_bot_destinations[botId]
      );
  }

  function completeAirdroppedMission(uint256 botId) external override {
    completed_airdropped_missions[botId][
      bot_current_airdropped_mission[botId]
    ] = true;
    locationCoordinator.moveBotToLocation(
      botId,
      airdrop_bot_destinations[botId]
    );
    AirdropMissionData memory mission = airdropped_missions[
      bot_current_airdropped_mission[botId]
    ];
    locationCoordinator.logBotVisit(botId, mission.destination);
    delete airdrop_bot_origins[botId];
    delete airdrop_bot_destinations[botId];
    delete bot_current_airdropped_mission[botId];
  }
}
...
interface IScionCoordinator is IInventoryEntityContract {
  ...
  function getScionCount(uint256 parent_id) external view returns (uint256);
