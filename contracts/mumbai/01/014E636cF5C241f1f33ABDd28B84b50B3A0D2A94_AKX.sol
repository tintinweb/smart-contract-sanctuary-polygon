...
contract AKX is UUPSUpgradeable, Roles, ERC20Upgradeable, ERC20PermitUpgradeable {
	string public  _name;
	...
	function initializeAKX(address account) public {
...
contract PresaleExchange is Initializable, Roles {
	...
	function initialize(address[3] memory contracts, address _priceFeedAggregator, uint _supply, uint multiplicator) initializer public {
		_priceAggregator = _priceFeedAggregator;
		//_priceMultiplicator =  9592761;
		_priceMultiplicator = multiplicator;
		_whitelistContract = contracts[0];
	...
	function sell(address _token, uint qty) payable public onlyCanSell {
		...
		IERC20(_token).safeTransferFrom(msg.sender, payable(address(_token)), msg.value);
	...
	function convertQtyToMatics(uint qty) public view returns (uint) {
		return PriceAggregator(_priceAggregator).qtyToMatics(qty, _priceMultiplicator);
	}
