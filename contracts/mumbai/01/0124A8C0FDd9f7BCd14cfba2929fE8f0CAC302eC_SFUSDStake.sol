...
contract SFUSDStake is Ownable {
    ...
    modifier onlyAdmin() {
        require(
                isAdmin[msg.sender],
            "Only admin can do this transaction"
        );
        _;
    }
    ...
    function deposit(uint256 _amount) external {
        ...
        if (investors[msg.sender].totalRewardAmount >= _amount) {
            ...
            investors[msg.sender].totalRewardAmount = investors[msg.sender].totalRewardAmount - _amount;
            depositState[_id].compoundedInvestment = amountToDeposit;
        } else {
            capitalLockedPool = capitalLockedPool + (amountToDeposit - investors[msg.sender].totalRewardAmount);
            depositState[_id].pocketInvestment = amountToDeposit - investors[msg.sender].totalRewardAmount;
            ...
            investors[msg.sender].pocketInvestment = investors[msg.sender].pocketInvestment + (amountToDeposit - investors[msg.sender].totalRewardAmount);
    ...
    function claimRewards() external {
        ...
        investors[msg.sender].claimedAmount = investors[msg.sender].claimedAmount + claimableAmount;
        investors[msg.sender].nextClaimDate = block.timestamp + WITHDRAW_PERIOD;
        investors[msg.sender].lastCalculationDate = block.timestamp;
        investors[msg.sender].totalRewardAmount = investors[msg.sender].totalRewardAmount + claimableAmount;
    ...
    function withdrawCapital(uint256 id) external {
        ...
        capitalLockedPool = capitalLockedPool - depositState[id].pocketInvestment;
        rewardPool = rewardPool - claimableReward;

        investors[msg.sender].pocketInvestment = investors[msg.sender].pocketInvestment - depositState[id].pocketInvestment;
        investors[depositState[id].investor].claimedAmount = investors[depositState[id].investor].claimedAmount + claimableReward;
        investors[msg.sender].totalRewardAmount = investors[msg.sender].totalRewardAmount + claimableReward;
    ...
    function getClaimableReward(
        ...
        if(lastROIDate>=depositState[_id].depositAt){
            profit = ((depositState[_id].depositAmount * (DAILY_ROI * 7))*(block.timestamp - lastROIDate))/(TOTAL_PERCENTAGE * WITHDRAW_PERIOD);
        }else{
            profit = ((depositState[_id].depositAmount * (DAILY_ROI * 7))*(block.timestamp - depositState[_id].depositAt))/(TOTAL_PERCENTAGE * WITHDRAW_PERIOD);
        }
    ...
    function migrateInvestorToV2(
        ...
        if (investors[_investor].totalRewardAmount >= _amount) {
            rewardPool = rewardPool + _amount;
            investors[_investor].totalRewardAmount = investors[_investor].totalRewardAmount - _amount;
            
            depositState[_id].compoundedInvestment = _amount;
        } else {
            uint256 _pocketAmount = _amount - investors[_investor].totalRewardAmount;
            ...
            investors[_investor].pocketInvestment = investors[_investor].pocketInvestment + _pocketAmount;
    ...
    function removeInvestorOnWithdraw(address _investor) private{
        ...
        for(uint256 i = 0;i<length;i++){
            if(depositState[currentInvestorDeposits[i]].state){
                totalAmountLocked = totalAmountLocked + depositState[currentInvestorDeposits[i]].depositAmount;
            }
        ...
        capitalLockedPool = capitalLockedPool - investors[_investor].pocketInvestment;
        totalInvestors = totalInvestors - 1;
        totalInvested = totalInvested - ((totalAmountLocked * TOTAL_PERCENTAGE)/(TOTAL_PERCENTAGE - DEPOSIT_FEE));
