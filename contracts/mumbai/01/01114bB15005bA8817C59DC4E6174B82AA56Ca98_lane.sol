...
contract lane is ERC721A, OperatorFilterer, Ownable, ReentrancyGuard {
    ...
    uint256 public maxFreeMintLimit = 1;
    uint256 public maxPaidMintLimit = 3;

    mapping(address => bool) freePublicMint;
    mapping(address => bool) freeWhitelistMint;

    bytes32 public merkleRoot;
    string public baseURI;

    constructor(string memory _initBaseURI) ERC721A("lane", "lne") {
    ...
    modifier mintCompliance(uint256 _mintAmount) {
        require(balanceOf(msg.sender) < 100, "You have already minted 3!");
        require(_mintAmount <= 100, "Max mint per transaction is 3!");
        require(totalSupply() + _mintAmount <= maxSupply, "Max Supply Exceeded!");
        _;
    }

    function teamMint() external onlyOwner {
        require(!teamMintClaimed, "Team mint already claimed!");
    ...
    function publicMint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) nonReentrant {
        ...
        if(balanceOf(_msgSender()) > 1) {
            ...
            require(msg.value >= (_mintAmount * publicMintCost), "Insufficient Funds!");
            freePublicMint[_msgSender()] = true;
        }
        _safeMint(_msgSender(), _mintAmount);
    }
