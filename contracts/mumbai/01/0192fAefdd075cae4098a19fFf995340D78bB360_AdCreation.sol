...
contract AdCreation is NFT721Metadata{
    ...
    event AdCreated(address, string, string[6]);

    function init(address registrationRole_, string memory name_, string memory symbol_) external initializer {
        ...
        external
    {
        ...
        AdInfo storage adInfo = adInfos[uniqueID];
        ...
        adInfo.tokenId = _mintInternal(_tokenIPFSPath);
        userAds[msg.sender].push(uniqueID);
        uniqueID++;

        emit AdCreated(msg.sender, _tokenIPFSPath, adDetails);
    }
}