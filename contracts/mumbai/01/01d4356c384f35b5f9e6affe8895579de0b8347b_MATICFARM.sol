...
contract MATICFARM {
	...
	uint256 constant public MAX_HOLD_PERCENT = 25;
    uint256 constant public FORCE_WITHDRAW_PENALTY = 25; // 25% Fee For WITHDRAWING EARLY IN PLAN 3 & 4
	...
	constructor(address payable wallet, address payable _developer) public {
        ...
        plans.push(Plan(7, 16)); // 16% per day for 7 days              
        plans.push(Plan(14, 90)); // 9% per day for 14 days                     
		plans.push(Plan(7, 18)); // 18% per day for 7 days (at the end)
        plans.push(Plan(14, 10)); // 10% per day for 14 days (at the end)
	}
    ...
    function forceWithdraw(uint256 index) public {
        ...
        uint256 totalAmount  = amount.sub(amount.mul(FORCE_WITHDRAW_PENALTY).div(PERCENTS_DIVIDER)); //75% 
