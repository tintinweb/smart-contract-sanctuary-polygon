    ...
    PFPDAORoleVariantManager
{
    ...
    mapping(address => bool) public isFreeLooted;
    ...
    function freeLoot() external {
        if (isFreeLooted[_msgSender()]) {
            revert FreeLooted(_msgSender());
        }
        _loot1();
        isFreeLooted[_msgSender()] = true;
    }

    mapping(bytes32 => bool) public nonceUsed;

    function whitelistLoot(uint8 time_, bytes calldata _signature) external {
        bytes32 messageHash = keccak256(abi.encodePacked(_msgSender(), time_, activeNonce));
        ...
        if (!signer.isValidSignatureNow(signedMessage, _signature)) {
