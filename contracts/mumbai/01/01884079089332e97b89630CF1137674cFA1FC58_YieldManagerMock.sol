...
contract YieldManagerMock is IYieldManager {
  ...
  function depositPaymentToken(uint256 amount) external override marketOnly {
  ...
  /// @param user User to recieve the payout
  /// @param amount Amount of payment token to pay to user
  function transferPaymentTokensToUser(address user, uint256 amount) external override marketOnly {
    // Transfer tokens back to LongShort contract.
    token.transfer(user, amount);
  }

  function removePaymentTokenFromMarket(uint256 amount) external override marketOnly {
    // Ensure token state is current.
    settle();
  ...
  function distributeYieldForTreasuryAndReturnMarketAllocation(
    uint256 totalValueRealizedForMarket,
    uint256 treasuryYieldPercent_e18
  ) external override marketOnly returns (uint256) {
    // Ensure token state is current.
    settle();
  ...
  function withdrawTreasuryFunds() external override marketOnly {}

  function initializeForMarket() external override marketOnly {
