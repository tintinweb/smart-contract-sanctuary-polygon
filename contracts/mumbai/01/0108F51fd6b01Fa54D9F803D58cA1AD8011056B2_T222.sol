...
//        IERC20(daoToken).transferFrom(_user, address(this), _leftFee);
        ...
        */
        if (tatalInvestNumber == 0) {
            accshareList[tatalInvestNumber] = 0;
        } else {
            accshareList[tatalInvestNumber] += (_amount * 25 / 100) * (10 ** 18) / tatalInvestAmount;
        }
    ...
    function getStaticReward(address _user) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < _num; i++) {
            ...
            _totalReward += _investAmount * (accshareList[tatalInvestNumber] - accshareList[_investId]) / (10 ** 18);
        }
        return _totalReward;
    }
