...
contract BlockchainAds is Ownable {
    ...
    modifier isAccountOwner(uint256 userId){
        require(users[userId].userAddress == msg.sender, "Address is not account owner");
        _;
    }
    ...
    function claimRewards(uint256 userId) external isAccountOwner(userId){
    ...
    function initDataCollection(uint256 userId) public dataCollectionInitiated isAccountOwner(userId){
    ...
    function deleteData(uint256 userId) public isAccountOwner(userId) {
        collectionStarted[userId] = 0;
