...
contract DoodleVault is Ownable, Pausable, ReentrancyGuard, IERC721Receiver {
    ...
    function calculateRewards(address account, uint256[] calldata _tokens) public view returns (uint256[] memory rewards) {
        rewards = new uint256[](_tokens.length);
        for(uint256 token; token < _tokens.length; token++) {
             rewards[token] = calculateReward(account, _tokens[token]);
        }
        return rewards;
    }
