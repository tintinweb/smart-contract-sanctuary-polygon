...
interface IGameQuery {
    function game(uint64 mainType, uint64 subType) external returns (address);
}
...
contract GamePool is Pool {
    ...
    // TODO 权限、事件
    function setOpenJackPool(bool open) external {
        openJackPool = open;
    }
    ...
    function distributeFT(
        ...
        ft; mainType; subType; player; // to avoid warnings of compiler
    ...
    event GameResult(uint indexed requestId, address player, uint256 gameCount, uint256 totalWin, address game, bytes result);
    ...
    function settlement(
        ...
        require(msg.sender == IGameQuery(platform).game(mainType, subType), "Not granted");
        ...
        // jackPool是独立事件，没有资金，就不会中奖，但gamePool没有资金的话，中奖金额来自adjustPool
        (winCount, gamePoolMinus, jackPoolMinus) =
            ARITHMETIC.getResult(random, gameCount, gamePool + adjustPool, jackPool);
    ...
    // TODO 权限控制，platform 的管理员才可以取，事件
    function withdrawFromAdjustPool(address receiver, uint256 amount) external {
        require(adjustPool >= amount, "Not enough");
        require(receiver != address(0), "Not enough");
...
// https://mumbai.polygonscan.com/address/0x4d776c3af18eae4b4421a45211a113ca5b672cf7#code