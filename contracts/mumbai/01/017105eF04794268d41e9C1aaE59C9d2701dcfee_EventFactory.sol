 ...
 * @author GET Protocol
 * @notice Contract responsible for deploying EventImplementation contracts
...
contract EventFactory is IEventFactory, OwnableUpgradeable, AuthModifiers, UUPSUpgradeable {
    ...
    function __EventFactory_init(address _registry, address _implementation) public initializer {
        __Ownable_init();
    ...
    /**
     * @notice Deploys an EventImplementation contract
    ...
    function createEvent(
        ...
        bytes memory _eventCalldata = abi.encodeWithSignature(
            "__EventImplementation_init(string,string,address)",
            _name,
            _symbol,
            address(registry)
        );
