...
abstract contract APPcore is  Ownable{
    ...
    bytes32 internal unLockPass;

    // event ChangeLock(uint256 tokenId, bool locked);
    event StartStake(uint256 tokenId, address holder,uint256 startTime);
    event EndStake(uint256 tokenId, address holder,uint256 startTime,uint256 endTime);
}
...
abstract contract APPadmin is APPcore,AccessControl,ERC721AQueryable,ERC2981{
    ...
    function setMinterContract(IMinter _minter) external onlyAdmin{
        minter = _minter;
    }
    ...
    function setUnLockPass(bytes32 _value) external onlyAdmin{
        unLockPass = _value;
    }
    ...
    function _setStartStake(uint256 _tokenId) internal{
        ...
        emit StartStake(_tokenId,msg.sender,block.timestamp);
    }
    
    function _setEndStake(uint256 _tokenId,string memory _pass) internal {
        require(stakeInfo[_tokenId].get128(0) == 1,"Not staked");
        require(keccak256(abi.encodePacked(_pass)) == unLockPass,"unlock password is different");

        if(address(minter) != address(0)){
            // Supports token distribution by staking
            minter.claimToken(_tokenId);
        }
    ...
    function setStartStake_adimin(uint256 _tokenId) external onlyAdmin{
        _setStartStake(_tokenId);
    }

    function setEndStake_admin(uint256 _tokenId,string memory _pass) external onlyAdmin {
        _setEndStake(_tokenId,_pass);
    }

    function setStakeArray_admin(uint256[] calldata _tokenId, bool[] calldata _staked,string memory _pass) external onlyAdmin{
        require(_tokenId.length == _staked.length,"list length is different");
        for(uint256 i = 0; i < _tokenId.length;i++){
            if(_staked[i] == true){
                _setStartStake(_tokenId[i]);
            }else{
                _setEndStake(_tokenId[i],_pass);
            }
        } 
    }
}
...
contract APP is APPadmin {
    ...
    constructor() ERC721A('Test Panda 1017', 'TST') {
    ...
    function setEndStake(uint256 _tokenId,string memory _pass) external {
        require(msg.sender == ownerOf(_tokenId),"Not the holder of tokenId");
        _setEndStake(_tokenId,_pass);
    }

    function setStakeArray(uint256[] calldata _tokenId, bool[] calldata _staked,string memory _pass)external {
        require(_tokenId.length == _staked.length,"list length is different");
        for(uint256 i = 0; i < _tokenId.length;i++){
    ...
    function getStakeState(address owner)external view returns (bool[] memory){
        unchecked {
            uint256 tokenIdsIdx;
    ...
    function _getStakeStartTimestamp(uint256 _tokenId) internal view returns(uint256 stakedTimestamp){
        require(stakeInfo[_tokenId].get128(0) == 1,"Not staked");
    ...
    function getStakeStartTimestamp(uint256 _tokenId) external view returns(uint256 stakedTimestamp){
        return _getStakeStartTimestamp(_tokenId);
    }
    ...
    function getStakeStartTimestamp_array(uint256[] calldata _tokenId) external view returns(uint256[] memory stakedTimestamp){
        ...
        for(uint256 i = 0; i < _tokenId.length;i++){
            tokenIdTimestamp[i] = _getStakeStartTimestamp(_tokenId[i]);
        }

        return tokenIdTimestamp;
    } 

    function _getStakeTimestampTerm(uint256 _tokenId) internal view returns(uint256 stakedTimestamp){
        require(stakeInfo[_tokenId].get128(0) == 1,"Not staked");
    ...
    function getStakeTimestampTerm(uint256 _tokenId) external view returns(uint256 stakedTimestamp){
        return _getStakeTimestampTerm(_tokenId);
    }

    function getStakeTimestampTerm_array(uint256[] calldata _tokenId) external view returns(uint256[] memory  stakedTimestamp){
        uint256[] memory tokenIdTimestamp = new uint256[](_tokenId.length);
        for(uint256 i = 0; i < _tokenId.length;i++){
            tokenIdTimestamp[i] = _getStakeTimestampTerm(_tokenId[i]);
        }
