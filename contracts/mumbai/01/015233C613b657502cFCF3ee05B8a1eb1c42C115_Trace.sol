...
contract Trace is AccessControl {
    ...
    mapping(string => Supplier) public supplier;
    ...
    constructor(address _supplier, address _rph, address _distributor, address _rumahMakan) {
    ...
    function inputSupplier(
        ...
        string memory ID = string.concat(
            "SUPLY",
            Strings.toString(index_supplier)
        );
    ...
    function inputPemotongan(
        string memory _ID_Supplier,
        string memory _tanggal_pemotongan,
        bool _status_kehalalan
    ) public onlyRole(RPH) {
        // Pengecekkan ketersediaan data supplier dan Pengecekkan kehalalan status produk
        require(
            keccak256(
            abi.encodePacked((
                supplier[_ID_Supplier].ID))) != keccak256(abi.encodePacked((""))), 
                "Data Supplier Tidak Terdaftar"
        );
        require(_status_kehalalan == true, "Status Produk harus halal");
    ...
    function inputMakanan(
        ...
        bool _status_kehalalan
    ) public onlyRole(RUMAH_MAKAN) returns(string memory) {        
    ...
    function getRelatedData(string memory _ID_Makanan) public view returns (
        Makanan memory,
    ...
    )
    {
        ...
        Supplier memory data5 = supplier[data4.ID_Supplier];
