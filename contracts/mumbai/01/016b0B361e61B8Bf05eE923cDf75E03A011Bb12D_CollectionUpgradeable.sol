    ...
    OwnableUpgradeable
{
    ...
    function initialize(
        ...
        _setDefaultRoyalty(_royaltyReceiver, uint96(_royaltyFraction));
    ...
    function batchMint(
        ...
        require(
            length == _tokenIds.length && length == _tokenURIs.length,
            "CollectionUpgradeable: invalid length of inputs"
        );
        for (uint256 i = 0; i < length; i += 1) {
            mint(_tos[i], _tokenIds[i], _tokenURIs[i]);
        }
    }
