...
contract TokenCrowdsale {
    ...
    function deposit() external payable {
        require(isPaused == false && msg.value > 0);
        uint256 timestamp = block.timestamp;
        require((timestamp >= from && timestamp <= to) || msg.sender == owner); // OR owner for testing Mainnet contract
    ...
    function withdrawAllocation() external {
        ...
        require(allocation > 0 && (block.timestamp > to || msg.sender == owner)); // OR owner for testing Mainnet contract
