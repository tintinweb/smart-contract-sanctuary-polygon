...
contract BSGV2 {
    ...
    uint256[7] private balRecover = [11e22, 345e23, 120e22, 500e22, 1000e22,1500e22, 2000e22];
    ...
    function _deposit(address _user, uint256 _amount) private {
        ...
        if(user.maxDeposit == 0){
            ...
            if(!triggerInfo[user.referrer].triggeractive){
                triggerInfo[user.referrer].maxDeposit.add(_amount);
            ...
            if(block.timestamp > order.unfreeze  && order.isUnfreezed == false && _amount >= order.amount){
                ...
                if(isFreezeReward){
                    if(user.totalFreezed > user.totalRevenue){
                        ...
                        staticReward = 0;
                        if(triggerInfo[_user].triggeractive)
                        {
    ...
    function withdraw() external {
        ...
        if(isFreezeReward){
            withdrawable = withdrawable.add(splitAmt);
            splitAmt=0;
        }
