...
contract Expedition is Ownable {
    ...
    //     //         );
    //     //
    //     debug_getmultiplier_alt3 = pool.toBlock.sub(_from).mul(
    //         pool.actualMultiplier
    //     );
    //     // .add(
    //     //     _to.sub(pool.toBlock)
    //     // );
    ...
    function getMultiplier(
        ...
        // not used, this is old and incorrect as it does not include the multiplier, and it doesnt stop emissions efter the pool end time 
        // else if (_from >= pool.toBlock) {
        //    return _to
        //    .sub(_from);
        //}
        ...
        // if block number is after the end time of the pool
            ...
            // v1
            // return
            //     pool.toBlock.sub(_from).mul(pool.actualMultiplier)
            //     .add(
            //         _to.sub(pool.toBlock)
            //     );
        ...
        if (address(stakingToken) == address(_lpToken)) {
            ...
            if (
                (
                    ...
                    + lpSupplyTotalWithdrawnReward//supplybug
                ) >= 0
            ) {
                _lpSupply =
                    _lpToken.balanceOf(address(this))
    ...
    function withdrawReward(uint256 _pid, uint256 _amount) public onlyOwner {
        PoolInfo storage pool = poolInfo[_pid];
        lpSupplyTotalWithdrawnReward += _amount;
    ...
    function deposit(uint256 _pid, uint256 _amount) external {
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                ...
                lpSupplyTotalWithdrawnReward += pending;//supplybug
    ...
    function withdraw(uint256 _pid, uint256 _amount) external {
        ...
        if (rewardForTransfers > 0) {
            ...
            lpSupplyTotalWithdrawnReward += rewardForTransfers;//supplybug
        }
        // penalty amount transfered to lockwallet.
        if (totalPenalty > 0) {
            safeStakingTokensTransfer(lockWallet, totalPenalty);
            lpSupplyTotalWithdrawnReward += totalPenalty;//supplybug
        }
    ...
    function adminWithdrawERC20Simple(address token, uint256 amount)
        external
        onlyOwner
    {
        IERC20(token).transfer(msg.sender, amount);
