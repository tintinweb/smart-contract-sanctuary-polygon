...
contract MatchHandler is IMatchHandler, SecuritizeConstants, UUPSUpgradeable, PausableUpgradeable, AccessControlUpgradeable {

    address public custodialWallet;
    bytes32 public constant OWNER_ROLE = keccak256("OWNER_ROLE");

    function initialize(address _masterAddress, address _owner, address _custodialWallet) public onlyProxy initializer override {
    ...
    function addOwner(address owner) external override onlyRole(DEFAULT_ADMIN_ROLE) {
        grantRole(OWNER_ROLE, owner);
    }

    function removeOwner(address owner) external override onlyRole(DEFAULT_ADMIN_ROLE) {
        revokeRole(OWNER_ROLE, owner);
    }

    function setCustodialWallet(address newCustodialWallet) external override onlyRole(DEFAULT_ADMIN_ROLE) {
        require(newCustodialWallet != address(0), "custodial wallet cannot be zero address");
    ...
    function matchOrder(
        ...
        address stableCoin,
        address seller,
        uint256 dsTokenAmount,
        uint256 stableCoinAmount,
        uint256 sellerFee,
        uint256 buyerFee,
        DataTypes.Investor memory buyer
    ) external override onlyRole(OWNER_ROLE) {
        // Register buyer
        _registerInvestor(IDSToken(dsToken), buyer);

        // Swap between seller & buyer
        IDSToken(dsToken).transferFrom(seller, buyer.wallet, dsTokenAmount);
        IERC20Upgradeable(stableCoin).transferFrom(buyer.wallet, seller, stableCoinAmount);

        // Securitize fees
        IERC20Upgradeable(stableCoin).transferFrom(seller, custodialWallet, sellerFee);
        IERC20Upgradeable(stableCoin).transferFrom(buyer.wallet, custodialWallet, buyerFee);

        emit Match(seller, buyer.wallet, dsToken, stableCoin, dsTokenAmount, stableCoinAmount, sellerFee, buyerFee, custodialWallet);
    }
    ...
    function _registerInvestor(IDSToken dsToken, DataTypes.Investor memory investor) private {
        IDSRegistryService registryService = IDSRegistryService(dsToken.getDSService(DS_REGISTRY_SERVICE));

        //Investor does not exist
        if (!registryService.isInvestor(investor.blockchainId)) {
            require(investor.investorAttributeIds.length == investor.investorAttributeValues.length &&
                investor.investorAttributeValues.length == investor.investorAttributeExpirations.length,
                "investor params incorrect length"
            );

            registryService.registerInvestor(investor.blockchainId, "");
            registryService.setCountry(investor.blockchainId, investor.country);
            for (uint256 i = 0; i < investor.investorAttributeIds.length; i++) {
                registryService.setAttribute(
                    investor.blockchainId,
                    investor.investorAttributeIds[i],
                    investor.investorAttributeValues[i],
                    investor.investorAttributeExpirations[i],
                    ""
                );
            }
        }
        ...
        string memory investorWithNewWallet = registryService.getInvestor(investor.wallet);
        if (CommonUtils.isEmptyString(investorWithNewWallet)) {
            registryService.addWallet(investor.wallet, investor.blockchainId);
        }
        else {
            require(CommonUtils.isEqualString(investor.blockchainId, investorWithNewWallet), "wallet does not belong to investor");
        }
    }
}
...
interface IMatchHandler {
    ...
    /**
     * @dev Function to be invoked by the proxy contract when the MatchHandler is deployed.
     * @param _masterAddress The address with administrative powers
     * @param _owner the address that can invoke order matches
     * @param _custodialWallet The address that receive order fees
    **/
    function initialize(address _masterAddress, address _owner, address _custodialWallet) external;
    ...
    **/
    ...
    /**
     * @dev Add a owner to this contract
     * @param owner The owner address to be added
     **/
    function addOwner(address owner) external;

    /**
     * @dev Remove a owner from this contract
     * @param owner The owner address to be removed
     **/
    function removeOwner(address owner) external;

    /**
     * @dev Set the custodial address
     * @param newCustodialWallet The address that receive order fees
     **/
    function setCustodialWallet(address newCustodialWallet) external;

    /**
     * @dev Match between buyer and seller orders
    **/
    function matchOrder(
        address dsToken,
        address stableCoin,
...
library DataTypes {
    ...
    struct Investor {
        ...
        address wallet;
        string country;
        uint8[] investorAttributeIds;
        uint256[] investorAttributeValues;
        uint256[] investorAttributeExpirations;
    }
}
