...
contract CrossingMysteryBoxStorefront is ICrossingNFT, AccessControl, ReentrancyGuard {
    ...
    event MysteryBoxCollectionPriceUpdated (
      uint256 indexed mysteryBoxId,
      uint256 price
    );
    ...
    modifier validMysteryBoxId (uint256 mysteryBoxId){
      require(mysteryBoxCollections[mysteryBoxId].size > 0, "Invalid mystery box ID");
      _;
    }
    ...
    function addMysteryBoxCollection(string memory mysteryBoxURI, bytes32[] memory proofOfIntegrityList, uint256 price)
      public
      onlyRole(CURATOR_ROLE)
      returns (uint256)
    {
      ...
      mysteryBoxCollections[mysteryBoxId] = MysteryBoxCollection(mysteryBoxURI, proofOfIntegrityList, proofOfIntegrityList.length, price);

      emit MysteryBoxCollectionAddedToStorefront(
        mysteryBoxId,
        proofOfIntegrityList.length,
        price
      );
      return mysteryBoxId;
    }
    ...
    function removeMysteryBoxCollection(uint256 mysteryBoxId)
      public
      onlyRole(CURATOR_ROLE)
      validMysteryBoxId(mysteryBoxId)
      ...
      validMysteryBoxId(mysteryBoxId)
    {
      ...
      emit MysteryBoxCollectionPriceUpdated(
        mysteryBoxId,
        newPrice
      );
    }
