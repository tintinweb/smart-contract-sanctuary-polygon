...
import "./interfaces/utils/IBetHelper.sol";
...
contract BetHistory is IBetHistory, AccessHandler {
    ...
    IRewardHandler private feeRewards;
    ...
    // bet hash => Bet (struct)
    mapping(bytes32 => BetData.Bet) public allBets; // TODO: Make it public in case TC needs it?
    // bet hash => pot size
    mapping(bytes32 => uint256) public unsettledPots; // TODO: Make it public in case TC needs it?
    // market hash => token => amount betted
     ...
     * @param _lp The Liquidity Pool contract, to match bets.
     * @param _betLockBox The bet LockBox contract address.
     * @param _feeRewards The fee handler contract address.
        ...
        returns (uint256 pot, bytes32 betHash)
    {
        ...
        // market hash => token => amount betted
        marketBetted[bet.marketHash][bet.token] = marketBetted[bet.marketHash][bet.token].add(bet.amount);
     ...
     * @return tokenAdd is the address of the token type to settle.
     * @return paidToBetter is the amount won from the bet
     * @return paidToLP is the amount returned to the LP.
     * @return paidToFee is the amount paid as win fees.
     */
        ...
        )
    {
        ...
        uint256 lockedAmount = betLockBox.getLockedAmount(better, tokenAdd);
        console.log("locked:", lockedAmount.div(1 ether));
        if (lockedAmount == 0 || potSize == 0) {
            return (better, tokenAdd, 0, 0, 0);
        }
        ...
        if (canceled || isVoid) {
            ...
            return (better, tokenAdd, paidToBetter, paidToLP, 0);
        }
        // Win
        ...
        if (win) {
            ...
            paidToFee = potSize.mul(BetData.WIN_FEE_PERMILLE).div(1000);
            paidToBetter = potSize.sub(paidToFee);
            // Add the fee to the common reward pools
            feeRewards.addRewards(better, paidToFee);
            return (better, tokenAdd, paidToBetter, 0, paidToFee);
        }
        // HalfWin
        bool halfWin = marketHistory.isMarketOutcome(
            bet.marketHash,
        ...
        if (halfWin) {
            ...
            paidToLP = matchedAmount.div(2);
            paidToFee = potSize.mul(BetData.WIN_FEE_PERMILLE)
                .div(1000).div(2);
            paidToBetter = potSize.sub(paidToFee).sub(paidToLP);
            // Add the fee to the common reward pools
            feeRewards.addRewards(better, paidToFee);
            return (better, tokenAdd, paidToBetter, paidToLP, paidToFee);
        }
        // HalfLoss
        bool halfLoss = marketHistory.isMarketOutcome(
            bet.marketHash,
            IMarketHistory.MarketOutcome.HalfLoss
        );
        if (halfLoss) {
            // Return half the bet amount to the better.
            // Return half the pot and half the matched amount to the LP.
            paidToBetter = bet.amount.div(2);
            paidToLP = potSize.sub(paidToBetter);
            return (better, tokenAdd, paidToBetter, paidToLP, 0);
        }
        // Loss
        bool loss = marketHistory.isMarketOutcome(
            bet.marketHash,
        ...
        if (loss) {
            ...
            paidToLP = potSize;
            return (better, tokenAdd, 0, paidToLP, 0);
        }
        ...
        )
    {
        ...
        uint256 potSize = unsettledPots[betHash];
        uint256 matchedAmount = potSize.sub(bet.amount);
        marketBetted[bet.marketHash][tokenAdd] = marketBetted[bet.marketHash][tokenAdd].sub(bet.amount);
        marketMatched[bet.marketHash][tokenAdd] = marketMatched[bet.marketHash][tokenAdd].sub(matchedAmount);
        unsettledPots[betHash] = 0;

        uint256 lockedAmount = betLockBox.getLockedAmount(better, tokenAdd);
        if (lockedAmount == 0 || potSize == 0) {
            return (better, tokenAdd, 0, 0);
        }
     ...
     */
    function matchBet(bytes32 marketHash, address tokenAdd, uint256 matchedAmount) private {
        lp.matchBet(tokenAdd, matchedAmount);
        betLockBox.lockAmount(address(lp), tokenAdd, matchedAmount);
        // market hash => token => amount matched
        marketMatched[marketHash][tokenAdd] = marketMatched[marketHash][tokenAdd].add(matchedAmount);
    }
}
...
library BetData {
    ...
    uint256 public constant WIN_FEE_PERMILLE = 50;
    ...
    function assertValidParams(Bet calldata bet) internal view {
        ...
        require(bet.token != address(0), "INVALID_TOKEN");
        require(bet.expiry > block.timestamp, "BET_EXPIRED");
    }
