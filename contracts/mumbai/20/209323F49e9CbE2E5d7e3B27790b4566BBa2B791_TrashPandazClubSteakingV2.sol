...
// File: TrashPandazClubSteakingV2.sol
...
contract TrashPandazClubSteakingV2 is IERC721Receiver, ReentrancyGuard, Ownable, Pausable {
    ...
    bool public enableMultipliers = true;
    ...
    function rewardMath(uint256 stakedAt) internal view returns(uint256){
        if (rewardDividedByPool == true){
            return (( rewardRate * 1 ether * (block.timestamp - stakedAt) / 86400 ) / 100 ) / totalStaked; //100rewardRate... / 100 = 1 token per day
        }
        else {
            return ( rewardRate * 1 ether * (block.timestamp - stakedAt) / 86400 ) / 100 ; //100rewardRate... / 100 = 1 token per day
        }
    ...
    function _claim(address account, uint256[] calldata tokenIds, bool _unstake) internal nonReentrant{
        ...
        earned = rewardMath(stakedAt); 
        earned *= getMultiplier(account); 
        vault[tokenId] = Stake({
            owner: account,
            tokenId: uint24(tokenId),
            timestamp: uint48(block.timestamp)
