...
contract EnglishAuction is AccessControl {
    event AddAuction(uint256 auctionId, address collection, uint256 tokenId, uint256 duration);
    ...
    mapping(uint256 => address[]) private _biddersByAuction;
    mapping(uint256 => uint256[]) private _bidsByAuction;
    mapping(uint256 => uint256[]) private _bidTimestampsByAuction;
    ...
    function setupAuction(
        ...
        emit AddAuction(_auctionId, _nftAddress, _tokenId, _duration);
    }
    ...
    function bid(uint256 _auctionId) external payable {
        ...
        _bidsByAuction[_auctionId].push(msg.value);
        _biddersByAuction[_auctionId].push(_msgSender());
        _bidTimestampsByAuction[_auctionId].push(block.timestamp);
    ...
    {
        address[] memory bidders = _biddersByAuction[_auctionId];
        uint256[] memory bids = _bidsByAuction[_auctionId];
        uint256[] memory timestamps = _bidTimestampsByAuction[_auctionId];
        return (bidders, bids, timestamps);
    }
