...
contract TheFlags is ERC721, Ownable {
    ...
    mapping(FormType => uint256[]) formsColumnOffsets;
    ...
    constructor() ERC721("TheFlags", "FLG") public {
        ...
        formsColumnOffsets[FormType.Default] = [ 0, 1, 2, 2, 2, 2, 1, 0, 0, 0, 0, 0, 1 ];
        formsColumnOffsets[FormType.Square] = [ 0, 1, 2, 2, 2, 1, 0, 0, 0, 1 ];
        formsColumnOffsets[FormType.Nepal] = [ 0, 1, 2, 2, 2, 2, 2, 2, 2, 2 ];
    }

    function safeMint(address to, uint256 tokenId, string memory name, bytes memory flag)
        public
        onlyOwner
    {
        flags[tokenId] = flag;
    ...
    function tokenURI(uint256 flagId) public view override(ERC721) returns (string memory) {
        require(flagId < 195, "Flag does't exist");
    ...
    function decompressImg(bytes memory data) private view returns (string memory svg) {
        ...
        {
            uint8 colorIndex = uint8(data[colorDataIndex] >> 4);
            color = data[1 + uint8(colorIndex) * 3] << 4 | data[1 + uint8(colorIndex) * 3 + 1] << 2 | data[1 + uint8(colorIndex) * 3 + 2];

            for (uint8 repeat = 0; repeat < uint8(data[colorDataIndex]) & 0xF; repeat++)
            {
    ...
    function getFormPixelByIterator(
        ...
        column = formsParameters[params.formType].startColumn + columnIndex + formsColumnOffsets[params.formType][columnIndex];
        row = formsParameters[params.formType].startRow + rowIndex;
    }
