...
library UserLib {
    ...
    function checkPayment(
        ...
        DataTypes.SinglePostPaymentInfo memory info = ISubscription(registry.subscription()).getSinglePostPaymentInfo(_subscriptionId);
    ...
    function hasAnyPayment(
        ...
        for(uint256 i = count; i > 0; i--) {
            if(hasPayment(_registry, _user, _paymentType, _postId, tokenIds[i-1])) {
                return true;
            }
        }
        return false;
    ...
    function hasPayment(
        ...
        DataTypes.SinglePostPaymentInfo memory info = ISubscription(_registry.subscription()).getSinglePostPaymentInfo(_subscriptionId);
    ...
    function isCorrectTime(DataTypes.SinglePostPaymentInfo memory _info) private view returns(bool) {
        uint256 currentTime = block.timestamp;
        return _info.startTime <= currentTime && currentTime <= _info.endTime;
    }

    function isCorrectPost(DataTypes.SinglePostPaymentInfo memory _info, uint256 _postId) private view returns(bool) {
        return _info.postId == _postId;
    }
}
...
contract HasPaymentForAccess is BasePlugin {

    using UserLib for IRegistry;

    constructor(address _registry) {
        PLUGIN_VERSION = 1;
        PLUGIN_NAME = PluginsList.SINGLE_HAS_PAYMENT_FOR_ACCESS;
        registry = IRegistry(_registry);
    }

    function read(
        address _user, uint256 _postId
        ...
        DataTypes.SinglePostInfo memory postInfo = ISinglePostData(registry.singlePostData()).readPost(vars);
        uint256 paymentType = postInfo.paymentType;

        return registry.hasAnyPayment(_user, paymentType, _postId);
    }
}
...
interface ISubscription is IERC721EnumerableUpgradeable {
    ...
    function getSinglePostPaymentInfo(uint256 _nftId) external view returns(DataTypes.SinglePostPaymentInfo memory);
}