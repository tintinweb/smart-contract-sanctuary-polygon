...
contract Seller is Ownable {
  ...
  Tarefa07Token tarefa07Token;  // Our Token Contract
  ...
  constructor(address tokenAddress) {
    tarefa07Token = Tarefa07Token(tokenAddress);
  }
  ...
  function buyTokens() public payable returns (uint256 tokenAmount) {
    ...
    uint256 SellerBalance = tarefa07Token.balanceOf(address(this)); // check if the Vendor Contract has enough amount of tokens for the transaction
    require(SellerBalance >= amountToBuy, "Vendor contract has not enough tokens in its balance");
    (bool sent) = tarefa07Token.transfer(msg.sender, amountToBuy); // Transfer token to the Msg.Sender
