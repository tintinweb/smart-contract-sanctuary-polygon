...
/**
 * @title DummyProjectSale contract
 * @author OpenPad
 */
contract DummyProjectSale is IProjectSale, ReentrancyGuard, Pausable, Ownable {
    ...
    uint256 private _projectTokenPrice = 1e18;
    uint256 private _totalSaleValue;
    uint256 private _totalSaleValueCap;
    uint256 private _generation;
    ProjectState private _state = ProjectState.NotStarted;
    EnumerableMap.AddressToUintMap private _registeredParticipants;
    mapping(address => Participant) private _participants;

    constructor(address _peggedToken, address _creditToken, address _projectToken) {
        require(_peggedToken != address(0), "ProjectSale: peggedToken is the zero address");
        require(_creditToken != address(0), "ProjectSale: creditToken is the zero address");
        require(_projectToken != address(0), "ProjectSale: projectToken is the zero address");
    ...
    function initializeGeneration(
        ...
        uint256[] memory creditAllocations
    ) public onlyOwner {
        require(
            addresses.length == allocations.length,
            "ProjectSale: addresses and allocations length mismatch"
        );
        require(
            addresses.length == creditAllocations.length,
            "ProjectSale: addresses and creditAllocations length mismatch"
        );
        _generation += 1;
        _state = ProjectState.NotStarted;
        uint256 totalAllocation = 0;
        for (uint256 i = 0; i < addresses.length; i++) {
            totalAllocation += allocations[i] + creditAllocations[i];
            _participants[addresses[i]] = Participant({
        ...
        _totalSaleValueCap = totalAllocation;
    }

    function startRegistration() public onlyOwner {
        require(_state == ProjectState.NotStarted, "ProjectSale: already started");
        _state = ProjectState.Registration;
    }

    function startStaking() public onlyOwner {
        require(_state == ProjectState.Registration, "ProjectSale: not in registration state");
        _state = ProjectState.Staking;
    }

    function startPublic() public onlyOwner {
        require(_state == ProjectState.Staking, "ProjectSale: not in staking state");
        _state = ProjectState.Public;
    }

    function finalizeSale() public onlyOwner returns (address) {
        ...
        projectToken.safeTransferFrom(msg.sender, vesting.getReleaser(), tokensSold());
        peggedToken.safeTransfer(msg.sender, _totalSaleValue);
        _state = ProjectState.Vesting;

        uint256 length = _registeredParticipants.length();
        for (uint256 i = 0; i < length; i++) {
            (address participant, uint256 generation) = _registeredParticipants.at(i);
            if (generation != _generation) {
                continue;
            }
            uint256 deposited = _participants[participant].deposited +
                _participants[participant].creditDeposited;
            vesting.setShares(participant, deposited.mul(10 ** 18).div(_projectTokenPrice));
        }
        return address(vesting);
    }

    function tokensSold() public view returns (uint256) {
        return _totalSaleValue.div(_projectTokenPrice).mul(10 ** 18);
    }
    ...
    function setProjectTokenPrice(uint256 _newProjectTokenPrice) public onlyOwner {
        _projectTokenPrice = _newProjectTokenPrice;
    }

    modifier inCurrentGeneration(address _participant) {
        require(
            _participants[_participant].generation == _generation,
            "ProjectSale: not in current generation"
        );
        _;
    }

    function register() public override inCurrentGeneration(msg.sender) {
        require(_state == ProjectState.Registration, "ProjectSale: not in registration state");
        require(!isRegistered(msg.sender), "ProjectSale: already registered");
        _registeredParticipants.set(msg.sender, _generation);
        emit Registered(msg.sender);
    }

    function isRegistered(address _account) public view override returns (bool) {
        (bool contains, uint256 generation) = _registeredParticipants.tryGet(_account);
        return contains && generation == _generation;
    }

    function deposit(uint256 _amount) public override inCurrentGeneration(msg.sender) {
        require(_amount > 0, "ProjectSale: amount must be greater than zero");

        uint256 finalTransferAmount = _amount;
        if (_state == ProjectState.Staking && isRegistered(msg.sender)) {
            uint256 totalDeposited = _participants[msg.sender].deposited + _amount;
            require(
                totalDeposited <= _participants[msg.sender].allocation,
                "ProjectSale: amount exceeds allocation"
            );
        } else if (_state == ProjectState.Public) {
            require(_totalSaleValue + _amount <= _totalSaleValueCap, "ProjectSale: cap exceeded");
            finalTransferAmount += _amount.mul(_fee).div(100);
        } else {
            revert("ProjectSale: not in staking or public state");
        }
        _participants[msg.sender].deposited = _participants[msg.sender].deposited.add(_amount);
        peggedToken.safeTransferFrom(msg.sender, address(this), finalTransferAmount);
        emit Deposit(msg.sender, _amount);
    }

    function depositedOf(address _account) public view override returns (uint256) {
        return _participants[_account].deposited;
    }

    function creditDepositedOf(address _account) public view override returns (uint256) {
        return _participants[_account].creditDeposited;
    }

    function depositableOf(address _account) public view override returns (uint256) {
        if (_participants[_account].generation != _generation) {
            return 0;
        }
        if (_state == ProjectState.Staking && isRegistered(_account)) {
            return _participants[_account].allocation - _participants[_account].deposited;
        } else if (_state == ProjectState.Public) {
            return _totalSaleValueCap - _totalSaleValue;
        } else {
            return 0;
        }
    }
    ...
    function creditDepositableOf(address _account) public view override returns (uint256) {
        ...
        if (
            (_state == ProjectState.Staking && isRegistered(_account)) ||
            _state == ProjectState.Public
        ) {
            return
                _participants[_account].creditAllocation - _participants[_account].creditDeposited;
        }
        return 0;
    }

    function totalSaleValue() public view override returns (uint256) {
        return _totalSaleValue;
    }

    function totalSaleValueCap() public view override returns (uint256) {
        return _totalSaleValueCap;
    }

    function getParticipant(address _participant) public view returns (Participant memory) {
        return _participants[_participant];
    }
