...
contract NENDCore is ERC20Burnable, Ownable {
    ...
    uint public advisor;
    uint public sale;
    uint public lastInflated;
    uint public inflationInterval = 1 weeks;
    uint public inflationAmount = 100 ether;
    ...
    modifier onlyBridge {
        require(bridgeAddress != address(0x0), "Bridge not set");
    ...
    function mint(address _account, uint _amount) external onlyBridge {
    ...
    function _tryInflate() internal {
        if (isMintChain && block.timestamp - lastInflated > inflationInterval && totalSupply() + inflationAmount < 100000000 ether) {
...
// File contracts/NENDAirdrop.sol
...
contract NENDAirdrop is NENDCore {
    ...
    function createAirdrop(address _receiver, uint _amount) external onlyOwner {
    ...
    function createAirdropBatch(address[] memory _receivers, uint _amount)
        external
        onlyOwner
    {
        require(
...
// File contracts/NENDCrowdSale.sol
...
contract NENDCrowdSale is NENDAirdrop {
    ...
    function buyNEND() external payable {
        ...
        uint nendsBought = msg.value * tokenExchangeRate;
