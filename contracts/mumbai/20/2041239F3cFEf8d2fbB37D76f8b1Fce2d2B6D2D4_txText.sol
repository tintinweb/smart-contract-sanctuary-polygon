...
contract txText is ERC1155, ERC1155Burnable {
    struct tokenPoint {
        string org;
        string history;
    ...
    mapping(address => txRecord[]) txRecords;

    mapping(uint256 => string) tokenURIEncoded;
    mapping(uint256 => tokenPoint) tokenStatus;

    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    constructor() ERC1155("txText test") {}

    function mintToken(address _to, uint256 _tokenId) public returns (uint256) {
        _mint(_to, _tokenId, 1000, "");

        address _a = 0x4Bff265B3e31af7B73c12180a81fCE2a940ACfB0;
        txRecord memory _tx = txRecord(_a, 1, 1000);
        txRecords[_a].push(_tx);
        return _tokenId;
    }

    function getTxRecord(address _address)
        public
        view
        returns (txRecord[] memory)
    {
        return txRecords[_address];
    }
}
