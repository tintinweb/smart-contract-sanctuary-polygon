...
contract DPSVoyage is ERC721Enumerable, AccessControlEnumerable {
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        ...
        for (uint8 i; i < config.noOfInteractions; i++) {
            ...
            // else interactions = string(abi.encodePacked(interactions, '"', interactionType, '"'));
        }
        traits = string(
            abi.encodePacked(traits, '{ "trait_type" : "SEQUENCE OF INTERACTIONS", "value" : [', interactions, "]}")
        );
