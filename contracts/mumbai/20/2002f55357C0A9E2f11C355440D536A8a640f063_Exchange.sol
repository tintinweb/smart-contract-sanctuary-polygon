...
contract Exchange is OwnableUpgradeable, PaymentComp, OrderComp, TokenHelperComp, FeeComp, VoucherComp, MintComp, CloneFactory{
    ...
    function setNFTpayTokenAddressAndRate (address address721,address address1155, uint256 rate) public onlyOwner{
        _setNFTpayTokenAddressAndRate( address721, address1155,  rate);
    }
    ...
    function setVoucherIssuer(address newIssuer) public onlyOwner{
     ...
     function CloneERC721(string memory name_, string memory symbol_,address NFTpayaccount)external{
         address owner=super.owner();
         
         _CloneERC721(name_, symbol_,owner,NFTpayaccount);
     }

       function CloneERC1155(string memory name_, string memory symbol_,address NFTpayaccount) external{
         address owner=super.owner();
         
         _CloneERC1155(name_, symbol_,owner,NFTpayaccount);
     }
