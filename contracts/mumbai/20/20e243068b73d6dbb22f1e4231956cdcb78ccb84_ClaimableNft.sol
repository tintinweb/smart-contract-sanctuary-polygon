...
contract ClaimableNft is ERC721URIStorage, ERC721Enumerable, Ownable {
    ...
    {
        return mintNFTTo(msg.sender, uri);
    }

    function mintNFTTo(address to, string memory uri) public onlyOwner returns (uint256) {
        _tokenIds.increment();
    ...
    function mintMultipleNFT(address[] memory _beneficiary, string memory uri) public {
        for (uint256 i = 0; i < _beneficiary.length; i++) {
            mintNFTTo(_beneficiary[i], uri);
        }
    }
