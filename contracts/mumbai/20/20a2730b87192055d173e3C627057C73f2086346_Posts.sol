...
contract Posts {
    ...
    * @param whatToGet : [0 = not comments / 1 = get Comments of user / 2 = get comments of post, isCommentOf message ID, repostType[0-2 (same as comments)]
    */
    function getMsgIDs(address usrAddress, uint256 startFrom, uint256[] calldata whatToGet) public view returns(uint256[] memory) {
        ...
        if ((whatToGet[0] == 2 || whatToGet[2] == 2) && whatToGet[1] > 0) {
        ...
        if (bucketKeyID != 0 && amountLeft > 0){
            ...
            for (uint i=postsMap[remainderBucketKey].length; i > postsMap[remainderBucketKey].length - amountLeft; i--) {
    ...
    function addPost(uint256 msgID, address addressPoster, uint256 isCommentOf, uint256 isRepostOf) public onlyAdmins {
    ...
    function removePost(uint256 msgID, address addressPoster, uint256 isCommentOf, uint256 isRepostOf) public onlyAdmins {
        ...
        if (isRepostOf > 0){
            ...
            prefix = KUtils.append(KUtils.toString(isCommentOf), '', reposts, '', '');
        }
