...
/**
 * @title D_ERC1155_Generic contract.
 ...
 * @dev `D` in the `D_ERC1155_Generic` contract means that token Metadata can be updated dynamically.
 */
contract D_ERC1155_Generic is ERC1155, Ownable, AccessControl {
    struct Token {
        uint256 level;
        string[] uris;
    }
     ...
     */
    event DynamicERC1155Minted(address receiver, string backendTokenKey, uint256 tokenId, uint256 amount);
     ...
     */
    event TokenBurned(address account, string backendTokenKey, uint256 tokenId, uint256 amount);
     ...
     * @param tokenId Token Id of burned tokens.
     */
    event MetadataSetted(string[] uris, string backendTokenKey, uint256 tokenId);

    /**
     * @dev Sets the name, symbol of ERC-1155 collection, and contract Uri.
     ...
     *
     * @param name_ Name of NFT collection of ERC1155 standard.
     * @param symbol_ Symbol of NFT collection of ERC1155 standard.
    ...
    constructor(
        ...
        require(bytes(name_).length != 0, "D_ERC1155_Generic: Incorrect name");
        require(bytes(symbol_).length != 0, "D_ERC1155_Generic: Incorrect symbol");
        require(bytes(contractUri_).length != 0, "D_ERC1155_Generic: Incorrect contract Uri");
     ...
     * - `backendTokenKey_` in `_backendTokenKeyToId` mapping required to exist at the moment of minting.
     *
     ...
     */
    function setToken(string memory backendTokenKey_, string[] memory uris_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(uris_.length > 0 && bytes(uris_[0]).length != 0, "D_ERC1155_Generic: Uris cannot be empty");
        require(_backendTokenKeyToId[backendTokenKey_] == 0, "D_ERC1155_Generic: Mint need to be unique");
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        _backendTokenKeyToId[backendTokenKey_] = tokenId;
        _tokens[tokenId].uris = uris_;
     ...
     */
    function changeToken(string memory backendTokenKey_, uint256 level_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        uint256 tokenId = getTokenKeyId(backendTokenKey_);
        require(level_ < (_tokens[tokenId].uris).length, "D_ERC1155_Generic: Update not available for this token");
        _tokens[tokenId].level = level_;
        emit NFTmetadataChanged(tokenId, _tokens[tokenId].uris[level_], level_);
    }
     ...
     * Requirements:
     *
     * - `_tokens[tokenId].uris` length must be greater than 0. Otherwise URI for this token was not setted.
     *
    ...
    function uri(uint256 tokenId_) public view override returns (string memory) {
        require((_tokens[tokenId_].uris).length != 0, "D_ERC1155_Generic: URI doesnt exist");
        uint256 level = _tokens[tokenId_].level;
        string memory tokenUri = _tokens[tokenId_].uris[level];
        return tokenUri;
    }
     ...
     */
    function getTokenKeyId(string memory backendTokenKey) public view returns (uint256) {
        require(_backendTokenKeyToId[backendTokenKey] != 0, "D_ERC1155_Generic: No token exist with this name");
        return _backendTokenKeyToId[backendTokenKey];
    }
