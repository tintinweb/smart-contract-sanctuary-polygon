...
contract MainController is MainControllerStorageV1, MainControllerInterface, Initializable, UUPSUpgradeable, OwnableUpgradeable {
    /// @notice Emitted when an admin supports a match 
    event MatchListed(address indexed aMatch);

    /// @notice Emitted when a match close time is changed by admin
    event NewClosedTime(address indexed aMatch, uint oldCloseTime, uint newCloseTime);
	...
	matchInfos[matchAddress] = Match(true, a, b, startTime, endTime);
    	...
    	emit MatchCreated(date, a, b, matchAddress);
	return matchAddress;
    }
...
abstract contract MatchInterface {
    ...
    function commonVotePool() external virtual view returns (uint winAmount, uint loseAmount, uint evenAmount);
...
contract MatchContract is MatchInterface{ 
    ...
    function commonVotePool() external override view returns (uint winAmount, uint loseAmount, uint evenAmount)
    {
	...
	scoreRewardPoolAmount[_teamAScore][_teamBScore] += amount * (1000 - uplineFee - upline2Fee) / 1000;
    	return true;
    }
	...
	if(scoreRewardPoolAmount[_teamAScore][_teamBScore] > 0)
		return false;
...
abstract contract MainControllerInterface {
    ...
    function setUpline(address userAddress, address uplineAddress) external virtual;
    function setUsdtAddress(address _usdtAddress) external virtual;

    /*** Events ***/
    event MatchCreated(string indexed date, uint8 indexed teamA, uint8 indexed teamB, address matchAddress);
}
