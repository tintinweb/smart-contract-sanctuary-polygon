...
contract OyesMatic {
    ...
    event NewUserPlace(address indexed user,uint indexed userId, address indexed referrer,uint referrerId,address regreferrer,uint regreferrerId, uint8 matrix, uint8 level, uint8 place,uint256 time,uint8 isHeMissed);
    event MissedMaticReceive(address indexed receiver,uint receiverId, address indexed from,uint indexed fromId, uint8 matrix, uint8 level,uint256 time);
    ...
    function updateO3Referrer(address userAddress, address referrerAddress, uint8 level) private {
        ...
        if (users[referrerAddress].O3Matrix[level].referrals.length < 3) {
            emit NewUserPlace(userAddress,users[userAddress].id, referrerAddress, users[referrerAddress].id,users[userAddress].referrer,users[users[userAddress].referrer].id, 1, level, uint8(users[referrerAddress].O3Matrix[level].referrals.length),block.timestamp,0);
            return sendMaticDividends(referrerAddress, userAddress, 1, level);
        }
        
        emit NewUserPlace(userAddress,users[userAddress].id, referrerAddress,users[referrerAddress].id,users[userAddress].referrer,users[users[userAddress].referrer].id, 1, level, 3,block.timestamp,0);
        //close matrix
    ...
    function updateO6Referrer(address userAddress, address referrerAddress, uint8 level) private {
        ...
        if (users[referrerAddress].O6Matrix[level].firstLevelReferrals.length < 2) {
            ...
            emit NewUserPlace(userAddress,users[userAddress].id, referrerAddress,users[referrerAddress].id,users[userAddress].referrer,users[users[userAddress].referrer].id, 2, level, uint8(users[referrerAddress].O6Matrix[level].firstLevelReferrals.length),block.timestamp,0);
            ...
            if ((len == 2) && 
                ...
                if (users[referrerAddress].O6Matrix[level].firstLevelReferrals.length == 1) {
                    emit NewUserPlace(userAddress,users[userAddress].id, ref,users[ref].id,users[userAddress].referrer,users[users[userAddress].referrer].id, 2, level, 5,block.timestamp,0);
                } else {
                    emit NewUserPlace(userAddress,users[userAddress].id,ref,users[ref].id,users[userAddress].referrer,users[users[userAddress].referrer].id, 2, level, 6,block.timestamp,0);
                }
            }  else if ((len == 1 || len == 2) &&
                    users[ref].O6Matrix[level].firstLevelReferrals[0] == referrerAddress) {
                if (users[referrerAddress].O6Matrix[level].firstLevelReferrals.length == 1) {
                    emit NewUserPlace(userAddress,users[userAddress].id, ref,users[ref].id,users[userAddress].referrer,users[users[userAddress].referrer].id, 2, level, 3,block.timestamp,0);
                } else {
                    emit NewUserPlace(userAddress,users[userAddress].id, ref,users[ref].id,users[userAddress].referrer,users[users[userAddress].referrer].id, 2, level, 4,block.timestamp,0);
                }
    ...
    function updateO6(address userAddress, address referrerAddress, uint8 level, bool x2) private {
        if (!x2) {
            ...
            emit NewUserPlace(userAddress,users[userAddress].id, users[referrerAddress].O6Matrix[level].firstLevelReferrals[0],users[users[referrerAddress].O6Matrix[level].firstLevelReferrals[0]].id,users[userAddress].referrer,users[users[userAddress].referrer].id, 2, level, uint8(users[users[referrerAddress].O6Matrix[level].firstLevelReferrals[0]].O6Matrix[level].firstLevelReferrals.length),block.timestamp,0);
            emit NewUserPlace(userAddress,users[userAddress].id, referrerAddress,users[referrerAddress].id,users[userAddress].referrer,users[users[userAddress].referrer].id, 2, level, 2 + uint8(users[users[referrerAddress].O6Matrix[level].firstLevelReferrals[0]].O6Matrix[level].firstLevelReferrals.length),block.timestamp,0);
            //set current level
            ...
            emit NewUserPlace(userAddress,users[userAddress].id, users[referrerAddress].O6Matrix[level].firstLevelReferrals[1],users[users[referrerAddress].O6Matrix[level].firstLevelReferrals[1]].id,users[userAddress].referrer,users[users[userAddress].referrer].id, 2, level, uint8(users[users[referrerAddress].O6Matrix[level].firstLevelReferrals[1]].O6Matrix[level].firstLevelReferrals.length),block.timestamp,0);
            emit NewUserPlace(userAddress,users[userAddress].id, referrerAddress,users[referrerAddress].id,users[userAddress].referrer,users[users[userAddress].referrer].id, 2, level, 4 + uint8(users[users[referrerAddress].O6Matrix[level].firstLevelReferrals[1]].O6Matrix[level].firstLevelReferrals.length),block.timestamp,0);
            //set current level
        ...
        if (matrix == 1) {
            while (true) {
                if (users[receiver].O3Matrix[level].blocked) {
                    emit MissedMaticReceive(receiver,users[receiver].id, _from,users[_from].id, 1, level,block.timestamp);
                    User memory fromuser = users[_from];
                    emit NewUserPlace(_from,fromuser.id, receiver,users[receiver].id,fromuser.referrer,users[fromuser.referrer].id, matrix, level, 0,block.timestamp,1);
            ...
            while (true) {
                if (users[receiver].O6Matrix[level].blocked) {
                    emit MissedMaticReceive(receiver,users[receiver].id, _from,users[_from].id, 2, level,block.timestamp);
                    User memory fromuser = users[_from];
                    emit NewUserPlace(_from,fromuser.id, receiver,users[receiver].id,fromuser.referrer,users[fromuser.referrer].id, 2, level, 0,block.timestamp,1);
                    isExtraDividends = true;
    ...
    function sendMaticDividends(address userAddress, address _from, uint8 matrix, uint8 level) private {
        ...
        if(matrix==1)
        { 
            users[userAddress].O3Income += (levelPrice[level] - feeamount);
        }
        else if(matrix==2)
        {
            users[userAddress].O6Income +=(levelPrice[level] - feeamount) ;    
        }
