...
contract ZKBadgeMinter is IZKBadgeMinter, Initializable, AccessControl, ERC1155, SismoConnect {
  ...
  address public constant DEFAULT_ZK_BADGE_CONFIG_OWNER =
    0x00c92065F759c3d1c94d08C27a2Ab97a1c874Cbc;

  string _sismoConnectZKBadgeURI;
  ...
  mapping(uint256 tokenId => ZKBadgeConfig zkBadgeConfig) private _zkBadgeConfigs;

  // mapping to know if a vaultId has minted a ZK Badge.We can retrieve its timestamp and balance.
  // We allow each vaultId to renew its ZK Badge on the same address he used to mint it.
  ...
  mapping(uint256 tokenId => mapping(uint256 vaultId => ZKBadgeData zkBadgeData))
    private _zkBadgesData;

  // mapping to know on which address a vaultId has minted a ZK Badge
  mapping(uint256 tokenId => mapping(address holder => uint256 vaultId)) private _vaultIds;
  ...
  function claimWithSismoConnect(bytes memory responseBytes, address to, uint256 tokenId) external {
    ...
    // check if vaultId has already minted this ZK Badge
    uint256 vaultId = result.getUserId(AuthType.VAULT);
    if (_zkBadgesData[tokenId][vaultId].timestamp != 0) {
      revert ZKBadgeAlreadyMintedWithVaultId(tokenId, vaultId);
    }

    // if a ZK Badge has already been minted on the address specified, we revert
    if (balanceOf(to, tokenId) != 0) {
      revert ZKBadgeAlreadyMintedWithAnotherVaultId(tokenId, vaultId, _vaultIds[tokenId][to]);
    }

    // update state to keep track of the vaultId for this ZKBadge
    _zkBadgesData[tokenId][vaultId] = ZKBadgeData({
      timestamp: uint32(block.timestamp),
      balance: amount,
      owner: to
    });
    _vaultIds[tokenId][to] = vaultId;

    emit ZKBadgeMinted(tokenId, vaultId, _zkBadgesData[tokenId][vaultId]);

    // stay compatible with ERC1155
    emit TransferSingle(address(this), address(0), to, tokenId, amount);
  }

  function renewWithSismoConnect(bytes memory responseBytes, uint256 tokenId) external {
    SismoConnectResponse memory response = abi.decode(responseBytes, (SismoConnectResponse));
    ...
    SismoConnectRequest memory request = buildRequest({
      auths: _authRequests,
      claims: _claimRequests,
      signature: buildSignature({message: abi.encode(tokenId)})
    });
    ...
    // if the vaultId is not used for this tokenId, we cannot renew the ZK Badge since it has not been minted yet
    if (_zkBadgesData[tokenId][vaultId].timestamp == 0) {
      revert VaultIdNotUsedForTokenId(tokenId, vaultId);
    }

    // update state to keep track of the vaultId for this ZKBadge
    _zkBadgesData[tokenId][vaultId].timestamp = uint32(block.timestamp);

    emit ZKBadgeRenewed(tokenId, vaultId, _zkBadgesData[tokenId][vaultId]);
  }
  ...
  function updateZKBadgeConfig(
    ...
    _setZKBadge(appId, isImpersonationMode, tokenURI, authRequests, claimRequests, owner, tokenId);

    emit ZKBadgeConfigUpdated(tokenId, _zkBadgeConfigs[tokenId]);
  }

  /**
   * @dev Set the URI for Sismo Connect ZK Badges. Can only be called by owner (default admin)
   * @param newUri new Sismo Connect URI
   */
  function setUri(string memory newUri) external override onlyRole(DEFAULT_ADMIN_ROLE) {
    _setSismoConnectZKBadgeURI(newUri);
  }

  /**
   * @dev Set the URI for legacy ZK Badges. Can only be called by owner (default admin)
   * @param legacyUri new legacy URI
   */
  function setLegacyUri(string memory legacyUri) external onlyRole(DEFAULT_ADMIN_ROLE) {
    _setURI(legacyUri);
  }

  function balanceOfVaultId(uint256 tokenId, uint256 vaultId) external view returns (uint256) {
    return _zkBadgesData[tokenId][vaultId].balance;
  }
  ...
  function uri(uint256 tokenId) public view override(ERC1155) returns (string memory) {
    ...
    return
      keccak256(bytes(_zkBadgeConfigs[tokenId].uri)) == keccak256(bytes(""))
        ? _sismoConnectZKBadgeURI
        : _zkBadgeConfigs[tokenId].uri;
  }
  ...
  function isVaultIdUsedForTokenId(uint256 tokenId, uint256 vaultId) external view returns (bool) {
    return _zkBadgesData[tokenId][vaultId].timestamp != 0;
  }
  ...
  function _createZKBadge(
    ...
    emit ZKBadgeConfigRegistered(tokenId, _zkBadgeConfigs[tokenId]);
  }
  ...
  function _setZKBadge(
    ...
    _zkBadgeConfigs[tokenId].uri = tokenURI;
    _setAuths(tokenId, authRequests);
    _setClaims(tokenId, claimRequests);
    _zkBadgeConfigs[tokenId].owner = owner == address(0) ? DEFAULT_ZK_BADGE_CONFIG_OWNER : owner;
  }
  ...
  function _isTokenIdFromLegacy(uint256 tokenId) private pure returns (bool) {
    ...
    // to avoid any conflict, we consider that any tokenId STRICTLY below 40_000_000 is reserved to legacy badges
    return tokenId < 40_000_000;
  }

  function _setSismoConnectZKBadgeURI(string memory newURI) internal {
    _sismoConnectZKBadgeURI = newURI;
  }
  ...
  function balanceOf(
    ...
    if (_isTokenIdFromLegacy(tokenId)) {
      return _attestationsRegistry.getAttestationValue(tokenId, account);
    }
    uint256 vaultId = _vaultIds[tokenId][account];
    return _zkBadgesData[tokenId][vaultId].balance;
  }
...
import {SismoConnectConfig, AuthRequest, ClaimRequest, AuthType} from "sismo-connect-solidity/SismoLib.sol";
...
interface IZKBadgeMinter {
  ...
  error ZKBadgeAlreadyMintedWithVaultId(uint256 tokenId, uint256 vaultId);
  error ZKBadgeAlreadyMintedWithAnotherVaultId(
    uint256 tokenId,
    uint256 vaultId,
    uint256 vaultIdWithZKBadge
  );
  error VaultIdNotUsedForTokenId(uint256 tokenId, uint256 vaultId);
  error TokenIdAlreadyUsedFromLegacy(uint256 tokenId);

  event ZKBadgeConfigRegistered(uint256 indexed tokenId, ZKBadgeConfig config);
  event ZKBadgeConfigUpdated(uint256 indexed tokenId, ZKBadgeConfig config);
  event ZKBadgeMinted(uint256 indexed tokenId, uint256 indexed vaultId, ZKBadgeData zkBadgeData);
  event ZKBadgeRenewed(uint256 indexed tokenId, uint256 indexed vaultId, ZKBadgeData zkBadgeData);
