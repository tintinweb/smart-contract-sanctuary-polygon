...
contract Snapshots is ERC1155Supply, Ownable {
    IWorld private immutable _world;
    IParticipationToken private immutable _participationToken;

    // Current Settings
    uint256 public tokensCostPerSnapshotCell;
    uint256 public elementsNeededPerSnapshotCell;

    mapping(uint256 => SnapshotData) private _snapshotData;
    ...
    function createSnapshot(
        ...
        int256 b = int256(_world.worldRadius()); // World Bound
        require(
        ...
        require(
            _participationToken.balanceOfAccount(_msgSender()) >=
                requiredTokens,
        ...
        for (uint256 x = 0; x < diameter_CU; ++x) {
            for (uint256 y = 0; y < diameter_CU; ++y) {
                ...
                myElements += _world.numberOfPlacedElementsInCellByAddress(
                    cX,
                    cY,
                    _msgSender()
                );
        ...
        _participationToken.burn(_msgSender(), requiredTokens);

        uint256 lastElementID = _world.placedElementsCount();

        _createSnapshot(_msgSender(), x_EU, y_EU, radius_CU, lastElementID);
    }
    ...
    function _createSnapshot(
        ...
        uint256 snapShotID = snapshotsCount++;

        _mint(to, snapShotID, 1, "");

        _snapshotData[snapShotID] = SnapshotData(
            x,
            y,
            radius_CU,
            lastElementID
...
interface IWorld {
    ...
    function cellDiameter() external view returns (uint256);
    ...
    function numberOfPlacedElementsInCellByAddress(
        int256 x_CU,
        int256 y_CU,
        address placedBy // rights holder TODO prepisat nazvy
