...
contract Vesting is Ownable, ReentrancyGuard {
    ...
    uint256 public constant SECONDS_IN_MONTH = 60*60;
    ...
    function getReleaseReadyAmount() public view returns (uint256 releaseReadyAmount) {
        for(uint i = 0; i < usersIndexes[msg.sender]; i++) {
            if (
                ...
                && getDiffMonth(
                    usersVestings[msg.sender][i].startDate,
                    usersVestings[msg.sender][i].lastReleaseDate
                ) < COUNT_OF_MONTH
            ) {
                if (
    ...
    function release() public nonReentrant {
        for(uint i = 0; i < usersIndexes[msg.sender]; i++) {
            if (
                ...
                usersVestings[msg.sender][i].lastReleaseDate = block.timestamp;

                IERC20(tokenAddress).transfer(msg.sender, releaseReadyAmount);
