...
contract TestToken is ERC721URIStorageEnumerable, AccessControl, ReentrancyGuard {
  ...
  address public beneficiary; // Receive user-transferred fee
  ...
  constructor(
    ...
    EIP712Base(DOMAIN_NAME, DOMAIN_VERSION, block.chainid)
    {
      ...
      require(_beneficiaryAddress != address(0), "invalid _beneficiaryAddress");
  ...
  {    
    ...
    weth.safeTransferFrom(_minter, beneficiary, _fee);
...
abstract contract AccessControl is EIP712MetaTransaction {
    ...
    modifier onlyDbilia() {
        require(msgSender() == owner || _authorizedAddressList[msgSender()] == true, 
