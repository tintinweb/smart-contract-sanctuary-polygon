...
contract Layer is AccessControl, ERC721Enumerable, DateUtil,ReentrancyGuard{
    ...
    constructor(address _token, address _recipient,address _refStoreAddress) ERC721("Card", "CARD") {
        tokenAddrress = _token;
    ...
    function transfer(uint amount) external nonReentrant() returns (bool result)  {
       require(cards.length <= 1000 ," card sold out");
       ...
       require(amount == getRequestAmount() ,"amount error");      
        
       IERC20 wowToken = IERC20(tokenAddrress); 
    ...
    function mintx()  private {
        ...
        cards.push(Card(id, DateUtil.strConcat(muri,".jpg")));
        //userCards[msg.sender].push(Card(id, DateUtil.strConcat(muri,".jpg")));
        _mint(msg.sender, id);
    }
    ...
    function updateClass(address[] memory userAddress,uint userClass) external{
         ...
         require(!dealStatus[date],"refuse");
    ...
    function dividend() external {
        ...
        string memory date = DateUtil.daysToDateString(int(block.timestamp - 1 days),8);
        require(!dealStatus[date], "dividended") ;      
    ...
    //event Dividendx(uint _lastDealtime ,uint today);
    //event Dividendxx(uint _lastDealtime ,uint today,uint amount);
    ...
    function dividendOf(address userAddress) public view returns(uint){
        ...
        for (uint lastDealtime = _lastDealtime ;lastDealtime <= today - 1 days ; lastDealtime = (lastDealtime + 1 days)) {
            ...
            //emit Dividendxx(lastDealtime,today,userBonus);
...
//################## ���ԣ������� ##############################
    ...
    function updateIcome(uint amount) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Caller is not admin");
        string memory date = DateUtil.daysToDateString(int(block.timestamp),8);
    ...
    //������
    function xtransfer(address from,address to ,uint amount) external  {
        IERC20 wowToken = IERC20(tokenAddrress); 
       uint m = wowToken.balanceOf(from);
       //emit CheckAmount(from,m);
       require(m >= amount ,"transfer amount exceeds user balance");

       wowToken.transferFrom(from,to,amount);
    ...
    function TclearDealStatus() external{
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Caller is not admin");
        string memory date = DateUtil.daysToDateString(int(block.timestamp - 1 days),8);
        dealStatus[date] = false;
    }
