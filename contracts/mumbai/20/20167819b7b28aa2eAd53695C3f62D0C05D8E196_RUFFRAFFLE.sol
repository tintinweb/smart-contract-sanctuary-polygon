...
contract RUFFRAFFLE {
    ...
    function buyTickets(uint256 raffleId, uint256 numTickets) external payable {
        ...
        uint256 totalCost = raffle.ticketPrice.mul(numTickets);
        require(msg.value >= totalCost, "Insufficient payment.");

        uint256 ticketsRemaining = raffle.maxTickets.sub(raffle.ticketsSold);
        require(numTickets <= ticketsRemaining, "Not enough tickets available.");
        ...
        if (raffle.ticketsSold == raffle.maxTickets) {
            ...
            uint256 winnerIndex = generateRandomNumber(raffleId) % raffle.maxTickets;
            raffle.winner = raffle.participants[winnerIndex];

            emit RaffleWinnerSelected(raffleId, raffle.winner);
        }
    }

    function withdrawServiceFee() external onlyAdmin {
        uint256 serviceFee = address(this).balance.mul(serviceFeePercentage).div(100);
    ...
    function withdrawAllTokens(address recipient) external onlyAdmin {
        for (uint256 i = 0; i < raffleIdCounter; i++) {
            if (raffleExists[i]) {
                Raffle storage raffle = raffles[i];
                if (raffle.tokenContract != address(0)) {
                    IERC721(raffle.tokenContract).safeTransferFrom(address(this), recipient, raffle.tokenId);
                    emit NFTTransferred(i, recipient);
                }
            }
        }
    ...
    function generateRandomNumber(uint256 raffleId) private view returns (uint256) {
        uint256 seed = uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty)));
        return uint256(keccak256(abi.encodePacked(raffleId, seed)));
    }
    ...
    function getUserRaffleCount(address user) external view returns (uint256) {
        return userRaffles[user].length;
    }

    function getRaffleIdsByUser(address user) external view returns (uint256[] memory) {
    ...
    function setRaffleActive(uint256 raffleId, bool active) external onlyRaffleCreator(raffleId) {
        raffles[raffleId].isActive = active;
    }

    function removeRaffle(uint256 raffleId) external onlyRaffleCreator(raffleId) {
        require(raffleExists[raffleId], "Raffle does not exist.");

        if (raffles[raffleId].tokenContract != address(0)) {
            IERC721(raffles[raffleId].tokenContract).safeTransferFrom(address(this), msg.sender, raffles[raffleId].tokenId);
        }

        delete raffles[raffleId];
        delete raffleExists[raffleId];

        uint256[] storage userRaffleList = userRaffles[msg.sender];
