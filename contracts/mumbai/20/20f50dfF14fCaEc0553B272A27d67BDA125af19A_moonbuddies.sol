...
contract moonbuddies is ERC721Enumerable, ERC721URIStorage, Ownable, ERC721Burnable, NonblockingReceiver, ClampedRandomizer{
  using Counters for Counters.Counter;
  Counters.Counter private _tokenIds;
  uint256 private _price;
  uint256 private _wlPrice;
  uint256 private _maxMintable = 0;
  ...
  uint256 private constant START_ID = 0;
  ...
  uint256 private wlTime;
  uint256 private publicTime;
  bool private paused;

  constructor(
  address payoutAddress1_,
  address payoutAddress2_,
  address royaltyAddress_,
  address lzEndpoint_
  ) ERC721("MoonBuddies", "MOOBS") ClampedRandomizer(_maxMintable){
    ...
    maxPerWallet = 100;
    paused = true;
    endpoint = ILayerZeroEndpoint(lzEndpoint_);
    payoutAddress1 = payoutAddress1_;
    payoutAddress2 = payoutAddress2_;
    ...
    function traverseChains(uint16 _chainId, uint256 tokenId) public payable {
        ...
        require(msg.value >= messageFee, "msg.value not enough to cover messageFee");
  ...
  function addUser(address[] memory _addressToWhitelist, uint256 amount) public onlyOwner {
  ...
  function setPrice(uint256 priceInMilli, uint256 wlPriceInMilli) public onlyOwner {
    _price = priceInMilli * 1000000000000000;
    _wlPrice = wlPriceInMilli * 1000000000000000;
  }
  function maxValues(uint256 maxMintable_, uint256 maxPerWallet_) public onlyOwner {
  ...
  function setPrivateAndPublicSale(uint256 privateTime_, uint256 publicTime_) public onlyOwner {
      wlTime = privateTime_;
  ...
  function mint(uint256 quantity) public payable {
    if(privateSale()) {
      require(quantity <= whitelistMints[msg.sender], "not whitelisted");
      require(msg.value >= (_wlPrice * quantity), "Not enough AVAX");
      whitelistMints[msg.sender] -= quantity ;}
    else if(publicSale()){
      require(publicMints[msg.sender] <= maxPerWallet, "minted max");
      require(msg.value >= (_price * quantity), "Not enough AVAX");
      publicMints[msg.sender] += quantity;
      }
    require(!paused, "not active.");
    require(quantity <= _maxMintPerTx, "more than max per tx");
   
    payable(payoutAddress1).transfer(msg.value / 2);
    payable(payoutAddress2).transfer(msg.value / 2);
