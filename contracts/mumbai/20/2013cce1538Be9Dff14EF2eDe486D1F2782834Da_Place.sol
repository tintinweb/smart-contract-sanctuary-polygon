...
// interface IGreylist {
// 	// function greylistedAddresses(address) external view returns (bool);
// 	function addToGreylist() external;
// 	function isGreylisted() external view returns (bool);
// }

contract Place is ERC721Enumerable, Ownable {
	string baseTokenURI;
	...
	constructor(string memory _baseTokenURI, uint256 _maxTokenIds, address _greylistContractAddress) ERC721("Blockplace Pixel", "BP") {
		baseTokenURI = _baseTokenURI;
		tokenIdsLeft = _maxTokenIds;

		// get the grey list interface
		greylist = Greylist(_greylistContractAddress);
	}
	...
	function mintPixel(uint256 tokenId) public payable onlyWhenNotPaused {
		...
		require(!greylist.isGreylisted(msg.sender), 'Denied: You are greylisted');
		...
		tokenIdsLeft -= 1;
		color[tokenId] = "#FFFFFF";
		greylist.addToGreylist(msg.sender);
		// _safeMint(msg.sender, uint tokenId)
	}
