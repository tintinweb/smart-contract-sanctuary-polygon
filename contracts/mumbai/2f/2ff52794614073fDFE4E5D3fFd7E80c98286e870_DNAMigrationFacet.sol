...
contract DNAMigrationFacet{

    function setTargetDNAVersion(uint256 _versionNumber) external {
        LibDiamond.enforceIsContractOwner();
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        require(_versionNumber > ds.targetDNAVersion, "DNAMigrationFacet: DNA version must be greater than previous value");
        require(_versionNumber < 256, "DNAMigrationFacet: version cannot be greater than 8 bits");
        ds.targetDNAVersion = _versionNumber;
    }

    function getTargetDNAVersion() external view returns (uint256){
        return LibDiamond.diamondStorage().targetDNAVersion;
    }

    function migrateUnicornsToV2DNA(
        uint256[] memory _tokenIds,
        uint16[] memory _firstNames,
        uint16[] memory _lastNames,
        bool _bypassDNAEvents
    ) external {
        LibDiamond.enforceIsContractOwner();
        uint len = _tokenIds.length;
        require(len == _firstNames.length, "DNAMigrationFacet: Mismatched tokenId and firstName array lengths");
        require(len == _lastNames.length, "DNAMigrationFacet: Mismatched tokenId and lastName array lengths");
        require(LibUnicornDNA._targetDNAVersion() <= 2, "DNAMigrationFacet: DNA target version mismatch");

        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        for(uint256 i = 0; i < len; ++i) {
            require(_firstNames[i] > 0, "DNAMigrationFacet: First Name cannot be index 0");
            require(_lastNames[i] > 0, "DNAMigrationFacet: Last Name cannot be index 0");
            require(_firstNames[i] < 1024, "DNAMigrationFacet: First Name cannot be greater than 10 bits");
            require(_lastNames[i] < 1024, "DNAMigrationFacet: Last Name cannot be greater than 10 bits");
            // LibIdempotence.enforceCleanState(_tokenIds[i]);
            uint256 dna = LibUnicornDNA._getCanonicalDNA(_tokenIds[i]);
            dna = LibUnicornDNA._setVersion(dna, 2);
            dna = LibUnicornDNA._setFirstNameIndex(dna, _firstNames[i]);
            dna = LibUnicornDNA._setLastNameIndex(dna, _lastNames[i]);

            if(_bypassDNAEvents) {
                //  direct assignment - bypassees safeguards and reporting
                ds.unicorn_dna[_tokenIds[i]] = dna;
            } else {
                //  preferred, but slower - an event is fired for each id
                LibUnicornDNA._setDNA(_tokenIds[i], dna);
            }
        }
    }

    //  Generate DNA for tokenIds [_firstTokenId - _lastTokenId] inclusive
    //  Name arrays must align with tokenId range
    function migrateUnicornsToV2DNAByIDRange(
        ...
        LibDiamond.enforceIsContractOwner();
        require(_lastTokenId > _firstTokenId, "DNAMigrationFacet: Invalid tokenId range");
        require(_lastTokenId <= 10000, "DNAMigrationFacet: Token ID out of range");
        uint256 len = (_lastTokenId - _firstTokenId) + 1;
        require(len == _firstNames.length, "DNAMigrationFacet: Mismatched tokenId and firstName array lengths");
        require(len == _lastNames.length, "DNAMigrationFacet: Mismatched tokenId and lastName array lengths");
        require(LibUnicornDNA._targetDNAVersion() <= 2, "DNAMigrationFacet: DNA target version mismatch");
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        for(uint256 tokenId = _firstTokenId; tokenId <= _lastTokenId; ++tokenId) {
            uint256 i = tokenId - _firstTokenId;

            require(_firstNames[i] > 0, "DNAMigrationFacet: First Name cannot be index 0");
            require(_lastNames[i] > 0, "DNAMigrationFacet: Last Name cannot be index 0");
            require(_firstNames[i] < 1024, "DNAMigrationFacet: First Name cannot be greater than 10 bits");
            require(_lastNames[i] < 1024, "DNAMigrationFacet: Last Name cannot be greater than 10 bits");
            // LibIdempotence.enforceCleanState(tokenId);
    ...
    function rollbackV2DNAToV1DNA(uint256[] memory _tokenIds, bool _bypassDNAEvents, bool _force) external {
        LibDiamond.enforceIsContractOwner();
        if(!_force) {
            require(LibUnicornDNA._targetDNAVersion() <= 2, "DNAMigrationFacet: DNA target version mismatch");
        }
        ...
        for(uint256 i = 0; i < len; ++i) {
            ...
            if(!_force) {
                require(LibUnicornDNA._getVersion(dna) == 2, "DNAMigrationFacet: DNA version mismatch");
                // LibIdempotence.enforceCleanState(_tokenIds[i]);
            }

            dna = LibUnicornDNA._setVersion(dna, 1);
            dna = LibUnicornDNA._setFirstNameIndex(dna, 0);
            dna = LibUnicornDNA._setLastNameIndex(dna, 0);
    ...
    //  For any of the tokenIds submitted, this method computes their DNA and
    //  persists it into cache. This allows us to repair bugs without re-rolling
    //  DNA that is in the middle of the hatching process.
    function cacheVolatileHatchDNA(uint256[] memory _tokenIds) external {
        LibDiamond.enforceIsContractOwner();
        LibUnicornDNA.LibDNAStorage storage lds = LibUnicornDNA.dnaStorage();
        uint256 len = _tokenIds.length;
        for(uint256 i = 0; i < len; ++i) {
            if (LibHatching.shouldUsePredictiveDNA(_tokenIds[i])) {
                lds.cachedDNA[_tokenIds[i]] = LibHatching.predictBabyDNA(_tokenIds[i]);
            }
        }
    }

    function dnaReport(uint256 _tokenId) external view returns(
        bool predictive,
        uint256 live,
        uint256 canonical,
        uint256 cached
    ) {
        predictive = LibHatching.shouldUsePredictiveDNA(_tokenId); 
        live = LibUnicornDNA._getDNA(_tokenId);
        canonical = LibUnicornDNA._getCanonicalDNA(_tokenId);
        cached = LibUnicornDNA.dnaStorage().cachedDNA[_tokenId];
    }

    function adminCorrectDNA(uint256 _tokenId, uint256 _dna) external {
        LibDiamond.enforceIsContractOwner();
        LibUnicornDNA._setDNA(_tokenId, _dna);
    }

    function adminCorrectBirthday(uint256 _tokenId, uint256 _birthday) external {
        LibDiamond.enforceIsContractOwner();
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        require(ds.erc721_owners[_tokenId] != address(0), "DNAMigrationFacet: Token does not exist");
        ds.hatch_birthday[_tokenId] = _birthday;
    }
}
