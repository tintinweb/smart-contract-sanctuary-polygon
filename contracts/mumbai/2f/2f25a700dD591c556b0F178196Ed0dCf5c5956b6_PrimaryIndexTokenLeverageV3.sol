	...
	ReentrancyGuardUpgradeable
{
	 ...
	 * @param lendingToken The address of the lending token.
	 */
	function _deferLiquidityCheck(address user, address lendingToken) internal view {
		uint totalPit = primaryIndexToken.totalPIT(user);
		uint totalOutstandingInUSD = primaryIndexToken.totalOutstandingInUSD(user);
		require(
				...
				if (_positionData.longAsset == projectToken) {
					if (remainingDepositedCount == 0) {
						deleteLeveragePosition(
							account,
							_positionData.shortAsset,
							_positionData.positionId
						);
					} else {
	...
	function addDepositedLongAsset(
		...
		if (currentLongAssetCountOfProjectToken == maxLongAssetCountOfProjectToken) return;
			...
			for (uint256 i = 0; i < _positionDatas.length; i++) {
				...
				if (_positionData.shortAsset == lendingToken) {
					if (totalBorrowCount == 0) {
						deleteLeveragePosition(account, lendingToken, _positionData.positionId);
					} else {
	...
	/**
	 * @notice RCheck whether a position is open with leverage borrow or not
	 * @param account The address of user wallet.
	 * @param projectToken The address of long asset.
	...
	/**
	 * @notice Allow a user to close the leverage position.
	...
	function closePositionByLongAsset(
		...
		PositionData storage _positionData = _positionDatas[0];
		for (uint256 i = 0; i < _positionDatas.length; i++) {
			_positionData = _positionDatas[i];
			if (
				_positionData.shortAsset == lendingToken && _positionData.positionId == positionId
			) {
		...
		if (amountReceivedLendingToken >= shortAmount) {
			deleteLeveragePosition(msg.sender, lendingToken, positionId);
		} else {
