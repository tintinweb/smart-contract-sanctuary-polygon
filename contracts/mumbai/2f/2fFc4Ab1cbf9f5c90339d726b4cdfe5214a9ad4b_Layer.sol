...
contract Layer is AccessControl {
    mapping(address => address) public referrer;
    event ReferrerAdded(address indexed to,address from);
    event TranfsferToken(address indexed from,address indexed to,uint amount);
    ...
    constructor(address _token, address _recipient,address _refStoreAddress
    ) {
    ...
    function _addReferrer(address from, address to) private {
        referrer[from] = to;
        //RefStore refStore = RefStore(refStoreAddress);
    ...
    function transfer(uint amount) external returns (bool result) {
       ...
       emit TranfsferToken(msg.sender,recipient,amount);
