...
contract GameNFT is ERC721A, Whitelist{
    using Strings for uint256;
    ...
    uint256 public constant NFT_MINT_PRICE = 0.01 ether;
    ...
    constructor(string memory _baseUri)
        ERC721A("Game NFT", "NFT")
    {
        setBaseURI(_baseUri);
    }
    
    function setSaleActivatedPauseOrUnpause(bool _saleActivated) public onlyOwner{
        require(saleActivated != _saleActivated, "setSaleActivate: Value should not be same");
    ...
    function bulkMint(uint256 _amount) external payable onlyWhitelisted returns(bool) {
        ...
        require(_amount <= MAX_BULK_TOKEN, "bulkMint: Mint limit reached");
        require(balanceOf(msg.sender).add(_amount) <= MAX_BULK_TOKEN, "bulkMint: Mint limit reached");
        require(msg.value == _amount.mul(NFT_MINT_PRICE), "bulkMint: Ether value sent is not correct");
        (bool success, ) = address(this).call{value: msg.value}("");
        _safeMint(msg.sender, _amount);
        tokenId = tokenId.add(_amount);
        return success;
    }
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        ...
        string memory notRevealedUri = baseURI;
        if(!revealed) return notRevealedUri;

        string memory token_URI = _tokenURI[_tokenId];
        return bytes(token_URI).length > 0  ? token_URI : notRevealedUri;
    }
