...
contract Participants_RaffleTest_27Dec is Ownable{

    //geneate Random Interface
    ...
    uint256 public constant RaffleType = 0;     //for NFT contract
    ...
    event userRegistered(uint RaffleID, address registerUser);
    ...
    struct Raffle{
        ...
        uint256 startingRounds; // first index for the number of rounds and second for the winners of that round.
        uint256 endingRound;
        uint256 threshold;
    ...
    modifier isGenerateRandomSet(){
        require (address(generateRandom) != address(0), "random number address not set.");
        _;
    }
    ...
    constructor(address _token){
        token = ERC721(_token);
        deployer = msg.sender;
        setRandomNumberContract(0xDD98d80f3CB2be3ef1a228bfE6059c70642bc027);    //polygon testnet address
        custodialWallet = token.getCustodialWallet();
    }
    ...
    function startWeb2Raffle(
        ...
        uint256 threshold,
        bool isRegistrationRequired
    ) external onlyCustodial isGenerateRandomSet{
    ...
    function sendWinnersAndAwards(uint256 raffleId, uint256 roundId, bytes memory winnersOfTheRaffle, bytes memory prizesOfTheWinners) external onlyCustodial checkRaffle(raffleId){
        ...
        address[] memory winners = abi.decode(winnersOfTheRaffle, (address[]));
    ...
    function startRaffle(
        ...
        bool regRequired
        // bool raffleType
        ) external onlyOwner isGenerateRandomSet {
    ...
    function UserRegister(uint raffleID) checkRaffle(raffleID) external {
        require(registeredUsers[raffleID] < getRaffle(raffleID).threshold, "Raffle has already been completed." );

        address msgSender = msg.sender;
        require(token.balanceOf(msgSender) > 0, "Not Enough NFT for registeration");
        ...
        raffle[raffleID].participants.push(msgSender);
        registeredUsers[raffleID] += 1 ;

        emit userRegistered(raffleID, msgSender);
    }
    ...
    function setParticipants(uint Raffle_id, bytes memory _participants) external onlyOwner checkRaffle(Raffle_id){
        ...
        require(getParticipants(Raffle_id).length == 0, "Participants already set.");
    ...
    function setRandomNumberContract(address _generateRandom) public onlyDeployer {
...
//    
    ...
    function pickWinners(uint256 raffleID, uint256 roundNum) public onlyOwner checkRaffle(raffleID){
        ...
        require(getParticipants(raffleID).length >= getQtyOfWinners(raffleID, (getRaffle(raffleID).currentRound + 1)), "Not enough participants");
        ...
        if(participants.length  == totalWinners){
            ...
            for(uint i = 0; i < totalWinners; i++){
                ...
                if(raffle[raffleID].ignorePreviousWinner == true){
                    delete raffle[raffleID].participants;
                }
    ...
    function getAwardsOfTheWinner(uint256 raffleID, uint256 roundNum, address winner) public view returns(bytes32){
