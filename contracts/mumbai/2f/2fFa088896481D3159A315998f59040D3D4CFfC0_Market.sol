...
interface IERC721 {
  ...
  function getRoyalities() external view returns (uint); 
  ...
  */
  function _sendRoyalitiesToCreators(uint256 offerId_, uint256 value_)  internal returns (uint256 toCreators) {
    ...
    for(uint256 i=0 ; i<collectionAddrees_.length ; i++){
        ...
        if(proxy.getRoyalities() >0){
          uint256 toTansfer = aux * proxy.getRoyalities();
          _sendFounds(offerId_,proxy.deployer(), value_);
          toCreators+=toTansfer;
         emit RoyalitiesSended(proxy.deployer(), toTansfer);
        }
    }
  }
