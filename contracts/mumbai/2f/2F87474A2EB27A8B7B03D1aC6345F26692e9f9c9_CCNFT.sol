...
contract CCNFT is ERC721A, Ownable, ReentrancyGuard{
    using Strings for uint256;

    uint256 public constant MAX_SUPPLY = 101;
    uint256 public constant MAX_PUBLIC_MINT = 50;
    uint256 public constant MAX_WHITELIST_MINT = 50;
    ...
    constructor() ERC721A("CCNFT", "CCT", MAX_PUBLIC_MINT, MAX_SUPPLY){
    ...
    function mint(uint256 _quantity) external payable callerIsUser{
        require(publicSale, "Public Mint Not Yet Active.");
        require((totalSupply() + _quantity) <= MAX_SUPPLY, "Cannot mint beyond max supply");
        require((totalPublicMint[msg.sender] +_quantity) <= MAX_PUBLIC_MINT, "Exceeded max mint amount");
    ...
    function whitelistMint(bytes32[] memory _merkleProof, uint256 _quantity) external payable callerIsUser{
        require(whiteListSale, "Whitelist Mint is not live");
        require((totalSupply() + _quantity) <= MAX_SUPPLY, "Cannot mint beyond max supply");
        require((totalWhitelistMint[msg.sender] + _quantity)  <= MAX_WHITELIST_MINT, "Exceeded max mint amount");
