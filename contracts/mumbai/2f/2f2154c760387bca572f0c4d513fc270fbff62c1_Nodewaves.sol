...
contract Nodewaves is ERC20, ERC20Burnable, Ownable, ReEntrancyGuard {
    ...
    function claimWithdraw(uint _paymentType) public noReentrant {
        require(_paymentType == 1 || _paymentType == 2, "Invalid payment type");
        if(_paymentType == 1){
            uint _amount = referral_balancesUSDT[msg.sender];
            referral_balancesUSDT[msg.sender]= 0;
            
            usdt.transfer(msg.sender, _amount);
        }else{
            uint _amount = referral_balances[msg.sender];
            referral_balances[msg.sender] = 0;
