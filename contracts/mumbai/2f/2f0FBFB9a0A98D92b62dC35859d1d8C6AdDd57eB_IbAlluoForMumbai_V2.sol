...
contract IbAlluoForMumbai_V2 is
    Initializable,
    PausableUpgradeable,
    ...
    UUPSUpgradeable 
{
    ...
    function remove_liquidity(address _token, uint256 _amount) external returns(uint256){
        if (IERC20Upgradeable(_token) == USDT) {
    ...
    function changeFee(uint usdt, uint usdc)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        usdtFee = usdt;
