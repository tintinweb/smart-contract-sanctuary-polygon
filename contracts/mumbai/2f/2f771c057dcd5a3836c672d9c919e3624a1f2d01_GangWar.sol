...
contract GangWar is UUPSUpgrade, OwnableUDS, VRFConsumerV2 {
    ...
    // function healBaron(uint256 baronId, uint256 exchangeType) external isActiveSeason {
    //     _verifyAuthorizedUser(msg.sender, baronId);
...
bytes32 constant DIAMOND_STORAGE_GANG_VAULT = keccak256("diamond.storage.gang.vault.season.xxx.06");
...
contract GMCChild is UUPSUpgrade, OwnableUDS, FxERC721EnumerableChild, GMCMarket {
    ...
    uint16 NUM_GANGSTERS = 10;
    uint256 currentBaronId = 10_000;
    ...
    function _mint(
        ...
        if (isBaron) numGangsters <<= 1;

        uint256 startId = supplies[0] + 1;
        uint256 gangSupply = supplies[gang] += numGangsters;

        supplies[0] += numGangsters;
        ...
        if (isBaron) {
            uint256 baronId = ++currentBaronId;

            if (baronId > 21) revert GangstersAlreadyMinted();
...
bytes32 constant DIAMOND_STORAGE_GMC_MARKET = keccak256("diamond.storage.gmc.market.season.xxx.06");
