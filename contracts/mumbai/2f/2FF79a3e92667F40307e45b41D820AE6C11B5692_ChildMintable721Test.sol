...
ContextMixin
{
    ...
    function mintBatch(address user, uint256 _amount) public only(DEFAULT_ADMIN_ROLE) {

        uint256 supply = totalSupply();

        for (uint256 i = 0; i < _amount; i++) {
            require(!withdrawnTokens[supply + i], "ChildMintableERC721: TOKEN_EXISTS_ON_ROOT_CHAIN");
