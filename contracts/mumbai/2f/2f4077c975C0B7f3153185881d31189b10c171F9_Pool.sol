...
/**
 * @title Pool - A simple contract for testing Gelato metatxs
 */
contract Pool is ERC2771Recipient, GelatoRelayContextERC2771 {
     ...
     * @dev msg.value doesn't work with ERC2771, so it's not possible to test sending a value
     * to a contract with ERC2771. A transfer from an ERC20 is more complicated than is
     * necessary in order to test working with Gelato. This function is a workaround to test the functionality.
     * It does nothing more than update a mapping to simulate a value transfer.
     * @param amount The amount to be deposited
     *
     */
    function deposit(uint256 amount) external {
        console.log("_msgSender() in deposit function", _msgSender());
        require(amount > 0, "Deposit must be greater than 0");
        balances[_msgSender()] += amount;
        emit LogDeposit(_msgSender(), amount);
    }
    ...
    function withdraw(uint256 amount) external onlyGelatoRelayERC2771 {
        console.log("_getMsgSender() in withdraw function", _getMsgSender());
        require(balances[_getMsgSender()] >= amount, "Insufficient balance");

        //Payment to Gelato - this function only allows contract to pay gas in ERC-20 tokens
        //_transferRelayFee();

        // This is the only way to allow the user to pay gas in ERC-20 tokens using Gelato
        // The Gelato funtion above uses "transfer" instead of "transferFrom", which results in
        // only the contract being able tp gas in ERC-20 tokens.
        IERC20(_getFeeToken()).transferFrom(_getMsgSender(), _getFeeCollector(), _getFee());

        balances[_getMsgSender()] -= amount;
        emit LogWithdrawal(_getMsgSender(), amount);
    }
}
