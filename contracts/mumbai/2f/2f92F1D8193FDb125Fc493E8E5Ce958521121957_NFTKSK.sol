...
contract NFTKSK is ERC721, AxelarExecutable, Upgradable, ERC721URIStorage, ERC721Burnable {
    ...
    function _sendMintedToken(
        ...
        bytes memory payload = abi.encode(originalChain, operator, originalTokenId, receiverAddress, tokenUri);
    ...
    function _sendNativeToken(
        ...
        string memory stringAddress = destinationAddress.toString();    //change from address(this).toString()
        //Pay for gas. We could also send the contract call here but then the sourceAddress will be that of the gas receiver which is a problem later.
        gasService.payNativeGasForContractCall{ value: msg.value }(address(this), destinationChain, stringAddress, payload, msg.sender);
    ...
    function _execute(
        ...
        //Decode the payload.
        (string memory originalChain, address operator, uint256 tokenId, address receiverAddress, string memory tokenUri) = abi.decode(
            payload,
        ...
        if (keccak256(bytes(originalChain)) == keccak256(bytes(chainName))) {
            IERC721(operator).transferFrom(address(this), receiverAddress, tokenId );
            ...
            safeMint(receiverAddress, tokenUri);
        }
    }
