...
// File: @thirdweb-dev/contracts/extension/interface/IClaimableERC721.sol
...
// File: contracts/DappTestNFT.sol
...
contract DappTestNFT is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  uint256 public maxSupply = 20; // количество нфт в вашей коллекции
  ...
  function claim(address _receiver, uint256 _mintAmount) public payable virtual {
    ...
    if (_receiver != owner()) {
        if(onlyWhitelisted == true) {
            require(isWhitelisted(_receiver), "user is not whitelisted");
            uint256 ownerMintedCount = addressMintedBalance[_receiver];
