    ...
    AccessControl
{
    ...
    event EscrowChanged(IEscrow previousEscrow, address newEscrow);
    ...
    function changeEscrow(address _escrow) external onlyRole("ADMIN_ROLE") {
        emit EscrowChanged(escrow, _escrow);
    ...
    function _beforeTokenTransfer(
        ...
        for(uint256 i = 0;i<ids.length; i++) {
            require(escrow.isRented(address(this), from, ids[i], amounts[i]), "ERC721Rental: NFT is on Rental");
        }
        }
