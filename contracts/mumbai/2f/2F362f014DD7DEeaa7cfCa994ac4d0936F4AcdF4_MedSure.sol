...
contract MedSure is ERC721 {
    ...
    constructor() ERC721("MedSure1", "MEDSURE1") {
        owner = msg.sender;
    }
    ...
    function createMedSureToken(string[] calldata _hashArray) public onlyOwner {
        for (uint256 i; i < _hashArray.length; i++) {
            string memory _hash = _hashArray[i];
            require(!_hashpool[_hash]);
            details.push(_hash);

            uint256 medsuretokenid = _tokenIds.current();
            _safeMint(msg.sender, medsuretokenid);
            _hashpool[_hash] = true;
            _tokenIds.increment();

            emit TokenAttached(medsuretokenid, _hash);
        }
    }
