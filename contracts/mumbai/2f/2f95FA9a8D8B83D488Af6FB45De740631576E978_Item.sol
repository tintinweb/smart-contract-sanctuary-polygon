...
contract Item is ERC1155, Ownable, ReentrancyGuard {
    ...
    /// @dev Returns the length of the itemTypes array.
    /// @return The length of the itemTypes array.
    ...
    function setPack(
        ...
        require(
            pack.itemIds.length <= itemTypes.length,
    ...
    ///
    /// The function verifies that the item ID exists within the itemTypes array.
    ///
    ...
    function setItemsTotalSupply(
        ...
        require(itemId < itemTypes.length, "item doesn't exist");
    ...
    ///
    /// The function verifies that the token address and price oracle address are valid (not equal to address(0)).
    /// It also checks that the token contract has a positive total supply and that the price oracle provides a positive price.
    ///
    /// @param tokenId The ID of the ERC20 token.
    ...
    ///
    /// The function verifies that the price oracle address is valid (not equal to address(0)).
    ///
    /// @param priceOracleAddress The address of the price oracle contract.
    ...
    /// @dev Sets the price oracle address for the specified ERC-20 token.
    ///
    /// The function verifies that the token contract and price oracle address are valid (not equal to address(0)).
    ///
    /// @param tokenId The ID of the ERC-20 token.
    /// @param priceOracleAddress The address of the price oracle contract.
    function setERC20PriceOracle(
        ...
        require(
            ERC20Tokens[tokenId].tokenContract != IERC20(address(0)) &&
                priceOracleAddress != address(0),
            "invalid token or price oracle address"
        );
    ...
    ///
    /// The function verifies that the minting is not paused for all items and the specified item.
    /// It also checks that the sent value is sufficient to cover the cost of the items.
    ///
    ...
    ///
    /// The function verifies that the minting and payment are not paused, and that the specified item
    /// and ERC-20 token exist and are not paused.
    /// It also checks that the sent value is sufficient to cover the cost of the items.
    ///
    /// @param id The ID of the item to mint.
    /// @param amount The number of items to mint.
    ...
    ///
    /// The function verifies that the minting and pack are not paused.
    /// It also checks that the sent value is sufficient to cover the cost of the pack.
    ///
    ...
    function mintPack(
        ...
        for (uint256 i = 0; i < amount; ++i) {
            _mintBatch(msg.sender, pack.itemIds, pack.itemAmounts, "");
        }
    ...
    ///
    /// The function verifies that the minting and payment are not paused, and that the specified pack
    /// and ERC-20 token exist and are not paused.
    /// It also checks that the sent value is sufficient to cover the cost of the pack.
    ///
    /// @param packId The ID of the pack to mint.
    ...
    ///
    /// The function verifies that the minted amounts of the items, multiplied by the specified amount,
    /// do not exceed the total supply of each item.
    ///
    /// @param itemIds An array containing the IDs of the items to check.
    ...
    function requireItemsMintable(
        ...
        for (uint256 i = 0; i < itemIds.length; ++i) {
            ItemInfo storage item = items[itemIds[i]];
            require(
    ...
    ///
    /// Requirements:
    /// - Transmutation must not be paused.
    /// - The item ID must exist.
    /// - The lengths of the amounts and slots arrays must match.
    /// - The sender must not be a null address.
    /// - The sent value must be greater than or equal to the transmuteFee.
    /// - The amounts must be greater than 0.
    /// - The sum of the minted amounts and the transmuted amounts must not exceed the total supply of the item.
    ///
    ...
    /// Requirements:
    /// - Transmutation must not be paused.
    /// - The lengths of the IDs, amounts, and slots arrays must match.
    /// - The sender must not be a null address.
    /// - The sent value must be greater than or equal to the transmuteFeeBatch.
    /// - The IDs array must not contain an item that doesn't exist.
    /// - The amounts must be greater than 0.
    /// - The sum of the minted amounts and the transmuted amounts must not exceed the total supply of each item.
    ///
    ...
    function mayneTransmuteBatch(
        ...
        require(msg.sender != address(0), "sender can't be null address");
        require(msg.value >= transmuteFeeBatch, "sent value not enough");

        uint256 totalFee = transmuteFeeBatch;
        for (uint256 i = 0; i < ids.length; ++i) {
            require(
                ids[i] < itemTypes.length,
    ...
    /// - The item ID must exist.
    /// - The sender must have enough items to burn.
    ...
    /// - Burning must not be paused.
    /// - The IDs array must not contain an item that doesn't exist.
    /// - The IDs and amounts arrays must have the same length.
    /// - The amounts must be greater than 0.
    /// - The sender must have enough items to burn.
    ///
    ...
    ///
    /// Requirements:
    /// - The arrays totalSupplies and priceUsds must have the same length.
    /// - The totalSupplies and priceUsds arrays must not be empty.
    ///
    ...
    ///
    /// Requirements:
    /// - The ID must correspond to an existing item.
    /// - The amount must be greater than 0.
    /// - The total supply of the item after minting must not exceed its maximum limit.
    /// - The player address must not be the null address.
    ///
    ...
    ///
    /// Requirements:
    /// - The number of IDs and amounts must be the same.
    /// - Each ID must correspond to an existing item.
    /// - Each amount must be greater than 0.
    /// - The total supply of each item after minting must not exceed its maximum limit.
    /// - The player address must not be the null address.
    ///
    ...
    ///
    /// Requirements:
    /// - The pack must exist (i.e., pack itemIds array is not empty).
    /// - The amount must be greater than 0.
    /// - The total supply of each item in the pack after minting must not exceed its maximum limit.
    /// - The player address must not be the null address.
    ///
    ...
    function mintPackAdmin(
        ...
        PackInfo storage pack = packs[packId];
        require(pack.itemIds.length > 0, "pack doesn't exist");
        require(amount > 0, "amount can't be zero");

        for (uint256 i = 0; i < pack.itemIds.length; ++i) {
            ItemInfo storage item = items[pack.itemIds[i]];
            uint256 maxMintable = pack.itemAmounts[i] * amount;
            require(
                item.minted + maxMintable <= item.totalSupply,
                "amount exceeds total supply"
            );
        }
        ...
        for (uint256 i = 0; i < amount; ++i) {
            _mintBatch(player, pack.itemIds, pack.itemAmounts, "");
        }
    ...
    ///
    /// Requirements:
    /// - The contract's balance must be greater than 0.
    ///
    ...
    ///
    /// Requirements:
    /// - The number of `tokenIds` cannot exceed `_ERC20WithdrawalLimit`.
    /// - The `tokenIds` array cannot be empty.
    /// - Each `tokenContract` associated with a `tokenId` must be set (not equal to address(0)).
    /// - The balance of each `tokenContract` must be greater than 0.
    ///
    ...
    function withdrawERC20(
        ...
        for (uint256 i = 0; i < tokenIds.length; ++i) {
            ...
            uint256 totalAmount = balance.mul(6).div(10);
            uint256 founderAmount = totalAmount.mul(4).div(6);
            uint256 devAmount = totalAmount.mul(2).div(6);

            tokenContract.transferFrom(address(this), founder1, founderAmount);
            tokenContract.transferFrom(address(this), founder2, founderAmount);
            tokenContract.transferFrom(address(this), dev, devAmount);
        }
    }
}