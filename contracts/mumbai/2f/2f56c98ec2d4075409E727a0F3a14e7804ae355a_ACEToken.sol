    ...
    OwnerRecovery
{
    ...
    uint256 public transferFee = 7;

    modifier onlyNFTManager() {
        address sender = _msgSender();
        require(
            sender == address(NFTManager),
            "ACEToken: Not NFTManager"
        );
        _;
    }

    constructor(address _NFTManager, address _treasury) ERC20("ACE Token", "ACE") {
        require(
            _NFTManager != address(0),
            "ACEToken: NFTManager is not set"
        );
        require(
            _treasury != address(0),
            "ACEToken: treasury is not set"
        );
    ...
    function _transfer(
        ...
        require(from != address(0), "ACEToken: transfer from the zero address");
        require(to != address(0), "ACEToken: transfer to the zero address");

        if (!_isExcludedFromFee[from]) {
            uint256 fees = amount.mul(transferFee).div(100);
    ...
    function excludeFromFee(address account, bool status) external onlyOwner {
        require(account != address(0), "ACEToken: zero address");
