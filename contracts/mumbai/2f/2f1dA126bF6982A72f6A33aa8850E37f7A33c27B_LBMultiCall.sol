...
// Import the contract that defines the Ticket struct
import "./luckblocksBTCNewV2.sol";
import './utils/Address.sol';
...
contract LBMultiCall is Ownable {
    ...
    function setContracts (address _luckBlocksAddress,address _luckBlocksWAddress,address _ethLuckBlocksAddress,address _ethLuckBlocksWAddress , 
    address _maticLuckBlocksAddress,address _maticLuckBlocksWAddress,address _KrstmLuckBlocksAddress, address _KrstmLuckBlocksWAddress) public onlyOwner() {

        contractsAddresses = [_luckBlocksAddress,_ethLuckBlocksAddress,
        _maticLuckBlocksAddress,_KrstmLuckBlocksAddress,
        _luckBlocksWAddress,_ethLuckBlocksWAddress,
        _maticLuckBlocksWAddress,_KrstmLuckBlocksWAddress];

    }
    
    function getLotteryInfos() external view returns (lotteryInfo[] memory){
            
            address[] memory contracts = getContracts(); // Get the list of lottery contract addresses

            lotteryInfo[] memory lotteryContracts = new lotteryInfo[](4);
            uint256 lotteryIndex;

            for (uint256 i = 0; i < contracts.length; i++) {
                address contractAddress = contracts[i];
                LotteryContract contractInstance = LotteryContract(contractAddress);
                
                uint256 currentDrawId = contractInstance.requestCounter();
                string memory identifier =  getLotteryIdentifier(contractAddress);

                uint256 lastAutoSpin = contractInstance.autoSpinTimestamp();
                
                uint256 jpWei = contractInstance.getJackpot();
                uint256 jpUsd = contractInstance.getJackpotinUSD();
                ...
                lotteryContracts[lotteryIndex] = lottery;
                lotteryIndex++;
                
            }

            return lotteryContracts;

    }

    function getLotteryInfosW() external view returns (lotteryInfo[] memory){
            
            address[] memory contracts = getContractsW(); // Get the list of weekly lottery contract addresses
    ...
    function getAllMyTickets(address user) external view returns (ticketArray[] memory) {
        ...
        for (uint256 i = 0; i < contracts.length; i++) {
            ...
            LotteryContract contractInstance = LotteryContract(contractAddress);
            uint256 currentDrawId = contractInstance.requestCounter();
            uint16[] memory ticketsByDraw = contractInstance.getUserTicketsByDraw(currentDrawId, user);
            totalTickets += ticketsByDraw.length;
        }

        ticketArray[] memory userTickets = new ticketArray[](totalTickets);
        uint256 ticketIndex;
        ...
        for (uint256 i = 0; i < contracts.length; i++) {
            ...
            for (uint256 j = 0; j < ticketsByDraw.length; j++) {

                uint256 ticketFormatted = uint256(ticketsByDraw[j]);
                string memory identifier =  getLotteryIdentifier(contractAddress);

                ticketArray memory ticket = ticketArray(
                    identifier,
                    ticketFormatted
                );
    ...
    function getAllMyTicketsW(address user) external view returns (ticketArray[] memory) {

        address[] memory contracts = getContractsW(); // Get the list of lottery contract addresses
        uint256 totalTickets;
...
function getPastDrawTickets(address contractAddress, uint256 drawId, address user) external view returns (pastTicketArray[] memory) {
    LotteryContract contractInstance = LotteryContract(contractAddress);
    
    require(drawId < contractInstance.requestCounter(), "Draw fetched should be past draws!!");
    
    uint16[] memory ticketsByDraw = contractInstance.getUserTicketsByDraw(drawId, user);
    uint256 totalTickets = ticketsByDraw.length;

    pastTicketArray[] memory pastDrawTickets = new pastTicketArray[](totalTickets);
    uint256 ticketIndex;

    LotteryContract.Ticket memory winnerTicket = contractInstance.getJackpotWinnerByLotteryId(drawId);

    bool winner;
    
    string memory identifier =  getLotteryIdentifier(contractAddress);
    
    for (uint256 i = 0; i < ticketsByDraw.length; i++) {
        uint256 ticketFormatted = uint256(ticketsByDraw[i]);

            if(winnerTicket.id == ticketFormatted){
                winner = true;
            }else{
                winner = false;
            }

        pastTicketArray memory ticket = pastTicketArray(
            identifier,
            ticketFormatted,
            winner
        );

        pastDrawTickets[ticketIndex] = ticket;
        ticketIndex++;
    }

    return pastDrawTickets;
}

    function getContracts() internal view returns (address[] memory) {
        // Return the list of lottery contract addresses
        address[] memory contracts = new address[](4);
        contracts[0] = contractsAddresses[0];
        contracts[1] = contractsAddresses[1];
        contracts[2] = contractsAddresses[2];
        contracts[3] = contractsAddresses[3];
        return contracts;
    }
    
    function getContractsW() internal view returns (address[] memory) {
        // Return the list of lottery contract addresses
        address[] memory contracts = new address[](4);
        contracts[0] = contractsAddresses[4];
        contracts[1] = contractsAddresses[5];
        contracts[2] = contractsAddresses[6];
        contracts[3] = contractsAddresses[7];
        return contracts;
    }

    function getLotteryIdentifier(address contractAddress) internal view returns (string memory) {
        // Return the lottery identifier based on the contract address
        if (contractAddress == contractsAddresses[0] || contractAddress == contractsAddresses[4]) {
            return "Bitcoin";
        } else if (contractAddress == contractsAddresses[1] || contractAddress == contractsAddresses[5]) {
            return "Ethereum";
        } else if (contractAddress == contractsAddresses[2] || contractAddress == contractsAddresses[6]) {
            return "Matic";
        } else if (contractAddress == contractsAddresses[3] || contractAddress == contractsAddresses[7]) {
            return "KRSTM";
        } else {
