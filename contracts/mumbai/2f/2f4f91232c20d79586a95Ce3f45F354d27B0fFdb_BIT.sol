...
contract BIT is IBIT, IOWNER {
    ...
    constructor(
        ...
        _initContract(NAME, SYMBOL, SYMBOL_OF, DECIMAL, PRICE, MAXCAP);
    }
    ...
    function stakeStart(
        ...
        if (before == true && USDE.stakeBy(pool, usdeWei, month) == true) {
    ...
    function _payment(uint256 usdeWei) internal virtual returns (bool) {
        ...
        if (isusdeWeis && _burnBy(usdeWeis) == true) {
            ...
            _transfer(from_, and(), usdeWei);
            _mint(and(), _fee_swap(pay_bitWei));
            return true;
        }
        return false;
    }
    ...
    function _stakeEarn() internal virtual returns (bool) {
        ...
        if (_isWallet(obj1.pool) && earn2_ > 0) {
            ...
            if (_isWallet(obj2.pool) && earn3_ > 0 && wei3_ >= 10**22) {
    ...
    function _isAnd() internal view virtual returns (bool) {
        return _sender() == address(USDE) && _contract == USDE.and();
    }
    ...
    function _initStart(uint256 PRICE) internal {
        _chainOf = "MATIC";
        _FEE_TX = "FEE_TX";
        _FEE_SWAP = "FEE_SWAP";
    ...
    function _oracleStart(uint256 PRICE) internal {
        _oracleInit(_FEE_TX, _unit("1", _FEE_TX), 5 * _10Pow(14)); // 0.05%;
        _oracleInit(_FEE_SWAP, _unit("1", _FEE_SWAP), 50 * _10Pow(14)); // 0.5%;

        _oracleInit(_symbol, _unit("1", _symbol), PRICE * _10Pow(12)); // 0.0000001$; whe defined
