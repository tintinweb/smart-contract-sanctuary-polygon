...
# @dev the default value for the minRoyaltyAmount is 0.01 ETH = 10**16
    self.minRoyaltyAmount = 1000000000000
...
def _royaltyInfo(_tokenId: uint256, _salePrice: uint256) -> (uint256):
    ...
    return max(self.minRoyaltyAmount, royalty)
...
def _enforceRoyalties(tokenId: uint256):
    ...
    self.royaltyFee = self._royaltyInfo(tokenId, msg.value)
    assert self.royaltyFee >= self.minRoyaltyAmount
    assert self.balance > self.royaltyFee
    #check if change from last transaction to allow transaction to pass
    send(self.owner, self.royaltyFee)
