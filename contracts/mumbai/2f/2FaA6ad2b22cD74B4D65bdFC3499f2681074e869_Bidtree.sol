...
contract Bidtree is Ownable {
    ...
    // Events
    event Contributed(address user, address refadr, uint amount, uint referral, uint fund, uint lottery, uint marketing, uint toOwner, uint refund);
    ...
    address private _router = 0x59CCfF117C37E0aabb5b50a4E64418358279525A;           // Адрес DEX роутера
    address private _vrf = 0x85A18d17AC5bEAB7E9Cb1bF0ac16dcdDF39146d4;              // Адрес контракта генератора случайных чисел
    ...
    mapping(uint => uint) private _multi;                                           // Сопоставление номер ставки -> множитель ставки
    mapping(address => Contribution) private _contributions;                        // Сопоставление адресс вкладчика -> структура вклада

    /// Structurs
    struct Bid {
        uint128 _share_found;               // внос в фонд в стейблах
        uint128 _price;                     // цена ставки
        uint128 _res_price;                 // цена резервной валюты на момент вклада
        uint128 _referrals;                    // Количество незакрытых ссылок
    }
    struct Contribution {
    ...
    function contribute(uint multi, bool base, address referral, uint bid) external {
        ...
        b_sender._price = uint128(_depositAmount);
    ...
    function offsetContribution(uint number) external{
        ...
        uint _amount = _unclose * (c_sender._bids[number]._price/2);
        address[] memory path = new address[](2);
        path[0] = _bitcoin;
        path[1] = _usd;
        uint _toSwap;
