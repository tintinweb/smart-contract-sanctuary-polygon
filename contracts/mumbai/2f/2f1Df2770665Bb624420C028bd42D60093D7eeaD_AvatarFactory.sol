    ...
    IRandomRequester
{
    ...
    mapping(uint256 => uint256) internal _pirateTokenIdToPirateAvatarTokenId;

    mapping(bytes32 => address) internal mintRequests;
    mapping(address => uint256) internal mintRequestsPirateTokenId;
    mapping(address => string) internal mintRequestsUri;    

    event MintingAvatar(bytes32 indexed requestId, address indexed account);
    event AvatarCreated(address indexed account, uint256 indexed pirateAvatarTokenId, bool indexed isGovernor);
    ...
    function mintPirateAvatar(uint256 pirateTokenId, string memory uri) public {
        ...
        mintRequestsPirateTokenId[msg.sender] = pirateTokenId;
        mintRequestsUri[msg.sender] = uri;
    ...
    function randomnessFulfilled(uint256 randomness, bytes32 requestId) external override {
        ...
        address account = mintRequests[requestId];
        ...
        uint256 pirateAvatarTokenId = avatarContract.mint(account, isGovernor);
        
        _pirateTokenIdToPirateAvatarTokenId[mintRequestsPirateTokenId[account]] = pirateAvatarTokenId;

        pirateAvatarMetadataContract.addPirateAvatar(
            pirateAvatarTokenId,
            isGovernor,
            mintRequestsUri[account]);

        mintRequestsPirateTokenId[account] = 0;

        emit AvatarCreated(account, pirateAvatarTokenId, isGovernor);
    }
}
