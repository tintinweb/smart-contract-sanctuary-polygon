...
import "./MerkleClaimERC721.sol";

// import "@openzeppelin/contracts/cryptography/MerkleProof.sol";


contract YourCollectible is ERC721, Ownable, ERC721Enumerable, MerkleClaimERC721 {
  ...
  constructor(bytes32 _merkleRoot) ERC721("Arcadians", "ARC") MerkleClaimERC721(_merkleRoot) {
    _tokenIds.increment();
...
// import { ERC721 } from "https://github.com/Rari-Capital/solmate/blob/main/src/tokens/ERC721.sol"; // Solmate: ERC721
import "@openzeppelin/contracts/token/ERC721/ERC721.sol";
...
abstract contract MerkleClaimERC721 is ERC721 {
  ...
  /// @notice Thrown "AlreadyClaimed" if address has already claimed
  /// @notice Thrown "NotInMerkle" if address/amount are not part of Merkle tree
  ...
  /// @param tokenId of tokens claimed
  ...
  /// @param tokenId owed to claimee
  /// @param proof merkle proof to prove address and token id are in tree
  ...
  /// @notice Allows claiming tokens if address is part of merkle tree
  /// @param to address of claimee
  /// @param tokenIds owed to claimee
  /// @param proofs merkle proofs to prove address and token ids are in tree
  function claimBatch(address to, uint256[] calldata tokenIds, bytes32[][] calldata proofs) external {
    // Throw if address has already claimed tokens
    require(!hasClaimed[to], "AlreadyClaimed");
    for (uint256 i = 0; i < tokenIds.length; i++) {
      uint256 tokenId = tokenIds[i];
      bytes32[] calldata proof = proofs[i];
      this.claim(to, tokenId, proof);
    }
    hasClaimed[to] = true;
  }
}
