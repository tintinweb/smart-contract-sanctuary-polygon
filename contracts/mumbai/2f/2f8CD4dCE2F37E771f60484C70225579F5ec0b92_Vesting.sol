...
contract Vesting is Ownable, ReentrancyGuard {
    ...
    function claim(uint256 totalAmountOfUser, bytes32[] calldata merkleProof) external {
        ...
        if (!isStartAmountClaimed[msg.sender]) {
            _amount += claimStartAmount(totalAmountOfUser);
    ...
    function _computeReleasableAmount(address _account, uint256 totalAmountOfUser) internal view returns (uint256) {
        ...
        if (currentTime < vestingStartTime) {
            ...
            return
                totalAmountOfUser.mul((100 - startVestingPercentage)).div(100).sub(
                    userAddressToAmountClaimed[_account]
                );
        } else {
            uint256 timeFromStart = currentTime.sub(vestingStartTime); //1678422867-1678421336=1531
            uint256 secondsPerSlice = vestingSlicePeriodSeconds; //60
            uint256 vestedSlicePeriods = timeFromStart.div(secondsPerSlice); //1531/60=25
            uint256 vestedSeconds = vestedSlicePeriods.mul(secondsPerSlice); //25*60=1500
            uint256 vestedAmount = (totalAmountOfUser.mul(vestedSeconds).div(vestingEndTime - vestingStartTime))
                ...
                .div(100);
            // 10000000000000000000*1500/259200000*70/100

            uint256 startAmount = claimStartAmount(totalAmountOfUser);
            if (userAddressToAmountClaimed[_account] == 0) {
                vestedAmount = vestedAmount.sub(userAddressToAmountClaimed[_account]);
            } else {
                vestedAmount = vestedAmount.sub(userAddressToAmountClaimed[_account] - startAmount);
            }
            // vestedAmount = vestedAmount.sub(userAddressToAmountClaimed[_account]);
