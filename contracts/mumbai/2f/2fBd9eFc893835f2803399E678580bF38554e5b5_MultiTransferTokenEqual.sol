...
contract MultiTransferTokenEqual is Ownable, Pausable {
    ...
    // uint256 public ticketPrice = 1 * 10**18; //50 NTC Tokens per ticket
   ...
   */
    ...
    @param _token Address of the NTC token
   */
    function claimTicket(address _token, uint256 amount) external whenNotPaused {
        INestcoin token = INestcoin(_token);
        require(
        ...
        token.safeTransferFrom(msg.sender, owner(), amount *10**18);
        ticket[msg.sender]++;
    }
