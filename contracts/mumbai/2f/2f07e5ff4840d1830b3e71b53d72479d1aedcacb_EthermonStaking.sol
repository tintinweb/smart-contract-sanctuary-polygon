...
contract EthermonStaking is EthermonStakingBasic {
    ...
    function updateTokens(
        ...
        uint256 sumTeamPower = (stakingData.SumTeamPower() > 1)
            ...
            : 1; // if (currentTime >= data.endTime)
        //     data.lastCalled = currentTime = data.endTime;
        ...
        uint256 hourlyEmon = (data.teamPower / sumTeamPower) *
            stakingData.EmonPerPeriod() *
            timeElapsed *
            // _level *
            // rarity *
        ...
        data.balance += (hourlyEmon * 10**decimal);
    ...
    function withdrawRewards(uint256 _lockId) external {
        ...
        TokenData memory data = stakingData.getTokenDataTup(_lockId);
        require(now >= data.endTime, "Time remaining to unstake");
        ...
        emit Deposite(data.owner, data.balance, data.pfpId, data.monId, data.emons);
