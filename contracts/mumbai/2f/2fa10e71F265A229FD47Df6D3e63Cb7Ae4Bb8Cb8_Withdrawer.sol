...
contract Withdrawer is ReentrancyGuard, Ownable {
    ...
    /// @notice only Authorised address can call most of the functions
    function setAuthorized(address _address, bool _value) external onlyOwner {
    ...
    function claimNFTback(
        ...
        if (ercStandard) {
            ...
            uint tokenBalance = IERC1155(tokenContract).balanceOf(address(this), tokenId);
            IERC1155(tokenContract).safeTransferFrom(address(this), tokenOwner, tokenId, 1, "0x0");
            require(
                (tokenBalance - 1) == IERC1155(tokenContract).balanceOf(address(this), tokenId),
                "Invalid Owner"
            );
        }
    ...
    function storeNFT(
        ...
        if (ercStandard) {
            ...
            uint tokenBalance = IERC1155(tokenContract).balanceOf(address(this), tokenId);
            IERC1155(tokenContract).safeTransferFrom(tokenOwner, address(this), tokenId, 1, "0x0");
            require(
                (tokenBalance + 1) == IERC1155(tokenContract).balanceOf(address(this), tokenId),
                "Invalid Owner"
            );
