  ...
  ReentrancyGuardUpgradeable
{
  ...
  // Record whether the user has mint，
  mapping(address => bool) public hasNoMint;
  ...
  // Check if the address has been minted
  function checkNoMint(address _address) public view returns (bool) {
     return hasNoMint[_address];
  }
  ...
  function mint(address toAddress) public nonReentrant onlyReceivingAddress returns (uint256 tokenId) {
    require(address(erc20) != address(0), "FixPrice: erc20 address is null.");
    require(!hasNoMint[address(toAddress)], "The address has already been minted");
    ...
    _safeMint(toAddress, tokenId); // mint PASS to user address
    hasNoMint[toAddress] = true;   // record that the address has been minted
    emit Mint(toAddress, tokenId);

    tokenIdTracker.increment(); // automate token id increment
  }

  function mintEth(address toAddress) public payable nonReentrant onlyReceivingAddress returns (uint256 tokenId) {
  ...
  modifier onlyReceivingAddress() {
    require(receivingAddress == msg.sender, "ReceivingAddress: caller is not the receiving address");
    _;
  }
