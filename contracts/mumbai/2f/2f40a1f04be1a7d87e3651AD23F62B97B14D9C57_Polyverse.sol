...
contract Polyverse {
    ...
    function addCreator(
        string memory _creatorData,
        uint _subscriptionFee
    ) public {
        _creatorIds.increment();
        uint256 creatorId = _creatorIds.current();
        Creator storage newCreator = _creators[_creatorIds.current()];
        address[] memory emptyAddressArray;
        newCreator.creatorData = _creatorData;
        newCreator.subscriptionFee = _subscriptionFee;
        newCreator.subscribers = emptyAddressArray;
        newCreator.balance = 0;
        //_creators[creatorId] = Creator(creatorData, subscriptionFee, emptyAddressArray, 0);

        _addressToCreatorId[msg.sender] = creatorId;

        emit CreatorAdded(creatorId, _subscriptionFee);
    }
    ...
    function listEvent(
        ...
        IPUSHCommInterface(EPNS_COMM_ADDRESS).sendNotification(
           0x4946a945aB5d49fa59E81A00Bb9d6677E0fcB8B4, // from channel
            ...
            bytes(
                string(
                    ...
                    abi.encodePacked(
                        ...
                        "+", // segregator
                        "A new Ticket been listed", // this is notificaiton title
                        "+", // segregator
                        addressToString(msg.sender),
                        "Just listed a new ticket", // notification body
                        " Polyverse to you!" // notification body
                    )
                )
            )
        );
    ...
    function purchaseTicket(uint256 eventId) public payable {
        ...
        IERC20(tokenAddress).approve(msg.sender, eventDetails.ticketPrice);
        ...
        uint256 amount = eventDetails.ticketPrice;

        //"0+3+Hooray! ", msg.sender, " sent ", token amount, " PUSH to you!"
        IPUSHCommInterface(EPNS_COMM_ADDRESS).sendNotification(
           0x4946a945aB5d49fa59E81A00Bb9d6677E0fcB8B4, // from channel
            eventDetails.owner, // to recipient, put address(this) in case you want Broadcast or Subset. For Targetted put the address to which you want to send
            bytes(
                string(
                    // We are passing identity here: https://docs.epns.io/developers/developer-guides/sending-notifications/advanced/notification-payload-types/identity/payload-identity-implementations
                    abi.encodePacked(
                        ...
                        "3", // this is payload type: https://docs.epns.io/developers/developer-guides/sending-notifications/advanced/notification-payload-types/payload (1, 3 or 4) = (Broadcast, targetted or subset)
                        "+", // segregator
                        "New Ticket sale", // this is notificaiton title
                        "+", // segregator
                        "You've got a new Ticket sale", // notification body
                        addressToString(msg.sender), // notification body
                        " Purchased a ticket ", // notification body
                        uint2str(amount), // notification body
    ...
    function getAllCreator() external view returns (Creator[] memory) {
        return allCreator;
    }

    function getAllEvent() external view returns (Event[] memory) {
        return allEvent;
    }

    function getAllTicket() external view returns (Ticket[] memory) {
        return allTickets;
    }
