...
import './CredentialingContract.sol';
contract Coupon is ERC721, Ownable{
    ...
    */
    ...
    for( uint i=0; i < allCouponStrings.length;i++){
        if(block.timestamp > couponInformation[showCouponStringAndCouponId[allCouponStrings[i]]]._startDate && 
            block.timestamp < couponInformation[showCouponStringAndCouponId[allCouponStrings[i]]]._endDate){
                livecouponCount++;
            }
    ...
    for(uint i = 0; i < allCouponStrings.length; i++){
        ...
        string memory couponString = allCouponStrings[i];
        uint256 couponId = showCouponStringAndCouponId[couponString];

            if(block.timestamp > couponInformation[couponId]._startDate && block.timestamp < couponInformation[couponId]._endDate){
