...
contract Commu is Ownable {
    ...
    mapping (address => uint256) public getMaxPendingBonus;
    ...
    function getPrincipleAmount(address _userAddress) public view returns(uint256){
        // User memory user = users[_userAddress];
        // require(address(this).balance >= users[_userAddress].deposits[0].depositAmount, "Contract balance is insufficient");
        //require
        uint256 PricipalAmount;
        if (users[_userAddress].deposits[0].depositedTimeStamp <= users[_userAddress].deposits[0].depositedTimeStamp + 20 minutes && userTeams[_userAddress].length <= 5){
            PricipalAmount = users[_userAddress].deposits[0].depositAmount;
            // IERC20(USDTAddress).transfer(_userAddress, users[_userAddress].deposits[0].depositAmount);
        }
    ...
    function getUserTotalROI(address _userAddress) public view returns(uint256){
        ...
        for(uint8 i = 0; i < users[_userAddress].deposits.length; i++) {
            ...
            if(reward >= users[_userAddress].deposits[i].maxRewardLimit.sub(getUserTotalLevelIncome(_userAddress))) {
                reward = users[_userAddress].deposits[i].maxRewardLimit.sub(getUserTotalLevelIncome(_userAddress));
            }
            totalReward = totalReward + reward; 
            // getMaxPendingBonus[_userAddress] = users[_userAddress].deposits[i].maxRewardLimit.sub(getUserTotalLevelIncome(_userAddress));
        }
    ...
    function withdraw(address _userAddress, uint256 _amount) public {
        uint256 getTotalWithdrwalAmount =balanceAfterWithdrawal(_userAddress).add(getPrincipleAmount(_userAddress));
        require(msg.sender == _userAddress, "Caller should be the owner of withdraw account");
        require(getTotalWithdrwalAmount >= _amount,"Amount is greater than withdrawal balance amount");
        require(_amount >= 10000000,"The Amount should be greater than 10 USDT");
    ...
    function getUserTotalEarning(address _userAddress) public view returns(uint256){
        return getUserTotalReturns(_userAddress).add(getPrincipleAmount(_userAddress));
    }
    ...
    function getUserPendingBonus(address _userAddress) public view returns(uint256){
        uint256 maxLimit = 2000;
        return (getUserTotalDeposit(_userAddress).mul(maxLimit).div(PERCENTS_DIVIDER)).sub(getUserTotalROI(_userAddress));
    }
