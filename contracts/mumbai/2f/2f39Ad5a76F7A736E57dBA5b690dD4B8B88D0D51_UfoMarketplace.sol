    ...
    ReentrancyGuardUpgradeable
{
    ...
    event AuctionSuccessful(address indexed _nftAddress, uint256 indexed _tokenId, uint256 _totalPrice, address _winner); //_totalPrice = actual Price + platform fee
    ...
    function buySellItem(
        ...
        require(_checkBuyNFTSignature(v, r, s, _info), Errors.SIGNATURE_IS_WRONG);
        ...
        crypto.safeTransferFrom(msg.sender, _info.seller, sellerProceeds);
        crypto.safeTransferFrom(msg.sender, address(this), fee);
        _transfer(_info.nftContract, _info.seller, msg.sender, _info.nftId);
        if (keccak256(abi.encodePacked(_info.sellType)) == keccak256(abi.encodePacked('fixed-listing'))) {
            emit FixedItemSuccessful(_info.nftContract, _info.nftId, _info.price, msg.sender);
        } else {
            emit AuctionSuccessful(_info.nftContract, _info.nftId, _info.price, msg.sender);
        }
    }
    ...
    function claimUAP(
        ...
        require(_checkClaimUAPSignature(v, r, s, _info), Errors.SIGNATURE_IS_WRONG);
        uapContract.mint(_info.user, _info.amount);
    ...
    function withdrawWETH(address addr) external onlyOwner {
        IERC20Upgradeable crypto = IERC20Upgradeable(weth_addr);
        uint256 balance = crypto.balanceOf(address(this));
        crypto.safeTransfer(addr, balance);
    }
}
...
abstract contract EIP712Signature {
    ...
    function _checkClaimUAPSignature(
        ...
        bytes32 hashStruct = keccak256(
            abi.encode(keccak256('UAPClaimInfo(uint256 amount,address user,uint256 salt)'), info.amount, info.user, info.salt)
        );
