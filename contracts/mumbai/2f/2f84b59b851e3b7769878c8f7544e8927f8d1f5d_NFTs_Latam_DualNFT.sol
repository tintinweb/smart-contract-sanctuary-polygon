...
interface ITokenERC721 is IERC721Upgradeable {
     ...
     *  @param onchain The Data to asing to the onchain NFT.
     *
...
contract NFTs_Latam_DualNFT is
    Initializable,
    INFTlatamContract,
    IOwnable,
    IRoyalty,
    ...
    ITokenERC721
{
    ...
    bytes32 private constant MODULE_TYPE = bytes32("NFTs_Latam_DualNFT");
        ...
        initializer
    {
        ...
        string memory _name = "EtherTulip V2 Concept";
        string memory _symbol = "DUAL";
        ...
        __EIP712_init("NFTs_Latam_DualNFT", "2");
    ...
    // Función auxiliar para convertir un uint256 a una cadena de texto
        ...
        returns (string memory)
    {
        ...
        //bytes memory base64data = onchain[_tokenId];

        //string memory json = string(abi.encodePacked('{"name": "', name(), '#', toString(_tokenId), '", "description": "Tulip generated and stored on chain.", "image": "data:image/png;base64,', onchain[_tokenId], '"}'));
                ...
                abi.encodePacked(
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                '{"name": "',
                                "Tuinier ", uint256ToString(_tokenId+1),  
                                '", "description":',
                                '"This concept of Dual NFT was developed by NFTsLATAM as a proposal for the V2 of EtherTulip. This NFT has unique characteristics as it features both on-chain and off-chain art. It is the first NFT that displays two images under the same Token ID and offers the best of both worlds: a complex JPEG stored on a centralized network and an everlasting PFP that exists on the blockchain. Additionally, the tulip has a mechanism that changes the background depending on whether it is day or night in the Netherlands."',
                                ', "image": "',
                                onchain[_tokenId],
                                '", "attributes": [',
                                '{ "trait_type": "Tulip",                  "value": "Orange" },',
                                '{ "trait_type": "Background",              "value": "Windmill" },',
                                '{ "trait_type": "Tuinier",     "value": "30" },',
                                '{ "trait_type": "Veld",     "value": "Orange" }',
                                ']}'
                            )
                        )
                    )
    ...
    function mintTo(
        ...
        return _mintTo(_to, _uri, _onchain);
    }
    ...
    function mintWithSignature(
        ...
        tokenIdMinted = _mintTo(receiver, _req.uri, _req.onchain);
