...
contract LicensTransactionContract is Ownable, ReentrancyGuard, ERC2771Recipient{
    ...
    mapping(address => bool) public paperWallets;

    mapping(uint256 => RoyaltyPayout) public royaltyDist;
    uint[] public licensesMinted;
    ...
    modifier onlyPaper(address _sender){
        require(paperWallets[_sender], "Only Paper wallet can call this function");
        _;
    }
    ...
    constructor(address _accountContract, 
        ...
        paperWallets[0x7754B94345BCE520f8dd4F6a5642567603e90E10] = true;//Paper wallet on testnet
    }
    ...
    function addPaperWallet(address[] memory _addresses) external onlyOwner{
        for(uint i; i < _addresses.length;i++){
            paperWallets[_addresses[i]] = true;
        }
    }

    function removePaperWallet(address[] memory _addresses) external onlyOwner{
        for(uint i; i < _addresses.length;i++){
            paperWallets[_addresses[i]] = false;
        }
    }
    ...
    function license(uint256 _listingId, address _to, string calldata _transactionId, bool _exclusivePurchase) public nonReentrant {
        ...
        uint[] memory _paidNet = new uint[](listing.royaltyAddresses.length);
        ...
        if(usdc.balanceOf(address(this)) >= afterTax){
            for(uint64 i; i < listing.royaltyAddresses.length; i++){
                usdc.transfer(listing.royaltyAddresses[i], afterTax * listing.royaltyShares[i] / 100);
                royaltyDist[licenseNFTTokenId].paidNet[i] = afterTax * listing.royaltyShares[i] / 100;
            }
            royaltyDist[licenseNFTTokenId].paid = true;
            royaltyDist[licenseNFTTokenId].paidTimestamp = block.timestamp;
            emit RoyaltyPaid(royaltyDist[licenseNFTTokenId]);
        }
        licensesMinted.push(licenseNFTTokenId);
        emit LicenseIssued(_listingId, licenseNFTTokenId, _transactionId, _to, listing.licenseCid, _exclusivePurchase);
    }

    function mint(uint256 _listingId, address _to, string calldata _transactionId, bool _exclusivePurchase) external nonReentrant onlyPaper(msg.sender) {
        license(_listingId, _to, _transactionId, _exclusivePurchase);
    }

    function payUnpaidRoyaltiesByLicense(uint256[] calldata _licenseIds) public onlyOwner {
        for (uint256 i = 0; i < _licenseIds.length; i++) {
            royaltiesPayment(_licenseIds[i]);
        }
    }
    function payUnpaidRoyaltiesByListing(uint256 _listingId) external onlyOwner {
        for(uint256 i = 0; i < licensesMinted.length; i++){
            RoyaltyPayout memory royaltyPayout = royaltyDist[licensesMinted[i]];
            if(!royaltyPayout.paid && royaltyPayout.listingId == _listingId){
                royaltiesPayment(royaltyPayout.licenseId);
            }
        }
    }
    function getPayoutOfListing(uint256 _listingId) public view returns(uint256 _paid, uint256 _unpaid){
        for(uint256 i = 0; i < licensesMinted.length; i++){
            RoyaltyPayout memory royaltyPayout = royaltyDist[licensesMinted[i]];
            if(royaltyPayout.listingId == _listingId){
                if(royaltyPayout.paid){
                    _paid += royaltyPayout.afterTax;
                }
                else{
                    _unpaid += royaltyPayout.afterTax;
                }
            }
        }
    }
    ...
    function royaltiesPayment(uint256 _licenseId) internal nonReentrant onlyOwner {
        ...
        for (uint256 i = 0; i < payout.royaltyAddresses.length; i++) {
            if (payout.royaltyAddresses[i] != address(0) && payout.royaltyShares[i] != 0) {
                usdc.transfer(payout.royaltyAddresses[i], payout.royaltyShares[i] * payout.afterTax / 100);
                payout.paidNet[i] = payout.royaltyShares[i] * payout.afterTax / 100;
            }
        }
