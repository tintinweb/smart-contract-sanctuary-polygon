...
contract PixelMap is ERC721URIStorage, Ownable {
    ...
    mapping(uint256 => address) boughtOwner;
    ...
    modifier validBlock(uint256 _x, uint256 _y) {
        require(_x < ROW_COUNT && _y < COL_COUNT, "Out of board");
        _;
    }

    function buy(uint256 _x, uint256 _y) external validBlock(_x, _y) {
        uint256 blockId = _x * 100 + _y;
        require(boughtOwner[blockId] == address(0), "Already sold");
        ...
        boughtOwner[blockId] = msg.sender;
    ...
    function mint(
        ...
        uint256 blockId = _x * 100 + _y;
        require(boughtOwner[blockId] == msg.sender, "Not owner");

        _mint(msg.sender, blockId);
        _setTokenURI(blockId, _tokenURI);

        emit Mint(msg.sender, _x, _y, _tokenURI);
    }
    ...
    function isMinted(uint256 _x, uint256 _y)
        public
        view
        validBlock(_x, _y)
        returns (bool)
    {
        uint256 blockId = _x * 100 + _y;
        return _ownerOf(blockId) != address(0);
    }
    ...
    {
        address owner = _ownerOf(tokenId);
        return owner == address(0) ? boughtOwner[tokenId] : owner;
    }
        ...
        )
    {
        ...
        bool blockMinted = isMinted(_x, _y);
        require(blockOwner != address(0), "No owner yet");
        return (blockOwner, blockMinted, blockMinted ? tokenURI(blockId) : "");
    }
    ...
    function setCostPerBlock(uint256 _costPerBlock) external onlyOwner {
        costPerBlock = _costPerBlock;
    }

    function setCostForMint(uint256 _costForMint) external onlyOwner {
        costForMint = _costForMint;
    }
