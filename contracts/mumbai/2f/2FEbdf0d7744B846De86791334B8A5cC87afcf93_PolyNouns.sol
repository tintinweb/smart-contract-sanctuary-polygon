...
contract PolyNouns is ReentrancyGuard, NounsAbstract {
    ...
    uint256 public constant MINTS_PER_TIER = 50; // free mints
    // max per mint
    ...
    function currentTokenCost() public view returns (uint256) {
        ...
        if (_totalSupply <= MINTS_PER_TIER) return 0;
        if (_totalSupply > MINTS_PER_TIER && _totalSupply <= 300)
    ...
    function mintPolyNoun() external nonReentrant {
        ...
        if (totalSupply() < MINTS_PER_TIER) return _mintInternal();
    ...
    function _mintInternal() internal {
        ...
        require(balanceOf(recipient) < maxPerWallet, "Maximum mint limit");
        require(totalSupply() < MAX_SUPPLY, "Maximum mint limit");

        _mintLoop(recipient, 1);
    }
}