 ...
 */
contract DoubleDiceProtocol is
    ...
    ExtraStorageGap
{
    bool private __MultipleInheritanceOptimization__rootInitialized;
    uint256[50] private __MultipleInheritanceOptimization__gap;
    ...
    struct DoubleDiceProtocolInitParams {
    ...
    function initialize(DoubleDiceProtocolInitParams calldata params)
        external
        initializer
    {
    ...
    struct CreatedVirtualFloorParams {
        ...
        uint256 bonusAmount;
        uint8 nonzeroOutcomeCount;
        uint256 minCommitmentAmount;
        uint256 maxCommitmentAmount;
        address creator;
        bool isOneTime;
        IDoubleDiceApplication application;
    }
    ...
    function getVirtualFloorParams(uint256 vfId) public view returns (CreatedVirtualFloorParams memory) {
        ...
        return CreatedVirtualFloorParams({
            ...
            creator: vf.creator,
            nonzeroOutcomeCount: vf.nonzeroOutcomeCount,
            application: vf.application
        });
    }

    function getVirtualFloorApplication(uint256 vfId) public view returns (IDoubleDiceApplication) {
        VirtualFloor storage vf = _vfs[vfId];
        return vf.application;
    }
        ...
        whenNotPaused
    {
        ...
        vf.application = IDoubleDiceApplication(_msgSender());
        vf.isOneTime = params.isOneTime;
        ...
        if (params.bonusAmount > 0) {
            ...
            params.paymentToken.safeTransferFrom(params.creator, address(this), params.bonusAmount);
        }
        ...
        emitVirtualFloorCreation(params, min, max);
    }

    /**
     * @dev Extracted to own function to avoid "Stack too deep" CompilerError
     */
    function emitVirtualFloorCreation(VirtualFloorCreationParams calldata params, uint256 min, uint256 max) private {
        // Extracting these values to local variables
        // averts a "Stack too deep" CompilerError in the
        // subsequent `emit`
        EncodedVirtualFloorMetadata calldata metadata = params.metadata;
        IDoubleDiceApplication application = IDoubleDiceApplication(_msgSender());
     ...
     */
    function commitToVirtualFloor(uint256 vfId, address committer, uint8 outcomeIndex, uint256 amount, uint256 optionalDeadline)
        external
        onlyRole(APPLICATION_ROLE)
        whenNotPaused
        nonReentrant
    {
        ...
        if (!(vfId.isValidVirtualFloorId())) revert UnsupportedLegacyVf();

        VirtualFloor storage vf = _vfs[vfId];

        if (!vf.isOpen()) revert WrongVirtualFloorState(vf.state());

        if (!(_msgSender() == address(vf.application))) revert UnauthorizedMsgSender();
        ...
        vf.paymentToken.safeTransferFrom(committer, address(this), amount);
        ...
        UFixed32x6 beta = vf.application.betaOf(vfId, outcomeIndex, timestamp);
        ...
        outcomeTotals.amountTimesBeta_e18 = outcomeTotals.amountTimesBeta_e18.add(beta.toUFixed256x18().mul0(amount));

        uint256 tokenId = ERC1155TokenIds.vfOutcomeBetaIdOf(vfId, outcomeIndex, beta);
        ...
        emit UserCommitment({
            ...
            timeslot: timestamp, // ToDo: Deprecate
            amount: amount,
            beta_e18: beta.toUFixed256x18(),
            tokenId: tokenId
        });
        _mint({
            to: committer,
    ...
    /**
     * @dev Legacy: No longer emitted, but kept here so that graph-codegen can still be aware
     * of this event, which has been emitted in the past.
     */
    ...
    // New
    event VirtualFloorResolutions(
        uint256 indexed vfId,
        uint256 winningOutcomeIndexFlags,
        VirtualFloorResolutionType resolutionType,
        uint256 winnerProfits,
        uint256 protocolFeeAmount,
        uint256 creatorFeeAmount
     ...
     */
    function resolve(uint256 vfId, uint256 winningOutcomeIndexFlags, address creatorFeeBeneficiary) external {
        ...
        uint256 nWinningOutcomes = 0;
        uint256 bonusPlusAllOutcomesTotalAmount = vf.bonusAmount;
        uint256 winningOutcomesTotalAmount = 0;
        UFixed256x18 winningOutcomesTotalAmountTimesBeta = UFIXED256X18_ZERO;
        {
            uint256 winningOutcomeIndexFlagsCopy = winningOutcomeIndexFlags;
            uint8 singleWinningOutcomeIndex;
            for (uint8 i = 0; i < vf.nOutcomes; i++) {
                bonusPlusAllOutcomesTotalAmount += vf.outcomeTotals[i].amount;
                if (winningOutcomeIndexFlagsCopy & 0x01 != 0) {
                    winningOutcomesTotalAmount += vf.outcomeTotals[i].amount;
                    winningOutcomesTotalAmountTimesBeta = winningOutcomesTotalAmountTimesBeta.add(vf.outcomeTotals[i].amountTimesBeta_e18);
                    singleWinningOutcomeIndex = i;
                    nWinningOutcomes += 1;
                }
                winningOutcomeIndexFlagsCopy >>= 1;
            }

            // There should be 1 or more winning outcomes, but there cannot be 0
            if (!(nWinningOutcomes >= 1)) revert OutcomeIndexOutOfRange();

            // After iterating through all the nOutcomes bit positions,
            // we should be left with 0.
            // Otherwise, it means that other higher bits were set.
            ...
            if (!(nWinningOutcomes < vf.nOutcomes)) revert TooManyWinningOutcomes();

            vf.winningOutcomeIndex = nWinningOutcomes == 1 ? singleWinningOutcomeIndex : 0xff;
            vf.winningOutcomeIndexFlags = winningOutcomeIndexFlags;
        }
        ...
        if (winningOutcomesTotalAmount == 0) {
            ...
            uint256 maxTotalFeeAmount = vf.totalFeeRate.toUFixed256x18().mul0(bonusPlusAllOutcomesTotalAmount).floorToUint256();

            // If needs be, limit the fee to ensure that there enough funds to be able to refund winner commitments in full.
            uint256 totalFeePlusTotalWinnerProfits = bonusPlusAllOutcomesTotalAmount - winningOutcomesTotalAmount;
            ...
            vf.winningOutcomesTotalAmountTimesBeta = winningOutcomesTotalAmountTimesBeta;
        ...
        emit VirtualFloorResolutions({
            vfId: vfId,
            winningOutcomeIndexFlags: winningOutcomeIndexFlags,
        ...
        whenNotPaused
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            (uint256 extractedVfId, /*outcomeIndex*/, /*beta*/) = tokenId.destructure();
        ...
        whenNotPaused
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            (uint256 extractedVfId, uint8 outcomeIndex, UFixed32x6 beta_e6) = tokenId.destructure();
            if (!(extractedVfId == vfId)) revert MismatchedVirtualFloorId(tokenId);
            uint256 amount = _balances[tokenId][msgSender];
            amounts[i] = amount;
            _balances[tokenId][msgSender] = 0;
            if (outcomeIndex == winningOutcomeIndex) {
                UFixed256x18 beta = beta_e6.toUFixed256x18();
     ...
     * If a tokenId is included multiple times, it will count only once.
     */
    function legacyClaimPayouts(uint256 vfId, uint256[] calldata tokenIds)
        public
        whenNotPaused
    {
        VirtualFloor storage vf = _vfs[vfId];
        ...
        uint256[] memory amounts = new uint256[](tokenIds.length);
        uint8 winningOutcomeIndex = vf.winningOutcomeIndex;
        UFixed256x18 winningOutcomeTotalAmountTimesBeta = vf.winningOutcomesTotalAmountTimesBeta;
        uint256 totalWinnerProfits = vf.winnerProfits;
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            (uint256 extractedVfId, uint8 outcomeIndex, uint32 timeslot) = tokenId.legacyDestructure();
            if (!(extractedVfId == vfId)) revert MismatchedVirtualFloorId(tokenId);
            uint256 amount = _balances[tokenId][msgSender];
            amounts[i] = amount;
            _balances[tokenId][msgSender] = 0;
            if (outcomeIndex == winningOutcomeIndex) {
                UFixed256x18 beta = vf.legacyBetaOf(timeslot);
                UFixed256x18 amountTimesBeta = beta.mul0(amount);
                uint256 profit = amountTimesBeta.mul0(totalWinnerProfits).divToUint256(winningOutcomeTotalAmountTimesBeta);
                totalPayout += amount + profit;
            }
    ...
    // ----------🎲 Lifecycle: VF lifecycle hooks 🎲----------

    function _onVirtualFloorConclusion(uint256 vfId) private {
        _vfs[vfId].application.onVirtualFloorConclusion(vfId);
    }
    ...
    // ----------🎲🎲 Legacy: ChallengeableCreatorOracle V1 🎲🎲----------

    IERC20MetadataUpgradeable private bondUsdErc20Token;

    mapping(uint256 => Resolution) public legacyResolutions;


    // ----------🎲🎲 Legacy: CreationQuotas V1 🎲🎲----------

    /**
     * @dev If contract is upgraded to V2 halfway through a VF's lifecycle,
     * when VF is concluded, quota will be restituted to new contract.
     */
    mapping(address => uint256) public legacyCreationQuotas;
...
struct VirtualFloorCreationParams {
    ...
    /**
     * @notice Premium status of the Virtual Floor.
     */
    bool isPremium;

    /**
     * @notice Premium fee amount that users pay
     */
    uint256 premiumFeeAmount;


    /**
     * @notice Onetime status of the Virtual Floor.
     */
    bool isOneTime;

    /**
     * @notice Onetime fee amount that users pay
     */
    uint256 oneTimeFeeAmount;
...
/**
 * @dev Not 256, because nOutcomes is stored in in a uint8,
 * and not 255, because 255 is used a special code to signal "multiple winning outcomes"
 */
uint256 constant _MAX_OUTCOMES_PER_VIRTUAL_FLOOR = 254;
...
struct VirtualFloor {
    ...
    bool isOneTime;                           // +  1 byte  ; one time or not
                                              // = 32 bytes => packed into 1 32-byte slot
    ...
    OutcomeTotals[255] outcomeTotals;
    ...
    // Storage slot 8
    UFixed256x18 winningOutcomesTotalAmountTimesBeta;
}
...
library VirtualFloors {
     ...
     */
    function legacyBetaOf(VirtualFloor storage vf, uint256 timestamp) internal view returns (UFixed256x18) {
        uint256 timeslot = MathUpgradeable.max(vf.tOpen, timestamp);
        UFixed256x18 betaOpenMinusBetaClose = vf.betaOpenMinusBetaClose.toUFixed256x18();
        return _BETA_CLOSE.add(betaOpenMinusBetaClose.mul0(vf.tClose - timeslot).div0(vf.tClose - vf.tOpen));
    }
