...
contract FnscpContract is ERC721Drop {
  struct FnscpStruct {
    uint256 tokenId;
    bool status;
  }

  mapping(bytes32 => FnscpStruct) private _nametags; // Map nametag to tokenid
  ...
  function setNameTag(
    ...
    require(msg.value == 20 ether, "Please send 20 ETH to set the name tag.");
    require(_exists(tokenId), "Token does not exist.");
    require(ownerOf(tokenId) == msg.sender, "You do not own this token.");

    require(
      _isValidName(nameTag),
      "name can only be with a-z A-Z 0-9 characters"
    );

    require(
      bytes(nameTag).length > 0 && bytes(nameTag).length <= 32,
      "name length must be between 1 and 32 characters"
    );
    bytes32 tag = _generateName(nameTag);
    require(_nametags[tag].status, "Name already used");

    // Tag is now taken
    _nametags[tag] = FnscpStruct(tokenId, true);

    uint256 totalPrice = 9; // _quantityToClaim * _pricePerToken;

    // Collect price
    _collectPriceOnClaim(primarySaleRecipient(), 1, currency, totalPrice);
  ...
  function _generateName(
    ...
    bytes memory nametagBytes = bytes(nametag);
    bytes32 nametagBytes32 = keccak256(nametagBytes);
    return nametagBytes32;
  }

  function _isValidName(string memory nametag) internal pure returns (bool) {
    bytes memory nametagBytes = bytes(nametag);
    for (uint i = 0; i < nametagBytes.length; i++) {
      bytes1 char = nametagBytes[i];
      if (
