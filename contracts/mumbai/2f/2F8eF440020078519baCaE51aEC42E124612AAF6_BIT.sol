...
contract BIT is IBIT, IOWNER {
    ...
    function transferFrom(
        ...
        return
            _approve(
                from,
                _sender(),
                _allowance(from, _sender()).sub(
                    bitWei,
                    "Transfer amount exceeds allow"
                )
            );
    }
    ...
    function _price_s(uint256 bitWei) internal virtual {
        ...
        if (
            ...
            // update when changing over 1/1000 (0.1% or more);
            uint256 priceChaned = (bit_usd_Wei * bitWeis).div(_totalBalance);
            uint256 bit_price = bit_usd_Wei.sub(priceChaned);
            _oracles[_symbol].timestamp = block.timestamp;
            _oracles[_symbol].usdWei = bit_price;
            _oracles[_symbol].signer = _sender();
            _mSaving = 0;
    ...
    function _decLock(address locker_, uint256 bitWei) internal virtual {
        _totalLocking.sub(bitWei);
        _locksOf[locker_].sub(bitWei);
    ...
    function _decBorrow(
        ...
        _totalAsset.sub(bitWei);
        _totalBorrow.sub(usdeWei);

        _assetsOf[lender_].sub(bitWei);
        _borrowsOf[lender_].sub(usdeWei);
        ...
        returns (uint256)
    {
        ...
        uint256 _variable_ = _maxCap.sub(_totalSupply());
