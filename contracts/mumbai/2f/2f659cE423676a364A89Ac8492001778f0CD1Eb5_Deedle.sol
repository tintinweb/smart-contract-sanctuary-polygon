...
import "./ERC5192/IERC5192Upgradeable.sol";

contract Deedle is ONFT721Upgradeable, IERC5192Upgradeable {
    using BitMapsUpgradeable for BitMapsUpgradeable.BitMap;

    BitMapsUpgradeable.BitMap private _locks;
    ...
    modifier notLocked(uint256 tokenId) {
        require(!_locks.get(tokenId), "ERC5192: Token is locked");
        _;
    }
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view override(ONFT721Upgradeable, IERC165Upgradeable) returns(bool) {
        return ONFT721Upgradeable.supportsInterface(interfaceId) || interfaceId == type(IERC5192Upgradeable).interfaceId;
    }
    ...
    function approve(
        address operator,
        uint256 tokenId
    ) public override(ERC721Upgradeable, IERC721Upgradeable) notLocked(tokenId) {
    ...
    function _unlock(uint256 tokenId) internal {
        require(_locks.get(tokenId), 'ERC5192: Token is unlocked');

        _locks.unset(tokenId);
