...
abstract contract BaseJarvixTokenDirectSale is CurrencyHandler, PriceHandlerProxy, SecuredProcessProxy,
    ...
    function getTokenAddress(bytes32 currency) public view returns (address) {
        return ProxyHub(proxyHubAddress).findProxyAddress(bytes4(currency));
    }
    /**
    ...
    function setTokenAddress(bytes32 currency, address tokenAddress) public onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        _setProxy(bytes4(currency), tokenAddress, getTokenInterfaceId(currency), true, true, false);
    }
...
abstract contract RoyaltyImplementerProxy is ProxyDiamond, Ownable, IERC2981, RoyaltiesV2, DefaultOperatorFilterer {
    ...
    function getRoyaltyHandler() internal view returns(RoyaltyHandler) {
        return RoyaltyHandler(getProxy(type(IRoyalty).interfaceId/*IRoyaltyInterfaceId*/));
    }
    /**
    ...
    function _setRoyaltyHandlerProxy(address royaltyHandlerAddress_) virtual internal {
        _setProxy(type(IRoyalty).interfaceId/*IRoyaltyInterfaceId*/, royaltyHandlerAddress_, type(IRoyalty).interfaceId/*IRoyaltyInterfaceId*/, false, true, true);
    }
...
abstract contract TokenDataHandlerProxy is ProxyDiamond {
    ...
    function getTokenDataHandler() internal view returns(TokenDataHandler) {
        return TokenDataHandler(getProxy(type(ITokenData).interfaceId/*ITokenDataInterfaceId*/));
    }
    function _setTokenDataHandlerProxy(address tokenDataHandlerAddress_) virtual internal {
        _setProxy(type(ITokenData).interfaceId/*ITokenDataInterfaceId*/, tokenDataHandlerAddress_, type(ITokenData).interfaceId/*ITokenDataInterfaceId*/, false, true, true);
    }
}
