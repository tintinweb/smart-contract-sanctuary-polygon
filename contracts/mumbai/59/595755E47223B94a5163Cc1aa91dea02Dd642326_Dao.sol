...
contract Dao {
    ...
    constructor(){
        owner = msg.sender;
        nextProposal = 1;
        daoContract = IdaoContract(0xF431DdAE3e00945C3058d33233e8C10951AF05EE);
        validTokens = [1];
    }
    ...
    function createProposal(string memory _description) public {
        ...
        for(uint256 i = 0; i < validTokens.length; i++){
            newProposal.canVote[i] = daoContract.ownerOf(validTokens[i]);
        }
        newProposal.maxVotes = newProposal.canVote.length;

        emit proposalCreated(nextProposal, _description, newProposal.canVote.length, msg.sender);
        nextProposal++;
    }
