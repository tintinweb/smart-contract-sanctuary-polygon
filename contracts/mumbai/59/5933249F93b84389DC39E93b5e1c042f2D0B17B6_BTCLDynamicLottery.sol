...
contract BTCLDynamicLottery is AutomationCompatible, AutomatedRandomness, ReentrancyGuard {
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomness) internal override {
        uint256 numberOfRoundsToDraw = 0;
        for (uint256 i = round; i < round + maxRounds; i++) { // Limiting to draw a maximum of 10 rounds
            if (rounds[i].status.requestId == requestId && rounds[i].status.roundStatus == BTCLCore.Status.Drawing) {
                ...
                rounds[i].status.randomness = new uint256[](totalWinners * 3);
                for (uint256 j = 0; j < totalWinners; j++) {
                    uint256 index = j * 3;
                    rounds[i].status.randomness[index] = randomness[index + (i - round) * totalWinners * 3];
                    rounds[i].status.randomness[index + 1] = randomness[index + (i - round) * totalWinners * 3 + 1];
                    rounds[i].status.randomness[index + 2] = randomness[index + (i - round) * totalWinners * 3 + 2];
                }
