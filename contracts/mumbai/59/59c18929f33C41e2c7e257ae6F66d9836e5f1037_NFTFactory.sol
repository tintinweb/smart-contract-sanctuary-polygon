...
interface INFT is IERC721 {
    ...
    function setTokenRoyalty(uint256 tokenId, address splitter, uint96 feeNumerator) external;
...
interface IRoyaltySplitter {
    ...
    function computeRoyaltyForwarderAddress(address collection, uint256 tokenId) external view returns (address);
}
    ...
    PullPaymentUpgradeable
{
    ...
    function createCollection(
        ...
        _createCollection(params.collectionId, params.name, params.symbol, params.itemLimit);
    ...
    function _mintItem(
        ...
        if (royalties.length > 0) {
            (address royaltyForwarder, uint96 totalShares) = royaltySplitter.registerRoyalty(
                collection.nft,
                tokenId,
                royalties
            );
