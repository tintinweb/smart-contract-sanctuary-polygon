...
contract SalePhases is Context, Ownable, Initializable {
    ...
    function standardUnlock() internal {
        if(whatUnlockPhaseWeAre() >= 20) { releaseAll(true,true,true,true); }
        bool UnlockedAll = whatUnlockPhaseWeAre() >= 20;
        _pendingpurchasedSaleTokens[msg.sender] += (_purchasedSaleTokens[msg.sender] / 100 * 5) * whatUnlockPhaseWeAre(); 
        if(!UnlockedAll) {require(_purchasedSaleTokens[msg.sender] >= _pendingpurchasedSaleTokens[msg.sender],"Math error");}
        if(whatUnlockPhaseWeAre() >= 3) { releaseAll(true,false,false,false); } else {_pendingpurchasedPrivateSaleTokens[msg.sender] += (_purchasedPrivateSaleTokens[msg.sender] / 100 * 10) * whatUnlockPhaseWeAre();}
        ...
        if(whatUnlockPhaseWeAre() >= 8) { releaseAll(true,true,true,false); } else {_pendingpurchasedPreSaleTokens[msg.sender] += (_purchasedPreSaleTokens[msg.sender] / 100 * 20) * whatUnlockPhaseWeAre();}
    }
