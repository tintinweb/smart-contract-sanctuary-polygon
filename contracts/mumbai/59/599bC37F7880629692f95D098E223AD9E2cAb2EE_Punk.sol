...
contract Punk is ERC721EnumerableUpgradeable {
    using StringsUpgradeable for uint256;
    ...
    event ContractDeployed(string baseURI, string name, string symbol);

    /// @dev Emit an event when mint success.
    event Mint(uint256 indexed tokenId, address indexed owner, string tokenUri);
     ...
     * @param _baseUri The metadata URI associated with the contract.
    ...
    function initialize(string memory _baseUri, string memory _name, string memory _symbol) public initializer {
        ...
        baseURI = _baseUri;
        emit ContractDeployed(_baseUri, _name, _symbol);
    }
    ...
    function setBaseURI(string memory _newUri) external {
        if (bytes(_newUri).length == 0) revert InvalidUri();
     ...
     */
    function mint(address _to) public payable returns (uint256 tokenId) {
        lastId.increment();
        _safeMint(_to, lastId.current());
        tokenId = lastId.current();
        emit Mint(tokenId, _to, tokenURI(tokenId));
    }
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory uri) {
        if (_tokenId < lastId.current()) revert TokenIdNotExists();
        uri = string(abi.encodePacked(_baseURI(), _tokenId.toString(), ".json"));
    }
