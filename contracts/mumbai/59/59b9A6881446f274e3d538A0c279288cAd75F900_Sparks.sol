    ...
    OwnableUpgradeable
{
    ...
    function setLinkCreator(address _linkCreator) external onlyOwner nonZeroAddress(_linkCreator) {
    ...
    /// @param amount Amount of sparks to burn.
    /// @param publicKey Public key of the invite.
    /// @dev Throws if the caller is not the owner of the token.
    function burnAndCreateInvite(
        uint256 amount,
        address publicKey
    ) external onlyEOA {
        // Burn the tokens
        ...
        // Create the invite
        linkCreator.createLink(account, amount, publicKey);
    }
