...
contract Marketplace is OwnableUpgradeable {
    ...
    event ListItem(string category, string imgHash, uint256 count, uint256 priceForBBOSS, uint256 priceForUSD);
    ...
    function listItem(string memory _category, string memory _imgHash, uint256 _count, uint256 _priceForBBOSS, uint256 _priceForUSD) public onlyOwner {
        ...
        emit ListItem(_category, _imgHash, _count, _priceForBBOSS, _priceForUSD);
    }

    function remove(string memory _category, string memory _imgHash, uint256 _count, uint256 _priceForBBOSS, uint256 _priceForUSD) public onlyOwner {
        require(_count > 0, "count must be over 0");
        items[index] = ItemInfo(_category, _imgHash, _count, _priceForBBOSS, _priceForUSD);
        index = index + 1;
