...
contract MustasheRPSGame {
    ...
    function joinGame(uint256 _gameId, bytes calldata _opponentSignature) external {
        Game storage game = games[_gameId];
        require(game.creator != address(0), "Game does not exist");
        require(game.opponent == address(0), "Game is already full!");

        bytes32 opponentSignatureHash = keccak256(_opponentSignature);
        bytes32 opponentSignatureBytes32 = bytes32(opponentSignatureHash);
        game.opponent = msg.sender;
        game.choices[msg.sender] = opponentSignatureBytes32;
        game.hasSigned[msg.sender] = true;

        // Check if the game creator has already signed the transaction, then proceed with choices
        if (game.hasSigned[game.creator]) {
    ...
    function play(uint256 _gameId, bytes32 _choiceHash, bytes calldata _creatorSignature) external {
        Game storage game = games[_gameId];
        ...
        address signer = messageHash.recover(_creatorSignature);
        require(signer == msg.sender, "Invalid creator signature");

        game.choices[msg.sender] = _choiceHash;
        game.hasSigned[msg.sender] = true;

        // Проверяем, если оппонент уже подписал транзакцию, то делаем выборы
        if (game.hasSigned[game.opponent]) {
            emit ChoicesRevealed(
                _gameId,
                game.creator,
                game.opponent,
