...
contract Farming is Ownable, ReentrancyGuard {
    ...
    function stakeCake() public {
        ...
        uint256 planId = 1;
        require(cakePlan[planId].planid == planId, "This plan is not valid");
        stakeId++;
        userCakeBalance[msg.sender] += cakeValue;
        stakeCount[msg.sender]++;
        userCakeStakedInfo[msg.sender][stakeId] = cakeStakeDetails(stakeId, cakeValue, block.timestamp, 0, planId, stakedIds[msg.sender].length);
    ...
    function popSlot(uint256 _id) private {
        ...
        uint256 lastIndex = stakedIds[sender].length - 1;
        uint256 lastID = stakedIds[sender][lastIndex];
        uint256 currentPos = userCakeStakedInfo[sender][_id].indexofid;
        stakedIds[sender][currentPos] = lastID;
        userCakeStakedInfo[sender][lastID].indexofid = currentPos;
        stakedIds[sender].pop();
    }
