    ...
    OwnableUpgradeable
{
    ...
    function initialize(
        ...
        _landContractAddr = landContractAddr;
    }
    ...
    function updateBaseTokenURI(string memory newBaseTokenURI)
        public
        onlyOwner
    {
    ...
    function createEstate(address to, string memory data) public onlyOwner {
        _tokenIds.increment();
        _mint(to, _tokenIds.current());
        _totalSupply.increment();
        emit CreateEstate(to, _tokenIds.current(), data);
    }

    function addLand(uint256 estateId, uint256 landId) public {
        require(_exists(estateId), "cannot add to unexisted estate");
        IERC721Upgradeable(_landContractAddr).transferFrom(
            _msgSender(),
            address(this),
            landId
        );
    ...
    function removeLand(uint256 estateId, uint256 landId) public {
        require(_exists(estateId), "cannot remove from unexisted estate");
        IERC721Upgradeable(_landContractAddr).transferFrom(
            address(this),
            _msgSender(),
            landId
        );
        emit RemoveLand(estateId, landId, _msgSender());
    }

    function updateEstateData(uint256 estateId, string memory data) external {
        emit Update(estateId, ownerOf(estateId), _msgSender(), data);
    }
    ...
    function landContractAddress() external view returns (address) {
        return _landContractAddr;
    }
