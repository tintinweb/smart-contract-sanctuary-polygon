...
*/
    ...
    mapping(uint256 => mapping(address => uint256)) public dataTokenUseAllowance;

    // MODIFIERS
    modifier onlyFactoryOrOwner() {
        require(msg.sender == nvg8Factory || msg.sender == owner(), "Only factory or owner can do this");
        _;
    }
    ...
    function unlistDataToken(uint256 _dataTokenId) public {
        ...
        require(dataTokens[_dataTokenId].owner == msg.sender || msg.sender == owner(), "Only the datatoken owner or contract owner can unlist data token");
        
        dataTokens[_dataTokenId].isActive = false;
    }

    function getDataToken(uint256 _dataTokenId) public view returns (DataToken memory) {
    ...
    function buyDataTokenForUse(uint256 _dataTokenId, uint256 _days) public {
        // TODO: validate _days
        require(dataTokens[_dataTokenId].erc20Token != address(0) && dataTokens[_dataTokenId].isActive, "Data token not enlisted");
        require(IERC20Template(dataTokens[_dataTokenId].erc20Token).balanceOf(msg.sender) >= dataTokens[_dataTokenId].usagePrice, "Not enough balance");
        ...
        IERC20Template(dataTokens[_dataTokenId].erc20Token).transferFrom(msg.sender, dataTokens[_dataTokenId].owner, dataTokens[_dataTokenId].usagePrice);
        dataTokenUseAllowance[_dataTokenId][msg.sender] = block.timestamp + _days * 24 * 60 * 60;

    }

    function isAllowedToUseDataToken(uint256 _dataTokenId) public view returns (bool _allowed) {
