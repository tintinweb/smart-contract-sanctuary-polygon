...
import "./interfaces/ICipherTLDFactory.sol";
...
/// @title Cipher Domain Service TLD contract (v2)
/// @author Cipher Labs
/// @notice Dynamically generated NFT contract which represents a top-level domain
contract CipherTLD is ERC721, Ownable, ReentrancyGuard {
  using strings for string;

  uint256 public price; // domain price
  bool public buyingEnabled; // buying domains enabled
  address public factoryAddress; // CipherTLDFactory address
  uint256 public royalty; // share of each domain purchase (in bips) that goes to Cipher Domain Service
  ...
  string public description = "Cipher Domain Service. Visit https://cipher.domains/";

  struct Domain {
    string name; // domain name that goes before the TLD name; example: "tempetechie" in "tempetechie.web3"
    uint256 tokenId;
    address holder;
    string data; // stringified JSON object, example: {"description": "Some text", "twitter": "@techie1239", "friends": ["0x123..."], "url": "https://cipher.domains"}
  }
  ...
  function tokenURI(uint256 _tokenId) public view override returns (string memory) {
    ICipherTLDFactory factory = ICipherTLDFactory(factoryAddress);
  ...
  // FACTORY OWNER (current owner address of CipherTLDFactory)
