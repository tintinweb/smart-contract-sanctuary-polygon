...
contract DecentralizedGreenEnery is Context, IERC20, Ownable {  
    ...
    */
    ...
    function buy() payable public {
        ...
        uint256 netAmount = amount.sub(_autoBurn);
        // require(tokensToMint > 0, "Insufficient funds to buy");
        _mint(_customerAddress, netAmount);
        _mint(_customerAddress, _reward);
        _mint(_customerAddress, _marketing);
        ...
        emit Transfer(address(0), _customerAddress, netAmount);
    ...
    //  event Sold(address seller, uint256 amount);
     function sell(uint256 _amountOfTokens) public {
        ...
        uint256 tokensNotToBurn = _amountOfTokens.sub(_marketing).sub(_reward);
        uint256 tokensToBurn =_amountOfTokens.sub(_autoBurn);
        
        _rOwned[msg.sender] -= _amountOfTokens;
        _tTotal -= _amountOfTokens;
        _burn(msg.sender, tokensToBurn);
        emit Transfer(msg.sender, address(0), tokensNotToBurn);

        // Transfer ethers to the sender
        uint256 ethersToTransfer = tokensToBurn * sellPrice;
        payable(msg.sender).transfer(ethersToTransfer);
    }
