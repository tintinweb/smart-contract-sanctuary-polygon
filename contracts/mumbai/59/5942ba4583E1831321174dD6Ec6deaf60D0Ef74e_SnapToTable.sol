    ...
    function safeMint(address nftContract) public onlyOwner {
        ...
        for (uint256 i = 0; i < supply; i++) {
            ...
            string memory row = string.concat(
                "(",
                StringsUpgradeable.toString(token),
                ",'",
            ...
            emit sqlUpdate(row,"row",_msgSender());
            insert = string.concat(insert, splitter, row);
        }
        ...
        if (supply>0) {
            ...
            string memory _snapName = string.concat(
                "snapshot_",
                StringsUpgradeable.toString(block.chainid),
                "_",
                StringsUpgradeable.toString(_snapshot)
            );
