...
contract IchibanDAO is VRFConsumerBaseV2, Ownable, ReentrancyGuard {
    ...
    struct PhysicalPrizeGame {
        address gameOwner;
        uint256 prizeTypesCount;
        uint256 remainingPrizeCount;
        uint256[] prizeRemain;
        string[] prizeInfo;
        uint256 price;
    }
    ...
    mapping(uint256 => OrderBook) orderBooks; // requestId => order book
    mapping(uint256 => mapping(address => uint256[])) ownedPrizes; // gameId -> owner -> prizes
    ...
    modifier onlyVerifyMerchant {
        require(verifyMerchants[msg.sender], "Only verified merchant can call this function");
        _;
    }
    ...
    function playPhysicalPrizeGame(uint gameId, uint8 playRounds)
        ...
        nonReentrant 
    {
        PhysicalPrizeGame memory physicalPrizeGame = physicalPrizeGames[gameId];
