...
/**
 * @title This is the CryptoTattooerz implementation for Manual Price Handling used by ERC20 tokens Direct Sale contract.
...
contract CryptoTattooerzPriceHandlerManual is CryptoTattooerzPriceHandler{
    ...
    constructor(uint8 tokenDecimals, uint256 tokenPriceUSD, uint8 tokenDecimalsUSD,
        ...
        _setPriceData(TOKEN, tokenDecimals, tokenPriceUSD, tokenDecimalsUSD);
    ...
    function getPriceData(bytes32 currency) public view override returns (CurrencyPriceData memory) {
        ...
        require(result.decimals != 0 || result.priceUSD != 0 || result.decimalsUSD != 0,
    ...
    function _setPriceData(bytes32 currency, uint8 decimals, uint256 priceUSD, uint8 decimalsUSD) internal {
        ...
        (priceUSD, decimalsUSD) = cleanFromTrailingZeros(priceUSD, decimalsUSD);
        if(priceData.priceUSD == priceUSD) {
            if(priceUSD == 0 || (priceData.decimals == decimals && priceData.decimalsUSD == decimalsUSD)) {
                return;
            }
            priceData.decimals = decimals;
            priceData.decimalsUSD = decimalsUSD;
            return;
        }
        if(priceData.priceUSD == 0) {
            _pricesData[currency] = CurrencyPriceData(decimals, priceUSD, decimalsUSD);
            _addToken(currency);
        }
        else if(priceUSD == 0) {
            priceData.decimals = 0;
            priceData.priceUSD = priceUSD;
            priceData.decimalsUSD = 0;
