...
contract NoCapMarketplace is Ownable, Initializable, EIP712Upgradeable, ReentrancyGuardUpgradeable {
    ...
    function hashVoucher(Voucher.NFTVoucher memory voucher) internal view returns(bytes32) {
        return _hashTypedDataV4(keccak256(abi.encode(keccak256("NFTVoucher(address seller,address NFTAddress,uint256 tokenId,uint256 maxFractions,uint256 pricePerFraction,bool toMint,address royaltyKeeper,uint96 royaltyFees,string tokenURI)"),
        voucher.seller,
        voucher.NFTAddress,
        voucher.tokenId,
    ...
    function bidTransfer(Voucher.NFTVoucher memory voucher, address _bidder, uint _fractions, address _currency) external onlyAdmin {
        require(allowedCurrencies[_currency],"Currency not allowed.");
        require(INoCapTemplate(voucher.NFTAddress).checkExist(voucher.tokenId),"NFT does not exist.");
        require(fractionsNFT[voucher.NFTAddress][voucher.tokenId].fractionsLeft==0,"Sale not allowed until all fractions are issued.");
        (address receiver,uint amount,uint royaltyAmount) = calculateTotalAmount(voucher,_fractions,false);
        IERC20(_currency).transferFrom(_bidder, voucher.seller, voucher.pricePerFraction);
        IERC20(_currency).transferFrom(_bidder, address(this), (platformFeePercent*voucher.pricePerFraction*_fractions)/10000);
        platformCollection[_currency] += (platformFeePercent*voucher.pricePerFraction*_fractions)/10000;
        IERC20(_currency).transferFrom(_bidder, receiver, royaltyAmount);
        IERC20(INoCapTemplate(voucher.NFTAddress).getSTOForTokenId(voucher.tokenId)).transferFrom(voucher.seller,_bidder,_fractions);
    }
    ...
    function enableRefundForSale(address _collectionAddress, uint _tokenId) external onlyAdmin{
         ...
         INoCapTemplate(_collectionAddress).refundIssued(_tokenId);
    }
...
interface INoCapTemplate {
   ...
   function refundIssued(uint _tokenId) external;
