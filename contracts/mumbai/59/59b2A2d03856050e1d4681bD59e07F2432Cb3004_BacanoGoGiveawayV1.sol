    ...
    DroppableUpgradeable
{
    ...
    function _beforeClaim(
        ...
        require(_quantity <= 100, "Transaction limit exceeded");
...
contract BacanoGoGiveawayV1 is BacanoGoCollection {
    ...
    function configureCharacters(
        ...
        string memory _cardTypeName,
        bool _randomDistribution
    ) external onlyOwner {
        CardSpec storage cardSpec = cardSpecs[_cardType];
        cardSpec.odds = _odds;
        cardSpec.randomDistribution = _randomDistribution;
        cardSpec.cardTypeName = _cardTypeName;

        require(_characters.length > 0, "Must have at least one character");
        require(
            cardSpec.characters.length == 0 || cardSpec.characters.length == _characters.length,
            "Can't change characters size"
        );

        // Reset the characters
        delete cardSpec.characters;

        // Add the new characters
        for (uint256 i = 0; i < _characters.length; i++) {
            cardSpec.characters.push(_characters[i]);
        }
    }

    function getCharacters(CardType _cardType)
        external
        view
        returns (CharacterSpec[] memory)
    {
        return cardSpecs[_cardType].characters;
    }

    function getTotalOdds() internal view returns (uint32) {
        return
    ...
    function getCardType(uint256 number) internal view returns (CardType) {
        if (number <= cardSpecs[CardType.CARNAVAL].odds) {
            return CardType.CARNAVAL;
        } else if (
            number <=
            (cardSpecs[CardType.CARNAVAL].odds +
                cardSpecs[CardType.HOLOFOIL].odds)
        ) {
            return CardType.HOLOFOIL;
        } else {
    ...
    function transferTokensOnClaim(
        ...
        uint256 odds = getTotalOdds();
        require(odds > 0, "No characters configured");
        ...
        for (uint32 i = 0; i < _quantity; i++) {
            ...
            CardType cardType = getCardType(target);
