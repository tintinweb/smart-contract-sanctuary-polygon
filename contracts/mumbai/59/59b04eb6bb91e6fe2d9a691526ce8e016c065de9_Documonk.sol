...
contract Documonk is AccessControl, Ownable {
  ...
  // delete wallet of institute from record
    ...
    function getWallet(string calldata UID ) public view returns (address[] memory) {
     ...
     // add new document
    function addDocument(string calldata UID, string calldata md5, string calldata sha_256, uint256 timstamp ) public returns (uint256){
      ...
      // we check md5 or sha256 already exist or not in our database if found then we dont add
       if(documentDetailsArray[0].timstamp > 0) {
         for (uint256 i = 0; i < totalItems; i++) {
            if (keccak256(abi.encodePacked(documentDetailsArray[i].md5)) == keccak256(abi.encodePacked(md5))) {
                 foundFlag = 1;
                 break;
            }
            if (keccak256(abi.encodePacked(documentDetailsArray[i].sha_256)) == keccak256(abi.encodePacked(sha_256))) {
                 foundFlag = 1;
                 break;
            }
        }
    ...
    function getDocumentSHA256(string calldata hash) public view returns (InstituteDoc[] memory) {
       ...
       for (uint256 i = 0; i < itemId; i++) {
            if (keccak256(abi.encodePacked(documentDetailsArray[i].md5)) == keccak256(abi.encodePacked(hash))) {
                 tempData[0].timstamp = documentDetailsArray[i].timstamp;
                 tempData[0].UID = documentDetailsArray[i].UID;
                 tempData[0].instituteName = instituteDetailsArray[documentDetailsArray[i].UID].name;
                 break;
            }
            if (keccak256(abi.encodePacked(documentDetailsArray[i].sha_256)) == keccak256(abi.encodePacked(hash))) {
                 tempData[0].timstamp = documentDetailsArray[i].timstamp;
                 tempData[0].UID = documentDetailsArray[i].UID;
                 tempData[0].instituteName = instituteDetailsArray[documentDetailsArray[i].UID].name;
                 break;
    ...
    function getDocumentsCountByUID(string calldata UID) public view returns (uint256) {
        ...
        uint256 itemCount = 0;
       for (uint256 i = 0; i < itemId; i++) {
            if (keccak256(abi.encodePacked(documentDetailsArray[i].UID)) == keccak256(abi.encodePacked(UID))) {
                 itemCount += 1;
            }
        }
      return itemCount;
