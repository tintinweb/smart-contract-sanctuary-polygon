...
contract NgageNFactory is AccessControl {
    bytes32 public constant CONTRACT_CREATOR = keccak256("CONTRACT_CREATOR");
    address ngageN;

    mapping(string => address) public factoryCreated;

    mapping(string => address) public ngageNContracts;
    ...
    function create(
        ...
        require(hasRole(CONTRACT_CREATOR, _msgSender()), "NF: Only the contract creator can create a factory");
        require(factoryCreated[_factoryName] == address(0), "NF: Factory already created");

        // CustomForwarder forwarder = new CustomForwarder();

        TransparentUpgradeableProxy proxy = new TransparentUpgradeableProxy(
            ngageN,
            _msgSender(),
            abi.encodeWithSelector(NgageNV2(address(0)).initialize.selector, _uri, _contractUri)
        );

        // ProofOfX proofOfX = new ProofOfX(INgageN(address(proxy)), forwarder);

        // NgageN(address(proxy)).grantRole(DEFAULT_ADMIN_ROLE, _msgSender());
        // NgageN(address(proxy)).grantRole(NgageN(address(proxy)).NFT_OPERATOR_ROLE(), _msgSender());
        // NgageN(address(proxy)).grantRole(NgageN(address(proxy)).NFT_OPERATOR_ROLE(), address(proofOfX));

        // ProofOfX(address(proofOfX)).grantRole(DEFAULT_ADMIN_ROLE, _msgSender());
        // ProofOfX(address(proofOfX)).grantRole(ProofOfX(address(proofOfX)).CREATOR(), _creator);
        // ProofOfX(address(proofOfX)).grantRole(ProofOfX(address(proofOfX)).OPERATOR(), _operator);

        // CustomForwarder(address(forwarder)).grantRole(CustomForwarder(address(forwarder)).RELAYER(), _relayer);

        factoryCreated[_factoryName] = _msgSender();
        ngageNContracts[_factoryName] = address(proxy);
        // factoryContracts[_factoryName] = FactoryContract(
        //     address(proxy),
        //     address(proofOfX),
        //     address(forwarder)
        // );
        // emit FactoryCreated(_msgSender(), _factoryName, address(proxy), address(proofOfX), address(forwarder));
        emit FactoryCreated(_msgSender(), _factoryName, address(proxy));
    }
...
contract NgageNV2 is AccessControlUpgradeable, ERC1155Upgradeable {
        ...
        initializer
    {
        ...
        _setupRole(DEFAULT_ADMIN_ROLE, ContextUpgradeable._msgSender());
        _setupRole(NFT_OPERATOR_ROLE, ContextUpgradeable._msgSender());
    }
