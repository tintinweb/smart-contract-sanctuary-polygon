...
contract CrossChainManager is Initializable, UUPSUpgradeable, ReentrancyGuardUpgradeable, NonblockingLzApp, PausableUpgradeable, AdminProxyManager {
    ...
    address public dstCrossChainManager; // dst cross chain manager
	...
	function _nonblockingLzReceive(
        ...
        (address targetSale, uint128 amountIn, address buyer) = abi.decode(_payload, (address, uint128, address));

        IPublicGovSale(targetSale)._buyToken(amountIn, buyer);
    }

    function finalizeRemote(bytes calldata _payload, bytes calldata _adapterParams, uint256 crossFee) external whenNotPaused nonReentrant {
        require(factory.isKnown(_msgSender()) || _msgSender() == owner(), "unknown");

        lzEndpoint.send{value: crossFee}( // {value: messageFee} will be paid out of this contract!
            dstChainId, // destination chainId
            abi.encodePacked(dstCrossChainManager), // destination address of Cross chain manager contract
        ...
        emit CrossChain(dstChainId, abi.encodePacked(dstCrossChainManager), _payload);
    }

    function setFactory(address _factory) external onlyOwner {
        factory = IPublicGovFactory(_factory);
    }

    function setDstCrossChainManager(address _dstCrossChainManager) external onlyOwner {
        dstCrossChainManager = _dstCrossChainManager;
    }
...
interface IPublicGovSale {    
	...
	function _buyToken(uint128, address) external;
}
