...
contract NFTRoyaltyToken is ERC721Royalty, ERC721Enumerable, ERC721URIStorage, Ownable{
    ...
    function setContractUri(string memory contractUri_) external onlyOwner{
        require(bytes(_contractUri).length > 0 ,"The _contractUri length should be greater than 0");
    ...
    function setTokenRoyalty( address receiver, uint96 feeNumerator)external onlyOwner{
        _setDefaultRoyalty( receiver, feeNumerator);
    }
}
// File: contracts/NFTRoyaltyChildToken.sol


pragma solidity 0.8.7;

contract NFTRoyaltyChildToken is NFTRoyaltyToken {
    uint256 public constant BATCH_LIMIT = 20;
    event WithdrawnBatch(address indexed user, uint256[] tokenIds);
    event TransferWithMetadata(address indexed from, address indexed to, uint256 indexed tokenId, bytes metaData);
    address private _childChainManagerProxy;

    constructor(string memory name, string memory symbol, address childChainManagerProxy) NFTRoyaltyToken(name,symbol) {
    ...
    external
    {
        require(_msgSender() == _childChainManagerProxy,"The address needs to be child chain manager proxy");
        // deposit single
