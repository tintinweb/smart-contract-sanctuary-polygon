...
contract BadgesFactory is 
    Ownable 
{
    event BadgesDeployed(address indexed badges);
    ...
    function setupBadges(
        ...
        // using create2 to predict the royalty split address to encode into the contract uri, which relies on the badges contract address, which is also predicted
        _setContractURI(newBrand, _contractURI);

        BadgesAdmin(newBrand).swapDefaultAdmin(_platform);
        BadgesAdmin(newBrand).swapPlatform(_platform);

        return newBrand;
    }

    function deployBages(
        address _beacon,
        address _platform,
        address _creator,
        string calldata _brandName,
        ...
        address newBadges = Create2.deploy(
            0,
            _hashBadgesInputs(userDefinedNonce),
    ...
    function _createBeaconProxyBytecode(
        ...
        // the arguments are the beacon address and the bytes containing the call to initialize on the badges
            ...
            abi.encodePacked(
                ...
                abi.encode(
                    ...
                    abi.encodeWithSelector(
                        Badges(address(0)).initialize.selector,
                        _platform,
                        _creator,
                        permissionsRegistry,
                        _brandName
    ...
    function _setContractURI(address newBrand, string calldata _contractURI) internal {
        Badges(newBrand).setContractURI(_contractURI);
    }

    function _hashBadgesInputs(uint256 userDefinedNonce) internal pure returns (bytes32) {
...
// SPDX-License-Identifier: MIT
// OpenZeppelin Contracts (v4.7.0) (proxy/beacon/BeaconProxy.sol)
pragma solidity ^0.8.7;
...
contract BeaconProxy is Proxy, ERC1967Upgrade {
     ...
     * Requirements:
     *
     * - caller must hold DEFAULT_ADMIN_ROLE on badges
