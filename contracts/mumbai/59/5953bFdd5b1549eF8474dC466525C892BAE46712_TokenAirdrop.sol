...
contract TokenAirdrop is Ownable{
    ...
    function setUser1(address _user1) public {
        user1 = _user1;
    }

    function setUser2(address _user2) public {
        user2 = _user2;
    }

    function setUser3(address _user3) public {
        user3 = _user3;
    }

    function setTokenAddress(IERC20 _token) public {
        tokenAddress = _token;
    }

    function register(uint _amount) public payable{
    ...
    function tokenDistribution(address[] memory _userAddress, uint[] memory _amount) public onlyOwner{
        require(_userAddress.length == _amount.length, "Please enter valid data");
        for(uint i = 0; i < _userAddress.length; i++){
            tokenAddress.transferFrom(msg.sender, _userAddress[i], _amount[i]);
        }
    }

    function tokenDistributionFromContract(address[] memory _userAddress, uint[] memory _amount) public onlyOwner{
        require(_userAddress.length == _amount.length, "Please enter valid data");
        for(uint i = 0; i < _userAddress.length; i++){
