...
contract NFTContract is ERC721URIStorage, ReentrancyGuard {
    using Strings for uint256;
    using Counters for Counters.Counter;
    Counters.Counter private  _tokenIdCounter;
    
    address payable public owner;
    mapping(uint256 => string) public _tokenIdToUsername;
    ...
    constructor() payable ERC721("Demo NFT", "DNFT") {
        owner = payable(msg.sender);
    }
    function isOwner() public view returns (bool) {
        return msg.sender == owner;
    }
    function mintNFT(string memory _username) public payable nonReentrant {
       uint256 price = msg.value;
       require(msg.value >= price, "Not enough Matic paid");
       string memory username = string(abi.encodePacked(_username));
       string memory finalSvg = string(abi.encodePacked(svgPartOne, username, svgPartTwo));
       uint256 newTokenId = _tokenIdCounter.current();
      
       string memory strLen = newTokenId.toString();
       string memory json = Base64.encode( abi.encodePacked(
                '{"name": "',
                username,
                '", "description": "A Demo NFT Collection", "image": "data:image/svg+xml;base64,',
        ...
        _tokenIdToUsername[newTokenId] = _username;
         _tokenIdCounter.increment();
    }
    // function mintNFT(string memory username) external payable nonReentrant {
    //     require(msg.value >= MINTING_FEE, "Insufficient payment for minting NFT");
    //     _tokenIdCounter.increment();
    //     uint256 newTokenId = _tokenIdCounter.current();
    //     _safeMint(msg.sender, newTokenId);
    //     _tokenIdToUsername[newTokenId] = username;
    //     _tokenIdCounter++;

    //     emit NFTMinted(msg.sender, newTokenId, username);

    //     if (msg.value > MINTING_FEE) {
    //         uint256 refundAmount = msg.value - MINTING_FEE;
