...
contract MetaForceContract is Initializable, Proxied, Governed, ReentrancyGuard, IMetaForceContract {
    ...
    function distributionRewardReferal(
        ...
        distributionInfo.tempAmountMFS = amountMFS;
        distributionInfo.tempAmountUSD = amountUSD;
        distributionInfo = distributionOnReferralTree(distributionInfo, userId, level, amountMFS, amountUSD, activate);
        if (distributionInfo.countLoop > distributionLoopsLimit) {
            return;
        }
        distributionInfo = distributionOnMarketingTree(distributionInfo, userId, level, amountMFS, amountUSD, activate);
        if (distributionInfo.countLoop > distributionLoopsLimit) {
            return;
        }
        if(distributionInfo.amountMFSForRealization>0)
        {
            revert MFCLevelMoreMaxPackLevel();
        }
        if(distributionInfo.amountMFSForRealization==0)
        ...
        realizeMFS(distributionInfo.amountMFSForRealization);
    }

    function distributionOnReferralTree(
        ...
        uint256 _amountUSD,
        bool _activate
    ) internal returns (DistributionInfo memory) {
        ICoreContract core = ICoreContract(registry.getCoreContract());
        uint256[] memory rewardsRefers = core.getRewardsDirectReferrers();
        uint256 refererId = _userId;
        for (uint256 i; i < rewardsRefers.length; ) {
            uint256 dRewardMFS = _amountMFS.mul(rewardsRefers[i]);
            uint256 dRewardUSD = _amountUSD.mul(rewardsRefers[i]);
            while (true) {
                ...
                unchecked {
                    _distributionInfo.countLoop += REFERRAL_TREE_STEP;
                }

                if (_distributionInfo.countLoop > distributionLoopsLimit) {
                    uint256 _tempAmountMFS = _distributionInfo.tempAmountMFS;
                    emit OutOfGas(_userId, refererId, _activate, _level, _tempAmountMFS, false, i);
                    return _distributionInfo;
                }
            }
            _distributionInfo.tempAmountMFS -= dRewardMFS;
            _distributionInfo.tempAmountUSD -= dRewardUSD;
            if (_activate) {
                ...
                if (typeReward == TypeReward.ONLY_MFS || core.getWorkflowStage() == Workflow.SaleOpen) {
                    ...
                    _distributionInfo.amountMFSForRealization += dRewardMFS;
                    ...
                    _distributionInfo.amountMFSForRealization += dRewardMFS / 2;
    ...
    function distributionOnMarketingTree(
        DistributionInfo memory _distributionInfo,
        uint256 _userId,
        uint256 _level,
        uint256 _amountMFS,
        ...
        for (uint256 i; i < rewardsMarketingRefers.length; ) {
            uint256 mRewardMFS = _amountMFS.mul(rewardsMarketingRefers[i]);
            uint256 mRewardUSD = _amountUSD.mul(rewardsMarketingRefers[i]);
            while (true) {
                ...
                unchecked {
                    _distributionInfo.countLoop += MARKETING_TREE_STEP;
                }

                if (_distributionInfo.countLoop > distributionLoopsLimit) {
                    uint256 _tempAmountMFS = _distributionInfo.tempAmountMFS;
                    emit OutOfGas(_userId, marketingRefererId, _activate, _level, _tempAmountMFS, true, i);
                    return _distributionInfo;
                }
            }

            if (i < rewardsMarketingRefers.length - 1) {
                _distributionInfo.tempAmountMFS -= mRewardMFS;
                _distributionInfo.tempAmountUSD -= mRewardUSD;
            } else {
                mRewardMFS = _distributionInfo.tempAmountMFS;
                mRewardUSD = _distributionInfo.tempAmountUSD;
            }
            if (_activate) {
                ...
                if (typeReward == TypeReward.ONLY_MFS || core.getWorkflowStage() == Workflow.SaleOpen) {
                    ...
                    _distributionInfo.amountMFSForRealization += mRewardMFS;
                    ...
                    _distributionInfo.amountMFSForRealization += mRewardMFS / 2;
...
struct DistributionInfo {
    uint256 tempAmountUSD;
    uint256 tempAmountMFS;
