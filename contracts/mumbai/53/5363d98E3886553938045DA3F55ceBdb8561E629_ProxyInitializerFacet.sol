...
library LibProxyInitialized {
    struct DiamondStorage {
        bool initialized;
    }

    bytes32 public constant DIAMOND_STORAGE_POSITION =
        keccak256("antic.storage.LibProxyInitialized");
...
library LibProxyInitializer {
    ...
    function diamondStorage() internal pure returns (InitData storage ds) {
        bytes32 storagePosition = DIAMOND_STORAGE_POSITION;
        assembly {
            ds.slot := storagePosition
        }
...
contract ProxyInitializerFacet {
    modifier initializer() {
        LibProxyInitialized.DiamondStorage storage ds = LibProxyInitialized
            .diamondStorage();
    ...
    function init(LibProxyInitializer.InitData memory initData)
        external
        initializer
    {
        // Roles
        for (uint256 i = 0; i < initData.members.length; i++) {
            LibRoles._grantRole(
                LibProxyInitializer.MEMBER_ROLE,
                initData.members[i]
            );
        }
    }

    function memberRole() external pure returns (bytes32) {
        return LibProxyInitializer.MEMBER_ROLE;
    }
}
