...
contract FunctionsConsumer is FunctionsClient, ConfirmedOwner {
  ...
  function getIntResponse() public view returns(uint256){
    uint256 decodedIntResponse = abi.decode(latestResponse, (uint256));
    return decodedIntResponse;
  }

  function getStringResponse() public view returns(string memory){
    string memory decodedStringResponse = abi.decode(latestResponse, (string));
    return decodedStringResponse;
  }  

  function getStringResponse2() public view returns(string memory){
    string memory decodedStringResponse = string(latestResponse);
    return decodedStringResponse;
  }  
}