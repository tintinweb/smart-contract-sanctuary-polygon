...
import "./erc20/PolygonIntellaX.sol";

contract SuperSwapX is Ownable {
    address public SuperStarX;
    ...
    function swapMatic2IX() public payable {
        require(type(uint256).max / ratio > msg.value);
        uint256 amount = msg.value * ratio;
        PolygonIntellaX(SuperStarX).transferFrom(owner(), msg.sender, amount);
    }
    ...
    function destruct() public onlyOwner {
        PolygonIntellaX(SuperStarX).transfer(
            owner(),
            PolygonIntellaX(SuperStarX).balanceOf(address(this))
        );
