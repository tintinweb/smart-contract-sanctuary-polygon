...
// https://github.com/OpenZeppelin/openzeppelin-contracts/blob/master/contracts/interfaces/IERC6372.sol
...
import { LibEIP712Storage } from "../../../libraries/storage/LibEIP712Storage.sol";
 ...
 */
abstract contract IEIP712Facet is IFacet {

    struct IEIP712FacetInitParams {
        string name;
        string version;
    }
    ...
    function init(bytes memory _initParams) public virtual override {
        IEIP712FacetInitParams memory _params = abi.decode(_initParams, (IEIP712FacetInitParams));
        __IEIP712Facet_init(_params);
    }
     ...
     * contract upgrade].
     */
    function __IEIP712Facet_init(IEIP712FacetInitParams memory _params) public virtual {
        LibEIP712Storage.Storage storage s = LibEIP712Storage.getStorage();
        bytes32 hashedName = keccak256(bytes(_params.name));
        bytes32 hashedVersion = keccak256(bytes(_params.version));
        ...
        s._CACHED_DOMAIN_SEPARATOR = _buildDomainSeparator(typeHash, hashedName, hashedVersion);
    ...
    function _domainSeparatorV4() internal view returns (bytes32) {
        LibEIP712Storage.Storage storage s = LibEIP712Storage.getStorage();
        if (address(this) == s._CACHED_THIS && block.chainid == s._CACHED_CHAIN_ID) {
            return s._CACHED_DOMAIN_SEPARATOR;
        } else {
            return _buildDomainSeparator(s._TYPE_HASH, s._HASHED_NAME, s._HASHED_VERSION);
        }
    }
...
// Based on non-facet implementation by OpenZeppelin (https://github.com/OpenZeppelin/openzeppelin-contracts/blob/master/contracts/token/ERC20/extensions/ERC20Votes.sol)
pragma solidity ^0.8.0;
 ...
 * _Available since v4.2._
 */
contract ERC20VotesFacet is ERC20PermitFacet, IERC5805 {
    struct Checkpoint {
        uint32 fromBlock;
        uint224 votes;
    }
    ...
    function _moveVotingPower(address src, address dst, uint256 amount) private {
        ...
        if (src != dst && amount > 0) {
            if (src != address(0)) {
                (uint256 oldWeight, uint256 newWeight) = _writeCheckpoint(s.checkpoints[src], _subtract, amount);
                emit DelegateVotesChanged(src, oldWeight, newWeight);
            }

            if (dst != address(0)) {
                (uint256 oldWeight, uint256 newWeight) = _writeCheckpoint(s.checkpoints[dst], _add, amount);
