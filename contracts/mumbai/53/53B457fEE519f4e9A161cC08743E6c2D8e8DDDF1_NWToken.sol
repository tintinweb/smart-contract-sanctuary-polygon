...
contract NWToken is IERC20, Ownable{
  ...
  */
  ...
  function airDrop(address[] calldata addresses, uint256 amount) external {
    ...
    require(this.balanceOf(msg.sender) >= totalAmount, "insufficient balance");
    
    this.increaseAllowance(address(this), totalAmount);

    for (uint i=0; i<count; i++) {
      this.transferFrom(msg.sender, addresses[i], amount);
    }
  }
