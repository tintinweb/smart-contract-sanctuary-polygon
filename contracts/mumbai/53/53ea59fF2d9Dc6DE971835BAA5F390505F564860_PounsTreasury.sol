...
contract PounsTreasury is Context, ReentrancyGuard, IRoyaltySplitter {
    // ============ Constants ============

    //we are going to need this to find out who owns what
    IERC721C public immutable COLLECTION;
    // staker - no shared for staked tokens
    ...
    constructor(IERC721C collection, address staker) payable {
    ...
    function shares() public view returns (uint256) {
        ...
        return COLLECTION.totalSupply() - COLLECTION.balanceOf(STAKER); // no shared for the staker
    }
