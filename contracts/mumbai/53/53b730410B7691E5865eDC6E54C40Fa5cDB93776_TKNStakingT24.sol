...
contract TKNStakingT24 is Ownable {
    ...
    function _stake(address _staker, uint256 _amount, uint256 _lockMode) internal {
        ...
        // [User has existing stake] We create a basic user info entry for enumerable rewards reading/tracking (i.e. for Remnant off-chain backend or partnered games to give in-game rewards for staking)
        if (user.deposits.length == 1)  // Condition: User has no stake prior to this stake
        {
            user.indexPosition = listOfStakers.length; // Set the user index position for tracking in listOfStakers array
            BasicUserInfo memory basicUserInfo =
                BasicUserInfo({
                    userAddress: address(_staker),
                    totalStakeWeighted: stakeWeight,
                    indexPosition: listOfStakers.length
                });

            listOfStakers.push(basicUserInfo);  // Action: Add enumerable staking trackability
        }
        else if (user.deposits.length > 1)  // [User has existing stake] Update total stake
        {
