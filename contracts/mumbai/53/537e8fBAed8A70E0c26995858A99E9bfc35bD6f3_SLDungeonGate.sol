    ...
    DungeonGateError
{
    ...
    // boost block count per 1 strange stone
    uint256 internal boostBlockCount;

    // soul stone amount per gate
    uint256 internal stonePerGate;

    // gate offerting distribution rates
    ...
    struct PercentageForGate {
        uint256[7] percentages; // E - S + Strange
    }
    ...
    mapping(RankType => Slot) internal slotPerHunterRank; // E - S
    ...
    // gateIds의 length를 hunterRank와 questBadge에 따라 허용

    /// @notice seasonId to slot quest badge tokenId
    mapping(uint256 => uint256) internal slotBadgeIds;
...
interface ISLDungeonGate {
     ...
     *  Distribution
     */
    function setGateDistribution(
        DungeonGateBase.Distribution calldata _distribution
    ) external;

    function getGateDistribution()
        external
        view
        returns (DungeonGateBase.Distribution memory);
    ...
    function setSlotPerHunterRank(
        DungeonGateBase.Slot[6] calldata _slots
    ) external;

    function setSlotBadgeId(uint256 _seasonId, uint256 _slotBadgeId) external;
    ...
    function getHunterSlot(
        uint256 _seasonId,
        address _hunter
    ) external view returns (uint256 _totalSlot, uint256 _usingSlot);
    ...
    function getSlotPerHunterRank()
        external
        view
        returns (DungeonGateBase.Slot[6] memory);
    ...
    function claimClearGate(
        uint256 _gateId,
        uint256 _strangeStoneAmount,
    ...
    function getRequiredStrangeStoneForClear(
        uint256 _gateId
    ) external view returns (uint256);

    function isClaimedGate(uint256 _gateId) external view returns (bool);
...
abstract contract Distribution is ISLDungeonGate, DungeonGateBase {
     ...
     */
    function setGateDistribution(
        Distribution calldata _distribution
    ) external onlyProviderMaster {
        if (!_isValidDistribution(_distribution)) revert InvalidDistribution();

        distribution = _distribution;

        emit SetDistribution(
            _distribution.providerRate,
            _distribution.operatorRate,
            _distribution.associatorRate,
            _distribution.creatorRate,
            _distribution.DAORate,
            block.timestamp
        );
    }

    function _isValidDistribution(
        Distribution calldata _distribution
    ...
    function _distributeGateFee(
        ...
        address[5] memory accounts = projectContract.getAccountsByCollectionId(
            soulStoneCollectionId
        );

        uint256[5] memory fees = _calculateFee(_price, distribution);

        _transfer(accounts, fees);
        ...
        emit GateFeeDistributed(
            ...
            _gateId,
            _price,
            accounts[uint256(AccountType.Provider)],
            fees[uint256(AccountType.Provider)],
            accounts[uint256(AccountType.Operator)],
            fees[uint256(AccountType.Operator)],
            accounts[uint256(AccountType.Associator)],
            fees[uint256(AccountType.Associator)],
            accounts[uint256(AccountType.Creator)],
            fees[uint256(AccountType.Creator)],
            accounts[uint256(AccountType.DAO)],
            fees[uint256(AccountType.DAO)],
            block.timestamp
        );
    }
    ...
    function _calculateFee(
        ...
        fees[uint256(AccountType.Provider)] =
            (_balance * _distribution.providerRate) /
            denominator;
        fees[uint256(AccountType.Operator)] =
            (_balance * _distribution.operatorRate) /
            denominator;
        fees[uint256(AccountType.Associator)] =
            (_balance * _distribution.associatorRate) /
            denominator;
        fees[uint256(AccountType.Creator)] =
            (_balance * _distribution.creatorRate) /
            denominator;
        fees[uint256(AccountType.DAO)] =
            (_balance * _distribution.DAORate) /
            denominator;
     ...
     *  View
     */
    function getGateDistribution() external view returns (Distribution memory) {
        return distribution;
    }
}
...
abstract contract Gate is Slot {
    ...
    function claimClearGate(
        ...
        if (isClaimedGate(_gateId)) revert AlreadyClaimedGate();
        ...
        uint256 requiredStrangeStone = getRequiredStrangeStoneForClear(_gateId);
        if (requiredStrangeStone != _strangeStoneAmount) revert NotClearGate();
        ...
        if (_strangeStoneAmount > 0) {
            soulStone.burn(
                hunter,
                uint256(StoneType.Strange),
                _strangeStoneAmount
            );

            gate.boosterCount += _strangeStoneAmount.toUint32();
        }
        ...
        soulStone.mintBatch(hunter, results, _asUintArray(1, results.length));
    ...
    function isClaimedGate(uint256 _gateId) public view returns (bool) {
        return gates[_gateId].claimed;
    }
...
abstract contract Slot is Distribution {
    ...
    // E - S
    function setSlotPerHunterRank(
        Slot[6] calldata _slots
    ) external onlyOperator {
        for (uint256 i = 0; i < _slots.length; i = i.increment()) {
            if (_slots[i].defaultSlot < 1) revert InvalidSlot();

            slotPerHunterRank[RankType(i)] = _slots[i];
        }
    }

    function setSlotBadgeId(
        uint256 _seasonId,
        uint256 _slotBadgeId
    ) external onlyOperator {
        if (seasonContract.isEndedSeasonById(_seasonId))
            revert InvalidSeasonId();

        uint256 questBadgeCollectionId = seasonContract
            .getSeasonById(_seasonId)
            .questBadgeCollectionId;
        ...
        if (!questBadge.exists(_slotBadgeId)) revert InvalidBadgeId();

        slotBadgeIds[_seasonId] = _slotBadgeId;
    }
    ...
    function getHunterSlot(
        uint256 _seasonId,
        address _hunter
    ) public view returns (uint256 totalSlot, uint256 usingSlot) {
        usingSlot = getHunterUsingSlot(_hunter);

        RankType hunterRank = seasonContract.getHunterRank(_seasonId, _hunter);
        Slot memory slot = slotPerHunterRank[hunterRank];
        uint256 slotBadgeBalance = seasonContract.getSeasonQuestBadgeBalance(
            _seasonId,
            _hunter,
            slotBadgeIds[_seasonId]
        );

        if (slotBadgeBalance > slot.addableSlot) {
            totalSlot = slot.defaultSlot + slot.addableSlot;
        } else {
            totalSlot = slot.defaultSlot + slotBadgeBalance;
        }
    }
    ...
    function getGateOfHunterSlot(
        address _hunter
    ) external view returns (Gate[] memory) {
        uint256[] memory gateIdOfHunterSlot = getGateIdOfHunterSlot(_hunter);

        Gate[] memory gateOfHunterSlot = new Gate[](gateIdOfHunterSlot.length);

        for (uint256 i = 0; i < gateIdOfHunterSlot.length; i = i.increment()) {
            gateOfHunterSlot[i] = gates[gateIdOfHunterSlot[i]];
        }

        return gateOfHunterSlot;
    }

    function getSlotPerHunterRank() external view returns (Slot[6] memory) {
        Slot[6] memory slots;
...
contract SLDungeonGate is Gate, UUPSUpgradeable {
    function initialize(
        ...
        distribution = Distribution({
            providerRate: 20_00000,
            operatorRate: 20_00000,
            associatorRate: 20_00000,
            creatorRate: 20_00000,
            DAORate: 20_00000
        });
    }
    ...
    function __gateInfoPerRank_init() private {
        gateInfoPerRank[RankType.E] = GateInfo(1000000000000000000, 1800);
        gateInfoPerRank[RankType.D] = GateInfo(1000000000000000000, 3600);
        gateInfoPerRank[RankType.C] = GateInfo(1000000000000000000, 7200);
        gateInfoPerRank[RankType.B] = GateInfo(1000000000000000000, 14400);
        gateInfoPerRank[RankType.A] = GateInfo(1000000000000000000, 28800);
        gateInfoPerRank[RankType.S] = GateInfo(1000000000000000000, 57600);
    }

    function __slotPerHunterRank_init() private {
        slotPerHunterRank[RankType.E] = Slot(1, 1);
        slotPerHunterRank[RankType.D] = Slot(1, 1);
        slotPerHunterRank[RankType.C] = Slot(1, 2);
        slotPerHunterRank[RankType.B] = Slot(1, 2);
        slotPerHunterRank[RankType.A] = Slot(1, 3);
        slotPerHunterRank[RankType.S] = Slot(1, 3);
    }
...
interface DungeonGateError {
    ...
    error InvalidGateId();
    error ExceedRemainingBlock();
    error InvalidGateSignature();
    error AlreadyClaimedGate();
    error NotClearGate();
