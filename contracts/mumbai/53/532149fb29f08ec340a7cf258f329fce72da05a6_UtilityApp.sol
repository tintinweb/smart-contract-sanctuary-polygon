...
contract UtilityApp{
     ...
     */
    function sendMessage(address from,address to, string calldata _msg, string calldata msgType) external {
        require(checkUserExists(msg.sender), "Create an account first!");
        require(checkUserExists(to), "User is not registered!");

        bytes32 chatCode = _getChatCode(from,to);
     ...
     * @param to address
     */
    function readMessage(address from,address to) external view returns(message[] memory) {
        require(checkUserExists(to), "Create an account first!");
        bytes32 chatCode = _getChatCode(from, to);
