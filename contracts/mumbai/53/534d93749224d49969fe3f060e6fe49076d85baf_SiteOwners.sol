...
contract SiteOwners is ChainlinkClient, Ownable, ISiteOwners {
    ...
    SiteOwner[] private _allSiteOwners;

    // The site owner request list (history of all)
    SiteOwnerRequest[] private _allSiteOwnerRequests;
    ...
    function verifyUrlOwner(string memory site) public returns (bytes32 requestId)
    {
        ...
        _allSiteOwnerRequests.push(SiteOwnerRequest(site, msg.sender, true, false));

        // Add the request
        _currentSiteOwnerRequestIndexes[requestId] = (_allSiteOwnerRequests.length -1); 

        emit SiteOwnerRequestAddedEvent(site, msg.sender, requestId, getTimestamp());
    ...
    * Get the site owner
    */
    function getSiteOwner(string memory site) external override view returns(SiteOwner memory siteOwner){
        ...
        if(compareStrings(siteOwner.Site, site))
            return siteOwner;

        // Index was not matched
        siteOwner = SiteOwner('',address(0),false); 
    }
    ...
    */
    ...
    function getSiteOwners(uint256 pageNumber, uint256 perPage) public view returns(SiteOwner[] memory siteOwners){
     ...
     // Get a page of a sites owners - pageNumber starts from 0
    function getSiteOwnerRequests(uint256 pageNumber, uint256 perPage) public view returns(SiteOwnerRequest[] memory siteOwners){
        // Validate page limit
        require(perPage <= _pageLimit, "Page limit exceeded");
        ...
        // Create the page
        SiteOwnerRequest[] memory pageOfSiteOwners = new SiteOwnerRequest[](pageSize);

        // Add each item to the page
        uint256 pageItemIndex = 0;
        for(uint256 i = startingIndex;i < (startingIndex + pageSize);i++){
           // Get the siteOwner
           SiteOwnerRequest memory siteOwner = _allSiteOwnerRequests[i];
    ...
    */
    function setExtension(string memory newExtension) public onlyOwner{
         ...
         // Fire update event
         emit ExtensionUpdatedEvent(newExtension, getTimestamp());
    }
    ...
    */
    function fulfill(bytes32 _requestId, address _owner) public recordChainlinkFulfillment (_requestId)
    {
        ...
        SiteOwnerRequest storage siteOwnerRequest = _allSiteOwnerRequests[index];
        ...
        if(siteOwnerRequest.Exists && siteOwnerRequest.Owner == _owner){  
            ...
            // Fire event
            emit SiteOwnerAddedEvent(siteOwnerRequest.Site, _owner, _requestId, getTimestamp());
        }
        else{
            // Deny request
            ...
            // Fire event
            emit SiteOwnerFailedEvent(_requestId, getTimestamp());
        }
    }
    ...
    function _addOrUpdateSiteOwner(string memory site, address owner) private returns(uint256){
        ...
        {
            // Get site owner
            SiteOwner storage siteOwner = _allSiteOwners[index];
        ...
        _allSiteOwners.push(SiteOwner(site, owner, true));
