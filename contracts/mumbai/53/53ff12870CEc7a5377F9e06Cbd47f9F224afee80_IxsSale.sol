...
contract IxsSale is Context, AccessControl, IIxsSale {
    ...
    function getCurrentStageIndex(uint256 _saleId) public view returns (uint8) {
        LaunchSale memory sale = onSales[_saleId];
        uint256 currentTime = block.timestamp;
        uint256 len = sale.deadlines.length;
        uint8 i = 0;
        while (i < len && currentTime > sale.deadlines[i]) {
        ...
        onlyRole(OPERATOR_ROLE) 
    {
        ...
        require(newTimestamp > current, "IxsSale: INVALID_PAST_TIMESTAMP");
        require(index > getCurrentStageIndex(_saleId), "IxsSale: STAGE_ALREADY_STARTED");
        LaunchSale memory oldSale = onSales[_saleId];
        uint256 prevDeadline = index > 0 ? oldSale.deadlines[index-1] : 0;
        require(newTimestamp > prevDeadline, "IxsSale: INVALID_TIMESTAMP");
