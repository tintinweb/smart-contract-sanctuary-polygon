...
contract BookStore is 
    ...
    function claimBookPaid(uint256 _bookId, uint256 _price, string memory _bookCopyUri) payable public {
        if(books[_bookId].bookID==0) revert BookNotFound();
        if( _price < books[_bookId].price) revert NotEnoughPrice();
        uriMapping[_bookId] = _bookCopyUri;
        payable(books[_bookId].bookAuther).transfer(_price);
