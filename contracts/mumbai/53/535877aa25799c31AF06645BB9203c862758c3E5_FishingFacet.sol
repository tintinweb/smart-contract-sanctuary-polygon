...
contract FishingFacet is WithStorage {
    ...
    function reel() public returns (uint256) {
    ...
    function playerBaitBalances(address player) public view returns (uint256[14] memory) {
        return [
            GameItemsFacet(address(this)).balanceOf(player, uint(FishSpecies.Goldfish)),
            GameItemsFacet(address(this)).balanceOf(player, uint(FishSpecies.Trout)),
            GameItemsFacet(address(this)).balanceOf(player, uint(FishSpecies.Catfish)),
            GameItemsFacet(address(this)).balanceOf(player, uint(FishSpecies.Tuna)),
            GameItemsFacet(address(this)).balanceOf(player, uint(FishSpecies.Swordfish)),
            GameItemsFacet(address(this)).balanceOf(player, uint(FishSpecies.Tigershark)),
            GameItemsFacet(address(this)).balanceOf(player, uint(FishSpecies.Lionfish)),
            GameItemsFacet(address(this)).balanceOf(player, uint(FishSpecies.Squid)),
            GameItemsFacet(address(this)).balanceOf(player, uint(FishSpecies.Lobster)),
            GameItemsFacet(address(this)).balanceOf(player, uint(FishSpecies.Piranha)),
            GameItemsFacet(address(this)).balanceOf(player, uint(FishSpecies.SeaHorse)),
            GameItemsFacet(address(this)).balanceOf(player, uint(FishSpecies.Pufferfish)),
            GameItemsFacet(address(this)).balanceOf(player, uint(FishSpecies.Angelfish)),
            GameItemsFacet(address(this)).balanceOf(player, uint(FishSpecies.Pogfish))
        ];
    }
}
...
contract SpawnPoolFacet is WithStorage, ReentrancyGuard, Modifiers {
    ...
    function _rewardsForFishId(uint256 fishId) private view returns (uint256) {
        uint8 species = uint8(LibFishUtils.getFishType(fishId));
