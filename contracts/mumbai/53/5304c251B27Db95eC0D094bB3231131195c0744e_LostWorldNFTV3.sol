    ...
    UUPSUpgradeable
{
    ...
    function getLostWorldLands(uint256 worldId) public view returns (LostLand[] memory) {
        ...
        require(landIds.length > 0, 'LWNFT: World does not contain any land');
    ...
    function editLand(
        ...
        if (playable) {
            if (enemyList.length == 0 && rewardList.length > 0 && rewardAmount.length > 0)
                landData[landId].playable = true;
