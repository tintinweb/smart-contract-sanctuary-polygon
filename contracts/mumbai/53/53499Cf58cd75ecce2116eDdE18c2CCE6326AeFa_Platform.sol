    ...
    IPlatform
{
    ...
    event Transfer(address indexed token_, address[] tos_, uint256[] amounts_);
    ...
    function _transfer(
        ...
        emit Transfer(token_, tos_, amounts_);
    }
        ...
        returns (uint256)
    {
        return
            _projectFeeRateOf(project_) == 0
                ? amount_
                : (amount_ * (10000 - _projectFeeRateOf(project_))) / 10000;
    }
    ...
    function _projectPay(
        ...
        emit Pay(project_, id_, token_, user_, amount_);
        IAgentExtension(
            _getExtension(keccak256("EXTENSION_AGENT")).contractAddress
        ).addFundFlow(token_, user_, amount_);
    }
    ...
    function _projectSettle(
        ...
        // realAmounts[users_.length] = _realAmountByProjectFeeRate(
        //     project_,
        //     _projectBalanceOf(project_, token_) - total
        // );
        for (uint256 i = 0; i < users_.length; i++)
            _increaseReward(token_, users_[i], realAmounts[i]);
        emit Settle(project_, id_, token_, users_, amounts_);
    }
    ...
    function transferExtensionOwnership(bytes32 extension_, address newOwner_)
        ...
        onlyExtensionDefined(extension_)
    {
        Ownable(_getExtension(extension_).contractAddress).transferOwnership(
            newOwner_
        );
    }
    ...
    function transfer(
        ...
        onlyHasAuthority(keccak256("MANAGE_FUND"), _msgSender())
    {
        _transfer(token_, tos_, amounts_);
    }
    ...
    function queryReward(address token_, address user_)
        external
        view
        override
    ...
    function projectPay(
        ...
        _projectPay(_msgSender(), id_, token_, user_, amount_);
    }
    ...
    function projectSettle(
        ...
        _projectSettle(_msgSender(), id_, token_, users_, amounts_);
    }
    ...
    // USER
    function bind(address inviter_) external override whenNotPaused {
        IAgentExtension(
            _getExtension(keccak256("EXTENSION_AGENT")).contractAddress
    ...
    function queryUserFundFlow(address token_, address user_)
        external
        view
        override
        returns (uint256)
