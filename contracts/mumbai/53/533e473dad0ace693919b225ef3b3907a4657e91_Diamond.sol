...
contract Diamond {
    constructor(address _contractOwner, address _diamondCutFacet, address _diamondLoupeFacet, address _ownershipFacet) {
        ...
        LibAccessControl.initialize(_contractOwner);
    }
    ...
    fallback() external payable {
        ...
        for (uint256 i = 0; i < roles.length; i++) {
            if(LibAccessControl.hasRole(roles[i], msg.sender)) {
                isAuthorized = true;
                break;
            }
        }
...
library LibAccessControl {
    ...
    bytes32 public constant ACCESS_CONTROL_STORAGE = keccak256("access_control.diamond.storage");
    ...
    function hasRole(bytes32 role, address account) public view returns (bool) {
        AccessControlStorage storage rs = accessControlStorage();
    ...
    function getRoleAdmin(bytes32 role) public view returns (bytes32) {
        AccessControlStorage storage rs = accessControlStorage();
        return rs.roles[role].adminRole;
    }
    ...
    /// @return ds the AccessControlStorage
