...
contract NeueWorld is Ownable,ERC1155{

    using SafeMath for uint;
    // variable to keep track of tokenid's
    ...
    uint private MAX_MINT_AMOUNT = 10;
    uint private _totalSupply = 1000;

    string private _name = "Ministry Of Design";
    string private _symbol = "MoD";

    mapping(uint8 => uint) private _batchSupply;
    mapping(uint8 => bool) private _mintableBatch;

    receive() external payable{}

    // constructor which takes baseurl, amount as arguments 
    constructor(
    ...
    function changeSaleState(bool newState) public onlyOwner{
        isSale = newState;
    }


    // function to set token url
    function setURI(string memory newuri) public onlyOwner {
        _setURI(newuri);
    }

    function approveBatch(uint8 _id) external onlyOwner{
        _mintableBatch[_id] = true;
    }

    // function to mint tokens
    function createNFT(uint8 _id, uint _amount,bytes memory _data) public payable{
            require(isSale == true,'sale has not started yet');
            require(_mintableBatch[_id] == true,'This batch token can not be minted');
            require(_batchSupply[_id] + _amount <= _totalSupply,'exceeding totalSupply');
            uint _payableAmount = SafeMath.mul(_tokenPrice, _amount);
            
            if(msg.sender != artist){
                require((SafeMath.add(balanceOf(msg.sender, _id) , _amount) <= MAX_MINT_AMOUNT),"limits is exceding than max mint account");
                require(msg.value >= _payableAmount,'amount should be greater than token price');
            }
            _mint(msg.sender ,_id, _amount, _data);
            _batchSupply[_id] += _amount;
