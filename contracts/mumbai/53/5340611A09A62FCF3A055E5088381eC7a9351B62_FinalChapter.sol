...
interface Episodes {
    function balanceOf(address _owner, uint256 _id) external view returns (uint256);
    function getBurnedTokens(address account) external view returns (uint256[] memory);
}

contract FinalChapter is Permissions, ReentrancyGuard, ERC1155Drop {

    Episodes public immutable episodes;
    uint256 private _minBurnedTokens;
    mapping(uint256 => bool) public _allowedTokens;
    mapping(uint256 => uint256) public _tokenTypes;
    mapping(address => bool) public hasMintedFinal;
    
    event MintedFinal(address account);

    constructor(
        string memory _name,
        string memory _symbol,
        address _episodesAddress,
        address _royaltyRecipient,
        uint128 _royaltyBps,
        address _primarySaleRecipient
    )
    ...
    {
        episodes = Episodes(_episodesAddress);
        _minBurnedTokens = 99;
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
    }

    function setE1AllowedTokens(uint256[][] memory tokensList) external onlyRole(DEFAULT_ADMIN_ROLE) {
        for (uint256 i = 0; i < tokensList.length; i++) {
            uint256[] memory currentTokenList = tokensList[i];
            for (uint256 j = 0; j < currentTokenList.length; j++) {
                uint256 tokenId = currentTokenList[j];
                _allowedTokens[tokenId] = true;
                _tokenTypes[tokenId] = i;
            }
        }
    }

    function minBurnedTokens() private view returns (bool) {
        uint256 burnedCount = 0;
        uint256 currentTokenType = 0;
        uint256[] memory burnedTokens = episodes.getBurnedTokens(msg.sender);
        for (uint256 i = 0; i < burnedTokens.length; i++) {
            if (_allowedTokens[burnedTokens[i]]) {
                if (burnedCount == 0) {
                    currentTokenType = _tokenTypes[burnedTokens[i]];
                } else if (_tokenTypes[burnedTokens[i]] != currentTokenType) {
                    continue;
                }
                burnedCount++;
                if (burnedCount >= _minBurnedTokens) {
                    return true;
                }
            }
        }
    ...
    function mintFinal() external nonReentrant {
        require(!hasMintedFinal[msg.sender], "You've already claimed FinalChapter!");
        uint256 numTokenIds = 990;
        bool hasAtLeastOneEpisode = false;
        for (uint256 i = 0; i < numTokenIds; i++) {
            if (episodes.balanceOf(msg.sender, i) >= 1) {
                hasAtLeastOneEpisode = true;
                break;
            }
        }
        require(hasAtLeastOneEpisode, "You must have at least one Episode in the wallet.");
        require(minBurnedTokens(), "You need to validate the 99 Episodes of the same color!");
        hasMintedFinal[msg.sender] = true;
        super._mint(msg.sender, 0, 1, "");
        emit MintedFinal(msg.sender);
    }
}