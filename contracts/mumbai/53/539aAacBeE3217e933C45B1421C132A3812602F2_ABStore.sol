...
contract ABStore is AccessControl, VRFConsumerBase {
    ...
    mapping(address => UltimatePackCommit) private ultimatePackCommits;
    ...
    //TODO: Account for rarity on accessories
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness)
        ...
        override
    {
        ultimatePackCommits[msg.sender].randomNumber = (randomness % 99) + 1;
    }
    ...
    function getCommitStatus() public view returns (bool) {
        return ultimatePackCommits[msg.sender].randomNumber != 0;
    }

    function mintRandomAngel(
        uint8 end,
        uint8 start,
        address newOwner
    ) internal {
        IABToken abTokenContract = IABToken(ABTokenDataContract);
        uint8 angelSeriesId = getRandomNumber(end, start, newOwner);
        uint8 auraRed;
        uint8 auraYellow;
        uint8 auraBlue;
        uint16 power;
        (auraRed, auraYellow, auraBlue) = getAura(angelSeriesId);
        (power) = getAngelPower(angelSeriesId);
        abTokenContract.mintABToken(
            newOwner,
            angelSeriesId,
            power,
