...
contract TheNFT is Ownable, ERC721A, ReentrancyGuard, ERC2981{
    ...
    // 1st 
    uint256 public firstSaleMintPrice    = 0.0004 ether; 
    // 2nd 
    uint256 public secondSaleMintPriceSP = 0.0004 ether;
    uint256 public secondSaleMintPriceWL = 0.00054 ether;
    uint256 public secondSaleMintPrice   = 0.0006 ether;
    // 3rd
    uint256 public thirdSaleMintPriceSP  = 0.0004 ether;
    uint256 public thirdSaleMintPriceWL  = 0.00068 ether;
    uint256 public thirdSaleMintPrice    = 0.00075 ether;
  ...
  function firstSaleMint(uint256 quantity, address to, bytes32[] calldata _merkleProof) public payable nonReentrant {
    ...
    require(firstSaleMintLimit >= totalMinted[msg.sender] + quantity, "You have no Mint left");
    
    bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
    require(MerkleProof.verify(_merkleProof, merkleRootWL, leaf),"Invalid Merkle Proof");
  ...
  function setMerkleRootSPWP(bytes32 _merkleRootSP, bytes32 _merkleRootWL) external onlyOwnerOrManager {
    merkleRootSP = _merkleRootSP;
    merkleRootWL = _merkleRootWL;
  }
  ...
  //set URI
  function setBaseURI(string calldata baseURI) external onlyOwnerOrManager {
    _baseTokenURI = baseURI;
  }
  function setHiddenBaseURI(string memory uri_) public onlyOwnerOrManager {
    revealUri = uri_;
  }
  ...
  // withdraw 
  function withdrawMoney() external onlyOwnerOrManager nonReentrant {
