...
enum GoalDayRecordRequestStatus { REQUESTED, FULFILLED }
...
contract StepsHunter is ChainlinkClient, ConfirmedOwner, AutomationCompatibleInterface {
    ...
    StepsHunterNft public stepsHunterNft;

    uint chainlinkRequestCostAmount = 100000000000000000; // 0.1 Ether
    uint stepsHunterGoalFee = 1 ether; // 1 Ether
    ...
    event FulfillResponse(bytes32 indexed _requestId, uint _stepsCount);

    constructor(ENV _env) ConfirmedOwner(msg.sender) {
        env = _env;
        // stepsHunterNft = _stepsHunterNft;
        stepsHunterNft = new StepsHunterNft();

        setChainlinkToken(0x326C977E6efc84E512bB9C30f76E30c160eD06FB); // Polygon Mumbai Link from https://docs.chain.link/docs/link-token-contracts/#mumbai-testnet
        setChainlinkOracle(0x40193c8518BB267228Fc409a613bDbD8eC5a97b3); // Polygon Mumbai oracle from https://docs.chain.link/docs/any-api/testnet-oracles/#operator-contracts
        jobId = 'ca98366cc7314957b8c012c72f05aeeb'; // Polygon Mumbai job id from https://docs.chain.link/docs/any-api/testnet-oracles/#job-ids
        fee = (1 * LINK_DIVISIBILITY) / 10; // 0,1 * 10**18 (Varies by network and job)

        i_link = LinkTokenInterface(0x326C977E6efc84E512bB9C30f76E30c160eD06FB);
        registrar = 0xDb8e8e2ccb5C033938736aa89Fe4fa1eDfD15a1d;
        i_registry = AutomationRegistryInterface(0x02777053d6764996e594c3E88AF1D58D5363a2e6);

        lostBetRecipientAddr = 0x46E40347CE742Bc0ab8c515025b6C81752220C6F;

        goalCheckInterval = 60 seconds;
    ...
    function createGoal(
       ...
       uint newGoalId = _goalIds.current();
        ...
        uint totalGoalChainlinkRequestsCostAmount = chainlinkRequestCostAmount * goalParams.durationDaysCount;
        uint totalGoalPayment = stepsHunterGoalFee + totalGoalChainlinkRequestsCostAmount + goal.betAmount;
        ...
        require(msg.value >= goal.betAmount, 'not enough balance');
    ...
    function addGoalDayRecord(
        ...
        goal.hasPendingRecordRequest = false;
        ...
        if (goal.failDaysCount >= goal.maxFailDaysCount) {
            ...
            bool isTransferred = i_link.transfer(lostBetRecipientAddr, goal.betAmount);
            require(isTransferred, "Failed to transfer bet back to user");

            // (bool isSent, bytes memory data) = payable(goal.userAddr).call{value: goal.betAmount}("");
        ...
        if (goal.dayRecordsCount == goal.durationDaysCount && goal.failDaysCount <= goal.maxFailDaysCount) {
            ...
            bool isTransferred = i_link.transfer(address(goal.userAddr), goal.betAmount);
            require(isTransferred, "Failed to transfer bet back to user");
        ...
        if (goal.dayRecordsCount == goal.durationDaysCount && goal.failDaysCount > goal.maxFailDaysCount) {
            ...
            // (bool isSent, bytes memory data) = payable(lostBetRecipientAddr).call{value: goal.betAmount}("");
            // require(isSent, "Failed to send Ether");
    ...
    function requestGoalDayRecord(uint goalId) internal returns (bytes32 requestId) {
        ...
        string memory url = string(abi.encodePacked('https://6367fe7fd1d09a8fa61ef32e.mockapi.io/goal-records/', Strings.toString(nextDayRecordId)));
        ...
        goal.hasPendingRecordRequest = true;
    ...
    function _fulfill(bytes32 _requestId, uint _stepsCount) public {
        ...
        addGoalDayRecord(goalDayRecordReq.goalId, goalDayRecordReq.requestId, goalDayRecordReq.stepsCount);
    }
    ...
    function fulfill(bytes32 _requestId, uint _stepsCount) public recordChainlinkFulfillment(_requestId) {
        ...
        emit FulfillResponse(_requestId, _stepsCount);
    }
...
contract StepsHunterNft is ERC721OnChainMetadata, Ownable {
    bytes constant goal_attribute_status_trait_type = "Duration days";
    uint constant goal_attribute_status_trait_index = 0;

    bytes constant goal_attribute_duration_days_count_trait_type = "Status";
    ...
    string constant SVG_START = '<svg viewBox="0 0 500 500" xmlns="http://www.w3.org/2000/svg" xmlns:bx="https://boxy-svg.com">';
    ...
    function getImagePart1(NftMetadataCreateParams memory nftParams) private view returns (string memory) {
        return string(abi.encodePacked(
            '<rect width="500" height="500"',
            ' style="stroke: rgb(0, 0, 0); fill: ', goalStatusStyleMap[nftParams.status], ';"',
            ' bx:origin="0.5 0.5"',
            '/>'
        ));
    }

    function getImagePart2(uint256 tokenId, NftMetadataCreateParams memory nftParams) private view returns (string memory) {
        return string(abi.encodePacked(
            TEXT_STYLE, '50">', "Goal #1", Strings.toString(tokenId), '</text>',
            TEXT_STYLE, '70">', "Status: ", goalStatusTitleMap[nftParams.status], '</text>',
            TEXT_STYLE, '90">', "Duration days: ", Strings.toString(nftParams.durationDaysCount), '</text>'
        ));
    }

    function getImagePart3(NftMetadataCreateParams memory nftParams) private pure returns (string memory) {
        return string(abi.encodePacked(
            TEXT_STYLE, '110">', "Tracked days: ", Strings.toString(nftParams.dayRecordsCount), '</text>',
            TEXT_STYLE, '130">', "Allowed failed days: ", Strings.toString(nftParams.maxFailDaysCount), '</text>',
            TEXT_STYLE, '150">', "Failed days: ", Strings.toString(nftParams.failDaysCount), '</text>'
        ));
    }

    function getImagePart4(NftMetadataCreateParams memory nftParams) private pure returns (string memory) {
        return string(abi.encodePacked(
            TEXT_STYLE, '170">', "Steps per day: ", Strings.toString(nftParams.dailyStepsCount), '</text>',
            TEXT_STYLE, '190">', "Bet amount: ", Strings.toString(nftParams.betAmount), '</text>'
        ));
    }
    ...
    function createImage(uint256 tokenId, NftMetadataCreateParams memory nftParams) private view returns (string memory) {
        ...
        return string(abi.encodePacked('data:image/svg+xml;base64,', Base64.encode(bytes(image))));
    }

    function setNftMedata(uint256 tokenId, NftMetadataCreateParams memory nftMetadata) private {
        _setValue(tokenId, key_token_name, abi.encode(abi.encodePacked("NFT #", Strings.toString(tokenId))));
