...
contract collection_contract is ERC721, Ownable {
    ...
    */
    ...
    function safeMint(
        ...
        //get default parameters from certhis smart contract

        //limit royalties to 25% per nft
        require(_nft.royalties <= 2500 && freeze_mint == false);

        //if collection is bulk detect if nft not minted
        if (collection_object.collection_full == 2) {
            require(!_exists(_nft.nft_id));
        }

        if (collection_object.check_contract != address(0)) {
            require(
                Icerthis_check(collection_object.check_contract).check(
                    _to
                ) == true
            );
        }
        ...
        if (collection_object.collection_full == 1) {
            uint256 sell_id_p_1 = _max_mint[collection_object.sell_id][
                _to
            ] + 1;
            bool return_error = false;
            if (
                (collection_object.max_mint_sell != 0 &&
                    collection_object.max_mint_sell < index_nft) ||
                (collection_object.mint_start != 0 &&
                    collection_object.mint_start > block.timestamp) ||
                (collection_object.mint_end != 0 &&
                    collection_object.mint_end < block.timestamp) ||
                collection_object.max_supply <= index_nft ||
                (sell_id_p_1 > collection_object.max_mint &&
                    collection_object.max_mint != 0)
            ) {
                return_error = true;
            }
            ...
            _max_mint[collection_object.sell_id][_to] =
                _max_mint[collection_object.sell_id][_to] +
                1;
        }

        //if collection is private
        if (collection_object.collection_type == true) {
        ...
        if (collection_object.collection_full == 2) {
            ...
            collection_object.currency_for_mint = _mint_proof.currency_for_mint;
            collection_object.price_for_mint = _mint_proof.price_for_mint;

            if (address(_mint_proof.currency_for_mint) == address(0)) {
                collection_object.mintable_activate = 2;
            } else {
                collection_object.mintable_activate = 1;
            }
        }
        //detect if is not free mint
        if (collection_object.price_for_mint > 0) {
            uint256 price_for_mint = collection_object.price_for_mint;
            uint16 type_mint = collection_object.mintable_activate;

            bool success = true;
            // if erc 20 mint
            if (type_mint == 1) {
                success = IERC20(collection_object.currency_for_mint)
                    .transferFrom(msg.sender, address(this), price_for_mint);
            }

            require(
                success ||
                    (msg.value >= collection_object.price_for_mint &&
                        type_mint != 1)
            );
            ...
            send_payouts(
                ...
                type_mint,
                collection_object.currency_for_mint
            );
        }
        ...
        _nfts[new_nft_id].collection_id = collection_object.collection_id;
        ...
        } else if (collection_object.share_royalties > 0) {
            _nfts[new_nft_id].royalties = collection_object.share_royalties;
        }
