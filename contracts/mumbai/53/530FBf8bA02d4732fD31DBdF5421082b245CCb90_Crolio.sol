...
contract Crolio {
    ...
    event BucketCreated(uint256 bucketId, string indexed bucketName, string description, address[] tokens, uint256[] weightages, address indexed creator);
    event InvestedInBucket(uint256 indexed bucketId, uint256 amountInvested, address userAddress, uint256 timestamp);
    event WithdrawnFromBucket(uint256 indexed bucketId, uint256 amountOut, address indexed investor, uint256 timestamp);
    ...
    function invest(uint256 _bucketId, uint256 _investValue, uint256 _deadline, uint8 v, bytes32 r, bytes32 s) external returns (bool) {
        ...
        emit InvestedInBucket(_bucketId, _investValue, msg.sender, block.timestamp);
        return true;
    }
    ...
    function withdraw(uint256 _bucketId) external returns (bool) {
        ...
        emit WithdrawnFromBucket(_bucketId, amountOut, msg.sender, block.timestamp);
        return true;
    }
