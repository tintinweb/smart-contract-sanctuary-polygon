...
contract FishNFT is ERC1155, Ownable, GelatoRelayContext {
    //  the Currency
    struct tokenInfo {
        IERC20 payToken;
        uint256 mintRate;
    ...
    function setSupply(uint256[] memory _range) external onlyOwner {
    ...
    function setPriceList(uint256[] memory _range) external onlyOwner {
        for (uint256 i = 0; i < _range.length; i++) {
            priceList.push(_range[i]);
        }
    }

    function resetSupplies() external onlyOwner {
    ...
    // Function to set/update URI
    function setURI(string memory newuri) public onlyOwner {
        mintUri = newuri;
        _setURI(mintUri);
    }

    // Function to set/Update Price
    function setPrice(uint256 id, uint256 price) public onlyOwner {
        priceList[id] = price;
    }

    // Function to set/Update Stock
    function setSupplies(uint256 id, uint256 stock) public onlyOwner {
        supplies[id] = stock;
    }

    // Function to set/Update Minter Tracker
    function setMinted(uint256 id, uint256 stock) public onlyOwner {
        minted[id] = stock;
    }
    ...
    // Function to execute the Smart Contract
