...
interface IXoiliumMarketplace {
    event TokenListed(address indexed seller, uint8 indexed nft, uint256 indexed tokenId, uint256 price, address allowedBuyer);
    event TokenListingUpdated(address indexed seller, uint8 indexed nft, uint256 indexed tokenId, uint256 price, address allowedBuyer);
    event ListingCanceled(address indexed seller, uint8 indexed nft, uint256 indexed tokenId);
    event TokenBought(address indexed buyer, uint8 indexed nft, uint256 indexed tokenId, Coin coin, uint256 price);
...
interface IXoiliumMarketplaceStorage is IXoiliumMarketplace {
    function setListing(uint8 nft, uint256 tokenId, Coin coin, uint256 price, address seller, uint256 endTs, address allowedBuyer) external;

    function deleteListing(uint8 nft, uint256 tokenId) external;

    function getListing(uint8 nft, uint256 tokenId) external view returns (Listing memory);

    function getListingByIdx(uint8 nft, uint idx) external view returns (Listing memory);
}


contract XM is IXoiliumMarketplace, ReentrancyGuard, Ownable, Pausable {

    using SafeMath for uint256;

    mapping(uint8 => IERC721) nftContracts;
    ...
    constructor(address marketplaceStorageAddress, address landsContractAddress, address botsContractAddress,
        ...
        nftContracts[0] = IERC721(landsContractAddress);
        nftContracts[1] = IERC721(botsContractAddress);
    ...
    function listToken(uint8 nft, uint256 tokenId, uint256 price, Coin coin, uint256 duration, address allowedBuyer) external whenNotPaused {
        ...
        require(!isTokenListedForRent(nft, tokenId), "Token already listed for sale");
    ...
    function cancelListing(uint8 nft, uint256 tokenId) external whenNotPaused {
        require(isOwnerOf(nft, tokenId, _msgSender()), "Caller is not an owner");
        require(isListed(nft, tokenId), "Token not listed");
    ...
    function updateListing(uint8 nft, uint256 tokenId, uint256 price, Coin coin, uint256 duration, address allowedBuyer) external nonReentrant whenNotPaused {
        require(isOwnerOf(nft, tokenId, _msgSender()), "Caller is not an owner");
        require(price >= 100, "Price is to loo");
        require(marketplaceStorage.getListing(nft, tokenId).seller != address(0), "Token is not listed");
        marketplaceStorage.setListing(nft, tokenId, coin, price, _msgSender(), block.timestamp + duration, allowedBuyer);
        emit TokenListingUpdated(msg.sender, nft, tokenId, price, allowedBuyer);
    }

    function buyItem(uint8 nft, uint256 tokenId) external nonReentrant whenNotPaused {
        Listing memory listing = marketplaceStorage.getListing(nft, tokenId);
        require(isOwnerOf(nft, tokenId, listing.seller), "Caller is not an owner");
        require(listing.endTs >= block.timestamp, "Listing not found");
    ...
    function transferPayment(Coin coin, uint256 price, address seller) internal returns (bool) {
    ...
    function getListing(uint8 nft, uint256 tokenId) external view returns (Listing memory) {
        Listing memory listing = marketplaceStorage.getListing(nft, tokenId);
        if (isOwnerOf(nft, tokenId, listing.seller)) {
            return listing;
        }
        revert('Not found');
    }

    function validListingExists(uint8 nft, uint256 tokenId) public view returns (bool) {
        Listing memory listing = marketplaceStorage.getListing(nft, tokenId);
        if (isOwnerOf(nft, tokenId, listing.seller)) {
            return true;
        }
    ...
    function deleteNotValidListings(uint8 nft, uint256 searchFromIdx, uint256 searchToIdx) external {
        ...
        for (uint i = searchFromIdx; i < searchToIdx; i++) {
            ...
            if (listing.seller != address(0) && !isOwnerOf(nft, listing.tokenId, listing.seller)) {
                marketplaceStorage.deleteListing(nft, listing.tokenId);
            }
        }
    }

    function isOwnerOf(uint8 nft, uint256 tokenId, address owner) internal view returns (bool) {
        return nftContracts[nft].ownerOf(tokenId) == owner;
    }

    function isListed(uint8 nft, uint256 tokenId) public view returns (bool) {
        return marketplaceStorage.getListing(nft, tokenId).seller != address(0);
    }

    function isApproved(uint8 nft, uint256 tokenId) public view returns (bool) {
        return nftContracts[nft].getApproved(tokenId) == address(this);
    }

    function isTokenListedForRent(uint8 nft, uint256 tokenId) internal view returns (bool) {
        if (nft == 0) {
    ...
    function getPlatformFee(uint256 price) internal view returns (uint256){
