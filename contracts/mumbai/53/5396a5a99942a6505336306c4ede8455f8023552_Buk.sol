...
contract Buk is ERC1155, IERC1155Receiver, Ownable, ReentrancyGuard{
    ...
    function cancelBookingUser(uint256 id, address user,string memory  nonce, bytes memory signature) external {
        ...
        if( owner() == msg.sender || msg.sender == idToHotel[bookingDetails[id].hotelId].hotelManager){
            ...
            uint256 feeHotel =  amount*cancellationFeeHotel/10000;
            uint256 feeBuk = amount*cancellationFeeBuk/10000;
            IERC20(currency).transfer(treasury, feeBuk); 
            IERC20(currency).transfer(idToHotel[bookingDetails[id].hotelId].hotelTreasury, feeHotel);
            IERC20(currency).transfer(user, amount - (feeBuk + feeHotel)); 
            }
        }
        emit BookingCancelled(user, id, bookingDetails[id].hotelId);
    }
    // to do pre check in optional
    ...
    function checkOut(uint256 id, address user) external {
        ...
        "Access Denied" );  
        require(block.timestamp > bookingDetails[id].time + 79200+ checkOutTime,"CheckOut window closed"); // to do  
        _burn( user, id, 1 ); 
        burnt[id] = true;
        IERC20(currency).transfer(idToHotel[bookingDetails[id].hotelId].hotelTreasury, // to do
        bookingDetails[id].price);
        checkedOut[id] = true;
        emit CheckedOut(id, user, bookingDetails[id].hotelId);
    }

    function checkintest(uint256 id) external {
        transferBlocked[id] = true;
    }
