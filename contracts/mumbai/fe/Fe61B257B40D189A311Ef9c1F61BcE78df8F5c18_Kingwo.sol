...
contract Kingwo is ERC721{
    ...
    function _baseURI() internal pure override returns (string memory) {
        return "https://bafybeihw5iuh7efoyy4xwl5t6ttpurdeihvulxdxk33us6acjg2ykxd7oq.ipfs.nftstorage.link/";
    }
...
contract BatchMintTest {
    Kingwo nft_contract;
    event Batch(uint256 start,uint256 end);
    constructor(address _nft_contract){
        nft_contract = Kingwo(_nft_contract);
    }
    // 铸造函数
    function batch_mint(uint256 start, uint256 end,address to) public {
        for(uint i=start;i<end;++i){
            //mint
            nft_contract.mint(to,i);
        }
        emit Batch(start,end);
    }
}
