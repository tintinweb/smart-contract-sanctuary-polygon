...
contract ChainlinkFunctions is AutomationCompatible, ChainlinkClient {
    using Chainlink for Chainlink.Request;
    ...
    function checkHasPrincipalGracePeriodExpired() private view returns (bool) {
        ...
        if (block.timestamp < repaymentschedule[0].time) return false;
    ...
    function checkHasInterestGracePeriodExpired() private view returns (bool) {
        uint[] memory interestPaymentTimes = loanContract
            .getInterestPaymentTimes();
        if (block.timestamp < interestPaymentTimes[0]) return false;
    ...
    event UpkeepPerformed(
        bool indexed _paymentPerformed,
        bool indexed _updateDefaultPerformed
    );
    ...
    function performUpkeep(bytes calldata /*e performData */) external {
        ...
        bool paymentPerformed;
        bool updateDefaultPerformed;
        LoanLibrary.DefaultInterestTracker
            memory defaultInterestTracker = loanContract
                .getDefaultInterestTracker();
        bool isInterestDue = loanContract.getDueAndUnpaidInterest() > 0;
        ...
        if (isAnyNewDefault) {
            loanContract.checkAndUpdateDefault(); // see todo
            updateDefaultPerformed = true;
        }
        emit UpkeepPerformed(paymentPerformed, updateDefaultPerformed);
    }
...
library LoanLibrary {
    ...
    function getDueAndUnpaidInterest(
        ...
        (
            uint lastLapsedInterestPaymentTime,
