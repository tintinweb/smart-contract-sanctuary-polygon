...
contract AdManager is Initializable {
    ...
    function addCampaign(
          ...
          address _oracle,
          address payable _broker) public returns (address) {

        address campaign = address(new Campaign(_reward, _spend, _metadata, _active, _payImpressionRewards, _thirdPartyValidation, _oracle, _broker));
        campaigns[msg.sender].push(campaign);
        emit campaignEvent(campaigns[msg.sender][campaigns[msg.sender].length - 1]);
        return campaign;
    }
    ...
    function setCampaign(
          ...
          bool _thirdPartyValidation,
          address _oracle,
          address payable _broker) public {
    ...
    function testUpgrade() public view returns (bool) {
        return true;
    }
 ...
 * bool true = click
 * referrer = publisher/platform
 * validator in case of impression = publisher/platform
...
contract Campaign is Accounts {
    ...
    address payable broker_address;

    // Address for our oracle
    ...
    event engagementEvent(Engagement e);

    // Trusted third party validators that must verify each engagement, typically this could for example be a trusted publishers
    OracleInterface thirdPartyValidators;

    constructor(Reward memory _reward, Spend memory _spend, string memory _metadata, bool _active, bool _payImpressionRewards, bool _thirdPartyValidation, address _oracle, address payable _broker) {
        owner = msg.sender;
        reward = _reward;
        spend = _spend;
        metadata = _metadata;
        ...
        broker_address = _broker;
        thirdPartyValidators = OracleInterface(oracle_address);
    }
     ...
     */
    function setEngagement(bool _click, string memory _token, address payable _client, address payable _referer) public {
        if ((msg.sender == owner || msg.sender == broker_address) && active == true) {
            userEngagements[_token].click = _click;
            userEngagements[_token].token = _token;
            userEngagements[_token].owner = _client;
            userEngagements[_token].referer = _referer;
            tokens.push(_token);
        }
        emit engagementEvent(userEngagements[_token]);
    }

    function validateEngagement(string memory _token, address payable _client) public returns (bool) {
        if (thirdPartyValidators.hasPublisher(msg.sender)) {
            if (userEngagements[_token].owner == _client && userEngagements[_token].referer == msg.sender) {
                userEngagements[_token].validated = true;
                return true;
            }
        }
        return false;
    ...
    /*
     * process the reward payments for all parties.
     * finally reset the engagments (allows for repeat engagements)
     */
    function processEngagements() public {
        if ((msg.sender == owner || msg.sender == broker_address) && active == true) {
            ...
            uint pltClkRwd = spend.CPC * reward.platformReward;
            ...
            uint pltImpRwd = spend.CPI * reward.platformReward;
            uint bkrImpRwd = spend.CPI * reward.brokerReward;
            for (uint i = 0; i < tokens.length; i++) {
                if (thirdPartyValidation == true && userEngagements[tokens[i]].validated != true) {
                    continue;
                }
                // add account addresses
                appendAccount(userEngagements[tokens[i]].owner);
                appendAccount(userEngagements[tokens[i]].referer);
                appendAccount(userEngagements[tokens[i]].platform);
                appendAccount(userEngagements[tokens[i]].broker);
                if (userEngagements[tokens[i]].click == true) {
                    addAccountBalance(userEngagements[tokens[i]].owner, usrClkRwd);
                    addAccountBalance(userEngagements[tokens[i]].referer, pubClkRwd);
                    addAccountBalance(userEngagements[tokens[i]].platform, pltClkRwd);
                    addAccountBalance(userEngagements[tokens[i]].broker, bkrClkRwd);
                } else if (payImpressionRewards == true) {
                    addAccountBalance(userEngagements[tokens[i]].owner, usrImpRwd);
                    addAccountBalance(userEngagements[tokens[i]].referer, pubImpRwd);
                    addAccountBalance(userEngagements[tokens[i]].platform, pltImpRwd);
                    addAccountBalance(userEngagements[tokens[i]].broker, bkrImpRwd);
                }
                delete userEngagements[tokens[i]];
            }
            delete tokens;
        }
    }
    ...
    /*
     * Iterate accounts and pay accounts.
     */
    function processPayment() public {
        if (msg.sender == owner || msg.sender == broker_address) {
            for (uint i = 0; i < getAccountsLength(); i++) {
                address payable addr = getAccountAddress(i);
                uint256 amount = getAccountBalance(addr);
                require(address(this).balance > amount, "Insufficient Contract Balance.");
                resetAccountBalance(addr);
    ...
    function setBroker(address payable _address) public {
        if (msg.sender == owner) {
            broker_address = _address;
        }
    }
...
contract Accounts {
    ...
    function appendAccount(address payable _addr) public {
        if (!exists(_addr)) {
            // Append
    ...
    function getAccountAddress(uint i) public view returns (address payable) {
        return store[i];
    }

    function exists(address payable _addr) private view returns (bool) {
    ...
    function addAccountBalance(address payable _addr, uint256 amount) public {
        if (msg.sender == owner) {
            if (exists(_addr)) {
    ...
    function resetAccountBalance(address payable _addr) public {
        if (msg.sender == owner) {
            if (exists(_addr)) {
                balance[_addr] = 0;
            }
    ...
    function getAccountBalance(address payable _addr) public view returns (uint256) {
