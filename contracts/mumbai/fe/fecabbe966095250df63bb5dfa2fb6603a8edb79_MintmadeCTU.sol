    ...
    OwnableUpgradeable
{
    ...
    event mintCostUpdated(uint256 indexed oldValue, uint256 indexed newValue);

    function initialize() public initializer {
        __ERC721_init("Cointelegraph News Clippings", "CNC");
        __Ownable_init();

        baseURI = "https://ax-api.mintmade.io/item/{id}.json";
        mintCost = 30000000000000000;
    }
    ...
    function setCost(uint256 _newCost) external onlyOwner {
        emit mintCostUpdated(mintCost, _newCost);
        mintCost = _newCost;
    }
    ...
    function mint(address _to, uint256 _id)
        ...
        payable
    {
        require(mintCost <= msg.value, "Not enough funds sent");

        _safeMint(_to, _id);

        emit LogTokenMinted(_to, _id);
    }
}
