...
library MakeTokenId {
    ...
    uint256 constant ADDITIONAL_ID = 1_000_000_000;
...
contract Cancel is IExecutePlugin, BasePlugin {
    using MakeTokenId for address;
    ISoulBound public soulBound;

    constructor(address _registry) {
        PLUGIN_VERSION = 1;
        PLUGIN_NAME = PluginsList.SAFE_DEAL_CANCEL;
    ...
    function execute(
        ...
        require(ISafeDeal(registry.safeDeal()).cancelDeal(vars),
            "Cancel: wrong exec"
        );
        makeMint(_executedId, _sender, uint256(DataTypes.UserRatesType.FOR_DEAL_GUARANTOR));

        return true;
    }

    function makeMint(bytes32 _executedId, address _user, uint256 _rateId) private {
        uint256 tokenId = _user.getSoulBoundTokenId(_rateId);
...
interface ISoulBound is IERC1155Upgradeable {
    ...
    function getTokenIdByAddressAndRate(
        address communityId,
        uint256 rateId
