...
contract token1Factory is Ownable {
    ...
    function createToken(
        ...
        address token = address(new Token1(newOwner_, name_, symbol_, decimals_, totalSupply_));
        Token1(token).transferOwnership(newOwner_);
        tokenFactoryListAddress.addToken(address(this), "Token1", token, newOwner_, name_, symbol_, decimals_, totalSupply_, _has_miner_mode, preSupply_);
    }
}