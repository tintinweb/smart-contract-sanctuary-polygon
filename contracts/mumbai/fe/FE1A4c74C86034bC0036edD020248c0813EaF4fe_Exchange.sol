...
contract Exchange is ERC20 {
  ...
  //constructor that takes the address of the crypto devs token as parameter
  // assign it to the state var cryptoDevsTokenAddress
  // make sure that the address inputed in the constructor during deployed will not be addres 0
  constructor(address _address) ERC20("CryptoDev LP Token", "CDLP") {
    require(_address != address(0), "Token address is not valid.");
    cryptoDevsTokenAddress = _address;
  }
  ...
  function addLiquidity(uint256 _amount) public payable returns (uint256) {
    ...
    if(cryptoDevsTokenReserve == 0) {
      ...
      require(_amount >= cryptoDevTokenAmount, "Amount of token is less than the minimum required");
  ...
  function removeLiquidity(uint _amount) public returns (uint, uint) {
    ...
    ERC20(cryptoDevsTokenAddress).transfer(msg.sender, cryptoDevsTokenAmount);
  ...
  function ethToCryptoDevToken(uint _minTokens) public payable {
    ...
    uint256 tokensBought = getAmountOfTokens(
      ...
      address(this).balance - msg.value, //reserved amt in the smart contract
