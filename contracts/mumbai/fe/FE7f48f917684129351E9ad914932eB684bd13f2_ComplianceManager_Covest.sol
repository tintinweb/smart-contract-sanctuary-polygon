...
contract ComplianceManager_Covest is
	Initializable,
	UUPSUpgradeable,
	ReentrancyGuardUpgradeable
{
	mapping(string => uint256) internal historyValueOfUser;
	mapping(address => mapping(address => uint256)) internal valueOfCM;
	...
	function fundInFlow(
		...
		valueOfCM[_asset][whoIsCM] += _value;
	 ...
	 *  - `msg.sender` Must be a Compliance Manager.
	...
	function withdraw(
		...
		require(whoIsCM == msg.sender, "CM: You are not CM.");
		require(
			valueOfCM[_asset][msg.sender] >= _value,
			"CM: Insufficient Funds."
		);
	 ...
	 * @param _policyId The policyId.
	 * @return The uint256 value of the balance of the policyId.
	 */
	...
	/**
	 * @notice Get balanceOfComplianceManager.
	 * @param _user The user address.
	 * @param _asset The asset address.
	 * @return The uint256 value of balanceOfComplianceManager of the user with asset.
	 */
	function balanceOfComplianceManager(address _user, address _asset)
		public
		view
		returns (uint256)
	{
		return valueOfCM[_asset][_user];
	}

	/**
	 * @notice Get whoIsComplianceManager.
	 * @return The address of the whoIsComplianceManager.
	 * @return The signature of the whoIsComplianceManager.
	 */
	function whoIsComplianceManager()
		public
		view
		returns (address, string memory)
	{
		return (whoIsCM, signature);
	}
}
