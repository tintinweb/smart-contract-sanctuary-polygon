...
mapping(uint256 => string) public tokenChainUrlMap ;
...
function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if(bytes(tokenChainUrlMap[tokenId]).length>0){
            ...
            return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, tokenId.toString(),"/",uint256(tokenLevels[tokenId]).toString())) : "";
        }
}

/*
function gettLentokenChainUr(uint256 tokenId) public view returns(uint256){
    return bytes(tokenChainUrlMap[tokenId]).length;
}

function getBaseUrl()public view returns(string memory,uint256){
    string memory baseURI = _baseURI();
    return (baseURI,bytes(baseURI).length);
}
function getUrl(uint256 tokenId) public view returns(string memory){
    string memory baseURI = _baseURI();
   return string(abi.encodePacked(baseURI, tokenId.toString(),"/",uint256(tokenLevels[tokenId]).toString()));
}*/
...
/**
        ...
        ) ERC721("medalToken","NFTMedal") public 
  {
    ...
    _baseTokenURI = "http://nftinfo.nftunit.top/";
