...
// File contracts/Authenticator/IKeyFeeBeacon.sol
...
abstract contract LamportBase is Pausable {
    using Address for address payable;

    IKeyFeeBeacon internal _keyFeeBeacon;

    bool allCallersApproved;
	...
	*/
    function isApprovedCaller(address caller) public view returns (bool) {
        return allCallersApproved || approvedCallers[caller];
    }
	...
	*/
    function _approveCaller(address caller) internal {
        require(!allCallersApproved, "LB:E1"); // Cannot approve caller when all callers are allowed
	...
	*/
    function _unapproveCaller(address caller) internal {
        require(!allCallersApproved, "LB:E2"); // LamportBaseA: Cannot unapprove caller when all callers are approved
        require(caller != _msgSender(), "LB:E3"); // LamportBaseA: Cannot unapprove self
        approvedCallers[caller] = false;
    }

    /*
		@name approveAllCallers
		@description approves all addresses to make calls to this contract
		@date Febuary 17th 2023
		@author William Doyle
	*/
    function allowAllCallers(
        bytes32[2][256] calldata publicKey,
        bytes[256] calldata signature
    )
        public
        ...
        redeemLamport(
            publicKey,
            signature,
            abi.encodePacked(Constants.ALLOW_ANY_CALLER_COMMAND)
        )
    {
        require(!allCallersApproved, "LB:E4"); // "LamportBaseA: All callers already approved"
        allCallersApproved = true;
    }

    /*
		@disallowAllCallers
		@description disapproves all addresses to make calls to this contract, only explicitly approved addresses can make calls
		@date Febuary 17th 2023
		@author William Doyle
	*/
    function disallowAllCallers(
        bytes32[2][256] calldata publicKey,
        bytes[256] calldata signature
    )
        public
        redeemLamport(
            publicKey,
            signature,
            abi.encodePacked(Constants.DISALLOW_ANY_CALLER_COMMAND)
        )
    {
        require(allCallersApproved, "LB:E5"); // LamportBaseA: All callers already disapproved
        allCallersApproved = false;
    }
    ...
    */
    function isRedeemable(bytes32 publicKeyHash) public view returns (bool) {
        return
            ...
            Constants.PublicKeyHashUsageStatus.POSTED;
    }
    ...
    modifier redeemLamport(
        ...
        require(isApprovedCaller(_msgSender()), "LB:E6"); // LamportBaseA: caller not approved
    ...
    function _addSinglePublicKeyHash(bytes32 publicKeyHashToAdd) internal {
        require(
            publicKeyHashes[publicKeyHashToAdd] ==
                Constants.PublicKeyHashUsageStatus.UNUSED,
            "LB:E10"
        ); // LamportBaseA: Public Key Hash Already Exists

        publicKeyHashes[publicKeyHashToAdd] = Constants
            .PublicKeyHashUsageStatus
            .POSTED;
        liveKeyCount++;
    }
    ...
    */
    function _removeSinglePublicKeyHash(
        ...
        publicKeyHashes[publicKeyHashToRemove] = Constants
            .PublicKeyHashUsageStatus
            .REDEEMED;
        liveKeyCount--;
    }
    ...
    */
    ...
    function togglePause(
        ...
        redeemLamport(
            publicKey,
            signature,
            abi.encodePacked(Constants.TOGGLE_PAUSE_COMMAND)
        )
    {
        if (paused()) {
            _unpause();
...
contract LambBedrockAuthenticationFactory  {
    IKeyFeeBeacon public keyFeeBeacon;
    address logic;
    event WalletCreated(address indexed wallet);

    constructor(address operatorWallet) {
        logic = address(new LambBedrockAuthentication(true));
        keyFeeBeacon = new KeyFeeBeacon(operatorWallet);
    }

    function _recordWalletDeployment(address wallet) internal {
        emit WalletCreated(wallet);
    }

    function redeem(
        Constants.WalletType _type,
        bytes32[] memory firstLamportKeys,
        address[] calldata callersToApprove
    ) public {
        bytes32 _salt = keccak256(
            abi.encodePacked(block.chainid, block.number, msg.sender)
        );

        address proxy = Clones.clone(logic);
        LambBedrockAuthentication(payable(proxy)).initialize(
            firstLamportKeys,
            callersToApprove,
            keyFeeBeacon
        );
        _recordWalletDeployment(proxy);
        return;
    }
}