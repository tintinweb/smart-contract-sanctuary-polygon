...
/**
 * @dev Staking contract to incentivize on-chain liquidity.
 * 
 * NOTE: `exit` is available only after the rewards distribution period is finished.
 */
contract StakingRewardsLockable is IStakingRewards, RewardsDistributionRecipient, ReentrancyGuard, Pausable {
    ...
    uint256 private _rewardsDuration = 180 days;
    ...
    /**
     * @dev See {RewardsDistributionRecipient-rewardsDistribution}.
     */
    function rewardsDistribution() public view override(IStakingRewards, RewardsDistributionRecipient) returns (address) {
        return RewardsDistributionRecipient.rewardsDistribution();
    }
    ...
    function rewardPerToken() public view returns (uint256) {
        ...
        return _rewardPerTokenStored.add(
    ...
    function stake(uint256 amount) external nonReentrant whenNotPaused updateReward(msg.sender) {
        require(amount > 0, "StakingRewardsLockable: cannot stake zero amount");
    ...
    function withdraw(uint256 amount) public nonReentrant updateReward(msg.sender) {
        require(block.timestamp > _periodFinish, "StakingRewardsLockable: rewards period is not finished");

        require(amount > 0, "StakingRewardsLockable: cannot withdraw zero amount");
    ...
    function notifyRewardAmount(uint256 reward) external override onlyRewardsDistribution updateReward(address(0)) {
        require(_periodFinish == 0, "StakingRewardsLockable: staking is started");
        ...
        require(_rewardRate <= balance.div(_rewardsDuration), "StakingRewardsLockable: provided reward too high");
    ...
    function recoverERC20(address tokenAddress, uint256 tokenAmount) external onlyOwner {
        require(tokenAddress != address(_stakingToken), "StakingRewardsLockable: cannot withdraw the staking token");
...
abstract contract RewardsDistributionRecipient is Ownable {
    ...
    function rewardsDistribution() public view virtual returns (address) {
