...
contract PlaylinkAirdrop is Ownable, ReentrancyGuard {
    ...
    struct AirdropCampaign {
        ...
        uint256 chargedFee; // the fee which is already charged for this campaign
        mapping(uint256 => address) winnerOf; // maps from an asset index to its corresponding winner
    }
    ...
    function getClaimableAssets(address winner, string calldata campaignId)
        external
        view
        returns (Asset[] memory)
    {
        AirdropCampaign storage campaign = _campaignById[campaignId];
        uint256 numClaimableAssets = 0;
        for (uint256 i = 0; i < campaign.assets.length; i++)
            if (campaign.winnerOf[i] == winner) numClaimableAssets++;
        Asset[] memory claimableAssets = new Asset[](numClaimableAssets);
        uint256 counter = 0;
        for (uint256 i = 0; i < campaign.assets.length; i++)
            if (campaign.winnerOf[i] == winner) {
                claimableAssets[counter] = campaign.assets[i];
                counter++;
            }
    ...
    function createAirdropCampaign(
        ...
        if (airdropMethod == AirdropMethod.AutoAirdrop)
            estimatedFee = estimateAirdropFee(assets.length);
        require(
            msg.value >= estimatedFee,
            "PlaylinkAirdrop: insufficient airdrop fee"
        );
        if (msg.value > estimatedFee) {
            (bool success, ) = payable(msg.sender).call{
                value: msg.value - estimatedFee
            }("");
        ...
        require(
            uint256(airdropMethod) <= 1,
            "PlaylinkAirdrop: invalid airdrop method"
        );
        ...
        campaign.airdropMethod = airdropMethod;
        campaign.maxBatchSize = _maxBatchSize;
        campaign.chargedFee = estimatedFee;
    ...
    function updateCampaign(
        ...
        require(
            campaign.creator == msg.sender,
            "PlaylinkAirdrop: only campaign owner can update"
        );
        ...
        // Check payment
        uint256 newEstimatedFee = 0;
        if (airdropMethod == AirdropMethod.AutoAirdrop)
            newEstimatedFee = estimateAirdropFee(assets.length);
        if (newEstimatedFee > campaign.chargedFee) {
            require(
                msg.value >= newEstimatedFee - campaign.chargedFee,
                "PlaylinkAirdrop: insufficient airdrop fee"
            );
            if (msg.value > newEstimatedFee - campaign.chargedFee) {
                (bool success, ) = payable(msg.sender).call{
                    value: msg.value + campaign.chargedFee - newEstimatedFee
                }("");
                require(success, "PlaylinkAirdrop: failed to return excess");
            }
        }
        ...
        campaign.maxBatchSize = _maxBatchSize;
        if (newEstimatedFee > campaign.chargedFee)
            campaign.chargedFee = newEstimatedFee;
    ...
    function processCampaignResult(
        string calldata campaignId,
        ...
        require(
            assetIndexes.length == winners.length,
            "PlaylinkAirdrop: lengths mismatch"
        );
        if (campaign.airdropMethod == AirdropMethod.AutoAirdrop)
            _airdropRewards(campaignId, assetIndexes, winners);
        else if (campaign.airdropMethod == AirdropMethod.WinnerClaim)
            _setRewards(campaignId, assetIndexes, winners);
    }
    ...
    function claimAssets(
        ...
        require(
            ...
            "PlaylinkAirdrop: trying to claim too many assets"
        );
        Asset[] memory claimedAssets = new Asset[](assetIndexes.length);
        for (uint256 i = 0; i < assetIndexes.length; i++) {
            require(
                campaign.winnerOf[assetIndexes[i]] == msg.sender,
                "PlaylinkAirdrop: only winner can claim"
            );
            claimedAssets[i] = campaign.assets[assetIndexes[i]];
            Asset storage asset = campaign.assets[assetIndexes[i]];
            require(
                asset.amount > 0,
                "PlaylinkAirdrop: re-claim is not allowed"
            );
            if (asset.assetType == AssetType.ERC20) {
                ...
                IERC1155(asset.assetAddress).safeTransferFrom(
                    ...
                    asset.assetId,
                    asset.amount,
                    abi.encodePacked("Claim ERC1155 assets")
                );
                asset.amount = 0;
            }
        }
    ...
    function _airdropRewards(
        ...
        require(
            assetIndexes.length <= campaign.maxBatchSize,
            "PlaylinkAirdrop: trying to airdrop too many assets"
        );
    ...
    function _setRewards(
        ...
        AirdropCampaign storage campaign = _campaignById[campaignId];
        Asset[] memory assignedAssets = new Asset[](assetIndexes.length);
        for (uint256 i = 0; i < assetIndexes.length; i++) {
            require(
                assetIndexes[i] < campaign.assets.length,
                "PlaylinkAirdrop: index out of bound"
            ...
            require(
                campaign.assets[assetIndexes[i]].amount > 0,
                "PlaylinkAirdrop: asset already claimed"
            );
            assignedAssets[i] = campaign.assets[assetIndexes[i]];
            campaign.winnerOf[assetIndexes[i]] = winners[i];
        }
