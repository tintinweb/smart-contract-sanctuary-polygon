...
contract Purchase is BaseContract, ReentrancyGuardUpgradeable {
    ///
    ...
    function buyInsurance(
        ...
        require(
            payTokens[_payToken] != address(0) && _payToken < payTokens.length,
        ...
        require(isValidSignature(msgHash, _signature), "invalid signature");

        if (payTokens[_payToken] == ETHEREUM) {
            require(msg.value == _premium, "not equal");
            (bool success, ) = payable(address(this)).call{value: msg.value}(
                "----"
            );
            if (success) {
                payable(staking).transfer(_premium.mul(stakingRate).div(100));
                payable(liquidity).transfer(
                    _premium.mul(liquidityRate).div(100)
                );
                payable(treasury).transfer(_premium.mul(treasuryRate).div(100));
            }
        } else {
            IERC20Upgradeable(payTokens[_payToken]).safeTransferFrom(
                msg.sender,
                address(this),
                _premium
            );
            IERC20Upgradeable(payTokens[_payToken]).safeTransfer(
                address(staking),
                _premium.mul(stakingRate).div(100)
            );
            IERC20Upgradeable(payTokens[_payToken]).safeTransfer(
                address(liquidity),
                _premium.mul(liquidityRate).div(100)
            ...
            IERC20Upgradeable(payTokens[_payToken]).safeTransfer(
                address(treasury),
                _premium.mul(treasuryRate).div(100)
            );
        }

        IStaking(staking).updateRewardPerFakt(
            _productId,
            _rewards.mul(stakingRate).div(100)
        );

        userOrders[msg.sender].push(orderIndex);

        Order storage order = insuranceOrders[orderIndex];

        order.buyer = payable(msg.sender);
        order.productId = _productId;
        order.amount = _amount;
        order.cover_token = _coverToken;
        order.premium = _premium;
        order.pay_token = _payToken;
        ...
        returns (Order[] memory)
    {
        ...
        for (uint256 i = 0; i < numOrders; i++) {
            orders[i] = insuranceOrders[userOrders[_user][i]];
        }
        return orders;
    }

    function getUserOrderIndexes(address _user)
        external
        view
    ...
    function setStakingAddr(address _stakingAddr) external onlyOwner {
        require(_stakingAddr != address(0), "_stakingAddr is zero");
        staking = _stakingAddr;
        emit SetStaking(_stakingAddr);
    }

    function setSurplusAddr(address _liquidityAddr) external onlyOwner {
        require(_liquidityAddr != address(0), "_liquidityAddr is zero");
        liquidity = _liquidityAddr;
        emit SetSurplus(_liquidityAddr);
    }
    ...
    function setRate(
        ...
        require(
            _stakingRate.add(_liquidityRate).add(_treasuryRate) == 100,
            "not equal 100"
        );
        stakingRate = _stakingRate;
        liquidityRate = _liquidityRate;
        treasuryRate = _treasuryRate;
        emit SetRate(stakingRate, liquidityRate, treasuryRate);
    }

    function addDivToken(address _token, uint256 _index) external onlyOwner {
        require(_token != address(0), "currency is zero");
        for (uint256 i = 0; i < payTokens.length; i++) {
            require(payTokens[i] != _token, "existed");
        }
        payTokens[_index] = _token;
        emit AddDivCurrency(_token);
    }

    function delDivToken(address _token) external onlyOwner {
        require(_token != address(0), "currency is zero");
        for (uint256 i = 0; i < payTokens.length; i++) {
            if (payTokens[i] == _token) {
                delete payTokens[i];
                emit DeleteDivCurrency(_token);
                break;
            }
        }
    }
    ...
    event SetSurplus(address indexed liquidity);
