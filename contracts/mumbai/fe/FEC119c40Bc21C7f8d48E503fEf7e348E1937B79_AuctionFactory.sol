...
contract AuctionFactory {
    ...
    event AuctionCreated(address indexed _auctionAddress, address indexed _client);

    constructor(address _admin) {
        admin = _admin ;
    }

    function createAuction(IERC20 _paymentToken, uint256 _minPrice, uint32 _noOfCopies, address _client, address _admin) external onlyAdmin returns(address) {
        Auction auction = new Auction(_paymentToken, _minPrice, _noOfCopies, _client, _admin);
...
contract Auction {
    ...
    //Declare an Event
    event AuctionCreated(address indexed _client, uint256 _minPrice, AuctionState _auctionState);
    ...
    constructor(IERC20 _paymentToken, uint256 _minPrice, uint32 _noOfCopies, address _client, address _admin) {
        ...
        client = _client;
        emit AuctionCreated(client, minPrice, auctionState);
    ...
    //Client selectBid
    function selectBid(address selectedAddress) public onlyClient {
        //require admin or admin
        Bid storage b = bids[selectedAddress];
        require(b.bidState == BidState.BIDDING, "Deal not bidding");
        b.isSelected = true;
    ...
    function endSelection() public {
        auctionState = AuctionState.VERIFICATION;
        emit BiddingEnded();
    }

    // admin page select all bids once time is up
    ...
    //Change it to success or failure accordingly
    function setBidDealSuccess(address bidder) public onlyAdmin {
    ...
    //sets bid deal to fail and payout based on a percentage
    ...
    function endVerificationFailure() public onlyAdmin {
