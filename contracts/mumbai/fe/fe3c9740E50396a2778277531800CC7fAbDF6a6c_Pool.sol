...
contract Pool is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, Core {
    ...
    /// @notice An event emitted when a deposit + stake is made to Pool
    event DepositAndStake(address indexed sender, address token, uint256 amount, uint256 liquidity, address indexed to);
    ...
    function deposit(
        ...
        if (autoStake) {
            liquidity = _deposit(asset, amount, address(this));
            uint256 pid = masterValve.getTokenPid(address(asset));
            asset.approve(address(masterValve), liquidity);
            masterValve.depositFor(pid, liquidity, to);
            emit DepositAndStake(msg.sender, token, amount, liquidity, to);
        } else {
