...
contract ApePayV1 is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    function createPayment(
        ...
        PaymentDetail memory paymentDetail = PaymentDetail(
            sender,
            token,
            amount,
            nonce_,
    ...
    /// @dev cancel payment. only sender can cancel payment after the deadline.
    function cancelPayment(uint128[] memory nonces) external nonReentrant {
        for (uint256 i = 0; i < nonces.length; i++) {
            ...
            require(paymentDetail.sender == msg.sender, "AP: not sender");
            require(paymentDetail.claimed == false, "AP: payment claimed already");
            require(block.timestamp > paymentDetail.deadline, "AP: payment not expired");
            // refund fund
            // @todo handle case of same token for multiple payment
            sendFund(paymentDetail, msg.sender);
    ...
    function verifyRecipient(
        ...
        bytes32 paymentHash = getPaymentHash(recipientEmailHash, nonce_, paymentId);

        // verify signature
        bytes32 message = ECDSA.toEthSignedMessageHash(paymentHash);
        address paymentSender = ECDSA.recover(message, signature);
        PaymentDetail memory paymentDetail = Payments[nonce_];
    ...
    function _isDelegateReceive(
        ...
        bytes32 message = ECDSA.toEthSignedMessageHash(keccak256(abi.encodePacked(nonce_)));
        address paymentRecipient = ECDSA.recover(message, recipientSignature);
    ...
    function receivePayment(
        ...
        _receivePayment(recipientEmailHash, nonce_, paymentId, signature, msg.sender);
    }
    
    // receive payment from gas payer on behalf of recipient
    /// @param recipientEmailHash keccak256 of recipient email
    /// @param nonce_ nonce of payment
    /// @param paymentId unique id generated and share to sender and recipient only
    /// @param senderSignature signature of sender to proof the recipient is valid
    /// @param recipient recipient address
    /// @param recipientSignature signature of recipient to proof the delegation is granted
    function receivePaymentDelegate(
        ...
        _isDelegateReceive(nonce_, recipient, recipientSignature);
        _receivePayment(recipientEmailHash, nonce_, paymentId, senderSignature, recipient);
    }
    ...
    function _receivePayment(
        ...
        PaymentDetail memory paymentDetail = verifyRecipient(recipientEmailHash, nonce_, paymentId, signature);
        // send fund
        // modify the paymentDetail.amount to collect fee
        paymentDetail.amount = paymentDetail.amount * feeBps / 10000; // always round down
        sendFund(paymentDetail, recipient);

        emit PaymentReceived(recipient, nonce_);
    }
