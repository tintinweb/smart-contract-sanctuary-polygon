...
contract Cyber9mumbai is ERC721Enumerable, Ownable, RNG, VRFConsumerBase {
    ...
    uint256 private constant REWARD = 100;  
    ...
    //Events
    event Attack(address indexed _from, uint256 _tokenId, uint256 _dmg, bool isCrit, int256 _enemyDmg);
    event CharacterDefeated(address indexed _from, bool _defeated);
    event EnemyDefeated(address indexed _from, bool _defeated);
    ...
    modifier status(uint256 _tokenId) {
        require(isStaked[_tokenId] == false, "Staked");
        _;
    }
    ...
    function mint(uint256 _amount) external payable {
        ...
        require(_amount > 0, "Minting 0");
        require(_amount < maxMintPerTx + 1, "Exceed max mint");
        require(amountMinted + _amount < maxSupply + 1, "Exceed max");
    ...
    function freeMint(uint256 _badgeId) external {
        require(amountMinted < maxSupply + 1, "Sold out");
    ...
    function burnCyber9(uint256 _tokenId, address _from) external { 
        require(msg.sender == enhancementContract, "Invalid addr");
    ...
    function getCharacterMaxHp(uint256 _tokenId) external view returns (int256) {
        return characterMaxHp[_tokenId];
    }
    
    function levelUp(uint256 _tokenId) external status(_tokenId) {
        require(characters[_tokenId].level < MAX_LEVEL+1, "Max lvl");
        require(characters[_tokenId].hp > 0, "Defeated");
        require(msg.sender == ownerOf(_tokenId), "Not owner");
        uint256 _expRequired = expRequired(_tokenId);
        require(characters[_tokenId].experience >= _expRequired, "Insufficient exp");
    ...
    function spawnEnemy() external {
        require(usable == true);
        require(walletOfOwner(msg.sender).length > 0, "Own 1");
        require(isSpawned[msg.sender] == false, "Spawned");
    ...
    function getEnemyStats(address _from) external view returns (int256 hp, uint256 str) {
    ...
    function attackEnemy(uint256[] memory _tokensId) external {
        ...
        for(uint256 i = 0; i<_tokensId.length;i++) {
            require(characters[_tokensId[i]].hp > 0, "Defeated");
            require(msg.sender == ownerOf(_tokensId[i]), "Not owner");
            require(isStaked[_tokensId[i]] == false, "Staked");
            require(block.timestamp > cooldown[_tokensId[i]], "CD");

            //Cooldown for attack
            setCooldown(60, _tokensId[i]);
            ...
            uint256 _enemyDmg = uint256(roll(enemy[msg.sender].strength, 2));
            emit Attack(msg.sender, _tokensId[i], _dmg, isCrit, int256(_enemyDmg));
            characters[_tokensId[i]].hp -= int256(_enemyDmg) - walletOfOwner(msg.sender).c6Bonus();
            characters[_tokensId[i]].experience += 7500 + walletOfOwner(msg.sender).c9Bonus();

            //Character defeated
            if(characters[_tokensId[i]].hp < 1) {
                characters[_tokensId[i]].hp = 0;
                setCooldown(defeatedCD, _tokensId[i]);
                emit CharacterDefeated(msg.sender, true);
            }           
            //Enemy defeated
            if(enemy[msg.sender].health <= 0 ){
                ...
                emit EnemyDefeated(msg.sender, true);
                return;
            }
        }                     
    }
    ...
    function revive(uint256 _tokenId) external {
        require(characters[_tokenId].hp == 0, "Alive");
        require(timeLeft(_tokenId) == 0, "CD");
        characters[_tokenId].hp = characterMaxHp[_tokenId];    
    }

    // ------------------------------------- ITEMS -------------------------------------
    function healCharacter(uint256 _tokenId) external {
        require(characters[_tokenId].hp > 0, "Reviving");
        characters[_tokenId].hp = characterMaxHp[_tokenId];
    ...
    function stake(uint256[] memory _tokenIds) external {
        ...
        for(uint256 i = 0; i<_tokenIds.length; i++) {
            require(characters[_tokenIds[i]].hp > 0, "Defeated");
            require(block.timestamp> cooldown[_tokenIds[i]], "CD");
            require(msg.sender == ownerOf(_tokenIds[i]), "Not owner");
            require(isStaked[_tokenIds[i]] == false, "Staked");
        
            uint256 _multiplier = characters[_tokenIds[i]].intelligence + walletOfOwner(msg.sender).c5Bonus() + characters[_tokenIds[i]].luck + walletOfOwner(msg.sender).c4Bonus();
            isStaked[_tokenIds[i]] = true;
            yieldPerToken[_tokenIds[i]] = (STAKING_TOKEN * _multiplier) + walletOfOwner(msg.sender).c8Bonus();
    ...
    function unstake(uint256[] memory _tokenIds) external {
        ...
        for(uint256 i =0; i<_tokenIds.length;i++) {
            require(msg.sender == ownerOf(_tokenIds[i]), "Not owner");
            require(isStaked[_tokenIds[i]] == true, "Not staked");
    ...
    function withdrawYield() external {
        ...
        require(toTransfer > 0 || storedBei[msg.sender] > 0, "Nothing");
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId) public virtual override status(tokenId) {
        require(characters[tokenId].hp > 0, "Defeated");
