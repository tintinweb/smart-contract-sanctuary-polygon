...
contract Factory{
    // Organzation address
    address public org;
    // Nft contracts minted
    uint public contractCounter;
    // Keeps in-track of the contracts adderesses
    mapping(uint => address) public addresses;

    // modifiers
    modifier onlyOrg(){
      require(msg.sender == org, "Unathourized Call!");
      _;
    }
    ...
    function deployNft(address owner_, string memory name, string memory symbol) external onlyOrg returns(address){
      // deploying the new NFT collection
      RewardNFT instance = new RewardNFT(owner_, org, name, symbol);
      address contractAddress = address(instance);
      // recording the new address
      addresses[contractCounter] = contractAddress;
      contractCounter ++;
...
contract RewardNFT is ERC721, Ownable{
    ...
    constructor(address _owner, address org_, string memory name, string memory symbol)
        ERC721(name, symbol)
    {
        org = org_;
        /**
        ...
        */
        ...
        // First toke will be transfered to the owner of the event
        _safeMint(_owner, 1);
        tokenCounter = 2;
