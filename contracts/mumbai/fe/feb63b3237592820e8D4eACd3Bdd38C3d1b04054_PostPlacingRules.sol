...
interface IPostPlacingRules is IBaseRulesWithPostId {
...
contract PostPlacingRules is IPostPlacingRules, Context {
    ...
    function validate(address _communityId, address _user, uint256 _nftId) external view override onlyPlugin returns(bool) {
        if (isActiveRule(_communityId, RulesList.FREE_FOR_EVERYONE)) {
            return true;
        }
        if (isActiveRule(_communityId, RulesList.PAYMENT_FROM_EVERYONE)) {
            DataTypes.PaymentInfo memory paymentInfo = ISubscription(registry.subscription()).getPaymentInfo(_nftId);
            require(
                paymentInfo.paymentType == DataTypes.PaymentType.FOR_MAKE_CONTENT
                && paymentInfo.owner == _user
                && paymentInfo.communityId == _communityId,
                "PostPlacingRules: do not have subscription token"
            );
            uint256 currentTime = block.timestamp;
            require(
                paymentInfo.startTime < currentTime &&  currentTime < paymentInfo.endTime,
                "PostPlacingRules: wrong subscription time"
            );
        }
