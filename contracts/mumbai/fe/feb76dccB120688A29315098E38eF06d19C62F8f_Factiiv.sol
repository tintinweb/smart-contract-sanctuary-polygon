...
contract Factiiv is IFactiiv, FactiivStore, AccessControl {
  ...
  bytes32 public constant override ARBITRATOR_ROLE = keccak256("ARBITRATOR_ROLE");
  bytes32 public constant override ADOPT_ROLE = keccak256("ADOPT_ROLE");

  constructor(address root, address forwarder, address _factsToken) {
    _setupRole(DEFAULT_ADMIN_ROLE, root);
    _setupRole(GOVERNANCE_ROLE, root);
    _setupRole(ADOPT_ROLE, root);
  ...
  function setUserParent(address userAddress,address desireParent) public onlyRole(ADOPT_ROLE) {
    require(_msgSender() == userBalances.userParent(userAddress), "Factiiv.setUserParent:: wrong parent");
    userBalances.balance[userAddress].parent = desireParent;
    emit UserParentSet(userAddress,desireParent);
  }
  ...
  function toForwarder(address userAddress, uint256 amount) external override {
    require(isTrustedForwarder(msg.sender), "Factiiv.toForwarder:: unauthorized");
  ...
  function initializeUser(address userAddress, address parentAddress) internal {
    require(isInitialized(parentAddress), "Factiiv.initializeUser: parent uninitialized");
  ...
  function payRewardTo(address userAddress, uint256 reward) external override {
    address sender = _msgSender();
    require(reward > 0, "Factiiv.payRewardTo:: reward is Zero");
  ...
  function createRelationship(bytes32 typeId, string memory desc, uint256 amount, address to,uint256 timeCreated) external override returns(bytes32 id) {
    address sender = _msgSender();
    if(!isInitialized(sender)) _initializeUser(sender, IOwned(trustedForwarder()).owner()); 
    if(!isInitialized(to)) _initializeUser(to, sender);
    id = _createRelationship(sender, typeId, desc, amount, to,timeCreated);
  }
  ...
  function updateRelationship(bytes32 id, Lifecycle lifecycle, string memory metadata) external override {
    ...
    if(s.lifecycle == Lifecycle.Proposed) {
      require(sender == r.to, "Factiiv.updateRelationship:: sender should be relationship Receiver");
      ...
      require(sender == r.to, "Factiiv.updateRelationship:: sender should be Receiver");
      _updateRelationship(sender, id, lifecycle, metadata);
      return;
  ...
  function deleteRelationship(bytes32 id) external override {
    require(
      ...
      "Factiiv.deleteRelationship:: not a participant, arbitrator or governance");
    require(
...
interface IFactiiv is IFactiivStore {
  ...
  event UserParentSet(address indexed userAddress,address indexed desiredParent);
  ...
  function ADOPT_ROLE() external pure returns(bytes32);
  ...
  function createRelationship(bytes32 typeId, string memory desc, uint256 amount, address to,uint256 timeCreated) external returns(bytes32 id);
...
contract FactiivStore is IFactiivStore {
    ...
    internal 
  {
    require(
      ...
      "FactiivStore.updateAttention:: unknown attestation"
    );
    Attestation storage a = attestationInfo[id];
    a.payload = payload;
  ...
  function _createRelationship(
    ...
    string memory desc,
    uint256 amount,
    address to,uint256 timeCreated
  ) 
    internal 
    returns (bytes32 id) 
  {
