  ...
  IERC721EnumerableUpgradeable 
{
  ...
  constructor(
    ...
    require(
      collectionSize_ > 0,
      "TritonNFT: collection must have a nonzero supply"
    );
    require(maxBatchSize_ > 0, "TritonNFT: max batch size must be nonzero");
    _name = name_;
    _symbol = symbol_;
  ...
  function tokenByIndex(uint256 index) public view override returns (uint256) {
    require(index < totalSupply(), "TritonNFT: global index out of bounds");
    return index;
  }
  ...
  function tokenOfOwnerByIndex(address owner, uint256 index)
    ...
    returns (uint256)
  {
    require(index < balanceOf(owner), "TritonNFT: owner index out of bounds");
    ...
    revert("TritonNFT: unable to get token of owner by index");
  }
  ...
  function balanceOf(address owner) public view override returns (uint256) {
    require(owner != address(0), "TritonNFT: balance query for the zero address");
  ...
  function _numberMinted(address owner) internal view returns (uint256) {
    require(
      owner != address(0),
      "TritonNFT: number minted query for the zero address"
    );
  ...
  {
    require(_exists(tokenId), "TritonNFT: owner query for nonexistent token");
    ...
    revert("TritonNFT: unable to determine the owner of token");
  }
  ...
  function approve(address to, uint256 tokenId) public override {
    address owner = TritonNFT.ownerOf(tokenId);
    require(to != owner, "TritonNFT: approval to current owner");

    require(
      _msgSender() == owner || isApprovedForAll(owner, _msgSender()),
      "TritonNFT: approve caller is not owner nor approved for all"
    );
  ...
  function getApproved(uint256 tokenId) public view override returns (address) {
    require(_exists(tokenId), "TritonNFT: approved query for nonexistent token");
  ...
  function setApprovalForAll(address operator, bool approved) public override {
    require(operator != _msgSender(), "TritonNFT: approve to caller");
  ...
  function safeTransferFrom(
    ...
    require(
      ...
      "TritonNFT: transfer to non ERC721Receiver implementer"
    );
  }
  ...
  function _safeMint(
    ...
    require(to != address(0), "TritonNFT: mint to the zero address");
    // We know if the first token in the batch doesn't exist, the other ones don't as well, because of serial ordering.
    require(!_exists(startTokenId), "TritonNFT: token already minted");
    require(quantity <= maxBatchSize, "TritonNFT: quantity to mint too high");
  ...
  function _transfer(
    ...
    require(
      isApprovedOrOwner,
      "TritonNFT: transfer caller is not owner nor approved"
    );

    require(
      prevOwnership.addr == from,
      "TritonNFT: transfer from incorrect owner"
    );
    require(to != address(0), "TritonNFT: transfer to the zero address");
  ...
  function _checkOnERC721Received(
    ...
    if (to.isContract()) {
      ...
      returns (bytes4 retval) {
        ...
        if (reason.length == 0) {
          revert("TritonNFT: transfer to non ERC721Receiver implementer");
        } else {
          assembly {
...
contract Triton is OwnableUpgradeable, TritonNFT, ReentrancyGuardUpgradeable {
  ...
  ) TritonNFT("Triton", "TRITON", maxBatchSize_, collectionSize_) {
