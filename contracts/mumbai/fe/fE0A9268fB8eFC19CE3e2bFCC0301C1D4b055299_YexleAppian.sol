...
contract YexleAppian is ERC721Burnable, Ownable {
    ...
    */
    function mint(address l1, address _to, uint256 _tokenId, string memory _tokenUri) external onlyAdmin{
    ...
    */
    ...
    function landDocumentViewRequestApprove(address l1, address _requester, uint tokenId, bool _status) external onlyAdmin{
        UserContract useC = UserContract(userContract);
        (bool status) = useC.verifyUser(_requester);
        if(!status){ revert userContractError();}
        if(l1 != L1Approver){ revert notL1Approver();}
        if(_status){
    ...
    function requestLandForSale(address req, uint _tokenId) external onlyAdmin{
        UserContract useC = UserContract(userContract);
        (bool status) = useC.verifyUser(req);
        if(!status){ revert userContractError();}
        if(viewAccessGranted[req][_tokenId]){
            landRequest[_tokenId][req] = true;
        }else{
    ...
    function registrationForLandByBuyer(address _requester, uint tokenId, string memory _DocumentUri) external onlyAdmin{
    ...
    function approveByL1(approverData memory _data) external onlyAdmin{
        UserContract useC = UserContract(userContract);
        (bool status) = useC.verifyUser(_data._sellingTo);
        if(!status){ revert userContractError();}
        if(_data._l1App != L1Approver){ revert notL1Approver();}
        if(voteRecord[msg.sender][_data._tokenId]){ revert alreadyApproved();}
    ...
    function approveByL2(approverDataForL2 memory _data) external{
        ...
        (bool status) = useC.verifyUser(_data._sellingTo);
        if(!status){ revert userContractError();}
        if(_data._l2App != L2Approver){ revert notL2Approver();}
