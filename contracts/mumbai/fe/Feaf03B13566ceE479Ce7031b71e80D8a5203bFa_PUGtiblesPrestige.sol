...
contract PUGtiblesPrestige is ERC721AQueryable, Ownable, ReentrancyGuard {
    ...
    function pugMint(uint256 mintAmount) external payable {
        ...
        require(lastTokenId + mintAmount < seasonSupply, "Max season supply reached");
        require(lastTokenId + mintAmount < TOTAL_SUPPLY, "Max supply reached");    
    ...
    function whiteListPugMint(uint8 mintAmount) external payable {
        uint256 lastTokenId = totalSupply();
    ...
    // Owner functions
    function changePugMintPauseState() external onlyOwner {
        pugMintPaused = !pugMintPaused;
    }

    function changePugRevealState() external onlyOwner {
        pugRevealed = !pugRevealed;
    }

    function changeIsWhiteListActiveState() external onlyOwner {
        isWhiteListActive = !isWhiteListActive;
    }

    function changeIsPublicSaleActiveState() external onlyOwner {
    ...
    function ownerMint(uint256 _mintAmount) external onlyOwner {
        ...
        require(lastTokenId + _mintAmount < TOTAL_SUPPLY, "Max supply reached");
        require(lastTokenId + _mintAmount < seasonSupply, "Max season supply reached");
    ...
    function withdraw() external onlyOwner nonReentrant{
        ...
        require(charityPay + artPay + devPay + marketingPay + communityPay == 1);
