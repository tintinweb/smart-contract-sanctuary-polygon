...
contract TangibleRevenueShare is RevenueShare, AccessControl {
    ...
    function getShare(address who) public view returns (uint256 share) {
        if (distributed > 0 && !_claimed[_snapshot][who]) {
            uint256[] memory tokens = _tokenIdsByClaimer[who];
            uint256 numTokens = tokens.length;
            for (uint256 i = 0; i < numTokens; i++) {
                share += nft.getShare(tokens[i]);
            }
        }
    }
    ...
    function claimable(address who) public view returns (uint256 amount) {
        if (distributed > 0 && !_claimed[_snapshot][who]) {
            ...
            for (uint256 i = 0; i < numTokens; i++) {
                ...
                amount += (share * distributed) / 1e9;
            }
        }
    }

    function updateClaimable(address who) private returns (uint256 amount) {
        if (distributed > 0 && !_claimed[_snapshot][who]) {
            uint256[] memory tokens = _tokenIdsByClaimer[who];
            uint256 numTokens = tokens.length;
            for (uint256 i = 0; i < numTokens; i++) {
                uint256 share = nft.updateShare(tokens[i]);
                amount += (share * distributed) / 1e9;
            }
        }
    }
...
contract PassiveIncomeNFT is ERC721Enumerable, AccessControl {
    ...
    {
        //uint start = 1651449600; // May 2nd, 2022
        uint256 start = 1649203200; // April 6th, 2022
        maxLockDuration = MAX_LOCK_DURATION;
        //boostStartTime = _currentSnapshot = start;
        boostStartTime = _currentSnapshot = start;
    ...
    function mint(
        ...
        require(block.timestamp >= boostStartTime, "too early");
        tokenId = ++_tokenIds;
