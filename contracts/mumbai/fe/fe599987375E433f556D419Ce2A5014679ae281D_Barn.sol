...
contract Barn is Ownable, IERC721Receiver, Pausable {  
    ...
    struct Constants{
        uint8 MAX_ALPHA;//= 8;
        uint256 GROAT_CLAIM_TAX_PERCENTAGE; //= 20;
        uint256 MAXIMUM_GLOBAL_GROAT; //= 2500000000 ether;
    }

    Constants constants = Constants({
        MAX_ALPHA:8,
        GROAT_CLAIM_TAX_PERCENTAGE:20,
    ...
    // number of Henchman staked in the Barn
    ...
    constructor(
        ...
        brutRelated.brutToEarnPerDay[0] = 10000;
        //....
    }
    ...
    function setMAXIMUM_GLOBAL_GROAT(uint256 _MAXIMUM_GLOBAL_GROAT)
        ...
        onlyOwner
    {
        constants.MAXIMUM_GLOBAL_GROAT = _MAXIMUM_GLOBAL_GROAT;
    }
    ...
    function levelupBrutality(uint256 tokenId) external {
        ...
        require(
            block.timestamp - barn[tokenId].levelTime >
    ...
    /**
     * adds Henchman and Merrymen to the Barn and Pack
        ...
        external
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            else _addHenchmanToBarn(account, tokenIds[i]);
        }
    }

    /**
     * adds a single Henchman to the Barn
     * @param account the address of the staker
     * @param tokenId the ID of the Henchman to add to the Barn
     */
    function _addHenchmanToBarn(address account, uint256 tokenId)
        internal
        whenNotPaused
    {
    ...
    /**
     * realize $GROAT earnings and optionally unstake tokens from the Barn / Pack
        ...
        whenNotPaused
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            else owed += _claimHenchmanFromBarn(tokenIds[i], unstake);
        }
        ...
        returns (uint256 owed)
    {
        if (contracts.warrior.getTokenTraits(tokenId).isMerry) {
            ...
            if (totalGroatEarned < constants.MAXIMUM_GLOBAL_GROAT) {
                owed =
     ...
     */
    function _claimHenchmanFromBarn(uint256 tokenId, bool unstake)
        internal
        returns (uint256 owed)
    {
        Stake memory stake = barn[tokenId];
        ...
        if (unstake) {
            ...
            _payMerrymenTax((owed * constants.GROAT_CLAIM_TAX_PERCENTAGE) / 100); // percentage tax to staked Merrymen
            owed = (owed * (100 - constants.GROAT_CLAIM_TAX_PERCENTAGE)) / 100; // remainder goes to Henchman owner
            barn[tokenId].value = uint80(block.timestamp); // reset stake
        }
    ...
    function _alphaForMerryman(uint256 tokenId) internal view returns (uint8) {
        IWarrior.AvtHtr memory temp = contracts.warrior.getTokenTraits(tokenId);
        return constants.MAX_ALPHA - temp.alphaIndex; // alpha index is 1-3
    }
    ...
    function randomMerrymanOwner(uint256 seed) external view returns (address) {
        ...
        for (uint256 i = constants.MAX_ALPHA - 3; i <= constants.MAX_ALPHA; i++) {
