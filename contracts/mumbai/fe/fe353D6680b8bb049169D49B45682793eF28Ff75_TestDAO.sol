...
contract TestDAO is ERC1155, Ownable {
    ...
    function createNewTokenId(
        ...
        _tokenIdInfo[tokenId] = TokenIdInfo(maxSupply, 0, tokenURI, true, true);
    }
    ...
    function setTokenIdInfo(
        ...
        bool tokenCanBeTransferred = _tokenIdInfo[tokenId].canBeTransferred;

        // Setting new information
        _tokenIdInfo[tokenId] = TokenIdInfo(maxSupply, tokenIdSupply, tokenURI, tokenCanBeEdited, tokenCanBeTransferred);
    }

    /**
     * @dev Disables the ability to transfer this token id
     ...
     * - can only be called by the owner of the contract
     */
    function disableTokenTransfer(uint256 tokenId) public onlyOwner existingToken(tokenId) {
        require(_tokenIdInfo[tokenId].canBeEdited == true, "Token ID cannot be edited");
        _tokenIdInfo[tokenId].canBeTransferred = false;
    }

    /**
     * @dev Enables the ability to transfer this token id
     * @param tokenId token id to enable
     *
     * Requirements:
     *
     * - `tokenId` must have been created before
     * - can only be called by the owner of the contract
     */
    function enableTokenTransfer(uint256 tokenId) public onlyOwner existingToken(tokenId) {
        require(_tokenIdInfo[tokenId].canBeEdited == true, "Token ID cannot be edited");
        _tokenIdInfo[tokenId].canBeTransferred = true;
    }
    ...
    //
    /**
     * @dev See {ERC1155-uri}. It only allows transferring a token if the flag
     *      canBeTransferred is active.
     */
    function _beforeTokenTransfer(
        ...
        // Checks whether all tokens can be transferred
        for (uint256 i = 0; i < ids.length; i++) {
            if (!_tokenIdInfo[ids[i]].canBeTransferred) {
                revert("One or more of the token ids cannot be transferred");
            }
        }
