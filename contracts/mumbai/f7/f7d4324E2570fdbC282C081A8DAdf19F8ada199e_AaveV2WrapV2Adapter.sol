...
contract AaveV2WrapV2Adapter {
    ...
    function getWrappedToken(address underlyingToken) external view returns (address aToken, string memory symbol)  {
    ...
    function getWrappedTokens(address[] memory underlyingTokens) external view returns (address[] memory aTokens, string[] memory symbols)  {
        ...
        for (uint256 i = 0; i < underlyingTokens.length; i++) {
            DataTypes.ReserveData memory reserve = pool.getReserveData(underlyingTokens[i]);
            aTokens[i] = reserve.aTokenAddress;
            symbols[i] = ERC20(reserve.aTokenAddress).symbol();
        }
    }

    function getUnderlyingToken(address wrappedToken) external view returns (address token, string memory symbol) {
    ...
    function getUnderlyingTokens(address[] memory wrappedTokens) external view returns (address[] memory tokens, string[] memory symbols) {
        ...
        for (uint256 i = 0; i < wrappedTokens.length; i++) {
            tokens[i] = IAToken(wrappedTokens[i]).UNDERLYING_ASSET_ADDRESS();
            symbols[i] = ERC20(tokens[i]).symbol();
        }
    }
