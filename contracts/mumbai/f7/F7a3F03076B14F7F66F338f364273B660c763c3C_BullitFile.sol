...
contract BullitFile is BullitFileAdmin {
    ...
    mapping(address => string[]) private fileSharedWithUser;
    ...
    function encrypt(string[] memory _uuid, uint256[] memory _fileId, string[] memory _hashFile, uint256[] memory _fileSize, string calldata _storageName, uint256 _amountToDecrypt, bool isFolder) external senderNonZeroAddress{
        ...
        (, uint256 space,) = ISubscription(subscriptionContract).isThereSubscription(msg.sender);

        if (users[msg.sender].userAddress == address(0x0)) {
            users[msg.sender].userAddress = msg.sender;        
        }

        if(checkIfTheUserGivenPeriodSubscriptionHasExpired()){
        ...
        //         if (users[msg.sender].storageSpaceUsed > space && block.timestamp > lastSubscriptionEndDate + timeLimitForDeletingFilesAfterEndSubscription) {
        //             users[msg.sender].storageSpace = space;
        //             users[msg.sender].storageSpaceUsed = 0;
        //             _deleteStorageForUser(msg.sender);
        //         }
        //         /* else if(users[msg.sender].storageSpaceUsed > space) {
    ...
    function deleteMyStorage() external senderNonZeroAddress {
        (, uint256 space,) = ISubscription(subscriptionContract).isThereSubscription(msg.sender);
        users[msg.sender].storageSpace = space;
        users[msg.sender].storageSpaceUsed = 0;
        _deleteStorageForUser(msg.sender);
    ...
    function _deleteStorageForUser(address _userAddress) internal {
        ...
        removeNumberOfFileOwnendAndSize(_userAddress, users[_userAddress].numberOfFilesOwend, users[_userAddress].storageSpaceUsed);

        filesCount -= userOwnedFiles[_userAddress].length;
    ...
    function removeItemFromArray (string[] storage _files, string memory _uuid) internal returns (bool) {
        for (uint256 i = 0; i < _files.length; i++) {
            if (keccak256(bytes(_files[i])) == keccak256(bytes(_uuid))) {
    ...
    function checkIfTheUserGivenPeriodSubscriptionHasExpired() public view returns (bool) {
        (bool isSubscription, uint256 space, uint256 lastSubscriptionEndDate) = ISubscription(subscriptionContract).isThereSubscription(msg.sender);
        if (!isSubscription) {
            if (users[msg.sender].storageSpace > space) {
                if (users[msg.sender].storageSpaceUsed > space && block.timestamp > lastSubscriptionEndDate + timeLimitForDeletingFilesAfterEndSubscription) {
