...
contract AuctionFixedPriceV2 is Initializable {
    ...
    function buyFixPrice(
        ...
        AuctionStructure.puchasing memory params = AuctionStructure.puchasing(
            ...
            _buyer,
            priceOfNFT,
            _feeStanService.ratioBuyFixedPriceVal,
            _feeStanService.ratioBuyFixedPriceDenomination,
            listingInstance.tokenId,
            AuctionStructure.Method.BUY,
            listingInstance.currency.getPaymentMethod(_currency)
        );
    ...
    function acceptOfferPvP(
        ...
        AuctionStructure.puchasing memory params = AuctionStructure.puchasing(
            ...
            _feeStanService.ratioAcceptOfferPvPVal,
            _feeStanService.ratioAcceptOfferPvPDenomination,
    ...
    function cancelOfferPvP(
        ...
        AuctionStructure.infoOffer memory infoOffer = offer.getInforOffer(
            _nftId,
            _subOfferId
        );

        uint256 fee = (_feeStanService.ratioCancelOfferPvPVal *
            infoOffer.amount) /
            10**_feeStanService.ratioCancelOfferPvPDenomination;

        AuctionStructure.stanFundParams memory stanFundInstance = stanFund.get(
            _sender
        );

        if (infoOffer.currency == AuctionStructure.Currency.CRYPTO) {
            require(
                stanFundInstance.result && stanFundInstance.userStanFund >= fee,
                "InvalidBalance()"
            );
            stanFund.set(fee, AuctionStructure.Operator.MINUS, _sender);
        }
    ...
    function transferNFTPvP(
        ...
        require(
            (paramStanFund.result &&
                paramStanFund.userStanFund >= feeStanFixed.feeTransferNFTPvP) ||
                _isWhiteList,
            "FeeExceedBalance"
        );
        ...
        require(
            listingInstance.ListingID.length == 0 ||
                listingInstance.state != AuctionStructure.StateOfListing.ACTIVE,
            "NFTisListed"
        );

        require(_maker == stanNFT.ownerOf(_tokenId), "InvalidOwnerOfNFT");

        if (!_isWhiteList)
            stanFund.set(
                feeStanFixed.feeTransferNFTPvP,
