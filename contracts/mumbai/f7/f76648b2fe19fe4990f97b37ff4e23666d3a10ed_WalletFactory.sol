...
contract Wallet is ProxyUtils {
    ...
    function initialize(
        ...
        require(
            signers.length == _signerConfigs.length,
            "Lengths of signer and signer config arrays must match."
        );
        ...
        if (_subscriptionPaymentsEnabled)
            lastSubscriptionPaymentTimestamp =
                block.timestamp -
                SUBSCRIPTION_PAYMENT_INTERVAL_SECONDS;
    }
    ...
    function _validateSignatures(
        ...
        for (uint256 i = 0; i < signatures.length; i++) {
            ...
            for (uint256 j = 0; j < i; j++)
                require(
                    signer != signers[j],
                    "Duplicate signer in signatures array."
                );
            signers[i] = signer;
            ...
            if (config.signingTimelock > 0) {
                uint256 timestamp = pendingSignatures[
                    keccak256(abi.encode(sig))
                ];
                require(timestamp > 0, "Signature not queued.");
                require(
                    timestamp + config.signingTimelock <= block.timestamp,
        ...
        require(
            _votes >= threshold,
            "Votes not greater than or equal to threshold."
        );

        // Relayer validation (if enabled)
        if (relayerWhitelistTimelock > 0 && requireRelayGuardian)
            walletFactory.checkRelayGuardian(msg.sender);
    }

    /// @notice Event emitted when a function call reverts.
    event FunctionCallReversion(
    ...
    ///     const A; // Maximum base gas recognized by `gasleft()` in `functionCall` (assume `value` > 0). Should this include some leeway for unexpected increased in return data?
    ...
    ///     const E; // Additional gas used per byte of calldata in the external function call.
    ///     const F; // Additional gas used per byte of return data in the external function call. Hopefully we don't need this one. If we do, need to create an `estimateFunctionCallGas` function to get return data length and gas usage at the same time (to avoid calls to both `eth_call` and `eth_estimateGas`). Either way, if we do need it, make sure to add some extra gas on top in case return data length goes over the simulation estimate.
    ///     let sigGas = 0;
    ///     for (const sig of signatures) sigGas += B + (signerConfig.signingTimelock > 0 ? C : 0);
    ///     return A + sigGas + E * data.length + F * returnData.length + eth_estimateGas({ from: address(wallet), to: target, data, value });
    ...
    ///     const W; // Maximum gas used per byte of calldata in the external function call.
    ...
    function functionCall(
        ...
        bytes memory packedData = abi.encode(
            ...
            address(this),
            ++nonce,
            this.functionCall.selector,
            target,
            data,
            value,
            feeData
        );
        revert(iToHex(packedData));
        bytes32 dataHash = sha256(packedData);
        _validateSignatures(signatures, dataHash, true);

        // Gas validation
        require(initialGas <= feeData[0], "Gas limit not satisfied.");
        require(tx.gasprice <= feeData[1], "maxFeePerGas not satisfied.");
        require(
            tx.gasprice - block.basefee <= feeData[2],
            "maxPriorityFeePerGas not satisfied."
        );
        ...
        if (!success) emit FunctionCallReversion(nonce, 0, 0, ret);

        // Send relayer the gas back
        msg.sender.call{
            value: ((initialGas - gasleft()) * tx.gasprice) + feeData[3]
        }("");
    }
    ...
    ///     const B; // Maximum gas used by `_validateSignatures` per signature, without `signingTimelock`s (but assume `checkRelayGuardian` for now).
    ///     const C; // Additional gas used by `_validateSignatures` per signature due to presence of `signingTimelock > 0`.
    ///     const D; // Base gas used per external function call: only necessary if we need to include some leeway for unexpected increased in return data.
    ///     let sigGas = 0;
    ///     for (const sig of signatures) sigGas += B + (signerConfig.signingTimelock > 0 ? C : 0);
    ...
    function functionCallMulti(
        ...
        bytes32 dataHash = sha256(
            abi.encode(
                ...
                address(this),
                ++nonce,
                this.functionCallMulti.selector,
                targets,
                data,
                values,
                feeData
        ...
        for (uint256 i = 0; i < targets.length; i++) {
            ...
            // If we don't have enough gas left to run the function, break now to avoid checked math reversion when subtracting 30000 from gas left
            if (gasl <= 30000) {
                emit FunctionCallReversion(
                    nonce,
                    0,
                    i,
                    "Wallet: Function call ran out of gas."
                );
                break;
            }
            ...
            if (!success) emit FunctionCallReversion(nonce, 0, i, ret);
        }
    ...
    /// @dev Requires that the sender is the zero address so that the function is read-only. (This prevents accidental use and might provide additional security, though it probably doesn't since we always revert at the end, but costs us nothing so why not?)
    function simulateFunctionCallMulti(
        ...
        require(
            ...
            "Sender must be the zero address for simulations."
        );
        ...
        // Call contracts
        for (uint256 i = 0; i < targets.length; i++) {
            // Simulate these 2 operations so we don't have to factor it into the off-chain gas calculation
            uint256 gasl = gasleft();
            if (gasl <= 30000)
                revert(
                    string(
        ...
        revert(
            string(
                abi.encodePacked(
                    "WALLET_SIMULATE_FUNCTION_CALL_MULTI_GAS_USAGE=",
                    StringsUpgradeable.toString(initialGas - gasleft())
                )
            )
        );
    }
    ...
    function functionCallBatch(
        ...
        // Input validation
        require(
            multi.length == signatures.length &&
                multi.length == signedData.length,
            "Input array lengths must be equal."
        );
        ...
        for (uint256 i = 0; i < multi.length; i++) {
            ...
            if (multi[i]) {
                ...
                uint256[] memory values;
                {
                    ...
                    require(
                        chainid == block.chainid &&
                            wallet == address(this) &&
                            _nonce == ++nonce &&
                            selector == this.functionCallMulti.selector,
                        "Invalid functionCallMulti signature."
                    );
                    ...
                    require(
                        tx.gasprice <= feeData[1],
                        "maxFeePerGas not satisfied."
                    );
                    require(
                        tx.gasprice - block.basefee <= feeData[2],
                        "maxPriorityFeePerGas not satisfied."
                ...
                uint256 value;
                {
                    ...
                    require(
                        chainid == block.chainid &&
                            wallet == address(this) &&
                            _nonce == ++nonce &&
                            selector == this.functionCall.selector,
                        "Invalid functionCall signature."
                    );
                    totalPaymasterIncentive += feeData[3];
                ...
                if (!success) emit FunctionCallReversion(nonce, i, 0, ret);
                require(minEndingGas <= gasleft(), "Gas limit not satisfied.");
            }
        }
        ...
        // Send relayer the gas back
        msg.sender.call{
            value: ((initialGas - gasleft()) * tx.gasprice) +
                totalPaymasterIncentive
        }("");
    }
    ...
    function modifySigners(
        ...
        require(
            signers.length == _signerConfigs.length,
            "Lengths of signer and config arrays must match."
        );
        require(_threshold > 0, "Vote threshold must be greater than 0.");
    ...
    function setRelayerWhitelistTimelock(uint256 _relayerWhitelistTimelock)
        external
        onlySelf
    {
        relayerWhitelistTimelock = _relayerWhitelistTimelock;
    ...
    function disableRelayerWhitelist(Signature[] calldata signatures) external {
        ...
        bytes32 dataHash = sha256(
            abi.encode(
                ...
                address(this),
                ++nonce,
                this.disableRelayerWhitelist.selector
            )
        );
        if (msg.sender != address(this))
            _validateSignatures(signatures, dataHash, false);
        ...
        if (relayerWhitelistTimelock > 0) {
            ...
            require(
                timestamp + relayerWhitelistTimelock <= block.timestamp,
                "Timelock not satisfied."
            );
        }
        ...
        require(
            ...
            "Relay whitelist already disabled."
        );
        relayerWhitelistTimelock = 0;
    }
    ...
    function queueDisableRelayerWhitelist(Signature[] calldata signatures)
        external
    {
        // Validate signatures
        bytes32 dataHash = sha256(
            abi.encode(
                ...
                address(this),
                ++nonce,
                this.queueDisableRelayerWhitelist.selector
            )
        );
        if (msg.sender != address(this))
            _validateSignatures(signatures, dataHash, false);
    ...
    /// @param signatures Only necessary if calling this function directly (i.e., not through `functionCall`).
    function unqueueDisableRelayerWhitelist(Signature[] calldata signatures)
        external
    {
        // Validate signatures
        bytes32 dataHash = sha256(
            abi.encode(
                ...
                address(this),
                ++nonce,
                this.unqueueDisableRelayerWhitelist.selector
            )
        );
        if (msg.sender != address(this))
            _validateSignatures(signatures, dataHash, false);
    ...
    function subscriptionPayments() external {
        require(
            ...
            "Subscription payments not enabled."
        );
        uint256 cycles = (block.timestamp - lastSubscriptionPaymentTimestamp) /
            SUBSCRIPTION_PAYMENT_INTERVAL_SECONDS;
        ...
        (bool success, ) = walletFactory.relayGuardianManager().call{
            value: amount
        }("");
    ...
    function setSubscriptionPaymentsEnabled(
        bool enabled,
        uint256 secondsPaidForAlready
    ) external {
        require(
            subscriptionPaymentsEnabled != enabled,
            "Status already set to desired status."
        );
        ...
        require(
            msg.sender == relayGuardian ||
                msg.sender == walletFactory.secondaryRelayGuardian() ||
                (msg.sender == address(this) &&
                    (enabled ||
                        (relayerWhitelistTimelock > 0 &&
                            relayGuardian != address(0)))),
            "Sender is not relay guardian or user enabling payments."
        );
        subscriptionPaymentsEnabled = enabled;
        if (enabled)
            lastSubscriptionPaymentTimestamp =
...
contract WalletFactory {
    ...
    function createWallet(
        ...
        require(
            msg.sender == relayGuardian,
            "Sender is not the relay guardian."
        );
        Wallet instance = Wallet(
            upgradeable
                ? payable(new Proxy{salt: bytes32(nonce)}(walletImplementation))
                : payable(
    ...
    modifier onlyRelayGuardianManager() {
        require(
            msg.sender == relayGuardianManager,
            "Sender is not the relay guardian manager."
        );
        _;
    }
    ...
    /// WARNING: If this variable is set to the zero address, the relay guardian whitelist will NOT be validated on wallets--so do NOT set this variable to the zero address unless you are sure you want to allow anyone to relay transactions.
    function setRelayGuardian(address _relayGuardian)
        external
        onlyRelayGuardianManager
    {
    ...
    /// WARNING: Even if the secondary guardian is set, if the primary guardian is not set, the `Wallet` contract does not validate that the relayer is a whitelisted guardian.
    function setSecondaryRelayGuardian(address _relayGuardian)
        external
        onlyRelayGuardianManager
    {
    ...
    function setRelayGuardianManager(address _relayGuardianManager)
        external
        onlyRelayGuardianManager
    {
        relayGuardianManager = _relayGuardianManager;
    ...
    function checkRelayGuardian(address sender) external view {
        ...
        require(
            sender == _relayGuardian ||
                sender == secondaryRelayGuardian ||
                _relayGuardian == address(0),
            "Sender is not relay guardian."
        );
    }
}