...
contract LeafNFTContract is ERC721URIStorage, Ownable2Step, EIP712, AccessControl, ReentrancyGuard{
    ...
    uint256 public constant UNITS = 10000;
    ...
    constructor() ERC721("Leaf-NFT", "Leaf")EIP712(SIGNING_DOMAIN, SIGNATURE_VERSION){
        require(msg.sender != address(0));
    }

    function payment(NFTVoucher calldata voucher) external payable nonReentrant{
        bytes32 messageHash = nftVoucherValidation(voucher);
        require(voucher.action == 0,"action error");
    ...
    function mintToken(NFTVoucher calldata voucher) external payable  nonReentrant{
        ...
        require(voucher.action == 1,"action error");
    ...
    function approve(address to, uint256 tokenId) public virtual override(ERC721)  nonReentrant{
        address owner = ERC721.ownerOf(tokenId);
        require(to != owner, "ERC721: approval to current owner");
        require(
            _msgSender() == owner || isApprovedForAll(owner, _msgSender()),
    ...
    function setApprovalForAll(address operator, bool approved) public virtual override(ERC721)  nonReentrant{
    ...
    function setMarketContract(address _address) external onlyOwner{
        require(_address != address(0), "Invalid address");
    ...
    function setNftSigner(address _address) external onlyOwner{
        require(_address != address(0), "Invalid address");
    ...
    function nftVoucherValidation(NFTVoucher calldata voucher) internal view returns(bytes32){
