...
contract MonwuPrivateSaleVesting is Ownable {
  ...
  function withdrawEther(uint256 amount) external onlyOwners {
  ...
  function investorRelease(uint256 amount) external onlyInvestor {
  ...
  function computeReleasableAmount() internal view returns(uint256) {
    ...
    totalReleasedTokens = (((block.timestamp - privateInvestor.start) / releaseDuration) + 1) * (privateInvestor.allocation / numberOfUnlocks);
...
contract MonwuPrivateSaleWhitelist is Ownable {
  ...
  function addToWhitelist(address investor, uint256 allocation, uint256 etherAmountToPay) 
    external onlyOwners nonZeroAddress(investor) uniqueInvestor(investor) {
  ...
  function editWhitelistedInvestor(address investorToEdit, uint256 editedAllocation, uint256 editedEtherAmountToPay)
    external onlyOwners nonZeroAddress(investorToEdit) {
  ...
  function removeFromWhitelist(address investor)
    external onlyOwners nonZeroAddress(investor) {
