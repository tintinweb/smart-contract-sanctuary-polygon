...
contract ContractFlashTri {
    ...
    function triangularArbitrage(
        ...
        address dummyToken;
        if (_tokens[0] != WMATIC && _tokens[1] != WMATIC && _tokens[2] != WMATIC) {
            dummyToken = WMATIC;
        } else if (
            _tokens[0] != USDT && _tokens[1] != USDT && _tokens[2] != USDT
        ) {
            dummyToken = USDT;
        } else if (
            _tokens[0] != WETH && _tokens[1] != WETH && _tokens[2] != WETH
        ) {
            dummyToken = WETH;
        } else {
            dummyToken = USDC;
