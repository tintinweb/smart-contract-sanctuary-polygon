...
contract NftFactory is Ownable, ERC721, ProxyTarget {
	...
	////nfts
	mapping(uint256 => TokenInfo) tokenInfo; // id to TokenInfo
	//COUNTS
	...
	// only owner
	function setReveal(bool revealed_) public onlyOwner {
		revealed = revealed_;
	}
	...
	function setContract(address _gameAddress) external onlyOwner {
		game = IGameEngine(_gameAddress);
	}
	...
	function tokenURI(uint256 tokenId) public view override(ERC721) returns (string memory) {
		...
		string memory nftTypeStr = tokenInfo[tokenId].nftType == 0 ? "bad" : "good";
		return
			bytes(currentBaseURI).length > 0
				? string(abi.encodePacked(currentBaseURI, nftTypeStr, uint256(tokenInfo[tokenId].level).toString(), baseExtension))
				: "";
	}
	...
	function mint(
		...
		for (uint256 i = 0; i < tokenAmount; i++) {
			...
			tokenInfo[tokenId_.current()].level = 1;
			tokenInfo[tokenId_.current()].nftType = 0;
			tokenInfo[tokenId_.current()].owner = receiver;
			amount[0] += 1;
			// good type mint
			tokenId_.increment();
			_safeMint(receiver, tokenId_.current());
			tokenInfo[tokenId_.current()].level = 1;
			tokenInfo[tokenId_.current()].nftType = 1;
			tokenInfo[tokenId_.current()].owner = receiver;
			amount[1] += 1;
		}
	}
	...
	function mintBatch(
		...
		require(tokenAmount.length == receiver.length, "Invalid input");
        for (uint256 j = 0; j < receiver.length; j++) {
            for (uint256 i = 0; i < tokenAmount[j]; i++) {
				// bad type mint
                tokenId_.increment();
                _safeMint(receiver[j], tokenId_.current());
                tokenInfo[tokenId_.current()].level = 1;
				tokenInfo[tokenId_.current()].nftType = 0;
                tokenInfo[tokenId_.current()].owner = receiver[j];
				amount[0] += 1;
				// good type mint
				tokenId_.increment();
                _safeMint(receiver[j], tokenId_.current());
    ...
    function restrictedChangeNft(uint tokenID, uint8 nftType, uint8 level) external  {
        ...
        if(tokenInfo[tokenID].nftType != nftType) {
            amount[nftType]++;
            amount[tokenInfo[tokenID].nftType]--;
        }

		tokenInfo[tokenID].nftType = nftType;
		tokenInfo[tokenID].level = level;
    }

	function tokenOwnerSetter(uint256 tokenId, address owner_) external {
		require(msg.sender == address(game));
		tokenInfo[tokenId].owner = owner_;
	}

	function burnNFT(uint256 tokenId) external {
		require(msg.sender == address(game), "Not GameAddress");
		_burn(tokenId);
	}
	...
	function _transfer(
		...
		if (to != address(game) && to != tokenInfo[tokenId].owner) {
	...
	function isApprovedForAll(address owner, address operator) public view virtual override returns (bool) {
		return (operator == address(game)) || super.isApprovedForAll(owner, operator);
	}
}
