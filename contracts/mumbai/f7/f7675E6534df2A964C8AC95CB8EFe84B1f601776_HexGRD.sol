...
import "../common/meta-transactions/AccessControlMixin.sol";
    ...
    ContextMixin
{
    ...
    bytes32 public constant STICKERBOOK    = keccak256(   "STICKERBOOK");
    ...
    event RevealURIChanged(string _revealURI);
    ...
    function setTokenRevealURI(string calldata _revealURI) external only(CONTRACT_ADMIN) {
        tokenRevealURI = _revealURI;
    ...
    function stickerBurn(uint16[] memory tokenIds) external onlyRole(STICKERBOOK) {
        for (uint pos = 0; pos < tokenIds.length; pos++) {
            _burn(uint256(tokenIds[pos]));
        }

    }

    function exists(uint256 tokenId) external view returns (bool) {
        return (_exists(tokenId) || withdrawnTokens[tokenId]);
    }
