...
contract Marketplace is ReentrancyGuard, UtilsCommon {
    ...
    // - Before import item, need to request user setAppovalForAll NFT first
    function importItem(
        address nftAddress,
        address payable owner,
        uint256 tokenId,
    ...
    function sellItem(uint256 id, uint256 price) external {
        ...
        if (item.price != price) {
            itemList[id].price = toWei(price);
        }
        itemList[id].onSale = true;
        _onSaleItemCount.increment();
        emit SellItem(id, item.nftAddress, item.tokenId, _owner, price);
    }

    function cancelSell(uint256 id) external {
        Item memory item = itemList[id];
        address _owner = item.owner;
        require(msg.sender == _owner, "Owner does not owned NFT");
        itemList[id].onSale = false;
        _onSaleItemCount.decrement();
        emit CancelSellItem(id, item.nftAddress, item.tokenId, _owner);
    }

    function getMyNFTs(uint256 pageSize, uint256 currentPage)
        external
        view
        returns (
            Item[] memory,
        ...
        )
    {
        ...
        if (balance <= 0) {
            return (new Item[](0), pageSize, currentPage, 0);
        }
        ...
        Item[] memory myNFTs = new Item[](end - start + 1);
    ...
    function getMyNFT(uint256 itemId) external view returns (Item memory) {
        require(
            msg.sender == itemList[itemId].owner,
            "Owner does not owned NFT"
        );
        return itemList[itemId];
    }

    function getOnSaleNFTs(uint256 pageSize, uint256 currentPage)
        external
        view
        returns (
            Item[] memory,
    ...
    {
        if (_onSaleItemCount.current() <= 0) {
            return (new Item[](0), pageSize, currentPage, 0);
        }
        validatePage(pageSize, currentPage, _onSaleItemCount.current());
        uint256 end = Math.min(
            currentPage * pageSize,
            _onSaleItemCount.current()
        );
        uint256 start = Math.max(currentPage * pageSize - pageSize + 1, 1);

        Item[] memory onSaleNFTs = new Item[](end - start + 1);
    ...
    function purchaseNFT(uint256 id) external payable nonReentrant {
        Item memory item = itemList[id];
        ...
        bool sent = payable(item.owner).send(item.price);
        require(sent, "Failed to send ETH to seller");
        bool sentFee = payable(feeReciever).send(totalPrice - item.price);
        require(sentFee, "Failed to send ETH to marketplace");

        ERC721 nft = ERC721(item.nftAddress);
        nft.safeTransferFrom(item.owner, msg.sender, item.tokenId);
        ...
        emit PurchaseItem(item.id, oldOwner, msg.sender, totalPrice);
    }

    function _getFeePercent() external view returns (uint256) {
        return feePercent;
    }

    function toWei(uint256 number) public pure returns (uint256) {
