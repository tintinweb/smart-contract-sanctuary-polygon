...
contract Receipt is My20, AccessControl,ReentrancyGuard, Initializable{
    ...
    function initialize(address _admin, address operator, string memory name_, string memory symbol_) external initializer{
        Admin = IAdmin(_admin); 
        _name = name_;
        _symbol = symbol_;
    ...
    **/
    ...
    function mint (address to , uint amount) external nonReentrant onlyRole(MINTER_ROLE){
        _mint(to,amount);
    ...
    @dev grant _operator role 
    @param _operator address who will get minter role
     **/

    function setMinter(address _operator) external onlyRole(DEFAULT_ADMIN_ROLE){
        _setupRole(MINTER_ROLE, _operator);
    }

    /** 
    Note only admin can call it 
    @dev grant _operator role 
    @param _operator address who will get burner role
     **/

    function setBurner(address _operator) external onlyRole(DEFAULT_ADMIN_ROLE){
        _setupRole(BURNER_ROLE, _operator);
    }
    ...
    **/
    ...
    function burn (address from, uint amount ) external nonReentrant onlyRole(BURNER_ROLE){
        _burn(from , amount);
    }
 ...
 */
contract My20 is Context, IERC20, IERC20Metadata {
