...
contract silver is ERC721URIStorage, Ownable{
    ...
    function addNewTemplate(Schema memory _newTemplate) external onlyOwner {
        templateCount++;
        uint256 templateId = templateCount * BASE_TEMPLATE_ID;
        metaData[templateId]= _newTemplate;
        emit TemplateCreated(templateId, block.timestamp);
    }

    /**
     * @dev Updates points and extenal link of the token.
     */
    function updateMetadata(uint256 _tokenId, mutableSchema memory _updateData) external onlyOwner{
        ...
        metaData[_tokenId].points = _updateData.points;
        metaData[_tokenId].externalLink = _updateData.externalLink;
    ...
    function templateData(uint256 _templateId) external view returns(Schema memory){
        require(
            templateCount >= (_templateId/BASE_TEMPLATE_ID),
    ...
    function mint(address _to,uint256 _templateId) public onlyOwner returns(uint){
        require(
            templateCount * BASE_TEMPLATE_ID >= _templateId, 
            "silver: Invalid template Id"
        );
        require(
            totalMintedNFTs[_templateId] < BASE_TEMPLATE_ID, 
     ...
     */
    function batchMint(address _to, uint256 _templateId, uint256 _totalNFTs) public onlyOwner{
