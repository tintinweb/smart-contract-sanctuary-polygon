...
contract UplusERC1155 {

   
    // ERC1155Token[] public tokens; //an array that contains different ERC1155 tokens deployed
    
    mapping(uint256 => ERC1155Token) public erc1155Tokens; //index to contract address mapping
    ...
    */
    function deployERC1155(uint256 _tokenId, string memory _contractName, string memory _uri, uint[] memory _ids, string[] memory _names) public returns (address) {
 
        ERC1155Token t = new ERC1155Token(_contractName, _uri, _names, _ids);

        erc1155Tokens[_tokenId] = t;
        indexToContract[ _tokenId ] = address(t);
        indexToOwner[ _tokenId ] = tx.origin;
    ...
    */
    function mintERC1155(uint256 _tokenId, string memory _name, uint256 amount) public {

        // collection 내에서 id ...
        ERC1155Token t = erc1155Tokens[_tokenId];
        uint id = getIdByName(_tokenId, _name);
        t.mint(indexToOwner[_tokenId], id, amount);
        emit ERC1155Minted( t.owner(), address(t), amount);
    }

    function getIdByName(uint256 _tokenId, string memory _name) public view returns (uint) {

        ERC1155Token t = erc1155Tokens[_tokenId];
        return t.nameToId(_name);

    }

    /**
    *  collection 내부에서, id 가 숫자 1, 2, ... 뭐 이런 식으로 됩니다.
    *  우리는 collection 처리를 하지 않음에 따라 ... 그냥 숫자로 합니다.
     */
    function mintERC1155Ext (uint256 _tokenId, string memory _tokenName, uint256 _mintAmount, string memory _tokenUri ) public {
        ...
        ERC1155Token t = new ERC1155Token(_tokenName, _tokenUri, _names, _ids);
        indexToContract[ _tokenId ] = address(t);
        indexToOwner[_tokenId] = tx.origin;
        erc1155Tokens[_tokenId] = t;
        erc1155Tokens[_tokenId].mint ( tx.origin, _tokenId, _mintAmount );

        emit ERC1155Minted( tx.origin, address(t), _mintAmount );
    ...
    */
    function getCountERC1155byIndex(uint256 _tokenId, uint256 _id) public view returns (uint amount) {

        ERC1155Token t = erc1155Tokens[_tokenId];
        return t.balanceOf(indexToOwner[_tokenId], _id);
        // return tokens[_index].balanceOf(indexToOwner[_index], _id);
    }

    function getCountERC1155byName(uint256 _tokenId, string calldata _name) public view returns (uint amount) {

        ERC1155Token t = erc1155Tokens[_tokenId];
        uint id = getIdByName( _tokenId, _name);
        return t.balanceOf(indexToOwner[ _tokenId ], id);
    }


    function getNameById(uint _tokenId, uint _id) public view returns (string memory) {

        ERC1155Token t = erc1155Tokens[_tokenId];

        return t.idToName(_id);
    }

    function getERC1155byIndexAndId(uint _tokenId, uint _id)
        public
        view
        returns (
            address _contract,
        ...
        )
    {
        ...
        return (address(t), t.owner(), t.uri(_id), t.balanceOf(indexToOwner[ _tokenId ], _id));
    }
}
...
contract ERC1155Token is ERC1155, Ownable {
    ...
    string[] public contentNames; //string array of names
    uint[] public contentIds; //uint array of ids
    ...
    */
    function createMapping() private {
        ...
        for (uint id = 0; id < contentIds.length; id++) {
            nameToId[contentNames[id]] = contentIds[id];
            idToName[contentIds[id]] = contentNames[id];
        }
    }
