 ...
 */
  ...
  function upgradeToSuperToken(ISuperToken superToken, uint256 amount)
    external
  {
    require(msg.sender == treasury, 'Only treasury can upgrade to super token');
    superToken.upgrade(amount);
  }

  function downgradeToERC20Token(ISuperToken superToken, uint256 amount)
    external
  {
    require(
      msg.sender == treasury,
      'Only treasury can downgrade to ERC20 token'
    );
