...
library MarketPlaceBorrowerLib {
    ...
    function _borrowTokensFromPool(
        ...
        console.log("ths is amount in MP browwer", _amount);
        if (_amount == 0) {
        ...
        console.log("THis is scaled borrow amount", scaledBorrowAmount);
        // borrowing xTokens from bb pool
        uint actualBorrowAmount = IERC4626StakingPool(
            _storageParams.poolToBorrowFrom
        ).borrow(_storageParams.allowedMarketPlace, scaledBorrowAmount);

        address xToken = IERC4626StakingPool(_storageParams.customVault)
            .xToken();
        IERC20(xToken).approve(_storageParams.customVault, scaledBorrowAmount);
        ...
        IERC20(stakeToken).transfer(msg.sender, scalDownBorrowAmount);
    ...
    function _repayToCustomVault(
        ...
        // address xToken = IERC4626StakingPool(_storageParams.customVault)
        //     .xToken();
        IERC20(stakeToken).approve(_storageParams.customVault, _amountToRepay);
        uint lastprofitofgauge = IMarketplaceBorrowerMeta(address(this))
            .lastProfitSentToGauge(); // TODO : can be passed as variable from calling function
        ...
        ICustomVault(_storageParams.customVault).repay(
            _storageParams.poolToBorrowFrom,
            _amountToRepay - lastprofitofgauge
        );

        if (lastprofitofgauge > 0) {
    ...
    AccessControl
{
    ...
    function borrowTokens(
        ...
        // TODO: borrow position will not be correct, fix this as function may returns wrong decimaled value
    ...
    function buyPropertyTokens(
        ...
        address stakeToken = ICustomVault(_storageParams.customVault)
            .stakeToken();
        _storageParams._repayToCustomVault(_repayAmount);
...
contract PositionManager {
    ...
    function calculateAndNotifyProfitOrLoss(
        ...
        if (_position.profitEarned > _position.lossFilled) {
            console.log("Yes mate 1 ", _position.profitEarned);
            ...
            console.log("Yes mate 2 ", lossToRealize);
