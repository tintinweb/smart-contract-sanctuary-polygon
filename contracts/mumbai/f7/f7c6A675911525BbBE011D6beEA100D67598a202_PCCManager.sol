...
/**
 * @title EnverX: PCCFactory smart contract
    ...
    BasicMetaTransaction
{
    ...
    */
    ...
    /**
        @notice getChildAddressBeforeDeployment: View function to fetch batch 
                contract before deployment
        @dev Uses CREATE2 opcode for analysing contract address before it is deployed
        @param _byteCode Byte-code of the PCC contract
        @param _salt Salt given at the time of creation/deployment
        @return address Batch smart contract address
     */
    ...
    /**
        @notice getBytecode: View function to fetch byte-code of batch contract
        @dev Bytecode is what gets stored on the Ethereum network and 
            executed when we interact with smart contracts
        @param _tokenName Batch contract's token name
        @param _tokenSymbol Batch contract's token symbol
        @return bytes Byte-code of PCC smart contract
     */
    function getBytecode(
        ...
        bytes memory _bytecode = type(PCCBatch).creationCode;
        return
            abi.encodePacked(
                _bytecode,
                abi.encode(
    ...
    function createNewBatch(
        ...
        PCCBatch(_batchAddress).mint(_batchOwner, _batchSupply);
    ...
    function grantManagerRoleForBatch(
        ...
        PCCBatch(_batchId).grantRole(EVX_MANAGER_ROLE, _address);
    }
    ...
    function _checkBeforeMintNewBatch(
        ...
        require(
            ...
            "_checkBeforeMintNewBatch: pccManagerContract / batchOwner cannot be zero"
        );
        require(
            (_projectId != 0) && (_commodityId != 0),
            "_checkBeforeMintNewBatch: Project Id / Commodity Id can not be zero"
        );
        require(
            (_batchSupply != 0) &&
                (_deliveryYear != 0) &&
            ...
            "_checkBeforeMintNewBatch: Batch supply / Delivery Year / UniqueIndentifier / BatchURI cannot be zero"
        );
    }
    ...
    function _createNewBatch(
        ...
        require(
            address(pccManagerContract) != address(0),
            "_createNewBatch: pccManagerContract cannot be zero"
        );
        PCCBatch _newChildBatch = new PCCBatch{salt: bytes32(_salt)}(
            _tokenName,
            _tokenSymbol,
            address(this),
            address(pccManagerContract)
        );
    ...
    ) internal onlyRole(EVX_MANAGER_ROLE) {
...
/**
 * @title EnverX: PCCBatch smart contract
 * @author EnverX Blockchain Engineering Team
 * @notice This Contract Is Used For Handling PCCBatch
 */

contract PCCBatch is ERC20, AccessControl, BasicMetaTransaction {
    /**
     * @notice Declaring access based roles
     */
    bytes32 public constant EVX_MANAGER_ROLE = keccak256("EVX_MANAGER_ROLE");
    ...
    function batchTransfer(
        ...
        require(
            ...
            "batchTransfer: Uneven length of arguments"
        );
...
import "@openzeppelin/contracts-upgradeable/proxy/utils/UUPSUpgradeable.sol";
import "../helper/BasicMetaTransaction.sol";
import "../interfaces/IPCCBatch.sol";
    ...
    BasicMetaTransaction
{
    ...
    */
    ...
    function mintMoreInABatch(
        ...
        IPCCBatch(_batchId).mint(_batchOwner, _amountToMint);
    ...
    function burnFromABatch(
        ...
        IPCCBatch(_batchId).burn(_batchOwner, _amountToBurn);
    ...
    function manyToManyBatchTransfer(
        ...
        require(
            ...
            "manyToManyBatchTransfer: Uneven length of arguments"
        );
    ...
    function _checkBeforeMintMoreAndBurnMore(
        ...
        require(
            (_projectId != 0) && (_commodityId != 0),
            "_checkBeforeMintMoreAndBurnMore: Project Id / Commodity Id can not be zero"
        );
        require(
            _amountToMintOrBurn != 0,
            "_checkBeforeMintMoreAndBurnMore: Amount can not be zero"
        );
        require(
            _batchId != address(0),
            "_checkBeforeMintMoreAndBurnMore: batch address can not be zero"
        );
    }
    ...
    function _checkBeforeUpdatingBatchDetails(
        ...
        require(
            (_projectId != 0) && (_commodityId != 0),
            "_checkBeforeUpdatingBatchDetails: Project Id / Commodity Id can not be zero"
        );
        require(
            _batchId != address(0),
            "_checkBeforeUpdatingBatchDetails: batch address can not be zero"
        );
    }
