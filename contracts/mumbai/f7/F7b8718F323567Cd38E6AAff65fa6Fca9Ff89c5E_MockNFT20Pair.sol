...
import "./TestFixture_NFT20_ERC20.sol";
...
contract MockNFT20Pair is ERC20, IERC721Receiver, ERC1155Receiver {
  ...
  modifier flashloansEnabled() {
    require(IFactory(factory).flashLoansEnabled(), "flashloans not allowed");
    _;
  }
  ...
  function withdraw(
    ...
    if (nftType == 1155) {
      if (_tokenIds.length == 1) {
        ...
        _withdraw1155(address(this), recipient, _tokenIds[0], amounts[0]);
      } else {
        _batchWithdraw1155(address(this), recipient, _tokenIds, amounts);
      }
    } else if (nftType == 721) {
  ...
  function _batchWithdraw1155(
    ...
    IERC1155(nftAddress).safeBatchTransferFrom(_from, _to, ids, amounts, "0x0");
  }
  ...
  function multi721Deposit(uint256[] memory _ids, address _referral) public {
    ...
    for (uint256 i = 0; i < _ids.length; i++) {
      IERC721(nftAddress).transferFrom(msg.sender, address(this), _ids[i]);
    }

    address referral = _referral == address(0x0) ? factory : _referral;

    _mint(msg.sender, ((nftValue * (_ids.length)) * (uint256(100) - (fee))) / (100));
    _mint(referral, ((nftValue * (_ids.length)) * (fee)) / (100));
  }
  ...
  function swap1155(
    ...
    IERC1155(nftAddress).safeBatchTransferFrom(address(this), msg.sender, out_ids, out_amounts, "0x0");
  ...
  function onERC721Received(
    ...
    address referral = bytesToAddress(data) == address(0x0) ? factory : bytesToAddress(data);

    _mint(referral, (nftValue * (fee)) / (100));
    _mint(operator, (nftValue * (uint256(100) - (fee))) / (100));
  ...
  function onERC1155Received(
    ...
    if (keccak256(data) != keccak256("INTERNAL")) {
      ...
      _mint(referral, ((nftValue * (value)) * (fee)) / (100));
      _mint(operator, ((nftValue * (value)) * (uint256(100) - (fee))) / (100));
    }
  ...
  function onERC1155BatchReceived(
    ...
    if (keccak256(data) != keccak256("INTERNAL")) {
      ...
      _mint(operator, ((nftValue * (qty)) * (uint256(100) - (fee))) / (100));
      _mint(referral, ((nftValue * (qty)) * (fee)) / (100));
    }
  ...
  function flashLoan(
    ...
    if (nftType == 1155) {
      IERC1155(nftAddress).safeBatchTransferFrom(address(this), _operator, _ids, _amounts, "0x0");
    } else {
      for (uint8 index; index < _ids.length; index++) {
        IERC721(nftAddress).safeTransferFrom(address(this), _operator, _ids[index]);
      }
    }
    require(IFlashLoanReceiver(_operator).executeOperation(_ids, _amounts, msg.sender, _params), "Execution Failed");

    if (nftType == 1155) {
      IERC1155(nftAddress).safeBatchTransferFrom(_operator, address(this), _ids, _amounts, "INTERNAL");
    } else {
      for (uint8 index; index < _ids.length; index++) {
        IERC721(nftAddress).transferFrom(_operator, address(this), _ids[index]);
      }
    }
  }
}
