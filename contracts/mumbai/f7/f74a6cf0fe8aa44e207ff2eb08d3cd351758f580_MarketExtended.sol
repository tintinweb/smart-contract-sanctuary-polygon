...
contract MarketExtendedCore is AccessControlledAndUpgradeableModifiers, MarketStorage, IMarketExtendedCore {
  ...
  function initializePools(InitializePoolsParams memory params) external override initializer returns (bool initializationSuccess) {
    ...
    require(uint256(initialAssetPrice) < type(uint128).max && initialAssetPrice > 0, "bad oracle price"); // Be double sure the chainlink price isn't broken -uint128 is big enough for any reasonable price feed.
...
contract MarketExtended is MarketExtendedCore, IMarketExtended {
  constructor(
    ...
    IRegistry _registry,
    uint256 _initialPoolTokenPrice
  ) MarketExtendedCore(_paymentToken, _registry, _initialPoolTokenPrice) {}
