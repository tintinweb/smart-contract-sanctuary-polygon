...
contract Rewarder is IRewarder, SybelAccessControlUpgradeable, ContentBadges, ListenerBadges, PushPullReward {
    ...
    event VariableTest(uint256 tpu, uint256 contentBadge, uint256 earningFactor, uint256 rewardForContentType);
    event RewardTest(string qualifier, uint256 reward);
    ...
    function balanceTest(
        ...
        (uint256 earningFactor, bool hasOnePaidFraktion) = earningFactorForListener(
            fraktionTypes,
            listener,
            contentId
        );
        ...
        emit VariableTest(tokenGenerationFactor, contentBadge, earningFactor, rewardForContentType);
        ...
        if (hasOnePaidFraktion) {
            ...
            emit RewardTest("contentPool", contentPoolReward);
        ...
        emit RewardTest("user", userReward);
        emit RewardTest("owner", ownerReward);
