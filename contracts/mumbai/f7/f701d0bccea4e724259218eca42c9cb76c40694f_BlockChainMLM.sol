...
contract BlockChainMLM is Ownable{
  ...
  function getBonusOfRate(uint levelCounter, uint256 amount) public view returns(uint256){
    return amount.mul(rates[levelCounter]).div(100);
  }
  ...
  function _payout(ERC20 token, address uplineAddress, uint256 amount, uint levelCounter) internal{
    ...
    token.transfer(uplineAddress, getBonusOfRate(levelCounter, amount));
