...
interface IInitialReSellerExt is IInitialReSeller{
    function saleData(ITangibleFractionsNFT ftnft) external view returns(FractionSaleData memory);
}
...
contract Marketplace is Exchange, IMarketplace, IERC721Receiver {
    ...
    ) external override returns(ITangibleFractionsNFT ftnft, uint256 tokenToSell){
    ...
    function buyFraction(
        ...
        if(existing.seller == initialReSeller){
            require(IInitialReSellerExt(initialReSeller).saleData(ftnft).endTimestamp > block.timestamp,"IREST");
        ...
        uint256 _sellFee = feesPerCategory[ftnft.tnft()] == 0 ? 250 : feesPerCategory[ftnft.tnft()];
        if (_sellFee > 0 && factory.fractionToTnftAndId(ftnft).initialSaleDone) {
    ...
    function setFeeForCategory(ITangibleNFT tnft, uint256 fee) external onlyAdmin{
    ...
    function _lockTngbl(
        ...
        uint256 lockedTNGBL = exchange(
            ...
            address(TNGBL),
            lockedAmount,
            quoteOut(address(USDC), address(TNGBL), lockedAmount)
        );
        TNGBL.safeTransfer(address(nft), lockedTNGBL);
        factory.lockTNGBLOnTNFT(nft, tokenId, _years, lockedTNGBL, onlyLock);
    }

    function setPaymentTokensRouterApprove(address _router, IERC20[] calldata approvedTokens) external onlyAdmin {
    ...
    function _buy(
        ...
        uint256 _sellFee = feesPerCategory[nft] == 0 ? 250 : feesPerCategory[nft];
...
interface IInitialReSeller {
    ...
    event StoreBuyer(ITangibleFractionsNFT indexed ftnft, uint256 indexed fractionId, address buyer, uint256 indexed amount );
    event EndDateExtended(ITangibleFractionsNFT indexed ftnft, uint256 indexed endDate );
    event SaleAmountTaken(ITangibleFractionsNFT indexed ftnft, IERC20 indexed paymentToken, uint256 amount);
    event SaleStarted(ITangibleFractionsNFT indexed ftnft, uint256 indexed endDate, uint256 sellingTokenId, uint256 askingPrice);
