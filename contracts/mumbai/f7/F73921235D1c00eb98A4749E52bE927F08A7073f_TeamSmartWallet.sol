...
interface IListingMarket {
    ...
    /// @param _price The price for which the token is listed.
    /// @param _signature The seller's signature, deadline and address
    function permitList(
        address _collection,
        uint256 _tokenId,
        address _erc20,
    ...
    /// @param _collection The address of the collection contract from which the token is to be delisted.
    /// @param _tokenId The ID of the token to be delisted.
    /// @param _erc20 The address of the ERC20 token that was to be used as the payment currency.
    /// @param _signature The seller's signature, deadline and address
    function permitDelist(address _collection, uint256 _tokenId, address _erc20, Signature memory _signature) external;
    ...
    /// @param _relayedTaxSignature The relayer's signature, deadline and address.
    function buy(
        address _collection,
        uint256 _tokenId,
        address _erc20,
        Tax memory _taxes,
        bool _payNow,
        Signature memory _relayedTaxSignature
    ) external;

    /// @notice Buys an NFT listed for sale with permitted signature
    /// @param _collection The address of the collection contract from which the token is to be bought.
    /// @param _tokenId The ID of the token to be bought.
    /// @param _erc20 The address of the ERC20 token to be used as the payment currency.
    /// @param _taxes The taxes to be paid.
    /// @param _payNow Specifies if payment will be made now or later.
    /// @param _signature The buyer's signature, deadline and address
    function permitBuy(
        address _collection,
        uint256 _tokenId,
        address _erc20,
...
interface IOfferMarket {
     ...
     * @param _erc20 The address of the ERC20 token to use as payment
     * @param _price The price at which to offer the token
     * @param _signature The buyers signature, deadline and address
     */
     ...
     * @param _collection The collection that the token belongs to
     * @param _tokenId The ID of the token whose offer to accept
     * @param _signature The sellers signature, deadline and address
     */
...
import {ITeamSmartWallet} from "./ITeamSmartWallet.sol";
import {Tax} from "../marketplace/taxes/Tax.sol";
import {Signature} from "../utils/Signature.sol";
 ...
 * contract to enable off-chain approval of transactions.
 */
contract TeamSmartWallet is ITeamSmartWallet, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function buy(
        ...
        if (_payNow) {
            ...
            uint256 allowance = IERC20(_erc20).allowance(address(this), vault);
            uint256 price = listingMarket.getListedPrice(_collection, _tokenId, _erc20);
            IERC20(_erc20).approve(vault, allowance + price + _taxes.amount);
        }

        listingMarket.buy(_collection, _tokenId, _erc20, _taxes, _payNow, _signature);
    }
    ...
    function offer(
        ...
        offerMarket.offer(_collection, _tokenId, _erc20, _price);
    }
    ...
    function updateSale(uint256 _saleId, IEscrow.State _newState) public virtual onlyRole(TRADING_ROLE) {
        ...
        if (sale.state == IEscrow.State.AwaitingERC20Deposit && _newState == IEscrow.State.PendingSale) {
            address vault = address(marketHub.getVault());
            uint256 allowance = IERC20(sale.erc20).allowance(address(this), vault);
            IERC20(sale.erc20).approve(vault, allowance + sale.price + sale.taxes.amount);
        }
     ...
     * @param _amount The amount to transfer.
     */
    function transferNative(address payable _recipient, uint256 _amount) public virtual onlyRole(FINANCIAL_ROLE) {
    ...
    function approveERC20(
        ...
        address _spender,
        uint256 _amount
    ) public virtual onlyRole(FINANCIAL_ROLE) returns (bool) {
        return _token.approve(_spender, _amount);
    }
     ...
     * @param _tokenId The ID of the token to approve.
     */
    function approveERC721(address _token, address _to, uint256 _tokenId) public virtual onlyRole(FINANCIAL_ROLE) {
