...
contract SkillsInterface {
    ...
    uint8[MAX_SKILLS][NUMBER_OF_CLASSES] public validSkillsForClass = [
     ...
     function rollSkillsForPlayer(uint256 entropy, uint8 classId, uint8 tier) public view returns (uint8[MAX_SKILLS] memory skillsToRoll) {
        ...
        uint8 seed = 1;
        uint8 skillIndex = 0;
        rolledSkills[0] = validSkillsForClass[classId][uint8(deriveRandomnessFromEntropy(entropy, seed) % MAX_SKILLS) + 1];
        skillIndex++;
        seed++;
         for(uint8 index = 1; index <= tier - 1; index++) {
               ...
               while(!done) {
                  ...
                  if(newValue != rolledSkills[0] &&
                     ...
                     rolledSkills[index] = validSkillsForClass[classId][newValue];
                     done = true;
                  }
               }
         }
