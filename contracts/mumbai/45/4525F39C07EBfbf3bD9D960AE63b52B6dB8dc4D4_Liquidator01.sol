 ...
 */
contract Liquidator01 is IERC721Receiver, AccessControl, ErrorReporter {
  // ----- Protocol Parameters -----
  ...
  event NewClawbackWindow(uint256 _oldWindow, uint256 _newWindow);
  event NewClawbackFee(uint256 _oldFee, uint256 _newFee);
  event NFTFactoryInitialized(address _factory);
  event NFT20PairInitialized(address _pair, address _hToken);
  event HErc20Initialized(address _hToken, address _underlying, address _collateral);
  event ClawbackExecuted(address _hToken, address _borrower, uint256 _collateralId, uint256 _amount);
  event NFT20LiquidationExecuted(address _pair, address _hToken, uint256 _collateralId);
  event TokenSweeped(IERC20 _token, uint256 _qty);
  ...
  constructor(
    ...
    if (_admin == address(0)) revert WrongParams();
   ...
   * @return true on success
   */
  function initializePair(HERC20I _hToken) external onlyRole(INITIALIZER_ROLE) returns (bool) {
    ...
    if (pair != address(0)) {
      ...
      if (NFT20Pair(pair).nftAddress() == address(collateralToken)) {
        poolToNiftyPair[address(_hToken)] = pair;
   ...
   * @return true on success
   */
  function initializeHErc20(HERC20I _hToken) external onlyRole(INITIALIZER_ROLE) returns (bool) {
    //TODO: require(msg.sender == contractFactory or other WL);

    if (!_hToken.isHToken()) revert WrongParams();

    // Add underlyings to mapping
    IERC20 underlyingToken = _hToken.underlyingToken();
    IERC721 collateralToken = _hToken.collateralToken();
    poolToUnderlyingToken[address(_hToken)] = underlyingToken;
    poolToCollateralToken[address(_hToken)] = collateralToken;

    // Unlimited approve the underlying token of the hToken
    underlyingToken.approve(address(_hToken), type(uint256).max);

    emit HErc20Initialized(address(_hToken), address(underlyingToken), address(collateralToken));
   ...
   * @param _hToken address of the HErc20 IRM
   * @param _collateralId collateral Id aka the NFT Token Id
   */

  function liquidateViaNFT20(HERC20I _hToken, uint256 _collateralId) external onlyRole(LIQUIDATOR_ROLE) returns (bool) {
    // Only if NFT20 pair exists
    address pair = poolToNiftyPair[address(_hToken)];
    if (pair == address(0)) revert ExternalFailure("NFT20 pair not found");

    // Only if HErc20 has been initialized
    IERC721 collateralToken = poolToCollateralToken[address(_hToken)];
    if (address(collateralToken) == address(0)) revert Uninitialized();

    // Only against active deposit coupons
    HERC20I.Coupon memory activeCoupon = _hToken.getSpecificCouponByCollateralId(_collateralId);
    if (activeCoupon.active != 2) revert InvalidCoupon();
    ...
    if (block.timestamp <= collectionToTokenToTimeReceived[collateralToken][_tokenId] + clawbackWindow) {
      revert Unauthorized();
    }
    ...
    // Reset mapping
    collectionToTokenToTimeReceived[collateralToken][_tokenId] = 0;
   ...
   * @param _hToken address of the HERC20 to repay on
   * @param _borrower address of the current owner of the deposit coupon
   * @param _collateralId collateral Id, aka the NFT token ID
   * @param _path tokens to swap through, starting at droplet and ending at HERC20's underlying
   ...
   */
  function swapDropletsAndRepayBorrow(
    HERC20I _hToken,
    address _borrower,
    ...
    // Retrieve address
    address droplet = poolToNiftyPair[address(_hToken)];

    // Set approval
    IERC20(droplet).approve(address(swapRouter), _amountIn);
    ...
    _hToken.closeoutLiquidation(_borrower, _tokenId);
   ...
   * @param _hToken address of the relevant HErc20 IRM
   * @param _borrower Holder of the deposit coupon
   * @param _collateralId ERC1155 tokenId of the target deposit coupon
   * @return true upon success
   */
  function clawback(
    HERC20I _hToken,
    ...
    IERC20 underlyingToken = poolToUnderlyingToken[address(_hToken)];
    IERC721 collateralToken = poolToCollateralToken[address(_hToken)];
    ...
    uint256 debt = _hToken.getBorrowFromCoupon(_collateralId);
    ...
    if (doUnderlyingTransferIn(underlyingToken, _borrower, repayAmount) != repayAmount) {
      revert Unexpected("Actual transfer < required");
    }
    ...
    collateralToken.transferFrom(address(this), _borrower, _tokenId);

    // Reset mapping
    collectionToTokenToTimeReceived[collateralToken][_tokenId] = 0;

    emit ClawbackExecuted(address(_hToken), _borrower, _collateralId, repayAmount);
  ...
  function onERC721Received(
    ...
    if (data.length > 0) {
      ...
      collectionToTokenToTimeReceived[collateralToken][_tokenId] = block.timestamp;
  ...
  function doUnderlyingTransferIn(
    ...
    _underlyingToken.transferFrom(_from, address(this), _amount);
   ...
   * @param _newWindow in seconds
   * @return true on success
   */
  function updateClawbackWindow(uint256 _newWindow) external onlyRole(DEFAULT_ADMIN_ROLE) returns (bool) {
    uint256 oldWindow = clawbackWindow;
    clawbackWindow = _newWindow;
    emit NewClawbackWindow(oldWindow, _newWindow);
    return true;
  }

  /**
   * @notice Change the clawback fee
   * @param _newFee as fraction of borrow scaled by 1e18
   * @return true on success
   */
  function updateClawbackFee(uint256 _newFee) external onlyRole(DEFAULT_ADMIN_ROLE) returns (bool) {
    uint256 oldFee = clawbackFeeMantissa;
    clawbackFeeMantissa = _newFee;
    emit NewClawbackFee(oldFee, _newFee);
    return true;
  }
   ...
   */
  function sweepToken(IERC20 _token) external onlyRole(DEFAULT_ADMIN_ROLE) {
    uint256 balance = _token.balanceOf(address(this));
    _token.transfer(treasury, balance);
    emit TokenSweeped(_token, balance);
  }
}
...
interface HERC20I {
  ...
  struct Coupon {
    ...
    address owner; // Who is the onwer of this coupon, this changes at transfer
  }
