...
contract GasMonster is ERC721A, VRFConsumerBaseV2, ChainlinkClient, Ownable {
	...
	string public ipfsMetadataURL =
		'hhttps://bafybeia4rwohoqg43hfjcavffdugyq45ejrknwhj7vnwq5ssnvuy7tdktu.ipfs.nftstorage.link/';

	// Returned from tokenUri() BEFORE reaching 7 level
	string public backendBaseURL = 'http://135.181.216.90:49214/api/tokens/';

	struct NFT {
		uint8 level; // 1-7
		/*
		 * Common parts (0-n), where n - is max number, 10 (NULL) - absence of params
		 */
		uint8 gender; // 0-1
		uint8 color; // 0-4
	 ...
	 */
	mapping(bytes32 => uint256) private updateRequests;
	 ...
	 * @dev backend expects gender=(0,1) and colour=(0,4) so you should sub 1 from property value
	 */
	...
	function triggerUpgrade(uint256 tokenId) external {
		...
		request.add('get', string(abi.encodePacked(backendBaseURL, 'canUpgrade/', Strings.toString(tokenId))));
	 ...
	 */
	function upgrade(bytes32 requestId, bool isUpgraded) public recordChainlinkFulfillment(requestId) {
		emit UpgradeEntrance(requestId, isUpgraded);
		// Stop if upgrade is not available
		if (!isUpgraded) {
			emit UpgradeFailed(requestId, isUpgraded);
			return;
		}
		...
		// Chainlink VRF call
		emit UpgradeTriggeredFulfillment(requestId);

		requestNFTPropsFullfillmentWithRandom(Ids);
	}

	event UpgradeEntrance(bytes32 requestId, bool isUpgraded);

	event UpgradeFailed(bytes32 requestId, bool isUpgraded);

	event UpgradeTriggeredFulfillment(bytes32 requestId);
	...
	function fillInMissingPropsOfToken(uint256 tokenId, uint256 _randomNumber) internal {
		...
		if (token.level == 1) {
			token.gender = uint8(randomNumber % 2);
			// Dividing by 100 to get other part of the number
			token.color = uint8((randomNumber / 100) % 5);
		} else if (token.level == 2) {
	...
	function fillInRoboPartOfToken(uint256 tokenId, uint256 randomNumber) internal {
		require(randomNumber > 9, 'Random number is too small to fill params');
		NFT storage token = Tokens[tokenId];
	 ...
	 */
	function generateMetadataFileName(uint256 tokenId) internal view returns (string memory) {
		NFT memory token = Tokens[tokenId];
		return
			string(
				abi.encodePacked(
	...
	function testChainlinkRequest(uint256 tokenId) external onlyOwner {
		Chainlink.Request memory request = buildChainlinkRequest(
			jobId,
			address(this),
			this.fulfillTestChainlinkRequest.selector
		);
		...
		emit BuiltRequest(
			requestId,
			string(abi.encodePacked(backendBaseURL, 'canUpgrade/', '0')),
			oracle,
			request,
			fee
		);
	...
	event BuiltRequest(bytes32 requestId, string url, address oracle, Chainlink.Request request, uint256 fee);

	function fulfillTestChainlinkRequest(bytes32 requestId, bool isUpgraded)
		public
		recordChainlinkFulfillment(requestId)
	{
		testRequestResult = isUpgraded;
	}

	bool public testRequestResult;
}
