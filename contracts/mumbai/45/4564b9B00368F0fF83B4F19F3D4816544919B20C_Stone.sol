...
contract Stone is ERC20, Authorizable {
    using SafeMath for uint256;
    string private TOKEN_NAME = "Minerverse stone";
    string private TOKEN_SYMBOL = "STONE";

    address public MINER_CONTRACT;

    // the base number of $STONE per miner (i.e. 0.75 $STONE)
    uint256 public BASE_HOLDER_STONES = 750000000000000000;

    // the number of $STONE per miner per day per kg (i.e. 0.25 $egg /chikn /day /kg)
    uint256 public STONES_PER_DAY_PER_KG = 250000000000000000;

    // how much stone it costs to skip the cooldown
    uint256 public COOLDOWN_BASE = 100000000000000000000; // base 100
    // how much additional stone it costs to skip the cooldown per kg
    ...
    struct StakedMinerObj {
        ...
        uint24 kg;
        // when to calculate stone from (max 20/02/36812, 11:36:16)
        uint32 sinceTs;
    ...
    function _stake(uint256 tid) internal {
        ...
        if (stakedMiner[tid].kg == 0) {
            ...
            // N.B. could be optimised for multi-stakes - but only saves 0.5c AUD per miner
            //- not worth it, this is a one time operation.
            totalStakedMiner += 1;
            totalKg += uint24(kg);
    ...
    /**
     * Calculates the TOTAL amount of stone that is claimable from ALL miners.
     */
    ...
    /**
     * Claims stones from the provided miners.
     */
    function _claimStones(uint256[] calldata tokenIds) internal {
        Miner x = Miner(MINER_CONTRACT);
        uint256 totalClaimableStone = 0;
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(x.ownerOf(tokenIds[i]) == msg.sender, "NOT OWNER");
            StakedMinerObj memory miner = stakedMiner[tokenIds[i]];
            // we only care about miner that have been staked (i.e. kg > 0) ...
            if (miner.kg > 0) {
                uint256 claimableStone = claimableView(tokenIds[i]);
                if (claimableStone > 0) {
                    totalClaimableStone = totalClaimableStone + claimableStone;
        ...
        if (totalClaimableStone > 0) {
            _mint(msg.sender, totalClaimableStone);
            emit Minted(msg.sender, totalClaimableStone);
        }
    }
     ...
     * Claims stones from the provided miners.
     */
    function claimStones(uint256[] calldata tokenIds) external {
        _claimStones(tokenIds);
    }
    ...
    /**
     * Unstakes MULTIPLE miner AND claims the stones.
     */
    ...
    /**
     * Public : update the miner's KG level.
     */
    ...
    /**
     * Internal: burns the given amount of stones from the wallet.
     */
    function _burnStones(address sender, uint256 stonesAmount) internal {
        // NOTE do we need to check this before burn?
        require(balanceOf(sender) >= stonesAmount, "NOT ENOUGH STONE");
        _burn(sender, stonesAmount);
        emit Burned(sender, stonesAmount);
    }

    /**
     * Burns the given amount of stones from the sender's wallet.
     */
    function burnStones(address sender, uint256 stonesAmount) external onlyAuthorized {
        _burnStones(sender, stonesAmount);
    }

    /**
     * Skips the "levelUp" cooling down period, in return for burning Egg.
     */
     function skipCoolingOff(uint256 tokenId, uint256 stoneAmt) external {
        StakedMinerObj memory miner = stakedMiner[tokenId];
        require(miner.kg != 0, "NOT STAKED");

        uint32 ts = uint32(block.timestamp);

        // check: enough stone in wallet to pay
        uint256 walletBalance = balanceOf(msg.sender);
        require( walletBalance >= stoneAmt, "NOT ENOUGH STONE IN WALLET");

        // check: provided stone amount is enough to skip this level
        require(stoneAmt >= checkSkipCoolingOffAmt(miner.kg), "NOT ENOUGH STONE TO SKIP");
        ...
        // burn stones
        _burnStones(msg.sender, stoneAmt);
    ...
    /**
     * Iron Feeding the miner
     */
    function feedMiner(uint256 tokenId, uint256 ironAmount)
        external
        onlyAuthorized
    {
        StakedMinerObj memory miner = stakedMiner[tokenId];
        require(miner.kg > 0, "NOT STAKED");
        require(ironAmount > 0, "NOTHING TO FEED");
        // update the block time as well as claimable
        miner.eatenAmount = uint48(ironAmount / 1e18) + miner.eatenAmount;
        stakedMiner[tokenId] = miner;
    }
    ...
    function updateSkipCooldownValues(
        ...
        BASE_HOLDER_STONES = d;
        STONES_PER_DAY_PER_KG = e;
    }

    // INTRA-CONTRACT: use this function to mint stone to users
    // this also get called by the FEED contract
    function mintStone(address sender, uint256 amount) external onlyAuthorized {
        _mint(sender, amount);
        emit Minted(sender, amount);
    }

    // ADMIN: drop stone to the given miner wallet owners (within the minerId range from->to).
    function airdropToExistingHolder(
        uint256 from,
        uint256 to,
        uint256 amountOfStone
    ) external onlyOwner {
        // mint 100 stone to every owners
        Miner instance = Miner(MINER_CONTRACT);
        for (uint256 i = from; i <= to; i++) {
            address currentOwner = instance.ownerOf(i);
            if (currentOwner != address(0)) {
                _mint(currentOwner, amountOfStone * 1e18);
            }
        }
    }

    // ADMIN: Rebalance user wallet by minting stone (within the minerId range from->to).
    // NOTE: This is use when we need to update stone production
    function rebalanceStoneClaimableToUserWallet(uint256 from, uint256 to)
        external
        onlyOwner
    {
        Miner instance = Miner(MINER_CONTRACT);
...
contract Miner is ERC721Upgradeable {
    // Royalty
    ...
    // map id to Miner obj
    mapping(uint256 => MinerInfo) public allMiner;
    ...
    event LevelUp(uint256 minerNumber, uint256 newKg);
    event NameChange(uint256 minerNumber, string name);
    ...
    function mint(uint256 numberOfToken) public payable {
        ...
        for (uint256 i = 1; i <= numberOfToken; i++) {
            ...
            MinerInfo memory newMiner = MinerInfo(
                newItemId,
                msg.sender,
                msg.sender,
                0,
            ...
            // add the token id to the allMiner
            allMiner[newItemId] = newMiner;
        }
    }
    ...
    function setPriceForSale(
        ...
        MinerInfo memory miner = allMiner[_tokenId];
        miner.price = _newPrice;
        miner.forSale = isForSale;
        allMiner[_tokenId] = miner;
    ...
    function getAllSaleTokens() public view returns (uint256[] memory) {
        ...
        for (uint256 i = 1; i <= _totalSupply; i++) {
            if (allMiner[i].forSale == true) {
                _tokenForSales[counter] = allMiner[i].tokenId;
                counter++;
            }
        }
    ...
    function buyToken(uint256 _tokenId) public payable {
        ...
        // get that token from all Miner mapping and create a memory of it defined as (struct => Miner)
        MinerInfo memory miner = allMiner[_tokenId];
    ...
    function _beforeTokenTransfer(
        ...
        MinerInfo memory miner = allMiner[tokenId];
        miner.currentOwner = to;
        miner.numberOfTransfers += 1;
        miner.forSale = false;
        allMiner[tokenId] = miner;
        if (from == address(0)) {
