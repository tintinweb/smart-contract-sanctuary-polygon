...
import "./libraries/AssemblyMath.sol";
import "./token/ERC721/ERC721Enumerable.sol";

/// @title PetGasWhitelistToken
contract PetGasWhitelistToken is ERC721Enumerable, Ownable, PaymentSplitter {
    using Strings for uint256;

    // URIs
    // TODO: Check me
    string public baseURI =
        "https://reflectnserve.mypinata.cloud/ipfs/QmYVdfESz5UkKxPshNQgdM3bnB1XmYyWpo4HaVJbujDt7Q/";
    string public contractURI =
        "https://reflectnserve.mypinata.cloud/ipfs/QmUCvMx9KPKxe63MnZGToBPN7G6VVkSCBG8ZLirAi6u9C3";
    ...
    // Supply limit
    uint256 public constant SUPPLY_STRICT_UPPER_BOUND = 4445;

    // For external use
    uint256 public constant maxSupply = 4444;

    address[] public _treasuries;
    ...
    // Constructor
    constructor(address[] memory treasuries, uint256[] memory treasuryShares)
        ERC721("PetGas: Whitelist Token Collection", "PGWLT")
        PaymentSplitter(treasuries, treasuryShares)
    {
     ...
     */
    modifier correctCost(uint256 msgValue, uint256 numberOfTokens) {
        require(
            numberOfTokens * cost == msgValue,
            "PetGasWhitelistToken: Incorrect ether amount provided."
        );
        _;
    }
    ...
    modifier meetsExistence(uint256 tokenId) {
        require(_exists(tokenId), "PetGasWhitelistToken: Nonexistent token.");
        _;
    }
     ...
     */
    modifier mintActive() {
        require(mintIsActive, "PetGasWhitelistToken: Mint is not active.");
        _;
    }
    ...
    modifier meetsSupplyConditions(uint256 count) {
        ...
        require(
            ...
            "PetGasWhitelistToken: Supply limit reached."
        );
        _;
    }
     ...
     * @notice Sets the value of the contract URI.
     * @dev Only contract owner can call this function.
     */
    ...
    /**
     * @param newCost New collection cost for public mint
    ...
    /**
     * @param nTokens Number of tokens to mint.
     ...
     */
    function publicMint(uint256 nTokens)
        external
        payable
        mintActive
        correctCost(msg.value, nTokens)
        meetsSupplyConditions(nTokens)
    {
        internalMint(msg.sender, nTokens);
    }
        ...
        onlyOwner
    {
        require(
            ...
            "PetGasWhitelistToken: Amount array length does not match recipient array or option length."
        );
        ...
        require(
            ...
            "PetGasWhitelistToken: Cannot mint more than supply limit."
        );
    ...
    /*************************************************************************/
    /**
     * @dev Withdraws all contract matic to respective owners.
     * @dev Can only be called by contract owner.
     */
...
import "../../libraries/AddressCustom.sol";

abstract contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    using AddressCustom for address;
    using Strings for uint256;
    ...
    function _exists(uint256 tokenId) internal view virtual returns (bool) {
        return
            tokenId < tokenOwners.length && tokenOwners[tokenId] != address(0);
    }
