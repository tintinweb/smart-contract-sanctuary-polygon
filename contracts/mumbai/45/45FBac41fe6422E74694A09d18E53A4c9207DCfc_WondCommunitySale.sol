...
/** 
* @author 0xr1 for Wonderlive
* @notice Implementation of the Community sale contract.
*/

contract WondCommunitySale is Ownable, ReentrancyGuard, PaymentSplitter {

    using FixedPointMathLib for uint256;

    struct RoundInfo {
        uint256 totalAllocation; /// total amount of $WOND token available for the community sale
        uint256 currentAllocation; /// current amount already allocated
        uint256[] tierPrice; /// price to pay in usd, each index represent a tier price
    ...
    /// @notice round data Info
    RoundInfo public round;
    ...
    ///  allocation size to be in the tier in $WOND
    uint256 public firstTier = 100e18; // 10k$ -> first tier (price : 0.01$)
    uint256 public secondTier = 300e18; // 30k$ -> first tier (price : 0.005$)
    /// safety check
    bool public communitySaleOpen;
    ...
    event AddInCommunitySale(address who, uint256 allocated);
    event RewardsClaim(address who, uint256 amount);
    event SetUpCommunitySale(uint256 alloc);
    ...
    constructor(
        ...
        // safety check
        if(_wond == address(0)) revert WrongAddress();
        ...
        communitySaleOpen = false;
    }

    /*** MODIFIERS ***/

    modifier onlyWhenSaleOpen(){
        require(communitySaleOpen, "COMMUNITY SALE IS CLOSES");
        _;
    }
    ...
    nonReentrant {
        ...
        emit AddInCommunitySale(who, allocation);
    }


    function enterCommunitySale (
        address who, 
        uint256 allocation
    ) 
    external
    ...
    nonReentrant {
        ...
        uint256 amountToPayUSD = checkPriceForAllocation(allocation);
    ...
    function computeClaimableRewards(address _user) public view returns(uint256) {
        ...
        if (startVesting == 0 || endVesting == 0) return 0;
    ...
    function checkPriceForAllocation(uint256 allocation) internal view returns(uint256) {
        uint256 toPay = allocation.fmul(round.tierPrice[0], FixedPointMathLib.WAD);
        /// here we are in the second tier 
        if (allocation >= firstTier && allocation < secondTier) {
            toPay = allocation.fmul(round.tierPrice[1], FixedPointMathLib.WAD);
        }
        /// third tier
        else if (allocation >= secondTier){
            toPay = allocation.fmul(round.tierPrice[2], FixedPointMathLib.WAD);
        }
        return toPay;
    }
    ...
    function setupCommunitySale (
        ...
        require(_totalAllocation > 0 && _totalAllocation > round.currentAllocation);
        require(_price[0] > 0);
        require(_minBuy < _maxBuy);

        uint256 roundAllocation = round.currentAllocation;

        round = RoundInfo({
        totalAllocation : _totalAllocation,
        currentAllocation : roundAllocation,
        tierPrice : _price,
        minBuy: _minBuy,
        maxBuy : _maxBuy
        });

        emit SetUpCommunitySale(_totalAllocation);
    }
}