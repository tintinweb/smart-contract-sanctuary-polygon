...
/// @title ClosestAddressNFT - A Smart Contract for an NFT game based on the proximity of addresses
...
/// TODO
/// [x] better mapping? (none)
/// [x] not indexed? (no leave it)
/// [x] assign variables for multiple uses (nope)
/// [x] is it cheaper to inherit OZ libraries?
/// [ ] Change name and symbol and URI stuff
contract ClosestAddressNFT {
    ...
    /// @notice Initializes the contract, setting the initial minimum difference and emitting a Transfer event
    constructor() {
        // address ACK = 0x03ee832367E29a5CD001f65093283eabB5382B62;
        // address ROBNESSVAULT = 0x7485ac6d8534691993348D51ab0F131a19FfF763;
        address ACK = 0x4200000000000000000000000000000000000000;
        address ROBNESSVAULT = 0x6999999999999999999999999999999999999999;

        uint160 distanceToA = uint160(ACK) > uint160(address(this)) ? uint160(ACK) - uint160(address(this)) : uint160(address(this)) - uint160(ACK);
        uint160 distanceToB = uint160(ROBNESSVAULT) > uint160(address(this)) ? uint160(ROBNESSVAULT) - uint160(address(this)) : uint160(address(this)) - uint160(ROBNESSVAULT);
    
        if (distanceToA > distanceToB) {
            emit Transfer(address(0), ACK, 1);
            emit Transfer(ACK, ROBNESSVAULT, 1);
            _minDifference = int256(uint256(uint160(ROBNESSVAULT)))-int256(uint256(uint160(address(this))));
            _currentClosest = ROBNESSVAULT;
        } else {
            emit Transfer(address(0), ROBNESSVAULT, 1);
            emit Transfer(ROBNESSVAULT, ACK, 1);
