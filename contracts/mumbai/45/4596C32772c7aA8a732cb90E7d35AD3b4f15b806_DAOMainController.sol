...
contract Treasury is   Ownable  , AccessControl , Pausable{
    ...
    //mapping ( uint256 => address[] ) public stockholders ;
  ...
  /// @dev Add an account to the admin role. Restricted to admins.
    ...
    // function getTreasuryStockholders () public view returns ( address [] memory  )
    // {
    //     return stockholders ; 
    // }
...
contract Voting is   Ownable  , AccessControl , Pausable{
    ...
    mapping ( uint256 => mapping ( address => FundVotingMembers ) ) FundMembersWeight ; 

    mapping ( uint256 => FundVotingMembers[] ) FundMembersInfo ; 
  ...
  /// @dev Add an account to the admin role. Restricted to admins.
    ...
    {  
        ...
        RoundInfo memory round = RoundInfo(  index , startTime , endTime , voteStartTime , voteEndTime , Lock , fund , 0  , 0 , 0 , false , false   , true , enabled ); 
        Rounds[index] = round ; 
    ...
    function recordFundingMembersInfo ( address voter  , uint256 roundId , uint256 proposalId , uint256 voterBalance  , uint256 treasuryBalance  ) public onlyContractOrAdmin whenNotPaused
    {
        FundVotingMembers memory fundMembers = FundVotingMembers ( voter  , voterBalance , treasuryBalance , roundId , proposalId  , false , true  );
        FundMembersWeight[roundId][voter] = fundMembers ;
        FundMembersInfo[roundId].push(fundMembers); 
    }

    function addTreasuryStockholders ( uint256 roundId , address stackholder ) public onlyContractOrAdmin whenNotPaused returns ( bool )
    {
        bool result = false ; 
        for ( uint256 i = 0 ; i < stockholders[roundId].length ; i++  )
        {
            if ( stockholders[roundId][i] == stackholder )
            {
                result = true ; 
                break ; 
            }
        ...
        if ( result ==  false )
        {
            stockholders[roundId].push(stackholder);
        }
        return true ; 
    }

    function getRounds( uint256 page , uint256  size) public view returns ( RoundInfo[] memory  )
    {

        require ( page > 0 && size > 0  , "page/size != 0");
        RoundInfo[] memory  result  = new  RoundInfo[] (  size );
        
        uint256 index = 0 ; 
        for ( uint256 i = 1 ; i <= size  ; i++ )
        {
            //UserStake storage temp = this.StakeUserInfo(msg.sender , ( page.sub(1) ).mul(size).add(i) ) ;
            if ( Rounds[ (page.sub(1) ).mul(size).add(i) ].exist == true  )
            {
                RoundInfo memory temp = Rounds[ (page.sub(1) ).mul(size).add(i) ] ;
                result[index] = temp ;
                index = index.add(1);
            }
    ...
    function getFundMembersWeight ( uint256 roundID , address voter) public view returns ( FundVotingMembers memory ) 
    {
        return FundMembersWeight [roundID][voter] ; 
    }

    function getFundMembers ( uint256 page , uint256  size , uint256 roundID )   public view returns (  FundVotingMembers [] memory )
    {
        require ( page > 0 && size > 0  , "page/size != 0");
        FundVotingMembers[] memory  result  = new  FundVotingMembers[] (  size );
        
        uint256 index = 0 ; 
        for ( uint256 i = 0 ; i < size  ; i++ )
        {
            //UserStake storage temp = this.StakeUserInfo(msg.sender , ( page.sub(1) ).mul(size).add(i) ) ;
            if (  (page.sub(1) ).mul(size).add(i) < FundMembersInfo[roundID].length &&   FundMembersInfo[roundID][ (page.sub(1) ).mul(size).add(i) ].exist == true  )
            {
                FundVotingMembers memory temp = FundMembersInfo[ roundID ][ (page.sub(1) ).mul(size).add(i) ] ;
                result[index] = temp ;
                index = index.add(1);
            }
    ...
    function getTreasuryStockholders ( uint256 roundId ) public view returns ( address [] memory  )
    {
        return stockholders[roundId] ; 
    }
...
contract MainController is   Ownable  , AccessControl , Pausable{
  ...
  /// @dev Add an account to the admin role. Restricted to admins.
    ...
    function createInvestment (   address token  , uint256 fundsDeposit , string memory details , uint256 proposalID   ) public   whenNotPaused    returns (  uint256 )
    {
        ...
        require ( investordb.getInvestor(msg.sender).exist == true , "E4"  );
     ...
     function createDAOInvestment (   address token  , uint256 fundsDeposit , string memory details , uint256 proposalID   ) public  onlyContractOrAdmin whenNotPaused      returns (  uint256 )
    {
        ...
        require ( p.enabled == true , "E2" ) ;
        require ( p.token == token , "E3" );
        require ( investordb.getInvestor(msg.sender).exist == true &&  investordb.getInvestor(msg.sender).isDAO == true , "E4"  );
    ...
    function abortInvestment (  uint256 bankID   ) public   whenNotPaused    returns (  uint256 )
    {
        ...
        require ( proposaldb.getProposal(  proposalID ).paid == false , "E12"   ) ;
    ...
    function withdrawProfitAndBalance (  uint256 bankID   ) public   whenNotPaused    returns (  uint256  , uint256)
    {
        ...
        bankdb.getBankAccount( bankID ).withdrawFund = true; 
        bankdb.getBankAccount( bankID ).investmentCompleted = true; 
        //uint256 allow = token_contract.allowance( msg.sender , address(this) ) ;
        //require ( allow >= minFundBase &&  allow.mod(minFundBase) == 0  && allow == fundsDeposit , "E7"   );
  ...
  /// @dev Add an account to the admin role. Restricted to admins.
    ...
    function InvestInRoundForProposal  ( uint256 roundID ) public onlyContractOrAdmin whenNotPaused 
    {
        ...
        if ( voting.GetProposalVoteWeightByRound(  max , roundID ) >= quorum.mul( treasury.GetTotalBalanceOfTreasury() ).div(100)   )
        {
            ...
            voting.GetRoundInfo(roundID).funded = true ; 
            //uint256 daoFund = voting.GetRoundInfo(roundID).fund ; 
            ...
            for ( uint256 i = 0 ; i < voting.getTreasuryStockholders(roundID).length ; i++  )
            {
                uint256 weight = treasury.GetTotalBalance( voting.getTreasuryStockholders(roundID)[i] ) ;
                //if (  weight > 0  )
                //{
                voting.recordFundingMembersInfo ( voting.getTreasuryStockholders(roundID)[i]   , roundID , max , weight , treasury.GetTotalBalanceOfTreasury()   );

            //}
            }

            uint256 bankId = main_controller.createDAOInvestment ( proposaldb.getProposal(max).token  , proposaldb.getProposal(max).fundsRequested.sub(fundinvested)   , "DAO Investment" , max );
            voting.GetRoundInfo(roundID).bankId = bankId ; 
        }
    ...
    function withdrawProfitAndBalance (  uint256 roundId  , address token  ) public  onlyContractOrAdmin  whenNotPaused    
    {

        (uint256 profit ,  ) = main_controller.withdrawProfitAndBalance( voting.GetRoundInfo(roundId).bankId );
        voting.GetRoundInfo(roundId).returned = true ; 
        voting.GetRoundInfo(roundId).profit = profit ;
        require( erc20db.checkERC20Info(token) == true , "E2" );
        //IERC20 token_contract = IERC20(token) ;
        for ( uint256 i = 0 ; i < voting.getTreasuryStockholders(roundId).length ; i++  )
        {
            Voting.FundVotingMembers memory  fundVotingMember =  voting.getFundMembersWeight (  roundId , voting.getTreasuryStockholders(roundId)[i] ) ; 
            uint256 amount = fundVotingMember.voterBalance.mul( voting.GetRoundInfo(roundId).profit ).div(fundVotingMember.treasuryBalance);
            //bool result = token_contract.transfer( msg.sender ,  amount );
            //require ( result == true , "E4");
            voting.getFundMembersWeight (  roundId , msg.sender).collected == true ; 
            treasury.Deposit( voting.getTreasuryStockholders(roundId)[i] , amount ,token ) ;
                //uint256 weight = treasury.GetTotalBalance( treasury.getTreasuryStockholders()[i] ) ;
                //if (  weight > 0  )
                //{
                //voting.recordFundingMembersInfo ( treasury.getTreasuryStockholders()[i]  , roundID , max , weight , treasury.GetTotalBalanceOfTreasury()   );
    ...
    // function withdrawProfitForStackholder ( uint256 roundId , address token  ) public whenNotPaused
    // {
    //     Voting.FundVotingMembers memory  fundVotingMember =  voting.getFundMembersWeight (  roundId , msg.sender) ; 
    //     require ( fundVotingMember.exist == true , "Fund Voting Info doesn't exist") ;
    //     require ( msg.sender == fundVotingMember.voter , "Access Denied" );
    //     require ( fundVotingMember.collected == false , "already Collected" ) ;
    //     require ( voting.GetRoundInfo(roundId).returned == true , "Profit was not returned");
    //     require( erc20db.checkERC20Info(token) == true , "E2" );
    //     IERC20 token_contract = IERC20(token) ;
    //     //require ( token_contract.allowance( msg.sender , address(this) ) == amount , "E3"   );
    //     uint256 amount = fundVotingMember.voterBalance.mul( voting.GetRoundInfo(roundId).profit ).div(fundVotingMember.treasuryBalance);
    ...
    function getRounds( uint256 page , uint256  size) public view returns ( Voting.RoundInfo[] memory  )  
    {
        return  voting.getRounds(page , size)  ; 
    }  

    function getFundMembers ( uint256 page , uint256  size , uint256 roundId )  public view returns ( Voting.FundVotingMembers[] memory  ) 
    {
        return voting.getFundMembers(page , size , roundId) ; 
    }
