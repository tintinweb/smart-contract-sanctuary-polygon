...
contract SubManager is ISubManager, Ownable {
    ...
    event CreatePlan(uint256 merchantTokenId, uint256 planIndex);
    event DisablePlan(uint256 merchantTokenId, uint256 planIndex);
    ...
    event Charge(uint256 indexed subscriptionTokenId);
    ...
    function createMerchant(string memory name, address payeeAddress)
        external
        returns (uint256)
    {
        uint256 merchantTokenId = IMerchantToken(merchantManager)
            .createMerchant(name, msg.sender, payeeAddress);
        emit CreateMerchant(msg.sender, name, payeeAddress, merchantTokenId);
    ...
    function updateMerchant(
        ...
        emit UpdateMerchant(merchantOwner, name, payeeAddress, merchantTokenId);
    }
    ...
    function createPlan(
        ...
        uint256 pricePerBillingPeriod //        Period.PeriodType termPeriod
    ) external {
        address merchantOwner = IERC721(merchantManager).ownerOf(
            merchantTokenId
        );
        ...
        emit CreatePlan(merchantTokenId, planIndex);
    }
        ...
        external
    {
        ...
        IERC20(plan.paymentToken).transferFrom(
            msg.sender,
            merchant.payeeAddress,
        ...
        emit CreateSubscription(merchantTokenId, planIndex, msg.sender, subscriptionTokenId);
    }
    ...
    function charge(uint256 subscriptionTokenId) external {
        ...
        IMerchantToken.Merchant memory merchant = IMerchantToken(
            merchantManager
        ).getMerchant(subInfo.merchantTokenId);
        IPlanManager.Plan memory plan = IPlanManager(planManager).getPlan(
        ...
        emit Charge(subscriptionTokenId);
    }
    ...
    // re-new sub
    //    function renewSub(uint subscriptionTokenId) external {
    //        require(msg.sender == IERC721(subTokenManager).ownerOf(subscriptionTokenId), "only sub owner");
    ...
    //        ISubInfoManager(subInfoManager).updateSubInfo(subInfo.merchantTokenId, subscriptionTokenId, subInfo);
    //    }
