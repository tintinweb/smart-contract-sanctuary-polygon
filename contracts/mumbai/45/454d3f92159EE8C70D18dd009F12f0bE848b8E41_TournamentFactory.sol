...
contract TournamentManager is VRFConsumerBaseV2 {
  ...
  function modifyTeamBeforeInscription(uint256 teamIndex, uint256[5] memory gotchis, uint256 gotchiLeader, uint256[10] memory gotchiFormation, string memory teamName) external {
    deleteTeam(teamIndex);
    createTeam(gotchis, gotchiLeader, gotchiFormation, teamName);
  }

  function modiftyTeamAfterInscription(uint256 teamIndex, uint256 gotchiLeader, uint256[10] memory gotchiFormation) external {
    Team memory teamToChange = teamIdToTeam[teamIndex];
    require(tournament_state == TOURNAMENT_STATE.BATTLE, "Not in battle mode");
    require(teamToChange.owner == msg.sender, "Msg.sender not owner of the team");
    require(participants[(msg.sender)] == true, "Participants didn't enter or is eliminated");
    require(checkGotchiFormationValidity(gotchiFormation) == true, "Gotchi formation not valid");
    require(isGotchiLeaderInTeam(teamToChange.gotchiTeam, gotchiLeader) == true, "Gotchi leader is not in the team");
    require(teamIdStillInCompetition[teamIndex] == true, "Participants is eliminated");

    teamToChange.gotchiLeader = gotchiLeader;
    teamToChange.gotchiFormation = gotchiFormation;

    Team[] storage teams = participantToTeams[teamToChange.owner];
    for (uint256 i = 0; i < teams.length; i++) {
  ...
  function getTeamsId(address participant) external view returns (uint256[] memory) {
        uint256[] memory teamsIds = new uint256[](participantToTeams[participant].length);
  ...
  function getNumberOfTeam(address participant) external view returns (uint256) {
  ...
  // default getter can't access array inside struct

  function getTeamGotchiTeam(uint256 teamId) public view returns (uint256[5] memory){
    return teamIdToTeam[teamId].gotchiTeam;
  }

     function getTeamGotchiFormation(uint256 teamId) public view returns (uint256[10] memory){
    return teamIdToTeam[teamId].gotchiFormation;
  }
