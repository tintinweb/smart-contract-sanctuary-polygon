...
contract PartnersOW {
    uint256 public constant INVESTMENT_ROUND_DURATION = 10 minutes;
    uint256 public constant INVESTMENT_ROUND_COOLDOWN = 2 hours;
    ...
    constructor() {
        _owner = msg.sender;
        _roundEndTime = block.timestamp + INVESTMENT_ROUND_DURATION;
    }

    function invest() external payable {
        require(msg.value >= MINIMUM_INVESTMENT, "Investment amount is less than minimum investment");

        // Check investment round cooldown
        require(block.timestamp >= _roundEndTime + INVESTMENT_ROUND_COOLDOWN, "Investment round is not available yet");
        ...
        // Check investment limit for this round
        uint256 roundInvestment = _totalInvestment + msg.value;
        require(roundInvestment <= address(this).balance, "Investment limit reached for this round");
