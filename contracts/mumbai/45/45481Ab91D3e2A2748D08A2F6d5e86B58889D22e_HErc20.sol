 ...
 * @author Honey Labs
 */
contract HErc20 is HToken, ReentrancyGuard {
  string public constant name = "HERC20 Test";
  string public constant symbol = "hTEST";
   ...
   * @param _hiveFeeMantissa hive fee rate, scaled by 1e18
   */
  constructor(
  ...
  function depositMultiCollateral(uint256[] calldata _collateralIds) external nonReentrant {
    uint256 i;
    for (i; i < _collateralIds.length; ++i) {
  ...
  /**
   * @notice Burn a deposit coupon NFT and withdraw the associated underlying ERC-721 NFT, for multiple collaterals
  ...
  function sweepToken(IERC20 _token) external {
    ...
    _token.transfer(dao, balance);
  }

  /**
   * @notice The sender adds directly to reserves.
   * @param _amount The amount of underlying token to add as reserves
   */
  function _addReserves(uint256 _amount) external {
    _addReservesInternal(_amount);
  }
}
...
interface HTokenHelperI {
  function uri(uint256 _id, address _htoken) external view returns (string memory);
}

import "hardhat/console.sol";

contract HToken is ERC1155, IERC721Receiver, ErrorReporter, AccessControl {
  ...
  /// @notice receivers of the protocol fees
  ...
  /// @notice dao address where sweeped tokens will be transmitted
  ...
  /// @notice borrow coupons mapping, saves a coupon like this collateralId => Coupon
  mapping(uint256 => Coupon) public borrowCoupons;

  /// @notice borrow coupons indexes per collateral, couponId => collateralId
  mapping(uint256 => Collateral) public collateralPerBorrowCouponId;

  /// @notice keeps how many coupons a user has
  mapping(address => uint256) public userToCoupons;

  /// @notice keeps the borrowBalancePerUser so we don't interate through the coupons to calculate it
  mapping(address => uint256) public borrowBalancePerUser;

  /*** Market Events ***/

  event InterestAccrued(uint256 _interestAccumulated, uint256 _interestPerToken, uint256 _totalBorrows);
  event AccrueCouponInterest(
    Coupon _activeCoupon,
    uint256 _debtPrior,
    uint256 _interestAccumulated,
    uint256 _interestPerTokenPrior,
    uint256 _interestPerTokenCurrent
  );
  event Redeem(address _initiator, uint256 _redeemAmount, uint256 _redeemTokens, uint256 _totalHTokenSupply);
  event Withdraw(address _initiator, uint256 _amountWithdrawn, uint256 _tokensWithdrawn, uint256 _totalHTokenSupply);
  event Borrow(address _borrower, uint256 _borrowAmount, uint256 _tokenId, uint256 _totalBorrows);
  event RepayBorrow(address indexed _payer, uint256 _repayAmount, uint256 _accountBorrows, uint256 _totalBorrows, uint256 _collateralId);
  event LiquidateBorrow(
    address indexed _liquidator,
    address _borrower,
    uint256 _repayAmount,
    address indexed _hTokenCollateral,
    uint256 _seizeTokens
  );

  event CollateralDeposited(address _initiator, uint256 _collateralId);
  event CollateralWithdrawn(address _initiator, uint256 _collateralId);
  event BorrowLiquidated(address _initiator, address _liqudidator, address _owner, uint256 _collateralId);
  event UnderlyingDeposited(address _initiator, uint256 _amount, uint256 _tokensToMint, uint256 _totalHtokenSupply);
  event LiquidationClosed(address _initiator, address _borrower, uint256 _collateralId, uint256 _borrowAmount);

  /*** Admin Events ***/

  event NewContract(address _oldReceiver, address _newReceiver);
  event NewMarketInterestRateModel(InterestRateModelI _oldInterestRateModel, InterestRateModelI _newInterestRateModel);
  event ReservesAdded(address _benefactor, uint256 _addAmount, uint256 _newTotalReserves);
  event ReservesReduced(address indexed _admin, uint256 _reduceAmount, uint256 _newTotalReserves);
  event AdminFeesWithdrew(uint256 _amount);
  event HiveFeesWithdrew(uint256 _amount);
  ...
  constructor(
    ...
    // reserving id 0 for hTokens so we need to increment
    idCounter.increment();

    // Set initial exchange rate
    initialExchangeRateMantissa = _initialExchangeRateMantissa;
    if (initialExchangeRateMantissa == 0) revert HTokenError(Error.INITIAL_EXCHANGE_MANTISSA);
    ...
    // Set reserve factor and fees
    _setProtocolFees(_hiveFeeMantissa, _adminFeeMantissa, _reserveFactorMantissa, _protocolFeeMantissa);
  }
   ...
   * @param _account the user to search for
   * @return user's coupons
   */
  function getUserCoupons(address _account) public view returns (Coupon[] memory) {
    unchecked {
      Coupon[] memory userCoupons = new Coupon[](userToCoupons[_account]);
      uint256 i;
      uint256 length = idCounter.current();
      uint256 counter;
      for (; i < length; ++i) {
        Collateral memory collateral = collateralPerBorrowCouponId[i];
        if (!collateral.active) continue;
        Coupon memory coupon = borrowCoupons[collateral.collateralId];

        if (coupon.owner == _account) {
   ...
   */
  function getUserCouponIndices(address _user) public view returns (uint256[] memory) {
    unchecked {
   ...
   */
  function getActiveCoupons() external view returns (Coupon[] memory depositedCoupons) {
    unchecked {
      uint256 length = idCounter.current();
      uint256 deposited;
      uint256 i;
      ...
      for (; i < length; ++i) {
        if (collateralPerBorrowCouponId[i].active) {
          depositedCoupons[j] = borrowCoupons[collateralPerBorrowCouponId[i].collateralId];
          j++;
        }
      }
    }
  ...
  /**
   * @dev Deposit underlying tokens and recieve hTokens as proof of deposit
  ...
  function depositUnderlyingInternal(uint256 _amount) internal {
    ...
    emit UnderlyingDeposited(msg.sender, _amount, tokensToMint, totalHTokenSupply);
  }

  /**
   * @dev Redeem a specified amount of hTokens for their underlying ERC-20 asset
  ...
  function redeemInternal(uint256 _amount) internal {
    ...
    uint256 underlyingToRedeem = (_amount * exchangeRateCurrent()) / 1e18;
    ...
    emit Redeem(msg.sender, _amount, underlyingToRedeem, totalHTokenSupply);
  }

  /**
   * @dev Withdraw a specified quantity of underlying ERC-20 asset, redeeming any amount of hTokens necessary
   * It does a check against Hivemind to see if it is allowed to redeem as the withdraw means redem
   * @param _amount of Underlying to be withdraw
   */
  function withdrawInternal(uint256 _amount) internal {
    hivemind.redeemAllowed(HTokenI(address(this)), msg.sender, _amount);

    uint256 tokensToWithdraw = (_amount * 1e18) / exchangeRateCurrent();

    totalHTokenSupply -= tokensToWithdraw;

    _burn(msg.sender, 0, tokensToWithdraw);

    doUnderlyingTransferOut(msg.sender, _amount);
    emit Withdraw(msg.sender, tokensToWithdraw, _amount, totalHTokenSupply);
  }

  /**
   * @dev does transfer out, meaning will withdraw underlying token from the contract to `_to`
  ...
  /**
   * @dev does transfer in, meaning will deposit underlying token to the contract's balance
  ...
  function depositCollateralInternal(uint256 _collateralId) internal {
    doCollateralTransferIn(msg.sender, _collateralId);

    _mintCoupon(msg.sender, _collateralId, 0);

    emit CollateralDeposited(msg.sender, _collateralId);
  }

  /**
   * @dev borrows an amount and accrued it to the coupon
   * @param _borrowAmount borrowed amount
   */
  function borrowInternal(uint256 _borrowAmount, uint256 _collateralId) internal {
    ...
    if (coupon.owner != msg.sender) revert NotOwner();

    // Our contract must own the relevant tokenId
    if (collateralToken.ownerOf(_collateralId) != address(this)) revert NotOwner();

    accrueInterestToCoupon(_collateralId);

    // Check for borrow allowed
    hivemind.borrowAllowed(HTokenI(address(this)), msg.sender, _collateralId, _borrowAmount);

    uint256 borrowIncurred = _borrowAmount + (_borrowAmount * protocolFeeMantissa) / 1e18;
  ...
  /**
   * @dev repays borrow for a collateral
   * @param _repayAmount the amount to repay
   * @param _collateralId to want collateral id to pay for
   */
  function repayBorrowInternal(
    ...
    borrowBalancePerUser[coupon.owner] -= repayment;

    emit RepayBorrow(msg.sender, repayment, debt, totalBorrows, _collateralId);
  }
  ...
  function withdrawCollateralInternal(uint256 _collateralId) internal {
    Coupon memory activeCoupon = borrowCoupons[_collateralId];
    ...
    burnAndDelete(msg.sender, activeCoupon.collateralId, activeCoupon.id);

    doCollateralTransferOut(msg.sender, activeCoupon.collateralId);

    emit CollateralWithdrawn(msg.sender, _collateralId);
  }

  /**
   * @dev minting a coupon when depositing collateral, this coupon will accrue borrows
   * @param _to the receiver (who deposits the collateral)
   * @param _collateralId the id of the token that is deposited
   * @param _borrowAmount the amount that was borrowed
   */
  function _mintCoupon(
    ...
    Coupon memory coupon = borrowCoupons[_collateralId];
    ...
    coupon.borrowAmount = _borrowAmount;
    coupon.active = COUPON_ACTIVE;
    coupon.interestPerToken = interestPerToken();
    coupon.owner = _to;

    borrowCoupons[_collateralId] = coupon;
  ...
  /**
   * @dev does collateral transfer out, transfer nft to the _to
   * @param _to where to transfer the collateral
   * @param _collateralId what to transfer, the collateral id
   */
  function doCollateralTransferOut(address _to, uint256 _collateralId) internal {
    collateralToken.transferFrom(address(this), _to, _collateralId);
  }

  /**
   * @dev does collateral transfer in, transfer nft to the hToken
   * @param _from where to get the collateral
   * @param _collateralId what to transfer, the collateral id
   */
  function doCollateralTransferIn(address _from, uint256 _collateralId) internal {
    collateralToken.safeTransferFrom(_from, address(this), _collateralId);
  }
  ...
  /**
   * @dev liquidate borrow once the health factor is over the collateral factor.
   *         This transfers the nft to the liquidator where you can clawback
   * @param _collateralId what to liquidate
   */
  function liquidateBorrowInternal(uint256 _collateralId) internal {
    if (!liquidationAllowedByCollateralId(_collateralId)) revert HTokenError(Error.LIQUIDATION_NOT_ALLOWED);
    Coupon memory activeCoupon = borrowCoupons[_collateralId];

    bytes memory data = abi.encode(address(this));

    collateralToken.safeTransferFrom(address(this), liquidator, _collateralId, data);

    emit BorrowLiquidated(msg.sender, liquidator, activeCoupon.owner, _collateralId);
  }

  /**
   * @dev check if liquidation is allowed for this coupon, meaning it's active by collateral id
   * @param _collateralId what coupon to check for
   * @return allowed true if it's allowed
   */
  function liquidationAllowedByCollateralId(uint256 _collateralId) internal view returns (bool allowed) {
    return borrowCoupons[_collateralId].active == COUPON_ACTIVE;
  }
  ...
  function closeoutLiquidationInternal(address _borrower, uint256 _collateralId) internal {
    ...
    // Repay borrow
    repayBorrowInternal(_borrower, coupon.borrowAmount, _collateralId);

    // Burn the NFT coupon
    burnAndDelete(_borrower, coupon.collateralId, coupon.id);
    emit LiquidationClosed(msg.sender, _borrower, _collateralId, coupon.borrowAmount);
  }

  /**
   * @dev burns a coupon and deletes it from the data structure
   * @param _account user to whom to burn
   * @param _collateralId collateral associated with this coupon
   * @param _couponId what to burn
   */
  function burnAndDelete(
    ...
    borrowCoupons[_collateralId].active = COUPON_INACTIVE;
  ...
  function exchangeRateStored() public view returns (uint256) {
    ...
    if (_totalSupply == 0) {
      ...
      /*
       * Otherwise:
       *  exchangeRate = (totalCash + totalBorrows - (totalReserves + totalHiveFees + totalAdminFees)) / totalSupply
       */
      uint256 totalCash = getCashPrior();
      uint256 cashPlusBorrowsMinusReserves = totalCash + totalBorrows - (totalReserves + totalHiveFees + totalAdminFees);

      uint256 exchangeRate = (cashPlusBorrowsMinusReserves * 10**18) / _totalSupply;
      return exchangeRate;
    }
  }
  ...
  function getBorrowAmountForCollateral(uint256 _collateralId) external view returns (uint256) {
    return getBorrowFromCoupon(_collateralId);
  }
   ...
   * @param _collateralId index of the borrwed coupon
   * @return Debt denominated in the underlying token
   */
  function getBorrowFromCoupon(uint256 _collateralId) public view returns (uint256) {
    uint256 principal = borrowCoupons[_collateralId].borrowAmount;
    if (decimals < 18) {
      return principal + (principal * (interestPerToken() - borrowCoupons[_collateralId].interestPerToken)) / 10**(18 - decimals);
    } else {
      return principal + (principal * (interestPerToken() - borrowCoupons[_collateralId].interestPerToken)) / 10**18;
    }
  }
  ...
  function interestPerToken() public view returns (uint256) {
    // Short-circuit if no protocol debt
    if (totalBorrows == 0) {
    ...
    uint256 cashPrior = getCashPrior() - totalAdminFees - totalHiveFees;
    ...
    return interestPerTokenStored + (simpleInterestFactor * 10**decimals) / totalBorrows;
  }
  ...
  function accrueInterest() public {
    /* TODO: Update the comment math
     * Calculate the interest accumulated into borrows and reserves and the new index:
     *  simpleInterestFactor = borrowRate * blockDelta
     *  interestAccumulated = simpleInterestFactor * totalBorrows
     *  totalBorrowsNew = interestAccumulated + totalBorrows
     *  totalReservesNew = interestAccumulated * reserveFactor + totalReserves
     *  totalHiveFeesNew = interestAccumulated * hiveFee + totalHiveFees
    ...
    for (i; i <= length; ++i) {
      Coupon memory coupon = borrowCoupons[i];
      if (coupon.active == COUPON_ACTIVE) {
        interestAccumulated += _accrueInterestToCoupon(i);
      }
    }

    totalBorrows += interestAccumulated;

    totalReserves += (reserveFactorMantissa * interestAccumulated) / 10**18;
    totalHiveFees += (hiveFeeMantissa * interestAccumulated) / 10**18;
    totalAdminFees += (adminFeeMantissa * interestAccumulated) / 10**18;

    emit InterestAccrued(interestAccumulated, interestPerTokenStored, totalBorrows);
  }
   ...
   */
  function accrueInterestToCoupon(uint256 _couponId) public returns (uint256) {
    uint256 currentBlockNumber = block.number;
    uint256 accrualBlockNumberPrior = accrualBlockNumber;
    ...
    uint256 earned = _accrueInterestToCoupon(_couponId);

    totalBorrows += earned;

    totalReserves += (reserveFactorMantissa * earned) / 10**18;
    totalHiveFees += (hiveFeeMantissa * earned) / 10**18;
    totalAdminFees += (adminFeeMantissa * earned) / 10**18;

    return earned;
  }

  function _accrueInterestToCoupon(uint256 _couponId) internal returns (uint256) {
    Coupon storage userCoupon = borrowCoupons[_couponId];
    uint256 principalPrior = userCoupon.borrowAmount;
    uint256 interestPerTokenPrior = userCoupon.interestPerToken;

    uint256 earned;
    if (decimals < 18) {
      earned = ((principalPrior * (interestPerTokenStored - userCoupon.interestPerToken)) / 10**(18 - decimals));
    } else {
      earned = ((principalPrior * (interestPerTokenStored - userCoupon.interestPerToken)) / 10**18);
    }

    userCoupon.borrowAmount += earned;
    userCoupon.interestPerToken = interestPerTokenStored;

    borrowBalancePerUser[userCoupon.owner] += earned;

    /* We emit an AccrueInterest event */
    emit AccrueCouponInterest(borrowCoupons[_couponId], principalPrior, earned, interestPerTokenPrior, interestPerTokenStored);
  ...
  function accrueInterestToCouponsInternal(uint256[] calldata _ids) internal returns (uint256) {
    ...
    for (i = 1; i <= length; ++i) {
      Collateral memory collateral = collateralPerBorrowCouponId[_ids[i]];

      if (!collateral.active) continue;
      Coupon memory coupon = borrowCoupons[_ids[i]];
      if (coupon.active == COUPON_ACTIVE) {
        interestAccumulated += _accrueInterestToCoupon(_ids[i]);
      }
    }
   ...
   */
  function _addReservesInternal(uint256 _addAmount) internal {
    if (!hasRole(DEFAULT_ADMIN_ROLE, msg.sender)) revert Unauthorized();

    accrueInterest();

    // _addReservesFresh emits reserve-addition-specific logs on errors, so we don't need to.
    _addReservesFresh(_addAmount);
  }

  /**
   * @dev adds reserves, does not accrues interests
   * @param _addAmount what to add
   */
  function _addReservesFresh(uint256 _addAmount) internal {
    ...
    actualAddAmount = doUnderlyingTransferIn(msg.sender, _addAmount);

    totalReservesNew = totalReserves + actualAddAmount;

    /* Revert on overflow */
    if (totalReservesNew < totalReserves) revert Unexpected("overflow");
  ...
  function _reduceReserves(uint256 _reduceAmount) external {
    ...
    _reduceReservesFresh(_reduceAmount);
  }
  ...
  function _withdrawHiveFees(uint256 _amount) external {
    ...
    if (totalHiveFees < _amount) {
      revert AdminError(Error.AMOUNT_TOO_BIG);
    }

    if (getCashPrior() < totalHiveFees) {
      revert AdminError(Error.TOKEN_INSUFFICIENT_CASH);
    }

    totalHiveFees -= _amount;

    doUnderlyingTransferOut(hiveFeeReceiver, _amount);

    emit HiveFeesWithdrew(_amount);
  }
   ...
   * @param _reduceAmount Amount of reduction to reserves
   */
  function _reduceReservesFresh(uint256 _reduceAmount) internal {
    ...
    emit ReservesReduced(msg.sender, _reduceAmount, totalReservesNew);
  }
  ...
  function _beforeTokenTransfer(
    ...
    for (j; j < _ids.length; j++) {
      // HTokens don't require coupon management
      if (_ids[j] > 0) {
        Collateral memory collateral = collateralPerBorrowCouponId[_ids[j]];
        if (!collateral.active) continue;
        Coupon storage coupon = borrowCoupons[collateral.collateralId];
        coupon.owner = _to;
        borrowBalancePerUser[_from] -= coupon.borrowAmount;
        borrowBalancePerUser[_to] += coupon.borrowAmount;
        lengthToModify++;
      }
    }
...
contract ErrorReporter {
  enum Error {
    ...
    WITHDRAW_NOT_ALLOWED, //15
    INITIAL_EXCHANGE_MANTISSA,
    TRANSFER_ERROR,
    COUPON_LOOKUP,
    TOKEN_INSUFFICIENT_CASH,
    BORROW_RATE_TOO_BIG, //20
