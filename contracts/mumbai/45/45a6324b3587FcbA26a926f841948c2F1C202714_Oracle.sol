...
contract KAIToken is ERC20 {
    address internal immutable _curve;
    ...
    constructor() ERC20("SupremeDAO KAI Currency", "KAI", 18) {
        _curve = msg.sender;
    }

    function mint(address to, uint256 amount) external onlyBondingCurve returns (bool) {
        _mint(to, amount);
        return true;
    }

    function burn(address from, uint256 amount) external onlyBondingCurve returns (bool) {
        _burn(from, amount);
        return true;
    }
}
...
contract SupplyCurve {
    ...
    constructor(address formula, address vaultAddress, uint256 mintAmount) {
        ...
        kai = new KAIToken();
        // TODO: premint or no?
        kai.mint({ to: vaultAddress, amount: mintAmount });
    ...
    function initialize(address connector, uint256 transferAmount, uint256 splitRate, uint32 connectorWeight) public {
        if (_initialized) {
            revert();
        }
        _initialized = true;
    ...
    function enter(uint256 amount) external {
        ...
        kai.mint(msg.sender, mintAmount);
    ...
    function exit(uint256 amount) external {
        ...
        kai.burn({ from: msg.sender, amount: amount });
    }
