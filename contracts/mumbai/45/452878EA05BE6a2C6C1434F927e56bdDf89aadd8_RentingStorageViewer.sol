...
contract RentingStorageViewer is IRentingTypes, Ownable {
    ...
    struct PlayerBotInfo {
        uint256 id;
        bool rented;
    }

    struct PlayerLands {
        PlayerLandInfo[] lands;
        bool hasNext;
    }
    ...
    function getPlayerLands(address owner, uint page) external view returns (PlayerLands memory) {
        uint256[] memory ids = idsOwnedByAddress(TradedNft.RBXL, owner, page);

        PlayerLandInfo[] memory lands = new PlayerLandInfo[](ids.length);
        for (uint i = 0; i < ids.length; i++) {
            RentingInfo memory rentInfo = storageContract.getRentingInfo(ids[i]);
            bool isRented = rentInfo.renter != address(0);
            if (!isRented) {
                lands[i] = PlayerLandInfo(ids[i], false, new uint256[](0), owner, 0, 0, 0, 0);
            } else {
                lands[i] = PlayerLandInfo(ids[i], true, rentInfo.battleSet.botsIds, rentInfo.owner, rentInfo.id,
                    rentInfo.renewedPeriodEndTs, rentInfo.rentingEndTs, rentInfo.revenueShare);
            }
        }

        return PlayerLands(lands, getERC721Contract(TradedNft.RBXL).balanceOf(owner) >= PAGE_SIZE * (page + 1));
    }


    function getPlayerBots(address owner, uint page) external view returns (PlayerBots memory) {
        uint256[] memory ids = idsOwnedByAddress(TradedNft.RBFB, owner, page);

        PlayerBotInfo[] memory bots = new PlayerBotInfo[](ids.length);
        for (uint i = 0; i < ids.length; i++) {
            bots[i] = PlayerBotInfo(ids[i], getBotStatus(ids[i]) == TokenRentingStatus.RENTED);
        }

        return PlayerBots(bots, getERC721Contract(TradedNft.RBFB).balanceOf(owner) >= PAGE_SIZE * (page + 1));
    }
