...
interface NftInterface {
    ...
    function mint(address _to, uint16[] calldata _attributes) external;
    ...
    function updateAvatar(uint256 _targetId, uint16[] calldata _attributes) external;

    /// @notice Checking the availability of attributes for creating Genesis Nfts
    function checkAttributesTaken(uint16[] calldata _attributes) external returns (bool);
}
...
contract CustomContract is Pausable, AccessControl, ReentrancyGuard {
    ...
    uint256 public nrOfRecycledNfts;

    /// @notice Maximum number of attributes that nfts can have.
    /// @dev Nfts with more attributes then this number will be considered invalid
    /// @dev This value should increase only?
    uint256 public maxNrAttributes;

    /// @notice Trusted signature that should be used for encoding random attributes
    ...
    /// @notice Event triggered on setting trusted signer address
    event SetSignerAddress(address signerAddress);

    /// @notice Event triggered on recycling avatars
    event Recycle(uint256 indexed tokenId, uint16[] newAttributes);
    ...
    event MintNft(address indexed to, uint16[] attributes);

    /**
     * @notice The values of the 'partPosition' represents the position of the attributes as:
     * @notice 0 -> head, 1 -> leftArm, 2 -> rightArm ...
     * @notice Then we use the positions to know what attribute to extract from the avatar
     */
    struct RecycleParam {
        uint256 tokenId;
        uint8[] partPosition;
    ...
    constructor(
        NftInterface _nft,
        uint256 _maxNrAttributes,
        uint256 _nrOfRecycledNfts
    ) {
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        nft = _nft;
        nrOfRecycledNfts = _nrOfRecycledNfts;
        maxNrAttributes = _maxNrAttributes;
    }
     ...
     * @param _signature represents the signature used on enconding the attributes.
     * This signature should correspond with a trusted signature
     */
    function mintNft(
        address _to,
        uint16[] calldata _attributes,
        bytes calldata _signature
    ) external whenNotPaused nonReentrant {
        /// @dev check if the signature used to generate the random attributes for Genesis is the right one
        // if (
        //     keccak256(
        //         abi.encodePacked(
        //             _attributes[0].toString(),
        //             ",",
        //             _attributes[1].toString(),
        //             ",",
        //             _attributes[2].toString(),
        //             ",",
        //             _attributes[3].toString(),
        //             ",",
        //             _attributes[4].toString(),
        //             ",",
        //             _attributes[5].toString()
        //         )
        //     ).toEthSignedMessageHash().recover(_signature) != signerAddress
        // ) {
        //     revert InvalidSignature();
        // }

        /// @dev check if the generated attributes have been used in the past by a Genesis
        if (nft.checkAttributesTaken(_attributes)) {
            revert AttributesAlreadyTaken();
        }

        nft.mint(_to, _attributes);
        emit MintNft(_to, _attributes);
    }
     ...
     * @param _targetId repressents the token id of the target Nft
     * @param _recycleNft represents an array of objects, where an object contains the token id that
     * will be burned and an array with what attributes to update from the burned nft
     */
    function recycle(uint256 _targetId, RecycleParam[] calldata _recycleNft) external whenNotPaused nonReentrant {
        if (_recycleNft.length != nrOfRecycledNfts) {
            revert InvalidNumberOfRecycleTokens();
        }

        /// @dev This check can revert
        checkForOwnership(_targetId);
        ...
        uint16[] memory newAttributes = new uint16[](maxNrAttributes);
        ...
        uint256 i;
        for (i; i < targetAvatar.attributes.length; i++) {
            newAttributes[i] = targetAvatar.attributes[i];
        }

        for (i = 0; i < _recycleNft.length; i++) {
            RecycleParam memory currentToken = _recycleNft[i];

            /// @dev This check can revert
            checkForOwnership(currentToken.tokenId);

            uint16[] memory currentTokenParts;

            /// @dev This check can revert
            checkForNrOfAttributes(currentToken.partPosition.length);

            if (currentToken.partPosition.length > 0) {
                /// Bring all attributes here to have them local and not make more calls
                currentTokenParts = nft.getAvatar(currentToken.tokenId).attributes;

                uint256 j;
                for (j; j < currentToken.partPosition.length; j++) {
                    uint256 currentPartPosition = currentToken.partPosition[j];
                    /// @dev This check can revert
                    checkForDuplicates(partExist[currentPartPosition]);
                    partExist[currentPartPosition] = 1;
                    hasReplacement = true;
                    newAttributes[currentPartPosition] = currentTokenParts[currentPartPosition];
                }
            }
        }
        ...
        /// @dev Burn used Nfts
        for (i = 0; i < _recycleNft.length; i++) {
            nft.burn(_recycleNft[i].tokenId);
        }
        /// @dev Update targetNft with the new attributes
        nft.updateAvatar(_targetId, newAttributes);
     ...
     */
    function checkForOwnership(uint256 _tokenId) private view {
     ...
     */
    function checkForNrOfAttributes(uint256 _length) private view {
        if (_length > maxNrAttributes) {
            revert TooManyPartsOnAnNft();
        }
    }
    ...
    /**
     * @notice Set the trusted signer address used to check the seed received from backend on minting genesis nfts
     * @param _address represents the signer adress that needs to be saved on blockchain
     */
    function setSignerAddress(address _address) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    /**
     * @notice Set maximum number of attributes
     */
    function setMaxNrAttributes(uint8 _maxNrAttributes) external onlyRole(DEFAULT_ADMIN_ROLE) {
        maxNrAttributes = _maxNrAttributes;
        emit SetMaxNrAttributes(_maxNrAttributes);
    }
