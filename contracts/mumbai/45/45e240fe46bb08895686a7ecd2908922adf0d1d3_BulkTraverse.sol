...
contract BulkTraverse is NonblockingReceiver {
    ...
    function estimateGasFee(address sender_address, uint16 _chainId, uint[] memory tokenIds, address dst_tokenAddress) public view returns (uint){
        bytes memory payload = abi.encode(sender_address, tokenIds, dst_tokenAddress);
        uint16 version = 1;
    ...
    function traverseChains(uint16 _chainId, uint[] memory tokenIds, address src_tokenAddress, address dst_tokenAddress) public payable {
        ...
        for(uint i = 0 ; i < tokenIds.length ; i++) {
            IERC721(src_tokenAddress).burn(tokenIds[i]);
        }
    ...
    function _LzReceive(uint16 _srcChainId, bytes memory _srcAddress, uint64 _nonce, bytes memory _payload) override internal {
        ...
        for(uint i = 0 ; i < tokenIds.length ; i++) {
            IERC721(dst_tokenAddress).safeMint(toAddr, tokenIds[i]);
        }
    }
}