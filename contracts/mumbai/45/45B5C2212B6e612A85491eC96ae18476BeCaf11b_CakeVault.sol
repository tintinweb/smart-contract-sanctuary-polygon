...
contract CakeVault is Ownable, Pausable {
    ...
    address public harvestTokenAddress;
    ...
    constructor(
        ...
        harvestTokenAddress = _harvestTokenAddress;
        uniRouterAddress = _uniRouterAddress;
        earnedToToken0Path = [harvestTokenAddress,tokens[0]];
        earnedToToken1Path = [harvestTokenAddress,tokens[0]];
    ...
    function harvest() external notContract whenNotPaused {
        ...
        uint256 bal = harvestAvailable();
        uint256 currentPerformanceFee = bal.mul(performanceFee).div(10000);
        harvestToken.safeTransfer(treasury, currentPerformanceFee);

        uint256 currentCallFee = bal.mul(callFee).div(10000);
        harvestToken.safeTransfer(msg.sender, currentCallFee);

        bal = bal.sub(currentPerformanceFee).sub(currentCallFee);

        harvestToken.safeIncreaseAllowance(
            uniRouterAddress,
            bal
        );

        if (harvestTokenAddress != token0Address) {
        ...
        if (harvestTokenAddress != token1Address) {
    ...
    function calculateHarvestCakeRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingECO(pid, address(this));
    ...
    function harvestAvailable() public view returns (uint256) {
