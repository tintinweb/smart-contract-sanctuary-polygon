...
contract Posts {
    ...
    * @param fromComments : [0 = not comments / 1 = get Comments of user / 2 = get comments of post, isCommentOf message ID]
    */
    function getMsgIDs(address usrAddress, uint256 startFrom, uint256[] calldata fromComments) public view returns(uint256[] memory) {

        // Get the prefix for by address
        string memory fullPrefix = KUtils.append(KUtils.addressToString(usrAddress), fromComments[0] > 0 ? '-c' : '', '', '', '');

        // If a message ID was sent, then change the prefix to ID
        if (fromComments[0] == 2 && fromComments[1] > 0) {
            fullPrefix = KUtils.append(KUtils.toString(fromComments[1]), fromComments[0] > 0 ? '-c' : '', '', '', '');
        }
    ...
    function addPost(uint256 msgID, address addressPoster, uint256 isCommentOf) public onlyAdmins {
        ...
        string memory prefix = KUtils.append(KUtils.addressToString(addressPoster), comments, '', '', '');
        ...
        if (isCommentOf > 0){
            // Now save it all to the post it's commenting on
    ...
    function removePost(uint256 msgID, address addressPoster, uint256 isCommentOf) public onlyAdmins {
