...
contract Faces is ERC721 {
    using Counters for Counters.Counter;
    using SafeMath for uint32;

    Counters.Counter private _tokenIds;

    uint32 private _maxTokenId = 200;
    ...
    function mint(address to,uint32 amount) public{
        ...
        require(_tokenIds.current()+amount <= _maxTokenId,"mint Exceeds supply");
    ...
    function changeBaseURI(string memory newBaseURI)
        public
    {
        require((keccak256(abi.encodePacked((_baseTokenURI))) != keccak256(abi.encodePacked((newBaseURI)))), "Base URI cannot be same as previous");
