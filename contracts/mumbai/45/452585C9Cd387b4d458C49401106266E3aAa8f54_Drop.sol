    ...
    IERC2981
{
        ...
        whenNotPaused
    {
        ...
        if (
            ...
            phaseStates[phaseIndex].phaseEnd = block.timestamp + 1 days;
        }
    }
    ...
    function getAllPhases() external view returns (Phase[] memory, uint256[] memory) {
        Phase[] memory phasesList = new Phase[](phaseStartTimestamps.length);
        for (uint256 i = 0; i < phaseStartTimestamps.length; i++) {
            phasesList[i] = phases[i];
        }
        return (phasesList, phaseStartTimestamps);
    }
    ...
    function _validatePublicMint(
        ...
        if (
            ...
            require(
                block.timestamp <= phaseStates[phaseIndex].phaseEnd,
