...
import "./interfaces/IFlexiCipherMetadata.sol";
...
/// @title Cipher Domain Service TLD contract (Flexi)
/// @author Cipher Labs
/// @notice Dynamically generated NFT contract which represents a top-level domain
contract FlexiCipherTLD is IBaseCipherTLD, ERC721, Ownable, ReentrancyGuard {
  using strings for string;

  // Domain struct is defined in IBaseCipherTLD

  address public immutable factoryAddress; // FlexiCipherTLDFactory address
  address public metadataAddress; // FlexiCipherMetadata address
  ...
  mapping (string => Domain) public override domains; // mapping (domain name => Domain struct); Domain struct is defined in IBaseCipherTLD
  ...
  function tokenURI(uint256 _tokenId) public view override returns (string memory) {
    return IFlexiCipherMetadata(metadataAddress).getMetadata(
  ...
  function _mintDomain(
    ...
    Domain memory newDomain; // Domain struct is defined in IBaseCipherTLD
...
interface IFlexiCipherMetadata {
