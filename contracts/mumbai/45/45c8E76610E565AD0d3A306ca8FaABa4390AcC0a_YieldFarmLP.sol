...
contract YieldFarmLP {
    ...
    // constructor
    constructor(address muskTokenAddress, address uniLP, address stakeContract, address communityVault) public {
        _musk = IERC20(muskTokenAddress);
        _uniLP = uniLP;
    ...
    function massHarvest() external returns (uint){
        ...
        if (totalDistributedValue > 0) {
            _musk.transferFrom(_communityVault, msg.sender, totalDistributedValue);
        }
    ...
    function harvest (uint128 epochId) external returns (uint){
        ...
        if (userReward > 0) {
            _musk.transferFrom(_communityVault, msg.sender, userReward);
        }
