...
contract WowTCommunityPoll is OwnableUpgradeable, AccessControlUpgradeable {
  ...
  bytes32 public constant ADMIN_ROLE = keccak256('ADMIN_ROLE'); // Admin role for authorization
    ...
    function createPoll(string memory _communityName, string memory _question) public {
        require(
    ...
    function vote(string memory _communityName, uint256 _pollId, bool _choice) public {
        require(
            community.checkCommunityExists(_communityName),
            "Community doesn't exist"
        );
    ...
    function getPoll(string memory _communityName, uint256 _pollId) public view returns (string memory, uint256, uint256) {
        require(
            community.checkCommunityExists(_communityName),
            "Community doesn't exist"
        );
        ...
        return (poll.question, poll.yesVotes, poll.noVotes);
    }
}
