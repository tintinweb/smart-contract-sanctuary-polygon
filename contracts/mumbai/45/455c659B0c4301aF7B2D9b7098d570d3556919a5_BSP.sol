...
// ERC1155 contract which is upgradable(UUPS), ownable (controled by admin), pausable (both mint and transferring)
contract BSP is Initializable, ERC1155Upgradeable, OwnableUpgradeable, PausableUpgradeable, ERC1155SupplyUpgradeable, UUPSUpgradeable {
    
    /*
        VARIBLES DECLERATION
    */

    // CONTRACT STATE VARIBLES
    // Stock UID
    ...
    // Upgradeable contract use initialize function instead of constructor
    // The constructor validate that the initialize run only once
    /// @custom:oz-upgrades-unsafe-allow constructor
    constructor() {
        _disableInitializers();
    }

    // Initialize the contract with the following attributes given from hardhat (configured in .env):
    // initURI, initCompanyWallet, initMaxSupply, initMintPrice
    // All of those changeable later
    function initialize(
    ...
    */
    ...
    // Direct the proxy to a new implementation contract during upgrade
    function _authorizeUpgrade(address newImplementation) internal onlyOwner override{}

    // Pause all functions labeled by whenNotPaused (mint, transfer)
    function pause()
    public onlyOwner
    {
        _pause();
    ...
    */
    ...
    function setCompanyWallet(address payable newCompanyWallet)
    public
    onlyOwner {
    ...
    */
    ...
    function mintStocks(uint256 amount)
    payable public
    {
        // Validate that the max stocks amount havn't reached
        require(totalSupply(Stock) + amount <= maxSupply, "Max supply reached");
        // Validate if the tokem amount sent is equal or greater to the price
        require(msg.value >= (amount * mintPrice), "The suggested price doesn't meet the requirement");
        // Send paid tokens to the comapny wallet
    ...
    function mintStocksAdmin(address account, uint256 amount)
    public onlyOwner
    {
        // Validate that the max stocks amount havn't reached
        require(totalSupply(Stock) + amount <= maxSupply, "Max supply reached");
    ...
    */
    ...
    function withdraw(address payable _to, uint256 amountWEI)
    public onlyOwner
    {
        // Require the balance is sufficient 
        require(amountWEI <= address(this).balance, "Insufficient balance in the contract");
    ...
    */
    ...
    {
        super._beforeTokenTransfer(operator, from, to, ids, amounts, data);
        // Block minting and transferring to non valideating addresses
    ...
    */
    ...
    // Move dividend released values from sender to receiver when transfer occured
    function updateDividendReleasedAfterTransfer(address from, address to, uint256 amount)
    private
    {
        ...
        {   
            ...
            uint256 _releasedPerToken = releasedPerToken(_fromReleased, _fromBalance, amount);
        ...
        {   
            ...
            _dividendReleased[to] = _dividendReleased[to]+((_releasedPerToken)*amount);
        }
    }

    // Return the already released value for each token
    function releasedPerToken(uint256 _fromReleased, uint256 _fromBalance, uint256 _amountTransfered)
    private pure
    returns(uint256)
    {
        // dividend released / tokens number + 1 (to overcome down rounding)
        uint256 _releasedPerToken = _fromReleased/(_amountTransfered+_fromBalance) + 1;
        return _releasedPerToken;
    }

    // Release dividends for the requester
    function dividendRelease()
    public virtual
    {
        // Get the requester address
        address payable _account = payable(msg.sender);  
        // Validiate the requester own shares
        require(sharesOf(_account) > 0, "DividendSplitter: account has no shares");
        // Get the releasable amount
        uint256 payment = releasableDividends(_account);
        // Validiate the payment isnt zero
    ...
    // Get the amount of dividends the account allowed to pull
    function releasableDividends(address account)
    public view
    returns (uint256)
    {
        // The total amount of tokens need to be splitted
        // Summarize the current balance + the alreay pulled amount
        uint256 totalReceived = address(this).balance + totalDividendReleased();
        // return the calculated releasable amount
        return _pendingDividendPayment(account, totalReceived, dividendReleased(account));
    }

    // Calculate the amount of dividends the account allowed to pull
    function _pendingDividendPayment(address account, uint256 totalReceived, uint256 alreadyReleased)
    private view
    returns (uint256)
    {
        // Total stocks supply
        ...
        // Validate shares alreay minted
        require(_totalShares != 0, "DividendSplitter: No shares yet");
        // Return the pending payment amount
        // all splittable dividends * precent of the account own shares out of the total shares
        uint256 allowedToRelease = (totalReceived * _accountShares) / _totalShares;
        ...
        // Disallowed to release any amount (might be less then zero because of up rounding in "updateDividendReleasedAfterTransfer")
        else {return 0;}
    }

    // Get the amount of dividend already released
    function dividendReleased(address account)
    public view
    returns (uint256)
    {
    ...
    function totalDividendReleased()
    public view
    returns (uint256)
    {
    ...
    // Get the total amount of shares for specific account
    ...
    returns (uint256)
    {
        ...
        else {return balanceOf(account, Stock);}
    }
    ...
    */
    ...
    function addToWhitelist(address newAddress)
    public
    onlyOwner
    {
    ...
    // Check either the given address is in the whitelist or not
    function isInWhitelist(address addressToValidate)
    public view
    returns(bool)
    {
        if (whitelist[addressToValidate]){return true;}
        else {return false;}
    }
}