...
contract VaultTesting {
	...
	function depositToken(address tokenAdd, uint256 amount) public {
		require(IERC20(tokenAdd).balanceOf(msg.sender) >= amount, "Your token amount must be greater then you are trying to deposit");
		require(IERC20(tokenAdd).approve(address(this), amount));
		require(IERC20(tokenAdd).transferFrom(msg.sender, address(this), amount));

//		tokenBalance[tokenAdd] += amount;
		emit tokenDepositComplete(tokenAdd, amount);
	}
