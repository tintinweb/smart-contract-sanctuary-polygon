 ...
 */
contract PNftToken is ERC721, DefaultOwnable, NativeMetaTransaction {
    ...
    event Activated(string _tokenURI);
    ...
    ) ERC721(_name, _symbol) DefaultOwnable() {
        require(_proxyRegistry != address(0), "Zero address provided");
    ...
    /**
        ...
        1 is always subtracted from the `tokenId` since it tracks the next available id.
     */
    ...
    /**
        @dev Checks if baseTokenURI is set.
        @return True if baseTokenURI is set, false otherwise.
     */
    ...
    /**
     * Override isApprovedForAll to whitelist user's OpenSea
     * proxy accounts to enable gas-less listings.
     */
