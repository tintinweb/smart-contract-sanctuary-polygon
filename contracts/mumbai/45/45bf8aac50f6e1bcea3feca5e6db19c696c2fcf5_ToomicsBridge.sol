    ...
    ReentrancyGuardUpgradeable
{
    ...
    function lockNft(
        ...
        // lockNFT deposit
        IMintNFTToon(tooNft).transferFrom(msg.sender, address(this), _tokenId);
    ...
    function unlockNft(
        ...
        bytes32[] memory sigS,
        address _recipient,
        uint256 _tokenIdDest,
        uint256 _tokenIdSource,
        bytes32 _interchainTX,
        ...
        require(
            IMintNFTToon(tooNft).ownerOf(_tokenIdDest) == address(this),
    ...
    function refundNft(
        ...
        require(
            _checkRefundNftSign(sigV, sigR, sigS, _recipient, _tokenId, _nonce),
            "Invalid signature"
        );
        require(
        ...
        require(
            IMintNFTToon(tooNft).ownerOf(_tokenId) == address(this),
            "Nft does not lock in contract"
        );
        require(
            refundNftCompleted[_nonce].tokenId == _tokenId,
            "Invalid amount"
        );
...
contract Validator {
    ...
    function _checkUnlockSig(
        ...
        bytes32 inputHash = keccak256(
            abi.encode(funcHash, _recipient, _amount, _interchainTX, _chainId)
        );
    ...
    function _checkRefundSig(
        ...
        // digest the data to transactionHash
        bytes32 inputHash = keccak256(
            abi.encode(funcHash, _recipient, _amount, _nonce)
        );
        for (uint256 index = 0; index < sigV.length; index++) {
            // address recoveredAddress = ecrecover(inputHash, sigV[index], sigR[index], sigS[index]);
            if (
