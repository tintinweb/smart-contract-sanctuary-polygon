...
import "./TournmentNft.sol";
import "./Fund.sol";
contract Contract is Ownable , Fund,TournmentNft
{
    ...
    */
    ...
    // 0x0FA8781a83E46826621b3BC094Ea2A0212e71B23  // usdc mumbai
    constructor() 
    ...
    */
    function updateRank  (    uint256 _index, uint256 _tokenId) public  onlyOwner {
    ...
    */
    function updateRankAll  (uint256 [] memory _tokenIds) public  onlyOwner {
    ...
    */
    function getLeaderboard  ( uint256 _index ) public view onlyOwner returns ( uint256 )  {
    ...
    */
    function getRankof( uint256 _tokenId ) public view onlyOwner returns ( uint256)  {
    ...
    */
    function deposit(string memory  _tokenURI) public payable  {
        ...
        //require (  balanceOf(msg.sender)== 0,"You can have only one NFT per address ");
        //approve(address(this), baseQuote);
        //here we will put the erc20 approve flow
      
        // then generate a erc721 and send to  msg.sender
        createNft(_tokenURI,msg.sender);
    }
    ...
    */
    function redeem() public payable  {
        require(cupIdEnded == true,"");
        require (  balanceOf(msg.sender)== 1,"You can have only one NFT per address ");     // require msg.sender is owner of erc721 
   
       
        // check the leaderboard       
        // if position 0 then send  prize 1
        // if position 1 then send  prize 2
        // if position 2 then send  prize 3
        // if position > 3 then send nothing
       
       
        // this will be removed when we will start using the erc20 approve flow 
...
contract TournmentNft is  ERC721URIStorage, Ownable {
    ...
    /**
    @notice Token identification number counter.
    */
    Counters.Counter private _tokenIds;
  
    /**
    @notice Send an event when a new token NFT is created
    */
    event TournmentNftCreated (
    ...
    /**
    @notice contructor of the nft contract 
    */
    constructor() ERC721("Tournment Nft for FCO", "FCO") {}

    /**  
    @notice Mints a nft token  
    */
    function createNft(string memory _tokenURI, address to) onlyOwner public returns (uint256) 
    {
        ...
        emit TournmentNftCreated(newTokenId, msg.sender, to);
        return newTokenId;
    }
    ...
    function getTokenIds(address _owner) public view returns (uint[] memory) {
        ...
        for (i=0;i<ERC721.balanceOf(_owner);i++){
            _tokensOfOwner[i] = IERC721Enumerable(address(this)).tokenOfOwnerByIndex(_owner, i);
        }
