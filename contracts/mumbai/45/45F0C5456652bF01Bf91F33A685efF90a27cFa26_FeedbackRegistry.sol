...
contract FeedbackRegistry is IFeedbackRegistry {
    ...
    using Paginator for bytes[];

    // course name => feedbacks (ipfs)
    mapping(bytes => bytes[]) public contractFeedbacks;
    ...
    function addFeedback(
        ...
        require(
            _verifySignature(keys_[0], ringSignature_) == true,
    ...
    function getAllFeedbacks() external view returns (bytes[] memory courses_, bytes[][] memory feedbacks_) {
        uint256 coursesLength_ = _courses.length;

        courses_ = new bytes[](coursesLength_);
        feedbacks_ = new bytes[][](coursesLength_);
