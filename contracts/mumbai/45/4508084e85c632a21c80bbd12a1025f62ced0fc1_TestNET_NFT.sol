...
contract TestNET_NFT is ERC721Enumerable, Ownable {
  using Strings for uint256;
  using Counters for Counters.Counter;

  Counters.Counter private supply1;

  string public baseURI;
  string public baseExtension = ".json";
  string public notRevealedUri;
  uint256 public cost = 0.000001 ether; //цена за минт 1 нфт
  uint256 public maxSupply = 10; // количество нфт в вашей коллекции
  uint256 public maxMintAmount = 2; //количество нфт которое можно сминтить при 1 транзакции
  uint256 public nftPerAddressLimit = 2; //максимальное колечество нфт которое может сминтить 1 адрес
  ...
  address payable commissions = payable(0x88D1282122a3c3a38F4fC6Cf70bE9799172149cD); // адрес куда будут поступать коммисии с перепродаж ваших нфт, вместо моего адреса напиши свой
  address[] public whitelistedAddresses;
  mapping(address => uint256) public addressMintedBalance;

  constructor() ERC721("TestNFT", "TNFT") {
    setBaseURI("ipfs://Qmbzm1TyXvcCFC9TdpMrqnxkY9XLoUZ4Kxda1YucdqBGi2");
    setNotRevealedURI("ipfs://QmZdgwjNzGwMcXSUi47gADQ6wRwfXfcePpRov9n6ghPzNS/hiddin.json");
  }

  modifier mintCompliance(uint256 _mintAmount) {
    require(_mintAmount > 0 && _mintAmount <= maxMintAmount, "Invalid mint amount!");
    require(supply1.current() + _mintAmount <= maxSupply, "Max supply exceeded!");
    _;
  }
  ...
  function _mintLoop(address _receiver, uint256 _mintAmount) internal {
    for (uint256 i = 0; i < _mintAmount; i++) {
      supply1.increment();
      _safeMint(_receiver, supply1.current());
    }
  }
}