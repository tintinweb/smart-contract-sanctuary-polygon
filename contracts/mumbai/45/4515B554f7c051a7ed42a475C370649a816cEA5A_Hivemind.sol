...
contract Hivemind is HivemindV1Storage, HivemindInterface, ComptrollerErrorReporter, ExponentialNoError, Ownable {
   ...
   */
  function enterMarkets(HTokenI[] calldata _hTokens) public override {
   ...
   */
  function exitMarket(HTokenI _hToken) external override returns (uint256) {
    /* Get sender tokensHeld and amountOwed underlying from the hToken */
    //TODO: check this
    (uint256 oErr, uint256 tokensHeld, uint256 amountOwed, ) = _hToken.getAccountSnapshot(msg.sender);
    ...
    uint256 allowed = redeemAllowedInternal(_hToken, msg.sender, tokensHeld);
    if (allowed != 0) {
   ...
   * @param _hToken The market to verify the mint against
   * @param _minter The account which would get the minted tokens
   * @param _mintAmount The amount of underlying being supplied to the market in exchange for tokens
  ...
  function mintAllowed(
    ...
    require(!mintGuardianPaused[_hToken], "mint is paused");
  ...
  function redeemAllowed(
    ...
    uint256 allowed = redeemAllowedInternal(_hToken, _redeemer, _redeemTokens);
  ...
  function redeemAllowedInternal(
    ...
    (Error err, , uint256 shortfall) = getHypotheticalAccountLiquidityRedeemInternal(_redeemer, _hToken, _redeemTokens);
   ...
   * @param _tokenId The token id that it's borrowed against
  ...
  function borrowAllowed(
    ...
    // Pausing is a very serious situation - we revert to sound the alarms
    require(!borrowGuardianPaused[_hToken], "borrow is paused");
    ...
    if (!markets[_hToken].accountMembership[_borrower]) {
      ...
      // require(msg.sender == address(_hToken), "sender must be hToken");
    ...
    if (borrowCap != 0) {
      uint256 totalBorrows = HTokenI(_hToken).totalBorrows();
      uint256 nextTotalBorrows = add_(totalBorrows, _borrowAmount);
    ...
    (, uint256 shortfall) = getHypotheticalAccountLiquidityBorrowInternal(_borrower, _hToken, _tokenId, _borrowAmount);
   ...
   * @param _borrower The account which would borrowed the asset
   * @param _repayAmount The amount of the underlying asset the account would repay
   * @param _tokenId The nft id that should repay for
  ...
  function repayBorrowAllowed(
    ...
    (, uint256 shortfall) = getHypotheticalAccountLiquidityBorrowInternal(_borrower, _hToken, _tokenId, 0);
  ...
  //   if (!markets[_hToken].isListed || !markets[hTokenBorrowed].isListed) {
  //     return uint256(Error.MARKET_NOT_LISTED);
  //   }

  //   if (HTokenI(_hToken).hivemind() != HTokenI(hTokenBorrowed).hivemind()) {
   ...
   * @param _transferTokens The number of hTokens to transfer
  ...
  function transferAllowed(
    ...
    uint256 allowed = redeemAllowedInternal(_hToken, _src, _transferTokens);
    if (allowed != uint256(Error.NO_ERROR)) {
      return allowed;
    }
  ...
  /**
     ...
     * @param _hTokenModify The market to hypothetically redeem/borrow in
     ...
     * @param _collateralId The collateral id we calculate the account liquidity for the borrowed amount
    ...
    )
  {
    ...
    if (_redeemTokens > 0)
      (err, liquidity, shortfall) = getHypotheticalAccountLiquidityRedeemInternal(_account, _hTokenModify, _redeemTokens);
    else if (_collateralId > 0)
      (liquidity, shortfall) = getHypotheticalAccountLiquidityBorrowInternal(
        _account,
        _hTokenModify,
        _collateralId,
        _borrowAmount
  ...
  /**
     ...
     * @param _account The account to determine liquidity for
     * @param _collateralId The 721 Coupon that is used to compute the price
     * @param _borrowAmount The amount of underlying to hypothetically borrow
  ...
  function getHypotheticalAccountLiquidityBorrowInternal(
    ...
    vars.sumCollateral = oracle.getUnderlyingNFTPrice(_collateralId);
    // Read the balances and exchange rate from the hToken
    //todo check this
    uint256 oErr;
    (oErr, vars.hTokenBalance, vars.borrowBalance, vars.exchangeRateMantissa) = _hToken.getAccountSnapshot(_account);
    if (oErr != 0) {
      // semi-opaque error code, we assume NO_ERROR == 0 is invariant between upgrades
      // return (Error.SNAPSHOT_ERROR, 0, 0);
      revert HTokenError(Error.SNAPSHOT);
    }

    vars.collateralFactorUint = markets[_hToken].collateralFactorMantissa;

    if (vars.sumCollateral == 0) {
      // return (Error.PRICE_ERROR, 0, 0);
      revert HivemindError(Error.PRICE_ERROR);
    }
    vars.sumBorrowPlusEffects = vars.borrowBalance + _borrowAmount;
    console.log("vars.borrowBalance ", vars.borrowBalance);
    console.log("borrowAmount ", _borrowAmount);
    ...
    )
  {
    ...
    for (i; i < assets.length; i++) {
      ...
      (oErr, vars.hTokenBalance, vars.borrowBalance, vars.exchangeRateMantissa) = asset.getAccountSnapshot(_account);
      if (oErr != 0) {
        // semi-opaque error code, we assume NO_ERROR == 0 is invariant between upgrades
        return (Error.SNAPSHOT_ERROR, 0, 0);
      }
  ...
  //   uint256 priceBorrowedMantissa = oracle.getUnderlyingPrice(hTokenBorrowed);
  //   uint256 priceCollateralMantissa = oracle.getUnderlyingPrice(hTokenCollateral);
   ...
   */
  function _setPriceOracle(PriceOracleI _newOracle) public returns (uint256) {
   ...
   */
  function _setCollateralFactor(HTokenI _hToken, uint256 _newCollateralFactorMantissa) external returns (uint256) {
    // Check caller is admin
    if (msg.sender != owner()) {
   ...
   */
  function _supportMarket(HTokenI _hToken) external returns (uint256) {
    if (msg.sender != owner()) revert Unauthorized();
  ...
  function _addMarketInternal(HTokenI _hToken) internal {
    ...
    for (i; i < allMarkets.length; i++) {
      require(allMarkets[i] != _hToken, "market already added");
    }
   ...
   * @return uint256 0=success, otherwise a failure. (See enum Error for details)
   */
  function _setPauseGuardian(address _newPauseGuardian) public returns (uint256) {
    if (msg.sender != owner()) revert Unauthorized();
  ...
  function _setMintPaused(HTokenI _hToken, bool _state) public returns (bool) {
    require(markets[_hToken].isListed, "cannot pause a market that is not listed");
    ...
    mintGuardianPaused[_hToken] = _state;
    emit ActionPausedhToken(_hToken, "Mint", _state);
    return _state;
  }

  function _setBorrowPaused(HTokenI _hToken, bool _state) public returns (bool) {
    require(markets[_hToken].isListed, "cannot pause a market that is not listed");
    require(msg.sender == pauseGuardian || msg.sender == owner(), "only pause guardian and admin can pause");
    require(msg.sender == owner() || _state == true, "only admin can unpause");

    borrowGuardianPaused[_hToken] = _state;
   ...
   * @param _hToken The market to check if deprecated
   */
  function isDeprecated(HTokenI _hToken) public returns (bool) {
    return
      markets[_hToken].collateralFactorMantissa == 0 &&
      borrowGuardianPaused[_hToken] == true &&
      _hToken.reserveFactorMantissa() == 1e18;
  }
...
error InvalidCoupon();
error HivemindError(ComptrollerErrorReporter.Error error);
error AdminError(TokenErrorReporter.Error error);
error MarketError(ComptrollerErrorReporter.Error error);
error HTokenError(ComptrollerErrorReporter.Error error);
...
abstract contract HivemindInterface {
  ...
  function exitMarket(HTokenI _hToken) external virtual returns (uint256);
...
contract HivemindV1Storage {
  ...
  mapping(HTokenI => bool) public mintGuardianPaused;
  mapping(HTokenI => bool) public borrowGuardianPaused;
