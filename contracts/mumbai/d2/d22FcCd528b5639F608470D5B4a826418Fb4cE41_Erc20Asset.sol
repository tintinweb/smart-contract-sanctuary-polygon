...
contract LedgerManager is ReentrancyGuard {
    ...
    */
    function removeOrder(uint256 orderId, uint256 rewardBaseAmount, bytes memory signature) external payable {
        ...
        require(success, "Order didn't exist");
...
contract PaypoolV1ERC20 is NonblockingLzAppUpgradeable, ERC20Upgradeable, PausableUpgradeable, IOFTUpgradeable {
    ...
    function initialize(ILayerZeroEndpoint _lzEndpoint, uint256 _globalSupply, uint256 _localSupply) public initializer {
        __PaypoolV1ERC20_init(_lzEndpoint, _globalSupply, _localSupply);
    }

    // solhint-disable-next-line func-name-mixedcase
    function __PaypoolV1ERC20_init(ILayerZeroEndpoint _lzEndpoint, uint256 _globalSupply, uint256 _localSupply) internal onlyInitializing {
        __PaypoolV1ERC20_init_unchained(_globalSupply, _localSupply);
        __Ownable_init_unchained();
        __ERC20_init_unchained("Bright Pool One", "BP1");
        __Pausable_init_unchained();
        __LzApp_init_unchained(_lzEndpoint);
    }

    // solhint-disable-next-line func-name-mixedcase
    function __PaypoolV1ERC20_init_unchained(uint256 _globalSupply, uint256 _localSupply) internal onlyInitializing {
        globalSupply = _globalSupply;
        _mint(_msgSender(), _localSupply);
    }
    ...
    function send(uint16 _dstChainId, bytes calldata _toAddress, uint _amount, address payable _refundAddress, address _zroPaymentAddress, bytes calldata _adapterParam) external payable virtual override {
        _send(_msgSender(), _dstChainId, _toAddress, _amount, _refundAddress, _zroPaymentAddress, _adapterParam);
    }

    function sendFrom(address _from, uint16 _dstChainId, bytes calldata _toAddress, uint _amount, address payable _refundAddress, address _zroPaymentAddress, bytes calldata _adapterParam) external payable virtual override {
        if (_from != _msgSender()) {
...
contract DailyLimit {
    ...
    uint256 public currentlyInPool;

    bytes32 constant public LEDGER_MANAGER_ROLE = keccak256("LEDGER_MANAGER_ROLE");
    modifier onlyManagerRole() {
        systemContext.checkRole(LEDGER_MANAGER_ROLE, msg.sender);
        _;
    }
    ...
    constructor (SystemContext systemContext_, uint256 startingPoolSize_, uint256[] memory startTimestamps_, uint256[] memory maxPerDays_, uint256[] memory maxExistingInPools_) {
        ...
        currentlyInPool = startingPoolSize_;
    ...
    function _recalculatePool(uint256 time) internal view returns(uint256, uint256) {
        if (lastReward == time) {
            return (currentlyInPool, scheduleIndex);
        }

        uint256 newPoolAmount = currentlyInPool;
    ...
    function _findMiddle(uint256 smaller, uint256 higher) internal pure returns(uint256) {
        uint256 diff = (higher - smaller) * 5000; // half of the diff with 4 decimals precision

        return smaller * 10000 + diff;
    }
    ...
    function _calculateReward(uint256 reward_, uint256 currentlyInPool_, uint256 scheduleIndex_) internal view returns(uint256) {
        ...
        uint256 adjustedReward = reward_;
        if (currentlyInPool_ > schedule.maxPerDay) {
            adjustedReward *= _findMiddle(schedule.maxPerDay, currentlyInPool_) / 10000;
        } else {
            adjustedReward *= _findMiddle(currentlyInPool_, schedule.maxPerDay) / 10000;
        }

        return adjustedReward / schedule.maxPerDay;
    }
    ...
    function withdrawReward(address userAddress, uint256 userReward) external onlyManagerRole() returns(uint256) {
        // solhint-disable-next-line not-rely-on-time
        (currentlyInPool, scheduleIndex) = _recalculatePool(block.timestamp);
        // solhint-disable-next-line not-rely-on-time
        lastReward = block.timestamp;
        uint256 reward = _calculateReward(userReward, currentlyInPool, scheduleIndex);
        currentlyInPool -= reward;
