  ...
  ReentrancyGuard
{
  ...
  uint256 private _rewardsDuration = 7 days;
    ...
    return
      rewardPerTokenStored.add(
        lastTimeRewardApplicable()
          .sub(lastUpdateTime)
          .mul(rewardRate)
          .mul(1e18)//this done because erc20 token has 18 decimals by default.
          .div(_totalSupply)
      );
  }
  ...
  function earned(address account) public view override returns (uint256) {
    return
      _balances[account]
        ...
        .div(1e18)//this done because erc20 token has 18 decimals by default.
        .add(rewards[account]);
  }
  ...
  {
    require(amount>0, 'staker cannot stake 0 tokens');
