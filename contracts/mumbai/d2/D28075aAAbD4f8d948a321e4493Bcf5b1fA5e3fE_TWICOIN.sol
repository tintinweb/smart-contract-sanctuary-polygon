...
interface ITWICOIN {
     ...
     * @dev payBind
     */
    function payBind(string memory twi_string_,uint256 fee) external;
...
abstract contract TIP is ContractTools, ERC20 {
  ...
  struct TipInfo{
    ...
    uint256 recipientCount;//number of people received who have received tips
    uint256 maxRecipientCount;//max number of people who can receive tips
    uint256 remainingAmount;//remaining amount
    bool isWhiteListRecipient;//white list recipient or not
    bool isRandomDistribution;//random distribution or not
    bool isVerifiedRecipient;//verified users or not
  ...
  uint256 internal constant MAX_TIP_AMOUNT = 1000000000000000000000000;//1M
    ...
    //event: create a tip
    event SetTip(address indexed creatorAddress, uint256 tipAmount, uint256 maxRecipientCount, bool isWhiteListRecipient_,
                    bool isRandomDistribution, bool isVerifiedRecipient, uint256 startTime, uint256 endTime);

    //event: get a tip
    event GetTip(address indexed receiver, uint256 Amount, uint256 receivedTime);

    //event: return the remaining tip amount
    event ReturnRemainingTip(address indexed receiver, uint256 Amount, uint256 timestamp);
   ...
   * @param merkleRoot_  white list merkle root
   */
    ...
    /**
   * _checkAddressInWhiteList
   * @param whiteListOwnerAddress white list owner address
   * @param proof proof
   */
    function _checkAddressInWhiteList(address whiteListOwnerAddress, bytes32[] memory proof, address leafAddress) view private returns(bool){
        ...
        bool isTwiInWhiteList = twiUserName.length>0?MerkleProof.verify(proof,whiteListMerkleRoot[whiteListOwnerAddress],keccak256(abi.encodePacked(twiUserName))):false;
   ...
   * @param maxRecipientCount_ max number of people who can receive this tip
   * @param isRandomDistribution_ random distribution or not
   * @param isVerifiedRecipient_ verified users or not
   * @param startTime_ start time
   * @param endTime_ end time
   */
    function setTip(string memory tipCode_,
        uint256 tipAmount_,
        uint256 maxRecipientCount_,
        bool isWhiteListRecipient_,
        bool isRandomDistribution_,
        bool isVerifiedRecipient_,
        uint256 startTime_,
        uint256 endTime_)
        ...
        require(maxRecipientCount_ >= MIN_RECEIVER_NUM && maxRecipientCount_ <= MAX_RECEIVER_NUM,"405");

        //The total amount must be greater than or equal to one times the total number of recipients
        require(tipAmount_ >= maxRecipientCount_.mul(MIN_SING_RECEIVE_VALUE),"406");

        //Return the remaining amount to the owner of the tip(if any)
        _returnRemainingTip(tipCode);

        _buildTip(tipCode, tipAmount_, maxRecipientCount_, isWhiteListRecipient_, isRandomDistribution_, isVerifiedRecipient_, startTime_, endTime_);
    ...
    function _buildTip(bytes memory tipCode, 
        ...
        tipAmount = isRandomDistribution_?tipAmount_:tipAmount_.sub(tipAmount_.mod(maxRecipientCount_));
        ...
        TipInfo memory tipInfo = TipInfo ({
            ...
            tipAmount: tipAmount,
            recipientCount: 0,
            maxRecipientCount: maxRecipientCount_,
            remainingAmount: tipAmount,
            isWhiteListRecipient: isWhiteListRecipient_,
            isRandomDistribution: isRandomDistribution_,
            isVerifiedRecipient: isVerifiedRecipient_,
        ...
        emit SetTip(_msgSender(), tipAmount, maxRecipientCount_, isWhiteListRecipient_, isRandomDistribution_, isVerifiedRecipient_, startTime_, endTime_);
    }
    ...
    * getTipByWhiteList
    * @param tipCode_  tip code
    * @param wlProof_  white list proof array
    ...
    */
    function getTipByWhiteList(string calldata tipCode_, bytes32[] calldata wlProof_) 
        ...
        //Check whether the recipient is in the white list
        if(tipInfo.isWhiteListRecipient){
    ...
    */
    function getTip(string calldata tipCode_)  
        ...
        require(!tipInfo.isWhiteListRecipient,"413");
    ...
    */
    function _checkTip(bytes memory tipCode,TipInfo memory tipInfo) private returns(uint256){
        ...
        //Tip code must exist
        require(tipInfo.remainingAmount > 0,"407");
        ...
        if(tipInfo.isVerifiedRecipient){
    ...
    */
    function _getTip(bytes memory tipCode,TipInfo memory tipInfo) private returns(uint256){
        ...
        if(!tipInfo.isRandomDistribution){
            receivedAmount = tipInfo.tipAmount.div(tipInfo.maxRecipientCount);
        }
        else{
            uint256 dCount = tipInfo.maxRecipientCount.sub(tipInfo.recipientCount);
            receivedAmount = dCount==1?tipInfo.remainingAmount:MIN_SING_RECEIVE_VALUE+random(tipInfo.remainingAmount.div(dCount).mul(2));
        }

        tipRecord[tipCode].recipientCount ++;
    ...
    //Return the remaining tip amount
    function returnRemainingTip(string calldata tipCode_) 
        ...
        bytes memory tipCode = bytes(tipCode_);
        if(_msgSender() ==  tipRecord[tipCode].creatorAddress){
            return _returnRemainingTip(bytes(tipCode));
        }else{
            revert();
        }
    }

    //After the tip time has passed, return the remaining tip amount to ex-creator address
    function _returnRemainingTip(bytes memory tipCode) private returns(uint256){
        uint256 returnAmount = tipRecord[tipCode].remainingAmount;
        address creatorAddress = tipRecord[tipCode].creatorAddress;
        if(returnAmount > 0 && tipRecord[tipCode].endTime < block.timestamp){

          delete tipRecord[tipCode];
          _transferFromDepositer(creatorAddress, returnAmount);
          emit ReturnRemainingTip(creatorAddress,returnAmount,block.timestamp);
...
contract TWICOIN is ITWICOIN, VERIFY, TIP{
  ...
  uint256 constant SEC_PER_DAY = 86400;
    ...
    modifier _verifyApprove(uint256 fee){

        require(fee > 0,"206");
        require(IERC20(linkToken).balanceOf(_msgSender()) >= fee,"207");
        require(getLinkTokenAllowance(_msgSender()) >= fee,"208");
    ...
    function _isBound(bytes memory twi_bytes) view private {
    ...
    */
    function bind(string calldata twi_string_) override public
        ...
        bindRecord[requestVerify(0,twi_string_, addressToString(_msgSender()), bindTagName)] = verifyObj;
    ...
    * payBind
    * @param twi_string_  Twitter username in string format
    * @param fee  Gas fee for pay
    */
    function payBind(string calldata twi_string_,uint256 fee) override public
        _isStrValid(twi_string_, USERNAME_MIN_LENGTH, USERNAME_MAX_LENGTH, NOT_ALL_NUM) 
        _verifyApprove(fee){
        ...
        require(IERC20(linkToken).transferFrom(_msgSender(),address(this),fee),"209");
        ...
        bindRecord[requestVerify(fee,twi_string_, addressToString(_msgSender()), bindTagName)] = verifyObj;
    ...
    * @param twiUserName  Twitter username to bind
    */
    function _doBind(address bindAddress, bytes memory twiUserName) private{
    ...
    * getBalanceByTwi
    * @param twi_string_  Twitter username in string format
    * @return balance  Balance of this Twitter user
    */
...
abstract contract VERIFY is Context, ChainlinkClient, ConfirmedOwner {
    ...
    function requestVerify(
        ...
        return sendOperatorRequestTo(oracleAddress, req, fee);
