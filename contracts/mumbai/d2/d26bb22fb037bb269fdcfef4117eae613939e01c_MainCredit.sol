...
// [233, 1000000000000000000, 1000547000000000000, "0x735CF571331D2A8C95834Dd8d1f05384b0Ca135E", "0x4bca564ce1a531063e2b88600ecdbd5d30aa9be3", 1, 1680235335, 0, 0]
    ...
    LoanCurrencyWrapper
{
    ...
    // Keeps track of each user's ETH balance in the contract
    ...
    // Weth address from goerli testnet
    ...
    /**
    @dev Allows a user to deposit ETH as collateral for a loan.
    @param _collateralAddress The address of the ERC721 contract for the collateral.
    @param _collateralId The ID of the collateral token being deposited.
    ...
    */
    function depositEth(
        ...
        external payable 
    {
        _depositEth(_collateralAddress, _collateralId);
    }
    ...
    function _createLoan(
        ...
        // Check if the loan currency is WETH or not
        if(_offer.loanCurrencyAddress == wethAddress) {
            // If the loan currency is WETH, transfer the principal amount from the lender to the contract
            _saveTransferEth(payable(msg.sender), _offer.principalAmount);

            // Update the balance of the sender to reflect the transfer of the principal amount
            balanceEth[_lender] -= _offer.principalAmount;
        } else {
            // If the loan currency is not WETH, transfer the principal amount from the lender to the borrower
            _safeFTTransferFrom(
                _offer.loanCurrencyAddress,
                _lender,
                msg.sender,
        ...
        internal
    {
        ...
        // Get the admin fee based on the collateral address and ID
        uint256 adminFee = _getAdminFee(_collateralAddress, _collateralId);
        // Get the total amount to be paid to the lender
        uint256 totalAmountforLender = _getUserInterest(_collateralAddress, _collateralId);

        // If the loan currency is WETH, transfer the total amount to the lender
        if(loanTerm.offer.loanCurrencyAddress == wethAddress) {
            _saveTransferEth(payable(loanTerm.lender), totalAmountforLender);
            
            // If there is an admin fee, transfer ETH it to the admin platform
            if (adminFee > 0) {
                _saveTransferEth(payable(adminPlatform), adminFee);
            }
        } else {
            // If the loan currency is not WETH, transfer the total amount to the lender
            _safeFTTransferFrom(
                loanTerm.offer.loanCurrencyAddress,
                msg.sender,
                loanTerm.lender,
                totalAmountforLender
            );

             // If there is an admin fee, transfer token it to the admin platform
            if (adminFee > 0) {
                _safeFTTransferFrom(
                    loanTerm.offer.loanCurrencyAddress,
                    msg.sender,
    ...
    /**
    * @dev Deposits ETH into the contract and updates the balance of the sender.
    * If the sender has already deposited ETH before, the new deposit must be greater
    * than or equal to the previous deposit. Otherwise, the excess amount will be returned
    * to the sender.
    */
    function _depositEth(address _collateralAddress, uint256 _collateralId) private {
        _checkCollateralIsNotLoaned(
            _collateralAddress,
            _collateralId
        );
        _checkCollateralIsWhitelisted(_collateralAddress);
        // Get the address of the sender
        address sender = msg.sender;
        // Get the amount of ETH being deposited
        uint256 currentDepositAmount = msg.value;
        // Flag indicating whether the function is returning excess ETH to the sender
        bool isRepayETH;

        // Check if the sender has already deposited ETH to this contract
        if(balanceEth[sender] == currentDepositAmount && balanceEth[sender] > 0) {
            // If the sender has already deposited the same amount of ETH before, throw an error
            revert("Amount of ETH sent to the previous contract");
        }

        // If the sender has deposited ETH before and the new deposit is less than the previous deposit
        if (currentDepositAmount < balanceEth[sender]) {
            // Calculate the amount of ETH to return to the sender
            uint256 amountToReturn = balanceEth[sender] - currentDepositAmount;
            // Check that the contract has enough ETH to return to the sender
            require(address(this).balance >= amountToReturn, "Insufficient contract balance");
            // Send the excess ETH back to the sender
            _saveTransferEth(payable(sender), amountToReturn + currentDepositAmount);
            // Update the sender's balance
            balanceEth[sender] -= amountToReturn;
            // Set the flag to indicate that the excess ETH was returned to the sender
            isRepayETH = true;
        } 
        // If the sender has not deposited ETH before or the new deposit is greater than the previous deposit
        else {
            // Calculate the amount of ETH to deposit
            uint256 amountToDeposit = currentDepositAmount - balanceEth[sender];
            // Check that the contract has enough ETH to deposit
            require(address(this).balance >= amountToDeposit, "Insufficient contract balance");
            // Send the current deposit amount to the sender
            _saveTransferEth(payable(sender), balanceEth[sender]);
            // Update the sender's balance
            balanceEth[sender] = currentDepositAmount;
        }
        // Emit an event to notify the client that the deposit was successful
        emit DepositEth(sender, currentDepositAmount, isRepayETH);
    }
