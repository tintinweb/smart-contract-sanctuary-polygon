...
contract TournamentManager is ERC1155("sample_uri"), Ownable, AccessControl {
    ...
    function getLeaderboard(uint256 tournamentId)
        public
        view
        returns (ITournamentManager.Rank[] memory ranks)
    {
        ITournamentManager.Tournament storage tournament = tournaments[
            tournamentId
        ];
        ranks = new ITournamentManager.Rank[](tournament.userCount);
        bytes32 rankIndex = tournament.rankFirst;
        uint256 counter = 0;
        while (rankIndex != 0) {
            ranks[counter] = tournamentUserRank[rankIndex];
            rankIndex = tournamentUserRank[rankIndex].next;
            counter++;
        }
    }
