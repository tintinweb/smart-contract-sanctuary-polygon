...
contract wUTILTokenICO is Ownable {
    ...
    // end sale
    function endsale(address usdtToken) external onlyOwner {
        // transfer all the remaining wUTIL tokens to admin
        require(wUTILtoken.transfer(msg.sender, wUTILtoken.balanceOf(address(this))), "Error to withdraw wUtil tokens");

        // transfer all the remaining USDT tokens to admin
        require(IERC20(usdtToken).transfer(msg.sender, IERC20(usdtToken).balanceOf(address(this))), "Error to withdraw USDT");
