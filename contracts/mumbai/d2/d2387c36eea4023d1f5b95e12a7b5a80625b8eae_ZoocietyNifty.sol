...
contract ZoocietyNifty is ERC721, Pausable, Ownable2Step, ERC721Burnable {
    ...
    mapping(uint256 => string) private access;
    ...
    constructor() ERC721("Zoociety Nifty", "zNFT") {
        mint(msg.sender);
    ...
    function mintSoul(address _address) public {
    ...
    function mintMe(address payable _affiliateId) public payable {
        require(msg.value >= price());
        require(!restricted[msg.sender]);
        if (isAffiliate[_affiliateId]) {
            require(msg.sender != _affiliateId);
            uint256 reward = msg.value.mul(100).div(10);
            _affiliateId.transfer(reward);
            admin.transfer(price().sub(reward));
        }
        mint(msg.sender);
    }
        ...
        returns (string memory)
    {
        ...
        string memory description = (
            (soulIds[ownerOf(tokenId)] != tokenId)
                ? "ZOOCIETY SOULBOUND NIFTY"
                : "ZOOCIETY NIFTY"
        );
        ...
        returns (string memory)
    {
        ...
        string memory backFigure = NiftyLibrary.figure(
            images[tokenId],
            tags[tokenId]
        );
    ...
    function addUsername(uint256 tokenId, string memory _tag) public onlyOwner {
        tags[tokenId] = _tag;
    }
