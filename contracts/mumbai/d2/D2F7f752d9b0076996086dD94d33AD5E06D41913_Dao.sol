...
contract Dao {
    ...
    function createProposal(string memory _description, string memory _offChainId, address[] memory _canVote, uint _deadline, string memory _category) public {
        ...
        newProposal.categogry = _category;

        emit proposalCreated(nextProposal,_offChainId, _description, _canVote.length, msg.sender, block.timestamp, _category);
        nextProposal++;
    }
    ...
    function countVotes(uint256 _id) public {
        ...
        emit proposalCount(_id, p.passed,block.timestamp);
    }
