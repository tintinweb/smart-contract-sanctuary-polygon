...
contract MuskTokenDistributor is Ownable {
    using ECDSA for bytes32;

    event Claim(address tokenOwner, uint256 day, uint256 amount);
    ...
    modifier onlyNotInitialized() {
        require(isInitialised == false, 'MuskTokenDistributor: contract already initialised');
        _;
    }
    ...
    modifier onlyInitialized() {
        require(isInitialised, 'MuskTokenDistributor: contract not initialised');
        _;
    }
    ...
    modifier onlyNotPaused() {
        require(isPaused == false, 'MuskTokenDistributor: contract is paused');
        _;
    }
    ...
    modifier onlyMerkleTreeOwner() {
        require(
            msg.sender == merkleTreeOwner,
            'MuskTokenDistributor: caller is not the merkle tree owner'
        );
        _;
    }
    ...
    constructor(
        ...
        require(_muskToken != address(0), 'MuskTokenDistributor: invalid musk token address');
    ...
    function initialise(uint256 _startTimestamp, bool _isPaused)
        ...
        onlyOwner
    {
        require(_startTimestamp > 0, 'MuskTokenDistributor: invalid start timestamp');
    ...
    function _setTreasuryAddress(address _treasuryAddress) internal {
        require(_treasuryAddress != address(0), 'MuskTokenDistributor: invalid treasury address');
    ...
    function _setTrustedWallet(address _trustedWallet) internal {
        require(
            _trustedWallet != address(0),
            'MuskTokenDistributor: invalid trusted wallet address'
        );
    ...
    function _setMerkleTreeOwner(address _merkleTreeOwner) internal {
        require(
            _merkleTreeOwner != address(0),
            'MuskTokenDistributor: invalid merkle tree owner address'
        );
        merkleTreeOwner = _merkleTreeOwner;
    }

    /**
     * @dev Add Merkle Tree Root for a day
     * @param merkleTreeRoot The Merkle Tree Root
     * @param day The day from start timestamp
     **/
    function addMerkleTreeRoot(bytes32 merkleTreeRoot, uint256 day) external onlyMerkleTreeOwner {
        require(
            merkleTreeRoots[day] == 0,
            'MuskTokenDistributor: merkle tree root already added for this day'
        );
        _setMerkleTreeRoot(merkleTreeRoot, day);
    }
     ...
     * @param day The day from start timestamp
     **/
    function _setMerkleTreeRoot(bytes32 merkleTreeRoot, uint256 day) internal {
        require(day > 0, 'MuskTokenDistributor: invalid day');
        uint256 today = _getDayFromStart(block.timestamp);
        require(day < today, 'MuskTokenDistributor: cannot add merkle tree after yesterday');
     ...
     * @param addresses The array of addresses where the tokens will be transferred
     * @param timestamps The array of timestamps (in seconds) for the claimed days
     * @param amounts The array of token amounts
     * @param signatures The array of signatures. Each signature is based on address + timestamp + amount
    ...
    function claim(
        ...
        require(addresses.length > 0, 'MuskTokenDistributor: empty addresses array');
        require(
            addresses.length == timestamps.length,
            'MuskTokenDistributor: arrays of different lengths'
        );
        require(
        ...
        require(
            addresses.length == signatures.length,
            'MuskTokenDistributor: arrays of different lengths'
        );
        require(
        ...
        for (uint256 i = 0; i < timestamps.length; i++) {
            _claim(addresses[i], timestamps[i], amounts[i], signatures[i], proofs[i], today);
        }
    }
     ...
     * @param timestamp The timestamp (in seconds) for the claimed day
     * @param amount The token amount
     * @param signature The signature based on address + timestamp + amount
    ...
    function _claim(
        ...
        require(tokenOwner != address(0), 'MuskTokenDistributor: invalid address');
        require(timestamp >= startTimestamp, 'MuskTokenDistributor: cannot claim before the start');
        uint256 day = _getDayFromStart(timestamp);
        require(day < today, 'MuskTokenDistributor: cannot claim after yesterday');
        require(amount > 0, 'MuskTokenDistributor: invalid amount');
        require(
            tokenClaims[tokenOwner][day] == false,
            'MuskTokenDistributor: tokens already claimed'
        );
        require(merkleTreeRoots[day] != 0, 'MuskTokenDistributor: no merkle tree root found');

        bytes32 hash = _getHash(tokenOwner, day, amount);
        (address recovered, ECDSA.RecoverError error) = hash.toEthSignedMessageHash().tryRecover(
            signature
        );
        require(
            error == ECDSA.RecoverError.NoError && recovered == trustedWallet,
            'MuskTokenDistributor: signer not trusted wallet'
        );
        require(
            MerkleProof.verify(proof, merkleTreeRoots[day], hash),
            'MuskTokenDistributor: invalid proof'
        );

        tokenClaims[tokenOwner][day] = true;
        require(
            muskToken.transferFrom(treasuryAddress, tokenOwner, amount),
            'MuskTokenDistributor: transfer amount failed'
        );
        emit Claim(tokenOwner, day, amount);
    }
