...
contract PerpetualUpdateLogic is PerpetualUpdateFunctions, IPerpetualUpdateLogic, IFunctionList {
    ...
    function updateAMMTargetFundSize(uint24 _iPerpetualId) external override {
        require(msg.sender == address(this), "onlythis");
        PerpetualData storage perpetual = _getPerpetual(_iPerpetualId);
        _updateAMMTargetFundSize(perpetual);
    }

    function updateDefaultFundTargetSizeRandom(uint8 _iPoolIndex) external override {
        require(msg.sender == address(this), "onlythis");
        _updateDefaultFundTargetSizeRandom(_iPoolIndex);
    }

    function updateDefaultFundTargetSize(uint24 _iPerpetualId) external override {
        require(msg.sender == address(this), "onlythis");
        _updateDefaultFundTargetSize(_iPerpetualId);
    }

    function updateFundingAndPricesBefore(uint24 _iPerpetualId0, uint24 _iPerpetualId1)
        external
        override
    {
        require(msg.sender == address(this), "onlythis");
        ...
        _checkOracleStatus(perpetual1);
        _checkOracleStatus(perpetual2);
        _accumulateFundingInPerp(perpetual1);
        _accumulateFundingInPerp(perpetual2);
    }

    function updateFundingAndPricesAfter(uint24 _iPerpetualId0, uint24 _iPerpetualId1)
        external
        override
    {
        require(msg.sender == address(this), "onlythis");
    ...
    function setNormalState(uint24 _iPerpetualId) external override {
        require(msg.sender == address(this) || msg.sender == maintainer(), "onlythis/mntnr");
        PerpetualData storage perpetual = _getPerpetual(_iPerpetualId);
        _setNormalState(perpetual);
    }

    function setEmergencyState(uint24 _iPerpetualId) external override {
        require(msg.sender == address(this) || msg.sender == maintainer(), "onlythis/mntnr");
        PerpetualData storage perpetual = _getPerpetual(_iPerpetualId);
    ...
    /**
     * Set external treasury (DAO)
     * @param _treasury treasury address
     */
    function setTreasury(address _treasury) external override {
        require(msg.sender == governance(), "only governance");
        require(_treasury != address(this) && _treasury != address(0), "invalid addr");
        address oldTreasury = treasuryAddress;
        treasuryAddress = _treasury;
        emit TransferAddressTo("treasury", oldTreasury, _treasury);
    }

    /**
     * Set order book factory (DAO)
     * @param _orderBookFactory order book factory address
     */
    function setOrderBookFactory(address _orderBookFactory) external override {
        require(msg.sender == governance(), "only governance");
        require(_orderBookFactory != address(0), "invalid _orderBookFactory");
        address oldAddress = orderBookFactory;
        orderBookFactory = _orderBookFactory;
        emit TransferAddressTo("ordrbk fctry", oldAddress, _orderBookFactory);
    }
     ...
     */
    function setOracleFactory(address _oracleFactory) external override {
        require(msg.sender == governance(), "only governance");
        require(_oracleFactory != address(0), "invalid _oracleFactory");
        address oldAddress = oracleFactoryAddress;
        oracleFactoryAddress = _oracleFactory;
        emit TransferAddressTo("oracle fctry", oldAddress, oracleFactoryAddress);
    }
     ...
     */
    function setBlockDelay(uint8 _delay) external override onlyMaintainer {
        require(_delay < 6, "out of range");
        iTradeDelaySec = _delay;
        emit SetBlockDelay(_delay);
    }
     ...
     * @param _maxAcceptableFeedAge Maximum age of price feed in seconds
     */
    function updatePriceFeeds(
        ...
        // only callable by LOB (when executing an order) and perp itself (liquidations)
        require(
            msg.sender == address(this) ||
                _getPerpetualGetter().getOrderBookAddress(_perpetualId) == msg.sender,
            "onlythis/order-bk"
        );
        // num ids == num updates received
        (bytes32[] memory ids, ) = _getPriceInfo(_perpetualId);
        bool allOnTime = ids.length == _updateData.length;
        require(allOnTime, "wrong num of updts");
        // submit
        try
            OracleFactory(oracleFactoryAddress).updatePriceFeeds{ value: msg.value }(
                _updateData,
                ids,
                _publishTimes,
            ...
            )
        {
            ...
            if (_isStringEqual(_reason, "not needed")) {
                // reverted because no update is needed, this is okay
                if (msg.value > 0) {
                    // send fee to treasury
                    // solhint-disable-next-line avoid-low-level-calls
                    (bool sucess, ) = payable(treasuryAddress).call{ value: msg.value }("");
                    require(sucess, "error storing fee");
                }
                return;
            } else {
                // true error
    ...
    function getFunctionList() external pure virtual override returns (bytes4[] memory, bytes32) {
        ...
        functionList[5] = this.setNormalState.selector;
        functionList[6] = this.setEmergencyState.selector;
        functionList[7] = this.setTreasury.selector;
        functionList[8] = this.setOrderBookFactory.selector;
        functionList[9] = this.setOracleFactory.selector;
        functionList[10] = this.setBlockDelay.selector;
        functionList[11] = this.updatePriceFeeds.selector;
...
contract PerpetualUpdateFunctions is PerpetualBaseFunctions {
    ...
    function _updateAMMTargetFundSize(PerpetualData storage _perpetual) internal {
        ...
        _perpetual.fTargetAMMFundSize = _getUpdatedTargetAMMFundSize(
            _perpetual,
...
contract PerpetualRebalanceFunctions is PerpetualBaseFunctions {
    ...
    function _updateKStar(PerpetualData storage _perpetual) internal {
        ...
        if (ccy == AMMPerpLogic.CollateralCurrency.BASE) {
            ...
            int128 denom = (int128(_perpetual.fSigma2) << 35)
                ...
                .exp()
                .sub(ONE_64x64);
            _perpetual.fkStar = s3.div(s2).mul(nominator.div(denom)).mul(fM).sub(K2);
        }
    }
     ...
     */
    function _withdrawFromBrokerPool(int128 _fAmount, LiquidityPoolData storage _pool)
        internal
        returns (int128)
    {
        ...
        emit UpdateBrokerFundCash(_pool.id, withdraw.neg(), _pool.fBrokerFundCashCC);
        return withdraw;
    }
    ...
    function _splitAmount(
        ...
        int128 fAvailCash = fPnLCash.add(_liquidityPool.fDefaultFundCashCC);
        require(_fAmount > 0, ">0 amount expected");
        require(!_isWithdrawn || fAvailCash >= _fAmount, "pre-cond not met");
        int128 fWeightPnLparticipants = fPnLCash.div(fAvailCash);
        ...
        int128 fAmountDF = _fAmount.sub(fAmountPnLparticipants);
...
contract PerpetualBaseFunctions is PerpStorage, ILibraryEvents {
    ...
    function _transferFromUserToVault(
        ...
        marginToken.safeTransferFrom(_userAddr, vault, amountWei);
     ...
     */
    function _getPerpetualAllocatedFunds(PerpetualData storage _perpetual)
        internal
        view
        returns (int128 fFunds)
    {
...
interface ILibraryEvents {
    ...
    event UpdateBrokerFundCash(uint8 indexed poolId, int128 fDeltaAmountCC, int128 fNewFundCash);
...
contract AMMPerpLogic is Ownable, IAMMPerpLogic {
     ...
     * @param _executorAddr Address of the executor
     ...
     * @return Fee paid to the executor, in collateral currency
     */
        ...
        )
    {
        ...
        int128 fReferralRebate = _executorAddr != address(0) ? _fReferralRebate : int128(0);
