...
contract Antb is Initializable, IERC20Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    string public constant _name = "AntyBank";
    string public constant _symbol = "ANTB";
    uint8 public constant _decimals = DECIMALS;
    ...
    uint256 private constant MAX_TAX = 250; // It's not possible to set a total tax above 25%
    ...
    uint256 private constant MAX_DISTRIBUTION_GAS = 900000;
    ...
    uint256 public _initRebaseStartTime; // timestamp of start of trading
    ...
    bool public isTaxBracketEnabled;
    uint256 private constant maxBracketTax = 10; // max bracket is holding 10%
    uint256 public taxBracketMultiplier;

    bool private isLaunchTaxEnabled;
    ...
    function initialize(
        ...
        _allowedFragments[address(this)][address(router)] = type(uint).max;
        pairAddress = pair;
        ...
        sellFee = 40;
        burnFee = 20;
        totalFee = liquidityFee.add(treasuryFee).add(insuranceFee).add(burnFee).add(reflectionFee);
        inSwap = false;
        _autoRebase = false;
        ...
        isLaunchTaxEnabled = true;

        _totalSupply = INITIAL_FRAGMENTS_SUPPLY;
        _gonBalances[treasuryReceiver] = TOTAL_GONS;
        _gonsPerFragment = TOTAL_GONS.div(_totalSupply);
        _lastRebasedTime = block.timestamp;
        _isTxLimitExempt[autoLiquidityReceiver] = true;
        _isTxLimitExempt[treasuryReceiver] = true;
        _isTxLimitExempt[insuranceReceiver] = true;
        _isFeeExempt[treasuryReceiver] = true;
        _isFeeExempt[address(this)] = true;
        _isDividendExempt[pairAddress] = true;
    ...
    function rebase() internal {
        ...
        console.log("Is rebasing %s times", times);
    ...
    function transferFrom(
        ...
        if (_allowedFragments[from][msg.sender] != type(uint).max) {
    ...
    function _transferFrom(
        ...
        console.log("Should rebase: %s", shouldRebase());
        if (shouldRebase()) {
            rebase();
        }

        console.log("Should addLiquidity: %s", shouldAddLiquidity());
        ...
        console.log("Should swapBack: %s", shouldSwapBack());
        if (shouldSwapBack()) {
            swapBack();
        }

        console.log("Shoud take fee %s", shouldTakeFee(sender, recipient));
        ...
        console.log("Should distribute sender %s", !_isDividendExempt[sender]);
        console.log("Should distribute recipient %s", !_isDividendExempt[recipient]);
        // Dividend tracker
        if (!_isDividendExempt[sender]) {
            try distributor.setShare(sender, _balanceOf(sender)) {} catch {}
        }

        if (!_isDividendExempt[recipient]) {
            try distributor.setShare(recipient, _balanceOf(recipient)) {} catch {}
        }
    ...
    function checkTxLimit(address sender, uint256 gonAmount) internal view {
        require(gonAmount <= _maxTxAmount || _isTxLimitExempt[sender], "TX Limit Exceeded");
    }
    ...
    function takeFee(
        ...
        if (recipient == pair) {
            ...
            if (isLaunchTaxEnabled) {
                console.log("Launch tax enabled");
                uint256 _launchTax = getCurrentLaunchTax();
                _totalFee = _totalFee.add(_launchTax);
                _treasuryFee = _treasuryFee.add(_launchTax);
            }
        }
        ...
        if (isTaxBracketEnabled) {
            ...
            _treasuryFee = _treasuryFee.add(_dynamicFee);
        }
        console.log("Total fee: %s", _totalFee);
        console.log("Treasury fee: %s", _treasuryFee);

        uint256 feeAmount = gonAmount.div(feeDenominator).mul(_totalFee);

        _gonBalances[DEAD] = _gonBalances[DEAD].add(gonAmount.div(feeDenominator).mul(burnFee));
    ...
    function getCurrentLaunchTax()
    internal
    returns (uint256)
    {
        uint256 _launchTaxFee = 0;
        uint256 deltaFromStart = block.timestamp.sub(_initRebaseStartTime);
        if (deltaFromStart <= 1 days) {
            _launchTaxFee = 320; // 50%
        } else if (deltaFromStart > 1 days && deltaFromStart <= 2 days) {
            _launchTaxFee = 270; // 45%
        } else if (deltaFromStart > 2 days && deltaFromStart <= 3 days) {
            _launchTaxFee = 220; // 40%
        } else if (deltaFromStart > 3 days && deltaFromStart <= 4 days) {
            _launchTaxFee = 170; // 35%
        } else if (deltaFromStart > 4 days && deltaFromStart <= 5 days) {
            _launchTaxFee = 120; // 30%
        } else if (deltaFromStart > 5 days && deltaFromStart <= 6 days) {
            _launchTaxFee = 70; // 25%
        } else if (deltaFromStart > 6 days && deltaFromStart <= 7 days) {
            _launchTaxFee = 20; // 20%
            isLaunchTaxEnabled = false;
        }

        return _launchTaxFee;
    }
    ...
    function getTokensInLPCirculation() public view returns (uint256) {
        ...
        iDexFeeCalculator = IUniswapV2Pair(pair);
    ...
    function setTaxMultiplier(uint256 _taxBracketMultiplier) external onlyOwner {
        require (_taxBracketMultiplier <= 50, "Cannot set a dynamic tax above 50%");
    ...
    // TODO: Test functionality of double reward
    // TODO: Add anti sniper and bot taxes (apollo)
    ...
    function swapBack() internal swapping nonReentrant {
        ...
        uint256 amountMATICToTreasury = amountMATIC.mul(treasuryFee).div(treasuryFee.add(insuranceFee));
        uint256 amountMATICToInsurance = amountMATIC.mul(insuranceFee).div(treasuryFee.add(insuranceFee));
        uint256 amountMATICReflection = amountMATIC.mul(reflectionFee).div(totalFee);

        try distributor.deposit{value: amountMATICReflection}() {} catch {}
        (bool success, ) = payable(treasuryReceiver).call{
            value: amountMATICToTreasury,
            gas: 30000
        }('');
        (success, ) = payable(insuranceReceiver).call{
            value: amountMATICToInsurance,
            gas: 30000
        }('');
    }
    ...
    function withdrawAllToTreasury() external swapping onlyOwner {
        uint256 amountToSwap = _gonBalances[address(this)].div(_gonsPerFragment);
        require(amountToSwap > 0, "No Antb token deposited in token contract");
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();
        router.swapExactTokensForETHSupportingFeeOnTransferTokens(
    ...
    function setIsDividendExempt(address holder, bool exempt) external onlyOwner {
        ...
        if (exempt) {
            ...
            distributor.setShare(holder, _balanceOf(holder));
        }
    }
    ...
    function setLP(address _address) external onlyOwner validRecipient(_address) {
    ...
    function setDistributorSettings(uint256 gas) external onlyOwner {
        require(gas < MAX_DISTRIBUTION_GAS, "Distribution gas no more than 900000");
        distributorGas = gas;
    }
    ...
    function setFees(
        ...
        burnFee = _burnFee;
        totalFee = liquidityFee.add(treasuryFee).add(insuranceFee).add(burnFee).add(reflectionFee);
        // TODO: check if the require here invalidate the entire call or not. In that case, move it at the top
        require(totalFee <= MAX_TAX, "Cannot set tax above 25%");
    }
...
contract DividendDistributor is IDividendDistributor {
    ...
    function process(uint256 gas) external override onlyToken {
        ...
        console.log("Processing dividends... %s", shareholderCount);
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if (amount > 0) {
            ...
            require(sent, "Failed to send Matic for double reward");
