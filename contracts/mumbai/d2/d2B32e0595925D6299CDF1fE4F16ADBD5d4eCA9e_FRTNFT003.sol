...
contract FRTNFT003 is ERC1155URIStorage, Ownable, Pausable  {
    ...
    constructor() ERC1155("https://ipfs.io/ipfs/") {
        _tokenIdCounter.increment();// Chapi to start nft ids from 1 instead of 0;
    }
    ...
    function mint(address _account, string memory _tokenURI, uint256 _amount)
        external
        minterAllowed
        returns (uint256 _tokenId)
    {
        ...
        publishers[_tokenId] = _account;

        _mint(_account, _tokenId, _amount, "");
        _setTokenURI(_tokenId, _tokenURI);
        _setApprovalForAll(_account, msg.sender, true);
    ...
    function burn(
        ...
        if(balanceOf(_account, _id) == 0){
            delete publishers[_id];
        }
    }

    function publisherOf(uint256 _tokenId) external view returns (address){
        return publishers[_tokenId];
    }
