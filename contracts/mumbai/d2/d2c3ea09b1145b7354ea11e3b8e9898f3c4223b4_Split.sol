    ...
    PaymentSplitterUpgradeable
{
    ...
    /// @dev Max bps in the rarescape system
    uint128 private constant MAX_BPS = 10_000;

    /// @dev The rarescape contract with fee related information.
    IRSFee public immutable rsFee;

    /// @dev Contract level metadata.
    string public contractURI;

    constructor(address _rsFee) initializer {
        rsFee = IRSFee(_rsFee);
    }
    ...
    function _release(address payable account) internal returns (uint256) {
        ...
        // fees
        uint256 fee = 0;
        (address feeRecipient, uint256 feeBps) = rsFee.getFeeInfo(address(this), FeeType.SPLIT);
