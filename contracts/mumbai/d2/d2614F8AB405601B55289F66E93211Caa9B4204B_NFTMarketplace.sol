...
contract NFTMarketplace is ReentrancyGuard {
    ...
    mapping(address => mapping(uint256 => bool)) private userOwnedNFTs;
    ...
    modifier onlyProductSeller(uint256 id) {
        require(
            ...
            "Only the product can perform this operation"
        );
        _;
    }
    ...
    modifier onlyItemOwner(uint256 id) {
        require(
            ...
            "Only the product owner can perform this operation"
        );
        _;
    }
    ...
    function getAllListedItems() external view returns (Item[] memory) {
        uint256 itemCount = s_nftIds.current();
        uint256 unSoldItemsCount = s_nftIds.current() - s_nftSold.current();
    ...
    function getOwnerListedItems() external view returns (Item[] memory) {
        uint256 totalListedItems = s_nftIds.current();
        uint256 itemCount = 0;
        uint256 currentIndex = 0;
...
[newNftId] = Item(
            ...
            _price,
            false,
            true // Set the item forSale to true
        );
    ...
    function listOwnedNFTForSale(uint256 tokenId, uint256 price) external {
        require(
            _isUserOwnedNFT(msg.sender, tokenId),
            "You don't own this NFT"
        );
        require(!Items[tokenId].forSale, "NFT already listed for sale");
        Items[tokenId].forSale = true;
        Items[tokenId].price = price;
        emit NFTListed(
            Items[tokenId].itemId,
            Items[tokenId].nftAddress,
            tokenId,
    ...
    // Private function to check if the user owns an NFT
    function _isUserOwnedNFT(address user, uint256 tokenId)
        private
        view
        returns (bool)
    {
        return userOwnedNFTs[user][tokenId];
    }
