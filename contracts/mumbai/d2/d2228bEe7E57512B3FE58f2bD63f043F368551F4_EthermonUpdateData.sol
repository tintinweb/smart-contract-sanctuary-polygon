...
interface IEthermonWorldNFT {
    function STAT_MAX_CLASS(uint32 _classId) external returns (uint8);
}
...
interface EthermonOracelInterface {
    function getEthRatesFromEmon(
        uint256 _amount
    ) external view returns (uint256);

    function getEmonRatesFromEth(
        uint256 _amount
    ) external view returns (uint256);
}
    ...
    SafeMathEthermon
{
    ...
    mapping(uint => uint256) public burnedMons;
    ...
    */
    ...
    /**
        ...
        add new stats provided. If provided _type is -1 or < 0 then it will not update type.
    */
    //TODO: Increase steps if mons class count < (certain number) class 1 - 1
    ...
    */
    function reRollMonsterStatsEMONA(
        uint64 _tokenId,
        uint64 _sacrificeTokenId
    ) external isActive {
        EtheremonDataBase data = EtheremonDataBase(dataContract);
        ...
        IEthermonWorldNFT worldNFT = IEthermonWorldNFT(worldNFTContract);
        uint256 _statMaxClass = worldNFT.STAT_MAX_CLASS(obj.classId);

        uint256 maxStat = (_statMaxClass > 0) ? _statMaxClass : STAT_MAX;
    ...
    function adminReroll(uint64 _tokenId) external onlyModerators isActive {
        EtheremonDataBase data = EtheremonDataBase(dataContract);
        ...
        //If mon isn't burned
        require(obj.trainer != address(0), "Monster is burned");

        EthermonTradeDataInterface tradeData = EthermonTradeDataInterface(
            tradeDataContract
        );
        require(!tradeData.isOnTrading(obj.monsterId), "Monster is on trading");
        ...
        IEthermonWorldNFT worldNFT = IEthermonWorldNFT(worldNFTContract);
        uint8 _statMaxClass = worldNFT.STAT_MAX_CLASS(obj.classId);
        uint8 maxStat = (_statMaxClass > 0) ? _statMaxClass : STAT_MAX;
        // generate base stat for the previous one
        for (uint8 i = 0; i < STAT_COUNT; i += 1) {
            uint256 seed = getRandom(
                obj.trainer,
            ...
            oldValue = data.getElementInArrayType(
                EthermonEnum.ArrayType.STAT_BASE,
                uint32(obj.classId),
                i
            );
            value = (value > oldValue) ? value : oldValue;
            data.updateIndexOfArrayType(
                EthermonEnum.ArrayType.STAT_BASE,
                obj.monsterId,
                i,
    ...
    */
    ...
    function reRollMonsterStatsMatic(
        uint64 _tokenId
    ...
    */
    ...
    function setCustomClassBurnPrice(
        uint32 _classId,
        uint256 _price_in_emon
    ) public onlyModerators {
        customClassBurnPrice[_classId] = _price_in_emon;
    }

    function getCustomClassBurnPrice(
        uint32 _classId
    ) external view returns (uint256 price_in_emon) {
        price_in_emon = customClassBurnPrice[_classId];
    }

    function getMonsterTypes(
        uint32 _classId
    ) public view returns (uint8 _type1, uint8 _type2, uint8 _type3) {
        EtheremonDataBase data = EtheremonDataBase(dataContract);
