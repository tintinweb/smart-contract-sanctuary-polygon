...
import "./TradingAccountNoOracle.sol";

contract SimpleFactory is Pausable, Ownable {

    address public swapRouter;
    //user=>address[]
    mapping(address=>address[]) public ownerGrids;
    ...
    function deploy(address _usd, address _token,uint24 _fee) public whenNotPaused{
        TradingAccount tradingAccount = new TradingAccount(_usd, _token, _fee, swapRouter);
        ownerGrids[_msgSender()].push(address(tradingAccount));
        
        //transfer Ownership from factor to msg.sender
        tradingAccount.transferOwnership(_msgSender());
    }
...
contract TradingAccount is Pausable, Ownable {
    ...
    function _swap(uint256 _amount,uint256 _price, bool _tradeType) internal returns(uint256 amountOut){
        ...
        if(_tradeType){//buy
            //计算滑点
            outMinimum = _calculateMinOutAmount(_amount, _price, _tradeType);
    ...
    function _swap(uint256 _inAmount, address _tokenIn, address _tokenOut, uint256 _outMinimum)internal returns(uint256 amountOut){
    ...
    function getLatestPrice() public view returns (uint256 price) {
        ...
        uint256 result = FullMath.mulDiv(numerator1, 10**token0Decimals, 1 << 192);

        if(!_usdIsToken0()){
            price = result;
        }else{
            price = 10**(token1Decimals+6)/result;
        }
    ...
    //     result = FullMath.mulDiv(numerator1, 10**token0Decimals, 1 << 192);
   ...
   function _autoAdjustGridArray(uint256 _n) internal{
    ...
    /// @notice calculate the minimal out amount because of  SLIPPAGE
    /// @param _inAmount The ERC-20 token to swap in.
    /// @param _price the trade price,the exact of price is _price/PRICE_PRECISION
    /// @param _isBuying which is buy or sell.
    function _calculateMinOutAmount(uint256 _inAmount, uint256 _price, bool _isBuying) internal view returns(uint256 minOutAmount){
        if(_isBuying){
            minOutAmount = FullMath.mulDiv(
                    _inAmount, 
                PRICE_PRECISION *(SLIPPAGE_PRECISION - slipPageTolerance), 
                _price*SLIPPAGE_PRECISION);
      
        }else{
            minOutAmount = FullMath.mulDiv(
                _inAmount,
                _price *(SLIPPAGE_PRECISION - slipPageTolerance),
                PRICE_PRECISION*SLIPPAGE_PRECISION);
        }
           
    }

    function _usdIsToken0() public view returns(bool result){
