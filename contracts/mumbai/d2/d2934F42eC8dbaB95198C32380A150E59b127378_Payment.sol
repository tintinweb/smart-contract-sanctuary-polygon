...
contract Payment {
    ...
    event GroupCreation(address[] group, string groupName);
    ...
    event updateRaisePayment(uint256 id, address[] payerList);
    event requestUserEvent(address user, uint256 amount);
    ...
    */
    function createPay(string memory message, uint256 amount) public {
        ...
        payCounter++;
        emit UserPayment(amount, message, msg.sender, address(0), false, payCounter);
    }

    /*   
    * Getter for Create Pay
    */
    /*  function getCreatePayRequests(address addr) public view returns (PayLink[] memory) {
        uint256 currentIndex = 0;
        uint256 itemCount = 0;

        for (uint256 i = 0; i < payCounter; i++) {
            if (idToPayLink[i].creator == addr) {
                itemCount += 1;
            }
        }

        PayLink[] memory items = new PayLink[](itemCount);

        for (uint256 i = 0; i < payCounter; i++) {
            if (idToPayLink[i].creator == addr) {
                uint256 currentId = i;

                PayLink storage currentItem = idToPayLink[currentId];
    ...
    */
    ...
    function createRaisePay(string memory message, uint256 amount, uint256 dead) public {
        ...
        raiseCounter++;
        emit RaisePayment(amount, message, msg.sender, payerList, false, raiseCounter, deadline, 0);
    }
    ...
    */
    function fundPay(uint256 id) public payable {
        ...
        emit updateRaisePayment(id, idToRaiseLink[id].payerList);
    }
    ...
    */
    function requestUser(string memory user, uint256 amount) public {
        ...
        requestId++;
        emit requestUserEvent(UserToAddr[_toLower(user)], amount);
    }
    ...
    */
    function getRequests(address user) public view returns (Request[] memory) {
        ...
        for (uint256 i = 0; i < requestId; i++) {
            if (idToRequest[i].user == user) {
                itemCount += 1;
            }
        }

        Request[] memory items = new Request[](itemCount);
    ...
    */
    function requestAddress(address user, uint256 amount) public {
        ...
        requestId++;
        emit requestUserEvent(user, amount);
    }
    ...
    */
    function sendToGroupUser(string[] calldata group, uint256 amount) external {
        ...
        for (uint256 i = 0; i < group.length; i++) {
            if (UserToAddr[_toLower(group[i])] == address(0)) {
        ...
        for (uint256 i = 0; i < group.length; i++) {
            balances[msg.sender] = balances[msg.sender] - maticAmount;
            balances[UserToAddr[_toLower(group[i])]] = balances[UserToAddr[_toLower(group[i])]] + maticAmount;
        }
    }
    ...
    * send a specific amount to a group of users
    */
    function sendToGroupAddr(address[] calldata group, uint256 amount) external {
        uint256 totalAmount = amount * group.length;
        require(balances[msg.sender] >= totalAmount, "Insufficient Wallet balance");
        for (uint256 i = 0; i < group.length; i++) {
            if (group[i] == address(0)) {
                revert("Zero Address");
            }
        }
