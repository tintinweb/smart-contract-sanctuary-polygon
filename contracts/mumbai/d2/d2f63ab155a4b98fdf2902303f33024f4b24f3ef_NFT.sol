...
// ERC2981
import "../ERC2981/ERC2981PayoutUpgradeable.sol";
// Key Storage
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function mint(
        ...
        if (royaltyInfo_.royaltyFraction > 0) {
            _setTokenRoyalty(
                newItemId,
                royaltyInfo_.receiver == address(0)
                    ? msg.sender
                    : royaltyInfo_.receiver,
                royaltyInfo_.royaltyFraction
            );
        }

        if (payoutInfo_.length > 0) {
            _setTokenPayout(newItemId, payoutInfo_.length, payoutInfo_);
        }
    ...
    {
        return
            interfaceId_ == type(ERC2981PayoutUpgradeable).interfaceId ||
            interfaceId_ ==
...
// Interfaces
import "../interfaces/IERC2981PayoutUpgradeable.sol";
 ...
 *
 */
abstract contract ERC2981PayoutUpgradeable is
    Initializable,
    IERC2981PayoutUpgradeable,
    ERC2981Upgradeable
{
    mapping(uint256 => PayoutInfo[]) private _tokenPayoutInfo;
    mapping(uint256 => uint256) private _tokenPayoutCount;
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(ERC2981Upgradeable, IERC165Upgradeable)
        returns (bool)
    {
        return
            interfaceId == type(IERC2981PayoutUpgradeable).interfaceId ||
            super.supportsInterface(interfaceId);
    }

    /**
     * @inheritdoc IERC2981PayoutUpgradeable
     */
    function payoutCount(uint256 _tokenId)
        public
        view
        virtual
        override
        returns (uint256)
    {
        return _tokenPayoutCount[_tokenId];
    }

    /**
     * @inheritdoc IERC2981PayoutUpgradeable
     */
    function payoutInfo(uint256 _tokenId, uint256 _salePrice)
        public
        view
        virtual
        override
        returns (address[] memory receivers, uint256[] memory payoutAmounts)
    {
        uint256 count = payoutCount(_tokenId);
        require(count == 0, "ERC2981Payout: no payout");

        PayoutInfo[] memory payouts = _tokenPayoutInfo[_tokenId];
        payoutAmounts = new uint256[](count);

        for (uint256 i; i < count; ++i) {
            receivers[i] = payouts[i].receiver;
            payoutAmounts[i] =
                (_salePrice * payouts[i].payoutFraction) /
                _feeDenominator();
        }
    }
    ...
    function _setTokenPayout(
        ...
        _tokenPayoutCount[tokenId] = payoutCount_;

        for (uint256 i; i < payoutCount_; ++i) {
            require(
                payoutInfo_[i].receiver != address(0),
                "ERC2981Payout: invalid receiver"
            );
        ...
        for (uint256 i; i < payoutCount_; ++i) {
            _tokenPayoutInfo[tokenId].push(payoutInfo_[i]);
        }
    }
     ...
     */
    function _resetTokenPayout(uint256 tokenId) internal virtual {
        delete _tokenPayoutCount[tokenId];
        delete _tokenPayoutInfo[tokenId];
    }
