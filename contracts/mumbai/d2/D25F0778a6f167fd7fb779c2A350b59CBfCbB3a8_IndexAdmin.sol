    ...
    ExtensionPause
{
        ...
        onlyRole(ADMIN_ROLE)
    {
        ...
        uint256 oldPrice = _calcCost(1e18);
        ...
        _rebalance(newAssets, usdAmount); // обновляет структуры Asset
    ...
    /**
     * @dev See {IIndex-getLastRebalance}.
     */
    ...
    /**
     * @dev See {IIndex-getActiveAssets}.
     */
    ...
    /**
     * @dev See {IIndex-lengthNewAssets}.
     */
    ...
    function _calcStake(uint256 amountLP) internal {
        uint256 len = _activeAssets.length;
        for (uint256 i; i < len; i++) {
            uint256 amountForSwap = (amountLP * _activeAssets[i].fixedAmount) /
                PRECISION;
        ...
        virtual
    {
        ...
        for (uint256 i; i < len; i++) {
            ...
            if (totalLP == 0) {
                ...
                _activeAssets[i].fixedAmount = amounts[amounts.length - 1];
            } else {
                uint256 amount = _swapDEX(amountForSwap, newAssets[i].path);
                _activeAssets[i].totalAmount += amount;
                _activeAssets[i].fixedAmount = (amount * PRECISION) / totalLP;
            }
        }
...
interface IIndex {
    ...
    /**
     * @dev  set a new staking name
     */
    function setNameIndex(string memory name) external;

    /**
     * @dev  set a new commission
     */
    ...
    /**
     * @dev set the new address of the token for which the index is purchased
     * the changes will take effect after rebalancing
     */
    function setActualToken(address newToken) external;

    /**
     * @dev initial index setup
     */
    function init(AssetData[] memory newAssets) external;

    /**
     * @dev  reconfiguring the index
