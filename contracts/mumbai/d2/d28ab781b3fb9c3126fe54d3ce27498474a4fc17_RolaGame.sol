...
import "./RolaGameInternal.sol";
import "./IRolaCoaster.sol";

contract RolaGame is RolaGameInternal, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable, ERC1155HolderUpgradeable {
    ...
    function betBear(uint256 roundId, uint256 amount) external override whenNotPaused nonReentrant notContract {
        IRolaCoaster rolaCoaster = IRolaCoaster(rolaAddress);
        
        require(amount <= rolaCoaster.balanceRola(_msgSender()), "Insufficient ROLA tokens");
        ...
        rolaCoaster.safeTransferRola(_msgSender(), address(this), amount, '');

        // Update round data
        Round storage round = roundInternal[roundId];
    ...
    function claim(uint256[] calldata epochs) external override nonReentrant notContract {
        ...
        for (uint256 i = 0; i < epochs.length; i++) {
            require(roundInternal[epochs[i]].startTimestamp != 0, "Round has not started");
            uint256 closeTimestamp = roundInternal[epochs[i]].startTimestamp + (2 * roundInternal[epochs[i]].roundExecutionTime);
            require(block.timestamp > closeTimestamp, "Round has not ended");

            uint256 addedReward = 0;

            // Round valid, claim rewards
            if (roundInternal[epochs[i]].DexCalled) {
                require(claimable(epochs[i], msg.sender), "Not eligible for claim");
                Round memory round = roundInternal[epochs[i]];
        ...
        if (reward > 0) {
            IRolaCoaster rolaCoaster = IRolaCoaster(rolaAddress);
            rolaCoaster.safeTransferRola(address(this), _msgSender(), reward, '');
        }
    }
    ...
    function executeRound(address dexAddress, uint256 roundId, uint256 startRoundTime, uint256 roundExecutionTime, uint256 preroundId, uint256 endroundId) external override whenNotPaused onlyOperator {
        ...
        Round memory round = roundInternal[preroundId];
        (int currentPrice) = _getPriceFromDex(round.dexAddress);
        _safeLockRound(preroundId, currentPrice);

        Round memory roundEnd = roundInternal[endroundId];
    ...
    function endRound(uint256 roundId) external override whenNotPaused onlyOperator {
        ...
        Round memory roundEnd = roundInternal[roundId];
        (int currentPriceEndRound) = _getPriceFromDex(roundEnd.dexAddress);
    ...
    function lockRound(uint256 roundId) external override whenNotPaused onlyOperator {
       ...
       Round memory round = roundInternal[roundId];
        (int currentPrice) = _getPriceFromDex(round.dexAddress);
    ...
    function claimTreasury() external override nonReentrant onlyAdmin {
        ...
        treasuryAmount = 0;
        IRolaCoaster rolaCoaster = IRolaCoaster(rolaAddress);
        rolaCoaster.safeTransferRola(address(this), adminAddress, currentTreasuryAmount, '');
    ...
    function claimable(uint256 epoch, address user) internal view returns (bool) {
        ...
        Round memory round = roundInternal[epoch];
     ...
     * @dev Callable by admin or operator
     */
    function pause() external override whenNotPaused onlyAdminOrOperator {
        _pause();
    }
...
import "./IRolaGame.sol";

abstract contract RolaGameInternal is IRolaGame {

    mapping(uint256 => Round) public roundInternal;
    ...
    */
    ...
    function _calculateRewards(uint256 roundId) internal {
        require(
            roundInternal[roundId].rewardBaseCalAmount == 0 &&
                roundInternal[roundId].rewardAmount == 0,
            "Rewards calculated"
        );
        Round storage round = roundInternal[roundId];
        uint256 rewardBaseCalAmount;
    ...
    */
    ...
    function _safeEndRound(uint256 roundId, int256 price) internal {
        uint256 closeTimestamp = roundInternal[roundId].startTimestamp + (2 * roundInternal[roundId].roundExecutionTime);
        uint256 lockTimestamp = roundInternal[roundId].startTimestamp + roundInternal[roundId].roundExecutionTime;
        require(
            lockTimestamp != 0,
    ...
    */
    ...
    function _safeLockRound(uint256 roundId, int256 price) internal {
        uint256 lockTimestamp = roundInternal[roundId].startTimestamp + roundInternal[roundId].roundExecutionTime;
        require(
            roundInternal[roundId].startTimestamp != 0,
    ...
    */
    ...
    function _safeStartRound(address dexAddress, uint256 roundId, uint256 startRoundTime, uint256 roundExecutionTime, uint256 endroundId) internal {
        uint256 closeTimestamp = roundInternal[endroundId].startTimestamp + (2 * roundInternal[endroundId].roundExecutionTime);
        require(
    ...
    */
    ...
    function _bettable(uint256 roundId) internal view returns (bool) {
        ...
        return
            roundInternal[roundId].startTimestamp != 0 &&
            lockTimestamp != 0 &&
            block.timestamp > roundInternal[roundId].startTimestamp &&
            block.timestamp < lockTimestamp;
    }
