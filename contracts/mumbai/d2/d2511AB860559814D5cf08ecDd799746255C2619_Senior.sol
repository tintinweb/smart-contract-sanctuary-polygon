...
contract Senior is Ownable, Initializable {
    ...
    function initialize(address tokenAdd, uint allocation, uint subsequentClaims, uint timestamp) external initializer {
        require(timestamp > 0, "Senior : timestamp > 0");
        require(subsequentClaims > 0,"Senior : subsequentClaims > 0");
    ...
    function addDistributionWallet( address[] memory account, uint[] memory amount, uint[] memory startTime) external onlyOwner {
        require(account.length < 30,"Senior : length < 30");
        require((account.length == amount.length) && (startTime.length == amount.length),"Senior : length mismatch");
        uint currentTime = block.timestamp;

        for(uint i=0; i< account.length; i++) {
            require((total[0] + amount[i]) <= token.balanceOf(address(this)), "Senior : insufficient balance to allocate");
    ...
    function revokeDistributionWallet(address[] memory account) external onlyOwner {
        ...
        for(uint i=0; i< account.length; i++) {  
            ...
            require(user_.totalClaimed < user_.balance, "Senior : user claimed all funds or user may not be added");
            uint amountToRevoke = user_.balance - user_.totalClaimed;
            delete user[account[i]];

            if(amountToRevoke > 0) {
                require(token.balanceOf(address(this)) >= amountToRevoke, "Senior : insufficient balance to revoke");
                require(token.transfer(owner(),amountToRevoke),"Senior : revoke transfer failed");
            }
        }
    }
    ...
    function claim() external {
        ...
        require(user_.balance > 0, "Senior : user not exist");
        require(user_.totalClaimed < user_.balance, "Senior : total claim < total balance");
        require((user_.lastClaim > 0) && ((user_.lastClaim + claimPeriod) <= block.timestamp), "Senior : lastClaim < block.timestamp");
    ...
    function failcase( address tokenAdd, uint amount) external onlyOwner{
        ...
        if(tokenAdd == address(0)) {
            require(self.balance >= amount, "Senior : insufficient balance");
            require(payable(owner()).send(amount), "Senior : transfer failed");
        } else {
            require(IBEP20(tokenAdd).balanceOf(self) >= amount, "Senior : insufficient balance");
            ...
            require(IBEP20(tokenAdd).transfer(owner(),amount), "Senior : transfer failed");
        }
    }
}