...
contract NFT is ERC721 {
	...
	struct ChangePixelData {
		uint8 row;
		uint8 col;
		uint8 newColor;
	}

	function changePixel(uint256 tokenId, ChangePixelData memory cpd) public {
		_requireCallerOwner(tokenId);
		_requireHasChangeablePixels(msg.sender, 1);

		_changePixel(tokenId, cpd);
	}

	function changePixels(uint256 tokenId, ChangePixelData[] memory cpds) external {
		_requireCallerOwner(tokenId);
		_requireHasChangeablePixels(msg.sender, cpds.length);

		for (uint256 i = 0; i < cpds.length; i++) {
			_changePixel(tokenId, cpds[i]);
		}
	}
	...
	function _changePixel(uint256 tokenId, ChangePixelData memory cpd) internal {
		_pixelsOf[tokenId][cpd.row][cpd.col] = cpd.newColor;
	}
