 ...
 */
contract RoverMCStakeableAdapter is IMissionControlStakeable, OwnableUpgradeable, ERC721HolderUpgradeable {
    ...
    function initialize(
        uint256 _resourceId,
        bool _isBase,
        address _s_randNumOracle,
        address _m3tam0d,
        uint256 _m3tam0dTokenId
    ) external initializer {
        resourceId = _resourceId;
        isBaseVal = _isBase;
        s_randNumOracle = IVRFManager(_s_randNumOracle);
    ...
    */
    ...
    function onCollect(address _userAddress, uint256 _nonce) external override onlyMissionControl returns (uint256, uint256) {
        ...
        s_requests[requestId] = Request({user: msg.sender, m3tam0dChance: m3tam0dChance});
        return (amount, id);
    }
    ...
    modifier onlyOracle() {
        if (msg.sender != address(s_randNumOracle)) revert Rover__NotOracle();
        _;
    }
}