...
contract Platform is Roles {
    ...
    // dailyRecord[_superNode][block.timestamp/(1 days)] += wd;
    ...
    function setSuperNode(address superNode, address selfNode) external onlyManager {
        ...
        require(channelOf[superNode] == address(0) && channelOf[selfNode] != selfNode, "Error2");
    ...
    function play(
        ...
        if (agent != address(0)) {
            ...
            if (_node != address(0)) {
                ...
                address _superNode = superOf[_node];
                // if _node != address(0) => _superNode != address(0)
                dailyRecord[_superNode][_date] += wd;
