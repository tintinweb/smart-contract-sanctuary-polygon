...
contract Dao {
  ...
  event proposalCount(
    ...
    function createProposal(string memory description, address[] memory _canVote) public {
        uint256 balance = _getElegibility(msg.sender);
        require(balance > 0);
        _createProp(description, _canVote, msg.sender);
    }

    function _createProp(string memory _description, address[] memory _canVote, address _proposer) internal {
        uint256 _maxVotes;
        for (uint i = 0; i < validTokens.length; i++){
            for (uint j = 0; j < _canVote.length; j++){
                _maxVotes += daoContract.balanceOf(_canVote[j], validTokens[i]);
        ...
        emit proposalCreated(nextProposal, _description, _maxVotes, _proposer);
        nextProposal++;
    }
