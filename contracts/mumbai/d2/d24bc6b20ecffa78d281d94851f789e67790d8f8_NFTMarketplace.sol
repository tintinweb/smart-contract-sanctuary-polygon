...
contract NFTMarketplace is ERC721URIStorage , ReentrancyGuard  {
    ...
    event owners (address owners ,address sender , uint tokenId );
    ...
    function resellToken(uint256 tokenId, uint256 price) public payable nonReentrant {
        emit owners (idToMarketItem[tokenId].owner , msg.sender , tokenId);
    ...
    function canselMrketItem(uint tokenId)public {
    ...
    function buyMarketItem(uint256 tokenId) public payable nonReentrant{
        ...
        if(buyCounter[tokenId]<=2){buyCounter[tokenId]+=1;}
        setStatus(buyCounter[tokenId] , tokenId);

        
        if(status[tokenId] == Royality.Active && royality>=10){
