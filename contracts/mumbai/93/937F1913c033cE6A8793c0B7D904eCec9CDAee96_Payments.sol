...
contract Payments is ReentrancyGuard, AccessControl, IMortgageInterest {
   address private relayAddress = 0x59C1E897f0A87a05B2B6f960dE5090485f86dd3D;
    ...
    function CalcDailyInterest(uint256 _IdMortgage) private {
    ...
    function dailyInterestPayment(uint256 _IdMortgage,uint256 _amountToPay,address _token) public nonReentrant {
    ...
    function CalcMortgageInterestPayment(address[] calldata _userWallet,uint256[] calldata _idMortgage) public onlyDev {
    ...
    ///@dev Funcion para realizar los pagos mensuales.
    function monthlyPayment(address _token, uint256 _IdMortgage,uint256 _amountToPay) public nonReentrant {
        if (_token != address(Busd) && _token != address(Usdc)) {
            revert("Token Invalid");
        }
        MortgageInterest memory mortgage = mortgageControl.getuserToMortgageInterest(msg.sender, _IdMortgage);
        ...
        if (mortgage.isMonthlyPaymentPayed) {
            revert("You already paid this month");
        }
        if (mortgage.liquidate) {
            revert("Liquidated Mortgage");
        }
    ...
    function advancePayment(address _token,uint256 _IdMortgage,uint256 _amountToPay) public nonReentrant {
        if (_token != address(Busd) && _token != address(Usdc)) {
            revert("Token Invalid");
        }
        MortgageInterest memory mortgage = mortgageControl.getuserToMortgageInterest(msg.sender, _IdMortgage);
   ...
   ///@dev Funcion para actualizar los porcentajes a pagar a Panoram y al Pool en pago puntual
    function updateNormalPercentages(uint16 _panoram, uint16 _pool) public onlyDev{
  ...
  ///@dev Funcion para actualizar los porcentajes a pagar a Panoram y al Pool en pago moratorio
    function updateDelayedPercentages(uint16 _panoram, uint16 _pool) public onlyDev{
    ...
    function updatePenalization(uint8 _penalization) public onlyDev {
