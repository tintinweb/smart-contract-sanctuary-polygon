    ...
    */
    ...
    returns(uint256){
        ...
        if (vestingSchedule.start == 0 || (currentTime < vestingSchedule.cliff) || vestingSchedule.revoked == true) {
            ...
            return 0;
        } else if (currentTime >= vestingSchedule.start + vestingSchedule.duration && firstReleaseAmount > releasedAmount) {
            // time is greater than vesting durations (vesting ended)
            return firstReleaseAmount - releasedAmount;
        } else if (currentTime >= vestingSchedule.start && firstReleaseAmount > releasedAmount) {
            // first percent release with slices
            uint256 timeFromStart = currentTime - vestingSchedule.start;
            uint secondsPerSlice = vestingSchedule.slicePeriodSeconds;
            uint256 vestedSlicePeriods = timeFromStart / secondsPerSlice;
            uint256 vestedSeconds = vestedSlicePeriods * secondsPerSlice;
            uint256 vestedAmount = firstReleaseAmount * vestedSeconds / vestingSchedule.duration;
            ...
            // second percent release (second release start)
