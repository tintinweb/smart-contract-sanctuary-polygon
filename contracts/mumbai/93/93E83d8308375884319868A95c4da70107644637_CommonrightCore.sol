...
contract CommonrightCore is Ownable, ERC721, EIP712FileSignature {
     ...
     * @param baseURI_ Root metadata URL
     * @param _contractURI Path to JSON within baseURI_ that contains the contract metadata for marketplaces
     * @param _metadataServiceAccount Service account responsible for uploading off chain metadata
     */
    constructor(
        string memory _name,
    ...
    ) Ownable() ERC721(_name, _symbol) EIP712FileSignature() {
    ...
    /**
     * @dev Adds an admin
     * @param _owner Address of wallet to update
     * @param _admin Set to true, if setting admin, false to revoke
     */
    function setAdmin(address _owner, bool _admin) public onlyOwner {
        admins[_owner] = _admin;
    }

    /**
     * @dev Check if the provided wallet is an admin
     * @param _owner Address of wallet to update
     */
    function isAdmin(address _owner) public view returns (bool) {
        return admins[_owner];
    }
    ...
    /**
     * @dev Sets the next token for the specified group
     * @param _groupId Unique group identifier
     */
    function setTokensForGroup(uint256 _groupId) private {
        tokensForGroup[_groupId].push(totalTokens());
    }

    /**
     * @dev Sets the next derivative token for the specified group
     * @param _groupId Unique group identifier
     */
     ...
     * @param _signature Off chain web3 wallet signature approving the attached files checksums
     */
    function mintWithNewGroup(uint256 _shares, string memory _signingName, string memory _signingMetadataHash, string memory _signingDataId, bytes calldata _signature) public onlyAdmin verifyFileSignature(_signature, msg.sender, _signingName, _signingMetadataHash, _signingDataId) {
    ...
    /**
     * @dev Mints a new token into an existing group. Can only be called by the group manager
     * @param _groupId Unique group identifier
     * @param _signingName The token or asset name
     * @param _signingMetadataHash The Sha3 hash of the metadata object
     * @param _signingDataId The reference id that is used for metadata reference and physical asset verification
     * @param _signature Off chain web3 wallet signature approving the attached files checksums
     */
    function mintWithGroupId(uint256 _groupId, string memory _signingName, string memory _signingMetadataHash, string memory _signingDataId, bytes calldata _signature) public onlyGroupManager(_groupId) verifyFileSignature(_signature, msg.sender, _signingName, _signingMetadataHash, _signingDataId) {
        require(saleIsActive, "Sale not active");

        setTokensForGroup(_groupId);
        setFilesSignatureForToken(_signature);
    ...
    /**
     * @dev Mints a new derivative token into an existing group. Can only be called by the group manager
     * @param _groupId Unique group identifier
     * @param _signingName The token or asset name
     * @param _signingMetadataHash The Sha3 hash of the metadata object
     * @param _signingDataId The reference id that is used for metadata reference and physical asset verification
     * @param _signature Off chain web3 wallet signature approving the attached files checksums
     */
    function mintDerivativeWithGroupId(uint256 _groupId, string memory _signingName, string memory _signingMetadataHash, string memory _signingDataId, bytes calldata _signature) public onlyGroupManager(_groupId) verifyFileSignature(_signature, msg.sender, _signingName, _signingMetadataHash, _signingDataId) {
        require(saleIsActive, "Sale not active");
