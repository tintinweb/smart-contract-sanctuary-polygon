...
contract LuckyMe is Ownable {
  ...
  ********************************************************/
  ...
  constructor(address _DAI, address _RandomNumberGenerator) {
    ...
    /**
      Registering user
    */
    uint256 Id = UsersIds.current();
    User[Id].Id = Id;
    User[Id].Ref = Id;
    User[Id].Plan = 1;
    User[Id].Address = owner();
    UserId[owner()] = Id;
    isUserExists[owner()] = true;
    User[Id].registerAt = block.timestamp;
  ...
  ********************************************************/
  ...
  function register(
    ...
    User[Id].Ref = _ref;
    User[Id].Plan = _plan;
    User[Id].Address = _user;
    User[Id].TimeToRenew = block.timestamp + Year;
    User[Id].registerAt = block.timestamp;
    UserId[_user] = Id;
    isUserExists[_user] = true;

    User[_ref].AllRef.push(Id);

    if (_plan == 0) Members++;
    if (_plan == 1) Partners++;

    registerUplineMemberRef(Id, _plan, _ref, _amount);

    emit _registered(Id, _ref, _plan, _user, block.timestamp);
  }
  ...
  function upgradePlan(uint256 _id) public noReentrant {
    ...
    require(isUserExists[_user.Address], "User not exists");
    require(_user.Plan == 0, "User plan already upgraded");
  ...
  function withdrawPrizes(uint8 _game, uint256 _gameId) public noReentrant {
    ...
    for (uint8 i = 0; i < 3; i++) {
      ...
      uplinePrizesRef(_User.Id, _User.Ref, _game, i);
