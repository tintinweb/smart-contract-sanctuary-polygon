...
contract RaffleMarketplace is  VRFV2SubscriptionManager,RaffleRegisterUpkeep {
    ...
    event RaffleCreated(
        ...
        RaffleLibrary.Raffle indexed raffle,
    ...
    event RaffleStageAdded(uint256 indexed raffleTicker, RaffleLibrary.RaffleStage indexed stage);
    ...
    function createRaffle(
        ...
        RaffleLibrary.PrizeCollectionCountry _collectionCountry
    ) external {
    ...
    function _addPrizeInStorage(RaffleLibrary.RafflePrize[] memory _prizes) internal {
        for (uint256 i = 0; i < _prizes.length; i++) {
            _raffleToRafflePrizes[raffleTicker].push(
                RaffleLibrary.RafflePrize(i, _prizes[i].prizeTitle, _prizes[i].country, _prizes[i].prizeAmount)
            );
        }
    }
    ...
    */
    ...
    function performUpkeep(
        ...
        uint256 requestId = i_vrfCoordinator.requestRandomWords(
            ...
            uint32(prizes.length) // noOfWinnersToPick - how many random numbers  to pick
        );
    ...
    function distributePrizes() onlyMarketplaceOwner() public {
    ...
    function _addStageInStorage(RaffleLibrary.RaffleStage[] memory _stages) internal {
        for (uint256 i = 0; i < _stages.length; i++) {
            ...
            raffleStagesArray.push(RaffleLibrary.RaffleStage(
                    _stages[i].stageType,
                    _stages[i].ticketsAvailable,
                    _stages[i].ticketPrice,
                    0
    ...
    function getCurrentStage() public view returns (RaffleLibrary.RaffleStage memory) {
        return raffleStages[currentStage];
    }

    function getStages() public view returns(RaffleLibrary.RaffleStage[] memory) {
        return raffleStagesArray;
    }

    function getStageInformation(uint256 stageType) public view returns(RaffleLibrary.RaffleStage memory){
...
library RaffleLibrary {
    ...
    function _shuffle(Players[] memory players) internal returns(address[] memory) {
...
interface IRaffleMarketplace{
   ...
   function updateTicketsSold(uint256 id, RaffleLibrary.StageType stageType,uint256 ticketsSold) external ;
