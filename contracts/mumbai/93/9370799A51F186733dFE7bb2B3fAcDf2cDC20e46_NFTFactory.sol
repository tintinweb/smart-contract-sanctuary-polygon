...
contract NFTFactory {
    ...
    function createContract(
        ...
        if (standart == 721) {
            ...
            NFTProxy newNFT = new NFTProxy{salt: byteSalt}(implementation721, "");

            address payable newNFTaddress = payable(address(newNFT));

            ERC721(newNFTaddress).initialize(
                name,
                symbol,
                baseURI,
                newOwner,
        ...
        if (standart == 1155) {
            bytes32 byteSalt = bytes32(_salt);
            NFTProxy newNFT = new NFTProxy{salt: byteSalt}(implementation1155, "");
     ...
     * @param _salt Some random number which effects to the ERC1155 address and used only for deploying
     */ 
    function predictAddress1155(uint256 _salt) public view returns (address newNFTAddress) {
        return
    ...
    /**
     * @notice Returns address of potentialy deployed ERC721 contract by 'createERC1155' function on TRON
     * Feel free to fill arguments and change only '_salt' for changing whole contract address
     * @param _salt Some random number which effects to the ERC1155 address and used only for deploying
     */
    function predictAddress721(uint256 _salt) public view returns (address newNFTAddress) {
        return
        address(uint160(uint(keccak256(abi.encodePacked(
                bytes1(0xff),
                address(this),
     ...
     */
    function setImplementation1155(address newImplementation) external {
        isOwner();
        implementation1155 = newImplementation;
    }

    /**
     * @notice Sets address of ERC721 implementation, avaliable only for Owner
     * @param newImplementation address of new implementation
     */
    function setImplementation721(address newImplementation) external {
        isOwner();
        implementation721 = newImplementation;
    }

    /**
     * @notice Sets Marketplace address for ERC721 trading, avaliable only for Owner
     * @param newMarketplace address of new Marketplace
     */
    function setMarketplace721(address newMarketplace) external {
        isOwner();
        marketplace721 = newMarketplace;
    }

    /**
     * @notice Sets Marketplace address for ERC1155 trading, avaliable only for Owner
     * @param newMarketplace address of new Marketplace
     */
    function setMarketplace1155(address newMarketplace) external {
        isOwner();
...
contract ERC1155 is Initializable, ERC1155Upgradeable, AccessControlUpgradeable, UUPSUpgradeable{
    ...
    function safeTransferFrom(
        ...
        require(
            from == _msgSender() || isApprovedForAll(from, _msgSender()) || isRecieverMarketplace(from, to),
            "ERC1155: caller is not token owner nor approved"
        );
        _safeTransferFrom(from, to, id, amount, data);
    }

    function isRecieverMarketplace(address from, address to) internal view returns(bool) {
