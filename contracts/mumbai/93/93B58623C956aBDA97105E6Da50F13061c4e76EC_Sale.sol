...
contract Sale is ReentrancyGuardUpgradeable, ERC721HolderUpgradeable, ERC1155HolderUpgradeable, OwnableUpgradeable {
    ...
    function _checkWinner(uint256 saleId, bytes32[] memory _merkleProof) private {
        bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
        require(MerkleProofUpgradeable.verify(_merkleProof, merkleRoots[saleId], leaf), "Invalid winner");
    }
    ...
    function setContract(address _project) external {
    ...
    function setMerkleRoot(uint256 _saleId, bytes32 _rootHash) external {
        require(controllers[_msgSender()], "Invalid controller");
    ...
    function bidSingle(uint256 _saleId, bytes32[] memory _merkleProof) external payable {
        _checkWinner(_saleId, _merkleProof);
    ...
    function bidMulti(uint256 _saleId, bytes32[] memory _markleProof, uint256 _amount) external payable {
        _checkWinner(_saleId, _markleProof);
        _checkOnSale(_saleId);
        SaleInfo storage sale = sales[_saleId];
        ProjectInfo memory _project = project.getProject(sale.projectId);
    ...
    function _close(ProjectInfo memory _project, SaleInfo memory _sale, address[] memory _accountBuyers, bool _isGive) private {
        for (uint256 i; i < _accountBuyers.length; i++) {
            if (bills[_sale.id][_accountBuyers[i]].account == _accountBuyers[i]) {
                ...
                address receiver;
                if (_project.minSales == 0 && !_project.isInstantPayment) receiver = billInfo.account;
                else if (_project.minSales > 0 && _project.sold < _project.minSales && !_isGive) receiver = _project.manager;
