...
contract DexilonTest_v10 {
    ...
    **/
    function trade(
        ...
        updateUserBalances(maker, asset, (isBuy ? int(1) : int(-1))*int(amountWei), int(rate), tradeId);
        updateUserBalances(taker, asset, (isBuy ? int(-1) : int(1))*int(amountWei), int(rate), tradeId);
        
        emit Trade(isBuy, maker, taker, asset, amountWei, rate, makerLeverage, takerLeverage, tradeId, block.timestamp);
    }
    ...
    **/
    function updateUserBalances(
        ...
        require(rate > 0, string(abi.encodePacked('Zero Rate! TradeId:', tradeId)));
        ...
        require(newLockedBalance >= 0, string(abi.encodePacked('newLockedBalance < 0! TradeId:', tradeId)));
    ...
    function batchTradeIndexed(tradingArrayIndexed[] memory batchTradingArray) public onlyOwner {
        ...
        for (uint256 i=0; i < max; i++) {
            trade(
                ...
                batchTradingArray[i].amountWei,
                batchTradingArray[i].rate,
