...
contract Followers {
    ...
    constructor(address _kutils, uint256 _maxItemsReturn, uint256 _maxItemsPerBucket) {
    ...
    */
    ...
    /**
    * Returns a list of users are following a given user
    * @param usrAddress : the address to retrieve the details for
    * @param startFrom : the number to start getting records from
    */
    function getFollowers(address usrAddress, uint256 startFrom) public view returns(address[] memory) {
    ...
    /**
    * Returns a list of users a given user is following
    * @param usrAddress : the address to retrieve the details for
    * @param startFrom : the number to start getting records from
    */
    function getFollowing(address usrAddress, uint256 startFrom) public view returns(address[] memory) {

        uint256 totalFollowing = followingMap[usrAddress].length;

        // If they pass a 0, then return newest set
        if (startFrom == 0){
            startFrom = totalFollowing;
            if (startFrom != 0){
                startFrom -= 1;
            } else {
                // It's empty, so end
        ...
        delete returnCount;
        delete totalFollowing;

        return followings;
    }


    function addFollower(address addressRequester, address addressTarget) public onlyAdmins {
        ...
        delete addressTargetStr;
        delete followingBucketKey;
        delete bucketKeyID;
    }
    ...
    function isUserFollowing(address addressRequester, address addressTarget) public view returns (bool){
