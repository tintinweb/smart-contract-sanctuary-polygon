...
contract GroupTokens is ERC721, ERC721Enumerable, Pausable, Ownable, ReentrancyGuard {

    // Admins
    mapping (address => bool) admins;

    uint256 lastMintBlock = 0;

    // Cost to mint a new group
//    uint256 public costToMintTier1 = 1000 ether;
//    uint256 public costToMintTier2 = 100 ether;
//    uint256 public costToMintTier3 = 10 ether;
//    uint256 public costToMintTier4 = 5 ether;

    uint256 public costToMintTier1 = 100;
    uint256 public costToMintTier2 = 10;
    uint256 public costToMintTier3 = 5;
    uint256 public costToMintTier4 = 1;
    ...
    */
    ...
    function updateContracts(address _kutils, address _groupMemberships) public onlyAdmins {
    ...
    */
    ...
    function isGroupAvailable(string calldata groupName) public view returns (bool){
        return Groups.isGroupAvailable(groupName);
    }

    function mintGroup(string calldata groupName) public payable nonReentrant {

        // Get the character count for reuse
        uint256 groupNameLen = bytes(groupName).length;

        // Make sure we have a minimum of 3 characters for a group
        require(groupNameLen > 2, "Minimum of 3 characters for a group");
        ...
        if (groupNameLen == 3){
            require(msg.value == costToMintTier1, "Bad payment amount for Tier 1");
        } else if (groupNameLen <= 5){
            require(msg.value == costToMintTier2, "Bad payment amount for TIer 2");
        } else if (groupNameLen <= 8){
            require(msg.value == costToMintTier3, "Bad payment amount for Tier 3");
        } else {
            require(msg.value == costToMintTier4, "Bad payment amount for Tier 4");
        }

        Address.sendValue(payable(vaultAddress), address(this).balance);

        // Check format of the group name
        // TODO: Need to let Emoji's be possible
        ...
        string memory _baseURI = append(baseURIPrefix, KUtils.addressToString(msg.sender),'/',KUtils.toString(groupID),'.json');

        // Set them to the owner & first member
        Groups.setInitialDetails(groupID, msg.sender, groupName, _baseURI);
    ...
    function tokenURI(uint256 _tokenID) public view virtual override(ERC721) returns (string memory) {
        return Groups.groupDetails(_tokenID).baseURI;
    }
...
interface IGroups {
    ...
    function setInitialDetails(uint256 _groupID, address _owner, string memory groupName, string memory _baseURI) external;
