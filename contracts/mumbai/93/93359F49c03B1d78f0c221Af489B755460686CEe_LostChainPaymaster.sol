...
contract LostChainPaymaster is BasePaymaster, UUPSUpgradeable {
    using ERC165CheckerUpgradeable for address;

    // Lost Chain RBA
    ILostChainRBA LCRBA;
    modifier onlyRole(bytes32 role) {
        require(LCRBA.checkAccountRole(role, msg.sender));
        _;
    }

    function setRBA(address rbaAddress) private {
        require(rbaAddress != address(0x0), "LCGDM: Initial RBA is zero address");
        LCRBA = ILostChainRBA(rbaAddress);

    }
    // End of Lost Chain RBA
    ...
    function initialize(address rbaAddress, address relayAddress, address trustedForwarder) initializer public {
        __Ownable_init();
    ...
    function changeTrustedForwarder(address trustedForwarder) public onlyRole(LCRBA.getRole("DEFAULT_ADMIN_ROLE")) {
 ...
 *  - postRelayedCall
 */
abstract contract BasePaymaster is IPaymaster, OwnableUpgradeable, ERC165 {
