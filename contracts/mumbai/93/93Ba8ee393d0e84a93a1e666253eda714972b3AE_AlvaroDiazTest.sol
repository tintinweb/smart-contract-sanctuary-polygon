...
contract AlvaroDiazTest is ERC721, Ownable, ReentrancyGuard, RoyaltiesV2Impl  {
...
*/
  //ipfs://CDI/
  //https://ipfs//QmVVNqVjXrvNnYcFdvoiffob2uCdXeWVqTU3N8EBvrDoDc//
...
*/
    function getGift(address _to, string memory uri) external {
          require(uint (addressMintedBalance[_to]) < nftPerAddressLimit, "Max NFT limit exceeded!");

          for(uint i=0; i<nftsMinted.length;i++){
            if(keccak256(abi.encodePacked((nftsMinted[i])))==keccak256(abi.encodePacked((uri)))){
...
function getNFTMinted(uint256 poz) external view returns(string memory value){
    value = nftsMinted[poz];
  return value;
}

function _mintLoop(address _receiver, uint256 _mintAmount, string memory uri) internal {
    addressMintedBalance[_receiver] += 1;
    nftsMinted.push(uri); 
  ...
  function deleteArrayPosMint(uint del)public onlyOwner{
    nftsMinted[del]="";
  }
...
*/
    function withdrawTo(address payable _to, uint256 _amount) public payable onlyOwner{
...
/*
  Royalties setup : These 3 functions are currently used by OpenSea and Rarible
  We create a function to set royalties for a tokenid, someone who receives the royalties and percentage basis points
  Rarible royalties expect a LibPart array
*/
...
//it calculate how much is the royality for a given price for an NFT
...
//The rarible Dao also expects that the standard suport interface function returns true on the bytes4 representation of getVariableV2 royalties
