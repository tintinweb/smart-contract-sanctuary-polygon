...
contract Marketplace is ReentrancyGuard {
    ...
    function createMarketSale(address nftContractAddress, uint256 marketItemId) public payable nonReentrant {
        ...
        address creator = marketItemIdToMarketItem[marketItemId].creator; 
        require(msg.value == price, "Please submit the asking price in order to continue");
        marketItemIdToMarketItem[marketItemId].owner = payable(msg.sender);
        marketItemIdToMarketItem[marketItemId].sold = true;

        if (seller == creator) {
           (address account, uint256 fee) = ERC2981Collection(nftContractAddress).royaltyInfo(tokenId, msg.value); 
           marketItemIdToMarketItem[marketItemId].seller.transfer(msg.value - fee);
