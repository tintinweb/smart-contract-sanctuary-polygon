...
contract COEConverter {
  ...
  constructor(
    ...
    conversions[_cards] = Conversion(_cards, 10);
    conversions[_ethernals] = Conversion(_ethernals, 10);
    conversions[_adventurers] = Conversion(_adventurers, 10);
    aegToken = _aegToken;
    cards = _cards;
  ...
  /**
   * @dev Converts cards to ethernals, adventurers or cards
   * @param _assetAddress address of the contract to convert from
   * @param _userAddress address of the user to mint the new tokens to
   * @param _types array of asset types to convert
   * @param _amounts array of amounts of each token to convert
   */

  function _convertCards(
    ...
    if (_assetAddress == ethernals) {
      ...
      uint256[] memory promoCards = CardInterface(cards).getPromoTypes();
      for (uint256 i = 0; i < _types.length; i++) {
        for (uint256 j = 0; j < promoCards.length; j++) {
          require(_types[i] != promoCards[j], "Cannot convert promo cards");
        }
      }
    } else {
    ...
    //PRICE = conversions[_assetAddress].price
    require(
      AEGInterface(aegToken).balanceOf(_userAddress) >=
        conversions[_assetAddress].price, // TODO: NEED TO CHECK HOW WE WILL DO CONVERSION PRICES
    ...
    AEGInterface(aegToken).transferFrom( // TODO: NEED TO CHECK HOW WE WILL DO CONVERSION PRICES
      _userAddress,
      address(this),
      conversions[_assetAddress].price
    );

    //call cards contract to mint the cards to the user
    for (uint256 i = 0; i < _types.length; i++) {
      NftInterface(_assetAddress).adminMint(
        _userAddress,
        _types[i],
        1,
        _amounts[i]
  ...
  // ADMIN=0xf0887ba65ee2024ea881d91b74c2450ef19e1557f03bed3ea9f16b037cbe2dc9
