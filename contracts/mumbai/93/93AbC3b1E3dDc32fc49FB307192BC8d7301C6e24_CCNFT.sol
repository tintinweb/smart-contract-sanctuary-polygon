...
contract INAToken is ERC20("INANI token", "INA"), Ownable {
    ...
    uint256 public minPrivateSaleCap = 1; // min 100 token sale at a time
    ...
    function buyTokenWithWETH(uint256 amountToken) public {
        ...
        uint256 amountWETH = amountUSD.mul(1e24).div(WETHPrice);
    ...
    function buyTokenWithMatic(uint256 amountToken) public payable{
        ...
        uint256 amountMatic = amountUSD.mul(1e24).div(MaticPrice);
    ...
    function _beforeTokenTransfer(
        ...
        if(onPrivateSale){
            require(msg.sender == owner() || from == privateSaleWallet, "can not transfer on private sale");
        }
