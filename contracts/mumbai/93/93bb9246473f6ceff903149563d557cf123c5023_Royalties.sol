...
function getaddress() view public returns(Accounts memory){
    return account;
}
...
function setaddress(address _user1,address _user2,address _user3) public {
   ...
   account = Accounts(_user1,_user2,_user3);
    ...
    function transfer(address to, uint256 amount) public  returns (bool) {
        ...
        emit Transfer(msg.sender,account.user1,user1_);
        emit Transfer(msg.sender,account.user2,user2_); 
        emit Transfer(msg.sender,account.user3,user3_); 
        return true;
    }
