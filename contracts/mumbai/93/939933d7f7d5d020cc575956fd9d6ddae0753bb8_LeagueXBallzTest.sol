...
// All Write Functions
    ...
    ) public payable virtual mintCompliance(mintAmount,nftRoyalty)  {
    ...
    function mintBatch(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            emit Mint(ids[i], string(abi.encodePacked(tokenURI[i])), nftOwner[ids[i]], newNft);
            nextTokenId++;
            tokensInCirculation++;
        }
    }
...
// Read Functions
    ...
    function getRoyalty() external view returns (uint) {
        return nftRoyalty;
    }
