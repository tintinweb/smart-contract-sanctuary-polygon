...
contract DataBank is Ownable {
    ...
    function approveReport(uint256 index) public onlyOwner {
    ...
    function toString(address x) internal pure returns (string memory) {
        ...
        for (uint256 i = 0; i < 20; i++)
            b[i] = bytes1(uint8(uint256(uint160(x)) / (2**(8 * (19 - i)))));
        return string(b);
    }

    // Returns the contents of a report in JSON format
    function getReportJSON(uint256 index) public view returns (string memory) {
        require(index < reportList.length, "Index out of bounds");

        report storage r = reportList[index];

        string memory reportType;
        if (r.reportType == 0) {
            reportType = "Car Accident";
        } else if (r.reportType == 1) {
            reportType = "Missing Sign";
        ...
        string memory reporter = toString(r.reporter);
        string memory stakedAmount = Strings.toString(r.stakedAmount);
