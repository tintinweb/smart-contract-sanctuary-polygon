...
interface IUltibetsReward {
    ...
    function freebetAmountForSBCRoundNFTPerk(uint8)
        external
        view
        returns (uint256);
}

contract BetsFuzzedUTBETS is UltibetsCore {
    using EnumerableSet for EnumerableSet.UintSet;

    IUltiBetsToken private UTBETSContract;

    address public ultiBetsLeaderBoard;
    address public squidBetNFTClaimer;
    address public ultibetsReward;

    mapping(address => uint256) public totalBetAmountByBettor;
    ...
    function setUltiBetsLeaderBoard(address _ultiBetsLeaderBoard)
        public
        onlyAdmin
    {
    ...
    function setSquidBetNFTClaimer(address _squidBetNFTClaimer)
        public
        onlyAdmin
    {
        squidBetNFTClaimer = _squidBetNFTClaimer;
    ...
    function getBalance() public view override returns (uint256) {
        return UTBETSContract.balanceOf(address(this));
    }
    ...
    function placeBetUsingPerk(
        ...
        require(
            eventList[_eventID].status == EventStatus.Open &&
                block.timestamp <= eventList[_eventID].startTime - 1 hours,
            "Non available bet."
        );
        ...
        emit BetPlaced(
            ...
            _eventValue,
            betAmount,
            IUltiBetsLeaderBoard(ultiBetsLeaderBoard).isInfluencer(msg.sender)
        );
    }
    ...
    function placeBet(
        ...
        require(
            UTBETSContract.balanceOf(msg.sender) >= _betAmount,
            "Not enough to bet on the round!"
        );

        UTBETSContract.approveOrg(address(this), _betAmount);
        UTBETSContract.transferFrom(msg.sender, address(this), _betAmount);
        ...
        totalBetAmountByBettor[msg.sender] += _betAmount;
    ...
    function withdrawGain(uint256 _eventID, EventResult _betValue) external {
        ...
        uint256 gain = getWinAmount(_eventID, _betValue);
    ...
    function EmergencySafeWithdraw() external onlyAdmin {
        UTBETSContract.transfer(ultiBetsTreasury, getBalance());
        // POST assertions
        assert(getBalance() == 0);
    }
}
...
contract UltibetsCore is CustomAdmin {
    ...
    uint256 totalEventNumber;

    CategoryInfo[] public categoryList;
    mapping(uint8 => mapping(uint8 => string)) subcategories;
    mapping(uint8 => EnumerableSet.UintSet) liveEventsByCategory;
    mapping(uint8 => mapping(string => EnumerableSet.UintSet)) dailyEventsByCategory;

    mapping(uint256 => EventInfo) eventList;
    mapping(address => BetData[]) betsByBettor; //bet list of a bettor
    mapping(address => mapping(uint256 => mapping(EventResult => BetData))) betDataByBettor; //betting history of a bettor
    ...
    function betsOfBettor(
        address _bettor,
        uint256 _page,
        uint256 _pageSize
    ) external view returns (BetHistoryData[] memory) {
        uint256 betNum = totalBetsOfBettor(_bettor);

        require(_pageSize > 0, "Page size should be positive.");
        require(_page > 0 && (_page - 1) * _pageSize < betNum, "Out of bounds");

        uint256 betsSize = _pageSize;
        if (_page * _pageSize > betNum) {
            betsSize = betNum - (_page - 1) * _pageSize;
        }
        BetHistoryData[] memory betList = new BetHistoryData[](betsSize);
        for (uint256 i; i < betsSize; i++) {
            BetData memory bet = betsByBettor[_bettor][
                (_page - 1) * _pageSize + i
            ];
            BetHistoryData memory betH = BetHistoryData(
                eventList[bet.eventID],
                bet
            );
            betList[i] = betH;
        }
    ...
    function addCategory(
        ...
        categoryList.push(
            CategoryInfo(_name, _type, uint8(_subcategories.length))
        );
        for (uint8 i; i < _subcategories.length; i++) {
            subcategories[uint8(categoryList.length)][i + 1] = _subcategories[i];
        }
    ...
    function removeExpiredEvent(uint8 _category, uint256 _eventID) internal {
        ...
        string memory date = eventList[_eventID].eDate;
        dailyEventsByCategory[_category][date].remove(_eventID);

        emit RemoveExpiredEvents(_category, _eventID);
    }

    function totalBetsOfBettor(address _bettor) public view returns (uint256) {
        return betsByBettor[_bettor].length;
    }

    ///@notice function to stop bet
    ///please note that this action can only be performed by an oracle.
    function stopBet(uint256 _eventID) external OnlyOracle {
        eventList[_eventID].status = EventStatus.Stop;
    }
    ...
    function checkBetResult(
        ...
        if (bet.betAmount > 0) {
            ...
            } else if (evt.status == EventStatus.Stop)
                result = BetStatus.Active;
    ...
    function getDailyEventsByCategory(uint8 _category, string memory _date)
        external
        view
        returns (EventInfo[] memory)
    {
        uint256 numberOfEvents = dailyEventsByCategory[_category][_date]
            .length();
        EventInfo[] memory eventDataList = new EventInfo[](numberOfEvents);
        for (uint256 i; i < numberOfEvents; i++) {
            eventDataList[i] = eventList[
                dailyEventsByCategory[_category][_date].at(i)
            ];
        }
    ...
    {
        require(categoryList.length > _category, "Invalid category number.");
        require(
    ...
    function getWinAmount(uint256 _eventID, EventResult _betValue)
        internal
        view
        returns (uint256)
    {
        uint256 betAmount = betDataByBettor[msg.sender][_eventID][_betValue]
            .betAmount;

        uint256 winAmount = (((eventList[_eventID].bettingVolume * betAmount) /
            betAmountsPerSideByEvent[_eventID][_betValue]) * feePercentage) /
            100;
        ...
        returns (EventDetailPerBettor memory)
    {
        ...
        EventType eType = categoryList[category - 1].eType;
