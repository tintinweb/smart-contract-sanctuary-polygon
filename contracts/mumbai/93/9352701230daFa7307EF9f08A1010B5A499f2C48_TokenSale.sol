...
contract TokenSale is Initializable, OwnableUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping(address => mapping(address => uint256)) public lastPurchasedAt; // account=>token=>timestamp
    ...
    function buy(address _token, uint256 _amount) public nonReentrant returns (uint256) {
        ...
        lastPurchasedAt[_msgSender()][_token] = block.timestamp;
    ...
    function sell(address _token, uint256 amount) public nonReentrant returns (uint256) {
        require(_isSameMonth(lastPurchasedAt[_msgSender()][_token], block.timestamp), "Invalid condition");
        ...
        IERC20MetadataUpgradeable(_token).safeTransferFrom(address(this), _msgSender(), _amount);
    ...
    function _isSameMonth(uint256 timestamp1, uint256 timestamp2) internal pure returns (bool) {
        return getMonth(timestamp1) == getMonth(timestamp2);
    }
