...
interface IMissionControlStakeable {
     ...
     */
    function isBase(uint256 _tokenId) external view returns (bool _isBase);

    function isRaidable(uint256 _tokenId) external view returns (bool _isRaidable);
    ...
    ERC1155HolderUpgradeable
{
    ...
    function checkTile(
        ...
        return __check_tile(_userAddress, _nonce, _pausedAt);
    }
    ...
    function onCollect(
        ...
        (uint256 available, uint256 id) = __check_tile(_userAddress, _nonce, _pausedAt);
    ...
    function __timeLeft(address _userAddress, uint256 _nonce)
        private
        view
        returns (int256 _timeLeft)
    {
    ...
    function __timeLeft_facility(
        ...
        uint256 localSeed = seeds[_userAddress][_nonce];
        require(localSeed != 0, "MCS1155_ADAPTER: No such token staked");
        uint256 completionTime = (tokenInfo[tokenId].resourceCap *
    ...
    function __check_tile(
        ...
        Distribution class = tokenInfo[tokenId].class;
        if (class == Distribution.Poisson) {
            return __checkTile_Poisson(_userAddress, tokenId, _nonce, _pausedAt);
        }
        if (class == Distribution.LinearDeterministic) {
            return __checkTile_deterministic_linear(_userAddress, tokenId, _nonce, _pausedAt);
        }
        if (class == Distribution.Facility) {
            return __checkTile_facility(_userAddress, tokenId, _nonce, _pausedAt);
        }
        revert("Critical: Distribution class not supported, blame whoever implemented it");
    }
    ...
    function __checkTile_deterministic_linear(
        ...
        require(localSeed != 0, "MCS1155_ADAPTER: No such token staked");
        uint256 untilTimestamp = _pausedAt > 0 ? _pausedAt : block.timestamp;
        uint256 ret = (tokenInfo[tokenId].resourceEV * (untilTimestamp - localSeed)) / DAY_IN_SEC;
        return (ret, tokenInfo[tokenId].resourceId);
    }
    ...
    function __checkTile_Poisson(
        ...
        uint256 untilTimestamp = _pausedAt > 0 ? _pausedAt : block.timestamp;
        int256 blockTime = int256(untilTimestamp);
