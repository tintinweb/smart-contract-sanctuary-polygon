...
import "./interfaces/IKramaNFT.sol";
import "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
import "@openzeppelin/contracts-upgradeable/access/AccessControlEnumerableUpgradeable.sol";
import "@openzeppelin/contracts-upgradeable/utils/CountersUpgradeable.sol";

contract VirtualNFTMarketplace is
    Initializable,
    AccessControlEnumerableUpgradeable
{
    IKramaNFT public KramaNFT;
    ...
    address payable public feeAccount; // EOA owned by Krama
    uint8 private SALE_FEE; // fee on all sales paid to Krama
    ...
    event PlacedBid(address indexed _bidder, uint256 _bid);
    event Trade(
    ...
    event SetForFixPriceSale(
        uint256 indexed fixPriceSaleId,
        uint256 indexed _tokenId,
        uint256 _amount,
        address indexed _owner
    );
    ...
    event UpdateSalePrice(uint256 indexed _fixPriceSaleId, uint256 _newPrice);
    event UpdateFee(address indexed _by, uint256 _newFee);
    ...
    modifier saleRequirements(uint256 _tokenId) {
        require(KramaNFT.isTokenExists(_tokenId), "nonexistent tokenId.");
        require(KramaNFT.ownerOf(_tokenId) == msg.sender, "not the owner");
        _;
    }
    ...
    modifier fixSaleReqs(uint256 _fixPriceSaleId) {
        require(
            fixPriceSales[_fixPriceSaleId]._owner == msg.sender,
            "not owner."
        );
        require(fixPriceSales[_fixPriceSaleId]._isSold == false, "NFT is sold");
        require(
            fixPriceSales[_fixPriceSaleId]._isCanceled == false,
            "not on sale"
        );
        _;
    }
    ...
    function initialize(address _NFT) public initializer {
        ...
        feeAccount = payable(_admin); // account or contract that can redeem funds from fees.

        SALE_FEE = 5; // 5% fee on all sales paid to Krama (artist receives the remainder, 95%)
        SECONDARY_SALE_FEE_ARTIST = 5; // 5% fee (profits) on all secondary sales paid to the artist (seller receives remainder after paying 5% secondary fees to artist and 5% to Krama, 90%)
        KramaNFT = IKramaNFT(_NFT);
    }

    /**
     * @dev setter function only callable by contract admin used to change the address to which fees are paid
     * @param _feeAccount is the address owned by Krama that will collect sales fees
     */
     ...
     * @param _fee is the new %age for Krama marketplace
     */
     ...
     */
    function setSecondarySaleFeeArtist(
        uint8 _fee
    ) external onlyAdmin isAcceptAbleFee(_fee) {
        SECONDARY_SALE_FEE_ARTIST = _fee;
        emit UpdateFee(msg.sender, SECONDARY_SALE_FEE_ARTIST);
    }

    /**
     * @dev mints a token using KramaNFT contract
     * @param _tokenURI is URI of the NFT's metadata
     */
    function mint(string memory _tokenURI) external {
        require(hasRole(ARTIST_ROLE, msg.sender), "not ARTIST"); // Must be an artist
        uint256 _tokenId = KramaNFT.totalSupply(); // set URI before minting otherwise the total supply will get increamented
        KramaNFT.mint(_tokenURI, msg.sender);
    ...
    function createAuction(
        ...
        KramaNFT.transferFrom(msg.sender, address(this), _tokenId);
        emit AuctionCreated(
    ...
    function settleAuction(
        uint256 _auctionId
    ) external auctionExists(_auctionId) hasAuctionEnded(_auctionId) {
    ...
    function placeBid(
        uint256 _auctionId
    ) public payable auctionExists(_auctionId) {
        Auction storage _foundAuction = auctions[_auctionId];
        require(
        ...
        if (!auctionBidders[_auctionId][msg.sender].isVaild) {
            if (_foundAuction._highestBid != 0)
                ...
                require(
                    msg.value >= _foundAuction._reservePrice,
                    "bid underflow"
                );
            auctionBidders[_auctionId][msg.sender] = Bidder(
                msg.value,
                false,
                true
            );
            ...
            uint256 oldBidAmount = auctionBidders[_auctionId][msg.sender]
                .amount;
            require(
                msg.value > oldBidAmount &&
                    msg.value > _foundAuction._highestBid,
                "low bid"
            );
     ...
     */
    function withdrawLostAuctionBid(
        uint256 _auctionId
    ) external auctionExists(_auctionId) hasAuctionEnded(_auctionId) {
        Auction memory _foundAuction = auctions[_auctionId];
        require(
    ...
    function purchaseNFT(
        uint256 _fixPriceSaleId
    ) external payable saleExists(_fixPriceSaleId) {
     ...
     */
    function cancelFixPriceSale(
        uint256 _fixPriceSaleId
    ) external saleExists(_fixPriceSaleId) fixSaleReqs(_fixPriceSaleId) {
        FixPriceSale storage _foundSale = fixPriceSales[_fixPriceSaleId];
        _foundSale._isCanceled = true;
        _foundSale._onSale = false;
        KramaNFT.transferFrom(address(this), msg.sender, _foundSale._tokenId);
     ...
     */
    function updatePriceOfFixedSale(
        uint256 _fixPriceSaleId,
        uint256 _newPrice
    ) external saleExists(_fixPriceSaleId) fixSaleReqs(_fixPriceSaleId) {
        FixPriceSale storage _foundSale = fixPriceSales[_fixPriceSaleId];
        require(
            (_foundSale._price != _newPrice) && (_newPrice > 0),
            "underflow"
        );
    ...
    function _trade(
        ...
        KramaNFT.transferFrom(address(this), _buyer, _tokenId);
        ...
        if (!secondarySale[_tokenId]) {
            ...
            uint256 _secondaryFeeArtist = (_amount *
                SECONDARY_SALE_FEE_ARTIST) / 100; // Fee paid by the user that fills the order, a.k.a. msg.sender.
            _feeAmount = _secondaryFeeArtist + _feeAmount;
            artists[_tokenId].transfer(_secondaryFeeArtist); // transfer secondary sale fees to fee artist
        }
        uint256 remainingFee = _amount - _feeAmount;
