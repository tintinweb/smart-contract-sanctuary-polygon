...
contract Example is ERC721, Ownable, PaymentSplitter {
    ...
    // % Of discount for whitlisted users - if discount = 5 => 5% of discount
    ...
    // Claimed cars (just once per id)
    mapping(uint => bool) public claimedCar;
    // Addres can change claimedCar(owner default) -> this can change for conect of a contract in future
    address public adminClaimCars;

    constructor(address[] memory _payees, uint256[] memory _shares) ERC721("Example", "EXMPL") PaymentSplitter(_payees, _shares) payable {
        //  Max amount of tokens
        maxAmntTokens = 10000;
        ...
        //  Max mints pre sale
        maxMintsPreSale = 1;
        // Default adminClaimer is owner
        adminClaimCars = owner();
    }
    ...
    //  instead of safeMint
    function mintNFT(uint256 amountTokens, address _to) public payable {
        ...
        require (msg.value == (getPrice(msg.sender).mul(amountTokens)), "Amount of Ether incorrect, try again.");
    ...
    // Change discount for mint of whitlisted address
    function setDiscount(uint256 _newDiscount) public onlyOwner{
        discount = _newDiscount;
    }

    // set mapping claimedCar to true for id - only call adminClaimCars address, can a contract or wallet
    function setClaimedCar(uint256 _id) public {
        require(msg.sender == address(adminClaimCars), "You dont admin of set claimedCar");
        require(!claimedCar[_id], "This id already claimed car");
        claimedCar[_id] = true;
    }

    // In future, can connect other contractat this adminClaimers to automate claimedCar mapping
    // Owner is default adminClaimerCars
    function setAdminClaimCars(address _newAdminClaimeCars) public onlyOwner {
        adminClaimCars = _newAdminClaimeCars;
    }

    function getPrice(address _sender) public view returns(uint){
        uint mintPrice = price;
        if (preSaleIsActive) {             
            if(reserveListPreSale[_sender] && reserveListPreSaleCounter[_sender] < maxMintsPreSale){
                //calculate discount and rest of standar price.
                mintPrice = price.sub(price * discount / 100);
            }
        }
        return mintPrice;
    }
