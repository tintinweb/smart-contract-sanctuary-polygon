...
import "./interfaces/IBoberiumVoterOld.sol";
import "./interfaces/IBoberiumVotingStorage.sol";

contract BoberiumMigrator is Ownable {
    ...
    IBoberiumVoterOld public oldVotingContract;
    IERC721 public votingNft;


    constructor(IBoberiumVotingStorage _votingStorage, IBoberiumVoterOld _oldVotingContract, IERC721 _votingNft) {
        votingStorage = _votingStorage;
        oldVotingContract = _oldVotingContract;
        votingNft = _votingNft;
        emit VotingStorageChanged(_votingStorage);
    }

    function updateVotingStorage(IBoberiumVotingStorage _votingStorage) public onlyOwner {
        votingStorage = _votingStorage;
        emit VotingStorageChanged(_votingStorage);
    }

    function getProposal(bytes32 proposalId) external view returns (Proposal memory proposal){
        uint256 oldId = newIdToOld(proposalId);
        IBoberiumVoterOld.OldProposal memory oldProposal = oldVotingContract.getProposal(oldId);

        proposal = Proposal({
            choicesCount: oldProposal.choicesCount,
            voteStart: oldProposal.voteStart,
            voteEnd: oldProposal.voteEnd,
            votingToken: votingNft,
            author: owner()
        });
    ...
    function getVotes(bytes32 proposalId) external view returns (uint256[] memory) {
        ...
        uint256[] memory votes = new uint256[](oldProposal.choicesCount);
        for (uint i = 0; i < oldProposal.choicesCount; i++) {
            votes[i] =  oldVotingContract.totalVotes(oldId, i);
        }
        return votes;
    }
    ...
    function oldIdToNew(
        ...
        return bytes32(oldId);
    }

    function newIdToOld(
        bytes32 newId
    ) public view returns (uint256) {
        return uint256(newId);
    }
}
...
interface IBoberiumVoterOld {
    ...
    function getProposal(uint256 proposalId) external view returns (OldProposal memory proposal);
    function getVotes(uint256 proposalId) external view returns (uint256[] memory);
    function totalVotes(uint256 proposalId, uint256 choice) external view returns (uint256);
}
...
contract BoberiumVotingNFT is ERC721Enumerable, Ownable {
...
contract BoberiumToken is ERC20, Pausable, Ownable {

    constructor() ERC20("Boberium Token", "BBRM") {}
