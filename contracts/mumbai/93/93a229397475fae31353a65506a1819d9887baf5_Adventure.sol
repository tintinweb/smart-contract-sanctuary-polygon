    ...
    IAdventure
{
    ...
    // itemId -> allowed
    mapping(uint256 => bool) allowedPotions;
    ...
    modifier onlyPassOwner(uint256 ottoId_) {
        require(
    ...
    modifier ottoIsReady(uint256 ottoId_) {
        require(
    ...
    modifier ottoInAdventure(uint256 ottoId_) {
        require(OTTO.ownerOf(ottoId_) == address(this), 'not in adventure');
        _;
    }

    modifier ottoIsResting(uint256 ottoId_) {
        require(block.timestamp < restingUntil[ottoId_], 'not resting');
        _;
    }

    modifier ottoCanRevive(uint256 ottoId_) {
        require(
    ...
    function initialize(
        ...
        setProducts(reviveProduct_, finishImmediatelyProduct_);
    }
    ...
    ) external onlyEOA onlyOttoOwner(ottoId_) ottoIsReady(ottoId_) {
    ...
    function finishFee(uint256 passId_) public view returns (uint256) {
        return _finishProductAmount(passId_) * 10**CLAM.decimals();
    }

    function _handleFinishImmediately(uint256 passId_) private {
        require(
            STORE.discountPriceOf(finishImmediatelyProduct) ==
                10**CLAM.decimals(),
            'fee decimals mismatch'
        );
        uint256 fee_ = finishFee(passId_);
        CLAM.transferFrom(msg.sender, address(this), fee_);
        CLAM.approve(address(STORE), fee_);
        STORE.buy(
            address(this),
    ...
    function usePotions(uint256 ottoId_, uint256[] memory potions_)
        public
        onlyPassOwner(ottoId_)
    {
        IOttoDiamondERC721.ItemActionInput[]
    ...
    function finish(
        ...
        uint256[] calldata potions_,
        Signature calldata sig_
    ) external onlyEOA onlyPassOwner(ottoId_) validPercentage(sr_) {
        _verify(
            keccak256(
                abi.encodePacked(
                    msg.sender,
    ...
    function revive(uint256 ottoId_)
        external
        onlyEOA
        onlyOttoOwner(ottoId_)
        ottoCanRevive(ottoId_)
    {
        uint256 passId = _ottoPass[ottoId_];
        Pass storage p = _passes[passId];
        uint256 price = STORE.discountPriceOf(reviveProduct);
        CLAM.transferFrom(msg.sender, address(this), price);
        CLAM.approve(address(STORE), price);
        STORE.buy(address(this), reviveProduct, 1);
        p.revived = true;
        _award(p.ottoId, false);

        emit Revive(passId, p.ottoId);
    }

    function _award(uint256 ottoId_, bool withPoints_) private {
        uint256 passId = _ottoPass[ottoId_];
        Pass storage p = _passes[passId];
    ...
    function stakedOttosOf(address owner_)
        external
        view
        returns (uint256[] memory)
    {
        uint256 n = PASS.balanceOf(owner_);
        uint256[] memory result = new uint256[](n);
        for (uint256 i = 0; i < n; i++) {
            result[i] = _passes[PASS.tokenOfOwnerByIndex(owner_, i)].ottoId;
        }
        return result;
    }
