...
contract Staking {
    ...
    modifier onlyDelegatorOf(address delegatedValidator) {
        require(_isDelegator(msg.sender, delegatedValidator), "Only delegator can call function");
        _;
    }
    ...
    function delegateTokens(address delegatedValidator) public payable onlyEOA {
        _delegateTo(delegatedValidator);
    }

    function delegatorUnstake(address delegatedValidator) public onlyEOA onlyDelegatorOf(delegatedValidator) {
        _delegatorUnstake(delegatedValidator);
    }

    function getDelegatedAmountFrom(address delegatedValidator) public view onlyDelegatorOf(delegatedValidator) returns (uint256) {
        return _addressToDelegatorInfo[msg.sender].validatorToStakedAmount[delegatedValidator];
    }
    ...
    function _delegateTo(address delegatedValidator) private {
        _stakedAmount += msg.value;

        _addressToDelegatorInfo[msg.sender].validatorToStakedAmount[delegatedValidator] += msg.value;

        if (_canBecomeDelegator(msg.sender, delegatedValidator)) {
            _appendToDelegatorSet(msg.sender, delegatedValidator);
        }

        emit DelegatorStaked(msg.sender, delegatedValidator, msg.value);
    }

    function _delegatorUnstake(address delegatedValidator) private {
        uint256 amount = _addressToDelegatorInfo[msg.sender].validatorToStakedAmount[delegatedValidator];

        _addressToDelegatorInfo[msg.sender].validatorToStakedAmount[delegatedValidator] = 0;
        _stakedAmount -= amount;

        if (_isDelegator(msg.sender, delegatedValidator)) {
            _deleteFromDelegatorsOfValidator(msg.sender, delegatedValidator);
            _deleteFromValidatorsOfDelegator(msg.sender, delegatedValidator);
        }
    ...
    function _deleteFromDelegatorsOfValidator(address account, address delegatedValidator) private {
        require(
            _addressToDelegators[delegatedValidator].length > _minimumNumDelegators,
            "Delegators can't be less than the minimum required"
        );

        require(
            _addressToDelegatorInfo[account].delegatorIndex[delegatedValidator] < _addressToDelegators[delegatedValidator].length,
            "index out of range"
        );

        // index of removed address
        uint256 index = _addressToDelegatorInfo[account].delegatorIndex[delegatedValidator];
        uint256 lastIndex = _addressToDelegators[delegatedValidator].length - 1;

        if (index != lastIndex) {
            // exchange between the element and last to pop for delete
            address lastAddr = _addressToDelegators[delegatedValidator][lastIndex];
            _addressToDelegators[delegatedValidator][index] = lastAddr;
            _addressToDelegatorInfo[account].delegatorIndex[lastAddr] = index;
        }

        _addressToDelegatorInfo[account].isSupportingValidator[delegatedValidator] = false;
        _addressToDelegatorInfo[account].delegatorIndex[delegatedValidator] = 0;
        _addressToDelegators[delegatedValidator].pop();
    }

    function _deleteFromValidatorsOfDelegator(address account, address delegatedValidator) private {
        require(
        ...
        require(
            _addressToDelegatorInfo[account].validatorIndex[delegatedValidator] < _addressToDelegatorInfo[account].validators.length,
            "index out of range"
        );

        // index of removed address
        uint256 index = _addressToDelegatorInfo[account].validatorIndex[delegatedValidator];
    ...
    function _appendToDelegatorSet(address newDelegator, address delegatedValidator) private {
        require(
            _addressToDelegators[delegatedValidator].length < _maximumNumDelegators,
            "Delegator set has reached full capacity"
        );

        _addressToDelegatorInfo[newDelegator].isSupportingValidator[delegatedValidator] = true;
        _addressToDelegatorInfo[newDelegator].delegatorIndex[delegatedValidator] = _addressToDelegators[delegatedValidator].length;
        _addressToDelegatorInfo[newDelegator].validators.push(delegatedValidator);
        _addressToDelegatorInfo[newDelegator].validatorIndex[delegatedValidator] = _addressToDelegatorInfo[newDelegator].validators.length;

        _addressToDelegators[delegatedValidator].push(newDelegator);
    }

    function _isDelegator(address account, address delegatedValidator) private view returns (bool) {
        return _addressToDelegatorInfo[account].isSupportingValidator[delegatedValidator];
    }

    function _canBecomeDelegator(address account, address delegatedValidator) private view returns (bool) {
        return !_addressToDelegatorInfo[account].isSupportingValidator[delegatedValidator]
        && _addressToDelegatorInfo[account].validatorToStakedAmount[delegatedValidator] >= DELEGATION_THRESHOLD;
    }
}