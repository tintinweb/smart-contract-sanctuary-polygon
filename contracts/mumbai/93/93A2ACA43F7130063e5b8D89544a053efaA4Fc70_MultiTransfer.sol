...
/**
 * @title Contract to transfer ERC20 token to multiple addresses in one call.
 * @author Dalton Tan
 */
contract MultiTransfer is ERC2771Recipient {
	/// @notice Argument given to the method is not correct.
	error InvalidArgument();

	/// @notice Transfers are done with this token.
	ERC20Permit public token;

	constructor(address _forwarder, ERC20Permit _token) {
		_setTrustedForwarder(_forwarder);
		token = _token;
	}

	/**
	 * @notice Sending token to multiple recipients.
	 * @param recipients List of address to send token to.
	 * @param amounts List of token amount to send to the respective recipients.
	 */
	function transfer(
		...
		for (uint i = 0; i < recipients.length; i++) {
			token.transferFrom(_msgSender(), recipients[i], amounts[i]);
		}
	}

	/**
	 * @notice Sending token to multiple recipients.
	 * @param from The account to take the token from.
	 * @param recipients List of address to send token to.
	 * @param amounts List of token amount to send to the respective recipients.
	 */
	...
	/**
	 * @notice Calling transfer() but with the additional ERC20Permit call before.
	 */
	function transferWithPermit(
		...
		transfer(recipients, amounts);
	}

	/**
	 * @notice Calling transferFrom() but with the additional ERC20Permit call before.
	 */
	function transferFromWithPermit(
		...
		token.permit(owner, spender, value, deadline, v, r, s);
		transferFrom(from, recipients, amounts);
	}
