...
contract Registry is Initializable, OwnableUpgradeable, EIP712Upgradeable {
    ...
    // Mappers
    mapping(address => Account) private accountRecords;
    ...
    function registerAccount(
        ...
        SigEncKeys memory seKeys,
        bytes memory deviceName,
        bytes32 deviceNameHash,
        bytes memory boxedSeed
    ) public {
        LocalCommonVars memory vars = LocalCommonVars(
            0,
            seKeys.deviceSigningPublicKey, signature,
            seKeys.signingPublicKey, seKeys.encryptionPublicKey,
            seKeys.deviceSigningPublicKey, seKeys.deviceEncryptionPublicKey,
    ...
    function registerDeviceByAdmin(
        ...
        _registerDevice(account, index,
            vars,
            boxedSeed);
    }

    function getRawSignatureChains(address account, uint offset, uint size) public view returns (SigChainItem[] memory) {
        require(offset >= 0, "offset must be non-negative");
        if (size <= 0) {
            size = accountRecords[account].items.length;
        }
        if (offset + size > accountRecords[account].items.length) {
            size = accountRecords[account].items.length - offset;
        }
        SigChainItem[] memory items = new SigChainItem[](size);
        uint itemLength = accountRecords[account].items.length;
        for(uint i = 0; (i+offset) < itemLength && i < size; i++) {
            items[i] = accountRecords[account].items[i+offset];
        }
        return items;
    }
    ...
    function getAccountPublicKeys(address account) public view returns (PublicKeyBundle memory bundle) {
        SigChainItem[] storage accItems = accountRecords[account].items;
        if (accItems.length == 0) { return bundle; }

        uint itemLength = accItems.length;
        for(uint i = itemLength-1; i >= 0; i--) {
            SigChainItem storage item = accItems[i];
            if (unpackAction(item.itemHeader) == SIG_ACTION_NEW_ACCOUNT
            || unpackAction(item.itemHeader) == SIG_ACTION_UPDATE_ACCOUNT_KEYS
            ) {
                bundle.timestamp = unpackTimestamp(item.itemHeader);
                bundle.version = unpackVersion(item.itemHeader);
    ...
    function getDevices(address account, bool populateDeviceNames, bool populateSigners) public view returns (DeviceInfo[] memory devices) {
        SigChainItem[] storage accItems = accountRecords[account].items;
        ...
        for(uint i = 0; i < itemLength; i++) {
            SigChainItem storage item = accItems[i];
            if (unpackAction(item.itemHeader) == SIG_ACTION_NEW_ACCOUNT
            || unpackAction(item.itemHeader) == SIG_ACTION_REGISTER_DEVICE
            ) {
                deviceCount++;
            }
        }
        ...
        for(uint i = 0; i < itemLength; i++) {
            ...
            DeviceInfo memory dev = devices[di];
            di++;
            dev.timestamp = unpackTimestamp(item.itemHeader);
            dev.version = unpackVersion(item.itemHeader);
            ...
            for(uint j = i+1; j < itemLength; j++) {
                SigChainItem storage rvItem = accItems[j];
                if (unpackAction(rvItem.itemHeader) == SIG_ACTION_UPDATE_ACCOUNT_KEYS) {
                    for(uint k = 0; i < rvItem.revokedKeys.length; k++) {
                        if (rvItem.revokedKeys[k] == dev.deviceSigningPublicKey || rvItem.revokedKeys[k] == dev.deviceEncryptionPublicKey) {
                            dev.isRevoked = true;
                            dev.revokedTimestamp = unpackTimestamp(rvItem.itemHeader);
                            break;
                        }
                    }
                }
    ...
    function getDraftDevices() public view returns (DeviceInfo[] memory devices) {
        ...
        for(uint i = 0; i < itemLength; i++) {
            SigChainItem storage item = accItems[i];
            if (unpackAction(item.itemHeader) == SIG_ACTION_LOCAL_ADD_DEVICE) {
                draftPks[drafti] = item.deviceSigningPublicKey;
                draftIxs[drafti] = drafti;
                drafti++;
            }
            else if (unpackAction(item.itemHeader) == SIG_ACTION_NEW_ACCOUNT
            || unpackAction(item.itemHeader) == SIG_ACTION_REGISTER_DEVICE
            ) {
        ...
        devices = new DeviceInfo[](drafti);
        for(uint i = 0; i < drafti; i++) {
            SigChainItem storage item = accItems[draftIxs[i]];
            devices[i].timestamp = unpackTimestamp(item.itemHeader);
            devices[i].version = unpackVersion(item.itemHeader);
    ...
    function getVaultSeeds() public view returns(SeedItem[] memory) {
    ...
    function _registerAccount(
        address account,
        LocalCommonVars memory vars,
        bytes memory boxedSeed
    ) private {
        require(accountRecords[account].items.length == 0, "Account Exists");
        Account storage acct = accountRecords[account];
        acct.items.push();
        SigChainItem storage item = acct.items[0];

        item.itemHeader = packHeader(1, uint64(block.timestamp), SIG_ACTION_NEW_ACCOUNT);
        ...
        if (keccak256(boxedSeed) != keccak256("")) {
            acct.seedItems.push();
            SeedItem storage sItem = acct.seedItems[acct.seedItems.length-1];
    ...
    function _registerDevice(
        ...
        SigChainItem[] storage acctItems = accountRecords[account].items;
        require(acctItems.length > 0, "Account Not Exists");
        require(acctItems.length == index-1, "Mismatched SigChain Length");
        _registerDeviceCheck(account, vars);

        acctItems.push();

        SigChainItem storage item = acctItems[acctItems.length - 1];

        item.itemHeader = packHeader(1, uint64(block.timestamp), SIG_ACTION_REGISTER_DEVICE);
        ...
        item.deviceName = vars.deviceName;
        item.deviceNameHash = vars.deviceNameHash;
        bytes memory data = bytes.concat(vars.deviceNameHash, vars.deviceName);
        item.deviceName = data;
        if (keccak256(boxedSeed) != keccak256("")) {
            SeedItem[] storage seedItems = accountRecords[account].seedItems;
            seedItems.push();
            SeedItem storage seedItem = seedItems[seedItems.length - 1];
    ...
    function _registerDeviceCheck(address account, LocalCommonVars memory vars) private view returns (bool) {
        SigChainItem[] storage acctItems = accountRecords[account].items;
        ...
        for(uint i = 0; i < itemLength; i++) {
            SigChainItem storage dvItem = acctItems[i];
            if (unpackAction(dvItem.itemHeader) == SIG_ACTION_LOCAL_DISAPPROVE_DEVICE
            || unpackAction(dvItem.itemHeader) == SIG_ACTION_REGISTER_DEVICE
            || unpackAction(dvItem.itemHeader) == SIG_ACTION_NEW_ACCOUNT
            ) {
