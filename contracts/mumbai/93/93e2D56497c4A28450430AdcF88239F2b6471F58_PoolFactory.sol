...
interface IContractOpenablePack721 is IPermissionsEnumerable, IERC1155Upgradeable {
    /**
     *  @notice All info relevant to packs.
     *
     *  @param perUnitAmounts           Mapping from a UID -> to the per-unit amount of that asset i.e. `Token` at that index.
     ...
     *  @param amountToOpen The number of packs to open at once.
     */
    function openPack(uint256 packId, uint256 amountToOpen, address recipient) external returns (uint256[] memory rewardTokenIds, PackInfo memory pack);
...
interface IPool is IERC721ReceiverUpgradeable, IERC1155ReceiverUpgradeable {
    ...
    function transferTokens(ITokenBundle.Token[] calldata token, address recipient) external;
    function transferERC721Tokens(IERC721Upgradeable assetContract, uint256[] calldata tokenIds, address recipient) external;
...
interface IPoolFactory is ITimeboundEIP712, IERC1155ReceiverUpgradeable {
    ...
    function createPool(bool is721) external returns(address _poolAddress);
