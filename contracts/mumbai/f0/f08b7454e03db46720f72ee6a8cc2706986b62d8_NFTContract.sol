...
contract NFTContract is ERC721, ERC721URIStorage, ERC721Burnable, Ownable {
    ...
    mapping (uint256 => NFTstruct) public NFTmapping ; // List of NFT data, pass tokenID of NFT as @param
    ...
    // shows balance in this contract
    function getContractBalance() public view returns( uint256 ){
        return address(this).balance;
    }
    
    // Used by owner to set some admin
    ...
    // Used by admin to send contract balance to anyone
    ...
    // sets selling price of NFTs
    function setNFTprice( uint256[] memory _ids, uint _price) public  {
         for (uint256 i = 0; i < _ids.length; i++)
         {
             require(_exists( _ids[i])== true, "ID does not exist" );
             require( NFTmapping[ _ids[i] ].owner == msg.sender, "Not owner of this NFT");
             NFTmapping[ _ids[i] ].price = _price;
             emit priceSetEvent( _ids[i] , true );
         }
    }

    // gets prices of some NFTs
    function getNFTprice( uint256[] memory _ids) public  returns (uint256[] memory) {
         ...
         {
             require(_exists( _ids[i])== true, "ID does not exist" );
             temp[i] = NFTmapping[ _ids[i] ].price;
             emit priceSetEvent( _ids[i] , true );
         }
    ...
    // takes array of tokenURIs and mints NFTs
    function batchMint( string[] memory tokenUriList) public payable returns (uint256 [] memory )
    {
        ...
        for (uint256 i = 0; i < tokenUriList.length; i++) 
        {
            ...
            NFTmapping[tokenId].id = tokenId;
            NFTmapping[tokenId].price = mintPrice;
            NFTmapping[tokenId].uri = tokenUriList[i];
            NFTmapping[tokenId].owner = msg.sender;
            NFTmapping[tokenId].mintTime = block.timestamp;
            NFTmapping[tokenId].forSale = false;
...
// takes list of ids and returns their price if they are on sale
function getSalePrice( uint256[] memory _ids) public view returns(uint256) {
        uint256 bundleCost = 0;
         for (uint256 i = 0; i < _ids.length; i++)
         {
             require(_exists( _ids[i])== true, "ID does not exist" );
             require( NFTmapping[ _ids[i] ].forSale == true, "NFT not for sale");
             bundleCost += NFTmapping[_ids[i]].price;
         }
         return bundleCost;
}

// cancels sale of NFTs. Can only be used by the owner of those NFTs
function cancelSale( uint256[] memory _ids) public  {
         for (uint256 i = 0; i < _ids.length; i++)
         {
             require(_exists( _ids[i])== true, "ID does not exist" );
             require( NFTmapping[ _ids[i] ].owner == msg.sender, "Not owner of this NFT");
             NFTmapping[_ids[i]].forSale = false ;

            //safeTransferFrom( contractAddress, msg.sender , _ids[i] );
         }
}

// sets some NFTs for sale
function setForSale( uint256[] memory _ids) public  {
         for (uint256 i = 0; i < _ids.length; i++)
         {
             require(_exists( _ids[i])== true, "ID does not exist" );
             require( NFTmapping[ _ids[i] ].owner == msg.sender, "Not owner of this NFT");
             NFTmapping[_ids[i]].forSale = true ;
...
// if some NFTs are on sale then you can buy them here.
function buy( uint256[] memory _ids) public  payable {
         ...
         for (uint256 i = 0; i < _ids.length; i++)
         {
            ...
            uint256 to_seller = NFTmapping[_ids[i]].price - ( NFTmapping[_ids[i]].price * royalty / 100 ) ;
            total_royalty += NFTmapping[_ids[i]].price * royalty / 100 ;
            ...
            NFTmapping[_ids[i]].forSale = false ;
            NFTmapping[_ids[i]].owner = msg.sender ;
