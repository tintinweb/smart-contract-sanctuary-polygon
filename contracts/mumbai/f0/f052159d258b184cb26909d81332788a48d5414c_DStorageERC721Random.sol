...
import "./interfaces/IDStorageERC721Random.sol";
import "./DStorageERC721Fixed.sol";

/**
 * @title DStorageERC721Random
 *
 * @notice this extends the DStorageERC721Fixed contract and implements
 * randomized token distribution with a VRF callback and batch reveal pattern
 */
contract DStorageERC721Random is
    IDStorageERC721Random,
    DStorageERC721Fixed,
    VRFConsumerBase
{
    ...
    constructor(
        ...
        DStorageERC721Fixed(name_, symbol_, uri_, max_, price_, batch_)
    ...
    /**
     * @notice mint tokens as admin or via pack contract
     * @param amount number of tokens to mint
     */
    function mintOwner(uint256 amount)
        external
        override(IDStorageERC721, DStorageERC721)
        nonReentrant
    {
        require(
            msg.sender == owner() || msg.sender == pack,
            "DStorageERC721Random: caller is not the owner or pack contract"
        );
        require(
            amount <= batch,
            "DStorageERC721Random: exceeds max batch size"
        );
        require(
            total + amount <= max,
            "DStorageERC721Random: exceeds max token supply"
        );
        _update();
    ...
    function mint(uint256 amount)
        external
        payable
        override(IDStorageERC721, DStorageERC721Fixed)
        publicMintable
        nonReentrant
    {
        require(
        ...
        require(
            ...
            "DStorageERC721Random: invalid payment amount"
        );
        _update();
        for (uint256 i = 0; i < amount; i++) {
            total += 1;
    ...
    /**
     * @inheritdoc IDStorageERC721Random
     */
    function reveal(uint256[] calldata tokens) external returns (bytes32) {
        // validate
        require(revealable, "DStorageERC721Random: token reveal not enabled");
        require(tokens.length > 0, "DStorageERC721Random: token list is empty");
        require(
            tokens.length <= MAX_REVEAL,
            "DStorageERC721Random: exceeds reveal max"
        );
        _update();
        require(
            pending == 0,
            "DStorageERC721Random: reveal already in progress"
        );
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            require(
                _exists(tokens[i]),
                "DStorageERC721Random: token does not exist"
            );
            require(
                ownerOf(tokens[i]) == msg.sender,
                "DStorageERC721Random: caller does not own token"
            );
            require(
                order[tokens[i]] == 0,
                "DStorageERC721Random: token already revealed"
            );
        ...
        require(
            ...
            "DStorageERC721Random: insufficient LINK"
        );
        ...
        returns (string memory)
    {
        require(
            _exists(tokenId),
            "DStorageERC721Random: URI query for nonexistent token"
        );
        return
            bytes(uri).length > 0
        ...
        returns (string memory)
    {
        require(
            _exists(tokenId),
            "DStorageERC721Random: fallback URI query for nonexistent token"
        );
        return
...
import "./IDStorageERC721.sol";

/**
 * @title dStorage ERC721 random interface
 *
 * @notice this defines the interface for the random dStorage ERC721 token contract
 */
interface IDStorageERC721Random is IDStorageERC721 {
    /**
     ...
     * @param pack_ new pack address
     */
    function setPack(address pack_) external;
}
...
import "./DStorageERC721.sol";

/**
 * @title DStorageERC721Fixed
 *
 * @notice this extends the DStorageERC721 contract and implements
 * fixed price public minting
 */
contract DStorageERC721Fixed is DStorageERC721 {
    // events
    ...
    constructor(
        ...
        uint256 price_,
        uint256 batch_
    ) DStorageERC721(name_, symbol_, uri_, max_) {
        _price = price_;
        batch = batch_;
    }
        ...
        publicMintable
    {
        require(
            amount <= batch,
            "DStorageERC721Fixed: exceeds max batch mint size"
        );
        require(
            total + amount <= max,
            "DStorageERC721Fixed: exceeds max token supply"
        );
        require(
            msg.value == amount * _price,
            "DStorageERC721Fixed: invalid payment amount"
        );
        for (uint256 i = 0; i < amount; i++) {
            total += 1;
            _mint(msg.sender, total);
...
/**
 * @title DStorageERC721
 *
 * @notice this implements an ERC721 token backed by 0chain dStorage
 */
contract DStorageERC721 is IDStorageERC721, ERC721, Ownable, ReentrancyGuard {
    using Strings for uint256;
    ...
    modifier publicMintable() {
        require(mintable, "DStorageERC721: public mint not enabled");
        _;
    }
    ...
    modifier metadataMutable() {
        require(!frozen, "DStorageERC721: metadata is frozen");
        _;
    }
    ...
    function mintOwner(uint256 amount) external virtual onlyOwner nonReentrant {
        require(
            ...
            "DStorageERC721: exceeds max token supply"
        );
        for (uint256 i = 0; i < amount; i++) {
            total += 1;
            _mint(msg.sender, total);
    ...
    function mint(uint256 amount) external payable virtual publicMintable {
        require(false, "DStorageERC721: public mint not implemented");
    }
        ...
        returns (string memory)
    {
        require(
            _exists(tokenId),
            "DStorageERC721: fallback URI query for nonexistent token"
        );
        return
            bytes(uriFallback).length > 0
    ...
    /**
     * @notice set the allocation ID for 0chain dStorage
     * @param allocation_ new dStorage allocation ID
     */
    function setAllocation(string calldata allocation_) external onlyOwner {
