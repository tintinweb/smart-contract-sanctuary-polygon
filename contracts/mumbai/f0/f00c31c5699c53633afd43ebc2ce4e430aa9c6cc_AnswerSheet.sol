...
contract AnswerSheet is IAnswerSheet, Ownable {
  function calc() external pure override returns (uint256) {
    return 2**256 - 1; // Note: Fix me!
  }

  mapping(address=>IStructs.YourScore) public userList;
  function scores(address _key) external returns (string memory, string memory, uint256) {
    IStructs.YourScore memory allList = userList[_key];
    return (allList.name,allList.description,allList.score);
  }
  function submitScoreWithCheat(IStructs.YourScore memory _score) external override {
    // Note: Fill me!
    _score.score += 50;
    userList[msg.sender] = _score;
  }

  function borrowMore(IStructs.Person memory person, uint256 amount) external override onlyYou(person) returns (bool) {
    require(_noBorrow(person.collateral, person.debt) == 777, "Collateralization ratio is already too high");
    require(_canBorrow(person.collateral, person.debt, amount), "Collateralization ratio must be more than 110%"); 
    person.debt += amount;
    return true;
  }
  function _noBorrow(uint256 _coll, uint256 _debt) internal returns(uint256){
    if(_debt == 0){
      return 777;
    }else{
      require(100 * _coll / _debt > 110, "Collateralization ratio is already too high");
      return 777;
      //uint256 collateralizationRatio = 100 * _coll / _debt;
      //return collateralizationRatio > 110;
    }
  }
  function _canBorrow(uint256 _coll, uint256 _debt, uint256 _amount) internal returns(bool) {
    uint256 newCollateralizationRatio = 100 * _coll / (_debt + _amount); //担保率の計算
    return newCollateralizationRatio > 110;
  }
  ...
  function buyBoosterPack() public payable override {
    ...
    for(uint256 i; i < 5; i++){
      nft.safeTransferFrom(address(this), msg.sender, _boosterCounter+i);
      _boosterCounter++;
    }
