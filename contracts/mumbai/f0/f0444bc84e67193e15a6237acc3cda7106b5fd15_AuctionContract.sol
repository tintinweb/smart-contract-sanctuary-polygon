...
contract AuctionContract{
   ...
   // mapping(uint256 => minimumbid) public minbid;
    ...
    function cancelAuction_new(uint256 _tokenId, address contractAddress) public {
        ERC721 token = ERC721(contractAddress);
        require(token.ownerOf(_tokenId) == msg.sender, "Only token owner can cancel auctions");
        auctions2[_tokenId].ended = true;
    }
   ...
   
    {
        ...
        if (block.timestamp < _auction.auctionbasetime)
        {
            revert ("auction not started");
        }

       else if (block.timestamp >  _auction.auctionendtime)
        {
        ...
        if (pendingReturns_new[_auction.auctionId][msg.sender] == 0 )
        {
