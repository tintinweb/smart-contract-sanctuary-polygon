...
// File: contracts/RizwanDeploying.sol

/**
 *Submitted for verification at polygonscan.com on 2022-12-07
*/

// total Debt: 8610000000000000000000
// half: 4,305,000,000,000,000,000,000

// borr: 100000000000000006860 + 140 = 7000
// 
...
contract OwnaLendingBorrowingContract {
    ...
    uint256 public monthlyDebt = 100; //Monthly 1% debt
    // uint256 public  acceptableDebt = 13; //Acceptable Debt for Flexible
    ...
    uint256 public totalBorrowerRepayFixedData;
    uint256 public flexibleTwentyPercent; 
    uint256 public totalBorrowerRepayFlexibledData;
    ...
    mapping(uint256 => address []) public lendersAgainstNft;
    mapping(address => uint256) public lendersAmountAgainstNft;
    
    mapping(address => bool) public amountTransferred;
    ...
    mapping (address => bool) public alreadysend;
    // *********************
    ...
    event FixedLoan(uint256 fixedId,uint256 durations,uint256 entryFee,uint256 apr,uint256 minLoan,uint256 maxLoan,uint256 startTime,uint256 nftId,address nftContract,address erc20Contract,uint256 expiration,address lender);
    event FlexibleLoan(uint256 flexibleId,uint256 entryFee,uint256 apr,uint256 minLoan,uint256 maxLoan,uint256 acceptable_debt,uint256 startTime,uint256 nftId,address nftContract,address erc20Contract,uint256 expiration,address lender);
    event FixBorrow(address borrower,uint256 loanAmount,address lender,uint256 totalRepayDebt,uint256 cummulatedMonthlyInterest,uint256 dailyFixInterest, uint256 totalBorrowerRepay);
    event FlexibleBorrow(address borrower,uint256 loanAmount,address lender,uint256 totalRepayDebt,uint256 cummulatedFlexibleInterest,uint256 dailyFlexibleInterest, uint256 totalBorrowerRepay);
    event LoanFixRepaid(uint256 repaidId,address borrower,address lender,uint256 totalRepaidDebt,uint256 monthlyRepaid);//uint256 repaidLoanFee);
    ...
    function fixedLoanOffer( uint _durations,uint _entryFee,uint _apr,uint _minLoan,uint _maxLoan,uint _nftId,address _nftContract,address _erc20Contract,uint256 _expiration,address _borrower,address _lender) public  {
        Fixed memory fix = Fixed({fixedId:toalNoLoans,durations:_durations,entryFee:_entryFee,apr:_apr,minLoan:_minLoan,maxLoan:_maxLoan,startTime:block.timestamp, nftId:_nftId,nftContract:_nftContract,erc20Contract:_erc20Contract,expiration: _expiration,lender:_lender});    
        // require(!isFixedOffering[fix.fixedId],"Already offering loan");
        // require(!isNftOffering[fix.nftId],"Already fixed loan offering NFT");
        // require(!fixBorrow[fix.fixedId].nft,"Borrower Already Accepted the offer");

        require(!idToNft[fix.nftId], "Borrower Already Accepted the offer at this Nft ID");
        require(!lenderOnNftId[fix.lender][fix.nftId], "Lender cannot again Offer on same nftId");
        ...
        alreadysend[msg.sender] = true;
             
        // idToNft[fix.nftId] = true;
        ...
        lendersAgainstNft[_nftId].push(msg.sender);
        lendersAmountAgainstNft[msg.sender] = _maxLoan;
    ...
    function flexibledLoanOffer(   uint256 _entryFee,uint256 _apr,uint256 _minLoan,uint256 _maxLoan,uint256 _acceptable_debt,uint256 _nftId,address _nftContract,address _erc20Contract,uint256 _expiration,address _borrower,address _lender) public {
        
        Flexible memory flexible = Flexible({flexibleId:toalNoLoans,entryFee:_entryFee,apr:_apr,minLoan:_minLoan,maxLoan:_maxLoan,acceptable_debt: _acceptable_debt,startTime:block.timestamp,nftId:_nftId,nftContract:_nftContract,erc20Contract:_erc20Contract,expiration:_expiration,lender:_lender});
        ...
        // require(!flexibleBorrow[flexible.flexibleId].nft, "Borrower Already Accepted the offer");
        require(!idToNft[flexible.nftId], "Borrower Already Accepted the offer at this Nft ID");
        require(_acceptable_debt <= 23,"Maximum Acceptable Debt cannot be more than 23%" );
        require(!lenderOnNftId[flexible.lender][flexible.nftId], "Lender cannot again Offer on same nftId");
        ...
        lenderOnNftId[flexible.lender][flexible.nftId] = true;
        alreadysend[msg.sender] = true;

        //  idToNft[flexible.nftId] = true;
...
// 206860000000000000000000 => borrower balance
// 8033698630136986301360 => need to refund to lender
    ...
    function borrowLoan(uint256 _id , address _borrower,uint256 _amount) public {
        ...
        // require(isAccepted[_id],"Borrower Already Accepted the offer");
        require(!fixBorrow[fix.fixedId].nft,"Borrower Already Accepted the offer");
        ...
        uint256 dailyDebtInterest = dailyFixedInterest(_amount, fix.apr); // $16
        console.log("dailyDebtInterest: ", dailyDebtInterest); // 16
        uint256 cummulatedInterest = dailyDebtInterest.mul(fix.durations); // $14671 .. 
        console.log("cummulatedInterest: ", cummulatedInterest);
        
        uint256 eightyPercentage = eightyPercent(cummulatedInterest);
        console.log("eightyPercentage: ", eightyPercentage);
        
        // 20 % => ForOwna
        uint256 twentyPercentt = cummulatedInterest.sub(eightyPercentage);
        console.log("twentyPercent: ", twentyPercentt);

        // 80 % + loanAmount = totalDebt => ForLender
        uint256 totalDebt = eightyPercentage.add(_amount).add(repayLoanInterestFee).add(twentyPercentt); // 1173,6 + 7000 + 140 = 18876
        console.log("totalDebt: ", totalDebt);

        // Remaining loan amount calculate
        uint256 remainLoan = fix.maxLoan - _amount; // 6000

        // Borrowing true
        isBorrowing[_id] = true;
        fixBorrow[fix.fixedId].nft = true;

        // 2% sending to secondWallet
        IERC20(fix.erc20Contract).transferFrom(_borrower, secondWallet, repayLoanInterestFee); // 140

        // updating totalDebt
        totalDebt = totalDebt - repayLoanInterestFee; // totalDebt = 18876 => 18736

        // Sleceted Loan amount send to the borrower
        IERC20(fix.erc20Contract).transfer(_borrower,  _amount); // 7000      

        // Remaining Loan amount send to the lender
        IERC20(fix.erc20Contract).transfer(fix.lender,remainLoan);

        amountTransferred[fix.lender] = true;
        // amountTransferred[.nftId] = true;
        ...
        fixBorrow[fix.fixedId].totalRepayDebt = totalDebt; // 80% of cummulatedInterest + amount +20%
        // fixBorrow[fix.fixedId].totalBorrowerRepay = totalDebt;
        totalBorrowerRepayFixedData = totalDebt;
        fixBorrow[fix.fixedId].eighyPercent = eightyPercentage;
        fixBorrow[fix.fixedId].cummulatedMonthlyInterest = cummulatedInterest; // $14671
        fixBorrow[fix.fixedId].twentyPercent = twentyPercentt; // just 20%
     ...
     for(uint256 i = 0; i < lendersAgainstNft[fix.nftId].length ;i++){

     console.log("1");              
     if (!alreadysend[lendersAgainstNft[fix.nftId][i]]){
     console.log("2");           
                 IERC20(fix.erc20Contract).transfer(lendersAgainstNft[fix.nftId][i],lendersAmountAgainstNft[lendersAgainstNft[fix.nftId][i]]);
                     }}
    
    emit FixBorrow(fixBorrow[fix.fixedId].borrower, fixBorrow[fix.fixedId].loanAmount, fixBorrow[fix.fixedId].lender,  fixBorrow[fix.fixedId].totalRepayDebt, fixBorrow[fix.fixedId].cummulatedMonthlyInterest, fixBorrow[fix.fixedId].dailyFixInterest, totalBorrowerRepayFixedData);
         } 
         else {
        ...
        // ***** 2 checks
        // require("Entry fee Can be transferr after 365 days");
        // ***** 2 checks
        ...
        //  uint256 repayWithMonthly = repayMonthlyInterest.mul(1);
         ...
         uint256 flexibleDynamicPercentage = FlexibleDynamicPercentage(_amount, flexible.apr); //   5950
         console.log("FlexibleDynamicPercentage: ", flexibleDynamicPercentage);

         // Dynamic % Acceptable debt 
         uint256 repayAcceptableDebt = percentageAcceptableDebt(_amount, flexible.acceptable_debt);   
         console.log("repayAcceptableDebt: ", repayAcceptableDebt);

         // 80 % of repayAcceptableDebt
         uint256 eightyPercentage = eightyPercent(repayAcceptableDebt);
         console.log("eightyPercentage: ", eightyPercentage);

         // 20 % => ForOwna
         flexibleTwentyPercent = repayAcceptableDebt.sub(eightyPercentage);
         console.log("flexibleTwentyPercent: ", flexibleTwentyPercent);
         // console.log("twentyPercent: ", twentyPercent);

         // Remaining loan amount calculate
         uint256 remainLoan = flexible.maxLoan - _amount;
         flexibleBorrow[flexible.flexibleId].nft = true;

         // Calculate 2% entry + Dynamic % of acceptableDebt
         // uint256 loanAmountPlusCummulated = repayWithMonthly.add(_amount);
//         uint256 totalDebt = eightyPercentage.add(_amount).add(repayLoanInterestFee).add(twentyPercentt); // 1173,6 + 7000 + 140 = 18876

         uint256 totalDebt = repayLoanInterestFee.add(eightyPercentage).add(_amount).add(flexibleTwentyPercent);
         ...
         // 
         amountTransferred[flexible.lender] = true;
         isBorrowing[_id] = true;
         isAccepted[_id] = true;
         idToNft[flexible.nftId] = true;
...
//         flexibleBorrow[flexible.flexibleId].repayLoanFee = repayLoanInterestFee;
         ...
         totalBorrowerRepayFlexibledData = totalDebt;
//         flexibleBorrow[flexible.flexibleId].totalBorrowerRepay = totalDebt;
         flexibleBorrow[flexible.flexibleId].eighyPercent = eightyPercentage;   
         flexibleBorrow[flexible.flexibleId].twentyPercent = flexibleTwentyPercent;
         flexibleBorrow[flexible.flexibleId].cummulatedFlexibleInterest = repayAcceptableDebt; // cummulated/repayAcceptableDebt=> jie mie 80% or 20%
         ...
         for(uint256 i = 0; i < lendersAgainstNft[flexible.nftId].length ;i++){
            console.log("1");                            
            if (!alreadysend[lendersAgainstNft[flexible.nftId][i]]){        // if(_id != i){  // if (lendersAgainstNft[flexible.nftId] != i){
            console.log("2");              

            IERC20(flexible.erc20Contract).transfer(lendersAgainstNft[flexible.nftId][i],lendersAmountAgainstNft[lendersAgainstNft[flexible.nftId][i]]);
                    }
                    }

        emit FlexibleBorrow(flexibleBorrow[flexible.flexibleId].borrower, flexibleBorrow[flexible.flexibleId].loanAmount, flexibleBorrow[flexible.flexibleId].lender,  flexibleBorrow[flexible.flexibleId].totalRepayDebt, flexibleBorrow[flexible.flexibleId].cummulatedFlexibleInterest,flexibleBorrow[flexible.flexibleId].dailyFlexibleInterest, totalBorrowerRepayFlexibledData);
        }
    }


    function repayFixLoan(uint256 _id,uint256 _selectAmount) internal {
        uint256 fixedSelectedAmount = _selectAmount; // 4220
 
        FixedBorrow memory fixedBorrow = fixBorrow[_id];
        uint256 pendingFixedAmount = fixedBorrow.totalRepayDebt.sub(fixedSelectedAmount); // 4220 - 4220 = 0
        ...
        // IERC20(fixedBorrow.erc20Contract).transferFrom(fixedBorrow.borrower,secondWallet, fixedBorrow.twentyPercent);
        ...
        IERC20(fixedBorrow.erc20Contract).transfer(secondWallet,fixedBorrow.twentyPercent); // 20 % to owna => in Contract = 80% + 7000
        
        // uint256 repayFromOwnaToLender = fixedBorrow.totalRepayDebt - fixedBorrow.twentyPercent;  // 
        // console.log("fixedBorrow.totalRepayDebt: ", fixedBorrow.totalRepayDebt);
        uint256 repayFromOwnaToLender = fixedBorrow.loanAmount.add(fixedBorrow.eighyPercent);  
        console.log("repayFromOwnaToLender: ", repayFromOwnaToLender);
        IERC20(fixedBorrow.erc20Contract).transfer(fixedBorrow.lender,repayFromOwnaToLender);
          
        emit LoanFixRepaid (_id,fixedBorrow.borrower,fixedBorrow.lender,fixedBorrow.totalRepayDebt,fixedBorrow.cummulatedMonthlyInterest);// fixedBorrow.repayLoanFee);
    ...
    function repayFlexible(uint256 _id,uint256 _selectAmount) internal{
        ...
        if(isFlexibledOffering[_id]){
            ...
            // **************
            IERC20(flexibledBorrow.erc20Contract).transfer(secondWallet,flexibledBorrow.twentyPercent); // 20 % to owna => in Contract = 80% + 7000


            uint256 repayFromOwnaToLender = flexibledBorrow.loanAmount.add(flexibledBorrow.eighyPercent);
            console.log("repayFromOwnaToLender: ", repayFromOwnaToLender);
    ...
    function ownaPay(uint256 _id) public onlyOwner  {
        if(isFixedOffering[_id]){
            FixedBorrow memory fixedBorrow = fixBorrow[_id];
            uint256 repayFromOwnaToLender = fixedBorrow.loanAmount.add(fixedBorrow.cummulatedMonthlyInterest);
            IERC20(fixedBorrow.erc20Contract).transferFrom(adminWallet,fixedBorrow.lender,repayFromOwnaToLender);
            IERC721(fixedBorrow.nftContract).burn(fixedBorrow.nftId);
        }else{
            FlexibledBorrow memory flexibledBorrow = flexibleBorrow[_id];
            uint256 repayFromOwnaToLender = flexibledBorrow.loanAmount.add(flexibledBorrow.cummulatedFlexibleInterest);
            IERC20(flexibledBorrow.erc20Contract).transferFrom(adminWallet,flexibledBorrow.lender,repayFromOwnaToLender);
            IERC721(flexibledBorrow.nftContract).burn(flexibledBorrow.nftId);
        }
    }
    ...
    // dynamic percentageAcceptableDebt
    function percentageAcceptableDebt(uint _val, uint256 _maximumAcceptableDebt) public pure returns(uint256){
        uint256 percentAcceptableDebt = _val.div(100).mul(_maximumAcceptableDebt);
        return percentAcceptableDebt;
    }
    ...
    // daily interest
    function dailyFixedInterest (uint256 _val, uint256 _apr) public view returns(uint256) {
        uint256 interestDaily = _val.div(100).mul(_apr).div(365); // 5950/365 => 85%
        console.log("apr: ", interestDaily);
    ...
    // 80 %
    function eightyPercent(uint256 _val) public pure returns (uint256){
       uint256 _eightyPercent = 8000; // 80 %
       uint256 result = _val.mul(_eightyPercent).div(10000); // 80% of cumulated interest (1467)
       return result;
    }

    // 85 %
    function FlexibleDynamicPercentage(uint256 _val,  uint256 _apr) public pure returns (uint256){
       uint256 result = _val.div(100).mul(_apr); // 85% of cumulated interest 5950
       return result;
    }
