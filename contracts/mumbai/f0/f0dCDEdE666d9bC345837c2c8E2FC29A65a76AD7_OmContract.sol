...
contract OmContract is SemaphoreCore, SemaphoreGroups {
    ...
    event OmMemberAdded(uint256 indexed groupId,uint256 indexed memberID); //Ok
    event OmGroupCreated(uint256 indexed groupId,address admin); //Ok
    event ProofVerified(uint256 indexed groupId,bytes32 signal);


    event OmMemberRemoved(uint256 indexed groupId,uint256 indexed memberID);
    event VoteCast(uint256 indexed groupId, uint256 indexed proposalId, bool indexed vote); 
    event ProposalCreated(uint groupId,uint indexed proposalCounter, Proposal proposalData);
    event DataAdded(uint256 indexed dataId,dataStructure dataInfos);
...
//----------Voting
    ...
    function createProposal(
        ...
        ProposalList[proposalCounter] = Proposal(title,description,0,0,startDate,endDate,proposalURI);


        emit ProposalCreated(groupId,proposalCounter, ProposalList[proposalCounter]);
    }
    ...
    function castVote(
        ...
        _verifyProof(signal, root, nullifierHash, externalNullifierProposalId, proof, verifier);
        // _verifyProof(signal, root, nullifierHash, externalNullifier, proof, verifier);
    ...
    function addData(
        ...
        dataList[dataFileCounter] = dataStructure(title,msg.sender,block.timestamp,dataURI,fileURI,groupId,dataType,symmetricKey);


   
        emit DataAdded(dataFileCounter,dataList[dataFileCounter]);
    }
    ...
    function getProposalData(
        uint256 proposalId
    ) public view returns(Proposal memory ProposalData){
