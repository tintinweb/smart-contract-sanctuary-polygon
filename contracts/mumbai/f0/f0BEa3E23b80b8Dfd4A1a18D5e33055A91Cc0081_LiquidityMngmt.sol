...
contract LiquidityMngmt is IERC721Receiver {


    uint public positionMinted;
    uint public feeCollectionFrequency;
    ...
    function mintNewPosition(address token0, address token1, uint24 poolFee, uint256 amount0ToMint, uint256 amount1ToMint)
    external returns
         (
            uint _tokenId,
            uint128 liquidity,
        ...
        )
    {
        ...
        emit PositionMinted(_tokenId, liquidity, amount0, amount1);
    }
    ...
    function collectAllFees() external returns (uint256 amount0, uint256 amount1) {
        ...
        //send collected feedback to owner
...
// Instead of tokenaId as parameter, try liquidity
    function decreaseLiquidityInHalf(uint128 liquidity) external returns (uint amount0, uint amount1) {
        //fetch liquidity data for tokenID
        uint128 liquidity = deposits[tokenId].liquidity;
        uint128 halfLiquidity = liquidity / 2;

        //amount0Min and amount1Min are price slippage checks for the pool
        // if the amount received after burning is not greater than the minimum specified, the transaction will fail
    ...
    // Transfers funds to the owner of NFT. tokenId is the Id of the NFT
