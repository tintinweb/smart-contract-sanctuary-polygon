  ...
  DefaultOperatorFilterer
{
  ...
  constructor(
    ...
    helper = _helper;
    _tokenIdCounter = Counters.Counter(_counter);
    counterStart = _counter;
    _royaltyAmount = 500;
    maxSupply = 3000;
    price = 275000;
    price2 = 180000;
  ...
  function bridgeAirdrop(
    address[] memory addresses,
    uint256[] memory tokenIds
  ) public onlyOwner {
    require(
      addresses.length == tokenIds.length,
      "Address and tokenIds length must be the same."
    );

    for (uint256 i = 0; i < addresses.length; i++) {
      require(!_exists(tokenIds[i]), "Token already minted.");
      require(tokenIds[i] < counterStart, "TokenId not in range.");
  ...
  function mint(address _paymentTokenAddress) public payable whenNotPaused {
    ...
    FunctionInterface(helper).purchaseWithToken(
      addressMinted[msg.sender] < 1 ? price : price2,
  ...
  function tokenURI(
    ...
    return
      bytes(customBaseURI).length > 0
        ? string(abi.encodePacked(customBaseURI, Strings.toString(tokenId)))
        : "";
  }
