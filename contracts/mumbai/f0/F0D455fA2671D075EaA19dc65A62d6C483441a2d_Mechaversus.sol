...
contract Mechaversus is Initializable, OwnableUpgradeable {
    ...
    // @dev player_address => joined_match_ids[]
    mapping( address => uint256[] ) private _joinedMatch;
    ...
    // TODO implement test for the following function
    function getMatchStartBlock( uint256 match_id ) public view returns ( uint256 ) {
        return _matches[ match_id ]._start_block;
    }

    function getMatchBlockToRespond( uint256 match_id ) public view returns ( uint256 ) {
        return _matches[ match_id ]._block_to_respond;
    }
    ...
    function getMatchJoined( address player ) public view returns ( uint256[] memory ) {
        return _joinedMatch[ player ];
    }
    // TODO reimplement test for the following function
    function getMatchActivePlayers( uint256 match_id ) public view returns ( address[] memory ) {
        address[] memory emptyAddresses;
        return _matches[ match_id ]._players.length == PLAYERS_COUNT
        ? _matches[ match_id ]._active_players : emptyAddresses;
    }
    ...
    function getMatchPlayers( uint256 match_id ) public view returns ( address[] memory ) {
        address[] memory emptyAddresses;
        return _matches[ match_id ]._players.length == PLAYERS_COUNT
        ? _matches[ match_id ]._players : emptyAddresses;
    }

    // TODO implement test for the following function
    function getMatchPlayersCount( uint256 match_id ) public view returns ( uint256 ) {
        return _matches[ match_id ]._players.length;
    }
    ...
    // TODO implement block_to_respond logic
    ...
    function joinMatch( uint256 match_id, uint256 deck_id ) public virtual {
        ...
        _joinedMatch[ msg.sender ].push( match_id );
    ...
    function leaveMatch( uint256 match_id ) public virtual callerIsPlayer(match_id){
        ...
        for( uint256 i = 0; i < _matches[ match_id ]._players.length; i++ ){
            if( _matches[ match_id ]._players[i] == msg.sender ) {
                _matches[ match_id ]._active_players = Arrays.popAddress( _matches[ match_id ]._active_players, msg.sender );
                _matches[ match_id ]._players = Arrays.popAddress( _matches[ match_id ]._players, msg.sender );
                _joinedMatch[ msg.sender ] = Arrays.popUint256( _joinedMatch[ msg.sender ], match_id );
    ...
    function endMatchConditionsChecks( uint256 match_id ) public virtual callerIsPlayer(match_id) {
        ...
        for( uint256 i = 0; i < _matches[ match_id ]._active_players.length; i++){
            ...
            if( _entities[ match_id ][ nexus_id ] == uint16(0) ){
                _loseMatch( match_id, _matches[ match_id ]._active_players[ idx ] );
                foundedLoser = true;
            }
            else{
                ...
                if( allMechaBroken ){
                    _loseMatch( match_id, _matches[ match_id ]._players[ idx ] );
                    foundedLoser = true;
                }
            }
        }

        require( foundedLoser , "No end match condition triggered on players" );
    }
    ...
    function _endMatch( uint256 match_id, address winner ) internal virtual {
        ...
        for( uint256 i = 0; i < _matches[ match_id ]._decks.length; i++ ) {
            _joinedMatch[ _matches[ match_id ]._players[i] ] = Arrays.popUint256( _joinedMatch[ _matches[ match_id ]._players[i] ], match_id );
            _usedDeck[ _matches[ match_id ]._decks[i] ] = false;
        }
    ...
    function _initEntities( uint256 match_id ) internal {
        Decks _Decks = _Arenas._Decks();

        // All mechas init
        for( uint256 i = 0; i < _matches[ match_id ]._performers_order.length; i++){
            _entities[ match_id ][ _matches[ match_id ]._performers_order[i] ] = 1;
        }
        // All nexus init
        for( uint256 i = 0; i < _matches[ match_id ]._decks.length; i++){
            uint256 nexus_id = _Decks.getNexusId( _matches[ match_id ]._decks[i] );
            _entities[ match_id ][ nexus_id ] = 1;
        }
    }
}
...
contract Arenas is Initializable, OwnableUpgradeable {
    ...
    // TODO implement a generate random uint128 with 116 max valorized bits
