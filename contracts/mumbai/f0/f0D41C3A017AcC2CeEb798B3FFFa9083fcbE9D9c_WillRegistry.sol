...
interface WillRegistryInterface {
  function register(
...
contract WillRegistry is WillRegistryInterface {
  struct Testament {
    address testator;
  ...
  onlyNotActivatedYet(_willAddress)
  {
    ...
    if (registry[_willAddress].testator != _testator) {
      registry[_willAddress].testator = _testator;
    }

    if (msg.value > 0) {
      registry[_willAddress].money += msg.value;
    }
