...
contract USDC is Context, IERC20, Ownable {
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public returns (bool) {
     _approve(_msgsender(), spender, _allowances[_msgsender()][spender].sub(subtractedValue, "CUSD: decreased allowance below zero"));
     return true;
    }
    ...
    function _mint(address account, uint256 amount) internal {
        require(account != address(0), "CUSD: mint to the zero address");
    ...
    function _burn(address account, uint256 amount) internal {
       require(account != address(0), " CUSD: burn from the zero address");
       _balances[account] = _balances[account].sub(amount, " CUSD: burn amount exceeds balance");
