...
contract IseedWhitelist is Ownable, ERC721 {
    using Address for address;
    using Strings for uint256;
    ...
    uint256 public startingId = 1000;

    string [] public tokenTypes = ["galaxy", "elite"]; //Token URI
    ...
    //Infinity pass
    IERC721 public infinityPass;

    mapping(uint256 => bool) public redeemedToken;
    uint256 public totalSupplyForPass = 0;
    ...
    constructor (
        ...
        // instanciate infinity pass
        require(address(_infinityPass) != address(0), "Invalid infinity pass address: zero address");
    ...
    function mint(string memory _community, uint256 _amount) public payable {
        ...
        // promo not terminated AND min amount for the promo reached AND there are enough tokens
        if(block.timestamp <= promoStartedAt + promoDuration && _amount >= minAmountForThePromo && _amount + amountGifted <= maxSupply) {
    ...
    function redeemPassWhitelist(uint256 [] memory _tokens) public {
        // user must have pass
        uint256 _length = _tokens.length;
        require(_length > 0, "Insert tokens");

        for(uint256 i = 0; i < _length;) {
            require(infinityPass.ownerOf(_tokens[i]) == msg.sender, "You do not own all the tokens entered");
            unchecked {i++;}
        }
        ...
        for(uint256 i = 0; i < _length;) {
            ...
            if (!redeemedToken[_id]) {
                totalSupplyForPass += 1;
                _safeMint(msg.sender, totalSupplyForPass);

                redeemedToken[_id] = true;
            }
    ...
    function mintToUsersWithPreorder() internal {
        ...
        for(uint256 i = 0; i < _length;) {
            for(uint256 j = 0; j < amountForUserWithPreorder[i];) {
                totalSupply += 1;
                _id = startingId + totalSupply;
                _safeMint(usersWithPreordainedItem[i], _id);
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        if (_tokenId <= 1000) {
            return string(abi.encodePacked(baseUri, tokenTypes[0], baseExtension));
        } else {
            return string(abi.encodePacked(baseUri, tokenTypes[1], baseExtension));
        }
    }
    ...
    function getUserWallet (address _user) public view returns(uint256 [] memory) {
        ...
        // elite token
        for (uint256 _tokenId = startingId + 1; _tokenId <= totalSupply + startingId; _tokenId ++) {
        ...
        // galaxy token
        for (uint256 _tokenId = 1; _tokenId <= totalSupplyForPass; _tokenId ++) {
            if (ownerOf(_tokenId) == _user) {
                _tokensAmount += 1;
            }
        }
    ...
    function getRewardForWhitelist() public view returns(uint256, uint256) {
    ...
    function setTokenTypes(string [] memory _newTypes) public onlyOwner {
        tokenTypes = _newTypes;
    }
