    ...
    Blacklist
{
    ...
    function initialize() public initializer {
        __ERC20_init("KicheeToken", "KTK");
        __ERC20Burnable_init();
        __Pausable_init();
        __AccessControl_init();
        __ERC20Permit_init("KicheeToken");
    ...
    function batchMint(address[] memory toAddress, uint256[] memory amount) public virtual onlyRole(MINTER_ROLE) {
        for (uint256 i = 0; i < toAddress.length; i++) {
            require(amount[i] > 0, "no exp");
            _mint(toAddress[i], amount[i]);
        }
    }
    ...
    function _beforeTokenTransfer(
        ...
        require(!isBlacklist(msg.sender), "KicheeToken: caller in blacklist can't transferFrom");
        require(!isBlacklist(from), "KicheeToken: from in blacklist can't transfer");
        require(!isBlacklist(to), "KicheeToken: not allow to transfer to recipient address in blacklist");
