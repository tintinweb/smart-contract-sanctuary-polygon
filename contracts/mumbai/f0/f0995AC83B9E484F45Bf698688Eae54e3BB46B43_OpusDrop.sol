...
abstract contract Drop is IDrop {
    ...
    function claim(
        ...
        claim(_receiver, _quantity, _currency, _pricePerToken, activeConditionId, _allowlistProof, _data);
    }
    ...
    /// @dev set active claim condition
    function setActiveClaimConditionId(uint256 _conditionId) public returns (uint256) {
        /* for (uint256 i = claimCondition.currentStartId + claimCondition.count; i > claimCondition.currentStartId; i--) {
            if (block.timestamp >= claimCondition.conditions[i - 1].startTimestamp) {
                return i - 1;
            }
        } */
        claimCondition.conditions[_conditionId].startTimestamp++;
        revert("!ACTIVE.");
    }
...
contract OpusDrop is ERC721Drop {
    constructor(
        string memory _name,
        string memory _symbol,
        address _royaltyRecipient,
