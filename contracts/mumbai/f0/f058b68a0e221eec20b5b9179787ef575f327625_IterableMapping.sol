...
contract IterableMapping {
  ...
  function comprarMultiplesTickets(uint[] memory numeros) public payable{
    uint len=numeros.length;
    require(msg.value==(PRICE*len), "Incorrect Amount");
    for (uint i = 0; i < len; i++) {
      sistema[1].keys.push(numeros[i]);
      sistema[1].values[numeros[i]] = msg.sender;
    }
  }
  ...
  function retirar(uint time) public{
    ...
    if(a > 0){
      ...
      if(pagogerdedor>0){
        monto = pagogerdedor*userNum.length;
      }
    }
    if(monto>0){
  ...
  function generar(uint time) public{
    uint win = 10;
    tiempos.push(time);
    premios[time]=win;
  }
