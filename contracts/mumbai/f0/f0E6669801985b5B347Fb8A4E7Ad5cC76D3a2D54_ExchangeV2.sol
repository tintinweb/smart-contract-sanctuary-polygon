...
library LibMath {
    ...
    function safeGetPartialAmountFloor(
        ...
        console.log("Num, denom, target %s %s %s", numerator, denominator, target);
...
library LibOrder {
    ...
    function calculateRemaining(Order memory order, uint fill, bool isMakeFill) internal view returns (uint makeValue, uint takeValue) {
        console.log("oreder make i fill %s %s",order.makeAsset.value, fill );
        if (isMakeFill){
            makeValue = order.makeAsset.value;
            console.log("Transfering %s %s %s", order.takeAsset.value, order.makeAsset.value, makeValue );
            takeValue = LibMath.safeGetPartialAmountFloor(order.takeAsset.value, order.makeAsset.value, makeValue);
        } else {
            takeValue = order.takeAsset.value.sub(fill);
            console.log("Take value is %s %s %s", order.takeAsset.value, takeValue );
...
library LibFill {
     ...
     */
    function fillOrder(LibOrder.Order memory leftOrder, LibOrder.Order memory rightOrder, uint leftOrderFill, uint rightOrderFill, bool leftIsMakeFill, bool rightIsMakeFill) internal view returns (FillResult memory) {
        
        console.log("Fill is", leftOrderFill, rightOrderFill);
    ...
    function fillRight(uint leftMakeValue, uint leftTakeValue, uint rightMakeValue, uint rightTakeValue) internal view returns (FillResult memory result) {
        console.log("in fill right %s %s %s", rightTakeValue, leftMakeValue, leftTakeValue);
    ...
    function fillLeft(uint leftMakeValue, uint leftTakeValue, uint rightMakeValue, uint rightTakeValue) internal view returns (FillResult memory result) {
        console.log("in fill left %s %s %s", leftTakeValue, rightMakeValue, rightTakeValue);
