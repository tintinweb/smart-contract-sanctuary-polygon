...
contract Cva is ERC165, ICva, Pausable {
  ...
  function decodeDepositData(
    bytes memory _depositData
  ) internal pure returns (string memory sender, address recipient, uint8 srcTokenDecimals, uint256 amount) {
    (sender, recipient, srcTokenDecimals, amount) = abi.decode(_depositData, (string, address, uint8, uint256));
  }
...
interface ICvaFactory {
  function cvaAddress() external view returns (address);
  ...
  function isCvaDeployed(address) external view returns (bool);

  function getCva(bytes calldata, bytes calldata) external view returns (address);

  function hasRole(bytes32 role, address account) external view returns (bool);

  function isValid() external view returns (bool);

  function createCva(bytes calldata, bytes calldata) external;
