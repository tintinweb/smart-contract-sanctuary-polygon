...
contract PolyBuildings is Ownable, ERC1155 {
  ...
  function uri(uint256 tokenId) public view virtual override returns (string memory) {
    bytes memory json = abi.encodePacked(
      '{"name": "',
      names[tokenId],
      '","description": "Autumn has come to the farmlands, we can see the beauty in the color of the trees and the falling leaves. But all suddenly Farmers appeared, they have an intriguing look but are the perfect addition to run your Farm, where do they come from is still a mistery...","image": "data:image/svg+xml;base64,',
  ...
  function setTokenMetadata(uint256 tokenId, string memory name, string memory image) public onlyOwner {
    names[tokenId] = name;
