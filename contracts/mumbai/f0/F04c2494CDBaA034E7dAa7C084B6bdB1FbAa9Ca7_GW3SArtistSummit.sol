...
contract GW3SArtistSummit is ERC721Enumerable, Ownable {
    ...
    function getSvg(uint256 tokenId) public view returns(string memory _svgFront, string memory _svgBack, string memory _svgLeft, string memory _svgRight) {
        address front = metadatas[tokenId].svg.front;
        address back = metadatas[tokenId].svg.back;
        address left = metadatas[tokenId].svg.left;
        address right = metadatas[tokenId].svg.right;

        _svgFront = string(front.code);
        _svgBack = string(back.code);
        _svgLeft = string(left.code);
        _svgRight = string(right.code);
    }
