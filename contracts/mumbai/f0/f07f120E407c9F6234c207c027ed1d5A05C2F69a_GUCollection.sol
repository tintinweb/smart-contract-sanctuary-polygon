...
contract GUCollection is ERC721, Ownable, IERC2981 {
    using Strings for uint256;
    ...
    function _beforeTokenTransfer(
        ...
        if(stakes[msg.sender].tokenId.length > 0) {
            require(from == address(0) || to == address(0), "Nft staked");
        }
    ...
    function stake(uint256[] calldata _tokenIds) external {
        //require(ownerOf(_tokenIds[0]) == msg.sender, "Not token owner");
        require(_tokenIds.length > 0, "Cannot stake 0 nft");
        stakes[msg.sender].tokenId = _tokenIds;
    ...
    function unStake(uint256[] calldata _tokenIds) external {
        ...
        require(_tokenIds.length > 0, "Cannot unstake 0 nft");
