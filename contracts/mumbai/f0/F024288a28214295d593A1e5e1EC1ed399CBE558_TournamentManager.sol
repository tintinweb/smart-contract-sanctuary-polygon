...
contract TournamentManager is Ownable {
    ...
    ) public isExist(id) isValidCreator {
        require(
            tournaments[id].creator == _msgSender(),
            "Error, not the creator"
        );
    ...
    function kick(
        ...
        address user,
        bytes32 reason
    ) public isValidCreator isExist(id) {
        require(
            tournaments[id].creator == _msgSender(),
            "Error, not the creator"
        );
    ...
    ) external payable isValidCreator isExist(id) {
    ...
    function close(
        ...
        for (uint256 index = 0; index < winners.length; index++) {
            ...
            bool isSuccess = IERC20(tournaments[id].joinToken).transferFrom(
                _msgSender(),
                winners[index],
                distribAmount
            );
