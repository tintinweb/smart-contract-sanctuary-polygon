...
struct StoredData {
    ...
    uint256 duration;
    uint256[] auctionIds;
    mapping(uint256 => Auction) auctions; // tokenId => Player
    mapping(uint256 => Player) players; // tokenId => Player
}

contract PonziBoss is
    ...
    ERC721HolderUpgradeable
{
    ...
    function initialize() public initializer {
        ...
        __ERC721_init("PB", "P_B");
        __Ownable_init();
        __UUPSUpgradeable_init();
        _baseURIString = "https://tableland.xyz/";
        stored.bossTokenId = 0;
        stored.baseScore = 2;
        stored.duration = 6000;
        stored.price = 1e14; // 1 ether
        stored.lowPrice = 1e15; // 0.001 eth
        _spawnPlayers();
    }

    function _spawnPlayers() private {
        for (uint i=0; i<3; i+=1) {
            uint256 tokenId = safeMint(address(this));
            stored.auctions[tokenId].startPrice = stored.price;
            stored.auctions[tokenId].startBlock = block.number;
            stored.auctionIds.push(tokenId);
            stored.players[tokenId].score = stored.baseScore;
            stored.players[tokenId].block = block.number;
        }
        stored.points += 10 * stored.baseScore;
        if (stored.price < 1e21) {
            stored.price = stored.price * 2;
            stored.duration += 250;
        }
    }

    function getScore(uint256 tokenId) public view returns (uint256 score) {
        return stored.players[tokenId].score;
    }
    
    function getPrice(uint256 tokenId) public view returns (uint256) {
        uint256 remainder = getRemainder(tokenId);
        if (remainder <= 0) {
            return 0;
        }
        uint256 price = (stored.auctions[tokenId].startPrice * remainder) / stored.duration;
        if (price <= stored.lowPrice) {
            return 0;
        }
        return price;
    }

    function getRemainder(uint256 tokenId) public view returns (uint256 blocks) {
        return stored.duration - getElapsed(tokenId);
    }

    function getElapsed(uint256 tokenId) public view returns (uint256 blocks) {
        uint256 timeElapsed = uint32(block.number) - stored.auctions[tokenId].startBlock;
        return timeElapsed;
    }

    function getBoss() public view returns (uint256 tokenId) {
        return stored.bossTokenId;
    }

    function getAuctions() public view returns (uint256[] memory) {
        return stored.auctionIds;
    }

    function _cancelAuction(uint256 tokenId) private {
        uint256 index = stored.auctionIds.length;
        for (uint256 i = 0; i < index; i+=1) {
            if (stored.auctionIds[i] == tokenId) {
                index = i;
            }
        }
        if (0 <= index) {
            stored.auctionIds[index] = stored.auctionIds[stored.auctionIds.length - 1];
            stored.auctionIds.pop();
        }
        delete stored.auctionIds[tokenId];
    }
    ...
    function buy(uint256 tokenId) external payable {
        ...
        super._safeTransfer(ownerOf(tokenId), msg.sender, tokenId, "");

        _cancelAuction(tokenId);
    }

    function _kill(uint256 tokenId) private {
        stored.points -= stored.players[tokenId].score;
        delete stored.players[tokenId];
        super._burn(tokenId);
    }

    function _checkKill(uint256 tokenId) private {
        if (stored.players[tokenId].score < 2) {
            if (0 < stored.players[tokenId].score) {
                stored.points -= stored.players[tokenId].score;
            }
            delete stored.players[tokenId];
            _kill(tokenId);
    ...
    function _checkBoss(uint256 loser, uint256 winner) private {
        if (loser == stored.bossTokenId) {
            _kill(stored.bossTokenId);
            stored.bossTokenId = winner;
            _spawnPlayers();
        } else if (winner == stored.bossTokenId) {
            address payable boss = payable(ownerOf(stored.bossTokenId));
            if(boss.send(0)) {
                uint256 winnings = address(this).balance * stored.players[stored.bossTokenId].score / stored.points;
                boss.transfer(winnings);
            }
        }
    }

    function _payWinner(uint256 loser, uint256 winner, uint256 points) private {
        stored.players[winner].score += points;
        stored.players[loser].score -= points;
    }

    function challenge(uint256 tokenId, uint256 opponentTokenId) public {
        require(_isApprovedOrOwner(_msgSender(), tokenId), "ERC721: caller is not token owner or approved");
        _requireMinted(tokenId);
        _requireMinted(opponentTokenId);
        Player memory challenger = stored.players[tokenId];
        Player memory player2 = stored.players[opponentTokenId];

        // Abort the game if either player has already played in the current block
        require(challenger.block < block.number, "Challenger has already played in the current block.");
        require(player2.block < block.number, "Player 2 has already played in the current block.");

        // Calculate the levels of the players
        uint256 level1 = level(tokenId);
        uint256 level2 = level(opponentTokenId);

        // Calculate the probability of challenger winning as a value between 0 and 1
        uint256 probability1 = (level1 * 10000) / (level1 + level2);

        // Generate a random number between 0 and 10000
        uint256 roll = 0; // always win
        // uint256 roll = random() * 10000;

        // Determine the winner based on the roll and the probability
        if (roll < probability1) { // Challenger wins!
            _payWinner(opponentTokenId, tokenId, 1 + (player2.score / 2));
            _checkKill(opponentTokenId);
            _checkBoss(opponentTokenId, tokenId);
        } else { // Player2 Wins!
            _payWinner(tokenId, opponentTokenId, 1 + (challenger.score / 4));
            _checkKill(tokenId);
            _checkBoss(tokenId, opponentTokenId);
        }
        // Mark both players as having played in the current block
        stored.players[tokenId].block = block.number;
        stored.players[opponentTokenId].block = block.number;
    }

    // Calculates a player's level based on their score
    function level(uint256 tokenId) public view returns (uint256) {
        // Calculate the percentage of the total game score that the player's score represents
        uint256 percentage = (stored.players[tokenId].score * 100) / stored.points * 10000;

        // Map the percentage to a level between 1 and 10
        if (99 >= percentage) {
            return 10;
        } else if (97 >= percentage) {
            return 9;
        } else if (95 >= percentage) {
            return 8;
        } else if (93 >= percentage) {
            return 7;
        } else if (89 >= percentage) {
            return 6;
        } else if (82 >= percentage) {
            return 5;
        } else if (71 >= percentage) {
            return 4;
        } else if (53 >= percentage) {
            return 3;
        ...
        return
            string.concat(
                ...
                string.concat(
                    '{"name":"',
                    strTokenId,
                    '","image":"https://download.vikidia.org/vikidia/fr/images/thumb/c/c9/CC0.svg/768px-CC0.svg.png","attributes":[{"trait_type":"last_played","value":"',
                    StringsUpgradeable.toString(stored.players[tokenId].block),
                    '"},{"trait_type":"score","value":"',
                    StringsUpgradeable.toString(stored.players[tokenId].score),
                    '"}]}'
                )
            );
    }
