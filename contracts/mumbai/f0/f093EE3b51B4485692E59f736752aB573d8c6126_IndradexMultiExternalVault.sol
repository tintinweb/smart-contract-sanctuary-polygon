...
abstract contract IndradexBaseVault is Ownable, ERC20 {
  ...
  /**
    ...
    */
  function vaultTotalValue() public view returns(uint256){
  ...
  */
    function scaledVaultTotalValue(uint256 amount) public view returns(uint256){
      return amount.mul(vaultTotalValue()).div(totalSupply());
  }

  /**
  * @dev returns the scaled tot balance for every tokens
  */
  function scaledTotalBalances(uint256 amount) external view returns(TokenTotalBalance[] memory){
  ...
  function deposit(uint256 amount) external {
    ...
    uint256 totalSupply = totalSupply();
    uint256 vaultTotalValue_ = vaultTotalValue();
    if(_maxCapDeposit > 0){
