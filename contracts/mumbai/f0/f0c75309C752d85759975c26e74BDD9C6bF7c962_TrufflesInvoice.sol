...
contract TrufflesInvoice is ERC721 {
    using Strings for uint256;
    ...
    ) public isNotSettled(params._tokenId) {
        require(!isNFTMinted(params._tokenId), "Token already minted.");
    ...
    function settleNft(
        ...
        uint256 _blockNumber
    ) public isNotSettled(_tokenId) {
        require(_exists(_tokenId), "Token doesn't exist.");
        require(
            ownerOf(_tokenId) == msg.sender,
            "Only the owner can update the NFT."
        );

        _tokenURIs[_tokenId]._invoiceStatus = _invoiceStatus;
        _tokenURIs[_tokenId]._blockNumber = _blockNumber;
        _tokenURIs[_tokenId]._isSettled = true;
    ...
    modifier isNotSettled(uint256 tokenId) {
        require(!_tokenURIs[tokenId]._isSettled, "Token is already settled.");
        _;
    }
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                '{"name":" Token ID #',
                                tokenId.toString(),
                                '", "description":"Truffles Invoice NFT", "image": "data:image/svg+xml;base64,',
                                image,
                                '"}'
                            )
                        )
    ...
    function createSVGParams(
        ...
        return
            NFTSVG.generateSVG(
                NFTSVG.SVGParams({
                    merkleProof: _tokenURIs[tokenId]._merkleProof,
                    userAddress: toAsciiString(ownerOf(tokenId)),
                    tokenId: tokenId,
                    invoiceID: _tokenURIs[tokenId]._invoiceNumber,
                    invoiceAmount: _tokenURIs[tokenId]._invoiceAmount.toString(),
    ...
    function _burn(uint256 tokenId) internal virtual override {
        ...
        uint256[] storage ownedTokens = _addressTokenId[owner];
        uint len = ownedTokens.length;
        for (uint i = 0; i < len; ) {
...
library NFTSVG {
        ...
        svg = string(
            abi.encodePacked(
                ...
                Base64.encode(
                    bytes(
                        abi.encodePacked(
                            "<svg width='290' height='500' viewBox='0 0 290 500' xmlns='http://www.w3.org/2000/svg'><rect width='290px' height='500px' fill='#152906'/></svg>"
                        )
                    )
                ),
                ...
                Base64.encode(
                    bytes(
                        abi.encodePacked(
                            "<svg width='290' height='500' viewBox='0 0 290 500' xmlns='http://www.w3.org/2000/svg'><circle cx='228' cy='122' r='120px' fill='#c02aaa'/></svg>"
                        )
                    )
                ),
                ...
                Base64.encode(
                    bytes(
                        abi.encodePacked(
                            "<svg width='290' height='500' viewBox='0 0 290 500' xmlns='http://www.w3.org/2000/svg'><circle cx='182' cy='145' r='100px' fill='#756cc2'/></svg>"
                        )
                    )
                ),
    ...
    function generateInvoiceDetails(
        ...
        string memory invoiceAmount,
        string memory invoiceCurrency,
        string memory invoiceStatus
    ) private pure returns (string memory svg) {
        uint256 str1length = bytes(tokenId).length + 4;
        uint256 str2length = bytes(invoiceID).length + 10;
        uint256 str3length = bytes(invoiceAmount).length + 10;
        uint256 str4length = bytes(invoiceCurrency).length + 10;
        uint256 str5length = bytes(invoiceStatus).length + 10;
        svg = string(
            abi.encodePacked(
                ' <g style="transform:translate(29px, 304px)">',
                '<rect width="',
                ...
                invoiceCurrency,
                "</text></g>",
                ' <g style="transform:translate(29px, 464px)">',
                '<rect width="',
