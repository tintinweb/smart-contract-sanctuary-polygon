...
contract MarketTestWert is AccessControl, ReentrancyGuard {
    ...
    uint256 private fee = 100; //100 is equal to 1%
    bool paused = false;
    ...
    constructor(address token, address _mainVaultAddress, address _walletFee)  {
        ...
        if(token != address(0)){
        permissions(token,_mainVaultAddress);
        }else{
            revert ("address Zero");
        }
    } 

    function buy(address _owner, uint256 _quantity, address _token) public isPaused nonReentrant {
        if(ICollectionNFT(Collection).getPresaleStatus()){
            revert ("Presale Open");
        }
        LocalMint memory locals;
        locals._price = ICollectionNFT(Collection).getPrice();
        ...
        IERC20(_token).safeTransfer(walletFee, locals.pan);
        IVaultFounders(mainVaultAddress).deposit(locals.sell, _token);
        

        for(uint256 i=1; i <= _quantity;){
            locals._nftId = ICollectionNFT(Collection).redeem(_owner, locals._price);
    
            emit NFTMinted(Collection, locals._nftId, _owner);
            unchecked {
             ++i;
            }
        }
    ...
    function calcFees(uint256 _fee) internal view returns(uint256 panoram){
        panoram = _getFee(_fee, fee);
        return panoram;
    }


    function updateFee(uint256 _newfee) public onlydev{
            fee = _newfee;
    }
    ...
    function updatePaused(bool _Status) public onlydev{
     ...
     function updateFeeWallet(address _newWalletFee) public onlydev{
            walletFee = _newWalletFee;
    }

    function setCollection(address _Collection) public onlydev{
        Collection = _Collection;
    }

    function permissions(address _token, address _mainVault) public onlydev {
        IERC20(_token).approve(_mainVault, 2**255);
    }

 

    function _getFee(uint256 _totalBid, uint256 _percentage) internal pure returns (uint256){
