    ...
    BaseRelayRecipient
{
    ...
    uint256 public cost;
    uint256 public generalNoOfFreeMints;
    ...
    function initialize(
        ...
        createrAddress = _msgSender();
        nextTokenId = 1;
        cost = 0.0001 ether;
        generalNoOfFreeMints = 10;
    ...
    mapping(uint256 => AlterBodyDetails) NFTLogs;

    mapping(address => uint256) public noOfFreeMintsPerWallet;

    mapping(address => bool) public freeMintWallet;

    event Mint(uint256 tokenId, address tokenOwner, AlterBodyDetails newNFT);

    event MintBatch(
        uint256 tokenId,
        address tokenOwner,
        AlterBodyDetails newNFT
    );
    ...
    modifier mintComplianceCostCheck() {
    ...
    function minter(
        ...
        AlterBodyDetails memory newNft = AlterBodyDetails({
            tokenID: nextTokenId,
            tokenURI: newURI,
            creator: _creator,
            owner: _creator,
        ...
        emit Mint(nextTokenId, _creator, newNft);
        nextTokenId++;
        tokensInCirculation++;
    }
    ...
    function mintBody(
        ...
        string memory tokenURI,
        uint256 _royalty
    ) public payable virtual mintCompliancePauseCheck {
        if (freeMintWallet[_msgSender()] == false) {
            freeMintWallet[_msgSender()] = true;
            noOfFreeMintsPerWallet[_msgSender()] = generalNoOfFreeMints;
            minter(tokenURI, _royalty, _msgSender(), mintAmount);
        } else {
            if (noOfFreeMintsPerWallet[_msgSender()] != 0) {
                minter(tokenURI, _royalty, _msgSender(), mintAmount);
                noOfFreeMintsPerWallet[_msgSender()] -= 1;
            } else {
                require(msg.value >= cost, "Not enough balance");
                minter(tokenURI, _royalty, msg.sender, mintAmount);
            }
        }
    }
    ...
    // Free Mints
    function setGeneralNoOfFreeMints(uint256 _freeMints) public virtual {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, msg.sender),
            "AlterBody: Must have admin role to change"
        );
        generalNoOfFreeMints = _freeMints;
    }
    ...
    // func to check free mint availability for a wallet
    function CheckfreeMintWallet(address _address) public view returns (bool) {
        return freeMintWallet[_address];
    }

    // func to check free mint availability for a wallet
    function CheckAvatarCreator(uint256 _id) public view returns (address) {
        return NFTLogs[_id].creator;
    }

    // func to check free mints available per wallet
    function CheckFreeMintsAvailablePerWallet(address _address)
        public
        view
        returns (uint256)
    {
        return noOfFreeMintsPerWallet[_address];
    }

    function getNFTDetails(uint256 id)
        external
        view
        returns (AlterBodyDetails memory)
    {
        return NFTLogs[id];
    }
