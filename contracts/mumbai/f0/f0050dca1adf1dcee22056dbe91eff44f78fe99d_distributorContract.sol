...
contract distributorContract is Ownable, ReentrancyGuard {
        ...
        function deposit (uint poolId,uint tokenAmount) external nonReentrant {
            ...
            //   token .transferFrom(msg.sender, address(this), treasury);
            //    token .transferFrom(msg.sender, address(this), rewards);
            //    token .transferFrom(msg.sender, address(this), marketing);
      ...
      function investBack(uint poolId, uint _nftId) external{
        ...
        function unstakeTokens (uint[] memory poolIds, uint[] memory _nftIds) external nonReentrant {
                ...
                for (uint i=0; i< poolIds.length; i++) {
                        ...
                        // token.transfer(insuranceFundAddress, insurance);
                        }
