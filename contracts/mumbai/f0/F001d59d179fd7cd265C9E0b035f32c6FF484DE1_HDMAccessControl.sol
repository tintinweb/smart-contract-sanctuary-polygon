...
contract HDMAccessControl is AccessControl {
    ...
    function grantPermissions(
        ...
        DataPermissions memory perms = permissions.push();
        perms.hash = _hash;
    ...
    function grantPermissionsFor(
        ...
        require(
            _owner != msg.sender, 
            "HDMAccessControl: user cannot self-grant permissions for data of their own."
        );

        DataPermissions memory parentPerms = permissions[_parentIndex];

        require(
            !parentPerms.isRevoked, 
            "HDMAccessControl: data had been revoked already."
        );

        require(
            parentPerms.hash != _hash, 
            "HDMAccessControl: new hash cannot reference itself."
        );

        require(
            parentPerms.owner == _owner, 
            "HDMAccessControl: data is not owned by the specified owner."
        );

        require(
            parentPerms.user == msg.sender, 
            "HDMAccessControl: data is not provided to the user granting permissions."
        );
        ...
        perms.hash = _hash;
        perms.owner = _owner;
        perms.parentHash = parentPerms.hash;
        perms.user = msg.sender;
        perms.isRevoked = false;
        perms.expiresAt = _expiresIn == 0 ? 0 : uint48(block.timestamp) + _expiresIn;

        permissionsByOwners[_owner].push(index);
    ...
    function checkPermissions(uint256 _hash)
        internal
        view
        returns (bool)
    {
        ...
        for (uint256 i = 0; i < indices.length; i++) {
            ...
            DataPermissions memory perms = permissions[index];
            if (perms.isRevoked)
                continue;
            ...
            if (perms.parentHash != 0x0) {
                return checkPermissions(perms.parentHash);
            } else {
                return true;
            }
        }
        ...
        returns (DataPermissions[] memory)
    {
        ...
        DataPermissions[] memory filtered = new DataPermissions[](indices.length);
        for (uint256 i = 0; i < indices.length; i++) {
            uint256 index = indices[i];
            DataPermissions memory perms = permissions[index];
            if (!checkPermissions(perms.hash))
                continue;
