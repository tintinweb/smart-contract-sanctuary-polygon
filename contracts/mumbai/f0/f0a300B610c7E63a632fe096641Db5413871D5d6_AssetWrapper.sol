...
import "../interfaces/core/IAssetWrapper.sol";
 ...
 * underlying assets.
 */
contract AssetWrapper is Context, ERC721Burnable, ERC1155Holder, ERC721Holder, IAssetWrapper {
    ...
    mapping(uint256 => bytes) public bundleMetadata;

    function getHoldings(uint256 tokenId)
        external
        view
        returns (
            ERC20Holding[] memory,
            ERC721Holding[] memory,
            ERC1155Holding[] memory,
            uint256
        )
    {
        return (
            bundleERC20Holdings[tokenId],
            bundleERC721Holdings[tokenId],
            bundleERC1155Holdings[tokenId],
            bundleETHHoldings[tokenId]
        );
    }
     ...
     */
    function initializeBundle(address to, bytes memory metadata) public override returns (uint256) {
        uint256 id = tokenIdTracker.current();
        _mint(to, id);
        tokenIdTracker.increment();
        bundleMetadata[id] = metadata;

        emit CreatedMetadata(metadata, id);
    ...
    function depositERC20(
        ...
        require(tokenAddresses.length == amounts.length, "AssetWrapper: invalid input length");
        ...
        for (uint8 i = 0; i < tokenAddresses.length; i++) {
            ...
            bundleERC20Holdings[bundleId].push(ERC20Holding(tokenAddresses[i], amounts[i]));
            emit DepositERC20(_msgSender(), bundleId, tokenAddresses[i], amounts[i]);
        }
    }
    ...
    function depositERC721(
        ...
        require(_exists(bundleId), "Bundle does not exist");
        require(tokenAddresses.length == tokenIds.length, "AssetWrapper: invalid input length");

        for (uint8 i = 0; i < tokenAddresses.length; i++) {
            IERC721(tokenAddresses[i]).safeTransferFrom(_msgSender(), address(this), tokenIds[i]);

            bundleERC721Holdings[bundleId].push(ERC721Holding(tokenAddresses[i], tokenIds[i]));
            emit DepositERC721(_msgSender(), bundleId, tokenAddresses[i], tokenIds[i]);
        }
    }
    ...
    function depositERC1155(
        ...
        require(
            ...
            "AssetWrapper: invalid input length"
        );

        for (uint8 i = 0; i < tokenAddresses.length; i++) {
            IERC1155(tokenAddresses[i]).safeTransferFrom(_msgSender(), address(this), tokenIds[i], amounts[i], "");

            bundleERC1155Holdings[bundleId].push(ERC1155Holding(tokenAddresses[i], tokenIds[i], amounts[i]));
            emit DepositERC1155(_msgSender(), bundleId, tokenAddresses[i], tokenIds[i], amounts[i]);
        }
    }
    ...
    function depositBatch(
        ...
        uint256 bundleId = initializeBundle(msg.sender, metadata) - 1;

        if (erc20.length > 0) depositERC20(erc20, erc20Amounts, bundleId);
        if (erc721.length > 0) depositERC721(erc721, erc721Ids, bundleId);
        if (erc1155.length > 0) depositERC1155(erc1155, erc1155Ids, erc1155amounts, bundleId);
        if (msg.value > 0) depositETH(bundleId);
    ...
    function withdraw(uint256 bundleId) external override {
        ...
        delete bundleMetadata[bundleId];
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(ERC721, IERC165, ERC1155Receiver)
        returns (bool)
    {
