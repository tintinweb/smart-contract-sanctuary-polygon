...
contract PolygonBSG {
    ...
    uint256 private constant ATHSTOPLOSS80 = 2000;
    uint256 private constant ATHSTOPLOSS60 = 4000;
    ...
    constructor(
        ...
        _myinsurance = new Insurance(_usdtAddr, address(this));
    ...
    function _distributeDeposit(uint256 _amount) private {
        ...
        if(isFreezeReward){
            ...
            usdt.transfer(address(_myinsurance), _insurance);
            insurancePool = insurancePool.add(_insurance);
            _myinsurance.approve(insurancePool);
    ...
    function _distributeLuckDiamond5Cycle() private {
        uint256 diamondCount = diamondUsers.length;
        uint256 _mymembership = checkDiamondMembership(msg.sender);
        uint256 _diamondClaimCount = getDiamondClaimCount(msg.sender);
        if(_mymembership >= 2 && _diamondClaimCount < 5){
            uint256 reward = diamondIncomePool.div(diamondCount.mul(5));
            rewardInfo[msg.sender].diamondLuckyReward = rewardInfo[msg.sender].diamondLuckyReward.add(reward);
    ...
    function _balActived(uint256 _bal) private {
        if(_bal >= AllTimeHigh.mul(balDown[2]).div(baseDivider)){
            balStatus[balDown[0]] = true;
            balStatus[balDown[1]] = true;
            AllTimeHigh = _bal;
            if(isFreezeReward){
                isFreezeReward = false;
                diamondcycleRewardPercents = diamondcycleRewardPercents.div(2);
            }
        }else{
            balStatus[balDown[0]] = false;
            balStatus[balDown[1]] = false;
        }
    }
    ...
    function _setFreezeReward(uint256 _bal, bool when) private {
        if(when){ //deposit - only isFreezed = true
            ...
            for(uint256 i = balDown.length; i > 0; i--){
                if(_bal < AllTimeHigh.mul(baseDivider.sub(ATHSTOPLOSS80)).div(baseDivider)){
                    isFreezeReward = true;
                    depositFromInsurance();
                    break;
                }
    ...
    function withdraw() external {
        ...
        userRewards.diamondLuckyReward = 0;
        ...
        if(_membership == 4){
            ...
            if(_count < 10){
                ...
                usdt.transfer(address(_myinsurance), 50e6);
            ...
            if(_count < 20){
                ...
                usdt.transfer(address(_myinsurance), 25e6);
    ...
    function _calCurStaticRewards(address _user) public view returns(uint256, uint256) {
        ...
        if(getMember(_user) > 0){
            uint256 _diamondMember = checkDiamondMembership(_user);
            if(_diamondMember >= 2){
                totalRewards = _capital.mul(diamondcycleRewardPercents).div(baseDivider);
            }else{
    ...
    function _calCurDynamicRewards(address _user) public view returns(uint256, uint256) {
        ...
        totalRewards = totalRewards.add(userRewards.diamondLuckyReward);
    ...
    function depositBylockusdt(uint256 amount) external {
        ...
        require(lockusdtLeft.mul(baseDivider).div(baseDivider - freshUSDTforlockPercents) <= _amount.mul(baseDivider).div(freshUSDTforlockPercents), "insufficient fresh lockusdt"); //fresh should be more than 50%
    ...
    function depositFromInsurance() private {
        ...
        if(allowanceAmount >= uint256(1000000))
        {
            if(allowanceAmount > insurancePool){
                usdt.transferFrom(address(_myinsurance), address(this), insurancePool);
            } else {
                usdt.transferFrom(address(_myinsurance), address(this), allowanceAmount);
            }
        }
    }
    ...
    function checkDiamondMembership(address _user) public view returns(uint256){
        ...
        if(_orderLength > 0){
            ...
            OrderInfo storage _firstOrder = _orderlist[0];
            _selfAmount = _finalOrder.amount;
            _selfFirstAmount = _firstOrder.amount;
            ...
            if(_selfAmount >= 2500e6 && _selfFirstAmount >= 2500e6 && _directs >=15 && _directSize >= 15000e6 && _totalTeamSize >= 400 && _totalTeamVolume >= 500000e6){
                _membership = 4;
            }else if(_selfAmount >= 2500e6 && _selfFirstAmount >= 2500e6 && _directs >=10 && _directSize >= 10000e6 && _totalTeamSize >= 180 && _totalTeamVolume >= 100000e6){
                _membership = 3;
    ...
    function getMember(address _user) public view returns(uint256) {
        uint256 _capital = userInfo[_user].maxDeposit;
        uint256 _member = 0;
    ...
    function getDiamondClaimCount(address _user) private view returns(uint256) {
        uint256 _count = 0;
        for (uint256 i = 0; i < diamondUsers.length; i++) {
            if(diamondUsers[i] == _user){
                _count = _count.add(1);
            }
        }
        return _count;
...
contract Insurance is Ownable {
    ...
    function approve(uint256 _amount) public onlyOwner {
        usdt.approve(ParentContract, _amount);
    }
}
