...
contract Permissions {
  ...
  modifier onlyPermits(){
    require(permits[msg.sender] == true, "You are not allowed to run this");
    _;
  }
  ...
  function addPermit(address _addr) public onlyPermits{
    require(permits[_addr] == false, "Already permitted");
    permits[_addr] = true;
  ...
  function removePermit(address _addr) public onlyPermits{
    require(_addr != msg.sender, "You can't remove permit yourself");
    permits[_addr] = false;
...
contract RATToken is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable, Permissions {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;

    constructor() ERC721("Risk Assesement Token", "RATToken") {}

    function safeMint(address to, string memory uri) public onlyPermits {
    ...
    function intTransfer(address _from, address _to, uint256 tokenId) external onlyPermits returns(bool){
        require(balanceOf(_from) >0, "ERC721: transfer of token that is not own");
