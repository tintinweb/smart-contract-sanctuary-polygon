...
interface IBridge {
  ...
  function transmitMessage(
    ...
    uint256 _amount,
    uint256 _tokenID,
    bytes memory _metaRouteCalldata
  ) external;
...
contract Portal is RelayRecipientUpgradeable {
    ...
    function sendSymbiMessage(
        uint256 _amount,
        uint256 _tokenID,
        bytes memory _metaRouteCalldata
    ) external payable whenNotPaused {
        (bool success, ) = feeCollector.call{value: msg.value}("");
        require(success, "Symb: Failed to send native");
        IBridge(bridge).transmitMessage(
            _msgSender(),
            _amount,
            _tokenID,
            _metaRouteCalldata
    ...
    /**
    * @notice Sets feeCollector address
     * @param _feeCollector Address of feeCollector
     */
    function setFeeCollector(address _feeCollector) external onlyOwner {
        require(address(_feeCollector) != address(0), "Symb: feeCollector cannot be zero address");
