...
contract PlutoV2forge is Ownable, Implementor {
    ...
    function changeMainContract(address _plutoV2_contract_address)
        external
        onlyOwner
    {
        plutoV2_contract_address = IERC721Enumerable(_plutoV2_contract_address);
    ...
    // REMOVE ON MAINNET onlyOwner
    function addRewardForAddressByAdmin(
        address _user,
        uint8[2] memory _arrayOfRewards
    ) public {
        for (uint16 i = 0; i < 2; i++) {
            claimableRewards[_user][_arrayOfRewards[i]] += 1;
        }
    ...
    function removeRewardForAddressByAdmin(address _user, uint8 _traitInput)
        public
        onlyOwner
    {
        claimableRewards[_user][_traitInput] -= 1;
    ...
    function getHash(
        ...
        return
            keccak256(abi.encodePacked(md5hash, _traitInput, rowID, timeLimit));
    }
    ...
    function claimRedeemable(
        ...
        // Validate.
        bytes32 hash = getHash(md5hash, _traitInputCheck, rowID, timeLimit);
    ...
    function resultReturn(
        ...
        bytes32 hash = getHash(md5hash, _traitInput, rowID, timeLimit);
        hash = ECDSA.toEthSignedMessageHash(hash);
        address signer = ECDSA.recover(hash, v, r, s);
        return signer;
    }
