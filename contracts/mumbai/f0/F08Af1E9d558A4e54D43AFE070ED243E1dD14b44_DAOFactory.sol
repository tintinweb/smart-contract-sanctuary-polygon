...
import "../BaseAdmin/BaseAdmin.sol";
import "../ERC721Properties/ERC721Properties.sol";

contract Admin is BaseAdmin {
    bytes32 constant ERC721_APP = keccak256("ERC721");

    function mint(
        address to,
        bytes32 tierValue
    ) external onlyAdminOrSudo {
        ERC721Properties(_kernel.getAppAddress(ERC721_APP)).mint(to, tierValue);
    }

    function batchMint(
        address[] calldata to,
        bytes32[] calldata tierValues
    ) external onlyAdminOrSudo {
        for (uint256 i = 0; i < to.length; i++) {
            ERC721Properties(_kernel.getAppAddress(ERC721_APP)).mint(to[i], tierValues[i]);
        }
    }

    function burn(uint32 tokenId) external onlyAdminOrSudo {
        ERC721Properties(_kernel.getAppAddress(ERC721_APP)).burn(tokenId);
    }

    function batchBurn(uint32[] calldata tokenIds) external onlyAdminOrSudo {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ERC721Properties(_kernel.getAppAddress(ERC721_APP)).burn(tokenIds[i]);
        }
    }

    function addAdmin(address adminAddress) external onlyAdminOrSudo {
        _addAdmin(adminAddress);
    }

    function removeAdmin(address adminAddress) external onlyAdminOrSudo {
        _removeAdmin(adminAddress);
    }
}
...
contract AppProxy is Proxy, UpgradeableApp {
    ...
    constructor(address _logic) {
        _getImplementationSlot().value = _logic;
    }
...
contract BaseAdmin is App {
    ...
    function _addAdmin(address adminAddress) internal {
        require(_adminAddresses.add(adminAddress), Exceptions.INVALID_AUTHORIZATION_ERROR);
    }

    function _removeAdmin(address adminAddress) internal {
        require(_adminAddresses.remove(adminAddress), Exceptions.INVALID_AUTHORIZATION_ERROR);
    }
...
contract Sudo is BaseAdmin {
    bytes32 constant ERC721_APP = keccak256("ERC721");

    function connectApp(bytes32 id, address appAddress) external onlyAdmin {
        _kernel.connectApp(id, appAddress);
    }
    ...
    function addPermission(
        ...
        _kernel.addPermission(requesterAppId, appId, permissionId);
    }

    function addAdmin(address adminAddress) external onlyAdmin {
        Admin adminApp = Admin(_kernel.getAppAddress(keccak256(abi.encodePacked("SUDO"))));
        adminApp.addAdmin(adminAddress);
    }

    function removeAdmin(address adminAddress) external onlyAdmin {
        Admin adminApp = Admin(_kernel.getAppAddress(keccak256(abi.encodePacked("SUDO"))));
        adminApp.removeAdmin(adminAddress);
    }
    ...
    function setAttribute(
        ...
        ERC721Properties(_kernel.getAppAddress(ERC721_APP)).setAttribute(propKey, propValue, attrKey, attrValue);
    }

    function addSudoAdmin(address adminAddress) external onlyAdmin {
        _addAdmin(adminAddress);
    }

    function removeSudoAdmin(address adminAddress) external onlyAdmin {
        _removeAdmin(adminAddress);
    }
}
...
import "../apps/Sudo/Sudo.sol";
import "../apps/Admin/Admin.sol";
...
contract DAOFactory {
    ...
    bytes32 constant SUDO_APP = keccak256(abi.encodePacked("SUDO"));
    bytes32 constant ERC721_APP = keccak256(abi.encodePacked("ERC721"));
    bytes32 constant ADMIN_APP = keccak256(abi.encodePacked("ADMIN"));

    address public immutable sudoAdmin;
    address public immutable kernelImpl;
    address public immutable sudoImpl;
    address public immutable adminImpl;
    address public immutable erc721Impl;
    ...
    constructor(
        ...
        sudoAdmin = sudoAdmin_;
        kernelImpl = kernelImpl_;
        sudoImpl = sudoImpl_;
        adminImpl = adminImpl_;
        erc721Impl = erc721Impl_;
    }

    function deployDAO(Deployment calldata deployment)
        external
        returns (
            address kernel,
            address sudo,
    ...
    {
        kernel = address(new AppProxy(kernelImpl));
        sudo = address(new AppProxy(sudoImpl));
        admin = address(new AppProxy(adminImpl));
        erc721 = address(new AppProxy(erc721Impl));

        Kernel(kernel).initialize(sudo);

        {
            address[] memory sudoAdmins = new address[](2);
            sudoAdmins[0] = sudoAdmin;
            sudoAdmins[1] = address(this);

            Sudo(sudo).initialize(IKernel(kernel), sudoAdmins);
        }

        Admin(admin).initialize(IKernel(kernel), deployment.admins);
        ERC721Properties(erc721).initialize(
            ERC721Properties.Initialization({
                kernel: IKernel(kernel),
                isTransferable: deployment.isTransferable,
                transferUnlocksAtHours: deployment.transferUnlocksAtHours,
                owner_: deployment.admins[0],
                baseURI: deployment.ipfsHash,
                name: deployment.name,
                symbol: deployment.symbol
            })
        );

        Sudo(address(sudo)).connectApp(ADMIN_APP, address(admin));
        Sudo(address(sudo)).connectApp(ERC721_APP, address(erc721));

        Sudo(sudo).addPermission(SUDO_APP, ERC721_APP, ERC721Properties(erc721).APP_UPDATER());
        Sudo(sudo).addPermission(SUDO_APP, ADMIN_APP, Admin(admin).APP_UPDATER());
        Sudo(sudo).addPermission(SUDO_APP, ADMIN_APP, Admin(admin).SUDO());
        Sudo(sudo).addPermission(ADMIN_APP, ERC721_APP, ERC721Properties(erc721).ADMIN());

        for (uint256 i; i != deployment.amountsByTier.length; ++i) {
            Sudo(sudo).setAttribute("TIER", deployment.tiers[i], "MAX_AMOUNT", bytes32(deployment.amountsByTier[i]));
        }

        Sudo(sudo).removeSudoAdmin(address(this));

        emit Deployed(tx.origin, msg.sender, kernel, sudo, admin, erc721, abi.encode(deployment));
    }
}
...
contract ACL is BaseStorage, IACL {
    ...
    function addPermission(
        ...
        bytes32 appId,
        uint8 permissionId
    ) external requirePermission(APP_UPDATER) {
...
contract AppManager is ACL, IAppManager {
    ...
    function connectApp(bytes32 id, address appAddress) external requirePermission(APP_UPDATER) {
        _setApp(id, appAddress);
    }
    ...
    function _setApp(bytes32 id, address app) internal {
...
contract Kernel is Initializable, AppManager, IKernel {
    bytes32 constant KERNEL = keccak256("KERNEL");
    bytes32 constant SUDO = keccak256("SUDO");

    function initialize(address sudo) external initializer {
        __BaseStorage_init();
        _setApp(KERNEL, address(this));
        _setApp(SUDO, sudo);
        _addPermission(SUDO, KERNEL, APP_UPDATER);
    }
}
