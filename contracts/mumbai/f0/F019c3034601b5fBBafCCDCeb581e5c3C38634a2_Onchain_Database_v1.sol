...
/*SPDX-License-Identifier: MIT
    ...
    shimadamasafumi Onchain_Database_v1 {
        SEQUENCE {
            [RegistrationDate(Unixtime), RegistrationNumber, InventoryNumber, Title_en, Title_ja, Material, Edition, Term&Year, Size, ImageURL, Details, Exhibition, Status]
        }
...
contract Onchain_Database_v1 {
    ...
    mapping(string => string) private artistMemo;
    ...
    event TextAdded(uint256 indexed workId, string inventoryNumber);
    ...
    constructor() {
        _sequence = "[RegistrationDate(Unixtime), RegistrationNumber, InventoryNumber, Title_en, Title_ja, Material, Edition, Term&Year, Size, ImageURL, Details, Exhibition, Status]";
    ...
    function ARTIST_LinkUrl() public view returns (string memory) {
        return _linkUrl;
    }
    ...
    function addText(string memory _inventoryNumber, string memory _artistMemo) external {
        bytes32 hash = keccak256(abi.encodePacked(_inventoryNumber));
        uint256 workId = workIdByHash[hash];
        require(workId > 0, "Work does not exist");
        artistMemo[_inventoryNumber] = _artistMemo;
        latestUpdateTitle = _inventoryNumber;
        latestUpdateFunction = "addText";
        emit TextAdded(workId, _inventoryNumber);
    }

    function SEARCH_TextForWorksByInventoryNumber(string memory _inventoryNumber) external view returns (string memory) {
        return artistMemo[_inventoryNumber];
    }
    ...
    function SEARCH_WorksByKeyword(string memory _keyword) external view returns (Work[] memory) {
    uint256 count = 0;
