    ...
    UUPSUpgradeable
{
    ...
    function initialize(address rng_, address cryptoHands_) public initializer {
        ...
        s_commissionPercentage = 3;
        s_refreeCommission = 1;
        s_divider = 10;
    ...
    function _getComissionFromBet(
        uint256 _bet,
        address _player
    ) internal view returns (uint256 _comission, uint256 _refreeCommision) {
        Player memory currentPlayer = s_players[_player];
        if (currentPlayer.refree == address(0)) {
            _comission = _bet * (s_commissionPercentage / 100);
            _refreeCommision = 0;
        } else {
            uint256 refreeCommsion = _bet * (s_refreeCommission / 100);
            uint256 gameCommision = (_bet * (s_commissionPercentage / 100));
            _refreeCommision = refreeCommsion;
            _comission = gameCommision - refreeCommsion;
        }
    }
    ...
    function _createAndSettleBet(
        ...
        Player memory currentPlayer = s_players[player_];

        address refree = address(0);

        if (currentPlayer.refree == address(0) && refree_ != address(0)) {
            currentPlayer.refree = refree_;
            refree = refree_;
            s_players[refree_].totalReferrals =
                s_players[refree_].totalReferrals +
                1;
        } else {
            refree = currentPlayer.refree;
        }
        ...
        if (refree == address(0)) {
            if (_result == Results.Win) {
                ...
                currentPlayer.totalGamesWonned =
                    currentPlayer.totalGamesWonned +
                    1;
                currentPlayer.totalEarnings =
                    currentPlayer.totalEarnings +
                    winAmount;
            }
            ...
            if (_result == Results.Win) {
                ...
                currentRefree.referralEarnings =
                    currentRefree.referralEarnings +
                    refreeCommision;
            }
            if (_result == Results.Tie) {
        ...
        currentPlayer.totalGamesPlayed = currentPlayer.totalGamesPlayed + 1;
    }
