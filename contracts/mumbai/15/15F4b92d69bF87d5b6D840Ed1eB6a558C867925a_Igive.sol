...
contract Igive is Initializable, UUPSUpgradeable, ERC20Upgradeable, ERC20PermitUpgradeable, ERC20VotesUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    using SafeMathUpgradeable for uint256;
    using SafeERC20Upgradeable for IERC20Upgradeable;
    IERC20Upgradeable public good;
    ...
    struct UserInfo {
        ...
        uint256 rewardGOODDebt; // Reward debt in GOOD.
        //
        // We do some fancy math here. Basically, any point in time, the amount of GOOD
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accGOODPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accGOODPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 lastTotalGOODReward; // last total rewards
        uint256 lastGOODRewardBalance; // last GOOD rewards tokens
        uint256 totalGOODReward; // total GOOD rewards tokens
    }
    ...
    // Bonus muliplier for early GOOD makers.
    ...
    // total GOOD staked
    uint256 public totalGOODStaked;
    // total GOOD used for purchase land
    uint256 public totalGoodUsedForPurchase;
    ...
    function initialize(        
        ...
        __ERC20_init_unchained("iGive", "iGive");
        __Pausable_init_unchained();
        ERC20PermitUpgradeable.__ERC20Permit_init("iGive");
    ...
    function add(uint256 _allocPoint, IERC20Upgradeable _lpToken, bool _withUpdate) public onlyOwner {
        ...
        poolInfo.push(PoolInfo({
            ...
            accGOODPerShare: 0,
            lastTotalGOODReward: 0,
            lastGOODRewardBalance: 0,
            totalGOODReward: 0
        }));
    }

    // Update the given pool's GOOD allocation point. Can only be called by the owner.
    ...
    function pendingGOOD(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accGOODPerShare = pool.accGOODPerShare;
        uint256 lpSupply = totalGOODStaked;
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 rewardBalance = pool.lpToken.balanceOf(address(this)).sub(totalGOODStaked.sub(totalGoodUsedForPurchase));
            uint256 _totalReward = rewardBalance.sub(pool.lastGOODRewardBalance);
            accGOODPerShare = accGOODPerShare.add(_totalReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accGOODPerShare).div(1e12).sub(user.rewardGOODDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 rewardBalance = pool.lpToken.balanceOf(address(this)).sub(totalGOODStaked.sub(totalGoodUsedForPurchase));
        uint256 _totalReward = pool.totalGOODReward.add(rewardBalance.sub(pool.lastGOODRewardBalance));
        pool.lastGOODRewardBalance = rewardBalance;
        pool.totalGOODReward = _totalReward;
        
        uint256 lpSupply = totalGOODStaked;
        if (lpSupply == 0) {
            pool.lastRewardBlock = block.number;
            pool.accGOODPerShare = 0;
            pool.lastTotalGOODReward = 0;
            user.rewardGOODDebt = 0;
            pool.lastGOODRewardBalance = 0;
            pool.totalGOODReward = 0;
            return;
        }
        
        uint256 reward = _totalReward.sub(pool.lastTotalGOODReward);
        pool.accGOODPerShare = pool.accGOODPerShare.add(reward.mul(1e12).div(lpSupply));
        pool.lastTotalGOODReward = _totalReward;
    }

    /**
    @notice Sorting the highest GOOD staker in pool
    @param _pid : pool id
    @param left : left
    @param right : right
    @dev Description :
        It is used for sorting the highest GOOD staker in pool. This function definition is marked
    ...
    */
    ...
    @dev Description :
    DAO governance will be performed by the top 50 wallets with the highest amount of staked GOOD tokens.
    */
    ...
    /**
    @notice GOOD staking track the Highest 50 staked users
    @param _pid : pool id
    ...
    */
    ...
    // Deposit GOOD tokens to MasterChef.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            uint256 goodReward = user.amount.mul(pool.accGOODPerShare).div(1e12).sub(user.rewardGOODDebt);
            pool.lpToken.safeTransfer(msg.sender, goodReward);
            pool.lastGOODRewardBalance = pool.lpToken.balanceOf(address(this)).sub(totalGOODStaked.sub(totalGoodUsedForPurchase));
        }
        pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        totalGOODStaked = totalGOODStaked.add(_amount);
        user.amount = user.amount.add(_amount);
        user.rewardGOODDebt = user.amount.mul(pool.accGOODPerShare).div(1e12);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        totalGOODStaked = totalGOODStaked.sub(_amount);
        user.rewardGOODDebt = user.amount.mul(pool.accGOODPerShare).div(1e12);
    ...
    // Earn GOOD tokens to MasterChef.
    function claimGOOD(uint256 _pid) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
    ...
    // Safe GOOD transfer function to admin.
    function accessGOODTokens(uint256 _pid, address _to, uint256 _amount) public {
        require(msg.sender == adminAddress, "sender must be admin address");
        require(totalGOODStaked.sub(totalGoodUsedForPurchase) >= _amount, "Amount must be less than staked GOOD amount");
        PoolInfo storage pool = poolInfo[_pid];
        uint256 GoodBal = pool.lpToken.balanceOf(address(this));
        if (_amount > GoodBal) {
            pool.lpToken.transfer(_to, GoodBal);
            totalGoodUsedForPurchase = totalGoodUsedForPurchase.add(GoodBal);
            emit EmergencyWithdraw(_to, _pid, GoodBal);
        } else {
            pool.lpToken.transfer(_to, _amount);
            totalGoodUsedForPurchase = totalGoodUsedForPurchase.add(_amount);
