...
contract MagicAggregator is IMagicAggregator, AccessControl, IERC721Receiver, IERC1155Receiver {
    ...
    function batchMint721(
        ...
        for (uint i=0; i<contracts.length; i++) {
            ...
            (bool success, bytes memory data) = contracts[i].call(abi.encodeWithSignature(abi_str, address(this), amounts[i]));
    ...
    function batchTransfer721(
        ...
        for (uint i=0; i<contracts.length; i++) {
            ...
            try underlying_contract.safeTransferFrom(address(this), to_addresses[i], token_ids[i]) {
    ...
    function batchMint1155(
        ...
        for (uint i=0; i<contracts.length; i++) {
            string memory abi_str = string.concat(function_names[i], abi_params);
            (bool success, bytes memory data) = contracts[i].call(abi.encodeWithSignature(abi_str, address(this), amounts[i], token_ids[i]));
    ...
    function batchTransfer1155(
        ...
        for (uint i=0; i<contracts.length; i++) {
            ...
            try underlying_contract.safeTransferFrom(address(this), to_addresses[i], token_ids[i], amounts[i], "") {
