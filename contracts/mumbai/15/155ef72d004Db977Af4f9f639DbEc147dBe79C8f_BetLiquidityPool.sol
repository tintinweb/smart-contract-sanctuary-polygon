...
contract BetLiquidityPool is LiquidityPool, IBetLiquidityPool {
    ...
    /**
     * @notice Transfer own tokens to betBox. Doesn't affect LPTokens.
...
abstract contract LiquidityBox is ILiquidityBox, TokenValidator {
    ...
    /**
     * @notice Transfer own tokens to external Box. Doesn't affect DistTokens.
     * @param tokenAdd The token type to transfer.
     * @param amount The amount to transfer.
     */
    ...
    /**
     * @notice Gets a users available balance of a DistToken.
     * @param owner Is the user that owns the DistTokens.
     * @param tokenAdd The token type that matches the DistToken.
     * @return uint256 The amount owned by the user.
     */
    function getUserDistTokenBalanceFromToken(address owner, address tokenAdd)
        external
        override
        view
        onlyAllowedToken(tokenAdd)
    ...
    function getFullBalance(address tokenAdd) public view returns (uint256) {
        console.log("getFullBalance(", tokenAdd, ")");
        uint256 fullTokenBalance = IERC20(tokenAdd).balanceOf(address(this));
        console.log("getFullBalance() extBox addr:", address(extLockBox));
    ...
    function getMatchingTokenAmount(address distTokenAdd, uint256 amount)
        ...
        override
        view
        onlyAllowedDistToken(distTokenAdd)
        returns (uint256 tokenAmount)
    {
        address tokenAdd = IDistToken(distTokenAdd).tokenAdd();
        uint256 totalSupplyDist = IDistToken(distTokenAdd).totalSupply();
        if (amount == 0 || totalSupplyDist == 0) {
            return 0;
        }
        tokenAmount = getFullBalance(tokenAdd).mul(amount).div(totalSupplyDist);
    }
        ...
        returns (uint256 distTokenAmount)
    {
        ...
        uint256 totalSupplyDist = IDistToken(distToken).totalSupply();
        if (amount == 0 || totalSupplyDist == 0) {
            return 0;
        }
        distTokenAmount = totalSupplyDist.mul(amount).div(getFullBalance(tokenAdd));
    }
    ...
    function deposit(
        ...
        if (totalSupply > 0) {
            ...
            distAmount = addAmount.mul(totalSupply).div(fullTokenBalance);
        }
        ...
        console.log("Will mint:", distAmount / 1 ether);
        IDistToken(distToken).mint(owner, distAmount);
...
import "./interfaces/tokens/ILPToken.sol";
...
contract LiquidityPool is LiquidityBox, ILiquidityPool {
    ...
    function withDrawAllTokens(address lpTokenAdd)
        external
        override
        onlyAllowedDistToken(lpTokenAdd)
    {
        address owner = msg.sender;
    ...
    *  @param lpTokenAdd The address of the lpToken type to return.
    ...
    {   // TODO: Determine if this is sufficient admin override to support our needs.
        ...
        uint256 matchingLPTokenBalanceLP = getMatchingDistTokenAmount(tokenAdd, tokenBalanceLP);
...
abstract contract TokenValidator is ITokenValidator, AccessHandler {
     ...
     * @param inDistToken The DistToken to add to the allowed list.
     */
    function _addTokenType(IDistToken inDistToken)
        internal
    {
