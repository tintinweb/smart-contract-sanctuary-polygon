...
contract ERCTEST is Ownable, ERC721A, ReentrancyGuard {
    ...
    uint private constant MAX_SUPPLY = 6;
    uint private constant MAX_WHITELIST = 2;
    uint private constant MAX_TEAM = 5;
    ...
    // Test 
    uint public wlMint = 0;
    
    mapping(address => bool) public whitelistClaimed;
    mapping(address => uint) public amountNFTsperWalletTeamMint;
    ...
    function mintWhitelist(address to, bytes32[] calldata _merkleProof) public {
        // require(totalSupply() + 1 <= MAX_WHITELIST,"Max Allowed to Whitelist exceeded");
        require(wlMint + 1 <= MAX_WHITELIST,"Max Allowed to Whitelist exceeded");
    ...
    function teamMint(uint256 _quantity) external onlyOwner {
        require(sellingStep == Step.TeamMint, "Team step is still active ");
        require(totalSupply() + _quantity <= MAX_SUPPLY,"Max supply exceeded");
        require(amountNFTsperWalletTeamMint[msg.sender] + _quantity <= MAX_TEAM, "Max 500 NFT For the team");
