...
/// @notice Collects memz, pages, & goo to be redistributed to community.
contract Reserve is Owned {
    Memz public immutable memz;
    Pages public immutable pages;
    Goo public immutable goo;

    constructor(Memz _memz, Pages _pages, Goo _goo, address _owner) Owned(_owner) {
        memz = _memz;
        pages = _pages;
        goo = _goo;
    }

    function withdrawalMemz(address _to, uint256[] calldata _ids) external onlyOwner {
        unchecked {
            for (uint256 i; i < _ids.length; ++i) {
                memz.transferFrom(address(this), _to, _ids[i]);
            }
        }
    }
