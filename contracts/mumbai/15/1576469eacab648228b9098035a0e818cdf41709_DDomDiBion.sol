...
// define a smart contract that generates and manages non-fungible multi-tokens (ERC1155)
contract DDomDiBion is ERC1155, Ownable, Pausable, ERC1155Supply
{
    // the default price for a new token in Wei
    uint256 private _defaultPrice = 100000000000000; // 0.0001 Ether or Matic (0.0001 * 10 ** 18)

    // various info about a token
    struct TokenInfo
    {
        // the token's list of owners
        //address[] owners;

        // the token's original owner
        address owner;

        // the tocken's price in Wei, initially it will be set to the default price
        uint256 price;
    } // TokenInfo

    // map token IDs to its info
    mapping(uint256 => TokenInfo) public tokenInfoMap;
    // the list containing all minted token IDs (we need this because the 'tokenInfoMap' is not iterable)
    uint256[] public tokenIDList;

    // map owner addresses to the number of token types held
    mapping(address => uint256) private _balances;


    // create the contract, define the base URI
    constructor()
        ERC1155("ipfs://bafybeic5kcia3yicinpn4wnkqls5a66egrucpfh4np3zmvsunt6zdwwia4/DDomDiBion_")
    {
    } // constructor

    // update token balances for the input accounts
    function _updateTokenBalances(address accountFrom, address accountTo, bool accountToNewToken, uint256 tokenID) internal
    {
        // update the number of token types held by the 'accountTo' account
        if (accountToNewToken)
            _balances[accountTo] += 1;

        // update the number of token types held by the 'accountFrom' account
        if (balanceOf(accountFrom, tokenID) == 0)
            _balances[accountFrom] -= 1;
    } // _updateTokenBalances

    // update token info for the input account
    function _updateTokenInfo(address account, uint256 tokenID) internal
    {
        // check if this is a newly minted token
        if (tokenInfoMap[tokenID].owner == address(0))
        {
            // store the first owner for the new token
            tokenInfoMap[tokenID].owner = account;
            tokenInfoMap[tokenID].price = _defaultPrice;
        ...
        // check if this is a newly minted token
    /*    if (tokenInfoMap[tokenID].owners.length == 0)
        {
            // add the first owner for the new token
            tokenInfoMap[tokenID].owners.push(account);
            tokenInfoMap[tokenID].price = _defaultPrice;
        ...
        {
            // check if the input account already owns this token
            bool isOwner = false;
            uint numOwners = tokenInfoMap[tokenID].owners.length;
            for (uint i = 0; i < numOwners; ++i)
            {
                if (tokenInfoMap[tokenID].owners[i] == account)
                {
                    isOwner = true;
                    break;
                }
            ...
            // add a new owner for the input token if necessary
            if (!isOwner)
                tokenInfoMap[tokenID].owners.push(account);
        }*/
    } // _updateTokenInfo

    // create new non fungible tokens ('amount'=1) or multi-tokens ('amount'>1) 
    // identified by the input 'tokenID' and assign them to the input 'account',
    ...
    function mint(address account, uint256 tokenID, uint256 amount, bytes memory data) public onlyOwner
    {
        ...
        // update the number of token types held by the input 'account'
        ...
        // update token info for the input 'account'
        _updateTokenInfo(account, tokenID);
    } // mint

    // create new batches of non fungible tokens or multi-tokens identified
    // by the input 'tokenIDs' and assign them to the input 'account',
    // only the owner can do this
    function mintBatch(address account, uint256[] memory tokenIDs, uint256[] memory amounts, bytes memory data) public onlyOwner
    {
        ...
        // update token info for the input 'account'
        for (uint i = 0; i < tokenIDs.length; ++i)
            _updateTokenInfo(account, tokenIDs[i]);
    } // mintBatch
    ...
    function safeTransferFrom(address accountFrom, address accountTo, uint256 tokenID, uint256 amount, bytes memory data) public virtual override
    {
        ...
        // update token balances for the input accounts
        _updateTokenBalances(accountFrom, accountTo, accountToNewToken, tokenID);

        // update token info for the 'accountTo' account
        _updateTokenInfo(accountTo, tokenID);
    } // safeTransferFrom
    ...
    function safeBatchTransferFrom(address accountFrom, address accountTo, uint256[] memory tokenIDs, uint256[] memory amounts, bytes memory data) public virtual override
    {
        ...
        // update token balances for the input accounts
        for (uint i = 0; i < tokenIDs.length; ++i)
            _updateTokenBalances(accountFrom, accountTo, accountToNewTokens[i],  tokenIDs[i]);

        // update token info for the 'accountTo' account
        for (uint i = 0; i < tokenIDs.length; ++i)
            _updateTokenInfo(accountTo, tokenIDs[i]);
    ...
    function safeBuyFrom(address payable accountFrom, uint256 tokenID, uint256 amount, bytes memory data) payable public
    {
        ...
        require(msg.value == tokenInfoMap[tokenID].price*amount);
    ...
    function safeBatchBuyFrom(address payable accountFrom, uint256[] memory tokenIDs, uint256[] memory amounts, bytes memory data) payable public
    {
        ...
        for (uint i = 0; i < tokenIDs.length; ++i)
            paymentValue += tokenInfoMap[tokenIDs[i]].price*amounts[i];
    ...
    // return the number of token types held by the input 'account'
    function balanceOf(address account) public view returns (uint256)
    {
        return _balances[account];
    } // balanceOf

    // change the default price of a new token in Wei
    ...
    // change the price of a token in Wei
    function setPrice(uint256 tokenID, uint256 price) public onlyOwner
    {
        //require(tokenInfoMap[tokenID].owners.length != 0);
        require(tokenInfoMap[tokenID].owner != address(0));
        tokenInfoMap[tokenID].price = price;
    } // setPrice
    
    // return the price of a token in Wei
    function getPrice(uint256 tokenID) public view returns (uint256)
    {
        return tokenInfoMap[tokenID].price;
    } // getPrice
    ...
    // trigger a stopped state for the contract,
    // only the owner can do this
    ...
    // return to the normal state of the contract,
    // only the owner can do this
