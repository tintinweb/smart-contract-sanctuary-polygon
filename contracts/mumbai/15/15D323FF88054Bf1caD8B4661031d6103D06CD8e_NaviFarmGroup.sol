...
/**
 * @title Contract representing farms and pool of some UniswapV2 Dex.
 * @notice Like nToken, but for LPs
 * @dev Each Dex implements its own farms, so this is abstract.
 **/
contract NaviFarmGroup is
    NaviMarginable,
    ReentrancyGuard,
    Ownable,
    DSMath
{
    ...
    mapping(address => uint256) private _totalLP;
    ...
    function hasEnoughMargin(
        ...
        //oracle.idByTokenAddress returns 0 if token is not registered
        if (
            id0 > 0 &&
            oracle.isnToken(id0) &&
