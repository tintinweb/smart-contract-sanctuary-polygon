...
contract PIX is IPIX, ERC721EnumerableUpgradeable, OwnableUpgradeable {
    ...
    function requestBatchMint(
        ...
        uint256 price = token == tokenForPrice
            ? dropPrices[dropId]
            : oracleManager.getAmountOut(tokenForPrice, token, dropPrices[dropId]);

        _registerRequest(token, dropId, playerId, price, count);
    }
    ...
    function requestBatchMintWithIXT(
        ...
        _registerRequest(address(pixToken), dropId, playerId, dropPrices[dropId], count);
    }
    ...
    function _registerRequest(
        ...
        emit Requested(dropId, playerId, packsPurchased[playerId][dropId] + 1, count);
    }
