...
// File contracts/market/auction/erc721/ERC721Interactions.sol
...
contract ERC721Interactions{
    ...
    function _transferTokens(address from, address to, uint256 id) internal returns(bool){
        IERC721 _token = IERC721(token);
...
// File contracts/market/auction/erc721/interfaces/IMarketplaceStorage.sol
...
// File contracts/market/auction/erc721/interfaces/IMarketplaceERC721.sol
...
// File contracts/market/auction/erc721/MarketplaceStorage.sol
...
contract MarketplaceStorage is IMarketplaceStorage{
   ...
   function _getLatestBidAmountAndProposer(uint256 id) internal view returns(address,uint256){
    ...
    function _getLatestProposer(uint256 id) internal view returns(address){
        Bid memory bid = bids[id];
        require(bid.bidProposer.length >=1,"MarketplaceStorage: bidProposer is empty");
    ...
    function _getLatestBid(uint256 id) internal view returns(uint256){
        Bid memory bid = bids[id];
        require(bid.bidAmount.length >=1,"MarketplaceStorage: bidAmount is empty");
...
// File contracts/market/auction/erc721/MarketplaceERC721Receiver.sol
...
// File contracts/market/auction/erc721/MarketplaceERC721.sol
...
contract NFTMarketplaceERC721 is MarketplaceStorage, MarketplaceERC721Receiver, ERC721Interactions, Ownable, IMarketplaceERC721, IBunzz{
    ...
    function createAuction(
        ...
        uint256 startingPrice,
        uint256 auctionStartDate,
        uint256 auctionEndDate) external override  
    {
        require(_sentFrom(tokenId)==msg.sender,"Marketplace: auction not created by the original token sender");
        require(auctionStartDate<auctionEndDate, "Marketplace: auction start date is higher then auction end date");

        address seller = msg.sender;

        Auction memory auction = Auction(seller,
         tokenId,
         buyoutPrice,
         startingPrice,
         auctionStartDate,
    ...
    function addBid(uint256 tokenId) external override payable {
        ...
        bool notInitialized;
        (notInitialized, lastElementPosition) = internalBids.bidAmount.length.trySub(1);
        if(notInitialized == true){
            require(msg.value > internalBids.bidAmount[lastElementPosition], "Marketplace: bid value is lower than the biggest bid");
        }
        Auction memory currentAuction = _getAuction(tokenId);
        require(currentAuction.auctionStartBlock < block.number, "Marketplace: start block is higher than block.number");
        require(currentAuction.auctionEndBlock > block.number, "Marketplace: end block is lower then block.number");
        require(currentAuction.startingPrice < msg.value , "Marketplace: bid value is lower than the starting price");
        require(msg.value <= currentAuction.buyoutPrice, "Marketplace: bid is higher then buyoutPrice");
        require(_balance(address(this))>=1, "Marketplace: the token is not for sale");
    ...
    function claimToken(uint256 tokenId) external override {
        ...
        require(auction.auctionEndBlock < block.number, "Marketplace: the auction is not yet closed");
        require(_getLatestProposer(tokenId) == msg.sender, "Marketplace: Claimer is not the auction winner");

        _transferTokens(address(this), msg.sender, tokenId);
    }


    function claimBids(uint256 tokenId) external override payable {
        Auction memory auction = _getAuction(tokenId);
    ...
    function retrieveTokenNotSold(uint256 tokenId) external override {
        ...
        require(auction.auctionEndBlock < block.number, "Marketplace: the auction is not yet closed");
        Bid memory bid = _getBid(tokenId);
        require(bid.bidAmount.length == 0, "Marketplace: the auction have bids");
    ...
    function withdraw(uint256 id) external {
        ...
        require(msg.sender == auction.seller,  "Marketplace: caller is not the seller of the token");
        require(auction.auctionEndBlock < block.number, "Marketplace: the auction is not yet closed");
        require(auction.tokensWithdrawed == false, "Marketplace: the funds have been already withdrawned");
    ...
    function getAuctionForTokenId(uint256 tokenId) public override view returns(Auction memory){
        return _getAuction(tokenId);
    }

    function getAuctionBids(uint256 tokenId) public override view returns(Bid memory){
