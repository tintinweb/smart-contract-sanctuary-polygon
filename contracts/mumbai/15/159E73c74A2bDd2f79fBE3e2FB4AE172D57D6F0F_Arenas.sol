...
contract Arenas is Initializable, OwnableUpgradeable {
    ...
    function getArenaHash( uint256 arena_id) public view returns ( uint128 hash ){
        return _arenas[ arena_id ]._hash;
    }

    // TODO IMPLEMENT generate random uint128 with 116 max valorized bits
    function genesysArenaHash() public pure returns ( uint128 hash ){
...
contract Decks is Initializable, OwnableUpgradeable {
    ...
    function deckAssetsOwnershipChecks( uint256 deck_id, address owner, uint8 deck_size ) public view {
        ...
        require( owner == player, "Player is not the owner of used Helmet" );

        uint256 nexus_id = getNexusId( deck_id );
        require( nft_1155.ownerOfNF( nexus_id ) == player, "Player is not the owner of used Nexus" );

        uint256[] memory mechas_ids = getMechaIds( deck_id );
...
contract Industries is Initializable, OwnableUpgradeable {
    ...
    uint8 public constant NFT_AMOUNT = 1;
    uint16 public constant MAX_DURABILITY = 1000;
    ...
    // @dev helmet_id => Helmet_struct{}
    mapping( uint256 => Helmet ) private _helmets;
    ...
    function getHelmet( uint256 helmet_id ) public view returns ( Helmet memory ) {
        return _helmets[ helmet_id ];
    }

    function getHelmetGenome( uint256 helmet_id ) public view returns ( uint256 ) {
        return  _helmets[ helmet_id ]._genome;
    }

    function validateAsLand( uint256 land_id ) public onlyOwner {
        _Validators.addLand( land_id );
    }

    function validateAsArena( uint256 arena_id ) public onlyOwner {
        _Validators.addArena( arena_id );
    }

    function validateAsHelmet( uint256 helmet_id ) public onlyOwner {
        _Validators.addHelmet( helmet_id );
    }

    function validateAsNexus( uint256 nexus_id ) public onlyOwner {
        _Validators.addNexus( nexus_id );
    }

    function validateAsCore( uint256 core_id ) public onlyOwner {
        _Validators.addCore( core_id );
    }

    function validateAsWeapon( uint256 weapon_id ) public onlyOwner {
        _Validators.addWeapon( weapon_id );
    }

    function validateAsAccessory( uint256 accessory_id ) public onlyOwner {
        _Validators.addAccessory( accessory_id );
    }

    function validateAsMotion( uint256 motion_id ) public onlyOwner {
    ...
    function createHelmet( address to, uint256 genome ) public onlyOwner {
        NFT_1155 nft_1155 = _Validators._NFT_1155();

        uint256 helmet_id = nft_1155.tokenID();
        createPart( to );
        validateAsHelmet( helmet_id );

        _helmets[ helmet_id ] = Helmet ({
            _id: helmet_id,
            _genome: genome,
            _h1_id: 0,
            _h2_id: 0
    ...
    function createHelmets( address to, uint256 quantity, uint256[] memory genomes ) public onlyOwner {
        for (uint256 i = 0; i < quantity; i++) createHelmet( to, genomes[i] );
    }
    ...
    function createAccessory( address to ) public onlyOwner {
        ...
        createPart( to );
        validateAsAccessory( accessory_id );
...
contract Validators is Initializable, OwnableUpgradeable {
    ...
    function isPartValidated( uint256 part_id ) public view returns ( bool ) {
        return (
            land_ids[ part_id ] ||
            arena_ids[ part_id ] ||
            helmet_ids[ part_id ] ||
            ...
            core_ids[ part_id ] ||
            weapon_ids[ part_id ] ||
            accessory_ids[ part_id ] ||
            motion_ids[ part_id ]
        );
    }

    function addLand( uint256 land_id ) public onlyOwner {
        require( !isPartValidated( land_id ), "This land_id is already added to Validator" );
    ...
    function addArena( uint256 arena_id ) public onlyOwner {
        require( !isPartValidated( arena_id ), "This arena_id is already added to Validator" );
    ...
    function addHelmet( uint256 helmet_id ) public onlyOwner {
        require( !isPartValidated( helmet_id ), "This helmet_id is already added to Validator" );
    ...
    function addNexus( uint256 nexus_id ) public onlyOwner {
        require( !isPartValidated( nexus_id ), "This nexus_id is already added to Validator" );
    ...
    function addCore( uint256 core_id ) public onlyOwner {
        require( !isPartValidated( core_id ), "This core_id is already added to Validator" );
    ...
    function addWeapon( uint256 weapon_id ) public onlyOwner {
        require( !isPartValidated( weapon_id ), "This weapon_id is already added to Validator" );
    ...
    function addAccessory( uint256 accessory_id ) public onlyOwner {
        require( !isPartValidated( accessory_id ), "This accessory_id is already added to Validator" );
    ...
    function addMotion( uint256 motion_id ) public onlyOwner {
        require( !isPartValidated( motion_id ), "This motion_id is already added to Validator" );
