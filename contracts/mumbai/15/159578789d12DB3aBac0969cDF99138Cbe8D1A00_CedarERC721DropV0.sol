...
import {IDropERC721V0} from "./interfaces/IDropERC721.sol";
...
import "../generated/impl/BaseCedarERC721DropV0.sol";
    ...
    BaseCedarERC721DropV0
{
    ...
    bytes32 private constant MODULE_TYPE = bytes32("CedarERC721DropV0");
    ...
    function collectClaimPrice(
        ...
        CurrencyTransferLib.transferCurrency(_currency, _msgSender(), primarySaleRecipient, totalPrice);
    }
    ...
    function verifyClaim(
        ...
        if(!(nextTokenIdToClaim + _quantity <= nextTokenIdToMint)) revert CrossedLimit();
        if(!(maxTotalSupply == 0 || nextTokenIdToClaim + _quantity <= maxTotalSupply)) revert CrossedLimit();
        if(!(maxWalletClaimCount == 0 || walletClaimCount[_claimer] + _quantity <= maxWalletClaimCount)) revert CrossedLimit();
    ...
    function verifyClaimMerkleProof(
        ...
        if (currentClaimPhase.merkleRoot != bytes32(0)) {
            ...
            if(!(!claimCondition.limitMerkleProofClaim[_conditionId].get(merkleProofIndex))) revert InvalidProof();
    ...
    function setMaxTotalSupply(uint256 _maxTotalSupply) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if(!(_maxTotalSupply < nextTokenIdToMint)) revert CrossedLimit();
...
interface IDropERC721V0 is IERC721Upgradeable, IDropClaimConditionV0 {
...
// Filename: './BaseCedarERC721DropV0.sol'
...
abstract contract BaseCedarERC721DropV0 is ICedarERC721DropV0 {
    function supportedFeatures() override public pure returns (string[] memory features) {
        ...
        features[2] = "issuance/ICedarDrop.sol:ICedarDropV0";
    ...
    function supportsInterface(bytes4 interfaceID) virtual override public view returns (bool) {
        return (interfaceID == type(IERC165).interfaceId) || ((interfaceID == type(ICedarFeaturesV0).interfaceId) || ((interfaceID == type(ICedarVersionedV0).interfaceId) || ((interfaceID == type(ICedarDropV0).interfaceId) || (interfaceID == type(IMulticallableV0).interfaceId))));
    }
...
import "../issuance/ICedarDrop.sol";
...
interface ICedarFeaturesV0 is IERC165 {
...
interface ICedarVersionedV0 is ICedarMinorVersionedV0, IERC165 {
    // Concrete implementation semantic version - provided for completeness but not designed to be the point of dispatch
