...
contract BSG {
    ...
    uint256 public doubleDiamondPool;
    ...
    function distributePoolRewards() public {
        if(block.timestamp > lastDistribute.add(timeStep)){
            ...
            _distributedoubleDiamondPool(dayNow);
            lastDistribute = block.timestamp;
        }
    }
    ...
    function _calLevelNow(address _user) private view returns(uint256) {
        ...
        if(total >= 1000e18){
            ...
            if(total >= 2000e18 && user.teamNum >= 5 && maxTeam >= 2000e18 && otherTeam >= 2000e18){
                levelNow = 5;
    ...
    function _distributedoubleDiamondPool(uint256 _dayNow) private {
        ...
        for(uint256 i = 0; i < 3; i++){
            ...
            if(userAddr != address(0)){
                uint256 reward = doubleDiamondPool.mul(rates[i]).div(baseDivider);
        ...
        if(doubleDiamondPool > totalReward){
            doubleDiamondPool = doubleDiamondPool.sub(totalReward);
        }else{
            doubleDiamondPool = 0;
        }
    }
    ...
    function _distributeDeposit(uint256 _amount) private {
        ...
        diamondPool = diamondPool.add(star);
        uint256 top = _amount.mul(doubleDiamondPoolPercents).div(baseDivider);
        doubleDiamondPool = doubleDiamondPool.add(top);
    }
