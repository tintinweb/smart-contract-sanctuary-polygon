...
contract OrderAdmin is OrderBase {
    ...
    function setMultiDexRouterAddress(address _multiDexRouterAddress) external onlyOwner {
        multiDexRouterAddress = _multiDexRouterAddress;
    }
    ...
    function cancelTask(bytes32 _taskId) public nonReentrant {
        ...
        if (y.tcc._type == OrderType.LimitOrder) {
            emit FundsEvent(block.number, block.timestamp, msg.sender, y.tcc._swapRouter[0], IERC20(y.tcc._swapRouter[0]).decimals(), y.tcc._limitItem._swapInAmount, "withdraw limitOrder");
    ...
    function editTaskSwapPriceZone(bytes32 _taskId, uint256 _minPrice, uint256 _maxPrice) external {
        ...
        TaskConfig storage y = taskConfigList[_taskId];
        require(msg.sender == owner() || msg.sender == y.tcd._owner, "No permission to modify");
        require(y.tcc._limitItem._swapPriceZone.length == 2, "B02");
        y.tcc._limitItem._swapPriceZone[0] = _minPrice;
        y.tcc._limitItem._swapPriceZone[1] = _maxPrice;
    }
...
contract OrderBase is OpsTaskCreator, Ownable, ReentrancyGuard {
    ...
    uint256 public gasPriceLimitForManual = 4 * 10 ** 9;
    uint256 public gasPriceLimitForAutomate = 4 * 10 ** 9;
