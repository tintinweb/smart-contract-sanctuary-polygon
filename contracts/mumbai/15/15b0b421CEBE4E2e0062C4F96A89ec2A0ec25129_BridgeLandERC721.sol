  ...
  ERC2981
{
  ...
  modifier onlyAllowedMinter() {
      require(allowedMinter == msg.sender, "BridgeLandERC721: caller not allowed");
      _;
  } 
  ...
  function safeMint(address to, uint256 tokenId, string memory _tokenURI) public onlyAllowedMinter {
    _safeMint(to, tokenId);
    _setTokenURI(tokenId, _tokenURI);
  }

  function safeBatchMint(address to, uint256[] calldata tokenIds, string[] calldata tokenURI_) public onlyAllowedMinter {
     require(tokenIds.length == tokenURI_.length, "BridgeLandERC721: tokenIds and tokenURI length mismatch");
    ...
    unchecked {
      for (uint256 i = 0; i < arraySize; i++) {
        ...
        _setTokenURI(tokenIds[i], tokenURI_[i]);
      }
    }
  }
