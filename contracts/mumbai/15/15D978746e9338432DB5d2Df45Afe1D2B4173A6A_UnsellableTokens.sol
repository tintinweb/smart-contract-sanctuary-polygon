...
contract UnsellableTokens is ReentrancyGuard, Pausable, Ownable {
    ...
    function setTokenPaymentAmount(
        ...
        require(
            _tokenPaymentAmount >= 0 gwei && _tokenPaymentAmount < 0.1 ether,
            "Token payment amount must be a reasonable number"
        );
    ...
    function sellToken(
        ...
        require(
            IERC20(tokenToSell).balanceOf(msg.sender) > amount,
            "Insufficient token balance"
        );

        uint256 totalServiceFeeAmount = serviceFee;
        ...
        uint256 totalAmountToPayCustomer = tokenPaymentAmount;

        IERC20(tokenToSell).safeTransferFrom(msg.sender, ourWallet, amount);
