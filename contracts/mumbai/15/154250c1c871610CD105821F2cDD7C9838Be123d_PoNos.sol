 ...
 */
abstract contract AbstractNouns is AccessControl, ERC721B {
...
contract PoNos is ReentrancyGuard, AbstractNouns {
    bytes32 public constant CURATOR_ROLE = keccak256("CURATOR_ROLE");

    // max amount that can be minted in this collection
    uint256 public constant MAX_SUPPLY = 1111;
    uint256 public constant MAX_FREE = 10; // free mints - to be 50
    // max per wallet - whales protection
    uint256 public constant MAX_PER_WALLET = 5; // to be 10
    ...
    constructor(
        INounsDescriptorMinimal _descriptor,
        INounsSeeder _seeder
    ) AbstractNouns(msg.sender, _descriptor, _seeder) {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(CURATOR_ROLE, msg.sender);
    }

    function name() external pure returns (string memory) {
        return "Polygon Nouns";
    }
    ...
    function currentTokenCost() public view returns (uint256) {
        ...
        if (_totalSupply > MAX_FREE && _totalSupply <= 20)
            // 300
            return 100000000000000000;
        if (_totalSupply > 20 && _totalSupply <= 30)
            // 300-600
            return 200000000000000000;
        if (_totalSupply > 30 && _totalSupply <= 40)
            // 600-900
            return 300000000000000000;
        if (_totalSupply > 40 && _totalSupply <= MAX_SUPPLY)
            // 900
     ...
     * @dev Sets the contract URI
     */
    function setURI(string memory uri) external onlyRole(CURATOR_ROLE) {
        _setURI(uri);
    }
