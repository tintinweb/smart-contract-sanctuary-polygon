...
contract Coupon is ERC721, Ownable{
    ...
    */
    ...
    function viewLiveCouponsOnly() external view returns(OnlyLive[] memory data){
        OnlyLive[] memory liveStatus = new OnlyLive[](allCouponStrings.length);
        for(uint i = 0; i < allCouponStrings.length; i++){
            if(block.timestamp > couponInformation[showCouponStringAndCouponId[allCouponStrings[i]]]._startDate && block.timestamp < couponInformation[showCouponStringAndCouponId[allCouponStrings[i]]]._endDate){
                // string memory tokenName = couponInformation[showCouponStringAndCouponId[allCouponStrings[i]]].couponName;
                // string memory localStatus = status_1;
