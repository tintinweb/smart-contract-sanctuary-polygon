...
contract IAmCheersNFT is ERC721, IIAmCheersNFT, AccessControl {
	// トークンのIDの合計数
	uint256 public tokenIds;

	constructor(address _cheers) ERC721("Test NFT", "TNFT") {
		_grantRole(DEFAULT_ADMIN_ROLE, _cheers);
	}

	function supportsInterface(bytes4 interfaceId) public view override(IERC165, ERC721, AccessControl) returns (bool) {
  ...
  // Cheers.solのcheer関数からのみ呼ばれる
	function mintNft(address _ownerAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
...
// projectのプール
contract ProjectPool is IProjectPool, AccessControl {
  ...
  // Cheers.solのcheer関数からのみ呼ばれる
  // このprojectをcheerする
  function receiveCher(uint256 _cheerCherAmount, address _from) external onlyRole(DEFAULT_ADMIN_ROLE) {
