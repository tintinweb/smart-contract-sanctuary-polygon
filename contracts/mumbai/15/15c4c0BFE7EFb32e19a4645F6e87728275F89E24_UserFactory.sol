...
contract UserFactory is IUserFactory, ERC2771Context {
    address[] public userList;
    mapping (address=>address) public userContract;
    modifier onlyUser(Wallet memory wallet) {
        require(
            _msgSender() == wallet.publicAddr,   //_msgSender() instead of msg.sender to avoid wrong address during metatransactions
            "Yousov : Operation not authorized"
        );
        _;
    }

    constructor(MinimalForwarder forwarder) ERC2771Context(address(forwarder)) {  // Initialize trusted forwarder
    }
    ...
    function newUser(
     ...
     {   
        // onlyUser(wallet)   //temporary removed
        require(isTrustedForwarder(msg.sender), "Yousov : Operation not authorized, not the trustedForwarder"); // ==> only trustedForwarder can call this fonction, msg.sender should refer to the forwarder contract

        // TODO check the unicity of the user!  _checkUserUnicity(pii,wallet,pseudonym,accountType,msg.sender);
        for (uint i = 0; i < userList.length; i++) {
            IUser _currentUser = IUser(userList[i]);
            if (accountType == AccountType.REGULAR && _currentUser.getAccountType() == AccountType.REGULAR) {
                if (keccak256(abi.encodePacked(pii.firstName, pii.middelName, pii.lastName ,  pii.birthPlace,  pii.uid, pii.birthDateTimeStamp, pii.gender ) ) == 
            ...
            if (accountType == AccountType.PSEUDO && _currentUser.getAccountType() == AccountType.PSEUDO) {    // else if ?
                if ( keccak256(abi.encodePacked(newPseudonym)) == keccak256(abi.encodePacked(_currentUser.pseudonym()))  ) {
                    revert("YOUSOV: User already exist");
                }
            }
        ...
        userContract[_msgSender()] = newUserContract;    //_msgSender()
        emit UserCreated();
    }
    ...
    function deleteUser() external override {
        ...
        for (uint i = 0; i < userList.length; i++) {
            if (
                _msgSender() == IUser(userList[i]).getWalletDetails().publicAddr
            ) {
                _userToDelete = userList[i];
                userList[i] = userList[userList.length - 1];
                userList.pop();
                delete userContract[_msgSender()];
                emit UserDeleted(_msgSender());
                break;
            }
        }
    }
