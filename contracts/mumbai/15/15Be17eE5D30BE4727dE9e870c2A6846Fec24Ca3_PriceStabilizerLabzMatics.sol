...
abstract contract PriceStabilizer {
    ...
    function getConvertedQuote(int256 _target) public view returns(int256) {
        AggregatorV3Interface _base = priceFeed;
        int256 decimals = int256(10 ** uint256(_defaultDecimals));
        uint8 baseDecimals = _base.decimals();
        int256 _basePrice = scalePrice(getLatestPrice(), baseDecimals, _defaultDecimals);
        int256 quotePrice = scalePrice(_target, _defaultDecimals, _defaultDecimals);
        return _basePrice * decimals / quotePrice;
    }
...
contract PriceStabilizerLabzMatics is PriceStabilizer {
    ...
    function _stabilize(int256 _target) internal virtual override returns(int256) {
    ...
    function getPrice(int256 _target) public returns(int256) {
