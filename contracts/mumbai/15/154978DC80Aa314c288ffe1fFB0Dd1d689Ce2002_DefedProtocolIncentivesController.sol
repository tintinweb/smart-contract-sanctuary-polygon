...
/**
 * @title DefedProtocolIncentivesController
 * @notice Distributor contract for rewards to the Defed protocol
 * @author Defed
 **/
contract DefedProtocolIncentivesController is
    ...
    DistributionManager
{
   ...
   function setOperator(address[] calldata _operators,bool[] calldata _isOperators) external onlyOwner {
    ...
    function setDefeConfig(address _veDEFE,address _virtualDefedToken) external onlyOwner{
        veDEFE = _veDEFE;
    ...
    function claimRewards(
        ...
        uint256 _amount,
        address user
    ) public override onlyOperator returns (uint256)  {
        if (_amount == 0) {
            return 0;
        }
        ...
        emit RewardsClaimed(user, amountToClaim,false);
     ...
     function claimAndLock(
        ...
        claimRewards(_assets,_amount,msg.sender);
        lockDefe(msg.sender,_amount,time);
        emit RewardsClaimed(msg.sender, _amount,true);
    }
    ...
    function lockDefe(address proxyAddr,uint256 amount,uint256 time) internal{
        ...
        IERC20Upgradeable(virtualDefedToken).approve(veDEFE,amount);
        if(locked.amount == 0){
...
contract DistributionManager is  Ownable {
    ...
    function getAssetsEmissionPerSecond(address[] calldata _assetAdrs)
        public
        view
        returns (uint256[] memory)
    {
        uint256[] memory emissionPerSeconds = new uint256[](_assetAdrs.length);
        for (uint256 i = 0; i < _assetAdrs.length; i++) {
            emissionPerSeconds[i] = assets[_assetAdrs[i]].emissionPerSecond;
        }
        return emissionPerSeconds;
    }
}
...
interface IIncentivesController {
    ...
    event RewardsClaimed(address indexed _user, uint256 _amount,bool isLock);
