...
contract Testyzz2 is ERC721A, ERC721AQueryable, Owned {

//project 
//for sale

  uint256 constant EXTRA_MINT_PRICE = 0.029 ether;
  uint256 TOTAL_SUPPLY = 7777;
  uint256 TOTAL_RESERVES = 3776;
  uint256 constant PUBLIC_SUPPLY_PLUS_ONE = 4002;
  uint256 constant MAX_PER_TRANSACTION_PLUS_ONE = 11;

    struct booleligible {
    bool iseligible;
    }
  mapping(address => booleligible) public emint;
  string tokenBaseUri = "ipfs://QmRSdQMbJ46fWo7HFNhF8gMDUkEqQCJ1H6ddWP2k1Li74i/";

  bool public paused = true;

  mapping(address => uint256) private _freeMintedCount;

  constructor() ERC721A("Testyzz2", "TSTZ") Owned(msg.sender) {
  }
  ...
  function mint(uint256 _quantity) external payable {
    unchecked {
      ...
      // require(
      //   msg.value == _quantity * EXTRA_MINT_PRICE,
      //   "INCORRECT ETH AMOUNT"
      // );

      require(emint[msg.sender].iseligible == true, "Normal Mint Issued");
  ...
  function freemint(uint256 _quantity) external payable {
    unchecked {
      ...
      // require(
      //   msg.value == (_quantity - 1) * EXTRA_MINT_PRICE,
      //   "INCORRECT ETH AMOUNT"
      // );

       require(emint[msg.sender].iseligible == false, "Free Claimed Missing");

      _mint(msg.sender, _quantity);
      emint[msg.sender].iseligible = true;
    }
  }
