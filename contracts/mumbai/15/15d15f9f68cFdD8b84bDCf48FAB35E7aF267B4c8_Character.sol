    ...
    CharacterData
{
    ...
    uint256 public constant ATTRIBUTE = 1;
    uint256 public constant CHARACTER_NFT_NUMBER = 1;
    uint256 public constant HAT_NFT_NUMBER = 2;
    // uint256 public constant ARMOR_NFT_NUMBER = 3;
    mapping(uint256 => string) tokenIdToURI;

    // string public constant tokenIdURI =
    //     "https://ipfs.io/ipfs/QmYEaS8wTbpi5XrN9aVMMKhREbFxaxBSAiDjeiZuFVpKWu";
    // string public constant updateTokenIdURI =
    ...
    constructor() ERC721("Character", "CHR") ERC3664(attrBaseURI) {
        _mint(CHARACTER_NFT_NUMBER, "CHARACTER", "character", "");
        _mint(HAT_NFT_NUMBER, "HAT", "hat", "");
    ...
    function _afterTokenMint(uint256 tokenId) internal virtual {
        ...
        uint256 id = Supply + tokenId * ATTRIBUTE;

        // HAT
        mintSubToken(HAT_NFT_NUMBER, tokenId, id + 1);
        ...
        string memory uri = bytes(currentBaseURI).length > 0
            ? string(
                abi.encodePacked(currentBaseURI, tokenId.toString(), extension)
            )
            : "";
