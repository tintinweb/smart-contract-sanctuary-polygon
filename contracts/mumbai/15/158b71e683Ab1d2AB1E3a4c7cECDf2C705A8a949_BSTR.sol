...
contract BSTR is ERC2771Context, ERC1155PresetMinterPauser, Ownable
, WNRoyalties 
, RevokableDefaultOperatorFilterer
{

    //Open sea metadata URI
    string private constant _contract_uri = 'ipfs://contractaddress';

    //Token collection data
    string private constant _name = 'BSTR';
    string private constant _symbol = 'BSTR';

    bool private _gelatoActive;

    //Constructor
    constructor(uint256 bps, address trustedForwarder)
        ERC1155PresetMinterPauser('ipfs://myaddress') 
         WNRoyalties(bps) ERC2771Context(trustedForwarder)
    {
        _gelatoActive = true;
    }
    ...
    function mintWithGelato(address to, uint256 id, uint256 amount, bytes memory data) external onlyTrustedForwarder {
    ...
    function mintBatchWithGelato(address to, uint256[] memory ids, uint256[] memory amounts, bytes memory data) external onlyTrustedForwarder {
        require(_gelatoActive, "Gelato is not active");
        require(hasRole(MINTER_ROLE, _msgSender()), "must have minter role to mint");
	...
	//Setup Royalties
	function setupRoyalties(address addr, uint256 bps) public onlyOwner {
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC1155PresetMinterPauser, WNRoyalties) returns (bool) {
        return ERC1155PresetMinterPauser.supportsInterface(interfaceId) || WNRoyalties.supportsInterface(interfaceId);
    } 
...
contract WNRoyalties is ERC165 {
