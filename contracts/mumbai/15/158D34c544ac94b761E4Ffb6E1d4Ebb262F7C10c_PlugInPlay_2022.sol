...
contract PlugInPlay_2022 is Ownable, RoyaltiesV2Impl, IERC1155MetadataURI, ERC1155  {
    ...
    uint96 public ROYALTIESPOINTS = 1000;
    ...
    function mintToUser(uint256 id, uint256 amount, bytes memory data) 
        ...
        require(msg.value == amount * _price, "Not enough ether sent");
        require(getTotalSupplyByTokenId(id) <= maxTokensPerId[id], "Not enough tokens left in the collection");
        require(balanceOf(msg.sender, id) <= maxTokensPerAddress[id], "Cannot mint more tokens with the same address");
