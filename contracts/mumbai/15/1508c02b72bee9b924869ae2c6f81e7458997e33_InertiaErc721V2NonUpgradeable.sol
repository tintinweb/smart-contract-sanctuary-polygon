    ...
    ERC721Burnable
{
    ...
    function grantRole(string memory role, address operator)
        public
        onlyRole(ADMIN_ROLE) {
        _grantRole(keccak256(abi.encodePacked(role)), operator);
    }

    function revokeRole(string memory role, address operator)
        public
        onlyRole(ADMIN_ROLE)
    {
        _revokeRole(keccak256(abi.encodePacked(role)), operator);
    }
