...
function upgradeCardCost(uint160[] calldata _tokenIds, uint8 _targetCardType, uint8 _collectionId, Coupon memory _coupon, uint _discount) public view validCardType(_targetCardType) returns (uint256 upgradeFeeCostWithDiscount, uint256 partnerFeeCost, uint256 cardsTypeValueDiffCost) {
    ...
    return (upgradecost.upgradeFeeCostWithDiscount, partnerFeeCost, cardsTypeValueDiffCost);
}
...
/**
  ...
  function upgradeCards(uint160[] calldata _tokenIds, uint8 _targetCardType, uint8 _collectionId,  Coupon memory _coupon, uint _discount, uint256 _value) public  validCardType(_targetCardType) {
    ...
    (uint256 upgradeFeeCost, uint256 partnerFeeCost, uint256 cardsTypeValueDiffCost) = upgradeCardCost(_tokenIds, _targetCardType, _collectionId, _coupon, _discount);
     ...
     require(_value >= upgradeFeeCost, "not enough funds sent");

      upgradeinfo.highestCardValue = 0;
      // Counts the total value of all the cards provided
      upgradeinfo.totalValue = 0;
      
      upgradeinfo.collectionsRatio = _collectionBuyBack  * 10**8 / buyBackValue ;
    ...
    
    {
      if(upgradeFeeCost > 0){
        ...
        bool success = eft.transferFrom(msg.sender, daoAddress, upgradeFeeCost);
      ...
      if(_value > (upgradeFeeCost + partnerFeeCost)) {
        bool success =  eft.transferFrom(gameTreasury, msg.sender, _value - (upgradeFeeCost + partnerFeeCost));
