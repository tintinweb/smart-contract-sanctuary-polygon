...
contract SuperCharge is AccessControl,Initializable{
    ...
    function claimSuperCharge(address user) external{
        ...
        for(uint i = userDetails[user].superChargeId+1; i <= end; i++){
            uint staked;
            if(superChargeRewards[i].previousStakedAmount[user] != 0){
                staked = superChargeRewards[i].previousStakedAmount[user];
            }else if(superChargeRewards[i].previousStakedAmount[user] == 0 && superChargeRewards[i].isEligible[user]){
                staked = 0;
            }else{
                staked = stakedAmount;
            }
