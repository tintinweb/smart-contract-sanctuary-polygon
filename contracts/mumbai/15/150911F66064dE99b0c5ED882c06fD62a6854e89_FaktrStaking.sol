...
contract FaktrStaking is ReentrancyGuardUpgradeable, BaseContract {
    ///
    ...
    struct PoolInfo {
        ...
        EnumerableSet.UintSet protocols;
        mapping(uint256 => uint256) faktInProtocols;
        mapping(uint256 => uint256) rewardPerFaktInProtocols;
    }
    ...
    function initialize(address _faktr, address _signer) public initializer {
        __BaseContract_init();
        signer = _signer;
        Fakt = IFakt(_faktr);
    }
    ...
    function deposit(
        ...
        require(_amount.length == _pId.length, "params not equal");

        for (uint256 i = 0; i < _amount.length; i++) {
            require(_amount[i] > 0, "Cannot stake 0");
        }
        // check admin signature
        bytes32 msgHash = keccak256(abi.encodePacked(_pId, _amount));
        ...
        for (uint256 i = 0; i < _pId.length; i++) {
            if (!user.protocols.contains(_pId[i])) {
                user.protocols.add(_pId[i]);
            }
            if (!pool.protocols.contains(_pId[i])) {
                pool.protocols.add(_pId[i]);
            }
            user.faktInProtocols[_pId[i]] = user.faktInProtocols[_pId[i]].add(
                _amount[i]
            );
            user.totalFakt = user.totalFakt.add(_amount[i]);
            pool.faktInProtocols[_pId[i]] = pool.faktInProtocols[_pId[i]].add(
                _amount[i]
            );
            Fakt.transferFrom(msg.sender, address(this), _amount[i]);
            emit Deposit(msg.sender, _amount[i], _pId[i]);
        }
    }
    ...
    function withdraw(
        ...
        PoolInfo storage pool = poolInfo;
        require(
            user.faktInProtocols[_pId] >= _amount,
    ...
    function burnOuts(
        ...
        for (uint256 i = 0; i < _burnUsers.length; i++) {
            ...
            user.faktInProtocols[_pId] = user.faktInProtocols[_pId].sub(
                _amounts[i]
            );
            user.debtInProtocols[_pId] = user
                .faktInProtocols[_pId]
                .mul(pool.rewardPerFaktInProtocols[_pId])
                .div(1e12);
            pool.faktInProtocols[_pId] = pool.faktInProtocols[_pId].sub(
                _amounts[i]
            );
    ...
    function claim() public nonReentrant {
        ...
        for (uint256 i = 0; i < user.protocols.length(); i++) {
            ...
            user.debtInProtocols[pId] = user
                .faktInProtocols[pId]
                .mul(pool.rewardPerFaktInProtocols[pId])
                .div(1e12);
        }
    ...
    function updateRewardPerFakt(uint256 _pId, uint256 _amount) external {
        require(
            msg.sender == purchase || msg.sender == owner(),
            "have no permission"
        );
        PoolInfo storage pool = poolInfo;
        ...
        if (faktInPool == 0) {
            ...
            pool.rewardPerFaktInProtocols[_pId] = pool
                .rewardPerFaktInProtocols[_pId]
    ...
    function getUserRewards(address _user) public view returns (uint256) {
        ...
        if (user.protocols.length() == 0) {
            ...
            for (uint256 i = 0; i < user.protocols.length(); i++) {
                uint256 rewardInProtocol = user
                    .faktInProtocols[user.protocols.at(i)]
                    .mul(pool.rewardPerFaktInProtocols[user.protocols.at(i)])
                    .div(1e12)
                    .sub(user.debtInProtocols[user.protocols.at(i)]);
        ...
        )
    {
        ...
        for (uint256 i = 0; i < numProtocols; i++) {
            ...
            rewardPerFaktInProtocols[i] = pool.rewardPerFaktInProtocols[
                pool.protocols.at(i)
            ];
        }
    ...
    function clearStuckEth(address _to, uint256 _amount) external onlyOwner {
        require(_to != address(0), "null address not allowed");
        require(_amount <= address(this).balance, "exceed amount");
        payable(_to).transfer(_amount);
    }

    function setPurchase(address _purchase) external onlyOwner {
        require(_purchase != address(0), "invalid address");
        purchase = _purchase;
    }
    ...
    event Withdraw(address indexed user, uint256 amount, uint256 protocolId);
    event Claim(address indexed user, uint256 amount);
    event Burn(address indexed user, uint256 amount, uint256 protocolId);
