...
contract ApprovalManagerFacet is IApprovals, IApprovalVerifier, IdentityVerifier {
    ...
    event ApprovalSet(uint256 indexed mnft, address target, bytes4 selector);
    event ApprovalDeleted(uint256 indexed mnft, address target, bytes4 selector);
    event ApprovalUsed(uint256 indexed mnft, address target, bytes4 selector);

    /**
     * @notice Set or change approval hook for specified operation
     * @param target Target contract which requires approval to send message to, zero address is wildcard for all contracts
     * @param selector Target method selector, 0x00000000 is wildcard selector used if there is no rule for specific selector
     * @param hook Hook data to call on the call to a target
     */
    function setApproval(
        ...
        address target,
        bytes4 selector,
        ApprovalHook calldata hook
    ) external {
        verifyAccess(mnft, IRules.Operation.MANAGE_RULES, credentials, msg.sender);
        _setApproval(mnft, wallet, target, selector, hook);
        emit ApprovalSet(mnft, target, selector);
    }
    ...
    function deleteApproval(
        ...
        verifyAccess(mnft, IRules.Operation.MANAGE_RULES, credentials, msg.sender);
        _deleteApproval(mnft, wallet, target, selector);
        emit ApprovalDeleted(mnft, target, selector);
    }

    /** @dev See {IApprovalVerifier} */
    function verifyApproval(
        ...
        require(ApprovalsStorage.layout().isWalletEnabled(mnft, wallet), "Wallet not enabled");
        ApprovalHook storage h = _findApprovalHook(mnft, wallet, target, selector);
        require(h.selector != bytes4(0), "Approval Hook not found");
        bytes memory hookMessage = abi.encodeWithSelector(h.selector, mnft, sender, target, message, h.initialData, h.actualData);
        address hookTarget = (h.target == address(0)) ? address(this) : h.target;
        //(bool success, bytes memory returnData) = h.target.call(dataWithSelector);
        (bool success, bytes memory returnData) = hookTarget.call(hookMessage);
        if (!success)
            assembly {
                revert(add(returnData, 32), returnData) // Reverts with an error message from the returnData
            }
        h.actualData = returnData;
        emit ApprovalUsed(mnft, target, selector);
        return true;
    }
    ...
    function _setApproval(
        ...
        address target,
        bytes4 selector,
        ApprovalHook memory hook
    ) private {
        require(hook.selector != bytes4(0), "Wrong hook selector");
        ApprovalsStorage.layout().setApprovalHook(mnft, wallet, target, selector, hook);
    }

    function _deleteApproval(
        uint256 mnft,
        address wallet,
        address target,
        bytes4 selector
    ) private {
        ApprovalsStorage.layout().deleteApprovalHook(mnft, wallet, target, selector);
    }

    /**
     * @notice Tries to find approval hook
     * @dev if no hook found - returns empty hook, which can be detected by zero selector
     */
    function _findApprovalHook(
        uint256 mnft,
        address wallet,
        address target,
        bytes4 selector
    ) internal view returns (ApprovalHook storage h) {
        ApprovalsStorage.ApprovalData storage ud = ApprovalsStorage.layout().getApprovalData(mnft, wallet);
        h = ud.approvals[target][selector]; // Check if exactly requested selector hook is set
        if (h.selector != bytes4(0)) return h;
        h = ud.approvals[target][bytes4(0)]; // Check if wildcard selector hook is set
        if (h.selector != bytes4(0)) return h;
        h = ud.approvals[address(0)][selector]; // Check if wildcard target with requested selector hook is set
        if (h.selector != bytes4(0)) return h;
        h = ud.approvals[address(0)][bytes4(0)]; // Return wildcard hook (if it is not set, it'll have zero selector)
    }
}
