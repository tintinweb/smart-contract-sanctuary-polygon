...
contract BetterTree is  ReentrancyGuard, ERC721Enumerable, Ownable{
    ...
    event treeRecycled(uint256 indexed token);
    ...
    address public vrf = 0x4F1e13b9f67C7C01275B4d5138b475A961015E60;//mumbai network
    ...
    constructor() ERC721("Betterverse Trees", "Tree"){

       _storedTrees.push(Tree(0,1,0)); //default metadata;
       _batchCID.push("https://gateway.pinata.cloud/ipfs/QmeH5SkPThZ4yTjM1vr6yaf63YRTkRECzuPCKS1aHjUnRX/"); //default tree holder
     ...
     */
    function dataReturn(uint256 _tokenId) public view returns(uint8, uint256, uint8){
        TokenData memory a = tokenInfo[_tokenId];
        return(a.causeID, a.treeValue, _storedTrees[a.treeID].rarity);
    }
     ...
     */
    function mint(uint8 _numberToMint, uint8 _cause, uint256 _treeValue, address _user)external isMinter{
    ...
    function _minting(address _user,  uint8 _cause, uint256 _treeValue)internal{
    ...
    function adminMint( uint8 count, address user, uint8 cause, uint256 value)external onlyOwner{
        ...
        for(uint8 i =0; i<count; i++){
            ...
            uint256 a = _storedTreesCount[3]-1;
            uint256 treeId = _rarityToTreeIndex[r][a];
            tokenInfo[tokenId].treeID =treeId;
            _rarityToTreeIndex[r].pop();
            _storedTreesCount[r] -=1;

            emit minted(tokenId, user);
        }
    }
    ...
    function setInitialTree(uint256 _tokenId, uint8 _cause, uint256 _treeValue, uint256 _time) internal{
     ...
     * @param rarity the rarity of the batch being passed in
     */

    function addTree(uint256 batchID, uint256[] memory jsonId, uint8[] memory rarity)external onlyOwner{

        require(batchID <  _batchCID.length, "Batch Does not exist");

        uint256 a = rarity.length;
        uint8 r = batchCat[batchID];
        require(a == jsonId.length, "Errors arrays need to be the same length");
        for(uint8 i =0; i<a; i++){
            _storedTrees.push(Tree(batchID, jsonId[i], rarity[i]));
    ...
    function tokenURI(uint256 _tree)public view override(ERC721) returns (string memory){

        require(_exists(_tree), "DNE");
        TokenData memory a = tokenInfo[_tree];
        Tree memory b = _storedTrees[a.treeID];
        uint8 c = _growthStage(_tree);
        string memory d;
        if(c==0){
            d="a";
        }
    ...
    function addValue(uint256 _amount, uint256 _tokenId, uint8 _causeId)public isMinter{
