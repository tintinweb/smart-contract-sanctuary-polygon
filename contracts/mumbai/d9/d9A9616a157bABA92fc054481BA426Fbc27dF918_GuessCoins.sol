...
contract GuessCoins is Ownable {
    ...
    function buy(uint256 value_, uint256 share_) public {
    ...
    function draw() public {
        ...
        uint256 value;
        for (uint8 i = 0; i < 5; i++) value += uint256(uint8(hash[i]));
        address[] memory winners = _chices[value % 2];
        uint256[] memory shares = new uint256[](winners.length);
        for (uint256 i = 0; i < winners.length; i++) {
            shares[i] = _shares[winners[i]];
        }
        emit Draw(winners, block.difficulty);
        _reset();
    }
}
...
interface IPlatform {
    ...
    function availableAmountOf(address project_, uint256 amount_)
        external
        view
        returns (uint256);
