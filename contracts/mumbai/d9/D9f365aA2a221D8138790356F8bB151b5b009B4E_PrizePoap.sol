...
contract PrizePoap is IPrizePoap, Ownable, ERC1155Supply, ReentrancyGuard {
    ...
    function mint(uint256 tokenId) external nonReentrant {
        if (!checkMintable(msg.sender, tokenId)) revert DontMeetRequirements();
        require(balanceOf(msg.sender, tokenId) == 0, "You have already minted");
        ...
        ICommunityPassport.Passport memory passport = passportContract
            .getPassport(msg.sender);
        require(
            passport.exp >= _prizeList[tokenId].requiredExp,
            "Not enough exp"
        );
    ...
    function checkMintable(
        ...
        if (balanceOf(target, tokenId) > 0) return false;
        uint32 communityId = _prizeList[tokenId].communityId;
        address passportAddr;
        (, passportAddr, , ) = communityPortal.getCommunity(communityId);
        ICommunityPassport passportContract = ICommunityPassport(passportAddr);
        ICommunityPassport.Passport memory passport = passportContract
            .getPassport(target);
        if (passport.exp < _prizeList[tokenId].requiredExp) return false;
        return true;
    }
    ...
    function checkBatchMintable(
        ...
        bool[] memory mintableList = new bool[](tokenIds.length);
        for (uint256 i; i < tokenIds.length; i++) {
            mintableList[i] = checkMintable(targets[i], tokenIds[i]);
        }
        return mintableList;
    }
