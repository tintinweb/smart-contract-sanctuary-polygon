...
contract Near8v3 is ERC721, ERC721URIStorage, Ownable, CantBeEvil(LicenseVersion.CBE_CC0) {
    ...
    //mapping of momentId to owner's addres and momentId to web2UsersId
    mapping(uint256 => address) internal momentIdToOwner;

    //Event which emits when a new moment is created
    event CreatedMoment(address indexed _to, uint256 indexed _momentId, string indexed _momentURI);

    //function to mint moment by userscreate hash and which executes _mint and assign a tokenId of the moment to holders address
    function createMoment(address to, string memory uri) public payable {
        to = msg.sender;
        require (msg.value >= 1 ether, 'Need to pay up!');

        uint256 newMomentId = _momentIdCounter.current();
        _momentIdCounter.increment();
        momentIdToOwner[newMomentId] = msg.sender;
        _safeMint(to, newMomentId);
        _setTokenURI(newMomentId, uri);
        emit CreatedMoment(msg.sender, newMomentId, uri);
    }
        ...
        onlyOwner
    {
        ...
        _momentIdCounter.increment();
        momentIdToOwner[tokenId] = to;
        _safeMint(to, tokenId);
        _setTokenURI(tokenId, uri);
        emit CreatedMoment(msg.sender, tokenId, uri);
    }

    function transfer(address _to, uint256 _momentId) public {
        require(msg.sender == momentIdToOwner[_momentId]);
        momentIdToOwner[_momentId] = _to;
