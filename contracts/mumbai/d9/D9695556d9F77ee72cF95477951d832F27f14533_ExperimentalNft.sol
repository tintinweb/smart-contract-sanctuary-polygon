...
contract ExperimentalNft is ERC1155MultiUri, Ownable {
    ...
    function mintSpecial(
        ...
        _mint(to, nftId, 1, "");
        attributes[nftId] = Attr(_name, _symbol);
    }

    function makeUri(uint256 tokenId) public view returns (string memory) {
        string memory json = Base64.encode(
            bytes(string(
                abi.encodePacked(
                    '{"name": "', attributes[tokenId].name, '",',
                    '"image_data": "', "https://api.mining.game/1.json", /* '",' ,
