...
// TODO: delete this code as soon as the new contracts are deployed!
interface IMarketLegacy {
  struct EpochInfo {
    uint32 latestExecutedEpochIndex;
    uint80 latestExecutedOracleRoundId;
  }

  function get_epochInfo() external view returns (EpochInfo memory);
}
...
contract Shifting is AccessControlledAndUpgradeable {
  ...
  event ShiftOrdersExecutedUntilIndex(address indexed marketFrom, uint256 executeUpUntilAndIncludingThisIndex);
  ...
  function _getPoolTokenPrice(
    ...
    uint32 currentExecutedEpoch = IMarketLegacy(market).get_epochInfo().latestExecutedEpochIndex;

    price = IMarket(market).get_poolToken_priceSnapshot(currentExecutedEpoch, poolType, poolIndex);
  }
  ...
  function _executeShiftOrder(address _marketFrom, uint256 _executeUpUntilAndIncludingThisIndex) internal {
    ...
    emit ShiftOrdersExecutedUntilIndex(_marketFrom, _executeUpUntilAndIncludingThisIndex);
  }
