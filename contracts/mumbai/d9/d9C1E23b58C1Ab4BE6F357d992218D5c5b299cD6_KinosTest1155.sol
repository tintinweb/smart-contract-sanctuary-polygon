...
contract KinosTest1155 is ERC1155, Pausable, AccessControlEnumerable, IERC2981 {
    ...
    mapping (uint256 => Poap) poaps; /* poap_id -> poap */
    ...
    event PoapUpdated(uint256 indexed token_id, string name, uint256 price, uint256 max);
    ...
    constructor(string memory _uri, address payable _beneficiary, address _signer, address[] memory admins) ERC1155(_uri) {
        ...
        setPoap(1, "Kinos x Slush 2022", 0, 1000000000);
    ...
    function setPoap(uint256 id, string memory _name, uint256 _price, uint256 _max) public onlyRole(DEFAULT_ADMIN_ROLE) {
        Poap storage poap = poaps[id];
        poap.name = _name;
        poap.price = _price;
        poap.max = _max;
        emit PoapUpdated(id, _name, _price, _max);
    }

    function getPoap(uint256 id) external view returns (Poap memory) {
    ...
    function mint(uint256 amount, uint256 slotId, uint256 tokenId, uint256 validUntil, bytes memory signature) external payable whenNotPaused {
        ...
        // Load poap
        Poap storage poap = poaps[tokenId];

        // Check amount. Also prevents minting for non-existent tokens.
        require(poap.supply + amount <= poap.max, "Cannot mint over maximum");

        // Check price.
        if (poap.price > 0) {
            require(msg.value >= (amount * poap.price), "Insufficient eth sent");
        }
    ...
    function mint(uint256 amount, uint256 slotId, uint256 tokenId, uint256 validUntil, bytes memory signature, address to) external payable whenNotPaused {
        // Check signature.
        require(_canMint(to, amount, slotId, tokenId, validUntil, signature), "Must have valid signing");
        ...
        require(!_slots.get(slotId), "Slot already used");
        _slots.set(slotId);
        emit SlotUsed(slotId, tokenId, amount, to);
    ...
    function batchMint(address[] memory accounts, uint256[] memory tokenIds, uint256[] memory amounts) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        for (uint256 i = 0; i < accounts.length; i++) {
            Poap storage poap = poaps[tokenIds[i]];
            poap.supply += amounts[i];
            require(poap.supply <= poap.max, "Cannot mint over maximum amount of poap");
    ...
    function totalSupply(uint256 id) external view returns (uint256) {
        Poap storage poap = poaps[id];
        require(poap.max > 0, "No such tokenId");
        return poap.supply;
    }
