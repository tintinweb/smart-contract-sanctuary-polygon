    ...
    Ownable
{
    ...
    uint256 private priceSize;
    mapping(uint256 => CurrencyPrice) private prices;
    ...
    ) ERC721(_symbol, _name)
    {
        ...
        bytes32 _couponAdminRole = keccak256("COUPON_ADMIN_ROLE");

        _setupContractURI(_contractURI);
        _setupRole(DEFAULT_ADMIN_ROLE, _defaultAdmin);
        _setupRole(_minterRole, _defaultAdmin);
        _setupRole(_transferRole, _defaultAdmin);
        _setupRole(_couponAdminRole, _defaultAdmin);
        ...
        couponAdminRole = _couponAdminRole;
    }

    function s2b(string calldata text) public pure returns (bytes32) {
        return bytes32(bytes(text));
    }

    function b2s(bytes32 text) public pure returns (string memory) {
        return string(abi.encodePacked(text));
    }

    function _getCouponIdx(bytes32 _coupon) private view returns (uint256) {
        for (uint256 i = 0; i < couponSize; i++) {
            if (coupons[i].id == _coupon) {
                return i;
            }
        }
        return couponSize + 1;
    ...
    function getCouponSize()
        public view
        onlyRole(couponAdminRole)
        returns (uint256)
    {
        return couponSize;
    }
    ...
    {
        uint256 couponIdx = _getCouponIdx(s2b(_coupon));
        Coupon memory coupon = Coupon(s2b(_coupon), _discount, _totalSupply, 0, _expireTimestamp);
        if (couponIdx > couponSize) {
            coupons[couponSize] = coupon;
            couponSize++;
        }
        else
        {
    ...
    function removeCoupon(
        ...
        onlyRole(couponAdminRole)
    {
        uint256 _id = _getCouponIdx(s2b(_coupon));
        require (_id < couponSize, "no coupon found");
        delete coupons[_id];
        couponSize -= 1;
    }
    ...
    {
        string[] memory couponIds = new string[](couponSize);
        for (uint256 i = 0; i < couponSize; i++) {
            couponIds[i] = b2s(coupons[i].id);
        }
        return couponIds;
    }
    ...
    function _getPriceIdx(address _currency, bytes32 _tier)
        private view returns (uint256)
    {
        for (uint256 i = 0; i < priceSize; i++) {
            if (prices[i].currency == _currency && prices[i].tier == _tier) {
                return i;
            }
        }
        return priceSize + 1;
    ...
    {
        bytes32 tier = s2b(_tier);
        uint256 _id = _getPriceIdx(_currency, tier);
        CurrencyPrice memory price = CurrencyPrice(_currency, tier, _price);
        if (_id > priceSize) {
            prices[priceSize] = price;
            priceSize += 1;
        } else {
            prices[_id] = price;
        }
    ...
    function removePrice(
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        uint256 _id = _getPriceIdx(_currency, s2b(_tier));
        require(_id < priceSize, "no price available for currency");
        delete prices[_id];
        priceSize -= 1;
    }
    ...
    {
        address[] memory _currencies = new address[](priceSize);
        for (uint256 i = 0; i < priceSize; i++) {
            _currencies[i] = prices[i].currency;
        }
        return _currencies;
    }

    function getTiers(address _currency)
        public view
        onlyRole(DEFAULT_ADMIN_ROLE)
        returns (string[] memory)
    {
        string[] memory _tiers = new string[](priceSize);
        for (uint256 i = 0; i < priceSize; i++) {
            if (prices[i].currency == _currency) {
                _tiers[i] = b2s(prices[i].tier);
            }
        }
        return _tiers;
    }

    function getPrice(address _currency, string calldata _tier)
        public view
        onlyRole(DEFAULT_ADMIN_ROLE)
        returns (uint256)
    {
    ...
    function calcPrice(
        ...
        (, discountedPrice) = _calcPrice(_coupon, _currency, _tier);
    }
    ...
    function _calcPrice(
        ...
        address _currency,
        string calldata _tier
    ) private view returns (Coupon memory coupon, uint256 discountedPrice) {
        uint256 couponIdx = _getCouponIdx(s2b(_coupon));

        require(couponIdx < couponSize, "no coupon found");

        coupon = coupons[couponIdx];

        require (coupon.expireTimestamp > block.timestamp || coupon.expireTimestamp == 0, "coupon already expired");

        require (coupon.usedCount < coupon.totalSupply, "coupons already used");

        uint256 priceIdx = _getPriceIdx(_currency, s2b(_tier));
        require (priceIdx < priceSize, "no price available for currency");

        uint256 basePrice = prices[priceIdx].price;
        discountedPrice = basePrice - ((basePrice * coupon.discount)/100);
    }
    ...
    function claimWithCoupon(
        ...
        (Coupon memory coupon, uint256 discountedPrice) = _calcPrice(_coupon, _currency, _tier);
        coupon.usedCount += 1;
        ...
        safeTransferFrom(ownerOf(_tokenId), _msgSender(), _tokenId);
    }
    ...
    function safeMintWithTier(
        ...
        string calldata _tier,
        bytes calldata _data
    ) public onlyRole(minterRole) returns (uint256  startTokenId) {

        lazyMint(_amount, _baseURIForTokens, _tier, _data);

        startTokenId = _currentIndex;

        uint256 startIdToMap = startTokenId;
        uint256 remaningToDistribute = _amount;

        uint256 qtyFulfilled = _getQuantityFulfilledByTier(_tier, remaningToDistribute);
        ...
        _mapTokensToTier(_tier, startIdToMap, qtyFulfilled);

        totalRemainingInTier[_tier] -= qtyFulfilled;
        totalsForTier[keccak256(abi.encodePacked(_tier, "minted"))] += qtyFulfilled;
    ...
    function _canRestrictTransfers() internal view virtual returns (bool) {
        return hasRole(transferRole, _msgSender());
    }
