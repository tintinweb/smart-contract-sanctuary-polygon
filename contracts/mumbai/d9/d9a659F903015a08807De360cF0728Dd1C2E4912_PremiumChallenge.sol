...
contract PremiumChallenge is ReentrancyGuard {
    ...
    function setWinners(address[] memory _winnersWallets) public senderIsAddress onlyOwner challengeWinnersIsNotSet challengeStateIsValidForSettingWinners {
        // validate wallets
        ...
        uint256[] memory _rewards = this.getRewardsDistibution(participantWallets.length);
    ...
    function getRewardsDistibution(uint256 _participantsCount) public senderIsAddress view returns (uint256[] memory) {
        uint256 _minReward = 1;
        uint256 _balance = this.getPrizePool();
        if (_balance == 0 || _participantsCount <= 0)
            return new uint256[](0);
        bool _condition = true;
        uint256 _remaningPrizePool = _balance;
        uint256 _size = 0;
        while (_condition) {
            bool _isLast = _remaningPrizePool < metaData.entryFee || _remaningPrizePool < _minReward || _size == _participantsCount;
            if (_isLast) {
                _condition = false;
            } else {
        ...
        uint256 _iterator = 0;
        while (_condition) {
            bool _isLast = _remaningPrizePool < metaData.entryFee || _remaningPrizePool < _minReward || _iterator + 1 == _participantsCount;
            if (_isLast) {
    ...
    // is participated
    function isParticipated(address _wallet) public senderIsAddress view returns (bool) {
    ...
    // get challenge state
    function getChallengeState() public senderIsAddress view returns (string memory) {
        ChallengeState _challengeState = forecastChallengeState();
    ...
    // get metaData
    function getMetaData() public senderIsAddress view returns (MetaData memory) {
        return metaData;
    }

    // get wallets
    function getParticipantWallets() public senderIsAddress onlyOwner view returns (address[] memory) {
    ...
    // get prize Pool
    function getPrizePool() public senderIsAddress view returns (uint256) {
        if (isWinnersSet)
            return balance;
