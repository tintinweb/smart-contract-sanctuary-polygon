...
contract WowTPoints is OwnableUpgradeable, AccessControlUpgradeable {
    ...
    function updateLeaderBoard(address account, uint256 totalPoints) private {
        ...
        // If the user is already in the leaderboard, remove their address
        if (userIndex < topLeaderBoardAddress.length) {
            ...
            topLeaderBoardAddress[topLeaderBoardAddress.length - 1] = address(
                0
            );
        }

        // Find the index to insert the user in the leaderboard based on their updated points
        uint256 insertIndex = topLeaderBoardAddress.length;
        for (uint256 i = 0; i < topLeaderBoardAddress.length; i++) {
            if (totalPoints > getPoints(topLeaderBoardAddress[i])) {
                insertIndex = i;
                break;
            }
        }

        // Shift the existing leaderboard entries to make room for the new entry
        for (
            uint256 i = topLeaderBoardAddress.length - 1;
            i > insertIndex;
            i--
        ) {
            topLeaderBoardAddress[i] = topLeaderBoardAddress[i - 1];
        ...
        // Insert the user's address into the leaderboard at the correct position
        topLeaderBoardAddress[insertIndex] = account;
    }
