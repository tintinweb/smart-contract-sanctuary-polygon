  ...
  ReentrancyGuardUpgradeable
{
  ...
  function initialize(
    ...
    (repeatMintNum,mintDeadlineTime,isNotWhitelist) = abi.decode(_newParameter,(uint256,uint256,bool));
    require(repeatMintNum > 0, "Must be greater than 0");
  }
  ...
  function mintEth() public payable nonReentrant returns (uint256 tokenId) {
    ...
    if (msg.value - rate > 0) {
      (bool success, ) = receivingAddress.call{value: msg.value - rate}(
        ""
      );
  ...
  function withdraw() public nonReentrant onlyOwner{
    if (address(erc20) == address(0)) {
      emit Withdraw(platform, _getBalance());

      (bool success, ) = payable(platform).call{value: _getBalance()}(
        ""
      );
      require(success, "Failed to send Ether");
    } else {
      uint256 amount = IERC20Upgradeable(erc20).balanceOf(address(this)); // get the amount of erc20 tokens reserved in contract
      IERC20Upgradeable(erc20).safeTransfer(platform, amount); // transfer erc20 tokens to contract owner address
