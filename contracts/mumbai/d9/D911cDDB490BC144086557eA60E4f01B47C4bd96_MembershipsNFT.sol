...
contract MembershipsNFT is ERC721URIStorage,Ownable{
  ...
  function mintMembership(address _to,uint _memberType, uint  _role, bool  _sex, bytes32 _name,  uint _loyaltyPoints, string memory _tokenURI,address _walletAddress) public onlyOwner {
    ...
    addressId[_to] = membershipCounter;
     // increment counter
    membershipCounter++;
  }

  function getId(address _to) public view returns(uint){
    return addressId[_to];
  }
  ...
  function updatePoint(uint[] memory _arrPoint, uint[] memory _ids) public onlyOwner{
    for(uint i = 0; i < _arrPoint.length - 1; i++) {
      Membership storage membership = memberships[_ids[i]];
