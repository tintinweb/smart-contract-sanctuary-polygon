...
contract BUSDOneV2 {
    token public BUSD = token(0x25Ca4E40c20c6e5a2e5eF7E8b207F94C4DfF1981);
    oldContract public OldContract = oldContract(0xa4f3209ef68493e089c9105dC1841Da4dF2de643);
	...
	function withdraw() external {
		...
		if(isOldStaker(msg.sender)) {
			require(user.lastWithdraw + 1 days < block.timestamp, "Not time to claim yet.");
		}
		else {
			require(user.lastWithdraw + 3 days < block.timestamp, "Not time to claim yet.");
		}
		...
		BUSD.transfer(developer, withdrawAmount * developer_percent / PERCENT_DIVIDER);
		BUSD.transfer(project, withdrawAmount * project_percent / PERCENT_DIVIDER);
		BUSD.transfer(leader, withdrawAmount * leader_percent / PERCENT_DIVIDER);
