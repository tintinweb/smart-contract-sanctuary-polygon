...
import {Account} from "../kycs/Account.sol";
import {BaseRoyalty} from "../royalties/BaseRoyalty.sol";
import {RoyaltyPool} from "../royalties/RoyaltyPool.sol";
import {SignatureValidator} from "../../utils/SignatureValidator.sol";
import {IEscrow, IERC165} from "./IEscrow.sol";

/**
 * @title EscrowUpgradeable
 * @notice This contract manages the escrow system for a marketplace trading ERC721 and ERC20 tokens.
 * It's upgradeable using the UUPS (Universal Upgradeable Proxy Standard) mechanism.
 * @dev Uses OpenZeppelin and other contracts for secure, standard compliant and upgradeable structures.
 */
contract EscrowUpgradeable is IEscrow, MarketHubRegistrarUpgradeable, SignatureValidator, UUPSUpgradeable {
    uint256 private saleIdCounter;
    mapping(uint256 => Sale) private sales;
    mapping(address => mapping(uint256 => bool)) private activeSales;
    uint256 public buyerChallengeWindow;
    uint256 public saleFundingWindow;

    /// @dev The constructor initializes the base contract.
    /// @custom:oz-upgrades-unsafe-allow constructor
    constructor() {
    ...
    /**
     * @dev Initializes the contract with the provided name.
     * Initializes the upgradeable parts of the contract, the signature validator and the market hub registrar.
     * Sets the buyerChallengeWindow to 48 and the saleFundingWindow to 24.
     * Emits two events: BuyerChallengeWindowChanged with the new value of buyerChallengeWindow,
     * and SaleFundingWindowChanged with the new value of saleFundingWindow.
     *
     * Requirements:
     * - This function can only be called once, at the time of contract initialization.
     *
     * @param _name string The name for the contract.
     */
    function initialize(string memory _name) public initializer {
        __UUPSUpgradeable_init();
        __SignatureValidator_init(_name, "v1.0");
        __MarketHubRegistrar_init();
        buyerChallengeWindow = 48;
        saleFundingWindow = 24;

        emit BuyerChallengeWindowChanged(buyerChallengeWindow);
        emit SaleFundingWindowChanged(saleFundingWindow);
    }

    /**
     * @notice Only functions with this modifier can be called by a registered market
     */
    modifier onlyMarket() {
        require(marketHub.isMarket(msg.sender), "Only market can call this function");
        _;
    }
     ...
     */
    function setBuyerChallengeWindow(uint256 _hours) public virtual onlyAdmin {
        buyerChallengeWindow = _hours;
        emit BuyerChallengeWindowChanged(buyerChallengeWindow);
    }
     ...
     */
    function setSaleFundingWindow(uint256 _hours) public virtual onlyAdmin {
        saleFundingWindow = _hours;
        emit SaleFundingWindowChanged(saleFundingWindow);
    }
    ...
    function createSale(
        ...
        require(marketHub.isERC20Registered(_erc20), "ERC20 not registered");
        require(marketHub.isERC721Registered(_erc721), "ERC721 not registered");
        ...
        require(!activeSales[_erc721][_tokenId], "Active sale already exists for this token");
        require(_seller != _buyer, "Buyer and seller cannot be the same actor");

        Account memory seller = marketHub.getAccount(_seller);
        require(marketHub.isRoyaltyRegistered(seller.tier), "Royalty not registered");

        IColleCollection colleCollection = IColleCollection(_erc721);
        require(colleCollection.isSaleMetadataSet(_tokenId), "Token sale metadata not set");

        _createSale(_buyer, _spender, _erc20, _price, _seller, _erc721, _tokenId, _payNow);
    }
    ...
    function _createSale(
        ...
        IColleCollection colleCollection = IColleCollection(_erc721);
        string memory ipfsHash = colleCollection.getSaleMetadata(_tokenId);

        // Create a new sale record
        uint256 newSaleId = saleIdCounter;
        State startingState = _payNow
            ? State.PendingSale
            : (_buyer == _spender ? State.AwaitingERC20Deposit : State.AwaitingSettlement);
        ...
        // Increment the saleIdCounter
        saleIdCounter++;

        activeSales[_erc721][_tokenId] = true;

        IVault vault = marketHub.getVault();
        vault.depositColleNFT(_erc721, _tokenId, _seller);
        if (_payNow) {
            vault.depositERC20(_erc20, _price, _spender);
        }
    }
     ...
     */
    function getSale(uint256 _saleId) external view virtual returns (Sale memory) {
    ...
    function hasActiveSale(address _erc721, uint256 _tokenId) external view virtual returns (bool) {
        return activeSales[_erc721][_tokenId];
    }

    /**
     * @notice Updates the state of a sale.
     * Can only be called by the buyer, the seller, or the colle.
     * @param _saleId The id of the sale.
     * @param _newState The new state of the sale.
     */
    function updateSale(uint256 _saleId, State _newState) external virtual {
        _updateSale(msg.sender, _saleId, _newState);
    }

    /**
     * @notice Allows a signer to permit the update of a sale's state.
     * @dev The function is virtual, can be overridden and can only be called by the Colle if the signature is valid.
    ...
    {
        _updateSale(_signer, _saleId, _newState);
    }
    ...
    function supportsInterface(
        ...
        return
            _interfaceId == type(IEscrow).interfaceId ||
            _interfaceId == type(IERC165).interfaceId ||
            AccessControlUpgradeable.supportsInterface(_interfaceId);
    }

    /**
     * @dev Internal function to update the state of a sale. Only the buyer, seller or the Colle can update the sale.
     * @param _sender The address initiating the sale update.
     * @param _saleId The id of the sale.
     * @param _newState The new state of the sale.
     */
    function _updateSale(address _sender, uint256 _saleId, State _newState) internal virtual {
        Sale memory sale = sales[_saleId];

        require(
            _sender == sale.buyer || _sender == sale.seller || isRelayer(_sender),
            "Only buyer, seller or Colle can update sale"
        );

        if (_newState == State.SaleCancelled) {
            _updateSaleToSaleCancelled(_sender, sale);
        } else if (_newState == State.PendingSale) {
            _updateSaleToPendingSale(_sender, sale);
        } else if (_newState == State.ProcessingSale) {
            _updateSaleToProcessingSale(_sender, sale);
        } else if (_newState == State.ShippingToBuyer) {
            _updateSaleToShippingToBuyer(_sender, sale);
        } else if (_newState == State.ShippingToColleForAuthentication) {
            _updateSaleToShippingToColleForAuthentication(_sender, sale);
        } else if (_newState == State.ColleProcessingSale) {
            _updateSaleToColleProcessingSale(_sender, sale);
        } else if (_newState == State.IssueWithProduct) {
            _updateSaleToIssueWithProduct(_sender, sale);
        } else if (_newState == State.IssueWithDelivery) {
            _updateSaleToIssueWithDelivery(_sender, sale);
        } else if (_newState == State.SaleSuccess) {
            _updateSaleToSaleSuccess(_sender, sale);
        } else if (_newState == State.Received) {
            _updateSaleToReceived(_sender, sale);
        } else if (_newState == State.ShippingToColleForDispute) {
            _updateSaleToShippingToColleForDispute(_sender, sale);
        }
    }

    /**
     * @dev Internal function to transition a sale to the "SaleCancelled" state.
     * @param _sender The address initiating the sale update.
     * @param _sale The sale struct in memory
     */
    function _updateSaleToSaleCancelled(address _sender, Sale memory _sale) internal virtual {
        require(
            _sale.state == State.AwaitingERC20Deposit ||
                _sale.state == State.PendingSale ||
                (_sale.state == State.AwaitingSettlement && isRelayer(_sender)) ||
                (_sale.state == State.ProcessingSale && (_sender == _sale.seller || isRelayer(_sender))),
            "Invalid state transition"
        );

        if (_sale.state == State.AwaitingERC20Deposit && _sender != _sale.buyer) {
            // solhint-disable-next-line not-rely-on-time
            require(block.timestamp >= _sale.createdTimestamp + saleFundingWindow * 1 hours, "Sale funding window");
        }

        bool fundsWereDeposited = _sale.state != State.AwaitingERC20Deposit && _sale.state != State.AwaitingSettlement;

        activeSales[_sale.erc721][_sale.tokenId] = false;

        _updateSaleState(_sale.id, State.SaleCancelled);
        emit SaleCancelled(_sale.id, _sale.spender, _sale.seller);

        IVault vault = marketHub.getVault();
        if (fundsWereDeposited) {
            vault.withdrawERC20(_sale.erc20, _sale.price, _sale.spender);
        }
        vault.withdrawColleNFT(_sale.erc721, _sale.tokenId, _sale.seller);

        // NOTE: We do not need to notify the markets that the sale has been closed
        // This is because the sale never progressed to a point where the item was shipped,
        // The seller still has it in their possession and we are not aware of any defects
        // Existing listing or offers will be unaffected by this sale being cancelled
    }

    /**
     * @dev Internal function to transition a sale to the "PendingSale" state.
     * @param _sender The address initiating the sale update.
     * @param _sale The id of the sale.
     */
    function _updateSaleToPendingSale(address _sender, Sale memory _sale) internal virtual {
        require(
            (_sale.state == State.AwaitingERC20Deposit && _sender == _sale.buyer) ||
                (_sale.state == State.AwaitingSettlement && isRelayer(_sender)),
            "Invalid state transition"
        );

        _updateSaleState(_sale.id, State.PendingSale);

        IVault vault = marketHub.getVault();
        vault.depositERC20(_sale.erc20, _sale.price, _sale.spender);
    }

    /**
     * @dev Internal function to transition a sale to the "ProcessingSale" state.
     * @param _sender The address initiating the sale update.
     * @param _sale The id of the sale.
     */
    function _updateSaleToProcessingSale(address _sender, Sale memory _sale) internal virtual {
        require(_sale.state == State.PendingSale && _sender == _sale.seller, "Invalid state transition");

        _updateSaleState(_sale.id, State.ProcessingSale);
    }

    /**
     * @dev Internal function to transition a sale to the "ColleProcessingSale" state.
     * @param _sender The address initiating the sale update.
     * @param _sale The id of the sale.
     */
    function _updateSaleToShippingToBuyer(address _sender, Sale memory _sale) internal virtual {
        require(
            (_sale.state == State.ProcessingSale && _sender == _sale.seller) ||
                (_sale.state == State.ColleProcessingSale && isRelayer(_sender)),
            "Invalid state transition"
        );

        if (_sale.state == State.ProcessingSale) {
            require(
                !marketHub.getRoyalty(marketHub.getAccount(_sale.seller).tier).requiresManualAuthentication(),
                "Seller tier does requires manual authentication"
            );
        }

        _updateSaleState(_sale.id, State.ShippingToBuyer);
    }

    /**
     * @dev Internal function to transition a sale to the "ShippingToColleForAuthentication" state.
     * @param _sender The address initiating the sale update.
     * @param _sale The id of the sale.
     */
    function _updateSaleToShippingToColleForAuthentication(address _sender, Sale memory _sale) internal virtual {
        require(_sale.state == State.ProcessingSale && _sender == _sale.seller, "Invalid state transition");
        require(
            marketHub.getRoyalty(marketHub.getAccount(_sale.seller).tier).requiresManualAuthentication(),
            "Seller tier does not require manual authentication"
        );

        _updateSaleState(_sale.id, State.ShippingToColleForAuthentication);
    }

    /**
     * @dev Internal function to transition a sale to the "ShippingToColleForDispute" state.
     * @param _sender The address initiating the sale update.
     * @param _sale The id of the sale.
     */
    function _updateSaleToColleProcessingSale(address _sender, Sale memory _sale) internal virtual {
        require(
            _sale.state == State.ShippingToColleForAuthentication && isRelayer(_sender),
            "Invalid state transition"
        );

        _updateSaleState(_sale.id, State.ColleProcessingSale);
    }
     ...
     * @param _sale The id of the sale.
     */
    function _updateSaleToIssueWithProduct(address _sender, Sale memory _sale) internal virtual {
        require(
            (_sale.state == State.ColleProcessingSale || _sale.state == State.ShippingToColleForDispute) &&
                isRelayer(_sender),
            "Invalid state transition"
        );

        activeSales[_sale.erc721][_sale.tokenId] = false;
        _updateSaleState(_sale.id, State.IssueWithProduct);
        emit SaleCancelled(_sale.id, _sale.spender, _sale.seller);
        ...
        // As item has a issue, notify the markets
        marketHub.notifySaleClosed(_sale.id);
    }
     ...
     * @param _sale The id of the sale.
     */
    function _updateSaleToIssueWithDelivery(address _sender, Sale memory _sale) internal virtual {
        require(
            (_sale.state == State.ShippingToColleForAuthentication ||
                _sale.state == State.ShippingToBuyer ||
                _sale.state == State.ShippingToColleForDispute) && isRelayer(_sender),
            "Invalid state transition"
        );

        activeSales[_sale.erc721][_sale.tokenId] = false;
        _updateSaleState(_sale.id, State.IssueWithDelivery);
        emit SaleCancelled(_sale.id, _sender, _sender);

        IVault vault = marketHub.getVault();
        vault.withdrawERC20(_sale.erc20, _sale.price, _sender);
        vault.withdrawColleNFT(_sale.erc721, _sale.tokenId, _sender);

        // As item has has a issue with delivery and may be lost/damanged, notify the markets
        marketHub.notifySaleClosed(_sale.id);
    }
     ...
     * @param _sale The id of the sale.
     */
    function _updateSaleToSaleSuccess(address _sender, Sale memory _sale) internal virtual {
        require(
            _sale.state == State.Received || (_sale.state == State.ShippingToColleForDispute && isRelayer(_sender)),
            "Invalid state transition"
        );

        if (_sale.state == State.Received && _sender != _sale.buyer) {
            require(
                // solhint-disable-next-line not-rely-on-time
                block.timestamp >= _sale.receivedTimestamp + buyerChallengeWindow * 1 hours,
                "Seller or Colle must wait until challenge window completes"
            );
        }

        activeSales[_sale.erc721][_sale.tokenId] = false;
        _updateSaleState(_sale.id, State.SaleSuccess);

        Account memory seller = marketHub.getAccount(_sale.seller);
        BaseRoyalty royalty = marketHub.getRoyalty(seller.tier);
        RoyaltyPool royaltyPool = marketHub.getRoyaltyPool();

        (uint256 royaltyPoolAmount, uint256 commissionPoolAmount) = royalty.getRoyaltyBreakdown(
            _sale.erc20,
            _sale.price
        );

        uint256 initialOwnerWeight = royaltyPool.getInitialOwnerWeight();
        uint256 totalPoolShares = royaltyPool.totalPoolShares(_sale.erc721, _sale.tokenId);

        uint256 initialOwnerRoyaltyPayout = (royaltyPoolAmount * initialOwnerWeight) / totalPoolShares;
        uint256 recentOwnerRoyaltyPayout = royaltyPoolAmount / totalPoolShares;
        uint256 amountToSeller = _sale.price - royaltyPoolAmount - commissionPoolAmount;

        _payout(_sale, commissionPoolAmount, initialOwnerRoyaltyPayout, recentOwnerRoyaltyPayout, amountToSeller);

        royaltyPool.trackNewOwner(_sale.erc721, _sale.tokenId, _sale.seller);

        // As item has been sold, cancel all offers/listings
        marketHub.notifySaleClosed(_sale.id);
    }

    /**
     * @dev Internal function to payout the seller, colle, and royalty pool after a sale completes.
     * @param _sale The id of the sale.
     * @param _colleComissionPoolAmount The amount of  comission funds to be distributed to the relayer
     * @param _initialOwnerRoyaltyPayout The amount of royalty funds to be distributed the initial seller
     * @param _recentOwnerRoyaltyPayout The amount of royalty funds to be distributed to each recent seller in the royalty pool
     * @param _amountToSeller The price of the sale, minus the royalty and comission pool amounts, to be sent to the current seller
     */
    function _payout(
        Sale memory _sale,
        ...
        address claimedInitialOwner = royaltyPool.getInitialOwner(_sale.erc721, _sale.tokenId);
        address initialOwner = claimedInitialOwner == address(0) ? _sale.seller : claimedInitialOwner;
        address[4] memory recentOwners = royaltyPool.getRecentOwners(_sale.erc721, _sale.tokenId);

        emit CommissionPayout(_sale.id, marketHub.getColleComissions(), _colleComissionPoolAmount);
        emit RoyaltyPayout(_sale.id, initialOwner, _initialOwnerRoyaltyPayout);
        for (uint256 i = 0; i < recentOwners.length; i++) {
            if (recentOwners[i] != address(0)) {
                emit RoyaltyPayout(_sale.id, recentOwners[i], _recentOwnerRoyaltyPayout);
            }
        }
        emit SaleComplete(_sale.id, _amountToSeller);

        // Distribute the royalty pool amount
        vault.withdrawERC20(_sale.erc20, _initialOwnerRoyaltyPayout, initialOwner);
        for (uint256 i = 0; i < recentOwners.length; i++) {
            if (recentOwners[i] != address(0)) {
                vault.withdrawERC20(_sale.erc20, _recentOwnerRoyaltyPayout, recentOwners[i]);
            }
        }

        // Distrbute the commission to Colle
        vault.withdrawERC20(_sale.erc20, _colleComissionPoolAmount, marketHub.getColleComissions());

        // Distribute remaining amount to seller & transfer NFT to buyer
        vault.withdrawERC20(_sale.erc20, _amountToSeller, _sale.seller);
        vault.withdrawColleNFT(_sale.erc721, _sale.tokenId, _sale.buyer);
    }
     ...
     * @param _sale The id of the sale.
     */
    function _updateSaleToReceived(address _sender, Sale memory _sale) internal virtual {
        require((_sale.state == State.ShippingToBuyer && isRelayer(_sender)), "Invalid state transition");

        // solhint-disable-next-line not-rely-on-time
        sales[_sale.id].receivedTimestamp = block.timestamp;

        _updateSaleState(_sale.id, State.Received);
    }
     ...
     * @param _sale The id of the sale.
     */
    function _updateSaleToShippingToColleForDispute(address _sender, Sale memory _sale) internal virtual {
        require((_sale.state == State.Received && _sender == _sale.buyer), "Invalid state transition");

        _updateSaleState(_sale.id, State.ShippingToColleForDispute);
    }

    function _updateSaleState(uint256 _saleId, State state) internal virtual {
        sales[_saleId].state = state;
        emit UpdateSale(sales[_saleId].id, state);
    }

    /**
     * @dev Internal function called on UUPS upgrade to verify that the contract is approved to update to the new implementation
     * @param _newImplementation The address of the new implementation.
     */
    function _authorizeUpgrade(address _newImplementation) internal override {
        require(
            marketHub.getUpgradeGatekeeper().getUpgradeTarget(address(this)) == _newImplementation,
    ...
    /**
     * @dev Internal function to get the expected EIP-712 signed hash to permit updating a sale's state
     * @param _signer The address of the signer.
     * @param _saleId The id of the sale.
     * @param _newState The new state of the sale.
