...
contract DaoProxy is BaseContract {
    ...
    function getUserData(
        ...
        uint256 _day,
        UserInfo memory UserInfo_,
        UserInvestItem memory UserInvestItem_,
        UserDaoItem memory UserDaoItem_,
        address[] memory referrals_,
        address[] memory allReferrers_,
        uint256 _staticReward,
        uint256[] memory _daoRewardList,
        BaseItem memory baseItem_,
        BaseItem2 memory baseItem2_
    ) {
        // 获取当前时间的年月日
        ...
        // 获取用户的静态收益总额
        _staticReward = _getStaticReward(_user);
        // 初始化 Dao 奖励列表并计算每个 Dao 等级的奖励总额
        _daoRewardList = new uint256[](4);
        _daoRewardList[0] = _getDaoReward(_user, 1);
        _daoRewardList[1] = _getDaoReward(_user, 2);
        _daoRewardList[2] = _getDaoReward(_user, 3);
        _daoRewardList[3] = _getDaoReward(_user, 4);
        baseItem_ = BaseItem(daoToken, totalCompensation, investIndex, tatalInvestAmount, remainingCredit, lastInvestTime, investEndIndex, compensationInterval, rewardAllRateList, weekReward);
        baseItem2_ = BaseItem2(weekTop1AddressList, rewardRateList, rewardRateListForInviteRanking, rewardRateListForInvestRanking, addressConfig);
    }
