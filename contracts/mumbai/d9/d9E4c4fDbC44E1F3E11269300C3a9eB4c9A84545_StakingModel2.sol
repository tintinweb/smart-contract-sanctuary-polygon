...
contract StakingCalculations{
    ...
    // @notice Calculate early withdraw reward tokens
    // @param timestamp of staked tokens
    // @param Amount of staked tokens
    // @param Days of maturity
    // @param earlyRewardAPY
    // @return early withdraw reward tokens
    function calculateEarlyWithdrawReward(uint256 timeStaked, uint256 amount, uint daysOfMaturity, uint earlyRewardAPY) _positive(amount) internal pure returns(uint256){
    ...
    function calculateMaturityRewardSeparate(uint256 staker, uint maturityRewardAPY) internal pure returns(uint256){
        return (staker.mul(maturityRewardAPY)).div(100);
    }
...
contract StakingModel2 is StakingCalculations{
    ...
    uint256 public maturityRewardAPY; //APY %age for maturity reward
    uint256 public earlyRewardAPY;  // APY %age for early reward
    ...
    constructor (
        ...
        stakingEnds = (block.timestamp).add(daysOpenForStake*86400); //(10*24*60*60);
        earlyWithdrawStarts = (block.timestamp).add(daysOfMandatoryLock*86400); //(10*24*60*60);
        earlyWithdrawEnds = (earlyWithdrawStarts).add(259200);//(3*24*60*60);
    ...
    function stake(uint256 _amount) _positive(_amount) public returns (bool _success) {
        ...
        uint256 newMaturityTime = block.timestamp.add(daysOfMaturity.mul(86400));
    ...
    function withdrawEarlySeparate(uint256 _id) _after(earlyWithdrawStarts) _before(earlyWithdrawEnds) public returns(bool){
        ...
        uint256 noOfDays = block.timestamp.add(daysOfMaturity.mul(86400));
        noOfDays = StakingCalculations.getNumberOfDays(noOfDays.sub(stakerDetails[_id].maturityTime));
        uint256 earlyReward = StakingCalculations.calculateEarlyWithdrawReward(noOfDays, calcStakes, daysOfMaturity, earlyRewardAPY);
    ...
    function withdrawEarlyCombined(uint _id) _after(earlyWithdrawStarts) _before(earlyWithdrawEnds) public returns(bool){
        ...
        for(uint256 i = 0; i<lenArray; i++){
            ...
            // Calculate number of days of maturity difference
            uint256 noOfDays = (block.timestamp).add(daysOfMaturity.mul(86400)); //10 days
            noOfDays = getNumberOfDays(noOfDays.sub(stakerDetails[id].maturityTime));

            // Calculate and add the early reward and stakes for the account 
            uint256 reward = StakingCalculations.calculateEarlyWithdrawReward(noOfDays, stakerDetails[id].stakeAmount, daysOfMaturity, earlyRewardAPY);
    ...
    // @notice claimedEarlyRewards function returns early rewards claimed by staker
    // @param staker(address): address of the staker
    // @return Returns uint256 amount of early rewards claimed 
    function claimedEarlyRewards(address staker) public view returns(uint256){
        return details[staker].totalClaimedEarlyReward;
    }

    // @notice claimedAfterMaturityRewards function returns after maturity rewards claimed by staker
    // @param staker(address): address of the staker
    // @return Returns uint256 amount of after maturity rewards claimed 
    function claimedAfterMaturityRewards(address staker) public view returns(uint256){
        return details[staker].totalClaimedMaturityReward;
    }

    // @notice claimedTotalRewards function returns total rewards claimed by staker
    // @param staker(address): address of the staker
    // @return Returns uint256 amount of total rewards claimed 
    function claimedTotalRewards(address staker) public view returns(uint256){
        return ((details[staker].totalClaimedEarlyReward).add(details[staker].totalClaimedMaturityReward));
    }

    // @notice checkEarlyRewardSeparate function calculates the amount of early reward tokens for separate model
    // @param id (uint256): id of staked tokens
    // @param staker(address): address of the staker
    // @return Returns uint256 amount of rewards calculated
    function checkEarlyRewardSeparate(uint256 id, address staker) public view returns(uint256){

        // Check if id contains stake amount and total stake amount is positive
        require(stakerDetails[id].stakeAmount > 0, "Id not present");
        require(search(staker, id) != MAX, "Id doesnt belong to this account");

        // Total days staked and calculate early reward
        uint256 noOfDays = block.timestamp.add(daysOfMaturity.mul(86400)); //10 days
        noOfDays = getNumberOfDays(noOfDays.sub(stakerDetails[id].maturityTime));
        return calculateEarlyWithdrawReward(noOfDays, stakerDetails[id].stakeAmount, daysOfMaturity, earlyRewardAPY);
    }

    // @notice checkEarlyRewardCombined function calculates the amount of early reward tokens for combined model
    // @param id (uint256): id of staked tokens
    // @param staker(address): address of the staker
    // @return Returns uint256 amount of rewards calculated 
    function checkEarlyRewardCombined(uint256 id,address staker) public view returns(uint256){

        // Check if id contains stake amount
        require(stakerDetails[id].stakeAmount > 0, "Id not present");
        uint256 earlyReward;
        uint256 searchId = search(staker, id);
        require(searchId != MAX, "Id doesnt belong to this account");
        uint lenArray = stakerIDs[staker].length;
        uint256 _id;
        for(uint256 i = 0; i<lenArray; i++){
            _id = stakerIDs[staker][i];
            ...
            // Calculate and add the early reward and stakes for the account 
            uint256 reward = StakingCalculations.calculateEarlyWithdrawReward(noOfDays, stakerDetails[_id].stakeAmount, daysOfMaturity, earlyRewardAPY);
            earlyReward = (earlyReward).add(reward);
            if(id == _id){
                break;
            }
    ...
    // @notice checkAfterMaturityCombinedReward function calculates the amount of reward tokens after maturity for combined model
    // @return Returns uint256 amount of rewards calculated
    function checkAfterMaturityCombinedReward(address staker) public view returns(uint256){
        ...
        // Calculate all rewards of the staker till the id
        for(uint256 i = 0; i<lenArray; i++){
            id = stakerIDs[staker][i];
            ...
            if(i == lenArray-1){
                ...
                noOfDays = getNumberOfDays(stakerDetails[stakerIDs[staker][i+1]].maturityTime.sub(stakerDetails[stakerIDs[staker][i]].maturityTime));
            }
        ...
        return maturityReward;
    }

    // @notice checkAfterMaturitySeparateReward function calculates the amount of reward tokens after maturity for separate model
    // @param id (uint256): id of staked tokens
    // @return Returns uint256 amount of rewards calculated
    function checkAfterMaturitySeparateReward(uint256 id) public view returns(uint256){
        return stakerDetails[id].maturityRewardSep;
    }
