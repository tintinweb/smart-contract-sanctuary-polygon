...
import "../core/Authority.sol";
import "./IAgentExtension.sol";

contract AgentExtension is Initializable, Authority, IAgentExtension {
    using EnumerableSet for EnumerableSet.AddressSet;
    using EnumerableMap for EnumerableMap.AddressToUintMap;

    event Bind(address indexed inviter_, address indexed user_);
    event AddFundFlow(
    ...
    mapping(address => mapping(address => uint256)) private _fundFlows;

    function initialize(address platform_) external initializer {
        Authority.__Authority_init();
        Authority._addAuthority(AUTHORITY_SUPER, platform_);
        Authority._removeAuthority(AUTHORITY_SUPER, _msgSender());
    }

    // ===> PRIVATE METHODS
    function _bind(address inviter_, address user_) internal {
        require(
            inviter_ != user_,
            "the inviter is must different from the user"
        );
        _inviters[user_] = inviter_;
        _agents[inviter_].add(user_);
        emit Bind(inviter_, user_);
    }

    function _queryInvister(address user_) internal view returns (address) {
        return _inviters[user_];
    }

    function _queryInvisteCount(address inviter_)
        internal
        view
        returns (uint256)
    {
        return _agents[inviter_].length();
    }
    ...
    function _addFundFlow(
        ...
        _fundFlows[user_][token_] += amount_;
        emit AddFundFlow(user_, token_, amount_);
    }

    function _queryFundFlow(address user_, address token_)
        internal
        view
        returns (uint256)
    {
        return _fundFlows[user_][token_];
    }

    // <=== PRIVATE METHODS

    // ===> PUBLIC METHODS
    function bind(address inviter_, address user_)
        external
        override
        onlyHasAuthority(AUTHORITY_SUPER, msg.sender)
    {
        _bind(inviter_, user_);
    }

    function queryInvister(address user_)
        external
        view
        override
        returns (address)
    {
        return _queryInvister(user_);
    }
    ...
    function queryInvisteCount(address inviter_)
        ...
        returns (uint256)
    {
        return _queryInvisteCount(inviter_);
    }
    ...
    function addFundFlow(
        ...
        _addFundFlow(user_, token_, amount_);
    }
    ...
    function queryFundFlow(address user_, address token_)
        ...
        returns (uint256)
    {
        return _queryFundFlow(user_, token_);
    }
    // <=== PUBLIC METHODS
}
