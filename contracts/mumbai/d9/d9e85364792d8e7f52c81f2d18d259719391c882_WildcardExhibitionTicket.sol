...
import "seadrop/ERC721PartnerSeaDrop.sol";

contract WildcardExhibitionTicket is ERC721PartnerSeaDrop {
    error ExhibitionAlreadyOccurred();
    error ExhibitionHasNotOccurred();

    /**
     * @notice Start time of the exhibition (in seconds since unix epoch time).
     *         Set in the constructor when the contract is deployed and then can be modified
     *         by contract owner until the exhibition starts.
     */
    uint256 public exhibitionStartTime;
    /// @notice mapping from ticket (token) Id to the Exhibition attendee, which is the address that held the ticket when the exhibition started
    mapping(uint256 => address) public ticketIdToAttendee;

    /// @notice Initialize the ticket contract with the exhibition start time
    constructor(
        string memory name,
        string memory symbol,
        address administrator,
        address[] memory allowedSeaDrop,
        uint256 _exhibitionStartTime
    ) ERC721PartnerSeaDrop(name, symbol, administrator, allowedSeaDrop) {
        exhibitionStartTime = _exhibitionStartTime;
    }

    /// @notice The contract owner can update the exhibition time as long as the exhibition has not yet started
    function setExhibitionStartTime(uint256 _exhibitionStartTime) public onlyOwner {
        if (exhibitionOccurred()) {
            revert ExhibitionAlreadyOccurred();
        }

        exhibitionStartTime = _exhibitionStartTime;
    }

    /// @notice Checks whether the exhibition has occurred
    function exhibitionOccurred() public view returns (bool) {
        // block.timestamp is in seconds
        return block.timestamp > exhibitionStartTime;
    }

    /// @notice Rips each ticket into a ticket stub, creating an onchain snapshot of who attended the event
    ///         i.e. Records the wallets that held each ticket when the event started
    function ripTickets(uint256 _startToken, uint256 _endToken) public onlyOwner {
        // cannot rip tickets if the exhibition has not occurred
        if (!exhibitionOccurred()) {
            revert ExhibitionHasNotOccurred();
        }

        // make sure start token makes sense. Can't start before the first token id
        if (_startToken < _startTokenId()) {
            _startToken = _startTokenId();
        }

        // make sure end token makes sense. Can't end after the last minted token
        if (_endToken > _totalMinted()) {
            _endToken = _totalMinted();
        }

        for (uint256 i = _startToken; i <= _endToken; ++i) {
            // skip recording attendance if the ticket does not exist
            if (!exists(i)) {
                continue;
            }

            // skip recording attendance if it has already been recorded for this ticket Id
            if (ticketIdToAttendee[i] != address(0)) {
                continue;
            }

            // record the ticket owner as attending the event
            address ticketOwner = ownerOf(i);
            ticketIdToAttendee[i] = ticketOwner;
        }
    }
