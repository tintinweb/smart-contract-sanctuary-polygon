  ...
  DefaultOperatorFilterer
{
  ...
  ) ERC721A("Shikkubo", "Shikkubo") PaymentSplitter(_payees, _shares) {
  ...
  function nonFreeAmount(address _owner, uint256 _amount) external view returns (uint256) {
    return _calculateNonFreeAmount(_owner, _amount);
  }
  ...
  function mint(uint256 _amount) external payable {
    uint256 _nonFreeAmount = _calculateNonFreeAmount(msg.sender, _amount);
  ...
  function _calculateNonFreeAmount(address _owner, uint256 _amount) internal view returns (uint256) {
    uint256 _freeAmountLeft = _numberMinted(_owner) >= mintRules.freePerWallet
      ? 0
      : mintRules.freePerWallet - _numberMinted(_owner);
