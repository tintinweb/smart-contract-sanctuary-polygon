...
contract NFT721 is ERC721, ERC721Enumerable, ERC721URIStorage, ERC721Burnable, AccessControl, IERC2981 {
    ...
    constructor() ERC721("Collectible", "AE") {
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(MINTER_ROLE, msg.sender);
    }

    function safeMint(address to, string memory _tokenURI, uint8 _royaltyPercentage) public onlyRole(MINTER_ROLE) {
    ...
    function safeMintBatch(address to, string[] memory _tokenURIList, uint8 _royaltyPercentage) public onlyRole(MINTER_ROLE) {
        for (uint i = 0; i < _tokenURIList.length; i++) {
            uint tokenId = _tokenIdCounter.current();

            _safeMint(to, tokenId);
            _setTokenURI(tokenId, _tokenURIList[i]);
            _tokenIdCounter.increment();
            setRoyalty(tokenId, to, _royaltyPercentage);
        }
    }
