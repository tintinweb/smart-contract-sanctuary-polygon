...
contract NFT1155 is ERC1155 {
     ...
     *
     * @param _copies, No of copies to be minted for NFT
     * @param _tokenURI, ipfs/image metadata URL for the NFT.
     * @return bool returns true on successful transaction
     */
    function mint(uint256 _copies, string memory _tokenURI)
        external
        returns (bool)
    {
        require(bytes(_tokenURI).length > 0, "Invalid URI");
        _mint(msg.sender, tokenId, _copies, "");
    ...
    function batchMintByOwner(
        address[] memory to,
        uint256[] memory _copies,
        string[] memory _tokenURI
    ) external returns (bool) {
        require(
            to.length == _tokenURI.length && to.length == _copies.length,
            "Length Mismatch"
        );
        for (uint256 i; i < to.length; i++) {
            require(
                mintByOwner(to[i], _copies[i], _tokenURI[i]),
                "Mint failed"
            );
        }
    ...
    function mintToMarketPlace(
        ...
        _mint(marketPlace, tokenId, copies, "");
