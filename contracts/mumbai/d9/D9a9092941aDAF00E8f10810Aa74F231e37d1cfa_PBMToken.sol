...
// File: contracts/interfaces/IPBM.sol
...
contract PBMToken is ERC20Pausable, AccessControl, IPBM {
    ...
    bytes32 public constant MERCHANT_ADMIN_ROLE =
        keccak256("MERCHANT_ADMIN_ROLE");
    ...
    modifier onlyApprovedMerchant(address recipient) {
        require(
            hasRole(MERCHANT_ROLE, recipient),
            "recipient not an approved merchant"
        );
        _;
    }
    ...
    constructor(
        ...
        owner = _msgSender();
        // Initialises the base TSGD token
    ...
    function wrapMint(
        address toUser,
        uint256 amount
    ) external onlyOwner whenNotExpired {
    ...
    function redeem(
        address toUser,
        uint256 amount
    ) external whenNotExpired onlyApprovedMerchant(toUser) {
    ...
    function withdraw() external onlyOwner returns (bool) {
        require(
            block.timestamp > contractExpiry,
            "contract expiry not reached"
        );
    ...
    function recover(address account) external onlyOwner returns (uint256) {
        uint256 overBalance = underlyingToken.balanceOf(address(this)) -
            totalSupply();
