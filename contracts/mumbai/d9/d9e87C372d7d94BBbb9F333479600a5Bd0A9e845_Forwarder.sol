...
contract FactToken is ERC20,  ERC20Burnable {
    ...
    uint256 constant private _INITIAL_SUPPLY = 2_000_000_000_000 * 10 ** 18;

    constructor() ERC20("FACTIIV", "FACT") {
...
import "./FactToken.sol";

contract Forwarder is Ownable {
  uint256 private constant PRECISION = 10**18;
  uint256 public constant userFee = 25000000000000000; // 2.5% precision 18

  uint256 public immutable factsTokenInitialSupply;
  address public factiiv;
  address public immutable factsToken;

  mapping (address => uint256) public replayNonce;
  mapping(bytes4 => uint256) public priceList;
  
  event Deployed(address sender, address factsToken);
  event Response(bool, bytes);
  event GaslessFee(address indexed user, bytes4 functionSel, uint256 txFeeFACTS, uint256 networkFeeFACTS, bool isBurning, uint256 burnFACTS, uint256 netFeeFACTS);
  event NewPrice(address indexed sender, bytes4 functionSignature, uint256 price);

  constructor(address factsTokenAddr) {
    factsToken = factsTokenAddr;
    factsTokenInitialSupply = FactToken(factsTokenAddr).totalSupply();
    emit Deployed(msg.sender, factsTokenAddr);
  }


  function isBurning() internal view returns(bool){
    return (factsTokenInitialSupply / 2) > FactToken(factsToken).totalSupply() ? false : true;
  }
  ...
  function updateFunctionCallPrice(bytes4 signature, uint256 price) public onlyOwner {
      priceList[signature] = price;
      emit NewPrice(_msgSender(), signature, price);
  }
//  function append(string memory a,string memory b) internal returns(string memory)
//  {
//    return string.concat(a,b);
//  }
  function processTransactionFee(bytes4 sig) internal {
    address sender = _msgSender();
    uint256 networkFee;
    uint256 txFee;
    uint256 burnAmount;
    uint256 netFee = txFee;
    bool burning = isBurning();

    bytes4 functionSignature = sig;
    uint256 price = priceList[functionSignature];
    // require(price > 0, "Forwarder::processTransactionFee: Function Price Not Set");
    txFee = price;

    require(IFactiiv(factiiv).getUserBalance(sender) > price, "Forwarder.processTransactionFee:: NSF");
    IFactiiv(factiiv).toForwarder(_msgSender(), txFee);

    networkFee = txFee * userFee / PRECISION;
    netFee = txFee - networkFee;

    // burn 1/2 of network fee if burning

    if(burning) {
      burnAmount = networkFee / 2;
      networkFee = networkFee - burnAmount;
      if(burnAmount > 0) FactToken(factsToken).burn(burnAmount);
    }
    
    // network fee to Forwarder owner

    FactToken(factsToken).transfer(owner(), networkFee);

    // netFee to relay

    IERC20(factsToken).transfer(msg.sender, netFee);

    emit GaslessFee(sender, functionSignature, txFee, networkFee, burning, burnAmount, netFee);
  }  
  ...
  public 
  {
    bytes4 funcSig = getSelector(data);
    // this is punative. Increase gas cost prior to abort.
    processTransactionFee(funcSig);
    require(block.timestamp < validTill,"Forwarder.metaTransfer:: message signature expired");
    
    // allow unpriced (subsidized) transactions
