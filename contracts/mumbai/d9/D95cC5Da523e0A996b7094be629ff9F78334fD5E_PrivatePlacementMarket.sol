...
contract PrivatePlacementMarket is OwnerPausable {
    ...
    event BagChanged(uint256[] bag0, uint256[] bag1, uint256[] bag2, uint256[] bag3);
    ...
    function initialize(
        ...
        address tdex) public initializer {
            ...
            _stockNftIds.add(100);
            _stockNftIds.add(2200);
            _stockNftIds.add(12000);
            _stockNftIds.add(39000);
    ...
    function buy(uint256 index) external whenNotPaused {
        ...
        //exchange half to tt
        IERC20Upgradeable(_usdtAddress).safeTransfer(_usdtIncomeAddress, requireUsdt/2);
        ...
        IERC20Upgradeable(_ttAddress).safeTransferFrom(_ttPayOutAddress, address(_stackingReward), ttAmount);
    ...
    function withdrawStockMargin() external whenNotPaused {
        ...
        for(uint256 i; i < ids.length; i++) {
            uint256 amount_ = _stockERC1155.balanceOf(msg.sender, ids[i]);
            if (amount_ > 0) {
                _stockERC1155.burn(msg.sender, ids[i], amount_);
            }
        }
        (uint256 principal, uint256 amount) = _stockMargin.withdrawMargin(msg.sender);
        emit MarginWithdrawn(msg.sender, principal, amount);
    }
    ...
    function setBag(
        ...
        _bags[0] = Bag(bag0[0], bag0[1], bag0[2]);
        _bags[1] = Bag(bag1[0], bag1[1], bag1[2]);
        _bags[2] = Bag(bag2[0], bag2[1], bag2[2]);
        _bags[3] = Bag(bag3[0], bag3[1], bag3[2]);
        ...
        emit BagChanged(bag0, bag1, bag2, bag3);
    }
    ...
    function setMarginWithdrawAddress(address marginWithdrawAddr) external onlyOwnerOrOperator {
        _stockMargin.setMarginWithdrawAddress(marginWithdrawAddr);
    }


    function setTTPayOutAddress(address ttPayOutAddress_) external onlyOwnerOrOperator {
        _ttPayOutAddress = ttPayOutAddress_;
    }
    ...
    function ttPayOutAddress() external view returns(address) {
        return _ttPayOutAddress;
    }
    ...
    function marginWithdrawAddress() external view returns(address) {
        return _stockMargin.marginWithdrawAddress();
    }
}
...
contract StockMargin is OwnerPausable {
    ...
    //     address usdtAddress, 
    //     address marginDepositAddr,
    //     address marginWithdrawAddr) OwnerPausable(owner) {
    //     _usdtAddress = usdtAddress;
    //     _marginDepositAddress = marginDepositAddr;
    //     _marginWithdrawAddress = marginWithdrawAddr;
    // }
...
contract StakingReward is OwnerPausable {
    ...
    function initialize(
        ...
        address ttAddress, 
        address ttPayOutAddress) public initializer {
        __OwnablePausable_init(owner);
    ...
    function calculatePendingRewards(address staker) public view returns(uint256 rewardDebt, uint256 pendingRewards) {
        ...
        for (uint256 i; i < userOrders.length; i++) {
            pendingRewards += _calculatePendingReward(userOrders[i], user.level);
        }
        return (user.rewardDebt, pendingRewards);
    }

    function _calculatePendingReward(OrderInfo memory userOrder, uint256 userLevel) internal view returns(uint256 pendingReward) {
        ...
        if (lastRewardBlock >= apyLevels[apySize-1].startBlock) {
            ...
            pendingReward += userOrder.remainedAmount*multiplier*apyLevels[apySize-1].apy*BASE_REWARD_PER_BLOCK/PRECISION_FACTOR;
        } else {
            ...
            for (uint256 j; j<apySize-1; j++) {
                ...
                if (matches >= 1) {
                    ...
                    pendingReward += userOrder.remainedAmount*multiplier*apyLevels[j].apy*BASE_REWARD_PER_BLOCK/PRECISION_FACTOR;
                    matches = 2;
                }
            }
            uint256 multiplier_ = _getMultiplier(apyLevels[apySize-1].startBlock, block.number, endBlock);
            pendingReward += userOrder.remainedAmount*multiplier_*apyLevels[apySize-1].apy*BASE_REWARD_PER_BLOCK/PRECISION_FACTOR;
        }
    }
    ...
    function claim() external whenNotPaused {
        ...
        IERC20Upgradeable(_ttAddress).safeTransferFrom(_ttPayOutAddress, staker, claimAmount);
        emit RewardsClaim(staker, user.rewardDebt, pendingRewards);
    }

    function withdraw(uint256 amount) external whenNotPaused returns(uint256 withdrawn){
    ...
    function withdrawMock(uint256 amount) external {
        ...
        IERC20Upgradeable(_ttAddress).safeTransferFrom(_ttPayOutAddress, msg.sender, withdrawn);
    }
    ...
    function _withdraw(address staker, uint256 amount) internal returns(uint256 withdrawn) {
        ...
        for (uint256 i; i < len; i++) {
            ...
            uint256 totalRelease = userOrders[i].totalAmount*period/RELEASE_CYCLE_TIMES;
            if (totalRelease > userOrders[i].releasedAmount) {
                uint256 pendingWithdrawAmount = totalRelease - (userOrders[i].totalAmount - userOrders[i].remainedAmount);
                if (pendingWithdrawAmount == 0) { //all releasedAmount was withdrawn 
                    continue;
                }

                pendingRewards += _calculatePendingReward(userOrders[i], user.level); //must happen before state changed
                userOrders[i].releasedAmount = totalRelease;
