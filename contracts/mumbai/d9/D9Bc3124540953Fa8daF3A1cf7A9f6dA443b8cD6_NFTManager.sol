    ...
    ResourceImplementationPointer
{
	...
	modifier checkPermissionsMultiple(uint256[] memory _nftIds) {
        address sender = _msgSender();
    ...
    function initialize(address _treasury) external initializer {
        ...
        changeCreateMinValue(1000 * (10**18)); // 1,000 Resources
        changeCompoundDelay(7200); // 2h
        changeRewardPerDay(34724);
        changeCompoundFee(0);
        changeCashoutFee(0);
        mintPrice = 100000000000000000;
        _baseURIExtended = "ipfs://QmZ5ozPL3C8D3yGPJf2M4uiaWeR5xgVCAAW4aqwMDyW5hA/";
    }
    ...
    function airdropNFTs(uint256 nftNumber) external nonReentrant whenNotPaused onlyOwner {
        ...
        require(
            nftNumber > 0 && nftNumber < 101,
            "NFTs: NFT Number should set between 1-100"
        );
        ...
        for (uint256 index = 0; index < nftNumber; index++) {
            ...
            require(
                maxSupply >= newNFTId,
                "NFTs: total supply can never exceed max supply"
            );
    ...
    function createNFTWithTokens(
        uint256 nftNumber,
        uint256 nftValue
    ) external payable nonReentrant whenNotPaused resourceSet {
        address sender = _msgSender();

        require(
            nftNumber > 0 && nftNumber < 11,
            "NFTs: NFT Number should set between 1-10"
        );
        require(
            nftValue >= createMinValue * nftNumber || nftValue == 0,
            "NFTs: NFT value set below minimum"
        );
        require(
            resource.balanceOf(sender) >= nftValue * nftNumber,
            "NFTs: Balance too low for creation"
        );
        require(
            mintPrice * nftNumber== msg.value,
            "NFTs: msg.value == mint_price * number"
        );

        if (nftValue > 0) {
            resource.accountBurn(sender, nftValue * nftNumber);
        }

        uint balance = address(this).balance;
        payable(treasury).transfer(balance);

        totalValueLocked += nftValue * nftNumber;
    ...
    function stakeTokensOnNFT(
        ...
        require(
            ...
            "NFTs: Balance too low for stake"
        );
        ...
        require(
            !isProcessable(nft),
            "NFTs: You must wait by next time"
        );
    ...
    function cashoutReward(uint256 _nftId)
        ...
        resourceSet
    {
        uint256 reward = _getNFTCashoutRewards(_nftId);
        ...
        resourceSet
    {
        ...
        for (uint256 i = 0; i < nftsOwned.length; i++) {
            rewardsTotal += _getNFTCashoutRewards(nftsOwned[i]);
        }
    ...
    function _calculateRewardsFromValue(
        ...
        return (rewards * _nftValue) / 100000;
    }
    ...
    function changeCreateMinValue(uint256 _createMinValue) public onlyOwner {
        require(
            _createMinValue > 0,
            "NFTs: Minimum price to create a NFT must be above 0"
        );
        createMinValue = _createMinValue;
    }
    ...
    function changeRewardPerDay(uint256 _rewardPerDay) public onlyOwner {
        require(
            _rewardPerDay > 0,
            "NFTs: rewardPerDay must be greater than 0"
        );
    ...
    function changeMaxSupply(uint256 _maxSupply) public onlyOwner {
        require(_maxSupply > 0, "NFTs:  max supply must be greater than 0");
