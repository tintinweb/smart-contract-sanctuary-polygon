...
contract MyContract is
  ...
  event LogFee(uint256 fee, uint256 blance);
...
// Selector for the destination chain (in the CCIP protocol)
  ...
  function enableChain(uint64 chainSelector, uint256 gasLimit, bool strict) external onlyOwner {
    bytes memory extraArgs = Client._argsToBytes(Client.EVMExtraArgsV1({gasLimit: gasLimit, strict: strict}));
  ...
  function ccipReceive(Client.Any2EVMMessage calldata message) external override onlyRouter validChain(message.sourceChainSelector) {
  ...
  function transferToken(
    ...
    require(sourceToken.balanceOf(msg.sender) >= amount, 'Insufficient balance');
    ...
    emit LogFee(fee, feeToken.balanceOf(msg.sender));
    // require(feeToken.balanceOf(msg.sender) >= fee, 'Insufficient balance fee');
    // s_feeToken.transferFrom(msg.sender, address(this), fee);

    // bytes32 messageId = router.ccipSend{value: fee}(destChainSelector, message);
