...
contract Facilitator is IFacilitator, Ownable, Pausable, ReentrancyGuard, EIP712 {
    ...
    modifier onlyListingOwner(address nft) {
        ...
        internal
    {
        ...
        if (tokenAddress == address(1)) {
            ...
            payable(fundCollector).call{value:totalFee}("");
            ...
            if (remainingBalance > 0) {
                payable(msg.sender).call{value:remainingBalance}("");
            }
        } else {
            // Transfer minting funds to the veiovia
    ...
    function addAllowedListForListing(address nft, uint256 seriesIndex, address[] calldata target, bool[] calldata allowedStatus) external onlyListingOwner(nft) {
        if (!listedNFTs[nft]) {
            revert NFTNotListed();
        }
    ...
    /// @param nft Address of the nft.
    /// @param seriesIndex Series index to add whitelist.
    function closeWhitelistingForSeries(address nft, uint256 seriesIndex) external onlyListingOwner(nft) {
        if (!listedNFTs[nft]) {
            revert NFTNotListed();
        }
        if (seriesIndex >= listings[nft].seriesDetails.length) {
