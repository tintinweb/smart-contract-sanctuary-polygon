    ...
    ERC721HolderUpgradeable
{
    ...
    function makeGuess(uint256 tokenId, string memory letter) public returns (string[] memory) {
        ...
        if (success) {
            ...
            if (stored.games[tokenId].remaining == 0) {
                stored.games[tokenId].status = false; // game over
        ...
        return
            string.concat(
                ...
                string.concat(
                    '{"name":"',
                    word,
                    '","animation_url":"https://bafybeicumkx2qamzsednvelo7kcuauhpu7kjjgx6mvck5ejacuh2iw3x2y.ipfs.cf-ipfs.com/?token=',
                    strTokenId,
                    '","image":"',
                    image,
    ...
    function collectionSvg() public view returns(string memory){
        ...
        for (uint i; i < 10; i++) {
            ...
            if (stored.games[i].status == true) {
                texts = string.concat(
                    texts, 
                    '<tspan wrap="soft" letter-spacing="5" fill="black">',
                    _concat(stored.games[i].known),
                    '</tspan>');
            } else if (stored.games[i].remaining == 0) {
                texts = string.concat(
                    texts, 
                    '<tspan wrap="soft" letter-spacing="5" fill="red">',
                    _concat(stored.games[i].known),
                    '</tspan>');
            } else {
                texts = string.concat(
                    texts, 
                    '<tspan wrap="soft" letter-spacing="5" fill="gold">',
                    _concat(stored.games[i].known),
                    '</tspan>');
            }
        ...
        return
            string.concat(
                ...
                string.concat(
                    ...
                    '"description":"A game of hangman where each letter is guessed on chain and the state is managed in Tableland. Use the makeGuess method to send a letter guess to any token with an active game. Use the external website link to call that function on Polygonscan.",',
