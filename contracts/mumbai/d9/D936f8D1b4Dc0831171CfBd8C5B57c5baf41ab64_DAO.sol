...
contract crowdFundFactory {
    ...
    function createCrowdfund( address _USDC, address _ownersAddress, uint256 _salt, uint256 deadline, address _nft, uint _amountProposed, string memory _topic,  bytes  memory _description, CrowdFund.Category _cat) public returns(address crowdFundAddr, address manager){
        CrowdFund crowdfund = (new CrowdFund){salt: bytes32(_salt)}(  _USDC,  _ownersAddress, _salt, deadline, _nft, _amountProposed, _topic, _description, _cat);
    ...
    function DonorsNFT(uint256 _index) external payable{
        CrowdFund crowdfund = crowdfunds[_index];
   ...
   /// @param _topic: the caller inputs the _topic, _description, and amount of funding for the project
    ...
    function voteProposal(uint prosalID) external {
        ...
        if(Pis.approved != true){
            revert  ProposalNotApproved("proposal is yet to be approved");
        }

        if(Pis.created == true){
            revert crowdFundCreated("Quorum has been reached, Thank you");
        }
