...
interface IProofOfBrand {
    function tokenForBuyer(address) external returns (uint256);
}

// TODO: Add Access Roles
contract BrandController is ITablelandController {

    // Only the brand can update the brand columns
    address public brand;

    // TODO: This should be variables?
    string[] public brandColumns = ['points', 'category', 'perks'];

    // TODO: This should be variables?
    string[] public buyerColumn = ['pfp_image_url', 'favorite_product'];

    IProofOfBrand public proofOfBrand;

    constructor(
        address _brand,
        address _proofOfBrandAddress
    ) {
        brand = _brand;
        proofOfBrand = IProofOfBrand(_proofOfBrandAddress);
    }
    ...
    {
        string memory whereClause;
        string[] memory updatableColumns;
        uint256 tokenId = proofOfBrand.tokenForBuyer(sender);
        bool hasToken = tokenId != 0;
        bool isBrand = brand == sender;
        if(hasToken) {
            whereClause = string.concat(
             "id = ",
             Strings.toString(tokenId)
            );
            updatableColumns = buyerColumn;
        } else if(isBrand) {
            updatableColumns = brandColumns;
        }

        return
        ITablelandController.Policy({
        // Only the owner can insert
        allowInsert: sender == address(proofOfBrand),
        // Only allow update if the user has a token
        allowUpdate: hasToken || isBrand,
        allowDelete: false,
