 ...
 */
contract BondPeriphery is VersionedInitializable {
    uint256 internal constant REVISION = 1;
    string public constant NAME = 'BondPeriphery';
    ...
    function setProfileMetadataURIWithSig(
        DataTypes.SetProfileMetadataWithSigData calldata vars
    ) external {
        console.log('setProfileMetadataURIWithSig %s %s', vars.profileId, vars.metadata);

        unchecked {
            address owner = IERC721Time(address(HUB)).ownerOf(vars.profileId);
            console.log('owner %s,%s', owner, sigNonces[owner]);
    ...
    function _toggleFollow(
        ...
        for (uint256 i = 0; i < profileIdsLength; ) {
            address followNFT = HUB.getFollowNFT(uint8(DataTypes.FollowType.Follow), profileIds[i]);
    ...
    function _validateCallerIsProfileOwner(uint256 profileId) internal view {
        if (msg.sender == IERC721Time(address(HUB)).ownerOf(profileId)) {
            return;
        }
    ...
    function _validateRecoveredAddress(
        ...
        console.log('recoveredAddress %s expectedAddress %s', recoveredAddress, expectedAddress);
