...
// NFT Payout
import "../NFTPayout/NFTPayoutsUpgradeable.sol";
// Key Storage
import "../KeyStorage/AmberfiKeyStorageUpgradeable.sol";
// Storage
import "../Storage/AmberfiStorage.sol";
// Interfaces
import "../interfaces/IERC721PayoutAmberfiContentFactory.sol";
// Libraries
...
contract NFT is
    ...
    IERC721PayoutAmberfiContentFactory,
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize(
        ...
        if (
            marketAddress_ == address(0) || amberfiStorageAddress_ == address(0)
        ) {
    ...
    function mint(
        address to_,
        uint256 tokenId_,
        PayoutTier calldata payoutInfo_,
        ...
        _setCreator(tokenId_, payoutInfo_.creator);

        uint256 primaryPayoutCount = payoutInfo_.primaryPayoutInfo.length;
        if (primaryPayoutCount > 0) {
            _setTokenPayout(
                newItemId,
                true,
                primaryPayoutCount,
                payoutInfo_.primaryPayoutInfo
            );
        }

        uint256 secondaryPayoutCount = payoutInfo_.secondaryPayoutInfo.length;
        if (secondaryPayoutCount > 0) {
            _setTokenPayout(
                newItemId,
                false,
                secondaryPayoutCount,
                payoutInfo_.secondaryPayoutInfo
            );
        }

        setApprovalForAll(marketAddress, true);

        _safeMint(to_ == address(0x0) ? msg.sender : to_, newItemId, "");
        ...
        returns (bool)
    {
        return
            ...
            interfaceId_ ==
            type(IERC721PayoutAmberfiContentFactory).interfaceId ||
...
// Interfaces
import "./INFTPayoutsUpgradeable.sol";
...
interface IERC721PayoutAmberfiContentFactory is INFTPayoutsUpgradeable {
    struct PayoutTier {
        address creator;
    ...
    /// @param payoutInfo_ (PayoutTier calldata)
...
/**
 * @dev TODO: Implementation of the NFT Royalty Standard plus Payout Info added, a standardized way to retrieve royalty payment and payout information.
 *
    ...
    INFTPayoutsUpgradeable
{
    ...
    function creator(uint256 tokenId_)
        public
        view
        virtual
        override
     ...
     */
    function payoutCount(uint256 tokenId_, bool isPayout_)
        public
        view
        virtual
        override
        ...
        returns (address[] memory receivers, uint256[] memory payoutAmounts)
    {
        ...
        PayoutInfo[] memory payouts = _tokenPayoutInfo[isPayout_][tokenId_];
        payoutAmounts = new uint256[](count);

        for (uint256 i; i < count; ) {
            receivers[i] = payouts[i].receiver;
            payoutAmounts[i] =
                (salePrice_ * payouts[i].payoutFraction) /
                _feeDenominator();
    ...
    function _setTokenPayout(
        ...
        for (uint256 i; i < payoutCount_; ++i) {
            require(
     ...
     */
    function _resetTokenPayout(uint256 tokenId_, bool isPayout_)
        internal
        virtual
    {
