...
contract CryptoplazaCommunity is ERC721, ERC721Burnable, Ownable {
    ...
    function tokenURI(
        ...
        return
            bytes(baseURI).length > 0
                ? string(abi.encodePacked(baseURI, Strings.toString(tenure), "/",Strings.toString(tokenId)))
                : "";
    }
    ...
    function _burn(uint256 tokenId) internal override(ERC721) {
     ...
     function burn(uint256 tokenId) public override (ERC721Burnable)  {
        require(_isApprovedOrOwner(_msgSender(), tokenId) || owner() == _msgSender(), "ERC721: caller is not token owner or approved");
        _burn(tokenId);
    }
}