...
contract AccessPass is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable, Ownable, ERC2981 {
    ...
    constructor(string memory name_, string memory symbol_, string memory CID, uint96 royaltyBasisPts, address royaltyAddress, address firstMint) {
        _name = name_;
        _symbol = symbol_;
        _CID = CID;
        _setDefaultRoyalty(royaltyAddress,royaltyBasisPts);
        _firstMint = firstMint;
    }
    ...
    function mint(uint256 amount) public onlyOwner{
        require(_balances[_firstMint]==0,"Can only mint when all minted tokens have been sold");
        require(_totalSupply + amount <= _maxSupply,"Max supply cannot be exceeded");
        _totalSupply += amount;
        _balances[_firstMint] += amount;
    }
    ...
    function ownerOf(uint256 tokenId) public view virtual override returns (address) {
        ...
        if(owner == address(0)) owner = _firstMint;
        return owner;
    }
