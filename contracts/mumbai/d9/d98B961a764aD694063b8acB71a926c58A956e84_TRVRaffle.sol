...
contract TRVRaffle is Base {
  ...
  mapping(uint256 => mapping(string => bool)) public released;

  event Pledge(address indexed account, uint256 timePledged, bytes notes);
  event FundsReleased(uint256 indexed timestamps, string tag, bytes winners, bytes notes);
  ...
  function releaseFunds(
    ...
    require(!released[_timestamps][_tag], "Already released");
    ...
    released[_timestamps][_tag] = true;
    emit FundsReleased(_timestamps, _tag, abi.encode(_winners), _notes);
  }
