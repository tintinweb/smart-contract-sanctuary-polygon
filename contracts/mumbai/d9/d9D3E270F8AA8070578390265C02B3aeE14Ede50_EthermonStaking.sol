...
contract EthermonStaking is EthermonStakingBasic {
    ...
    uint16[] daysToStake = [30, 90, 180, 365];
    uint256 minDepositeValue = 1000 * 10**18;
    uint256 maxDepositeValue = 100000 * 10**18;

    address public stakingDataContract;
    IERC20 emon;

    constructor(address _stakingDataContract, address _emon) public {
    ...
    function depositeTokens(uint16 _day, uint256 _amount) external {
        EthermonStakingData stakingData = EthermonStakingData(
            stakingDataContract
        );
        require(_amount >= minDepositeValue && _amount <= maxDepositeValue);
        for (uint16 i = 0; i < daysToStake.length; i++) {
            if (daysToStake[i] == _day) break;
            if (i >= daysToStake.length) revert();
        }
        uint256 balance = emon.balanceOf(msgSender());
        require(balance > 0);

        uint256 dayTime = block.timestamp + (_day * 1 days);
        stakingData.addTokenData(msgSender(), dayTime, _amount);

        emon.safeTransferFrom(msgSender(), stakingDataContract, _amount);
        emit Deposite(msgSender(), address(this), _amount);
    }
    ...
    function withdrawTokens(uint256 _tokenId) external {
        ...
        TokenData[] memory data = stakingData.getTokenData(msgSender());
        for (uint256 i = 0; i < data.length; i++) {
            if (data[i].tokenId == _tokenId) {
                uint256 balance = emon.balanceOf(stakingDataContract);
                require(balance > 0 && data[i].day <= block.timestamp);

                stakingData.removeTokenData(msgSender(), _tokenId);
                ...
                emit Withdraw(address(this), msgSender(), data[i].amount);
                break;
            }
        }
    }
