...
contract StellarTokenNFT is ERC721URIStorage {
    ...
    event NFTBought(uint256 tokenId, address indexed buyer, uint256 amount);
    event TokensReceived(address sender, uint256 amount);
    event StellarTokenContractSet(address indexed contractAddress);
    ...
    function setStellarTokenContract(address payable _stellarTokenContract) external onlyOwner {
        ...
        emit StellarTokenContractSet(_stellarTokenContract);
    }
    ...
    //     require(quantity <= 10, "Max of 10 mint per transaction");
    //     require((ownedTokens[msg.sender].length() + quantity) < 50, "Exceeded maximum own limit - 50");
    //     address recipient = msg.sender;

    //     marketingWallet.transfer(mintPrice * quantity);

    //     for (uint256 i = 0; i < quantity; i++) {
    //         uint8 level = getRandomLevel(i);
    //         string memory rarity = getRarity(level);

    //         uint8 currentLevel = findHighestLevel(recipient);
    ...
    //                 level, 
    //                 rarity, 
    //                 getRandomBackground(level, i), 
    //                 getRandomSpaceshipFilename(level, i), '', '', '', '');
    //         }
    ...
    //                 getRandomBackground(level, i), 
    //                 getRandomSpaceshipFilename(level, i),
    //                 getRandomPlanetFilename(level, i), '', '', '');
    //         }
    //         if(level == 3) {
    //             tokenIdToMetadata[nextTokenId] = NFTMetadata(
    //                 level, 
    ...
    //                 getRandomSpaceshipFilename(level, i),
    //                 getRandomPlanetFilename(level, i),
    //                 getRandomSmallPlanetFilename(level, i), '', '');
    //         }
    //         if(level == 4) {
    //             tokenIdToMetadata[nextTokenId] = NFTMetadata(
    //                 level, 
    ...
    //                 getRandomPlanetFilename(level, i),
    //                 getRandomSmallPlanetFilename(level, i),
    //                 getRandomMeteorFilename(level, i), '');
    //         }
    //         if(level == 5 || level == 6) {
    //             tokenIdToMetadata[nextTokenId] = NFTMetadata(
    //                 level, 
    ...
    //                 getRandomSmallPlanetFilename(level, i),
    //                 getRandomMeteorFilename(level, i),
    //                 getRandomSatelliteFilename(level, i));
    //         }

    //         availableTokens.add(nextTokenId);
    //         tokenIdToLevel[nextTokenId] = level;
    //         totalSupply++;
    //         nextTokenId++;
    //         mintedCount++;
    ...
    function mintNFTDev(uint256 quantity, uint8 level) external payable onlyOwner {
        ...
        require(totalSupply + quantity <= 100001, "Exceeds supply");
        require(msg.value >= mintPrice * quantity, "Insufficient funds");
        require(quantity <= 10, "Max of 10 allowed per transaction");
        require((ownedTokens[msg.sender].length() + quantity) <= 50, "Exceeded maximum mint limit - 50");
    ...
    function findHighestLevel(address addr) internal view returns (uint8) {
        ...
        for (uint256 i = 0; i < tokens.length(); i++) {
            ...
            uint8 level = tokenIdToLevel[tokenId];
    ...
    function getRandomLevel(uint256 index) internal view returns (uint8) {
        uint256 randomNumber = uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty, nextTokenId, index + 500)));
        uint8 level;
    ...
    function getRandomBackground(uint8 level, uint256 index) internal view returns (string memory) {
        uint256 mixer =  level * index + 1000;
        ...
        uint256 randIndex = uint256(keccak256(abi.encodePacked(block.timestamp, msg.sender, mixer))) % filenames.length;
        
        return filenames[randIndex];
    }

    function getRandomSpaceshipFilename(uint8 level, uint256 index) internal view returns (string memory) {
        uint256 mixer =  level * index + 1000;
    ...
    function getRandomPlanetFilename(uint8 level, uint256 index) internal view returns (string memory) {
        uint256 mixer =  level * index + 1000;
    ...
    function getRandomSmallPlanetFilename(uint8 level, uint256 index) internal view returns (string memory) {
        uint256 mixer =  level * index + 1000;
    ...
    function getRandomMeteorFilename(uint8 level, uint256 index) internal view returns (string memory) {
        uint256 mixer =  level * index + 1000;
    ...
    function getRandomSatelliteFilename(uint8 level, uint256 index) internal view returns (string memory) {
        uint256 mixer =  level * index + 1000;
