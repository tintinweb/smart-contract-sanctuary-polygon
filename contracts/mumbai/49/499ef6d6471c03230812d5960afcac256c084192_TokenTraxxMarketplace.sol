    ...
    ErrorCodes
{
    ...
    function verifyWalletWhitelist(bytes32[] memory proof, bytes32 root, bytes32 leaf) public pure returns (bool) {
        return MerkleProof.verify(proof, root, leaf);
    }

    // TODO : Need to optimize if else
    function validateSaleParameters(Listing memory listingData, bytes32[] memory proof, address buyer, uint256 quantityToBuy) internal {
        ...
        if(listingData.tokenType == TokenType.ERC721){
            ...
            for (uint256 i = 0; i < collectionData.whitelistParams.length; i++) {
                if (block.timestamp > collectionData.whitelistParams[i].startTime && block.timestamp < collectionData.whitelistParams[i].endTime) {
            ...
            if (merkleRoot != 0x00) {
                require(
                    verifyWalletWhitelist(proof, merkleRoot, keccak256(abi.encodePacked(buyer))), 
                ...
                if (collectionData.userPurchaseLimit != -1) {
                    require(
                        erc721ContractInstance.getPrimarySaleUserCount(
                            msg.sender,
                            collectionId
                        ) + quantityToBuy < uint256(collectionData.userPurchaseLimit)
                    );
                }
            }
        }
            ...
            IERC1155Custom.Collection memory collectionData = erc1155ContractInstance
                .getCollectionData(collectionId);
            ...
            if (merkleRoot != 0x00) {
                require(verifyWalletWhitelist(proof, merkleRoot, keccak256(abi.encodePacked(buyer))), "Address not whitelisted");
            }
            if (
