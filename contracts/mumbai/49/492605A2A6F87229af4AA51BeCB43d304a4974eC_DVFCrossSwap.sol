...
abstract contract Swap is Storage, UserWallet {
    ...
    bytes32 public constant _SWAP_TYPEHASH =
        keccak256(
            "Swap(address user,bytes32 swapGateway,address tokenFrom,address tokenTo,uint256 amountFrom,uint256 minAmountTo,uint256 nonce,uint256 deadline,uint256 chainId)"
        );
    ...
    function __Swap_init(address _paraswap, address _paraswapTransferProxy)
        internal
        onlyInitializing
    {
        paraswap = _paraswap;
    ...
    function executeSwapWithSignature(
        ...
        (, uint256 amountToUser, uint256 amountToFee) = performSwap(
        ...
        )
    {
        ...
        IERC20Upgradeable(tokenFrom).safeApprove(
        ...
        tokenFromAmount =
            tokenFromAmount -
            IERC20Upgradeable(tokenFrom).balanceOf(address(this));
        amountToUser =
            IERC20Upgradeable(tokenTo).balanceOf(address(this)) -
            amountToUser;
    ...
    function safeExecuteOnParaswap(bytes memory _data) private {
        AddressUpgradeable.functionCall(
            paraswap,
            _data,
            "PARASWAP_CALL_FAILED"
        );
    }
    ...
    function verifySwapSignature(
        ...
        require(
            swapConstraints.nonce > userNonces[swapConstraints.user],
            "NONCE_ALREADY_USED"
        );
...
abstract contract EIP712Upgradeable is Initializable {
    ...
    function updateEip712(string memory name, string memory version) internal {
...
abstract contract UserWallet is Storage, DVFAccessControl, EIP712Upgradeable {
    ...
    bytes32 public constant _WITHDRAW_TYPEHASH =
        keccak256(
            "Withdraw(address user,address token,address to,uint256 amount,uint256 maxFee,uint256 nonce,uint256 deadline,uint256 chainId)"
        );
    ...
    function _contractBalance(IERC20Upgradeable token)
        internal
        view
        returns (uint256)
    {
    ...
    function verifyWithdrawSignature(
        ...
        require(
            withdrawConstraints.nonce > userNonces[withdrawConstraints.user],
            "NONCE_ALREADY_USED"
        );
...
abstract contract DVFAccessControl is AccessControlUpgradeable, Storage {
    ...
    bytes32 public constant LIQUIDITY_SPENDER_ROLE =
        keccak256("LIQUIDITY_SPENDER_ROLE");
