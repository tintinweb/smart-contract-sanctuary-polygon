...
contract CrednetialNFT is ERC721URIStorage, Ownable {
    ...
    mapping(address => string[]) userMetaDataURI;
    ...
    function mint(address to, string memory _tokenURI) external virtual {
        if (userMetaDataURI[to].length < 1) {
            tokenIdTracker.increment();
            _mint(to, tokenIdTracker.current());
            _setTokenURI(tokenIdTracker.current(), _tokenURI);
            userMetaDataURI[to].push(_tokenURI);
        } else {
            for (uint256 i = 0; i < userMetaDataURI[to].length; i++) {
                require(
                    keccak256(bytes(userMetaDataURI[to][i])) !=
