// SPDX-License-Identifier: CC0-1.0

pragma solidity 0.6.11;
pragma experimental ABIEncoderV2;

contract ArrayTestContract {
    mapping(uint256 => bytes) random_data;

    uint256 public counter = 0;

    constructor() public {
        bytes
            memory random208Bytes = hex"";

        for (uint256 i = 0; i < 100; i++) {
            random_data[i] = random208Bytes;
            ++counter;
        }
    }

    function add100ValuesToRandomData() external {
        bytes
            memory random208Bytes = hex"";

        for (uint256 i = 0; i < 100; i++) {
            ++counter;
            random_data[counter] = random208Bytes;
        }
    }

    function getRandomDataArray() public view returns (bytes[] memory) {
        bytes[] memory returnedArray = new bytes[](counter);
        for (uint256 i = 0; i < counter; i++) {
            returnedArray[i] = random_data[i];
        }
        return returnedArray;
    }

    function getRandomDataDynamicArray()
        public
        view
        returns (bytes[] memory returnedArray)
    {
        for (uint256 i = 0; i < counter; i++) {
            returnedArray[i] = random_data[i];
        }
        return returnedArray;
    }

    function getRandomDataByIndex(uint256 index)
        public
        view
        returns (bytes memory)
    {
        return random_data[index];
    }
}