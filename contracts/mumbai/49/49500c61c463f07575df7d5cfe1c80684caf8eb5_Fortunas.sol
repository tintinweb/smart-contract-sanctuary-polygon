...
contract Fortunas {
    ...
    function buyNewLevel(uint256 _amount) external { 
        ...
        require(royaltyInfo[msg.sender].totalincome==users[msg.sender].totalDeposit*4, "Total Earning not reached 400%!");
    ...
    function _deposit(address _user, uint256 _amount) private {     
        ...
        uint256 distamount=_amount/2;
        _distributeDeposit(distamount);
        //uint256 _directincome=this.maxPayoutOf(users[_user].referrer,distamount*directPercents/100);
        //if(_directincome>0)
        //{            
            royaltyInfo[users[_user].referrer].directincome += distamount*directPercents/100;                       
            royaltyInfo[users[_user].referrer].totalincome +=distamount*directPercents/100;
            emit Transaction(users[_user].referrer,_user,distamount*directPercents/100,1,1);
        //}
        _distributelevelIncome(users[_user].referrer, distamount);       
    } 
