...
He's upgradable and pausable in case of emergenci/upgrades.
Since the capital and interest are refunded at the end of maturity , th price raise is check every time we've got the comfirmation of the interest from 
the S.C.P.I.
After the sale is completed , the funds are locked till refund.
 ...
 */
  ...
  /** Check if an address is whitelisted and kyced (In a non forbidden country)
  -canBuy -> returns a boolean]
  check {./interface/IDatabase.sol}*/
  IDatabase public dataBase;

  /** Check if an address is whitelisted and kyced (In a non forbidden country)
  -canBuy -> return a boolean */
  address public treasury;

  /** Check if the vault has been refunded. Bool use twice the gas of uint, but we're on l2 and it's more readable */
  bool public refunded;

  /**  Two timestamps one for the begining of the auction , the ther is the deadline */
  uint public depositsStart;
  uint public depositsStop;

  uint public maxDepositPerUser;

  /**variables  softCap is the minimum assets that have to be on the vault at the  depositsStop, if not reached , sale failed */
  uint public softCap;
  /**when reached , even befor the depositsStop , complete the Sale */
  uint public hardCap;

  /**Price raise artifically , following the interests, every month. But is reajusted with the total refund*/
  uint public assetPrice;
  /**Collected capital fix the total assets raised*/
  uint public collectedCapital;

  /**
   *****************************Mappings******************************************
   */
  /**Amount a wallet paid ( to check maxDeposit etc...)*/
  ...
  event TimesUpdated(uint depositsStart, uint depositsStop);
  event TreasuryUpdated(address oldTreasury, address newTreasury);
  /**
   *****************************Errors******************************************
   */
  ...
  error NegativeCapital(int capitalAndInterest);
  ...
  @dev Using int to simplify readability of negative value new price is used to compare expected interest and  real interest
  new price ise used as mantissa
 */
  function refundCapital(int capitalAndInterest) external onlyRole(TREASURY) {
    SaleStep step = getStep();
    if (step != SaleStep.SALE_COMPLETE) revert WrongStep(step);
    if (capitalAndInterest < 0) revert NegativeCapital(capitalAndInterest);

    int collectedCapital_ = int(collectedCapital);
    IERC20Upgradeable(asset()).safeTransferFrom(
      _msgSender(),
      address(this),
      uint(capitalAndInterest)
    );

    emit CapitalRefunded(uint(capitalAndInterest), uint(collectedCapital_));

    int interest = capitalAndInterest - collectedCapital_;
...
@param amount have to be less than softCap
 */
  ...
  */
  function deposit(
    uint assets,
    address receiver
  ) public override nonReentrant whenNotPaused returns (uint shares) {
  ...
  */
  function mint(
    uint shares,
    address receiver
  ) public override nonReentrant whenNotPaused returns (uint assets) {
    address sender = _msgSender();
    if (!dataBase.canBuy(sender)) revert MsgSenderUnauthorized(sender);
  ...
  function maxDeposit(address user) public view override returns (uint) {
  ...
  */
  function maxMint(address user) public view override returns (uint) {
 ...
 */
  ...
  function _convertToAssets(
    ...
    assets = shares > 0
      ? shares.mulDiv(assetPrice, originalPrice, rounding)
      : 0;
  }
  ...
  function _convertToShares(
    ...
    shares = assets > 0
      ? assets.mulDiv(originalPrice, assetPrice, rounding)
      : 0;
  }
   ...
   */
  function _maxDeposit(address user) internal view returns (uint) {
    ...
    uint paidByUser = paid[user];
    if (hardCap_ <= tsInAssets || maxDepositPerUser <= paidByUser) return 0;

    uint capedMax = maxDepositPerUser - paidByUser;
    uint userMax = hardCap_ - tsInAssets;
   ...
   * @dev @return  the smallest amount between:
   ...
   -userMax:  the maxDepositPerUser & (converted in assets) already paid's difference
   */
  function _maxMint(address user) internal view returns (uint) {
    uint ts = totalSupply();
    ...
    uint capedMax = _convertToShares(
      maxDepositPerUser - paidByUser,
      MathUpgradeable.Rounding.Down
    );
    uint userMax = hardCapInShares - ts;
