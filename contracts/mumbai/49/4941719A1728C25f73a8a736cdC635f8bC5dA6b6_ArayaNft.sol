...
contract ArayaNft is ERC1155, AccessControl, OpenSeaCompatible, NativeMetaTransaction, ContextMixin
{
    /**
    ...
    uint64 private constant MINTING_CHAINID = 80001; // FIXME - put real chain id

    mapping (uint256 => uint256) public mintedCounts;
    mapping (uint256 => uint256) public mintingLimits;

    constructor(string memory uri) ERC1155("Araya NFT TEST", "ARAYANFTTEST", uri) { // FIXME - put normal name/ticker
        _initializeEIP712(uri);
    ...
    * - Amount to mint must not exceed the difference between minting limit and already minted amount of tokens.
    */
    function initialMint(address account, uint256 id, uint256 amount, bytes calldata data) external onlyRole(MINTER_ROLE) {
        require(block.chainid == MINTING_CHAINID, "Minting is allowed only on Polygon.");
        require(mintingLimits[id] == 0 || amount <= mintingLimits[id] - mintedCounts[id], "Amount exceeds the token minting limit.");
    ...
    */
    function setMintingLimit(uint256 id, uint256 limit) external onlyRole(ADMIN_ROLE) {
        require(mintingLimits[id] == 0, "Minting limit for this id is already set.");
