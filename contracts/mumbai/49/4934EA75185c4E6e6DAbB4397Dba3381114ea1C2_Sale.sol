...
interface INFT {
    ...
    function batchMint(address[] memory) external;
}
...
contract Sale is Ownable {
    ...
    function buyPre(uint8 _amount) external payable onlyWhitelist walletLimit(_amount) {
        require(preOpenTime <= block.timestamp, "not opened");
        require(openTime >= block.timestamp, "closed");

        _mint(_amount);
    }

    function buy(uint8 _amount) external payable walletLimit(_amount) {
        require(openTime <= block.timestamp, "not opened");
