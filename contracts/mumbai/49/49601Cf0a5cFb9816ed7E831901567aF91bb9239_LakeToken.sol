...
abstract contract BlacklistManager is Ownable, VestingManager {
    ...
    function addToBlacklist(address _account) external onlyBlacklistManager returns (bool) {
        require(_account != vesting, 'BlacklistManager: cannot add vesting contract address to blacklist');
...
abstract contract VestingManager is Ownable {
    /**
    ...
    function setVestingAddress(address _address) external onlyOwner {
        require(_address != address(0x0), 'VestingManager: address cannot be a zero address');
        vesting = _address;
