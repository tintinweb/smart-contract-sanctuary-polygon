...
contract RewardPool is Ownable {
    ...
    function rewardTo(address _account, uint256 _rewardAmount)
        external
        onlyManager
    {
        require(
            IERC20(token).balanceOf(address(this)) > _rewardAmount,
...
contract GrushManager {
    ...
    uint256 public maxDiggersPerUser;
    ...
    address private PROJECT_DEVELOPMENT =
        0x43db4fc2e4ed1B10A7d18F7DA503042Ed129E7fA;
    ...
    function createArgonautDigger() external NotBlacklist {
        require(
            diggersOwned[msg.sender].length < maxDiggersPerUser,
            "You have reached the maximum amount of diggers!"
        );
        require(
            msg.sender != TREASURY && msg.sender != REWARDS,
            "Treasury and Reward pools cannot create diggers!"
        );
        // require(token.balanceOf(msg.sender) >= costPerDigger, "Not enough tokens to create Digger!");
        uint256 _cost = 110 * 1e18;
        token.transferFrom(msg.sender, address(this), _cost);
        buyingFees(_cost);
        Digger memory newDigger = Digger(
            "Argonaut",
            block.timestamp,
            block.timestamp,
            2.21 * 1e18,
    ...
    // Buying Fees
    function buyingFees(uint256 _cost) internal {
        uint256 _development = (_cost * 23) / 100;
        uint256 _liquidity = (_cost * 10) / 100;
        uint256 _reward = (_cost * 67) / 100;
    ...
    function calculateRewards(address _user) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < diggersOwned[_user].length; i++) {
            uint256 epochsPassed = (block.timestamp -
                diggersOwned[_user][i].lastClaimTime) / 1 days;
    ...
    function claimRewards() external NotBlacklist {
        ...
        // Set new Claim Time
        for (uint256 i = 0; i < diggersOwned[msg.sender].length; i++) {
    ...
    function levelUpDigger(address _user, uint256 _diggerOwnedID)
        external
        NotBlacklist
    {
        require(
            diggersOwned[_user][_diggerOwnedID].level < 3,
            "Digger already at Max Level!"
        );
        uint256 _cost;
        uint256 _rewardRate;
        if (diggersOwned[_user][_diggerOwnedID].level == 0) {
            _cost = 15 * 1e18;
            _rewardRate = 2.56 * 1e18;
        } else if (diggersOwned[_user][_diggerOwnedID].level == 1) {
            _cost = 20 * 1e18;
            _rewardRate = 3.06 * 1e18;
        } else if (diggersOwned[_user][_diggerOwnedID].level == 2) {
            _cost = 30 * 1e18;
            _rewardRate = 3.84 * 1e18;
        }
        token.transferFrom(msg.sender, address(this), _cost);
        buyingFees(_cost);
    ...
    // SETTERS
    function setMaxDiggersPerUser(uint256 _max) external onlyOwner {
    ...
    // GETTERS
    function getOwnedDiggersInfo(address _user)
        external
        view
        returns (Digger[] memory)
    {
    ...
    function recoverTokens(IERC20 _erc20, address _to) public onlyOwner {
        require(
            address(_erc20) != address(token),
            "You can't recover default token"
        );
