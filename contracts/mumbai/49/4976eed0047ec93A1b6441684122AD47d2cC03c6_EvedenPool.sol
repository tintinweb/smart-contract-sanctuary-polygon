...
contract EvedenPool is Ownable, ERC1155Receiver {
  ...
  constructor(address collection_, address currency_, uint price_, address beneficiary_, address issuer_, uint available_) {
  ...
  function updateAvailable(int d) external onlyOwner {
    if (d >= 0) {
      available += uint(d);
    } else {
      available -= uint(-d);
  ...
  function buy(uint amount) external {
    require(!pause, "pause");
    require(amount <= available, "not enough stock");
    available -= amount;
    emit Buy(_msgSender(), amount);
    currency.safeTransferFrom(_msgSender(), beneficiary, price * amount);
  }
  ...
  function onERC1155Received(
    ...
    if (from == address(0) || id != TICKET_ID) return this.onERC1155Received.selector;

    require(!pause, "pause");
    require(value <= available, "not enough stock");
    available -= value;
    emit Buy(from, value);

    collection.burn(TICKET_ID, value);
