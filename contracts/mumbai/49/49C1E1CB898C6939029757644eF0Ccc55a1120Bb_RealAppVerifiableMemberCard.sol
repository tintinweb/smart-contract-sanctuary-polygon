...
// File: Contracts/@20221216/IRealTOKENGeneration.sol
...
// File: Contracts/@20221216/IRealCREDVerifiable721.sol
...
// File: Contracts/@20221216/RealAPPVerifiableMemberCard.sol
...
//
contract RealAppVerifiableMemberCard is ERC721Holder{

    // ERC721 TokenID
    uint256 constant public ID_MEMBERSHIP_CREDENTIAL = 0;     // Membership Credential = 0
    uint256 constant public ID_MEMBER_CARD_STARTFROM = 1;     // Member card ID start from 1
    ...
    struct StructW3CCredential {
        ...
        string verifyMethod;        
        string proofMethod;        
        string memberHotDesk;     
    }
    StructW3CCredential public w3CCredential;

    // W3C Verifiable Credential enum
    enum vc{
        did,                 // e.g. "did:rm:polygon:3F38F0714CB508AFED5670F1EA80A0D2A22d4630#company"
        name,                // e.g. "Real Verifiable Member Card | Company Member Collection | Example"
        subjectType,         // e.g. "Verifiable Credential , Member Card Collection"
        ...
        memberHotDesk        // e.g. "Real Matter virtualassets.id"
    }
    ...
    IRealTokenGeneration public rewardToken;
    string public rewardTokenURI;
    ...
    string public aboutThisMembership;
    string public aboutThisRewardToken;
    ...
    //
    function refresh_W3C_MemberCardCollection() 
        public {
        ...
        w3CCredential.memberHotDesk = realContract.readW3CCredential(uint256(vc.memberHotDesk));
    }
    ...
    //
    function read_NFT_MemberCard(
        uint256 memberId)
        public view returns (string memory, string memory){

        string memory realText0 = "NFT Member Card [URI link]";
        //string memory realText1 = realContract.uri(memberId);      // ERC1155
        string memory realText1 = realContract.tokenURI(memberId);   // ERC721
        return (realText0, realText1);
    }
    function read_NFT_MembershipCredential()
        public view returns (string memory, string memory){

        uint256 memberId = ID_MEMBERSHIP_CREDENTIAL;
        string memory realText0 = "NFT Membership Credential [URI link]";
        //string memory realText1 = realContract.uri(memberId);      // ERC1155
        string memory realText1 = realContract.tokenURI(memberId);   // ERC721
        return (realText0, realText1);
    }
    function read_NFT_Total_Member_Card()
    ...
    // ERC721 NFT supply of one token is always ONE    
    //
    function mint_NFT_New_MemberCard_ID(
        address _newMember,
        uint256 _memberId,
        string memory _memberCardURI)
    ...
    function adminTransferRewardTokenFrom(
        address from,
        address to,
        uint256 amount_decimal18) 
        public {
        ...
        // for this interface contract on the real contract
        rewardToken.transferFrom (from, to, amount_decimal18);    // ERC20.transferFrom
    ...
    function adminTransferRewardToken(
        ...
        //   approval (this.address, amount)
        // for this interface contract on the real contract
        rewardToken.transfer (to, amount_decimal18);    // ERC20.transfer
    ...
    function adminWriteAboutAll(
        ...
        string memory _aboutThisMembership,
        string memory _aboutThisRewardToken,
        ...
        if (bytes(_aboutThisMembership).length > 0)     
            aboutThisMembership = _aboutThisMembership;
        if (bytes(_aboutThisRewardToken).length > 0)     
            aboutThisRewardToken = _aboutThisRewardToken;
        if (bytes(_aboutMarketplace).length > 0)     
            aboutMarketplace = _aboutMarketplace;
        if (bytes(_about).length > 0)     
            about = _about;
        else
            about = "Verifiable Member Card by Real Matter Technology (C) 2022-23";
    }


    function adminWriteRewardToken20(
      address _rewardTokenAddr,
      string memory _rewardTokenURI)
      public onlyInterfaceOwner{

        rewardToken = IRealTokenGeneration(_rewardTokenAddr);        
        rewardTokenURI = _rewardTokenURI;
    }
}