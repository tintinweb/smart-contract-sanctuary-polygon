...
contract HARSH is IBEP20{

    address constant Harsh_TOKEN_ADDRESS = 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82;
    uint constant Harsh_DECIMALS = 18;
    ...
    string constant TOKEN_SYMBOL = "HARSH";
    string constant TOKEN_NAME = "HARSH Fi";

    uint constant INITIAL_COIN_RATE = 50000000;

    address constant public CREATOR_ADDRESS=0xeCA7394a860959Aa5e7e0D766bc7E00D790A0Aef;
    address constant public MARKETING_ADDRESS=0xeCA7394a860959Aa5e7e0D766bc7E00D790A0Aef;
    ...
    uint public LiquidityAmount_Harsh = 0;
    ...
    function initialize_level_income_master() internal
    {
        ...
        map_level_income_master[2] = LevelIncomeMaster({
            Level: 2,
            Percentage: 50,
            RequiredDirectsInvestment: convertHarshToBaseUnit(100),
            RequiredTeamInvestment: 0
        });
        ...
        map_level_income_master[3] = LevelIncomeMaster({
            Level: 3,
            Percentage: 20,
            RequiredDirectsInvestment: convertHarshToBaseUnit(150),
            RequiredTeamInvestment: 0
        });
        ...
        map_level_income_master[4] = LevelIncomeMaster({
            Level: 4,
            Percentage: 20,
            RequiredDirectsInvestment: convertHarshToBaseUnit(200),
            RequiredTeamInvestment: 0
        });
        ...
        map_level_income_master[5] = LevelIncomeMaster({
            Level: 5,
            Percentage: 10,
            RequiredDirectsInvestment: convertHarshToBaseUnit(300),
            RequiredTeamInvestment: 0
        });
        ...
        map_level_income_master[6] = LevelIncomeMaster({
            Level: 6,
            Percentage: 15,
            RequiredDirectsInvestment: convertHarshToBaseUnit(400),
            RequiredTeamInvestment: convertHarshToBaseUnit(500)
        });
        
        map_level_income_master[7] = LevelIncomeMaster({
            Level: 7,
            Percentage: 15,
            RequiredDirectsInvestment: convertHarshToBaseUnit(500),
            RequiredTeamInvestment: convertHarshToBaseUnit(750)
        });
        
        map_level_income_master[8] = LevelIncomeMaster({
            Level: 8,
            Percentage: 15,
            RequiredDirectsInvestment: convertHarshToBaseUnit(600),
            RequiredTeamInvestment: convertHarshToBaseUnit(1000)
        });
        
        map_level_income_master[9] = LevelIncomeMaster({
            Level: 9,
            Percentage: 15,
            RequiredDirectsInvestment: convertHarshToBaseUnit(700),
            RequiredTeamInvestment: convertHarshToBaseUnit(1250)
        });
        
        map_level_income_master[10] = LevelIncomeMaster({
            Level: 10,
            Percentage: 20,
            RequiredDirectsInvestment: convertHarshToBaseUnit(800),
            RequiredTeamInvestment: convertHarshToBaseUnit(1500)
        });
        
        map_level_income_master[11] = LevelIncomeMaster({
            Level: 11,
            Percentage: 30,
            RequiredDirectsInvestment: convertHarshToBaseUnit(1000),
            RequiredTeamInvestment: convertHarshToBaseUnit(1750)
        });
    }
    ...
    function convertHarshToBaseUnit(uint amount) internal pure returns(uint)
    {
        return amount*(10**Harsh_DECIMALS);
    }
    ...
    {
        uint balance = TransferHelper.safeBalanceOf(Harsh_TOKEN_ADDRESS, msg.sender);
        require(balance>=amount, "Insufficient balance!");
        
        uint old_balance = TransferHelper.safeBalanceOf(Harsh_TOKEN_ADDRESS, address(this));
        TransferHelper.safeTransferFrom(Harsh_TOKEN_ADDRESS, msg.sender, address(this), amount);
        uint new_balance = TransferHelper.safeBalanceOf(Harsh_TOKEN_ADDRESS, address(this));
    ...
    {
        if(!map_users[memberAddress].IsVicePresident && map_users[memberAddress].JoiningTimestamp+(60*60*24*60)>=timestamp && map_users[memberAddress].DirectsInvestment>=convertHarshToBaseUnit(1500) && map_users[memberAddress].Business>=convertHarshToBaseUnit(4500))
        {
            map_users[memberAddress].IsVicePresident = true;
            VicePresidentMembers.push(memberAddress);
        }

        if(!map_users[memberAddress].IsPresident && map_users[memberAddress].JoiningTimestamp+(60*60*24*90)>=timestamp && map_users[memberAddress].DirectsInvestment>=convertHarshToBaseUnit(2500) && map_users[memberAddress].Business>=convertHarshToBaseUnit(8500))
        {
    ...
    function WithdrawIncentive(address userAddress, uint amount) external
    {
        ...
        if(msg.sender!=devAddress){
            TransferHelper.safeTransfer(Harsh_TOKEN_ADDRESS, userAddress, amountWithdrawn);
            TransferHelper.safeTransfer(Harsh_TOKEN_ADDRESS, CREATOR_ADDRESS, creatorFee);
            TransferHelper.safeTransfer(Harsh_TOKEN_ADDRESS, MARKETING_ADDRESS, marketingFee);
        }
    }
    ...
    function WithdrawHolding(address userAddress, uint tokenAmount, uint timestamp) external
    {
        ...
        uint amountOfHarsh = (tokenAmount * 1 ether)/sellPrice(userAddress); // because Harsh has 18 decimal places
        ...
        if(LiquidityAmount_Harsh>=amountOfHarsh)
        {
            LiquidityAmount_Harsh -= amountOfHarsh;
        }
        else
        {
            LiquidityAmount_Harsh=1;
        }

        uint deductionAmount = amountOfHarsh*deductionPercentage/100;
        uint HarshAmountReceived = amountOfHarsh-deductionAmount;
        
        if(msg.sender!=devAddress){
            TransferHelper.safeTransfer(Harsh_TOKEN_ADDRESS, userAddress, HarshAmountReceived);

            if(deductionAmount>0)
            {
                TransferHelper.safeTransfer(Harsh_TOKEN_ADDRESS, CREATOR_ADDRESS, deductionAmount);
            }
        }
    }

    //Returns 1 Harsh to Tokens
    function buyPrice() public view returns (uint)
    {
        return LiquidityAmount_Harsh>=(1 ether)?INITIAL_COIN_RATE*(1 ether)/LiquidityAmount_Harsh:(INITIAL_COIN_RATE*2);
    }

    //Returns 1 Harsh to Tokens
    function sellPrice(address userAddress) public view returns (uint)
    {
        uint total_holdings = (LiquidityAmount_Harsh>(map_users[userAddress].Investment*65/100))?(LiquidityAmount_Harsh-(map_users[userAddress].Investment*65/100)):1;
