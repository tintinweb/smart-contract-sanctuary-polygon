...
contract EtheremonBulkUpdateUri is BasicAccessControl {
    mapping(uint32 => bool) public neglectedClasses;

    address public monsterNFTContract;
    address public dataContract;
    string private metaUriPrefix = 'https://ethermon.io/api/matic/monster/get_data?monster_ids=';
    ...
    constructor(address _dataContract, address _monsterNFTContract) public {
    ...
    function setNeglectedClass(uint32 _classId)
        public
        onlyModerators
    {
        if (_classId < 1 || neglectedClasses[_classId]) return;
        neglectedClasses[_classId] = true;
    }

    function setMetaUriPrefix(string memory _uri)
        public
        onlyModerators
    {
        metaUriPrefix = _uri;
    ...
    function removeNeglectedClass(uint32 _classId) public onlyModerators {
        if (neglectedClasses[_classId] == false) return;
        delete neglectedClasses[_classId];
    }

    function checkNeglectedToken(uint32 _classId)
        external
        view
        returns (bool)
    {
        neglectedClasses[_classId];
    }
    ...
    function setBulkTokenURI(
        ...
        for (; _startMonId <= _endMonID; _startMonId++) {
            ...
            if (neglectedClasses[obj.classId] != true) {
                string memory _token_id_str = Strings.toString(uint(_startMonId));
               	string memory _uri = string(abi.encodePacked(metaUriPrefix, _token_id_str));
                monsterContract.setTokenURI(_startMonId, _uri);
            }
        }
    }

    function getTokenURI(uint32 _classId)
        external
        view
        returns (string memory)
    {
        ...
        return monsterContract.tokenURI(_classId);
    }
}