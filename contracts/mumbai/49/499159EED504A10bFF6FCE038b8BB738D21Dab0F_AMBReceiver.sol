...
contract AMBReceiver is ReentrancyGuard, Ownable {
    ...
    function execute(address recipient) external nonReentrant onlyRelayer {
        (bool success, ) = recipient.call(msg.data);

        if (!success) revert AMBReceiver_ExecuteFailed();

        emit TransactionExecuted(recipient, msg.data);
    }
}
