...
contract LendingPool is Ownable, ReentrancyGuard {
    ...
    uint256 public _oneDayInSeconds;
    uint256 public _oneYearInSeconds;
    ...
    function setDailyPayoutDuration(uint256 durationInSeconds) public {
        ...
        _oneDayInSeconds = durationInSeconds;
    }
    ...
    function setYearlyPayoutDuration(uint256 durationInSeconds) public {
        ...
        _oneYearInSeconds = durationInSeconds;
    }
    ...
    function createPool(
        ...
        require(
            ...
            isDailyPool || lockingPeriod >= _oneYearInSeconds * 365, // temporarily used for testing
    ...
    function _getWithdrawableAmount(
        ...
        uint256 _payoutTimes = (block.timestamp - depositTime) /
            (isDailyPool ? _oneDayInSeconds : _oneYearInSeconds); // temporarily used for testing
        uint256 _denominator = isDailyPool ? 36500 : 100;
        if (depositToken == rewardToken) {
            require(
                (depositAmount * APY * _payoutTimes) / _denominator > claimedAmount,
        ...
        uint256 _rewardDecimals = IERC20Metadata(address(rewardToken))
            .decimals();
        require(
            (depositAmount * APY * rateSum) / (10 ** _rewardDecimals * _denominator) >
                claimedAmount,
            "Lending-pool: no reward left to claim"
        );
        return
    ...
    // ) private view returns (uint256) {
    //     uint256 payoutTimes = (block.timestamp - depositTime) / ONE_DAY_IN_SECONDS;
    //     if (depositToken == rewardToken) {
    //         return ((depositAmount * APY * payoutTimes) / 36500) - claimedAmount;
    //     }
    //     require(
    //         payoutDayNum == payoutTimes,
    ...
    function _getWithdrawableAmountRaw(
        ...
        uint256 _denominator = isDailyPool ? 36500 : 100;
        if (depositToken == rewardToken) {
            if ((depositAmount * APY * _payoutTimes) / _denominator < claimedAmount) {
                return 0;
            }
            return
                ((depositAmount * APY * _payoutTimes) / _denominator) - claimedAmount;
