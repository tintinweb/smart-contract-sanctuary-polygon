...
*/
contract BukTrips is AccessControl {
    ...
    */
    Counters.Counter private _supplierIds;
    ...
    */
    event SetTimeLock(uint256 supplier_id, uint256 nft_id, uint256 time);
    /**
    ...
    */
    event BookRoom(uint256 booking);
    /**
    * @dev Emitted when multiple rooms are booked together.
    */
    event BookRooms(uint256[] bookings, bool status, uint256 total, uint256 commission);
    /**
    * @dev Emitted when room bookings are confirmed.
    */
    event ConfirmRooms(uint256[] bookings, bool status);
    /**
    ...
    */
    event CancelRoom(uint256 booking, bool status);

    /**
    * @dev Modifier to check the booking status.
    */ 
    modifier checkBookingStatus(uint256[] memory _ids, BookingStatus status) {
        bool flag = true;
        for(uint8 i=0; i<_ids.length;i++) {
            if(BookingDetails[_ids[i]].status != status)
                flag = false;
        }
        require(flag, "Check the Booking status");
        _;
    }
    /**
    * @dev Modifier to check the ownership of bookings.
    */
    modifier checkOwner(uint256[] memory _ids) {
        bool flag = true;
        for(uint8 i=0; i<_ids.length;i++) {
            if(BookingDetails[_ids[i]].owner != _msgSender())
                flag = false;
        }
        require(flag, "Only booking owner has access");
        _;
    }
    /**
    ...
    */
    function toggleNFTStatus(uint _id, bool status) external checkAccess(_id) {
        require((BookingDetails[_id].tokenID != 0), "NFT doesnot exist");
        uint256 threshold = BookingDetails[_id].checkin - TimeLocks[BookingDetails[_id].supplierId][_id];
        require((block.timestamp < threshold), "The NFT status cannot be toggled for now");
    ...
    */
    function registerSupplier(string memory _name, address _supplier_owner, string memory _contract_uri) external onlyRole(ADMIN_ROLE) {
        _supplierIds.increment();
        address utility_contract_addr = IBukSupplierUtilityDeployer(utility_deployer).deploySupplierUtility(_supplierIds.current(),_name, _contract_uri);
        address supplier_contract_addr = IBukSupplierDeployer(supplier_deployer).deploySupplier(_supplierIds.current(),_name, _supplier_owner, utility_contract_addr, _contract_uri);
        ISupplierContractUtility(utility_contract_addr).grantSupplierRole(supplier_contract_addr);
        Suppliers[_supplierIds.current()].id = _supplierIds.current();
        Suppliers[_supplierIds.current()].status = true;
        Suppliers[_supplierIds.current()].supplier_contract = supplier_contract_addr;
        Suppliers[_supplierIds.current()].supplier_owner = _supplier_owner;
        Suppliers[_supplierIds.current()].utility_contract = utility_contract_addr;
        emit RegisterSupplier(_supplierIds.current(), supplier_contract_addr, utility_contract_addr);
    }
    ...
    */
    function bookRoom(uint256 _supplierId, uint256 _count, uint256[] memory _total, uint256[] memory _baseRate, uint256 _checkin, uint256 _checkout) external returns (bool) {
        require(Suppliers[_supplierId].status, "Only registered suppliers have access");
        uint256[] memory bookings = new uint256[](_count);
        uint total = 0;
        uint commissionTotal = 0;
        for(uint8 i=0; i<_count;i++) {
            _bookingIds.increment();
            BookingDetails[_bookingIds.current()] = Booking(_bookingIds.current(), BookingStatus.booked, 0, _msgSender(), _supplierId, _checkin, _checkout, _total[i], _baseRate[i]);
            bookings[i] = _bookingIds.current();
            total+=_total[i];
            commissionTotal+= _baseRate[i]*commission/100;
            emit BookRoom(_bookingIds.current());
        }
        ...
        if(collectCommission) {
            ...
            if(collectPayment) {
                emit BookRooms(bookings, true, total, commissionTotal);
                return true;
            } else {
                bool revertCommission = IERC20(currency).transferFrom(buk_wallet, _msgSender(), commissionTotal);
                bool revertPayment = IERC20(currency).transferFrom(treasury, _msgSender(), total);
                return false;
            }
        } else {
            bool revertCommission = IERC20(currency).transferFrom(buk_wallet, _msgSender(), commissionTotal);
    ...
    */
    function confirmRoom(uint256 _supplierId, uint256[] memory _ids, string[] memory _uri, bool _status) external checkBookingStatus(_ids, BookingStatus.booked) checkOwner(_ids) {
        require(Suppliers[_supplierId].status, "Only the registered suppliers can be accessed");
        require((_ids.length == _uri.length), "Ids and URIs should be of same size");
    ...
    */
    function checkout(uint256 _supplierId, uint256[] memory _ids ) external checkBookingStatus(_ids, BookingStatus.confirmed) onlyRole(ADMIN_ROLE)  {
        require(Suppliers[_supplierId].status, "Only the registered suppliers can be accessed");
        for(uint8 i=0; i<_ids.length;i++) {
    ...
    */
    function cancelRoom(uint256 _supplierId, uint256 _id, uint256 _penalty, uint256 _refund, uint256 _charges ) external onlyRole(ADMIN_ROLE) {
        require(Suppliers[_supplierId].status, "Only the registered suppliers can be accessed");
        require((BookingDetails[_id].status == BookingStatus.confirmed), "Only the registered suppliers can be accessed");
        ISupplierContract _SupplierContract = ISupplierContract(Suppliers[_supplierId].supplier_contract);
