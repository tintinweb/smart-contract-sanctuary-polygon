...
contract YPredictToken is ERC20, Ownable {
    ...
    address payable public marketingWallet =
        payable(0x31B6c544168342E6f5F4283F50F749A3f44B4586);
    address payable public ctoWallet =
        payable(0x31B6c544168342E6f5F4283F50F749A3f44B4586);
    address payable public researchWallet =
        payable(0x31B6c544168342E6f5F4283F50F749A3f44B4586);

    uint256 public nativeAmountToSwap = 300000000 ether;
    ...
    uint256 public ctoFeeOnBuy;
    uint256 public ctoFeeOnSell;
    uint256 public researchFeeOnBuy;
    uint256 public researchFeeOnSell;
    ...
    uint256 public ctoShare;
    uint256 public researchShare;
    ...
    event CtoWalletUpdated(
        address indexed newCtoWallet,
        address indexed oldCtoWallet
    );

    event ResearchWalletUpdated(
        address indexed newResearchWallet,
        address indexed oldResearchWallet
    );
    ...
    constructor() ERC20("yPredict", "YPRED") {
        ...
        devShare = devFeeOnBuy + devFeeOnSell;
        marketingShare = marketingFeeOnBuy + marketingFeeOnSell;
        liquidityShare = liquidityFeeOnBuy + liquidityFeeOnSell;
        ctoShare = ctoFeeOnBuy + ctoFeeOnSell;
        researchShare = researchFeeOnBuy + researchFeeOnSell;

        totalFeesOnBuy = devFeeOnBuy
            .add(marketingFeeOnBuy)
            .add(liquidityFeeOnBuy)
            .add(ctoFeeOnBuy)
            .add(researchFeeOnSell);
        totalFeesOnSell = devFeeOnSell
        ...
        excludeFromFees(ctoWallet, true);
        excludeFromFees(researchWallet, true);
    ...
    function updateDevWallet(address newDevWallet) public onlyOwner {
        require(
            newDevWallet != devWallet,
            "The dev wallet is already this address"
        );
    ...
    function updateCtoWallet(address _ctoWallet) external onlyOwner {
        require(
            _ctoWallet != ctoWallet,
            "The CTO wallet is already this address"
        );
        excludeFromFees(_ctoWallet, true);
        emit CtoWalletUpdated(_ctoWallet, ctoWallet);
        ctoWallet = payable(_ctoWallet);
    }

    function updateResearchWallet(address _researchWallet) external onlyOwner {
        require(
            _researchWallet != researchWallet,
            "The research wallet is already this address"
        );
        excludeFromFees(_researchWallet, true);
        emit ResearchWalletUpdated(_researchWallet, researchWallet);
        researchWallet = payable(_researchWallet);
    }
    ...
    function swapTokens(uint256 tokenAmount) private {
        ...
        uint256 nativeForCto = swappedNative.mul(ctoShare).div(totalShares);
        uint256 nativeForResearch = swappedNative.mul(researchShare).div(
            totalShares
        );
        uint256 nativeForDev = swappedNative
            .sub(nativeForMarketing)
            .sub(nativeForLiquidity)
            .sub(nativeForCto)
            .sub(nativeForResearch);
        ...
        if (nativeForCto > 0) {
            transferNativeToCtoWallet(nativeForCto);
        }
        if (nativeForResearch > 0) {
            transferNativeToResearchWallet(nativeForResearch);
        }
        if (nativeForDev > 0) {
            transferNativeToDevWallet(nativeForDev);
        }
    }
    ...
    function addLiquidity(uint256 tokenAmount, uint256 nativeAmount) private {
        ...
        // add the liquidity
        quickSwapRouter.addLiquidityETH{value: nativeAmount}(
            address(this),
            tokenAmount,
    ...
    function _getTokenAmountFromNative() internal view returns (uint256) {
        ...
        path[1] = address(this);
        uint256[] memory amounts = quickSwapRouter.getAmountsOut(
            nativeAmountToSwap,
            path
        );
    ...
    function transferNativeToDevWallet(uint256 amount) private {
    ...
    function transferNativeToCtoWallet(uint256 amount) private {
        ctoWallet.transfer(amount);
    }

    function transferNativeToResearchWallet(uint256 amount) private {
        researchWallet.transfer(amount);
    }
}
