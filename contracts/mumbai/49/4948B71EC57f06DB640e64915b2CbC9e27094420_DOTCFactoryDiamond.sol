...
contract DOTCUserFacet is DOTCFacetBase {
    ...
    function queryVIPApplyParams() external view returns(uint amount, uint vipDOTC){
        amount = db.stakingTable.poolA[db.config.dotcContract].accountStakings[msg.sender].balance.add(db.stakingTable.poolB[db.config.dotcContract].accountStakings[msg.sender].balance);
        vipDOTC = consts.vipDOTC;
        return (amount, vipDOTC);
    }
...
contract SushiOracleRobot is IDOTCOracleRobot {
    using SafeMath for uint;
    using FixedPoint for *;
...
//      require(consts.priceMode==1,'DOTC Price mode is auto');
   ...
   function _initParams() internal{
        {
          consts.priceMode= 1 ;//0-auto,1-manual
