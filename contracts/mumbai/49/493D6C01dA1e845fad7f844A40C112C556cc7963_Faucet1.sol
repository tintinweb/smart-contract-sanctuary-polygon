...
contract Faucet1 { 
    ...
    function initialize (address nftAddress) onlyRole(ds.ADMIN) external {   
        ...
        lib._lockId = 1;
        lib.SIGNER = 0x3C44CdDdB6a900fa2b585dd299e03d12FA4293BC;  //Hardhat 3rd account
        ...
        lib.WETH = 0x9c3C9283D3e44854697Cd22D3Faa240Cfb032889; // testnet polygon
        ...
        public payable
    {
        ...
        require(lib.ETH == _addr[0] ? 
        msg.value >= totalAmount : IERC20(_addr[0]).transferFrom(msg.sender, address(this), totalAmount), "tx.failed");
    ...
    function claim(uint256 _id, address _swapToken) public payable {
        ...
        if (asset.features[1] && eventIs && asset.creator != asset.beneficiary){
            console.log("Faucet1_claim()_newAmount", newAmount);
        ...
        if (asset.features[2] && asset.token != _swapToken) {
            ...
            amountOraclePrice = (((uint(ls.getLatestPrice(token.priceFeedAddress)) * newAmount ) / 10** token.decimal )  
            * 10**lib._tokenVsIndex[_swapToken].decimal) / 10 ** oraclePriceLenght;
           
            amountOraclePrice -= (amountOraclePrice * lib.slippage) / 100;   // tollerable slippage rate 
            if (amountOutMinimum >= amountOraclePrice){ 
                if(giftRewardReady){
                    require(swap(asset.token, _swapToken, giftreward, asset.creator));
                }
                amountOutMinimum = ls.getAmountOutMin(asset.token, _swapToken, newAmount) ;
                require(swapped = swap(asset.token, _swapToken, newAmount,receiver));
            } else {
                if (asset.features[0]){
        ...
        asset.claimedAmount = (!asset.features[0]) ? ((asset.features[2] && swapped) ? amountOutMinimum : newAmount) : 0 ;
        gemChest.burn(_id);
