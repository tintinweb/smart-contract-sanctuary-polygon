...
contract Swapper{
    ...
    constructor(address addrSugar_, address addrMilk_, address addrCake_){
        addrSugar = addrSugar_;
        addrMilk = addrMilk_;
        addrCake = addrCake_;
    }
    ...
    function swap(address token_, uint amount) external {
        require(token_ == addrSugar || token_ == addrMilk, "Wrong token");
        require(ERC20(token_).balanceOf(msg.sender) >= amount, "Insuficient balance");

        ERC20(token_).transferFrom(msg.sender,address(this), amount);
        Cake(addrCake).mint(msg.sender, amount);
    ...
    function unswap(address token_, uint amount) external {
        require(token_ == addrSugar || token_ == addrMilk, "Wrong token");
        require(ERC20(token_).balanceOf(address(this)) >= amount, "Insufficient contract balance");
        require(Cake(addrCake).balanceOf(msg.sender) >= amount, "Insufficient balance");

        Cake(addrCake).burn(msg.sender, amount);
        ERC20(token_).increaseAllowance(address(this), amount);
        ERC20(token_).transfer(msg.sender, amount);
    }
}