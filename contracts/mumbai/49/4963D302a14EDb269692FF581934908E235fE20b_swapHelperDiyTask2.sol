...
contract swapHelperDiyTask2 is OpsTaskCreator, Ownable {
    ...
    function createTask(string memory _taskName, IAocoRouter02 _routerAddress, address[] memory _swapRouter, address[] memory _swapRouter2, uint256 _swapAmount, uint256 _interval, uint256[] memory _start_end_Time, uint256[] memory _timeList) external payable {
        lo memory llo = lo(bytes32(""), bytes(""), bytes32(""));
        setSwapInfo(_routerAddress, _swapRouter, _swapRouter2);
        llo.md5 = keccak256(abi.encodePacked(_taskName, block.timestamp, block.difficulty, msg.sender, _start_end_Time));
        require(!md5List[llo.md5], "e001");
        md5List[llo.md5] = true;
        ...
        bytes memory execData = abi.encodeCall(this.swapToken, (msg.sender, llo.md5));
        ...
        //            moduleData.args[1] = _singleExecModuleArg();
        // }
        llo.id = _createTask(address(this), execData, moduleData, ETH);
        taskList[taskAmount] = llo.id;
        userTaskList[msg.sender].push(llo.id);
        md5TaskList[llo.md5] = llo.id;
        taskConfig memory k = taskConfig(_routerAddress, msg.sender, _swapRouter, _swapRouter2, _swapAmount, _interval, 0, taskAmount, llo.md5, taskDataItem(address(this), execData, moduleData, ETH, true, _taskName, _start_end_Time, _timeList));
        taskConfigList[llo.id] = k;
        emit CounterTaskCreated(block.timestamp, msg.sender, taskAmount, llo.id);
