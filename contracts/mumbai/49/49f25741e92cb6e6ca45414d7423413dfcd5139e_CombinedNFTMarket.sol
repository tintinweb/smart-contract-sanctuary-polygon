...
contract CharityContract is ReentrancyGuard {
    ...
    uint256 maxCharitiesPerToken = 5;
    ...
    mapping(string => CharityEntry) creatorNftToCharityItemArray;
    ...
    function addCreatorNftCharities(address owner_, address nftAddress_, uint256 nftTokenId_, address[] memory charityAddresses_, uint256[] memory charityPercentages_, uint256 totalPercentage_, uint256 tokensAmount_) public {
        ...
        require(charityAddresses_.length <= maxCharitiesPerToken, "invalid number of charities");
        ...
        for (uint i = 0; i < charityPercentages_.length; i++) {
            ...
            sumCharityPercentages = sumCharityPercentages.add(charityPercentages_[i]);
        }
    ...
    function getMaxCharitiesPerToken() public view virtual returns (uint256) {
        return maxCharitiesPerToken;
    }

    function setMaxCharitiesPerToken(uint256 maxCharitiesPerToken_) external {
        require(_managers[msg.sender], "!manager");
        maxCharitiesPerToken = maxCharitiesPerToken_;
    }
    ...
    mapping(address => OwnerCharityDonations[]) ownerCharityDonations;
    mapping(address => TotalDonationsSent[]) donationsSent;
    mapping(address => TotalDonationsReceived[]) donationsReceived;
    mapping(string => NFTTokenCharityDonations[]) nftTokenCharityDonations;
    ...
    function getAllCreatorNftDonationsForCharityAndAsset(address owner_, address charityAddress_, address erc20Address_) public view returns (OwnerCharityDonations memory){
        OwnerCharityDonations memory returnDonation;
        for (uint i = 0; i < ownerCharityDonations[owner_].length; i++) {
            OwnerCharityDonations memory ownerCharityDonation = ownerCharityDonations[owner_][i];
            if (ownerCharityDonation.charityAddress == charityAddress_ && ownerCharityDonation.erc20Address == erc20Address_) {
                returnDonation = ownerCharityDonation;
            }
        }
        return returnDonation;
    }

    function getAllCreatorNftDonationsForCharity(address owner_, address charityAddress_) public view returns (OwnerCharityDonations[] memory){
        uint arraySize = 0;
        for (uint i = 0; i < ownerCharityDonations[owner_].length; i++) {
            OwnerCharityDonations memory ownerCharityDonation = ownerCharityDonations[owner_][i];
            if (ownerCharityDonation.charityAddress == charityAddress_) {
                arraySize = arraySize + 1;
            }
        }
        uint addedArrayLocation = 0;
        OwnerCharityDonations[] memory returnDonations = new OwnerCharityDonations[](arraySize);
        for (uint i = 0; i < ownerCharityDonations[owner_].length; i++) {
            OwnerCharityDonations memory ownerCharityDonation = ownerCharityDonations[owner_][i];
            if (ownerCharityDonation.charityAddress == charityAddress_) {
                returnDonations[addedArrayLocation] = ownerCharityDonation;
                addedArrayLocation = addedArrayLocation + 1;
            }
        }
    ...
    function addDonationToCharityStats(AddDonationToCharityStruct memory addDonationToCharityStruct_) public {
        ...
        if (hasNftCharities(addDonationToCharityStruct_.owner_, addDonationToCharityStruct_.nftAddress_, addDonationToCharityStruct_.nftTokenId_)) {
            ...
            bool ownerCharityDonationsIndex = false;
            for (uint i = 0; i < ownerCharityDonations[addDonationToCharityStruct_.owner_].length; i++) {
                ...
                if (ownerCharityDonation.charityAddress == addDonationToCharityStruct_.charityAddress_ && ownerCharityDonation.erc20Address == addDonationToCharityStruct_.erc20Address_) {
                    ownerCharityDonationsIndex = true;
                    ownerCharityDonations[addDonationToCharityStruct_.owner_][i].amount = ownerCharityDonations[addDonationToCharityStruct_.owner_][i].amount.add(addDonationToCharityStruct_.amount_);
                    break;
                }
            }
            if (!ownerCharityDonationsIndex) {
            ...
            for (uint i = 0; i < donationsSent[addDonationToCharityStruct_.owner_].length; i++) {
                ...
                if (donationSent.erc20Address == addDonationToCharityStruct_.erc20Address_) {
                    donationsSentIndex = true;
                    donationsSent[addDonationToCharityStruct_.owner_][i].amount = donationsSent[addDonationToCharityStruct_.owner_][i].amount.add(addDonationToCharityStruct_.amount_);
                    break;
                }
            }
            if (!donationsSentIndex) {
            ...
            bool donationsReceivedIndex = false;
            for (uint i = 0; i < donationsReceived[addDonationToCharityStruct_.charityAddress_].length; i++) {
                ...
                if (donationReceived.erc20Address == addDonationToCharityStruct_.erc20Address_) {
                    donationsReceivedIndex = true;
                    donationsReceived[addDonationToCharityStruct_.charityAddress_][i].amount = donationsReceived[addDonationToCharityStruct_.charityAddress_][i].amount.add(addDonationToCharityStruct_.amount_);

                    break;
                }
            }
            if (!donationsReceivedIndex) {
            ...
            bool nftTokenCharityDonationsIndex = false;
            for (uint i = 0; i < nftTokenCharityDonations[nftTokenCharityId].length; i++) {
                ...
                if (nftTokenCharityDonation.erc20Address == addDonationToCharityStruct_.erc20Address_) {
                    nftTokenCharityDonationsIndex = true;
                    nftTokenCharityDonations[nftTokenCharityId][i].amount = nftTokenCharityDonations[nftTokenCharityId][i].amount.add(addDonationToCharityStruct_.amount_);
                    break;
                }
            }
            if (!nftTokenCharityDonationsIndex) {
...
contract CombinedNFTMarket is ReentrancyGuard, Ownable {
    ...
    function addCreatorNftCharities(address owner_, address nftAddress_, uint256 nftTokenId_, address[] memory charityAddresses_, uint256[] memory charityPercentages_, uint256 totalPercentage_, uint256 tokensAmount_) public onlyOwner{
        CharityContract(charityContractAddress).addCreatorNftCharities(owner_, nftAddress_, nftTokenId_, charityAddresses_, charityPercentages_, totalPercentage_, tokensAmount_);
    }
    ...
    function getCharityContractAddress() public view virtual returns (address) {
        return charityContractAddress;
    }

    function setCharityContractAddress(address charityContractAddress_) external onlyOwner {
        charityContractAddress = charityContractAddress_;
    }
