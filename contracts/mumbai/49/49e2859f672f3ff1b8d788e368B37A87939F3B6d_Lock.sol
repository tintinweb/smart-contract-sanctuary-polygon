...
contract Lock is Context, Ownable, IERC721Receiver {
    ...
    // _ownedTokenInfos[owner][index] = TokenInfo
    mapping(address => mapping(uint256 => TokenInfo)) private _ownedTokenInfos;

    // _ownerIndex[nft][tokenId] = OwnerIndex
    ...
    function tokens(address owner, uint256 index) public view virtual returns (TokenInfo memory) {
    ...
    function config() public view virtual returns (address) {
        return owner();
    }

    function updateConfig(address newOwner) public virtual onlyOwner {
        _transferOwnership(newOwner);
    }
    ...
    function unlock(address nftAddress, uint256 tokenId) public virtual onlyOwner {
        ...
        delete _ownedTokenInfos[ownerIndex.owner][ownerIndex.index];
