...
contract ERC721MintMore is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function _safeMintMore(address to, uint startTokenID, uint count) internal virtual {
        _safeMintMore(to, startTokenID, count, '');
    }

    function _safeMintMore(address to, uint startTokenID, uint count, bytes memory _data) internal  {
        require(to != address(0), 'ERC721: mint to the zero address');
...
contract NFT is ERC721MintMore, Ownable {
    ...
    function mintMore(address _recipient, string memory _name, uint _count) public returns (uint) {
        ...
        _safeMintMore(_recipient, tokenCount, _count);
