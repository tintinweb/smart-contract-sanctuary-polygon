 ...
 */
contract ApnaMarketplaceAuction is
    Initializable,
    OwnableUpgradeable,
    ReentrancyGuardUpgradeable
{
    ...
    event ApnaAuctionMarketplaceContractDeployed(
    ...
    modifier onlyRoleNotSeller(address _assetContract, uint256 _tokenId) {
        require(
            _msgSender() != auctions[_assetContract][_tokenId].admin &&
                _msgSender() != auctions[_assetContract][_tokenId].owner &&
    ...
    modifier onlyRoleTopBidder(address assetContract, uint256 tokenId) {
        require(
            highestBids[assetContract][tokenId].bidder != address(0) &&
                _msgSender() == highestBids[assetContract][tokenId].bidder,
            "you are not the highest bidder"
        );
        _;
    }

    modifier onlyRoleSellerORTopBidder(address assetContract, uint256 tokenId) {
        require(
            (highestBids[assetContract][tokenId].bidder != address(0) &&
                _msgSender() == highestBids[assetContract][tokenId].bidder) ||
                (_msgSender() == IERC721(assetContract).ownerOf(tokenId) &&
                    _msgSender() == auctions[assetContract][tokenId].owner) ||
                _msgSender() == auctions[assetContract][tokenId].admin,
    ...
    modifier onlyRoleSeller(address assetContract, uint256 tokenId) {
        require(
            (_msgSender() == IERC721(assetContract).ownerOf(tokenId) &&
                _msgSender() == auctions[assetContract][tokenId].owner) ||
                _msgSender() == auctions[assetContract][tokenId].admin,
    ...
    function init(
        ...
        require(
            _bidWithdrawalLockTime >= 1200 && _bidWithdrawalLockTime <= 86400,
        ...
        emit ApnaAuctionMarketplaceContractDeployed(
    ...
    function createNewNftAuction(
        ...
        _onlyApproved(_assetContract, _msgSender());
        ...
        require(
            auctions[_assetContract][_tokenId].endTime == 0,
        ...
        auctions[_assetContract][_tokenId].inState = State.DONT_EXIST;

        // Setup the auction
        // @dev todo: Currently owner and admin are kept same
        auctions[_assetContract][_tokenId] = Auction({
            owner: _msgSender(),
            admin: _msgSender(),
        ...
        return auctions[_assetContract][_tokenId].Id;
    }
    ...
    function makeBid(
        ...
        nonReentrant
        whenNotPaused
        onlyRoleNotSeller(_assetContract, _tokenId)
        onlyRoleNotContracts
        returns (bool success)
    {
        Auction storage auction = auctions[_assetContract][_tokenId];
        ...
        HighestBid storage highestBid = highestBids[_assetContract][_tokenId];
        ...
        require(
            _bidAmount >= highestBid.bid.add(minBidIncrement),
    ...
    function withdrawBid(
        ...
        nonReentrant
        whenNotPaused
        onlyRoleTopBidder(_assetContract, _tokenId)
    {
        // check auction end time is over and withdrawal lock time is over
        require(
            block.timestamp < auctions[_assetContract][_tokenId].endTime &&
                (auctions[_assetContract][_tokenId].endTime - block.timestamp >=
        ...
        delete highestBids[_assetContract][_tokenId];
        ...
        emit BidWithdrawn(_assetContract, _tokenId, _msgSender(), previousBid);
    }
    ...
    function settleAuction(
        ...
        external
        onlyRoleSellerORTopBidder(_assetContract, _tokenId)
        onlyApproved(_assetContract, _msgSender())
    {
        Auction storage auction = auctions[_assetContract][_tokenId];
        ...
        delete auctions[_assetContract][_tokenId];
        sold++;
    ...
    function withdrawAuction(
        address _assetContract,
        uint256 _tokenId
    ) external nonReentrant onlyRoleSeller(_assetContract, _tokenId) {
    ...
    function takeHighestBid(
        ...
        external
        onlyRoleSeller(_assetContract, _tokenId)
        onlyApproved(_assetContract, _msgSender())
    {
        Auction storage auction = auctions[_assetContract][_tokenId];
    ...
    function updateAuctionEndTime(
        ...
        uint256 _tokenId,
        uint256 _endTimestamp
    ) external onlyRoleSeller(_assetContract, _tokenId) {
        Auction storage auction = auctions[_assetContract][_tokenId];
    ...
    function updateAuctionStartTime(
        ...
        emit AuctionStartTimeUpdated(_assetContract, _tokenId, _startTime);
    }
    ...
    function updateAuctionReservePrice(
        ...
        external
        onlyRoleSeller(_assetContract, _tokenId)
        onlyReservePriceForUpdate(_assetContract, _tokenId, _newReservePrice)
    {
        Auction storage auction = auctions[_assetContract][_tokenId];
        require(auction.endTime > 0, "no auction exists");
    ...
    function updateAuctionPayToken(
        ...
        uint256 tokenId,
        address newPayToken
    ) external onlyRoleSeller(assetContract, tokenId) {
        Auction storage auction = auctions[assetContract][tokenId];
    ...
    function updateMinimumPrice(
        ...
        external
        onlyRoleSeller(_assetContract, _tokenId)
        onlyMinBidForUpdate(_assetContract, _tokenId, _newMinBid)
    {
        Auction storage auction = auctions[_assetContract][_tokenId];
        require(auction.endTime > 0, "no auction exists");
        require(block.timestamp <= auction.endTime, "auction has ended");
    ...
    function _onlyReservePriceForUpdate(
        ...
        Auction memory auction = auctions[_assetContract][_tokenId];
