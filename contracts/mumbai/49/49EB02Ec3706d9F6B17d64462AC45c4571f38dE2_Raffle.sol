...
contract Raffle is VRFConsumerBaseV2, KeeperCompatibleInterface {
    ...
    event winnerPicked(uint256 ZodiaAnimal);
    ...
    function enterRaffle(ZodiaAnimal _playerChoosen) public payable {
        // revert()
        ...
        if (exists) {
            //update the fee
            s_players[index].entranceFee += msg.value;
            emit raffleEnterUpdate(msg.sender);
        } else {
            Player memory _player;
            _player.playerAddress = payable(msg.sender);
            _player.entranceFee = msg.value;
            _player.playerChoosen = _playerChoosen;
            s_players.push(_player);

            //update the total zodia fee
            // s_zodiaAnimalToal[uint(_playerChoosen)] += msg.value;
    ...
    function fulfillRandomWords(
        ...
        s_winnerZodia = indexOfWinnerZodiaAnimal;
        while(s_players.length>0){
            if(getPlayerChoosen(s_players.length-1)==ZodiaAnimal(indexOfWinnerZodiaAnimal)){
                (bool success, ) = getPlayerAddress(s_players.length-1).call{
                    value: (getPlayerEntranceFee(s_players.length-1)*4)
                }("");
        ...
        //         //     revert Raffle__TransferFail();
        //         // }
        //         // console.log(s_players[i].playerAddress);
        //     }
        // }
        ...
        emit winnerPicked(indexOfWinnerZodiaAnimal);
    }

    // function remove(uint _index) public {
    //     require(_index < s_players.length, "index out of bound");

    //     for (uint i = _index; i < s_players.length - 1; i++) {
    ...
    function getWinnerZodia() public view returns (uint256) {
        return s_winnerZodia;
    }

    // function getZodiaNumb() public view returns (ZodiaAnimal) {
    //     return ZodiaAnimal.RAT;
    // }
