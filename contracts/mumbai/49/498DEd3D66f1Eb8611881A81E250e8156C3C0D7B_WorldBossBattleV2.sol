...
contract WorldBossBattle is Initializable, ContextUpgradeable, GameStakeOps, PausableUpgradeable, ReentrancyGuardUpgradeable, ERC1155Holder {
    ...
    event SetBattlePeriodUnstakeStatus(bool _status);
     ...
     */
    function setToken(address _tokenAddress) external hasManagerRole {
     ...
     */
    function setCDHNFTContractAddress(address _nftContract) external hasManagerRole {
        address msgSender = _msgSender();
     ...
     */
    function setWBBActionsContract(address _wbbActionsAddress) external hasManagerRole {
        address msgSender = _msgSender();
     ...
     */
    function setMaxStakeCount(uint256 _maxCount) external hasManagerRole {
        address msgSender = _msgSender();
     ...
     */
    function setBattlePeriodUnstakeStatus(bool _unstakeStatus) external hasManagerRole {
        enableBattlePeriodUnstake = _unstakeStatus;
        emit SetBattlePeriodUnstakeStatus(_unstakeStatus);
    }
     ...
     */
    function setMinTokensRequired(uint256 _value) external hasManagerRole {
    ...
    function stake(uint256 _tokenId, string memory _battleId) external override whenBattleIsActive(_battleId) {
        // require(!wbbActions.checkBattleEnded(_battleId), "WBB: battle not started");
    ...
    function internalStake(address _player, uint256 _tokenId, string memory _battleId) internal whenNotPaused nonReentrant {
        ...
        if (checkRequiredTokenBalance(existingStakedTokensCount + 1, tokenBalance)) {
            ...
            // ICDHNFTInventory(cdhNFT).safeTransferFrom(_player, address(this), _tokenId, 1, "0x");
    ...
    function stakeAllTokens(string memory _battleId) external whenBattleIsActive(_battleId) {
        address player = _msgSender();
        uint256[] memory allTokens = cdhNFT.getAllTokens(player);
        uint256 nftBalance = allTokens.length;
        uint256[] memory tokenAmounts = new uint256[](allTokens.length);
        if (nftBalance > 0) {
            for (uint i = 0; i < nftBalance; i++) {
                internalStake(player, allTokens[i], _battleId);
                tokenAmounts[i] = 1;
            }
        }
        ICDHNFTInventory(cdhNFT).safeBatchTransferFrom(_msgSender(), address(this), allTokens, tokenAmounts, "0x");
     ...
     * @dev it calls internalUnStakeAll function for further processing
     */
    function unstakeAllInternal(address _player, string memory _battleId) external unstakeValidate(_battleId) hasManagerRole {
    ...
    ) external unstakeValidate(_battleId) hasManagerRole {
     ...
     */
    function pause() external hasManagerRole {
        _pause();
    }
     ...
     */
    function unpause() external hasManagerRole {
        _unpause();
    }
}
...
interface GameStakeOps {
    ...
    // TODO: function stakeAllTokens(uint256 _battleId) external;
