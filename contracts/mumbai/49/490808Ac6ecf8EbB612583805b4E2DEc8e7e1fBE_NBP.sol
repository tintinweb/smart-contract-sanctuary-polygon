...
import "./libraries/EqualUtils.sol";
...
contract Projects {
    ...
    function updateProjectById(
        ...
        // That name has been taken.
        require(EqualUtils._equalStrings(currentName, _name) || getProjectIdByName(_name) == 0);

        if (bytes(_name).length != 0 && !EqualUtils._equalStrings(currentName, _name)) {
            _projects[_projectId].name = _name;
            _projectMapping[_name] = _projectId;
            delete _projectMapping[currentName];
        }

        if (bytes(_imageUrl).length != 0 && !EqualUtils._equalStrings(currentProject.imageUrl, _imageUrl)) {
            _projects[_projectId].imageUrl = _imageUrl;
        }

        if (bytes(_description).length != 0 && !EqualUtils._equalStrings(currentProject.description, _description)) {
...
contract SBT is Projects, ERC165, IERC1155, IERC1155MetadataURI {
    ...
    function _mint(address _to, uint256 _id, uint256 _projectId) internal {
        ...
        // Project does not exist.
        require(!EqualUtils._equalStrings(getProjectById(_projectId).name, ""));

        // Already minted to this user.
        require(_balances[_id][_to] == 0);
