...
// mock class for testing ConsentRegistry
contract ConsentRegistryMock is ConsentRegistry {

    constructor() {
        addConsent('MARKETING', 'A', true, bytes32(0x3ac225168df54212a25c1c01fd35bebfea408fdac2e31ddd6f80a4bbf9a5f1cb));
        addConsent('RODO', 'B', false, bytes32(0x67fad3bfa1e0321bd021ca805ce14876e50acac8ca8532eda8cbf924da565160));
        addConsent('MEDICAL', 'C', true, bytes32(0x4e03657aea45a94fc7d47ba826c8d667c0d1e6e33a64a036ec44f58fa12d6c45));
    }
...
contract ManagersMock is Managers {

    constructor() Managers() {}

    function test_updateManager(address manager, uint32[] memory consentsIds) external {
        _updateManager(manager, consentsIds);
    }

    function test_isManagerAllowedToChangeConsents(uint32[] memory consentsIds) view external returns (bool) {
        return _isManagerAllowedToChangeConsents(consentsIds);
    }
...
interface IConsentsOwnable {

    function isOwner(bytes32 id) external view returns (bool);

    function ownerExists(bytes32 id) external view returns (bool);

    function addOwner(bytes32 id, address owner) external; 

    function removeOwner(bytes32 id, address owner) external;

    event ConsentsOwnerAdded(bytes32 indexed id, address indexed owner, address addedBy);

    event ConsentsOwnerRemoved(bytes32 indexed id, address indexed owner, address removedBy);
}