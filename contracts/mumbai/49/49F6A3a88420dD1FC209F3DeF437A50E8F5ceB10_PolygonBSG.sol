...
contract PolygonBSG {
    ...
    //for debug
    event Debug(address user, uint256 pos, uint256 amount);
    ...
    function _deposit(address _user, uint256 _amount) private {
        ...
        emit Debug(_user, 0, user.totalDeposit);
        emit Debug(_user, 1, user.totalFreezed);
        ...
        emit Debug(_user, 3, reward.cycleNumber);
        ...
        for (uint256 i = 0; i < orderInfos[_user].length; i++) {
            ...
            if (block.timestamp > order.unfreeze && !order.isClaimed && order.cycle < rewardInfo[_user].cycleNumber.sub(1)) {
                ...
                if (isStopLoss30ofATH || isStopLoss50ofATH) {
                    ...
                    emit Debug(_user, 4, rewardInfo[_user].capitals);
                    emit Debug(_user, 5, rewardInfo[_user].statics);
                }
        ...
        if(day1kDepositCount > 0){
            for(uint256 i = day1kDepositCount; i > 0; i--){
                ...
                if(userAddr != address(0)){
                    ...
                    emit Debug(userAddr, 7, rewardInfo[userAddr].more1k);
                }
            }
            more1kIncomePool = 0;
        }
    ...
    function _updateReferInfo(address _user, uint256 _amount) private {
        ...
        if (upline != address(0) && _user != upline) {
            ...
            emit Debug(upline, 9, userInfo[upline].teamTotalDeposit);
            emit Debug(upline, 10, userInfo[upline].teamTotalVolume);
    ...
    function _releaseUpRewards(address _user, uint256 _amount) private {
        ...
        if (upline != address(0)) {
            ...
            emit Debug(_user, 16, upRewards.infinityBonusReleased);
        }
    }
    ...
    function withdraw() external {
        ...
        emit Debug(msg.sender,20, staticReward);
        emit Debug(msg.sender,21, staticSplit);
        uint256 splitAmt = staticSplit;
        uint256 withdrawable = staticReward;

        (uint256 dynamicReward, uint256 dynamicSplit) = _calCurDynamicRewards(msg.sender);
        emit Debug(msg.sender,22, staticReward);
        emit Debug(msg.sender,23, staticSplit);
        ...
        emit Debug(msg.sender,24, userRewards.split);
    ...
    function _updateMembership(address _user) private {
        ...
        emit Debug(_user, 2, user.membership);
    }
