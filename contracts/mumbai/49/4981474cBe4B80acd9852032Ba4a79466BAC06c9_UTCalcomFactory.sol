...
import "../implementers/UTCalcomStorage.sol";
import "../implementers/UTCalcomController.sol";

/**
* @dev This factory is used to create new traits (perks) in a community
*/
contract UTCalcomFactory is BlackHolePrevention {
    ...
    string      public constant     REGISTRY_KEY_FACTORY = "TRAIT_TYPE_7_FACTORY";
    ...
    // Errors
    error UTCalcomFactoryNotCurrent(address);
    error UTCalcomFactoryInvalidCommunityId(uint32);
    error UTCalcomFactoryUnauthorized();
    error UTCalcomFactoryTraitRegistryNotInstalled();
    error UTCalcomFactoryTokenNotInstalled();
    ...
    function addTrait(
        ...
        if(GalaxisRegistry.getRegistryAddress(REGISTRY_KEY_FACTORY) != address(this)) {
            revert UTCalcomFactoryNotCurrent(address(this));
        }
        ...
        if(crAddr == address(0)) {
            revert UTCalcomFactoryInvalidCommunityId(_inputTrait.communityId);
        }
        ...
        // NFT must exist!
        if(ERC721 == address(0)) {
            revert UTCalcomFactoryTokenNotInstalled();
        }
        ...
        if(controllerAddr == address(0)) {
            ...
            controllerCtr = UTCalcomController(controllerAddr);
        }

        // Deploy _storage_
        UTCalcomStorage storageCtr = new UTCalcomStorage(address(traitRegistry), traitId);
 ...
 *     ██████  ██████  ██   ████    ██    ██   ██  ██████  ███████ ███████ ███████ ██   ██ 
 *                                                                                                     
 *    GALAXIS - Cal.com appointment Trait (logic) controller
 *
 *    This contract is used for changing trait values in a UTCalcomStorage contract
 *
 */
...
contract UTCalcomController is UTGenericTimedController {

    uint256     public constant version              = 20230701;

    uint8       public constant TRAIT_TYPE           = 7;                       // Cal.com appointment
    string      public constant APP                  = "calcomappointment";     // Application in the claim URL
    uint8              constant TRAIT_OPEN_VALUE     = 1;
    uint8              constant TRAIT_REDEEMED_VALUE = 2;

    // Errors
    error UTCalcomControllerNoToken();
    error UTCalcomControllerNotOwnerOfToken(address);
    error UTCalcomControllerInvalidState(uint8);
    ...
    function setValue(uint16[] memory tokenId, uint8 value, address traitStorage) public checkValidity {
        if(tokenId.length == 0) {
            revert UTCalcomControllerNoToken();
        }
        ...
        for(uint8 i = 0; i < tokenId.length; i++) {
            ...
            if(currentValue == 1 && value == 2) {
                ...
                if(erc721.ownerOf(currentTokenId) != msg.sender) {
                    revert UTCalcomControllerNotOwnerOfToken(msg.sender);
                }
                ITraitStorage(traitStorage).setValue(currentTokenId, 2);
            } else {
                revert UTCalcomControllerInvalidState(value);
            }
        }
    }
}
