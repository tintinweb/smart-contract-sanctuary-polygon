...
// File: contracts/AsicNFT_Staking4.sol
...
contract Staking_NFT_D is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 constant REQUIRED_STAKE_TOKENS = 100 * 10 ** 18; // if your token has 18 decimals
    ...
    function stake(uint256[] calldata _tokenIds) external whenNotPaused {
        require(stakeToken.balanceOf(msg.sender) >= REQUIRED_STAKE_TOKENS, "Not enough stake tokens");

        stakeToken.safeTransferFrom(msg.sender, address(this), REQUIRED_STAKE_TOKENS);
        ...
        if (staker.stakedTokenIds.length > 0) {
            ...
            staker.stakingTimeLeft = REQUIRED_STAKE_TOKENS / 10 ** 18 * SECONDS_IN_MINUTE; // convert token amount to minutes then to seconds
        }
    ...
    function updateRewards(address _stakerAddress) private {
        Staker storage staker = stakers[_stakerAddress];

        uint256 rewards = calculateRewards(_stakerAddress);

        staker.timeOfLastUpdate = block.timestamp;
        staker.unclaimedRewards += rewards;
    }

    function calculateRewards(address _stakerAddress) private view returns (uint256) {
        Staker storage staker = stakers[_stakerAddress];

        uint256 timeSinceLastUpdate = block.timestamp - staker.timeOfLastUpdate;
        uint256 rewardTime = timeSinceLastUpdate < staker.stakingTimeLeft ? timeSinceLastUpdate : staker.stakingTimeLeft; // limit reward time by staking time
        uint256 minutesElapsed = rewardTime / SECONDS_IN_MINUTE;
        uint256 stakedTokenIdsCount = staker.stakedTokenIds.length;

        return minutesElapsed * stakedTokenIdsCount * rewardsPerMinute;
    }

    function setRewardsPerMinute(uint256 _newRewardsPerMinute) external onlyOwner {
        rewardsPerMinute = _newRewardsPerMinute;
    }
