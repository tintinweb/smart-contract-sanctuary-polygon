...
contract Stacking is Ownable {
    ...
    IBEP20 public rewardToken=IBEP20(0x0DbC6724fFACF4D4AcEaE8b97661d10176339dD1);
    struct Plan {
        uint256 time;
        uint256 percent;
		uint256 minAmount;
    ...
    */
    function calclulateReward(uint256 id) public view returns (uint256) {
        if(Stack[id].amount > 0) {
            ...
            uint256 tokensPerSecond =Stack[id].amount.mul(plans[Stack[id].plan].time).div(100).div(31536000);
    ...
    function claim(uint256 id) public {
        uint256 finish =Stack[id].time.add(plans[Stack[id].plan].time.mul(1 days));
        require(hasStartStakcing, "Staking isn't started right now!");
        require(Stack[id].isWithdrawal == false, "Stake already withdrawn!");
        require(Stack[id].userAddress == msg.sender, "Only stake owner can claim tokens!");
        ...
        require(
            stakedToken.balanceOf(address(this)) >= Stack[id].amount,
            "Insufficent Balance"
        );

        rewardToken.safeTransfer(Stack[id].userAddress, reward);
        stakedToken.burn(Stack[id].amount);
    ...
    */
    ...
    constructor() {
        plans.push(Plan(730, 20,100000000000000000000));
        plans.push(Plan(1095, 22,200000000000000000000));
        plans.push(Plan(1825, 26,500000000000000000000));
        plans.push(Plan(3650, 30,1000000000000000000000));
    }
}