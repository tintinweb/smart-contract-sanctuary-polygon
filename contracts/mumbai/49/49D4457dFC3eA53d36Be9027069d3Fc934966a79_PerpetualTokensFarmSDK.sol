...
contract PerpetualTokensFarmSDK is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    /**
     * @notice function is getting necessary info for withdraw
     *
     * @param _user - address of user
        ...
        returns(uint256[] memory, uint256[] memory)
    {
        ...
        uint256 noOfStakeToBeConsumed;
        if(lastStakeThatShouldBeConsumed == firstStakeThatShouldBeConsumed){
            noOfStakeToBeConsumed = 1;
        }
        else {
            noOfStakeToBeConsumed = lastStakeThatShouldBeConsumed.sub(firstStakeThatShouldBeConsumed);
            noOfStakeToBeConsumed++;
        }

        uint256[] memory idOfStakesToBeConsumed = new uint256[](noOfStakeToBeConsumed);
        uint256[] memory amountFromStakesToBeTaken = new uint256[](noOfStakeToBeConsumed);

        for(uint256 i = 0; i < noOfStakeToBeConsumed; i++){
            uint256 stakeId = firstStakeThatShouldBeConsumed + i;
            idOfStakesToBeConsumed[i] = stakeId;
            StakeInfo memory stake = stakeInfo[_user][stakeId];
            if(stakeId == lastStakeThatShouldBeConsumed) {
                amountFromStakesToBeTaken[i] = stake.amount.sub(noOfCollectedAmount.sub(_amount));
            }
            else{
                amountFromStakesToBeTaken[i] = stake.amount;
            }
        }

        return(amountFromStakesToBeTaken, idOfStakesToBeConsumed);
    }
        ...
        validateStakeByStakeId(_user, stakeId)
    {
        ...
        if(stakeInfo[_user][lastStakeConsumed[_user]].amount == 0){
            require((lastStakeConsumed[_user] + 1) == stakeId);
        }
        else{
            require(lastStakeConsumed[_user] == stakeId);
        }
        ...
        uint256 pendingAmount = _payoutRewardsAndUpdateState(_user, stakeId, _amount, true);
        lastStakeConsumed[_user] = stakeId;

        // Emit Withdraw event
        emit ReducedStake(_user, stakeId, _amount, pendingAmount, stake.epochId);
    }
        ...
        external
    {
        ...
        uint256 _amount = totalPendingAmountForUser[addressOfUser];
        _checkValueBiggerThan(_amount, 0);
        rewardToken.transfer(addressOfUser, _amount);
        totalPendingAmountForUser[addressOfUser] = 0;

        emit Withdraw(addressOfUser, _amount);
    }
