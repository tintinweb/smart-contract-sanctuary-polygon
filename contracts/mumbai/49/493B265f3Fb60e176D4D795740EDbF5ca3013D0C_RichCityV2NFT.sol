...
interface IRichcityNFT {
    ...
    function mint(uint256 amount, IERC20 token) external;
}
...
contract RichCityV2NFT is ERC721Enumerable, Ownable, IERC721Receiver, Pausable {
    ...
    //    IRichcityNFT public logicv1;
    //    function setLogicV1(IRichcityNFT _logicv1) public onlyOwner {
    //        logicv1 = _logicv1;
    //    }


    uint256 public gasPrice = 1000000;

    function mint(uint256 amount, IERC20 token) external nonReentrant {

        //        (bool success, bytes memory result) = logicv1.mint(amount,token).delegatecall{gas : gasPrice}(bytes4(keccak256("mint(uint256,address)")),amount,token);
        (bool success, bytes memory result) = logicv1.delegatecall(abi.encodeWithSignature("mint(uint256,address)", amount, token));
        require(success, "gas not enough");

        uint256 totalCost = 0;
        uint256 costValue = randomSource.get_public_price_for_different_round(minted);
        if (block.timestamp < PUBLIC_SALE_TIME && minted <= 1000) {
            require(minted + amount <= 1000, "Mint exceed");
            require(
                randomSource.whiteListedOne(_msgSender()) || randomSource.whiteListedTwo(_msgSender()),
                "Not in Whitelist"
            );
            require(
            ...
            if (randomSource.whiteListedOne(_msgSender()) && randomSource.whiteListedTwo(_msgSender())) {
                costValue = ONE_WHITE_PRICE;
            } else if (randomSource.whiteListedTwo(_msgSender())) {
        ...
        for (uint256 i = 0; i < amount; i++) {
            if (block.timestamp < PUBLIC_SALE_TIME && minted <= 2000) {//todo 前2000为白名单
                totalCost += costValue;
            } else {
                totalCost += randomSource.get_public_price_for_different_round(minted);
            }
        }
        ...
        for (uint256 id = 0; id < tokenIds.length; id++) {
            ...
            if (t.kind == 3) {
                ...
                if (offer[id] >= costValue) {
                    ...
                    uint256 seed = random(minted);
                    randomSource.update(minted ^ seed);
                    if ((seed & 0xFFFF) % 100 < successProbability) {// 根据offer比例成功
                        CASH.burnFrom(msg.sender, costValue * (10 ** CASH.decimals()));
                        t.level++;
                    } else {
                        CASH.burnFrom(msg.sender, costValue * (10 ** CASH.decimals()));
    ...
    {
        if (tokenTypes[tokenId].kind == 1) {//升级
    ...
    function generatePublic(
        ...
        t2.kind = p;
        t2.level = 1;
        if (p == 4) {//kind 4 是 gangster_leader
            t2.kind = 1;
            t2.level = 4;
        }
        t2.mintCount = 0;
    ...
    function mintCost() public view returns (uint256) {
        return randomSource.get_public_price_for_different_round(minted);
    }

    function whiteListedOne(address addr) public view returns(bool){
        return randomSource.whiteListedOne(addr);
    }

    function whiteListedTwo(address addr)public view returns(bool){
        return randomSource.whiteListedTwo(addr);
    }
}