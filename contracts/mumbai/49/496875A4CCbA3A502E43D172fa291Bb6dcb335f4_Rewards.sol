...
contract Rewards is Initializable, PausableUpgradeable, OwnableUpgradeable, ERC20Upgradeable, ReentrancyGuardUpgradeable {
    ...
    */
    ...
    function checkRewards(uint256 msgID, uint256 postCount) public onlyAdmins {

        // If the user doesn't have a reward Tier for this Reward ID
        if (userRewardTier[tx.origin].length < currentRewardID + 1){

            // Loop through until they get to it and initialize it to zero
            for (uint256 i=0; i <= currentRewardID; i++) {
                userRewardTier[tx.origin].push(0);
            }
        }

        // Make sure they're on the current reward ID to be tracked
        if (userRewardID[tx.origin] != currentRewardID){

            // If not, change it and reset their level
            userRewardID[tx.origin] = currentRewardID;
            userRewardTier[tx.origin][currentRewardID] = 0;
        }

        uint256 level = userRewardTier[tx.origin][currentRewardID];
        ...
        if (level < amountPerLevel[currentRewardID].length) {
            ...
            if (postCount >= amountPerLevel[currentRewardID][level]){
                ...
                userRewardTier[tx.origin][currentRewardID]++;
                ...
                postResponse(userRewardTier[tx.origin][currentRewardID]);
    ...
    */
    function postResponse(uint256 level) private whenNotPaused {
        ...
        taggedAccounts[0] = tx.origin;

        string memory message = string(abi.encodePacked("Congrats to @", KUtils.addressToString(tx.origin), "! They won a Raffle Ticket NFT and are now at level ", KUtils.toString(level)));
        message = string(abi.encodePacked(message, "! Raffle Tickets holders are entered to win a whitelist spot for the minting of the coveted Amulets! Only Amulet holders will be able to mint a Kultist! The more Raffle Tickets you have, the more chances you have to win!"));
        ...
        uint256 groupID = 82901138019809681465386440910526222986491416502260907764523583795075734899591;  // Are you posting on behalf of a group? If so, add the GroupID here (KUTHULU-Rewards)
