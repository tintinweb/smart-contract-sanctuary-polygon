...
contract Kyc is AccessControl {
  ...
  function activateKyc() external onlyRole(DEV) {
    kycActivated = 1;
  }

  function deactivateKyc() external onlyRole(DEV) {
    kycActivated = 0;
  }
  ...
  function addWhitelist(address toWhitelist) external onlyRole(DEV) {
  ...
  function canBuy(address toCheck) public view returns (bool) {
    return
      (kycActivated == 0 || !cannotBuy[countryCode[toCheck]]) &&
      isWhitelisted[toCheck];
  }
