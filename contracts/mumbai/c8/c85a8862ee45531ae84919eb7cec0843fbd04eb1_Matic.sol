    ...
    function payoutOf(address _addr) view external returns(uint256 payout, uint256 max_payout, uint256 pending_payout) {
        ...
        if(users[_addr].deposit_payouts < max_payout) {
            ...
            if(users[_addr].package == 1){
                payout = (users[_addr].deposit_amount * ((now - users[_addr].deposit_time) / payoutPeriod) / 273) - users[_addr].deposit_payouts;
            }
            ...
            if(users[_addr].package == 3){
                payout = (users[_addr].deposit_amount * ((now - users[_addr].deposit_time) / payoutPeriod) / 231) - users[_addr].deposit_payouts;
            }
