    ...
    PausableUpgradeable
{
    ...
    function deposit(
        ...
        uint256 round,
        uint256 amount
    ) external whenNotPaused onlyOperator subcriptionExist(subscriptionID) {
        Subscription storage subscription = _subscriptions[subscriptionID];
        Scheme memory scheme = _schemes[subscription.schemeId];

        // uint256 totalRound = scheme.vestingDuration / month;
        //! for test
        uint256 totalRound = scheme.vestingDuration / day;
    ...
    function claim(uint256 subscriptionID) external whenNotPaused {
        ...
        require(subscription.totalAmount > 0, "total supply is zero");

        if (scheme.schemeType == SchemeType.investor) {
            if (block.timestamp > subscription.endTimeVesting) {
                uint256 totalClaim = subscription.totalAmount;
                erc20Token.safeTransfer(msg.sender, totalClaim);
                ...
                erc20Token.safeTransfer(msg.sender, claimAvailable);
            ...
            if (block.timestamp > subscription.endTimeVesting) {
                uint256 totalClaim = subscription.totalAmount;
                erc20Token.safeTransfer(msg.sender, subscription.totalAmount);
                subscription.vestedAmount += totalClaim;
                subscription.totalAmount -= totalClaim;
                subscription.numberOfclaimed += 1;
                subscription.nextTimeClaim = 0;
