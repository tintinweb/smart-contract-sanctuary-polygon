...
//-------------------[PRICE CHART STORAGE]----------------
    ...
    uint constant JOIN_PRICE=33e18; // join price / entry price
...
// Replace below address with main token token
   ...
   struct userInfo {
    ...
    struct userIncome{
        ...
        uint creditFund;    //transfer fund from other user.
    ...
    function reTopup() external returns(bool) {
		...
		userGains[msg.sender].withdrawLimit += JOIN_PRICE*2;
        }
    ...
    function _activateUser() internal {
        ...
        _royaltyQualify(msg.sender);
         for (uint j=1; j<=TEAM_DEPTH;j++){

                _distributeUnilevelIncome(msg.sender,ref,j);
                ref= userInfos[ref].referral;
                

            }
        }
        userInfos[msg.sender].globalPoolCount+=1;
        userGains[msg.sender].topup_Count+=1;
        
        _royaltyQualify(ref);
        _royaltyCollection(1);
        _closingRoyality();
    ...
    function totalGains_() public view returns (uint){

        userIncome memory gains;

        gains= userGains[msg.sender];
    ...
    function withdrawFund() external returns (bool) {
        ...
        uint balance=totalGains-userGains[msg.sender].totalWithdrawn;
...
function _updateTeamNum(address _user) private {
        ...
        for(uint i=1; i <= TEAM_DEPTH; i++){
            ...
            if(ref != defaultAddress){
                ...
                strogLeg=userInfos[ref].teamCount;
                //data update
                _distributeUnilevelIncome(_user,ref,i);
                // change referral
                ref = userInfos[ref].referral;
                // update strong leg
                if(userInfos[ref].strongTeam<strogLeg){

                    userInfos[ref].strongTeam=strogLeg;
                }
                ...
                if (  i!=TEAM_DEPTH+1 ){
                     ...
                     _distributeUnilevelIncome(_user,defaultAddress,i);
  ...
  function _distributeGapBonus(address _user)  internal returns(bool){
     ...
     for (uint i=1;i<=5;i++){
        ...
        if(ref!=defaultAddress){
            ...
            if (directAct>=GAP_DIRECT ){
                 ...
                 // ELIGIBLE 
                uint strongLeg = userInfos[ref].strongTeam; // 3 is user Direct team here
                uint totalLeg=userInfos[ref].teamCount; // 3 is user Direct team here;
                ...
                if ((totalLeg-GR_STRONG_LEG)>=GR_OTHER_LEG && strongLeg>=GR_STRONG_LEG ){
                    ...
                    emit gapGenerationBonus_Ev(_user,ref ,amount);
                        ...
                        i--; // current call increment will setttle through this code chunk
    ...
    function _distributeUnilevelIncome(address _user,address ref, uint level) internal {
        // get its referral
       
        uint activeDirect = userInfos[ref].activeDirect;
        uint amnt = getLevelDistRate(level);
        
           if (activeDirect>=2 && ref != defaultAddress ){
...
// global royalty
  function _royaltyQualify(address _ref) internal {
      ...
      //active Royalty
      if(directCount>=AR_DIRECTS && userInfos[_ref].GR_Qualify==false){
            //Qualified
      ...
      else if(userInfos[_ref].AR_VaildityIndex<royalty_Index && userInfos[_ref].GR_Qualify==true){
          //disqualified
          userInfos[_ref].GR_Qualify=false;
          AR_Qualifier_Count--;
      ...
      if (directCount>=GR_DIRECT && userInfos[_ref].GR_Qualify==false){
        ...
        if ((userInfos[_ref].teamCount-GR_STRONG_LEG)>=GR_OTHER_LEG && userInfos[_ref].strongTeam>=GR_STRONG_LEG){
         	//Qualified
