...
import "./Token/interfaces/IPazaToken.sol";
...
contract Factory is
    ...
    AccessControl
{
    // bytes32 override public constant BANK_ROLE = keccak256("BANK_ROLE");
    using SafeERC20 for IPazaToken;
     ...
     * @param _masterPool address of pool contract
     * @param _paza address of paza token
     * @param _nft address of LoanNFT contract
     * @param _postpaidUsdcWallet address of USDC wallet
    ...
    function initialize(
        ...
        Paza = IPazaToken(_paza);
        NFT = ILoanNFT(_nft);
        postpaidUsdcWallet = _postpaidUsdcWallet;
    ...
    /**
     * @notice used set the USDC wallet
     * @param _wallet address of USDC wallet to be updated
     **/
    function setpostpaidUsdcWallet(address _wallet)
        external
        validation(_wallet)
    ...
    function setprepaidUsdcWallet(address _wallet)
        external
        validation(_wallet)
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    /**
     * @notice used to set the foundation wallet
     * @param _wallet address of foundation wallet
     **/
    function setCefiWallet(address _wallet)
        external
        validation(_wallet)
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function setcoinbaseWallet(address _wallet)
        external
        validation(_wallet)
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
     ...
     * @param _nft address of LoanNFT contract
     **/
    function setNftContract(address _nft)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        NFT = ILoanNFT(_nft);
    ...
    function setDefiWallet(address _defiWallet)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        defiWallet = _defiWallet;
    ...
    function setSharePercent(uint256[3] calldata _sharePercentage)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        uint256 total;
    ...
    function setPazaContract(address _address)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        Paza = IPazaToken(_address);
    }
    ...
    function mintLoanNft(
        ...
        uint borrowerPazaAmount = Paza.getContinuousMintReward(_details.borrowerShare);
    ...
    function _prepaidPostpaid(LoanLib.Loan memory _details) internal {
        uint256 totalUSDC = (_details.borrowerShare + _details.foundationShare);
        uint256 totalPaza = Paza.getContinuousMintReward(totalUSDC);

        if (_details.isPrepaid) {
            //calculate paza and then transfer
            Paza.transferFrom(
                _details.bank,
                defiWallet,
            ...
            Paza.buy(totalUSDC, postpaidUsdcWallet, address(this));
            Paza.transfer(
                defiWallet,
                (totalPaza * defiSharePercent) / PCT_BASE
            );
    ...
    function prepaidPazaBuy(
        ...
        // uint
        if (usdc.balanceOf(prepaidUsdcWallet) >= _amount) {
            Paza.buy(_amount, prepaidUsdcWallet, _lender);
     ...
     * @param _bank address of bank
     **/
    function blackListBankPool(address _bank)
        public
        validation(_bank)
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
     ...
     **/
    function checkBankInfo(address _bank)
        external
        view
        returns (ControllerLib.bankInfo memory)
    {
        return banks[_bank];
    }

    function setMasterPool(address _newImplementation)
        public
        validation(_newImplementation)
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
...
import "./IBondingCurveV2.sol";
import "@openzeppelin/contracts-upgradeable/token/ERC20/IERC20Upgradeable.sol";


interface IPazaToken is IBondingCurveV2, IERC20Upgradeable {
    function buy(uint _usdcAmount, address _sender, address _receiver) external;
    function sell(uint _usdcAmount, address _sender, address _receiver) external;
    function reserveBalance() external view returns(uint);
    function taxCollected() external view returns (uint);
...
interface IPool {
    function initialize(address _owner, address _token, address _factory) external;
    function updateUserDetail(LoanLib.Loan memory _details, uint _loanId) external;
    function stake(address _user, uint256 _amount, uint256 _loanId) external;
...
library LoanLib {
   struct Loan {
        ...
        uint discount;
        uint borrowerShare;
        uint foundationShare;
        bool isRefinance;
        uint disbursementInterval;
        uint refinanceTokenID;
