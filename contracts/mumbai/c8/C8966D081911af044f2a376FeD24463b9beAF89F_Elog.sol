...
contract Elog is ERC721, Ownable {
  ...
  // 作者-tokenId 對應表
  mapping(address => uint256[]) private _posts;

  // tag-tokenId 對應表
  mapping(string => uint256[]) private _tags;
  // tags
  string[] private _alltags;

  // tokenId-Log 對應表
  mapping(uint256 => Log) private _logs;
  ...
  function post(
    ...
    _logs[tokenId] = Log(title, msg.sender, content, tags);
    for (uint256 i = 0; i < tags.length; i++) {
      if (_tags[tags[i]].length == 0) {
        _alltags.push(tags[i]);
      }
      _tags[tags[i]].push(tokenId);
    }
  ...
  // 取得Content
  function getPost(uint256 tokenId) public view returns (Log memory) {
