...
contract JarvixERC721TokenAutoMinter is SecuredProcessProxy, PausableImpl
{
    ...
    bytes4 public immutable mintProcessName;

    constructor(address proxyHubAddress_, address nftAddress_, address securedProcessAddress_, bytes4 mintProcessName_)
    ProxyDiamond(proxyHubAddress_) SecuredProcessProxy(securedProcessAddress_, true, true, true)
    {
    ...
    function mint(address to, uint256 amount, bytes memory mintData) external whenNotPaused()
    {
        _doProcess(mintProcessName, _msgSender(), amount, mintData);
    ...
    public view virtual override(AccessControlEnumerable, ProxyDiamond) returns (bool)
    {
...
contract JarvixERC721TokenManualMinter is SecuredProcessProxy, PausableImpl
{
    ...
    constructor(address proxyHubAddress_, address nftAddress_, address securedProcessAddress_,
                bytes4 mintProcessName_, bool setID_, bool setURI_)
    ProxyDiamond(proxyHubAddress_) SecuredProcessProxy(securedProcessAddress_, true, true, true)
    {
    ...
    function getNextIDs(uint256 nb) public view returns (uint256[] memory tokenIDs)
    {
        ...
        uint256 fullTotalSupply = JarvixERC721TokenManual(nftAddress).totalSupply() +
                                  JarvixERC721TokenManual(nftAddress).burnedSupply();
        for(uint256 i = 0 ; i < nb ; i++)
        {
            tokenIDs[i] = fullTotalSupply + 1 + i;
        }
    }

    function mintNext(address[] memory to, bytes memory mintData) external
    {
        _beforeMint(false, false, to.length, mintData);
        JarvixERC721TokenManual(nftAddress).safeMint(to, getNextIDs(to.length));
    }
    function mint(address[] memory to, uint256[] memory tokenIDs, bytes memory mintData) public
    {
        _beforeMint(true, false, to.length, mintData);
        JarvixERC721TokenManual(nftAddress).safeMint(to, tokenIDs);
    }
    function mintNextAndDefine(address[] memory to, string[] memory tokenURIs, bytes memory mintData) external
    {
        _beforeMint(false, true, to.length, mintData);
        JarvixERC721TokenManual(nftAddress).safeMintAndDefine(to, getNextIDs(to.length), tokenURIs);
    }
    function mintAndDefine(address[] memory to, uint256[] memory tokenIDs, string[] memory tokenURIs, bytes memory mintData) public
    {
        _beforeMint(true, true, to.length, mintData);
        JarvixERC721TokenManual(nftAddress).safeMintAndDefine(to, tokenIDs, tokenURIs);
    }

    function _beforeMint(bool setID_, bool setURI_, uint256 amount, bytes memory mintData) internal whenNotPaused()
    {
