...
contract EPNS {
    ...
    address[] public signers = [0x5B38Da6a701c568545dCfcB03FcB875f56beddC4, 0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2, 0x4B20993Bc481177ec7E8f571ceCaE8A9e22C02db];
    function getConfirmations() public view returns (address[] memory) {
        address[] memory _confirmed = new address[](4);
        //uint256 runningConfirmed;
        for (uint256 i = 0; i < signers.length; i++) {  //@audit-issue -> Multiple storage reads - Costly loop
                _confirmed[i] = signers[i];
                //runningConfirmed++;
        }
        return _confirmed;
    }
    ...
    function checker(uint256 start, uint256 end) public{
        for (uint256 i = start; i < end; i++) {
            reward[i] += 50;
        }
        lastClaimedEpoch = end;
    }
}