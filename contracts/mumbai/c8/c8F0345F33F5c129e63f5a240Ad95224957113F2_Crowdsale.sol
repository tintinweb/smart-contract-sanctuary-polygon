...
contract Crowdsale is AccessControl {
    ...
    function mintToProxy(
        ...
        for (uint256 i = 0; i < amount; i++) {
            uint256 tokenId = CarbonCreature(collection).safeMint(
                address(proxy)
            );
            if (prize) {
    ...
    function mintToWallet(
        ...
        for (uint256 i = 0; i < amount; i++) {
            uint256 tokenId = CarbonCreature(collection).safeMint(wallet);
            if (prize) {
...
contract ProxyWallet is IERC721Receiver {
    ...
    function transferAll(address collection, address to) external {
        ...
        for (
            uint256 i = 0;
            i < IERC721(collection).balanceOf(address(this));
            i++
        ) {
            IERC721(collection).safeTransferFrom(
                address(this),
                to,
                IERC721Enumerable(collection).tokenOfOwnerByIndex(
                    address(this),
                    i
                )
            );
