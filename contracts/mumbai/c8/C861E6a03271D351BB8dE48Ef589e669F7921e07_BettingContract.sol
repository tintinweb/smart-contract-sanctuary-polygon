...
contract BettingContract {
    ...
    function createLobby(uint256 tokenAmount) external {
        require(tokenAmount > 0, "Invalid bet amount");
        require(_amberToken.allowance(msg.sender, address(this)) >= tokenAmount, "Insufficient allowance");

        _amberToken.transferFrom(msg.sender, address(this), tokenAmount);
        _lobbyCount++;
        _lobbies[_lobbyCount] = Lobby(_lobbyCount, tokenAmount, msg.sender, address(0), false);
    }

    function placeBet(uint256 lobbyId, uint256 tokenAmount) external {
        ...
        _amberToken.transferFrom(msg.sender, address(this), tokenAmount);
        _lobbies[lobbyId].betAmount = _lobbies[lobbyId].betAmount.add(tokenAmount);
    ...
    function closeLobby(uint256 lobbyId) external {
        ...
        if (_lobbies[lobbyId].player2 == address(0)) {
            ...
            _amberToken.transfer(_lobbies[lobbyId].player1, _lobbies[lobbyId].betAmount);
        } else {
            // Calculate winner and distribute tokens
