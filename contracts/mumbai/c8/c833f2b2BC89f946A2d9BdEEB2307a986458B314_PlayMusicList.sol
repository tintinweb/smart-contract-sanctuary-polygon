...
contract PlayMusicList is Ownable {
    ...
    event Vote(address indexed user, uint256 indexed musicID, uint256 voteNumber);
    event Blackball(address indexed user, uint256 indexed musicID, uint256 voteNumber);
    event BackVote(address indexed user, uint256 indexed musicID, uint256 backNumber);
    event BackBlackball(address indexed user, uint256 indexed musicID, uint256 backNumber);

    event VoteReward(address indexed user, uint256 indexed musicID, uint256 rewardAmount, uint256 teamReward, uint256 addedReward);
    event BlackballReward(address indexed user, uint256 indexed musicID, uint256 rewardAmount, uint256 teamReward, uint256 addedReward);
    ...
    function _claimReward(uint256 musicIDBefore, address userAddress) internal {
        ...
        if (stakePeriod > 0 && userBefore.likeVoteNumber > 0) {
            ...
            if (musicRank > 0) {
                if (userBefore.stakeRank == 0) {
                    rankUp = lengthLimit.add(1).sub(musicRank);
            ...
            emit VoteReward(userAddress, musicIDBefore, rewardAmount, rewardAmount.mul(4).div(10), rewardAmount.div(10));
        }
        if (stakePeriod > 0 && userBefore.dislikeVoteNumber > 0) {
            ...
            if (userBefore.stakeRank > 0) {
                if (musicRank == 0) {
                    rankDown = lengthLimit.add(1).sub(userBefore.stakeRank);
                } else if (userBefore.stakeRank < musicRank) {
            ...
            emit BlackballReward(userAddress, musicIDBefore, rewardAmount, rewardAmount.mul(4).div(10), rewardAmount.div(10));
        }
    }
