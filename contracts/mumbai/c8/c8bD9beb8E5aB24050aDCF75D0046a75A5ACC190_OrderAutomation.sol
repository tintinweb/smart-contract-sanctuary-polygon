...
contract OrderAutomation is AutomationCompatibleInterface, OwnableUpgradeable, IOrderAutomation {
     ...
     */
    function addRequestToBatch(uint _orderId) external override onlyProspecting {
    ...
    function shouldPerformUpkeep() public view returns (bool) {
        if (
            !queue[first].pending &&
            last >= first &&
            (block.timestamp - queue[first].timestamp > interval || queue[first].orderIds.length >= batchMaxSize)
        ) {
            return true;
        }
        return false;
    ...
    function checkAutomate() external view 
    returns (bool canExec, bytes memory execPayload) {
        canExec = shouldPerformUpkeep();

        execPayload = abi.encodeWithSelector(this.performUpkeep.selector, "0x");
    }
    ...
    function getRandomNumberForOrder(uint _orderId) external view override returns (uint) {
