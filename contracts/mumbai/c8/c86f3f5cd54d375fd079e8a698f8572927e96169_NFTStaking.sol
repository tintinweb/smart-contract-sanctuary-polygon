...
contract NFTStaking is Ownable, ReentrancyGuard {
    ...
    mapping(address => uint8) public totalRarity;
    ...
    function stake(uint256[] calldata _tokenIds) external nonReentrant {
        ...
        for (uint256 i; i < len; ++i) {
            ...
            totalRarity[msg.sender] += IDsRarity[i] ;
        ...
        returns (uint256 _rewards)
    {
        ...
        if( endStakingTime < block.timestamp )
            return (
                ((endStakingTime - _stake.timestampLastUpdate) * _stake.amountStaked)
            * rewardsPerHour * (totalRarity[_staker] / _stake.amountStaked ) / 3600 );
        else
            return (
                ((block.timestamp - _stake.timestampLastUpdate) * _stake.amountStaked)
            * rewardsPerHour * (totalRarity[_staker] / _stake.amountStaked ) / 3600 );
