    ...
    Pausable
{
    ...
    function createTables(address registry) external payable whenNotPaused onlyRole(ADMIN) {
        _tableland = ITablelandTables(registry);
        _metadataTableId = _tableland.createTable(
            address(this),
            SQLHelpers.toCreateFromSchema("id int, thumb text, image text, animation_url text", _tablePrefix)
        );
        _metadataTable = string.concat(_tablePrefix, Strings.toString(_metadataTableId));

        _attributesTableId = _tableland.createTable(
            address(this), SQLHelpers.toCreateFromSchema("type text, value text, token_id text", _attributesPrefix)
        );
        _attributesTable = string.concat(_attributesPrefix, Strings.toString(_attributesTableId));
    }
        ...
        returns (uint256)
    {
        ...
        _tableland.runSQL(
            ...
            SQLHelpers.toInsert(
                _tablePrefix,
                _metadataTableId,
                "id, thumb, image, animation_url",
                string.concat(Strings.toString(tokenId), thumb, image, _animationBaseURI)
            )
        );
