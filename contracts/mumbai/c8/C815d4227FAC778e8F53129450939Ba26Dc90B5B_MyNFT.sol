...
contract MyNFT is ERC721LazyMint {
    ...
    address mainCollection = address(0);
    ...
    function setMain(address _addy) public onlyOwner {
        mainCollection = _addy;
    }

    function verifyClaim(address _claimer, uint256 _quantity) public view virtual override {
        uint[] memory _owned = NftInterface(mainCollection).tokensOfOwner(_claimer);
        uint _num = _owned.length;
        require(_num >= _quantity, "You don't have enough of the main collection.");
        uint v;
