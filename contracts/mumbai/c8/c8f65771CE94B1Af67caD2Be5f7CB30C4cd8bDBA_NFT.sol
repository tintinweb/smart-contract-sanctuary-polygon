...
contract NFT is ERC721, ERC721Enumerable, ERC2981, ERC721URIStorage, Ownable, INFT, Multicall {
	// string baseURI;
	///@notice add the given address as minters
	mapping(address => bool) public minters;

	///@notice add the given address as admins
	...
	///@notice to check whether the sender address is minter or not
	///@dev modifier to check whether the sender address is minter or not
	modifier onlyMinters() {
		require(minters[_msgSender()], 'caller is not the minter');
		_;
	}

	modifier onlyTokenOwnerorAdmin(uint256 _tokenId) {
		address owner = ownerOf(_tokenId);
		if (owner != msg.sender && !admins[msg.sender] && !minters[msg.sender]) revert('Not Authorized');
		_;
	}

	///@notice to check whether the sender address is admin
	///@dev modifier to check whether the sender address is admin
	modifier onlyAdmin() {
	...
	///@dev onlyMinters can mint the nft
	...
	///@dev onlyMinters can burn the token
	function burn(uint256 _tokenId) public override onlyTokenOwnerorAdmin(_tokenId) {
		_burn(_tokenId);
	}

	function checkNft(uint256 _tokenId) public override returns (bool exist) {
		return _exists(_tokenId);
	}

	///@notice add new address as minters
	///@param _minter address to approve as minter
	///@param _status status to set
	///@dev onlyAdmin can add new minters
	function manageMinters(address _minter, bool _status) public override onlyAdmin {
		require(_minter != address(0x0), '0x0 minter');
		minters[_minter] = _status;
	}

	// function changeBaseURI(string memory baseURI_) external onlyMinters {
	// 	baseURI = baseURI_;
	// }
...
interface INFT {
	...
	function royaltyInfo(uint256 _tokenId, uint256 _salePrice) external view returns (address reciever, uint256 _rate);
