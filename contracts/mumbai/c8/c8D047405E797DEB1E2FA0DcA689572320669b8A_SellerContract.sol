...
contract SellerContract is Ownable {

  ExchangeToken exchangeToken;  // Our Token Contract
  ...
  constructor(address tokenAddress) {
    exchangeToken = ExchangeToken(tokenAddress);
  }
  ...
  function buyTokens() public payable returns (uint256 tokenAmount) {
    ...
    uint256 SellerBalance = exchangeToken.balanceOf(address(this)); // check if the Vendor Contract has enough amount of tokens for the transaction
    require(SellerBalance >= amountToBuy, "Vendor contract has not enough tokens in its balance");
    (bool sent) = exchangeToken.transfer(msg.sender, amountToBuy); // Transfer token to the Msg.Sender
...
contract ExchangeToken is ERC20, Ownable  {
    constructor() ERC20("ExchangeToken", "EXT") {
