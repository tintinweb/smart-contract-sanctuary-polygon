...
contract NftDuel is VRFConsumerBaseV2, ERC721Holder, Ownable {
    ...
    uint32 public clCallbackGasLimit = 2500000;
    ...
    function getGamesBatch(address projectAddress, uint256 startIndex, uint256 batchSize) public view returns (Game[] memory) {
        require(startIndex < gameCountPerProject[projectAddress], "WRONG_START_INDEX");
        require(batchSize > 0, "WRONG_BATCH_SIZE");
    ...
    function createGame(address nftContractAddress, uint256 nftId, uint256 nftPower, uint256 requiredFeeInWei, bytes32[] calldata feesMerkleProof, bytes32[] calldata nftPowerMerkleProof) public payable {
        require(isContractEnabled, "GAME_DISABLED");
        requireFee(feesMerkleProof, nftContractAddress, requiredFeeInWei, joinFeesMerkleRoot);
        require(verifyNftPower(nftPowerMerkleProof, nftId, nftPower, nftPowerMerkleRoots[nftContractAddress]), "POWER_PROOF_INVALID");
    ...
    function joinGame(address nftContractAddress, uint256 gameId, uint256 nftId, uint256 nftPower, uint256 requiredFeeInWei, bytes32[] calldata feesMerkleProof, bytes32[] calldata nftPowerMerkleProof) public payable {
        require(isContractEnabled, "GAME_DISABLED");
        require(projectGames[nftContractAddress][gameId].status == GameStatus.WaitingForPlayers, "GAME_INACTIVE");
        requireFee(feesMerkleProof, nftContractAddress, requiredFeeInWei, joinFeesMerkleRoot);
        require(verifyNftPower(nftPowerMerkleProof, nftId, nftPower, nftPowerMerkleRoots[nftContractAddress]), "POWER_PROOF_INVALID");
    ...
    function leaveGame(address nftContractAddress, uint256 gameId) public {
        require(projectGames[nftContractAddress][gameId].status == GameStatus.WaitingForPlayers, "GAME_INACTIVE");
        require(projectGames[nftContractAddress][gameId].player1.playerAddress == msg.sender, "NOT_A_PLAYER");
    ...
    function claimPrize(address nftContractAddress, uint256 gameId, uint256 requiredFeeInWei, bytes32[] calldata merkleProof) public payable {
        require(projectGames[nftContractAddress][gameId].status == GameStatus.Completed, "GAME_NOT_COMPLETED");
        require(projectGames[nftContractAddress][gameId].winner.playerAddress == msg.sender, "NOT_A_WINNER");
    ...
    function requireNftApprovalAndOwnership(IERC721 nftContract, uint256 nftId) private view {
        require(nftContract.ownerOf(nftId) == msg.sender, "NOT_NFT_OWNER");
        require(nftContract.isApprovedForAll(msg.sender, address(this)), "NO_NFT_APPROVAL");
    }
    ...
    function requireFee(bytes32[] calldata merkleProof, address nftContractAddress, uint256 requiredFeeInWei, bytes32 merkleRoot) private view {
        ...
        require(msg.value >= requiredFeeInWei, "INCORRECT_FEE");
    }
    ...
    function retryRandomWordsRequest(address nftContractAddress, uint256 gameId) external onlyOwner {
        require(projectGames[nftContractAddress][gameId].status == GameStatus.InProgress, "WRONG_STATUS");
        requestRandomWords(nftContractAddress, gameId);
    }
