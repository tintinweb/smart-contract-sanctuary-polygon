...
contract ProgressNFT is ERC721, IProgressNFT {

    mapping (uint => uint) public tokenIdToLevel;
    address public owner;    
    address public progmon;
    ...
    modifier onlyProgmon () {
        require(msg.sender == progmon, "Only Progmon contract can call this method!!!");
        _;
    }

    constructor() ERC721 ("Progmon-ProgressNFT", "ProgNFT") {
        owner = msg.sender;
    }

    function setProgmon (address _progmon) public returns (bool success) {
        progmon = _progmon;
        success = true;
    }

    function mint (address player) public onlyProgmon returns (uint tokenId) {
    ...
    function levelUp (uint tokenId) public onlyProgmon returns (bool success) {
