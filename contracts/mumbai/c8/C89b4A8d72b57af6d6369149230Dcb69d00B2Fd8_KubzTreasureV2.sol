    ...
    ReentrancyGuardUpgradeable
{
    ...
    EnumerableSet.UintSet claimedKubz;
    ...
    function kubzClaim(
        ...
        require(!claimedKubz.contains(kubzTokenId), "Already claimed");
        require(
    ...
    function getClaimedKubzLength() external view returns (uint256) {
        return claimedKubz.length();
    }

    function getClaimedKubz(uint256 fromIdx, uint256 toIdx)
        external
        view
        returns (uint256[] memory)
    {
        toIdx = Math.min(toIdx, claimedKubz.length());
        uint256[] memory part = new uint256[](toIdx - fromIdx);
        for (uint256 i = 0; i < toIdx - fromIdx; i++) {
            part[i] = claimedKubz.at(i + fromIdx);
        }
        return part;
    }
    
    function getIfKubzClaimed(uint256[] calldata kubzTokenIds)
        external
        view
        returns (bool[] memory)
    {
        bool[] memory part = new bool[](kubzTokenIds.length);
        for (uint256 i = 0; i < kubzTokenIds.length; i++) {
            uint256 kubzTokenId = kubzTokenIds[i];
            part[i] = claimedKubz.contains(kubzTokenId);
        }
        return part;
    }

    function getClaimedKubzAll() external view returns (uint256[] memory) {
        return claimedKubz.values();
    }
