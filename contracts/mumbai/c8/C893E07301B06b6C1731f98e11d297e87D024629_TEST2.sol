...
contract TEST2 is IBEP20, Auth {
    ...
    uint256 totalFee = liquidityFee + buybackFee + reflectionFee + NFTholdersFee + marketingdevelopmentFee + burnFee + extraFeeOnSell;
    ...
    function setFees(uint256 _liquidityFee, uint256 _buybackFee, uint256 _reflectionFee, uint256 _NFTholdersFee, uint256 _marketingdevelopmentFee, uint256 _burnFee, uint256 _extraFeeOnSell, uint256 _feeDenominator) external authorized {
        ...
        totalFee = _liquidityFee.add(_reflectionFee).add(_NFTholdersFee).add(_marketingdevelopmentFee).add(_extraFeeOnSell);
