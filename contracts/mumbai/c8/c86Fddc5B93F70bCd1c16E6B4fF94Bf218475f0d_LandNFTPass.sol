...
contract LandNFTPass is ERC721Enumerable, Ownable {
	...
	constructor(IPassport _passport, IERC20 _matic) ERC721("Actocracy Land NFT Pass", "LandNFTPassMint") {
		pos = 0;
	...
	function mint(uint256 nftAmount) external payable {
        ...
        IPassport.infoData memory temp = passport._totalInfo(passportID);
        if(temp.level == 1) {
            mintableAmount = (1 + temp.referralCount) - _mintAmount[msg.sender];
        } else {
            mintableAmount = (2 + temp.referralCount) - _mintAmount[msg.sender];
        }
        ...
        if(keccak256(abi.encodePacked(temp.referer)) != keccak256(abi.encodePacked("no"))) {
            uint256 fpassportID = passport._nftIdByReferral(temp.referer);
            IPassport.infoData memory firstLvl = passport._totalInfo(fpassportID);
            require(firstLvl.ownerAddr != address(0), "No referer");
            payable(firstLvl.ownerAddr).transfer((msg.value * 7) / 100);
            feeAmount = feeAmount + ((msg.value * 7) / 100);
            if(keccak256(abi.encodePacked(firstLvl.referer)) != keccak256(abi.encodePacked("no"))) {
                uint256 spassportID = passport._nftIdByReferral(firstLvl.referer);
                IPassport.infoData memory secondLvl = passport._totalInfo(spassportID);
                require(secondLvl.ownerAddr != address(0), "No referer");
                payable(secondLvl.ownerAddr).transfer((msg.value * 5) / 100);
                feeAmount = feeAmount + ((msg.value * 5) / 100);
                if(keccak256(abi.encodePacked(firstLvl.referer)) != keccak256(abi.encodePacked("no"))) {
                    uint256 tpassportID = passport._nftIdByReferral(secondLvl.referer);
                    IPassport.infoData memory thirdLvl = passport._totalInfo(tpassportID);
                    require(thirdLvl.ownerAddr != address(0), "No referer");
                    payable(thirdLvl.ownerAddr).transfer((msg.value * 3) / 100);
