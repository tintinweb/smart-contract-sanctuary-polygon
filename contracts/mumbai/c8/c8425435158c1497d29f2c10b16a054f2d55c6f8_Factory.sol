...
contract Factory  {
     ...
     * @dev Create FT Token
     */
    function createFT(string memory name, string memory symbol, uint8 decimals, uint256 cap, address owner) external returns (address token) {
        // constructor(string memory name_, string memory symbol_, uint8 decimals_, uint256 cap_, address owner_) ERC20(name_, symbol_) {
        // address token = address(new ERC20Template(name, symbol, decimals, cap, owner));
        // userTokens[msg.sender].push(token);        
        // totalFT = totalFT + 1;

        bytes memory bytecode = type(ERC20Template).creationCode;
        bytes memory createcode = abi.encodePacked(bytecode, abi.encode(name, symbol, decimals, cap, owner));
        bytes32 salt = keccak256(abi.encodePacked(totalFT));
        assembly {
            token := create2(0, add(createcode, 32), mload(createcode), salt)
        }

        emit Deployed(msg.sender, token);
