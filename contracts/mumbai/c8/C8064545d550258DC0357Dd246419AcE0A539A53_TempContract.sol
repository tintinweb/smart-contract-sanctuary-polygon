...
contract TempContract is FunctionsClient, ERC20 {
  ...
  modifier onlyRelayerOrOwner { // modifier for checking only that function is called by only owner
  ...
  function executeRequest(
    ...
    uint64 subscriptionId,
    uint32 gasLimit
  ) public onlyRelayerOrOwner returns (bytes32) {
    Functions.Request memory req;
