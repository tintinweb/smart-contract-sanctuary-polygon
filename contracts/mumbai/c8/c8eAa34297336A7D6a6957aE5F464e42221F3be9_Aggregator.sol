...
//        uint256 dexType; // 1:erc20  2:erc721 3:erc1155
    ...
    function swapTokenToNft(SwapNftOrder memory order, bytes memory _sign) external nonReentrant {
        ...
        uint256 _toTokenInputAmt =0;
        if(order._dex== address(0)){
            uint256 feeAmount = _inputAmount.mul(order.fee).div(1000000);
            TransferHelper.safeApprove(
                order.fromToken,
                order._dex,
            ...
            (bool success, ) = order._dex.call(order._data);
            require(success, "DEX_SWAP_FAILED");
             _toTokenInputAmt = IERC20(order.toToken)
            .balanceOf(address(this))
            .sub(_toTokenBalanceOrigin);
            require(
                _toTokenInputAmt >= order.minReturnAmount,
    ...
    function callDexListTest(address _dex,uint256 fromAmount, bytes memory _data ) external payable nonReentrant {
        (bool success, ) = _dex.call{value:fromAmount}(_data);
        require(success, "DEX_SWAP_FAILED");
    }
}
