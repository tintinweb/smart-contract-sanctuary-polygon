...
contract DeLottery
{
    ...
    event WINNER (address indexed winner,uint indexed prize);
    function winner () public
    {
        ...
        emit WINNER (players[index],(address(this).balance*90/100));
        players[index].transfer(address(this).balance*90/100); // The owner get 10% as gas 
    ...
    function winner_name (uint password) public view returns (string memory winner_)
    {
        if (password==12345)
        {
            return USERS.Name;
    ...
    function reset_time (uint minute) public 
    {
        require (msg.sender==owner, "only owner can reset the time");
        require (starttime+(TIMES.Ticket_Time*60)<block.timestamp,"Not yet");
        starttime=block.timestamp;
        TIMES.Ticket_Time=minute;
    } 
}