...
contract ZaiFighting is Ownable, Pausable {
    ...
    function updateInterfaces() external onlyOwner{
        ...
        payment = IPayments(gameAddresses.getPaymentsAddress());
    ...
    function useRestPotion(uint256 _zaiId, uint256 _potionId) external returns(uint256){        
        require(
            IZai.ownerOf(_zaiId) == msg.sender ||
            IDel.gotDelegationForZai(_zaiId,msg.sender)
            , "Not your zai");
    ...
    function initFighting(
        ...
        whenNotPaused
        returns(
            uint256[30] memory result //[0: zaiId,1:myScore, 2:challengerScore, 3-11: ElementByRoundOfChallenger, 12-20: PowerUseByChallengerByRound, 21 number of potions used by challenger, 22-23-24 type of potions if relevent, 25-26-27 power of potions if relevent, 28 xpWon, 29 BZAI won ]
            ){
        ...
        require((ownerOfZai == msg.sender && isDelegated) == false, "You delegated your Zai");
        ...
        require(IRank.updatePlayerRankings(msg.sender),"Ranking error");
        ...
        _toReturn = IFightLib.getNewPattern(_randoms,c,_toReturn);

        // update the fighting progress in the return array
        _toReturn = IFightLib.updateFightingProgress( _toReturn,_elements,_powers);

        // update all counters 
        _toReturn[28] = _getXpToWin(_usedPotions,z,_powers);
        _toReturn = _updateCounterWinLoss(z,ownerOfZai,_zaiId,_toReturn,isDelegated);
    ...
    function _getXpToWin(
        ...
        uint256[5] memory powers = _getZaiPowersByElement(z,_usedPotions);
        ...
        for(uint256 i = 0; i < 3;){
            if(_usedPotions[i] !=0){
                ...
                _xp += (p.powers.xp * xpRewardByFight);
            }
            unchecked{ ++i; }
        }
        return _xp;
    ...
    function _updateCounterWinLoss(
        ...
        require(IStats(gameAddresses.getZaiStatsAddress()).updateCounterWinLoss(_zaiId,_toReturn[0],_toReturn), "Stat issue");
        ...
        if(_toReturn[1] < _toReturn[2]){
            ...
            //Player win 
            if(_lastWinTimestamp[_zaiId] < IRanking(gameAddresses.getRankingContract()).getDayBegining()){
            ...
            if(_dayWinCounter[_zaiId] < bzaiRewardCountPerDay[z.metadata.state]){
                _bzaiWon = IRewardsWinningFound(gameAddresses.getWinRewardsAddress()).getWinningRewards(z.level);
                if(isDelegated){
                    ZaiStruct.DelegateData memory d = IDel.getDelegateDatasByZai(_zaiId);
                    require(IDel.updateLastScholarPlayed(_zaiId));
                    _paySchoolarAndOwner(d.scholarAddress, ownerOfZai, d.percentageForScholar, _bzaiWon);
                    _bzaiWon = _bzaiWon * d.percentageForScholar / 100;
                }else{
    ...
    function _getZaiPowersByElement(ZaiStruct.Zai memory z,uint256[3] memory _potions) internal view returns(uint256[5] memory){
