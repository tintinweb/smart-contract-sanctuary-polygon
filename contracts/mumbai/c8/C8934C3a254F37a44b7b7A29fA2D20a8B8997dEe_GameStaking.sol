...
contract GameStaking is Ownable, ReentrancyGuard {
    ...
    event TiketPriceSet(uint256 newtiketPrice);
    ...
    function stake() external payable nonReentrant {
        ...
        if (tiketPrice > 0) rewardToken.safeTransferFrom(msg.sender, address(this), tiketPrice);
    }
    ...
    function setTiketPrice(uint256 newTiketPrice) external onlyOwner returns (bool) {
        ...
        emit TiketPriceSet(newTiketPrice);
        return true;
    }
        ...
        returns (bool)
    {
        ...
        for (uint256 i = 0; i < playersAddreses.length; i++) {
    ...
    function calculateReverds(address playerAddress) internal view returns (uint256 rewards) {
        ...
        rewards = ((((player.deposit * rate) / ONE_HUNDRED_PERCENT) * (lastTimestamp - calimTimestamp)) /
            SECONDS_IN_A_DAY);
    }
    ...
    // !!! SAVE REWARD EVERYTIME when change Multiplier
