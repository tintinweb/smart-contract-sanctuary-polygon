...
import {MetaRewardPool} from "../MetaRewardPool.sol";
...
contract TwoPeriodsRewardCalculator is IRewardCalculator, AccessControl {
    ...
    function runCampaign(uint256 reward, uint256 duration) external {
        require(hasRole(REWARD_DISTRIBUTION, _msgSender()), "not reward distribution");
        require(((MetaRewardPool(rewardPool).getRewardsAvailable()) >= reward), "not enough rewards available");
...
contract MetaRewardPool is StakeTokenWrapper, AccessControl, ReentrancyGuard, ERC2771Handler {
