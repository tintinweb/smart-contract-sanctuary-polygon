    ...
    ReentrancyGuardUpgradeable
{
    ...
    modifier horseLimitStaking() {
        ...
        require(
            starBalance[tx.origin].div(1e18).add(freeSlot) >
                getUserTotalHorse(tx.origin),
            "Slot not enough."
        );
        _;
    }
    ...
    modifier plotLimitStaking(uint256 _tokenId) {
        require(
            getPlotAvailable(tx.origin) >= (facility.size(_tokenId)),
            "Plot not enough."
        );
        _;
    }
    ...
    modifier slotLimitStakeing(uint256 _slot) {
        uint256 usedSlot = userInfoSlots[_slot].totalHorse.add(
            userInfoSlots[_slot].stables.length.mul(2)
        );
        require(usedSlot <= maxHorseStaking, "Slot is Full.");
        _;
    }

    modifier ownedSlot(uint256 _slot) {
        require(slotOwner[_slot] == tx.origin, "You not owned this slot.");
        _;
    }

    modifier canUseThisSlot(uint256 _slot) {
        require(
            slotOwner[_slot] == tx.origin || slotOwner[_slot] == address(0),
            "Slot not empty."
        );
        _;
    }
    ...
    function setSpeedPerBlock(uint256 _amount) external onlyOwner {
        ...
        emit SetSpeedPerBlock(tx.origin, _amount);
    }

    function depositStar(uint256 _amount) external {
        require(
            star.allowance(tx.origin, address(this)) >= _amount,
            "credit not enougth"
        );

        star.transferFrom(tx.origin, address(this), _amount);
        starBalance[tx.origin] = starBalance[tx.origin].add(_amount);

        emit DepositStar(
            tx.origin,
            _amount,
            userInfo[tx.origin].totalHorse,
            freeSlot.add(starBalance[tx.origin])
        );
    }

    function withdrawStar(uint256 _amount) external {
        require(starBalance[tx.origin] >= _amount, "withdraw over balance.");
        if (userInfo[tx.origin].totalHorse >= freeSlot) {
            require(
                userInfo[tx.origin].totalHorse.sub(freeSlot) <=
                    starBalance[tx.origin].sub(_amount).div(1e18),
                "withdraw horse before unstake."
            );
        }

        starBalance[tx.origin] = starBalance[tx.origin].sub(_amount);
        star.transfer(tx.origin, _amount);
    ...
    function getUserDepositStable(
        uint256 _slot
    ) external view returns (Stable[] memory) {
        return userInfoSlots[_slot].stables;
    }

    function getUserDepositHorse(
        uint256 _slot
    ) external view returns (Horse[] memory) {
        return userInfoSlots[_slot].horses;
    }

    function getUserDepositHorseInStable(
        uint256 _slot,
        uint256 _stableId
    ) external view returns (Horse[] memory) {
    ...
    function claimNativeToken() external onlyOwner {
        ...
        (bool sent, ) = tx.origin.call{value: nativeBalance}("");
        require(sent, "Failed to send Native Token");
        emit ClaimNativeToken(tx.origin, nativeBalance);
    }
    ...
    function updatePoolValue(uint256 _amount) public{
    ...
    function payReward(UserInfo storage _user) internal {
        ...
        if (pending > 0) {
            ...
            if (_user.amount > newPopularity) {
                ...
                distributeBonus(tx.origin);
                // updateUserRanchBonus(_slot);

                poolInfo.totalStake = poolInfo.totalStake.add(_user.bonus);
            }
            safeSpeedTransfer(tx.origin, normalizeReward);
        }
    }

    function calculateReward(
        Horse[] memory _horses,
    ...
    function getHorseInStable(
        uint256 _slot,
        uint256 _stableId
    ) external view returns (uint256[2][2] memory horses) {
        UserInfo storage user = userInfoSlots[_slot];
        for (uint256 index = 0; index < user.stables.length; index++) {
    ...
    function getPopularityInStable(
        uint256 _stableId,
        uint256 _slot
    ) public view returns (uint256) {
        UserInfo storage user = userInfoSlots[_slot];
    ...
    function depositFacility(
        uint256 _tokenId,
        uint256 _slot
    ) external plotLimitStaking(_tokenId) poolIsEnable canUseThisSlot(_slot) {
        updateUserSlotIndex(tx.origin, _slot);
        UserInfo storage user = userInfoSlots[_slot];
        ...
        facility.transferFrom(tx.origin, address(this), _tokenId);
        ...
        userPlots[tx.origin] = userPlots[tx.origin].add(
        ...
        user.rewardDebt = user
            ...
            .add(user.bonus)
            .mul(poolInfo.accSpeedPerShare)
            .div(1e12); //pendingSpeed(tx.origin);
        ...
        emit DepositFacility(
            ...
            popularity,
            userPlots[tx.origin],
            getPlotAvailable(tx.origin),
            _slot
        );
    }

    function withdrawFacility(
        uint256 _tokenId,
        uint256 _slot
    ) external nonReentrant ownedSlot(_slot) {
        PoolInfo storage pool = poolInfo;
        UserInfo storage user = userInfoSlots[_slot];
        ...
        facility.transferFrom(address(this), address(tx.origin), _tokenId);
        ...
        user.rewardDebt = user
            ...
            .mul(poolInfo.accSpeedPerShare)
            .div(1e12); //pendingSpeed(tx.origin);
        userPlots[tx.origin] = userPlots[tx.origin].sub(
            facility.size(_tokenId)
        );
    ...
    function depositStable(
        uint256 _tokenId
    ) external plotLimitStaking(_tokenId) poolIsEnable {
        uint256 userActiveSlot = getUserActiveSlot(tx.origin);
        updateUserSlotIndex(tx.origin, userActiveSlot);
        ...
        facility.transferFrom(address(tx.origin), address(this), _tokenId);
    ...
    function withdrawStable(
        ...
        require(user.ownedStable[_tokenId], "WithdrawStable:No stable staking");
        ...
        facility.transferFrom(address(this), tx.origin, _tokenId);
        ...
        slotLimitStakeing(_slot)
    {
        ...
        horse.transferFrom(address(tx.origin), address(this), _horseTokenId);
        // update info owner
    ...
    function withdrawHorseInStable(
        ...
        pool.totalStake = pool
            ...
            .add(user.bonus);
        horse.transferFrom(address(this), tx.origin, _horseTokenId);
    ...
    function getUserActiveSlot(
        address _user
    ) public view returns (uint256 _slot) {
    ...
    function depositHorse(
        uint256 _tokenId
    ) external horseLimitStaking poolIsEnable {
        require(
            horse.isApprovedForAll(tx.origin, address(this)),
            "Please set approval"
        );
        uint256 userActiveSlot = getUserActiveSlot(tx.origin);
        updateUserSlotIndex(tx.origin, userActiveSlot);
        ...
        horse.transferFrom(address(tx.origin), address(this), _tokenId);
    ...
    //     horse.transferFrom(address(this), address(tx.origin), _tokenId);
    //     user.rewardDebt = user
    //         .amount
    //         .add(user.bonus)
    //         .mul(poolInfo.accSpeedPerShare)
    ...
    function withdrawHorse(
        uint256 _tokenId,
        uint256 _slot
    ) external ownedSlot(_slot) {
        PoolInfo storage pool = poolInfo;
        UserInfo storage user = userInfoSlots[_slot];
    ...
    function adminWithDrawNFT(
        ...
        IERC721Upgradeable(_address).transferFrom(
            address(this),
            _receiver,
            _tokenId
        );
