...
// 
    function claimAdvisorAndAuditorMonthlyRemuneration() external {
        require(block.timestamp > deployTime + counterForAdv*3 minutes,'Salary not unlocked for the next month');
    ...
    function claimDevsAndOwnerMonthlyRemuneration() external {
        require(block.timestamp >deployTime + counterForDevsOwner*3 minutes,'Salary not unlocked for the next month');
