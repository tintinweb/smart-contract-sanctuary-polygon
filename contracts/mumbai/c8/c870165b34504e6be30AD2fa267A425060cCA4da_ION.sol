...
contract ION is ERC20, Ownable {
    ...
    uint256 public coolDownPeriod = 1500; //change before deployment
    ...
    uint256 public maxLimit = 5000000 * 10**18; //check before deployment
    ...
    function _checkSellLimit(address user, uint256 transactAmount) internal {
        if (userLimits[user].startTime + 600 < block.timestamp) {
            //change this to 24 hours before deployment
