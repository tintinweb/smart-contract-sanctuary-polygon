...
contract CryptoRunnerState is Initializable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    event RunStarted(address indexed sender, uint256 indexed tokenId, uint256 indexed runId, CryptoRunner cryptoRunner);
    event RunEnded(
    ...
    function _startRunArgs(bytes memory data_) internal returns (uint256[] memory) {
        (bytes memory decodedBytes, uint256[] memory args) = abi.decode(data_, (bytes, uint256[]));
        _isValidSignature(decodedBytes, abi.encodePacked(args));
    ...
    function _endRunArgs(bytes memory data_) internal returns (uint256[] memory, address) {
        (bytes memory decodedBytes, uint256[] memory args, address to) = abi.decode(data_, (bytes, uint256[], address));
        _isValidSignature(decodedBytes, abi.encodePacked(args, to));
    ...
    /** @dev Types: ['bytes','uint256[]','address']
             uint256[] order: tokenId, runId, notorietyPoints, dataReward, nonce */
    /// @param data_ abi encoded data
    function endRun(bytes memory data_) external {
        ...
        cryptoRunner.notorietyPoints += runArgs[3];
        cryptoRunners[runArgs[0]] = cryptoRunner;
        _transferData(to, runArgs[4]);
        emit RunEnded(to, runArgs[0], runArgs[1], cryptoRunner, runArgs[4]);
    }
    ...
    function getRunningTokenIds(uint256[] memory tokenIds_) public view returns (uint256[] memory) {
    ...
    function getRunner(uint256 tokenId_) public view returns (CryptoRunner memory) {
        return cryptoRunners[tokenId_];
    }

    function getClock(uint256 tokenId_) public view returns (uint256) {
        CryptoRunner memory cryptoRunner = cryptoRunners[tokenId_];
        return (((block.timestamp - cryptoRunner.lastRunTimestamp) / 60) * clockRegen) + cryptoRunner.remainingClock;
    }
    ...
    /** @dev Types: ['bytes','uint256[]']
             uint256[] order: tokenId, runId, requiredClock, nonce */
    /// @param data_ abi encoded data
    function startRun(bytes memory data_) external returns (CryptoRunner memory) {
        ...
        emit RunStarted(msg.sender, runArgs[0], runArgs[1], cryptoRunner);
        return cryptoRunner;
    }
