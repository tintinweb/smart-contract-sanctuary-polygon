...
contract WavePortal is Ownable {
    ...
    function setPrize(uint256 _newPrize) public {
        _fundByUser[msg.sender].prize = _newPrize;
    }

    function getPrize() public view returns (uint256) {
        return _fundByUser[msg.sender].prize;
    }

    function fund() public payable {
        require(_fundByUser[msg.sender].prize > 0, 'You need to specify the prize first!');
        _fundByUser[msg.sender].total.add(msg.value);
    }
    ...
    function withdraw() public {
        ...
        _fundByUser[msg.sender].total.sub(_fundByUser[msg.sender].total);
    }
    ...
    function wave(string memory _message, address _to) public {
        ...
        if (_fundByUser[_to].prize > 0) {
            require(
                (
                    _fundByUser[_to].prize <= address(this).balance
                    && _fundByUser[_to].prize <= _fundByUser[_to].total
                ),
                'Insufficient funds for withdrawing ETH after Wave'
            );
            ( bool success, ) = msg.sender.call{ value: _fundByUser[_to].prize }("");
            require(success, 'Failed to withdraw ETH though the funds are sufficient');
            _fundByUser[msg.sender].total.sub(_fundByUser[msg.sender].prize);
        }
