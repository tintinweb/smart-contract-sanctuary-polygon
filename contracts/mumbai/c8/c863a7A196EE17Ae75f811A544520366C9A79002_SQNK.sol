...
contract SQNK is IERC20, Ownable {
    ...
    string private constant _symbol = "SQNK1"; //Token Symbol
    ...
    constructor(address _marketingAddress, address _rewardAddress) {
        ...
        isExcludedFromMaxTx[tokenPair] = true;
        ...
        isExcludedFromMaxWallet[tokenPair] = true;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (isExcludedFromFee[sender] || isExcludedFromFee[recipient]) {
            ...
            if (sender == tokenPair) {
                ...
                emit SenderCheck(sender, recipient, tokenPair);
            } else if (recipient == tokenPair) {
                totalTax = sellTax;
    ...
    function takeFee(address sender, uint256 amount, uint256 marketingTax, uint256 liquidityTax, uint256 rewardTax) private {
        ...
        _balances[tokenPair] = _balances[tokenPair].add(liquidityFee);

        emit Transfer(sender, marketingAddress, marketingFee);
        emit Transfer(sender, rewardAddress, rewardFee);
        emit Transfer(sender, tokenPair, liquidityFee);
    }
    ...
    function setTokenPair(address _tokenPair) public onlyOwner {
        require(_tokenPair != address(0), "Token Pair Address is the zero address");
        tokenPair = _tokenPair;
    }

    function setMarketingAddress(address _marketingAddress) public onlyOwner {
        require(_marketingAddress != address(0), "Marketing wallet is the zero address");
        marketingAddress = _marketingAddress;
    }

    function setRewardAddress(address _rewardAddress) public onlyOwner {
        require(_rewardAddress != address(0), "Reward wallet is the zero address");
        rewardAddress = _rewardAddress;
    }

    function setSqnetAddress(address _sqnetAddress) public onlyOwner {
        require(_sqnetAddress != address(0), "SQNET Address is the zero address");
    ...
    function setBuyRewardTax(uint256 _buyRewardTax) public onlyOwner {
        require(_buyRewardTax <= buyTax, "Buy reward tax cannot be more than buy Tax");
        buyRewardTax = _buyRewardTax;
    }

    function setBuyLiquidityTax(uint256 _buyLiquidityTax) public onlyOwner {
        require(_buyLiquidityTax <= buyTax, "Buy liquidity tax cannot be more than buy Tax");
        buyLiquidityTax = _buyLiquidityTax;
    }

    function setBuyMarketingTax(uint256 _buyMarketingTax) public onlyOwner {
        require(_buyMarketingTax <= buyTax, "Buy marketing tax cannot be more than buy Tax");
    ...
    function setSellRewardTax(uint256 _sellRewardTax) public onlyOwner {
        require(_sellRewardTax <= sellTax, "Sell reward tax cannot be more than sell Tax");
        sellRewardTax = _sellRewardTax;
    }

    function setSellLiquidityTax(uint256 _sellLiquidityTax) public onlyOwner {
        require(_sellLiquidityTax <= sellTax, "Sell liquidity tax cannot be more than sell Tax");
        sellLiquidityTax = _sellLiquidityTax;
    }

    function setSellMarketingTax(uint256 _sellMarketingTax) public onlyOwner {
        require(_sellMarketingTax <= sellTax, "Sell marketing tax cannot be more than sell Tax");
