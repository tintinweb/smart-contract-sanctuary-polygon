...
contract BabyBearsMumbai is ERC721, Ownable, VRFConsumerBase {
    ...
    uint public monthlyTributeFee = 0.05 ether;
     ...
     */
    constructor(address _honeyAddress, address _charity, address _rewardPool, address _VRFCoordinator, address _linkToken, bytes32 _keyhash) 
        VRFConsumerBase(
            _VRFCoordinator, // VRF Coordinator
            _linkToken  // LINK Token
        )
        ERC721("Baby Bears Mumbai", "BBMumbai")
    {
        keyHash = _keyhash;
    ...
    */
    function publicMint(uint _amount) external {
        ...
        require(LINK.balanceOf(address(this)) >= fee * _amount, "Not enough LINK");
        
        uint price = Price * _amount;
    ...
    */
    ...
    function mintInternal(uint _index, uint _range, address _recipient) internal  {
        ...
        //Set the token id as the value plus the tokenId addition, in case more supple is added
        uint tokenId = value + tokenIdAddition;
        ...
        //Check if the index chosen isn't the last index in the range
        if(_index < _range) {
    ...
    */
    ...
    function payMultipleMaintenanceFees(uint[] calldata _ids, uint[] calldata _numMonths) external payable {
        ...
        for(uint i = 0; i < _ids.length; i++) {
            ...
            totalFee += numberMonths * feePerMonth;
        ...
        require(totalFee == msg.value, "incorrect fee");
    ...
    */
    function internalGetTokenEmissionRate(uint _tokenId, EmissionRate memory _emissionRate) internal pure returns(uint) {
    ...
    * Requires the sender to be the owner of this address
    */
    function setEmissionRates(uint[] calldata _ranges, uint[] calldata _rates) onlyOwner external {

        require(_ranges.length == _rates.length, "rates and ranges should be the same length");

        emissionRates.push(EmissionRate(_rates, _ranges, block.timestamp));
    ...
    */
    ...
    /**
    * @dev Airdrops mints to community using chainlink to generate a randomnumber an mint the nft
    * Requires the sender to be the owner of this address
    */
    function AirdropMint(uint[] calldata _amount, address[] calldata _reciepient) external onlyOwner {

        require(isPublicMint == false, "Cannot call when minting is public");

        require(_amount.length == _reciepient.length, "Arrays need to be the same size");
        ...
        for(uint i = 0; i < _amount.length; i++) {
            ...
            for(uint j = 0; j < amount; j++) {
                ...
                VRFRequests[getRandomNumber()] = _reciepient[i];
    ...
    */
    ...
    /**
    * @dev Increase max supply by 10k, only if all previous have been minted and the community agrees to it
    */
    function increaseMaxSupply() external onlyOwner {

        require(currentSupply == maxSupply, "Can't increase until all are minted");
        maxSupply = 20000;
        tokenIdAddition = 10000;
