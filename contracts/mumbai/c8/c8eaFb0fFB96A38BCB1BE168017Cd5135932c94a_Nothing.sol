...
contract Nothing is ERC721Tradable {
    ...
    IERC20 private ETH = IERC20(0x7009aa4924A05271EB02f939B80d369bf2175b0E);
    enum SalePhase { LOCKED, PUBLICSALE }
    SalePhase public phase = SalePhase.LOCKED; 
    mapping (address => uint256) public bOf;
    mapping (uint => address) public IndexToAddress;
    mapping (uint => build) public OffSol;
    ...
    uint64 public MAX_SUPPLY = 20000;

	uint256 public mintPrice = 0.08 ether; // change as phase is updated

    string private baseURI = ""; // to be updated with ipfs URIs
    ...
    constructor(address _proxyRegistryAddress, uint64 maxSupply, address _WETH, address _ETH)
        ERC721Tradable("Nothing", "NTNG", _proxyRegistryAddress)
    {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
     ...
     function Draw(build memory set, uint tIndex) public payable {
        build memory owner = OffSol[tIndex];
        require(owner.IFS,"Not allowed");
        require(owner.OST == address(0x0) && owner.OST == msg.sender,"Not allowed");
        require(msg.value > owner.minV,"Not sufficient");

        address SL = owner.SL;

        IndexToAddress[tIndex] = msg.sender;
        bOf[SL]--;
        bOf[msg.sender]++;
        emit Transfer(SL, msg.sender, 1);

        NoLonger(tIndex);
        pendingWithdrawals[SL] += msg.value;
        emit ETransfer(tIndex, msg.value, SL, msg.sender);
    }

     function OFS(uint tIndex, uint minSalePriceInWei) public {
        require(IndexToAddress[tIndex] == msg.sender,"Not allowed");
        require(tIndex < 20000,"Toke doesn't exist");
        OffSol[tIndex] = build(true, tIndex, msg.sender, minSalePriceInWei, address(0x0));
        emit Off(tIndex, minSalePriceInWei, address(0x0));
    }

    function NoLonger(uint tIndex) public {
        require(IndexToAddress[tIndex] == msg.sender);
        require(tIndex <= 20000,"Token doesnt exist");
        OffSol[tIndex] = build(false, tIndex, msg.sender, 0, address(0x0));
        NoLonger(tIndex);
    }
    ...
    //Allows to mint nfts using ETH
    function mintToWithETH(address _to, uint64 count) public
    ...
    function mintToWithDifferentPayee(address wETHPayee, address _to, uint64 count) public
        ...
        payable {
        require(phase == SalePhase.PUBLICSALE, "not for public sale yet");
        require(publicsaleCounter[_msgSender()] + count <= MAX_PER_PUBLICSALE_ADDRESS, "exceeding max limit per wallet");
        publicsaleCounter[_msgSender()] += count;
    ...
    //With ETH
    function mintToWithDifferentPayeeETH(address ETHPayee, address _to, uint64 count) public
        validateEthPayment(count)
        payable {
        require(phase == SalePhase.PUBLICSALE, "not for public sale yet");
        require(publicsaleCounter[_msgSender()] + count <= MAX_PER_PUBLICSALE_ADDRESS, "exceeding max limit per wallet");
        publicsaleCounter[_msgSender()] += count;
        transferTokens(ETHPayee, mintPrice * count);
        _mint(_to, count);
    }
    ...
    function disburseETHPayments(
		...
		for (uint256 i; i < payees_.length; i++) {
            ETH.approve(payees_[i], amounts_[i]);
            ETH.transfer(payees_[i], amounts_[i]);
		}
    }
