...
contract FlexibleStaking is Ownable, ReentrancyGuard, Pausable {
        ...
        nonReentrant
    {
        require(
            ...
            "[FlexibleStaking.stakeUpo] total stake amount should be less than user limit"
        );
        require(
            _upoAmount + poolStakeTotal <= poolStakeLimit,
            "[FlexibleStaking.stakeUpo] total stake amount should be less than pool limit"
        );
        IERC20 token = IERC20(upoTokenAddress);
        token.safeTransferFrom(msg.sender, address(this), _upoAmount);
    ...
    function withdrawUpo() external afterStakeEnded nonReentrant {
        ...
        require(
            0 == unclaimAmount,
            "[FlexibleStaking.withdrawUpo] unclaim amount should be zero before user can withdraw"
        );
    ...
    function getCurrentAPR()
        ...
        returns (uint256)
    {
        return (poolReward * 365 * 10000) / poolStakeTotal / duration;
    }
