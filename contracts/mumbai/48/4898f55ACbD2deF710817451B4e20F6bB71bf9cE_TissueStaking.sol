...
contract TissueNFT is ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("TissueNFT", "TSU") {}
...
contract TissueReward is ERC20, ERC20Burnable, Ownable {
  mapping(address => bool) private controllers;
  address[] private controllersArray;

  constructor() ERC20("TissueReward", "TSR") { }
  ...
  function addController(address controller) external onlyOwner {
    require(!controllers[controller], "Controller is already on the list");
  ...
  function getControllers() external view returns (address[] memory) {
    uint256 count = 0;
    for (uint256 i = 0; i < controllersArray.length; i++) {
      if (controllers[controllersArray[i]]) {
        count++;
      }
    }
    ...
    for (uint256 i = 0; i < controllersArray.length; i++) {
      if (controllers[controllersArray[i]]) {
        result[index] = controllersArray[i];
        index++;
      }
    }
...
contract TissueStaking is Ownable, IERC721Receiver {
  ...
  mapping(uint256 => Stake) public vault; 
   ...
   constructor(TissueNFT _nft, TissueReward _token) { 
    nft = _nft;
    token = _token;
  }
  ...
  function stakeAll() external {
    uint256 tokenId;
    uint256[] memory tokenIds = nft.walletOfOwner(msg.sender);
  ...
  function _claim(address account, uint256[] memory tokenIds, bool _unstake) internal {
    uint256 tokenId;
    uint256 earned = 0;
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      ...
      rewardmath = 8640000 ether * (block.timestamp - stakedAt) / 86400 ;
  ...
  function earningInfo() external view returns (uint256[1] memory info) {
    uint256[] memory tokenIds = myStaking();
  ...
  function myStaking() public view returns (uint256[] memory ownerTokens){
