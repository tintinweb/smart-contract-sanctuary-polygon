...
contract Sof is ERC1155, ERC1155Supply, Ownable {
    ...
    modifier isSecured(uint8 mintPhase) {
        ...
        require(mintPhase > 2, 'MINT_PHASE_IS_INVALID');
    ...
    function publicMint(uint256 numOfTokens, uint8 mintPhase) external isSecured(mintPhase) payable {
        require(numOfTokens + totalSupply(SOF_TOKEN) <= PHASE_SUPPLY[mintPhase], "NOT_ENOUGH_SUPPLY");
    ...
    function freeMint(uint256 numOfTokens, uint8 mintPhase, bytes32[] memory proof) external isSecured(mintPhase) payable {
