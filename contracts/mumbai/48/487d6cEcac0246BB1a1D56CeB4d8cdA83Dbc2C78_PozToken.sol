    ...
    ERC20Upgradeable
{
    ...
    function dynamicAdjustment(bool isTrade) public onlyRift {
        require(prevBackingAmount > 0, "PozToken: unknown backing amount");

        if(!isTrade) {
            require(prevTransferTo != address(0), "PozToken: unknown to address");
            require(_balances[backing.reserveAddress] >= prevBackingAmount, "PozToken: insufficient backing poz");
            require(_balances[emission.reserveAddress] >= prevFeeAmount - prevBackingAmount, "PozToken: insufficient emission poz");
    ...
    function setIsPauseTrade(bool _isPauseTrade) external onlyRift {
        if(_isPauseTrade == false) {
            require(prevBackingAmount == 0, "PozToken: price is still lower");
        }
