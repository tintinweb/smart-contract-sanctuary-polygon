...
import "../registryV2/ITraitValueImplementer.sol";
import "../registryV2/IECRegistry8.sol";
...
interface iHexToken {
    ...
    function stickerBurn(uint16[] memory tokenIDs) external;
...
contract galaxis_stickerbook_mark_two_transfer is ERC1155, Ownable {
    using Strings for uint256;

    mapping(address =>bool)                             public allowed;

    // stickerbook stuff
    uint256                                             public nextBook;
    mapping(string  => uint256)                         public stickerbooks;
    mapping(uint256 => stickerBookInfo)                 public stickerBookData;
    mapping(uint256 =>                                  // book id
            mapping(uint256 =>  sticker[]))             public stickers;     // sticker set in a particular book 1 - stickerCount -1 

    mapping(uint16 => ITraitValueImplementer)           public _implementers;
    mapping(string => uint16)                           public traitNumbers;
    IERC721                                 immutable   public nft;
    IECRegistry                             immutable   public reg;


    modifier onlyAllowed {
        require(allowed[msg.sender],"StickerBook : unauthorised");
        _;
    }
    ...
    //--- ADMIN
    function setAllowed(address user, bool state) external onlyOwner {
        allowed[user] = state;
    }

    function newStickerBook(
        string calldata name
    ) external onlyAllowed {
        _newStickerBook(name);
    }
    ...
    function _newStickerBook(
        ...
        uint256 bookPos = (nextBook += 1);
        stickerbooks[name] = bookPos;
        stickerBookData[bookPos] = stickerBookInfo(name,bookPos,false,0,"");
    }

    function getStickerBookID(string memory name) internal view returns (uint256) {
        uint256 stickerBookID = stickerbooks[name];
        if (stickerBookID == 0) {
    ...
    function addSticker(
        string calldata sticker_name,
        stickerInput[] calldata thisStickerData
    ) external onlyAllowed {
        uint256 stickerBookID = getStickerBookID(sticker_name);
        _addSticker(stickerBookID,thisStickerData);
    ...
    function _addSticker(
        ...
        uint256 stickerPosition = stickerBookData[stickerBookID].stickerCount;
    ...
    function convert(stickerInput memory si) internal returns (sticker memory sm) {
        traitStruct memory ts;
        (ts.name,ts.implementer,ts.traitType,ts.start,ts.end) = reg.traits(si.traitID);
        sm.name = ts.name;
        sm.traitID = si.traitID;

        if (address(_implementers[si.traitID]) == address(0)) {
        if (ts.implementer == address(0)) {  // boolean
                _implementers[si.traitID] = ITraitValueImplementer(address(reg));
            } else {
                _implementers[si.traitID] = ITraitValueImplementer(ts.implementer);
            }
            
        }
        if (address(_implementers[si.traitID]) == address(reg)) {  // boolean
            require(si.alternative_values.length == 1,"Boolean traits can only have one value");
            require(si.alternative_values[0] < 2,"Boolean values must be 0 or 1");
    ...
    function setRewardURI(
        ...
        uint256 stickerBookID = getStickerBookID(name);
    ...
    function claim(string calldata name, uint16[] calldata tokenIds) external {
        uint16[] memory stix = new uint16[](tokenIds.length);
        uint256 stickerBookID = getStickerBookID(name);
        ...
        for (uint pos = 0; pos < tokenIds.length; pos++) {
            ...
            for (uint256 criteria = 0; criteria < numberOfCriteria; criteria++) {
                ...
                ITraitValueImplementer imp = _implementers[st.traitID];
                ...
                uint256 traitNum = _implementers[st.traitID].getValue(uint16(tokenId)); // get the value of that trait for this nft / tokenID
        ...
        //iHexToken(address(nft)).stickerBurn(stix);
        _mint(
            msg.sender,
            stickerBookID,
            1,
    ...
    function eligible(string memory name, uint16 tokenId) external view returns (uint256[] memory) {
       uint256 stickerBookID = getStickerBookID(name);
        uint256 count = stickerBookData[stickerBookID].stickerCount;
        uint256[] memory temp = new uint256[](count);
        uint256 pointer;
 
        for (uint256 position = 0; position < count; position++) {
            if (meetsCriteria(name,tokenId,position)) {
                temp[pointer++] = position;
            }
        }
        if (pointer == count) return temp;
    ...
    function satisfies(string memory name, uint256 position, uint16[] memory tokenIds) external view returns (bool[] memory) {
    ...
    function meetsCriteria(string memory name, uint16 tokenId, uint256 pos) public view returns (bool) {
        uint256 stickerBookID = getStickerBookID(name);
        uint256 numberOfCriteria = stickers[stickerBookID][pos].length;
        for (uint256 criteria = 0; criteria < numberOfCriteria; criteria++) {
            sticker memory st = stickers[stickerBookID][pos][criteria];
    ...
    function setURI(string calldata _newURI) external onlyAllowed {
        _setURI(_newURI);
    }
    ...
    function addFullStickerBook(fullStickerBookData calldata fsbd) external onlyAllowed {
        _newStickerBook(fsbd.name);
        uint256 stickerBookID = getStickerBookID(fsbd.name);
        for (uint pos = 0; pos < fsbd.stix.length; pos++) {
            _addSticker(stickerBookID,fsbd.stix[pos]);
            stickerBookData[stickerBookID].stickerCount++;
        }
    }

    function getStickers(string memory stickerBookName,uint256 stickerPosition) external view returns (sticker[] memory) {
        uint256 stickerBookID = getStickerBookID(stickerBookName);
        require(stickerPosition < stickerBookData[stickerBookID].stickerCount,"Invalid sticker number");
        uint256 numberOfStickers = stickers[stickerBookID][stickerPosition].length;
        sticker[] memory result = new sticker[](numberOfStickers);
        for (uint pos = 0; pos < numberOfStickers; pos++) {
            result[pos] = stickers[stickerBookID][stickerPosition][pos];
        }
        return result;
    }
...
interface ITraitValueImplementer {

    function setValue(uint16 _tokenId, uint8 _value) external;

    function setData(uint16[] memory _tokenIds, uint8[] memory _value) external;
