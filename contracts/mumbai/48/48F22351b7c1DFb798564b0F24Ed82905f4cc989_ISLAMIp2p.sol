...
*/  
    ...
    uint256 public orN; //represents order number created in the array
    ...
    //mapping(uint => orderDetails[]) public allOrders;
...
*/
    ...
    function createOrder(
        ...
        orN++;// = OrderDetails.length + 1;
        uint256 _price;
        p2p[orN].orderIndex = OrderDetails.length;
        p2p[orN].orderLife = block.timestamp.add(3 days);
        ...
        if(_type == 1){ //sell ISLAMI
            ...
            p2p[orN].remainCurrency = _currencyAmount.mul(USDdcml);
    ...
    function cancleOrder() external nonReentrant{
        ...
        uint256 _orderIndex = p2p[userOrder[msg.sender]].orderIndex;
        ...
        delete userOrder[msg.sender];
        delete p2p[_orderNumber];
        fixOrders(_orderIndex);
        canCreateOrder[msg.sender] = true;
    }
    /*function fixIndex(uint256 _orderIndex) internal{

    }*/
    function fixOrders(uint256 _orderIndex) internal{
        for(uint i = _orderIndex; i < OrderDetails.length-1 ; i++){
            OrderDetails[_orderIndex] = OrderDetails[i+1];
            p2p[_orderIndex + 2].orderIndex -= 1;
        }
        //OrderDetails[_orderIndex] = OrderDetails[OrderDetails.length - 1];
        OrderDetails.pop();
        //delete OrderDetails[_orderIndex];
    }
    ...
    function takeOrder(uint256 _orN, uint256 _amount) external nonReentrant{
        ...
        uint256 amount = _amount.mul(USDdcml);
        uint256 toReceive = amount.div(priceUSD).mul(ISLAMIdcml);
        ...
        require(_amount.mul(ISLAMIdcml) <= p2p[_orN].remainAmount, "Seller has less ISLAMI than order");
            if(_amount.mul(ISLAMIdcml) < p2p[_orN].remainAmount){
                ...
                revert notOk("Wrong order number");
            }
        }
        ...
        require(_amount.mul(USDdcml) <= p2p[_orN].remainCurrency, "Seller has less USD than order");
            if(_amount.mul(USDdcml) < p2p[_orN].remainCurrency){
                ...
                _currency.transfer(msg.sender, _amount.mul(USDdcml));
