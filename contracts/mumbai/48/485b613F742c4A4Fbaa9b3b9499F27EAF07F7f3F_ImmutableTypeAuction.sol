...
contract ImmutableTypeAuction is Pausable, ERC721Holder {
    uint256 feeCut = 5;
    uint256 salesCut = 10;
    ...
    event OrderUpdated(bytes32 orderId, uint256 askingPrice);
    ...
    event inElse(uint256 expiry, uint256 current);

    event DataChanged(Order order, uint256 currentTime);
        ...
        whenNotPaused
    {
        ...
        require(
            ...
            "ImmutableAuction: unauthorized sender"
        );
    ...
    function updateOrder(
        ...
        require(
            order.orderId != 0,
            "ImmutableAuction: Order not yet published"
        );
        require(
            order.seller == msg.sender,
            "ImmutableAuction: sender is not allowed"
        );
        require(
            order.askingPrice > 0,
            "ImmutableAuction: Price should be bigger than 0"
        );
    ...
    function safeExecuteOrder(
        ...
        require(
            order.askingPrice == _askingPrice,
            "ImmutableAuction: invalid price"
        );
        require(
            order.seller != msg.sender,
            "ImmutableAuction: unauthorized sender"
        ...
        whenNotPaused
    {
        ...
        require(
            order.orderId != 0,
            "ImmutableAuction: This order doesn't exist"
        );
        require(
            order.seller != msg.sender,
            "ImmutableAuction: The owner of NFT cannot place bid itself"
        );
    ...
    function acceptBidandExecuteOrder(
        ...
        require(
            ...
            "ImmutableAuction: invalid bid price"
        );
        ...
        payable(owner()).transfer(bid.bidPrice);
    ...
    function _createOrder(
        ...
        require(
            ...
            "ImmutableAuction: Only the asset owner can create orders"
        );
        require(_askingPrice > 0, "ImmutableAuction: not enough funds send");
    ...
    function _createBid(
        ...
        Bid memory bid = bidByOrderId[_tokenAddress][_tokenId];
        if (bid.bidId != 0) {
            uint256 eTime = order.expiryTime;
            uint256 tToCompare = block.timestamp;
            require(
                eTime > tToCompare,
                "ImmutableAuction: Bidding Time Expired"
            );
            uint256 validBid = bid.bidPrice + ((bid.bidPrice * 10) / 100);
            require(
                value >= validBid,
                "ImmutableAuction: Bid must be 10% higher than the last bid"
            );
            ...
            require(
                ...
                "ImmutableAuction: bid should be greater than or equal to reserve price"
            );
        }
        ...
        returns (Order memory order)
    {
        ...
        require(order.orderId != 0, "ImmutableAuction: asset not published");
        return order;
    }
