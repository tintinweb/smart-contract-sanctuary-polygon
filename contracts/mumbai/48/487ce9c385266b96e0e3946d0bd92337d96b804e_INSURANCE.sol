...
contract ROIV2 {
    ...
    constructor(address payable CreatorAddr, address payable DevAddr,address MAIN_CONTRACT,address _restartAddress) {
        INSURANCE_CONTRACT = payable(new INSURANCE());
        owner = CreatorAddr;
        dev = DevAddr;
        restartAddress = _restartAddress;
        ROI_V1 = ROIV1(MAIN_CONTRACT);
    ...
    function _refPayout(address _addr, uint256 _amount) private {
        address up = ROI_V1.getReferral(_addr);
        uint256 i = 0;
...
contract ROIDeployer
{
    ...
    function deploy(address payable _owner,address payable dev,address payable mainContract) external
    {
        require(deployer[msg.sender],"Invalid");
        ROIV2 contracts = new ROIV2(_owner,dev,mainContract,address(this));
