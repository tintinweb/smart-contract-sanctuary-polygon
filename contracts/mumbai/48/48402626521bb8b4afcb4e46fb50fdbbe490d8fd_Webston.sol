...
contract Webston {
    ...
    function deposit(address _refferel, uint256 _dai) public  {
        ...
        //referel
        dai.transfer(_refferel,_dai.mul(30).div(100));
        players[_refferel].b14Direct[poolNo]+=_dai.mul(30).div(100);
        //x3
        uint256 x3Amt=_dai.mul(10).div(100);
        if(players[_refferel].b3_level[poolNo].mod(3) != 0){
            dai.transfer(_refferel,x3Amt);
            players[_refferel].b3Income[poolNo]+=x3Amt;
        }else{
            checkB3refer(_refferel,x3Amt,poolNo);
        }
        //x14
    ...
    function _setb14(uint256 poolNo,address _referral,address _addr,uint256 _amount) private{
        ...
        players[b14[poolNo][_parent]].b14Income[poolNo]+=_amount.mul(5).div(100);
        //2nd upline
