...
// File: EpikoErc721.sol


pragma solidity ^0.8.1;

contract EpikoMarket is ERC721, IERC2981, ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("Epiko721", "EPIKO") {}

    function mint(address to, uint256 royaltyFraction, string memory _uri) external returns(uint256 _id){
        require(msg.sender == marketplaceAddress, "ERC721: Not authorised");
        ...
        _tokenRoyaltyInfo[_tokenIds.current()].royaltyFraction = royaltyFraction;
    ...
    function showDetailOfNFT(uint256 tokenId) public view returns (address receiver_, uint256 royaltyFraction_){
        require(_isExist(tokenId), "ERC721: Token not Exist");
    ...
    function addMarketplaceContract(address marketplace) external onlyOwner {
        require(marketplace != address(0), "Address zero provided");
    ...
    function royaltyInfo(uint256 tokenId, uint256 salePrice) public view override returns (address receiver, uint256 royaltyAmount_) {
        // require(_isExist(tokenId), "ERC721: Token not Exist");

        Royalty memory royalty = _tokenRoyaltyInfo[tokenId];
        uint256 _royaltyAmount = salePrice.mul(royalty.royaltyFraction).div(PERCENTAGE_DENOMINATOR);
