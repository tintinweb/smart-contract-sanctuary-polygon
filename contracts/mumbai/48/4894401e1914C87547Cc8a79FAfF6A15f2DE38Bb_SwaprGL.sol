...
contract SwaprGL is BaseGovernanceWithUserUpgradable, ListingHelper {
    ...
    function initialize(bytes calldata data) public initializer {
        ...
        _swaprFee = ISwaprFee(payable(swaprFeeAddress));
    ...
    function buyNowOrder(
        ...
        bytes memory data,
        uint split
    ) public payable onlyMarketplace(data) {
        (bytes memory listingData, ) = _decodeBytesToBytesBytes(data);
    ...
    function claim(bytes calldata data) external onlyMarketplace(data) {
        ...
        if (_verify(claimant, listingData)) {
            //for seller
            require(block.timestamp > bid.listingEndTime, "AUC_NOT_ENDED");
            PayNow memory payOps = PayNow(
                auction.toEOA,
                auction.acceptedToken,
                bid.bidder,
    ...
    function _withdraw(address claimant, address lock, uint nftId) internal {
        require(swaprWallet.isNFTLocked(lock, nftId), "ASSET_NOT_AVAILB");
...
contract ListingHelper {
    ...
    modifier onlyMarketplace(bytes memory data) {
        require(_verify(getMarketplace(), data), "INV_SIG");
        _;
    }
    ...
    function _validateAsFirstBid(
        bytes memory listingData,
        Bid memory bid
    ) internal view returns (bool isValid, uint proposedEndTime) {
    ...
    function _validateClaim(
        ...
        if (_verify(claimant, listingData)) {
            //for seller
            require(claimant == auction.seller, "INV_CLAIMANT");
            uint bidderTotalBalance = swaprWallet.getBalance(
                lastBid.bidder,
                auction.acceptedToken
            );
            ...
            //for bidder/buyer
            require(claimant == lastBid.bidder, "INV_CLAIMANT");
            if (swaprWallet.isNFTLocked(auction.lock, auction.nftId)) {
                isValid = true;
            }
        } else {
    ...
    function _getProposedEndTime(
        uint listingEndTime
    ) internal view returns (uint proposedEndTime) {
