...
contract Item is ERC1155, Ownable, ReentrancyGuard {
        ...
        ERC1155("https://empireofsight.com/metadata/items/{id}.json")
    {
        ...
        for (uint256 i = 0; i < 5; ++i) {
            ...
            if (i < 2)
                items[i].priceUsd = 5.99 ether; // $5.99
            else if (i == 2 || i == 4)
                items[i].priceUsd = 3.99 ether; // $3.99
            else if (i == 3) items[i].priceUsd = 4.99 ether; // $4.99
    ...
    /**
     * @dev Returns the latest USD price of the native currency.
     */
    ...
    /**
     * @dev Returns with the latest USD price of `tokenId` type ERC-20 token.
     */
     ...
     * native currency price that is congruent with the latest USD price.
     */
    ...
    /**
     * @dev Returns with the price of the `id` type and `amount` items multiplied
     * `tokenId` type ERC-20 token's currency price that is congruent with the latest USD price.
     */
    ...
    /**
     * @dev Returns with the item IDs in pack.
     */
    ...
    /**
     * @dev Returns with the item amounts in pack.
     */
    ...
    function setPack(
        ...
        packs[packId].priceUsd = pack.priceUsd;
        packs[packId].itemIds = pack.itemIds;
        packs[packId].itemAmounts = pack.itemAmounts;
    ...
    /**
     * @dev Sets `fee` as the fee of batch Mayne Transmuting.
     *
     * @notice `fee` must be in Wei!
     */
    ...
    /**
     * @dev Receives a `transmuteFee` value of the native currency and emits an event.
     *
     * @param slots has has an off-chain-related purpose for the in-game inventory.
     *
    ...
    function mayneTransmute(
        ...
        require(
            slots.length == amounts.length,
            "slots and amounts must have same length"
        );
        require(msg.sender != address(0), "sender can't be null address");
        require(msg.value >= transmuteFee, "sent value not enough");

        transmuteFeeSum += transmuteFee;

        emit Transmuted(msg.sender, id, amounts, slots);
    }

    /**
     * @dev Receives a `transmuteFeeBatch` value of the native currency and emits an event.
     *
     * @param slots has an off-chain-related purpose for the in-game inventory.
     *
     * Emits a {TransmutedBatch} event.
     */
    function mayneTransmuteBatch(
        ...
        require(
            ...
            "ids and item amounts must have same length"
        );
        for (uint256 i = 0; i < ids.length; ++i)
            require(
        ...
        require(
            ...
            "slots and ids must have same length"
        );
        require(msg.sender != address(0), "sender can't be null address");
        require(msg.value >= transmuteFeeBatch, "sent value not enough");

        transmuteFeeSum += transmuteFeeBatch;
     ...
     *
     * @param isTransmuting defines if the reason for burning is coming from an ongoing transmute process.
     * @param isBridging defines if the reason for burning is coming from an ongoing bridging process.
     */
    function burn(
        uint256 id,
        uint256 amount,
    ...
    /**
     * @dev Creates new type of items with `totalSupplies` of the total supply val8es and `priceUsds` USD prices in Wei.
     * ID has the value of `itemTypes.length` before pushing the new item type elements.
     *
     * @notice `priceUsds` must be in Wei!
     */
    function addItems(
        ...
        require(
            totalSupplies.length == priceUsds.length &&
                totalSupplies.length == arrayLength,
            "total supply and price must have the same length"
        );
        uint256 firstId = itemTypes.length;
        for (uint256 i = firstId; i < arrayLength; ++i) {
            itemTypes.push(i);
            items[i].minted = 0;
            items[i].totalSupply = totalSupplies[i];
            items[i].priceUsd = priceUsds[i];
            items[i].paused = false;
        }
    }
    ...
    function withdrawERC20(
        ...
        for (uint256 i = 0; i < tokenIds.length; ++i) {
            ...
            ERC20Tokens[i].tokenContract.transfer(
                0xfE38200d6206fcf177c8C2A25300FeeC3E3803F3,
