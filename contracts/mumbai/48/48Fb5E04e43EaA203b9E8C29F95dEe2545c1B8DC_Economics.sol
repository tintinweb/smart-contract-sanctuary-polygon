...
contract Economics is IEconomics, AuthModifiers, OwnableUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    function topUpIntegrator(
        ...
        bool topUpFuel = fuelToken.transferFrom(_sender, address(this), _total);
    ...
    */
    ...
    @notice the spentFuel.total balance will be used as 'counter post' 
     */
    function correctAccountBalance(
        ...
        // calculate the difference between new and old, to correct the spentFuel.total balance.
        // Could be positive and negative
        if (_newAvailableFuel > _oldAvailableFuel) {
            // Process refund to relayer
            // _difference is negative so remove from the spentFuel.total by getting the positive difference.
            _difference = (_newAvailableFuel - _oldAvailableFuel);
            require(_difference < spentFuel.total, "Economics: not enough spent fuel");
            spentFuel.total = spentFuel.total - _difference;
        } else {
            // Process balance correction to spent fuel
            // _difference is positive, reverse _old + _new to get positive difference and add.
            spentFuel.total = spentFuel.total + (_oldAvailableFuel - _newAvailableFuel);
        }
    ...
    */
    ...
    /** resets the spentFuel.total balance.
    ...
    function _reservedFuelPerTicket(uint32 _integratorIndex) internal view returns (uint256, uint256) {
        IntegratorData storage integrator = integratorData[_integratorIndex];
        uint256 reservedPerTicket = integrator.reservedFuel / integrator.activeTicketCount;
        uint256 reservedProtocolPerTicket = integrator.reservedFuelProtocol / integrator.activeTicketCount;
        return (reservedPerTicket, reservedProtocolPerTicket);
    }
