...
contract CHARACTER_FACTORY_V1 is Pausable, Ownable {
    ...
    /**
     * @dev Deploy CHARACTER_V1 and set up settings that is needed for Mint
     *
     * royaltyFeeNumerator's max is 10000 so 100 === 1%
     */
        ...
        public returns (address)
    {
        ...
        CHARACTER_V1 mint = new CHARACTER_V1(name, symbol);

        // Setting Role
        // Because Factory becomes Owner by default.
        mint.transferOwnership(msg.sender);
        mint.grantRole(mint.DEFAULT_ADMIN_ROLE(), msg.sender);
        mint.grantRole(mint.PAUSER_ROLE(), msg.sender);
        mint.grantRole(mint.ROYALTY_ROLE(), msg.sender);

        // Setting Royalty & minter role
        mint.setDefaultRoyalty(royaltyReceiver, royaltyFeeNumerator);
        for (uint i = 0; i < minterRoleAddress.length; i++) {
            mint.grantRole(mint.MINTER_ROLE(), minterRoleAddress[i]);
        }

        // revoke roles of factory
        mint.renounceRole(mint.DEFAULT_ADMIN_ROLE(), address(this));
        mint.renounceRole(mint.PAUSER_ROLE(), address(this));
        mint.renounceRole(mint.ROYALTY_ROLE(), address(this));
        mint.renounceRole(mint.MINTER_ROLE(), address(this));

        emit ContractDeployed(msg.sender, address(mint));
        return address(mint);
    }
}
