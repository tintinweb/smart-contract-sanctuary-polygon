...
contract Operations is VRFConsumerBaseV2, ERC2771Recipient {
    ...
    constructor(address owner, address payable core, address vrfCoordinatorV2, uint64 subscriptionId, bytes32 gasLane, uint32 callbackGasLimit) VRFConsumerBaseV2(vrfCoordinatorV2) {
        s_opsOwner = owner;
    ...
    function placeBet(address token, uint256 totalStake, uint256 totalPayout, bytes32[4] calldata betDetails, bool[4] calldata controls, uint256[2] calldata operators, bool callOracle) public {
        ...
        if (controls[2]) i_core.handleHL(provider, token, totalPayout, operators[0]);
        if (controls[3]) i_core.handlePayout(provider, token, totalPayout, operators[1]);
