    ...
    Ownable
{
    ...
    /// @dev Only FIAT_ROLE holders can nullify price on `claimWithCoupon`.
    bytes32 private fiatRole;
    ...
    /// @dev Only transfers to or from TRANSFER_ROLE holders are valid, when transfers are restricted.
    bytes32 private constant TRANSFER_ROLE = keccak256("TRANSFER_ADMIN_ROLE");
    ...
    ) ERC721(_symbol, _name)
    {
        ...
        bytes32 _priceAdminRole = keccak256("PRICE_ADMIN_ROLE");
        bytes32 _couponAdminRole = keccak256("COUPON_ADMIN_ROLE");
        bytes32 _transferRole = keccak256("TRANSFER_ADMIN_ROLE");
        bytes32 _minterRole = keccak256("MINTER_ADMIN_ROLE");
        bytes32 _fiatRole = keccak256("FIAT_ADMIN_ROLE");

        _setupContractURI(_contractURI);
        _setupRole(DEFAULT_ADMIN_ROLE, _defaultAdmin);
        _setupRole(_minterRole, _defaultAdmin);
        _setupRole(_fiatRole, _defaultAdmin);
        _setupRole(_transferRole, _defaultAdmin);
        _setupRole(_transferRole, address(0));
        _setupRole(_couponAdminRole, _defaultAdmin);
        _setupRole(_priceAdminRole, _defaultAdmin);
        _setupPrimarySaleRecipient(_saleRecipient);

        transferRole = _transferRole;
        priceAdminRole = _priceAdminRole;
    ...
    function claimWithCoupon(
        ...
        //     bytes32 messageHash = keccak256(abi.encodePacked(_msgSender(), "+", _tier)).toEthSignedMessageHash();

        //     // Verify that the message's signer has the fiat role
        //     address signer = messageHash.recover(bytes(_coupon));
        //     if (!hasRole(fiatRole, signer))
        //         revert CurrencyNotValid();
        // }
        // else
        // {
            Coupon memory coupon = _getCoupon(_coupon);
            uint256 discountedPrice = this.calcPrice(coupon, _currency, _tier);
            coupon.usedCount += 1;
    ...
    function safeMintWithTier(
        ...
        startTokenId = _safeMintWithTier(_tokenId, _tier, _recipient);
    }
    ...
    function _safeMintWithTier(
        ...
        uint256 remaningToDistribute = 1;
    ...
    function _mapTokensToTier(
        ...
        TokenRange[] memory _tokensInTier = tokensInTier[_tier];
        uint256 len = _tokensInTier.length;

        uint256 qtyRemaining = _quantity;

        for (uint256 i = 0; i < len; i += 1) {
            TokenRange memory range = _tokensInTier[i];
            uint256 gap = 0;
            ...
            if (range.startIdInclusive <= nextIdFromTier && nextIdFromTier < range.endIdNonInclusive) {
                ...
                if (nextIdFromTier + gap < range.endIdNonInclusive) {
                    ...
                    nextIdFromTier = _tokensInTier[i + 1].startIdInclusive;
                } else {
    ...
    /// @dev Only COUPON_ADMIN_ROLE holders can add and modify a coupon.
    ...
    function _getCoupon(string calldata _coupon)
        ...
        returns (Coupon memory)
    {
        if (bytes(_coupon).length == 0)
            return Coupon("", 0, 1, 0, 0, "INVALID");
   ...
   function getCouponIdx(string calldata _coupon)
    ...
    /// @dev Only PRICE_ADMIN_ROLE holders can add and modify a price.
    bytes32 private priceAdminRole;
    ...
    function getPriceIdx(address _currency, string memory _tier)
        private view
        returns (uint256 idx) {
    ...
    function calcPrices(
        ...
        for (uint256 i = 0; i < prices.length; i++) {
            if (strcmp(prices[i].tier, _tier)) {
                ...
                cpd[j].price = this.calcPrice(coupon, prices[i].currency, _tier);
