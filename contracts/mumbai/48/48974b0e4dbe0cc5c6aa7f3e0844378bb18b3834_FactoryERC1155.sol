  ...
  UUPSUpgradeable 
{
  ...
  address private _CUT_RECEIVER; // Page Tree address to receive cut
    ...
    ERC1155Supply 
{
    ...
    // Page Tree cut
    uint16 public _PAGETREE_CUT; // 1% equal 100
    ...
    // Calculate Page Tree cut
    function calculateCut(uint256 amount) 
        internal view
        returns (uint256)
    {
        uint256 cut = amount * _PAGETREE_CUT / 10000;
        if (amount <= 10) {
        return 0;
        } else if(cut <= 1) {
        return 1;
