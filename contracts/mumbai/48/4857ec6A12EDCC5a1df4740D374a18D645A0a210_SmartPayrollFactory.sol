...
contract SmartPayrollByTime is AutomationCompatibleInterface {
  ...
  function performUpkeep(bytes calldata /* performData */) external override {
    ...
    if ((block.timestamp - lastTimeStamp) > interval) {
      ...
      // ISmartPayrollFactory(factory).CreditMint(credit,receiver,receiver);
      lastTimeStamp = block.timestamp;
      counter = counter + 1;
    }
...
contract SmartPayrollFactory {
  ...
  function CreditMint(address _creditAddress,address _employer,address _employee) external {
    IcreditToken(_creditAddress).mint(_employer,10);
    IcreditToken(_creditAddress).mint(_employee,10);
