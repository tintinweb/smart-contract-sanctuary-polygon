  ...
  ERC1155BurnableUpgradeable
{
  ...
  ) external onlyTeamOrOwner returns (uint256 packId) {
  ...
  function updatePack(
    ...
    require(exists(_packId), "Not found");
  ...
  function openPack(uint256 _packId) external returns (uint256[] memory) {
    require(msg.sender == tx.origin, "opener must be eoa");
  ...
  function getPack(uint256 _tokenId) public view returns (Pack memory) {
    require(exists(_tokenId), "Not found");
  ...
  function _burnToken(PackItem memory _token) internal {
    ...
    (bool _success, bytes memory _owner) = _token.tokenAddress.call(signature);
    require(_success);
  }
}
  ...
  SignatureMintableUpgradeable
{
  ...
  function _beforeTokenTransfer(
    ...
    for (uint256 i = 0; i < ids.length; i++) {
      Pack memory pack = _packs[ids[i]];
      if (from != address(0)) {
        require(!pack.nonTradeable, "Nontradeable");
      }

      if (from == address(0)) {
        require(!pack.closed, "Token closed");

        minted[to][ids[i]] = minted[to][ids[i]] + amounts[i];
        require(minted[to][ids[i]] <= pack.maxPerWallet, "MPW");
        _packs[ids[i]].minted = pack.minted + amounts[i];
      }
    }
