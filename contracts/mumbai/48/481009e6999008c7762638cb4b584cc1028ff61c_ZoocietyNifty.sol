    ...
    IERC4907
{
    ...
    string private contractSellerFee;
    ...
    mapping(uint256 => mapping(uint8 => bool)) public isAccessPassRunning; // running means unpacked
    ...
    constructor() ERC721("Zoociety Nifty", "NIFTY") {
        newAccessPass(0, "AFFILIATE", 0);
        newAccessPass(1, "AIMAGE", 10);
        newAccessPass(2, "TOOLKAIT", 10);
        newAccessPass(3, "PROMPT", 10);
        isAffiliate[_msgSender()] = true;
        administrator = payable(msg.sender);
        defaultReward = 10;
        mint(_msgSender(), 0, false);
        mint(_msgSender(), 1, false);
        mint(_msgSender(), 2, true);
    }
    ...
    // administrator AND DESIGNER FUNCTIONS
    ...
    function newAffiliate(string memory _userId, address _referredBy) public {
        ...
        if (!isAffiliate[_referredBy] || !restricted[_referredBy]) {
    ...
    function newTld(string memory _tld) public {
        require(!niftynameservice.isValidTld(_tld));
        niftynameservice.createTld(_tld);
    }
    ...
    function newRootDomainName(string memory _domain, string memory _tld)
        public
    {
        require(niftynameservice.isValidTld(_tld));
        niftynameservice.setRootDomain(_domain, _tld);
    }
    ...
    function getAffiliateId(address) public view returns (string memory) {
        return affiliateIds[msg.sender];
    }
    ...
    function mintNiftyWithAccess(
        ...
        require(_accessId != 0);
        require(_accessId <= validAccessLength);
        if (msg.sender != administrator) {
            require(_receiver != address(0), "Address is Zero");
            require(msg.value >= price(_accessId), "Not Enough Matic");
            require(!restricted[_receiver], "Restricted User");
            require(
                _receiver != _affiliate,
                "Receiver must not equal to affiliate"
            );
            require(!restricted[_affiliate], "Restricted Affiliate");
            if (isAffiliate[_affiliate]) {
                uint256 reward = msg.value.mul(percentRewards[_affiliate]).div(
                    100
                );

                if (invitedBy[_affiliate] == address(0)) {
                    _affiliate.transfer(reward);
                } else {
                    uint256 rreward = reward
                        .mul(percentRewards[_affiliate])
                        .div(100);
                    _affiliate.transfer(reward.sub(rreward));
                    payable(invitedBy[_affiliate]).transfer(rreward);
                }
                administrator.transfer(msg.value.sub(reward));
            } else {
                administrator.transfer(msg.value);
            }
        }
        mint(_receiver, _accessId, _packed);
    }

    // Minting without Affiliate
    function mintMe(uint8 _accessId, bool _packed) public payable {
        require(_accessId != 0);
        require(_accessId <= validAccessLength);
        require(msg.value >= price(_accessId), "Not Enough Matic");
        mint(msg.sender, _accessId, _packed);
    }
        ...
        payable
    {
        ...
        for (uint256 i = 0; i < _accessIds.length; i++) {
            require(i != 0);
            require(i <= validAccessLength);
            totalprice.add(price(_accessIds[i]));
        }

        require(msg.value >= totalprice, "Not Enough Matic");
        for (uint256 i = 0; i < _accessIds.length; i++) {
            recordAccessOfToken(tokenId, _accessIds[i], false);
        }
    }
    ...
    function mint(
        ...
        recordAccessOfToken(totalSupply, _accessId, _packed);
        packed[totalSupply] = _packed;
        totalSupply++;
    }

    function mintPacked(address _address, uint8 _accessId) public payable {
        require(_accessId != 0);
        require(_accessId <= validAccessLength);
        require(msg.value >= price(_accessId), "Not Enough Matic");
        administrator.transfer(msg.value);
        mint(_address, _accessId, true);
    }
    ...
    function addExpiration(uint256 tokenId, uint8 _accessId) internal {
        expirations[tokenId][_accessId] = block.timestamp + 30 days;
    }
    ...
    function getAccessOfToken(uint256 tokenId, uint8 _i)
        public
        view
        returns (string memory)
    {
        ...
        returns (string[] memory)
    {
        ...
        for (uint256 i = 0; i < len; i++) {
            string memory list = getAccessOfToken(tokenId, uint8(i));
    ...
    function setContract(
        ...
        string memory _extLink,
        string memory sellerFee,
        string memory sellerAddr
    ) external onlyOwner {
        contractImage = _image;
        contractName = _name;
        contractDescription = _description;
        contractExtLink = _extLink;
        contractSellerFee = sellerFee;
        contractSellerAddr = sellerAddr;
    }
        ...
        returns (string memory)
    {
        ...
        string memory name_ = "ZOOCIETY NIFTY";
        string memory description = (
            isSoulToken(tokenId)
                ? string(
                    abi.encodePacked(
                        "[Join us at Zoociety!](https://zoociety.xyz/join/",
        ...
        string memory image = (
            isWithImage[tokenId] ? images[tokenId] : contractImage
        );
        ...
        returns (string memory)
    {
        ...
        string memory affiliatetag = (
            isSoulToken(tokenId) ? "<li>AFFILIATE CARD</li>" : ""
        );
