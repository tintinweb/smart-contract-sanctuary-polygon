...
contract CreaticlesDapp is ContextUpgradeable, ERC721Holder, ERC1155Holder {
    ...
    mapping(uint256 => mapping(uint256 => uint256)) public prizeAmounts;
    ...
    */
    ...
    function isERC20(address _token) public view returns (bool) {
        return ERC165(_token).supportsInterface(0x36372b07); // ERC20 interfaceId
    }

    function isERC721(address _token) public view returns (bool) {
        return ERC165(_token).supportsInterface(0x80ac58cd); // ERC721 interfaceId
    }

    function isERC1155(address _token) public view returns (bool) {
        return ERC165(_token).supportsInterface(0xd9b67a26); // ERC1155 interfaceId
    }
    ...
    @param _paymentTokenAddress => ERC20, ERC721 or ERC1155
    @param _paymentValue => Token amount for ERC20(index 0) or ERC721 or ERC1155(NFT)
    @param _distribution => Prize distribution ratio array for ERC20 or token id array for ERC721 or ERC1155(NFT)
    */
    function createRequest(bytes32 _detailsHash, uint16 _numberOfWinners, uint32 _duration, uint256 _numMintPerToken, address _paymentTokenAddress, uint256 [] memory _paymentValue, uint256 [] memory _distribution) public payable returns(uint256) {
        require(_numberOfWinners > 0);
        require(_numberOfWinners <= 100);
        require(_paymentValue.length > 0 && _paymentValue[0] > 0);
        require(_distribution.length == _numberOfWinners, 'Distribution array length should be same as number of winners.');

        uint256 _cval;
        uint256 _value;
        {
            if (_paymentTokenAddress == address(0)) { // zero address corresponds to ethereum payment, the default
                require(msg.value == _paymentValue[0]);
                _cval = (msg.value * 25) / 1000;    // 2.5% commision
                _value = msg.value - _cval;
                cval += _cval;
            }
            else if (_paymentTokenAddress == creaticles) {
                IERC20(_paymentTokenAddress).transferFrom(msg.sender, address(this), _paymentValue[0]);
                _value = _paymentValue[0];
            }
            else {
                if (isERC20(_paymentTokenAddress)) {
                    // Here we explore additional ERC20 payment options
                    IERC20(_paymentTokenAddress).transferFrom(msg.sender, address(this), _paymentValue[0]);
                    _cval = (_paymentValue[0] * 25) / 1000;    // 2.5% commision
                    _value = _paymentValue[0] - _cval;
                } else {
                    _value = 0;
                    if (isERC721(_paymentTokenAddress)) {
                        require(_paymentValue.length == _numberOfWinners, 'Token id length should be same as number of winners.');
                        for (_cval = 0; _cval < _numberOfWinners; _cval++) {
                            IERC721(_paymentTokenAddress).safeTransferFrom(msg.sender, address(this), _distribution[_cval]);
                            prizeAmounts[numberOfRequests][_cval] = _paymentValue[_cval];
                        }
                    } else if (isERC1155(_paymentTokenAddress)) {
                        require(_paymentValue.length == _numberOfWinners, 'Token id length should be same as number of winners.');
                        for (_cval = 0; _cval < _numberOfWinners; _cval++) {
                            IERC1155(_paymentTokenAddress).safeTransferFrom(msg.sender, address(this), _distribution[_cval], _paymentValue[_cval], "");
                            prizeAmounts[numberOfRequests][_cval] = _paymentValue[_cval];
                        }
                    }
                }
            }
            request_erc20_addresses[numberOfRequests] = _paymentTokenAddress;
            for (_cval = 0; _cval < _numberOfWinners; _cval++) {
                distributions[numberOfRequests][_cval] = _distribution[_cval];
            }
    ...
    */
    function acceptProposals(address _to, uint256 _requestId, uint256[] memory _proposalId, uint256[] memory _tokenIds, string[] memory _tokenURLs, address[] memory _winners, uint256 _tokenSupplies) public isCreaticlesNFTContract() {
        ...
        if (distributions[_requestId][0] > 0) {
            uint256 _original_value = _request.value;
            //loop through winners and send their ETH
            for (uint256 i = 0; i < _winners.length; i++) {
                if (!isERC721(request_erc20_address) && !isERC1155(request_erc20_address)) {
                    _winnerValue = _original_value * distributions[_requestId][i] / 1000;
                    _request.value -= _winnerValue;

                    if (request_erc20_address == address(0)) {
                        require(payable(_winners[i]).send(_winnerValue), "Failed to send Ether");
                    } else if (isERC20(request_erc20_address)) {
                        // if we are not sending ether, we send ERC20 token
                        IERC20(request_erc20_address).transfer(_winners[i], _winnerValue);
                    } 
                } else {
                    uint256 _distribution = distributions[_requestId][i];
                    uint256 _prizeAmount = prizeAmounts[_requestId][i];
                    if (isERC721(request_erc20_address)) {
                        IERC721(request_erc20_address).safeTransferFrom(address(this), _winners[i], _distribution);
                    } else if (isERC1155(request_erc20_address)) {
                        IERC1155(request_erc20_address).safeTransferFrom(address(this), _winners[i], _distribution, _prizeAmount, "");
                    }
                }
            }
        } else {
    ...
    function reclaimFunds(uint256 _requestId) public {
        ...
        if (request_erc20_address == address(0)) {
            ...
            if (!isERC721(request_erc20_address) && !isERC1155(request_erc20_address)) {
                ...
                for (i = 0; i < _request.numberOfWinners; i++) {
                    _distribution = distributions[_requestId][i];
                    _prizeAmount = prizeAmounts[_requestId][i];
                    if (isERC721(request_erc20_address)) {
                        IERC721(request_erc20_address).safeTransferFrom(address(this), msg.sender, _distribution);
                    } else if (isERC1155(request_erc20_address)) {
                        IERC1155(request_erc20_address).safeTransferFrom(address(this), msg.sender, _distribution, _prizeAmount, "");
                    }
                }
            }
        }
    ...
    */
    ...
    function addSubAdmin(address _subadmin) external isAdmin() {
        subAdmins.add(_subadmin);
    }

    function removeSubAdmin(address _subadmin) external isAdmin() {
