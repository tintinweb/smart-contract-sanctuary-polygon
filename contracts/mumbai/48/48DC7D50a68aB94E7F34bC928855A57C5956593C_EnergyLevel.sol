...
contract EnergyLevel is Multicall, HSystemChecker {
    ...
    function _unpack(uint64 packedEnergyLevel)
        ...
        returns (uint16[4] memory unpackedArray)
    {
        uint16[4] memory unpackedEnergyLevel;

        unpackedEnergyLevel[0] = uint16(packedEnergyLevel); // energy: represents 4 diff energy types X 16 uint value
        unpackedEnergyLevel[1] = uint16(packedEnergyLevel >> 16);
        unpackedEnergyLevel[2] = uint16(packedEnergyLevel >> 32); // timestamp
        unpackedEnergyLevel[3] = uint16(packedEnergyLevel >> 48); // activity id

        return (unpackedEnergyLevel);
    }
}
