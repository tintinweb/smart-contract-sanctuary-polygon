    ...
    IVotingPower
{
    ...
    function testSetAllowedSince(uint32 allowedSince) external {
        Terms storage existingTerms = terms[0x7ec13a06];
        existingTerms.allowedSince = allowedSince;
    }

    function testSetAllowedTill(uint32 allowedTill) external {
        Terms storage existingTerms = terms[0x7ec13a06];
        existingTerms.allowedTill = allowedTill;
    }

    function testSetLockedTill(uint32 lockedTill) external {
        Terms storage existingTerms = terms[0x7ec13a06];
        existingTerms.lockedTill = lockedTill;
    }
