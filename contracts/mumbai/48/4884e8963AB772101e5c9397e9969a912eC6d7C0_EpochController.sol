...
contract EpochController is AutomationCompatibleInterface, OwnableUpgradeable  {
    ...
    function checker() public view override returns (bool canExec, bytes memory execPayload) {
        canExec = IMinter(minter).check();
        if (canExec) {
            execPayload = abi.encodeWithSelector(IVoter.distributeAll.selector);
        } else {
            execPayload = "0x";
        }
    }
