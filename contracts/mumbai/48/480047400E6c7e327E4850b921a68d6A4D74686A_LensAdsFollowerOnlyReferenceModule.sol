    ...
    FeeModuleBase
{
    ...
    function initializeReferenceModule(
        ...
        if (!_currencyWhitelisted(_currency)) revert('Errors.currency()');
        if (postOwner == address(0)) revert('Errors.what?');
        if (_budget == 0) revert('Errors.budget0()');
        if (_budget > s_profileToGlobalBudget[profileId][_currency])
            revert('Errors.toolitlebudget()');
        if (_reward == 0) revert('Errors.reward0()');
    ...
    function processMirror(
        uint256 profileId, // el q hace el mirror
        uint256 profileIdPointed, // la marca
        ...
        if (s_followerHasMirrored[profileId][pubIdPointed] == true) {
            revert('LensAds__FollowerAlreadyMirrored()');
        }
    ...
    function withdrawBudget(address _currency, uint256 _profileId) public {
        if (IERC721(HUB).ownerOf(_profileId) == msg.sender) {
            IERC20(_currency).safeTransferFrom(
                address(this),
                msg.sender,
                s_profileToGlobalBudget[_profileId][_currency]
            );
            s_profileToGlobalBudget[_profileId][_currency] = 0;
        } else {
            revert('LensAds__NotSenderProfile()');
        }
    }
