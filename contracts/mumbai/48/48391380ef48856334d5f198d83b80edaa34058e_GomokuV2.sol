...
contract Gomoku  is Initializable, UUPSUpgradeable, OwnableUpgradeable{
   ...
   function _authorizeUpgrade(address) internal override onlyOwner {}
    ...
    function createGame(uint8 availableStatus) public returns(uint256)
    {
        ...
        uint256 gameKey;
        //キーの作成
        gameKey = gomokuMath.createGameHash(gameOwnerMap[msg.sender].nonce, msg.sender);
        //nonce加算
...
contract GomokuMath 
{
    ...
    uint8 maxColumns;
    uint8 maxRows;
    function createGameHash(uint256 nonce,address ownerAddr) public pure returns(uint256)
    {
...
contract GomokuV2  is Gomoku {
    ...
    function getVersionGomoku() public pure returns(uint256)
    {
        return 2;
    }

    function surrender(uint256 gamekey) public
    {
        require(wholeGame[gamekey].player1Addr == msg.sender || wholeGame[gamekey].player2Addr == msg.sender,"you are not entry");

        if(wholeGame[gamekey].player1Addr == msg.sender)
        {
            wholeGame[gamekey].endFlg = 3;
            wholeGame[gamekey].winnerAddr = wholeGame[gamekey].player2Addr; 
        }
        else
        {
            wholeGame[gamekey].endFlg = 1;
            wholeGame[gamekey].winnerAddr = wholeGame[gamekey].player1Addr;   
        }
        emit gameEndEvent(gamekey,wholeGame[gamekey].endFlg,wholeGame[gamekey].winnerAddr);
    }
}