    ...
    AccessControl
{
    ...
    mapping(uint256 => address) public loanIDBank;
    ...
    function setUSDCWallet(address _wallet)
        external
        validation(_wallet)
    ...
    function deployPool(address _owner, address _bank) internal {
    ...
    function mintLoanNft(
        ...
        if (!_details.isRefinance) {
            ...
            loanIDBank[_details.loanID] = _details.bank;
            ...
            prepaidPostpaid(_details);
            borrowerBank[_details.borrower][_details.loanID] = _details.bank;
            ...
            // change 1234 to refinanceloanId

            NFT.freezeNFT(_details.refinanceLoanID);
            NFT.mint(_to, _tokenId, _tokenURI, _details);
            prepaidPostpaid(_details);
            borrowerBank[_details.borrower][_details.loanID] = _details.bank;

            if (
                borrowerBank[_details.borrower][_details.refinanceLoanID] ==
                _details.bank
            ) {
                IPool(pools[_details.bank]).transferStakedAmount(
                    _details.borrower,
                    _details.bank,
                    _details.refinanceLoanID
                );
            } else {
                IPool(
                    pools[
    ...
    function prepaidPostpaid(LoanLib.Loan memory _details) internal {
        if (_details.isPrepaid) {
            Paza.transferFrom(
                _details.bank,
                foundationWallet,
                _details.foundationShare
            );
            Paza.transferFrom(
                _details.bank,
                pools[_details.bank],
            ...
            Paza.buyWithApproval(
                USDCwallet,
                address(this),
                _details.foundationShare + _details.borrowerShare
            );
            Paza.safeTransfer(pools[_details.bank], _details.borrowerShare);
            Paza.safeTransfer(foundationWallet, _details.foundationShare);

            // Paza.mint(USDCwallet, _details.foundationShare);
            // Paza.mint(pools[_details.bank], _details.borrowerShare);
        }
    }
...
interface IController is IAccessControl {
    ...
    function Paza() external view returns(IPaza);
    function NFT() external view returns(ILoanNFT);
    function foundation() external view returns(address);

    function deployPool(address) external;
    function mintLoanNft(address, uint, string memory, LoanLib.Loan memory) external;
    function whitelistBank(address, ControllerLib.bankInfo memory) external;
    function checkBankInfo(address) view external returns(ControllerLib.bankInfo memory);
}
...
interface IPaza is IERC20, IAccessControl {
       function mint(address to, uint256 amount) external;
       function buyWithApproval(address _payee, address _beneficiary, uint _amount)external;
}
...
interface IFactory{
   ...
   function setUSDCWallet(address _wallet) external;  
