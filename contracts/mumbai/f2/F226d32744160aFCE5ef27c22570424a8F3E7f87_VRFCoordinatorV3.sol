...
// File contracts/VRFCoordinatorV3.sol
...
contract VRFCoordinatorV3 is VRF, ConfirmedOwner, TypeAndVersionInterface, VRFCoordinatorV3Interface {
  ...
  constructor(address blockhashStore) ConfirmedOwner(msg.sender) {
    BLOCKHASH_STORE = BlockhashStoreInterface(blockhashStore);
  }

  /**
   * @notice Registers a proving key to an oracle. Todo 向预言机注册一个证明密钥
   * @param oracle address of the oracle Todo 预言机的预言机地址，即传输随机数地址
   * @param publicProvingKey key that oracle can use to submit vrf fulfillments Todo 可用于提交的公共私钥
   */
   ...
   * @param minimumRequestConfirmations global min for request confirmations Todo 您可以定义在您的应用程序发出请求交易时，在生成可验证随机性并在链上交付之前必须通过多少块确认。范围是从 3 到 200 个块。
   * @param maxGasLimit global max for request gas limit Todo 允许您在智能合约应用程序收到可验证的随机性时调整回调气体限制。消费合约可以在接收随机值的回调请求函数中执行更复杂的逻辑。涉及交付随机性的任务在响应过程中处理。
   */
  function setConfig(uint16 minimumRequestConfirmations, uint32 maxGasLimit) external onlyOwner {
  ...
  function getConfig() external view returns (uint16 minimumRequestConfirmations, uint32 maxGasLimit) {
    return (s_config.minimumRequestConfirmations, s_config.maxGasLimit);
  }

  /**
   * @inheritdoc VRFCoordinatorV3Interface
   */
  ...
  function requestRandomWords(
    ...
    // Initialize the nonce to 0, indicating the consumer is allocated.
    ...
    (uint256 requestId, uint256 preSeed) = computeRequestId(keyHash, msg.sender, nonce);
  ...
  function computeRequestId(
    ...
    uint256 preSeed = uint256(keccak256(abi.encode(keyHash, sender, nonce)));
    // Todo 32字节编码补齐后使用keccak256哈希算法
  ...
  function getRandomnessFromProof(
    ...
    if (
      commitment != keccak256(abi.encode(requestId, rc.blockNum, rc.callbackGasLimit, rc.numWords, rc.sender))
    ) {
