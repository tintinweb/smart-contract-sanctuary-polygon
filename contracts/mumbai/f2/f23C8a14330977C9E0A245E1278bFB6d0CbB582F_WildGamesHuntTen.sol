...
contract WildGamesHuntTen is Ownable {
    ...
    constructor(address _paymentToken, address _vaultAddress) {
        paymentToken = IERC20(_paymentToken); // 0xc2132D05D31c914a87C6611C10748AEb04B58e8F - polygon USDT
    ...
    function enterinGame (uint _indexGame, uint256 _playerNote) public {
        ...
        console.log(currentGame.extraGameFundCounter);
        console.log(amountGamesUntilExtraGame);
        console.log(currentGame.extraGameFundCounter % amountGamesUntilExtraGame == 0);
 ...
 ////////////////////////////////////////////
    ...
    function drawProcess(uint _indexGame) internal {
        ...
        paymentToken.transfer(wildGamesVault, (currentGame.betValue / 10)); //10%
    ...
    function random(uint _value, uint _indexGame) internal view returns(uint){
        Game memory currentGame = AllGames[_indexGame];
        return uint(keccak256(abi.encodePacked(block.timestamp,block.difficulty,blockhash(block.number), currentGame.playerNotes, msg.sender))) % _value; //11 + add -1 to block number
    }
}
