...
import "../token/StakeNoteToken.sol";
...
contract StakeNote is Ownable, ReentrancyGuard {
    ...
    *   @dev Emitted when a new stake is issued
     */
    event NewStake(uint256 stakeAmount, uint256 _stakeID, address upline);
    ...
    *   @dev Emitted when a new stake is withdrawed
    ...
    uint256 public intrestValue;
    StakeNoteToken private stakenotetoken;

    bool public isPaused = false;
    uint256 private constant _DECIMALS = 18;
	uint256 public total_users;
	uint256 public total_directbonus;
	uint8[] public direct_bonuses;
    // uint256 private constant _INTEREST_PERIOD = 1 hours;    //One Hours
    uint256 private constant _INTEREST_PERIOD = 10 minutes;    //10 Mins
    ...
    constructor(address _tokenAddress, uint256 _intrestValue) {
        
        intrestValue 		= 	_intrestValue * (10**_DECIMALS);
        stakenotetoken 		= 	StakeNoteToken(_tokenAddress);
		
		direct_bonuses.push(10);
        direct_bonuses.push(5);
    }
	
	function setMinStakeSmount(uint256 MIN_STAKE_AMOUNT) external onlyAuthorized {
        _MIN_STAKE_AMOUNT = MIN_STAKE_AMOUNT * (10**_DECIMALS);
    }
	
	function setIntrestValue(uint256 _intrestValue) external onlyAuthorized {
        intrestValue = _intrestValue * (10**_DECIMALS);
    }
	
	function pauseEverything(bool pausestatus) external onlyAuthorized() {
        isPaused	=	pausestatus;
    }
	
	
	/**
     * @dev Update Tokonization contract address
     * Can only be called by the current owner.
     */
    function setOwner2(address _owner2) public virtual onlyAuthorized {
        require(_owner2 != address(0), "Ownable: is the zero address");
        require(!_owner2.isContract(), "Only Contract Address Allowed");
    ...
    function _setUpline(address _addr, address _upline) private {
		...
		if(_upline!=owner()){
			if(users[_addr].upline == address(0) && stake[_upline].length > 0 && _upline != _addr && _addr != owner()) {
				users[_addr].upline = _upline;
				total_users++;
			}
		}else if(users[_addr].upline == address(0) && _upline != _addr && _addr != owner()) {
			users[_addr].upline = _upline;
			total_users++;
		}
		require(users[_addr].upline != address(0), "Invalid Upline2");
    }
    ...
    *   @param _amount Amoun to stake
     */
    function stakeToken(uint256 _amount, address _upline) external nonReentrant {
		
		
        address staker = _msgSender();
        require(!isPaused, "System Paused");
        require(!_upline.isContract(), "Invalid Upline1");
        require(_amount >= _MIN_STAKE_AMOUNT, "You must stake at least 1 tokens");
        require(stakenotetoken.balanceOf(_msgSender())>=_amount, "You do not have enough balance");
		
		_setUpline(_msgSender(), _upline);
		
        require(users[staker].upline != address(0), "Invalid Upline2");
        ...
        newStake.interest = intrestValue;
        stake[staker].push(newStake);
		_total_token_staked	+=	_amount;	
        if(stakenotetoken.transferFrom(_msgSender(), address(this), _amount)){
            emit NewStake(_amount, stake[staker].length-1, users[staker].upline);
        }else{
            revert("Unable to transfer funds");
        }
		console.log("upline: ");
		console.log(users[staker].upline);
		if(users[staker].upline != address(0)) { 
			address up = users[staker].upline;
			uint8 i=0;
			
			
			while(i < direct_bonuses.length && up != address(0)){
				console.log(stake[up].length);
				if(stake[up].length > 0){
					
					uint256 directbonus 			=	_amount.mul(direct_bonuses[i]).div(100);
					total_directbonus 				= 	total_directbonus.add(directbonus);
					stakenotetoken.transfer(up, directbonus);
					users[up].total_direct_bonus 	= 	users[up].total_direct_bonus.add(directbonus);
					i++;
				}
				up = users[up].upline;
			}
    ...
    *   @param _stakeID reInvest from stake reward
    ...
    function reInvest(uint256 _stakeID) external nonReentrant {
		...
		emit NewStake(rewardToWithdraw, stake[staker].length-1, users[staker].upline);
    }
    ...
    function withdrawReward(uint256 _stakeID) public nonReentrant returns (bool){
        ...
        if(stakenotetoken.transfer(_account, rewardToWithdraw)){
    ...
    *   @param _stakeID The ID of the stake to be returned
    ...
    function unStake(uint256 _stakeID) external nonReentrant returns (bool){
        ...
        if(stakenotetoken.transfer(_msgSender(), deposited_amount + rewardToWithdraw)){
    ...
    * @return Number of active stake
    ...
    function calculateRewardToWithdraw(address _account, uint256 _stakeID) internal view returns (uint256){
		...
		if(!_stake.returned){
			...
			uint256 total_interest 	= interest.div(1e18).mul(periods);
    ...
    function calculateCompoundInterest(uint256 _stakeID) external view returns (uint256){
        ...
        for(uint256 i = 0; i < periods; i++){
            ...
            amount_staked = amount_staked.add(period_interest.div(1e18));
        }
	...
	function getBalance() public view returns (uint256){
        return stakenotetoken.balanceOf(address(this));
    }
}
...
contract StakeNoteToken is Ownable, ERC20 {
	
	constructor(uint256 initialSupply) ERC20("StakeNoteToken", "SNT") {
