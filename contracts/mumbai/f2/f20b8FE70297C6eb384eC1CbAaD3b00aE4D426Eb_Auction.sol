...
contract Auction is
    ...
    */
    ...
    function executeLazyAuction(
        ...
        IGlipERC721Lazy(_bid.token).mintAndTransferEncodedData(_bid.tokenData);
        ...
        value = ((value.sub(auctioneerTake)).sub(minterTake)).sub(platformTake);

        for (uint256 i = 0; i < _tokenData.payouts.length; i++) {
            stake[_tokenData.payouts[i].account] = stake[_tokenData.payouts[i].account].add((value.mul(_tokenData.payouts[i].value)).div(100));
        }
...
interface IGlipERC721Lazy is IGlipERC721LazyData, IERC721Upgradeable {
    ...
    function decodeLazyMintData(bytes calldata encoded) external virtual view returns(DecodedMintData memory);

    function mintAndTransferEncodedData(bytes memory encoded) external;
