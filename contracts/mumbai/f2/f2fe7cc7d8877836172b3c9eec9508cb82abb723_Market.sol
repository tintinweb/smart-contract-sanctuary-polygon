    ...
    ReentrancyGuardUpgradeable
{
    ...
    modifier onlyMarketOwnerRole() {
        if (!MarketRegistry(_marketRegistry).hasMarketOwnerRole(_msgSender())) {
     ...
     * @param trustedForwarder_ (address) Trusted Forwarder address
    ...
    function pause() public onlyOwner onlyMarketOwnerRole {
        _pause();
    }

    function unpause() public onlyOwner onlyMarketOwnerRole {
        _unpause();
    }
    ...
    function addPaymentToken(
        ...
        bool native_
    ) external onlyOwner onlyMarketOwnerRole {
    ...
    function createAuction(
        ...
        uint256 weightedStartPriceUSD = getRate(
            startPrice_,
            paymentTokenIndex_
        );
        if (weightedStartPriceUSD < _minAuctionListingPriceUSD) {
            revert AmberfiLib.Market_AuctionCreateFailed(
     ...
     * @param auctionId_ (uint256) Auction ID
     * @param paymentTokenIndex_ (uint256) PaymentToken index
     * @param paymentAmount_ (uint256) Payment amount in PaymentToken
     */
    function buyNow(
        uint256 auctionId_,
        uint256 paymentTokenIndex_,
        ...
        uint256 paymentAmount = msg.value > 0 ? msg.value : paymentAmount_;
        ...
        uint256 weightedPaymentAmountUSD = getRate(
            paymentAmount,
            paymentTokenIndex_
        );
        if (weightedPaymentAmountUSD < auction.priceUSD) {
            revert AmberfiLib.Market_AuctionBuyNowFailed(
    ...
    function redeemNFT(uint256 auctionId_)
        ...
        isValidAuction(auctionId_)
    {
        AmberfiLib.Auction memory auction = _auctions[auctionId_ - 1];
        ...
        isValidAuction(auctionId_)
    {
        ...
        // uint256 serviceFee = (highestBid.bidAmount * getServiceFeePercent()) /
        //     10000;
        ...
        //     for (uint256 i; i < feeTiersLength; ++i) {
        //         if (feeTiers[i].feeRecipient == address(this)) continue;
        //         uint256 fee = (highestBid.bidAmount * feeTiers[i].feeTier) /
        //             10000;
        ...
        onlyOwner
    {
        ...
        for (uint256 i; i < feeTiersLength; ++i) {
            _feeTiers[i] = AmberfiLib.MarketFeeTier(
        ...
        returns (uint256 pricePaymentToken)
    {
        ...
        uint8 aggregatorDecimals = _paymentTokens[paymentTokenIndex_]
            .aggregatorDecimals;
        uint8 tokenDecimals = _paymentTokens[paymentTokenIndex_].tokenDecimals;
        AggregatorV3Interface priceFeed = AggregatorV3Interface(
            _paymentTokens[paymentTokenIndex_].aggregator
        );
        (, int256 price, , , ) = priceFeed.latestRoundData();
        pricePaymentToken =
            (usdAmount_ * 10**(tokenDecimals + aggregatorDecimals - 6)) /
            (uint256)(price);
    }
        ...
        returns (uint256 priceUSD)
    {
        ...
        priceUSD =
            ...
            10**(tokenDecimals + aggregatorDecimals - 6); // Should be always 6 digits
    }
