...
contract Vesting {
    ...
    uint256 constant INITIAL_UNLOCK = 5; // 5% : released percent at TGE stage
    uint256 constant UNLOCK_UNIT = 528; // 5.28% of the total allocation will be unlocked
    uint256 constant CLIFF_PERIOD = 90 days; // cliff period
    uint256 constant INITIAL_LOCK_PERIOD = CLIFF_PERIOD + 30 days; // No tokens will be unlocked for the first 120 days
    ...
    function withdrawToken(address _recipient) external returns (uint256) {
        ...
        require(SBCToken.transfer(_recipient, _withdrawable));
    ...
    function getVested(address beneficiary) public view virtual returns (uint256 _amountVested) {
        ...
        uint256 vestingPeriod = 510 days;
        ...
        if (block.timestamp > firstVestingPoint && block.timestamp <= secondVestingPoint) {
            vestedPercent = INITIAL_UNLOCK.add(UNLOCK_UNIT.div(100)) + (block.timestamp - firstVestingPoint).mul(9500-UNLOCK_UNIT).div(100).div(vestingPeriod);
