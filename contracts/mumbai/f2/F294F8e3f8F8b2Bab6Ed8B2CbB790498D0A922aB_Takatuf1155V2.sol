...
/**
 * @title Takatuf1155 contract.
 * @dev  Takatuf1155 is a implementation contract of both access control contract and ERC1155 contract.
 */
contract Takatuf1155V2 is ERC1155Upgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    address private _takatufMarketplace;
    ...
    modifier isAdmin() {
        if (!hasRole(DEFAULT_ADMIN_ROLE, msg.sender))
            revert GeneralError("TKF:101");
        _;
    }

    modifier isTakatufMarketplace() {
        if (msg.sender != _takatufMarketplace) revert GeneralError("ERR:106");
        _;
    }
    ...
    modifier validatePayouts(
        ...
        if (percentage_.length != receivers_.length)
            revert GeneralError("TKF:201");

        // make sure all revenues and receivers are non zero.
        uint256 sum = 0;
        for (uint256 i = 0; i < percentage_.length; i++) {
            if (percentage_[i] <= 0) revert GeneralError("TKF:202");
            if (receivers_[i] == address(0)) revert GeneralError("TKF:205");
            sum = sum + percentage_[i];
        }

        // make sure if percentage sum less than 100%
        if (sum >= PERCENT_UNIT) revert GeneralError("TKF:203");
        _;
    }
    ...
    /**
     *@notice sets configuration of token with baseUri and address of takatufMarketplace.
     *@dev only admin can update token URI.
     *@param baseUri_ base URI of the token.
     *@param takatufMarketplace_ address of takatufmarketplace contract.
     */
    function setConfiguration(
        ...
        if (takatufMarketplace_ != address(0))
            _takatufMarketplace = takatufMarketplace_;
    }
        ...
        isAdmin
    {
        ...
        ) revert GeneralError("TKF:204");
        if (!exists(tokenId_)) revert GeneralError("TKF:206");
        _nftInfos[tokenId_].uri = uri_;
    }

    //*********************** Getter Functions ***********************//
    /**
     *@notice  returns the configuration with baseUri and address of takatufMarketplace.
     *@return baseUri_ the baseUri of token.
     *@return takatufMarketplace_ address of takatufMarketplace.
     */

    function getConfiguration()
        external
        view
        returns (string memory baseUri_, address takatufMarketplace_)
    {
        baseUri_ = _baseTokenURI;
        takatufMarketplace_ = _takatufMarketplace;
    }
    ...
    function uri(uint256 tokenId) public view override returns (string memory) {
        if (!exists(tokenId)) revert GeneralError("TKF:206");
        return string(abi.encodePacked(_baseTokenURI, _nftInfos[tokenId].uri));
    }
    ...
    function updateRoyalties(
        ...
        if (
            ...
            _nftInfos[tokenId_].minter != msg.sender
        ) revert GeneralError("TKF:107");
    ...
    function transferNft(
        ...
        uint256 tokenId_,
        uint256 quantity_
    ) external isTakatufMarketplace nonReentrant{
        _safeTransferFrom(from_, to_, tokenId_, quantity_, "");
        if (from_ == _nftInfos[tokenId_].minter) {
            uint256 currentTotalSale = _nftInfos[tokenId_].firstSaleQuantity +
                quantity_;
            if (currentTotalSale > _totalSupply[tokenId_]) {
                _nftInfos[tokenId_].firstSaleQuantity = _totalSupply[tokenId_];
            }
        }
    }
     ...
     *@dev Only creator or owner can burn the token otherwise throws an error-TKF:104-'Not creator or Owner'.
