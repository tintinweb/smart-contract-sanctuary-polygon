...
contract Finde {
        ...
        if (compareStringsbyBytes(_country, "") == true && compareStringsbyBytes(_tag, "") == true) {
            ...
            if(tagToUris[_tag].length > 0 && tagToUris[_tag].length <= _limit){
                ...
                for (uint256 i = 0; i < tagToUris[_tag].length; i++) {
                    ...
                    data[dataLength] = Provider({
                        uris: temp_uris,
                        votes: indexToProvider[uriToIndex[tagToUris[_tag][i]]].votes,
                        addr: indexToProvider[uriToIndex[tagToUris[_tag][i]]].addr,
                        country: indexToProvider[uriToIndex[tagToUris[_tag][i]]].country,
                        countryLocalIndex: indexToProvider[uriToIndex[tagToUris[_tag][i]]].countryLocalIndex
                    });
                    dataLength++;
                }
            ...
            if(tagToCountryToUris[_tag][_country].length > 0 ){
                for (uint256 i = _offset; i < tagToCountryToUris[_tag][_country].length; i++) {
                    ...
                    data[dataLength] = Provider({
                        uris: temp_uris,
                        votes: indexToProvider[uriToIndex[tagToCountryToUris[_tag][_country][i]]].votes,
                        addr: indexToProvider[uriToIndex[tagToCountryToUris[_tag][_country][i]]].addr,
                        country: indexToProvider[uriToIndex[tagToCountryToUris[_tag][_country][i]]].country,
                        countryLocalIndex: indexToProvider[uriToIndex[tagToCountryToUris[_tag][_country][i]]].countryLocalIndex
                    });
                    dataLength++;
                }
            }else{
    ...
    function getProviderURIs(
        ...
        require(_limit > 0 && _limit <= 100, "FINDE: Invalid limit.");
        require(indexToProvider[addressToIndex[_addr]].uris.length > 0, "FINDE: Invalid address.");
        require(_offset+_limit <= indexToProvider[addressToIndex[_addr]].uris.length, "FINDE: Invalid offset.");
    ...
    function getRemainingTimeForVote(address _voter) external view returns (int256) {
        int256 timeLeft = int256((voterTimeRecord[_voter] + votingWaitTime) - block.timestamp );
        if (timeLeft >= 0) {
            return timeLeft;
        }
        return 0;
