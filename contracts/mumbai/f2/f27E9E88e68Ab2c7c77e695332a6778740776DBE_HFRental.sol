...
contract HFRental is Ownable {
    ...
    function checkLatest(uint256 tokenId)
        public
        view
        returns (RentOffer memory)
    {
    ...
    function isLive(uint256 tokenId) public view returns (bool) {
        if (totalOffers[tokenId].length > 0) {
            ...
            if (
                endTime == 0 &&
                checkLatest(tokenId).minPrice > 0 &&
                !accepted &&
                (created + 7 days) > block.timestamp
            ) {
                return true;
            }
 ...
 */
    function listOffer(
        ...
        rentOffer[totalOfferred] = RentOffer(
            tokenId,
            totalOfferred,
            IERC721(nftContract).ownerOf(tokenId),
            address(0),
    ...
    function closeBids(uint256 tokenId) public {
        ...
        if (bids[offer.offerId].length > 0) {
            ...
            rentOffer[totalOffers[tokenId][totalOffers[tokenId].length - 1]]
                .borrower = bid.wallet;
            rentOffer[totalOffers[tokenId][totalOffers[tokenId].length - 1]]
    ...
    function getRefund(uint256 amount) external payable {
        require(
            amount <= pendingDeposit[msg.sender],
            "You dont have this much amount"
        );
    ...
    function cancelOffer(uint256 tokenId) external {
        require(
            msg.sender == checkLatest(tokenId).owner ||
                msg.sender == checkLatest(tokenId).borrower,
            "You are not auth"
        );
    ...
    function checkCurrentHolding(address wallet)
        public
        view
        returns (uint256[] memory)
    {
        uint256[] memory tuple = gotOffer[wallet];
        uint256[] memory finaltuple = new uint256[](tuple.length);

        for (uint256 i = 0; i < tuple.length; i++) {
            RentOffer memory trial = checkLatest(tuple[i]);
            if (trial.endTime > block.timestamp) {
