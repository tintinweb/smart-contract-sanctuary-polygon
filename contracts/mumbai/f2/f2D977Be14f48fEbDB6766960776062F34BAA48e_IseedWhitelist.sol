...
contract IseedWhitelist is Ownable, ERC1155 {
    ...
    function mint(string memory _community) public payable {
    ...
    function burnWhitelistForIseed (uint256 _amount, address _user) external {
        require(msg.sender == IseedContractAddress, "User not approved");
        _burn(_user, 1, _amount);
    }
    ...
    function withdraw (address _firstLiquidityPool, uint256 _percentageForFirstLiquidityPool, address _secondLiquidityPool) public onlyOwner {
        payable(_firstLiquidityPool).transfer(address(this).balance * (_percentageForFirstLiquidityPool / 100));
        payable(_secondLiquidityPool).transfer(address(this).balance);
    }
}