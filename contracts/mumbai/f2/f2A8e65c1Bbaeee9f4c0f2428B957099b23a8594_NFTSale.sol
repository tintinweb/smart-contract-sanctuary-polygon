...
contract NFTSale is Ownable, IERC721Receiver, ReentrancyGuard, EIP712Base, BaseRelayRecipient, Pausable {
    ...
    mapping(address => mapping(uint256 => uint256)) public _nftToListingId; // NFT contract Address -> TokenID -> ListingId
    ...
    event ListingAdded(
        ...
        uint256 expiryTime,
        address reservedFor,
        string indexed bundleUUID
    );
    event ListingCancelled(uint256 indexed _listingId, address indexed _owner, uint256 _timestamp, string indexed bundleUUID);
    event Sold(
        uint256  _listingId,
    ...
    function transferWithTreasury(
        ...
        uint256 treasuryFee = (_amount * (treasuryPercentage)) / (100) / (100);
    ...
    function sellNFTBundle(
        ...
        require(_expiryTime > block.timestamp, "Expiry Time cannot be in the past");
        require(_reservedFor != _msgSender(), "Cannot reserve the listing for owner");
        require(!_reservedFor.isContract(), "Cannot reserve the listing for a contract");
     ...
     * @param _listingId - id of the NFT Bundle
     */
    function cancelListingBundle(uint256 _listingId) external nonReentrant whenNotPaused {
     ...
     * @param _listingId - id of the NFT Bundle
     */
    function purchaseNFT(uint256 _listingId, uint256 _amount) external nonReentrant whenNotPaused {
        require(listings[_listingId].expiryTime > block.timestamp, "Listing has expired");
        ...
        if (listings[_listingId].reservedFor != address(0)) {
            require(listings[_listingId].reservedFor == buyer, "This listing is reserved for another user");
        }
        ...
        emit Sold(_listingId, seller, buyer, price, block.timestamp, listings[_listingId].bundleUUID);
    }
    ...
    function acceptOffer(
        ...
        require(listings[listingId].expiryTime > block.timestamp, "Listing has expired");
        ...
        emit Sold(listingId, seller, offerSender, amount, block.timestamp, listings[listingId].bundleUUID);
    }
    ...
    function cancelListing(uint256 _listingId) internal {
        if (listings[_listingId].isActive) {
            ...
            emit ListingCancelled(_listingId, _msgSender(), block.timestamp, listings[_listingId].bundleUUID);
        }
    }

    /**
     * Cancels batch of listings and returns nfts to the user
     */
    function batchCancelListings(uint256[] memory cancelListingIds) external onlyAdmin {
        for (uint256 listingIdIndex = 0; listingIdIndex < cancelListingIds.length; listingIdIndex++) {
            uint256 _listingId = cancelListingIds[listingIdIndex];
            cancelListing(_listingId);
        }
    }

    /**
     * Cancels all of listings and returns nfts to the user
     */
    function cancelAllListings() external onlyAdmin {
        uint256 currentListingCount = _listingIds.current();
        for (uint256 listingIdIndex = 1; listingIdIndex <= currentListingCount; listingIdIndex++) {
            cancelListing(listingIdIndex);
        }
    }
}
