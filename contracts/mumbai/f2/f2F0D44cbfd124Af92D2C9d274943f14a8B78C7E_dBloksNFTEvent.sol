...
contract dBloksNFTEvent is ERC2771Recipient, ERC721URIStorage {
    ...
    function eventTicketMint(address client, string memory tokenURI) public onlyOwner returns (uint256) {
        _tokenIds.increment();

        uint256 eventTicketId = _tokenIds.current();
        _mint(client, eventTicketId);
        _setTokenURI(eventTicketId, tokenURI);
