...
contract StakingContract is Ownable, ERC721Holder {
    ...
    /*
    IERC721 public nft;
    uint256 public nftReward = 1 ether; // 1 DRK Token,
    // uint256 public nftInterval = 45 hours; // 1 Token every 1.875 Days
    uint256 public nftInterval = 1 seconds; // 86400 Token every 1 Days
    */
    ...
    function unstake(address _nftContract, uint256 _tokenId) external {
        ...
        Staking memory targetStaking = getUserIdStaking(msg.sender, _tokenId);
    ...
    function harvestAll() public {
        ...
        for (uint256 i = 0; i < userNftStaked.length; i++) {
            if (userNftStaked[i].stakedAt != 0) {
                ...
                Staking memory targetStaking = getUserIdStaking(
                    msg.sender,
                    _tokenId
                );
    ...
    function getUserIdStaking(address user, uint256 _tokenId)
        public
        view
        returns (Staking memory staking)
    {
