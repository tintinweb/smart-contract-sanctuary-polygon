...
contract USDRExchange is AddressAccessor, IExchange, Pausable {
    ...
    function updateMintingStats(int128[] calldata delta) external onlyRole(DEFAULT_ADMIN_ROLE) {
        mintingStats.tngblToUSDR = uint256(int256(mintingStats.tngblToUSDR) + delta[0]);
        mintingStats.underlyingToUSDR = uint256(int256(mintingStats.underlyingToUSDR) + delta[1]);
        mintingStats.usdrToPromissory = uint256(int256(mintingStats.usdrToPromissory) + delta[2]);
        mintingStats.usdrToTNGBL = uint256(int256(mintingStats.usdrToTNGBL) + delta[3]);
        mintingStats.usdrToUnderlying = uint256(int256(mintingStats.usdrToUnderlying) + delta[4]);
    }

    function maxTNGBLMintingAmount() external view returns (uint256) {
        return _maxTNGBLMintingAmount(addressProvider.getAddress(TNGBL_ORACLE_ADDRESS));
    }
    ...
    function swapFromTNGBL(
        ...
        require(amountIn <= _maxTNGBLMintingAmount(tngblOracle), "amount too high");
    ...
    function _maxTNGBLMintingAmount(address tngblOracle) private view returns (uint256) {
        uint256 minted = mintingStats.underlyingToUSDR;
        uint256 redeemed = mintingStats.usdrToUnderlying + mintingStats.usdrToPromissory + mintingStats.usdrToTNGBL + 10 * mintingStats.tngblToUSDR;
        if (redeemed >= minted) return 0;
        return ((minted - redeemed) * 1e26) / IPriceOracle(tngblOracle).quote(1e18);
    }
