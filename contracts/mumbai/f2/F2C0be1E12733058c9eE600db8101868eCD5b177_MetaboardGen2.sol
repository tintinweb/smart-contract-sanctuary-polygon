...
contract MetaboardGen2 is EnumerableERC721, SameRoyaltiesForAll, Punchable{
    ...
    function setStartingIndex() external onlyOwner {
        ...
        while( startingIndex == 0) {
            startingIndex = uint16(uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp))) % (maxTokens - 1));
        }
    }
...
contract ERC_721 is IERC721, ERC165, Ownable, IERC721Metadata{
    ...
    */
    modifier exists( uint256 _tokenId) {
        require(tokenOwner[_tokenId] != address(0), "Nonexisten NFT");
        _;
    }
    ...
    returns (address _owner)
  {
    ...
    require(_owner != address(0), "Nonexisten NFT");
  }
