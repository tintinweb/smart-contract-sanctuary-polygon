...
contract SkillsInterface {
    ...
    uint8[MAX_SKILLS][NUMBER_OF_CLASSES] public validSkillsForClass = [
     ...
     function rollSkillsForPlayer(uint256 entropy, uint8 classId, uint8 tier) public view returns (uint8[] memory skillsToRoll) {
        bool done = false;
        uint8[6] memory rolledSkills = [0, 0, 0, 0, 0, 0];
        uint8 seed = 1;
        uint8 skillIndex = 0;
        uint8[MAX_SKILLS] memory globalSkillIndexes = validSkillsForClass[classId];
        rolledSkills[0] = uint8(globalSkillIndexes[deriveRandomnessFromEntropy(entropy, seed) % MAX_SKILLS]);
        skillIndex++;
         for(uint8 index = 1; index <= tier + 1; index++) {
            seed++;
               while(rolledSkills[index] == rolledSkills[index - 1]) {
                  uint8 newDerivedRandomness = uint8(deriveRandomnessFromEntropy(entropy, seed * index));
                  rolledSkills[index] = uint8(globalSkillIndexes[newDerivedRandomness % MAX_SKILLS]);
                  seed++;
               }
         }
     }
