...
import { KnightStorage as KNHT, knightType, Knight, KnightModifiers} from "../storage/KnightStorage.sol";
...
contract DemoFightFacet is KnightModifiers {
  ...
  function getStakeTotal() public view returns(uint256 totalStake){
    (totalStake, , , , , ) = META.AAVE().getUserAccountData(address(this));
    totalStake /= 100;
  //totalStake = AAVEUSDT.balanceOf(address(this));
  }

  function setCoinAddress(address Coin) public {
    META.layout().USDT = IERC20(Coin);
  }

  function getStakeByKnights() public view returns(uint256 knightsStake){
    knightsStake = KNHT.knightPrice(knightType.AAVE) * ITEM.totalKnightSupply();
  }

  function getCurrentReward() public view returns(uint256 reward){
    reward = getStakeTotal() - getStakeByKnights() - lockedUntilClaimed();
  }

  function battleWonBy(uint winnerId) public isKnight(winnerId) {
    uint256 reward = getCurrentReward();
    DemoFightStorage.layout().knightRewards[winnerId] += reward;
    DemoFightStorage.layout().lockedUntilClaimed += reward;
    emit NewWinner(winnerId, reward);
  }

  function claimReward(uint knightId) public {
    uint256 reward = knightRewards(knightId);
    DemoFightStorage.layout().lockedUntilClaimed -= reward;
    DemoFightStorage.layout().knightRewards[knightId] = 0;
    META.AAVE().withdraw(
      address(META.USDT()),
      reward,
      KNHT.knightOwner(knightId)
    );
    emit RewardClaimed(knightId, knightRewards(knightId));
  }

  function knightRewards(uint256 knightId) public view isKnight(knightId) returns(uint256) {
    return DemoFightStorage.knightRewards(knightId);
  }

  function lockedUntilClaimed() public view returns(uint256) {
    return DemoFightStorage.lockedUntilClaimed();
  }
...
library DemoFightStorage {
  struct Layout {
    mapping (uint256 => uint256) knightRewards;
    uint256 lockedUntilClaimed;
  }
  ...
  function knightRewards(uint256 knightId) internal view returns (uint256) {
    return layout().knightRewards[knightId];
  }
  
  function lockedUntilClaimed() internal view returns (uint256) {
    return layout().lockedUntilClaimed;
  }
}
