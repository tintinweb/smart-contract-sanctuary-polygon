...
contract PlushController is Ownable {
    ...
    function getAvailableBalanceForWithdrawalToController() public view returns (uint256)
    {
        return plushCoinWallets.getWalletAmount(address(this));
    }

    function getAvailableBalanceForWithdrawalFromController() public view returns (uint256)
    {
        return address(this).balance;
    }

    function withdrawalToControllerFromPlush() external onlyOwner
    {
        require(getAvailableBalanceForWithdrawalToController() > 0, 'No funds available for withdrawal.');

        plushCoinWallets.withdraw(getAvailableBalanceForWithdrawalToController());
    }
    ...
    function withdrawalToUser(address _address, uint256 _amount) external onlyOwner
    {
        ...
        require(getAvailableBalanceForWithdrawalFromController() >= _amount, 'Not enough balance.');
