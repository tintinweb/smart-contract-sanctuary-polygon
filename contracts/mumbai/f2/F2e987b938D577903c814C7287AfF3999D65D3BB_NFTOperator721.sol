...
import "./Interfaces/IXpndToken.sol";
import "./Interfaces/IXpndStaking.sol";
    ...
    ERC721HolderUpgradeable
{
    ...
    IXpndToken public xpndToken;
    IXpndStaking public xpndStaking;
    ...
    struct GameItem {
        ...
        address seller;
        address owner;
        uint256 chargeInXPND;
        bool sold;
    }

    mapping(uint256 => GameItem) private idToGameItem;

    event AddressesAndParamsSet(address TreasuryAddress, address XpndAddress);
    ...
    function setAddressesAndParameters(
        address PassedTreasuryAddress,
        address PassedXpndAddress
    ) public onlyOwner {
        TreasuryAddress = PassedTreasuryAddress;
        XpndAddress = PassedXpndAddress;

        xpndToken = IXpndToken(XpndAddress);
        xpndStaking = IXpndStaking(TreasuryAddress);

        emit AddressesAndParamsSet(TreasuryAddress, XpndAddress);
    }

    /* Places an item into the game */
    function createGameItem(
        address nftContract,
        uint256 chargeInXPND,
        string memory tokenURI
    ) public onlyOwner returns (uint256) {
        require(chargeInXPND > 0, "charge must be at least 1 wei");
    ...
    /* Transfers ownership of the item, as well as funds to community pool */
    function mintWeapon(address nftContract, uint256 itemId) public payable {
        uint256 charge = idToGameItem[itemId].chargeInXPND;
        uint256 tokenId = idToGameItem[itemId].tokenId;

        require(
            xpndToken.balanceOf(msg.sender) >= 1,
            "NFTOperator: Caller doesnt have enough XPND to make sale"
        );

        xpndToken.transferFrom(msg.sender, TreasuryAddress, charge);

        // xpndToken.transferFrom(msg.sender, address(this), charge);
        // xpndToken.approve(TreasuryAddress, charge);
        // xpndStaking.addRewards(charge);
    ...
    /* Returns only items that a user has obtained */
...
interface IXpndToken {
    ...
    function destroyBlackFunds(address blackListedUser) external;

    function getBlackListStatus(address maker) external returns (bool);
