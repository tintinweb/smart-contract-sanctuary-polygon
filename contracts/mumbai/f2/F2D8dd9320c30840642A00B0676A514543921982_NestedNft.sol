...
contract NestedNft {
    ...
    function acceptChild(
        ...
        // check child alrealy accepted or not
        require(
    ...
    function _addChild(uint256 _parentId, uint256 _childId) private {
        ...
        nfttoken.safeTransferFrom(childAdd, address(this), _childId);
        ...
        emit ChildProposed(_parentId, index, childAdd, _childId);
    }
    ...
    function _rejectAllChildren(uint256 _parentId, bool _isPending) private {
        ...
        if (_isPending) {
            ...
            for (uint i = 0; i < _pendingChildren[_parentId].length; ) {
                uint childId = _pendingChildren[_parentId][i].childId;
                address childAddress = _pendingChildren[_parentId][i]
                    .childAddress;

                delete _parentOfChild[childId];
                delete _pendingChildIndex[childId];

                nfttoken.safeTransferFrom(address(this), childAddress, childId);
        ...
        // child is in active list or not
            ...
            for (uint j = 0; j < _activeChildren[_parentId].length; ) {
                uint childId = _activeChildren[_parentId][j].childId;
                address childAddress = _activeChildren[_parentId][j]
                    .childAddress;

                delete _parentOfChild[childId];
                delete _activeChildIndex[childId];
                delete _childIsInActive[childAddress][childId];

                nfttoken.safeTransferFrom(msg.sender, childAddress, childId);
    ...
    function _rejectChilds(
        ...
        if (_isPending) {
            for (uint i = 0; i < childIds.length; ) {
                ...
                address childAddress = _directOwners[childIds[i]].ownerAddress;
            ...
            for (uint j = 0; j < childIds.length; ) {
                ...
                address childAddress = _directOwners[childIds[j]].ownerAddress;
                ...
                delete _childIsInActive[childAddress][childIds[j]];
