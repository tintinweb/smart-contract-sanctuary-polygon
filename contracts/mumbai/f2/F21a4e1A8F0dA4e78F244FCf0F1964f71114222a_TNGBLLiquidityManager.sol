...
contract TNGBLLiquidityManager is AddressAccessor, Pausable {
    ...
    function increaseLiquidity(uint256 underlyingAmount)
        ...
        whenNotPaused
    {
        (address underlying, address tngbl, address treasury) = abi.decode(
            addressProvider.getAddresses(
                abi.encode(UNDERLYING_ADDRESS, TNGBL_ADDRESS, TREASURY_ADDRESS)
            ),
        ...
        uint256 tngblAmount;
        {
            IUniswapV2Pair pair = IUniswapV2Pair(
                IUniswapV2Factory(SUSHI_FACTORY).getPair(underlying, tngbl)
            );
            ...
            tngblAmount = (underlyingAmount * reserveB) / reserveA;
        }
        ...
        IERC20(tngbl).transferFrom(treasury, address(this), tngblAmount);
    ...
    function missingLiquidity() external view returns (uint256) {
        ...
        uint256 minSize = IERC20(usdr).totalSupply() * 1e7 * _minSizePercent;
        uint256 liquidity_ = liquidity();
        return
            liquidity_ < minSize
                ? (minSize - liquidity_) / (10**(18 - underlyingDecimals))
                : 0;
    }
    ...
    function withdrawLPToken() external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        address pool = IUniswapV2Factory(SUSHI_FACTORY).getPair(
            underlying,
            tngbl
        );
    ...
    function liquidity() public view returns (uint256) {
        (address underlying, address tngbl, address tngblOracle) = abi.decode(
            addressProvider.getAddresses(
                abi.encode(
                    UNDERLYING_ADDRESS,
                    TNGBL_ADDRESS,
        ...
        uint8 underlyingDecimals = IERC20Metadata(underlying).decimals();
        uint256 underlyingBalance = IERC20(underlying).balanceOf(pool) *
            (10**(18 - underlyingDecimals));
        uint256 tngblBalance = (IERC20(tngbl).balanceOf(pool) * 1e18) /
            IPriceOracle(tngblOracle).quote(1e18);
        return
            ((underlyingBalance + tngblBalance) *
