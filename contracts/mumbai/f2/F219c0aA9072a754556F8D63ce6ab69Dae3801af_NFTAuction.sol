...
contract NFTAuction {
    ...
    uint32 public defaultAuctionMinPeriod;
    ...
    event AuctionSettled(address nftContractAddress, uint256 tokenId, address auctionSettler, address winner);

    event AuctionStoped(address nftContractAddress, uint256 tokenId, uint256 auctionEndPeriod);
    ...
    constructor(address _artist) {
        ...
        defaultAuctionMinPeriod = 259200; //3 days
        minimumSettableIncreasePercentage = 100;

        defaultFeeRecipients = [_artist];
        defaultFeePercentages = [500];
    }
    ...
    function _doesBidMeetBidRequirements(
        ...
        //the bid needs to be a % higher than the previous bid
        uint256 payValueToIncrease = nftContractAuctions[_nftContractAddress][_tokenId].nftHighestBid;
        if (payValueToIncrease == 0) {
            payValueToIncrease = nftContractAuctions[_nftContractAddress][_tokenId].minPrice;
        }

        uint256 bidIncreaseAmount = (payValueToIncrease *
     ...
     * (1) Auction already created - can accept MATIC or Specified Token
     *  --------> Cannot bid with MATIC & an ERC20 Token together in any circumstance<------
     * (2) Auction not created - only MATIC accepted (cannot early bid with an ERC20 Token
     * (3) Cannot make a zero bid (no MATIC or Token amount)
     */
    ...
    function _getAuctionMinDuration(address _nftContractAddress, uint256 _tokenId) internal view returns (uint64) {
        uint64 auctionMinDuration = nftContractAuctions[_nftContractAddress][_tokenId].auctionMinDuration;
        ...
        isFeePercentagesLessThanMaximum(_feePercentages)
    {
        ...
        nftContractAuctions[_nftContractAddress][_tokenId].nftSeller = msg.sender;
        nftContractAuctions[_nftContractAddress][_tokenId].auctionStart = _auctionStart;
    }
        ...
        increasePercentageAboveMinimum(_bidIncreasePercentage)
    {
        ...
        nftContractAuctions[_nftContractAddress][_tokenId].auctionMinDuration = _auctionMinDuration;
    ...
    ) external payable auctionOngoing(_nftContractAddress, _tokenId) notZeroAddress(_nftRecipient) {
    ...
    function _updateAuctionEnd(address _nftContractAddress, uint256 _tokenId) internal {
        uint256 currentAuctionEnd;

        if (nftContractAuctions[_nftContractAddress][_tokenId].auctionEnd > 0) {
            currentAuctionEnd = nftContractAuctions[_nftContractAddress][_tokenId].auctionEnd;
        } else {
            currentAuctionEnd =
                nftContractAuctions[_nftContractAddress][_tokenId].auctionStart +
                _getAuctionMinDuration(_nftContractAddress, _tokenId);
        }

        uint256 timeLeft = currentAuctionEnd - uint256(block.timestamp);

        if (_getAuctionBidPeriod(_nftContractAddress, _tokenId) > timeLeft) {
            uint32 prolongation = uint32(_getAuctionBidPeriod(_nftContractAddress, _tokenId) - timeLeft);

            nftContractAuctions[_nftContractAddress][_tokenId].auctionEnd = uint64(currentAuctionEnd + prolongation);
            ...
            nftContractAuctions[_nftContractAddress][_tokenId].auctionEnd = uint64(currentAuctionEnd);
        }
    }
        ...
        isAuctionOver(_nftContractAddress, _tokenId)
    {
        ...
        emit AuctionSettled(_nftContractAddress, _tokenId, msg.sender, highestBidder);
    }

    /*
     * The NFT seller can opt to end an auction.
     */
    function stopAuction(address _nftContractAddress, uint256 _tokenId)
        external
        onlyNftSeller(_nftContractAddress, _tokenId)
    {
        require(!_isABidMade(_nftContractAddress, _tokenId), 'cannot stop auction with bids');
        require(
            nftContractAuctions[_nftContractAddress][_tokenId].auctionMinDuration +
                nftContractAuctions[_nftContractAddress][_tokenId].auctionStart <
                block.timestamp,
            'cannot stop auction when min period does not pass'
        );
        ...
        emit AuctionStoped(_nftContractAddress, _tokenId, block.timestamp);
    }
