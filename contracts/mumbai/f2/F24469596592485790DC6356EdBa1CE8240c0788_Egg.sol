...
contract Egg is ERC4907A, SignatureCheck, DefaultOperatorFilterer, LockRegistry, AccessControl {
    
    bytes4 private constant _INTERFACE_ID_ERC721x = 0x706e8489;

    bytes32 public constant BURN_TYPEHASH =
        keccak256(
            "Burn(address owner,uint256 tokenId,uint256 nonce,uint256 deadline)"
        );
    ...
    event Stake(uint256 tokenId, address owner, uint256 stakedAt);
    event Unstake(
        uint256 tokenId,
        address owner,
        uint256 stakedAt,
    ...
    function airdrop(address[] memory receivers, uint256[] memory quantities) external onlyOwner {
        uint256 count = receivers.length;
        require(count > 0 && count == quantities.length, "Invalid receivers and quantities length");
    ...
    function claim(
        ...
        bytes32 hashStruct = keccak256(
            abi.encode(CLAIM_TYPEHASH, sender, quantity, nonce, deadline)
        );

        require(verifySignature(admin, hashStruct, v, r, s), "Wrong signature");
        safeMint(sender, quantity);
    }

    // =============== staking ===============
    function setStakeStatus(bool status) external onlyOwner {
        canStake = status;
    }
    ...
    function stake(uint256 tokenId) public {
        ...
        require(
            _msgSender() == ERC721A.ownerOf(tokenId),
            "Caller must be owner of token"
        );
    ...
    function unstake(uint256 tokenId) public {
        address sender = _msgSender();
        require(
            sender == ERC721A.ownerOf(tokenId),
            "Caller must be owner of token"
        );
    ...
    // =============== ERC721x ===============
    function supportsInterface(bytes4 _interfaceId) public view virtual override(ERC4907A, AccessControl) returns (bool) {
    ...
    function adminBurn(
        ...
        address tokenOwner = ERC721A.ownerOf(tokenId);
        uint256 nonce = nonces[tokenOwner]++;
        bytes32 hashStruct = keccak256(
            abi.encode(BURN_TYPEHASH, tokenOwner, tokenId, nonce, deadline)
        );

        require(verifySignature(tokenOwner, hashStruct, v, r, s), "Wrong signature");
    ...
    function burn(
        ...
        address tokenOwner = ERC721A.ownerOf(tokenId);
        require(_msgSender() == tokenOwner, "Not token owner");
