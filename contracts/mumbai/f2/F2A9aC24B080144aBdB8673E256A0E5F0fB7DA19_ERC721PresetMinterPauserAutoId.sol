    ...
    MintPageFee
{
    ...
    uint128 public constant MIN_PRICE = 10**13;
    uint128 public constant MAX_PRICE = 10**25;
    ...
    function mintRandom(
        ...
        require(
            msg.value >= MIN_PRICE && msg.value <= MAX_PRICE,
            "ERC721PresetMinterPauserAutoId:Insufficient balance. Please try again"
        );
        uint256 totalAmount = msg.value;
        _tokenIdTracker.increment();
        _mint(msg.sender, _tokenIdTracker.current());
        _setTokenURI(_tokenIdTracker.current(), _tokenURI);
        _updateTokenCreator(_tokenIdTracker.current(), payable(msg.sender));
        _saveRoyalties(_tokenIdTracker.current(), _royalties);
        address treasuryWallet = getCommunityWallet();
        _distributeFee(treasuryWallet, _royalties[0].account, totalAmount);
    }

    /**
     * @dev Creates tokens for user who call the function. User can mint up to 100 tokens per once.
     * Tokens IDs will be automatically assigned
    ...
    function mintBatchRandom(
        string[] memory _tokenURIs,
        LibPart.Part[] memory _royalties
    ) public payable nonReentrant {
        require(
            _tokenURIs.length <= 100,
            "ERC721PresetMinterPauserAutoId: No more than 100 mint per Once"
        );
        require(
            msg.value >= MIN_PRICE && msg.value <= MAX_PRICE,
            "ERC721PresetMinterPauserAutoId:Insufficient balance. Please try again"
...
// SPDX-License-Identifier: MIT
import "./exchange/SendValueWithFallbackWithdraw.sol";
import "./access/AccessControlEnumerable.sol";
import "./exchange/SafeMathUpgradeable.sol";

pragma solidity ^0.8.0;

contract MintPageFee is SendValueWithFallbackWithdraw, AccessControlEnumerable {
    event DistributedFee(
        address indexed treasuryFee,
        uint256 feeAmount,
        address indexed creatoObtained,
        uint256 feeForCreator
    );
    using SafeMathUpgradeable for uint256;

    uint256 public constant BASIS_POINTS = 10000;
    uint256 public protocolBasicPoint = 500;
    address payable private communityWallet;

    /**
     * @dev Distribute Fee for protocol Fee address (_comunityWallet)
     *                 Fee for creator      address (_creator)
     *
     *                 amount that user sent uint256 (_amount)
     *
     */
    ...
    function _distributeFee(
        ...
        (uint256 feeForProtocol, uint256 feeForCreator) = _calculateFee(
            _amount
        );
        _sendValueWithFallbackWithdrawWithMediumGasLimit(
            payable(_comunityWallet),
            feeForProtocol
        );
        _sendValueWithFallbackWithdrawWithMediumGasLimit(
            payable(_creator),
    ...
    /**
     * @dev Function For calculation Fee
     */
    ...
    {
        uint256 feeForProtocol = (_amount * protocolBasicPoint) / BASIS_POINTS;
        uint256 feeForCreator = _amount - feeForProtocol;
        return (feeForProtocol, feeForCreator);
    }

    /**
     * @dev Function For set Protocol Fee
     * allow to change fee for Protocol
     */

    function setProtocolFee(uint256 _protocolBasicPoint)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(
            _protocolBasicPoint <= 10000,
            "ERC721MintFee: Royalty can not be more than 100%"
        );

        protocolBasicPoint = _protocolBasicPoint;
    }

    /**
     * @dev Function for calculate Protocol Fee
     */

    function _calculateProtocolFees(uint256 amount)
        public
        view
        returns (uint256)
    {
        require(
            protocolBasicPoint <= 500,
            "NFTMarketFees: marketplace Fees > 5%"
        );
        return (amount.mul(protocolBasicPoint) / BASIS_POINTS);
    }

    /**
     * @dev Function to set new Community wallet
     *
     *    Required:
        ...
        nonReentrant
    {
        require(
            ...
            "NFTMarketFees: Only admin can change community Wallet"
        );
    ...
    /**
     * @dev Function to get Community wallet
     */
    function getCommunityWallet() public view returns (address payable) {
